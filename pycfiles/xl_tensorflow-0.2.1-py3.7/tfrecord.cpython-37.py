...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xl_tensorflow\models\vision\detection\utils\tfrecord.py
# Compiled at: 2020-04-26 04:59:12
# Size of source mod 2**32: 21115 bytes
import logging, os, xml, tensorflow as tf
from math import ceil
from xl_tool.xl_io import file_scanning, save_to_json
import threading, tensorflow_addons as tfa, random
coordinate_name = [
 'xmin', 'ymin', 'xmax', 'ymax']
...
def get_bndbox(xml_file):
    ...
    return (
     xmins, ymins, xmaxs, ymaxs, classes_text)
...
def image2tfexample(filename, label='', class_id=0):
    ...
    example = tf.train.Example(features=tf.train.Features(feature={'width':_int64_feature(width), 
...
def voc2tfexample(image_file, xml_file, label2id):
    ...
    image_array = tf.image.decode_image(image_bytes)
    height, width = image_array.shape[0:2]
    filename = tf.compat.as_bytes(os.path.basename(image_file))
    encoded_image_data = image_bytes
    ...
    classes = [label2id[name] for name in classes_text]
    classes_text = [tf.compat.as_bytes(name) for name in classes_text]
    example = tf.train.Example(features=tf.train.Features(feature={'image/height':_int64_feature(height), 
...
def vocs2tfrecord(image_path, xml_path, record_file, label2id, label2id_file=None, mul_thread=None):
    ...
    valid_files = set([os.path.basename(file).split('.')[0] for file in image_files]) & set([os.path.basename(file).split('.')[0] for file in xml_files])
    image_files = sorted([file for file in image_files if os.path.basename(file).split('.')[0] in valid_files], key=(lambda x: os.path.basename(x).split('.')[0]))
    xml_files = sorted([file for file in xml_files if os.path.basename(file).split('.')[0] in valid_files], key=(lambda x: os.path.basename(x).split('.')[0]))
    ...
    if not mul_thread or mul_thread < 2:
        ...
        for i in range(mul_thread):
            ...
            thread = threading.Thread(target=write_voc_tfrecord, args=(
             sub_record_file, sub_thread_files, label2id))
...
def images2tfrecord(root_path, record_file, label2id_file, classes=None, mul_thread=None):
    ...
    if not mul_thread or mul_thread < 2:
        ...
        for i in range(mul_thread):
            sub_thread_files = files[i * number:(i + 1) * number]
            sub_record_file = record_file + str(i)
            thread = threading.Thread(target=write_image_tfrecord, args=(
             sub_record_file, sub_thread_files, label2id))
            threads.append(thread)
...
def tf_image_augmentation(image, size, target_size=(224, 224), adjust_gamma=None, random_brightness=None, resize_method='bilinear', random_contrast=None, rotate=None, zoom_range=None, random_crop=None, random_flip_left_right=None, random_flip_up_down=None, keep_aspect=True, noise=None, random_aspect=False):
    """
    Args:
        image: image tensor
        size: (height,wight)
    ...
    if adjust_gamma:
        ...
        if noise:
            if type(noise) in (float, tf.float32):
            ...
            if zoom_range:
                zoom = random.uniform(1.0 - zoom_range, 1.0 + zoom_range) if type(zoom_range) == float else (random.uniform)(*zoom_range)
                zoom_size = (
                ...
                if zoom > 1.0:
                    image = tf.image.crop_to_bounding_box(image, (zoom_size[0] - size[0]) // 2, (zoom_size[1] - size[1]) // 2, size[0], size[1])
                else:
                    image = tf.image.pad_to_bounding_box(image, -(zoom_size[0] - size[0]) // 2, -(zoom_size[1] - size[1]) // 2, size[0], size[1])
        if random_crop:
            crop_size = (random_crop * tf.cast(size[0], tf.float32), random_crop * tf.cast(size[1], tf.float32), 3) if type(random_crop) in (float, tf.float32) else (
        ...
        if target_size:
            if random_aspect:
                ...
                image = tf.image.resize(image, target_size, method=resize_method) if not aspect else (tf.image.resize_with_pad)(image, *target_size)
            else:
                image = tf.image.resize(image, target_size, method=resize_method) if not keep_aspect else (tf.image.resize_with_pad)(image, *target_size)
    return tf.clip_by_value(image, 0, 1)


def image_from_tfrecord(tf_record_files, num_classes=6, batch_size=8, buffer_size=20000, num_parallel_calls=tf.data.experimental.AUTOTUNE, target_size=(224, 224), resize_method='bilinear', normalized_mean=0.0, normalized_std=255.0, adjust_gamma=None, random_brightness=None, random_contrast=None, rotate=None, zoom_range=None, random_crop=None, random_flip_left_right=None, random_flip_up_down=None, keep_aspect=True, noise=None, random_aspect=False):
    """load data from tfrecord"""

    def parse_map_function(eg):
        example = tf.io.parse_example(eg[tf.newaxis], {'image':tf.io.FixedLenFeature(shape=(), dtype=tf.string), 
         'class_id':tf.io.FixedLenFeature(shape=(), dtype=tf.int64), 
         'width':tf.io.FixedLenFeature(shape=(), dtype=tf.int64), 
         'height':tf.io.FixedLenFeature(shape=(), dtype=tf.int64)})
        image = (tf.cast(tf.io.decode_jpeg((example['image'][0]), channels=3), tf.float32) - normalized_mean) / normalized_std
    ...
    parsed_dataset = raw_dataset.map(parse_map_function, num_parallel_calls=num_parallel_calls).shuffle(buffer_size, reshuffle_each_iteration=True).batch(batch_size)
    return parsed_dataset


def image_from_tfrecord_hand(tf_record_files, num_classes=6, batch_size=8, buffer_size=20000, num_parallel_calls=tf.data.experimental.AUTOTUNE, target_size=(224, 224), resize_method='bilinear', normalized_mean=0.0, normalized_std=255.0, adjust_gamma=None, random_brightness=None, random_contrast=None, rotate=None, zoom_range=None, random_crop=None, random_flip_left_right=None, random_flip_up_down=None, keep_aspect=True, noise=None, random_aspect=False):
    """load data from tfrecord"""
...
if __name__ == '__main__':
    vocs2tfrecord('E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\1_真实场景\\0_已标框', 'E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\1_真实场景\\0_已标框',
      'E:\\Programming\\Python\\8_Ganlanz\\food_recognition\\dataset\\自建数据集\\1_真实场景\\fuck.tfrecord',
      {'bacon':0, 
     'broccoli':1,  'corn':2,  'corn kernels':3,  'hamburger':4,  'pizza':5,  'pork belly piece':6},
      mul_thread=3)