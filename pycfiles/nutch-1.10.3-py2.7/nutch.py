...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/nutch/nutch.py
# Compiled at: 2015-10-16 01:16:05
...
LegalJobs = [
 'INJECT', 'GENERATE', 'FETCH', 'PARSE', 'UPDATEDB', 'CRAWL']
...
class Server:
    ...
    def call(self, verb, servicePath, data=None, headers=JsonAcceptHeader, forceText=False):
        ...
        if content_type == 'application/json' and not forceText:
            ...
            return resp.json()
        if content_type == 'text/plain' or forceText:
            if Verbose:
...
def main(argv=None):
    ...
    for opt, val in opts:
        if opt in ('-h', '--help'):
            ...
            serverEndpoint = 'http://localhost:%s' % val
        elif opt in ('-m', '--mock'):
            Mock = 1
