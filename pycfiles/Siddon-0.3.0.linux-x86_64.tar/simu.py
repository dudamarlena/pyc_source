...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/siddon/simu.py
# Compiled at: 2011-01-11 11:47:19
"""
Regroup functions to easily perform simulations.
"""
import copy, numpy as np, siddon, fitsarray as fa
default_image_dict = {'NAXIS': 2, 'NAXIS1': 1, 'NAXIS2': 1, 'BITPIX': -64, 
   'SIMPLE': True, 'CRPIX1': 0.5, 
   'CRPIX2': 0.5, 'CDELT1': 1.0, 'CDELT2': 1.0, 'LON': 0.0, 
   'LAT': 0.0, 'ROL': 0.0, 'D': 1.0, 
   'M1': 1.0, 'M2': 0.0, 'M3': 0.0}
image_keys = default_image_dict.keys()
default_image = fa.fitsarray_from_header(default_image_dict)
default_object_dict = {'NAXIS': 3, 'NAXIS1': 1, 'NAXIS2': 1, 'NAXIS3': 1, 'BITPIX': -64, 
   'SIMPLE': True, 'CRPIX1': 0.5, 
   'CRPIX2': 0.5, 'CRPIX3': 0.5, 'CRVAL1': 0.0, 
   'CRVAL2': 0.0, 'CRVAL3': 0.0, 'CDELT1': 1.0, 
   'CDELT2': 1.0, 'CDELT3': 1.0}
...
class Image(fa.InfoArray):
    ...
    def __new__(subtype, shape, data=None, dtype=float, buffer=None, offset=0, strides=None, order=None, header=None):
        if header is None:
            header = copy.copy(default_image_dict)
        obj = fa.InfoArray.__new__(subtype, shape=shape, data=data, dtype=dtype, buffer=buffer, offset=offset, strides=strides, order=order, header=header)
        for k in image_keys:
...
class Object(fa.FitsArray):
    ...
    def __new__(subtype, shape, dtype=float, buffer=None, offset=0, strides=None, order=None, header=None):
        if header is None:
        ...
        obj = fa.FitsArray.__new__(subtype, shape, dtype=dtype, buffer=buffer, offset=offset, strides=strides, order=order, header=header)
        return obj
