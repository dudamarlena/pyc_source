...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/siddon/siddon.py
# Compiled at: 2011-02-08 10:17:15
"""
...
"""
import numpy as np, time, copy, os, fitsarray as fa
from parse_templates import siddon_dict_list, suffix_str, ctypes_inv, obstacles_inv
c_methods = [
 'conic_full_projector', 'conic_image_projector', 'ray_projector',
...
def projector(data, cube, mask=None, obstacle=None, nthread=0):
    ...
    my_siddon_dict = {'ctype': ctypes_inv[data.dtype.name], 'obstacle': obstacles_inv[obstacle], 
       'pj': 'pj'}
...
def C_full_unit_vector(data):
    ...
    my_siddon_dict = {'ctype': ctypes_inv[data.dtype.name], 'obstacle': 'none', 
       'pj': 'pj'}
...
def dataarray_from_header(header):
    ...
    """
    shape = [ int(header[0][('NAXIS' + str(i + 1))]) for i in xrange(int(header[0]['NAXIS']))
            ]
    shape += (len(header),)
...
def centered_cubic_map_header(pshape, shape, dtype=np.float64):
    ...
    if pshape.size == 1:
        ...
        if pshape.size == 2 or pshape.size > 3:
        ...
        for i in xrange(3):
            ...
            header['CRPIX' + str(i + 1)] = shape[i] / 2.0
            header['CDELT' + str(i + 1)] = pshape[i] / float(shape[i])
            header['CRVAL' + str(i + 1)] = 0.0

    map_borders(header)
    return header


def centered_cubic_map(pshape, shape, fill=0.0, dtype=np.float64):
    """
    Generate a centered cubic map header
...
def centered_image(pshape, shape, fill=0.0, dtype=np.float64):
    """
    Generate a centered cubic map.
...
def centered_stack(pshape, shape, n_images=1.0, radius=1.0, min_lon=0.0, max_lon=np.pi, fill=0.0, dtype=np.float64):
    """
...
def full_unit_vector(data):
    ...
    for i, h in enumerate(data.header):
        ...
        L, G = np.meshgrid(l, g)
        u2[(Ellipsis, 0)] = np.cos(L) * np.cos(G)
        u2[(Ellipsis, 1)] = np.cos(L) * np.sin(G)
        u2[(Ellipsis, 2)] = np.sin(L)
...
def apply_rotation(R, u2):
    ...
    u0[(Ellipsis, 0)] = R[(0, 0)] * u2[(Ellipsis, 0)] + R[(0, 1)] * u2[(Ellipsis, 1)] + R[(0,
                                                                                           2)] * u2[(Ellipsis,
                                                                                                     2)]
    u0[(Ellipsis, 1)] = R[(1, 0)] * u2[(Ellipsis, 0)] + R[(1, 1)] * u2[(Ellipsis, 1)] + R[(1,
                                                                                           2)] * u2[(Ellipsis,
                                                                                                     2)]
    u0[(Ellipsis, 2)] = R[(2, 0)] * u2[(Ellipsis, 0)] + R[(2, 1)] * u2[(Ellipsis, 1)] + R[(2,
                                                                                           2)] * u2[(Ellipsis,
                                                                                                     2)]
    return u0
...
def full_intersect_cube(a1, an):
    ...
    amin = max3(Imin[(Ellipsis, 0)], Imin[(Ellipsis, 1)], Imin[(Ellipsis, 2)])
    amax = min3(Imax[(Ellipsis, 0)], Imax[(Ellipsis, 1)], Imax[(Ellipsis, 2)])
    return (amin, amax)
...
def initialize_raytracing(data, obj, u, p, a1, amin):
    ...
    del e
    next = iv + (update + 1) / 2.0
    next[update == 0.0] = np.inf
    D = np.empty(u.shape)
    for k in xrange(3):
