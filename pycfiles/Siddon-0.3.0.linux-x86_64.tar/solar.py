...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/siddon/solar.py
# Compiled at: 2011-01-12 09:10:11
"""
Generic code for WCS compatible data.
"""
import os, time, copy, pyfits, numpy as np, fitsarray as fa
solar_radius = 695000000.0
arcsecond_to_radian = np.pi / 648000
...
def convert_time(time_str):
    ...
    dpos = time_str.rfind('.')
    if dpos == -1:
        time_str, sec_float = time_str, 0.0
    else:
...
def filter_files(files, instrume=None, obsrvtry=None, detector=None, time_window=None, time_step=None):
    out_files = list()
...
def define_data_mask(data, data_rmin=None, data_rmax=None, ring=None, mask_negative=False, mask_nan=True, **kwargs):
    """
...
def compute_rsun(data):
    ...
    for i in xrange(data.shape[(-1)]):
        d = data.header[i]['D']
        rsun1[i] = np.arctan(1.0 / d) / data.header[i]['CDELT1']
        rsun2[i] = np.arctan(1.0 / d) / data.header[i]['CDELT2']
