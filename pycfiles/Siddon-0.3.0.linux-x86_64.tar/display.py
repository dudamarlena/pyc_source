...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/siddon/display.py
...
def data_movie(data, fig=None, **kwargs):
    ...
    a = fig.gca()
    n = data.shape[(-1)]
    im0 = a.imshow(data[(Ellipsis, 0)].T, **kwargs)
    plt.draw()
    for k in xrange(n):
...
def sinogram(data, r, amin=-np.pi, amax=np.pi, n=None, fig=None, **kwargs):
    ...
    for i in xrange(data.shape[(-1)]):
        a0 = a - np.radians(data[i].header.get('SC_ROLL', 0.0)[i])
        r0 = r * rsun[i] / 2
        x = r0 * np.cos(a0) + data[i].header.get('CRPIX1', 0.0)[i]
        y = r0 * np.sin(a0) + data[i].header.get('CRPIX2', 0.0)[i]
...
def equirectangular(obj, r=None, nlon=360.0, nlat=180.0, maxlat=np.pi, **kwargs):
    """
    Equirectangular projection.
    """
    lon = 2 * np.pi * np.linspace(0, 1, nlon)
    lat = np.pi * np.linspace(0, 1, nlat) - np.pi / 2.0
...
def gnomonic(obj, r=None, nlon=360.0, nlat=180.0, maxlat=3 * np.pi / 4.0, pole='north', **kwargs):
    """
    Gnomonic projection.
    """
    convert_pole = {'north': 1, 'south': -1}
    sign = convert_pole[pole]
    x = maxlat * np.linspace(-0.5, 0.5, obj.shape[0])
    y = maxlat * np.linspace(-0.5, 0.5, obj.shape[1])
    X, Y = np.meshgrid(x, y)
    lon = np.arctan2(Y, X)
    lat = sign * (np.pi / 2.0 - np.sqrt(X ** 2 + Y ** 2))
    x, y, z = sphe2cart(r, lon, lat)
    x, y, z = phy2pix(obj.header, (x, y, z))
    return map_coordinates(obj, (x, y, z))
...
def orthographic(obj, r=None, **kwargs):
    ...
    """
    x = np.linspace(-0.5, 0.5, obj.shape[0])
    y = np.linspace(-0.5, 0.5, obj.shape[1])
    X, Y = np.meshgrid(x, y)
    Z2 = r ** 2 - X ** 2 - Y ** 2
    Z = np.sqrt(Z2)
    X, Y, Z = phy2pix(obj.header, (X, Y, Z))
...
_map_projections = [
 equirectangular, gnomonic, orthographic]
...
def display_surface(obj, proj, title='', xlabel='', ylabel='', imshow_kwargs={}, **kwargs):
    """
