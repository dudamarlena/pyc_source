...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/of/common/messaging/websocket.py
# Compiled at: 2016-09-15 19:52:05
# Size of source mod 2**32: 7669 bytes
"""
...
from of.common.logging import write_to_log, EC_COMMUNICATION, SEV_DEBUG, SEV_ERROR, EC_INTERNAL, SEV_INFO, make_sparse_log_message
...
class OptimalWebSocket(object):
    ...
    def init(self, _session_id):
        ...
        try:
            ...
            raise Exception(write_to_log('OptimalWebSocket: Exception registering web socket:' + str(e), _category=EC_COMMUNICATION, _severity=SEV_ERROR))
    ...
    def write_dbg_info(self, _data):
        write_to_log(self.log_prefix + str(_data), _category=EC_COMMUNICATION, _severity=SEV_DEBUG, _process_id=self.process_id)

    def start_monitoring_message_queue(self):
        """
        Start monitoring the message queue
        """
        self.monitor_message_queue_thread = threading.Thread(target=self.monitor_message_queue, name=self.address + ' message queue monitor thread')
    ...
    def received_message(self, message):
        ...
        if str(message) != '':
            ...
            self.send_message(reply_with_error_message(_runtime_instance=os.getpid(), _error_message='Cannot send empty messages to agent', _message={}))
    ...
    def monitor_message_queue(self):
        ...
        while not self.monitor_message_queue_thread.terminated:
            try:
                _message = self.message_queue.get(True, 0.1)
                try:
                    self.send_message(_message)
                except Exception as e:
                    write_to_log(self.log_prefix + 'Error sending message:' + str(e) + '\nTraceback:' + traceback.format_exc(), _category=EC_COMMUNICATION, _severity=SEV_ERROR)
        ...
        self.write_dbg_info(' stopped message queue monitoring. Exiting thread "' + str(self.monitor_message_queue_thread.name) + '"')
    ...
    def send_message(self, message):
        ...
        if of.common.logging.severity < SEV_INFO:
            print(make_sparse_log_message('Sending message:' + str(message), _category=EC_COMMUNICATION, _severity=SEV_DEBUG, _address=self.address_own, _process_id=self.process_id))
    ...
    def closed(self, code, reason=None):
        ...
        if code == ABNORMAL_CLOSE:
            write_to_log(self.log_prefix + 'The connection to ' + self.address + ' has been abnormally closed.', _category=EC_COMMUNICATION, _severity=SEV_ERROR)
    ...
    def error_handler(self, exception):
        ...
        """
        write_to_log(self.log_prefix + 'An exception handle in the socket, closing. Error: ' + str(exception), _category=EC_COMMUNICATION, _severity=SEV_ERROR)
