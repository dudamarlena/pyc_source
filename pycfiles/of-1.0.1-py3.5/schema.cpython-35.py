...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/of/schemas/schema.py
# Compiled at: 2016-10-11 09:00:41
...
class SchemaTools:
    ...
    def __init__(self, _json_schema_folders=[], _uri_handlers=None):
        ...
        write_to_log('Schemas loaded and resolved: ' + str.join(', ', ['"' + _curr_schema['title'] + '"' for _curr_schema in self.json_schema_objects.values()]), _category=EC_NOTIFICATION, _severity=SEV_DEBUG)
    ...
    def check_schema_fields(_curr_schema_obj, _curr_file):
        ...
        def raise_field_error(_collection):
            raise Exception('Schematools.check_schema_fields: The "' + _collection + '"' + ' field is not in the schema-"' + _curr_file + '"')
    ...
    def load_schema_from_file(self, _file_name):
        ...
        try:
            ...
            raise Exception('load_schema_from_file: Error loading "' + _file_name + '": ' + str(e))

        try:
            _json_schema_obj = json.load(_curr_file)
        except Exception as e:
            raise Exception('load_schema_from_file: Error parsing "' + _file_name + '"' + str(e))

        _curr_file.close()
        try:
            self.check_schema_fields(_json_schema_obj, _file_name)
        except SchemaError as scherr:
            raise Exception('load_schema_from_file: SchemaError in ' + _file_name + ' at path:' + str(scherr.path) + '\nMessage:\n' + str(scherr.message))
        except Exception as e:
