...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/of/common/plugins.py
# Compiled at: 2016-12-25 11:27:08
...
class CherryPyPlugins(object):
            ...
            if 'hooks_instance' in _curr_plugin:
                ...
                if hasattr(_hooks_instance, _hook_name):
                    try:
                        ...
                        write_to_log('An error occurred ' + 'Calling ' + _hook_name + ' in ' + _curr_plugin_name + ':' + str(e), _category=EC_SERVICE, _severity=SEV_ERROR)
                        if 'failOnError' in _curr_plugin and _curr_plugin['failOnError']:
                            write_to_log('Setting ' + _curr_plugin_name + ' as Failed. No more hooks will be called for this plugin.', _category=EC_SERVICE, _severity=SEV_INFO)
                            _curr_plugin['failed'] = True
                        else:
                            write_to_log('Ignores error, this plugin will continue to attempt initialization.', _category=EC_SERVICE, _severity=SEV_INFO)
    ...
    def load_plugin(self, _plugins_dir, _plugin_name):
        ...
        try:
            ...
            if not os.path.exists(_definitions_filename):
                raise Exception('Error loading plugin ' + _dirname + ', definition file (' + _definitions_filename + ') missing.')
            ...
            if 'namespaces' in _definitions_data:
                ...
                self.schema_tools.resolver.handlers.update({_curr_namespace:self.uri_handler for _curr_namespace in _definitions_data['namespaces']})
            ...
            write_to_log('An error occurred importing ' + _plugin_name + ':' + str(e), _category=EC_SERVICE, _severity=SEV_ERROR)
            write_to_log('Setting ' + _plugin_name + ' as Failed. No dependent plugins will be loaded.', _category=EC_SERVICE, _severity=SEV_INFO)
        ...
        if 'hooks_module' in _plugin_data:
            ...
            try:
                ...
                write_to_log('An error occurred importing ' + _hooks_modulename + ' in ' + _plugin_data['description'] + ':' + str(e), _category=EC_SERVICE, _severity=SEV_ERROR)
                if 'failOnError' in _plugin_data and _plugin_data['failOnError']:
                    write_to_log('Setting ' + _hooks_modulename + ' as Failed. No hooks will be called for this plugin.', _category=EC_SERVICE, _severity=SEV_INFO)
                    _plugin_data['failed'] = True
                else:
                    write_to_log('Ignores error, the plugin will continue to attempt initialization.', _category=EC_SERVICE, _severity=SEV_INFO)
    ...
    def refresh_plugins(self, _plugins_folder):
        ...
        for _plugin_name in _plugin_names:
            if os.path.isdir(os.path.join(_plugins_folder, _plugin_name)) and _plugin_name[0:2] != '__' and _plugin_name[0] != '.':
                self.load_plugin(_plugins_folder, _plugin_name)
                self.write_debug_info('Loaded plugin ' + _plugin_name)

        self.write_debug_info('Schemas in ' + str(', ').join([_curr_plugin['description'] for _curr_plugin in self.plugins.values()]) + ' loaded and resolved:  ' + str.join(', ', ['"' + _curr_schema['title'] + '"' for _curr_schema in self._unresolved_schemas.values()]))
