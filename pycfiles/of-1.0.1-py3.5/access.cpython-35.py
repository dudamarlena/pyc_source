...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/of/broker/lib/access.py
# Compiled at: 2016-09-15 19:16:39
    ...
    def verify_document(self, _data, _collection_name, _caller_name):
        ...
        if 'schemaRef' in _data:
            try:
                ...
                raise Exception(_caller_name + ': Error - invalid data structure, schemaRef not found: ' + _data['schemaRef'])

            if _curr_schema['collection'] != _collection_name:
                raise Exception(_caller_name + ': Invalid schemaRef: "' + _curr_schema['collection'] + '". ' + _caller_name + ' is restricted to the "' + _collection_name + '"-collection.')
        else:
    ...
    def save(self, _document, _user, _old_document=None, _allow_save_id=False):
        ...
        if _old_document is None and '_id' in _document:
            ...
            if _old_document is not None and str(_old_document['schemaRef']) != str(_document['schemaRef']):
                raise Exception('Access.save: Cannot change schema of an existing node, remove and add. _id: ' + str(_document['_id']) + ', new:' + str(_old_document['schemaRef']) + ', old:' + str(_old_document['schemaRef']))
