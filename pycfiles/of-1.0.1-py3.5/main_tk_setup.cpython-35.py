...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/of/tools/setup/lib/main_tk_setup.py
# Compiled at: 2016-11-22 17:49:02
# Size of source mod 2**32: 14572 bytes
"""
...
from of.tools.setup.lib.widgets_misc import VerticalScrolledFrame, BaseFrame, TextExtension, Selector, Status_Bar, make_entry

class SetupMain(VerticalScrolledFrame):
    __doc__ = 'The main class for the GUI of the application'
    setup = None
    setup_filename = None
    fr_src_dataset = None
    ...
    def __init__(self, _setup=None, _setup_filename=None, *args, **kw):
        self.parent = Tk()
        super(SetupMain, self).__init__(self.parent, args, relief=SUNKEN, *bd=1, **kw)
    ...
    def init_GUI(self):
        ...
        self.install_location, self.l_install_location, self.e_install_location, self.b_install_location = make_entry(self.fr_settings, 'Install location:', 0, _button_caption='..')
        self.b_install_location.config(command=self.on_select_install_folder)
        self.plugins_folder, self.l_plugin_location, self.e_plugin_location, self.b_plugins_folder = make_entry(self.fr_settings, 'Plugins folder:', 1, _button_caption='..')
        self.b_plugins_folder.config(command=self.on_select_plugins_folder)
        self.install_repository_url, self.l_install_repository_url, self.e_install_repository_url = make_entry(self.fr_settings, 'Repository URL', 2)
        ...
        self.plugin_url, self.l_plugin_url, self.e_plugin_url = make_entry(self.fr_plugin, 'Repository URL(http):', 0)
    ...
    def on_save_json(self, *args):
        ...
        _filename = filedialog.asksaveasfilename(initialdir=os.path.dirname(self.setup_filename.get()), initialfile=os.path.basename(self.setup_filename.get()), defaultextension='.json', filetypes=[
         ('JSON files', '.json'), ('all files', '.*')], title='Choose location')
        if _filename:
            self.gui_to_setup()
    ...
    def on_load_json(self, *args):
        ...
        _setup_filename = filedialog.askopenfilename(defaultextension='.json', filetypes=[
         ('JSON files', '.json'), ('all files', '.*')], title='Choose file')
