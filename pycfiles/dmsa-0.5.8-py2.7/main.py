...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dmsa/main.py
...
def main():
    ...
    if args['ddl']:
        ...
        output = ddl.generate(args['<model>'], args['<model_version>'], args['<dialect>'], args['--tables'], args['--constraints'], args['--indexes'], args['--drop'], args['--delete-data'], args['--nologging'], args['--logging'], args['--service'])
        if args['--output']:
        ...
        return erd.write(args['<model>'], args['<model_version>'], args['--service'], args['--output'])
    if args['serve']:
