...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dmsa/service.py
# Compiled at: 2016-02-12 12:18:44
import os, sys
from flask import Flask, Response, request, send_file, render_template, redirect, url_for
from dmsa import ddl, erd, __version__
from dmsa.utility import PRETTY_MODELS, PRETTY_DIALECTS, get_model_json, get_service_version, get_template_models, get_template_dialects, ReverseProxied, dmsa_version
app = Flask('dmsa')
...
def version_route(model_name, version_name):
    ...
    return render_template('version.html', model=model, version=version, dialects=app.config['dialects'])
...
def ddl_route(model, version, dialect, elements):
    ...
    if elements in ('indexes', 'all'):
        indexes = True
    ddl_str = ddl.generate(model, version, dialect, tables=tables, constraints=constraints, indexes=indexes, service=app.config['service'])
...
def drop_route(model, version, dialect, elements):
    ...
    if elements in ('indexes', 'all'):
        indexes = True
    ddl_str = ddl.generate(model, version, dialect, tables=tables, constraints=constraints, indexes=indexes, drop=True, service=app.config['service'])
    resp = Response(ddl_str, status='200 OK', mimetype='text/plain')
    return resp
...
def delete_route(model, version, dialect):
    ddl_str = ddl.generate(model, version, dialect, delete_data=True, service=app.config['service'])
    resp = Response(ddl_str, status='200 OK', mimetype='text/plain')
    return resp
...
def create_erd_route(model, version):
    ...
    filename = ('{0}_{1}_dms_{2}_dmsa_{3}.{4}').format(model, version, app.config['service_version'], __version__, ext)
    ...
    return redirect(url_for('erd_route', model=model, version=version, filename=filename))
...
def logging_route(model, version, elements):
    ...
    if elements in ('indexes', 'all'):
        indexes = True
    ddl_str = ddl.generate(model, version, 'oracle', tables=tables, constraints=constraints, indexes=indexes, logging=True, service=app.config['service'])
    resp = Response(ddl_str, status='200 OK', mimetype='text/plain')
    return resp
...
def nologging_route(model, version, elements):
    ...
    if elements in ('indexes', 'all'):
        indexes = True
    ddl_str = ddl.generate(model, version, 'oracle', tables=tables, constraints=constraints, indexes=indexes, nologging=True, service=app.config['service'])
    resp = Response(ddl_str, status='200 OK', mimetype='text/plain')
    return resp
