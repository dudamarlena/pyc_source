...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dmsa/utility.py
# Compiled at: 2016-02-12 12:18:44
import os, requests
from functools import wraps
from flask import make_response
from dmsa import __version__
PRETTY_MODELS = {'i2b2': 'i2b2', 
   'i2b2_pedsnet': 'i2b2 for PEDSnet', 
   'omop': 'OMOP', 
   'pcornet': 'PCORnet', 
   'pedsnet': 'PEDSnet'}
PRETTY_DIALECTS = {'postgresql': 'PostgreSQL', 
   'oracle': 'Oracle', 
   'mssql': 'MS SQL Server', 
   'mysql': 'MySQL', 
   'sqlite': 'SQLite'}
RELEASE_COLORS = {'alpha': 'red', 
   'beta': 'orange', 
   'final': ''}
    ...
    for svc_model in svc_models:
        for model in models:
            if model['name'] == svc_model['name']:
                model['versions'].append({'name': svc_model['version'], 
                   'release_level': svc_model['release']['level'], 
                   'release_color': RELEASE_COLORS[svc_model['release']['level']]})
                break
        else:
            models.append({'pretty': PRETTY_MODELS.get(svc_model['name'], svc_model['name']), 
               'name': svc_model['name'], 
               'versions': [
...
def dmsa_version(f):
    ...
    return add_response_headers({'User-Agent': ('DMSA/{0} (+https://github.com/chop-dbhi/data-models-sqlalchemy)').format(__version__)})(f)
