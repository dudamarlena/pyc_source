...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dmsa/ddl.py
# Compiled at: 2016-02-12 12:18:44
from datetime import datetime
from sqlalchemy import create_engine, MetaData, Table, Column, Integer, Numeric, String, DateTime, text
from sqlalchemy.ext.compiler import compiles
from sqlalchemy.schema import CreateTable, AddConstraint, CreateIndex, DropTable, DropConstraint, DropIndex, ForeignKeyConstraint, CheckConstraint, UniqueConstraint, PrimaryKeyConstraint
...
def generate(model, model_version, dialect, tables=True, constraints=True, indexes=True, drop=False, delete_data=False, nologging=False, logging=False, service='http://data-models.origins.link/'):
    ...
    output = []
    INSERT = "INSERT INTO version_history (operation, model, model_version, dms_version, dmsa_version) VALUES ('{operation}', '" + model + "', '" + model_version + "', '" + service_version + "', '" + __version__ + "');\n\n"
    if dialect.startswith('oracle'):
        INSERT = INSERT + 'COMMIT;\n\n'
    version_history = Table('version_history', MetaData(), Column('datetime', DateTime(), primary_key=True, server_default=text('CURRENT_TIMESTAMP')), Column('operation', String(24)), Column('model', String(16)), Column('model_version', String(16)), Column('dms_version', String(16)), Column('dmsa_version', String(16)))
    version_tbl_ddl = str(CreateTable(version_history).compile(dialect=engine.dialect)).strip()
    if dialect.startswith('mssql'):
        version_tbl_ddl = "IF OBJECT_ID ('version_history', 'U') IS NULL " + version_tbl_ddl + ';'
    else:
        if dialect.startswith('oracle'):
            version_tbl_ddl = 'BEGIN\n' + "EXECUTE IMMEDIATE '" + version_tbl_ddl + "';\n" + 'EXCEPTION\n' + 'WHEN OTHERS THEN\n' + 'IF SQLCODE = -955 THEN NULL;\n' + 'ELSE RAISE;\n' + 'END IF;\nEND;\n/'
        else:
            version_tbl_ddl = version_tbl_ddl.replace('CREATE TABLE', 'CREATE TABLE IF NOT EXISTS')
...
def delete_ddl(tables, engine):
    output = []
    for table in tables:
        output.append(str(table.delete().compile(dialect=engine.dialect)).strip())
        output.append(';\n\n')
...
def constraint_ddl(tables, engine, drop=False):
    output = []
    for table in tables:
        constraints = sorted(list(table.constraints), key=lambda k: k.name, reverse=drop)
...
def index_ddl(tables, engine, drop=False):
    output = []
    for table in tables:
        indexes = sorted(list(table.indexes), key=lambda k: k.name, reverse=drop)
