...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/titan/tools/network.py
# Compiled at: 2014-10-16 21:36:56
"""
...
def parse_date(line):
    ...
    if ret:
        ...
        if ret.month > today.month or ret.month == today.month and ret.day > today.day:
            return
    return ret
...
def is_mac_addr(mac):
    ...
    if type(mac) != 'str':
        ...
        if re.match('([0-9A-F]{2}[:-]){5}([0-9A-F]{2})', mac, re.IGNORECASE):
            return True
        return False
