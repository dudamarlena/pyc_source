...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/titan/tools/data_science.py
# Compiled at: 2014-10-16 21:33:51
"""
...
class DataScience:
    ...
    def __init__(self, orm_object, new_data, tablename, key='name', all_data=None):
        self.orm = orm_object
        ...
        if self.new_data and self.all_data:
            for i in self.new_data:
                ...
                if data:
                    ...
                    del i_copy['date']
                    if i_copy != data_copy:
                        master = self.__master_string(self.tablename, i_copy[self.key])
                        data['date'] = i['date']
                        for key, value in i_copy.iteritems():
                            if i[key] != data[key]:
                                changed_field = key
                                diff_string = diff(str(i[key]), str(data[key]))
                                string = self.__diff_string(changed_field, i[key], data, key, diff_string)
                                data[key] = i[key]
                                master += string
