...
# [GCC 8.4.0]
# Embedded file name: titan/tools/filesystem.py
# Compiled at: 2014-10-16 21:35:21
"""
...
from titan.tools.system import execute_command as shell_out
...
def list_dirs_in_dir(directory):
    ...
    return [ directory + f for f in listdir(directory) if isdir(join(directory, f))
           ]
...
def hash_kext(kextfind, kext):
    ...
    for i in kextfind:
        ...
        path = join(ext_root, '%s.kext' % (kext,), 'Contents', 'MacOS', kext)
        if isfile(path):
            return hash_file(path)
        path = join(ext_root, 'Apple%s.kext' % (kext,), 'Contents', 'MacOS', 'Apple%s' % (kext,))
        if isfile(path):
            return hash_file(path)
        path = join(ext_root, '%s.kext' % (kext,), kext)
        if isfile(path):
            return hash_file(path)
        path = join('/System', 'Library', 'Filesystems', 'AppleShare', '%s.kext' % (kext,), 'Contents', 'MacOS', kext)
        if isfile(path):
            return hash_file(path)
...
def find_with_perms(directory, perms):
    ...
    for i, _, _ in walk(directory):
        if match('%s' % (perms,), oct(stat(i)[ST_MODE])[-3:]):
            files.append(i)
...
def list_current_host_pref_files():
    ...
    for home_dir in list_home_dirs():
        try:
            files += list_files_in_dir(home_dir + '/Library/Preferences/ByHost/')
        except OSError:
            pass
