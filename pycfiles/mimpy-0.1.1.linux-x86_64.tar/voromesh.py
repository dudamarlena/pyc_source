...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/mesh/voromesh.py
...
class VoroMesh(mesh.Mesh):
    ...
    def build_mesh(self, voro_file_name, K):
        ...
        for line in voro_file:
            ...
            vertices = [ np.array([ float(v) for v in vertex.split(',') ]) for vertex in vertices
                       ]
            faces = full_input[5]
            faces = faces.replace(')', '')
            faces = faces.replace('(', '')
            ...
            face_normals = face_normals.replace('(', '')
            face_normals = [ np.array([ float(n) for n in normal.split(',') ]) for normal in face_normals.split()
                           ]
            for face in faces:
                face.reverse()
            ...
            for local_face_index, global_cell_index in enumerate(neighbors):
                if global_cell_index >= 0:
                    if global_cell_index not in done_cells:
                        new_face_points = [ local_to_global_point_index[vertex_index] for vertex_index in faces[local_face_index]
                                          ]
                        points_to_add = []
                        for point in new_face_points:
                            too_close = False
                            for existing_point in points_to_add:
                                if np.linalg.norm(self.get_point(existing_point) - self.get_point(point)) < 1e-08:
                                    too_close = True
                     ...
                     global_cell_index, current_cell) in neighbor_to_face_hash:
                        current_cell_faces.append(neighbor_to_face_hash[(global_cell_index, current_cell)])
                        current_cell_orientations.append(-1)
                        if use_shifted_points:
                            self.set_face_shifted_centroid(neighbor_to_face_hash[(global_cell_index, current_cell)], (voronoi_point + self.get_cell_shifted_centroid(global_cell_index)) * 0.5)
                            shifted_point = (voronoi_point + self.get_cell_shifted_centroid(global_cell_index)) * 0.5
                else:
                    new_face_points = [ local_to_global_point_index[vertex_index] for vertex_index in faces[local_face_index]
                                      ]
                    points_to_add = []
                    for point in new_face_points:
                        too_close = False
                        for existing_point in points_to_add:
                            if np.linalg.norm(self.get_point(existing_point) - self.get_point(point)) < 1e-12:
                                too_close = True
