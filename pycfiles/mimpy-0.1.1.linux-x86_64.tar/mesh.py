...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/mesh/mesh.py
# Compiled at: 2016-03-31 16:42:47
""" Mesh module.
"""
...
import mimpy.mesh.mesh_cython as mesh_cython, mimpy
...
class variable_array:
    ...
    def add_entry(self, data):
        ...
        :param  dtype data: Generic data to be added. Usually        either scalar type (float, int ...) or ndarray type.
        ...
        if self.number_of_entries < len(self.pointers):
            ...
            self.pointers.resize((new_array_size, 2), refcheck=False)
            self.pointers[(self.number_of_entries, 0)] = self.next_data_pos
            self.pointers[(self.number_of_entries, 1)] = len(data)
        if len(self.data) > self.next_data_pos + len(data):
            ...
            if self.dim == 1:
                self.data.resize(self.new_size(len(self.data)), refcheck=False)
            else:
                self.data.resize((self.new_size(len(self.data)), self.dim), refcheck=False)
            self.data[(self.next_data_pos):(self.next_data_pos + len(data))] = data
...
class Mesh:
    ...
    def add_point(self, new_point):
        ...
        if self.number_of_points < len(self.points):
            ...
            new_array_size = (
             len(self.points) + len(self.points) / 2 + 1, 3)
    ...
    def add_face(self, list_of_points):
        ...
        :param list list_of_points: List of point indices             making up the new face.
        ...
        if self.has_face_shifted_centroid:
            if len(self.face_shifted_centroids) - 1 < new_face_index:
                new_size = self._memory_extension(len(self.face_shifted_centroids))
                self.face_shifted_centroids.resize((new_size, 3), refcheck=False)
    ...
    def remove_from_face_to_cell(self, face_index, cell_index):
        ...
        if self.face_to_cell[(face_index, 0)] == cell_index:
            ...
            raise Exception('cell_index ' + str(cell_index) + ' not found in face_to_cell for ' + str(face_index))
    ...
    def add_to_face_to_cell(self, face_index, cell_index):
        ...
        :param int cell_index: The cell index that will be associated        with the face.
        """
        if self.face_to_cell[(face_index, 0)] == -1:
            ...
            raise Exception('cell_index ' + str(cell_index) + ' could not be added to ' + str(face_index))
    ...
    def is_line_seg_intersect_face(self, face_index, p1, p2):
        ...
        d = np.dot(self.get_face_real_centroid(face_index) - p1, self.get_face_normal(face_index))
    ...
    def load_mesh(self, input_file):
        ...
        for line in input_file:
            ...
            if line_split[0] == 'POINTS':
                ...
                self.faces.data = np.loadtxt(islice(input_file, n_data_entries), dtype=np.dtype('i'))
                current_line = next(input_file)
                n_pointers = int(current_line)
                self.faces.pointers = np.loadtxt(islice(input_file, n_pointers), dtype=np.dtype('i'))
                ...
                number_of_faces = int(line_split[1])
                self.face_to_cell = np.loadtxt(islice(input_file, number_of_faces))
            elif line_split[0] == 'CELLS':
                number_of_cells = int(line_split[1])
                self.cells.number_of_entries = number_of_cells
                current_line = next(input_file)
                n_data_entries = int(current_line)
                self.cells.data = np.loadtxt(islice(input_file, n_data_entries), dtype=np.dtype('i'))
                current_line = next(input_file)
                n_pointers = int(current_line)
                self.cells.pointers = np.loadtxt(islice(input_file, n_pointers), dtype=np.dtype('i'))
            elif line_split[0] == 'CELL_NORMAL_ORIENTATION':
                number_of_cells = int(line_split[1])
                self.cell_normal_orientation.number_of_entries = number_of_cells
                current_line = next(input_file)
                n_data_entries = int(current_line)
                self.cell_normal_orientation.data = np.loadtxt(islice(input_file, n_data_entries), dtype=np.dtype('i'))
                current_line = next(input_file)
                n_pointers = int(current_line)
                self.cell_normal_orientation.pointers = np.loadtxt(islice(input_file, n_pointers), dtype=np.dtype('i'))
            elif line_split[0] == 'CELL_VOLUMES':
                number_of_entries = int(line_split[1])
                self.cell_volume = np.loadtxt(islice(input_file, number_of_entries))
            elif line_split[0] == 'CELL_REAL_CENTROIDS':
                number_of_entries = int(line_split[1])
                self.cell_real_centroid = np.loadtxt(islice(input_file, number_of_entries))
            elif line_split[0] == 'CELL_SHIFTED_CENTROIDS':
                number_of_entries = int(line_split[1])
                self.cell_shifted_centroid = np.loadtxt(islice(input_file, number_of_entries))
                ...
                for line_index in range(number_of_boundary_markers):
                    ...
                    while entries:
                        self.add_boundary_face(boundary_marker, entries.pop(0), entries.pop(0))
                ...
                for line_index in range(number_of_pointers):
                    ...
                    self.set_dirichlet_face_pointer(key, orientation, cell_index)
                ...
                for line_index in range(number_of_cells):
                    ...
                    self.set_forcing_pointer(cell_index, face_list, orientation_list)
                ...
                for line_index in range(number_of_pointers):
                    ...
                    orientation = int(line_split[2])
                    self.set_face_to_lagrange_pointer(face_index, orientation, lagrange_index)
                ...
                for line_index in range(number_of_pointers):
                    ...
                    orientation = int(line_split[2])
                    self.set_lagrange_to_face_pointers(lagrange_index, face_index, orientation)
    ...
    def save_cell(self, cell_index, output_file):
        ...
        for face_index, orientation in zip(self.get_cell(cell_index), self.get_cell_normal_orientation(cell_index)):
            current_face = []
            ...
            temp_mesh.set_face_normal(new_face_index, self.get_face_normal(face_index))
    ...
    def save_mesh(self, output_file):
        ...
        """
        output_file.write('this is just a test\n')
        ...
        output_file.write('CELL_NORMAL_ORIENTATION ')
        output_file.write(tb(str(self.get_number_of_cells()) + '\n'))
        ...
        for marker_index in self.boundary_markers:
            ...
            for face_index, face_orientation in self.get_boundary_faces_by_marker(marker_index):
                output_file.write(tb(str(face_index) + ' ' + str(face_orientation) + ' '))

            output_file.write('\n')

        output_file.write('DIRICHLET_BOUNDARY_POINTERS ')
        output_file.write(tb(str(len(list(self.dirichlet_boundary_pointers.keys()))) + '\n'))
        ...
        output_file.write('INTERNAL_NO_FLOW ')
        ...
        output_file.write('FORCING_FUNCTION_POINTERS ')
        ...
        output_file.write('FACE_TO_LAGRANGE_POINTERS ')
        ...
        output_file.write('LAGRANGE_TO_FACE_POINTERS ')
    ...
    def add_cell(self, list_of_faces, list_of_orientations):
        ...
        :param list list_of_orientations: List consisting of 1s and -1s            indicating whether normals are pointing out (1) or in (-1) of cell.
        ...
        for face_index in list_of_faces:
            if self.face_to_cell[face_index][0] == -1:
                ...
                raise Exception('setting face ' + str(face_index) + ' to cell ' + str(new_cell_index) + ' already set' + ' to two cells ' + str(self.face_to_cell[face_index]))
        ...
        if self.has_cell_shifted_centroid:
            if len(self.cell_shifted_centroid) - 1 < new_cell_index:
                new_size = self._memory_extension(len(self.cell_shifted_centroid))
    ...
    def get_cell_normal_orientation(self, cell_index):
        ...
        :return: List of faces orientations in cell. The        list is made up of 1s and -1s, 1 if the corresponding        face normal is pointing out of the cell, and -1 if the        corresponding face normal is pointing into the cell.
    ...
    def add_boundary_face(self, boundary_marker, face_index, face_orientation):
        ...
        :param  int face_orientation: Orientation of face normal             relative to the domain. (1) if pointing out, (-1) if             if pointing in.
    ...
    def set_boundary_faces(self, boundary_marker, face_orientation_list):
        ...
        :param list face_orienation_list: A list of tuples of the form             [face_index, orientation] to be associated with the             indicated boundary marker.
    ...
    def get_boundary_faces_by_marker(self, boundary_marker):
        ...
        :return: List of tupes [face_index, orientation] associated with             boundary_marker.
        :rtype: list
        """
    ...
    def add_internal_no_flow(self, face_index, face_orientation):
        ...
        :param int face_orientation: Orientation of face relative to the        domain, (1) for pointing out, (-1) for pointing in.
        """
    ...
    def set_dirichlet_face_pointer(self, face_index, face_orientation, cell_index):
    ...
    def set_lagrange_to_face_pointers(self, lagrange_index, face_index, orientation):
        """ Sets the lagrange multiplier to the source faces
        in order to impose zero flux across the boundary.
        """
        self.lagrange_to_face_pointers[lagrange_index] = zip(face_index, orientation)
    ...
    def set_periodic_boundary(self, face_index_1, face_orientation_1, face_index_2, face_orientation_2):
    ...
    def set_forcing_pointer(self, cell_index, face_indices, face_orientations):
        ...
        """
        self.forcing_function_pointers[cell_index] = list(zip(face_indices, face_orientations))
    ...
    def find_face_normal(self, face_index):
        ...
        for i in range(len(face)):
            ...
            if np.linalg.norm(new_face_normal) > 1e-10:
    ...
    def find_centroid_for_coordinates(self, face_index, coordinates):
        ...
        for index in range(len(current_face)):
            ...
            C_1 += (current_point[index_1] + next_point[index_1]) * (current_point[index_1] * next_point[index_2] - next_point[index_1] * current_point[index_2])
            C_2 += (current_point[index_2] + next_point[index_2]) * (current_point[index_1] * next_point[index_2] - next_point[index_1] * current_point[index_2])
    ...
    def find_face_centroid(self, face_index):
        ...
        assert np.linalg.norm(v2) > 1e-12
        assert np.linalg.norm(v1) > 1e-12
        ...
        polygon_projected = list(zip(polygon_projected_v1, polygon_projected_v2))
        area = self.compute_polygon_area(polygon_projected)
        centroid_x = 0.0
        centroid_y = 0.0
        N = len(polygon_projected)
        for i in range(N - 1):
            centroid_x += (polygon_projected[i][0] + polygon_projected[(i + 1)][0]) * (polygon_projected[i][0] * polygon_projected[(i + 1)][1] - polygon_projected[(i + 1)][0] * polygon_projected[i][1])
            centroid_y += (polygon_projected[i][1] + polygon_projected[(i + 1)][1]) * (polygon_projected[i][0] * polygon_projected[(i + 1)][1] - polygon_projected[(i + 1)][0] * polygon_projected[i][1])

        centroid_x += (polygon_projected[(N - 1)][0] + polygon_projected[0][0]) * (polygon_projected[(N - 1)][0] * polygon_projected[0][1] - polygon_projected[0][0] * polygon_projected[(N - 1)][1])
        centroid_y += (polygon_projected[(N - 1)][1] + polygon_projected[0][1]) * (polygon_projected[(N - 1)][0] * polygon_projected[0][1] - polygon_projected[0][0] * polygon_projected[(N - 1)][1])
    ...
    def compute_polygon_area(self, polygon, dims=[
 0, 1]):
        ...
        *dims* represents which two coordinates to use
        ...
        N = len(polygon)
        for i in range(N - 1):
            area += polygon[i][current_x] * polygon[(i + 1)][current_y] - polygon[(i + 1)][current_x] * polygon[i][current_y]

        area += polygon[(N - 1)][current_x] * polygon[0][current_y] - polygon[0][current_x] * polygon[(N - 1)][current_y]
        area *= 0.5
        return area
    ...
    def find_volume_centroid_all(self):
        ...
        zero3 = np.zeros(3)
        for cell_index in range(self.get_number_of_cells()):
            self.set_cell_volume(cell_index, 0.0)
            self.set_cell_real_centroid(cell_index, zero3)

        mesh_cython.all_cell_volumes_centroids(self.cells.pointers, len(self.cells), self.cells.data, self.cell_normal_orientation.data, self.points, self.cell_volume, self.cell_real_centroid, self.faces.pointers, len(self.faces), self.faces.data, self.face_normals, self.face_to_cell)
    ...
    def find_volume_from_faces(self, face_list, orientation_list):
        ...
        for face_index, face_orientation in zip(face_list, orientation_list):
            ...
            if abs(current_normal[0]) > abs(current_normal[1]) and abs(current_normal[0]) > abs(current_normal[2]):
                C = 0
            else:
                if abs(current_normal[1]) > abs(current_normal[2]):
                    C = 1
                else:
                    C = 2
                A = (C + 1) % 3
            ...
            Faa = k1 * Paa
            Fbb = k1 * Pbb
            Fcc = k3 * (current_normal[A] * current_normal[A] * Paa + 2 * current_normal[A] * current_normal[B] * Pab + current_normal[B] * current_normal[B] * Pbb + w * (2.0 * (current_normal[A] * Pa + current_normal[B] * Pb) + w * P1))
            if A == 0:
    ...
    def output_vector_field(self, file_name, vector_magnitudes=[], vector_labels=[]):
        ...
        for data_index in range(len(vector_labels)):
            ...
            for face_index in range(len(vector_magnitudes[data_index])):
                current_vector = vector_magnitudes[data_index][face_index] * self.get_face_normal(face_index)
    ...
    def output_vtk_faces(self, file_name, face_indices, face_values=[], face_value_labels=[]):
        ...
        for face_index in face_indices:
            total_polygon_points += self.get_number_of_face_points(face_index) + 1
    ...
    def subdivide_by_domain(self, cells):
        ...
        for face_index in lagrange_faces:
            ...
            self.set_face_real_centroid(new_face_index, self.get_face_real_centroid(face_index))
            self.set_face_real_centroid(lagrange_face_index, self.get_face_real_centroid(face_index))
            self.set_face_area(new_face_index, self.get_face_area(face_index))
            self.set_face_area(lagrange_face_index, self.get_face_area(face_index))
    ...
    def find_domain_faces(self, domain):
        ...
        for cell_index in self.get_cells_in_domain(domain):
            cell_cell_normal = zip(self.get_cell(cell_index), self.get_cell_normal_orientation(cell_index))
        ...
        return (
         boundary_faces, boundary_orientation)
    ...
    def construct_polygon_from_segs(self, segments):
        ...
        while len(current_segments) > 0:
            ...
            for index, segment in enumerate(current_segments):
                if np.linalg.norm(self.get_point(point_to_match) - self.get_point(segment[0])) < 1e-07:
                    new_face.append(segment[0])
                    to_be_removed = index
                    next_point_to_match = segment[1]
                    hits += 1
                elif np.linalg.norm(self.get_point(point_to_match) - self.get_point(segment[1])) < 1e-07:
        ...
        for face_index in current_cell:
            ...
            if len(new_face_2) > 0:
                ...
                if self.is_boundary_face(face_index, self.get_boundary_markers()):
                    boundary_marker = self.find_boundary_marker(face_index, self.get_boundary_markers())
                    self.add_boundary_face(boundary_marker, new_face_index, cell_orientations[local_face_index])
                self.set_cell_orientation(cell_index, np.array(list(cell_orientations) + [
                ...
                if len(cell_next_door) == 1:
                    ...
                    self.set_cell_orientation(cell_next_door[0], next_door_orientations)
    ...
    def build_frac_from_faces(self, faces, width=0.0001):
            ...
            for local_point_index in range(len(current_face_points) - 1):
                ...
                for local_face_index_2 in range(local_face_index + 1, len(faces)):
                    ...
                    for local_point_index_2 in range(len(current_face_points_2) - 1):
                        ...
                        if np.linalg.norm(abs(point_1 - point_1_2) + abs(point_2 - point_2_2)) < 1e-12:
                        ...
                        if np.linalg.norm(abs(point_2 - point_1_2) + abs(point_1 - point_2_2)) < 1e-12:
                            connections.append([local_face_index,
                             local_face_index_2,
                             local_point_index,
                             local_point_index + 1,
        ...
        for connection_index, connection in enumerate(connections):
            ...
            for remote_connection_index in range(connection_index + 1, len(connections)):
                if remote_connection_index not in multiple_connection_indices:
                    ...
                    if np.linalg.norm(abs(point1 - point1_remote) + abs(point2 - point2_remote)) < 1e-10 or np.linalg.norm(abs(point2 - point1_remote) + abs(point1 - point2_remote)) < 1e-10:
        ...
        for group in multiple_connection_groups:
            ...
            for connection in group:
                ...
                home_vector = current_centroid - (point1 + point2) / 2.0
                ...
                if (
                 current_face, 'TOP') not in done_faces:
                ...
                if (
                 current_face, 'BOT') not in done_faces:
        ...
        for connection in connections:
            ...
            for remote_connection in connections:
                if remote_connection[0] == face_1 and remote_connection[1] == face_2:
                    pass
                else:
        ...
        for connection in connections:
            ...
            top_points[connection[0]][connection[3] % len(self.get_face(faces[connection[0]]))] = point_3_index
            bot_points[connection[0]][connection[2]] = point_1_index
            bot_points[connection[0]][connection[3] % len(self.get_face(faces[connection[0]]))] = point_2_index
            if connection[6] == 0:
                top_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_4_index
                top_points[connection[1]][connection[4]] = point_3_index
                bot_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_1_index
                bot_points[connection[1]][connection[4]] = point_2_index
            else:
                bot_points[connection[1]][connection[4]] = point_4_index
                bot_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_3_index
                top_points[connection[1]][connection[4]] = point_1_index
                top_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_2_index
        ...
        for connection in subface_connections:
            ...
            if connection[7] == 'TOP':
                ...
                mid_points[connection[0]][connection[3] % len(self.get_face(faces[connection[0]]))] = point_2_index
                top_points[connection[0]][connection[3] % len(self.get_face(faces[connection[0]]))] = point_3_index
                top_points[connection[0]][connection[2]] = point_4_index
            elif connection[7] == 'BOT':
                bot_points[connection[0]][connection[2]] = point_1_index
                ...
                mid_points[connection[0]][connection[3] % len(self.get_face(faces[connection[0]]))] = point_3_index
            if connection[7] == 'TOP':
                if connection[6] == 0:
                    top_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_4_index
                    top_points[connection[1]][connection[4] % len(self.get_face(faces[connection[1]]))] = point_3_index
                    mid_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_1_index
                    mid_points[connection[1]][connection[4] % len(self.get_face(faces[connection[1]]))] = point_2_index
                else:
                    bot_points[connection[1]][connection[4]] = point_4_index
                    bot_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_3_index
                    mid_points[connection[1]][connection[4]] = point_1_index
                    mid_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_2_index
            if connection[7] == 'BOT':
                if connection[6] == 0:
                    mid_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_4_index
                    mid_points[connection[1]][connection[4]] = point_3_index
                    bot_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_1_index
                    bot_points[connection[1]][connection[4]] = point_2_index
                else:
                    mid_points[connection[1]][connection[4]] = point_4_index
                    mid_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_3_index
                    top_points[connection[1]][connection[4] % len(self.get_face(faces[connection[1]]))] = point_1_index
                    top_points[connection[1]][connection[5] % len(self.get_face(faces[connection[1]]))] = point_2_index
            new_faces.append(new_face_index)
        ...
        for local_face_index, face in enumerate(non_connected_edges):
            for point1 in face:
                ...
                if self.has_face_shifted_centroid:
                    self.set_face_shifted_centroid(new_face_index, centroid)
                self.set_face_area(new_face_index, area)
                if np.dot(current_face_normal, centroid - self.get_face_real_centroid(global_face_index)) > 0.0:
        ...
        for face in face_to_walls:
            ...
            if self.face_to_cell[(top_res_face_index, 0)] >= 0 and self.face_to_cell[(top_res_face_index, 1)] >= 0:
                ...
                self.set_face_real_centroid(bot_res_face_index, self.get_face_real_centroid(top_res_face_index))
                if self.has_face_shifted_centroid:
                    self.set_face_shifted_centroid(bot_res_face_index, self.get_face_real_centroid(top_res_face_index))
                ...
                top_res_face_orientation = self.get_cell_normal_orientation(top_cell_index)[local_top_face_index_in_cell]
            ...
            new_cell_index = self.add_cell(array.array('i', [ x[0] for x in face_to_walls[face] ]), array.array('i', [ x[1] for x in face_to_walls[face] ]))
            ...
            self.set_cell_k(new_cell_index, np.eye(3) * 1.0)
            ...
            self.set_dirichlet_face_pointer(top_res_face_index, top_res_face_orientation, new_cell_index)
            self.set_dirichlet_face_pointer(bot_res_face_index, -top_res_face_orientation, new_cell_index)
