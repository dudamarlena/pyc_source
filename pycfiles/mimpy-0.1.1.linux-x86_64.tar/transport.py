...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/models/transport.py
# Compiled at: 2016-03-31 16:42:47
from __future__ import absolute_import
from __future__ import print_function
import numpy as np
from scipy import sparse, diag, integrate, special
import math, scipy.sparse.linalg.dsolve as dsolve, os
...
class Transport:
    ...
    def initialize_system(self):
        ...
        self.div = sparse.coo_matrix((np.array(div_data),
         (
          np.add(np.array(div_row), -self.mesh.get_number_of_faces()),
          np.array(div_col))))
    ...
    def update_pressure(self):
        ...
        for cell_index in range(self.mesh.get_number_of_cells()):
            ...
            rhs_current[(self.mesh.get_number_of_faces() + cell_index)] += c_entry * self.current_pressure[cell_index]
            self.lhs_coo.data[self.c_start + cell_index] = c_entry

        for index, cell_index in enumerate(self.rate_wells):
            rhs_current[(self.mesh.get_number_of_faces() + cell_index)] += self.rate_wells_rate[index]
    ...
    def find_upwinding_direction(self):
        ...
        for cell_index in range(self.mesh.get_number_of_cells()):
            for face_index, orientation in zip(self.mesh.get_cell(cell_index), self.mesh.get_cell_normal_orientation(cell_index)):
        ...
        for face_index in self.mesh.get_dirichlet_pointer_faces():
            ...
            if self.current_velocity[face_index] * orientation < 0.0:
                self.upwinded_face_cell.append([face_index, cell_index])
