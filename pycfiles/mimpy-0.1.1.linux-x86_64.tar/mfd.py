...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/mfd/mfd.py
# Compiled at: 2016-03-31 16:42:47
from __future__ import absolute_import
from __future__ import print_function
import numpy as np, sys, copy, array, mimpy.mfd.mfd_cython as mfd_cython
from scipy import sparse, diag
import scipy.sparse.linalg.dsolve as dsolve, scipy.sparse.linalg as linalg
...
class MFD:
    ...
    def renumber_for_neumann(self):
        ...
        """
        index_offset = 0
        self.face_to_flux = np.zeros(shape=(self.mesh.get_number_of_faces(), 1), dtype=np.dtype('i'))
    ...
    def build_div_full(self, shift=1):
        ...
        for current_cell_index in range(self.mesh.get_number_of_cells()):
            ...
            current_cell_orientations = self.mesh.get_cell_normal_orientation(current_cell_index)
            neumann_faces = self.get_cell_faces_neumann(current_cell_index)
            for face_index, face_orientation in zip(current_cell, current_cell_orientations):
                new_entry = face_orientation * self.mesh.get_face_area(face_index)
                div_data.append(new_entry)
                div_row.append(current_cell_index + shift * self.mesh.get_number_of_faces())
                div_col.append(face_index)
                if face_index not in neumann_faces:
                    div_t_data.append(-new_entry)
                    div_t_row.append(face_index)
                    div_t_col.append(current_cell_index + shift * self.mesh.get_number_of_faces())
    ...
    def build_n_e(self, cell_index, k_unity=False):
        ...
        counter = 0
        face_face_orientation = list(zip(self.mesh.get_cell(cell_index), self.mesh.get_cell_normal_orientation(cell_index)))
        if k_unity:
            ...
            for face_index, face_orientation in face_face_orientation:
                n_e[counter, :] = np.dot(self.mesh.get_cell_k(cell_index), self.mesh.get_face_normal(face_index))
                n_e[counter, :] *= face_orientation
                counter += 1
    ...
    def build_m_e(self, cell_index, k_unity=False):
        ...
        if self.m_e_construction_method == 0:
            ...
            u = 0.0
            for face_index in self.mesh.get_cell(cell_index):
                x_e_min_x_E = self.mesh.get_face_real_centroid(face_index) - self.mesh.get_cell_real_centroid(cell_index)
            ...
            diagonal = []
            face_orientation_list = zip(self.mesh.get_cell(cell_index), self.mesh.get_cell_normal_orientation(cell_index))
            for face_index, orientation in face_orientation_list:
                ...
                entry *= np.linalg.norm(self.mesh.get_face_real_centroid(face_index) - self.mesh.get_cell_real_centroid(cell_index))
                diagonal.append(entry)

            diagonal = np.diag(diagonal)
            m_e += diagonal.dot(c_e.dot(c_e.T))
        if self.check_m_e:
            if np.linalg.norm(np.dot(m_e, n_e) - r_e) > 1e-06:
                print('M_E N ne R', np.linalg.norm(np.dot(m_e, n_e) - r_e))
        if self.compute_diagonality:
            self.diagonality_index_list[cell_index] = np.linalg.norm(m_e - np.diag(np.diag(m_e))) / np.linalg.norm(m_e)
            if self.diagonality_index_list[cell_index] > 1e-08:
            ...
            for i in range(len(m_e)):
                ...
                if global_i not in neumann_faces:
                    for j in range(len(m_e)):
                        ...
                        if abs(m_e[i][j]) / m_e_norm > 1e-12:
                            current_length += 1
                            m_data.append(m_e[(i, j)] * current_orientation[i] * current_orientation[j])
    ...
    def proj_vector(self, vector_f):
        ...
        for cell_index in range(self.mesh.get_number_of_cells()):
            vector_p.append([])
            face_orient = zip(self.mesh.get_cell(cell_index), self.mesh.get_cell_normal_orientation(cell_index))
    ...
    def update_m(self, m_coo, multipliers):
        ...
        """
        mfd_cython.update_m_fast(m_coo, multipliers, self.m_data_for_update, self.m_e_locations, self.mesh.get_number_of_cells())
    ...
    def build_bottom_right(self, alpha=0.0, shift=1):
        ...
        if self.mesh.is_using_alpha_list:
            for cell_index in range(self.mesh.get_number_of_cells()):
                entry = self.mesh.get_alpha_by_cell(cell_index) * self.mesh.get_cell_volume(cell_index)
        ...
        return [
         bottom_right_data, bottom_right_row, bottom_right_col]
    ...
    def build_coupling_terms(self):
        ...
        for periodic in self.mesh.periodic_boundaries:
            ...
            coupling_data.append(self.mesh.get_face_area(face_index_1) * face_orientation_1)
            coupling_row.append(self.face_to_flux[face_index_1])
            coupling_col.append(self.face_to_flux[lagrange_index_1])
            coupling_data.append(self.mesh.get_face_area(face_index_2) * face_orientation_2)
    ...
    def add_gravity(self):
        ...
        for face_index in range(self.mesh.get_number_of_faces()):
            ...
            gravity_force[face_index] *= np.dot(self.mesh.get_gravity_vector(), self.mesh.get_face_normal(face_index))
    ...
    def build_rhs_forcing(self):
        ...
        """
        for cell_index in range(self.mesh.get_number_of_cells()):
            self.rhs[(cell_index + self.flux_dof)] += self.cell_forcing_function[cell_index]
    ...
    def apply_neumann_from_function(self, boundary_marker, grad_u):
        ...
        for boundary_index, boundary_orientation in self.mesh.get_boundary_faces_by_marker(boundary_marker):
            ...
            if cell_index in self.cell_faces_neumann:
                ...
                self.cell_faces_neumann[cell_index] = [
                 boundary_index]
            self.neumann_needs_updating = True
    ...
    def apply_dirichlet_from_function(self, boundary_marker, p_function):
        ...
        for boundary_index, boundary_orientation in self.mesh.get_boundary_faces_by_marker(boundary_marker):
            ...
            self.dirichlet_boundary_values[boundary_index] = dirichlet_value * boundary_orientation
    ...
    def get_analytical_pressure_solution(self, p_function, quadrature_method=0):
        ...
        if quadrature_method == 0:
            p_array = [ p_function(point) for point in self.mesh.get_all_cell_real_centroids() ]
        elif quadrature_method == 1:
            p_array = [ p_function(point) for point in self.mesh.get_all_cell_shifted_centroids() ]
        return p_array
    ...
    def l2_error_velocity_per_cell(self, grad_p):
        ...
        for cell_index in range(self.mesh.get_number_of_cells()):
            ...
            for face_index in self.mesh.get_cell(cell_index):
                ...
                current_center = self.mesh.get_face_real_centroid(face_index)
                exact_flux = np.dot(grad_p(current_center), current_normal)
                error_flux_numerator += self.mesh.get_cell_volume(cell_index) * (self.solution[face_index] - exact_flux) ** 2
                error_flux_denominator += self.mesh.get_cell_volume(cell_index) * exact_flux ** 2
    ...
    def compute_x_error_velocity(self, grad_p):
        ...
        for face_index in range(self.mesh.get_number_of_faces()):
            current_centroid = self.mesh.get_face_real_centroid(face_index)
            current_normal = self.mesh.get_face_normal(face_index)
            grad_p_i_min_v_h[face_index] = np.dot(grad_p(current_centroid), current_normal)
    ...
    def compute_l2_error_pressure(self, p_function, quadrature_method=0):
        ...
        for cell_index in range(self.mesh.get_number_of_cells()):
            if quadrature_method == 0:
                ...
                exact_pressure = p_function(current_center)
            error_numerator += self.mesh.get_cell_volume(cell_index) * (self.solution[(self.mesh.get_number_of_faces() + cell_index)] - exact_pressure) ** 2
            error_denominator += self.mesh.get_cell_volume(cell_index) * exact_pressure ** 2
    ...
    def solve(self, initial_guess=0.0, solver='spsolve'):
        ...
        if solver == 'spsolve':
            ...
            self.lhs = self.lhs.tocsc()
            self.solution, converged = linalg.gmres(self.lhs, self.rhs, tol=1e-08)
            if converged > 0:
                print('did not converge after', converged)
        return self.solution

    def cg_inner(self, apply_lhs, b, tol=1e-08):
    ...
    def solve_divided(self):
        ...
        def apply_lhs(x):
            return -self.div.dot(self.cg_inner(self.m.dot, self.div_t.dot(x), tol=1e-16))
        ...
        current_v = -self.cg(self.m.dot, self.div_t.dot(current_p), tol=1e-10) + f1_tilde
    ...
    def lhs_rank(self):
        ...
        return len([ x for x in np.linalg.svd(lhs)[1] if abs(x) > 1e-09 ])
