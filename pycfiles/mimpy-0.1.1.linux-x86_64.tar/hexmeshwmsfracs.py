...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/mesh/hexmeshwmsfracs.py
# Compiled at: 2016-03-31 16:42:47
import numpy as np, math, array
...
def three_d_line_line_intersection(a1, a2, b1, b2):
    ...
    if np.linalg.norm(a_intersect - b_intersect) > 1e-05:
        return (None, [-1, -1])
    else:
        return (
         a_intersect, ts)
...
class FracData():
    ...
    def output_vtk(self, file_name):
        ...
        print >> output, 'POINTS', len(self.points), 'float'
        for point_index in range(len(self.points)):
            point = self.points[point_index]
            print >> output, point[0], point[1], point[2]

        print >> output, 'CELLS', 1
        print >> output, len(self.points) + 1
        print >> output, len(self.points),
        for point_index in self.face:
            print >> output, point_index,

        print >> output, '\n'
        print >> output, 'CELL_TYPES', 1
        print >> output, 7
        output.close()
    ...
    def generate_polygon(self, level):
        ...
        print >> poly_out, self.center[0], self.center[1], self.center[2]
        print >> poly_out, self.center[0] + frac_normal[0],
        print >> poly_out, self.center[1] + frac_normal[1],
        print >> poly_out, self.center[2] + frac_normal[2]
        print >> poly_out, '\n'
    ...
    def is_point_on_frac(self, p):
        ...
        az_rot = np.array([[np.cos(theta), np.sin(theta)],
         [
          -np.sin(theta), np.cos(theta)]])
        if abs(p_prime[2]) < 1e-08:
    ...
    def get_normal(self):
        theta = np.pi / 2.0 - self.azimuth
        phi = self.dip
        normal = np.array([np.sin(phi) * np.cos(theta),
    ...
    def line_frac_boundary_intercept(self, p1, p2):
        ...
        for frac_point in self.face[1:] + [self.face[0]]:
            ...
            ts = np.linalg.lstsq(np.array([p1 - p2, -p2_frac + p1_frac]).T, p1_frac - p1)[0]
            if 0.0 < ts[0] < 1.0:
                print >> self.gnu_out, p1[0], p1[1], p1[2]
                print >> self.gnu_out, p2[0], p2[1], p2[2]
                print >> self.gnu_out, '\n'
                print >> self.gnu_out, p1_frac[0], p1_frac[1], p1_frac[2]
                print >> self.gnu_out, p2_frac[0], p2_frac[1], p2_frac[2]
                print >> self.gnu_out, '\n'
...
class HexMeshWMSFracs(HexMesh):
    ...
    def build_mesh(self, ni, nj, nk, K, dim_x, dim_y, dim_z, modification_function=lambda x, i, j, k: x):
        HexMesh.build_mesh(self, ni, nj, nk, K, dim_x, dim_y, dim_z, modification_function)
        self.intersecting_cells = [
         0] * self.get_number_of_cells()
    ...
    def add_point_to_other_faces(self, faces, point):
        ...
        for face_index in faces:
            ...
            for point_index in current_face:
                ...
                if np.linalg.norm(vec2) > 1e-08 and np.linalg.norm(vec2) < length and np.linalg.norm(point - next_point) > 1e-08 and abs(np.linalg.norm(vec2) - np.dot(vec2, vec1)) < 1e-08:
    ...
    def construct_polygon_from_segments(self, segments):
        ...
        while len(current_segments) > 0:
            ...
            for index, segment in enumerate(current_segments):
                if np.linalg.norm(self.get_point(point_to_match) - self.get_point(segment[0])) < 1e-08:
                    new_face.append(segment[0])
                    to_be_removed = index
                    next_point_to_match = segment[1]
                    hits += 1
                elif np.linalg.norm(self.get_point(point_to_match) - self.get_point(segment[1])) < 1e-08:
    ...
    def add_fracture_meshes(self, fracture_mesh):
        ...
        for face_index in range(fracture_mesh.get_number_of_faces()):
            ...
            res_face = [ frac_point_to_res_point[point_index] for point_index in frac_face ]
            new_face_index = self.add_face(res_face)
            frac_face_to_res_face[face_index] = new_face_index
            self.set_face_real_centroid(new_face_index, fracture_mesh.get_face_real_centroid(face_index))
            self.set_face_area(new_face_index, fracture_mesh.get_face_area(face_index))
            self.set_face_normal(new_face_index, fracture_mesh.get_face_normal(face_index))
        ...
        for cell_index in range(fracture_mesh.get_number_of_cells()):
            ...
            self.set_cell_real_centroid(new_cell_index, fracture_mesh.get_cell_real_centroid(cell_index))
            self.set_cell_volume(new_cell_index, fracture_mesh.get_cell_volume(cell_index))
            ...
            for frac_cell_index in fracture_mesh.fracture_id_to_cell[fracture_id]:
                ...
                if fracture_mesh.is_point_on_fracture_face(frac_cell_index, centroid):
                    ...
                    if len(self.get_face_to_cell(face_index)) == 2:
                        bot_res_face_index = self.add_face(list(self.get_face(face_index)))
                        self.set_face_area(bot_res_face_index, self.get_face_area(face_index))
                        self.set_face_normal(bot_res_face_index, self.get_face_normal(face_index))
                        self.set_face_real_centroid(bot_res_face_index, self.get_face_real_centroid(face_index))
                        if self.has_face_shifted_centroid:
                            self.set_face_shifted_centroid(bot_res_face_index, self.get_face_real_centroid(face_index))
                        bottom_cell = self.get_face_to_cell(face_index)[1]
                        new_cell_faces = array.array('i', self.get_cell(bottom_cell))
                        local_face_index_in_cell = list(new_cell_faces).index(face_index)
                        new_cell_faces[local_face_index_in_cell] = bot_res_face_index
                        top_cell_index = self.get_face_to_cell(face_index)[0]
                        local_top_face_index_in_cell = list(self.get_cell(top_cell_index)).index(face_index)
                    ...
                    if frac_cell_to_res_face.has_key(cell_index):
                        frac_cell_to_res_face[cell_index] += [face_index, bot_res_face_index]
                        frac_cell_to_res_orientation[cell_index] += [top_res_face_orientation,
                         -top_res_face_orientation]
                    else:
                        frac_cell_to_res_face[cell_index] = [
                         face_index, bot_res_face_index]
                        frac_cell_to_res_orientation[cell_index] = [top_res_face_orientation,
                         -top_res_face_orientation]
                    self.set_dirichlet_face_pointer(face_index, top_res_face_orientation, cell_index)
                    self.set_dirichlet_face_pointer(bot_res_face_index, -top_res_face_orientation, cell_index)

        for cell_index in frac_cell_to_res_face:
            self.set_forcing_pointer(cell_index, frac_cell_to_res_face[cell_index], frac_cell_to_res_orientation[cell_index])

        fracture_boundary_marker = self.create_new_boundary_marker('fracture_boundary')
        for frac_face_index, orientation in fracture_mesh.get_boundary_faces_by_marker(0):
            face_index = frac_face_to_res_face[frac_face_index]
            self.add_boundary_face(fracture_boundary_marker, face_index, orientation)

        self.apply_neumann_from_function(fracture_boundary_marker, lambda x: np.array([0.0, 0.0, 0.0]))
        self.output_vtk_faces('internal_faces_', list_of_faces, [
         list_of_cells], [
         'CELLS'])
    ...
    def add_fractures(self, fracture, segments=[]):
                    ...
                    if abs(denom) < 1e-10:
                        ...
                        if d <= length + 1e-08 and d > 1e-08:
                            ...
                            if fracture.is_point_on_frac(intersection_point):
                                ...
                                print >> gnu_2, intersection_point[0], intersection_point[1], intersection_point[2]
                            else:
                                print >> gnu_1, intersection_point[0], intersection_point[1], intersection_point[2]
        ...
        for cell_index in cells_to_divide:
            ...
            faces_left = filter(lambda x: x not in done_faces, faces_left)
            for face_index in faces_left:
                ...
                if len(new_face_2) > 0:
                    ...
                    for point in new_face_1 + [new_face_1[0]]:
                        ...
                        print >> polygon_out, current_point[0], current_point[1], current_point[2]
                    ...
                    if self.face_to_fracture.has_key(face_index):
                        self.face_to_fracture[new_face_index] = self.face_to_fracture[face_index]
                        self.fracture_to_face[self.face_to_fracture[face_index]].append(new_face_index)
            ...
            if len(interior_face_segments) > 0:
                ...
                has_intersection, poly_1_list, poly2 = self.get_fracture_intersection_diff(fracture, new_face)
                new_face = [
                 new_face]
                if has_intersection:
                    new_face = []
                    for poly in poly_1_list:
                        new_face.append([])
                        for point in poly:
                            print >> clipping_out, point[0], point[1], point[2]
                    ...
                    for poly in poly2:
                        new_face_2 = []
                        for point in poly:
                            print >> clipping_2_out, point[0], point[1], point[2]
                            new_point_index = self.add_point(point)
                            new_face_2.append(new_point_index)

                        print >> clipping_2_out, '\n'
                        new_face_index_2 = self.add_face(new_face_2)
                        new_faces_2_list.append(new_face_index_2)
                        done_faces.append(new_face_index_2)
                        face_area, face_centroid = self.find_face_centroid(new_face_index_2)
                ...
                for point_index in self.get_face(new_face_index):
                    self.add_point_to_other_faces(faces_for_cell_1, self.get_point(point_index))
                    self.add_point_to_other_faces(faces_for_cell_2, self.get_point(point_index))
                ...
                for face_index in faces_for_cell_2:
                    if has_intersection:
                        if face_index != new_face_index and face_index not in new_faces_2_list:
                            self.remove_from_face_to_cell(face_index, cell_index)
                    else:
                        if face_index != new_face_index:
                            self.remove_from_face_to_cell(face_index, cell_index)
                        try:
                            pass
                        except:
                            print cell_index, self.get_face_to_cell(face_index), new_face_index_2, face_index
                            1 / 0
