...
# [GCC 8.4.0]
# Embedded file name: /opt/apps/ossw/applications/python/python-2.7.11/c7/lib/python2.7/site-packages/mimpy/models/twophase.py
...
import scipy.sparse.linalg.dsolve as dsolve, scipy.sparse.linalg as linalg, mimpy.mfd.mfd as mfd
...
class TwoPhase:
    ...
    def set_corey_relperm(self, n_o, n_w, k_rw_o=1.0):
        ...
        def krw(se):
            ...
            return_array += se
            return_array[return_array < 0.0] = 0.0
            return_array[return_array > 1.0] = 1.0
    ...
    def sefromsw(self, water_saturation):
        ...
        se /= 1.0 - self.residual_saturation_water - self.residual_saturation_oil
        return se
    ...
    def water_mobility(self, water_saturation, water_pressure):
        ...
        mobility *= 1.0 + self.compressibility_water * water_pressure
        return mobility
    ...
    def oil_mobility(self, water_saturation, oil_pressure):
        ...
        mobility *= 1.0 + self.compressibility_oil * oil_pressure
        return mobility
    ...
    def apply_pressure_boundary_from_function(self, boundary_marker, p_function):
        """ Applies static pressure (dirichlet) boundary
        conditions from function.
        """
        self.mfd.apply_dirichlet_from_function(boundary_marker, p_function)

    def apply_flux_boundary_from_function(self, boundary_marker, f_function):
        """ Applies static flux (neumann) boundary
        conditions from function.
        """
        self.mfd.apply_neumann_from_function(boundary_marker, f_function)
    ...
    def initialize_system(self):
        ...
        (div_data, div_row, div_col), (div_t_data, div_t_row, div_t_col) = self.mfd.build_div()
        print('building m')
        self.m_x_coo_data, m_x_coo_row, m_x_coo_col = self.mfd.build_m(save_update_info=True)
        ...
        self.div = sparse.coo_matrix((np.array(div_data),
         (
          np.add(np.array(div_row), -self.mfd.flux_dof),
          np.array(div_col))))
        ...
        self.newton_solution = np.zeros(self.mesh.get_number_of_cells() + self.mfd.flux_dof)
        if self.solver == 1:
            ...
            c_info = self.mfd.build_bottom_right(0.0, shift=0)
            ...
            self.m_petsc.createAIJWithArrays(size=m_csr.shape, csr=(
             m_csr.indptr,
             m_csr.indices,
             m_csr.data))
            self.m_petsc.assemblyBegin()
            self.m_petsc.assemblyEnd()
            self.div_coo = sparse.coo_matrix((div_info[0],
             (
              div_info[1], div_info[2])), shape=(
            ...
            self.div_petsc.createAIJWithArrays(size=(
             self.mesh.get_number_of_cells(),
             self.mfd.flux_dof), csr=(
             self.div_csr.indptr,
             self.div_csr.indices,
             self.div_csr.data))
            self.div_petsc.setUp()
            self.div_t_coo = sparse.coo_matrix((div_t_info[0],
             (
              div_t_info[1], div_t_info[2])), shape=(
             self.mfd.flux_dof,
             self.mesh.get_number_of_cells()))
            self.div_t_csr = self.div_t_coo.tocsr()
            self.div_t_petsc = PETSc.Mat()
            self.div_t_petsc.create(PETSc.COMM_WORLD)
            self.div_t_petsc.createAIJWithArrays(size=(
             self.mfd.flux_dof,
             self.mesh.get_number_of_cells()), csr=(
             self.div_t_csr.indptr,
             self.div_t_csr.indices,
             self.div_t_csr.data))
            self.div_t_petsc.setUp()
            self.c_coo = sparse.coo_matrix((c_info[0],
             (
              c_info[1], c_info[2])), shape=(
             self.mesh.get_number_of_cells(),
             self.mesh.get_number_of_cells()))
            c_csr = self.c_coo.tocsr()
            self.c_petsc = PETSc.Mat()
            self.c_petsc.create(PETSc.COMM_WORLD)
            self.c_petsc.createAIJWithArrays(size=(
             self.mesh.get_number_of_cells(),
             self.mesh.get_number_of_cells()), csr=(
             c_csr.indptr,
             c_csr.indices,
            ...
            self.schur_mat = SchurComplementMat(self.div_petsc, self.div_t_petsc, self.m_petsc, self.c_petsc)
            ...
            self.pc_petsc.createAIJWithArrays(size=(
             self.mesh.get_number_of_cells(),
             self.mesh.get_number_of_cells()), csr=(
             pc_matrix.indptr,
             pc_matrix.indices,
    ...
    def start_solving(self):
        ...
        for time_step in range(1, self.number_of_time_steps + 1):
            ...
            if time_step % self.prod_output_frequency == 0:
                for cell_index, output in zip(self.rate_wells, self.pressure_files):
    ...
    def update_pressure(self, time_step):
        ...
        while abs(newton_residual > self.newton_threshold):
            ...
            current_total_mobility = 1.0 / current_total_mobility
            current_c_matrix = self.ref_density_water * self.current_s_w
            current_c_matrix *= self.compressibility_water
            current_c_matrix += self.ref_density_oil * (self.compressibility_oil * (1.0 - self.current_s_w))
            current_c_matrix *= self.porosities
            current_c_matrix *= self.mesh.cell_volume[:self.mesh.get_number_of_cells()]
            current_c_matrix /= self.delta_t
            self.mfd.update_m(self.lhs_coo.data[:self.m_x_coo_length], current_total_mobility)
            for cell_index, pressure_pi in zip(self.pressure_wells, self.pressure_wells_pi):
                current_c_matrix[cell_index] += pressure_pi * 1.0 / current_total_mobility[cell_index]
            ...
            f2sum3_l += self.ref_density_water * (1.0 + self.compressibility_water * self.current_p_o)
            f2sum3_l *= self.current_s_w
            f2sum3_l += self.ref_density_oil * (1 + self.compressibility_oil * self.current_p_o) * (1.0 - self.current_s_w)
            ...
            for well_index, cell_index in enumerate(self.rate_wells):
                rhs[(cell_index + self.mfd.flux_dof)] += -self.get_well_rate_water(well_index)
                rhs[(cell_index + self.mfd.flux_dof)] += -self.get_well_rate_oil(well_index)

            for cell_index, bhp, pressure_pi in zip(self.pressure_wells, self.pressure_wells_bhp, self.pressure_wells_pi):
                rhs[(cell_index + self.mfd.flux_dof)] -= pressure_pi * bhp * 1.0 / current_total_mobility[cell_index]
            ...
            if newton_residual > self.newton_threshold:
                ...
                if self.solver == 1:
                    ...
                    f1_minvp.setArray(-rhs[:self.mfd.flux_dof] - self.div_t_coo.dot(y.getArray()))
    ...
    def find_upwinding_direction(self):
        ...
        for face_index in self.mesh.get_dirichlet_pointer_faces():
            ...
            if self.current_u_t[flux_index] * orientation < 0.0:
                self.upwinded_face_cell.append([flux_index, cell_index])
    ...
    def update_saturation(self, time_step):
        ...
        for face_index, cell_index in self.upwinded_face_cell:
            ...
            self.current_f_w[face_index] /= water_mob[cell_index] + oil_mob[cell_index]
        ...
        for boundary_marker in list(self.saturation_boundaries.keys()):
            ...
            for boundary_index, boundary_orientation in self.mesh.get_boundary_faces_by_marker(boundary_marker):
                flux_index = self.mfd.face_to_flux[boundary_index]
                if boundary_orientation * self.current_u_t[flux_index] < 0:
                    face_centroid = self.mesh.get_face_real_centroid(boundary_index)
                    current_saturation = saturation_function(face_centroid)
                    current_saturation = np.array([current_saturation])
                    current_pressure = np.array([self.mfd.get_dirichlet_value(boundary_index)])
                    self.current_f_w[flux_index] = self.water_mobility(current_saturation, current_pressure)
                    self.current_f_w[flux_index] /= self.water_mobility(current_saturation, current_pressure) + self.oil_mobility(current_saturation, current_pressure)
        ...
        new_s_w_l = previous_p_w * self.compressibility_water + 1.0
        new_s_w_l *= self.ref_density_water
        new_s_w_l *= self.current_s_w
        new_s_w_l /= self.ref_density_water
        new_s_w_l /= 1.0 + self.compressibility_water * current_p_w
        next_s_w += new_s_w_l
        new_s_w_l = 1.0 / self.porosities
        new_s_w_l /= self.ref_density_water
        new_s_w_l /= 1.0 + self.compressibility_water * current_p_w
        ...
        for cell_index in self.rate_wells:
            ...
            new_s_w /= 1.0 + self.compressibility_water * current_p_w[cell_index]
        ...
        for well_number in range(len(self.pressure_wells)):
            ...
            water_production /= 1.0 + self.compressibility_water * current_p_w[cell_index]
            ...
            oil_production /= 1.0 + self.compressibility_oil * self.current_p_o[cell_index]
