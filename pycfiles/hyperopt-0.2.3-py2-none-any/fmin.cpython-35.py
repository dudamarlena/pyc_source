...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/fmin.py
...
import functools, logging, os, sys, time
...
class FMinIter(object):
    __doc__ = 'Object for conducting search experiments.\n    '
    catch_eval_exceptions = False
    pickle_protocol = -1

    def __init__(self, algo, domain, trials, rstate, asynchronous=None, max_queue_len=1, poll_interval_secs=1.0, max_evals=sys.maxsize, verbose=0, show_progressbar=True):
        self.algo = algo
        self.domain = domain
        self.trials = trials
    ...
    def is_cancelled(self):
        ...
        """
        if hasattr(self.trials, '_fmin_cancelled') and self.trials._fmin_cancelled:
            return True
        return False
    ...
    def run(self, N, block_until_done=True):
        ...
        with std_out_err_redirect_tqdm() as (orig_stdout):
            with tqdm(total=N + qlen, file=orig_stdout, postfix='best loss: ?', disable=not self.show_progressbar, dynamic_ncols=True) as (pbar):
                while n_queued < N:
                    qlen = get_queue_len()
                    while qlen < self.max_queue_len and n_queued < N and not self.is_cancelled:
                        ...
                        self.trials.refresh()
                        new_trials = algo(new_ids, self.domain, trials, self.rstate.randint(2147483647))
                    ...
                    if self.asynchronous:
                        ...
                        self.serial_evaluate()
                    try:
                        best_loss = min([d['result']['loss'] for d in self.trials.trials if d['result']['status'] == 'ok'])
...
def fmin(fn, space, algo, max_evals, trials=None, rstate=None, allow_trials_fmin=True, pass_expr_memo_ctrl=None, catch_eval_exceptions=False, verbose=0, return_argmin=True, points_to_evaluate=None, max_queue_len=1, show_progressbar=True):
    ...
    if rstate is None:
        ...
        if allow_trials_fmin and hasattr(trials, 'fmin'):
            pass
        return trials.fmin(fn, space, algo=algo, max_evals=max_evals, max_queue_len=max_queue_len, rstate=rstate, pass_expr_memo_ctrl=pass_expr_memo_ctrl, verbose=verbose, catch_eval_exceptions=catch_eval_exceptions, return_argmin=return_argmin, show_progressbar=show_progressbar)
    else:
        if trials is None:
        ...
        rval = FMinIter(algo, domain, trials, max_evals=max_evals, rstate=rstate, verbose=verbose, max_queue_len=max_queue_len, show_progressbar=show_progressbar)
