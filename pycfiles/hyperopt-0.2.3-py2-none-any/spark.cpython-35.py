...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/spark.py
# Compiled at: 2019-10-29 06:30:14
# Size of source mod 2**32: 19347 bytes
from __future__ import print_function
import copy, numbers, threading, time, timeit
...
class SparkTrials(Trials):
    ...
    def __init__(self, parallelism=None, timeout=None, spark_session=None):
        ...
        if not _have_spark:
            raise Exception("SparkTrials cannot import pyspark classes.  Make sure that PySpark is available in your environment.  E.g., try running 'import pyspark'")
        if timeout is not None and (not isinstance(timeout, numbers.Number) or timeout <= 0 or isinstance(timeout, bool)):
            raise Exception('The timeout argument should be None or a positive value. Given value: {timeout}'.format(timeout=timeout))
        self._spark_context = SparkSession.builder.getOrCreate().sparkContext if spark_session is None else spark_session.sparkContext
        self._spark_supports_job_cancelling = hasattr(self._spark_context.parallelize([1]), 'collectWithJobGroup')
        ...
        if not self._spark_supports_job_cancelling and timeout is not None:
            logger.warning('SparkTrials was constructed with a timeout specified, but this Apache Spark version does not support job group-based cancellation. The timeout will be respected when starting new Spark jobs, but SparkTrials will not be able to cancel running Spark jobs which exceed the timeout.')
    ...
    def _decide_parallelism(max_num_concurrent_tasks, parallelism):
        ...
        if max_num_concurrent_tasks == 0:
            raise Exception('There are no available spark executors.  Add workers to your Spark cluster to use SparkTrials.')
        if parallelism is None:
            parallelism = max_num_concurrent_tasks
        else:
            if parallelism <= 0:
                logger.warning('User-specified parallelism was invalid value ({p}), so parallelism will be set to max_num_concurrent_tasks ({c}).'.format(p=parallelism, c=max_num_concurrent_tasks))
                parallelism = max_num_concurrent_tasks
            elif parallelism > max_num_concurrent_tasks:
                logger.warning('User-specified parallelism ({p}) is greater than max_num_concurrent_tasks ({c}), so parallelism will be set to max_num_concurrent_tasks.'.format(p=parallelism, c=max_num_concurrent_tasks))
                parallelism = max_num_concurrent_tasks
        if parallelism > SparkTrials.MAX_CONCURRENT_JOBS_ALLOWED:
            logger.warning('Parallelism ({p}) is being decreased to the hard cap of SparkTrials.MAX_CONCURRENT_JOBS_ALLOWED ({c})'.format(p=parallelism, c=SparkTrials.MAX_CONCURRENT_JOBS_ALLOWED))
    ...
    def count_total_trials(self):
        ...
        """
        total_states = [
         base.JOB_STATE_DONE, base.JOB_STATE_ERROR, base.JOB_STATE_CANCEL]
    ...
    def fmin(self, fn, space, algo, max_evals, max_queue_len, rstate, verbose, pass_expr_memo_ctrl, catch_eval_exceptions, return_argmin, show_progressbar):
        """
        ...
        try:
            try:
                res = fmin(fn, space, algo, max_evals, max_queue_len=max_queue_len, trials=self, allow_trials_fmin=False, rstate=rstate, pass_expr_memo_ctrl=None, catch_eval_exceptions=catch_eval_exceptions, verbose=verbose, return_argmin=return_argmin, points_to_evaluate=None, show_progressbar=show_progressbar)
            ...
            logger.info('Total Trials: {t}: {s} succeeded, {f} failed, {c} cancelled.'.format(t=self.count_total_trials(), s=self.count_successful_trials(), f=self.count_failed_trials(), c=self.count_cancelled_trials()))
...
class _SparkFMinState:
    ...
    def __init__(self, spark_context, eval_function, space, trials):
        ...
        if self.trials._spark_supports_job_cancelling:
            ...
            logger.debug('Job group id: {g}, job desc: {d}, job interrupt on cancel: {i}'.format(g=self._job_group_id, d=self._job_desc, i=self._job_interrupt_on_cancel))
    ...
    def _finish_trial_run(self, is_success, is_cancelled, trial, data):
        ...
        """
        if is_cancelled:
            logger.debug('trial task {tid} cancelled, exception is {e}'.format(tid=trial['tid'], e=str(data)))
            self._write_cancellation_back(trial, e=data)
        else:
            if is_success:
                logger.debug('trial task {tid} succeeded, result is {r}'.format(tid=trial['tid'], r=data))
                self._write_result_back(trial, result=data)
            else:
                logger.debug('trial task {tid} failed, exception is {e}'.format(tid=trial['tid'], e=str(data)))
    ...
    def launch_dispatcher(self):
        ...
        def run_dispatcher():
            ...
            while not self._fmin_done:
                ...
                if self.trials.timeout is not None and elapsed_time > self.trials.timeout and not self.trials._fmin_cancelled:
    ...
    def _run_trial_async(self, trial):
        ...
        def run_task_thread():
            ...
            try:
                ...
                if self.trials._spark_supports_job_cancelling:
                    result = worker_rdd.mapPartitions(run_task_on_executor).collectWithJobGroup(self._job_group_id, self._job_desc, self._job_interrupt_on_cancel)[0]
                else:
                    result = worker_rdd.mapPartitions(run_task_on_executor).collect()[0]
            except BaseException as e:
                self._finish_trial_run(is_success=False, is_cancelled=self.trials._fmin_cancelled, trial=trial, data=e)
                logger.debug('trial {tid} task thread catches an exception and writes the info back correctly.'.format(tid=trial['tid']))
            else:
                self._finish_trial_run(is_success=True, is_cancelled=self.trials._fmin_cancelled, trial=trial, data=result)
                logger.debug('trial {tid} task thread exits normally and writes results back correctly.'.format(tid=trial['tid']))
    ...
    def _cancel_running_trials(self):
        if self.trials._spark_supports_job_cancelling:
            logger.debug('Cancelling all running jobs in job group {g}'.format(g=self._job_group_id))
            ...
            logger.info('Because the current Apache PySpark version does not support cancelling jobs by job group ID, SparkTrials will block until all of its running Spark jobs finish.')
