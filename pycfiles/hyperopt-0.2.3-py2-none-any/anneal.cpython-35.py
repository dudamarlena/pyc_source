...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/anneal.py
# Compiled at: 2019-10-17 07:38:06
# Size of source mod 2**32: 14621 bytes
"""
...
from .pyll.stochastic import categorical, normal, lognormal, qnormal, qlognormal, uniform, loguniform, quniform, qloguniform
...
class AnnealingAlgo(SuggestAlgo):
    ...
    def __init__(self, domain, trials, seed, avg_best_idx=2.0, shrink_coef=0.1):
        ...
        self.node_tids, self.node_vals = miscs_to_idxs_vals([d['misc'] for tid, (d, l) in self.tid_docs_losses], keys=list(domain.params.keys()))
        self.best_tids = []
    ...
    def choose_ltv(self, label, size):
        ...
        return (
         picks_loss, picks_tids, picks_vals)
    ...
    def hp_uniform(self, memo, node, label, tid, val, log_scale=False, pass_q=False, uniform_like=uniform):
        """
        ...
        if pass_q:
            return uniform_like(low=clipped_midpt - half, high=clipped_midpt + half, rng=self.rng, q=memo[node.arg['q']], size=memo[node.arg['size']])
        else:
            return uniform_like(low=clipped_midpt - half, high=clipped_midpt + half, rng=self.rng, size=memo[node.arg['size']])

    def hp_quniform(self, *args, **kwargs):
        return self.hp_uniform(*args, **kwargs)
    ...
    def hp_normal(self, memo, node, label, tid, val):
        ...
        Parameters: See `hp_uniform`
        """
        return normal(mu=val, sigma=memo[node.arg['sigma']] * self.shrinking(label), rng=self.rng, size=memo[node.arg['size']])

    def hp_lognormal(self, memo, node, label, tid, val):
        """
        Parameters: See `hp_uniform`
        """
        return lognormal(mu=np.log(val), sigma=memo[node.arg['sigma']] * self.shrinking(label), rng=self.rng, size=memo[node.arg['size']])

    def hp_qlognormal(self, memo, node, label, tid, val):
        """
        Parameters: See `hp_uniform`
        """
        return qlognormal(mu=np.log(1e-16 + val), sigma=memo[node.arg['sigma']] * self.shrinking(label), q=memo[node.arg['q']], rng=self.rng, size=memo[node.arg['size']])

    def hp_qnormal(self, memo, node, label, tid, val):
        """
        Parameters: See `hp_uniform`
        """
        return qnormal(mu=val, sigma=memo[node.arg['sigma']] * self.shrinking(label), q=memo[node.arg['q']], rng=self.rng, size=memo[node.arg['size']])
