...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/vectorize.py
# Compiled at: 2019-10-17 07:38:06
# Size of source mod 2**32: 17283 bytes
...
def idxs_map(idxs, cmd, *args, **kwargs):
    ...
    for ii in idxs:
        ...
        try:
            kwargs_nn = dict([(kw, arg_imap[ii]) for kw, arg_imap in list(kwargs_imap.items())])
        except:
            ERR('args_nn %s' % cmd)
            ERR('ii %s' % ii)
            ERR('kw %s' % kw)
...
def vectorize_stochastic(orig):
    if orig.name == 'idxs_map' and orig.pos_args[1]._obj in stoch:
        ...
        def foo(arg):
            ...
            if arg_vals.name == 'idxs_take' and arg_vals.arg['vals'].name == 'asarray' and arg_vals.arg['vals'].inputs()[0].name == 'repeat':
...
class VectorizeHelper(object):
    ...
    def assert_integrity_idxs_take(self):
        ...
        for node in idxs_memo:
            ...
            for take in take_memo[node]:
                ...
                if not [idxs, vals] == take.pos_args[:2]:
    ...
    def idxs_by_label(self):
        return dict([(name, self.idxs_memo[node]) for name, node in list(self.params.items())])

    def vals_by_label(self):
        return dict([(name, self.take_memo[node][0].pos_args[1]) for name, node in list(self.params.items())])