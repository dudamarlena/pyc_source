...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/tests/test_domains.py
# Compiled at: 2019-10-16 09:30:13
...
def q1_lognormal():
    ...
    optimize a one-variable quadratic function.
    """
    return {'loss': scope.min(0.1 * (hp.lognormal('x', 0, 2) - 10) ** 2, 10), 
...
def branin():
    ...
    pi = float(np.pi)
    loss = (y - old_div(5.1, 4 * pi ** 2) * x ** 2 + 5 * x / pi - 6) ** 2 + 10 * (1 - old_div(1, 8 * pi)) * scope.cos(x) + 10
    return {'loss': loss, 
     'loss_variance': 0, 
     'status': base.STATUS_OK}
...
class DomainExperimentMixin(object):
    ...
    def test_basic(self):
        ...
        trials = Trials()
        fmin(lambda x: x, domain.expr, trials=trials, algo=suggest, max_evals=self._n_steps)
