...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/atpe.py
# Compiled at: 2019-10-17 07:38:06
# Size of source mod 2**32: 60266 bytes
"""
...
import re, functools, random, numpy, numpy.random, pkg_resources, tempfile, scipy.stats, os, math, hyperopt, datetime, json, copy
...
class Hyperparameter:
    __doc__ = ' This class represents a hyperparameter.'
        ...
        if 'anyOf' in self.config or 'oneOf' in self.config:
            ...
            for index, subParam in enumerate(data[1:]):
                ...
                if log10_cardinality - other_log10_cardinality > 3:
                    ...
                    if other_log10_cardinality - log10_cardinality > 3:
    ...
    def convertToFlatValues(self, params):
        ...
        def recurse(key, value, root):
            ...
            if isinstance(value, str):
                ...
                if isinstance(value, float) or isinstance(value, bool) or isinstance(value, int):
                    flatParams[result_key[1:]] = value
    ...
    def createHyperparameterConfigForHyperoptDomain(domain):
        ...
        if domain.name == 'qlognormal':
            ...
            data['rounding'] = domain.pos_args[2]._obj
            return data
        if domain.name == 'literal':
            data = {'type': 'string', 
             'constant': domain._obj}
            return data
        if domain.name == 'randint':
            data = {'type': 'number'}
...
class ATPEOptimizer:
    ...
    atpeParameterCascadeOrdering = [
     ...
     'gamma',
     'nEICandidates']
    atpeParameterValues = {'resultFilteringMode': ['age', 'loss_rank', 'none', 'random'], 
    ...
    def recommendNextParameters(self, hyperparameterSpace, results, currentTrials, lockedValues=None):
        rstate = numpy.random.RandomState(seed=int(random.randint(1, 4294967295)))
        params = {'param': {}}
            ...
            for atpeParamIndex, atpeParameter in enumerate(self.atpeParameterCascadeOrdering):
                ...
                for atpeParamFeature in atpeParamFeatures:
                    if atpeParamFeature == 'resultFilteringAgeMultiplier' and atpeParams['resultFilteringMode'] != 'age':
                        ...
                        if atpeParamFeature == 'secondaryTopLockingPercentile' and atpeParams['secondaryLockingMode'] != 'top':
                            vector.append(-3)
                        else:
                ...
                atpeParamDetails[atpeParameter] = {'value': None, 
                 'reason': None}
                if atpeParameter in self.atpeParameterValues:
                    ...
                    for atpeParamValueIndex, atpeParamValue in enumerate(self.atpeParameterValues[atpeParameter]):
                        value[atpeParamValueIndex] = max(value[atpeParamValueIndex], maxVal * 0.15)
        ...
        if atpeParams['resultFilteringMode'] == 'none':
            ...
            if atpeParams['resultFilteringMode'] == 'age':
                atpeParams['resultFilteringLossRankMultiplier'] = None
                atpeParams['resultFilteringRandomProbability'] = None
            else:
                if atpeParams['resultFilteringMode'] == 'loss_rank':
                    atpeParams['resultFilteringAgeMultiplier'] = None
                    atpeParams['resultFilteringRandomProbability'] = None
                else:
                    if atpeParams['resultFilteringMode'] == 'random':
                        atpeParams['resultFilteringAgeMultiplier'] = None
                        atpeParams['resultFilteringLossRankMultiplier'] = None
                ...
                def computePrimarySecondary():
                    if len(results) < initializationRounds:
                        return (parameters, [], [0.5] * len(parameters))
        ...
        hyperopt.fmin(fn=sample, space=Hyperparameter(hyperparameterSpace).createHyperoptSpace(lockedValues), algo=functools.partial(hyperopt.tpe.suggest, n_startup_jobs=initializationRounds, gamma=atpeParams['gamma'], n_EI_candidates=int(atpeParams['nEICandidates'])), max_evals=1, trials=self.convertResultsToTrials(hyperparameterSpace, filteredResults), rstate=rstate, show_progressbar=False)
    ...
    def chooseRandomValueForParameter(self, parameter):
        if parameter.config.get('mode', 'uniform') == 'uniform':
            ...
            if parameter.config.get('mode', 'uniform') == 'normal':
    ...
    def convertResultsToTrials(self, hyperparameterSpace, results):
        ...
        for resultIndex, result in enumerate(results):
            data = {'book_time': datetime.datetime.now(), 
             'exp_key': None, 
...
def suggest(new_ids, domain, trials, seed):
    ...
    for new_id in new_ids:
        ...
        rval_results = [
         domain.new_result()]
        rval_miscs = [
         dict(tid=new_id, cmd=domain.cmd, workdir=domain.workdir, idxs={key:[0] for key in flatParameters}, vals={key:[flatParameters[key]] for key in flatParameters})]
