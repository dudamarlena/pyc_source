...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/tpe.py
# Compiled at: 2019-10-17 07:38:06
# Size of source mod 2**32: 32007 bytes
"""
...
def GMM1_lpdf(samples, weights, mus, sigmas, low=None, high=None, q=None):
    ...
    if low is None and high is None:
        ...
        p_accept = np.sum(weights * (normal_cdf(high, mus, sigmas) - normal_cdf(low, mus, sigmas)))
    if q is None:
    ...
    if len(mus) == 0:
        ...
        if len(mus) == 1:
            ...
            if len(mus) >= 2:
                ...
                sigma[1:-1] = np.maximum(srtd_mus[1:-1] - srtd_mus[0:-2], srtd_mus[2:] - srtd_mus[1:-1])
        ...
        if not np.all(sigma > 0):
            raise AssertionError((sigma.min(), minsigma, maxsigma))
    srtd_weights /= srtd_weights.sum()
    return (
     srtd_weights, srtd_mus, sigma)
...
def ap_loguniform_sampler(obs, prior_weight, low, high, size=(), rng=None):
    prior_mu = 0.5 * (high + low)
    prior_sigma = 1.0 * (high - low)
    weights, mus, sigmas = scope.adaptive_parzen_normal(scope.log(obs), prior_weight, prior_mu, prior_sigma)
...
def ap_qloguniform_sampler(obs, prior_weight, low, high, q, size=(), rng=None):
    prior_mu = 0.5 * (high + low)
    prior_sigma = 1.0 * (high - low)
    weights, mus, sigmas = scope.adaptive_parzen_normal(scope.log(scope.maximum(obs, scope.maximum(EPS, scope.exp(low)))), prior_weight, prior_mu, prior_sigma)
...
def ap_loglognormal_sampler(obs, prior_weight, mu, sigma, size=(), rng=None):
    weights, mus, sigmas = scope.adaptive_parzen_normal(scope.log(obs), prior_weight, mu, sigma)
...
def ap_qlognormal_sampler(obs, prior_weight, mu, sigma, q, size=(), rng=None):
    ...
    weights, mus, sigmas = scope.adaptive_parzen_normal(log_obs, prior_weight, mu, sigma)
...
def ap_categorical_sampler(obs, prior_weight, upper, size=(), rng=None, LF=DEFAULT_LF):
    ...
    return scope.categorical(old_div(pseudocounts, scope.sum(pseudocounts)), upper=upper, size=size, rng=rng)
...
def ap_categorical_sampler(obs, prior_weight, p, upper=None, size=(), rng=None, LF=DEFAULT_LF):
    weights = scope.linear_forgetting_weights(scope.len(obs), LF=LF)
    counts = scope.bincount(obs, minlength=upper, weights=weights)
...
def build_posterior(specs, prior_idxs, prior_vals, obs_idxs, obs_vals, oloss_idxs, oloss_vals, oloss_gamma, prior_weight):
    """
    ...
    """
    assert all(isinstance(arg, pyll.Apply) for arg in [oloss_idxs, oloss_vals, oloss_gamma])
    expr = pyll.as_apply([specs, prior_idxs, prior_vals])
    nodes = pyll.dfs(expr)
    memo = {}
    obs_memo = {}
    for nid in prior_vals:
        obs_below, obs_above = scope.ap_filter_trials(obs_idxs[nid], obs_vals[nid], oloss_idxs, oloss_vals, oloss_gamma)
    ...
    for node in nodes:
        if node not in memo:
            ...
            if node in obs_memo:
                ...
                a_kwargs = dict([(n, a) for n, a in a_post.named_args if n not in ('rng',
                                                                                   'size')])
                b_kwargs = dict([(n, a) for n, a in b_post.named_args if n not in ('rng',
                                                                                   'size')])
...
def tpe_transform(domain, prior_weight, gamma):
    ...
    specs, idxs, vals = build_posterior(domain.vh.v_expr, domain.vh.idxs_by_label(), domain.vh.vals_by_label(), observed['idxs'], observed['vals'], observed_loss['idxs'], observed_loss['vals'], pyll.Literal(gamma), s_prior_weight)
    return (
     s_prior_weight, observed, observed_loss,
     specs, idxs, vals)


def suggest(new_ids, domain, trials, seed, prior_weight=_default_prior_weight, n_startup_jobs=_default_n_startup_jobs, n_EI_candidates=_default_n_EI_candidates, gamma=_default_gamma, linear_forgetting=_default_linear_forgetting):
    new_id = new_ids[0]
    t0 = time.time()
    ...
    o_idxs_d, o_vals_d = miscs_to_idxs_vals([d['misc'] for d in docs], keys=list(domain.params.keys()))
    memo[observed['idxs']] = o_idxs_d
    memo[observed['vals']] = o_vals_d
    memo[observed_loss['idxs']] = tids
    memo[observed_loss['vals']] = losses
    idxs, vals = pyll.rec_eval([opt_idxs, opt_vals], memo=memo, print_node_on_error=False)
    rval_specs = [
     None]
