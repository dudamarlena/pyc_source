...
# [GCC 8.4.0]
# Embedded file name: /home/max/code/oss/hyperopt/hyperopt/tests/test_tpe.py
# Compiled at: 2019-10-16 09:30:48
# Size of source mod 2**32: 23908 bytes
...
import os, unittest, nose, numpy as np
try:
...
import hyperopt.rand as rand, hyperopt.tpe as tpe, hyperopt.atpe as atpe
...
def test_adaptive_parzen_normal_orig():
    ...
    prior_sigma = 2
    mus = rng.randn(10) + 5
    weights2, mus2, sigmas2 = adaptive_parzen_normal_orig(mus, 3.3, prior_mu, prior_sigma)
    print(weights2)
    print(mus2)
    print(sigmas2)
...
class TestGMM1(unittest.TestCase):
    ...
    def test_mu_is_used_correctly(self):
        assert np.allclose(10, GMM1([1], [10.0], [1e-07], rng=self.rng))
    ...
    def test_mus_make_variance(self):
        samples = GMM1([0.5, 0.5], [0.0, 1.0], [1e-06, 1e-06], rng=self.rng, size=[1000])
        print(samples.shape)
        assert 0.45 < np.mean(samples) < 0.55, np.mean(samples)
        assert 0.2 < np.var(samples) < 0.3, np.var(samples)

    def test_weights(self):
        samples = GMM1([0.9999, 0.0001], [0.0, 1.0], [1e-06, 1e-06], rng=self.rng, size=[
         1000])
    ...
    def test_lpdf_scalar_one_component(self):
        llval = GMM1_lpdf(1.0, [
         ...
         2.0])
        assert llval.shape == ()
        assert np.allclose(llval, np.log(old_div(1.0, np.sqrt(2 * np.pi * 4.0))))
    ...
    def test_lpdf_scalar_N_components(self):
        llval = GMM1_lpdf(1.0, [
         ...
         1.0, 2.0, 5.0])
        print(llval)
        a = 0.25 / np.sqrt(2 * np.pi * 1.0) * np.exp(-0.5)
        a += old_div(0.25, np.sqrt(2 * np.pi * 4.0))
        a += 0.5 / np.sqrt(2 * np.pi * 25.0) * np.exp(-0.5 * old_div(1.0, 5.0) ** 2)

    def test_lpdf_vector_N_components(self):
        llval = GMM1_lpdf([1.0, 0.0], [
         0.25, 0.25, 0.5], [
         0.0, 1.0, 2.0], [
         1.0, 2.0, 5.0])
        a = 0.25 / np.sqrt(2 * np.pi * 1.0) * np.exp(-0.5)
        ...
        a = old_div(0.25, np.sqrt(2 * np.pi * 1.0))
        a += 0.25 / np.sqrt(2 * np.pi * 4.0) * np.exp(-0.5 * old_div(1.0, 2.0) ** 2)
        a += 0.5 / np.sqrt(2 * np.pi * 25.0) * np.exp(-0.5 * old_div(2.0, 5.0) ** 2)
...
class TestGMM1Math(unittest.TestCase):
    ...
    def work(self):
        self.worked = True
        kwargs = dict(weights=self.weights, mus=self.mus, sigmas=self.sigmas, low=self.low, high=self.high, q=self.q)
        samples = GMM1(rng=self.rng, size=(
...
class TestQGMM1Math(unittest.TestCase):
    ...
    def work(self, **kwargs):
        ...
        del kwargs
        self.worked = True
        gkwargs = dict(weights=self.weights, mus=self.mus, sigmas=self.sigmas, low=self.low, high=self.high, q=self.q)
        samples = old_div(GMM1(rng=self.rng, size=(
         self.n_samples,), **gkwargs), self.q)
    ...
    def test_bounded_3(self):
        self.work(weights=[
         0.14285714, 0.28571429, 0.28571429, 0.28571429], mus=[
         5.505, 7.0, 2.0, 10.0], sigmas=[
         8.99, 5.0, 8.0, 8.0], q=1, low=1.01, high=10, n_samples=10000)

    def test_bounded_3b(self):
        self.work(weights=[
         0.33333333, 0.66666667], mus=[
         5.505, 5.0], sigmas=[
         8.99, 5.19], q=1, low=1.01, high=10, n_samples=10000)
...
class TestLGMM1Math(unittest.TestCase):
    ...
    def LGMM1_kwargs(self):
        return dict(weights=self.weights, mus=self.mus, sigmas=self.sigmas, low=self.low, high=self.high)
    ...
    def work(self, **kwargs):
        self.__dict__.update(kwargs)
        self.worked = True
        samples = LGMM1(rng=self.rng, size=(
         self.n_samples,), **self.LGMM1_kwargs)
        samples = np.sort(samples)
        edges = samples[::self.samples_per_bin]
...
class TestQLGMM1Math(unittest.TestCase):
    ...
    @property
    def kwargs(self):
        return dict(weights=self.weights, mus=self.mus, sigmas=self.sigmas, low=self.low, high=self.high, q=self.q)
    ...
    def work(self, **kwargs):
        self.__dict__.update(kwargs)
        self.worked = True
        samples = old_div(LGMM1(rng=self.rng, size=(
         self.n_samples,), **self.kwargs), self.q)
        assert np.all(samples == samples.astype('int'))
        min_max = (int(samples.min()), int(samples.max()))
...
class TestSuggest(unittest.TestCase, CasePerDomain):
    ...
    def work(self):
        trials = Trials()
        fmin(passthrough, space=self.bandit.expr, algo=partial(tpe.suggest, n_EI_candidates=3), trials=trials, max_evals=10)
...
class TestSuggestAtpe(unittest.TestCase, NonCategoricalCasePerDomain):
    ...
    def work(self):
        trials = Trials()
        fmin(passthrough, space=self.bandit.expr, algo=atpe.suggest, trials=trials, max_evals=10)


class TestOpt(unittest.TestCase, CasePerDomain):
    thresholds = dict(quadratic1=1e-05, q1_lognormal=0.01, distractor=-1.96, gauss_wave=-2.0, gauss_wave2=-2.0, n_arms=-2.5, many_dists=0.0005, branin=0.7)
    LEN = dict(quadratic1=1000, many_dists=200, distractor=100, q1_lognormal=250, gauss_wave2=75, branin=200)
    gammas = dict(distractor=0.05)
    prior_weights = dict(distractor=0.01)
    n_EIs = dict(quadratic1=5, distractor=15)
    ...
    def work(self):
        ...
        algo = partial(tpe.suggest, gamma=self.gammas.get(bandit.name, tpe._default_gamma), prior_weight=self.prior_weights.get(bandit.name, tpe._default_prior_weight), n_EI_candidates=self.n_EIs.get(bandit.name, tpe._default_n_EI_candidates))
        LEN = self.LEN.get(bandit.name, 50)
        trials = Trials()
        fmin(passthrough, space=bandit.expr, algo=algo, trials=trials, max_evals=LEN, rstate=np.random.RandomState(123), catch_eval_exceptions=False)
        assert len(trials) == LEN
        rtrials = Trials()
        fmin(passthrough, space=bandit.expr, algo=rand.suggest, trials=rtrials, max_evals=LEN)
...
class TestOptQUniform(object):
    ...
    def work(self, **kwargs):
        ...
        prior_weight = 2.5
        gamma = 0.2
        algo = partial(tpe.suggest, prior_weight=prior_weight, n_startup_jobs=2, n_EI_candidates=128, gamma=gamma)
        trials = Trials()
        fmin(passthrough, space=bandit.expr, algo=algo, trials=trials, max_evals=self.LEN)
        if self.show_vars:
        ...
        b_kwargs = dict([(n, a) for n, a in b_post.named_args if n not in ('rng', 'size')])
