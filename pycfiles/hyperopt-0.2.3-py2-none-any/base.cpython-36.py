...
STATUS_OK = 'ok'
STATUS_FAIL = 'fail'
STATUS_STRINGS = ('new', 'running', 'suspended', 'ok', 'fail')
JOB_STATE_NEW = 0
JOB_STATE_RUNNING = 1
JOB_STATE_DONE = 2
JOB_STATE_ERROR = 3
...
def miscs_update_idxs_vals(miscs, idxs, vals, assert_all_vals_used=True, idxs_map=None):
    ...
    if idxs_map is None:
        idxs_map = {}
    elif not set(idxs.keys()) == set(vals.keys()):
...
class Trials(object):
    ...
    def refresh(self):
        if self._exp_key is None:
            ...
            self._trials = [tt for tt in self._dynamic_trials if tt['state'] in JOB_VALID_STATES if tt['exp_key'] == self._exp_key]
    ...
    def best_trial(self):
        ...
        """
        candidates = [t for t in self.trials if t['result']['status'] == STATUS_OK if not np.isnan(t['result']['loss'])]
        if not candidates:
    ...
    def fmin(self, fn, space, algo, max_evals, max_queue_len=1, rstate=None, verbose=0, pass_expr_memo_ctrl=None, catch_eval_exceptions=False, return_argmin=True, show_progressbar=True):
        ...
        return fmin(fn,
          space, algo, max_evals, trials=self,
          rstate=rstate,
          verbose=verbose,
...
class Ctrl(object):
    ...
    def inject_results(self, specs, results, miscs, new_tids=None):
        ...
        new_trials = self.trials.source_trial_docs(tids=new_tids, specs=specs,
          results=results,
          miscs=miscs,
          sources=[
         trial])
