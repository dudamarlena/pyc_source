...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymills/pyodict.py
...
class _odict(object):
    ...
    def __init__(self, data=(), **kwds):
        ...
        """
        if kwds:
            raise TypeError('__init__() of ordered dict takes no keyword arguments to avoid an ordering trap.')
    ...
    def __setitem__(self, key, val):
        ...
        try:
            ...
            if dict_impl.__getattribute__(self, 'lt') == _nil:
                ...
                dict_impl.__getitem__(self, dict_impl.__getattribute__(self, 'lt'))[2] = key
    ...
    def update(self, data=(), **kwds):
        if kwds:
            raise TypeError('update() of ordered dict takes no keyword arguments to avoid an ordering trap.')
        if hasattr(data, 'iteritems'):
            for key, val in data.iteritems():
                self[key] = val
