...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/loader_directory.py
# Compiled at: 2019-04-28 04:54:30
"""Utilities for loading and reasoning about unparsed tools in directories."""
import fnmatch, glob, logging, os, re, sys, yaml
...
TOOL_REGEX = re.compile('<tool\\s')
DATA_MANAGER_REGEX = re.compile('\\stool_type=\\"manage_data\\"')
YAML_EXTENSIONS = [
 '.yaml', '.yml', '.json']
...
def _load_tools_from_path(path, load_exception_handler, recursive, register_load_errors, loader_func, enable_beta_formats):
    loaded_objects = []
    for possible_tool_file in find_possible_tools_from_path(path, recursive=recursive, enable_beta_formats=enable_beta_formats):
        try:
...
def looks_like_xml(path, regex=TOOL_REGEX):
    ...
    if not os.path.getsize(full_path):
        return False
    if checkers.check_binary(full_path) or checkers.check_image(full_path) or checkers.is_gzip(full_path) or checkers.is_bz2(full_path) or checkers.is_zip(full_path):
        return False
    with open(path, 'r') as (f):
        start_contents = f.read(5120)
...
def resolved_path(path_or_uri_like):
    ...
    if '://' not in path_or_uri_like:
        ...
        if path_or_uri_like.startswith('file://'):
            return path_or_uri_like[len('file://'):]
        return UNRESOLVED_URI


BETA_TOOL_CHECKERS = {'yaml': looks_like_a_tool_yaml, 
   'cwl': looks_like_a_tool_cwl}
__all__ = ('find_possible_tools_from_path', 'is_a_yaml_with_class', 'is_tool_load_error',
           'load_tool_elements_from_path', 'load_tool_sources_from_path', 'looks_like_a_cwl_artifact',
           'looks_like_a_tool_cwl', 'looks_like_a_tool_xml', 'looks_like_a_tool_yaml')