...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/cwl/schema.py
# Compiled at: 2018-09-15 08:40:24
"""Abstraction around cwltool and related libraries for loading a CWL artifact."""
import os
from collections import namedtuple
from six.moves.urllib.parse import urldefrag
from .cwltool_deps import ensure_cwltool_available, load_tool, LoadingContext, schema_salad, workflow
...
class SchemaLoader(object):
    ...
    def process_definition(self, raw_reference):
        document_loader, avsc_names, process_object, metadata, uri = load_tool.validate_document(self.raw_document_loader, raw_reference.process_object, raw_reference.uri)
        process_def = ProcessDefinition(process_object, metadata, document_loader, avsc_names, raw_reference)
        return process_def
    ...
    def tool(self, **kwds):
        ...
        if LoadingContext is not None:
            ...
            tool = load_tool.make_tool(process_definition.document_loader, process_definition.avsc_names, process_definition.metadata, process_definition.raw_process_reference.uri, loading_context)
        else:
            tool = load_tool.make_tool(process_definition.document_loader, process_definition.avsc_names, process_definition.metadata, process_definition.raw_process_reference.uri, make_tool, args)
        return tool
