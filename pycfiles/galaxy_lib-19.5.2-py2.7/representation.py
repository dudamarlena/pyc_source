...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/cwl/representation.py
...
INPUT_TYPE = Bunch(DATA='data', INTEGER='integer', FLOAT='float', TEXT='text', BOOLEAN='boolean', SELECT='select', FIELD='field', CONDITIONAL='conditional', DATA_COLLECTON='data_collection')
...
TYPE_REPRESENTATIONS = [
 ...
 TypeRepresentation('enum', INPUT_TYPE.TEXT, 'enum value', None),
...
if not USE_FIELD_TYPES:
    CWL_TYPE_TO_REPRESENTATIONS = {'Any': ['integer', 'float', 'file', 'boolean', 'text', 'record', 'json'], 'array': [
               'array'], 
       'string': [
                'text'], 
       'boolean': [
    ...
    CWL_TYPE_TO_REPRESENTATIONS = {'Any': ['field'], 'array': [
               'array'], 
       'string': [
                'text'], 
       'boolean': [
...
def to_cwl_job(tool, param_dict, local_working_directory):
    ...
    def simple_value(input, param_dict_value, type_representation_name=None):
        ...
        if type_representation.name == 'file':
            ...
            if type_representation.name == 'directory':
                dataset_wrapper = param_dict_value
                return dataset_wrapper_to_directory_json(inputs_dir, dataset_wrapper)
            if type_representation.name == 'integer':
                return int(str(param_dict_value))
            if type_representation.name == 'long':
                return int(str(param_dict_value))
            if type_representation.name in ('float', 'double'):
                return float(str(param_dict_value))
            if type_representation.name == 'boolean':
                return string_as_bool(param_dict_value)
            if type_representation.name == 'text':
                return str(param_dict_value)
            if type_representation.name == 'enum':
                return str(param_dict_value)
            if type_representation.name == 'json':
                raw_value = param_dict_value.value
                return json.loads(raw_value)
            if type_representation.name == 'field':
                if param_dict_value is None:
                    return
                if hasattr(param_dict_value, 'value'):
                    return param_dict_value.value['value']
                if not param_dict_value.is_collection:
                ...
                if hdca_wrapper.collection_type.collection_type == 'record':
                    return collection_wrapper_to_record(inputs_dir, hdca_wrapper)
            else:
                if type_representation.name == 'array':
                    return collection_wrapper_to_array(inputs_dir, param_dict_value)
                else:
                    if type_representation.name == 'record':
    ...
    for input_name, input in inputs.items():
        ...
        if galaxy_input_type == 'repeat':
            ...
            if (as_dict_value is NOT_PRESENT or as_dict_value is None) and 'null' in case_strings:
                ...
                raise RequestParameterInvalidException('Cannot translate CWL datatype - value [%s] of type [%s] with case_strings [%s]. Non-null property must be set.' % (
