...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/requirements.py
...
class ContainerDescription(object):
    ...
    def __init__(self, identifier=None, type=DEFAULT_CONTAINER_TYPE, resolve_dependencies=DEFAULT_CONTAINER_RESOLVE_DEPENDENCIES, shell=DEFAULT_CONTAINER_SHELL):
    ...
    def to_dict(self):
        return dict(identifier=self.identifier, type=self.type, resolve_dependencies=self.resolve_dependencies, shell=self.shell)
    ...
    def from_dict(dict):
        ...
        return ContainerDescription(identifier=identifier, type=type, resolve_dependencies=resolve_dependencies, shell=shell)
...
def container_from_element(container_elem):
    ...
    container = ContainerDescription(identifier=identifier, type=type, resolve_dependencies=resolve_dependencies, shell=shell)
    return container