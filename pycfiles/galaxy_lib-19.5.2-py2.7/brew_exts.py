...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/brew_exts.py
# Compiled at: 2019-04-28 04:54:30
from __future__ import print_function
import argparse, contextlib, glob, json, os, re, string, subprocess, sys
...
class RecipeContext(object):
    ...
    def __tap_path(self, recipe):
        ...
        if len(parts) == 1:
            ...
            blob_index = from_url_parts.index('blob')
            if blob_index < 2:
                raise Exception(CANNOT_DETERMINE_TAP_ERROR_MESSAGE)
...
class EnvAction(object):
    ...
    def __init__(self, keg_root, action_description):
        ...
        self.value = string.Template(action_description['value']).safe_substitute({'KEG_ROOT': keg_root})
    ...
    def __eval(self, template):
        return string.Template(template).safe_substitute(variable=self.variable, value=self.value)
...
def extended_brew_info(recipe):
    ...
    extra_info = dict(from_url=None, build_dependencies=[], required_dependencies=[], recommended_dependencies=[], optional_dependencies=[])
...
def ensure_brew_on_path(args):
    ...
    if args and args.brew:
        ...
        if brew_on_path:
            return brew_on_path
        if os.path.exists(default_brew_path):
