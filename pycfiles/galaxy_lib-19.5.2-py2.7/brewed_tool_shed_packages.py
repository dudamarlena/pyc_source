...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/resolvers/brewed_tool_shed_packages.py
...
class HomebrewToolShedDependencyResolver(DependencyResolver, UsesHomebrewMixin, UsesToolDependencyDirMixin, UsesInstalledRepositoriesMixin):
    ...
    def _resolve_from_installed_tool_dependency(self, name, version, installed_tool_dependency):
        ...
        recipe_name = build_recipe_name(package_name=name, package_version=version, repository_owner=tool_shed_repository.owner, repository_name=tool_shed_repository.name)
    ...
    def _resolve_from_tool_dependencies_path(self, name, version, tool_dependencies_path):
        ...
        if not raw_dependency:
            ...
            recipe_name = build_recipe_name(package_name=name, package_version=version, repository_owner=raw_dependency.repository_owner, repository_name=raw_dependency.repository_name)
