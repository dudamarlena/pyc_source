...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/jobs/metrics/instrumenters/cgroup.py
...
TITLES = {'memory.memsw.max_usage_in_bytes': 'Max memory usage (MEM+SWP)', 
...
CONVERSION = {'memory.oom_control.oom_kill_disable': lambda x: 'No' if x == 1 else 'Yes', 
   'memory.oom_control.under_oom': lambda x: 'Yes' if x == 1 else 'No', 
   'cpuacct.usage': lambda x: formatting.seconds_to_str(x / 1000000000)}
