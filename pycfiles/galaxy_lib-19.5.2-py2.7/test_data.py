...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/verify/test_data.py
# Compiled at: 2019-04-28 04:54:30
from __future__ import print_function
import hashlib, os, re, subprocess
from string import Template
from galaxy.util import asbool, in_directory, smart_str
UPDATE_TEMPLATE = Template('git --work-tree $dir --git-dir $dir/.git fetch && git --work-tree $dir --git-dir $dir/.git merge origin/master')
UPDATE_FAILED_TEMPLATE = Template('Warning failed to update test repository $dir - update stdout was [$stdout] and stderr was [$stderr].')
...
class GitDataResolver(FileDataResolver):
    ...
    def execute(self, cmd):
        subprocess_kwds = dict(shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print('Executing %s' % cmd)
        p = subprocess.Popen(cmd, **subprocess_kwds)
        stdout, stderr = p.communicate()
        if p.returncode != 0:
            kwds = {'dir': self.file_dir, 'stdout': stdout, 
               'stderr': stderr}
