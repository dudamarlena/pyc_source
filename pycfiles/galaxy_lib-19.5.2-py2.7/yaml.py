...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/parser/yaml.py
...
class YamlToolSource(ToolSource):
    ...
    def _parse_output_collection(self, tool, name, output_dict):
        ...
        structure = ToolOutputCollectionStructure(collection_type=collection_type, collection_type_source=collection_type_source, structured_like=structured_like, dataset_collector_descriptions=dataset_collector_descriptions)
        output_collection = ToolOutputCollection(name, structure, label=label, filters=filters, default_format=default_format, inherit_format=inherit_format, inherit_metadata=inherit_metadata, default_format_source=default_format_source, default_metadata_source=default_metadata_source)
...
def __to_test_assert_list(assertions):
    ...
    for assertion in assertions:
        ...
        assert_dict = dict(tag=assertion['that'], attributes=assertion, children=children)
        assert_list.append(assert_dict)

    return assert_list or None
