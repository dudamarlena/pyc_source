...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/util/json.py
# Compiled at: 2019-04-28 04:54:30
from __future__ import absolute_import
import collections, copy, json, logging, math, random, string
from six import iteritems, string_types
__all__ = ('safe_dumps', 'validate_jsonrpc_request', 'validate_jsonrpc_response', 'jsonrpc_request',
           'jsonrpc_response')
...
def safe_dumps(*args, **kwargs):
    ...
    """
    try:
        dumped = json.dumps(allow_nan=False, *args, **kwargs)
    except ValueError:
...
def validate_jsonrpc_request(request, regular_methods, notification_methods):
    try:
        ...
        return (
         False, request,
         jsonrpc_response(id=None, error=dict(code=-32700, message='Parse error', data=str(e))))

    try:
        assert 'jsonrpc' in request, 'This server requires JSON-RPC 2.0 and no "jsonrpc" member was sent with the Request object as per the JSON-RPC 2.0 Specification.'
        assert request['jsonrpc'] == '2.0', 'Requested JSON-RPC version "%s" != required version "2.0".' % request['jsonrpc']
        assert 'method' in request, 'No "method" member was sent with the Request object'
    except AssertionError as e:
        return (
         False, request,
         jsonrpc_response(request=request, error=dict(code=-32600, message='Invalid Request', data=str(e))))
    ...
    try:
        ...
        return (
         False, request,
         jsonrpc_response(request=request, error=dict(code=-32601, message='Method not found', data='Valid methods are: %s' % (', ').join(regular_methods + notification_methods))))

    try:
        assert request['method'] in regular_methods and 'id' in request, 'No "id" member was sent with the Request object and the requested method "%s" is not a notification method' % request['method']
    except AssertionError as e:
        return (
         False, request,
         jsonrpc_response(request=request, error=dict(code=-32600, message='Invalid Request', data=str(e))))
...
def validate_jsonrpc_response(response, id=None):
    ...
    try:
        assert 'jsonrpc' in response, 'This server requires JSON-RPC 2.0 and no "jsonrpc" member was sent with the Response object as per the JSON-RPC 2.0 Specification.'
        assert 'result' in response or 'error' in response, 'Neither of "result" or "error" members were sent with the Response object.'
        if 'error' in response:
            assert int(response['error']['code']), 'The "code" member of the "error" object in the Response is missing or not an integer.'
