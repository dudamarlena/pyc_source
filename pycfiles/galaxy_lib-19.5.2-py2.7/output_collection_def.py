...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/parser/output_collection_def.py
...
DEFAULT_EXTRA_FILENAME_PATTERN = 'primary_DATASET_ID_(?P<designation>[^_]+)_(?P<visible>[^_]+)_(?P<ext>[^_]+)(_(?P<dbkey>[^_]+))?'
DEFAULT_SORT_BY = 'filename'
DEFAULT_SORT_COMP = 'lexical'
NAMED_PATTERNS = {'__default__': DEFAULT_EXTRA_FILENAME_PATTERN, 
   '__name__': '(?P<name>.*)', 
   '__designation__': '(?P<designation>.*)', 
   '__name_and_ext__': '(?P<name>.*)\\.(?P<ext>[^\\.]+)?', 
   '__designation_and_ext__': '(?P<designation>.*)\\.(?P<ext>[^\\._]+)?'}
INPUT_DBKEY_TOKEN = '__input__'
LEGACY_DEFAULT_DBKEY = None
