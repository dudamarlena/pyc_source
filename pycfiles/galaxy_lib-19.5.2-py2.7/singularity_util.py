...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/singularity_util.py
...
def pull_mulled_singularity_command(docker_image_identifier, cache_directory, namespace=None, singularity_cmd=DEFAULT_SINGULARITY_COMMAND, sudo=DEFAULT_SUDO, sudo_cmd=DEFAULT_SUDO_COMMAND):
    command_parts = []
    command_parts += _singularity_prefix(singularity_cmd=singularity_cmd, sudo=sudo, sudo_cmd=sudo_cmd)
...
def build_singularity_run_command(container_command, image, volumes=[], env=[], working_directory=DEFAULT_WORKING_DIRECTORY, singularity_cmd=DEFAULT_SINGULARITY_COMMAND, run_extra_arguments=DEFAULT_RUN_EXTRA_ARGUMENTS, sudo=DEFAULT_SUDO, sudo_cmd=DEFAULT_SUDO_COMMAND):
    command_parts = []
    home = None
