...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/objectstore/rods.py
...
from posixpath import basename as path_basename, dirname as path_dirname, join as path_join
try:
    import irods
except ImportError:
    irods = None
...
class IRODSObjectStore(DiskObjectStore):
    ...
    def create(self, obj, **kwargs):
        rods_path = self.exists(obj, **kwargs) or self.__get_rods_path(obj, **kwargs)
        ...
        if not dir_only:
            ...
            if not status >= 0:
                raise AssertionError('create(): rcDataObjCreate() failed: %s: %s: %s' % (rods_path, status, irods.strerror(status)))
