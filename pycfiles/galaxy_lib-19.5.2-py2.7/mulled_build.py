...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/mulled/mulled_build.py
...
from .util import build_target, conda_build_target_str, create_repository, PrintProgress, quay_repository, v1_image_name, v2_image_name
...
def get_affected_packages(args):
    ...
    for pkg in pkg_list:
        if pkg and os.path.exists(os.path.join(recipes_dir, pkg)):
            yield (
             get_pkg_name(args, pkg), get_tests(args, pkg))
...
def mull_targets(targets, involucro_context=None, command='build', channels=DEFAULT_CHANNELS, namespace='biocontainers', test='true', test_files=None, image_build=None, name_override=None, repository_template=DEFAULT_REPOSITORY_TEMPLATE, dry_run=False, conda_version=None, verbose=False, binds=DEFAULT_BINDS, rebuild=True, oauth_token=None, hash_func='v2', singularity=False, singularity_image_dir='singularity_import'):
    ...
    if len(targets) > 1 and image_build is None:
        image_build = '0'
    repo_template_kwds = {'namespace': namespace, 
    ...
    if not rebuild or 'push' in command:
        ...
        if not rebuild:
            ...
            if ':' in repo_template_kwds['image']:
                ...
                if not len(image_name_parts) == 2:
                    raise AssertionError(': not allowed in image name [%s]' % repo_template_kwds['image'])
        ...
        if test_files:
            ...
            for test_file in test_files:
                ...
                elif os.path.exists(test_file.split(':')[0]):
        ...
        if not dry_run:
            ...
            if singularity:
                os.path.exists(singularity_image_dir) or safe_makedirs(singularity_image_dir)
...
def add_build_arguments(parser):
    ...
    parser.add_argument('--involucro-path', dest='involucro_path', default=None, help='Path to involucro (if not set will look in working directory and on PATH).')
    ...
    parser.add_argument('-r', '--repository_template', dest='repository_template', default=DEFAULT_REPOSITORY_TEMPLATE, help='Docker repository target for publication (only quay.io or compat. API is currently supported).')
    parser.add_argument('-c', '--channels', dest='channels', default=(',').join(DEFAULT_CHANNELS), help='Comma separated list of target conda channels.')
    parser.add_argument('--conda-version', dest='conda_version', default=None, help='Change to specified version of Conda before installing packages.')
    parser.add_argument('--oauth-token', dest='oauth_token', default=None, help='If set, use this token when communicating with quay.io API.')
...
def add_single_image_arguments(parser):
    parser.add_argument('--name-override', dest='name_override', default=None, help='Override mulled image name - this is not recommended since metadata will not be detectable from the name of resulting images')
...
def main(argv=None):
    ...
    parser.add_argument('--test-files', help='Provide test-files that may be required to run the test command. Individual mounts are separated by comma.The source:dest docker syntax is respected. If relative file paths are given, files will be mounted in /source/<relative_file_path>')
