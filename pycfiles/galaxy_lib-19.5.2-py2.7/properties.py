...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/util/properties.py
...
def find_config_file(names, exts=None, dirs=None, include_samples=False):
    ...
    >>> rmtree(d)
    """
    found = __find_config_files(names, exts=exts or extensions['yaml'] + extensions['ini'], dirs=dirs or [os.getcwd(), os.path.join(os.getcwd(), 'config')], include_samples=include_samples)
    if not found:
        return None
    else:
        return found[0]
