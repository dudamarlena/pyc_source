...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/util/heartbeat.py
...
class Heartbeat(threading.Thread):
    ...
    def thread_is_sleeping(self, last_stack_frame):
        ...
        if _funcname == 'accept' and _text[-14:] == '_sock.accept()':
            return True
        if _funcname in ('monitor', '__monitor', 'app_loop', 'check') and _text.startswith('time.sleep(') and _text.endswith(')'):
            return True
    ...
    def get_interesting_stack_frame(self, stack_frames):
        ...
        for _filename, _line, _funcname, _text in reversed(stack_frames):
            ...
            if idx != -1:
                ...
                return (
                 relative_filename, _line, _funcname, _text)
