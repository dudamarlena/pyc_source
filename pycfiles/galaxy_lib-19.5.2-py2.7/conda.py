...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/resolvers/conda.py
...
"""
import logging, os, re, galaxy.tools.deps.installable, galaxy.tools.deps.requirements
from . import Dependency, DependencyException, DependencyResolver, InstallableDependencyResolver, ListableDependencyResolver, MappableDependencyResolver, MultipleDependencyResolver, NullDependency, SpecificationPatternDependencyResolver
from ..conda_util import build_isolated_environment, cleanup_failed_install, cleanup_failed_install_of_environment, CondaContext, CondaTarget, hash_conda_packages, install_conda, install_conda_target, install_conda_targets, installed_conda_targets, is_conda_target_installed, USE_PATH_EXEC_DEFAULT
...
class CondaDependencyResolver(DependencyResolver, MultipleDependencyResolver, ListableDependencyResolver, InstallableDependencyResolver, SpecificationPatternDependencyResolver, MappableDependencyResolver):
    ...
    config_options = {'prefix': None, 
       ...
       'use_local': False}
    _specification_pattern = re.compile('https\\:\\/\\/anaconda.org\\/\\w+\\/\\w+')
    ...
    def __init__(self, dependency_manager, **kwds):
        ...
        if ensure_channels is None:
            ensure_channels = DEFAULT_ENSURE_CHANNELS
        conda_context = CondaContext(conda_prefix=conda_prefix, conda_exec=conda_exec, debug=debug, ensure_channels=ensure_channels, condarc_override=condarc_override, use_path_exec=use_path_exec, copy_dependencies=copy_dependencies, use_local=use_local)
    ...
    def resolve_all(self, requirements, **kwds):
        ...
        if len(requirements) == 0:
            ...
            if is_installed:
                for requirement in requirements:
                    dependency = MergedCondaDependency(self.conda_context, self.conda_context.env_path(env), exact=not self.versionless or requirement.version is None, name=requirement.name, version=requirement.version, preserve_python_environment=preserve_python_environment)
    ...
    def resolve(self, requirement, **kwds):
        ...
        if not os.path.isdir(self.conda_context.conda_prefix):
            ...
            return CondaDependency(self.conda_context, conda_environment, exact, name, version, preserve_python_environment=preserve_python_environment)
    ...
    def unused_dependency_paths(self, toolbox_requirements_status):
        ...
        unused_paths = dir_contents.difference(used_paths)
...
class CondaDependency(Dependency):
    ...
    def build_environment(self):
        env_path, exit_code = build_isolated_environment(CondaTarget(self.name, self.version), conda_context=self.conda_context, path=self.environment_path, copy=self.conda_context.copy_dependencies)
        if exit_code:
            if len(os.path.abspath(self.environment_path)) > 79:
                raise DependencyException('Conda dependency failed to build job environment. This is most likely a limitation in conda. You can try to shorten the path to the job_working_directory.')
...
__all__ = (
 'CondaDependencyResolver', 'DEFAULT_ENSURE_CHANNELS')