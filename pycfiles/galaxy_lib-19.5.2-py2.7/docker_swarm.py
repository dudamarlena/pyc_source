...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/containers/docker_swarm.py
...
try:
    ...
    docker = Bunch(types=Bunch(ContainerSpec=None, RestartPolicy=None, Resources=None, Placement=None))
...
SWARM_MANAGER_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.pardir, os.path.pardir, os.path.pardir, 'scripts', 'docker_swarm_manager.py'))

class DockerSwarmInterface(DockerInterface):
    container_class = DockerService
    conf_defaults = {'ignore_volumes': False, 
       'node_prefix': None, 
    ...
    def run_in_container(self, command, image=None, **kwopts):
        ...
        if kwopts.get('publish_all_ports', False):
            if kwopts.get('publish_port_random', False) or kwopts.get('ports', False):
                ...
                raise ContainerRunError('Publishing all ports is not supported in Docker swarm mode, use `publish_port_random` or `ports`', image=image, command=command)
        ...
        if kwopts.get('volumes', None):
            if self._conf.ignore_volumes:
                log.warning("'volumes' kwopt is set and not supported in Docker swarm mode, volumes will not be passed (set 'ignore_volumes: False' in containers config to fail instead): %s" % kwopts['volumes'])
            else:
                raise ContainerRunError("'volumes' kwopt is set and not supported in Docker swarm mode (set 'ignore_volumes: True' in containers config to warn instead): %s" % kwopts['volumes'], image=image, command=command)
    ...
    def services_clean(self):
        cleaned_service_ids = []
        completed_services = list(self.services_completed())
...
class DockerSwarmCLIInterface(DockerSwarmInterface, DockerCLIInterface):
    ...
    option_map = {'constraint': {'flag': '--constraint', 'type': 'list_of_kovtrips'}, 'replicas': {'flag': '--replicas', 'type': 'string'}, 'restart_condition': {'flag': '--restart-condition', 'type': 'string'}, 'environment': {'flag': '--env', 'type': 'list_of_kvpairs'}, 'name': {'flag': '--name', 'type': 'string'}, 'publish_port_random': {'flag': '--publish', 'type': 'string'}, 'cpu_limit': {'flag': '--limit-cpu', 'type': 'string'}, 'mem_limit': {'flag': '--limit-memory', 'type': 'string'}, 'cpu_reservation': {'flag': '--reserve-cpu', 'type': 'string'}, 'mem_reservation': {'flag': '--reserve-memory', 'type': 'string'}, 'label_add': {'flag': '--label-add', 'type': 'list_of_kvpairs'}, 'label_rm': {'flag': '--label-rm', 'type': 'list_of_kvpairs'}, 'availability': {'flag': '--availability', 'type': 'string'}}
    ...
    def service_create(self, command, image=None, **kwopts):
        ...
        args = ('{kwopts} {image} {command}').format(kwopts=self._stringify_kwopts(kwopts), image=image if image else '', command=command if command else '').strip()
    ...
    def node_update(self, node_id, **kwopts):
        return self._run_docker(subcommand='node update', args=('{kwopts} {node_id}').format(kwopts=self._stringify_kwopts(kwopts), node_id=node_id))
...
class DockerSwarmAPIInterface(DockerSwarmInterface, DockerAPIInterface):
    ...
    service_mode_option_map = {'service_mode': {'param': 0, 'default': 'replicated'}, 'replicas': {'default': 1}}
    endpoint_spec_option_map = {'ports': {}}
    resources_option_map = {'cpus': {'params': ('cpu_limit', 'cpu_reservation'), 'map': lambda x: int(x * 1000000000)}, 
       'memory': {'params': ('mem_limit', 'mem_reservation')}}
    container_spec_option_map = {'image': {'param': 0}, 'command': {}, 'environment': {'param': 'env'}, 'labels': {}}
    restart_policy_option_map = {'restart_condition': {'param': 'condition', 'default': 'none'}, 'restart_delay': {'param': 'delay'}, 'restart_max_attempts': {'param': 'max_attemps'}}
    task_template_option_map = {'_container_spec': {'spec_class': docker.types.ContainerSpec, 'required': True}, '_resources': {'spec_class': docker.types.Resources}, '_restart_policy': {'spec_class': docker.types.RestartPolicy}, '_placement': {'spec_class': docker.types.Placement}}
    node_spec_option_map = {'availability': {'param': 'Availability'}, 'name': {'param': 'Name'}, 'role': {'param': 'Role'}, 'labels': {'param': 'Labels'}}
    ...
    def service_create(self, command, image=None, **kwopts):
        ...
        if 'publish_port_random' in kwopts:
            kwopts['ports'] = [
             DockerSwarmAPIInterface.create_random_port_spec(kwopts.pop('publish_port_random'))]
        ...
        service = self._client.create_service(task_template, mode=service_mode, endpoint_spec=endpoint_spec, success_test=success_test, max_tries=5, **kwopts)
