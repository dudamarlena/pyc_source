...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/dockerfiles.py
...
def docker_host_args(**kwds):
    return dict(docker_cmd=kwds['docker_cmd'], sudo=kwds['docker_sudo'], sudo_cmd=kwds['docker_sudo_cmd'], host=kwds['docker_host'])
...
def dockerfile_build(path, dockerfile=None, error=log.error, **kwds):
    ...
    if docker_image_cache:
        ...
        save_image_command_parts = docker_util.build_save_image_command(image_identifier, destination, **docker_host_args(**kwds))
