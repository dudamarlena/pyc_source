...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/resolvers/tool_shed_packages.py
...
class ToolShedPackageDependencyResolver(BaseGalaxyPackageDependencyResolver, UsesInstalledRepositoriesMixin):
    ...
    def _get_package_installed_dependency_path(self, installed_tool_dependency, name, version):
        ...
        return join(base_path, name, version, tool_shed_repository.owner, tool_shed_repository.name, tool_shed_repository.installed_changeset_revision)

    def _get_set_environment_installed_dependency_script_path(self, installed_tool_dependency, name):
        tool_shed_repository = installed_tool_dependency.tool_shed_repository
        base_path = self.base_path
        path = abspath(join(base_path, 'environment_settings', name, tool_shed_repository.owner, tool_shed_repository.name, tool_shed_repository.installed_changeset_revision))
        if exists(path):
