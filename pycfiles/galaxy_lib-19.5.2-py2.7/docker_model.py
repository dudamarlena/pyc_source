...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/containers/docker_model.py
...
class DockerContainer(Container):
    ...
    def ports(self):
        ...
        try:
            ...
            log.warning("Failed to get ports for container %s from `docker inspect` output at ['NetworkSettings']['Ports']: %s: %s", self.id, exc.__class__.__name__, str(exc))
            return
...
class DockerService(Container):
    ...
    def ports(self):
        ...
        try:
            ...
            log.warning("Failed to get ports for container %s from `docker service inspect` output at ['Endpoint']['Ports']: %s: %s", self.id, exc.__class__.__name__, str(exc))
            return

        for binding in port_mappings:
            rval.append(ContainerPort(binding['TargetPort'], binding['Protocol'], self.address, binding['PublishedPort']))
...
class DockerNode(object):
    ...
    def from_cli(cls, interface, n, task_list):
        node = cls(interface, id=n['ID'], name=n['HOSTNAME'], status=n['STATUS'], availability=n['AVAILABILITY'], manager=True if n['MANAGER STATUS'] else False)
        for task_dict in task_list:
...
class DockerTask(object):
    ...
    def __init__(self, interface, id=None, name=None, image=None, desired_state=None, state=None, error=None, ports=None, service=None, node=None):
        self._interface = interface
        self._id = id
        self._name = name
        self._image = image
    ...
    def from_cli(cls, interface, t, service=None, node=None):
        ...
        return cls(interface, id=t['ID'], name=t['NAME'], image=t['IMAGE'], desired_state=t['DESIRED STATE'], state=state, error=t['ERROR'], ports=t['PORTS'], service=service, node=node)
    ...
    def from_api(cls, interface, t, service=None, node=None):
        ...
        if service:
            ...
            name = t['ID']
        image = (
         t['Spec']['ContainerSpec']['Image'].split('@', 1)[0],)
        return cls(interface, id=t['ID'], name=name, image=image, desired_state=t['DesiredState'], state=t['Status']['State'], ports=t['Status']['PortStatus'], error=t['Status']['Message'], service=service, node=node)
    ...
    def inspect(self):
        if not self._inspect:
            try:
                ...
                log.error('Task could not be inspected because Docker claims it does not exist: %s (%s)', self.name, self.id)
                return
