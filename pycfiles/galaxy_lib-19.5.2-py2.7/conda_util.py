...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/conda_util.py
# Compiled at: 2019-04-28 04:54:30
import functools, hashlib, json, logging, os, re, shutil, sys, tempfile, packaging.version, six
from six.moves import shlex_quote
from galaxy.tools.deps.commands import CommandLineException
from galaxy.util import smart_str, unicodify
from . import commands, installable
log = logging.getLogger(__name__)
SHELL_UNSAFE_PATTERN = re.compile('[\\s\\"\']')
IS_OS_X = sys.platform == 'darwin'
CONDA_LICENSE = 'http://docs.continuum.io/anaconda/eula'
VERSIONED_ENV_DIR_NAME = re.compile('__(.*)@(.*)')
UNVERSIONED_ENV_DIR_NAME = re.compile('__(.*)@_uv_')
...
def find_conda_prefix(conda_prefix=None):
    ...
    if conda_prefix is None:
        ...
        if os.path.exists(miniconda_2_dest):
...
class CondaContext(installable.InstallableContext):
    ...
    def __init__(self, conda_prefix=None, conda_exec=None, shell_exec=None, debug=False, ensure_channels='', condarc_override=None, use_path_exec=USE_PATH_EXEC_DEFAULT, copy_dependencies=False, use_local=USE_LOCAL_DEFAULT):
    ...
    def ensure_conda_build_installed_if_needed(self):
        if self.use_local and not self.conda_build_available:
            conda_targets = [
             CondaTarget('conda-build', version=CONDA_BUILD_VERSION)]
    ...
    def can_install_conda(self):
        ...
        if conda_exec == conda_prefix_plus_exec:
            if not os.path.exists(self.conda_prefix):
                ...
                if os.listdir(self.conda_prefix) == []:
                    os.rmdir(self.conda_prefix)
                    return True
                log.warning("Cannot install Conda because conda_prefix '%s' exists and is not empty.", self.conda_prefix)
                return False

        else:
            log.warning("Skipping installation of Conda into conda_prefix '%s', since conda_exec '%s' is set to a path outside of conda_prefix.", self.conda_prefix, self.conda_exec)
            return False
    ...
    def exec_command(self, operation, args, stdout_path=None):
        ...
        try:
            try:
                ...
                conda_exec_home = env['HOME'] = tempfile.mkdtemp(prefix='conda_exec_home_')
...
def install_conda_target(conda_target, conda_context, skip_environment=False):
    ...
    if not skip_environment:
        create_args = ['--name', conda_target.install_environment,
...
def best_search_result(conda_target, conda_context, channels_override=None, offline=False):
    ...
    """
    search_cmd = [
     conda_context.conda_exec, 'search', '--full-name', '--json']
    if offline:
...
def build_isolated_environment(conda_packages, conda_context, path=None, copy=False, quiet=False):
    ...
    try:
        ...
        create_args = [
         '--unknown']
        offline_works = conda_context.conda_version < packaging.version.parse('4.3') or conda_context.conda_version >= packaging.version.parse('4.4')
        if offline_works:
