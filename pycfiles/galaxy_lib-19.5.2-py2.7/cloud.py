...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/objectstore/cloud.py
...
NO_CLOUDBRIDGE_ERROR_MESSAGE = 'Cloud ObjectStore is configured, but no CloudBridge dependency available.Please install CloudBridge or modify ObjectStore configuration.'
...
class Cloud(ObjectStore, CloudConfigMixin):
    ...
    def _configure_connection(self):
        ...
        aws_config = {'aws_access_key': self.access_key, 'aws_secret_key': self.secret_key}
    ...
    def file_ready(self, obj, **kwargs):
        ...
        if self._in_cache(rel_path):
            if os.path.getsize(self._get_cache_path(rel_path)) == self._get_size_in_cloud(rel_path):
                return True
            log.debug('Waiting for dataset %s to transfer from OS: %s/%s', rel_path, os.path.getsize(self._get_cache_path(rel_path)), self._get_size_in_cloud(rel_path))
        return False
