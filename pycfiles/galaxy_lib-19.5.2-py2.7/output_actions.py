...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/parser/output_actions.py
...
class ToolOutputActionOption(object):
    ...
    def from_elem(cls, parent, elem):
        """Loads the proper action by the type attribute of elem"""
        if elem is None:
            option_type = NullToolOutputActionOption.tag
        else:
...
class FromFileToolOutputActionOption(ToolOutputActionOption):
    ...
    def __init__(self, parent, elem):
        ...
        if not self.column is not None:
            raise AssertionError("Required 'column' attribute missing from FromFileToolOutputActionOption")
            ...
            self.options = []
            data_file = self.name
            data_file = os.path.isabs(data_file) or os.path.join(self.tool.app.config.tool_data_path, data_file)
...
class StringFunctionFilter(ToolOutputActionOptionFilter):
    ...
    def __init__(self, parent, elem):
        ...
        assert self.function in ('lower', 'upper'), "Required function 'name' missing or invalid from StringFunctionFilter"
