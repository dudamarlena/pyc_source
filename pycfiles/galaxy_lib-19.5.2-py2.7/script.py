...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/verify/script.py
...
def main(argv=None):
    ...
    galaxy_interactor_kwds = {'galaxy_url': args.galaxy_url, 
    ...
    for test_index in test_indices:
        ...
        def register(job_data):
            test_results.append({'id': tool_id + '-' + str(test_index), 
               'has_data': True, 
               'data': job_data})

        try:
            verify_tool(tool_id, galaxy_interactor, test_index=test_index, tool_version=tool_version, register_job_data=register, quiet=not verbose, force_path_paste=args.force_path_paste)
            if verbose:
