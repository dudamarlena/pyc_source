...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/container_resolvers/mulled.py
# Compiled at: 2019-04-28 04:54:30
"""This module describes the :class:`MulledContainerResolver` ContainerResolver plugin."""
import collections, logging, os, subprocess, six
...
from ..mulled.mulled_build import DEFAULT_CHANNELS, ensure_installed, InvolucroContext, mull_targets
            ...
            for cached_image in cached_images:
                ...
                if version_hash is None:
                    if package_hash == cached_image.package_hash:
                        image = cached_image
                        break
                elif package_hash == cached_image.package_hash and version_hash == cached_image.version_hash:
                    image = cached_image
                    break
...
def docker_cached_container_description(targets, namespace, hash_func='v2', shell=DEFAULT_CONTAINER_SHELL):
    if len(targets) == 0:
        ...
        container = None
        if image:
            container = ContainerDescription(image.image_identifier, type='docker', shell=shell)
        return container
...
def singularity_cached_container_description(targets, cache_directory, hash_func='v2', shell=DEFAULT_CONTAINER_SHELL):
    if len(targets) == 0:
        ...
        container = None
        if image:
            container = ContainerDescription(os.path.abspath(os.path.join(cache_directory, image.image_identifier)), type='singularity', shell=shell)
        return container
    ...
    def resolve(self, enabled_container_types, tool_info, install=False, **kwds):
        ...
        if len(targets) == 0:
            ...
            if name:
                ...
                container_description = ContainerDescription(container_id, type=self.container_type, shell=self.shell)
                destination_for_container_type = kwds.get('destination_for_container_type')
                if install and destination_for_container_type and not self.cached_container_description(targets, namespace=self.namespace, hash_func=self.hash_func):
                    container = CONTAINER_CLASSES[self.container_type](container_description.identifier, self.app_info, tool_info, destination_for_container_type(self.container_type), {}, container_description)
                    self.pull(container)
                if not self.auto_install:
                    container_description = self.cached_container_description(targets, namespace=self.namespace, hash_func=self.hash_func)
...
class MulledSingularityContainerResolver(MulledDockerContainerResolver):
    ...
    def cached_container_description(self, targets, namespace, hash_func):
        return singularity_cached_container_description(targets, cache_directory=self.cache_directory, hash_func=hash_func)
...
class BuildMulledDockerContainerResolver(ContainerResolver):
    ...
    def __init__(self, app_info=None, namespace='local', hash_func='v2', auto_install=True, **kwds):
        ...
        self.hash_func = hash_func
        self.auto_install = string_as_bool(auto_install)
        self._mulled_kwds = {'namespace': namespace, 
...
class BuildMulledSingularityContainerResolver(ContainerResolver):
    ...
    def __init__(self, app_info=None, hash_func='v2', auto_install=True, **kwds):
        ...
        self.hash_func = hash_func
        self.auto_install = string_as_bool(auto_install)
        self._mulled_kwds = {'channels': self._get_config_option('mulled_channels', DEFAULT_CHANNELS), 
           'hash_func': self.hash_func, 
           'command': 'build-and-test', 
           'singularity': True, 
...
__all__ = ('CachedMulledDockerContainerResolver', 'CachedMulledSingularityContainerResolver',
           'MulledDockerContainerResolver', 'MulledSingularityContainerResolver',
