...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/views.py
...
class DependencyResolversView(object):
    ...
    def show_dependencies(self, tool_requirements_d, installed_tool_dependencies=None):
        ...
        """
        kwds = {'install': False, 'return_null': True, 
    ...
    def tool_ids_by_requirements(self):
        ...
        for tid, tool in self._app.toolbox.tools_by_id.items():
            if tool.tool_requirements not in tool_ids_by_requirements:
                tool_ids_by_requirements[tool.tool_requirements] = [
                 tid]
            else:
    ...
    def toolbox_requirements_status(self):
        return {r:self.get_requirements_status(tool_requirements_d={tids[0]: r}, installed_tool_dependencies=self._app.toolbox.tools_by_id[tids[0]].installed_tool_dependencies) for r, tids in self.tool_ids_by_requirements.items()}
