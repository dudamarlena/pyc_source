...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/toolbox/base.py
...
from galaxy.util import ExecutionTimer, listify, parse_xml, string_as_bool
...
ToolConfRepository = namedtuple('ToolConfRepository', (
...
class AbstractToolBox(Dictifiable, ManagesIntegratedToolPanelMixin):
    ...
    def _init_tools_from_config(self, config_filename):
        ...
        for item in tool_conf_source.parse_items():
            ...
            self.load_item(item, tool_path=tool_path, load_panel_dict=load_panel_dict, guid=item.get('guid'), index=index, internal=True)

        if parsing_shed_tool_conf:
            shed_tool_conf_dict = dict(config_filename=config_filename, tool_path=tool_path, config_elems=config_elems)
    ...
    def get_section(self, section_id, new_label=None, create_if_needed=False):
        ...
        if tool_panel_section_key in self._tool_panel:
            ...
            section_dict = {'name': new_label, 'id': section_id, 
               'version': ''}
        ...
        if tool.hidden:
            ...
            if related_tool:
                if self._newer_tool(tool, related_tool):
                    panel_dict.replace_tool(previous_tool_id=related_tool.id, new_tool_id=tool_id, tool=tool)
                ...
                if not inserted:
                    ...
                    if not inserted:
                        if tool.guid is None or tool.tool_shed is None or tool.repository_name is None or tool.repository_owner is None or tool.installed_changeset_revision is None:
    ...
    def _load_tool_panel(self):
        ...
        for key, item_type, val in self._integrated_tool_panel.panel_items_iter():
            if item_type == panel_item_types.TOOL:
                ...
                section_dict = {'id': val.id or '', 'name': val.name or '', 
    ...
    def get_tool(self, tool_id, tool_version=None, get_all_versions=False, exact=False):
        ...
        for tool_id in tool_ids:
            if tool_id in self._tools_by_id and not get_all_versions:
                ...
                if tool_version in self._tool_versions_by_id[tool_id]:
    ...
    def get_tool_repository_from_xml_item(self, item, path):
        ...
        repository = self._get_tool_shed_repository(tool_shed=tool_shed, name=repository_name, owner=repository_owner, installed_changeset_revision=installed_changeset_revision)
        if not repository:
            msg = "Attempted to load tool shed tool, but the repository with name '%s' from owner '%s' was not found in database. Tool will be loaded without install database."
            log.warning(msg, repository_name, repository_owner)
            repository = ToolConfRepository(tool_shed, repository_name, repository_owner, installed_changeset_revision, installed_changeset_revision, None)
    ...
    def _load_section_tag_set(self, item, tool_path, load_panel_dict, index=None, internal=False):
        ...
        for sub_index, sub_item in enumerate(item.items):
            self.load_item(sub_item, tool_path=tool_path, panel_dict=elems, integrated_panel_dict=integrated_elems, load_panel_dict=load_panel_dict, guid=sub_item.get('guid'), index=sub_index, internal=internal)
    ...
    def find_section_id(self, tool_panel_section_id):
        ...
        elif tool_panel_section_id not in self._tool_panel:
