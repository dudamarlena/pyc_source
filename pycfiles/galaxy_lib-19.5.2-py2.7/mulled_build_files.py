...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/mulled/mulled_build_files.py
...
from .mulled_build import add_build_arguments, args_to_mull_targets_kwds, BuildExistsException, mull_targets, target_str_to_targets
...
def main(argv=None):
    ...
    parser.add_argument('files', metavar='FILES', default='.', help='Path to directory (or single file) of TSV files describing composite recipes.')
    args = parser.parse_args()
    for targets, image_build, name_override in generate_targets(args.files):
        try:
            ret = mull_targets(targets, image_build=image_build, name_override=name_override, **args_to_mull_targets_kwds(args))
