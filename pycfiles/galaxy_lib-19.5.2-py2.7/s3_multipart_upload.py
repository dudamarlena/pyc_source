...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/objectstore/s3_multipart_upload.py
...
"""
import contextlib, functools, glob, multiprocessing, os, subprocess
...
def mp_from_ids(s3server, mp_id, mp_keyname, mp_bucketname):
    ...
    """
    if s3server['host']:
        conn = boto.connect_s3(aws_access_key_id=s3server['access_key'], aws_secret_access_key=s3server['secret_key'], is_secure=s3server['is_secure'], host=s3server['host'], port=s3server['port'], calling_format=boto.s3.connection.OrdinaryCallingFormat(), path=s3server['conn_path'])
    else:
...
def multipart_upload(s3server, bucket, s3_key_name, tarball, mb_size):
    ...
    def split_file(in_file, mb_size, split_num=5):
        ...
        if not os.path.exists('%saa' % prefix):
            cl = [
             'split', '-b%sm' % split_size, in_file, prefix]
