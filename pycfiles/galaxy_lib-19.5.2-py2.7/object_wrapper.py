...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/util/object_wrapper.py
# Compiled at: 2018-09-15 08:40:24
"""
Classes for wrapping Objects and Sanitizing string output.
"""
from __future__ import absolute_import
import collections, inspect, logging, string, sys
from numbers import Number
from types import BuiltinFunctionType, BuiltinMethodType, CodeType, FrameType, FunctionType, GeneratorType, GetSetDescriptorType, MemberDescriptorType, MethodType, ModuleType, TracebackType
try:
...
__WRAP_NO_SUBCLASS__ = (
 ModuleType, XRangeType, SliceType, BufferType, TracebackType, FrameType, DictProxyType,
 GetSetDescriptorType, MemberDescriptorType) + __CALLABLE_TYPES__
__DONT_SANITIZE_TYPES__ = (
 Number, bool, NoneType, NotImplementedType, EllipsisType, bytearray)
__DONT_WRAP_TYPES__ = tuple()
__WRAP_SEQUENCES__ = (
 tuple, list)
__WRAP_SETS__ = (set, frozenset)
__WRAP_MAPPINGS__ = (dict, UserDict)
VALID_CHARACTERS = set(string.ascii_letters + string.digits + ' -=_.()/+*^,:?!@')
CHARACTER_MAP = {'>': '__gt__', '<': '__lt__', 
   "'": '__sq__', 
   '"': '__dq__', 
   '[': '__ob__', 
   ']': '__cb__', 
    ...
    def __do_wrap(value):
        ...
        if wrapped_class not in (SafeStringWrapper, CallableSafeStringWrapper):
            ...
            def pickle_safe_object(safe_object):
                return (
                 wrapped_class, (safe_object.unsanitized, do_wrap_func))
