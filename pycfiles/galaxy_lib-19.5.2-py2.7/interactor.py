...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/verify/interactor.py
# Compiled at: 2019-04-28 04:54:30
from __future__ import absolute_import
from __future__ import print_function
import os, re, shutil, sys, tarfile, tempfile, time
...
class GalaxyInteractorApi(object):
    ...
    def verify_output_dataset(self, history_id, hda_id, outfile, attributes, tool_id):
        ...
        verify_hid(outfile, hda_id=hda_id, attributes=attributes, dataset_fetcher=fetcher, test_data_downloader=test_data_downloader, keep_outputs_dir=self.keep_outputs_dir)
    ...
    def stage_data_async(self, test_data, history_id, tool_id, force_path_paste=False):
        ...
        tool_input = {'file_type': test_data['ftype'], 
        ...
        if composite_data:
            ...
            for i, file_name in enumerate(composite_data):
                if force_path_paste:
                    ...
                    tool_input.update({'files_%d|url_paste' % i: 'file://' + file_path})
                else:
                    file_content = self.test_data_download(tool_id, file_name)
                    files['files_%s|file_data' % i] = file_content
                tool_input.update({'files_%d|type' % i: 'upload_dataset'})

            name = test_data['name']
        else:
            name = fname
            tool_input.update({'files_0|NAME': name, 
               'files_0|type': 'upload_dataset'})
            files = {}
            if force_path_paste:
                file_name = self.test_data_path(tool_id, fname)
                tool_input.update({'files_0|url_paste': 'file://' + file_name})
            else:
    ...
    def run_tool(self, testdef, history_id, resource_parameters={}):
        ...
        for key, value in inputs_tree.items():
            values = [
             isinstance(value, list) or value] if 1 else value
            new_values = []
            for value in values:
        ...
        try:
            return Bunch(inputs=inputs_tree, outputs=self.__dictify_outputs(submit_response_object), output_collections=self.__dictify_output_collections(submit_response_object), jobs=submit_response_object['jobs'])
        except KeyError:
    ...
    def _create_collection(self, history_id, collection_def):
        create_payload = dict(name=collection_def.name, element_identifiers=dumps(self._element_identifiers(collection_def)), collection_type=collection_def.collection_type, history_id=history_id)
    ...
    def _element_identifiers(self, collection_def):
        ...
        for element_dict in collection_def.elements:
            ...
            if isinstance(element_def, TestCollectionDef):
                ...
                element = dict(name=element_identifier, src='new_collection', collection_type=element_def.collection_type, element_identifiers=subelement_identifiers)
            else:
    ...
    def __submit_tool(self, history_id, tool_id, tool_input, extra_data={}, files=None):
        data = dict(history_id=history_id, tool_id=tool_id, inputs=dumps(tool_input), **extra_data)
    ...
    def __inject_api_key(self, data, key, admin, anon):
        ...
        if not anon:
            if not key:
                key = (admin or self).api_key if 1 else self.master_api_key
            params['key'] = key
        return (
         params, data)
...
def verify_hid(filename, hda_id, attributes, test_data_downloader, hid='', dataset_fetcher=None, keep_outputs_dir=False):
    ...
    verify(item_label, data, attributes=attributes, filename=filename, get_filecontent=test_data_downloader, keep_outputs_dir=keep_outputs_dir, verify_extra_files=verify_extra_files)
    return
...
def _verify_composite_datatype_file_content(file_name, hda_id, base_name=None, attributes=None, dataset_fetcher=None, test_data_downloader=None, keep_outputs_dir=False, mode='file'):
    ...
    try:
        verify(item_label, data, attributes=attributes, filename=file_name, get_filecontent=test_data_downloader, keep_outputs_dir=keep_outputs_dir, mode=mode)
...
def verify_tool(tool_id, galaxy_interactor, resource_parameters=None, register_job_data=None, test_index=0, tool_version=None, quiet=False, test_history=None, force_path_paste=False):
    ...
    try:
        ...
        if not (expected_failure_occurred or data_list or data_collection_list):
...
def _verify_outputs(testdef, history, jobs, tool_id, data_list, data_collection_list, galaxy_interactor, quiet=False):
    if not len(jobs) == 1:
        raise AssertionError('Test framework logic error, somehow tool test resulted in more than one job.')
        job = jobs[0]
        ...
        if testdef.expect_failure and testdef.outputs:
    ...
    other_checks = {'command_line': 'Command produced by the job', 'stdout': 'Standard output of the job', 
            ...
            def verify_elements(element_objects, element_tests):
                for element_identifier, (element_outfile, element_attrib) in element_tests.items():
                    ...
                    if element_type != 'dataset_collection':
                        hda = element['object']
                        galaxy_interactor.verify_output_dataset(history, hda_id=hda['id'], outfile=element_outfile, attributes=element_attrib, tool_id=tool_id)
                    if element_type == 'dataset_collection':
...
class ToolTestDescription(object):
    ...
    def to_dict(self):
        ...
        return {'inputs': inputs_dict, 'outputs': self.outputs, 
...
def test_data_iter(required_files):
    for fname, extra in required_files:
        data_dict = dict(fname=fname, metadata=extra.get('metadata', {}), composite_data=extra.get('composite_data', []), ftype=extra.get('ftype', DEFAULT_FTYPE), dbkey=extra.get('dbkey', DEFAULT_DBKEY))
