...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/parser/output_objects.py
...
class ToolOutput(ToolOutputBase):
    ...
    def __init__(self, name, format=None, format_source=None, metadata_source=None, parent=None, label=None, filters=None, actions=None, hidden=False, implicit=False):
...
class ToolOutputCollection(ToolOutputBase):
    ...
    def __init__(self, name, structure, label=None, filters=None, hidden=False, default_format='data', default_format_source=None, default_metadata_source=None, inherit_format=False, inherit_metadata=False):
    ...
    def known_outputs(self, inputs, type_registry):
        ...
        if len(self.outputs) > 1:
            ...
            def prototype_dataset_element_to_output(element, parent_ids=[]):
                ...
                output = ToolOutput(name, format=format, format_source=self.format_source, metadata_source=self.metadata_source, implicit=True)
                if self.inherit_metadata:
                    output.metadata_source = element.dataset_instance
                return ToolOutputCollectionPart(self, element.element_identifier, output, parent_ids=parent_ids)
...
class ToolOutputCollectionStructure(object):
    ...
    def __init__(self, collection_type, collection_type_source=None, collection_type_from_rules=None, structured_like=None, dataset_collector_descriptions=None):
