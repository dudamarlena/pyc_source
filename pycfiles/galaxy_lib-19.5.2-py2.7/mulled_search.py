...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/galaxy/tools/deps/mulled/mulled_search.py
...
class QuaySearch:
    ...
    def search_repository(self, search_string, non_strict):
        ...
        with self.index.searcher() as (searcher):
            ...
            sys.stdout.write('The query \x1b[1m %s \x1b[0m resulted in %s result(s).\n' % (search_string, len(results)))
            if non_strict:
...
def main(argv=None):
    ...
    parser.add_argument('-o', '--organization', dest='organization_string', default='biocontainers', help='Change organization. Default is biocontainers.')
    parser.add_argument('--non-strict', dest='non_strict', action='store_true', help='Autocorrection of typos activated. Lists more results but can be confusing.                        For too many queries quay.io blocks the request and the results can be incomplete.')
