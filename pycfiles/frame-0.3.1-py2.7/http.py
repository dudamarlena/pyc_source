...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/server/http.py
# Compiled at: 2013-08-10 12:52:27
import socket, threading
from headers import RequestHeaders, RequestLine
import modulemonitor, signal, sys, os, traceback, cgi
...
class Connection(object):
    ...
    def handle_request(self, request):
        if not request:
            ...
            wsgi_environ = {'wsgi.multiprocess': False, 
            ...
            other_headers = {'SERVER_ADDR': self.server.host, 
               ...
               'SCRIPT_NAME': '', 
               'REDIRECT_STATUS': 200}
            all_headers = dict(self.request_headers.items() + wsgi_environ.items() + other_headers.items() + uri_headers.items())
            try:
                try:
...
class HTTPServer(object):
    ...
    def __init__(self, app, host='localhost', port=8080, listen=5, max_read=8092, chunk_size=1024, auto_reload=True):
        ...
        self.environment = Environment(loader=ChoiceLoader([PackageLoader('frame', 'templates'),
