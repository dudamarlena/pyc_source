...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/_app.py
...
import driverdatabase, contextlib
...
class App(Singleton):
  ...
  """
    ...
    def _setup_driver_database(self):
        ...
        drivers.add_interface('postprocessor', driverdatabase.PostprocessorInterface)
        drivers.add_interface('preprocessor', driverdatabase.PreprocessorInterface)
        drivers.add_interface('dispatcher', driverdatabase.DispatcherInterface, config=config.application)
        drivers.add_interface('hook', driverdatabase.DriverInterface, config=config.hooks)
        drivers.add_interface('init_hook', driverdatabase.DriverInterface, config=config.init_hooks)
        return drivers
    ...
    """
        ...
        hooks = map(lambda x: self.drivers.hook.load_driver(x, self, match.im_self), config.hooks)
