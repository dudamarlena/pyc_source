...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/_logger.py
...
class Logger(object):
    ...
    def populate_format_data(self, request, response, response_length):
        ...
        return {'remote_host': headers.remote_addr if 'remote_addr' in headers else '-', 
           'timestamp': timestamp, 
           'request_line': '%s %s %s' % (
    ...
    def log_request(self, request, response, response_length):
        ...
        self.out.write('%(remote_host)s [%(timestamp)s] "%(request_line)s" %(status_code)s %(body_size)s "%(referer)s" "%(user_agent)s"\n' % format_data)
...
class LogInterface(DriverInterface):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.update({'stdout': StdoutLogger, 
           'null': NullLogger, 
