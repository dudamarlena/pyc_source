...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/extensions/templates.py
...
import os
from .. import errors
templates_config = DotDict({'driver': 'jinja2', 
   'globals': {}, 'filters': {}, 'jinja2': {'environment': None}, 
   'loaders': [
...
class Jinja2Driver(TemplateDriver):
    ...
    def __init__(self, **options):
        ...
        if templates_config.jinja2.environment:
            ...
            self.environment = Environment(loader=ChoiceLoader(loaders), extensions=options['extensions'])
...
def templatize(request, response):
    if isinstance(response.body, dict) or response.body is None:
        ...
        template_path = os.path.join(class_name.lower(), action_name + templates_config.suffix)
        try:
            response.body = response.app.template_engine.render(template_path, response.body or {})
...
def HTTPError_setup_response(self, status, headers, body):
    ...
    if not body:
        try:
            body = app.template_engine.render('__errors/jinja2/%s.html' % split[0], self.kwargs)
        except jinja2.exceptions.TemplateNotFound:
            body = app.template_engine.render('__errors/jinja2/generic.html', self.kwargs)
        except Exception as e:
...
def register_driver(drivers):
    ...
    drivers.add_interface('template', TemplateInterface, config=templates_config, drivers={'jinja2': Jinja2Driver})
    errors.HTTPError.setup_response = HTTPError_setup_response

    def init_hook(app):
        current_driver = templates_config.driver
        app.template_engine = drivers.template.load_current(loaders=templates_config.loaders, globals=templates_config.globals, filters=templates_config.filters, suffix=templates_config.suffix, directory=templates_config.directory, extensions=templates_config.extensions)
