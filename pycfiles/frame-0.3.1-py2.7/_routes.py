...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/_routes.py
    ...
    def match(self, *args, **kwargs):
        ...
        if match:
            if not hasattr(match['action'], '__call__'):
                ...
                action = match['action']
                controller = None
            self.thread_data['current_route'] = {'controller': controller, 
               'action': action}
            return (
             action, match)
        else:
    ...
    def resource(self, controller, mount_point=None):
        ...
        uri_map = [
         ...
         {'uri': '%s/{slug}/edit', 
            ...
            'method': 'GET'}]
        for i in uri_map:
            self.connect(path=i['uri'] % mount_point, controller='%s#%s' % (controller, i['action']), conditions={'method': i['method']})
