...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/staticgenerator.py
...
class StaticResponse(object):
    ...
    def save(self, path=None, filename='index.html', overwrite=False):
        if not path:
            path = os.path.join(self.generator.path, self.uri.lstrip('/'), filename)
...
class StaticGenerator(object):
    ...
    def generate(self, uris, environ=None, link_static_maps=True):
        if not environ:
            environ = {'REQUEST_METHOD': 'GET', 'SCRIPT_NAME': '', 
