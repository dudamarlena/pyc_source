...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/frame/orm/datatypes.py
...
class CustomType(object):
    ...
    def make_form_element(self, title, key, value=None, new=False, failed=False):
        return self._environment.get_template(self.template).render(key=key, title=title, value=value, arguments=self.kwargs, new=new, failed=failed)
...
def make_form_element(title, key, value=None, new=False, failed=False):
    return CustomType._environment.get_template('forms/elements/generic_element.html').render(key=key, title=title, value=value, new=new, failed=failed)
...
class SubmitType(CustomType):
    ...
    def make_form_element(self):
        return self._environment.get_template('forms/elements/submit.html').render(label=self.label, arguments=self.kwargs)
...
class BoolType(CustomType):
    ...
    def make_form_element(self, title, key, new=False, value=None, failed=False):
        return self._environment.get_template('forms/elements/%s.html' % self.style).render(key=key, value=value, title=key.title(), choices=self.choices, default=self.default, new=new, failed=failed)
...
class EmailType(CustomType):
    ...
    def make_form_element(self, title, key, value=None, new=False, failed=False):
        return self._environment.get_template('forms/elements/email.html').render(key=key, value=value, title=key.title(), new=new, failed=failed)
