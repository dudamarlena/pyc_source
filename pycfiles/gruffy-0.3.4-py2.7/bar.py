...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gruffy/bar.py
# Compiled at: 2013-01-27 08:35:04
...
class Bar(base.Base):
    ...
    def _draw_bars(self):
        ...
        self.bar_width = self.graph_width / float(self.column_count * len(self.gdata))
        ...
        if self.minimum_value >= 0:
            ...
            for row_index, data_row in enumerate(self.norm_data):
                for point_index, data_point in enumerate(data_row['values']):
                    left_x = self.graph_left + self.bar_width * (row_index + point_index + (len(self.gdata) - 1) * point_index) + padding
                    ...
                    label_center = self.graph_left + len(self.gdata) * self.bar_width * point_index + len(self.gdata) * self.bar_width / 2.0
