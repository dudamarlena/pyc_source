...
class Dot(base.Base):
    ...
    def draw(self):
        ...
        self.item_width = self.items_width * spacing_factor / len(self.norm_data)
        dl = DrawableList()
        ...
        for row_index, data_row in enumerate(self.norm_data):
            for point_index, data_point in enumerate(data_row['values']):
                x_pos = self.graph_left + data_point * self.graph_width - round(float(self.item_width) / 6.0)
                y_pos = self.graph_top + self.items_width * point_index + padding + round(float(self.item_width) / 2.0)
                if row_index == 0:
                ...
                dl.append(DrawableCircle(x_pos, y_pos, x_pos + round(float(self.item_width) / 3.0), y_pos))
    ...
    def draw_line_markers(self):
        ...
        for index in range(number_of_lines + 1):
            ...
            dl.append(DrawableLine(x, self.graph_bottom, x, self.graph_bottom + 0.5 * base.LABEL_MARGIN))
