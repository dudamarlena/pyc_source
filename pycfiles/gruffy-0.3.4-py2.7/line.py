...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gruffy/line.py
# Compiled at: 2013-01-27 09:12:36
from gruffy import base
from pgmagick import Color, DrawableCircle, DrawableFillColor, DrawableFillOpacity, DrawableLine, DrawableList, DrawableScaling, DrawableStrokeColor, DrawableDashArray, DrawableStrokeOpacity, DrawableStrokeWidth
        ...
        if not self.has_gdata:
            ...
            for data_row in self.norm_data:
                ...
                for index, data_point in enumerate(data_row['values']):
                    ...
                    dl.append(DrawableStrokeWidth(self.line_width or self.clip_value_if_greater_than(self.columns / (len(self.norm_data[0]['values']) * 4), 5.0)))
                    circle_radius = self.dot_radius or self.clip_value_if_greater_than(self.columns / (len(self.norm_data[0]['values']) * 2.5), 5.0)
