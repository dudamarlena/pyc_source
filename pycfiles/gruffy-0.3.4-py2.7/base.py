...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gruffy/base.py
# Compiled at: 2013-01-27 08:27:28
import math, os
from pgmagick import Geometry, Image, Color, DrawableStrokeAntialias, DrawableList, DrawableFillColor, DrawableLine, DrawableFont, DrawableStrokeColor, DrawablePointSize, DrawableGravity, DrawableText, DrawableScaling, DrawableRotation, DrawableRectangle, TypeMetric, GravityType, StyleType, StretchType
DATA_COLOR_INDEX = 2
...
class Base(object):
    ...
    def _initialize_ivars(self):
        ...
        if 'MAGICK_FONT_PATH' in os.environ:
            vera_font_path = os.path.join(os.environ['MAGICK_FONT_PATH'], DEFAULT_FONT)
        else:
    ...
    def theme_keynote(self):
        ...
        self.set_theme({'colors': self.colors, 'marker_color': 'white', 
           'font_color': 'white', 
           'background_colors': [
                               'black', '#4a465a']})
    ...
    def theme_37signals(self):
        ...
        self.set_theme({'colors': self.colors, 'marker_color': 'black', 
           'font_color': 'black', 
           'background_colors': [
                               '#d1edf5', 'white']})
    ...
    def theme_pastel(self):
        ...
        self.colors = [
         ...
         '#daaeda',
         '#dadada']
        self.set_theme({'colors': self.colors, 'marker_color': '#aea9a9', 
           'font_color': 'black', 
    ...
    def theme_django(self):
        ...
        self.colors = [
         '#a9dada', '#aedaa9', '#94da3a',
         '#487854', '#a9a9da', '#dadada']
        self.set_theme({'colors': self.colors, 'marker_color': '#ab5603', 
           'font_color': 'white', 
           'background_colors': [
                               '#092e20', '#234f32']})
    ...
    def render_gradiated_background(self, top_color, bottom_color):
        im = Image(Geometry(int(self.columns), int(self.rows)), Color('transparent'))
    ...
    def set_theme(self, options):
        ...
        defaults = {'colors': [
                    'black', 'white'], 
           'additional_line_colors': [], 'marker_color': 'white', 
           'font_color': 'black', 
    ...
    def data(self, name, data_points=[], color=None):
        ...
        self.gdata.append({'label': name, 'values': data_points, 
        ...
        if self.hide_line_markers:
            ...
            for index in range(self.marker_count + 1):
                ...
                if not self.hide_line_numbers:
                    ...
                    dl.append(DrawableText(self.graph_left - LABEL_MARGIN - x, y, self.label(marker_label)))
    ...
    def draw_label(self, x_offset, index):
        ...
        if index in self.labels and index not in self.labels_seen:
            ...
            label_text_width = self.calculate_width(self.marker_font_size, self.labels[index])
            dl.append(DrawableText(x_offset - label_text_width / 2.0, y_offset + label_font_height, self.labels[index]))
            dl.append(DrawableScaling(self.scale, self.scale))
            self.base_image.draw(dl)
    ...
    def draw_no_data(self):
        ...
        dl.append(DrawableFillColor(Color(self.font_color)))
        dl.append(DrawableFont(font, StyleType.NormalStyle, 800, StretchType.NormalStretch))
    ...
    def draw_axis_labels(self):
        if self.x_axis_label:
            dl = DrawableList()
            x_axis_label_y_coordinate = self.graph_bottom + LABEL_MARGIN * 2 + self.marker_caps_height
            dl.append(DrawableFillColor(Color(self.font_color)))
            font = self.font if self.font else DEFAULT_FONT
            dl.append(DrawableFont(font, StyleType.NormalStyle, 400, StretchType.NormalStretch))
            dl.append(DrawableStrokeColor('transparent'))
            ...
            dl.append(DrawableText(0.0, x_axis_label_y_coordinate, self.x_axis_label))
            dl.append(DrawableScaling(self.scale, self.scale))
            self.base_image.draw(dl)
    ...
    def draw_legend(self):
        ...
        for index, legend_label in enumerate(self.legend_labels):
            ...
            dl.append(DrawableRectangle(current_x_offset, current_y_offset - legend_square_width / 2.0, current_x_offset + legend_square_width, current_y_offset + legend_square_width / 2.0))
            dl.append(DrawableFillColor(Color(self.font_color)))
            font = self.font if self.font else DEFAULT_FONT
            dl.append(DrawableFont(font, StyleType.NormalStyle, 400, StretchType.NormalStretch))
            dl.append(DrawablePointSize(self.legend_font_size))
    ...
    def normalize(self, force=False):
        if self.norm_data is None or force:
            ...
            for data_row in self.gdata:
                ...
                self.norm_data.append({'label': data_row['label'], 'values': norm_data_points, 
    ...
    def setup_graph_measurements(self):
        ...
        if self.hide_line_markers:
            self.graph_left, self.graph_right_margin, self.graph_bottom_margin = self.left_margin, self.right_margin, self.bottom_margin
        else:
            ...
            if self.has_left_labels:
                ...
                longest_left_label_width = self.calculate_width(self.marker_font_size, longest_value) * 1.25
            else:
                longest_left_label_width = self.calculate_width(self.marker_font_size, self.label(float(self.maximum_value)))
            ...
            if len(tmp):
                ...
                last_label = 0
            if last_label >= self.column_count - 1 and self.center_labels_over_point:
                extra_room_for_long_label = self.calculate_width(self.marker_font_size, self.labels[last_label]) / 2.0
            else:
