...
# [GCC 8.4.0]
# Embedded file name: /var/www/django-hstore/django_hstore/query.py
# Compiled at: 2015-12-07 14:14:02
# Size of source mod 2**32: 10909 bytes
...
class HStoreWhereNode(WhereNode):
    ...
    def make_atom(self, child, qn, connection):
        ...
        if lvalue:
            if lvalue.field and hasattr(lvalue.field, 'db_type') and lvalue.field.db_type(**kwargs) == 'hstore':
                try:
                        ...
                        if lookup_type in ('contains', 'icontains'):
                            if isinstance(param, dict):
                                ...
                                if len(values) == 1 and isinstance(values[0], (list, tuple)):
                                    return (
                                     "%s->'%s' = ANY(%%s)" % (field, keys[0]), [[str(x) for x in values[0]]])
                                if len(keys) == 1 and len(values) == 1:
                                    cast = get_cast_for_param(value_annot, keys[0])
                                    return (
                                     "(%s->'%s')%s = %%s" % (field, keys[0], cast), [values[0]])
                            ...
                            if isinstance(param, (list, tuple)):
                                ...
                                if isinstance(param, six.string_types):
                                    ...
                                    if hasattr(child[0].field, 'serializer'):
                                        try:
