...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/imgtags/adapter.py
# Compiled at: 2016-07-15 10:47:31
import chardet
...
from ztfy.imgtags.interfaces import IImageTags, IImageTagString, IImageTagIndexValue, IBaseTagInfo, IExifTag, IIptcTag, IXmpTag
...
class ImageTagsAdapter(object):
    ...
    def getGPSLocation(self, precision=None):
        ...
        if 'Exif.GPSInfo.GPSLongitude' in keys and 'Exif.GPSInfo.GPSLatitude' in keys:
            lon = gpsToWgs(self.getTag('Exif.GPSInfo.GPSLongitude'))
            lon_ref = self.getTag('Exif.GPSInfo.GPSLongitudeRef')
            if lon_ref and lon_ref.value != 'E':
                lon = -lon
            lat = gpsToWgs(self.getTag('Exif.GPSInfo.GPSLatitude'))
            lat_ref = self.getTag('Exif.GPSInfo.GPSLatitudeRef')
            if lat_ref and lat_ref.value != 'N':
                lat = -lat
        if precision:
            if lon:
...
class DefaultTagStringAdapter(object):
    ...
    def toString(self, encoding='utf-8'):
        try:
            return (
             self.context.name or self.context.key,
...
class ExifTagStringAdapter(object):
    ...
    def toString(self, encoding='utf-8'):
        try:
            return (
             self.context.label or self.context.name or self.context.key,
...
class IptcTagStringAdapter(object):
    ...
    def toString(self, encoding='utf-8'):
        try:
            return (
             self.context.title or self.context.name or self.context.key,
