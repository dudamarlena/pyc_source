...
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/tests/api_v2/test_os.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 48845 bytes
import unittest
...
class AddUsrLocalBinToPathTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_add_usr_local_bin_raises_exception_on_failure(self, *args, **kwargs):
...
class DirAttribsTests(DockerBasedTests):
    ...
    def test_dir_attribs_with_sudo_updates_perms(self, *args, **kwargs):
        ...
        sudo('echo > /tmp/temp1')
        os_helpers.dir_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=True)
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_attribs_with_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        os_helpers.dir_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
    ...
    def test_dir_attribs_without_sudo_updates_perms(self, *args, **kwargs):
        ...
        sudo('/usr/sbin/groupadd group1')
        sudo('echo > /tmp/temp1')
        os_helpers.dir_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=False)
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_attribs_without_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        os_helpers.dir_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=False)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, 'drwxr-xr-x')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    def test_dir_attribs_returns_True_on_sucess(self, *args, **kwargs):
        ...
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        result = os_helpers.dir_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
    ...
    def test_dir_attribs_raises_exception_on_failure(self, *args, **kwargs):
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.dir_attribs(location='/tmp/no-valid-path', mode='755', owner='not-a-valid-user', group='not-a-valid-group', recursive=True, use_sudo=False)
        self.assertEqual(cm.exception.code, 1)
...
class DirEnsureTests(DockerBasedTests):
    ...
    def test_dir_ensure_creates_dir_if_missing(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=False)
    ...
    def test_dir_ensure_creates_dir_if_missing_as_root(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=True)
        self.assertTrue(sudo('ls -d /tmp/temp1'))
    ...
    def test_dir_ensure_creates_dir_recursive_if_missing(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/level1/level2', mode='755', owner='user1', group='group1', recursive=True, use_sudo=False)
        self.assertTrue(sudo('ls -d /tmp/level1/level2'))

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_ensure_creates_dir_recursive_if_missing_as_root(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/level1/level2', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
        self.assertTrue(sudo('ls -d /tmp/level1/level2'))
    ...
    def test_dir_ensure_with_sudo_updates_perms(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/temp1', mode='700', owner='user1', group='group1', recursive=False, use_sudo=True)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_ensure_with_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/level1')
        sudo('mkdir /tmp/level1/level2')
        os_helpers.dir_ensure(location='/tmp/level1', mode='700', owner='user1', group='group1', recursive=True, use_sudo=True)
    ...
    def test_dir_ensure_without_sudo_updates_perms(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        os_helpers.dir_ensure(location='/tmp/temp1', mode='700', owner='user1', group='group1', recursive=False, use_sudo=False)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, 'drwx------')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_ensure_without_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir -p /tmp/level1/level2')
        os_helpers.dir_ensure(location='/tmp/level1', mode='700', owner='user1', group='group1', recursive=True, use_sudo=False)
        for folder in ['/tmp/level1', '/tmp/level1/level2']:
            perms, x, owner, group = sudo('ls -ld %s' % folder).split(' ')[0:4]
            self.assertEquals(perms, 'drwx------')
            self.assertEquals(owner, 'user1')
    ...
    def test_dir_ensure_returns_True_on_sucess(self, *args, **kwargs):
        ...
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/temp1')
        result = os_helpers.dir_ensure(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
        self.assertTrue(result)
    ...
    def test_dir_ensure_raises_exception_on_failure(self, *args, **kwargs):
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.dir_ensure(location='/tmp/no-valid-path', mode='755', owner='not-a-valid-user', group='not-a-valid-group', recursive=True, use_sudo=False)
        self.assertEqual(cm.exception.code, 1)
...
class DirExistsTests(DockerBasedTests):
    ...
    def test_dir_exists_returns_True(self, *args, **kwargs):
        sudo('mkdir /tmp/temp1')
        self.assertTrue(os_helpers.dir_exists(location='/tmp/temp1', use_sudo=False))

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_exists_returns_False_if_missing(self, *args, **kwargs):
        self.assertFalse(os_helpers.dir_exists(location='/tmp/temp1', use_sudo=False))
    ...
    def test_dir_exists_returns_True_with_sudo(self, *args, **kwargs):
        ...
        self.assertTrue(os_helpers.dir_exists(location='/tmp/level1/level2', use_sudo=True))

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_exists_returns_False_if_missing_with_sudo(self, *args, **kwargs):
        self.assertFalse(os_helpers.dir_exists(location='/tmp/level1/level2', use_sudo=False))

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_dir_exists_returns_False_without_sudo_when_folder_is_root_owned(self, *args, **kwargs):
        sudo('mkdir -p /tmp/level1/level2')
        sudo('chmod 700 /tmp/level1')
        self.assertTrue(os_helpers.dir_exists(location='/tmp/level1/level2', use_sudo=False))
...
class DisableEnvResetOnSudoTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_env_reset_on_sudo_raises_exception_on_failure(self, *args, **kwargs):
        sudo('rm /etc/sudoers')
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.disable_env_reset_on_sudo()
        self.assertEqual(cm.exception.code, 1)
...
class DisableRequirettyOnSudoersTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sudoers_updates_sudoers(self, *args, **kwargs):
        os = sudo('cat /etc/os-release').lower()
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sudoers_is_idempodent(self, *args, **kwargs):
        os = sudo('cat /etc/os-release').lower()
        md5sum = sudo('md5sum /etc/sudoers').split(' ')[0]
        if 'ubuntu' in os:
            sudo('echo "Defaults    requiretty" >> /etc/sudoers')
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sudoers_raises_exception_on_failure(self, *args, **kwargs):
        sudo('rm /etc/sudoers')
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.disable_requiretty_on_sudoers()
        self.assertEqual(cm.exception.code, 1)
...
class DisableRequirettyOnSshdConfigTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sshd_config_updates_sshd_config(self, *args, **kwargs):
        os = sudo('cat /etc/os-release').lower()
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sshd_config_is_idempodent(self, *args, **kwargs):
        os = sudo('cat /etc/os-release').lower()
        sudo('echo "Requiretty yes" >> /etc/ssh/sshd_config')
        os_helpers.disable_requiretty_on_sshd_config()
        os_helpers.disable_requiretty_on_sshd_config()
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sshd_config_returns_True(self, *args, **kwargs):
        self.assertTrue(os_helpers.disable_requiretty_on_sshd_config())

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_disable_requiretty_on_sshd_config_raises_exception_on_failure(self, *args, **kwargs):
...
class FileAttribsTests(DockerBasedTests):
    ...
    def test_file_attribs_with_sudo_updates_perms(self, *args, **kwargs):
        ...
        sudo('/usr/sbin/groupadd group1')
        sudo('echo > /tmp/temp1')
        os_helpers.file_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=True)
        perms, x, owner, group = sudo('ls -l /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, '-rwxr-xr-x')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_file_attribs_with_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        os_helpers.file_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, 'drwxr-xr-x')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    def test_file_attribs_without_sudo_updates_perms(self, *args, **kwargs):
        ...
        sudo('/usr/sbin/groupadd group1')
        sudo('echo > /tmp/temp1')
        os_helpers.file_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=False, use_sudo=False)
        perms, x, owner, group = sudo('ls -l /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, '-rwxr-xr-x')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty', 'centos-7'])
    def test_file_attribs_without_sudo_updates_perms_recursively(self, *args, **kwargs):
        sudo('/usr/sbin/useradd user1')
        sudo('/usr/sbin/groupadd group1')
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        os_helpers.file_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=False)
        perms, x, owner, group = sudo('ls -ld /tmp/temp1').split(' ')[0:4]
        self.assertEquals(perms, 'drwxr-xr-x')
        self.assertEquals(owner, 'user1')
        self.assertEquals(group, 'group1')
    ...
    def test_file_attribs_returns_True_on_sucess(self, *args, **kwargs):
        ...
        sudo('mkdir /tmp/temp1')
        sudo('echo > /tmp/temp1/file1')
        result = os_helpers.file_attribs(location='/tmp/temp1', mode='755', owner='user1', group='group1', recursive=True, use_sudo=True)
        self.assertTrue(result)
    ...
    def test_file_attribs_raises_exception_on_failure(self, *args, **kwargs):
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.file_attribs(location='/tmp/no-valid-path', mode='755', owner='not-a-valid-user', group='not-a-valid-group', recursive=True, use_sudo=False)
        self.assertEqual(cm.exception.code, 1)
...
class OsReleaseTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid'])
    def test_os_release_on_ubuntu_vivid_returns_os_release_string(self, *args, **kwargs):
        print(os_helpers.os_release())
        self.assertEquals(os_helpers.os_release(), {'SUPPORT_URL': 'http://help.ubuntu.com/', 
         'NAME': 'Ubuntu', 
         'ID_LIKE': 'debian', 
         'VERSION_ID': '15.04', 
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-trusty'])
    def test_os_release_on_ubuntu_trusty_returns_os_release_string(self, *args, **kwargs):
        self.assertEquals(os_helpers.os_release(), {'SUPPORT_URL': 'http://help.ubuntu.com/', 
         'NAME': 'Ubuntu', 
         'ID_LIKE': 'debian', 
         'VERSION_ID': '14.04', 
    ...
    @with_ephemeral_container(distributions=[
     'centos-7'])
    def test_os_release_on_centos_7_returns_os_release_string(self, *args, **kwargs):
        self.assertEquals(os_helpers.os_release(), {'NAME': 'CentOS Linux', 
         'ANSI_COLOR': '0;31', 
         'ID_LIKE': 'rhel fedora', 
         'VERSION_ID': '7', 
...
class LinuxDistributionTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'centos-7'])
    def test_linux_distribution_on_centos_returns_centos_string(self, *args, **kwargs):
        self.assertTrue(os_helpers.linux_distribution(), 'centos')

    @with_ephemeral_container(distributions=[
     'ubuntu-trusty'])
    def test_linux_distribution_on_ubuntu_trusty_returns_ubuntu_string(self, *args, **kwargs):
        self.assertTrue(os_helpers.linux_distribution(), 'ubuntu')

    @with_ephemeral_container(distributions=[
     'ubuntu-vivid'])
    def test_linux_distribution_on_ubuntu_vivid_returns_ubuntu_string(self, *args, **kwargs):
        self.assertTrue(os_helpers.linux_distribution(), 'ubuntu')
...
class LsbReleaseTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid'])
    def test_lsb_release_on_ubuntu_vivid_returns_lsb_release_string(self, *args, **kwargs):
        sudo('apt-get update')
        sudo('apt-get -y install lsb-release')
        self.assertEquals(os_helpers.lsb_release(), {'DISTRIB_CODENAME': 'vivid', 
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-trusty'])
    def test_lsb_release_on_ubuntu_trusty_returns_lsb_release_string(self, *args, **kwargs):
        sudo('apt-get update')
        sudo('apt-get -y install lsb-release')
        self.assertEquals(os_helpers.lsb_release(), {'DISTRIB_CODENAME': 'trusty', 
...
class RestartServiceTests(DockerBasedTests):
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty'])
    def test_restart_service__on_ubuntu_restarts_service(self, *args, **kwargs):
        sudo('apt-get update')
        sudo('apt-get install -y xinetd')
        sudo('service xinetd start')
        pid_before_restart = sudo('ps -ef | grep xinetd | grep -v grep | awk "{ print $2 }"')
        os_helpers.restart_service('xinetd')
        pid_after_restart = sudo('ps -ef | grep xinetd | grep -v grep | awk "{ print $2 }"')
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty'])
    def test_restart_service_on_ubuntu_returns_True_on_success(self, *args, **kwargs):
        sudo('apt-get update')
        sudo('apt-get install -y xinetd')
        sudo('service xinetd start')
    ...
    @with_ephemeral_container(distributions=[
     'centos-7'])
    def test_restart_service_on_centos_returns_True_on_success(self, *args, **kwargs):
        sudo('apt-get update')
        sudo('apt-get install -y xinetd')
        sudo('service xinetd start')
        self.assertTrue(os_helpers.restart_service('xinetd'))
    ...
    @with_ephemeral_container(distributions=[
     'ubuntu-vivid', 'ubuntu-trusty'])
    def test_restart_service_on_ubuntu_raises_exception_on_failure(self, *args, **kwargs):
        with self.assertRaises(SystemExit) as (cm):
    ...
    @with_ephemeral_container(distributions=[
     'centos-7'])
    def test_restart_service_on_centos_raises_exception_on_failure(self, *args, **kwargs):
        with self.assertRaises(SystemExit) as (cm):
            os_helpers.restart_service('fake-service')
        self.assertEqual(cm.exception.code, 1)
...
class AddFirewalldServiceTests(DockerBasedTests):
    ...
    @unittest.skip('Required systemd running on the container')
    @with_ephemeral_container(distributions=['centos-7'])
    def test_add_firewalld_service_raises_exception_on_failure(self, *args, **kwargs):
        self.assertTrue(False)
        with self.assertRaises(SystemExit) as (cm):
            self.assertTrue(True)
        self.assertEqual(cm.exception.code, 1)
...
class AddFirewalldPortTests(DockerBasedTests):
    ...
    @unittest.skip('Required systemd running on the container')
    @with_ephemeral_container(distributions=['centos-7'])
    def test_add_firewalld_port_raises_exception_on_failure(self, *args, **kwargs):
        self.assertTrue(False)
        with self.assertRaises(SystemExit) as (cm):
            self.assertTrue(True)
        self.assertEqual(cm.exception.code, 1)
...
class EnableFirewalldServiceTests(DockerBasedTests):
    ...
    @unittest.skip('Required systemd running on the container')
    @with_ephemeral_container(distributions=['centos-7'])
    def test_enable_firewalld_service_returns_True_on_sucess(self, *args, **kwargs):
        self.assertTrue(False)

    @unittest.skip('Required systemd running on the container')
    @with_ephemeral_container(distributions=['centos-7'])
    def test_enable_firewalld_service_raises_exception_on_failure(self, *args, **kwargs):
        self.assertTrue(False)
        with self.assertRaises(SystemExit) as (cm):
            self.assertTrue(True)
        self.assertEqual(cm.exception.code, 1)
