...
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/api_v3/rackspace.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 7995 bytes
...
class RackspaceInstance(object):
    ...
    def create_from_config(cls, config, distro, region):
        ...
        state = RackspaceState(instance_name=instance_name, ip_address='', distro=distro, region=region)
    ...
    def upload_key(self):
        try:
            ...
            with open(self.config.public_key_filename) as (keyfile):
                self._nova.keypairs.create(self.config.key_pair, keyfile.read())
    ...
    def _create_server(self):
        ...
        server = self._nova.servers.create(name=self.state.instance_name, flavor=flavor.id, image=image.id, region=self.state.region, availability_zone=self.state.region, key_name=self.config.key_pair)
    ...
    def _connect_to_rackspace(self):
        ...
        pyrax.set_credentials(self.config.access_key_id, self.config.secret_access_key)
    ...
    def create_image(self, image_name):
        ...
        image_id = self._nova.servers.create_image(server.id, image_name=image_name)
        image = self._nova.images.get(image_id).status.lower()
        log_green('creating rackspace image...')
        sleep_time = 20
        elapsed = 0
        while self._nova.images.get(image_id).status.lower() not in ('active', 'error'):
            log_green('building rackspace image, this could take a bit: elapsed {}s.'.format(elapsed))
            sleep(20)
            elapsed += sleep_time
    ...
    def list_images(self):
        ...
        for image in sorted(images, key=lambda x: x.created):
            log_green('{}\t{:50}\t{}'.format(image.created, image.human_id, image.id))
    ...
    def get_state(self):
        ...
        """
        data = {'instance_name': self.state.instance_name, 
