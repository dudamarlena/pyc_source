...
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/api_v2/os_helpers.py
# Compiled at: 2016-08-21 18:45:37
# Size of source mod 2**32: 11355 bytes
...
def add_usr_local_bin_to_path(log=False):
    ...
    with settings(hide('warnings', 'running', 'stdout', 'stderr'), capture=True):
        try:
            sudo('echo "export PATH=/usr/local/bin:$PATH" |sudo /usr/bin/tee /etc/profile.d/fix-path.sh')
            return True
        except:
            raise SystemExit(1)
...
def dir_attribs(location, mode=None, owner=None, group=None, recursive=False, use_sudo=False):
...
def dir_ensure(location, recursive=False, mode=None, owner=None, group=None, use_sudo=False):
    ...
    if owner or group or mode:
        if use_sudo:
            dir_attribs(location, owner=owner, group=group, mode=mode, recursive=recursive, use_sudo=True)
    else:
...
def disable_env_reset_on_sudo(log=False):
    ...
    file_append('/etc/sudoers', 'Defaults:%wheel !env_reset,!secure_path', use_sudo=True, partial=True)
    return True
...
def disable_requiretty_on_sudoers(log=False):
    ...
    if log:
        bookshelf2.logging_helpers.log_green('disabling requiretty on sudo calls')
    comment_line('/etc/sudoers', '^Defaults.*requiretty', use_sudo=True)
    return True
...
def disable_requiretty_on_sshd_config(log=False):
    """ allow sudo calls through ssh without a tty """
    if log:
        bookshelf2.logging_helpers.log_green('disabling requiretty on sshd_config')
    comment_line('/etc/ssh/sshd_config', '^Defaults.*requiretty', use_sudo=True)
    return True
...
def enable_selinux():
    """ disables selinux """
    if not contains(filename='/etc/selinux/config', text='SELINUX=enforcing'):
        sed('/etc/selinux/config', 'SELINUX=.*', 'SELINUX=enforcing', use_sudo=True)
    if contains(filename='/etc/selinux/config', text='SELINUXTYPE=targeted'):
        sed('/etc/selinux/config', 'SELINUXTYPE=.*', 'SELINUX=targeted', use_sudo=True)
...
def file_attribs(location, mode=None, owner=None, group=None, use_sudo=False, recursive=True):
    """Updates the mode/owner/group for the remote file at the given
    location."""
    return dir_attribs(location=location, mode=mode, owner=owner, group=group, recursive=recursive, use_sudo=False)


def os_release():
    """ returns /etc/os-release in a dictionary """
    with settings(hide('warnings', 'running', 'stderr'), warn_only=True, capture=True):
        release = {}
...
def restart_service(service, log=False):
    ...
    with settings():
        if log:
            bookshelf2.logging_helpers.log_yellow('stoping service %s' % service)
        sudo('service %s stop' % service)
        if log:
            bookshelf2.logging_helpers.log_yellow('starting service %s' % service)
...
def install_os_updates(distribution, force=False):
    """ installs OS updates """
    if 'centos' in distribution or 'rhel' in distribution or 'redhat' in distribution:
