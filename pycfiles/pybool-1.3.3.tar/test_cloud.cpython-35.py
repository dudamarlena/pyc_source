...
# Embedded file name: /usr/lib/python3.5/site-packages/bookshelf/tests/api_v3/test_cloud.py
# Compiled at: 2016-08-21 18:37:21
# Size of source mod 2**32: 14804 bytes
import unittest
from subprocess import check_output
from uuid import uuid4
import yaml, os
from bookshelf.api_v3.cloud_instance import Distribution, ICloudInstance
from bookshelf.api_v3.rackspace import RackspaceInstance, RackspaceConfiguration
...
class CloudInstanceTestMixin(object):
    ...
    def _make_instance(self):
        ...
        """
        instance = self.instance_factory.create_from_config(self.config, self.distribution, self.region)
    ...
    def _restore_from_state(self, state):
        ...
        """
        instance = self.instance_factory.create_from_saved_state(self.config, state)
        return instance
    ...
    def _assert_instances_are_same(self, instance1, instance2, verify_ips=True):
        """
    ...
    def test_instance_factory_and_instance(self):
        ...
        self._assert_instances_are_same(instance, revived_instance, verify_ips=False)
        unique_id = revived_instance.create_image('testing-image-%s' % str(uuid4()))
...
class _Optional(object):
    ...
    def __repr__(self):
        return '{} (optional default={})'.format(self.description, repr(self.default))
...
def _extract_substructure(base, substructure):
    ...
    """
    if type(substructure) is not dict and type(base) is not dict:
        return base
    if type(base) is not dict:
        raise MissingConfigError('Found non-dict value {} when expecting a sub-configuration {}.'.format(repr(base), repr(substructure)))
    if type(substructure) is not dict:
        raise MissingConfigError('Found dict value {} when expecting a simple configuration value {}.'.format(repr(base), repr(substructure)))
    try:
        subdict = []
        ...
        raise MissingConfigError('Missing key {} in configuration'.format(e.args[0]))
...
def _load_config_from_yaml():
    ...
    if not filename:
        print('Must set {} to an acceptance.yaml file (http://doc-dev.clusterhq.com/gettinginvolved/appendix.html#acceptance-testing-configuration) plus additional keys in order to run this test.'.format(_ENV_VAR))
...
class RackspaceTests(unittest.TestCase, CloudInstanceTestMixin):
    ...
    def setUp(self):
        ...
        credentials = _get_yaml_config({'rackspace': {'keyname': '<Rackspace keypair name>', 
                       'username': '<Rackspace username>', 
                       'key': '<Rackspace API key>', 
                       'region': _Optional('HKG', '<Rackspace 3 letter region>')}, 
         
         'ssh_keys': {'rackspace': {'private_key_file': '<path-to-private-keypair-file>', 
                                    'public_key_file': '<path-to-corresponding-public-key>'}}})
        ssh_keys = credentials['ssh_keys']['rackspace']
        self.config = RackspaceConfiguration(username='root', instance_type='1GB Standard Instance', key_pair=credentials['rackspace']['keyname'], public_key_filename=ssh_keys['public_key_file'], private_key_filename=ssh_keys['private_key_file'], access_key_id=credentials['rackspace']['username'], secret_access_key=credentials['rackspace']['key'], ami='CentOS 7 (PVHVM)', description='rackspace-test-instance-description', image_basename='rackspace-test-image', instance_name='rackspace-test-instance').serialize()
...
class GCETests(unittest.TestCase, CloudInstanceTestMixin):
    ...
    def setUp(self):
        ...
        credentials = _get_yaml_config({'gce': {'project': '<GCE project>', 
                 'zone': _Optional('us-central1-f', '<GCE zone>'), 
                 'gce_credentials': {'private_key': _Optional('', '<full-private-key-to-authenticate-as-service-account>'), 
                                     
                                     'client_email': _Optional('', '<service-account-email>')}, 
                 
                 'machine_type': _Optional('f1-micro', '<GCE machine type>')}, 
         
         'ssh_keys': {'gce': {'private_key_file': '<path-to-private-key-file>', 
                              'public_key_file': '<path-to-corresponding-public-key>'}}})
        keys = credentials['ssh_keys']['gce']
        service_account_creds = credentials['gce']['gce_credentials']
        self.config = GCEConfiguration(credentials_private_key=service_account_creds.get('private_key', ''), credentials_email=service_account_creds.get('client_email', ''), public_key_filename=keys['public_key_file'], private_key_filename=keys['private_key_file'], project=credentials['gce']['project'], machine_type=credentials['gce']['machine_type'], image_basename='gce-test-image', username='gce-username-xyz', description='gce-test-description', instance_name='gce-test-instance', base_image_prefix='ubuntu-1404', base_image_project='ubuntu-os-cloud').serialize()
        self.distribution = Distribution.UBUNTU1404
        self.region = credentials['gce']['zone']
        self.instance_factory = GCEInstance


_UBUNTU_AMIS = {'us-east-1': 'ami-ffe3c695', 
...
class EC2Tests(unittest.TestCase, CloudInstanceTestMixin):
    ...
    def setUp(self):
        ...
        credentials = _get_yaml_config({'aws': {'access_key': '<AWS-ACCESS-KEY>', 
                 'secret_access_token': '<AWS-SECRET-ACCESS-TOKEN>', 
                 'keyname': '<aws-ssh-keypair-name>', 
                 'region': _Optional('us-west-2', '<ec2-region>'), 
                 'instance_type': _Optional('t2.micro', '<ec2-instance-type>')}, 
         
         'ssh_keys': {'aws': {'private_key_file': '<path-to-private-key-for-keypair>'}}})
        self.region = credentials['aws']['region']
        self.config = EC2Configuration(credentials=EC2Credentials(access_key_id=credentials['aws']['access_key'], secret_access_key=credentials['aws']['secret_access_token']), username='ubuntu', disk_name='/dev/sda1', disk_size='48', instance_name='ec2-test-instance', tags={'name': 'test-instance-with-tags'}, image_description='ec2-test-description', image_basename='ec2-image-basename', ami=_UBUNTU_AMIS[self.region], key_filename=credentials['ssh_keys']['aws']['private_key_file'], key_pair=credentials['aws']['keyname'], instance_type=credentials['aws']['instance_type'], security_groups=[
         'ssh']).serialize()
        self.distribution = Distribution.UBUNTU1404
