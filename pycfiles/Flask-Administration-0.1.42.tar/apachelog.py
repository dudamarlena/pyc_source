...
# [GCC 8.4.0]
# Embedded file name: /Users/bluemoon/Desktop/flask-administration/flask_administration/apachelog.py
# Compiled at: 2012-03-29 18:50:32
"""Apache Log Parser
...
formats = {'common': '%h %l %u %t \\"%r\\" %>s %b', 
   'vhcommon': '%v %h %l %u %t \\"%r\\" %>s %b', 
   'extended': '%h %l %u %t \\"%r\\" %>s %b \\"%{Referer}i\\" \\"%{User-agent}i\\"', 
   'lighttpd': '%h %d - %t \\"%r\\" %s %b \\"%{Referer}i\\" \\"%{User-Agent}i\\"'}
if __name__ == '__main__':
    ...
    class TestApacheLogParser(unittest.TestCase):
        ...
        def setUp(self):
            ...
            self.fields = ('%h %l %u %t %r %>s %b %{Referer}i %{User-Agent}i').split(' ')
            self.pattern = '^(\\S*) (\\S*) (\\S*) (\\[[^\\]]+\\]) \\"([^"\\\\]*(?:\\\\.[^"\\\\]*)*)\\" (\\S*) (\\S*) \\"([^"\\\\]*(?:\\\\.[^"\\\\]*)*)\\" \\"([^"\\\\]*(?:\\\\.[^"\\\\]*)*)\\"$'
            self.line1 = '212.74.15.68 - - [23/Jan/2004:11:36:20 +0000] "GET /images/previous.png HTTP/1.1" 200 2607 "http://peterhi.dyndns.org/bandwidth/index.html" "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.2) Gecko/20021202"'
            self.line2 = '212.74.15.68 - - [23/Jan/2004:11:36:20 +0000] "GET /images/previous.png=\\" HTTP/1.1" 200 2607 "http://peterhi.dyndns.org/bandwidth/index.html" "Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.2) Gecko/20021202"'
            self.line3 = '4.224.234.46 - - [20/Jul/2004:13:18:55 -0700] "GET /core/listing/pl_boat_detail.jsp?&units=Feet&checked_boats=1176818&slim=broker&&hosturl=giffordmarine&&ywo=giffordmarine& HTTP/1.1" 200 2888 "http://search.yahoo.com/bin/search?p=\\"grady%20white%20306%20bimini\\"" "Mozilla/4.0 (compatible; MSIE 6.0; Windows 98; YPC 3.0.3; yplus 4.0.00d)"'
            self.line4 = '111.235.72.230 terzo.org - [08/Aug/2011:05:53:11 +0000] "GET /route1.jpg HTTP/1.1" 200 202073 "http://terzo.org/" "Mozilla/5.0 (Windows NT 5.1; rv:6.0) Gecko/20100101 Firefox/6.0"'
        ...
        def testline1(self):
            ...
            self.assertEqual(data['%r'], 'GET /images/previous.png HTTP/1.1', msg='Line 1 %r')
            self.assertEqual(data['%>s'], '200', msg='Line 1 %>s')
            self.assertEqual(data['%b'], '2607', msg='Line 1 %b')
            self.assertEqual(data['%{Referer}i'], 'http://peterhi.dyndns.org/bandwidth/index.html', msg='Line 1 %{Referer}i')
            self.assertEqual(data['%{User-Agent}i'], 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.2) Gecko/20021202', msg='Line 1 %{User-Agent}i')
        ...
        def testline2(self):
            ...
            self.assertEqual(data['%t'], '[23/Jan/2004:11:36:20 +0000]', msg='Line 2 %t')
            self.assertEqual(data['%r'], 'GET /images/previous.png=\\" HTTP/1.1', msg='Line 2 %r')
            self.assertEqual(data['%>s'], '200', msg='Line 2 %>s')
            self.assertEqual(data['%b'], '2607', msg='Line 2 %b')
            self.assertEqual(data['%{Referer}i'], 'http://peterhi.dyndns.org/bandwidth/index.html', msg='Line 2 %{Referer}i')
            self.assertEqual(data['%{User-Agent}i'], 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.2) Gecko/20021202', msg='Line 2 %{User-Agent}i')
        ...
        def testline3(self):
            ...
            self.assertEqual(data['%t'], '[20/Jul/2004:13:18:55 -0700]', msg='Line 3 %t')
            self.assertEqual(data['%r'], 'GET /core/listing/pl_boat_detail.jsp?&units=Feet&checked_boats=1176818&slim=broker&&hosturl=giffordmarine&&ywo=giffordmarine& HTTP/1.1', msg='Line 3 %r')
            self.assertEqual(data['%>s'], '200', msg='Line 3 %>s')
            self.assertEqual(data['%b'], '2888', msg='Line 3 %b')
            self.assertEqual(data['%{Referer}i'], 'http://search.yahoo.com/bin/search?p=\\"grady%20white%20306%20bimini\\"', msg='Line 3 %{Referer}i')
            self.assertEqual(data['%{User-Agent}i'], 'Mozilla/4.0 (compatible; MSIE 6.0; Windows 98; YPC 3.0.3; yplus 4.0.00d)', msg='Line 3 %{User-Agent}i')

        def testline4(self):
            p = parser(formats['lighttpd'])
            data = p.parse(self.line4)
            self.assertEqual(data['%h'], '111.235.72.230', msg='Line 4 %h')
            self.assertEqual(data['%d'], 'terzo.org', msg='Line 4 %d')
            self.assertEqual(data['%t'], '[08/Aug/2011:05:53:11 +0000]', 'Line 4 %t')
            self.assertEqual(data['%b'], '202073', msg='Line 4 %t')
            self.assertEqual(data['%{Referer}i'], 'http://terzo.org/', msg='Line 4 %{Referer}i')
            self.assertEqual(data['%{User-Agent}i'], 'Mozilla/5.0 (Windows NT 5.1; rv:6.0) Gecko/20100101 Firefox/6.0', msg='Line 4 %{User-Agent}i')
