...
class DateRange(GeneralSlice):
    ...
    def intersection(self, other):
        ...
        """
        startopen = other.startopen if self.start is None else self.startopen if other.start is None else other.startopen if self.start < other.start else self.startopen if self.start > other.start else self.startopen or 
        endopen = other.endopen if self.end is None else self.endopen if other.end is None else other.endopen if self.end > other.end else self.endopen if self.end < other.end else self.endopen or 
...
Parse error at or near `RETURN_VALUE' instruction at offset 48
    ...
    def __setstate__(self, state):
        ...
        self.end = state['end']
        self.interval = state.get('interval') or 
        self.step = 1