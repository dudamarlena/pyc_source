...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/zourite/plugin/linkedin/linkedin.py
# Compiled at: 2010-04-30 09:56:10
"""
...
Enjoy!
"""
import hashlib, httplib, time, urllib2
from xml.dom.minidom import parseString
import logging, socket, oauth
from zourite.common import version
version.getInstance().submitRevision('$Revision: 260 $')
...
class LinkedInApi:
    ...
    def doApiRequest(self, url, access_token):
        ...
        xml = self._call_and_handle_low_level_error(lambda : self.linkedin.connection.getresponse().read())
        dom = parseString(xml)
...
class CommunicationApi(LinkedInApi):
    ...
    def sendMessage(self, recipients, subject, body, access_token):
        ...
        """
        xmlPost = "<?xml version='1.0' encoding='UTF-8'?>" + '<mailbox-item>' + '<recipients>'
        for recipient in recipients:
            xmlPost = xmlPost + "<recipient> <person path='/people/" + recipient + "' />" + '</recipient>'

        xmlPost = xmlPost + '</recipients>' + '<subject>' + subject + '</subject>' + '<body>' + body + '</body>' + '</mailbox-item>'
    ...
    def sendInvitation(self, id, subject, body, x_li_auth_token, access_token):
        ...
        xmlPost = "<?xml version='1.0' encoding='UTF-8'?>" + '<mailbox-item>' + '<recipients>' + '<recipient>' + " <person path='/people/d=" + id + "' />" + '</recipient>' + '</recipients>' + '<subject>' + subject + '</subject>' + '<body>' + body + '</body>' + '<item-content>' + '<invitation-request>' + '<connect-type>friend</connect-type>' + '<authorization>' + '<name>' + auth_token_name + '</name>' + '<value>' + auth_token_value + '</value>' + '</authorization>' + '</invitation-request>' + '</item-content>' + '</mailbox-item>'
        status = self.doApiPostRequest(self.POST_MESSAGE_URL, xmlPost, access_token)
        if status == 201:
            pass
        else:
...
class ProfileApi(LinkedInApi):
    ...
    def getShortProfile(self, contactId, access_token):
        ...
        """
        dom = self.doApiRequest(self.GET_CONNECTION_PROFILE_URL + contactId + self.SHORT_PROFILE_ATTRIBUTES, access_token)
        personDom = dom.getElementsByTagName('person')[0]
        return personDom
    ...
    def getFullProfile(self, contactId, access_token):
        ...
        The xml contains the profil.
        """
        dom = self.doApiRequest(self.GET_CONNECTION_PROFILE_URL + contactId + self.FULL_PROFILE_ATTRIBUTES, access_token)
        personDom = dom.getElementsByTagName('person')[0]
        return personDom
    ...
    def getApiStandardProfile(self, contactId, access_token):
        ...
        """
        dom = self.doApiRequest(self.GET_CONNECTION_PROFILE_URL + contactId + self.API_STANDARD_PROFILE_ATTRIBUTES, access_token)
...
class NetworkApi(LinkedInApi):
    ...
    def sendNewsMessage(self, body, access_token):
        ...
        """
        xmlPost = "<?xml version='1.0' encoding='UTF-8'?>" + '<activity locale="en_US">' + '<content-type>linkedin-html</content-type>' + '<body>' + body + '</body>' + '</activity>'
