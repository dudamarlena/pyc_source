...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/zourite/gui/zouriteGui.py
# Compiled at: 2010-04-30 08:08:49
"""
Created on 28 févr. 2010

@author: thierry
"""
import gtk, hildon, gobject, threading
from zourite.core import zourite, plugin
import logging, sys, traceback, webbrowser, mock
from Queue import *
import dbus, gdata.projecthosting.client, gdata.projecthosting.data, gdata.gauth, gdata.client, gdata.data, atom.http_core, atom.core
from zourite.common import version
version.getInstance().submitRevision('$Revision: 258 $')
...
def activate_led():
    """
        ...
        """
    bus = dbus.SystemBus()
    csd_call = dbus.Interface(bus.get_object('com.nokia.mce', '/com/nokia/mce/request'), 'com.nokia.mce.request')
    csd_call.req_led_pattern_activate('PatternCommunicationIM')
...
def deactivate_led():
    ...
    bus = dbus.SystemBus()
    csd_call = dbus.Interface(bus.get_object('com.nokia.mce', '/com/nokia/mce/request'), 'com.nokia.mce.request')
    csd_call.req_led_pattern_deactivate('PatternCommunicationIM')
...
class ZouriteGui():
    ...
    def _show_settings_from_menu(self):
    ...
    def _show_contact_list(self):
        window = ContactListView(self.select_contact, self.place_call, self.place_email, self.zcore, self)
        self.init_menu(window)
        self.program.add_window(window)
        window.show_all()
    ...
    def _show_network_profile_update_detail(self, netUpdateConn):
        sp = self.zcore.getShortProfileForNetUpdateProfile(netUpdateConn)
        self._show_detail_profil(sp)

    def _show_network_update_status_detail(self, netUpdateConn):
        sp = self.zcore.getShortProfileForNetUpdateStatus(netUpdateConn)
        self._show_detail_profil(sp)

    def _show_network_join_group_detail(self, netUpdateConn):
        sp = self.zcore.getShortProfileForNetUpdateJoinGroup(netUpdateConn)
        self._show_detail_profil(sp)
    ...
    def _update_network_news(self, view, force=False):
        self.zcore.setForceRefresh(force)
        try:
    ...
    def show_settings_from_menu(self, widget, data):
        ...
        """
        self._call_handled_method(self._show_settings_from_menu)
    ...
    def show_network_news(self):
        ...
        """
        auto_refresh = self.zcore.isNewsAutoRefreh()
        window = NetworkNewsView(self.update_network_news, self.select_an_update_network_new, self.show_update_status, self.show_post_news, auto_refresh, self.zcore, self)
        window.set_title('Network news')
        self.init_menu(window)
        self.program.add_window(window)
        window.show_all()

    def update_network_news(self, view, force=False):
        """
        callback function invoked by the NetworkNewsView when it want
        to be refreshed
        view is the NetworkNewsView
        """
        self._call_handled_method(self._update_network_news, view, force)
    ...
    def set_presence(self, status):
        """
        Change the message status with MissionControl
        """
        pass
    ...
    def place_email(self, person):
        ...
        """
        bus = dbus.SystemBus()
        csd_call = dbus.Interface(bus.get_object('com.nokia.modest', '/com/nokia/modest'), 'com.nokia.modest')
    ...
    def select_an_update_network_new(self, netUpdateConn):
        ...
        elif isinstance(netUpdateConn, plugin.NetworkProfileUpdate):
            self.show_network_profile_update_detail(netUpdateConn)
        elif isinstance(netUpdateConn, plugin.NetworkUpdateStatus):
            self.show_network_update_status_detail(netUpdateConn)
        elif isinstance(netUpdateConn, plugin.NetworkJoinGroupUpdate):
            self.show_network_join_group_detail(netUpdateConn)
        else:
            self.show_note_information('the type ' + netUpdateConn.__class__.__name__ + ' is not supported yet')

    def show_network_update_status_detail(self, netUpdateConn):
        """
        Show the detail of the update status event
        """
        self._call_handled_method(self._show_network_update_status_detail, netUpdateConn)

    def show_network_join_group_detail(self, netUpdateConn):
        """
        Show the deatil of a join group event
        """
        self._call_handled_method(self._show_network_join_group_detail, netUpdateConn)
    ...
    def show_network_profile_update_detail(self, netUpdateConn):
        """
        Show the detail of the profile update event
        """
        self._call_handled_method(self._show_network_profile_update_detail, netUpdateConn)
...
class ContactListView(ZouriteStackableWindow):
    ...
    def __init__(self, select_contact_callback, phone_call_callback, email_callback, zcore, main_gui):
        self.zcore = zcore
    ...
    def _refresh(self, force=False):
        ...
        for people in peopleList:
            ...
            gtk.gdk.threads_enter()
            self.model.append([self.zcore.getGtkPixBufForProfile(people).scale_simple(80, 80, gtk.gdk.INTERP_BILINEAR), contact, headline, emailPict, phonePict, people])
            gtk.gdk.threads_leave()
    ...
    def refresh(self, force=False):
        ...
        AsyncTask(lambda : self._refresh(force=force), self.main_gui).start()


class AsyncTask(threading.Thread):

    def __init__(self, task, exception_handler):
    ...
    def run(self):
        ...
        try:
            ...
            logging.error('uncaught exception in async task')
            self.exception_handler.send_exception_signal()
        else:
...
class SettingsView(ZouriteStackableWindow):
    ...
    def __init__(self, save_callback, default_callback, reset_cache_callback, zcore):
        ...
        self.auto_refresh_news = zcore.isNewsAutoRefreh()
    ...
    def init_center_view(self, centerview):
        ...
        newsAutoRefreshBtn = hildon.CheckButton(gtk.HILDON_SIZE_AUTO)
        newsAutoRefreshBtn.set_label('Auto refresh new')
        newsAutoRefreshBtn.set_active(self.zcore.isNewsAutoRefreh())
        newsAutoRefreshBtn.connect('toggled', self.on_auto_refresh_news_toggle_event, None)
        centerview.pack_start(newsAutoRefreshBtn, False, False, 0)
        return

    def on_auto_refresh_news_toggle_event(self, widget, data):
        self.auto_refresh_news = widget.get_active()
    ...
    def some_page_func(self, notebook, current_page_number, userdata):
        ...
        if current_page_number == 0:
            ...
            if len(selectedRows) == 1:
                ...
                return False
        elif len(self.pluginConfData) > 0:
    ...
    def on_save_clicked(self, widget, data):
        ...
        settings.news_auto_refresh = self.auto_refresh_news
...
class FullProfileView(ZouriteStackableWindow):
    ...
    def init_bottom_button(self, hbox):
        if self.zcore.isConnectedTo(self.fullProfile):
...
class NetworkUpdateConnectionView(ZouriteStackableWindow):
    ...
    def init_bottom_button(self, hbox):
        ...
        if not self.zcore.isConnectedTo(self.newConnShortProfile):
            inviteBtn = self.create_button('Invite')
    ...
    def create_shortProfile(self, shortProfile, on_detail_profil_request, zcore):
        ...
        return (
         pictAlign, nameAlign, labelAlign)
...
class NetworkNewsView(ZouriteStackableWindow):
    ...
    def __init__(self, refresh_function, select_function, on_status_bouton_clicked, on_news_bouton_clicked, auto_refresh, zcore, main_gui):
        """
        ...
        if auto_refresh:
            ...
            self.refresh_handler_id = gobject.timeout_add(REFRESH_PERIOD, self.receive_refresh_signal)
            self.connect('destroy', self.destroy_window, None)
        return

    def destroy_window(self, widget, data):
        if self.refresh_handler_id is not None:
            gobject.source_remove(self.refresh_handler_id)
        return

    def receive_refresh_signal(self):
        first = self.model.get_iter_root()
        netUpdate = None
        if first is not None:
            (netUpdate,) = self.model.get(first, 2)
        t = self.refresh(force=True)
        t.join()
        first = self.model.get_iter_root()
        new_netUpdate = None
        if first is not None:
            (new_netUpdate,) = self.model.get(first, 2)
            if not netUpdate.timestamp == new_netUpdate.timestamp:
                activate_led()
                handler_id = gobject.timeout_add(30000, deactivate_led)
        return True
    ...
    def refresh(self, force=False):
        ...
        Return the thread which effetcively accomplish the task
        """
        hildon.hildon_gtk_window_set_progress_indicator(self, 1)
        t = AsyncTask(lambda : self.refresh_func(self, force=force), self.main_gui)
        t.start()
        return t
