...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cli.py
# Compiled at: 2020-04-11 18:24:19
...
def register_arguments(tentacles_parser) -> None:
    tentacles_parser.add_argument('-c', '--config', help=f"Path to the config file to use. Default is {DEFAULT_CONFIG_FILE}",
      type=str,
      default=(get_resource_or_file_name(DEFAULT_CONFIG_FILE)))
    tentacles_parser.add_argument('-d', '--target-directory', help="Path to the target directory to process. Default is '.'",
      type=str,
      default='.')
    tentacles_parser.add_argument('-f', '--skip-confirmations', help='Skip user confirmations.',
      action='store_true')
