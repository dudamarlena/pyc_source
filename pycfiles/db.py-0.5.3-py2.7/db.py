...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/db/db.py
# Compiled at: 2017-03-31 10:40:59
import threading, glob, gzip
try:
...
class DB(object):
    ...
    def __init__(self, username=None, password=None, hostname='localhost', port=None, filename=None, dbname=None, dbtype=None, schemas=None, profile='default', exclude_system_tables=True, limit=1000, keys_per_column=None, driver=None, cache=False):
        if port is None:
            if dbtype == 'postgres':
                port = 5432
        ...
        if self.dbtype == 'postgres' or self.dbtype == 'redshift':
            ...
            self.con = pg.connect(user=self.username, password=self.password, host=self.hostname, port=self.port, dbname=self.dbname)
            ...
            if HAS_ODBC:
                base_con = ('Driver={driver};Server={server};Database={database};').format(driver=self.driver or 'SQL Server', server=self.hostname or 'localhost', database=self.dbname or '')
                conn_str = self.username and self.password and ('{}{}').format(base_con, ('User Id={username};Password={password};').format(username=self.username, password=self.password)) or ('{}{}').format(base_con, 'Trusted_Connection=Yes;')
                try:
                    self.con = pyo.connect(conn_str)
                    self.cur = self.con.cursor()
                except:
                    self.con = pyo.connect(driver=self.driver or 'SQL Server', server=self.hostname or 'localhost', port=self.port, database=self.dbname or '', uid=self.username, pwd=self.password)
                    self.cur = self.con.cursor()
                ...
                if '\\' in self.hostname:
                    ...
                    hostname = self.hostname
                self.con = pymssql.connect(host=hostname, user=self.username, password=self.password, database=self.dbname)
                self.cur = self.con.cursor()
    ...
    def __str__(self):
        return ('DB[{dbtype}][{hostname}]:{port} > {user}@{dbname}').format(dbtype=self.dbtype, hostname=self.hostname, port=self.port, user=self.username, dbname=self.dbname)
    ...
    def refresh_schema(self, exclude_system_tables=True, use_cache=False):
        ...
        if use_cache:
            self._tables = TableSet([ Table(self.con, self._query_templates, table_meta[t]['schema'], t, tables[t], keys_per_column=self.keys_per_column, foreign_keys=table_meta[t]['foreign_keys']['columns'], ref_keys=table_meta[t]['ref_keys']['columns']) for t in sorted(tables.keys())
                                    ])
            ...
            self._tables = TableSet([ Table(self.con, self._query_templates, tables[t][0].schema, t, tables[t], keys_per_column=self.keys_per_column, foreign_keys=table_db_foreign_keys[t], ref_keys=table_db_ref_keys[t]) for t in sorted(tables.keys())
                                    ])
        elif not use_cache:
            self._tables = TableSet([ Table(self.con, self._query_templates, tables[t][0].schema, t, tables[t], keys_per_column=self.keys_per_column) for t in sorted(tables.keys())
                                    ])
    ...
    def _get_db_metadata(self, exclude_system_tables, use_cache):
        ...
        if use_cache and self._metadata_cache:
            ...
            for table in self._metadata_cache:
                table_meta[table['name']] = {k:table[k] for k in ('schema', 'name',
                                                                  'foreign_keys',
                                                                  'ref_keys')}
    ...
    def _gen_tables_from_col_tuples(self, cols):
        ...
        for table_schema, table_name, column_name, data_type in cols:
            if table_name not in tables:
                tables[table_name] = []
            tables[table_name].append(Column(self.con, self._query_templates, table_schema, table_name, column_name, data_type, self.keys_per_column))
    ...
    def to_redshift(self, name, df, drop_if_exists=False, chunk_size=10000, AWS_ACCESS_KEY=None, AWS_SECRET_KEY=None, s3=None, print_sql=False, bucket_location=None, s3_bucket=None):
        """
        ...
        sql = ("\n        copy {name} from 's3://{bucket_name}/data'\n        credentials 'aws_access_key_id={AWS_ACCESS_KEY};aws_secret_access_key={AWS_SECRET_KEY}'\n        CSV IGNOREHEADER as 1 GZIP;\n        ").format(name=name, bucket_name=bucket_name, AWS_ACCESS_KEY=AWS_ACCESS_KEY, AWS_SECRET_KEY=AWS_SECRET_KEY)
        if print_sql:
            sys.stderr.write(sql + '\n')
        self._try_command(sql)
        self.con.commit()
