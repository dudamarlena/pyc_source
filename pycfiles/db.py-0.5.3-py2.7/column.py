...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/db/column.py
# Compiled at: 2016-10-28 17:36:05
...
class Column(object):
    ...
    def head(self, n=6):
        ...
        Name: City, dtype: object
        """
        q = self._query_templates['column']['head'].format(column=self.name, schema=self.schema, table=self.table, n=n)
    ...
    def all(self):
        ...
        59
        """
        q = self._query_templates['column']['all'].format(column=self.name, schema=self.schema, table=self.table)
        return pd.read_sql(q, self._con)[self.name]
    ...
    def unique(self):
        ...
        59
        """
        q = self._query_templates['column']['unique'].format(column=self.name, schema=self.schema, table=self.table)
        return pd.read_sql(q, self._con)[self.name]
    ...
    def sample(self, n=10):
        ...
        10
        """
        q = self._query_templates['column']['sample'].format(column=self.name, schema=self.schema, table=self.table, n=n)
        return pd.read_sql(q, self._con)[self.name]
