...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/agiletoolkit/utils.py
...
def semantic_version(tag):
    ...
    try:
        ...
        raise CommandError('Could not parse "%s", please use MAJOR.MINOR.PATCH' % tag) from exc
...
def gitrepo(root=None):
    if not root:
        ...
        remotes = [x.split() for x in filter(lambda x: x.endswith('(fetch)'), sh('git remote -v', cwd=root).strip().splitlines())]
        if cwd != root:
            os.chdir(cwd)
    return {'head':{'id':gitlog[0],  'author_name':gitlog[1], 
