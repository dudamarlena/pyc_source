...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/agiletoolkit/api/components.py
...
class Component:
    __doc__ = 'Base class for github components\n    '
...
class RepoComponents(Component):
    ...
    def update(self, id, data):
        ...
        """
        id = self.as_id(id)
        response = self.http.patch(('%s/%s' % (self, id)),
    ...
    def delete(self, id):
        ...
        """
        id = self.as_id(id)
        response = self.http.delete(('%s/%s' % (self.api_url, id)),
          auth=(self.auth))
        response.raise_for_status()
