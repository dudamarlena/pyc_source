...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/svgutils/transform.py
# Compiled at: 2018-01-25 09:27:07
...
class TextElement(FigureElement):
    ...
    def __init__(self, x, y, text, size=8, font='Verdana', weight='normal', letterspacing=0, anchor='start', color='black'):
        txt = etree.Element(SVG + 'text', {'x': str(x), 'y': str(y), 'font-size': str(size), 
           'font-family': font, 
           'font-weight': weight, 
...
class LineElement(FigureElement):
    ...
    def __init__(self, points, width=1, color='black'):
        ...
        line = etree.Element(SVG + 'path', {'d': linedata, 'stroke-width': str(width), 
           'stroke': color})
...
class SVGFigure(object):
    ...
    def save(self, fname):
        ...
        out = etree.tostring(self.root, xml_declaration=True, standalone=True, pretty_print=True)
        fid = open(fname, 'wb')
        fid.write(out)
        fid.close()
    ...
    def get_size(self):
        """Get figure size"""
        return (
         self.root.get('width'), self.root.get('height'))
...
def from_mpl(fig, savefig_kw=None):
    ...
    try:
        ...
        raise (
         ValueError, 'No matplotlib SVG backend')
