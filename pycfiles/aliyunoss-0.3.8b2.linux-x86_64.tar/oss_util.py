...
# [GCC 8.4.0]
# Embedded file name: /home/vagrant/virtualenv/oss/lib/python2.7/site-packages/oss/oss_util.py
# Compiled at: 2015-05-09 11:06:49
import platform, urllib, base64, hmac
from hashlib import sha1 as sha
import os, StringIO
from threading import Thread
import Queue, threading, ConfigParser, logging
from logging.handlers import RotatingFileHandler
from xml.sax.saxutils import escape
import socket, sys, calendar, imp
try:
...
class Logger:
    ...
    def __init__(self, debug, log_name, log_level, logger):
        ...
        if debug:
            ...
            hdlr = RotatingFileHandler(logfile, mode='a', maxBytes=max_log_size, backupCount=backup_count)
...
def create_object_group_msg_xml(part_msg_list=None):
    ...
    for part in part_msg_list:
        if len(part) >= 3:
            ...
            xml_string += '<Part>'
            xml_string += '<PartNumber>' + str(part[0]) + '</PartNumber>'
            xml_string += '<PartName>' + str(file_path) + '</PartName>'
            xml_string += '<ETag>"' + str(part[2]).upper() + '"</ETag>'
            xml_string += '</Part>'
        else:
...
def create_object_link_msg_xml_by_name(object_list=None):
    ...
    if not object_list:
        object_list = []
    xml_string = '<CreateObjectLink>'
    for i in range(len(object_list)):
        part = str(object_list[i]).strip()
        file_path = convert_utf8(part)
        file_path = escape(file_path)
        xml_string += '<Part>'
        xml_string += '<PartNumber>' + str(i + 1) + '</PartNumber>'
        xml_string += '<PartName>' + str(file_path) + '</PartName>'
        xml_string += '</Part>'
...
def create_part_xml(part_msg_list=None):
    ...
    if not part_msg_list:
        part_msg_list = []
    xml_string = '<CompleteMultipartUpload>'
    for part in part_msg_list:
        if len(part) >= 3:
            xml_string += '<Part>'
            xml_string += '<PartNumber>' + str(part[0]) + '</PartNumber>'
...
def create_delete_object_msg_xml(object_list=None, is_quiet=False, is_defult=False):
    ...
    if not is_defult:
        if is_quiet:
            xml_string += '<Quiet>true</Quiet>'
        else:
            xml_string += '<Quiet>false</Quiet>'
    for object in object_list:
        key = convert_utf8(object)
        key = escape(key)
        xml_string += '<Object><Key>%s</Key></Object>' % key
...
class GetAllObjects:
    ...
    def get_object_dir_in_bucket(self, oss, bucket='', marker='', prefix='', delimiter=''):
        ...
        try:
            ...
            if len(fl) != 0:
                for i in fl:
                    object_list.append((i[0], i[3], i[1]))
    ...
    def get_all_object_dir_in_bucket(self, oss, bucket='', marker='', prefix='', delimiter=''):
        ...
        return (
         self.object_list, self.dir_list)
...
def split_large_file(file_path, object_prefix='', max_part_num=1000, part_size=10485760, buffer_size=10485760, check_md5=False):
    parts_list = []
    if os.path.isfile(file_path):
        ...
        for i in xrange(0, part_num):
            ...
            part_msg = (
             part_order, file_name, md5sum, real_part_size, offset, base64md5)
