...
# [GCC 8.4.0]
# Embedded file name: D:\Development\django2-propeller\django2_propeller\forms.py
...
from .propeller import get_propeller_setting, get_form_renderer, get_field_renderer, get_formset_renderer, PROPELLER_SET_REQUIRED_SET_DISABLED
...
def render_button(content, button_type=None, icon=None, button_class='btn-default', size='', href='', name=None, value=None, title=None, style='default', extra_classes='', _id=''):
    """Render a button with content"""
    return Button(content, button_type, icon, button_class, size, href, name, value, title, style, extra_classes, _id).as_html()


def render_fab(content, button_type=None, icon=None, button_class='btn-default', size='', href='', name=None, value=None, title=None, style='default', extra_classes='', _id=''):
    """Render a button with content"""
    return FAB(content, button_type, icon, button_class, size, href, name, value, title, style, extra_classes, _id).as_html()
...
def render_field_and_label(field, label, field_class='', label_for=None, label_class='', layout='', **kwargs):
    ...
    if label:
        html = render_label(label,
          label_for=label_for, label_class=label_class) + html
    return html


def render_form_group(content, css_class=FORM_GROUP_CLASS):
    """Render a Bootstrap form group"""
    return '<div class="{klass}">{content}</div>'.format(klass=css_class,
      content=content)
...
def is_widget_required_attribute(widget):
    ...
    if PROPELLER_SET_REQUIRED_SET_DISABLED:
        if not get_propeller_setting('set_required'):
            return False
    else:
        return widget.is_required or False
