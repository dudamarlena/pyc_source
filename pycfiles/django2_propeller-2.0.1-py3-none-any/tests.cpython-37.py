...
# [GCC 8.4.0]
# Embedded file name: D:\Development\django2-propeller\django2_propeller\tests.py
# Compiled at: 2019-04-26 08:32:14
# Size of source mod 2**32: 37822 bytes
from __future__ import unicode_literals
import re
from django import forms
from django.contrib.admin.widgets import AdminSplitDateTime
import django.contrib.messages as DEFAULT_MESSAGE_LEVELS
...
from .test_data import DemoCard1, DemoCard2, DemoCard3, DemoCard4, TestNavbar1, TestNavbar2, TestNavbar3, NavBarLinkItem, NavBarDropDownDivider, NavBarDropDownItem, NavBar, TestNavbar4, DemoMediaImage1, DemoMediaImage3, DemoMedia1, DemoMedia2, DemoMedia3, DemoCard5, DemoMedia4, DemoMedia5
from .test_results import RESULT_CARD1, RESULT_CARD2, RESULT_CARD3, RESULT_CARD4, RESULT_NAVBAR1, RESULT_NAVBAR2, RESULT_NAVBAR3, RESULT_CARD5
try:
...
class TestForm(forms.Form):
    ...
    category2 = forms.MultipleChoiceField(choices=RADIO_CHOICES,
      widget=(forms.CheckboxSelectMultiple),
      help_text='Check as many as you like.')
    category3 = forms.ChoiceField(widget=(forms.RadioSelect),
      choices=MEDIA_CHOICES)
    category4 = forms.MultipleChoiceField(choices=MEDIA_CHOICES,
      widget=(forms.CheckboxSelectMultiple),
      help_text='Check as many as you like.')
...
class SettingsTest(TestCase):
    ...
    def test_propeller_javascript_tag(self):
        ...
        self.assertEqual(res.strip(), '<script src="//maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>')

    def test_propeller_css_tag(self):
        res = self.render_template_with_form('{% propeller_css %}').strip()
        self.assertIn('<link href="//maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet">', res)
...
class TemplateTest(TestCase):
    ...
    def test_propeller_template(self):
        res = self.render_template('{% extends "propeller/propeller.html" %}{% block propeller_content %}test_propeller_content{% endblock %}')
...
class FormTest(TestCase):
    ...
    def test_exclude(self):
        form = TestForm()
        res = self.render_template_with_form('{% propeller_form form exclude="cc_myself" %}', {'form': form})
        self.assertNotIn('cc_myself', res)

    def test_layout_horizontal(self):
        form = TestForm()
        res = self.render_template_with_form('{% propeller_form form layout="horizontal" %}', {'form': form})
        self.assertIn('col-md-3', res)
        self.assertIn('col-md-9', res)
        res = self.render_template_with_form('{% propeller_form form layout="horizontal" horizontal_label_class="hlabel" horizontal_field_class="hfield" %}', {'form': form})
    ...
    def test_required_class(self):
        ...
        res = self.render_template_with_form('{% propeller_form form %}', {'form': form})
        self.assertIn('bootstrap3-req', res)
        res = self.render_template_with_form('{% propeller_form form required_css_class="successful-test" %}', {'form': form})
        self.assertIn('successful-test', res)
        res = self.render_template_with_form('{% propeller_form form required_css_class="" %}', {'form': form})
...
class FieldTest(TestCase):
    ...
    def test_required_field(self):
        ...
        form_field = 'form.subject'
        rendered = self.render_template_with_form('{% propeller_field ' + form_field + ' required_css_class="test-required" %}')
    ...
    def test_input_group_addon_button(self):
        res = self.render_template_with_form('{% propeller_field form.subject addon_before="$" addon_before_class="input-group-btn" addon_after=".00" addon_after_class="input-group-btn" %}')
        self.assertIn('class="input-group"', res)
...
class ComponentsTest(TestCase):
    ...
    def test_icon(self):
        ...
        self.assertEqual(res.strip(), '<i class="material-icons pmd-sm">star</i>')
...
class MessagesTest(TestCase):
    ...
    def test_messages(self):
        ...
        res = self.render_template_with_form('{% propeller_messages messages %}', {'messages': messages})
        ...
        self.assertEqual(re.sub(pattern, '', res).replace('rel="nofollow"', ''), re.sub(pattern, '', expected).replace('rel="nofollow"', ''))
        messages = [
         FakeMessage(DEFAULT_MESSAGE_LEVELS.ERROR, 'hello\nthere')]
        res = self.render_template_with_form('{% propeller_messages messages %}', {'messages': messages})
        expected = '\n    <div class="alert alert-danger alert-dismissable">\n        <button type="button" class="close" data-dismiss="alert"\n            aria-hidden="true">&#215;</button>\n        hello there\n    </div>\n        '
...
class ButtonTest(TestCase):
    ...
    def test_button(self):
        ...
        self.assertEqual('<button class="btn btn-default pmd-ripple-effect btn-lg pmd-btn-default" type="button">button</button>', res.strip())
        res = self.render_template_with_form("{% propeller_button 'button' size='lg' href='#' %}")
        self.assertIn('<a class="btn btn-default pmd-ripple-effect btn-lg pmd-btn-default" href="#" type="button">button</a>', res.strip())
...
class ShowLabelTest(TestCase):
    ...
    def test_show_label(self):
        form = TestForm()
        res = self.render_template_with_form('{% propeller_form form show_label=False %}', {'form': form})
        self.assertIn('sr-only', res)

    def test_for_formset(self):
        TestFormSet = formset_factory(TestForm, extra=1)
        test_formset = TestFormSet()
        res = self.render_template_with_form('{% propeller_formset formset show_label=False %}', {'formset': test_formset})
        self.assertIn('sr-only', res)

    def test_button_with_icon(self):
        res = self.render_template_with_form("{% propeller_button 'test' icon='info-sign' %}")
        self.assertEqual('<button class="btn btn-default pmd-ripple-effect pmd-btn-default" href="#" type="button"><i class="material-icons pmd-sm">info-sign</i> test</button>', res.strip())
        res = self.render_template_with_form("{% propeller_button 'test' icon='info-sign' button_class='btn-primary' %}")
        self.assertEqual('<button class="btn btn-primary pmd-ripple-effect pmd-btn-default" href="#" type="button"><i class="material-icons pmd-sm">info-sign</i> test</button>', res.strip())
        res = self.render_template_with_form("{% propeller_button 'test' icon='info-sign' button_type='submit' %}")
        self.assertEqual('<button class="btn btn-default pmd-ripple-effect pmd-btn-default" href="#" type="submit"><i class="material-icons pmd-sm">info-sign</i> test</button>', res.strip())


class TypographyTest(TestCase):

    def test_marked_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_mark_text }}', {'text': 'This is a test'})
        self.assertInHTML('<mark>This is a test</mark>', res)

    def test_striked_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_strike_text }}', {'text': 'This is a test'})
        self.assertInHTML('<s>This is a test</s>', res)

    def test_underlined_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_underline_text }}', {'text': 'This is a test'})
        self.assertInHTML('<u>This is a test</u>', res)

    def test_bold_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_bold_text }}', {'text': 'This is a test'})
        self.assertInHTML('<strong>This is a test</strong>', res)

    def test_italic_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_italic_text }}', {'text': 'This is a test'})
        self.assertInHTML('<em>This is a test</em>', res)

    def test_lead_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_lead_text }}', {'text': 'This is a test'})
        self.assertInHTML('<span class="lead">This is a test</span>', res)

    def test_display_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_display_text:1 }}', {'text': 'This is a test'})
        self.assertInHTML('<span class="pmd-display1">This is a test</span>', res)
        res = self.render_template_with_propeller('{{ text|pmd_display_text:2 }}', {'text': 'This is a test'})
        self.assertInHTML('<span class="pmd-display2">This is a test</span>', res)
        res = self.render_template_with_propeller('{{ text|pmd_display_text:3 }}', {'text': 'This is a test'})
        self.assertInHTML('<span class="pmd-display3">This is a test</span>', res)
        res = self.render_template_with_propeller('{{ text|pmd_display_text:4 }}', {'text': 'This is a test'})
        self.assertInHTML('<span class="pmd-display4">This is a test</span>', res)

    def test_muted_text_filter(self):
        res = self.render_template_with_propeller('{{ text|pmd_muted_text }}', {'text': 'This is a test'})
...
class FABsTests(TestCase):
    ...
    def test_default_fab(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="star" %}')
        self.assertInHTML('<button class="btn pmd-btn-fab pmd-btn-default pmd-ripple-effect btn-default" type="button"><i class="material-icons pmd-sm">star</i></button>', res)

    def test_default_fab_with_btn_class(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="warning" button_class="btn-warning" %}')
        self.assertInHTML('<button class="btn btn-warning pmd-btn-fab pmd-ripple-effect pmd-btn-default" type="button"><i class="material-icons pmd-sm">warning</i></button>', res)

    def test_default_fab_with_link(self):
        res = self.render_template_with_propeller('{% propeller_fab "Link" button_class="btn-link" %}')
        self.assertInHTML('<button class="btn pmd-btn-fab pmd-btn-default pmd-ripple-effect btn-link" type="button">Link</button>', res)

    def test_raised_fab(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="star" style="raised" %}')
        self.assertInHTML('<button class="btn btn-default pmd-btn-fab pmd-ripple-effect pmd-btn-raised" type="button"><i class="material-icons pmd-sm">star</i></button>', res)

    def test_raised_fab_with_btn_class(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="error" style="raised" button_class="btn-danger" %}')
        self.assertInHTML('<button class="btn btn-danger pmd-btn-fab pmd-ripple-effect pmd-btn-raised" type="button"><i class="material-icons pmd-sm">error</i></button>', res)

    def test_raised_fab_with_link(self):
        res = self.render_template_with_propeller('{% propeller_fab "Link" style="raised" button_class="btn-link" %}')
        self.assertInHTML('<button class="btn btn-link pmd-btn-fab pmd-ripple-effect pmd-btn-raised" type="button">Link</button>', res)

    def test_flat_fab(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="star" style="flat" %}')
        self.assertInHTML('<button class="btn btn-default pmd-btn-fab pmd-ripple-effect pmd-btn-flat" type="button"><i class="material-icons pmd-sm">star</i></button>', res)

    def test_flat_fab_with_btn_class(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="info" style="flat" button_class="btn-danger" %}')
        self.assertInHTML('<button class="btn btn-danger pmd-btn-fab pmd-ripple-effect pmd-btn-flat" type="button"><i class="material-icons pmd-sm">info</i></button>', res)

    def test_flat_fab_with_link(self):
        res = self.render_template_with_propeller('{% propeller_fab "Link" style="flat" button_class="btn-link" %}')
        self.assertInHTML('<button class="btn btn-link pmd-btn-fab pmd-ripple-effect pmd-btn-flat" type="button">Link</button>', res)

    def test_outline_fab(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="star" style="outline" %}')
        self.assertInHTML('<button class="btn btn-default pmd-btn-fab pmd-ripple-effect pmd-btn-outline" type="button"><i class="material-icons pmd-sm">star</i></button>', res)

    def test_outline_fab_with_btn_class(self):
        res = self.render_template_with_propeller('{% propeller_fab "" icon="check" style="outline" button_class="btn-danger" %}')
        self.assertInHTML('<button class="btn btn-danger pmd-btn-fab pmd-ripple-effect pmd-btn-outline" type="button"><i class="material-icons pmd-sm">check</i></button>', res)

    def test_outline_fab_with_link(self):
        res = self.render_template_with_propeller('{% propeller_fab "Link" style="outline" button_class="btn-link" %}')
        self.assertInHTML('<button class="btn btn-link pmd-btn-fab pmd-ripple-effect pmd-btn-outline" type="button">Link</button>', res)
...
class PropellerNavBarTests(TestCase):
    ...
    def test_rendered_template(self):
        res = self.render_template_with_propeller('{% propeller_navbar testnav1 %}',
          context={'testnav1': TestNavbar1()})
        self.assertInHTML(RESULT_NAVBAR1, res)
        res = self.render_template_with_propeller('{% propeller_navbar testnav2 %}',
          context={'testnav2': TestNavbar2()})
        self.assertInHTML(RESULT_NAVBAR2, res)
        res = self.render_template_with_propeller('{% propeller_navbar testnav3 %}', {'testnav3': TestNavbar3()})
...
class PropellerCardTests(TestCase):
    ...
    def test_card_media(self):
        ...
        self.assertEqual('<div class="pmd-card-media"><div class="media-body"><img src="http://propeller.in/assets/images/profile-pic.png" class="img-responsive"></div></div>', res)
        res = DemoMedia3().as_html()
        self.assertEqual('<div class="pmd-card-media"><div class="media-body"></div><div class="media-right media-middle"><img src="http://propeller.in/assets/images/profile-pic.png" width="80" height="80"></div></div>', res)

    def test_rendered_template(self):
        res = self.render_template_with_propeller('{% propeller_card card1 %}',
          context={'card1': DemoCard1()})
        self.assertInHTML(RESULT_CARD1, res)
        res = self.render_template_with_propeller('{% propeller_card card2 %}',
          context={'card2': DemoCard2()})
        self.assertInHTML(RESULT_CARD2, res)
        res = self.render_template_with_propeller('{% propeller_card card3 %}', {'card3': DemoCard3()})
        self.assertInHTML(RESULT_CARD3, res)
        res = self.render_template_with_propeller('{% propeller_card card4 %}', {'card4': DemoCard4()})
        self.assertInHTML(RESULT_CARD4, res)
        res = self.render_template_with_propeller('{% propeller_card card5 %}', {'card5': DemoCard5()})
