...
# [GCC 8.4.0]
# Embedded file name: D:\Development\django2-propeller\django2_propeller\renderers.py
# Compiled at: 2019-04-30 10:49:49
# Size of source mod 2**32: 22233 bytes
...
from .forms import render_form, render_field, render_label, render_form_group, is_widget_with_placeholder, FORM_GROUP_CLASS, is_widget_required_attribute, render_button
...
class BaseRenderer(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.horizontal_label_class = kwargs.get('horizontal_label_class', get_propeller_setting('horizontal_label_class'))
        self.horizontal_field_class = kwargs.get('horizontal_field_class', get_propeller_setting('horizontal_field_class'))
...
class FormsetRenderer(BaseRenderer):
    ...
    def __init__(self, formset, *args, **kwargs):
        if not isinstance(formset, BaseFormSet):
            raise PropellerError('Parameter "formset" should contain a valid Django Formset.')
    ...
    def render_forms(self):
        ...
        for form in self.formset.forms:
            rendered_forms.append(self.render_form(form,
              layout=(self.layout),
    ...
    def render_errors(self):
        ...
        if formset_errors:
            return render_template_file('propeller/form_errors.html',
              context={'errors':formset_errors, 
             'form':self.formset, 
             'layout':self.layout})
        return ''
...
class FormRenderer(BaseRenderer):
    ...
    def __init__(self, form, *args, **kwargs):
        if not isinstance(form, BaseForm):
            raise PropellerError('Parameter "form" should contain a valid Django Form.')
        self.form = form
    ...
    def get_fields_errors(self):
        form_errors = []
        for field in self.form:
            if field.is_hidden or field.errors:
...
class FieldRenderer(BaseRenderer):
    ...
    def __init__(self, field, *args, **kwargs):
        if not isinstance(field, BoundField):
            ...
            if 'placeholder' in kwargs:
                ...
                if get_propeller_setting('set_placeholder'):
            ...
            if error_css_class is not None:
                ...
                self.error_css_class = getattr(field.form, 'error_css_class', get_propeller_setting('error_css_class'))
            if required_css_class is not None:
                self.required_css_class = required_css_class
            else:
                self.required_css_class = getattr(field.form, 'required_css_class', get_propeller_setting('required_css_class'))
            if bound_css_class is not None:
                self.success_css_class = bound_css_class
            else:
                self.success_css_class = getattr(field.form, 'bound_css_class', get_propeller_setting('success_css_class'))
    ...
    def add_placeholder_attrs(self, widget=None):
        ...
        if placeholder:
            if self.set_placeholder:
                if is_widget_with_placeholder(widget):
    ...
    def add_required_attrs(self, widget=None):
        ...
        if self.set_required:
            if is_widget_required_attribute(widget):
    ...
    def put_inside_label(self, html):
        content = '{field} {label}'.format(field=html,
          label=(self.field.label))
        return render_label(content=(mark_safe(content)),
    ...
    def post_widget_render(self, html):
        if isinstance(self.widget, RadioSelect):
            ...
            if isinstance(self.widget, CheckboxSelectMultiple):
                ...
                if isinstance(self.widget, SelectDateWidget):
    ...
    def make_input_group(self, html):
        if self.addon_before or self.addon_after:
            if isinstance(self.widget, (TextInput, DateInput, Select)):
                before = '<span class="{input_class}">{addon}</span>'.format(input_class=(self.addon_before_class), addon=(self.addon_before)) if self.addon_before else ''
                after = '<span class="{input_class}">{addon}</span>'.format(input_class=(self.addon_after_class), addon=(self.addon_after)) if self.addon_after else ''
                html = '<div class="input-group">{before}{html}{after}</div>'.format(before=before,
                  after=after,
                  html=html)
        return html
    ...
    def append_to_field(self, html):
        ...
        if help_text_and_errors:
            help_html = render_template_file('propeller/field_help_text_and_errors.html',
