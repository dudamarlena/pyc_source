...
# [GCC 8.4.0]
# Embedded file name: D:\Development\django2-propeller\django2_propeller\components.py
...
def render_icon(icon, size='sm', **kwargs):
    ...
    attrs = {'class': add_css_class('material-icons pmd-{size}'.format(size=size), kwargs.get('extra_classes', ''))}
...
def render_alert(content, alert_type=None, dismissable=True):
    ...
    return mark_safe(render_tag('div',
      attrs={'class': ' '.join(css_classes)},
      content=(button_placeholder + text_value(content))).replace(button_placeholder, button))


class Image(object):
    __doc__ = 'Render an image object'
    source = ''
    link = None
    width = None
    height = None
...
class Button(object):
    __doc__ = 'Render a button with content'
    attrs = {}
    content = ''
    classes = []

    def __init__(self, content, button_type='button', icon=None, button_class=None, size=None, href=None, name=None, value=None, title=None, style='default', extra_classes='', _id=''):
        ...
        if size == 'xs':
            ...
            if size == 'sm' or size == 'small':
                ...
                if size == 'lg' or size == 'large':
                    self.classes = add_css_class(self.classes, 'btn-lg')
                else:
                    if (size == 'md' or size) == 'medium':
                        pass
                    else:
                        if size:
                            raise PropellerError('Parameter "size" should be "xs", "sm", "lg" or ' + 'empty ("{}" given).'.format(size))
                        else:
                            if button_type:
                                if button_type not in ('submit', 'reset', 'button',
                                                       'link'):
                                    raise PropellerError('Parameter "button_type" should be "submit", "reset", ' + '"button", "link" or empty  ("{}" given).'.format(button_type))
                                self.attrs['type'] = button_type
                            if style not in ('default', 'raised', 'flat', 'outline'):
                                raise PropellerError('Parameter "style" should be "default", "raised", ' + '"flat", "outline" or empty  ("{}" given).'.format(style))
                            else:
    ...
    def as_html(self):
        ...
        return render_tag((self.tag), attrs=(self.attrs), content=(mark_safe(text_concat((self.icon_content), (self.content), separator=' '))))


class FAB(object):
    __doc__ = 'Render a floating action button'
    attrs = {}
    content = ''
    classes = []
                        ...
                        if size:
                            ...
                            if button_type:
                                if button_type not in ('submit', 'reset', 'button',
                                    ...
                                    raise PropellerError('Parameter "button_type" should be "submit", "reset", ' + '"button", "link" or empty  ("{}" given).'.format(button_type))
                                self.attrs['type'] = button_type
                            elif style not in ('default', 'raised', 'flat', 'outline'):
                                raise PropellerError('Parameter "style" should be "default", "raised", ' + '"flat", "outline" or empty  ("{}" given).'.format(style))
                            else:
                                self.classes = add_css_class(self.classes, 'pmd-btn-%s' % style)
                            self.classes = add_css_class(self.classes, pmd_class)
