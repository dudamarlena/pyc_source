...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/competition/admin.py
...
from competition.models import Competition, CompetitionEntry, CompetitionPreferences, CompetitionAnswerOption
...
class CompetitionAnswerOptionAdminFormSet(BaseInlineFormSet):
    ...
    def clean(self):
        ...
        if self.instance:
            ...
            elif self.instance.answer_type == 'multiple_choice_selection':
...
class CompetitionAdmin(ModelBaseAdmin):
    inlines = (
     CompetitionAnswerOptionAdmin,)
    form = CompetitionAdminForm
    list_display = ('title', 'start_date', 'end_date', 'description', '_entries', '_get_absolute_url',
                    '_actions')
...
class CompetitionEntryAdmin(admin.ModelAdmin):
    list_display = ('__unicode__', 'user_link', 'user_fullname', 'user_email', 'user_mobile_number',
                    'has_correct_answer', 'file_link', 'winner')
    ...
    def get_urls(self):
        ...
        csv_urls = patterns('', url('^exportcsv/$', self.admin_site.admin_view(self.csv_export), name='competition-csv-export'))
    ...
    def csv_export(self, request):
        ...
        queryset = self.get_changelist(request)(request, self.model, self.list_display, self.list_display_links, self.list_filter, self.date_hierarchy, self.search_fields, self.list_select_related, self.list_per_page, self.list_max_show_all, self.list_editable, self).get_query_set(request)
        competition_map = {}
        ids = queryset.distinct('competition').values_list('competition_id', flat=True)
        for obj in Competition.objects.filter(id__in=ids):
            competition_map[obj.id] = obj

        member_mobile_number_map = {}
        ids = queryset.distinct('user').values_list('user_id', flat=True)
        for di in Member.objects.filter(id__in=ids).values('id', 'mobile_number'):
        ...
        for entry in queryset:
            ...
            row = [
             ...
             entry.winner,
             entry.timestamp]
            writer.writerow([ '' if f is None else unicode(f) for f in row ])
