...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/competition/forms.py
...
class CompetitionBaseEntryForm(forms.Form):
    accept_terms = forms.BooleanField(required=True, label='')
    ...
    def save(self):
        entry = CompetitionEntry(user=self.request.user, competition=self.competition)
        entry.save()
...
class SingleAnswerEntryForm(CompetitionBaseEntryForm):
    ...
    def save(self):
        entry = CompetitionEntry(user=self.request.user, competition=self.competition, answer_text=self.cleaned_data['answer'])
        entry.save()


class MultichoiceEntryForm(CompetitionBaseEntryForm):
    option = forms.ChoiceField(widget=RadioSelect, choices=[], required=True)

    def __init__(self, *args, **kwargs):
        super(MultichoiceEntryForm, self).__init__(*args, **kwargs)
        self.fields['option'].choices = [ (o.id, o.text) for o in self.competition.competitionansweroption_set.all() ]

    def save(self):
        entry = CompetitionEntry(user=self.request.user, competition=self.competition, answer_option_id=self.cleaned_data['option'])
        entry.save()
...
class FileUploadEntryForm(CompetitionBaseEntryForm):
    ...
    def clean_file(self):
        if self.cleaned_data['file'].size > self.competition.max_file_size * 1024:
            raise forms.ValidationError(_('The file is too large. It may not be larger than %d kB.' % self.competition.max_file_size))
        return self.cleaned_data['file']

    def save(self):
        entry = CompetitionEntry(user=self.request.user, competition=self.competition, answer_file=self.cleaned_data['file'])
        entry.save()