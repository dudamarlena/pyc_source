...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/competition/models.py
...
class Competition(ModelBase):
    content = RichTextField(help_text='Background info and explanation of the competition.')
    check_in_distance = models.PositiveIntegerField(default=0, help_text='Distance threshold for check ins, in metres. 0 to disable checking in to enter.', blank=True, null=True)
    ...
    answer_type = models.CharField(max_length=32, choices=(
     ('free_text_input', 'Free text input'),
     ('multiple_choice_selection', 'Multiple choice selection'),
     ('file_upload', 'File upload')), blank=True, null=True, help_text='What type of answer is expected of the user, if any?')
    correct_answer = models.CharField(max_length=255, blank=True, null=True, help_text="Answer used to determine winning entries. If there are multiple correct answers, enter a comma-separated list. <b>(only for answer type 'Free text input')</b>")
    max_file_size = models.PositiveIntegerField(blank=True, null=True, help_text="The maximum size in kB for a file upload.  <b>(only for answer type 'File upload')</b>")
    rules = RichTextField(blank=True, null=True, help_text='Rules specific to this competition.')
    start_date = models.DateField(help_text='Date the competition starts.')
    end_date = models.DateField(help_text='Date the competition ends.')
    max_entries_per_user = models.IntegerField(default=1, help_text='Maximum number of times a user can enter the competition.')
    ...
    def can_enter(self, request):
        if request.user.is_authenticated():
            ...
            if current_date > self.end_date:
                return (False, 'ended')
            if CompetitionEntry.objects.filter(user=request.user, competition=self).count() >= self.max_entries_per_user:
...
class CompetitionAnswerOption(models.Model):
    text = models.CharField(max_length=255, help_text="The option text shown to the user. <b>(only for answer type 'Multiple choice selection')</b>")
...
class CompetitionEntry(models.Model):
    ...
    answer_option = models.ForeignKey(CompetitionAnswerOption, null=True, blank=True)
    answer_file = models.FileField(upload_to=get_file_upload_path, null=True, blank=True)
