...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tt/actions/read/report.py
...
def action_report(colorizer, activity):
    ...
    for date, details in sorted(report.items()):
        ...
        print((details['weekday']), sep, date, sep, start_time, sep, end_time, sep, (format_time(break_duration, colorizer)),
          sep, (format_time(details['sum'], colorizer)), sep, (details['notes']), sep='')

    should_hours = 8 * len(report.items())
    should_hours_str = str(should_hours) + ':00'
    print()
    print('Based on your current entries, you should have logged ', (colorizer.green(should_hours_str)), ' ; you instead logged ', (format_time_seconds(total_time, colorizer)),
      sep='')
