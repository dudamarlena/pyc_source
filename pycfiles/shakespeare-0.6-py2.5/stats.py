...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/shakespeare/stats.py
...
"""
import re, xapian, shakespeare.model as model
...
class Stats(object):
    ...
    def statsify(self, material, fileobj):
        ...
        for k in stats:
            model.Statistic(text=material, word=k, freq=stats[k])

        model.Session.flush()

    def freq(self, text, word):
        stat = model.Statistic.query.filter_by(text=text).filter_by(word=word).first()
        if stat:
            return stat.freq
        else:
            return 0
    ...
    def text_stats(self, text):
        ...
        stats = model.Statistic.query.filter_by(text=text).order_by(model.Statistic.freq.desc()).all()
        return stats

    def word_stats(self, word):
        """Statistics for word (i.e. which texts use it) in order or
        usage."""
        stats = model.Statistic.query.filter_by(word=word).order_by(model.Statistic.freq.desc()).all()
        return stats