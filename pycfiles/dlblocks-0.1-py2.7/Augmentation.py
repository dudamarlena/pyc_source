...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/dlblocks/Augmentation.py
# Compiled at: 2018-12-11 12:52:10
import numpy as np, cv2, random, time, math, glob, os
...
def rotate_image(image, angle):
    ...
    result = cv2.warpAffine(image, affine_mat, (
     new_w, new_h), flags=cv2.INTER_LINEAR)
    return result
...
def augmentCVImages(cv_images, seed=None):
    ...
    for im in cv_images:
        ...
        image_rotated_cropped = crop_around_center(image_rotated, *largest_rotated_rect(width, height, math.radians(rotation)))
        ...
        windowY4 -= shiftY
        if flip == 1:
            windowX1, windowX2, windowX3, windowX4 = (
             windowX2, windowX1, windowX4, windowX3)
