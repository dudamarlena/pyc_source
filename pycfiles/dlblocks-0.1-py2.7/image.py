...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/dlblocks/image.py
# Compiled at: 2018-12-11 12:52:10
import numpy as np, cv2, pyutils
IMG_WIDTH = 224
IMG_HIEGHT = 224
VGGIMG_SIZE = 224
import threading
...
def readimgtar(fname, tarf, FLAGS=1):
    with lock:
        ...
        return cv2.imdecode(get_np_array_from_tar_object(xx), FLAGS)
...
def getImageVec(path, width=IMG_WIDTH, height=IMG_HIEGHT, cropCoordinates=None, augment=False, augmentationSeed=None, imgNorm='sub_mean', pipelineOrder=['crop', 'augment', 'resize', 'norm'], maintainAspectRatio=False, src_type='file', src=None, ordering='th', readMode=1):
    if src_type == 'file':
        ...
        for fn in pipelineOrder:
            if fn == 'crop':
                if cropCoordinates is not None:
            ...
            if fn == 'resize' and width is not None:
...
def getImageClassificationVec(d, baseImagePath, nClasses, width=IMG_WIDTH, height=IMG_HIEGHT, augment=False, augmentationSeed=None, imgNorm='sub_mean', maintainAspectRatio=False):
    return (
     getImageVec(baseImagePath + d['img'], width=width, height=height, augment=augment, augmentationSeed=augmentationSeed, imgNorm=imgNorm, maintainAspectRatio=maintainAspectRatio),
