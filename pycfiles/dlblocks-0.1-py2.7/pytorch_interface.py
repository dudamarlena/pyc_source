...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/dlblocks/pytorch_interface.py
...
def aggregate_loss_list(losses, loss_weights=None):
    ...
    def loss_function(self, x=None, y=None, model_output=None):
        if y is None:
            y = [
             None] * len(model_output)
        if loss_weights2 is None:
            loss_weights2 = [
             1.0] * len(model_output)
        loss_t = None
