...
# [GCC 8.4.0]
# Embedded file name: C:\Users\faoustin\Workspace\todotxt-cli\todotxt\task.py
# Compiled at: 2019-10-13 04:27:10
...
class Task(object):
    ...
    def __str__(self):
        ...
        if self.done:
            for_str.append('x')
        if self.priority:
            for_str.append('(%s)' % self.priority)
        if self.completed:
            if self.created:
                for_str.append(self.completed)
        if self.created:
            for_str.append(self.created)
