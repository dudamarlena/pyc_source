...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.7.1/lib/python3.7/site-packages/satella/files.py
# Compiled at: 2020-04-29 12:14:00
# Size of source mod 2**32: 3510 bytes
import typing as tp, re, os
__all__ = [
 'read_re_sub_and_write', 'find_files', 'split']
SEPARATORS = {
 '\\', '/'}
...
def read_re_sub_and_write(path: str, pattern: tp.Union[(re.compile, str)], repl: tp.Union[tp.Callable[([tp.Any], str)]]) -> None:
    """
...
def find_files(path: str, wildcard: str='(.*)', prefix_with: tp.Optional[str]=None, scan_subdirectories: bool=True, apply_wildcard_to_entire_path: bool=False, prefix_with_path: bool=True) -> tp.Iterator[str]:
    """
    ...
    for filename in os.listdir(path):
        if scan_subdirectories:
            if os.path.isdir(os.path.join(path, filename)):
                ...
                yield from find_files((os.path.join(path, filename)), wildcard, prefix_with=new_prefix, prefix_with_path=False)
        elif apply_wildcard_to_entire_path:
