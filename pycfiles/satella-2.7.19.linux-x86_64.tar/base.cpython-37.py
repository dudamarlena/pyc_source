...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.7.1/lib/python3.7/site-packages/satella/instrumentation/metrics/metric_types/base.py
...
class Metric:
    ...
    def __init__(self, name, root_metric: 'Metric'=None, metric_level: tp.Optional[tp.Union[(MetricLevel, int)]]=None, internal: bool=False, *args, **kwargs):
        ...
        if self.name == '':
            if self.level == MetricLevel.INHERIT:
                raise AssertionError('Unable to set INHERIT for root metric!')
        self.children = []
    ...
    def level(self, value: MetricLevel) -> None:
        if value == MetricLevel.INHERIT:
            if self.name == '':
                raise AssertionError('Cannot set INHERIT for the root metric!')
        self._level = value
