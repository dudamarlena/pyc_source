...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vision/flickr.py
# Compiled at: 2011-01-20 19:34:56
import httplib, urllib, random, time, Image, os
from xml.etree import ElementTree
import logging, StringIO
all_extras = 'description,license,date_upload,date_taken,owner_name,icon_server,original_format,last_update,geo,tags,machine_tags,o_dims,views,media,path_alias,url_sq,url_t,url_s,url_m,url_o'
keys = []
...
def search(tags, perpage=100):
    ...
    (page, pages) = (1, 2)
    while page < pages:
        photos = request('flickr.photos.search', {'tags': tags, 
           'page': page, 
           'per_page': perpage, 
           'extras': all_extras})
...
def recent(perpage=500):
    ...
    """
    photos = request('flickr.photos.getRecent', {'per_page': perpage, 
       'extras': all_extras})
...
def pascal(tags, range=(2003, 2010)):
    ...
    while True:
        ...
        rtime = time.localtime(random.randint(start, stop))[0:3] + (0, 0, 0, 0, 0,
                                                                    -1)
        rtime = time.mktime(rtime)
        stm, etm = int(rtime), int(rtime + 86400)
        tag = random.choice(tags)
        r = request('flickr.photos.search', {'text': tag, 
           'min_upload_date': stm, 
           'max_upload_date': etm, 
           'per_page': perpage, 
           'page': 1})
