...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/tests/setup.py
# Compiled at: 2016-04-06 06:07:34
# Size of source mod 2**32: 15116 bytes
"""
...
import os
from ninecms.models import PageType, Node, NodeRevision, MenuItem, ContentBlock, Image, TaxonomyTerm, File, Video, validate_file_ext, validate_video_ext
...
def create_page(page_type_name, page_type_description, alias, language, title):
    ...
    node, created = Node.objects.get_or_create(page_type=page_type, language=language, title=title, user=admin, body=title + ' page.', alias=alias, status=True)
...
def get_second_language():
    ...
    if settings.I18N_URLS:
        return next((val for i, val in enumerate(settings.LANGUAGES) if settings.LANGUAGE_CODE not in val), ('',
                                                                                                             ''))[0]
    else:
...
def create_menu(language=''):
    ...
    if language == '' or language == 'en':
        menu, created = MenuItem.objects.get_or_create(parent=None, weight=0, language=language, path='', title='Main Menu')
        ...
        MenuItem.objects.get_or_create(parent=menu, weight=3, language=language, path='http://google.com/', title='Google')
        MenuItem.objects.get_or_create(parent=menu, weight=4, language=language, path='#bookmark', title='A bookmark')
    else:
        menu, created = MenuItem.objects.get_or_create(parent=None, weight=0, language='el', path='', title='Kyriws Menu')
...
def url_with_lang(url):
    ...
    """
    prefix = ''
    if 1 < len(settings.LANGUAGES) < 80:
