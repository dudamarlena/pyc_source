...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/tests/tests_content_login.py
...
from ninecms.tests.setup import create_front, create_basic, create_user, create_image, create_block_simple, get_front_title, assert_front, data_login, data_node, get_basic_title, create_video, create_file, data_page_type, create_terms
from ninecms.models import PageType, Node, PageLayoutElement, ContentBlock, TaxonomyTerm
import os

class ContentLoginTests(TestCase):
    __doc__ = ' Tests with content, with login '
    ...
    def setUpTestData(cls):
        ...
        PageLayoutElement(page_type=cls.node_rev_front.node.page_type, region='footer', block=cls.block_user_menu, weight=0).save()
    ...
    def test_admin_index_page(self):
        ...
        self.assertContains(response, '<span class="stat-latest-users">Latest: <a href="/admin/auth/user/">admin</a></span>', html=True)
        self.assertContains(response, '<span class="stat-count-staff h1">1</span>', html=True)
        self.assertContains(response, '<span class="stat-latest-staff">Latest: <a href="/admin/auth/user/">admin</a></span>', html=True)
        self.assertContains(response, '<span class="stat-count-superusers h1">1</span>', html=True)
        self.assertContains(response, '<span class="stat-latest-superusers">Latest: <a href="/admin/auth/user/">admin</a></span>', html=True)
        self.assertContains(response, '<span class="stat-count-nodes h1">2</span>', html=True)
        self.assertContains(response, '<span class="stat-latest-nodes">Latest: <a href="/admin/ninecms/node/">%s</a></span>' % get_basic_title(settings.LANGUAGE_CODE, ''), html=True)
        self.assertContains(response, '<span class="stat-count-page-types h4">2</span>', html=True)
        self.assertContains(response, '<span class="stat-latest-page-types">Latest: <a href="/admin/ninecms/pagetype/">Basic Page</a></span>', html=True)
        self.assertContains(response, '<span class="stat-count-images h4">1</span>', html=True)
        self.assertContains(response, '<span class="stat-latest-images">Latest: <a href="/admin/ninecms/node/">%s</a></span>' % get_basic_title(settings.LANGUAGE_CODE, ''), html=True)
    ...
    def test_admin_node_change_page(self):
        ...
        self.assertContains(response, '<input class="form-control vTextField" id="id_title" maxlength="255" name="title" placeholder="Title" required="required" title="" type="text" value="%s">' % self.node_rev_basic.node.title, html=True)
    ...
    def test_admin_node_add_page(self):
        ...
        self.assertContains(response, '<input checked="checked" class="" id="id_status" name="status" type="checkbox" value="1">', html=True)
        self.assertContains(response, '<input class="" id="id_promote" name="promote" type="checkbox">', html=True)
        self.assertNotContains(response, '<input checked="checked"  class="" id="id_promote" name="promote" type="checkbox">', html=True)
        self.assertContains(response, '<input class="" id="id_sticky" name="sticky" type="checkbox">', html=True)
        self.assertNotContains(response, '<input checked="checked"  class="" id="id_sticky" name="sticky" type="checkbox">', html=True)
    ...
    def test_content_node_edit_form_valid(self):
        ...
        self.assertEqual(form.cleaned_data['body'], '<div>\xa0</div>&lt;script&gt;alert("This is a test.");&lt;/script&gt;')
    ...
    def test_admin_page_type_change_page(self):
        ...
        :return: None
        """
        response = self.client.get(reverse('admin:ninecms_pagetype_change', args=(
         self.node_rev_basic.node.page_type_id,)))
        self.assertContains(response, '<input class="form-control vTextField" id="id_name" maxlength="100" name="name" placeholder="Name"')
        self.assertContains(response, '<select multiple="multiple" class="selectfilterstacked" id="id_blocks" name="blocks" title="">')
    ...
    def test_admin_content_block_page(self):
        ...
        self.assertContains(response, '<th scope="col" class="sortable column-name"><a href="?o=1">Name</a></th>', html=True)
        self.assertContains(response, '<a href="%s">user-menu</a>' % reverse('admin:ninecms_contentblock_change', args=(
         self.block_user_menu.pk,)), html=True)
        self.assertContains(response, '<a href="%s">login</a>' % reverse('admin:ninecms_contentblock_change', args=(
         self.block_login.pk,)), html=True)
        self.assertContains(response, '<th scope="col"  class="column-page_types_list"><span>Page types</span></th>', html=True)
        self.assertContains(response, '<a href="%s">Front Page</a>' % reverse('admin:ninecms_pagetype_change', args=(self.node_rev_front.pk,)), html=True)
