...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/rabelvideo/ninecms/utils/sanitize.py
# Compiled at: 2015-11-06 07:28:53
...
def sanitize(t, allow_html=True, full_html=False):
    ...
    if full_html:
        allowed_tags += ['div']
    allowed_attributes = {'a': ['href', 'title', 'name', 'target', 'class'],  'abbr': [
              'title'], 
     'acronym': [
                 'title'], 
     'p': [
     ...
     'img': [
             'src', 'alt', 'title', 'class'], 
     'iframe': [
                'src', 'height', 'width', 'class'], 
     'table': [
               'border', 'cellpadding', 'cellspacing'], 
     'th': [
            'scope', 'rowspan', 'colspan', 'class'], 
     'td': [
            'scope', 'rowspan', 'colspan', 'class'], 
     'span': [
              'style', 'class'], 
     'div': [
             'style', 'class']}
    allowed_styles = [
     'margin-left', 'text-align', 'width', 'page-break-after', 'display', 'float']
...
class ModelSanitizeForm(forms.ModelForm):
    __doc__ = ' A ModelForm that sanitizes specified fields '
