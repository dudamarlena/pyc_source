...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/tests/tests_content_login_simple.py
...
class ContentLoginSimpleTests(TestCase):
    __doc__ = ' Tests for access, with content, with login from simple user\n    The first page (front) is allowed to be edited by simple user\n    The second is not.\n    '
    ...
    def test_admin_node_change_page(self):
        ...
        :return: None
        """
        response = self.client.get(reverse('admin:ninecms_node_change', args=(self.node_rev_basic.node_id,)), follow=True)
        ...
        self.assertContains(response, '<select class="form-control form-control-inline" id="id_user" name="user" title=""><option value="">---------</option><option value="%d">%s</option></select>' % (
         self.simple_user.pk, self.simple_user.username), html=True)
    ...
    def test_admin_node_add_page(self):
        ...
        self.assertNotContains(response, '<label for="id_alias">Alias:</label>', html=True)
        self.assertNotContains(response, '<label class="vCheckboxLabel" for="id_redirect">Redirect</label>', html=True)
        self.assertContains(response, '<select class="form-control form-control-inline" id="id_user" name="user" title=""><option value="">---------</option><option value="%d" selected="selected">%s</option></select>' % (
         self.simple_user.pk, self.simple_user.username), html=True)
