...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/admin.py
# Compiled at: 2016-04-06 06:07:34
# Size of source mod 2**32: 14679 bytes
...
class PageTypeAdmin(admin.ModelAdmin):
    __doc__ = ' Get a list of Page Types '
    ...
    def get_urls(self):
        ...
        """
        urls = [
         url('^(?P<type_id>\\d+)/perms/$', self.admin_site.admin_view(views.ContentTypePermsView.as_view()), name='ninecms_pagetype_perms')]
        return urls + super(PageTypeAdmin, self).get_urls()


class NodeRevisionInline(admin.StackedInline):
    __doc__ = ' Node Revision stacked inline to be displayed in Nodes (NodeAdmin) '
    model = models.NodeRevision
    extra = 0


class ImageInline(admin.StackedInline):
    __doc__ = ' Images inline to be displayed in Nodes (NodeAdmin) '
    model = models.Image
...
class FileInline(admin.StackedInline):
    __doc__ = ' Files inline to be displayed in Nodes (NodeAdmin) '
    model = models.File
    form = forms.FileForm
    extra = 0
...
class VideoInline(admin.StackedInline):
    __doc__ = ' Videos inline to be displayed in Nodes (NodeAdmin) '
    model = models.Video
...
class NodeAdmin(admin.ModelAdmin):
    __doc__ = ' Get a list of Nodes, also use inlines in Node form '
    list_display = ('title', 'page_type', 'language', 'alias', 'user', 'status', 'promote',
                    'sticky', 'created', 'changed', 'original_translation', 'redirect',
                    'operations')
    ...
    inlines = [
     ImageInline, FileInline, VideoInline, NodeRevisionInline]
    ...
    def get_actions(self, request):
        ...
        if not request.user.has_perm('ninecms.delete_node'):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if request.user.is_superuser:
            return (
             ...
             (
              'Node management',
              {'fields': ('status', 'promote', 'sticky', 'redirect', 'user', 'created', 'original_translation',
 'weight')}),
             (
              'Terms', {'fields': ('terms', )}))
        else:
            return (('Node', {'fields': ('page_type', 'language', 'title')}),
             (
              'Body', {'fields': ('highlight', 'summary', 'body', 'link')}),
             (
              'Node management',
              {'fields': ('status', 'promote', 'sticky', 'user', 'created', 'original_translation', 'weight')}),
             (
              'Terms', {'fields': ('terms', )}))
...
class MenuItemAdmin(MPTTModelAdmin):
    __doc__ = ' Get a list of Menu Items '
...
class ContentBlockAdmin(admin.ModelAdmin):
    ...
    def page_types_list(self, obj):
        ...
        for page_type in obj.page_types.all():
            r.append('<a href="%s">%s</a>' % (reverse('admin:ninecms_pagetype_change', args=(page_type.id,)), page_type))
...
class TaxonomyTermAdmin(MPTTModelAdmin):
    __doc__ = ' Get a list of Taxonomy Terms '
