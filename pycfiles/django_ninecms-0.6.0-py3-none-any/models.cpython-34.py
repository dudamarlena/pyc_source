...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/models.py
# Compiled at: 2016-04-06 06:07:34
# Size of source mod 2**32: 15694 bytes
...
from ninecms.utils.media import image_path_file_name, file_path_file_name, validate_file_ext, video_path_file_name, validate_video_ext
from ninecms.utils.transliterate import transliterate
import re

class PageType(models.Model):
    __doc__ = ' Page Type Model: acts as a single page layout '
    name = models.CharField(max_length=100, unique=True, verbose_name=_('name'), help_text=_('Specify a unique page type name. A machine name is recommended if to be used in code.'))
    description = models.CharField(max_length=255, verbose_name=_('description'), help_text=_('Describe the page type.'))
    guidelines = models.CharField(max_length=255, blank=True, verbose_name=_('guidelines'), help_text=_('Provide content submission guidelines for this page type.'))
    url_pattern = models.CharField(max_length=255, blank=True, help_text=_('Default pattern for page type, if no alias is specified in node edit. <a href="https://github.com/Wtower/django-ninecms#url-aliases" target="_blank">More info</a>.'))
...
class Node(models.Model):
    __doc__ = ' Node Model: basic content record '
    ...
    original_translation = models.ForeignKey('self', null=True, blank=True, related_name='Translations', verbose_name=_('original translation'))
    ...
    def save(self, *args, **kwargs):
        ...
        """
        if not self.alias:
            if self.page_type.url_pattern:
                self.alias = self.page_type.url_pattern.replace('[node:title]', transliterate(self.title, False, True))
            ...
            if self.alias:
                if Node.objects.filter(alias=self.alias).filter(language=self.language).count() > 1:
...
class NodeRevision(models.Model):
    __doc__ = ' Node Revision Model: Basic content archive, Drupal style '
...
class MenuItem(MPTTModel):
    __doc__ = ' Menu Item Model: menu tree item in a single tree, paths can be empty for parents '
    ...
    class MPTTMeta:
        __doc__ = ' Set order when inserting items for mptt '
...
class ContentBlock(models.Model):
    __doc__ = ' Content Block Model: basic block instance which can be used in several page layouts '
    name = models.CharField(max_length=100, null=True, verbose_name=_('name'), help_text=_('Specify a unique block machine name.'))
    BLOCK_TYPES = (
     (
    ...
    type = models.CharField(max_length=50, choices=BLOCK_TYPES, default='static', verbose_name=_('type'), help_text=_('How to render the block.'))
    classes = models.CharField(max_length=255, blank=True, help_text=_('Additional CSS classes to append to block.'))
    node = models.ForeignKey(Node, null=True, blank=True, verbose_name=_('node'), help_text=_('The related node with this block (block type: static only).'))
    menu_item = TreeForeignKey(MenuItem, null=True, blank=True, verbose_name=_('menu item'), help_text=_('The related parent menu item related (block type: menu only).'))
    signal = models.CharField(max_length=100, blank=True, help_text=_('The signal name to trigger for a <a href="https://github.com/Wtower/django-ninecms#views" target="_blank">custom view</a> (block type: signal only).'))
    page_types = models.ManyToManyField(PageType, blank=True, related_name='blocks', verbose_name=_('page types'))
...
class PageLayoutElement(models.Model):
    __doc__ = ' Page Layout Element Model: a set of these records define the layout for each page type '
    page_type = models.ForeignKey(PageType, verbose_name=_('page type'))
    region = models.CharField(max_length=50, db_index=True, verbose_name=_('region'), help_text=_('A hard coded region name that is rendered in template index and also used in <a href="https://github.com/Wtower/django-ninecms#theme-suggestions" target="_blank">theme suggestions</a>.'))
    block = models.ForeignKey(ContentBlock, verbose_name=_('content block'))
    weight = models.IntegerField(default=0, db_index=True, verbose_name=_('order weight'), help_text=_('Elements with greater number in the same region sink to the bottom of the page.'))
...
class Media(models.Model):
    __doc__ = ' Media Model: abstract model for media, one node-many images relationship '
    node = models.ForeignKey(Node, verbose_name=_('node'))
...
class Image(Media):
    __doc__ = ' Image Model: the basic image record '
...
class File(Media):
    __doc__ = ' File Model: a file field '
    file = models.FileField(upload_to=file_path_file_name, max_length=255, validators=[validate_file_ext], verbose_name=_('file'))
...
class Video(Media):
    __doc__ = ' Video Model: a video file field '
    video = models.FileField(upload_to=video_path_file_name, max_length=255, validators=[validate_video_ext])
    VIDEO_TYPES = (('mp4', 'video/mp4'), ('webm', 'video/webm'), ('ogg', 'video/ogg'),
                   ('flv', 'video/flv'), ('swf', 'application/x-shockwave-flash'),
                   ('jpg', 'image/jpeg'))
...
class TaxonomyTerm(MPTTModel):
    __doc__ = ' Taxonomy term model: the basic term record, m2m relationship with nodes '
    parent = TreeForeignKey('self', null=True, blank=True, related_name='children', verbose_name=_('parent term'))
    name = models.CharField(max_length=50, verbose_name=_('name'))
    weight = models.IntegerField(default=0, db_index=True, verbose_name=_('order weight'))
    description_node = models.ForeignKey(Node, null=True, blank=True, related_name='term_described', verbose_name=_('description node'))
