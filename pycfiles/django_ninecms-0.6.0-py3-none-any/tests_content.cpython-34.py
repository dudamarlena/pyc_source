...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/tests/tests_content.py
# Compiled at: 2016-04-06 03:40:49
# Size of source mod 2**32: 20148 bytes
"""
...
from ninecms.tests.setup import create_front, create_basic, create_menu, create_block_static, create_block_menu, create_block_signal_terms, create_block_simple, create_page, create_image, create_file, create_video, create_terms, assert_front, assert_basic, create_user, assert_image, data_contact, get_front_title, data_login, url_with_lang

class ContentTests(TestCase):
    __doc__ = ' Tests with some default initial content, no login '
    ...
    def test_node_util_methods(self):
        ...
        self.assertEqual(transliterate('Τάχιστη αλώπηξ βαφής ψημένη γη, δρασκελίζει υπέρ νωθρού κυνός'), 'Tachisti-alopix-bafis-psimeni-gi-draskelizei-yper-nothroy-kynos')
        self.assertEqual(transliterate('Ξεσκεπάζω την ψυχοφθόρα βδελυγμία%.doc', True, True), 'xeskepazo_tin_psychofthora_bdelygmia.doc')
    ...
    def test_content_view_alias_redirect(self):
        ...
        :return: None
        """
        Node.objects.get_or_create(page_type=self.node_rev_basic.node.page_type, language=settings.LANGUAGE_CODE, title='redirect', user=create_user(), alias='redirect', link='/', redirect=True)
    ...
    def test_node_aliases(self):
        ...
        :return: None
        """
        page_type = PageType.objects.create(name='test_aliases', description='Test aliases', url_pattern='test/[node:title]/[node:created:Y-m-d]/[node:id]')
    ...
    def test_node_aliases_duplicates(self):
        ...
        :return: None
        """
        page_type = PageType.objects.create(name='test_aliases_duplicates', description='Test aliases duplicates', url_pattern='test/[node:title]')
        node = Node.objects.create(page_type=page_type, title='Test aliases node', user=self.node_rev_basic.node.user)
