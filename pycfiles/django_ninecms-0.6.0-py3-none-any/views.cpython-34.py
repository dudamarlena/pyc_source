...
# [GCC 8.4.0]
# Embedded file name: /home/gkarak/workspace/python/django-ninecms/ninecms/views.py
...
class ContentNodeView(NodeView):
    ...
    def get(self, request, **kwargs):
        ...
        if not node.status:
            if not request.user.has_perm('ninecms.view_unpublished'):
                raise PermissionDenied
        return self.render(node, request)


class AliasView(NodeView):
    __doc__ = ' Render content based on Url Alias '
...
class ContactView(View):
    __doc__ = ' Handle contact post request '
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            c = {'sender_name': form.cleaned_data['sender_name'], 
...
class LoginView(View):
    __doc__ = ' Handle login post request '
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                msg = _('Unfortunately the username or password are not correct. If you have forgotten your password use the link below the form to recover it.')
                messages.warning(request, msg)
        else:
...
class LogoutView(View):
    __doc__ = ' Handle logout post request '
...
class ContentTypePermsView(View):
    __doc__ = ' Content edit form at /cms/types/<id>/edit '
    ...
    def get(self, request, **kwargs):
        ...
        return render(request, 'admin/ninecms/pagetype/perms_form.html', {'permissions_form': permissions_form, 
         'groups': groups, 
         'page_type': page_type, 
    ...
    def post(self, request, **kwargs):
        ...
        if permissions_form.is_valid():
            if request.user.has_perm(('guardian.add_groupobjectpermission', 'guardian.change_groupobjectpermission',
...
class StatusView(View):
    __doc__ = ' Status page at cms/status '
