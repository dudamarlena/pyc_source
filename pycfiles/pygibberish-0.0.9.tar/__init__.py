...
# [GCC 8.4.0]
# Embedded file name: pygibberish/__init__.py
# Compiled at: 2018-06-13 07:09:44
import math, os, pickle, sys, itertools, time, logging, logging.handlers
...
class Gibberish(object):
    ...
    def __init__(self, par):
        if isinstance(par, int):
            ...
            for i in xrange(self.gram - 2):
                self.pos_x = [ ('').join(i) for i in itertools.product(self.pos_x, self.accepted_chars) ]
    ...
    def train(self, big):
        ...
        if not os.path.exists(big):
            return False
        self.matrix = [ [ 1 for i in xrange(len(self.pos_y)) ] for i in xrange(len(self.pos_x)) ]
    ...
    def save(self, fname):
        ...
        if os.path.exists(fname):
            ...
            return False
        data = {'matrix': self.matrix, 'accepted_chars': self.accepted_chars, 
           'pos_y': self.pos_y, 
           'pos_x': self.pos_x, 
           'gram': self.gram}
...
if __name__ == '__main__':
    ...
    def initlogger(loglevel, logo):
        ...
        Rthandler = logging.handlers.RotatingFileHandler(log_path + '/%s.%d.%s.log' % (
         os.path.basename(str(sys.argv[0])), os.getpid(), logo), maxBytes=314572800, backupCount=20)
