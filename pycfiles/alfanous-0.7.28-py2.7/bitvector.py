...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/support/bitvector.py
...
class BitVector(object):
    ...
    def __str__(self):
        get = self.__getitem__
        return ('').join(('1' if get(i) else '0') for i in xrange(0, self.size))
    ...
    def count(self):
        ...
        if self.bcount is None:
            self.bcount = sum(BYTE_COUNTS[(b & 255)] for b in self.bits)
        return self.bcount
