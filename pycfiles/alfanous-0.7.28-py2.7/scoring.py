...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/scoring.py
...
class BM25F(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        if not ixreader.scorable(fieldnum):
            return weight
...
class DFree(Weighting):
    ...
    def score(self, searcher, fieldnum, text, docnum, weight, QTF=1):
        ...
        return QTF * norm * (weight * -log(prior * invprior, 2) + (weight + 1.0) * +log(post * invprior, 2) + 0.5 * log(post / prior, 2))
...
class MultiFieldSorter(FieldSorter):
    ...
    def order(self, searcher, docnums, reverse=False):
        ...
        return sorted(docnums, key=lambda x: tuple(c[x] for c in caches), reverse=reverse)