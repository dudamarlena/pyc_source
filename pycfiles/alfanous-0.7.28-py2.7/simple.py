...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/qparser/simple.py
...
class SimpleParser(object):
    ...
    def __init__(self, fieldname, schema=None, termclass=Term, phraseclass=Phrase, minmatch=0, minpercent=0.75, phrasefields=None):
...
class DisMaxParser(SimpleParser):
    ...
    def __init__(self, fieldboosts, schema=None, termclass=Term, phraseclass=Phrase, minmatch=0, minpercent=0.75, tiebreak=0.0, phrasefields=None):
    ...
    def make_clause(self, text):
        clauses = [ self.make_basic_clause(fieldname, text, boost=boost) for fieldname, boost in self.fieldboosts.iteritems()
                  ]
        return DisjunctionMax(clauses, tiebreak=self.tiebreak)

    def make_filter_clause(self, text):
        return Or([ self.make_basic_clause(fieldname, text) for fieldname in self.fieldboosts.iterkeys()
                  ])
