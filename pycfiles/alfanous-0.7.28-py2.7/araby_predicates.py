...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/PyArabic/araby_predicates.py
...
def isArabicword(word):
    ...
    if len(word) == 0:
        ...
        if re.search('([^\u0600-ْ%s%s%s\\w])' % (LAM_ALEF, LAM_ALEF_HAMZA_ABOVE, LAM_ALEF_MADDA_ABOVE), word):
            return False
        if isHaraka(word[0]) or word[0] in (WAW_HAMZA, YEH_HAMZA):
            return False
