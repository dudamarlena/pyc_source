...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/filebeat_oracle.py
# Compiled at: 2020-01-10 21:07:18
...
if __name__ == '__main__':
    ...
    for env in ('oracle_name', 'oracle_version', 'oracle_alert_file_path', 'elastic_host',
                'elastic_index_format'):
        if os.environ.get(env) is None:
            print('env {} is Nnone.'.format(env))
            sys.exit(1)
    else:
        oracle_alert_file_timestamp_regex = '^[A-Za-z]{3}\\s[A-Za-z]{3}\\s[0-9]{2}\\s[0-9]{2}:[0-9]{2}:[0-9]{2}\\s[0-9]{4}$'
        if oracle_version in (9, 10):
            oracle_alert_file_timestamp_regex = '^[A-Za-z]{3}\\s[A-Za-z]{3}\\s[0-9]{2}\\s[0-9]{2}:[0-9]{2}:[0-9]{2}\\sCST\\s[0-9]{4}$'
        main()