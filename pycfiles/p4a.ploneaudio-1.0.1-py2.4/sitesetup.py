...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/p4a/ploneaudio/sitesetup.py
...
def setup_site(site):
    ...
    if not sm.queryUtility(interfaces.IAudioSupport):
        if HAS_FLSM:
            sm.registerUtility(content.AudioSupport('audio_support'), interfaces.IAudioSupport)
        else:
            sm.registerUtility(interfaces.IAudioSupport, content.AudioSupport('audio_support'))
...
def setup_indexes(portal):
    ...
    if 'audio_genre_id' not in pc.indexes():
    ...
    if 'audio_artist' not in pc.indexes():
        extra = SimpleRecord(lexicon_id='plaintext_lexicon', index_type='Okapi BM25 Rank')
...
INDEX_MAPPING = {'audio_artist': {'name': 'Artist name', 'description': 'The name of the artist.', 'enabled': True, 'criteria': ('ATSimpleStringCriterion', )}, 'audio_genre_id': {'name': 'Genre', 'description': 'The genre id of the song.this is a number 0-147. See genre.py for the genre names.', 'enabled': True, 'criteria': ('ATSimpleIntCriterion', )}, 'Format': {'name': 'MIME Types', 'description': 'The MIME type of the file. For an MP3 file, this is audio/mpeg.', 'enabled': True, 'criteria': ('ATSimpleStringCriterion', )}}
...
def _cleanup_utilities(site):
    raise NotImplementedError('Current ISiteManager support does not include ability to clean up')
...
def unsetup_portal(portal):
    ...
    logger.warn('Removed IAudioEnhanced and IAudioContainerEnhanced interfaces from %i objects for cleanup' % count)