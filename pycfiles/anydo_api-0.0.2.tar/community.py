...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/peerdiscovery/community.py
# Compiled at: 2019-05-21 05:50:08
from __future__ import absolute_import
from binascii import unhexlify
from random import choice
from time import time
from .churn import DiscoveryStrategy, RandomChurn
from .payload import DiscoveryIntroductionRequestPayload, PingPayload, PongPayload, SimilarityRequestPayload, SimilarityResponsePayload
...
class DiscoveryCommunity(Community):
    version = '\x02'
    master_peer = Peer(unhexlify(''))
    ...
    def __init__(self, my_peer, endpoint, network, max_peers=DEFAULT_MAX_PEERS, anonymize=False):
        ...
        self.decode_map.update({chr(1): self.on_similarity_request, 
    ...
    def on_introduction_request(self, source_address, data):
        if self.max_peers >= 0 and len(self.get_peers()) > self.max_peers:
            self.logger.info('Dropping introduction request from (%s, %d): too many peers!', source_address[0], source_address[1])
            return
        else:
            try:
            ...
            packet = self.create_introduction_response(payload.destination_address, source_address, payload.identifier, introduction=introduction)
    ...
    def on_similarity_response(self, node, dist, payload):
        if self.max_peers >= 0 and len(self.get_peers()) > self.max_peers and node not in self.network.verified_peers:
            self.logger.info('Dropping similarity response from (%s, %d): too many peers!', node.address[0], node.address[1])
            return
    ...
    def get_my_overlays(self, peer):
        return [ service_id for service_id, overlay in self.network.service_overlays.items() if overlay.my_peer == peer
               ]
    ...
    def create_similarity_request(self, peer):
        ...
        payload = SimilarityRequestPayload(global_time, self.my_estimated_lan, self.my_estimated_wan, 'unknown', self.get_my_overlays(peer)).to_pack_list()
