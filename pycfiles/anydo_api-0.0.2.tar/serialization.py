...
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/messaging/serialization.py
...
class Bits(object):
    ...
    def pack(self, bit_7=0, bit_6=0, bit_5=0, bit_4=0, bit_3=0, bit_2=0, bit_1=0, bit_0=0):
        ...
        """
        byte = 0
        byte |= 128 if bit_7 else 0
        byte |= 64 if bit_6 else 0
        byte |= 32 if bit_5 else 0
        byte |= 16 if bit_4 else 0
        byte |= 8 if bit_3 else 0
        byte |= 4 if bit_2 else 0
        byte |= 2 if bit_1 else 0
        byte |= 1 if bit_0 else 0
        return (
         pack('>B', byte), 1)
    ...
    def unpack_from(self, data, offset):
        ...
        bit_7 = 1 if 128 & byte else 0
        bit_6 = 1 if 64 & byte else 0
        bit_5 = 1 if 32 & byte else 0
        bit_4 = 1 if 16 & byte else 0
        bit_3 = 1 if 8 & byte else 0
        bit_2 = 1 if 4 & byte else 0
        bit_1 = 1 if 2 & byte else 0
        bit_0 = 1 if 1 & byte else 0
        return (
         [
          bit_7, bit_6, bit_5, bit_4, bit_3, bit_2, bit_1, bit_0], 1)
...
class VarLen(object):
    ...
    def pack(self, *data):
        ...
        return (
         pack('>%s%ds' % (self.format, len(raw)), length, raw), size)
...
class DefaultStruct(Struct):
    ...
    def pack(self, *data):
        return (
         super(DefaultStruct, self).pack(*data), self.size)
...
class Serializer(object):
    ...
    def __init__(self):
        ...
        self._packers = {'?': DefaultStruct('>?', True), 
        ...
        for packable in pack_list:
            try:
                ...
                six.reraise(PackError, PackError('Could not pack item %d: %s\n%s: %s' % (index, repr(packable),
                 type(e).__name__, str(e))), sys.exc_info()[2])
    ...
    def ez_pack_serializables(self, serializables):
        ...
        :rtype: bytes or str
        """
        out, _ = self.pack_multiple(list(itertools.chain.from_iterable(serializable.to_pack_list() for serializable in serializables)))
        return out
    ...
    def unpack_to_serializables(self, serializables, data):
        ...
        for serializable in serializables:
            try:
                unpack_list, offset = self.unpack_multiple(serializable.format_list, data, serializable.optional_format_list, offset)
            except Exception as e:
