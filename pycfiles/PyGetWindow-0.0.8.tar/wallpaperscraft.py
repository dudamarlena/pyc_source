...
# Embedded file name: /media/user/D/projects/github/pygetwallpapers/pygetwallpapers/spiders/wallpaperscraft.py
...
__SIZES__ = [
 '240x320', '240x400', '320x240', '320x480', '360x640', '400x480', '480x800', '480x854', '540x960', '640x960', '640x1136', '720x1280', '750x1334', '800x600', '800x1280', '960x544', '1024x600', '1024x768', '1024x1024', '1080x1920', '1152x864', '1280x720', '1280x800', '1280x900',
...
__CATEGORYES__ = [
 'all', '3d', 'abstract', 'animals', 'anime', 'brands', 'cars', 'city', 'fantasy',
...
class WallpaperSpider(Spider):
    ...
    def __init__(self, ARGS=None):
        ...
        self.urls = [ '%s/page%d' % (self.url_template, page) for page in xrange(2, self.pages + 1)
                    ]
        self.urls.insert(0, self.url_template)
        self.outputBuffer = codecs.open(self.output, self.args['append'], encoding='utf-8')
    ...
    def task_generator(self):
        u"""Проходим в цикле по всем страницам и генерируем задачи для парсера Spider"""
        try:
    ...
    def get_url(self):
        u"""Вернет URL страницы из сгенерированного массива"""
        url = self.urls[self.current_page]
        self.current_page = self.current_page + 1
        return url

    def get_total_pages(self, url=None):
        u"""Вернет число страниц для построения цикла"""
        g = Grab()
        g.go(self.url_template)
        return int(g.doc.select(self.total_pages_XPath).text())

    def writeUrl(self, url=''):
        u"""Приводим ссылки в нужный формат для скачивания"""
        dl_domain = url.index(__DOMAIN__)
        result = re.search('^' + re.escape(url[:dl_domain]) + '(' + re.escape(__DOMAIN__) + ')\\/(image)\\/(.+)_([x0-9]+)\\.([a-z]+)', url)
    ...
    def task_search(self, grab, task):
        u"""Собираем ссылки на превью картинки со страницы"""
