...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gdcdictionary/schema_test.py
# Compiled at: 2019-11-08 14:26:37
...
import copy, yaml, glob, os, argparse, json, unittest
    ...
    for key in b:
        if key in a:
            if isinstance(a[key], dict) and isinstance(b[key], dict):
                ...
                print(("Overriding '{}':\n\t- {}\n\t+ {}").format(('.').join(path + [str(key)]), a[key], b[key]))
                a[key] = b[key]
        else:
...
def validate_entity(entity, schemata, project=None, name=''):
    ...
    """
    local_schema = get_project_specific_schema(projects, project, schemata[entity['type']], entity['type'])
...
def validate_schemata(schemata, metaschema):
    ...
    for s in schemata.values():
        ...
        def assert_link_is_also_prop(link):
            assert link in s['properties'], ("Entity '{}' has '{}' as a link but not property").format(s['id'], link)
...
class SchemaTest(unittest.TestCase):
    ...
    def add_system_props(self, doc):
        ...
        for key in schema['systemProperties']:
            use_def_default = '$ref' in schema['properties'][key] and key in self.definitions and 'default' in self.definitions[key]
            if use_def_default:
...
if __name__ == '__main__':
    ...
    parser.add_argument('jsonfiles', metavar='file', type=argparse.FileType('r'), nargs='*', help='json files to test if (in)valid')
    parser.add_argument('--invalid', action='store_true', default=False, help='expect the files to be invalid instead of valid')
