...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xooof/xmldispatcher/tools/unittest/TestCaseBase.py
...
class StructEqualFailureExceptionExpectedNotFound(StructEqualFailureException):
    ...
    def __str__(self):
        return 'expected struct file not found: %s (%s)' % (self.__expectedStructFileName, self.__exception)
...
class TestCaseBase(unittest.TestCase):
    ...
    def failUnlessUserException(self, callableObj, args=(), expectedCode=None, expectedText=None, expectedCodeIsRegExp=0, expectedTextIsRegExp=0):
        ...
        try:
            ...
            self.fail('Method succeeded, but it should have raised a user error')
        except XMLDispatcherUserException, e:
            if expectedCode is not None:
                if not _matchString(expectedCode, expectedCodeIsRegExp, e.getCode()):
                    self.fail("Method raised a user exception with an unexpected code:\nexpected '%s',\nreturned '%s'.\nDescr : '%s'\nSource: %s" % (expectedCode, e.getCode(), e.getDescr(), e.getSource()))
            if expectedText is not None:
                if not _matchString(expectedText, expectedTextIsRegExp, e.getDescr()):
                    self.fail("Method raised a user exception with an unexpected text:\nexpected '%s',\nreturned '%s'.\nCode  : '%s'\nSource: %s" % (expectedText, e.getDescr(), e.getCode(), e.getSource()))
    ...
    def failUnlessStructEqual(self, expectedStruct, lifeStruct, fieldsToIgnore=[], preprocessFunctions=[]):
        ...
        if type(expectedStruct) in types.StringTypes:
            try:
                ...
                raise StructEqualFailureExceptionExpectedNotFound(expectedStruct, lifeStructIndented, e)
        ...
        if expectedStructIndented != lifeStructIndented:
            raise StructEqualFailureExceptionMismatch(expectedStructIndented, lifeStructIndented)
        return