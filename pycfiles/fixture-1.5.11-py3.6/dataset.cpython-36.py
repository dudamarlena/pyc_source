...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/dataset/dataset.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 27180 bytes
...
class RefValue(object):
    __doc__ = 'A reference to a value in a row of a DataSet class.'
...
def is_row_class(attr):
    return isclass(attr) and attr.__name__ != 'Meta' and not issubclass(attr, DataContainer.Meta)
...
class DataType(type):
    ...
    def __init__(cls, name, bases, cls_attr):
        ...
        if 'Meta' in cls_attr:
            if hasattr(cls_attr['Meta'], 'primary_key'):
    ...
    def decorate_row(cls, row, name, bases, cls_attr):
        ...
        names_to_uninherit = []
        for name in dir(row):
            if name in cls_attr['_primary_key'] and name not in row.__dict__:
        ...
        for base_c, base_pos in bases_to_replace:
            new_base = class_factory(base_c.__name__, base_c.__bases__, dict([(k, getattr(base_c, k)) for k in dir(base_c) if not k.startswith('_') if k not in names_to_uninherit]))
...
class DataSetStore(list):
    __doc__ = 'keeps track of actual objects stored in a dataset.'
    ...
    def get_object(self, key):
        ...
        try:
            ...
            reraise(etype, etype("row '%s' hasn't been loaded for %s (loaded: %s)" % (
...
class DataSet(with_metaclass(DataType, DataContainer)):
    ...
    def __init__(self, default_refclass=None, default_meta=None):
        ...
        def mkref():
            ...
            return default_refclass(*[ds.shared_instance(default_refclass=default_refclass) for ds in iter(self.meta.references)])

        if len(self.meta.references) > 0:
            self.ref = mkref()
        for key, data in self.data():
            if key in self:
                raise ValueError("data() cannot redeclare key '%s' (this is already an attribute)" % key)
            ...
            if not is_row_class(val):
                ...
                for col_name in public_dir(row_class):
                    ...
                    if isinstance(col_val, Ref):
                        ...
                        if isinstance(col_val, (list, tuple, set)):
                            for c in col_val:
                                if is_rowlike(c):
                                    add_ref_from_rowlike(c)
                                else:
                                    if isinstance(c, (string_types, bool, float, int)):
                                        continue
                                    else:
                                        raise TypeError('multi-value columns can only contain rowlike objects, not %s of type %s' % (
                                         col_val, type(col_val)))

                        else:
                            if is_rowlike(col_val):
        ...
        if SuperSet._setdataset(self, dataset, key=key, isref=isref):
            for k, row in dataset:
                if k in self.meta.keys_to_datasets:
                    raise ValueError("cannot add key '%s' for %s because it was already added by %s" % (
