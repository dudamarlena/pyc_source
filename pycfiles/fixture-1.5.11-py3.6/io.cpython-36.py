...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/io.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 7189 bytes
...
class DirPath(str):
    ...
    def putfile(self, fname, contents, mode=None):
        ...
        if relpath.startswith(os.path.sep):
            raise TypeError("argument 1 must be a relative path, not '%s' (the path will be created relative to the tmp root, currently '%s')" % (
             relpath, self))
        if relpath:
            if not self.join(relpath).exists():
                self.mkdir(relpath)
