...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/test/test_base.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 9789 bytes
import sys, nose.case, nose.loader, nose.tools
...
class TestFixture:
    ...
    def test_with_data_aborts_teardown_on_interrupt(self):
        ...
        eq_(mock_call_log[0], (MockLoader, 'load', StubSuperSet))
        eq_(len(mock_call_log), 1, 'unexpected additional calls were made: %s' % mock_call_log)
    ...
    def test_with_data_does_soft_teardown_on_exception(self):
        ...
        assert 'ValueError: An exception during teardown' in saved_err, 'unexpected stderr capture: \n<<<<<<\n%s>>>>>>\n' % saved_err
        eq_(mock_call_log[0], (AbusiveMockLoader, 'load', StubSuperSet))
    ...
    def test_generated_error_raises_soft_exception_in_teardown(self):
        ...
        try:
            try:
                ...
                if not 'error raised from some_generator' in str(val):
                    raise AssertionError('Unexpected: %s' % val)
            else:
    ...
    def test_with_data_preserves_a_decorated_callable(self):
        ...
        @nose.tools.with_setup(setup=my_custom_setup,
          teardown=my_custom_teardown)
        @self.fxt.with_data(StubDataset1, StubDataset2)
        def some_callable(data):
            mock_call_log.append(('some_callable', data.__class__))
