...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/loadable/loadable.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 16961 bytes
...
"""
__all__ = [
 'LoadableFixture', 'EnvLoadableFixture', 'DBLoadableFixture', 'DeferredStoredObject']
import sys, types
...
class StorageMediumAdapter(object):
    __doc__ = 'common interface for working with storable objects.\n    '
    ...
    def clearall(self):
        ...
        for obj in self.dataset.meta._stored_objects:
            try:
                ...
                reraise(UnloadError, UnloadError(etype, val, (self.dataset), stored_object=obj))
...
class LoadableFixture(Fixture):
    ...
    def load_dataset(self, ds, level=1):
        ...
        treelog.info('%s%s%s (%s)', level * '  ', levsep, ds.__class__.__name__, is_parent and 'parent' or level)
            ...
            try:
                ...
                def column_vals():
                    for c in row.columns():
                        yield (
                         c, self.resolve_stored_object(getattr(row, c)))
    ...
    def unload(self):
        ...
        if self.loaded is None:
            raise UninitializedError('Cannot unload data because it has not yet been loaded in this process.  Call data.setup() before data.teardown()')
        ...
        if ds.meta.storage_medium is not None:
            ...
            if not storable:
                ...
                if not storable:
                    ...
                    raise self.StorageMediaNotFound("could not find %s '%s' for dataset %s in self.env (%s)" % (
                     self.Medium, ds.meta.storable_name, ds, repr_env))
            if storable == ds.__class__:
                raise ValueError('cannot use %s %s as a storable object of itself! (perhaps your style object was not configured right?)' % (
