...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/test/test_loadable/test_sqlalchemy_loadable.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 18698 bytes
import unittest
...
class TestCascadingReferences(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Session = get_transactional_session()
        self.session = Session()
        self.fixture = SQLAlchemyFixture(env=(globals()),
          engine=(metadata.bind),
          style=(NamedDataStyle()))
        clear_mappers()
        mapper(Category, categories)
        mapper(Product, products, properties={'category': relation(Category, backref='products')})
        mapper(Offer, offers, properties={'product':relation(Product, backref='offers'), 
...
class TestTableObjects(unittest.TestCase):
    ...
    def setUp(self):
        ...
        Session = get_transactional_session()
        self.session = Session()
        self.fixture = SQLAlchemyFixture(env={'CategoryData': categories},
          engine=(metadata.bind))
        clear_mappers()
        mapper(Category, categories)
...
class TestTableObjectsExplicitConn(object):
    ...
    def test_with_engine_connection(self):
        fixture = SQLAlchemyFixture(env={'CategoryData': categories},
...
def test_fixture_can_be_disposed():
    ...
    Session = get_transactional_session()
    session = Session()
    fixture = SQLAlchemyFixture(env={'CategoryData': Category},
      engine=(metadata.bind))
