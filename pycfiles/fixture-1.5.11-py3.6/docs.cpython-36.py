...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/docs.py
...
def get_object_from_path(rawpath):
    ...
    if len(fromlist):
        ...
        if obj:
            if obj.find('.') != -1:
                raise NotImplementedError('todo: need to split the object in a getattr loop')
            obj = getattr(mod, obj)
        else:
            obj = mod
    return obj
...
def shell(name, arguments, options, content, lineno, content_offset, block_text, state, state_machine):
    ...
    if options.get('setup'):
        ...
        if options.get('run_on_method'):
            ...
            cmdlist = []
            for part in re.split('\\s*', cmd.strip()):
                part = decode(part)
                part = unquot(part)
                e = part.find('=')
                if e != -1:
...
def setup_command_data():
    ...
    from fixture.examples.db.sqlalchemy_examples import Author, authors, Book, books, metadata
