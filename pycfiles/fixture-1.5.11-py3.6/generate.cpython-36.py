...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/command/generate/generate.py
...
"""
import inspect, optparse, os, pkg_resources, sys
...
class NoData(LookupError):
    __doc__ = 'no data was returned by a query'
...
class DataSetGenerator(object):
    __doc__ = 'produces a callable object that can generate DataSet code.\n    '
    template = None
        ...
        for h in handler_registry:
            ...
            if recognizes_obj:
                handler = h(object_path, self.options, obj=obj, 
                 template=self.template, **kw)
                break

        if handler is None:
            raise UnrecognizedObject('no handler recognizes object %s at %s (importable? %s); tried handlers %s' % (
    ...
    def code(self):
        ...
        code = [
         self.template.header(self.handler)]
...
class DataHandler(with_metaclass(HandlerType)):
    __doc__ = 'handles an object that can provide fixture data.\n    '
    ...
    def resolve_data_dict(self, datadef, fset):
        ...
        for k, v in fset.data_dict.items():
            if isinstance(v, FixtureSet):
                ...
                datadef.add_reference(fxt_class, fxt_var=(linked_fset.mk_var_name()))
...
def dataset_generator(argv):
    ...
    parser.add_option('--dsn', help='Sets db connection for a handler that uses a db')
    parser.add_option('-w', '--where', help='SQL where clause, i.e. "id = 1705" ')
    d = 'Data'
    parser.add_option('--suffix', help=('String suffix for all dataset class names (default: %s; i.e. an Employee object becomes EmployeeData)' % d),
      default=d)
    parser.add_option('--prefix', help='String prefix for all dataset class names (default: None)',
      default='')
    parser.add_option('--env', help='Module path to use as an environment for finding objects.  declaring multiple --env values will be recognized',
      action='append',
      default=[])
    parser.add_option('--require-egg', dest='required_eggs',
      help='A requirement string to enable importing from a module that was installed in multi-version mode by setuptools.  I.E. foo==1.0.  You can repeat this option as many times as necessary.',
      action='append',
      default=[])
    default_tpl = templates.default()
    parser.add_option('--template', help=('Template to use; choices: %s, default: %s' % (
     tuple([t for t in templates]), default_tpl)),
      default=default_tpl)
    parser.add_option('-c', '--connect', metavar='FUNCTION_PATH',
      action='append',
      default=[],
      help="Path to a function that performs a custom connection, accepting a single parameter, DSN.  I.E. 'some_module.submod:connect' will be called as connect(DSN).  Called *after* OBJECT_PATH is imported but *before* any queries are made. This option can be declared multiple times.")
    parser.add_option('-s', '--setup', metavar='FUNCTION_PATH',
      action='append',
      default=[],
      help="Path to a function that sets up data objects, accepting no parameters. I.E. 'some_module.submod:setup_all' will be called as setup_all().  Called *after* OBJECT_PATH is imported but *before* any queries are made and *before* connect(DSN) is called. This option can be declared multiple times.")
