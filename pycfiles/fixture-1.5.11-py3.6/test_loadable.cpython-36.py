...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fixture/test/test_loadable/test_loadable.py
# Compiled at: 2017-10-02 03:31:12
# Size of source mod 2**32: 14083 bytes
from fixture import DataSet, NamedDataStyle
from fixture import TempIO
from fixture.loadable import LoadableFixture, EnvLoadableFixture, DBLoadableFixture
...
class LoadableTest(object):
    __doc__ = 'tests the behavior of fixture.loadable.LoadableFixture object.\n    \n    to test combinations of loaders and datasets, mix this into a TestCase.\n    '
    fixture = None
    ...
    def test_DataTestCase(self):
        ...
        class SomeDataTestCase(DataTestCase):
...
class TestDBLoadableRowReferences(object):
    ...
    def test_row_column_refs_are_resolved(self):
        ...
        ldr = StubLoadableFixture(style=(NamedDataStyle()),
          medium=MockStorageMedium,
          env=(locals()))
        ldr.begin()
        ldr.load_dataset(PetData())
        eq_(calls[0], (Person, 'save'))
        eq_(calls[1], (Pet, 'save'))
        bob_db_obj = ldr.loaded[PersonData].meta._stored_objects.get_object('bob')
        eq_(bob_db_obj.name, PersonData.bob.name)
        fido_db_obj = ldr.loaded[PetData].meta._stored_objects.get_object('fido')
    ...
    def test_lists_of_row_refs_are_resolved(self):
        ...
        class PetData(DataSet):
            ...
            class fido:
                owners = [
                 PersonData.bob, PersonData.stacy]
        ...
        bob_db_obj = ldr.loaded[PersonData].meta._stored_objects.get_object('bob')
        stacy_db_obj = ldr.loaded[PersonData].meta._stored_objects.get_object('stacy')
        fido_db_obj = ldr.loaded[PetData].meta._stored_objects.get_object('fido')
    ...
    def test_DataSet_cannot_ref_self(self):
        ...
        bob_db_obj = ldr.loaded[PersonData].meta._stored_objects.get_object('bob')
        jenny_db_obj = ldr.loaded[PersonData].meta._stored_objects.get_object('jenny')
