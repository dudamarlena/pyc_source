...
# [GCC 8.4.0]
# Embedded file name: /Users/lidayan/pyenv/python3/pypi-distribute/lib/python3.6/site-packages/wxmgmt/admin.py
...
class TenantAdmin(admin.ModelAdmin):
    ...
    def get_msg_tpls(self, request, queryset):
        ...
        for tenant in queryset:
            ...
            if not tpls:
                self.message_user(request, ('同步公众号（%s）的消息模板失败' % tenant.name), level=(messages.ERROR))
            for item in tpls:
                MessageTemplate.sync_tpl(tenant, item)

        if status == 'success':
            self.message_user(request, '同步公众号（%s）的消息模板成功', level=(messages.INFO))
    ...
    def get_menu(self, request, queryset):
        ...
        for tenant in queryset:
            if not tenant.get_menu():
                status = 'failed'
                self.message_user(request, ('获取公众号（%s）的menu失败' % tenant.name), level=(messages.ERROR))

        if status == 'success':
            self.message_user(request, '获取公众号的menu成功', level=(messages.INFO))
    ...
    def delete_menu(self, request, queryset):
        ...
        for tenant in queryset:
            if not tenant.delete_menu():
                status = 'failed'
                self.message_user(request, ('删除公众号（%s）的menu失败' % tenant.name), level=(messages.ERROR))

        if status == 'success':
            self.message_user(request, '删除公众号的menu成功', level=(messages.INFO))
    ...
    def create_menu(self, request, queryset):
        ...
        for tenant in queryset:
            if not tenant.create_menu():
                status = 'failed'
                self.message_user(request, ('创建公众号（%s）的menu失败' % tenant.name), level=(messages.ERROR))

        if status == 'success':
            self.message_user(request, '创建公众号的menu成功', level=(messages.INFO))
        ...
        for tenant in queryset:
            for openid in tenant.get_openids():
                if not Account.sync_account(tenant, openid):
                    status = 'failed'
                    self.message_user(request, ('同步公众号（%s）的关注账号失败' % tenant.name), level=(messages.ERROR))

        if status == 'success':
            self.message_user(request, '同步公众号的关注账号成功', level=(messages.INFO))
