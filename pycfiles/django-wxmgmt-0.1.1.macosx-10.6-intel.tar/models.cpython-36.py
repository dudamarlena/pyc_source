...
# [GCC 8.4.0]
# Embedded file name: /Users/lidayan/pyenv/python3/pypi-distribute/lib/python3.6/site-packages/wxmgmt/models.py
# Compiled at: 2018-02-25 20:14:49
# Size of source mod 2**32: 15009 bytes
...
class Tenant(models.Model):
    chinese_name = models.CharField(max_length=50, help_text='中文名称，标示该商户')
    name = models.CharField(max_length=50, unique=True)
    enable = models.BooleanField(default=True)
    appid = models.CharField(max_length=200, unique=True)
    secret = models.CharField(max_length=200, unique=True, help_text='开发者密码')
    token = models.CharField(max_length=100, help_text='令牌', default='')
    encoding_aeskey = models.CharField(blank=True, max_length=200, help_text='消息加解密密钥', default='')
    ...
    shanghu_id = models.CharField(max_length=50, blank=True, default='', help_text='微信支付商户号')
    payment_api_key = models.CharField(max_length=200, blank=True, default='', help_text='微信支付商户的API密钥')
    payment_notify_url = models.CharField(max_length=200, blank=True, default='', help_text='支付回调接口')
    refund_notify_url = models.CharField(max_length=200, blank=True, default='', help_text='退款回调接口')
    ...
    managers = models.ManyToManyField('Account', blank=True, related_name='parents', help_text='设置管理员账号，用于接收通知类消息')
    ...
    def jsapi_ticket(self):
        if self._jsapi_ticket:
            if self.jsapi_ticket_expired_at > timezone.now():
    ...
    def get_openids(self):
        ...
        params = {'access_token':self.access_token, 
         'next_openid':''}
        while True:
            logger.debug('requests url(%s) params(%s)' % (url, params))
            resp = requests.get(url, params=params)
            if resp.status_code != 200:
                logger.error('get openids failed -- %s %s' % (resp.status_code, resp.content))
                return
            resp_json = resp.json()
            if 'errmsg' in resp_json:
                logger.error('get openids failed -- %s' % json.dumps(resp_json))
                return
    ...
    def delete_menu(self):
        if not self.access_token:
            ...
            if resp.status_code != 200:
                ...
                return
            resp_json = resp.json()
            if 'errcode' not in resp_json or resp_json.get('errcode') != 0:
                logger.error('delete menu failed -- %s %s' % (resp.status_code, resp.content))
                return
            self.menu = None
            self.save()
    ...
    def sync_account(cls, tenant, openid):
        ...
        if not userinfo:
            ...
            if tenant.account_set.filter(openid=openid):
                ...
                user = tenant.account_set.create(openid=(userinfo['openid']),
...
class MessageTemplate(models.Model):
    ...
    def sync_tpl(tenant, item):
        ...
        if tenant.messagetemplate_set.filter(template_id=template_id):
            ...
            tpl = tenant.messagetemplate_set.create(template_id=(item['template_id']),
    ...
    def send(self, data, account):
        if not self.access_token:
            ...
            params = {'access_token': self.access_token}
            req_data = {'touser':account.openid, 
