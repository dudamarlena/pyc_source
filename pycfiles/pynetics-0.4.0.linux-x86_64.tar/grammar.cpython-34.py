...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynetics/gggp/grammar.py
# Compiled at: 2016-03-12 06:09:25
# Size of source mod 2**32: 20732 bytes
import abc, collections, operator, random
from pynetics import take_chances

class Term(metaclass=abc.ABCMeta):
    __doc__ = ' Any of the elements in the right part of a production.\n\n    A term is an element that represents terminals, non-terminals, connectors,\n    multipliers or any combination of them.\n    '
    ...
    def __init__(self, value):
        ...
        :param value: The value for this term. Is expected that subclasses
            ...
            maintain its value.
        """
        self._Term__value = value

    @property
    def value(self):
        """ Returns the value of this term. """
        return self._Term__value
...
class Connector(Term, metaclass=abc.ABCMeta):
    ...
    def __init__(self, *args):
        ...
        :raises ValueError: If the value is not a tuple with strings, instances
            of Term or both.
        """
        super().__init__(value=self._Connector__process_args(args))
    ...
    def __process_args(args):
        ...
        if not isinstance(args, tuple):
            ...
            if error_msg:
                ...
                return tuple((SingleTerm(t) if isinstance(t, str) else t) for t in args)
...
class Or(Connector):
    ...
    def __init__(self, *args):
        ...
        :param value: A list of terms or a list of tuples in the form (t, w)
            ...
            others in the list.
        """
        value, weights = self._Or__process_value(args)
        super().__init__(*value)
        self._Or__weights = self._Or__validate_weights(weights)
    ...
    def random_derivation(self):
        ...
        terms_with_prob = sorted([(t, w / total_w) for t, w in zip(self.value, self.weights)], key=operator.itemgetter(1))
...
class Multiplier(Term, metaclass=abc.ABCMeta):
    __doc__ = ' A Multiplier instance that holds a term that may appear many times. '
    ...
    def __init__(self, value, *, lower=None, upper=None, p=0.5):
        ...
        """
        super().__init__(value=self._Multiplier__process_value(value))
        self._Multiplier__lower, self._Multiplier__upper = self._Multiplier__process_limits(lower, upper)
        self._Multiplier__p = self._Multiplier__check_probability(p)
    ...
    def __check_probability(p):
        ...
        if p is None or not isinstance(p, float) or not 0.0 <= p <= 1.0:
...
class SingleTerm(Term):
    ...
    def __init__(self, value):
        ...
        """
        super().__init__(value=self._SingleTerm__check_value(value))
...
class Production:
    ...
    def __init__(self, variable, term):
        ...
        """
        self._Production__variable = variable
        self._Production__term = self._Production__validate_term(term)
    ...
    def variable(self):
        ...
        return self._Production__variable

    @property
    def term(self):
        """ Returns the beta part of this production rule. """
        return self._Production__term
...
class Node:
    ...
    def __init__(self, value, parent, processed=False):
        self._Node__value = value
        self._Node__parent = parent
        self._Node__processed = processed

    @property
    def value(self):
        return self._Node__value

    @property
    def parent(self):
        return self._Node__parent

    @property
    def processed(self):
        return self._Node__processed
...
class Grammar:
    __doc__ = ' An object which represents a context free grammar. '
    ...
    def __init__(self, start_symbol=None, productions=None):
        ...
        """
        self._Grammar__productions = self._Grammar__process_productions(productions)
        self._Grammar__start_symbol = self._Grammar__process_start_symbol(start_symbol, productions)
        self._Grammar__variables = None
        self._Grammar__terminals = None
    ...
    def __process_productions(productions):
        ...
        for variable, terms in all_prods.items():
            new_productions.append(Production(variable=variable, term=Or(*terms) if len(terms) > 1 else terms[0]))
    ...
    def start_symbol(self):
        ...
        return self._Grammar__start_symbol

    @property
    def productions(self):
        """ Returns the production rules for this grammar. """
        return self._Grammar__productions

    @property
    def variables(self):
        """ Returns the variables for this grammar. """
        if self._Grammar__variables is None:
            self._Grammar__variables = set(p.variable for p in self.productions)
        return self._Grammar__variables

    @property
    def terminals(self):
        """ Returns the terminals for this grammar. """
        if self._Grammar__terminals is None:
            productions = {p.variable:p.term for p in self.productions}
            elements = [self.start_symbol]
            self._Grammar__terminals = set()
            while elements:
                ...
                if isinstance(element, (SingleTerm, Multiplier)):
                    ...
                    self._Grammar__terminals.add(element)

        return self._Grammar__terminals
    ...
    def random_tree(self, initial_symbol=None):
        ...
        """
        productions = {p.variable:p.term for p in self.productions}
        root = initial_symbol or self._Grammar__start_symbol
        ...
        while i < len(tree):
            ...
            if node.processed:
                ...
                if isinstance(node.value, Term):
                    elements = [Node(value=element, parent=node.parent) for element in node.value.random_derivation()]
                else:
                    if node.value in self.variables:
                        elements = [node,
                         Node(value=productions[node.value], parent=i)]
                    else:
                        elements = [
                         node]
                tree[i:i + 1] = elements
...
class DerivationTree(list):
    ...
    def __init__(self, grammar, *args, **kwargs):
        ...
        self._DerivationTree__grammar = grammar
        self._DerivationTree__word = None

    def word(self):
        if self._DerivationTree__word is None:
            word = [
             self[0]]
            i = 0
            while i < len(word):
                ...
                if node.value in self.grammar.terminals:
                    ...
                    children = [n for n in self if n.parent == self.index(node)]
                    word[i:i + 1] = children
                    i = 0

            self._DerivationTree__word = tuple(n.value for n in word)
        return self._DerivationTree__word

    @property
    def grammar(self):
        return self._DerivationTree__grammar