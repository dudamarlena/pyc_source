...
class PyneticsError(Exception):
    __doc__ = ' Base class for the errors raised in pynetics library. '


class InvalidSize(PyneticsError):
    __doc__ = ' Raised when an instance is not of the expected class. '
...
class WrongValueForInterval(ValueError):
    __doc__ = ' When a value does not belong to an interval. '
...
class NotAProbabilityError(WrongValueForInterval):
    __doc__ = ' If a value is not a valid probability. '
