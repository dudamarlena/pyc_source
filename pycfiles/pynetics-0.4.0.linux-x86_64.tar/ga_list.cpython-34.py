...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynetics/ga_list.py
# Compiled at: 2016-04-13 18:00:04
# Size of source mod 2**32: 12753 bytes
import random
from abc import ABCMeta, abstractmethod
from pynetics import SpawningPool, Individual, Recombination, take_chances, Mutation, Diversity

class Alleles(metaclass=ABCMeta):
    __doc__ = ' The alleles are all the possible values a gene can take. '
...
class FiniteSetAlleles(Alleles):
    __doc__ = ' The possible alleles belong to a finite set of symbols. '
...
class ListIndividualsWithFiniteSetAllelesDiversity(Diversity):
    __doc__ = ' Computes the diversity in a set of BinaryIndividual instances. '
    ...
    def __call__(self, individuals):
        ...
        total_diversity = len(individuals[0].population.spawning_pool.alleles.symbols) * len(individuals[0])
...
class ListIndividual(Individual, list):
    __doc__ = ' An individual whose representation is a list of finite values. '
    ...
    def __eq__(self, individual):
        ...
        """
        return len(self) == len(individual) and all([x == y for x, y in zip(self, individual)])
...
class ListRecombination(Recombination, metaclass=ABCMeta):
    __doc__ = ' Behavior for recombinations where lengths should be the same. '
...
class SingleGeneRandomValue(Mutation):
    __doc__ = ' Mutates the individual by changing the value to a random gene. '
...
class NGeneRandomValue(Mutation):
    ...
    def __call__(self, individual, p):
        ...
        :param p: The probability of mutation.
        """
        indexes = random.sample(range(len(individual)), min(self.n or len(individual), len(individual)))
        clone = individual.clone()
        for i in indexes:
            if take_chances(probability=p):
                new_gene = self.alleles.get()
