...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynetics/gggp/test_gggp_grammar.py
# Compiled at: 2016-03-12 06:09:25
# Size of source mod 2**32: 20950 bytes
from unittest import TestCase
from pynetics.gggp import grammar

class SingleTermTestCase(TestCase):
    __doc__ = ' Test for SingleTerm instances. '
    ...
    def test_different_instances_with_same_content_are_equal(self):
        ...
        self.assertEquals(grammar.SingleTerm('t'), grammar.SingleTerm('t'))
...
class AndTestCase(TestCase):
    __doc__ = ' Test for And connector instances. '
    ...
    def test_different_instances_with_same_content_are_equal(self):
        ...
        value = (
         ...
         'terminal2')
        self.assertEquals(grammar.And(*value), grammar.And(*value))
...
class OrTestCase(TestCase):
    __doc__ = ' Test for Or connector instances. '
    ...
    def test_different_instances_with_same_content_are_equal(self):
        ...
        value = (
         grammar.SingleTerm('terminal1'),
         'terminal2')
        self.assertEquals(grammar.Or(*value), grammar.Or(*value))
...
class MultiplierTestCase(TestCase):
    __doc__ = ' Test for Multiplier instances. '

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._MultiplierTestCase__default_lower = 0
        self._MultiplierTestCase__default_upper = float('Inf')
        self._MultiplierTestCase__default_p = 0.5
    ...
    def test_different_instances_with_same_content_are_equal(self):
        """ If two different instances has the same content are equal. """
        self.assertEquals(grammar.Multiplier('t'), grammar.Multiplier('t'))
    ...
    def test_correct_construction_with_string_without_anything(self):
        ...
        self.assertEquals(self._MultiplierTestCase__default_lower, multiplier.lower)
        self.assertEquals(self._MultiplierTestCase__default_upper, multiplier.upper)
        self.assertAlmostEqual(self._MultiplierTestCase__default_p, multiplier.p)
    ...
    def test_correct_construction_with_string_with_lower_upper_and_p(self):
        ...
        upper = 7
        p = 0.75
        multiplier = grammar.Multiplier(str_value, lower=lower, upper=upper, p=p)
        self.assertEquals(value, multiplier.value)
        self.assertEquals(lower, multiplier.lower)
        self.assertEquals(upper, multiplier.upper)
        self.assertAlmostEqual(p, multiplier.p)
...
class TestProduction(TestCase):
    __doc__ = ' Tests the production rules work properly. '
    ...
    def test_init_fails_if_term_is_not_a_term_instance(self):
        ...
        for invalid_term in (
         ...
         (
          grammar.EpsilonTerminal,), {'terminal': grammar.EpsilonTerminal},
         None,
         '7',
         42):
            with self.assertRaises(ValueError):
...
class GrammarTestCase(TestCase):
    __doc__ = ' Tests for Grammar instances. '
    ...
    def test_start_symbol_should_exists_as_variable_in_productions(self):
        ...
        productions = [grammar.Production(existent_start_symbol, grammar.EpsilonTerminal) for _ in range(4)]
        grammar.Grammar(start_symbol=existent_start_symbol, productions=productions)
        with self.assertRaises(ValueError):
            grammar.Grammar(start_symbol=non_existent_start_symbol, productions=productions)

    def test_productions_should_be_an_list_of_production_instances(self):
        """ Productions should be a list of Production instances. """
        all_but_one = [grammar.Production('t', grammar.EpsilonTerminal) for _ in range(4)]
    ...
    def test_correct_construction(self):
        ...
        productions = [grammar.Production(base_symbol + str(i), grammar.EpsilonTerminal) for i in range(4)]
        a_grammar = grammar.Grammar(start_symbol=start_symbol, productions=productions)
    ...
    def test_repeated_productions_are_shrunk(self):
        ...
        """
        start_symbol = 'expr'
        productions = [grammar.Production('expr', grammar.SingleTerm(str(i))) for i in range(10)]
