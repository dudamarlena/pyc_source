...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/coinmarketcap.py
...
class coinmarketcap(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(coinmarketcap, self).describe(), {'id': 'coinmarketcap', 
           'name': 'CoinMarketCap', 
           'rateLimit': 10000, 
           'version': 'v1', 
           'countries': 'US', 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/28244244-9be6312a-69ed-11e7-99c1-7c1797275265.jpg', 
                    'api': {'public': 'https://api.coinmarketcap.com', 
    ...
    def fetch_markets(self):
        markets = self.publicGetTicker({'limit': 0})
        result = []
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        request = self.extend({'convert': market['quote'], 
           'id': market['baseId']}, params)
        response = self.publicGetTickerId(request)
        ticker = response[0]
        return self.parse_ticker(ticker, market)

    def fetch_currencies(self, params={}):
        currencies = self.publicGetTicker(self.extend({'limit': 0}, params))
        result = {}
        for i in range(0, len(currencies)):
