...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/ccex.py
...
class ccex(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(ccex, self).describe(), {'id': 'ccex', 
           'name': 'C-CEX', 
           'countries': [
                       'DE', 'EU'], 
           'rateLimit': 1500, 
           ...
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/27766433-16881f90-5ed8-11e7-92f8-3d92cc747a6c.jpg', 
                    'api': {'web': 'https://c-cex.com/t', 
           ...
           'api': {'web': {'get': [
                   ...
                   'private': {'get': [
                                     ...
                                     'mytrades',
                                     'selllimit']}}, 
           'fees': {'trading': {'taker': 0.2 / 100, 
                                'maker': 0.2 / 100}}, 
           'commonCurrencies': {'IOT': 'IoTcoin', 
                                'BLC': 'Cryptobullcoin', 
            ...
            for j in range(0, len(marketIds)):
                ...
                if symbol in self.markets_by_id:
                    ...
                    if invertedId in self.markets_by_id:
                        ...
                        symbol = market['symbol']
                if symbol not in list(orderbooks.keys()):
                    orderbooks[symbol] = {}
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        response = self.webGetMarket(self.extend({'market': market['id'].lower()}, params))
    ...
    def fetch_trades(self, symbol, since=None, limit=None, params={}):
        self.load_markets()
        market = self.market(symbol)
        response = self.publicGetMarkethistory(self.extend({'market': market['id'], 
           'type': 'both', 
           'depth': 100}, params))
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        response = getattr(self, method)(self.extend({'market': self.market_id(symbol), 
           'quantity': amount, 
           'rate': price}, params))
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            query = self.keysort(self.extend({'a': path, 
               'apikey': self.apiKey, 
               'nonce': nonce}, params))
            url += '?' + self.urlencode(query)
            headers = {'apisign': self.hmac(self.encode(url), self.encode(self.secret), hashlib.sha512)}
        elif api == 'public':
            url += '?' + self.urlencode(self.extend({'a': 'get' + path}, params))
        else:
