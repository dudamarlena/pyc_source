...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/anyex/lykke.py
...
class lykke(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(lykke, self).describe(), {'id': 'lykke', 
           'name': 'Lykke', 
           'countries': 'CH', 
           'version': 'v1', 
           'rateLimit': 200, 
           ...
           'requiredCredentials': {'apiKey': True, 
                                   'secret': False}, 
           'urls': {'logo': 'https://user-images.githubusercontent.com/1294454/34487620-3139a7b0-efe6-11e7-90f5-e520cef74451.jpg', 
                    'api': {'mobile': 'https://api.lykkex.com/api', 
                            'public': 'https://hft-api.lykke.com/api', 
                            'private': 'https://hft-api.lykke.com/api', 
                            'test': {'mobile': 'https://api.lykkex.com/api', 
    ...
    def fetch_markets(self):
        ...
        for i in range(0, len(markets)):
            ...
            symbol = market['Name']
            precision = {'amount': market['Accuracy'], 
    ...
    def fetch_ticker(self, symbol, params={}):
        self.load_markets()
        market = self.market(symbol)
        ticker = self.mobileGetAllAssetPairRatesMarket(self.extend({'market': market['id']}, params))
        return self.parse_ticker(ticker, market)

    def parse_order_status(self, status):
        if status == 'Pending':
            return 'open'
        if status == 'InOrderBook':
            return 'open'
        ...
        if status == 'Cancelled':
            return 'canceled'
        if status == 'NotEnoughFunds':
            return 'NotEnoughFunds'
        if status == 'NoLiquidity':
            return 'NoLiquidity'
        if status == 'UnknownAsset':
            return 'UnknownAsset'
        if status == 'LeadToNegativeSpread':
    ...
    def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        response = self.privateGetOrders(self.extend({'status': 'InOrderBook'}, params))
        return self.parse_orders(response, None, since, limit)

    def fetch_closed_orders(self, symbol=None, since=None, limit=None, params={}):
        self.load_markets()
        response = self.privateGetOrders(self.extend({'status': 'Matched'}, params))
        return self.parse_orders(response, None, since, limit)

    def fetch_order_book(self, symbol, limit=None, params={}):
        self.load_markets()
        response = self.publicGetOrderBooksAssetPairId(self.extend({'AssetPairId': self.market_id(symbol)}, params))
