...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ulif/rest/tests/DocutilsTestSupport.py
...
import sys, os, unittest, re, inspect, traceback
...
try:
    import docutils_difflib, package_unittest, docutils, docutils.core
...
class CustomTestCase(StandardTestCase):
    ...
    def __init__(self, method_name, input, expected, id, run_in_debugger=0, suite_settings=None):
        """
...
class CustomTestSuite(unittest.TestSuite):
    ...
    def __init__(self, tests=(), id=None, suite_settings=None):
        ...
        if id is None:
            mypath = os.path.abspath(sys.modules[CustomTestSuite.__module__].__file__)
            ...
            if callerpath.startswith(mydir):
                self.id = callerpath[len(mydir) + 1:]
            else:
                self.id = callerpath
        else:
            self.id = id
    ...
    def addTestCase(self, test_case_class, method_name, input, expected, id=None, run_in_debugger=0, **kwargs):
        """
        ...
        tc = test_case_class(method_name, input, expected, tcid, run_in_debugger=run_in_debugger, **kwargs)
        self.addTest(tc)
        return tc
...
class TransformTestSuite(CustomTestSuite):
    ...
    def generateTests(self, dict, dictname='totest', testmethod='test_transforms'):
        """
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[2]:
                        ...
                        continue
                self.addTestCase(TransformTestCase, testmethod, transforms=transforms, parser=self.parser, input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[2]:
                        ...
                        continue
                self.addTestCase(self.test_case_class, 'test_parser', input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
...
class GridTableParserTestCase(CustomTestCase):
    ...
    def test_parse(self):
        try:
            output = self.parser.parse(StringList(string2lines(self.input), 'test data'))
        except Exception, details:
            output = '%s: %s' % (details.__class__.__name__, details)
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 4:
                    if case[(-1)]:
                        ...
                        continue
                self.addTestCase(self.test_case_class, 'test_parse_table', input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
                self.addTestCase(self.test_case_class, 'test_parse', input=case[0], expected=case[2], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[(-1)]:
                        ...
                        continue
                self.addTestCase(self.test_case_class, 'test_parse', input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
...
class PythonModuleParserTestSuite(CustomTestSuite):
    ...
    def generateTests(self, dict, dictname='totest', testmethod='test_parser'):
        """
        Stock the suite with test cases generated from a test data dictionary.
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[2]:
                        ...
                        continue
                self.addTestCase(PythonModuleParserTestCase, testmethod, input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger)
...
class WriterPublishTestCase(CustomTestCase, docutils.SettingsSpec):
    ...
    def test_publish(self):
        if self.run_in_debugger:
            pdb.set_trace()
        output = docutils.core.publish_string(source=self.input, reader_name='standalone', parser_name='restructuredtext', writer_name=self.writer_name, settings_spec=self, settings_overrides=self.suite_settings)
        self.compare_output(self.input, output, self.expected)
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[2]:
                        ...
                        continue
                self.addTestCase(self.test_class, 'test_publish', input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger, writer_name=self.writer_name)
            ...
            for casenum in range(len(cases)):
                ...
                if len(case) == 3:
                    if case[2]:
                        ...
                        continue
                self.addTestCase(HtmlWriterPublishPartsTestCase, 'test_publish', input=case[0], expected=case[1], id='%s[%r][%s]' % (dictname, name, casenum), run_in_debugger=run_in_debugger, suite_settings=settings)
...
class HtmlWriterPublishPartsTestCase(WriterPublishTestCase):
    ...
    __module__ = __name__
    writer_name = 'html'
    settings_default_overrides = WriterPublishTestCase.settings_default_overrides.copy()
    settings_default_overrides['stylesheet'] = ''

    def test_publish(self):
        if self.run_in_debugger:
            pdb.set_trace()
        parts = docutils.core.publish_parts(source=self.input, reader_name='standalone', parser_name='restructuredtext', writer_name=self.writer_name, settings_spec=self, settings_overrides=self.suite_settings)
    ...
    standard_content_type_template = '<meta http-equiv="Content-Type" content="text/html; charset=%s" />\n'
    standard_generator_template = '<meta name="generator" content="Docutils %s: http://docutils.sourceforge.net/" />\n'
    standard_html_meta_value = standard_content_type_template + standard_generator_template % docutils.__version__
    standard_meta_value = standard_html_meta_value % 'utf-8'
    standard_html_prolog = '<?xml version="1.0" encoding="%s" ?>\n<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">\n'
    ...
    def format_output(self, parts):
        ...
        parts['html_head'] = parts['html_head'].replace(self.standard_html_meta_value, '...')
        parts['html_prolog'] = parts['html_prolog'].replace(self.standard_html_prolog, '')
        ...
        output = []
        for key in keys:
            output.append("%r: '''%s'''" % (key, parts[key].encode('raw_unicode_escape')))
...
def exception_data(code):
    ...
    try:
        ...
        return (
         detail, detail.args, '%s: %s' % (detail.__class__.__name__, detail))
        ...
        if isinstance(i, StringType) or isinstance(i, UnicodeType):
            if '\n' in i:
                ...
                if isinstance(i, UnicodeType):
                    ...
                    r = r[1:]
                quote_char = r[0]
                raise quote_char in ("'", '"') or AssertionError
            assert r[0] == r[(-1)]
            r = r[1:-1]
