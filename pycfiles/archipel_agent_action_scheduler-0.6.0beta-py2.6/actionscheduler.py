...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentactionscheduler/actionscheduler.py
# Compiled at: 2013-03-20 13:50:16
import sqlite3, uuid, xmpp
...
class TNActionScheduler(TNArchipelPlugin):
    ...
    def __init__(self, configuration, entity, entry_point_group):
        ...
        self.restore_jobs()
        self.supported_actions_for_vm = ('create', 'shutdown', 'destroy', 'suspend',
                                         'resume', 'reboot', 'migrate', 'pause')
    ...
    def iq_schedule(self, iq):
        ...
        try:
            ...
            entityClass = self.entity.__class__.__name__
            param = None
            if entityClass == 'TNArchipelVirtualMachine' and job not in self.supported_actions_for_vm:
                raise Exception('action %s is not valid' % job)
            elif entityClass == 'TNArchipelHypervisor' and job not in self.supported_actions_for_hypervisor:
                raise Exception('action %s is not valid' % job)
