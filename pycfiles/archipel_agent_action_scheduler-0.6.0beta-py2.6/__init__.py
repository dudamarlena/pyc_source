...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagentactionscheduler/__init__.py
...
def make_archipel_plugin(configuration, entity, group):
    ...
    """
    return [
     {'info': actionscheduler.TNActionScheduler.plugin_info(), 'plugin': actionscheduler.TNActionScheduler(configuration, entity, group)}]
...
def version():
    ...
    return (
     __name__, pkg_resources.get_distribution('archipel-agent-action-scheduler').version, [actionscheduler.TNActionScheduler.plugin_info()])