...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/colormeshop/models/sale.py
# Compiled at: 2018-07-29 03:49:51
# Size of source mod 2**32: 42982 bytes
r"""
    カラーミーショップ API
...
class Sale(object):
    ...
    def accepted_mail_state(self, accepted_mail_state):
        ...
        if accepted_mail_state not in allowed_values:
            raise ValueError('Invalid value for `accepted_mail_state` ({0}), must be one of {1}'.format(accepted_mail_state, allowed_values))
    ...
    def paid_mail_state(self, paid_mail_state):
        ...
        if paid_mail_state not in allowed_values:
            raise ValueError('Invalid value for `paid_mail_state` ({0}), must be one of {1}'.format(paid_mail_state, allowed_values))
    ...
    def delivered_mail_state(self, delivered_mail_state):
        ...
        if delivered_mail_state not in allowed_values:
            raise ValueError('Invalid value for `delivered_mail_state` ({0}), must be one of {1}'.format(delivered_mail_state, allowed_values))
    ...
    def point_state(self, point_state):
        ...
        """
        allowed_values = [
         'assumed', 'fixed', 'canceled']
        if point_state not in allowed_values:
            raise ValueError('Invalid value for `point_state` ({0}), must be one of {1}'.format(point_state, allowed_values))
    ...
    def gmo_point_state(self, gmo_point_state):
        ...
        if gmo_point_state not in allowed_values:
            raise ValueError('Invalid value for `gmo_point_state` ({0}), must be one of {1}'.format(gmo_point_state, allowed_values))
    ...
    def yahoo_point_state(self, yahoo_point_state):
        ...
        if yahoo_point_state not in allowed_values:
            raise ValueError('Invalid value for `yahoo_point_state` ({0}), must be one of {1}'.format(yahoo_point_state, allowed_values))
