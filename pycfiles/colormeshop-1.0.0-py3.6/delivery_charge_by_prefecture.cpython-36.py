...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/colormeshop/models/delivery_charge_by_prefecture.py
...
class DeliveryChargeByPrefecture(object):
    ...
    openapi_types = {'pref_id':'int', 
     'pref_name':'str', 
     'charge':'int'}
    attribute_map = {'pref_id':'pref_id', 
     'pref_name':'pref_name', 
     'charge':'charge'}

    def __init__(self, pref_id=None, pref_name=None, charge=None):
        """DeliveryChargeByPrefecture - a model defined in OpenAPI"""
        self._pref_id = None
    ...
    def pref_id(self, pref_id):
        ...
        if pref_id is not None:
            if pref_id > 48:
                raise ValueError('Invalid value for `pref_id`, must be a value less than or equal to `48`')
        if pref_id is not None:
            if pref_id < 1:
                raise ValueError('Invalid value for `pref_id`, must be a value greater than or equal to `1`')
