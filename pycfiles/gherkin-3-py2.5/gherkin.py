...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/gherkin.py
# Compiled at: 2007-06-14 22:09:45
"""
...
class Gherkin(object):
    ...
    def __init__(self):
        ...
        self.decoder = {self.protocol[TupleType]: self.dec_tuple_type, 
           ...
           self.protocol[ComplexType]: self.dec_complex_type}
    ...
    def enc_complex_type(self, obj):
        return '%s%s' % (self.protocol[ComplexType], pack('!dd', obj.imag, obj.real))
    ...
    def dec_set_type(self, data):
        return set(self.build_sequence(data))

    def dec_tuple_type(self, data):
        return tuple(self.build_sequence(data))

    def dec_list_type(self, data):
        return list(self.build_sequence(data))

    def dec_dict_type(self, data):
        return dict(self.build_sequence(data))
    ...
    def dec_complex_type(self, data):
        (imag, real) = unpack('!dd', data.read(self.float_size * 2))
    ...
    def loads(self, data):
        ...
        if header != self.header:
            raise ValueError('Not valid gherkin data.')
        if self.version != int(buffer.read(1), 10):
            raise ValueError('Gherkin versions are incompatible.')
        self.int_size = int(buffer.read(1), 10)
        self.float_size = int(buffer.read(1), 10)
        try:
            value = self.decoder[buffer.read(1)](buffer)
        except KeyError, e:
            raise ValueError, 'Type prefix not supported. Bad Data? (%s)' % e
