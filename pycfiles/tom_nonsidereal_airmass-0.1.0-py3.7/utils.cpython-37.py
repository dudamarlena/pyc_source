...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tom_nonsidereal_airmass/utils.py
# Compiled at: 2019-09-20 20:00:31
...
def get_rise_set(observer, target, start_time, end_time):
    ...
    while start_time < end_time:
        ...
        next_setting = ephem_to_datetime(observer.next_setting(target))
        if next_rising > start_time:
            if next_rising < end_time:
    ...
    for observing_facility in facility.get_service_classes():
        ...
        for site, site_details in sites.items():
            ...
            while curr_interval <= end_time:
                ...
                positions[1].append((sunup or airmass).value if (airmass.value > 1 and airmass.value <= airmass_limit) else None)
