...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/SyncCommand.py
...
class SyncCommand(PluginCommand, CloudPluginCommand):
    topics = {'sync': 'system', 'rsync': 'system'}
        ...
        if cloudname is None:
            ...
            if arguments['put']:
                if localdir is None:
                    ...
                    return ''
                result = Sync.sync(cloudname=cloudname, localdir=localdir, remotedir=remotedir, operation='put')
                ...
                if localdir is None:
                    Console.error('Please provide the [LOCALDIR] argument.')
                    return ''
                result = Sync.sync(cloudname=cloudname, localdir=localdir, remotedir=remotedir, operation='get')
                if result is not None:
                    Console.ok('Successuly synced local and remote directories.')
            return ''