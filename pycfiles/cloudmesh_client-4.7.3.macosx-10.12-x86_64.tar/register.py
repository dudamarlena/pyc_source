...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/register.py
...
class CloudRegister(object):
    ...
    def read_rc_file(cls, host, openrc, force=False):
        ...
        if host not in config['cloudmesh.clouds'] or force:
            config['cloudmesh']['clouds'][host] = {'credentials': credentials, 'cm_heading': 'TBD', 
               'cm_host': 'TBD', 
               'cm_label': 'TBD', 
               'cm_type': 'TBD', 
               'cm_type_version': 'TBD'}
    ...
    def ec2(cls, cloud, zipfile):
        ...
        dest = sanitize(os.path.join(path_expand('~'), '.cloudmesh', 'clouds', cloud, os.path.basename(zipfile)))
        ...
        data['DEST'] = dest
        data['CLOUD'] = cloud
        d = {'cm_heading': ('{CLOUD}, EC2').format(**data), 
           'cm_host': None, 
           'cm_label': ('{CLOUD}_ec2').format(**data), 
           'cm_type': 'ec2', 
           'cm_type_version': 'ec2', 
           'credentials': {'EC2_ACCESS_KEY': ('{EC2_ACCESS_KEY}').format(**data), 
    ...
    def from_file(cls, filename):
        ...
        config.save()
        Console.ok(('Overwritten the TBD of cloudmesh.yaml with {} contents').format(filename))
    ...
    def from_environ(cls, provider):
        ...
        if env_config_data['OS_AUTH_URL'] is None:
            ...
            cloudname_to_use = input(('Name of the cloud (Default: {:}): ').format(cloudname_suggest)) or cloudname_suggest
            cm_label = input(('Label for the cloud (Default: {:}): ').format(cloudname_to_use)) or ('{:}').format(cloudname_to_use)
            cm_heading = input(('Heading for the cloud (Default: {:} Cloud): ').format(cm_label)) or ('{:} Cloud').format(cm_label)
            cm_host = input(('Cloud host name (Default: {:}): ').format(cloudname_suggest)) or ('{:}').format(cloudname_suggest)
            if provider is None:
                cm_type = input('Type of the cloud- openstack/azure/ec2 (Default: openstack): ') or 'openstack'
            else:
                cm_type = provider
            ...
            cm_type_version = input(('Version of type {:} (Default: null): ').format(cm_type)) or None
            yaml_data['cloudmesh']['clouds'][cloudname_to_use] = {'cm_heading': cm_heading, 'cm_host': cm_host, 
               'cm_label': cm_label, 
               'cm_type': cm_type, 
               'cm_type_version': cm_type_version, 
               'credentials': env_config_data}
            default_flavor = input('Default flavor for the cloud instances(Default: null): ') or None
            default_image = input('Default image for the cloud instances (Default: null): ') or None
            default_location = input('Default location for the cloud instances (Default: null): ') or None
            yaml_data['cloudmesh']['clouds'][cloudname_to_use]['default'] = {'flavor': default_flavor, 'image': default_image, 
