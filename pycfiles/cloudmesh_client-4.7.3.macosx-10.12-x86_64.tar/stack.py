...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/stack.py
# Compiled at: 2017-05-15 14:43:21
from __future__ import print_function
import copy, os, stat, shutil, subprocess, sys, time
...
class BigDataStack(object):
    ...
    def deploy(self, ips=None, name=None, user=None, playbooks=None, defines=None, ping_max=10, ping_sleep=10):
        ...
        if not ips is not None:
            ...
            for i in xrange(ping_max):
                ...
                try:
                    Subprocess(['ansible', 'all', '-m', 'ping', '-u', user], cwd=self.path, env=self._env, stdout=None, stderr=None)
                    ping_ok = True
...
class SanityChecker(object):
    ...
    def check_program(self, program):
        ...
        raise SanityCheckError(('{} not installed correctly').format(program), ('`{}` not found in $PATH').format(program))
    ...
    def check_github(self):
        ...
        try:
            ...
            if "You've successfully authenticated" not in e.stderr:
                raise SanityCheckError('SSH authentication to github.com failed', 'did you add your public key to https://github.com/settings/ssh ?')
