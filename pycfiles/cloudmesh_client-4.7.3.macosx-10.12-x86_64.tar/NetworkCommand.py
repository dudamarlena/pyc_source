...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/NetworkCommand.py
...
class NetworkCommand(PluginCommand, CloudPluginCommand):
    ...
    def do_network(self, args, arguments):
        ...
        if cloudname is None:
            Console.error('Default cloud has not been set!Please use the following to set it:\ncm default cloud=CLOUDNAME\nor provide it via the --cloud=CLOUDNAME argument.')
            return ''
        else:
            if arguments['get'] and arguments['fixed']:
                fixed_ip = arguments['FIXED_IP']
                result = Network.get_fixed_ip(cloudname, fixed_ip_addr=fixed_ip)
                Console.msg(result)
            elif arguments['get'] and arguments['floating']:
                floating_ip_id = arguments['FLOATING_IP_ID']
                result = Network.get_floating_ip(cloudname, floating_ip_or_id=floating_ip_id)
                Console.msg(result)
            elif arguments['reserve'] and arguments['fixed']:
                fixed_ip = arguments['FIXED_IP']
                result = Network.reserve_fixed_ip(cloudname=cloudname, fixed_ip_addr=fixed_ip)
                if result is not None:
                    Console.ok(('Reserve fixed ip address {} complete.').format(fixed_ip))
            elif arguments['unreserve'] and arguments['fixed']:
                fixed_ip = arguments['FIXED_IP']
                result = Network.unreserve_fixed_ip(cloudname=cloudname, fixed_ip_addr=fixed_ip)
                if result is not None:
                    Console.ok(('Un-Reserve fixed ip address {} complete.').format(fixed_ip))
            elif arguments['associate'] and arguments['floating']:
                group_name = arguments['--group']
                instance_id = arguments['--instance']
                floating_ip = arguments['FLOATING_IP']
                if group_name is not None:
                    group = Group.get_info(name=group_name, category=cloudname, output='json')
                    if group is not None:
                        group = json.loads(group)
                        for item in group:
                            instance_id = group[item]['value']
                            floating_ip = Network.find_assign_floating_ip(cloudname=cloudname, instance_id=instance_id)
                            if floating_ip is not None:
                                Console.ok(('Created and assigned Floating IP {} to instance {}.').format(floating_ip, instance_id))
                                self.refresh_vm(cloudname)

                    else:
                        Console.error(('No group {} in the Cloudmesh database.').format(group_name))
                        return ''
                elif not floating_ip and instance_id is not None:
                    floating_ip = Network.find_assign_floating_ip(cloudname=cloudname, instance_id=instance_id)
                    if floating_ip is not None:
                        Console.ok(('Associated floating IP {} to instance {}.').format(floating_ip, instance_id))
                elif instance_id is not None:
                    Network.find_assign_floating_ip(cloudname=cloudname, instance_id=instance_id, floating_ip=floating_ip[0])
                else:
                    ...
                    return ''
                self.refresh_vm(cloudname)
            elif arguments['disassociate'] and arguments['floating']:
                group_name = arguments['--group']
                instance_id = arguments['--instance']
                floating_ip = arguments['FLOATING_IP']
                if group_name is not None:
                    ...
                    if group is not None:
                        ...
                        for item in group:
                            instance_id = group[item]['value']
                            instance_dict = Network.get_instance_dict(cloudname=cloudname, instance_id=instance_id)
                            ...
                            if floating_ip is None:
                                Console.error(('Instance{} does not have a floating_ip.').format(instance_name))
                                return ''
                            result = Network.disassociate_floating_ip(cloudname=cloudname, instance_name=instance_name, floating_ip=floating_ip)
                            if result is not None:
                                Console.ok(('Disassociated Floating IP {} from instance {}.').format(floating_ip, instance_name))
                    ...
                    if _floating_ip != floating_ip:
                        Console.error(('Invalid floating_ip {} for instance {}.').format(floating_ip, instance_name))
                        return ''
                    result = Network.disassociate_floating_ip(cloudname=cloudname, instance_name=instance_name, floating_ip=floating_ip)
                    if result is not None:
                        Console.ok(('Disassociated Floating IP {} from instance {}.').format(floating_ip, instance_name))
                    ...
                    return ''
                self.refresh_vm(cloudname)
            elif arguments['create'] and arguments['floating']:
                floating_pool = arguments['--pool']
                result = Network.create_floating_ip(cloudname=cloudname, floating_pool=floating_pool)
                if result is not None:
                    Console.ok(('Created new floating IP {}').format(result))
                else:
                    Console.error('Failed to create floating IP! Please check arguments.')
            elif arguments['delete'] and arguments['floating']:
                if arguments['--unused']:
                    ...
                    if unused_floating_ips:
                        for floating_ip in unused_floating_ips:
                            self._delete_floating_ip(cloudname=cloudname, floating_ip=floating_ip['id'])
                ...
                for floating_ip in floating_ips:
                    self._delete_floating_ip(cloudname=cloudname, floating_ip=floating_ip)

            elif arguments['list'] and arguments['floating'] and arguments['pool']:
                result = Network.list_floating_ip_pool(cloudname)
                Console.msg(result)
            elif arguments['list'] and arguments['floating']:
                ...
                if instance_id is not None:
                    ...
                    if ip_or_id is None:
                        Console.error(('Instance with ID {} does not have a floating IP address!').format(instance_id))
                        return ''
                if ip_or_id is not None:
                    result = Network.get_floating_ip(cloudname, floating_ip_or_id=ip_or_id)
                    if result is not None:
                        Console.msg(result)
                    else:
                ...
                group_name = arguments['--group'] or Default.get(name='group', category=cloudname)
                group = Group.get_info(name=group_name, category=cloudname, output='json')
                if group is not None:
                    group = json.loads(group)
                    public_keys = ''
                    ...
                    for item in group:
                        ...
                        floating_ip = instance_dict['floating_ip']
                        if floating_ip is None:
                            floating_ip = Network.create_assign_floating_ip(cloudname=cloudname, instance_name=instance_name)
                            if floating_ip is not None:
                                Console.ok(('Created and assigned Floating IP {} to instance {}.').format(floating_ip, instance_name))
                                self.refresh_vm(cloudname)
    ...
    def _delete_floating_ip(cls, cloudname, floating_ip):
        result = Network.delete_floating_ip(cloudname=cloudname, floating_ip_or_id=floating_ip)
        if result is not None:
            Console.ok(result)
        else:
    ...
    def _assign_floating_ip(cls, cloudname, instance_id, floating_ip):
        ...
        if instance_dict is None:
            ...
            instance_name = instance_dict['name']
            result = Network.associate_floating_ip(cloudname=cloudname, instance_name=instance_name, floating_ip=floating_ip)
            if result is not None:
                Console.ok(('Associated Floating IP {} to instance {}.').format(floating_ip, instance_name))
            return