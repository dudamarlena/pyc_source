...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/StackCommand.py
...
class Command(object):
    ...
    def init(self, stackname='bds', activate=True, name=None, username=None, branch=None, overrides=None, playbooks=None, ips=None, force=False, update=False):
        factory = ProjectFactory()
        if stackname == 'bds':
            factory.use_bds()
        else:
            raise NotImplementedError(stackname)
        factory.set_project_name(name).set_user_name(os.getenv('USER') if username is '$USER' else username).set_branch(branch).set_ips(ips).set_overrides(overrides).set_playbooks(playbooks).activate(activate).set_force(force=force).set_update(update)
        project = factory()
...
class StackCommand(PluginCommand, CloudPluginCommand):
    ...
    def do_stack(self, args, arguments):
        ...
        if a.init:
            ...
            cmd.init(stackname=a['--stack'], name=a['--name'], branch=a['--branch'], username=a['--username'], activate=not a['--no-activate'], overrides=defns, playbooks=plays, ips=a['<ip>'], force=a['--force'], update=a['--update'])
        if a.deploy:
            cmd.deploy(project_name=a['--name'], force=a['--force'])
        if a.project:
            cmd.project(list_projects=a['--list'], name=a['<name>'])
        return ''