...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/RegisterCommand.py
# Compiled at: 2017-04-23 10:30:41
from __future__ import print_function
import getpass, json, os, os.path
    ...
    def do_register(self, args, arguments):
        ...
        if arguments['info']:
            ...
            if os.path.isfile(filename):
                ...
                d = CloudRegister.list(filename, Default.cloud, info=False, output='table')
                print(d)
            else:
        ...
        if arguments['backup']:
            ...
            return ''
        if arguments['new']:
            import shutil, cloudmesh_client.etc
            config = ConfigDict('cloudmesh.yaml')
            data = dotdict({'dir': cloudmesh_client.etc.__file__, 
               'filename': os.path.join(os.path.dirname(cloudmesh_client.etc.__file__), 'cloudmesh.yaml'), 
        ...
        if arguments['clean']:
            ...
            if filename is not None:
                ...
                if exists(filename):
                    ...
                    if not force:
                        force = yn_choice('Would you like to delete the cloudmesh.yaml file')
                        print(force)
                    if force:
                        os.remove(filename)
        ...
        if arguments['edit']:
            ...
            if exists(filename):
                try:
                    data = {'editor': os.environ['EDITOR'], 'filename': filename}
            ...
            return ''
        if arguments['list'] and arguments['ssh']:
            ...
            if arguments['list']:
                ...
                if not filename:
                    ...
                    d = CloudRegister.list(filename, Default.cloud, info=info, output=output)
                    print(d)
                return ''
            if arguments['check']:
                filename = _get_config_yaml_file(arguments)
            ...
            if arguments['ec2']:
                ...
                if cloud is None:
                    clouds = [
                     ConfigDict(filename='cloudmesh.yaml')['cloudmesh']['active'][0]]
                else:
                    clouds = [
                     cloud]
                for cloud in clouds:
            ...
            if arguments['user']:
                ...
                return ''
            if arguments['ENTRY'].lower() in ('chameleon', ):
                config = ConfigDict('cloudmesh.yaml')
                credentials = dotdict(config['cloudmesh.clouds.chameleon.credentials'])
                default = credentials.OS_USERNAME
                username = input(('Please enter the username for {:} [{}]: ').format('chameleon', default))
                username = username or default
                while True:
                    default = credentials.OS_PROJECT_NAME
                    project = input(('Please enter the project id for {:} [{}]: ').format('chameleon', default))
