...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/default.py
...
class Default(object):
    ...
    def set(cls, key, value, category='general', user=None, type='str'):
        ...
        try:
            ...
            if o is not None:
                cls.cm.update(kind=cls.__kind__, provider=cls.__provider__, filter={'name': key}, update={'value': value, 'type': type, 
                   'user': user, 
                   'category': category})
            else:
    ...
    def get(cls, name=None, category='general'):
        o = cls.cm.find(provider='general', kind='default', scope='first', category=category, name=name, output='dict')
        if o is None:
            return
        else:
            if o.type == 'int':
    ...
    @classmethod
    def generate_name(cls, counter_name, display_name=None, prefix=None, fill=3):
    ...
    def load(cls, filename):
        ...
        for cloud in clouds:
            db = {'image': cls.get(name='image', category=cloud), 
        ...
        if 'keys' in config:
            ...
            if exist_key is None:
                cls.set_key(name)
        elif cls.key is not None and cls.user is not None:
            pass
