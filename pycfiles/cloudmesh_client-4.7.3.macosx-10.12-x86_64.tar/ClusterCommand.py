...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/ClusterCommand.py
...
def boot_from_args(arg):
    ...
    for index in range(0, arg.count):
        vm_details = dotdict({'cloud': arg.cloud, 
           ...
           'user': arg.user})
        vm_details.username = Image.guess_username_from_category(vm_details.cloud, vm_details.image, username=arg.username)
        try:
            if arg.dryrun:
...
class ClusterCommand(PluginCommand, CloudPluginCommand):
    ...
    def do_cluster(self, args, arguments):
        ...
        if arg.create:
            ...
            Console.ok('.ok')
            return ''
        if arg.list and arg.NAME is not None:
            if arg.short:
                vms = Vm.get_vms_by_group(arg.group)
                if vms is None:
                    Console.error(('no vms found for {}').format(arg.group))
                else:
                    print(Printer.list(vms, header=[
                     'Group', 'Vm'], order=[
                     'name', 'member'], output=arg.format))
            else:
                ...
                if vms is None:
                    ...
                    print(Printer.list(vms, order=[
                     'name', 'cluster', 'flavor', 'image', 'status', 'user_id', 'floating_ip'], output=arg.format))
            return ''
        if arg.setup:
            ...
            if vms is None:
                ...
                for vm in vms:
                    ...
                    chameleon = 'chameleon' in ConfigDict(filename='cloudmesh.yaml')['cloudmesh']['clouds'][cloud]['cm_host']
                    print('C', chameleon)
                    if chameleon:
                        vm.username = 'cc'
                    elif vm.category == 'azure':
                        vm.username = ConfigDict(filename='cloudmesh.yaml')['cloudmesh']['clouds']['azure']['default']['username']
                    elif vm.username is None:
