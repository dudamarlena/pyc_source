...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/CometCommand.py
# Compiled at: 2017-06-23 10:34:35
    ...
    def do_comet(self, args, arguments):
        ...
        if arguments['init']:
            ...
            if 'endpoints' in cometConf.keys():
                ...
                if len(endpoints) < 1:
                    Console.error('No service endpoints available. Please check the config template', traceflag=False)
                    return ''
            if 'username' in cometConf.keys():
                default_username = cometConf['username']
                if 'TBD' == default_username:
                    set_default_user = input('Set a default username (RETURN to skip): ')
                    if set_default_user:
                        config.data['cloudmesh']['comet']['username'] = set_default_user
                        config.save()
                        Console.ok('Comet default username set!')
            if 'active' in cometConf.keys():
                active_endpoint = cometConf['active']
                set_active_endpoint = input('Set the active service endpoint to use. The availalbe endpoints are - %s [%s]: ' % (
                 ('/').join(endpoints),
                 active_endpoint))
                if set_active_endpoint:
                    if set_active_endpoint in endpoints:
                        config.data['cloudmesh']['comet']['active'] = set_active_endpoint
                        config.save()
                        Console.ok('Comet active service endpoint set!')
                    else:
                        Console.error('The provided endpoint does not match any available service endpoints. Try %s' % ('/').join(endpoints), traceflag=False)
            if cometConf['active'] in endpoints:
                endpoint_url = cometConf['endpoints'][cometConf['active']]['nucleus_base_url']
                api_version = cometConf['endpoints'][cometConf['active']]['api_version']
                set_endpoint_url = input('Set the base url for the nucleus %s service [%s]: ' % (
                 cometConf['active'],
                 endpoint_url))
                if set_endpoint_url:
                    if set_endpoint_url != endpoint_url:
                        config.data['cloudmesh']['comet']['endpoints'][cometConf['active']]['nucleus_base_url'] = set_endpoint_url
                        config.save()
                        Console.ok('Service base url set!')
                set_api_version = input('Set the api version for the nucleus %s service [%s]: ' % (
                 cometConf['active'],
                 api_version))
                if set_api_version:
                    if set_api_version != api_version:
                        config.data['cloudmesh']['comet']['endpoints'][cometConf['active']]['api_version'] = set_api_version
                        config.save()
                        Console.ok('Service api version set!')
                print 'Authenticating to the nucleus %s service and obtaining the apikey...' % cometConf['active']
            ...
            if endpoint:
                if 'endpoints' in cometConf.keys():
                    ...
                    if endpoint in endpoints:
                        config.data['cloudmesh']['comet']['active'] = endpoint
                        config.save()
                        Console.ok('Comet active service endpoint set to: %s' % endpoint)
                    else:
                        Console.error('The provided endpoint does not match any available service endpoints. Try %s.' % ('/').join(endpoints), traceflag=False)
                else:
                    Console.error('No available endpoint to set. Check config file!', traceflag=False)
            elif 'active' in cometConf.keys():
                active_endpoint = cometConf['active']
                Console.ok('Current active service endpoint is: %s' % active_endpoint)
            else:
                Console.error('Cannot set active endpoint. Check config file!', traceflag=False)
        try:
            endpoint = None
            config = ConfigDict('cloudmesh.yaml')
            cometConf = config['cloudmesh.comet']
        ...
        if arguments['ll']:
            ...
            print Cluster.list(cluster_id, format=output_format, sort=sortkey, view=view)
            ...
            try:
                ...
                Console.error('No allocation available for the specified cluster.Please check with the comet help team', traceflag=False)
                return ''
            ...
            if computenodeids:
                ...
                if param <= 0:
                    Console.error('count value has to be greather than zero', traceflag=False)
                    return ''
                numnodes = param
            else:
                Console.error('You have to specify either the count of nodes, or the names of nodes in hostlist format', traceflag=False)
                return ''
            ...
            if not walltime:
                print 'No valid walltime specified. Using system default (2 days)'
            if not allocation:
                if len(allocations) == 1:
                    allocation = allocations[0]
                else:
                    allocation = Cluster.display_get_allocation(allocations)
            print Cluster.computeset_start(clusterid, computenodeids, numnodes, allocation, reservation, walltime)
            ...
            if arguments['upload']:
                ...
                if os.path.isfile(isofile):
                    ...
                    print ('File does not exist - {}').format(arguments['PATHISOFILE'])
                    return ''
            ...
            if arguments['info']:
                ...
                if len(oldnames) != len(newnames):
                    Console.error('Length of OLDNAMES and NEWNAMES have to be the same', traceflag=False)
                    return ''
                else:
                    for newname in newnames:
                        if newname.strip() == '':
                            Console.error('Newname cannot be empty string', traceflag=False)
                            return ''
                    ...
                    if len(cluster_data) > 0:
                        ...
                        Console.error('Error obtaining the cluster information', traceflag=False)
                        return ''
                    oldset = set(oldnames)
                    newset = set(newnames)
                    ...
                    if not oldset <= currentset:
                        ...
                        if keptset & newset != set():
                            Console.error('Not proceeding as otherwise introducing duplicated names', traceflag=False)
                        else:
                            ...
                            confirm = input('Confirm batch renaming (Y/y to confirm, any other key to abort):')
                            if confirm.lower() == 'y':
            ...
            if arguments['create'] or arguments['update'] or arguments['delete']:
