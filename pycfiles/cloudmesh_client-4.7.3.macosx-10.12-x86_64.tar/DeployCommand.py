...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/shell/plugins/DeployCommand.py
...
class AnsibleCommand(object):
    ...
    def role(self, cluster=None, uris=None, hostsPattern='all', become=True, username=None, modifyKnownHosts=True):
        ...
        play = AnsiblePlaybook(roles=roles, inventory=inventory, username=username, modifyKnownHosts=modifyKnownHosts, subprocess_kwargs=dict(stdout=None, stderr=None))
        play.run()
        return
...
class DeployCommand(PluginCommand, CloudPluginCommand):
    ...
    def do_deploy(self, args, arguments):
        ...
        if arguments.ansible and arguments.role:
            ...
            ansible.role(cluster=arguments.CLUSTER, hostsPattern=arguments['--hosts'], uris=arguments.URI, become=not arguments['--no-become'], username=arguments['--username'], modifyKnownHosts=not arguments['--no-modify-known-hosts'])
