...
# [GCC 8.4.0]
# Embedded file name: /Users/grey/.pyenv/versions/cm/lib/python2.7/site-packages/cloudmesh_client/cloud/key.py
...
class Key(ListResource):
    ...
    def get_from_dir(cls, directory=None, store=True):
        ...
        files = [ file for file in os.listdir(expanduser(Config.path_expand(directory))) if file.lower().endswith('.pub')
                ]
        d = []
        for file in files:
            ...
            sshkey['source'] = 'file'
            if store:
                cls._add_from_sshkey(dict(sshkey), keyname=sshkey['name'], source=sshkey['source'], uri=sshkey['uri'])
            else:
    ...
    @classmethod
    def _add_from_sshkey(cls, sshkey, keyname=None, user=None, source=None, uri=None):
    ...
    def list_on_cloud(cls, cloud, live=False, format='table'):
        ...
        try:
            ...
            return Printer.write(keys, order=order, header=header, output=format)
        except Exception as ex:
            Console.error(ex.message)
    ...
    def add_azure_key_to_db(cls, key_name, key_path, certificate_path, pfx_path):
        ...
        cls.add_from_path(key_path, key_name, source='ssh', uri='file://' + key_path)
    ...
    @classmethod
    def add_from_path(cls, path, keyname=None, user=None, source=None, uri=None, store=True):
        """
        ...
        if store:
            cls._add_from_sshkey(sshkey.__key__, keyname, user, source=source, uri=uri)
        else:
