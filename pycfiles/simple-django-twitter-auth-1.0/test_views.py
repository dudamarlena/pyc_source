...
# [GCC 8.4.0]
# Embedded file name: C:\Users\Dylan\projects\simple_django_twitter_auth_example\django_twitter_auth\tests\test_views.py
# Compiled at: 2015-03-28 17:45:50
...
class LoginViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_auth = {'auth_url': 'http://faketwitterurl.com/auth/', 
           'oauth_token': 'tempoauthtoken', 
           'oauth_token_secret': 'tempoauthtokensecret'}
    ...
    def test_Twython_instance_initialized_with_app_tokens(self):
        response = self.client.get(reverse('django_twitter_auth:login'))
        self.mock_Twython.assert_called_once_with(settings.TWITTER_APP_KEY, settings.TWITTER_APP_SECRET)

    def test_callback_url_set_to_login_view(self):
        callback_url = settings.HOST + reverse('django_twitter_auth:login') + '?redir_to='
        response = self.client.get(reverse('django_twitter_auth:login'))
        self.mock_Twython_instance.get_authentication_tokens.assert_called_once_with(callback_url=callback_url)

    def test_temporary_OAuth2_tokens_stored_in_session(self):
        response = self.client.get(reverse('django_twitter_auth:login'))
        self.assertEqual(self.client.session['oauth_token'], self.mock_auth['oauth_token'])
        self.assertEqual(self.client.session['oauth_token_secret'], self.mock_auth['oauth_token_secret'])
...
class LoginViewCallbackTest(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_Twython = patcher.start()
        self.addCleanup(patcher.stop)
        self.mock_final_auth = {'oauth_token': 'finaloauthtoken', 
        ...
        profile = mommy.make(TwitterProfile, pk=1)
        self.mock_get_or_create.return_value = (profile, False)

    def test_view_raises_403_if_oauth_tokens_dont_match(self):
        response = self.client.get(reverse('django_twitter_auth:login') + '?oauth_verifier=twitteroauthverifiertoken' + '&oauth_token=invalidtempoauthtoken')
        self.assertEqual(response.status_code, 403)

    def test_Twython_instance_initialized_with_temp_user_OAuth2_tokens(self):
        response = self.client.get(reverse('django_twitter_auth:login') + '?oauth_verifier=twitteroauthverifiertoken' + '&oauth_token=' + self.client.session['oauth_token'])
        self.mock_Twython.assert_called_once_with(settings.TWITTER_APP_KEY, settings.TWITTER_APP_SECRET, self.client.session['oauth_token'], self.client.session['oauth_token_secret'])
    ...
    def test_get_authorized_tokens_called_with_oauth_verifier_param(self):
        """
                ...
                """
        response = self.client.get(reverse('django_twitter_auth:login') + '?oauth_verifier=twitteroauthverifiertoken' + '&oauth_token=' + self.client.session['oauth_token'])
        self.mock_Twython_instance.get_authorized_tokens.assert_called_once_with('twitteroauthverifiertoken')
    ...
    def test_view_redirects_to_denied_view_if_denied_param_found(self):
        """
                ...
                """
        response = self.client.get(reverse('django_twitter_auth:login') + '?denied=True')
    ...
    def test_view_redirects_to_redir_param_if_no_new_user_setting_defined(self):
        response = self.client.get(reverse('django_twitter_auth:login') + '?oauth_verifier=twitteroauthverifiertoken' + '&oauth_token=' + self.client.session['oauth_token'] + '&redir_to=' + '/redirection-url/')
