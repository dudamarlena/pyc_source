...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/parsers/yaml_parser.py
...
class YAML(Parser):
    help = 'Parses New HEPData YAML format. Input parameter should be path to the directory where submission.yaml file is present (or direct filepath to the submission.yaml file)'
    pool = Pool()
    ...
    def parse(self, data_in, *args, **kwargs):
        ...
        for i in range(0, len(submission_data)):
            ...
            with open(table_filepath, 'r') as (table_file):
                ...
                if not data_file_validator.validate(data=table_data, file_path=table_filepath):
                    raise RuntimeError('Data file (%s) did not pass validation: %s' % (
                     table_filepath,
                     self._pretty_print_errors(data_file_validator.get_messages())))
                index = index + 1
                table = Table(index=index, metadata=submission_data[i], data=table_data)
                tables.append(table)
