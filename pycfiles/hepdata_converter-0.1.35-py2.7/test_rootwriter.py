...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/testsuite/test_rootwriter.py
...
def walk(tdirectory, path=None, depth=0):
    ...
    if path:
        ...
        if not isinstance(tdirectory, ROOT.TFile):
            dirpath = tdirectory.GetName()
        else:
            dirpath = ''
        yield (
         dirpath, dirnames, objectnames)
        for dirname in dirnames:
            rdir = tdirectory.GetDirectory(dirname)
            for x in walk(rdir, depth=depth + 1, path=dirpath):
                yield x
...
class ROOTWriterTestSuite(WriterTestSuite):
    ...
    def test_simple_parse(self, yaml_full_path, full_root_path):
        ...
        hepdata_converter.convert(yaml_full_path, output_file_path, options={'output_format': 'root', 'validator_schema_version': '0.1.0'})
        ...
        with open(output_file_path, 'w') as (output):
            hepdata_converter.convert(yaml_full_path, output, options={'output_format': 'root', 'validator_schema_version': '0.1.0'})
        self.assertNotEqual(os.stat(output_file_path).st_size, 0, 'output root file is empty')
        f = ROOT.TFile.Open(output_file_path)
        f_orig = ROOT.TFile.Open(full_root_path)
        self.assertEqual(list(walk(f)), list(walk(f_orig)))
    ...
    def test_th1_parse(self, yaml_full_path, full_root_path):
        output_file_path = os.path.join(self.current_tmp, 'datafile.root')
        hepdata_converter.convert(yaml_full_path, output_file_path, options={'output_format': 'root', 'table': 'data2.yaml', 
    ...
    def test_parse_all(self, test_submissions):
        for idx, test_submission in enumerate(test_submissions):
            ...
            hepdata_converter.convert(test_submission, output_file_path, options={'output_format': 'root'})
            self.assertNotEqual(os.stat(output_file_path).st_size, 0, 'output root file is empty')