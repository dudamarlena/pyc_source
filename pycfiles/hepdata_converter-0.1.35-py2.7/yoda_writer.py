...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/writers/yoda_writer.py
...
class ScatterYodaClass(ObjectWrapper):
    ...
    def _create_scatter(self):
        ...
        error_breakdown = yaml.safe_dump(self.err_breakdown, default_flow_style=True, default_style='', width=1000000.0)
...
class YODA(ArrayWriter):
    help = 'Writes YODA output for table specified by --table parameter, the output should be defined as filepath to output yoda file'
    class_list = [
     Scatter3DYodaClass, Scatter2DYodaClass, Scatter1DYodaClass]

    @classmethod
    def options(cls):
        options = ArrayWriter.options()
        options['rivet_analysis_name'] = Option('rivet-analysis-name', 'r', type=str, default='RIVET_ANALYSIS_NAME', required=False, variable_mapping='rivet_analysis_name', help='Rivet analysis name, e.g. "ATLAS_2016_I1424838"')
        return options
