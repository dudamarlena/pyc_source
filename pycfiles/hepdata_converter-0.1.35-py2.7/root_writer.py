...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/writers/root_writer.py
...
import ROOT as ROOTModule, array, tempfile, os
...
class THFRootClass(ObjectWrapper):
    ...
    def get_hist_classes(self):
        return [
         ROOTModule.TH1F, ROOTModule.TH2F, ROOTModule.TH3F]
    ...
    def match(cls, independent_variables_map, dependent_variable):
        if not ObjectWrapper.match(independent_variables_map, dependent_variable):
            return False
        if len(independent_variables_map) == cls.dim:
...
class TGraph2DErrorsClass(ObjectWrapper):
    ...
    def create_objects(self):
        self.calculate_total_errors()
        tol = 1e-15
        if any([ a - b > tol for a, b in zip(self.xerr_plus[0], self.xerr_minus[0]) ]) or any([ a - b > tol for a, b in zip(self.xerr_plus[1], self.xerr_minus[1]) ]) or any([ a - b > tol for a, b in zip(self.yerr_plus, self.yerr_minus) ]):
            return []
        if len(self.xval[0]):
            graph = ROOTModule.TGraph2DErrors(len(self.xval[0]), array.array('d', self.xval[0]), array.array('d', self.xval[1]), array.array('d', self.yval), array.array('d', self.xerr_plus[0]), array.array('d', self.xerr_plus[1]), array.array('d', self.yerr_plus))
        else:
            return []
...
class TGraphAsymmErrorsRootClass(ObjectWrapper):
    ...
    def create_objects(self):
        self.calculate_total_errors()
        if len(self.xval[0]):
            graph = ROOTModule.TGraphAsymmErrors(len(self.xval[0]), array.array('d', self.xval[0]), array.array('d', self.yval), array.array('d', self.xerr_minus[0]), array.array('d', self.xerr_plus[0]), array.array('d', self.yerr_minus), array.array('d', self.yerr_plus))
        else:
            return []
    ...
    def _write_table(self, data_out, table):
        ...
        for ii, independent_variable in enumerate(table.independent_variables):
            if False in ObjectWrapper.is_number_var(independent_variable):
                independent_variable_bins = {'header': {'name': independent_variable['header']['name'] + ' bin'}, 'values': [], 'labels': []}
