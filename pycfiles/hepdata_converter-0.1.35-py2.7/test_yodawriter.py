...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/testsuite/test_yodawriter.py
...
class YODAWriterTestSuite(WriterTestSuite):
    ...
    def test_simple_parse(self, yaml_simple_path, yoda_template):
        ...
        hepdata_converter.convert(yaml_simple_path, output_file_path, options={'output_format': 'yoda', 'validator_schema_version': '0.1.0'})
    ...
    def test_parse_all(self, test_submissions):
        for idx, test_submission in enumerate(test_submissions):
            ...
            hepdata_converter.convert(test_submission, output_file_path, options={'output_format': 'yoda'})
            self.assertNotEqual(os.stat(output_file_path).st_size, 0, 'output yoda file is empty')