...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hepdata_converter/writers/csv_writer.py
...
class CSV(ArrayWriter):
    help = 'Writes to CSV format, it can write either one table (specified by --table parameter) or all tables from the input file. In the case of one table output must be filepath to the new csv file, in the case of multiple tables the output must be specified to be a directory to which all table files should be written'

    @classmethod
    def options(cls):
        options = ArrayWriter.options()
        options['pack'] = Option('pack', type=bool, default=False, required=False, help='If specified, dependent variables will be put in one table, instead of creating one table per dependent variable in CSV file')
        options['separator'] = Option('separator', type=str, default=',', required=False, help='Defines separator for CSV file, the default is comma: ","')
        return options
    ...
    def _write_metadata(self, data_out, table):
        ...
        if 'data_license' in table.metadata and table.metadata['data_license']:
            license_text = (table.metadata['data_license'].get('name') or '') + ' ' + (table.metadata['data_license'].get('url') or '') + ' ' + (table.metadata['data_license'].get('description') or '')
