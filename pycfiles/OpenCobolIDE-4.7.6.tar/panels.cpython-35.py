...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/managers/panels.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 10668 bytes
"""
...
class PanelsManager(Manager):
    ...
    def append(self, panel, position=Panel.Position.LEFT):
        ...
        pos_to_string = {Panel.Position.BOTTOM: 'bottom', 
         Panel.Position.LEFT: 'left', 
         Panel.Position.RIGHT: 'right', 
         Panel.Position.TOP: 'top'}
        _logger().log(5, 'adding panel %s at %r', panel.name, pos_to_string[position])
    ...
    def resize(self):
        ...
        for panel in panels:
            if not panel.isVisible():
                ...
                panel.setGeometry(crect.left() + left, crect.top() + s_top, size_hint.width(), crect.height() - s_bottom - s_top - h_offset)
        ...
        for panel in panels:
            if not panel.isVisible():
                ...
                size_hint = panel.sizeHint()
                panel.setGeometry(crect.right() - right - size_hint.width() - w_offset, crect.top() + s_top, size_hint.width(), crect.height() - s_bottom - s_top - h_offset)
        ...
        for panel in panels:
            if not panel.isVisible():
                ...
                size_hint = panel.sizeHint()
                panel.setGeometry(crect.left(), crect.top() + top, crect.width() - w_offset, size_hint.height())
        ...
        for panel in panels:
            if not panel.isVisible():
                ...
                size_hint = panel.sizeHint()
                panel.setGeometry(crect.left(), crect.bottom() - bottom - size_hint.height() - h_offset, crect.width() - w_offset, size_hint.height())
    ...
    def _update(self, rect, delta_y, force_update_margins=False):
        ...
        for zones_id, zone in self._panels.items():
            if not zones_id == Panel.Position.TOP:
                if zones_id == Panel.Position.BOTTOM:
                    pass
                else:
