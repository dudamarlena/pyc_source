...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/modula2.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 52564 bytes
"""
    ...
    def get_tokens_unprocessed(self, text):
        for index, token, value in RegexLexer.get_tokens_unprocessed(self, text):
            ...
            if self.dialect not in ('unknown', 'm2r10', 'objm2'):
                ...
                if self.dialect in ('m2r10', 'objm2'):
                    if token is Number.Oct:
                        token = Error
                    else:
                        if token is Number.Hex and 'H' in value:
                            token = Error
                        elif token is Number.Float and 'E' in value:
                            token = Error
                else:
                    if token in Comment:
                        if token is Comment.Single and self.dialect not in ('unknown',
                                                                            'm2r10',
                                                                            'objm2'):
                            token = Error
                    ...
                    elif value in self.lexemes_to_reject:
                        token = Error
                if self.algol_publication_mode:
                    if value == '#':
                        value = '≠'
