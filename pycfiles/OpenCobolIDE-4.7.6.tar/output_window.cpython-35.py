...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/output_window.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 56836 bytes
"""
...
"""
import logging, os, re, sys, string
...
class OutputWindow(CodeEdit):
    ...
    def __init__(self, parent=None, color_scheme=None, formatter=None, input_handler=None, backend=server.__file__, link_regex=re.compile('("|\\\')(?P<url>(/|[a-zA-Z]:\\\\)[\\w/\\s\\\\\\.\\-]*)("|\\\')(, line (?P<line>\\d*))?')):
        """
    ...
    def _on_process_finished(self):
        ...
        if self._process.exitStatus() != self._process.NormalExit:
            exit_code = 139
        self._formatter.append_message('\x1b[0m\nProcess finished with exit code %d' % exit_code, output_format=OutputFormat.CustomFormat)
...
class AnsiEscapeCodeParser(object):
    ...
    _semicolon = ';'
    _color_terminator = 'm'
    _supported_commands = re.compile('^(?P<n>\\d*;?\\d*)(?P<cmd>[ABCDEFGHJKfP]{1})')
    _unsupported_command = re.compile('^(\\?\\d+h)|^(\\??\\d+l)|^(\\d+d)|^(\\d+X)|^(\\([AB01])|^(\\)[AB01])|^(\\d*;?\\d*r)|^(=)|^(>)|^(\\d;.*\\x07)')
    _commands = {'A': 'cursor_up', 
     'B': 'cursor_down', 
     'C': 'cursor_forward', 
     'D': 'cursor_back', 
     'E': 'cursor_next_line', 
                    ...
                    if self._pending_text in [self._escape] + self._escape_alts:
                        ...
                        if m and self._pending_text == self._escape:
                            ...
                            if m:
                                ...
                                if self._pending_text in ('\x1b=', '\x1b>'):
                                    self._pending_text = ''
                                continue
                            str_nbr = ''
                            numbers = []
    ...
    if ctrl:
        ...
        if event.key() == QtCore.Qt.Key_Return:
            ...
            if event.key() == QtCore.Qt.Key_F12:
                return b'F'
            if event.text() in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789[],=-.;/`&^~*@|#(){}$><%+?"_!\'\\ :':
...
OutputWindow.LinuxColorScheme = OutputWindow.create_color_scheme(background=QColor('black'), foreground=QColor('white'), red=QColor('#FF5555'), green=QColor('#55FF55'), yellow=QColor('#FFFF55'), blue=QColor('#5555FF'), magenta=QColor('#FF55FF'), cyan=QColor('#55FFFF'))
OutputWindow.TangoColorScheme = OutputWindow.create_color_scheme(background=QColor('black'), foreground=QColor('white'), red=QColor('#CC0000'), green=QColor('#4E9A06'), yellow=QColor('#C4A000'), blue=QColor('#3465A4'), magenta=QColor('#75507B'), cyan=QColor('#06989A'))
OutputWindow.SolarizedColorScheme = OutputWindow.create_color_scheme(background=QColor('#073642'), foreground=QColor('#EEE8D5'), red=QColor('#DC322F'), green=QColor('#859900'), yellow=QColor('#B58900'), blue=QColor('#268BD2'), magenta=QColor('#D33682'), cyan=QColor('#2AA198'))
