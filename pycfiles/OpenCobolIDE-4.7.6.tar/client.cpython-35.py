...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/api/client.py
...
"""
import locale, json, logging, socket, struct, sys, uuid
...
SOCKET_ERROR_STRINGS = {0: 'the connection was refused by the peer (or timed out).', 
 ...
 6: "the datagram was larger than the operating system's limit (which can be as low as 8192 bytes).", 
 7: 'an error occurred with the network (e.g., the network cable was accidentally plugged out).', 
 -1: 'an unidentified error occurred.'}
PROCESS_ERROR_STRING = {0: 'the process failed to start. Either the invoked program is missing, or you may have insufficient permissions to invoke the program.', 
...
class JsonTcpClient(QtNetwork.QTcpSocket):
    ...
    def __init__(self, parent, port, worker_class_or_function, args, on_receive=None):
    ...
    def _on_error(self, error):
        if error not in SOCKET_ERROR_STRINGS:
            error = -1
        if error == 1 and self.is_connected or not self.is_connected and error == 0 and not self._closed:
            log_fct = comm
        else:
    ...
    def _on_disconnected(self):
        try:
            comm('disconnected from backend: %s:%d', self.peerName(), self.peerPort())
