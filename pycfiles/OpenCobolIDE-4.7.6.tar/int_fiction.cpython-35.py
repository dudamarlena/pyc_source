...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/int_fiction.py
...
class Inform6Lexer(RegexLexer):
    ...
    tokens = {'root': [
     ...
     'value': [
       ...
       'random', 'sibling', 'younger', 'youngest'), suffix='\\b'),
                Name.Builtin, '#pop'),
               (
       ...
       'RV__Pr', 'Symb__Tab', 'Unsigned__Compare', 'WV__Pr', 'Z__Region'), prefix='(?i)', suffix='\\b'),
                Name.Builtin, '#pop'),
               (
       ...
       'USE_MODULES', 'WORDSIZE'), prefix='(?i)', suffix='\\b'),
                Name.Builtin, '#pop'),
               (
                _name, Name, '#pop')], 
     ...
     '_directive-keyword!': [
       ...
       'time', 'topic', 'warning', 'with'), suffix='\\b'),
                              Keyword, '#pop'),
                             (
     ...
     'trace-keyword?': [
       ...
       'on', 'symbols', 'tokens', 'verbs'), suffix='\\b'),
                         Keyword, '#pop'),
                        default('#pop')], 
     ...
     'statements': [
       ...
       'read', 'remove', 'return', 'rfalse', 'rtrue', 'spaces', 'string', 'until'), suffix='\\b'),
                     Keyword, 'default'),
                    (
                     '(do|else)\\b', Keyword),
                    (
    ...
    for level in token_variants:
        tokens[level] = {'+i6-root': list(Inform6Lexer.tokens['root']), 
         ...
         '+phrase-definition': [
                                ...
                                (
                                 '(\\([%s])(.*?)([%s]\\))' % (_dash, _dash),
                                 bygroups(Punctuation, using(this, state=('+i6-root', 'directive', 'default', 'statements'), i6t='+i6t-inline'), Punctuation), '#pop'),
                                default('#pop')], 
