...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/shell.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 30992 bytes
"""
...
class BatchLexer(RegexLexer):
    ...
    def _make_begin_state(compound, _core_token=_core_token, _core_token_compound=_core_token_compound, _keyword_terminator=_keyword_terminator, _nl=_nl, _punct=_punct, _string=_string, _space=_space, _start_label=_start_label, _stoken=_stoken, _token_terminator=_token_terminator, _variable=_variable, _ws=_ws):
       ...
       'ver', 'verify', 'vol'), suffix=_keyword_terminator), Keyword, 'follow%s' % suffix),
         (
          '(call)(%s?)(:)' % _space,
    ...
    tokens = {'root': _make_begin_state(False), 
     ...
     'if': [
            ...
            (
             '(%s%s?)(==)(%s?%s)' % (_stoken, _space, _space, _stoken),
            ...
            (
             '(%s%s)(%s)(%s%s)' % (_stoken, _space, _opword, _space, _stoken),
...
class PowerShellLexer(RegexLexer):
    ...
    verbs = 'write where wait use update unregister undo trace test tee take suspend stop start split sort skip show set send select scroll resume restore restart resolve resize reset rename remove register receive read push pop ping out new move measure limit join invoke import group get format foreach export expand exit enter enable disconnect disable debug cxnew copy convertto convertfrom convert connect complete compare clear checkpoint aggregate add'.split()
