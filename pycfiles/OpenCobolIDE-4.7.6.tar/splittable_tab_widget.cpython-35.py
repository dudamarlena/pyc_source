...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/splittable_tab_widget.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 60085 bytes
"""
This module contains the splittable tab widget API
"""
import inspect, logging, mimetypes, os, io, sys, uuid, weakref
...
class BaseTabWidget(QtWidgets.QTabWidget):
            ...
            if name is None and slot is None:
                ...
                if slot == self.close and self.a_close is None:
                    ...
                    if slot == self.close_left:
                        ...
                        if slot == self.close_right:
                            self.a_close_right = qaction
                        else:
                            if slot == self.close_others:
    ...
    def _show_tab_context_menu(self, position):
        if self.count():
            ...
            SplittableTabWidget.tab_under_menu = self.widget(self.tab_under_menu())
    ...
    def _on_tab_close_requested(self, index):
        ...
        if not dirty:
            ...
            dlg = DlgUnsavedFiles(self, files=[path])
        if dlg.exec_() == dlg.Accepted:
            rm = True
            if not dlg.discarded:
                try:
    ...
    def _on_split_requested(self):
        ...
        if 'horizontally' in orientation:
            self.split_requested.emit(widget, QtCore.Qt.Horizontal)
        else:
            self.split_requested.emit(widget, QtCore.Qt.Vertical)
...
class OpenFilesPopup(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None, qsettings=None):
        ...
        self.sort_enabled = bool(self.settings.value('sortOpenFilesAlphabetically', False))
    ...
    def set_filenames(self, filenames):
        ...
        if self.sort_enabled:
            filenames = sorted(filenames, key=lambda x: QtCore.QFileInfo(x).fileName().lower())
    ...
    def _on_sort_changed(self, *_):
        ...
        self.settings.setValue('sortOpenFilesAlphabetically', self.sort_enabled)
...
class SplittableTabWidget(QtWidgets.QSplitter):
    ...
    def __init__(self, parent=None, root=True, create_popup=True, qsettings=None):
        super(SplittableTabWidget, self).__init__(parent)
        SplittableTabWidget.tab_widget_klass._detached_window_class = SplittableTabWidget.detached_window_klass
        if root:
        ...
        self.main_tab_widget.last_tab_closed.connect(self._on_last_tab_closed)
    ...
    def add_tab(self, tab, title='', icon=None):
        ...
        self.main_tab_widget.setCurrentIndex(self.main_tab_widget.indexOf(tab))
    ...
    def _show_popup(self):
        ...
        self.popup.set_filenames([editor.file.path for editor in self.widgets()])
        self.popup.show()
    ...
    def split(self, widget, orientation):
        ...
        splitter.add_tab(clone, title=self.main_tab_widget.tabText(self.main_tab_widget.indexOf(widget)), icon=icon)
...
class CodeEditTabWidget(BaseTabWidget):
    ...
    def get_filter(cls, mimetype):
        ...
        """
        filters = ' '.join(['*%s' % ext for ext in mimetypes.guess_all_extensions(mimetype)])
    ...
    def _ask_path(cls, editor):
        ...
        return QtWidgets.QFileDialog.getSaveFileName(editor, _('Save file as'), cls.default_directory, filter)
...
class DetachedEditorWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        action = tb.addAction(QtGui.QIcon.fromTheme('document-save'), _('Save'))
...
class SplittableCodeEditTabWidget(SplittableTabWidget):
    ...
    def __init__(self, parent=None, root=True, qsettings=None):
        ...
        super(SplittableCodeEditTabWidget, self).__init__(parent, root, qsettings=qsettings)
        self.main_tab_widget.tabBar().double_clicked.connect(self.tab_bar_double_clicked.emit)
        if root:
            self.closed_tabs_history_btn = QtWidgets.QToolButton()
            self.closed_tabs_history_btn.setAutoRaise(True)
            self.closed_tabs_history_btn.setIcon(QtGui.QIcon.fromTheme('user-trash', QtGui.QIcon(':/pyqode-icons/rc/edit-trash.png')))
            self.closed_tabs_history_btn.setPopupMode(QtWidgets.QToolButton.InstantPopup)
    ...
    def register_code_edit(cls, code_edit_class):
        ...
        for mimetype in code_edit_class.mimetypes:
            if mimetype in cls.editors:
                _logger().warn('editor for mimetype already registered, skipping')
    ...
    def save_current_as(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(self, _('Failed to save file as'), _('Failed to save file as %s\nError=%s') % (
            ...
            if not success:
                ...
                tw.setTabText(tw.indexOf(widget), os.path.split(widget.file.path)[1])
    ...
    def _save(self, widget):
        ...
        if widget.dirty:
            try:
                ...
                QtWidgets.QMessageBox.warning(self, 'Failed to save file', 'Failed to save file: %s\nError=%s' % (
                 widget.file.path, str(e)))
            else:
    ...
    def _create_code_edit(self, mimetype, *args, **kwargs):
        ...
        if mimetype in self.editors.keys():
            return self.editors[mimetype](*args, **kwargs)
        editor = self.fallback_editor(*args, **kwargs)
        return editor

    def create_new_document(self, base_name='New Document', extension='.txt', preferred_eol=0, autodetect_eol=True, **kwargs):
        """
        ...
        tab = self._create_code_edit(self.guess_mimetype(name), **kwargs)
    ...
    def open_document(self, path, encoding=None, replace_tabs_by_spaces=True, clean_trailing_whitespaces=True, safe_save=True, restore_cursor_position=True, preferred_eol=0, autodetect_eol=True, show_whitespaces=False, **kwargs):
        """
        Opens a document.
        ...
        for tab in self.widgets():
            ...
            if title == name:
                ...
                new_name = os.path.join(os.path.split(os.path.dirname(tab.file.path))[1], title)
                tw.setTabText(tw.indexOf(tab), new_name)
                use_parent_dir = True

        if use_parent_dir:
            name = os.path.join(os.path.split(os.path.dirname(path))[1], name)
            use_parent_dir = False
        tab = self._create_code_edit(self.guess_mimetype(path), **kwargs)
        self.editor_created.emit(tab)
        tab.open_parameters = {'encoding': encoding, 
        ...
        try:
            ...
            self.closed_tabs_history_btn.setEnabled(len(self.closed_tabs_menu.actions()) > 0)
            return tab
    ...
    def _on_current_changed(self, new):
        old, new = super(SplittableCodeEditTabWidget, self)._on_current_changed(new)
        if new:
    ...
    def split(self, widget, orientation):
        ...
        if splitter:
            splitter.tab_bar_double_clicked.connect(self.tab_bar_double_clicked.emit)
        ...
        try:
            ...
            for i, action in enumerate(self.closed_tabs_menu.actions()):
                ...
                try:
                    ...
                    action = self.closed_tabs_menu.addAction(self._icon(path), filename)
                else:
                    action = QtWidgets.QAction(self._icon(path), filename, self.closed_tabs_menu)
                    self.closed_tabs_menu.insertAction(before, action)
                ...
                while nb_actions > self.CLOSED_TABS_HISTORY_LIMIT:
                    self.closed_tabs_menu.removeAction(self.closed_tabs_menu.actions()[(-1)])
                    nb_actions = len(self.closed_tabs_menu.actions())
