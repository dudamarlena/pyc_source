...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/configs.py
...
class DockerLexer(RegexLexer):
    ...
    tokens = {'root': [
              (
               '^(ONBUILD)(\\s+)(%s)\\b' % (_keywords,),
               bygroups(Name.Keyword, Whitespace, Keyword)),
              (
               '^(%s)\\b(.*)' % (_keywords,), bygroups(Keyword, String)),
              (
               '#.*', Comment),
              (
               'RUN', Keyword),
...
class TerraformLexer(RegexLexer):
    ...
    tokens = {'root': [
     ...
     'basic': [
               ...
               (
                words(('variable', 'resource', 'provider', 'provisioner', 'module'), prefix='\\b', suffix='\\b'), Keyword.Reserved, 'function'),
               (
                words(('ingress', 'egress', 'listener', 'default', 'connection'), prefix='\\b', suffix='\\b'), Keyword.Declaration),
               (
     ...
     'var_builtin': [
                     (
                      '\\$\\{', String.Interpol, '#push'),
                     (
                      words(('concat', 'file', 'join', 'lookup', 'element'), prefix='\\b', suffix='\\b'), Name.Builtin),
                     include('string'),
                     include('punctuation'),
                     (
                      '\\s+', Text),
