...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/api/code_edit.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 46313 bytes
"""
...
class CodeEdit(QtWidgets.QPlainTextEdit):
    ...
    def link(self, clone):
        ...
        for original_panel, panel in zip(list(self.panels), list(clone.panels)):
    ...
    def close(self, clear=True):
        ...
        self.panels.clear()
        self.backend.stop()
        Cache().set_cursor_position(self.file.path, self.textCursor().position())
    ...
    def eventFilter(self, obj, event):
        if obj == self and event.type() == QtCore.QEvent.KeyPress:
            if event.key() == QtCore.Qt.Key_X and int(event.modifiers()) == QtCore.Qt.ControlModifier:
                self.cut()
                return True
            if event.key() == QtCore.Qt.Key_C and int(event.modifiers()) == QtCore.Qt.ControlModifier:
                self.copy()
                return True
            return False
    ...
    def keyPressEvent(self, event):
        ...
        if not event.isAccepted():
            if event.key() == QtCore.Qt.Key_Tab and event.modifiers() == QtCore.Qt.NoModifier:
                self.indent()
                event.accept()
            elif event.key() == QtCore.Qt.Key_Backtab and event.modifiers() == QtCore.Qt.NoModifier:
                self.un_indent()
                event.accept()
        elif event.key() == QtCore.Qt.Key_Home and int(event.modifiers()) & QtCore.Qt.ControlModifier == 0:
            self._do_home_key(event, int(event.modifiers()) & QtCore.Qt.ShiftModifier)
        if not event.isAccepted():
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            for sel in self.decorations:
                if sel.cursor.blockNumber() == cursor.blockNumber() and sel.contains_cursor(cursor):
    ...
    def mouseMoveEvent(self, event):
        ...
        for sel in self.decorations:
            if sel.contains_cursor(cursor) and sel.tooltip:
                if self._prev_tooltip_block_nbr != cursor.blockNumber() or not QtWidgets.QToolTip.isVisible():
                    pos = event.pos()
                    pos.setX(pos.x() + self.panels.margin_size())
                    pos.setY(pos.y() + self.panels.margin_size(0))
                    self._tooltips_runner.request_job(self.show_tooltip, self.mapToGlobal(pos), sel.tooltip[0:1024], sel)
    ...
    def _show_context_menu(self, point):
        ...
        if nc.position() not in range(tc.selectionStart(), tc.selectionEnd()):
