...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/jvm.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 66829 bytes
"""
    pygments.lexers.jvm
    ~~~~~~~~~~~~~~~~~~~
...
class JavaLexer(RegexLexer):
    ...
    tokens = {'root': [
              ...
              (
               '([^\\W\\d]|\\$)[\\w$]*', Name),
              (
               '([0-9](_*[0-9]+)*\\.([0-9](_*[0-9]+)*)?|([0-9](_*[0-9]+)*)?\\.[0-9](_*[0-9]+)*)([eE][+\\-]?[0-9](_*[0-9]+)*)?[fFdD]?|[0-9][eE][+\\-]?[0-9](_*[0-9]+)*[fFdD]?|[0-9]([eE][+\\-]?[0-9](_*[0-9]+)*)?[fFdD]|0[xX]([0-9a-fA-F](_*[0-9a-fA-F]+)*\\.?|([0-9a-fA-F](_*[0-9a-fA-F]+)*)?\\.[0-9a-fA-F](_*[0-9a-fA-F]+)*)[pP][+\\-]?[0-9](_*[0-9]+)*[fFdD]?',
               Number.Float),
              (
               '0[xX][0-9a-fA-F](_*[0-9a-fA-F]+)*[lL]?', Number.Hex),
              (
               '0[bB][01](_*[01]+)*[lL]?', Number.Bin),
              (
               '0(_*[0-7]+)+[lL]?', Number.Oct),
              (
               '0|[1-9](_*[0-9]+)*[lL]?', Number.Integer),
              (
               '[~^*!%&\\[\\](){}<>|+=:;,./?-]', Operator),
              (
               '\\n', Text)], 
    ...
    def get_tokens_unprocessed(self, text):
        for index, token, value in JavaLexer.get_tokens_unprocessed(self, text):
            if token is Name and value in self.aj_keywords:
                ...
                if token is Name.Label and value in self.aj_inter_type:
                    yield (
...
class KotlinLexer(RegexLexer):
    ...
    tokens = {'root': [
              ...
              (
               ...
               Number),
              (
               '(class)(\\s+)(object)', bygroups(Keyword, Text, Keyword)),
              (
...
class JasminLexer(RegexLexer):
    ...
    tokens = {'default': [
       ...
       'ret_w', 'return', 'saload', 'sastore', 'sipush', 'swap'), suffix=_break), Keyword.Reserved),
                 (
       ...
       'ifne', 'ifnonnull', 'ifnull', 'jsr', 'jsr_w'), suffix=_break),
