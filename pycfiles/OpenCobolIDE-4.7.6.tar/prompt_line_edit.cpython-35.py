...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/prompt_line_edit.py
...
class PromptLineEdit(QtWidgets.QLineEdit):
    ...
    def __init__(self, parent=None, prompt_text=_(' Search'), button_icon=None):
        ...
        if button_icon is None:
            button_icon = icons.icon('edit-clear', ':/pyqode-icons/rc/clear-left.png', 'fa.times-circle')
        self.button.setIcon(button_icon)
        self.button.setToolTip(_('Clear'))
        self.button.setStyleSheet('QToolButton { border: none; padding: 5px; }')
    ...
    def paintEvent(self, event):
        ...
        if self._prompt_text and not self.text() and self.isEnabled():
            ...
            rect = self.style().subElementRect(QtWidgets.QStyle.SE_LineEditContents, option, self).adjusted(2, 0, 0, 0).adjusted(left, top, -right, -bottom)
