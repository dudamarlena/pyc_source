...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/api/utils.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 38429 bytes
"""
This module contains utility functions/classes.
"""
import functools, logging, weakref
...
class TextHelper(object):
    ...
    def word_under_cursor(self, select_whole_word=False, text_cursor=None):
        ...
        while not text_cursor.atStart():
            ...
            try:
                ...
                if selected_txt in word_separators and selected_txt != 'n' and selected_txt != 't' or char.isspace():
                    break
            except IndexError:
                break
    ...
    def clean_document(self):
        ...
        if text_cursor.selectedText():
            ...
            offset = pos[1] - eaten
            text_cursor.movePosition(text_cursor.Right, text_cursor.MoveAnchor, offset)
        else:
            text_cursor.setPosition(cpos)
        editor.setTextCursor(text_cursor)
    ...
    def select_lines(self, start=0, end=-1, apply_selection=True):
        ...
        if end > start:
            text_cursor.movePosition(text_cursor.Down, text_cursor.KeepAnchor, end - start)
            ...
            if end < start:
                ...
                text_cursor.movePosition(text_cursor.Up, text_cursor.KeepAnchor, start - end)
    ...
    def line_pos_from_number(self, line_number):
        ...
        if block.isValid():
            return int(editor.blockBoundingGeometry(block).translated(editor.contentOffset()).top())
        else:
            if line_number <= 0:
                return 0
    ...
    def move_right(self, keep_anchor=False, nb_chars=1):
        ...
        """
        text_cursor = self._editor.textCursor()
        text_cursor.movePosition(text_cursor.Right, text_cursor.KeepAnchor if keep_anchor else text_cursor.MoveAnchor, nb_chars)
        self._editor.setTextCursor(text_cursor)
    ...
    def search_text(self, text_cursor, search_txt, search_flags):
        ...
        def compare_cursors(cursor_a, cursor_b):
            ...
            :returns; True if both cursor are identical (same position, same
                selection)
            """
            return cursor_b.selectionStart() >= cursor_a.selectionStart() and cursor_b.selectionEnd() <= cursor_a.selectionEnd()
    ...
    def match_select(self, ignored_symbols=None):
        ...
        def find_opening_symbol(cursor, matching):
            ...
            start_pos = None
            opening_char = None
            closed = {k:0 for k in matching.values() if k not in ('"', "'") if k not in ('"',
                                                                                         "'")}
            stop = False
        ...
        def find_closing_symbol(cursor, matching, opening_char, original_pos):
            ...
            opened = {k:0 for k in rev_matching.values() if k not in ('"', "'") if k not in ('"',
                                                                                             "'")}
            stop = False
        ...
        if opening_char:
            end_pos = find_closing_symbol(cursor, matching, opening_char, original_pos)
        if start_pos and end_pos:
            cursor.setPosition(start_pos)
            cursor.movePosition(cursor.Right, cursor.KeepAnchor, end_pos - start_pos)
            self._editor.setTextCursor(cursor)
...
class TextBlockHelper(object):
    ...
    def set_state(block, state):
        ...
        if user_state == -1:
            user_state = 0
        higher_part = user_state & 2147418112
        state &= 65535
        state |= higher_part
...
def get_block_symbol_data(editor, block):
    ...
    parentheses = sorted(list_symbols(editor, block, '(') + list_symbols(editor, block, ')'), key=lambda x: x.position)
    square_brackets = sorted(list_symbols(editor, block, '[') + list_symbols(editor, block, ']'), key=lambda x: x.position)
    braces = sorted(list_symbols(editor, block, '{') + list_symbols(editor, block, '}'), key=lambda x: x.position)
