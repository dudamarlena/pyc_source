...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/modes/filewatcher.py
...
class FileWatcherMode(Mode, QtCore.QObject):
    ...
    def _notify(self, title, message, expected_action=None):
        ...
        if self._auto_reload or QtWidgets.QMessageBox.question(self.editor, title, message, dlg_type, QtWidgets.QMessageBox.Yes) == QtWidgets.QMessageBox.Yes:
    ...
    def _notify_change(self):
        ...
        def inner_action(*args):
            ...
            Cache().set_cursor_position(self.editor.file.path, self.editor.textCursor().position())
        ...
        args = (_('File changed'),
         _('The file <i>%s</i> has changed externally.\nDo you want to reload it?') % os.path.basename(self.editor.file.path))
