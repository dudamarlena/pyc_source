...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/panels/folding.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 32014 bytes
"""
This module contains the marker panel
"""
import logging, os, sys
from pyqode.core.api import TextBlockHelper, folding, TextDecoration, DelayJobRunner
...
class FoldingPanel(Panel):
    ...
    def custom_indicators_icons(self, value):
        ...
        if self.editor:
            for clone in self.editor.clones:
                try:
                    clone.modes.get(self.__class__).custom_indicators_icons = value
                except KeyError:
                    pass
    ...
    def custom_fold_region_background(self, value):
        ...
        if self.editor:
            for clone in self.editor.clones:
                try:
                    clone.modes.get(self.__class__).custom_fold_region_background = value
                except KeyError:
                    pass
    ...
    def __init__(self, highlight_caret_scope=False):
        Panel.__init__(self)
        self._native = True
        self._custom_indicators = (':/pyqode-icons/rc/arrow_right_off.png', ':/pyqode-icons/rc/arrow_right_on.png',
                                   ':/pyqode-icons/rc/arrow_down_off.png', ':/pyqode-icons/rc/arrow_down_on.png')
    ...
    def _draw_rect(self, rect, painter):
        ...
        painter.drawLine(rect.topLeft() + QtCore.QPointF(1, 0), rect.topRight() - QtCore.QPointF(1, 0))
        painter.drawLine(rect.bottomLeft() + QtCore.QPointF(1, 0), rect.bottomRight() - QtCore.QPointF(1, 0))
        painter.drawLine(rect.topRight() + QtCore.QPointF(0, 1), rect.bottomRight() - QtCore.QPointF(0, 1))
        painter.drawLine(rect.topLeft() + QtCore.QPointF(0, 1), rect.bottomLeft() - QtCore.QPointF(0, 1))
    ...
    def _draw_fold_indicator(self, top, mouse_over, collapsed, painter):
        ...
        :param painter: QPainter
        """
        rect = QtCore.QRect(0, top, self.sizeHint().width(), self.sizeHint().height())
        if self._native:
            ...
            opt.rect = rect
            opt.state = QtWidgets.QStyle.State_Active | QtWidgets.QStyle.State_Item | QtWidgets.QStyle.State_Children
            if not collapsed:
                opt.state |= QtWidgets.QStyle.State_Open
            if mouse_over:
                opt.state |= QtWidgets.QStyle.State_MouseOver | QtWidgets.QStyle.State_Enabled | QtWidgets.QStyle.State_Selected
                opt.palette.setBrush(QtGui.QPalette.Window, self.palette().highlight())
            opt.rect.translate(-2, 0)
            self.style().drawPrimitive(QtWidgets.QStyle.PE_IndicatorBranch, opt, painter, self)
        else:
            index = 0
            if not collapsed:
                index = 2
    ...
    def _add_scope_deco(self, start, end, parent_start, parent_end, base_color, factor):
        """
        ...
        if start > 0:
            d = TextDecoration(self.editor.document(), start_line=parent_start, end_line=start)
        ...
        if end <= self.editor.document().blockCount():
            d = TextDecoration(self.editor.document(), start_line=end, end_line=parent_end + 1)
            d.set_full_width(True, clear=False)
            d.draw_order = 2
            d.set_background(color)
            self.editor.decorations.append(d)
    ...
    def _add_scope_decorations(self, block, start, end):
        ...
        if TextBlockHelper.is_fold_trigger(block):
            ...
            self._add_scope_deco(start, end + 1, parent_start, parent_end, base_color, factor + factor_step)
        else:
    ...
    def _highlight_surrounding_scopes(self, block):
        ...
        if self._current_scope is None or self._current_scope.get_range() != scope.get_range():
    ...
    def mouseMoveEvent(self, event):
        ...
        if line >= 0:
            block = FoldScope.find_parent_scope(self.editor.document().findBlockByNumber(line))
            if TextBlockHelper.is_fold_trigger(block):
                ...
                if self._mouse_over_line != block.blockNumber() and self._mouse_over_line is not None:
                    ...
                    self._mouse_over_line = block.blockNumber()
                    self._highlight_runner.request_job(self._highlight_surrounding_scopes, block)
    ...
    def toggle_fold_trigger(self, block):
        ...
        if region.collapsed:
            region.unfold()
            if self._mouse_over_line is not None:
                self._add_scope_decorations(region._trigger, *region.get_range())
        else:
            region.fold()
            self._clear_scope_decos()
    ...
    def refresh_decorations(self, force=False):
        ...
        """
        cursor = self.editor.textCursor()
        if self._prev_cursor is None or force or self._prev_cursor.blockNumber() != cursor.blockNumber():
            for deco in self._block_decos:
                self.editor.decorations.remove(deco)
