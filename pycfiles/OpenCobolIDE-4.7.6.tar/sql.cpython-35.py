...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/sql.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 26252 bytes
"""
    pygments.lexers.sql
    ~~~~~~~~~~~~~~~~~~~
...
def language_callback(lexer, match):
    ...
    if m is not None:
        ...
        if l:
            ...
            yield (
             match.start(3), String, match.group(3))
        else:
            yield (
             match.start(), String, match.group())
...
class PostgresLexer(PostgresBase, RegexLexer):
    ...
    tokens = {'root': [
              ...
              (
               '[0-9]+', Number.Integer),
              (
               "(E|U&)?'", String.Single, 'string'),
              (
               '(U&)?"', String.Name, 'quoted-ident'),
              (
               '(?s)(\\$[^$]*\\$)(.*?)(\\1)', language_callback),
              (
               '[a-z_]\\w*', Name),
              (
