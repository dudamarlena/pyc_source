...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/filesystem_treeview.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 30766 bytes
"""
This module contains the file system tree view.
"""
import sys, fnmatch, locale, logging, os, platform, shutil, subprocess
...
class FileSystemTreeView(QtWidgets.QTreeView):
    ...
    def set_root_path(self, path, hide_extra_columns=True):
        ...
        if sys.platform == 'win32' and os.path.splitunc(path)[0]:
            ...
            item = QtGui.QStandardItem(QtGui.QIcon.fromTheme('dialog-warning', QtGui.QIcon(':/pyqode-icons/rc/dialog-warning.png')), 'UNC pathnames not supported.')
        ...
        self._fs_model_source.setFilter(QtCore.QDir.Dirs | QtCore.QDir.Files | QtCore.QDir.NoDotAndDotDot | QtCore.QDir.Hidden)
    ...
    def _show_context_menu(self, point):
        if self.context_menu:
            self.about_to_show_context_menu.emit(FileSystemHelper(self).get_current_path())
    ...
    def select_path(self, path):
        if not self.isVisible():
            ...
            self.setCurrentIndex(self._fs_model_proxy.mapFromSource(self._fs_model_source.index(path)))
...
class FileSystemHelper:
    ...
    class _UrlListMimeData(QtCore.QMimeData):
        ...
        def formats(self):
            return [
             self.format(self.copy)]

        @classmethod
        def format(cls, copy=True):
            if copy:
                return 'text/tv-copy-url-list'
            return 'text/tv-cut-url-list'
    ...
    def paste_from_clipboard(self):
        ...
        if paste_operation is not None:
            self._paste(self._UrlListMimeData.list_from(mime, copy=paste_operation), to, copy=paste_operation)
    ...
    def _paste(self, sources, destination, copy):
        ...
        for src in sources:
            ...
            filename, status = QtWidgets.QInputDialog.getText(self.tree_view, _('Copy'), _('New name:'), QtWidgets.QLineEdit.Normal, original)
            if filename == '' or not status:
                return
            filename = filename + ext
            final_dest = os.path.join(destination, filename)
            if os.path.exists(final_dest):
                rep = QtWidgets.QMessageBox.question(self.tree_view, _('File exists'), _('File <%s> already exists. Do you want to erase it?') % final_dest, QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                ...
                if not perform_copy:
                    ...
                    try:
                        ...
                        QtWidgets.QMessageBox.warning(self.tree_view, _('Copy failed'), _('Failed to copy "%s" to "%s".\n\n%s' % (
                         src, destination, str(e))))
                        _logger().exception('failed to copy "%s" to "%s', src, destination)
                    else:
    ...
    def delete(self):
        ...
        """
        urls = self.selected_urls()
        rep = QtWidgets.QMessageBox.question(self.tree_view, _('Confirm delete'), _('Are you sure about deleting the selected files/directories?'), QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.Yes)
        if rep == QtWidgets.QMessageBox.Yes:
            ...
            for fn in urls:
                try:
                    ...
                    QtWidgets.QMessageBox.warning(self.tree_view, _('Delete failed'), _('Failed to remove "%s".\n\n%s') % (fn, str(e)))
    ...
    def get_current_path(self):
        ...
        """
        path = self.tree_view.fileInfo(self.tree_view.currentIndex()).filePath()
        if not path:
    ...
    def rename(self):
        ...
        new_name, status = QtWidgets.QInputDialog.getText(self.tree_view, _('Rename '), _('New name:'), QtWidgets.QLineEdit.Normal, name)
        if status:
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(self.tree_view, _('Rename failed'), _('Failed to rename "%s" into "%s".\n\n%s') % (src, dest, str(e)))
            else:
                if os.path.isdir(dest):
                    ...
                    new_files = [
                     dest]
                self.tree_view.file_renamed.emit(os.path.normpath(src), os.path.normpath(dest))
                renamed_files = []
    ...
    def create_directory(self):
        ...
        if status:
            ...
            for i in fatal_names:
                if i == name:
                    QtWidgets.QMessageBox.critical(self.tree_view, _('Error'), _('Wrong directory name'))
                    return
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(self.tree_view, _('Failed to create directory'), _('Failed to create directory: "%s".\n\n%s') % (dir_name, str(e)))
    ...
    def create_file(self):
        ...
        """
        src = self.get_current_path()
        name, status = QtWidgets.QInputDialog.getText(self.tree_view, _('Create new file'), _('File name:'), QtWidgets.QLineEdit.Normal, '')
        if status:
            fatal_names = [
             '.', '..', os.sep]
            for i in fatal_names:
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(self.tree_view, _('Failed to create new file'), _('Failed to create file: "%s".\n\n%s') % (path, str(e)))
            else:
...
class FileSystemContextMenu(QtWidgets.QMenu):
    ...
    def update_show_in_explorer_action(self):
        self.action_show_in_explorer.setText(_('Show in %s') % self.get_file_explorer_name())
    ...
    def get_linux_file_explorer(cls):
        if cls._explorer is None:
            ...
            if output:
                explorer = output.splitlines()[0].replace('.desktop', '').replace('-folder-handler', '').split('.')[(-1)].lower()
    ...
    def show_in_explorer(cls, path, parent):
        try:
            ...
            QtWidgets.QMessageBox.warning(parent, _('Open in explorer'), _('Failed to open file in explorer.\n\n%s') % str(e))