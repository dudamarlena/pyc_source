...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/encodings.py
    ...
    def _refresh_items(self):
        ...
        for i, encoding in enumerate(sorted(Cache().preferred_encodings)):
            ...
            try:
                ...
                if encoding == self._current_encoding or convert_to_codec_key(alias) == self._current_encoding:
...
class EncodingsMenu(QtWidgets.QMenu):
    ...
    def __init__(self, title=_('Encodings'), parent=None, selected_encoding=locale.getpreferredencoding()):
...
class EncodingsContextMenu(EncodingsMenu):
    ...
    def __init__(self, title='Encodings', parent=None, selected_encoding=locale.getpreferredencoding()):
        from pyqode.core.api import CodeEdit
        assert isinstance(parent, CodeEdit)
        super(EncodingsContextMenu, self).__init__(title, parent, selected_encoding)
    ...
    def _refresh(self):
        self._current_encoding = convert_to_codec_key(self.parent().file.encoding)
    ...
    def _reload(self):
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(self.parent(), _('Decoding error'), _('Failed to reload file with encoding %s') % self._encoding)
        else:
            try:
