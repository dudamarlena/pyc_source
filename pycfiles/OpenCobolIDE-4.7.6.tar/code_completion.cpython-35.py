...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/modes/code_completion.py
# Compiled at: 2016-12-29 07:29:18
# Size of source mod 2**32: 24627 bytes
"""
    ...
    def filterAcceptsRow(self, row, _):
        ...
        if len(self.prefix) == 1:
            try:
                ...
                self.sourceModel().setData(self.sourceModel().index(row, 0), rank, QtCore.Qt.UserRole)
                return prefix in completion
            except ValueError:
                return False

        for i, patterns in enumerate(zip(self.filter_patterns, self.filter_patterns_case_sensitive, self.sort_patterns)):
...
class SubsequenceCompleter(QtWidgets.QCompleter):
    ...
    def __init__(self, *args):
        ...
        self.filterProxyModel = SubsequenceSortFilterProxyModel(self.caseSensitivity(), parent=self)
    ...
    def update_model(self):
        if self.completionCount() or len(self.local_completion_prefix) <= 1 or self._force_next_update:
            self.filterProxyModel.set_prefix(self.local_completion_prefix)
            self.filterProxyModel.invalidate()
...
class CodeCompletionMode(Mode, QtCore.QObject):
    ...
    def completion_prefix(self):
        ...
        """
        return self._helper.word_under_cursor(select_whole_word=False).selectedText().strip()
    ...
    def _create_completer(self):
        ...
        self._completer.highlighted.connect(self._on_selected_completion_changed)
        ...
        def _handle_completer_events():
            ...
            if event.key() in [
             ...
             QtCore.Qt.Key_Tab]:
                ...
                if event.key() in [
                 QtCore.Qt.Key_Escape, QtCore.Qt.Key_Backtab] or nav_key and ctrl:
    ...
    def _on_key_released(self, event):
        ...
        word = self._helper.word_under_cursor(select_whole_word=True).selectedText()
        debug('word: %s' % word)
        if event.text():
            if event.key() == QtCore.Qt.Key_Escape:
                self._hide_popup()
                return
            if self._is_navigation_key(event) and (not self._is_popup_visible() or word == ''):
                self._reset_sync_data()
                return
            ...
            if event.text() in self._trigger_symbols:
                ...
                if len(word) >= self._trigger_len and event.text() not in self.editor.word_separators:
                    ...
                    self._reset_sync_data()
        elif self._is_navigation_key(event):
    ...
    def _on_results_available(self, results):
        debug('completion results (completions=%r), prefix=%s', results, self.completion_prefix)
        context = results[0]
        results = results[1:]
        ...
        if line == self._last_cursor_line and column == self._last_cursor_column:
            if self.editor:
                all_results = []
                for res in results:
                    all_results += res
    ...
    def request_completion(self):
        ...
        column = self._helper.current_column_nbr() - len(self.completion_prefix)
        same_context = line == self._last_cursor_line and column == self._last_cursor_column
        if same_context:
        ...
        data = {'code': self.editor.toPlainText(), 
         'line': line, 
         'column': column, 
        ...
        try:
            self.editor.backend.send_request(backend.CodeCompletionWorker, args=data, on_receive=self._on_results_available)
        except NotRunning:
    ...
    def _hide_popup(self):
        ...
        """
        debug('hide popup')
        if self._completer.popup() is not None and self._completer.popup().isVisible():
            self._completer.popup().hide()
            self._last_cursor_column = -1
            self._last_cursor_line = -1
    ...
    def _get_popup_rect(self):
        ...
        cursor_rec.translate(self.editor.panels.margin_size() - prefix_len, self.editor.panels.margin_size(0) + 5)
        popup = self._completer.popup()
        width = popup.verticalScrollBar().sizeHint().width()
        cursor_rec.setWidth(self._completer.popup().sizeHintForColumn(0) + width)
        return cursor_rec
    ...
    def _show_popup(self, index=0):
        ...
        :return:
        """
        full_prefix = self._helper.word_under_cursor(select_whole_word=False).selectedText()
        ...
        if full_prefix == selected and cnt == 1:
            ...
            if self.editor.isVisible():
                ...
                self._completer.popup().setCurrentIndex(self._completer.completionModel().index(index, 0))
                debug('popup shown: %r' % self._completer.popup().isVisible())
            else:
    ...
    def _show_completions(self, completions):
        ...
        elapsed = time.time() - t
        debug('completion model updated: %d items in %f seconds', self._completer.model().rowCount(), elapsed)
        self._show_popup()
    ...
    def _is_navigation_key(event):
        return event.key() == QtCore.Qt.Key_Backspace or event.key() == QtCore.Qt.Key_Back or event.key() == QtCore.Qt.Key_Delete or event.key() == QtCore.Qt.Key_End or event.key() == QtCore.Qt.Key_Home or event.key() == QtCore.Qt.Key_Left or event.key() == QtCore.Qt.Key_Right or event.key() == QtCore.Qt.Key_Up or event.key() == QtCore.Qt.Key_Down or event.key() == QtCore.Qt.Key_Space