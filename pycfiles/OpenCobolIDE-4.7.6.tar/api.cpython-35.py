...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-4yaip7h6/qcrash/qcrash/api.py
...
def install_except_hook(except_hook=_hooks.except_hook):
    ...
    global _except_hook
    if not _backends:
        raise ValueError('no backends found, you must at least install one backend before calling this function')
...
def show_report_dialog(window_title='Report an issue...', window_icon=None, traceback=None, issue_title='', issue_description='', parent=None, modal=None, include_log=True, include_sys_info=True):
    """
    ...
    dlg = DlgReport(_backends, window_title=window_title, window_icon=window_icon, traceback=traceback, issue_title=issue_title, issue_description=issue_description, parent=parent, include_log=include_log, include_sys_info=include_sys_info)
    if modal:
        dlg.show()
        return dlg
    dlg.exec_()
