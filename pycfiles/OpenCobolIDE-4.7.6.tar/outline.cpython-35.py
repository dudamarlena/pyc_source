...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/outline.py
...
class OutlineTreeWidget(QtWidgets.QTreeWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._action_sync = QtWidgets.QAction(QtGui.QIcon.fromTheme('view-refresh'), _('Sync with editor'), None)
    ...
    def set_editor(self, editor):
        ...
        try:
            self._folding_panel.trigger_state_changed.disconnect(self._on_block_state_changed)
        except (AttributeError, TypeError, RuntimeError, ReferenceError):
            pass
        ...
        if editor is not None:
            ...
            try:
                self._folding_panel = weakref.proxy(editor.panels.get(FoldingPanel))
            except KeyError:
                pass
            else:
                self._folding_panel.trigger_state_changed.connect(self._on_block_state_changed)
            try:
    ...
    def to_tree_widget_items(self, definitions, to_collapse=None):
        ...
        def convert(name, editor, to_collapse):
            ...
            if isinstance(name.icon, list):
                icon = QtGui.QIcon.fromTheme(name.icon[0], QtGui.QIcon(name.icon[1]))
            else:
            ...
            if to_collapse is not None and TextBlockHelper.is_collapsed(name.block):
    ...
    def sync(self):
        if not self.sync_with_editor or self._editor is None or not self._definitions:
            return
        to_select = None
        previous = None
