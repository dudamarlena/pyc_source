...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/tabs.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 16208 bytes
"""
...
class TabWidget(QTabWidget):
    ...
    def _ensure_unique_name(self, code_edit, name):
        ...
        if self._name_exists(file_name):
            file_name = self._rename_duplicate_tabs(code_edit, code_edit.file.name, code_edit.file.path)
    ...
    def save_current(self, path=None):
        ...
        try:
            if not path and not self._current.file.path:
                path, filter = QtWidgets.QFileDialog.getSaveFileName(self, _('Choose destination path'))
                ...
                if path and old_path != path:
                    ...
                    if ext != old_ext or not old_path:
                        icon = QtWidgets.QFileIconProvider().icon(QtCore.QFileInfo(code_edit.file.path))
    ...
    def _save_editor(self, code_edit, path=None):
        if not path:
            ...
            if not os.path.exists(path):
                path, status = QtWidgets.QFileDialog.getSaveFileName(self, _('Save as (%s)') % code_edit.file.path)
            if path:
                try:
    ...
    def _rename_duplicate_tabs(self, current, name, path):
        ...
        for i in range(self.count()):
            if self.widget(i)._tab_name == name and self.widget(i) != current:
                ...
                if file_path:
                    parent_dir = os.path.split(os.path.abspath(os.path.join(file_path, os.pardir)))[1]
        ...
        if path:
            parent_dir = os.path.split(os.path.abspath(os.path.join(path, os.pardir)))[1]
    ...
    def _on_tab_close_requested(self, index):
        ...
        try:
            if not widget.dirty:
                ...
                dlg = DlgUnsavedFiles(self, files=[
                 widget.file.path if widget.file.path else widget._tab_name])
