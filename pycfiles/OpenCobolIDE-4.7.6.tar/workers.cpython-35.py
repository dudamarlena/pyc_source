...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/backend/workers.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 8151 bytes
"""
...
class CodeCompletionWorker(object):
    ...
    def __call__(self, data):
        ...
        for prov in CodeCompletionWorker.providers:
            try:
                results = prov.complete(code, line, column, path, encoding, prefix)
        ...
        return [
         (
          line, column, req_id)] + completions
...
def finditer_noregex(string, sub, whole_word):
    ...
    while True:
        ...
        if whole_word:
            ...
            if nchar in DocumentWordsProvider.separators and pchar in DocumentWordsProvider.separators:
                yield start
            start += len(sub)
        else:
            yield start
