...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.cobol/pyqode/cobol/api/parsers/names.py
# Compiled at: 2016-12-29 07:22:52
...
class Name(object):
    ...
    def __repr__(self):
        type_names = {self.Type.Root: 'Root', 
...
def cmp_name(first_node, second_node):
    ...
    if len(first_node.children) == len(second_node.children):
        for first_child, second_child in zip(first_node.children, second_node.children):
            for key in first_child.__dict__.keys():
                if key.startswith('_'):
                    pass
                elif first_child.__dict__[key] != second_child.__dict__[key]:
                    return 1
...
def parse_pic_field(l, c, last_section_node, last_vars, line):
    ...
    if name in ALL_KEYWORDS or name in ('-', '/'):
        return
    m = re.findall('pic.*\\.', line, re.IGNORECASE)
    if m:
        ...
        if not free_format:
            ...
            if not line.isspace() and not line.strip().startswith('*'):
                ...
                if regex.DIVISION.indexIn(line.upper()) != -1 and 'EXIT' not in line.upper():
                    ...
                    last_div_node, last_section_node = parse_division(i, column, line, root_node, last_section_node)
                    ...
                    last_section_node = parse_section(i, column, last_div_node, last_vars, line)
        else:
            if last_div_node is not None and 'PROCEDURE DIVISION' in last_div_node.name.upper():
                tokens = line.upper().split(' ')
                if len(tokens) == 1 and tokens[0] not in ALL_KEYWORDS:
                    p = parse_paragraph(i, column, last_div_node, last_section_node, line)
                    if p:
                        paragraphs.append(p)
                        if last_par:
                            last_par.end_line = i
                ...
                v = parse_pic_field(i, column, last_section_node, last_vars, line)
                if v:
                    variables.append(v)
