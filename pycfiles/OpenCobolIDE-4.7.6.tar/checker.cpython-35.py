...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/panels/checker.py
...
class CheckerPanel(Panel):
    __doc__ = ' Shows messages collected by one or more checker modes '
    ...
    def __init__(self):
        ...
        self.info_icon = icons.icon('dialog-info', ':pyqode-icons/rc/dialog-info.png', 'fa.info-circle', qta_options={'color': '#4040DD'})
        self.warning_icon = icons.icon('dialog-warning', ':pyqode-icons/rc/dialog-warning.png', 'fa.exclamation-triangle', qta_options={'color': '#DDDD40'})
        self.error_icon = icons.icon('dialog-error', ':pyqode-icons/rc/dialog-error.png', 'fa.exclamation-circle', qta_options={'color': '#DD4040'})
    ...
    def _icon_from_message(self, message):
        icons = {CheckerMessages.INFO: self.info_icon, 
        ...
        if line:
            ...
            if len(markers):
                if self._previous_line != line:
                    top = TextHelper(self.editor).line_pos_from_number(markers[0].line)
                    if top:
