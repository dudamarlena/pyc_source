...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexers/qvt.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 5876 bytes
"""
    pygments.lexers.qvt
    ~~~~~~~~~~~~~~~~~~~
...
class QVToLexer(RegexLexer):
    ...
    tokens = {'root': [
              ...
              (
               '(metamodel|class|exception|primitive|enum|transformation|library)(\\s+)([a-zA-Z0-9_]+)',
               bygroups(Keyword.Word, Text, Name.Class)),
              (
               '(exception)(\\s+)([a-zA-Z0-9_]+)', bygroups(Keyword.Word, Text, Name.Exception)),
              (
               '(main)\\b', Name.Function),
              (
               '(mapping|helper|query)(\\s+)', bygroups(Keyword.Declaration, Text), 'operation'),
              (
     ...
     'operation': [
                   (
                    '::', Text),
                   (
                    '(.*::)([a-zA-Z_][a-zA-Z0-9_]*)[ \\t]*(\\()', bygroups(Text, Name.Function, Text), '#pop')], 
     ...
     'keywords': [
                  (
                   '(abstract|access|any|assert|blackbox|break|case|collect|collectNested|collectOne|collectselect|collectselectOne|composes|compute|configuration|constructor|continue|datatype|default|derived|disjuncts|do|elif|else|end|endif|except|exists|extends|forAll|forEach|forOne|from|if|implies|in|inherits|init|inout|intermediate|invresolve|invresolveIn|invresolveone|invresolveoneIn|isUnique|iterate|late|let|literal|log|map|merges|modeltype|new|object|one|ordered|out|package|population|property|raise|readonly|references|refines|reject|resolve|resolveIn|resolveone|resolveoneIn|return|select|selectOne|sortedBy|static|switch|tag|then|try|typedef|unlimited|uses|when|where|while|with|xcollect|xmap|xselect)\\b',
                   Keyword)], 
