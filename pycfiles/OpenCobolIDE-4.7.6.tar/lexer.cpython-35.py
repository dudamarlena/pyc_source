...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-build-jqog4noo/pygments/pygments/lexer.py
# Compiled at: 2016-12-29 05:31:34
# Size of source mod 2**32: 31040 bytes
"""
    pygments.lexer
    ~~~~~~~~~~~~~~
...
class RegexLexerMeta(LexerMeta):
    ...
    def _process_token(cls, token):
        ...
        if not type(token) is _TokenType:
            assert callable(token), 'token type must be simple type or callable, not %r' % (token,)
            return token
        ...
        if isinstance(new_state, tuple):
            for istate in new_state:
                if not istate in unprocessed:
                    if not istate in ('#pop', '#push'):
                        raise AssertionError('unknown new state ' + istate)
