...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/panels/search_and_replace.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 24146 bytes
"""
...
class SearchAndReplacePanel(Panel, Ui_SearchPanel):
    ...
    def __init__(self):
        ...
        self._init_actions()
        self._init_style()
        self.checkBoxRegex.stateChanged.connect(self.checkBoxWholeWords.setDisabled)
    ...
    def on_state_changed(self, state):
        ...
        if state:
            ...
            self.checkBoxWholeWords.stateChanged.disconnect(self.request_search)
            self.checkBoxRegex.stateChanged.disconnect(self.request_search)
            self.checkBoxInSelection.stateChanged.disconnect(self.request_search)
    ...
    def _show_error(self, error):
        if error:
            self._set_widget_background_color(self.lineEditSearch, QtGui.QColor('#FFCCCC'))
            self.lineEditSearch.setToolTip(str(error))
        else:
            self._set_widget_background_color(self.lineEditSearch, self.palette().color(self.palette().Base))
    ...
    def select_next(self):
        ...
        if cursor_pos not in range(current[0], current[1] + 1) or current_occurence == -1:
            current_occurence = 0
            ...
            if current_occurence == -1 or current_occurence >= len(occurrences) - 1:
                current_occurence = 0
            else:
                current_occurence += 1
        ...
        try:
            ...
            cursor.setPosition(occurrences[current_occurence][1], cursor.KeepAnchor)
    ...
    def _on_results_available(self, results):
        self._occurrences = [(start + self._offset, end + self._offset) for start, end in results]
        self._on_search_finished()
