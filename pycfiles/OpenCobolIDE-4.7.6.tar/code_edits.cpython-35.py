...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/code_edits.py
...
from pyqode.core.api import CodeEdit, Panel, SyntaxHighlighter, CharBasedFoldDetector, IndentFoldDetector, ColorScheme
...
class TextCodeEdit(CodeEdit):
    ...
    mimetypes = [
     'text/x-plain', 'text/x-log', 'text/plain']
    DEFAULT_SERVER = server.__file__

    def __init__(self, parent=None, server_script=None, interpreter=sys.executable, args=None, create_default_actions=True, color_scheme='qt', reuse_backend=False):
        from pyqode.core import panels
        from pyqode.core import modes
        if server_script is None:
            server_script = TextCodeEdit.DEFAULT_SERVER
        super(TextCodeEdit, self).__init__(parent, create_default_actions)
        self.backend.start(server_script, interpreter, args, reuse=reuse_backend)
        self.panels.append(panels.SearchAndReplacePanel(), Panel.Position.BOTTOM)
        ...
        self.modes.append(TextCodeEdit.TextSH(self.document(), ColorScheme(color_scheme)))
    ...
    def clone(self):
        clone = self.__class__(parent=self.parent(), server_script=self.backend.server_script, interpreter=self.backend.interpreter, args=self.backend.args, color_scheme=self.syntax_highlighter.color_scheme.name)
        return clone
...
class GenericCodeEdit(CodeEdit):
    ...
    def __init__(self, parent=None, server_script=None, interpreter=sys.executable, args=None, create_default_actions=True, color_scheme='qt', reuse_backend=False):
        ...
        self.modes.append(modes.CaretLineHighlighterMode())
        self.modes.append(modes.RightMarginMode())
        self.modes.append(modes.PygmentsSyntaxHighlighter(self.document(), color_scheme=ColorScheme(color_scheme)))
        self.modes.append(modes.ZoomMode())
        self.modes.append(modes.CodeCompletionMode())
        self.modes.append(modes.AutoIndentMode())
        self.modes.append(modes.IndenterMode())
