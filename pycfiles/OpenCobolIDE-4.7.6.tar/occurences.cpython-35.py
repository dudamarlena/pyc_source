...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/modes/occurences.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 6453 bytes
"""
...
class OccurrencesHighlighterMode(Mode):
    __doc__ = ' Highlights occurrences of the word under the text text cursor.\n\n    The ``delay`` before searching for occurrences is configurable.\n    '
    ...
    def _request_highlight(self):
        if self.editor is not None:
            sub = TextHelper(self.editor).word_under_cursor(select_whole_word=True).selectedText()
            if sub != self._sub:
                self._clear_decos()
                if len(sub) > 1:
    ...
    def _send_request(self):
        ...
        if not cursor.hasSelection() or cursor.selectedText() == self._sub:
            request_data = {'string': self.editor.toPlainText(), 
             'sub': self._sub, 
             'regex': False, 
             'whole_word': True, 
        ...
        if len(results) > 1:
            for start, end in results:
                if start <= current <= end:
                    ...
                    deco = TextDecoration(self.editor.textCursor(), start_pos=start, end_pos=end)
                    if self.underlined:
