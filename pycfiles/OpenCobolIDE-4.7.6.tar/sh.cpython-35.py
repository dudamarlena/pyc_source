...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.cobol/pyqode/cobol/modes/sh.py
# Compiled at: 2016-12-30 06:32:41
...
def make_cobol_patterns(fixed_format=True):
    if fixed_format:
        ...
        comment = any('comment', ['\\*>[^\\n]*'])
    keywords_reserved = any('keyword_reserved', [
     '(^|(?<=[^0-9a-zA-Z_\\-]))(%s)\\s*($|(?=[^0-9a-zA-Z_\\-]))' % '|'.join(kw.RESERVED + kw.SQL_COBOL_KEYWORDS + kw.MNEMONICS)])
    constants = any('constant', [
     '(^|(?<=[^0-9a-zA-Z_\\-]))(^|(?<=[^0-9a-zA-Z_\\-]))(equal|equals|ne|lt|le|gt|ge|greater|less|than|not|and|or)\\s*($|(?=[^0-9a-zA-Z_\\-]))'])
    types = any('type', [
     '(PIC\\s+.+?(?=(\\s|\\.\\s?))|PICTURE\\s+.+?(?=(\\s|\\.\\s?))|(COMPUTATIONAL)(-[1-5X])?|(COMP)(-[1-5X])?|BINARY-C-LONG|BINARY-CHAR|BINARY-DOUBLE|BINARY-LONG|BINARY-SHORT|BINARY)\\s*($|(?=[^0-9a-zA-Z_\\-]))'])
    punctuation = any('punctuation', ['([(),;:&%.])'])
    name_builtin = any('builtin', [
