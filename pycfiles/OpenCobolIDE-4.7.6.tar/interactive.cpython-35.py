...
# [GCC 8.4.0]
# Embedded file name: /home/colin/Projects/pyqode.core/pyqode/core/widgets/interactive.py
# Compiled at: 2016-12-29 05:31:31
# Size of source mod 2**32: 15546 bytes
"""
...
class InteractiveConsole(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.panels.append(SearchAndReplacePanel(), SearchAndReplacePanel.Position.TOP)
    ...
    def _write_started(self):
        if not self._write_app_messages:
            return
        self._writer(self, '{0} {1}\n'.format(self._process_name, ' '.join(self._args)), self._app_msg_col)
        self._running = True
    ...
    def _write_error(self, error):
        if self is None:
            return
        if self._user_stop:
            self._writer(self, '\nProcess stopped by the user', self.app_msg_color)
        else:
