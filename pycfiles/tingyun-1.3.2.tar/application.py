...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/battlefield/application.py
...
class Application(object):
    ...
    def activate_session(self):
        ...
        connect_thread = threading.Thread(target=self.connect_to_headquarters, name='TingYun-session-thread-%s' % int(time.time()))
        ...
        if self._active_session:
            ...
            while not self._active_session:
                ...
                try:
                    ...
                    console.warning('Invalid license in configuration, agent will stop to work please fix license andrestart agent again')
                    break
                except Exception:
    ...
    def harvest(self, last_harvest, current_harvest, shutdown=False):
        ...
        if not self._active_session:
            ...
            return (
             CONSTANCE_SESSION_NOT_ACTIVED, self._app_name, self._linked_app)
        else:
            if not self._is_license_valid:
                console.debug('The license is invalid, skip harvest data.')
                return (
                 CONSTANCE_LICENSE_INVALID, self._app_name, self._linked_app)
            with self._packets_lock:
                self._tracker_count = 0
                self._last_tracker = 0.0
            ...
            try:
                ...
                console.debug('This exception indicates server service can not touched. if this error continues. please report to us for further investigation. thank u')
            except Exception as err:
                ret_code = CONSTANCE_HARVEST_ERROR
                console.exception('Some error occurred in agent code. if this error continues. please report to us for further investigation. thank u.')
    ...
    def record_tracker(self, tracker):
        ...
        try:
            ...
            console.exception("Unexpected error occurred when record tracker data, that's maybe the agent code issue if this continues, please report to us for further investigation. %s", err)
    ...
    def get_performance_metric(self, stat, last_harvest, current_harvest, audit_mode=False):
        ...
        performance = {'type': 'perfMetrics', 'timeFrom': int(last_harvest), 
    ...
    def get_error_trace_data(self, stat):
        ...
        :return:
        """
        error_trace = {'type': 'errorTraceData', 
    ...
    def get_external_error_trace(self, stat):
        ...
        error_trace = {'type': 'externalErrorTraceData', 
           'errors': error_data}
        if 0 == len(error_data):
            error_trace = None
        return error_trace
    ...
    def cmd_StartProfiler(self, cid, args):
        ...
        if not hasattr(sys, '_current_frames'):
            console.warning('The current Python interpreter being used is not support thread profiling. For more help about the thread profiling, please contact our support.')
            return
        ...
        if not self.profile_status:
            ...
            return
        console.info('Starting thread profile for application %s, with duration %ss,  interval %sms', self._app_name, duration, interval)
    ...
    def cmd_StopProfiler(self, cid, args):
        ...
        if profile is None:
            console.warning('Get stop profile commands, but threading profiling is not running. if this continues, please report to us for more investigation.')
        elif profile_id != profile.profile_id:
            self._active_session.send_profile_data({'id': cmd_id, 'result': {}})
            console.warning('Get stop profile commands, but the specified profile id[%s] not match currentprofile id[%s]. This command will be ignored.', profile_id, profile.profile_id)
            return
