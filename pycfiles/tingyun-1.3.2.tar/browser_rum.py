...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/armoury/trigger/browser_rum.py
...
body_re = re.compile('<body[^>]*>', re.IGNORECASE)
xua_meta_re = re.compile('<\\s*meta[^>]+http-equiv\\s*=\\s*[\'"]\\s*x-ua-compatible\\s*[\'"][^>]*>', re.IGNORECASE)
charset_meta_re = re.compile('<\\s*meta[^>]+charset\\s*=\\s*[\'"][^>]*[\'"][^>]*>', re.IGNORECASE)
attachment_re = re.compile('<\\s*meta[^>]+http-equiv\\s*=\\s*[\'"]\\s*content-disposition\\s*[\'"][^>]*content\\s*=\\s*(?P<quote>[\'"])\\s*attachment(\\s*;[^>]*)?(?P=quote)[^>]*>', re.IGNORECASE)
    ...
    def insert_js_to_html(self, data, agent_js, search_size=65536):
        ...
        if not body:
            ...
            def combine_data(position):
                return ('').join((data[:position], agent_js, data[position:]))
