...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/logistics/warehouse/tracker_node.py
...
class TrackerNode(_TrackerNode):
    ...
    def apdex_metrics(self):
        ...
        if self.errors:
            frustrating = 1
        elif self.duration <= self.apdex_t:
            satisfying = 1
        ...
        yield ApdexMetric(name=name, satisfying=satisfying, tolerating=tolerating, frustrating=frustrating, apdex_t=self.apdex_t)
    ...
    def traced_error(self):
        ...
        for error in self.errors:
            error_item = [
             error.error_time, self.path, error.http_status or 500, error.error_class_name, error.message,
             1, self.request_uri]
            stack_detail = []
            ...
            error_params = {'params': {'threadName': error.thread_name, 'httpStatus': error.http_status, 'referer': error.referer}, 'requestParams': error.request_params, 
    ...
    def slow_action_trace(self, limit, threshold):
        ...
        slow_trace = [start_time, self.request_params,
         {'httpStatus': self.http_status, 'threadName': self.thread_name, 'referer': self.referer}, trace_node]
        return [
         start_time, duration, self.path, self.request_uri, str(slow_trace),
         trace_id.encode('utf8'), trace_guid.encode('utf8')]
    ...
    def traced_external_error(self):
        ...
        for error in self.external_error:
            ...
            error_item = [error.error_time, metric_name.encode('utf8'), error.status_code,
             error.error_class_name, 1, self.path.encode('utf8')]
            stack_detail = []
            for line in error.stack_trace:
                if len(line) >= 4 and 'tingyun' not in line[0]:
                    stack_detail.append('%s(%s:%s)' % (line[2], line[0], line[1]))

            error_params = {'params': {'threadName': error.thread_name, 'httpStatus': error.http_status or 500}, 'requestParams': error.request_params, 'stacktrace': stack_detail}
            error_item.append(str(error_params).encode('utf8'))
            error_filter_key = '%s_|%s_|%s' % (metric_name, error.status_code, error.error_class_name)
            yield TracedExternalError(error_filter_key=error_filter_key, trace_data=error_item, tracker_type=error.tracker_type, status_code=error.status_code)