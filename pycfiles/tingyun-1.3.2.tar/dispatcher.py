...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/battlefield/dispatcher.py
...
"""
import atexit, sys, os, time, logging, threading
...
class Dispatcher(object):
    ...
    def _do_harvest(self, last_harvest, current_harvest, shutdown=False):
        ...
        for name, application in six.iteritems(self._applications):
            if not application.application_config or not application.application_config.enabled:
                return
            try:
    ...
    def active_dispatcher(self):
        ...
        try:
            ...
            console.fatal('This exception indicates maybe internal agent python code error. please report to us for further investigation. thank u.')
        ...
        if not self._config.enabled:
            ...
            try:
                if not app:
                    ...
                    self._connect_status.append(app_name)
                    active_app = True
            except Exception as err:
    ...
    def record_tracker(self, app_name, tracker_node):
        ...
        if application is None:
            console.warning('Application %s not exist, if this is not expected, please contact us for help.', app_name)
            return
        else:
