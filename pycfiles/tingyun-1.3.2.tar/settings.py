...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/config/settings.py
# Compiled at: 2016-08-01 02:23:59
import copy, logging, os, json, re
...
class Naming(Settings):
    ...
    def __init__(self, rules=None):
        self.rules = (isinstance(rules, list) or json.loads)(rules) if 1 else rules
        self.cached = {}
        self.regex_url = {}
        self.loaded = False
        self.method = {'0': 'GET|POST|PUT|DELETE|HEAD', '1': 'GET', '2': 'POST', '3': 'PUT', '4': 'DELETE', '5': 'HEAD'}
        self.match_type = {'1': self.match_equal, '2': self.match_start_with, '3': self.match_end_with, '4': self.match_include, 
           '5': self.match_regex, '0': self.any_value}
            ...
            for rule in self.rules:
                ...
                for params in rule['match']['params']:
                    if not isinstance(params, dict):
                        ...
                        matched_param = match_func(src_value, params['value']) or False
                        break
...
class TransactionTraceSettings(Settings):
    pass
...
_settings.transaction_tracer = TransactionTraceSettings()
_settings.naming = Naming([])
_settings.x_tingyun_id = '%s;c=1;x=%s'
...
_settings.verify_certification = False
...
_settings.stack_trace_count = 30
_settings.explain_plan_count = 30
...
_settings.action_tracer.action_threshold = 2000
_settings.action_tracer.top_n = 12
_settings.action_tracer.stack_trace_threshold = 500
...
_settings.action_tracer.explain_threshold = 500
...
_settings.log_level_mapping = {'CRITICAL': logging.CRITICAL, 
   'ERROR': logging.ERROR, 
...
def get_upload_settings():
    ...
    :return: settings
    """
    dump_settings = {'nbs.license_key': _settings.license_key, 
...
def apply_config_setting(settings_object, name, value):
    ...
    if fields[0] == 'urls_captured' and value:
        try:
            value = [ re.compile('%s' % v) for v in value.split('\n') ]
        except Exception as err:
