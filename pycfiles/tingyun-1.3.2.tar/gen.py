...
# [GCC 8.4.0]
# Embedded file name: /home/nb/publish/btw/tingyun/tingyun/armoury/ammunition/tornado_4/gen.py
...
def trace_runner_init(wrapped, instance, args, kwargs):
    ...
    if tracker is None:
        ...
        if '__name__' in frame.f_globals and frame.f_globals['__name__'] == 'tornado.gen' and 'func' in frame.f_locals and 'replace_callback' in frame.f_locals and frame.f_code.co_name == 'wrapper':
...
def trace_runner_run(wrapped, instance, args, kwargs):
    ...
    if tracker is None:
        ...
        if result is None:
            result = NoneProxy()
        if hasattr(instance, '_self_coroutine_name') and instance._self_coroutine_name is not None:
