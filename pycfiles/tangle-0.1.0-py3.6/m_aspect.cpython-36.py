...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/tangle/m_aspect.py
# Compiled at: 2017-08-24 10:01:20
# Size of source mod 2**32: 6433 bytes
import inspect, re, six, types
...
class Pointcut(object):
        ...
        class SubPointcut(Pointcut):
            ...
            def match_inst(self, inst):
                if not class_pattern:
                    ...
                    class_regex = re.sub('\\.', '\\.', class_pattern)
                    class_regex = re.sub('\\*', '.*', class_regex)
                    klass = type(inst)
            ...
            def match_method(self, join_point):
                if not method_pattern:
                    ...
                    method_regex = re.sub('\\*', '.*', method_pattern)
    ...
    def _create_operation_binary(self, other, match_operation):
        ...
        class PointcutOperation(Pointcut):
            ...
            def match(self, join_point):
                return match_operation(inst.match(join_point), other.match(join_point))
...
class Advise(object):
    ...
    def advise(self, join_point):
        if self.pointcut.match(join_point):
            ...
            advise = {Advise.type_before: 'before_advises', 
             Advise.type_after: 'after_advises', 
             Advise.type_after_return: 'return_advises', 
             Advise.type_after_error: 'error_advises'}.get(self.advise_type)
