...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\samplify\timeseries.py
# Compiled at: 2016-04-24 20:32:12
import time, zlib, bz2, struct, base64, datetime
from .si import *
...
class Timeseries(object):
    ...
    def binarize_timestamps(self):
        return ('').join([ struct.pack('!d', epoch_secs(x)) for x in self.get_timestamps()
                         ])
    ...
    def debinarize(data):
        ...
        for x in xrange(data_len):
            measurements.append(Measure.from_binary(measurement_bin[x * Measure.BIN_STRUCT.size:(x + 1) * Measure.BIN_STRUCT.size]))
