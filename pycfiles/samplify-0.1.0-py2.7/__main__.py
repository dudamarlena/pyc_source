...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\samplify\__main__.py
# Compiled at: 2016-04-24 20:32:11
import bz2, zlib, random, time
from . import *
from si import *
...
def examples():
    ...
    from_a_to_b = from_string.timeslice(min(pt_a[0], pt_b[0]), max(pt_a[0], pt_b[0]))
    print ('There are {} datapoints between {} and {}.').format(from_a_to_b.length, min(pt_a[0], pt_b[0]), max(pt_a[0], pt_b[0]))
    pt_a = random.choice(from_string)
    pt_b = random.choice(from_string)
    from_a_to_b = from_string.valueslice(min(pt_a[1], pt_b[1]), max(pt_a[1], pt_b[1]))
    print ('There are {} datapoints between {} and {}.').format(from_a_to_b.length, min(pt_a[1], pt_b[1]), max(pt_a[1], pt_b[1]))
