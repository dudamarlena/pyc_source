...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/docbucket/views.py
# Compiled at: 2010-12-14 16:13:37
import os
...
def home(request):
    ...
    return render_to_response('docbucket/home.html', cntx, context_instance=RequestContext(request))
...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            writer = ix.writer()
            writer.add_document(name=name, content=('\n').join(outputs.values()).decode('utf-8'), doc_id=unicode(doc.id))
            writer.commit()
            for f in pages:
    ...
    return render_to_response('docbucket/create.html', {'form': form, 'empty': empty}, context_instance=RequestContext(request))
...
def list(request, doc_class=None):
    ...
    cntx = {'documents': documents.object_list, 'paginator': documents, 'classes': classes}
    return render_to_response('docbucket/list.html', cntx, context_instance=RequestContext(request))
...
def manage(request):
    ...
    return render_to_response('docbucket/manage.html', cntx, context_instance=RequestContext(request))
...
def show(request, doc_id):
    ...
    doc.save()
    return render_to_response('docbucket/show.html', {'doc': doc}, context_instance=RequestContext(request))
...
def search(request):
    ...
    if query is not None and query != '':
        ...
        parser = MultifieldParser(('name', 'content', 'doc_id'), schema=ix.schema)
        try:
    ...
    cntx = {'documents': found_docs, 'search': found, 'query': query, 'page_results': len(found_docs), 
       'next_page': page + 1, 'previous_page': page - 1, 
       'page': page, 'max_pages': found.pagecount}
    return render_to_response('docbucket/search.html', cntx, context_instance=RequestContext(request))
