...
# [GCC 8.4.0]
# Embedded file name: /Users/tmp/Documents/CLEARumor/src/util.py
# Compiled at: 2019-10-01 16:33:29
# Size of source mod 2**32: 14083 bytes
import json
...
from src.dataset import ELMO_OPTIONS_FILE, ELMO_WEIGHTS_FILE, Post, SdqcInstance, VerifInstance
...
class DatasetHelper(Dataset):
 ...
 np.ndarray, np.ndarray, np.ndarray):
        ...
        if post.platform == Post.Platform.twitter:
            post_author = [
             ...
             post.friends_count,
             post.followers_count / (post.friends_count + 1e-08)]
        ...
        if not post.has_source_depth:
            ...
            post_similarity_to_source = F.cosine_similarity(post_emb_mean,
              source_emb_mean, dim=0).cpu().numpy()
    ...
    def calc_stats_for_aux_feature(self, index: int, filter_func: Optional[Callable[([str], bool)]]=None) -> (
 float, float, float, float):
        ...
        feature_values = np.array([post['features'][index].item() for post in self._dataset if filter_func(post['post_id'])])
        return (feature_values.min(),
         feature_values.max(),
         feature_values.mean(),
         feature_values.std())

    def min_max_scale_aux_feature(self, index: int, min: float, max: float, filter_func: Optional[Callable[([str], bool)]]=None) -> None:
        if not filter_func:
    ...
    def standard_scale_aux_feature(self, index: int, mean: float, std: float, filter_func: Optional[Callable[([str], bool)]]=None) -> None:
        if not filter_func:
...
def calculate_post_elmo_embeddings(posts: Dict[(str, Post)], max_sentence_length: int, batch_size: int, scalar_mix_parameters: List[float], device: torch.device, elmo_options_file: Path=ELMO_OPTIONS_FILE, elmo_weights_file: Path=ELMO_WEIGHTS_FILE) -> Dict[(str, torch.Tensor)]:
    ...
    time_before = time()
    elmo = Elmo(elmo_options_file, elmo_weights_file,
...
def generate_folds_for_k_fold_cross_validation(posts: Dict[(str, Post)], num_folds: int) -> List[Set[str]]:
    posts_per_discriminator = defaultdict(set)
    for post in posts.values():
        if post.platform == Post.Platform.twitter:
            discriminator = post.topic
        else:
            if post.platform == Post.Platform.reddit:
...
def arrange_folds_for_k_fold_cross_validation(folds: List[Set[str]], index: int) -> (
 Set[str], Set[str]):
    train_post_ids = set(chain.from_iterable((fold for i, fold in enumerate(folds) if i != index)))
    test_post_ids = folds[index]
    return (train_post_ids, test_post_ids)


def filter_instances(train_post_ids: Set[str], test_post_ids: Set[str], instances: Iterable[Union[(SdqcInstance, VerifInstance)]]) -> (
...
def rmse_score(labels, predictions, confidences):
    ...
    for label, prediction, confidence in zip(labels, predictions, confidences):
        if not label == prediction or label == VerifInstance.Label.true.value or label == VerifInstance.Label.false.value:
            rmse += (1 - confidence) ** 2
...
def display_results(sdqc_accs: Iterable[float], sdqc_f1s: Iterable[float], sdqc_reports: Iterable[Dict[(str, Dict[(str, float)])]], verif_accs: Iterable[float], verif_f1s: Iterable[float], verif_rmses: Iterable[float], verif_reports: Iterable[Dict[(str, Dict[(str, float)])]]):
    ...
    def display_report(reports: Iterable[Dict[(str, Dict[(str, float)])]]):
        ...
        for outer_key, inner_report in report_lists.items():
            ...
            for inner_key, values in inner_report.items():
                report_stats[outer_key][inner_key] = '{:.1%}±{:.1%}'.format(np.mean(values), np.std(values))

        pprint(report_stats)

    sdqc_acc = (
     np.mean(sdqc_accs), np.std(sdqc_accs))
    sdqc_f1 = (np.mean(sdqc_f1s), np.std(sdqc_f1s))
    print('Task A: SDQC')
    print('  Accuracy: {:.1%}±{:.1%}  F1-Score: {:.1%}±{:.1%}'.format(sdqc_acc[0], sdqc_acc[1], sdqc_f1[0], sdqc_f1[1]))
    display_report(sdqc_reports)
    verif_acc = (
     np.mean(verif_accs), np.std(verif_accs))
    verif_f1 = (np.mean(verif_f1s), np.std(verif_f1s))
    verif_rmse = (np.mean(verif_rmses), np.std(verif_rmses))
    print('Task B: Verification')
    print('  Accuracy: {:.1%}±{:.1%}  F1-Score: {:.1%}±{:.1%}  RMSE: {:.3f}±{:.3f}'.format(verif_acc[0], verif_acc[1], verif_f1[0], verif_f1[1], verif_rmse[0], verif_rmse[1]))
    display_report(verif_reports)


def write_answers_json(path: Path, sdqc_instances: List[SdqcInstance], verif_instances: List[SdqcInstance], sdqc_estimates: Dict[(str,
 Tuple[(SdqcInstance.Label,
  Dict[(SdqcInstance.Label, float)])])], verif_estimates: Dict[(str, Tuple[(VerifInstance.Label, float)])]):
