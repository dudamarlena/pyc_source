...
# [GCC 8.4.0]
# Embedded file name: /Users/tmp/Documents/CLEARumor/src/dataset.py
# Compiled at: 2019-08-20 11:48:10
# Size of source mod 2**32: 20406 bytes
...
ELMO_WEIGHTS_FILE = EXTERNAL_DATA_DIR / 'elmo_2x4096_512_2048cnn_2xhighway_5.5B_weights.hdf5'
ELMO_OPTIONS_FILE = EXTERNAL_DATA_DIR / 'elmo_2x4096_512_2048cnn_2xhighway_5.5B_options.json'
TRAINING_DATA_ARCHIVE_FILE = EXTERNAL_DATA_DIR / 'rumoureval-2019-training-data.zip'
TEST_DATA_ARCHIVE_FILE = EXTERNAL_DATA_DIR / 'rumoureval-2019-test-data.zip'
    ...
    for required_file in [ELMO_WEIGHTS_FILE, ELMO_OPTIONS_FILE,
     ...
     EVALUATION_SCRIPT_FILE]:
        if not required_file.exists():
            exit('Required file "{}" is not present. See the README on how to obtain it.'.format(required_file))


TOKENIZER_ARGS = {'preserve_case':False, 
...
class Post:
    ...
    def url(self) -> str:
        ...
        if self.platform == self.Platform.reddit:
            if self.source_id == self.id:
                return 'https://reddit.com//comments/{}'.format(self.id)
            return 'https://reddit.com//comments/{}//{}'.format(self.source_id, self.id)
        raise ValueError('Invalid post source value, must be either Twitter or Reddit.')

    def __repr__(self) -> str:
        return 'Post {}'.format(vars(self))

    @classmethod
    def load_from_twitter_dict(cls, twitter_dict: Dict, post_depths: Dict[(str, int)], source_id: Optional[str]=None, topic: Optional[str]=None) -> 'Post':
        ...
        return Post(id=id, text=(twitter_dict['text']),
    ...
    @classmethod
    def load_from_reddit_dict(cls, reddit_dict: Dict, post_depths: Dict[(str, int)], source_id: Optional[str]=None) -> 'Post':
        ...
        if 'children' in data:
            if isinstance(data['children'][0], dict):
                data = data['children'][0]['data']
        id = data['id']
        return Post(id=id, text=(data.get('title') or data.get('body') or ''),
          depth=(post_depths[id]),
          platform=(cls.Platform.reddit),
          has_media=('domain' in data and not data['domain'].startswith('self.')),
          source_id=source_id,
      ...
      test_data_archive, twitter_en_test_data.items())]:
        for topic, threads in topics:
            for thread in threads.values():
                post_depths = calc_post_depths_from_thread_structure(json.loads(archive.read(thread['structure.json'])))
                source_post = Post.load_from_twitter_dict((json.loads(archive.read(next(iter(thread['source-tweet'].values()))))),
                  post_depths,
                  topic=topic)
                posts[source_post.id] = source_post
                for reply in thread.get('replies', {}).values():
                    reply_post = Post.load_from_twitter_dict((json.loads(archive.read(reply))),
                      post_depths,
                      source_id=(source_post.id),
                      topic=topic)
                    posts[reply_post.id] = reply_post

    for archive, threads in [(training_data_archive, chain(reddit_training_data.values(), reddit_dev_data.values())),
     (
      test_data_archive, reddit_test_data.values())]:
        for thread in threads:
            post_depths = calc_post_depths_from_thread_structure(json.loads(archive.read(thread['structure.json'])))
            source_post = Post.load_from_reddit_dict(json.loads(archive.read(next(iter(thread['source-tweet'].values())))), post_depths)
            posts[source_post.id] = source_post
            for reply in thread.get('replies', {}).values():
                reply_post = Post.load_from_reddit_dict((json.loads(archive.read(reply))),
                  post_depths,
                  source_id=(source_post.id))
                posts[reply_post.id] = reply_post

    print('  Number of posts: {:d} (Reddit={:d}, Twitter={:d})'.format(len(posts), sum((1 for p in posts.values() if p.platform == Post.Platform.reddit)), sum((1 for p in posts.values() if p.platform == Post.Platform.twitter))))
    time_after = time()
...
def load_sdcq_instances() -> (
 ...
 Optional[List[SdqcInstance]]):
    ...
    def load_from_json_dict(json_dict: Dict[(str, Dict[(str, str)])]) -> List[SdqcInstance]:
        return [SdqcInstance(post_id, SdqcInstance.Label[label]) for post_id, label in json_dict['subtaskaenglish'].items()]

    training_data_archive = ZipFile(TRAINING_DATA_ARCHIVE_FILE)
    train = load_from_json_dict(json.loads(training_data_archive.read('rumoureval-2019-training-data/train-key.json')))
    dev = load_from_json_dict(json.loads(training_data_archive.read('rumoureval-2019-training-data/dev-key.json')))
    test = None
...
def load_verif_instances() -> (
 ...
 Optional[List[VerifInstance]]):
    ...
    def load_from_json_dict(json_dict: Dict[(str, Dict[(str, str)])]) -> List[VerifInstance]:
        return [VerifInstance(post_id, VerifInstance.Label[label]) for post_id, label in json_dict['subtaskbenglish'].items()]
