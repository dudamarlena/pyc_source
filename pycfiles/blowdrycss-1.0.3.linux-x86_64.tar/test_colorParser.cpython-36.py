...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/unit_tests/test_colorParser.py
...
class TestColorParser(TestCase):
    ...
    def test_property_name_allows_color(self):
        ...
        property_names_false = {
         'font-weight', 'padding', 'height', 'width', 'float'}
    ...
    def test_replace_h_with_hash_valid_property_name(self):
        ...
        for i, value in enumerate(input_values):
            self.assertEqual(color_parser.replace_h_with_hash(value=value),
              (expected_values[i]),
              msg=value)
    ...
    def test_add_color_parenthetical_valid_property_name(self):
        ...
        color_parser = ColorParser(property_name=valid_property_name)
        for i, value in enumerate(input_values):
            self.assertEqual(color_parser.add_color_parenthetical(value=value),
              (expected_values[i]),
              msg=value)

    def test_add_color_parenthetical_invalid_property_name(self):
        input_values = [
         'rgb 0 255 0', 'rgba 255 0 0 0.5', 'hsl 120 60% 70%', 'hsla 120 60% 70% 0.3', 'blue', '#000']
        expected_values = input_values
        color_parser = ColorParser()
        for i, value in enumerate(input_values):
            self.assertEqual(color_parser.add_color_parenthetical(value=value),
