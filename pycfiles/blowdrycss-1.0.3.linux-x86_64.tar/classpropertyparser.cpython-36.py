...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/classpropertyparser.py
# Compiled at: 2018-03-07 18:42:13
# Size of source mod 2**32: 28102 bytes
...
from blowdrycss.datalibrary import ordered_property_dict, property_alias_dict, property_regex_dict, pseudo_classes, pseudo_elements
...
class ClassPropertyParser(object):
    ...
    def underscores_valid(css_class=''):
        ...
        **Rules:**
        ...
        if css_class[0] == '_' or css_class[(-1)] == '_' or css_class[0] == '-' or css_class[(-1)] == '-':
            return False
        else:
            ...
            if index > 0:
                valid = set(css_class[(index - 1)]) <= allowed_before
                valid = valid and set(css_class[(index + 1)]) <= allowed_after
            else:
                valid = True
            return valid
        ...
        **Reference:**
        ...
        for css_class in self.class_set:
            if not css_class:
                ...
                continue
                set(css_class[0]) <= allowed_first or invalid_css_classes.append(css_class)
                reasons.append(' (Only a-z allowed for first character of class.)')
                continue
                set(css_class) <= allowed_middle or invalid_css_classes.append(css_class)
                reasons.append(' (Only a-z, 0-9, "_", and "-" are allowed in class name.)')
                continue
                set(css_class[(-1)]) <= allowed_last or invalid_css_classes.append(css_class)
                reasons.append(' (Only a-z and 0-9 allowed for last character of class.)')
            else:
                if not self.underscores_valid(css_class=css_class):
                    invalid_css_classes.append(css_class)
    ...
    def get_property_name(css_class=''):
        ...
        **Invalid:**
        ...
        for property_name, aliases in ordered_property_dict.items():
            if css_class == property_name or css_class == property_name + '-':
                return ''
            ...
            for alias in aliases:
                if css_class == alias:
                    if alias.endswith('-'):
                        return ''
                    elif css_class.startswith(alias):
                        return property_name
    ...
    def is_valid_pseudo_format(self, pseudo_item='', css_class=''):
        ...
        **Rules**
        ...
        if css_class.endswith(suffix):
            if len(css_class) > len(suffix):
                return True
        if css_class.endswith(isuffix):
            if len(css_class) > len(isuffix):
                return True
        return False
