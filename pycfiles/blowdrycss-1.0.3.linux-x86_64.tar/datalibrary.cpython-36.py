...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/datalibrary.py
# Compiled at: 2018-03-07 18:42:13
# Size of source mod 2**32: 29762 bytes
...
class DataLibrary(object):
    ...
    def __init__(self):
        self.property_regex_dict = {'color': {
                   '(h[0-9a-fA-F]{3} ?)$', '(h[0-9a-fA-F]{6} ?)$',
                   '(h[0-9a-fA-F]{3}-i ?)$', '(h[0-9a-fA-F]{6}-i ?)$',
                   '(h[0-9a-fA-F]{3}-.*? ?)$', '(h[0-9a-fA-F]{6}-.* ?)$',
                   '(h[0-9a-fA-F]{3}-.*?-i ?)$', '(h[0-9a-fA-F]{6}-.*-i ?)$',
                   '(h[0-9a-fA-F]{3}-i-.*? ?)$', '(h[0-9a-fA-F]{6}-i-.*? ?)$'}}
        self.custom_property_alias_dict = settings.custom_property_alias_dict
        self.property_value_as_alias_dict = {'background-repeat':{
         ...
         'direction':{
          'ltr', 'rtl'}, 
         'display':{
          'inline', 'block', 'flex', 'list-item', 'inline-block', 'inline-flex', 'run-in',
         ...
         'unicode-bidi':{
          'embed', 'bidi-override'}, 
         'vertical-align':{
          'baseline', 'sub', 'super', 'middle', 'text-top', 'text-bottom'}, 
         'visibility':{
        ...
        self.autogen_property_alias_dict()
        self.merge_dictionaries()
        self.set_clashing_aliases()
        self.remove_clashing_aliases()
        ...
        self.clashing_alias_markdown = self.dict_to_markdown(h1_text=(str('Clashing Aliases')),
          key_title=(str('Property Name')),
          value_title=(str('Invalid Clashing Aliases')),
          _dict=(self.alphabetical_clashing_dict))
        self.property_alias_markdown = self.dict_to_markdown(h1_text=(str('Valid Property Aliases')),
          key_title=(str('Property Name')),
          value_title=(str('Valid Aliases')),
          _dict=(self.alphabetical_property_dict))
        self.clashing_alias_html = self.dict_to_html(h1_text=(str('Invalid Clashing Aliases')),
          key_title=(str('Property Name')),
          value_title=(str('Clashing Aliases')),
          _dict=(self.alphabetical_clashing_dict))
        self.property_alias_html = self.dict_to_html(h1_text=(str('Valid Property Aliases')),
          key_title=(str('Property Name')),
          value_title=(str('Valid Aliases')),
          _dict=(self.alphabetical_property_dict))
        clashing_html = self.clashing_alias_html.replace('&emsp;', '   ')
        property_html = self.property_alias_html.replace('&emsp;', '   ')
        ...
        self.ordered_property_dict = OrderedDict(sorted((self.property_alias_dict.items()), key=(lambda t: len(t[0])), reverse=True))
        ...
        if len(property_name) <= 5:
            ...
            if '-' in property_name:
                ...
                if '-' in suffix:
                    ...
                    aliases.add(property_name[0] + property_name[(dash_index1 + 1)] + suffix[(dash_index2 + 1)] + '-')
    ...
    def set_clashing_aliases(self):
        ...
        for key1, alias_set1 in self.property_alias_dict.items():
            for key2, alias_set2 in clone_dict.items():
                ...
                if len(intersection) > 0 and key1 != key2:
                    try:
        ...
        for property_name in self.property_alias_dict:
            try:
                ...
                for clashing_alias in clashing_aliases:
                    if clashing_alias in self.property_alias_dict[property_name]:
                        clean_dict[property_name].remove(clashing_alias)
    ...
    def dict_to_html(h1_text='', key_title='', value_title='', _dict=None):
        ...
        *HTML Table Format:*
        ...
        count = 1
        for key, value in _dict.items():
            classes = common_classes + alternating_bg if count % 2 == 0 else common_classes
            value_str = ''
            _html += '\t\t\t\t<tr>\n'
            if isinstance(value, set):
                vcount = 1
            ...
            _html += str('\t\t\t\t\t<td class="' + classes + '">' + key + '</td>\n' + '\t\t\t\t\t<td class="' + classes + '">' + str(value_str) + '</td>\n\t\t\t\t</tr>\n')
            count += 1
