...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/unit_tests/test_classPropertyParser.py
# Compiled at: 2018-03-07 18:42:13
# Size of source mod 2**32: 20396 bytes
...
class TestClassPropertyParser(TestCase):
    ...
    def test_underscores_valid_is_true(self):
        valid_classes = {
         '6_3', 'padding-5_2rem', 'height-24_48p', '1_2-5_75-1_2-5_75', 'n5_25cm'}
    ...
    def test_clean_class_set(self):
        ...
        class_parser = ClassPropertyParser(class_set=(set()))
        class_parser.class_set = valid_classes.union(invalid_classes)
        class_parser.clean_class_set()
        self.assertEqual(class_parser.class_set, valid_classes)
    ...
    def test_get_property_name_by_alias(self):
        class_alias_set = {
         'bold', 'bolder', 'lighter', 'fweight-200', 'f-weight-100', 'fw-bold', 'font-w-900'}
        expected_property_name = 'font-weight'
        class_parser = ClassPropertyParser(class_set=(set()))
    ...
    def test_is_valid_pseudo_format_True(self):
        ...
        for i, valid_input in enumerate(valid_inputs):
            self.assertTrue((class_parser.is_valid_pseudo_format(pseudo_items[i], valid_input)),
              msg=valid_input)

    def test_is_valid_pseudo_format_False(self):
        invalid_inputs = ('-hover-blue', 'pa-active-10rem-i', 'bgc-', 'margin-10-medium-up',
                          '-after-blue', 'pa-before-10rem-i', 'bgc-', 'width-10-small-up')
        pseudo_items = ('hover', 'active', 'invalid', 'invalid', 'after', 'before',
                        'invalid', 'invalid')
        class_parser = ClassPropertyParser(class_set=(set()))
        for i, invalid_input in enumerate(invalid_inputs):
            self.assertFalse(class_parser.is_valid_pseudo_format((pseudo_items[i]), css_class=invalid_input),
              msg=invalid_input)
    ...
    def test_get_pseudo_element(self):
        valid_inputs = ('color-blue-after', 'padding-10rem-i-before', 'bgc-h048-selection',
                        'color-hfff-before')
    ...
    def test_strip_property_name_matching(self):
        ...
        class_parser = ClassPropertyParser(class_set=(set()))
        encoded_property_value = class_parser.strip_property_name(property_name=property_name,
    ...
    def test_strip_pseudo_item_not_matching(self):
        ...
        self.assertEqual(result, css_class)
    ...
    def test_strip_property_abbreviation_matching(self):
        ...
        expected_encoded_property_value = '400'
        class_parser = ClassPropertyParser(class_set=(set()))
        css_class = class_parser.strip_property_abbreviation(property_name=property_name,
          css_class=css_class)
    ...
    def test_get_encoded_property_value(self):
        ...
        for i, css_class in enumerate(css_classes):
            encoded_property_value = class_parser.get_encoded_property_value(property_name=(property_names[i]),
              css_class=css_class)
    ...
    def test_get_property_value_valid_patterns(self):
        property_name = 'color'
        encoded_property_values = ('green', 'h0ff48f', 'hfff', 'rgba-255-0-0-0_5',
                                   'hsla-120-60p-70p-0_3', 'blue', 'hf8f8f8')
        expected_property_values = ('green', '#0ff48f', '#fff', 'rgba(255, 0, 0, 0.5)',
                                    'hsla(120, 60%, 70%, 0.3)', 'blue', '#f8f8f8')
    ...
    def test_is_important_True(self):
        expected_true = ('p-10-i', 'c-green-i-hover', 'hfff-hover-i', 'hfff-i-hover',
                         'color-hfff-hover-i')
        class_parser = ClassPropertyParser(class_set=(set()))
    ...
    def test_strip_priority_designator(self):
        ...
        expected_value = 'p-10'
        class_parser = ClassPropertyParser(class_set=(set()))
        value = class_parser.strip_priority_designator(css_class=important)
        self.assertEqual(value, expected_value)
        value = class_parser.strip_priority_designator(css_class=not_important)
        self.assertEqual(value, expected_value)
