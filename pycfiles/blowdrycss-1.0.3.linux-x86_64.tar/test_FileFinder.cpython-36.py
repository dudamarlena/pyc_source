...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/unit_tests/test_FileFinder.py
...
class TestFileFinder(TestCase):
    ...
    def test_set_file_dict(self):
        ...
        valid_keys = [
         '.html', '.aspx', '.jinja2']
        settings.file_types = ('*.html', '*.aspx', '*.jinja2')
        project_directory = settings.project_directory
        settings.project_directory = unittest_file_path()
        file_finder = FileFinder(recent=False)
        for valid_key in valid_keys:
            self.assertTrue((valid_key in file_finder.file_dict), msg=(file_finder.file_dict))
            self.assertEqual((file_finder.file_dict[valid_key]),
    ...
    def test_set_file_dict_extension_not_found(self):
        ...
        settings.file_types = ('*.not_found', )
        project_directory = settings.project_directory
        settings.project_directory = unittest_file_path()
        file_finder = FileFinder(recent=False)
        for valid_key in valid_keys:
            self.assertTrue((valid_key in file_finder.file_dict), msg=(file_finder.file_dict))
            self.assertEqual((file_finder.file_dict[valid_key]), (valid_dict[valid_key]), msg=(file_finder.file_dict[valid_key]))
    ...
    def test_set_recent_file_dict(self):
        ...
        temp_file = unittest_file_path('test_recent', 'new.html')
