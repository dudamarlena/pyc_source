...
# [GCC 8.4.0]
# Embedded file name: /home/starwater/blowdrycss_venv/lib/python3.6/site-packages/blowdrycss/colorparser.py
...
class ColorParser(object):
    __doc__ = ' Extracts plain text, hexidecimal, rgb, rgba, hsl, and hsla color codes from encoded class selectors.\n\n    '
    ...
    def replace_h_with_hash(self, value=''):
        ...
        if self.property_name_allows_color():
            h_index = self.find_h_index(value=value)
            ...
            if contains_a_digit(string=value):
                ...
                for key in keywords:
                    if value.startswith(key):
                        value = value.replace(key, key.strip() + '(')
                        value += ')'
                        value = value.replace(' ', ', ')
                        break
