...
# [GCC 8.4.0]
# Embedded file name: puke/System.py
# Compiled at: 2011-12-07 14:52:40
...
def get_package_version(name, index=0):
    ...
    if check[index] == 'hard':
        if _PLATFORM == MACOS:
            version = sh('toto=\'%s\';test=`brew info $toto | grep -Ei \'(?not installed|error)\'`;\t\t\t\t\t\t\t if [[ -n "$test" ]]; then echo ""; else echo `brew info $toto | head -n 1`; fi' % name, header=None, output=False)
        else:
            version = sh('toto=\'%s\';test=`aptitude show $toto | grep -Ei \'(?not installed|unable to locate)\'`;\t\t\t\t\t\t\t if [[ -n "$test" ]]; then echo ""; else echo `aptitude show $toto | grep -i "version:"`; fi' % name, header=None, output=False)
    else:
