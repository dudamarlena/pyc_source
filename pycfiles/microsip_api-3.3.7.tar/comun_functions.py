...
# [GCC 8.4.0]
# Embedded file name: C:\repositorios\web\djmicrosip_apps\django-microsip-api\microsip_api\comun\comun_functions.py
...
def split_letranumero(folio):
    ...
    if serie != '':
        ...
        serie_consecutivo = (
         '', int(folio))
    return (
     serie, int(serie_consecutivo[1]))


def get_long_folio(serie, num):
    long_num = 9 - len(str(serie))
    num = str(num).zfill(long_num)
    return '%s%s' % (serie, num)
...
def get_short_folio(folio):
    if folio.strip(' ') == '':
        ...
        folio = '%s%s' % (letra_numero[0], letra_numero[1])
    return folio
...
def get_claves_comercio_digital():
    book = xlrd.open_workbook('%s\\comercio_digital_claves.xlsx' % EXTRA_INFO['rura_datos_facturacion'])
...
def certificar_factura(empresa_clave, using, clave_comercio_digital, **kwargs):
    ...
    datos_emisor = {'Rfc': datos_empresa.get(nombre='Rfc').get_value().replace('-', '').replace(' ', ''), 
    ...
    xml_data.append([
     ...
     [
      ...
      [
       'IVATasa', '0']]])
    import codecs
    ruta_facturas = '%s\\data\\facturas' % ruta_proyecto
    ruta_api = '%s\\libs\\comercio_digital' % ruta_proyecto
    ruta_archivo_ini = '%s\\%s.ini' % (ruta_facturas, documento['folio'])
