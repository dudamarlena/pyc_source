...
# [GCC 8.4.0]
# Embedded file name: /Users/2017-A/Dropbox/python_libraries/Pywire/pywire/main.py
# Compiled at: 2018-02-10 11:24:33
# Size of source mod 2**32: 11841 bytes
import ast, inspect, copy, logging
...
def replace_args_with_signals(original, replace_dict):
    for node in ast.walk(original):
        if isinstance(node, ast.Name) and node.id in replace_dict.keys():
        ...
        for node in ast.walk(top_node):
            if isinstance(node, ast.If):
                for subnode in ast.walk(node.test):
                    if isinstance(subnode, ast.Name) and subnode.id in all_input_signals and subnode.id not in driving_names:
...
def __ast_magic(signal):
    ...
    if isinstance(top_node.body[0].body[0], ast.Return):
        ...
        return (
         compare_signals, signal.name + ' <= ' + as_vhdl_string(return_val, width=(len(signal))) + ';\n')
    else:
...
def generate_signal_vhdl(signal):
    ...
    return (
     arch_text, body_text, sub_generated)
