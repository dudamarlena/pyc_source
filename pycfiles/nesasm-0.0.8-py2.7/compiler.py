...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nesasm/compiler.py
# Compiled at: 2017-01-19 16:57:01
...
asm65_tokens = [
 dict(type='T_INSTRUCTION', regex='^(ADC|AND|ASL|BCC|BCS|BEQ|BIT|BMI|BNE|BPL|BRK|BVC|BVS|CLC|CLD|CLI|CLV|CMP|CPX|CPY|DEC|DEX|DEY|EOR|INC|INX|INY|JMP|JSR|LDA|LDX|LDY|LSR|NOP|ORA|PHA|PHP|PLA|PLP|ROL|ROR|RTI|RTS|SBC|SEC|SED|SEI|STA|STX|STY|TAX|TAY|TSX|TXA|TXS|TYA)', store=True),
 dict(type='T_ADDRESS', regex='\\$([\\dA-F]{2,4})', store=True),
 dict(type='T_HEX_NUMBER', regex='\\#\\$([\\dA-F]{2})', store=True),
 dict(type='T_BINARY_NUMBER', regex='\\#?%([01]{8})', store=True),
 dict(type='T_DECIMAL_NUMBER', regex='\\#(\\d{1,3})', store=True),
 dict(type='T_LABEL', regex='^([a-zA-Z]{2}[a-zA-Z\\d]*)\\:', store=True),
 dict(type='T_MARKER', regex='^[a-zA-Z]{2}[a-zA-Z\\d]*', store=True),
 dict(type='T_STRING', regex='^"[^"]*"', store=True),
 dict(type='T_SEPARATOR', regex='^,', store=True),
 dict(type='T_ACCUMULATOR', regex='^(A|a)', store=True),
 dict(type='T_REGISTER', regex='^(X|x|Y|y)', store=True),
 dict(type='T_MODIFIER', regex='^(#LOW|#HIGH)', store=True),
 dict(type='T_OPEN', regex='^\\(', store=True),
 dict(type='T_CLOSE', regex='^\\)', store=True),
 dict(type='T_OPEN_SQUARE_BRACKETS', regex='^\\[', store=True),
 dict(type='T_CLOSE_SQUARE_BRACKETS', regex='^\\]', store=True),
 dict(type='T_DIRECTIVE', regex='^\\.[a-z]+', store=True),
 dict(type='T_DECIMAL_ARGUMENT', regex='^[\\d]+', store=True),
 dict(type='T_ENDLINE', regex='^\\n', store=True),
 dict(type='T_WHITESPACE', regex='^[ \\t\\r]+', store=False),
 dict(type='T_COMMENT', regex='^;[^\\n]*', store=False)]
...
def t_relative(tokens, index):
    if look_ahead(tokens, index, 'T_INSTRUCTION') and tokens[index]['value'] in ('BCC',
                                                                                 'BCS',
                                                                                 'BEQ',
                                                                                 'BNE',
                                                                                 'BMI',
...
def t_list(tokens, index):
    if t_address_or_t_binary_number(tokens, index) and t_separator(tokens, index + 1):
        ...
        while islist:
            ...
            if t_endline(tokens, index + arg * 2 + 3) or index + arg * 2 + 3 == len(tokens):
                break
            arg += 1
...
asm65_bnf = [
 ...
 dict(type='S_IMMEDIATE_WITH_MODIFIER', bnf=[
  t_instruction, t_modifier, t_open, t_address_or_t_marker,
  t_close]),
 dict(type='S_ACCUMULATOR', bnf=[t_instruction, t_accumulator]),
 dict(type='S_ZEROPAGE_X', bnf=[
  t_instruction, t_zeropage, t_separator, t_register_x]),
 dict(type='S_ZEROPAGE_Y', bnf=[
  t_instruction, t_zeropage, t_separator, t_register_y]),
 dict(type='S_ZEROPAGE', bnf=[t_instruction, t_zeropage]),
 dict(type='S_ABSOLUTE_X', bnf=[
  t_instruction, t_address_or_t_marker, t_separator,
  t_register_x]),
 dict(type='S_ABSOLUTE_Y', bnf=[
  t_instruction, t_address_or_t_marker, t_separator,
  t_register_y]),
 dict(type='S_ABSOLUTE', bnf=[t_instruction, t_address_or_t_marker]),
 dict(type='S_INDIRECT_X', bnf=[
  t_instruction, t_nesasm_compatible_open, t_address_or_t_marker,
  t_separator, t_register_x, t_nesasm_compatible_close]),
 dict(type='S_INDIRECT_Y', bnf=[
  t_instruction, t_nesasm_compatible_open, t_address_or_t_marker,
...
def get_labels(ast, cart=None):
    ...
    if cart and len(cart.banks) == 1:
        ...
        address = 0
    for leaf in ast:
        if 'S_DIRECTIVE' == leaf['type'] and '.org' == leaf['children'][0]['value']:
