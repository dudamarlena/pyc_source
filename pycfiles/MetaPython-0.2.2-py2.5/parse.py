...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/metapython/parse.py
# Compiled at: 2009-05-01 09:54:54
    ...
    def reindent():
        ...
        for tok in toks:
            if tok.match(token.INDENT):
                cur_indent = indent[(-1)]
                if tok.value.startswith(cur_indent) and len(tok.value) > indent:
                    cur_indent = tok.value
                    yield tok
                else:
                    cur_indent += '    '
                    tok = Token.make(tok.token, cur_indent, tok.begin, tok.end, tok.line)
                ...
                if len(indent) > 1:
                    ...
                    if tok.value != indent[(-1)]:
                        tok = Token.make(tok.token, indent[(-1)], tok.begin, tok.end, tok.line)
                    yield tok
            else:
                yield tok
...
def tokens_from_string(s):
    ...
    strm = (Token.make(*py_tok) for py_tok in tokenize.generate_tokens(readline))
    return strm


def tokens_from_file(fp):
    """Convert a file to a stream of tokens."""
    strm = (Token.make(*py_tok) for py_tok in tokenize.generate_tokens(fp.readline))
    return strm
        ...
        while t.match(token.ERRORTOKEN, '$'):
            ...
            if t.match(token.OP, ':') and not expr_toks:
                ...
                if t.match(token.NEWLINE):
                    ...
                    prologue, suite, epilogue = suite[0], suite[1:-1], suite[(-1)]
                else:
                    suite = [
                     t] + list(_read_to_newline(inp))
            ...
            expanded_expr_toks = expand_macros(expanded_expr_toks, glbls, lcls, filename)
...
def expand_inline_codequotes(inp, filename='<string>'):
    ...
    while True:
        ...
        while t.match(token.ERRORTOKEN, '?'):
            ...
            text = '_mpy.q(%r, inline=True)' % quoted_expr_str
            for tt in parse_string(text, filename):
                yield tt
...
def parse_stream(tokenstream, filename='<string>', inline=False):
    ...
    def gen():
        ...
        try:
            while True:
                t = inp.next()
                if t.match(token.ERRORTOKEN, ' ') or t.match(token.ENDMARKER) or t.tok_name == 'NL':
                    continue
                cur_line.append(t)
                if t.match(token.NEWLINE):
                    yield Stmt(cur_line)
                    ...
                    if t.match(token.NEWLINE):
                        ...
                        prologue, tokens, epilogue = tokens[0], tokens[1:-1], tokens[(-1)]
...
class Stmt(object):
    ...
    def quote(self):
        toks = list(self)
        if toks[0].match(token.ERRORTOKEN, '$') and len(toks) > 1 and toks[1].match(token.NAME, *KEYWORDS):
...
class Suite(Stmt):
    ...
    def __init__(self, header, body, prologue=None, epilogue=None):
        ...
        if not header[(-1)].match(token.NEWLINE):
            header = header + [
             Token.make(token.NEWLINE, '\n', header[(-1)].end, header[(-1)].end, header[(-1)].line)]
        (self.header, self.body, self.prologue, self.epilogue) = (header, body, prologue, epilogue)
        self.eol = False
        return
    ...
    def __iter__(self):
        ...
        for tok in self.prologue:
            yield tok

        for tok in self.body:
            yield tok

        for tok in self.epilogue:
            yield tok
    ...
    def expand_defcode_blocks(self):
        if self.first().match(token.NAME, 'defcode'):
            ...
            if self.header[2].match(token.OP, '('):
                ...
                args = (parse_stream(a).expand_defcode_blocks() for a in args)
            ...
            yield Suite(self.header, expanded_body, self.prologue, self.epilogue)
    ...
    def quote(self):
        if self.header[0].match(token.ERRORTOKEN, '$'):
            ...
            if new_header[0].match(token.OP, ':'):
                ...
                return Suite(new_header, new_body, self.prologue, self.epilogue)
        else:
            result = Block()
            ...
            result.append('_mpy.append_suite(%r, globals(), locals())' % header, {}, {})
            return result
...
class Block(object):
    ...
    def sanitize(self, glbls, lcls, *omit_names):
        ...
        myfun = Suite(list(parse_string('def _():')), self, [
         Token.make(token.INDENT, '    ', (0, 0), (0, 0), '')], [
         Token.make(token.DEDENT, '    ', (0, 0), (0, 0), ''),
         Token.make(token.NEWLINE, '\n', (0, 0), (0, 0), '\n')])
        ns = {}
        ...
        def translate():
            for tok in self:
                if tok.match(token.NAME):
                    yield tok.replace(value=newnames.get(tok.value, tok.value))
...
class Token(tuple):
    ...
    field_names = ('token', 'value', 'begin', 'end', 'line')
    ...
    def __repr__(self):
        t = (
         '_mpy.token.%s' % self.tok_name,) + self[1:]
        if len(self) != 5:
            print tuple(self)
            import pdb
            pdb.set_trace()
    ...
    def assertMatch(self, tok, *values, **kw):
        ...
        if not self.match(tok, *values):
            raise SyntaxError('Expected %s, found %s' % ((token.tok_name[tok], values), self), (
             filename, self.begin[0], self.begin[1], self.line))

    def __eq__(self, other):
        if self.token != other.token:
            return False
        if self.token in (token.INDENT, token.DEDENT):
            return True
        return self.value == other.value

    def indent(self):
        if self.match(token.INDENT):
            return Token.make(self.token, self.value + '    ', self.begin, self.end, self.line)
        else:
            return self
        ...
        if first_token is None and t.match(token.OP, '<'):
            for tok in _read_nested(tokenstream, '>'):
                if not tok.match(token.OP, '>'):
                    ...
                    yield Token.make(token.ERRORTOKEN, ' ', tok.begin, tok.end, tok.line)
