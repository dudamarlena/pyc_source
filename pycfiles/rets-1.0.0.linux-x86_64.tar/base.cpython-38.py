...
# [GCC 8.4.0]
# Embedded file name: /home/matthew/Documents/rets/env/lib/python3.8/site-packages/rets/parsers/base.py
...
class Base(object):
    ...
    def get_attributes(input_dict):
        ...
        :return: dict
        """
        return {v:k.lstrip('@') for k, v in input_dict.items() if k[0] == '@' if k[0] == '@'}
    ...
    def data_columns_to_dict(columns_string, dict_string, delimiter=None):
        ...
        """
        if delimiter:
            return {v:k for k, v in zip(columns_string.split(delimiter), dict_string.split(delimiter))}
        return {v:k for k, v in zip(columns_string.split(), dict_string.split())}
    ...
    def analyze_reply_code(self, xml_response_dict):
        ...
        elif 'RETS' not in xml_response_dict:
