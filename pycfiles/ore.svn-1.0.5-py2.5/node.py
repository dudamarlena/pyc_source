...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ore/svn/node.py
...
class SubversionObject(object):
    ...
    def getLog(self, revision=None):
        ...
        resource_ctx = svn_ctx.getResourceContext()
        if revision is None:
            revision = fs.node_created_rev(resource_ctx.root, self.svn_path, resource_ctx.pool)
            if -1 is revision:
                ...
                return SubversionLogEntry(author=txn.author, message=txn.message, revision=revision, date=datetime.now())
    ...
    def getLogEntries(self, changed_paths=0, strict_history=False, start_revision=None, end_revision=0):
        ...
        try:
            repos.svn_repos_get_logs(resource_ctx.repository, [
             self.svn_path], start_revision, end_revision, changed_paths, strict_history, collection.addLog, pool)
        finally:
            pass
    ...
    def getProperty(self, property_name):
        ...
        resource_ctx = svn_ctx.getResourceContext()
        value = fs.node_prop(resource_ctx.root, self.svn_path, property_name, resource_ctx.pool)
        return value
    ...
    def getProperties(self):
        ...
        revision = svn_ctx.getRevision()
        resource_ctx = svn_ctx.getResourceContext()
        properties = fs.node_proplist(resource_ctx.root, self.svn_path, resource_ctx.pool)
        return properties
    ...
    def setProperty(self, property_name, property_value):
        ...
        svn_ctx = self.getSVNContext()
        resource_ctx = svn_ctx.getResourceContext()
        fs.change_node_prop(resource_ctx.txnroot, self.svn_path, property_name, property_value, resource_ctx.pool)
    ...
    def getRevisionCreated(self):
        ...
        pool = core.svn_pool_create(resource_ctx.pool)
        try:
            revision = fs.node_created_rev(resource_ctx.fsroot, self.svn_path, pool)
        finally:
            core.svn_pool_destroy(pool)
