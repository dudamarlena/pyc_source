...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/ore/svn/file.py
...
class SubversionFile(SubversionObject):
    ...
    def getContents(self, revision=None, writer=None):
        ...
        stream = fs.file_contents(resource_ctx.root, self.svn_path, resource_ctx.pool)
        if writer is None:
            writer = charbuffer()
        while 1:
    ...
    def getAnnotatedLines(self, revision_set=(), include_copies=1):
        ...
        try:
            for r in revision_set:
                ...
                rev_author = fs.revision_prop(resource_ctx.fsptr, r, core.SVN_PROP_REVISION_AUTHOR, pool) or ''
                stream = fs.file_contents(revision_root, revision_path_map[r], pool)
                buffer = charbuffer()
                while 1:
                    data = core.svn_stream_read(stream, CHUNK_SIZE)
                    if not data:
                ...
                delta = difflib.ndiff(previous_contents.splitlines(1), contents.splitlines(1))
                li = 0
                for d in delta:
                    flag = d[0]
                    if flag == ' ':
    ...
    def lock(self, token=None, comment='', expiration_date=0, steal=False):
        ...
        lock = fs.lock(ctx.fsptr, self.svn_path, token, comment, False, expiration_date, ctx.revision, bool(steal), ctx.pool)
        fs.access_add_lock_token(ctx.access, lock.token)
        return SubversionLock(lock)

    def unlock(self, token=None, break_lock=False):
        """
...
class SubversionLock(object):
    ...
    def __init__(self, svn_lock):
        ...
        self.expiration_date = svn_lock.expiration_date and make_time(svn_lock.expiration_date) or None
        return
