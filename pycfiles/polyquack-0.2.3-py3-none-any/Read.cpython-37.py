...
# Embedded file name: build/bdist.linux-x86_64/egg/polypy/Read.py
# Compiled at: 2018-11-28 10:07:10
# Size of source mod 2**32: 4332 bytes
import os, sys, numpy as np, polypy as pp
...
def read_config(file, atom_list):
    ...
    if os.path.isfile(file):
        ...
        vec = lv.sum(axis=0)
        data = {'label':atname, 
         'trajectories':coords,  'lv':vec,  'timesteps':1,  'natoms':natoms}
    else:
        print('File cannot be found')
        sys.exit(0)
    if natoms == 0:
