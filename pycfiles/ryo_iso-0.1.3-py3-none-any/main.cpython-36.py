...
# [GCC 8.4.0]
# Embedded file name: /home/user/src_import/ryo-iso/ryo_iso/tasks/main.py
# Compiled at: 2019-12-03 09:45:32
# Size of source mod 2**32: 38532 bytes
import os as _os, sys as _sys, glob as _glob, subprocess as _subprocess, time as _time, shlex as _shlex, doit as _doit, shutil as _shutil, importlib_resources as _resources
from pathlib import Path as _Path
from contextlib import contextmanager as _contextmanager
from loguru import logger as _logger
from ryo_iso.tasks.init import task_init
import ryo_iso.config as _config, ryo_iso.utils as _utils
...
if _config._ctx is not None:
    _ctx = _config._ctx
    _logger.remove()
    if 'LOGURU_LEVEL' not in _os.environ.keys():
        if not _ctx['config'].app_cfg['debug']:
            _logger.add((_sys.stderr), level='WARNING')
        else:
            _logger.add((_sys.stderr), level='DEBUG')

        @_ctx['config'].app_cfg['progress'] or _contextmanager
...
def task__hash_check():
    ...
    return {'actions':[
     ...
     'uptodate':[
      _doit.tools.result_dep('_hash_cache')]}
...
def task__hash_version():
    ...
    return {'actions':[
      _ctx['config'].distro_hash_version], 
     'uptodate':[
      _doit.tools.result_dep('_hash_check')]}
...
def task__base_image_download():
    ...
    def download_iso():
        ...
        if iso_path.is_file():
            ...
            with _tqdm(desc=(iso_path.name), total=iso_size,
              unit='byte',
              unit_scale=True,
              ascii=False) as (status):
                with _subprocess.Popen((_shlex.split(cmd)), bufsize=0, cwd=(str(iso_path.parent)),
    ...
    return {'actions':[
      download_iso], 
     'uptodate':[
      _doit.tools.result_dep('_hash_version')]}
...
def task__base_image_extract():
    ...
    return {'actions':[
      (
       _logger.debug, ['extract image']),
      _utils.profile_start,
      'sudo xorriso -osirrox on -indev base_image.iso -extract / image > %s 2>&1' % task_log,
      _utils.profile_stop], 
     'file_dep':[
      'base_image.iso'], 
     'targets':[
...
def task__squashfs_extract():
    ...
    return {'actions':[
      (
       _logger.debug, ['extract squashfs']),
      _utils.profile_start,
      'sudo unsquashfs image/casper/filesystem.squashfs > %s 2>&1' % task_log,
      _utils.profile_stop], 
     'file_dep':[
      'base_image.iso', 'image/.disk/base_installable'], 
     'targets':[
      'squashfs-root/etc/os-release']}
...
def task__mount_dev():
    ...
    mnt = (
     {'src':'/dev', 
      'dst':'squashfs-root/dev',  'opt':'--bind'},
     {'src':'/dev/pts', 
      'dst':'squashfs-root/dev/pts',  'opt':'--bind'},
     {'src':'none', 
      'dst':'squashfs-root/proc',  'opt':'-t proc'},
     {'src':'none', 
      'dst':'squashfs-root/sys',  'opt':'-t sysfs'})
    for mount in mnt:
        if _os.path.isdir(mount['dst']) and not _os.path.ismount(mount['dst']):
            yield {'name':mount['dst'],  'actions':[
              'sudo mount %s %s %s' % (mount['opt'],
               mount['src'],
               mount['dst'])]}
...
def task__squashfs_apt_proxy():
    ...
    if 'apt' in _ctx['config'].iso_cfg.keys():
        return {'actions':['echo \'Acquire::http { Proxy "http://localhost:3142"; };\' | sudo tee squashfs-root/etc/apt/apt.conf.d/01proxy > /dev/null'],  'targets':[
          'squashfs-root/etc/apt/apt.conf.d/01proxy'], 
         'task_dep':[
          '_mount_dev'], 
         'uptodate':[
...
def task__squashfs_apt_repo():
    ...
    if 'apt' in _ctx['config'].iso_cfg.keys() and 'repository' in _ctx['config'].iso_cfg['apt'].keys():
        ...
        yield {'name':None,  'doc':'Install custom APT repositories'}
        for repo in repos:
            if repo in components or repo.startswith('ppa:') or repo.startswith('deb ') or repo.startswith('deb-src '):
                yield {'name':repo,  'actions':[
                  (
                   _logger.debug, ['Adding repository: %s' % repo]),
                  'sudo chroot squashfs-root add-apt-repository --yes "%s" >> %s 2>&1' % (
                   repo, task_log)], 
                 'task_dep':[
                  '_squashfs_apt_repo:%s' % update if update else '_squashfs_apt_key', '_squashfs_resolv'], 
                 'uptodate':[
                  False]}
                ...
                if _Path(repo).name == 'sources.list':
                    yield {'name':repo,  'actions':[
                      (
                       _logger.debug, ['Updating sources.list: %s' % _Path(repo).name]),
                      'sudo cp %s squashfs-root/etc/apt/sources.list' % repo], 
                     'targets':[
                      'squashfs-root/etc/apt/sources.list'], 
                     'task_dep':[
                      '_squashfs_apt_key'], 
                     'uptodate':[
                      _doit.tools.check_timestamp_unchanged('squashfs-root/etc/apt/sources.list')]}
                else:
                    yield {'name':repo,  'actions':[
                      (
                       _logger.debug, ['Adding to sources.list.d: %s' % _Path(repo).name]),
                      'sudo cp %s squashfs-root/etc/apt/sources.list.d/.' % repo], 
                     'targets':[
                      'squashfs-root/etc/apt/sources.list.d/%s' % _Path(repo).name], 
                     'task_dep':[
                      '_squashfs_apt_key'], 
                     'uptodate':[
...
def task__chroot_apt_up():
    ...
    return {'actions':[
      (
       _logger.debug, ['apt update/upgrade']),
      _utils.profile_start,
      'sudo chroot squashfs-root apt-get update -y > %s 2>&1' % task_log,
...
def task__chroot_apt_purge():
    ...
    if 'apt' in _ctx['config'].iso_cfg.keys():
        if 'purge' in _ctx['config'].iso_cfg['apt'].keys():
            pkgs = _ctx['config'].iso_cfg['apt']['purge']
            for pkg in pkgs:
                yield {'name':pkg,  'actions':[
                  (
                   _logger.debug, ['apt purge: ' + pkg]),
                  'sudo chroot squashfs-root apt-get purge -y %s >> %s' % (
                   pkg, task_log)], 
                 'task_dep':[
                  '_chroot_apt_up']}
...
def task__chroot_apt_install():
    ...
    if 'apt' in _ctx['config'].iso_cfg.keys():
        if 'install' in _ctx['config'].iso_cfg['apt'].keys():
            yield {'name':None, 
            ...
            for pkg in pkgs:
                yield {'name':pkg,  'actions':[
                  (
                   _logger.debug, ['apt install: ' + pkg]),
                  'sudo chroot squashfs-root apt-get install -y %s < /dev/null >> %s 2>&1' % (
                   pkg, task_log)], 
                 'task_dep':[
                  '_chroot_apt_install:%s' % prev if prev else '_chroot_apt_purge']}
                prev = pkg
            ...
            yield {'name':'teardown', 
             ...
             'actions':[
              _utils.profile_stop], 
             'task_dep':[
              '_chroot_apt_install:%s' % prev if prev else '_chroot_apt_install:start']}
...
def task__chroot_dpkg_install():
    ...
    if 'dpkg' in _ctx['config'].iso_cfg.keys():
        if 'install' in _ctx['config'].iso_cfg['dpkg'].keys():
            ...
            for pkg in pkgs:
                yield {'name':pkg,  'actions':[
                  ...
                  'sudo chroot squashfs-root dpkg -i %s/%s >> %s 2>&1' % (
                   dst, pkg, task_log)], 
                 'targets':[
                  'squashfs-root%s/%s' % (dst, pkg)], 
                 'task_dep':[
                  '_chroot_dpkg_install:%s' % prev if prev else '_chroot_apt_install']}
                prev = pkg
...
def task__chroot_pip_cache():
    ...
    if 'pip' in _ctx['config'].iso_cfg.keys() and pip_dir.is_dir():
        pip_cache = _ctx['config'].iso_cfg['pip']['cache']
        return {'actions':[
          'sudo chroot squashfs-root mkdir -p %s' % pip_cache,
          'sudo cp -r %s squashfs-root/%s/wheels' % (pip_dir, pip_cache)], 
         'task_dep':[
          '_chroot_dpkg_install', '_chroot_apt_install']}
    else:
        return {'actions':None,  'task_dep':[
          '_chroot_dpkg_install', '_chroot_apt_install']}
...
def task__chroot_pip_install():
    ...
    if 'pip' in _ctx['config'].iso_cfg.keys():
        if 'install' in _ctx['config'].iso_cfg['pip'].keys():
            yield {'name':None, 
            ...
            for pkg in pkgs:
                yield {'name':pkg,  'actions':[
                  (
                   _logger.debug, ['pip install: ' + pkg]),
                  '%s %s >> %s 2>&1' % (cmd, pkg, task_log)], 
                 'task_dep':[
                  '_chroot_pip_install:%s' % prev if prev else '_chroot_pip_cache']}
                prev = pkg
            ...
            yield {'name':'teardown', 
             ...
             'actions':[
              _utils.profile_stop], 
             'task_dep':[
              '_chroot_pip_install:%s' % prev if prev else '_chroot_pip_install:start']}
...
def task__chroot_pip_local():
    ...
    if 'pip' in _ctx['config'].iso_cfg.keys():
        if 'local' in _ctx['config'].iso_cfg['pip'].keys():
            pip_cache = _ctx['config'].iso_cfg['pip']['cache']
            target = _ctx['config'].iso_cfg['pip']['target']
            cmd = 'sudo chroot squashfs-root pip3 install'
            ...
            for pkg in pkgs:
                yield {'name':pkg,  'actions':[
                  (
                   _logger.debug, ['pip local: ' + pkg]),
                  '%s %s >> %s' % (cmd, pkg, task_log)], 
                 'task_dep':[
                  '_chroot_pip_local:%s' % prev if prev else '_chroot_pip_cache']}
                prev = pkg
...
def task__chroot():
    ...
    """
    return {'actions':[
      'sudo mv squashfs-root/etc/resolv.conf.orig squashfs-root/etc/resolv.conf',
...
def task__image_preseed():
    ...
    if _os.path.isfile('ubuntu.seed'):
        return {'actions':[
          'sudo cp ubuntu.seed image/preseed/ubuntu.seed'], 
         'targets':[
          'image/preseed/ubuntu.seed'], 
         'task_dep':[
          '_umount_dev'], 
         'uptodate':[
          _doit.tools.check_timestamp_unchanged('image/preseed/ubuntu.seed')]}
...
def task__image_squashfs_size():
    ...
    if _os.path.isfile('image/casper/filesystem.size'):
        return {'actions':[
          'sudo du -sx --block-size=1 squashfs-root/ | cut -f1 | sudo tee image/casper/filesystem.size > /dev/null'], 
         'targets':[
...
def task__image_squashfs_make():
    ...
    """
    return {'actions':[
      'sudo mksquashfs squashfs-root image/casper/filesystem.squashfs -b 1048576 -noappend'], 
     'targets':[
...
def task__image_checksum():
    ...
    """
    return {'actions':[
      _doit.tools.CmdAction('find -type f -print0 | xargs -0 md5sum | grep -v isolinux | grep -v md5sum.txt | sudo tee md5sum.txt > ../build/log/md5sum.log', cwd='./image')], 
     'targets':[
      'image/md5sum.txt'], 
     'task_dep':[
...
def task__qemu_setup():
    ...
    def create_image():
        ...
        with _tqdm(desc='Creating QEMU disk image', total=total_bytes,
          unit='byte',
          unit_scale=True,
          ascii=False) as (status):
            with task_log.open('w') as (log):
                with _subprocess.Popen((_shlex.split(cmd)), bufsize=0, stdout=log,
                  stderr=log) as (proc):
                    while proc.poll() is None:
                        if status:
                            if disk_image.is_file():
...
def task_start():
    ...
    Start VM from image
    """
    if 'qemu' in _ctx['config'].iso_cfg.keys():
        if 'args' in _ctx['config'].iso_cfg['qemu'].keys():
...
def task_build():
    ...
    return {'actions':[
      (
       _logger.debug, ['Volume ID: ' + volume_id]),
      'xorriso -as mkisofs -V "%s" -R -J -joliet-long -l -cache-inodes -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -partition_offset 16 -b isolinux/isolinux.bin -c isolinux/boot.cat -boot-load-size 4 -boot-info-table  -no-emul-boot -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -isohybrid-apm-hfsplus -o %s image > %s 2>&1' % (
       volume_id, output_image, task_log)], 
     'targets':[
      output_image], 
     'task_dep':[
      '_reset_sudo_timestamp',
      '_image'], 
     'clean':[
      cleanup]}