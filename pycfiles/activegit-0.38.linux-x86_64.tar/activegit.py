...
# [GCC 8.4.0]
# Embedded file name: /users/claw/miniconda/lib/python2.7/site-packages/activegit/activegit.py
...
std_files = [
 'classifier.pkl', 'testing.pkl', 'training.pkl']
...
class ActiveGit:
        ...
        if version in self.versions:
            ...
            if 'working' in self.repo.branch().stdout:
                if force:
                    ...
                    return
            stdout = self.repo.checkout(version, b='working').stdout
    ...
    def commit_version(self, version, msg=None):
        ...
        if not version not in self.versions:
            raise AssertionError('Will not overwrite a version name.')
            feat, targ = msg or self.training_data
