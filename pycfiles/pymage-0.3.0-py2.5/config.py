...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/pymage/config.py
# Compiled at: 2007-09-07 20:18:44
"""
...
_gsPrims = {'resource': resman.Resource, 'image': resman.ImageResource, 
        ...
        if isinstance(configFile, (basestring, vfs.Path)):
            ...
            if game is None:
                ...
                if os.path.exists(path):
                    ...
                    continue
            elif not game.filesystem.exists(configFile):
...
def _getValue(value_string):
    ...
    """
    boolLiterals = {'false': False, 'no': False, 
       'off': False, 
       'true': True, 
       'yes': True, 
       'on': True}
...
def _getSiteConfig(doc, config_files):
    ...
    for child in doc.documentElement.childNodes:
        if child.nodeType == minidom.Node.ELEMENT_NODE and child.tagName == 'config-file':
...
def _processGameSite(doc, config):
    ...
    :Parameters:
        ...
        config : dict
            Configuration dictionary
    """
    handlers = {'playlist': _handlePlaylist, 'group': _handleGroup}
    handlers.update(dict.fromkeys(_gsPrims, _handlePrimitive))
    for child in doc.documentElement.childNodes:
        if child.nodeType == minidom.Node.ELEMENT_NODE and child.tagName in handlers:
...
def _handlePlaylist(elem, config):
    ...
    for sub in elem.childNodes:
        if sub.nodeType == minidom.Node.ELEMENT_NODE:
            if sub.tagName == 'path':
                warnings.warn('%s using old path-based playlist' % key, GameSiteWarning)
...
def _handleGroup(elem, config):
    ...
    groupKeys = set()
    for sub in elem.childNodes:
        if sub.nodeType == minidom.Node.ELEMENT_NODE and sub.tagName in _gsPrims:
            if sub.hasAttribute('ref'):
...
def _getText(elem, post=True):
    ...
    preserve = elem.hasAttributeNS(xmlNS, 'space') and elem.getAttributeNS(xmlNS, 'space') == 'preserve'
