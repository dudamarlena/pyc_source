...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/text_processing_ml/matching/matching.py
# Compiled at: 2019-05-06 07:46:29
# Size of source mod 2**32: 5998 bytes
import itertools, jellyfish
...
class MatchText:
    ...
    def generate_list_combinations_unordered(self, first: list, second: list):
        iterator = map(''.join, itertools.chain(itertools.product(first, second), itertools.product(second, first)))
    ...
    def combine_lists(self, lists, ordered=True):
        ...
        for start_index in range(1, num_lists):
            for index in range(1, num_lists):
                final_list += generate_list_combinations(final_list, lists[index])
    ...
    def match_lists(self, lists: list, text: str, substring_len: int, ordered: bool=True):
        ...
        found_substrings = self.filter_by_length(found_substrings, substring_len)
    ...
    def fuzzy_match_lists(self, lists: list, text: str, substring_len: int, ordered: bool=True, tolerance: int=1):
        ...
        fuzzy_matches = []
        for substring in substrings:
            match = fuzzy_match(substring, text, ordered=ordered,
              tolerance=tolerance)
            fuzzy_matches.append(match)

        fuzzy_matches = self.filter_by_length(fuzzy_matches, substring_len)
