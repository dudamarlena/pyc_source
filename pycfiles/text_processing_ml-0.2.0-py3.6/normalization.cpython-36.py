...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/text_processing_ml/normalization/normalization.py
...
import en_core_web_sm, copy, string
...
class NormalizeText:
    ...
    def initialize_spellchecker(self, corpus='', corpus_name=False, words=[], ner_text='', add_ner=False):
        """
        * Initializes the spellchecker and allows for 
        ...
        if add_ner:
            if text:
                doc = nlp(text)
                words += [entity.text for entity in doc.ents]
    ...
    def _update_spelling(self, previous_word, word):
        ...
        if not previous_word:
            ...
            new_word = self.spell_checker.correction(previous_word, word)
        return new_word
    ...
    def make_spelling_correction(self, text: str) -> str:
        ...
        * If there are spellings you want to not be corrected, 
        ...
        for token_index in range(len(tokens)):
            ...
            word, punctuation = self.strip_punctuation(token, return_punctuation=True)
