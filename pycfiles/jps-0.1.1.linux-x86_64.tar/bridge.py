...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/jps/bridge.py
...
class UploadSingleTopicBridge(object):
    ...
    def __init__(self, topic, remote_host=None, remote_pub_port=None):
        ...
        self._remote_publisher = Publisher(topic, host=remote_host, pub_port=remote_pub_port)
...
class DownloadSingleTopicBridge(object):
    ...
    def __init__(self, topic, remote_host=None, remote_sub_port=None):
        ...
        self._remote_subscriber = Subscriber(topic, self.remote_to_local, host=remote_host, sub_port=remote_sub_port)
...
class Bridge(object):
    ...
    def __init__(self, upload_topic_names, download_topic_names, remote_host=None, remote_pub_port=None, remote_sub_port=None):
        """
        ...
        if len(set(set(upload_topic_names) & set(download_topic_names))) > 0:
            raise Error('upload_topic_names and download_topic_names should not' + 'contain same names')
        self._bridges = []
        for topic in upload_topic_names:
            self._bridges.append(UploadSingleTopicBridge(topic, remote_host, remote_pub_port=remote_pub_port))

        for topic in download_topic_names:
            self._bridges.append(DownloadSingleTopicBridge(topic, remote_host, remote_sub_port=remote_sub_port))
