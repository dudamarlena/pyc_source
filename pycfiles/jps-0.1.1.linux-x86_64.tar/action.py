...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/jps/action.py
...
class ActionServer(object):
    ...
    def __init__(self, base_topic_name, callback, host=None, pub_port=None, sub_port=None, serializer='DEFAULT', deserializer='DEFAULT'):
        self._req_subscriber = Subscriber(base_topic_name + REQUEST_SUFFIX, self._request_callback, host=host, sub_port=sub_port, deserializer=deserializer)
        self._res_publisher = Publisher(base_topic_name + RESPONSE_SUFFIX, host=host, pub_port=pub_port, serializer=serializer)
...
class ActionClient(object):
    ...
    def __init__(self, base_topic_name, host=None, pub_port=None, sub_port=None, serializer='DEFAULT', deserializer='DEFAULT'):
        self._req_publisher = Publisher(base_topic_name + REQUEST_SUFFIX, host=host, pub_port=pub_port, serializer=serializer)
        self._res_subscriber = Subscriber(base_topic_name + RESPONSE_SUFFIX, host=host, sub_port=sub_port, deserializer=deserializer)
        self._str_for_hash = self.__str__()

    def _create_hash(self):
        return hashlib.md5(('{}_{}').format(self._str_for_hash, time.time())).hexdigest()
