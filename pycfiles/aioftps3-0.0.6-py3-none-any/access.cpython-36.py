...
# Embedded file name: /mnt/c/Users/luc_t_000/projects/freepybox/aiofreepybox/access.py
# Compiled at: 2019-03-10 17:41:47
...
class Access:
    ...
    async def _get_challenge(self, base_url, timeout=10):
        ...
        if not resp.get('success'):
            raise AuthorizationError('Getting challenge failed (APIResponse: {0})'.format(json.dumps(resp)))
    ...
    async def _refresh_session_token(self):
        session_token, session_permissions = await self._get_session_token(self.base_url, self.app_token, self.app_id, self.timeout)
    ...
    async def _perform_request(self, verb, end_url, **kwargs):
        ...
        if not self.session_token:
            ...
            request_params = {**kwargs, **{'headers':self._get_headers(), 
