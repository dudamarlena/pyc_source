...
# [GCC 8.4.0]
# Embedded file name: word2vec.py
# Compiled at: 2016-01-15 16:30:53
...
class Word2Vec:
    ...
    def load_word2vec_format(cls, fname, binary=False, encoding='utf8', unicode_errors='strict'):
        ...
        with smart_open(fname) as (fin):
            ...
            if binary:
                ...
                for line_no in xrange(vocab_size):
                    ...
                    word = to_unicode(('').join(word), encoding=encoding, errors=unicode_errors)
        ...
        if result.syn0.shape[0] != len(result.vocab):
            warnings.warn('duplicate words detected, shrinking matrix size from %i to %i', result.syn0.shape[0], len(result.vocab))
            result.syn0 = ascontiguousarray(result.syn0[:len(result.vocab)])
        assert (
         len(result.vocab), result.vector_size) == result.syn0.shape
        return result