...
# [GCC 8.4.0]
# Embedded file name: /home/zeus/PyTorch-Hackathon-2019/rectifai/models/posenet/decode.py
# Compiled at: 2019-09-16 17:03:27
...
def traverse_to_targ_keypoint(edge_id, source_keypoint, target_keypoint_id, scores, offsets, output_stride, displacements):
    ...
    source_keypoint_indices = np.clip((np.round(source_keypoint / output_stride)),
    ...
    displaced_point_indices = np.clip((np.round(displaced_point / output_stride)),
      a_min=0, a_max=[height - 1, width - 1]).astype(np.int32)
...
def decode_multiple_poses(scores, offsets, displacements_fwd, displacements_bwd, output_stride=16, max_pose_detections=10, score_threshold=0.5, nms_radius=20, min_pose_score=0.15):
    ...
    height = scores.shape[1]
    width = scores.shape[2]
    offsets = offsets.cpu().numpy().reshape(2, -1, height, width).transpose((1, 2,
                                                                             3, 0))
    displacements_fwd = displacements_fwd.cpu().numpy().reshape(2, -1, height, width).transpose((1,
                                                                                                 2,
                                                                                                 3,
                                                                                                 0))
    displacements_bwd = displacements_bwd.cpu().numpy().reshape(2, -1, height, width).transpose((1,
                                                                                                 2,
                                                                                                 3,
                                                                                                 0))
