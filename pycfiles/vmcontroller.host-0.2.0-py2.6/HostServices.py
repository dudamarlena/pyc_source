...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vmcontroller/host/services/HostServices.py
# Compiled at: 2011-03-06 15:17:54
"""
...
"""
try:
    import logging, inject, time, uuid, inspect
...
class VMRegistry(object):
    ...
    def getIdForName(self, vmName):
        ...
        if not id_:
            self.logger.error("Unable to match VM name '%s' to any registered VM (is                 the VM controller really running from within a VM?)", vmName)
...
class HostXMLRPCService(xmlrpc.XMLRPC, object):
    ...
    def xmlrpc_ping(self, vm, timeout_secs=5.0):
...
def _getPrintableFunctionSignature(f):
    ...
    if not defs:
        defs = ()
    argsWithDefs = reversed(map(lambda a, d: d and '%s=%s' % (a, d) or str(a), reversed(args), reversed(defs)))
...
class Host(object):
    ...
    def sendCmdRequest(self, toVmName, cmd, args=(), env={}, path=None, fileForStdin=''):
        ...
        toSend = self.words['CMD_RUN']().howToSay(toVmId, cmdId, cmd, args, env, path, fileForStdin)
        requestKeys = ('timestamp', 'toVmName', 'toVmId', 'cmd', 'args', 'env', 'path',
                       'fileForStdin')
