...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vmcontroller/host/__main__.py
...
"""
try:
    import os, sys, logging, warnings, multiprocessing, time, inject
...
def init():
    ...
    """
    parser = OptionParser()
    parser.add_option('-c', '--config', dest='configfile', help='Read configuration from FILE. (Overrides default config file.)', metavar='FILE')
    parser.add_option('-a', '--host', dest='xmlrpc_host', help='Listen on specified address for XMLRPC interface (default 127.0.0.1)', metavar='ADDR')
    parser.add_option('-p', '--port', dest='xmlrpc_port', help='Listen on specified port for XMLRPC interface (default 50505)', type='int', metavar='PORT')
    parser.add_option('-l', '--logfile', dest='logfile', help='Log to specified file.', metavar='FILE')
    parser.add_option('--debug', action='store_true', dest='debug', default=False, help='Sets logging to debug (unless logging configured in config file).')
