...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ofxstatement/plugins/revolut.py
# Compiled at: 2020-05-09 07:57:17
# Size of source mod 2**32: 5386 bytes
import csv, re
...
class RevolutCSVStatementParser(CsvStatementParser):
    ...
    def parse_amount(self, value):
        return value and value.strip() or 0
    ...
    def parse_record(self, line):
        ...
        reference = line[c[('Reference' if 'Reference' in c.keys() else 'Description')]].strip()
        trntype = False
        ...
        if trntype == 'POS':
            ...
            if reference.startswith('Cash at '):
                stmt_line.payee, stmt_line.memo = self.parse_payee_memo(reference[8:])
            else:
                if reference.startswith('To ') or reference.startswith('From '):
                    stmt_line.payee = self.parse_value(reference[reference.find(' '):], 'payee')
                else:
        ...
        if 'Notes' in c.keys():
            if not stmt_line.memo:
                ...
                if len(stmt_line.memo.strip()) > 0:
                    stmt_line.memo += ' '
                stmt_line.memo += '({})'.format(line[c['Notes']].strip())
        return stmt_line
        ...
        if 'Completed Date' in columns:
            if 'Paid Out (...)' in columns and 'Paid In (...)' in columns:
                if 'Reference' in columns or 'Description' in columns:
                    ...
                    return parser
        raise Exception('No suitable Revolut parser found for this statement file.')