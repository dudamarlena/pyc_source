...
# [GCC 8.4.0]
# Embedded file name: C:\repositorios\web\djmicrosip_apps\django-microsip-liquida\django-microsip-liquida\views.py
# Compiled at: 2019-10-31 11:43:02
...
class InitialConfiguration(object):
    ...
    def is_valid(self):
        ...
        try:
            ...
            from_lleno = False
            try:
                from_lleno = Registry.objects.get(nombre='SIC_EnlaceLiquida_ConfigFormLleno').get_value() == '1'
            except ObjectDoesNotExist:
                self.errors.append('Por favor inicializa la configuracion de la aplicacion dando  <a href="/enlace_liquida/preferencias/inicializar_configuracion/">click aqui</a>')
...
def agregar_empresa_a_ignorar(empresa):
    ...
    str_actual = '%s%s' % (str(actual_year), str(actual_month))
        ...
        for using in databases[0:ultimo]:
            try:
                ...
                for factura in facturas:
                    ...
                    facturas_dic.append({'id': '%s;%s' % (using, factura[0]), 
                       'folio': factura[1], 
                       'fecha': fecha, 
                       'empresa': using, 
...
def PrepararEmpresasView(request, template_name='django-microsip-liquida/preparar_empresas.html'):
    ...
    if form.is_valid():
        ...
        if destino_articulo:
            ...
            destino_articulo = Articulo.objects.using(database_to_clone).create(nombre=origen_articulo.nombre, es_almacenable=origen_articulo.es_almacenable, estatus=origen_articulo.estatus, seguimiento=origen_articulo.seguimiento, cuenta_ventas=origen_articulo.cuenta_ventas, nota_ventas=origen_articulo.nota_ventas, unidad_venta=origen_articulo.unidad_venta, unidad_compra=origen_articulo.unidad_compra, costo_ultima_compra=origen_articulo.costo_ultima_compra, usuario_ult_modif=origen_articulo.usuario_ult_modif)
        ...
        if not destino_articulo_clave:
            ArticuloClave.objects.using(database_to_clone).create(clave=articulo_clave, articulo=destino_articulo, rol=rol_principal)
        ...
        if not destino_impuesto:
            ...
            destino_impuesto = Impuesto.objects.using(database_to_clone).create(nombre=origen_impuesto.nombre, tipoImpuesto=tipo_impuesto, tipo_iva=origen_impuesto.tipo_iva, porcentaje=origen_impuesto.porcentaje)
        ...
        if not destino_impuesto_articulo:
            ImpuestosArticulo.objects.using(database_to_clone).create(articulo=destino_articulo, impuesto=destino_impuesto)
        ...
        if not destino_condicion_pago:
            destino_condicion_pago = CondicionPago.objects.using(database_to_clone).create(nombre=origen_condicion_pago.nombre, dias_ppag=origen_condicion_pago.dias_ppag, porcentaje_descuento_ppago=origen_condicion_pago.porcentaje_descuento_ppago, es_predet=origen_condicion_pago.es_predet, usuario_creador=origen_condicion_pago.usuario_creador, usuario_ult_modif=origen_condicion_pago.usuario_creador)
        ...
        if not destino_primer_plazo:
            CondicionPagoPlazo.objects.using(database_to_clone).create(condicion_de_pago=destino_condicion_pago, dias=origen_primer_plazo.dias, porcentaje_de_venta=origen_primer_plazo.porcentaje_de_venta)
        ...
        if not destino_cliente:
            destino_cliente = Cliente.objects.using(database_to_clone).create(nombre=origen_cliente.nombre, condicion_de_pago=destino_condicion_pago, moneda=origen_cliente.moneda, cobrar_impuestos=origen_cliente.cobrar_impuestos, generar_interereses=origen_cliente.generar_interereses, emir_estado_cuenta=origen_cliente.emir_estado_cuenta)
        else:
        ...
        if not destino_pais:
            destino_pais = Pais.objects.using(database_to_clone).create(nombre=origen_pais.nombre, es_predet=origen_pais.es_predet, nombre_abreviado=origen_pais.nombre_abreviado)
        if not destino_estado:
            destino_estado = Estado.objects.using(database_to_clone).create(nombre=origen_estado.nombre, nombre_abreviado=origen_estado.nombre_abreviado, es_predet=origen_estado.es_predet, pais=destino_pais)
        if not destino_ciudad:
            destino_ciudad = Ciudad.objects.using(database_to_clone).create(nombre=origen_ciudad.nombre, es_predet=origen_ciudad.es_predet, estado=destino_estado)
        ...
        if not destino_direccion:
            destino_direccion = ClienteDireccion.objects.using(database_to_clone).create(cliente=destino_cliente, rfc_curp=origen_direccion.rfc_curp, ciudad=destino_ciudad, estado=destino_estado, pais=destino_pais, colonia=origen_direccion.colonia, nombre_consignatario=origen_direccion.nombre_consignatario, calle=origen_direccion.calle, es_ppal=origen_direccion.es_ppal, referencia=origen_direccion.referencia, codigo_postal=origen_direccion.codigo_postal, calle_nombre=origen_direccion.calle_nombre, numero_exterior=origen_direccion.numero_exterior, numero_interior=origen_direccion.numero_interior, email=origen_direccion.email)
        else:
...
def PreferenciasManageView(request, template_name='django-microsip-liquida/herramientas/preferencias.html'):
    msg = ''
    form_initial = {'facturaa_cliente': first_or_none(Cliente.objects.filter(nombre=Registry.objects.get(nombre='SIC_EnlaceLiquida_FacturarA_ClienteNombre').get_value())), 
...
def UpdateConfigurationDatabase(request):
    ...
    if request.user.is_superuser and padre:
        ...
        if not RegistryLong.objects.filter(nombre='SIC_EnlaceLiquida_EmpresasFacturaCertificada').exists():
            RegistryLong.objects.create(id=1, nombre='SIC_EnlaceLiquida_EmpresasFacturaCertificada', referencia='', valor='')
        if not Registry.objects.filter(nombre='SIC_EnlaceLiquida_EmpresasAIgnorar').exists():
            Registry.objects.create(nombre='SIC_EnlaceLiquida_EmpresasAIgnorar', tipo='V', padre=padre, valor='')
        if not Registry.objects.filter(nombre='SIC_EnlaceLiquida_FacturarA_ClienteNombre').exists():
            Registry.objects.create(nombre='SIC_EnlaceLiquida_FacturarA_ClienteNombre', tipo='V', padre=padre, valor='')
        if not Registry.objects.filter(nombre='SIC_EnlaceLiquida_FacturarA_ArticuloClave').exists():
            Registry.objects.create(nombre='SIC_EnlaceLiquida_FacturarA_ArticuloClave', tipo='V', padre=padre, valor='')
        if not Registry.objects.filter(nombre='SIC_EnlaceLiquida_RutaLiquidacion').exists():
            Registry.objects.create(nombre='SIC_EnlaceLiquida_RutaLiquidacion', tipo='V', padre=padre, valor='C:\\Liquida')
        if not Registry.objects.filter(nombre='SIC_INDICE_BASES_DATOS_SYN').exists():
            Registry.objects.create(nombre='SIC_INDICE_BASES_DATOS_SYN', tipo='V', padre=padre, valor='')
        if not Registry.objects.filter(nombre='SIC_EnlaceLiquida_ConfigFormLleno').exists():
            Registry.objects.create(nombre='SIC_EnlaceLiquida_ConfigFormLleno', tipo='V', padre=padre, valor='0')
        if not Registry.objects.filter(nombre='SIC_CarpetaFacturacionSAT').exists():
            Registry.objects.create(nombre='SIC_CarpetaFacturacionSAT', tipo='V', padre=padre, valor='')
    ...
    if not errors:
        ...
        if using in settings.MICROSIP_DATABASES:
            ...
            if documento_sin_generar and consecutivo_folio_definido and cliente_existe:
                ...
                documento = VentasDocumento.objects.using(using).create(tipo='F', subtipo='N', fecha=datetime.datetime.now(), cliente=cliente, descripcion=descripcion_documentos, cliente_clave='', cliente_direccion=direccion, direccion_consignatario=direccion, almacen=almacen, moneda=moneda, estado='N', aplicado='N', importe_neto=factura['importe_neto'], sistema_origen='VE', condicion_pago=condicion_pago, es_cfd='S', modalidad_facturacion='CFDI', uso_cfdi=uso_cfdi, metodo_pago_sat='PPD', envio_fechahora=None, creacion_usuario=request.user.username)
                ...
                for plazo in plazos:
                    ...
                    c = connections[using].cursor()
                    query = 'INSERT INTO "VENCIMIENTOS_CARGOS_VE" ("DOCTO_VE_ID", "FECHA_VENCIMIENTO", "PCTJE_VEN")                         VALUES (%s, %s, %s)'
                ...
                for detalle in factura['detalles']:
                    ...
                    VentasDocumentoDetalle.objects.using(using).create(documento=documento, articulo_clave=facturaa_articulo_clave, articulo=articulo, unidades=unidades, precio_unitario=precio, precio_total_neto=importe_neto)
