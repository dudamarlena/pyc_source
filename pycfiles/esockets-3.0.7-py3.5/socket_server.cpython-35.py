...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ESocketS/socket_server.py
# Compiled at: 2015-12-25 07:06:44
# Size of source mod 2**32: 9424 bytes
import socket, threading, selectors, queue, errno
...
class Socket:
    ...
    def __init__(self, port=1234, host=socket.gethostbyname(socket.gethostname()), queue_size=1000, block_time=2, selector=selectors.PollSelector()):
        self.host = host
        self.port = port
    ...
    def restart(self):
        ...
        self.__init__(port=self.port, host=self.host, queue_size=self.queue_size, block_time=self.block_time)
        self.start()
    ...
    def run_in_subthread(self, function, yes_no):
        if hasattr(function, __name__) and function.__name__ in self._run_in_subthread:
            ...
            if type(function) == str and (function in self._run_in_subthread or function == 'all'):
                key = function
            else:
