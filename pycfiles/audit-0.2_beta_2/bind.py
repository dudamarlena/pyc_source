...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\audit\bind.py
# Compiled at: 2010-04-29 16:00:41
from datetime import date
from django.contrib.auth.models import User
from django.contrib.contenttypes.models import ContentType
from django.db.models.signals import post_save, pre_delete, m2m_changed
from django.utils.encoding import smart_unicode
from audit.models import AuditTrail, AuditField, AuditedModel, AuditM2MField, AuditGroup, get_formatter
...
def get_audit_name(instance):
    ...
    if not audit_name:
        audit_name = ('{name}({pk})').format(name=ct.model.upper(), pk=instance.pk)
...
def generate_audits(instance, action):
    ...
    (admin_audit, _) = AuditTrail.objects.get_or_create(type=0, content_type=ct, object_id=instance.pk, action=action, audit_on=date.today())
    ...
    if instance.audit.has_public_fields:
        (public_audit, _) = AuditTrail.objects.get_or_create(type=1, content_type=ct, object_id=instance.pk, action=action, audit_on=date.today())
...
def save_audit(sender, instance=None, created=False, **kwargs):
    ...
    if not created:
        for (field, (new, original)) in instance.audit.dirty_fields.iteritems():
            ...
            f = AuditField(audit=admin_audit, name=field, new=new, original=original, modified_by=instance.audit.modified_by)
            f.save()
            if instance.audit.fields[field].public:
                add_group(public_audit, instance.audit.fields[field].group)
                f = AuditField(audit=public_audit, name=field, new=new, original=original, modified_by=instance.audit.modified_by)
                f.save()
        ...
        for (field, changes) in instance.audit.m2m_dirty.iteritems():
            ...
            f = AuditM2MField(audit=admin_audit, name=field, added=added, removed=removed, modified_by=instance.audit.modified_by)
            f.save()
            if instance.audit.fields[field].public:
                add_group(public_audit, instance.audit.fields[field].group)
                f = AuditM2MField(audit=public_audit, name=field, added=added, removed=removed, modified_by=instance.audit.modified_by)
                f.save()
...
def m2m_audit(sender, instance, action, reverse, model, pk_set, **kwargs):
    if not isinstance(instance, AuditedModel) or action not in ('post_add', 'pre_remove',
                                                                'pre_clear') or isinstance(instance, model):
        return
    if action == 'post_add':
