...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/metaimage/models.py
# Compiled at: 2011-05-20 21:56:03
...
class MetaImage(ImageModel, BaseModel):
    ...
    slug = AutoSlugField(max_length=100, editable=True, populate_from='title', unique=True, help_text='In general do NOT edit slugs manually.')
    caption = models.TextField(_('caption'), blank=True)
    source_url = models.URLField(_('source URL'), blank=True, null=True, verify_exists=True, help_text='The external URL where the image originated from.')
    source_note = models.CharField(_('source note'), max_length=500, blank=True, null=True, help_text='Attribution text, copyright notices, permissions, etc.')
    safetylevel = models.IntegerField(_('safetylevel'), choices=SAFETY_LEVEL, default=1)
    privacy = models.IntegerField(_('privacy'), choices=PRIVACY_CHOICES, default=1)
    imageset = models.ManyToManyField(ImageSet, blank=True, null=True, verbose_name=_('image set'))
    ...
    def save(self, *args, **kwargs):
        ...
        if getattr(self, 'source_url'):
            if not getattr(self, 'image'):
                ...
                local_img_filename = self.generate_filename_from_data(raw_image_data)
            if raw_image_data and local_img_filename:
                pass
            else:
                raise self.is_str_an_image(raw_image_data) or AssertionError
    ...
    def generate_filename_from_url(self, the_url=None):
        ...
        non_alphanum_regex = re.compile('[^a-zA-Z0-9\\-\\.]')
    ...
    def render(self, the_size='width500', linked=False):
        ...
        img_html = mark_safe('<img src="%s" height="%s" width="%s" alt="%s">' % (
         mimage_url, the_height, the_width, str(self.title)))
        if linked:
            return mark_safe('<a href="%s" class="invisible">%s</a>' % (
