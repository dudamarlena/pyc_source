...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/metaimage/forms.py
# Compiled at: 2011-02-18 15:22:51
from django import forms
from metaimage.models import MetaImage
model_fields_to_exclude = ('admin_notes', 'creator', 'is_active', 'slug', 'imageset',
                           'effect', 'crop_from')
model_fields_to_include = ('image', 'source_url', 'title', 'caption', 'privacy', 'source_note',
                           'safetylevel', 'tags')
...
class MetaImageUploadForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not local_image and not remote_image_url:
            raise forms.ValidationError('Either a local image, or a remote image URL, must be given.')
        return c_data
...
class MetaImageEditForm(forms.ModelForm):
    ...
    class Meta:
        model = MetaImage
        fields = model_fields_to_include = ('title', 'caption', 'privacy', 'source_note',
                                            'safetylevel', 'tags')
