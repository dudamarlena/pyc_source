...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtviews/dockers.py
# Compiled at: 2013-09-07 10:39:09
...
class Docker(QtGui.QDockWidget):
    ...
    def __init__(self, mainWindow, child):
        ...
        self.customContextMenuRequested.connect(lambda pnt: self.mainWindow.workspaceContextMenuDocked(self.child, pnt))
...
class TabbedWorkspaceMixin(object):
    ...
    def saveView(self, name):
        ...
        tabs = [ w._docker_meta.settingsKey for w in self.windows('tabs') if w._docker_meta.factory is not None
               ]
        docks = [ w._docker_meta.settingsKey for w in self.windows('docks') if w._docker_meta.factory is not None
                ]
    ...
    def addWorkspaceWindow(self, widget, title=None, factory=None, settingsKey=None, addto=None):
        ...
        """
        if not (isinstance(widget, QtGui.QWidget) or factory == widget or factory == None):
            raise AssertionError('This is a bizarre API with a silly limitation')
            factory = widget
    ...
    def workspaceWindowByKey(self, settingsKey, include=None):
        ...
        """
        ws = [ w for w in self.windows(include) if w._docker_meta.settingsKey == settingsKey
             ]
        if len(ws):
            return ws[0]
        else:
    ...
    def addSharedContextActions(self, w, menu):
        ...
        a.triggered.connect(lambda key=w._docker_meta.settingsKey: self.closeWindow(w._docker_meta.settingsKey))
        a = menu.addAction('Rename')
        a.triggered.connect(lambda key=w._docker_meta.settingsKey: self.renameWindow(key))
        if not w._docker_meta.is_detached():
            a = menu.addAction('Detach Visual Settings')
            a.triggered.connect(lambda key=w._docker_meta.settingsKey: self.detachVisualSettings(key))

    def workspaceContextMenuDocked(self, w, pnt):
        self.menu = QtGui.QMenu()
        a = self.menu.addAction('Tabify')
        a.triggered.connect(lambda key=w._docker_meta.settingsKey: self.undockWorkspaceWindow(key))
    ...
    def workspaceContextMenuTabbed(self, pnt):
        ...
        if self.workspace.currentIndex() >= 0 and tb.tabAt(pnt) == self.workspace.currentIndex():
            ...
            a.triggered.connect(lambda key=w._docker_meta.settingsKey: self.dockWorkspaceWindow(key))
            self.addSharedContextActions(w, self.menu)
    ...
    def tabsInWindowMenu(self):
        for i in range(self.workspace.count()):
            ...
            action.triggered.connect(lambda checked, key=child._docker_meta.settingsKey: self.selectTab(key))
