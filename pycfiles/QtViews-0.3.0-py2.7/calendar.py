...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/qtviews/calendar.py
# Compiled at: 2013-09-07 10:48:33
...
def GetContrastingTextColor(c):
    if max([c.red(), c.green(), c.blue()]) >= 128:
...
class CalendarDelegate(QtGui.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        for entry in entries[:visible_count]:
            ...
            eventRect = index.internalPointer().entryBlock(entry, index, options.rect.translated(-options.rect.topLeft()))
...
class CalendarView(TableView):
    ...
    def setEventList(self, events, startDate, endDate, text, bkColor):
        events = [ EventWrapper(e, startDate(e), endDate(e), text(e), bkColor(e)) for e in events
                 ]
        datarows = []
