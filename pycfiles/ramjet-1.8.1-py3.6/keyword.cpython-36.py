...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/ramjet/tasks/keyword.py
    ...
    for docu in db.posts.find():
        if docu.get('post_password'):
            ...
            if k_cn_each:
                ...
                if k_en_each:
                    tags = k_en_each[:5]
            db.posts.update_one({'_id': docu['_id']}, {'$set': {'post_tags': tags}})
...
def load_english_keyword(content):
    ...
    for k in kws:
        ...
        if not k in STOP_WORDS:
            if not eng_kw_regex.match(k):
                pass
            else:
                key_map[k] += 1

    return [ks[0] for ks in heapq.nlargest(N_ENGLISH_KEYWORDS, (key_map.items()), key=(itemgetter(1)))]
