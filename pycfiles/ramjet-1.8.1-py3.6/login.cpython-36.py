...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/ramjet/tasks/twitter/login.py
...
class OAuthHandle(web.View):
    ...
    async def get(self):
        ...
        try:
            ...
            docu.update({'access_token_secret':access_token_secret,  'access_token':access_token, 
             'last_update':utcnow()})
            self.save_userinfo(docu)
            session['username'] = docu['username']
            docu = {'id':123, 
             'username':'laisky'}
            token = {'source':'twitter', 
             'id':docu['id'], 
             'username':docu['username']}
            return {'info':'welcome {}'.format(docu['username']), 
    ...
    def save_userinfo(self, docu):
        ...
        col.update({'id': docu['id']},
          {'$set': docu},
          upsert=True)

    def get_userinfo(self):
        logger.debug('get_userinfo')
        docu = self.api.verify_credentials()
        return {'id':docu.id, 
         'username':docu.name}