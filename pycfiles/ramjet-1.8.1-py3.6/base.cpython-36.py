...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/ramjet/tasks/twitter/base.py
...
def twitter_history_parser(tweet):
    ...
    if len(eurls) != len(surls):
        _u = set()
        eurls = [u for u in eurls if u not in _u if not _u.add(u)]
        if len(eurls) != len(surls):
