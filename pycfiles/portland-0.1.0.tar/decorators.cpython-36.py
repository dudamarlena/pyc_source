...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/portkey/decorators.py
...
def on(selector: str, event: str, filter_selector: str=None, throttle=False, stop_propagation=False):
    ...
    def decorator(func):
        assert inspect.iscoroutinefunction(func), 'Only a coroutine function (a function defined with an async def syntax) can be registered as an event handler.'
...
def onclick(selector: str, filter_selector: str=None, throttle=False, stop_propagation=False):
    ...
    """
    return on(selector, 'click', filter_selector=filter_selector,
      throttle=throttle,
...
def with_ui_data(**d):
    ...
    def decorator(func):
        handler_info = get_handler_info(func)
        assert handler_info.uiData is None, 'You should only register a set of remote attributes on a handler function.'
