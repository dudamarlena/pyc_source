...
class TaurusCommandsForm(TaurusWidget):
    ...
    def _onCommandExecuted(self, result):
        ...
        output = '<i>%s</i><br>' % timestamp.strftime('%Y-%m-%d %H:%M:%S') + '<b>Command:</b> %s<br>' % cmdbutton.getCommand() + '<b>Pars:</b> %s<br>' % repr(cmdbutton.getParameters()) + '<b>Return Value:</b><br>%s' % str(result)
