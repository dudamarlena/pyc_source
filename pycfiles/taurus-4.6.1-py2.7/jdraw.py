...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/graphic/jdraw/jdraw.py
...
LINESTYLE_JDW2QT = {0: Qt.Qt.SolidLine, 1: Qt.Qt.DotLine, 
   2: Qt.Qt.DashLine, 
   3: Qt.Qt.DashLine, 
   4: Qt.Qt.DashDotLine}
FILLSTYLE_JDW2QT = {0: Qt.Qt.NoBrush, 1: Qt.Qt.SolidPattern, 
...
TEXTHINT_JDW2QT = CaselessDict({'helvetica': Qt.QFont.Helvetica, 
   ...
   '': Qt.QFont.AnyStyle})
ALIGNMENT = {0: Qt.Qt.AlignHCenter, 
   1: Qt.Qt.AlignLeft, 
   2: Qt.Qt.AlignRight}
VALIGNMENT = {0: Qt.Qt.AlignVCenter, 
   1: Qt.Qt.AlignTop, 
   2: Qt.Qt.AlignBottom}
...
class TaurusJDrawGraphicsFactory(Singleton, TaurusBaseGraphicsFactory, Logger):
    ...
    def readLabelObj(self, item, params):
        ...
        txt = params.get('text')
        if txt:
            if any(isinstance(txt, t) for t in (list, tuple, set)):
                txt = ('\n').join(txt)
        ...
        if not item:
            ...
            if hasattr(item, 'brush'):
                ...
                if fillStyle == Qt.Qt.LinearGradientPattern:
                    ...
                    gradient = Qt.QLinearGradient(ox + params.get('gradX1', 0), oy + params.get('gradY1', 0), ox + params.get('gradX2', 0), oy + params.get('gradY2', 0))
