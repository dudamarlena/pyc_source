...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusconfigurationpanel.py
# Compiled at: 2019-08-19 15:09:30
"""This module provides a set of basic taurus widgets based on QLineEdit"""
__all__ = [
 'TaurusConfigurationPanel', 'TangoConfigLineEdit',
...
class TangoConfigLineEdit(Qt.QLineEdit, TaurusBaseWritableWidget):
    ...
    def notifyValueChanged(self):
        if str(self.text()) != str(self.getValue()):
            style = 'TangoConfigLineEdit {color: %s; font-weight: %s}' % ('blue', 'bold')
            self.setStyleSheet(style)
        else:
            style = 'TangoConfigLineEdit {color: %s; font-weight: %s}' % ('black',
                                                                          'normal')
            self.setStyleSheet(style)
    ...
    def writeValue(self):
        ...
        style = 'TangoConfigLineEdit {color: %s; font-weight: %s}' % ('black', 'normal')
        self.setStyleSheet(style)
        return
