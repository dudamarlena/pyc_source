...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/extra_guiqwt/taurustrend2d.py
...
from taurus.qt.qtgui.extra_guiqwt.tools import TaurusModelChooserTool, TimeAxisTool, AutoScrollTool, AutoScaleXTool, AutoScaleYTool, AutoScaleZTool
...
class TaurusTrend2DDialog(ImageDialog, TaurusBaseWidget):
    ...
    def __init__(self, parent=None, designMode=False, toolbar=True, stackMode='deltatime', buffersize=512, options=None, autoscale='xyz', **kwargs):
        """
        ...
        self.registerConfigDelegate(self.trendItem or BaseConfigurableClass(), name='trendItem')
        self.registerConfigDelegate(self.get_tool(AutoScrollTool), name='AutoScrollTool')
        self.registerConfigDelegate(self.get_tool(AutoScaleXTool), name='AutoScaleXTool')
        self.registerConfigDelegate(self.get_tool(AutoScaleYTool), name='AutoScaleYTool')
        self.registerConfigDelegate(self.get_tool(AutoScaleZTool), name='AutoScaleZTool')
        self.registerConfigProperty(self.getStackMode, self.setStackMode, 'stackMode')
        self.registerConfigProperty(self._get_axes_conf, self._set_axes_conf, 'axes_confs')
        return
...
@click.option('-x', '--x-axis-mode', 'x_axis_mode', type=click.Choice(['t', 'd', 'e']), default='d', show_default=True, help='interpret X values as timestamps (t), ' + 'time deltas (d) or event numbers (e). ')
@click.option('-b', '--buffer', 'max_buffer_size', type=int, default=512, show_default=True, help='maximum number of values to be stacked ' + '(when reached, the oldest values will be ' + 'discarded)')
...
def trend2d_cmd(model, x_axis_mode, max_buffer_size, demo, window_name):
    ...
    w = TaurusTrend2DDialog(stackMode=stackMode, wintitle=window_name, buffersize=max_buffer_size)
    if model:
        w.setModel(model)
    w.show()
    sys.exit(app.exec_())
