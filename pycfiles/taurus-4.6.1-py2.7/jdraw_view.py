...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/graphic/jdraw/jdraw_view.py
...
class TaurusJDrawSynopticsView(Qt.QGraphicsView, TaurusBaseWidget):
    ...
    def openJDraw(self):
        ifile, _ = compat.getOpenFileName(self, 'Load JDraw File', '', 'JDraw File (*.jdw)')
        if not ifile:
            return
    ...
    def get_item_colors(self, emit=False):
        ...
        try:
            ...
            if emit:
                self.itemsChanged.emit(self.modelName.split('/')[(-1)].split('.')[0], item_colors)
        except:
    ...
    def get_sizes(self):
        ...
        sizes = [ x for s in (self.size(), self.sizeHint(), srect.size()) for x in (
         s.width(), s.height())
                ]
        try:
            s = self.parent().size()
    ...
    def getModelMimeData(self):
        ...
        try:
            ...
            if not selected:
                ...
                return None
            model = getattr(([ s for s in selected if s.isUnderMouse() and getattr(s, '_name', '')
                             ] or [selected])[0], '_name', '')
...
def jdraw_view_main():
    import sys, time, taurus.qt.qtgui.graphic
