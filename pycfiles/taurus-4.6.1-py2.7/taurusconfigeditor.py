...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusconfigeditor.py
...
import pickle, os, shutil, tempfile, click
...
class QConfigEditorModel(Qt.QStandardItemModel):
    ...
    def setData(self, index, value, role=Qt.Qt.DisplayRole):
        ...
        try:
            ...
            self.showError.emit('Wrong value!', 'The value you entered is wrong. The old value will be restored.')
    ...
    def valueChanged(self, value, index):
        ...
        changedItem.setData('Value has been changed. Old value: ' + str(changedItem.text()), Qt.Qt.ToolTipRole)
    ...
    def fillTaurusConfig(self, item, configdict):
        ...
        customkeys = [ k for k in configdict if k not in ('__orderedConfigNames__',
                                                          '__itemConfigurations__',
                                                          'ConfigVersion', '__pickable__') ]
        if len(customkeys) > 0:
    ...
    def saveSettings(self, group=None):
        ...
        self._settings.setValue('TaurusConfig', Qt.QByteArray(pickle.dumps(self._configurationDictionaries[group])))
        if group is not None:
            self._settings.endGroup()
        return
    ...
    def clearChanges(self):
        ...
        for index in self.markedItems:
            ...
            while itemToMark is not None:
                itemToMark.setData(Qt.QFont('Arial', 10, Qt.QFont.Normal), Qt.Qt.FontRole)
...
class QConfigEditor(TaurusWidget):
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self._toolbar.addAction(Qt.QIcon.fromTheme('document-open'), 'Open File', self.loadFile)
        self._toolbar.addAction(Qt.QIcon.fromTheme('document-save'), 'Save File', self.saveFile)
        self._toolbar.addAction(Qt.QIcon.fromTheme('edit-undo'), 'Reload from file', self.restoreOriginal)
    ...
    def contextMenuEvent(self, event):
        ...
        if self.tree._toDeleteIndex.column() in (1, 2) or text in ('LAST', '[custom]') or text in self.tree.perspectives:
            return
        menu = Qt.QMenu()
        menu.addAction(Qt.QIcon.fromTheme('process-stop'), 'Delete branch: ' + text, self.tree.deleteBranch)
        menu.addSeparator()
    ...
    def loadFile(self, iniFileName=None):
        ...
        if iniFileName is None:
            ...
            iniFileName, _ = compat.getOpenFileName(self, 'Select a settings file', path, 'Ini Files (*.ini)')
            if not iniFileName:
                return
