...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/qdataexportdialog.py
...
class QDataExportDialog(Qt.QDialog):
    ...
    def exportCurrentData(self, set=None, ofile=None, verbose=True, AllowCloseAfter=True):
        ...
        if ofile is None:
            if set == self.allInSingleFile:
                ...
                name += '.dat'
            ofile, _ = compat.getSaveFileName(self, 'Export File Name', name, 'All Files (*)')
            if not ofile:
                return False
        try:
            try:
        ...
        if verbose:
            ...
            Qt.QMessageBox.information(self, 'Set exported', msg, Qt.QMessageBox.Ok)
    ...
    def exportAllData(self, preffix=None):
        ...
        if preffix is None:
            outputdir = Qt.QFileDialog.getExistingDirectory(self, 'Export Directory', '')
            if not outputdir:
                return False
            ...
            for curve_name in self.sortedNames:
                ...
                if previous is None:
                    ...
                    if key == self.allInSingleFile:
                        self.dataTE.clear()
                        Qt.QMessageBox.critical(self, 'Unable to display', 'X axes of all sets in the plot must be ' + 'exactly the same for saving in a single ' + 'file!. Curves will be saved each one in ' + 'its own file', Qt.QMessageBox.Ok)
