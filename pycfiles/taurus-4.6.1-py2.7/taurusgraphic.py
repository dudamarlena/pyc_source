...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/graphic/taurusgraphic.py
...
import re, os, subprocess, traceback, collections
...
from taurus.qt.qtgui.util import QT_ATTRIBUTE_QUALITY_PALETTE, QT_DEVICE_STATE_PALETTE, ExternalAppAction, TaurusWidgetFactory
__all__ = [
...
def parseTangoUri(name):
    ...
    validator = {tango.TangoDevice: TangoDeviceNameValidator, tango.TangoAttribute: TangoAttributeNameValidator}
    try:
        val = validator[tango.TangoFactory().findObjectClass(name)]()
        params = val.getUriGroups(name)
        if '_devslashname' in params:
            return params
        return
    except:
        return
...
class TaurusGraphicsScene(Qt.QGraphicsScene):
    ...
    def __init__(self, parent=None, strt=True):
        ...
        try:
            ...
            if not self.TRACE_ALL:
                ...
                self.debug = self.info = self.warning = self.error = lambda l: self.logger.warning(l)
        except:
    ...
    def getItemByName(self, item_name, strict=None):
        ...
        target = str(item_name).strip().split()[0].lower().replace('/state', '')
    ...
    def selectGraphicItem(self, item_name):
        ...
        if selected:
            ...
            if not iname.strip():
                self.clearSelection()
                return False
            if any(iname not in i for i in selected):
                self.clearSelection()
            else:
    ...
    def _displaySelectionAsEllipse(self, items):
        ...
        for item in items:
            try:
                if isinstance(item, TaurusGraphicsItem) and item.getExtensions().get('noSelect') or item in self._selection:
                    continue
                x, y = item.x(), item.y()
    ...
    def setSelectionMark(self, picture=None, w=10, h=10):
        ...
        """
        if picture is None:
            self.SelectionMark = None
        else:
            self.SelectionMark = lambda p=picture, x=w, y=h: self.getSelectionMark(p, x, y)
    ...
    def drawSelectionMark(self, x, y, w, h, oversize=1):
        ...
        MAX_CIRCLE_SIZE = (srect.width(), srect.height())
        ...
        if isinstance(mark, Qt.QGraphicsEllipseItem):
            if None not in [w, h]:
                if w > MAX_CIRCLE_SIZE[0] or h > MAX_CIRCLE_SIZE[1]:
                    x, y = x + w / 2.0 - 0.5 * MAX_CIRCLE_SIZE[0], y + h / 2.0 - 0.5 * MAX_CIRCLE_SIZE[1]
    ...
    def getClass(self, clName):
        if not clName or clName == 'noPanel':
            return
        if clName in ('atkpanel.MainPanel', 'atkpanel'):
...
class QGraphicsTextBoxing(Qt.QGraphicsItemGroup):
    ...
    def __init__(self, parent=None, scene=None):
        ...
        self._text.setTransform(Qt.QTransform.fromScale(self._TEXT_RATIO, self._TEXT_RATIO), True)
...
class TaurusGraphicsAttributeItem(TaurusGraphicsItem):
    ...
    @deprecation_decorator(alt=".getDisplayValue(fragmentName='rvalue.units')", rel='4.0.3')
    def getUnit(self):
...
TYPE_TO_GRAPHICS = {None: {'Rectangle': Qt.QGraphicsRectItem, 'RoundRectangle': TaurusRoundRectItem, 
          ...
          'Spline': QSpline}, 
   TaurusDevice: {'Rectangle': TaurusRectStateItem, 'RoundRectangle': TaurusRoundRectStateItem, 
   ...
   TaurusAttribute: {'Rectangle': TaurusRectStateItem, 'RoundRectangle': TaurusRoundRectStateItem, 
                     'Ellipse': TaurusEllipseStateItem, 
                     'Polyline': TaurusPolygonStateItem, 
...
class TaurusBaseGraphicsFactory(object):
    ...
    def getGraphicsItem(self, type_, params):
        ...
        cls = None
        if '/' in name:
            if name.lower().startswith('tango:') and (name.count('/') == 2 or 'tango:/' not in name.lower()):
