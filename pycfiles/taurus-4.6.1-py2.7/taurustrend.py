...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/qwt5/taurustrend.py
...
import time, numpy, re, gc, weakref
...
__all__ = [
 'ScanTrendsSet', 'TaurusTrend', 'TaurusTrendsSet']
...
class TaurusTrendsSet(Qt.QObject, TaurusBaseComponent):
    ...
    def compileTitles(self, basetitle):
        ...
        if '<trend_index>' in basetitle:
            ret = [ basetitle.replace('<trend_index>', '%i' % i) for i in range(ntrends) ]
        else:
            ret = [
             basetitle] * ntrends
        return ret
        ...
        **Example of return values:** Consider history of 10 events for a
        ...
        if value is not None:
            ...
            try:
                ...
                self.warning('Problem updating history (%s=%s):%s', model, v, e)
                value = None
        ...
        if self.parent().getXIsTime():
            ...
            try:
                self._xBuffer.append(1.0 + self._xBuffer[(-1)])
            except IndexError:
                self._xBuffer.append(0)

        return (
         self._xBuffer.contents(), self._yBuffer.contents())
...
class TaurusTrend(TaurusPlot):
    ...
    def __initActions(self):
        ...
        self._setForcedReadingPeriodAction.triggered.connect(partial(self.setForcedReadingPeriod, msec=None, tsetnames=None))
        ...
        self._setMaxBufferSizeAction.triggered.connect(partial(self.setMaxDataBufferSize, maxSize=None))
    ...
    def isTimerNeeded(self, checkMinimized=True):
        ...
        :return: (bool)
        """
        return self._replotTimer is not None and not self.size().isEmpty() and bool(len(self.trendSets)) and self.isVisible() and not (checkMinimized and self.isMinimized())
    ...
    def resizeEvent(self, event):
        ...
        if event.oldSize().isEmpty():
            if self.isTimerNeeded():
                ...
                self._replotTimer.start()
            elif self._replotTimer is not None:
    ...
    def setXIsTime(self, enable, axis=Qwt5.QwtPlot.xBottom):
        ...
        if enable:
            self.setAxisScale(axis, self._startingTime - 60, self._startingTime)
        else:
            self.setAxisScale(axis, 0, 10)
        ...
        if enable and not self._designMode:
            ...
            self.axisWidget(axis).scaleDivChanged.connect(self.rescheduleReplot)
        else:
            self._replotTimer = None
        return
    ...
    def updateCurves(self, names):
        ...
        try:
            ...
            for name in names:
                name = str(name)
                if '|' in name:
                    raise ValueError('composed ("X|Y") models are not supported by TaurusTrend')
                if name not in self.trendSets:
                    matchScan = re.search('scan:\\/\\/(.*)', name)
                    if matchScan:
                        tset = ScanTrendsSet(name, parent=self, autoClear=self.getScansAutoClear(), xDataKey=self._scansXDataKey)
                        self.__qdoorname = matchScan.group(1)
        ...
        try:
            ...
            if curve is not None and self.getXDynScale() and len(curve._xValues) > 0:
                ...
                if datamax > currmax or datamax < currmin:
                    minstep = datamax - currmax
                    maxstep = datamax - currmin
    ...
    def rescheduleReplot(self, axis=Qwt5.QwtPlot.xBottom, width=1080):
        ...
        if self.xIsTime:
            ...
            if self._replotTimer.interval() != plot_refresh:
                ...
                self.debug('New replot period is %1.2f seconds', plot_refresh / 1000.0)
        else:
    ...
    def createConfig(self, tsnames=None, **kwargs):
        ...
        """
        configdict = TaurusPlot.createConfig(self, curvenames=None)
        if tsnames is None:
    ...
    def getQtDesignerPluginInfo(cls):
        ...
        return {'module': 'taurus.qt.qtgui.qwt5', 
    ...
    def _onUseArchivingAction(self, enable):
        ...
        """
        if enable:
            self._archivingWarningThresshold = self._startingTime - 600
    ...
    def setMaxDataBufferSize(self, maxSize=None):
        ...
        if maxSize is None:
            ...
            maxSize, ok = qgetint(self, 'New buffer data size', 'Enter the number of points to be kept in memory for each curve', maxSize, 2, 10000000, 1000)
            if not ok:
                return
        choiceOnClear = None
        self.curves_lock.acquire()
    ...
    def onChangeXDataKeyAction(self):
        options = [
         '[Auto Selection]', '[Internal Scan Index]']
        if self.__qdoorname is not None:
            scanname = 'scan://%s' % self.__qdoorname
            tset = self.getTrendSet(scanname)
        ...
        key, ok = Qt.QInputDialog.getItem(self, 'X data source selection', 'Which data is to be used for the abscissas in scans?', options, 0, True)
        if ok:
            key = str(key)
            if key == options[0]:
                key = None
    ...
    def setForcedReadingPeriod(self, msec=None, tsetnames=None):
        ...
        if msec is None:
            ...
            msec, ok = qgetint(self, 'New forced reading period', 'Enter the new period for forced reading (in ms).\n Enter "0" for disabling', max(0, msec), 0, 604800000, 100)
            if not ok:
                return
            if msec == 0:
                msec = -1
...
def trend_main(models=(), config_file=None, x_axis_mode='n', use_archiving=False, max_buffer_size=None, forced_read_period=-1, demo=False, window_name='TaurusTrend (qwt5)'):
    import sys
    from taurus.qt.qtgui.application import TaurusApplication
