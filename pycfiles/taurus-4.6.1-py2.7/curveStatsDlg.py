...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/qwt5/curveStatsDlg.py
...
class CurveStatsDialog(Qt.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.refreshCurves()
        cbs = (
         self.ui.npointsStatCB, self.ui.minStatCB, self.ui.maxStatCB,
         self.ui.meanStatCB, self.ui.stdStatCB, self.ui.rmsStatCB)
        self._checkboxToColMap = dict(zip(cbs, range(len(self.statColumns))))
        self.minPicker = Qwt5.QwtPlotPicker(Qwt5.QwtPlot.xBottom, Qwt5.QwtPlot.yLeft, Qwt5.QwtPicker.PointSelection, Qwt5.QwtPicker.VLineRubberBand, Qwt5.QwtPicker.AlwaysOn, plot.canvas())
        self.maxPicker = Qwt5.QwtPlotPicker(Qwt5.QwtPlot.xBottom, Qwt5.QwtPlot.yLeft, Qwt5.QwtPicker.PointSelection, Qwt5.QwtPicker.VLineRubberBand, Qwt5.QwtPicker.AlwaysOn, plot.canvas())
        ...
        refreshAction = Qt.QAction(Qt.QIcon.fromTheme('view-refresh'), 'Refresh available curves', self.ui.statsTW)
    ...
    def _QDateTimeToTimestamp(self, qdt):
        return qdt.toTime_t() + qdt.time().msec() / 1000.0
    ...
    def onCalculate(self):
        ...
        if self.ui.minCB.isChecked():
            if plot.getXIsTime():
                xmin = self.ui.minDTE.dateTime().toTime_t() + self.ui.minDTE.time().msec() / 1000.0
            else:
                xmin = self.ui.minSB.value()
        if self.ui.maxCB.isChecked():
            if plot.getXIsTime():
                xmax = self.ui.maxDTE.dateTime().toTime_t() + self.ui.maxDTE.time().msec() / 1000.0
            else:
        ...
        for row, name in zip(selectedRows, selectedCurves):
            ...
            if minxy is None:
                ...
                if plot.xIsTime:
                    text = 't=%s \ny=%g' % (
                     datetime.fromtimestamp(minxy[0]).isoformat(), minxy[1])
                else:
                    text = 'x=%g \ny=%g' % minxy
                ...
                if maxxy is None:
                    ...
                    text = 't=%s \ny=%g' % (
                     datetime.fromtimestamp(maxxy[0]).isoformat(), maxxy[1])
                else:
                    text = 'x=%g \ny=%g' % maxxy
