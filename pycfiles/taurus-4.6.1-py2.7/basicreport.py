...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/report/basicreport.py
# Compiled at: 2019-08-19 15:09:30
"""This module provides a panel to display taurus messages"""
__all__ = [
 'ClipboardReportHandler', 'SMTPReportHandler']
...
class ClipboardReportHandler(TaurusMessageReportHandler):
    ...
    def report(self, message):
        ...
        Qt.QMessageBox.information(None, 'Done!', 'Message Copied to clipboard')
        return
...
class SendMailDialog(Qt.QDialog):
    ...
    def getMailInfo(self):
        return (
         self.getFrom(), self.getTo(), self.getSubject(),
         self.getMessage())
    ...
    def report(self, message):
        ...
        if not dialog.exec_():
            ...
            try:
                ...
                Qt.QMessageBox.information(None, 'Done!', 'Email has been sent!')
            except:
                import sys, traceback
                einfo = sys.exc_info()[:2]
                msg = ('').join(traceback.format_exception_only(*einfo))
                Qt.QMessageBox.warning(None, 'Failed to send email', 'Failed to send email. Reason:\n\n' + msg)
