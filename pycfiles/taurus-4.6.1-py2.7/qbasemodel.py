...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/model/qbasemodel.py
...
__all__ = [
 'QBaseModelWidget', 'TaurusBaseModelWidget',
...
class FilterToolBar(BaseToolBar):
    ...
    def __init__(self, view=None, parent=None, designMode=False):
        BaseToolBar.__init__(self, name='Taurus filter toolbar', view=view, parent=parent, designMode=designMode)
        ...
        af = ActionFactory()
        self._clearFilterAction = af.createAction(self, 'Clear', icon=Qt.QIcon.fromTheme('edit-clear'), tip='Clears the filter', triggered=self.onClearFilter)
...
class EditorToolBar(BaseToolBar):
    ...
    def __init__(self, view=None, parent=None, designMode=False):
        BaseToolBar.__init__(self, name='Taurus editor toolbar', view=view, parent=parent, designMode=designMode)
        af = ActionFactory()
        self._addAction = af.createAction(self, 'New item', icon=Qt.QIcon.fromTheme('list-add'), tip='Add new item', triggered=self.onAdd)
        self._removeAction = af.createAction(self, 'Remove item', icon=Qt.QIcon.fromTheme('list-remove'), tip='Remove item', triggered=self.onRemove)
        self._moveTopAction = af.createAction(self, 'To top', icon=Qt.QIcon.fromTheme('go-top'), tip='Move selected item to top', triggered=self.onMoveTop)
        self._moveUpAction = af.createAction(self, 'Move up', icon=Qt.QIcon.fromTheme('go-up'), tip='Move selected item up one level', triggered=self.onMoveUp)
        self._moveDownAction = af.createAction(self, 'Move down', icon=Qt.QIcon.fromTheme('go-down'), tip='Move selected item down one level', triggered=self.onMoveDown)
        self._moveBottomAction = af.createAction(self, 'To bottom', icon=Qt.QIcon.fromTheme('go-bottom'), tip='Move selected item to bottom', triggered=self.onMoveBottom)
...
class SelectionToolBar(BaseToolBar):
    ...
    def __init__(self, view=None, parent=None, designMode=False):
        BaseToolBar.__init__(self, name='Taurus selection toolbar', view=view, parent=parent, designMode=designMode)
        af = ActionFactory()
        self._selectAllAction = af.createAction(self, 'Select All', icon=Qt.QIcon.fromTheme('edit-select-all'), tip='Select all items', triggered=self.onSelectAll)
        self._clearSelectionAction = af.createAction(self, 'Clear selection', icon=Qt.QIcon.fromTheme('edit-clear'), tip='Clears current selection', triggered=self.onclearSelection)
...
class RefreshToolBar(BaseToolBar):
    ...
    def __init__(self, view=None, parent=None, designMode=False):
        BaseToolBar.__init__(self, name='Taurus refresh toolbar', view=view, parent=parent, designMode=designMode)
        af = ActionFactory()
        self._refreshAction = af.createAction(self, 'Refresh', icon=Qt.QIcon.fromTheme('view-refresh'), tip='Refresh view', triggered=self.onRefresh)
...
class PerspectiveToolBar(BaseToolBar):
    ...
    def __init__(self, perspective, view=None, parent=None, designMode=False):
        ...
        for persp, persp_data in view.KnownPerspectives.items():
            ...
            action = af.createAction(self, label, icon=icon, tip=tip, triggered=self.onSwitchPerspective)
...
class QBaseModelWidget(Qt.QMainWindow):
    ...
    def __init__(self, parent=None, designMode=False, with_filter_widget=True, with_selection_widget=True, with_refresh_widget=True, perspective=None, proxy=None):
    ...
    def setQModel(self, qmodel):
        ...
        if old_selection_model is not None:
            old_selection_model.currentChanged.disconnect(self.viewCurrentIndexChanged)
            old_selection_model.selectionChanged.disconnect(self.viewSelectionChanged)
        view.setModel(qmodel)
        new_selection_model = view.selectionModel()
        if new_selection_model is not None:
            new_selection_model.currentChanged.connect(self.viewCurrentIndexChanged)
            new_selection_model.selectionChanged.connect(self.viewSelectionChanged)
    ...
    def viewCurrentIndexChanged(self, current, previous):
        ...
        self.currentItemChanged.emit(currentTaurusTreeItem, previousTaurusTreeItem)
        return
    ...
    def _setPerspective(self, perspective):
        ...
        qmodel_class, qmodel_proxy_classes = qmodel_classes[(-1)], qmodel_classes[-2::-1]
...
class TaurusBaseModelWidget(TaurusBaseWidget):
    ...
    def __init__(self, designMode=False):
