...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/taurusfactory.py
...
class TaurusFactory(object):
    ...
    def getAuthority(self, name=None):
        ...
        if not v.isValid(name):
            msg = ("Invalid {scheme} authority name '{name}'").format(scheme=self.schemes[0], name=name)
    ...
    def getDevice(self, name, **kw):
        ...
        if not v.isValid(name):
            msg = ("Invalid {scheme} device name '{name}'").format(scheme=self.schemes[0], name=name)
            raise TaurusException(msg)
        fullname, _, _ = v.getNames(name)
    ...
    def getAttribute(self, name):
        ...
        if not v.isValid(name):
            msg = ("Invalid {scheme} attribute name '{name}'").format(scheme=self.schemes[0], name=name)
            raise TaurusException(msg)
        fullname, _, _ = v.getNames(name)
    ...
    def getAuthorityNameValidator(self):
        raise NotImplementedError('getAuthorityNameValidator cannot be called for abstract TaurusFactory')

    def getDeviceNameValidator(self):
        raise NotImplementedError('getDeviceNameValidator cannot be called for abstract TaurusFactory')

    def getAttributeNameValidator(self):
        raise NotImplementedError('getAttributeNameValidator cannot be called for abstract TaurusFactory')
    ...
    def getObject(self, cls, name):
        ...
        if issubclass(cls, TaurusAuthority):
            ...
            if issubclass(cls, TaurusDevice):
                ...
                if issubclass(cls, TaurusAttribute):
                    return self.getAttribute(name)
                if issubclass(cls, TaurusConfiguration):
                    self.deprecated(dep='TaurusConfiguration', alt='TaurusAttribute', rel='4.0', dbg_msg=t4_msg)
                    return self.getAttribute(name)
                if issubclass(cls, TaurusConfigurationProxy):
                    self.deprecated(dep='TaurusConfigurationProxy', alt='TaurusAttribute', rel='4.0', dbg_msg=t4_msg)
                    return self.getAttribute(name)
                return
    ...
    def findObjectClass(self, absolute_name):
        ...
        try:
            ...
            msg = ('generic findObjectClass called but %s does ' + 'not define elementTypesMap.') % self.__class__.__name__
