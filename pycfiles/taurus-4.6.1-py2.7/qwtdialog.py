...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/qwt5/qwtdialog.py
...
class TaurusPlotConfigDialog(Qt.QDialog):
    ...
    def __init__(self, parent=None, flags=Qt.Qt.WindowFlags()):
        ...
        self.ui.buttonBox.button(Qt.QDialogButtonBox.Close).clicked.connect(self.hide)
        self.ui.buttonBox.button(Qt.QDialogButtonBox.Apply).clicked.connect(self.apply)
        ...
        self.curvesAppearanceChooser.assignToY1BT.clicked.connect(partial(self.setCurvesYAxis, curvesNamesList=None, axis=None))
        self.curvesAppearanceChooser.assignToY2BT.clicked.connect(partial(self.setCurvesYAxis, curvesNamesList=None, axis=None))
    ...
    def validate(self):
        ...
        if self.ui.xDynScaleCheckBox.isChecked():
            ...
            if self.parent.getXIsTime():
                ...
                if not (ok1 and ok2) or xMin >= xMax:
                    Qt.QMessageBox.warning(self, 'Invalid parameters for X axis', 'Min value must be strictly less than max value')
                    return False
        if self.ui.y1AutoGroupBox.isChecked():
            ...
            if not (ok1 and ok2) or y1Min >= y1Max:
                Qt.QMessageBox.warning(self, 'Invalid parameters for Y1 axis', 'Min value must be strictly less than max value')
                return False
        if self.ui.y2AutoGroupBox.isChecked():
            ...
            if not (ok1 and ok2) or y2Min >= y2Max:
                Qt.QMessageBox.warning(self, 'Invalid parameters for Y2 axis', 'Min value must be strictly less than max value')
                return False
    ...
    def setXDynScale(self, checked):
        if checked:
            if self.parent.getXIsTime():
                rg = self.deltatime2str(self.parent.getXAxisRange(), fuzzy=True)
            else:
