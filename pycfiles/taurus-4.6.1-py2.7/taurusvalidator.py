...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/taurusvalidator.py
...
__all__ = [
 'TaurusAuthorityNameValidator', 'TaurusDeviceNameValidator',
...
class _TaurusBaseValidator(Singleton):
    ...
    def __init__(self):
        if self.scheme is None:
            msg = 'This is  an abstract name validator class. ' + 'Only scheme-specific derived classes can be instantiated'
    ...
    def namePattern(self):
        ...
        URI segments"""
        return self.pattern % dict(scheme=self.scheme, authority=self.authority, path=self.path, query=self.query, fragment=self.fragment)
    ...
    def _isValidAtLevel(self, name, matchLevel=None):
        msg = 'matchLevel is a Tango-centric concept. Avoid it outside ' + 'the tango scheme'
    ...
    def getParams(self, name):
        msg = '%s.getParams() is deprecated. Use getUriGroups() instead.' % self.__class__.__name__
        from taurus import warning
        warning(msg)
    ...
    def getNames(self, fullname, factory=None):
        ...
        **fullname** URI. If the given URI is valid but it is not the full name,
        ...
        """
        raise NotImplementedError('getNames must be implemented in derived ' + 'classes')
...
class TaurusDatabaseNameValidator(TaurusAuthorityNameValidator):
    ...
    def __init__(self, *args, **kwargs):
        msg = '%s is deprecated. Use "Authority" instead of "Database"' % self.__class__.__name__
        from taurus import warning
        warning(msg)
