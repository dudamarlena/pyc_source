...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/tango/tangoattribute.py
# Compiled at: 2019-08-19 15:09:29
"""This module contains all taurus tango attribute"""
from builtins import range
from builtins import str
import re, time, threading, weakref, PyTango, numpy
from functools import partial
from taurus import Manager
from taurus.core.units import Quantity, UR
from taurus import tauruscustomsettings
from taurus.core.taurusattribute import TaurusAttribute
from taurus.core.taurusbasetypes import TaurusEventType, TaurusSerializationMode, SubscriptionState, TaurusAttrValue, DataFormat, DataType
from taurus.core.taurusoperation import WriteAttrOperation
from taurus.core.util.event import EventListener, _BoundMethodWeakrefWithCall
from taurus.core.util.log import debug, taurus4_deprecation, deprecation_decorator
from taurus.core.tango.enums import EVENT_TO_POLLING_EXCEPTIONS, FROM_TANGO_TO_NUMPY_TYPE, DevState
from .util.tango_taurus import description_from_tango, display_level_from_tango, quality_from_tango, standard_display_format_from_tango, quantity_from_tango_str, data_format_from_tango, data_type_from_tango
__all__ = [
 'TangoAttribute', 'TangoAttributeEventListener', 'TangoAttrValue']
...
class TangoAttrValue(TaurusAttrValue):
    ...
    def __init__(self, attr=None, pytango_dev_attr=None, config=None):
        ...
        if p is None:
            ...
            elif p.is_empty and self._attrRef.data_format != DataFormat._0D:
...
class TangoAttribute(TaurusAttribute):
        ...
        with self.__read_lock:
            try:
                if single:
                    ...
                    filter_old_event = getattr(tauruscustomsettings, 'FILTER_OLD_TANGO_EVENTS', False)
                    if self.__attr_value is not None and filter_old_event and time is not None and time < self.__attr_value.time.totime():
                        return
        ...
        if cache:
            ...
            if dt < self.getPollingPeriod():
                ...
                if self.__attr_err is not None:
                    self.debug('[Tango] read from cache failed (%s): %s', self.fullname, self.__attr_err)
                    raise self.__attr_err
        if not cache or self.__subscription_state in (
         SubscriptionState.PendingSubscribe,
         SubscriptionState.Unsubscribed) and not self.isPollingActive():
            with self.__read_lock:
                try:
                    dev = self.getParentObj()
    ...
    def addListener(self, listener):
        ...
        if len(listeners) > 1 and (initial_subscription_state == SubscriptionState.Subscribed or self.isPollingActive()):
            ...
            if sm == TaurusSerializationMode.TangoSerial:
                ...
                Manager().enqueueJob(job, job_args=(
                 (
    ...
    def _call_dev_hw_subscribe_event(self, stateless=True):
        ...
        if self.__chg_evt_id is not None:
            ...
            attr_name = self.getSimpleName()
            self.__chg_evt_id = self.__dev_hw_obj.subscribe_event(attr_name, PyTango.EventType.CHANGE_EVENT, _BoundMethodWeakrefWithCall(self.push_event), [], stateless)
    ...
    def _subscribeConfEvents(self):
        ...
        if self.__cfg_evt_id is not None:
            ...
            attr_name = self.getSimpleName()
            try:
                self.__cfg_evt_id = self.__dev_hw_obj.subscribe_event(attr_name, PyTango.EventType.ATTR_CONF_EVENT, _BoundMethodWeakrefWithCall(self.push_event), [], True)
    ...
    def _unsubscribeConfEvents(self):
        if self.__cfg_evt_id is not None and self.__dev_hw_obj is not None:
            self.trace('Unsubscribing to configuration events (ID=%s)', str(self.__cfg_evt_id))
            try:
    ...
    def subscribePendingEvents(self):
        ...
        """
        if self.__subscription_state == SubscriptionState.Unsubscribed or self.isPollingActive():
            self.__subscription_state = SubscriptionState.PendingSubscribe
        self._subscribeConfEvents()
        self._call_dev_hw_subscribe_event(True)
    ...
    def push_event(self, event):
        ...
        with self.__read_lock:
            ...
            if sm == TaurusSerializationMode.TangoSerial:
                ...
                manager.enqueueJob(job, job_args=(etype, evalue), job_kwargs={'listeners': listeners}, serialization_mode=sm)
        return
    ...
    def _pushAttrEvent(self, event):
        ...
        if not event.err:
            ...
            if self.__attr_value is not None and self.__subscription_state == SubscriptionState.Subscribed and filter_old_event and time < self.__attr_value.time.totime():
                return [None, None]
            ...
            if event.errors[0].reason in EVENT_TO_POLLING_EXCEPTIONS:
