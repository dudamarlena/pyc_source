...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/util/event.py
...
import sys, weakref, threading, time, collections, taurus.core
__all__ = [
...
class _BoundMethodWeakrefWithCall(BoundMethodWeakref):
    ...
    def __init__(self, bound_method, del_cb=None):
        ...
        super(_BoundMethodWeakrefWithCall, self).__init__(bound_method, del_cb=del_cb)
...
class EventGenerator(Object):
    ...
    def subscribeEvent(self, cb, data=None, with_first_event=True):
        ...
        if not self.events_active:
            raise RuntimeError('%s does not have events/polling active' % self.event_name)
    ...
    def waitEvent(self, val=None, equal=True, any=False, timeout=None, stack=None):
        ...
        try:
            ...
            while True:
                ...
                if any:
                    ...
                    block = equal and val != curr_val or not equal and val == curr_val
                if timeout:
...
class AttributeEventWait(object):
    ...
    def unlock(self):
        ...
        if self._cond._is_owned():
            ...
            if th is not None:
                ...
                name = '<unknown>'
            print('WARNING: Thread %s trying to unlock condition previously locked by thread %s' % (
             curr_th.name, name))
        return
    ...
    def eventReceived(self, s, t, v):
        ...
        if t == taurus.core.taurusbasetypes.TaurusEventType.Config:
            ...
            if t == taurus.core.taurusbasetypes.TaurusEventType.Error:
                self.fireEvent(None)
            else:
    ...
    def waitEvent(self, val, after=0, equal=True, timeout=None, retries=-1, any=False):
        ...
        if retries == 0:
            ...
            try:
                try:
                    ...
                    sys.stderr.write('AttributeEventWait: Caught exception while waiting: %s\n' % str(e))
                    raise e
...
class AttributeEventIterator(object):
    ...
    def unlock(self):
        if self._cond._is_owned():
            ...
            th = getattr(lock, '_RLock__owner')
            curr_th = threading.current_thread()
            print(('WARNING: Thread %s trying to unlock condition previously ' + 'locked by thread %s') % (curr_th.name, th.name))
