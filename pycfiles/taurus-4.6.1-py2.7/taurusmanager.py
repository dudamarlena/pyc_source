...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/taurusmanager.py
# Compiled at: 2019-08-19 15:09:29
"""This module contains the taurus base manager class"""
from __future__ import print_function
from builtins import range
import os, atexit, pkg_resources
...
class TaurusManager(Singleton, Logger):
    ...
    def reInit(self):
        ...
        if self._state == ManagerState.INITED:
            ...
            self._thread_pool = ThreadPool(name='TaurusTP', parent=self, Psize=5, Qsize=1000)
            self._sthread_pool = ThreadPool(name='TaurusTSP', parent=self, Psize=1, Qsize=0)
            self._plugins = None
    ...
    def enqueueJob(self, job, callback=None, job_args=(), job_kwargs=None, serialization_mode=None):
        ...
        if serialization_mode == TaurusSerializationMode.Concurrent:
            ...
            if not hasattr(self, '_sthread_pool') or self._sthread_pool is None:
                self.info('Job cannot be processed.')
                self.debug('The requested job cannot be processed. ' + 'Make sure this manager is initialized')
                return
            self._sthread_pool.add(job, callback, *job_args, **job_kwargs)
        else:
            raise TaurusException(('{} serialization mode not supported').format(serialization_mode))
        return
        ...
        for plugin_class in plugin_classes:
            ...
            for scheme in schemes:
                if scheme in plugins:
                    k = plugins[scheme]
                    self.warning('Conflicting plugins: %s and %s both implement scheme %s. Will keep using %s' % (
                     k.__name__,
    ...
    def _get_plugin_classes(self):
        ...
        full_module_names.extend(getattr(taurus.core, 'PLUGIN_SCHEME_MODULES', []))
        ...
        for full_module_name in full_module_names:
            ...
            for s in m.__dict__.values():
                plugin = None
                try:
                    if issubclass(s, TaurusFactory) and issubclass(s, Singleton):
