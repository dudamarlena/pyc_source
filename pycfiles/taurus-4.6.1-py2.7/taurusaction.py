...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/util/taurusaction.py
...
class ExternalAppAction(Qt.QAction, BaseConfigurableClass):
    ...
    def actionTriggered(self, args=None):
        ...
        try:
            ...
            msg = 'Cannot launch application:\n' + (' ').join(self.__cmdargs) + '\nHint: Check that %s is installed and in the path' % str(self.text())
            if self.interactive:
...
class TaurusAction(Qt.QAction):
    ...
    def __init__(self, parent):
        ...
        if parent is None or not isinstance(parent, Qt.QWidget) or not isinstance(parent, taurus.qt.qtgui.base.TaurusBaseComponent):
