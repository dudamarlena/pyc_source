...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusvalue.py
# Compiled at: 2019-08-19 15:09:30
"""
taurusvalue.py:
"""
__all__ = [
 'TaurusValue', 'TaurusValuesFrame', 'DefaultTaurusValueCheckBox',
...
class DefaultLabelWidget(TaurusLabel):
    ...
    _BCK_COMPAT_TAGS = {'<attr_name>': '{attr.name}', '<attr_fullname>': '{attr.fullname}', 
       '<dev_alias>': '{dev.name}', 
       '<dev_name>': '{dev.name}', 
...
class TaurusValue(Qt.QWidget, TaurusBaseWidget):
        ...
        if modeltype == TaurusElementType.Attribute:
            try:
                if modelobj.isBoolean():
                    result = [
                     CenteredLed, DefaultReadWidgetLabel]
            except:
                pass
            ...
            if modelobj.data_format == DataFormat._0D:
                if modelobj.type == DataType.Boolean:
                    ...
                    result = [
                     TaurusStatusLabel, DefaultReadWidgetLabel]
                else:
                    result = [
    ...
    def getDefaultWriteWidgetClass(self, returnAll=False):
        ...
        if modelobj.data_format == DataFormat._0D:
            if modelType == DataType.Boolean:
                result = [
                 DefaultTaurusValueCheckBox, TaurusValueLineEdit]
            else:
                result = [
                 UnitLessLineEdit,
            ...
            self.debug('Unsupported attribute type for writing: %s' % str(DataType.whatis(modelType)))
            result = [None]
        if returnAll:
            return result
        else:
    ...
    def addLabelWidgetToLayout(self):
        if self._labelWidget is not None and self.parent() is not None:
            alignment = getattr(self._labelWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
            self.parent().layout().addWidget(self._labelWidget, self._row, 1, 1, 1, alignment)
        return

    def addReadWidgetToLayout(self):
        if self._readWidget is not None and self.parent() is not None:
            alignment = getattr(self._readWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
            if self._writeWidget is None:
                self.parent().layout().addWidget(self._readWidget, self._row, 2, 1, 2, alignment)
            else:
                self.parent().layout().addWidget(self._readWidget, self._row, 2, 1, 1, alignment)
        return

    def addWriteWidgetToLayout(self):
        if self._writeWidget is not None and self.parent() is not None:
            alignment = getattr(self._writeWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
            self.parent().layout().addWidget(self._writeWidget, self._row, 3, 1, 1, alignment)
        return

    def addUnitsWidgetToLayout(self):
        if self._unitsWidget is not None and self.parent() is not None:
            alignment = getattr(self._unitsWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
            self.parent().layout().addWidget(self._unitsWidget, self._row, 4, 1, 1, alignment)
        return

    def addCustomWidgetToLayout(self):
        if self._customWidget is not None and self.parent() is not None:
            alignment = getattr(self._customWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
            self.parent().layout().addWidget(self._customWidget, self._row, 1, 1, -1, alignment)
        return
    ...
    def addExtraWidgetToLayout(self):
        ...
        if parent is not None:
            if self._extraWidget is None:
                ...
                alignment = getattr(self._extraWidget, 'layoutAlignment', Qt.Qt.AlignmentFlag(0))
                parent.layout().addWidget(self._extraWidget, self._row, 5, 1, 1, alignment)
        return
    ...
    def createConfig(self, allowUnpickable=False):
        ...
        for key in ('LabelWidget', 'ReadWidget', 'WriteWidget', 'UnitsWidget', 'CustomWidget',
            ...
            if isinstance(classID, string_types) or allowUnpickable:
    ...
    def updatePendingOpsStyle(self):
        if self._labelWidget is None:
            ...
            if self.hasPendingOperations():
                newstyle = '%s {border-style: solid ; border-width: 1px; border-color: blue; color: blue; border-radius:4px;}' % self._labelWidget.__class__.__name__
                oldstyle = self._labelWidget.styleSheet()
                if newstyle != oldstyle:
                    self._labelWidget.setStyleSheet(newstyle)
            else:
                newstyle = '%s {border-style: solid; border-width: 1px; border-color: transparent; color: black;  border-radius:4px;}' % self._labelWidget.__class__.__name__
                oldstyle = self._labelWidget.styleSheet()
                if newstyle != oldstyle:
                    self._labelWidget.setStyleSheet(newstyle)
            return
...
if __name__ == '__main__':
    ...
    try:
        ...
        container.setCustomWidgetMap({'SimuMotor': PoolMotorSlim, 'Motor': PoolMotorSlim, 
