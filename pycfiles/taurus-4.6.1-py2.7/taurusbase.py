...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/base/taurusbase.py
...
__all__ = [
 'TaurusBaseComponent', 'TaurusBaseWidget',
...
class TaurusBaseComponent(TaurusListener, BaseConfigurableClass):
    ...
    FORMAT = None
    defaultFormatDict = {float: '{:.{bc.modelObj.precision}f}', Enum: '{0.name}', 
    ...
    def __init__(self, name='', parent=None, designMode=False, **kwargs):
        ...
        if self.FORMAT is None:
            self.setFormat(getattr(taurus.tauruscustomsettings, 'DEFAULT_FORMATTER', defaultFormatter))
    ...
    def contextMenuEvent(self, event):
        ...
        if self.taurusMenu is not None:
            if self.__explicitPopupMenu:
                self.taurusMenu.exec_(event.globalPos())
                return
            self.deprecated(dep='taurusMenu context Menu API', alt='custom contextMenuEvent to show taurusMenu', rel='4.5.3a')
        event.ignore()
        return
    ...
    def filterEvent(self, evt_src=-1, evt_type=-1, evt_value=-1):
        ...
        if evt == (-1, -1, -1):
            ...
            evt = filterEvent(filters=self._eventFilters, *evt)
            if evt is not None:
    ...
    def displayValue(self, v):
        ...
        if self._format is None:
            try:
                ...
                self.warning('Cannot update format. Reverting to default.' + ' Reason: %r', e)
...
class TaurusBaseWidget(TaurusBaseComponent):
    ...
    def showFormatterDlg(self):
        ...
        formatter, ok = Qt.QInputDialog.getText(self, 'Set formatter', 'Enter a formatter:', Qt.QLineEdit.Normal, current_format)
        if ok and formatter:
            return formatter
        else:
            return
        ...
        if self.getUseParentModel():
            ...
            if evt_type == Qt.QEvent.ParentChange:
                if self._parentTaurusComponent:
                    self._parentTaurusComponent.modelChanged.disconnect(self.parentModelChanged)
                self._updateUseParentModel(True)
                self.setModelCheck(self.getModel(), False)
    ...
    def mouseMoveEvent(self, event):
        ...
        if (event.pos() - self.dragStartPosition).manhattanLength() < Qt.QApplication.startDragDistance():
            return self.getQtClass().mouseMoveEvent(self, event)
        ret = self.getQtClass().mouseMoveEvent(self, event)
        event.accept()
        drag = Qt.QDrag(self)
    ...
    def safeApplyOperations(self, ops=None):
        ...
        if self.getForceDangerousOperations():
            ...
            dangerMsgs = [ op.getDangerMessage() for op in ops if len(op.getDangerMessage()) > 0
                         ]
        if len(dangerMsgs) == 1:
            result = Qt.QMessageBox.warning(self, 'Potentially dangerous action', '%s\nProceed?' % dangerMsgs[0], Qt.QMessageBox.Ok | Qt.QMessageBox.Cancel, Qt.QMessageBox.Ok)
            if result != Qt.QMessageBox.Ok:
                return False
        elif len(dangerMsgs) > 1:
            warningDlg = Qt.QMessageBox(Qt.QMessageBox.Warning, ' %d potentially dangerous actions' % len(dangerMsgs), 'You are about to apply %d actions that may be potentially dangerous. Proceed?' % len(dangerMsgs), Qt.QMessageBox.Ok | Qt.QMessageBox.Cancel, self)
            details = ('\n').join(dangerMsgs)
    ...
    def getQtDesignerPluginInfo(cls):
        ...
        return {'group': 'Taurus [Unclassified]', 
           'icon': 'logos:taurus.png', 
           'container': False}
