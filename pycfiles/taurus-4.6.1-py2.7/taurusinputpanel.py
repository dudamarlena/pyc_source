...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusinputpanel.py
...
class TaurusInputPanel(Qt.QWidget):
    ...
    def create_single_input_panel(self, input_data):
        ...
        is_seq = not isinstance(data_type, string_types) and isinstance(data_type, collections.Sequence)
        if is_seq:
    ...
    def _create_combobox_panel(self, input_data):
        ...
        for item in items:
            is_seq = not isinstance(item, string_types) and isinstance(item, collections.Sequence)
            if is_seq:
                text, userData = item
            else:
    ...
    def _create_radiobutton_panel(self, input_data):
        ...
        return (
         panel, self._get_radiobutton_value)
    ...
    def _create_multi_selection_panel(self, input_data):
        ...
        if default_value is None:
            default_value = ()
        dft_is_seq = not isinstance(default_value, string_types) and isinstance(default_value, collections.Sequence)
        if not dft_is_seq:
            default_value = (
             default_value,)
...
def main():
    ...
    d = dict(prompt="What's your favourite car brand?", data_type=[
     'Mazda', 'Skoda', 'Citroen',
