...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/extra_guiqwt/plot.py
...
__all__ = [
 'TaurusCurveDialog', 'TaurusTrendDialog', 'TaurusImageDialog']
...
def taurusTrendDlgMain():
    from taurus.qt.qtgui.application import TaurusApplication
    import taurus.core, sys
    parser = taurus.core.util.argparse.get_taurus_parser()
    parser.set_usage('%prog [options] <model>')
    parser.set_description('a Taurus application for plotting trends of scalars')
    parser.add_option('-x', '--x-axis-mode', dest='x_axis_mode', default='t', metavar='t|d|e', help='interpret X values as timestamps (t), time deltas (d) or event numbers (e). Accepted values: t|d|e')
    parser.add_option('-b', '--buffer', dest='max_buffer_size', default='10000', help='maximum number of values to be plotted (when reached, the oldest values will be discarded)')
...
@click.option('-c', '--color-mode', 'color_mode', type=click.Choice(['gray', 'rgb']), default='gray', show_default=True, help='Color mode expected from the attribute')
...
def image_cmd(model, color_mode, demo, window_name):
    from taurus.qt.qtgui.application import TaurusApplication
    import sys
    app = TaurusApplication(cmd_line_parser=None, app_name='Taurus Image Dialog')
