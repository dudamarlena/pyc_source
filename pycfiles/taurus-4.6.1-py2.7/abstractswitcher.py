...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/compact/abstractswitcher.py
...
class TaurusReadWriteSwitcher(TaurusWidget):
    ...
    writeWClass = None
    enterEditTriggers = (
     Qt.Qt.Key_F2, Qt.QEvent.MouseButtonDblClick)
    exitEditTriggers = (Qt.Qt.Key_Escape, Qt.QEvent.FocusOut, 'applied')

    def __init__(self, parent=None, designMode=False, readWClass=None, writeWClass=None, enterEditTriggers=None, exitEditTriggers=None):
        ...
        for sig in self.exitEditSignals:
            try:
                ...
                if isinstance(e, AttributeError) and hasattr(Qt, 'SIGNAL'):
                    self.connect(self.writeWidget, Qt.SIGNAL(sig), self.exitEdit)
                    self.debug('Cannot connect %s using new style signal.' + 'Falling back to old style', sig)
                else:
                    self.debug('Cannot connect signal. Reason: %s', e)
        ...
        for w in (self.readWidget, self.writeWidget):
            if w is not None:
                ...
                if policy is None:
                    ...
                    policy = Qt.QSizePolicy(Qt.QSizePolicy.Policy(h), Qt.QSizePolicy.Policy(v))
...
def demo3():
    ...
    app = TaurusApplication(cmd_line_parser=None)
    w1 = TaurusReadWriteSwitcher(readWClass=TaurusLabel, writeWClass=TaurusValueLineEdit)
    w1.model = 'sys/tg_test/1/long_scalar'
    w2 = TaurusReadWriteSwitcher(readWClass=TaurusLed, writeWClass=TaurusValueCheckBox)
    w2.model = 'sys/tg_test/1/boolean_scalar'
    f = Qt.QWidget()
    f.setLayout(Qt.QVBoxLayout())
    f.layout().addWidget(w1)
    f.layout().addWidget(w2)
    f.layout().addWidget(TaurusReadWriteSwitcher())
    f.show()
    sys.exit(app.exec_())
    return
