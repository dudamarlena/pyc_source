...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/util/argparse/taurusargparse.py
...
__log.deprecated(dep='taurus.core.util.argparse', rel='4.5.4', alt='argparse or (better) click')
...
def get_taurus_parser(parser=None):
    ...
    if g is None:
        group = optparse.OptionGroup(parser, 'Taurus Options', 'Basic options present in any taurus application')
        help_tauruslog = 'taurus log level. Allowed values are (case insensitive): critical, error, warning/warn, info, debug, trace'
        ...
        help_taurusserial = 'taurus serialization mode. Allowed values are (case insensitive): serial, concurrent (default)'
        help_rcport = 'enables remote debugging using the given port'
        help_formatter = 'Override the default formatter'
        group.add_option('--taurus-log-level', dest='taurus_log_level', metavar='LEVEL', help=help_tauruslog, type='str', default='info')
        group.add_option('--taurus-polling-period', dest='taurus_polling_period', metavar='MILLISEC', help=help_tauruspolling, type='int', default=None)
        group.add_option('--taurus-serialization-mode', dest='taurus_serialization_mode', metavar='SERIAL', help=help_taurusserial, type='str', default='Concurrent')
        group.add_option('--tango-host', dest='tango_host', metavar='TANGO_HOST', help=help_tangohost, type='str', default=None)
        group.add_option('--remote-console-port', dest='remote_console_port', metavar='PORT', help=help_rcport, type='int', default=None)
        group.add_option('--default-formatter', dest='default_formatter', metavar='FORMATTER', help=help_formatter, type='str', default=None)
...
def init_taurus_args(parser=None, args=None, values=None):
    ...
    if options.default_formatter is not None:
        ...
        setattr(tauruscustomsettings, 'DEFAULT_FORMATTER', options.default_formatter)
    return (parser, options, args)
