...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/taurusgui/appsettingswizard.py
...
import os, re, sys, shutil, copy, datetime, glob
from lxml import etree
from taurus import tauruscustomsettings, warning
from taurus.external.qt import Qt, compat
import taurus.qt.qtgui.panel, taurus.qt.qtgui.taurusgui.paneldescriptionwizard, taurus.qt.qtgui.input
from taurus.core.util.enumeration import Enumeration
from taurus.qt.qtgui.util import ExternalAppAction
__all__ = [
 'AppSettingsWizard', 'ExternalAppEditor']
...
class IntroPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self.setPixmap(Qt.QWizard.WatermarkPixmap, Qt.QIcon.fromTheme('document-properties').pixmap(120, 120))
    ...
    def getIntroText(self):
        text = 'This wizard will guide you through the process of creating a ' + 'GUI based on TaurusGUI.\n' + 'TaurusGui-based applications are very customizable. The user can ' + 'add/remove elements at run time and store those customizations. So ' + 'with this wizard you will define just the default contents of the GUI.'
        return text
...
class ProjectPage(BasePage):
    ...
    def _setupUI(self):
        ...
        self._projectDirBT = Qt.QPushButton(Qt.QIcon.fromTheme('document-properties'), '...')
    ...
    def onSelectDir(self):
        dirname = str(Qt.QFileDialog.getExistingDirectory(self, 'Choose the project directory', self._projectDirLE.text()))
        if not dirname:
            return
    ...
    def validatePage(self):
        ...
        if not os.path.exists(dirname):
            try:
                ...
                Qt.QMessageBox.warning(self, 'Error creating project directory', 'Could not create the project directory.\nReason:%s' % repr(e), Qt.QMessageBox.Cancel)
                return False

        configs_found = glob.glob(os.path.join(dirname, 'tgconf_*', self.wizard().getXmlConfigFileName()))
        if len(configs_found) == 1:
            fname = configs_found[0]
            option = Qt.QMessageBox.question(self, 'Overwrite project?', 'The "%s" file already exists in the project directory.\n Do you want to edit the existing project?' % os.path.basename(fname), Qt.QMessageBox.Yes | Qt.QMessageBox.Cancel)
            ...
            option = Qt.QMessageBox.question(self, 'Non empty project dir', 'The project directory ("%s") is not empty.\nAre you sure you want to use it?' % os.path.basename(dirname), Qt.QMessageBox.Yes | Qt.QMessageBox.No)
            if option != Qt.QMessageBox.Yes:
                return False
        return True
...
class GeneralSettings(BasePage):
    ...
    def _getOrganizationNames(self):
        return [
         'TAURUS', 'ALBA', 'DESY', 'Elettra', 'ESRF', 'MAX-lab', 'SOLEIL', 'XFEL']
...
class CustomLogoPage(BasePage):
    ...
    def __init__(self, parent=None):
        BasePage.__init__(self, parent)
        self._customLogoDefaultPath = getattr(tauruscustomsettings, 'ORGANIZATION_LOGO', 'logos:taurus.png')
    ...
    def _setupUI(self):
        ...
        self._customLogoRemoveButton.setIcon(Qt.QIcon('emblems:emblem-unreadable.svg'))
    ...
    def _setNoImage(self):
        self._customLogo.setPixmap(Qt.QIcon.fromTheme('image-missing').pixmap(50, 50))
    ...
    def _selectImage(self):
        fileName, _ = compat.getOpenFileName(self, self.tr('Open File'), Qt.QDir.homePath(), self.tr('Images (*.png *.xpm *.jpg *.jpeg *.svg)'))
    ...
    def _setImage(self, image):
        if type(image) == Qt.QPixmap:
            ...
            self._customLogo.setPixmap(Qt.QPixmap('image-missing').scaled(50, 50))
            self._customLogoPath = None
        return
...
class SynopticPage(BasePage):
    ...
    def _addSynoptic(self):
        ...
        fileNames, _ = compat.getOpenFileNames(self, self.tr('Open File'), pdir, self.tr('JDW (*.jdw );; All files (*)'))
...
class MacroServerInfoPage(BasePage):
    ...
    def _setupUI(self):
        ...
        from sardana.taurus.qt.qtgui.extra_macroexecutor.common import TaurusMacroConfigurationDialog
        ...
        self.setStatusLabelPalette(self._status_label)
        self._layout.addWidget(self._status_label, 9, 0, 1, 1)
        self._confWidget.macroServerComboBox.currentIndexChanged.connect(self.checkData)
        self._confWidget.doorComboBox.currentIndexChanged.connect(self.checkData)
    ...
    def _getDoorName(self):
        if self._macroGroupBox.isChecked() and len(self._confWidget.macroServerComboBox.currentText()):
            ...
            complete_name, _, _ = TangoDeviceNameValidator().getNames(door_name)
            return complete_name
        else:
            return
            return
...
class ExternalAppEditor(Qt.QDialog):
    ...
    def _selectExecFile(self):
        filePath, _ = compat.getOpenFileName(self, self.tr('Open File'), Qt.QDir.homePath(), self.tr('All files (*)'))
        if len(filePath):
...
class OutroPage(BasePage):
    ...
    def saveFile(self, fileName):
        ...
        if not file.open(Qt.QFile.WriteOnly | Qt.QFile.Text):
            Qt.QMessageBox.warning(self, self.tr('Saving XML...'), self.tr('Cannot write file %1:\n%2.').arg(fileName).arg(file.errorString()))
            return False
    ...
    def validatePage(self):
        try:
            ...
            Qt.QMessageBox.warning(self, 'Error creating project', 'Could not create project files. \nReason:%s' % repr(e), Qt.QMessageBox.Cancel)
            import traceback
            traceback.print_exc()
            return False
    ...
    def createProject(self):
        ...
        for i in range(self._substTable.rowCount()):
            ...
            dst = os.path.join(install_dir, str(self._substTable.item(i, 1).text()))
            if os.path.normpath(src) != os.path.normpath(dst):
                shutil.copy(src, dst)
                logfile.write('File copied: %s --> %s\n' % (src, dst))

        xmlcfgfilename = os.path.join(install_dir, self.wizard().getXmlConfigFileName())
        f = open(xmlcfgfilename, 'w')
        f.write(str(self._xml.toPlainText()))
        f.close()
        logfile.write('XML Config file created: "%s"\n' % xmlcfgfilename)
        pycfgfilename = os.path.join(install_dir, '%s.py' % self.wizard().getConfigFilePrefix())
        f = open(pycfgfilename, 'w')
        f.write("XML_CONFIG = '%s'" % self.wizard().getXmlConfigFileName())
        f.close()
        logfile.write('Python config file created: "%s"\n' % pycfgfilename)
        init_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'res', 'init.template')
        ...
        setup_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'res', 'setup.template')
        ...
        f.write(template)
        f.close()
        manifest_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'res', 'manifest.template')
...
class AppSettingsWizard(Qt.QWizard):
    ...
    user will find when launching the GUI for the first time.
    """
    Pages = Enumeration('Pages', ('IntroPage', 'ProjectPage', 'GeneralSettings', 'CustomLogoPage',
                                  'SynopticPage', 'MacroServerInfo', 'InstrumentsPage',
                                  'PanelsPage', 'ExternalAppPage', 'MonitorPage',
                                  'OutroPage'))
    ...
    def generateXml(self):
        ...
        mod_dir = os.path.join(self.__getitem__('projectDir'), 'tgconf_' + guiName.text)
        ...
        if synopticList:
            ...
            for src in synopticList:
                ...
                child = etree.SubElement(synoptics, 'synoptic', str=os.path.basename(dst))
                f = open(src, 'r')
                contents = f.read()
                f.close()
                for ref in re.findall('file_name:\\"(.+?)\\"', contents):
                    ...
                    if ref != refdst:
                        ...
                        _long = 'The synoptic file "%s" references a file that has been copied to the project dir in order to make the project portable. Please edit "%s" and replace "%s" by "%s"' % (
                         dst, dst, ref, refdst)
