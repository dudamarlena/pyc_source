...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/dialog/taurusmessagebox.py
...
__all__ = [
 'TaurusMessageBox', 'protectTaurusMessageBox',
...
class TaurusMessageBox(Qt.QDialog):
    ...
    def __init__(self, err_type=None, err_value=None, err_traceback=None, parent=None, designMode=False):
        ...
        self._panel = MsgPanel(err_type, err_value, err_traceback, self, designMode)
...
def _ProtectMessageBox(err_type=None, err_value=None, err_traceback=None, parent=None):
    global _PROTECT_MESSAGE_BOX
    box = _PROTECT_MESSAGE_BOX
    if box is None:
        _PROTECT_MESSAGE_BOX = box = TaurusMessageBox(err_type=err_type, err_value=err_value, err_traceback=err_traceback, parent=parent)
        box.panel().setCheckBoxVisible(True)
    else:
        box.setError(err_type=err_type, err_value=err_value, err_traceback=err_traceback)
...
class TaurusExceptHookMessageBox(BaseExceptHook):
    ...
    def _getMessageBox(self, err_type=None, err_value=None, err_traceback=None, parent=None):
        box = self.__class__.MSG_BOX
        if box is None:
            self.__class__.MSG_BOX = box = TaurusMessageBox(err_type=err_type, err_value=err_value, err_traceback=err_traceback, parent=parent)
            box.panel().setCheckBoxVisible(True)
        else:
            box.setError(err_type=err_type, err_value=err_value, err_traceback=err_traceback)
        if box.panel().checkBoxState() == Qt.Qt.Checked:
