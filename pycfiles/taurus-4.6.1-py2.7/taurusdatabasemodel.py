...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtcore/model/taurusdatabasemodel.py
...
__all__ = [
 'TaurusTreeDevicePartItem', 'TaurusTreeDeviceDomainItem',
...
class TaurusTreeSimpleDeviceItem(TaurusTreeDbBaseItem):
    ...
    def data(self, index):
        ...
        if role == ElemType.Device or role == ElemType.Name:
            return obj.name()
        if role == ElemType.DeviceAlias:
            return obj.alias()
        if role == ElemType.Server:
            return obj.server().name()
        if role == ElemType.DeviceClass:
            return obj.klass().name()
        if role == ElemType.Exported:
            return obj.state()
        if role == ElemType.Host:
            return obj.host()
        if role == ElemType.Domain:
            return obj.domain()
        if role == ElemType.Family:
            return obj.family()
        if role == ElemType.Member:
            return obj.member()
...
class TaurusTreeAttributeItem(TaurusTreeDbBaseItem):
    ...
    def toolTip(self, index):
        if index.column() > 0:
            ...
            items = dict(name="'" + di.label + "' (" + data.name() + ')', description=di.description.replace('<', '&lt;').replace('>', '&gt;'), unit=di.unit, limits='[%s, %s]' % (di.min_value, di.max_value), alarms='[%s, %s]' % (di.alarms.min_alarm, di.alarms.max_alarm), warnings='[%s, %s]' % (di.alarms.min_warning, di.alarms.max_warning))
...
class TaurusTreeServerItem(TaurusTreeDbBaseItem):
    ...
    def data(self, index):
        ...
        if role == ElemType.ServerName:
            return self._itemData.serverName()
        if role == ElemType.ServerInstance:
...
class TaurusDbBaseModel(TaurusBaseModel):
    ...
    def mimeTypes(self):
        return [
         'text/plain', taurus.qt.qtcore.mimetypes.TAURUS_MODEL_LIST_MIME_TYPE, taurus.qt.qtcore.mimetypes.TAURUS_MODEL_MIME_TYPE]
    ...
    def mimeData(self, indexes):
        ...
        if len(data) == 1:
            ret.setData(taurus.qt.qtcore.mimetypes.TAURUS_MODEL_MIME_TYPE, data[0])
        return ret
...
class TaurusDbPlainDeviceModel(TaurusDbBaseModel):
    ...
    ColumnNames = ('Device', 'Alias', 'Server', 'Class', 'Alive', 'Host')
    ColumnRoles = (
     (ElemType.Device, ElemType.Device,
...
class TaurusDbDeviceModel(TaurusDbBaseModel):
    ...
    ColumnRoles = (
     (
      ElemType.Device, ElemType.Domain, ElemType.Family, ElemType.Member,
      ElemType.Attribute), ElemType.DeviceAlias, ElemType.Server, ElemType.DeviceClass, ElemType.Exported, ElemType.Host)
...
class TaurusDbServerModel(TaurusDbBaseModel):
    ...
    ColumnNames = ('Server', 'Alive', 'Host')
    ColumnRoles = (
     (ElemType.Server, ElemType.ServerName, ElemType.ServerInstance,
...
class TaurusDbDeviceClassModel(TaurusDbBaseModel):
    ...
    ColumnRoles = (
     (ElemType.DeviceClass, ElemType.DeviceClass,
...
class TaurusDbDeviceProxyModel(TaurusDbBaseProxyModel):
    ...
    def filterAcceptsRow(self, sourceRow, sourceParent):
        ...
        if isinstance(treeItem, TaurusTreeDeviceFamilyItem):
            ...
            return False
        if isinstance(treeItem, TaurusTreeDeviceItem) or isinstance(treeItem, TaurusTreeSimpleDeviceItem) or isinstance(treeItem, TaurusTreeDeviceMemberItem):
