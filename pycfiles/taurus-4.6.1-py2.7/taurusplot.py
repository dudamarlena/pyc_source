...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/qwt5/taurusplot.py
...
from taurus.qt.qtgui.qwt5 import TaurusPlotConfigDialog, FancyScaleDraw, DateTimeScaleEngine, FixedLabelsScaleEngine, FixedLabelsScaleDraw
from .curvesAppearanceChooserDlg import CurveAppearanceProperties
__all__ = [
 'TaurusCurve', 'TaurusCurveMarker',
...
def isodatestr2float(s, sep='_'):
    ...
    return time.mktime(d.timetuple()) + d.microsecond * 1e-06
...
class TaurusCurveMarker(Qwt5.QwtPlotMarker, TaurusBaseComponent):
    ...
    def __init__(self, name, parent=None, labelOpacity=0.7):
        ...
        text.setBackgroundBrush(Qt.QBrush(Qt.QColor(0, 255, 0, int(255 * labelOpacity))))
        self.setLabel(text)
        self.setSymbol(Qwt5.QwtSymbol(Qwt5.QwtSymbol.Diamond, Qt.QBrush(Qt.Qt.yellow), Qt.QPen(Qt.Qt.green), Qt.QSize(7, 7)))
...
class TaurusCurve(Qwt5.QwtPlotCurve, TaurusBaseComponent):
    ...
    **Important**:
    ...
    def __init__(self, name, xname=None, parent=None, rawData=None, optimized=False):
        ...
        self.isRawData = rawData is not None
    ...
    def compileTitleText(self, titletext):
        ...
        *Note*: Some placeholders may not make sense for certain curves (e.g.
        ...
        if dev is not None:
            titletext = titletext.replace('<dev_alias>', dev.getSimpleName() or '---')
            titletext = titletext.replace('<dev_name>', dev.getNormalName() or '---')
    ...
    def _updateMarkers(self):
        ...
        if self.isVisible():
            if self._showMaxPeak:
                try:
                    maxpoint = [
                     self._xValues[self._yValues.argmax()], self._yValues.max()]
                except:
                    maxpoint = [
                     0, 0]
            ...
            if self._showMinPeak:
                try:
                    minpoint = [
                     self._xValues[self._yValues.argmin()], self._yValues.min()]
                except:
                    minpoint = [
                     0, 0]
    ...
    def setXYFromModel(self, value):
        ...
        if attr.data_format == DataFormat._1D:
            ...
            raise ValueError('TaurusCurve only supports SPECTRUM attributes (a %s was passed). \nNote: if you want a trend plot, use TaurusTrendCurve instead of TaurusCurve.' % str(DataFormat[attr.data_format]))
    ...
    def getStats(self, limits=None, inclusive=(True, True), imin=None, imax=None, ignorenans=True):
        ...
        if x.size > 0:
            argmin = y.argmin()
            argmax = y.argmax()
            ret.update({'min': (x[argmin], y[argmin]), 'max': (
                     x[argmax], y[argmax]), 
               'mean': y.mean(), 
               'std': y.std(), 
...
class TaurusPlot(Qwt5.QwtPlot, TaurusBaseWidget):
    ...
    **Important**: although TaurusPlot subclasses QwtPlot and therefore it is
    ...
    def __init__(self, parent=None, designMode=False):
        ...
        self._supportedConfigVersions = [
         'tpc-1', 'tpc-1.1']
        self._axesnames = {Qwt5.QwtPlot.xBottom: 'X', Qwt5.QwtPlot.xTop: 'X2', Qwt5.QwtPlot.yLeft: 'Y1', 
           Qwt5.QwtPlot.yRight: 'Y2'}
        ...
        self._xRegionPicker = Qwt5.QwtPlotPicker(Qwt5.QwtPlot.xBottom, Qwt5.QwtPlot.yLeft, Qwt5.QwtPicker.PointSelection, Qwt5.QwtPicker.VLineRubberBand, Qwt5.QwtPicker.AlwaysOn, self.canvas())
    ...
    def __initActions(self):
        ...
        self._toggleZoomAxisAction.triggered.connect(partial(self.toggleZoomer, axis=None))
        ...
        self._saveConfigAction.triggered.connect(partial(self.saveConfig, ofile=None, curvenames=None))
        self._loadConfigAction = Qt.QAction('&Retrieve saved settings...', None)
        self._loadConfigAction.setShortcut(Qt.QKeySequence.Open)
        self._loadConfigAction.triggered.connect(partial(self.loadConfig, ifile=None))
        ...
        self._exportPdfAction.triggered.connect(partial(self.exportPdf, fileName=None))
        self._exportAsciiAction = Qt.QAction('Export data to &ASCII...', None)
        self._exportAsciiAction.triggered.connect(partial(self.exportAscii, curves=None))
        self._setCurvesTitleAction = Qt.QAction('Change Curves Titles...', None)
        self._setCurvesTitleAction.triggered.connect(partial(self.changeCurvesTitlesDialog, curveNamesList=None))
        self._closeWindowAction = Qt.QAction(Qt.QIcon.fromTheme('process-stop'), 'Close Plot', self)
    ...
    def setAxisCustomLabels(self, axis, pos_and_labels, rotation=0, alignment=None):
        ...
        """
        positions, labels = list(zip(*pos_and_labels))
    ...
    def getZoomers(self, axis=None):
        ...
        if axis is None:
            ...
            if axis == Qwt5.QwtPlot.yLeft:
                return [self._zoomer1]
            if axis == Qwt5.QwtPlot.yRight:
    ...
    def setAxesLabelFormat(self, format=None, xformat=None, y1format=None, y2format=None):
        ...
        if format is None:
            formats = [
             xformat, y1format, y2format]
        else:
            formats = [
             format] * 3
        axes = [
         Qwt5.QwtPlot.xBottom, Qwt5.QwtPlot.yLeft, Qwt5.QwtPlot.yRight]
    ...
    def attachRawData(self, rawdata, properties=None, id=None):
        ...
        *Note*: using "name" in the rawdata dictionary is a still-supported-but-deprecated synonim of "title".
        ...
        if fx is None:
            ...
            if y is not None:
                raise ValueError('only one of "f(x)" or "y" keys can be present')
            if x is None:
            ...
            try:
                ...
                self.warning("the function '%s' could not be evaluated (skipping)" % title)
                return
    ...
    def clearAllRawData(self):
        ...
        self.curves_lock.acquire()
        try:
            names = [ name for name in self.curves if self.curves[name].isRawData ]
        finally:
            self.curves_lock.release()
    ...
    def updateCurves(self, names):
        ...
        try:
            ...
            del_curves = [ name for name in self.curves if name not in ynames
                         ]
    ...
    def updateLegend(self, force=False):
        ...
        if self._showLegend:
            if force or not self.legend():
                ...
                self._legend.setDisplayPolicy(Qwt5.QwtLegend.FixedIdentifier, Qwt5.QwtLegendItem.ShowLine | Qwt5.QwtLegendItem.ShowSymbol | Qwt5.QwtLegendItem.ShowText)
    ...
    def showLegend(self, show, forever=True):
        ...
        elif not self._legendDecissionIsForever:
            self._showLegend = show
        self.updateLegend()
        self._showLegendAction.setChecked(self._showLegend)
    ...
    def _createAxesDict(self):
        ...
        axesdict = {'xMin': xMin, 'xMax': xMax, 'y1Min': y1Min, 'y1Max': y1Max, 'y2Min': y2Min, 'y2Max': y2Max, 'xMode': int(self.getAxisTransformationType(Qwt5.QwtPlot.xBottom)), 
    ...
    def _createMiscDict(self):
        ...
        """
        miscdict = {'defaultCurvesTitle': self.getDefaultCurvesTitle(), 'canvasBackground': self.canvasBackground(), 
    ...
    def createConfig(self, allowUnpickable=False, curvenames=None, **kwargs):
        ...
        configdict = {'Axes': axesdict, 'Misc': miscdict, 'RawData': rawdatadict, 'TangoCurves': tangodict, 
           'CurveProp': propdict, 'ConfigVersion': self._supportedConfigVersions[(-1)], 
           'model': model}
        return configdict
    ...
    def saveConfig(self, ofile=None, curvenames=None):
        ...
        import pickle
        if ofile is None:
            ofile, _ = compat.getSaveFileName(self, 'Save Taurusplot Configuration', 'TaurusplotConfig.pck', 'TaurusPlot Curve Properties File (*.pck)')
            if not ofile:
                return
    ...
    def loadConfig(self, ifile=None):
        ...
        import pickle
        if ifile is None:
            ifile, _ = compat.getOpenFileName(self, 'Load Taurusplot Configuration', '', 'TaurusPlot Curve Properties File (*.pck)')
            if not ifile:
                return
    ...
    def getAxisScale(self, axis):
        ...
        if self.axisAutoScale(axis):
            ...
            return (
             self.axisScaleDiv(axis).lowerBound(), self.axisScaleDiv(axis).upperBound())
    ...
    def exportPdf(self, fileName=None):
        ...
        """
        if fileName is None:
            fileName, _ = compat.getSaveFileName(self, 'Export File Name', 'plot.pdf', 'PDF Documents (*.pdf)')
        ...
        if fileName:
            try:
                ...
                Qt.QMessageBox.warning(self, 'File Error', "Can't write to\n'%s'" % fileName, Qt.QMessageBox.Ok)
                return
    ...
    def exportPrint(self):
        ...
        if dialog.exec_():
            ...
            if Qt.QPrinter.GrayScale == printer.colorMode():
                filter.setOptions(Qwt5.QwtPlotPrintFilter.PrintAll & ~Qwt5.QwtPlotPrintFilter.PrintBackground | Qwt5.QwtPlotPrintFilter.PrintFrameWithScales)
    ...
    def exportAscii(self, curves=None):
        ...
        dialog = klass(parent=self, datadict=frozendata, sortedNames=curves)
    ...
    def importAscii(self, filenames=None, xcol=None, **kwargs):
        ...
        if filenames is None:
            filenames, _ = compat.getOpenFileNames(self, 'Choose input files', '', 'Ascii file (*)')
        if not filenames:
            return False
        else:
            rawdata = {}
    ...
    def autoShowYAxes(self):
        ...
        self.curves_lock.acquire()
        try:
            axes = list(set([ curve.yAxis() for curve in self.curves.values() if curve.isVisible() ]))
            n = len(axes)
            if n == 0:
    ...
    def pickDataPoint(self, pos, scope=20, showMarker=True, targetCurveNames=None):
                ...
                for i in range(data.size()):
                    ...
                    if scopeRect.contains(point):
                        ...
                        if dist < mindist:
                            ...
                            _displayValue = getattr(curve, 'owner', curve).displayValue
        ...
        return (
         picked, pickedCurveName, pickedIndex)
    ...
    def toggleDataInspectorMode(self, enable=None):
        ...
        self._pointPicker.setEnabled(enable)
        self._zoomer.setEnabled(self._allowZoomers and not enable)
        if enable:
    ...
    def setUseParentModel(self, yesno):
        ...
        if yesno:
            self.deprecated(dep='setUseParentModel(True)', rel='4.3.2', alt='explicit models including the parent model')
        if yesno and self._designMode:
            Qt.QMessageBox.information(self, 'UseParentModel usage note', 'Using the UseParentModel feature may require you to call ' + 'recheckTaurusParent() manually for this widget after calling ' + 'setupUi in your code.' + 'See the documentation of TaurusBaseWidget.recheckTaurusParent()')
    ...
    def getLegendPosition(self):
        ...
        if self._legendPos == Qwt5.QwtPlot.LeftLegend:
            return 0
        if self._legendPos == Qwt5.QwtPlot.RightLegend:
            return 1
        if self._legendPos == Qwt5.QwtPlot.BottomLegend:
            return 2
        if self._legendPos == Qwt5.QwtPlot.TopLegend:
            return 3
        if self._legendPos == Qwt5.QwtPlot.ExternalLegend:
            return 4
    ...
    def setCurvesTitle(self, titletext, curveNamesList=None):
        ...
        try:
            if curveNamesList is None:
                curveNamesList = [ n for n, c in self.curves.items() if not c.isRawData ]
...
def plot_main(models=(), config_file=None, x_axis_mode='n', demo=False, window_name='TaurusPlot (qwt)'):
    import sys
