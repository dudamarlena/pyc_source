...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusmessagepanel.py
...
__all__ = [
 'TaurusMessagePanel', 'TaurusMessageErrorHandler',
...
def get_report_handlers():
    ...
    if _REPORT_HANDLERS is not None:
        ...
        import os.path, functools, inspect
...
class TaurusMessagePanel(Qt.QWidget):
    ...
    def _onReportTriggered(self, index):
        ...
        txt = _REPORT.format(appName=app.applicationName(), appVersion=app.applicationVersion(), time=datetime.datetime.now().ctime(), text=self.getText(), detail=self.getDetailedText(), origin=self.getOriginText())
        report.report(txt)
