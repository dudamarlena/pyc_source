...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/input/qwheel.py
...
class QWheelEdit(Qt.QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._value = 0
        self._value_str = '0'
        self._minValue = numpy.finfo('d').min
        self._maxValue = numpy.finfo('d').max
        self._editor = None
        self._editing = False
    ...
    def _build(self):
        ...
        rect = Qt.QRect(l.cellRect(1, 0).topLeft(), l.cellRect(1, l.columnCount() - 1).bottomRight())
        ed.setGeometry(rect)
        ed.setAlignment(Qt.Qt.AlignRight)
        ed.validator().setRange(self.getMinValue(), self.getMaxValue(), self.getDecDigitCount())
        ed.setVisible(False)
        self._editor = ed
        min_height = max(ed.minimumSizeHint().height(), signLabel.minimumSizeHint().height())
        if self.getShowArrowButtons():
    ...
    def _setDigits(self, int_nb=None, dec_nb=None):
        ...
        """
        if int_nb is not None:
            self._intDigitCount = int_nb
        if dec_nb is not None:
    ...
    showArrowButtons = Qt.pyqtProperty('bool', getShowArrowButtons, setShowArrowButtons, resetShowArrowButtons)
    integerDigits = Qt.pyqtProperty('int', getIntDigitCount, setIntDigitCount, resetIntDigitCount)
    decimalDigits = Qt.pyqtProperty('int', getDecDigitCount, setDecDigitCount, resetDecDigitCount)
    value = Qt.pyqtProperty('double', getValue, setValue, resetValue)
    minValue = Qt.pyqtProperty('double', getMinValue, setMinValue, resetMinValue)
    maxValue = Qt.pyqtProperty('double', getMaxValue, setMaxValue, resetMaxValue)
    autoRepeat = Qt.pyqtProperty('bool', getAutoRepeat, setAutoRepeat, resetAutoRepeat)
    autoRepeatDelay = Qt.pyqtProperty('int', getAutoRepeatDelay, setAutoRepeatDelay)
    autoRepeatInterval = Qt.pyqtProperty('int', getAutoRepeatInterval, setAutoRepeatInterval)
