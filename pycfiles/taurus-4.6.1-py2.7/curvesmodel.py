...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/extra_guiqwt/curvesmodel.py
...
__all__ = [
 'TaurusCurveItemTableModel', 'CurveItemConf', 'CurveItemConfDlg']
import copy
...
if __guiqwt_version <= [2, 3, 1]:
    ...
    import qwt
AXIS_ID2NAME = {qwt.QwtPlot.yLeft: 'left', qwt.QwtPlot.yRight: 'right', qwt.QwtPlot.xBottom: 'bottom', 
    ...
    def processSrc(self, src):
        ...
        if src is None:
            ...
            if src == '':
                ...
                return
            if src.startswith('='):
                self.display, self.icon, self.ok = src[1:].strip(), Qt.QIcon.fromTheme('accessories-calculator'), True
                return
            try:
            ...
            if not dev.isValidDev():
                ...
                return
            self.display, self.icon, self.ok = attr.getSimpleName(), Qt.QIcon('logos:taurus.png'), True
            return
...
class TaurusCurveItemTableModel(Qt.QAbstractTableModel):
    ...
    def __init__(self, curves=None):
        if curves is None:
            curves = []
        curves = [ CurveItemConf.fromAny(c) for c in curves ]
...
class CurveItemConfDlg(Qt.QWidget):
    ...
    def onTableContextMenu(self, pos):
        ...
        if row >= 0:
            removeThisAction = menu.addAction(Qt.QIcon.fromTheme('list-remove'), 'Remove this curve', self._onRemoveThisAction)
        removeAllAction = menu.addAction(Qt.QIcon.fromTheme('edit-clear'), 'Clear all', self.model.clearAll)
        addRowAction = menu.addAction(Qt.QIcon.fromTheme('list-add'), 'Add new row', self.model.insertRows)
        if row >= 0:
            editParsAction = menu.addAction(Qt.QIcon.fromTheme('preferences-system'), 'Edit parameters of this curve...', self._onEditParsAction)
    ...
    def _onEditParsAction(self):
        ...
        if row >= 0:
            ...
            if params:
                ...
                self.dataChanged.emit(self.model.index(row, 0), self.model.index(row, self.model.rowCount() - 1))
        return
    ...
    def onModelsAdded(self, models):
        ...
        for i, m in enumerate(models):
            ...
            title = self.model.data(self.model.index(rowcount + i, Y))
