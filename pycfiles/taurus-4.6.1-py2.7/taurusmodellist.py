...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/panel/taurusmodellist.py
...
__all__ = [
 'TaurusModelModel', 'TaurusModelItem', 'TaurusModelList']
...
class TaurusModelItem(object):
    ...
    def setSrc(self, src):
        ...
        if src is None:
            self._src, self.display, self.icon, self.ok = (
             '', '(Empty)', Qt.QIcon(),
             True)
            return
        else:
            ...
            try:
                ...
                if dev.getDeviceProxy() is not None:
                    ...
                    return
                self.display, self.icon, self.ok = src, Qt.QIcon.fromTheme('network-error'), False
                return
            except:
                try:
                    ...
                    self.display, self.icon, self.ok = src, Qt.QIcon.fromTheme('dialog-warning'), False
                    return
                except Exception:
                    self.display, self.icon, self.ok = src, Qt.QIcon.fromTheme('network-error'), False
                    return

                if getSchemeFromName(attr.getFullName()) == 'tango' and dev.getDeviceProxy() is None:
                    self.display, self.icon, self.ok = src, Qt.QIcon.fromTheme('network-error'), False
                    return
...
class TaurusModelModel(Qt.QAbstractListModel):
    ...
    def dropMimeData(self, data, action, row, column, parent):
        ...
        if data.hasFormat(TAURUS_ATTR_MIME_TYPE):
            items = [
             bytes(data.data(TAURUS_ATTR_MIME_TYPE)).decode('utf-8')]
        elif data.hasFormat(TAURUS_MODEL_MIME_TYPE):
            items = [
             bytes(data.data(TAURUS_MODEL_MIME_TYPE)).decode('utf-8')]
        elif data.hasFormat(TAURUS_MODEL_LIST_MIME_TYPE):
            items = bytes(data.data(TAURUS_MODEL_LIST_MIME_TYPE)).decode('utf-8').split()
        elif data.hasText():
            items = [
             str(data.text())]
        else:
...
class TaurusModelList(Qt.QListView):
    ...
    def __init__(self, parent=None, items=None, designMode=False):
        ...
        self.addRowAction = self._contextMenu.addAction(Qt.QIcon.fromTheme('list-add'), 'Add new row', self.newRow, Qt.QKeySequence.New)
        self.removeSelectedAction = self._contextMenu.addAction(Qt.QIcon.fromTheme('list-remove'), 'Remove Selected', self.removeSelected, Qt.QKeySequence.Delete)
        self.removeAllAction = self._contextMenu.addAction(Qt.QIcon.fromTheme('edit-clear'), 'Clear all', self.clear, Qt.QKeySequence('Ctrl+Del'))
        self.moveUpAction = self._contextMenu.addAction(Qt.QIcon.fromTheme('go-up'), 'Move up in the list', self._onMoveUpAction, Qt.QKeySequence('Alt+Up'))
        self.moveDownAction = self._contextMenu.addAction(Qt.QIcon.fromTheme('go-down'), 'Move down in the list', self._onMoveDownAction, Qt.QKeySequence('Alt+Down'))
        self.editDisplayAction = self._contextMenu.addAction(Qt.QIcon('actions:format-text-italic.svg'), 'Edit the display (leave the source)', self._onEditDisplay, Qt.QKeySequence('Alt+D'))
