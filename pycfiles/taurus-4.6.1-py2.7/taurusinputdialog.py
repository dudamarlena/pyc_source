...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtgui/dialog/taurusinputdialog.py
# Compiled at: 2019-08-19 15:09:29
"""This module provides a set of dialog based widgets"""
from __future__ import print_function
from taurus.external.qt import Qt
__all__ = [
 'TaurusInputDialog', 'get_input']
...
def get_input(input_data, parent=None, input_panel_klass=None):
    ...
    dialog = TaurusInputDialog(input_data=input_data, parent=parent, input_panel_klass=input_panel_klass)
    dialog.exec_()
...
class TaurusInputDialog(Qt.QDialog):
    ...
    def __init__(self, input_data=None, parent=None, input_panel_klass=None, designMode=False):
        if input_panel_klass is None:
            from taurus.qt.qtgui.panel import TaurusInputPanel
            input_panel_klass = TaurusInputPanel
...
def demo():
    """Input Dialog"""
    input_data = dict(prompt="What's your favourite car brand?", data_type=[
     'Mazda', 'Skoda', 'Citroen',
     'Mercedes', 'Audi', 'Ferrari'], default_value='Mercedes')
...
def main():
    ...
    d1 = dict(prompt="What's your name?", data_type='String')
    d2 = dict(prompt="What's your age?", data_type='Integer', default_value=4, maximum=100, key='Age', unit='years')
    d3 = dict(prompt="What's your favourite number?", data_type='Float', default_value=0.1, maximum=88.8, key='Number')
    d4 = dict(prompt="What's your favourite car brand?", data_type=[
     'Mazda', 'Skoda', 'Citroen',
     'Mercedes', 'Audi', 'Ferrari'], default_value='Mercedes')
    d5 = dict(prompt='Select some car brands', allow_multiple=True, data_type=[
     'Mazda', 'Skoda', 'Citroen',
     'Mercedes', 'Audi', 'Ferrari'], default_value=[
     'Mercedes', 'Citroen'])
    d6 = dict(prompt="What's your favourite color?", key='Color', data_type=[
     'blue', 'red', 'green'], default_value='red')
    d7 = dict(prompt='Do you like bears?', data_type='Boolean', key='Yes/No', default_value=True)
    d8 = dict(prompt='Please write your memo', data_type='Text', key='Memo', default_value='By default a memo is\na long thing')
    for d in [d1, d2, d3, d4, d5, d6, d7, d8]:
