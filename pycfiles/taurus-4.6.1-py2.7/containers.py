...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/core/util/containers.py
...
import copy, collections, time, weakref
__all__ = [
 'CaselessList', 'CaselessDict', 'CaselessWeakValueDict', 'LoopList',
...
class CaselessList(list):
    ...
    def __init__(self, inlist=[]):
        ...
        for entry in inlist:
            if not isinstance(entry, string_types):
                raise TypeError('Members of this object must be strings. You supplied "%s" which is "%s"' % (
                 entry, type(entry)))
            self.append(entry)
    ...
    def findentry(self, item):
        ...
        if not isinstance(item, string_types):
            raise TypeError('Members of this object must be strings. You supplied "%s"' % type(item))
        for entry in self:
    ...
    def extend(self, item):
        ...
        if not isinstance(item, list):
            raise TypeError('You can only extend lists with lists. You supplied "%s"' % type(item))
        for entry in item:
            if not isinstance(entry, string_types):
    ...
    def insert(self, i, x):
        ...
        if not isinstance(x, string_types):
            raise TypeError('Members of this object must be strings. You supplied "%s"' % type(x))
    ...
    def __setitem__(self, index, value):
        ...
        if isinstance(index, int):
            if not isinstance(value, string_types):
                raise TypeError('Members of this object must be strings. You supplied "%s"' % type(value))
...
try:
    ...
    class defaultdict(dict):
        ...
        def __reduce__(self):
            ...
            return (
             type(self), args, None, None, list(self.items()))
...
class ArrayBuffer(object):
    ...
    def extendLeft(self, a):
        ...
        if newend < self.__bsize:
            self.__buffer[len_a:newend] = self.__buffer[0:self.__end]
            self.__buffer[0:len_a] = a
            self.__end = newend
        else:
            if newend < self.__maxSize:
                self.resizeBuffer(min(2 * self.__bsize, self.__maxSize))
