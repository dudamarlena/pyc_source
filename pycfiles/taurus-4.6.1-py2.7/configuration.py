...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/taurus/qt/qtcore/configuration/configuration.py
...
__all__ = [
 'configurableProperty', 'BaseConfigurableClass']
...
class configurableProperty(object):
    ...
    def createConfig(self, allowUnpickable=False):
        ...
        if isinstance(self.fget, string_types):
    ...
    *restoring the settings on startup*. It also provides a mechanism for
    ...
    def isTaurusConfig(x):
        ...
        if not isinstance(x, dict):
            return False
        for k in ('__orderedConfigNames__', '__itemConfigurations__', 'ConfigVersion',
                  '__pickable__'):
            if k not in x:
                return False
    ...
    def createConfig(self, allowUnpickable=False):
        ...
        """
        configdict = {'ConfigVersion': self._supportedConfigVersions[(-1)], '__pickable__': True}
        itemcfgs = {}
    ...
    def _registerConfigurableItem(self, item, name=None):
        ...
        if name in self.__configurableItemNames:
            raise ValueError('_registerConfigurableItem: An object with name "%s" is already registered' % name)
    ...
    def saveConfigFile(self, ofile=None):
        ...
        if ofile is None:
            ...
            ofile, _ = compat.getSaveFileName(self, 'Save Configuration', '%s.pck' % self.__class__.__name__, 'Configuration File (*.pck)')
            if not ofile:
                return
    ...
    def loadConfigFile(self, ifile=None):
        ...
        if ifile is None:
            from taurus.external.qt import compat
            ifile, _ = compat.getOpenFileName(self, 'Load Configuration', '', 'Configuration File (*.pck)')
            if not ifile:
                return
