...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/leocornus/plonecrypto/crypto.py
# Compiled at: 2010-04-16 02:38:02
"""
...
class PloneCryptoTool(UniqueObject, SimpleItemWithProperties):
    ...
    manage_options = ({'label': 'Keys', 'action': 'manage_keys'},) + SimpleItemWithProperties.manage_options
    ...
    def manage_addNewKey(self, REQUEST=None):
        ...
        if self.maximumKeysAmount > 0 and self.crypter.keysAmount() > self.maximumKeysAmount:
            self.crypter.removeOldestKey()
        if REQUEST:
            REQUEST.RESPONSE.redirect('%s/manage_keys?manage_tabs_message=%s' % (self.absolute_url(), 'New+key+created.'))
    ...
    def manage_clearAndRegenerate(self, REQUEST=None):
        ...
        if REQUEST:
            REQUEST.RESPONSE.redirect('%s/manage_keys?manage_tabs_message=%s' % (self.absolute_url(), 'All+keys+cleared.'))
    ...
    def manage_clearLogs(self, REQUEST=None):
        ...
        if REQUEST:
            REQUEST.RESPONSE.redirect('%s/manage_keys?manage_tabs_message=%s' % (self.absolute_url(), 'All+logs+removed.'))
