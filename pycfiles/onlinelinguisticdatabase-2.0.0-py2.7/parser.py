...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/parser.py
...
class Parse(object):
    ...
    def esc_RE_meta_chars(self, string):
        r"""Escapes regex metacharacters in ``string``.
    ...
    def morpheme_splitter(self):
        ...
        try:
            ...
            if delimiters:
                self._morpheme_splitter = re.compile('([%s])' % ('').join([ self.esc_RE_meta_chars(d) for d in delimiters ])).split
            return self._morpheme_splitter
    ...
    def morpheme_only_splitter(self):
        ...
        try:
            ...
            if delimiters:
                self._morpheme_only_splitter = re.compile('[%s]' % ('').join([ self.esc_RE_meta_chars(d) for d in delimiters ])).split
            return self._morpheme_only_splitter
...
class Command(object):
    ...
    def object_type(self):
        try:
            ...
            if hasattr(self, '__tablename__'):
                self._object_type = self.tablename2object_type.get(self.__tablename__, self.__tablename__)
                return self._object_type
            raise AttributeError("'%s' object has not attribute 'object_type'" % self.__class__.__name__)
    ...
    def get_file_path(self, file_type=None):
        ...
        """
        return os.path.join(self.directory, '%s%s' % (self.file_name, self.file_type2extension.get(file_type, '')))
    ...
    def run(self, cmd, timeout):
        ...
        return (
         self.process.returncode, stdout)
    ...
    def get_process_children_mac(self, pid):
        ...
        p = Popen('ps -o pid -o ppid', shell=True, stdout=PIPE, stderr=PIPE)
        stdout, stderr = p.communicate()
        try:
            return [ int(p) for p, pp in [ line.strip().split() for line in stdout.splitlines()[1:] ] if int(pp) == pid
                   ]
        except Exception:
            return []
...
class FomaFST(Command):
    ...
    def file_type2extension(self):
        ...
        if getattr(self, '_file_type2extension', None):
            ...
            self._file_type2extension.update({'script': '.script', 
               'binary': '.foma', 
               'compiler': '.sh'})
            return self._file_type2extension
            return
    ...
    def apply(self, direction, input_, boundaries=None):
        ...
        with codecs.open(inputs_file_path, 'w', 'utf8') as (f):
            if boundaries:
                f.write(('\n').join(input_.join([self.word_boundary_symbol, self.word_boundary_symbol]) for input_ in inputs))
            else:
                f.write(('\n').join(inputs))
        with codecs.open(apply_file_path, 'w', 'utf8') as (f):
            f.write('#!/bin/sh\ncat %s | flookup %s%s' % (
             inputs_file_path, {'up': '', 'down': '-i '}.get(direction, '-i '),
             binary_path))
        os.chmod(apply_file_path, 484)
    ...
    def foma_output_file2dict(self, file_, remove_word_boundaries=True):
        ...
        def word_boundary_remover(x):
            if (
             x[0:1], x[-1:]) == (self.word_boundary_symbol, self.word_boundary_symbol):
                return x[1:-1]
            else:
                return x
    ...
    def compile(self, timeout=1800, verification_string=None):
        ...
        try:
            ...
            if verification_string in output:
                if returncode == 0:
                    if os.path.isfile(binary_path) and binary_mod_time != self.get_modification_time(binary_path):
                        self.compile_succeeded = True
                        self.compile_message = 'Compilation process terminated successfully and new binary file was written.'
                    else:
                        self.compile_message = 'Compilation process terminated successfully yet no new binary file was written.'
                else:
...
class MorphologyFST(FomaFST, Parse):
    ...
    def file_type2extension(self):
        if getattr(self, '_file_type2extension', None):
            ...
            self._file_type2extension.update({'lexicon': '.pickle', 
...
class LanguageModel(Command, Parse):
    ...
    toolkits = {'mitlm': {'executable': 'estimate-ngram', 
    ...
    @property
    def verification_string(self):
        return self.toolkits[self.toolkit]['verification_string_getter'](self.get_file_path('arpa'))
    ...
    def file_type2extension(self):
        if getattr(self, '_file_type2extension', None):
            ...
            self._file_type2extension.update({'corpus': '.txt', 
               'arpa': '.lm', 
               'trie': '.pickle', 
               'vocabulary': '.vocab'})
            return self._file_type2extension
    ...
    def get_probabilities(self, input_):
        ...
        morpheme_sequences = [ (morpheme_sequence, [self.start_symbol] + splitter.split(morpheme_sequence) + [self.end_symbol]) for morpheme_sequence in morpheme_sequences
                             ]
        trie = self.trie
        return dict((morpheme_sequence, self.get_probability_one(morpheme_sequence_list, trie)) for morpheme_sequence, morpheme_sequence_list in morpheme_sequences)
    ...
    def write_arpa(self, timeout):
        ...
        succeeded = verification_string in output and returncode == 0 and os.path.isfile(arpa_path) and arpa_mod_time != self.get_modification_time(arpa_path)
        if not succeeded:
...
class MorphologicalParser(FomaFST, Parse):
    ...
    @property
    def verification_string(self):
        return 'defined %s: ' % self.object_type2file_name.get(self.object_type, self.object_type)
    ...
    def pretty_parse(self, input_):
        ...
        return dict((transcription, parse.triplet) for transcription, (parse, candidates) in parses.iteritems())
    ...
    def parse(self, transcriptions, parse_objects=False, max_candidates=10):
        ...
        for transcription in transcriptions:
            ...
            if cached_parse is not False:
                parsed[transcription] = (
                 cached_parse, cached_candidates)
            else:
                unparsed.append(transcription)

        unparsed = self.get_candidates(unparsed)
        for transcription, candidates in unparsed.iteritems():
            ...
            self.cache[transcription] = parsed[transcription] = (
             parse, sorted_candidates)

        if self.persist_cache:
            self.cache.persist()
        if parse_objects:
            return dict((transcription, (self.get_parse_object(parse), map(self.get_parse_object, candidates))) for transcription, (parse, candidates) in parsed.iteritems())
        return parsed
    ...
    def get_parse_object(self, parse_string):
        ...
        """
        return Parse(parse_string, morpheme_delimiters=getattr(self, 'morpheme_delimiters', None), rare_delimiter=self.my_morphology.rare_delimiter)
        ...
        if not candidates:
            ...
            for candidate in candidates:
                ...
                if self.my_language_model.categorial:
                    lm_input = [ morpheme.split(self.my_morphology.rare_delimiter)[2] for morpheme in lm_input ]
                lm_input = [
                 self.my_language_model.start_symbol] + lm_input + [
    ...
    def disambiguate(self, candidates):
        ...
        try:
            ...
            for transcription, candidate_list in candidates.iteritems():
                ...
                for candidate in candidate_list:
                    ...
                    for index, morpheme in enumerate(morphemes):
                        if index % 2 == 0:
                            homographs = [ [morpheme, gloss, category] for gloss, category in dictionary[morpheme] ]
            ...
            log.warn('some kind of exception occured in morphologicalparsers.py disambiguate_candidates: %s' % e)
    ...
    def export(self):
        ...
        """
        return {'phonology': {'word_boundary_symbol': getattr(self.phonology, 'word_boundary_symbol', '#')}, 
           'morphology': {'word_boundary_symbol': getattr(self.my_morphology, 'word_boundary_symbol', '#'), 
           ...
           'language_model': {'rare_delimiter': getattr(self.my_language_model, 'rare_delimiter', '⦀'), 
           ...
           'parser': {'word_boundary_symbol': getattr(self, 'word_boundary_symbol', '#'), 
                      'morpheme_delimiters': getattr(self, 'morpheme_delimiters', None)}}
