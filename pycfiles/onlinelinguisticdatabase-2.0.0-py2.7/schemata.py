...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/schemata.py
# Compiled at: 2016-09-19 13:27:02
from formencode.variabledecode import NestedVariables
from formencode.schema import Schema
from formencode.validators import Invalid, FancyValidator, Int, DateConverter, UnicodeString, OneOf, Regex, Email, StringBoolean, String, URL, Number
...
class ValidTranslations(FancyValidator):
    ...
    """
    messages = {'one_translation': 'Please enter one or more translations', 
       'invalid_grammaticality': ('').join(['At least one submitted translation ',
                                'grammaticality does not match any of the available options.'])}
    ...
    def _to_python(self, value, state):
        ...
        try:
            ...
            grammaticalities = [ t['grammaticality'] for t in value if t['grammaticality'].strip() ]
        except (AttributeError, KeyError, TypeError):
            translations = []
            grammaticalities = []

        valid_grammaticalities = get_grammaticalities()
        bad_translation_grammaticalities = [ g for g in grammaticalities if g not in valid_grammaticalities
                                           ]
        if not translations:
...
class ValidOrthographicTranscription(UnicodeString):
    ...
    """
    messages = {'invalid_transcription': ('').join(['The orthographic transcription you have entered ',
                               'is not valid. Only graphemes from the specified ',
                               'storage orthography, punctuation characters and ',
                               'the space character are permitted.'])}

    def validate_python(self, value, state):
        transcription = h.to_single_space(h.normalize(value))
        if not form_is_foreign_word(state.full_dict) and not transcription_is_valid(transcription, 'orthographic_validation', 'orthographic_inventory'):
            raise Invalid(self.message('invalid_transcription', state), value, state)
...
class ValidNarrowPhoneticTranscription(UnicodeString):
    ...
    """
    messages = {'invalid_transcription': ('').join(['The narrow phonetic transcription you have entered ',
                               'is not valid. Only graphemes from the specified ',
                               'narrow phonetic inventory and ',
                               'the space character are permitted.'])}

    def validate_python(self, value, state):
        transcription = h.to_single_space(h.normalize(value))
        if not form_is_foreign_word(state.full_dict) and not transcription_is_valid(transcription, 'narrow_phonetic_validation', 'narrow_phonetic_inventory'):
            raise Invalid(self.message('invalid_transcription', state), value, state)
...
class ValidBroadPhoneticTranscription(UnicodeString):
    ...
    """
    messages = {'invalid_transcription': ('').join(['The broad phonetic transcription you have entered ',
                               'is not valid. Only graphemes from the specified ',
                               'broad phonetic inventory and ',
                               'the space character are permitted.'])}

    def validate_python(self, value, state):
        transcription = h.to_single_space(h.normalize(value))
        if not form_is_foreign_word(state.full_dict) and not transcription_is_valid(transcription, 'broad_phonetic_validation', 'broad_phonetic_inventory'):
            raise Invalid(self.message('invalid_transcription', state), value, state)
...
class ValidMorphemeBreakTranscription(UnicodeString):
    ...
    morpheme delimiters.
    """
    messages = {'invalid_transcription': ('').join(['The morpheme segmentation you have entered ',
                               'is not valid.  Only graphemes from the ',
                               '%(inventory)s, the specified morpheme delimiters ',
                               'and the space character are permitted.'])}

    def validate_python(self, value, state):
        transcription = h.to_single_space(h.normalize(value))
        try:
            morpheme_break_is_orthographic = getattr(getattr(getattr(app_globals, 'application_settings', None), 'application_settings', None), 'morpheme_break_is_orthographic', False)
        except TypeError:
        ...
        if not form_is_foreign_word(state.full_dict) and not transcription_is_valid(transcription, 'morpheme_break_validation', 'morpheme_break_inventory'):
            raise Invalid(self.message('invalid_transcription', state, inventory=inventory), value, state)
        return
...
def transcription_is_valid(transcription, validation_name, inventory_name):
    ...
    """
    if getattr(getattr(getattr(app_globals, 'application_settings', None), 'application_settings', None), validation_name, None) == 'Error':
        return getattr(getattr(app_globals, 'application_settings', None), inventory_name, None).string_is_valid(transcription)
    else:
        return True


class ValidGrammaticality(FancyValidator):
    messages = {'invalid_grammaticality': 'The grammaticality submitted does not match any of the available options.'}
...
def get_grammaticalities():
    try:
        ...
        if not applicationSettings:
            ...
            grammaticalities = getattr(applicationSettings, 'grammaticalities', [''])
...
class ValidOLDModelObject(FancyValidator):
    ...
    """
    messages = {'invalid_model': 'There is no %(model_name_eng)s with id %(id)d.', 
       'restricted_model': 'You are not authorized to access the %(model_name_eng)s with id %(id)d.'}
    ...
    def _to_python(self, value, state):
        if value in ('', None):
            ...
            if model_object is None:
                raise Invalid(self.message('invalid_model', state, id=id, model_name_eng=h.camel_case2lower_space(self.model_name)), value, state)
            elif self.model_name in ('Form', 'File', 'Collection') and getattr(state, 'user', None):
                ...
                if h.user_is_authorized_to_access_model(state.user, model_object, unrestricted_users):
                    return model_object
                raise Invalid(self.message('restricted_model', state, id=id, model_name_eng=h.camel_case2lower_space(self.model_name)), value, state)
            else:
                return model_object
            return
...
class AtLeastOneTranscriptionTypeValue(FancyValidator):
    ...
    """
    messages = {'transcription': 'You must enter a value in at least one of the following fields: transcription, morpheme break, phonetic transcription, or narrow phonetic transcription.'}

    def _to_python(self, values, state):
        if not values['transcription'].strip() and not values['phonetic_transcription'].strip() and not values['narrow_phonetic_transcription'].strip() and not values['morpheme_break'].strip():
...
class FormSchema(Schema):
    ...
    filter_extra_fields = True
    chained_validators = [
     AtLeastOneTranscriptionTypeValue()]
...
class ValidBase64EncodedFile(String):
    ...
    messages = {'invalid_base64_encoded_file': 'The uploaded file must be base64 encoded.'}
...
class ValidFileName(UnicodeString):
    ...
    """
    messages = {'invalid_type': 'The file upload failed because the file type %(MIME_type)s is not allowed.'}
...
class AddMIMETypeToValues(FancyValidator):
    ...
    """
    messages = {'mismatched_type': "The file extension does not match the file's true type (%(x)s vs. %(y)s, respectively)."}
    ...
    def _to_python(self, values, state):
        ...
        try:
            ...
            if MIME_type_from_contents != MIME_type_from_filename:
                raise Invalid(self.message('mismatched_type', state, x=MIME_type_from_filename, y=MIME_type_from_contents), values, state)
...
class ValidAudioVideoFile(FancyValidator):
    ...
    """
    messages = {'invalid_file': 'There is no file with id %(id)d.', 
       'restricted_file': 'You are not authorized to access the file with id %(id)d.', 
       'not_av': 'File %(id)d is not an audio or a video file.', 
       'empty': 'An id corresponding to an existing audio or video file must be provided.', 
       'ref_ref': 'The parent file cannot itself be a subinterval-referencing file.'}
...
class ValidSubinterval(FancyValidator):
    ...
    to floats.
    """
    messages = {'invalid': 'The start value must be less than the end value.', 
       'not_numbers': 'The start and end values must be numbers.'}

    def _to_python(self, values, state):
        if type(values['start']) not in (int, float) or type(values['end']) not in (int, float):
...
class ValidBibTeXEntryType(FancyValidator):
    ...
    """
    messages = {'invalid_bibtex_entry': '%(submitted_entry)s is not a valid BibTeX entry type'}

    def _to_python(self, value, state):
        if value.lower() in bibtex.entry_types.keys():
            return value.lower()
        raise Invalid(self.message('invalid_bibtex_entry', state, submitted_entry=value), value, state)
...
class ValidBibTexKey(FancyValidator):
    ...
    """
    messages = {'invalid_bibtex_key_format': 'Source keys can only contain letters, numerals and symbols (except the comma)', 
    ...
    def validate_python(self, value, state):
        ...
        if set(list(value)) - set(list(valid)):
            raise Invalid(self.message('invalid_bibtex_key_format', state), value, state)
        id = getattr(state, 'id', None)
        query = Session.query(model.Source)
        if id and query.filter(and_(model.Source.key == value, model.Source.id != id)).first() or not id and query.filter(model.Source.key == value).first():
...
class ValidBibTeXEntry(FancyValidator):
    ...
    def parse_requirements(self, entry_type):
        ...
        if disjunctively_required_fields:
            msg = '%s as well as a value for %s' % (msg,
             coordinate([ 'at least one of %s' % coordinate(dr) for dr in disjunctively_required_fields
             ]))
        return (
         required_fields, disjunctively_required_fields, '%s.' % msg)
    ...
    def get_required_value(self, values, required_field):
        ...
        if values.get(required_field):
            ...
            if getattr(values.get('crossref_source'), required_field, None):
    ...
    def validate_python(self, values, state):
        ...
        required_fields_values = filter(None, [ self.get_required_value(values, rf) for rf in required_fields ])
...
class ValidCrossref(FancyValidator):
    ...
    def _to_python(self, values, state):
        if values.get('crossref') in (None, ''):
            ...
            crossref_source = Session.query(model.Source).filter(model.Source.key == crossref).first()
            if crossref_source is None:
                raise Invalid(self.message('invalid_crossref', state, crossref=crossref), values, state)
            else:
...
class UniqueUnicodeValue(UnicodeString):
    ...
    """
    messages = {'not_unique': 'The submitted value for %(model_name)s.%(attribute_name)s is not unique.'}
    ...
    def validate_python(self, value, state):
        ...
        if id and query.filter(and_(attribute == value, getattr(model_, 'id') != id)).first() or not id and query.filter(attribute == value).first():
            raise Invalid(self.message('not_unique', state, model_name=self.model_name, attribute_name=self.attribute_name), value, state)
        return
...
class PageSchema(Schema):
    ...
    allow_extra_fields = True
    filter_extra_fields = True
    name = UniqueUnicodeValue(max=255, not_empty=True, model_name='Page', attribute_name='name')
...
class ValidTagName(FancyValidator):
    ...
    """
    messages = {'unchangeable': 'The names of the restricted and foreign word tags cannot be changed.'}
...
class KeyboardSchema(Schema):
    ...
    allow_extra_fields = True
    filter_extra_fields = True
    name = UniqueUnicodeValue(max=255, not_empty=True, model_name='Keyboard', attribute_name='name')
    description = UnicodeString()
...
class ValidUsernameAndPassword(FancyValidator):
    ...
    messages = {'bad_password': (' ').join([
       ...
       'not_confirmed': 'The password and password_confirm values do not match.', 
       'nonunique_username': 'The username %(username)s is already taken.', 
       'illegal_chars': 'The username %(username)s is invalid; only letters of the English alphabet, numbers and the underscore are permitted.', 
    ...
    def _to_python(self, values, state):
        ...
        username_is_a_non_empty_string = type(username) in (str, unicode) and username != ''
        password = values.get('password', None)
        password_is_a_non_empty_string = type(password) in (str, unicode) and password != ''
        ...
        def is_high_entropy_ASCII(password):
            ...
            return re.search('[a-z]', password) is not None and re.search('[A-Z]', password) is not None and re.search('[0-9]', password) is not None and symbol_patt.search(password) is not None

        if password_is_a_non_empty_string:
            if len(password) < 8 or not contains_non_ASCII_chars(password) and not is_high_entropy_ASCII(password):
        ...
        if username_is_a_non_empty_string:
            ...
            if re.search('[^\\w]+', username):
                raise Invalid(self.message('illegal_chars', state, username=username), 'username', state)
            elif id and query.filter(and_(User.username == username, User.id != id)).first() or not id and query.filter(User.username == username).first():
                raise Invalid(self.message('nonunique_username', state, username=username), 'username', state)
            elif user_to_update and username != user_to_update['username'] and user_attempting_update['role'] != 'administrator':
                raise Invalid(self.message('non_admin_username_update', state, username=username), 'username', state)
            else:
...
class LicitRoleChange(FancyValidator):
    ...
    messages = {'non_admin_role_update': 'Only administrators can update roles.'}

    def validate_python(self, values, state):
        role = values.get('role')
        user_to_update = getattr(state, 'user_to_update', {})
        user_attempting_update = getattr(state, 'user', {})
        if user_to_update and user_to_update['role'] != role and user_attempting_update['role'] != 'administrator':
...
class ValidFormReferences(FancyValidator):
    ...
    def _to_python(self, values, state):
        if values.get('form_search'):
            values['forms'] = SQLAQueryBuilder().get_SQLA_query(json.loads(values['form_search'].search)).all()
            return values
        form_references = list(set(model.Corpus.get_form_references(values.get('content', ''))))
...
class CompatibleParserComponents(FancyValidator):
    ...
    """
    messages = {'rare_no_match': "A parser's non-categorial LM must have the same rare_delimiter value as its morphology."}
