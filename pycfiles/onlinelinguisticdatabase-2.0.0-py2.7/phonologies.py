...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/phonologies.py
...
class PhonologiesController(BaseController):
    ...
    def history(self, id):
        ...
        if phonology or previous_versions:
            return {'phonology': phonology, 'previous_versions': previous_versions}
        else:
            response.status_int = 404
        ...
        if phonology:
            if h.foma_installed():
                foma_worker_q.put({'id': h.generate_salt(), 
                   ...
                   'args': {'phonology_id': phonology.id, 
...
def create_new_phonology(data):
    ...
    """
    phonology = Phonology(parent_directory=h.get_OLD_directory_path('phonologies', config=config), word_boundary_symbol=h.word_boundary_symbol, UUID=unicode(uuid4()), name=h.normalize(data['name']), description=h.normalize(data['description']), script=h.normalize(data['script']).replace('\r', ''), enterer=session['user'], modifier=session['user'], datetime_modified=h.now(), datetime_entered=h.now())
    return phonology
