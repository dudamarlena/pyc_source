...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/simplelm/SimpleARPA2WFSA.py
    ...
    def arpa_to_wfsa(self):
        ...
        for line in open(self.arpa_file, 'r'):
            ...
            if self.order > 0 and not line.startswith('\\') and not line == '':
                ...
                self.max_order = int(re.sub('^ngram\\s+(\\d+)=.*$', '\\1', line))
            elif re.match('^\\\\\\d+', line):
                self.order = int(re.sub('^\\\\(\\d+).*$', '\\1', line))

    def print_syms(self):
        if self.eps in self.isyms:
            self.isyms.remove(self.eps)
        if self.phi in self.isyms:
            self.isyms.remove(self.phi)
        if self.eps in self.ssyms:
            self.ssyms.remove(self.eps)
        self._print_isyms()
        self._print_ssyms()
