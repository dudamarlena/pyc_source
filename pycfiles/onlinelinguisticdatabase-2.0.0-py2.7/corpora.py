...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/corpora.py
...
class CorporaController(BaseController):
    ...
    def search_corpora(self):
        ...
        try:
            ...
            SQLAQuery = self.query_builder_for_ordering.get_SQLA_query(python_search_params.get('query'))
    ...
    def search(self, id):
        ...
        if corpus:
            try:
                json_search_params = unicode(request.body, request.charset)
                python_search_params = json.loads(json_search_params)
                query = h.eagerload_form(self.query_builder.get_SQLA_query(python_search_params.get('query')))
    ...
    def new_search_corpora(self):
        ...
        """
        return {'search_parameters': h.get_search_parameters(self.query_builder_for_ordering)}
    ...
    def edit(self, id):
        ...
        corpus = h.eagerload_corpus(Session.query(Corpus)).get(id)
        if corpus:
            return {'data': get_data_for_new_edit(request.GET), 'corpus': corpus}
        else:
            response.status_int = 404
            return {'error': 'There is no corpus with id %s' % id}
    ...
    def servefile(self, id, file_id):
        ...
        if corpus:
            try:
                ...
                corpus_file_path = os.path.join(get_corpus_dir_path(corpus), '%s.gz' % corpus_file.filename)
    ...
    def tgrep2(self, id):
        ...
        if not h.command_line_program_installed('tgrep2'):
            ...
            if corpus:
                try:
                    treebank_corpus_file_object = filter(lambda cf: cf.format == 'treebank', corpus.files)[0]
                    corpus_dir_path = get_corpus_dir_path(corpus)
                    tgrep2_corpus_file_path = os.path.join(corpus_dir_path, '%s.t2c' % treebank_corpus_file_object.filename)
                except Exception:
                    response.status_int = 400
                ...
                try:
                    ...
                    try:
                        ...
                        response.status_int = 400
                        return {'errors': {'tgrep2pattern': 'A tgrep2pattern attribute must be supplied and must have a unicode/string value'}}

                    tmp_path = os.path.join(corpus_dir_path, '%s%s.txt' % (session['user'].username, h.generate_salt()))
                    with open(os.devnull, 'w') as (fnull):
                        with open(tmp_path, 'w') as (stdout):
                            process = Popen(['tgrep2', '-c', tgrep2_corpus_file_path, '-wu', tgrep2pattern], stdout=stdout, stderr=fnull)
                            process.communicate()
    ...
    def get_word_category_sequences(self, id):
        ...
        if corpus:
            ...
            if minimum_token_count:
                word_category_sequences = [ (('').join(sequence), ids) for sequence, ids in word_category_sequences if len(ids) >= minimum_token_count ]
...
def authorized_to_access_corpus_file(user, corpus_file):
    ...
    if corpus_file.restricted and user.role != 'administrator' and user not in h.get_unrestricted_users():
        return False
    return True
...
def write_to_file(corpus, format_):
    ...
    """
    error_msg = lambda msg: {'error': 'Unable to write corpus %d to file with format "%s". (%s)' % (
    ...
    update = os.path.exists(corpus_file_path)
    restricted = False
    try:
    ...
    try:
        ...
        if update:
            try:
                ...
                generate_new_corpus_file(corpus, corpus_filename, format_, user, now, restricted)
...
def create_tgrep2_corpus_file(gzipped_corpus_file_path, format_):
    ...
    """
    if format_ == 'treebank' and h.command_line_program_installed('tgrep2'):
...
def get_corpus_file_path(corpus, format_):
    ...
    return os.path.join(get_corpus_dir_path(corpus), 'corpus_%d%s.%s' % (corpus.id, sfx, ext))


def get_corpus_dir_path(corpus):
    return os.path.join(h.get_OLD_directory_path('corpora', config=config), 'corpus_%d' % corpus.id)
...
def get_data_for_new_edit(GET_params):
    ...
    mandatory_attributes = [
     'corpus_formats']
    model_name_map = {'form_searches': 'FormSearch', 
       'users': 'User', 
       'tags': 'Tag'}
    getter_map = {'form_searches': h.get_mini_dicts_getter('FormSearch'), 
