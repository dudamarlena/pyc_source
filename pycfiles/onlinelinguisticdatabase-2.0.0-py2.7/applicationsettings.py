...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/model/applicationsettings.py
...
class ApplicationSettingsUser(Base):
    ...
    id = Column(Integer, Sequence('applicationsettingsuser_seq_id', optional=True), primary_key=True)
    applicationsettings_id = Column(Integer, ForeignKey('applicationsettings.id'))
...
class ApplicationSettings(Base):
    ...
    id = Column(Integer, Sequence('applicationsettings_seq_id', optional=True), primary_key=True)
    ...
    storage_orthography = relation('Orthography', primaryjoin='ApplicationSettings.storage_orthography_id==Orthography.id')
    input_orthography_id = Column(Integer, ForeignKey('orthography.id', ondelete='SET NULL'))
    input_orthography = relation('Orthography', primaryjoin='ApplicationSettings.input_orthography_id==Orthography.id')
    output_orthography_id = Column(Integer, ForeignKey('orthography.id', ondelete='SET NULL'))
    output_orthography = relation('Orthography', primaryjoin='ApplicationSettings.output_orthography_id==Orthography.id')
    datetime_modified = Column(DateTime, default=now)
