...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/model/morphologicalparser.py
...
class MorphologicalParser(MorphologicalParser, Base):
    ...
    def my_morphology(self):
        ...
        try:
            ...
            self._my_morphology = MorphologyFST(parent_directory=self.directory, rare_delimiter=self.morphology_rare_delimiter, word_boundary_symbol=self.word_boundary_symbol, rules_generated=self.morphology_rules_generated, rich_upper=self.morphology_rich_upper, rich_lower=self.morphology_rich_lower, morpheme_delimiters=self.morpheme_delimiters)
            return self._my_morphology
    ...
    def my_language_model(self):
        ...
        try:
            ...
            self._my_language_model = LanguageModel(parent_directory=self.directory, start_symbol=self.language_model_start_symbol, end_symbol=self.language_model_end_symbol, categorial=self.language_model_categorial)
            return self._my_language_model
...
class Cache(object):
    ...
    def __getitem__(self, k):
        try:
            ...
            parse = Session.query(Parse).filter(Parse.parser_id == self.parser.id).filter(Parse.transcription == k).first()
            if parse:
    ...
    def persist(self):
        ...
        """
        if self.updated:
            persisted = [ parse.transcription for parse in Session.query(Parse).filter(Parse.parser_id == self.parser.id).filter(Parse.transcription.in_(self._store.keys())).all()
                        ]
            unpersisted = [ Parse(transcription=transcription, parse=parse, candidates=self.json_dumps_candidates(candidates), parser=self.parser) for transcription, (parse, candidates) in self._store.iteritems() if transcription not in persisted
                          ]
    ...
    def clear(self, persist=False):
        ...
        self._store = {}
        if persist:
            delete = Parse.__table__.delete().where(Parse.__table__.c.parser_id == self.parser.id)
            Session.execute(delete)

    def export(self):
        """Update the local store with the persistence layer and return the store.
        """
        persisted = dict((p.transcription, (p.parse, json.loads(p.candidates))) for p in Session.query(Parse).filter(Parse.parser_id == self.parser.id).all())
