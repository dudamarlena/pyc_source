...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/speakers.py
...
class SpeakersController(BaseController):
    ...
    def edit(self, id):
        ...
        speaker = Session.query(Speaker).get(id)
        if speaker:
            return {'data': {'markup_languages': h.markup_languages}, 'speaker': speaker}
        else:
            response.status_int = 404
            return {'error': 'There is no speaker with id %s' % id}
...
def update_speaker(speaker, data):
    ...
    changed = speaker.set_attr('html', h.get_HTML_from_contents(speaker.page_content, speaker.markup_language), changed)
    if changed:
        speaker.datetime_modified = datetime.datetime.utcnow()
        return speaker
    return changed