...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/rememberedforms.py
...
class RememberedformsController(BaseController):
    ...
    def show(self, id):
        ...
        if user:
            try:
                query = h.eagerload_form(Session.query(Form)).filter(Form.memorizers.contains(user))
