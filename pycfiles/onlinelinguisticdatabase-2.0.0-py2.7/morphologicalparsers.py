...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/morphologicalparsers.py
...
"""
import logging, simplejson as json, os
...
class MorphologicalparsersController(BaseController):
        ...
        if morphological_parser:
            ...
            if request.GET.get('script') == '1':
                ...
                if os.path.isfile(morphological_parser_script_path):
                    ...
                    morphological_parser_dict['script'] = ''
    ...
    def history(self, id):
        ...
        if morphological_parser or previous_versions:
            return {'morphological_parser': morphological_parser, 'previous_versions': previous_versions}
        else:
            response.status_int = 404
            ...
            if h.foma_installed():
                ...
                if os.path.isfile(binary_path):
                    try:
                        ...
                        schema, key = {'up': (TranscriptionsSchema, 'transcriptions'), 'down': (
                                  MorphemeSequencesSchema, 'morpheme_sequences')}.get(direction, (MorphemeSequencesSchema, 'morpheme_sequences'))
                    ...
                    response.status_int = 400
                    return json.dumps({'error': 'The morphophonology foma script of MorphologicalParser %d has not been compiled yet.' % parser.id})
            else:
                response.status_int = 400
    ...
    def parse(self, id):
        ...
        try:
            ...
            return dict((transcription, parse) for transcription, (parse, candidates) in parses.iteritems())
        except h.JSONDecodeError:
            response.status_int = 400
            return h.JSONDecodeErrorResponse
        except Invalid as e:
            ...
            log.warn(e)
            response.status_int = 400
            return {'error': 'Parse request raised an error.'}
    ...
    def export_deprecated(self, id):
        ...
        try:
            ...
            log.warn(e)
            response.status_int = 400
            return json.dumps({'error': 'An error occured while attempting to export morphological parser %s' % id})
    ...
    def export(self, id):
        ...
        try:
            ...
            for file_name in os.listdir(directory):
                if os.path.splitext(file_name)[1] not in ('.log', '.sh', '.zip') and file_name != 'morpheme_language_model.pickle':
            ...
            log.warn(e)
            response.status_int = 400
            return json.dumps({'error': 'An error occured while attempting to export morphological parser %s: %s' % (
                       id, e)})


def get_data_for_new_edit(GET_params):
    """Return the data needed to create a new morphological parser or edit one."""
    model_name_map = {'morpheme_language_models': 'MorphemeLanguageModel', 
       'phonologies': 'Phonology', 
       'morphologies': 'Morphology'}
    getter_map = {'morpheme_language_models': h.get_mini_dicts_getter('MorphemeLanguageModel'), 
...
def create_new_morphological_parser(data):
    ...
    """
    morphological_parser = MorphologicalParser(parent_directory=h.get_OLD_directory_path('morphologicalparsers', config=config), UUID=unicode(uuid4()), name=h.normalize(data['name']), description=h.normalize(data['description']), enterer=session['user'], modifier=session['user'], datetime_modified=h.now(), datetime_entered=h.now(), phonology=data['phonology'], morphology=data['morphology'], language_model=data['language_model'])
    return morphological_parser
...
def generate_and_compile_morphological_parser(morphological_parser_id, compile_=True):
    ...
    if compile_ and not h.foma_installed():
        response.status_int = 400
        return {'error': 'Foma and flookup are not installed.'}
    foma_worker_q.put({'id': h.generate_salt(), 
       'func': 'generate_and_compile_parser', 
       'args': {'morphological_parser_id': morphological_parser.id, 
                'compile': compile_, 
