...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/SQLAQueryBuilder.py
...
class SQLAQueryBuilder(object):
    ...
    def __init__(self, model_name='Form', primary_key='id', **kwargs):
        ...
        self.RDBMSName = get_RDBMS_name(**kwargs)
    ...
    def _get_SQLA_order_by(self, order_by, primary_key='id'):
        ...
        if order_by is None:
            ...
            try:
                ...
                if self.RDBMSName == 'sqlite' and attribute is not None and isinstance(attribute.property.columns[0].type, self.SQLAlchemyStringTypes):
    ...
    def _python2sqla(self, python):
        ...
        try:
            if python[0] in ('and', 'or'):
                return {'and': and_, 'or': or_}[python[0]](*[ self._python2sqla(x) for x in python[1] ])
            if python[0] == 'not':
    ...
    def _get_date_value(self, date_string):
        ...
        if date_string is None:
            ...
            if date is None:
                self._add_to_errors('date %s' % str(date_string), 'Date search parameters must be valid ISO 8601 date strings.')
            return date
    ...
    def _get_model(self, model_name, add_to_joins=True):
        try:
            ...
            model = None
            self._add_to_errors(model_name, 'The OLD has no model %s' % model_name)
        ...
        if add_to_joins and model_name != self.model_name:
            ...
            if model_name in join_models:
                ...
                join_collection = getattr(getattr(old_model, self.model_name), join_collection_name)
                model = aliased(model)
                self.joins.append((model, join_collection))
            else:
                self._add_to_errors(model_name, 'Searching the %s model by joining on the %s model is not possible' % (
                 self.model_name, model_name))
        return model
    ...
    def _get_attribute_model_name(self, attribute_name, model_name):
        ...
        try:
            ...
            self._add_to_errors('%s.%s' % (model_name, attribute_name), 'The %s attribute of the %s model does not represent a many-to-one relation.' % (
    ...
    def _get_attribute_dict(self, attribute_name, model_name, report_error=False):
        ...
        invalid.
        """
        attribute_dict = self.schema.get(model_name, {}).get(attribute_name, None)
        if attribute_dict is None and report_error:
            self._add_to_errors('%s.%s' % (model_name, attribute_name), 'Searching on %s.%s is not permitted' % (model_name, attribute_name))
    ...
    def _get_attribute(self, attribute_name, model, model_name):
        try:
            ...
            attribute = None
            self._add_to_errors('%s.%s' % (model_name, attribute_name), 'There is no attribute %s of %s' % (attribute_name, model_name))
    ...
    def _get_relation_dict(self, relation_name, model_name, attribute_name, report_error=False):
        ...
        if relation_dict is None and report_error:
            self._add_to_errors('%s.%s.%s' % (model_name, attribute_name, relation_name), 'The relation %s is not permitted for %s.%s' % (relation_name, model_name, attribute_name))
    ...
    def _get_relation(self, relation_name, attribute, attribute_name, model_name):
        try:
            ...
            relation = None
            self._add_to_errors('%s.%s.%s' % (model_name, attribute_name, relation_name), "There is no relation '%s' of '%s.%s'" % (relation_name, model_name, attribute_name))
    ...
    def _get_invalid_filter_expression_message(self, model_name, attribute_name, relation_name, value):
        return 'Invalid filter expression: %s.%s.%s(%s)' % (model_name,
         attribute_name, relation_name, repr(value))

    def _get_invalid_model_attribute_errors(self, relation, value, model_name, attribute_name, relation_name, attribute, attribute_model_name, attribute_model_attribute_name):
        ...
        if attribute_model_name:
            ...
            if self.errors.get(error_key) == 'Searching on the %s is not permitted' % error_key:
                e.append(('%s.%s.%s' % (attribute_model_name, attribute_model_attribute_name, relation_name),
                 self._get_invalid_filter_expression_message(attribute_model_name, attribute_model_attribute_name, relation_name, value)))
        ...
        if self.errors.get(error_key) == 'Searching on %s is not permitted' % error_key:
            e.append(('%s.%s.%s' % (model_name, attribute_name, relation_name),
             self._get_invalid_filter_expression_message(model_name, attribute_name, relation_name, value)))
        return e
    ...
    def _get_meta_relation(self, attribute, model_name, attribute_name):
        ...
        """
        return getattr(attribute, {'scalar': 'has', 'collection': 'any'}[self.schema[model_name][attribute_name]['type']])

    def _get_filter_expression(self, relation, value, model_name, attribute_name, relation_name, attribute=None, attribute_model_name=None, attribute_model_attribute_name=None):
        ...
        one.
        """
        invalid_model_attribute_errors = self._get_invalid_model_attribute_errors(relation, value, model_name, attribute_name, relation_name, attribute, attribute_model_name, attribute_model_attribute_name)
        if invalid_model_attribute_errors:
            ...
            try:
                ...
                filter_expression = None
                self._add_to_errors('%s.%s' % (model_name, attribute_name), 'The %s.%s attribute does not represent a many-to-one relation.' % (
                 model_name, attribute_name))
            except TypeError:
                filter_expression = None
                self._add_to_errors('%s.%s.%s' % (model_name, attribute_name, relation_name), self._get_invalid_filter_expression_message(model_name, attribute_name, relation_name, value))
    ...
    def _get_simple_filter_expression(self, *args):
        ...
        2. ['Form', 'enterer', 'first_name', 'like', 'J%'] =>
           Session.query(model.Form)                .filter(model.Form.enterer.has(model.User.first_name.like(u'J%')))

        3. ['Tag', 'name', 'like', '%abc%'] (when searching the Form model) =>
           aliased_tag = aliased(model.Tag)
           Session.query(model.Form)                .filter(aliased_tag.name.like(u'%abc%'))                .outerjoin(aliased_tag, model.Form.tags)

        4. ['Form', 'tags', 'name', 'like', '%abc%'] =>
           Session.query(model.Form)                .filter(model.Form.tags.any(model.Tag.name.like(u'%abc%')))
        """
        ...
        if len(args) == 4:
            ...
            return self._get_filter_expression(relation, value, model_name, attribute_name, relation_name, attribute, attribute_model_name, attribute_model_attribute_name)