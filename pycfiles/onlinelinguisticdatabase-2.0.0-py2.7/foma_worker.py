...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/foma_worker.py
...
def generate_language_model(**kwargs):
    ...
    try:
        ...
        lm.generate_message = 'Error writing the corpus file. %s' % e

    try:
        lm.write_vocabulary()
    except Exception as e:
        lm.generate_message = 'Error writing the vocabulary file. %s' % e

    try:
        lm.write_arpa(kwargs['timeout'])
    except Exception as e:
        lm.generate_message = 'Error writing the ARPA file. %s' % e

    try:
        lm.generate_trie()
    except Exception as e:
        lm.generate_message = 'Error generating the LMTrie instance. %s' % e

    if lm.get_modification_time(trie_path) != trie_mod_time:
        lm.generate_succeeded = True
        lm.generate_message = 'Language model successfully generated.'
    else:
        lm.generate_message = 'Error generating the LMTrie instance.'
