...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/morphologies.py
...
"""
import logging, simplejson as json, os, cPickle
...
class MorphologiesController(BaseController):
    ...
    def history(self, id):
        ...
        if morphology or previous_versions:
            return {'morphology': morphology, 'previous_versions': previous_versions}
        else:
            response.status_int = 404
...
def generate_and_compile_morphology(morphology_id, compile_=True):
    ...
    foma_worker_q.put({'id': h.generate_salt(), 
       ...
       'args': {'morphology_id': morphology.id, 
                'compile': compile_, 
