...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/morphemelanguagemodels.py
...
class MorphemelanguagemodelsController(BaseController):
    ...
    def edit(self, id):
        ...
        lm = h.eagerload_morpheme_language_model(Session.query(MorphemeLanguageModel)).get(id)
        if lm:
            return {'data': get_data_for_new_edit(dict(request.GET)), 'morpheme_language_model': lm}
        else:
            response.status_int = 404
            return {'error': 'There is no morpheme language model with id %s' % id}
    ...
    def history(self, id):
        ...
        if lm or previous_versions:
            return {'morpheme_language_model': lm, 'previous_versions': previous_versions}
        else:
            response.status_int = 404
    ...
    def generate(self, id):
        ...
        if not lm:
            response.status_int = 404
            return {'error': 'There is no morpheme language model with id %s' % id}
        args = {'morpheme_language_model_id': lm.id, 
    ...
    def serve_arpa(self, id):
        ...
        if lm:
            ...
            if os.path.isfile(arpa_path):
                ...
                response.status_int = 404
                return json.dumps({'error': 'The ARPA file for morpheme language model %s has not been compiled yet.' % id})
        else:
            response.status_int = 404
...
def authorized_to_access_arpa_file(user, morpheme_language_model):
    ...
    if morpheme_language_model.restricted and user.role != 'administrator' and user not in h.get_unrestricted_users():
        return False
    return True
...
def create_new_morpheme_language_model(data):
    ...
    """
    morpheme_language_model = MorphemeLanguageModel(parent_directory=h.get_OLD_directory_path('morphemelanguagemodels', config=config), rare_delimiter=h.rare_delimiter, start_symbol=h.lm_start, end_symbol=h.lm_end, morpheme_delimiters=h.get_morpheme_delimiters(type_='unicode'), UUID=unicode(uuid4()), name=h.normalize(data['name']), description=h.normalize(data['description']), enterer=session['user'], modifier=session['user'], datetime_modified=h.now(), datetime_entered=h.now(), vocabulary_morphology=data['vocabulary_morphology'], corpus=data['corpus'], toolkit=data['toolkit'], order=data['order'], smoothing=data['smoothing'], categorial=data['categorial'])
    return morpheme_language_model
