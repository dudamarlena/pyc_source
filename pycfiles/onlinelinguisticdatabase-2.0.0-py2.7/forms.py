...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/forms.py
...
"""
import logging, re, simplejson as json
...
class FormsController(BaseController):
    ...
    def delete(self, id):
        ...
        if form:
            if session['user'].role == 'administrator' or getattr(form.enterer, 'id', None) == getattr(session['user'], 'id', False):
                form_dict = form.get_dict()
                backup_form(form_dict)
    ...
    def history(self, id):
        ...
        if form or previous_versions:
            ...
            unrestricted_previous_versions = [ fb for fb in previous_versions if accessible(user, fb, unrestricted_users)
                                             ]
            ...
            if form_is_restricted or previous_versions_are_restricted:
                response.status_int = 403
                return h.unauthorized_msg
            return {'form': form, 'previous_versions': unrestricted_previous_versions}
        else:
            response.status_int = 404
    ...
    def remember(self):
        ...
        if forms:
            ...
            unrestricted_users = h.get_unrestricted_users()
            user = session['user']
            unrestricted_forms = [ f for f in forms if accessible(user, f, unrestricted_users)
                                 ]
...
def get_new_edit_form_data(GET_params):
    ...
    *only* if the datetime does *not* match the most recent ``datetime_modified``
    ...
    """
    param_name2model_name = {'grammaticalities': 'ApplicationSettings', 
       ...
       'users': 'User', 
       'sources': 'Source'}
    map_ = {'grammaticalities': h.get_grammaticalities, 
        ...
        for key in map_:
            ...
            if val:
                ...
                if val_as_datetime_obj:
                    if val_as_datetime_obj != h.get_most_recent_modification_datetime(param_name2model_name[key]):
                        result[key] = map_[key]()
                else:
                    result[key] = map_[key]()
...
def create_new_form(data):
    ...
    form.phonetic_transcription = h.to_single_space(h.normalize(data['phonetic_transcription']))
    form.narrow_phonetic_transcription = h.to_single_space(h.normalize(data['narrow_phonetic_transcription']))
    ...
    Session.add(form)
    form.morpheme_break_ids, form.morpheme_gloss_ids, form.syntactic_category_string, form.break_gloss_category, cache = compile_morphemic_analysis(form)
    return form
...
def update_form(form, data):
    ...
    """
    changed = False
    changed = form.set_attr('transcription', h.to_single_space(h.normalize(data['transcription'])), changed)
    changed = form.set_attr('phonetic_transcription', h.to_single_space(h.normalize(data['phonetic_transcription'])), changed)
    changed = form.set_attr('narrow_phonetic_transcription', h.to_single_space(h.normalize(data['narrow_phonetic_transcription'])), changed)
    changed = form.set_attr('morpheme_break', h.to_single_space(h.normalize(data['morpheme_break'])), changed)
    changed = form.set_attr('morpheme_gloss', h.to_single_space(h.normalize(data['morpheme_gloss'])), changed)
    ...
    if set(tags_to_add) != set(form.tags):
        ...
        changed = True
    morpheme_break_ids, morpheme_gloss_ids, syntactic_category_string, break_gloss_category, cache = compile_morphemic_analysis(form)
...
def update_morpheme_references_of_form(form, valid_delimiters=None, **kwargs):
    ...
    """
    changed = False
    morpheme_break_ids, morpheme_gloss_ids, syntactic_category_string, break_gloss_category, cache = compile_morphemic_analysis(form, valid_delimiters, **kwargs)
    changed = form.set_attr('morpheme_break_ids', morpheme_break_ids, changed)
    changed = form.set_attr('morpheme_gloss_ids', morpheme_gloss_ids, changed)
    changed = form.set_attr('syntactic_category_string', syntactic_category_string, changed)
    changed = form.set_attr('break_gloss_category', break_gloss_category, changed)
...
def update_morpheme_references_of_forms(forms, valid_delimiters, **kwargs):
    ...
    for form in forms:
        morpheme_break_ids, morpheme_gloss_ids, syntactic_category_string, break_gloss_category, kwargs['cache'] = compile_morphemic_analysis(form, valid_delimiters, **kwargs)
        if (morpheme_break_ids, morpheme_gloss_ids, syntactic_category_string, break_gloss_category) != (
         form.morpheme_break_ids, form.morpheme_gloss_ids, form.syntactic_category_string, form.break_gloss_category):
            form_buffer.append({'id_': form.id, 
    ...
    if form_buffer:
        ...
        update = form_table.update().where(form_table.c.id == bindparam('id_')).values(**dict([ (k, bindparam(k)) for k in form_buffer[0] if k != 'id_' ]))
...
def compileMorphemicAnalysis_(form, morpheme_delimiters=None, **kwargs):
    ...
    */a/b/c/d*, *a* is the ``morpheme_break`` value, *b* is the ``morpheme_gloss``
    ...
    def join(bgc, morpheme_delimiters, bgc_delimiter):
        ...
        """
        if bgc[0] in morpheme_delimiters and bgc[1] in morpheme_delimiters and bgc[2] in morpheme_delimiters:
            return bgc[0]
    ...
    def morphemic_analysis_is_consistent(**kwargs):
        ...
        """
        return kwargs['morpheme_break'] != '' and kwargs['morpheme_gloss'] != '' and len(kwargs['mb_words']) == len(kwargs['mg_words']) and [ len(re.split(kwargs['morpheme_splitter'], mbw)) for mbw in kwargs['mb_words'] ] == [ len(re.split(kwargs['morpheme_splitter'], mgw)) for mgw in kwargs['mg_words'] ]
    ...
    def get_category_from_partial_match(morpheme_matches, gloss_matches):
        ...
        """
        return filter(None, [ getattr(m.syntactic_category, 'name', None) for m in morpheme_matches ] + [ getattr(g.syntactic_category, 'name', None) for g in gloss_matches ] + [h.unknown_category])[0]

    def get_break_gloss_category(morpheme_delimiters, morpheme_break, morpheme_gloss, syntactic_category_string, bgc_delimiter):
        """Return a ``break_gloss_category`` string, e.g., u'le|the|Det-s|PL|Num chien|dog|N-s|PL|Num'."""
        try:
            delimiters = [
             ' '] + morpheme_delimiters
            splitter = '([%s])' % ('').join([ h.esc_RE_meta_chars(d) for d in delimiters ])
            ...
            return ('').join([ join(bgc, delimiters, bgc_delimiter) for bgc in break_gloss_category ])
        except TypeError:
            return
    ...
    def get_perfect_matches(form, word_index, morpheme_index, morpheme, gloss, matches_found, lexical_items, deleted_lexical_items, whole_db):
        ...
        *and* ``f.morpheme_gloss==gloss``.
        ...
        """
        if (
         morpheme, gloss) in matches_found:
        ...
        if whole_db:
            ...
            extant_perfect_matches_originally = [ (x[0][0], x[1][1], x[0][1], x[0][2]) for x in zip(extant_morpheme_break_ids[word_index][morpheme_index], extant_morpheme_gloss_ids[word_index][morpheme_index]) if x[0][0] == x[1][0]
                                                ]
            extant_perfect_matches = [ get_fake_form(m) for m in extant_perfect_matches_originally if m[0] not in [ f.id for f in lexical_items + deleted_lexical_items ]
                                     ]
            perfect_matches_in_lexical_items = [ f for f in lexical_items if f.morpheme_break == morpheme and f.morpheme_gloss == gloss
                                               ]
            perfect_matches_now = sorted(extant_perfect_matches + perfect_matches_in_lexical_items, key=lambda f: f.id)
            ...
            result = Session.query(Form).filter(Form.morpheme_break == morpheme).filter(Form.morpheme_gloss == gloss).order_by(asc(Form.id)).all()
    ...
    def get_partial_matches(form, word_index, morpheme_index, matches_found, **kwargs):
        ...
        force_query = kwargs.get('force_query')
        ...
        if (morpheme, gloss) in matches_found:
            ...
            if whole_db:
                ...
                if value in force_query:
                    ...
                    extant_partial_matches = [ (x[0], None, x[1], x[2]) if morpheme else (x[0], x[1], None, x[2]) for x in extant_analyses
                                             ]
                    extant_partial_matches = [ get_fake_form(m) for m in extant_partial_matches if m[0] not in [ f.id for f in lexical_items + deleted_lexical_items ]
                                             ]
                    partial_matches_in_lexical_items = [ f for f in lexical_items if getattr(f, attribute) == value
                                                       ]
                    result = sorted(extant_partial_matches + partial_matches_in_lexical_items, key=lambda f: f.id)
            else:
                result = Session.query(Form).filter(getattr(Form, attribute) == value).order_by(asc(Form.id)).all()
            matches_found[(morpheme, gloss)] = result
            return (result, matches_found)
    ...
    morpheme_splitter = morpheme_delimiters and '[%s]' % ('').join([ h.esc_RE_meta_chars(d) for d in morpheme_delimiters ]) or ''
    morpheme_break = form.morpheme_break
    morpheme_gloss = form.morpheme_gloss
    mb_words = morpheme_break.split()
    mg_words = morpheme_gloss.split()
    sc_words = morpheme_break.split()[:]
    if morphemic_analysis_is_consistent(morpheme_delimiters=morpheme_delimiters, morpheme_break=morpheme_break, morpheme_gloss=morpheme_gloss, mb_words=mb_words, mg_words=mg_words, morpheme_splitter=morpheme_splitter):
        for i in range(len(mb_words)):
            ...
            mb_word_morphemes_list = re.split(morpheme_and_delimiter_splitter, mb_word)[::2]
            mg_word_morphemes_list = re.split(morpheme_and_delimiter_splitter, mg_word)[::2]
            sc_word_analysis = re.split(morpheme_and_delimiter_splitter, sc_word)
            for j in range(len(mb_word_morphemes_list)):
                morpheme = mb_word_morphemes_list[j]
                gloss = mg_word_morphemes_list[j]
                perfect_matches, matches_found = get_perfect_matches(form, i, j, morpheme, gloss, matches_found, lexical_items, deleted_lexical_items, whole_db)
                if perfect_matches and type(perfect_matches) is list:
                    mb_word_analysis.append([ (f.id, f.morpheme_gloss, getattr(f.syntactic_category, 'name', None)) for f in perfect_matches
                                            ])
                    mg_word_analysis.append([ (f.id, f.morpheme_break, getattr(f.syntactic_category, 'name', None)) for f in perfect_matches
                                            ])
                    sc_word_analysis[j * 2] = getattr(perfect_matches[0].syntactic_category, 'name', h.unknown_category)
                else:
                    morpheme_matches, matches_found = get_partial_matches(form, i, j, matches_found, morpheme=morpheme, force_query=perfect_matches, lexical_items=lexical_items, deleted_lexical_items=deleted_lexical_items, whole_db=whole_db)
                    if morpheme_matches:
                        mb_word_analysis.append([ (f.id, f.morpheme_gloss, getattr(f.syntactic_category, 'name', None)) for f in morpheme_matches
                                                ])
                    else:
                        mb_word_analysis.append([])
                    gloss_matches, matches_found = get_partial_matches(form, i, j, matches_found, gloss=gloss, force_query=perfect_matches, lexical_items=lexical_items, deleted_lexical_items=deleted_lexical_items, whole_db=whole_db)
                    if gloss_matches:
                        mg_word_analysis.append([ (f.id, f.morpheme_break, getattr(f.syntactic_category, 'name', None)) for f in gloss_matches
                                                ])
                    else:
        ...
        break_gloss_category = get_break_gloss_category(morpheme_delimiters, morpheme_break, morpheme_gloss, syntactic_category_string, bgc_delimiter)
    else:
        morpheme_break_ids = morpheme_gloss_ids = syntactic_category_string = break_gloss_category = None
    return (
     unicode(json.dumps(morpheme_break_ids)), unicode(json.dumps(morpheme_gloss_ids)),
...
def update_forms_containing_this_form_as_morpheme(form, change='create', previous_version=None):
    ...
    if h.is_lexical(form):
        ...
        start_patt = '(%s)' % ('|').join(escaped_morpheme_delimiters + [' ', '^'])
        end_patt = '(%s)' % ('|').join(escaped_morpheme_delimiters + [' ', '$'])
        ...
        if change == 'delete':
            updated_form_ids = update_morpheme_references_of_forms(matches, morpheme_delimiters, deleted_lexical_items=[form])
        else:
            updated_form_ids = update_morpheme_references_of_forms(matches, morpheme_delimiters, lexical_items=[form])
...
def update_has_changed_the_analysis(form, form_dict):
    ...
    return form.morpheme_break != form_dict['morpheme_break'] or form.morpheme_gloss != form_dict['morpheme_gloss'] or form.break_gloss_category != form_dict['break_gloss_category'] or getattr(form.syntactic_category, 'name', None) != old_syntactic_category_name
...
def update_collections_referencing_this_form(form):
    ...
    collections_referencing_this_form = Session.query(Collection).filter(Collection.contents.op('regexp')(pattern)).all()
