...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/syntacticcategories.py
...
class SyntacticcategoriesController(BaseController):
    ...
    def edit(self, id):
        ...
        syntactic_category = Session.query(SyntacticCategory).get(id)
        if syntactic_category:
            return {'data': {'syntactic_category_types': h.syntactic_category_types}, 'syntactic_category': syntactic_category}
        else:
            response.status_int = 404
            return {'error': 'There is no syntactic category with id %s' % id}
