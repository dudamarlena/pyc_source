...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/model/model.py
...
class Model(object):
    ...
    __table_args__ = {'mysql_charset': 'utf8', 
       'mysql_engine': 'MyISAM'}
    table_name2core_attributes = {'corpus': [
                'id', 'name'], 
       'corpusfile': [
                    'id', 'filename', 'datetime_modified', 'format', 'restricted'], 
       'elicitationmethod': [
                           'id', 'name'], 
       'file': [
              'id', 'name', 'filename', 'MIME_type', 'size', 'url', 'lossy_filename'], 
       'formsearch': [
                    'id', 'name'], 
       ...
       'morphology': [
                    'id', 'name'], 
       'orthography': [
                     'id', 'name', 'orthography', 'lowercase', 'initial_glottal_stops'], 
       'phonology': [
                   'id', 'name'], 
       'source': [
                'id', 'crossref', 'crossref_source', 'type', 'key',
                'journal', 'editor', 'chapter', 'pages', 'publisher', 'booktitle',
                'school', 'institution', 'year', 'author', 'title', 'note'], 
       'speaker': [
                 'id', 'first_name', 'last_name', 'dialect'], 
       'syntacticcategory': [
                           'id', 'name'], 
       'tag': [
             'id', 'name'], 
       'translation': [
                     'id', 'transcription', 'grammaticality'], 
       'user': [
    ...
    def get_mini_dict(self, model=None):
        model = model or self
        return self.get_dict_from_model(model, self.table_name2core_attributes.get(model.__tablename__, []))
