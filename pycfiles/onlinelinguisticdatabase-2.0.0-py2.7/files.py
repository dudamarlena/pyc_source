...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/files.py
...
"""
import logging, datetime, os, shutil, simplejson as json
...
from onlinelinguisticdatabase.lib.schemata import FileCreateWithBase64EncodedFiledataSchema, FileCreateWithFiledataSchema, FileSubintervalReferencingSchema, FileExternallyHostedSchema, FileUpdateSchema
...
class FilesController(BaseController):
    ...
    def search(self):
        ...
        try:
            ...
            SQLAQuery = h.eagerload_file(self.query_builder.get_SQLA_query(python_search_params.get('query')))
    ...
    def create(self):
        ...
        try:
            if request.content_type == 'application/json':
                if len(request.body) > 20971520:
                    response.status_int = 400
                    return {'error': 'The request body is too large; use the multipart/form-data Content-Type when uploading files greater than 20MB.'}
    ...
    def delete(self, id):
        ...
        if file:
            if session['user'].role == 'administrator' or file.enterer is session['user']:
                delete_file(file)
                return file
            else:
                response.status_int = 403
...
def serve_file(id, reduced=False):
    ...
    if getattr(file, 'parent_file', None):
        ...
        response.status_int = 400
        return json.dumps({'error': 'The content of file %s is stored elsewhere at %s' % (id, file.url)})
    if file:
        ...
        if reduced:
            ...
            if not filename:
                response.status_int = 404
                return json.dumps({'error': 'There is no size-reduced copy of file %s' % id})
...
def get_unique_file_path(file_path):
    ...
    while 1:
        ...
        file_path = '%s_%s%s' % (file_path_parts[0][:230],
...
def create_subinterval_referencing_file(data):
    ...
    file.name = h.normalize(data['name']) or file.parent_file.filename
...
def delete_file(file):
    ...
    if getattr(file, 'filename', None):
        file_path = os.path.join(h.get_OLD_directory_path('files', config=config), file.filename)
        os.remove(file_path)
    if getattr(file, 'lossy_filename', None):
        file_path = os.path.join(h.get_OLD_directory_path('reduced_files', config=config), file.lossy_filename)
        os.remove(file_path)
    Session.delete(file)
    Session.commit()
    return
...
def get_new_edit_file_data(GET_params):
    ...
    """
    param_name2model_name = {'tags': 'Tag', 
       'speakers': 'Speaker', 
       'users': 'User'}
    map_ = {'tags': h.get_mini_dicts_getter('Tag'), 
