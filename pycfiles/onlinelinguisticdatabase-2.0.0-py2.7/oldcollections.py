...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/controllers/oldcollections.py
...
class OldcollectionsController(BaseController):
    ...
    def search(self):
        ...
        try:
            ...
            SQLAQuery = h.eagerload_collection(self.query_builder.get_SQLA_query(python_search_params.get('query')))
    ...
    def create(self):
        ...
        try:
            ...
            collections_referenced = get_collections_referenced(values['contents'], user, unrestricted_users)
            ...
            response.status_int = 400
            return {'error': 'Invalid collection reference error: there is no collection with id %d' % e.args[0]}
        except UnauthorizedCollectionReferenceError:
            response.status_int = 403
            return {'error': 'Unauthorized collection reference error: you are not authorized to access collection %d' % e.args[0]}
        except Invalid as e:
            response.status_int = 400
            return {'errors': e.unpack_errors()}
    ...
    def update(self, id):
        ...
        """
        collection = h.eagerload_collection(Session.query(Collection), eagerload_forms=True).get(int(id))
        if collection:
            unrestricted_users = h.get_unrestricted_users()
            user = session['user']
            if h.user_is_authorized_to_access_model(user, collection, unrestricted_users):
                try:
                    schema = CollectionSchema()
                    values = json.loads(unicode(request.body, request.charset))
                    collections_referenced = get_collections_referenced(values['contents'], user, unrestricted_users, id)
                    values = add_contents_unpacked_to_values(values, collections_referenced)
                    values = add_form_ids_list_to_values(values)
                    state = h.get_state_object(values)
                    data = schema.to_python(values, state)
                    collection_dict = collection.get_full_dict()
                    collection, restricted, contents_changed = update_collection(collection, data, collections_referenced)
                    if collection:
                        backup_collection(collection_dict)
                        update_collections_that_reference_this_collection(collection, self.query_builder, restricted=restricted, contents_changed=contents_changed)
                        Session.add(collection)
                        Session.commit()
                        return collection.get_full_dict()
                    response.status_int = 400
                    ...
                    response.status_int = 400
                    return {'error': 'Circular collection reference error: collection %d references collection %d.' % (id, e.args[0])}
                except InvalidCollectionReferenceError as e:
                    response.status_int = 400
                    return {'error': 'Invalid collection reference error: there is no collection with id %d' % e.args[0]}
                except UnauthorizedCollectionReferenceError:
    ...
    def delete(self, id):
        ...
        """
        collection = h.eagerload_collection(Session.query(Collection), eagerload_forms=True).get(id)
        if collection:
            if session['user'].role == 'administrator' or collection.enterer is session['user']:
                session['user'] = Session.merge(session['user'])
                collection.modifier = session['user']
                collection_dict = collection.get_full_dict()
                backup_collection(collection_dict)
                update_collections_that_reference_this_collection(collection, self.query_builder, deleted=True)
    ...
    def edit(self, id):
        ...
        if collection:
            unrestricted_users = h.get_unrestricted_users()
            if h.user_is_authorized_to_access_model(session['user'], collection, unrestricted_users):
    ...
    def history(self, id):
        ...
        if collection or previous_versions:
            ...
            unrestricted_previous_versions = [ cb for cb in previous_versions if accessible(user, cb, unrestricted_users)
                                             ]
            ...
            if collection_is_restricted or previous_versions_are_restricted:
                response.status_int = 403
                return h.unauthorized_msg
            return {'collection': collection, 'previous_versions': unrestricted_previous_versions}
        else:
            response.status_int = 404
...
def get_collections_referenced(contents, user=None, unrestricted_users=None, collection_id=None, patt=None):
    ...
    collections_referenced = dict([ (int(id), get_collection(int(id), user, unrestricted_users)) for id in patt.findall(contents)
                                  ])
    ...
    [ collections_referenced.update(get_collections_referenced(collections_referenced[id].contents, user, unrestricted_users, collection_id, patt)) for id in temp
    ]
...
def get_collections_referenced_in_contents(collection, collections_referenced):
    ...
    """
    return [ collections_referenced[int(id)] for id in h.collection_reference_pattern.findall(collection.contents)
           ]
...
def update_collections_that_reference_this_collection(collection, query_builder, **kwargs):
    ...
    def update_contents_unpacked_etc(collection, **kwargs):
        ...
        collection.contents_unpacked = generate_contents_unpacked(collection.contents, collections_referenced)
        collection.html = h.get_HTML_from_contents(collection.contents_unpacked, collection.markup_language)
        collection.forms = [ Session.query(Form).get(int(id)) for id in h.form_reference_pattern.findall(collection.contents_unpacked)
                           ]
    ...
    if restricted or contents_changed or deleted:
        collections_referencing_this_collection = get_collections_referencing_this_collection(collection, query_builder)
        collections_referencing_this_collection_dicts = [ c.get_full_dict() for c in collections_referencing_this_collection
                                                        ]
        now = h.now()
        if restricted:
        ...
        if deleted:
            [ update_contents_unpacked_etc(c, collection_id=collection.id, deleted=True) for c in collections_referencing_this_collection ]
...
def get_collections_referencing_this_collection(collection, query_builder):
    ...
    """
    patt = h.collection_reference_pattern.pattern.replace('\\d+', str(collection.id)).replace('\\', '')
...
def update_collection_by_deletion_of_referenced_form(collection, referenced_form):
    ...
    backup_collection(collection_dict)
    update_collections_that_reference_this_collection(collection, OldcollectionsController.query_builder, contents_changed=True)
    Session.add(collection)
    Session.commit()
...
def remove_references_to_this_form(contents, form_id):
    ...
    """
    patt = re.compile(h.form_reference_pattern.pattern.replace('\\d+', str(form_id)))
...
def remove_references_to_this_collection(contents, collection_id):
    ...
    """
    patt = re.compile(h.collection_reference_pattern.pattern.replace('\\d+', str(collection_id)))
    return patt.sub('', contents)
...
def get_contents(collection_id, collections_referenced):
    ...
    """
    return getattr(collections_referenced[collection_id], 'contents', 'Collection %d has no contents.' % collection_id)
...
def generate_contents_unpacked(contents, collections_referenced, patt=None):
    ...
    """
    patt = patt or re.compile(h.collection_reference_pattern)
    return patt.sub(lambda m: generate_contents_unpacked(get_contents(int(m.group(1)), collections_referenced), collections_referenced, patt), contents)
...
def get_collection(collection_id, user, unrestricted_users):
    ...
    if collection:
        if user is None or unrestricted_users is None or h.user_is_authorized_to_access_model(user, collection, unrestricted_users):
            return collection
...
def get_new_edit_collection_data(GET_params):
    ...
    """
    param_name2model_name = {'speakers': 'Speaker', 
       'users': 'User', 
       'tags': 'Tag', 
       'sources': 'Source'}
    map_ = {'speakers': h.get_mini_dicts_getter('Speaker'), 
...
def create_new_collection(data, collections_referenced):
    ...
    immediately_referenced_collections = get_collections_referenced_in_contents(collection, collections_referenced)
...
def update_collection(collection, data, collections_referenced):
    ...
    changed = collection.set_attr('html', h.get_HTML_from_contents(collection.contents_unpacked, collection.markup_language), changed)
    ...
    if set(tags_to_add) != set(collection.tags):
        if 'restricted' in [ t.name for t in tags_to_add ] and 'restricted' not in [ t.name for t in collection.tags ]:
            restricted = True
    ...
    if changed:
        ...
        collection.modifier = session['user']
        return (
         collection, restricted, contents_changed)
    return (
     changed, restricted, contents_changed)