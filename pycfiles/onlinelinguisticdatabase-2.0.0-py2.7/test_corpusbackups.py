...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/tests/functional/test_corpusbackups.py
...
class TestCorpusbackupsController(TestController):
    ...
    def test_index(self):
        ...
        response = self.app.get(url('corpusbackups'), paginator, headers=self.json_headers, extra_environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        assert len(resp['items']) == 1
        assert resp['paginator']['count'] == 2
        assert response.content_type == 'application/json'
        assert resp['items'][0]['id'] == all_corpus_backup_ids[1]
        order_by_params = {'order_by_model': 'CorpusBackup', 'order_by_attribute': 'id', 
           'order_by_direction': 'desc'}
        response = self.app.get(url('corpusbackups'), order_by_params, headers=self.json_headers, extra_environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        result_set = list(reversed(all_corpus_backup_ids))
        assert [ cb['id'] for cb in resp ] == result_set
        params = {'order_by_model': 'CorpusBackup', 'order_by_attribute': 'id', 'order_by_direction': 'desc', 
           'items_per_page': 1, 'page': 1}
        response = self.app.get(url('corpusbackups'), params, headers=self.json_headers, extra_environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        assert result_set[0] == resp['items'][0]['id']
        response = self.app.get(url('corpusbackup', id=all_corpus_backup_ids[0]), headers=self.json_headers, extra_environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        assert resp['description'] == 'Covers a lot of the data.'
        assert resp['content'] == test_corpus_content
        assert response.content_type == 'application/json'
        response = self.app.get(url('corpusbackup', id=100987), headers=self.json_headers, extra_environ=self.extra_environ_view, status=404)
        resp = json.loads(response.body)
        ...
        response = self.app.post(url('/corpusbackups/search'), json_query, self.json_headers, extra_environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        ...
        json_query = json.dumps({'query': {'filter': [
                              'CorpusBackup', 'description', 'like', '%less%']}})
        response = self.app.request(url('corpusbackups'), method='SEARCH', body=json_query, headers=self.json_headers, environ=self.extra_environ_admin)
        resp = json.loads(response.body)
        assert len(resp) == len(result_set) == 1
        assert resp[0]['description'] == result_set[0]
        assert response.content_type == 'application/json'
    ...
    def test_new_search(self):
        ...
        response = self.app.get(url('/corpusbackups/new_search'), headers=self.json_headers, extra_environ=self.extra_environ_view)
        resp = json.loads(response.body)
