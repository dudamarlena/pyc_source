...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/onlinelinguisticdatabase/lib/simplelm/SimpleModKN.py
...
class ModKNSmoother():
    ...
    def __init__(self, order=3, sb='<s>', se='</s>'):
        ...
        self.nonZeros = [ defaultdict(lambda : defaultdict(float))
    ...
    def _kn_recurse(self, ngram_stack, i):
        ...
        self.nonZeros[i][denom][ngram_stack[(-1)]] += 1.0
    ...
    def print_ARPA(self):
        ...
        print '-99.00000\t%s\t%0.7f' % (self.sb, log(lmda, 10.0))
        for key in sorted(self.UN.iterkeys()):
            if key == self.se:
                print '%0.7f\t%s\t-99' % (log(self.UN[key] / self.UD, 10.0), key)
                continue
            d = self._get_discount(0, key)
            lmda = d / self.denominators[0][key]
            print '%0.7f\t%s\t%0.7f' % (log(self.UN[key] / self.UD, 10.0), key, log(lmda, 10.0))
        ...
        for o in xrange(0, self.order - 2):
            ...
            for key in sorted(self.numerators[o].iterkeys()):
                if key.endswith(self.se):
                    ...
                    print '%0.7f\t%s' % (log(prob, 10.0), key)
                    continue
                d = self._get_discount(o + 1, key)
                lmda = d / self.denominators[(o + 1)][key]
                prob = self._compute_interpolated_prob(key)
                print '%0.7f\t%s\t%0.7f' % (log(prob, 10.0), key, log(lmda, 10.0))
