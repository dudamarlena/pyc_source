...
# [GCC 8.4.0]
# Embedded file name: /home/lev/Documents/repositories/github.com/pageit/pageit/render.py
# Compiled at: 2015-02-24 08:15:18
"""pageit generator"""
from fnmatch import fnmatch
from os import path as osp
import codecs, logging, os, re, sys
from argh import arg, expects_obj, ArghParser
from mako.lookup import TemplateLookup
import mako.exceptions, yaml
...
try:
    ...
    import __init__ as pageit

logging.basicConfig(format='%(levelname)-8s %(message)s')
RE_MAKO_IMPORT = re.compile('<%(include|inherit|namespace)\\s+file="([^"]*)"')
DEFAULT = Namespace(log=logging.getLogger('com.metaist.pageit.render'), path='.', tmp=None, config='pageit.yml', env='default', ext='.mako', port=80, verbosity=1)
MSG_PRE = tools.MSG_PRE
MSG = tools.MSG + Namespace(DONE=MSG_PRE + 'done', T_RENDER=MSG_PRE + 'started rendering <%s>', T_MTIME=MSG_PRE + 'mtime of <%s>', T_MTIME_END=MSG_PRE + 'dependencies: %s (%s)', DRY=' (dry run)', DRY_RUN=MSG_PRE + '** Dry Run! No files will be altered. **', IGNORE_MTIME=MSG_PRE + 'Ignoring modification times.', NO_CHANGE=MSG_PRE + 'no change in <%s>', DELETE=MSG_PRE + 'deleted <%s>', DELETE_ERR=MSG_PRE + 'cannot delete %s', RENDER=MSG_PRE + 'rendered <%s>', RENDER_ERR=MSG_PRE + 'cannot render %s', WRITE=MSG_PRE + 'wrote <%s>', WRITE_ERR=MSG_PRE + 'cannot write to %s', NO_ENV=MSG_PRE + 'missing environment <%s> in <%s>', LOAD_ENV=MSG_PRE + 'loading environment <%s> in <%s>')
...
class Pageit(object):
    ...
    def __init__(self, path=DEFAULT.path, ext=DEFAULT.ext, dry_run=False, noerr=False, ignore_mtime=False, watcher=None, tmpl=None, site=None, log=None):
        ...
        self.args = Namespace(ext=ext, noerr=noerr, dry_run=dry_run, ignore_mtime=ignore_mtime)
        if dry_run:
            self._dry = MSG.DRY
        ...
        for path in self.list():
            ...
            if not self.args.ignore_mtime:
                if osp.isfile(dest):
                    ...
                    self.log.debug(MSG_PRE + 'output: %s', '[MTIME]', output_changed)
                    self.log.debug(MSG_PRE + 'template: %s', '[MTIME]', template_changed)
    ...
    def mako(self, path, dest=None):
        ...
        page = DeepNamespace(path=name, output=osp.relpath(dest, self.path), dirname=osp.dirname(name), basedir=osp.relpath(self.path, osp.dirname(path)))
        try:
            if not self.args.dry_run:
        ...
        if not (self.args.noerr and has_errors):
            try:
                ...
                self.log.debug(MSG.WRITE + self._dry, _context, osp.relpath(dest, self.path))
    ...
    def mako_deps(self, path):
        ...
        for line in open(path):
            groups = RE_MAKO_IMPORT.search(line)
            if groups:
                dep = groups.group(2)
                if '/' == dep[0]:
    ...
    def mako_mtime(self, path, levels=5):
        ...
        self.log.debug(MSG.T_MTIME_END, _context, len(mtimes) - 1, (', ').join([ osp.relpath(dep, self.path) for dep in done[1:]
                                                                               ]))
        return max(mtimes)
...
def create_lookup(path=DEFAULT.path, tmp=None):
    ...
    return TemplateLookup(directories=[
     path], module_directory=tmp, input_encoding='utf-8', output_encoding='utf-8')
...
@arg('-r', '--render', default=False, help='render templates after --clean')
@arg('-w', '--watch', default=False, help='watch for file modifications')
@arg('-s', '--serve', metavar='PORT', nargs='?', const=DEFAULT.port, help='run basic HTTP server; deafult port is ' + str(DEFAULT.port))
@arg('-f', '--config', metavar='PATH', default=DEFAULT.config, help='yaml config file')
...
def render(args):
    ...
    runner = Pageit(path=args.path, ext=args.ext, dry_run=args.dry_run, noerr=args.noerr, ignore_mtime=args.ignore_mtime, site=site, tmpl=tmpl, log=log)
    if args.clean:
        runner.clean()
    ...
    with tools.watch(watch_path, runner.on_change, log) as (watcher):
        ...
        if args.serve:
            tools.serve(args.path, args.serve, log)
        elif args.watch:
            watcher.loop()
    return
...
def main():
    ...
    """
    parser = ArghParser(prog='pageit', description=pageit.__doc__, epilog=pageit.__epilog__)
    parser.add_argument('--version', action='version', version='%(prog)s ' + pageit.__version__)
    parser.add_argument('-v', '--verbose', dest='verbosity', action='count', default=1, help='show logging messages')
    parser.add_argument('-q', '--quiet', dest='verbosity', action='store_const', const=0, help='suppress logging messages')
