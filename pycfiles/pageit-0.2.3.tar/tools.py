...
# [GCC 8.4.0]
# Embedded file name: /home/lev/Documents/repositories/github.com/pageit/pageit/tools.py
# Compiled at: 2013-11-28 22:21:13
...
DEFAULT = Namespace(log=logging.getLogger('com.metaist.pageit.tools'), port=80)
MSG_PRE = '%-9s '
MSG = Namespace(START=MSG_PRE + 'started', STOP=MSG_PRE + 'stopped', T_SERVE=MSG_PRE + '%s on port [%s]', T_WATCH=MSG_PRE + '%s', CHANGE=MSG_PRE + 'change in <%s>', PATH_ERR=MSG_PRE + 'cannot find <%s>')
...
@contextmanager
def watch(path=None, callback=None, log=None):
...
class Watcher(FileSystemEventHandler):
    ...
    def start(self):
        ...
        if self.path:
            self._watch = self.observer.schedule(self, path=self.path, recursive=True)
        self.observer.start()
    ...
    def loop(self):
        ...
        Returns:
            Watcher: for method chaining
        """
        if not self.observer or not self.path:
            return self
        while True:
            try:
                time.sleep(1)
