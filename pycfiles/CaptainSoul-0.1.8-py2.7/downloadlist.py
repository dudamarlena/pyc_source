...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cptsoul/downloadmanager/downloadlist.py
...
class DownloadList(gtk.TreeView, CptCommon):
    ...
    def startFileTransfer(self, info, name, size, path):
        ...
        FileGetter(info, name, path, size, PreparedCaller(self.progressCallback, key=key), PreparedCaller(self.endCallback, key=key), PreparedCaller(self.errorCallback, key=key))
