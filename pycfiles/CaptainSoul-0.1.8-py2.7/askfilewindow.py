...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cptsoul/downloadmanager/askfilewindow.py
...
class AskFileWindow(gtk.Window, CptCommon):
    ...
    def __init__(self, info, name, size, desc):
        ...
        self.set_properties(title='CaptainSoul - File request', resizable=False, border_width=5)
    ...
    def acceptFileEvent(self, widget, info, name, size):
        dialog = gtk.FileChooserDialog(title='CatpainSoul - Choose destination', action=gtk.FILE_CHOOSER_ACTION_SAVE, buttons=(
