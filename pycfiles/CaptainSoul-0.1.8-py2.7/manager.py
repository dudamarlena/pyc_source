...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cptsoul/manager.py
# Compiled at: 2014-01-13 07:12:38
...
class Manager(gobject.GObject, ClientFactory, CptCommon):
    ...
    __gsignals__ = {'reconnecting': (
       ...
       'who': (
             gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT]), 
       'state': (
               gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT, gobject.TYPE_STRING]), 
       'is-typing': (
                   gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT]), 
       'cancel-typing': (
                       gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT]), 
       ...
       'contact-deleted': (
                         gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_STRING]), 
       'send-msg': (
                  gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_STRING, gobject.TYPE_PYOBJECT]), 
       'send-raw': (
                  gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_STRING]), 
       'get-raw': (
                 gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_STRING]), 
       'file-ask': (
                  gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT, gobject.TYPE_STRING, gobject.TYPE_ULONG, gobject.TYPE_STRING]), 
       'file-start': (
                    gobject.SIGNAL_RUN_LAST, gobject.TYPE_NONE, [gobject.TYPE_PYOBJECT, gobject.TYPE_STRING, gobject.TYPE_STRING, gobject.TYPE_INT])}
    ...
    def sendState(self, state):
        if self._protocol is not None:
            logging.info('Manager : Send state %s' % state)
            self._protocol.sendState(state)
        else:
            logging.warning('Manager : Try send state %s' % state)
        return

    def sendWatch(self, sendWho=True):
        if self._protocol is not None:
            logging.info('Manager : Send watch (send who = %s)' % sendWho)
            self._protocol.sendWatch(sendWho)
        else:
            logging.warning('Manager : Try send watch (send who = %s)' % sendWho)
        return

    def sendMsg(self, msg, dests):
        if self._protocol is not None:
            logging.info('Manager : Send msg "%s" to %s' % (msg, dests))
            self.emit('send-msg', msg, dests)
            self._protocol.sendMsg(msg, dests)
        else:
            logging.warning('Manager : Try send msg "%s" to %s' % (msg, dests))
        return
    ...
    def cmdFileAskHook(self, info, name, size, desc):
        logging.info('Manager : Cmd %s file ask %s %dB' % (info, name, size))
        self.emit('file-ask', info, name, size, desc)

    def cmdFileStartHook(self, info, name, ip, port):
        logging.info('Manager : Cmd %s file start %s %s:%d' % (info, name, ip, port))
