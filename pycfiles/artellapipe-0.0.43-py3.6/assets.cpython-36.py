...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/assets.py
# Compiled at: 2020-04-25 12:27:56
# Size of source mod 2**32: 23085 bytes
"""
...
import os, logging, inspect, traceback, importlib
...
class ArtellaAssetsManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-assets',
    ...
    def register_asset_class(self, asset_class):
        ...
        if asset_class in self._registered_asset_classes:
            LOGGER.warning('Asset Class: "{}" is already registered. Skipping ...'.format(asset_class))
            return False
        else:
    ...
    def get_assets_by_type(self, asset_type):
        ...
        if not self.is_valid_asset_type(asset_type):
            ...
            return [asset for asset in self.assets if asset if asset.FILE_TYPE == asset_type]
    ...
    def find_all_assets(self, force_update=False, force_login=True):
        ...
        if not artellapipe.Tracker().is_logged():
            LOGGER.warning('Impossible to find assets of current project because user is not log into production tracker')
            return
        else:
    ...
    def get_latest_published_versions(self, asset_path, file_type=None):
        ...
        if not status_data:
            ...
            if valid_version and version_found:
                ...
                latest_version.append({'version':ordered_versions.keys()[current_index], 
        ...
        if not allowed_types:
            if not allowed_tags:
                ...
                for asset in all_assets:
                    ...
                    if asset.FILE_TYPE in allowed_types or asset_category in allowed_types or asset.FILE_TYPE in allowed_tags:
            ...
            valid_assets or LOGGER.warning('No valid assets found in current scene!')
            return
        else:
            ...
            for namespace in all_namespaces:
                ...
                if clean_namespace not in all_ids:
                    ...
                    if namespace != node_id:
                        continue
                    namespace_nodes = tp.Dcc.all_nodes_in_namespace(namespace)
                    namespace_nodes or LOGGER.warning('Namespace "{}" is empty!'.format(namespace_nodes))
                else:
                    for namespace_node in namespace_nodes:
                        ...
                        if split_name not in namespace_node:
                            ...
                            if as_nodes:
                                asset_node = artellapipe.AssetNode(project=(self._project),
                                  node=root_node,
                                  asset=asset_node,
                                  id=namespace)
                            else:
    ...
    def get_assets_in_shot(self, shot, force_login=True):
        ...
        if not artellapipe.Tracker().is_logged():
            if force_login:
                artellapipe.Tracker().login()
            artellapipe.Tracker().is_logged() or LOGGER.warning('Impossible to find assets of current project because user is not log into production tracker')
            return
        else:
            tracker = artellapipe.Tracker()
    ...
    def get_asset_renderable_shapes(self, asset, remove_namespace=False, full_path=True):
        ...
        transform_relatives = tp.Dcc.list_relatives(node=(asset.get_name()),
          all_hierarchy=True,
          full_path=full_path,
            ...
            for asset_type, asset_type_info in self._config.get('types', default={}).items():
                ...
                if not full_asset_class:
                    ...
                    try:
                        ...
                        LOGGER.warning('Impossible to import Asset Module: {} | {} | {}'.format(asset_module, exc, traceback.format_exc()))
                        continue
