...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/utils/shader.py
# Compiled at: 2020-04-25 12:27:56
# Size of source mod 2**32: 16937 bytes
"""
...
import os, json, logging.config, tpDcc as tp
...
class ShadingNetwork(object):
        ...
        if not shader_extension:
            ...
            for shader in shaders:
                if shader not in IGNORE_SHADERS:
                    ...
                    shading_group or LOGGER.warning('No shading group linked to shader: "{}"'.format(shader))
                    continue
            ...
            if key == 'icon':
                ...
                if existing_material is not None:
                    if as_type == 'asShader':
                        ...
                        continue
                if as_type == 'asShader':
                    node = cls.create_shader_node(node_type=node_type, as_type=as_type,
                      name=key)
                    if node_type == 'displacementShader':
                        pass
                    else:
                        node_sg = maya.cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name=(key + 'SG'))
                        tp.Dcc.connect_attribute(source_node=node, source_attribute='outColor', target_node=node_sg,
                          target_attribute='surfaceShader',
                          force=True)
                else:
    ...
    def get_shading_group(cls, shader_node, prefix=None):
        ...
        for n in out_connections:
            if not not n:
                if not tp.Dcc.object_exists(n) or not tp.Dcc.node_type(n) == 'shadingEngine':
                    pass
                else:
        ...
        if not shading_groups_found:
            ...
            if len(shading_groups_found) > 1:
                ...
                if not same_sg:
                    LOGGER.warning('Multiple Shading Groups found for "{}": "{}". First one will be used exported ...'.format(prefix_name, shading_groups_found))
    ...
    def _attrs_to_dict(cls, shader_node, prefix=None):
        ...
        """
        attrs = {'asType':None, 
         'type':None,  'attr':dict(),  'connection':dict()}
        ...
        for attr in shader_attrs:
            if not maya.cmds.connectionInfo(('{0}.{1}'.format(shader_node, attr)), isDestination=True):
                ...
                cconnection_info = maya.cmds.connectionInfo(('{0}.{1}'.format(shader_node, attr)),
                  sourceFromDestination=True).split('.')
                ...
                if prefix:
                    ...
                    new_connection_name = '{}.{}'.format(connected_node, connection)
    ...
    def _set_attrs(cls, shader_node, attrs):
        for attr in attrs['connection']:
            ...
            try:
                tp.Dcc.connect_attribute(source_node=con_node, source_attribute=con_attr, target_node=shader_node,
                  target_attribute=attr,
                  force=True)
            except Exception:
                continue

        if 'notes' not in attrs['attr']:
            if tp.Dcc.object_exists(shader_node):
                if tp.Dcc.attribute_exists(node=shader_node, attribute_name='notes'):
                    tp.Dcc.set_string_attribute_value(node=shader_node, attribute_name='notes', attribute_value='')
        for attr in attrs['attr']:
            attr_type = None
            if attr in IGNORE_ATTRS:
                pass
            elif attr in attrs['connection']:
                continue
            if isinstance(attrs['attr'][attr], list):
                if len(attrs['attr'][attr]) == 3:
                    attr_type = 'double3'
                else:
                    if len(attrs['attr'][attr]) == 2:
                        attr_type = 'double2'
                    attribute = attrs['attr'][attr]
                    try:
                        (maya.cmds.setAttr)(
 '{0}.{1}'.format(shader_node, attr), *attribute, **{'type': attr_type})
                    except Exception as exc:
                        LOGGER.debug('ShaderLibrary: setAttr {0} skipped!'.format(attr))
                        continue

            elif isinstance(attrs['attr'][attr], basestring):
                if attr == 'notes':
                    if not tp.Dcc.attribute_exists(node=shader_node, attribute_name='notes'):
                ...
                try:
                    ...
                    tp.Dcc.set_string_attribute_value(node=shader_node,
                      attribute_name=attr,
                ...
                try:
                    tp.Dcc.set_attribute_value(node=shader_node,
                      attribute_name=attr,
                      attribute_value=(attrs['attr'][attr]))
                except Exception as exc:
                    LOGGER.debug('ShaderLibrary: setAttr {0} skipped!'.format(attr))
                    continue