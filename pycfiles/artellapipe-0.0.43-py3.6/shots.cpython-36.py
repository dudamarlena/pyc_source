...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/shots.py
# Compiled at: 2020-04-25 12:27:56
# Size of source mod 2**32: 15403 bytes
"""
...
class ArtellaShotsManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-shots',
    ...
    def register_shot_class(self, shot_class):
        ...
        if shot_class in self._registered_shot_classes:
            LOGGER.warning('Shot Class: "{}" is already registered. Skipping ...'.format(shot_class))
            return False
        else:
    ...
    def find_all_shots(self, force_update=False, force_login=True):
        ...
        if not artellapipe.Tracker().is_logged():
            LOGGER.warning('Impossible to find shots of current project because user is not log into production tracker')
            return
        else:
    ...
    def export_shot(self, shot_name, start_frame=101, new_version=False, comment=None):
        ...
        if not file_type:
            LOGGER.warning('Impossible to export shot "{}" because file type "{}" was not found!'.format(shot_name, shot_layout_file_type))
            return False
        ...
        if not valid_export:
            ...
            if new_version:
                ...
                valid_version = artellapipe.FilesMgr().upload_working_version(file_path=shot_file_path,
                  skip_saving=True,
                  notify=True,
                  comment=comment)
                if not valid_version:
            ...
            for shot_type, shot_type_info in self._config.get('types', default={}).items():
                ...
                if not full_shot_class:
                    ...
                    try:
                        ...
                        LOGGER.warning('Impossible to import Shot Module: {} | {} | {}'.format(shot_module, exc, traceback.format_exc()))
                        continue
