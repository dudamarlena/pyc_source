...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/playblasts.py
...
import os, logging, tempfile, tpDcc as tp
...
class PlayblastsManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-playblasts',
    ...
    def get_presets_paths(self):
        ...
        return [path_utils.clean_path(os.path.join(self._project.get_path(), p)) for p in playblasts_paths]
    ...
    def register_token(self, token, fn, label=''):
        ...
        if not (token.startswith('<') and token.endswith('>')):
            ...
            raise AssertionError
        self._registered_tokens[token] = {'fn':fn, 
         'label':label}
