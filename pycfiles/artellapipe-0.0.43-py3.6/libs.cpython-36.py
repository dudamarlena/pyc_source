...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/libs.py
# Compiled at: 2020-04-25 12:27:56
# Size of source mod 2**32: 8388 bytes
"""
...
import os, importlib, traceback, logging.config, tpDcc
...
class LibsManager(object):
    ...
    def register_lib(self, project, pkg_loader, do_reload=False):
        ...
        lib_config = self._create_lib_config(project_name=project_name,
          lib_path=lib_path,
          project=project,
        ...
        if not lib_id:
            artellapipe.logger.warning('Impossible to register library "{}" because its ID is not defined'.format(lib_path))
            return False
        if not lib_name:
            artellapipe.logger.warning('Impossible to register library "{}" because its ID is not defined'.format(lib_path))
            return False
            ...
            if lib_id in self._libs:
                artellapipe.logger.warning('Impossible to register library "{}" because its ID "{}" is already defined.'.format(lib_path, lib_id))
                return False
            else:
                version_found = None
                for sub_module in loader.walk_packages([pkg_loader.filename]):
                    ...
                    if skip_module:
                        continue
                    if not sub_module_name in skip_modules:
                        if sub_module_name:
                            pass
                        else:
                            ...
                            try:
                                ...
                                msg = 'Impossible to register Library: {}. Exception raised: {} | {}'.format(lib_path, exc, traceback.format_exc())
            ...
            self._libs[lib_id] = {'name':lib_name,  'config':lib_config, 
    ...
    def load_library(self, lib_path, do_reload=False, debug=False):
        ...
        if not lib_to_load or lib_to_load not in self._libs:
            ...
            logging.config.fileConfig((lib_config.data.get('logging_file', default_logging_config)),
    ...
    def _create_lib_config(self, project_name, lib_path, project, config_dict):
        lib_config = tpDcc.ConfigsMgr().get_config(config_name=(lib_path.replace('.', '-')),
