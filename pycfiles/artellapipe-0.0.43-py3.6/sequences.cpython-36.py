...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/sequences.py
# Compiled at: 2020-04-25 12:27:56
# Size of source mod 2**32: 11677 bytes
"""
...
class ArtellaSequencesManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-sequences',
    ...
    def register_sequence_class(self, sequence_class):
        ...
        if sequence_class in self._registered_sequence_classes:
            LOGGER.warning('Sequence Class: "{}" is already registered. Skipping ...'.format(sequence_class))
            return False
        else:
    ...
    def find_all_sequences(self, force_update=False, force_login=True):
        ...
        if not artellapipe.Tracker().is_logged():
            LOGGER.warning('Impossible to find sequences of current project because user is not log into production tracker')
            return
        else:
            ...
            for sequence_type, sequence_type_info in self._config.get('types', default={}).items():
                ...
                if not full_sequence_class:
                    ...
                    try:
                        ...
                        LOGGER.warning('Impossible to import Sequence Module: {} | {} | {}'.format(sequence_module, exc, traceback.format_exc()))
                        continue
