...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/thesaurus/thesaurus.py
# Compiled at: 2013-12-02 04:09:35
from persistent import Persistent
import BTrees.Length, re
...
from ztfy.thesaurus.interfaces.thesaurus import IThesaurus, IThesaurusExtractInfo, IThesaurusExtracts
...
CUSTOM_SEARCH = re.compile('\\*|\\"|\\sAND\\s|\\sOR\\s|\\sNOT\\s|\\(|\\)', re.IGNORECASE)
...
class Thesaurus(Persistent, Contained):
    ...
    def top_terms(self, value):
        self._top_terms = [ removeSecurityProxy(term) for term in value or () if term.usage is None
                          ]
        return
    ...
    def initCatalog(self):
        ...
        index = TextIndexNG('label', IThesaurusTerm, field_callable=False, languages=self.language, splitter='txng.splitters.default', storage='txng.storages.term_frequencies', dedicated_storage=False, use_stopwords=True, use_normalizer=True, ranking=True)
        locate(index, catalog, 'label')
        catalog['label'] = index
        index = TextIndexNG('label', IThesaurusTerm, field_callable=False, languages=self.language, splitter='txng.splitters.default', storage='txng.storages.term_frequencies', dedicated_storage=False, use_stopwords=True, use_normalizer=True, use_stemmer=True, ranking=True)
    ...
    def resetTopTerms(self):
        self.top_terms = [ term for term in self.terms.itervalues() if not term.generic and not term.usage
                         ]

    def findTerms(self, query=None, extract=None, autoexpand='always', glob='end', limit=None, exact=False, exact_only=False, stemmed=False):
        if not (exact or not exact_only):
            raise AssertionError
            query_util = getUtility(IQuery)
            terms = []
            if exact:
                search = ThesaurusValueSearch((self.catalog, 'base_label'), translateString(query, escapeSlashes=True, forceLower=True, spaces=' '))
                terms = list(query_util.searchResults(search))
            searches = exact_only or []
            if stemmed and not re.search('\\*', query):
                index = (
                 self.catalog, 'stemm_label')
                searches.append(ThesaurusIndexSearch(index, {'query': (' ').join(m for m in query.split() if len(m) > 2), 'autoexpand': 'off', 
                   'ranking': False}))
            start = ''
            end = ''
            if CUSTOM_SEARCH.search(query):
                ...
                query_text = (' ').join('%s%s%s' % (start, m, end) for m in query.split() if len(m) > 2)
            index = (
             self.catalog, 'label')
            searches.append(ThesaurusIndexSearch(index, {'query': query_text, 'autoexpand': autoexpand, 
               'ranking': False}))
            try:
