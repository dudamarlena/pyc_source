...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/email_filter/models.py
# Compiled at: 2014-07-02 07:00:11
...
class ERManager(models.Manager):
    ...
    def get_or_create_back_email(self, email):
        ...
        if return_email is None:
            er = self.create(email_in=email, email_redirect='username@mail.com')
...
def upload_to_date(instance, filename, prefix):
    ...
    return ('{prefix}/{month_number}/{filename}').format(prefix=prefix, filename=filename, month_number=month_number, instance=instance)


class EmailLog(models.Model):
    sender = models.EmailField(max_length=250)
    recipient = models.CharField(max_length=750)
    cc = models.CharField(max_length=500, default='', blank='')
    bcc = models.CharField(max_length=500, default='', blank='')
