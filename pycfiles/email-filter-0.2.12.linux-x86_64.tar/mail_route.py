...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/email_filter/mail_route.py
# Compiled at: 2014-07-04 06:46:17
import smtplib, sys, os, email, logging, traceback, cgitb, uuid, pytils
...
if __name__ == '__main__':
    try:
        from mail_route_settings import PROJECT_BASE_DIR, DJANGO_SETTINGS_MODULE, LOG_PATH, PROJECT_NAME
...
from email_filter.models import EmailLog, EmailRedirect, get_name_from_email, get_cleaned_email, is_project_recipient, is_local_email, EmailAttachment
...
def send_them(email_obj, msg, send_email):
    ...
    user_to = get_object_or_None(User, username=username_to)
    user_from = get_first_object_or_None(User, email=get_cleaned_email(email_obj.sender))
    if 'BCC' in msg:
        bcc = msg['BCC'].split(',')
    ...
    if user_to is not None and user_from is None:
        ...
        sender = EmailRedirect.objects.get_or_create_back_email(get_cleaned_email(email_obj.sender))
    if user_from is not None and user_to is None:
        recipient = EmailRedirect.objects.get_original_from_back_email(get_cleaned_email(email_obj.recipient))
        sender = '%s%s' % (
    ...
    if not recipient:
        ...
        mail_for_unknown_user.send(sender=EmailLog, instance=email_obj, original_msg=msg, mail_sender_email=sender, send_email=send_email, smtp_sendmail=smtp_server.sendmail)
        smtp_server.quit()
        return
    else:
...
def save_email_object(raw_email, send_email=False):
    ...
    email_obj = EmailLog.objects.create(sender=msg['From'], recipient=msg['To'], cc=msg['CC'] or '', bcc=msg['BCC'] or '', subject=msg['Subject'], raw_body=dest_document_body, message_id=msg['Message-ID'] or '', in_reply_to=msg['In-Reply-To'] or '')
    ...
    if ',' in email_obj.recipient:
        ...
        for recipient in recipients:
            ...
            msg = send_them(email_obj, msg, send_email) or msg
