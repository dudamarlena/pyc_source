...
# [GCC 8.4.0]
# Embedded file name: /home/tron/dev/irc-env/lib/python3.3/site-packages/irc3_pugbot/irc.py
# Compiled at: 2014-01-04 16:07:52
# Size of source mod 2**32: 6024 bytes
import functools, irc3
...
class PugBot:
    ...
    def __init__(self, bot):
        ...
        if self.pug_type == irc3_pugbot.pug.PugType.highlander:
            ...
            if self.pug_type == irc3_pugbot.pug.PugType.fours:
    ...
    def remove(self, mask, target, args):
        ...
        if self.staging_task:
            if not self.pug.can_stage():
    ...
    def pick(self, mask, target, args):
        ...
        if self.pug.staged_players is None:
            ...
            if mask.nick not in self.pug.captains:
                self.privmsg('{0}, only captains can pick'.format(mask.nick))
            else:
                if mask.nick != self.pug.captains[self.pug.picking_team]:
