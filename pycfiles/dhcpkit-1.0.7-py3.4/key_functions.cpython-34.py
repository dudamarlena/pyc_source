...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit/ipv6/server/extensions/rate_limit/key_functions.py
# Compiled at: 2016-09-14 11:24:40
# Size of source mod 2**32: 3479 bytes
"""
...
def duid_key(bundle: TransactionBundle) -> str:
    ...
    return 'duid:{}'.format(codecs.encode(duid.save(), 'hex').decode('ascii'))
...
def interface_id_key(bundle: TransactionBundle) -> str:
    ...
    if interface_id_option:
        return 'interface-id:{}'.format(codecs.encode(interface_id_option.interface_id, 'hex').decode('ascii'))
    else:
...
def remote_id_key(bundle: TransactionBundle) -> str:
    ...
    if remote_id_option:
        return 'remote-id:{}:{}'.format(remote_id_option.enterprise_number, codecs.encode(remote_id_option.remote_id, 'hex').decode('ascii'))
    else:
        return duid_key(bundle)
...
def subscriber_id_key(bundle: TransactionBundle) -> str:
    ...
    if subscriber_id_option:
        return 'subscriber-id:{}'.format(codecs.encode(subscriber_id_option.subscriber_id, 'hex').decode('ascii'))
    else:
        return duid_key(bundle)
...
def linklayer_id_key(bundle: TransactionBundle) -> str:
    ...
    if linklayer_id_option:
        return 'linklayer-id:{}:{}'.format(linklayer_id_option.link_layer_type, codecs.encode(linklayer_id_option.link_layer_address, 'hex').decode('ascii'))
    else:
        return duid_key(bundle)


key_function_map = {'duid': duid_key, 
