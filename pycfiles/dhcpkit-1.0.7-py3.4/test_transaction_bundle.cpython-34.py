...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit/tests/ipv6/server/test_transaction_bundle.py
...
class TransactionBundleTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.deep_bundle = TransactionBundle(RelayForwardMessage(hop_count=0, link_address=IPv6Address('2001:db8:ffff:2::1'), peer_address=IPv6Address('fe80::3631:c4ff:fe3c:b2f1'), options=[
         RelayMessageOption(relayed_message=relayed_solicit_message)]), received_over_multicast=False, marks=['some', 'marks'])
        self.ia_bundle = TransactionBundle(SolicitMessage(options=[
         ...
         IATAOption(b'0004'),
         IAPDOption(b'0005'),
         IAPDOption(b'0006')]), received_over_multicast=False)
        self.option_handlers = [
         InterfaceIdOptionHandler()]

    def test_str(self):
        bundle_str = str(self.bundle)
        self.assertEqual(bundle_str, 'SolicitMessage from 000300013431c43cb2f1 at fe80::3631:c4ff:fe3c:b2f1 via LDRA -> 2001:db8:ffff:1::1')
        ...
        self.assertRegex(bundle_str, '^SolicitMessage from 000300013431c43cb2f1 at fe80::3631:c4ff:fe3c:b2f1 via LDRA -> 2001:db8:ffff:1::1 -> 2001:db8:ffff:2::1 with marks .*$')
