...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit/tests/ipv6/extensions/test_prefix_delegation.py
# Compiled at: 2017-06-23 17:22:45
# Size of source mod 2**32: 6441 bytes
"""
...
class IAPDOptionTestCase(test_option.OptionTestCase):
    ...
    def setUp(self):
        self.option_bytes = bytes.fromhex('0019004541424344000000290000002a001a001900000000000000003020010db8000000000000000000000000000d0018000045766572797468696e6720697320617765736f6d6521')
...
class IAPrefixOptionTestCase(test_option.OptionTestCase):
    ...
    def setUp(self):
        self.option_bytes = bytes.fromhex('001a00360001518000093a803020010db8000100000000000000000000000d0019000457686572652064696420796f752067657420746861743f')
        self.option_object = IAPrefixOption(preferred_lifetime=86400, valid_lifetime=604800, prefix=IPv6Network('2001:db8:1::/48'), options=[
    ...
    def test_bad_option_length(self):
        ...
        with self.assertRaisesRegex(ValueError, 'length does not match'):
            IAPrefixOption.parse(bytes.fromhex('001a00350001518000093a803020010db8000100000000000000000000000d0019000457686572652064696420796f752067657420746861743f'))
