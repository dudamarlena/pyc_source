...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/dhcpkit/ipv6/server/statistics.py
# Compiled at: 2016-11-16 16:30:43
# Size of source mod 2**32: 14805 bytes
"""
...
class Statistics:
    ...
    def __init__(self):
        ...
        for key in message_registry_keys:
            ...
            if message_class.from_client_to_server:
                if issubclass(message_class, ClientServerMessage):
...
class ServerStatistics:
    ...
    def get_update_set(self, interface_name: str=None,
                       bundle: TransactionBundle=None) -> StatisticsSet:
        """
        ...
        stats_set = [
         self.global_stats]
        if interface_name:
            if interface_name in self.interface_stats:
    ...
    def __str__(self):
        lines = [
         'Global']
        lines += [('- ' if not line.startswith('- ') else '  ') + line for line in str(self.global_stats).split('\n')]
        ...
        def get_category_lines(type_name: str, category_data: Dict[(Hashable, Statistics)]) -> List[str]:
            ...
            for name in keys:
                ...
                sub_lines += [('- ' if not line.startswith('- ') else '  ') + line for line in str(category_data[name]).split('\n')]
