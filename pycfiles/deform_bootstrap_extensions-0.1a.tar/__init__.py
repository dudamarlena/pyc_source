...
# [GCC 8.4.0]
# Embedded file name: /home/gas/autonomie/deform_bootstrap_extensions/deform_bootstrap_extensions/__init__.py
# Compiled at: 2015-02-05 10:12:49
"""
Provide custom colander schemas and associated widgets to render forms
differently
"""
import cgi, json, warnings, colander, deform, random
...
TEMPLATES_PATH = 'deform_bootstrap_extensions:templates/'
...
class VoidWidget(object):
    ...
    def render_template(self, template):
        ...
        """
        return ("<div class='span{0}'><br /></div>").format(self.width)
...
class LocalizationWidget(deform.widget.TextInputWidget):
    ...
    """
    template = TEMPLATES_PATH + 'localization'
    readonly_template = TEMPLATES_PATH + 'localization'
...
class HiddenLocalizationWidget(deform.widget.TextInputWidget):
    ...
    A hidden version
    """
    template = TEMPLATES_PATH + 'hidden_localization'
    readonly_template = TEMPLATES_PATH + 'hidden_localization'
...
class CustomDateInputWidget(deform.widget.Widget):
    ...
    **Attributes/Arguments**
    ...
    def serialize(self, field, cstruct, readonly=False):
        ...
        return field.renderer(template, field=field, cstruct=cstruct, options=self.options)
...
class CustomDateTimeInputWidget(CustomDateInputWidget):
    ...
    **Attributes/Arguments**
    ...
    readonly_template
        ...
        Default: ``readonly/textinput``.
    """
    template = TEMPLATES_PATH + 'datetimeinput'
    readonly_template = 'readonly/textinput'
    type_name = 'datetime'
    size = None
    style = None
    ...
    def serialize(self, field, cstruct, readonly=False):
        ...
        template = readonly and self.readonly_template or self.template
        return field.renderer(template, field=field, cstruct=cstruct, options=options)
...
class CustomSequenceWidget(deform.widget.SequenceWidget):
    ...
    def prototype(self, field):
        ...
        proto = field.renderer(self.item_template, field=item_field, cstruct=item_field.schema.serialize(colander.null), parent=field)
    ...
    def serialize(self, field, cstruct, readonly=False):
        ...
        add_template_mapping = dict(subitem_title=translate(item_field.title), subitem_description=translate(item_field.description), subitem_name=item_field.name)
        if isinstance(self.add_subitem_text_template, TranslationString):
            add_subitem_text = self.add_subitem_text_template % add_template_mapping
        else:
            add_subitem_text = _(self.add_subitem_text_template, mapping=add_template_mapping)
        return field.renderer(template, field=field, cstruct=cstruct, subfields=subfields, item_field=item_field, add_subitem_text=add_subitem_text)
