...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/site-packages/tawhiri/models.py
...
def make_drag_descent(sea_level_descent_rate):
    ...
    def drag_descent(t, lat, lng, alt):
        return (
         0.0, 0.0, -drag_coefficient / math.sqrt(density(alt)))
...
def standard_profile(ascent_rate, burst_altitude, descent_rate, wind_dataset, elevation_dataset):
    ...
    return (
     (
      model_up, term_up), (model_down, term_down))
...
def float_profile(ascent_rate, float_altitude, stop_time, dataset):
    ...
    return (
     (
      model_up, term_up), (model_float, term_float))