...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/help.py
# Compiled at: 2014-04-26 11:09:41
"""Help Messages
...
def format_msg(msg):
    return msg.strip().replace('\t\t', '\t').replace('\t', '   ').split('\n')
...
class Commands(Component):
    ...
    def commands(self, source, target, args):
        ...
        if not args:
            return ('All available commands: {0:s}').format((' ').join(chain(*commands.values())))
        q = first(args.split(' ', 1))
        if q not in plugins:
            return ('No commands for {0:s} or {0:s} is not loaded').format(q)
        return ('Available commands for {0:s}: {1:s}').format(q, (' ').join(commands[q]))
    ...
    def help(self, source, target, args):
        ...
        if q in plugins:
            ...
            msg = None
        if msg is None:
            msg = ('No help available for: {0:s}. To get a list of plugins, type: plugins').format(q)
    ...
    def info(self, source, target, args):
        ...
        if name in plugins:
            ...
            msg = ('{0:s} - {1:s} v{2:s} by {3:s}').format(name, description, version, author)
        else:
            msg = ('No info available for: {0:s}. To get a list of plugins, type: plugins').format(name)
        return format_msg(msg)
