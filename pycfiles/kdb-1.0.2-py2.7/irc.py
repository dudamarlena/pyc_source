...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/irc.py
...
class Commands(Component):
    ...
    def ircinfo(self, source, target, args):
        ...
        msg = ('I am {0:s} on the {1:s} IRC Network. Connected to {2:s} Running version {3:s}').format(('{0:s}!{1:s}@{2:s}').format(state('nick'), state('ident'), state('host')), state('network', 'Unknown'), state('server_host', 'Unknown'), state('server_version', 'Unknown'))
        return msg
...
class IRC(BasePlugin):
    ...
    def _on_numeric(self, source, target, numeric, arg, message):
        if numeric == '1':
            m = search('Welcome to the ([a-zA-Z0-9]*) Internet Relay Chat Network', message)
            self.data.state['network'] = m is not None and m.group(1) or 'Unknown'
        elif numeric == 2:
            m = search('Your host is ([a-zA-Z.-]*)', message)
            self.data.state['server_host'] = m is not None and m.group(1) or self.bot.host
            m = search('running version (.*)', message)
            self.data.state['server_version'] = m is not None and m.group(1) or 'unknown'
        elif numeric == 433:
