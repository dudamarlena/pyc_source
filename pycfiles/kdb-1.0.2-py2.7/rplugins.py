...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/rplugins.py
...
class RPluginsCommands(Component):
    ...
    def add(self, source, target, args):
        ...
        if url in data['enabled']:
            ...
            value = yield self.call(task(verify_plugin, url, config['path'], data['allowed']), 'workerprocesses')
            allowed, plugin = value.value
            if allowed:
                msg = log('Remote Plugin {0:s} ({1:s}) is already authorized.', url, plugin)
                yield msg
            else:
                data['pending'][plugin] = url
                msg = log('Remote Plugin {0:s} ({1:s}) pending authorization.', url, plugin)
                yield msg
    ...
    def auth(self, source, target, args):
        ...
        if plugin in data['pending']:
            ...
            allowed, plugin = value.value
            if allowed:
                msg = log('Remote Plugin {0:s} ({1:s}) successfully authorized.', url, plugin)
                yield msg
            else:
                del data['allowed'][plugin]
                msg = log('Remote Plugin {0:s} ({1:s}) failed authorization.', url, plugin)
                yield msg
        else:
...
class RPlugins(BasePlugin):
    ...
    def init(self, *args, **kwargs):
        ...
        self.data.init({'rplugins': {'allowed': {}, 'pending': {}, 'enabled': {}}})
