...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/bot.py
...
from circuits.protocols.irc import IRC, NICK, NOTICE, PASS, PRIVMSG, USER
...
class Bot(BaseComponent):
    ...
    def init(self, data, config):
        ...
        self.plugins = cidict()
        self.data.init({'state': {'host': self.auth['host'], 
    ...
    def _on_message_or_notice(self, event, source, target, message):
        addressed, target, message = self.is_addressed(source, target, message)
            ...
            if command not in self.command:
                ...
                try:
                    ...
                    if value.errors:
                        ...
                        msg = log('ERROR: {0:s}: ({1:s})', evalue, repr(message))
                        log(format_exc())
                        self.fire(Reply(target, msg))
                    else:
