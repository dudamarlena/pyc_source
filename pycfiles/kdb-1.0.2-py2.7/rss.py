...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/rss.py
...
class Feed(object):
    ...
    def __call__(self):
        ...
        new = []
        for v in d.entries:
            e = {'time': mktime(v.updated_parsed), 'title': v.title, 
        ...
        if not new == []:
            s = []
            s.append(('\x02RSS:\x02 {0:s} ({1:s})').format(self.title, self.link))
            for e in new[:3]:
                x = sum([len(e['title']), len(e['summary']), len(e['link'])])
                if x > 450:
                    y = sum([len(e['title']), len(e['link'])])
                    s.append((' * {0:s}: {1:s} ... <{2:s}>').format(e['title'], e['summary'][:450 - y], e['link']))
                else:
                    s.append((' * {0:s}: {1:s} <{2:s}>').format(e['title'], e['summary'], e['link']))
...
class Commands(Component):
    ...
    def radd(self, source, target, args):
        ...
        if target in feeds:
            ...
            feeds[target] = [
             feed]
        value = yield self.call(task(check_feed, feed), 'workerprocesses')
        yield value.value
    ...
    def rlist(self, source, target, args):
        ...
        if target in feeds:
            msg = [
             ('RSS Feeds ({0:s}):').format(target)]
            for i, feed in enumerate(feeds[target]):
                msg.append((' {0:d}. {1:s} ({2:s}) / {3:d}mins (Next Update in {4:d}mins)').format(i + 1, feed.title, feed.url, feed.interval, int((feed.next - time()) / 60)))
...
class RSS(BasePlugin):
    ...
    def init(self, *args, **kwargs):
        ...
        if filename is not None:
            self.data.init({'feeds': self.load(filename)})
        else:
            self.data.init({'feeds': {}})
