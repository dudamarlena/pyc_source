...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/weather.py
...
class Commands(Component):
    ...
    def weather(self, source, target, args):
        ...
        try:
            ...
            msg = log('ERROR: Could not get weather report for {0:s}: {1:s}', station, e)
            return msg

        rp = ReportParser()
        pr = rp.ParseReport(rep)
        msgs = []
        msgs.append(('Weather report for {0:s} ({1:s}) as of {2:s}').format(pr.getStationName(), station, pr.getISOTime()))
        msgs.append(('Weather: {0:s}').format(pr.getWeather()))
        msgs.append(('Sky: {0:s}').format(pr.getSkyConditions()))
        msgs.append(('Temperature: {0:0.2f} C / {1:0.2f} F').format(pr.getTemperatureCelsius(), pr.getTemperatureFahrenheit()))
        msgs.append(('Humidity: {0:d}%').format(pr.getHumidity()))
        if pr.getWindSpeed() is not None:
            msgs.append(('Wind: {0:0.2f} m/s [{1:d} deg ({2:s})]').format(pr.getWindSpeed(), pr.getWindDirection(), pr.getWindCompass()))
        else:
        ...
        msgs.append(('Dew: {0:f} C / {1:f} F').format(pr.getDewPointCelsius(), pr.getDewPointFahrenheit()))
        return msgs
