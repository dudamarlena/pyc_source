...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kdb/plugins/stats.py
...
class Commands(Component):
    ...
    def uptime(self, source, target, args):
        ...
        msg = ('Uptime: {0:d}+{1:d}:{2:d}:{3:d} (CPU: {4:0.2f}s {5:0.2f}%)').format(*(uptime + (cpu, rate)))
        return msg
    ...
    def cstats(self, source, target, args):
        ...
        top5_commands = [ ('{0:s}:{1:d}').format(*x) for x in commands.most_common(5)
                        ]
        moving_average, _ = buildAverage(time, total_commands)
        msg = ('Command Stats: {0:s} Total: {1:d} Top 5: {2:s}').format(moving_average, total_commands, (' ').join(top5_commands))
        return msg
    ...
    def nstats(self, source, target, args):
        ...
        msg = ('Traffic: {0:s} / {1:s} ({2:s})').format(('{0:0.2f}{1:s}').format(*bytes(bytesin)), ('{0:0.2f}{1:s}').format(*bytes(bytesout)), ('{0:0.2f}{1:s}').format(*bytes(bytestotal)))
        return msg
    ...
    def version(self, source, target, args):
        ...
        msg = ('{0:s} [ {1:s} ] v{2:s} by {3:s} - {4:s}').format(me, kdb.__description__, kdb.__version__, kdb.__author__, kdb.__copyright__)
        return msg
    ...
    def mstats(self, source, target, args):
        ...
        msg = ('Memory: {0:s} {1:s} {2:s}').format(('{0:0.2f}{1:s}').format(*bytes(m.size)), ('{0:0.2f}{1:s}').format(*bytes(m.rss)), ('{0:0.2f}{1:s}').format(*bytes(m.stack)))
        return msg
...
class Stats(BasePlugin):
    ...
    def init(self, *args, **kwargs):
        ...
        self.data.init({'commands': Counter(), 
           'bytes': (0, 0), 
           'time': time(), 
           'events': 0, 
           'errors': 0})
