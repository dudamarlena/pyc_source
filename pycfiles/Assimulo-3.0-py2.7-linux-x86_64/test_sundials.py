...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/assimulo/tests/solvers/test_sundials.py
...
class Test_CVode():
    ...
    def test_get_used_initial_step(self):
        ...
        self.simulator.inith = 1e-08
        self.simulator.simulate(1.0)
        step = self.simulator.get_used_initial_step()
        assert N.abs(step - 1e-08) < 0.01
    ...
    def test_usejac(self):
        ...
        jac = lambda t, x: N.array([[0.0, 1.0], [0.0, 0.0]])
    ...
    def test_ncp_list(self):
        ...
        sim = CVode(prob)
        t, y = sim.simulate(7, ncp_list=N.arange(0, 7, 0.1))
    ...
    def test_spgmr(self):
        ...
        jacv = lambda t, y, fy, v: N.dot(N.array([[0, 1.0], [0, 0]]), v)
        jacvsw = lambda t, y, fy, v, sw: N.dot(N.array([[0, 1.0], [0, 0]]), v)
        jacvp = lambda t, y, fy, v, p: N.dot(N.array([[0, 1.0], [0, 0]]), v)
        jacvswp = lambda t, y, fy, v, sw, p: N.dot(N.array([[0, 1.0], [0, 0]]), v)
        y0 = [1.0, 0.0]
    ...
    def test_terminate_simulation(self):
        ...
        simulator(3.0)
        nose.tools.assert_almost_equal(simulator.t, 2.0, 4)
...
class Test_IDA():
    ...
    def test_time_event(self):
        ...
        sim = IDA(mod)
        sim.simulate(5.0)
        nose.tools.assert_almost_equal(sim.y_sol[38], 1.0, 5)
        nose.tools.assert_almost_equal(sim.y_sol[87], 1.0, 5)
        sim = IDA(mod, [1.0], [1.0])
        sim.simulate(2.0)
        nose.tools.assert_almost_equal(sim.t_sol[(-1)], 2.0, 5)
    ...
    def test_usejac(self):
        ...
        jac = lambda c, t, x, xd: N.array([[c, -1.0], [0.0, c]])
        imp_mod = Implicit_Problem(f, [1.0, 0.0], [0.0, -9.82])
        imp_mod.jac = jac
        imp_sim = IDA(imp_mod)
        imp_sim.simulate(3, 100)
        assert imp_sim.statistics['nfcnjacs'] == 0
        nose.tools.assert_almost_equal(imp_sim.y_sol[(-1)][0], 45.19, 4)
        imp_sim.reset()
        imp_sim.usejac = False
        imp_sim.simulate(3.0, 100)
        nose.tools.assert_almost_equal(imp_sim.y_sol[(-1)][0], 45.19, 4)
        assert imp_sim.statistics['nfcnjacs'] > 0
    ...
    def test_terminate_simulation(self):
        ...
        sim = IDA(prob)
        sim.simulate(2.5)
        nose.tools.assert_almost_equal(sim.t, 2.0, 4)
    ...
    def test_make_consistency(self):
        ...
        nose.tools.assert_almost_equal(y[1], 0.0)
        nose.tools.assert_almost_equal(y[0], -1.0)
        nose.tools.assert_almost_equal(yd[0], 1.0)
        nose.tools.assert_almost_equal(yd[1], 0.0)
    ...
    def test_switches(self):
        ...
        state_events = lambda t, x, xd, sw: N.array([x[0] - 1.0])
...
class Test_Sundials():
    ...
    def test_atol(self):
        ...
        """
        assert self.simulators[1].atol == 1e-06
        assert self.simulators[0].atol == 1e-06
        for i in range(len(self.simulators)):
            ...
            self.simulators[i].atol = 1e-05
            assert self.simulators[i].atol == 1e-05
            ...
            self.simulators[i].atol = [N.array([1e-05])]
    ...
    def test_rtol(self):
        ...
        for i in range(len(self.simulators)):
            ...
            self.simulators[i].rtol = 1e-05
            assert self.simulators[i].rtol == 1e-05
    ...
    def test_maxh(self):
        ...
        for i in range(len(self.simulators)):
            ...
            self.simulators[i].maxh = 1e-05
            assert self.simulators[i].maxh == 1e-05
    ...
    def test_pbar(self):
        ...
        exp_sim = CVode(exp_mod)
        nose.tools.assert_almost_equal(exp_sim.pbar[0], 1000.0, 4)
        nose.tools.assert_almost_equal(exp_sim.pbar[1], 100.0, 4)
        f = lambda t, y, yd, p: N.array([0.0] * len(y))
        yd0 = [0.0] * 2
        imp_mod = Implicit_Problem(f, y0, yd0, p0=p0)
        imp_sim = IDA(imp_mod)
        nose.tools.assert_almost_equal(imp_sim.pbar[0], 1000.0, 4)
        nose.tools.assert_almost_equal(imp_sim.pbar[1], 100.0, 4)