...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/assimulo/solvers/runge_kutta.py
...
class RungeKutta34(Explicit_ODE):
            ...
            if t + h < tf and flag == ID_PY_OK:
                ...
                if opts['report_continuously']:
                    ...
                    try:
                        while output_list[output_index] <= t:
                            yield (
                             flag, output_list[output_index], self.interpolate(output_list[output_index]))
    ...
    def _step(self, t, y, h):
        ...
        scaling = N.array(abs(y) * self.rtol + self.atol)
        f = self.f
        f(self.Y2, t + h / 2.0, y + h * self.Y1 / 2.0)
        f(self.Y3, t + h / 2.0, y + h * self.Y2 / 2.0)
        f(self.Z3, t + h, y - h * self.Y1 + 2.0 * h * self.Y2)
        f(self.Y4, t + h, y + h * self.Y3)
        error = N.linalg.norm(h / 6.0 * (2.0 * self.Y2 + self.Z3 - 2.0 * self.Y3 - self.Y4) / scaling)
        t_next = t + h
        ...
        def interpolate(time):
            ...
            f_high = self.Y1
            return (1 - thetha) * y + thetha * y_next + thetha * (thetha - 1) * ((1 - 2 * thetha) * (y_next - y) + (thetha - 1) * h * f_low + thetha * h * f_high)
...
class RungeKutta4(Explicit_ODE):
    ...
    def _step(self, t, y, h):
        ...
        f(self.Y4, t + h, y + h * self.Y3)
        return (
         t + h, y + h / 6.0 * (self.Y1 + 2.0 * self.Y2 + 2.0 * self.Y3 + self.Y4))
