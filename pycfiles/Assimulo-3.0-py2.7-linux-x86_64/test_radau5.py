...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/assimulo/tests/solvers/test_radau5.py
...
class Test_Explicit_Radau5():
    ...
    def test_collocation_polynomial(self):
        ...
        self.sim.simulate(2.0, 200)
        assert self.sim.statistics['nsteps'] < 300
        nose.tools.assert_almost_equal(self.sim.y_sol[(-1)][0], 1.706168035, 4)
        self.sim.report_continuously = True
        self.sim.reset()
        self.sim.simulate(2.0, 200)
        assert self.sim.statistics['nsteps'] < 300
        nose.tools.assert_almost_equal(self.sim.y_sol[(-1)][0], 1.706168035, 4)
        self.sim_t0.simulate(3.0)
        nose.tools.assert_almost_equal(self.sim_t0.t_sol[0], 1.0, 4)
        nose.tools.assert_almost_equal(self.sim_t0.t_sol[(-1)], 3.0, 4)
        nose.tools.assert_almost_equal(self.sim_t0.y_sol[(-1)][0], 1.706168035, 4)
    ...
    def test_atol(self):
        ...
        assert steps2 > steps
        self.sim.reset()
        self.sim.atol = [1e-08, 1e-08]
        steps3 = self.sim.statistics['nsteps']
        assert steps3 == steps2
        nose.tools.assert_raises(Radau_Exception, self.sim._set_atol, [1e-06, 1e-06, 1e-06])
