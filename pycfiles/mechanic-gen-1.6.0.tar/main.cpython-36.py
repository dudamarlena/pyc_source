...
# [GCC 8.4.0]
# Embedded file name: /home/zackschrag/fctn/mechanic/mechanic/main.py
# Compiled at: 2018-03-28 15:27:28
...
"""
import os, pkg_resources, datetime, json, copy, re, glob, yaml, yamlordereddictloader
from yamlordereddictloader import OrderedDict
from docopt import docopt
from mechanic.src.merger import Merger, SpecMerger
REMOVE_KEYS = [
 'x-mechanic-controller', 'x-mechanic-db', 'x-mechanic-schema', 'x-mechanic-tags',
 'x-mechanic-public',
...
OPENAPI_TO_MARSHMALLOW_TYPE = {'integer':'Integer', 
 'number':'Number', 
 'string':'String', 
 'boolean':'Boolean', 
 'array':'List', 
...
def _convert_authorization_to_swagger_format(oapi_authorizations):
    ...
    for auth_name, auth_item in oapi_authorizations.items():
        auth[auth_name] = {'name':'Authorization',  'in':'header', 
         'required':True, 
         'type':'string'}
...
def main():
    ...
    if args['<openapi_file>'].endswith('.yaml') or args['<openapi_file>'].endswith('.yml') or args['<openapi_file>'].endswith('.json'):
        exclude_tag_set, filter_schema_name, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version = parse_oapi(args)
    else:
    ...
    if args['clean']:
        ...
        with open(args['<output_file>'], 'w') as (f):
            yaml_data = yaml.dump((OrderedDict(clean_oapi)), Dumper=(yamlordereddictloader.Dumper),
              default_flow_style=False)
            f.write(yaml_data)
    else:
        if args['scaffold']:
            models_context = {'timestamp':datetime.datetime.utcnow(),  'codeblocks':[],  'openapi_to_marshmallow':OPENAPI_TO_MARSHMALLOW_TYPE}
            ...
            for uri, path in oapi_obj.get('paths', {}).items():
                ...
                namespace_contexts[path_ns] = {'timestamp':datetime.datetime.utcnow(), 
                 'codeblocks':[],  'project_name':project_name, 
                 'imported':[],  'namespace':path_ns, 
                 'description':path.get('description', '')}

            exclude_templates = [
             MODELS_TPL,
             NAMESPACE_TPL]
            _create_directory_structure({'project_name':project_name, 
             'namespace':list(namespaces), 
             'version':oapi_version},
              dir_name,
              excluded_tpl=exclude_templates, tpl_dir=tpl)
            ...
            for _, c in namespace_contexts.items():
                imported = generate_namespaces(c, exclude_tag_set,
                  filter_tag_set,
                  filter_tags,
                  merger,
                  oapi_obj,
            ...
            context = {'timestamp':datetime.datetime.utcnow(),  'codeblocks':[],  'openapi_to_marshmallow':OPENAPI_TO_MARSHMALLOW_TYPE}
            ...
            if args['--models'] or args['--schemas']:
                if args['--namespaces']:
                    template = 'templates/code.tpl'
            if args['--tests']:
                generate_tests(context, exclude_tag_set, filter_schema_name, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version)
    ...
    if args['--output-docs']:
        ...
        with open(output_docs, 'w') as (f):
            if output_docs.endswith('.json'):
                ...
                if output_docs.endswith('.yaml') or output_docs.endswith('.yml'):
                    yaml_data = yaml.dump((OrderedDict(clean_oapi)), Dumper=(yamlordereddictloader.Dumper),
                      default_flow_style=False)
                    f.write(yaml_data)
                else:
...
def generate_tests(context, exclude_tag_set, filter_schema_name, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version):
    for model_name, model in oapi_obj['components']['schemas'].items():
        ...
        if model_name in filter_schema_name:
            context['codeblocks'].append({'type':'tests', 
...
def generate_models(context, exclude_tag_set, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version):
    for table_name, table_def in oapi_obj['components'].get('x-mechanic-db-tables', {}).items():
        ...
        if not exclude_tag_set.intersection(s2) and filter_tag_set <= s2 or len(filter_tags) == 0:
            context['codeblocks'].append({'type':'table', 
    ...
    for model_name, model in oapi_obj['components']['schemas'].items():
        ...
        s2 = set(model.get('x-mechanic-tags', []))
        if (not exclude_tag_set.intersection(s2) and filter_tag_set <= s2 or len(filter_tags) == 0) and model.get('x-mechanic-model'):
            context['codeblocks'].append({'type':'model', 
...
def generate_namespaces(context, exclude_tag_set, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version, project_name=None, filter_ns=[]):
    ...
    for path_name, path in paths_copy.items():
        if filter_ns:
            if path.get('x-mechanic-namespace', project_name) not in filter_ns:
                continue
            for method_name, method in path.items():
                if method_name in MECHANIC_SUPPORTED_HTTP_METHODS:
                    if not oapi_obj['paths'][path_name].get('x-mechanic-controller'):
                    ...
                    elif not oapi_obj['paths'][path_name]['x-mechanic-controller'].get('methods'):
                        ...
                        if not path_security_name:
                            if path_security_name != []:
                    ...
                    for response_code, response_obj in method.get('responses', {}).items():
                        if response_code.startswith('2'):
                            ...
                            resp_schema = response_obj.get('content', {}).get('application/json', {}).get('schema', {})
                            if resp_schema.get('items'):
                    ...
                    if method.get('requestBody'):
                        req_schema = method.get('requestBody').get('content', {}).get('application/json', {}).get('schema', {})
            ...
            if not exclude_tag_set.intersection(s2) and filter_tag_set <= s2 or len(filter_tags) == 0:
                VAR_PATTERN = '{([\\w_-]*)}'
                context['codeblocks'].append({'type':'namespace', 
                 ...
                 'route':re.sub(VAR_PATTERN, '<string:\\1>', path_name).replace('-', '_'), 
...
def generate_schemas(context, exclude_tag_set, filter_tag_set, filter_tags, merger, oapi_obj, oapi_version):
    for model_name, model in oapi_obj['components']['schemas'].items():
        ...
        s2 = set(model.get('x-mechanic-tags', []))
        if (not exclude_tag_set.intersection(s2) and filter_tag_set <= s2 or len(filter_tags) == 0) and model.get('x-mechanic-schema', {}).get('generate', True):
            context['codeblocks'].append({'type':'schema', 
