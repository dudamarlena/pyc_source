...
# [GCC 8.4.0]
# Embedded file name: /home/zackschrag/fctn/mechanic/mechanic/exceptions.py
...
class MechanicDeleteResourceDependenceException(MechanicException):
    message = 'The resource cannot be deleted because it is referenced by one or more resource.'
    resolution = 'Remove the references and retry the operation'
    status_code = 409

    def __init__(self, msg=message, res=resolution):
        super(MechanicDeleteResourceDependenceException, self).__init__(self.message)
        self.message = msg
        self.resolution = res
...
class MechanicDeveloperError(MechanicException):
    status_code = 500
