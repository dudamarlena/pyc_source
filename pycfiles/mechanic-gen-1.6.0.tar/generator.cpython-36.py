...
# [GCC 8.4.0]
# Embedded file name: /home/zackschrag/fctn/mechanic/mechanic/src/generator.py
# Compiled at: 2018-01-24 12:27:26
# Size of source mod 2**32: 17489 bytes
import os, datetime, pkg_resources, shutil, json, datetime as dt, errno, yaml, jinja2
...
class Generator(object):
    ...
    def generate(self):
        ...
        for namespace, obj in self.mech_obj['namespaces'].items():
            model_filename = utils.replace_template_var(models_path, namespace=namespace,
              version=(self.mech_obj['version']))
            schema_filename = utils.replace_template_var(schemas_path, namespace=namespace,
              version=(self.mech_obj['version']))
            controller_filename = utils.replace_template_var(controllers_path, namespace=namespace,
              version=(self.mech_obj['version']))
    ...
    def build_models_file(self, filename, namespace):
        ...
        if len(namespaced_models):
            models_result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'models.tpl')), context={'timestamp':dt.datetime.utcnow(), 
    ...
    def build_schemas_file(self, filename, namespace):
        ...
        if len(namespaced_schemas):
            schemas_result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'schemas.tpl')), context={'timestamp':dt.datetime.utcnow(), 
             'app_name':self.options[reader.APP_NAME_KEY], 
    ...
    def build_controllers_file(self, filename, namespace):
        ...
        if len(namespaced_controllers):
            result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'controllers.tpl')), context={'timestamp':dt.datetime.utcnow(), 
             'app_name':self.options[reader.APP_NAME_KEY], 
    ...
    def build_init_app_file(self):
        ...
        result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'init_app.tpl')), context={'timestamp':dt.datetime.utcnow(), 
         'app_name':self.options[reader.APP_NAME_KEY], 
    ...
    def build_init_api_file(self):
        ...
        for controller_name, controller in self.mech_obj['controllers'].items():
            ...
            api_controllers.append({'uri':controller['uri'], 
        ...
        result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'init_api.tpl')), context={'timestamp':dt.datetime.utcnow(), 
         'app_name':self.options[reader.APP_NAME_KEY], 
         'dependent_controllers':dependent_controllers, 
    ...
    def _add_run_file(self):
        self._replace_app_name_in_file(pkg_resources.resource_filename(__name__, '../mechanic/run.py'), self.directory + '/run.py')

    def _add_requirements_txt(self):
        shutil.copy(pkg_resources.resource_filename(__name__, '../mechanic/requirements.txt'), self.directory + '/requirements.txt')
    ...
    def _add_mechanic_base_package(self):
        ...
        self._replace_app_name_in_file(pkg_resources.resource_filename(__name__, '../mechanic/base/schemas.py'), self.directory + '/mechanic/base/schemas.py')
        self._replace_app_name_in_file(pkg_resources.resource_filename(__name__, '../mechanic/base/models.py'), self.directory + '/mechanic/base/models.py')
        self._replace_app_name_in_file(pkg_resources.resource_filename(__name__, '../mechanic/base/controllers.py'), self.directory + '/mechanic/base/controllers.py')
        self._replace_app_name_in_file(pkg_resources.resource_filename(__name__, '../mechanic/utils/db_helper.py'), self.directory + '/mechanic/utils/db_helper.py')
    ...
    def _add_package_files(self, path, imports=None):
        ...
        for dir in dir_path:
            if not dir.endswith('.py'):
                ...
                if imports:
                    import_result = self._render((pkg_resources.resource_filename(__name__, self.TEMPLATE_DIR + 'init_models.tpl')),
                      context={'module_paths': imports})
                ...
                with open(self.directory + '/' + dir + '/__init__.py', 'a') as (f):
                    if import_result:
                        if import_result not in current_contents:
                            f.write(import_result)

    def _replace_app_name_in_file(self, src_file, output_file):
        result = self._render(src_file, context={'timestamp':dt.datetime.utcnow(), 
         'app_name':self.options[reader.APP_NAME_KEY]})
