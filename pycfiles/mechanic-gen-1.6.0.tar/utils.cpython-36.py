...
# [GCC 8.4.0]
# Embedded file name: /home/zackschrag/fctn/mechanic/mechanic/utils.py
# Compiled at: 2018-04-09 15:56:30
# Size of source mod 2**32: 14623 bytes
import pkg_resources, re, falcon, flask, werkzeug.exceptions, mechanic.exceptions
        ...
        def wrapper(*args, **kwargs):
            ...
            if response:
                ...
                if isinstance(resp, tuple):
                    if len(resp) != 2:
                        error_msg = 'The return value from endpoints must either be a single item with the response body, or a tuple of format (<response_body>, <response_headers>). Failing method: %s' % str(f)
...
class MechanicAPISpec(APISpec):
    ...
    def to_dict(self):
        ...
        if self.openapi_version.version[0] == 2:
            ...
            if self.openapi_version.version[0] == 3:
...
class MechanicAPI(object):
    ...
    def __init__(self, title='Mechanic Project', version='1.0.0', security_schemes=None, docs_path='/', docs_theme='swagger-ui', **kwargs):
        """
        :param title:
        :param version:
        ...
        :param kwargs:
        """
        self.spec = MechanicAPISpec(title=title, 
         version=version, 
         plugins=[
 'apispec.ext.marshmallow'], 
         openapi_version='3.0.0', **kwargs)
        self.api = falcon.API()
    ...
    def _check_parameter(req, resp, resource, params, param_name=None, param_in=None, param_required=False):
        if param_in == 'query':
            if param_required:
                if param_name not in req.params.keys():
                    raise APIContractException(f"Missing query parameter {param_name}")
        if param_in == 'header':
            if param_required:
                if not req.get_header(param_name):
                ...
                for hm in falcon.COMBINED_METHODS:
                    if m == f"on_{hm.lower()}":
                        ...
                        for match in matches:
                            ...
                            self.method_map[op_method]['parameters'][match] = {'name':match, 
                             'in':'path', 
                             'required':True}
            ...
            if security == []:
                ...
                for s in security:
                    for scheme_name, scheme_val in s.items():
                        ...
                        if scheme_obj['type'] == 'http':
                            if scheme_obj.get('scheme') == 'bearer':
                                f = falcon.before((self._check_parameter), param_name='Authorization',
                                  param_in='header',
                                  param_required=True)(f)
                        if scheme_obj['type'] == 'http' and scheme_obj.get('scheme') == 'basic':
                            f = falcon.before((self._check_parameter), param_name='Authorization',
                              param_in='header',
                              param_required=True)(f)
                        else:
                            if scheme_obj['type'] == 'apiKey':
                                f = falcon.before((self._check_parameter), param_name=(scheme_obj['name']),
    ...
    def parameter(self, param):
        ...
        def hook(f):
            ...
            f = falcon.before((self._check_parameter), param_name=(param['name']),
              param_in=(param['in']),
    ...
    def request(self, schema, schema_name=None, content='application/json', **kwargs):
        ...
        def hook(f):
            if not self.method_map.get(f.__qualname__):
                ...
                if not self.method_map.get(f.__qualname__, {}).get('parameters'):
                    self.method_map[f.__qualname__]['parameters'] = dict()
                self.method_map[f.__qualname__]['requestBody'] = self.method_map.get(f.__qualname__, {}).get('requestBody') or dict()
            self.spec.definition(schema_name, schema=schema)
            self.method_map[f.__qualname__]['requestBody'] = {'content': {f"{content}": {'schema': {'$ref': f"#/components/schemas/{schema_name}"}}}}
    ...
    def response(self, schema, schema_name=None, success_status_code=falcon.HTTP_200, content='application/json', **kwargs):
        ...
        def hook(f):
            if not self.method_map.get(f.__qualname__):
                ...
                if not self.method_map.get(f.__qualname__, {}).get('parameters'):
                    self.method_map[f.__qualname__]['parameters'] = dict()
                self.method_map[f.__qualname__]['responses'] = self.method_map.get(f.__qualname__, {}).get('response') or dict()
            self.method_map[f.__qualname__]['responses'] = {f"{str(success_status_code.split(' ')[0])}": {'content': {f"{content}": {'schema': {'$ref': f"#/components/schemas/{schema_name}"}}}}}
            self.spec.definition(schema_name, schema=schema)
