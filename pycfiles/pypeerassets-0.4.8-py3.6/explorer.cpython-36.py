...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/provider/explorer.py
# Compiled at: 2018-07-07 07:14:43
...
class Explorer(Provider):
    __doc__ = 'API wrapper for https://explorer.peercoin.net blockexplorer.'
    ...
    def select_inputs(self, address: str, amount: int) -> dict:
        ...
        for tx in self.listunspent(address=address):
            ...
            utxo_sum += Decimal(tx['value'] / 100000000)
            if utxo_sum >= amount:
                return {'utxos':utxos, 
                 'total':utxo_sum}
