...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/provider/cryptoid.py
# Compiled at: 2018-07-04 14:47:32
...
class Cryptoid(Provider):
    __doc__ = 'API wrapper for http://chainz.cryptoid.info blockexplorer.'
    ...
    def getblock(self, blockhash: str) -> dict:
        ...
        query = 'block.raw.dws?coin={net}&hash={blockhash}'.format(net=(self.format_name(self.net)),
          blockhash=blockhash)
    ...
    def getrawtransaction(self, txid: str, decrypt: int=0) -> dict:
        query = 'tx.raw.dws?coin={net}&id={txid}'.format(net=(self.format_name(self.net)),
          txid=txid)
        if not decrypt:
            query += '&hex'
    ...
    def listtransactions(self, address: str) -> list:
        query = 'address.summary.dws?coin={net}&id={addr}'.format(net=(self.format_name(self.net)),
          addr=address)
