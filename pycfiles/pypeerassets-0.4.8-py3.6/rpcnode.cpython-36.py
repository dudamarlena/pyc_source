...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pypeerassets/provider/rpcnode.py
# Compiled at: 2018-09-04 08:10:45
...
try:
    ...
    raise ImportError('peercoin_rpc library is required for this to work,                       use the pip to install it.')

class RpcNode(Client, Provider):
    __doc__ = 'JSON-RPC connection to local Peercoin node'
        ...
        for tx in sorted(self.listunspent(address=address), key=(itemgetter('confirmations'))):
            if tx['address'] not in (self.pa_parameters.P2TH_addr,
             self.pa_parameters.test_P2TH_addr):
                utxos.append(MutableTxIn(txid=(tx['txid']), txout=(tx['vout']),
    ...
    def listunspent(self, address: str='', minconf: int=1, maxconf: int=999999) -> list:
        """list UTXOs
