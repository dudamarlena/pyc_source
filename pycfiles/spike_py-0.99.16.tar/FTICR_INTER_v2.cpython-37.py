...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/Interactive/FTICR_INTER_v2.py
# Compiled at: 2020-03-02 09:37:22
# Size of source mod 2**32: 28950 bytes
"""
...
class MR(object):
    __doc__ = 'this class handles multiresolution datasets'
    ...
    def __init__(self, name, report=True, Debug=False):
        ...
        self.highmass = self.data[0].highmass
        self.Debug = Debug
        if report:
            self.report()
    ...
    def to_display(self, zoom=((0, FTICR.FTMS.HighestMass), (0, FTICR.FTMS.HighestMass)), verbose=False):
        ...
        zooml = (
         dl.axis1.itomz(z1lo), dl.axis1.itomz(z1up), dl.axis2.itomz(z2lo), dl.axis2.itomz(z2up))
...
class MR_interact(MR):
    ...
    def zoom_box(self):
        ...
        self.z1l = wf(min=(ref.axis1.lowmass), max=(ref.highmass[0]), tooltip='vertical zoom',
          style=style,
          layout=lay)
        self.z1h = wf(min=(ref.axis1.lowmass), max=(ref.highmass[0]), style=style,
          layout=lay)
        self.z2l = wf(min=(ref.axis2.lowmass), max=(ref.highmass[1]), style=style,
          layout=lay)
        self.z2h = wf(min=(ref.axis2.lowmass), max=(ref.highmass[1]), style=style,
          layout=lay)
        self.fullzoom()
        self.bb('b_zupdate', 'Apply', (lambda e: self.display()), layout=Layout(width='100px'),
          tooltip='Set zoom to values')
        self.bb('b_reset', 'Reset', (self.reset), layout=Layout(width='100px'),
          tooltip='Set zoom to values')
    ...
    def spec_box(self):
        ...
        self.scale = widgets.FloatSlider(description='scale:', value=1.0, min=1.0, max=20, step=0.1, tooltip='Set display scale',
          layout=Layout(width='100px', height='400px'),
          continuous_update=HEAVY,
          orientation='vertical')
        self.scale.observe(self.ob)
        self.bb('b_redraw', 'Redraw', (lambda e: self.display()), layout=Layout(width='100px'))
        box = VBox([self.b_reset, self.scale, self.b_redraw])
        return HBox([box, self.pltfig.canvas])

    def scale_up(self, step):
        self.scale.value *= 1.1892 ** step
    ...
    def display(self, zoom=None, scale=None):
        ...
        if zoom is not None:
            ...
            _zoom = (
             self.z1l.value, self.z1h.value, self.z2l.value, self.z2h.value)
        ...
        self.check_fig()
        self.pltaxe.clear()
        datasel.display(zoom=zz, scale=(self.scale.value), absmax=(self.absmax), xlabel='F2    m/z',
          ylabel='F1    m/z',
          show=False,
          figure=(self.pltaxe))
    ...
    def ext_box(self):
        ...
        self.b_accu = widgets.Dropdown(options=['off', 'graphic', 'sum'], value='off',
...
class MSPeaker(object):
    __doc__ = 'a peak-picker for MS experiments'
    ...
    def __init__(self, npkd, pkname):
        ...
        self.npkd = npkd
        self.pkname = pkname
        self.zoom = widgets.FloatRangeSlider(value=[npkd.axis1.lowmass, npkd.axis1.highmass], min=(npkd.axis1.lowmass),
        ...
        self.thresh = widgets.FloatLogSlider(value=20.0, min=(np.log10(1)),
          max=2.0,
          base=10,
          step=0.01,
        ...
        self.bexport = widgets.Button(description='Export', layout=Layout(width='7%'), button_style='success',
          tooltip='Export to csv file')
        self.bexport.on_click(self.pkexport)
        self.bprint = widgets.Button(description='Print', layout=Layout(width='7%'), button_style='success',
          tooltip='Print to screen')
        self.bprint.on_click(self.pkprint)
        self.bdone = widgets.Button(description='Done', layout=Layout(width='7%'), button_style='warning',
          tooltip='Fix results')
    ...
    def display(self, event={'name': 'value'}):
        ...
        if event['name'] == 'value':
            ...
            try:
                ...
                self.ax.plot(x, y, ':r')
                self.ax.annotate(('%d peaks detected' % len(self.npkd.peaks)), (0.05,
                                                                                0.95), xycoords='figure fraction')
            except:
                pass
...
class Calib(object):
    __doc__ = 'a simple tool to show and modify calibration cste'
    ...
    def __init__(self, data):
        ...
        self.bupdate = widgets.Button(description='Update', button_style='success',
          tooltip='set current data-sets to displayed values')
        self.bupdate.on_click(self.update)
        self.bback = widgets.Button(description='Restore', button_style='success',
            ...
            if self.direct.value != 'off':
                ...
                if self.direct.value == 'up':
                    ...
                    if self.direct.value == 'down':
                        ...
                        return
                FTICRData(name=(self.name.value)).set_unit('m/z').mult(mult).display(new_fig=(self.fig),
                  scale=scale,
...
class SuperImpose(object):
    __doc__ = 'a tool to superimpose spectra'
    ...
    def __init__(self, base=None, filetype='*.msh5', N=None):
        ...
        self.bsel = widgets.Button(description='Copy', layout=Layout(width='10%'), button_style='info',
        ...
        self.bdisplay = widgets.Button(description='Display', layout=Layout(width='10%'), button_style='info',
...
class MS2Dscene(HBox):
    __doc__ = 'a widget to set all MS tools into one screen'

    def __init__(self, show=True, root='/'):
        super(self.__class__, self).__init__()
        self.MR2D = None
        self.FC = FileChooser(root, filetype='*.msh5', mode='r')
        self.bdisp2D = Button(description='Load 2D', layout=Layout(width='20%'), button_style='success',
          tooltip='load and display interactive 2D map')
    ...
    def load2D(self, e):
        ...
        try:
            self.MR2D = MR_interact((self.FC.selected), report=False,
              show=False,
              figsize=(15, 15),
              Debug=False)
