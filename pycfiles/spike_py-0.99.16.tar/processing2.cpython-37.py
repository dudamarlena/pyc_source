...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/Miscellaneous/processing2.py
# Compiled at: 2017-08-31 16:40:33
# Size of source mod 2**32: 23530 bytes
"""
Processing.py
...
def do_proc_F2(dinp, doutp):
    ...
    widgets = [
     'Processing F2: ', pg.Percentage(), ' ', pg.Bar(marker='-', left='[', right=']'), pg.ETA()]
...
def _do_proc_F1_flip_modu(data):
    ...
    c0, c1, shift, size = data
    d = FTICRData(buffer=(np.zeros((c0.size1, 2))))
    d.set_col(0, c0)
    d.set_col(1, c1)
    d.axis1.itype = 0
    d.axis2.itype = 1
    d.flipphase(0.0, 180 * shift)
    p = d.col(0)
    apod(p, size)
    p.rfft()
    p.modulus()
...
class Proc_Parameters(object):
    ...
    def load(self, cp):
        ...
        self.do_flip = cp.getboolean('processing', 'do_flip', str(self.do_flip))
...
class Test(unittest.TestCase):
    ...
    def test_zf(self):
        ...
        if SIZEMIN == 1024:
            self.assertEqual(sizes, [(2048, 32768), (1024, 16384), (1024, 8192), (1024, 1024)])
