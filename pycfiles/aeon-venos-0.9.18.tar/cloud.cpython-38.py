...
# Embedded file name: build/bdist.linux-x86_64/egg/aeolus/plot/cloud.py
# Compiled at: 2020-03-21 16:20:49
# Size of source mod 2**32: 5519 bytes
__doc__ = 'Utilities to plot cloud-related output of UM.'
import warnings, iris, matplotlib as mpl, numpy as np
from .cm_custom import cloudtypes_cmap
__all__ = ('CloudPlotter', 'cloudtypes_cmap')

class CloudPlotter:
    """CloudPlotter"""
    _stash_items = {'lo':'m01s09i203', 
     'me':'m01s09i204',  'hi':'m01s09i205'}
    ...
    def scale_data(self, threshold=0.1):
        ...
        for i, (key, cube) in enumerate(self.cubes.items()):
            cp_cube = cube.copy(data=(np.where(cube.data >= threshold, cube.data * self.factor + self._scales[key], 0.0)))
            if i == 0:
        ...
        self.aggr_cld.attributes['source'] = ' + '.join([f"({self._stash_items[k]}*{self.factor}+{self._scales[k]})" for k in self.cubes.keys()])
    ...
    def _make_norm(self, cmap, nsteps):
        ...
        lvls = [base]
        lvls.append(base + self._scales['lo'])
        lvls.append(base + self._scales['me'])
        lvls.append(base * 2 + self._scales['lo'] + self._scales['me'])
        lvls.append(base + self._scales['hi'])
        lvls.append(base * 2 + self._scales['lo'] + self._scales['hi'])
        lvls.append(base * 2 + self._scales['hi'] + self._scales['me'])
        lvls.append(base * 3 + self._scales['lo'] + self._scales['me'] + self._scales['hi'])
        lvls.append([1000])
        midpoints = []
        if nsteps == 1:
