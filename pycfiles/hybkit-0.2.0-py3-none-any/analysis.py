...
# [GCC 8.4.0]
# Embedded file name: /home/ds/hybkit/hybkit/analysis.py
# Compiled at: 2020-02-25 09:38:45
"""
...
TYPE_ANALYSIS_KEYS = [
 'hybrid_type_counts', 'seg1_types', 'seg2_types', 'all_seg_types']
...
def combine_type_dicts(analysis_dicts):
    ...
    """
    if not (isinstance(analysis_dicts, list) or isinstance(analysis_dicts, tuple)) or len(analysis_dicts) < 2 or not all(isinstance(item, dict) for item in analysis_dicts):
...
def addto_type(record, analysis_dict, count_mode=DEFAULT_COUNT_MODE, type_sep=DEFAULT_HYBRID_TYPE_SEP, mirna_centric_sorting=True):
    """
...
def write_type(file_name_base, analysis_dict, name=None, multi_files=DEFAULT_WRITE_MULTI_FILES, sep=DEFAULT_ENTRY_SEP, file_suffix=DEFAULT_FILE_SUFFIX, make_plots=DEFAULT_MAKE_PLOTS):
    r"""
...
def addto_mirna_count(record, analysis_dict, count_mode=DEFAULT_COUNT_MODE):
    """
...
def write_mirna_count(file_name_base, analysis_dict, name=None, multi_files=DEFAULT_WRITE_MULTI_FILES, sep=DEFAULT_ENTRY_SEP, file_suffix=DEFAULT_FILE_SUFFIX, make_plots=DEFAULT_MAKE_PLOTS):
    r"""
...
def addto_summary(record, analysis_dict, count_mode=DEFAULT_COUNT_MODE, type_sep=DEFAULT_HYBRID_TYPE_SEP, mirna_centric_sorting=True):
    """
...
def write_summary(file_name_base, analysis_dict, name=None, multi_files=DEFAULT_WRITE_MULTI_FILES, sep=DEFAULT_ENTRY_SEP, file_suffix=DEFAULT_FILE_SUFFIX, make_plots=DEFAULT_MAKE_PLOTS):
    r"""
...
def addto_mirna_target(record, analysis_dict, count_mode=DEFAULT_COUNT_MODE, double_count_duplexes=False, mirna_contains=None, mirna_matches=None, target_contains=None, target_matches=None):
    """
    ...
    if record.has_property('has_mirna'):
        mirna_target_pair = (
         (
          record.mirna_info['ref'], record.mirna_details['mirna_seg_type']),
         (
    ...
    for (mirna, mirna_seg_type), (target, target_seg_type) in check_pairs:
        if any(check is not None for check in (mirna_contains, mirna_matches,
         target_contains, target_matches)):
            use_pair = False
            if mirna_contains is not None and mirna_contains in mirna or mirna_matches is not None and mirna_matches == mirna or target_contains is not None and target_contains in target or target_matches is not None and target_matches == target:
                use_pair = True
        else:
            use_pair = True
        if use_pair:
            mirna_id = (
             mirna, mirna_seg_type)
...
def format_mirna_target(analysis_dict, counts=None, sep=DEFAULT_ENTRY_SEP, spacer_line=DEFAULT_TARGET_SPACER_LINE):
    r"""
    ...
    for mirna_id in analysis_dict:
        ...
        if counts is not None:
            line_items = [
             'mirna', '-', 'total', '-', str(counts[mirna_id])]
...
def write_mirna_target(file_name_base, analysis_dict, counts_dict=None, target_types_count_dict=None, name=None, multi_files=DEFAULT_WRITE_MULTI_FILES, sep=DEFAULT_ENTRY_SEP, file_suffix=DEFAULT_FILE_SUFFIX, spacer_line=DEFAULT_TARGET_SPACER_LINE, make_plots=DEFAULT_MAKE_PLOTS, max_mirna=DEFAULT_MAX_MIRNA):
    r"""
    ...
    if multi_files:
        for mirna_id in analysis_dict:
            ...
            write_lines = format_mirna_target(one_mirna_dict, one_counts_dict, sep=sep, spacer_line=False)
            with open(_sanitize_name(analysis_file_name), 'w') as (out_file):
                out_file.write(('\n').join(write_lines))
            if make_plots:
                target_plot_name = _sanitize_name(file_name_base + '_' + mirna)
                hybkit.plot.mirna_target(target_plot_name, mirna, analysis_dict[mirna_id], name=name)
                target_type_plot_name = _sanitize_name(file_name_base + '_' + mirna + '_types')
                hybkit.plot.mirna_target_type(target_type_plot_name, mirna, target_types_count_dict[mirna_id], name=name)
...
def addto_mirna_fold(record, analysis_dict, count_mode=DEFAULT_COUNT_MODE, allow_duplexes=False, skip_no_fold_record=False):
    """
    Add the information from a :class:`~hybkit.HybRecord` to a mirna_target analysis.
    If the record contains a single miRNA, the miRNA and target are identified.
    The count for this miRNA and its target is then added to the dict.
    ...
    if not record.has_property('has_mirna'):
    ...
    elif record.has_property('has_mirna'):
        ...
        if '(' in mirna_fold and ')' in mirna_fold or '(' not in mirna_fold and ')' not in mirna_fold or len(mirna_fold) < 5:
            analysis_dict['no_folds'] += count
...
def process_mirna_fold(analysis_dict):
    ...
    if total_count == 0:
        ...
        raise Exception(message)
    test_sums = [
     (('all_mirna', 'no_mirna'), 'all_evaluated'),
...
def format_mirna_fold(analysis_dict, sep=DEFAULT_ENTRY_SEP):
    r"""
...
def write_mirna_fold(file_name_base, analysis_dict, name=None, multi_files=DEFAULT_WRITE_MULTI_FILES, sep=DEFAULT_ENTRY_SEP, file_suffix=DEFAULT_FILE_SUFFIX, spacer_line=DEFAULT_TARGET_SPACER_LINE, make_plots=DEFAULT_MAKE_PLOTS):
    r"""
    ...
    if make_plots:
        hybkit.plot.mirna_fold(file_name_base, analysis_dict, name=name)
    return
