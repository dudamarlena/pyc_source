...
# [GCC 8.4.0]
# Embedded file name: ./src/IRBuilder.py
# Compiled at: 2014-08-01 13:34:49
# Size of source mod 2**32: 12323 bytes
from binding import *
...
class IRBuilder:
    ...
    CreateAggregateRet = CustomMethod('IRBuilder_CreateAggregateRet', PyObjectPtr, PyObjectPtr, cast(int, Unsigned))
    CreateBr = Method(ptr(BranchInst), ptr(BasicBlock))
    CreateCondBr = Method(ptr(BranchInst), ptr(Value), ptr(BasicBlock), ptr(BasicBlock), ptr(MDNode)).require_only(3)
    CreateSwitch = Method(ptr(SwitchInst), ptr(Value), ptr(BasicBlock), cast(int, Unsigned), ptr(MDNode)).require_only(2)
    CreateIndirectBr = Method(ptr(IndirectBrInst), ptr(Value), cast(int, Unsigned)).require_only(1)
    _CreateInvoke = Method(ptr(InvokeInst), ptr(Value), ptr(BasicBlock), ptr(BasicBlock), ref(SmallVector_Value), cast(str, StringRef)).require_only(4)
    ...
    def _binop_has_nsw_nuw():
        sig = [
         ptr(Value), ptr(Value), ptr(Value), cast(str, StringRef),
    ...
    def _unop_has_nsw_nuw():
        sig = [
         ptr(Value), ptr(Value), cast(str, StringRef),
         cast(bool, Bool), cast(bool, Bool)]
    ...
    def _float_unop():
        sig = [
         ptr(Value), ptr(Value), cast(str, StringRef), ptr(MDNode)]
        op = Method(*sig).require_only(1)
        return op

    CreateFNeg = _float_unop()
    CreateNot = Method(ptr(Value), ptr(Value), cast(str, StringRef)).require_only(1)
    CreateAlloca = Method(ptr(AllocaInst), ptr(Type), ptr(Value), cast(str, StringRef)).require_only(1)
    CreateLoad = Method(ptr(LoadInst), ptr(Value), cast(str, StringRef)).require_only(1)
    CreateStore = Method(ptr(StoreInst), ptr(Value), ptr(Value), cast(bool, Bool)).require_only(2)
    CreateAlignedLoad = Method(ptr(LoadInst), ptr(Value), cast(int, Unsigned), cast(bool, Bool), cast(str, StringRef))
    CreateAlignedLoad.require_only(2)
    CreateAlignedStore = Method(ptr(StoreInst), ptr(Value), ptr(Value), cast(int, Unsigned), cast(bool, Bool))
    CreateAlignedStore.require_only(3)
    CreateFence = Method(ptr(FenceInst), AtomicOrdering, SynchronizationScope).require_only(1)
    CreateAtomicCmpXchg = Method(ptr(AtomicCmpXchgInst), ptr(Value), ptr(Value), ptr(Value), AtomicOrdering, SynchronizationScope)
    CreateAtomicCmpXchg.require_only(4)
    CreateAtomicRMW = Method(ptr(AtomicRMWInst), AtomicRMWInst.BinOp, ptr(Value), ptr(Value), AtomicOrdering, SynchronizationScope)
    CreateAtomicRMW.require_only(4)
    _CreateGEP = Method(ptr(Value), ptr(Value), ref(SmallVector_Value), cast(str, StringRef))
    ...
    _CreateInBoundsGEP = Method(ptr(Value), ptr(Value), ref(SmallVector_Value), cast(str, StringRef))
    ...
    CreateStructGEP = Method(ptr(Value), ptr(Value), cast(int, Unsigned), cast(str, StringRef)).require_only(2)
    CreateGlobalStringPtr = Method(ptr(Value), cast(str, StringRef), cast(str, StringRef)).require_only(1)

    def _value_type():
        sig = [
         ptr(Value), ptr(Value), ptr(Type), cast(str, StringRef)]
        op = Method(*sig).require_only(2)
        return op
    ...
    CreateZExtOrTrunc = Method(ptr(Value), ptr(Value), ptr(IntegerType), cast(str, StringRef)).require_only(2)
    CreateSExtOrTrunc = Method(ptr(Value), ptr(Value), ptr(IntegerType), cast(str, StringRef)).require_only(2)
    ...
    CreateIntCast = Method(ptr(Value), ptr(Value), ptr(Type), cast(bool, Bool), cast(str, StringRef)).require_only(3)
    CreateFPCast = _value_type()
    _CreateCall = Method(ptr(CallInst), ptr(Value), ref(SmallVector_Value), cast(str, StringRef)).require_only(2)
    ...
    CreateICmp = Method(ptr(Value), CmpInst.Predicate, ptr(Value), ptr(Value), cast(str, StringRef)).require_only(3)
    CreateFCmp = Method(ptr(Value), CmpInst.Predicate, ptr(Value), ptr(Value), cast(str, StringRef)).require_only(3)
    CreatePHI = Method(ptr(PHINode), ptr(Type), cast(int, Unsigned), cast(str, StringRef)).require_only(2)
    CreateSelect = Method(ptr(Value), ptr(Value), ptr(Value), ptr(Value), cast(str, StringRef)).require_only(3)
    CreateVAArg = Method(ptr(VAArgInst), ptr(Value), ptr(Type), cast(str, StringRef)).require_only(2)
    CreateExtractElement = _binop_basic()
    CreateInsertElement = Method(ptr(Value), ptr(Value), ptr(Value), ptr(Value), cast(str, StringRef)).require_only(3)
    CreateShuffleVector = Method(ptr(Value), ptr(Value), ptr(Value), ptr(Value), cast(str, StringRef))
    CreateShuffleVector.require_only(3)
    _CreateExtractValue = Method(ptr(Value), ptr(Value), ref(SmallVector_Unsigned), cast(str, StringRef))
    ...
    _CreateInsertValue = Method(ptr(Value), ptr(Value), ptr(Value), ref(SmallVector_Unsigned), cast(str, StringRef)).require_only(3)
    ...
    CreateLandingPad = Method(ptr(LandingPadInst), ptr(Type), ptr(Value), cast(int, Unsigned), cast(str, StringRef))
    ...
    Insert = Method(ptr(Instruction), ptr(Instruction), cast(str, StringRef)).require_only(1)