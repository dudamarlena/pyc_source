...
# [GCC 8.4.0]
# Embedded file name: ./src/EngineBuilder.py
...
class EngineBuilder:
    ...
    _setMAttrs = CustomMethod('EngineBuilder_setMAttrs', PyObjectPtr, PyObjectPtr)
    ...
    create = Method(ptr(ExecutionEngine), ownedptr(TargetMachine)).require_only(0)
    _selectTarget0 = Method(ptr(TargetMachine))
    _selectTarget0.realname = 'selectTarget'
    _selectTarget1 = CustomMethod('EngineBuilder_selectTarget', PyObjectPtr, const(ref(Triple)), cast(str, StringRef), cast(str, StringRef), PyObjectPtr)
