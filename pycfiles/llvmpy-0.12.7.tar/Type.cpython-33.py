...
# [GCC 8.4.0]
# Embedded file name: ./src/Type.py
...
class Type:
    ...
    getIntNPtrTy = StaticMethod(ptr(PointerType), ref(LLVMContext), cast(int, Unsigned))
...
class VectorType:
    ...
    getExtendedElementVectorType = StaticMethod(ptr(VectorType), ptr(VectorType))
    getTruncatedElementVectorType = StaticMethod(ptr(VectorType), ptr(VectorType))
    isValidElementType = StaticMethod(cast(Bool, bool), ptr(Type))
...
class StructType:
    ...
    setBody = CustomMethod('StructType_setBody', PyObjectPtr, PyObjectPtr, cast(bool, Bool)).require_only(1)
    getNumElements = Method(cast(Unsigned, int))
    getElementType = Method(ptr(Type), cast(int, Unsigned))
    create = StaticMethod(ptr(StructType), ref(LLVMContext), cast(str, StringRef)).require_only(1)
    get = CustomStaticMethod('StructType_get', PyObjectPtr, ref(LLVMContext), PyObjectPtr, cast(bool, Bool)).require_only(2)
    isLayoutIdentical = Method(cast(Bool, bool), ptr(StructType))
    isValidElementType = StaticMethod(cast(Bool, bool), ptr(Type))