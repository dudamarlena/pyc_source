...
# [GCC 8.4.0]
# Embedded file name: ./src/Instruction.py
...
AtomicOrdering = llvm.Enum('AtomicOrdering', 'NotAtomic', 'Unordered', 'Monotonic', 'Acquire', 'Release', 'AcquireRelease', 'SequentiallyConsistent')
SynchronizationScope = llvm.Enum('SynchronizationScope', 'SingleThread', 'CrossThread')
...
class AtomicRMWInst:
    _downcast_ = (
     Value, User, Instruction)
    BinOp = Enum('Xchg', 'Add', 'Sub', 'And', 'Nand', 'Or', 'Xor', 'Max', 'Min', 'UMax', 'UMin', 'FIRST_BINOP', 'LAST_BINOP', 'BAD_BINOP')
...
class CallInst:
    ...
    CreateMalloc = StaticMethod(ptr(Instruction), ptr(BasicBlock), ptr(Type), ptr(Type), ptr(Value), ptr(Value), ptr(Function), cast(str, StringRef)).require_only(4)
...
class StoreInst:
    ...
    setAtomic = Method(Void, AtomicOrdering, SynchronizationScope).require_only(1)
...
class ReturnInst:
    _downcast_ = (
     Value, User, Instruction, TerminatorInst)
