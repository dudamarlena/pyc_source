...
# [GCC 8.4.0]
# Embedded file name: /Users/sklam/dev/llvmpy/dist/0.12.7/llvmpy/gen/binding.py
# Compiled at: 2014-08-01 13:34:49
# Size of source mod 2**32: 27794 bytes
import inspect, textwrap, codegen as cg, os
_rank = 0
namespaces = {}
    ...
    def generate_submodule_table(self, println, extras=()):
        ...
        with writer.indent():
            for cls in self.classes:
                ...
                writer.println('{ "%(name)s", meth_%(table)s, NULL },' % locals())
...
UnsignedLongLong = BuiltinTypes('unsigned long long')
...
class Method(object):
    ...
    def _add_signature(self, return_type, *args):
        ...
        if cur_len in prev_lens:
            raise Exception('Only support overloading with different number of arguments')
    ...
    def realname(self):
        try:
            return self._Method__realname
        except AttributeError:
            return self.name

    @realname.setter
    def realname(self, v):
        self._Method__realname = v
        ...
        with writer.py_function(self.c_name):
            if len(self.signatures) == 1:
                ...
                for sig in self.signatures:
                    ...
                    argtys = sig[1:]
                    expect = len(argtys)
                    if not isinstance(self, StaticMethod):
    ...
    def compile_py(self, writer):
        ...
        with decl as (this, varargs):
            ...
            ret = writer.call('_api.%s' % func, args=(
             unwrap_this,), varargs=unwrapped)
...
class ptr(_Type):
    ...
    def wrap(self, writer, val):
        return writer.pycapsule_new(val, self.element.capsule_name, self.element.fullname)
...
class Attr(object):
    ...
    def compile_cpp(self, writer):
        with writer.py_function(self.getter_c_name):
            ...
            attr = self.name
            ret = writer.declare(self.getter.fullname, '%(this)s->%(attr)s' % locals())
            writer.return_value(self.getter.wrap(writer, ret))
        with writer.py_function(self.setter_c_name):
            this, value = writer.parse_arguments('args', ptr(self.parent), self.setter)
            attr = self.name
    ...
    def compile_py(self, writer):
        ...
        with writer.block('def %(name)s(self, value):' % locals()):
            ...
            writer.println('return _api.%(setter)s(%(unself)s, %(unvalue)s)' % locals())
        writer.println()
