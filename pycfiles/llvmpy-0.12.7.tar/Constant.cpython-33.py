...
# [GCC 8.4.0]
# Embedded file name: ./src/Constant.py
# Compiled at: 2014-08-01 13:34:49
# Size of source mod 2**32: 9209 bytes
from binding import *
...
class Constant:
    ...
    getIntegerValue = CustomStaticMethod('Constant_getIntegerValue', PyObjectPtr, ptr(Type), PyObjectPtr)
...
class ConstantInt:
    _downcast_ = (
     Constant, User, Value)
    get = StaticMethod(ptr(ConstantInt), ptr(IntegerType), cast(int, Uint64), cast(bool, Bool)).require_only(2)
    isValueValidForType = StaticMethod(cast(Bool, bool), ptr(Type), cast(int, Int64))
...
class ConstantArray:
    _downcast_ = (
     Constant, User, Value)
    get = CustomStaticMethod('ConstantArray_get', PyObjectPtr, ptr(ArrayType), PyObjectPtr)
...
class ConstantStruct:
    _downcast_ = (
     Constant, User, Value)
    get = CustomStaticMethod('ConstantStruct_get', PyObjectPtr, ptr(StructType), PyObjectPtr)
    getAnon = CustomStaticMethod('ConstantStruct_getAnon', PyObjectPtr, PyObjectPtr, cast(bool, Bool)).require_only(1)
...
class ConstantVector:
    _downcast_ = (
     Constant, User, Value)
    get = CustomStaticMethod('ConstantVector_get', PyObjectPtr, PyObjectPtr)
...
class ConstantDataArray:
    _downcast_ = (
     Constant, User, Value)
    getString = StaticMethod(ptr(Constant), ref(LLVMContext), cast(str, StringRef), cast(bool, Bool)).require_only(2)
...
class ConstantExpr:
    ...
    _getInsertValue = _factory(ptr(Constant), ptr(Constant), ref(SmallVector_Unsigned))
