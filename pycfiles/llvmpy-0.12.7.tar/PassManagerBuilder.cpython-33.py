...
# [GCC 8.4.0]
# Embedded file name: ./src/Transforms/PassManagerBuilder.py
...
class PassManagerBuilder:
    ...
    populateLTOPassManager = Method(Void, ref(PassManagerBase), cast(bool, Bool), cast(bool, Bool), cast(bool, Bool)).require_only(3)

    def _attr_int():
        return Attr(getter=cast(Unsigned, int), setter=cast(int, Unsigned))

    OptLevel = _attr_int()
    SizeLevel = _attr_int()

    def _attr_bool():
        return Attr(getter=cast(Bool, bool), setter=cast(bool, Bool))
    ...
    LibraryInfo = Attr(getter=ownedptr(TargetLibraryInfo), setter=ownedptr(TargetLibraryInfo))
    Inliner = Attr(getter=ownedptr(Pass), setter=ownedptr(Pass))