...
# [GCC 8.4.0]
# Embedded file name: ./src/ExecutionEngine/ExecutionEngine.py
...
class ExecutionEngine:
    _include_ = ('llvm/ExecutionEngine/ExecutionEngine.h', 'llvm/ExecutionEngine/JIT.h')
    delete = Destructor()
    create = CustomStaticMethod('ExecutionEngine_create', ptr(ExecutionEngine), ownedptr(Module), cast(bool, Bool), PyObjectPtr, CodeGenOpt.Level, cast(bool, Bool)).require_only(1)
    createJIT = CustomStaticMethod('ExecutionEngine_createJIT', ptr(ExecutionEngine), ownedptr(Module), PyObjectPtr, ptr(JITMemoryManager), CodeGenOpt.Level, cast(bool, Bool), Reloc.Model, CodeModel.Model).require_only(1)
    ...
    getPointerToNamedFunction = Method(cast(VoidPtr, int), cast(str, StdString), cast(bool, Bool)).require_only(1)
    runStaticConstructorsDestructors = Method(Void, cast(Bool, bool))
    runStaticConstructorsDestructors |= Method(Void, ptr(Module), cast(Bool, bool))
    ...
    updateGlobalMapping = Method(cast(VoidPtr, int), ptr(GlobalValue), cast(int, VoidPtr))
    ...
    StoreValueToMemory = Method(Void, ref(GenericValue), ptr(GenericValue), ptr(Type))
    ...
    _runFunction = CustomMethod('ExecutionEngine_RunFunction', PyObjectPtr, ptr(Function), PyObjectPtr)
