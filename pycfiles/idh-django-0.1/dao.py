...
# [GCC 8.4.0]
# Embedded file name: .\django\core\basic\dao.py
# Compiled at: 2018-02-12 22:31:11
# Size of source mod 2**32: 13728 bytes
from datetime import datetime
from django.db import models
from django.db.models.fields import BooleanField
from django.db.models.fields import DateTimeField
from idh.django.beans import Struct
from idh.django.utils import _import
...
class BasicDao(object):
        ...
        if hasattr(o, 'page'):
            ...
            if p is not None:
                if isinstance(p, dict):
                    p = Struct(p)
                if hasattr(p, 'index') and p.index is not None and isinstance(p.index, int):
                    page.index = p.index
                if hasattr(p, 'limit') and p.limit is not None and isinstance(p.limit, int):
                    page.limit = p.limit
                if hasattr(p, 'count') and p.count is not None and isinstance(p.count, bool):
                    page.count = p.count
        if page.index < 1:
            page.index = 1
        if page.limit < 1:
    ...
    def _get_filter(o):
        ...
        """
        filter = {}
        if hasattr(o, 'filter') and o.filter is not None:
    ...
    def _get_order(o):
        ...
        """
        order = []
        if hasattr(o, 'order') and o.order is not None:
    ...
    def _get_field(o):
        ...
        """
        field = None
        if hasattr(o, 'field') and o.field is not None:
                        ...
                        if tval is None:
                            ...
                            if isinstance(value, f.related_model):
                                ...
                                if '' == value:
                                    if f.null:
                                        setattr(target, name, None)
                                elif str(tval.pk) != value:
                                    rval = f.related_model.objects.get(pk=value)
                                    setattr(target, rval)
                    else:
                        if issubclass(clazz, BooleanField):
                            ...
                            if issubclass(clazz, DateTimeField):
    ...
    def list(o):
        ...
        if hasattr(o, 'limit') and o.limit is not None and isinstance(o.limit, int):
            limit = o.limit
        if limit > 0:
            if is_field:
