...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pythologist/write.py
# Compiled at: 2018-03-24 01:37:52
# Size of source mod 2**32: 11270 bytes
import os, json, pythologist, pandas as pd, numpy as np
...
def write_inForm(idf, output_directory, overwrite=False):
    ...
    for folder in idf['folder'].unique():
        ...
        if sub['sample'].unique().shape[0] > 1:
            raise ValueError('more than one sample name present in folder ' + folder)
        for frame in sub['frame'].unique():
            ...
            if len(idf.scored_stains) > 1:
                ...
                if len(idf.scored_stains) == 1:
                    ...
                    raise ValueError("don't know how to do this with zero scored stains yet")
...
def _make_summary_file(opath, sample, frame, base, fdf, type):
    ...
    for tissue in tissues + ['All']:
        for phenotype in phenotypes + ['All']:
            ...
            if tissue != 'All':
                mytissue = fdf['tissue'] == tissue
            mypheno = np.array([True] * fdf.shape[0])
            if phenotype != 'All':
                mypheno = fdf['phenotype'] == phenotype
            ...
            if tissue == 'All':
                tpx = 0
                if o['Total Cells'] > 0:
                    tpx = fdf['total_area'].iloc[0]
                o['Tissue Category Area (pixels)'] = int(tpx)
                o['Cell Density (per megapixel)'] = 0
                if tpx > 0:
                    o['Cell Density (per megapixel)'] = 1000000 * specific.shape[0] / fdf['total_area'].iloc[0]
            else:
                tpx = 0
                if o['Total Cells'] > 0:
                    tpx = td[tissue]
                ...
                o['Cell Density (per megapixel)'] = 0
                if tpx > 0:
                    o['Cell Density (per megapixel)'] = 1000000 * specific.shape[0] / td[tissue]
            o['Cell X Position'] = ''
            o['Cell Y Position'] = ''
            o['Process Region ID'] = ''
...
def _make_segmentation_file(opath, sample, frame, base, fdf, type):
    ...
    for row in fdf.itertuples(index=False):
        ...
        o['Distance from Tissue Category Edge (pixels)'] = 0
...
def _make_score_file_single(opath, sample, frame, base, fdf):
    ...
    for tissue in score_df['tissue'].unique():
        ...
        if tissue == 'any':
            tissue = ''
        if tdf.shape[0] > 1:
            raise ValueError('cant output multiple stains as single')
        o = OrderedDict()
        o['Path'] = opath
        o['Sample Name'] = sample
        o['Tissue Category'] = tissue
