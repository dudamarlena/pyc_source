...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pythologist/image.py
# Compiled at: 2018-05-04 16:03:40
# Size of source mod 2**32: 8748 bytes
...
class ComponentImageFrame(pd.DataFrame):
    ...
    def to_hdf(self, path, mode='r+'):
        pd.DataFrame(self).to_hdf(path, 'component_image_frame',
          mode=mode,
          format='table',
          complib='zlib',
          complevel=9)
    ...
    def read(self, path, verbose=False, limit=None, threads=0):
        ...
        for result in results:
            ...
            for name in result['ci']:
                ...
                id = str(uuid4())
                images[id] = image
                s = pd.Series(OrderedDict({'folder':folder, 
                 'sample':sample, 
                 'frame':frame, 
                 'stain':name, 
                 'image_id':id}))
    ...
    def _read_frame(vars):
        ...
        return OrderedDict({'folder':folder, 
         'sample':sample, 
         'frame':frame, 
         'ci':ci})
    ...
    def read_image(self, input_tiff, quality=50, verbose=False):
        ...
        with TiffFile(input_tiff) as (tif):
            ...
            for page in tif.pages:
                ...
                d = df.applymap(lambda x: 255 * (x - rng[0]) / (rng[1] - rng[0])).applymap(np.uint8).as_matrix().astype(np.uint8)
                bs = BytesIO()
        ...
        with TiffFile(input_tiff) as (tif):
            ...
            for page in tif.pages:
                ...
                if 'image_description' in page.tags:
                    description = page.tags['image_description'].value
                else:
