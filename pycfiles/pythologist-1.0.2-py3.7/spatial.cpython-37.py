...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pythologist/spatial.py
# Compiled at: 2018-05-23 21:58:24
# Size of source mod 2**32: 8908 bytes
from multiprocessing import Pool
from functools import partial
from collections import OrderedDict
import pandas as pd, numpy as np, math, h5py, pythologist
...
def _pointcross(df, k, idlab, dlim, point):
    ...
    if dlim is not None:
        dists = dists[(dists <= dlim)]
    dists = dists.nsmallest(k).sort_values()
    return OrderedDict(dists.to_dict())


def kNearestNeighborsCross2(df, markerA, markerB, k=1, xlab='x', ylab='y', classlab='full_phenotype', idlab='id', threads=1, dlim=None):
...
class CellFrameNearestNeighbors:
    ...
    def to_hdf(self, path):
        ...
        self._nn.to_hdf(path, 'nn', mode='r+',
          format='table',
          complib='zlib',
          complevel=9)
    ...
    def plot_continuous_histogram(self, markerA, markerB, continuous, dlim=None, step=30, facets=[
 'sample'], standardize=True, logscale=True, minimum_A=0, minimum_B=0, rank=1, scales=None):
        cf = self._cf
        nn = self._nn
        v = nn[((nn['markerA_name'] == markerA) & (nn['markerB_name'] == markerB) & (nn['rank'] == rank))]
        if dlim is None:
            dlim = int(v['distance'].max())
        else:
            cnt = v[(v['distance'] <= dlim)].groupby(['sample', 'frame']).count().reset_index()[[
             'sample', 'frame', 'rank']].rename(columns={'rank': 'frame_count_A'})
            cntcut = cnt.copy()
            cntcut = cntcut[(cntcut['frame_count_A'] >= minimum_A)]
            cnt2 = v[(v['distance'] <= dlim)].groupby(['sample', 'frame']).apply(lambda x: len(x['markerB_id'].unique().tolist())).reset_index().rename(columns={0: 'frame_count_B'})
            cntcut2 = cnt2.copy()
            ...
            ocnt['dlim'] = dlim
            v = v.merge(cntcut, on=['sample', 'frame']).reset_index().merge(cntcut2,
              on=['sample', 'frame']).reset_index()
            choice = 'markerA_id'
            ...
            counts = v2.groupby(facets + ['bin']).apply(lambda x: len(list(x['distance']))).reset_index().rename(columns={0: 'count'})
            means = v2.groupby(facets + ['bin']).apply(lambda x: sum(list(x[continuous])) / len(list(x[continuous]))).reset_index().rename(columns={0: continuous})
            ...
            if standardize:
                ...
                j[continuous] = n
            g = ggplot(j.sort_values(facets), aes(x='bin', y='count', fill=continuous)) + geom_bar(stat='identity') + theme_bw() + theme(axis_text_x=element_text(rotation=90, hjust=0))
            if logscale:
                g += scale_y_log10()
            if scales:
                ...
                g += facet_wrap(facets)
        pair = v2.groupby(facets).apply(lambda x: {'continuous':x[continuous].tolist(),  'distance':x['distance'].tolist()})
        pair = pd.DataFrame(pair.apply(lambda x: dict(zip(('r', 'p'), spearmanr(x['continuous'], x['distance'])))))
