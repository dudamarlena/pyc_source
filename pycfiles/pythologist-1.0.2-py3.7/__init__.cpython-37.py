...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pythologist/__init__.py
# Compiled at: 2019-03-30 21:17:31
# Size of source mod 2**32: 27710 bytes
import pandas as pd, numpy as np, sys, json, h5py
...
class CellDataFrame(pd.DataFrame):
    ...
    _metadata = ['_microns_per_pixel', '_db']
    ...
    def get_valid_cell_indecies(self):
        ...
        """
        return pd.DataFrame(self).groupby(self.frame_columns).apply(lambda x: list(x['cell_index'])).reset_index().rename(columns={0: 'valid'})
    ...
    def phenotypes_to_scored(self, phenotypes=None, overwrite=False):
        ...
        if not self.is_uniform():
            raise ValueError('inconsistent phenotypes')
        elif phenotypes is None:
        ...
        def _post(binary, phenotype_label, phenotypes, overwrite):
            d = binary.copy()
            if len(set(phenotypes) & set(list(binary.keys()))) > 0:
                if overwrite == False:
                    raise ValueError('Error, phenotype already exists as a scored type')
            for label in phenotypes:
                d[label] = 0

            if phenotype_label == phenotype_label:
                if phenotype_label in phenotypes:
                    d[phenotype_label] = 1
            return d

        output = self.copy()
        output['scored_calls'] = output.apply(lambda x: _post(x['scored_calls'], x['phenotype_label'], phenotypes, overwrite), 1)
        return output
    ...
    def read_hdf(cls, path, key=None):
        ...
        df['neighbors'] = df['neighbors'].apply(lambda x:         if not isinstance(x, dict):
np.nan # Avoid dead code: dict(zip([int(y) for y in x.keys()], x.values())))
    ...
    def microns_per_pixel(self):
        ...
        """
        if not hasattr(self, '_microns_per_pixel'):
            return
    ...
    def is_uniform(self, verbose=True):
        ...
        """
        uni = pd.Series(self['phenotype_calls'].apply(lambda x: json.dumps(x)).unique()).apply(lambda x: json.loads(x)).apply(lambda x: tuple(sorted(x.keys()))).unique()
        if len(uni) > 1:
            if verbose:
                sys.stderr.write('WARNING: phenotypes differ across the dataframe \n' + str(uni) + '\n')
            return False
        uni = pd.Series(self['scored_calls'].apply(lambda x: json.dumps(x)).unique()).apply(lambda x: json.loads(x)).apply(lambda x: tuple(sorted(x.keys()))).unique()
        if len(uni) > 1:
            if verbose:
                sys.stderr.write('WARNING: scored_calls differ across the dataframe \n' + str(uni) + '\n')
            return False
        return True
    ...
    def get_measured_regions(self):
        ...
        mergeon = [
         ...
         'sample_id', 'sample_name',
         'frame_id', 'frame_name']
        temp = self.loc[:, mergeon + ['regions']].set_index(mergeon)['regions'].apply(json.dumps).reset_index().drop_duplicates()
        ...
        rows = pd.DataFrame(rows).drop(columns='regions').drop_duplicates()[(mergeon + ['region_label', 'region_area_pixels'])]
    ...
    def segmentation_images(self, *args, **kwargs):
        ...
        """
        if not self.db:
            raise ValueError('Need to set db')
    ...
    def merge_scores(self, df_addition, reference_markers='all', addition_markers='all', on=['project_name', 'sample_name', 'frame_name', 'cell_index']):
        if isinstance(reference_markers, str):
            ...
            if reference_markers is None:
                reference_markers = []
            elif isinstance(addition_markers, str):
                addition_markers = df_addition.scored_names
            else:
                if additionmarkers is None:
                    addition_markers = []
            df_addition = df_addition.copy()
            df_addition['_key'] = 1
            df = self.merge(df_addition[(['scored_calls', '_key'] + on)].rename(columns={'scored_calls': '_addition'}), on=on,
              how='left')
            df['_sub1'] = df['scored_calls'].apply(lambda x: dict(((k, x[k]) for k in reference_markers)))
            df['_sub2'] = df['_addition'].apply(lambda x: dict(((k, x[k]) for k in addition_markers)))
    ...
    def rename_scored_calls(self, change):
        output = self.copy()
        output['scored_calls'] = output.apply(lambda x: _dict_rename(x['scored_calls'], change), 1)
        return output
    ...
    def threshold(self, phenotype, scored_name, positive_label=None, negative_label=None):
        if positive_label is None and negative_label is not None or negative_label is None:
            if positive_label is not None:
                raise ValueError('Error if you want to specify labels, give both positive and negative')
        if phenotype not in self.phenotypes:
            raise ValueError('Error phenotype ' + str(phenotype) + ' is not in the data.')
        if scored_name not in self.scored_names:
            raise ValueError('Error scored_name ' + str(scored_name) + ' is not in the data.')
        if positive_label is None and negative_label is None:
            ...
            if positive_label == negative_label:
                raise ValueError('Cant have the same label for positive and negative.')

        def _swap_in(d, pheno, scored, phenotype_calls, scored_calls, pos, neg):
            if pheno not in phenotype_calls.keys():
                return d
            else:
                keepers = [(k, v) for k, v in phenotype_calls.items() if k != phenotype]
                if scored not in scored_calls.keys():
                    raise ValueError('Error scored calls are not unified across samples')
                else:
                    ...
                    if phenotype_value == 0:
                        ...
                        if scored_value == 1:
                            ...
                            if scored_value == 0:
                                ...
                                raise ValueError('Format error.  These values should only ever be zero or one.')
            return dict(keepers)

        data = self.copy()
        data['phenotype_calls'] = self.apply(lambda x: _swap_in(x, phenotype, scored_name, x['phenotype_calls'], x['scored_calls'], positive_label, negative_label), 1)
    ...
    def combine_regions(self, input_region_labels, output_region_label, verbose=True):
        if isinstance(input_region_labels, str):
            input_region_labels = [
             input_region_labels]
        bad_regions = set(input_region_labels) - set(self.regions)
        if len(bad_regions) > 0:
            raise ValueError('Error regions(s) ' + str(bad_regions) + ' are not in the data.')
        data = self.copy()
        if len(input_region_labels) == 0:
            return data
        ...
        data['regions'] = data.apply(lambda x: _swap_in(x['regions'], input_region_labels, output_region_label), 1)
...
def _extract_unique_keys_from_series(s):
    uni = pd.Series(s.apply(lambda x: json.dumps(x)).unique()).apply(lambda x: json.loads(x)).apply(lambda x: set(sorted(x.keys())))
