...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/game_theory/mclennan_tourky.py
...
def mclennan_tourky(g, init=None, epsilon=0.001, max_iter=200, full_output=False):
    ...
    if init is None:
        ...
        if l != N:
            raise ValueError('init must be of length {N}'.format(N=N))
        ...
        x_star, converged, num_iter = _compute_fixed_point_ig(_best_response_selection, x_init, max_iter, verbose=0,
          print_skip=1,
...
def _best_response_selection(x, g, indptr=None):
    ...
    if N == 2:
        ...
        for i in range(N):
            opponent_actions = tuple((x[indptr[((i + j) % N)]:indptr[((i + j) % N + 1)]] for j in range(1, N)))
...
def _flatten_action_profile(action_profile, indptr):
    ...
    for i in range(N):
        if isinstance(action_profile[i], numbers.Integral):
