...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/tests/test_quad.py
# Compiled at: 2019-11-04 18:33:25
# Size of source mod 2**32: 16491 bytes
"""
Tests for quad.py
...
import pandas as pd
from quantecon.quad import qnwcheb, qnwequi, qnwlege, qnwnorm, qnwlogn, qnwsimp, qnwtrap, qnwunif, quadrect, qnwbeta, qnwgamma
    ...
    def setUpClass(cls):
        ...
        for i, func in enumerate([f1, f2, f3]):
            ...
            for kind in kinds:
                for num in n:
                    ...
                    quad_rect_res1d.loc[(func_name, num)][kind] = quadrect(func, num_in, a, b, kind)

        cls.data1d = quad_rect_res1d
        kinds2 = [
         'lege', 'trap', 'simp', 'N', 'W', 'H', 'R']
        ...
        for num in n:
            for kind in kinds2[:4]:
                data2.loc[(num ** 2)][kind] = quadrect(f1_2, [num, num], a[0], b[0], kind)

            for kind in kinds2[4:]:
                data2.loc[(num ** 2)][kind] = quadrect(f1_2, num ** 2, a[0], b[0], kind)
