...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/graph_tools.py
# Compiled at: 2018-05-14 00:27:06
# Size of source mod 2**32: 13583 bytes
"""
...
class DiGraph:
    ...
    def node_labels(self, values):
        if values is None:
            ...
            if values.ndim < 1 or values.shape[0] != self.n:
                raise ValueError('node_labels must be an array_like of length n')
            if np.issubdtype(values.dtype, np.object_):
                raise ValueError('data in node_labels must be homogeneous in type')
    ...
    def _find_scc(self):
        ...
        * github.com/scipy/scipy/blob/master/scipy/sparse/csgraph/_traversal.pyx
        ...
        """
        self._num_scc, self._scc_proj = csgraph.connected_components((self.csgraph), connection='strong')
    ...
    def strongly_connected_components_indices(self):
        if self.is_strongly_connected:
            return [
             np.arange(self.n)]
        return [np.where(self.scc_proj == k)[0] for k in range(self.num_strongly_connected_components)]
    ...
    def sink_strongly_connected_components_indices(self):
        if self.is_strongly_connected:
            return [
             np.arange(self.n)]
        return [np.where(self.scc_proj == k)[0] for k in self.sink_scc_labels.tolist()]
    ...
    def _compute_period(self):
        ...
        if not self.is_strongly_connected:
            raise NotImplementedError('Not defined for a non strongly-connected digraph')
        if np.any(self.csgraph.diagonal() > 0):
            self._period = 1
            self._cyclic_components_proj = np.zeros((self.n), dtype=int)
            return
        node_order, predecessors = csgraph.breadth_first_order((self.csgraph), i_start=0)
        bfs_tree_csr = csgraph.reconstruct_path(self.csgraph, predecessors)
    ...
    def cyclic_components_indices(self):
        if self.is_aperiodic:
            return [
             np.arange(self.n)]
        return [np.where(self._cyclic_components_proj == k)[0] for k in range(self.period)]
