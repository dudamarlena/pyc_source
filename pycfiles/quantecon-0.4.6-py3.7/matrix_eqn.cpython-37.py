...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/matrix_eqn.py
...
import scipy.linalg as sp_solve_discrete_lyapunov
import scipy.linalg as sp_solve_discrete_are
...
def solve_discrete_riccati(A, B, Q, R, N=None, tolerance=1e-10, max_iter=500, method='doubling'):
    ...
    if method == 'qz':
        ...
        return X
    current_min = np.inf
    candidates = (0.01, 0.1, 0.25, 0.5, 1.0, 2.0, 10.0, 100.0, 1000000.0)
    BB = dot(B.T, B)
    BTA = dot(B.T, A)
...
def solve_discrete_riccati_system(Π, As, Bs, Cs, Qs, Rs, Ns, beta, tolerance=1e-10, max_iter=1000):
    """
        ...
        if iteration > max_iter:
            ...
            for i in range(m):
                ...
                for j in range(m):
                    ...
                    sum2 += Π[(i, j)] * (beta * As[i].T @ Ps[j] @ Bs[i] + Ns[i].T) @ solve(Qs[i] + beta * Bs[i].T @ Ps[j] @ Bs[i], beta * Bs[i].T @ Ps[j] @ As[i] + Ns[i])
