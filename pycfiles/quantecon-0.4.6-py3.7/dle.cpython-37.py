...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/dle.py
# Compiled at: 2018-09-17 00:03:08
# Size of source mod 2**32: 14017 bytes
"""
...
class DLE(object):
    ...
    def __init__(self, information, technology, preferences):
        ...
        a12 = np.vstack((
         self.thetah.dot(phiinc).dot(self.ud), np.zeros((self.nk, self.nz))))
        a11 = np.vstack((np.hstack((self.deltah, a1)),
         np.hstack((
        ...
        self.H = np.hstack((self.llambda, self.pih.dot(uc).dot(phiin).dot(self.gamma),
         self.pih.dot(uc).dot(phiin).dot(self.ud) - self.ub, -self.pih.dot(uc).dot(phiin).dot(self.phii)))
        ...
        lq = LQ((self.Q), (self.R), (self.A), (self.B), (self.C),
          N=(self.W), beta=(self.beta))
        ...
        self.Sk1 = np.hstack((np.zeros((self.nk, self.nh)),
         np.eye(self.nk), np.zeros((self.nk, self.nz))))
        self.Si = -self.F
        ...
        self.Ss = self.llambda.dot(np.hstack((np.eye(self.nh),
         np.zeros((
        ...
        self.Mk = -2 * np.asscalar(self.beta) * np.hstack((np.zeros((self.nk, self.nh)),
         np.eye(self.nk), np.zeros((self.nk, self.nz)))).dot(self.P).dot(self.A0)
        self.Mh = -2 * np.asscalar(self.beta) * np.hstack((np.eye(self.nh),
         np.zeros((
