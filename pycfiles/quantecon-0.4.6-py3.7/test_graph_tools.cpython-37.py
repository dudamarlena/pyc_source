...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/tests/test_graph_tools.py
# Compiled at: 2019-07-07 21:19:40
# Size of source mod 2**32: 10967 bytes
"""
...
class Graphs:
    ...
    def __init__(self):
        ...
        graph_dict = {'A':np.array([[1, 0, 0], [1, 0, 1], [0, 0, 1]]), 
         'strongly_connected_components':[
        ...
        graph_dict = {'A':np.array([[0, 1, 0, 0],
         ...
         'cyclic_components':[
          np.arange(4)]}
        self.strongly_connected_graph_dicts.append(graph_dict)
        graph_dict = {'A':np.array([[0, 0.5], [2, 0]]), 
         'weighted':True, 
         'strongly_connected_components':[
          np.arange(2)], 
         'sink_strongly_connected_components':[
        ...
        graph_dict = {'A':np.array([[1]]), 
         ...
         'cyclic_components':[
          np.array([0])]}
        self.strongly_connected_graph_dicts.append(graph_dict)
        self.graph_dicts = self.strongly_connected_graph_dicts + self.not_strongly_connected_graph_dicts


class TestDiGraph:
    __doc__ = 'Test the methods in Digraph'
    ...
    def test_strongly_connected_components(self):
        for graph_dict in self.graphs.graph_dicts:
            list_of_array_equal(sorted((graph_dict['g'].strongly_connected_components), key=(lambda x: x[0])), sorted((graph_dict['strongly_connected_components']), key=(lambda x: x[0])))

    def test_num_strongly_connected_components(self):
        for graph_dict in self.graphs.graph_dicts:
            eq_(graph_dict['g'].num_strongly_connected_components, len(graph_dict['strongly_connected_components']))

    def test_sink_strongly_connected_components(self):
        for graph_dict in self.graphs.graph_dicts:
            list_of_array_equal(sorted((graph_dict['g'].sink_strongly_connected_components), key=(lambda x: x[0])), sorted((graph_dict['sink_strongly_connected_components']), key=(lambda x: x[0])))

    def test_num_sink_strongly_connected_components(self):
        for graph_dict in self.graphs.graph_dicts:
            eq_(graph_dict['g'].num_sink_strongly_connected_components, len(graph_dict['sink_strongly_connected_components']))

    def test_is_strongly_connected(self):
        for graph_dict in self.graphs.graph_dicts:
            eq_(graph_dict['g'].is_strongly_connected, graph_dict['is_strongly_connected'])
    ...
    def test_is_aperiodic(self):
        for graph_dict in self.graphs.graph_dicts:
            try:
                eq_(graph_dict['g'].is_aperiodic, graph_dict['is_aperiodic'])
            except NotImplementedError:
                eq_(graph_dict['g'].is_strongly_connected, False)
    ...
    def test_cyclic_components(self):
        for graph_dict in self.graphs.graph_dicts:
            try:
                list_of_array_equal(sorted((graph_dict['g'].cyclic_components), key=(lambda x: x[0])), sorted((graph_dict['cyclic_components']), key=(lambda x: x[0])))
            except NotImplementedError:
                eq_(graph_dict['g'].is_strongly_connected, False)
...
def test_subgraph():
    ...
    assert_array_equal(g.subgraph(nodes).csgraph.toarray(), subgraph_adj_matrix)


def test_subgraph_weighted():
    adj_matrix = np.arange(9).reshape(3, 3)
    ...
    for prop0, components_ind in zip(properties, [sccs, sink_sccs]):
        for return_indices in (True, False):
            if return_indices:
                ...
                prop = prop0
            list_of_array_equal(sorted((getattr(g, prop)), key=(lambda x: x[0])), sorted(components, key=(lambda x: x[0])))
