...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/lqnash.py
...
def nnash(A, B1, B2, R1, R2, Q1, Q2, S1, S2, W1, W2, M1, M2, beta=1.0, tol=1e-08, max_iter=1000, random_state=None):
    r"""
    ...
    """
    params = (
     A, B1, B2, R1, R2, Q1, Q2, S1, S2, W1, W2, M1, M2)
    ...
    for it in range(max_iter):
        ...
        F1_right = H1.dot(A) + G1.dot(W1.T) - dot(H1.dot(B2) + G1.dot(M1.T), H2.dot(A) + G2.dot(W2.T))
        ...
        P1 = dot(Lambda1.T, P1.dot(Lambda1)) + Pi1 - dot(dot(Lambda1.T, P1.dot(B1)) + W1 - F2.T.dot(M1), F1)
        P2 = dot(Lambda2.T, P2.dot(Lambda2)) + Pi2 - dot(dot(Lambda2.T, P2.dot(B2)) + W2 - F1.T.dot(M2), F2)
