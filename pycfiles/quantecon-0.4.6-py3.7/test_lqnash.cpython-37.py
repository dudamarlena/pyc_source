...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/tests/test_lqnash.py
...
class TestLQNash(unittest.TestCase):
    ...
    def test_noninteractive(self):
        ...
        a = np.array([[0.95, 0.0], [0, 0.95]])
        b1 = np.array([0.95, 0.0])
        b2 = np.array([0.0, 0.95])
        r1 = np.array([[-0.25, 0.0], [0.0, 0.0]])
        r2 = np.array([[0.0, 0.0], [0.0, -0.25]])
        q1 = np.array([[-0.15]])
        q2 = np.array([[-0.15]])
        f1, f2, p1, p2 = nnash(a, b1, b2, r1, r2, q1, q2, 0, 0, 0, 0, 0, 0, tol=1e-08,
          max_iter=10000)
        alq = a[:1, :1]
        blq = b1[:1].reshape((1, 1))
        rlq = r1[:1, :1]
        qlq = q1
        lq_obj = LQ(qlq, rlq, alq, blq, beta=1.0)
    ...
    def test_nnash(self):
        ...
        f1_ml = np.asarray(np.matrix('           0.243666582208565,   0.027236062661951, -6.827882928738190;\n           0.392370733875639,   0.139696450885998, -37.734107291009138'))
        f2_ml = np.asarray(np.matrix('           0.027236062661951,   0.243666582208565,  -6.827882928738186;\n           0.139696450885998,   0.392370733875639, -37.734107291009131'))
