...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/markov/core.py
# Compiled at: 2019-07-07 21:19:40
# Size of source mod 2**32: 25278 bytes
r"""
...
class MarkovChain:
    ...
    def __init__(self, P, state_values=None):
        ...
        if len(self.P.shape) != 2 or self.P.shape[0] != self.P.shape[1]:
            ...
            if not self.is_sparse:
                data_nonnegative = self.P >= 0
            else:
                data_nonnegative = self.P.data >= 0
    ...
    def state_values(self, values):
        if values is None:
            ...
            if values.ndim < 1 or values.shape[0] != self.n:
                raise ValueError('state_values must be an array_like of length n')
            if np.issubdtype(values.dtype, np.object_):
                raise ValueError('data in state_values must be homogeneous in type')
    ...
    def get_index(self, value):
        ...
        if values.ndim == state_values_ndim:
            k = values.shape[0]
    ...
    def _compute_stationary(self):
        ...
        if self.is_irreducible:
            if not self.is_sparse:
                ...
                stationary_dists = gth_solve((self.P.toarray()), overwrite=True).reshape(1, self.n)
        else:
            ...
            for i, rec_class in enumerate(rec_classes):
                ...
                stationary_dists[(i, rec_class)] = gth_solve(P_rec_class, overwrite=True)
    ...
    def simulate_indices(self, ts_length, init=None, num_reps=None, random_state=None):
        ...
        try:
            ...
            if num_reps is not None:
                ...
                if isinstance(init, numbers.Integral) and not init >= self.n:
                    if init < -self.n:
        ...
        if not self.is_sparse:
            _generate_sample_paths((self.cdfs),
              init_states, random_values, out=X)
        else:
            _generate_sample_paths_sparse((self.cdfs1d),
              (self.P.indices), (self.P.indptr), init_states, random_values,
              out=X)
        if dim == 1:
            return X[0]
        return X
    ...
    def simulate(self, ts_length, init=None, num_reps=None, random_state=None):
        ...
        if init is not None:
            ...
            init_idx = None
        X = self.simulate_indices(ts_length, init=init_idx, num_reps=num_reps, random_state=random_state)
...
@jit(nopython=True)
def _generate_sample_paths_sparse(P_cdfs1d, indices, indptr, init_states, random_values, out):
    """
    For sparse matrix.
    ...
    for i in range(num_reps):
        out[(i, 0)] = init_states[i]
        for t in range(ts_length - 1):
            k = searchsorted(P_cdfs1d[indptr[out[(i, t)]]:indptr[(out[(i, t)] + 1)]], random_values[(i, t)])
