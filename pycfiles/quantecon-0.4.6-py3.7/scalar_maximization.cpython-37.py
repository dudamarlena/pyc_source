...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/optimize/scalar_maximization.py
...
@njit
def brent_max(func, a, b, args=(), xtol=1e-05, maxiter=500):
    """
    ...
    while np.abs(xf - xm) > tol2 - 0.5 * (b - a):
        ...
        if np.abs(e) > tol1:
            ...
            r = e
            e = rat
            if np.abs(p) < np.abs(0.5 * q * r) and p > q * (a - xf) and p < q * (b - xf):
                rat = (p + 0.0) / q
                x = xf + rat
        ...
        if fu <= fnfc or nfc == xf:
            ...
            if fu <= ffulc or fulc == xf or fulc == nfc:
                fulc, ffulc = x, fu
        xm = 0.5 * (a + b)
        tol1 = sqrt_eps * np.abs(xf) + xtol / 3.0
        tol2 = 2.0 * tol1
