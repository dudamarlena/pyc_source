...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/quad.py
# Compiled at: 2018-12-17 00:08:05
# Size of source mod 2**32: 31006 bytes
"""
...
def _qnwnorm1(n):
    ...
    for i in range(m):
        if i == 0:
            z = np.sqrt(2 * n + 1) - 1.85575 * (2 * n + 1) ** (-0.1639344262295082)
        else:
            if i == 1:
                z = z - 1.14 * n ** 0.426 / z
            else:
...
def _qnwbeta1(n, a=1.0, b=1.0):
    ...
    nodes = (1 - nodes) / 2
    weights = weights * math.exp(gammaln(a + n) + gammaln(b + n) - gammaln(n + 1) - gammaln(n + ab + 1))
    weights = weights / (2 * math.exp(gammaln(a + 1) + gammaln(b + 1) - gammaln(ab + 2)))
    return (
     nodes, weights)
    ...
    for i in range(n):
        if i == 0:
            ...
            if i == 1:
                ...
                j = i - 1
                z = z + ((1 + 2.55 * j) / (1.9 * j) + 1.26 * j * a / (1 + 3.5 * j)) * (z - nodes[(j - 1)]) / (1 + 0.3 * a)
        its = 0
        z1 = -10000
