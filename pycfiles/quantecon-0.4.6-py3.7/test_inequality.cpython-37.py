...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/tests/test_inequality.py
# Compiled at: 2018-12-09 20:22:44
...
def test_lorenz_curve():
    ...
    assert_allclose(cum_people, cum_income, rtol=0.001)
    y = np.repeat(0.001, n)
    y[4] = 100000
    pop_cum, income_cum = lorenz_curve(y)
    expected_income_cum = np.repeat(0.0, n + 1)
    expected_income_cum[-1] = 1.0
    assert_allclose(expected_income_cum, income_cum, atol=0.0001)
...
def test_gini_coeff():
    ...
    assert_allclose(expected, coeff, rtol=0.1)
    a = np.random.randint(2, 15)
    expected = 1 - 2 ** (-1 / a)
...
def test_shorrocks_index():
    ...
    assert_allclose(expected, index, rtol=0.01)