...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/estspec.py
...
def smooth(x, window_len=7, window='hanning'):
    ...
    if len(x) < window_len:
        ...
        windows = {'hanning':np.hanning,  'hamming':np.hamming, 
...
def periodogram(x, window=None, window_len=7):
    ...
    w, I_w = w[:int(n / 2) + 1], I_w[:int(n / 2) + 1]
    if window:
...
def ar_periodogram(x, window='hanning', window_len=7):
    ...
    I_w = I_w / np.abs(1 - phi * np.exp(complex(0.0, 1.0) * w)) ** 2
    return (
     w, I_w)