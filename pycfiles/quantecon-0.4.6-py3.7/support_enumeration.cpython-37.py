...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/game_theory/support_enumeration.py
    ...
    for k in range(1, n_min + 1):
        ...
        while supps[0][(-1)] < nums_actions[0]:
            ...
            while supps[1][(-1)] < nums_actions[1]:
                if _indiff_mixed_action(payoff_matrices[0], supps[0], supps[1], A, actions[1]):
                    if _indiff_mixed_action(payoff_matrices[1], supps[1], supps[0], A, actions[0]):
                        out = (np.zeros(nums_actions[0]),
                         np.zeros(nums_actions[1]))
                        for p, (supp, action) in enumerate(zip(supps, actions)):
                            out[p][supp] = action[:-1]
...
def _indiff_mixed_action(payoff_matrix, own_supp, opp_supp, A, out):
    ...
    for i in range(k):
        for j in range(k):
            A[(j, i)] = payoff_matrix[(own_supp[i], opp_supp[j])]
