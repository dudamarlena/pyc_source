...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/util/notebooks.py
        ...
        for fl in files[directory]:
            ...
            if not overwrite:
                if os.path.isfile(fl):
                    if verbose:
                        print('A file named %s already exists in the specified directory ... skipping download.' % fl)
                    status.append(False)
                    continue
                else:
                    if verbose:
                        print('Overwriting file %s ...' % fl)
                if verbose:
                    print('Fetching file: %s' % fl)
