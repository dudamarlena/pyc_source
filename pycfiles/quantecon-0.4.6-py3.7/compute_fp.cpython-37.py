...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/compute_fp.py
# Compiled at: 2019-07-07 21:19:40
# Size of source mod 2**32: 11808 bytes
"""
...
def _print_after_skip(skip, it=None, dist=None, etime=None):
    if it is None:
        msg = '{i:<13}{d:<15}{t:<17}'.format(i='Iteration', d='Distance',
          t='Elapsed (seconds)')
        print(msg)
        print('-' * len(msg))
        return
...
_non_convergence_msg = 'max_iter attained before convergence in compute_fixed_point'
...
def compute_fixed_point(T, v, error_tol=0.001, max_iter=50, verbose=2, print_skip=5, method='iteration', *args, **kwargs):
    ...
    elif verbose not in (0, 1, 2):
        raise ValueError('verbose should be 0, 1 or 2')
    else:
        if method not in ('iteration', 'imitation_game'):
            raise ValueError('invalid method')
        if method == 'imitation_game':
            is_approx_fp = lambda v: _is_approx_fp(T, v, error_tol, *args, **kwargs)
            v_star, converged, iterate = _compute_fixed_point_ig(T, v, max_iter, verbose, print_skip,
...
def _compute_fixed_point_ig(T, v, max_iter, verbose, print_skip, is_approx_fp, *args, **kwargs):
    ...
    shape = (
     buff_size,) + np.asarray(x_new).shape
    ...
    while 1:
        ...
        if not converged:
            ...
            converged, num_iter = _lemke_howson_tbl(tableaux_curr,
              bases_curr, init_pivot=(m - 1), max_iter=max_piv)
    ...
    if verbose == 2:
        ...
        _print_after_skip(print_skip, iterate, error, etime)
    if verbose >= 1:
        converged or warnings.warn(_non_convergence_msg, RuntimeWarning)
    else:
        if verbose == 2:
            print(_convergence_msg.format(iterate=iterate))
