...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/quantecon/game_theory/lemke_howson.py
# Compiled at: 2018-05-14 00:27:06
# Size of source mod 2**32: 19159 bytes
"""
...
def lemke_howson(g, init_pivot=0, max_iter=1000000, capping=None, full_output=False):
    """
    ...
    * If `capping` is set to a positive integer, the routine is executed
    ...
    if N != 2:
        ...
        msg = '`init_pivot` must be an integer k' + 'such that 0 <= k < {0}'.format(total_num)
        ...
        if capping is None:
            capping = max_iter
        tableaux = tuple((np.empty((nums_actions[(1 - i)], total_num + 1)) for i in range(N)))
        bases = tuple((np.empty((nums_actions[(1 - i)]), dtype=int) for i in range(N)))
        converged, num_iter, init_pivot_used = _lemke_howson_capping(payoff_matrices, tableaux, bases, init_pivot, max_iter, capping)
        NE = _get_mixed_actions(tableaux, bases)
        return full_output or NE
    res = NashResult(NE=NE, converged=converged,
      num_iter=num_iter,
      max_iter=max_iter,
...
def _lemke_howson_capping(payoff_matrices, tableaux, bases, init_pivot, max_iter, capping):
    """
    ...
    for k in range(m + n - 1):
        ...
        converged, num_iter = _lemke_howson_tbl(tableaux, bases, init_pivot_curr, capping_curr)
        total_num_iter += num_iter
        if not converged:
            if total_num_iter >= max_iter:
                return (
                 converged, total_num_iter, init_pivot_curr)
            init_pivot_curr += 1
    ...
    _initialize_tableaux(payoff_matrices, tableaux, bases)
    converged, num_iter = _lemke_howson_tbl(tableaux, bases, init_pivot_curr, max_iter_curr)
    total_num_iter += num_iter
    return (
     converged, total_num_iter, init_pivot_curr)
...
def _initialize_tableaux(payoff_matrices, tableaux, bases):
    ...
    * `bases[0]` and `bases[1]` contain basic variable indices, which
    ...
    for pl, (py_start, sl_start) in enumerate(zip((0, nums_actions[0]), (nums_actions[0], 0))):
        for i in range(nums_actions[(1 - pl)]):
            for j in range(nums_actions[pl]):
                tableaux[pl][(i, py_start + j)] = payoff_matrices[(1 - pl)][(i, j)] + consts[(1 - pl)]
...
def _lemke_howson_tbl(tableaux, bases, init_pivot, max_iter):
    ...
    while True:
        for pl in pls:
            row_min = _lex_min_ratio_test(tableaux[pl], pivot, slack_starts[pl], argmins)
...
@jit(nopython=True, cache=True)
def _min_ratio_test_no_tie_breaking(tableau, pivot, test_col, argmins, num_candidates):
    """
...
def _lex_min_ratio_test(tableau, pivot, slack_start, argmins):
    ...
    num_argmins = _min_ratio_test_no_tie_breaking(tableau, pivot, -1, argmins, num_candidates)
    if num_argmins == 1:
        return argmins[0]
    for j in range(slack_start, slack_start + nrows):
        if j == pivot:
            continue
        num_argmins = _min_ratio_test_no_tie_breaking(tableau, pivot, j, argmins, num_argmins)
        if num_argmins == 1:
            break
...
def _get_mixed_actions(tableaux, bases):
    ...
    """
    nums_actions = (
     tableaux[1].shape[0], tableaux[0].shape[0])
    ...
    return (
     out[:nums_actions[0]], out[nums_actions[0]:])