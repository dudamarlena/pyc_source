...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/jetbrains.py
# Compiled at: 2018-01-20 04:19:56
...
class Jetbrains(RepositoryModel):
    ...
    def public_url_list(self):
        return [
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/updatePlugins.xml$', (self.wrap_view('plugin_index')),
           name='plugin_index'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/a/updatePlugins.xml$', (self.wrap_view('plugin_index')), name='plugin_index'),
    ...
    def plugin_index(self, request: HttpRequest, rid, repo_slug, state_slug=None):
        ...
        return TemplateResponse(request, 'repositories/jetbrains/updatePlugins.xml', {'elements': base_query}, content_type='application/xml')

    def index(self, request, rid):
        repo = get_object_or_404((Repository.reader_queryset(request)), id=rid, archive_type=(self.archive_type))
        states = [state for state in ArchiveState.objects.filter(repository=repo).order_by('name')]
        tab_infos = [
         (reverse('repository:jetbrains:plugin_index', kwargs={'rid':repo.id,  'repo_slug':repo.slug}),
          ArchiveState(name=(_('All states')), slug='all-states'), states)]
        tab_infos += [(reverse('repository:jetbrains:plugin_index', kwargs={'rid':repo.id,  'repo_slug':repo.slug,  'state_slug':state.slug}), state, [state]) for state in states]
