...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repository/storages.py
...
class FlatStorage(BaseStorage):
    ...
    def store(self, uid, path):
        ...
        """
        components = [
         self.root] + self.split_uid(uid) + [uid]
    ...
    def store_descriptor(self, uid, filename, fd):
        ...
        """
        components = [
         self.root] + self.split_uid(uid) + [uid, filename]
        abs_path = (os.path.join)(*components)
        makedir(os.path.dirname(abs_path))
