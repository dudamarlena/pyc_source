...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/pypi.py
# Compiled at: 2017-11-02 16:22:00
# Size of source mod 2**32: 17007 bytes
import os.path, tarfile, zipfile
...
class Pypi(Aptitude):
    ...
    def public_url_list(self):
        ...
        """
        pattern_list = [
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/simple/?$', (self.wrap_view('simple')),
           name='simple'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/simple/(?P<search_pattern>[a-z\\d_\\-]*)/?$', (self.wrap_view('simple')),
           name='simple'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/rpc/?$', self.wrap_view('xmlrpc', csrf_exempt=True),
           name='xmlrpc'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/simple/?$', (self.wrap_view('simple')), name='simple'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/simple/(?P<search_pattern>[a-zA-Z\\d_\\-]*)/?$', (self.wrap_view('simple')),
           name='simple'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/rpc/?$', self.wrap_view('xmlrpc', csrf_exempt=True),
           name='xmlrpc'),
         url('^\\d+/[\\w\\-\\._]+/[\\w\\-\\._]+/f/(?P<eid>\\d+)/.*$', (self.wrap_view('get_filename')), name='get_file'),
    ...
    def index(self, request, rid):
        ...
        template_values = {'repo':repo,  'states':states,  'upload_allowed':repo.upload_allowed(request),  'admin_allowed':repo.admin_allowed(request)}
        ...
        for state in states:
            tab_infos.append((
             reverse(view_name, kwargs={'rid':repo.id, 
              'repo_slug':repo.slug,  'state_slug':state.slug}),
             [
              state], state))
    ...
    def xr_search(request, rpc_args, self, rid, repo_slug, state_slug=None):
        ...
        filters = None
        for query_name, attr_name in (('name', 'archive'), ('version', 'version'),
                                      ('home_page', 'official_link'), ('description', 'long_description')):
        ...
        res = [{'name':x.archive,  'version':x.version,  'summary':x.long_description,  '_pypi_ordering':1.0} for x in query]
        return res
    ...
    def xr_user_packages(request, rpc_args, self, rid, repo_slug, state_slug=None):
        if len(rpc_args) == 0:
            ...
            if state_slug:
                state = get_object_or_404(ArchiveState, repository=repo, name=state_slug)
                query = Element.objects.filter(repository=repo, state=state).filter(author__name__icontains=author).order_by('name')
            else:
    ...
    def xr_release_urls(request, rpc_args, self, rid, repo_slug, state_slug=None):
        ...
        for element in query:
            ...
            results.append({'url':package_url,  'packagetype':'sdist',  'filename':element.filename, 
             'size':element.filesize,  'downloads':0,  'comment_text':element.long_description, 
             'md5_dist':element.md5,  'has_sig':False,  'python_version':'source'})
    ...
    def xr_release_data(request, rpc_args, self, rid, repo_slug, state_slug=None):
        ...
        if not query:
            ...
            for k in ('author_email', 'maintainer', 'maintainer_email', 'license',
