...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repository/models.py
# Compiled at: 2018-01-20 04:54:00
# Size of source mod 2**32: 13042 bytes
import logging, os, uuid, functools
...
class BaseModel(models.Model):
    ...
    author = models.ForeignKey((settings.AUTH_USER_MODEL), verbose_name=(_('Author')), db_index=True, blank=True, null=True, on_delete=(models.SET_NULL))
...
class Repository(BaseModel):
    ...
    admin_group = models.ManyToManyField(Group, verbose_name=(_('Admin groups')), db_index=True, blank=True, related_name='repository_admin')
    reader_group = models.ManyToManyField(Group, verbose_name=(_('Readers groups')), db_index=True, blank=True, related_name='repository_reader')
    ...
    def get_absolute_url(self):
        return reverse(('repository:%s:index' % self.archive_type), kwargs={'rid': self.id})
    ...
    def reader_queryset(request):
        ...
        if user.is_anonymous:
            ...
            return Repository.objects.filter(Q(is_private=False) | Q(author=user) | Q(reader_group__in=(user.groups.all())) | Q(admin_group__in=(user.groups.all()))).distinct()
    ...
    def reader_allowed(self, request):
        ...
        if user.is_anonymous:
            ...
            return not self.is_private or self.author == user or not {x.id for x in user.groups.all()}.isdisjoint({x.id for x in self.reader_group.all()}) or not {x.id for x in user.groups.all()}.isdisjoint({x.id for x in self.admin_group.all()})
...
class Element(BaseModel):
    ...
    full_name_normalized = models.CharField((_('Normalized complete name')), max_length=255, blank=True, db_index=True, help_text=(_('Complete name without special chars nor accents')))
    ...
    filename = models.CharField((_('filename')), max_length=255, blank=True, db_index=True, default='', help_text=(_('Automatically generated on creation')))
    uuid = models.CharField((_('UUID')), db_index=True, max_length=40, blank=True, help_text=(_('Unique identifier, automatically generated on first save')))
    md5 = models.CharField((_('MD5 sum')), max_length=120, blank=True, db_index=True, default='', help_text=(_('Automatically generated on creation')))
    sha1 = models.CharField((_('SHA1 sum')), max_length=120, blank=True, db_index=True, default='', help_text=(_('Automatically generated on creation')))
    sha256 = models.CharField((_('SHA256 sum')), max_length=120, blank=True, db_index=True, default='', help_text=(_('Automatically generated on creation')))
    filesize = models.IntegerField((_('File size')), default=0, blank=True, db_index=True, help_text=(_('Automatically generated on creation')))
    extension = models.CharField((_('File extension')), db_index=True, max_length=20, blank=True, default='', help_text=(_('Automatically generated on creation')))
    mimetype = models.CharField((_('MIME type')), db_index=True, max_length=40, blank=True, default='', help_text=(_('Guessed on creation')))
    archive_file = models.FileField((_('Archive file')), blank=True, default='', null=True, upload_to=(settings.UPLOAD_ROOT),
      help_text=(_('Any text, binary or archive file.')))
    ...
    def save(self, *args, **kwargs):
        if not self.uuid:
        ...
        elif self.archive_file is not None and self.archive_file.name:
            ...
            if len(exceptions) == 1:
                raise InvalidRepositoryException(_('Repository %(repo)s is unable to handle this file.') % {'repo': exceptions[0]})
            else:
                if len(exceptions) > 1:
                    raise InvalidRepositoryException(_('Repositories %(repo)s are unable to handle this file.') % {'repo': _(', ').join(exceptions)})
