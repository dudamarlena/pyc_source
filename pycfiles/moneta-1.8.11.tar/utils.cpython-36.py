...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/utils.py
# Compiled at: 2017-07-14 09:22:06
...
import gzip, logging, logging.handlers, os, shutil, stat, tempfile, unicodedata, zlib
...
def import_path(middleware_path):
    ...
    try:
        ...
        raise exceptions.ImproperlyConfigured('Middleware module "%s" does not define "%s"' % (mw_module, mw_fnname))
...
class ZlibFile(object):
    ...
    def __init__(self, fileobj):
        self._ZlibFile__fileobj = fileobj
        self._ZlibFile__decompressor = zlib.decompressobj()
        self._ZlibFile__is_flushed = False

    def read(self, bufsize=10240):
        uncompressed_data = ''
        while not uncompressed_data:
            if not self._ZlibFile__is_flushed:
                compressed_data = self._ZlibFile__fileobj.read(bufsize)
                self._ZlibFile__is_flushed = compressed_data or True
                uncompressed_data = self._ZlibFile__decompressor.flush()
            else:
                uncompressed_data = self._ZlibFile__decompressor.decompress(compressed_data)
...
class BZ2File(object):
    ...
    def __init__(self, fileobj):
        self._BZ2File__fileobj = fileobj
        self._BZ2File__is_finished = False
        self._BZ2File__decompressor = BZ2Decompressor()

    def read(self, bufsize):
        uncompressed_data = ''
        while not uncompressed_data:
            if not self._BZ2File__is_finished:
                compressed_data = self._BZ2File__fileobj.read(bufsize)
                self._BZ2File__is_finished = compressed_data or True
            else:
                uncompressed_data = self._BZ2File__decompressor.decompress(compressed_data)
...
def parse_control_data(control_data, continue_line=' ', split=': ', skip_after_blank=False):
    ...
    for line in control_data.splitlines():
        ...
        if add_to_description:
            description += '\n'
            description += line
        elif not line or line[0:offset] == continue_line:
            if key is not None:
                value += '\n'
                value += line[offset:]
            else:
