...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/vagrant.py
# Compiled at: 2018-01-20 04:21:12
...
class Vagrant(RepositoryModel):
    ...
    def public_url_list(self):
        return [
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]+)/boxes/(?P<eid>\\d+)-(?P<archive>[\\w\\-\\._]+)/versions/(?P<version>[\\w\\-\\._]+)/providers/(?P<provider>[\\w\\-\\._]+)\\.box', (self.wrap_view('get_box')),
           name='get_box'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/(?P<archive>[^/]+)\\.json$', (self.wrap_view('archive_json')),
           name='archive_json'),
         url('^(?P<rid>\\d+)/s(?P<state_slug>[\\w\\-\\._]*)\\.html$', (self.wrap_view('index')), name='index'),
         url('^(?P<rid>\\d+)\\.html$', (self.wrap_view('index')), name='index')]
        ...
        for element in base_query:
            if not element.extra_data:
                ...
                element_infos.append((
                 element.name,
                 reverse('repository:vagrant:get_box', kwargs={'rid':rid, 
                  'repo_slug':repo.slug,  'eid':element.id,  'provider':provider,  'archive':element.archive, 
                  'version':element.version}),
                 element.sha1))

        states = [state for state in ArchiveState.objects.filter(repository=repo).order_by('name')]
        tab_infos = []
        tab_infos += [(reverse('repository:vagrant:index', kwargs={'rid':repo.id,  'state_slug':state.slug}), state, [state], state.slug) for state in states]
        tab_infos += [
         (reverse('repository:vagrant:index', kwargs={'rid':repo.id,  'state_slug':''}),
          ArchiveState(name=(_('All boxes')), slug='all-states'), states, '')]
        template_values = {'repo':repo, 
         'states':states,  'upload_allowed':repo.upload_allowed(request),  'state_slug':state_slug, 
         'elements':element_infos,  'index_url':reverse(moneta_url(repo, 'index'), kwargs={'rid': repo.id}), 
         'tab_infos':tab_infos, 
    ...
    def get_providers_by_version(self, request, rid, state_slug, archive):
        ...
        for element in base_query:
            ...
            versions.setdefault(element.version, []).append({'name':metadata['provider'], 
             'url':'%s%s' % (settings.SERVER_BASE_URL[:-1], element.get_direct_link()), 
             'checksum_type':'sha1', 
             'checksum':element.sha1})

        result = {'name':archive, 
         'description':_('This box contains %(name)s') % {'name': archive},  'versions':[{'version':k,  'status':'active',  'providers':v} for k, v in versions.items()]}
        return result