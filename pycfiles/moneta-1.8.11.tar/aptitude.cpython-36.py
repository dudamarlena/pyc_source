...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/aptitude.py
# Compiled at: 2018-01-20 08:31:56
# Size of source mod 2**32: 24713 bytes
import bz2, gzip, hashlib, io, os.path, tarfile, tempfile
...
class Aptitude(RepositoryModel):
    ...
    def finish_element(self, element, states):
        """
        ...
        """
        Element.states.through.objects.exclude(element__version=(element.version)).filter(archivestate__in=states,
          element__archive=(element.archive)).delete()
    ...
    def url_list(self):
        ...
        """
        pattern_list = [
         url('^(?P<rid>\\d+)/force_index/(?P<repo_slug>[\\w\\-\\._]+)/$', (self.wrap_view('force_index')), name='force_index')]
        return pattern_list
    ...
    def public_url_list(self):
        ...
        pattern_list = [
         url('^(?P<rid>\\d+)/pool/(?P<repo_slug>[\\w\\-\\._]+)/(?P<state_slug>[\\w\\-\\._]+)/(?P<folder>[\\w\\-\\.~_]+)/$', (self.wrap_view('folder_index')),
           name='folder_index'),
         url('^(?P<rid>\\d+)/pool/(?P<repo_slug>[\\w\\-\\._]+)/(?P<state_slug>[\\w\\-\\._]+)/(?P<folder>[\\w\\-\\.~_]+)/(?P<filename>[\\w\\-\\.~_ ]+)$', (self.wrap_view('get_file')),
           name='get_file'),
         url('^(?P<rid>\\d+)/dists/(?P<repo_slug>[\\w\\-\\._]+)/(?P<filename>InRelease|Release|Release.gpg)$', (self.wrap_view('repo_release')),
           name='repo_release'),
         url(('^(?P<rid>\\d+)/dists/(?P<repo_slug>[\\w\\-\\._]+)/Contents-(?P<arch>[\\w]+)%s$' % compression),
           (self.wrap_view('arch_contents')), name='arch_contents'),
         url(('^(?P<rid>\\d+)/dists/(?P<repo_slug>[\\w\\-\\._]+)/(?P<state_slug>[\\w\\-\\._]+)/Contents-(?P<arch>[\\w\\-\\.]+)%s$' % compression),
           (self.wrap_view('state_contents')),
           name='state_contents'),
         url(('^(?P<rid>\\d+)/dists/(?P<repo_slug>[\\w\\-\\._]+)/(?P<state_slug>[\\w\\-\\._]+)/binary-(?P<arch>[\\w\\-\\.]+)/(?P<filename>Packages|Release)%s$' % compression),
           (self.wrap_view('state_files')),
           name='state_files'),
         url('^(?P<rid>\\d+)/(?P<slug2>[\\w\\-\\._]+)/(?P<repo_slug>[\\w\\-\\._]+).asc$', (self.wrap_view('gpg_key')), name='gpg_key'),
         url('^(?P<rid>\\d+)/$', (self.wrap_view('index')), name='index')]
        return pattern_list
    ...
    def extra_index_urls(self, request, repo):
        ...
        result = []
        force_index = (
         reverse(('repository:auth-%s:force_index' % self.archive_type), kwargs={'rid':repo.id, 
          'repo_slug':repo.slug}), _('Index packages'))
    ...
    def state_files(self, request, rid, repo_slug, state_slug, arch, filename, compression):
        filename = 'dists/%(repo)s/%(state)s/binary-%(arch)s/%(filename)s%(comp)s' % {'repo':repo_slug, 
         'state':state_slug,  'arch':arch,  'filename':filename,  'comp':compression}
        return self.index_file(request, rid, filename, 'text/plain')
    ...
    def state_contents(self, request, rid, repo_slug, state_slug, arch, compression):
        filename = 'dists/%(repo)s/%(state)s/Contents-%(arch)s%(comp)s' % {'repo':repo_slug, 
         'arch':arch,  'comp':compression,  'state':state_slug}
        return self.index_file(request, rid, filename, 'text/plain')
    ...
    def folder_index(self, request, rid, repo_slug, state_slug, folder):
        ...
        template_values = {'repo':repo,  'state':state_slug,  'element_count':element_count,  'elements':element_query, 
         'folder':folder,  'upload_allowed':repo.upload_allowed(request)}
    ...
    def index(self, request, rid):
        ...
        template_values = {'repo':repo, 
         'states':states,  'upload_allowed':repo.upload_allowed(request),  'index_url':reverse(moneta_url(repo, 'index'), kwargs={'rid': repo.id}), 
         'tab_infos':tab_infos, 
    ...
    def generate_indexes(self, repository, states=None, validity=365):
        ...
        states = [state for state in states if Element.objects.filter(repository=repository, states=state).count() > 0]
        ...
        for state in states:
            ...
            for element in Element.objects.filter(repository=repository, states=state).order_by('filename'):
                ...
                for architecture in elt_architectures:
                    ...
                    filename = 'dists/%(repo)s/%(state)s/binary-%(architecture)s/Packages' % {'repo':repo_slug, 
                     'state':state.name,  'architecture':architecture}
                    ...
                    package_url = reverse(('repository:%s:get_file' % self.archive_type), kwargs={'rid':repository.id, 
                     'repo_slug':repo_slug,  'filename':element.filename, 
                     'state_slug':state.slug,  'folder':element.filename[0:1]})
            ...
            for architecture in state_architectures:
                filename = 'dists/%(repo)s/%(state)s/binary-%(architecture)s/Release' % {'repo':repo_slug, 
                 'state':state.slug,  'architecture':architecture}
                open_files[filename] = tempfile.TemporaryFile(mode='w+b', dir=(settings.FILE_UPLOAD_TEMP_DIR))
                content = render_to_string('repositories/aptitude/architecture_release.txt', {'architecture':architecture, 
        ...
        for architecture, file_list in complete_file_list.items():
            file_list.sort()
            filename = 'dists/%(repo)s/Contents-%(architecture)s' % {'repo':repo_slug,  'architecture':architecture}
            open_files[filename] = tempfile.TemporaryFile(mode='w+b', dir=(settings.FILE_UPLOAD_TEMP_DIR))
        ...
        now_str = now.strftime('%a, %d %b %Y %H:%M:%S UTC')
        until = (now + datetime.timedelta(validity)).strftime('%a, %d %b %Y %H:%M:%S UTC')
        content = render_to_string('repositories/aptitude/state_release.txt', {'architectures':all_states_architectures, 
         'until':until,  'states':all_states, 
         'repository':repository,  'date':now_str})
