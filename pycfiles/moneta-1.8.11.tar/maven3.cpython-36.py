...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/maven3.py
# Compiled at: 2017-11-02 16:21:31
# Size of source mod 2**32: 12871 bytes
r"""
...
class Maven3(Aptitude):
        ...
        if query_string[-1:] == '/':
            ...
            if sep == '/':
                ...
                if sep == '/':
                    ...
                    if ext not in ('sha1', 'sha256', 'md5'):
                        if sep == '.':
                            no_ext = filename
                    query = Element.objects.filter(Q(archive=group_artifact, version=version, filename=no_ext) | Q(archive=dotted_gav, version=filename) | Q(archive=dotted_gavf) | Q(archive__startswith=(dotted_gavf + '.')))
                else:
                    query = Element.objects.filter(Q(archive=dotted_gav, version=filename) | Q(archive=dotted_gavf) | Q(archive__startswith=(dotted_gavf + '.')))
            else:
    ...
    def browse_repository(self, request: HttpRequest, rid, repo_slug, query_string='', state_slug=None) -> HttpResponse:
        ...
        if isinstance(result, Element):
            ...
            if isinstance(result, set):
                ...
                assert isinstance(result, dict)
                url_list = [(new_query_string + key, self.get_browse_url(repo, (new_query_string + key), state_slug=state_slug), None) for key in result]
            template_values = {'repo':repo, 
             'upload_allowed':repo.upload_allowed(request),  'repo_slug':repo_slug,  'admin':True, 
             'paths':url_list,  'request_path':new_query_string,  'state_slug':state_slug,  'bread_crumbs':bread_crumbs}
    ...
    def get_browse_url(repo, new_query_string, state_slug=None):
        ...
        if state_slug is None:
            kwargs = {'rid':repo.id, 
             'repo_slug':repo.slug,  'query_string':new_query_string}
            return reverse(browse_view_name, kwargs=kwargs)
        else:
            kwargs = {'rid':repo.id, 
             'repo_slug':repo.slug,  'query_string':new_query_string,  'state_slug':state_slug}
            return reverse(browse_view_name, kwargs=kwargs)
    ...
    def public_url_list(self):
        ...
        """
        pattern_list = [
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]+)/browse/(?P<query_string>.*)$', (self.wrap_view('browse_repository')),
           name='browse'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]+)/s/(?P<state_slug>[\\w\\-\\._]+)/browse/(?P<query_string>.*)$', (self.wrap_view('browse_repository')),
           name='browse'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]+)/browse/$', (self.wrap_view('browse_repository')), name='browse'),
         url('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]+)/s/(?P<state_slug>[\\w\\-\\._]+)/browse/$', (self.wrap_view('browse_repository')),
           name='browse'),
