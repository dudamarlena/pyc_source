...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/views.py
# Compiled at: 2018-01-20 04:59:14
# Size of source mod 2**32: 25540 bytes
import base64, hashlib, mimetypes, os, tarfile, tempfile, time, zipfile
...
def index(request: HttpRequest):
    ...
    if not user.has_perm('repository.add_repository'):
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                repo = Repository(author=author, name=(form.cleaned_data['name']), on_index=(form.cleaned_data['on_index']), archive_type=(form.cleaned_data['archive_type']),
    ...
    template_values = {'repositories':repositories,  'form':form,  'request':request,  'upload_ids':upload_ids, 
     'admin_ids':admin_ids}
...
def check(request: HttpRequest):
    ...
    user = request.user
    template_values = {'media_root':settings.MEDIA_ROOT, 
     'media_url':settings.MEDIA_URL,  'static_root':settings.STATIC_ROOT, 
     'static_url':settings.STATIC_URL,  'debug':settings.DEBUG, 
     'use_x_forwared_host':settings.USE_X_FORWARDED_HOST,  'secure_proxy_ssl_header_name':s_h[0][5:], 
...
def modify_repository(request: HttpRequest, rid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for name in new_state_names - old_state_names:
        ...
        form = RepositoryUpdateForm(initial={'on_index':repo.on_index,  'is_private':repo.is_private,  'reader_group':list(repo.reader_group.all()), 
         'states':' '.join([x.name for x in repo.archivestate_set.all()]), 
         'admin_group':list(repo.admin_group.all())})
    template_values = {'form':form, 
     'repo':repo,  'upload_allowed':repo.upload_allowed(request)}
...
def search_package(request: HttpRequest, rid):
    ...
    class ElementSearchForm(forms.Form):
        ...
        states = forms.ModelMultipleChoiceField(label=(_('Selected states')), required=False, initial=repo_states,
    ...
    template_values = {'elements':elements, 
     'repo':repo,  'pattern':search_pattern,  'form':form,  'upload_allowed':repo.upload_allowed(request)}
...
def add_element(request: HttpRequest, rid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                element = generic_add_element(request, repo, form.cleaned_data['package'], [x.name for x in form.cleaned_data['states']])
    ...
    try:
        ...
        if request.method != 'POST':
            ...
            class ElementForm(forms.Form):
                ...
                states = forms.MultipleChoiceField(choices=[(x.name, x.name) for x in repo.archivestate_set.all()], label=(_('States')))
            ...
            try:
                try:
                    element = generic_add_element(request, repo, uploaded_file, (form.cleaned_data['states']), name=(form.cleaned_data.get('name')),
...
def show_file(request: HttpRequest, eid):
    ...
    if len(elements) == 0:
        raise Http404
    element = elements[0]
    template_values = {'element':element,  'repo':element.repository,  'upload_allowed':element.repository.upload_allowed(request)}
...
def compare_states(request: HttpRequest, rid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            template_values.update({'only_left':only_left,  'only_right':only_right,  'invalid_values':invalid_values,  'state_left':state_left, 
