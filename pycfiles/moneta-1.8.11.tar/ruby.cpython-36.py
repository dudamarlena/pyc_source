...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/ruby.py
# Compiled at: 2017-11-02 16:19:04
# Size of source mod 2**32: 16841 bytes
import base64, gzip, io, json, subprocess, tarfile, tempfile, zlib
...
class RubyGem(Aptitude):
    ...
    def update_element(self, element):
        ...
        p = subprocess.Popen(['ruby', '-e', 'puts Marshal.dump (Gem::Specification.from_yaml(ARGF.read))'], stdin=(subprocess.PIPE),
    ...
    def public_url_list(self):
        ...
        """
        src_pattern_list = [
         ('(?P<filename>(specs\\.4\\.8|prerelease_specs\\.4\\.8|latest_specs\\.4\\.8|Marshal\\.4\\.8|versions\\.list|names\\.list)(\\.gz)?)',
 'specs', 'specs'),
         ('gems/(?P<filename>.+)', 'download', 'download'),
         ('specs/(?P<filename>.+)\\.gemspec', 'gem_specs', 'gem_specs'),
         ('quick/Marshal\\.4\\.8/(?P<filename>.+)\\.gemspec(?P<compression>(\\.rz|))', 'quick_gem_specs',
 'quick_gem_specs'),
         ('', 'index', 'index')]
        pattern_list = []
        for pattern, view, name in src_pattern_list:
            pattern_list.append(url(('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/a/%s$' % pattern), (self.wrap_view(view)), name=name))
            pattern_list.append(url(('^(?P<rid>\\d+)/(?P<repo_slug>[\\w\\-\\._]*)/s/(?P<state_slug>[\\w\\-\\._]+)/%s$' % pattern), (self.wrap_view(view)),
              name=name))
    ...
    def generate_indexes(self, repo: Repository, states=None, validity=365):
        ...
        for state_info in state_infos:
            ...
            self._RubyGem__write_file(repo, '%s/specs.4.8' % folder_name, lambda x: write(x, all_elements))
            self._RubyGem__write_file(repo, '%s/latest_specs.4.8' % folder_name, lambda x: write(x, last_elements_by_state))
            self._RubyGem__write_file(repo, '%s/prerelease_specs.4.8' % folder_name, lambda x: write(x, []))
            self._RubyGem__write_file(repo, '%s/Marshal.4.8' % folder_name, lambda x: write(x, all_specs))
            self._RubyGem__write_file(repo, '%s/versions.list' % folder_name, versions_write_fn)
            self._RubyGem__write_file(repo, '%s/names.list' % folder_name, names_write_fn)
