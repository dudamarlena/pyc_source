...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repository/forms.py
...
def get_repository_form():
    ...
    class RepositoryForm(forms.Form):
        ...
        states = forms.CharField(label=(_('Possible states for archives')), initial='qualif prod', help_text=(_('Please separate values by spaces')),
          validators=[
         RegexValidator('\\w+(\\s\\w)*')])
        admin_group = forms.ModelMultipleChoiceField((Group.objects.all()), label=(_('Groups allowed to upload packages')), required=False)
        reader_group = forms.ModelMultipleChoiceField((Group.objects.all()), widget=(forms.HiddenInput()), label=(_('Groups allowed to download packages')),
          help_text=(_('Only if downloads require authentication')),
          required=False)
...
class DeleteRepositoryForm(forms.Form):
    confirm = forms.CharField(max_length=80, label=(_('Please enter “%(confirm)s”') % {'confirm': CONFIRM_PHRASE}), validators=[
     confirm_phrase])
...
class RepositoryUpdateForm(forms.Form):
    ...
    states = forms.CharField(label=(_('Possible states')), initial='dev qualif prod', help_text=(_('Please separate values by spaces')),
      validators=[
     RegexValidator('\\w+(\\s\\w)*')])
    admin_group = forms.ModelMultipleChoiceField((Group.objects.all()), label=(_('Groups allowed to upload')), required=False)


class SignatureForm(forms.Form):
    sha256 = forms.CharField(label=(_('SHA256')), max_length=255, validators=[RegexValidator('\\w{64}')])
