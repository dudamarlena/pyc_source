...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/Moneta/moneta/repositories/rpm.py
...
import hashlib, re, stat, struct, sys
if sys.version < '3':
    try:
    ...
    def _match_composite(self):
        try:
            for idx, name in enumerate(self.header[1117]):
                ...
                self.filelist.append(RPMFile(name=(dirname + name),
        ...
        try:
            if self.header[1047]:
                for name, flags, version in zip(self.header[1047], self.header[1112], self.header[1113]):
                    self.provides.append(RPMprco(name=name, flags=flags, str_flags=(self.RPM_PRCO_FLAGS_MAP[(flags & 15)]), version=(self._stringToVersion(version))))
        ...
        try:
            if self.header[1049]:
                for name, flags, version in zip(self.header[1049], self.header[1048], self.header[1050]):
                    self.requires.append(RPMprco(name=name, flags=flags, str_flags=(self.RPM_PRCO_FLAGS_MAP[(flags & 15)]), version=(self._stringToVersion(version))))
        ...
        try:
            if self.header[1090]:
                for name, flags, version in zip(self.header[1090], self.header[1114], self.header[1115]):
                    self.obsoletes.append(RPMprco(name=name, flags=flags, str_flags=(self.RPM_PRCO_FLAGS_MAP[(flags & 15)]), version=(self._stringToVersion(version))))
        ...
        try:
            if self.header[1054]:
                for name, flags, version in zip(self.header[1054], self.header[1053], self.header[1055]):
                    self.conflicts.append(RPMprco(name=name, flags=flags, str_flags=(self.RPM_PRCO_FLAGS_MAP[(flags & 15)]), version=(self._stringToVersion(version))))
