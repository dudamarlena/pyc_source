...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.8.2/x64/lib/python3.8/site-packages/musicbot/commands/folder.py
# Compiled at: 2020-04-15 22:39:47
# Size of source mod 2**32: 4934 bytes
import logging, os, json, click
...
def tracks(folders, output):
    ...
    if output == 'json':
        tracks_dict = [{'title':t.title, 
         'artist':t.artist,  'album':t.album} for t in tracks]
...
checks = [
 'no-title', 'bad-comment', 'invalid-artist', 'no-genre', 'no-album', 'no-artist', 'no-rating', 'invalid-track-number']
