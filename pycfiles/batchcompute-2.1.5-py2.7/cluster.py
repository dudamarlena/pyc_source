...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/batchcompute/resources/cluster.py
# Compiled at: 2019-12-18 08:07:41
import copy
...
class Classic(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'AllowSecurityGroup': list, 
...
class GroupDescription(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'DesiredVMCount': NUMBER, 
       'InstanceType': STRING, 
       'ResourceType': STRING, 
       'SpotStrategy': STRING, 
...
class VPC(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'CidrBlock': STRING, 
       'VpcId': STRING, 
...
class NAS(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'AccessGroup': list, 
       'FileSystem': list}
...
class OSS(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'AccessKeyId': STRING, 
...
class Configs(Jsonizable):
    ...
    def add_system_disk(self, size, type_='cloud'):
        assert isinstance(size, NUMBER) and isinstance(type_, STRING), 'size must be number and type_ must be string'
        disk = {'Type': type_, 
           'Size': size}
        if 'Disks' in self._d:
    ...
    def add_data_disk(self, size, type_='cloud', mount_point=''):
        assert isinstance(size, NUMBER) and isinstance(type_, STRING) and isinstance(mount_point, STRING), 'size must be number and type_ must be string'
        disk = {'Type': type_, 
           'Size': size, 
    ...
    def set_networks_type(self, classicnetworks=False):
        defaultVpc = {'CidrBlock': '192.168.0.0/16'}
        if classicnetworks:
...
class ClusterDescription(Jsonizable):
    ...
    def _validate_group(self, group):
        if group.__class__ == GroupDescription:
            return copy.deepcopy(group)
        return GroupDescription(group)
...
class ModifyClusterDescription(Jsonizable):
    ...
    def _validate_group(self, group):
        if group.__class__ == ModifyGroupDescription:
            return copy.deepcopy(group)
        return ModifyGroupDescription(group)
