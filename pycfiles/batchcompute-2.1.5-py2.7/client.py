...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/batchcompute/client/client.py
# Compiled at: 2019-12-02 03:05:33
"""Implementation of batchcompute service Client."""
import time, datetime
from functools import wraps
from .response import Job, Task, Image, Instance, Image, Cluster, ClusterInstance, RawResponse, ListResponse, GetResponse, ActionResponse, CreateResponse, Quota, App, AppRevision, SynchronousInstance, AvailableResource
from batchcompute.core import Api
from batchcompute.core.exceptions import ClientError, FieldError
from batchcompute.resources import JobDescription, ImageDescription, ClusterDescription, ModifyClusterDescription, AppDescription, AppJobDescription
from batchcompute.utils import CamelCasedClass, remap, add_metaclass, get_logger
from batchcompute.utils.functions import import_json, add_region, partial, set_api_version, get_api_version, get_all_region
...
class CheckedApi(object):
    ...
    def __init__(self, region, access_key_id, access_key_secret, security_token, security_conn, human_readable=False, **headers):
        self._c = Api(region, access_key_id, access_key_secret, security_token, security_conn, **headers)
...
class Client(object):
    ...
    def __init__(self, region, access_key_id, access_key_secret, security_token='', security_conn=False, human_readable=False, **headers):
        """
        ...
        """
        self._h = human_readable
        self._sa = CheckedApi(region, access_key_id, access_key_secret, security_token, security_conn, self._h, **headers)
    ...
    def _do_create(self, resource_type, resource_desc, idempotent_token, params={}, sync=False):
        ...
        if idempotent_token and isinstance(idempotent_token, STRING):
            p = {'IdempotentToken': idempotent_token}
...
'.join(errs))
    ...
    def get_cluster_instance(self, cluster_id, group_name, instance_id):
        ...
        """
        instance_info = [
         'groups', group_name, 'instances', instance_id]
    ...
    def recreate_cluster_instance(self, cluster_id, group_name, instance_id):
        ...
        """
        instance_info = [
         'groups', group_name, 'instances', instance_id, 'recreate']
    ...
    def list_cluster_instances(self, cluster_id, group_name, marker, max_item_count):
        ...
        """
        cluster_info = [
         cluster_id, 'groups', group_name, 'instances']
        p = {'Marker': marker, 
           'MaxItemCount': str(max_item_count)}
