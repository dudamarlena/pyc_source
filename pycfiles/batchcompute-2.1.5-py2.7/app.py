...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/batchcompute/resources/app.py
...
class Parameter(Jsonizable):
    ...
    descriptor_map = {'Description': STRING, 
       'Type': STRING, 
       'Default': ANY, 
       'LocalPath': STRING}
    required = []
    available_types = {'String': STRING + (type(None),), 
...
class VM(Jsonizable):
    """VM"""
    descriptor_type = 'data descriptor'
    descriptor_map = {'ECSImageId': STRING}
    required = [
     'ECSImageId']
...
class Config(Jsonizable):
    ...
    descriptor_map = {'ResourceType': (
       ...
       'DiskType': (
                  dict, DiskType), 
       'MinDataDiskSize': (
                         dict, MinDataDiskSize), 
       'DataDiskType': (
                      dict, DataDiskType), 
       'DataDiskMountPoint': (
                            dict, DataDiskMountPoint), 
       'MaxRetryCount': (
                       dict, MaxRetryCount), 
       'Timeout': (
                 dict, Timeout)}
...
class AppDescription(Jsonizable):
    ...
    def _validate_input_parameter(self, params):
        if params.__class__ == InputParameter:
            return copy.deepcopy(params)
        return InputParameter(params)
    ...
    def _validate_output_parameter(self, params):
        if params.__class__ == OutputParameter:
            return copy.deepcopy(params)
        return OutputParameter(params)
