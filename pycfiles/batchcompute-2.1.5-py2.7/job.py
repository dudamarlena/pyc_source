...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/batchcompute/resources/job.py
# Compiled at: 2019-12-18 08:05:43
import copy
...
class AutoCluster(Jsonizable):
    ...
    descriptor_map = {'InstanceType': STRING, 
       'ResourceType': STRING, 
       'SpotStrategy': STRING, 
...
class Command(Jsonizable):
    ...
    """
    descriptor_type = 'data descriptor'
    descriptor_map = {'CommandLine': STRING, 
       'PackagePath': STRING, 
       'EnvVars': dict, 
       'Docker': (
                Docker, dict)}
...
class DAG(Jsonizable):
    ...
    def _validate_task(self, task):
        if task.__class__ == TaskDescription:
...
class Config(Jsonizable):
    """Config for Job"""
    descriptor_type = 'data descriptor'
    descriptor_map = {'ResourceType': STRING, 
       'InstanceType': STRING, 
       'InstanceCount': NUMBER, 
       'MinDiskSize': NUMBER, 
       'DiskType': STRING, 
...
class Logging(Jsonizable):
    """Logging for Job"""
    descriptor_type = 'data descriptor'
    descriptor_map = {'StdoutPath': STRING, 
       'StderrPath': STRING}
    required = []
