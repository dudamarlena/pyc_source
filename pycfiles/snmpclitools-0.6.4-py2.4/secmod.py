...
class _SMGenerator(base.GeneratorTemplate):
    ...
    def n_MasterAuthKey(self, cbCtx, node):
        ...
        if len(node) > 2:
            ...
            p = node[1].attr
        if len(p) < 8:
            raise error.PySnmpError('Short master authentication key (8+ chars required)')
        ctx['masterAuthKey'] = p
    ...
    def n_LocalizedAuthKey(self, cbCtx, node):
        ...
        if len(node) > 2:
            ...
            p = node[1].attr
        if len(p) < 8:
            raise error.PySnmpError('Short localized authentication key (8+ chars required)')
    ...
    def n_MasterPrivKey(self, cbCtx, node):
        ...
        if len(node) > 2:
            ...
            p = node[1].attr
        if len(p) < 8:
            raise error.PySnmpError('Short master privacy key (8+ chars required)')
        ctx['masterPrivKey'] = p
    ...
    def n_LocalizedPrivKey(self, cbCtx, node):
        ...
        if len(node) > 2:
            ...
            p = node[1].attr
        if len(p) < 8:
            raise error.PySnmpError('Short localized privacy key (8+ chars required)')
...
def generator(cbCtx, ast):
    ...
    if ctx['versionId'] == 3:
        ...
        if authKeyType == config.usmKeyTypeLocalized or privKeyType == config.usmKeyTypeLocalized:
        ...
        if 'privKey' in ctx:
            ...
            ctx['privKey'] = None
        config.addV3User(snmpEngine, ctx['securityName'], ctx['authProtocol'], ctx['authKey'], ctx['privProtocol'], ctx['privKey'], securityEngineId=securityEngineId, securityName=ctx['securityName'], authKeyType=authKeyType, privKeyType=privKeyType)
        mibBuilder = snmpEngine.msgAndPduDsp.mibInstrumController.mibBuilder
        if 'engineBoots' in ctx:
            (snmpEngineBoots,) = mibBuilder.importSymbols('__SNMP-FRAMEWORK-MIB', 'snmpEngineBoots')
            snmpEngineBoots.setSyntax(snmpEngineBoots.getSyntax().clone(ctx['engineBoots']))
        if 'engineTime' in ctx:
            (snmpEngineTime,) = mibBuilder.importSymbols('__SNMP-FRAMEWORK-MIB', 'snmpEngineTime')
