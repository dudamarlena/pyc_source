...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpclitools/cli/target.py
...
class _TargetGeneratorPassOne(base.GeneratorTemplate):
    __module__ = __name__
    DEFAULT_PORT = '161'
    SNMP_DOMAIN_MAP = {'udp': (udp.snmpUDPDomain, udp.UdpSocketTransport,
             lambda h, p: (
    ...
    SNMP_DOMAIN_NAME_MAP = {2: 'udp', 10: 'udp6'}

    def n_Transport(self, cbCtx, node):
        (snmpEngine, ctx) = cbCtx
        if node[0].attr in self.SNMP_DOMAIN_MAP:
            (ctx['transportDomain'], ctx['transportModule'], ctx['addrRewriteFun']) = self.SNMP_DOMAIN_MAP[node[0].attr]
        else:
    ...
    def n_IPv6(self, cbCtx, node):
        ...
        if not len(node):
            if 'transportDomain' not in ctx:
                (ctx['transportDomain'], ctx['transportModule'], ctx['addrRewriteFun']) = self.SNMP_DOMAIN_MAP['udp6']
            return
    ...
    def n_Agent_exit(self, cbCtx, node):
        ...
        if 'transportDomain' not in ctx:
            try:
                ...
                (ctx['transportDomain'], ctx['transportModule'], ctx['addrRewriteFun']) = self.SNMP_DOMAIN_MAP[self.SNMP_DOMAIN_NAME_MAP.get(f, 'udp')]
        if 'transportFormat' in ctx:
            ...
            ctx['transportAddress'] = (
             ctx['transportAddress'], self.DEFAULT_PORT)
...
def generator(cbCtx, ast):
    (snmpEngine, ctx) = cbCtx
    _TargetGeneratorPassTwo().preorder(_TargetGeneratorPassOne().preorder((snmpEngine, ctx), ast), ast)


def generatorTrap(cbCtx, ast):
    (snmpEngine, ctx) = cbCtx
    _TargetGeneratorTrapPassTwo().preorder(_TargetGeneratorTrapPassOne().preorder((snmpEngine, ctx), ast), ast)