...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notetool/tool/pathtool.py
# Compiled at: 2020-03-15 22:54:27
# Size of source mod 2**32: 6885 bytes
import os
from notetool.tool import log
logger = log(__name__)
__all__ = [
 'exists', 'exist_and_create', 'exists_file', 'exists_dir', 'meta', 'list_file', 'LocalPath',
...
def path_parse(path):
    if path is None:
        ...
        return path.startswith('/') or os.path.join(os.getcwd(), path)
    return path
        ...
        if file_path is not None:
            ...
            if file_dir is not None:
                if file_name is not None:
                    ...
                    return False
            elif os.path.exists(file_dir) and os.path.isdir(file_dir):
