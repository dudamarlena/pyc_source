...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/notetool/download/story.py
# Compiled at: 2020-03-15 22:43:51
# Size of source mod 2**32: 12164 bytes
import os, queue, sqlite3, sys, threading, urllib.parse
...
class m3u8Dataset:
    ...
    def test(self):
        res = self.execute("insert into {} (movie_url, movie_name,is_download) values ('url','name', 0)".format(self.table_name))
        return res
    ...
    def exists(self, url='', name='', param1='', param2='', param3=''):
        conf = self.condition(url=url, name=name, param1=param1, param2=param2, param3=param3)
        res = self.execute('select * from  {} where {}'.format(self.table_name, conf))
        for _ in res:
            return True
    ...
    def select(self, size=50):
        res = self.execute('select * from  {} where is_download=0 limit {} \n            '.format(self.table_name, size))
        urls = []
        for line in res:
    ...
    def get_real_url(self, m3u8_url):
        ...
        if r.ok:
            ...
            if body:
                ...
                for n in body_list:
                    if n:
                        ts_url = n.startswith('#') or urllib.parse.urljoin(m3u8_url, n.strip())
            ...
            if body:
                ...
                for line in body_list:
                    if '#EXT-X-KEY' in line:
                        ...
                        key_url = file.url.rsplit('/', 1)[0] + '/' + key_path
                        res = requests.get(key_url, headers=headers)
                        file.key = res.content
                        print('key：', file.key)
                    if line:
                        line.startswith('#') or file.ts_list.append(urllib.parse.urljoin(file.url, line.strip()))
