...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/core/plots/interactive.py
...
def write_to_html(fig, filename):
    ...
    data += "<script src='https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.1/MathJax.js?config=TeX-MML-AM_SVG'></script>"
    with open(filename, 'w') as (f):
        data += div
        f.write(data)


def histogram2d(x, y, xlabel='x', ylabel='y', contour=False, contour_color='Blues', marker_color='rgba(248,148,6,1)', dimensions={'width':900,  'height':900}, write_to_html_file='interactive_2d_histogram.html', showgrid=False, showlegend=False):
...
def ridgeline(data, labels, xlabel='x', palette='colorblind', colors=None, width=3, write_to_html_file='interactive_ridgeline.html', showlegend=False, dimensions={'width':1100, 
 'height':700}):
...
def corner(data, labels, dimensions={'width':900, 
 'height':900}, show_diagonal=False, colors={'selected':'rgba(248,148,6,1)', 
 'not_selected':'rgba(0,0,0,1)'}, show_upper_half=False, write_to_html_file='interactive_corner.html'):
    ...
    write_to_html_file: str
        Name of the html file you wish to write the figure to
    """
    data_structure = [dict(label=label, values=value) for label, value in zip(labels, data)]
    fig = go.Figure(data=go.Splom(dimensions=data_structure,
      marker=dict(color=(colors['not_selected']),
      showscale=False,
      line_color='white',
      line_width=0.5,
      size=3),
