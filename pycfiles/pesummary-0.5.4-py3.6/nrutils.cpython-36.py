...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/file/nrutils.py
# Compiled at: 2020-04-21 06:57:35
# Size of source mod 2**32: 30978 bytes
import numpy as np
from pesummary.utils.utils import logger
from lalinference.imrtgr import nrutils
import lalsimulation
from lalsimulation import SimInspiralGetSpinFreqFromApproximant, SIM_INSPIRAL_SPINS_CASEBYCASE, SIM_INSPIRAL_SPINS_FLOW
try:
...
NRSUR_FITS = [
 'final_mass', 'final_spin', 'final_kick']
...
def _bbh_final_spin_precessing_using_non_precessing_fit(mass_1, mass_2, spin_1z, spin_2z, fit):
    ...
    """
    return nrutils.bbh_final_spin_precessing(mass_1, mass_2, np.abs(spin_1z), np.abs(spin_2z), 0.5 * np.pi * (1 - np.sign(spin_1z)), 0.5 * np.pi * (1 - np.sign(spin_2z)), np.zeros_like(mass_1), fit)
...
def _bbh_final_spin_precessing_projected(mass_1, mass_2, a_1, a_2, tilt_1, tilt_2, phi_12, function=None):
    ...
    a_perp_squared = a_1perp * a_1perp + a_2perp * a_2perp + 2.0 * a_1perp * a_2perp * np.cos(phi_12)
    return (final_spin_aligned_squared + a_perp_squared / total_mass ** 4.0) ** 0.5
...
def bbh_final_spin_precessing_projected_Healyetal(*args):
    ...
    phi_12: float/np.ndarray
        ...
        components
    """
    return _bbh_final_spin_precessing_projected(*args, **{'function': bbh_final_spin_precessing_Healyetal})
...
def bbh_final_spin_precessing_projected_UIB2016(*args):
    ...
    phi_12: float/np.ndarray
        ...
        components
    """
    return _bbh_final_spin_precessing_projected(*args, **{'function': bbh_final_spin_precessing_UIB2016})
...
def _bbh_average_quantity(*args, fits=None, cls=None, quantity=None, return_fits_used=False):
    ...
    *args: tuple
    ...
    logger.info('Averaging the {} from the following fits: {}'.format(quantity, ', '.join(used_fits)))
    if return_fits_used:
...
def bbh_final_mass_average(*args, fits=FINALMASS_FITS, return_fits_used=False):
    ...
    return_fits_used: Bool, optional
        if True, return the fits that were used to calculate the average
    """
    return _bbh_average_quantity(*args, fits=fits, cls=FinalMassFits, quantity='final mass', return_fits_used=return_fits_used)


def bbh_final_spin_average_non_precessing(*args, fits=FINALSPIN_FITS, return_fits_used=False):
    ...
    return_fits_used: Bool, optional
        if True, return the fits that were used to calculate the average
    """
    return _bbh_average_quantity(*args, fits=fits, cls=FinalSpinFits, quantity='final spin', return_fits_used=return_fits_used)


def bbh_final_spin_average_precessing(*args, fits=FINALSPIN_FITS, return_fits_used=False):
    ...
    return_fits_used: Bool, optional
        if True, return the fits that were used to calculate the average
    """
    return _bbh_average_quantity(*args, fits=fits, cls=FinalSpinPrecessingFits, quantity='final spin', return_fits_used=return_fits_used)
...
def bbh_peak_luminosity_average(*args, fits=LPEAK_FITS, return_fits_used=False):
    ...
    return_fits_used: Bool, optional
        if True, return the fits that were used to calculate the average
    """
    return _bbh_average_quantity(*args, fits=fits, cls=PeakLuminosityFits, quantity='peak luminosity', return_fits_used=return_fits_used)
...
def NRSur_fit(mass_1, mass_2, a_1, a_2, tilt_1, tilt_2, phi_12, phi_jl, theta_jn, phi_ref, f_low=20.0, f_ref=20.0, model=NRSUR_MODEL, fits=NRSUR_FITS, return_fits_used=False, approximant=None, **kwargs):
    ...
    from pesummary.gw.file.conversions import component_spins, magnitude_from_vector
    from pesummary.utils.utils import iterator
    import copy
    if not NRSUR_MODULE:
        raise ImportError('Unable to import `lalsimulation.nrfits.eval_fits`. This is likely due to the installed version of lalsimulation. Please update.')
    fits_map = {'final_mass':'FinalMass', 
     'final_spin':'FinalSpin',  'final_kick':'RecoilKick'}
    ...
    spins = np.array(component_spins(theta_jn, phi_jl, tilt_1, tilt_2, phi_12, a_1, a_2, mass_1, mass_2, f_ref, phi_ref))
    a_1_vec = np.array([spins.T[1], spins.T[2], spins.T[3]]).T
    a_2_vec = np.array([spins.T[4], spins.T[5], spins.T[6]]).T
    mass_1 *= MSUN_SI
    mass_2 *= MSUN_SI
    _fits = [eval_nrfit((mass_1[num]), (mass_2[num]), (a_1_vec[num]), (a_2_vec[num]), model, converted_fits, f_low=f_low, f_ref=(f_ref[num]), approximant=approximant, extra_params_dict=kwargs) for num in iterator((range(len(mass_1))),
      desc=description, tqdm=True, total=(len(mass_1)))]
    nr_fits = {key:np.array([dic[key] for dic in _fits]) for key in _fits[0]}
    if fits_map['final_mass'] in nr_fits.keys():
        nr_fits[fits_map['final_mass']] = np.array([final_mass[0] for final_mass in nr_fits[fits_map['final_mass']]]) / MSUN_SI
    if fits_map['final_kick'] in nr_fits.keys():
        nr_fits[fits_map['final_kick']] *= C_SI / 1000
        final_kick_abs = magnitude_from_vector(nr_fits[fits_map['final_kick']])
        nr_fits[fits_map['final_kick']] = final_kick_abs
    if fits_map['final_spin'] in nr_fits.keys():
        final_spin_abs = magnitude_from_vector(nr_fits[fits_map['final_spin']])
        nr_fits[fits_map['final_spin']] = final_spin_abs
    return {(key if key not in inverse_fits_map.keys() else inverse_fits_map[key]):item for key, item in nr_fits.items()}