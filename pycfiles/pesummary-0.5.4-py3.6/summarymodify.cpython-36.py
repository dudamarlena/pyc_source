...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summarymodify.py
# Compiled at: 2020-05-08 05:31:08
# Size of source mod 2**32: 16245 bytes
import os, numpy as np, math, argparse, json, h5py
...
class _Input(_GWInput):
    __doc__ = 'Super class to handle the command line arguments\n    '
    ...
    def labels(self, labels):
        ...
        if labels is not None:
            if isinstance(labels, dict):
                self._labels = labels
        if labels is not None:
            raise InputError('Please provide an existing labels and the label you wish to replace it with `--labels existing:new`.')
    ...
    def kwargs(self, kwargs):
        ...
        if kwargs is not None:
            if isinstance(kwargs, dict):
                self._kwargs = kwargs
        if kwargs is not None:
            raise InputError("Please provide the label, kwarg and value with '--kwargs label:kwarg:value`")
    ...
    def replace_posterior(self, replace_posterior):
        ...
        if replace_posterior is not None:
            if isinstance(replace_posterior, dict):
                self._replace_posterior = replace_posterior
        if replace_posterior is not None:
            raise InputError("Please provide the label, posterior and file path with value with '--replace_posterior label;posterior:/path/to/posterior.dat where ';' is the chosen delimiter and provided with '--delimiter ;`")
    ...
    def remove_posterior(self, remove_posterior):
        ...
        if remove_posterior is not None:
            if isinstance(remove_posterior, dict):
                self._remove_posterior = remove_posterior
        if remove_posterior is not None:
            raise InputError("Please provide the label and posterior with '--remove_posterior label:posterior`")
    ...
    def samples(self, samples):
        if samples is None:
            raise InputError('Please provide a result file that you wish to modify')
        else:
            if len(samples) > 1:
                raise InputError('Only a single result file can be passed')
            samples = samples[0]
            raise self.is_pesummary_metafile(samples) or InputError('Please provide a PESummary metafile to this executable')
        ...
        if extension == '.h5' or extension == '.hdf5':
            ...
            with h5py.File(self.samples, 'r') as (f):
                ...
                for label in data.keys():
                    try:
                        data[label]['posterior_samples'] = DataFrame(data[label]['posterior_samples']).to_records(index=False,
                          column_dtypes=(np.float))
                    except KeyError:
                        pass
                    except Exception:
                        ...
                        samples = np.array([j for j in data[label]['posterior_samples']['samples']].copy())
                        data[label]['posterior_samples'] = DataFrame.from_dict({param:samples.T[num] for num, param in enumerate(parameters)}).to_records(index=False,
                          column_dtypes=(np.float))
...
def command_line():
    ...
    parser.add_argument('--labels',
      dest='labels', nargs='+', action=DelimiterSplitAction, help="labels you wish to modify. Syntax: `--labels existing:new` where ':' is the default delimiter",
      default=None)
    parser.add_argument('-s',
      '--samples', dest='samples', default=None, nargs='+', help='Path to PESummary meta file you wish to modify')
    parser.add_argument('-w',
      '--webdir', dest='webdir', default='./', metavar='DIR', help='Directory to write the output file')
    parser.add_argument('--save_to_json',
      action='store_true', default=False, help='save the modified data in json format')
    parser.add_argument('--delimiter',
      dest='delimiter', default=':', help='Delimiter used to seperate the existing and new quantity')
    parser.add_argument('--kwargs',
      dest='kwargs', nargs='+', action=DelimiterSplitAction, help="kwargs you wish to modify. Syntax: `--kwargs label/kwarg:item` where '/' is a delimiter of your choosing (it cannot be ':'), kwarg is the kwarg name and item is the value of the kwarg",
      default=None)
    parser.add_argument('--overwrite',
      action='store_true', default=False, help='Overwrite the supplied PESummary meta file with the modified version')
    parser.add_argument('--replace_posterior',
      nargs='+', action=DelimiterSplitAction, help="Replace the posterior for a given label. Syntax: --replace_posterior label;a:/path/to/posterior.dat where ';' is a delimiter of your choosing (it cannot be '/' or ':'), a is the posterior you wish to replace and item is a path to a one column ascii file containing the posterior samples (/path/to/posterior.dat)",
      default=None)
    parser.add_argument('--remove_posterior',
      nargs='+', action=DelimiterSplitAction, help='Remove a posterior distribution for a given label. Syntax: --remove_posterior label:a where a is the posterior you wish to remove',
      default=None)
    return parser
...
def _modify_labels(data, labels=None):
    ...
    for existing, new in labels.items():
        if existing not in data.keys():
            logger.warn("Unable to find label '{}' in the root of the metafile. Checking inside the groups".format(existing))
...
def _modify_kwargs(data, kwargs=None):
    ...
    def add_to_meta_data(data, label, string):
        ...
        if group == 'other':
            if group not in data[label]['meta_data'].keys():
    ...
    def _replace_posterior(data, string):
        ...
        if math.isnan(_data[0]):
            ...
            if posterior in data[label]['posterior_samples'].dtype.names:
                ...
                data[label]['posterior_samples'] = append_fields((data[label]['posterior_samples']),
                  posterior, _data, usemask=False)
        return data
...
def _remove_posterior(data, kwargs=None):
    ...
    def _rmfield(array, *fieldnames_to_remove):
        return array[[name for name in array.dtype.names if name not in fieldnames_to_remove]]
...
def modify(data, function, **kwargs):
    ...
    """
    func_map = {'labels':_modify_labels, 
...
def main(args=None):
    ...
    if not args.overwrite:
        meta_file = os.path.join(args.webdir, 'modified_posterior_samples.{}'.format('h5' if args.hdf5 else 'json'))
    ...
    if args.replace_posterior is not None:
        ...
        logger.info("Saving the modified data to '{}'".format(meta_file))
        if args.hdf5:
            _GWMetaFile.save_to_hdf5(modified_data,
              (list(modified_data.keys())), None, meta_file, no_convert=True)
        else:
