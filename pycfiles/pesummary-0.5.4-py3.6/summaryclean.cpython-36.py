...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summaryclean.py
...
def command_line():
    ...
    parser.add_argument('--labels', dest='labels', help='labels used to distinguish runs',
      nargs='+',
      default=None)
    parser.add_argument('--file_format', dest='file_format', help='Save the cleaned data in this format',
      choices=[
...
def save(pesummary_object, file_format, webdir=None, label=None):
    ...
    if file_format == 'dat':
        ...
        if file_format == 'lalinference':
            pesummary_object.to_lalinference(outdir=webdir, label=label)
        else:
            if file_format == 'lalinference_dat':
