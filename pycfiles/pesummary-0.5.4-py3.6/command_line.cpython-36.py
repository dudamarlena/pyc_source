...
def insert_gwspecific_option_group(parser):
    ...
    gw_group.add_argument('--cosmology', dest='cosmology', help='The cosmology to use when calculating the redshift',
      default='Planck15')
    gw_group.add_argument('--no_conversion', action='store_true', help='Do not generate any conversions',
      default=False)
    remnant_group = _remnant_command_line_arguments(parser)
    return gw_group


def add_dynamic_argparse(existing_namespace, pattern, example='--{}_psd', default={}, nargs='+', action=DictionaryAction, command_line=None):
...
def add_dynamic_PSD_to_namespace(existing_namespace, command_line=None):
    ...
    add_dynamic_argparse(existing_namespace,
      '--*_psd', command_line=command_line)
...
def add_dynamic_calibration_to_namespace(existing_namespace, command_line=None):
    ...
    """
    add_dynamic_argparse(existing_namespace,
      '--*_calibration', example='--{}_calibration', command_line=command_line)