...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/file/conversions.py
# Compiled at: 2020-05-08 05:31:08
# Size of source mod 2**32: 93818 bytes
import numpy as np
...
try:
    ...
    from lalsimulation import SimInspiralTransformPrecessingNewInitialConditions, SimInspiralTransformPrecessingWvf2PE, DetectorPrefixToLALDetector, FLAG_SEOBNRv4P_HAMILTONIAN_DERIVATIVE_NUMERICAL, FLAG_SEOBNRv4P_EULEREXT_QNM_SIMPLE_PRECESSION, FLAG_SEOBNRv4P_ZFRAME_L
...
try:
    ...
    ASTROPY = False
    logger.warning('You do not have astropy installed currently. You will not be able to use some of the prebuilt functions.')

DEFAULT_SEOBFLAGS = {'SEOBNRv4P_SpinAlignedEOBversion':4, 
...
def _z_from_dL_exact(luminosity_distance, cosmology):
    ...
    """
    return z_at_value(cosmology.luminosity_distance, luminosity_distance * u.Mpc)
...
def z_from_dL_exact(luminosity_distance, cosmology='Planck15'):
    ...
    logger.warning('Estimating the exact redshift for every luminosity distance. This may take a few minutes.')
...
def z_from_dL_approx(luminosity_distance, N=100, cosmology='Planck15'):
    ...
    """
    from pesummary.gw.cosmology import get_cosmology
    logger.warning('The redshift is being approximated using interpolation. Bear in mind that this does introduce a small error.')
    cosmo = get_cosmology(cosmology)
...
def m1_from_mchirp_q(mchirp, q):
    ...
    mass ratio
    """
    return (1.0 / q) ** 0.4 * (1.0 + 1.0 / q) ** 0.2 * mchirp
...
def m2_from_mchirp_q(mchirp, q):
    ...
    mass ratio
    """
    return (1.0 / q) ** (-0.6) * (1.0 + 1.0 / q) ** 0.2 * mchirp
...
def invq_from_m1_m2(mass1, mass2):
    ...
    """
    return 1.0 / q_from_m1_m2(mass1, mass2)
...
def mchirp_from_mtotal_q(total_mass, mass_ratio):
    ...
    """
    mass1 = 1.0 / mass_ratio * total_mass / (1.0 + 1.0 / mass_ratio)
    mass2 = total_mass / (1.0 + 1.0 / mass_ratio)
    return eta_from_m1_m2(mass1, mass2) ** 0.6 * (mass1 + mass2)
...
def chi_p(mass1, mass2, spin1x, spin1y, spin2x, spin2y):
    ...
    chi_p = np.maximum(S1_perp, (4 * mass_ratio + 3) / (3 * mass_ratio + 4) * mass_ratio * S2_perp)
    return chi_p
...
def phi_12_from_phi1_phi2(phi1, phi2):
    ...
    """
    phi12 = phi2 - phi1
    if isinstance(phi12, float):
        if phi12 < 0.0:
            phi12 += 2 * np.pi
    if isinstance(phi12, np.ndarray):
        ind = np.where(phi12 < 0.0)
        phi12[ind] += 2 * np.pi
    return phi12
...
@array_input
def spin_angles(mass_1, mass_2, inc, spin1x, spin1y, spin1z, spin2x, spin2y, spin2z, f_ref, phase):
    ...
    if LALINFERENCE_INSTALL:
        ...
        for i in range(len(mass_1)):
            theta_jn, phi_jl, tilt_1, tilt_2, phi_12, a_1, a_2 = SimInspiralTransformPrecessingWvf2PE(incl=(inc[i]),
              m1=(mass_1[i]),
              m2=(mass_2[i]),
              S1x=(spin1x[i]),
              S1y=(spin1y[i]),
...
@array_input
def component_spins(theta_jn, phi_jl, tilt_1, tilt_2, phi_12, a_1, a_2, mass_1, mass_2, f_ref, phase):
    ...
    if LALINFERENCE_INSTALL:
        ...
        for i in range(len(theta_jn)):
            iota, S1x, S1y, S1z, S2x, S2y, S2z = SimInspiralTransformPrecessingNewInitialConditions(theta_jn[i], phi_jl[i], tilt_1[i], tilt_2[i], phi_12[i], a_1[i], a_2[i], mass_1[i] * MSUN_SI, mass_2[i] * MSUN_SI, float(f_ref[i]), float(phase[i]))
...
@array_input
def spin_angles_from_azimuthal_and_polar_angles(a_1, a_2, a_1_azimuthal, a_1_polar, a_2_azimuthal, a_2_polar):
    ...
    data = [[s1x, s1y, s1z, s2x, s2y, s2z] for s1x, s1y, s1z, s2x, s2y, s2z in zip(spin1x, spin1y, spin1z, spin2x, spin2y, spin2z)]
    return data
...
def time_in_each_ifo(detector, ra, dec, time_gps):
    ...
    if LALINFERENCE_INSTALL:
        if ASTROPY:
            ...
            n = np.array([i, j, k])
            dx = [
             0, 0, 0] - DetectorPrefixToLALDetector(detector).location
            dt = dx.dot(n) / C_SI
            return time_gps + dt
    raise Exception('Please install LALSuite and astropy for full conversions')
...
def lambda_tilde_from_lambda1_lambda2(lambda1, lambda2, mass1, mass2):
    ...
    lambda_tilde = 0.6153846153846154 * ((1 + 7 * eta - 31 * eta ** 2) * plus + (1 - 4 * eta) ** 0.5 * (1 + 9 * eta - 11 * eta ** 2) * minus)
    return lambda_tilde
...
def delta_lambda_from_lambda1_lambda2(lambda1, lambda2, mass1, mass2):
    ...
    plus = lambda1 + lambda2
    minus = lambda1 - lambda2
    delta_lambda = 0.5 * ((1 - 4 * eta) ** 0.5 * (1 - 10.062168309325246 * eta + 6.780894617134193 * eta ** 2) * plus + (1 - 12.062168309325246 * eta + 24.905231235784687 * eta ** 2 + 2.562547384382108 * eta ** 3) * minus)
    return delta_lambda
...
def lambda1_from_lambda_tilde(lambda_tilde, mass1, mass2):
    ...
    lambda1 = 1.625 * lambda_tilde / ((1 + 7 * eta - 31 * eta ** 2) * (1 + q ** (-5)) + (1 - 4 * eta) ** 0.5 * (1 + 9 * eta - 11 * eta ** 2) * (1 - q ** (-5)))
    return lambda1
...
def tilt_angles_and_phi_12_from_spin_vectors_and_L(a_1, a_2, Ln):
    ...
    cos_phi_12 = np.dot(a_1_perp, a_2_perp) / (np.linalg.norm(a_1_perp) * np.linalg.norm(a_2_perp))
    phi_12 = np.arccos(cos_phi_12)
    if np.sign(np.dot(Ln, np.cross(a_1, a_2))) < 0.0:
        phi_12 = 2.0 * np.pi - phi_12
...
def _return_final_mass_and_final_spin_from_waveform(mass_function=None, spin_function=None, mass_function_args=[], spin_function_args=[], mass_function_return_function=None, mass_function_return_index=None, spin_function_return_function=None, spin_function_return_index=None, mass_1_index=0, mass_2_index=1, nsamples=0, approximant=None, default_SEOBNRv4P_kwargs=False):
    ...
    if mass_function_return_function is not None:
        ...
        if mass_function_return_index is not None:
    ...
    final_mass = fm * (mass_function_args[mass_1_index] + mass_function_args[mass_2_index]) / MSUN_SI
    final_spin = fs
    return (final_mass, final_spin)


def _setup_SEOBNRv4P_args(mode=[
 2, 2], seob_flags=DEFAULT_SEOBFLAGS):
    """Setup the SEOBNRv4P[HM] kwargs
    """
    from lalsimulation import SimInspiralCreateModeArray, SimInspiralModeArrayActivateMode
    ...
    return (
     mode_array, _seob_flags)


@array_input
def _final_from_initial(mass_1, mass_2, spin_1x, spin_1y, spin_1z, spin_2x, spin_2y, spin_2z, approximant='SEOBNRv4', iota=None, luminosity_distance=None, f_ref=None, phi_ref=None, mode=[2, 2], delta_t=0.000244140625, seob_flags=DEFAULT_SEOBFLAGS, return_fits_used=False, multi_process=None):
    ...
    """
    from lalsimulation import SimIMREOBFinalMassSpin, SimIMREOBFinalMassSpinPrec, SimInspiralGetSpinSupportFromApproximant, SimIMRSpinPrecEOBWaveformAll, SimPhenomUtilsIMRPhenomDFinalMass, SimPhenomUtilsPhenomPv2FinalSpin
    ...
    try:
        ...
        raise ValueError("The waveform '{}' is not supported by lalsimulation")
    ...
    if approximant.lower() in ('seobnrv4p', 'seobnrv4phm'):
        if any(i is None for i in [iota, luminosity_distance, f_ref, phi_ref]):
            raise ValueError("The approximant '{}' requires samples for iota, f_ref, phi_ref and luminosity_distance. Please pass these samples.".format(approximant))
        if len(delta_t) == 1:
            delta_t = [
             delta_t[0]] * len(mass_1)
        else:
            if len(delta_t) != len(mass_1):
                raise ValueError("Please provide either a single 'delta_t' that is is used for all samples, or a single 'delta_t' for each sample")
        mode_array, _seob_flags = _setup_SEOBNRv4P_args(mode=mode,
          seob_flags=seob_flags)
        args = np.array([
         phi_ref, delta_t, m1, m2, f_ref, luminosity_distance, iota,
         spin_1x, spin_1y, spin_1z, spin_2x, spin_2y, spin_2z,
         [
          mode_array] * len(mass_1), [_seob_flags] * len(mass_1)])
        kwargs.update({'mass_function':SimIMRSpinPrecEOBWaveformAll, 
        ...
        if approximant.lower() in ('seobnrv4', ):
            ...
            if 'phenompv3' in approximant.lower():
                ...
                raise ValueError("The waveform '{}' is not support by this function.".format(approximant))
        ...
        if multi_process is not None and multi_process[0] != 1:
            ...
            if approximant.lower() in ('seobnrv4p', 'seobnrv4phm'):
                logger.warn("Ignoring passed 'mode' and 'seob_flags' options. Defaults must be used with multiprocessing. If you wish to use custom options, please set `multi_process=None`")
                _kwargs = kwargs.copy()
            ...
            with multiprocessing.Pool(_multi_process) as (pool):
                data = np.array(list(iterator((pool.imap(_wrapper_return_final_mass_and_final_spin_from_waveform, args)),
                  tqdm=True,
            ...
            _iterator = iterator((range(kwargs['nsamples'])),
              tqdm=True, total=(len(mass_1)), desc=('Evaluating {} fit'.format(approximant)))
            for i in _iterator:
                data = _wrapper_return_final_mass_and_final_spin_from_waveform(args[i])
...
def final_remnant_properties_from_NRSurrogate(*args, f_low=20.0, f_ref=20.0, model='NRSur7dq4Remnant', return_fits_used=False, properties=[
 'final_mass', 'final_spin', 'final_kick'], approximant='SEOBNRv4PHM'):
    ...
    fit = NRSur_fit(*args, f_low=f_low, f_ref=f_ref, model=model, fits=properties, approximant=approximant)
    if return_fits_used:
        return (fit, [model])
    else:
        return fit
...
def final_mass_of_merger_from_NR(*args, NRfit='average', final_spin=None, return_fits_used=False):
    ...
    if NRfit.lower() == 'average':
        ...
        if 'panetal' in NRfit.lower():
            func = getattr(nrutils, 'bbh_final_mass_non_spinning_Panetal')
        else:
...
def final_mass_of_merger_from_NRSurrogate(*args, model='NRSur7dq4Remnant', return_fits_used=False, approximant='SEOBNRv4PHM'):
    """Return the final mass resulting from a BBH merger using NRSurrogate
    fits
    """
    data = final_remnant_properties_from_NRSurrogate(*args, model=model, properties=['final_mass'], return_fits_used=return_fits_used, 
...
def final_spin_of_merger_from_NR(*args, NRfit='average', return_fits_used=False):
    ...
    if NRfit.lower() == 'average':
        ...
        if 'pan' in NRfit.lower():
            func = getattr(nrutils, 'bbh_final_spin_non_spinning_Panetal')
        else:
            if 'precessing' in NRfit.lower():
                func = getattr(nrutils, 'bbh_final_spin_precessing_{}'.format(NRfit.split('precessing_')[1]))
            else:
...
def final_spin_of_merger_from_NRSurrogate(*args, model='NRSur7dq4Remnant', return_fits_used=False, approximant='SEOBNRv4PHM'):
    """Return the final spin resulting from a BBH merger using NRSurrogate
    fits
    """
    data = final_remnant_properties_from_NRSurrogate(*args, model=model, properties=['final_spin'], return_fits_used=return_fits_used, 
     approximant=approximant)
    if return_fits_used:
...
def final_kick_of_merger_from_NRSurrogate(*args, model='NRSur7dq4Remnant', return_fits_used=False, approximant='SEOBNRv4PHM'):
    """Return the final kick of the remnant resulting from a BBH merger
    using NRSurrogate fits
    """
    data = final_remnant_properties_from_NRSurrogate(*args, model=model, properties=['final_kick'], return_fits_used=return_fits_used, 
     approximant=approximant)
    if return_fits_used:
...
def final_mass_of_merger(*args, method='NR', approximant='SEOBNRv4', NRfit='average', final_spin=None, return_fits_used=False, model='NRSur7dq4Remnant'):
    ...
    if method.lower() == 'nr':
        ...
        kwargs = {'NRfit':NRfit, 
         'final_spin':final_spin,  'return_fits_used':return_fits_used}
    else:
        if 'nrsur' in method.lower():
            mass_func = final_mass_of_merger_from_NRSurrogate
            kwargs = {'approximant':approximant, 
             'return_fits_used':return_fits_used,  'model':model}
        else:
...
def final_spin_of_merger(*args, method='NR', approximant='SEOBNRv4', NRfit='average', return_fits_used: False, model='NRSur7dq4Remnant'):
    """Return the final mass resulting from a BBH merger
...
def final_kick_of_merger(*args, method='NR', approximant='SEOBNRv4', NRfit='average', return_fits_used: False, model='NRSur7dq4Remnant'):
    ...
    if 'nrsur' not in method.lower():
        raise NotImplementedError('Currently you can only work out the final kick velocity using NRSurrogate fits.')
...
def peak_luminosity_of_merger(*args, NRfit='average', return_fits_used=False):
    ...
    if NRfit.lower() == 'average':
        ...
        func = getattr(nrutils, 'bbh_peak_luminosity_non_precessing_{}'.format(NRfit))
    if NRfit.lower() == 'average':
        return func(*args, **{'return_fits_used': return_fits_used})
    else:
        return func(*args)
...
class _Conversion(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if redshift_method not in ('approx', 'exact'):
            raise ValueError("'redshift_method' can either be 'approx' corresponding to an approximant method, or 'exact' corresponding to an exact method of calculating the redshift")
        if isinstance(NRSurrogate, bool):
            if NRSurrogate:
                raise ValueError("'NRSur_fits' must be a string corresponding to the NRSurrogate model you wish to use to calculate the remnant quantities")
        waveform_fits = kwargs.get('waveform_fits', False)
        if NRSurrogate:
            if waveform_fits:
                raise ValueError('Unable to use both the NRSurrogate and {} to calculate remnant quantities. Please select only one option'.format(approximant))
        evolve_spins = kwargs.get('evolve_spins', False)
        if isinstance(evolve_spins, bool):
            if evolve_spins:
                raise ValueError("'evolve_spins' must be a float, the final velocity to evolve the spins up to, or a string, 'ISCO', meaning evolve the spins up to the ISCO frequency")
        if not evolve_spins and (NRSurrogate or waveform_fits):
            if 'eob' in approximant or NRSurrogate:
                logger.warn('Only evolved spin remnant quantities are returned by the {} fits.'.format('NRSurrogate' if NRSurrogate else approximant))
        else:
            if evolve_spins:
                if NRSurrogate or waveform_fits:
                    if 'eob' in approximant or NRSurrogate:
                        logger.warn('The {} fits already evolve the spins. Therefore additional spin evolution will not be performed.'.format('NRSurrogate' if NRSurrogate else approximant))
                    else:
                        logger.warn('The {} fits are not applied with spin evolution.'.format(approximant))
                    evolve_spins = False
        if f_low is not None:
            if 'f_low' in extra_kwargs['meta_data'].keys():
                logger.warn(base_replace.format('f_low', extra_kwargs['meta_data']['f_low'], f_low))
                extra_kwargs['meta_data']['f_low'] = f_low
        if f_low is not None:
            extra_kwargs['meta_data']['f_low'] = f_low
        if approximant is not None:
            if 'approximant' in extra_kwargs['meta_data'].keys():
                logger.warn(base_replace.format('approximant', extra_kwargs['meta_data']['approximant'], approximant))
                extra_kwargs['meta_data']['approximant'] = approximant
        if approximant is not None:
            extra_kwargs['meta_data']['approximant'] = approximant
        if f_ref is not None:
            if 'f_ref' in extra_kwargs['meta_data'].keys():
                logger.warn(base_replace.format('f_ref', extra_kwargs['meta_data']['f_ref'], f_ref))
        ...
        obj.__init__(parameters, samples, extra_kwargs, evolve_spins, NRSurrogate, waveform_fits, multi_process, regenerate, redshift_method, cosmology, force_non_evolved)
        ...
        if kwargs.get('return_dict', True):
            ...
            if return_kwargs:
                return (
                 obj.parameters, obj.samples, obj.extra_kwargs)
            return (obj.parameters, obj.samples)

    def __init__(self, parameters, samples, extra_kwargs, evolve_spins, NRSurrogate, waveform_fits, multi_process, regenerate, redshift_method, cosmology, force_non_evolved):
    ...
    def remove_posterior(self, parameter):
        if parameter in self.parameters:
            logger.info("Removing the posterior samples for '{}'".format(parameter))
            ...
            logger.info("'{}' is not in the table of posterior samples. Unable to remove".format(parameter))
    ...
    def _invert_q(self):
        ...
        for num, i in enumerate(self.samples):
            self.samples[num][ind] = 1.0 / self.samples[num][ind]
    ...
    def _reference_frequency(self):
        ...
        if extra_kwargs != {}:
            if 'f_ref' in list(extra_kwargs.keys()):
                self.append_data('reference_frequency', [float(extra_kwargs['f_ref'])] * nsamples)
        else:
            logger.warn('Could not find reference_frequency in input file. Using 20Hz as default')
            self.append_data('reference_frequency', [20.0] * nsamples)
    ...
    def _spin_angles(self):
        angles = [
         ...
         'a_1', 'a_2']
        spin_angles_to_calculate = [i for i in angles if i not in self.parameters]
        spin_components = [
        ...
        if 'phase' in self.parameters:
            ...
            logger.warn('Phase it not given, we will be assuming that a reference phase of 0 to calculate all the spin angles')
            samples.append([0] * len(samples[0]))
        angles = spin_angles(samples[0], samples[1], samples[2], samples[3], samples[4], samples[5], samples[6], samples[7], samples[8], samples[9], samples[10])
    ...
    def _non_precessing_component_spins(self):
        ...
        if all(i in self.parameters for i in angles):
            ...
            spins_to_calculate = [i for i in spins if i not in self.parameters]
            if cond1 and cond1:
                spin_1x = np.array([0.0] * len(samples[0]))
                spin_1y = np.array([0.0] * len(samples[0]))
                spin_1z = samples[0] * np.cos(samples[3])
                spin_2x = np.array([0.0] * len(samples[0]))
                spin_2y = np.array([0.0] * len(samples[0]))
    ...
    def _component_spins(self):
        ...
        if 'phase' in self.parameters:
            ...
            logger.warn('Phase it not given, we will be assuming that a reference phase of 0 to calculate all the spin angles')
            samples.append([0] * len(samples[0]))
        spin_components = component_spins(samples[0], samples[1], samples[2], samples[3], samples[4], samples[5], samples[6], samples[7], samples[8], samples[9], samples[10])
        for i in spins_to_calculate:
            ind = spins.index(i)
    ...
    def _component_spins_from_azimuthal_and_polar_angles(self):
        ...
        angles = [
         ...
         'a_2_polar']
        samples = self.specific_parameter_samples(angles)
        spin_components = spin_angles_from_azimuthal_and_polar_angles(samples[0], samples[1], samples[2], samples[3], samples[4], samples[5])
        for i in spins_to_calculate:
            ind = spins.index(i)
            data = np.array([i[ind] for i in spin_components])
            self.append_data(i, data)
    ...
    def _chi_p(self):
        ...
        chi_p_samples = chi_p(samples[0], samples[1], samples[2], samples[3], samples[4], samples[5])
    ...
    def _lambda_tilde_from_lambda1_lambda2(self):
        ...
        lambda_tilde = lambda_tilde_from_lambda1_lambda2(samples[0], samples[1], samples[2], samples[3])
        self.append_data('lambda_tilde', lambda_tilde)

    def _delta_lambda_from_lambda1_lambda2(self):
        samples = self.specific_parameter_samples([
         'lambda_1', 'lambda_2', 'mass_1', 'mass_2'])
        delta_lambda = delta_lambda_from_lambda1_lambda2(samples[0], samples[1], samples[2], samples[3])
        self.append_data('delta_lambda', delta_lambda)

    def _ifo_snr(self):
        abs_snrs = [i for i in self.parameters if '_matched_filter_abs_snr' in i]
        angle_snrs = [i for i in self.parameters if '_matched_filter_snr_angle' in i]
    ...
    def _matched_filter_network_snr(self):
        snrs = [i for i in self.parameters if '_matched_filter_snr' in i if '_angle' not in i if '_abs' not in i]
        if len(snrs) == 0:
            snrs = [i for i in self.parameters if '_matched_filter_snr_abs' in i if '_angle' not in i]
        samples = self.specific_parameter_samples(snrs)
        snr = network_snr(samples)
    ...
    def _retrieve_f_low(self):
        extra_kwargs = self.extra_kwargs['meta_data']
        if extra_kwargs != {}:
            if 'f_low' in list(extra_kwargs.keys()):
                f_low = extra_kwargs['f_low']
        else:
            raise ValueError('Could not find f_low in input file. Please either modify the input file or pass it from the command line')
        return f_low

    def _retrieve_approximant(self):
        extra_kwargs = self.extra_kwargs['meta_data']
        if extra_kwargs != {}:
            if 'approximant' in list(extra_kwargs.keys()):
                approximant = extra_kwargs['approximant']
        else:
            raise ValueError('Unable to find the approximant used to generate the posterior samples in the result file.')
        return approximant
    ...
    def _evolve_spins(self, final_velocity='ISCO'):
        ...
        tilt_1_evolved, tilt_2_evolved, phi_12_evolved = evolve_spins((samples[0]),
          (samples[1]), (samples[2]), (samples[3]), (samples[4]), (samples[5]),
          (samples[6]), f_low, (samples[7][0]), approximant,
    ...
    @staticmethod
    def _evolved_vs_non_evolved_parameter(parameter, evolved=False, core_param=False):
        if evolved:
            if core_param:
    ...
    def _precessing_vs_non_precessing_parameters(self, non_precessing=False, evolved=False):
        if not non_precessing:
            tilt_1 = self._evolved_vs_non_evolved_parameter('tilt_1',
              evolved=evolved, core_param=True)
            tilt_2 = self._evolved_vs_non_evolved_parameter('tilt_2',
              evolved=evolved, core_param=True)
            samples = self.specific_parameter_samples([
             'mass_1', 'mass_2', 'a_1', 'a_2', tilt_1, tilt_2])
            if 'phi_12' in self.parameters:
                if evolved:
                    phi_12_samples = self.specific_parameter_samples([
                     self._evolved_vs_non_evolved_parameter('phi_12',
                       evolved=True, core_param=True)])[0]
            if 'phi_12' in self.parameters:
            ...
            spin_1z = self._evolved_vs_non_evolved_parameter('spin_1z',
              evolved=evolved, core_param=True)
            spin_2z = self._evolved_vs_non_evolved_parameter('spin_2z',
              evolved=evolved, core_param=True)
            samples = self.specific_parameter_samples([
             'mass_1', 'mass_2', spin_1z, spin_2z])
            samples = [
    ...
    def _peak_luminosity_of_merger(self, evolved=False):
        param = self._evolved_vs_non_evolved_parameter('peak_luminosity',
          evolved=evolved)
        spin_1z_param = self._evolved_vs_non_evolved_parameter('spin_1z',
          evolved=evolved, core_param=True)
        spin_2z_param = self._evolved_vs_non_evolved_parameter('spin_2z',
          evolved=evolved, core_param=True)
        samples = self.specific_parameter_samples([
         'mass_1', 'mass_2', spin_1z_param, spin_2z_param])
        peak_luminosity, fits = peak_luminosity_of_merger((samples[0]),
          (samples[1]), (samples[2]), (samples[3]), return_fits_used=True)
        self.append_data(param, peak_luminosity)
        self.extra_kwargs['meta_data']['peak_luminosity_NR_fits'] = fits

    def _final_remnant_properties_from_NRSurrogate(self, non_precessing=False, parameters=['final_mass', 'final_spin', 'final_kick']):
        f_low = self._retrieve_f_low()
        approximant = self._retrieve_approximant()
        samples = self._precessing_vs_non_precessing_parameters(non_precessing=non_precessing,
          evolved=False)
        frequency_samples = self.specific_parameter_samples([
         'reference_frequency'])
        data, fits = final_remnant_properties_from_NRSurrogate(*samples, f_low=f_low, f_ref=frequency_samples[0], properties=parameters, 
         return_fits_used=True, approximant=approximant)
        for param in parameters:
            self.append_data(param, data[param])
            self.extra_kwargs['meta_data']['{}_NR_fits'.format(param)] = fits

    def _final_remnant_properties_from_waveform(self, non_precessing=False, parameters=['final_mass', 'final_spin']):
        f_low = self._retrieve_f_low()
        approximant = self._retrieve_approximant()
        ...
        if 'seob' in approximant.lower():
            logger.warn("Could not find 'delta_t' in the meta data. Using {} as default.".format(delta_t))
        else:
            if non_precessing:
                sample_params = ['mass_1', 'mass_2', 'empty', 'empty', 'spin_1z', 'empty',
                 'empty', 'spin_2z', 'iota', 'luminosity_distance',
                 'phase']
            else:
                sample_params = ['mass_1', 'mass_2', 'spin_1x', 'spin_1y', 'spin_1z',
        ...
        _data, fits = _final_from_initial(*samples[:8], iota=samples[8], luminosity_distance=samples[9], f_ref=[
 f_low] * len(samples[0]), 
         phi_ref=samples[10], delta_t=0.000244140625, 
         approximant=approximant, return_fits_used=True, multi_process=self.multi_process)
        data = {'final_mass':_data[0], 
         'final_spin':_data[1]}
        for param in parameters:
            self.append_data(param, data[param])
            self.extra_kwargs['meta_data']['{}_NR_fits'.format(param)] = fits
    ...
    def _final_mass_of_merger(self, evolved=False):
        param = self._evolved_vs_non_evolved_parameter('final_mass',
          evolved=evolved)
        spin_1z_param = self._evolved_vs_non_evolved_parameter('spin_1z',
          evolved=evolved, core_param=True)
        spin_2z_param = self._evolved_vs_non_evolved_parameter('spin_2z',
        ...
        samples = self.specific_parameter_samples([
         'mass_1', 'mass_2', spin_1z_param, spin_2z_param])
        final_mass, fits = final_mass_of_merger(*samples, **{'return_fits_used': True})
    ...
    def _final_mass_source(self, evolved=False):
        ...
        final_mass_source = _source_from_detector(samples[0], samples[1])
    ...
    def _final_spin_of_merger(self, non_precessing=False, evolved=False):
        param = self._evolved_vs_non_evolved_parameter('final_spin',
          evolved=evolved)
        samples = self._precessing_vs_non_precessing_parameters(non_precessing=non_precessing,
          evolved=evolved)
        final_spin, fits = final_spin_of_merger(*samples, **{'return_fits_used': True})
    ...
    def _radiated_energy(self, evolved=False):
        param = self._evolved_vs_non_evolved_parameter('radiated_energy',
          evolved=evolved)
        final_mass_param = self._evolved_vs_non_evolved_parameter('final_mass_source',
          evolved=evolved)
        samples = self.specific_parameter_samples([
    ...
    def _check_parameters(self):
        params = [
         'mass_1', 'mass_2', 'a_1', 'a_2', 'mass_1_source', 'mass_2_source',
         'mass_ratio', 'total_mass', 'chirp_mass']
        for i in params:
            if i in self.parameters:
                samples = self.specific_parameter_samples([i])
                if 'mass' in i:
                    cond = any(np.array(samples[0]) <= 0.0)
                else:
                    cond = any(np.array(samples[0]) < 0.0)
                if cond:
                    if 'mass' in i:
                        ind = np.argwhere(np.array(samples[0]) <= 0.0)
                    else:
                        ind = np.argwhere(np.array(samples[0]) < 0.0)
                    logger.warn('Removing %s samples because they have unphysical values (%s < 0)' % (
                     len(ind), i))
    ...
    def generate_all_posterior_samples(self, evolve_spins=False):
        ...
        if 'cos_theta_jn' in self.parameters:
            if 'theta_jn' not in self.parameters:
                self._cos_angle('theta_jn', reverse=True)
        if 'cos_iota' in self.parameters:
            if 'iota' not in self.parameters:
                self._cos_angle('iota', reverse=True)
        if 'cos_tilt_1' in self.parameters:
            if 'tilt_1' not in self.parameters:
                self._cos_angle('tilt_1', reverse=True)
        if 'cos_tilt_2' in self.parameters:
            if 'tilt_2' not in self.parameters:
        ...
        if all(i in self.parameters for i in spin_magnitudes):
            if all(i not in self.parameters for i in angles):
                ...
                for num, i in enumerate(self.samples):
                    self.samples[num].append(np.arccos(np.sign(i[self.parameters.index('a_1')])))
                    self.samples[num].append(np.arccos(np.sign(i[self.parameters.index('a_2')])))
        ...
        if not all(i in self.parameters for i in spin_magnitudes):
            cartesian = ['spin_1x', 'spin_1y', 'spin_1z', 'spin_2x', 'spin_2y',
             'spin_2z']
        ...
        if 'mass_ratio' not in self.parameters:
            if 'symmetric_mass_ratio' in self.parameters:
                self._q_from_eta()
        if 'mass_ratio' not in self.parameters:
        ...
        if 'mass_ratio' in self.parameters:
            ...
            if median > 1.0:
                self._invert_q()
        if 'inverted_mass_ratio' not in self.parameters:
            if 'mass_ratio' in self.parameters:
                self._invq_from_q()
        if 'chirp_mass' not in self.parameters:
        ...
        if 'mass_1' in self.parameters:
            if 'mass_2' in self.parameters:
                if 'total_mass' not in self.parameters:
                    ...
                    elif 'symmetric_mass_ratio' not in self.parameters:
                    ...
                    if evolve_condition:
                        ...
                        non_precessing_NR_params = ['{}_evolved'.format(i) for i in non_precessing_NR_params]
                ...
                check_for_evolved_parameter = --- This code section failed: ---

 L.2201         0  LOAD_GLOBAL              len
                2  LOAD_FAST                'suffix'
               ...
               14  JUMP_IF_FALSE_OR_POP    34  'to 34'
               16  LOAD_FAST                'param'
               18  LOAD_FAST                'suffix'
               20  BINARY_ADD       
               22  LOAD_FAST                'params'
...
Parse error at or near `None' instruction at offset -1
                condition_peak_luminosity = check_for_evolved_parameter(evolve_suffix, 'peak_luminosity', self.parameters)
                condition_final_spin = check_for_evolved_parameter(evolve_suffix, 'final_spin', self.parameters)
                condition_final_mass = check_for_evolved_parameter(evolve_suffix, 'final_mass', self.parameters)
                if self.NRSurrogate or self.waveform_fit:
                    ...
                    if all(i in self.parameters for i in final_spin_params):
                        ...
                        if all(i in self.parameters for i in non_precessing_NR_params):
                            if condition_final_spin or self.force_non_evolved:
                                self._final_spin_of_merger(non_precessing=True,
                                  evolved=False)
                        if all(i in self.parameters for i in non_precessing_NR_params):
                            if condition_peak_luminosity or self.force_non_evolved:
                                self._peak_luminosity_of_merger(evolved=evolve_condition)
        ...
        if 'cos_tilt_1' not in self.parameters:
            if 'tilt_1' in self.parameters:
                self._cos_tilt_1_from_tilt_1()
        if 'cos_tilt_2' not in self.parameters:
            if 'tilt_2' in self.parameters:
        ...
        if 'redshift' in self.parameters:
            if 'mass_1_source' not in self.parameters:
                ...
                condition_final_mass_source = check_for_evolved_parameter(evolve_suffix, 'final_mass_source', self.parameters)
        ...
        if 'total_mass_source' in self.parameters:
            if 'final_mass_source{}'.format(evolve_suffix) in self.parameters:
                condition_radiated_energy = check_for_evolved_parameter(evolve_suffix, 'radiated_energy', self.parameters)
        ...
        if all(i in self.parameters for i in location):
            try:
                ...
                logger.warn('Failed to generate posterior samples for the time in each detector because %s' % e)
        ...
        if 'theta_jn' in self.parameters:
            if 'cos_theta_jn' not in self.parameters:
                self._cos_angle('cos_theta_jn')
        if 'iota' in self.parameters:
            if 'cos_iota' not in self.parameters:
