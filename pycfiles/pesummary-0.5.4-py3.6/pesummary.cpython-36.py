...
class PESummary(GWRead, CorePESummary):
    ...
    def load(self, function, **kwargs):
        ...
        if 'psd' in self.data.keys():
            ...
            try:
                self.psd = {label:{ifo:PSD(value) for ifo, value in psd_data.items()} for label, psd_data in self.data['psd'].items()}
        ...
        if 'calibration' in self.data.keys():
            ...
            try:
                self.calibration = {label:{ifo:Calibration(value) for ifo, value in calibration_data.items()} for label, calibration_data in self.data['calibration'].items()}
            except (KeyError, AttributeError):
        ...
        if 'calibration' in self.data['prior'].keys():
            from pesummary.gw.file.calibration import Calibration
            try:
                self.priors['calibration'] = {label:{ifo:Calibration(value) for ifo, value in calibration_data.items()} for label, calibration_data in self.data['prior']['calibration'].items()}
            except (KeyError, AttributeError):
                pass
    ...
    def _grab_data_from_dictionary(dictionary):
        """
        """
        stored_data = CorePESummary._grab_data_from_dictionary(dictionary=dictionary)
        approx_list = list()
        psd_dict, cal_dict = {}, {}
        psd, cal = (None, None)
    ...
    def calibration_data_in_results_file(self):
        if self.calibration:
            ...
            total = [[self.calibration[key][ifo] for ifo in keys[num]] for num, key in enumerate(self.labels)]
            return (total, keys)
    ...
    def generate_all_posterior_samples(self, **conversion_kwargs):
        ...
        for param, samples, kwargs in zip(self.parameters, self.samples, self.extra_kwargs):
            if conversion_kwargs.get('evolve_spins', False):
                ...
                data = _Conversion(param, samples, extra_kwargs=kwargs, return_dict=False, **conversion_kwargs)
        ...
        if converted_kwargs != []:
            self.extra_kwargs = {label:converted_kwargs[num] for num, label in enumerate(self.labels)}
    ...
    def to_bilby(self):
        ...
        for num, label in enumerate(self.labels):
            priors = PriorDict()
            logger.warn('No prior information is known so setting it to a default')
            priors.update({parameter:Prior() for parameter in self.parameters[num]})
            posterior_data_frame = DataFrame((self.samples[num]),
              columns=(self.parameters[num]))
            meta_data = {'likelihood': {'waveform_arguments':{'waveform_approximant': self.approximant[num]}, 
                            'interferometers':self.detectors[num]}}
            bilby_object = CompactBinaryCoalescenceResult(search_parameter_keys=(self.parameters[num]),
              posterior=posterior_data_frame,
              label=('pesummary_%s' % label),
    ...
    def to_lalinference(self, outdir='./', labels=None, overwrite=False, sampler='lalinference_nest', dat=False, filenames=None):
        ...
        if labels is not None:
            for label in labels:
                if label not in self.labels:
                    raise ValueError("The label '{}' does not exist in the metafile".format(label))

        else:
            labels = self.labels
        if filenames is not None:
            if not all(label in filenames.keys() for label in labels):
                raise ValueError('Please provide a filename for all labels')
        else:
            filenames = {label:None for label in labels}
        for num, label in enumerate(labels):
            write_to_file((self.samples_dict[label]),
              outdir=outdir, label=label, overwrite=overwrite,
              sampler=sampler,
              dat=dat,
...
class PESummaryDeprecated(PESummary):
    ...
    @property
    def load_kwargs(self):
        return {'grab_data_from_dictionary': PESummaryDeprecated._grab_data_from_dictionary}

    @staticmethod
    def _grab_data_from_dictionary(dictionary):
        """
        """
        data = CorePESummaryDeprecated._grab_data_from_dictionary(dictionary=dictionary)
        approx_list = list()
        psd, cal = (None, None)
