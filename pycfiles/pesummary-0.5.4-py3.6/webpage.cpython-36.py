...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/core/webpage/webpage.py
# Compiled at: 2020-05-08 05:31:08
# Size of source mod 2**32: 44821 bytes
import pesummary
...
def make_html(web_dir, title='Summary Pages', pages=None, stylesheets=[], label=None):
    ...
    for i in pages:
        ...
        stylesheet_elements = ''.join(["  <link rel='stylesheet' href='../css/{0:s}.css'>\n".format(s) for s in stylesheets])
...
class page(Base):
    __doc__ = 'Class to generate and manipulate an html page.\n    '
    ...
    def _footer(self, user, rundir):
        ...
        self.add_content('</script>')
        self.make_div(_class='jumbotron',
          _style='margin-bottom:0; line-height: 0.5;background-color:#989898; bottom:0; position:bottom;width:100%')
        ...
        self.add_content("<a href='https://git.ligo.org/lscsoft/pesummary/tree/v{}' data-toggle='tooltip' title='View PESummary-v{}'><i class='fa fa-code' style='font-size: 30px; color: #E8E8E8; font-weight: 900; padding-right:10px'></i></a>".format(pesummary.__short_version__, pesummary.__short_version__))
        self.add_content("<a href='https://git.ligo.org/lscsoft/pesummary/issues' data-toggle='tooltip' title='Open an issue ticket'><i class='fa fa-ticket' style='font-size: 30px; color: #E8E8E8; font-weight: 900; padding-right:10px'></i></a>")
        self.add_content("<a href='https://lscsoft.docs.ligo.org/pesummary/' data-toggle='tooltip' title='View the docs!'><i class='fa fa-book' style='font-size: 30px; color: #E8E8E8; font-weight: 900; padding-right:10px'></i></a>")
        link = 'https://lscsoft.docs.ligo.org/pesummary/stable_docs/tutorials/make_your_own_page_from_metafile.html'
        self.add_content("<a href='{}' data-toggle='tooltip' title='Make your own page'><i class='fa fa-window-restore' style='font-size: 30px; color: #E8E8E8; font-weight: 900; padding-right:10px'></i></a>".format(link))
        self.add_content('</div>', indent=6)
        self.add_content('</div>', indent=4)
        self.add_content("<div class='col-sm-7'>", indent=4)
        self.add_content("<p style='color: #E8E8E8; font-weight: bold; font-family: arial-body; margin-top:14px'>This page was produced by {} at {} on {}</p>".format(user, time.strftime('%H:%M'), time.strftime('%B %d %Y')))
        self.add_content('</div>', indent=4)
    ...
    def _setup_navbar(self, background_colour):
        if background_colour == 'navbar-dark' or background_colour is None:
            self.add_content("<nav class='navbar navbar-expand-sm navbar-dark bg-dark fixed-top'>\n")
        else:
            self.add_content("<nav class='navbar navbar-expand-sm fixed-top navbar-custom' style='background-color: %s'>" % background_colour)
        self.add_content("<a class='navbar-brand' href='#' style='color: white'>Navbar</a>\n", indent=2)
        self.add_content("<button class='navbar-toggler' type='button' data-toggle='collapse' data-target='#collapsibleNavbar'>\n", indent=2)
    ...
    def make_banner(self, approximant=None, key='Summary', _style=None, link=None):
        ...
        if key == 'Summary':
            self.add_content('The figures below show the summary plots for the run')
        else:
            if key == 'config':
                self.add_content('Below is the config file for %s' % approximant)
            else:
                if key == 'corner':
                    self.add_content('Below is the custom corner plotter for %s' % approximant)
                else:
                    if key == 'interactive_corner':
                        self.add_content('Below are interative corner plots for %s. Simply use the Box Select to select the points you wish to look at' % approximant)
                    else:
                        if key == 'Comparison':
                            self.add_content('Below are the summary comparison plots')
                        else:
                            if key == 'Version':
                                self.add_content('Below is the version information for all files passed and the PESummary version used to generate these pages')
                            else:
                                if key == 'Publication':
                                    self.add_content('Below are publication quality plots for the passed result files')
                                else:
                                    if key == 'Logging':
                                        ...
                                        if key == 'Notes':
                                            ...
                                            if key == 'classification':
                                                self.add_content('Below we look at the source probabilities and plots for the passed result file')
                                            else:
                                                if key == 'sampler_kwargs':
                                                    self.add_content('Sampler information extracted from the result file')
                                                else:
                                                    if key == 'meta_data':
                                                        self.add_content('Meta data extracted from the result file')
                                                    else:
                                                        if key == 'command_line':
                                                            if link is not None:
                                                                self.add_content('This page was generated with the following command-line call from the directory %s' % link)
                                                            else:
                                                                self.add_content('This page was generated with the following command-line call:')
                                                        else:
                                                            if key == 'detchar':
                                                                base_string = 'Below are summary plots for the detector %s.{}' % approximant
                                                                if link is not None:
                                                                    base_string = base_string.format(' For more details see the LIGO summary pages <a href=%s>here</a>' % link)
                                                                else:
                                                                    base_string = base_string.format('')
                                                                self.add_content(base_string)
                                                            else:
                                                                if key == 'Downloads':
                                                                    self.add_content('Below are links to download all relevant information')
                                                                else:
                                                                    if key == 'About':
                                                                        self.add_content('Below is information about how these pages were generated')
                                                                    else:
                                                                        self.add_content('The figures below show the plots for %s' % approximant)
        self.end_div()

    def make_navbar(self, links=None, samples_path='./samples', search=True, histogram_download=None, background_color='navbar-dark', hdf5=False, about=True):
        ...
        for i in links:
            if type(i) == list:
                ...
                self.add_content("<a class='nav-link dropdown-toggle' href='#' id='navbarDropdown' role='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>\n", indent=12)
                    ...
                    if type(j) == list:
                        if len(j) > 1:
                            if type(j[1]) == list:
                                ...
                                self.add_content(("<a class='dropdown-toggle' id='{}' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>{}</a>\n".format(j[0], j[0])),
                                  indent=16)
                                self.add_content(("<ul class='dropdown-menu' aria-labelledby='{}'>\n".format(j[0])),
                                  indent=16)
                                for k in j[1]:
                                    if type(k) == dict:
                                        key = list(k.keys())[0]
                                        self.add_content(('<li class=\'dropdown-item\' href=\'#\' onclick=\'grab_html("{}", label="{}")\'><a>{}</a></li>\n'.format(key, k[key], key)),
                                          indent=18)
                                    else:
                                        self.add_content(('<li class=\'dropdown-item\' href=\'#\' onclick=\'grab_html("{}")\'><a>{}</a></li>\n'.format(k, k)),
                                          indent=18)
                ...
                self.add_content("<li class='nav-item'>\n", indent=8)
                if i == 'home':
                    self.add_content(('<a class=\'nav-link\' href=\'#\' onclick=\'grab_html("{}")\'>{}</a>\n'.format(i, i)),
                      indent=10)
                else:
                    if type(i) == dict:
                        key = list(i.keys())[0]
                        self.add_content(('<a class=\'nav-link\' href=\'#\' onclick=\'grab_html("{}", label="{}")\'>{}</a>\n'.format(key, i[key], key)),
                          indent=10)
                    else:
                        self.add_content(('<a class=\'nav-link\' href=\'#\' onclick=\'grab_html("{}")\'>{}</a>\n'.format(i, i)),
                          indent=10)
        ...
        if histogram_download:
            self.add_content(("<a href='%s' download>" % histogram_download), indent=4)
            self.add_content("<button type='submit' style='margin-right: 15px; cursor:pointer'> <i class='fa fa-download'></i> Histogram Data</button>",
              indent=6)
            self.add_content('</a>', indent=4)
        self.add_content("<div class='collapse navbar-collapse' id='collapsibleNavbar'>\n", indent=4)
        self.add_content("<ul class='navbar-nav flex-row ml-md-auto d-none d-md-flex'style='margin-right:1em;'>\n",
          indent=6)
    ...
    def make_table(self, headings=None, contents=None, heading_span=1, colors=None, accordian_header='Summary Table', accordian=True, format='table-striped table-sm', **kwargs):
        ...
        if accordian:
            ...
            self.make_div(indent=4,
              _class='accordian',
              _style='width: 100%',
              _id=('accordian%s' % label))
            self.make_div(indent=6, _class='card')
            self.make_div(indent=8,
              _class='card-header',
              _style='background-color: #E0E0E0',
              _id='table')
            self.add_content("<h5 class='mb-0'>", indent=10)
            self.make_div(indent=12, _class='row justify-content-center')
            self.add_content(("<button class='btn btn-link collapsed' type='button' data-toggle='collapse' data-target='#collapsetable%s' aria-expanded='false' aria-controls='collapsetable'>" % label),
              indent=14)
            self.add_content(accordian_header)
            self.add_content('</button>')
            self.end_div(indent=12)
            self.end_div(indent=10)
            self.add_content(("<div id='collapsetable%s' class='collapse' aria-labelledby='table' data-parent='#accordian%s'>" % (
             label, label)),
              indent=12)
        ...
        if type(contents) == list:
            ...
            if heading_span > 1:
                self.add_content("<table class='table table-sm' style='max-width:1400px'>\n",
                  indent=22)
            else:
                self.add_content(("<table class='table %s' style='max-width:1400px'>\n" % format),
                  indent=24)
            ...
            if type(contents) == dict:
                ...
                for j in contents.keys():
                    ...
                    self.add_content(("<th colspan='{}' class='text-center'>{}</th>\n".format(len(contents[j][0]), j)),
                      indent=30)
                    self.add_content('</tr>\n', indent=28)
                    self.add_content('<tr>\n', indent=28)
                    for i in headings:
                        self.add_content(("<th colspan='{}' class='text-center'>{}</th>\n".format(heading_span, i)),
                          indent=30)
    ...
    def make_table_of_images(self, contents=None, rows=None, columns=None, code='modal', cli=None, autoscale=False, unique_id=None, captions=None, extra_div=False, mcmc_samples=False):
        ...
        """
        table = tables.table_of_images(contents, rows, columns, (self.html_file), code=code,
          cli=cli,
          autoscale=autoscale,
          unique_id=unique_id,
          captions=captions,
    ...
    def insert_image(self, path, justify='center', code=None):
        ...
        self.make_container()
        _id = path.split('/')[(-1)][:-4]
        string = "<img src='{}' id='{}' alt='No image available' ".format(path, _id) + "style='align-items:center; width:850px; cursor: pointer'"
        if justify == 'center':
    ...
    def make_accordian(self, headings=None, content=None):
        ...
        for num, i in enumerate(headings):
            ...
            self.add_content(("<button class='btn btn-link collapsed' type='button' data-toggle='collapse' data-target='#collapse{}' aria-expanded='false' ".format(i) + "aria-controls='collapse{}'>\n".format(i)),
              indent=10)
            self.add_content(('{}\n'.format(i)), indent=12)
            self.add_content('</button>\n', indent=10)
            self.add_content('</h5>\n', indent=8)
            self.add_content('</div>\n', indent=6)
            self.add_content(("<div id='collapse{}' class='collapse' ".format(i) + "aria-labelledby='{}' data-parent='#accordian'>\n".format(i)),
              indent=6)
            self.add_content("<div class='card-body'>\n", indent=8)
            self.add_content(("<img src='{}' ".format(content[num]) + "alt='No image available' style='width:700px;' " + "class='mx-auto d-block'>\n"),
              indent=10)
            self.add_content('</div>\n', indent=8)
            self.add_content('</div>\n', indent=6)
            self.add_content('</div>\n', indent=4)
    ...
    def make_search_bar(self, sidebar=None, popular_options=None, label=None, code='combine'):
        ...
        self.add_content("<div class='w3-sidebar w3-bar-block w3-border-right sidenav' style='display:none' id='mySidebar'>\n")
        self.add_content("<button onclick='side_bar_close()' class='close'>&times;</button>\n", indent=2)
        if sidebar:
            for i in sidebar:
                self.add_content(("<input type='checkbox' name='type' value='{}' id='{}' style='text-align: center; margin: 0 5px 0;'>{}<br>\n".format(i, i, i, i)),
                  indent=2)

        self.add_content('</div>')
        self.add_content("<div class='row justify-content-center'>")
        self.add_content("<p style='margin-top:2.5em; font-family: Arial-body; font-size:14px'> Input the parameter names that you would like to compare</p>",
          indent=2)
        self.add_content('</div>')
        self.add_content("<div class='row justify-content-center'>\n")
        self.add_content("<input type='text' placeholder='search' id='corner_search'>\n", indent=2)
        self.add_content(('<button type=\'submit\' onclick=\'{}(undefined, label="{}")\' style=\'cursor: pointer\'>Submit</button>\n'.format(code, label)),
          indent=2)
        self.add_content("<button class='w3-button w3-teal w3-xlarge' onclick='side_bar_open()' style='cursor: pointer'>&#8801</button>\n",
          indent=2)
        self.add_content('</div>\n')
        self.add_content("<div class='row justify-content-center'>\n")
        if popular_options:
            for i in popular_options:
                if type(i) == dict and list(i.keys()) == ['all']:
                    self.add_content(('<button type=\'button\' class=\'btn btn-info\' onclick=\'{}("{}", label="{}")\' style=\'margin-left:0.25em; margin-right:0.25em; margin-top: 1.0em; cursor: pointer\'>all</button>\n'.format(code, i['all'], label)),
                      indent=2)
                else:
                    self.add_content(('<button type=\'button\' class=\'btn btn-info\' onclick=\'{}("{}", label="{}")\' style=\'margin-left:0.25em; margin-right:0.25em; margin-top: 1.0em; cursor: pointer\'>{}</button>\n'.format(code, i, label, i)),
                      indent=2)
    ...
    def make_modal_carousel(self, images=None, unique_id=None):
        ...
        if unique_id is not None:
            ...
            modal_id = 'MyModal'
            demo_id = 'demo'
        self.add_content("<div class='modal fade bs-example-modal-lg' tabindex='-1' role='dialog' aria-labelledby='myLargeModalLabel' aria-hidden='true' id='{}' style='margin-top: 200px;'>\n".format(modal_id))
        self.add_content("<div class='modal-dialog modal-lg' style='width:90%'>\n", indent=2)
        self.add_content("<div class='modal-content'>\n", indent=4)
        self.add_content(("<div id='{}' class='carousel slide' data-ride='carousel' data-interval='false'>\n".format(demo_id)),
          indent=6)
        self.add_content("<ul class='carousel-indicators'>\n", indent=8)
        for num, i in enumerate(images):
            if num == 0:
                self.add_content(("<li data-target='#%s' data-slide-to-'%s' class='active'></li>\n" % (
                 demo_id, num)),
                  indent=10)
            self.add_content(("<li data-target='#%s' data-slide-to-'%s'></li>\n" % (
             demo_id, num)),
              indent=10)

        self.add_content(("<li data-target='#{}' data-slide-to='0' class='active'></li>\n".format(demo_id)),
          indent=10)
        self.add_content(("<li data-target='#{}' data-slide-to='1'></li>\n".format(demo_id)),
          indent=10)
        self.add_content(("<li data-target='#{}' data-slide-to='2'></li>\n".format(demo_id)),
          indent=10)
        self.add_content('</ul>\n', indent=8)
        ...
        for num, i in enumerate(images):
            ...
            self.add_content(("<img src={} style='align-items:center;' class='mx-auto d-block'>\n".format(i)),
              indent=12)
            self.add_content('</div>\n', indent=10)

        self.add_content('</div>\n', indent=8)
        self.add_content(("<a class='carousel-control-prev' href='#{}' data-slide='prev'>\n".format(demo_id)),
          indent=8)
        self.add_content("<span class='carousel-control-prev-icon'></span>\n", indent=10)
        self.add_content('</a>\n', indent=8)
        self.add_content(("<a class='carousel-control-next' href='#{}' data-slide='next'>\n".format(demo_id)),
          indent=8)
    ...
    def make_cli_button(self, cli):
        ...
        self.add_content(("<button value='test' class='btn btn-info btn-xs' style='cursor: pointer' data-toggle='popover' data-placement='top' data-content='%s'>Command Line</button>" % cli),
          indent=12)
        self.end_div(0)

    def export(self, filename, csv=True, json=False, shell=False, histogram_dat=None, requirements=False, margin_top='-4em', margin_bottom='5em'):
        ...
        if '.' in filename:
            ...
            basename = filename + '.{}'
        self.add_content("<div class='container' style='margin-top:{}; margin-bottom:{}; max-width: 1400px'>".format(margin_top, margin_bottom))
        json_margin = '0em'
        bash_margin = '0.5em'
        if csv:
            if json:
        ...
        if csv:
            self.add_content('<button type=\'button\' onclick=\'export_table_to_csv("{}")\' class=\'btn btn-outline-secondary btn-table\'>Export to CSV</button>'.format(basename.format('csv')))
        if requirements:
            self.add_content('<button type=\'button\' onclick=\'export_table_to_pip("{}")\' class=\'btn btn-outline-secondary btn-table\'>Export to pip</button>'.format(basename.format('txt')))
        if json:
            self.add_content('<button type=\'button\' onclick=\'export_table_to_json("{}")\' style=\'margin-left: {}\' class=\'btn btn-outline-secondary btn-table\'>Export to JSON</button>'.format(basename.format('json'), json_margin))
        if shell:
            self.add_content('<button type=\'button\' onclick=\'export_table_to_shell("{}")\' style=\'margin-left: {}; margin-bottom: {}\' class=\'btn btn-outline-secondary btn-table\'>Export to bash</button>'.format(basename.format('sh'), json_margin, bash_margin))
        if histogram_dat is not None:
            self.add_content("<a href='%s' download>" % histogram_dat)
            self.add_content("<button type='button' style='margin-left: {}; margin-bottom: {}' class='btn btn-outline-secondary btn-table'>Export to dat</button>".format(json_margin, margin_bottom))
            self.add_content('</a>')
        if csv:
            if json:
                self.end_div()
