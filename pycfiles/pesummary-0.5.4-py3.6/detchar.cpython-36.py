...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/plots/detchar.py
...
def spectrogram(strain, vmin=1e-23, vmax=1e-19, cmap='viridis', ylim=[40, 2000], **kwargs):
    ...
    for num, key in enumerate(list(strain.keys())):
        ...
        try:
            try:
                specgram = strain[key].spectrogram(20,
                  fftlength=8, overlap=4) ** 0.5
            except Exception as e:
                specgram = strain[key].spectrogram(strain[key].duration / 2.0)
            ...
            cbar = plt.colorbar()
            cbar.set_label('ASD [strain/$\\sqrt{Hz}$]')
        except Exception as e:
            logger.info('Failed to generate an spectrogram for {} because {}'.format(key, e))

    return figs


def omegascan(strain, gps, window=4, vmin=0, vmax=25, cmap='viridis', ylim=[40, 2000], **kwargs):
    ...
    for num, key in enumerate(detectors):
        ...
        try:
            try:
                ...
                qtransform = cropped_data.q_transform(gps=gps,
                  outseg=(gps - 0.5 * window, gps + 0.5 * window),
                  logf=True)
            except Exception as e:
            ...
            logger.info('Failed to generate an omegascan for {} because {}'.format(key, e))
            figs[key] = plt.figure(figsize=(12, 6))
