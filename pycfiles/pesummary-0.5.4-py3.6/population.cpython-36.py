...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/core/plots/population.py
...
def scatter_plot(parameters, sample_dict, latex_labels, colors=None, xerr=None, yerr=None):
    ...
    for analysis in runs:
        if all(i in sample_dict[analysis].keys() for i in parameters):
            ...
            logger.warn("'{}' does not include samples for '{}' and/or '{}'. This analysis will not be added to the plot".format(analysis, parameters[0], parameters[1]))
        if xerr is not None:
            if parameters[0] in xerr[analysis].keys():
    ...
    if xerr is not None or yerr is not None:
        plt.errorbar(xdata,
          ydata, color=colors, xerr=(xerrdata.T), yerr=(yerrdata.T), linestyle=' ')
    else:
