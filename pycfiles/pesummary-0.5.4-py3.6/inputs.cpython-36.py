...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/gw/inputs.py
# Compiled at: 2020-05-08 05:31:08
# Size of source mod 2**32: 38919 bytes
import os, numpy as np, pesummary
...
class _GWInput(_Input):
    __doc__ = 'Super class to handle gw specific command line inputs\n    '
 ...
 existing_file, webdir, compare=compare, read_function=GWRead, **kwargs)
        ...
        if f.psd is not None:
            if f.psd != {}:
                for i in labels:
                    if f.psd[i] != {}:
                        psd[i] = {ifo:PSD(f.psd[i][ifo]) for ifo in f.psd[i].keys()}
        ...
        if f.calibration is not None:
            if f.calibration != {}:
                for i in labels:
                    if f.calibration[i] != {}:
                        calibration[i] = {ifo:Calibration(f.calibration[i][ifo]) for ifo in f.calibration[i].keys()}

        data.update({'approximant':{i:j for i, j in zip(labels, [f.approximant[ind] for ind in data['indicies']])}, 
         'psd':psd, 
    ...
    def grab_data_kwargs(self):
        ...
        try:
            for num, label in enumerate(self.labels):
                kwargs[label].update(dict(evolve_spins=(self.evolve_spins),
                  f_low=(self.f_low[num]),
                  approximant=(approx[num]),
                  f_ref=(self.f_ref[num]),
            ...
            logger.warn('Unable to find an f_ref, f_low and approximant for each label. Using and f_ref={}, f_low={} and approximant={} for all result files'.format(self.f_ref[0], self.f_low[0], approx[0]))
            for num, label in enumerate(self.labels):
                kwargs[label].update(dict(evolve_spins=(self.evolve_spins),
                  f_low=(self.f_low[0]),
    ...
    @staticmethod
    def grab_data_from_file(file, label, config=None, injection=None, file_format=None, **kwargs):
 ...
 file, label, config=config, injection=injection, read_function=GWRead, 
    ...
    def _set_samples(self, *args, **kwargs):
        ...
        if 'calibration' not in self.priors:
            self.priors['calibration'] = {label:{} for label in self.labels}
    ...
    def cosmology(self, cosmology):
        ...
        if cosmology.lower() not in available_cosmologies:
            logger.warn('Unrecognised cosmology: {}. Using {} as default'.format(cosmology, conf.cosmology))
    ...
    def approximant(self, approximant):
        if not hasattr(self, '_approximant'):
            ...
            if approximant is None:
                logger.warn('No approximant passed. Waveform plots will not be generated')
            else:
                if approximant is not None:
                    if len(approximant) != len(self.labels):
                        raise InputError('Please pass an approximant for each result file')
    ...
    def gracedb(self, gracedb):
        ...
        if gracedb is not None:
            ...
            if first_letter != 'G':
                if first_letter != 'g':
                    if first_letter != 'S':
                        raise InputError('Invalid GraceDB ID passed. The GraceDB ID must be of the form G0000 or S0000')
    ...
    def detectors(self, detectors):
        ...
        if not detectors:
            for i in self.samples.keys():
                ...
                individual_detectors = sorted([str(i) for i in individual_detectors])
    ...
    def calibration(self, calibration):
        if not hasattr(self, '_calibration'):
            ...
            if calibration != {}:
                prior_data = self.get_psd_or_calibration_data(calibration, self.extract_calibration_data_from_file)
                ...
                for label in self.labels:
                    if hasattr(self.opts, '{}_calibration'.format(label)):
                        ...
                        if cal_data != {} and cal_data is not None:
                            prior_data[label] = {ifo:self.extract_calibration_data_from_file(cal_data[ifo]) for ifo in cal_data.keys()}
            ...
            for num, i in enumerate(self.result_files):
                ...
                if calibration_data is None:
                    data[labels[num]] = {None: None}
                else:
                    if isinstance(f, pesummary.gw.file.formats.pesummary.PESummary):
                        for num in range(len(calibration_data[0])):
                            data[labels[num]] = {j:k for j, k in zip(calibration_data[1][num], calibration_data[0][num])}

                    else:
                        data[labels[num]] = {j:k for j, k in zip(calibration_data[1], calibration_data[0])}
    ...
    def psd(self, psd):
        if not hasattr(self, '_psd'):
            data = {i:{} for i in self.labels}
            if psd != {}:
                data = self.get_psd_or_calibration_data(psd, self.extract_psd_data_from_file)
            else:
                for label in self.labels:
                    if hasattr(self.opts, '{}_psd'.format(label)):
                        psd_data = getattr(self.opts, '{}_psd'.format(label))
                        if psd_data != {} and psd_data is not None:
                            data[label] = {ifo:self.extract_psd_data_from_file(psd_data[ifo]) for ifo in psd_data.keys()}
    ...
    def nsamples_for_skymap(self, nsamples_for_skymap):
        ...
        if nsamples_for_skymap is not None:
            ...
            number_of_samples = [data.number_of_samples for label, data in self.samples.items()]
            if not all(i > self._nsamples_for_skymap for i in number_of_samples):
                min_arg = np.argmin(number_of_samples)
                logger.warn('You have specified that you would like to use {} samples to generate the skymap but the file {} only has {} samples. Reducing the number of samples to generate the skymap to {}'.format(self._nsamples_for_skymap, self.result_files[min_arg], number_of_samples[min_arg], number_of_samples[min_arg]))
            ...
            if isinstance(gwdata, dict):
                for i in gwdata.keys():
                    if not os.path.isfile(gwdata[i]):
                        raise InputError('The file {} does not exist. Please check the path to your strain file'.format(gwdata[i]))

            else:
                if len(gwdata) > 1:
                    logger.warn('Multiple files passed. Only using {}'.format(gwdata[0]))
                if not os.path.isfile(gwdata[0]):
                    raise InputError('The file {} does not exist. Please check the path to your strain file'.format(gwdata[0]))
                gwdata = gwdata[0]
    ...
    def evolve_spins(self, evolve_spins):
        ...
        if evolve_spins:
            ...
            self._evolve_spins = 0.408248290463863
    ...
    def NRSur_fits(self, NRSur_fits):
        ...
        base = "Using the '{}' NRSurrogate model to calculate the remnant quantities"
    ...
    def waveform_fits(self, waveform_fits):
        ...
        if waveform_fits:
            logger.info('Evaluating the remnant quantities using the provided approximant')
    ...
    def pepredicates_probs(self, pepredicates_probs):
        ...
        if self.mcmc_samples:
            logger.debug('Averaging classification probability across mcmc samples')
            classifications[self.labels[0]] = {prior:{key:np.round(np.mean([val[prior][key] for val in classifications.values()]), 3) for key in _probs.keys()} for prior, _probs in list(classifications.values())[0].items()}
    ...
    def pastro_probs(self, pastro_probs):
        ...
        for num, i in enumerate(list(self.samples.keys())):
            ...
            if em_bright is not None:
                probabilities[i] = {'default':em_bright[0],  'population':em_bright[1]}
            else:
                probabilities[i] = None

        if self.mcmc_samples:
            logger.debug('Averaging em_bright probability across mcmc samples')
            probabilities[self.labels[0]] = {prior:{key:np.round(np.mean([val[prior][key] for val in probabilities.values()]), 3) for key in _probs.keys()} for prior, _probs in list(probabilities.values())[0].items()}
    ...
    def extract_psd_data_from_file(file):
        ...
        """
        from pesummary.gw.file.psd import PSD
        general = 'Failed to read in PSD data because {}. The PSD plot will be generated and the PSD data will not be added to the metafile.'
        try:
            f = np.genfromtxt(file, skip_footer=2)
            return PSD(f)
        except FileNotFoundError:
            logger.info(general.format('the file {} does not exist'.format(file)))
            return {}
    ...
    def extract_calibration_data_from_file(file):
        ...
        """
        general = 'Failed to read in calibration data because {}. The calibration plot will not be generated and the calibration data will not be added to the metafile'
        from pesummary.gw.file.calibration import Calibration
        try:
    ...
    def get_ifo_from_file_name(file):
        ...
        if any(j in file_name for j in ('H1', '_0', 'IFO0')):
            ...
            if any(j in file_name for j in ('L1', '_1', 'IFO1')):
                ifo = 'L1'
            else:
                if any(j in file_name for j in ('V1', '_2', 'IFO2')):
                    ifo = 'V1'
                else:
                    ifo = file_name
        return ifo
            ...
            if isinstance(input, dict):
                ...
                if isinstance(input[keys[0]], list):
                    if not all(len(input[i]) == len(self.labels) for i in list(keys)):
                        raise InputError('Please ensure the number of calibration/psd files matches the number of result files passed')
                    for idx in range(len(input[keys[0]])):
                        data[self.labels[idx]] = {i:executable(input[i][idx]) for i in list(keys)}

            else:
                if isinstance(input, dict):
                    for i in self.labels:
                        data[i] = {j:executable(input[j]) for j in list(input.keys())}

                else:
                    if isinstance(input, list):
                        for i in self.labels:
                            data[i] = {self.get_ifo_from_file_name(j):executable(j) for j in input}

                    else:
                        raise InputError("Did not understand the psd/calibration input. Please use the following format 'H1:path/to/file'")
            return data
    ...
    def grab_priors_from_inputs(self, priors):
        ...
        if priors is not None:
            ...
            if len(priors) != len(self.labels):
                if len(priors) == 1:
                    logger.warn('You have only specified a single prior file for {} result files. Assuming the same prior file for all result files'.format(len(self.labels)))
            ...
            if len(priors) != len(self.labels):
                raise InputError('Please provide a prior file for each result file')
            else:
                for num, i in enumerate(priors):
                    ...
                    if self.labels[num] in self.grab_data_kwargs.keys():
                        grab_data_kwargs = self.grab_data_kwargs[self.labels[num]]
                    else:
...
class GWInput(_GWInput, Input):
    ...
    def __init__(self, opts):
        ...
        if not self.no_ligo_skymap and self.multi_process > 1:
            ...
            self.multi_threading_for_plots = int(total / 2.0)
            self.multi_threading_for_skymap = total - self.multi_threading_for_plots
            logger.info('Assigning {} process{}to skymap generation and {} process{}to other plots'.format(self.multi_threading_for_skymap, 'es ' if self.multi_threading_for_skymap > 1 else ' ', self.multi_threading_for_plots, 'es ' if self.multi_threading_for_plots > 1 else ' '))
        ...
        for label in self.labels:
            if self.psd[label] != {}:
                for ifo in self.psd[label].keys():
                    self.psd[label][ifo].save_to_file(os.path.join(self.webdir, 'psds', '{}_{}_psd.dat'.format(label, ifo)))

            if label in self.priors['calibration'].keys() and self.priors['calibration'][label] != {}:
                for ifo in self.priors['calibration'][label].keys():
                    self.priors['calibration'][label][ifo].save_to_file(os.path.join(self.webdir, 'calibration', '{}_{}_cal.txt'.format(label, ifo)))
...
class GWPostProcessing(PostProcessing):
    ...
    def maxL_samples(self, maxL_samples):
        ...
        maxL_samples = {i:{j:key_data[i][j]['maxL'] for j in key_data[i].keys()} for i in key_data.keys()}
        for i in self.labels:
    ...
    def grab_key_data_from_result_files(self):
        ...
        def smart_average(data, _type='mean', multiple=False):
            if not multiple:
                ...
                if not multiple:
                    if _type == 'maxL':
                        return data.maxL
                return multiple or data.average(_type)
            else:
        ...
        key_data = {key:{j:{_type:smart_average((val[j]), multiple=(self.mcmc_samples), _type=_type) for _type in ('mean',
                                                                                                                   'median',
                                                                                                                   'std',
                                                                                                                   'maxL')} for j in val.keys()} for key, val in self.samples.items()}
        return key_data