...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/core/file/mcmc.py
...
def _number_of_negative_steps(samples, logger_level='debug'):
    ...
    step_param = [alternative for alternative in STEP_NUMBER_PARAMS if alternative in parameters]
    if not len(step_param):
        logger.warn('Unable to find a step number in the MCMCSamplesDict object. Aborting discard')
        return {key:None for key in _samples.keys()}
    else:
        if len(step_param) > 1:
            step_param = step_param[0]
            getattr(logger, logger_level)('Multiple columns found with possible step numbers. Using {}'.format(step_param))
        else:
            step_param = step_param[0]
        keys = _samples.keys()
        step_idx = [np.arange(_samples[key].number_of_samples)[(_samples[key][step_param] > 0)] for key in _samples.keys()]
        return {key:(step[0] if len(step_idx) else 0) for key, step in zip(keys, step_idx)}
...
def burnin_by_first_n(samples, N, step_number=False, logger_level='debug'):
    ...
    if step_number:
        n_samples = {key:item + N for key, item in _number_of_negative_steps(_samples,
          logger_level=logger_level).items()}
    return _samples.discard_samples(n_samples)