...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pesummary/cli/summarypageslw.py
...
def command_line():
    ...
    parser.add_argument('--labels', dest='labels', help='labels used to distinguish runs',
      nargs='+',
      default=None)
    parser.add_argument('--parameters', dest='parameters', nargs='+', help='list of parameters you wish to include in the summarypages',
      default=None)
    return parser
...
class LWInput(_GWInput):
    ...
    def parameters_to_include(self, parameters_to_include):
        ...
        if parameters_to_include is None:
            raise InputError('Please provide a list of parameters you wish to plot')
        for num, label in enumerate(self.labels):
            params = [i for i in parameters_to_include if i in list(self.samples[label].keys())]
            not_included_params = [i for i in parameters_to_include if i not in list(self.samples[label].keys())]
            if len(not_included_params) != 0:
                logger.warn('The parameters {} are not in the file {}. They will not be included in the final pages'.format(', '.join(not_included_params), self.result_files[num]))
...
class LWWebpageGeneration(_WebpageGeneration):
    ...
    def __init__(self, webdir=None, labels=None, samples=None, colors=None, user=None, config=None, same_parameters=None, baseurl=None, file_versions=None):
        super(LWWebpageGeneration, self).__init__(webdir=webdir,
          samples=samples,
          labels=labels,
          publication=False,
          user=user,
    ...
    def make_navbar_for_result_page(self):
        ...
        final_links = {i:['home', ['Result Pages', self._result_page_links()], links[i]] for i in self.labels}
        if self.make_comparison:
...
def make_plots(inputs):
    ...
    plotting_object = _PlotGeneration(webdir=(inputs.webdir),
...
def make_webpages(inputs):
    ...
    """
    from pesummary.core.webpage.main import _WebpageGeneration
    webpage_object = LWWebpageGeneration(webdir=(inputs.webdir),
      labels=(inputs.labels),
      samples=(inputs.samples),
      colors=(inputs.colors),
      user=(inputs.user),
