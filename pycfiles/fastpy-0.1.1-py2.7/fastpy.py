...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastpy/fastpy.py
# Compiled at: 2016-07-10 22:45:16
import logging, sys, numpy as np
from itertools import tee, izip
import llvm.core as lc, llvm.passes as lp, llvm.ee as le
...
def arg_pytype(arg):
    if isinstance(arg, np.ndarray):
        ...
        if arg.dtype == np.dtype('int64'):
            return array(int64)
        if arg.dtype == np.dtype('double'):
            return array(double64)
        if arg.dtype == np.dtype('float'):
            return array(float32)
    else:
        if isinstance(arg, int) & (arg < sys.maxint):
            return int64
...
def codegen(ast, specializer, retty, argtys):
    ...
    pms = lp.build_pass_managers(tm=tm, fpm=False, mod=module, opt=3, vectorize=False, loop_vectorize=True)
    pms.pm.run(module)
    debug(cgen.function)
