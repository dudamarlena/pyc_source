...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fastpy/type_mapping.py
# Compiled at: 2016-07-10 22:45:16
import ctypes, llvm.core as lc, numpy as np
_nptypemap = {'i': ctypes.c_int, 
   'f': ctypes.c_float, 
   'd': ctypes.c_double}
...
def wrap_type(llvm_type):
    ...
    if kind == lc.TYPE_INTEGER:
        ...
        ctype = type(ctypes.Structure)(struct_name, (ctypes.Structure,), {'__module__': 'numpile'})
        fields = [ (name, wrap_type(elem)) for name, elem in zip(names, llvm_type.elements)
                 ]
