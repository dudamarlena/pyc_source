...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/centinel/backend.py
# Compiled at: 2016-09-18 11:38:10
from base64 import urlsafe_b64encode
import glob, json, logging, os, re, requests, time, uuid, centinel.utils as utils
...
class User():
    ...
    def request(self, slug):
        ...
        try:
            req = requests.get(url, auth=self.auth, proxies=self.config['proxy']['proxy'], verify=self.verify)
    ...
    def recommended_version(self):
        try:
            ...
            logging.exception('Exception trying to get recommended version: %s ' % exp)
            raise exp
    ...
    def submit_result(self, file_name):
        ...
        with open(file_name) as (result_file):
            ...
            try:
                req = requests.post(url, files=files, auth=self.auth, proxies=self.config['proxy']['proxy'], timeout=timeout, verify=self.verify)
                req.raise_for_status()
                if 'delete_after_sync' in self.config['results'].keys() and self.config['results']['delete_after_sync']:
                    os.remove(file_name)
            except Exception as exp:
    ...
    def sync_scheduler(self):
        ...
        if not os.path.exists(sched_filename):
            with open(sched_filename, 'w') as (file_p):
                json.dump(server_sched, file_p, indent=2, separators=(',', ': '))
            return
        client_sched = {}
        try:
        ...
        with open(sched_filename, 'w') as (file_p):
            json.dump(client_sched, file_p, indent=2, separators=(',', ': '))
    ...
    def download_experiment(self, name):
        ...
        name = '%s' % name
        with open(os.path.join(self.config['dirs']['experiments_dir'], name), 'w') as (exp_fh):
    ...
    def download_input_file(self, name):
        ...
        name = '%s' % name
        with open(os.path.join(self.config['dirs']['data_dir'], name), 'w') as (exp_fh):
            exp_fh.write(req.content)

    def register(self, username, password):
        logging.info('Registering new user %s' % username)
        url = '%s/%s' % (self.config['server']['server_url'], 'register')
        payload = {'username': username, 'password': password, 'is_vpn': self.config['user'].get('is_vpn')}
        headers = {'content-type': 'application/json'}
        try:
            req = requests.post(url, data=json.dumps(payload), proxies=self.config['proxy']['proxy'], headers=headers, verify=self.verify)
            req.raise_for_status()
            return req.json()
        except Exception as exp:
    ...
    def create_user(self):
        ...
        try:
            ...
            with open(self.config['server']['login_file'], 'w') as (login_fh):
                ...
                json.dump(login_details, login_fh, indent=2, separators=(',', ': '))
        except Exception as exp:
...
def sync(config):
    ...
    result_files = glob.glob(os.path.join(config['dirs']['results_dir'], '[!_]*.bz2'))
    if config['results']['upload_pcaps'] is False:
        for pcap_file in glob.glob(os.path.join(config['dirs']['results_dir'], '[!_]*.pcap.bz2')):
    ...
    for path in result_files:
        try:
            ...
            if re.search('418', str(exp)) is not None:
                logging.error('You have not completed the informed consent and will be unable to submit results or get new experiments until you do.')
    ...
    if time.time() - start > config['server']['total_timeout']:
        ...
        dload_exps, del_exps = utils.compute_files_to_download(client_exps, server_exps)
        ...
        try:
            ...
            logging.exception('Unable to retrive user inputs due to Exception: %s. Preempting' % exp)
            return

        if time.time() - start > config['server']['total_timeout']:
            logging.error('Interaction with server took too long. Preempting')
            return
        dload_inputs, del_inputs = utils.compute_files_to_download(client_inputs, server_inputs)
...
def set_vpn_info(config, ip=None, country=None):
    logging.info('Setting country as %s and IP address as %s' % (
     country, ip))
    try:
        user = User(config)
    except Exception as exp:
...
def get_meta(config, ip=''):
    url = '%s/%s/%s' % (config['server']['server_url'],
     'meta', ip)
    try:
        req = requests.get(url, proxies=config['proxy']['proxy'], verify=config['server']['verify'], timeout=10)
        req.raise_for_status()
        return req.json()
    except Exception as exp:
