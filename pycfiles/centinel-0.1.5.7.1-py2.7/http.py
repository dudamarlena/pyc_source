...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/centinel/primitives/http.py
# Compiled at: 2017-03-31 21:58:48
import base64, logging, threading, time, random, BeautifulSoup, re
...
def get_request(netloc, path='/', headers=None, ssl=False, external=None, url=None, log_prefix=''):
    http_results = {}
    if headers is None:
        headers = {'User-Agent': random.choice(user_agent_pool)}
    else:
        if type(headers) is dict and 'User-Agent' not in headers:
            headers['user-Agent'] = random.choice(user_agent_pool)
        first_response = _get_http_request(netloc, path, headers, ssl)
        if 'failure' in first_response['response']:
            first_response_information = {'redirect_count': 0, 'redirect_loop': False, 'full_url': url, 
    ...
    if is_redirecting:
        ...
        first_response_information = {'full_url': url, 'response': first_response['response'], 'request': first_response['request']}
        http_results['redirects'][0] = first_response_information
        redirect_http_result = None
        redirect_number = 1
        redirect_failed = False
        while redirect_http_result is None or is_redirecting and not redirect_failed and redirect_number <= REDIRECT_LOOP_THRESHOLD:
            ...
            redirect_information = {'full_url': redirect_url, 'response': redirect_http_result['response'], 
               'request': redirect_http_result['request']}
            http_results['redirects'][redirect_number] = redirect_information
            redirect_number += 1

    else:
        first_response_information = {'redirect_count': 0, 'redirect_loop': False, 
           'full_url': url, 
           'response': first_response['response'], 
           'request': first_response['request']}
        http_results = first_response_information
