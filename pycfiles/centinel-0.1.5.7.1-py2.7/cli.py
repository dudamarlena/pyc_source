...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/centinel/vpn/cli.py
# Compiled at: 2016-12-27 09:39:39
import argparse, logging
from random import shuffle
import os, time, sys, signal, dns.resolver, json, centinel.backend, centinel.client, centinel.config, centinel.vpn.openvpn as openvpn, centinel.vpn.hma as hma, centinel.vpn.ipvanish as ipvanish, centinel.vpn.purevpn as purevpn, centinel.vpn.vpngate as vpngate
PID_FILE = '/tmp/centinel.lock'

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--auth-file', '-u', dest='auth_file', default=None, help='File with HMA username on first line, \nHMA password on second line')
    parser.add_argument('--crt-file', '-r', dest='crt_file', default=None, help='Certificate file for the current vpn\nprovider, if provided')
    parser.add_argument('--tls-auth', '-t', dest='tls_auth', default=None, help='Key for additional layer of authentication')
    parser.add_argument('--key-direction', '-k', dest='key_direction', default=None, help='Key direction for tls auth, must specify when tls-auth is used')
    parser.add_argument('--reduce-endpoint', dest='reduce_vp', action='store_true', default=False, help='Reduce the number of vantage points by only connect to one vantage point for each AS and country combination')
    g1 = parser.add_mutually_exclusive_group()
    g1.add_argument('--create-hma-configs', dest='create_HMA', action='store_true', help='Create the openvpn config files for HMA')
    g1.add_argument('--create-ipvanish-configs', dest='create_IPVANISH', action='store_true', help='Create the openvpn config files for IPVanish')
    g1.add_argument('--create-purevpn-configs', dest='create_PUREVPN', action='store_true', help='Create the openvpn config files for PureVPN')
    g1.add_argument('--create-vpngate-configs', dest='create_VPNGATE', action='store_true', help='Create the openvpn config files for VPN Gate')
    parser.add_argument('--shuffle', '-s', dest='shuffle_lists', action='store_true', default=False, help='Randomize the order of vantage points')
    parser.add_argument('--exclude', '-e', dest='exclude_list', default=None, help='Countries to exclude when scanning (comma separated two letter country codes)')
    parser.add_argument('--log-file', '-l', dest='log_file', default=None, help='Log file location')
    g2 = parser.add_mutually_exclusive_group(required=True)
    g2.add_argument('--directory', '-d', dest='directory', help='Directory with experiments, config files, etc.')
    create_conf_help = 'Create configuration files for the given openvpn config files so that we can treat each one as a client. The argument should be a directory with a subdirectory called openvpn that contains the openvpn config files'
    g2.add_argument('--create-config', '-c', help=create_conf_help, dest='create_conf_dir')
    parser.add_argument('--vm-num', dest='vm_num', type=int, default=1, help='Specify the number of VMs running concurrently')
    parser.add_argument('--vm-index', dest='vm_index', type=int, default=1, help='The index of current VM, must be >= 1 and <= vm_num')
    return parser.parse_args()


def scan_vpns(directory, auth_file, crt_file, tls_auth, key_direction, exclude_list, shuffle_lists, vm_num, vm_index, reduce_vp):
    """
    ...
    if reduce_vp:
        ...
        for filename in conf_list:
            ...
            try:
                ...
                if 'country' in meta and 'as_number' in meta and meta['country'] and meta['as_number']:
    ...
    if external_ip is None:
        ...
        for filename in conf_list:
            ...
            if current_ip is None:
                        ...
                        for task in sched_info:
                            if 'python_exps' in sched_info[task] and 'baseline' in sched_info[task]['python_exps']:
                                if 'params' in sched_info[task]['python_exps']['baseline']:
                                    sched_info[task]['python_exps']['baseline']['params']['exclude_nameservers'] = local_nameservers
                                else:
                                    sched_info[task]['python_exps']['baseline']['params'] = {'exclude_nameservers': local_nameservers}
                ...
                vpn = openvpn.OpenVPN(timeout=60, auth_file=auth_file, config_file=vpn_config, crt_file=crt_file, tls_auth=tls_auth, key_direction=key_direction)
                vpn.start()
                if not vpn.started:
...
def experiments_available(config):
    logging.info('Starting to check for experiments with %s', config['server']['server_url'])
    try:
...
def run():
    ...
    try:
        ...
        sys.stderr.write('Error to writing the lock file: %s\n' % exp)
        sys.exit(1)
...
def _run():
    ...
    if args.vm_num < 1:
        ...
        if args.create_conf_dir:
            ...
            if args.tls_auth is not None and args.key_direction is None or args.tls_auth is None and args.key_direction is not None:
                logging.error('tls_auth and key_direction must be specified together!')
                return
            scan_vpns(directory=args.directory, auth_file=args.auth_file, crt_file=args.crt_file, tls_auth=args.tls_auth, key_direction=args.key_direction, exclude_list=args.exclude_list, shuffle_lists=args.shuffle_lists, vm_num=args.vm_num, vm_index=args.vm_index, reduce_vp=args.reduce_vp)
        return
