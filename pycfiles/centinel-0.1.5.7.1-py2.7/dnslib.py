...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/centinel/primitives/dnslib.py
# Compiled at: 2017-02-27 23:23:39
from base64 import b64encode
import dns.rdatatype, dns.message, dns.resolver, logging, select, socket, threading, time
...
def lookup_domain(domain, nameservers=[], rtype='A', exclude_nameservers=[], timeout=2):
    ...
    dns_exp = DNSQuery(domains=[domain], nameservers=nameservers, rtype=rtype, exclude_nameservers=exclude_nameservers, timeout=timeout)
    return dns_exp.lookup_domain(domain)


def lookup_domains(domains, results={}, nameservers=[], exclude_nameservers=[], rtype='A', timeout=2):
    dns_exp = DNSQuery(domains=domains, results=results, nameservers=nameservers, rtype=rtype, exclude_nameservers=exclude_nameservers, timeout=timeout)
...
class DNSQuery:
    ...
    def __init__(self, domains=[], results={}, nameservers=[], exclude_nameservers=[], rtype='A', timeout=10, max_threads=100):
    ...
    def send_chaos_queries(self):
        ...
        for name in names:
            ...
            for nameserver in self.nameservers:
                ...
                query = dns.message.make_query(name, dns.rdatatype.from_text('TXT'), dns.rdataclass.from_text('CH'))
    ...
    def lookup_domains(self):
        ...
        for domain in self.domains:
            for nameserver in self.nameservers:
                ...
                thread = threading.Thread(target=self.lookup_domain, args=(
                 domain, nameserver,
                 log_prefix))
                thread.setDaemon(1)
    ...
    def lookup_domain(self, domain, nameserver=None, log_prefix=''):
        ...
        if interrupt:
            ...
            logging.debug('%sQuerying DNS enteries for %s (nameserver: %s).' % (
             log_prefix, domain, nameserver))
            request = dns.message.make_query(domain, dns.rdatatype.from_text(self.rtype))
            ...
            try:
                ...
                logging.debug('%sQuerying DNS enteries for %s (nameserver: %s) timed out!' % (
                 log_prefix, domain, nameserver))
                sock.close()
