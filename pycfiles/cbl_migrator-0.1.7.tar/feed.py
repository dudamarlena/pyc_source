...
# Embedded file name: build/bdist.linux-i686/egg/cblog/controllers/feed.py
# Compiled at: 2006-12-13 19:49:51
__all__ = [
 'CBlogFeedController']
import logging
...
class CBlogFeedController(FeedController):
    ...
    expose(template='cblog.templates.feed.atom1_0', format='xml', content_type='application/atom+xml')
    ...
    def atom1_0(self, *args, **kw):
        ...
        feed.update(self.get_feed_info('atom1_0', format=kw.get('format', 'summary'), **url_params))
        feed['id'] = feed['href']
        return feed

    def get_feed_data(self, **kw):
        """Fetch feed data from model and assemble into dictionary."""
        feed = dict(title=self.title, author=dict(name=self.author), subtitle=self.subtitle, categories=[])
        if config.get('cblog.feed.logo'):
            feed['logo'] = url('/static/images/' + config.get('cblog.feed.logo'))
        tag = kw.get('tag')
        if tag:
            try:
                tag = model.Tag.select(func.LOWER(model.Tag.q.name) == tag.lower())[0]
            except (SQLObjectNotFound, IndexError):
                raise cherrypy.HTTPError(status=404)
            else:
                entries = list(tag.entries)
                feed['categories'].append(tag.name)
                feed['title'] += ' (%s %s)' % (_('Category'), tag.name)
        else:
        ...
        for entry in entries:
            feed_entry = dict(link=absolute_url('article/%s' % entry.id), published=entry.created, title=entry.title, author=dict(name=entry.author.display_name), summary=entry.teaser, categories=[ tag.name for tag in entry.tags ])
    ...
    def get_feed_info(self, version='atom1_0', **kw):
        ...
        """
        info = dict(title=config.get('cblog.title'), href=self.get_feed_url(version, **kw))
        tag = kw.get('tag')
        if tag:
            info['title'] += ' (%s %s)' % (_('Category'), tag)
        return info
    ...
    def get_feed_url(self, version='atom1_0', name=None, format='full', **kw):
        ...
        if name is None:
            name = version + '.xml'
        base_url = absolute_url(('%s/%s/%s/%s' % (self.path, version, format, name)), **kw)
        return base_url