...
# [GCC 8.4.0]
# Embedded file name: /Users/jacob/Projects/python-storymarket/tests/fakeserver.py
# Compiled at: 2011-05-30 18:13:02
"""
...
class FakeClient(StorymarketClient):
    ...
    def get_content_category(self, **kw):
        return (
         200, [self.get_content_category_1()[1]])
    ...
    def get_content_category_1(self, **kw):
        return (
         ...
         {'id': 1, 
            'name': 'Technology', 
            'links': [
                    {'rel': 'self', 'href': 'content/category/1/', 
                       'allowed_methods': [
                                         'GET']}], 
            'description': 'Tech!'})
    ...
    def get_content_sub_type(self, **kw):
        return (
         200, [self.get_content_sub_type_proto()[1]])

    def get_content_sub_type_proto(self, type_model='text', id=1, **kw):
        return (200,
         {'id': 1, 
            'name': type_model.title(), 
            'is_default': True, 
            'links': [
                    {'rel': 'self', 'href': 'content/sub_type/%d/' % id, 
                       'allowed_methods': [
                                         'GET']}]})
    ...
    def get_content_data(self, **kw):
        return (
         200, [self.get_content_data_1()[1]])

    def get_content_data_1(self, **kw):
        return (
         200,
         self._content_dict('data', title='Data', data='http://example.com/datas/2010/05/14/revsys.ppt'))

    def get_content_audio(self, **kw):
        return (
         200, [self.get_content_audio_1()[1]])

    def get_content_audio_1(self, **kw):
        return (
         200,
         self._content_dict('audio', title='Audio', audio='http://example.com/audios/2010/05/14/revsys.avi'))

    def get_orgs(self, **kw):
        return (
         200, [self.get_orgs_1()[1]])
    ...
    def get_orgs_1(self, **kw):
        return (
         ...
         {'id': 1, 
            'name': 'Test Org', 
            'links': [
                    {'rel': 'self', 'href': 'orgs/1/', 
                       'allowed_methods': [
                                         'GET']}]})

    def get_content_photo(self, **kw):
        return (
         200, [self.get_content_photo_1()[1]])

    def get_content_photo_1(self, **kw):
        return (
         200,
         self._content_dict('photo', title='Photo', photo='http://eample.com/photos/2010/05/15/cat.jpg', caption='My cat'))

    def get_pricing(self, **kw):
        return (
         200, [self.get_pricing_1()[1]])
    ...
    def get_pricing_1(self, **kw):
        return (
         ...
         {'id': 1, 
            ...
            'links': [
                    {'rel': 'self', 'href': 'pricing/1/', 
                       'allowed_methods': [
                                         'GET']}], 
            'default': True, 
            'org_prices': [], 'base_price_small': None, 
            ...
            'group_prices': [], 'description': ''})

    def get_rights(self, **kw):
        return (
         200, [self.get_rights_1()[1]])
    ...
    def get_rights_1(self, **kw):
        return (
         ...
         {'id': 1, 
            ...
            'name': 'Public', 
            'links': [
                    {'rel': 'self', 'href': 'rights/1/', 
                       'allowed_methods': [
                                         'GET']}], 
            'default': False, 
            'tv_only': False, 
            'online_only': False, 
            'included_states': [], 'print_only': False, 
            'org': self.get_orgs_1(), 
            'exclude': True, 
            'include': False, 
            'excluded_states': [], 'public': True, 
            'description': ''})

    def get_content_text(self, **kw):
        return (
         200, [self.get_content_text_1()[1]])

    def get_content_text_1(self, **kw):
        return (
         200, self._content_dict('text', title='Text', content='lorum ipsum...'))

    def get_content_video(self, **kw):
        return (
         200, [self.get_content_video_1()[1]])

    def get_content_video_1(self, **kw):
        return (
         200,
         self._content_dict('video', title='Video', photo='http://eample.com/photos/2010/05/15/cat.mov'))
    ...
    def _check_post_method(self, body, required=(), optional=()):
        assert_has_keys(body, required=required + ('org', 'category', 'title'), optional=optional + ('author',
                                                                                                     'sub_type',
                                                                                                     'one_off_author',
                                                                                                     'fact_checked',
                                                                                                     'description',
    ...
    def get_content_package(self, **kw):
        return (
         200, [self.get_content_package_1()[1]])

    def get_content_package_1(self, **kw):
        return (
         200,
         self._content_dict('package', audio_items=[
          self.get_content_audio_1()[1]], data_items=[
          self.get_content_data_1()[1]], photo_items=[
          self.get_content_photo_1()[1]], text_items=[
          self.get_content_text_1()[1]], video_items=[
          self.get_content_video_1()[1]]))

    def post_content_package(self, **kw):
        assert_in('body', kw)
        related_keys = ('audio_items', 'data_items', 'photo_items', 'text_items', 'video_items')
    ...
    def _content_dict(self, type, **kw):
        ...
        d = {'id': 1, 
           ...
           'sub_type': self.get_content_sub_type_proto(type_model='text')[1], 
           'uploaded_by': {'username': 'frank', 
                           'first_name': 'Frank', 
                           'last_name': 'Wiles', 
                           'email': 'frank@revsys.com'}, 
           'description': '', 
           'links': [
                   {'rel': 'self', 'href': 'content/%s/1/' % type, 
                      'allowed_methods': [
                                        'GET', 'POST', 'PUT', 'DELETE']}], 
           'title': 'Test resource', 
