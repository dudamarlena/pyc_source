...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hanzo/arc2warc.py
# Compiled at: 2013-01-14 05:25:26
"""arc2warc - convert one arc to a new warc"""
import os, sys, hashlib, uuid, sys, os.path, datetime, socket
...
parser.set_defaults(output_directory=None, limit=None, log_level='info', gzip=False, description='', operator='', publisher='', audience='', resource=[], response=[])
...
class ArcTransformer(object):
    ...
    def convert_filedesc(self, record):
        ...
        warcmeta_headers = [(WarcRecord.TYPE, WarcRecord.METADATA),
         (
...
def main(argv):
    ...
    warcinfo = warcinfo_fields(description=options.description, operator=options.operator, publisher=options.publisher, audience=options.audience)
