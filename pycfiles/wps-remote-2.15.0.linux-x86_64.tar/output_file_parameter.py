...
# [GCC 8.4.0]
# Embedded file name: /home/alessio/Envs/remote-wps/lib/python2.7/site-packages/wpsremote/output_file_parameter.py
# Compiled at: 2018-09-28 10:53:07
__author__ = 'Alessio Fabiani'
__copyright__ = 'Copyright 2016 Open Source Geospatial Foundation - all rights reserved'
__license__ = 'GPL'
import os, tempfile, path, json, uuid
...
class OutputFileParameter(object):
    ...
    def as_json_string(self):
        res = {}
        attrib_to_convert = [
         '_type', '_description', '_title', '_output_mime_type', '_publish_as_layer', '_publish_layer_name', '_publish_default_style', '_publish_target_workspace']
    ...
    def get_value(self):
        if self._backup_on_wps_execution_shared_dir != None and self._backup_on_wps_execution_shared_dir.lower() == 'true' and self._wps_execution_shared_dir != None:
            ...
            if self._upload_data != None and self._upload_data.lower() == 'true' and self._uploader != None:
                unique_dirname = str(uuid.uuid4())
...
class OWCFileParameter(object):
    ...
    def as_json_string(self):
        res = {}
        attrib_to_convert = [
         '_type', '_description', '_title', '_output_mime_type', '_publish_as_layer', '_publish_layer_name', '_files_to_publish', '_default_styles', '_target_workspaces']
        attribute_list = [ a for a in dir(self) if not a.startswith('__') and not callable(getattr(self, a)) ]
        attribute_list_filtered = [ x for x in attribute_list if x in attrib_to_convert ]
        for a in attribute_list_filtered:
            res[a[1:]] = getattr(self, a)
