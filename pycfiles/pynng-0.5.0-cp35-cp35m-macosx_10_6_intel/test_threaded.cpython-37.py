...
# Embedded file name: /Users/leland/PycharmProjects/pynndescent/pynndescent/tests/test_threaded.py
# Compiled at: 2019-12-12 19:26:45
# Size of source mod 2**32: 11774 bytes
...
def test_effective_n_jobs_with_context():
    ...
    assert_equal(threaded.effective_n_jobs_with_context(-1), joblib.cpu_count(), 'Use all cores with num_jobs=-1')
    assert_equal(threaded.effective_n_jobs_with_context(2), 2, 'Use n_jobs if specified')
    with joblib.parallel_backend('threading'):
        assert_equal(threaded.effective_n_jobs_with_context(), joblib.cpu_count(), 'Use all cores with context manager')
    with joblib.parallel_backend('threading', n_jobs=3):
        assert_equal(threaded.effective_n_jobs_with_context(), 3, 'Use n_jobs from context manager')
    with joblib.parallel_backend('threading', n_jobs=3):
        assert_equal(threaded.effective_n_jobs_with_context(2), 2, 'Use n_jobs specified rather than from context manager')


def test_init_random():
    current_graph = utils.make_heap(data.shape[0], n_neighbors)
    pynndescent_.init_random(n_neighbors,
      data,
      current_graph,
      dist,
      dist_args,
    ...
    threaded.init_random(current_graph_threaded,
      data,
      dist,
      dist_args,
      n_neighbors,
...
def test_init_rp_tree():
    ...
    leaf_array = rptree_leaf_array(_rp_forest)
    parallel = joblib.Parallel(n_jobs=2, prefer='threads')
    threaded.init_rp_tree(data, dist, dist_args, current_graph_threaded, leaf_array, chunk_size, parallel)
    assert_allclose(current_graph_threaded, current_graph)
...
def test_new_build_candidates():
    ...
    pynndescent_.init_random(n_neighbors,
      ...
      (new_rng_state()),
      seed_per_row=True)
    new_candidate_neighbors, old_candidate_neighbors = utils.new_build_candidates(current_graph,
      n_vertices,
      n_neighbors,
      max_candidates,
      rng_state=(new_rng_state()),
    ...
    pynndescent_.init_random(n_neighbors,
      ...
      seed_per_row=True)
    parallel = joblib.Parallel(n_jobs=2, prefer='threads')
    new_candidate_neighbors_threaded, old_candidate_neighbors_threaded = threaded.new_build_candidates(current_graph,
      n_vertices,
      n_neighbors,
      max_candidates,
      chunk_size=chunk_size,
...
def test_mark_candidate_results():
    ...
    pynndescent_.nn_descent_internal_low_memory_parallel(current_graph,
      data, n_neighbors, (new_rng_state()), n_iters=2, seed_per_row=True)
    ...
    new_candidate_neighbors, old_candidate_neighbors = utils.new_build_candidates(current_graph,
      ...
      seed_per_row=True)
    parallel = joblib.Parallel(n_jobs=2, prefer='threads')
    new_candidate_neighbors_threaded, old_candidate_neighbors_threaded = threaded.new_build_candidates(current_graph_threaded,
      n_vertices,
      n_neighbors,
      max_candidates,
      chunk_size=chunk_size,
...
def test_nn_descent():
    ...
    n_neighbors = 25
    data = np.random.rand(N, D).astype(np.float32)
    nn_indices, nn_distances = NNDescent(data,
      ...
      delta=0,
      tree_init=False,
      seed_per_row=True)._neighbor_graph
    for i in range(data.shape[0]):
        assert_equal(len(nn_indices[i]), len(np.unique(nn_indices[i])), 'Duplicate graph_indices in unthreaded knn graph')

    nn_indices_threaded, nn_distances_threaded = NNDescent(data,
      n_neighbors=n_neighbors,
      max_candidates=max_candidates,
      n_iters=1,
      random_state=42,
      ...
      tree_init=False,
      seed_per_row=True,
      n_jobs=2)._neighbor_graph
    for i in range(data.shape[0]):
        assert_equal(len(nn_indices_threaded[i]), len(np.unique(nn_indices_threaded[i])), 'Duplicate graph_indices in threaded knn graph')
