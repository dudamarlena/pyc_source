...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/dbrow.py
# Compiled at: 2008-01-23 23:19:32
...
class DBRow(propertied.Propertied):
    ...
    __data = basic.BasicProperty('_DBRow__data', 'The original data for this row, from the data-base')
    __newValues = common.DictionaryProperty('_DBRow__newValues', 'Newly set, but not-yet committed values')
    __wrappedValues = common.DictionaryProperty('_DBRow__wrappedValues', 'Retrieved and wrapped versions for data-values')

    def __getattr__(self, key):
        """Delegate attribute lookup to our resultSet or schema"""
        for target in ('schema', ):
            if key != target:
                source = self.__dict__.get(target) or getattr(self.__class__, target, None)
                try:
    ...
    def setValue(self, fieldSchema, value):
        ...
        self.__newValues[fieldSchema.name] = value
        self.__wrappedValues[fieldSchema.name] = value
        return value

    def getValue(self, fieldSchema):
        """Get the field's value for this row"""
        if self.__newValues.has_key(fieldSchema.name):
            value = self.__newValues.get(fieldSchema.name)
            if value is DELETED:
                raise AttributeError("%r instance' %r field has been deleted" % (
                 self.__class__.__name__,
                 fieldSchema.name))
            return value
        elif self.__wrappedValues.has_key(fieldSchema.name):
            return self.__wrappedValues.get(fieldSchema.name)
        else:
            if hasattr(self, '_DBRow__data'):
                for possible in (fieldSchema.name, fieldSchema.name.lower(), fieldSchema.name.upper()):
                    value = self.__data.get(possible)
    ...
    def delValue(self, fieldSchema):
        ...
        if not (fieldSchema.nullOk or hasattr(fieldSchema, 'defaultValue')):
            raise TypeError('%r instance field %s requires a non-null value and has no defaultValue, cannot delete' % (
             self.__class__.__name__,
             fieldSchema.name))
        for source in ('_DBRow__data', '_DBRow__wrappedValues', '_DBRow__newValues'):
        ...
        if not found:
            raise AttributeError('%r instance has no attribute %r to delete' % (
             self.__class__.__name__,
             fieldSchema.name))
        self.__newValues[fieldSchema.name] = DELETED
        return value
    ...
    def dirty(self):
        ...
        return len(self.__newValues)

    def getProperties(cls):
        """Get (dbproperty) properties for this object"""
        base = super(DBRow, cls).getProperties()
        base = [ item for item in base if not item.name.startswith('_DBRow__')
               ]
        map = dict([ (x.name, x) for x in base ])
        result = [ map.pop(schema.name) for schema in cls.schema.fields if map.has_key(schema.name)
                 ]
        result += map.values()
        return result
 ...
 L. 305         0  SETUP_EXCEPT         19  'to 22'

 L. 306         3  LOAD_FAST             0  'self'
                6  LOAD_ATTR             0  '_keyToFieldSchema'
                9  LOAD_FAST             1  'key'
               12  CALL_FUNCTION_1       1  None
               15  STORE_FAST            3  'prop'
               18  POP_BLOCK        
               19  JUMP_FORWARD         19  'to 41'
             22_0  COME_FROM             0  '0'

 L. 307        22  DUP_TOP          
               23  LOAD_GLOBAL           1  'KeyError'
 ...
 L. 308        36  JUMP_FORWARD         55  'to 94'
               39  POP_TOP          
               40  END_FINALLY      
             41_0  COME_FROM            19  '19'

 L. 310        41  SETUP_EXCEPT         19  'to 63'

 L. 311        44  LOAD_FAST             0  'self'
               47  LOAD_ATTR             2  'getValue'
               50  LOAD_FAST             3  'prop'
               53  CALL_FUNCTION_1       1  None
               56  STORE_FAST            4  'value'
               59  POP_BLOCK        
               60  JUMP_FORWARD         31  'to 94'
             63_0  COME_FROM            41  '41'

 L. 312        63  DUP_TOP          
               64  LOAD_GLOBAL           3  'AttributeError'
               67  LOAD_GLOBAL           1  'KeyError'
               70  LOAD_GLOBAL           4  'ValueError'
               73  LOAD_GLOBAL           5  'TypeError'
               76  BUILD_TUPLE_4         4 
               79  COMPARE_OP           10  exception-match
               82  JUMP_IF_FALSE         7  'to 92'
               85  POP_TOP          
               86  POP_TOP          
               87  POP_TOP          
               88  POP_TOP          

 L. 313        89  JUMP_FORWARD          2  'to 94'
               92  POP_TOP          
               93  END_FINALLY      
             94_0  COME_FROM            89  '89'
             94_1  COME_FROM            60  '60'
             94_2  COME_FROM            36  '36'

 L. 314        94  LOAD_GLOBAL           6  'getattr'
               97  LOAD_FAST             0  'self'
              100  LOAD_FAST             1  'key'
              103  LOAD_FAST             2  'default'
              106  CALL_FUNCTION_3       3  None
              109  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `RETURN_VALUE' instruction at offset 109
    ...
    def foreignSimpleRef(cls, key):
        ...
        if foreignRef:
            ...
            if len(foreignFields) == 1:
                foreignTable = cls.schema.lookupName(foreignRef.foreignTable)
    ...
    def fromDict(cls, dictionary):
        ...
        return cls(**dict([ (k, v) for (k, v) in dictionary.iteritems() if k != '__create'
                          ]))
        ...
        if foreignTable:
            ...
            try:
                ...
                if doInsert:
                    ...
                    raise KeyError('Subrecord reference without __create = True for %s (%s) did not find referenced record: %s' % (
    ...
    def referenceSubRecord(cls, key, record, fullPrefix=None):
        ...
        for (subKey, subValue) in properties.items():
            ...
            if isinstance(subValue, (dict, DBRow)):
                log.info(' sub-record')
                subRecordResult = foreignTable.itemClass.referenceSubRecord(subKey, subValue, subKeyFull)
            ...
            keySetFragments.append('%(subKey)s = %(subValueRef)s' % locals())
    ...
    def all(cls):
        ...
        return findTable(cls.schema.name).query('SELECT * FROM %(table)s %(order)s', connection, table=cls.schema.name, order=order)
    ...
    def keySetProperties(self, properties):
        ...
        for k in properties.keys():
            ...
            if properties[k] is None:
                ...
                comparison = ' = '
            keySetFragments.append('%s%s%s' % (k, comparison, asValueReference(k, properties[k])))
...
class RowInsert(RowAction):
    ...
    def __call__(self, cursor, client, doResolveQuery=None, doFullQuery=0, **named):
        ...
        if doResolveQuery or doFullQuery:
            try:
                ...
                for keySet in client.getUniqueKeys():
                    ...
                    for field in keySet:
                        try:
                            set.append(self.getFieldValue(client, field, originalOnly=False))
                        except (KeyError, ValueError, TypeError, AttributeError), err:
                            field = getattr(client.__class__, field)
                            try:
                                newKeyValue = field.nextValue(cursor.connection)
        ...
        if not properties:
            ...
            VALUE_TYPE = 'VALUES'
        properties = self.createSubRecords(client, properties)
        return super(RowInsert, self).__call__(cursor, tableName=client.schema.name, tableColumns=tableColumns, tableColumnSubs=tableColumnSubs, clientObject=client, doResolveQuery=doResolveQuery, doFullQuery=doFullQuery, VALUE_TYPE=VALUE_TYPE, **properties)
        ...
        if doResolveQuery or doFullQuery:
            try:
                ...
                try:
                    ...
                    if doFullQuery:
                        fields = None
                    newCursor = cursor.connection.driver.getInsertedRow(cursor, tableName=clientObject.schema.name, fields=fields)
...
class RowCurrent(RowAction):
    ...
    def __call__(self, cursor, client, **named):
        ...
        keySetFragments = (' AND ').join(keySetFragments)
        return super(RowCurrent, self).__call__(cursor, tableName=tableName, keySetFragments=keySetFragments, clientObject=client, **expanded)
    ...
    def processResults(self, cursor, clientObject, **named):
        ...
        return clientObject.__class__(_DBRow__data=data)
...
class RowUpdate(RowAction):
    ...
    def __call__(self, cursor, client, **named):
        ...
        if not client._DBRow__newValues:
            raise ValueError('Update called on %r which has no new data' % (client,))
            ...
            if value is DELETED:
                ...
                if value is not NULL:
                    ...
                    columnFragments.append('%s=%%(%s)s' % (
                     key,
                     setName))
            elif isinstance(value, (dict, DBRow)):
                if self.CREATE_SUBRECORDS:
                    value = client.createSubRecords(key, value)
                newExpanded = client.referenceSubRecord(key, value)
        ...
        return super(RowUpdate, self).__call__(cursor, tableName=tableName, columnFragments=columnFragments, keySetFragments=keySetFragments, clientObject=client, **expanded)
...
class RowDelete(RowAction):
    ...
    def __call__(self, cursor, client, **named):
        ...
        (tableName, properties) = self.getObjectSpec(client)
        keySetFragments = (' AND ').join([ '%s=%%(%s)s' % (key, key) for key in properties.keys()
                                         ])
        return super(RowDelete, self).__call__(cursor, tableName=tableName, keySetFragments=keySetFragments, clientObject=client, **properties)