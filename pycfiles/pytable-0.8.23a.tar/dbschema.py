...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/dbschema.py
# Compiled at: 2007-10-09 11:43:11
...
class Schema(propertied.Propertied):
    ...
    actions = common.ListProperty('actions', 'Actions defined for use with this schema-item\n\n\t\tSome possible actions:\n\t\t\tselectAll/refreshView\n\t\t\trevert:\n\t\t\t\tall\n\t\t\t\trow(s)\n\t\t\t\trow,field\n\t\t\tsave data:\n\t\t\t\tupdate all rows\n\t\t\t\tupdate row\n\t\t\t\tcommit\n\t\t\t\tappend row\n\t\t\tinsert row\n\t\t\tdelete row\n\t\t', baseType=callable.Callables, defaultFunction=lambda prop, client: [])
    comment = common.StringProperty('comment', 'Optional comment/documentation for this schema item', defaultValue='', setDefaultOnGet=0, dataType='str.long')
    __namespace = weak.WeakProperty('_Schema__namespace', 'Weak ref to namespace in which we are defined')
    __names = common.DictionaryProperty('_Schema__names', 'Mapping from name: object for the schema', defaultFunction=lambda property, client: client.buildNamespace())
    ...
    def lookupName(self, name=None, requiredType=None, alreadySeen=None):
        ...
        if isinstance(name, (str, unicode)):
            ...
            if isinstance(self, requiredType):
                ...
                if not alreadySeen.has_key(id(self.__namespace)):
                    return self.__namespace.lookupName(requiredType=requiredType, alreadySeen=alreadySeen)
        ...
        if hasattr(self, '_Schema__namespace'):
            ...
            alreadySeen[id(self)] = True
            if not alreadySeen.has_key(id(self.__namespace)):
                return self.__namespace.lookupName(name, requiredType=requiredType, alreadySeen=alreadySeen)
    ...
    def findParent(self, requiredType=None):
        ...
        while 1:
            ...
            if current is None:
                break
            elif requiredType is not None and isinstance(current, requiredType):
                lastParent = current
    ...
    def lookupName(self, name=None, requiredType=None, alreadySeen=None):
        ...
        if isinstance(name, (str, unicode)):
            ...
            if isinstance(self, requiredType):
                ...
                if not alreadySeen:
                    alreadySeen = {}
                alreadySeen[id(self)] = True
                return self._Schema__namespace.lookupName(requiredType=requiredType, alreadySeen=alreadySeen)
            else:
                raise NameError("Couldn't find any object of type %r in this namespace" % (requiredType,))
        ...
        if hasattr(self, '_Schema__namespace'):
            ...
            if not alreadySeen.has_key(id(self._Schema__namespace)):
                alreadySeen[id(self)] = True
                return self._Schema__namespace.lookupName(name, requiredType=requiredType, alreadySeen=alreadySeen)
        raise NameError("Couldn't find name %r in the namespace(s) of %r object %r" % (
         name,
         type(self),
         getattr(self, 'name', '')))
...
class ConstraintSchema(Schema):
    ...
    name = common.StringProperty('name', 'Name of the constraint, often not specified', defaultValue='', setDefaultOnGet=0)
    fields = common.StringsProperty('fields', 'The list of fields affected by the constraint')
    deferrable = common.BooleanProperty('deferrable', 'Whether checking can be deferred to the end of the transaction', defaultValue=0)
    deferred = common.BooleanProperty('deferred', 'Whether constraint is currently deferred', defaultValue=0)
...
class CheckConstraint(ConstraintSchema):
    ...
    expression = common.StringProperty('expression', 'The SQL-formatted expression to test on insertions/updates', defaultValue='')


from basictypes import enumeration
actions = enumeration.EnumerationSet()
actions.new(value='NO ACTION', name='noAction', friendlyName='Do Nothing')
actions.new(value='CASCADE', name='cascade', friendlyName='Cascade Deletion')
actions.new(value='SET NULL', name='null', friendlyName='Set to NULL')
actions.new(value='SET DEFAULT', name='default', friendlyName='Set to Default')
...
class ForeignKeyConstraint(ConstraintSchema):
    ...
    foreignTable = common.StringProperty('foreignTable', 'The name of the table which constrains us', defaultValue='')
    foreignFields = common.StringsProperty('foreignFields', 'The fields which constrain us')
    onDelete = ActionProperty('onDelete', 'What to do when the constraint is violated\n\n\tCASCADE causes the local row to be deleted\n\t\n\tdefault is normally "NO ACTION", which raises\n\terrors.\n\t')
    onUpdate = ActionProperty('onUpdate', 'What to do when the referenced table.column changes\n\n\tCASCADE copies the new value\n\t\n\tdefault is normally "NO ACTION", which raises\n\terrors.\n\t\t')
    ...
    def resolve(self):
        ...
        if self.foreignFields:
            ...
            raise RuntimeError('Reference to foreign table %r retrieved a non-table object %r' % (
             self.foreignTable,
             type(table)))
...
def constraint_factories(cls):
    return [
     ForeignKeyConstraint, PrimaryConstraint, UniqueConstraint, CheckConstraint]


ConstraintSchemas = list_types.listof(ConstraintSchema, name='ConstraintSchemas', dataType='list.ConstraintSchemas', factories=classmethod(constraint_factories))

class SequenceSchema(Schema):
    """A sequence object used for implementing e.g. serial fields"""
    dbObjectType = 'SEQUENCE'
    name = common.StringProperty('name', 'Name of the sequence')
    increment = common.IntegerProperty('increment', 'Amount to increment on each "next" call', defaultValue=1, setDefaultOnGet=0)
    ascending = basic.BasicProperty('ascending', 'Whether this is an ascending value', _getValue=lambda property, client: client.increment > 0)
    ...
    minimumValue = common.IntegerProperty('minimumValue', 'Minimum value to which we will traverse', defaultFunction=defaultMinimum, setDefaultOnGet=0)
    maximumValue = common.IntegerProperty('maximumValue', 'Maximum value to which we will traverse', defaultFunction=defaultMaximum, setDefaultOnGet=0)
    start = common.IntegerProperty('start', 'Initial starting value', defaultFunction=defaultStart, setDefaultOnGet=0)
    del defaultMinimum
    del defaultMaximum
    del defaultStart
    cycle = common.BooleanProperty('cycle', 'Whether to loop when reach end or produce errors', defaultValue=0)


SequenceSchemas = list_types.listof(SequenceSchema, name='SequenceSchemas', dataType='list.SequenceSchema')
...
class FieldSchema(ParentSchema):
    """Schema for a particular field of a table
        ...
        """
    dbObjectType = 'COLUMN'
    name = common.StringProperty('name', 'Name of the field (required)', defaultValue='', setDefaultOnGet=0)
    ...
    friendlyName = common.StringProperty('friendlyName', 'Name of the field (required)', defaultFunction=defaultFriendlyName, setDefaultOnGet=0)
    del defaultFriendlyName
    index = common.IntegerProperty('index', 'Index of the field in its table/view', defaultValue=-1, setDefaultOnGet=0)
    displaySize = basic.BasicProperty('displaySize', 'Text-mode display size of the value (can be a tuple)', defaultValue=-1, setDefaultOnGet=0)
    internalSize = basic.BasicProperty('internalSize', 'Binary internal size of the value', defaultValue=1, setDefaultOnGet=0)
    ...
    sequenceName = common.StringProperty('sequenceName', 'Sequence name for serial field data-types', defaultFunction=defaultSequenceName)
    ...
    nullOk = common.BooleanProperty('nullOk', 'Whether value can be left null when inserting/updating\n\n\tThis is actually just mirroring the presence of a\n\tNotNullConstraint in our constraints set.\n\t', defaultFunction=defaultNullOk, setDefaultOnGet=0)
    del defaultNullOk
    defaultValue = basic.BasicProperty('defaultValue', 'A string representation of the default value for the field')
    constraints = common.ListProperty('constraints', 'Constraint objects which constrain this field', baseType=ConstraintSchemas)
    ...
    def defaultDataType(prop, client):
        ...
        try:
            ...
            if databaseSchema and hasattr(databaseSchema, 'driver') and databaseSchema.driver:
    ...
    dataType = common.StringProperty('dataType', 'wxoo string dataType declaration\n\n\tIf not explicitly set, this value can normally be\n\tinferred from the database itself.  Explicitly\n\tsetting the value allows for defining "special"\n\tdataTypes such as for a choice from a referenced\n\ttable.\n\t', defaultFunction=defaultDataType, setDefaultOnGet=1)
    del defaultDataType
    dbDataType = common.StringProperty('dbDataType', "database's SQL dataType declaration", defaultValue='', setDefaultOnGet=0)
    ...
    propertyClass = common.ClassByNameProperty('propertyClass', 'The class used to represent this property object', defaultFunction=defaultPropertyClass, setDefaultOnGet=0)
    del defaultPropertyClass
    baseClass = common.ClassByNameProperty('baseClass', 'The class used to represent items, also used for dataType and the like')
    control = common.ClassByNameProperty('control', 'Explicitly-set control class to use for this field')
    gridViewer = common.ClassByNameProperty('gridViewer', 'Explicitly-set gridViewer class to use for this field')
    gridEditor = common.ClassByNameProperty('gridEditor', 'Explicitly-set gridEditor class to use for this field\n\n\tNote: if this property is not set, but the control property\n\t\tis set, the control will be auto-wrapped as a grid editor.\n\t')

    def buildProperty(self):
        """Build a BasicProperty which uses this schema"""
        return self.propertyClass(name=self.name, schema=self)
...
FieldSchemas = list_types.listof(FieldSchema, name='FieldSchemas', dataType='list.FieldSchemas')

class BaseTableSchema(ParentSchema):
    """Base-class for table-like schemas"""
    name = common.StringProperty('name', 'Name of the table (required)', setDefaultOnGet=0)
    fields = common.ListProperty('fields', 'FieldSchema objects for the table', baseType=FieldSchemas)
    properties = common.DictionaryProperty('properties', "Table/View's property-descriptor objects\n\n\t\tCreated from fields via wrapping\n\t\t", setDefaultOnGet=0, defaultFunction=lambda property, client: client.buildProperties())
    propertiesCalculated = common.BooleanProperty('propertiesCalculated', 'Whether property-set was automatically calculated\n\n\tUsed to determine whether to re-calculate if necessary\n\t', defaultValue=0, setDefaultOnGet=0)
    collectionClass = common.ClassByNameProperty('collectionClass', 'The class used to wrap result-sets as collection objects', defaultValue='pytable.dbresultset.DBResultSet')
    itemClass = common.ClassByNameProperty('itemClass', 'The class used to wrap result-set-row objects', defaultFunction=lambda property, client: client.buildRowClass(), setDefaultOnGet=1)
    baseItemClass = common.ClassByNameProperty('baseItemClass', 'The default base-class for itemclass', defaultValue='pytable.dbrow.DBRow', setDefaultOnGet=0)
...
class TableSchema(BaseTableSchema):
    """High-level representation of a table's structure/schema
        ...
        """
    dbObjectType = 'TABLE'
    indices = common.ListProperty('indices', 'IndexSchema objects for indices on this table')
    constraints = common.ListProperty('constraints', 'Constraint objects which constrain this table', baseType=ConstraintSchemas)
    inherits = common.StringsProperty('inherits', 'List of tables from which this table inherits\n\t\t\n\tInherits is a PostgreSQL extension that creates an\n\tobject-relational system with single and multiple\n\tinheritence types.  See PostgreSQL docs for details.\n\t')
    temporary = common.BooleanProperty('temporary', 'Whether this table should be created in temporary storage\n\n\tTemporary tables do not persist across connections\n\t', defaultValue=0, setDefaultOnGet=0)
    withOIDs = common.BooleanProperty('withOIDs', 'Whether table should explicitly use OIDs (postgres only)', defaultValue=False, setDefaultOnGet=False)
    defaultRecords = common.ListProperty('defaultRecords', 'Set of default records to be included in table on creation')

    def defaultFriendlyNameField(property, client):
        """Try to figure out if there's a good candidate for friendlyName"""
        possibleNames = [
         client.name + '_name', client.name + 'name', 'name']
    ...
    friendlyNameField = common.StringProperty('friendlyNameField', 'Field to be used as friendly name for rows (i.e. used in UIs)', defaultFunction=defaultFriendlyNameField)
    ...
    def fromConnection(cls, connection, tableName, **named):
        ...
        if hasattr(connection.driver, 'tableStructure'):
            new = connection.driver.tableStructure(cursor=connection.cursor(), tableName=tableName, tableClass=cls, descriptorClass=FieldSchema, indexClass=IndexSchema, **named)
    ...
    actions = [
     sqlquery.SQLQuery(name='query', sql='SELECT * FROM %(tableName)s;')]
    ...
    def query(self, query=None, connection=None, **named):
        ...
        if query:
            ...
            return self.collectionClass(schema=self, cursor=cursor)
        else:
...
TableSchemas = list_types.listof(TableSchema, name='TableSchemas', dataType='list.TableSchemas')
...
class IndexSchema(Schema):
    """Schema for defining a particular index (on a particular table)
        ...
        """
    dbObjectType = 'INDEX'
    name = common.StringProperty('name', 'Name of the index', setDefaultOnGet=0, defaultValue='')
    table = common.StringProperty('table', 'The table on which we are going to operate')
    fields = common.StringsProperty('fields', 'Set of field-names to index')
    unique = common.BooleanProperty('unique', 'Whether index values must be unique', defaultValue=1)
    primary = common.BooleanProperty('primary', 'Whether index is a primary-key index', defaultValue=0)
    accessMethod = common.StringProperty('accessMethod', 'The internal access method for the table\n\n\tInternally will likely default to "BTREE", but we\n\tspecify default as "" to use the DB\'s default.\n\n\tThis is not necessarily supported by your db, of course.\n\t')
    functionName = common.StringProperty('functionName', 'SQL function name taking fields and returning indexable value')
    where = common.StringProperty('where', 'SQL-formatted string defining a test expression\n\n\tOnly records matching the where expression will be\n\tincluded in the index.\n\t')
    ...
    def resolve(self):
        ...
        for field in self.fields:
            try:
                ...
                raise NameError("Couldn't get field %r for index %r from table %r: known fields %s" % (
                 field,
                 self,
                 table.name, [ f.name for f in getattr(table, 'fields', ()) ]))


IndexSchemas = list_types.listof(IndexSchema, name='IndexSchemas', dataType='list.IndexSchemas')
...
class NamespaceSchema(ParentSchema):
    """Schema for a database namespace/sub-schema
        ...
        """
    dbObjectType = 'SCHEMA'
    name = common.StringProperty('name', 'Name of the schema/namespace', defaultValue='')
    tables = common.ListProperty('tables', 'Tables in the schema', baseType=TableSchemas)
    sequences = common.ListProperty('sequences', 'Sequence/counter objects in the schema', baseType=SequenceSchemas)
...
NamespaceSchemas = list_types.listof(NamespaceSchema, name='NamespaceSchemas', dataType='list.NamespaceSchemas')

class DatabaseSchema(NamespaceSchema):
    """Schema for an overall database object"""
    dbObjectType = 'DATABASE'
    name = common.StringProperty('name', 'Name of the database', defaultValue='')
    namespaces = common.ListProperty('namespaces', 'Sub-namespaces of the database', baseType=NamespaceSchemas)
    readOnly = common.BooleanProperty('readOnly', 'Disable all writing to the database if true', defaultValue=0)
    driver = basic.BasicProperty('driver', 'Optional binding to the driver with which the schema works')
            ...
            if '.' in name:
                try:
                    if alreadySeen is None:
                        alreadySeen = {}
                    alreadySeen[id(self)] = True
                    namespace = super(DatabaseSchema, self).lookupName(name=name.split('.')[0], alreadySeen=alreadySeen)
                except NameError, err:
                    pass
                else:
                    return namespace.lookupName(name=name, requiredType=requiredType, alreadySeen=alreadySeen)
        return super(DatabaseSchema, self).lookupName(name=name, requiredType=requiredType, alreadySeen=alreadySeen)
