...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/dbproperty.py
# Compiled at: 2007-01-26 04:47:45
...
class DBProperty(basic.BasicProperty):
    """Database-row-object properties
        ...
        """
    schema = basic.BasicProperty('schema', 'The controlling schema for this field', baseType=typeunion.TypeUnion((
     dbschema.FieldSchema,
     viewschema.ViewFieldSchema)))
    name = common.StringProperty('name', 'Name of the field (required)', defaultFunction=lambda prop, client: client.schema.name, setDefaultOnGet=0)
    friendlyName = common.StringProperty('friendlyName', 'Friendly field-name for GUI presentations', defaultFunction=lambda prop, client: client.schema.friendlyName, setDefaultOnGet=0)
    baseType = common.ClassByNameProperty('baseType', 'The base-type of the field, often merely None', defaultFunction=_defaultBaseType, setDefaultOnGet=1)
    dataType = common.StringProperty('dataType', "String data-type for the field's values", defaultFunction=_defaultDataType, setDefaultOnGet=1)
    documention = common.StringProperty('documention', 'The documentation for the field', defaultFunction=lambda prop, client: client.schema.comment, setDefaultOnGet=0)
    setDefaultOnGet = 0
    ...
    def nextValue(self, connection):
        ...
        if getattr(self.schema, 'defaultValue', None):
            for row in sqlquery.SQLQuery('SELECT %(value)s')(connection, value=self.schema.defaultValue):
                return row[0]

        if getattr(self.schema, 'sequenceName', None):
            for row in sqlquery.SQLQuery('SELECT nextval(%%(sequenceName)s)')(connection, sequenceName=self.schema.sequenceName):
                return row[0]

        raise AttributeError("Sorry, don't seem to have a default value or sequence name for %s" % (
         self.schema.name,))
        return
...
class ReferenceProperty(DBProperty):
    ...
    def commonValues(self, connection):
        ...
        fields = (',').join(fields)
        records = sqlquery.SQLQuery(sql=sql)(connection, fields=fields, foreignTable=constraint.foreignTable, friendlyNameField=friendlyNameField).fetchall()
        return records
