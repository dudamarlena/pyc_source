...
# [GCC 8.4.0]
# Embedded file name: /oldroot/var/pylive/directleap/conf-agi/pytable/viewschema.py
...
class ViewFieldSchema(dbschema.FieldSchema):
    """A view-field description, adds source table and field props
        ...
        actions.
        """
    sourceTable = basic.BasicProperty('sourceTable', 'The source-table schema for this field (may be null)', baseType=dbschema.TableSchema)
    sourceField = basic.BasicProperty('sourceField', 'The source-field schema for this field (may be null)', baseType=dbschema.FieldSchema)


ViewFieldSchemas = list_types.listof(ViewFieldSchema, name='ViewFieldSchemas', dataType='list.ViewFieldSchemas')

class JoinTable(dbschema.Schema):
    """A Join of a table, basically just a name:table item"""
    name = common.StringProperty('name', 'Name of the table (required)', defaultFunction=lambda prop, client: client.table.name)
    table = basic.BasicProperty('table', 'Table object which participates in the schema', baseType=dbschema.TableSchema)
...
JoinTables = list_types.listof(JoinTable, name='JoinTables', dataType='list.JoinTables')
...
class ViewSchema(dbschema.BaseTableSchema):
    """A query-view table-structure/schema
        ...
        """
    tables = common.ListProperty('tables', 'List of JoinTable objects which participate in the schema', baseType=JoinTables)
    fields = common.ListProperty('fields', 'FieldSchema objects for the table', baseType=ViewFieldSchemas)


ViewSchemas = list_types.listof(ViewSchema, name='ViewSchemas', dataType='list.ViewSchemas')