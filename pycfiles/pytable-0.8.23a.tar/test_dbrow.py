...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/tests/test_dbrow.py
...
cats = table('cats', (
 field('cat_id', 'serial', 0, 'Unique cat identifier', constraints=(notNull(), primary())), field('cat_name', 'varchar', 255, "The cat's name", constraints=(notNull(),)), field('declawed', 'bool', 0, "Poor lil' kitty")), 'Primary table for storing critical info about cats', defaultRecords=[{'cat_name': 'felis', 'declawed': 1}, {'cat_name': 'boris', 'declawed': 1}])
...
class DBRowTests(unittest.TestCase):
    ...
    if driver.capabilities.serial:
        ...
        def testActionLookup(self):
            cat = cats.itemClass(schema=cats, connection=self.connection)
            cat.cat_name = 'test'
