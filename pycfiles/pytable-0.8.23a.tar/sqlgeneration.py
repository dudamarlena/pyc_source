...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/sqlgeneration.py
# Compiled at: 2007-10-10 13:37:34
...
class SQLStatements(propertied.Propertied):
    ...
    def __init__(self, driver=None, **named):
        """Initialise the statement-generator
                ...
                """
        super(SQLStatements, self).__init__(driver=driver, **named)
...
class SQLCreateStatements(SQLStatements):
    ...
    def database(self, schema, *arguments, **named):
        ...
        fragments.extend([ self.dispatch(s, *arguments, **named) for s in schema.sequences
                         ])
        for s in schema.tables:
            fragments.extend(self.dispatch(s, *arguments, **named))

        fragments.extend([ self.dispatch(s, *arguments, **named) for s in getattr(schema, 'namespaces', ())
                         ])
    ...
    def namespace(self, schema, **named):
        ...
        return [
         'CREATE SCHEMA %s;' % (schema.name.lower(),)] + self.database(schema)
        ...
        if isinstance(value, dict):
            ...
            for (key, value) in [ item for item in value.items() if item[0] != '__create'
                ...
                if isinstance(value, dict):
                    ...
                    valueSQL = self.recordReference(remoteTable, remoteTable.lookupName(remoteField), value)
                else:
                    valueSQL = sqlutils.sqlEscape(value, dbDataType=table.lookupName(key).dbDataType)
                wheres.append('%s = %s' % (
                 key,
                 valueSQL))
    ...
    def createSubRecords(self, table, field, record):
        ...
        if record.get('__create', True):
            items = [ (k, v) for (k, v) in record.items() if k != '__create' ]
            for (key, value) in items:
                if isinstance(value, dict):
                    ...
                    remoteTableName = remoteReference.foreignTable
                    remoteTable = table.lookupName(remoteTableName)
                    result.extend(self.createSubRecords(remoteTable, remoteTable.lookupName(remoteField), value))
            ...
            for (key, value) in items:
                try:
                    ...
                    raise NameError("Couldn't find field %r for table %s defined fields: %s" % (
                ...
                if isinstance(value, dict):
                    ...
                    remoteTableName = remoteReference.foreignTable
                    remoteTable = table.lookupName(remoteTableName)
                    sqlValue = self.recordReference(remoteTable, remoteTable.lookupName(remoteField), value)
                else:
            ...
            result.append('INSERT INTO %(tableName)s(%(fields)s) VALUES (%(values)s);' % locals())
        return result
    ...
    def records(self, table, dictionaries):
        ...
        for dictionary in dictionaries:
            fragments.extend(self.createSubRecords(table, None, dictionary))
    ...
    def tableField(self, schema, **named):
        ...
        if hasattr(schema, 'defaultValue'):
            if not (self.driver and self.driver.capabilities.serial and getattr(schema, 'dbDataType', None) in ('serial',
                                                                                                                'bigserial')):
    ...
    def fieldConstraint(self, constraint, target):
        ...
        if constraint.dbConstraintType in ('UNIQUE', 'NULL', 'PRIMARY KEY'):
    ...
    def tableConstraint(self, constraint, target):
        ...
        if constraint.name:
            fragments.append('CONSTRAINT %s' % constraint.name.lower())
        if constraint.dbConstraintType in ('UNIQUE', 'PRIMARY KEY', 'FOREIGN KEY'):
            fragments.append(constraint.dbConstraintType)
        ...
        if isinstance(target, dbschema.FieldSchema):
            if len(constraint.fields) > 1:
                ...
                if field.lower() != target.name.lower():
                    raise ValueError('Constraint %r on field %r specifies different field name, specifies %s, should be %s' % (
                     constraint,
                     target,
                     constraint.fields,
                     [
    ...
    def foreignKey(self, constraint, target):
        ...
        fragments = [
         'REFERENCES', constraint.foreignTable]
    ...
    def tableIndex(self, schema, target=None):
        ...
        if not schema.name:
            schema.name = (('_').join([ n.lower() for n in [
    ...
    dispatchMapping = {dbschema.FieldSchema: tableField, 
...
class SQLDropStatements(SQLStatements):
    ...
    dispatchMapping = {dbschema.TableSchema: standAlone, 
...
class SQLGrantStatements(SQLStatements):
    ...
    dispatchMapping = {dbschema.TableSchema: general, 
