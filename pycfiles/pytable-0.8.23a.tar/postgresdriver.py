...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/_postgresql/postgresdriver.py
# Compiled at: 2007-06-20 17:00:43
...
class PostgresDriver(dbdriver.DBDriver):
    """Base PostgreSQL database driver
        """
    baseModule = None
    capabilities = dbdriver.DriverCapabilities(serial=1, inherits=1, queryUnicode=1, schemaSupport=1, oids=1)
    systemDBName = common.StringProperty('systemDBName', "Default name for driver's system database (for listDatabases connections)", defaultValue='template1')
    ...
    def getInsertedRow(self, cursor, tableName, fields=None):
        ...
        if not isinstance(oidValue, (int, long)):
            raise ValueError('oidValue for getInsertedRow is not an integer, did the insert work? %r' % (
             oidValue,))
        newCursor = sqlquery.SQLQuery(sql='SELECT %(fieldNames)s FROM %(tableName)s\n\t\t\tWHERE oid = %(oidValue)s;')(cursor.connection, tableName=tableName, fieldNames=fieldNames, oidValue=oidValue)
        return newCursor
    ...
    def getLastOID(cls, cursor):
        ...
        try:
            ...
            oidValue = cursor.lastrowid

        return oidValue

    getLastOID = classmethod(getLastOID)
    queries = {'listDatabases': tableactions.ListDatabases(), 
    ...
    dataTypeRegistry = dbdriver.DBDriver.dataTypeRegistry + [
     ...
     ('timestampz', 'datetime', 'mx.DateTime.DateTimeType'),
