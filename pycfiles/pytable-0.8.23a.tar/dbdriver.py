...
# [GCC 8.4.0]
# Embedded file name: /home/mcfletch/pylive/table/pytable/dbdriver.py
# Compiled at: 2007-06-27 13:21:50
...
class DriverCapabilities(propertied.Propertied):
    """Object storing set of driver capability declarations
        ...
        databases.
        """
    serial = common.BooleanProperty('serial', "Whether we have native support for serial datatype\n\n\tIf the serial datatype is not supported, we'll have to\n\tgenerate code to create a sequence, track the name of\n\tthat sequence and potentially rewrite the defaultValue\n\tfield of the schema.\n\t", defaultValue=0, setDefaultOnGet=0)
    inherits = common.BooleanProperty('inherits', "Whether we support PostgreSQL-style inheritance on tables\n\n\tInherits is an object-relational database feature that\n\tprovides (multiple) inheritance for database tables. If\n\tit's available, we can model object hierarchies more\n\tnaturally.  If not, we need to do (complex, slow) joins\n\tacross multiple tables to load base-class and sub-class\n\tdata.\n\t", defaultValue=0, setDefaultOnGet=0)
    queryUnicode = common.BooleanProperty('queryUnicode', 'Whether can accept Unicode for query strings\n\n\tIf this is false, then sqlquery needs to translate queries into\n\tstrings, which it will do using utf-8 encodings.\n\t', defaultValue=1, setDefaultOnGet=0)
    queryPyformat = common.BooleanProperty('queryPyformat', 'Whether can handle pyformat paramstyle, if not will use pyformatsupport', defaultValue=1)
    schemaSupport = common.BooleanProperty('schemaSupport', 'Whether the database supports sub-namespace/sub-schemas\n\n\t\tAFAIK only PostgreSQL actually supports the feature\n\t', defaultValue=0, setDefaultOnGet=0)
    lastRowID = common.BooleanProperty('lastRowID', 'Whether the database supports last-row-id reporting', defaultValue=True, setDefaultOnGet=0)
    oids = common.BooleanProperty('oids', 'PostgreSQL concept of OID columns on tables', defaultValue=False, setDefaultOnGet=0)
...
class DBDriver(propertied.Propertied):
    ...
    name = basic.BasicProperty('name', 'DriverName enumeration identifying the driver in DBSpecifiers', baseType=DriverName)
    connections = common.WeakValueDictionaryProperty('connections', 'Mapping of specifier objects to live connections\n\n\tThe DBDriver retains weak references to all active\n\tconnections, to allow for connection sharing and/or\n\tproperly shutting down all connections on exit.\n\t', setDefaultOnGet=1)
    defaultConnectionClass = common.ClassByNameProperty('defaultConnectionClass', 'Default DBConnection sub-class to be used for this driver', defaultValue='pytable.dbconnection.DBConnection')
    friendlyName = common.StringProperty('friendlyName', 'Friendly name of the DBDriver type', defaultValue='')
    paramstyle = common.StringProperty('paramstyle', 'DBAPI 2.0 parameter-style value', defaultValue='pyformat')
    threadsafety = common.IntegerProperty('paramstyle', 'DBAPI 2.0 threadsafety value XXX should be an enumeration!', defaultValue=0)
    apilevel = common.StringProperty('apilevel', 'DBAPI 2.0 apilevel value', defaultValue='2.0')
    systemDBName = common.StringProperty('systemDBName', "Default name for driver's system database (for listDatabases connections)", defaultValue='')
    queries = {}
    userDescription = ''
    ...
    def copyErrorsFromModule(cls, source):
        ...
        for name in ('Date', 'Time', 'Timestamp', 'DateFromTicks', 'TimeFromTicks',
                     'TimestampFromTicks', 'Binary', 'STRING', 'BINARY', 'NUMBER',
                     'DATETIME', 'ROWID', 'BOOLEAN'):
    ...
    def wrapConnection(self, connection):
        ...
        return self.defaultConnectionClass(connection=connection, driver=self)
    ...
    dataTypeRegistry = [
     ...
     ('timestamptz', 'datetime', 'mx.DateTime.DateTimeType'),
     (
      'cash', 'decimal', int),
     (
      'money', 'decimal', int),
