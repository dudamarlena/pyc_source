...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='STAR', anchor='star', href='https://github.com/alexdobin/STAR', info='is an ultrafast universal RNA-seq aligner.')
        self.star_data = dict()
        ...
        if len(self.star_genecounts_unstranded) > 0:
            self.add_section(name='Gene Counts', anchor='star_geneCounts', description='Statistics from results generated using <code>--quantMode GeneCounts</code>. ' + 'The three tabs show counts for unstranded RNA-seq, counts for the 1st read strand ' + 'aligned with RNA and counts for the 2nd read strand aligned with RNA.', plot=self.star_genecount_chart())
        return
