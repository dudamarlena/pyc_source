...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Stacks', anchor='stacks', href='http://catchenlab.life.illinois.edu/stacks/',
        ...
        if len(self.cov_data) == 0:
            if len(self.sumstats_data) == 0:
                if len(self.distribs_loci) == 0:
                    raise UserWarning
        ...
        config_table = {'id':'gstacks_table', 
         'namespace':'stacks'}
        self.add_section(name='Sample statistics',
        ...
        config_table = {'id':'sumstats_table', 
         'namespace':'stacks'}
        self.add_section(name='Population summary statistics',
        ...
        config_distribs = {'id':'distribs_plot', 
         ...
         'data_labels':[
          {'name':'Samples per loci', 
           'ylab':'# loci',  'xlab':'# samples'},
          {'name':'SNPs per loci', 
           'ylab':'# loci',  'xlab':'# SNPs'}]}
        self.add_section(name='Population plots',
    ...
    def parse_gstacks(self, file_contents, s_name):
        ...
        for l in file_contents.splitlines():
            if l.startswith('sample\tn_loci\tn_used_fw_reads'):
                ...
                if l.startswith('END effective_coverages_per_sample'):
                    break
                else:
    ...
    def parse_populations(self, file_contents, s_name):
        ...
        for match in pat.finditer(file_contents):
            ...
            if title == 'missing_samples_per_loc_postfilters':
                ...
                if title == 'snps_per_loc_postfilters':
                    if s_name is not None:
                        title = s_name
