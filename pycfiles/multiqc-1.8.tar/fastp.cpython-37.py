...
# Compiled at: 2019-03-30 18:41:17
# Size of source mod 2**32: 16934 bytes
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='fastp', anchor='fastp', href='https://github.com/OpenGene/fastp',
        ...
        for k in ('read1_before_filtering', 'read2_before_filtering', 'read1_after_filtering',
        ...
        self.add_section(name='Filtered Reads',
        ...
        if len(self.fastp_duplication_plotdata) > 0:
            self.add_section(name='Duplication Rates',
              anchor='fastp-duprates',
              description='Duplication rates of sampled reads.',
              plot=(linegraph.plot(self.fastp_duplication_plotdata, {'id':'fastp-duprates-plot', 
        ...
        if len(self.fastp_insert_size_data) > 0:
            self.add_section(name='Insert Sizes',
              anchor='fastp-insert-size',
              description='Insert size estimation of sampled reads.',
              plot=(linegraph.plot(self.fastp_insert_size_data, {'id':'fastp-insert-size-plot', 
    ...
    def filter_pconfig_pdata_subplots(self, data, label):
        data_labels = []
        pdata = []
        config = {'read1_before_filtering':{'name':'Read 1: Before filtering', 
          'ylab':'R1 Before filtering: {}'.format(label)}, 
         'read1_after_filtering':{'name':'Read 1: After filtering', 
          'ylab':'R1 After filtering: {}'.format(label)}, 
         'read2_before_filtering':{'name':'Read 2: Before filtering', 
          'ylab':'R2 Before filtering: {}'.format(label)}, 
         'read2_after_filtering':{'name':'Read 2: After filtering', 
