...
# Size of source mod 2**32: 23510 bytes
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='bcl2fastq',
          anchor='bcl2fastq',
        ...
        if len(self.bcl2fastq_bylane) == 0:
            if len(self.bcl2fastq_bysample) == 0:
                raise UserWarning
        for s in self.source_files.keys():
            self.add_data_source(s_name=s,
        ...
        self.add_section(name='Lane Statistics',
        ...
        self.add_section(name='Clusters by lane',
          ...
          plot=(bargraph.plot(self.get_bar_data_from_counts(self.bcl2fastq_bylane), cats, {'id':'bcl2fastq_lane_counts', 
        ...
        self.add_section(name='Clusters by sample',
        ...
        self.add_section(name='Undetermined barcodes by lane',
          anchor='undetermine_by_lane',
          description='Count of the top twenty most abundant undetermined barcodes by lanes',
          plot=(bargraph.plot(self.get_bar_data_from_undetermined(self.bcl2fastq_bylane), None, {'id':'bcl2fastq_undetermined', 
        ...
        try:
            ...
            for conversionResult in content.get('ConversionResults', []):
                ...
                for demuxResult in conversionResult.get('DemuxResults', []):
                    ...
                    for r in range(1, 5):
                        if not lsample['R{}_yield'.format(r)]:
                            lsample['R{}_Q30'.format(r)] or lsample['R{}_trimmed_bases'.format(r)] or lsample.pop('R{}_yield'.format(r))
        ...
        for run_id, r in self.bcl2fastq_data.items():
            for lane_id, lane in r.items():
                ...
                for sample_id, sample in lane['samples'].items():
                    for r in range(1, 5):
                        try:
                            if not self.bcl2fastq_bysample[sample_id]['R{}_yield'.format(r)]:
                                if not self.bcl2fastq_bysample[sample_id]['R{}_Q30'.format(r)]:
                                    if not self.bcl2fastq_bysample[sample_id]['R{}_trimmed_bases'.format(r)]:
    ...
    def get_unknown_barcodes(self, lane_unknown_barcode):
        ...
        """
        try:
            sorted_barcodes = OrderedDict(sorted((lane_unknown_barcode.items()),
    ...
    def get_bar_data_from_undetermined(self, flowcells):
        ...
        bar_data = OrderedDict(sorted((bar_data.items()),
