...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Peddy', anchor='peddy', href='https://github.com/brentp/peddy',
    ...
    def peddy_relatedness_plot(self):
        data = dict()
        for s_name, d in self.peddy_data.items():
            if 'ibs0_ped_check' in d:
                if 'ibs2_ped_check' in d:
                    data[s_name] = {'x':d['ibs0_ped_check'], 
                     'y':d['ibs2_ped_check']}
                if 'rel_ped_check' in d:
                    if d['rel_ped_check'] < 0.25:
                        data[s_name]['color'] = 'rgba(109, 164, 202, 0.9)'
                    else:
                        if d['rel_ped_check'] < 0.5:
    ...
    def peddy_het_check_plot(self):
        ...
        if len(data) > 0:
            self.add_section(name='Het Check',
    ...
    def peddy_sex_check_plot(self):
        ...
        pconfig = {'id':'peddy_sex_check_plot', 
         ...
         'categories':[
          'Female', 'Male', 'Unknown']}
        if len(data) > 0:
            self.add_section(name='Sex Check',
