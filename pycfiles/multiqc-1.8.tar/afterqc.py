...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/afterqc/afterqc.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='AfterQC', anchor='afterqc', href='https://github.com/OpenGene/AfterQC', info='Automatic Filtering, Trimming, Error Removing and Quality Control for fastq data.')
        ...
        self.add_section(name='Bad Reads', anchor='after_qc', description='Filtering statistics of sampled reads.', plot=self.after_qc_bad_reads_chart())
    ...
    def parse_afterqc_log(self, f):
        ...
        if 'summary' in parsed_json:
            ...
            if 'afterqc_main_summary' in parsed_json:
    ...
    def afterqc_general_stats_table(self):
        ...
        headers = OrderedDict()
        headers['pct_good_bases'] = {'title': '% Good Bases', 
           'description': 'Percent Good Bases', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'BuGn'}
        headers['good_reads'] = {'title': ('{} Good Reads').format(config.read_count_prefix), 
           'description': ('Good Reads ({})').format(config.read_count_desc), 
           'min': 0, 
           'modify': lambda x: x * config.read_count_multiplier, 
           'scale': 'GnBu', 
           'shared_key': 'read_count'}
        headers['total_reads'] = {'title': ('{} Total Reads').format(config.read_count_prefix), 
           'description': ('Total Reads ({})').format(config.read_count_desc), 
           'min': 0, 
           'modify': lambda x: x * config.read_count_multiplier, 
           'scale': 'Blues', 
           'shared_key': 'read_count'}
        headers['readlen'] = {'title': 'Read Length', 
    ...
    def after_qc_bad_reads_chart(self):
        ...
        pconfig = {'id': 'afterqc_bad_reads_plot', 
