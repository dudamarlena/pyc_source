...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='SexDetErrmine', anchor='sexdeterrmine', href='https://github.com/TCLamnidis/Sex.DetERRmine', info='A python script to calculate the relative coverage of X and Y chromosomes,\n            and their associated error bars, from the depth of coverage at specified SNPs. ')
    ...
    def read_count_barplot(self):
        ...
        config = {'id': 'sexdeterrmine-readcounts-plot', 
           ...
           'ylab': '# Reads'}
        self.add_section(name='Read Counts', anchor='sexdeterrmine-readcounts', description='The number of reads covering positions on the autosomes, X and Y chromosomes.', plot=bargraph.plot(self.sexdet_data, cats, config))
    ...
    def snp_count_barplot(self):
        ...
        config = {'id': 'sexdeterrmine-snps-plot', 
           ...
           'ylab': '# Reads'}
        self.add_section(name='SNP Counts', anchor='sexdeterrmine-snps', description='Total number of SNP positions. When supplied with a BED file, this includes only positions specified there.', plot=bargraph.plot(self.sexdet_data, cats, config))