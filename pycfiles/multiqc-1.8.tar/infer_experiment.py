...
def parse_reports(self):
    ...
    if len(self.infer_exp) > 0:
        ...
        pconfig = {'id': 'rseqc_infer_experiment_plot', 
           ...
           'ylab_format': '{value}%', 
           'cpswitch': False}
        self.add_section(name='Infer experiment', anchor='rseqc-infer_experiment', description='<a href="http://rseqc.sourceforge.net/#infer-experiment-py" target="_blank">Infer experiment</a> counts the percentage of reads and read pairs that match the strandedness of overlapping transcripts. It can be used to infer whether RNA-seq library preps are stranded (sense or antisense).', plot=bargraph.plot(pdata, keys, pconfig))
