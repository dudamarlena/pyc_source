...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/hicexplorer/hicexplorer.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='HiCExplorer', anchor='hicexplorer', href='https://hicexplorer.readthedocs.io', info=' addresses the common tasks of Hi-C analysis from processing to visualization.')
        self.mod_data = dict()
        ...
        for data_ in self.mod_data:
            if 'Pairs mappable, unique and high quality' not in self.mod_data[data_]:
        ...
        keys_categorization_of_reads_considered = [
         'Pairs mappable, unique and high quality', 'One mate unmapped',
        ...
        self.add_section(name='Mapping statistics', anchor='hicexplorer_categorization_of_considered_reads', plot=self.hicexplorer_create_plot(keys_categorization_of_reads_considered, 'HiCExplorer: Categorization of considered reads', 'categorization'), description='This shows how the sequenced read pairs were mapped and those filtered due to mapping problems.', helptext='\n                * **Pairs mappable, unique and high quality**\n                    * The count of reads that were considered as valid reads and were not one of the following:\n                * **One mate unmapped**\n                    * Filtered out read because one mate was not mapped.\n                * **One mate not unique**\n                    * Filtered out read because one mate was not unique.\n                * **One mate low quality**\n                    * Filtered out because one mate was having a low quality.\n            ')
        self.add_section(name='Read filtering', anchor='hicexplorer_pairs_categorized', plot=self.hicexplorer_create_plot(keys_mappable_unique_and_high_quality, 'HiCExplorer: Categorization of reads - Pairs mappable, unique and high quality', 'mapping'), description='This figure contains the number of reads that were finally used to build the Hi-C matrix along with the reads that where filtered out.', helptext="\n            * **Dangling ends**\n                * These are reads that start with the restriction site and constitute reads that were digested but no ligated.\n            * **Same fragment**\n                * These are read mates, facing inward, separated by up to 800 bp that do not have a restriction enzyme in between.\n                * These read pairs are not valid Hi-C pairs.\n            * **Self circle**\n                * Self circles are defined as pairs within 25kb with 'outward' read orientation\n            * **Self ligation**\n                * These are read pairs with a restriction site in between that are within 800 bp.")
        self.add_section(name='Contact distance', anchor='hicexplorer_contact_distance', plot=self.hicexplorer_create_plot(keys_list_contact_distance, 'HiCExplorer: Contact distance', 'contact_distance'), description='This figure contains information about the distance and location of the valid pairs used.', helptext='\n            * **Long range**\n                * Pairs with a distance greater than 20 kilobases\n            * **Short range**\n                * Pairs with a distance less than 20 kilobases\n            * **Inter chromosomal**\n                * Interchromosomal pairs.\n            ')
        self.add_section(name='Read orientation', anchor='hicexplorer_read_orientation', plot=self.hicexplorer_create_plot(keys_list_read_orientation, 'HiCExplorer: Read orientation', 'orientation'), description='This figure contains information about the orientation of the read pairs.', helptext='\n                * **Inward pairs**\n                    * First mate is a forward read, second is reverse.\n                    * `--------------->              <----------------`\n                * **Outward pairs**\n                    * First mate is a reverse read, second is forward.\n                    * `--------------->              <----------------`\n                * **Left pairs**\n                    * Both are reverse reads.\n                    * `<---------------              <----------------`\n                * **Right pairs**\n                    * Both are forward reads.\n                    * `--------------->              ---------------->`\n            ')
    ...
    def hicexplorer_basic_statistics(self):
        ...
        for file in self.mod_data:
            ...
            data_ = {'Pairs considered': self.mod_data[file]['Pairs considered'][0], 'Pairs used': self.mod_data[file]['Pairs used'][0] / total_pairs, 
        ...
        headers = OrderedDict()
        headers['Pairs considered'] = {'title': ('{} Pairs').format(config.read_count_prefix), 
           'description': ('Total number of read pairs ({})').format(config.read_count_desc), 
           'shared_key': 'read_count'}
        headers['Pairs used'] = {'title': '% Used pairs', 
           'max': 100, 
           'min': 0, 
           'modify': lambda x: x * 100, 
           'suffix': '%'}
        headers['Mapped'] = {'title': '% Mapped', 
           'max': 100, 
           'min': 0, 
           'modify': lambda x: (1 - x) * 100, 
           'scale': 'RdYlGn', 
           'suffix': '%'}
        headers['Min rest. site distance'] = {'title': 'Min RE dist', 
           'description': 'Minimum restriction site distance (bp)', 
           'format': '{:.0f}', 
           'suffix': ' bp'}
        headers[max_distance_key] = {'title': 'Max RE dist', 
    ...
    def hicexplorer_create_plot(self, pKeyList, pTitle, pId):
        ...
        config = {'id': 'hicexplorer_' + pId + '_plot', 'title': pTitle, 
