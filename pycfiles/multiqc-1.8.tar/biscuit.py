...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/biscuit/biscuit.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='BISCUIT', anchor='biscuit', href='https://github.com/zwdzwd/biscuit', info='is a tool to map bisulfite converted sequence reads and determine cytosine methylation states.')
        self.mdata = {'align_mapq': {}, 'align_strand': {}, 'align_isize': {}, 'covdist': {}, 'covdist_q40': {}, 'covdist_q40_botgc': {}, 'covdist_q40_topgc': {}, 'covdist_cpg': {}, 'covdist_cpg_q40': {}, 'covdist_cpg_q40_botgc': {}, 'covdist_cpg_q40_topgc': {}, 'qc_cv': {}, 'qc_cpg_cv': {}, 'qc_cpg_dist': {}, 'dup_report': {}, 'markdup_report': {}, 'retention_dist': {}, 'retention_dist_byread': {}, 'retention_cpg_readpos': {}, 'retention_cph_readpos': {}, 'retention_rate_byread': {}, 'retention_rate_bybase': {}}
        file_suffixes = [
         '_mapq_table',
         '_strand_table',
         '_markdup_report',
    ...
    def chart_align_mapq(self):
        ...
        self.add_section(name='Mapping Summary', anchor='biscuit-mapping', description='This shows the fraction of optimally aligned reads, which is defined by mapQ >= 40.', helptext='A good library should have high fraction of reads optimally aligned. Suboptimally aligned reads include both nonunique alignments and imperfect alignments.', plot=bargraph.plot(pd, OrderedDict([
         (
          'OAligned', {'name': 'Optimally Aligned Reads'}),
         (
          'SAligned', {'name': 'Suboptimally Aligned Reads'}),
         (
          'UAligned', {'name': 'Unaligned Reads'})]), {'id': 'biscuit_mapping_summary', 'title': 'BISCUIT: Mapping Summary', 
        ...
        self.add_section(name='Mapping Quality Distribution', anchor='biscuit-mapq', description='This plot shows the distribution of primary mapping quality.', plot=linegraph.plot(pd_mapping, {'id': 'biscuit_mapping', 'title': 'BISCUIT: Mapping Information', 
           'ymin': 0, 
           'yLabelFormat': '{value}%', 'tt_label': '<strong>Q{point.x}:</strong> {point.y:.2f}% of reads', 
           'name': 'Mapping Quality', 
           'ylab': '% Primary Mapped Reads', 'xlab': 'Mapping Quality'}))

    def parse_logs_align_strand(self, f, fn):
        data = {'read1': {}, 'read2': {}}
        for l in f.splitlines()[1:]:
            m = re.search('strand\\t([12])([+-]*)\\t(\\d+)', l)
            if m is not None:
                if m.group(1) == '1':
        ...
        self.add_section(name='Mapping Strand Distribution', anchor='biscuit-strands', description='This plot shows the distribution of strand of mapping and strand of bisulfite conversion.', helptext="Most bisulfite libraries has read 1 goes to parent `++` or `--` and read 2 goes to daughter/synthesized `+-` or `-+`. PBAT or most single-cell/low input libraries typically don't observe this rule.", plot=bargraph.plot([pd1, pd2], [
         ...
         OrderedDict([
          ...
          (
           '-+', {'name': '-+: Crick-Aligned, Waston-Bisulfite Conversion', 'color': '#46B29D'}),
          (
           '--', {'name': '--: Crick-Aligned, Crick-Bisulfite Conversion', 'color': '#324D5C'})])], {'id': 'biscuit_strands', 'title': 'BISCUIT: Mapping Strand Distribution', 
           'ylab': 'Number of Reads', 
    ...
    def chart_align_isize(self):
        ...
        self.add_section(name='Insert Size Distribution', anchor='biscuit-isize', description='This plot shows the distribution of insert size.', plot=linegraph.plot(pd_isize, {'id': 'biscuit_isize', 'title': 'BISCUIT: Insert Size Distribution', 'ymin': 0, 
           'yLabelFormat': '{value}%', 'smooth_points': 500, 
    ...
    def chart_covdist(self):
        ...
        self.add_section(name='Cumulative Base Coverage', anchor='biscuit-coverage-base', description='This plot shows the cummulative base coverage. High and low GC content region are the top and bottom 10% 100bp window in GC content.', helptext='Q40 means only reads mapped with mapping quality (Q) greater than or equal to 40 are considered.', plot=linegraph.plot(mdata, {'id': 'biscuit_coverage_base', 'title': 'BISCUIT: Cumulative Base Coverage', 
           'xLabelFormat': '{value}X', 
           'xlab': 'Sequencing Depth', 
           'data_labels': [{'name': 'All', 'ylab': 'Million Bases'}, {'name': 'Q40', 'ylab': 'Million Bases (Q40)'}, {'name': 'Q40 low GC', 'ylab': 'Million Low-GC Bases (Q40)'}, {'name': 'Q40 high GC', 'ylab': 'Million High-GC Bases (Q40)'}, {'name': 'CpG (all)', 'ylab': 'Million CpGs'}, {'name': 'CpG Q40', 'ylab': 'Million CpGs (Q40)'}, {'name': 'CpG Q40 low GC', 'ylab': 'Million Low-GC CpGs (Q40)'}, {'name': 'CpG Q40 high GC', 'ylab': 'Million High-GC CpGs (Q40)'}]}))
        ...
        if len(basecov) == 0:
            return
        self.add_section(name='Coverage by At Least One Read', anchor='biscuit-coverage-base-table', description='The fraction of genome/genomic CpGs covered by at least one read.', plot=table.plot(basecov, {'all': {'title': 'Genome (All)', 'max': 100, 'min': 0, 'suffix': '%'}, 'uniq': {'title': 'Genome (Unique)', 'max': 100, 'min': 0, 'suffix': '%'}, 'cpg_all': {'title': 'CpG (All)', 'max': 100, 'min': 0, 'suffix': '%'}, 'cpg_uniq': {'title': 'CpG (Unique)', 'max': 100, 'min': 0, 'suffix': '%'}}))
    ...
    def chart_qc_cv(self):
        ...
        self.add_section(name='Sequencing Depth', anchor='biscuit-seq-depth', description='This plot shows sequence depth mean and uniformity measured in Coefficient of Variation (mu/sigma), mapQ>40 only. CG.* shows the depth on CpG only. GC contents were measured on 100bp non-overlapping windows.', plot=table.plot(pd, pheader))

    def parse_logs_qc_cpg_cv(self, f, fn):
        data = {}
        targets = [
         'cpg', 'cpg_topgc', 'cpg_botgc']
        for k in targets:
            m = re.search(('_{}\t([\\d\\.]+)\t([\\d\\.]+)\t([\\d\\.]+)').format(k), f, re.MULTILINE)
            if m is None:
                data[k] = {'mu': -1, 'sigma': -1, 'cv': -1}
    ...
    def parse_logs_qc_cpg_dist(self, f, fn):
        ...
        m = re.search('#CpG Islands\\t(\\d+)', f, re.MULTILINE)
    ...
    def chart_qc_cpg_dist(self):
        ...
        self.add_section(name='CpG Coverage vs Genomic Features', anchor='biscuit-coverage-cpg-dist', description='The top row shows how CpGs breaks down to different categories. Each other row shows the how CpGs uniquely covered by the given data breaks down to these categories. It is the fraction of CpGs in the given category out of all CpGs covered by the data.', plot=table.plot(pd, hdr))
        pd = dict([ (sid, dd['cgi_coverage']) for sid, dd in self.mdata['qc_cpg_dist'].items() ])
        self.add_section(name='CpG Island Coverage', anchor='biscuit-coverage-cgi', description='Each row shows the percentage of CpG islands (out of all CpG islands in the genome) that are covered in different numbers of CpGs. Coverage is based on reads with mapQ >= 40.', plot=table.plot(pd, OrderedDict([
         (
          'one',
          {'title': '>=1', 'suffix': '%', 
         ...
         (
          'ten',
          {'title': '>=10', 'suffix': '%', 
             'description': 'CpG islands with at least ten CpGs covered'})]), {'id': 'cgi-cov-table'}))

    def parse_logs_dup_report(self, f, fn):
        patterns = [
         ('#bases covered by all reads: (\\d+)', '#bases covered by duplicate reads: (\\d+)',
 'all'),
         ('#high-GC bases covered by all reads: (\\d+)', '#high-GC bases covered by duplicate reads: (\\d+)',
 'topGC'),
         ('#low-GC bases covered by all reads: (\\d+)', '#low-GC bases covered by duplicate reads: (\\d+)',
 'lowGC'),
         ('#bases covered by all q40-reads: (\\d+)', '#bases covered by duplicate q40-reads: (\\d+)',
 'all-q40'),
         ('#high-GC bases covered by all q40-reads: (\\d+)', '#high-GC bases covered by duplicate q40-reads: (\\d+)',
 'topGC-q40'),
         ('#low-GC bases covered by all q40-reads: (\\d+)', '#low-GC bases covered by duplicate q40-reads: (\\d+)',
 'botGC-q40')]
        data = {}
    ...
    def parse_logs_markdup_report(self, f, fn):
        data = {}
        m = re.search('marked (\\d+) duplicates from (\\d+) paired-end reads', f, re.MULTILINE)
        if m is None:
            ...
            if data['PE'] > 0:
                ...
                data['dupRatePE'] = None
        m = re.search('marked (\\d+) duplicates from (\\d+) single-end reads', f, re.MULTILINE)
        if m is None:
            ...
            if data['SE'] > 0:
                ...
                data['dupRateSE'] = None
        m = re.search('identified (\\d+) dangling paired-end reads', f, re.MULTILINE)
        if m is None:
    ...
    def chart_retention_dist(self):
        ...
        self.add_section(name='Number of Retention Distribution', anchor='biscuit-retention-read', description='This plot shows the distribution of the number of retained cytosine in each read, up to 10.', plot=linegraph.plot(pd, {'id': 'biscuit_retention_read_cpa', 
           'xlab': 'Number of Retention within Read', 
           'title': 'BISCUIT: Retention Distribution', 
           'data_labels': [{'name': 'CpG retention', 'ylab': 'Fraction of cytosine in CpG context', 'xlab': 'Retention Level (%)'}, {'name': 'Within-read CpA', 'ylab': 'Number of Reads'}, {'name': 'Within-read CpC', 'ylab': 'Number of Reads'}, {'name': 'Within-read CpG', 'ylab': 'Number of Reads'}, {'name': 'Within-read CpT', 'ylab': 'Number of Reads'}]}))
    ...
    def chart_retention_cpg_readpos(self):
        ...
        self.add_section(name='Retention vs. Base Position in Read', anchor='biscuit-retention-cytosine', description='This plot (aka. mbias plot) shows the distribution of cytosine retention rate in read.', plot=linegraph.plot(mdata, {'id': 'biscuit_retention_cytosine', 
           'xlab': 'Position in Read', 
           'ymin': 0, 'ymax': 100, 'yMinRange': 0, 'yFloor': 0, 'title': 'BISCUIT: Retention vs. Base Position in Read', 
           'data_labels': [{'name': 'CpH Read 1', 'ylab': 'CpH Retention Rate (%)', 'ymin': 0, 'ymax': 100}, {'name': 'CpH Read 2', 'ylab': 'CpH Retention Rate (%)', 'ymin': 0, 'ymax': 100}, {'name': 'CpG Read 1', 'ylab': 'CpG Retention Rate (%)', 'ymin': 0, 'ymax': 100}, {'name': 'CpG Read 2', 'ylab': 'CpG Retention Rate (%)', 'ymin': 0, 'ymax': 100}]}))
    ...
    def parse_logs_retention_rate_byread(self, f, fn):
        data = {}
        m = re.match('([\\d.]+)\\t([\\d.]+)\\t([\\d.]+)\\t([\\d.]+)', f.splitlines()[2])
    ...
    def chart_retention_rate_byread(self):
        ...
        self.add_section(name='Cytosine Retention', anchor='biscuit-retention', description="This plot shows cytosine retention rate. `r.` stands for read-averaging and `b.` stands for 'base-averaging.", helptext='**Cytosine retention rate** is `1.0 - cytosine conversion rate`. Assuming full (complete but not over) bisulfite conversion, **cytosine retention rate** is the average cytosine modification (including 5mC, 5hmC etc) rate.', plot=table.plot(pdata, pheader))
