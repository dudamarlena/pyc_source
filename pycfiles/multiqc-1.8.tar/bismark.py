...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/bismark/bismark.py
...
regexes = {'alignment': {'total_reads': 'Sequence(?:s| pairs) analysed in total:\\s+(\\d+)', 
                 'aligned_reads': 'Number of(?: paired-end)? alignments with a unique best hit(?: from the different alignments)?:\\s+(\\d+)', 
                 'no_alignments': 'Sequence(?:s| pairs) with no alignments under any condition:\\s+(\\d+)', 
                 'ambig_reads': 'Sequence(?:s| pairs) did not map uniquely:\\s+(\\d+)', 
                 'discarded_reads': 'Sequence(?:s| pairs) which were discarded because genomic sequence could not be extracted:\\s+(\\d+)', 
                 'total_c': "Total number of C's analysed:\\s+(\\d+)", 
                 'meth_cpg': "Total methylated C's in CpG context:\\s+(\\d+)", 
                 'meth_chg': "Total methylated C's in CHG context:\\s+(\\d+)", 
                 'meth_chh': "Total methylated C's in CHH context:\\s+(\\d+)", 
                 'unmeth_cpg': "Total unmethylated C's in CpG context:\\s+(\\d+)", 
                 'unmeth_chg': "Total unmethylated C's in CHG context:\\s+(\\d+)", 
                 'unmeth_chh': "Total unmethylated C's in CHH context:\\s+(\\d+)", 
                 'percent_cpg_meth': 'C methylated in CpG context:\\s+([\\d\\.]+)%', 
                 'percent_chg_meth': 'C methylated in CHG context:\\s+([\\d\\.]+)%', 
                 'percent_chh_meth': 'C methylated in CHH context:\\s+([\\d\\.]+)%', 
                 'strand_ot': 'CT(?:\\/GA)?\\/CT:\\s+(\\d+)\\s+\\(\\(converted\\) top strand\\)', 
                 'strand_ctot': 'GA(?:\\/CT)?\\/CT:\\s+(\\d+)\\s+\\(complementary to \\(converted\\) top strand\\)', 
                 'strand_ctob': 'GA(?:\\/CT)?\\/GA:\\s+(\\d+)\\s+\\(complementary to \\(converted\\) bottom strand\\)', 
                 'strand_ob': 'CT(?:\\/GA)?\\/GA:\\s+(\\d+)\\s+\\(\\(converted\\) bottom strand\\)', 
                 'strand_directional': "Option '--(directional)' specified \\(default mode\\): alignments to complementary strands \\(CTOT, CTOB\\) were ignored \\(i.e. not performed\\)"}, 
   'dedup': {'aligned_reads': 'Total number of alignments analysed in .+:\\s+(\\d+)', 
             'dup_reads': 'Total number duplicated alignments removed:\\s+(\\d+)', 
             'dup_reads_percent': 'Total number duplicated alignments removed:\\s+\\d+\\s+\\(([\\d\\.]+)%\\)', 
             'dedup_reads': 'Total count of deduplicated leftover sequences:\\s+(\\d+)', 
             'dedup_reads_percent': 'Total count of deduplicated leftover sequences:\\s+\\d+\\s+\\(([\\d\\.]+)% of total\\)'}, 
   'methextract': {'total_c': "Total number of C's analysed:\\s+(\\d+)", 
                   'meth_cpg': "Total methylated C's in CpG context:\\s+(\\d+)", 
                   'meth_chg': "Total methylated C's in CHG context:\\s+(\\d+)", 
                   'meth_chh': "Total methylated C's in CHH context:\\s+(\\d+)", 
                   'unmeth_cpg': 'Total C to T conversions in CpG context:\\s+(\\d+)', 
                   'unmeth_chg': 'Total C to T conversions in CHG context:\\s+(\\d+)', 
                   'unmeth_chh': 'Total C to T conversions in CHH context:\\s+(\\d+)', 
                   'percent_cpg_meth': 'C methylated in CpG context:\\s+([\\d\\.]+)%', 
                   'percent_chg_meth': 'C methylated in CHG context:\\s+([\\d\\.]+)%', 
                   'percent_chh_meth': 'C methylated in CHH context:\\s+([\\d\\.]+)%'}}
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Bismark', anchor='bismark', href='http://www.bioinformatics.babraham.ac.uk/projects/bismark/', info='is a tool to map bisulfite converted sequence reads and determine cytosine methylation states.')
        self.bismark_data = {'alignment': {}, 'dedup': {}, 'methextract': {}, 'bam2nuc': {}}
        self.bismark_mbias_data = {'meth': {'CpG_R1': {}, 'CHG_R1': {}, 'CHH_R1': {}, 'CpG_R2': {}, 'CHG_R2': {}, 'CHH_R2': {}}, 'cov': {'CpG_R1': {}, 'CHG_R1': {}, 'CHH_R1': {}, 'CpG_R2': {}, 'CHG_R2': {}, 'CHH_R2': {}}}
    ...
    def parse_bismark_mbias(self, f):
        ...
        for t in self.bismark_mbias_data:
            for k in self.bismark_mbias_data[t]:
                self.bismark_mbias_data[t][k] = {s_name:self.bismark_mbias_data[t][k][s_name] for s_name in self.bismark_mbias_data[t][k] if len(self.bismark_mbias_data[t][k][s_name]) > 0}
    ...
    def bismark_stats_table(self):
        ...
        headers = {'alignment': OrderedDict(), 'dedup': OrderedDict(), 
           'methextract': OrderedDict(), 
           'bam2nuc': OrderedDict()}
        headers['methextract']['percent_cpg_meth'] = {'title': '% mCpG', 
           'description': '% Cytosines methylated in CpG context', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'Greens'}
        headers['methextract']['percent_chg_meth'] = {'title': '% mCHG', 
           'description': '% Cytosines methylated in CHG context', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'Oranges'}
        headers['methextract']['percent_chh_meth'] = {'title': '% mCHH', 
           'description': '% Cytosines methylated in CHH context', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'Oranges'}
        headers['methextract']['total_c'] = {'title': "M C's", 
           'description': "Total number of C's analysed, in millions", 
           'min': 0, 
           'scale': 'Purples', 
           'modify': lambda x: x / 1000000}
        headers['bam2nuc']['C_coverage'] = {'title': 'C Coverage', 
           'description': 'Cyotosine Coverage', 
           'min': 0, 
           'suffix': 'X', 
           'scale': 'Greens', 
           'format': '{:,.2f}'}
        headers['dedup']['dup_reads_percent'] = {'title': '% Dups', 
           'description': 'Percent Duplicated Alignments', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'RdYlGn-rev'}
        headers['dedup']['dedup_reads'] = {'title': ('{} Unique').format(config.read_count_prefix), 
           ...
           'hidden': True}
        headers['alignment']['aligned_reads'] = {'title': ('{} Aligned').format(config.read_count_prefix), 
           ...
           'shared_key': 'read_count', 
           'hidden': True}
        headers['alignment']['percent_aligned'] = {'title': '% Aligned', 
    ...
    def bismark_strand_chart(self):
        ...
        config = {'id': 'bismark_strand_alignment', 
    ...
    def bismark_dedup_chart(self):
        ...
        config = {'id': 'bismark_deduplication', 
    ...
    def bismark_methlyation_chart(self):
        ...
        self.add_section(name='Cytosine Methylation', anchor='bismark-methylation', plot=beeswarm.plot(self.bismark_data['methextract'], keys, {'id': 'bismark-methylation-dp'}))
    ...
    def bismark_mbias_plot(self):
        ...
        pconfig = {'id': 'bismark_mbias', 
           ...
           'data_labels': [{'name': 'CpG R1', 'ylab': '% Methylation', 'ymax': 100}, {'name': 'CHG R1', 'ylab': '% Methylation', 'ymax': 100}, {'name': 'CHH R1', 'ylab': '% Methylation', 'ymax': 100}]}
        datasets = [
        ...
        self.add_section(name='M-Bias', anchor='bismark-mbias', description=description, plot=linegraph.plot(datasets, pconfig))