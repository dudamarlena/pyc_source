...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/flash/flash.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='FLASh', anchor='flash', href='https://ccb.jhu.edu/software/FLASH/', info='is a very fast and accurate software tool to merge paired-end reads from next-generation sequencing experiments.')
        ...
        try:
            ...
            self.add_section(name='Read combination statistics', anchor='flash-bargraph', plot=self.summary_plot(self.flash_data))
    ...
    def clean_pe_name(self, nlog, root):
        ...
        if use_output_name:
            name = re.search('Output files\\:\\n\\[FLASH\\]\\s+(.+?)\\n', nlog)
        else:
            name = re.search('Input files\\:\\n\\[FLASH\\]\\s+(.+?)\\n', nlog)
        if not name:
            return None
        else:
            name = name.group(1)
    ...
    def stats_table(self, data):
        ...
        headers = OrderedDict()
        headers['combopairs'] = {'title': 'Combined pairs', 
           'description': 'Num read pairs combined', 
           'shared_key': 'read_count', 
           'hidden': True, 
           'scale': False}
        headers['perccombo'] = {'title': '% Combined', 
    ...
    def summary_plot(data):
        ...
        cats = OrderedDict()
        cats = {'inniepairs': {'name': 'Combined innie pairs', 
                          'color': '#191970'}, 
           'outiepairs': {'name': 'Combined outie pairs', 
                          'color': '#00A08A'}, 
           'uncombopairs': {'name': 'Uncombined pairs', 
                            'color': '#cd1076'}, 
           'discardpairs': {'name': 'Discarded pairs', 
                            'color': '#ffd700'}}
    ...
    def freqpoly_plot(data):
        ...
        fplotconfig = {'data_labels': [{'name': 'Absolute', 'ylab': 'Frequency', 'xlab': 'Merged Read Length'}, {'name': 'Relative', 'ylab': 'Relative Frequency', 'xlab': 'Merged Read Length'}], 'id': 'flash_freqpoly_plot', 
           'title': 'FLASh: Frequency of merged read lengths', 'colors': dict(zip(data.keys(), MultiqcModule.get_colors(len(data))))}
