...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/kallisto/kallisto.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Kallisto', anchor='kallisto', href='http://pachterlab.github.io/kallisto/', info='is a program for quantifying abundances of transcripts from RNA-Seq data.')
    ...
    def parse_kallisto_log(self, f):
        ...
        for l in f['f']:
            match = re.search('\\[quant\\] will process (pair|file) 1: (\\S+)', l)
            if match:
                s_name = self.clean_s_name(os.path.basename(match.group(2)), f['root'])
            if s_name is not None:
                aligned = re.search('\\[quant\\] processed ([\\d,]+) reads, ([\\d,]+) reads pseudoaligned', l)
                if aligned:
                    total_reads = float(aligned.group(1).replace(',', ''))
                    paligned_reads = float(aligned.group(2).replace(',', ''))
                flength = re.search('\\[quant\\] estimated average fragment length: ([\\d\\.]+)', l)
                if flength:
                ...
                if 'quantifying the abundances' in l:
                    ...
                    self.kallisto_data[s_name] = {'total_reads': total_reads, 
    ...
    def kallisto_general_stats_table(self):
        ...
        headers = OrderedDict()
        headers['fragment_length'] = {'title': 'Frag Length', 
           'description': 'Estimated average fragment length', 
           'min': 0, 
           'suffix': 'bp', 
           'scale': 'RdYlGn'}
        headers['percent_aligned'] = {'title': '% Aligned', 
           'description': '% processed reads that were pseudoaligned', 
           'max': 100, 
           'min': 0, 
           'suffix': '%', 
           'scale': 'YlGn'}
        headers['pseudoaligned_reads'] = {'title': ('{} Aligned').format(config.read_count_prefix), 
