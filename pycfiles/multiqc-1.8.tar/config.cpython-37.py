...
git_hash_short = None
try:
    git_hash = subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=script_path,
    ...
    for c, v in conf.items():
        if c == 'sp':
            ...
            if os.path.exists(v):
                ...
                if conf_path is not None and os.path.exists(os.path.join(os.path.dirname(conf_path), v)):
