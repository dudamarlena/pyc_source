...
def parse_reports(self):
    ...
    if len(self.read_gc) > 0:
        pconfig = {'id': 'rseqc_read_gc_plot', 
           ...
           'data_labels': [{'name': 'Counts', 'ylab': 'Number of Reads'}, {'name': 'Percentages', 'ylab': 'Percentage of Reads'}]}
        self.add_section(name='Read GC Content', anchor='rseqc-read_gc', description='<a href="http://rseqc.sourceforge.net/#read-gc-py" target="_blank">read_GC</a> calculates a histogram of read GC content.</p>', plot=linegraph.plot([self.read_gc, self.read_gc_pct], pconfig))
