...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/supernova/supernova.py
# Compiled at: 2019-11-13 05:22:42
# Size of source mod 2**32: 21518 bytes
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Supernova', anchor='supernova', href='https://www.10xgenomics.com/',
          info='is a de novo genome assembler 10X Genomics linked-reads.')
        self.headers = OrderedDict()
        self.headers['Asm size'] = {'description':'assembly size (in megabases) ;only scaffolds >= 10 kb', 
         'modify':lambda x: x / 1000000.0, 
         'suffix':'Mb', 
         'scale':'YlGn'}
        self.headers['% missing 10Kb'] = {'rid':'pct_missing_10Kb', 
         'description':'% of base assembly missing from scaffolds >= 10 kb', 
         'suffix':'%', 
         'scale':'YlGn'}
        self.headers['# Long scaffs'] = {'rid':'num_long_scaffs', 
         'description':'number of scaffolds >= 10 kb', 
         'scale':'YlGn', 
         'format':'{:,.0f}', 
         'hidden':True}
        self.headers['Scaff N50'] = {'description':'N50 scaffold size (in kilobases)', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'RdYlGn'}
        self.headers['Phase N50'] = {'description':'N50 phase block size (in kilobases)', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'RdYlGn', 
         'hidden':True}
        self.headers['Contig N50'] = {'description':'N50 contig size (in kilobases)', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'RdYlGn', 
         'hidden':True}
        self.headers['Edge N50'] = {'description':'N50 edge size (in kilobases)', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'RdYlGn', 
         'hidden':True}
        self.headers['Mol size'] = {'description':'weighted mean molecule size (in kilobases); ideal 50-100', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'BuGn'}
        self.headers['Read len'] = {'description':'mean read length (in bases) after trimming; ideal 140', 
         'suffix':'b', 
         'scale':'PuBu', 
         'format':'{:,.0f}', 
         'hidden':True}
        self.headers['# Reads'] = {'rid':'num_reads', 
         'description':'number of reads (in millions); ideal 800M-1200M for human', 
         'modify':lambda x: x / 1000000.0, 
         'suffix':'M', 
         'scale':'PuBu'}
        self.headers['Raw coverage'] = {'description':'raw coverage; ideal ~56', 
         'suffix':'x', 
         'scale':'PuBu', 
         'hidden':True}
        self.headers['Coverage'] = {'description':'effective read coverage; ideal ~42 for nominal 56x cov', 
         'suffix':'x', 
         'scale':'PuBu'}
        self.headers['% Dup'] = {'rid':'pct_Dup', 
         'description':'fraction of reads that are duplicates', 
         'suffix':'%', 
         'scale':'OrRd'}
        self.headers['% R2 Q30'] = {'rid':'pct_R2_Q30', 
         'description':'fraction of Q30 bases in read 2; ideal 75-85%', 
         'suffix':'%', 
         'scale':'OrRd'}
        self.headers['Insert size'] = {'description':'median insert size (in bases); ideal 0.35-0.40 Kb', 
         'suffix':'b', 
         'scale':'OrRd', 
         'format':'{:,.0f}', 
         'hidden':True}
        self.headers['% proper'] = {'rid':'pct_proper', 
         'description':'fraction of proper read pairs; ideal >= 75%', 
         'suffix':'%', 
         'scale':'OrRd', 
         'hidden':True}
        self.headers['BC usage'] = {'description':'fraction of barcodes used; between 0 and 1', 
         'scale':'OrRd', 
         'hidden':True}
        self.headers['Est size'] = {'description':'estimated genome size', 
         'modify':lambda x: x / 1000000.0, 
         'suffix':'Mb', 
         'scale':'YlGn', 
         'hidden':True}
        self.headers['% repeats'] = {'rid':'pct_repeats', 
         'description':'Estimated repetitive fraction (of genome)', 
         'scale':'YlGn', 
         'suffix':'%', 
         'hidden':True}
        self.headers['% AT'] = {'rid':'pct_AT', 
         'description':'high AT index (of genome)', 
         'scale':'YlGn', 
         'suffix':'%', 
         'hidden':True}
        self.headers['Het dist'] = {'description':'mean distance between heterozygous SNPs (in kilobases)', 
         'modify':lambda x: x / 1000.0, 
         'suffix':'Kb', 
         'scale':'YlGn', 
         'format':'{:,.0f}', 
         'hidden':True}
        self.headers['p10'] = {'description':'molecule count extending 10 kb on both sides', 
         'scale':'BuGn', 
         'hidden':True}
        self.headers['% missing BC'] = {'rid':'pct_missing_BC', 
         'description':'fraction of reads that are not barcoded', 
         'suffix':'%', 
         'scale':'BuGn'}
        self.headers['Barcode N50'] = {'description':'N50 reads per barcode (in bases)', 
         'suffix':'b', 
         'scale':'BuGn', 
         'format':'{:,.0f}'}
        self.headers['% Phased'] = {'rid':'pct_Phased', 
        ...
        config_table = {'id':'supernova_table', 
         'namespace':'supernova'}
        self.add_section(name='Assembly statistics',
          anchor='supernova-table',
          description='Statistics gathered from the summary report(s) of Supernova. Note! There are more columns available but they are hidden by default.',
          helptext='As a bare minimum these numbers are generated from the file report.txt, found in the folder `sampleID/outs/`. If available the stats in the report file will be superseded by the higher precision numbers found in the file `sampleID/outs/assembly/stats/summary.json`',
        ...
        if len(molecules) > 0:
            try:
                ...
                config_molecules = {'id':'supernova_molecules', 
                 ...
                 'xmax':max_x}
                self.add_section(name='Molecule Lengths',
                  anchor='supernova-molecules',
                  description='Shows the inferred molecule lengths of the input 10X library.',
                  helptext='Inferred in the `patch` step of the Supernova pipeline. It is worth keeping in mind that the mean molecule length from the report is a length-weighted mean. See the [source code](https://github.com/10XGenomics/supernova/search?q=lw_mean_mol_len&type=) for how this value is calculated.',
    ...
    def parse_summary(self, content):
        ...
        if 'CS_SAMPLE_ID' in cdict.keys():
            sid = cdict['CS_SAMPLE_ID']
        else:
            if 'sample_id' in cdict.keys():
                sid = cdict['sample_id']
            else:
                raise RuntimeError
