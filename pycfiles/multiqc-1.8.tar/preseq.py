...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/preseq/preseq.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def _make_preseq_length_trimmed_plot(self, data_raw, is_basepairs):
        ...
        d_covs = {sn:(_counts_to_coverages(sample_data, counts_in_1x) if counts_in_1x else None) for sn, sample_data in data_raw.items()}
        data = dict()
        ...
        x_lbl, y_tt_lbl, x_axis_name, y_lbl, x_tt_lbl, y_axis_name = _prepare_labels(is_basepairs, max_y_cov, x_axis, y_axis)
        description = ''
        pconfig = {'id': 'preseq_plot', 
        ...
        real_vals_all, real_vals_unq = _prep_real_counts(real_cnts_all, real_cnts_unq, is_basepairs, counts_in_1x, x_axis, y_axis)
        ...
        if getattr(config, 'preseq', {}).get('notrim', False) is not True:
            ...
            pconfig['xmax'] = max_x
            description += '<p>Note that the x axis is trimmed at the point where all the datasets                 show 80% of their maximum y-value, to avoid ridiculous scales.</p>'
        pconfig['extra_series'].append({'name': 'a perfect library where each read is unique', 
           'data': [
                  [
                   0, 0], [max_yx, max_y]], 
           'dashStyle': 'Dash', 
        ...
        self.add_section(name='Complexity curve', description=description, anchor='preseq-plot', plot=linegraph.plot(data, pconfig))
    ...
    def _parse_real_counts(self, sample_names):
        ...
        return (
         real_counts_total, real_counts_unique)
...
def _parse_preseq_logs(f):
    ...
    if header.startswith('TOTAL_READS\tEXPECTED_DISTINCT'):
        ...
        if header.startswith('TOTAL_BASES\tEXPECTED_DISTINCT'):
            data_is_bases = True
...
def _modify_raw_data(sample_data, is_basepairs):
    ...
    """
    return OrderedDict((_modify_raw_val(x, is_basepairs), _modify_raw_val(y, is_basepairs)) for x, y in sample_data.items())
...
def _get_counts_in_1x(data_is_basepairs):
    ...
    if genome_size:
        try:
            ...
            presets = {'hg19_genome': 2897310462, 'hg38_genome': 3049315783, 
    ...
    if xs_by_sample:
        ...
        for si, sn in enumerate(sorted(data.keys())):
            if sn in xs_by_sample:
                ...
                point = {'color': default_colors[(si % len(default_colors))], 
                   'showInLegend': False, 
                   'marker': {'enabled': True, 
                              'symbol': 'diamond', 
                              'lineColor': 'black', 
                ...
                if sn in yx_by_sample:
                    ...
                    if x > max(xs):
                        log.warning(('Total reads for {} ({}) > max preseq value ({}) - skipping this point..').format(sn, x, max(xs)))
                    else:
                        interp_y = np.interp(x, xs, ys)
                        point['data'] = [[x, interp_y]]
                        point['name'] = sn + ': actual read count (externally calculated)'
                        series.append(point)
                        log.debug(('Found real count for {} - Total: {:.2f} (preseq unique reads: {:.2f})').format(sn, x, interp_y))
