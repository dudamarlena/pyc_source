...
class estimateReadFilteringMixin:
    ...
    def parse_estimateReadFiltering(self):
        ...
        if len(self.deeptools_estimateReadFiltering) > 0:
            ...
            self.add_section(name='Filtering metrics', anchor='estimateReadFiltering', description='Estimated percentages of alignments filtered independently for each setting in `estimateReadFiltering`', plot=table.plot(tdata, header, config))
