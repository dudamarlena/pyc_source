...
def parse_reports(parent_module):
    ...
    if len(data) > 0:
        ...
        parent_module.add_section(name='Variant Types',
        ...
        parent_module.add_section(name='Variant Labels',
...
def table_in(filehandle, pre_header_string):
    ...
    for line in stripped(filehandle):
        if not in_histogram:
            if line.startswith(pre_header_string):
                in_histogram = True
                next_is_header = True
        if in_histogram:
            if next_is_header:
...
def derive_data(data):
    ...
    for s_name, values in data.items():
        ...
        for value_name in ('TOTAL_SNPS', 'TOTAL_COMPLEX_INDELS', 'TOTAL_MULTIALLELIC_SNPS',
                           'TOTAL_INDELS'):
