...
class Relatedness2Mixin:
    ...
    def parse_relatedness2(self):
        ...
        for name, m in matrices.items():
            idx += 1
            self.add_section(name='Relatedness2', anchor=('vcftools-relatedness2-{}').format(idx), description=('**Input:** `{}`.\n\n Heatmap of `RELATEDNESS_PHI` values from the output of vcftools relatedness2.').format(name), helptext=helptext, plot=heatmap.plot(m.data, xcats=m.x_labels, ycats=m.y_labels, pconfig={'id': ('vcftools-relatedness2-heatmap-{}').format(idx), 
