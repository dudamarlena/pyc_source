...
# Size of source mod 2**32: 14705 bytes
...
def _default_data_entry():
    return {'WARNING_count':0, 
     'ERROR_count':0,  'file_validation_status':'pass'}
...
def _parse_reports_by_type(self):
    ...
    for file_meta in self.find_log_files('picard/sam_file_validation', filehandles=True):
        ...
        if 'No errors found' in first_line:
            ...
            if first_line.startswith('ERROR') or first_line.startswith('WARNING'):
...
def _add_section_to_report(self, data):
    ...
    for sample_data in data.values():
        if sample_data['file_validation_status'] == 'fail':
            ...
            if sample_data['file_validation_status'] == 'warn':
                only_warning_count += 1
            else:
                pass_count += 1

    plot_html = []
    note_html = _generate_overview_note(pass_count=pass_count,
    ...
    self.add_section(name='SAM/BAM File Validation',
...
def _generate_overview_note(pass_count, only_warning_count, error_count, total_count):
    ...
    for b in pbars:
        if b[0]:
            note_html.append('<div class="progress-bar progress-bar-{pbcol}" style="width: {pct}%" data-toggle="tooltip" title="{count} {sample} {txt}">{count}</div>'.format(pbcol=(b[1]),
              count=(int(b[0])),
    ...
    for problems in data.values():
        for problem in problems:
            if problem not in headers:
                if problem in WARNING_DESCRIPTIONS:
