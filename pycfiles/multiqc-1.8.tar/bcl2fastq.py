...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/bcl2fastq/bcl2fastq.py
# Compiled at: 2019-11-20 10:26:16
import json, logging, operator, os
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='bcl2fastq', anchor='bcl2fastq', href='https://support.illumina.com/sequencing/sequencing_software/bcl2fastq-conversion-software.html', info='can be used to both demultiplex data and convert BCL files to FASTQ file formats for downstream analysis.')
        ...
        self.add_section(name='Lane Statistics', anchor='bcl2fastq-lanestats', description='Statistics about each lane for each flowcell', plot=self.lane_stats_table())
        cats = OrderedDict()
        cats['perfect'] = {'name': 'Perfect Index Reads'}
        cats['imperfect'] = {'name': 'Mismatched Index Reads'}
        cats['undetermined'] = {'name': 'Undetermined Reads'}
        self.add_section(name='Clusters by lane', anchor='bcl2fastq-bylane', description='Number of reads per lane (with number of perfect index reads).', helptext='Perfect index reads are those that do not have a single mismatch.\n                All samples of a lane are combined. Undetermined reads are treated as a third category.', plot=bargraph.plot(self.get_bar_data_from_counts(self.bcl2fastq_bylane), cats, {'id': 'bcl2fastq_lane_counts', 
        ...
        self.add_section(name='Clusters by sample', anchor='bcl2fastq-bysample', description='Number of reads per sample.', helptext='Perfect index reads are those that do not have a single mismatch.\n                All samples are aggregated across lanes combinned. Undetermined reads are ignored.\n                Undetermined reads are treated as a separate sample.', plot=bargraph.plot([
         self.get_bar_data_from_counts(self.bcl2fastq_bysample),
         self.bcl2fastq_bysample_lane], [
         cats, lcats], {'id': 'bcl2fastq_sample_counts', 
           'title': 'bcl2fastq: Clusters by sample', 
           'hide_zero_cats': False, 
           'ylab': 'Number of clusters', 
           'data_labels': [
                         'Index mismatches', 'Counts per lane']}))
        self.add_section(name='Undetermined barcodes by lane', anchor='undetermine_by_lane', description='Count of the top twenty most abundant undetermined barcodes by lanes', plot=bargraph.plot(self.get_bar_data_from_undetermined(self.bcl2fastq_bylane), None, {'id': 'bcl2fastq_undetermined', 
    ...
    def parse_file_as_json(self, myfile):
        ...
        for conversionResult in content.get('ConversionResults', []):
            ...
            run_data[lane] = {'total': 0, 'total_yield': 0, 
               'perfectIndex': 0, 
               'samples': dict(), 
               'yieldQ30': 0, 
               'qscore_sum': 0}
            ...
            for demuxResult in conversionResult.get('DemuxResults', []):
                ...
                run_data[lane]['samples'][sample] = {'total': 0, 'total_yield': 0, 
                   'perfectIndex': 0, 
            ...
            if 'Undetermined' in conversionResult:
                ...
                run_data[lane]['samples']['undetermined'] = {'total': conversionResult['Undetermined']['NumberReads'], 'total_yield': conversionResult['Undetermined']['Yield'], 
                   'perfectIndex': 0, 
    ...
    def split_data_by_lane_and_sample(self):
        for run_id, r in self.bcl2fastq_data.items():
            for lane_id, lane in r.items():
                ...
                self.bcl2fastq_bylane[uniqLaneName] = {'total': lane['total'], 
                ...
                for sample_id, sample in lane['samples'].items():
                    if sample_id not in self.bcl2fastq_bysample:
                        self.bcl2fastq_bysample[sample_id] = {'total': 0, 'total_yield': 0, 
                           'perfectIndex': 0, 
                           'yieldQ30': 0, 
                           'qscore_sum': 0}
                        for r in range(1, 5):
    ...
    def add_general_stats(self):
        ...
        headers = OrderedDict()
        headers['total'] = {'title': ('{} Clusters').format(config.read_count_prefix), 
        ...
        headers['yieldQ30'] = {'title': ('{} Yield &ge; Q30').format(config.base_count_prefix), 
           'description': ('Number of bases with a Phred score of 30 or higher ({})').format(config.base_count_desc), 
           'scale': 'Greens', 
           'shared_key': 'base_count'}
        for r in range(1, 5):
            headers[('percent_R{}_Q30').format(r)] = {'title': ('% R{} Yield &ge; Q30').format(r), 
        ...
        headers['perfectPercent'] = {'title': '% Perfect Index', 
        ...
        for r in range(1, 5):
            ...
            try:
                headers[('R{}_trimmed_bases').format(r)] = {'title': ('{} R{} trimmed').format(config.base_count_prefix, r), 
    ...
    def lane_stats_table(self):
        ...
        headers['percent_perfectIndex'] = {'title': '% Perfect Index', 
           ...
           'scale': 'RdYlGn', 
           'suffix': '%'}
        table_config = {'namespace': 'bcl2fastq', 
    ...
    def get_bar_data_from_counts(self, counts):
        ...
        for key, value in counts.items():
            bar_data[key] = {'perfect': value['perfectIndex'], 
