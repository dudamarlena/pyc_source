...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/gatk/varianteval.py
...
class VariantEvalMixin:
    ...
    def parse_gatk_varianteval(self):
        ...
        if n_reports_found > 0:
            ...
            varianteval_headers['known_titv'] = {'title': 'TiTV ratio (known)', 
               'description': ("TiTV ratio from variants found in '{}'").format(titv_ref), 
               'min': 0, 
               'scale': 'Blues', 
               'shared_key': 'titv_ratio'}
            varianteval_headers['novel_titv'] = {'title': 'TiTV ratio (novel)', 
            ...
            self.add_section(name='Variant Counts', anchor='gatk-count-variants', plot=count_variants_barplot(self.gatk_varianteval))
            self.add_section(name='Compare Overlap', anchor='gatk-compare-overlap', plot=comp_overlap_table(self.gatk_varianteval))
...
def count_variants_barplot(data):
    ...
    plot_conf = {'id': 'gatk_varianteval_variant_plot', 
...
def comp_overlap_table(data):
    ...
    headers = OrderedDict()
    headers['comp_rate'] = {'title': 'Compare rate', 
       ...
       'format': '{:,.2f}', 
       'scale': 'Blues'}
    headers['concordant_rate'] = {'title': 'Concordant rate', 
       ...
       'format': '{:,.2f}', 
       'scale': 'Blues'}
    headers['eval_variants'] = {'title': 'M Evaluated variants', 
       'description': 'Number of called variants (millions)', 
       'namespace': 'GATK', 
       'min': 0, 
       'modify': lambda x: float(x) / 1000000.0}
    headers['known_sites'] = {'title': 'M Known sites', 
       'description': 'Number of known variants (millions)', 
       'namespace': 'GATK', 
       'min': 0, 
       'modify': lambda x: float(x) / 1000000.0}
    headers['novel_sites'] = {'title': 'M Novel sites', 
