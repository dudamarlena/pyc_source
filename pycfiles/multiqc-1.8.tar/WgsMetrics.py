...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/picard/WgsMetrics.py
...
def parse_reports(self):
    ...
    if len(self.picard_wgsmetrics_data) > 0:
        ...
        for c in covs:
            self.general_stats_headers[('PCT_{}X').format(c)] = {'id': ('picard_target_bases_{}X').format(c), 
        ...
        if len(self.picard_wgsmetrics_histogram) > 0:
            ...
            pconfig = {'id': 'picard_wgs_metrics_histogram', 
               ...
               'ymin': 0, 
               'ymax': 100, 
               'data_labels': [{'name': 'Percentage Drop-Off', 'ylab': 'Percentage of Bases', 'ymax': 100}, {'name': 'Counts Histogram', 'ylab': 'Coverage', 'ymax': maxval}]}
            self.add_section(name='WGS Coverage', anchor='picard-wgsmetrics-cov', description='The number of bases in the genome territory for each fold coverage. ' + 'Note that final 1% of data is hidden to prevent very long tails.', plot=linegraph.plot([data_percent, data], pconfig))
            pdata = dict()
            ...
            pconfig = {'id': 'picard_wgs_metrics_bases', 
               'title': 'Picard: WGS Filtered Bases', 
               'cpswitch': False, 
               'ylab': '% Bases', 
               'ymax': 100}
            self.add_section(name='WGS Filtered Bases', anchor='picard-wgsmetrics-bases', description='For more information about the filtered categories, see the ' + '<a href="http://broadinstitute.github.io/picard/picard-metric-definitions.html#CollectWgsMetrics.WgsMetrics" target="_blank">Picard documentation</a>.', plot=bargraph.plot(pdata, keys, pconfig))
