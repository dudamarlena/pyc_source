...
class plotEnrichmentMixin:
    ...
    def parse_plotEnrichment(self):
        ...
        if len(self.deeptools_plotEnrichment) > 0:
            ...
            config = {'data_labels':[
              {'name':'Counts in features', 
               'ylab':'Counts in feature'},
              {'name':'Percents in features', 
               'ylab':'Percent of reads in feature'}],  'id':'deeptools_enrichment_plot', 
             'title':'deepTools: Signal enrichment per feature', 
             'ylab':'Counts in feature', 
             'categories':True, 
             'ymin':0.0}
            self.add_section(name='Feature enrichment', description='Signal enrichment per feature according to plotEnrichment',
