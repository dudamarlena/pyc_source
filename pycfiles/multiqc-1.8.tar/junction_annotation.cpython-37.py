...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/rseqc/junction_annotation.py
...
def parse_reports(self):
    ...
    regexes = {'total_splicing_events':'^Total splicing  Events:\\s*(\\d+)$', 
     'known_splicing_events':'^Known Splicing Events:\\s*(\\d+)$', 
     'partial_novel_splicing_events':'^Partial Novel Splicing Events:\\s*(\\d+)$', 
     'novel_splicing_events':'^Novel Splicing Events:\\s*(\\d+)$', 
     'total_splicing_junctions':'^Total splicing  Junctions:\\s*(\\d+)$', 
     'known_splicing_junctions':'^Known Splicing Junctions:\\s*(\\d+)$', 
     'partial_novel_splicing_junctions':'^Partial Novel Splicing Junctions:\\s*(\\d+)$', 
     'novel_splicing_junctions':'^Novel Splicing Junctions:\\s*(\\d+)$'}
    ...
    if len(self.junction_annotation_data) > 0:
        ...
        pconfig = {'id':'rseqc_junction_annotation_junctions_plot', 
         'title':'RSeQC: Splicing Junctions', 
         'ylab':'% Junctions', 
         'cpswitch_c_active':False, 
         'data_labels':[
          'Junctions', 'Events']}
        self.add_section(name='Junction Annotation',
          anchor='rseqc_junction_annotation',
          description='<a href="http://rseqc.sourceforge.net/#junction-annotation-py" target="_blank">Junction annotation</a> compares detected splice junctions to a reference gene model. An RNA read can be spliced 2 or more times, each time is called a splicing event.',
