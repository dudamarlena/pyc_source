...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='MinIONQC', anchor='minionqc', href='https://github.com/roblanf/minion_qc', info=' is a QC tool for Oxford Nanopore sequencing data')
    ...
    def table_qALL(self):
        ...
        self.add_section(name='Stats: All reads', anchor='minionqc-stats-qAll', description='MinIONQC statistics for all reads', plot=table.plot(self.minionqc_data, self.headers_to_use(), {'namespace': 'MinIONQC', 
    ...
    def table_qfiltered(self):
        ...
        self.add_section(name='Stats: Quality filtered reads', anchor='minionqc-stats-qFilt', description=description, plot=table.plot(self.qfilt_data, self.headers_to_use(), {'namespace': 'MinIONQC', 
    ...
    def plot_readlengths(self):
        ...
        pconfig = {'id': 'minionqc_read_lengths', 
           ...
           'categories': True, 
           'data_labels': [{'name': 'All reads: Num reads', 'ylab': '# reads'}, {'name': 'All reads: Num gigabases', 'ylab': '# gigabases'}]}
        for qfilt in list(self.q_threshold_list):
            try:
                ...
                pconfig['data_labels'].extend([{'name': ('{}: Num reads').format(qfilt), 'ylab': '# reads'}, {'name': ('{}: Num gigabases').format(qfilt), 'ylab': '# gigabases'}])
            except KeyError:
                pass

        self.add_section(name='Read length output', anchor='minionqc-read-length-output', description='Number of reads / bp sequenced at given read length thresholds.', plot=linegraph.plot(pdata, pconfig=pconfig))