...
def make_table(dt):
    ...
    for idx, k, header in dt.get_headers_in_order():
        ...
        t_headers[rid] = ('<th id="header_{rid}" class="{rid} {h}" {da}>{c}</th>').format(rid=rid, h=hide, da=data_attr, c=cell_contents)
        empty_cells[rid] = ('<td class="data-coloured {rid} {h}"></td>').format(rid=rid, h=hide)
        t_modal_headers[rid] = ('\n        <tr class="{rid}{muted}" style="background-color: rgba({col}, 0.15);">\n          <td class="sorthandle ui-sortable-handle">||</span></td>\n          <td style="text-align:center;">\n            <input class="mqc_table_col_visible" type="checkbox" {checked} value="{rid}" data-target="#{tid}">\n          </td>\n          <td>{name}</td>\n          <td>{title}</td>\n          <td>{desc}</td>\n          <td>{col_id}</td>\n          <td>{sk}</td>\n        </tr>').format(rid=rid, muted=muted, checked=checked, tid=table_id, col=header['colour'], name=header['namespace'], title=header['title'], desc=header['description'], col_id=('<code>{}</code>').format(k), sk=header.get('shared_key', ''))
