...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/homer/tagdirectory.py
...
class TagDirReportMixin:
    ...
    def parse_gc_content(self):
        ...
        if len(self.tagdir_data['GCcontent']) > 0:
            self.add_section(name='Per Sequence GC Content', anchor='homer_per_sequence_gc_content', description='This plot shows the distribution of GC content.', plot=self.GCcontent_plot())
        return
    ...
    def parse_re_dist(self):
        ...
        if len(self.tagdir_data['restriction']) > 0:
            self.add_section(name='Restriction Site Tag Dist', anchor='homer-restrictionDist', description='This plot shows the distribution of tags around restriction enzyme cut sites.', helptext='Hi-C data often involves the digestion of DNA using a restriction enzyme.\n                    A good quality control for the experiment is the centering of reads around the\n                    restriction enzyme cut site.', plot=self.restriction_dist_chart())
        return
    ...
    def parse_tagLength_dist(self):
        ...
        if len(self.tagdir_data['length']) > 0:
            self.add_section(name='Tag Length Distribution', anchor='homer-tagLength', description='This plot shows the distribution of tag length.', helptext='This is a good quality control for tag length inputed into Homer.', plot=self.length_dist_chart())
        return
    ...
    def parse_tagInfo_data(self):
        ...
        if len(self.tagdir_data['taginfo_total']) > 0:
            self.add_section(name='Chromosomal Coverage', anchor='homer-tagInfo', description='This plot shows the distribution of tags along chromosomes.', helptext='This is a good quality control for tag distribution and\n                        could be a good indication of large duplications or deletions.', plot=self.tag_info_chart())
        return
    ...
    def parse_FreqDistribution_data(self):
        ...
        if len(self.tagdir_data['FreqDistribution']) > 0:
            self.add_section(name='Frequency Distribution', anchor='homer-FreqDistribution', description='This plot shows the distribution of distance between PE tags.', helptext='It is expected the the frequency of PE tags decays with\n                    increasing distance between the PE tags. This plot gives an idea\n                     of the proportion of short-range versus long-range interactions.', plot=self.FreqDist_chart())
        return
    ...
    def homer_stats_table_tagInfo(self):
        ...
        if len(self.tagdir_data['header']) == 0:
            ...
            headers = OrderedDict()
            headers['UniqPositions'] = {'title': 'Uniq Pos', 
               'description': 'Numer of Unique Di-Tags Passed Through HOMER', 
               'format': '{:,.0f}', 
               'modify': lambda x: x * 1e-06, 
               'suffix': 'M'}
            headers['TotalPositions'] = {'title': 'Total Pos', 
               'description': 'Numer of Total Di-Tags Passed Through HOMER', 
               'format': '{:,.0f}', 
               'modify': lambda x: x * 1e-06, 
               'suffix': 'M'}
            headers['fragmentLengthEstimate'] = {'title': 'fragment Length', 
               'description': 'Estimate of Fragnment Length', 
               'format': '{:,.0f}'}
            headers['peakSizeEstimate'] = {'title': 'Peak Size', 
               'description': 'Estimate of Peak Size', 
               'format': '{:,.0f}'}
            headers['tagsPerBP'] = {'title': 'tagsPerBP', 
               'description': 'average tags Per basepair', 
               'format': '{:,.3f}'}
            headers['TagsPerPosition'] = {'title': 'averageTagsPerPosition', 
               'description': 'Average Tags Per Position', 
               'format': '{:,.2f}'}
            headers['averageTagLength'] = {'title': 'TagLength', 
               'description': 'Average Tag Length', 
               'format': '{:,.0f}'}
            headers['averageFragmentGCcontent'] = {'title': 'GCcontent', 
    ...
    def homer_stats_table_interChr(self):
        ...
        headers = OrderedDict()
        headers['InterChr'] = {'title': 'InterChr', 
    ...
    def restriction_dist_chart(self):
        ...
        pconfig = {'id': 'petagRestrictionDistribution', 
    ...
    def length_dist_chart(self):
        ...
        pconfig = {'id': 'tagLengthDistribution', 
           'cpswitch': True, 
    ...
    def GCcontent_plot(self):
        ...
        pconfig = {'id': 'homer-tag-directory-gc-content', 
