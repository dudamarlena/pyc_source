...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Supernova', anchor='supernova', href='https://www.10xgenomics.com/', info='is a de novo genome assembler 10X Genomics linked-reads.')
        ...
        if len(molecules) > 0:
            ...
            config_molecules = {'id': 'supernova_molecules', 
               ...
               'xmax': max_x}
            self.add_section(name='Molecule Lengths', anchor='supernova-molecules', description='Shows the inferred molecule lengths of the input 10X library.', helptext='Inferred in the `patch` step of the Supernova pipeline. It is worth keeping in mind that the mean molecule length from the report is a length-weighted mean. See the [source code](https://github.com/10XGenomics/supernova/search?q=lw_mean_mol_len&type=) for how this value is calculated.', plot=linegraph.plot(molecules, config_molecules))
        if len(kmers) > 0:
            try:
