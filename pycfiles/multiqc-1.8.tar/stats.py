...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/samtools/stats.py
...
class StatsReportMixin:
    ...
    def parse_samtools_stats(self):
        ...
        if len(self.samtools_stats) > 0:
            ...
            stats_headers['error_rate'] = {'title': 'Error rate', 
            ...
            stats_headers['non-primary_alignments'] = {'title': ('{} Non-Primary').format(config.read_count_prefix), 
               'description': ('Non-primary alignments ({})').format(config.read_count_desc), 
               'min': 0, 
               'scale': 'PuBu', 
               'modify': lambda x: x * config.read_count_multiplier, 
               'shared_key': 'read_count'}
            stats_headers['reads_mapped'] = {'title': ('{} Reads Mapped').format(config.read_count_prefix), 
               'description': ('Reads Mapped in the bam file ({})').format(config.read_count_desc), 
               'min': 0, 
               'modify': lambda x: x * config.read_count_multiplier, 
               'shared_key': 'read_count'}
            stats_headers['reads_mapped_percent'] = {'title': '% Mapped', 
               'description': '% Mapped Reads', 
               'max': 100, 
               'min': 0, 
               'suffix': '%', 
               'scale': 'RdYlGn'}
            stats_headers['reads_properly_paired_percent'] = {'title': '% Proper Pairs', 
            ...
            stats_headers['reads_MQ0_percent'] = {'title': '% MapQ 0 Reads', 
               ...
               'scale': 'OrRd', 
               'hidden': True}
            stats_headers['raw_total_sequences'] = {'title': ('{} Total seqs').format(config.read_count_prefix), 
            ...
            self.add_section(name='Alignment metrics', anchor='samtools-stats', description='This module parses the output from <code>samtools stats</code>. All numbers in millions.', plot=beeswarm.plot(self.samtools_stats, keys, {'id': 'samtools-stats-dp'}))
...
def alignment_chart(data):
    ...
    plot_conf = {'id': 'samtools_alignment_plot', 
