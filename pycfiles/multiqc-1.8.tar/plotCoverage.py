...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/deeptools/plotCoverage.py
    ...
    def parse_plotCoverage(self):
        ...
        if len(self.deeptools_plotCoverageStdout) > 0:
            ...
            header['max'] = {'title': 'Max', 
               ...
               'shared_key': 'coverage'}
            header['std'] = {'title': 'Std. Dev.', 
               'description': 'Coverage standard deviation', 
               'shared_key': 'coverage'}
            config = {'namespace': 'deepTools plotCoverage'}
            self.add_section(name='Coverage metrics', anchor='deeptools_coverage_metrics', plot=table.plot(self.deeptools_plotCoverageStdout, header, config))
        if len(self.deeptools_plotCoverageOutRawCounts) > 0:
            config = {'id': 'deeptools_coverage_metrics_plot', 'title': 'deepTools: Coverage distribution', 
               'xlab': 'Coverage', 
               'ylab': 'Fraction of bases sampled'}
            self.add_section(name='Coverage distribution', anchor='deeptools_coverage_distribution', description='The fraction of bases with a given number of read/fragment coverage', plot=linegraph.plot(self.deeptools_plotCoverageOutRawCounts, config))
        return (
         len(self.deeptools_plotCoverageStdout), len(self.deeptools_plotCoverageOutRawCounts))
