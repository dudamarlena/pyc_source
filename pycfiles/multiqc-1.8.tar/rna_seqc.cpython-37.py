...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/rna_seqc/rna_seqc.py
# Compiled at: 2019-06-14 06:14:44
# Size of source mod 2**32: 10363 bytes
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='RNA-SeQC', anchor='rna_seqc', href='https://software.broadinstitute.org/cancer/cga/rna-seqc',
        ...
        num_found = max(len(self.rna_seqc_metrics), len(self.rna_seqc_norm_high_cov), len(self.rna_seqc_norm_medium_cov), len(self.rna_seqc_norm_low_cov))
    ...
    def rnaseqc_general_stats(self):
        ...
        """
        headers = OrderedDict()
        headers['Expression Profiling Efficiency'] = {'title':'% Expression Efficiency', 
        ...
        headers['Genes Detected'] = {'title':'# Genes', 
         'description':'Number of genes detected with at least 5 reads.', 
         'min':0, 
         'scale':'Bu', 
         'format':'{:,.0f}'}
        headers['rRNA rate'] = {'title':'% rRNA Alignment', 
    ...
    def transcript_associated_plot(self):
        ...
        pconfig = {'id':'rna_seqc_position_plot', 
        ...
        self.add_section(name='Transcript-associated reads',
          anchor='Transcript_associated',
          helptext='All of the above rates are per mapped read. Exonic Rate is the fraction mapping within exons. Intronic Rate is the fraction mapping within introns. Intergenic Rate is the fraction mapping in the genomic space between genes. ',
    ...
    def strand_barplot(self):
        ...
        pconfig = {'id':'rna_seqc_strandedness_plot', 
         ...
         'ymin':0, 
         'cpswitch_c_active':False}
        self.add_section(name='Strand Specificity',
          anchor='rna_seqc_strand_specificity',
          helptext='End 1/2 Sense are the number of End 1 or 2 reads that were sequenced in the sense direction. Similarly, End 1/2 Antisense are the number of End 1 or 2 reads that were sequenced in the antisense direction',
          plot=(bargraph.plot(self.rna_seqc_metrics, keys, pconfig)))
    ...
    def parse_coverage(self, f):
        ...
        if f['fn'] == 'meanCoverageNorm_high.txt':
            ...
            if f['fn'] == 'meanCoverageNorm_medium.txt':
                self.rna_seqc_norm_medium_cov.update(data)
            else:
                if f['fn'] == 'meanCoverageNorm_low.txt':
    ...
    def coverage_lineplot(self):
        ...
        pconfig = {'id':'rna_seqc_mean_coverage_plot',  'title':'RNA-SeQC: Gene Body Coverage', 
         'ylab':'% Coverage', 
    ...
    def parse_correlation(self, f):
        ...
        if f['fn'] == 'corrMatrixPearson.txt':
            self.rna_seqc_pearson = (
             s_names, data)
        else:
            if f['fn'] == 'corrMatrixSpearman.txt':
                self.rna_seqc_spearman = (
                 s_names, data)
    ...
    def plot_correlation_heatmap(self):
        ...
        if self.rna_seqc_spearman is not None and correlation_type != 'pearson':
            ...
            if self.rna_seqc_pearson is not None:
                ...
                corr_type = 'Pearson'
        if data is not None:
            pconfig = {'id':'rna_seqc_correlation_heatmap',  'title':'RNA-SeQC: {} Sample Correlation'.format(corr_type)}
            self.add_section(name=('{} Correlation'.format(corr_type)),
