...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/picard/HsMetrics.py
...
DESC = {'BAIT_SET': 'The name of the bait set used in the hybrid selection.', 
...
def parse_reports(self):
    ...
    if len(self.picard_HsMetrics_data) > 0:
        ...
        self.general_stats_headers['FOLD_ENRICHMENT'] = {'title': 'Fold Enrichment', 'min': 0, 
           'format': '{:,.0f}', 
           'scale': 'Blues'}
        try:
        ...
        for c in covs:
            self.general_stats_headers[('PCT_TARGET_BASES_{}X').format(c)] = {'id': ('picard_target_bases_{}X').format(c), 
        ...
        self.add_section(name='HSMetrics', anchor='picard_hsmetrics', plot=table.plot(data_table, _get_headers(data_table)))
        tbases = _add_target_bases(data)
        self.add_section(name=tbases['name'], anchor=tbases['anchor'], description=tbases['description'], plot=tbases['plot'])
        hs_pen = _add_hs_penalty(data)
        if hs_pen is not None:
            self.add_section(name=hs_pen['name'], anchor=hs_pen['anchor'], description=hs_pen['description'], plot=hs_pen['plot'])
...
def _get_headers(data):
    ...
    for s in data:
        for h in data[s]:
            ...
            if h not in header:
                this = {'title': h.replace('_', ' ').lower().capitalize(), 'description': DESC[h] if h in DESC else None, 
                   'scale': 'RdYlGn', 
                   'min': 0, 
                   'namespace': 'HsMetrics'}
                if h.find('READS') > -1:
                    ...
                    this.update({'shared_key': 'base_count', 'modify': lambda x: x * config.base_count_multiplier})
                    this['title'] = ('{} {}').format(config.read_count_prefix, this['title'])
                if h in ('BAIT_TERRITORY', 'TOTAL_READS', 'TARGET_TERRITORY', 'AT_DROPOUT',
                         'GC_DROPOUT'):
...
def _add_target_bases(data):
    ...
    pconfig = {'id': 'picard_percentage_target_bases', 'title': 'Picard: Percentage of target bases', 
       'xlab': 'Fold Coverage', 
       'ylab': 'Pct of bases', 
       'ymax': 100, 
       'ymin': 0, 
