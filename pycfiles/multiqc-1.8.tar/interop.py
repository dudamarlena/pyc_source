...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/interop/interop.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Illumina InterOp Statistics', anchor='interop', href='http://illumina.github.io/interop/index.html', info=' - a set of common routines used for reading and writing InterOp metric files.')
        log = logging.getLogger(__name__)
        self.runSummary = {}
        self.indexSummary = {}
        ...
        if len(self.runSummary) > 0:
            self.add_section(name='Read Metrics Summary', anchor='interop-runmetrics-summary', description='Summary statistics for Total read count from each run.', plot=self.run_metrics_summary_table(self.runSummary))
            self.add_section(name='Read Metrics per Lane', anchor='interop-runmetrics-details', plot=self.run_metrics_details_table(self.runSummary))
        if len(self.indexSummary) > 0:
            self.add_section(name='Indexing QC Metrics summary', anchor='interop-indexmetrics-summary', description='Summary metrics about each lane', plot=self.index_metrics_summary_table(self.indexSummary))
            self.add_section(name='Indexing QC Metrics details', anchor='interop-indexmetrics-details', description=' Detail Metrics about each lane', plot=self.index_metrics_details_table(self.indexSummary))
    ...
    def run_metrics_summary_table(self, data):
        headers = OrderedDict()
        headers['Yield'] = {'rid': 'summary_Yield', 
        ...
        headers['Aligned'] = {'rid': 'summary_Aligned', 
           'title': 'Aligned (%)', 
           ...
           'suffix': '%', 
           'scale': 'PiYG'}
        headers['Error Rate'] = {'title': 'Error Rate (%)', 
           'description': '', 
           'min': 0, 
           'max': 100, 
           'suffix': '%', 
           'scale': 'OrRd'}
        headers['Intensity C1'] = {'rid': 'summary_Intensity_C1', 
           'title': 'Intensity Cycle 1', 
           'description': 'The intensity statistic at cycle 1.'}
        headers['%>=Q30'] = {'rid': 'summary_Q30', 
           'title': '% >= Q30', 
           ...
           'suffix': '%', 
           'scale': 'RdYlGn'}
        table_config = {'namespace': 'interop', 
    ...
    def run_metrics_details_table(self, data):
        ...
        headers['Phased'] = {'title': 'Phased (%)', 
           ...
           'suffix': '%', 
           'scale': 'OrRd'}
        headers['Prephased'] = {'title': 'Prephased (%)', 
           'description': 'The value used by RTA for the percentage of molecules in a cluster for which sequencing falls behind (phasing) or jumps ahead (prephasing) the current cycle within a read.', 
           'format': '{:.,2f}', 
           'min': 0, 
           'max': 100, 
        ...
        headers['Error (35)'] = {'title': 'Error Rate 35 Cycles (%)', 
           ...
           'scale': 'OrRd', 
           'hidden': True}
        headers['Error (75)'] = {'title': 'Error Rate 75 Cycles (%)', 
           ...
           'scale': 'OrRd', 
           'hidden': True}
        headers['Error (100)'] = {'title': 'Error Rate 100 Cycles (%)', 
           ...
           'scale': 'OrRd', 
           'hidden': True}
        headers['Intensity C1'] = {'title': 'Intensity Cycle 1', 
           'description': 'The intensity statistic at cycle 1.'}
        headers['%>=Q30'] = {'title': '%>=Q30', 
