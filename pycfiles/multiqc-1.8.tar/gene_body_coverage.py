...
def parse_reports(self):
    ...
    if len(self.gene_body_cov_hist_counts) > 0:
        ...
        pconfig = {'id': 'rseqc_gene_body_coverage_plot', 
           ...
           'data_labels': [{'name': 'Percentages', 'ylab': 'Percentage Coverage'}, {'name': 'Counts', 'ylab': 'Coverage'}]}
        self.add_section(name='Gene Body Coverage', anchor='rseqc-gene_body_coverage', description='<a href="http://rseqc.sourceforge.net/#genebody-coverage-py" target="_blank">Gene Body Coverage</a> calculates read coverage over gene bodies. This is used to check if reads coverage is uniform and if there is any 5\' or 3\' bias.', plot=linegraph.plot([self.gene_body_cov_hist_percent, self.gene_body_cov_hist_counts], pconfig))
