...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='mosdepth', anchor='mosdepth', href='https://github.com/brentp/mosdepth', info='performs fast BAM/CRAM depth calculation for WGS, exome, or targeted sequencing')
        ...
        if dist_data:
            self.add_section(name='Coverage distribution', anchor='mosdepth-coverage-dist', description='Distribution of the number of locations in the reference genome with a given depth of coverage', helptext=genome_fraction_helptext, plot=linegraph.plot(dist_data, {'id': 'mosdepth-coverage-dist-id', 
               'xlab': 'Coverage (X)', 
               ...
               'smooth_points': 500}))
        if cov_data:
            self.add_section(name='Coverage plot', anchor='mosdepth-coverage-cov', description='Number of locations in the reference genome with a given depth of coverage', helptext=coverage_histogram_helptext, plot=linegraph.plot(cov_data, {'id': 'mosdepth-coverage-plot-id', 
               'xlab': 'Coverage (X)', 
        ...
        if perchrom_avg_data:
            self.add_section(name='Average coverage per contig', anchor='mosdepth-coverage-per-contig-id', description='Average coverage per contig or chromosome', plot=linegraph.plot(perchrom_avg_data, {'id': 'mosdepth-coverage-per-contig', 
               'xlab': 'region', 
