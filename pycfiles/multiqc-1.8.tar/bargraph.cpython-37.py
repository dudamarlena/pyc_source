...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/plots/bargraph.py
...
import inspect, io, logging, math, os, random, re, sys
...
def plot(data, cats=None, pconfig=None):
    ...
    if pconfig is None:
        ...
        if config.lint:
            ...
            if not re.match('^[^:]*\\S: \\S[^:]*$', pconfig.get('title', '')):
        ...
        try:
            ...
            if config.plots_force_flat or (config.plots_force_interactive or len(plotsamples[0])) > config.plots_flat_numseries:
                try:
...
def highcharts_bargraph(plotdata, plotsamples=None, pconfig=None):
    ...
    report.plot_data[pconfig['id']] = {'plot_type':'bar_graph', 
     'samples':plotsamples, 
     'datasets':plotdata, 
     'config':pconfig}
    return html
    ...
    if pconfig is None:
        ...
        if pconfig.get('cpswitch') is not False and not config.simple_output:
            ...
            if len(plotdata) > 1:
                html += ' &nbsp; &nbsp; '
            if len(plotdata) > 1:
                config.simple_output or html += '<div class="btn-group mpl_switch_group mqc_mplplot_bargraph_switchds">\n'
                for k, p in enumerate(plotdata):
                    pid = pids[k]
                    active = 'active' if k == 0 else ''
                    try:
    ...
    for pidx, pdata in enumerate(plotdata):
        ...
        for plot_pct in plot_pcts:
            ...
            if plot_pct is True:
                ...
                if pconfig.get('cpswitch_c_active', True) is True:
                    hide_plot = True
            elif pconfig.get('cpswitch_c_active', True) is not True:
                hide_plot = True
            plt_height = len(plotsamples[pidx]) / 2.3
            plt_height = max(6, plt_height)
            plt_height = min(30, plt_height)
            bar_width = 0.8
            ...
            axes.set_yticks(y_ind)
            axes.set_yticklabels(plotsamples[pidx])
            axes.set_ylim((-0.5, len(y_ind) - 0.5))
            if plot_pct is True:
                axes.set_xlim((0, 100))
            ...
            lgd = axes.legend(dlabels, loc='lower center', bbox_to_anchor=(0, bottom_gap, 1, 0.102), ncol=5, mode='expand', fontsize=8, frameon=False)
            hidediv = ''
            if not pidx > 0:
                if hide_plot:
