...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/quast/quast.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='QUAST', anchor='quast', href='http://quast.bioinf.spbau.ru/', info='is a quality assessment tool for genome assemblies, written by the Center for Algorithmic Biotechnology.')
        ...
        self.total_length_multiplier = qconfig.get('total_length_multiplier', 1e-06)
        ...
        self.add_section(name='Number of Contigs', anchor='quast-contigs', description='This plot shows the number of contigs found for each assembly, broken\n                    down by length.', plot=self.quast_contigs_barplot())
        ng_pdata = self.quast_predicted_genes_barplot()
        if ng_pdata:
            self.add_section(name='Number of Predicted Genes', anchor='quast-genes', description='This plot shows the number of predicted genes found for each\n                          assembly, broken down by length.', plot=ng_pdata)
        ng_pdata = self.quast_predicted_genes_barplot(partial=True)
        if ng_pdata:
            self.add_section(name='Number of Partially Predicted Genes', anchor='quast-partial-genes', description='This plot shows the number of partially predicted genes found for each\n                          assembly, broken down by length.', plot=ng_pdata)
    ...
    def quast_general_stats_table(self):
        ...
        headers = OrderedDict()
        headers['N50'] = {'title': ('N50 ({})').format(self.contig_length_suffix), 
        ...
        headers['Total length'] = {'title': ('Length ({})').format(self.total_length_suffix), 
    ...
    def quast_table(self):
        ...
        headers['N50'] = {'title': ('N50 ({})').format(self.contig_length_suffix), 
           ...
           'scale': 'RdYlGn', 
           'modify': lambda x: x * self.contig_length_multiplier}
        headers['N75'] = {'title': ('N75 ({})').format(self.contig_length_suffix), 
           ...
           'scale': 'RdYlGn', 
           'modify': lambda x: x * self.contig_length_multiplier}
        headers['L50'] = {'title': ('L50 ({})').format(self.total_number_contigs_suffix) if self.total_number_contigs_suffix else 'L50', 
        ...
        headers['L75'] = {'title': ('L75 ({})').format(self.total_number_contigs_suffix) if self.total_number_contigs_suffix else 'L75', 
        ...
        headers['Largest contig'] = {'title': ('Largest contig ({})').format(self.contig_length_suffix), 
    ...
    def quast_predicted_genes_barplot(self, partial=False):
        ...
        for each assembly
        """
        pattern = re.compile('# predicted genes \\(>= (\\d+) bp\\)' + ('_partial' if partial else ''))
        data = {}
        all_categories = []
        ...
        for s_name, d in self.quast_data.items():
            ...
            highest_cat = (highest_threshold, ('>= {} bp').format(highest_threshold))
        ...
        if len(all_categories) > 0:
            return bargraph.plot(data, all_categories, {'id': 'quast_' + ('partially_' if partial else '') + 'predicted_genes', 'title': 'QUAST: Number of ' + ('partially ' if partial else '') + 'predicted genes', 
