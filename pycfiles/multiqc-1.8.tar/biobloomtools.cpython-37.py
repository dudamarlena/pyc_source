...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/biobloomtools/biobloomtools.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='BioBloom Tools', anchor='biobloomtools', href='https://github.com/bcgsc/biobloom/',
          info='creates filters for a genome reference and categorises sequences. This is faster than alignment and can be used for pre-processing and QC applications such as contamination detection.')
        self.bbt_data = dict()
        self.num_orgs = 0
    ...
    def bbt_simple_plot(self):
        ...
        for s_name in self.bbt_data:
            ...
            for org in self.bbt_data[s_name]:
                ...
                if org not in cats and org != 'multiMatch':
                    if org != 'noMatch':
                        if org.lower().endswith('.fa'):
                            cname = org[:-3]
                        else:
                            if org.lower().endswith('.fasta'):
                                cname = org[:-6]
                            else:
                                cname = org
                    cats[org] = {'name': cname}

        pconfig = {'id':'biobloom_tools', 
         'title':'BioBloom Tools: Alignment counts per species', 
         'ylab':'Number of hits', 
         'hide_zero_cats':False}
        cats['multiMatch'] = {'name':'Multiple Genomes', 
         'color':'#820000'}
