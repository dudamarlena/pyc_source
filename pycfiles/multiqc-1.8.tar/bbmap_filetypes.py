...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/bbmap/bbmap_filetypes.py
...
section_order = [
 ...
 'statsfile_machine',
 'statsfile']
file_types = {'stats': {'title': 'BBDuk filtering statistics', 
             'descr': 'Proportion of reads that matched adapters/contaminants.', 
             'help_text': '', 
             'kvrows': [
                      'Total', 'Matched'], 
             'kv_descriptions': {'Total': (
             ...
             'plot_func': None, 
             'plot_params': {}}, 
   'aqhist': {'title': 'Read quality', 
              'descr': 'Histogram of average read qualities (`aqhist`). Plot shows the number of reads at each quality score.', 
              'help_text': '', 
              'cols': odict[
                     'Quality':int,
                     'count1':int,
              ...
              'plot_params': {'xPlotBands': [{'from': 28, 'to': 100, 'color': '#c3e6c3'}, {'from': 20, 'to': 28, 'color': '#e6dcc3'}, {'from': 0, 'to': 20, 'color': '#e6c3c3'}], 'yLog': True}}, 
   'bhist': {'title': 'Base composition', 
             'descr': 'Base composition histogram by position (`bhist`). The plot shows the percentage of `G+C`, `A+T`, and `N` bases for each position in the reads.', 
             'help_text': 'Relative composition', 
             'cols': odict[
                    'Pos':int, 'A':float, 'C':float, 'G':float, 'T':float, 'N':float], 
             'plot_func': plot_bhist, 
             'plot_params': {}}, 
   'bincov': {'title': 'Binned coverage', 
              ...
              'plot_params': {}, 'not_implemented': ''}, 
   'bqhist': {'title': 'Base quality', 
              'descr': 'Quality histogram designed for box plots (`bqhist`). Refer to original source files for complete boxplot data. Plot shows mean base quality for each read position. ', 
              'help_text': '', 
              'cols': odict[
                     'BaseNum':int,
              ...
              'plot_params': {}}, 
   'covhist': {'title': 'Coverage histogram', 
               'descr': 'Histogram of number of occurrences of each coverage depth level (`covhist`). Note that lines have been smoothed to 400 points; higher resolution data might be available in the original data source. ', 
               'help_text': '', 
               'cols': odict['Coverage':int, 'numBases':int], 
               'plot_func': plot_covhist, 
               'plot_params': {'yLog': True}}, 
   'covstats': {'title': 'Coverage stats', 
   ...
   'gchist': {'title': 'GC content', 
              ...
              'kv_descriptions': {'Mean': (
                                  ...
                                  'STDev': (
                                          'Standard deviation of average GC content', {})}, 
              'cols': odict['GC':float, 'Count':int], 
              'plot_func': plot_basic_hist, 
              'plot_params': {'xlab': 'Proportion GC', 
                              'ylab': '# Reads'}}, 
   'idhist': {'title': 'Identity histogram', 
              'descr': 'Histogram of read count versus percent base pair identity of aligned reads (`idhist`).', 
              'help_text': '', 
              'kvrows': [
                       'Mean_reads', 'Mean_bases',
                       'Median_reads', 'Median_bases',
                       'Mode_reads', 'Mode_bases',
                       'STDev_reads', 'STDev_bases'], 
              'kv_descriptions': {'Mean_reads': (
                                               'Average percent identity of aligned reads', {}), 
                                  'Mean_bases': (
                                               'Average percent identity of aligned bases', {}), 
                                  'Median_reads': (
                                                 'Median percent identity of aligned reads', {}), 
                                  'Median_bases': (
                                                 'Median percent identity of aligned bases', {}), 
                                  'Mode_reads': (
              ...
              'plot_params': {}}, 
   'ihist': {'title': 'Insert sizes', 
             'descr': 'Histogram of computed insert sizes, for paired reads (`ihist`). Plotted data has been cut off at 99% to prevent long tails; Complete data available in original source files.', 
             'help_text': 'The insert size is the length of the sequence between the sequencing adapters, which for most common insert sizes is longer than the sum of both read pairs. In some cases, the insert size is shorter than the length of the two read pairs combined, resulting in an insert size shorter than the sum of the length of the reads pairs.', 
             'kvrows': [
                      'Mean', 'Median', 'STDev', 'PercentOfPairs'], 
             'kv_descriptions': {'Mean': (
                                        'Average insert length', {}), 
                                 'Median': (
                                          'Median insert length', {}), 
                                 'STDev': (
                                         'Standard deviation of insert size length distribution', {}), 
                                 'PercentOfPairs': (
                                                  '', {})}, 
             'cols': odict['InsertSize':int, 'Count':int], 
             'plot_func': plot_ihist, 
             'plot_params': {}}, 
   'indelhist': {'title': 'Indel lengths', 
                 'descr': 'Indel length histogram (`indelhist`). The plots show the number of observed insertions and deletions, for each insertion and deletion length.', 
                 'help_text': '', 
                 'cols': odict['Length':int, 'Deletions':int, 'Insertions':int], 
                 'plot_func': plot_indelhist, 
                 'plot_params': {}}, 
   'lhist': {'title': 'Read lengths', 
             'descr': 'Read length histogram (`lhist`).', 
             'help_text': '', 
             'cols': odict['Length':int, 'Count':int], 
             'plot_func': plot_basic_hist, 
             'plot_params': {'xlab': 'Read length (base pairs)', 
                             'ylab': '# Reads'}}, 
   'mhist': {'title': 'Match, substitution, deletion, and insertion rates', 
             ...
             'plot_params': {}}, 
   'qahist': {'title': 'Quality accuracy', 
              'descr': 'Base quality accuracy histogram of error rates versus quality score (`qahist`). The plots show the observed count of each type of alignment by base quality score.', 
              'help_text': '', 
              'kvrows': [
                       'Deviation', 'DeviationSub'], 
              'kv_descriptions': {'Deviation': (
                                              '', {}), 
                                  'DeviationSub': (
                                                 '', {})}, 
              'cols': odict[
              ...
              'plot_params': {}}, 
   'qhist': {'title': 'Sequence Quality Histograms', 
             'descr': 'Quality histogram by position (`qhist`). The plots show the average quality for each position in the reads, using the linear values, logarithmically scaled values, and the actual measured qualities based on the alignments.', 
             'help_text': '', 
             'cols': odict[
                    'BaseNum':int,
                    'Read1_linear':float,
             ...
             'plot_params': {}}, 
   'rpkm': {'title': 'RPKM/FPKM', 
            'descr': 'Per-scaffold RPKM/FPKM counts (`rpkm`).', 
            'help_text': '', 
            'kvrows': [
                     'File', 'Reads', 'Mapped', 'RefSequences'], 
            'kv_descriptions': {'File': (
                                       '', {}), 
                                'Reads': (
                                        '', {}), 
                                'Mapped': (
            ...
            'plot_func': plot_basic_hist, 
            'plot_params': {}, 'not_implemented': ''}, 
   'statsfile_machine': {'title': 'General stats', 
                         'descr': 'General Stats (`statsfile_machine`).', 
                         'help_text': '', 
                         'cols': [], 'plot_func': plot_basic_hist, 
                         'plot_params': {}}, 
   'statsfile': {'title': 'General stats', 
                 'descr': 'General Stats (`statsfile`).', 
                 'help_text': '', 
                 'cols': [], 'plot_params': {}, 'plot_func': plot_basic_hist, 
                 'not_implemented': ''}}
