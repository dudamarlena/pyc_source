...
# Compiled at: 2018-06-29 03:25:16
        ...
        for l in f['f']:
            if s_name is not None and in_hist is True:
                ...
                if s_name is not None:
                    if 'CollectWgsMetrics$WgsMetrics' in l:
    ...
    if len(self.picard_wgsmetrics_data) > 0:
        ...
        if len(self.picard_wgsmetrics_histogram) > 0:
            ...
            pconfig = {'id':'picard_wgs_metrics_histogram', 
             ...
             'ymax':100, 
             'data_labels':[
              {'name':'Percentage Drop-Off', 
               'ylab':'Percentage of Bases',  'ymax':100},
              {'name':'Counts Histogram', 
               'ylab':'Coverage',  'ymax':maxval}]}
            self.add_section(name='WGS Coverage',
              anchor='picard-wgsmetrics-cov',
              description='The number of bases in the genome territory for each fold coverage. Note that final 1% of data is hidden to prevent very long tails.',
            ...
            pconfig = {'id':'picard_wgs_metrics_bases', 
             ...
             'ylab':'% Bases', 
             'ymax':100}
            self.add_section(name='WGS Filtered Bases',
              anchor='picard-wgsmetrics-bases',
              description='For more information about the filtered categories, see the <a href="http://broadinstitute.github.io/picard/picard-metric-definitions.html#CollectWgsMetrics.WgsMetrics" target="_blank">Picard documentation</a>.',
