...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='BBTools', anchor='bbmap', href='http://jgi.doe.gov/data-and-tools/bbtools/', info='is a suite of fast multithreaded bioinformatics tools\n            designed for the analysis of DNA and RNA sequence data.')
        ...
        for file_type in section_order:
            if len(self.mod_data[file_type]) > 0:
                ...
                if file_types[file_type]['plot_func']:
                    self.add_section(name=file_types[file_type]['title'], anchor='bbmap-' + file_type, description=file_types[file_type]['descr'], helptext=file_types[file_type]['help_text'], plot=self.plot(file_type))
            if any(self.mod_data[file_type][sample]['kv'] for sample in self.mod_data[file_type]):
                self.add_section(name=file_types[file_type]['title'] + ' summary table', anchor='bbmap-' + file_type + '-table', description=file_types[file_type]['descr'], helptext=file_types[file_type]['help_text'], plot=self.make_basic_table(file_type))
