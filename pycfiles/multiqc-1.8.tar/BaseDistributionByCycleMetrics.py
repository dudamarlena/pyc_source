...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/picard/BaseDistributionByCycleMetrics.py
# Compiled at: 2018-02-16 12:07:12
    ...
    try:
        while True:
            ...
            if 'BaseDistributionByCycle' in new_line and 'INPUT' in new_line:
                fn_search = re.search('INPUT=?\\s*(\\[?[^\\s]+\\]?)', new_line, flags=re.IGNORECASE)
                if fn_search:
...
def parse_reports(self):
    ...
    if len(self.picard_baseDistributionByCycle_data) > 0:
        ...
        pconfig = {'id': 'picard_base_distribution_by_cycle', 
           ...
           'ymax': 100, 
           'ymin': 0, 
           'data_labels': [{'name': '% Adenine', 'ylab': '% Adenine'}, {'name': '% Cytosine', 'ylab': '% Cytosine'}, {'name': '% Guanine', 'ylab': '% Guanine'}, {'name': '% Thymine', 'ylab': '% Thymine'}, {'name': '% Undetermined', 'ylab': '% Undetermined'}]}
        linegraph_data = [{}, {}, {}, {}, {}]
        ...
        self.add_section(name='Base Distribution', anchor='picard-base-distribution-by-cycle', description='Plot shows the distribution of bases by cycle.', plot=linegraph.plot(linegraph_data, pconfig))
