...
@click.option('-dd', '--dirs-depth', 'dirs_depth', type=int, help='Prepend [INT] directories to sample names. Negative number to take from start of path.')
@click.option('-s', '--fullnames', 'no_clean_sname', is_flag=True, help='Do not clean the sample names (leave as full file name)')
@click.option('-i', '--title', type=str, help='Report title. Printed as page header, used for filename if not otherwise specified.')
@click.option('-b', '--comment', 'report_comment', type=str, help='Custom comment, will be printed at the top of the report.')
...
@click.option('--tag', 'module_tag', type=str, multiple=True, help='Use only modules which tagged with this keyword, eg. RNA')
@click.option('--view-tags', '--view_tags', is_flag=True, callback=util_functions.view_all_tags, expose_value=False, is_eager=True, help='View the available tags and which modules they load')
@click.option('-x', '--ignore', type=str, multiple=True, help='Ignore analysis files (glob expression)')
@click.option('--ignore-samples', 'ignore_samples', type=str, multiple=True, help='Ignore sample names (glob expression)')
@click.option('--ignore-symlinks', 'ignore_symlinks', is_flag=True, help='Ignore symlinked directories and files')
@click.option('--sample-names', 'sample_names', type=click.Path(exists=True, readable=True), help='File containing alternative sample names')
@click.option('-l', '--file-list', is_flag=True, help='Supply a file containing a list of file paths to be searched, one per row')
@click.option('-e', '--exclude', metavar='[module name]', type=click.Choice(sorted(['general_stats'] + list(config.avail_modules.keys()))), multiple=True, help='Do not use this module. Can specify multiple times.')
@click.option('-m', '--module', metavar='[module name]', type=click.Choice(sorted(config.avail_modules.keys())), multiple=True, help='Use only this module. Can specify multiple times.')
@click.option('--data-dir', 'make_data_dir', is_flag=True, help='Force the parsed data directory to be created.')
@click.option('--no-data-dir', 'no_data_dir', is_flag=True, help='Prevent the parsed data directory from being created.')
@click.option('-k', '--data-format', 'data_format', type=click.Choice(config.data_format_extensions.keys()), help=('Output parsed data in a different format. Default: {}').format(config.data_format))
@click.option('-z', '--zip-data-dir', 'zip_data_dir', is_flag=True, help='Compress the data directory.')
@click.option('-p', '--export', 'export_plots', is_flag=True, help='Export plots as static images in addition to the report')
@click.option('-fp', '--flat', 'plots_flat', is_flag=True, help='Use only flat plots (static images)')
@click.option('-ip', '--interactive', 'plots_interactive', is_flag=True, help='Use only interactive plots (HighCharts Javascript)')
@click.option('--lint', 'lint', is_flag=True, help='Use strict linting (validation) to help code development')
@click.option('--pdf', 'make_pdf', is_flag=True, help="Creates PDF report with 'simple' template. Requires Pandoc to be installed.")
@click.option('--no-megaqc-upload', 'no_megaqc_upload', is_flag=True, help="Don't upload generated report to MegaQC, even if MegaQC options are found")
@click.option('-c', '--config', 'config_file', type=click.Path(exists=True, readable=True), multiple=True, help='Specific config file to load, after those in MultiQC dir / home dir / working dir.')
@click.option('--cl-config', '--cl_config', type=str, multiple=True, help='Specify MultiQC config YAML on the command line')
...
def run_cli(analysis_dir, dirs, dirs_depth, no_clean_sname, title, report_comment, template, module_tag, module, exclude, outdir, ignore, ignore_samples, sample_names, file_list, filename, make_data_dir, no_data_dir, data_format, zip_data_dir, force, ignore_symlinks, export_plots, plots_flat, plots_interactive, lint, make_pdf, no_megaqc_upload, config_file, cl_config, verbose, quiet, no_ansi, **kwargs):
    ...
    """
    sys_exit_code = run(analysis_dir=analysis_dir, dirs=dirs, dirs_depth=dirs_depth, no_clean_sname=no_clean_sname, title=title, report_comment=report_comment, template=template, module_tag=module_tag, module=module, exclude=exclude, outdir=outdir, ignore=ignore, ignore_samples=ignore_samples, sample_names=sample_names, file_list=file_list, filename=filename, make_data_dir=make_data_dir, no_data_dir=no_data_dir, data_format=data_format, zip_data_dir=zip_data_dir, force=force, ignore_symlinks=ignore_symlinks, export_plots=export_plots, plots_flat=plots_flat, plots_interactive=plots_interactive, lint=lint, make_pdf=make_pdf, no_megaqc_upload=no_megaqc_upload, config_file=config_file, cl_config=cl_config, verbose=verbose, quiet=quiet, no_ansi=no_ansi, kwargs=kwargs)
    sys.exit(sys_exit_code)


def run(analysis_dir, dirs=False, dirs_depth=None, no_clean_sname=False, title=None, report_comment=None, template=None, module_tag=(), module=(), exclude=(), outdir=None, ignore=(), ignore_samples=(), sample_names=None, file_list=False, filename=None, make_data_dir=False, no_data_dir=False, data_format=None, zip_data_dir=False, force=True, ignore_symlinks=False, export_plots=False, plots_flat=False, plots_interactive=False, lint=False, make_pdf=False, no_megaqc_upload=False, config_file=(), cl_config=(), verbose=0, quiet=False, no_ansi=False, kwargs={}):
