...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/homer/findpeaks.py
...
class FindPeaksReportMixin:
    ...
    def parse_homer_findpeaks(self):
        ...
        if len(self.homer_findpeaks) > 0:
            ...
            stats_headers['approximate_ip_efficiency'] = {'title':'% Efficiency', 
             'description':'Approximate IP efficiency', 
             'min':0, 
             'max':100, 
             'suffix':'%', 
             'scale':'RdYlGn'}
            stats_headers['total_peaks'] = {'title':'Total Peaks', 
             'min':0, 
             'format':'{:,.0f}', 
             'scale':'GnBu'}
            stats_headers['expected_tags_per_peak'] = {'title':'Tags/Peak', 
