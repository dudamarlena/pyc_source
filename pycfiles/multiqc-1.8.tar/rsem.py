...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Rsem', anchor='rsem', href='https://deweylab.github.io/RSEM/', info='RSEM (RNA-Seq by Expectation-Maximization) is a software package forestimating gene and isoform expression levels from RNA-Seq data.')
    ...
    def rsem_mapped_reads_plot(self):
        ...
        self.add_section(name='Mapped Reads', anchor='rsem_mapped_reads', description='A breakdown of how all reads were aligned for each sample.', plot=bargraph.plot(self.rsem_mapped_data, keys, config))
    ...
    def rsem_multimapping_plot(self):
        ...
        self.add_section(name='Multimapping rates', anchor='rsem_multimapping', description='A frequency histogram showing how many reads were aligned to `n` reference regions.', helptext='In an ideal world, every sequence reads would align uniquely to a single location in the\n                reference. However, due to factors such as repeititve sequences, short reads and sequencing errors,\n                reads can be align to the reference 0, 1 or more times. This plot shows the frequency of each factor\n                of multimapping. Good samples should have the majority of reads aligning once.', plot=linegraph.plot(self.rsem_multimapping_data, pconfig))