...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/macs2/macs2.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='MACS2', anchor='macs', href='https://github.com/taoliu/MACS', info='identifies transcription factor binding sites in ChIP-seq data.')
        self.macs_data = dict()
    ...
    def parse_macs(self, f):
        regexes = {'name': '# name = (.+)$', 
           'fragment_size': '# (?:fragment|tag) size is determined as (\\d+) bps', 
           'treatment_fragments_total': '# total (?:fragments|tags) in treatment: (\\d+)', 
           'treatment_fragments_after_filtering': '# (?:fragments|tags) after filtering in treatment: (\\d+)', 
           'treatment_max_duplicates': '# maximum duplicate (?:fragments|tags at the same position) in treatment = (\\d+)', 
           'treatment_redundant_rate': '# Redundant rate in treatment: ([\\d\\.]+)', 
           'control_fragments_total': '# total (?:fragments|tags) in control: (\\d+)', 
           'control_fragments_after_filtering': '# (?:fragments|tags) after filtering in control: (\\d+)', 
           'control_max_duplicates': '# maximum duplicate (?:fragments|tags at the same position) in control = (\\d+)', 
           'control_redundant_rate': '# Redundant rate in control: ([\\d\\.]+)', 
           'd': '# d = (\\d+)'}
        s_name = f['s_name']
        parsed_data = dict()
        for l in f['f']:
    ...
    def macs_general_stats(self):
        ...
        headers = OrderedDict()
        headers['d'] = {'title': 'Fragment Length', 
           'min': 0, 
           'format': '{:,.0f}'}
        headers['treatment_redundant_rate'] = {'title': 'Treatment Redundancy', 
           'description': 'Redundant rate in treatment', 
           'max': 1, 
           'min': 0, 
           'format': '{:,.2f}', 
           'scale': 'RdYlBu-rev'}
        headers['control_redundant_rate'] = {'title': 'Control Redundancy', 
