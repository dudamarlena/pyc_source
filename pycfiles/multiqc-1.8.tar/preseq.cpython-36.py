...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Preseq', anchor='preseq', href='http://smithlabresearch.org/software/preseq/',
        ...
        for f in self.find_log_files('preseq'):
            ...
            if sample_data_raw is None:
                ...
                if data_is_bases is not None:
                    if sample_data_is_bases != data_is_bases:
    ...
    def _make_preseq_length_trimmed_plot(self, data_raw, is_basepairs):
        ...
        pconfig['extra_series'].append({'name':'a perfect library where each read is unique', 
         ...
         'showInLegend':False})
        self.add_section(name='Complexity curve',
...
def _parse_preseq_logs(f):
    ...
    if header.startswith('TOTAL_READS\tEXPECTED_DISTINCT'):
        ...
        if header.startswith('TOTAL_BASES\tEXPECTED_DISTINCT'):
            ...
            if header.startswith('total_reads\tdistinct_reads'):
                pass
            else:
