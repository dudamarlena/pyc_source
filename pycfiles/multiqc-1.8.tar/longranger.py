...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/longranger/longranger.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Long Ranger', anchor='longranger', href='https://www.10xgenomics.com/', info='A set of analysis pipelines that perform sample demultiplexing, barcode processing, alignment, quality control, variant calling, phasing, and structural variant calling.')
        ...
        self.headers['large_sv_calls'] = {'title': 'Large SVs', 
           'description': 'Large structural variants called by Longranger. Not including blacklisted regions.', 
           'format': '{:,.0f}', 
           'scale': 'PuRd'}
        self.headers['short_deletion_calls'] = {'title': 'Short dels', 
           'description': 'Short deletions called by Longranger.', 
           'format': '{:,.0f}', 
           'scale': 'PuRd', 
           'hidden': True}
        self.headers['genes_phased_lt_100kb'] = {'title': 'genes phased < 100kb', 
        ...
        self.headers['on_target_bases'] = {'title': 'On target', 
           ...
           'modify': lambda x: try_float_lambda(x, '*', 100.0), 
           'scale': 'Greens'}
        self.headers['zero_coverage'] = {'title': 'Zero cov', 
           ...
           'min': 0.0, 
           'scale': 'RdGy-rev'}
        self.headers['mean_depth'] = {'title': 'Depth', 
           ...
           'suffix': 'X', 
           'scale': 'PuBu'}
        self.headers['pcr_duplication'] = {'title': 'PCR Dup', 
        ...
        self.headers['mean_dna_per_gem'] = {'title': 'DNA per gem', 
           ...
           'scale': 'OrRd', 
           'hidden': True}
        self.headers['gems_detected'] = {'title': 'M Gems', 
           ...
           'modify': lambda x: try_float_lambda(x, '/', 1000000.0), 
           'scale': 'OrRd'}
        self.headers['corrected_loaded_mass_ng'] = {'title': 'Loaded (corrected)', 
           ...
           'suffix': 'ng', 
           'scale': 'RdYlGn'}
        self.headers['loaded_mass_ng'] = {'title': 'Loaded', 
           ...
           'suffix': 'ng', 
           'scale': 'RdYlGn'}
        self.headers['instrument_ids'] = {'title': 'Instrument ID', 
           'description': 'The list of instrument IDs used to generate the input reads.', 
           'scale': False, 
           'hidden': True}
        self.headers['longranger_version'] = {'title': 'Long Ranger Version', 
        ...
        config_table = {'id': 'longranger_table', 
           'namespace': 'longranger'}
        self.add_section(name='Run stats', anchor='longranger-run-stats', description='Statistics gathered from Longranger reports. There are more columns available but they are hidden by default.' + version_str, helptext='Parses the files `summary.csv` and `_invocation` found in the\n                    output directory of Longranger. If `_invocation` is not found\n                    the sample IDs will be missing and they will be given a running\n                    number. E.g., `longranger#1` and `longranger#2`.', plot=table.plot(self.longranger_data, self.headers, config_table))
        phase_pdata = {}
        snps_phased_pct = {}
        genes_phased_pct = {}
        for s_name in self.longranger_data:
            try:
                phase_pdata[s_name] = {'longest_phase_block': float(self.longranger_data[s_name]['longest_phase_block']), 
        ...
        if len(phase_pdata) > 0:
            self.add_section(name='Phasing', anchor='longranger-phasing', description='Phasing performance from Long Ranger. Genes are only considered if &le; 100kbp in length and with at least one heterozygous SNP.', helptext='\n                        * Longest phased\n                            * Size of the longest phase block, in base pairs\n                        * N50 phased\n                            * N50 length of the called phase blocks, in base pairs.\n                        * % SNPs phased\n                            * Percentage of called SNPs that were phased.\n                        * % Genes Phased\n                            * Percentage of genes shorter than 100kb with >1 heterozygous SNP that are phased into a single phase block.\n                        ', plot=bargraph.plot([
             phase_pdata, snps_phased_pct, genes_phased_pct], phase_plot_cats, {'id': 'longranger-phasing-plot', 
               'title': 'Long Ranger: Phasing Statistics', 
               'data_labels': [{'name': 'N50 Phased', 'ylab': 'N50 of called phase blocks (bp)'}, {'name': '% SNPs Phased', 'ylab': '% SNPs Phased', 'ymax': 100}, {'name': '% Genes Phased', 'ylab': '% Genes Phased', 'ymax': 100}], 'cpswitch': False, 
               'stacking': None, 
        ...
        for s_name in self.longranger_data:
            ...
            mapping_counts_data[s_name] = {'unique_reads': unique_reads, 
               'dup_reads': dup_reads, 
               'unmapped_reads': unmapped_reads}

        mapping_counts_cats = OrderedDict()
        mapping_counts_cats['unique_reads'] = {'name': 'Uniquely Aligned Reads', 
           'color': '#437bb1'}
        mapping_counts_cats['dup_reads'] = {'name': 'PCR Duplicate Aligned Reads', 
           'color': '#7cb5ec'}
        mapping_counts_cats['unmapped_reads'] = {'name': 'Unaligned Reads', 
           'color': '#7f0000'}
        self.add_section(name='Alignment', anchor='longranger-alignment', description='Long Ranger alignment against the reference genome. To be marked as PCR duplicates, reads must have the same mapping extents on the genome and the same 10x barcode.', plot=bargraph.plot(mapping_counts_data, mapping_counts_cats, {'id': 'longranger-alignment-plot', 
