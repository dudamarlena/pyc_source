...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/picard/ValidateSamFile.py
...
WARNING_DESCRIPTIONS = {'ADJACENT_INDEL_IN_CIGAR': 'CIGAR string contains an insertion (I) followed by deletion (D), or vice versa', 
...
ERROR_DESCRIPTIONS = {'CIGAR_MAPS_OFF_REFERENCE': 'Bases corresponding to M operator in CIGAR extend beyond reference', 
...
def _add_section_to_report(self, data):
    ...
    plot_html = []
    note_html = _generate_overview_note(pass_count=pass_count, only_warning_count=only_warning_count, error_count=error_count, total_count=len(data))
    ...
    self.add_section(name='SAM/BAM File Validation', anchor='picard_validatesamfile', description='This tool reports on the validity of a SAM or BAM file relative to the SAM-format specification.', helptext='\n            A detailed table is only shown if errors or warnings are found. Details about the errors and warnings are only shown if a `SUMMARY` report was parsed.\n\n            For more information on the warnings, errors and possible fixes please read [this broadinstitute article](https://software.broadinstitute.org/gatk/documentation/article.php?id=7571).', plot=('\n').join(plot_html))
...
def _get_general_stats_headers():
    ...
    """
    headers = OrderedDict()
    headers['file_validation_status'] = {'title': 'Validation', 
       'description': 'ValidateSamFile (File Validation)'}
    headers['WARNING_count'] = {'title': '# Warnings', 
       ...
       'format': '{:.0f}', 
       'hidden': True}
    headers['ERROR_count'] = {'title': '# Errors', 
...
def _generate_overview_note(pass_count, only_warning_count, error_count, total_count):
    ...
    for b in pbars:
        if b[0]:
            note_html.append(('<div class="progress-bar progress-bar-{pbcol}" style="width: {pct}%" data-toggle="tooltip" title="{count} {sample} {txt}">{count}</div>').format(pbcol=b[1], count=int(b[0]), pct=b[0] / float(total_count) * 100.0, txt=b[2], sample='samples' if b[0] > 1 else 'sample'))
    ...
    for problems in data.values():
        for problem in problems:
            if problem not in headers and problem in WARNING_DESCRIPTIONS:
                ...
                headers[problem] = {'description': WARNING_DESCRIPTIONS[problem], 
                   'namespace': 'WARNING', 
            ...
            if problem not in headers and problem in ERROR_DESCRIPTIONS:
                ...
                headers[problem] = {'description': ERROR_DESCRIPTIONS[problem], 
                   'namespace': 'ERROR', 
