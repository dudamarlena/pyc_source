...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/rseqc/read_distribution.py
# Compiled at: 2019-11-13 09:32:10
...
def parse_reports(self):
    ...
    self.read_dist = dict()
    first_regexes = {'total_reads': 'Total Reads\\s+(\\d+)\\s*', 
       'total_tags': 'Total Tags\\s+(\\d+)\\s*', 
       'total_assigned_tags': 'Total Assigned Tags\\s+(\\d+)\\s*'}
    second_regexes = {'cds_exons': 'CDS_Exons\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       '5_utr_exons': "5'UTR_Exons\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*", 
       '3_utr_exons': "3'UTR_Exons\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*", 
       'introns': 'Introns\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tss_up_1kb': 'TSS_up_1kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tss_up_5kb': 'TSS_up_5kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tss_up_10kb': 'TSS_up_10kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tes_down_1kb': 'TES_down_1kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tes_down_5kb': 'TES_down_5kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*', 
       'tes_down_10kb': 'TES_down_10kb\\s+(\\d+)\\s+(\\d+)\\s+([\\d\\.]+)\\s*'}
    ...
    if len(self.read_dist) > 0:
        ...
        pconfig = {'id': 'rseqc_read_distribution_plot', 
           'title': 'RSeQC: Read Distribution', 
           'ylab': '# Tags', 
           'cpswitch_counts_label': 'Number of Tags', 
           'cpswitch_c_active': False}
        self.add_section(name='Read Distribution', anchor='rseqc-read_distribution', description='<a href="http://rseqc.sourceforge.net/#read-distribution-py" target="_blank">Read Distribution</a> calculates how mapped reads are distributed over genome features.', plot=bargraph.plot(self.read_dist, keys, pconfig))
