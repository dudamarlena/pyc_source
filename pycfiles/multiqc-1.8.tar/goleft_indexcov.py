...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='goleft indexcov', anchor='goleft_indexcov', href='https://github.com/brentp/goleft/tree/master/indexcov', info='quickly estimates coverage from a whole-genome bam index.')
    ...
    def roc_plot(self):
        ...
        if data:
            ...
            self.add_section(name='Scaled coverage ROC plot', anchor='goleft_indexcov-roc', description='Coverage (ROC) plot that shows genome coverage at at given (scaled) depth.', helptext=helptext, plot=linegraph.plot([ data[c] for c in chroms ], pconfig))
            return True
        else:
            return False
            return
    ...
    def bin_plot(self):
        ...
        if data:
            ...
            self.add_section(name='Problem coverage bins', anchor='goleft_indexcov-bin', description='This plot identifies problematic samples using binned coverage distributions.', helptext=helptext, plot=scatter.plot(data, pconfig))
            return True
        else:
            return False