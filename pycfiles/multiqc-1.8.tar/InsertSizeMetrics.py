...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/picard/InsertSizeMetrics.py
...
def parse_reports(self):
    ...
    if len(self.picard_insertSize_data) > 0:
        ...
        self.general_stats_headers['summed_median'] = {'title': 'Insert Size', 
           'description': 'Median Insert Size, all read orientations (bp)', 
           'min': 0, 
           'suffix': ' bp', 
           'format': '{:,.0f}', 
           'scale': 'GnBu'}
        self.general_stats_headers['summed_mean'] = {'title': 'Mean Insert Size', 
        ...
        if len(self.picard_insertSize_histogram) > 0:
            ...
            pconfig = {'smooth_points': insertsize_smooth_points, 
               ...
               'ymin': 0, 
               'data_labels': [{'name': 'Counts', 'ylab': 'Coverage'}, {'name': 'Percentages', 'ylab': 'Percentage of Counts'}]}
            self.add_section(name='Insert Size', anchor='picard-insertsize', description='Plot shows the number of reads at a given insert size. Reads with different orientations are summed.', plot=linegraph.plot([self.picard_insertSize_histogram, data_percent], pconfig))
