...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/hicpro/hicpro.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='HiC-Pro', anchor='hicpro', href='https://github.com/nservant/HiC-Pro', info='is an efficient and flexible pipeline for Hi-C data processing. The MultiQC module is supported since HiC-Pro v2.11.0.')
        ...
        self.add_section(name='Read Mapping', anchor='hicpro-mapping', description='Alignment of reads in single-end mode.', helptext="\n            HiC-Pro uses a two steps mapping strategy. End-to-end mapping is first\n            performed (*Full reads Alignments*). Unmapped reads are then trimmed\n            at their ligation site, and the 5' end is re-aligned on the reference\n            genome (*Trimmed Reads Alignments*)", plot=self.hicpro_mapping_chart())
        self.add_section(name='Read Pairing', anchor='hicpro-pairing', description='Pairing of single-end mapping.', helptext='\n            HiC-Pro combines aligned reads as pairs. Singleton, low quality pairs or \n            pairs involving multi-mapped reads are usually discarded. Note that the filtering \n            at pairing level can change accrding to the parameters used.', plot=self.hicpro_pairing_chart())
        self.add_section(name='Read Pair Filtering', anchor='hicpro-filtering', description='Filtering of read pairs.', helptext='\n            Aligned read pairs are filtered to select valid 3C products from two \n            different restriction fragments. Read pairs coming from the same fragments, \n            such as self-ligation or unligated (danging-end) fragments, are discarded.\n            Ligation products involving neighboring restriction fragment (religation) are also discarded.\n            Filtered pairs correspond to ligation products discarded based on the range of insert/fragment sizes defined\n            in the analysis. Finally, as the ligation should be a random process, valid read pairs from all \n            orientations (R=Reverse, F=forward) are expected to be observed in the same proportion.', plot=self.hicpro_filtering_chart())
        self.add_section(name='Contact Statistics', anchor='hicpro-rmdup', description='Contacts statistics after duplicates removal.', helptext='\n            Description of contact frequency after duplicates removal. Intra-chromosomal \n            (*cis*) interaction are expected to be more frequent than inter-chromosomal contacts (*trans*)', plot=self.hicpro_contact_chart())
        allele_plot = self.hicpro_as_chart()
        if allele_plot:
            self.add_section(name='Allele-specific analysis', anchor='hicpro-asan', description='Assignment of valid interactions (afer duplicates removal) to parental alleles.', helptext='\n                Description of allele-specific contacts. Valid interactions (0-1 and 1-1, resp. 0-2, 2-2) \n                are used to build the genome1 (resp. genome2) parental maps.', plot=allele_plot)
        capture_plot = self.hicpro_capture_chart()
        if capture_plot:
            self.add_section(name='Capture analysis', anchor='hicpro-cap', description='Selection of interactions overlaping the targeted region(s).', helptext='\n                Description of capture efficiency. Valid interactions with either two (capture-capture) or \n                one (capture-reporter) interactors overlapping with the target(s) are reported.', plot=capture_plot)
    ...
    def hicpro_stats_table(self):
        ...
        headers = OrderedDict()
        headers['percent_duplicates'] = {'title': '% Duplicates', 
           ...
           'scale': 'YlOrRd', 
           'hidden': True}
        headers['valid_interaction_rmdup'] = {'title': ('{} Valid Pairs Unique').format(config.read_count_prefix), 
           'description': ('Number of valid pairs after duplicates removal ({})').format(config.read_count_desc), 
           'min': 0, 
           'scale': 'RdYlBu', 
           'modify': lambda x: x * config.read_count_multiplier, 
           'shared_key': 'read_count'}
        headers['percent_valid'] = {'title': '% Valid Pairs', 
           ...
           'scale': 'YlGn', 
           'hidden': True}
        headers['valid_interaction'] = {'title': ('{} Valid Pairs').format(config.read_count_prefix), 
           ...
           'shared_key': 'read_count', 
           'hidden': True}
        headers['percent_paired_reads'] = {'title': '% Reported', 
        ...
        headers['percent_mapped_R2'] = {'title': '% Aligned [R2]', 
           ...
           'scale': 'YlGn', 
           'hidden': True}
        headers['mapped_R2'] = {'title': 'Aligned [R2]', 
           ...
           'shared_key': 'read_count', 
           'hidden': True}
        headers['percent_mapped_R1'] = {'title': '% Aligned [R1]', 
           ...
           'scale': 'YlGn', 
           'hidden': True}
        headers['mapped_R1'] = {'title': 'Aligned [R1]', 
    ...
    def hicpro_mapping_chart(self):
        ...
        for s_name in self.hicpro_data:
            for r in [1, 2]:
                data[(r - 1)][('{} [R{}]').format(s_name, r)] = {'Full_Alignments_Read': self.hicpro_data[s_name][('global_R{}').format(r)], 'Trimmed_Alignments_Read': self.hicpro_data[s_name][('local_R{}').format(r)], 
                   'Failed_To_Align_Read': int(self.hicpro_data[s_name][('total_R{}').format(r)]) - int(self.hicpro_data[s_name][('mapped_R{}').format(r)])}

        config = {'id': 'hicpro_mapping_stats_plot', 
           'title': 'HiC-Pro: Mapping Statistics', 
           'ylab': '# Reads', 
           'ylab': '# Reads: Read 1', 
           'data_labels': [{'name': 'Read 1', 'ylab': '# Reads: Read 1'}, {'name': 'Read 2', 'ylab': '# Reads: Read 2'}]}
    ...
    def hicpro_pairing_chart(self):
        ...
        config = {'id': 'hicpro_pairing_stats_plot', 
    ...
    def hicpro_filtering_chart(self):
        ...
        config = {'id': 'hicpro_filtering_plot', 
