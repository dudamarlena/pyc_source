...
# [GCC 8.4.0]
# Embedded file name: /Users/philewels/GitHub/MultiQC/multiqc/modules/slamdunk/slamdunk.py
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(name='Slamdunk', anchor='slamdunk', href='http://t-neumann.github.io/slamdunk/', info='is a tool to analyze SLAMSeq data.')
        num_reports = 0
        self.plot_cols = [
    ...
    def slamdunkGeneralStatsTable(self):
        ...
        headers = OrderedDict()
        headers['counted'] = {'title': ('{} Counted').format(config.read_count_prefix), 
        ...
        headers['retained'] = {'title': ('{} Retained').format(config.read_count_prefix), 
           ...
           'scale': 'YlGn', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['mapped'] = {'title': ('{} Mapped').format(config.read_count_prefix), 
           ...
           'scale': 'YlGn', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['sequenced'] = {'title': ('{} Sequenced').format(config.read_count_prefix), 
    ...
    def slamdunkFilterStatsTable(self):
        ...
        headers = OrderedDict()
        headers['mapped'] = {'namespace': 'Slamdunk', 
           ...
           'scale': 'YlGn', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['multimapper'] = {'namespace': 'Slamdunk', 
           ...
           'scale': 'OrRd', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['nmfiltered'] = {'namespace': 'Slamdunk', 
           ...
           'scale': 'OrRd', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['idfiltered'] = {'namespace': 'Slamdunk', 
           ...
           'scale': 'OrRd', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        headers['mqfiltered'] = {'namespace': 'Slamdunk', 
           ...
           'scale': 'OrRd', 
           'modify': lambda x: float(x) * config.read_count_multiplier}
        pconfig = {'id': 'slamdunk_filtering_table', 
           'min': 0}
    ...
    def slamdunkUtrRatesPlot(self):
        ...
        pconfig = {'id': 'slamdunk_utrratesplot', 'title': 'Slamdunk: Overall conversion rates per UTR', 
           'cpswitch': False, 
           'cpswitch_c_active': False, 
    ...
    def slamdunkPCAPlot(self):
        ...
        pconfig = {'id': 'slamdunk_pca', 
           'title': 'Slamdunk: PCA', 
           'xlab': 'PC1', 
           'ylab': 'PC2', 
           'tt_label': 'PC1 {point.x:.2f}: PC2 {point.y:.2f}'}
        self.add_section(name='PCA (T&gt;C based)', anchor='slamdunk_PCA', description='This plot shows the principal components of samples based\n                        on the distribution of reads with T&gt;C conversions within UTRs\n                        (see the <a href="http://t-neumann.github.io/slamdunk/docs.html#summary" target="_blank">slamdunk docs</a>).', plot=scatter.plot(self.PCA_data, pconfig))

    def slamdunkTcPerReadPosPlot(self):
        """ Generate the tc per read pos plots """
        pconfig_nontc = {'id': 'slamdunk_nontcperreadpos_plot', 
           ...
           'data_labels': [{'name': 'Forward reads +', 'ylab': 'Percent mismatches %'}, {'name': 'Reverse reads -', 'ylab': 'Percent mismatches %'}]}
        pconfig_tc = {'id': 'slamdunk_tcperreadpos_plot', 
           ...
           'ymin': 0, 
           'tt_label': '<b>Pos {point.x}</b>: {point.y:.2f} %', 
           'data_labels': [{'name': 'Forward reads +', 'ylab': 'Percent converted %'}, {'name': 'Reverse reads -', 'ylab': 'Percent converted %'}]}
        self.add_section(name='Non T&gt;C mismatches over read positions', anchor='slamdunk_nontcperreadpos', description='This plot shows the distribution of non T&gt;C mismatches across read positions\n                        (see the <a href="http://t-neumann.github.io/slamdunk/docs.html#tcperreadpos" target="_blank">slamdunk docs</a>).', plot=linegraph.plot([self.nontc_per_readpos_plus, self.nontc_per_readpos_minus], pconfig_nontc))
        self.add_section(name='T&gt;C conversions over read positions', anchor='slamdunk_tcperreadpos', description='This plot shows the distribution of T&gt;C conversions across read positions\n                        (see the <a href="http://t-neumann.github.io/slamdunk/docs.html#tcperreadpos" target="_blank">slamdunk docs</a>).', plot=linegraph.plot([self.tc_per_readpos_plus, self.tc_per_readpos_minus], pconfig_tc))

    def slamdunkTcPerUTRPosPlot(self):
        """ Generate the tc per UTR pos plots """
        pconfig_nontc = {'id': 'slamdunk_slamdunk_nontcperutrpos_plot', 
           ...
           'ymin': 0, 
           'tt_label': '<b>Pos {point.x}</b>: {point.y:.2f} %', 
           'data_labels': [{'name': 'UTRs on plus strand', 'ylab': 'Percent mismatches %'}, {'name': 'UTRs on minus strand', 'ylab': 'Percent mismatches %'}]}
        pconfig_tc = {'id': 'slamdunk_slamdunk_tcperutrpos_plot', 
           ...
           'ymin': 0, 
           'tt_label': '<b>Pos {point.x}</b>: {point.y:.2f} %', 
           'data_labels': [{'name': 'UTRs on plus strand', 'ylab': 'Percent converted %'}, {'name': 'UTRs on minus strand', 'ylab': 'Percent converted %'}]}
        self.add_section(name='Non T&gt;C mismatches over UTR positions', anchor='slamdunk_nontcperutrpos', description='This plot shows the distribution of non T&gt;C mismatches across UTR positions for the last 250 bp from the 3\' UTR end\n                        (see the <a href="http://t-neumann.github.io/slamdunk/docs.html#tcperutrpos" target="_blank">slamdunk docs</a>).', plot=linegraph.plot([self.nontc_per_utrpos_plus, self.nontc_per_utrpos_minus], pconfig_nontc))
        self.add_section(name='T&gt;C conversions over UTR positions', anchor='tcperutrpos', description='This plot shows the distribution of T&gt;C conversions across UTR positions for the last 250 bp from the 3\' UTR end\n                        (see the <a href="http://t-neumann.github.io/slamdunk/docs.html#tcperutrpos" target="_blank">slamdunk docs</a>).', plot=linegraph.plot([self.tc_per_utrpos_plus, self.tc_per_utrpos_minus], pconfig_tc))