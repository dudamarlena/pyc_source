...
# [GCC 8.4.0]
# Embedded file name: /home/adi/chevah/buildbot-configuration-builder/chevah/buildbot_configuration_builder/builder.py
# Compiled at: 2016-04-30 15:56:47
...
class RunStepsFactory(BuildFactory, object):
    ...
    def _add_step_source_command(self, step):
        ...
        self.addStep(Git(name='get code for ' + self._project.name, mode=mode, repourl=self._project.repo, branch=branch, shallow=False))
        return
    ...
    def _add_step_slave_command(self, step):
        ...
        self.addStep(UnixCommand(name=name, command=final_command, doStepIf=do_step_if, env=step_environment, description=name, descriptionDone=done_name))
    ...
    def _add_step_sequential_group(self, step):
        ...
        for target in self._project.getGroupMembersBuilderNames(target_group):
            step = Trigger(schedulerNames=[
             target], waitForFinish=True, updateSourceStamp=True, set_properties=set_properties, copy_properties=copy_properties)
            self.addStep(step)
    ...
    def _add_step_parallel_group(self, step):
        ...
        self.addStep(Trigger(schedulerNames=targets, waitForFinish=True, updateSourceStamp=True, set_properties=set_properties, copy_properties=copy_properties, haltOnFailure=True, flunkOnFailure=True))
    ...
    def _add_step_master_command(self, step):
        ...
        self.addStep(MasterShellCommand(name=name, command=step['command'], haltOnFailure=True))
    ...
    def _add_step_directory_upload(self, step):
        ...
        self.addStep(DirectoryUpload(name=done_name, slavesrc=step['source'], masterdest=step['destination'], haltOnFailure=True, doStepIf=do_step_if))
...
class ParallelFactory(BuildFactory, object):
    ...
    def __init__(self, target_builder_names, steps):
        ...
        self.addStep(Trigger(schedulerNames=target_builder_names, waitForFinish=True, updateSourceStamp=True, set_properties={}, copy_properties=copy_properties, haltOnFailure=True, flunkOnFailure=True))
        return
...
class MailNotifier(BuildbotMailNotifier, object):
    ...
    def __init__(self, server, mode, builders, recipients, user_to_email_mapper=None, subject=None):
        kwargs = {}
        kwargs.update(server)
        kwargs.update({'messageFormatter': message_formatter, 
...
class ProjectConfiguration(object):
    ...
    def addProject(self):
        ...
        for group_name, members in self._raw['groups'].items():
            for member_name in members:
                ...
                builder = BuilderConfig(name=builder_name, slavenames=slaves, category=self._name, factory=RunStepsFactory(project=self, steps=steps, environment=self._parent.getStepEnvironment(member_name)))
    ...
    def _addChangeSource(self, project, repo, branches):
        ...
        if not branches:
            return
        self._parent.addChangeSource(ChevahGitPoller(repourl=repo, branches=branches, project=project, category=project, pollinterval=POLL_INTERVAL, workdir='gitpoller_%s_%s' % (project, ('_').join(branches))))
    ...
    def _addTryBuilders(self):
        ...
        for group in self._raw['groups'].keys():
            try:
                target_builder_names = [ self._getEnvironmentBuilderName(name) for name in self._raw['groups'][group]
                                       ]
            except KeyError:
                raise AssertionError('Failed to enable try group. No such group "%s" for project "%s"' % (
                 group, project))

            assert target_builder_names, 'There are no builders in group: %s' % group_builder_name
            group_builder_name = self._getGroupBuilderName(group)
            self._parent.addTryTarget(group_builder_name)
            steps = self._getSteps()
            builder = BuilderConfig(name=group_builder_name, slavenames=self._parent.getTrySlaves(), factory=ParallelFactory(target_builder_names=target_builder_names, steps=steps), category=self._name)
            self._parent.addBuilder(builder)
    ...
    def _addGateKeepers(self):
        ...
        for keeper, keeper_data in self._raw['gatekeepers'].items():
            ...
            if isinstance(scheduler, basestring):
                ...
                self._parent.addScheduler(SingleBranchScheduler(name=builder_name, change_filter=ChangeFilter(project=self._name, branch=scheduler), treeStableTimer=stable_timer, builderNames=[
                 builder_name]))
            else:
            ...
            if send_github_status:
                ...
                gatekeeper_properties.update({'github_repo_owner': parts[0], 
                   'github_repo_name': parts[1]})
            step_environment = self._parent.getStepEnvironment(data['environment'])
            self._parent.addBuilder(BuilderConfig(name=builder_name, slavenames=slaves, factory=RunStepsFactory(project=self, steps=data['steps'], environment=step_environment), category=self._name, properties=gatekeeper_properties))
            self._parent.addNotifications(builder=builder_name, configuration=data.get('notifications', {}))

        self._addChangeSource(project=self._name, repo=self._repo, branches=poll_branches)
...
class ConfigurationBuilder(object):
    ...
    def __init__(self, configuration):
        ...
        self.poll_interval = change_sources_configuration.get('poll_interval', POLL_INTERVAL)
        ...
        for project, project_configuration in self._raw['projects'].items():
            if project == DEFAULT:
                continue
            project = ProjectConfiguration(project, project_configuration, self)
            project.addProject()
    ...
    def _getGithHubStatus(self):
        ...
        if not configuration:
            return []
        result.append(GitHubStatus(token=configuration['token'], repoOwner=Interpolate('%(prop:github_repo_owner)s'), repoName=Interpolate('%(prop:github_repo_name)s')))
        return result
    ...
    def _getTrySchedulers(self):
        ...
        try_scheduler = Try_Userpass(name='try', port=configuration['port'], userpass=configuration['credentials'], builderNames=self._try_targets, properties={})
    ...
    def getStepEnvironment(self, name):
        ...
        run_environment.update({'CI': 'true', 
           'BUILDBOT': 'true', 
    ...
    def addNotifications(self, builder, configuration):
        ...
        modes = {'email_all': 'all', 
           ...
           'email_error': [
                         'failing', 'problem', 'warnings', 'exception'], 
           'email_change': [
                          'change']}
        supported_types = []
        ...
        for configured_type in configuration.keys():
            if configured_type not in supported_types:
                raise AssertionError('Unknown notification %s for %s' % (
        ...
        for email_type, mode in modes.items():
            ...
            if not recipients:
                continue
            self._addMailNotification(mode=mode, recipients=recipients, builder=builder)

    def _addMailNotification(self, mode, recipients, builder):
        """
        Add email notifications.
        """
        assert recipients, 'No recipients for %s on %s.' % (builder, mode)
        subject = self._email.get('subject', None)
        user_to_email_mapper = self._email.get('user_to_email_mapper', None)
        self._buildbot['status'].append(MailNotifier(mode=mode, server=self._email['server'], recipients=recipients, builders=[
         builder], user_to_email_mapper=user_to_email_mapper, subject=subject))
        return
