...
# [GCC 8.4.0]
# Embedded file name: /home/uli/WorkShop/devel/zope/zope.pytest/tags/0.1/src/zope/pytest/setup.py
...
import ZODB.interfaces, transaction
...
def configure(request, module, zcml):
    ...
    return request.cached_setup(setup=setup_function, teardown=teardown_config, scope='session')
...
def setup_config(package, zcml_file):
    ...
    """
    zcml_file = os.path.join(os.path.dirname(package.__file__), zcml_file)
    setHooks()
    context = config.ConfigurationMachine()
    xmlconfig.registerCommonDirectives(context)
    return xmlconfig.file(zcml_file, package=package, context=context, execute=True)
...
def teardown_db(db):
    ...
    base.unregisterUtility(db, ZODB.interfaces.IDatabase, 'main')
    db.close()
