...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ao/social/linkedin_.py
# Compiled at: 2010-04-14 15:39:24
import re
...
class LinkedInClient(OAuthClient):
    ...
    _default_config = {'request_token_url': 'https://api.linkedin.com/uas/oauth/requestToken', 
       'access_token_url': 'https://api.linkedin.com/uas/oauth/accessToken', 
       'authorize_url': 'https://api.linkedin.com/uas/oauth/authorize', 
       'account_verification_url': 'https://api.linkedin.com/v1/people/~', 
       'update_url': 'http://api.linkedin.com/v1/people/~/current-status'}
    _set_status = '<?xml version="1.0" encoding="UTF-8"?><current-status>%(status)s</current-status>'
    ...
    def lookup_user_info(self, access_token, access_secret):
        ...
        return {'id': rx_key.search(resp.content).group(1), 
    ...
    def post(self, text, token='', secret=''):
        ...
        data = self._set_status % {'status': text.encode('utf-8')}
        token = oauth.OAuthToken(token, secret)
        request = oauth.OAuthRequest.from_consumer_and_token(self._consumer, token=token, http_url=url, http_method='PUT')
        request.sign_request(self._signature_method, self._consumer, token)
        urlfetch.fetch(url, payload=data, method=urlfetch.PUT, headers=request.to_header())