...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ao/social/templatetags/social.py
# Compiled at: 2010-04-14 15:38:28
import urllib
...
def apikey(parser, token):
    ...
    if method not in ('facebook', 'twitter', 'linkedin'):
        raise template.TemplateSyntaxError('Unsupported method for `apikey`: %r' % method)
...
class LoginButton(template.Node):
    ...
    def render(self, context):
        ...
        postlogin = "opener.location.href='%s'" % urllib.quote(environ['PATH_INFO'])
        if self.method == 'facebook':
            if self.onlogin is not None:
                return '<fb:login-button onlogin="%s"></fb:login-button>' % self.onlogin
            button = '<fb:login-button onlogin="location.href=\'%s\'"></fb:login-button>'
            postlogin = "location.href=\\'%s\\'" % urllib.quote(environ['PATH_INFO'])
...
class Avatar(template.Node):
    ...
    def gravatar(email, size=120):
        ...
        return 'http://www.gravatar.com/avatar.php?%s' % urllib.urlencode({'gravatar_id': md5((email or '').lower()).hexdigest(), 
           'size': str(size)})
    ...
    def render(self, context):
        ...
        url = user.avatar or self.gravatar(user.email, max(map(int, (self.width, self.height))))
