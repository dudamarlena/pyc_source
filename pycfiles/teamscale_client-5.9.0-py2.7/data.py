...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/teamscale_client/data.py
...
class Finding(object):
    ...
    def __init__(self, finding_type_id, message, assessment=Assessment.YELLOW, start_offset=None, end_offset=None, start_line=None, end_line=None, identifier=None, uniform_path=None, finding_properties=None, finding_id=None):
    ...
    def __eq__(self, other):
        ...
        return (
         self.uniformPath, self.startLine, self.assessment, self.message, self.endLine, self.endOffset,
    ...
    def __lt__(self, other):
        ...
        return (
         self.uniformPath, self.startLine, self.endLine) < (other.uniformPath, other.startLine, other.endLine)
...
class MetricDescription(object):
    ...
    def __init__(self, metric_id, display_name, description, group_id, aggregation=MetricAggregation.SUM, value_type=MetricValueType.NUMERIC, properties=(MetricProperties.SIZE_METRIC,)):
        ...
        self.metricDefinition = {b'name': display_name, 
...
class NonCodeMetricEntry(object):
    ...
    def __init__(self, path, content=b'', count=1, assessment={}, time=0.0):
        self.path = path
        self.content = content
        self.count = count
        self.assessment = assessment
...
class Baseline(object):
    ...
    def __init__(self, name, description, date=None, timestamp=None):
        if timestamp is None and date is None:
            raise Exception(b'Either date or timestamp must be given.')
        self.name = name
        self.description = description
...
class ProjectInfo(object):
    ...
    def __init__(self, project_id, name, description=None, creation_timestamp=None, alias=None, deleting=False, reanalyzing=False):
        self.id = project_id
        self.name = name
        self.description = description
...
class SourceCodeConnectorConfiguration(ConnectorConfiguration):
    ...
    def __init__(self, connector_type, included_file_names, excluded_file_names=b'', repository_identifier=b'repository1', enable_branch_analysis=False, included_branches=b'.*', excluded_branches=b'_anon.*', start_revision=b'', content_exclude=b'', polling_interval=60, prepend_repository_identifier=False, end_revision=b'', text_filter=b'', source_library_connector=False, run_to_exhaustion=False, delta_size=500, path_prefix_transformation=b'', path_transformation=b'', encoding=b'', author_transformation=b'', branch_transformation=b'', preserve_empty_commits=False):
        super(SourceCodeConnectorConfiguration, self).__init__(connector_type)
        self.options = {b'Included file names': included_file_names, 
           b'Excluded file names': excluded_file_names, 
           b'Repository identifier': repository_identifier, 
           b'Enable branch analysis': enable_branch_analysis, 
           b'Included branches': included_branches, 
           b'Excluded branches': excluded_branches, 
           b'Start revision': start_revision, 
           b'Content exclude': content_exclude, 
           b'Polling interval': polling_interval, 
           b'Prepend repository identifier': prepend_repository_identifier, 
           b'End revision': end_revision, 
           b'Text filter': text_filter, 
           b'Source library connector': source_library_connector, 
           b'Run to exhaustion': run_to_exhaustion, 
           b'Delta size': delta_size, 
           b'Path prefix transformation': path_prefix_transformation, 
           b'Path transformation': path_transformation, 
           b'Encoding': encoding, 
           b'Author transformation': author_transformation, 
           b'Branch transformation': branch_transformation, 
           b'Preserve empty commits': preserve_empty_commits}
...
class FileSystemSourceCodeConnectorConfiguration(SourceCodeConnectorConfiguration):
    ...
    def __init__(self, input_directory, *args, **kwargs):
        super(FileSystemSourceCodeConnectorConfiguration, self).__init__(connector_type=ConnectorType.FILE_SYSTEM, *args, **kwargs)
        self.options[b'Input directory'] = input_directory
...
class MultiVersionFileSystemSourceCodeConnectorConfiguration(SourceCodeConnectorConfiguration):
    ...
    def __init__(self, input_directory, *args, **kwargs):
        super(MultiVersionFileSystemSourceCodeConnectorConfiguration, self).__init__(connector_type=ConnectorType.MULTI_VERSION_FILE_SYSTEM, *args, **kwargs)
        self.options[b'Input directory'] = input_directory
...
class GitSourceCodeConnectorConfiguration(SourceCodeConnectorConfiguration):
    ...
    def __init__(self, default_branch_name, account, path_suffix=b'', include_submodules=False, submodule_recursion_depth=10, connector_type=ConnectorType.GIT, *args, **kwargs):
        super(GitSourceCodeConnectorConfiguration, self).__init__(connector_type=connector_type, *args, **kwargs)
        self.options[b'Default branch name'] = default_branch_name
        self.options[b'Account'] = account
        self.options[b'Path suffix'] = path_suffix
        self.options[b'Include Submodules'] = include_submodules
        self.options[b'Submodule recursion depth'] = submodule_recursion_depth
...
class GerritSourceCodeConnectorConfiguration(GitSourceCodeConnectorConfiguration):
    ...
    def __init__(self, project_name, enable_voting=False, enable_detailed_line_comments=True, ignore_yellow_findings_for_votes=False, ignore_yellow_findings_for_comments=False, number_of_ref_batches_for_updates=1, review_label=b'Code-Review', *args, **kwargs):
        super(GerritSourceCodeConnectorConfiguration, self).__init__(connector_type=ConnectorType.GERRIT, *args, **kwargs)
        self.options[b'Project Name'] = project_name
        self.options[b'Enable Voting'] = enable_voting
        self.options[b'Enable Detailed Line Comments'] = enable_detailed_line_comments
        self.options[b'Ignore Yellow Findings For Votes'] = ignore_yellow_findings_for_votes
        self.options[b'Ignore Yellow Findings For Comments'] = ignore_yellow_findings_for_comments
        self.options[b'Review Label'] = review_label
...
class TFSSourceCodeConnectorConfiguration(SourceCodeConnectorConfiguration):
    ...
    def __init__(self, account, branch_path_suffix=b'', path_suffix=b'', branch_lookup_paths=b'', *args, **kwargs):
        super(TFSSourceCodeConnectorConfiguration, self).__init__(connector_type=ConnectorType.TFS, *args, **kwargs)
        self.options[b'Account'] = account
        self.options[b'Branch path suffix'] = branch_path_suffix
        self.options[b'Path suffix'] = path_suffix
        self.options[b'Branch lookup paths'] = branch_lookup_paths
...
class SubversionSourceCodeConnectorConfiguration(SourceCodeConnectorConfiguration):
    ...
    def __init__(self, account, enable_externals=False, externals_includes=b'', externals_excludes=b'', path_suffix=b'', *args, **kwargs):
        super(SubversionSourceCodeConnectorConfiguration, self).__init__(connector_type=ConnectorType.SVN, *args, **kwargs)
        self.options[b'Account'] = account
        self.options[b'Enable Externals'] = enable_externals
        self.options[b'Path suffix'] = path_suffix
        self.options[b'Externals Includes'] = externals_includes
        self.options[b'Externals Excludes'] = externals_excludes
...
class Task(object):
    ...
    def __init__(self, id, subject, author, description, assignee, status, resolution, findings, comments, tags, created, updated, updatedBy):
        self.id = id
    ...
    def from_json(cls, json):
        comments = []
        if json[b'comments']:
            for comment_json in json[b'comments']:
                comments.append(Comment.from_json(comment_json))

        return Task(json[b'id'], json[b'subject'], json[b'author'], json.get(b'description', b''), json.get(b'assignee', b''), json[b'status'], json[b'resolution'], json[b'findings'], comments, json[b'tags'], json[b'created'], json[b'updated'], json[b'updatedBy'])
...
class Comment(object):
    ...
    @classmethod
    def from_json(cls, json):
        return Comment(json[b'author'], json[b'date'], json[b'text'], json[b'changeComment'])