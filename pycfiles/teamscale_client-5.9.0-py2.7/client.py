...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/teamscale_client/client.py
# Compiled at: 2020-05-08 10:47:58
...
class TeamscaleClient:
    ...
    def from_client_config(config, sslverify=True, timeout=30.0, branch=None):
        ...
        Args:
            ...
            branch (str): The branch name for which to upload/retrieve data
        """
        return TeamscaleClient(config.url, config.username, config.access_token, config.project_id, sslverify, timeout, branch)
    ...
    def check_api_version(self):
        ...
        """
        url = self.get_global_service_url(b'service-api-info')
        response = self.get(url)
        json_response = response.json()
        api_version = json_response[b'apiVersion']
        if api_version < 6:
            raise ServiceError(b'Server api version ' + str(api_version) + b' too low and not compatible. This client requires Teamscale 4.1 or newer.')
    ...
    def get(self, url, parameters=None):
        ...
        if not response.ok:
            raise ServiceError((b'ERROR: GET {url}: {r.status_code}:{r.text}').format(url=url, r=response))
        return response
    ...
    def put(self, url, json=None, parameters=None, data=None):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        headers = {b'Accept': b'application/json', b'Content-Type': b'application/json'}
        response = requests.put(url, params=parameters, json=json, data=data, headers=headers, auth=self.auth_header, verify=self.sslverify, timeout=self.timeout)
        if not response.ok:
            raise ServiceError((b'ERROR: PUT {url}: {r.status_code}:{r.text}').format(url=url, r=response))
        return response
    ...
    def add_findings_group(self, name, mapping_pattern):
        ...
        Returns:
            requests.Response: request's response
        """
        url = b'%s/%s' % (self.get_global_service_url(b'external-findings-group'), name)
        return self.put(url, {b'groupName': name, b'mapping': mapping_pattern})
    ...
    def add_finding_descriptions(self, descriptions):
        ...
        Returns:
            requests.Response: request's response
        """
        base_url = self.get_global_service_url(b'external-findings-description')
        response = None
        for finding_description in descriptions:
            some_description = dict()
            some_description[b'typeId'] = finding_description.typeid
            some_description[b'description'] = finding_description.description
            some_description[b'enablement'] = finding_description.enablement
            some_description[b'name'] = finding_description.name
            url = b'%s/%s' % (base_url, finding_description.typeid)
            response = self.put(url, some_description)
            if response.text != b'success':
                return response
    ...
    def update_findings_schema(self):
        ...
        url = self.get_global_service_url(b'update-findings-schema')
        return self.get(url, {b'projects': self.project})
    ...
    def upload_findings(self, findings, timestamp, message, partition):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        return self._upload_external_data(b'add-external-findings', findings, timestamp, message, partition)
    ...
    def upload_metrics(self, metrics, timestamp, message, partition):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        return self._upload_external_data(b'add-external-metrics', metrics, timestamp, message, partition)
    ...
    def _upload_external_data(self, service_name, json_data, timestamp, message, partition):
        ...
        parameters = {b't': self._get_timestamp_parameter(timestamp), 
           b'message': message, 
           b'partition': partition, 
           b'skip-session': b'true', 
           b'adjusttimestamp': b'true'}
    ...
    def add_metric_descriptions(self, metric_descriptions):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_global_service_url(b'external-metric')
    ...
    def upload_report(self, report_files, report_format, timestamp, message, partition, move_to_last_commit=True):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'external-report')
        parameters = {b't': self._get_timestamp_parameter(timestamp), 
           b'message': message, 
           b'partition': partition, 
           b'format': report_format, 
           b'adjusttimestamp': b'true', 
           b'movetolastcommit': str(move_to_last_commit).lower()}
        multiple_files = [ (b'report', open(filename, b'rb')) for filename in report_files ]
        ...
        if not response.ok:
            raise ServiceError((b'ERROR: POST {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
        return response
    ...
    def upload_architectures(self, architectures, timestamp, message):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'architecture-upload')
        parameters = {b't': self._get_timestamp_parameter(timestamp), 
           b'adjusttimestamp': b'true', 
           b'message': message}
        architecture_files = [ (path, open(filename, b'rb')) for path, filename in architectures.items() ]
    ...
    def upload_non_code_metrics(self, metrics, timestamp, message, partition):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        return self._upload_external_data(b'add-non-code-metrics', metrics, timestamp, message, partition)
    ...
    def get_baselines(self):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'baselines')
        parameters = {b'detail': True}
        headers = {b'Accept': b'application/json'}
        ...
        if not response.ok:
            raise ServiceError((b'ERROR: GET {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
        return [ Baseline(x[b'name'], x[b'description'], timestamp=x[b'timestamp']) for x in response.json() ]
    ...
    def get_projects(self):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_global_service_url(b'projects')
        parameters = {b'detail': True}
        response = self.get(service_url, parameters)
        return [ ProjectInfo(project_id=x[b'id'], name=x[b'name'], description=x.get(b'description'), creation_timestamp=x[b'creationTimestamp'], alias=x.get(b'alias'), deleting=x[b'deleting'], reanalyzing=x[b'reanalyzing']) for x in response.json()
               ]
    ...
    def _add_project(self, project_configuration, skip_project_validation, perfrom_update_call):
        ...
        Raises:
            ServiceError: If anything goes wrong.
        """
        service_url = self.get_global_service_url(b'create-project')
        parameters = {b'skip-project-validation': skip_project_validation, 
           b'only-config-update': perfrom_update_call}
        ...
        if response_message != b'success':
            raise ServiceError((b'ERROR: GET {url}: {status_code}:{message}').format(url=service_url, status_code=response.status_code, message=response_message))
        return response
    ...
    def _get_response_message(response):
        ...
        """
        return response.json().get(b'message')
    ...
    def _get_timestamp_parameter(self, timestamp, branch=None):
        ...
        if default_branch_name:
            return default_branch_name + b':' + timestamp_or_head
    ...
    def get_global_service_url(self, service_name):
        ...
        Returns:
            str: The full url
        """
        return b'%s/%s/' % (self.url, service_name)
    ...
    def get_project_service_url(self, service_name):
        ...
        Returns:
            str: The full url
        """
        return (b'{client.url}/p/{client.project}/{service}/').format(client=self, service=service_name)
    ...
    def upload_files_for_precommit_analysis(self, timestamp, precommit_data):
        ...
        """
        service_url = self.get_project_service_url(b'pre-commit') + self._get_timestamp_parameter(timestamp)
    ...
    def get_precommit_analysis_results(self):
        ...
        """
        service_url = self.get_project_service_url(b'pre-commit')
        while True:
    ...
    def _parse_findings_response(self, service_url, response):
        ...
        if not response.ok:
            raise ServiceError((b'ERROR: GET {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
        added_findings = self._findings_from_json(response.json()[b'addedFindings'])
        findings_in_changed_code = self._findings_from_json(response.json()[b'findingsInChangedCode'])
        removed_findings = self._findings_from_json(response.json()[b'removedFindings'])
        return (
    ...
    def _findings_from_json(self, findings_json):
        ...
        """
        return [ self._finding_from_json(finding) for finding in findings_json
               ]
    ...
    def _finding_from_json(self, finding_json):
        ...
        """
        return Finding(finding_type_id=finding_json[b'typeId'], message=finding_json[b'message'], assessment=finding_json[b'assessment'], start_offset=self._get_finding_location_entry(finding_json, b'rawStartOffset', 0), end_offset=self._get_finding_location_entry(finding_json, b'rawEndOffset', 0), start_line=self._get_finding_location_entry(finding_json, b'rawStartLine', 1), end_line=self._get_finding_location_entry(finding_json, b'rawEndLine', 1), uniform_path=finding_json[b'location'][b'uniformPath'], finding_id=finding_json[b'id'])
    ...
    def _get_finding_location_entry(self, finding_json, key, defaultValue):
        ...
        """
        value = finding_json[b'location'].get(key)
        if value is None:
            return defaultValue
        else:
            return value
    ...
    def get_findings(self, uniform_path, timestamp, recursive=True):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'findings') + uniform_path
        parameters = {b't': self._get_timestamp_parameter(timestamp=timestamp), 
    ...
    def get_finding_by_id(self, finding_id, branch=None, timestamp=None):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'findings-by-id') + finding_id
        parameters = {b't': self._get_timestamp_parameter(timestamp=timestamp, branch=branch)}
        response = self.get(service_url, parameters=parameters)
        if not response.ok:
            raise ServiceError((b'ERROR: GET {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
    ...
    def get_finding_url(self, finding):
        ...
        if not finding.finding_id:
            ...
            return (b'{client.url}/findings.html#details/{client.project}/?id={finding_id}').format(client=self, finding_id=finding.finding_id)

    def get_tasks(self, status=b'OPEN', details=True, start=0, max=300):
        ...
        Raises:
            ServiceError: If anything goes wrong
            """
        service_url = self.get_project_service_url(b'tasks')
        parameters = {b'status': status, 
           b'details': details, 
           b'start': start, 
           b'max': max, 
           b'with-count': False}
    ...
    def add_task_comment(self, task_id, comment):
        ...
        Raises:
            ServiceError: If anything goes wrong
        """
        service_url = self.get_project_service_url(b'comment-task') + str(task_id)
        response = self.put(service_url, data=to_json(comment))
        if not response.ok:
            raise ServiceError((b'ERROR: PUT {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
        return response
    ...
    def add_issue_metric(self, name, issue_query):
        ...
        Returns:
            requests.Response: request's response
        """
        url = b'%s/%s' % (self.get_project_service_url(b'issue-metrics'), name)
        return self.put(url, {b'name': name, b'query': issue_query})
    ...
    def create_dashboard(self, dashboard_descriptor):
        ...
        Returns:
            requests.Response: request's response
        """
        service_url = self.get_global_service_url(b'dashboard-export')
        multiple_files = [(b'dashboardDescriptor', dashboard_descriptor)]
        return requests.post(service_url, auth=self.auth_header, verify=self.sslverify, files=multiple_files, timeout=self.timeout)
    ...
    def get_project_configuration(self, project_id):
        """Adds a new dashboard from the given template.
                ...
                """
        url = b'%s%s' % (self.get_global_service_url(b'create-project'), project_id)
    ...
    def get_architectures(self):
        ...
        """
        service_url = self.get_project_service_url(b'arch-assessment')
        parameters = {b'list': True}
        response = self.get(service_url, parameters=parameters)
        if not response.ok:
            raise ServiceError((b'ERROR: GET {url}: {r.status_code}:{r.text}').format(url=service_url, r=response))
        return [ architecture_overview[b'uniformPath'] for architecture_overview in response.json() ]