...
# Embedded file name: build/bdist.linux-i686/egg/buildutils/command/stats.py
...
class stats(Command):
    __module__ = __name__
    description = 'show high level statistics (number of files, modules, packages, lines)'
    user_options = [
     ('exclude-tests', 't', "don't include modules that start with 'test_'"), ('extra-packages=', 'p', 'Also count code from the given packages'), ('totals', None, 'Show only the totals')]
    ...
    def dump_stats(self):
        ...
        line_count = totals[1]
        print '%d lines of code, %d module%s, %d package%s.' % (line_count, module_count, module_s, package_count, package_s)
    ...
    def dump_line_counts(self):
        ...
        if test_line_counts:
            ...
            print 'Test code: %s  other code: %s  ratio: %s code to tests' % (test_code, other_code, ratio_string(test_code, other_code))
        print
        return totals
...
def testing_module(mod_name):
    ...
    """
    return re.search('\\btests?\\b|\\btest_', mod_name)