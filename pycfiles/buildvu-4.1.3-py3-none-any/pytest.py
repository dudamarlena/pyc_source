...
# Embedded file name: build/bdist.linux-i686/egg/buildutils/pytest_command/pytest.py
...
class pytest(Command):
    ...
    user_options = [
     ('tests=', 't', 'list of directories/files containing tests.'), ('exitfirst', 'x', 'exit instantly on first error or failed test.'), ('no-capture', 's', 'disable catching of sys.stdout/stderr output.'), ('showlocals', 'l', 'show locals in tracebacks.'), ('pdb', None, 'start pdb (The Python Debugger) on errors.'), ('fulltrace', None, "don't cut any tracebacks (default is to cut)"), ('no-magic', None, 'refrain from using magic as much as possible'), ('collectonly', None, 'show what tests are collected but do not run them.'), ('traceconfig', None, 'trace considerations of conftest.py files'), ('tkinter', None, 'use tkinter test session frontend.'), ('looponfailing', 'f', 'loop on failing test set.')]
    boolean_options = [
     'exitfirst', 'no-capture', 'showlocals', 'pdb', 'fulltrace', 'no-magic', 'collectonly', 'traceconfig', 'tkinter', 'looponfailing']
    expanding_options = [
     'tests']
