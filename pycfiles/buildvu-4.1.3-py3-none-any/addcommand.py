...
# Embedded file name: build/bdist.linux-i686/egg/buildutils/command/addcommand.py
...
class addcommand(option_base):
    ...
    user_options = [
     ('package=', 'p', 'Module of the command you want to add'), ('list-commands', 'l', 'List available commands'), ('force', 'f', 'Force it to succede')] + option_base.user_options
    boolean_options = [
     'list-commands', 'force'] + option_base.boolean_options
    ...
    def check_command(self):
        ...
        if not self.package:
            raise DistutilsOptionError('You must provide a --package option')
        try:
            pkg = self.import_module(pkg_name)
        except ImportError, e:
            log.debug('Error while importing %s: %s' % (pkg_name, e))
            raise DistutilsOptionError('There is no module with the name %r' % pkg_name)

        filename = pkg.__file__
        if not os.path.splitext(os.path.basename(filename))[0] == '__init__':
            raise DistutilsOptionError('The command package %s is a module; you must give the package name' % pkg_name)
        ...
        for command_name in self.sub_modules(pkg):
            ...
            try:
                ...
                debug('Could not import %s: %s' % (mod_name, e))
                continue

            try:
                cls_obj = getattr(mod, command_name)
            except AttributeError, e:
                debug('Module %s has no %s attribute' % (mod_name, command_name))
                continue

            if not isinstance(cls_obj, types.ClassType) or not issubclass(cls_obj, DistutilsCommand):
                debug('Class %s.%s is not a subclass of distutils.cmd.Command' % (mod_name, command_name))
                continue
            found = True
    ...
    def import_module(self, mod_name):
        assert isinstance(mod_name, basestring), 'Bad value for mod_name: %r' % mod_name
        __import__(mod_name)
    ...
    def run(self):
        ...
        if self.package in cur_setting:
            if not self.force:
                raise DistutilsOptionError('The package %s is already listed in %s' % (self.package, self.filename))
            else:
                log.debug('The package %s is already listed in %s' % (self.package, self.filename))
                return
    ...
    def find_commands(self, pkg_mod_name):
        ...
        for mod_name in self.sub_modules(pkg_mod):
            ...
            try:
                ...
                log.info('Cannot load %s: %s' % (full_name, e))
                continue

            try:
                cmd = getattr(mod, mod_name)
            except AttributeError:
                log.debug('Module %s contains no %s attribute' % (full_name, mod_name))
