...
# Embedded file name: build/bdist.linux-x86_64/egg/gitdata/exectools.py
...
import subprocess, time, shlex, pushd, assertion, logging
SUCCESS = 0
...
class Exec(object):
    ...
    def check_assert(self, cmd, retries=1, pollrate=60, on_retry=None):
        ...
        for try_num in range(0, retries):
            if try_num > 0:
                self.logger.debug(('assert: Failed {} times. Retrying in {} seconds: {}').format(try_num, pollrate, cmd))
                time.sleep(pollrate)
                if on_retry is not None:
                    self.gather(on_retry)
            result, stdout, stderr = self.gather(cmd)
            if result == SUCCESS:
                break

        self.logger.debug(('assert: Final result = {} in {} tries.').format(result, try_num))
        assertion.success(result, ('Error running [{}] {}.\n{}').format(pushd.Dir.getcwd(), cmd, stderr))
        return (
         stdout, stderr)
    ...
    def gather(self, cmd):
        ...
        rc = proc.returncode
        self.logger.debug(('Process {}: exited with: {}\nstdout>>{}<<\nstderr>>{}<<\n').format(cmd_info, rc, out, err))
        return (
         rc, out, err)