...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/s3iotools-project/s3iotools/io/dataframe.py
# Compiled at: 2019-05-19 22:48:53
"""
s3 IO tools.
"""
import attr, pandas as pd
...
class S3Dataframe(object):
    ...
    bucket_name = attr.ib(validator=attr.validators.optional(attr.validators.instance_of(string_types)), default=None)
    _bucket = attr.ib(default=None)
    key = attr.ib(validator=attr.validators.optional(attr.validators.instance_of(string_types)), default=None)
    _object = attr.ib(default=None)
    df = attr.ib(validator=attr.validators.optional(attr.validators.instance_of(pd.DataFrame)), default=None)
    ...
    to_csv_kwargs_default = {'encoding': 'utf-8', 
       'index': False}
    ...
    def to_csv(self, bucket=None, key=None, gzip_compressed=False, **to_csv_kwargs):
        ...
        """
        bucket, key, kwargs = self.prepare_args(bucket, key, to_csv_kwargs, self.to_csv_kwargs_default)
    ...
    read_csv_kwargs_default = {'encoding': 'utf-8'}
    ...
    def read_csv(self, bucket=None, key=None, gzip_compressed=False, **read_csv_kwargs):
        ...
        """
        bucket, key, kwargs = self.prepare_args(bucket, key, read_csv_kwargs, self.read_csv_kwargs_default)
    ...
    to_json_kwargs_default = {'force_ascii': False}
    ...
    def to_json(self, bucket=None, key=None, gzip_compressed=False, **to_json_kwargs):
        ...
        :return: s3.Bucket.put_object() response
        """
        bucket, key, kwargs = self.prepare_args(bucket, key, to_json_kwargs, self.to_json_kwargs_default)
    ...
    read_json_kwargs_default = {'encoding': 'utf-8'}
    ...
    def read_json(self, bucket=None, key=None, gzip_compressed=False, **read_json_kwargs):
        ...
        :return: s3.Object.get() response
        """
        bucket, key, kwargs = self.prepare_args(bucket, key, read_json_kwargs, self.read_json_kwargs_default)
        obj = bucket.Object(key)
        response = obj.get()
        body = response['Body'].read()
        if gzip_compressed:
    ...
    def to_parquet(self, bucket=None, key=None, compression=None, **write_table_kwargs):
        ...
        bucket, key, kwargs = self.prepare_args(bucket, key, write_table_kwargs, self.write_table_kwargs_default)
        buffer = BytesIO()
        parquet.write_table(pyarrow.Table.from_pandas(self.df), buffer, compression=compression, **write_table_kwargs)
    ...
    def read_parquet(self, bucket=None, key=None, **read_table_kwargs):
        ...
        """
        from pyarrow import parquet
        bucket, key, kwargs = self.prepare_args(bucket, key, read_table_kwargs, self.read_table_kwargs_default)
        obj = bucket.Object(key)
        response = obj.get()
        buffer = BytesIO()
