...
# [GCC 8.4.0]
# Embedded file name: /home/mahnve/.virtualenvs/blog/lib/python2.7/site-packages/sinor/html_content.py
# Compiled at: 2015-04-12 09:53:13
import lxml.html
from lxml import etree
from sinor import file_util
EMPTY_RESULT = {'title': '', 'date': '', 
   'content': '', 
   'status': '', 
   'tags': []}

def _get_text_value(html, id_name):
    return _extract_data(html, id_name, lambda t: t.text, '')


def _get_list(html, id_name):
    return _extract_data(html, id_name, lambda t: [ li.xpath('string()') for li in t if li.xpath('string()') ], [])


def _get_sub_tree(html, id_name):
    return _extract_data(html, id_name, lambda t: etree.tostring(t), '')
...
def from_file(file_name):
    return from_string(file_util.read_file(file_name), {'file_name': file_name, 'absolute_url': file_util.absolute_href_for_file(file_name), 
