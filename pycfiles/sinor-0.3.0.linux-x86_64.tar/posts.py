...
# [GCC 8.4.0]
# Embedded file name: /home/mahnve/.virtualenvs/blog/lib/python2.7/site-packages/sinor/posts.py
# Compiled at: 2015-04-13 14:47:19
...
def render_post_list(file_names, template, count, markdown_file=None):
    ...
    return build_post_list(render_mustache_page_for_template, file_names, count, markdown_file)
...
def render_mustache_page(template, content={}):
    ...
    mustache_renderer = pystache.Renderer(search_dirs=partials_dir(template), file_encoding='utf8')
...
def build_post_list(func, file_names, count, markdown_file=None):
    ...
    if markdown_file:
        ...
        content = {}
    return func(dict({'posts': posts, 'tags': tags}, **content))
...
def common_data(to_merge={}):
    return dict({'year': date.today().year, 'author': config.author(), 
       'blog_title': config.blog_title()}, **to_merge)
