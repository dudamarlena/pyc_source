...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/list_ACE.py
...
class ListACE(Miner):
    ...
    def formatACE(self, truste, subject, accesstype, perm):
        ...
        return (
         r(truste), r(subject), accesstype, self.type2human(perm))
    ...
    def run(self, options, doc):
        ...
        if options.subject:
            ...
            bigquery = {'$or': [{'sd_value.DACL.ACEList': {'$elemMatch': query}}, {'sd_value.SACL.ACEList': {'$elemMatch': query}}]}
            ...
            for raw_sd in self.sd_table.find(bigquery):
                ...
                query = {'nTSecurityDescriptor': securitydescriptor.sd_id, 
                   'objectSid': {'$exists': 1}, 'objectCategory': {'$in': [self.categories.person, self.categories.group]}}
                subjects = set()
