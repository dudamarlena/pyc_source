...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miner.py
# Compiled at: 2015-10-20 16:27:01
import argparse, pkgutil, sys, bta.docstruct
from bta.docstruct import LiveRootDoc, RootDoc
import bta.backend, bta.formatters, bta.miners
...
class Miner(object):
    ...
    def main(cls):
        ...
        parser.add_argument('-C', dest='connection', help="DB connection string. Ex: 'dbname=test user=john' for PostgreSQL or '[ip]:[port]:dbname' for mongo)", metavar='CNX')
        parser.add_argument('-B', dest='backend_type', default='mongo', choices=bta.backend.Backend.backends.keys(), help='database backend')
        parser.add_argument('--force-consistency', dest='force_consistency', action='store_true', help='Do not run consistency checks')
        parser.add_argument('--live-output', dest='live_output', action='store_true', help='Provides a live output')
        parser.add_argument('-t', '--output-type', dest='output_type', help='output document type', choices=bta.formatters.Formatter._formatters_.keys())
        parser.add_argument('-o', '--output-file', dest='output', help='output file', metavar='FILENAME')
        parser.add_argument('-e', '--encoding', dest='encoding', default='utf8', help='output encoding. Default: utf8', metavar='ENCODING')
    ...
    def assert_field_exists(cls, table, field):
        ...
        cnt = c.limit(1).count(with_limit_and_skip=True)
