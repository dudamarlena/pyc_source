...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/passwords.py
...
class Passwords(Miner):
    ...
    def account_creation_date(self, doc, account_type):
        ...
        t.add()
        for account in self.datatable.find({'whenCreated': {'$exists': True}, 'objectCategory': {'$in': [account_type]}, '$or': [{'isDeleted': False}, {'isDeleted': {'$exists': False}}]}):
    ...
    def never_logged(self, doc, field, account_type):
        ...
        t.add()
        for account in self.datatable.find({field: {'$exists': False}, 'objectCategory': {'$in': [account_type]}, '$or': [{'isDeleted': False}, {'isDeleted': {'$exists': False}}]}):
