...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/libesedb/libesedb.py
...
class LibESEDB(object):
    ...
    def __init__(self, ignore_errors=False, report_error=lambda x: None):
        ...
        try:
            ...
            if e.args[0].endswith('cannot open shared object file: No such file or directory'):
                raise ESEDB_Error('%s. Did you install it or did you use LD_LIBRARY_PATH correctly ?' % e.message)
            raise
    ...
    def record_get_value(self, record, value_num):
        ...
        return (
         string_at(data, datalen.value), flags.value)
...
class ESEValue(object):
    __slots__ = [
     'record', 'lib', 'num', 'type', 'flags', 'value']
