...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/list_group.py
...
class ListGroup(Miner):
    ...
    def get_members_of(self, group, recursive=False):
        ...
        for link in self.link_table.find({'link_DNT': group['DNT_col']}):
            ...
            guid = row['objectGUID']
            if '1.2.840.113556.1.5.8' in objectClass:
    ...
    def run(self, options, doc):
        ...
        if options.match:
            match = {'$and': [{'objectClass': '1.2.840.113556.1.5.8'}, {'$or': [{'name': {'$regex': options.match}}, {'objectSid': {'$regex': options.match}}]}]}
        listemptyGroup = []
        for group in self.datatable.find(match):
            ...
            for objectType, linkBase, sid, deleted, fromgrp, name, samAccountName, guid in deleted_last(membership):
                ...
                if objectType == 'User' and linkBase == 1:
            ...
            if len(memberUsersTable) != 0:
                headers = [
                 'Name', 'SAM Account Name', 'SID', 'Deletion', 'Flags', 'Recursive']
            ...
            if len(otherLinkedTable) != 0:
                headers = [
                 'Object Type', 'Link Type', 'Name', 'SAM Account Name', 'SID', 'Deletion', 'Recursive']
