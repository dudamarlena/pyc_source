...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/miners/schema.py
...
class Schema(Miner):
    ...
    def change(self, year, month, day, atrib):
        ...
        req = {'$and': [{'objectCategory': atrib}, {'whenChanged': {'$gt': start, '$lt': end}}]}
    ...
    def create(self, year, month, day, atrib):
        ...
        req = {'$and': [{'objectCategory': atrib}, {'whenCreated': {'$gt': start, '$lt': end}}]}
        for entry in self.datatable.find(req):
            result.append([entry['cn'], entry['objectGUID']])
