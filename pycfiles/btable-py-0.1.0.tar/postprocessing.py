...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/postprocessing.py
# Compiled at: 2015-10-20 16:27:01
from collections import defaultdict
import bta.backend, bta.dblog, bta.tools.registry, logging
...
class PostProcessing(object):
    ...
    def memberOf(self):
        ...
        for member_DNT, groups_DNT in members.iteritems():
            ...
            memberOf.insert({'member_DNT': member_DNT, 'member_name': member_name_dn['name'], 'member_dn': member_name_dn['DName'], 
               'groups': groups_DNT, 
               'group_dn': group_dn, 'group_names': group_names})
...
def main():
    ...
    parser.add_argument('-C', dest='connection', help="Backend connection string. Ex: 'dbname=test user=john' for PostgreSQL or '[ip]:[port]:dbname' for mongo)", metavar='CNX')
    parser.add_argument('-B', dest='backend_class', default='mongo', help='database backend', choices=bta.backend.Backend.backends.keys())
    parser.add_argument('--only', dest='only', help='Only run POSTPROC', choices=PostProcessing.list_post_processors())
    parser.add_argument('--overwrite', dest='overwrite', action='store_true', help='Delete tables that already exist in db')
