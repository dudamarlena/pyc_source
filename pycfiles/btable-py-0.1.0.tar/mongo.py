...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/bta/backend/mongo.py
# Compiled at: 2015-10-20 16:27:01
import pymongo, struct
from bta.normalization import TypeFactory, Normalizer
from bta.backend import Backend, RawTable, VirtualTable
import bson.binary, bta.sd, bta.datatable, bta.tools.decoding, bta.tools.expr, bta.dbmeta, logging, functools, re
...
class MongoIntNormalizer(MongoNormalizer):
    ...
    def normal(self, val):
        if -9223372036854775808 <= val < 9223372036854775808:
            return val
        return str(val)
...
class MongoTable(RawTable):
    ...
    def insert_fields(self, values, ignore_normalization_errors=False):
        try:
            d = {name:norm.normal(v) for (name, norm), v in zip(self.fields, values) if not norm.empty(v) if not norm.empty(v)}
        ...
        * --overwrite has not been passed
        ...
        if db_data_format_version is None:
            ...
            if getattr(self.options, 'ignore_version_mismatch', False):
                log.info('Format version mismatch (stored: %d, supported: %d) ignored.' % (db_data_format_version, self.data_format_version))
                return False
            if getattr(self.options, 'overwrite', False):
                log.info('Re-creating tables with updated data format version (%d -> %d).' % (db_data_format_version, self.data_format_version))
                return False
            log.error('Importer version mismatch. Database %s has already been imported using version %d importer format version. This program uses format version %d. You should either re-import the database using --overwrite, or continue with an older version of this tool. \nUsing --ignore-version-mismatch might lead to incorrect results.' % (
...
class VirtualDataSD(VirtualTable):
    ...
    def find(self, req, proj={}):
        ...
        for dtres in self.datatable.find(dtreq):
            ...
            resp = dtres if not proj else {dtres[k] for k, v in proj.iteritems() if v and k in dtres}
            yield resp
