...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/gbdx_buffet/__init__.py
# Compiled at: 2018-04-24 14:50:21
# Size of source mod 2**32: 12188 bytes
import argparse, json, os
from datetime import datetime
from logging import getLogger
from pprint import pprint
import fiona, regex, sh
...
try:
    ...
    print('All operations on GBDX require credentials. You can sign up for aGBDX account at https://gbdx.geobigdata.io. Your GBDX credentialsare found under your account profile.gbdxtools expects a config file to exist at ~/.gbdx-config withyour credentials. (See formatting for thisfile here: https://github.com/tdg-platform/gbdx-auth#ini-file.)')
...
def download(prefix, output, verbose=False, dryrun=False):
    ...
    while try_again:
        try:
            ...
            if verbose:
                print(all_folders)
            all_folders = regex.findall(regex.escape(prefix) + '[^ ]*/', str(all_folders))
            print(all_folders)
...
def workflow_cli():
    ...
    parser.add_argument('-i', '--catids', help='Comma list of CATALOG IDS to be read (10400100175E5C00,104A0100159AFE00,104001002A779400,1040010026627600)', type=(lambda s: s.split(',')))
    ...
    parser.add_argument('-w', '--wkt', help='WKT indicating where to clip images e.g. POLYGON ((109.79359281016 18.3095645755021, ....))',
      default='')
    ...
    if args.shapefile:
        ...
        if 'CATALOGID' in args.shapefile.columns:
            ...
            if 'cat_id' in args.shapefile.columns:
                catalog_ids = list(args.shapefile.cat_id.values)
            else:
                if 'catid' in args.shapefile.columns:
...
def launch_workflows(catalog_ids, name=datetime.now().isoformat().split('T')[0], pansharpen=False, dra=False, acomp=True, wkt=None, ortho=True):
    return [launch_workflow((o['acquisition_id']), name, pansharpen=pansharpen, dra=dra, acomp=acomp, wkt=wkt, ortho=ortho) for o in gbdx.ordering.location(catalog_ids)['acquisitions']]
...
def launch_workflow(cat_id, name, pansharpen=False, dra=False, acomp=True, wkt=None, ortho=True):
    ...
    if ortho:
        aop = gbdx.Task('AOP_Strip_Processor', data=(order.outputs.s3_location.value),
...
def check_workflow_cli():
    parser = argparse.ArgumentParser(description='Launch a workflow to order images from GBDX')
    parser.add_argument('workflow_ids', help='Comma list of Worflow IDS to be read (4756293649288340653,4756293656541265420,4756293664199537883)', type=(lambda s: s.split(',')))
    ...
    def extract_detects(self, cli=False):
        if self.itemType == 'all':
            ...
            if geoaoi is not None:
                ...
                for aoi in geoaoi:
                    results = gbdx.vectors.query((str(shape(aoi['geometry']))), query=es_query,
...
def fetch_results_cli():
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--inAoi', help='Input *.shp or *.geojson /path/to/file',
      type=str,
      required=True)
    parser.add_argument('-o', '--outDir', help='Geojson /path/to/store/geojson',
      type=str,
      required=True)
    parser.add_argument('-vi', '--gbdxVecIndex', default='vector-*',
      help='Name of GBDx vector index, ex: vector-deepcore-extract-cars*, vector-openskynet*, vector-*',
      type=str)
    parser.add_argument('-it', '--itemType', default='all',
      help='Name of object type, options: all, Airliner, Passenger Cars, Aircraft, etc.',
      type=str)
    parser.add_argument('-std', '--startDate', default='*',
      help='Beginning date for index query. ex: 2000-01-01 or *',
      type=str)
    parser.add_argument('-end', '--endDate', default='now',
      help='End date for index query. ex: 2017-01-01 or now',
      type=str)
    parser.add_argument('-max', '--maxCount', default=200000,
      help='Maximum number of features return by GBDx vector index.',
      type=int)
    args = parser.parse_args()
    if not os.path.isfile(args.inAoi):
        raise AssertionError
    else:
        if not os.path.splitext(args.inAoi)[1] == '.shp':
            if not os.path.splitext(args.inAoi)[1] == '.geojson':
                raise AssertionError
        assert os.path.isdir(args.outDir)
    fetcher = FetchGBDxResults(args.inAoi, args.outDir, args.startDate, args.endDate, args.gbdxVecIndex, args.itemType, args.maxCount)
