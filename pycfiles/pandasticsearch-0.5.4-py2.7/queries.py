...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pandasticsearch/queries.py
# Compiled at: 2019-09-08 22:29:59
...
class Query(collections.MutableSequence):
    ...
    def print_json(self):
        indented_json = json.dumps(self._result_dict, sort_keys=True, separators=(',',
                ...
                if 'buckets' in v:
                    for sub_bucket in v['buckets']:
                        if 'key_as_string' in sub_bucket:
                            ...
                            key = sub_bucket['key']
                        for x in Agg._process_agg(sub_bucket, indexes + (key,), names + (k,)):
                            yield x
