...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/pandasticsearch/dataframe.py
# Compiled at: 2020-01-28 08:45:41
...
class DataFrame(object):
    ...
    def index(self):
        ...
        if self._index is None:
            ...
            if self._doc_type:
                return self._index + '/' + self._doc_type
            return self._index
    ...
    def columns(self):
        ...
        """
        if self._mapping:
            return sorted(self._get_cols(self._mapping))
        else:
            return
    ...
    def from_es(**kwargs):
        ...
        return DataFrame(client=RestClient(url, endpoint, username, password, verify_ssl), mapping=mapping, index=index, doc_type=doc_type, compat=compat)
    ...
    def __getitem__(self, item):
        if isinstance(item, six.string_types):
            ...
            return Column(item)
        if isinstance(item, BooleanFilter):
            self._filter = item
            return self
    ...
    def filter(self, condition):
        ...
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=_filter, groupby=self._groupby, aggregation=self._aggregation, projection=self._projection, sort=self._sort, limit=self._limit, compat=self._compat)
    ...
    def select(self, *cols):
        ...
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=self._groupby, aggregation=self._aggregation, projection=projection, sort=self._sort, limit=self._limit, compat=self._compat)
    ...
    def limit(self, num):
        ...
        assert num >= 1
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=self._groupby, aggregation=self._aggregation, projection=self._projection, sort=self._sort, limit=num, compat=self._compat)
    ...
    def groupby(self, *cols):
        ...
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=groupby, aggregation=self._aggregation, projection=self._projection, sort=self._sort, limit=self.limit, compat=self._compat)
    ...
    def agg(self, *aggs):
        ...
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=self._groupby, aggregation=aggregation, projection=self._projection, sort=self._sort, limit=self._limit, compat=self._compat)
    ...
    def sort(self, *cols):
        ...
        return DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=self._groupby, aggregation=self._aggregation, projection=self._projection, sort=sorts, limit=self._limit, compat=self._compat)
    ...
    def count(self):
        ...
        [2, 1]
        """
        df = DataFrame(client=self._client, index=self._index, doc_type=self._doc_type, mapping=self._mapping, filter=self._filter, groupby=self._groupby, aggregation=_count_aggregator, projection=self._projection, sort=self._sort, limit=self._limit, compat=self._compat)
        return df
    ...
    def print_schema(self):
        ...
        if self._index is None:
            ...
            elif self._doc_type is not None:
    ...
    def resolve_schema(self, json_prop, res_schema='', depth=1):
        for field in json_prop:
            if 'properties' in json_prop[field]:
                res_schema += ('{}|--{}:\n').format('  ' * depth, field)
                res_schema = self.resolve_schema(json_prop[field]['properties'], res_schema, depth=depth + 1)
            else:
