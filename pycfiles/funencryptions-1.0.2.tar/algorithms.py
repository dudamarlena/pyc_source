...
# [GCC 8.4.0]
# Embedded file name: funencryptions/algorithms.py
# Compiled at: 2013-04-26 08:12:06
...
def backwards_interleave(string, decode=False):
    ...
    if remainder:
        backward += ' '
    result = ('').join([ forward[i] + backward[i] for i in range(len(forward)) ])
    if remainder:
        result = result[:-1]
    return result
...
def character_code_timestamp_date(string, decode=False, fmt='%d.%m.%Y %H:%M:%S'):
    ...
    if decode:
        ...
        result = ('').join([ unichr(code) for code in codes ])
    else:
