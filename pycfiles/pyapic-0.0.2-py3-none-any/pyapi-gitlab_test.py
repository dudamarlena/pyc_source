...
# Embedded file name: /usr/local/lib/python2.7/site-packages/gitlab_tests/pyapi-gitlab_test.py
# Compiled at: 2015-09-08 15:26:34
__doc__ = '\npyapi-gitlab tests\n'
import unittest2 as unittest, gitlab, os, time, random, string
try:
...
class GitlabTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.project = cls.git.createproject(name=name, visibility_level='private', import_url='https://github.com/Itxaka/pyapi-gitlab.git')
        time.sleep(20)
    ...
    def test_user(self):
        assert isinstance(self.git.createuser(name='test', username='test', password='test1234', email='test@test.com', skype='this', linkedin='that'), dict)
        assert isinstance(self.git.getusers(), list)
        ...
        user = self.git.createuser('random', 'random', 'random1234', 'random@random.org', can_create_group='false')
        self.assertFalse(self.git.getuser(user['id'])['can_create_group'])
        self.git.deleteuser(user['id'])
        user = self.git.createuser('random', 'random', 'random1234', 'random@random.org', can_create_group='true')
        ...
        self.assertFalse(self.git.getuser(user['id'])['can_create_group'])
        self.git.deleteuser(user['id'])
        assert isinstance(self.git.getusers(page=2), list)
        assert isinstance(self.git.getusers(per_page=4), list)
        self.assertEqual(self.git.getusers(page=800), list(''))
        self.assertTrue(self.git.getusers(per_page=43))
    ...
    def test_project(self):
        ...
        assert isinstance(self.git.getprojecthook(self.project_id, self.git.getprojecthooks(self.project_id)[0]['id']), dict)
        self.assertTrue(self.git.editprojecthook(self.project_id, self.git.getprojecthooks(self.project_id)[0]['id'], 'http://another.com'))
        self.assertTrue(self.git.deleteprojecthook(self.project_id, self.git.getprojecthooks(self.project_id)[0]['id']))
        name = ('').join(random.choice(string.ascii_uppercase + string.digits) for _ in range(8))
    ...
    def test_deploykeys(self):
        ...
        if ssh_test:
            ...
            assert isinstance(self.git.adddeploykey(project_id=self.project_id, title=name, key=str(rsa_key.publickey().exportKey(format='OpenSSH'))), dict)
            keys = self.git.getdeploykeys(self.project_id)
    ...
    def test_sshkeys(self):
        ...
        if ssh_test:
            ...
            name = ('').join(random.choice(string.ascii_uppercase + string.digits) for _ in range(8))
            rsa_key = RSA.generate(1024)
            self.assertTrue(self.git.addsshkeyuser(self.user_id, title=name, key=str(rsa_key.publickey().exportKey(format='OpenSSH'))))
            self.assertGreater(self.git.getsshkeys(), 0)
            keys = self.git.getsshkeys()
            assert isinstance(keys, list)
            key = self.git.getsshkeys()[0]
    ...
    def test_repositories(self):
        ...
        assert isinstance(self.git.compare_branches_tags_commits(self.project_id, from_id=commit[1]['id'], to_id=commit[0]['id']), dict)
