...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.6/site-packages/abap.py
# Compiled at: 2017-08-03 17:58:35
# Size of source mod 2**32: 27844 bytes
import abc, argparse, collections, copy, datetime, itertools, logging, mimetypes, operator, pathlib, string, sys, time, typing, urllib.parse, xml.dom.minidom, xml.etree.cElementTree as ET, pkg_resources, schema, taglib, tornado.web, yaml
__version__ = '0.1.0'
...
AUDIO_EXTENSIONS = ('m4a', 'm4b', 'mp3', 'ogg', 'opus')
IMAGE_EXTENSIONS = ('jpeg', 'jpg', 'png')
COVER_FILENAMES = ('cover', 'cover_art', 'folder')
EXPORTABLE_ABOOK_KEYS = ('authors', 'title', 'slug', 'description', 'categories', 'explicit',
                         'cover', 'items')
EXPORTABLE_ITEM_KEYS = ('authors', 'categories', 'chapters', 'description', 'explicit',
                        'path', 'title')
...
common_parser.add_argument('directory',
  type=(pathlib.Path),
  default='.')

def make_filename_matcher(filenames=None, extensions=None) -> typing.Callable[([pathlib.Path], bool)]:
...
cover_matcher = make_filename_matcher(filenames=COVER_FILENAMES,
...
by_abook_key = collections.OrderedDict(zip(EXPORTABLE_ABOOK_KEYS, range(len(EXPORTABLE_ABOOK_KEYS)))).get
...
def render_chapter(chapter: dict) -> ET.Element:
    return ET.Element((psc('chapter')),
      attrib={'title':chapter['name'], 
     'start':format_duration(chapter['start'])})


def labeled_scan(path: pathlib.Path, label_funcs: typing.Dict[(str, LabelFunction)]):
    return {k:list(map(second, g)) for k, g in itertools.groupby(sorted((labeled_scan_iter(path, label_funcs)), key=first),
      key=first)}


def labeled_scan_iter(path: pathlib.Path, label_funcs: typing.Dict[(str, LabelFunction)]) -> ScanResult:
...
def make_item_sorter(items):
    original_sequences = {item['path']:idx for idx, item in enumerate(sorted(items, key=by_path), start=1)}
...
def parse_duration(ds: str) -> int:
    if not ds:
        ...
        ms_sep_pos = ds.rfind('.')
        if not ms_sep_pos == -1:
            ms = round(float(ds[ms_sep_pos:]) * 1000)
            ds = ds[:ms_sep_pos]
        else:
            ms = 0
        n = ds.count(':')
...
CHAPTER_SCHEMA = schema.Schema({'name': str, 
 'start': schema.Use(parse_duration), 
 schema.Optional('url'): str})
ITEM_SCHEMA = schema.Schema({'path': str, 
 ...
 schema.Optional('chapters'): [
                               CHAPTER_SCHEMA]})
ABOOK_SCHEMA = schema.Schema({schema.Optional('authors'): [
                              str], 
...
class Abook(collections.abc.Mapping):
    ...
    def merge_manifest(self, yaml_data=None):
        ...
        items = d.get('items', [])
        d.update({'items': sorted(items, key=(make_item_sorter(items)))})
        self._d = d
...
class XMLRenderer(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def render_item(self, abook: Abook, item: dict, sequence: int=0) -> ETGenerator:
...
class RSSRenderer(XMLRenderer):
    ...
    def render_channel(self, abook: Abook) -> ETGenerator:
        ...
        if abook.get('description'):
            ...
            if abook.has_manifest:
                dt = datetime.datetime.fromtimestamp(abook.manifest.stat().st_mtime)
            else:
    ...
    def render_item(self, abook: Abook, item: dict, sequence: int=0) -> ETGenerator:
        ...
        pub_date.text = time.strftime(RFC822, (datetime.datetime.now() - datetime.timedelta(minutes=sequence)).timetuple())
        yield pub_date
        yield ET.Element('enclosure', attrib={'type':item['mimetype'], 
         'length':str(item['size']), 
         'url':self.reverse_uri('stream', abook['slug'], str(sequence), item['path'].suffix.lstrip('.'))})
...
class PodloveChapterRenderer(XMLRenderer):
    ...
    def render_item(self, abook: Abook, item: dict, sequence: int=0) -> ETGenerator:
        if item.get('chapters'):
            chapters = ET.Element((psc('chapters')),
              attrib={'version': PSC_VERSION})
...
def merge(directory: pathlib.Path, data: typing.MutableMapping, yaml_data: typing.MutableMapping) -> typing.MutableMapping:
    ...
    if 'title' in yaml_data:
        if not yaml_data['title'] == data['title']:
...
def from_dir(directory: pathlib.Path) -> dict:
    authors, albums, descriptions, categories = map(lambda i: collections.OrderedDict(), range(4))
    all_non_explicit = True
    items = []
    results = labeled_scan(directory, {'audio':audio_matcher, 
     'cover':cover_matcher})
    ...
    global_data = {'title':'Unknown title',  'authors':[
      'Unknown author'], 
     'categories':[],  'items':items}
    if not all_non_explicit:
        global_data.update({'explicit': True})
    if authors:
        global_data.update({'authors': list(authors)})
    if albums:
        if len(albums) > 1:
            LOG.warn('Multiple values for album title found. Will use the first one')
        global_data.update({'title': first(list(albums.keys()))})
    if descriptions:
        if len(descriptions) > 1:
            LOG.warn('Multiple values for description found. Will use the first one')
        global_data.update({'description': first(descriptions)})
    if categories:
        global_data.setdefault('categories', []).extend(sorted(list({c for cats in categories for c in cats})))
    covers = results.get('cover', [])
    if covers:
        global_data.update({'cover': first(covers)})
    global_data.update({'slug': slugify(global_data['title'])})
    return global_data
...
def get_tags(file_path: pathlib.Path) -> dict:
    ...
    start_chapter = 1 if (start_chapter is None and 'CHAPTER001' in tags) else None
    if start_chapter is not None:
        for ch_no in range(start_chapter, 1000):
            ...
            if not (start and name):
                break
            chapters.append({'name':name, 
             'start':start, 
             'url':url})

    return {'authors':multi(tags, 'ARTIST'), 
...
def pretty_print_xml(tree: ET.Element) -> bytes:
    return xml.dom.minidom.parseString(ET.tostring(tree, encoding=DEFAULT_XML_ENCODING)).toprettyxml(encoding=DEFAULT_XML_ENCODING)
...
def build_rss(directory: pathlib.Path, abook: typing.Mapping, reverse_url=lambda n, *a: n) -> ET.Element:
    renderers = load_renderers()
    extensions = collections.OrderedDict([(n, cls(reverse_url)) for n, cls in renderers.items()])
    ...
    for idx, item in enumerate((abook.get('items', [])), start=1):
        ...
        for ext_name, ext in extensions.items():
            LOG.debug(f"Rendering item #{idx} elements with renderer: {ext_name}")
...
class CoverHandler(tornado.web.StaticFileHandler, AbookHandler):
    ...
    def get(self, slug):
        ...
        self.set_header('Content-Type', first(mimetypes.guess_type(str(cover))))
...
class RSSHandler(AbookHandler):
    ...
    def get(self, slug: str):
        self.assert_slug(slug)
        self.set_header('Content-Type', f'application/rss+xml; charset="{DEFAULT_XML_ENCODING}"')

        def make_url_reverse(reverse_func, base_url):

            def url_reverse(endpoint, *args, **kwargs):
                if endpoint:
                    return urllib.parse.urljoin(base_url, reverse_func(endpoint, *args, **kwargs))
                else:
                    return base_url
        ...
        self.write(pretty_print_xml(build_rss((self.abook.directory),
          (self.abook),
...
def make_app(abook: Abook):
    app = tornado.web.Application([
     tornado.web.URLSpec('/(?P<slug>\\w+)',
       RSSHandler,
       name='rss'),
     tornado.web.URLSpec('/(?P<slug>\\w+)/stream/(?P<sequence>\\d+).(?P<ext>[\\w]{1,})',
       StreamHandler,
       {'path': abook.directory},
       name='stream'),
     tornado.web.URLSpec('/(?P<slug>\\w+)/cover',
       CoverHandler,
       {'path': abook.directory},
       name='cover')])
    app.abook = abook
...
def get_parsers():
    parser = argparse.ArgumentParser(prog='abap',
...
def _prepare_for_export(directory: pathlib.Path, d: dict) -> dict:
    ...
    if len(authors) == len(result.get('authors', [])) and sorted(authors) == sorted(result.get('authors', [])):
        for item in result.get('items', []):
            item.pop('authors', None)

    return {k:result[k] for k in sorted(result, key=by_abook_key)}


class InitCommand(AbapCommand):
    __doc__ = 'initialize an abook for the audiobook in a given directory'
    parent_parsers = [
     common_parser]

    def init_parser(self, parser):
        parser.add_argument('-o',
          '--output', type=argparse.FileType(mode='w'),
          default='-')
...
class ServeCommand(AbapCommand):
    __doc__ = 'serve the RSS feed of the abook'
    parent_parsers = [
     common_parser]
...
def main(argv=None):
    ...
    for entry_point in pkg_resources.iter_entry_points('abap.command'):
        ...
        cmd_parser = subparsers.add_parser((entry_point.name),
          parents=(cmd_class.parent_parsers), help=(cmd_class.__doc__))
        cmd = cmd_class()
