...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/turbofeeds/controllers.py
# Compiled at: 2009-09-26 22:09:43
...
class FeedController(tg.controllers.Controller):
    ...
    """
    formats = [
     'atom1.0', 'atom0.3', 'rss2.0', 'mrss1.1.1']
    ...
    def mrss1_1_1(self, *args, **kwargs):
        ...
        feed.update(self.get_feed_data(*args, **kwargs))
        self.format_dates(feed, 822)
        feed.setdefault('link', self.get_feed_url('mrss_1_1_1', *args, **kwargs))
        log.debug(feed)
        return feed
    ...
    def init_feed(self, format):
        ...
        engine = self.feed_params.get('engine', tg.config.get('tg.defaultview', 'text'))
        feed = {'xml_stylesheet': partial(xml_stylesheet, engine=engine), 
