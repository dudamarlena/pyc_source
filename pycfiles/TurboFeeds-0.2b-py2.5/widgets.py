...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/turbofeeds/widgets.py
# Compiled at: 2009-09-26 20:01:35
...
from turbogears.widgets import CSSLink, Widget, WidgetDescription, register_static_directory
...
class FeedLinks(Widget):
    ...
    """
    params = [
     'base_url', 'controller', 'feed_types', 'mimetypes', 'title',
     'url_params']
    ...
    title = '%(type)s'
    url_params = {}
    mimetypes = {'atom0_3': 'application/atom+xml', 
       'atom1_0': 'application/rss+xml', 
       'mrss1_1_1': 'application/rss+xml', 
       'rss2_0': 'application/rss+xml'}
    params_doc = {'base_url': 'The base_url of the feed. The feed format will be appended to this. Can be determined from "controller", if given.', 
       'controller': 'The FeedController instance serving the feeds the links will point to.', 
       'feed_types': 'A list of 2-item tuples matching format identifier to format name. A link will be generated for each format.', 
       'title': 'String to use for "title" attribute of feed links. May use "%(type)s" placeholder for the feed format name.', 
       'url_params': 'Dictionary containing extra URL parameters appended to the feed URL'}
    ...
    def update_params(self, params):
        ...
        params['feed_url'] = lambda type: self.feed_url(base_url, type, params['url_params'])
...
class FeedButton(Widget):
    ...
    """
    params = [
     'base_url', 'title', 'type', 'url_params']
    ...
    css = [CSSLink('turbofeeds', 'css/feeds.css', media='screen')]
    base_url = '/feed'
    types = dict(atom0_3='Atom 0.3', atom1_0='Atom 1.0', rdf='RDF', rss2_0='RSS 2.0')
    type = 'atom1_0'
    title = ''
    url_params = {}
    params_doc = {'base_url': 'The base_url of the feed. The feed format will be appended to this. Can be determined from "controler", if given.', 
       'title': 'String to use for "title" attribute of feed links. May use "%(type)s" placeholder for the feed format name.', 
       'url_params': 'Dictionary containing extra URL parameters appended to the feed URL'}
...
if __name__ == '__main__':
    ...
    print
    print fl.render('View %(type)s feed', title='Click to view feed in browser', url_params=dict(compat=True))
