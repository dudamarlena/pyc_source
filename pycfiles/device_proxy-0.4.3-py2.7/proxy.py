...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/devproxy/proxy.py
# Compiled at: 2014-07-30 09:31:58
import urlparse
        ...
        for handler in self.handlers:
            ...
            for header in headers:
                for k, v in header.items():
                    request.setHeader(k.encode(self.encoding), v.encode(self.encoding))
...
class ReverseProxyResource(proxy.ReverseProxyResource):
    ...
    def getChild(self, path, request):
        ...
        return ReverseProxyResource(self.handlers, '', '', '', self.host, self.port, self.path + '/' + urlquote(path, safe=''), self.reactor)
        ...
        for handler in self.handlers:
            ...
            for header in headers:
                for key, value in header.items():
                    request.requestHeaders.addRawHeader(key.encode(self.encoding), value.encode(self.encoding))

            cookies = (yield handler.get_cookies(request)) or []
            for cookie in cookies:
                request.addCookie(cookie.key, cookie.value, **cookie.get_params())
    ...
    def connect_upstream(self, request):
        ...
        if qs:
            ...
            rest = self.path
        clientFactory = self.proxyClientFactoryClass(request.method, rest, request.clientproto, request.getAllHeaders(), request.content.read(), request)
...
class ProxySite(server.Site):
    ...
    def setup_resource(self, results):
        ...
        self.resource = self.resourceClass(started_handlers, self.debug_path, self.health_path, self.echo_path, self.upstream_host, int(self.upstream_port), self.path)