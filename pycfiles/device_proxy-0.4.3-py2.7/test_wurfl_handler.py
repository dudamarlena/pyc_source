...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/devproxy/tests/test_wurfl_handler.py
...
class WurlfHandlerTestCase(ProxyTestCase):
    ...
    def setUp(self):
        ...
        self.patch(SimpleWurflTestHandler, 'connect_to_memcached', lambda _: succeed(True))
        self.wurfl_handlers = yield self.start_handlers([
         SimpleWurflTestHandler({'header_name': 'X-UA-header', 
            'cache_prefix': 'prefix', 
    ...
    def test_debug_path(self):
        proxy, url = self.start_proxy(self.wurfl_handlers)
        request_path = '/_debug'
        response = yield http.request('%s%s' % (url, request_path), headers={'User-Agent': self.iphone_ua}, method='GET')
