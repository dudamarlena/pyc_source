...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/devproxy/handlers/wurfl_handler/base.py
...
class WurflHandler(BaseHandler):
    ...
    def get_headers(self, request):
        ...
        if cached:
            ...
            headers = yield self.handle_request_and_cache(cache_key, user_agent, request)
        returnValue(headers)
    ...
    def handle_request_and_cache(self, cache_key, user_agent, request):
        ...
        yield self.memcached.set(cache_key, json.dumps(headers), expireTime=self.cache_lifetime)
        returnValue(headers)
        return
