...
# Embedded file name: build/bdist.linux-x86_64/egg/pyload/script.py
# Compiled at: 2015-11-21 11:14:10
# Size of source mod 2**32: 10167 bytes
__doc__ = 'The main module which contains command line program\n'
import os, pip, shutil, zipfile, argparse, tempfile, contextlib, pkg_resources
...
class PyLoadBuilder(object):
    ...
    def __init__(self, packages, out_dir, include=INCLUDE, exclude=EXCLUDE, single=False):
        ...
        if self.single:
            for name in self.packages:
                if not binaries or name in binaries:
                    yield (
                     name, None, os.path.splitext(name)[0])

            return
        pkg_resources.working_set.add_entry(target_dir)
        installed = {p.project_name.lower():p for p in pkg_resources.working_set}
        ...
        with chdir(tmp):
            for root, dirs, files in os.walk('.'):
                for fname in files:
                    ...
                    if any(map(lambda x: fnmatch(fname, os.path.join('.', x)), self.exclude)):
                        pass
                    elif compile_python:
    ...
    def create_binaries(self, binaries=[], zipmode=zipfile.ZIP_STORED, patch=None, compile_python=True):
        ...
        with directory() as (tmp):
            ...
            with pkg_resources.resource_stream('pyload.resources', 'xlib.zip') as (src_stream):
...
def main():
    parser = argparse.ArgumentParser(description='%s: %s' % (
     NAME, DESCRIPTION))
    parser.add_argument('arguments', type=str, nargs='+', help='Packages to binarize in pip format')
    parser.add_argument('-o', '--out-dir', dest='out_dir', action='store', default='./out', help='The folder where resulting binaries will stored')
    parser.add_argument('-c', '--compress', dest='compress', action='store_true', default=False, help='If set, the payload will be compressed.')
    parser.add_argument('-s', '--single', dest='single', action='store_true', default=False, help='If set, argument is threaten as standalone python script file')
    parser.add_argument('-I', '--include', dest='include', action='append', default=INCLUDE, help='Packages to add to the binary which are not defined as requirements')
    parser.add_argument('-E', '--exclude', dest='exclude', action='append', default=EXCLUDE, help='Glob patterns for modules/packages to be excluded from resulting binary')
    parser.add_argument('-b', '--binary', dest='binaries', action='append', default=[], help='Name of the binaries to make. By default create all binaries defined in package and its dependencies.')
    parser.add_argument('-p', '--patch', dest='patch', action='store', default=None, help='Patch file to monkey patching the resulting payload (read the docs)')
    parser.add_argument('-nC', '--no-compile', dest='compile', action='store_false', default=True, help='Do not compile python files during binary creatoion.')
    args = parser.parse_args()
    if args.compress:
        zipmode = zipfile.ZIP_DEFLATED
    else:
        zipmode = zipfile.ZIP_STORED
    builder = PyLoadBuilder(args.arguments, args.out_dir, args.include, args.exclude, args.single)
    builder.create_binaries(args.binaries, zipmode, args.patch, args.compile)