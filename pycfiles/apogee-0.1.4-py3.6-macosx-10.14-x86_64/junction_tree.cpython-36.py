...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/apogee/inference/junction_tree.py
# Compiled at: 2019-11-27 14:42:48
        ...
        if neighbours is not None:
            for node, attrs in self.graph.nodes.items():
                if node != variable and attrs['tau'] in neighbours:
                    self.graph.add_edge(variable,
                      node,
                      messages={(
 variable, node): None, (node, variable): None})
        ...
        while self._message_count() < 2.0 * len(self.graph.edges):
            for source, target in self.graph.edges.keys():
                if self._can_send(source, target):
                    ...
                    if self._can_send(target, source):
    ...
    def _can_send(self, source, target):
        if self._message(source, target) is None:
            ...
            if n_received == n_neighbours - 1:
                if not self._has_received(target, source):
                    return True
            if n_received == n_neighbours:
                if self._has_received(target, source):
                    return True
        return False
    ...
    def _has_received(self, source, target):
        if self._message(source, target) is not None:
            return True
        else:
            return False
    ...
    def from_factors(cls, factor_set):
        ...
        for variable, _ in zip(*get_elimination_ordering(factor_set.adjacency_matrix)):
            current_factor_scope = union1d(*[scope for scope in factor_scopes if variable in scope])
