...
import sys, urllib, urllib2, types, mimetools, copy, urlparse, htmlentitydefs, re, random
...
class _AbstractFormParser():
    ...
    def start_form(self, attrs):
        ...
        self._current_form = (
         (
          name, action, method, enctype), d, controls)
        return
...
def _create_bs_classes(bs, icbinbs):
    ...
    return (
     RobustFormParser, NestingRobustFormParser)
...
def _ParseFileEx(file, base_uri, select_default=False, ignore_errors=False, form_parser_class=FormParser, request_class=urllib2.Request, entitydefs=None, backwards_compat=True, encoding=DEFAULT_ENCODING, _urljoin=urlparse.urljoin, _urlparse=urlparse.urlparse, _urlunparse=urlparse.urlunparse):
    if backwards_compat:
...
class Item():
    ...
    def __repr__(self):
        attrs = [
         (
          'name', self.name), ('id', self.id)] + self.attrs.items()
        return '<%s %s>' % (
...
class ListControl(Control):
    ...
    def __getattr__(self, name):
        if name == 'value':
            ...
            if self.name is None:
                return []
            return [ o.name for o in self.items if o.selected if not o.disabled or compat
                   ]
        else:
            raise AttributeError("%s instance has no attribute '%s'" % (
             self.__class__.__name__, name))
    ...
    def _multiple_set_value(self, value):
        compat = self._form.backwards_compat
        turn_on = []
        turn_off = [ item for item in self.items if item.selected if not item.disabled or compat
                   ]
        names = {}
        for nn in value:
            if nn in names.keys():
    ...
    def _totally_ordered_pairs(self):
        if self.disabled or self.name is None:
            ...
            return [ (o._index, self.name, o.name) for o in self.items if o.selected if not o.disabled ]
        return
