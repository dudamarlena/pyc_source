...
# [GCC 8.4.0]
# Embedded file name: /home/jarek/work/bip/src/bip/models.py
# Compiled at: 2019-09-28 16:30:56
...
class Directory(db.Model, Timestamp):
    ...
    children = db.relationship('Directory',
      backref=db.backref('parent', remote_side=[pk]))
    created_by_pk = db.Column((db.Integer), (db.ForeignKey('users.pk')), nullable=False)
    created_by = db.relationship('User',
      foreign_keys=[created_by_pk], backref=db.backref('directories_created', lazy='dynamic'))
    updated_by_pk = db.Column(db.Integer, db.ForeignKey('users.pk'))
    updated_by = db.relationship('User',
      foreign_keys=[updated_by_pk], backref=db.backref('directories_updated', lazy='dynamic'))
    page_pk = db.Column((db.Integer), (db.ForeignKey('page.pk')), nullable=False)
    page = db.relationship('SubjectPage',
      backref=db.backref('directories', lazy='dynamic'))
...
class SubjectPage(db.Model, Timestamp):
    ...
    created_by_pk = db.Column((db.Integer), (db.ForeignKey('users.pk')), nullable=False)
    created_by = db.relationship('User',
      foreign_keys=[created_by_pk], backref=db.backref('pages_created', lazy='dynamic'))
    updated_by_pk = db.Column(db.Integer, db.ForeignKey('users.pk'))
    updated_by = db.relationship('User',
      foreign_keys=[updated_by_pk], backref=db.backref('pages_updated', lazy='dynamic'))
    description = db.Column(db.Text)
    active = db.Column((db.Boolean), default=True, index=True)
...
class ChangeRecord(db.Model):
    ...
    change_dt = db.Column((db.DateTime),
...
def log_change(obj, change_type, user, description):
    return ChangeRecord(object_pk=(obj.pk),
