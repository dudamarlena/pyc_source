...
# [GCC 8.4.0]
# Embedded file name: /home/jarek/work/bip/src/bip/cli.py
# Compiled at: 2019-09-28 15:02:38
# Size of source mod 2**32: 11076 bytes
import difflib, sys, click, keyring
from dotenv import find_dotenv, load_dotenv
from flask.cli import FlaskGroup, with_appcontext
import flask_migrate.cli as migrate_ops
...
@click.group(cls=FlaskGroup,
  ...
  help='Zarządzanie aplikacją BIP')
def cli():
    pass
...
@click.option('--password',
  '-p', prompt=True, hide_input=True, required=True, help='Hasło użytkownika')
@click.option('--clear',
  '-c', is_flag=True, default=False, help='Wyczyść dane logowania (domyślnie: NIE)')
def user_login(user, password, clear):
    ...
    if not (user_obj and pwd_context.verify(password, user_obj.password)):
        raise click.ClickException('nieprawidłowe dane logowania - nie znaleziono użytkownika lub nieprawidłowe hasło')
    if clear:
...
@click.option('--active/--inactive',
  default=None, help='Wyświetl tylko aktywne lub nieaktywne (domyślnie: wszystkie)')
...
@click.option('--active/--inactive',
  default=False, help='Czy konto ma być od razu aktywne (domyślnie: NIE)')
@click.option('--admin/--regular',
  default=False, help='Czy konto ma mieć uprawnienia administracyjne (domyślnie: NIE)')
...
@click.option('--name',
  '-n', required=True, help='Nazwa konta użytkownika które ma zostać zmienione')
@click.option('--email',
  '-e', default=None, required=False, help='Email użytkownika (domyślnie: bez zmiany)')
@click.option('--active/--inactive',
  default=None, help='Zmiana aktywacji konta (domyślnie: bez zmiany)')
@click.option('--user',
  '-u', required=True, help='Wykonaj operację jako wskazany użytkownik')
...
@click.option('--directory/--no-directory',
  default=False, help='Czy kategoria jest katalogiem (domyślnie: NIE)')
@click.option('--active/--inactive',
  default=False, help='Czy kategoria ma być od razu aktywna (domyślnie: NIE)')
@click.option('--order',
  '-o', type=int, default=None, help='Kolejność kategorii w menu (domyślnie: bez ustalania kolejności)')
@click.option('--user',
  '-u', required=True, help='Wykonaj operację jako wskazany użytkownik')
def category_create(title, directory, active, order, user):
    user_obj = login_user(user)
    c_page = SubjectPage(title=title,
      created_by=user_obj,
      active=active,
      text=title)
    c_dir = None
    ...
    c_menuitem = ObjectMenuItem(directory=c_dir,
      page=c_page,
      title=title,
      active=active,
      menu_order=order)
...
@click.option('--description',
  '-d', is_flag=True, default=False, help='Zmień opis kategorii (domyślnie: NIE); uruchamia zdefiniowany edytor tekstu')
@click.option('--title', '-t', default=None, help='Zmień tytuł kategorii')
@click.option('--active/--inactive',
  default=None, help='Zmień stan aktywności kategorii')
@click.option('--order',
  '-o', type=int, default=None, help='Zmień kolejność kategorii w menu')
@click.option('--user',
  '-u', required=True, help='Wykonaj operację jako wskazany użytkownik')
def category_change(category, description, title, active, order, user):
    ...
    if description:
        ...
        if new_description:
            data_diff = list(difflib.unified_diff(orig_description.splitlines(keepends=True), new_description.splitlines(keepends=True)))
            if data_diff:
                diff_s = '\n'.join(data_diff)
    ...
    if title:
        ...
        if changes:
            ...
            db.session.add(log_change(cat_obj,
              (ChangeType.updated), user_obj, description=('\n'.join(changes))))
            db.session.commit()
