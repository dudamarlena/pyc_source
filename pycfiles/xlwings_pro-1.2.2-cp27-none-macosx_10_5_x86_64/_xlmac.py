...
# [GCC 8.4.0]
# Embedded file name: <xlwings_pro-1.2.2>/xlwings/_xlmac.py
# Compiled at: 2020-03-09 05:37:56
import os, re, datetime as dt, subprocess, struct, shutil, atexit, psutil, aem, appscript
...
class Books(object):
    ...
    def open(self, fullname, update_links=None, read_only=None, format=None, password=None, write_res_password=None, ignore_read_only_recommended=None, origin=None, delimiter=None, editable=None, notify=None, converter=None, add_to_mru=None, local=None, corrupt_load=None):
        ...
        self.app.xl.open_workbook(workbook_file_name=fullname, update_links=update_links, read_only=read_only, format=format, password=password, write_reserved_password=write_res_password, ignore_read_only_recommended=ignore_read_only_recommended, origin=origin, delimiter=delimiter, editable=editable, notify=notify, converter=converter, add_to_mru=add_to_mru)
...
class Range(object):
    ...
    def coords(self):
        if self._coords is None:
            self._coords = (self.xl.first_row_index.get(),
    ...
    def paste(self, paste=None, operation=None, skip_blanks=False, transpose=False):
        pastes = {'all': kw.paste_all, 
           ...
           None: None}
        operations = {'add': kw.paste_special_operation_add, 
    ...
    def add_hyperlink(self, address, text_to_display=None, screen_tip=None):
        if self.xl is not None:
            self.xl.make(at=self.xl, new=kw.hyperlink, with_properties={kw.address: address, kw.text_to_display: text_to_display, 
    ...
    def __call__(self, arg1, arg2=None):
        if arg2 is None:
            ...
            return Range(self.sheet, self.sheet.xl.rows[(self.row + arg1 - 1)].columns[(self.column + arg2 - 1)].get_address())
            return
    ...
    def rows(self):
        ...
        return [ self.sheet.range((row + i, col1), (row + i, col2)) for i in range(self.shape[0])
               ]
    ...
    def columns(self):
        ...
        sht = self.sheet
        return [ sht.range((row1, col + i), (row2, col + i)) for i in range(self.shape[1])
               ]
...
class Charts(Collection):
    ...
    def add(self, left, top, width, height):
        ...
        return Chart(self.parent, self.parent.xl.make(at=self.parent.book.xl.sheets[sheet_index], new=kw.chart_object, with_properties={kw.width: width, 
           kw.top: top, 
           kw.left_position: left, 
           kw.height: height}).name.get())
...
class Pictures(Collection):
    ...
    def add(self, filename, link_to_file, save_with_document, left, top, width, height):
        ...
        sheet_index = self.parent.xl.entry_index.get()
        picture = Picture(self.parent, self.parent.xl.make(at=self.parent.book.xl.sheets[sheet_index], new=kw.picture, with_properties={kw.file_name: posix_to_hfs_path(filename), 
...
class Names(object):
    ...
    def add(self, name, refers_to):
        return Name(self.parent, self.parent.xl.make(at=self.parent.xl, new=kw.named_item, with_properties={kw.references: refers_to, 
           kw.name: name}))
...
class Name(object):
    ...
    def refers_to_range(self):
        ...
        match = re.search("\\](.*?)'?!(.*)", external_address)
...
def hfs_to_posix_path(hfs_path):
    ...
    """
    url = mactypes.convertpathtourl(hfs_path, 1)
    return mactypes.converturltopath(url, 0)
...
def _clean_value_data_element(value, datetime_builder, empty_as, number_builder):
    ...
    if isinstance(value, dt.datetime) and datetime_builder is not dt.datetime:
        value = datetime_builder(month=value.month, day=value.day, year=value.year, hour=value.hour, minute=value.minute, second=value.second, microsecond=value.microsecond, tzinfo=None)
...
def prepare_xl_data_element(x):
    if x is None:
        ...
        if np and isinstance(x, np.number):
            return float(x)
        if pd and isinstance(x, pd.Timestamp):
            return x.to_pydatetime().replace(tzinfo=None)
        if pd and isinstance(x, type(pd.NaT)):
            return
...
chart_types_k2s = {kw.ThreeD_area: '3d_area', 
...
directions_s2k = {'d': kw.toward_the_bottom, 
...
directions_k2s = {kw.toward_the_bottom: 'down', 
   kw.toward_the_left: 'left', 
   kw.toward_the_right: 'right', 
   kw.toward_the_top: 'up'}
calculation_k2s = {kw.calculation_automatic: 'automatic', 
...
shape_types_k2s = {kw.shape_type_auto: 'auto_shape', 
