...
# [GCC 8.4.0]
# Embedded file name: <xlwings_pro-1.2.2>/xlwings/conversion/framework.py
...
class Converter(Accessor):
    ...
    @classmethod
    def reader(cls, options):
        return cls.base_reader(options).append_stage(cls.FromValueStage(cls.read_value, options))

    @classmethod
    def writer(cls, options):
        return cls.base_writer(options).prepend_stage(cls.ToValueStage(cls.write_value, options))