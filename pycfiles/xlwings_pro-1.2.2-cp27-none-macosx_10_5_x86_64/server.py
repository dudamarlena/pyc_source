...
# [GCC 8.4.0]
# Embedded file name: <xlwings_pro-1.2.2>/xlwings/server.py
...
import logging, sys, os, collections
cwd = os.getcwd()
if not hasattr(sys, 'frozen'):
    os.chdir(sys.exec_prefix)
import win32api, win32event
os.chdir(cwd)
import pythoncom, pywintypes, win32com.client, win32com.server.util as serverutil, win32com.server.dispatcher, win32com.server.policy
...
class XLPythonEnumerator:
    ...
    def Skip(self, count):
        raise win32com.server.exception.COMException(scode=2147500033)
...
class XLPython(object):
    _public_methods_ = [
     'Module', 'Tuple', 'TupleFromArray', 'Dict', 'DictFromArray', 'List', 'ListFromArray', 'Obj',
...
def serve(clsid='{506e67c3-55b5-48c3-a035-eed5deea7d6d}'):
    ...
    while True:
        ...
        rc = win32event.MsgWaitForMultipleObjects(waitables, 0, timeout, win32event.QS_ALLEVENTS)
