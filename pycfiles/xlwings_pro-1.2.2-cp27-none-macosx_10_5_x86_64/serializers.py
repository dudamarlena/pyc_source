...
# [GCC 8.4.0]
# Embedded file name: <xlwings_pro-1.2.2>/xlwings/rest/serializers.py
# Compiled at: 2020-03-09 05:37:56


def serialize_app(app):
    return {'version': str(app.version), 'visible': app.visible, 
       ...
       'books': [ book.fullname for book in app.books ], 'pid': app.pid}


def serialize_book(book):
    return {'app': book.app.pid, 
       'name': book.name, 
       'sheets': [ sheet.name for sheet in book.sheets ], 'fullname': book.fullname, 
       'names': [ name.name for name in book.names ], 'selection': book.selection.get_address(include_sheetname=True) if book.selection else None}


def serialize_sheet(sheet):
    return {'name': sheet.name, 
       'names': [ name.name for name in sheet.names ], 'charts': [ chart.name for chart in sheet.charts ], 'shapes': [ shape.name for shape in sheet.shapes ], 'pictures': [ picture.name for picture in sheet.pictures ], 'used_range': sheet.used_range.address}
