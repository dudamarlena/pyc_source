...
# [GCC 8.4.0]
# Embedded file name: <xlwings_pro-1.2.2>/xlwings/udfs.py
# Compiled at: 2020-03-09 05:37:56
import os, os.path, sys, re, time, tempfile, inspect, glob, shutil
from importlib import import_module
from threading import Thread
from win32com.client import Dispatch, CDispatch
from . import conversion, xlplatform, Range, apps, PY3, Book
from .utils import VBAWriter, dump_embedded_code
cache = {}
...
class AsyncThread(Thread):
    ...
    def run(self):
        ...
        if self.expand:
            apps[self.pid].books[self.book].sheets[self.sheet][self.address].formula_array = apps[self.pid].books[self.book].sheets[self.sheet][self.address].formula_array
        else:
            apps[self.pid].books[self.book].sheets[self.sheet][self.address].formula = apps[self.pid].books[self.book].sheets[self.sheet][self.address].formula
...
if PY3:
    ...
    def func_sig(f):
        ...
        if s.keywords:
            raise Exception('xlwings does not support UDFs with keyword arguments')
        return {'args': s.args + [s.varargs] if s.varargs else s.args, 'defaults': s.defaults or [], 
           'vararg': s.varargs}
...
def xlfunc(f=None, **kwargs):
    ...
    def inner(f):
        if not hasattr(f, '__xlfunc__'):
            ...
            xlf['ret'] = {'doc': f.__doc__ if f.__doc__ is not None else "Python function '" + f.__name__ + "' defined in '" + str(f.__code__.co_filename) + "'.", 'options': {}}
...
try:
    for subdir in glob.glob(tmp_base_path + '/*/'):
        if os.path.getmtime(subdir) < time.time() - 2592000:
...
def get_udf_module(module_name, xl_workbook):
    ...
    if module_info is not None:
        ...
        udf_modules[module_name] = {'filename': filename, 
           'filetime': mtime, 
           'module': module}
    return module
...
def get_cache_key(func, args, caller):
    ...
    return func.__name__ + str(args) + str(xw_caller.sheet.book.app.pid) + xw_caller.sheet.book.name + xw_caller.sheet.name + xw_caller.address.split(':')[0]
...
def call_udf(module_name, func_name, args, this_workbook=None, caller=None):
    ...
    if writing and writing == caller.Address:
        ...
        for i, arg in enumerate(args):
            ...
            if type(arg) is int and arg == -2147352572:
        ...
        if func_info['async_mode'] and func_info['async_mode'] == 'threading':
            ...
            if cached_value is not None:
                ...
                xw_caller = Range(impl=xlplatform.Range(xl=caller))
                thread = AsyncThread(xw_caller.sheet.book.app.pid, xw_caller.sheet.book.name, xw_caller.sheet.name, xw_caller.address, func, args, cache_key, is_dynamic_array)
                thread.start()
        ...
        if is_dynamic_array:
            current_size = (
             caller.Rows.Count, caller.Columns.Count)
            result_size = (1, 1)
            ...
            if current_size != result_size:
                ...
                add_idle_task(DelayedResizeDynamicArrayFormula(Range(impl=xlplatform.Range(xl=caller)).resize(*result_size), caller, current_size[0] > result_size[0] or current_size[1] > result_size[1]))
            else:
                del cache[cache_key]
        return xl_result
            ...
            with vba.block(func_sig):
                ...
                if ftype == 'Sub':
                    with vba.block('#If App = "Microsoft Excel" Then'):
                        vba.writeln('Py.CallUDF "{module_name}", "{fname}", {args_vba}, ThisWorkbook, Application.Caller', module_name=module_name, fname=fname, args_vba=args_vba)
                    with vba.block('#Else'):
                        vba.writeln('Py.CallUDF "{module_name}", "{fname}", {args_vba}', module_name=module_name, fname=fname, args_vba=args_vba)
                    vba.writeln('#End If')
                else:
                    with vba.block('#If App = "Microsoft Excel" Then'):
                        vba.writeln('If TypeOf Application.Caller Is Range Then On Error GoTo failed')
                        vba.writeln('{fname} = Py.CallUDF("{module_name}", "{fname}", {args_vba}, ThisWorkbook, Application.Caller)', module_name=module_name, fname=fname, args_vba=args_vba)
                        vba.writeln('Exit ' + ftype)
                    with vba.block('#Else'):
                        vba.writeln('{fname} = Py.CallUDF("{module_name}", "{fname}", {args_vba})', module_name=module_name, fname=fname, args_vba=args_vba)
                        vba.writeln('Exit ' + ftype)
                    vba.writeln('#End If')
        ...
        for mvar in map(lambda attr: getattr(module, attr), dir(module)):
            if hasattr(mvar, '__xlfunc__'):
                ...
                if excel_version[0] >= 14:
                    ...
                    xl_workbook.Application.MacroOptions("'" + xl_workbook.Name + "'!" + fname, Description=fdoc, HasMenu=False, MenuText=None, HasShortcutKey=False, ShortcutKey=None, Category=fcategory, StatusBar=None, HelpContextID=None, HelpFile=None, ArgumentDescriptions=argdocs if argdocs else None)
                else:
