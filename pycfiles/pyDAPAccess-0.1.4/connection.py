...
# Embedded file name: build/bdist.macosx-10.7-intel/egg/pydap/handlers/sql/connection.py
# Compiled at: 2012-04-02 16:31:56
import urllib
...
def _conn_pgsql(location):
    ...
    if host:
        conn_str.append(('host', host))
    if port:
        conn_str.append(('port', str(port)))
    if user:
        conn_str.append(('user', user))
    if passwd:
        conn_str.append(('password', passwd))
...
def _conn_mssql(location):
    ...
    conn_str = [('Driver', '{SQL Server}'),
     (
      'Database', db)]
    if host:
        conn_str.append(('Server', host))
    if port:
        conn_str.append(('Port', str(port)))
    if user:
        conn_str.append(('Uid', user))
    if passwd:
        conn_str.append(('Pwd', passwd))
    conn_str = [ ('=').join(t) for t in conn_str ]
    conn_str = (';').join(conn_str)
    conn = connect(conn_str)
    return conn
