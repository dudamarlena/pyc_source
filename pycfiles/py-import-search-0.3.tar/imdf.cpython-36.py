...
# Embedded file name: /Users/lucas.selfslagh/anaconda3/lib/python3.6/site-packages/pyimdf/imdf.py
# Compiled at: 2019-06-30 09:46:10
# Size of source mod 2**32: 9877 bytes
import uuid, json
...
class Level:
    ...
    def __init__(self, data, building, verbose):
        ...
        self.dictionary = self.as_dict(verbose)
        print(f"===> ✨ NEW LEVEL: {self.ordinal} FOR BUILDING {self.building}\n")
        if self.ordinal == -1:
...
class Footprint:
    ...
    def as_dict(self, verbose):
        footprint_collection = {'type':'FeatureCollection', 
         'features':[]}
        footprint = {'id':self.uid, 
         'type':'Feature', 
        ...
        dict = {'type':'FeatureCollection', 
         'features':[
          {'id':self.uid, 
           ...
           'geometry':{'type':'Polygon', 
            'coordinates':self.geometry}, 
           'properties':{'category':self.props['category'], 
            'restriction':self.props['restrictio'], 
            'name':{'en': self.props['name']}, 
            'alt_name':{'en': self.props['alt_name']}, 
