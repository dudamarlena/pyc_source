...
# [GCC 8.4.0]
# Embedded file name: att/Network/Encrypter.py
...
def tobinary16(i):
    return unhexlify(('{:04x}').format(i & 65535))
...
class Connection(object):
    ...
    def __init__(self, Encoder, connection, id, ext_handshake=False, encrypted=None, options=None):
        ...
        if self.locally_initiated:
            ...
            if encrypted:
                ...
                self.write(chr(len(protocol_name)) + protocol_name + option_pattern + self.Encoder.download_id)
    ...
    def read_header(self, s):
        if self._read_header(s):
            ...
            if self.locally_initiated and not self.encrypted:
                return None
            if not self.Encoder.config['crypto_allowed']:
                return None
    ...
    def read_crypto_header(self, s):
        ...
        if self.locally_initiated:
            if self.Encoder.config['crypto_only']:
                cryptmode = '\x00\x00\x00\x02'
            else:
                cryptmode = '\x00\x00\x00\x03'
            padc = self.encrypter.padding()
            self.write(self.encrypter.block3a + self.encrypter.block3b + self.encrypter.encrypt('\x00\x00\x00\x00\x00\x00\x00\x00' + cryptmode + tobinary16(len(padc)) + padc + '\x00\x00'))
            self._max_search = 520
            return (
             1, self.read_crypto_block4a)
        self.write(self.encrypter.pubkey + self.encrypter.padding())
        self._max_search = 520
    ...
    def read_crypto_block3a(self, s):
        ...
        return (
         len(self.encrypter.block3b), self.read_crypto_block3b)
    ...
    def read_crypto_block3c(self, s):
        if s[:8] != '\x00\x00\x00\x00\x00\x00\x00\x00':
            return None
        else:
            ...
            if padlen > 512:
                return None
            return (
             padlen + 2, self.read_crypto_pad3)
    ...
    def read_crypto_pad3(self, s):
        ...
        if ialen > 65535:
            ...
            if self.cryptmode == 1:
                cryptmode = '\x00\x00\x00\x01'
            else:
                cryptmode = '\x00\x00\x00\x02'
            padd = self.encrypter.padding()
            self.write('\x00\x00\x00\x00\x00\x00\x00\x00' + cryptmode + tobinary16(len(padd)) + padd)
            if ialen:
    ...
    def read_crypto_block3done(self, ia=''):
        ...
        return (
         1 + len(protocol_name), self.read_encrypted_header)
    ...
    def read_download_id(self, s):
        ...
        if not self.locally_initiated:
            if not self.encrypted:
                self.Encoder.connecter.external_connection_made += 1
            self.write(chr(len(protocol_name)) + protocol_name + option_pattern + self.Encoder.download_id + self.Encoder.my_id)
        return (
         20, self.read_peer_id)
...
class Encoder(object):
    ...
    def __init__(self, connecter, raw_server, my_id, max_len, schedulefunc, keepalive_delay, download_id, measurefunc, config, bans=_dummy_banlist()):
    ...
    def start_connection(self, dns, id, encrypted=None):
        if self.paused or len(self.connections) >= self.max_connections or id == self.my_id or not self.check_ip(ip=dns[0]):
            return True
    ...
    def externally_handshaked_connection_made(self, connection, options, already_read, encrypted=None):
        if self.paused or len(self.connections) >= self.max_connections or not self.check_ip(connection=connection):
            connection.close()
            return False
        else:
            con = Connection(self, connection, None, ext_handshake=True, encrypted=encrypted, options=options)
            self.connections[connection] = con
            connection.set_handler(con)
            if already_read:
