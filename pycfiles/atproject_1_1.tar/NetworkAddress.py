...
# [GCC 8.4.0]
# Embedded file name: att/Network/NetworkAddress.py
# Compiled at: 2017-03-18 13:15:28
...
ranges.
"""
import struct, socket, bisect, operator, win_inet_pton
...
class Address(long):
    ...
    def __str__(self):
        ...
        words = (4294967295 & self >> i for i in xrange(self.bits - 32, -1, -32))
    ...
    def fromString(cls, address):
        ...
        shiftword = lambda x, y: 4294967296 * x + y
        structdesc = ('>{:d}L').format(cls.bits / 32)
        return cls(reduce(shiftword, struct.unpack(structdesc, socket.inet_pton(cls.af, address))))
    ...
    def __add__(self, addr):
        if isinstance(addr, AddressRange):
            if addr.start > self.end + 1:
                ...
                if self.start > addr.end + 1:
                    return (addr, self)
                return AddressRange(min(self.start, addr.start), max(self.end, addr.end))

        if addr > self.end + 1:
            return (self, AddressRange(addr))
        else:
            if self.start > addr + 1:
