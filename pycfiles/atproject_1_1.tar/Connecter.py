...
# [GCC 8.4.0]
# Embedded file name: att/Client/Connecter.py
...
def tobinary(i):
    return unhexlify(('{:08x}').format(i & 4294967295))
...
class Connection:
    ...
    def send_request(self, index, begin, length):
        ...
        if DEBUG1:
            print (
             self.ccount, 'sent request', index, begin, begin + length)

    def send_cancel(self, index, begin, length):
        self._send_message(CANCEL + tobinary(index) + tobinary(begin) + tobinary(length))
        if DEBUG1:
            print (
             self.ccount, 'sent cancel', index, begin, begin + length)
    ...
    def _send_message(self, s):
        if DEBUG2:
            if s:
                print (
                 self.ccount, 'SENDING MESSAGE', ord(s[0]), len(s))
            else:
                print (
                 self.ccount, 'SENDING MESSAGE', -1, 0)
        s = tobinary(len(s)) + s
        if self.partial_message:
    ...
    def send_partial(self, bytes):
        if self.connection.closed:
            ...
            if self.partial_message is None:
                ...
                if DEBUG1:
                    print (
                     self.ccount, 'sending chunk', index, begin,
                     begin + len(piece))
...
class Connecter:
    ...
    def __init__(self, make_upload, downloader, choker, numpieces, totalup, config, ratelimiter, sched=None):
    ...
    def connection_flushed(self, connection):
        ...
        if conn.next_upload is None and (conn.partial_message is not None or len(conn.upload.buffer) > 0):
    ...
    def got_message(self, connection, message):
        ...
        if DEBUG2:
            print (
             c.ccount, 'message received', ord(t))
        if t == BITFIELD and c.got_anything:
            if DEBUG2:
                print (
                 c.ccount, 'misplaced bitfield')
            connection.close()
            return
        c.got_anything = True
        if t in [CHOKE, UNCHOKE, INTERESTED, NOT_INTERESTED] and len(message) != 1:
            if DEBUG2:
                print (
                 c.ccount, 'bad message length')
            connection.close()
            return
        if t == CHOKE:
