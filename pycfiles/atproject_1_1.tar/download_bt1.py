...
# [GCC 8.4.0]
# Embedded file name: /Users/Yifei/Desktop/atproject/atproject/Client/download_bt1.py
# Compiled at: 2017-04-23 16:34:12
import os, socket, threading
from atproject.Meta.Info import check_type, check_info
from atproject.Network.zurllib import urlopen
from urlparse import urlparse
from .Choker import Choker
from atproject.Storage.Storage import Storage
from atproject.Storage.StorageWrapper import StorageWrapper
from atproject.Storage.FileSelector import FileSelector
from .Uploader import Upload
from .Downloader import Downloader
from .HTTPDownloader import HTTPDownloader
from .Connecter import Connecter
from .RateLimiter import RateLimiter
from atproject.Network.Encrypter import Encoder
from atproject.Network.RawServer import autodetect_socket_style
...
from atproject.Application.parseargs import parseargs, formatDefinitions, defaultargs
...
defaults = [
 ...
 ('keepalive_interval', 120.0, 'number of seconds to pause between sending keepalives'),
 (
  'download_slice_size', 16384,
  'How many bytes to query for per request.'),
 ('upload_unit_size', 1460, 'when limiting upload rate, how many bytes to send at a time'),
 ('request_backlog', 10, 'maximum number of requests to keep in a single pipe at once.'),
 (
 ...
 ('random_port', 1, 'whether to choose randomly inside the port range instead of counting up linearly'),
 ...
 ('crypto_only', 0, 'whether to only create or allow encrypted connections'),
 ('crypto_stealth', 0, 'whether to prevent all non-encrypted connection attempts; will result in an effectively firewalled state on older trackers'),
 ('selector_enabled', 1, 'whether to enable the file selector and fast resume function'),
 ('expire_cache_data', 10, 'the number of days after which you wish to expire old cache data (0 = disabled)'),
 ('priority', '', 'a list of file priorities separated by commas, must be one per file, 0 = highest, 1 = normal, 2 = lowest, -1 = download disabled'),
 ('saveas', '', 'local file name to save the file as, null indicates query user'),
 ('timeout', 300.0, 'time to wait between closing sockets which nothing has been received on'),
 ('timeout_check_interval', 60.0, 'time to wait between checking if any connections have timed out'),
 (
 ...
 ('upnp_nat_access', 1, 'attempt to autoconfigure a UPnP router to forward a server port (0 = disabled, 1 = mode 1 [fast], 2 = mode 2 [slow])'),
 ('upload_rate_fudge', 5.0, 'time equivalent of writing to kernel-level TCP buffer, for rate adjustment'),
 ('tcp_ack_fudge', 0.03, 'how much TCP ACK download overhead to add to upload rate calculations (0 = disabled)'),
 ('display_interval', 0.5, 'time between updates of displayed information'),
 (
  'rerequest_interval', 300,
  'time to wait between requesting more peers'),
 ('min_peers', 20, 'minimum number of peers to not do rerequesting'),
 ('http_timeout', 60, 'number of seconds to wait before assuming that an http connection hastimed out'),
 ('max_initiate', 40, 'number of peers at which to stop initiating new connections'),
 ('check_hashes', 1, 'whether to check hashes on disk'),
 ('max_upload_rate', 0, 'maximum kB/s to upload at (0 = no limit, -1 = automatic)'),
 ('max_download_rate', 0, 'maximum kB/s to download at (0 = no limit)'),
 ('alloc_type', 'normal', 'allocation type (may be normal, background, pre-allocate or sparse)'),
 ('alloc_rate', 2.0, 'rate (in MiB/s) to allocate space at using background allocation'),
 ('buffer_reads', 1, 'whether to buffer disk reads'),
 ('write_buffer_size', 4, 'the maximum amount of space to use for buffering disk writes (in megabytes, 0 = disabled)'),
 ('breakup_seed_bitfield', 1, 'sends an incomplete bitfield and then fills with have messages, in order to get around stupid ISP manipulation'),
 ('snub_time', 30.0, "seconds to wait for data to come in over a connection before assumingit's semi-permanently choked"),
 ('spew', 0, 'whether to display diagnostic info to stdout'),
 ('rarest_first_cutoff', 2, 'number of downloads at which to switch from random to rarest first'),
 ('rarest_first_priority_cutoff', 5, 'the number of peers which need to have a piece before other partials take priority over rarest first'),
 ('min_uploads', 4, 'the number of uploads to fill out to with extra optimistic unchokes'),
 ('max_files_open', 50, 'the maximum number of files to keep open at a time, 0 means no limit'),
 ('round_robin_period', 30, "the number of seconds between the client's switching upload targets"),
 ('super_seeder', 0, 'whether to use special upload-efficiency-maximizing routines (only for dedicated seeds)'),
 ('security', 1, 'whether to enable extra security features intended to prevent abuse'),
 ('max_connections', 0, 'the absolute maximum number of peers to connect with (0 = no limit)'),
 ('auto_kick', 1, 'whether to allow the client to automatically kick/ban peers that send bad data'),
 ('double_check', 1, 'whether to double-check data being written to the disk for errors (may increase CPU load)'),
 ('triple_check', 0, 'whether to thoroughly check data being written to the disk (may slowdisk access)'),
 ('lock_files', 1, 'whether to lock files the client is working with'),
 ('lock_while_reading', 0, 'whether to lock access to files being read'),
 ('auto_flush', 0, 'minutes between automatic flushes to disk (0 = disabled)'),
 ('dedicated_seed_id', '', 'code to send to tracker identifying as a dedicated seed')]
...
class BT1Download:
    ...
    def __init__(self, statusfunc, finfunc, errorfunc, excfunc, doneflag, config, response, infohash, id, rawserver, port, appdataobj=None):
        ...
        self.checking = False
        self.started = False
        self.picker = PiecePicker(self.len_pieces, config['rarest_first_cutoff'], config['rarest_first_priority_cutoff'])
    ...
    def checkSaveLocation(self, loc):
        ...
        return any(os.path.exists(os.path.join(loc, x['path'][0])) for x in self.info['files'])
    ...
    def saveAs(self, filefunc, pathfunc=None):
        try:
            ...
            if 'length' in self.info:
                ...
                file = filefunc(self.info['name'], file_length, self.config['saveas'], False)
                if file is None:
                    return
                make(file)
                ...
                file = filefunc(self.info['name'], file_length, self.config['saveas'], True)
                if file is None:
                    return
                existing = 0
                if os.path.exists(file):
                    ...
                    if len(os.listdir(file)) > 0:
                        existing = any(os.path.exists(os.path.join(file, x['path'][0])) for x in self.info['files'])
                        if not existing:
    ...
    def _data_flunked(self, amount, index):
        ...
        if not self.doneflag.isSet():
            self.errorfunc(('piece {:d} failed hash check, re-downloading it').format(index))
    ...
    def initFiles(self, old_style=False, statusfunc=None):
        if self.doneflag.isSet():
            ...
            try:
                try:
                    self.storage = Storage(self.files, self.info['piece length'], self.doneflag, self.config, disabled_files)
                except IOError as e:
                ...
                if self.doneflag.isSet():
                    return
                self.storagewrapper = StorageWrapper(self.storage, self.config['download_slice_size'], self.pieces, self.info['piece length'], self._finished, self._failed, statusfunc, self.doneflag, self.config['check_hashes'], self._data_flunked, self.rawserver.add_task, self.config, self.unpauseflag)
            except ValueError as e:
            ...
            if self.doneflag.isSet():
                return
            if self.selector_enabled:
                self.fileselector = FileSelector(self.files, self.info['piece length'], self.appdataobj.getPieceDir(self.infohash), self.storage, self.storagewrapper, self.rawserver.add_task, self._failed)
                if data:
    ...
    def startEngine(self, ratelimiter=None, statusfunc=None):
        ...
        self.downloader = Downloader(self.storagewrapper, self.picker, self.config['request_backlog'], self.config['max_rate_period'], self.len_pieces, self.config['download_slice_size'], self._received_data, self.config['snub_time'], self.config['auto_kick'], self._kick_peer, self._ban_peer)
        self.downloader.set_download_rate(self.config['max_download_rate'])
        self.connecter = Connecter(self._make_upload, self.downloader, self.choker, self.len_pieces, self.upmeasure, self.config, self.ratelimiter, self.rawserver.add_task)
        self.encoder = Encoder(self.connecter, self.rawserver, self.myid, self.config['max_message_length'], self.rawserver.add_task, self.config['keepalive_interval'], self.infohash, self._received_raw_data, self.config)
        self.encoder_ban = self.encoder.ban
        self.httpdownloader = HTTPDownloader(self.storagewrapper, self.picker, self.rawserver, self.finflag, self.errorfunc, self.downloader, self.config['max_rate_period'], self.infohash, self._received_http_data, self.connecter.got_piece)
        ...
        if self.selector_enabled:
            self.fileselector.tie_in(self.picker, self._cancelfunc, self._reqmorefunc, self.rerequest_ondownloadmore)
            if self.priority:
    ...
    def startRerequester(self, seededfunc=None, force_rapid_update=False):
        ...
        self.rerequest = Rerequester(self.port, self.myid, self.infohash, trackerlist, self.config, self.rawserver.add_task, self.rawserver.add_task, self.errorfunc, self.excfunc, self.encoder.start_connections, self.connecter.how_many_connections, self.storagewrapper.get_amount_left, self.upmeasure.get_total, self.downmeasure.get_total, self.upmeasure.get_rate, self.downmeasure.get_rate, self.doneflag, self.unpauseflag, seededfunc, force_rapid_update)
        self.rerequest.start()

    def _init_stats(self):
        self.statistics = Statistics(self.upmeasure, self.downmeasure, self.connecter, self.httpdownloader, self.ratelimiter, self.rerequest_lastfailed, self.filedatflag)
    ...
    def autoStats(self, displayfunc=None):
        ...
        self._init_stats()
        DownloaderFeedback(self.choker, self.httpdownloader, self.rawserver.add_task, self.upmeasure.get_rate, self.downmeasure.get_rate, self.ratemeasure, self.storagewrapper.get_stats, self.datalength, self.finflag, self.spewflag, self.statistics, displayfunc, self.config['display_interval'])

    def startStats(self):
        self._init_stats()
        d = DownloaderFeedback(self.choker, self.httpdownloader, self.rawserver.add_task, self.upmeasure.get_rate, self.downmeasure.get_rate, self.ratemeasure, self.storagewrapper.get_stats, self.datalength, self.finflag, self.spewflag, self.statistics)
        return d.gather
