...
# [GCC 8.4.0]
# Embedded file name: att/Meta/Info.py
...
def get_piece_len(size):
    ...
    """
    if size > 8 * 1073741824:
        piece_len_exp = 21
    elif size > 2 * 1073741824:
        piece_len_exp = 20
    elif size > 512 * 1048576:
        piece_len_exp = 19
    elif size > 64 * 1048576:
        piece_len_exp = 18
    elif size > 16 * 1048576:
        piece_len_exp = 17
    elif size > 4 * 1048576:
        piece_len_exp = 16
    else:
...
def check_info(info):
    ...
    check_type(info.get('pieces'), str, berr + 'bad pieces key', lambda x: len(x) % 20 != 0)
    check_type(info.get('piece length'), (int, long), berr + 'illegal piece length', lambda x: x <= 0)
    name = info.get('name')
    ...
    if 'length' in info:
        check_type(info['length'], (int, long), berr + 'bad length', lambda x: x < 0)
    else:
        ...
        for finfo in files:
            ...
            check_type(finfo.get('length'), (int, long), berr + 'bad length', lambda x: x < 0)
            path = finfo.get('path')
            ...
            for directory in path:
                ...
                if not valid_name.match(directory):
                    raise ValueError(('path {} disallowed for security reasons').format(directory))
...
class PieceHasher(object):
    ...
    def update(self, data, progress=lambda x: None):
        ...
        is optional
        """
        tofinish = self.pieceLength - self.done
        ...
        if remainder:
            ...
            hashes = [ self._hashtype(remainder[i:i + self.pieceLength]) for i in xrange(0, toHash, self.pieceLength)
                     ]
            progress(toHash)
    ...
    def __repr__(self):
        return ('<PieceHasher[{:d}] ({})>').format(len(self.pieces), self._hash.hexdigest())
...
class Info(dict):
    ...
    def __init__(self, name, size=None, progress=lambda x: None, progress_percent=False, **params):
        """
        Parameters
        ...
        if encoding == 'UTF-8':
            self.encode = lambda x: x if isinstance(x, str) else x.encode('utf-8')
            self.decode = lambda x: x.decode('utf-8')
        else:
            self.encode = lambda x: x if isinstance(x, str) else unicode(x, encoding).encode('utf-8')
        ...
        if 'pieces' in params:
            ...
            self.hasher.pieces = [ pieces[i:i + 20] for i in xrange(0, len(pieces), 20)
                                 ]
    ...
    def add_file_info(self, size, path):
        ...
        """
        self._files.append({'length': size, 'path': self._uniconvert(path)})
...
class MetaInfo(dict):
    ...
    validKeys = set(('info', 'announce', 'creation date', 'comment', 'announce-list',
                     'httpseeds'))
    ...
    def __init__(self, **params):
        ...
        if real_announce_list:
            ...
            self['announce-list'] = [ tier.split(',') for tier in announce_list.split('|') ]
        if real_httpseeds:
        ...
        super(MetaInfo, self).__init__((k, v) for k, v in params.iteritems() if k in self.validKeys and v != '')
        return
    ...
    def update(self, itr=None, **params):
        ...
        super(MetaInfo, self).update((key, params[key]) for key in params if key in self.validKeys)
        return
