...
# [GCC 8.4.0]
# Embedded file name: att/bitfield.py
...
def _int_to_booleans(integer):
    ...
    in an integer
    """
    return tuple(bool(integer << nbits & 128) for nbits in xrange(8))
...
class Bitfield(list):
    ...
    def __str__(self):
        ...
        return ('').join(BITCHARMAP[tuple(bits[x:x + 8])] for x in xrange(0, len(bits), 8))
...
def test_bitfield():
    ...
    assert _test_exception(ValueError, Bitfield, 9, chr(0) + chr(64))
    assert str(Bitfield(0, '')) == ''
    assert str(Bitfield(1, chr(128))) == chr(128)
    assert str(Bitfield(7, chr(2))) == chr(2)
    assert str(Bitfield(8, chr(255))) == chr(255)
    assert str(Bitfield(9, chr(0) + chr(128))) == chr(0) + chr(128)
    testx = Bitfield(1)
    ...
    testx[0] = 1
    assert testx.numfalse == 0
    assert str(testx) == chr(128)
    testx = Bitfield(7)
    assert len(testx) == 7
    testx[6] = 1
    assert testx.numfalse == 6
    assert str(testx) == chr(2)
    testx = Bitfield(8)
    testx[7] = 1
    ...
    assert str(testx) == chr(0) + chr(128)
    testx = Bitfield(8, chr(196))
    assert len(testx) == 8
    assert testx.numfalse == 5
    assert str(testx) == chr(196)