...
# [GCC 8.4.0]
# Embedded file name: att/Client/Downloader.py
# Compiled at: 2017-03-18 13:15:00
import random
...
class SingleDownload():
    ...
    def _backlog(self, just_unchoked):
        self.backlog = min(2 + int(4 * self.measure.get_rate() / self.downloader.chunksize), 2 * just_unchoked + self.downloader.queue_limit())
        if self.backlog > 50:
                ...
                if d is not self:
                    if d.interested:
                        if d.choked:
                            ...
                            try:
                                d.active_requests.remove((index, begin,
                                 length))
                            except ValueError:
                                continue
    ...
    def _request_more(self, new_unchoke=False):
        ...
        if self.downloader.endgamemode:
            ...
            while len(self.active_requests) < self.backlog:
                interest = self.downloader.picker.next(self.have, self.downloader.storage.do_I_have_requests, self.downloader.too_many_partials())
                if interest is None:
                    break
            ...
            if lost_interests:
                for d in self.downloader.downloads:
                    ...
                    interest = self.downloader.picker.next(d.have, self.downloader.storage.do_I_have_requests, self.downloader.too_many_partials())
                    if interest is None:
    ...
    def _check_interests(self):
        if self.interested or self.downloader.paused:
            return
        for i, have in enumerate(self.have):
            if have and not self.downloader.picker.is_blocked(i) and (self.downloader.endgamemode or self.downloader.storage.do_I_have_requests(i)):
                self.send_interested()
                return
    ...
    def is_snubbed(self):
        if self.interested and not self.choked and clock() - self.last2 > self.downloader.snub_time:
            for index, begin, length in self.active_requests:
...
class Downloader():
    ...
    def __init__(self, storage, picker, backlog, max_rate_period, numpieces, chunksize, measurefunc, snub_time, kickbans_ok, kickfunc, banfunc):
    ...
    def start_endgame(self):
        ...
        assert not self.all_requests
        for d in self.downloads:
            assert d.active_requests and d.interested and not d.choked
