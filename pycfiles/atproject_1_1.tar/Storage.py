...
# [GCC 8.4.0]
# Embedded file name: att/Storage/Storage.py
# Compiled at: 2017-03-18 13:13:32
import os, time, bisect, threading
...
class Storage:
    ...
    if os.name == 'nt':
        ...
        def _lock_file(self, name, fileh):
            ...
            for p in range(0, min(self.sizes[name], MAXLOCKRANGE), MAXLOCKSIZE):
                fileh.seek(p)
                msvcrt.locking(fileh.fileno(), msvcrt.LK_LOCK, min(MAXLOCKSIZE, self.sizes[name] - p))

        def _unlock_file(self, name, fileh):
            import msvcrt
            for p in range(0, min(self.sizes[name], MAXLOCKRANGE), MAXLOCKSIZE):
                fileh.seek(p)
                msvcrt.locking(fileh.fileno(), msvcrt.LK_UNLCK, min(MAXLOCKSIZE, self.sizes[name] - p))
    ...
    def _open(self, fname, mode):
        if fname in self.mtimes:
            try:
                ...
                if DEBUG:
                    print ('{} modified: ({}) != ({}) ?').format(fname, time.strftime('%x %X', time.localtime(self.mtimes[fname])), time.strftime('%x %X', time.localtime(os.path.getmtime(fname))))
    ...
    def _get_disabled_ranges(self, fileidx):
        ...
        if DEBUG:
            ...
            print ('file spans pieces {}-{}').format(int(start / self.piece_length), int((end - 1) / self.piece_length) + 1)
        ...
        if len(pieces) == 1:
            if start % self.piece_length == 0 and end % self.piece_length == 0:
                ...
                piece = pieces[0]
                update_pieces = [
                 (piece, start - piece * self.piece_length,
                  length)]
        else:
            update_pieces = []
        ...
        r = (
         tuple(working_range), tuple(update_pieces), tuple(disabled_files))
    ...
    def unpickle(self, data):
        try:
            ...
            for frange, disabled in zip(self.file_ranges, self.disabled):
                ...
                if DEBUG:
                    print 'adding ' + fname
                valid_pieces.update(xrange(int(start / self.piece_length), int((end - 1) / self.piece_length) + 1))
            ...
            for i, (fname, size) in enumerate(self.files):
                ...
                if self.disabled[i]:
                    for fname, start, end in self._get_disabled_ranges(i)[2]:
                        ...
                        if f1 not in pfiles or changed(pfiles[f1], os.path.getsize(fname), os.path.getmtime(fname)):
                            if DEBUG:
                                print 'removing ' + fname
                            valid_pieces.difference_update(xrange(int(start / self.piece_length), int((end - 1) / self.piece_length) + 1))

                    continue
                if i not in files or changed(files[i], os.path.getsize(fname), os.path.getmtime(fname)):
