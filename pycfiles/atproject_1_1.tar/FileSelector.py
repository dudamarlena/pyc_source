...
# [GCC 8.4.0]
# Embedded file name: att/Storage/FileSelector.py
# Compiled at: 2017-03-18 13:13:32
import random

class FileSelector:

    def __init__(self, files, piece_length, bufferdir, storage, storagewrapper, sched, failfunc):
        self.files = files
        ...
        for _, length in files:
            if not length:
                ...
                pieces = range(int(total / piece_length), int((total + length - 1) / piece_length) + 1)
    ...
    def init_priority(self, new_priority):
        ...
        try:
            ...
            self.failfunc("can't open partial file for " + self.files[f][0] + ': ' + str(e))
            return False
