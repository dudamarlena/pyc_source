...
# [GCC 8.4.0]
# Embedded file name: att/Client/PiecePicker.py
...
class PiecePicker:
    ...
    def next(self, haves, wantfunc, complete_first=False):
        ...
        if haves.complete:
            r = [
             (
              0, min(bestnum, len(self.interests)))]
        else:
            if cutoff and len(self.interests) > self.cutoff:
                r = [
                 (
                  self.cutoff, min(bestnum, len(self.interests))),
                 (
                  0, self.cutoff)]
            else:
                r = [
