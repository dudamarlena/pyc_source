...
# [GCC 8.4.0]
# Embedded file name: /Users/Yifei/Desktop/atproject/atproject/Client/Rerequester.py
# Compiled at: 2017-04-23 16:37:18
import os, time, base64, threading, socket, random, urllib, hashlib
from atproject.Network.zurllib import urlopen
from atproject.Meta.Info import check_type
...
def add_key(tracker):
    keys[tracker] = base64.urlsafe_b64encode(hashlib.sha1(basekeydata + tracker).digest()[-6:])
...
def check_peers(message):
    ...
    if isinstance(peers, list):
        for peer in peers:
            ...
            if 'peer id' in peer:
                check_type(peer.get('peer id'), str, pred=lambda x: len(x) != 20)

    elif not isinstance(peers, str) or len(peers) % 6 != 0:
        raise ValueError
    check_type(message.get('interval', 1), (int, long), pred=lambda x: x <= 0)
    check_type(message.get('min interval', 1), (int, long), pred=lambda x: x <= 0)
...
class Rerequester():
    ...
    def __init__(self, port, myid, infohash, trackerlist, config, sched, externalsched, errorfunc, excfunc, connect, howmany, amount_left, up, down, upratefunc, downratefunc, doneflag, unpauseflag=fakeflag(True), seededfunc=None, force_rapid_update=False):
        self.sched = sched
    ...
    def c(self):
        if self.stopped:
            return
        if not self.unpauseflag.isSet() and (self.howmany() < self.minpeers or self.force_rapid_update):
    ...
    def announce(self, event=3, callback=lambda : None, specialurl=None):
        if specialurl is not None:
            ...
            s = ('{}&uploaded={}&downloaded={}&left={}').format(self.url, self.up(), self.down(), self.amount_left())
    ...
    def snoop(self, peers, callback=lambda : None):
        self.rerequest(self.url + '&event=stopped&port=0&uploaded=0&downloaded=0&left=1&tracker=1&numwant=' + str(peers), callback)
    ...
    def rerequest_single(self, t, s, callback):
        l = self.lock.set()
        rq = threading.Thread(target=self._rerequest_single, args=[
         t, s + get_key(t), l, callback])
        rq.setDaemon(False)
        rq.start()
        self.lock.wait()
