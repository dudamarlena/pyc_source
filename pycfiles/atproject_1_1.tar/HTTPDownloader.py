...
# [GCC 8.4.0]
# Embedded file name: att/Client/HTTPDownloader.py
# Compiled at: 2017-03-18 13:15:00
import threading, random, httplib, urllib
...
class SingleDownload:
    ...
    def _request(self):
        import encodings.ascii, encodings.punycode, encodings.idna
        self.error = None
    ...
    def _fulfill_requests(self):
        ...
        while self.requests:
            ...
            if not self.downloader.storage.piece_came_in(self.index, begin, self.received_data[start:start + length]):
                success = False
                break
            start += length
