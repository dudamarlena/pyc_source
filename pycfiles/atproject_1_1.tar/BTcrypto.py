...
# [GCC 8.4.0]
# Embedded file name: att/Network/BTcrypto.py
...
KEY_LENGTH = 160
DH_PRIME = int('0xFFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD129024E088A67CC74020BBEA63B139B22514A08798E3404DDEF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245E485B576625E7EC6F44C42E9A63A36210000000000090563', 16)
PAD_MAX = 200
DH_BYTES = 96
...
class Crypto(object):
    ...
    def _gen_block3b(self, SKEY):
        ...
        return ('').join(chr(ord(a) ^ ord(b)) for a, b in zip(req2key, self.block3bkey))
    ...
    def VC_pattern(self):
        if not self._VC_pattern:
            self._VC_pattern = self.decrypt('\x00\x00\x00\x00\x00\x00\x00\x00')
