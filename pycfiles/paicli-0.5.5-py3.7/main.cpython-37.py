...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/paicli/main.py
# Compiled at: 2019-07-22 01:15:24
# Size of source mod 2**32: 10830 bytes
"""paicli: A CLI tool for PAI (Platform for AI).

Author: Sotetsu KOYAMADA
"""
from __future__ import print_function
import sys, click, logging, json
...
@click.command('config', help='Write your configuration to a file. File path is $HOME/.paicli')
...
@click.command(name='submit', help="Submit your job. With no json file specification, or when '-' is specified, read standard input.")
...
def submitcmd(job_config_json, profile):
    ...
    for _job_config_json in job_config_json_list:
        try:
            ...
            print(colored('Successfully submitted!', 'green') + ': {}'.format(json.loads(_job_config_json)['jobName']))
        except requests.HTTPError as e:
            try:
                status_code = e.response.status_code
                if status_code == 401:
