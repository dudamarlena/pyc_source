...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/oparse/command.py
# Compiled at: 2009-11-25 08:54:55
"""
command.py
...
"""
import os, sys, optparse, ConfigParser, pkg_resources, oparse
...
class CommandOption(dict):
    ...
    def __init__(self, options):
        for option_name in dir(options):
            if not option_name.startswith('_') and option_name not in ('ensure_value',
                                                                       'read_file',
                                                                       'read_module'):
...
class Command(object):
    ...
    def adopt_config_parameters(self, args):
        ...
        if not self.config_filename or not os.path.isfile(self.config_filename):
            return
    ...
    def __run(self, *args):
        ...
        if hasattr(self, 'parser') and isinstance(self.parser, oparse.OptionParser):
            self.parser.add_options_to_parser(parser)
            for (command_name, command_inst) in subcommand_instances.items():
                if hasattr(command_inst, 'parser') and isinstance(command_inst.parser, oparse.OptionParser) and command_inst.parser.has_options():
