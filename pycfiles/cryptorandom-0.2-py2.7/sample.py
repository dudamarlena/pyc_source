...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/cryptorandom/sample.py
...
def random_sample(a, size, replace=False, p=None, method='sample_by_index', prng=None):
    ...
    if isinstance(a, (list, np.ndarray)):
        ...
        if isinstance(a, int):
            ...
            a = np.arange(N)
            if not N > 0:
                raise AssertionError('Population size must be nonnegative')
            else:
                raise ValueError('a must be an integer or array-like')
            assert p is not None and len(p) == N
            assert replace or size <= N
        methods = {'Fisher-Yates': lambda N, n: fykd_sample(N, n, prng=prng), 
        ...
        if replace is False and p is None:
            try:
                sam = np.array(methods[method](N, size), dtype=np.int) - 1
            except ValueError:
                print 'Sampling method is incompatible with the inputs'

        elif replace is True and method in ('Fisher-Yates', 'PIKK', 'recursive', 'Waterman_R',
                                            'Vitter_Z', 'sample_by_index'):
...
def random_permutation(a, method='Fisher-Yates', prng=None):
    ...
    if isinstance(a, (list, np.ndarray)):
        ...
        if isinstance(a, int):
            ...
            raise ValueError('a must be an integer or array-like')
        methods = {'Fisher-Yates': lambda N: fykd_sample(N, N, prng=prng), 
           'random_sort': lambda N: pikk(N, N, prng=prng), 
           'permute_by_index': lambda N: sample_by_index(N, N, prng=prng)}
        try:
            sam = np.array(methods[method](N), dtype=np.int) - 1
        except ValueError:
...
def elimination_sample(k, p, replace=True, prng=None):
    ...
    """
    prng = get_prng(prng)
    weights = np.array(p).astype(float)
    if any(weights < 0):
        raise ValueError('negative item weight')
    else:
        n = len(weights)
        if replace:
            wc = np.cumsum(weights) / np.sum(weights)
