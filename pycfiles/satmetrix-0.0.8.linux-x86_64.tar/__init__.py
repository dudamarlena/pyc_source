...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/satmetrix/__init__.py
# Compiled at: 2017-09-01 08:17:50
import logging, json
...
class Satmetrix(object):
    ...
    def __search(self, _object, offset=0, limit=50, page_limit=0, callback=None, **kwargs):
        response = []
        ...
        count = 1
        logging.info('Processed %s out of %s pages, added %s rows, next url: %s', count, page_count, len(resp['data']), next)
        while next:
    ...
    def invitation(self, offset=0, limit=50, page_limit=0, callback=None, **kwargs):
        return self.__search('invitation', offset, limit, page_limit, callback, **kwargs)

    def feedback(self, offset=0, limit=50, page_limit=0, callback=None, **kwargs):
        return self.__search('feedback', offset, limit, page_limit, callback, **kwargs)