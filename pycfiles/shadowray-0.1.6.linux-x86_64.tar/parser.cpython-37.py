...
# [GCC 8.4.0]
# Embedded file name: /home/mt/PycharmProjects/Shadowray/venv/lib/python3.7/site-packages/shadowray/subscribe/parser.py
# Compiled at: 2019-06-22 22:08:55
    ...
    def get_url(self, url, **kwargs):
        ...
        for t in text:
            ...
            if t[0] == 'vmess':
                ...
                vmess_server = Configuration.ProtocolSetting.Outbound.VMess.Server(addr=(t[1]['add']), port=(int(t[1]['port'])))
                vmess_server.add_user(id=(t[1]['id']), aid=(int(t[1].get('aid', 0))),
                  security=(t[1].get('type', 'auto')),
                  level=(t[1].get('v', 0)))
                vmess.add_server(vmess_server)
                outbound.set_settings(vmess)
                stream = Configuration.StreamSetting(type=(Configuration.StreamSetting.STREAMSETTING), network=(t[1]['net']))
                outbound.set_stream(stream)
                config.add_ontbound(outbound)
                self.servers.append({'protocol':t[0], 
