...
# [GCC 8.4.0]
# Embedded file name: /home/mt/PycharmProjects/Shadowray/venv/lib/python3.7/site-packages/shadowray/__init__.py
# Compiled at: 2019-06-22 23:01:14
...
from shadowray.config.v2ray import V2RAY_BINARY, SUBSCRIBE_FILE, SERVER_FILE, V2RAY_FOLDER, RESOURCES_FOLDER, PROJECT_PATH, V2CTL_BINARY, SHADOWRAY_CONFIG_FOLDER, V2RAY_PID_FILE
from shadowray.core.manager import Manager
import requests, json, platform, zipfile, os, stat, signal
...
def create_basic_config_file():
    ...
    if os.path.exists(PROJECT_CONFIG_FILE) is False:
        ...
        f.write('{                     "v2ray_binary": null,                     "servers_file": null,                     "subscribe_file": null                     }')
        f.close()
...
def main():
    ...
    for op_name, op_value in opts:
        ...
        if op_name in ('--v2ray-update', ):
            if have_config():
                download_latest_v2ray()
            if op_name in ('--ping', ) and have_config():
                prepare_ping()