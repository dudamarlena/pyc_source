...
# [GCC 8.4.0]
# Embedded file name: /home/tomislav/dev/seveno_pyutil/build/lib/seveno_pyutil/datetime_utilities.py
# Compiled at: 2019-01-18 04:39:44
# Size of source mod 2**32: 4317 bytes
import re
from datetime import datetime, timedelta, tzinfo
import pytz
from dateutil.tz import tzoffset
_ISO_8601_OFFSET = re.compile('([+-]?)([0-9]{2})[:]?([0-9]{0,2})')
...
def timezone_or_offset(from_):
    ...
    if from_ is None:
        ...
        if offset_obj or from_.strip() in ('Z', ''):
            offset_obj = pytz.utc
    else:
        match_object = _ISO_8601_OFFSET.match(from_)
    if match_object:
        sign, hours, minutes = match_object.groups()
        offset_obj = tzoffset(name=from_,
          offset=timedelta(hours=((-1 if sign == '-' else 1) * int(hours)),
        ...
        if isinstance(from_, timedelta):
            ...
            if isinstance(from_, int):
                ...
                if issubclass(type(from_), tzinfo) or all(hasattr(from_, attr_name) for attr_name in ('dst',
                                                                                                      'fromutc',
                                                                                                      'tzname',
                                                                                                      'utcoffset')):
                    offset_obj = from_
...
def ensure_tzinfo(val, tz_or_offset='UTC', is_dst=False):
    ...
    if not isinstance(val, datetime):
        raise ValueError("Input is not datetime! ensure_tzinfo doesn't parse datetime, you need to do that beforehand.")
    else:
