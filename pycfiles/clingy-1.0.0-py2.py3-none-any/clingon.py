...
# Embedded file name: build/bdist.linux-x86_64/egg/clingon/clingon.py
# Compiled at: 2016-09-05 07:34:59
...
class Clizer(object):
        ...
        if kwargs:
            for k, v in kwargs.items():
                if not isinstance(v, Sequence):
                    raise ValueError("Decorator's keyword '%s' value must be a string or a tuple of strings, found: %s" % (k, v))
    ...
    def __init__(self, func):
        ...
        self.options = OrderedDict([ ('-' + x if len(x) == 1 else '--' + ('-').join(x.split('_')), options[x]) for x in options ])
        ...
        if DEBUG:
            print('clize default parameters:', self.reqargs + options.values() + (['*' + self.varargs] if self.varargs else []))
    ...
    def _format_option(self, option, opt_type, width=1):
        ...
        option_dict = dict(option=option, width=width, shorts=self._format_aliases(option), opt_type=self._format_type(opt_type, ' '), default='(default=%r)' % opt_type)
    ...
    def _print_usage(self, prepend='usage: ', file=sys.stdout):
        ...
        usage_dict = dict(prepend=prepend, basename=os.path.basename(self.file), reqargs=(' ').join(self.reqargs) + ' ', varargs='[varargs] ' if self.varargs else '', version='[' + (' | ').join(self._version_options) + '] ' if self._get_variable('VERSION') else '', help='[' + (' | ').join(self._help_options) + ']')
        many_options = False
        if len(self._options) == 1 and len(list(self._options)[0]) < 9:
            usage_dict.update(options='[' + self._format_option(*listitems(self._options)[0]) + '] ')
        elif len(self._options) > 0:
            usage_dict.update(options='[options] ', version='')
            many_options = True
        else:
            usage_dict.update(options='')
    ...
    def _print_help(self):
        ...
        if self.docstring:
            ...
            for doc in doc_string.split('\n'):
                doc = doc.strip()
                if len(doc) > 2:
                    print(textwrap.fill(textwrap.dedent(doc).strip(), width=80, initial_indent='  ', subsequent_indent='   '))
        ...
        if options:
            ...
            if self._get_variable('VERSION'):
                ...
                print('| ' + (' | ').join(self._version_options[1:]), 'print version', '(%s)' % self._get_variable('VERSION'))
...
def clingon_script():
    return clize(make_link=('m', 's', 'l'), force=('f', 'o'), target_path='p', target_name='n')(set_variables(VERSION=__version__)(make_script))
