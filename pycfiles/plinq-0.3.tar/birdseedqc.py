...
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/birdseedqc.py
...
iNUM_BUCKETS = 10
lstChipTypes = [
 'GenomeWideSNP_6', 'BI_SNP']
dctAnnotationFile = {'GenomeWideSNP_6': os.path.join(strMETADATA_DIR, 'GenomeWideSNP_6.na22.annot.csv'), 'BI_SNP': os.path.join(strMETADATA_DIR, 'GenomeWideSNP_5.na23.annot.csv')}
lstRequiredOptions = [
 'chip_type']
...
class FragmentLengthBucketizer(object):
    ...
    def __init__(self, strAnnotationFile):
        ...
        for lstFields in utils.parseAnnotationFile(strAnnotationFile):
            ...
            self.dctFragmentLengthBySNP[strSNP] = (
             iNspFragmentLength, iStyFragmentLength)
    ...
    def getBucketNumbers(self, strSNPName):
        ...
        return (
         calcBucketNumber(iNspFragmentLength, self.iNspBucketSize),
...
def write_table(fIntensityMean, lstBucketCounts, buckets, strSampleName, strOutputDir):
    ...
    for iRow in xrange(iNUM_BUCKETS + 1):
        print >> fOut, ('\t').join([str(iRow)] + [ str(buckets[iRow][iCol] / (fIntensityMean * lstBucketCounts[iRow][iCol])) for iCol in xrange(iNUM_BUCKETS + 1) ])
...
def main(argv=None):
    ...
    parser.add_option('-o', '--out_dir', default='.', help='Where to write output files.  Default: %default')
    ...
    lstSummarizeArgs = ['/fg/software/Affymetrix/1chip/apt-probeset-summarize.sh',
     ...
     '-o', dctOptions.out_dir] + lstArgs[1:]
