...
lstRequiredOptions = [
 'basename', 'cnv_defs', 'output', 'probe_locus']
...
def makeBirdseyeDirList(strBasename, strRootDir):
    ...
    lstDirs = [ strDir for strDir in os.listdir(strRootDir) if strDir.startswith(strDirectoryPrefix) if strDir.endswith(pre_birdseye.strBIRDSEYE_DIRECTORY_EXTENSION)
              ]
...
def makeBirdseyeMergedFileReader(strRootDir, strBasename):
    ...
    lstDirs = makeBirdseyeDirList(strBasename, strRootDir)
    return fileinput.input([ os.path.join(strRootDir, os.path.join(strDir, strBIRDSEYE_OUTPUT_FILENAME)) for strDir in lstDirs
                           ])
...
class BirdseyeOutputIterator(object):
    ...
    def advance(self):
        ...
        if self._fromCallsFile and strCurrentLine.startswith('sample'):
            strCurrentLine = self._fIn.readline()
...
class CanaryOutputIterator(object):
    ...
    def __init__(self, strCNVDefinitionsFile, strCallsFile, strConfsFile, fConfidenceThreshold, fOutputScore):
        ...
        if len(dctCNVCalls) == 0:
            ...
            iNumSamples = len(dctCNVCalls.values()[0])
        ...
        for (iSampleNum, lstCNVTuples) in enumerate(lstCNVsBySample):
            ...
            for tupCNV in lstCNVTuples:
                ...
                if len(self._lstByChromosome) < cnvDef.iChr:
                    self._lstByChromosome += [ copy.deepcopy(lstTemplateBySample) for i in xrange(cnvDef.iChr - len(self._lstByChromosome))
                                             ]
                lstTuplesForSample = self._lstByChromosome[(cnvDef.iChr - 1)][iSampleNum]
                tupToInsert = [
                 cnvDef.iStartPosn, cnvDef.iEndPosn, fConf, iCall, cnvDef.strCNVName]
                ...
                if iInsertPosn > 0 and lstTuplesForSample[(iInsertPosn - 1)][1] >= cnvDef.iStartPosn:
                    continue
                if iInsertPosn < len(lstTuplesForSample) and lstTuplesForSample[iInsertPosn][0] <= cnvDef.iEndPosn:
                    continue
...
def main(argv=None):
    ...
    parser.add_option('-b', '--basename', help='(Required)  Used to find the birdseye subdirectories.')
    parser.add_option('--input_dir', default='.', help='(Option) Parent directory of the birdseye subdirectories.  Default: current directory.\n                      If this option is used, the --input_birdseye_calls option should not be used.')
    ...
    parser.add_option('--canary_calls', help='(Required)  Canary calls file.')
    parser.add_option('--canary_confs', help='(Required)  Canary confidences file.')
    parser.add_option('--input_birdseye_calls', help='(Optional) The matrix of birdseye calls (often produced by --birdseye_calls).  If the --input_dir is specified, this is not needed.')
    parser.add_option('-o', '--output', help='(Required) Output file.')
    parser.add_option('--birdseye_calls', help='If present, all the ' + strBIRDSEYE_OUTPUT_FILENAME + ' files will be concatenated into this file, \nwith sample names added.')
    parser.add_option('--cluster_output', help='CN probe cluster file.  If present, all the CNclusters.txt files will be concatenated into this file.')
    parser.add_option('-t', '--canary_threshold', type='float', default='0.1', help='Discard canary calls > this.  Default: %default')
    parser.add_option('--canary_score', type='float', default='10.0', help='All canary calls withing canary_threshold get this score in the output.  Default: %default')
    parser.add_option('--birdseye_canary_overlap_threshold', type='int', default='5', help='If a birdseye call completely encloses a canary call, has the same copy number\nas the canary call, and is not longer (in number of CN and SNP probes spanned) by at least this amount,\nthen discard the birdseye call.  Default: %default')
    ...
    canaryIterator = CanaryOutputIterator(dctOptions.cnv_defs, dctOptions.canary_calls, dctOptions.canary_confs, dctOptions.canary_threshold, dctOptions.canary_score)
    ...
    while True:
        ...
        if iCmp < 0:
            ...
            birdseyeIterator.advance()
        elif lstCanary[iCOPY_NUMBER_INDEX] == lstBirdseye[iCOPY_NUMBER_INDEX]:
            bChangeMade = False
            ...
            if lstBirdseye[iSTART_INDEX] <= lstCanary[iSTART_INDEX]:
                if lstBirdseye[iEND_INDEX] >= lstCanary[iEND_INDEX]:
                    if abs(nCanaryProbes - nBirdseyeProbes) < dctOptions.birdseye_canary_overlap_threshold:
                        ...
                        canaryIterator.advance()
                        continue
                if lstCanary[iSTART_INDEX] <= lstBirdseye[iSTART_INDEX] and lstCanary[iEND_INDEX] >= lstBirdseye[iEND_INDEX]:
                    birdseyeIterator.advance()
                    continue
        elif lstCanary[iSTART_INDEX] <= lstBirdseye[iSTART_INDEX]:
