...
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/find_birdseye_call.py
...
import bisect, optparse, sys
from mpgutils import utils
lstRequiredOptions = [
 'sample_snp', 'snp_locus', 'tfam', 'calls']
...
def main(argv=None):
    ...
    parser.add_option('--sample_snp', help='(Required) Find the Birdseye call that produced the CN used to call this sample and SNP.\nThe first column of this file is Family ID, second column is Individual ID, third column is SNP, and the rest of the\ncolumns are ignored.  This is the format of the *.mendel file produced by mendel_check.py.')
    parser.add_option('--snp_locus', help='(Required) Locus for each SNP.')
    parser.add_option('--tfam', help='(Required) Used to determine the positional index of each individual.')
    parser.add_option('--calls', help='(Required) Birdseye calls.  These are actually a combination of Birdseye and Canary calls,\nmerged into the same format.')
    parser.add_option('-o', '--output', help='Where to write the output.  Default: stdout.')
