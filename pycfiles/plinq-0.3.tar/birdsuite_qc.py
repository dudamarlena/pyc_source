...
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/birdsuite_qc.py
...
def main(argv=None):
    ...
    parser.add_option('--cel_files_out', help='(Required) Where to write the list of cel files that pass mini-DM threshold.')
    parser.add_option('--gender_out', help='If present, write the genders for the cel files that pass mini-DM threshold to this file.')
    parser.add_option('--map_out', help='If present, write a file that maps cel file name to gender.')
    parser.add_option('--chip_type', default=strDEFAULT_CHIP_TYPE, help='Which chip type the cel files are. Default: %default')
    parser.add_option('--metadata_dir', default=strMETADATA_DIR, help='Location of metadata files.  Default: %default')
    parser.add_option('--exe_dir', default=strEXECUTABLE_DIR, help='Location of executable files.  Default: %default')
    parser.add_option('--cel_files', help="Text file specifying cel files to process, one per line\nthe first line being 'cel_files'. Default: get cel files from command line.")
    parser.add_option('-t', '--threshold', default=fMINI_DM_THRESHOLD, type='float', help='Mini-DM threshold.  Default: %default')
    parser.add_option('-s', '--strict', default=False, action='store_true', help="Require that all mini-DM scores are above threshold, instead of just the 'all' score.  Default: %default")
    parser.add_option('--force', default=False, action='store_true', help='Pass --force option to apt-geno-qc.  Default: %default')
    parser.add_option('-d', '--debug', default=False, action='store_true', help='Do not delete apt-geno-qc output file.')
    parser.add_option('-o', '--output', help='Where to write the apt-geno-qc output file.  By default this is written to a temp file\nand deleted after information if extracted from it.  Use this option to place it where you like.')
