...
# Embedded file name: build/bdist.macosx-10.6-i386/egg/analysis/calc_callrate_stats.py
# Compiled at: 2010-07-13 12:32:45
__doc__ = 'usage: %prog [options] call-file confidence-file\n\nCalculates call rate statistics from a set of calls and confidences.\n'
from __future__ import division
import sys, optparse, os.path, pickle, cPickle
...
def loadRefcalls(guideFilePath, rsFilePath, libDirPath, snps, lstSampleIndicesToProcess=None):
...
def load_guide(strGuidePath, lstSampleIndicesToProcess=None):
    f = open(strGuidePath)
    try:
        return [ strNA.strip() for (iLineNum, strNA) in enumerate(f) if lstSampleIndicesToProcess is None or iLineNum + 1 in lstSampleIndicesToProcess
               ]
    finally:
        f.close()
...
class CallRateStatsCalculator(object):
    ...
    def __init__(self, callFilePath, confFilePath, threshold=None, snpList=None, refCalls=None, bPerClusterStats=False, lstSampleIndicesToProcess=None):
        ...
        while True:
            ...
            for i in range(1, len(confs)):
                ...
                if self.bPerClusterStats:
                    assert refcall != -1 and lstReferenceClusterSizes[refcall] > 0
                    if refcall in (0, 2):
        ...
        callFile.close()
        confFile.close()
        self.sampleCallRates = ratios(self.sampleCallCounts, [ len(self.processedSnps) for x in self.processedSamples ])
        ...
        self.snpCallRates = ratios(self.snpCallCounts, [ len(self.processedSamples) for x in self.processedSnps ])
    ...
    def printPerSampleOutput(self, fPerSample):
        if not self.calcConcordance:
            self.sampleConcRates = [
             0.0] * len(self.processedSamples)
        print >> fPerSample, '%s\t%s\t%s\t%s' % ('Sample', 'CallRate', 'Concordance',
                                                 'HetPercentage')
...
def removeDuplicateSamples(lstSampleIndicesToProcess, strGuideFilePath):
    ...
    for (i, strNA) in enumerate(lstNAs):
        if i + 1 in lstSampleIndicesToProcess:
            dctIndicesBySample[strNA] = [
             i + 1] + dctIndicesBySample.get(strNA, [])
...
def main(argv=None):
    ...
    parser.add_option('-c', '--calc-concordance', action='store_true', dest='calcConcordance', default=False, help='If true, then calculate concordance statistics.\nA guide file must be supplied.  Generally useful for HapMap plates only.')
    parser.add_option('-r', '--ref-calls', dest='refCallsPath', help='Calculate concordance using the refererence calls from the given file.')
    parser.add_option('-g', '--guide-file', dest='guideFilePath', help='File containing the NA number (one per line) for each cel file on the command line.\nThe number of lines in the guide file must equal the number of cel files on the command line,\nand the order of files on the command line must agree with the order of NA numbers in the guide file.')
    strFilesRequiredInLibDir = (', ').join([RS_FILE] + [ tup[1] for tup in lstPopulations ])
    parser.add_option('-l', '--lib-dir', dest='libDir', help='Directory containing the files with genotype calls for each population,\nand the file that maps SNP names to rs numbers.  This directory must contain the following files: ' + strFilesRequiredInLibDir + '.  Default: ' + LIB_LOCATION, default=LIB_LOCATION)
    parser.add_option('-s', '--snp-file', dest='snpFilePath', help="File containing subset of SNPS to be used in evaluation.\nFile should contain one snp name per line in the first column.\nThe file can optionally contain a header with 'probeset_id' in the first line.\nDefault is to use all snps in the call and confidence files.", default=None)
    parser.add_option('--sample-file', dest='sampleFilePath', help='File containing subset of samples to be used in evaluation.\nFile should contain one sample per line.\nDefault is to use all samples in the call and confidence files.', default=None)
    parser.add_option('-t', '--threshold', dest='threshold', help='Confidence threshold for making a call.\nIf not supplied, then the values in the call file are used (-1 is a no call).', default=None)
    parser.add_option('-p', '--per-snp-output', dest='perSNPOutputPath', help='Write per-SNP call rate and concordance to this file.', default=None)
    parser.add_option('--per-sample-output', dest='perSampleOutputPath', help='Write per-sample call rate and concordance to this file.', default=None)
    parser.add_option('--hapmap-output', dest='hapMapCallsOutputPath', help='Write Hapmap calls in birdseed call format to this file.', default=None)
    parser.add_option('--per-cluster-size-output', dest='perClusterSizeOutputPath', help='Write per-cluster-size call rate and concordance to this file.', default=None)
    parser.add_option('--summary-output', dest='summaryOutputPath', help='Where to write summary output.  Default: stdout')
    parser.add_option('--skip-duplicates', dest='skipDuplicates', action='store_true', help='If an individual appears more than once in the input calls and confidences file,\n                      ignore all but the last appearance of that individual.  guide file is required to determine\n                      individual ID.  Default: False')
    parser.add_option('--ignore_missing_refcalls', action='store_true', default=False, help='When doing concordance, if there are no reference calls at all for a sample,\nemit a warning and continue instead of aborting.  Default: %default')
    ...
    confFile = lstArgs[2]
    snps = None
    if options.snpFilePath is not None:
    ...
    if options.calcConcordance:
        ...
        if options.refCallsPath is None:
            refCalls = HapMapRefCalls(options.guideFilePath, os.path.join(options.libDir, RS_FILE), options.libDir, snps, lstSampleIndicesToProcess=lstSampleIndicesToProcess)
        else:
            refCalls = AffyRefCalls(options.guideFilePath, options.refCallsPath, snps, lstSampleIndicesToProcess=lstSampleIndicesToProcess, bIgnoreMissingRefSamples=options.ignore_missing_refcalls)
    ...
    calculator = CallRateStatsCalculator(callFile, confFile, threshold, snpList=snps, refCalls=refCalls, bPerClusterStats=options.perClusterSizeOutputPath is not None, lstSampleIndicesToProcess=lstSampleIndicesToProcess)
