...
# Embedded file name: build/bdist.macosx-10.6-i386/egg/birdsuite/mendel_check.py
...
import math, operator, optparse, scipy.stats.stats, sys, calls_confidences_filter
from mpgutils import utils
lstRequiredOptions = [
 'calls', 'confs', 'tfam', 'special_probes', 'output']
...
def triallelicMendelError(thisCall, lstParentCalls, tupUsualCopiesByGender, iThisGender):
    ...
    elif tupUsualCopiesByGender[iThisGender] == 2:
...
def mendelCheck(lstPed, lstCalls, tupUsualCopiesByGender, noCall, bTryTriallelic):
    ...
    for (i, thisCall) in enumerate(lstCalls):
        ...
        if bFoundError:
            continue
        if iThisGender == iMALE and tupUsualCopiesByGender[iMALE] == 1 and tupUsualCopiesByGender[iFEMALE] == 2 and not lstParentCalls[iFEMALE].isNoCall() and thisCall > lstParentCalls[iFEMALE]:
            ...
            continue
        if tupUsualCopiesByGender[iMALE] == 1 and tupUsualCopiesByGender[iFEMALE] == 1 and not lstParentCalls[iFEMALE].isNoCall() and thisCall > lstParentCalls[iFEMALE]:
            lstRet.append((i, thisCall, lstParentCalls[iFEMALE], lstParentCalls[iMALE], 6, 'Child has more copies than mother of MT probe'))
            continue
        if not lstParentCalls[iMALE].isNoCall() and not lstParentCalls[iFEMALE].isNoCall() and thisCall > lstParentCalls[iMALE] + lstParentCalls[iFEMALE]:
...
lstThreeAlleles = [
 LarryBirdCall([0, 0]), LarryBirdCall([1, 0]), LarryBirdCall([0, 1])]
...
def hardy_weinberg(lstPed, lstObjCalls, tupUsualCopiesByGender):
    ...
    return [
     ('/').join([ str(iCount) for iCount in dctCountByClass.values() ]), fChiSquared, fPValue] + lstProbabilities
...
def main(argv=None):
    ...
    parser.add_option('-c', '--calls', help='(Required) Input calls file from Canary or Larry_bird.')
    parser.add_option('--confs', help='(Required) Input confs file in traditional apt-probeset-genotype format.')
    parser.add_option('--tfam', help='(Required) Input pedigree file in tfam format.  Lines of this file should be in\nthe same order as samples in calls and confs file.')
    parser.add_option('--special_probes', help='(Required) special_probes file.  For larry_bird calls, this is special_snps.\nFor Canary calls, this is special_cn_probes file.')
    parser.add_option('-o', '--output', help='(Required) Basename for output files.  Output files will be\nBASENAME.mendel, BASENAME.imendel and BASENAME.lmendel.')
    parser.add_option('--assume_triallelic', default=False, action='store_true', help='If enabled, if a SNP has no calls with copy number > 2, assume alleles are {0, A, B},\nwhich allows for stricter Mendel check.  This is only valid for larry_bird calls.  Default: %default.')
    parser.add_option('-b', '--birdseed', default=False, action='store_true', help='If enabled, assume the calls file is Birdseed calls.  Selecting this option also\nenables --assume_triallelic.  Default: %default.')
    ...
    fileObjectFactory = calls_confidences_filter.FilteringCallsAndConfidencesFileFactory(strCallsPath=dctOptions.calls, strConfidencesPath=dctOptions.confs, strSamplesListPath=dctOptions.samples, strSNPsListPath=dctOptions.probes)
    ...
    for strLine in fCalls:
        ...
        if bIsCanaryCall is None:
            if dctOptions.birdseed:
                ...
                bIsCanaryCall = lstCalls[0].find(',') == -1
            if bIsCanaryCall:
                strNoCall = '-1'
        ...
        bTriallelic = dctOptions.assume_triallelic and isTriallelic(lstPed, lstObjCalls, tupUsualCopiesByGender, clsCall.noCall())
