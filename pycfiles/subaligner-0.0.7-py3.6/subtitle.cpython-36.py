...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/subaligner/subtitle.py
# Compiled at: 2020-02-25 13:05:05
# Size of source mod 2**32: 12609 bytes
import pysrt, tempfile, os, re, xml.etree.ElementTree as ElementTree
        ...
        for sub in subs:
            if se_uppercase is not None:
                if se_uppercase:
                    if sub.text.isupper():
                        new_subs.remove(sub)
                if not se_uppercase and sub.text.islower():
                    new_subs.remove(sub)
