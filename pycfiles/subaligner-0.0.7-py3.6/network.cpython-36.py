...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/subaligner/network.py
# Compiled at: 2020-05-03 19:52:37
# Size of source mod 2**32: 22714 bytes
import os, math, importlib, psutil, numpy as np, tensorflow as tf, tensorflow.keras.optimizers as tf_optimizers
...
class Network(object):
    ...
    def __init__(self, secret, input_shape, hyperparameters, model_path=None, backend='tensorflow'):
        ...
        if not secret == Network._Network__secret:
            ...
            if input_shape is None:
                if hyperparameters.network_type == Network._Network__UNKNOWN:
    ...
    def fit_and_get_history(self, train_data, labels, model_filepath, weights_filepath, logs_dir, training_log, resume):
        ...
        if resume:
            ...
            if not self.hyperparameters.epochs > initial_epoch:
                raise AssertionError('Existing model has been trained for {} epochs'.format(initial_epoch))
    ...
    def __set_keras_backend(backend):
        if K.backend() != backend:
            ...
            importlib.reload(K)
            if not K.backend() == backend:
                raise AssertionError('Unable to set backend to {}'.format(backend))
        else:
