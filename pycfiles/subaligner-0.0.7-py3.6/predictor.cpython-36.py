...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/subaligner/predictor.py
# Compiled at: 2020-04-28 18:04:16
# Size of source mod 2**32: 24688 bytes
import os, datetime, traceback, threading, concurrent.futures, numpy as np, multiprocessing as mp
...
class Predictor(Singleton):
    __doc__ = ' Predictor for working out the time to shift subtitles\n    '
    _Predictor__LOGGER = Logger().get_logger(__name__)
    _Predictor__MAX_SHIFT_IN_SECS = 100
    _Predictor__MAX_CHARS_PER_SEC = 50
    _Predictor__MAX_HEAD_ROOM = 20000
    ...
    def __init__(self, **kwargs):
        ...
        """
        self._Predictor__feature_embedder = FeatureEmbedder(**kwargs)
        self._Predictor__lock = threading.Lock()

    def predict_single_pass(self, video_file_path, subtitle_file_path, weights_dir='models/training/weights'):
        ...
        try:
            try:
                subs, audio_file_path, voice_probabilities = self._Predictor__predict(video_file_path, subtitle_file_path, weights_dir)
            except Exception:
                raise
            else:
                frame_rate = MediaHelper.get_frame_rate(video_file_path)
                self._Predictor__feature_embedder.step_sample = 1 / frame_rate
                self._Predictor__on_frame_timecodes(subs)
                return (subs, audio_file_path, voice_probabilities)
        finally:
            if os.path.exists(audio_file_path):
                os.remove(audio_file_path)

    def predict_dual_pass(self, video_file_path, subtitle_file_path, weights_dir='models/training/weights', stretch=False):
        """Predict time to shift with single pass
        ...
        try:
            try:
                subs, audio_file_path, voice_probabilities = self._Predictor__predict(video_file_path, subtitle_file_path, weights_dir)
                new_subs = self._Predictor__predict_2nd_pass(audio_file_path,
                  subs, weights_dir=weights_dir, stretch=stretch)
            except Exception:
                raise
            else:
                frame_rate = MediaHelper.get_frame_rate(video_file_path)
                self._Predictor__feature_embedder.step_sample = 1 / frame_rate
                self._Predictor__on_frame_timecodes(new_subs)
    ...
    def get_log_loss(self, voice_probabilities, subs):
        ...
        """
        subtitle_mask = Predictor._Predictor__get_subtitle_mask(self, subs)
        ...
        result = log_loss(subtitle_mask,
          (local_vp[:len(subtitle_mask)]), labels=[0, 1])
        Predictor._Predictor__LOGGER.debug('Log loss: {}'.format(result))
        return result
    ...
    def get_min_log_loss_and_index(self, voice_probabilities, subs):
        ...
        subtitle_mask = Predictor._Predictor__get_subtitle_mask(self, local_subs)
        if len(subtitle_mask) == 0:
            raise TerminalException('Subtitle is empty')
        else:
            head_room = len(voice_probabilities) - len(subtitle_mask)
            Predictor._Predictor__LOGGER.debug('head room: {}'.format(head_room))
            if head_room < 0:
                local_vp = np.vstack([
                 voice_probabilities,
                 [
        ...
        if head_room > Predictor._Predictor__MAX_HEAD_ROOM:
            Predictor._Predictor__LOGGER.error('head room: {}'.format(head_room))
            raise TerminalException('Maximum head room reached due to the suspicious audio or subtitle duration')
        log_losses = []
        Predictor._Predictor__LOGGER.debug('Start calculating {} log loss(es)...'.format(head_room))
        for i in np.arange(0, head_room):
            log_losses.append(log_loss(subtitle_mask,
        ...
        if log_losses:
            ...
            return (
             min_log_loss, log_losses.index(min_log_loss))
        else:
            return (None, 0)

    def __predict(self, video_file_path, subtitle_file_path, model_weights_dir, audio_file_path=None, subtitles=None, max_shift_secs=None, previous_gap=None):
        ...
        weights_files = [file for file in files if file.startswith('weights')]
        files = os.listdir(config_dir)
        hyperparams_files = [file for file in files if file.startswith('hyperparameters')]
        if not model_files:
            raise TerminalException('Cannot find model files at {}'.format(model_weights_dir))
        elif not weights_files:
            raise TerminalException('Cannot find weights files at {}'.format(model_weights_dir))
        else:
            Predictor._Predictor__LOGGER.debug('model files: {}'.format(model_files))
            Predictor._Predictor__LOGGER.debug('weigths files: {}'.format(weights_files))
            Predictor._Predictor__LOGGER.debug('config files: {}'.format(hyperparams_files))
            ...
            if audio_file_path is not None:
                ...
                if video_file_path is not None:
                    ...
                    audio_file_path = MediaHelper.extract_audio(video_file_path, True, 16000)
                    Predictor._Predictor__LOGGER.debug('Audio extracted after {}'.format(str(datetime.datetime.now() - t)))
        ...
        if subtitle_file_path is not None:
            ...
            try:
                train_data, labels = self._Predictor__feature_embedder.extract_data_and_label_from_audio(audio_file_path,
                  None, subtitles=subs)
            ...
            Predictor._Predictor__LOGGER.debug('input shape: {}'.format(input_shape))
            self._Predictor__lock.acquire()
            try:
                hyperparams = Hyperparameters.from_file(hyperparams_path)
                network = Network.get_from_model(model_path, hyperparams)
                Predictor._Predictor__LOGGER.debug('Start predicting...')
                pred_start = datetime.datetime.now()
                voice_probabilities = network.get_predictions(train_data, weights_path)
            finally:
                self._Predictor__lock.release()
            ...
            Predictor._Predictor__LOGGER.debug('Start {}'.format(original_start))
            shifted_subs = deepcopy(subs)
            subs.shift(seconds=(-original_start))
            Predictor._Predictor__LOGGER.info('Aligning subtitle with video...')
            ...
            Predictor._Predictor__LOGGER.info('Subtitle aligned')
            if subtitle_file_path is not None:
                seconds_to_shift = self._Predictor__feature_embedder.pos_to_sec(pos_to_delay) - original_start
            else:
                if subtitles is not None:
                    seconds_to_shift = self._Predictor__feature_embedder.pos_to_sec(pos_to_delay) - previous_gap
                else:
                    if os.path.exists(audio_file_path):
                        os.remove(audio_file_path)
                    raise ValueError('Error: No subtitles passed in')
        if abs(seconds_to_shift) > Predictor._Predictor__MAX_SHIFT_IN_SECS:
            if os.path.exists(audio_file_path):
                os.remove(audio_file_path)
            raise TerminalException('Average shift duration ({} secs) have been reached'.format(Predictor._Predictor__MAX_SHIFT_IN_SECS))
        else:
            ...
            Predictor._Predictor__LOGGER.debug('Statistics: {}'.format(result))
            Predictor._Predictor__LOGGER.debug('Total Time: {}'.format(total_elapsed_time))
            Predictor._Predictor__LOGGER.debug('Seconds to shift: {}'.format(seconds_to_shift))
            if max_shift_secs is not None:
                if seconds_to_shift <= max_shift_secs:
                    shifted_subs.shift(seconds=seconds_to_shift)
            elif max_shift_secs is not None and seconds_to_shift > max_shift_secs:
                Predictor._Predictor__LOGGER.warning('Maximum {} seconds shift has reached'.format(max_shift_secs))
                shifted_subs.shift(seconds=max_shift_secs)
            else:
                shifted_subs.shift(seconds=seconds_to_shift)
        return (
         shifted_subs, audio_file_path, voice_probabilities)
    ...
    def __predict_2nd_pass(self, audio_file_path, subs, weights_dir, stretch):
        ...
        for index, sub in enumerate(subs):
            Predictor._Predictor__LOGGER.debug('Subtitle chunk #{0}: start time: {1} ------> end time: {2}'.format(index, sub[0].start, sub[(len(sub) - 1)].end))

        if not len(segment_starts) == len(segment_ends):
            raise AssertionError('Segment start times and end times do not match')
        elif not len(segment_starts) == len(subs):
            raise AssertionError('Segment size and subtitle size do not match')
        subs_list = []
        max_workers = int(os.getenv('MAX_WORKERS', mp.cpu_count() / 2))
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as (executor):
            futures = [executor.submit(Predictor._Predictor__predict_in_multithreads, self, i, segment_starts, segment_ends, weights_dir, audio_file_path, subs, subs_copy, stretch) for i in range(len(segment_starts))]
            concurrent.futures.wait(futures)
            for future in futures:
                try:
                    new_subs = future.result()
                except Exception as e:
                    Predictor._Predictor__LOGGER.error('Exception: {}\n{}'.format(str(e), traceback.format_stack()))
                    raise
                else:
                    subs_list.append(new_subs)

        subs_list = [sub_item for subs in subs_list for sub_item in subs]
        return subs_list

    def __normalise_seconds_to_shift(seconds_to_shift, step_sample):
        return round(seconds_to_shift / step_sample) * step_sample

    def __predict_in_multithreads(self, segment_index, segment_starts, segment_ends, weights_dir, audio_file_path, subs, subs_copy, stretch):
        segment_path = ''
        try:
            if segment_index == len(segment_starts) - 1:
                segment_path, segment_duration = MediaHelper.extract_audio_from_start_to_end(audio_file_path, segment_starts[segment_index], None)
            else:
                segment_path, segment_duration = MediaHelper.extract_audio_from_start_to_end(audio_file_path, segment_starts[segment_index], segment_ends[segment_index])
            subtitle_duration = FeatureEmbedder.time_to_sec(subs[segment_index][(len(subs[segment_index]) - 1)].end) - FeatureEmbedder.time_to_sec(subs[segment_index][0].start)
            ...
            if segment_index == 0:
                ...
                previous_gap = FeatureEmbedder.time_to_sec(subs[segment_index][0].start) - FeatureEmbedder.time_to_sec(subs[(segment_index - 1)][(len(subs[(segment_index - 1)]) - 1)].end)
            subs_new, _, _ = self._Predictor__predict(video_file_path=None,
            ...
            if stretch:
                subs_new = self._Predictor__adjust_durations(subs_new, audio_file_path)
            return subs_new
        finally:
    ...
    def __get_subtitle_mask(self, subs):
        pos = self._Predictor__feature_embedder.time_to_pos(subs[(len(subs) - 1)].end) - 1
        subtitle_mask = np.zeros(pos if pos > 0 else 0)
        for sub in subs:
            start_pos = self._Predictor__feature_embedder.time_to_pos(sub.start)
            end_pos = self._Predictor__feature_embedder.time_to_pos(sub.end)
    ...
    def __adjust_durations(self, subs, audio_file_path):
        task_config_string = 'task_language=eng|os_task_file_format=srt|is_text_type=subtitles'
        runtime_config_string = 'dtw_algorithm=stripe'
        task = Task(config_string=task_config_string)
        try:
            segment_path, _ = MediaHelper.extract_audio_from_start_to_end(audio_file_path, str(subs[0].start), str(subs[(len(subs) - 1)].end))
            ...
            ExecuteTask(task=task,
              ...
              logger=AeneasLogger(tee=(Logger.VERBOSE))).execute()
            ...
            adjusted_subs.shift(seconds=MediaHelper.get_duration_in_seconds(start=None,
              end=(str(subs[0].start))))
            return adjusted_subs
        finally:
            if task.audio_file_path_absolute is not None:
                if os.path.exists(task.audio_file_path_absolute):
                    os.remove(task.audio_file_path_absolute)
                if task.text_file_path_absolute is not None:
                    if os.path.exists(task.text_file_path_absolute):
                        os.remove(task.text_file_path_absolute)
            else:
                if task.sync_map_file_path_absolute is not None:
    ...
    def __on_frame_timecodes(self, subs):
        for sub in subs:
            millis_per_frame = self._Predictor__feature_embedder.step_sample * 1000
