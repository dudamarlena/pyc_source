...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/subaligner/media_helper.py
        ...
        for i in range(len(subs)):
            ...
            if i == len(subs) - 1:
                ...
                if duration < MediaHelper._MediaHelper__MIN_SECS_PER_WORD:
                    ...
                    if subs[i].end == subs[(i + 1)].start or gap < MediaHelper._MediaHelper__MIN_GAP_IN_SECS:
                        combined.append(subs[i])
                    else:
                        combined.append(subs[i])
                        segment_starts.append(current_start)
