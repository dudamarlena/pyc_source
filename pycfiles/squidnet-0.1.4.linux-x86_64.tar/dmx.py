...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/squidnet/dmx.py
# Compiled at: 2010-04-06 18:14:05
import socket, time
KINET_MAGIC = chr(4) + chr(1) + chr(220) + chr(74)
KINET_VERSION = chr(1) + chr(0)
KINET_TYPE_DMXOUT = chr(1) + chr(1)
...
class DmxConnection:
    ...
    def send_dmx(self, data):
        ...
        out += chr(self.dmx_port)
        out += chr(0)
        out += chr(0) + chr(0)
        out += chr(255) + chr(255) + chr(255) + chr(255)
        out += data
...
class sPDS480caConnection:
    ...
    def __init__(self, address, universe):
        ...
        self.magic = b'\x04\x01\xdcJ\x01\x00\x08\x01\x00\x00\x00\x00\x00\x00\x00\x00' + chr(universe) + b'\xd1\x00\x00\x00\x02\x00'
...
class LightPanel:
    ...
    def __init__(self, dmx, comp):
        self.lights = [ [ RGBLight(j, i) for i in range(0, 12) ] for j in range(0, 12)
                      ]
        self.dmx = dmx
        self.width = 12
        self.height = 12
...
class HalfLightPanel:
    ...
    def __init__(self, dmx, direction):
        ...
        self.lights = [ [ RGBLight(j, i) for i in range(0, self.width) ] for j in range(0, self.height)
                      ]
        self.dmx = dmx
        self.time = time.time()
