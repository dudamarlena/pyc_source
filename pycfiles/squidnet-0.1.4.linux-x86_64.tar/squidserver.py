...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/squidnet/squidserver.py
# Compiled at: 2010-04-07 08:54:00
import socket, traceback
from threading import Thread
import signal, thread, os, time
...
class BroadcastServer(Thread):
    ...
    def run(self):
        ...
        while self.go:
            try:
                (message, address) = self.s.recvfrom(4194304)
...
class HandlerServer(Thread):
    ...
    def run(self):
        ...
        while self.go:
            try:
                (message, address) = self.s.recvfrom(2097152)
                print 'Got data from', address
                xs = sexp.read_all(message)
                for x in xs:
                    print 'Message:', x
...
if __name__ == '__main__':
    ...
    serv.add_device(d1)
    d1.add_message(s.SquidMessage('set', 'Set the lights to a color', [
     s.SquidArgument('color', s.SquidColorType(), s.SquidValue(s.SquidColorType(), [
      25, 25, 25]))], test_handler))
    d1.add_message(s.SquidMessage('stop', 'Stops the lights', []))
    run_server(serv)