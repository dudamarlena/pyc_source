...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/squidnet/squidprotocol.py
# Compiled at: 2010-04-07 16:47:35
...
class SquidServer(object):
    ...
    def handle(self, s):
        ...
        if s[0] == sexp.Symbol('server-request'):
            if sexp.plist_find(s[1:], sexp.Symbol('name:'), None) == self.name and sexp.plist_find(s[1:], sexp.Symbol('host:'), None) == self.host and sexp.plist_find(s[1:], sexp.Symbol('port:'), None) == self.port:
    ...
    def get_sexp(self):
        return [
         ...
         sexp.Symbol('devices:'), [ x.get_sexp() for x in self.devices ]]

    @staticmethod
    def load_sexp(s):
        if s[0] == sexp.Symbol('server'):
            sv = SquidServer(sexp.plist_find(s[1:], sexp.Symbol('name:'), None), sexp.plist_find(s[1:], sexp.Symbol('host:'), None), sexp.plist_find(s[1:], sexp.Symbol('port:'), None), sexp.plist_find(s[1:], sexp.Symbol('desc:'), None))
            ds = [ SquidDevice.load_sexp(d) for d in sexp.plist_find(s[1:], sexp.Symbol('devices:'), ())
                 ]
            for d in ds:
                sv.add_device(d)
...
class SquidDevice(object):
    ...
    def get_sexp(self):
        return [
         ...
         sexp.Symbol('name:'), self.name,
         sexp.Symbol('desc:'), self.desc,
         sexp.Symbol('messages:'), [ x.get_sexp() for x in self.messages ]]

    @staticmethod
    def load_sexp(s):
        if s[0] == sexp.Symbol('device'):
            d = SquidDevice(sexp.plist_find(s[1:], sexp.Symbol('name:'), None), sexp.plist_find(s[1:], sexp.Symbol('desc:'), None))
            ms = [ SquidMessage.load_sexp(m) for m in sexp.plist_find(s[1:], sexp.Symbol('messages:'), None) or []
                 ]
            for m in ms:
                d.add_message(m)
...
class SquidMessage(object):
    ...
    def get_sexp(self):
        return [sexp.Symbol('message'),
         sexp.Symbol('name:'), self.name,
         sexp.Symbol('desc:'), self.desc,
         sexp.Symbol('arguments:'), [ x.get_sexp() for x in self.arguments ]]

    @staticmethod
    def load_sexp(s):
        if s[0] == sexp.Symbol('message'):
            ars = [ SquidArgument.load_sexp(a) for a in sexp.plist_find(s[1:], sexp.Symbol('arguments:'), None) or [] ]
            m = SquidMessage(sexp.plist_find(s[1:], sexp.Symbol('name:'), None), sexp.plist_find(s[1:], sexp.Symbol('desc:'), None), ars)
            return m
        else:
...
class SquidArgument(object):
    ...
    def request(self, args):
        ...
        if self.name in args:
            return [
             sexp.Symbol(str(self.name)), args[self.name].get_sexp()]
        elif self.default is not None:
            return [
             self.name, self.default.get_sexp()]
        else:
    ...
    def load_sexp(s):
        if s[0] == sexp.Symbol('argument'):
            ...
            m = SquidArgument(sexp.plist_find(s[1:], sexp.Symbol('name:'), None), argtype, default)
            return m
        else:
...
if __name__ == '__main__':
    ...
    s.add_device(d1)
    d1.add_message(SquidMessage('set', 'Set the lights to a color', [
     SquidArgument('color', SquidColorType(), SquidValue(SquidColorType(), [25, 25, 25]))], test_handler))
    d1.add_message(SquidMessage('stop', 'Stops the lights', []))
