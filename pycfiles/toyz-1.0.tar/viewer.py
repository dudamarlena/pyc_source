...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/toyz/toyz/web/viewer.py
...
viewer_variables = {'filepath': None, 
   'img_file': None}
...
def import_fits():
    try:
        ...
        try:
            ...
            raise ToyzJobError('You must have astropy or pyfits installed to view FITS images')
    ...
    if session_vars.filepath == file_info['filepath']:
        ...
        if file_info['ext'] == 'fits':
            ...
            try:
                ...
                raise ToyzJobError('You must have PIL (Python Imaging Library) installed to open files of this type')
...
def get_file_info(file_info):
    ...
    if file_info['ext'] == 'fits':
        ...
        if 'images' not in file_info:
            if len(hdulist) > 1:
                file_info['images'] = OrderedDict([ [str(n), {'frame': str(n)}] for n, hdu in enumerate(hdulist) if 'imagehdu' in hdu.__class__.__name__.lower()
                                                  ])
            else:
        ...
        file_info['images'] = {'0': {'frame': '0'}}
    file_defaults = {'frame': next(iter(file_info['images'])), 
       'resampling': 'NEAREST', 
       'invert_x': False, 
       'invert_y': False, 
       'tile_format': 'png', 
...
def get_best_fit(data_width, data_height, img_viewer):
    x_scale = img_viewer['width'] / data_width * 0.97
    y_scale = img_viewer['height'] / data_height * 0.97
...
def get_tile_filename(file_info, img_info, x0_idx, xf_idx, y0_idx, yf_idx):
    filename_params = [
     file_info['filename'], file_info['frame'],
...
def get_tile_info(file_info, img_info):
    ...
    for row in range(minRow, maxRow):
        ...
        for col in range(minCol, maxCol):
            ...
            if tile_idx not in img_info['tiles'] or 'loaded' not in img_info['tiles'][tile_idx] or not img_info['tiles'][tile_idx]['loaded']:
                ...
                new_filepath = get_tile_filename(file_info, img_info, x0_idx, xf_idx, y0_idx, yf_idx)
                tile = {'idx': tile_idx, 
                   'left': x0, 
                   'right': xf, 
                   'top': y0, 
    ...
    if img_info['scale'] == 1:
        ...
        try:
            ...
            if img_info['scale'] > 1:
                ...
                tile_width = min(file_info['tile_width'], int((img_info['width'] - tile_info['x0_idx']) * img_info['scale']) - 1)
                tile_height = min(file_info['tile_height'], int((img_info['height'] - tile_info['y0_idx']) * img_info['scale']) - 1)
...
def get_img_data(data_type, file_info, img_info, **kwargs):
    ...
    if data_type == 'data':
        ...
        if kwargs['x'] < data.shape[1] and kwargs['y'] < data.shape[0] and kwargs['x'] >= 0 and kwargs['y'] >= 0:
            response = {'id': 'datapoint', 'px_value': float(data[(kwargs['y'], kwargs['x'])])}
        else:
