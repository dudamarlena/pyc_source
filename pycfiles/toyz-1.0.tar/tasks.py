...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/toyz/toyz/web/tasks.py
# Compiled at: 2015-09-01 16:22:53
"""
...
def load_user_settings(toyz_settings, tid, params):
    ...
    if tid['user_id'] == 'admin' or 'admin' in groups:
        ...
        user_settings = load_user_info(toyz_settings, tid, {'user_id': 'admin', 
           'user_attr': [
                       'groups', 'modules', 'toyz', 'paths']})
        group_settings = load_user_info(toyz_settings, tid, {'group_id': 'admin', 
           'user_attr': [
                       'groups', 'modules', 'toyz', 'paths']})
        ...
        response.update({'config': toyz_settings.config.__dict__, 
    ...
    if 'modify_toyz' in groups or 'admin' in groups or tid['user_id'] == 'admin':
        response.update({'modules': db_utils.get_param(dbs, 'modules', user_id=tid['user_id']), 
...
def save_user_info(toyz_settings, tid, params):
    ...
    if ('modules' in params or 'toyz' in params or 'third_party' in params) and tid['user_id'] != 'admin' and 'admin' not in groups and 'modify_toyz' not in groups:
        raise ToyzJobError("You must be an administrator or belong to the 'modify_toyz' group to modify a users toyz or module access")
...
def update_toyz_settings(toyz_settings, tid, params):
    ...
    security = toyz_settings.security.__dict__
    db.update(params['db'])
    params['config']['path'] = os.path.join(params['config']['root_path'], params['config']['config_path'])
...
def load_directory(toyz_settings, tid, params):
    ...
    if params['path'][0] == '$' and params['path'][(-1)] == '$':
        ...
        if shortcut not in shortcuts:
            raise ToyzJobError(("Shortcut '{0}' not found for user {1}").format(shortcut, tid['user_id']))
    ...
    for f in os.listdir(params['path']):
        if f[0] != '.' or show_hidden:
            ...
            if admin:
                ...
                permissions = file_access.get_parent_permissions(toyz_settings.db, f_path, user_id=tid['user_id'])
...
def create_paths(toyz_settings, tid, params):
    ...
    permissions = file_access.get_parent_permissions(toyz_settings.db, path, user_id=tid['user_id'])
...
def old_load_data_file(toyz_settings, tid, params):
    ...
    ``get_io_info``.
    """
    import toyz.utils.io as io, toyz.utils.sources as sources
    ...
    if 'data_type' in params:
        ...
        data_type = None
    print('PARAMS:', params, '\n\n')
    session_vars.data_sources[src_id] = src_types[src_type](user_id=tid['user_id'], data_type=data_type, paths=params['paths'])
    session_vars.data_sources[src_id].src_id = src_id
    session_vars.data_sources[src_id].name = src_id
    response = {'id': 'data_file', 
...
def load_data_file(toyz_settings, tid, params):
    ...
    ``get_io_info``.
    """
    import toyz.utils.io as io, toyz.utils.sources as sources, time
    time1 = time.time()
    if not hasattr(session_vars, 'data_sources'):
        session_vars.data_sources = {}
    src_id = params['src_id']
    ...
    session_vars.data_sources[src_id] = module_info['data_sources'][toyz_module][src_type](module_info, user_id=tid['user_id'], paths=params['paths'])
    session_vars.data_sources[src_id].src_id = src_id
    session_vars.data_sources[src_id].name = src_id
    time2 = time.time()
    response = {'id': 'data_file', 
       'columns': session_vars.data_sources[src_id].columns, 
       'benchmark': {'load_time': time2 - time1}}
    return response
...
def get_src_columns(toyz_settings, tid, params):
    ...
    for src_id, src in params.items():
        ...
        if len(src['columns']) > 0:
            sources[src_id] = {'data_type': 'columns', 'data': {}}
            sources[src_id]['data'] = session_vars.data_sources[src_id].to_dict(src['columns'])

    time2 = time.time()
    response = {'id': 'src_columns', 
       'sources': sources, 
       'benchmark': {'load time': time2 - time1}}
    return response
    ...
    if work_id in workspaces and params['overwrite'] is False:
        ...
        if 'user_id' in params and params['user_id'] != tid['user_id']:
            ...
            if permissions['modify']:
                ...
                raise ToyzJobError(('You do not have permission to save {0}').format(params['work_id']))
        db_utils.update_param(toyz_settings.db, 'workspaces', workspaces=params['workspaces'], user_id=user_id)
        response = {'id': 'notification', 
...
def load_workspace(toyz_settings, tid, params):
    ...
    response = {'id': 'workspace', 'work_id': params['work_id'], 
...
def get_workspace_sharing(toyz_settings, tid, params):
    ...
    """
    core.check4keys(params, ['work_id'])
    shared_workspaces = db_utils.get_workspace_sharing(toyz_settings.db, user_id=tid['user_id'], work_id=params['work_id'])
    ...
    response = {'id': 'get_workspace_sharing', 
       'ws_users': ws_users, 
       'ws_groups': ws_groups}
    return response
...
def get_file_info(toyz_settings, tid, params):
    ...
    if tid['user_id'] != 'admin':
        permissions = file_access.get_parent_permissions(toyz_settings.db, params['file_info']['filepath'], user_id=tid['user_id'])
        if permissions is None or 'r' not in permissions:
            raise ToyzJobError('You do not have permission to view the requested file.Please contact your network administrator if you believe this is an error.')
    file_info = viewer.get_file_info(params['file_info'])
    img_info = file_info['images'][file_info['frame']]
    img_info.update(params['img_info'])
    result = get_img_info(toyz_settings, tid, {'file_info': file_info, 
       'img_info': img_info})
    file_info['images'][file_info['frame']] = result['img_info']
    response = {'id': 'file info', 
       'file_info': file_info, 
...
def get_img_info(toyz_settings, tid, params):
    ...
    result = get_tile_info(toyz_settings, tid, {'file_info': params['file_info'], 
       'img_info': img_info})
    img_info['tiles'] = result['new_tiles']
    response = {'id': 'img info', 
       'img_info': img_info, 
       'new_tiles': result['new_tiles']}
    print('************************************************')
    return response
...
def get_img_tile(toyz_settings, tid, params):
    ...
    if tid['user_id'] != 'admin':
        ...
        if 'r' not in permissions:
            raise ToyzJobError('You do not have permission to view the requested file.Please contact your network administrator if you believe this is an error.')
    created, tile_info = viewer.create_tile(params['file_info'], params['img_info'], params['tile_info'])
    response = {'id': 'tile created', 
       'success': created, 
       'tile_info': tile_info}
    return response
...
def log_benchmark(toyz_settings, tid, params):
    ...
    return {'id': 'log_benchmark', 'status': 'success', 
