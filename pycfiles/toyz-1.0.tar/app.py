...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/toyz/toyz/web/app.py
# Compiled at: 2015-09-05 13:05:44
"""
Runs the webapp for Toyz
"""
from __future__ import division, print_function
import sys, platform, os.path, shutil, importlib, socket, multiprocessing, tornado.ioloop, tornado.options, tornado.web, tornado.websocket, tornado.escape, tornado.gen
...
class ToyzHandler:
    ...
    def get_toyz_path(self, path, path_type):
        ...
        config = core.get_toyz_module(self.application.toyz_settings, self.get_user_id(), toy_name + '.config')
        if path_type == 'static':
...
class AuthStaticFileHandler(AuthHandler, tornado.web.StaticFileHandler):
    ...
    def validate_absolute_path(self, root, full_path):
        ...
        """
        permissions = file_access.get_parent_permissions(self.application.toyz_settings.db, full_path, user_id=self.get_current_user().strip('"'))
        if 'r' in permissions or 'x' in permissions:
            absolute_path = tornado.web.StaticFileHandler.validate_absolute_path(self, root, full_path)
        else:
            absolute_path = None
...
class AuthLoginHandler(AuthHandler, tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if core.check_pwd(self.application.toyz_settings, user_id, pwd):
            ...
            redirect += '&next=' + self.get_argument('next', '/')
...
class ToyzWorkspaceHandler(ToyzHandler, tornado.web.RequestHandler):
    ...
    def get(self, path):
        ...
        ws_options = {'user_theme': self.get_user_theme(), 
           'user_id': '', 
           'work_id': ''}
...
class ToyzWebApp(tornado.web.Application):
    ...
    def __init__(self):
        ...
        handlers = [
         ...
         (
          '/static/(.*)', static_handler, {'path': core.ROOT_DIR}),
         (
          '/workspace/(.*)', workspace_handler),
         (
          '/file/(.*)', static_handler, {'path': file_path}),
         (
          '/toyz/static/(.*)', toyz_static_handler, {'path': '/'}),
         (
          '/toyz/templates/(.*)', toyz_template_handler),
         (
          '/third_party/(.*)', third_party_handler, {'path': core.ROOT_DIR}),
         (
          '/session/(.*)', WebSocketHandler),
         (
          '/benchmark/(.*)', benchmark_handler),
         (
          '/(favicon.ico)', static_handler, {'path': os.path.join(core.ROOT_DIR, 'web', 'static')})]
        settings = {'cookie_secret': self.toyz_settings.web.cookie_secret, 
    ...
    def new_session(self, user_id, websocket, session_id=None):
        ...
        shortcuts = db_utils.get_param(self.toyz_settings.db, 'shortcuts', user_id=user_id)
        shortcuts = core.check_user_shortcuts(self.toyz_settings, user_id, shortcuts)
        websocket.session = {'user_id': user_id, 
        ...
        websocket.process = multiprocessing.Process(target=job_process, args=(session_id, remote_pipe, websocket.job_pipe))
        websocket.process.start()
        remote_pipe.close()
        process_events = tornado.ioloop.IOLoop.READ | tornado.ioloop.IOLoop.ERROR
        tornado.ioloop.IOLoop.current().add_handler(websocket.job_pipe, websocket.send_response, process_events)
        websocket.write_message({'id': 'initialize', 
           'user_id': user_id, 
           'session_id': session_id})
        return
    ...
    def close_session(self, session):
        ...
        tornado.ioloop.IOLoop.current().remove_handler(self.user_sessions[session['user_id']][session['session_id']].job_pipe)
        ...
        if len(self.user_sessions[session['user_id']]) == 0:
            shortcuts = db_utils.get_param(self.toyz_settings.db, 'shortcuts', user_id=session['user_id'])
...
def init_web_app():
    ...
    tornado.options.define('root_path', default=None, help='Use root_path as the root directory for a Toyz instance')
