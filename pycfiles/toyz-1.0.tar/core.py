...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/toyz/toyz/utils/core.py
# Compiled at: 2015-08-06 15:32:06
from __future__ import division, print_function
import os, sys, importlib, imp, base64, uuid
try:
...
default_settings = {'config': {'root_path': os.path.join(ROOT_DIR), 
   ...
   'security': {'encrypt_db': False, 
                'user_login': True, 
                'pwd_context': {'schemes': [
                                          'sha512_crypt', 'pbkdf2_sha512'], 
...
def str_2_bool(bool_str):
    ...
    if ('true').startswith(lower_str) or ('yes').startswith(lower_str):
        return True
    if ('false').startswith(lower_str) or ('no').startswith(lower_str):
        return False
    raise ToyzError(("'{0}' did not match a boolean expression  (true/false, yes/no, t/f, y/n)").format(bool_str))
...
def check_user_shortcuts(toyz_settings, user_id, shortcuts=None):
    ...
    if 'user' not in shortcuts:
        ...
        db_utils.update_param(toyz_settings.db, 'shortcuts', user_id=user_id, shortcuts={'user': shortcuts['user']})
        create_paths([shortcuts['user']])
        modified = True
    if 'temp' not in shortcuts:
        shortcuts['temp'] = os.path.join(shortcuts['user'], 'temp')
        db_utils.update_param(toyz_settings.db, 'shortcuts', user_id=user_id, shortcuts={'temp': shortcuts['temp']})
    ...
    if modified:
        ...
        if shortcuts['user'] not in paths or paths[shortcuts['user']] != 'frwx':
            db_utils.update_param(toyz_settings.db, 'paths', user_id=user_id, paths={shortcuts['user']: 'frwx'})
    return shortcuts
...
def check_user_modules(toyz_settings, user_id, module):
    ...
    if module in user_modules or module.endswith('.tasks') and module[:-6] in user_modules or module.endswith('.config') and module[:-7] in user_modules:
        return True
    return False
...
def get_user_toyz(toyz_settings, user_id, toy):
    ...
    if toy in user_toyz:
        ...
        if toy.endswith('.tasks') and toy[:-6] in user_toyz:
            return imp.load_source(toy, os.path.join(user_toyz[toy[:-6]], 'tasks.py'))
        if toy.endswith('.config') and toy[:-7] in user_toyz:
...
def run_job(toyz_settings, pipe, job):
    ...
    try:
        try:
            ...
            ToyzJobError(job['module'] + ' not found in ' + job['id']['user_id'] + "'s approved modules")
        ...
        response = {'id': 'ERROR', 'error': 'PYTHON ERROR:' + type(error).__name__ + str(error.args), 
           'traceback': traceback.format_exc()}
...
def progress_log(msg):
    ...
    if hasattr(session_vars, 'pipe'):
        notification = {'id': 'notification', 'msg': msg}
...
class ToyzSettings:
    ...
    def first_time_setup(self, config_root_path):
        ...
        while not get_bool(("Create new Toyz configuration in '{0}'? ").format(self.config.root_path)):
            self.config.root_path = normalize_path(raw_input('new path: '))

        self.config.path = os.path.join(self.config.root_path, self.config.config_path)
        ...
        db_utils.update_param(self.db, 'paths', group_id='all', paths={os.path.join(ROOT_DIR, 'web', 'static'): 'fr', 
...
def check_version(db_settings):
    ...
    if toyz_version != db_version:
        ...
        db.update_version(db_settings, {'toyz_version': toyz_version, 
...
def get_workspace_permissions(toyz_settings, tid, params):
    ...
    shared_workspaces = db_utils.get_workspace_sharing(toyz_settings.db, user_id=params['user_id'], work_id=params['work_id'])
    ...
    elif len(shared_workspaces) > 0:
        for shared_user in shared_workspaces:
            if shared_user['share_id_type'] == 'user_id' and shared_user['share_id'] == tid['user_id']:
                for p in permissions:
                    permissions[p] = permissions[p] or shared_user[p]

            elif shared_user['share_id_type'] == 'group_id' and shared_user['share_id'] in groups:
                for p in permissions:
                    permissions[p] = permissions[p] or shared_user[p]
    ...
    *No longer implemented, but may be in the future*
    ...
    def initialize(self):
        ...
        for p in range(process_count):
            process = multiprocessing.Process(name=queue_name + '-' + str(p), target=self.job_worker, args=(
             self.queue,))
            process.daemon = True
            process.start()
