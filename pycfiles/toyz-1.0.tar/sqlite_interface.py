...
# [GCC 8.4.0]
# Embedded file name: /media/data-beta/users/fmooleka/git_projects/toyz/toyz/utils/db_interfaces/sqlite_interface.py
...
def update_param(db_settings, param_type, **params):
    ...
    if param_format['format'] == 'single':
        ...
        sql = ('replace into {0} ({1}) values ({2})').format(tbl, (',').join(cols), (',').join([ '?' for i in cols ]))
...
def create_toyz_database(db_settings):
    ...
    if os.path.isfile(db_settings.path):
        ...
        if overwrite:
            overwrite = core.get_bool('This will remove all user and group settings and workspaces for this Toyz instance (it may be a good idea to copy your current DB before)you proceed. Are you sure you want to overwrite the current DB? ')
        if overwrite:
    ...
    db.execute("insert into db_info (key, value, timestamp) values ('current',?,?)", (
     version.version, timestamp))
    db.execute('create table users (user_id text not null, user_type text not null, pwd text not null);')
    ...
    db.execute('create table paths (user_id text not null, user_type text not null, path text not null, permissions text not null);')
    db.execute('create unique index paths_idx on paths (user_id, user_type, path);')
    db.execute('create table modules (user_id text not null, user_type text not null, module text not null);')
    db.execute('create unique index modules_idx on modules (user_id, user_type, module);')
    db.execute('create table toyz (user_id text not null, user_type text not null, toy_id text not null, path text not null);')
    db.execute('create unique index toyz_idx on toyz (toy_id, user_id, user_type, toy_id);')
    db.execute('create table shortcuts (user_id text not null, short_id text not null, path text not null);')
    db.execute('create unique index shortcuts_idx on shortcuts (user_id, short_id);')
    db.execute('create table workspaces (user_id text not null, user_type text not null, work_id text not null, work_settings text not null);')
...
def get_path_info(db_settings, path):
    ...
    if len(all_info) == 0:
        ...
        user_info = {p[0]:p[1] for p in all_info if p[2] == 'user_id' if p[2] == 'user_id'}
        group_info = {p[0]:p[1] for p in all_info if p[2] == 'group_id' if p[2] == 'group_id'}
...
def get_db_info(db_settings):
    ...
    db_info = {info[0]:{'value': info[1], 'timestamp': info[2]} for info in meta}
    return db_info
...
def insert_ws_row(db, user_id, work_id, row):
    ...
    sql = ('insert into shared_workspaces ({0}) values ({1})').format((',').join(cols), vals)
