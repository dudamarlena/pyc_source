...
class LayerConfig(typing.NamedTuple):
    __doc__ = 'Configuration for layers'
    name = None
    name: str
    module = None
    module: str
...
class LayerContext:
    __doc__ = 'Stores important runtime information, like the current resolution and the OSC dispatcher'
    ...
    def checkevent(self, event: inotify_simple.Event) -> bool:
        ...
        if event.name == self.config.filepath.name:
            if event.mask & (flags.CREATE | flags.MODIFY | flags.MOVED_TO):
                ...
                if event.mask & (flags.DELETE | flags.DELETE_SELF):
                    self.valid = False
