...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/analyze_muse/analyze_muse_data.py
# Compiled at: 2020-01-29 15:40:00
# Size of source mod 2**32: 176776 bytes
""" 
...
import analyze_muse.resources.resources_rc, analyze_muse.ABCS_version
muse_EEG_data = []
muse_marker_data = []
EEG_Dict = {}
eeg_stats = []
...
Filter_Order = 3
Filter_Type = 0
NOTCH_B, NOTCH_A = butter(4, (np.array([55, 65]) / 128.0), btype='bandstop')
Verbosity = 0
Save_DB = False
session_dict = {}
gui_dict = {}
...
class The_GUI(QDialog):
    ...
    def plot_button_clicked(self):
        ...
        eeg_power = self.checkBoxEEG_PowerBands.isChecked()
        ...
        gui_dict.update({'firstName':first_name,  'lastName':last_name,  'session_notes':session_notes, 
         'checkBoxInteractive':interative_GUI, 
         'checkBoxEEG':plot_EEG, 
         'checkBoxCoherence':coherence, 
         'checkBoxPowerBands':power_bands, 
         'checkBoxEEG_PowerBands':eeg_power, 
    ...
    def createTopRightGroupBox(self):
        ...
        self.notesTextEdit.setPlainText('Add any details about your meditation session.  For example, mood, place, music, etc.\n')
    ...
    def openFileNameDialog(self):
        ...
        fileName, _ = dialog.getOpenFileName(self, 'Select EEG CSV File',
          data_dir,
...
def manage_session_data(init=False, new_data={}, session_date='', date_time=''):
    ...
    if init:
        ...
        session_dict = {'ABCS Info':{'Version': ABCS_FORMAT_VERSION_NUM}, 
         'Muse Info':{'Headband Version': '2016'}, 
         'Session_Data':{'session_date':session_date, 
          'date':date_time, 
         ...
         'Participants':{'Meditators':{0:{'name':'Debra', 
            'age':63, 
            'gender':'female'}, 
           1:{'name':'Savahn', 
            'age':38, 
            'gender':'female'}}, 
          'Coordinators':{0: {'name':'Patti', 
               'age':32, 
               'gender':'female'}}, 
          'Helpers':{0: {'name':'Oni', 
               'age':23, 
               'gender':'male'}}, 
          'Participants':{0: {'name':'Shiloh', 
               'age':28, 
               'gender':'male'}}}}
...
def connect_to_DB(date_time_now):
    ...
    conn = sqlite3.connect(db_fname)
    c = conn.cursor()
    sql_insert = "INSERT INTO eeg_data VALUES ( '" + date_time_now + "','" + json.dumps(session_dict) + "','" + json.dumps(gui_dict) + "', 100, 0.05)"
    if Verbosity > 2:
...
def read_eeg_data(fname, date_time_now):
    ...
    muse_EEG_data = pd.read_csv(fname, parse_dates=['TimeStamp'], date_parser=(pd.to_datetime),
      dtype=dtypes,
      compression='infer',
      verbose=csv_verbosity)
    ...
    raw_df = pd.DataFrame(muse_EEG_data, columns=[
     'RAW_TP9', 'RAW_AF7', 'RAW_AF8', 'RAW_TP10'])
    delta_df = pd.DataFrame(muse_EEG_data, columns=[
     'Delta_TP9', 'Delta_AF7', 'Delta_AF8', 'Delta_TP10'])
    theta_df = pd.DataFrame(muse_EEG_data, columns=[
     'Theta_TP9', 'Theta_AF7', 'Theta_AF8', 'Theta_TP10'])
    alpha_df = pd.DataFrame(muse_EEG_data, columns=[
     'Alpha_TP9', 'Alpha_AF7', 'Alpha_AF8', 'Alpha_TP10'])
    beta_df = pd.DataFrame(muse_EEG_data, columns=[
     'Beta_TP9', 'Beta_AF7', 'Beta_AF8', 'Beta_TP10'])
    gamma_df = pd.DataFrame(muse_EEG_data, columns=[
     'Gamma_TP9', 'Gamma_AF7', 'Gamma_AF8', 'Gamma_TP10'])
    if EEG_data_source == 'Mind Monitor':
        data_keys = ['Delta_TP9', 'Delta_AF7', 'Delta_AF8', 'Delta_TP10',
         'Theta_TP9', 'Theta_AF7', 'Theta_AF8', 'Theta_TP10',
    ...
    muse_marker_data = elements_df.copy()
    ...
    parms_dict = {'Analysis Parameters': {'lowcut':Filter_Lowcut, 
                             'highcut':Filter_Highcut,  'filter_order':Filter_Order,  'sample_length':sample_length, 
                             'sample_time_sec':sample_time_sec,  'sample_time_min':sample_time_min}}
    session_dict = manage_session_data(init=True, session_date=(str(time_df['TimeStamp'][0])),
...
def check_file_type(fname):
    ...
    if time_diff_seconds == 0:
        ...
        if time_diff_seconds > 1:
            ...
            if Verbosity > 1:
                pause_and_prompt(0.1, 'Data successfuly checked')
...
def write_hdf5_data(muse_EEG_data, data_fname):
    ...
    data_stats = (
     EEG_Dict['RAW_AF7']['25%'], EEG_Dict['RAW_AF7']['75%'],
    ...
    with h5py.File(data_fname, 'w') as (f):
        ...
        stats = g_stats.create_dataset('stats', compression='gzip', compression_opts=9, data=data_stats)
        delta = g_power.create_dataset('p_delta', dtype='f8', compression='gzip', compression_opts=9, data=(
         delta_df['Delta_TP9'], delta_df['Delta_AF7'],
         delta_df['Delta_AF8'], delta_df['Delta_TP10']))
        theta = g_power.create_dataset('p_theta', dtype='f8', compression='gzip', compression_opts=9, data=(
         theta_df['Theta_TP9'], theta_df['Theta_AF7'],
         theta_df['Theta_AF8'], theta_df['Theta_TP10']))
        alpha = g_power.create_dataset('p_alpha', dtype='f8', compression='gzip', compression_opts=9, data=(
         alpha_df['Alpha_TP9'], alpha_df['Alpha_AF7'],
         alpha_df['Alpha_AF8'], alpha_df['Alpha_TP10']))
        beta = g_power.create_dataset('p_beta', dtype='f8', compression='gzip', compression_opts=9, data=(
         beta_df['Beta_TP9'], beta_df['Beta_AF7'],
         beta_df['Beta_AF8'], beta_df['Beta_TP10']))
        gamma = g_power.create_dataset('p_gamma', dtype='f8', compression='gzip', compression_opts=9, data=(
         gamma_df['Gamma_TP9'], gamma_df['Gamma_AF7'],
         gamma_df['Gamma_AF8'], gamma_df['Gamma_TP10']))
        raw = g_raw.create_dataset('raw', dtype='f8', compression='gzip', compression_opts=9, data=(
         df['RAW_TP9'], df['RAW_AF7'], df['RAW_AF8'], df['RAW_TP10']))
        accel = g_motion.create_dataset('accel', dtype='f8', compression='gzip', compression_opts=9, data=(
         motion_df['Accelerometer_X'], motion_df['Accelerometer_Y'], motion_df['Accelerometer_Z']))
        gyro = g_motion.create_dataset('gyro', dtype='f8', compression='gzip', compression_opts=9, data=(
         motion_df['Gyro_X'], motion_df['Gyro_Y'], motion_df['Gyro_Z']))
    return True


def scale(x, out_range=(-1, 1), axis=None):
    domain = (
     np.min(x, axis), np.max(x, axis))
...
def filter_all_data(muse_EEG_data):
    ...
    if Verbosity > 0:
        ...
        if Filter_Type == 0:
            ...
            print("filter_all_data() - muse_EEG_data['RAW_AF7'].describe: ", muse_EEG_data['RAW_AF7'].describe())
            muse_EEG_data['RAW_AF8'] = filter_data(muse_EEG_data['RAW_AF8'])
            print("filter_all_data() - muse_EEG_data['RAW_AF8'].describe: ", muse_EEG_data['RAW_AF8'].describe())
            muse_EEG_data['RAW_TP10'] = filter_data(muse_EEG_data['RAW_TP10'])
            print("filter_all_data() - muse_EEG_data['RAW_TP10'].describe: ", muse_EEG_data['RAW_TP10'].describe())
            if Verbosity > 2:
        ...
        if Filter_Type == 1:
            muse_EEG_data['RAW_TP9'] = butter_lowpass_filter(muse_EEG_data['RAW_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_AF7'] = butter_lowpass_filter(muse_EEG_data['RAW_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_AF8'] = butter_lowpass_filter(muse_EEG_data['RAW_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_TP10'] = butter_lowpass_filter(muse_EEG_data['RAW_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering raw data')
            muse_EEG_data['Delta_TP9'] = butter_lowpass_filter(muse_EEG_data['Delta_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_AF7'] = butter_lowpass_filter(muse_EEG_data['Delta_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_AF8'] = butter_lowpass_filter(muse_EEG_data['Delta_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_TP10'] = butter_lowpass_filter(muse_EEG_data['Delta_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering delta data')
            muse_EEG_data['Theta_TP9'] = butter_lowpass_filter(muse_EEG_data['Theta_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_AF7'] = butter_lowpass_filter(muse_EEG_data['Theta_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_AF8'] = butter_lowpass_filter(muse_EEG_data['Theta_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_TP10'] = butter_lowpass_filter(muse_EEG_data['Theta_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering theta data')
            muse_EEG_data['Alpha_TP9'] = butter_lowpass_filter(muse_EEG_data['Alpha_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_AF7'] = butter_lowpass_filter(muse_EEG_data['Alpha_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_AF8'] = butter_lowpass_filter(muse_EEG_data['Alpha_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_TP10'] = butter_lowpass_filter(muse_EEG_data['Alpha_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering alpha data')
            muse_EEG_data['Beta_TP9'] = butter_lowpass_filter(muse_EEG_data['Beta_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_AF7'] = butter_lowpass_filter(muse_EEG_data['Beta_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_AF8'] = butter_lowpass_filter(muse_EEG_data['Beta_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_TP10'] = butter_lowpass_filter(muse_EEG_data['Beta_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering beta data')
            muse_EEG_data['Gamma_TP9'] = butter_lowpass_filter(muse_EEG_data['Gamma_TP9'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_AF7'] = butter_lowpass_filter(muse_EEG_data['Gamma_AF7'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_AF8'] = butter_lowpass_filter(muse_EEG_data['Gamma_AF8'], Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_TP10'] = butter_lowpass_filter(muse_EEG_data['Gamma_TP10'], Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering gamma data')
        if Filter_Type == 2:
            muse_EEG_data['RAW_TP9'] = butter_bandpass_filter(muse_EEG_data['RAW_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_AF7'] = butter_bandpass_filter(muse_EEG_data['RAW_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_AF8'] = butter_bandpass_filter(muse_EEG_data['RAW_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['RAW_TP10'] = butter_bandpass_filter(muse_EEG_data['RAW_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering raw data')
            muse_EEG_data['Delta_TP9'] = butter_bandpass_filter(muse_EEG_data['Delta_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_AF7'] = butter_bandpass_filter(muse_EEG_data['Delta_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_AF8'] = butter_bandpass_filter(muse_EEG_data['Delta_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Delta_TP10'] = butter_bandpass_filter(muse_EEG_data['Delta_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering delta data')
            muse_EEG_data['Theta_TP9'] = butter_bandpass_filter(muse_EEG_data['Theta_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_AF7'] = butter_bandpass_filter(muse_EEG_data['Theta_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_AF8'] = butter_bandpass_filter(muse_EEG_data['Theta_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Theta_TP10'] = butter_bandpass_filter(muse_EEG_data['Theta_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering theta data')
            muse_EEG_data['Alpha_TP9'] = butter_bandpass_filter(muse_EEG_data['Alpha_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_AF7'] = butter_bandpass_filter(muse_EEG_data['Alpha_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_AF8'] = butter_bandpass_filter(muse_EEG_data['Alpha_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Alpha_TP10'] = butter_bandpass_filter(muse_EEG_data['Alpha_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering alpha data')
            muse_EEG_data['Beta_TP9'] = butter_bandpass_filter(muse_EEG_data['Beta_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_AF7'] = butter_bandpass_filter(muse_EEG_data['Beta_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_AF8'] = butter_bandpass_filter(muse_EEG_data['Beta_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Beta_TP10'] = butter_bandpass_filter(muse_EEG_data['Beta_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering beta data')
            muse_EEG_data['Gamma_TP9'] = butter_bandpass_filter(muse_EEG_data['Gamma_TP9'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_AF7'] = butter_bandpass_filter(muse_EEG_data['Gamma_AF7'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_AF8'] = butter_bandpass_filter(muse_EEG_data['Gamma_AF8'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            muse_EEG_data['Gamma_TP10'] = butter_bandpass_filter(muse_EEG_data['Gamma_TP10'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, Filter_Order)
            if Verbosity > 2:
                print('filter_all_data() finished filtering gamma data')
...
def filter_data(data_in):
    ...
    b, a = butter_lowpass(Filter_Highcut, Sampling_Rate, order=Filter_Order)
...
def butter_lowpass(cutoff, fs, order=5):
    ...
    if Verbosity > 2:
        print('butter_lowpass() nyq: ', nyq, ' cutoff ', cutoff, 'normal_cutoff:', normal_cutoff, ' fs ', fs, ' order ', order)
    b, a = butter(order, normal_cutoff, btype='low', analog=False)
    return (
     b, a)


def butter_lowpass_filter(data_in, cutoff, fs, order=5):
    b, a = butter_lowpass(cutoff, fs, order=order)
    y = lfilter(b, a, data_in)
    return y
...
def butter_bandpass(lowcut, highcut, fs, order=5):
    ...
    high = highcut / nyq
    if Verbosity > 0:
        print('butter_bandpass() nyq: ', nyq, ' lowcut ', lowcut, ' highcut ', highcut, 'low:', low, 'high:', high, ' fs ', fs, ' order ', order)
    b, a = butter(order, [low, high], btype='band')
    return (
     b, a)


def butter_bandpass_filter(data_in, lowcut, highcut, fs, order=5):
    data_to_filter = np.array(data_in)
    b, a = butter_bandpass(lowcut, highcut, fs, order=order)
    y = lfilter(b, a, data_to_filter)
    return y


def plot_all(muse_EEG_data, data_fname, plot_fname, date_time_now, title, data_stats, analysis_parms, fig_num):
    ...
    fig, axs = plt.subplots(nrows=9, num=fig_num, figsize=(8, 8), dpi=PLOT_DPI,
      facecolor='w',
      edgecolor='k',
      sharex=True,
    ...
    axs[0].plot(x_series, tp9_df, alpha=1.0, marker='.', mec='xkcd:dark pink', color=(plot_color_scheme['RawTP9']),
      label='TP9')
    ...
    axs[1].plot(x_series, af7_df, alpha=1.0, marker='.', mec='xkcd:salmon', color=(plot_color_scheme['RawAF7']),
      label='AF7')
    ...
    axs[2].plot(x_series, af8_df, alpha=1.0, marker='.', mec='xkcd:cerulean', color=(plot_color_scheme['RawAF8']),
      label='AF8')
    ...
    axs[3].plot(x_series, tp10_df, alpha=1.0, marker='.', mec='xkcd:dark lilac', color=(plot_color_scheme['RawTP10']),
      label='TP10')
    ...
    axs[4].plot(x_series, gamma_df.mean(axis=1), color=(plot_color_scheme['Gamma']), marker='.', mec='xkcd:dark pink', alpha=plot_alpha,
      label='Gamma')
    ...
    l1 = axs[5].plot(x_series, beta_df.mean(axis=1), color=(plot_color_scheme['Beta']), marker='.',
      mec='xkcd:dark teal',
      alpha=plot_alpha,
      label='Beta')
    ...
    axs[6].plot(x_series, alpha_df.mean(axis=1), color=(plot_color_scheme['Alpha']), marker='.', mec='xkcd:dark brown', alpha=plot_alpha,
      label='Alpha')
    ...
    axs[7].plot(x_series, theta_df.mean(axis=1), color=(plot_color_scheme['Theta']), marker='.', mec='xkcd:crimson', alpha=plot_alpha,
      label='Theta')
    ...
    axs[8].plot(x_series, delta_df.mean(axis=1), color=(plot_color_scheme['Delta']), marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Delta')
    ...
    plt.text(0.175, 13.2, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
...
def plot_coherence_scatter(x, y, a, b, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    ...
    fig, axs = plt.subplots(nrows=1, num=fig_num, figsize=FIGURE_SIZE, dpi=PLOT_DPI,
      facecolor='w',
      edgecolor='k',
      sharex=True,
      sharey=True,
    ...
    plt.text(0.175, 1.025, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
...
def plot_coherence_data(tp9, af7, af8, tp10, data_fname, plot_fname, date_time_now, title, data_stats, analysis_parms, fig_num):
    ...
    period = 1.0 / Sampling_Rate
    x_series = np.arange(0, t_len * period, period)
    fig, axs = plt.subplots(nrows=2, num=fig_num, figsize=FIGURE_SIZE, dpi=PLOT_DPI,
      facecolor='w',
      edgecolor='k',
      sharex=True,
    ...
    clip_padding = 50.0
    y_limits = [data_min - clip_padding, data_max + clip_padding]
    axs[0].plot(x_series, af_diff, alpha=0.8, marker='.', mec='xkcd:dark brown', color=(plot_color_scheme['RawAF8']),
      label='AF Diff')
    axs[0].set(title='AF7 - AF8', ylabel='Amp uV')
    axs[0].set_ylim((af_min, af_max))
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[0], 'Coherence')
    axs[1].plot(x_series, tp_diff, alpha=0.8, marker='.', mec='xkcd:wine', color=(plot_color_scheme['RawTP9']),
      label='TP Diff')
    ...
    plt.text(0.175, 2.275, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
    plt.text(1.01, 1.75, ('Mean: ' + '{:.3f}'.format(af_mean) + '\nStd: ' + '{:.3f}'.format(af_std) + '\nMin: ' + '{:.3f}'.format(af_min) + '\nMax: ' + '{:.3f}'.format(af_max)),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 0.5, ('Mean: ' + '{:.3f}'.format(tp_mean) + '\nStd: ' + '{:.3f}'.format(tp_std) + '\nMin: ' + '{:.3f}'.format(tp_min) + '\nMax: ' + '{:.3f}'.format(tp_max)),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.savefig(plot_fname, dpi=300)
...
def plot_sensor_data(tp9, af7, af8, tp10, data_fname, plot_fname, date_time_now, title, data_stats, analysis_parms, fig_num):
    if Verbosity > 0:
        print('plot_sensor_data() called')
    t_len = len(tp9)
    period = 1.0 / Sampling_Rate
    x_series = np.arange(0, t_len * period, period)
    fig, axs = plt.subplots(nrows=5, num=fig_num, figsize=FIGURE_SIZE, dpi=PLOT_DPI,
      facecolor='w',
      edgecolor='k',
      sharex=True,
      sharey=(gui_dict['checkBoxVerticalLock']),
    ...
    clip_padding = 100.0
    y_limits = [data_min - clip_padding, data_max + clip_padding]
    axs[0].plot(x_series, tp9, alpha=0.8, marker='.', mec='xkcd:dark pink', color=(plot_color_scheme['RawTP9']),
      label='TP9')
    axs[0].plot(x_series, af7, alpha=0.8, marker='.', mec='xkcd:salmon', color=(plot_color_scheme['RawAF7']),
      label='AF7')
    axs[0].plot(x_series, af8, alpha=0.8, marker='.', mec='xkcd:cerulean', color=(plot_color_scheme['RawAF8']),
      label='AF8')
    axs[0].plot(x_series, tp10, alpha=0.8, marker='.', mec='xkcd:dark lilac', color=(plot_color_scheme['RawTP10']),
      label='TP10')
    ...
    axs[0].text(0.975, 0.025, '(All Sensor Data Combined)', verticalalignment='bottom',
      horizontalalignment='right',
      transform=(axs[0].transAxes),
      color='green')
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[0], 'RAW_TP10')
    axs[1].plot(x_series, tp9, alpha=1.0, marker='.', mec='xkcd:dark pink', color=(plot_color_scheme['RawTP9']),
      label='TP9')
    ...
    axs[2].plot(x_series, af7, alpha=1.0, marker='.', mec='xkcd:salmon', color=(plot_color_scheme['RawAF7']),
      label='AF7')
    ...
    axs[3].plot(x_series, af8, alpha=1.0, marker='.', mec='xkcd:cerulean', color=(plot_color_scheme['RawAF8']),
      label='AF8')
    ...
    axs[4].plot(x_series, tp10, alpha=1.0, marker='.', mec='xkcd:dark lilac', color=(plot_color_scheme['RawTP10']),
      label='TP10')
    ...
    plt.text(0.175, 6.1, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
    plt.text(1.01, 4.25, ('Mean: ' + '{:.3f}'.format(EEG_Dict['RAW_TP9']['mean']) + '\nStd: ' + '{:.3f}'.format(EEG_Dict['RAW_TP9']['std']) + '\nMin: ' + '{:.3f}'.format(EEG_Dict['RAW_TP9']['min']) + '\nMax: ' + '{:.3f}'.format(EEG_Dict['RAW_TP9']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 3.0, ('Mean: ' + '{:.3f}'.format(EEG_Dict['RAW_AF7']['mean']) + '\nStd: ' + '{:.3f}'.format(EEG_Dict['RAW_AF7']['std']) + '\nMin: ' + '{:.3f}'.format(EEG_Dict['RAW_AF7']['min']) + '\nMax: ' + '{:.3f}'.format(EEG_Dict['RAW_AF7']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 1.75, ('Mean: ' + '{:.3f}'.format(EEG_Dict['RAW_AF8']['mean']) + '\nStd: ' + '{:.3f}'.format(EEG_Dict['RAW_AF8']['std']) + '\nMin: ' + '{:.3f}'.format(EEG_Dict['RAW_AF8']['min']) + '\nMax: ' + '{:.3f}'.format(EEG_Dict['RAW_AF8']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 0.5, ('Mean: ' + '{:.3f}'.format(EEG_Dict['RAW_TP10']['mean']) + '\nStd: ' + '{:.3f}'.format(EEG_Dict['RAW_TP10']['std']) + '\nMin: ' + '{:.3f}'.format(EEG_Dict['RAW_TP10']['min']) + '\nMax: ' + '{:.3f}'.format(EEG_Dict['RAW_TP10']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.savefig(plot_fname, dpi=300)
...
def plot_sensor_data_single(tp9, af7, af8, tp10, data_fname, plot_fname, date_time_now, title, data_stats, analysis_parms, fig_num):
    if Verbosity > 0:
    ...
    y_limits = [data_min - clip_padding, data_max + clip_padding]
    line_alpha = 0.5
    axs.plot(x_series, tp9, alpha=line_alpha, marker='.', mec='xkcd:dark pink', color=(plot_color_scheme['RawTP9']),
      label='TP9')
    axs.plot(x_series, af7, alpha=line_alpha, marker='.', mec='xkcd:salmon', color=(plot_color_scheme['RawAF7']),
      label='AF7')
    axs.plot(x_series, af8, alpha=line_alpha, marker='.', mec='xkcd:cerulean', color=(plot_color_scheme['RawAF8']),
      label='AF8')
    axs.plot(x_series, tp10, alpha=line_alpha, marker='.', mec='xkcd:dark lilac', color=(plot_color_scheme['RawTP10']),
      label='TP10')
...
def plot_all_power_bands(delta, theta, alpha, beta, gamma, lowcut, highcut, fs, point_sz, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    plot_alpha = 0.95
    if Verbosity > 0:
    ...
    axs[0].xaxis.set_major_locator(ticker.AutoLocator())
    axs[0].xaxis.set_minor_locator(ticker.AutoMinorLocator())
    l0 = axs[0].plot(x_series, gamma, color=(plot_color_scheme['Gamma']), marker='.', mec='xkcd:dark pink', alpha=plot_alpha,
      label='Gamma')
    axs[0].legend(loc='upper right', prop={'size': 6})
    axs[0].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[0], 'Gamma_TP10')
    l1 = axs[1].plot(x_series, beta, color=(plot_color_scheme['Beta']), marker='.', mec='xkcd:dark teal', alpha=plot_alpha,
      label='Beta')
    axs[1].legend(loc='upper right', prop={'size': 6})
    axs[1].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[1], 'Beta_TP10')
    l2 = axs[2].plot(x_series, alpha, color=(plot_color_scheme['Alpha']), marker='.', mec='xkcd:dark brown', alpha=plot_alpha,
      label='Alpha')
    axs[2].legend(loc='upper right', prop={'size': 6})
    axs[2].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[2], 'Alpha_TP10')
    l3 = axs[3].plot(x_series, theta, color=(plot_color_scheme['Theta']), marker='.', mec='xkcd:crimson', alpha=plot_alpha,
      label='Theta')
    axs[3].legend(loc='upper right', prop={'size': 6})
    axs[3].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[3], 'Theta_TP10')
    l4 = axs[4].plot(x_series, delta, color=(plot_color_scheme['Delta']), marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Delta')
    axs[4].legend(loc='upper right', prop={'size': 6})
    axs[4].grid(True)
    if gui_dict['checkBoxDataMarkers']:
    ...
    plt.text(1.01, 5.5, ('Mean: ' + '{:.3f}'.format(data_stats['gamma']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['gamma']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['gamma']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['gamma']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 4.25, ('Mean: ' + '{:.3f}'.format(data_stats['beta']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['beta']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['beta']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['beta']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 3.0, ('Mean: ' + '{:.3f}'.format(data_stats['alpha']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['alpha']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['alpha']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['alpha']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 1.75, ('Mean: ' + '{:.3f}'.format(data_stats['theta']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['theta']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['theta']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['theta']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 0.5, ('Mean: ' + '{:.3f}'.format(data_stats['delta']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['delta']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['delta']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['delta']['max'])),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(0.175, 6.1, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
...
def plot_all_power_bands_single(delta, theta, alpha, beta, gamma, lowcut, highcut, fs, point_sz, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    plot_alpha = 0.75
    if Verbosity > 0:
    ...
    axs.xaxis.set_major_locator(ticker.AutoLocator())
    axs.xaxis.set_minor_locator(ticker.AutoMinorLocator())
    l0 = axs.plot(x_series, gamma, color=(plot_color_scheme['Gamma']), marker='.', mec='xkcd:dark pink', alpha=plot_alpha,
      label='Gamma')
    l1 = axs.plot(x_series, beta, color=(plot_color_scheme['Beta']), marker='.', mec='xkcd:dark teal', alpha=plot_alpha,
      label='Beta')
    l2 = axs.plot(x_series, alpha, color=(plot_color_scheme['Alpha']), marker='.', mec='xkcd:dark brown', alpha=plot_alpha,
      label='Alpha')
    l3 = axs.plot(x_series, theta, color=(plot_color_scheme['Theta']), marker='.', mec='xkcd:crimson', alpha=plot_alpha,
      label='Theta')
    l4 = axs.plot(x_series, delta, color=(plot_color_scheme['Delta']), marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Delta')
    if gui_dict['checkBoxDataMarkers']:
    ...
    plt.text(1.01, 0.5, ('Mean: ' + '{:.3f}'.format(data_stats['delta']['mean']) + '\nStd: ' + '{:.3f}'.format(data_stats['delta']['std']) + '\nMin: ' + '{:.3f}'.format(data_stats['delta']['min']) + '\nMax: ' + '{:.3f}'.format(data_stats['delta']['max'])),
      ...
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(0.175, 1.1, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
...
def plot_sensor_power_bands(delta, theta, alpha, beta, gamma, lowcut, highcut, fs, point_sz, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    plot_alpha = 0.8
    if Verbosity > 0:
...
def plot_combined_power_bands(delta_raw, theta_raw, alpha_raw, beta_raw, gamma_raw, delta, theta, alpha, beta, gamma, lowcut, highcut, fs, point_sz, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    if Verbosity > 0:
    ...
    plt_axes = plt.gca()
    y_limits = [
     data_stats['gamma']['min'], data_stats['gamma']['max']]
    t_len = len(delta)
    period = 1.0 / Sampling_Rate
    x_series = np.arange(0, t_len * period, period)
    axs[0].set(title=title)
    axs[0].xaxis.set_major_locator(ticker.AutoLocator())
    axs[0].xaxis.set_minor_locator(ticker.AutoMinorLocator())
    l0 = axs[0].plot(x_series, gamma_raw, color=(plot_color_scheme['Gamma']), marker='.', mec='xkcd:dark pink', alpha=plot_alpha,
      label='Gamma Raw')
    l00 = axs[0].scatter(x_series, gamma, s=1, color=(plot_color_scheme['Gamma']), alpha=plot_alpha,
      label='Gamma')
    axs[0].legend(loc='upper right', prop={'size': 6})
    axs[0].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[0], 'Gamma_TP10')
    l1 = axs[1].plot(x_series, beta_raw, color=(plot_color_scheme['Beta']), marker='.', mec='xkcd:dark teal', alpha=plot_alpha,
      label='Beta Raw')
    l11 = axs[1].scatter(x_series, beta, s=1, color=(plot_color_scheme['Beta']), alpha=plot_alpha,
      label='Beta')
    axs[1].legend(loc='upper right', prop={'size': 6})
    axs[1].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[1], 'Beta_TP10')
    l2 = axs[2].plot(x_series, alpha_raw, color=(plot_color_scheme['Alpha']), marker='.', mec='xkcd:dark brown', alpha=plot_alpha,
      label='Alpha Raw')
    l22 = axs[2].scatter(x_series, alpha, s=1, color=(plot_color_scheme['Alpha']), alpha=plot_alpha,
      label='Alpha')
    axs[2].legend(loc='upper right', prop={'size': 6})
    axs[2].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[2], 'Alpha_TP10')
    l3 = axs[3].plot(x_series, theta_raw, color=(plot_color_scheme['Theta']), marker='.', mec='xkcd:crimson', alpha=plot_alpha,
      label='Theta Raw')
    l33 = axs[3].scatter(x_series, theta, s=1, color=(plot_color_scheme['Theta']), alpha=plot_alpha,
      label='Theta')
    axs[3].legend(loc='upper right', prop={'size': 6})
    axs[3].grid(True)
    if gui_dict['checkBoxDataMarkers']:
        generate_data_markers(muse_EEG_data, axs[3], 'Theta_TP10')
    l4 = axs[4].plot(x_series, delta_raw, color=(plot_color_scheme['Delta']), marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Delta Raw')
    l44 = axs[4].scatter(x_series, delta, s=1, color=(plot_color_scheme['Delta']), alpha=plot_alpha,
      label='Delta')
    axs[4].legend(loc='upper right', prop={'size': 6})
    axs[4].grid(True)
    if gui_dict['checkBoxDataMarkers']:
...
def plot_mellow_concentration(mellow, concentration, lowcut, highcut, fs, point_sz, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    plot_alpha = 0.9
    if Verbosity > 0:
    ...
    axs[0].xaxis.set_major_locator(ticker.AutoLocator())
    axs[0].xaxis.set_minor_locator(ticker.AutoMinorLocator())
    l0 = axs[0].plot(x_series, mellow, color='b', alpha=plot_alpha,
      marker='.',
      mec='xkcd:wine',
      label='Mellow')
    axs[0].legend(loc='upper right', prop={'size': 6})
    ...
    l1 = axs[1].plot(x_series, concentration, color='g', alpha=plot_alpha,
      marker='.',
      mec='xkcd:wine',
      label='Concentration')
    axs[1].legend(loc='upper right', prop={'size': 6})
    ...
    plt.text(1.01, 1.95, ('Mean: ' + '{:.3f}'.format(mellow_mean) + '\nStd: ' + '{:.3f}'.format(mellow_std) + '\nMin: ' + '{:.3f}'.format(mellow_min) + '\nMax: ' + '{:.3f}'.format(mellow_max)),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(1.01, 0.75, ('Mean: ' + '{:.3f}'.format(concentration_mean) + '\nStd: ' + '{:.3f}'.format(concentration_std) + '\nMin: ' + '{:.3f}'.format(concentration_min) + '\nMax: ' + '{:.3f}'.format(concentration_max)),
      style='italic', transform=(plt_axes.transAxes),
      bbox={'facecolor':'blue', 
     'alpha':0.05,  'pad':1})
    plt.text(0.175, 2.3, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
    ...
    basename = os.path.basename(data_fname)
    create_file_date_text(0.0, -0.225, 0.0, -0.15, plt_axes, basename, date_time_now)
    plt.savefig(plot_fname, dpi=300)
    if gui_dict['checkBoxInteractive']:
        plt.show()
    plt.close()
...
def plot_accel_gryo_data(acc_gyro_df, title, data_fname, plot_fname, date_time_now, analysis_parms, fig_num):
    ...
    period = 1.0 / Sampling_Rate
    x_series = np.arange(0, t_len * period, period)
    fig, axs = plt.subplots(nrows=6, num=fig_num, figsize=FIGURE_SIZE, dpi=PLOT_DPI,
      facecolor='w',
      edgecolor='k',
      sharex=True,
      gridspec_kw={'hspace': 0.25},
    ...
    axs[0].xaxis.set_major_locator(ticker.AutoLocator())
    axs[0].xaxis.set_minor_locator(ticker.AutoMinorLocator())
    l0 = axs[0].plot(x_series, (acc_gyro_df['Accelerometer_X']), color='r', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='X')
    axs[0].legend(loc='upper right', prop={'size': 6})
    axs[0].grid(True)
    axs[0].set_ylim((-1.5, 1.5))
    axs[0].set(ylabel='Accelerometer X')
    l1 = axs[1].plot(x_series, (acc_gyro_df['Accelerometer_Y']), color='g', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Y')
    axs[1].legend(loc='upper right', prop={'size': 6})
    axs[1].grid(True)
    axs[1].set_ylim((-1.5, 1.5))
    axs[1].set(ylabel='Accelerometer Y')
    l2 = axs[2].plot(x_series, (acc_gyro_df['Accelerometer_Z']), color='b', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Z')
    axs[2].legend(loc='upper right', prop={'size': 6})
    axs[2].grid(True)
    ...
    l3 = axs[3].plot(x_series, (acc_gyro_df['Gyro_X']), color='#FF3AAA', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Pitch')
    axs[3].legend(loc='upper right', prop={'size': 6})
    axs[3].grid(True)
    axs[3].set_ylim((-50.0, 50.0))
    axs[3].set(ylabel='Gyro Pitch')
    l4 = axs[4].plot(x_series, (acc_gyro_df['Gyro_Y']), color='#3affe5', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Yaw')
    axs[4].legend(loc='upper right', prop={'size': 6})
    axs[4].grid(True)
    axs[4].set_ylim((-50.0, 50.0))
    axs[4].set(ylabel='Gyro Yaw')
    l5 = axs[5].plot(x_series, (acc_gyro_df['Gyro_Z']), color='#FFC73A', marker='.', mec='xkcd:wine', alpha=plot_alpha,
      label='Roll')
    axs[5].legend(loc='upper right', prop={'size': 6})
    axs[5].grid(True)
    ...
    plt.text(0.175, 7.35, ('Session Date: ' + session_dict['Session_Data']['session_date']), transform=(plt_axes.transAxes),
      style='italic',
      horizontalalignment='right',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
...
def calculate_power_stats(delta, theta, alpha, beta, gamma):
    ...
    data_stats = {'delta':{'min':delta_min, 
      'max':delta_max,  'std':delta_std,  'mean':delta_mean}, 
     'theta':{'min':theta_min, 
      'max':theta_max,  'std':theta_std,  'mean':theta_mean}, 
     'alpha':{'min':alpha_min, 
      'max':alpha_max,  'std':alpha_std,  'mean':alpha_mean}, 
     'beta':{'min':beta_min, 
      'max':beta_max,  'std':beta_std,  'mean':beta_mean}, 
     'gamma':{'min':gamma_min, 
      'max':gamma_max,  'std':gamma_std,  'mean':gamma_mean}}
    if Verbosity > 2:
    ...
    for index, row in elements_df.iterrows():
        if Verbosity > 5:
            ...
            if 'jaw' in row['Elements']:
                ...
                if 'blink' in row['Elements']:
                    continue
                else:
                    if '/Marker' in row['Elements']:
                        marker_text = row['Elements'][8:]
                    else:
            ...
            if col_select == 'No Offset':
                ...
                if col_select == 'Coherence':
                    y_offset = 10
                    y_offset_txt = 2
                else:
                    if col_select == 'Accelerometer_X':
                        y_offset = np.max(new_df[index:index + 10])
                        y_offset_txt = y_offset + 0.1
                    else:
                        if col_select == 'Gyro_X':
                            y_offset = np.max(new_df[index:index + 10])
        ...
        axs.annotate(marker_text, xy=(index / Sampling_Rate, y_offset), xytext=(
         index / Sampling_Rate + 2, y_offset_txt),
          bbox=dict(boxstyle='round', alpha=0.1),
          ha='right',
          va='center',
          rotation=33,
          size=8,
          arrowprops=dict(arrowstyle='simple', color='blue', alpha=0.5, connectionstyle='arc3, rad=0.03'))

    return True


def create_file_date_text(x1, y1, x2, y2, plt_axes, data_fname, date_time_now):
    plt.text(x1, y1, ('file: ' + data_fname), transform=(plt_axes.transAxes),
      style='italic',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
    plt.text(x2, y2, ('Date: ' + date_time_now), transform=(plt_axes.transAxes),
      style='italic',
      bbox={'facecolor':'blue', 
     'alpha':0.1,  'pad':1})
    return True
...
def create_analysis_parms_text(x, y, plt_axes, analysis_parms):
    if gui_dict['checkBoxFilter']:
        ...
        text_string = 'Sample Time: ' + '{:.2f}'.format(analysis_parms['sample_time_min']) + ' (minutes) ' + '{:.2f}'.format(analysis_parms['sample_time_sec']) + ' (seconds)' + '\nSample Rate: ' + '{:.2f}'.format(Sampling_Rate) + '  Sample Length: ' + '{:d}'.format(analysis_parms['sample_length']) + '\nFilter Type: ' + filter_type_name + '  Filter Order: ' + '{:.1f}'.format(analysis_parms['filter_order']) + '\nLow Cut: ' + '{:.1f}'.format(analysis_parms['lowcut']) + ' HZ ' + ' High Cut: ' + '{:.1f}'.format(analysis_parms['highcut']) + ' HZ '
    else:
        text_string = 'Sample Time: ' + '{:.2f}'.format(analysis_parms['sample_time_min']) + ' (minutes) ' + '{:.2f}'.format(analysis_parms['sample_time_sec']) + ' (seconds)' + '\nSample Rate: ' + '{:.2f}'.format(Sampling_Rate) + ' Sample Length: ' + '{:d}'.format(analysis_parms['sample_length'])
    plt.text(x, y, text_string, style='italic',
...
def generate_plots(muse_EEG_data, data_fname, date_time_now):
    ...
    analysis_parms = {'lowcut':Filter_Lowcut, 
     ...
     'filter_order':Filter_Order,  'sample_length':sample_length, 
     'sample_time_sec':sample_time_sec,  'sample_time_min':sample_time_min}
    point_sz = 1
    data_stats = (
     EEG_Dict['RAW_AF7']['25%'], EEG_Dict['RAW_AF7']['75%'],
                ...
                if args.plot_style == 4:
                    ...
                    if args.plot_style == 5:
                        ...
                        if gui_dict['checkBoxEEG_PowerBands']:
                            plot_all(muse_EEG_data, data_fname, out_dirname + '/plots/22-ABCS_all_' + date_time_now + '.png', date_time_now, 'EEG Raw & Power Bands', data_stats, analysis_parms, 99)
                        if gui_dict['checkBoxEEG']:
                            plot_sensor_data(df['RAW_TP9'], df['RAW_AF7'], df['RAW_AF8'], df['RAW_TP10'], data_fname, out_dirname + '/plots/20-ABCS_eeg_raw_' + date_time_now + '.png', date_time_now, 'Raw EEG', data_stats, analysis_parms, 20)
                            if Verbosity > 4:
                                print('generate_plots() - Calling smooth_data() before calling plot_sensor_data_single()')
                            plot_sensor_data_single(smooth_data(df['RAW_TP9'].copy(), smooth_sz), smooth_data(df['RAW_AF7'].copy(), smooth_sz), smooth_data(df['RAW_AF8'].copy(), smooth_sz), smooth_data(df['RAW_TP10'].copy(), smooth_sz), data_fname, out_dirname + '/plots/25-ABCS_eeg_smoothed_single_' + date_time_now + '.png', date_time_now, 'Smoothed EEG', data_stats, analysis_parms, 25)
                            if Verbosity > 4:
                                print('generate_plots() - Calling smooth_data() before calling plot_sensor_data()')
                            plot_sensor_data(smooth_data(df['RAW_TP9'].copy(), smooth_sz), smooth_data(df['RAW_AF7'].copy(), smooth_sz), smooth_data(df['RAW_AF8'].copy(), smooth_sz), smooth_data(df['RAW_TP10'].copy(), smooth_sz), data_fname, out_dirname + '/plots/21-ABCS_eeg_smoothed_' + date_time_now + '.png', date_time_now, 'Smoothed EEG', data_stats, analysis_parms, 21)
                        if gui_dict['checkBoxCoherence']:
                            if gui_dict['checkBoxSmoothData']:
                                plot_coherence_scatter(smooth_data(df['RAW_AF7'].copy(), smooth_sz), smooth_data(df['RAW_AF8'].copy(), smooth_sz), smooth_data(df['RAW_TP9'].copy(), smooth_sz), smooth_data(df['RAW_TP10'].copy(), smooth_sz), 'Raw Data - Coherence', data_fname, out_dirname + '/plots/10-ABCS_eeg_raw_coherence_data_' + date_time_now + '.png', date_time_now, analysis_parms, 10)
                                plot_coherence_data(smooth_data(df['RAW_AF7'].copy(), smooth_sz), smooth_data(df['RAW_AF8'].copy(), smooth_sz), smooth_data(df['RAW_TP9'].copy(), smooth_sz), smooth_data(df['RAW_TP10'].copy(), smooth_sz), data_fname, out_dirname + '/plots/12-ABCS_eeg_coherence_' + date_time_now + '.png', date_time_now, 'EEG Coherence', data_stats, analysis_parms, 12)
                            else:
                                plot_coherence_scatter(df['RAW_AF7'], df['RAW_AF8'], df['RAW_TP9'], df['RAW_TP10'], 'Raw Data - Coherence', data_fname, out_dirname + '/plots/10-ABCS_eeg_raw_coherence_data_' + date_time_now + '.png', date_time_now, analysis_parms, 10)
                                plot_coherence_data(df['RAW_TP9'], df['RAW_AF7'], df['RAW_AF8'], df['RAW_TP10'], data_fname, out_dirname + '/plots/12-ABCS_eeg_coherence_' + date_time_now + '.png', date_time_now, 'EEG Coherence', data_stats, analysis_parms, 12)
                        if gui_dict['checkBoxPowerBands']:
                            ...
                            gamma_df = pd.DataFrame(muse_EEG_data, columns=[
                             'Gamma_TP9', 'Gamma_AF7', 'Gamma_AF8', 'Gamma_TP10'])
                            plot_sensor_power_bands(delta_df, theta_df, alpha_df, beta_df, gamma_df, Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands (All Sensors-Raw)', data_fname, out_dirname + '/plots/30-ABCS_all_sensors_power_raw_' + date_time_now + '.png', date_time_now, analysis_parms, 30)
                            if gui_dict['checkBoxSmoothData']:
                                ...
                                plot_all_power_bands(sm_delta_df, sm_theta_df, sm_alpha_df, sm_beta_df, sm_gamma_df, Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands (Mean Average - Smoothed)', data_fname, out_dirname + '/plots/31.1-ABCS_power_smooth_mean_' + date_time_now + '.png', date_time_now, analysis_parms, 31)
                                plot_all_power_bands_single(sm_delta_df, sm_theta_df, sm_alpha_df, sm_beta_df, sm_gamma_df, Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands (Mean Average - Smoothed)', data_fname, out_dirname + '/plots/31.1-ABCS_power_smooth_mean_' + date_time_now + '.png', date_time_now, analysis_parms, 31)
                            else:
                                plot_all_power_bands(delta_df.mean(axis=1), theta_df.mean(axis=1), alpha_df.mean(axis=1), beta_df.mean(axis=1), gamma_df.mean(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands (Mean Average)', data_fname, out_dirname + '/plots/31-ABCS_power_mean_' + date_time_now + '.png', date_time_now, analysis_parms, 31)
                                plot_all_power_bands_single(delta_df.mean(axis=1), theta_df.mean(axis=1), alpha_df.mean(axis=1), beta_df.mean(axis=1), gamma_df.mean(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands (Mean Average)', data_fname, out_dirname + '/plots/31-ABCS_power_mean_' + date_time_now + '.png', date_time_now, analysis_parms, 31)
                            if gui_dict['checkBoxStatistical']:
                                if gui_dict['checkBoxSmoothData']:
                                    plot_combined_power_bands(sm_delta_df.mean(axis=1), sm_theta_df.mean(axis=1), sm_alpha_df.mean(axis=1), sm_beta_df.mean(axis=1), sm_gamma_df.mean(axis=1), sm_delta_df.median(axis=1), sm_theta_df.median(axis=1), sm_alpha_df.median(axis=1), sm_beta_df.median(axis=1), sm_gamma_df.median(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands Mean & Median', data_fname, out_dirname + '/plots/32-ABCS_power_bands_median_mean' + date_time_now + '.png', date_time_now, analysis_parms, 32)
                                    plot_combined_power_bands(delta_df, theta_df, alpha_df, beta_df, gamma_df, sm_delta_df.mean(axis=1), sm_theta_df.mean(axis=1), sm_alpha_df.mean(axis=1), sm_beta_df.mean(axis=1), sm_gamma_df.mean(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands Mean (Smoothed) & Raw', data_fname, out_dirname + '/plots/33-ABCS_power_bands_raw_mean' + date_time_now + '.png', date_time_now, analysis_parms, 33)
                                else:
                                    plot_combined_power_bands(delta_df.mean(axis=1), theta_df.mean(axis=1), alpha_df.mean(axis=1), beta_df.mean(axis=1), gamma_df.mean(axis=1), delta_df.median(axis=1), theta_df.median(axis=1), alpha_df.median(axis=1), beta_df.median(axis=1), gamma_df.median(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands Mean & Median', data_fname, out_dirname + '/plots/32-ABCS_power_bands_median_mean' + date_time_now + '.png', date_time_now, analysis_parms, 32)
                                    plot_combined_power_bands(delta_df, theta_df, alpha_df, beta_df, gamma_df, delta_df.mean(axis=1), theta_df.mean(axis=1), alpha_df.mean(axis=1), beta_df.mean(axis=1), gamma_df.mean(axis=1), Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Power Bands Mean & Raw', data_fname, out_dirname + '/plots/33-ABCS_power_bands_raw_mean' + date_time_now + '.png', date_time_now, analysis_parms, 33)
                        if gui_dict['checkBoxAccelGyro']:
                            acc_gyro_df = pd.DataFrame(muse_EEG_data, columns=[
                             'Accelerometer_X', 'Accelerometer_Y', 'Accelerometer_Z',
                             'Gyro_X', 'Gyro_Y', 'Gyro_Z'])
                            plot_accel_gryo_data(acc_gyro_df, 'Accelerometer/Gyro', data_fname, out_dirname + '/plots/40-ABCS_accel_gyro_' + date_time_now + '.png', date_time_now, analysis_parms, 40)
                        if gui_dict['checkBoxMellowConcentration']:
                            ...
                            if 'Mellow' in muse_EEG_data.keys():
                                plot_mellow_concentration(mc_df['Mellow'], mc_df['Concentration'], Filter_Lowcut, Filter_Highcut, Sampling_Rate, point_sz, 'Mellow/Concentration', data_fname, out_dirname + '/plots/50-ABCS_accel_gyro_' + date_time_now + '.png', date_time_now, analysis_parms, 50)
                            else:
...
def initialize_GUI_vars(date_time_now):
    ...
    gui_dict.update({'firstName':'',  'lastName':'',  'session_notes':'', 
...
def main(date_time_now):
    ...
    if Verbosity > 1:
        ...
        if my_rc_file.is_file():
            ...
            Filter_Highcut = rc_obj['Filter HighCut']
            Filter_LowCut = rc_obj['Filter LowCut']
            Filter_Order = rc_obj['Filter Order']
            Filter_Type = rc_obj['Filter Type']
    ...
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['RAW_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['RAW_TP10'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_TP9'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_AF7'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_TP10'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Theta_TP9'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Theta_AF7'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Theta_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Theta_TP10'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Alpha_TP9'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Alpha_AF7'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Alpha_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Alpha_TP10'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Beta_TP9'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Beta_AF7'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Beta_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Beta_TP10'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_TP9'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_AF7'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_AF8'].isnull())]
    muse_EEG_data = muse_EEG_data[(~muse_EEG_data['Delta_TP10'].isnull())]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pm', '--plot_markers', help='Add Plot Markers', action='store_true')
    ...
    parser.add_argument('-ep', '--eeg_power', help='Plot EEG & Power Data Combined', action='store_true')
    ...
    parser.add_argument('-mc', '--mellow_concentration', help='Plot Mellow and Concentration Data (Only For Mind Monitor Data)',
      action='store_true')
    ...
    parser.add_argument('--plot_style', help='Plot Syle: 1=seaborn, 2=seaborn-pastel, 3=seaborn-ticks, 4=fast, 5=bmh',
      type=int)
    ...
    parser.add_argument('-ft', '--filter_type', help='Filter Type 0=default 1=low pass, 2=bandpass',
      type=int)
    ...
    parser.add_argument('-db', '--data_base', help='Send session data and statistics to database',
      action='store_true')
    args = parser.parse_args()
    if args.version:
        print('Current version: ', analyze_muse.ABCS_version.ABCS_version)
        sys.exit(0)
    else:
        if args.verbose:
        ...
        if args.plot_style:
            ...
            if args.data_markers:
                ...
                if args.plot_markers:
                    if Verbosity > 0:
                        print('plot_markers:')
                        print(args.plot_markers)
            else:
                args.plot_markers = False
            if args.eeg_power:
                if Verbosity > 0:
                    print('eeg_power:')
                    print(args.eeg_power)
        else:
            args.eeg_power = False
        if args.batch:
            if Verbosity > 0:
                print('batch:')
            print(args.batch)
