...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spamfighter/txjsonrpc/web/jsonrpc.py
# Compiled at: 2009-01-29 04:03:45
...
class JSONRPC(resource.Resource):
    ...
    def render(self, request):
        ...
        try:
            ...
            defer.maybeDeferred(function, *args).addErrback(self._ebRender, requestId).addCallback(self._cbRender, request, requestId)
    ...
    def _ebRender(self, failure, requestId):
        if isinstance(failure.value, jsonrpclib.Fault):
            failure.value.requestId = requestId
            return failure.value
        log.err(failure)
        return jsonrpclib.Fault(self.FAILURE, 'error', requestId=requestId)
    ...
    def _getFunction(self, functionPath):
        ...
        if functionPath.find(self.separator) != -1:
            ...
            if handler is None:
                raise jsonrpclib.NoSuchFunction(jsonrpclib.METHOD_NOT_FOUND, 'no such sub-handler %s' % prefix)
            return handler._getFunction(functionPath)
        else:
            f = getattr(self, 'jsonrpc_%s' % functionPath, None)
            if not f:
                raise jsonrpclib.NoSuchFunction(jsonrpclib.METHOD_NOT_FOUND, 'function %s not found' % functionPath)
            elif not callable(f):
                raise jsonrpclib.NoSuchFunction(jsonrpclib.METHOD_NOT_CALLABLE, 'function %s not callable' % functionPath)
            else:
                return f
            return
...
class JSONRPCIntrospection(JSONRPC):
    ...
    jsonrpc_methodSignature.signature = [
     [
      'array', 'string'],
     [
      'string', 'string']]
...
class QueryFactory(protocol.ClientFactory):
    ...
    def __init__(self, path, host, method, user=None, password=None, *args):
        ...
        self.payload = jsonrpclib.dumps({'method': method, 
           'params': args})
...
class Proxy:
    ...
    def callRemote(self, method, *args):
        factory = QueryFactory(self.path, self.host, method, self.user, self.password, *args)
        if self.secure:
