...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spamfighter/service.py
...
def makeService():
    ...
    if config.http.enabled == 'yes' or config.https.enabled == 'yes':
        ...
        if config.https.enabled == 'yes':
            ...
            h = internet.SSLServer(config.https.port, server.Site(root), ssl.DefaultOpenSSLContextFactory(config.https.cert.private_key_file, config.https.cert.certificate_file))
            h.setServiceParent(s)
    if config.manhole.enabled == 'yes':
        ...
        f.protocol = lambda : TelnetTransport(TelnetBootstrapProtocol, insults.ServerProtocol, ColoredManhole, None)
