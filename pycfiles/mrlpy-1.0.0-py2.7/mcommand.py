...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/mrlpy/mcommand.py
# Compiled at: 2017-08-14 14:54:36
...
"""
import websocket, logging, signal
try:
    import thread
except ImportError:
    import _thread

import time, os, sys, threading, json, requests
...
def connect(bypassRegisters=False, forceReconnect=False):
    ...
    if socket == None or socket.sock == None or forceReconnect:
        if bypassRegisters:
            ...
            socket = websocket.WebSocketApp('ws://' + MRL_URL + ':' + MRL_PORT + '/api/messages', on_message=on_message, on_error=on_error, on_close=on_close)
...
def isSequence(arg):
    """
        ...
        """
    return not hasattr(arg, 'strip') and not hasattr(arg, 'values') and (hasattr(arg, '__getitem__') or hasattr(arg, '__iter__'))
