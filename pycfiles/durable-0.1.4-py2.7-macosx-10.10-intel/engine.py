...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/durable/engine.py
# Compiled at: 2015-01-24 14:25:06
import json, copy, rules, threading, inspect, random
    ...
    def _transform(self, chart_definition, rules):
        ...
        for stage_name, stage in chart_definition.iteritems():
            ...
            if 'to' in stage:
                if isinstance(stage['to'], basestring):
                    ...
                    elif isinstance(next_stage['run'], basestring):
                    ...
                    for transition_name, transition in stage['to'].iteritems():
                        ...
                        if 'all' in transition:
                            ...
                            rule['all'] = [
                             stage_test, {'m$any': transition['any']}]
                        else:
                            rule['all'] = [
                             stage_test]
        ...
        for stage_name, stage in chart_definition.iteritems():
            if stage_name not in visited:
                ...
                elif isinstance(stage['run'], basestring):
