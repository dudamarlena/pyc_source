...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/durable/interface.py
# Compiled at: 2014-12-29 16:00:53
import os, json, engine
...
class Application(object):
    ...
    def _visual_request(self, environ, start_response):
        middleware = SharedDataMiddleware(self._not_found, {'/': os.path.join(os.path.dirname(__file__), 'ux')})
        return middleware(environ, start_response)

    def _admin_request(self, environ, start_response, ruleset_name, sid):
        middleware = SharedDataMiddleware(self._not_found, {('/{0}/{1}/').format(ruleset_name, sid): os.path.join(os.path.dirname(__file__), 'ux')})
        return middleware(environ, start_response)
