...
# [GCC 8.4.0]
# Embedded file name: /Users/paulmitchell-gears/git/peoplewise-enable/env/lib/python3.5/site-packages/serialization_spec/serialization.py
# Compiled at: 2020-02-21 09:05:31
# Size of source mod 2**32: 10984 bytes
...
class SerializationSpecPlugin:
    __doc__ = ' These methods can access self.key to get the key '
...
def get_only_fields(model, serialization_spec):
    ...
    return [field for field in get_fields(serialization_spec) if field in fields]
...
def make_serializer_class(model, serialization_spec):
    ...
    return type('MySerializer', (
     ModelSerializer,), {**({'Meta': type('Meta', (
              object,), {'model': model, 'fields': get_fields(serialization_spec)})}), **({key:SerializerLambdaField(impl=lambda value: [str(each.id) for each in getattr(value, key).all()]) for key in get_only_fields(model, serialization_spec) if key in relations and relations[key].to_many}), **({key:(SerializerLambdaField(impl=values.get_value) if isinstance(values, SerializationSpecPlugin) else make_serializer_class(relations[field_name].related_model, values)(many=relations[field_name].to_many)) for key, field_name, values in [handle_filtered(item) for each in get_childspecs(serialization_spec) for item in each.items()]})})


def has_plugin(spec):
    return any(isinstance(childspec, SerializationSpecPlugin) or has_plugin(childspec) for each in spec if isinstance(each, dict) for key, childspec in each.items())
                ...
                if isinstance(childspec, SerializationSpecPlugin):
                    ...
                    if relation.model_field and relation.model_field.one_to_one or use_select_related and not relation.to_many and not has_plugin(childspec):
                        ...
                        if relation.reverse and not relation.has_through_model:
                            ...
                            queryset = queryset.prefetch_related(Prefetch(key_path, queryset=inner_queryset, **{'to_attr': to_attr} if to_attr else {}))

        elif each in relations:
...
def expand_nested_specs(serialization_spec, request_user):
    ...
    return serialization_spec + sum([get_serialization_spec(childspec) for each in serialization_spec if isinstance(each, dict) for key, childspec in each.items() if isinstance(childspec, SerializationSpecPlugin)], [])
...
def normalise_spec(serialization_spec):
    ...
    def combine(normalised_spec):
        return normalised_spec.spec or list(normalised_spec.fields.keys()) + ([{key:combine(value) for key, value in normalised_spec.relations.items()}] if normalised_spec.relations else [])
...
SerializationSpec = List[Union[(str,
 Dict[(str,
  Union[(Filtered, SerializationSpecPlugin,
   List[Union[(str,
    Dict[(str,
     Union[(Filtered, SerializationSpecPlugin,
      List[Union[(str,
