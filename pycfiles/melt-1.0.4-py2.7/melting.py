...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/melting.py
# Compiled at: 2016-05-23 22:07:59
"""
melting.py
...
def temp(s, DNA_c=5000.0, Na_c=10.0, Mg_c=20.0, dNTPs_c=10.0, uncorrected=False):
    ...
    dh, ds = _tercorr(s)
    k = DNA_c * 1e-09
    dh_coeffs = {'AA': -7.9, 'TT': -7.9, 'AT': -7.2, 
       'TA': -7.2, 
       'CA': -8.5, 
       'TG': -8.5, 'GT': -8.4, 
       'AC': -8.4, 'CT': -7.8, 
       ...
       'GG': -8.0, 
       'CC': -8.0}
    ds_coeffs = {'AA': -22.2, 'TT': -22.2, 'AT': -20.4, 
       'TA': -21.3, 
       'CA': -22.7, 
       'TG': -22.7, 'GT': -22.4, 
       'AC': -22.4, 'CT': -21.0, 
    ...
    if cation_ratio < 0.22:
        tm = 1 / (1 / tm + ((4.29 * fgc - 3.95) * log(MNa) + 0.94 * log(MNa) ** 2) * 1e-05)
    else:
        a = 3.92
        d = 1.42
        g = 8.31
        ...
        if cation_ratio < 6.0:
            ...
            d = d * (1.279 - 4.03 * log(MNa) * 0.001 - 8.03 * log(MNa) ** 2 * 0.001)
            g = g * (0.486 - 0.258 * log(MNa) + 5.25 * log(MNa) ** 3 * 0.001)
        tm = 1 / (1 / tm + (a - 0.911 * log(Fmg) + fgc * (6.26 + d * log(Fmg)) + 1 / (2 * (len(s) - 1)) * (-48.2 + 52.5 * log(Fmg) + g * log(Fmg) ** 2)) * 1e-05)
    return tm - 273.15
...
def main(argv=sys.argv[1:]):
    ...
    parser.add_argument('seq', help='nucleotide sequence', type=nucleotide_sequence)
    parser.add_argument('--uncorrected', action='store_true', default=False, help='Do not use monovalent/divalent cation corrections')
    parser.add_argument('-d', '--dna', type=float, default=5000.0, help='DNA concentration (nM)')
    parser.add_argument('--na', type=float, default=10.0, help='Na+ concentration (mM)')
    parser.add_argument('--mg', type=float, default=20.0, help='Mg++ concentration (mM)')
    parser.add_argument('--dntp', type=float, default=10.0, help='Nucleotide triphosphate concentration (mM)')
    parser.add_argument('--version', action='version', version=get_distribution('melt').version)
    args = parser.parse_args(argv)
    Tm = temp(args.seq, DNA_c=args.dna, Na_c=args.na, Mg_c=args.mg, dNTPs_c=args.dntp, uncorrected=args.uncorrected)
