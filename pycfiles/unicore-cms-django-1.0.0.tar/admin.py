...
# [GCC 8.4.0]
# Embedded file name: /Users/sdehaan/Documents/Repositories/unicore-cms-django/cms/admin.py
# Compiled at: 2014-10-23 14:28:01
import json
...
from cms.models import Post, Category, Localisation, ContentRepository, PublishingTarget
...
class PostSourceListFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return Post.objects.filter(post__isnull=False).values_list('slug', 'title')
...
class CategorySourceListFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        return Category.objects.filter(category__isnull=False).distinct().values_list('slug', 'title')
...
class TranslatableModelAdmin(admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        ...
        if obj is not None:
            ...
            extra_context['sourceform'] = helpers.AdminForm(form, self.get_fieldsets(request, obj), self.get_prepopulated_fields(request, obj), self.get_readonly_fields(request, obj), model_admin=self)
        return super(TranslatableModelAdmin, self).add_view(request, form_url, extra_context)
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        if obj is None:
            raise Http404('Post object with primary key %(key)r does not exist.' % {'key': escape(object_id)})
        if obj.source:
            ModelForm = self.get_form(request, obj.source)
            form = ModelForm(instance=obj.source)
            extra_context['sourceform'] = helpers.AdminForm(form, self.get_fieldsets(request, obj.source), self.get_prepopulated_fields(request, obj.source), self.get_readonly_fields(request, obj.source), model_admin=self)
        return super(TranslatableModelAdmin, self).change_view(request, object_id, form_url, extra_context)


class PostAdmin(TranslatableModelAdmin):
    form = PostForm
    list_display = ('title', 'subtitle', 'primary_category', 'created_at', 'localisation',
    ...
    fieldsets = (
     ...
     (
      None,
      {'fields': ('primary_category', 'localisation', 'featured_in_category', 'featured', 'related_posts')}),
     (
      'Meta',
...
class CategoryAdmin(TranslatableModelAdmin):
    form = CategoryForm
    list_filter = (
     'localisation', CategorySourceListFilter)
    list_display = ('title', 'subtitle', 'localisation', 'featured_in_navbar', 'source',
                    '_derivatives', 'uuid')
...
class ContentRepositoryAdmin(admin.ModelAdmin):
    ...
    def get_object(self, request, object_id):
        obj = super(ContentRepositoryAdmin, self).get_object(request, object_id)
        if obj is None:
            return
        else:
...
def my_view(request, *args, **kwargs):
    ...
    context = {'github_url': settings.GIT_REPO_URL, 
       'repo': workspace.repo, 
       'commits': [ {'message': c.message, 'author': c.author.name, 'commit_time': datetime.fromtimestamp(c.committed_date)} for c in commits
                ]}
...
def push_to_github(request, *args, **kwargs):
    tasks.push_to_git.delay(settings.GIT_REPO_PATH, settings.ELASTIC_GIT_INDEX_PREFIX)
    if request.is_ajax():
