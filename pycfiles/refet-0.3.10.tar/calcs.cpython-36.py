...
# [GCC 8.4.0]
# Embedded file name: C:\Projects\RefET\refet\calcs.py
# Compiled at: 2019-02-26 15:00:13
# Size of source mod 2**32: 22666 bytes
...
def _air_pressure(elev, method='asce'):
    ...
    if method == 'asce':
        ...
        if method == 'refet':
            pair += 293
            pair /= 293
            np.power(pair, 5.255114352360781, out=pair)
    pair *= 101.3
    return pair
...
def _es_slope(tmean, method='asce'):
    ...
    """
    if method == 'refet':
        es_slope = 4098.0 * _sat_vapor_pressure(tmean) / np.power(tmean + 237.3, 2)
    else:
        if method == 'asce':
            es_slope = 2503.0 * np.exp(17.27 * tmean / (tmean + 237.3)) / np.power(tmean + 237.3, 2)
    return es_slope
...
def _ra_daily(lat, doy, method='asce'):
    ...
    if method == 'asce':
        ra = 24.0 / math.pi * 4.92 * _dr(doy) * theta
    else:
        if method == 'refet':
            ra = 24.0 / math.pi * 4.9212 * _dr(doy) * theta
    return ra
...
def _ra_hourly(lat, lon, doy, time_mid, method='asce'):
    ...
    theta = (omega2 - omega1) * np.sin(lat) * np.sin(delta) + np.cos(lat) * np.cos(delta) * (np.sin(omega2) - np.sin(omega1))
    if method == 'asce':
        ra = 12.0 / math.pi * 4.92 * _dr(doy) * theta
    else:
        if method == 'refet':
            ra = 12.0 / math.pi * 4.9212 * _dr(doy) * theta
    return ra
...
def _rso_daily(ra, ea, pair, doy, lat):
    ...
    """
    sin_beta_24 = np.sin(0.85 + 0.3 * lat * np.sin(_doy_fraction(doy) - 1.39) - 0.42 * np.power(lat, 2))
    sin_beta_24 = np.maximum(sin_beta_24, 0.1)
    w = _precipitable_water(pair, ea)
    kb = 0.98 * np.exp(-0.00146 * pair / sin_beta_24 - 0.075 * np.power(w / sin_beta_24, 0.4))
...
def _rso_hourly(ra, ea, pair, doy, time_mid, lat, lon, method='asce'):
    ...
    w = _precipitable_water(pair, ea)
    kt = 1.0
    kb = 0.98 * np.exp(-0.00146 * pair / (kt * np.maximum(sin_beta, 0.01)) - 0.075 * np.power(w / np.maximum(sin_beta, 0.01), 0.4))
    kd = np.minimum(-0.36 * kb + 0.35, 0.82 * kb + 0.18)
    rso = ra * (kb + kd)
    return rso
...
def _rnl_daily(tmax, tmin, ea, fcd):
    ...
    """
    return 4.901e-09 * fcd * (0.34 - 0.14 * np.sqrt(ea)) * 0.5 * (np.power(tmax + 273.16, 4) + np.power(tmin + 273.16, 4))
