...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pelican_slug.py
# Compiled at: 2014-02-14 08:49:02
import logging
...
def init(pelican):
    if 'CATEGORY_SLUGS' in pelican.settings:
        replace_urlwrapper_slugify(urlwrappers.Category, pelican.settings['CATEGORY_SLUGS'])
    if 'TAG_SLUGS' in pelican.settings:
        replace_urlwrapper_slugify(urlwrappers.Tag, pelican.settings['TAG_SLUGS'])
...
def make_urlwrapper_patch(slugs):
    ...
    def _normalize_key(self, other):
        return dict_slugify(other.name, self.settings.get('SLUG_SUBSTITUTIONS', ()), slug_dict)
    ...
    def name(self, name):
        self._name = name
        self.slug = dict_slugify(name, self.settings.get('SLUG_SUBSTITUTIONS', ()), slug_dict)

    return (
     __init__, _normalize_key, name)
