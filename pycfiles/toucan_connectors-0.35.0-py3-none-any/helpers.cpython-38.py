...
# [GCC 8.4.0]
# Embedded file name: /home/rachelle2/webprojects/toucan-connectors/toucan_connectors/aircall/helpers.py
# Compiled at: 2020-05-04 10:47:31
...
def resolve_calls_df(team_data, call_data) -> pd.DataFrame:
    ...
    if len(team_data) > 0 and len(call_data):
        df = team_data.merge(call_data, sort=False, on='user_id', how='right').drop(columns=[
         'user_name_y', 'user_created_at']).assign(user_name=(lambda x: x['user_name_x']))
    else:
        if len(call_data) > 0:
            df = call_data
        else:
...
def build_df(dataset: str, list_of_data: List[dict]) -> pd.DataFrame:
    ...
    """
    if dataset == 'users':
        return pd.concat(list_of_data, sort=False, ignore_index=True).drop_duplicates([
         'user_id'],
          keep='first').assign(user_created_at=(lambda x: x['user_created_at'].str[:10]),
    ...
    if dataset == 'calls':
        ...
        total_df = pd.concat([empty_df, df], sort=False, ignore_index=True).assign(answered_at=(lambda t: pd.to_datetime((t['answered_at']), unit='s')),
...
def format_calls_data(call_obj: dict) -> dict:
    ...
    if call_obj:
        return {'id':call_obj.get('id'),  'direction':call_obj.get('direction'), 
...
def format_teams_data(team_obj: dict) -> dict:
    ...
    if team_obj:
        return list(map(lambda user: {'team':team_obj.get('name'), 
         'user_id':user.get('id'), 
         'user_name':user.get('name'), 
         'user_created_at':user.get('created_at')}, team_obj.get('users')))


def format_users_data(user_obj: dict) -> dict:
    """Provides a filter for users"""
    if user_obj:
        return {'user_id':user_obj.get('id'),  'user_name':user_obj.get('name'), 
         'user_created_at':user_obj.get('created_at')}


DICTIONARY_OF_FORMATTERS = {'calls':format_calls_data, 
