...
# [GCC 8.4.0]
# Embedded file name: /home/rachelle2/webprojects/toucan-connectors/toucan_connectors/facebook_insights/facebook_insights_connector.py
...
import facebook, pandas as pd, requests
...
def get_longlived_token(shortlived_token, appid, appsecret):
    ...
    resp = requests.get('https://graph.facebook.com/oauth/access_token',
     ...
     'fb_exchange_token':shortlived_token}).json()
...
def get_page_tokens(longlived_token) -> Dict[(str, str)]:
    ...
    page_tokens = {item['access_token']:item['id'] for item in pages_data['data']}
    return page_tokens


class FacebookInsightsDataSource(ToucanDataSource):
    __doc__ = 'cf. https://developers.facebook.com/docs/graph-api/reference/v2.8/insights\n    '
    pages: Dict[(str, str)]
    metrics: List[str]
    period = 'week'
    period: str
...
class FacebookInsightsConnector(ToucanConnector):
    ...
    def _retrieve_data(self, data_source: FacebookInsightsDataSource) -> pd.DataFrame:
        ...
        for pageid, pagetoken in data_source.pages.items():
            insight = graph.get_object(id=f"{pageid}/insights",
