...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/tests/test_update_form_mixin.py
...
class MyForm(forms.ModelForm):
    first_name = forms.CharField(initial='initial-first-name', required=False)
...
class UpdateFormMixinTests(SpyAgency, TestCase):
    ...
    def test_build_form_errors_with_form(self):
        ...
        form = MyForm(data={})
        self.assertEqual(self.mixin.build_form_errors(form=form), {'username': [
    ...
    def test_build_form_errors_with_validation_error_dict(self):
        ...
        using dictionary
        """
        errors = ValidationError({'field1': [
                    'This field is required.'], 
           'field2': [
                    'This is not a valid choice.']})
        self.assertEqual(self.mixin.build_form_errors(errors=errors), {'field1': [
                    'This field is required.'], 
           'field2': [
                    'This is not a valid choice.']})
    ...
    def test_build_form_errors_with_validation_error_list(self):
        ...
        self.assertEqual(self.mixin.build_form_errors(errors=errors), {'__all__': [
    ...
    def test_build_form_success_response_with_is_created_true(self):
        ...
        response = self.mixin.build_form_success_response(form=form, instance=user, is_created=True)
    ...
    def test_build_form_success_response_with_is_created_false(self):
        ...
        form = MyForm()
        user = User(username='my-user')
        response = self.mixin.build_form_success_response(form=form, instance=user, is_created=False)
        self.assertEqual(response, (
         200,
         {'user': user}))
    ...
    def test_create_form(self):
        ...
        form = self.mixin.create_form(data={'username': 'test-user', 
           'password': 'test-pass'}, request=None, form_kwargs={'custom': True})
    ...
    def test_create_form_with_instance(self):
        ...
        user = User(username='test-user', password='test-pass', first_name='test-first-name', last_name='test-last-name')
        form = self.mixin.create_form(data={'username': 'new-test-user', 
           'last_name': 'new-last-name'}, request=None, instance=user, form_kwargs={'custom': True})
        self.assertTrue(form.custom)
    ...
    def test_create_form_with_parse_field(self):
        ...
        self.mixin.parse_username_field = lambda value, my_state, **kwargs: 'test-%s-%s' % (value, my_state)
        form = self.mixin.create_form(data={'username': 'user', 
           'password': 'test-pass'}, request=None, form_kwargs={'custom': True}, my_state=123)
        self.assertTrue(form.custom)
    ...
    def test_create_form_with_parse_field_raising_error(self):
        ...
        with self.assertRaises(ValidationError) as (ctx):
            self.mixin.create_form(data={'username': 'user', 
               'password': 'pass'}, request=None, form_kwargs={'custom': True})
        self.assertEqual(ctx.exception.error_dict, {'username': [
                      'This field is just bad.'], 
           'password': [
                      'This field is just bad.']})
        return
    ...
    def test_handle_form_request(self):
        ...
        response = self.mixin.handle_form_request(data={'username': 'test-user', 
           'password': 'test-pass'}, request=None, form_kwargs={'custom': True}, save_kwargs={'commit': False}, my_state=123)
    ...
    def test_handle_form_request_with_instance(self):
        ...
        user = User(username='test-user', password='test-pass')
        response = self.mixin.handle_form_request(data={'username': 'new-user', 
           'first_name': 'new-first-name', 
           'last_name': 'new-last-name'}, request=None, instance=user, form_kwargs={'custom': True}, my_state=123)
    ...
    def test_handle_form_request_with_error(self):
        ...
        response = self.mixin.handle_form_request(data={}, request=None, form_kwargs={'custom': True}, my_state=123)
        self.assertEqual(response, (
         INVALID_FORM_DATA,
         {'fields': {'username': [
                                  'This field is required.'], 
    ...
    def test_handle_form_request_with_parse_field(self):
        ...
        self.mixin.parse_username_field = lambda value, my_state, **kwargs: 'test-%s-%s' % (value, my_state)
        self.spy_on(MyForm.save)
        response = self.mixin.handle_form_request(data={'username': 'user', 
           'password': 'test-pass'}, request=None, form_kwargs={'custom': True}, my_state=123)
        user = MyForm.save.last_call.return_value
    ...
    def test_save_form(self):
        ...
        form = MyForm(data={'username': 'test-user', 
           'password': 'test-pass'})
