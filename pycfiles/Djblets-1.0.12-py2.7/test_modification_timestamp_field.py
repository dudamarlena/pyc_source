...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/tests/test_modification_timestamp_field.py
...
from djblets.db.fields.modification_timestamp_field import ModificationTimestampField
...
class MultitipleModificationTimestampFieldTestModel(models.Model):
    timestamp1 = ModificationTimestampField(b'timestamp1')
    timestamp2 = ModificationTimestampField(b'timestamp2')


class ModificationTimestampFieldTests(SpyAgency, TestModelsLoaderMixin, TestCase):
    ...
    def test_pre_save_with_existing_instance(self):
        ...
        """
        timestamp = datetime(2018, 3, 6, 23, 31, 30, tzinfo=timezone.utc)
        obj = ModificationTimestampFieldTestModel.objects.create(field=timestamp)
        self.assertEqual(obj.field, timestamp)
        obj.save()
    ...
    def test_get_internal_type(self):
        ...
        obj = ModificationTimestampFieldTestModel()
        self.assertEqual(obj._meta.get_field(b'field').get_internal_type(), b'DateTimeField')
    ...
    def test_save_doesnt_overwrite_modified_value_with_tz(self):
        ...
        with self.settings(USE_TZ=True):
            ...
            self.assertEqual(instance.field, timestamp + timedelta(hours=1))
