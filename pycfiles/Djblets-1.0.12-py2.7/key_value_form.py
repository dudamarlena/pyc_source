...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/forms/forms/key_value_form.py
...
class KeyValueForm(forms.Form):
    ...
    def __init__(self, data=None, files=None, instance=None, *args, **kwargs):
        ...
        """
        super(KeyValueForm, self).__init__(data=data, files=files, *args, **kwargs)
    ...
    def load(self):
        ...
        for field_name, field in six.iteritems(self.fields):
            if self.instance is not None and field_name not in load_blacklist:
                ...
                deserialize_func = getattr(self, b'deserialize_%s_field' % field_name, None)
                if deserialize_func is not None:
                    value = deserialize_func(value)
                field.initial = value
            if field_name in disabled_fields:
                field.widget.attrs[b'disabled'] = b'disabled'
    ...
    def save(self, commit=True, extra_save_blacklist=[]):
        ...
        """
        if self.errors:
            raise ValueError(b'The form could not be saved due to one or more errors.')
        ...
        for key, value in six.iteritems(self.cleaned_data):
            if key not in blacklist:
                serialize_func = getattr(self, b'serialize_%s_field' % key, None)
    ...
    def create_instance(self):
        ...
        """
        raise NotImplementedError(b'%r must implement create_instance()' % self.__class__)
    ...
    def get_load_blacklist(self):
        ...
        if hasattr(self, b'Meta'):
            return getattr(self.Meta, b'load_blacklist', self.get_save_blacklist())
        return []
    ...
    def get_save_blacklist(self):
        ...
        """
        if hasattr(self, b'Meta'):
            return getattr(self.Meta, b'save_blacklist', [])
        return []