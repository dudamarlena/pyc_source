...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/util/templatetags/djblets_utils.py
...
def definevar(context, nodelist, varname, *options):
    ...
    if b'spaceless' in options:
        result = strip_spaces_between_tags(result.strip())
    elif b'strip' in options:
        result = result.strip()
    if b'unsafe' in options:
...
def _check_userorperm(context, user, perm):
    ...
    req_user = context.get(b'user', None)
    if isinstance(req_user, AnonymousUser):
        return False
    else:
        if req_user.has_perm(perm):
            return True
        return isinstance(user, User) and user == req_user or user == req_user.pk
...
def attr(context, nodelist, attrname, *options):
    ...
    if b'nocondense' not in options:
        content = WS_RE.sub(b' ', content.strip())
    if not content:
        return b''
    return format_html(b' {0}="{1}"', attrname, six.text_type(content))
...
register.filter(b'humanize_list', humanize_list)
...
def exclude_item(container, item):
    ...
    if not isinstance(container, list):
        raise TemplateSyntaxError(b'remove_item expects a list')
...
def indent(value, numspaces=4):
    ...
    """
    indent_str = b' ' * numspaces
    return indent_str + value.replace(b'\n', b'\n' + indent_str)
...
def querystring_with(context, attr, value):
    ...
    """
    existing_query = get_url_params_except(context[b'request'].GET, attr)
    new_query = urlencode({attr.encode(b'utf-8'): value.encode(b'utf-8')})
    if existing_query:
        result = b'?%s&%s' % (existing_query, new_query)
    else:
        result = b'?%s' % new_query
