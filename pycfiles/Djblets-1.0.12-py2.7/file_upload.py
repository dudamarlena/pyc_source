...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/avatars/services/file_upload.py
...
class FileUploadServiceForm(AvatarServiceConfigForm):
    ...
    avatar_service_id = b'file-upload'
    js_view_class = b'Djblets.Avatars.FileUploadSettingsFormView'
    template_name = b'avatars/services/file_upload_form.html'
    avatar_upload = forms.FileField(required=True, widget=widgets.FileInput(attrs={b'accept': b'image/png, image/jpeg, image/gif'}))
    ...
    def clean_file(self):
        ...
        """
        f = self.cleaned_data[b'avatar_upload']
        if f.size > self.MAX_FILE_SIZE:
            raise ValidationError(_(b'The file is too large.'))
        content_type = f.content_type.split(b'/')[0]
        if content_type != b'image':
            raise ValidationError(_(b'Only images are supported.'))
        return f
    ...
    def save(self):
        ...
        uploaded_file = self.cleaned_data[b'avatar_upload']
        file_hash = md5()
        for chunk in uploaded_file.chunks():
            file_hash.update(chunk)

        file_path = storage.get_valid_name(b'%s%s' % (
         self.service.get_unique_filename(username),
         os.path.splitext(self.cleaned_data[b'avatar_upload'].name)[1]))
        file_path = os.path.join(username[0], username[:2], file_path)
        if self.service.file_path_prefix:
            file_path = os.path.join(self.service.file_path_prefix, file_path)
        file_path = storage.save(file_path, uploaded_file)
        return {b'file_path': file_path, 
           b'file_hash': file_hash.hexdigest()}


class FileUploadService(AvatarService):
    """An avatar service for uploaded images."""
    avatar_service_id = b'file-upload'
    name = _(b'File Upload')
    ...
    def file_path_prefix(self):
        ...
        """
        return getattr(settings, b'UPLOADED_AVATARS_PATH', os.path.join(b'uploaded', b'avatars'))
    ...
    def get_avatar_urls_uncached(self, user, size):
        ...
        if not configuration:
            return {}
        return {b'1x': storage.url(configuration[b'file_path'])}
    ...
    def cleanup(self, user):
        ...
        configuration = settings_manager.configuration_for(self.avatar_service_id)
        try:
            del configuration[b'file_hash']
        except KeyError:
            pass

        settings_manager.save()
        storage = DefaultStorage()
        storage.delete(configuration[b'file_path'])
    ...
    def get_etag_data(self, user):
        ...
        settings_manager = self._settings_manager_class(user)
        configuration = settings_manager.configuration_for(self.avatar_service_id)
        file_hash = configuration.get(b'file_hash')
        if not file_hash:
            storage = DefaultStorage()
            file_hash = md5()
            with storage.open(configuration[b'file_path'], b'rb') as (f):
                file_hash.update(f.read())
            configuration[b'file_hash'] = file_hash.hexdigest()
            settings_manager.save()
