...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/features/templatetags/features.py
...
class IfFeatureNode(Node):
    ...
    def __init__(self, nodelist_enabled, nodelist_disabled, feature_id, extra_kwargs):
    ...
    def __repr__(self):
        ...
        """
        return b'<IfFeatureNode for %s>' % self.feature_id
    ...
    def render(self, context):
        ...
        extra_kwargs = {key:value.resolve(context) for key, value in six.iteritems(self.extra_kwargs)}
        feature = get_features_registry().get_feature(feature_id)
        if feature:
            enabled = feature.is_enabled(request=context.get(b'request'), **extra_kwargs)
        else:
            enabled = False
        if enabled:
...
def _if_feature(parser, token, enabled_first):
    ...
    end_tag = b'end%s' % tag
    if len(bits) < 2:
        raise TemplateSyntaxError(b'%r requires a feature ID argument' % tag)
    nodelist_1 = parser.parse((b'else', end_tag))
    ...
    return IfFeatureNode(nodelist_enabled, nodelist_disabled, feature_id, extra_kwargs)
