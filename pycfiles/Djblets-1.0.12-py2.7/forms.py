...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/resources/mixins/forms.py
...
class UpdateFormMixin(object):
    ...
    def handle_form_request(self, request, data=None, files=None, instance=None, form_kwargs=None, save_kwargs=None, **kwargs):
        ...
        try:
            form = self.create_form(instance=instance, data=data, files=files, request=request, form_kwargs=form_kwargs, **kwargs)
        except ValidationError as e:
            return self.build_form_error_response(errors=e, instance=instance, **kwargs)

        if not form.is_valid():
            return self.build_form_error_response(form=form, instance=instance, **kwargs)
        else:
            instance = self.save_form(form=form, save_kwargs=save_kwargs, **kwargs)
            return self.build_form_success_response(form=form, instance=instance, is_created=is_created, **kwargs)

    def create_form(self, data, request, files=None, instance=None, form_kwargs=None, **kwargs):
        ...
        """
        assert self.form_class, b'%s must define a form_class attribute.' % self.__class__.__name__
        ...
        if instance is not None:
            meta = form_cls._meta
            form_data.update(model_to_dict(instance=instance, fields=meta.fields, exclude=meta.exclude))
        ...
        if form_data is not None:
            form_kwargs[b'data'] = form_data
        if files is not None:
            form_kwargs[b'files'] = files
        if instance is not None:
            form_kwargs[b'instance'] = instance
    ...
    def build_form_errors(self, form=None, errors=None, **kwargs):
        ...
        """
        assert (form is None) != (errors is None), b'Only one of form or errors can be provided.'
        if form is not None:
            return {field_name:[ force_text(e) for e in field_errors ] for field_name, field_errors in six.iteritems(form.errors)}
        else:
            if hasattr(errors, b'error_dict'):
                return errors.message_dict
            else:
                return {NON_FIELD_ERRORS: errors.messages}

            return

    def build_form_success_response(self, form, instance, is_created, **kwargs):
    ...
    def build_form_error_response(self, form=None, errors=None, instance=None, **kwargs):
        ...
        return (
         INVALID_FORM_DATA,
         {b'fields': self.build_form_errors(form=form, errors=errors)})
    ...
    def _parse_form_data(self, form_data, request, **kwargs):
        ...
        for field, value in six.iteritems(form_data):
            parser = getattr(self, b'parse_%s_field' % field, None)
            if parser is not None:
                try:
                    parsed_data[field] = parser(value, request=request, **kwargs)
                except ValidationError as e:
