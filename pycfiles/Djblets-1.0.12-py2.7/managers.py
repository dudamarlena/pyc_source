...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/managers.py
...
class WebAPITokenManager(Manager):
    ...
    def generate_token(self, user, max_attempts=20, note=None, policy={}, auto_generated=False, **kwargs):
        ...
        for attempt in range(max_attempts):
            raw_token = prefix + six.text_type(attempt) + timezone.now().isoformat()
            sha = hashlib.sha1(raw_token.encode(b'utf-8'))
            token = sha.hexdigest()
            try:
                token = self.create(user=user, token=token, note=(note or b''), policy=policy, **kwargs)
            except IntegrityError:
                pass
            else:
                webapi_token_created.send(instance=token, auto_generated=auto_generated, sender=type(token))
                return token

        logger.error(b'Unable to generate unique API token for %s after %d attempts.', user.username, max_attempts)
        raise WebAPITokenGenerationError(_(b'Could not create a unique API token. Please try again.'))