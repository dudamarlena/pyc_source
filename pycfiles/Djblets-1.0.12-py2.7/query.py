...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/query.py
...
class LocalDataQuerySet(object):
    ...
    * select_related
    ...
    def exclude(self, **kwargs):
        ...
        """
        return LocalDataQuerySet(list(self._filter_or_exclude(return_matches=False, **kwargs)))
    ...
    def filter(self, **kwargs):
        ...
        """
        return LocalDataQuerySet(list(self._filter_or_exclude(return_matches=True, **kwargs)))
    ...
    def get(self, **kwargs):
        ...
        if count == 1:
            return clone[0]
        if count == 0:
            raise ObjectDoesNotExist(b'%s matching query does not exist.' % self._data.__class__.__name__)
        else:
            raise MultipleObjectsReturned(b'get() returned more than one %s -- it returned %s!' % (
...
class _Django16ChainableSelectRelatedQuerySet(QuerySet):
    ...
    def select_related(self, *fields):
        ...
        """
        obj = super(_Django16ChainableSelectRelatedQuerySet, self).select_related(*fields)
        if fields and fields != (None, ) and self.query.select_related and isinstance(self.query.select_related, dict):
            self._merge_dicts(obj.query.select_related, self.query.select_related)
        return obj
...
def chainable_select_related_queryset(queryset):
    ...
    if django.VERSION[:2] == (1, 6):
        queryset = queryset._clone(klass=_Django16ChainableSelectRelatedQuerySet)
    return queryset
...
def prefix_q(prefix, q, clone=True):
    ...
    for i, child in enumerate(q.children):
        if isinstance(child, Q):
            ...
            q.children[i] = (
             b'%s__%s' % (prefix.encode(b'utf-8'), key),
             value)
...
def _clone_q(q):
    ...
    """
    if hasattr(q, b'clone'):
        q = q.clone()
    else:
        new_q = q.__class__._new_instance(children=[], connector=q.connector, negated=q.negated)
        for child in q.children:
            if isinstance(child, Q):
                new_child = _clone_q(child)
            elif hasattr(child, b'clone'):
