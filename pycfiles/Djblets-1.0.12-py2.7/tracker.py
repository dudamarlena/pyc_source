...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/privacy/consent/tracker.py
...
class BaseConsentTracker(object):
    ...
    def get_audit_identifier(self, user):
        ...
        """
        return hashlib.sha256(user.email.encode(b'utf-8')).hexdigest()
    ...
    def get_pending_consent_requirements(self, user):
        ...
        return [ consent_requirement for consent_requirement in get_consent_requirements_registry() if consent_requirement.requirement_id not in all_consent
               ]
    ...
    def get_all_consent(self, user):
        ...
        """
        return cache_memoize(self._get_user_cache_key(user), lambda : self.get_all_consent_uncached(user))
    ...
    def _get_user_cache_key(self, user):
        ...
        """
        return b'privacy-consent:%s' % user.pk
...
class DatabaseConsentTracker(BaseConsentTracker):
    ...
    def _get_stored_consent_for_user(self, user):
        ...
        """
        if not hasattr(user, b'_djblets_stored_consent'):
            user._djblets_stored_consent = get_object_or_none(self.model, user=user)
...
def get_consent_tracker():
    ...
    if _consent_tracker is None:
        tracker_class_name = getattr(settings, b'DJBLETS_PRIVACY_CONSENT_TRACKER', b'%s.%s' % (DatabaseConsentTracker.__module__,
         DatabaseConsentTracker.__name__))
        module_name, class_name = tracker_class_name.rsplit(b'.', 1)
        try:
            _consent_tracker = getattr(import_module(module_name), class_name)()
        except (AttributeError, ImportError):
            raise ImproperlyConfigured(b'settings.DJBLETS_CONSENT_TRACKER must be set to a valid class path.')
