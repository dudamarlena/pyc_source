...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/resources/root.py
...
class RootResource(WebAPIResource):
    ...
    def serialize_root(self, request, *args, **kwargs):
        ...
        if self._include_uri_templates:
            data[b'uri_templates'] = self.get_uri_templates(request, *args, **kwargs)
        return data
    ...
    def _walk_resources(self, resource, list_href):
        yield (
         resource.name_plural, list_href)
        for child in resource.list_child_resources:
            child_href = list_href + child.uri_name + b'/'
            for name, href in self._walk_resources(child, child_href):
                yield (name, href)

        if resource.uri_object_key:
            object_href = b'%s{%s}/' % (list_href, resource.uri_object_key)
            yield (
             resource.name, object_href)
            for child in resource.item_child_resources:
                child_href = object_href + child.uri_name + b'/'
                for name, href in self._walk_resources(child, child_href):
                    yield (name, href)
    ...
    def api_404_handler(self, request, api_format=None, *args, **kwargs):
        ...
        """
        return WebAPIResponseError(request, err=DOES_NOT_EXIST, api_format=api_format)
    ...
    def get_url_patterns(self):
        ...
        urlpatterns += never_cache_patterns(b'', url(b'.*', self.api_404_handler))
        return urlpatterns