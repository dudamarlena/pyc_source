...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/conditions/operators.py
# Compiled at: 2019-06-12 01:17:17
"""Base support and standard operators for condition choices."""
from __future__ import unicode_literals
from django.utils.translation import ugettext_lazy as _
from djblets.conditions.errors import ConditionOperatorConflictError, ConditionOperatorNotFoundError
...
class BaseConditionOperator(object):
    ...
    def has_custom_value_field(self):
        ...
        return self.__class__.value_field is not BaseConditionOperator.value_field
...
class IsNotOneOfOperator(BaseConditionOperator):
    ...
    This is equivalent to::
        ...
        if match_value not in condition_value:
            ...
    """
    operator_id = b'not-one-of'
    name = _(b'Is not one of')
...
class DoesNotContainOperator(BaseConditionOperator):
    ...
    """
    operator_id = b'does-not-contain'
    name = _(b'Does not contain')
...
class DoesNotContainAnyOperator(BaseConditionOperator):
    ...
    """
    operator_id = b'does-not-contain-any'
    name = _(b'Not any of')
...
class StartsWithOperator(BaseConditionOperator):
    ...
    It's equivalent to::
        ...
        if match_value.startswith(condition_value):
            ...
    """
    operator_id = b'starts-with'
    name = _(b'Starts with')
    ...
    def matches(self, match_value, condition_value, **kwargs):
        ...
        try:
            ...
            raise TypeError(_(b'Lookup value %r does not support startswith()'))
...
class GreaterThanOperator(BaseConditionOperator):
    ...
    It's equivalent to::
        ...
        if match_value > condition_value:
            ...
    """
    operator_id = b'greater-than'
    name = _(b'Greater than')
...
class MatchesRegexOperator(BaseConditionOperator):
    ...
    It's equivalent to::
        ...
        if condition_value.match(match_value):
            ...
    """
    operator_id = b'matches-regex'
    name = _(b'Matches regex')
...
class DoesNotMatchRegexOperator(BaseConditionOperator):
    ...
    It's equivalent to::
        ...
        if not condition_value.match(match_value):
            ...
    """
    operator_id = b'does-not-match-regex'
    name = _(b'Does not match regex')
    value_field = ConditionValueRegexField()
...
class ConditionOperators(OrderedRegistry):
    ...
    default_errors = dict(DEFAULT_ERRORS, **{ALREADY_REGISTERED: _(b'Could not register condition operator %(item)s: This operator is already registered or its ID conflicts with another operator.'), 
       ATTRIBUTE_REGISTERED: _(b'Could not register condition operator %(item)s: Another operator %(duplicate)s) is already registered with the same ID.'), 
       NOT_REGISTERED: _(b'No operator was found matching "%(attr_value)s".'), 
       UNREGISTER: _(b'Could not unregister condition operator %(item)s: This condition was not yet registered.')})
    ...
    def get_operator(self, operator_id, choice):
        ...
        """
        operator_cls = self.get(b'operator_id', operator_id)
