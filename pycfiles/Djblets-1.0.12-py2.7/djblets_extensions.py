...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/extensions/templatetags/djblets_extensions.py
            ...
            try:
                if hook.applies_to(request):
                    ...
                    try:
                        ...
                        logger.exception(b'Error rendering TemplateHook %r: %s', hook, e)

                    context.pop()
            except Exception as e:
                logger.exception(b'Error when calling applies_to for TemplateHook %r: %s', hook, e)

    return (b'').join(_render_hooks())
...
def _render_bundle(context, node_cls, extension, name, bundle_type):
    try:
        return node_cls(b'"%s"' % extension.get_bundle_id(name)).render(context)
    except:
        logger.exception(b'Unable to load %s bundle "%s" for extension "%s" (%s)', bundle_type, name, extension.info.name, extension.id)
        raise
...
def _render_css_bundle(context, extension, name):
    ...
    Returns:
        ...
        The rendered HTML.
    """
    return _render_bundle(context, ExtensionStylesheetNode, extension, name, b'CSS')
...
def _render_js_bundle(context, extension, name):
    ...
    Returns:
        ...
        The rendered HTML.
    """
    return _render_bundle(context, ExtensionJavascriptNode, extension, name, b'JS')
...
def _get_extension_bundles(extension_manager_key, context, bundle_attr, default_bundles, renderer):
    ...
    """
    request = context[b'request']
    if not getattr(request, b'resolver_match', None):
        return
    else:
        requested_url_name = request.resolver_match.url_name
        default_bundles = set(default_bundles.split(b','))
        for manager in get_extension_managers():
            ...
            for extension in manager.get_enabled_extensions():
                ...
                for bundle_name, bundle in six.iteritems(bundles):
                    if bundle_name in default_bundles or requested_url_name in bundle.get(b'apply_to', []):
                        for include_bundle in bundle.get(b'include_bundles', []):
...
@register.simple_tag(takes_context=True)
def load_extensions_css(context, extension_manager_key, default_bundles=b'default'):
    ...
    Returns:
        ...
        The HTML used to include the bundled content.
    """
    return (b'').join(_get_extension_bundles(extension_manager_key, context, b'css_bundles', default_bundles, _render_css_bundle))


@register.simple_tag(takes_context=True)
def load_extensions_js(context, extension_manager_key, default_bundles=b'default'):
    ...
    Returns:
        ...
        The HTML used to include the bundled content.
    """
    return (b'').join(_get_extension_bundles(extension_manager_key, context, b'js_bundles', default_bundles, _render_js_bundle))


@register.inclusion_tag(b'extensions/init_js_extensions.html', takes_context=True)
        ...
        if manager.key == extension_manager_key:
            ...
            for js_extension in js_extensions:
                ...
                if arg_spec.keywords is None:
                    warnings.warn(b'%s.get_model_data will need to take keyword arguments. The old function signature is deprecated.' % js_extension_cls.__name__)
                    model_data = js_extension.get_model_data()
                else:
                    model_data = js_extension.get_model_data(request=request)
                js_extension_items.append({b'js_extension': js_extension, 
                   b'model_data': model_data})

            return {b'url_name': url_name, 
               b'js_extension_items': js_extension_items}
