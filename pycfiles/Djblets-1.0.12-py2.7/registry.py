...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/resources/registry.py
    ...
    def __getattr__(self, name):
        ...
        if name.startswith(b'__'):
            ...
            if name not in self.__dict__:
                instance_name = b'%s_resource' % name
                found = False
                error = None
                for search_path in self.resource_search_path:
                    try:
                        mod = __import__(b'%s.%s' % (search_path, name), {}, {}, [instance_name])
                ...
                if not found:
                    logger.exception(b'Unable to load webapi resource %s: %s', name, error)
                    raise AttributeError(b'%s is not a valid resource name' % name)
...
def get_resource_for_object(obj):
    ...
    cls = obj.__class__
    if getattr(obj, b'_deferred', False):
        cls = cls.__bases__[0]
