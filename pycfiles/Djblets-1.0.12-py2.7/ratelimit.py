...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/auth/ratelimit.py
...
DEFAULT_LOGIN_LIMIT_RATE = b'5/m'
DEFAULT_API_ANONYMOUS_LIMIT_RATE = b'1000/h'
DEFAULT_API_AUTHENTICATED_LIMIT_RATE = b'10000/h'
_RATE_LIMIT_DATA = {RATE_LIMIT_LOGIN: (
                    b'LOGIN_LIMIT_RATE',
                    DEFAULT_LOGIN_LIMIT_RATE,
                    b'login-ratelimit'), 
   RATE_LIMIT_API_ANONYMOUS: (
                            b'API_ANONYMOUS_LIMIT_RATE',
                            DEFAULT_API_ANONYMOUS_LIMIT_RATE,
                            b'api-anonymous-ratelimit'), 
   RATE_LIMIT_API_AUTHENTICATED: (
                                b'API_AUTHENTICATED_LIMIT_RATE',
                                DEFAULT_API_AUTHENTICATED_LIMIT_RATE,
                                b'api-authenticated-ratelimit')}
...
def get_user_id_or_ip(request):
    ...
    """
    if hasattr(request, b'user') and request.user.is_authenticated():
        return six.text_type(request.user.pk)
    try:
        return request.META[b'HTTP_X_REAL_IP']
    except KeyError:
        try:
            return request.META[b'HTTP_X_FORWARDED_FOR'].split(b',')[0].strip()
        except KeyError:
            return request.META[b'REMOTE_ADDR']
...
def is_ratelimited(request, increment=False, limit_type=RATE_LIMIT_LOGIN):
    ...
    return usage is not None and usage[b'count'] > usage[b'limit']
...
def get_usage_count(request, increment=False, limit_type=RATE_LIMIT_LOGIN):
    ...
    try:
        try:
            settings_key, default_value, cache_key_prefix = _RATE_LIMIT_DATA[limit_type]
        except KeyError:
            raise ValueError(b'"limit_type" argument had unexpected value "%s"' % limit_type)
        ...
        raise ImproperlyConfigured(b'LOGIN_LIMIT_RATE setting could not be parsed.')

    limit = rate_limit.count
    period = rate_limit.seconds
    user_id_or_ip = get_user_id_or_ip(request)
    cache_key = make_cache_key(b'%s:%d/%d%s%s' % (
...
class Rate(object):
    ...
    def parse(cls, rate_str):
        ...
        if m:
            ...
            raise ValueError(b'Could not parse given rate: %s.' % rate_str)
        seconds = Rate.PERIODS[(period or b's')]
        if multiplier:
