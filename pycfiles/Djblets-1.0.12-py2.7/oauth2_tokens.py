...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/resources/mixins/oauth2_tokens.py
...
class ResourceOAuth2TokenMixin(object):
    ...
    HTTP_SCOPE_METHOD_MAP = {b'DELETE': b'destroy', 
       b'GET': b'read', 
       b'HEAD': b'read', 
       b'OPTIONS': b'read', 
       b'POST': b'write', 
    ...
    def call_method_view(self, request, method, view, *args, **kwargs):
        ...
        if token is not None:
            if token.is_expired():
                ...
                if not self.oauth2_token_access_allowed:
                    return OAUTH_ACCESS_DENIED_ERROR
                if not self.__is_resource_method_allowed(method, token):
    ...
    def __get_oauth2_token_for_request(self, request):
        ...
        try:
            ...
            raise ImproperlyConfigured(b'WebAPIOAuth2TokenAuthBackend requires django-oauth-toolkit>=0.9,<0.9.999.')

        token = getattr(request, b'_oauth2_token', None)
        from_session = False
        if not token:
            token_pk = request.session.get(b'oauth2_token_id')
            if token_pk:
                from_session = True
    ...
    def __is_resource_method_allowed(self, method, token):
        ...
        if not suffix:
            return False
        required_scope = b'%s:%s' % (self.scope_name, suffix)
