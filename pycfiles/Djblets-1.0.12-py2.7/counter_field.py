...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/fields/counter_field.py
...
try:
    ...
    from django.db.models.expressions import ExpressionNode as QueryExpressionType
...
class CounterField(models.IntegerField):
    ...
    def _update_values(cls, model_instance, values, reload_object, multiplier):
        ...
        """
        cls._set_values(model_instance=model_instance, values={attname:F(attname) + value * multiplier for attname, value in six.iteritems(values) if value != 0}, reload_object=reload_object)
    ...
    def __init__(self, verbose_name=None, name=None, initializer=None, default=None, **kwargs):
        ...
        """
        kwargs.update({b'blank': True, 
           b'null': True})
        super(CounterField, self).__init__(verbose_name, name, default=default, **kwargs)
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, b'increment_%s' % self.name, _increment)
        setattr(cls, b'decrement_%s' % self.name, _decrement)
        setattr(cls, b'reload_%s' % self.name, _reload)
        setattr(cls, b'reinit_%s' % self.name, _reinit)
        setattr(cls, self.attname, self)
        setattr(cls, b'_do_update', type(self)._model_do_update)
    ...
    def _increment(self, model_instance, reload_object=True, increment_by=1):
        ...
        if increment_by != 0:
            ...
            self.increment(cls.objects.filter(pk=model_instance.pk), increment_by)
            if reload_object:
                self._reload(model_instance)
    ...
    def _decrement(self, model_instance, reload_object=True, decrement_by=1):
        ...
        if decrement_by != 0:
            cls = type(model_instance)
            self.decrement(cls.objects.filter(pk=model_instance.pk), decrement_by)
            if reload_object:
                self._reload(model_instance)
    ...
    def _reinit(self, model_instance):
        ...
        """
        if not (model_instance.pk or self._initializer or six.callable(self._initializer)):
            return
        else:
            value = 0
            ...
            if value is not None:
                ...
                if is_expr:
                    ...
                    cls.objects.filter(pk=model_instance.pk).update(**{self.attname: value})
                    self._reload_model_instance(model_instance, [self.attname])
                else:
    ...
    def _do_post_init(self, instance):
        ...
        if value is None:
            reinit = getattr(instance, b'reinit_%s' % self.name)
            reinit()
        return

    @staticmethod
    def _model_do_update(model, base_qs, using, pk_val, values, update_fields, forced_update):
        ...
        """
        values = [ value for value in values if not isinstance(value[0], CounterField) or value[2] is None or update_fields and value[0].attname in update_fields
                 ]
        return super(model.__class__, model)._do_update(base_qs, using, pk_val, values, update_fields, forced_update)