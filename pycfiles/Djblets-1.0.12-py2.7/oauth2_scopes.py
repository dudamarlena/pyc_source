...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/oauth2_scopes.py
...
from djblets.extensions.signals import extension_enabled, extension_disabled
...
def get_scope_dictionary():
    ...
    global _scopes
    if _scopes is None:
        scopes_cls_name = getattr(settings, b'WEB_API_SCOPE_DICT_CLASS', b'djblets.webapi.oauth2_scopes.WebAPIScopeDictionary')
        scopes_cls_mod, scopes_cls_attr = scopes_cls_name.rsplit(b'.', 1)
        try:
            scopes_cls = getattr(import_module(scopes_cls_mod), scopes_cls_attr)
        except (AttributeError, ImportError) as e:
            raise ImproperlyConfigured(b'settings.WEB_API_SCOPE_DICT_CLASS %r could not be imported: %s' % (
             scopes_cls_name, e))

        try:
            root_resource_path = getattr(settings, b'WEB_API_ROOT_RESOURCE')
        except AttributeError:
            raise ImproperlyConfigured(b'settings.WEB_API_ROOT_RESOURCE is required.')

        root_resource_mod, root_resource_attr = root_resource_path.rsplit(b'.', 1)
        try:
            root_resource = getattr(import_module(root_resource_mod), root_resource_attr)
        except (AttributeError, ImportError) as e:
            raise ImproperlyConfigured(b'settings.WEB_API_ROOT_RESOURCE %r could not be imported: %s' % (
...
class WebAPIScopeDictionary(object):
    ...
    def scope_dict(self):
        ...
        if not self._scope_dict:
            with self._update_lock:
                self._scope_dict or self._walk_resources(self.resource_trees)
    ...
    def _walk_resources(self, resources):
        ...
        for resource in resources:
            ...
            if not isinstance(resource, ResourceOAuth2TokenMixin):
                logging.warning(b'Resource %r does not inherit from djblets.webapi.resources.mixins.oauth2_tokens.ResourceOAuth2TokenMixin: it will not be accessible with OAuth2 tokens. It is recommended that all your resources inherit from a base class that includes this mixin.', type(resource))
                continue
            ...
            for method in resource.allowed_methods:
                try:
                    ...
                    logger.error(b'Unknown HTTP method %s not present in HTTP_SCOPE_METHOD_MAP: a scope will not be generated for this method.', method)
                    continue

                scope_to_methods[suffix].add(method)

            for suffix, methods in six.iteritems(scope_to_methods):
                scope_name = b'%s:%s' % (resource.scope_name, suffix)
                self._scope_dict[scope_name] = _(b'Ability to perform HTTP %(methods)s on the %(name)s resource') % {b'methods': (b', ').join(methods), 
    ...
    def __repr__(self):
        ...
        """
        return b'%s(%r)' % (type(self).__name__,
...
class ExtensionEnabledWebAPIScopeDictionary(WebAPIScopeDictionary):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        super(ExtensionEnabledWebAPIScopeDictionary, self).__init__(*args, **kwargs)
...
def enable_web_api_scopes(*args, **kwargs):
    ...
    """
    with _enable_lock:
        if not isinstance(settings.OAUTH2_PROVIDER[b'SCOPES'], WebAPIScopeDictionary):
            settings.OAUTH2_PROVIDER[b'SCOPES'] = get_scope_dictionary()