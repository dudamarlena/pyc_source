...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/conditions/conditions.py
...
class Condition(object):
    ...
    @classmethod
    def deserialize(cls, choices, data, condition_index=None, choice_kwargs={}):
        ...
        """
        try:
            choice_id = data[b'choice']
        except KeyError:
            logging.debug(b'Condition.deserialize: Missing "choice" key for condition %r', data)
            raise ConditionChoiceNotFoundError(_(b'A choice is required.'), condition_index=condition_index)

        try:
            operator_id = data[b'op']
        except KeyError:
            logging.debug(b'Condition.deserialize: Missing "op" key for condition %r', data)
            raise ConditionOperatorNotFoundError(_(b'An operator is required.'), condition_index=condition_index)

        try:
            choice = choices.get_choice(choice_id, choice_kwargs=choice_kwargs)
        except ConditionChoiceNotFoundError as e:
            logging.debug(b'Condition.deserialize: Invalid "choice" value "%s" for condition %r', choice_id, data)
            raise ConditionChoiceNotFoundError(six.text_type(e), choice_id=choice_id, condition_index=condition_index)

        try:
            operator = choice.get_operator(operator_id)
        except ConditionOperatorNotFoundError as e:
            logging.debug(b'Condition.deserialize: Invalid "op" value "%s" for condition %r', operator_id, data)
            raise ConditionOperatorNotFoundError(six.text_type(e), operator_id=operator_id, condition_index=condition_index)

        if operator.value_field is not None:
            try:
                raw_value = data[b'value']
                value = operator.value_field.deserialize_value(raw_value)
            except KeyError:
                logging.debug(b'Condition.deserialize: Missing "value" value for condition %r', data)
                raise InvalidConditionValueError(_(b'A value is required.'), condition_index=condition_index)
            except InvalidConditionValueError as e:
                logging.debug(b'Condition.deserialize: Invalid "value" value %r for condition %r', raw_value, data)
            ...
            raw_value = None
            value = None
        return cls(choice=choice, operator=operator, value=value, raw_value=raw_value)
    ...
    def matches(self, value, value_state_cache=None):
        ...
        if value_state_cache is None:
            value_state_cache = {}
        return self.choice.matches(operator=self.operator, match_value=value, condition_value=self.value, value_state_cache=value_state_cache)
    ...
    def serialize(self):
        ...
        Returns:
            ...
            serialized to JSON.
        """
        data = {b'choice': self.choice.choice_id, 
           b'op': self.operator.operator_id}
...
class ConditionSet(object):
    ...
    """
    MODE_ALWAYS = b'always'
    MODE_ALL = b'all'
    MODE_ANY = b'any'
    CONDITIONS = (
     MODE_ALWAYS, MODE_ALL, MODE_ANY)
    ...
    def deserialize(cls, choices, data, choice_kwargs={}):
        ...
        """
        mode = data.get(b'mode')
        if mode not in cls.CONDITIONS:
            logging.debug(b'ConditionSet.deserialize: Invalid "mode" value "%s" for condition set %r', mode, data)
            raise InvalidConditionModeError(_(b'"%s" is not a valid condition mode.') % mode)
        return cls(mode, [ Condition.deserialize(choices, condition_data, i, choice_kwargs) for i, condition_data in enumerate(data.get(b'conditions', []))
                         ])
    ...
    def matches(self, **values):
        ...
        if self.mode == self.MODE_ALWAYS:
            ...
            raise ValueError(b'Invalid condition mode %r' % self.mode)
        return match_conditions(self._get_condition_results(self.conditions, values))
    ...
    def serialize(self):
        ...
        Returns:
            ...
            serialized to JSON.
        """
        return {b'mode': self.mode, 
           b'conditions': [ condition.serialize() for condition in self.conditions
                        ]}
    ...
    def _get_condition_results(self, conditions, values):
        ...
        for condition in conditions:
            ...
            if value_kwarg in values:
                yield condition.matches(values[value_kwarg], value_state_cache=value_state_cache)
            else:
                yield False
