...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/fields/modification_timestamp_field.py
...
class ModificationTimestampField(models.DateTimeField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        kwargs.update({b'editable': False, 
           b'blank': True})
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, self.name, ModificationTrackedValue(self.name, state_name))
    ...
    def pre_save(self, model_instance, add):
        ...
        """
        state = getattr(model_instance, ModificationState.get_attr_name(self.name))
        existing_value = getattr(model_instance, self.attname)
        if not add and not state.modified or add and existing_value is None:
            value = timezone.now()
            setattr(model_instance, self.attname, value)
        elif not add and state.modified or add and existing_value is not None:
            value = super(ModificationTimestampField, self).pre_save(model_instance, add)
