...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/cache/serials.py
...
def generate_media_serial():
    ...
    """
    MEDIA_SERIAL = getattr(settings, b'MEDIA_SERIAL', 0)
    if not MEDIA_SERIAL:
        media_dirs = getattr(settings, b'MEDIA_SERIAL_DIRS', [b'.'])
        ...
        setattr(settings, b'MEDIA_SERIAL', MEDIA_SERIAL)
...
def generate_ajax_serial():
    ...
    return that value.
    """
    AJAX_SERIAL = getattr(settings, b'AJAX_SERIAL', 0)
    if not AJAX_SERIAL:
        template_dirs = getattr(settings, b'TEMPLATE_DIRS', [b'.'])
        ...
        setattr(settings, b'AJAX_SERIAL', AJAX_SERIAL)
...
def generate_locale_serial(packages):
    ...
    for package in packages:
        try:
            ...
            path = os.path.join(os.path.dirname(p.__file__), b'locale')
            paths.append(path)
        except Exception as e:
            logger.exception(b'Failed to import package %s to compute locale serial: %s', package, e)

    for locale_path in paths:
        for root, dirs, files in os.walk(locale_path):
            for name in files:
                if name.endswith(b'.mo'):
                    mtime = int(os.stat(os.path.join(root, name)).st_mtime)
                    if mtime > serial:
                        serial = mtime
