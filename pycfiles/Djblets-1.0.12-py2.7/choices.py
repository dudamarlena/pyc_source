...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/conditions/choices.py
# Compiled at: 2019-06-12 01:17:17
"""Base support and standard choices for conditions."""
from __future__ import unicode_literals
from django.utils.translation import ugettext_lazy as _
from djblets.conditions.errors import ConditionChoiceConflictError, ConditionChoiceNotFoundError
from djblets.conditions.operators import AnyOperator, ConditionOperators, ContainsOperator, DoesNotContainOperator, DoesNotMatchRegexOperator, EndsWithOperator, GreaterThanOperator, IsNotOneOfOperator, IsNotOperator, IsOneOfOperator, IsOperator, LessThanOperator, MatchesRegexOperator, StartsWithOperator, UnsetOperator
from djblets.conditions.values import ConditionValueBooleanField, ConditionValueCharField, ConditionValueIntegerField, ConditionValueModelField, ConditionValueMultipleModelField
from djblets.registries.registry import ALREADY_REGISTERED, ATTRIBUTE_REGISTERED, DEFAULT_ERRORS, NOT_REGISTERED, OrderedRegistry, UNREGISTER
...
class BaseConditionChoice(object):
    ...
    def matches(self, operator, match_value, condition_value, value_state_cache, **kwargs):
        ...
        """
        return operator.matches(match_value=self.get_match_value(match_value, value_state_cache=value_state_cache), condition_value=condition_value)
...
class ConditionChoiceMatchListItemsMixin(object):
    ...
    def matches(self, operator, match_value, condition_value, value_state_cache, **kwargs):
        ...
        Returns:
            ...
            ``False`` if it does not.
        """
        match_value = self.get_match_value(match_value, value_state_cache=value_state_cache)
        if not match_value:
            return False
        if self.require_match_all_items:
            ...
            check_results = any
        return check_results(operator.matches(match_value=match_item_value, condition_value=condition_value) for match_item_value in match_value)
...
class ModelQueryChoiceMixin(object):
    ...
    def get_queryset(self):
        ...
        if self.queryset is None:
            raise ValueError(b'%s.queryset cannot be None!' % self.__class__.__name__)
...
class BaseConditionModelMultipleChoice(ModelQueryChoiceMixin, BaseConditionChoice):
    ...
    """
    operators = ConditionOperators([
     AnyOperator.with_overrides(name=_(b'Any')),
     UnsetOperator.with_overrides(name=_(b'None')),
     IsOneOfOperator,
     IsNotOneOfOperator])
...
class ConditionChoices(OrderedRegistry):
    ...
    default_errors = dict(DEFAULT_ERRORS, **{ALREADY_REGISTERED: _(b'Could not register condition choice %(item)s: This choice is already registered or its ID conflicts with another choice.'), 
       ATTRIBUTE_REGISTERED: _(b'Could not register condition choice %(item)s: Another choice (%(duplicate)s) is already registered with the same ID.'), 
       NOT_REGISTERED: _(b'No condition choice was found matching "%(attr_value)s".'), 
       UNREGISTER: _(b'Could not unregister condition choice %(item)s: This condition was not yet registered.')})
    ...
    def get_choice(self, choice_id, choice_kwargs={}):
        ...
        """
        choice_cls = self.get(b'choice_id', choice_id)
