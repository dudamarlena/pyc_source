...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/fields/json_field.py
...
class JSONFormField(forms.CharField):
    ...
    def __init__(self, encoder=None, encoder_cls=None, encoder_kwargs=None, *args, **kwargs):
        ...
        if encoder is None:
            if encoder_cls is None:
                ...
                encoder_kwargs = {b'strip_datetime_ms': False}
            encoder = encoder_cls(**dict({b'indent': 2, 
               b'separators': (',', ': '), 
               b'sort_keys': True}, **encoder_kwargs))
    ...
    def to_python(self, value):
        ...
        if isinstance(value, six.string_types):
            ...
            try:
                ...
                raise ValidationError(six.text_type(e), code=b'invalid', params={b'value': value})
...
class JSONField(models.TextField):
    ...
    default_error_messages = {b'invalid_type': _(b'%(type)s is not a supported value type.')}

    def __init__(self, verbose_name=None, name=None, encoder=None, encoder_cls=None, encoder_kwargs=None, **kwargs):
        """Initialize the field.
        ...
        """
        super(JSONField, self).__init__(verbose_name, name, blank=kwargs.pop(b'blank', True), **kwargs)
        if encoder is not None:
            warnings.warn(b'The encoder argument to JSONField has been replaced by the encoder_cls and encoder_kwargs arguments. Support for encoder is deprecated.', RemovedInDjblets20Warning)
            self.encoder = encoder
        ...
        self.encoder_kwargs[b'sort_keys'] = True
        return
    ...
    def contribute_to_class(self, cls, name):
        ...
        setattr(cls, b'get_%s_json' % self.name, get_json)
        setattr(cls, b'set_%s_json' % self.name, set_json)
    ...
    def post_init(self, instance, **kwargs):
        ...
        if isinstance(value, (dict, list)):
            ...
            raise ValidationError(self.error_messages[b'invalid_type'], code=b'invalid', params={b'type': type(value)})
    ...
    def get_db_prep_save(self, value, *args, **kwargs):
        ...
        """
        return super(JSONField, self).get_db_prep_save(self.dumps(value), *args, **kwargs)
    ...
    def loads(self, val):
        ...
        try:
            ...
            if isinstance(val, six.string_types):
                logger.warning(b'JSONField decode error. Expected dictionary, got string for input "%s"', val)
                val = json.loads(val, encoding=settings.DEFAULT_CHARSET)
        except ValueError:
            try:
                val = literal_eval(val)
            except Exception as e:
                logger.error(b'Failed to eval JSONField data "%r": %s', val, e)
                val = {}

            if isinstance(val, six.string_types):
                logger.warning(b'JSONField decode error after literal_eval: Expected dictionary, got string: %r', val)
                val = {}
    ...
    def formfield(self, **kwargs):
        ...
        """
        formfield_kwargs = dict({b'form_class': JSONFormField, 
           b'encoder': None, 
           b'encoder_cls': self.encoder_cls, 
           b'encoder_kwargs': self.encoder_kwargs}, **kwargs)
        if self.encoder_cls is None:
            formfield_kwargs[b'encoder'] = self.encoder
    ...
    def deconstruct(self):
        ...
        Returns:
            ...
            arguments).
        """
        name, path, field_args, field_kwargs = super(JSONField, self).deconstruct()
        default = field_kwargs.get(b'default')
        if default is not None:
            if isinstance(default, type):
                default = self.dumps(default())
            else:
                default = self.dumps(default)
            field_kwargs[b'default'] = default
