...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/integrations/manager.py
# Compiled at: 2019-06-12 01:17:17
"""A manager for tracking and working with integrations."""
from __future__ import unicode_literals
import atexit, logging, threading, weakref
from django.conf import settings
from django.core.exceptions import ImproperlyConfigured
from django.db.models.signals import post_delete, post_save
from django.utils import six
from djblets.cache.synchronizer import GenerationSynchronizer
from djblets.integrations.errors import IntegrationAlreadyRegisteredError, IntegrationNotRegisteredError, IntegrationRegistrationError
...
class IntegrationManager(object):
    ...
    def __init__(self, config_model):
        ...
        Args:
            config_model (type):
                ...
                :py:class:`djblets.integrations.models.BaseIntegrationConfig`.
        """
        if b'djblets.integrations' not in settings.INSTALLED_APPS:
            raise ImproperlyConfigured(b'IntegrationManager requires djblets.integrations to be listed in settings.INSTALLED_APPS.')
        middleware = b'djblets.integrations.middleware.IntegrationsMiddleware'
        if middleware not in settings.MIDDLEWARE_CLASSES:
            raise ImproperlyConfigured(b'IntegrationManager requires %s to be listed in settings.MIDDLEWARE_CLASSES' % middleware)
        self.config_model = config_model
        key = b'integrationmgr:%s.%s' % (
        ...
        self._gen_sync = GenerationSynchronizer(b'%s:gen' % key)
        instance_id = id(self)
        _integration_managers[instance_id] = self
        dispatch_uid = b'%s:%s' % (key, id(self))
        post_delete.connect(self._on_config_changes, sender=config_model, dispatch_uid=dispatch_uid)
        post_save.connect(self._on_config_changes, sender=config_model, dispatch_uid=dispatch_uid)
    ...
    def get_integration_configs(self, integration_cls=None, **filter_kwargs):
        ...
        """
        key = self._make_config_filter_cache_key(integration_cls, **filter_kwargs)
        try:
            configs = self._integration_configs[key]
        except KeyError:
            queryset = self.config_model.objects.all()
            if integration_cls:
                queryset = queryset.filter(integration_id=integration_cls.integration_id)
            if filter_kwargs:
    ...
    def register_integration_class(self, integration_cls):
        ...
        if not integration_cls.integration_id:
            integration_cls.integration_id = b'%s.%s' % (
        ...
        try:
            ...
            raise IntegrationRegistrationError(b'Unexpected error when constructing integration %s: %s' % (
    ...
    def unregister_integration_class(self, integration_cls):
        ...
        """
        integration_id = integration_cls.integration_id
        if not integration_id or integration_id not in self._integration_classes:
            raise IntegrationNotRegisteredError(integration_id)
        ...
        if integration.enabled:
            try:
                ...
                logging.exception(b'Unexpected error when shutting down integration %r: %s', integration_cls, e)
    ...
    def _recalc_enabled_integrations(self):
        ...
        """
        enabled_integration_ids = set(self.config_model.objects.filter(enabled=True).distinct().values_list(b'integration_id', flat=True))
        for integration in self.get_integrations():
            should_enable = integration.integration_id in enabled_integration_ids
    ...
    def _make_config_filter_cache_key(self, integration_cls, **filter_kwargs):
        ...
        """
        if integration_cls:
            return b'%s:%s' % (integration_cls.integration_id, filter_kwargs)
        else:
            return b'*:%s' % (filter_kwargs,)
