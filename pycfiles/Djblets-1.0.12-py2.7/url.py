...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/avatars/services/url.py
...
class URLAvatarService(AvatarService):
    ...
    """
    avatar_service_id = b'url'
    name = _(b'URL Avatar Service')
    hidden = True
    ...
    def get_avatar_urls_uncached(self, user, size):
        ...
        Returns:
            ...
            avatar URLs.
        """
        return self._settings_manager_class(user).configuration_for(self.avatar_service_id)
    ...
    def get_etag_data(self, user):
        ...
        """
        urls = self._settings_manager_class(user).configuration_for(self.avatar_service_id)
        data = [
