...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/conditions/tests/test_values.py
...
from djblets.conditions.values import ConditionValueBooleanField, ConditionValueFormField, ConditionValueIntegerField, ConditionValueModelField, ConditionValueMultipleModelField, ConditionValueRegexField
...
class ConditionValueFormFieldTests(TestCase):
    ...
    def test_serialize_value(self):
        ...
        field = ConditionValueFormField(forms.ModelChoiceField(queryset=Site.objects.all()))
    ...
    def test_get_from_form_data(self):
        ...
        data = QueryDict(b'test-field=abc123')
        files = QueryDict(b'')
        self.assertEqual(field.get_from_form_data(data, files, b'test-field'), b'abc123')

    def test_render_html(self):
        """Testing ConditionValueFormField.render_html"""
        field = ConditionValueFormField(forms.CharField())
        self.assertHTMLEqual(field.render_html(), b'<input name="XXX" type="text" />')
...
class ConditionValueBooleanFieldTests(TestCase):
    ...
    def test_deserialize_value_with_true(self):
        ...
        self.assertTrue(self.value_field.deserialize_value(b'true'))
        self.assertTrue(self.value_field.deserialize_value(b'True'))
        self.assertTrue(self.value_field.deserialize_value(b'1'))
    ...
    def test_deserialize_value_with_false(self):
        ...
        self.assertFalse(self.value_field.deserialize_value(b'false'))
        self.assertFalse(self.value_field.deserialize_value(b'False'))
        self.assertFalse(self.value_field.deserialize_value(b'0'))

    def test_prepare_value_for_widget(self):
        """Testing ConditionValueBooleanField.prepare_value_for_widget"""
        self.assertEqual(self.value_field.prepare_value_for_widget(True), b'True')
        self.assertEqual(self.value_field.prepare_value_for_widget(False), b'False')
...
class ConditionValueIntegerFieldTests(TestCase):
    ...
    def test_deserialize_value(self):
        ...
        self.assertEqual(self.value_field.deserialize_value(b'100'), 100)
...
class ConditionValueMultipleModelFieldTests(TestCase):
    ...
    def test_init_with_callable(self):
        ...
        callable
        """
        field = ConditionValueMultipleModelField(queryset=lambda : Site.objects.all())
        self.assertTrue(isinstance(field.field.queryset, QuerySet))
...
class ConditionValueRegexFieldTests(TestCase):
    ...
    def test_deserialize_value(self):
        ...
        regex_obj = self.value_field.deserialize_value(b'ab[^c]d+e?')
        regex_type = type(re.compile(b''))
        self.assertTrue(isinstance(regex_obj, regex_type))
        self.assertEqual(regex_obj.pattern, re.compile(b'ab[^c]d+e?').pattern)
    ...
    def test_deserialize_value_with_bad_pattern(self):
        ...
        with self.assertRaises(InvalidConditionValueError):
            self.value_field.deserialize_value(b'*')

    def test_serialize_value(self):
        """Testing ConditionValueRegexField.serialize_value"""
        regex_obj = re.compile(b'ab[^c]d+e?')
        self.assertEqual(self.value_field.serialize_value(regex_obj), b'ab[^c]d+e?')