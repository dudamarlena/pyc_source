...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/db/tests/test_json_field.py
...
class JSONFieldTests(TestCase):
    ...
    def test_init_with_custom_encoder_class(self):
        ...
        with warnings.catch_warnings(record=True) as (w):
            field = JSONField(encoder_cls=MyEncoder, encoder_kwargs={b'default_msg': b'What even is this?'})
        self.assertEqual(field.dumps(MyEncoder), b'"What even is this?"')
    ...
    def test_init_with_custom_encoder_instance(self):
        ...
        self.assertEqual(six.text_type(message), b'The encoder argument to JSONField has been replaced by the encoder_cls and encoder_kwargs arguments. Support for encoder is deprecated.')
    ...
    def test_init_with_dict_value(self):
        ...
        value[b'c'] = 3
        self.assertEqual(model.myfield, {b'a': 1, 
           b'b': 2})
        self.assertEqual(model.get_myfield_json(), b'{"a": 1, "b": 2}')
    ...
    def test_init_with_dict_value_empty(self):
        ...
        self.assertEqual(model.get_myfield_json(), b'{}')
    ...
    def test_init_with_list_value(self):
        ...
        self.assertEqual(model.get_myfield_json(), b'[1, 2]')
    ...
    def test_init_with_json_string_value(self):
        ...
        model = MyModel(myfield=b'{"a": 1, "b": 2}')
        self.assertEqual(model.myfield, {b'a': 1, 
           b'b': 2})
        self.assertEqual(model.get_myfield_json(), b'{"a": 1, "b": 2}')
    ...
    def test_init_with_bad_type(self):
        ...
        message = b"<class 'djblets.db.tests.test_json_field.MyModel'> is not a supported value type."
        with self.assertRaisesMessage(ValidationError, message):
            MyModel(myfield=MyModel())

    def test_dumps_with_json_dict(self):
        """Testing JSONField with dumping a JSON dictionary"""
        result = self.field.dumps({b'a': 1, b'b': 2})
    ...
    def test_loading_json_broken_dict(self):
        ...
        result = self.field.loads(b'{u"a": 1, u"b": 2}')
        self.assertTrue(isinstance(result, dict))
        self.assertTrue(b'a' in result)
        self.assertTrue(b'b' in result)
...
class JSONFormFieldTests(TestCase):
    ...
    def test_init_with_custom_encoder_class(self):
        ...
        field = JSONFormField(encoder_cls=MyEncoder, encoder_kwargs={b'default_msg': b'What even is this?'})
        self.assertEqual(field.prepare_value({b'a': 1, 
           b'b': 2, 
    ...
    def test_to_python_with_validation_error(self):
        ...
        with self.assertRaises(ValidationError) as (cm):
            field.to_python(b'{a: 1}')
        self.assertEqual(cm.exception.message, b'Expecting property name enclosed in double quotes: line 1 column 2 (char 1)')
        self.assertEqual(cm.exception.code, b'invalid')
        self.assertEqual(cm.exception.params, {b'value': b'{a: 1}'})