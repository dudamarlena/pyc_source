...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/webapi/testing/testcases.py
...
class WebAPITestCaseMixin(TestCase):
    ...
    def shortDescription(self):
        ...
        if self.sample_api_url:
            ...
            if getattr(test_method, b'is_test_template', False):
                desc = desc.replace(b'<URL>', self.sample_api_url)
        return desc
    ...
    def api_get_with_response(self, path, data={}, **kwargs):
        ...
        """
        return self.api_call(client_http_method=self.client.get, path=path, data=data, **kwargs)
    ...
    def api_post_with_response(self, path, data={}, expected_status=201, **kwargs):
        ...
        Returns:
            ...
            :py:class:`HttpResponse`.
        """
        return self.api_call(client_http_method=self.client.post, path=path, data=data, expected_status=expected_status, **kwargs)
    ...
    def api_put_with_response(self, path, data={}, content_type=MULTIPART_CONTENT, **kwargs):
        ...
        Returns:
            ...
            :py:class:`HttpResponse`.
        """
        return self.api_call(client_http_method=self.client.put, path=path, data=data, content_type=content_type, **kwargs)
    ...
    def api_delete_with_response(self, path, expected_status=204, **kwargs):
        ...
        Returns:
            ...
            :py:class:`HttpResponse`.
        """
        return self.api_call(client_http_method=self.client.delete, path=path, expected_status=expected_status, **kwargs)

    def assertHttpOK(self, response, check_last_modified=False, check_etag=False):
        ...
        if check_last_modified:
            self.assertIn(b'Last-Modified', response)
        if check_etag:
            self.assertIn(b'ETag', response)
    ...
    def api_call(self, client_http_method, path, data=None, follow_redirects=False, expected_status=200, expected_redirects=[], expected_headers={}, expected_mimetype=None, expected_num_queries=None, expected_json=True, return_http_response=True, **extra):
        ...
        def _call_api():
            return client_http_method(path=path, data=data, follow=follow_redirects, HTTP_X_REQUESTED_WITH=b'XMLHttpRequest', **extra)

        if path.startswith(self.base_url):
            path = path[len(self.base_url):]
        if b'query' in extra:
            warnings.warn(b'%s should be passed a `data` argument. `query` is deprecated and will be removed in Djblets 2.0.', RemovedInDjblets20Warning)
            data = extra.pop(b'query')
        if extra.get(b'content_type') == MULTIPART_CONTENT:
            data = encode_multipart(BOUNDARY, data)
        print(b'Performing HTTP %s for API %s' % (
         client_http_method.__name__.upper(), path))
        if data is not None:
            print(b'Request data = %r' % data)
        if expected_num_queries is None:
            response = _call_api()
        else:
            with self.assertNumQueries(expected_num_queries):
                response = _call_api()
        print(b'Raw API response: %r' % response.content)
        ...
        if expected_status in (204, 405):
            ...
            if expected_status != 302 and expected_json:
                ...
                rsp = response.content
            print(b'Parsed API response:')
            pprint.pprint(rsp)
            if expected_status >= 400:
                self.assertIsNone(expected_mimetype)
                if expected_status != 405:
                    self.assertEqual(response[b'Content-Type'], self.error_mimetype)
            elif expected_status != 302:
                self.assertIsNotNone(expected_mimetype)
                self.assertEqual(response[b'Content-Type'], expected_mimetype)
            if expected_redirects:
                self.assertEqual(len(response.redirect_chain), len(expected_redirects))
                for redirect in expected_redirects:
                    self.assertEqual(response.redirect_chain[0][0], self.base_url + expected_redirects[0])
