...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djblets/util/http.py
...
def set_last_modified(response, timestamp):
    ...
    """
    response[b'Last-Modified'] = http_date(timestamp)
...
def get_modified_since(request, last_modified):
    ...
    """
    if_modified_since = request.META.get(b'HTTP_IF_MODIFIED_SINCE', None)
...
def etag_if_none_match(request, etag):
    ...
    """
    return etag == request.META.get(b'HTTP_IF_NONE_MATCH', None)
...
def etag_if_match(request, etag):
    ...
    """
    return etag == request.META.get(b'HTTP_IF_MATCH', None)
...
def get_http_accept_lists(request):
    ...
    """
    if hasattr(request, b'djblets_acceptable_mimetypes') and hasattr(request, b'djblets_unacceptable_mimetypes'):
        return (request.djblets_acceptable_mimetypes,
         request.djblets_unacceptable_mimetypes)
    acceptable_mimetypes = []
    unacceptable_mimetypes = []
    for accept_item in request.META.get(b'HTTP_ACCEPT', b'').strip().split(b','):
        parts = accept_item.strip().split(b';')
        mimetype = parts[0]
        priority = 1.0
        for part in parts[1:]:
            try:
    ...
    setattr(request, b'djblets_acceptable_mimetypes', acceptable_mimetypes)
    setattr(request, b'djblets_unacceptable_mimetypes', unacceptable_mimetypes)
    return (
...
def get_http_requested_mimetype(request, supported_mimetypes):
    ...
    """
    acceptable_mimetypes, unacceptable_mimetypes = get_http_accept_lists(request)
    ...
    if not supported_mimetypes_set.intersection(acceptable_mimetypes_set):
        if b'*/*' in unacceptable_mimetypes:
            acceptable_mimetypes = []
            unacceptable_mimetypes = supported_mimetypes
        else:
            acceptable_mimetypes = [ mimetype for mimetype in supported_mimetypes if mimetype not in unacceptable_mimetypes_set
                                   ]
...
def is_mimetype_a(mimetype, parent_mimetype):
    ...
    parent_parts = parent_mimetype.split(b'/')
    return parts[0] == parent_parts[0] and (parts[1] == parent_parts[1] or parts[1].endswith(b'+' + parent_parts[1]))
...
def get_url_params_except(query, *params):
    ...
    others.
    """
    return urlencode([ (key.encode(b'utf-8'), value.encode(b'utf-8')) for key, value in six.iteritems(query) if key not in params
                     ])