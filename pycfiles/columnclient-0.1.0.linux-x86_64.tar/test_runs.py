...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/tests/test_runs.py
...
class TestRunManager(testtools.TestCase):
    ...
    def test_create(self, mock_session):
        ...
        instance.post.return_value = utils.MockResponse(text='{"playbook_path":"/hello_world.yml","progress": 0,"state":"RUNNING","id":"0904eef0-563f-47c4-b586-851c6ea2ba88"}')
        col_client = client.Client()
        result = col_client.runs.create('playbook_file', inventory_file='inventory_file', user='user', password='password', private_key_file='private_key_file')
        url = 'http://127.0.0.1:48620/runs'
        data = {'playbook_path': 'playbook_file', 
        ...
        instance.post.assert_called_once_with(url, data=json.dumps(data), headers=headers)
    ...
    def test_get(self, mock_session):
        instance = mock_session.return_value
        instance.get.return_value = utils.MockResponse(text='{"playbook_path":"/hello_world.yml","progress": 0,"state":"RUNNING","id":"0904eef0-563f-47c4-b586-851c6ea2ba88"}')
        run_data = {'playbook_path': '/hello_world.yml', 'progress': 0, 
           'state': 'RUNNING', 
           'id': '0904eef0-563f-47c4-b586-851c6ea2ba88'}
        run = runs.Run('manager', run_data)
        col_client = client.Client()
        result = col_client.runs.get(run)
        url = 'http://127.0.0.1:48620/runs/0904eef0-563f-47c4-b586-851c6ea2ba88'
    ...
    def test_list(self, mock_session):
        url = 'http://127.0.0.1:48620/runs'
        instance = mock_session.return_value
        instance.get.return_value = utils.MockResponse(text='[{"playbook_path":"/hello_world.yml","progress": 0,"state":"RUNNING","id":"0904eef0-563f-47c4-b586-851c6ea2ba88"}]', status_code=200)
        col_client = client.Client()
