...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/asynctwitch/bots.py
# Compiled at: 2017-08-12 06:52:49
# Size of source mod 2**32: 41467 bytes
import asyncio, traceback, sys, os, re, math, json, configparser, time, subprocess, functools, sqlite3, pathlib
...
class Bot:
    ...
    def __init__(self, **kwargs):
        ...
        self.message_count = 1
        self.regex = {'data':re.compile('^(?:@(?P<tags>\\S+)\\s)?:(?P<data>\\S+)(?:\\s)(?P<action>[A-Z]+)(?:\\s#)(?P<channel>\\S+)(?:\\s(?::)?(?P<content>.+))?'), 
         'ping':re.compile('PING (?P<content>.+)'), 
         'author':re.compile('(?P<author>[a-zA-Z0-9_]+)!(?P=author)@(?P=author).tmi.twitch.tv'), 
    ...
    def _get_stats(self):
        ...
        while True:
            try:
                for c in self.chan:
                    j = yield from _get_url(self.loop, 'https://api.twitch.tv/kraken/channels/{}?client_id={}'.format(c[1:], self.client_id))
                    self.channel_stats[c] = {'mature':j['mature'], 
                     'title':j['status'], 
                     'game':j['game'], 
                     'id':j['_id'], 
                    ...
                    j = yield from _get_url(self.loop, 'https://tmi.twitch.tv/group/user/{}/chatters'.format(c[1:]))
                    ...
                    self.viewers[c]['global_moderators'] = j['chatters']['global_mods']
    ...
    def say(self, channel, message):
        ...
        """
        if len(message) > 500:
            raise Exception('The maximum amount of characters in one message is 500, you tried to send {} characters'.format(len(message)))
    ...
    def timeout(self, user, seconds=600, reason=''):
        ...
        """
        yield from self._send_privmsg(user.channel, '.timeout {} {} {}'.format(user.name, seconds, reason))
        if False:
            yield None
    ...
    def _tcp_echo_client(self):
        ...
        self.reader, self.writer = yield from asyncio.open_connection((self.host),
                                                                ...
                                                                if action == 'CLEARCHAT':
                                                                    if not content:
                                                                        ...
                                                                        if 'ban-duration' in tags.keys():
                                                                            yield from self.event_timeout(User(content, channel), tags)
                                                                        else:
                                                                            yield from self.event_ban(User(content, channel), tags)
                                                                else:
                                                                    if action == 'HOSTTARGET':
                                                                        ...
                                                                        if action == 'USERNOTICE':
                                                                            message = content or ''
                                                                            user = tags['login']
                                                                            yield from self.event_subscribe(Message(message, user, channel, tags), tags)
                                                                        else:
                                                                            if action == 'CAP':
                                                                                continue
                                                                            else:
    ...
    def play_file(self, file):
        ...
        j = yield from self.loop.run_in_executor(None, subprocess.check_output, ['ffprobe', '-v', '-8',
         '-print_format', 'json',
         '-show_format', file])
        ...
        try:
            ...
            if self.song == Song():
                self.song.setattrs({'title':' '.join(file.split('.')[:-1]), 
                 'duration':t})
        ...
        if rm.content.startswith(self.prefix):
            ...
            if w in self.commands:
                if not self.commands[w].unprefixed:
                    if self.commands[w].admin:
                        if rm.author.name not in self.admins:
...
class CurrencyBot(Bot):
    __doc__ = ' A Bot with support for currency '
    ...
    def add_currency(self, user, amount):
        ...
        self.currency_cursor.execute('UPDATE currency SET balance = ? WHERE username = ?', (
         balance + amount, user))
    ...
    def remove_currency(self, user, amount, force_remove=False):
        ...
        if amount > balance:
            if not force_remove:
                raise Exception('{} owns {} {0.currency_name}, unable to remove {}.Use force_remove=True to force this action.'.format(user, balance, self, amount))
        self.currency_cursor.execute('UPDATE currency SET balance = ? WHERE username = ?', (
         balance - amount, user))
...
class ViewTimeBot(Bot):
    __doc__ = ' A Bot to track view time '
    ...
    def add_time(self, user, amount):
        ...
        self.time_cursor.execute('UPDATE time_watched SET time = ? WHERE username = ?', (
         time + amount, user))
...
class RankedBot(ViewTimeBot, CurrencyBot):
    ...
    def check_user_rank(self, user, rank):
        """ Check if the user is already in the database """
        return bool(list(self.time_cursor.execute('SELECT * FROM user_ranks WHERE username = ? AND rank = ?', (
         user, rank))))
    ...
    def add_rank(self, name, points=0, time_watched=0, type_rank='points'):
        if type_rank == 'points':
            ...
            if type_rank == 'time_watched':
                self.rank_cursor.execute('INSERT INTO watched_ranks VALUES (?,?)', (
                 time_watched, name))
            else:
