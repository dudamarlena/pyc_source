...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgpaginate/controllers.py
# Compiled at: 2008-07-29 11:53:18
...
def paginate(var_name, default_order='', limit=10, allow_limit_override=False, max_pages=5):
    ...
    def entangle(func):
        ...
        def decorated(func, *args, **kw):
            ...
            if not allow_limit_override:
                limit_ = limit
            log.debug('Pagination params: page=%s, limit=%s, order=%s, reversed=%s', page, limit_, order, reversed)
            output = func(*args, **kw)
            ...
            if order and not default_order:
                raise 'If you want to enable ordering you need to provide a default_order'
            row_count = 0
            ...
            if page_count <= max_pages:
                ...
                pages_to_show = _select_pages_to_show(page_count=page_count, current_page=page, max_pages=max_pages)
            input_values = kw.copy()
            input_values.pop('self', None)
            for (k, v) in cherrypy.request.params.iteritems():
                if k.startswith('tg_paginate_') and not k.startswith('tg_paginate_%s_' % var_name):
                    input_values[k] = v

            paginate = Paginate(var_name=var_name, current_page=page, limit=limit_, pages=pages_to_show, page_count=page_count, order=order, input_values=input_values, reversed=reversed)
            ...
            headers['tg_paginate_%s_reversed' % var_name] = reversed and 1 or 0
            headers['tg_paginate_%s_nav_links' % var_name] = (',').join(map(str, pages_to_show))
...
class Paginate:
    ...
    def __init__(self, var_name, current_page, pages, page_count, input_values, limit, order, reversed):
        self.var_name = var_name
        self.pages = pages
        self.limit = limit
    ...
    def get_href(self, page, order=None, reverse_order=None):
        ...
        input_values.update({tg_paginate_no: page, tg_paginate_order: order, tg_paginate_reversed: reversed})
