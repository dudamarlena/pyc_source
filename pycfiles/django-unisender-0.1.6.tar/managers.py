...
# [GCC 8.4.0]
# Embedded file name: /home/africa/work/python/djnago-unisender/unisender/tests/managers.py
# Compiled at: 2014-08-26 07:08:28
from django.test import TestCase
from mock import patch
from unisender.models import Tag, Field, SubscribeList, Campaign

class TagManagerTestCase(TestCase):
    tag_result = {'result': [{'id': 1, 'name': 'test 1'}, {'id': 2, 'name': 'test 2'}]}
    ...
    def test__get_tags(self):
        ...
        resulted_tags = {'result': [{'id': 1, 'name': 'test 1'}, {'id': 2, 'name': 'test 2'}]}
...
class FieldManagerTestCase(TestCase):
    field_result = {'result': [
                {'id': 1, 'name': 'test 1', 'type': 'string', 
                   'is_visible': 1, 'view_pos': 1},
                {'id': 2, 'name': 'test 2', 'type': 'text', 
                   'is_visible': 1, 'view_pos': 3}]}
    ...
    def test__get_and_update_fields(self):
        Field.objects.create(name='test 1', field_type='string', sort=3)
...
class ListManagerTestCase(TestCase):
    list_get_result = {'result': [{'id': 1, 'title': 'test 1'}, {'id': 2, 'title': 'test 2'}]}
    ...
    def test__get_subscribe_lists(self):
        ...
        resulted_list = {'result': [{'id': 1, 'title': 'test 1'}, {'id': 2, 'title': 'test 2'}]}
...
class CampaignManagerTestCase(TestCase):
    list_get_campaings = {'result': [
                {'id': 1, 'start_time': '2011-08-01 19:30:00', 
                   'status': 'completed'},
                {'id': 2, 'start_time': '2011-08-10 19:30:00', 
                   'status': 'analysed'}]}
