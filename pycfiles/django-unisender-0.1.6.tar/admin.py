...
# [GCC 8.4.0]
# Embedded file name: /home/africa/work/python/djnago-unisender/unisender/tests/admin.py
# Compiled at: 2014-08-26 07:10:29
...
from unisender.models import Tag, Field, SubscribeList, Subscriber, EmailMessage, Campaign
from unisender.admin import FieldAdmin, SubscribeListAdmin, SubscriberAdmin, EmailMessageAdmin, CampaignAdmin, AttachmentInline, AttachmentInlineReadOnly
...
class UnisenderAdminTestCase(TestCase):
    ...
    def test_open_unisender_app(self):
        u"""Открываем страницу приложения unisneder в админке"""
        response = self.client.get(reverse('admin:app_list', kwargs={'app_label': 'unisender'}))
...
class TagAdminTestCase(TestCase):
    ...
    def test_open_unisender_page(self):
        u"""Открываем страницу Tags в админке"""
    ...
    def test_add_tag(self):
        ...
        post_data = {'name': 'test_tag', 'csrfmiddlewaretoken': csrf}
...
class FieldAdminTestCase(TestCase):
    ...
    def test_add_field(self):
        ...
        self.assertEqual(response.status_code, 200)
        csrf = get_csrf_token(response)
        post_data = {'name': 'test_field', 'csrfmiddlewaretoken': csrf, 
           'sort': 1, 
           'field_type': 'string'}
        response = self.client.post(url, post_data)
...
class SubscribeListAdminTestCase(TestCase):
    ...
    def test_open_unisender_page(self):
        u"""Открываем страницу subscribeList в админке"""
        response = self.client.get(reverse('admin:unisender_subscribelist_changelist'))
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], SUBSCRIBELIST_LIST)

    def test_add_subscribe_list(self):
        u"""Добавляем список рассылки"""
        start_subscribe_list_count = SubscribeList.objects.count()
        url = reverse('admin:unisender_subscribelist_add')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        csrf = get_csrf_token(response)
        post_data = {'title': 'test_subscribe_list', 'csrfmiddlewaretoken': csrf}
        response = self.client.post(url, post_data)
    ...
    def test_update_subscribe_list(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], SUBSCRIBELIST_DETAIL + str(subscribe_list.unisender_id))
        csrf = get_csrf_token(response)
        post_data = {'title': 'test_subscribe_list', 'csrfmiddlewaretoken': csrf, 
...
class SubscriberAdminTestCase(TestCase):
    ...
    def test_get_readonly_fields(self):
        ...
        self.assertIn('contact', self.admin.get_readonly_fields(self.request, obj=subscribe_list))
        self.assertIn('contact_type', self.admin.get_readonly_fields(self.request, subscribe_list))

    def test_open_unisender_page(self):
        u"""Открываем страницу подписчиков в админке"""
        response = self.client.get(reverse('admin:unisender_subscriber_changelist'))
        self.assertEqual(response.status_code, 200)

    def test_add_subscribe_list(self):
        u"""Добавляем подписчика через админку"""
        ...
        csrf = get_csrf_token(response)
        subscribe_list = SubscribeList.objects.create(title='test')
        post_data = {'list_ids': subscribe_list.pk, 'csrfmiddlewaretoken': csrf, 
           'contact_type': 'email', 
...
class EmailMessageAdminTestCase(TestCase):
    ...
    def test_open_unisender_page(self):
        u"""Открываем страницу email сообщений в админке"""
        response = self.client.get(reverse('admin:unisender_emailmessage_changelist'))
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], EMAIL_MESSAGES_LIST)

    def test_add_email_message(self):
        u"""Добавляем email сообщение через админку"""
        ...
        csrf = get_csrf_token(response)
        subscribe_list = SubscribeList.objects.create(title='test')
        post_data = {'list_id': subscribe_list.pk, 'csrfmiddlewaretoken': csrf, 
           'sender_name': 'test', 
    ...
    def test_update_email_message(self):
        u"""Редактируем сообщение через админку"""
        subscribe_list = SubscribeList.objects.create(title='test')
        email = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list, unisender_id=1, sync=True)
        url = reverse('admin:unisender_emailmessage_change', args=(email.pk,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], EMAIL_MESSAGES_DETAIL + str(email.unisender_id))

    def test_delete_email_message(self):
        u"""Удаляем email сообщение"""
        start_unisender_count = EmailMessage.objects.count()
        subscribe_list = SubscribeList.objects.create(title='test')
        message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list)
    ...
    def test_inlines_readonly(self):
        u"""Проверяем что вложения становятся readonly"""
    ...
    def test_delete_selected_emails(self):
        ...
        subscribe_list = SubscribeList.objects.create(title='test')
        for x in xrange(3):
            EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list)
    ...
    def test_save_model(self):
        ...
        message_2 = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list, unisender_id=1)
        message_2.create_email_message = Mock(return_value=None)
        self.admin.response_add(self.request, message_2)
        self.assertEqual(EmailMessage.objects.count(), start_unisender_count + 2)
    ...
    def test_get_readonly_fields(self):
        ...
        self.assertListEqual(readonly_fields_initial, self.admin.get_readonly_fields(self.request))
        subscribe_list = SubscribeList.objects.create(title='test')
        message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list, sync=True)
        ...
        self.assertItemsEqual(readonly_fields_all, self.admin.get_readonly_fields(self.request, message))

    def test_get_fieldsets(self):
        fieldsets_initial = [
         [
          'Unisender', {'fields': ['unisender_id', 'sync', 'get_last_error']}],
         [
          'Сообщение',
         ...
         [
          'Автоматическая отправка', {'fields': ['series_day', 'series_time']}]]
        self.assertItemsEqual(fieldsets_initial, self.admin.get_fieldsets(self.request))
        subscribe_list = SubscribeList.objects.create(title='test')
        message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list, unisender_id=1)
        fieldsets_all = [
         [
          'Unisender', {'fields': ['unisender_id', 'sync', 'get_last_error']}],
         [
         ...
         [
          'Автоматическая отправка', {'fields': ['series_day', 'series_time']}]]
        self.assertItemsEqual(fieldsets_all, self.admin.get_fieldsets(self.request, message))
...
class CampaignAdminTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.request = RequestFactory().get(reverse('admin:index'))
        subscribe_list = SubscribeList.objects.create(title='test')
        self.message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', lang='ru', generate_text=1, wrap_type='skip', list_id=subscribe_list, sync=True)
        self.request.user = self.user
        self.maxDiff = None
        return

    def test_open_unisender_page(self):
        u"""Открываем страницу рассылки в админке"""
        response = self.client.get(reverse('admin:unisender_campaign_changelist'))
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], CAMPAIGN_LIST)

    def test_add_campaign(self):
        u"""Добавляем рассылку через админку"""
        start_subscriber_count = Campaign.objects.count()
        url = reverse('admin:unisender_campaign_add')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        csrf = get_csrf_token(response)
        post_data = {'csrfmiddlewaretoken': csrf, 'name': 'test', 
        ...
        response = self.client.post(url, post_data)
        self.assertRedirects(response, reverse('admin:unisender_campaign_changelist'))
        self.assertEqual(Campaign.objects.count(), start_subscriber_count + 1)

    def test_update_campaign(self):
        u"""Редактируем рассылку"""
        campaign = Campaign.objects.create(name='test', email_message=self.message, unisender_id=1)
        url = reverse('admin:unisender_campaign_change', args=(campaign.pk,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertIn('unisender_site', response.context)
        self.assertEqual(response.context['unisender_site'], CAMPAIGN_DETAIL + str(campaign.unisender_id))
        csrf = get_csrf_token(response)
        post_data = {'csrfmiddlewaretoken': csrf, 'name': 'test-2', 
           'email_message': self.message.pk, 
           'track_read': 0, 
           'track_links': 0, 
           'track_ga': 0, 
        ...
        response = self.client.post(url, post_data)
        self.assertRedirects(response, reverse('admin:unisender_campaign_changelist'))
        self.assertTrue(Campaign.objects.filter(name='test-2').exists())

    def test_delete_campaign(self):
        u"""Удаляем рассылку"""
        start_unisender_count = Campaign.objects.count()
        campaign = Campaign.objects.create(name='test', email_message=self.message)
    ...
    def test_delete_selected_emails(self):
        start_unisender_count = Campaign.objects.count()
        for x in xrange(3):
            Campaign.objects.create(name='test', email_message=self.message)
    ...
    def test_get_readonly_fields(self):
        ...
        self.assertListEqual(readonly_fields_initial, self.admin.get_readonly_fields(self.request))
        campaign = Campaign.objects.create(name='test', email_message=self.message, sync=True)
        readonly_fields_all = [
         ...
         'err_dest_invalid', 'id', 'err_spam_skipped', 'err_spam_folder',
        ...
        self.assertItemsEqual(readonly_fields_all, self.admin.get_readonly_fields(self.request, campaign))
    ...
    def test_get_fieldsets(self):
        ...
        self.assertItemsEqual(fieldsets_all, self.admin.get_fieldsets(self.request, campaign))

    def test_get_urls(self):
        campaign = Campaign.objects.create(name='test', email_message=self.message, sync=True)
        self.assertEqual(reverse('admin:unisender_campaign_get_statistic', kwargs={'pk': campaign.pk}), '/admin/unisender/campaign/%s/get_statistic/' % campaign.pk)