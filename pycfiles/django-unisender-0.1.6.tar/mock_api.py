...
# [GCC 8.4.0]
# Embedded file name: /home/africa/work/python/djnago-unisender/unisender/tests/mock_api.py
...
def unisender_test_api(UnisenderModel):
    ...
    class UnisenderMockAPI(object):
        ...
        def createCampaign(self, **kwargs):
            return {'result': {'campaign_id': 1, 'status': 'scheduled', 'count': 2}}

        def getCampaignStatus(self, **kwargs):
            return {'result': {'status': 'completed', 'creation_time': '2011-09-21 19:47:31', 
                          'start_time': '2011-09-21 20:00:00'}}

        def getCampaignAggregateStats(self, **kwargs):
            return {'result': {'total': 241, 'data': {'ok_read': 239, 
                                   'err_will_retry': 2}}}
...
def unisender_test_api_correct_values(UnisenderModel):
    ...
    class UnisenderMockAPI(object):
        ...
        def all_requirement_fields_present(self, requirement_fields, kwargs):
            for item in requirement_fields:
                if item not in kwargs:
                    raise NameError('В передаваемых значениях отсутствует обязательное поле < %s >' % item)
        ...
        def not_documented_fields_not_present(self, requirement_fields, fields, kwargs):
            ...
            for item in kwargs.keys():
                if item not in all_fields:
                    raise NameError('В передаваемых значениях присутствует поле < %s > котрое не указано в документации' % item)
        ...
        def createField(self, **kwargs):
            ...
            self.not_documented_fields_not_present(requirement_fields, fields, kwargs)
            return {}
        ...
        def deleteField(self, **kwargs):
            requirement_fields = [
             'id']
            self.all_requirement_fields_present(requirement_fields, kwargs)
            self.not_documented_fields_not_present(requirement_fields, [], kwargs)
            return {}
