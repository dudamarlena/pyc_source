...
# [GCC 8.4.0]
# Embedded file name: /home/africa/work/python/djnago-unisender/unisender/tests/models.py
# Compiled at: 2014-07-25 05:41:03
from dateutil.tz import tzutc
from datetime import datetime
from django.test import TestCase
from django.core.exceptions import ValidationError
from mock import patch
from unisender.models import Tag, Field, SubscribeList, Subscriber, SubscriberFields, EmailMessage, Campaign, validate_field_name_field, VisitedLink, OptinEmail
from unisender.error_codes import UNISENDER_COMMON_ERRORS
from .mock_api import unisender_test_api, unisender_test_api_errors, unisender_test_api_correct_values
...
class FieldModelTestCase(TestCase):
    ...
    def test__get_last_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], field.get_last_error())
    ...
    def test__create_field_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], self.field.get_last_error())
    ...
    def test__validate_field_name_field(self):
        self.assertRaises(ValidationError, validate_field_name_field, 'тест')
...
class SubscribeListTestCase(TestCase):
    ...
    def test__update_list_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], self.list.get_last_error())
...
class OptinEmailTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.optin_email = OptinEmail.objects.create(list_id=subscribe_list, sender_name='test', sender_email='test@example.com', subject='test')
    ...
    def test__update_optin_email_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], email.get_last_error())
...
class SubscriberTestCase(TestCase):
    ...
    def test__serialize_fields(self):
        ...
        subscriber_2 = Subscriber.objects.create(contact='9123456789', contact_type='phone')
        self.assertEquals({'email': 'mail@example.com'}, subscriber.serialize_fields())
        self.assertEquals({'phone': '9123456789'}, subscriber_2.serialize_fields())
        field_1 = Field.objects.create(name='test')
        SubscriberFields.objects.create(subscriber=subscriber, field=field_1, value='test_value')
        self.assertDictEqual({'test': 'test_value', 'email': 'mail@example.com'}, subscriber.serialize_fields())
        field_2 = Field.objects.create(name='test_2')
        SubscriberFields.objects.create(subscriber=subscriber_2, field=field_1, value='test_value')
        SubscriberFields.objects.create(subscriber=subscriber_2, field=field_2, value='test_value_2')
        self.assertDictEqual({'test': 'test_value', 'phone': '9123456789', 'test_2': 'test_value_2'}, subscriber_2.serialize_fields())

    @patch.object(Subscriber, 'get_api', unisender_test_api)
    @patch.object(SubscribeList, 'get_api', unisender_test_api)
    def test__serialize_list_id(self):
        subscriber = Subscriber.objects.create(contact='mail@example.com')
        subscriber_list_1 = SubscribeList.objects.create(title='test', unisender_id=1)
        subscriber.list_ids.add(subscriber_list_1)
        self.assertEquals(str(subscriber_list_1.pk), subscriber.serialize_list_id())
        subscriber_list_2 = SubscribeList.objects.create(title='test_2', unisender_id=2)
    ...
    def test__subscribe_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], subscriber.get_last_error())
...
class EmailMessageTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', list_id=subscriber_list)
    ...
    def test__create_email_message_error(self):
        ...
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], self.message.get_last_error())
...
class CampaignTestCase(TestCase):
    ...
    def setUp(self):
        subscriber = Subscriber.objects.create(contact='mail@example.com')
        subscriber_list = SubscribeList.objects.create(title='test')
        message = EmailMessage.objects.create(sender_name='test', sender_email='mail@example.com', subject='test', body='test', list_id=subscriber_list)
    ...
    def test__serrialize_contacts(self):
        self.assertEquals('mail@example.com', self.campaign.serrialize_contacts())
        subscriber = Subscriber.objects.create(contact='mail_2@example.com')
        self.campaign.contacts.add(subscriber)
        self.assertEquals('mail@example.com,mail_2@example.com', self.campaign.serrialize_contacts())
    ...
    def test__create_campaign_error(self):
        self.campaign.create_campaign()
        self.assertEquals(UNISENDER_COMMON_ERRORS['invalid_arg'], self.campaign.get_last_error())
