...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/footylib/footylib.py
...
class Footy(object):
    ...
    def __init__(self):
        self.logger = logging.getLogger('{base}.{suffix}'.format(base=LOGGER_BASENAME,
    ...
    def competitions(self):
        ...
        if not self._competitions:
            league_page = self._Footy__league_page.find('div', {'id': 'league-page'})
    ...
    def get_team(self, team_name):
        ...
        for competition in self.competitions:
            team = next((team for team in competition.teams if team.name.lower() == team_name.encode('utf-8').lower()), None)
            if team:
                break
...
class Competition(object):
    ...
    def __init__(self, footy_instance, url):
        self._logger = logging.getLogger('{base}.{suffix}'.format(base=LOGGER_BASENAME,
          suffix=(self.__class__.__name__)))
...
class Team(object):
    ...
    def matches(self):
        ...
        :return: list of Match objects
        """
        return [match for match in self.competition.matches if self.name in match.title]
...
class Match(object):
    ...
    def _populate(self, match_details):
        ...
        """
        try:
            self.datetime = self._Match__string_to_datetime(match_details.datetime)
    ...
    def _get_team(self, home_team=True):
        ...
        team = next((team for team in self.competitions.teams if team.name == match), None)
        return team
    ...
    def visiting_team_goals(self):
        ...
        if not self._visiting_team_goals:
            self._visiting_team_goals = self._get_match_goals(home_team_goals=False)
    ...
    def __string_to_datetime(datetime_string):
        ...
        try:
            datetime_object = parse(date_string=datetime_string, date_formats=[
             '%d.%m.%Y %H:%M'],
