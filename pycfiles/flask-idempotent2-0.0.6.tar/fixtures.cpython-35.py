...
# [GCC 8.4.0]
# Embedded file name: /Users/hit9/uav/flask-idempotent2/tests/fixtures.py
# Compiled at: 2017-05-25 08:13:15
# Size of source mod 2**32: 5049 bytes
import json, sys, random, functools
...
engine = create_engine('postgresql+psycopg2://localhost/flask_idempotent2_unittesting')
DBSession = scoped_session(sessionmaker(bind=engine, autocommit=False, expire_on_commit=False))
...
def app(db_clear, redis_client, redis_clear):
    Session = scoped_session(sessionmaker(bind=engine, autocommit=False, expire_on_commit=False), scopefunc=_app_ctx_stack.__ident_func__)
    app = Flask(__name__)
    ...
    def create_or_return_user():
        data = request.get_json()
        user = Session().query(User).filter(User.email == data['email']).scalar()
        if not user:
            user = User(email=data['email'], password=data['password'])
            Session().add(user)
        return jsonify(email=user.email, password=user.password)
...
@pytest.fixture(scope='function')
def app_client(app_native_client):
