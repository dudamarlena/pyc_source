...
# Embedded file name: build/bdist.linux-x86_64/egg/okera/_pure_sasl_mechanisms.py
# Compiled at: 2020-02-27 14:06:57
# Size of source mod 2**32: 19022 bytes
import base64, hashlib, hmac, random, struct, sys
from okera._pure_sasl import SASLError, SASLProtocolException, QOP
try:
    import kerberos
    have_kerberos = True
except ImportError:
...
class DigestMD5Mechanism(Mechanism):
    ...
    def response(self):
        ...
        self._digest_uri = to_bytes(self.sasl.service) + '/' + to_bytes(self.sasl.host)
        ...
        return ','.join([to_bytes(k) + '=' + to_bytes(v) for k, v in resp.items()])
    ...
    def gen_hash(self, a2):
        ...
        rv += ':' + self.nonce
        rv += ':' + to_bytes('%08x' % self.nc)
        rv += ':' + self.cnonce
        rv += ':' + self.qop
        rv += ':' + to_bytes(hashlib.md5(a2).hexdigest().lower())
        response.update(rv)
