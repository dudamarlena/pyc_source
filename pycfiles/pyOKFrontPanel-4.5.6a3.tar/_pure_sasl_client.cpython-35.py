...
# Embedded file name: build/bdist.linux-x86_64/egg/okera/_pure_sasl_client.py
# Compiled at: 2020-02-27 14:06:57
# Size of source mod 2**32: 9960 bytes
from functools import wraps
from warnings import warn
import okera._pure_sasl_mechanisms as mech_mod
from okera._pure_sasl import SASLError, QOP
...
class SASLClient(object):
        ...
        if self.mechanism is not None:
            try:
                ...
                if mechanism == gssapi and not mech_mod.have_kerberos:
                    raise SASLError('kerberos module not installed, {0} unavailable'.format(gssapi))
                else:
    ...
    def choose_mechanism(self, mechanism_choices, allow_anonymous=True, allow_plaintext=True, allow_active=True, allow_dictionary=True):
        ...
        if gssapi in mechanism_choices and not mech_mod.have_kerberos:
            warn('kerberos module not installed, {0} will be ignored'.format(gssapi), SASLWarning)
