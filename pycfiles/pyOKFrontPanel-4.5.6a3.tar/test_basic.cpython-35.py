...
# Embedded file name: build/bdist.linux-x86_64/egg/okera/tests/test_basic.py
# Compiled at: 2020-02-27 14:06:57
# Size of source mod 2**32: 20339 bytes
import unittest
...
class BasicTest(unittest.TestCase):
    ...
    def test_requesting_user(self):
        ...
        planner = common.get_planner()
        ddl = planner.execute_ddl('describe okera_sample.users', requesting_user='testuser')
        self.assertEqual([
         [
          'ccn',
          'string',
         ...
         [
          'gender', 'string', '', ''],
         [
          'uid', 'string', 'Unique user id', '']], sorted(ddl, key=lambda item: item[0]))
        planner = common.get_planner()
        with self.assertRaises(_thrift_api.TRecordServiceException):
            planner.execute_ddl('describe okera_system.attributes', requesting_user='testuser')
    ...
    def test_all_types_pandas(self):
        ...
        self.assertEqual('hello', pd['string_col'][0])
        self.assertEqual('world', pd['string_col'][1])
        self.assertEqual('vchar1', pd['varchar_col'][0])
        self.assertEqual('vchar2', pd['varchar_col'][1])
        self.assertEqual('char1', pd['char_col'][0])
        self.assertEqual('char2', pd['char_col'][1])
    ...
    def test_all_types_empty(self):
        ...
        self.assertEqual([
         ...
         'timestamp_col', 'decimal_col'], list(pd.columns), msg=pd)
    ...
    def test_filter_with_empty_rows_pandas(self):
        planner = common.get_planner()
        pd = planner.scan_as_pandas("SELECT * FROM okera_sample.users where gender = 'lol'")
        planner.close()
        self.assertEqual(0, len(pd), msg=pd)

    def test_column_order_scan_as_pandas(self):
        planner = common.get_planner()
        self.assertEqual(list(planner.scan_as_pandas('okera_sample.users').columns), [
         'uid', 'dob', 'gender', 'ccn'])
        self.assertEqual(list(planner.scan_as_pandas('select dob, uid from okera_sample.users').columns), [
         'dob', 'uid'])
        self.assertEqual(list(planner.scan_as_pandas('rs.alltypes_s3').columns), [
         'bool_col', 'tinyint_col', 'smallint_col', 'int_col', 'bigint_col',
         'float_col', 'double_col', 'string_col', 'varchar_col', 'char_col',
         'timestamp_col', 'decimal_col'])
        self.assertEqual(list(planner.scan_as_pandas('select decimal_col, bigint_col from rs.alltypes_s3').columns), [
    ...
    def test_binary_data(self):
        with common.get_planner() as (planner):
            ...
            self.assertEqual('This is a sample test file.', df['record'][0], msg=df)
    ...
    def test_get_objects_at_path(self):
        ...
        with ctx.connect() as (planner):
            ...
            result = planner.get_catalog_objects_at('s3://cerebrodata-test/prefixtests/root')
            self.assertEqual(2, len(result), msg=result)
            result = planner.get_catalog_objects_at('s3://cerebrodata-test/prefixtests/root/alltypes/')
            self.assertEqual(1, len(result), msg=result)

    def verify_decimal_value_json(self, planner, dec_in, prec, scale, expected=None):
        if not expected:
            expected = Decimal(dec_in)
        sql = 'select CAST({} as DECIMAL({},{})) as mycol'.format(dec_in, prec, scale)
        res = planner.scan_as_json(sql)[0]['mycol']
        self.assertTrue(res == expected)

    def verify_decimal_value_pandas(self, planner, dec_in, prec, scale, expected=None):
        if not expected:
            expected = Decimal(dec_in)
        sql = 'select CAST({} as DECIMAL({},{})) as mycol'.format(dec_in, prec, scale)
    ...
    def test_decimal_serialization_json(self):
        ...
        with ctx.connect() as (p):
            ...
            self.verify_decimal_value_json(p, '17014118346046923173168730371588410579', 38, 0, Decimal('1.701411834604692317316873037E+37'))
            self.verify_decimal_value_json(p, '-17014118346046923173168730371588410579', 38, 0, Decimal('-1.701411834604692317316873037E+37'))
    ...
    def test_decimal_serialization_pandas(self):
        ...
        with ctx.connect() as (p):
            ...
            self.verify_decimal_value_pandas(p, '-18446744073709551616.0000', 38, 4)
            self.verify_decimal_value_pandas(p, '17014118346046923173168730371588410579', 38, 0, Decimal('1.701411834604692317316873037E+37'))
            self.verify_decimal_value_pandas(p, '-17014118346046923173168730371588410579', 38, 0, Decimal('-1.701411834604692317316873037E+37'))
