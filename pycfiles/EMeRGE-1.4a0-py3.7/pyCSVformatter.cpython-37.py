...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\CSVformatter\pyCSVformatter.py
# Compiled at: 2020-03-23 23:16:18
# Size of source mod 2**32: 20946 bytes
import os, pathlib, toml, pandas as pd, shutil, numpy as np
...
def export_linecsvs(settings_dict, line_or_cable, ht_or_lt, unique_geometry):
    ...
    node_df = {'shapeid':list(node_dataframe.shapeid), 
    ...
    for keys, items in settings_dict['line_column_mapper'].items():
        if keys in attribute_df:
            if items[0] == 'force':
                attribute_df[keys] = [
                 items[1]] * len(attribute_dataframe)
            elif list(set(items) & set(columns)) != []:
    ...
    if settings_dict['force_lt_be_three_phase'] == 'yes':
        if ht_or_lt == 'lt':
            attribute_df['num_of_cond'] = [
             4] * len(attribute_dataframe)
    ...
    for id, val in enumerate(attribute_df['cname']):
        ...
        if cond in cond_with_geom:
            ...
            if geomid not in list(unique_geometry['ID']):
                ...
                if 'nname' not in attribute_df:
                    if attribute_df['num_of_cond'][id] > num_of_phases:
...
def export_transformercsvs(settings_dict, type):
    ...
    if settings_dict['MVA_to_KVA_conversion_for_PT'] == 'yes':
        if type != 'DTs':
...
def export_consumercsvs(list_of_csvs, settings_dict, type):
    ...
    for el in attribute_df['phase']:
        if el in settings_dict['single_phase']:
            if type == 'lt':
                attribute_df['kv'].append(settings_dict['Consumer_kv']['lt_consumer_phase'])
            else:
                if el == settings_dict['three_phase']:
                    if type == 'lt':
    ...
    for el in tariff_data:
        for keys, items in settings_dict['consumer_class_by_tariff'].items():
            if el in items:
                attribute_df['cust_type'].append(keys.lower())
    ...
    if settings_dict['consumer_column_mapper']['estimate_consumer_peakkw'] == 'yes':
        ...
        for el in tdata:
            for keys, items in settings_dict['consumer_class_by_tariff'].items():
                if el in items:
                    consumerdata.append(keys.lower())
...
class CSVFormatter:
    ...
    def __init__(self, setting_toml_file):
        ...
        if settings_dict['ht_line']['node_file_name'] in list_of_csvs:
            if settings_dict['ht_line']['node_file_name'] in list_of_csvs:
                export_linecsvs(settings_dict, 'line', 'ht', unique_geometry)
        if settings_dict['ht_cable']['node_file_name'] in list_of_csvs:
            if settings_dict['ht_cable']['node_file_name'] in list_of_csvs:
                export_linecsvs(settings_dict, 'cable', 'ht', unique_geometry)
        if settings_dict['lt_line']['node_file_name'] in list_of_csvs:
            if settings_dict['lt_line']['node_file_name'] in list_of_csvs:
                export_linecsvs(settings_dict, 'line', 'lt', unique_geometry)
        if settings_dict['lt_cable']['node_file_name'] in list_of_csvs:
            if settings_dict['lt_cable']['node_file_name'] in list_of_csvs:
