...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ResultDashboard\Dashboard\InitialAssessmentTab.py
# Compiled at: 2020-03-15 13:05:00
...
class InitialAssessmentTab:
    ...
    def FirstLayer(self):
        ...
        self.Detail_content = ' Utility often want to know the shape of load profile. Increasing PV penetration affects these load shapes. This tool allows utility to see load profile for different consumer class and also see how the load shape changes when PV penetration increases. Note the PV penetration in this tool is defined based on percentage of peak load of the feeder and is not same as PV scenario defined in other tabs. Also it does take into account the location where they are installed. '
        Col = html.Div([html.Div([Paragraph(self.Heading_content, self.Detail_content).layout()]),
         html.Div([
          html.Div([html.H6('Select PV penetration (in %)'),
           dcc.Slider(id='pv', min=0, max=100, value=0, marks={i:'{}%'.format(i) for i in range(0, 100, 10)}, className='h2'),
           dcc.Graph(id='load-duration-curve', className='mt-5')],
            className='DropDown'),
