...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ResultDashboard\Dashboard\DSSRiskAnalyzer\ResultContainer\pyResult.py
# Compiled at: 2020-03-15 13:08:24
# Size of source mod 2**32: 11836 bytes
...
class ResultProcessor:
    ...
    def __init__(self, *argv):
        ...
        SystemLevelMetrics = [
         'SARDI_voltage', 'SARDI_line', 'SARDI_transformer', 'SARDI_aggregated', 'SE_line', 'SE_transformer', 'SE', 'SALOF_transformer', 'SOG']
        self.system_level_metrics = {Metric:[] for Metric in SystemLevelMetrics}
        ColNamesForTimeSeriesSystemLevelMetrics = [
         'TimeStamp'] + SystemLevelMetrics
        self.system_level_metrics_time_series = {Column:[] for Column in ColNamesForTimeSeriesSystemLevelMetrics}
        AssetLevelMetrics = [
         'NVRI', 'LVRI', 'CRI', 'LE', 'TE', 'TLOF', 'TOG']
        self.Asset_level_metric = {Metric:{} for Metric in AssetLevelMetrics}
        self.Asset_level_metric_time_series = {}
        AsssetLevelParameters = [
         'voltagemag', 'lineloading', 'transformerloading']
        ...
        self.AssetLevelParameters['lineloading'] = self.ProcessAssetLevelParameters([LineMetricDict[TimeStamp]['PULoading'] for TimeStamp in LineMetricDict.keys()], TimeStampForRecordingData)
        self.AssetLevelParameters['transformerloading'] = self.ProcessAssetLevelParameters([TransformerMetricDict[TimeStamp]['PULoading'] for TimeStamp in TransformerMetricDict.keys()], TimeStampForRecordingData)
    ...
    def ProcessTransformerLoadingListofDict(self, ListOfDict, ListofTimeStamps, TemperatureData, TransformerLifePrameters):
        ListOfKeys = [
         'TimeStamp'] + [keys for keys in ListOfDict[0].keys()]
    ...
    def ProcessForEfficiency(self, ListOfLists, ListofTimeStamps, Indexes):
        ...
        return (
         self.ComputeEfficiency(sum(LossSequence), sum(PowerSequence)), [self.ComputeEfficiency(LossSequencearr[i], PowerSequencearr[i]) for i in range(len(LossSequencearr))])
