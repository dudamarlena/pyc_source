...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ResultDashboard\Dashboard\PVConnectionTab.py
# Compiled at: 2020-03-15 13:05:20
# Size of source mod 2**32: 16645 bytes
...
class PVConnectionTab:
    ...
    def FirstLayer(self):
        ...
        self.Detail_content = ' Before accepting PV connection request, utility might need to perform initial assessment of whether such PV connection might affect the grid in an adverse manner or not. This tool is exactly for that. When consumer submits PV connection request utiliy can input the parameter here and will show impact on different distribution metrics which helps utility to accept or reject the request.'
    ...
    def SecondLayer(self):
        ...
        self.Detail_content = ' User should input valid data for all the fields below. Please zoom in on the network shown on the right and  hover over the node to see the name of the node where PV connection is requested.'
        left_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         ...
         html.Div([daq.BooleanSwitch(id='Request', on=False, color='#9B51E0', label='Switch to process request', style={'font-family':'Times New Roman',  'margin-top':'20px'})])],
          className='col-4')
        ...
        middle_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         ...
         html.Div([
          dcc.Input(id='time', placeholder='Enter a value...', type='text', value='', style={'margin-left': '30px'})]),
         ...
         html.Div([
          daq.BooleanSwitch(id='Accept', on=False, color='#9B51E0', label='Switch to accept request',
        ...
        right_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         html.Div([dcc.Graph(figure=(GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), (self.x_coordinate), (self.y_coordinate), (self.DataObject.initial_x), (self.DataObject.initial_y), height=350, hovertext=texts, hoverinfo='text').layout()))])],
          className='col-4')
        return html.Div([left_col, middle_col, right_col], className='row')

    def ThirdLayer(self):
        self.Heading_content = 'Voltage profile'
        self.Detail_content = ' The plot below shows voltage heatmap on top of network             for last time stamp of the simulation.'
        left_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         html.Div([dcc.Graph(id='voltagediff')])],
          className='col')
        self.Heading_content = 'Line loading heatmap'
        self.Detail_content = ' The plot below shows a line loading heatmap on top of network for last time stamp of the             simulation.'
        right_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         html.Div([dcc.Graph(id='linediff')])],
          className='col')
        self.Heading_content = 'Transformer loading heatmap'
        self.Detail_content = ' The plot below shows a transformer loading heatmap on top of network for last time stamp of the                    simulation.'
    ...
    def FourthLayer(self):
        ...
        self.Detail_content = ' The plot below shows voltage heatmap on top of network             for last time stamp of the simulation.'
        left_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         html.Div([dcc.Graph(id='voltageBase')])],
          className='col')
        self.Heading_content = 'Line loading heatmap (Base)'
        self.Detail_content = ' The plot below shows a line loading heatmap on top of network for last time stamp of the             simulation.'
        right_col = html.Div([Paragraph(self.Heading_content, self.Detail_content).layout(),
         html.Div([dcc.Graph(id='lineBase')])],
          className='col')
    ...
    def Callbacks(self):
        ...
        def UpdateGauze(Capacity, LoadName, Phase, Accept, Time, Day, Request):
            Basemap = GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), (self.x_coordinate),
              (self.y_coordinate), (self.DataObject.initial_x),
              (self.DataObject.initial_y),
              height=350, marker={'size': 0}).layout()
            if Request:
                Mode = 'Daily'
                PVDict = {'LoadName':LoadName,  'Irradiance':0.98,  'Capacity':float(Capacity),  'InverterOverSizeFactor':0.9,  'KVAR':0, 
                 'CutIn':0.05,  'CutOut':0.05,  'Profile':'solarmult',  'Phases':Phase}
                ...
                return (
                 risk, overgeneration, loss,
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), x_coordinatevoltage, y_coordinatevoltage,
                   (self.DataObject.initial_x), (self.DataObject.initial_y), marker=(self.MarkerDict(markerarrayvoltage, 'Delta (p.u.)')),
                   height=350).layout(),
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), x_coordinateline, y_coordinateline, (self.DataObject.initial_x), (self.DataObject.initial_y),
                   marker=(self.MarkerDict(markerarrayline, 'Delta (p.u.)')), height=350).layout(),
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), x_coordinatetransformer, y_coordinatetransformer,
                   (self.DataObject.initial_x), (self.DataObject.initial_y), marker=(self.MarkerDict(markerarraytransformer, 'Delta (p.u.)')),
                   height=350).layout(),
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines),
                   x_coordinatevoltage, y_coordinatevoltage, (self.DataObject.initial_x), (self.DataObject.initial_y), marker=(self.MarkerDict([values for keys, values in voltage.items()], 'Base (p.u.)')),
                   height=350).layout(),
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), x_coordinateline, y_coordinateline, (self.DataObject.initial_x),
                   (self.DataObject.initial_y), marker=(self.MarkerDict([values for keys, values in line.items()], 'Base (p.u.)')),
                   height=350).layout(),
                 GeoScatterMap((self.DataObject.x_lines), (self.DataObject.y_lines), x_coordinatetransformer,
                   y_coordinatetransformer, (self.DataObject.initial_x), (self.DataObject.initial_y), marker=(self.MarkerDict([values for keys, values in transformer.items()], 'Base (p.u.)')), height=350).layout())
