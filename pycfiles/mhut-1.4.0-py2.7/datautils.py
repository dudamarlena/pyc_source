...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mhut/datautils.py
# Compiled at: 2016-09-22 02:38:59
"""
...
"""
import re, pandas as pd, numpy as np, logging, datetime as dt
_ANYOP_ = re.compile('[><=&|]')
...
def roundoff_df(df, places=0, columns=None, indices=None):
    ...
    """
    tmp = df.copy()
    if columns == None and indices == None:
        for j in tmp.columns:
            tmp[j] = tmp[j].round(places)

    elif columns != None and indices == None:
        for j in columns:
            tmp[j] = tmp[j].round(places)
...
def parse2df(fname):
    ...
    fin.close()
    sep_pragma = '#<pr:sep>'
    txtlist = [ t.replace(sep_pragma, ' ' * len(sep_pragma)) if t.lstrip().startswith(sep_pragma) else t for t in txtlist
              ]
    df = txt2df(('').join(txtlist), header_sep='|', header=True, skip_comment=True, index='default')
    return df


def txt2df(strtxt, header_sep='|', header=True, skip_comment=True, index='default', strip_pct=True):
    ...
    for line in strtxt.split('\n'):
        ...
        if len(positions) == 0:
            ...
            if strip_pct:
                tmp[1:] = [ t.replace('%', '') for t in tmp[1:] ]
            lol.append(tmp)
...
def pprint_dict(adict, orderlist=[]):
    ...
    for k in printsets:
        ...
        if type(v) == pd.Series or type(v) == pd.DataFrame:
            ...
            print '-' * 80
            continue
        if type(v) == dt.datetime:
            v = dt.datetime.ctime(v)
...
def isnumeric(a):
    ...
    if type(a) == str:
        if a.replace('.', '').isdigit() and a.count('.') < 2:
            return True
    elif np.dtype(a) == int or np.dtype(a) == float:
        return True
    return False
...
def broadcast(alist, n, axis=1):
    ...
    if type(alist) == list:
        if axis == 0:
            ...
            return (np.array([alist]).T * np.ones(n)).tolist()

    elif type(aa) == np.ndarray:
        if axis == 0:
            return np.array(np.mat(np.ones(n)).T * aa)
        else:
            return np.array([aa]).T * np.ones(n)

    elif type(ma) == np.matrix:
        if axis == 0:
            return np.mat(np.ones(n)).T * alist
        else:
            return alist.T * np.ones(n)

    elif type(sa) == pd.Series:
        if axis == 0:
            return pd.DataFrame(np.mat(np.ones(n)).T * np.mat(sa))
        else:
            return pd.DataFrame(np.mat(aa).T * np.ones(n))
