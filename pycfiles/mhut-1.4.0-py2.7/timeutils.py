...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mhut/timeutils.py
# Compiled at: 2016-09-22 02:45:05
"""
...
_DAY_ = {6: 'sunday', 0: 'monday', 1: 'tuesday', 2: 'wednesday', 3: 'thursday', 4: 'friday', 
   5: 'saturday'}
_MTH_ = {'jan': '01', 'feb': '02', 'mar': '03', 'apr': '04', 'may': '05', 'jun': '06', 'jul': '07', 
...
def curr_time(loc=None):
    ...
    elif loc in all_timezones:
        curr = dt.datetime.now(loc)
    elif loc.lower() == 'utc':
        curr = dt.datetime.now(timezone('UTC'))
    elif loc.lower() == 'eastern':
        curr = dt.datetime.now(timezone('US/Eastern'))
    elif loc.lower() == 'central':
        curr = dt.datetime.now(timezone('US/Central'))
    elif loc.lower() == 'pacific':
        curr = dt.datetime.now(timezone('US/Pacific'))
    else:
...
def first_of_month():
    """Gets the first of a month. This is NOT timezone aware"""
    d = date.today()
...
def parse_date(adate):
    ...
    """
    d = date.today()
    dflt_date = date(d.year, d.month, 1)
    ...
    def get_md(monthday):
        if monthday.isalpha():
            ...
            day = re.findall('\\d+', monthday)[0]
            mth_str = re.split('\\d+', monthday)[0]
    ...
    if x.isdigit():
        ...
        if all_digits:
            if len(y) == 3:
                if slash_based:
                    fyear, fmonth, fday = int(pad_year(y[2])), int(y[0]), int(y[1])
                else:
                    fyear, fmonth, fday = int(y[0]), int(y[1]), int(y[2])
            elif len(y) == 2:
                if slash_based:
                    fyear, fmonth, fday = int(pad_year(y[1])), int(y[0]), 1
                else:
                    fyear, fmonth, fday = int(y[0]), int(y[1]), 1
            else:
                if len(y) < 2:
        ...
        try:
            date_obj = date(fyear, fmonth, fday)
...
def iso_date(adate):
    ...
    if len(alist) != 3:
        return False
    y, m, d = alist
    return y.isdigit() and len(y) == 4 and m.isdigit() and len(m) == 2 and d.isdigit() and len(d) == 2
