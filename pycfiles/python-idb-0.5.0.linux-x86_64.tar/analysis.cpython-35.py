...
# [GCC 8.4.0]
# Embedded file name: /home/user/env/lib/python3.5/site-packages/idb/analysis.py
# Compiled at: 2018-06-15 13:52:59
# Size of source mod 2**32: 36899 bytes
import types, struct, logging, binascii, datetime, itertools
...
def as_unix_timestamp(buf, wordsize=None):
    ...
    """
    q = struct.unpack_from('<I', buf, 0)[0]
...
def unpack_dd(buf, offset=0):
    ...
    if offset != 0:
        buf = buf[offset:]
    header = six.indexbytes(buf, 0)
    if header & 128 == 0:
        return (header, 1)
    else:
        if header & 192 != 192:
            return (((header & 127) << 8) + six.indexbytes(buf, 1), 2)
        if header & 224 == 224:
            hi = (six.indexbytes(buf, 1) << 8) + six.indexbytes(buf, 2)
            low = (six.indexbytes(buf, 3) << 8) + six.indexbytes(buf, 4)
            size = 5
        else:
            hi = ((header & 63) << 8) + six.indexbytes(buf, 1)
            low = (six.indexbytes(buf, 2) << 8) + six.indexbytes(buf, 3)
            size = 4
        return (
         (hi << 16) + low, size)
...
def unpack_dw(buf, offset=0):
    ...
    if header & 128 == 0:
        ...
        if header & 192 != 192:
            return ((header << 8) + six.indexbytes(buf, 1) & 32767, 2)
        return ((six.indexbytes(buf, 1) << 8) + six.indexbytes(buf, 2), 3)
...
class _Analysis(object):
    ...
    def __init__(self, db, nodeid, fields):
        ...
        self._fields_by_name = {f.name:f for f in self.fields if not f.minver or f.minver and idb_version >= f.minver}
    ...
    def _is_node(self, index):
        ...
        if self.idb.wordsize == 4:
            return index & 4278190080 == 4278190080
        if self.idb.wordsize == 8:
            return index & 18374686479671623680 == 18374686479671623680
        raise RuntimeError('unexpected wordsize')
    ...
    def __getattr__(self, key):
        ...
        if key not in self._fields_by_name:
            ...
            if field.index == ADDRESSES:
                ...
                if field.index == NUMBERS:
                    nfilter = self._is_number
                else:
                    if field.index == NODES:
...
class IdaInfo(vstruct.VStruct):
    ...
    def __init__(self, wordsize):
        ...
        self.tag = v_str(size=3)
        self.zero = v_bytes(size=0)
        self.version = v_uint16()
        self.procname_size = v_bytes(size=0)
        self.procname = v_str(size=16)
...
Root = Analysis('Root Node', [
 ...
 Field('idainfo', 'S', 4307348, as_cast(IdaInfo)),
...
class StructMember:
    ...
    def get_type(self):
        v = self.netnode.supval(tag='S', index=12288)
        s = TypeString()
        s.vsParse(v)
        return s.s

    def get_enum_id(self):
        return self.altval(tag='A', index=11)

    def get_struct_id(self):
        return self.altval(tag='A', index=3)

    def get_member_comment(self):
        return self.supstr(tag='S', index=0)

    def get_repeatable_member_comment(self):
        return self.supstr(tag='S', index=1)
...
class Function:
    ...
    def get_signature(self):
        typebuf = self.netnode.supval(tag='S', index=12288)
        namebuf = self.netnode.supval(tag='S', index=12289)
        if six.indexbytes(typebuf, 0) != 12:
            raise RuntimeError('unexpected signature header')
        if six.indexbytes(typebuf, 1) == ord('S'):
            conv = '__stdcall'
        else:
        ...
        params = []
        typeoffset = 2 + len(rtype) + 1
        nameoffset = 0
        while typeoffset < len(typebuf):
            if six.indexbytes(typebuf, typeoffset) == 0:
                break
            typename = TypeString()
    ...
    def get_chunks(self):
        v = self.netnode.supval(tag='S', index=28672)
        last_ea = 0
        last_length = 0
        if self.idb.wordsize == 4:
            unpacker = unpack_dds
    ...
    def get_stack_change_points(self):
        try:
            v = self.netnode.supval(tag='S', index=4096)
        except KeyError:
            return
