...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.6/site-packages/wintx/drivers/MySQLFabric/Driver.py
# Compiled at: 2016-03-23 14:50:18
"""MySQL Fabric driver for Wintx"""
import hashlib, mysql.connector, Queue
...
class Driver(object):
    ...
    QUERY_SELECT_DATA = 'SELECT Data.timestamp, X(Location.lat_lon), Y(Location.lat_lon), Level.level, Level.level_type, Variable.datatype, Variable.variable_name, Data.data FROM Data INNER JOIN Location ON Data.loc_id=Location.loc_id INNER JOIN Level ON Data.level_id=Level.level_id INNER JOIN Variable ON Data.var_id=Variable.var_id'

    def __init__(self, host='127.0.0.1', port=32274, user='user', password='abc123', database_name='wintx', fabric_user='user', fabric_password='abc123', global_group='global', shard_groups=[], timeout=60, poolsize=5, bulkquantity=1000, attempts=3):
    ...
    """
        ...
        self.db_config = {'fabric': {'username': fabric_user, 
    ...
    def __setGlobalConnectionProperty__(self, connection=None, readonly=False):
        ...
        connection.set_property(mode=mode, scope=fabric.SCOPE_GLOBAL, group=self.global_group, tables=None, key=None)
    ...
    def __setLocalConnectionProperty__(self, key, connection=None, readonly=False):
        ...
        if readonly:
            mode = fabric.MODE_READONLY
        connection.set_property(mode=mode, scope=fabric.SCOPE_LOCAL, group=None, tables=[
         '%s.Data' % self.db_config['database']], key=key)
        self.connectDbServer(connection)
        return
    ...
    def __setLocalConnectionGroupProperty__(self, group, connection=None, readonly=False):
        ...
        if readonly:
            mode = fabric.MODE_READONLY
        connection.set_property(mode=mode, scope=fabric.SCOPE_LOCAL, group=group, tables=None, key=None)
        self.connectDbServer(connection)
        return
    ...
    def connect(self):
        ...
        for i in range(1, attempts + 1):
            try:
                ...
                if i < attempts:
                    ...
                    raise WintxDatabaseError('Was unable to form a stable connection to %s@%s:%s.' % (
        ...
        if not self.isConnectedToServer():
            raise WintxDatabaseError('Was unable to connect to %s@%s:%s for an unknown reason.' % (
        ...
        for i in range(1, attempts + 1):
            try:
                ...
                if i < attempts:
                    ...
                    raise WintxDatabaseError('Failed to form underlying connection to database. MySQL Fabric reset the connection too many times.')
        ...
        for i in range(1, attempts + 1):
            try:
                ...
                if i < attempts:
                    ...
                    raise WintxDatabaseError('Was unable to form a stable connection to %s@%s:%s while executing command "%s".' % (
                     self.db_config['user'],
    ...
    def __startTransaction__(self, snapshot=False, readonly=False):
       ...
       global cursor at a time."""
        ...
        if self.cursor is not None or self.connection.in_transaction:
            raise WintxDatabaseError('A transaction is currently active. A new transaction may not begin until the current transaction has completed.')
        try:
    ...
    def __multiThreadedConnections__(self, numTasks, thread_function, args, kwargs):
        ...
        if numConnections > 1:
            for i in range(numConnections):
                threads.append(Thread(name='Wintx-Thread-%05d' % i, target=self.__connectionThread__, args=(
    ...
    def __prepareQuery__(self, query_dict, restricted=None, polygonWhere=None):
        ...
        query_table_wheres = {'location': [], 'variable': [], 'level': [], 'data': []}
        ...
        for key in query_dict:
            ...
            if key == 'time':
                ...
                raise WintxDatabaseError('Invalid column name found in query. Column "%s" does not exist.' % key)
            where_columns = []
        ...
        query = 'SELECT Data.timestamp, X(Location.lat_lon), Y(Location.lat_lon), Level.level, Level.level_type, Variable.datatype, Variable.variable_name, Data.data FROM (%s) AS Data INNER JOIN Location ON Data.loc_id=Location.loc_id INNER JOIN Level ON Data.level_id=Level.level_id INNER JOIN Variable ON Data.var_id=Variable.var_id' % data_subquery
        return query
    ...
    def __finalizeRecords__(self, results, sort=None):
        ...
        for record in results:
            records.append({'time': self.__getTime__(record[0]), 'latitude': record[1], 'longitude': record[2], 
               'level': record[3], 'leveltype': record[4], 'datatype': record[5], 
               'varname': record[6], 'value': record[7]})
    ...
    """
        ...
        data_dict = {'timestamp': timestamp, 'data_id': None, 'value': record['value'], 
           'loc_id': loc_id, 'var_id': var_id, 
           'level_id': level_id}
        ...
        try:
            ...
            if err.errno == 1452:
                if 'Location' in str(err):
                    ...
                    var_id = (
                     record['datatype'], record['varname'])
    ...
    def insertRecords(self, records, clear_cache=False):
        ...
        results = {'insertedRecords': 0, 
           'duplicateRecords': [], 'errors': []}
        insert_loc = set()
        insert_var = set()
        insert_lev = set()
        ...
        metadataList = {'global_loc': [], 'global_lev': [], 'global_var': []}
        if len(insert_loc) > 0:
    ...
    def getLocationCorners(self):
        ...
        self.__setGlobalConnectionProperty__()
        self.__startTransaction__(readonly=True)
        self.cursor.execute('SELECT MAX(X(`lat_lon`)) AS `MAX_LAT`, MIN(X(`lat_lon`)) AS `MIN_LAT`, MAX(Y(`lat_lon`)) AS `MAX_LON`, MIN(Y(`lat_lon`)) AS `MIN_LON` FROM `Location`')
        results = self.cursor.fetchall()
        self.__endTransaction__()
        lat_max = results[0][0]
        lat_min = results[0][1]
    ...
    """
        self.__connectedOrDie__()
        stat_dict = {'totalSize': 0, 
           'numRecords': 0, 
           'avgRecordSize': 0, 
           'numIndexes': 0, 
           'indexSize': 0, 
           'indexes': []}
        stats = {'database': stat_dict.copy(), 
           'tables': {}}
    ...
    """
        self.__connectedOrDie__()
        query_templ = 'SELECT Data.timestamp, Variable.datatype, Variable.variable_name FROM (SELECT DISTINCT Data.timestamp, Data.var_id FROM Data %s) AS Data INNER JOIN Variable ON Data.var_id=Variable.var_id ORDER BY Data.timestamp, Variable.datatype, Variable.variable_name'
