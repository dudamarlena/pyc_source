...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/libs/pyblish/validators/scene/noshaders.py
...
class ValidateNoShadersInScene(pyblish.api.ContextPlugin):
    ...
    def process(self, context):
        ...
        for shading_node in shading_nodes:
            if shading_node not in ('initialParticleSE', 'initialShadingGroup') and shading_node not in extra_shaders:
                extra_shaders.append(shading_node)

        if extra_shaders:
            context.data['extra_shaders'] = extra_shaders
        elif not not extra_shaders:
            raise AssertionError('Current scene has non default shaders: "{}"'.format(extra_shaders))