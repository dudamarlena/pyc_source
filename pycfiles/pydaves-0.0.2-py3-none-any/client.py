...
# Embedded file name: build/bdist.linux-i686/egg/WebDAV/client.py
# Compiled at: 2006-08-16 17:27:47
__doc__ = 'HTTP 1.1 / WebDAV client library.'
__version__ = '$Revision: 1.4 $'[11:-2]
import sys, os, string, time, types, re, socket, httplib0, mimetools
...
class HTTP(httplib0.HTTP):
    ...
    def getreply(self):
        ...
        return (
         ver, code, msg, headers)


class HTTPS(httplib0.HTTPS):
    __module__ = __name__
    read_bug = sys.version[:5] in ('1.5 (', '1.5.1')
...
class Resource:
    ...
    def __init__(self, url, username=None, password=None):
        ...
        mo = urlreg.match(url)
        if mo:
            (proto, host, port, uri) = mo.group(1, 2, 3, 4)
            self.proto = proto
            self.host = host
            if proto == 'http':
                self.port = port and string.atoi(port[1:]) or 80
            else:
                self.port = port and string.atoi(port[1:]) or 443
            self.uri = uri or '/'
        else:
    ...
    def __getattr__(self, name):
        ...
        return self.__class__(url, username=self.username, password=self.password)
    ...
    def __set_authtoken(self, headers, atype='Basic'):
        ...
        if headers.has_key('Authorization'):
            return headers
        if atype == 'Basic':
            headers['Authorization'] = 'Basic %s' % string.replace(encodestring('%s:%s' % (self.username, self.password)), '\n', '')
            return headers
    ...
    def put(self, file='', content_type='', content_enc='', isbin=re.compile('[\\000-\\006\\177-\\277]').search, **kw):
        headers = self.__get_headers(kw)
        filetype = type(file)
        if filetype is type('') and isbin(file) is None and os.path.exists(file):
            ob = open(file, 'rb')
            body = ob.read()
            ob.close()
    ...
    def lock(self, scope='exclusive', type='write', owner='', depth='infinity', timeout='Infinite', **kw):
        ...
        if scope not in ('shared', 'exclusive'):
...
urlreg = re.compile('(http|https)://([^:/]+)(:[0-9]+)?(/.+)?', re.I)
...
vartypes = (
 (
  'int', type(1), marshal_int), ('float', type(1.0), marshal_float), ('long', type(1), marshal_long), ('list', type([]), marshal_list), ('tuple', type(()), marshal_tuple), ('string', type(''), marshal_string), ('file', types.FileType, None))
...
class MultiPart:
    ...
    _extmap = {'': 'text/plain', 'rdb': 'text/plain', 'html': 'text/html', 'dtml': 'text/html', 'htm': 'text/html', 'dtm': 'text/html', 'gif': 'image/gif', 'jpg': 'image/jpeg', 'exe': 'application/octet-stream', None: 'application/octet-stream'}
    _encmap = {'image/gif': 'binary', 'image/jpg': 'binary', 'application/octet-stream': 'binary'}


bri = Resource('http://tarzan.digicool.com/dev/brian3/', username='brian', password='123')
abri = Resource('http://tarzan.digicool.com/dev/brian3/')
dav = Resource('http://tarzan.digicool.com/dev/dav/', username='brian', password='123')
