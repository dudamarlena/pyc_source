...
# [GCC 8.4.0]
# Embedded file name: QrCodec/__init__.py
# Compiled at: 2007-12-18 12:33:56
...
def encode(string, image_width=default_image_width, case_sensitive=default_case_sensitiveness, version=default_version, error_correction_level=default_error_correction, encoding_hint=default_encoding):
    ...
    (version, width, data) = _qrencode(string + '\x00', case_sensitive, version, error_correction_level, encoding_hint)
...
def decode(image_file):
    ...
    if _getBitValue(status, 14):
        ...
        if _getBitValue(status, 8):
            comment += 'QR_IMAGEREADER_NOT_INVALID_SRC_IMAGE, '
        if _getBitValue(status, 9):
            comment += 'QR_IMAGEREADER_NOT_FOUND_FINDER_PATTERN, '
        if _getBitValue(status, 10):
            comment += 'QR_IMAGEREADER_NOT_FOUND_CODE_AREA, '
        if _getBitValue(status, 11):
            comment += 'QR_IMAGEREADER_NOT_DETERMINABLE_CODE_AREA, '
    else:
        if _getBitValue(status, 12):
            comment += 'QR_IMAGEREADER_WORKING, '
        if _getBitValue(status, 13):
            comment += 'QR_IMAGEREADER_DECODED, '
            ret_status = True
        if _getBitValue(status, 8):
            comment += 'QR_VERSIONINFO_INVALID, '
        if _getBitValue(status, 9):
            comment += 'QR_VERSIONINFO_MISMATCH, '
        if _getBitValue(status, 11):
            comment += 'QR_VERSIONINFO_UNRECOVERABLE, '
        if _getBitValue(status, 4):
            comment += 'QR_FORMATINFO_INVALID_LEVE, '
        if _getBitValue(status, 7):
            comment += 'QR_FORMATINFO_UNRECOVERABLE, '
        if _getBitValue(status, 0):
            comment += 'QR_CODEDATA_NOT_SUPPORT_ECI, '
        if _getBitValue(status, 1):
            comment += 'QR_CODEDATA_LENGTH_MISMATCH, '
        if _getBitValue(status, 3):
            comment += 'QR_CODEDATA_UNRECOVERABLE, '
    if comment == '':
