...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/wmpy_util/tf_util.py
# Compiled at: 2020-01-15 02:38:00
...
def graph_summay(session, log_dir):
    writter = tf.summary.FileWriter(log_dir, session.graph)
...
def load_model_from_hub(url, output_name=None, config=None):
    ...
    with tf.Graph().as_default() as (graph):
        ...
        session.run(init)
    return (
     session, input_tensor, output_tensor)
...
def get_input_output_from_graph(graph: tf.Graph):
    ...
    return (
     input_tensor, output_tensor, is_training)