...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/Documents/ripe/atlas/virtualenv/src/django-crocodile/crocodile/templatetags/crocodile.py
# Compiled at: 2013-03-19 10:50:40
...
class StaticfileNode(template.Node):
    ...
    def render(self, context):
        ...
        cache_filename = os.path.join(settings.MEDIA_ROOT, 'cache', self.type, '%s.%s' % (
         md5(getattr(settings, 'RELEASE', '') + source_markup).hexdigest(),
         self.type))
        ...
        return self._markup(cache_filename.replace(settings.MEDIA_ROOT, settings.MEDIA_URL))
    ...
    def _fetch_url(self, url):
        request = urllib2.Request(re.sub('^//(.*)', 'https://\\1', url), None, {'User-Agent': 'django-crocodile %s' % __version__})
...
class JavascriptNode(StaticfileNode):
    ...
    def _compile(self, context):
        ...
        for line in self.nodelist.render(context).split('\n'):
            path = re.match('^.*<script.*src=.([^\'\\"]+).*$', line)
            ignore = re.match('^.*</?script.*', line)
            if path:
...
class CSSNode(StaticfileNode):
        ...
        for line in self.nodelist.render(context).split('\n'):
            ...
            if comment:
                if '-->' in line:
                    comment = False
                continue
            source = re.match('^.*<link.*href=.([^\'\\"]+).*$', line)
            ignore = re.match('^.*</?style.*', line)
            if source:
                if not re.search('media=(\'|\\")?print(\'|\\")?', line):
    ...
    def _fetch_url(self, url):
        ...
        return re.sub(self._fetch_regex, lambda m: "url('%s://%s%s')" % (
         url.scheme,
         url.netloc,
         os.path.normpath(os.path.join(os.path.dirname(url.path), m.group(1)))), super(CSSNode, self)._fetch_url(url.geturl()))

    def _get_local_file(self, filename):
        return re.sub(self._fetch_regex, self._fix_remote_reference_in_local_file, super(CSSNode, self)._get_local_file(filename))

    def _fix_remote_reference_in_local_file(self, match):
        url = match.group(1)
        if re.match('^https?://.*', url):
            return "url('%s')" % url
        return "url('%s')" % os.path.normpath(os.path.join(os.path.dirname(filename), m.group(1)))