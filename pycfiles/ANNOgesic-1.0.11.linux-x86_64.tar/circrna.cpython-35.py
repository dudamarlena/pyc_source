...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/circrna.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 26143 bytes
...
class CircRNADetection(object):
    __doc__ = 'Detection of circRNA'

    def __init__(self, args_circ):
        self.multiparser = Multiparser()
        self.helper = Helper()
        self.converter = Converter()
        self.alignment_path = os.path.join(args_circ.output_folder, 'segemehl_alignment_files')
        self.splice_path = os.path.join(args_circ.output_folder, 'segemehl_splice_results')
        self.candidate_path = os.path.join(args_circ.output_folder, 'circRNA_tables')
        ...
        for reads in read_files:
            ...
            for read in reads['files']:
                if read.endswith('.bz2'):
                    ...
                    if '.fa' not in mod_read and '.fasta' not in mod_read and '.fna' not in mod_read and '.fq' not in mod_read and '.fastq' not in mod_read:
    ...
    def _run_segemehl_fasta_index(self, segemehl_path, fasta_path, index, fasta, log):
    ...
    def _run_segemehl_align(self, args_circ, index, fasta, read, sam_file, log_file, fasta_prefix, log):
        out = open(os.path.join(self.alignment_path, fasta_prefix, sam_file), 'w')
        log = open(os.path.join(self.alignment_path, fasta_prefix, log_file), 'w')
        ...
        p = Popen([args_circ.segemehl_path,
         '-i', os.path.join(self.fasta_path, index),
         '-d', os.path.join(self.fasta_path, fasta),
         '-q', read, '-S'], stdout=out, stderr=log)
        return p
    ...
    def _align(self, args_circ, read_datas, log):
        ...
        for fasta in os.listdir(self.fasta_path):
            ...
            self._run_segemehl_fasta_index(args_circ.segemehl_path, self.fasta_path, index, fasta, log)
            processes = []
            num_process = 0
            fasta_prefix = fasta.replace('.fa', '')
            prefixs.append(fasta_prefix)
            self.helper.check_make_folder(os.path.join(self.alignment_path, fasta_prefix))
            log.write('Running for {0}.\n'.format(fasta_prefix))
            for reads in read_datas:
                for read in reads['files']:
                    num_process += 1
                    read_name = read.split('/')[(-1)]
                    if read_name.endswith('.fa') or read_name.endswith('.fna') or read_name.endswith('.fasta') or read_name.endswith('.fq') or read_name.endswith('.fastq'):
                        ...
                        p = self._run_segemehl_align(args_circ, index, fasta, read, sam_file, log_file, fasta_prefix, log)
                        processes.append(p)
            ...
            log.write('Done!\n')
            log.write('The following files are generated in {0}:\n'.format(os.path.join(self.alignment_path, fasta_prefix)))
            for file_ in os.listdir(os.path.join(self.alignment_path, fasta_prefix)):
                log.write('\t' + file_ + '\n')
    ...
    def _convert_sam2bam(self, sub_alignment_path, samtools_path, align_files, log):
        ...
        for sam in os.listdir(sub_alignment_path):
            ...
            if sam.endswith('.sam'):
                ...
                self._run_samtools_convert_bam(samtools_path, pre_sam, out_bam, log)
                ...
                if sam.endswith('.bam'):
                    if pre_sam not in convert_ones and pre_sam not in remove_ones:
        ...
        return (
         bam_files, convert_ones, remove_ones)

    def _run_samtools_merge_sort(self, samtools_path, prefix, out_folder, bam_datas, log):
        log.write('Using Samtools for merging, sorting and converting the BAM files.\n')
        log.write('Make sure the version Samtools is at least 1.3.1.\n')
        for bam_data in bam_datas:
            print('Merging bam files for {0} of {1}'.format(prefix, bam_data['sample']))
            ...
            if len(bam_data['files']) <= 1:
                ...
                os.system(' '.join([samtools_path, 'merge',
                 sample_bam, file_line]))
            print('Sorting bam files for {0} of {1}'.format(prefix, bam_data['sample']))
            sort_sample = os.path.join(out_folder, '_'.join([prefix, bam_data['sample'] + '_sort.bam']))
            ...
            print('Converting bam files to sam files for {0} of {1}'.format(prefix, bam_data['sample']))
    ...
    def _merge_sort_aligment_file(self, bam_datas, read_datas, samtools_path, out_folder, convert_ones, tmp_reads, remove_ones, prefix, log):
        if bam_datas is None:
            ...
            for read_data in read_datas:
                ...
                for read in read_data['files']:
                    ...
                    bam_files.append(os.path.join(self.alignment_path, prefix, '_'.join([read_prefix, prefix + '.bam'])))

                merge_bam_datas.append({'sample': read_data['sample'], 
                 'files': bam_files})

        else:
            if bam_datas is not None and read_datas is not None:
                ...
                for bam_data in merge_bam_datas:
                    for read_data in read_datas:
                        if bam_data['sample'] == read_data['sample']:
                            for read in read_data['files']:
                                read_prefix = '.'.join(read.split('/')[(-1)].split('.')[:-1])
                                bam = os.path.join(self.alignment_path, prefix, '_'.join([read_prefix, prefix + '.bam']))
                                if bam not in bam_data['files']:
                                    bam_data['files'].append(bam)

            else:
                merge_bam_datas = copy.deepcopy(bam_datas)
        self._run_samtools_merge_sort(samtools_path, prefix, out_folder, merge_bam_datas, log)
        ...
        for sam_file in os.listdir(out_folder):
            if sam_file.endswith('sort.sam'):
                ...
                command = ' '.join([
                 ...
                 '-U',
                 os.path.join(sub_splice_path, sample_prefix + '_splicesites.bed'),
                 '-T',
                 os.path.join(sub_splice_path, sample_prefix + '_transrealigned.bed')])
    ...
    def _merge_bed(self, fastas, splice_path, output_folder):
        ...
        for fasta in os.listdir(fastas):
            headers = []
            if fasta.endswith('.fa') or fasta.endswith('.fna') or fasta.endswith('.fasta'):
                ...
                for header in headers:
                    for splice in os.listdir(os.path.join(splice_path, header)):
                        if splice.endswith('.bed'):
                            if self.splices['file'] in splice:
                                sample = splice.replace(header, '')
                                sample = sample.replace(self.splices['file'], '')
                                if sample not in samples:
                                    samples.append(sample)
                                shutil.copyfile(os.path.join(splice_path, header, splice), os.path.join(bed_folder, 'tmp_' + splice))
                ...
                for sample in samples:
                    ...
                    for file_ in os.listdir(bed_folder):
                        if self.splices['splice'] in file_ and sample in file_:
                            self.helper.merge_file(os.path.join(bed_folder, file_), out_splice)
                        elif self.trans['trans'] in file_ and sample in file_:
                            self.helper.merge_file(os.path.join(bed_folder, file_), out_trans)
    ...
    def _stat_and_gen_gff(self, prefixs, samples, args_circ, log):
        ...
        for prefix in prefixs:
            self.helper.check_make_folder(os.path.join(self.gff_folder, prefix))
            self.helper.check_make_folder(os.path.join(self.splice_path, prefix))
            for bed in os.listdir(os.path.join(args_circ.output_folder, prefix)):
                if bed.split('_')[0] != 'tmp' and bed.endswith('.bed'):
                    shutil.copy(os.path.join(args_circ.output_folder, prefix, bed), os.path.join(self.splice_path, prefix))

            self.helper.check_make_folder(os.path.join(self.candidate_path, prefix))
            print('Comparing circular RNAs with annotations of {0}'.format(prefix))
            for sample in samples:
                splice_file = os.path.join(self.splice_path, prefix, ''.join([prefix, sample, self.splices['file']]))
                stat_file = os.path.join(args_circ.stat_folder, ''.join(['stat_', prefix, sample,
                 'circRNA.csv']))
                csv_all = os.path.join(self.candidate_path, prefix, ''.join([prefix, sample, 'circRNA_all.csv']))
                csv_best = os.path.join(self.candidate_path, prefix, ''.join([prefix, sample, 'circRNA_best.csv']))
                gff_all = os.path.join(self.gff_folder, prefix, ''.join([prefix, sample, 'circRNA_all.gff']))
                gff_best = os.path.join(self.gff_folder, prefix, ''.join([prefix, sample, 'circRNA_best.gff']))
                detect_circrna(splice_file, os.path.join(self.gff_path, prefix + '.gff'), csv_all, args_circ, stat_file)
                self.converter.convert_circ2gff(os.path.join(self.candidate_path, prefix, ''.join([prefix, sample, 'circRNA_all.csv'])), args_circ, gff_all, gff_best)
                log.write('\t' + stat_file + '\n')
                log.write('\t' + csv_all + '\n')
                log.write('\t' + csv_best + '\n')
                log.write('\t' + gff_all + '\n')
    ...
    def _extract_input_files(self, inputs):
        ...
        for input_ in inputs:
            datas = input_.split(':')
            if len(datas) != 2:
                print('Error: the format of --bam_files or --read_files is wrong!')
                sys.exit()
            for file_ in datas[(-1)].split(','):
                if not os.path.exists(file_):
                    print('Error: some files in --bam_files or --read_files do not exist!')
                    sys.exit()
    ...
    def _combine_read_bam(self, bam_files, bam_datas, read_datas):
        if bam_datas is not None:
                ...
                for read_data in read_datas:
                    if bam_data['sample'] == read_data['sample']:
                        for read in read_data['files']:
                            ...
                            bam = os.path.join(self.alignment_path, prefix + '.bam')
                            if bam in bam_files and bam not in bam_data['files']:
                                bam_data['files'].append(bam)
            ...
            for read_data in read_datas:
                ...
                for read in read_data['files']:
                    prefix = '.'.join(read.split('/')[(-1)].split('.')[:-1])
                    bam_files.append(os.path.join(self.alignment_path, prefix + '.bam'))
    ...
    def run_circrna(self, args_circ, log):
        ...
        for gff in os.listdir(args_circ.gffs):
            if gff.endswith('.gff'):
                self.helper.check_uni_attributes(os.path.join(args_circ.gffs, gff))
        ...
        self.multiparser.combine_gff(args_circ.fastas, self.gff_path, 'fasta', None)
        tmp_reads = []
        ...
        for prefix in prefixs:
            if args_circ.read_files:
                ...
                bam_files, convert_ones, remove_ones = self._convert_sam2bam(sub_alignment_path, args_circ.samtools_path, align_files, log)
            else:
                convert_ones = []
                remove_ones = []
            self._merge_sort_aligment_file(bam_datas, read_datas, args_circ.samtools_path, args_circ.output_folder, convert_ones, tmp_reads, remove_ones, prefix, log)
            self._run_testrealign(prefix, args_circ.testrealign_path, args_circ.output_folder, log)

        samples, fa_prefixs = self._merge_bed(args_circ.fastas, self.splice_path, args_circ.output_folder)
