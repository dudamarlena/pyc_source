...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/merge_manual.py
# Compiled at: 2019-06-28 10:13:07
# Size of source mod 2**32: 28039 bytes
import os, math
    ...
    for strain, tracks in wigs.items():
        if strain == tss.seq_id:
            ...
            for wig in tracks.values():
                if tss.start + 1 <= len(wig) and ref.start + 1 <= len(wig):
                    if tss.strand == '+':
                        diff_t = wig[(tss.start - 1)]['coverage'] - wig[(tss.start - 2)]['coverage']
                        diff_r = wig[(ref.start - 1)]['coverage'] - wig[(ref.start - 2)]['coverage']
                    else:
                        diff_t = wig[(tss.start - 1)]['coverage'] - wig[tss.start]['coverage']
                        diff_r = wig[(ref.start - 1)]['coverage'] - wig[ref.start]['coverage']
    ...
    for tss in tsss:
        ...
        for type_ in types:
            if type_ == 'Primary' and detect['pri'] is False:
                ...
                if type_ == 'Primary' and detect['pri'] is True:
                    compare_utr = int(utrs[index].split('_')[1])
                    if compare_utr < pri_utr:
                        pri_utr = compare_utr
                        real_index = index
                else:
                    if type_ == 'Secondary' and detect['sec'] is False:
                        detect['sec'] = True
                        sec_utr = int(utrs[index].split('_')[1])
                        real_index2 = index
                    else:
                        if type_ == 'Secondary' and detect['sec'] is True:
                            compare_utr = int(utrs[index].split('_')[1])
                            if compare_utr < sec_utr:
                                sec_utr = compare_utr
                                real_index2 = index
                        elif type_ == 'Antisense' or type_ == 'Internal' or type_ == 'Orphan':
...
def fix_primary_type(tsss, wigs_f, wigs_r):
    ...
    for tss in tsss:
        if 'Primary' in tss.attributes['type']:
            ...
            for ref in tsss:
                if ref.seq_id == tss.seq_id and ref.strand == tss.strand and ref.start == tss.start:
                    pass
                elif 'Primary' in ref.attributes['type']:
                    ref_entrys = get_primary_locus_tag(ref)
                    for tss_entry in tss_entrys:
                        for ref_entry in ref_entrys:
                            if tss_entry['locus'] == ref_entry['locus'] and tss_entry['type'] == 'Primary' and ref_entry['type'] == 'Primary' and tss.seq_id == ref.seq_id:
                                if tss.strand == '+':
                                    tss_cover, ref_cover = detect_coverage(wigs_f, tss, ref)
                            else:
                                tss_cover, ref_cover = detect_coverage(wigs_r, tss, ref)
...
def same_strand_tss_gene(gene, tss, anti_ends, gene_ends, checks, tss_entry):
    ...
    if is_primary(gene.start, gene.end, tss.start, tss.strand):
        ...
        if tss.strand == '+':
            if anti_ends['reverse'] != -1 and anti_ends['reverse'] - gene.start > 0 or anti_ends['reverse'] == -1:
                tss_entry = import_to_tss('Primary', gene.start, tss, locus_tag, tss_entry)
                checks['orphan'] = False
                gene_ends['forward'] = gene.start
            elif anti_ends['reverse'] != -1 and anti_ends['reverse'] - gene.start < 0 and (checks['int_anti'] is True or tss.start - anti_ends['reverse'] > 0):
                tss_entry = import_to_tss('Primary', gene.start, tss, locus_tag, tss_entry)
                checks['orphan'] = False
                gene_ends['forward'] = gene.start
        elif anti_ends['forward'] != -1 and gene.end - anti_ends['forward'] > 0 or anti_ends['forward'] == -1:
    ...
    if is_antisense(gene.start, gene.end, tss.start, tss.strand):
        ...
        if tss.strand == '-':
            ...
            if gene_ends['reverse'] != -1 and gene.start - gene_ends['reverse'] > 0:
                if is_internal(gene.start, gene.end, tss.start, tss.strand):
                    pass
                elif tss.start - gene.end > 0:
                    tss_entry = remove_primary(tss, tss_entry)
            else:
                anti_ends['reverse'] = gene.end
                if is_internal(gene.start, gene.end, tss.start, tss.strand):
                    checks['int_anti'] = True
                if gene_ends['forward'] != -1 and gene.start - gene_ends['forward'] > 0 and gene.start - tss.start > 0:
                    tss_entry = remove_primary(tss, tss_entry)
                locus_tag = gene.attributes['locus_tag']
...
def compare_tss_gene(tss, genes):
    ...
    for gene in genes:
        if gene.strand == tss.strand:
            tss_entry = same_strand_tss_gene(gene, tss, anti_ends, gene_ends, checks, tss_entry)
        else:
            tss_entry = diff_strand_tss_gene(gene, tss, anti_ends, gene_ends, checks, tss_entry)
...
def is_primary(cds_start, cds_end, tss_pos, strand):
    if strand == '+':
        if is_utr(cds_start, tss_pos, 300) and cds_start >= tss_pos:
            return True
    elif is_utr(tss_pos, cds_end, 300) and cds_end <= tss_pos:
        return True


def is_internal(cds_start, cds_end, tss_pos, strand):
    if cds_start < tss_pos and cds_end > tss_pos or strand == '+' and tss_pos == cds_end or strand == '-' and tss_pos == cds_start:
        return True


def is_antisense(cds_start, cds_end, tss_pos, strand):
    if is_utr(cds_start, tss_pos, 100) and cds_start >= tss_pos or is_utr(tss_pos, cds_end, 100) and cds_end <= tss_pos or is_internal(cds_start, cds_end, tss_pos, strand):
        return True
...
def print_all_unique(out, overlap_num, nums):
    if (nums['tss_p'] != 0 or overlap_num != 0) and (nums['tss_m'] != 0 or overlap_num != 0):
        out.write('the number of overlap between TSSpredator and manual = {0} '.format(overlap_num))
        out.write('(overlap of all TSSpredator = {0}, '.format(float(overlap_num) / (float(nums['tss_p']) + float(overlap_num))))
        out.write('overlap of all manual = {0})\n'.format(float(overlap_num) / (float(nums['tss_m']) + float(overlap_num))))
        out.write('the number of unique in TSSpredator = {0} ({1})\n'.format(nums['tss_p'], float(nums['tss_p']) / (float(nums['tss_p']) + float(overlap_num))))
        out.write('the number of unique in manual = {0} ({1})\n'.format(nums['tss_m'], float(nums['tss_m']) / (float(nums['tss_m']) + float(overlap_num))))
    else:
...
def print_stat(num_strain, stat_file, overlap_num, nums):
    if len(num_strain) != 0:
        ...
        for strain in num_strain.keys():
            if num_strain[strain]['tsspredator'] == 0 and num_strain[strain]['overlap'] == 0:
                perc_tsspredator = 'NA'
                perc_tsspredator_uni = 'NA'
            else:
                perc_tsspredator = str(float(num_strain[strain]['overlap']) / (float(num_strain[strain]['tsspredator']) + float(num_strain[strain]['overlap'])))
                perc_tsspredator_uni = str(float(num_strain[strain]['tsspredator']) / (float(num_strain[strain]['tsspredator']) + float(num_strain[strain]['overlap'])))
            if num_strain[strain]['manual'] == 0 and num_strain[strain]['overlap'] == 0:
                perc_manual = 'NA'
                perc_manual_uni = 'NA'
            else:
                perc_manual = str(float(num_strain[strain]['overlap']) / (float(num_strain[strain]['manual']) + float(num_strain[strain]['overlap'])))
                perc_manual_uni = str(float(num_strain[strain]['manual']) / (float(num_strain[strain]['manual']) + float(num_strain[strain]['overlap'])))
            out.write(strain + ': \n')
            out.write('the number of overlap between TSSpredator and manual = {0} '.format(num_strain[strain]['overlap']))
            out.write('(overlap of all TSSpredator = {0}, '.format(perc_tsspredator))
            out.write('overlap of all manual = {0})\n'.format(perc_manual))
            out.write('the number of unique in TSSpredator = {0} ({1})\n'.format(num_strain[strain]['tsspredator'], perc_tsspredator_uni))
            out.write('the number of unique in manual = {0} ({1})\n'.format(num_strain[strain]['manual'], perc_manual_uni))
...
def read_wig(filename, strand):
    ...
    if filename:
        ...
        for entry in wig_parser.parser(wig_fh, strand):
            ...
            wigs[strain][entry.track].append({'pos': entry.pos, 'coverage': entry.coverage, 
             'strand': entry.strand})
...
def merge_libs(input_libs, wig_folder, program):
    ...
    for lib in input_libs:
        ...
        if datas[1] == type_ and datas[4] == '+':
            Helper().merge_file(os.path.join(wig_folder, datas[0]), os.path.join(os.getcwd(), 'merge_forward.wig'))
        elif datas[1] == type_ and datas[4] == '-':
            Helper().merge_file(os.path.join(wig_folder, datas[0]), os.path.join(os.getcwd(), 'merge_reverse.wig'))


def check_overlap(overlap, pre_tss, nums, length, num_strain, overlap_num, tss_m, tss_p, tsss, pre_pos, cdss, genes):
    ...
    if overlap:
        ...
        if not length or tss.start <= int(length):
        ...
        if not length or tss_m.start <= int(length):
            num_strain[tss_m.seq_id]['manual'] += 1
            nums['tss_m'] += 1
            nums['tss'] += 1
        return (
         overlap, pre_pos, overlap_num)
    ...
    for tss_m in tsss['tsss_m']:
        ...
        if 'all' in lengths.keys():
            ...
            for tss_p in tsss['tsss_p']:
                start = True
                if tss_p.strand == tss_m.strand and tss_p.seq_id == tss_m.seq_id:
            ...
            if (start or not overlap) and tss_m.seq_id not in num_strain.keys():
        ...
        if pre_tss is None:
            if tss_p.seq_id not in num_strain.keys():
                num_strain[tss_p.seq_id] = {'overlap': 0, 'tsspredator': 0, 
                 'manual': 0}
        elif pre_tss.seq_id not in num_strain.keys():
            num_strain[pre_tss.seq_id] = {'overlap': 0, 'tsspredator': 0, 
             'manual': 0}
        datas = check_overlap(overlap, pre_tss, nums, length, num_strain, overlap_num, tss_m, tss_p, tsss, pre_pos, cdss, genes)
        overlap = datas[0]
        pre_pos = datas[1]
        overlap_num = datas[2]
    ...
    for tss in final_tsss:
        ...
        if program == 'TSS':
            ...
            tss.attributes['Name'] = 'processing:' + '_'.join([
             str(tss.start), strand])
        tss.attribute_string = ';'.join(['='.join(items) for items in tss.attributes.items()])
...
def merge_manual_predict_tss(tss_predict_file, stat_file, out_gff, gff_file, args_tss, manual, seq_file):
    ...
    tsss, cdss, genes = read_gff(tss_predict_file, manual, gff_file, args_tss.strain_lengths)
    overlap_num, num_strain = intersection(tsss, args_tss.cluster, nums, args_tss.strain_lengths, cdss, genes, seqs)
