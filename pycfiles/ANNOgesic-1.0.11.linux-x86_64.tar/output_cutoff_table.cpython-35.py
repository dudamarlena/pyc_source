...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/output_cutoff_table.py
    ...
    for row in csv.reader(fh, delimiter='\t'):
        if row[0] != 'rank':
            ...
            if float(row[10]) >= cutoff_cover:
                ...
                rank += 1
                for gff in gffs:
                    if row[1] == gff.seq_id and row[3] == str(gff.start) and row[4] == str(gff.end) and row[5] == gff.strand:
                        new_gffs.append(gff)
