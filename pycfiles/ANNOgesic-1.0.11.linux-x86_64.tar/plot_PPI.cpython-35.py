...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/plot_PPI.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 10645 bytes
import os, networkx as nx, matplotlib
matplotlib.use('Agg')
...
def node(item, nodes, center, colors, labels1, labels2):
    if item not in nodes:
        ...
        if len(item) > 5:
            ...
            labels1[item] = item
            labels2[item] = ''
        if center['locus_tag'] == item or center['gene_name'] == item:
            colors[item] = '#FFFF66'
    else:
        colors[item] = '#CCFFCC'
...
def add_edge(G, ppi, style, weight, colorppi):
    G.add_edge(ppi['item_a'], ppi['item_b'], color=float(colorppi), style=style, weight=weight)
...
def create_node(ppis, scores, nodes, center, colors, labels1, labels2, edges, G, cutoff_score, check_na, pre_ppi):
    first = True
    for ppi in ppis:
        ...
        if (ppi['item_a'], ppi['item_b']) not in edges or (
         ppi['item_b'], ppi['item_a']) not in edges:
            ...
            if ppi['best'] == 'NA':
                ...
                if float(ppi['best']) <= cutoff_score:
...
def nx_node(G, pos, node_size, colors, color_list):
    """draw the node"""
    nx.draw_networkx_nodes(G, pos, node_size=node_size, node_shape='o', nodelist=colors.keys(), node_color=color_list, linewidths=1)


def nx_edge(G, pos, edges, colors, styles, weights):
    """draw the edge"""
    color_edge = nx.draw_networkx_edges(G, pos, edges=edges, edge_color=colors, style=styles, width=weights, edge_vmin=-1, edge_vmax=1, vmin=-1, vmax=1)
    return color_edge
...
def nx_color_style(G, edges):
    ...
    for u, v in edges:
        ...
        if G[u][v]['style'] == 'solid' or G[u][v]['style'] == 'dashdot':
            check_na = False

    return (
     colors, styles, check_na)
    ...
    if not na:
        plt.title('|'.join([center['locus_tag'],
         ' '.join([center['gene_name'],
          '(based on the score of best literature)'])]), fontsize='16')
    else:
        plt.title('|'.join([center['locus_tag'],
         ' '.join([center['gene_name'],
          '(based on the score of best literature)'])]) + '\n the numbers of supported literatures in all interactions are 0', fontsize='16')
...
def plot(ppis, center, strain, cutoff_score, node_size, out_folder):
    ...
    G = nx.Graph()
    pre_ppi = create_node(ppis, scores, nodes, center, colors, labels1, labels2, edges, G, cutoff_score, check_na, pre_ppi)
    ...
    plt.savefig(os.path.join(out_folder, strain, '_'.join([center['locus_tag'], center['gene_name'] + '.png'])), bbox_inches='tight')
    plt.clf()
    plt.close('all')
    return check_na
...
def interaction(first, pre_ppi, scores, ppis, match, center, cutoff_score, node_size, out_folder):
    ...
    if first:
        ...
        if match:
            ...
            print('No interacted partner with {0} | {1}'.format(center['locus_tag'], center['gene_name']))
        scores = {'score': 0, 'below': 0}
        ppis = []
        first = True
    return (
     first, scores, match, ppis)
    ...
    with open(PPI_file) as (fh):
        for line in fh:
            ...
            if row[0].startswith('Interaction'):
                first, scores, match, ppis = interaction(first, pre_ppi, scores, ppis, match, center, cutoff_score, node_size, out_folder)
                datas = row[0].split(' | ')
                ...
                if row[0] == 'Genome':
                    ...
                    ppi = {'strain': row[0], 'item_a': row[1], 'item_b': row[2], 
                     'mode': row[3]}
                    if ppi['item_a'] == center['locus_tag'] or ppi['item_a'] == center['gene_name'] or ppi['item_b'] == center['locus_tag'] or ppi['item_b'] == center['gene_name']:
                        match = True
                    if first:
                        first = False
                        ...
                        if ppi['strain'] == pre_ppi['strain'] and ppi['item_a'] == pre_ppi['item_a'] and ppi['item_b'] == pre_ppi['item_b']:
