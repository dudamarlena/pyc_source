...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/transcript.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 17992 bytes
...
class TranscriptDetection(object):
    __doc__ = 'doing for transcript detection'
    ...
    def _compute_transcript(self, wig_f, wig_r, wig_folder, wig_type, strain, libs, args_tran):
    ...
    def _compute(self, wig_type, wigs, libs, args_tran):
        ...
        for strain in strains:
            ...
            r_file = os.path.join(wig_folder, '_'.join([
             strain, 'reverse.wig']))
            self._compute_transcript(f_file, r_file, wigs, wig_type, strain, libs, args_tran)

        return strains

    def _compare_tss(self, tas, args_tran, log):
        self.multiparser.parser_gff(args_tran.compare_tss, 'TSS')
        self.multiparser.combine_gff(self.gff_outfolder, os.path.join(args_tran.compare_tss, 'tmp'), 'transcript', 'TSS')
        print('Comaring of transcripts and TSSs')
        log.write('Running stat_TA_comparison.py to compare transcripts with TSSs.\n')
        tss_folder = os.path.join(args_tran.compare_tss, 'tmp')
        for ta in tas:
            ta_file = os.path.join(self.gff_outfolder, '_'.join([ta, self.endfix_tran]))
            stat_tss_out = os.path.join(self.stat_path, ''.join([
             'stat_compare_transcript_TSS_',
             ta, '.csv']))
            for tss in os.listdir(tss_folder):
                filename = tss.split('_TSS')
                if filename[0] == ta and tss.endswith('.gff'):
                    stat_ta_tss(ta_file, os.path.join(tss_folder, tss), stat_tss_out, self.tmps['ta_tss'], self.tmps['tss_ta'], args_tran.fuzzy)
                    os.remove(ta_file)
                    os.remove(os.path.join(tss_folder, tss))
                    self.helper.sort_gff(self.tmps['ta_tss'], ta_file)
                    self.helper.sort_gff(self.tmps['tss_ta'], os.path.join(args_tran.compare_tss, tss))
    ...
    def _compare_cds(self, tas, args_tran, log):
        ...
        self.multiparser.combine_gff(self.gff_outfolder, os.path.join(args_tran.gffs, 'tmp'), 'transcript', None)
        print('Comaring of transcripts and genome annotations')
        cds_folder = os.path.join(args_tran.gffs, 'tmp')
        log.write('Running stat_TA_comparison.py to compare transcripts with genome annotations.\n')
        for ta in tas:
            ta_file = os.path.join(self.gff_outfolder, '_'.join([ta, self.endfix_tran]))
            ...
            for gff in os.listdir(cds_folder):
                if gff[:-4] == ta and gff.endswith('.gff'):
                    ...
                    stat_ta_gff(ta_file, cds_file, stat_gff_out, self.tmps['ta_gff'], self.tmps['gff_ta'], args_tran.c_feature)
                    os.remove(ta_file)
                    os.remove(os.path.join(args_tran.gffs, gff))
                    self.helper.sort_gff(self.tmps['ta_gff'], ta_file)
                    self.helper.sort_gff(self.tmps['gff_ta'], os.path.join(args_tran.gffs, gff))
    ...
    def _compare_tss_cds(self, tas, args_tran, log):
        ...
        if args_tran.compare_tss is not None and args_tran.c_feature is not None:
            ...
            if args_tran.c_feature is not None and args_tran.compare_tss is None:
                self.multiparser.parser_gff(self.gff_outfolder, 'transcript')
                self._compare_cds(tas, args_tran, log)
            elif args_tran.c_feature is None and args_tran.compare_tss is not None:
                self.multiparser.parser_gff(self.gff_outfolder, 'transcript')
                self._compare_tss(tas, args_tran, log)
    ...
    def _for_one_wig(self, type_, args_tran):
        ...
        for strain in strains:
            ...
            print(os.path.join(args_tran.out_folder, '_'.join([strain, type_])))
            self.helper.sort_gff(os.path.join(args_tran.out_folder, '_'.join([strain, type_])), out)
            os.remove(os.path.join(args_tran.out_folder, '_'.join([strain, type_])))

        return strains

    def _for_two_wigs(self, strains, args_tran, log):
        """merge the results of fragemented and tex treated libs"""
        if args_tran.frag_wigs is not None and args_tran.tex_wigs is not None:
            log.write('Running combine_frag_tex.py to merge the results from fragmented libs and dRNA-Seq libs.\n')
            print('Merging fragmented and tex treated ones')
            for strain in strains:
                frag_gff = os.path.join(self.gff_outfolder, '_'.join([strain, self.frag]))
                tex_gff = os.path.join(self.gff_outfolder, '_'.join([strain, self.tex]))
                final_gff = os.path.join(self.gff_outfolder, '_'.join([strain, self.endfix_tran]))
                for gff in os.listdir(self.gff_outfolder):
                    if '_transcript_' in gff:
                        filename = gff.split('_transcript_')
                        if strain == filename[0] and 'tex_notex.gff' == filename[1]:
                            tex_file = gff
                        elif strain == filename[0] and 'fragment.gff' == filename[1]:
                            frag_file = gff

                combine(os.path.join(self.gff_outfolder, frag_file), os.path.join(self.gff_outfolder, tex_file), args_tran.tolerance, os.path.join(self.gff_outfolder, '_'.join([strain, self.endfix_tran])))
                os.remove(frag_gff)
                os.remove(tex_gff)
    ...
    def _post_modify(self, tas, args_tran):
        ...
        for ta in tas:
            ...
            fill_gap(os.path.join(args_tran.gffs, gff), os.path.join(self.tran_path, '_'.join([ta, self.endfix_tran])), 'overlap', self.tmps['overlap'], args_tran.modify)
            fill_gap(os.path.join(args_tran.gffs, gff), os.path.join(self.tran_path, '_'.join([ta, self.endfix_tran])), 'uni', self.tmps['uni'], args_tran.modify)
            ...
            os.remove(tmp_merge)
            final_out = os.path.join(self.gff_outfolder, '_'.join(['final', ta]))
            longer_ta(tmp_out, args_tran.length, final_out)
            shutil.move(final_out, os.path.join(self.tmps['tran'], '_'.join([ta, self.endfix_tran])))
            os.remove(tmp_out)
    ...
    def _compare_term_tran(self, args_tran, log):
        ...
        if args_tran.terms is not None:
            ...
            if args_tran.gffs is not None:
                self.multiparser.combine_gff(args_tran.gffs, os.path.join(args_tran.terms, 'tmp'), None, 'term')
            log.write('Running compare_tran_term.py to compare transcripts with terminators.\n')
            compare_term_tran(self.gff_outfolder, os.path.join(args_tran.terms, 'tmp'), args_tran.fuzzy_term, args_tran.fuzzy_term, args_tran.out_folder, 'transcript', args_tran.terms, self.gff_outfolder)
    ...
    def _re_table(self, args_tran, log):
        ...
        for gff in os.listdir(self.gff_outfolder):
            if os.path.isfile(os.path.join(self.gff_outfolder, gff)):
                tran_table = os.path.join(args_tran.out_folder, 'tables', gff.replace('.gff', '.csv'))
                reorganize_table(args_tran.libs, args_tran.merge_wigs, 'Coverage_details', tran_table)
    ...
    def run_transcript(self, args_tran, log):
        if args_tran.frag_wigs is None and args_tran.tex_wigs is None:
            ...
            sys.exit()
        if args_tran.frag_wigs is not None:
            log.write('Running transcript_detection.py for detecting transcripts based on fragmented libs.\n')
            strains = self._for_one_wig('fragment', args_tran)
        if args_tran.tex_wigs is not None:
            log.write('Running transcript_detection.py for detecting transcripts based on dRNA-Seq libs.\n')
        ...
        if 'none' not in args_tran.modify:
            for gff in os.listdir(args_tran.gffs):
                if gff.endswith('.gff'):
                    self.helper.sort_gff(os.path.join(args_tran.gffs, gff), self.tmps['gff'])
                    shutil.move(self.tmps['gff'], os.path.join(args_tran.gffs, gff))

            self.multiparser.combine_gff(args_tran.gffs, os.path.join(args_tran.gffs, 'tmp'), None, None)
            self.multiparser.parser_gff(self.gff_outfolder, 'transcript')
            self.multiparser.combine_gff(args_tran.gffs, self.tran_path, None, 'transcript')
            self.helper.check_make_folder(self.tmps['tran'])
            for ta in os.listdir(self.tran_path):
                if ta.endswith('.gff') and os.path.getsize(os.path.join(self.tran_path, ta)) != 0:
                    tas.append(ta.replace('_' + self.endfix_tran, ''))

            log.write('Running fill_gap.py to modify transcripts based on genome annotations.\n')
        ...
        log.write('Running plot_tran to plot the distribution of the length of the transcripts.\n')
