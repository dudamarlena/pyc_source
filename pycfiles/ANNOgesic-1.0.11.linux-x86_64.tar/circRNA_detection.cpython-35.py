...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/circRNA_detection.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 8285 bytes
...
def detect_conflict(gffs, circ, num, out, out_best, args_circ):
    ...
    gff = None
    for gff in gffs:
        if gff.seq_id == circ.strain and gff.strand == circ.strand and (gff.start <= circ.start and gff.end >= circ.start and gff.end <= circ.end or gff.start >= circ.start and gff.end <= circ.end or gff.start >= circ.start and gff.start <= circ.end and gff.end >= circ.end and gff.start <= circ.start and gff.end >= circ.end):
            detect = True
            break

    if detect:
        feature = get_feature(gff)
        out.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(circ.strain, circ.strand, circ.start, circ.end, feature, circ.supported_reads, float(circ.supported_reads) / float(circ.start_site_reads), float(circ.supported_reads) / float(circ.end_site_reads)))
    else:
        ...
        if circ.supported_reads >= args_circ.support and start_read >= args_circ.start_ratio and end_read >= args_circ.end_ratio:
            out_best.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(circ.strain, circ.strand, circ.start, circ.end, 'NA', circ.supported_reads, start_read, end_read))
        out.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format(circ.strain, circ.strand, circ.start, circ.end, 'NA', circ.supported_reads, start_read, end_read))
    return detect
        ...
        if circ.situation != 'F' and circ.splice_type == 'C':
            ...
            for support in range(0, high + 5, 5):
                if circ.supported_reads >= int(support):
                    ...
                    if detect is False and float(circ.supported_reads) / float(circ.start_site_reads) >= args_circ.start_ratio and float(circ.supported_reads) / float(circ.end_site_reads) >= args_circ.end_ratio:
...
def detect_circrna(input_file, gff_file, output_file, args_circ, statistics):
    ...
    out.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n'.format('Genome', 'Strand', 'Start', 'End', 'Annotation_overlap', 'Supported_reads', 'Supported_reads/Reads_at_start', 'Supported_reads/Reads_at_end'))
