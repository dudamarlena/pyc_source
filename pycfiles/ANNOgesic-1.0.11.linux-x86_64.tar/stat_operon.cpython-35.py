...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_operon.py
...
def print_stat(operons, total_num, class_operon, out):
    ...
    for key, value in num_features.items():
        out.write('\tthe number of operons which {0} = {1} ({2})\n'.format(key, value, float(value) / float(total_num)))

    out.write('mono/polycistronic:\n')
    out.write('\tmonocistronic: {0} ({1})\n'.format(class_operon['mono'], float(class_operon['mono']) / float(class_operon['total'])))
    out.write('\tpolycistronic: {0} ({1})\n'.format(class_operon['poly'], float(class_operon['poly']) / float(class_operon['total'])))
...
def stat(input_file, out_file):
    ...
    for row in csv.reader(f_h, delimiter='\t'):
        if row[0] != 'Operon_ID' and row[0] != tmp_id:
            if pre_seq_id != row[1]:
                pre_seq_id = row[1]
                operons[row[1]] = []
                total_num[row[1]] = 0
