...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/lib_reader.py
...
def read_libs(input_libs, wig_folder):
    ...
    for lib1 in libs:
        if lib1['type'] == 'frag':
            continue
        if lib1['type'] == 'tex' or lib1['type'] == 'notex':
            prefix1 = lib1['cond'].split('_')[0]
            for lib2 in libs:
                prefix2 = lib2['cond'].split('_')[0]
                if prefix1 == prefix2 and lib1['rep'] == lib2['rep'] and lib1['type'] == 'tex' and lib2['type'] == 'notex' and lib1['strand'] == lib2['strand']:
                    texs[lib1['name'] + '@AND@' + lib2['name']] = 0

        else:
            print('Error: Wrong library types are detected, please assign frag, tex or notex.')
            sys.exit()
...
def read_wig(filename, strand, libs):
    ...
    if filename is not False:
        ...
        for entry in wig_parser.parser(wig_fh, strand):
            ...
            for lib in libs:
                if lib['name'] == entry.track and lib['strand'] == entry.strand:
                    lib_name = '|'.join([
        ...
        for strain, conds in wigs.items():
            for cond, lib_names in conds.items():
                for lib_name, cover_list in lib_names.items():
                    wigs[strain][cond][lib_name] = np.array(wigs[strain][cond][lib_name])
