...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/srna_target.py
# Compiled at: 2019-07-04 06:35:08
# Size of source mod 2**32: 29224 bytes
...
class sRNATargetPrediction(object):
    __doc__ = 'detection of sRNA-target interaction'
    ...
    def _run_rnaplfold(self, rnaplfold_path, file_type, win_size, span, unstr_region, seq_path, prefix, out_path, log):
        current = os.getcwd()
        os.chdir(out_path)
        ...
        if file_type == 'sRNA':
            log.write('<'.join([command,
             os.path.join(current, seq_path, '_'.join([self.tmps['tmp'], prefix,
              file_type + '.fa']))]) + '\n')
            ...
            log.write('<'.join([command,
             os.path.join(current, seq_path, '_'.join([prefix, file_type + '.fa']))]) + '\n')
            os.system('<'.join([command,
             os.path.join(current, seq_path, '_'.join([prefix, file_type + '.fa']))]))
        os.chdir(current)
    ...
    def _get_specific_seq(self, srna_file, seq_file, srna_out, querys):
        for query in querys:
            ...
            for entry in self.gff_parser.entries(gff_f):
                if entry.seq_id == srna['seq_id'] and entry.strand == srna['strand'] and entry.start == srna['start'] and entry.end == srna['end']:
                    detect = True
                    ...
                    out.write('>{0}|{1}|{2}|{3}|{4}\n{5}\n'.format(id_, entry.seq_id, entry.start, entry.end, entry.strand, gene))
                    num += 1
    ...
    def _gen_seq(self, prefixs, args_tar):
        ...
        for srna in os.listdir(self.srna_path):
            if srna.endswith('_sRNA.gff'):
                ...
                if 'all' in args_tar.query:
                    self.helper.get_seq(os.path.join(self.srna_path, srna), os.path.join(self.fasta_path, prefix + '.fa'), srna_out)
                else:
                    if '_'.join([prefix, 'sRNA.fa']) in os.listdir(self.srna_seq_path):
                        os.remove(srna_out)
                    self._get_specific_seq(os.path.join(self.srna_path, srna), os.path.join(self.fasta_path, prefix + '.fa'), srna_out, args_tar.query)
        ...
        for gff in os.listdir(self.gff_path):
            if gff.endswith('.gff'):
                ...
                potential_target(os.path.join(self.gff_path, gff), os.path.join(self.fasta_path, prefix + '.fa'), os.path.join(self.target_seq_path), args_tar)
                file_num = 1
                num = 0
                sub_prefix = os.path.join(self.target_seq_path, '_'.join([prefix, 'target']))
    ...
    def _run_rnaplex(self, prefix, rnaplfold_folder, args_tar, log):
        ...
        for seq in os.listdir(self.target_seq_path):
            if prefix in seq and '_target_' in seq:
                ...
                out_rnaplex = open(os.path.join(self.rnaplex_path, prefix, '_'.join([
                 prefix, 'RNAplex', str(num_process) + '.txt'])), 'w')
                num_process += 1
                log.write(' '.join([args_tar.rnaplex_path,
                 '-q',
                 os.path.join(self.srna_seq_path, '_'.join([
        ...
        log.write('The following temporary files for storing results of {0} are generated:\n'.format(prefix))
    ...
    def _rna_plex(self, prefixs, args_tar, log):
        ...
        for prefix in prefixs:
            ...
            rnaplfold_folder = os.path.join(self.rnaplex_path, prefix, 'RNAplfold')
            os.mkdir(rnaplfold_folder)
            self._run_rnaplfold(args_tar.rnaplfold_path, 'sRNA', args_tar.win_size_s, args_tar.span_s, args_tar.unstr_region_rnaplex_s, self.srna_seq_path, prefix, rnaplfold_folder, log)
            self._run_rnaplfold(args_tar.rnaplfold_path, 'target', args_tar.win_size_t, args_tar.span_t, args_tar.unstr_region_rnaplex_t, self.target_seq_path, prefix, rnaplfold_folder, log)
            num_process = self._run_rnaplex(prefix, rnaplfold_folder, args_tar, log)
            rnaplex_file = os.path.join(self.rnaplex_path, prefix, '_'.join([prefix, 'RNAplex.txt']))
            if '_'.join([prefix, 'RNAplex.txt']) in os.listdir(os.path.join(self.rnaplex_path, prefix)):
                os.remove(rnaplex_file)
            for index in range(0, num_process):
                log.write('Using helper.py to merge the temporary files.\n')
                self.helper.merge_file(os.path.join(self.rnaplex_path, prefix, '_'.join([
                 prefix, 'RNAplex', str(index) + '.txt'])), rnaplex_file)

            log.write('\t' + rnaplex_file + ' is generated.\n')
            self.helper.remove_all_content(os.path.join(self.rnaplex_path, prefix), '_RNAplex_', 'file')
            self.fixer.fix_rnaplex(rnaplex_file, self.tmps['tmp'])
            shutil.move(self.tmps['tmp'], rnaplex_file)
            shutil.rmtree(rnaplfold_folder)

    def _run_rnaup(self, num_up, processes, prefix, out_rnaup, out_log, args_tar, log):
        for index in range(1, num_up + 1):
            out_tmp_up = open(os.path.join(args_tar.out_folder, ''.join([self.tmps['rnaup'],
             str(index), '.txt'])), 'w')
            out_err = open(os.path.join(args_tar.out_folder, ''.join([self.tmps['log'],
             str(index), '.txt'])), 'w')
            in_up = open(os.path.join(args_tar.out_folder, ''.join([self.tmps['tmp'],
             str(index), '.fa'])), 'r')
            ...
            p = Popen([args_tar.rnaup_path,
             '-u', str(args_tar.unstr_region_rnaup),
             '-o', '--interaction_first'], stdin=in_up, stdout=out_tmp_up, stderr=out_err)
            processes.append(p)
        ...
        if len(processes) != 0:
            ...
            os.system('rm ' + os.path.join(args_tar.out_folder, self.tmps['all_fa']))
            self._merge_txt(num_up, out_rnaup, out_log, args_tar.out_folder)
            os.system('rm ' + os.path.join(args_tar.out_folder, self.tmps['all_txt']))

    def _merge_txt(self, num_up, out_rnaup, out_log, out_folder):
        for index in range(1, num_up + 1):
            self.helper.merge_file(os.path.join(out_folder, ''.join([self.tmps['rnaup'],
             str(index), '.txt'])), out_rnaup)
            self.helper.merge_file(os.path.join(out_folder, ''.join([self.tmps['log'],
    ...
    def _rnaup(self, prefixs, args_tar, log):
        ...
        for prefix in prefixs:
            ...
            num_up = 0
            processes = []
            out_rnaup = os.path.join(self.rnaup_path, prefix, '_'.join([prefix + '_RNAup.txt']))
            out_log = os.path.join(self.rnaup_path, prefix, '_'.join([prefix + '_RNAup.log']))
            if '_'.join([prefix, 'RNAup.txt']) in os.listdir(os.path.join(self.rnaup_path, prefix)):
                    ...
                    for line in s_f:
                        ...
                        if line.startswith('>'):
                            if line[1:] in srnas:
                                ...
                                num_up += 1
                                out_up = open(os.path.join(args_tar.out_folder, ''.join([self.tmps['tmp'],
                                 str(num_up), '.fa'])), 'w')
                                out_up.write(line + '\n')
                        elif start:
                            out_up.write(line + '\n')
                            out_up.close()
                            self.helper.merge_file(os.path.join(self.target_seq_path, '_'.join([prefix, 'target.fa'])), os.path.join(args_tar.out_folder, ''.join([self.tmps['tmp'],
                             str(num_up), '.fa'])))
                            if num_up == args_tar.core_up:
                                self._run_rnaup(num_up, processes, prefix, out_rnaup, out_log, args_tar, log)
                                processes = []
                                num_up = 0
    ...
    def _intarna(self, prefixs, args_tar, log):
        ...
        for prefix in prefixs:
            ...
            intarna_file = os.path.join(self.intarna_path, prefix, prefix + '_IntaRNA.txt')
            self.helper.check_make_folder(os.path.join(self.intarna_path, prefix))
    ...
    def _merge_rnaplex_rnaup(self, prefixs, args_tar, log):
        ...
        log.write('Running merge_rnaplex_rnaup.py to merge the results from RNAplex, RNAup, and IntaRNA for generating finanl output.\n')
        ...
        for prefix in prefixs:
            ...
            intarna_file = None
            out_intarna = None
            self.helper.check_make_folder(os.path.join(self.merge_path, prefix))
            print('Ranking {0} now'.format(prefix))
            if 'RNAplex' in args_tar.program:
                rnaplex_file = os.path.join(self.rnaplex_path, prefix, '_'.join([prefix, 'RNAplex.txt']))
                out_rnaplex = os.path.join(self.rnaplex_path, prefix, '_'.join([prefix, 'RNAplex_rank.csv']))
                self._remove_repeat(rnaplex_file, 'RNAplex')
            if 'RNAup' in args_tar.program:
                rnaup_file = os.path.join(self.rnaup_path, prefix, '_'.join([prefix, 'RNAup.txt']))
                out_rnaup = os.path.join(self.rnaup_path, prefix, '_'.join([prefix, 'RNAup_rank.csv']))
                self._remove_repeat(rnaup_file, 'RNAup')
            if 'IntaRNA' in args_tar.program:
                intarna_file = os.path.join(self.intarna_path, prefix, '_'.join([prefix, 'IntaRNA.txt']))
                out_intarna = os.path.join(self.intarna_path, prefix, '_'.join([prefix, 'IntaRNA_rank.csv']))
                self._remove_repeat(intarna_file, 'IntaRNA')
            overlap_file = os.path.join(self.merge_path, prefix, '_'.join([prefix, 'overlap.csv']))
            merge_file = os.path.join(self.merge_path, prefix, '_'.join([prefix, 'merge.csv']))
            merge_srna_target(rnaplex_file, rnaup_file, intarna_file, args_tar, out_rnaplex, out_rnaup, out_intarna, os.path.join(self.fasta_path, prefix + '.fa'), merge_file, overlap_file, os.path.join(self.srna_path, '_'.join([prefix, 'sRNA.gff'])), os.path.join(self.gff_path, prefix + '.gff'))
            if 'RNAplex' in args_tar.program:
    ...
    def _remove_rnaplex(self, line, num, pre_num, pre, checks, out_tmp, print_):
        if line.startswith('>'):
            if num % 2 == 1:
                print_ = False
                pre = line
            ...
            pre_num = num
        return (
         num, pre_num, print_, pre)

    def _remove_rnaup(self, line, pre, num, pre_num, srna_info, checks, out_tmp, print_, tar):
        if line.startswith('>'):
            print_ = False
            tar = False
            if pre.startswith('>'):
                if pre not in checks:
                    ...
                    srna_info = pre
                    print_ = True
                elif line not in checks[pre]:
                    checks[pre].append(line)
                    print_ = True
            elif num != 1 and line not in checks[srna_info]:
    ...
    def _remove_repeat(self, interact_file, type_):
        ...
        with open(interact_file) as (fh):
            for line in fh:
                line = line.strip()
                if type_ == 'RNAplex':
                    num, pre_num, print_, pre = self._remove_rnaplex(line, num, pre_num, pre, checks, out_tmp, print_)
                else:
                    if type_ == 'RNAup':
                        num, pre_num, print_, pre, tar, srna_info = self._remove_rnaup(line, pre, num, pre_num, srna_info, checks, out_tmp, print_, tar)
                    elif type_ == 'IntaRNA':
                        tar, srna_info, seq = self._remove_intarna(line, checks, tar, srna_info, seq, out_tmp)
    ...
    def run_srna_target_prediction(self, args_tar, log):
        ...
        self.helper.remove_all_content(args_tar.out_folder, self.tmps['tmp'], 'dir')
        self.helper.remove_all_content(args_tar.out_folder, self.tmps['tmp'], 'file')
