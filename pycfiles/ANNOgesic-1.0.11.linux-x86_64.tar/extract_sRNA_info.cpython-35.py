...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/extract_sRNA_info.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 12864 bytes
...
def get_proteins(datas, checks, blast_f, score_n):
    ...
    for data in datas:
        if nums['index'] % 4 == 0 and nums['index'] != 0:
            ...
            if 'hypothetical' in name or 'Hypothetical' in name or 'unknown' in name or 'Unknown' in name or 'predicted coding region' in name or 'Predicted coding region' in name or 'PREDICTED:' in name or 'putative' in name or 'Putative' in name:
                nums['hypo'] += 1
            if not checks['detect']:
                for line in blast_f:
                    line = line.strip()
                    if 'Expect' in line and 'Score' in line and 'Method' in line:
                        ...
                        if score_n is None:
                            ...
                            if score_n is not None and float(score) >= score_n:
                                checks['detect'] = True
                                checks['print'] = True
                                proteins.append({'name': name, 'strain': strain, 
                                 'e': e_value, 'tag': tag, 
...
def detect_hypo(proteins, blasts, type_):
    ...
    for protein in proteins:
        ...
        if 'hypothetical' not in name and 'Hypothetical' not in name and 'Unknown' not in name and 'unknown' not in name and 'Predicted coding region' not in name and 'predicted coding region' not in name and 'PREDICTED:' not in name and 'putative' not in name and 'Putative' not in name:
    ...
    return (
     protein_names, protein['e'], protein['score'])
        ...
        for line in blast_f:
            ...
            if 'Expect =' in line:
                ...
                if score_s is None:
                    ...
                    if score_s is not None and float(score) >= score_s:
                        print_ = True
                    break

    if print_:
        blasts['name'] = blasts['name'].lstrip().replace('\n', '')
        out_t.write('{0}\t{1}\t{2}\t{3}\n'.format(prefix, blasts['name'], e_value, score))
        blasts['blast'] = True
    if print_:
        return 2
    else:
...
def read_gff(srna_file, data_type):
    ...
    for entry in Gff3Parser().entries(srna_f):
        ...
        for key, value in entry.attributes.items():
            if data_type == 'sRNA' and 'sRNA_hit' not in key:
                attributes[key] = value
            elif data_type == 'nr' and 'nr_hit' not in key:
                attributes[key] = value

        entry.attributes = attributes
        attribute_string = ';'.join(['='.join(items) for items in entry.attributes.items()])
...
def gen_out_flie(blasts, out_t, prefix, out_f, database, srna, names):
    if not blasts['blast']:
        ...
        print_file(database, out_f, srna.info, len(names), blasts['hit_num'])
...
def extract_blast(blast_result, srna_file, output_file, output_table, database, score_s, score_n):
                            ...
                            if line.find('No hits found') != -1:
                                ...
                                for line in blast_f:
                                    ...
                                    if line.startswith('Effective search space'):
                                        ...
                                        break
                                    if database == 'sRNA':
                                        p = detect_srna(line, blast_f, out_t, blasts, prefix, score_s)
                                        if p:
                                            print_ = p
                                        if len(blasts['name']) > 0 and blasts['name'] not in names:
                                            names.append(blasts['name'])
                                    elif database == 'nr':
                                        p = detect_nr(line, blast_f, out_t, blasts, prefix, score_n)
                                        if p:
                                            print_ = p

                                gen_out_flie(blasts, out_t, prefix, out_f, database, srna, names)
                                blasts['hit_num'] = 0
                                break
        ...
        with open(sec_file, 'r') as (d_f):
            for structure in d_f:
                ...
                if get_length:
                    ...
                    get_length = False
                if structure.startswith('>') and ('|'.join([srna.attributes['ID'], srna.seq_id,
                 str(srna.start),
                 str(srna.end), srna.strand]) == structure[1:] or '|'.join([srna.feature, srna.seq_id,
                 str(srna.start),
                 str(srna.end), srna.strand]) == structure[1:]):
                    check = True
                    get_length = True
                if check and (structure[0] == '(' or structure[0] == ')' or structure[0] == '.') and structure[(-1)] == ')':
                    check = False
