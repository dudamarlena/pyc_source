...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/helper.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 15341 bytes
import os, sys, copy, shutil, csv, re
from Bio.Seq import Seq
from Bio.Alphabet import generic_dna
from annogesiclib.gff3 import Gff3Parser

class Helper(object):
    __doc__ = 'For some small and regular modules for ANNOgesic'

    def __init__(self):
        self.gff3parser = Gff3Parser()

    def feature_without_notgene(self, entry):
        if entry.feature != 'gene' and entry.feature != 'exon' and entry.feature != 'source' and entry.feature != 'region' and entry.feature != 'repeat_region' and entry.feature != 'transcript' and entry.feature != 'STS' and entry.feature != 'remark':
            utr_markers = "[^\\'\\ \\-35]"
            ...
            for line in fh:
                ...
                if prefix == '>':
                    if prefix in line and first:
                        first = False
                    elif prefix in line and not first and not line.startswith(prefix):
    ...
    def move_all_content(self, ref_folder, tar_folder, features):
        ...
        for file_ in os.listdir(ref_folder):
            ...
            if features is not None:
                ...
                move = True
            if move:
                shutil.move(os.path.join(ref_folder, file_), os.path.join(tar_folder, file_))
        ...
        if folder is not None and os.path.isdir(folder):
            for file_ in os.listdir(ori_folder):
                for type_ in types:
                    ...
                    if file_.endswith(type_) and os.path.isfile(ori_file):
    ...
    def check_uni_attributes(self, gff_file):
        ...
        for entry in self.gff3parser.entries(fh):
            if entry.feature == 'source' or entry.feature == 'region' or entry.feature == 'remark':
                if entry.seq_id not in lens.keys():
                    lens[entry.seq_id] = entry.end
                else:
                    if entry.end > lens[entry.seq_id]:
                        lens[entry.seq_id] = entry.end
                    print('Warning: Detect repeated source/region/remark of {0}! The longest end point is used as the length of the genome.'.format(entry.seq_id))
                gffs.append(entry)
        ...
        pre_gff = None
        for gff in gffs:
            if gff.feature != 'source' and gff.feature != 'region' and gff.feature != 'remark' and gff.seq_id in lens.keys() and gff.end > lens[gff.seq_id]:
            ...
            elif gff.seq_id == pre_gff.seq_id:
                pass
            if 'ID' in gff.attributes.keys():
                if gff.attributes['ID'] in ids:
                    print('Warninng: Repeat ID {0} in gff file!!!'.format(gff.attributes['ID']))
                else:
                    self._add_element(ids, 'ID', gff)
                if 'locus_tag' in gff.attributes.keys():
                    if gff.attributes['locus_tag'] in ids:
                        print('Warning: Repeat locus_tag {0} in gff file!!!'.format(gff.attributes['locus_tag']))
                    else:
                        self._add_element(locus_tags, 'locus_tag', gff)
