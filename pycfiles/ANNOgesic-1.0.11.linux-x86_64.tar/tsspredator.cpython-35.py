...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/tsspredator.py
# Compiled at: 2020-01-14 08:25:30
# Size of source mod 2**32: 31783 bytes
...
class TSSpredator(object):
    ...
    def __init__(self, args_tss):
        ...
        self.tmps = {'tss': 'tmp_TSS', 'ta_tss': 'tmp_ta_tss', 'tss_ta': 'tmp_tss', 
         'tmp': 'tmp'}
    ...
    def _print_lib(self, lib_num, lib_list, out, wig_folder, prefix, rep_set):
        for num_id in range(1, lib_num + 1):
            ...
            for cond in cond_sort_list:
                out.write('{0}_{1}{2} = {3}\n'.format(prefix, cond['condition'], cond['replicate'], os.path.join(wig_folder, cond['wig'])))
    ...
    def _import_lib(self, libs, wig_folder, project_strain_name, out, gff, program, fasta):
        ...
        for lib in libs:
            ...
            for wig in os.listdir(wig_folder):
                ...
                if filename[0] == lib_datas[0][:-4] and filename[1][:-4] == project_strain_name:
                    lib_datas[0] = wig
        ...
        if program.lower() == 'tss':
            self._print_lib(lib_num, lib_dict['fm'], out, wig_folder, 'fivePrimeMinus', rep_set)
            self._print_lib(lib_num, lib_dict['fp'], out, wig_folder, 'fivePrimePlus', rep_set)
        else:
            if program.lower() == 'ps':
                self._print_lib(lib_num, lib_dict['nm'], out, wig_folder, 'fivePrimeMinus', rep_set)
                self._print_lib(lib_num, lib_dict['np'], out, wig_folder, 'fivePrimePlus', rep_set)
            else:
                print('Error: Wrong program name! Please assing tss or processing_site.')
                sys.exit()
        for num_id in range(1, lib_num + 1):
        ...
        for best_file in os.listdir(args_tss.auto_load):
            if best_file == '_'.join(['best', prefix + '.csv']):
                ...
                if len(lines[(len(lines) - 1)].split('\t')) < 8:
                    print('Error: some information in {0} is missing. It may be due to that "optimize_tss_ps" did not finish successfully.'.format(best_file))
                    ...
                    detect_all = all(elem in para_info for elem in ['he', 'rh', 'fa', 'rf',
                     'bh', 'ef', 'pf'])
                    if not detect_all or len(para_info) != 14:
                        print('Error: {0} is complete. Some parameters are missing!'.format(best_file))
                        log.write('Error: {0} is complete. Some parameters are missing!\n'.format(best_file))
                        sys.exit()
                    else:
                        detect = True
                        height = para_info[(para_info.index('he') + 1)]
                        height_reduction = para_info[(para_info.index('rh') + 1)]
                        factor = para_info[(para_info.index('fa') + 1)]
                        factor_reduction = para_info[(para_info.index('rf') + 1)]
                        base_height = para_info[(para_info.index('bh') + 1)]
                        enrichment_factor = para_info[(para_info.index('ef') + 1)]
                        processing_factor = para_info[(para_info.index('pf') + 1)]

        if detect:
            return (height, height_reduction, factor, factor_reduction,
             base_height, enrichment_factor, processing_factor)
        print('Error: No best_{0}.csv can be found in {1}! '.format(prefix, args_tss.auto_load))
        log.write('Error: No best_{0}.csv can be found in {1}\n'.format(prefix, args_tss.auto_load))
        sys.exit()
    ...
    def _get_input_para(self, args_tss, prefix, log):
        if args_tss.genome_order is None:
            ...
            if prefix not in args_tss.genome_order:
                print('Error: the parameters for {0} were not assigned!'.format(prefix))
    ...
    def _gen_config(self, project_strain_name, args_tss, gff, wig_folder, fasta, config_file, log):
        """generation of config files"""
        log.write('Generating config files for TSSpredator.\n')
        if args_tss.auto_load is not None:
            height, height_reduction, factor, factor_reduction, base_height, enrichment_factor, processing_factor = self._extract_best_para(args_tss, project_strain_name, log)
        else:
            height, height_reduction, factor, factor_reduction, base_height, enrichment_factor, processing_factor = self._get_input_para(args_tss, project_strain_name, log)
        ...
        lib_num, num_id, rep_set, lib_dict, list_num_id = self._import_lib(args_tss.libs, wig_folder, project_strain_name, out, gff, args_tss.program, fasta)
        out.write('idList = ')
        ...
        if args_tss.program.lower() == 'tss':
            self._print_lib(lib_num, lib_dict['nm'], out, wig_folder, 'normalMinus', rep_set)
            self._print_lib(lib_num, lib_dict['np'], out, wig_folder, 'normalPlus', rep_set)
        else:
            self._print_lib(lib_num, lib_dict['fm'], out, wig_folder, 'normalMinus', rep_set)
            self._print_lib(lib_num, lib_dict['fp'], out, wig_folder, 'normalPlus', rep_set)
        ...
        for prefix_id in range(len(args_tss.output_prefixs)):
            out.write('outputPrefix_{0} = {1}\n'.format(prefix_id + 1, args_tss.output_prefixs[prefix_id]))
        ...
        for prefix in prefixs:
            ...
            if 'MasterTable.tsv' not in os.listdir(out_path):
                ...
                if args_tss.program.lower() == 'processing':
                    ...
                    if args_tss.program.lower() == 'tss':
                        feature = 'TSS'
                    self.converter.convert_mastertable2gff(os.path.join(out_path, 'MasterTable.tsv'), 'ANNOgesic', feature, prefix, out_file)
    ...
    def _merge_manual(self, tsss, args_tss):
        ...
        self.helper.check_make_folder(os.path.join(os.getcwd(), self.tmps['tss']))
        for tss in tsss:
            ...
            if os.path.exists(manual):
                print('Merging and classiflying manually-detected TSSs for {0}'.format(tss))
                merge_manual_predict_tss(predict, stat_file, os.path.join(self.tmps['tss'], filename), os.path.join(args_tss.gffs, gff), args_tss, manual, fasta)
            if os.path.exists(stat_file):
                shutil.move(stat_file, os.path.join(args_tss.out_folder, 'statistics', tss, stat_file))

        self.helper.move_all_content(self.tmps['tss'], self.gff_outfolder, ['.gff'])
    ...
    def _validate(self, tsss, args_tss, log):
        ...
        log.write('Running validate_gene.py to compare genome annotations and TSSs/PSs.\n')
        for tss in tsss:
            for gff in os.listdir(args_tss.gffs):
                if gff[:-4] == tss and '.gff' in gff:
                    break

            stat_file = os.path.join(self.stat_outfolder, tss, ''.join(['stat_gene_vali_', tss, '.csv']))
            ...
            if args_tss.program.lower() == 'tss':
                ...
                compare_file = os.path.join(self.gff_outfolder, '_'.join([tss, 'processing.gff']))
            validate_gff(compare_file, os.path.join(args_tss.gffs, gff), stat_file, out_cds_file, args_tss.utr_length, args_tss.program.lower())
    ...
    def _compare_ta(self, tsss, args_tss, log):
        ...
        detect = False
        log.write('Running stat_TA_comparison to compare transcripts and TSSs/PSs.\n')
        print('Comparing transcripts and TSSs')
        self.multiparser.parser_gff(args_tss.ta_files, 'transcript')
        self.multiparser.combine_gff(args_tss.gffs, self.tmps['ta'], None, 'transcript')
        for tss in tsss:
            stat_out = os.path.join(self.stat_outfolder, tss, ''.join([
            ...
            compare_file = os.path.join(self.gff_outfolder, '_'.join([tss, 'TSS.gff']))
            if detect:
                stat_ta_tss(os.path.join(self.tmps['ta'], ta), compare_file, stat_out, self.tmps['ta_tss'], self.tmps['tss_ta'], args_tss.fuzzy)
                self.helper.sort_gff(self.tmps['tss_ta'], compare_file)
                self.helper.sort_gff(self.tmps['ta_tss'], os.path.join(args_tss.ta_files, ta))
    ...
    def _stat_tss(self, tsss, feature, log):
        ...
        for tss in tsss:
            compare_file = os.path.join(self.gff_outfolder, '_'.join([tss, feature]) + '.gff')
            stat_tsspredator(compare_file, feature, os.path.join(self.stat_outfolder, tss, '_'.join([
             'stat', feature, 'class', tss]) + '.csv'), os.path.join(self.stat_outfolder, tss, '_'.join([
             'stat', feature, 'libs', tss]) + '.csv'))
            self.helper.move_all_content(os.getcwd(), os.path.join(self.stat_outfolder, tss), ['_class', '.png'])
            if os.path.exists(os.path.join(self.stat_outfolder, 'TSSstatistics.tsv')):
                shutil.move(os.path.join(self.stat_outfolder, 'TSSstatistics.tsv'), os.path.join(self.stat_outfolder, tss, 'TSSstatistics.tsv'))
            plot_venn(compare_file, feature)
            self.helper.move_all_content(os.getcwd(), os.path.join(self.stat_outfolder, tss), ['_venn', '.png'])
            log.write('The following files in {0} are generated:\n'.format(os.path.join(self.stat_outfolder, tss)))
            for file_ in os.listdir(os.path.join(self.stat_outfolder, tss)):
                log.write('\t' + file_ + '\n')
    ...
    def _merge_wigs(self, wig_folder, prefix, libs):
        self.helper.check_make_folder(os.path.join(os.getcwd(), self.tmps['tmp']))
        for wig_file in os.listdir(wig_folder):
            for lib in libs:
                info = lib.split(':')
                if info[0][:-4] in wig_file and info[(-1)] == '+' and prefix in wig_file and os.path.isfile(os.path.join(wig_folder, wig_file)):
                    Helper().merge_file(os.path.join(wig_folder, wig_file), os.path.join('tmp', 'merge_forward.wig'))
                if info[0][:-4] in wig_file and info[(-1)] == '-' and prefix in wig_file and os.path.isfile(os.path.join(wig_folder, wig_file)):
                    Helper().merge_file(os.path.join(wig_folder, wig_file), os.path.join('tmp', 'merge_reverse.wig'))
    ...
    def _check_orphan(self, prefixs, wig_folder, args_tss):
        ...
        for prefix in prefixs:
            ...
            pre_tss = os.path.join(self.gff_outfolder, '_'.join([
             prefix, args_tss.program + '.gff']))
            check_orphan(pre_tss, os.path.join(args_tss.gffs, prefix + '.gff'), 'tmp/merge_forward.wig', 'tmp/merge_reverse.wig', tmp_tss)
            ...
            if args_tss.program.lower() == 'tss':
                for tss in os.listdir(out_folder):
                    if tss.endswith('_TSS.gff'):
                        ref = self.helper.get_correct_file(args_tss.overlap_gffs, '_processing.gff', tss.replace('_TSS.gff', ''), None, None)
                        filter_tss_pro(os.path.join(out_folder, tss), ref, args_tss.program, args_tss.cluster)

            elif args_tss.program.lower() == 'processing':
                for tss in os.listdir(out_folder):
                    if tss.endswith('_processing.gff'):
                        ref = self.helper.get_correct_file(args_tss.overlap_gffs, '_TSS.gff', tss.replace('_processing.gff', ''), None, None)
                        filter_tss_pro(os.path.join(out_folder, tss), ref, args_tss.program, args_tss.cluster)
    ...
    def _low_expression(self, args_tss, gff_folder):
        ...
        for gff in os.listdir(gff_folder):
            if args_tss.program.lower() == 'tss' and gff.endswith('_TSS.gff'):
                prefix = gff.replace('_TSS.gff', '')
            elif args_tss.program.lower() == 'processing' and gff.endswith('_processing.gff'):
                prefix = gff.replace('_processing.gff', '')
            if prefix:
                out = open(os.path.join(self.stat_outfolder, prefix, '_'.join([
                 'stat', prefix, 'low_expression_cutoff.csv'])), 'w')
                out.write('\t'.join(['Genome', 'Cutoff_coverage']) + '\n')
                cutoff = filter_low_expression(os.path.join(gff_folder, gff), args_tss, 'tmp/merge_forward.wig', 'tmp/merge_reverse.wig', 'tmp/without_low_expression.gff')
    ...
    def run_tsspredator(self, args_tss, log):
        ...
        for gff in os.listdir(args_tss.gffs):
            if gff.endswith('.gff'):
                self.helper.check_uni_attributes(os.path.join(args_tss.gffs, gff))
        ...
        for prefix in prefixs:
            ...
            self._gen_config(prefix, args_tss, os.path.join(self.gff_path, prefix + '.gff'), self.wig_path, os.path.join(self.fasta_path, prefix + '.fa'), config, log)
            ...
            self._start_to_run(args_tss.tsspredator_path, config_file, out_path, prefix, log)
            if os.path.exists(os.path.join(out_path, 'TSSstatistics.tsv')):
                shutil.move(os.path.join(out_path, 'TSSstatistics.tsv'), os.path.join(self.stat_outfolder, 'TSSstatistics.tsv'))
        ...
        if args_tss.check_orphan:
            ...
            self._check_orphan(prefixs, os.path.join(args_tss.wig_folder, 'tmp'), args_tss)
        self.multiparser.combine_gff(args_tss.gffs, self.gff_outfolder, None, args_tss.program)
        datas = []
        ...
        if args_tss.remove_low_expression is not None:
            log.write('Running filter_low_expression.py to filter out low expressed TSS/PS.\n')
            self._low_expression(args_tss, self.gff_outfolder)
        if args_tss.manual is not None:
            self.multiparser.parser_gff(args_tss.manual, None)
            self.multiparser.combine_gff(args_tss.gffs, self.manual_path, None, None)
            self.multiparser.combine_fasta(args_tss.gffs, self.fasta_path, None)
            self.multiparser.combine_wig(args_tss.gffs, self.wig_path, None, args_tss.libs)
            log.write('Running merge_manual.py to merge the manual TSSs.\n')
            self._merge_manual(datas, args_tss)
        log.write('Running filter_TSS_pro.py to deal with the overlap position between TSS and PS.\n')
