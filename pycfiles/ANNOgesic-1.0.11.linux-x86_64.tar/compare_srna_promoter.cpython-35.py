...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/compare_srna_promoter.py
...
def read_file(gff_file, args_srna):
    ...
    for row in csv.reader(fh, delimiter='\t'):
        if row[0] != 'Genome' and row[3] in args_srna.promoter_name:
...
def compare_srna_promoter(srna_gff, srna_table, args_srna):
    ...
    for srna in srnas:
        tsss = []
        detect = False
        if 'with_TSS' in srna.attributes.keys() and srna.attributes['with_TSS'] != 'NA':
        ...
        if len(tsss) != 0:
            for tss in tsss:
                for pro in pros:
                    if srna.seq_id == pro['strain'] and tss['strand'] == pro['strand'] and tss['pos'] == pro['pos']:
                        detect = True
        ...
        if detect:
            out_g.write(srna.info + ';promoter=' + srna.attributes['promoter'] + '\n')
        else:
