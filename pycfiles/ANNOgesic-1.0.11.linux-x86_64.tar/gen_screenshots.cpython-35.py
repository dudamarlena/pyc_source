...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gen_screenshots.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 5607 bytes
import os, sys
...
def load_wigs(out, lib_t, lib_n, lib_f):
    if lib_t and lib_n:
        for index in range(len(lib_t)):
            out.write('load {0}\n'.format(os.path.join(os.getcwd(), lib_t[index])))
            out.write('load {0}\n'.format(os.path.join(os.getcwd(), lib_n[index])))
...
def print_batch(args_sc, out, strand, lib_t, lib_n, lib_f, strain):
    ...
    if strand == '+':
        out.write('snapshotDirectory {0}\n'.format(os.path.join(os.getcwd(), args_sc.output_folder, strain, 'forward')))
    else:
        out.write('snapshotDirectory {0}\n'.format(os.path.join(os.getcwd(), args_sc.output_folder, strain, 'reverse')))
...
def gen_batch(lib_t, lib_n, lib_f, strand, gffs, out, seq):
    ...
    for gff in gffs:
        if gff.seq_id not in seq.keys():
            print('Error: The genome names in fasta file and gff file are different!!')
            sys.exit()
        if gff.start - 200 <= 0:
            start = 1
        else:
...
def gen_screenshot(args_sc, libs, forward_file, reverse_file, strain):
    ...
    print_batch(args_sc, out_f, '+', libs['ft'], libs['fn'], libs['ff'], strain)
    out_r = open(reverse_file, 'w')
    print_batch(args_sc, out_r, '-', libs['rt'], libs['rn'], libs['rf'], strain)
