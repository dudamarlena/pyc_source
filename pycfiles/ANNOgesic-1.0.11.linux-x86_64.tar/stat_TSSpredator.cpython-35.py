...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_TSSpredator.py
...
def stat(tsss, strain, feature_name, out_stat, file_type, out_lib):
    ...
    plot(len(tss_type['Primary']), len(tss_type['Secondary']), len(tss_type['Antisense']), len(tss_type['Internal']), len(tss_type['Orphan']), num_tss, num_tss_more, strain, feature_name, file_type)
    ...
    out_stat.write('total number of {0} (if one {1} belongs to two classes, it count two times) = {2}\n'.format(feature_name, feature_name, num_tss_more))
    out_stat.write('total number of unique {0} (if one {1} belongs to two classes, it count only one time) = {2}\n'.format(feature_name, feature_name, num_tss))
    for it in range(1, 5):
        for tss in itertools.combinations(tss_type.keys(), it):
            ...
            out_stat.write('{0} = {1} ({2})\n'.format('-'.join(tss), len(union), float(len(union)) / float(num_tss)))
