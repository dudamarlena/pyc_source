...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/meme.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 21213 bytes
...
class MEME(object):
    ...
    def _run_normal_motif(self, input_path, out_path, filename, fasta, width, args_pro, log):
        """run MEME with specific width"""
        folder = '_'.join(['promoter_motifs', filename,
         str(width), 'nt'])
        if args_pro.program.lower() == 'meme' or args_pro.program.lower() == 'both':
    ...
    def _run_small_motif(self, input_path, out_path, filename, fasta, width, args_pro, log):
    ...
    def _get_fasta_file(self, fasta_path, prefix):
        for fasta in os.listdir(fasta_path):
            if fasta.endswith('.fa') and prefix == fasta.replace('.fa', ''):
                break
            else:
                if fasta.endswith('.fna') and prefix == fasta.replace('.fna', ''):
                    break
                elif fasta.endswith('.fasta') and prefix == fasta.replace('.fasta', ''):
                    break
    ...
    def _split_fasta_by_strain(self, input_path):
        ...
        for fasta in os.listdir(input_path):
            if fasta.endswith('.fa'):
                    ...
                    for line in f_h:
                        ...
                        if line.startswith('>'):
                            ...
                            if pre_strain != strain:
                                ...
                                if out is not None:
                                    out.close()
                                out = open(os.path.join(input_path, ''.join([
                                 filename[0], strain,
                                 filename[(-1)]])), 'a')
                                pre_strain = strain
                            out.write(line + '\n')
                ...
                if num_strain <= 1:
                    os.remove(os.path.join(input_path, ''.join([filename[0], strain, filename[(-1)]])))
    ...
    def _run_program(self, prefixs, args_pro, log, input_fastas):
        ...
        for prefix in prefixs:
            ...
            if args_pro.program.lower() == 'both':
                ...
                if args_pro.program.lower() == 'meme':
                    self.helper.check_make_folder(os.path.join(out_path, 'MEME'))
            ...
            for fasta in os.listdir(input_path):
                ...
                if names[(-1)] in input_fastas or '_'.join(names[-2:]) == 'all_types' and 'all_types' in input_fastas or '_'.join(names[-2:]) == 'without_orphan' and 'without_orphan' in input_fastas:
                    for width in args_pro.widths:
                        ...
                        if '-' in width:
                            self._run_small_motif(input_path, out_path, filename, fasta, width, args_pro, log)
                        else:
                            self._run_normal_motif(input_path, out_path, filename, fasta, width, args_pro, log)

            log.write('Promoter search for {0} is done.\n'.format(prefix))
            log.write('All the output files from MEME or GLAM2 are generated and stored in {0}.\n'.format(out_path))
    ...
    def _combine_file(self, prefixs, args_pro):
        ...
        if args_pro.source:
            ...
            for fasta in os.listdir(args_pro.fastas):
                if fasta.endswith('.fa') or fasta.endswith('.fna') or fasta.endswith('.fasta'):
                    self.helper.merge_file(os.path.join(args_pro.fastas, fasta), self.all_fasta)

        else:
            for tss in os.listdir(os.path.join(args_pro.output_folder, 'TSS_classes')):
                if tss.endswith('_TSS.gff'):
                    self.helper.merge_file(os.path.join(self.tss_path, tss), self.all_tss)
        ...
        self.helper.check_make_folder(os.path.join(args_pro.output_folder, 'allfasta'))
    ...
    def _gen_table(self, output_folder, prefixs, combine, program, log):
        ...
        log.write('Running gen_promoter_table.py to generate promoter table which is useful for sRNA prediction.\n')
        ...
        for strain in strains:
            ...
            if program.lower() == 'both' or program.lower() == 'meme':
                for folder in os.listdir(os.path.join(output_folder, strain, 'MEME')):
                    csv_file = os.path.join(output_folder, strain, 'MEME', folder, 'meme.csv')
                    gen_promoter_table(os.path.join(output_folder, strain, 'MEME', folder, 'meme.txt'), csv_file, tss_file, 'meme')
                    log.write('\t' + csv_file + '\n')

            if program.lower() == 'both' or program.lower() == 'glam2':
                for folder in os.listdir(os.path.join(output_folder, strain, 'GLAM2')):
                    csv_file = os.path.join(output_folder, strain, 'GLAM2', folder, 'glam2.csv')
                    gen_promoter_table(os.path.join(output_folder, strain, 'GLAM2', folder, 'glam2.txt'), csv_file, tss_file, 'glam2')
                    log.write('\t' + csv_file + '\n')
    ...
    def _get_upstream(self, args_pro, prefix, tss, fasta):
        ...
        if args_pro.source:
            ...
            upstream(os.path.join(self.tss_path, tss), os.path.join(args_pro.fastas, fasta), None, None, args_pro, prefix)
        else:
            ...
            upstream(os.path.join(self.tss_path, tss), os.path.join(args_pro.fastas, fasta), os.path.join(self.gff_path, prefix + '.gff'), os.path.join(args_pro.output_folder, 'TSS_classes', '_'.join([prefix, 'TSS.gff'])), args_pro, prefix)
    ...
    def run_meme(self, args_pro, log):
        if 'allfasta.fa' in os.listdir(args_pro.fastas):
            ...
            if 'allfasta.fa_folder' in os.listdir(args_pro.fastas):
                shutil.rmtree(os.path.join(args_pro.fastas, 'allfasta.fa_folder'))
            ...
            if args_pro.gffs is not None:
                ...
                self.multiparser.combine_gff(args_pro.fastas, self.gff_path, 'fasta', None)
            self._check_gff(args_pro.tsss)
            self.multiparser.combine_gff(args_pro.fastas, self.tss_path, 'fasta', 'TSS')
            self.helper.check_make_folder(self.out_fasta)
            self.helper.check_make_folder(self.tmp_folder)
            prefixs = []
            log.write('Running .TSS_upstream.py to extract the upstream sequences of TSSs.\n')
            log.write('The following files are generated:\n')
            for tss in os.listdir(self.tss_path):
                prefix = tss.replace('_TSS.gff', '')
                prefixs.append(prefix)
                self.helper.check_make_folder(os.path.join(args_pro.output_folder, prefix))
            ...
            if args_pro.combine:
                ...
                for file_ in os.listdir(os.path.join(self.out_fasta, 'allfasta')):
                    log.write('\t' + os.path.join(self.out_fasta, 'allfasta', file_) + '\n')
            ...
            self._gen_table(args_pro.output_folder, prefixs, args_pro.combine, args_pro.program, log)
