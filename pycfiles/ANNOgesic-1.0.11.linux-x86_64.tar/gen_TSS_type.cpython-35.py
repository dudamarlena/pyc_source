...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gen_TSS_type.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 15786 bytes
import math, copy
    ...
    if is_primary(gene.start, gene.end, tss.start, tss.strand):
        ...
        if tss.strand == '+':
            if anti_ends['reverse'] != -1 and anti_ends['reverse'] - gene.start > 0 or anti_ends['reverse'] == -1:
                tss_entry = import_to_tss('Primary', gene.start, tss, locus_tag, ori_entry)
                checks['orphan'] = False
                gene_ends['forward'] = gene.start
            elif anti_ends['reverse'] != -1 and anti_ends['reverse'] - gene.start < 0 and (checks['int_anti'] or tss.start - anti_ends['reverse'] > 0):
                tss_entry = import_to_tss('Primary', gene.start, tss, locus_tag, ori_entry)
                checks['orphan'] = False
                gene_ends['forward'] = gene.start
        elif anti_ends['forward'] != -1 and gene.end - anti_ends['forward'] > 0 or anti_ends['forward'] == -1:
            tss_entry = import_to_tss('Primary', gene.end, tss, locus_tag, ori_entry)
            checks['orphan'] = False
...
def diff_strand_tss_gene(gene, tss, anti_ends, gene_ends, checks, tss_entry):
    ...
    if is_antisense(gene.start, gene.end, tss.start, tss.strand):
        ...
        if tss.strand == '-':
            ...
            if gene_ends['reverse'] != -1 and gene.start - gene_ends['reverse'] > 0 and is_internal(gene.start, gene.end, tss.start, tss.strand):
                pass
        else:
...
def compare_tss_cds(tss, cdss, genes):
    ...
    for data in datas:
        ...
        if data.strand == tss.strand:
            tss_entry = same_strand_tss_gene(data, tss, anti_ends, gene_ends, checks, ori_entry)
        else:
            tss_entry = diff_strand_tss_gene(data, tss, anti_ends, gene_ends, checks, ori_entry)
        ...
        if strain == tss.seq_id:
            ...
            for cond, tracks in conds.items():
                for lib_name, covers in tracks.items():
                    if tss.start + 1 <= len(covers) and ref.start + 1 <= len(covers):
                        if tss.strand == '+':
                            diff_t = covers[(tss.start - 1)] - covers[(tss.start - 2)]
                            diff_r = covers[(ref.start - 1)] - covers[(ref.start - 2)]
                        else:
                            diff_t = covers[(tss.start - 1)] - covers[tss.start]
                            diff_r = covers[(ref.start - 1)] - covers[ref.start]
