...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/converter.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 23180 bytes
import os, csv
...
class Converter(object):
    __doc__ = 'Converting from one format to another format'
    ...
    def _check_locus_tag(self, entry, genes):
        ...
        if 'locus_tag' in entry.attributes.keys():
            ...
            for gene in genes:
                if gene.attributes['ID'] in entry.attributes['Parent'].split(','):
            ...
            for row in f_h:
                ...
                if row[0:2] == 'ID':
                    ...
                    if row[0:2] == 'FT':
                        ...
                        if note_name != 'misc' and row[5] == ' ' and row[21] == '/':
                            if first:
                                first = False
                            else:
                                line = line + ';'
                            ...
                            if note_name != 'misc' and row[5] == ' ' and row[21] != '/':
                                ...
                                note = note.replace(';', ',')
                                line = line + ' ' + note
                            if note_name != 'misc' and row[5] != ' ' and row[5:11] != 'source':
                                first = True
                                if info != 'Wrong':
                                    for pos in info['pos']:
                                        out.write('{0}\tRefseq\t{1}\t{2}\t{3}\t.\t{4}\t.\t{5}\n'.format(id_name, info['source'], pos['start'], pos['end'], info['strand'], line))
    ...
    def _multi_tss_class(self, tss, tss_index, tss_features, nums, utrs):
        ...
        if tss_type not in tss_features['tss_types'] or tss.locus_tag not in tss_features['locus_tags']:
    ...
    def _print_tssfile(self, nums, tss_features, tss, tss_pro, strain, method, out, tss_libs):
        ...
        attribute_string = ';'.join(['='.join(items) for items in (
         ...
         [
          'ID',
          strain + '_' + tss_pro.lower() + str(nums['tss_uni'])],
         [
          'type', tss_merge_type],
         [
    ...
    def convert_gff2rntptt(self, gff_file, fasta_file, ptt_file, rnt_file, srna_input_file, srna_output_file):
        ...
        out_p.close()
        out_r.close()
        if srna_input_file is not None and srna_output_file is not None:
            self._srna2rntptt(srna_input_file, srna_output_file, srnas, str(len(seq)))
        else:
            if srna_input_file is None and srna_output_file is None:
                pass
            else:
                print('Error: Lack sRNA input gff files or the name sRNA output rnt files\n')
    ...
    def convert_mastertable2gff(self, tss_file, method, tss_pro, strain, out_gff):
        ...
        for tss in self.tssparser.entries(tss_fh):
            detect_run = True
            key = '_'.join([str(tss.super_pos), tss.super_strand])
            if tss.super_pos == temps['tss'] and temps['strand'] == tss.super_strand and tss.class_count == 1:
                pass
            elif tss.super_pos != temps['tss'] or temps['strand'] != tss.super_strand:
                check_print = False
                nums['class'] = 1
                ...
                if tss.class_count == 1 or nums['class'] > tss.class_count:
            ...
            for line in t_h:
                ...
                if len(row) < 10:
                    ...
                    if len(row) == 14:
                        ...
                        strand = row[3]
                        gene = 'missing'
                    entry = Gff3Entry({'seq_id': transterm_file.split('/')[(-1)][:-1 * len('_best_terminator_after_gene.bag')], 
    ...
    def convert_circ2gff(self, circ_file, args_circ, out_all, out_filter):
        ...
        for circ in circs:
            ...
            out_a.write('\t'.join([str(field) for field in [
             ...
             '.', circ['strand'], '.',
             attribute_string]]) + '\n')
            if circ['depth'] >= args_circ.support and circ['conflict'] == 'NA' and circ['per_start'] >= args_circ.start_ratio and circ['per_end'] >= args_circ.end_ratio:
