...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/stat_sorf.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 8988 bytes
...
def create_dict(nums, strain, utr_detect):
    ...
    if utr_detect:
        types = [
         'all', "5'UTR_derived", "3'UTR_derived",
         'interCDS', 'intergenic', 'antisense']
    else:
        types = [
         'all', 'intergenic', 'antisense']
    for type_ in types:
        nums[strain][type_] = {}
...
def plus_data(nums, strain, sorf_types, features, utr_detect):
    for sorf_type in sorf_types:
        if not utr_detect and (sorf_type == 'intergenic' or sorf_type == 'antisense' or sorf_type == 'all') or utr_detect:
            for feature in features:
                nums[strain][sorf_type][feature] += 1


def print_num(out, num, nums, strain, type_):
    out.write('(for genome {0}; '.format(float(num) / float(nums[strain]['all']['all'])))
...
def print_stat(nums, nums_best, strain, out, utr_detect):
    ...
    for type_, features in nums[strain].items():
        ...
        out.write('(for this genome - {0})\n'.format(float(nums[strain][type_]['all']) / float(nums[strain]['all']['all'])))
        ...
        if strain in nums_best.keys():
            ...
            out.write('\t\tThe best sORF which without overlap with sRNA are {0}'.format(nums_best[strain][type_]['sRNA']))
...
def get_stat_num(sorfs_all, utr_detect):
    ...
    for sorf in sorfs_all:
        ...
        if sorf.attributes['sORF_type'] == 'intergenic':
            ...
            if sorf.attributes['sORF_type'] == 'antisense':
                sorf_type = 'antisense'
            else:
                if '5utr' in sorf.attributes['sORF_type']:
                    sorf_type = "5'UTR_derived"
                else:
                    if '3utr' in sorf.attributes['sORF_type']:
...
def check_class(sorf, nums, sorf_type, utr_detect, strain):
    if sorf.attributes['with_TSS'] != 'NA' and sorf.attributes['sRNA'] == 'NA' and sorf.attributes['rbs'] != 'NA':
        ...
        if sorf.attributes['with_TSS'] != 'NA' and sorf.attributes['sRNA'] == 'NA':
            plus_data(nums, 'total', [sorf_type, 'all'], [
             'all', 'TSS', 'sRNA', 'TSS_sRNA'], utr_detect)
            plus_data(nums, strain, [sorf_type, 'all'], [
             'all', 'TSS', 'sRNA', 'TSS_sRNA'], utr_detect)
        else:
            if sorf.attributes['with_TSS'] != 'NA' and sorf.attributes['rbs'] != 'NA':
                plus_data(nums, 'total', [sorf_type, 'all'], [
                 'all', 'TSS', 'RBS', 'TSS_RBS'], utr_detect)
                plus_data(nums, strain, [sorf_type, 'all'], [
                 'all', 'TSS', 'RBS', 'TSS_RBS'], utr_detect)
            else:
                if sorf.attributes['rbs'] != 'NA' and sorf.attributes['sRNA'] == 'NA':
                    plus_data(nums, 'total', [sorf_type, 'all'], [
                     'all', 'RBS', 'sRNA', 'RBS_sRNA'], utr_detect)
                    plus_data(nums, strain, [sorf_type, 'all'], [
                     'all', 'RBS', 'sRNA', 'RBS_sRNA'], utr_detect)
                else:
                    if sorf.attributes['with_TSS'] != 'NA':
                        plus_data(nums, 'total', [sorf_type, 'all'], [
                         'all', 'TSS'], utr_detect)
                        plus_data(nums, strain, [sorf_type, 'all'], [
                         'all', 'TSS'], utr_detect)
                    else:
                        if sorf.attributes['sRNA'] == 'NA':
                            plus_data(nums, 'total', [sorf_type, 'all'], [
                             'all', 'sRNA'], utr_detect)
                            plus_data(nums, strain, [sorf_type, 'all'], [
                             'all', 'sRNA'], utr_detect)
                        else:
                            if sorf.attributes['rbs'] != 'NA':
                                plus_data(nums, 'total', [sorf_type, 'all'], [
                                 'all', 'RBS'], utr_detect)
                                plus_data(nums, strain, [sorf_type, 'all'], [
                                 'all', 'RBS'], utr_detect)
