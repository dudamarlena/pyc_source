...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/screen.py
...
class Screen(object):
    __doc__ = 'generation of screenshot'
    ...
    def __init__(self, args_sc, out_folder):
        ...
        self.helper.check_make_folder(os.path.join(args_sc.output_folder, self.strain))
        self.forward_file = os.path.join(args_sc.output_folder, self.strain, 'forward')
        self.reverse_file = os.path.join(args_sc.output_folder, self.strain, 'reverse')
    ...
    def _import_libs(self, texs, strand, lib_dict):
        ...
        for flib in texs:
            if flib[1] == 'tex':
                ...
                for nlib in texs:
                    if nlib[1] == 'notex' and flib[2] == nlib[2] and flib[3] == nlib[3]:
    ...
    def screenshot(self, args_sc, log):
        ...
        gen_screenshot(args_sc, lib_dict, self.forward_file + '.txt', self.reverse_file + '.txt', self.strain)
