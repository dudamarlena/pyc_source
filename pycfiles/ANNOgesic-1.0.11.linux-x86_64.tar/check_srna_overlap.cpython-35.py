...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/check_srna_overlap.py
...
def check_overlap(table_file, gff_file):
    ...
    for row in csv.reader(fh, delimiter='\t'):
        if row[3] != 'Start':
            ...
            for gff in gffs:
                if gff.end < end and gff.end > start and gff.start <= start or gff.start > start and gff.start < end and gff.end >= end or gff.end >= end and gff.start <= start or gff.end <= end and gff.start >= start:
            ...
            if len(overlaps['reverse']) == 0:
                overlaps['reverse'] = [
                 'NA']
                overlaps['CDS_r'] = ['NA']
            out.write('\t'.join(row[0:19] + [';'.join(overlaps['CDS_f']), ';'.join(overlaps['forward']), ';'.join(overlaps['CDS_r']), ';'.join(overlaps['reverse'])] + row[21:]) + '\n')
