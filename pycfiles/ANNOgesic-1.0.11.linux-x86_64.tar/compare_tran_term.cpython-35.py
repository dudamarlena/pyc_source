...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/compare_tran_term.py
...
def comparing(ta, ter, fuzzy_down_ta, fuzzy_up_ta, stats):
    ...
    if ta.seq_id == ter.seq_id:
        if ta.strand == ter.strand and ta.strand == '+' and (ta.end >= ter.start and ta.end <= ter.end or ta.end <= ter.start and ter.start - ta.end <= fuzzy_down_ta or ta.end >= ter.end and ta.end - ter.end <= fuzzy_up_ta or ta.start <= ter.start and ta.end >= ter.end) and ter.attributes['Parent'] == 'NA':
            stats[ta.seq_id]['overlap'] += 1
            ta.attributes['associated_term'] = 'terminator:' + str(ter.start) + '-' + str(ter.end) + '_' + ter.strand
            if 'ID' in ta.attributes.keys():
                ter.attributes['Parent'] = ta.attributes['ID']
            else:
                ter.attributes['Parent'] = 'transcript:' + str(ta.start) + '-' + str(ta.end) + '_' + ta.strand
    else:
        if ta.start >= ter.start and ta.start <= ter.end or ta.start <= ter.start and ter.start - ta.start <= fuzzy_up_ta or ta.start >= ter.end and ta.start - ter.end <= fuzzy_down_ta or ta.start <= ter.start and ta.end >= ter.end:
...
def output_term(ters, term_file, type_, term_outfolder):
    ...
    for ter in ters:
        attribute_string = ';'.join(['='.join(items) for items in ter.attributes.items()])
    ...
    if type_ == 'terminator':
        ...
        for row in csv.reader(fh, delimiter='\t'):
            if row[0] != 'genome':
                for ter in ters:
                    if row[0] == ter.seq_id and row[2] == str(ter.start) and row[3] == str(ter.end) and row[4] == ter.strand:
...
def compare_term_tran(trans, terms, fuzzy_up_ta, fuzzy_down_ta, out_folder, type_, term_outfolder, tran_outfolder):
    ...
    for tran in os.listdir(trans):
        if tran.endswith('_transcript.gff'):
            ...
            ters = read_gff(os.path.join(terms, prefix + '_term.gff'), 'Parent')
            stats = {}
            pre_seq = ''
            for ta in tas:
            ...
            shutil.copy(os.path.join(trans, tran) + 'tmp', os.path.join(tran_outfolder, tran))
            shutil.move(os.path.join(trans, tran) + 'tmp', os.path.join(trans, tran))
            output_term(ters, os.path.join(terms, prefix + '_term.gff'), type_, term_outfolder)
            out = open(os.path.join(out_folder, 'statistics/stat_compare_transcript_terminator_' + prefix + '.csv'), 'w')
            for strain, stat in stats.items():
                out.write(strain + ':\n')
                out.write('\tThe overlap between transcripts and terminators are {0}\n'.format(stat['overlap']))
                out.write('\tThe overlap percentage of transcripts are {0}\n'.format(float(stat['overlap']) / float(stat['all_tran'])))
                out.write('\tThe overlap percentage of terminators are {0}\n'.format(float(stat['overlap']) / float(stat['all_term'])))
