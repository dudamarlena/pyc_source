...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/terminator.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 30408 bytes
...
class Terminator(object):
    __doc__ = 'detection of terminator'
    ...
    def __init__(self, args_term):
        ...
        self.terms = {'all': os.path.join(self.outfolder['term'], 'all_candidates'), 
         
         'express': os.path.join(self.outfolder['term'], 'expressed_candidates'), 
         
         'best': os.path.join(self.outfolder['term'], 'best_candidates'), 
         
         'non': os.path.join(self.outfolder['term'], 'non_expressed_candidates')}
        self.csvs = {'all': os.path.join(self.outfolder['csv'], 'all_candidates'), 
         
         'express': os.path.join(self.outfolder['csv'], 'expressed_candidates'), 
         
         'best': os.path.join(self.outfolder['csv'], 'best_candidates'), 
         
         'non': os.path.join(self.outfolder['csv'], 'non_expressed_candidates')}
        ...
        with open(combine_file, 'w') as (result):
            for file_ in files:
                if file_.endswith('.ptt') and os.stat(file_).st_size == 0:
                    print('Warning: No CDS information, TransTermHP can not work!')
                    return 'NO_CDS'
                if os.path.exists(file_) and os.stat(file_).st_size != 0:
                    check_start = False
                    fh = open(file_, 'r')
                    for line in fh:
                        if check_start:
        ...
        for gff in os.listdir(gff_path):
            if gff.endswith('.gff'):
                ...
                if sRNAs:
                    ...
                    srna = self.helper.get_correct_file(self.srna_path, '_sRNA.gff', prefix, None, None)
                    if srna and fasta:
                        log.write('Running converter.py to convert {0} and {1} to {2}, {3}, and {4}.\n'.format(gff_file, srna, ptt_file, rnt_file, srna.replace('.gff', '.rnt')))
                        self.converter.convert_gff2rntptt(gff_file, fasta, ptt_file, rnt_file, srna, srna.replace('.gff', '.rnt'))
                        file_types[prefix] = 'srna'
                        log.write('The following files are generated:\n')
                        log.write('\t{0}\n\t{1}\n\t{2}\n'.format(ptt_file, rnt_file, srna.replace('.gff', '.rnt')))
                    if not srna and fasta:
                        log.write('Running converter.py to convert {0} to {1}, and {2}.\n'.format(gff_file, ptt_file, rnt_file))
                        self.converter.convert_gff2rntptt(gff_file, fasta, ptt_file, rnt_file, None, None)
    ...
    def _TransTermHP(self, fasta, file_, out_path, prefix, out, args_term, log):
        call([args_term.TransTermHP_path, '-p', args_term.expterm_path,
         ...
         '--bag-output',
         os.path.join(out_path, '_'.join([
          prefix, 'best_terminator_after_gene.bag']))], stdout=out)
    ...
    def _run_TransTermHP(self, args_term, log):
        ...
        for file_ in os.listdir(self.combine_path):
            if '.ptt' in file_:
                ...
                fasta = self.helper.get_correct_file(self.fasta_path, '.fa', prefix, None, None)
                if not fasta:
                    log.write('{0}.fa can not be found!.\n'.format(prefix))
                    print('Error: {0}.fa can not be found!'.format(prefix))
                    sys.exit()
                ...
                self._TransTermHP(fasta, file_, out_path, prefix, out, args_term, log)
                log.write('Done!\n')
    ...
    def _convert_to_gff(self, prefixs, args_term, log):
        log.write('Running coverter.py to convert the results of TransTermHP to gff3 format.\n')
                ...
                if prefix == folder:
                    ...
                    for file_ in os.listdir(out_path):
                        if file_.endswith('.bag'):
                            out_file = os.path.join(self.tmps['transterm'], '_'.join([prefix, self.tmps['hp_gff']]))
                            self.converter.convert_transtermhp2gff(os.path.join(out_path, file_), out_file)
                            log.write('\t' + out_file + ' is generated.\n')

        self.multiparser.combine_gff(args_term.gffs, self.tmps['transterm'], None, self.tmps['hp'])

    def _combine_wigs(self, args_term):
        if args_term.tex_wigs is not None and args_term.frag_wigs is not None:
            ...
            for wig in os.listdir(args_term.tex_wigs):
                if os.path.isdir(os.path.join(args_term.tex_wigs, wig)):
                    ...
                    shutil.copy(os.path.join(args_term.tex_wigs, wig), merge_wigs)

            for wig in os.listdir(args_term.frag_wigs):
                if os.path.isdir(os.path.join(args_term.frag_wigs, wig)):
                    pass
                else:
                    shutil.copy(os.path.join(args_term.frag_wigs, wig), merge_wigs)

        else:
            if args_term.tex_wigs is not None:
                merge_wigs = args_term.tex_wigs
            else:
                if args_term.frag_wigs is not None:
    ...
    def _merge_sRNA(self, sRNAs, prefixs, gff_path):
        ...
        if sRNAs is not None:
            ...
            for prefix in prefixs:
                ...
                self.helper.merge_file(os.path.join(self.srna_path, '_'.join([prefix, 'sRNA.gff'])), tmp_gff)
                self.helper.sort_gff(tmp_gff, os.path.join(self.tmps['merge'], prefix + '.gff'))
                os.remove(tmp_gff)
    ...
    def _move_file(self, term_outfolder, csv_outfolder):
        for gff in os.listdir(term_outfolder):
            if gff.endswith('_term.gff'):
                self.helper.sort_gff(os.path.join(term_outfolder, gff), self.tmps['gff'])
                shutil.move(self.tmps['gff'], os.path.join(term_outfolder, gff))
                prefix = gff.replace('_term.gff', '')
                new_gff = os.path.join(self.terms['all'], '_'.join([
                 prefix, self.suffixs['allgff']]))
                csv_file = os.path.join(os.path.join(self.csvs['all'], '_'.join([
                 prefix, self.suffixs['csv']])))
                out = open(new_gff, 'w')
                out.write('##gff-version 3\n')
                out.close()
                self.helper.merge_file(os.path.join(term_outfolder, gff), os.path.join(self.terms['all'], '_'.join([
                 prefix, self.suffixs['allgff']])))
                os.remove(os.path.join(term_outfolder, gff))
                pre_strain = ''
                if '_'.join([prefix, self.suffixs['csv']]) in os.listdir(self.csvs['all']):
                    os.remove(csv_file)
                ...
                for entry in self.gff_parser.entries(fh):
                    if entry.seq_id != pre_strain:
                        self.helper.merge_file(os.path.join(self.tmps['term_table'], '_'.join([
                         entry.seq_id, 'term_raw.csv'])), os.path.join(self.csvs['all'], '_'.join([
                         prefix, self.suffixs['csv']])))
    ...
    def _compute_intersection_forward_reverse(self, prefixs, merge_path, wig_path, merge_wigs, args_term, log):
        """the approach for searching gene converged region terminator"""
        log.write('Searching terminators which located in gene converged region.\n')
        for prefix in prefixs:
            tmp_seq = os.path.join(args_term.out_folder, '_'.join(['inter_seq', prefix]))
            tmp_index = os.path.join(args_term.out_folder, '_'.join(['inter_index', prefix]))
            tmp_sec = os.path.join(args_term.out_folder, '_'.join(['inter_sec', prefix]))
            tran_file = os.path.join(self.tran_path, '_'.join([prefix, 'transcript.gff']))
            gff_file = os.path.join(merge_path, prefix + '.gff')
            tmp_cand = tmp_cand = os.path.join(args_term.out_folder, '_'.join(['term_candidates', prefix]))
            if os.path.exists(tran_file):
                print('Extracting sequences of {0}'.format(prefix))
                log.write('Running get_inter_seq.py to extract the potential sequences from {0}.\n'.format(prefix))
                intergenic_seq(os.path.join(self.fasta_path, prefix + '.fa'), tran_file, gff_file, tmp_seq, tmp_index, args_term)
                log.write('\t' + tmp_seq + ' is generated for storing the potential sequences.\n')
                self._run_rnafold(args_term.RNAfold_path, tmp_seq, tmp_sec, prefix, log)
                log.write('Running extract_sec_info.py to extract the information of secondary structure from {0}.\n'.format(prefix))
                extract_info_sec(tmp_sec, tmp_seq, tmp_index)
                os.remove(tmp_index)
                log.write('Running get_polyT.py to detect the terminator candidates for {0}.\n'.format(prefix))
                poly_t(tmp_seq, tmp_sec, gff_file, tran_file, tmp_cand, args_term)
                log.write('\t' + tmp_cand + ' which temporary stores terminator candidates is generated.\n')
            print('Detecting terminators for ' + prefix)
            log.write('Running detect_coverage_term.py to gain high-confidence terminators for {0}.\n'.format(prefix))
            detect_coverage(tmp_cand, os.path.join(merge_path, prefix + '.gff'), os.path.join(self.tran_path, '_'.join([
             prefix, 'transcript.gff'])), os.path.join(self.fasta_path, prefix + '.fa'), os.path.join(wig_path, '_'.join([prefix, 'forward.wig'])), os.path.join(wig_path, '_'.join([prefix, 'reverse.wig'])), os.path.join(self.tmps['hp_path'], '_'.join([
             prefix, self.tmps['hp_gff']])), merge_wigs, os.path.join(self.outfolder['term'], '_'.join([
             prefix, self.suffixs['gff']])), os.path.join(self.tmps['term_table'], '_'.join([
             prefix, 'term_raw.csv'])), args_term)

        self.multiparser.combine_gff(args_term.gffs, self.outfolder['term'], None, 'term')
    ...
    def _remove_tmp_file(self, merge_wigs, args_term):
        ...
        self.helper.remove_all_content(args_term.out_folder, 'inter_seq_', 'file')
        self.helper.remove_all_content(self.outfolder['term'], '_term.gff', 'file')
        self.helper.remove_all_content(args_term.out_folder, 'inter_sec_', 'file')
        self.helper.remove_all_content(args_term.out_folder, 'term_candidates_', 'file')
    ...
    def _compute_stat(self, args_term, log):
        ...
        for gff in os.listdir(self.terms['all']):
            if gff.endswith('_term_all.gff'):
                ...
                for entry in self.gff_parser.entries(fh):
                    name = '%0*d' % (5, num)
                    entry.attributes['ID'] = entry.seq_id + '_terminator' + str(num)
                    entry.attributes['Name'] = '_'.join(['terminator_' + name])
                    entry.attribute_string = ';'.join(['='.join(items) for items in entry.attributes.items()])
                ...
                out_tmp.close()
                fh.close()
                shutil.move(self.tmps['gff'], os.path.join(self.terms['all'], '_'.join([new_prefix, self.suffixs['gff']])))
        ...
        for prefix in new_prefixs:
            stat_term(os.path.join(self.terms['all'], '_'.join([prefix, self.suffixs['gff']])), os.path.join(self.csvs['all'], '_'.join([prefix, self.suffixs['csv']])), os.path.join(stat_path, '_'.join(['stat', prefix + '.csv'])), os.path.join(self.terms['best'], '_'.join([prefix, 'term'])), os.path.join(self.terms['express'], '_'.join([prefix, 'term'])), os.path.join(self.terms['non'], '_'.join([prefix, 'term'])))
            shutil.move(os.path.join(self.terms['best'], '_'.join([prefix, self.suffixs['csv']])), os.path.join(self.csvs['best'], '_'.join([prefix, self.suffixs['csv']])))
            shutil.move(os.path.join(self.terms['express'], '_'.join([prefix, self.suffixs['csv']])), os.path.join(self.csvs['express'], '_'.join([prefix, self.suffixs['csv']])))
            shutil.move(os.path.join(self.terms['non'], '_'.join([prefix, self.suffixs['csv']])), os.path.join(self.csvs['non'], '_'.join([prefix, self.suffixs['csv']])))
            os.remove(os.path.join(self.terms['all'], '_'.join([prefix, self.suffixs['allgff']])))
            log.write('\t' + os.path.join(self.terms['all'], '_'.join([prefix, self.suffixs['gff']])) + '\n')
    ...
    def _compare_term_tran(self, args_term, prefixs, log):
        ...
        self.multiparser.combine_gff(args_term.gffs, self.tran_path, None, 'transcript')
        prefixs = []
        ...
        log.write('Running compare_tran_term.py for comparing transcripts and terminators.\n')
        log.write('The following files are generated:\n')
        for type_ in ('best_candidates', 'expressed_candidates', 'all_candidates'):
            compare_term_tran(self.tran_path, os.path.join(self.outfolder['term'], type_), args_term.fuzzy_up_ta, args_term.fuzzy_down_ta, args_term.out_folder, 'terminator', self.outfolder['term'], args_term.trans)
            for prefix in prefixs:
                shutil.move(os.path.join(args_term.out_folder, 'statistics', 'stat_compare_transcript_terminator_' + prefix + '.csv'), os.path.join(args_term.out_folder, 'statistics', '_'.join(['stat_compare_terminator_transcript', prefix,
                 type_ + '.csv'])))
                log.write('\t' + os.path.join(args_term.out_folder, 'statistics', '_'.join(['stat_compare_terminator_transcript', prefix, type_ + '.csv'])) + '\n')
    ...
    def _re_table(self, args_term, prefixs, log):
        ...
        for type_ in ['all_candidates', 'best_candidates',
         'expressed_candidates', 'non_expressed_candidates']:
            for table in os.listdir(os.path.join(args_term.out_folder, 'tables', type_)):
                term_table = os.path.join(args_term.out_folder, 'tables', type_, table)
                reorganize_table(args_term.libs, args_term.merge_wigs, 'Coverage_detail', term_table)
    ...
    def run_terminator(self, args_term, log):
        ...
        if not args_term.gffs or not args_term.fastas:
            ...
            sys.exit()
        file_types, prefixs = self._convert_gff2rntptt(self.gff_path, self.fasta_path, args_term.srnas, log)
        check = self._combine_ptt_rnt(self.gff_path, file_types, self.srna_path)
        ...
        self._compute_intersection_forward_reverse(prefixs, merge_path, args_term.wig_path, args_term.merge_wigs, args_term, log)
