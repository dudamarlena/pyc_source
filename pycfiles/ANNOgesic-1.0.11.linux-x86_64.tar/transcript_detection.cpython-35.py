...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/transcript_detection.py
            ...
            if lib['name'] == track:
                ...
                if len(texs) != 0:
                    for key, num in texs.items():
                        if track in key and texs[key] >= tex_notex and key not in check_tex:
...
def detect_hight_toler(cover, height, tmp_covers, tracks, lib_track):
    ...
    elif cover > tmp_covers['toler']:
        tmp_covers['toler'] = cover


def elongation(lib_conds, template_texs, libs, strand, trans, args_tran, strain, tolers):
    ...
    for cond, lib_tracks in lib_conds.items():
        for lib_name, covers in lib_tracks.items():
            ...
            for cover in covers:
                for cond, lib_tracks in lib_conds.items():
                    for lib_track in lib_tracks.keys():
                        ...
                        if index_pos < len(lib_tracks[lib_track]):
                            ...
                            compare_cover = 0
                        detect_hight_toler(compare_cover, args_tran.height, tmp_covers, tracks, real_track)
                ...
                check_tex_conds(tracks, libs, texs, check_tex, conds, args_tran.tex)
...
def transfer_to_tran(wigs, libs, template_texs, strand, args_tran):
    ...
    for strain, lib_conds in wigs.items():
        if strain not in trans:
            trans[strain] = []
            tolers[strain] = []
        elongation(lib_conds, template_texs, libs, strand, trans, args_tran, strain, tolers[strain])
...
def fill_gap_and_print(trans, strand, finals, tolers, wig_type, args_tran):
    ...
    for strain, covers in trans.items():
        ...
        for cover in covers:
            ...
            if cover_pos - pre_pos <= args_tran.tolerance:
                if cover_pos - pre_pos > 1:
                    for toler_strain, toler_datas in tolers.items():
                        if toler_strain == strain:
                            toler_covers = toler_datas[pre_pos - 1:cover_pos]
                ...
                if fit:
                    ...
                    if low_cover > cover:
                        low_cover = cover
                    if cover_pos - pre_pos > args_tran.tolerance or not fit:
                        if start != -1 and end != -1 and end - start >= args_tran.width:
                            finals[strain].append({'start': start, 'end': end, 'strand': strand, 
        ...
        if len(covers) != 0 and not first and start != -1 and end != -1 and end - start >= args_tran.width:
            finals[strain].append({'start': start, 'end': end, 'strand': strand, 
             'high': high_cover, 'low': low_cover, 
             'wig': wig_type})
...
def detect_transcript(wig_f_file, wig_r_file, wig_folder, input_lib, out_file, wig_type, args_tran):
    out = open(out_file, 'w')
