...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gen_srna_output.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 17323 bytes
...
def import_data(row, type_, args_srna, term_path):
    if type_ == 'gff':
        ...
        if term_path is not None and args_srna.promoter_table is None:
            ...
            if term_path is None and args_srna.promoter_table is not None:
                data['track'] = row[10]
                data['promoter'] = row[13]
            else:
                if term_path is not None and args_srna.promoter_table is not None:
                    data['track'] = row[10]
                    data['with_term'] = row[13]
...
def merge_info(blasts):
    ...
    if len(blasts) != 0:
        for blast in blasts:
            ...
            elif pre_blast['strain'] == blast['strain'] and pre_blast['strand'] == blast['strand'] and pre_blast['start'] == blast['start'] and pre_blast['end'] == blast['end']:
                if repeat < 2:
                    pre_blast['hits'] = ';'.join([
...
def compare_srna_table(srna_tables, srna, final, args_srna):
    ...
    for table in srna_tables:
        ...
        pros = []
        cands = []
        if table['strain'] == srna.seq_id and table['strand'] == srna.strand and table['start'] == srna.start and table['end'] == srna.end:
            ...
            for data in start_datas:
                if 'TSS' in data:
                    if table['start'] != int(data.split(':')[1][:-2]):
                        ...
                        if 'Cleavage' in data and table['end'] != int(data.split(':')[1][:-2]):
            ...
            for tss in tsss:
                for pro in pros:
                    if pro - tss >= args_srna.min_len and pro - tss <= args_srna.max_len:
                        cands.append('-'.join([str(tss), str(pro)]))

            final['candidates'] = ';'.join(cands)
            if 'tex' in table['conds'] and 'frag' in table['conds']:
                final['type'] = 'TEX+/-;Fragmented'
            else:
                if 'tex' in table['conds']:
                    final['type'] = 'TEX+/-'
...
def compare_blast(blasts, srna, final, hit):
    for blast in blasts:
        if srna.seq_id == blast['strain'] and srna.strand == blast['strand'] and srna.start == blast['start'] and srna.end == blast['end']:
    ...
    for srna in srnas:
        ...
        if srna.attributes['sRNA_type'] == 'intergenic':
            ...
            if srna.attributes['sRNA_type'] == 'in_CDS':
                ...
                if srna.attributes['sRNA_type'] == 'antisense':
                    final['utr'] = 'Antisense'
                else:
                    if ',' in srna.attributes['sRNA_type']:
                        final['utr'] = "5'UTR_derived;3'UTR_derived"
                    else:
                        if srna.attributes['sRNA_type'] == '5utr':
                            final['utr'] = "5'UTR_derived"
                        else:
                            if srna.attributes['sRNA_type'] == '3utr':
...
def print_file(finals, out, srnas, out_gff):
    ...
    for srna in srnas:
        for final in finals:
            if srna.seq_id == final['strain'] and srna.start == final['start'] and srna.end == final['end'] and srna.strand == final['strand'] and 'sRNA_hit' in final.keys() and final['sRNA_hit'] != 'NA':
                names = change_srna_name(final)
...
def read_table(srna_table_file, nr_blast, srna_blast_file, args_srna, term_path):
    srna_tables = []
    nr_blasts = []
    srna_blasts = []
...
def gen_srna_table(srna_gff, srna_table_file, nr_blast, srna_blast_file, args_srna, out_file, term_path):
    """generate the sRNA table for more details"""
    srnas = read_gff(srna_gff)
    srna_tables, nr_blasts, srna_blasts = read_table(srna_table_file, nr_blast, srna_blast_file, args_srna, term_path)
    out = open(out_file, 'w')
    ...
    finals = compare(srnas, srna_tables, nr_blasts, srna_blasts, args_srna)
...
def check_energy(srna, import_info, energy, detect):
    ...
    if import_info is not None:
        if 'sec_str' in import_info and '2d_energy' in srna.attributes.keys():
    ...
    if 'tss' in import_info:
        if 'with_TSS' in srna.attributes.keys():
            if srna.attributes['with_TSS'] != 'NA':
                detect['TSS'] = True
            elif srna.attributes['sRNA_type'] != 'intergenic' and srna.attributes['sRNA_type'] != 'in_CDS' and srna.attributes['sRNA_type'] != 'antisense' and ('3utr' in srna.attributes['sRNA_type'] or 'interCDS' in srna.attributes['sRNA_type']) and srna.attributes['start_cleavage'] != 'NA':
                detect['TSS'] = True
        else:
            detect['TSS'] = True
    else:
...
def check_nr_hit(srna, nr_hits_num, detect, import_info):
    ...
    if import_info is not None:
        if 'nr_hit' in srna.attributes.keys() and 'blast_nr' in import_info:
            if srna.attributes['nr_hit'] == 'NA' or int(srna.attributes['nr_hit']) <= nr_hits_num:
                detect['nr_hit'] = True
        else:
            detect['nr_hit'] = True
    else:
...
def check_sorf(import_info, srna, detect):
    ...
    if import_info is not None:
        if 'sorf' in import_info:
            if 'sORF' in srna.attributes.keys() and srna.attributes['sORF'] == 'NA':
                detect['sORF'] = True
        else:
            detect['sORF'] = True
    else:
...
def check_srna_hit(srna, import_info, detect):
    ...
    if import_info is not None:
        if 'sRNA_hit' in srna.attributes.keys() and 'blast_srna' in import_info:
    ...
    if 'term' in import_info:
        if 'with_term' in srna.attributes.keys():
            if srna.attributes['with_term'] != 'NA':
                detect['term'] = True
            elif 'end_cleavage' in srna.attributes.keys() and srna.attributes['end_cleavage'] != 'NA':
                detect['term'] = True
        else:
            detect['term'] = True
    else:
...
def check_promoter(import_info, srna, detect):
    ...
    if import_info is not None:
        if 'promoter' in import_info:
            if 'promoter' in srna.attributes.keys() and srna.attributes['promoter'] != 'NA':
...
def gen_best_srna(srna_file, out_file, args_srna):
    ...
    for srna in srnas:
        detect = {'energy': False, 'TSS': False, 'nr_hit': False, 
         ...
         'promoter': False}
        check_energy(srna, args_srna.import_info, args_srna.energy, detect)
        check_tss(args_srna.import_info, srna, detect)
        check_nr_hit(srna, args_srna.nr_hits_num, detect, args_srna.import_info)
