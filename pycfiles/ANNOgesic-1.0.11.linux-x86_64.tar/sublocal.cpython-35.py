...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/sublocal.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 13066 bytes
...
class SubLocal(object):
    __doc__ = 'detection of subcellular localization'
    ...
    def _compare_cds_tran(self, gff_file, tran_file, log):
        ...
        for cds in cdss:
            for ta in trans:
                if cds.strand == ta.strand and cds.seq_id == ta.seq_id and (cds.end < ta.end and cds.end > ta.start and cds.start <= ta.start or cds.start > ta.start and cds.start < ta.end and cds.end >= ta.end or cds.end >= ta.end and cds.start <= ta.start or cds.end <= ta.end and cds.start >= ta.start):
                    out.write(cds.info + '\n')
                    break
        ...
        th.close()
        out.close()
        log.write('\t' + os.path.join(self.out_all, 'tmp_cds.gff') + ' is temporary generated.\n')
    ...
    def _get_protein_seq(self, gff, tmp_path, tran_path, args_sub, log):
        ...
        if tran_path is not None:
            log.write('Predicting subcellular localization for expressed CDSs for {0}.\n'.format(prefix))
            self._compare_cds_tran(os.path.join(self.gff_path, gff), os.path.join(tran_path, '_'.join([
             prefix, 'transcript.gff'])), log)
            log.write('Running helper.py to extract sequences for CDSs.\n')
            self.helper.get_cds_seq(os.path.join(self.out_all, 'tmp_cds.gff'), fasta, dna_seq_file)
            os.remove(os.path.join(self.out_all, 'tmp_cds.gff'))
        else:
            log.write('Predicting subcellular localization for all CDSs for {0}.\n'.format(prefix))
            log.write('Running helper.py to extract sequences for CDSs.\n')
            self.helper.get_cds_seq(os.path.join(self.gff_path, gff), fasta, dna_seq_file)
        log.write('\t' + dna_seq_file + ' is generated.\n')
        print('Transfering DNA sequences to protein sequence of {0}'.format(prefix))
        log.write('Running helper.py to translate DNA sequences to Protein sequences.\n')
    ...
    def _psortb(self, psortb_path, strain_type, prot_seq_file, out_raw, out_err, log):
        log.write(' '.join([psortb_path, strain_type, prot_seq_file]) + '\n')
        call([psortb_path, strain_type, prot_seq_file], stdout=out_raw, stderr=out_err)

    def _run_psortb(self, args_sub, prefix, out_folder, tmp_path, tmp_result, log):
        print('Running psortb of {0}'.format(prefix))
        log.write('Running Psortb for predict subcellular localization for {0}.\n'.format(prefix))
        out_err = open(os.path.join(out_folder, 'tmp_log'), 'w')
        out_raw = open(os.path.join(tmp_result, '_'.join([prefix, self.endfix_raw])), 'w')
        prot_seq_file = os.path.join(tmp_path, '_'.join([prefix, 'protein.fa']))
        if args_sub.gram == 'positive':
            self._psortb(args_sub.psortb_path, '-p', prot_seq_file, out_raw, out_err, log)
        else:
            if args_sub.gram == 'negative':
                self._psortb(args_sub.psortb_path, '-n', prot_seq_file, out_raw, out_err, log)
            else:
                log.write('Please assign "positive" or "negative" to --bacteria_type.\n')
                print('Error: {0} is not a proper bacteria type! Please assign positive or negative.'.format(args_sub.gram))
                sys.exit()
        log.write('\t' + os.path.join(tmp_result, '_'.join([prefix, self.endfix_raw])) + ' is temporary generated.\n')
        out_err.close()
        out_raw.close()

    def _extract_result(self, args_sub, tmp_psortb_path, prefix, gff_file, log):
        """extract the result of psortb"""
        log.write('Running extract_psortb.py to extract the information of localization.\n')
        extract_psortb(os.path.join(tmp_psortb_path, '_'.join([prefix, self.endfix_raw])), os.path.join(tmp_psortb_path, '_'.join([
         prefix, self.endfix_table])), None, None, args_sub.fuzzy)
        log.write('\t' + os.path.join(tmp_psortb_path, '_'.join([prefix, self.endfix_table])) + ' is tempoaray generated.\n')
    ...
    def _merge_and_stat(self, gffs, tmp_psortb_path, stat_path, psortb_result, log):
        for folder in os.listdir(gffs):
            if folder.endswith('.gff_folder'):
                prefix = folder.replace('.gff_folder', '')
                self.helper.check_make_folder(os.path.join(psortb_result, prefix))
                merge_table = os.path.join(psortb_result, prefix, '_'.join([prefix, self.endfix_table]))
                for gff in os.listdir(os.path.join(gffs, folder)):
                    result = self.helper.get_correct_file(tmp_psortb_path, '_' + self.endfix_raw, gff.replace('.gff', ''), None, None)
                    shutil.copy(result, os.path.join(psortb_result, prefix))
                    result = self.helper.get_correct_file(tmp_psortb_path, '_' + self.endfix_table, gff.replace('.gff', ''), None, None)
                ...
                stat_sublocal(merge_table, os.path.join(stat_folder, prefix), stat_file)
    ...
    def run_sub_local(self, args_sub, log):
        for gff in os.listdir(args_sub.gffs):
            if gff.endswith('.gff'):
                self.helper.check_uni_attributes(os.path.join(args_sub.gffs, gff))
        ...
        for gff in os.listdir(self.gff_path):
            if args_sub.trans is not None:
                ...
                prefix = self._get_protein_seq(gff, self.express_tmp_path, self.tran_path, args_sub, log)
                self._run_psortb(args_sub, prefix, self.out_express, self.express_tmp_path, self.express_tmp_result, log)
                self._extract_result(args_sub, self.express_tmp_result, prefix, os.path.join(self.gff_path, gff), log)
            print('Running all genes now')
            prefix = self._get_protein_seq(gff, self.all_tmp_path, None, args_sub, log)
            self._run_psortb(args_sub, prefix, self.out_all, self.all_tmp_path, self.all_tmp_result, log)
            self._extract_result(args_sub, self.all_tmp_result, prefix, os.path.join(self.gff_path, gff), log)

        log.write('Running stat_sublocal.py to do statistics, generate merged tables, and plot figures.\n')
        log.write('The following files are generated:\n')
        self._merge_and_stat(args_sub.gffs, self.all_tmp_result, self.all_stat_path, self.all_result, log)
        if args_sub.trans is not None:
            self._merge_and_stat(args_sub.gffs, self.express_tmp_result, self.express_stat_path, self.express_result, log)
