...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/sORF_intergenic.py
...
def get_type(inter, gffs):
    ...
    anti = False
    for gff in gffs:
        if gff.seq_id == inter['strain'] and gff.strand == inter['strand']:
            if gff.strand == '+':
                ...
                if inter['start'] - 1 == gff.end:
                    utr5 = True
        elif gff.seq_id == inter['strain'] and gff.strand != inter['strand'] and (inter['start'] <= gff.start and inter['end'] >= gff.end or inter['start'] >= gff.start and inter['end'] <= gff.end or inter['start'] <= gff.start and inter['end'] <= gff.end and inter['end'] >= gff.start or inter['start'] >= gff.start and inter['start'] <= gff.end and inter['end'] >= gff.end):
            anti = True
...
def read_gff(gff_file, tran_file, hypo):
    ...
    for entry in Gff3Parser().entries(gh):
        if Helper().feature_without_notgene(entry) and entry.feature != 'sORF':
...
def compare_tran_cds(trans, gffs):
    ...
    for tran in trans:
        poss = [
         {'start': tran.start, 'end': tran.end}]
        for pos in poss:
            exclude = False
            for gff in gffs:
                if tran.seq_id == gff.seq_id and tran.strand == gff.strand:
                    if gff.start <= pos['start'] and gff.end >= pos['start'] and gff.end < pos['end']:
                        pos['start'] = gff.end + 1
                    else:
                        if gff.start > pos['start'] and gff.start <= pos['end'] and gff.end >= pos['end']:
                            pos['end'] = gff.start - 1
                        else:
                            if gff.start <= pos['start'] and gff.end >= pos['end']:
                                exclude = True
                                break
                            elif gff.start > pos['start'] and gff.end < pos['end']:
...
def get_intergenic(gff_file, tran_file, out_file, utr_detect, hypo, extend_5, extend_3):
    ...
    for inter in inters:
        get_type(inter, gffs)
        name = '%0*d' % (5, num)
        if inter['source'] != 'intergenic' and inter['source'] != 'antisense':
            source = 'UTR_derived'
            if utr_detect:
            ...
            if inter['source'] == 'intergenic':
                ...
                attribute_string = ';'.join(['='.join(items) for items in (
                 ...
                 [
                  'Name', 'sORF_' + name])])
            if source == 'UTR_derived' and utr_detect or source == 'intergenic' or source == 'antisense':
                if inter['strand'] == '+':
