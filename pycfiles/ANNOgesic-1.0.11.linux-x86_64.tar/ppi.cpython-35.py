...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/ppi.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 26018 bytes
...
class PPINetwork(object):
    ...
    def _wget_id(self, strain, locus, strain_id, files, log):
        ...
        if strain == strain_id['ptt']:
            print('Retrieving STRING ID for {0} of {1} -- {2}'.format(locus, strain_id['string'], strain_id['file']))
            id_source = 'http://string-db.org/api/tsv/resolve?identifier={0}&species={1}'.format(locus, strain_id['string'])
            self._run_wget(id_source, os.path.join(files['id_list'], locus), files['id_log'], log)
            detect_id = True
        return detect_id
    ...
    def _retrieve_id(self, strain_id, genes, files, log):
        ...
        for gene in genes:
            if gene['gene'] != '-':
                detect_id = self._wget_id(gene['strain'], gene['gene'], strain_id, files, log)
                self.ref_tags[gene['gene']] = gene['locus_tag']
            else:
                detect_id = self._wget_id(gene['strain'], gene['locus_tag'], strain_id, files, log)
                self.ref_tags[gene['locus_tag']] = gene['locus_tag']
            if not detect_id:
                log.write('{0} is not found in {1}.\n'.format(gene, strain_id['file']))
                print('Error: There is no {0} in {1}'.format(gene, strain_id['file']))

        log.write('The temporary files are generated and stored in the following folders:\n')
    ...
    def _get_prefer_name(self, row_a, strain_id, files, querys, log):
        ...
        if filename[1] not in os.listdir(files['id_list']) and 'all' not in querys or 'all' in querys:
            self._wget_id(strain_id['ptt'], filename[1], strain_id, files, log)
        if (filename[1] in os.listdir(files['id_list']) or 'all' in querys) and filename[1] in os.listdir(files['id_list']):
    ...
    def _print_title(self, out, id_file, id_folder):
        ...
        out.write('Interaction of {0} | {1}\n'.format(locus, prefername))
        out.write('Genome\tItem_id_a\tItem_id_b\tMode\tAction\ta_is_acting\tSTRING_action_score\tPubmed_id\tPubmed_score\n')

    def _get_pubmed(self, row, strain_id, mode, actor, id_file, first_output, ptt, files, paths, args_ppi, log):
        prefer1 = self._get_prefer_name(row[0], strain_id, files, args_ppi.querys, log)
        prefer2 = self._get_prefer_name(row[1], strain_id, files, args_ppi.querys, log)
        if len(prefer1) > 0 and len(prefer2) > 0:
            if args_ppi.no_specific:
                pubmed_source = 'http://www.ncbi.nlm.nih.gov/CBBresearch/Wilbur/IRET/PIE/getppi.cgi?term={0}+{1}'.format(prefer1, prefer2)
                self._run_wget(pubmed_source, self.tmp_files['nospecific'], files['pubmed_log'], log)
            strain_id['pie'] = '+'.join(strain_id['pie'].split(' '))
            pubmed_source = 'http://www.ncbi.nlm.nih.gov/CBBresearch/Wilbur/IRET/PIE/getppi.cgi?term={0}+{1}+{2}'.format(prefer1, prefer2, strain_id['pie'])
            self._run_wget(pubmed_source, self.tmp_files['specific'], files['pubmed_log'], log)
            row[2] = mode
            row[4] = actor
            row[0] = prefer1
            row[1] = prefer2
            self._merge_information(first_output, self.tmp_files['specific'], files['all_specific'], files['best_specific'], row, args_ppi.score, id_file, files['id_list'], 'specific', os.path.join(paths['all'], self.with_strain), os.path.join(paths['best'], self.with_strain), ptt)
            if args_ppi.no_specific:
                self._merge_information(first_output, self.tmp_files['nospecific'], files['all_nospecific'], files['best_nospecific'], row, args_ppi.score, id_file, files['id_list'], 'nospecific', os.path.join(paths['all'], self.without_strain), os.path.join(paths['best'], self.without_strain), ptt)
    ...
    def _merge_information(self, first_output, filename, out_all, out_best, row_a, score, id_file, id_folder, file_type, all_folder, best_folder, ptt):
        if os.path.getsize(filename) != 0:
            f_h = open(filename, 'r')
            out_all_single = open(os.path.join(all_folder, ptt, '_'.join([row_a[0], row_a[1] + '.csv'])), 'w')
            out_best_single = open(os.path.join(best_folder, ptt, '_'.join([row_a[0], row_a[1] + '.csv'])), 'w')
        ...
        for row in csv.reader(fh, delimiter='\t'):
            ...
            if 'all' in args_ppi.querys:
                ...
                for query in args_ppi.querys:
                    ...
                    strand = datas[3]
                    if len(row) > 1 and row[0] != 'Location' and name == strain and start == row[0].split('..')[0] and end == row[0].split('..')[1] and strand == row[1]:
                        genes.append({'strain': name, 'locus_tag': row[4], 
                         'gene': row[5]})
    ...
    def _setup_folder_and_read_file(self, strain_id, pre_file, files, paths, args_ppi):
        if strain_id['file'].endswith('.ptt'):
            if strain_id['file'] != pre_file:
                ...
                self.helper.check_make_folder(os.path.join(self.all_result, strain_id['file'][:-4]))
                self.helper.check_make_folder(os.path.join(self.best_result, strain_id['file'][:-4]))
                self.helper.check_make_folder(os.path.join(self.fig, strain_id['file'][:-4]))
                ...
                files['id_log'] = open(os.path.join(files['id_list'], self.tmp_files['log']), 'w')
                files['action_log'] = open(os.path.join(args_ppi.out_folder, self.tmp_files['action']), 'w')
                files['pubmed_log'] = open(os.path.join(args_ppi.out_folder, self.tmp_files['pubmed']), 'w')
    ...
    def _wget_actions(self, files, id_file, strain_id, out_folder, log):
        ...
        print('Retrieving STRING actions for {0} of {1} -- {2}'.format(id_file, strain_id['string'], strain_id['file']))
        for row in csv.reader(t_h, delimiter='\t'):
            if row[0].startswith('stringId'):
                ...
                if row[1] == strain_id['string']:
                    action_source = 'http://string-db.org/api/tsv/actions?identifier={0}&species={1}'.format(row[0], row[1])
                    self._run_wget(action_source, self.tmp_files['wget_action'], files['action_log'], log)
    ...
    def _retrieve_actions(self, files, strain_id, paths, args_ppi, log):
        ...
        log.write('Using STRING and PIE to retrieve the interaction information for {0}.\n'.format(strain_id['ptt']))
        for id_file in os.listdir(files['id_list']):
            if id_file != self.tmp_files['log']:
                detect_id = self._wget_actions(files, id_file, strain_id, args_ppi.out_folder, log)
                if detect_id:
                    ...
                    print('Retrieving Pubmed IDs for {0} of {1} -- {2}'.format(id_file, strain_id['string'], strain_id['file']))
                    for row_a in csv.reader(a_h, delimiter='\t'):
                        ...
                        if row_a[0].startswith('item_id_a'):
                            ...
                            if first:
                                ...
                                if row_a[0] != pre_row[0] or row_a[1] != pre_row[1]:
                                    self._get_pubmed(pre_row, strain_id, mode, actor, id_file, first_output, strain_id['ptt'], files, paths, args_ppi, log)
                                    mode = row_a[2]
                                    actor = row_a[4]
                                else:
                                    mode = mode + ';' + row_a[2]
                    ...
                    if detect:
                        detect = False
                        self._get_pubmed(row_a, strain_id, mode, actor, id_file, first_output, strain_id['ptt'], files, paths, args_ppi, log)
    ...
    def _plot(self, args_ppi, files, log):
        ...
        for folder in os.listdir(self.all_result):
            if folder in os.listdir(self.fig):
                ...
                out_folder_spe = os.path.join(self.fig, folder, self.with_strain)
                plot_ppi(os.path.join(self.all_result, folder, '_'.join([folder, self.with_strain + '.csv'])), args_ppi.score, out_folder_spe, args_ppi.size)
                for file_ in os.listdir(out_folder_spe):
                    log.write('\t' + os.path.join(out_folder_spe, file_) + '\n')

                if args_ppi.no_specific:
                    out_folder_nospe = os.path.join(self.fig, folder, self.without_strain)
                    plot_ppi(os.path.join(self.all_result, folder, '_'.join([folder, self.without_strain + '.csv'])), args_ppi.score, out_folder_nospe, args_ppi.size)
                    for file_ in os.listdir(out_folder_nospe):
                        log.write('\t' + os.path.join(out_folder_nospe, file_) + '\n')

    def _remove_tmps(self, args_ppi):
        self.helper.remove_all_content(os.path.join(args_ppi.out_folder), 'tmp', 'file')
        self.helper.remove_all_content(os.path.join(args_ppi.out_folder), 'tmp', 'dir')
        for file_ in os.listdir(args_ppi.ptts):
            if file_.startswith('PPI_'):
                os.remove(os.path.join(args_ppi.ptts, file_))

        self.helper.remove_all_content(os.path.join(args_ppi.out_folder), 'temp', 'dir')
            ...
            for query in args_ppi.querys:
                ...
                for gff in os.listdir(args_ppi.ptts):
                    ...
                    for entry in Gff3Parser().entries(gff_f):
                        if entry.seq_id == datas[0] and entry.start == int(datas[1]) and entry.end == int(datas[2]) and entry.strand == datas[3]:
                            detect = True
                            break
    ...
    def retrieve_ppi_network(self, args_ppi, log):
        ...
        for strain in args_ppi.strains:
            ...
            self.converter.convert_gff2rntptt(os.path.join(args_ppi.ptts, datas[0]), '0', os.path.join(args_ppi.ptts, ptt_file), os.path.join(args_ppi.ptts, rnt_file), None, None)
        ...
        for strain_id in strain_ids:
            genes = self._setup_folder_and_read_file(strain_id, pre_file, files, paths, args_ppi)
