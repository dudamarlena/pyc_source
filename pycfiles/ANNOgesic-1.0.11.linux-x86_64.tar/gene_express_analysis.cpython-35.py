...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/gene_express_analysis.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 11624 bytes
import os, sys, copy
...
def detect_express(wigs, gff, cutoff_coverage, detects, percent_tex, percent_frag, texs, cond, tex_notex, track, plots, cover_type, name):
    total = 0
    high = 0
    ...
    if cover_type == 'average':
        ...
        if cover_type == 'high':
            ...
            sys.exit()
        diff, cutoff_percent = set_cutoff(cond, percent_tex, percent_frag, detects, gff)
...
def compare_wigs(wigs, gff, tex_notex, template_texs, replicates, stats, outs, plots, cover_type, cutoff_coverage, percent_tex, percent_frag):
        ...
        if gff.seq_id == strain:
            ...
            for cond, tracks in conds.items():
                ...
                for track, wigs in tracks.items():
                    ...
                    plots[cond][name] = 0
                    detects['express'] = 0
                    detect_express(wigs, gff, cutoff_coverage, detects, percent_tex, percent_frag, texs, cond, tex_notex, track, plots, cover_type, name)

                if 'tex' in cond or 'notex' in cond:
                    if detects['track'] >= replicates['tex']:
                        detects['import'] = True
                elif 'frag' in cond and detects['track'] >= replicates['frag']:
                    detects['import'] = True
                if detects['import']:
            ...
            if detects['cond'] == num_conds:
                ...
                stats[strain]['all'] += 1
                outs['all'].append(gff)
            if detects['cond'] <= num_conds and detects['cond'] > 0:
...
def output_stat(stats, stat_folder, prefix):
    for feature, strains in stats.items():
        out = open(os.path.join(stat_folder, '_'.join([prefix, feature + '.csv'])), 'w')
...
def output_gff(outs, out_gff_folder, prefix):
    for feature, conds in outs.items():
        for cond, gffs in conds.items():
            if cond == 'least_one':
                out = open(os.path.join(out_gff_folder, '_'.join([
...
def deal_repeat_tag(gff, plots, feature, repeat, tag, tags):
    if gff.attributes[tag] in tags and gff.attributes[tag] not in repeat.keys():
        plots[feature].append({gff.attributes[tag] + '_2': {}})
        repeat[gff.attributes[tag]] = 2
        name = gff.attributes[tag] + '_2'
    else:
        if gff.attributes[tag] in tags and gff.attributes[tag] in repeat.keys():
...
def plot(plots, stat_folder, max_color, min_color, cover_type):
    for feature in plots:
        plot_table(plots[feature], max_color, min_color, os.path.join(stat_folder, '_'.join([
         feature, cover_type, 'express_analysis.png'])))


def gene_expression(input_libs, gff_folder, percent_tex, percent_frag, wig_f_file, wig_r_file, features, wigs, cutoff_coverage, tex_notex, replicates, stat_folder, out_gff_folder, cover_type, max_color, min_color):
    ...
    for gff in os.listdir(gff_folder):
        if gff.endswith('.gff'):
            ...
            gff_list, stats, outs = read_data(os.path.join(gff_folder, gff), features)
            for feature, gffs in gff_list.items():
                ...
                for gff in gffs:
                    ...
                    if gff.strand == '+':
                        compare_wigs(wig_fs, gff, tex_notex, texs, replicates, stats[feature], outs[feature], plots[feature][num][name], cover_type, cutoff_coverage, percent_tex, percent_frag)
                    elif gff.strand == '-':
                        compare_wigs(wig_rs, gff, tex_notex, texs, replicates, stats[feature], outs[feature], plots[feature][num][name], cover_type, cutoff_coverage, percent_tex, percent_frag)
                    num += 1
