...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/controller.py
# Compiled at: 2020-01-14 08:21:20
# Size of source mod 2**32: 55047 bytes
import os, sys
...
class Controller(object):
    ...
    def check_folder(self, folders, flags, log):
        ...
        for folder, flag in zip(folders, flags):
            if folder is None:
                log.write('{0} of {1} is not found. Please check it!'.format(folder, flag))
                print('Error: {0} of {1} is not found. Please check it!'.format(folder, flag))
                sys.exit()
                ...
                if files is not None:
                    for file_ in files:
                        if not os.path.exists(file_):
                            print('Error: {0} in {1} does not exist!'.format(file_, flag))
    ...
    def check_parameter(self, paras, names, log):
        ...
        for i in range(len(paras)):
            if paras[i] is None:
                print('Error: {0} can not be None. Please check it!'.format(names[i]))
    ...
    def check_execute_file(self, exe, log):
        ...
        if not detect:
            ...
            sys.exit()
        if os.path.isfile(full_exe) or os.path.isfile(os.path.join(exe_folder, exe)):
            log.write('The execute path is ' + os.popen('which ' + exe).read())
            return full_exe
        ...
        for i in range(len(files)):
            if require:
                if files[i] is None:
                    ...
                    sys.exit()
                elif not os.path.isfile(files[i]):
                    print('Error: {0} is not found. Please check it!'.format(files[i]))
    ...
    def get_input(self):
        ...
        if self._args.ref_gff is True:
            ...
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, 'gff', log)
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, '_genomic.gff.gz', log)
        if self._args.ref_fasta is True:
            log.write('Get fasta files\n')
            get_file(self._args.ftp_path, self._paths.ref_fasta_folder, 'fna', log)
            get_file(self._args.ftp_path, self._paths.ref_fasta_folder, '_genomic.fna.gz', log)
        if self._args.ref_gbk is True:
            log.write('Get gbk files\n')
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, 'gbk', log)
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, 'gbff', log)
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, '_genomic.gbff.gz', log)
        if self._args.ref_ptt is True:
            log.write('Get ptt files\n')
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, 'ptt', log)
        if self._args.ref_rnt is True:
            log.write('Get rnt files\n')
            get_file(self._args.ftp_path, self._paths.ref_annotation_folder, 'rnt', log)
        if self._args.convert_embl is True:
            ...
            log.write('Running converter.py for converting gbk file to embl formet\n')
    ...
    def get_target_fasta(self):
        ...
        target = TargetFasta(self._paths.tar_fasta_folder, self._args.related_fasta_files)
        target.get_target_fasta(self._args.mutation_table, self._paths.tar_fasta_folder, self._args.related_fasta_files, self._args.updated_seq_name, self._paths.target_base_folder, log)
        log.close()
    ...
    def ratt(self):
        ...
        if self._args.transfer_type != 'Strain' and self._args.transfer_type != 'Assembly' and self._args.transfer_type != 'Species' and self._args.transfer_type != 'Assembly.Repetitive' and self._args.transfer_type != 'Strain.Repetitive' and self._args.transfer_type != 'Species.Repetitive' and self._args.transfer_type != 'Multiple' and self._args.transfer_type != 'Free':
            log.write("Incorrect --transfer_type. Please assign 'Assembly', 'Species', 'Assembly.Repetitive', 'Strain.Repetitive', 'Species.Repetitive', 'Multiple' or 'Free'\n")
            print('Error: please assign correct --transfer_type!')
            sys.exit()
        if self._args.related_embl_files is None and self._args.related_gbk_files is None:
            ...
            sys.exit()
        elif self._args.related_embl_files is not None and self._args.related_gbk_files is not None:
            log.write('Please choose --related_gbk_files as input or --related_embl_files as input. Do not assign both.\n')
        ...
        args_ratt = self.args_container.container_ratt(self._args.ratt_path, self._args.element, self._args.transfer_type, self._args.related_embl_files, self._args.related_gbk_files, self._args.target_fasta_files, self._args.related_fasta_files, self._paths.ratt_folder, self._paths.tar_annotation_folder, self._args.compare_pair)
        ratt = RATT(args_ratt)
    ...
    def tsspredator(self):
        ...
        if self._args.program.lower() == 'tss':
            ...
            log = open(os.path.join(self._paths.tsspredator_folder, 'log.txt'), 'w')
            log.write('Running TSS prediction.\n')
        else:
            if self._args.program.lower() == 'ps':
                ...
                log = open(os.path.join(self._paths.processing_site_folder, 'log.txt'), 'w')
        ...
        self._args.tsspredator_path = self.check_execute_file(self._args.tsspredator_path, log)
        args_tss = self.args_container.container_tsspredator(self._args.tsspredator_path, self._args.program, self._args.fasta_files, self._args.annotation_files, self._args.tex_notex_libs, self._args.condition_names, self._args.auto_load_optimized_parameters, self._args.genome_order, self._args.height, self._args.height_reduction, self._args.factor, self._args.factor_reduction, self._args.base_height, self._args.enrichment_factor, self._args.processing_factor, self._args.replicate_tex, out_folder, self._args.validate_gene, self._args.manual_files, self._args.curated_sequence_length, self._args.compare_transcript_files, self._args.tolerance, self._args.utr_length, self._args.cluster, self._args.re_check_orphan, self._args.remove_overlap_feature, self._args.compare_overlap_gff, self._args.remove_low_expression)
    ...
    def optimize(self):
        ...
        if self._args.program.lower() == 'tss':
            ...
            log = open(os.path.join(out_folder, 'optimized_TSSpredator', 'log.txt'), 'w')
    ...
    def color(self):
        ...
        if not os.path.exists(os.path.join(self._args.screenshot_folder, 'screenshots')):
            print('The folder -- screenshots needs to be found in {0}.'.format(self._args.screenshot_folder))
            sys.exit()
        log = open(os.path.join(self._args.screenshot_folder, 'screenshots', 'color_log.txt'), 'w')
        ...
        self._args.imagemagick_covert_path = self.check_execute_file(self._args.imagemagick_covert_path, log)
        color = ColorPNG()
        color.generate_color_png(self._args.track_number, self._args.screenshot_folder, self._args.imagemagick_covert_path, log)
        log.close()
    ...
    def terminator(self):
        ...
        for prop in ('transterm_path', 'expterm_path', 'rnafold_path'):
            setattr(self._args, prop, self.check_execute_file(getattr(self._args, prop), log))

        args_term = self.args_container.container_terminator(self._args.transterm_path, self._args.expterm_path, self._args.rnafold_path, self._paths.transterm_folder, self._args.fasta_files, self._args.annotation_files, self._args.transcript_files, self._args.srna_files, self._args.decrease, self._args.highest_coverage, self._args.tolerance_detect_coverage, self._args.tolerance_within_transcript, self._args.tolerance_downstream_transcript, self._args.tolerance_within_gene, self._args.tolerance_downstream_gene, self._paths.transtermhp_folder, self._args.tex_notex_libs, self._args.frag_libs, self._args.tex_notex, self._args.replicate_tex, self._args.replicate_frag, self._args.min_loop_length, self._args.max_loop_length, self._args.min_stem_length, self._args.max_stem_length, self._args.min_u_tail, self._args.miss_rate, self._args.mutation_u_tail, self._args.keep_multi_term, self._args.window_size, self._args.window_shift)
    ...
    def transcript(self):
        ...
        project_creator.create_subfolders(self._paths.required_folders('transcript'))
    ...
    def utr_detection(self):
        ...
        args_utr = self.args_container.container_utr(self._args.tss_files, self._args.annotation_files, self._args.transcript_files, self._args.terminator_files, self._args.terminator_tolerance, self._paths.utr_folder, self._args.tss_source, self._args.base_5utr, self._args.utr_length, self._args.base_3utr)
        utr = UTRDetection(args_utr)
        utr.run_utr_detection(args_utr, log)

    def _check_filter_input(self, files, info, filters, log):
        if files is None:
            print('Error: The {0} has to be provided if "{1}" in --filter_info!'.format(info, filters))
            log.write('The {0} has to be provided if "{1}" in --filter_info!\n'.format(info, filters))
            sys.exit()
    ...
    def _check_database(self, database, flag, info, log):
        ...
        if database is None:
            ...
            if not os.path.isfile(database):
                if os.path.isfile(database + '.fa') or os.path.isfile(database + '.fna') or os.path.isfile(database + '.fasta'):
                    wrong = False
            else:
                wrong = True
        if wrong:
            print('Error: {0} is required if {1} is in --filter_info. But the assignment of {0} is empty or wrong. Please check the {0} or remove {1} from --filter_info!'.format(flag, info))
            log.write('{0} is required if {1} is in --filter_info. But the assignment of {0} is empty or wrong. Please check the {0} or remove {1} from --filter_info!\n'.format(flag, info))
            sys.exit()
        ...
        for info in self._args.filter_info:
            if 'sec_str' == info:
                if not self._args.compute_sec_structures:
                    ...
                    print('Error: --compute_sec_structures is not switch on, but sec_str is still in --filter_info.')
                    sys.exit()
                self._check_filter_input(self._args.fasta_files, 'fasta file', 'sec_str', log)
                for prop in ('rnafold_path', 'relplot_path', 'mountain_path'):
                    setattr(self._args, prop, self.check_execute_file(getattr(self._args, prop), log))
    ...
    def meme(self):
        ...
        if self._args.program == 'both' or self._args.program == 'meme':
            self._args.meme_path = self.check_execute_file(self._args.meme_path, log)
        elif self._args.program == 'both' or self._args.program == 'glam2':
            self._args.glam2_path = self.check_execute_file(self._args.glam2_path, log)
        args_pro = self.args_container.container_promoter(self._args.meme_path, self._args.glam2_path, self._paths.promoter_output_folder, self._args.tex_libs, self._args.tss_files, self._args.fasta_files, self._args.num_motifs, self._args.nt_before_tss, self._args.motif_width, self._args.tss_source, self._args.annotation_files, self._args.end_run, self._args.combine_all, self._args.e_value, self._args.parallels, self._args.program, self._args.use_tss_type)
        meme = MEME(args_pro)
    ...
    def operon(self):
        ...
        args_op = self.args_container.container_operon(self._args.tss_files, self._args.annotation_files, self._args.transcript_files, self._args.terminator_files, self._args.tss_tolerance, self._args.terminator_tolerance, self._args.min_length, self._paths.operon_output_folder, self._paths.operon_statistics_folder)
    ...
    def circrna(self):
        ...
        log = open(os.path.join(self._paths.circrna_output_folder, 'log.txt'), 'w')
        if self._args.read_files:
            self._args.segemehl_path = self.check_execute_file(self._args.segemehl_path, log)
    ...
    def goterm(self):
        ...
        args_go = self.args_container.container_goterm(self._args.annotation_files, self._paths.goterm_output_folder, self._args.uniprot_id, self._args.go_obo, self._args.goslim_obo, self._args.transcript_files)
    ...
    def srna_target(self):
        ...
        log = open(os.path.join(self._paths.starget_output_folder, 'log.txt'), 'w')
        self.check_multi_files([
         self._args.fasta_files, self._args.srna_files,
    ...
    def snp(self):
        ...
        log = open(os.path.join(self._paths.snp_output_folder, 'log.txt'), 'w')
        self.check_multi_files([
         self._args.fasta_files], [
         '--fasta_files'], log)
        if self._args.bam_type != 'related_genome' and self._args.bam_type != 'reference_genome':
            print('Error: Please assign "related_genome" or "reference_genome" to --bam_type!')
            sys.exit()
        if self._args.ploidy != 'haploid' and self._args.ploidy != 'diploid':
            print('Error: Please assign "haploid" or "diploid" to --chromosome_type!')
        if self._args.caller != 'c' and self._args.caller != 'm':
            print('Error: Please assign "c" or "m" to --caller!')
        for prop in ('bcftools_path', 'samtools_path'):
            setattr(self._args, prop, self.check_execute_file(getattr(self._args, prop), log))

        args_snp = self.args_container.container_snp(self._args.samtools_path, self._args.bcftools_path, self._args.bam_type, self._args.program, self._args.fasta_files, self._args.bam_files, self._args.quality, self._args.read_depth_range, self._paths.snp_output_folder, self._args.indel_fraction, self._args.ploidy, self._args.rg_tag, self._args.caller, self._args.filter_tag_info, self._args.dp4_cutoff)
    ...
    def ppi(self):
        ...
        log = open(os.path.join(self._paths.ppi_output_folder, 'log.txt'), 'w')
        ...
        args_ppi = self.args_container.container_ppi(self._args.annotation_files, self._args.query_strains, self._args.without_strain_pubmed, self._args.species_string, self._args.score, self._paths.ppi_output_folder, self._args.node_size, self._args.query)
    ...
    def sublocal(self):
        ...
        log = open(os.path.join(self._paths.sublocal_output_folder, 'log.txt'), 'w')
        self.check_multi_files([
        ...
        if self._args.bacteria_type != 'positive' and self._args.bacteria_type != 'negative':
            print('Error: Please assign "positive" or "negative" to --bacteria_type!')
            sys.exit()
        self._args.psortb_path = self.check_execute_file(self._args.psortb_path, log)
        args_sub = self.args_container.container_sublocal(self._args.psortb_path, self._args.annotation_files, self._args.fasta_files, self._args.bacteria_type, self._args.difference_multi, self._paths.sublocal_output_folder, self._args.transcript_files)
    ...
    def ribos(self):
        ...
        if self._args.program == 'both':
            ...
            project_creator.create_subfolders(self._paths.required_folders('thermometer'))
            log_r = open(os.path.join(self._paths.ribos_output_folder, 'log.txt'), 'w')
            log_t = open(os.path.join(self._paths.thermo_output_folder, 'log.txt'), 'w')
            ...
            if self._args.program == 'thermometer':
                project_creator.create_subfolders(self._paths.required_folders('thermometer'))
                log_t = open(os.path.join(self._paths.thermo_output_folder, 'log.txt'), 'w')
                self.check_file([self._args.rna_thermometer_id_file,
                 self._args.rfam_path], [
                ...
                if self._args.program == 'riboswitch':
                    project_creator.create_subfolders(self._paths.required_folders('riboswitch'))
                    log_r = open(os.path.join(self._paths.ribos_output_folder, 'log.txt'), 'w')
                    self.check_file([self._args.riboswitch_id_file, self._args.rfam_path], [
                     '--riboswitch_id_file', '--rfam_path'], True, log_r)
                    ...
                    log.write('Please assign "thermometer", "riboswitch" or "both" in --program.\n')
                    print('Error: Please assign "thermometer", "riboswitch" or "both" in --program!')
                    sys.exit()
        ...
        args_ribo = self.args_container.container_ribos(self._args.program, self._args.rna_thermometer_id_file, self._args.cmscan_path, self._args.cmpress_path, self._args.riboswitch_id_file, self._args.annotation_files, self._args.fasta_files, self._args.tss_files, self._args.transcript_files, self._args.rfam_path, ribos_path, thermo_path, self._args.cutoff, self._args.output_all, self._paths.database_folder, self._args.tolerance, self._args.without_rbs, self._args.rbs_seq, self._args.tolerance_rbs, self._args.utr_length)
    ...
    def crispr(self):
        ...
        project_creator.create_subfolders(self._paths.required_folders('crispr'))
        log = open(os.path.join(self._paths.crispr_output_folder, 'log.txt'), 'w')
        self.check_multi_files([
         self._args.fasta_files, self._args.annotation_files], [
         '--fasta_files', '--annotation_files'], log)
        self._args.crt_path = self.check_execute_file(self._args.crt_path, log)
        args_cris = self.args_container.container_cris(self._args.fasta_files, self._args.annotation_files, self._args.crt_path, self._args.window_size, self._args.min_number_repeats, self._args.min_length_repeat, self._args.Max_length_repeat, self._args.min_length_spacer, self._args.Max_length_spacer, self._paths.crispr_output_folder, self._args.ignore_hypothetical_protein)
    ...
    def merge(self):
        ...
        merge_folder = os.path.join(self._paths.output_folder, 'merge_all_features')
        ...
        run_merge(merge_folder, self._args.transcript_file, self._args.other_features_files, self._args.terminator_tolerance, self._args.tss_tolerance, os.path.join(merge_folder, self._args.output_prefix), log)
    ...
    def screen(self):
        ...
        if os.path.exists(out_folder):
            print('Error: The {0} already exists!'.format(out_folder))
            sys.exit()
        else:
            os.mkdir(out_folder)
        ...
        if self._args.output_folder is None:
            ...
            sys.exit()
        if self._args.present != 'expand' and self._args.present != 'collapse' and self._args.present != 'squish':
            log.write('Please assign "expand" or "collapse" or "squish" to --present.\n')
            print('Error: Please assign "expand" or "collapse" or "squish" to --present!')
            sys.exit()
        args_sc = self.args_container.container_screen(self._args.main_gff, self._args.side_gffs, self._args.fasta_file, self._args.height, self._args.tex_notex_libs, self._args.frag_libs, self._args.present, self._args.output_folder)
