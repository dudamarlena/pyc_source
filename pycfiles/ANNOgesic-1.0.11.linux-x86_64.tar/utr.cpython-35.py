...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/utr.py
...
class UTRDetection(object):
    ...
    def _compute_utr(self, args_utr, log):
        ...
        for gff in os.listdir(args_utr.gffs):
            if gff.endswith('.gff'):
                ...
                if args_utr.terms:
                    term = self.helper.get_correct_file(os.path.join(args_utr.terms, 'tmp'), '_term.gff', prefix, None, None)
                else:
                    term = None
                print("Computing 5'UTRs of {0}".format(prefix))
                detect_5utr(tss, os.path.join(args_utr.gffs, gff), tran, os.path.join(self.utr5_path, 'gffs', '_'.join([prefix, '5UTR.gff'])), args_utr)
                print("Computing 3'UTRs of {0}".format(prefix))
                detect_3utr(tran, os.path.join(args_utr.gffs, gff), term, os.path.join(self.utr3_path, 'gffs', '_'.join([prefix, '3UTR.gff'])), args_utr)
                self.helper.move_all_content(os.getcwd(), self.utr5_stat_path, ['_5utr_length.png'])
    ...
    def run_utr_detection(self, args_utr, log):
        ...
        self.multiparser.combine_gff(args_utr.gffs, self.tran_path, None, 'transcript')
        if args_utr.terms:
            self.multiparser.parser_gff(args_utr.terms, 'term')
            self.multiparser.combine_gff(args_utr.gffs, os.path.join(args_utr.terms, 'tmp'), None, 'term')
