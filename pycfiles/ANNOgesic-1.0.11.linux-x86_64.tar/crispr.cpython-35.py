...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/crispr.py
...
class Crispr(object):
    ...
    def __init__(self, args_cris):
        ...
        self.all_out = os.path.join(args_cris.out_folder, 'gffs', 'all_candidates')
        self.best_out = os.path.join(args_cris.out_folder, 'gffs', 'best_candidates')
    ...
    def _run_crt(self, args_cris, log):
        ...
        for seq in os.listdir(self.fasta_path):
            ...
            call(['java', '-cp', args_cris.crt_path, 'crt', '-minNR',
             ...
             str(args_cris.win_size), os.path.join(self.fasta_path, seq),
             os.path.join(self.data_folder, prefix + '.txt')])
            log.write('\t' + os.path.join(self.data_folder, prefix + '.txt') + ' is generated.\n')

    def _read_gff(self, txt):
        gffs = []
        gh = open(os.path.join(self.gff_path, txt.replace('.txt', '.gff')), 'r')
    ...
    def _compare_gff(self, strain, start, end, gffs, bh, indexs, ignore_hypo):
        ...
        id_ = None
        for gff in gffs:
            if gff.seq_id == strain and (gff.start <= start and gff.end >= end or gff.start >= start and gff.end <= end or gff.start <= start and gff.end > start and gff.end <= end or gff.start >= start and gff.start < end and gff.end >= end) and 'product' in gff.attributes.keys() and (not ignore_hypo and 'hypothetical protein' in gff.attributes['product'] or 'hypothetical protein' not in gff.attributes['product']):
                overlap = True
    ...
    def _print_repeat(self, row, strain, file_h, indexs, id_, best):
        ...
        if best:
            ...
            num = indexs['re_all']
        if not row[0].startswith('-') and not row[0].startswith('Repeats:') and not row[0].startswith('CRISPR') and not row[0].startswith('POSITION'):
            start = row[0].strip()
    ...
    def _convert_gff(self, ignore_hypo):
        ...
        for txt in os.listdir(self.data_folder):
            ...
            oh = open(os.path.join(self.all_out, txt.replace('.txt', '_CRISPR.gff')), 'w')
            bh = open(os.path.join(self.best_out, txt.replace('.txt', '_CRISPR.gff')), 'w')
            ...
            for row in csv.reader(fh, delimiter='\t'):
                if len(row) != 0:
                    ...
                    overlap, over_id = self._compare_gff(strain, int(start), int(end), gffs, bh, indexs, ignore_hypo)
                    indexs['all'] += 1
                    indexs['run'] = True
                if indexs['run']:
                    indexs['re_all'] = self._print_repeat(row, strain, oh, indexs, id_, False)
                    if not overlap:
                        indexs['re_best'] = self._print_repeat(row, strain, bh, indexs, over_id, True)
    ...
    def _print_file(self, sh, cri_res_all, cri_res_best):
        ...
        sh.write('\tthe number of CRISPR which not overlap with genome annotation - {0}\n'.format(cri_res_best['cri']))
            ...
            if len(strains) == 1:
                ...
                if len(strains) <= 2:
                    for strain, cri_res in strains.items():
                        if strain != 'all':
                            sh.write(strain + ':\n')
                            self._print_file(sh, cri_res, stats['best'][prefix][strain])

                else:
                    sh.write('All strains:\n')
                    self._print_file(sh, stats['all'][prefix]['all'], stats['best'][prefix]['all'])
                    for strain, cri_res in strains.items():
                        if strain != 'all':
                            sh.write(strain + ':\n')
    ...
    def run_crispr(self, args_cris, log):
        ...
        self.multiparser.combine_gff(args_cris.gffs, self.all_out, None, 'CRISPR')
        print('Best candidates:')
        self.multiparser.combine_gff(args_cris.gffs, self.best_out, None, 'CRISPR')
