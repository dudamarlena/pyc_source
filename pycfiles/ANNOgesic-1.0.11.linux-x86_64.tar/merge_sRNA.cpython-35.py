...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/merge_sRNA.py
# Compiled at: 2020-03-10 05:52:19
# Size of source mod 2**32: 25033 bytes
import os, csv, math, copy, numpy as np
...
def modify_attributes(pre_srna, srna, srna_type, input_type):
    if srna_type == 'UTR' or srna_type == 'both':
        if pre_srna.attributes['sRNA_type'] != srna.attributes['sRNA_type'] and input_type == 'pre' and 'antisense' in pre_srna.attributes['sRNA_type']:
            pre_srna.attributes['sRNA_type'] = srna.attributes['sRNA_type']
    elif 'antisense' not in pre_srna.attributes['sRNA_type']:
...
def detect_overlap(srna, pre_srna, srna_type, overlap):
    ...
    if srna.seq_id == pre_srna.seq_id and srna.strand == pre_srna.strand:
        if pre_srna.start >= srna.start and pre_srna.end <= srna.end:
            modify_attributes(pre_srna, srna, srna_type, None)
            overlap = True
        else:
            if pre_srna.start >= srna.start and pre_srna.start <= srna.end and pre_srna.end >= srna.end:
                modify_attributes(pre_srna, srna, srna_type, None)
                overlap = True
            else:
                if pre_srna.start <= srna.start and pre_srna.end >= srna.start and pre_srna.end <= srna.end:
                    modify_attributes(pre_srna, srna, srna_type, None)
                    overlap = True
                elif pre_srna.start <= srna.start and pre_srna.end >= srna.end:
                    overlap = True
...
def merge_tss_pro(pre_srna, srna, feature):
    if feature not in pre_srna.attributes.keys() and feature in srna.attributes.keys():
        if srna.attributes[feature] != 'NA':
            pre_srna.attributes[feature] = srna.attributes[feature]
    else:
        if feature in pre_srna.attributes.keys() and feature in srna.attributes.keys():
            if pre_srna.attributes[feature] == 'NA' and srna.attributes[feature] != 'NA':
                pre_srna.attributes[feature] = srna.attributes[feature]
        elif srna.attributes[feature] not in pre_srna.attributes[feature] and srna.attributes[feature] != 'NA':
...
def modify_overlap(pre_srna, srna):
    ...
    if srna.attributes['sRNA_type'] == '5utr' or srna.attributes['sRNA_type'] == '3utr' or srna.attributes['sRNA_type'] == 'interCDS':
...
def merge_srna(srnas, srna_type):
    ...
    for srna in srnas:
        ...
        if first:
            first = False
            pre_srna = srna
        elif srna.seq_id != pre_srna.seq_id:
            pass
        if not overlap:
            ...
            if overlap:
                ...
                if srna.attributes['sRNA_type'] != 'antisense' and pre_srna.attributes['sRNA_type'] == 'antisense':
                    pre_srna = srna
            else:
                if pre_srna not in final_srnas:
                    final_srnas.append(pre_srna)
    ...
    if os.path.exists(gff_file):
        for entry in Gff3Parser().entries(open(gff_file)):
            if type_ == 'sRNA':
                ...
                if type_ == 'tss':
                    datas.append(entry)
                elif Helper().feature_without_notgene(entry):
...
def compare_srna_cds(srna, cdss, cutoff_overlap):
    ...
    overlap = False
    for cds in cdss:
        if srna.seq_id == cds.seq_id and srna.strand == cds.strand and (srna.end < cds.end and srna.end > cds.start and srna.start <= cds.start or srna.start > cds.start and srna.start < cds.end and srna.end >= cds.end or srna.end >= cds.end and srna.start <= cds.start or srna.end <= cds.end and srna.start >= cds.start):
            overlap = True
            per_c = float(min(srna.end, cds.end) - max(srna.start, cds.start)) / float(cds.end - cds.start)
            if per_c <= cutoff_overlap:
                ...
                if 'overlap_cds' not in srna.attributes.keys():
                    ...
                    srna.attributes['overlap_cds'] = ','.join([srna.attributes['overlap_cds'],
                     cds_name])
                    srna.attributes['overlap_percent'] = ','.join([srna.attributes['overlap_percent'],
                     str(per_c)])
                detect = True
...
def merge_srna_gff(gffs, in_cds, cutoff_overlap, gff_file, ex_srna):
    ...
    for srna in sort_srnas:
        ...
        if new_srna:
            new_srna.attributes['ID'] = new_srna.seq_id + '_srna' + str(num_srna)
            name = '%0*d' % (5, num_srna)
            ...
            attribute_string = ';'.join(['='.join(items) for items in new_srna.attributes.items()])
            ...
            out.write(new_srna.info_without_attributes + '\t' + attribute_string + '\n')
            num_srna += 1
...
def get_cutoff(srna, tsss, type_, tables, args_srna):
    if type_ == 'inter':
        ...
        if tsss is not None:
            for tss in tsss:
                if srna.seq_id == tss.seq_id and srna.strand == tss.strand:
                    if srna.strand == '+':
                        if math.fabs(srna.start - tss.start) <= args_srna.fuzzy_inter:
                            tss_type = tss.attributes['type']
                            if srna.start == tss.start:
                                break
                elif math.fabs(srna.end - tss.start) <= args_srna.fuzzy_inter:
                    tss_type = tss.attributes['type']
        ...
        for row in csv.reader(fh, delimiter='\t'):
            if row[0] == srna.seq_id and row[1] == srna.attributes['sRNA_type']:
...
def devide_covers(covers):
    ...
    return (
     frag_covers, tex_covers)


def merge_srna_datas(srna_datas_tex, srna_datas_frag):
    if len(srna_datas_tex['conds']) != 0 and len(srna_datas_frag['conds']) != 0:
        ...
        if len(srna_datas_tex['conds']) != 0:
            srna_datas = copy.deepcopy(srna_datas_tex)
        else:
            if len(srna_datas_frag['conds']) != 0:
...
def compare_table(srna, tables, type_, wigs_f, wigs_r, texs, out, tsss, args_srna):
    detect = False
    ...
    if not detect:
        if type_ == 'inter':
            ...
            srna_datas_tex = replicate_comparison(args_srna, tex_covers, srna.strand, 'normal', None, None, None, cut['notex'], cut['tex'], texs)
            srna_datas_frag = replicate_comparison(args_srna, frag_covers, srna.strand, 'normal', None, None, None, None, cut['frag'], texs)
        ...
        if srna.strand == '+':
            ...
            covers = get_coverage(wigs_r, srna)
        cut = get_cutoff(srna, tsss, type_, tables, args_srna)
        srna_datas = replicate_comparison(args_srna, covers, srna.strand, 'sRNA_utr_derived', cut[srna.attributes['sRNA_type']], cut, srna.attributes['sRNA_type'], None, cut, texs)
    if len(srna_datas['conds']) != 0:
        out.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t'.format(srna.seq_id, srna.attributes['Name'], srna.start, srna.end, srna.strand, ';'.join(srna_datas['conds'].keys()), ';'.join(srna_datas['conds'].values()), tss_pro, end_pro, srna_datas['best']))
        first = True
        for data in srna_datas['detail']:
            if first:
                out.write('{0}({1})'.format(data['track'], data['avg']))
                first = False
            else:
                out.write(';{0}({1})'.format(data['track'], data['avg']))
        ...
        if wig_strain == srna.seq_id:
            for cond, tracks in conds.items():
                ...
                for lib_name, covers in tracks.items():
                    ...
                    cover_sets['diff'] = 0
                    first = True
                    c_start, c_end = check_start_and_end(srna.start, srna.end, covers)
                    ...
                    for cover in covers:
                        if lib_strand == srna.strand:
                            if srna.strand == '+':
                                ...
                                cover_pos = c_end - pos
                            if srna.start <= cover_pos and srna.end >= cover_pos:
                                cover_sets['total'] = cover_sets['total'] + cover
                                first = coverage_comparison(cover, cover_sets, poss, first, srna.strand, cover_pos)
                            else:
                                if srna.strand == '+' and cover_pos > srna.end:
                    ...
                    avg = cover_sets['total'] / float(srna.end - srna.start + 1)
...
def get_tss_pro(type_, srna):
    ...
    if type_ == 'utr':
        ...
        if 'end_cleavage' not in srna.attributes.keys():
            srna.attributes['end_cleavage'] = 'NA'
        if srna.attributes['with_TSS'] != 'NA' and srna.attributes['start_cleavage'] != 'NA':
            ...
            if srna.attributes['with_TSS'] != 'NA':
                ...
                if srna.attributes['start_cleavage'] != 'NA':
...
def merge_srna_table(srna_file, csvs, wigs_f, wigs_r, tss_file, args_srna):
    ...
    for srna in srnas:
        if '5utr' in srna.attributes['sRNA_type'] or '3utr' in srna.attributes['sRNA_type'] or 'interCDS' in srna.attributes['sRNA_type']:
            compare_table(srna, utrs, 'utr', wigs_f, wigs_r, texs, out, tsss, args_srna)
        elif 'intergenic' in srna.attributes['sRNA_type'] or 'in_CDS' in srna.attributes['sRNA_type'] or 'antisense' in srna.attributes['sRNA_type']:
            compare_table(srna, inters, 'inter', wigs_f, wigs_r, texs, out, tsss, args_srna)
