...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/sorf.py
...
class sORFDetection(object):
    ...
    def _check_necessary_files(self, args_sorf, log):
        if args_sorf.gffs is None or args_sorf.trans is None or args_sorf.tex_wigs is None and args_sorf.frag_wigs is None:
            print('Error: lack required files!')
            log.write('genome annotation, transcript file or wiggle files are not assigned.\n')
            sys.exit()
        if args_sorf.utr_detect and args_sorf.tsss is None:
            print('Error: TSS files are required for UTR derived sORF detection!')
            log.write('TSS files are required for UTR derived sORF detection!\n')
            sys.exit()
        ...
        if args_sorf.tsss is not None:
            ...
            self.multiparser.combine_gff(args_sorf.gffs, self.tss_path, None, 'TSS')
        self._check_gff(args_sorf.trans)
        if args_sorf.srnas is not None:
            self._check_gff(args_sorf.srnas)
            self.multiparser.parser_gff(args_sorf.srnas, 'sRNA')
            self.multiparser.combine_gff(args_sorf.gffs, self.srna_path, None, 'sRNA')
    ...
    def _start_stop_codon(self, prefixs, args_sorf, log):
        ...
        for prefix in prefixs:
            ...
            if self.srna_path is not None:
                srna_file = os.path.join(self.srna_path, '_'.join([prefix, 'sRNA.gff']))
            else:
                srna_file = None
            if self.tss_path is not None:
                tss_file = os.path.join(self.tss_path, '_'.join([prefix, 'TSS.gff']))
            else:
                tss_file = None
            sorf_detection(os.path.join(self.fasta_path, prefix + '.fa'), srna_file, os.path.join(args_sorf.out_folder, '_'.join([prefix, 'inter.gff'])), tss_file, os.path.join(args_sorf.wig_path, '_'.join([prefix, 'forward.wig'])), os.path.join(args_sorf.wig_path, '_'.join([prefix, 'reverse.wig'])), os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF'])), args_sorf)
            if '_'.join([prefix, 'sORF_all.gff']) in os.listdir(os.path.join(self.gff_output, self.all_cand)):
                gff_all = os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF.gff']))
                gff_best = os.path.join(self.gff_output, self.best, '_'.join([prefix, 'sORF.gff']))
                csv_all = os.path.join(self.table_output, self.all_cand, '_'.join([prefix, 'sORF.csv']))
                csv_best = os.path.join(self.table_output, self.best, '_'.join([prefix, 'sORF.csv']))
                shutil.move(os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF_all.gff'])), gff_all)
                shutil.move(os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF_best.gff'])), gff_best)
                shutil.move(os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF_all.csv'])), csv_all)
                shutil.move(os.path.join(self.gff_output, self.all_cand, '_'.join([prefix, 'sORF_best.csv'])), csv_best)
                log.write('\t' + gff_all + '\n')
                log.write('\t' + gff_best + '\n')
                log.write('\t' + csv_all + '\n')
                log.write('\t' + csv_best + '\n')
    ...
    def _compare_tran_cds(self, args_sorf, log):
        ...
        prefixs = []
        log.write('Running sORF_intergenic.py to extract the sequences of potential sORFs\n')
        for gff in os.listdir(args_sorf.gffs):
            if gff.endswith('.gff'):
                prefix = gff.replace('.gff', '')
                prefixs.append(prefix)
                print('Comparing transcripts and CDSs of {0}'.format(prefix))
                get_intergenic(os.path.join(args_sorf.gffs, gff), os.path.join(self.tran_path, '_'.join([prefix, 'transcript.gff'])), os.path.join(args_sorf.out_folder, '_'.join([prefix, 'inter.gff'])), args_sorf.utr_detect, args_sorf.hypo, args_sorf.extend_5, args_sorf.extend_3)
                log.write('\t' + os.path.join(args_sorf.out_folder, '_'.join([prefix, 'inter.gff'])) + ' is generated to temporary store the sequences.\n')
    ...
    def _re_table(self, args_sorf, prefixs, log):
        ...
        for type_ in ['all_candidates', 'best_candidates']:
            for prefix in prefixs:
                table_file = os.path.join(args_sorf.out_folder, 'tables', type_, '_'.join([
                 prefix, 'sORF.csv']))
                reorganize_table(args_sorf.libs, args_sorf.merge_wigs, 'Track_detail', table_file)
    ...
    def run_sorf_detection(self, args_sorf, log):
        ...
        self.multiparser.combine_gff(args_sorf.gffs, self.tran_path, None, 'transcript')
        ...
        for sorf in os.listdir(os.path.join(self.gff_output, self.all_cand)):
            ...
            if sorf.endswith('_sORF.gff'):
                stat_file = os.path.join(args_sorf.out_folder, 'statistics', '_'.join(['stat', sorf.replace('.gff', '.csv')]))
                stat(os.path.join(self.gff_output, self.all_cand, sorf), os.path.join(self.gff_output, self.best, sorf), stat_file, args_sorf.utr_detect)
