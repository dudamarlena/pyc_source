...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/ribo_gff.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 7006 bytes
import csv
...
def get_overlap(pre_ribo, ribo, overlap, overlaps):
    if pre_ribo['strain'] == ribo['strain'] and pre_ribo['strand'] == ribo['strand'] and pre_ribo['ID'] == ribo['ID']:
        overlap = True
    if overlap:
        detect = False
...
def print_number(stats, repeat, out, strain, feature):
    out.write('Total number of potential {0} are {1}\n'.format(feature.replace('_', ' '), stats[strain]['total']))
    out.write('The number of potential {0} which have overlap region with others are {1}\n'.format(feature.replace('_', ' '), repeat))
...
def stat_and_covert2gff(ribo_table, rfam_table, gff_file, fuzzy, out_stat, feature):
    ...
    for ribo in ribos:
        ...
        if pre_gff is not None:
            if pre_gff['strain'] == ribo['strain'] and pre_gff['strand'] == ribo['strand'] and pre_gff['start_seq'] == ribo['start_seq'] and pre_gff['end_seq'] == ribo['end_seq']:
