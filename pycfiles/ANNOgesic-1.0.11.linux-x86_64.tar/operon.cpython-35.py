...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/operon.py
...
class OperonDetection(object):
    ...
    def _detect_operon(self, prefixs, args_op, log):
        ...
        for prefix in prefixs:
            out_gff = os.path.join(args_op.output_folder, 'gffs', '_'.join([prefix, 'operon.gff']))
            out_table = os.path.join(self.table_path, '_'.join([prefix, 'operon.csv']))
            ...
            gff = self.helper.get_correct_file(args_op.gffs, '.gff', prefix, None, None)
            if self.term_path is None:
                term = False
            else:
                term = self.helper.get_correct_file(self.term_path, '_term.gff', prefix, None, None)
            operon(tran, tss, gff, term, args_op.tss_fuzzy, args_op.term_fuzzy, args_op.length, out_table, out_gff)
            log.write('\t' + out_table + '\n')
            log.write('\t' + out_gff + '\n')
    ...
    def _check_and_parser_gff(self, args_op):
        ...
        self.multiparser.combine_gff(args_op.gffs, self.tran_path, None, 'transcript')
        if args_op.tsss is not None:
        ...
        if args_op.terms is not None:
            ...
            self.multiparser.combine_gff(args_op.gffs, self.term_path, None, 'term')
