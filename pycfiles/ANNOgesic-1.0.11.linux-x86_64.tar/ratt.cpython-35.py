...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/ratt.py
...
class RATT(object):
    ...
    def _convert_to_pttrnt(self, gffs, files, log):
        for gff in files:
            if gff.endswith('.gff'):
                ...
                rnt = gff[:-3] + 'rnt'
                ptt = gff[:-3] + 'ptt'
                fasta = self.helper.get_correct_file(self.tmp_files['tar'], '.fa', prefix, None, None)
                if fasta:
    ...
    def _remove_files(self, args_ratt, out_gbk, log):
        ...
        self.helper.move_all_content(self.tmp_files['out_gff'], args_ratt.gff_outfolder, None)
    ...
    def _convert_to_gff(self, ratt_result, args_ratt, files, log):
        ...
        self.converter.convert_embl2gff(os.path.join(args_ratt.output_path, ratt_result), output_file)
        self.format_fixer.fix_ratt(output_file, '.'.join(name[1:-2]), 'tmp_gff')
        shutil.move('tmp_gff', output_file)
        shutil.copy(output_file, os.path.join(args_ratt.gff_outfolder, filename))
        log.write('\t' + os.path.join(args_ratt.gff_outfolder, filename) + ' is generated.\n')
            ...
            with open(file_, 'r') as (f_h):
                for line in f_h:
                    ...
                    if line.startswith('//'):
                        out.close()
                        close = True
                        shutil.move(self.gbk_tmp, os.path.join(self.gbk, filename))
    ...
    def _convert_embl(self, ref_embls, log):
        ...
        for embl in os.listdir(ref_embls):
            if embl.endswith('.gbk') or embl.endswith('.gbff') or embl.endswith('.gb'):
                detect_gbk = True
    ...
    def _run_ratt(self, args_ratt, tar, ref, out, log):
        if not os.path.exists(self.embl) or not os.path.exists(os.path.join(self.tmp_files['tar'], tar + '.fa')) or not os.path.exists(os.path.join(self.tmp_files['ref'], ref + '.fa')):
            print('Error: Please check --compare_pair, the strain names should be the same as the strain names in fasta, genbank or embl files!')
            log.write('The strain names in --compare_pair should be the same as the strain names in fasta, genbank, or embl files.\n')
            sys.exit()
        ...
        call([args_ratt.ratt_path, self.embl,
         os.path.join(self.tmp_files['tar'], tar + '.fa'),
         args_ratt.element, args_ratt.transfer_type,
         os.path.join(self.tmp_files['ref'], ref + '.fa')], stdout=out, stderr=DEVNULL)
        log.write('Done!\n')
        ...
        for pair in args_ratt.pairs:
            ...
            for filename in os.listdir():
                if 'final' in filename:
                    log.write('\t' + filename + '\n')
                    shutil.move(filename, os.path.join(args_ratt.output_path, filename))
                elif args_ratt.element in filename or 'query' in filename or 'Reference' in filename or 'Query' in filename or 'Sequences' in filename:
                    log.write('\t' + filename + '\n')
    ...
    def annotation_transfer(self, args_ratt, log):
        ...
        for data in os.listdir(args_ratt.output_path):
            if 'final.embl' in data:
                log.write('Running converter.py to convert embl files in {0} to gff, ptt, and rnt format.\n'.format(data))
        ...
        for folder in os.listdir(args_ratt.tar_fastas):
            ...
            if '_folder' in folder:
                ...
                for file_ in os.listdir(os.path.join(args_ratt.tar_fastas, folder)):
                    files.append(file_[:-3])

                for gff in os.listdir(args_ratt.gff_outfolder):
                    for file_ in files:
                        if '.gff' in gff and file_ == gff[:-4]:
                            self.helper.merge_file(os.path.join(args_ratt.gff_outfolder, gff), self.tmp_files['gff'])
                        if '.ptt' in gff and file_ == gff[:-4]:
                            self.helper.merge_file(os.path.join(args_ratt.gff_outfolder, gff), self.tmp_files['ptt'])
                        if '.rnt' in gff and file_ == gff[:-4]:
                            self.helper.merge_file(os.path.join(args_ratt.gff_outfolder, gff), self.tmp_files['rnt'])

                if os.path.exists(self.tmp_files['gff']):
                    shutil.move(self.tmp_files['gff'], os.path.join(self.tmp_files['out_gff'], prefix + '.gff'))
                    shutil.move(self.tmp_files['ptt'], os.path.join(self.tmp_files['out_gff'], prefix + '.ptt'))
                    shutil.move(self.tmp_files['rnt'], os.path.join(self.tmp_files['out_gff'], prefix + '.rnt'))
                else:
                    print('Error: Please check your fasta or annotation files, they should only contain the query genome. And make sure your RATT can work properly (check $ANNOgesic/output/annotation_transfer/ratt_log.txt).')
                    log.write('Please check your fasta or annotation files, they should only contain the query genome. And make sure your RATT can work properly (check $ANNOgesic/output/annotation_transfer/ratt_log.txt).\n')
