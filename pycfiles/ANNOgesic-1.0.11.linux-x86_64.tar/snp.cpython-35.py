...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/snp.py
...
class SNPCalling(object):
    ...
    def __init__(self, args_snp):
        ...
        self.stat_path = os.path.join(args_snp.out_folder, file_type, 'statistics')
        self.fig_path = os.path.join(self.stat_path, 'figs')
        self.helper.check_make_folder(self.fig_path)
        self.outputs = {'table': os.path.join(args_snp.out_folder, file_type, 'SNP_tables'), 
         
         'raw': os.path.join(args_snp.out_folder, file_type, 'SNP_raw_outputs'), 
         
         'tmp': os.path.join(args_snp.out_folder, 'tmp_bcf'), 
         'depth': os.path.join(args_snp.out_folder, 'tmp_depth')}
        self.bams = {'whole': os.path.join(args_snp.out_folder, 'whole_reads.bam'), 
    ...
    def _transcript_snp(self, fasta, out_table_prefix, type_, prefix, bam_datas, table_path, args_snp):
        ...
        for bam in bam_datas:
            ...
            snp_detect(fasta, snp_file, self.outputs['depth'] + bam['sample'], '_'.join([out_table_prefix, bam['sample']]), os.path.join(seq_path, '_'.join([prefix, bam['sample']])), bam['bam_number'], stat_prefix, args_snp, bam['rep'])
    ...
    def _run_tools(self, fasta_file, type_, args_snp, bam_datas, log):
        ...
        for bam in bam_datas:
            bam_file = os.path.join(args_snp.out_folder, bam['sample'] + '.bam')
            if type_ == 'with':
                command = [
                 args_snp.samtools_path, 'mpileup', '-t', 'DP']
            else:
                if type_ == 'without':
                    command = [
                     args_snp.samtools_path, 'mpileup', '-t', 'DP', '-B']
                elif type_ == 'extend':
                    command = [
                     args_snp.samtools_path, 'mpileup', '-t', 'DP', '-E']
                if args_snp.rg:
        ...
        for fa in os.listdir(args_snp.fastas):
            if fa != 'all.fa' and not fa.endswith('.fai'):
                ...
                if num_prog == 0:
                    ...
                    self.helper.check_make_folder(os.path.join(self.outputs['table'], fa_prefix))
                self.helper.check_make_folder(os.path.join(self.seq_path, self.baqs[type_], fa_prefix))
                for vcf in os.listdir(self.outputs['raw']):
                    if vcf.endswith('.vcf'):
                        out = open(os.path.join(vcf_folder, '_'.join([
                         fa_prefix, vcf])), 'w')
                        with open(os.path.join(self.outputs['raw'], vcf)) as (vh):
                            for line in vh:
                                line = line.strip()
                                if line.startswith('#'):
                                    out.write(line + '\n')
                                elif line.split('\t')[0] in seq_names:
                                    out.write(line + '\n')

                        out.close()
                        log.write('\t' + os.path.join(vcf_folder, '_'.join([fa_prefix, vcf])) + ' is generated.\n')
    ...
    def _run_sub(self, args_snp, all_fasta, type_, bam_datas, num_prog, log):
        ...
        log.write('Running transcript_SNP.py to do statistics, filter SNPs, and generate potential sequences.\n')
        log.write('The following files are generated:\n')
        for fa_prefix in fa_prefixs:
            ...
            self._transcript_snp(fasta_file, table_prefix, type_, fa_prefix, bam_datas, table_path, args_snp)
    ...
    def _run_program(self, all_fasta, bam_datas, args_snp, log):
        num_prog = 0
        log.write('Running Samtools to mpileup, and using Bcftools to call snp.\n')
            ...
            if index == 'with_BAQ':
                ...
                if index == 'without_BAQ':
                    ...
                    if index == 'extend_BAQ':
                        ...
                        print('Error: No correct program, please assign "with_BAQ", "without_BAQ", "extend_BAQ"!')
                        log.write('No valid program can be found, please assign"with_BAQ", "without_BAQ", "extend_BAQ".\n')
                        sys.exit()
    ...
    def _merge_bams(self, args_snp, bam_datas, log):
        ...
        for bam in bam_datas:
            ...
            if len(bam['bams']) == 1:
                ...
                self._run_bam(args_snp.samtools_path, 'sort', out_bam, bam['bams'][0], log)
                bam['bam_number'] = 1
            else:
                print('Merging BAM files of ' + bam['sample'])
                self._run_bam(args_snp.samtools_path, 'merge', ' '.join(bam['bams']), 'all', log)
                print('Sorting BAM files of ' + bam['sample'])
                self._run_bam(args_snp.samtools_path, 'sort', out_bam, 'all', log)
                bam['bam_number'] += 1
    ...
    def _modify_header(self, fastas):
        for fasta in os.listdir(fastas):
            if fasta.endswith('fasta') or fasta.endswith('fa') or fasta.endswith('fna'):
    ...
    def _get_header(self, samtools_path, bam, seq_names):
        ...
        for row in csv.reader(fh, delimiter='\t'):
            if row[0] == '@SQ' and row[1].split(':')[1] not in seq_names:
        ...
        for bam in bams:
            ...
            for file_ in datas[(-1)].split(','):
                if not os.path.exists(file_):
                    print('Error: there are some Bam files which do not exist!')
    ...
    def run_snp_calling(self, args_snp, log):
        ...
        if 'with_BAQ' not in args_snp.program and 'without_BAQ' not in args_snp.program and 'extend_BAQ' not in args_snp.program:
            print('Error: Please assign a correct programs: "with_BAQ", "without_BAQ", "extend_BAQ".')
            sys.exit()
        else:
