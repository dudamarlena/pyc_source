...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/goterm.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 7777 bytes
...
class GoTermFinding(object):
    __doc__ = 'Retrieving the GO term'
    ...
    def __init__(self, args_go):
        ...
        self.result_express_path = os.path.join(self.out_express, 'GO_term_results')
    ...
    def _retrieve_go(self, uniprot, out_path, type_, log):
        ...
        for gff in os.listdir(self.gff_path):
            ...
            if self.tran_path is not None:
                ...
                tran_file = None
            retrieve_uniprot(uniprot, os.path.join(self.gff_path, gff), out_file, tran_file, type_)
    ...
    def _merge_files(self, gffs, out_path, out_folder, log):
        ...
        folders = []
        log.write('Merging the output files based on the input genome information.\n')
            ...
            if folder.endswith('gff_folder'):
                ...
                if len(filenames) > 1:
                    ...
                    for filename in filenames:
                        ...
                        self.helper.merge_file(os.path.join(out_path, filename, csv_file), out_all)
                        self._remove_header(out_all)
                        shutil.copy(os.path.join(out_path, filename, csv_file), folder_path)

                else:
                    shutil.copyfile(os.path.join(out_path, filenames[0], '_'.join([filenames[0], 'uniprot.csv'])), out_all)
        ...
        for folder in folders:
            ...
            for file_ in os.listdir(os.path.join(out_path, folder_prefix)):
                log.write('\t' + os.path.join(out_path, folder_prefix, file_) + ' is generated.\n')

    def _stat(self, out_path, stat_path, go, goslim, out_folder, log):
        log.write('Running gene_ontology.py to Retrieve GOslim terms and do statistics.\n')
        ...
        for folder in os.listdir(out_path):
            ...
            if 'fig' not in os.listdir(strain_stat_path):
                os.mkdir(fig_path)
            stat_file = os.path.join(strain_stat_path, '_'.join(['stat', folder + '.csv']))
            map2goslim(goslim, go, os.path.join(out_path, folder, self.all_strain), stat_file, out_folder)
            log.write('\t' + stat_file + '\n')
    ...
    def run_go_term(self, args_go, log):
        for gff in os.listdir(args_go.gffs):
            if gff.endswith('.gff'):
                self.helper.check_uni_attributes(os.path.join(args_go.gffs, gff))
        ...
        self._stat(self.result_all_path, self.stat_all_path, args_go.go, args_go.goslim, self.out_all, log)
        if args_go.trans is not None:
            log.write('Retrieving GO terms only for expressed CDSs.\n')
            print('Computing express CDSs')
            self._retrieve_go(args_go.uniprot, self.result_express_path, 'express', log)
            self._merge_files(args_go.gffs, self.result_express_path, self.out_express, log)
            self._stat(self.result_express_path, self.stat_express_path, args_go.go, args_go.goslim, self.out_express, log)
