...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/plot_TSS_venn.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 6304 bytes
...
def ellipse(x, y, angle, face, al, plt):
    ellipse_ = mpl.patches.Ellipse(xy=(x, y), width=0.8, height=0.3, angle=angle, facecolor=face, alpha=al)
    plt.gca().add_artist(ellipse_)
    return plt


def line(x, y, angle, plt):
    line_ = mpl.patches.Ellipse(xy=(x, y), width=0.8, height=0.3, angle=angle, facecolor='none', edgecolor='#000000', linewidth=3)
    plt.gca().add_artist(line_)
    return plt


def plot_text(plt, xy1, xy2, tss_type, size, color_text):
    plt.text(xy1, xy2, tss_type, ha='center', va='center', fontsize=15, fontweight='bold', color=color_text)
...
def text(xy, tss_type, num, plt):
    if tss_type == 'Primary' or tss_type == 'Antisense' or tss_type == 'Antisense_Primary':
...
def plot(types, file_type, feature_name, total_types, tss_num):
    for strain, tss_types in types.items():
        if len(types.keys()) <= 2 and strain == 'all':
            pass
        else:
            ...
            circ = mpl.patches.Ellipse(xy=(0.8, 0.2), width=0.09, height=0.15, facecolor='none', edgecolor='#000000', linewidth=3)
