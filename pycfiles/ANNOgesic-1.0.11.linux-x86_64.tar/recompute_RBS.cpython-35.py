...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/recompute_RBS.py
...
def print_file(ribos, out_t, out_s, seq_name, seqs):
    if len(ribos) != 0:
        for rbs in ribos:
            if rbs['detect'] == '!':
                out_t.write('\t'.join(seq_name.split('|') + [
            ...
            for seq in seqs:
                if rbs['seq_name'] == seq['name']:
                    ...
                    if rbs['end'] > rbs['start'] - 1:
                        if tags[2] == '+':
                            out_s.write('>' + '|'.join(['|'.join(tags[0:-2]), str(int(tags[(-2)]) + rbs['start'] - 1), str(int(tags[(-2)]) + rbs['end'] - 1)]) + '\n')
                        else:
                            out_s.write('>' + '|'.join(['|'.join(tags[0:-2]), str(int(tags[(-1)]) - rbs['end'] + 1), str(int(tags[(-1)]) - rbs['start'] + 1)]) + '\n')
            ...
            if rbs['detect'] == '!':
                for first in firsts:
                    if first['seq_name'] == '|'.join(rbs['seq_name'].split('|')[0:4]):
                        ...
                        if first['acc'] == rbs['name'] and first[type_] > rbs[type_]:
                            first['print'] = True
                            first['e'] = rbs['e']
                ...
                if not same:
                    if len(extras) == 0:
                        ...
                        for extra in extras:
                            if '|'.join(extra['seq_name'].split('|')[0:4]) == '|'.join(rbs['seq_name'].split('|')[0:4]):
                                check = True
        ...
        for line in a_h:
            ...
            if line.startswith('#'):
                ...
                if line.startswith('Query:'):
                    ...
                    if line.startswith('Hit alignments:'):
                        hit = False
                        compare_first_result(ribos, firsts, seq_name, out, extras, cutoff)
