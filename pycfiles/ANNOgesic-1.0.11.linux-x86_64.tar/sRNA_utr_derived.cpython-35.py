...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/sRNA_utr_derived.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 37169 bytes
...
def get_terminal(cdss, inters, seq, type_):
    ...
    if type_ == 'start':
        ...
        for cds in reversed(cdss):
            ...
            if cds.strand == '+' and first_p:
                first_p = False
                inters.append(import_inter(cds.strand, cds.seq_id, cds.end, len(seq[cds.seq_id]), cds.end - cds.start))
            elif cds.strand == '-' and first_m:
                first_m = False
                inters.append(import_inter(cds.strand, cds.seq_id, cds.start, len(seq[cds.seq_id]), cds.end - cds.start))


def check_pos(cover, check_point, checks, cover_pos):
    if cover_pos >= min(check_point['utr_start'], check_point['utr_end']) and cover_pos <= max(check_point['utr_start'], check_point['utr_end']):
        checks['utr'] = True
    if cover_pos >= min(check_point['srna_start'], check_point['srna_end']) and cover_pos <= max(check_point['srna_start'], check_point['srna_end']):
        checks['srna'] = True
...
def set_cover_and_point(cover_results, inter, covers, pos, fuzzy_end):
    ...
    if inter['strand'] == '-':
        c_start, c_end = check_start_and_end(ori_start, pos['ori_end'], covers, '-', fuzzy_end)
...
def detect_cover_utr_srna(cover_results, pos, inter, cond, track, args_srna, lib_type, start, end, strand):
    ...
    for cover in cover_results['covers']:
        ...
        check_pos(cover, cover_results['check_point'], datas['checks'], cover_pos)
        if datas['checks']['utr']:
            datas['cover_tmp']['ori_total'] = datas['cover_tmp']['ori_total'] + cover
        if datas['checks']['srna']:
            datas['cover_tmp']['total'] = datas['cover_tmp']['total'] + cover
            datas['checks']['first'] = coverage_comparison(cover, cover_results['cover_sets'], cover_results['pos'], datas['checks']['first'], inter['strand'], cover_pos)
            if datas['checks']['first'] is not True and cover_results['cover_sets']['high'] > 0 and (cover_results['type'] == '5utr' or cover_results['type'] == '3utr' or cover_results['type'] == 'interCDS' and cover_results['intercds'] == 'TSS') and cover_results['cover_sets']['low'] / cover_results['cover_sets']['high'] < args_srna.decrease_utr and cover_results['cover_sets']['low'] > -1:
                datas['checks']['detect_decrease'] = True
                datas['cover_tmp']['5utr'] = cover
            if datas['checks']['detect_decrease']:
                go_out = get_cover_5utr(datas, cover_results['cover_sets'], cover, inter, args_srna, cover_pos)
                if go_out is True:
                    break
                index_pos += 1

    if datas['checks']['first'] is not True and cover_results['cover_sets']['high'] > 0:
        check_import_srna_covers(datas, cover_results, inter, cond, track, cover, pos, args_srna, lib_type)
        ...
        if wig_strain == inter['strain']:
            for cond, tracks in conds.items():
                ...
                for lib_name, covers in tracks.items():
                    ...
                    start, end = set_cover_and_point(cover_results, inter, covers, pos, args_srna.fuzzy_utr)
                    detect_cover_utr_srna(cover_results, pos, inter, cond, track, args_srna, lib_type, start, end, lib_strand)

    return (
     cover_results['srna_covers'], cover_results['utr_covers'])


def get_cover_5utr(datas, cover_sets, cover, inter, args_srna, cover_pos):
    go_out = False
    if datas['num'] == args_srna.fuzzy_utr or datas['cover_tmp']['5utr'] == 0 or cover > datas['cover_tmp']['5utr'] and cover / datas['cover_tmp']['5utr'] > 1 + args_srna.decrease_utr:
        if inter['strand'] == '+':
        ...
        if cover <= datas['cover_tmp']['5utr']:
            if cover / datas['cover_tmp']['5utr'] >= args_srna.decrease_utr / 2:
                datas['num'] += 1
            else:
                datas['num'] = 0
            datas['cover_tmp']['5utr'] = cover
            cover_sets['low'] = cover
        elif cover > datas['cover_tmp']['5utr'] and cover / datas['cover_tmp']['5utr'] <= 1 + args_srna.decrease_utr:
            datas['num'] += 1
    return go_out


def import_cover(inter, covers, track, cover_sets, avgs, cover, final_poss, pos, lib_type):
    if final_poss['start'] < final_poss['end']:
        if inter['strand'] == '+' and final_poss['end'] > pos['end']:
            final_poss['end'] = pos['end']
    else:
        if inter['strand'] == '-' and final_poss['start'] < pos['start']:
...
def check_import_srna_covers(datas, cover_results, inter, cond, track, cover, pos, args_srna, lib_type):
    avgs = {'avg': datas['cover_tmp']['total'] / float(pos['end'] - pos['start'] + 1), 
     
     'ori_avg': datas['cover_tmp']['ori_total'] / float(pos['ori_end'] - pos['ori_start'] + 1)}
    if cover_results['type'] == '5utr' and (cover_results['intercds'] == 'tsspro' or datas['checks']['detect_decrease']) or cover_results['type'] == '3utr' and cover_results['intercds'] == 'two_pro' or cover_results['type'] == '3utr' and cover_results['intercds'] != 'two_pro' and datas['checks']['detect_decrease'] or cover_results['type'] == '3utr' and cover_results['intercds'] != 'two_pro' and pos['end'] - pos['start'] >= args_srna.min_len and pos['end'] - pos['start'] <= args_srna.max_len or cover_results['type'] == 'interCDS' and cover_results['intercds'] == 'TSS' and datas['checks']['detect_decrease'] or cover_results['type'] == 'interCDS' and (cover_results['intercds'] == 'tss_pro' or cover_results['intercds'] == 'two_pro'):
        import_cover(inter, cover_results['srna_covers'][cond], track, cover_results['cover_sets'], avgs, cover, datas['final_poss'], pos, lib_type)
        import_cover(inter, cover_results['utr_covers'][cond], track, cover_results['cover_sets'], avgs, cover, datas['final_poss'], pos, lib_type)
    else:
        import_cover(inter, cover_results['utr_covers'][cond], track, cover_results['cover_sets'], avgs, cover, datas['final_poss'], pos, lib_type)
...
def detect_3utr_pro(inter, pos, wigs, utr_type, args_srna):
    ...
    for pro in args_srna.pros:
        if pro.seq_id == inter['strain'] and pro.strand == inter['strand']:
            if pro.start >= pos['start'] and pro.start <= pos['end']:
                if pro.strand == '+':
                    if pos['end'] - pro.start >= args_srna.min_len and pos['end'] - pro.start <= args_srna.max_len:
                        n_pos = import_position(pro.start, pos['end'], pos['ori_start'], pos['ori_end'])
                        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'pro', args_srna)
                        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
                        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, '3utr', 'cleavage', 'Cleavage:' + '_'.join([
                         str(pro.start), pro.strand]), srna_covers, 'NA'))
                    elif pos['end'] - pro.start > args_srna.max_len:
                        detect_twopro(inter, pos, wigs, utr_type, '3utr', args_srna)
                else:
                    if pro.start - pos['start'] >= args_srna.min_len and pro.start - pos['start'] <= args_srna.max_len:
                        n_pos = import_position(pos['start'], pro.start, pos['ori_start'], pos['ori_end'])
                        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'pro', args_srna)
                        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
                        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, '3utr', 'cleavage', 'Cleavage:' + '_'.join([
                         str(pro.start), pro.strand]), srna_covers, 'NA'))
                    else:
                        if pro.start - pos['start'] > args_srna.max_len:
                            detect_twopro(inter, pos, wigs, utr_type, '3utr', args_srna)
...
def detect_twopro(inter, pos, wigs, utr_type, import_type, args_srna):
    ...
    pros = []
    for pro in args_srna.pros:
        if pro.seq_id == inter['strain'] and pro.strand == inter['strand'] and pro.start >= pos['start'] and pro.start <= pos['end']:
            pros.append(pro)
    ...
    for pro in pros:
        if first:
            first = False
        elif pro.start - pre_pro.start >= args_srna.min_len and pro.start - pre_pro.start <= args_srna.max_len:
            n_pos = import_position(pre_pro.start, pro.start, pos['ori_start'], pos['ori_end'])
            srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'two_pro', args_srna)
            args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'Cleavage:' + '_'.join([
             str(pro.start), pro.strand])))
            args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, import_type, 'cleavage', 'Cleavage:' + '_'.join([
             str(pre_pro.start), pro.strand]), srna_covers, 'Cleavage:' + '_'.join([
             str(pro.start), pro.strand])))
        pre_pro = pro
            ...
            for key, value in data.items():
                if key == pos:
                    if longer == -1:
                        ...
                        if strand == '+' and value > longer and longer != -1:
                            longer = value
...
def get_decrease(inter, wigs, tss, pos, utr_type, args_srna):
    ...
    if inter['strand'] == '+':
        n_pos = import_position(tss.start, pos['end'], pos['ori_start'], pos['ori_end'])
        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'TSS', args_srna)
        utr_pos = decrease_pos(utr_covers, 'final_end', '+')
        n_pos = import_position(tss.start, utr_pos, pos['ori_start'], pos['ori_end'])
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
        if len(srna_covers) != 0:
            srna_pos = decrease_pos(srna_covers, 'final_end', '+')
            n_pos = import_position(tss.start, srna_pos, pos['ori_start'], pos['ori_end'])
            args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'TSS', 'TSS:' + '_'.join([str(tss.start), tss.strand]), srna_covers, 'NA'))
    else:
        n_pos = import_position(pos['start'], tss.start, pos['ori_start'], pos['ori_end'])
        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'TSS', args_srna)
        utr_pos = decrease_pos(utr_covers, 'final_start', '-')
        n_pos = import_position(utr_pos, tss.start, pos['ori_start'], pos['ori_end'])
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
    if len(srna_covers) != 0:
        srna_pos = decrease_pos(srna_covers, 'final_start', '-')
        n_pos = import_position(srna_pos, tss.start, pos['ori_start'], pos['ori_end'])
        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'TSS', 'TSS:' + '_'.join([str(tss.start), tss.strand]), srna_covers, 'NA'))


def import_append_normal(inter, tss, pro, pos, wigs, utr_type, args_srna):
    if inter['strand'] == '+':
        n_pos = import_position(tss.start, pro.start, pos['ori_start'], pos['ori_end'])
        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'tsspro', args_srna)
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'Cleavage:' + '_'.join([str(pro.start), pro.strand])))
        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'TSS', 'TSS:' + '_'.join([str(tss.start), tss.strand]), srna_covers, 'Cleavage:' + '_'.join([str(pro.start), pro.strand])))
    else:
        n_pos = import_position(pro.start, tss.start, pos['ori_start'], pos['ori_end'])
        srna_covers, utr_covers = get_coverage(wigs, inter, n_pos, utr_type, 'tsspro', args_srna)
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'NA', 'NA', utr_covers, 'Cleavage:' + '_'.join([str(pro.start), pro.strand])))
        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], n_pos, utr_type, 'TSS', 'TSS:' + '_'.join([str(tss.start), tss.strand]), srna_covers, 'Cleavage:' + '_'.join([str(pro.start), pro.strand])))
...
def detect_normal(diff, wigs, inter, pos, utr_type, tss, args_srna):
    ...
    if diff >= args_srna.min_len and diff <= args_srna.max_len:
        srna_covers, utr_covers = get_coverage(wigs, inter, pos, utr_type, 'TSS', args_srna)
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
        args_srna.srnas.append(import_data(inter['strand'], inter['strain'], pos, utr_type, 'TSS', 'TSS:' + '_'.join([str(tss.start), tss.strand]), srna_covers, 'NA'))
    elif diff > args_srna.max_len and len(args_srna.pros) != 0:
        detect = False
        for pro in args_srna.pros:
            if pro.seq_id == inter['strain'] and pro.strand == inter['strand'] and pro.start >= pos['start'] and pro.start <= pos['end'] and pro.start - pos['start'] >= args_srna.min_len and pro.start - pos['start'] <= args_srna.max_len:
                detect = True
                import_append_normal(inter, tss, pro, pos, wigs, utr_type, args_srna)
...
def detect_utr(start, end, inter, utr_type, wigs, args_srna):
    ...
    if 'p_' in args_srna.fuzzy_tsss[utr_type]:
        ...
        args_srna.fuzzy_tsss[utr_type] = float(args_srna.fuzzy_tsss[utr_type].split('_')[(-1)])
    for tss in args_srna.tsss:
        if tss.seq_id == inter['strain'] and tss.strand == inter['strand']:
            if tss.strand == '+':
                ...
                if tss.start >= start_fuzzy and tss.start <= end:
                    ...
                    detect_normal(end - tss.start, wigs, inter, n_pos, utr_type, tss, args_srna)
                elif tss.start > end:
                    break
            else:
                ...
                if tss.start >= start and tss.start <= end_fuzzy:
                    ...
                    detect_normal(tss.start - start, wigs, inter, n_pos, utr_type, tss, args_srna)
...
def run_utr_detection(wigs, inter, start, end, utr_type, args_srna):
    if end - start >= 0:
        ...
        pos = import_position(start, end, start, end)
        srna_covers, utr_covers = get_coverage(wigs, inter, pos, utr_type, 'NA', args_srna)
        args_srna.utrs.append(import_data(inter['strand'], inter['strain'], pos, utr_type, 'NA', 'NA', utr_covers, 'NA'))
...
def class_utr(inter, ta, args_srna, wig_fs, wig_rs):
    ...
    if inter['strand'] == '+':
        if inter['start'] <= ta.end and inter['end'] >= ta.end and ta.start <= inter['start']:
            run_utr_detection(wig_fs, inter, inter['start'] + 1, ta.end, '3utr', args_srna)
        else:
            if inter['start'] <= ta.start and inter['end'] >= ta.start and ta.end >= inter['end']:
                run_utr_detection(wig_fs, inter, ta.start, inter['end'] - 1, '5utr', args_srna)
            elif inter['start'] >= ta.start and inter['end'] <= ta.end:
                run_utr_detection(wig_fs, inter, inter['start'] + 1, inter['end'] - 1, 'interCDS', args_srna)
    else:
        if inter['start'] <= ta.end and inter['end'] >= ta.end and ta.start <= inter['start']:
            run_utr_detection(wig_rs, inter, inter['start'] + 1, ta.end, '5utr', args_srna)
        else:
            if inter['start'] <= ta.start and inter['end'] >= ta.start and ta.end >= inter['end']:
                run_utr_detection(wig_rs, inter, ta.start, inter['end'] - 1, '3utr', args_srna)
            elif inter['start'] >= ta.start and inter['end'] <= ta.end:
                run_utr_detection(wig_rs, inter, inter['start'] + 1, inter['end'] - 1, 'interCDS', args_srna)
...
def print_file(num, srna, start, end, srna_datas, args_srna):
    name = '%0*d' % (5, num)
    args_srna.out_t.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t'.format(srna['strain'], name, start, end, srna['strand'], ';'.join(srna_datas['conds'].keys()), ';'.join(srna_datas['conds'].values()), srna_datas['best'], srna_datas['high'], srna_datas['low']))
    ...
    for data in srna_datas['detail']:
        if first:
            args_srna.out_t.write('{0}(avg={1};high={2};low={3})'.format(data['track'], data['avg'], data['high'], data['low']))
            first = False
        else:
            args_srna.out_t.write(';{0}(avg={1};high={2};low={3})'.format(data['track'], data['avg'], data['high'], data['low']))
    ...
    if len(args_srna.srnas) != 0:
        for srna in args_srna.srnas:
            if srna['strain'] in median.keys():
                srna_datas = replicate_comparison(args_srna, srna['datas'], srna['strand'], 'sRNA_utr_derived', median[srna['strain']][srna['utr']], args_srna.coverages, srna['utr'], None, None, args_srna.texs)
                if srna_datas['best'] != 0:
                    if srna['utr'] == '5utr' or srna['utr'] == 'interCDS':
                        ...
                        if math.fabs(start - end) >= args_srna.min_len and math.fabs(start - end) <= args_srna.max_len:
                            print_file(num, srna, start, end, srna_datas, args_srna)
                            num += 1
...
def read_data(args_srna):
    ...
    for entry in gff_parser.entries(fh):
        if entry.feature == 'CDS' or entry.feature == 'tRNA' or entry.feature == 'rRNA':
...
def get_inter(cdss, inters):
    ...
    for cds1 in cdss:
        for cds2 in cdss:
            if cds1.seq_id == cds2.seq_id and cds1.strand == cds2.strand and cds2.start > cds1.start:
                if cds2.start - cds1.end > 1:
                    if cds1.strand == '+':
                        length = cds1.end - cds1.start
                    else:
                        length = cds2.end - cds2.start
                    inters.append(import_inter(cds1.strand, cds1.seq_id, cds1.end, cds2.start, length))
                break
                ...
                for track, avgs in tracks.items():
                    ...
                    if args_srna.cover_notex is not None:
                        for keys in args_srna.texs.keys():
                            ...
                            if tracks[0] == track:
                                get_utr_cutoff(args_srna.coverages[utr], mediandict, avgs, strain, utr, track)
                                break
                            elif tracks[1] == track:
                                get_utr_cutoff(args_srna.cover_notex[utr], mediandict, avgs, strain, utr, track)
                                break
...
def utr_derived_srna(args_srna, libs, texs, wig_fs, wig_rs):
    ...
    inters = sorted(inters, key=lambda k: (k['strain'], k['start'],
     k['end'], k['strand']))
    args_srna = ArgsContainer().extend_utr_container(args_srna, cdss, tsss, pros, out, out_t, texs)
    for inter in inters:
        for ta in tas:
            if inter['strain'] == ta.seq_id and inter['strand'] == ta.strand:
