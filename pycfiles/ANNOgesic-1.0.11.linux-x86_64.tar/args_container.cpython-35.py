...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/args_container.py
# Compiled at: 2020-03-10 05:55:50
# Size of source mod 2**32: 64086 bytes
...
class ArgsContainer(object):
    ...
    def _check_strain_length(self, strain_lens, flag):
        ...
        for m_l in strain_lens:
            if ':' not in m_l:
                print('Error: The assignment of {0} needs to contain genome names and their length of checked region!'.format(flag))
                sys.exit()
            else:
                if m_l.split(':')[(-1)] == 'all':
    ...
    def _check_track_name(self, lib, tracks, strand):
        if lib.split('/')[(-1)] in tracks['file']:
            print('Error: {0} of the wiggle files is repeated!'.format(lib.split('/')[(-1)]))
            sys.exit()
        else:
            ...
            for line in fh:
                ...
                if line.startswith('track'):
                    ...
                    if track_name in tracks['track'][strand]:
                        print('Error: {0} of the tracks in the wiggle files is repeated!'.format(track_name))
                        sys.exit()
                    else:
        ...
        if libs is not None:
            ...
            for lib in libs:
                if not os.path.exists(lib.split(':')[0]):
                    print('Error: {0} of the wiggle files is not found!'.format(lib.split(':')[0]))
                    sys.exit()
                self._check_track_name(lib.split(':')[0], tracks, lib.split(':')[(-1)])
    ...
    def _check_replicates(self, replicates_tex, replicates_frag, tex_lib, frag_lib):
        ...
        if replicates['tex'] != -1:
            for rep in replicates['tex']:
                if '_' not in rep:
                    print('Error: Please check the input format of replicate_tex! It should also contain condition name.')
                    sys.exit()

        if replicates['frag'] != -1:
            for rep in replicates['frag']:
                if '_' not in rep:
                    print('Error: Please check the input format of replicate_frag! It should also contain condition name.')
                    sys.exit()
    ...
    def _check_assign_info(self, infos, file_type, wig_type):
        ...
        for info, num in sorted(infos.items()):
            if file_type == 'cond' or file_type == 'rep':
                if file_type == 'cond':
                    if info != index:
                        print('Error: The condition number and order of --tex_notex_libs should follow 1, 2, 3.')
                        sys.exit()
                elif file_type == 'rep' and ord(info) != index:
                    print('Error: The replicate index and order of --tex_notex_libs should follow a, b, c.')
                    sys.exit()
                if wig_type == 'tex' and num % 4 != 0:
                    print('Error: The --tex_notex_libs was assinged incorrectly. Please check it again.')
                    sys.exit()
                elif wig_type == 'frag' and num % 2 != 0:
                    print('Error: The --frag_libs was assinged incorrectly. Please check it again.')
                    sys.exit()
                index += 1
            elif file_type == 'strand':
                if wig_type == 'frag' and infos['+'] != infos['-']:
                    print('Error: The --frag_libs was assinged incorrectly. Please check it again.')
                    sys.exit()
                if wig_type == 'tex' and num % 2 != 0 and infos['+'] != infos['-']:
                    print('Error: The --tex_notex_libs was assinged incorrectly.  Please check it again.')
                    sys.exit()
    ...
    def _check_tex_frag(self, libs, wig_type):
        ...
        for lib in libs:
            ...
            if not datas[0].endswith('.wig'):
                ...
                sys.exit()
            if datas[1] != 'notex' and datas[1] != 'tex' and datas[1] != 'frag':
                print('Error: Please assign "tex", "notex" or "frag" to your input libraries.')
                sys.exit()
            try:
    ...
    def _check_condition_num(self, out_prefix, libs):
        ...
        if len(out_prefix) != high:
            print('Error: The number of --condition_names should be the same to the condition of input libraries!')
            sys.exit()
    ...
    def _merge_by_strain(self, wig_path, libs):
        ...
        for strain in strains:
            ...
            for wig in os.listdir(wig_path):
                ...
                if '_STRAIN_' in wig and filename[(-1)].replace('.wig', '') == strain:
                    for lib in libs:
                        if filename[0] in lib and lib[(-1)] == '+':
                            self.helper.merge_file(os.path.join(wig_path, wig), os.path.join(merge_folder, 'tmp_forward.wig'))
                            change_f = True
                        elif filename[0] in lib and lib[(-1)] == '-':
                            self.helper.merge_file(os.path.join(wig_path, wig), os.path.join(merge_folder, 'tmp_reverse.wig'))
                            change_r = True

            if change_f and change_r:
                change_f = False
                change_r = False
                shutil.move(os.path.join(merge_folder, 'tmp_forward.wig'), os.path.join(merge_folder, strain + '_forward.wig'))
                shutil.move(os.path.join(merge_folder, 'tmp_reverse.wig'), os.path.join(merge_folder, strain + '_reverse.wig'))
            else:
                print('Error: comparing input files of {0} failed. Please check the seq IDs of all gff and fasta files, they should be the same.\nPlease also check the wiggle files which should contain forward and reverse files.'.format(strain))
                sys.exit()
    ...
    def _parser_combine_wigs(self, subcommand):
        ...
        if self.tex_wigs is not None:
            ...
            if self.gffs is not None:
                self.multiparser.combine_wig(gff_path, self.tex_path, None, self.libs)
            else:
        ...
        if self.frag_wigs is not None:
            ...
            if self.gffs is not None:
                self.multiparser.combine_wig(gff_path, self.frag_path, None, self.libs)
            else:
                self._merge_by_strain(self.frag_path, self.libs)
            self.merge_wigs = self.frag_wigs
            self.wig_path = self.frag_path
        if self.tex_path is not None and self.frag_path is not None:
            self = self._merge_wig()
        if self.tex_path is None and self.frag_path is None:
    ...
    def _merge_wig(self):
        ...
        if self.tex_wigs is not None and self.frag_wigs is not None:
            ...
            for wig in os.listdir(self.tex_wigs):
                if os.path.isfile(os.path.join(self.tex_wigs, wig)):
                    shutil.copy(os.path.join(self.tex_wigs, wig), self.merge_wigs)

            for wig in os.listdir(self.frag_wigs):
                if os.path.isfile(os.path.join(self.frag_wigs, wig)):
                    shutil.copy(os.path.join(self.frag_wigs, wig), self.merge_wigs)

            for wig in os.listdir(self.tex_path):
                if os.path.isfile(os.path.join(self.tex_path, wig)):
                    shutil.copy(os.path.join(self.tex_path, wig), self.wig_path)

            for wig in os.listdir(self.frag_path):
                if os.path.isfile(os.path.join(self.frag_path, wig)):
                    self.helper.merge_file(os.path.join(self.frag_path, wig), os.path.join(self.wig_path, wig))
    ...
    def _deal_multi_inputs(self, inputs, file_type, num, command):
        ...
        if inputs is not None:
            datas = inputs.split(',')
            if num is not None and len(datas) != num:
    ...
    def _gen_copy_new_folder(self, file_types, out_folder, folder_name, ref_files, flag):
        if ref_files is not None:
            ...
            for files in ref_files:
                ...
                if not detect:
                    print("Error: {0} doesn't exist or the {0} is/are not ended with {1}!".format(files, ' '.join(file_types)))
                    sys.exit()
                shutil.copy(file_, new_ref_folder)
                if file_.endswith('.gff3'):
                    new_name = os.path.basename(file_)[:-1]
                    shutil.move(os.path.join(new_ref_folder, os.path.basename(file_)), os.path.join(new_ref_folder, new_name))

            return new_ref_folder
        else:
            return

    def _check_tss_parameter_setting(self, auto_load, genome_order, height, height_reduction, factor, factor_reduction, base_height, enrichment_factor, processing_factor):
        if auto_load:
            if not os.path.exists(auto_load):
                print('Error: {0} is not found. Please assign proper folder to --auto_load_optimized_parameters!'.format(auto_load))
                sys.exit()
        else:
            para_lists = [
            ...
            if genome_order is not None:
                for para_list, para_name in zip(para_lists, para_names):
                    if len(genome_order) != len(para_list):
                        print('Error: --genome_order and {0} have different number of of genomes!'.format(para_name))
                        sys.exit()

            else:
                for para_list, para_name in zip(para_lists, para_names):
                    if len(para_list) != 1:
                        print('Error: --genome_order is default (using one parameter set to all genomes) but {0} has more than 1 input values!'.format(para_name))
                        sys.exit()

    def container_ratt(self, ratt_path, element, transfer_type, ref_embl, ref_gbk, target_fasta, ref_fasta, ratt_folder, tar_annotation_folder, compare_pair):
        self.ratt_path = ratt_path
        self.element = element
        self.transfer_type = transfer_type
        self.ref_embls = self._gen_copy_new_folder([
         '.embl'], ratt_folder, 'temp_embl', ref_embl, ['--ref_embl_files'])
        self.ref_gbk = self._gen_copy_new_folder([
         '.gbk', '.gbff', '.gb'], ratt_folder, 'temp_gbk', ref_gbk, ['--ref_gbk_files'])
        file_types = ['.fa', '.fna', '.fasta']
        self.tar_fastas = self._gen_copy_new_folder(file_types, ratt_folder, 'temp_tar', target_fasta, [
         '--ref_fasta_files'])
        self.ref_fastas = self._gen_copy_new_folder(file_types, ratt_folder, 'temp_ref', ref_fasta, [
         '--target_fasta_files'])
    ...
    def container_tsspredator(self, TSSpredator_path, compute_program, fasta_files, annotation_files, lib, output_prefix, auto_load, genome_order, height, height_reduction, factor, factor_reduction, base_height, enrichment_factor, processing_factor, replicate_match, out_folder, validate_gene, merge_manual, strain_lengths, compare_transcript_assembly, fuzzy, utr_length, cluster, re_check_orphan, overlap_feature, overlap_gff, remove_low_expression):
        ...
        self._check_tss_parameter_setting(auto_load, genome_order, height, height_reduction, factor, factor_reduction, base_height, enrichment_factor, processing_factor)
        ...
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.libs = self._create_working_wigs(out_folder, lib, self.wig_folder, tracks)
    ...
    def container_optimize(self, TSSpredator_path, fasta_file, annotation_file, manual, out_folder, max_height, max_height_reduction, max_factor, max_factor_reduction, max_base_height, max_enrichment_factor, max_processing_factor, utr_length, lib, output_prefix, cluster, strain_lengths, core, program, replicate_match, steps):
        self.tsspredator_path = TSSpredator_path
        if strain_lengths is not None:
            nt_lengths = self._check_strain_length(strain_lengths, '--genome_lengths')
            self.strain_lengths = nt_lengths
        ...
        if manual is not None:
            self.strain_lengths = {'all': 'all'}
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], out_folder, 'tmp_fasta', fasta_file, ['--fasta_files'])
        self.manuals = self._gen_copy_new_folder([
         '.gff', '.gff3'], out_folder, 'tmp_manual', manual, ['--manual_files'])
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], out_folder, 'tmp_anno', annotation_file, ['--annotation_files'])
        self.wigs = os.path.join(out_folder, 'tmp_wig')
        self.helper.check_make_folder(self.wigs)
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.libs = self._create_working_wigs(out_folder, lib, self.wigs, tracks)
        self.libs = self._check_libs(self.libs, None)
    ...
    def container_terminator(self, TransTermHP_path, expterm_path, RNAfold_path, out_folder, fasta_files, annotation_files, transcript_files, srna, decrease, highest_coverage, fuzzy_detect_coverage, fuzzy_within_transcript, fuzzy_downstream_transcript, fuzzy_within_gene, fuzzy_downstream_gene, transtermhp_folder, tex_notex_libs, frag_libs, tex_notex, replicates_tex, replicates_frag, min_loop_length, max_loop_length, min_stem_length, max_stem_length, min_AT_tail, miss_rate, mut_u, keep_multi, window, shift):
        ...
        self.tex_wigs = self._create_wig_folder(os.path.join(out_folder, 'tmp_wig', 'tex_notex'), tex_notex_libs)
        self.frag_wigs = self._create_wig_folder(os.path.join(out_folder, 'tmp_wig', 'frag'), frag_libs)
        ...
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.tlibs = self._create_working_wigs(out_folder, tex_notex_libs, self.tex_wigs, tracks)
        self.flibs = self._create_working_wigs(out_folder, frag_libs, self.frag_wigs, tracks)
        ...
        self.replicates = self._check_replicates(replicates_tex, replicates_frag, tex_notex_libs, frag_libs)
    ...
    def container_transcript(self, tex_notex, modifys, length, annotation_files, height, width, tolerance, tolerance_coverage, replicates_tex, replicates_frag, out_folder, tss_files, TSS_fuzzy, tex_treated_libs, fragmented_libs, compare_feature_genome, terminator_files, fuzzy_term, max_dist):
        if compare_feature_genome is not None and annotation_files is None:
            print('Error: --annotation_files needs to be assigned if --compare_feature_genome is assigned.')
            sys.exit()
        for modify in modifys:
            if modify != 'merge_overlap' and modify != 'extend_5end' and modify != 'extend_3end' and modify != 'within_extend_ends' and modify != 'none':
                print('Error: --modify_transcript need to be assign as "merge_overlap", "extend_5end", "extend_3end", "within_extend_ends" or "none". The assignment is wrong!')
                sys.exit()

        self.modify = modifys
        self.helper.check_make_folder(os.path.join(out_folder, 'tmp_wig'))
        self.tex_wigs = self._create_wig_folder(os.path.join(out_folder, 'tmp_wig', 'tex_notex'), tex_treated_libs)
        self.frag_wigs = self._create_wig_folder(os.path.join(out_folder, 'tmp_wig', 'frag'), fragmented_libs)
        self.tex = tex_notex
        self.length = length
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], out_folder, 'tmp_anno', annotation_files, [
        ...
        self.replicates_tex = replicates_tex
        self.replicates_frag = replicates_frag
        self.replicates = self._check_replicates(replicates_tex, replicates_frag, tex_treated_libs, fragmented_libs)
        self.out_folder = out_folder
        self.compare_tss = self._gen_copy_new_folder([
         '.gff', '.gff3'], out_folder, 'tmp_tss', tss_files, ['--tss_files'])
        self.fuzzy = TSS_fuzzy
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.tlibs = self._create_working_wigs(out_folder, tex_treated_libs, self.tex_wigs, tracks)
        self.flibs = self._create_working_wigs(out_folder, fragmented_libs, self.frag_wigs, tracks)
        self.libs = self._check_libs(self.tlibs, self.flibs)
    ...
    def container_utr(self, tss_files, annotation_files, transcript_assembly_files, terminator_files, terminator_fuzzy, utr_folder, tss_source, base_5utr, length, base_3utr):
    ...
    def container_srna(self, rnafold, relplot_pl, mountain_pl, blastn, blastx, blastdb, srna_folder, UTR_derived_sRNA, annotation_files, TSS_files, transcript_files, TSS_intergenic_fuzzy, TSS_5UTR_fuzzy, TSS_3UTR_fuzzy, TSS_interCDS_fuzzy, import_info, processing_site_files, fasta_files, mountain_plot, nr_format, srna_format, sRNA_database_path, nr_database_path, cutoff_energy, para_blast, blast_score_s, blast_score_n, run_intergenic_TEX_coverage, run_intergenic_noTEX_coverage, run_intergenic_fragmented_coverage, break_tran, run_antisense_TEX_coverage, run_antisense_noTEX_coverage, run_antisense_fragmented_coverage, run_utr_TEX_coverage, run_utr_noTEX_coverage, run_utr_fragmented_coverage, max_length, min_length, tex_notex_libs, frag_libs, replicates_tex, replicates_frag, tex_notex, blast_e_nr, blast_e_srna, detect_sRNA_in_CDS, decrease_intergenic, decrease_utr, fuzzy_intergenic, fuzzy_utr, cutoff_nr_hit, sORF, overlap_percent_CDS, terminator_files, terminator_fuzzy_in_sRNA, terminator_fuzzy_out_sRNA, ignore_hypothetical_protein, TSS_source, min_utr_coverage, promoter_tables, ranking_promoter, promoter_name, compute_sec_str, len_u, num_u, mut_u, ex_srna):
        ...
        self.tex_wigs = self._create_wig_folder(os.path.join(srna_folder, 'temp_wig', 'tex_notex'), tex_notex_libs)
        self.frag_wigs = self._create_wig_folder(os.path.join(srna_folder, 'temp_wig', 'frag'), frag_libs)
        self.pro_folder = self._gen_copy_new_folder([
         '.gff', '.gff3'], srna_folder, 'temp_pro', processing_site_files, [
         '--processing_site_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], srna_folder, 'temp_fasta', fasta_files, ['--fasta_files'])
        ...
        self.coverage_tex = self._deal_multi_inputs(run_intergenic_TEX_coverage, 'float', 5, '--run_intergenic_TEX_coverage')
        self.coverage_notex = self._deal_multi_inputs(run_intergenic_noTEX_coverage, 'float', 5, '--run_intergenic_noTEX_coverage')
        self.coverage_frag = self._deal_multi_inputs(run_intergenic_fragmented_coverage, 'float', 5, '--run_intergenic_fragmented_coverage')
        self.anti_cover_tex = self._deal_multi_inputs(run_antisense_TEX_coverage, 'float', 5, '--run_antisense_TEX_coverage')
        self.anti_cover_notex = self._deal_multi_inputs(run_antisense_noTEX_coverage, 'float', 5, '--run_antisense_noTEX_coverage')
        self.anti_cover_frag = self._deal_multi_inputs(run_antisense_fragmented_coverage, 'float', 5, '--run_antisense_fragmented_coverage')
        self.break_tran = self._deal_multi_inputs(break_tran, 'float', 3, '--run_break_transcript')
        self.utr_tex_cover = self._deal_multi_inputs(run_utr_TEX_coverage, 'str', 3, '--run_utr_TEX_coverage')
        self.utr_notex_cover = self._deal_multi_inputs(run_utr_noTEX_coverage, 'str', 3, '--run_utr_TEX_coverage')
        self.utr_frag_cover = self._deal_multi_inputs(run_utr_fragmented_coverage, 'str', 3, '--run_utr_fragmented_coverage')
        self.max_len = max_length
        self.min_len = min_length
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.tlibs = self._create_working_wigs(srna_folder, tex_notex_libs, self.tex_wigs, tracks)
        self.flibs = self._create_working_wigs(srna_folder, frag_libs, self.frag_wigs, tracks)
        self.libs = self._check_libs(self.tlibs, self.flibs)
        self.replicates_tex = replicates_tex
        self.replicates_frag = replicates_frag
        self.replicates = self._check_replicates(replicates_tex, replicates_frag, tex_notex_libs, frag_libs)
        ...
        self.promoter_table = self._combine_files(promoter_tables, srna_folder, 'tmp_promoter_table')
    ...
    def container_intersrna(self, file_type, files, args_srna, prefix, gff_file, tran_file, tss_file, pro_file, fuzzy):
        ...
        if file_type == 'frag':
            args_srna.wig_f_file = os.path.join(args_srna.frag_path, '_'.join([prefix, 'forward.wig']))
            args_srna.wig_r_file = os.path.join(args_srna.frag_path, '_'.join([prefix, 'reverse.wig']))
            ...
            args_srna.wig_f_file = os.path.join(args_srna.tex_path, '_'.join([prefix, 'forward.wig']))
            args_srna.wig_r_file = os.path.join(args_srna.tex_path, '_'.join([prefix, 'reverse.wig']))
    ...
    def container_utrsrna(self, gff, tran, tss, files, pro, fasta, file_type, prefix, args_srna):
    ...
    def extend_inter_container(self, args_srna, tsss, pros, nums, output, out_table, texs, detects, cutoff_coverage, notex):
        """Especially for intergenic and antisense sRNA"""
        args_srna.tsss = tsss
        args_srna.pros = pros
        args_srna.nums = nums
    ...
    def extend_utr_container(self, args_srna, cdss, tsss, pros, out, out_t, texs):
        """Especially for UTR-derived sRNA"""
        args_srna.cdss = cdss
        args_srna.tsss = tsss
        args_srna.pros = pros
    ...
    def container_sorf(self, sorf_folder, UTR_derived_sORF, transcript_files, annotation_files, TSS_files, utr_length, min_length, max_length, cutoff_intergenic_coverage, cutoff_antisense_coverage, cutoff_5utr_coverage, cutoff_3utr_coverage, cutoff_interCDS_coverage, fasta_files, tex_notex_libs, frag_libs, tex_notex, replicates_tex, replicates_frag, sRNA_files, start_codon, stop_codon, cutoff_background, rbs_seq, fuzzy_rbs, rbs_not_after_TSS, print_all_combination, best_no_sRNA, best_no_TSS, ignore_hypothetical_protein, min_rbs_distance, max_rbs_distance, extend_3, extend_5, multi_stop):
        ...
        self.tex_wigs = self._create_wig_folder(os.path.join(sorf_folder, 'temp_wig', 'tex_notex'), tex_notex_libs)
        self.frag_wigs = self._create_wig_folder(os.path.join(sorf_folder, 'temp_wig', 'frag'), frag_libs)
        ...
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], sorf_folder, 'temp_fasta', fasta_files, ['--fasta_files'])
        tracks = {'file': [], 'track': {'+': [], '-': []}}
        self.tlibs = self._create_working_wigs(sorf_folder, tex_notex_libs, self.tex_wigs, tracks)
        self.flibs = self._create_working_wigs(sorf_folder, frag_libs, self.frag_wigs, tracks)
        self.libs = self._check_libs(self.tlibs, self.flibs)
        self.tex_notex = tex_notex
        self.replicates_tex = replicates_tex
        self.replicates_frag = replicates_frag
    ...
    def container_srna_target(self, rnaplfold_path, rnaplex_path, rnaup_path, intarna_path, annotation_files, fasta_files, sRNA_files, query_sRNA, program, interaction_length, window_size_target, span_target, window_size_srna, span_srna, unstructured_region_RNAplex_target, unstructured_region_RNAplex_srna, unstructured_region_RNAup, energy_threshold, duplex_distance, top, starget_output_folder, process_rnaplex, process_rnaup, process_intarna, continue_rnaup, slide_win_srna_intarna, max_loop_srna, slide_win_target_intarna, max_loop_target, mode_intarna, potential_target_start, potential_target_end, target_feature):
        self.rnaplfold_path = rnaplfold_path
        self.rnaplex_path = rnaplex_path
        self.rnaup_path = rnaup_path
        self.intarna_path = intarna_path
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], starget_output_folder, 'tmp_anno', annotation_files, ['--annotation_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], starget_output_folder, 'tmp_fasta', fasta_files, ['--fasta_files'])
        self.srnas = self._gen_copy_new_folder([
    ...
    def container_goterm(self, annotation_files, goterm_output_folder, UniProt_id, go_obo, goslim_obo, transcript_files):
        self.gffs = self._gen_copy_new_folder([
    ...
    def container_sublocal(self, Psortb_path, annotation_files, fasta_files, bacteria_type, difference_multi, sublocal_output_folder, transcript_files):
        self.psortb_path = Psortb_path
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], sublocal_output_folder, 'tmp_anno', annotation_files, ['--annotation_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], sublocal_output_folder, 'tmp_fa', fasta_files, ['--fasta_files'])
        self.gram = bacteria_type
        self.fuzzy = difference_multi
        self.out_folder = sublocal_output_folder
        self.trans = self._gen_copy_new_folder([
         '.gff', '.gff3'], sublocal_output_folder, 'tmp_ta', transcript_files, ['--transcript_files'])
        return self

    def container_ppi(self, annotation_files, proteinID_strains, without_strain_pubmed, species_STRING, score, ppi_output_folder, node_size, query):
        self.ptts = self._gen_copy_new_folder([
         '.gff', '.gff3'], ppi_output_folder, 'temp_anno', annotation_files, ['--annotation_files'])
    ...
    def container_promoter(self, MEME_path, GLAM2_path, out_folder, tex_libs, TSS_files, fasta_files, num_motif, nt_before_TSS, motif_width, TSS_source, annotation_files, end_run, combine_all, e_value, para, program, use_tss_type):
        self.meme_path = MEME_path
        self.glam2_path = GLAM2_path
        self.program = program
        self.end_run = end_run
        if program.lower() != 'both' and program.lower() != 'meme' and program.lower() != 'glam2':
        ...
        self.tsss = self._gen_copy_new_folder([
         '.gff', '.gff3'], out_folder, 'tmp_tss', TSS_files, ['--tss_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], out_folder, 'tmp_fasta', fasta_files, ['--fasta_files'])
        ...
        if not TSS_source:
            if annotation_files is None:
                print('Error: if --tss_source is False, please assign --annotation_files as well!')
                sys.exit()
            if tex_libs is not None:
                self.helper.check_make_folder(os.path.join(out_folder, 'tmp_wig'))
                self.tex_wigs = self._create_wig_folder(os.path.join(out_folder, 'tmp_wig', 'tex_notex'), tex_libs)
                tracks = {'file': [], 'track': {'+': [], '-': []}}
                self.input_libs = self._create_working_wigs(out_folder, tex_libs, self.tex_wigs, tracks)
                self.libs = self.input_libs
                self = self._parser_combine_wigs('promoter')
        return self

    def container_operon(self, TSS_files, annotation_files, transcript_files, term_files, TSS_fuzzy, term_fuzzy, min_length, operon_output_folder, operon_statistics_folder):
        self.tsss = self._gen_copy_new_folder([
         '.gff', '.gff3'], operon_output_folder, 'tmp_tss', TSS_files, ['--tss_files'])
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], operon_output_folder, 'tmp_anno', annotation_files, ['--annotation_files'])
        self.trans = self._gen_copy_new_folder([
         '.gff', '.gff3'], operon_output_folder, 'tmp_ta', transcript_files, ['--transcript_files'])
        self.terms = self._gen_copy_new_folder([
         '.gff', '.gff3'], operon_output_folder, 'tmp_term', term_files, ['--term_files'])
    ...
    def container_snp(self, samtools_path, bcftools_path, bam_type, program, fasta_files, bam_files, quality, read_depth_range, snp_output_folder, indel_fraction, chrom, rg, caller, filters, DP4_cutoff):
        self.samtools_path = samtools_path
        self.bcftools_path = bcftools_path
        self.types = bam_type
        self.program = program
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], snp_output_folder, 'tmp_fa', fasta_files, ['--fasta_files'])
        self.bams = bam_files
    ...
    def container_circrna(self, process, fasta_files, annotation_files, bam_files, read_files, circrna_stat_folder, support_reads, segemehl_path, testrealign, samtools_path, start_ratio, end_ratio, ignore_hypothetical_protein, out_folder):
        self.cores = process
        self.fastas = self._gen_copy_new_folder([
    ...
    def container_ribos(self, program, thermo_ID, cmscan_path, cmpress_path, riboswitch_ID, annotation_files, fasta_files, tss_files, transcript_files, Rfam, ribos_output_folder, thermo_output_folder, cutoff, output_all, database_folder, fuzzy, without_rbs, rbs_seq, fuzzy_rbs, UTR_length):
        self.program = program
        self.without_rbs = without_rbs
        self.rbs_seq = rbs_seq
        if program.lower() == 'riboswitch' or program.lower() == 'both':
        ...
        self.gffs = self._gen_copy_new_folder([
         ...
         '--annotation_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], output, 'temp_fa', fasta_files, ['--fasta_files'])
        self.tsss = self._gen_copy_new_folder([
    ...
    def container_cris(self, fasta_files, annotation_files, CRT_path, window_size, min_number_repeat, min_length_repeat, Max_length_repeat, min_length_spacer, Max_length_spacer, cris_out_folder, ignore_hypo):
        self.gffs = self._gen_copy_new_folder([
         '.gff', '.gff3'], cris_out_folder, 'tmp_anno', annotation_files, [
         '--annotation_files'])
        self.fastas = self._gen_copy_new_folder([
         '.fa', '.fna', '.fasta'], cris_out_folder, 'tmp_fa', fasta_files, ['--fasta_files'])
    ...
    def container_screen(self, main_gff, side_gffs, fasta, height, tex_libs, frag_libs, present, output_folder):
