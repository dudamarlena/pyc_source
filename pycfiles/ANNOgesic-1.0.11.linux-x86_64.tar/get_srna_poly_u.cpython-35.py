...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/get_srna_poly_u.py
...
def get_table_entry(tabs, srna):
    for tab in tabs:
        if srna.seq_id == tab['seq_id'] and srna.start == tab['start'] and srna.end == tab['end'] and srna.strand == tab['strand']:
            return tab
...
def backward_t(seq, start, end, strand, mut_u):
    ...
    while 1:
        if strand == '+':
            ...
            nt = Helper().extract_gene(seq, start + ext, start + ext, strand)
        if nt == 'U' or nt == 'T':
            pass
        else:
            no_ut += 1
...
def search_t(seq, start, end, strand, ext_b, ext_f, args_srna):
    if strand == '+':
        ...
        if seq_end > len(seq):
            seq_end = len(seq)
        seq_u = Helper().extract_gene(seq, end - ext_b - 1, seq_end, strand)
    else:
        seq_start = start - args_srna.len_u - ext_f - 1
        if seq_start < 1:
            seq_start = 1
        seq_u = Helper().extract_gene(seq, seq_start, start + ext_b + 1, strand)
    ...
    if strand == '+':
        ...
        if final_end - start >= args_srna.max_len:
...
def get_srna_poly_u(srna_file, seq_file, srna_table, args_srna):
    ...
    for entry in Gff3Parser().entries(gff_f):
        ...
        ext_b = backward_t(seq, entry.start, entry.end, entry.strand, args_srna.mut_u)
        ext_f = forward_t(seq, entry.start - args_srna.len_u, entry.end + args_srna.len_u, entry.strand, args_srna.mut_u)
        final_start, final_end = search_t(seq, entry.start, entry.end, entry.strand, ext_b, ext_f, args_srna)
