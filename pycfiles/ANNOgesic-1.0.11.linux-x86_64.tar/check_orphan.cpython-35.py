...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/check_orphan.py
# Compiled at: 2019-01-22 09:59:27
# Size of source mod 2**32: 14229 bytes
import os
...
def get_attributes(tss, cds):
    if tss.attributes['associated_gene'] == 'orphan':
        if 'locus_tag' in cds.attributes.keys():
            ...
            tss.attributes['associated_gene'] = cds.feature + ':' + str(cds.start) + '-' + str(cds.end) + '_' + strand
    else:
        if 'locus_tag' in cds.attributes.keys():
            ...
            tss.attributes['associated_gene'] = '&'.join([
             tss.attributes['associated_gene'],
             cds.feature + ':' + str(cds.start) + '-' + str(cds.end) + '_' + strand])
    ...
    for tss in tsss:
        ...
        for type_ in types:
            if type_ == 'Primary' and not detect_pri:
                ...
                if type_ == 'Primary' and detect_pri:
                    compare_utr = int(utrs[index].split('_')[1])
                    if compare_utr < pri_utr:
                        pri_utr = compare_utr
                        real_index = index
                else:
                    if type_ == 'Secondary' and not detect_sec:
                        detect_sec = True
                        sec_utr = int(utrs[index].split('_')[1])
                        real_index2 = index
                    else:
                        if type_ == 'Secondary' and detect_sec:
                            compare_utr = int(utrs[index].split('_')[1])
...
def compare_cds_check_orphan(tsss, cdss):
    ...
    for tss in tsss:
        if tss.attributes['type'] == 'Orphan':
            for cds in cdss:
                if tss.seq_id == cds.seq_id and tss.strand == cds.strand:
                    ...
                    tss.attributes['type'] = 'Primary'
                    if tss.strand == '+':
                        tss.attributes['UTR_length'] = 'Primary_' + str(cds.start - tss.start)
                    else:
                        tss.attributes['UTR_length'] = 'Primary_' + str(tss.start - cds.end)
                    get_attributes(tss, cds)
                if is_internal(cds.start, cds.end, tss.start, tss.strand):
                    if 'locus_tag' in cds.attributes.keys() and cds.attributes['locus_tag'] not in tss.attributes['associated_gene']:
