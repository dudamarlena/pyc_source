...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/merge_rnaplex_rnaup.py
# Compiled at: 2019-07-09 11:17:54
# Size of source mod 2**32: 32087 bytes
...
def check_parent_gene(cds, genes):
    ...
    if target_gene is None:
        for gene in genes:
            if gene.seq_id == cds.seq_id and gene.strand == cds.strand:
                if cds.start <= gene.start and cds.end >= gene.end or cds.start >= gene.start and cds.end <= gene.end or cds.start <= gene.start and cds.end <= gene.end and cds.end >= gene.start or cds.start >= gene.start and cds.start <= gene.end and cds.end >= gene.end:
                    target_gene = gene
    ...
    for method, srna_datas in srnas.items():
        for srna_id, targets in srna_datas.items():
            ...
            for target in sort_targets:
                if target['tar_pos'] != 'NA' and target['energy'] < 0:
                    rank += 1
                    target['rank'] = rank
                    if rank <= args_tar.top and method == feature:
                        ...
                        s_start, s_end = mod_srna_tar_pos(srna_info, target['srna_pos'], 'srna', args_tar.tar_start, args_tar.tar_end, length)
        ...
        for line in p_h:
            ...
            if line.startswith('>'):
                ...
                if count_seq == 1:
                    ...
                    gene_id, tar, target_id, target_locus = get_gene_id(detail, genes, genomes, features)
                elif count_seq == 2:
                    srna = line[1:]
                    if srna not in srnas['RNAplex'].keys():
                        srnas['RNAplex'][srna] = []
                    srnas['RNAplex'][srna].append({'target_id': target_id, 'gene_id': gene_id, 
            ...
            if line.startswith('>'):
                if line[1:] in srna_names:
                    ...
                    if srna in srnas['RNAup'].keys():
                        srnas['RNAup'][srna].append({'target_id': target_id, 'target_locus': target_locus, 
                         'detail': detail, 'energy': 0, 
                         'gene_id': gene_id})
                    else:
                        srnas['RNAup'][srna] = []
...
def read_intarna(intarna, srnas, genes, genomes, features):
    with open(intarna, 'r') as (i_h):
        for line in i_h:
            ...
            if inter[0] != 'id1' and len(inter) == 9:
                srna = inter[3]
                    ...
                    if len(tags[0]) != 0:
                        ...
                        if tar is not None:
                            if srna not in srnas['IntaRNA'].keys():
                                srnas['IntaRNA'][srna] = []
                            srnas['IntaRNA'][srna].append({'target_id': target_id, 'target_locus': target_locus, 
...
def get_gene_id(detail, genes, gffs, features):
    tar = None
    for gff in gffs:
        if gff.feature in features:
            ...
            strand = detail.split('_')[(-1)]
            if gff.start == start and gff.end == end and gff.strand == strand:
                tar = gff
                break
    ...
    target_locus = 'NA'
    if tar is not None:
        if 'ID' in tar.attributes.keys():
            target_id = tar.attributes['ID']
        if 'locus_tag' in tar.attributes.keys():
            target_locus = tar.attributes['locus_tag']
        for gene in genes:
            if 'Parent' in tar.attributes.keys() and 'ID' in gene.attributes.keys() and gene.attributes['ID'] in tar.attributes['Parent'].split(','):
                gene_id = gene.attributes['ID']
                return (
                 gene_id, tar, target_id, target_locus)
            if gene_id == 'NA' and gene.seq_id == tar.seq_id and gene.strand == tar.strand and tar.start == gene.start and tar.end == gene.end and 'ID' in gene.attributes.keys():
                gene_id = gene.attributes['ID']
                return (gene_id, tar, target_id, target_locus)

        if gene_id == 'NA':
            for gene in genes:
                if gene.seq_id == tar.seq_id and gene.strand == tar.strand and (tar.start <= gene.start and tar.end >= gene.end or tar.start >= gene.start and tar.end <= gene.end or tar.start <= gene.start and tar.end <= gene.end and tar.end >= gene.start or tar.start >= gene.start and tar.start <= gene.end and tar.end >= gene.end) and 'ID' in gene.attributes.keys():
                    gene_id = gene.attributes['ID']
                    return (gene_id, tar, target_id, target_locus)
...
def append_merge_three_methods(name, srna_info, ps_pos, pt_pos, u_data, i_data, srna_m1, target_info, merges):
...
def import_merge(merges, name, srna_info, srna_m1, srna_m2, srna_m3, target_info, pre_target, suf_target, length, num_method):
    ps_start, ps_end = mod_srna_tar_pos(srna_info, srna_m1['srna_pos'], 'srna', pre_target, suf_target, length)
    pt_start, pt_end = mod_srna_tar_pos(target_info, srna_m1['tar_pos'], 'tar', pre_target, suf_target, length)
    ...
    if srna_m1['detail'] == srna_m2['detail']:
        us_start, us_end = mod_srna_tar_pos(srna_info, srna_m2['srna_pos'], 'srna', pre_target, suf_target, length)
        ut_start, ut_end = mod_srna_tar_pos(target_info, srna_m2['tar_pos'], 'tar', pre_target, suf_target, length)
    ...
    if srna_m3 is not None:
        if srna_m1['detail'] == srna_m3['detail']:
            is_start, is_end = mod_srna_tar_pos(srna_info, srna_m3['srna_pos'], 'srna', pre_target, suf_target, length)
            it_start, it_end = mod_srna_tar_pos(target_info, srna_m3['tar_pos'], 'tar', pre_target, suf_target, length)
        ...
        append_merge_three_methods(name, srna_info, ps_pos, pt_pos, u_data, i_data, srna_m1, target_info, merges)
    else:
        if num_method == 2:
            merges.append([name, srna_info.seq_id,
             '-'.join([str(srna_info.start), str(srna_info.end)]),
...
def get_srna_name(gffs, srna):
    ...
    srna_info = None
    for gff in gffs:
        if gff.attributes['ID'] == srna and 'Name' in gff.attributes.keys():
...
def get_target_info(gffs, target):
    tmp_gff = None
    for gff in gffs:
        if str(gff.start) + '-' + str(gff.end) + '_' + gff.strand == target['detail']:
            if gff.feature == 'gene':
                target_info = gff
                if 'locus_tag' in gff.attributes.keys() and 'gene' in gff.attributes.keys():
                    if gff.attributes['gene'] not in target['target_locus']:
                        target['target_locus'] = '|'.join([
                         target['target_locus'], gff.attributes['gene']])
                else:
                    if 'locus_tag' in gff.attributes.keys() and 'Name' in gff.attributes.keys() and gff.attributes['Name'] not in target['target_locus']:
                        target['target_locus'] = '|'.join([
...
def merge_three(srnas, method3, srna, detect, srna_m1, top):
    ...
    if method3 is not None:
        for srna_m3 in srnas[method3][srna]:
            if srna_m1['detail'] == srna_m3['detail'] and 'rank' in srna_m3.keys():
                if srna_m3['rank'] != 'NA' and srna_m3['rank'] <= top and 'print' not in srna_m3.keys() and srna_m3['tar_pos'] != 'NA':
                    srna_m3['print'] = True
                    detect['3'] = True
                return srna_m3
...
def check_non_overlap(detect, methods, srna_m1, srna_m2, srna_m3, gffs, merges, name, srna_info, args_tar, length, method3, srna, srnas):
    if not detect['2'] and len(methods) == 2 or (not detect['2'] or not detect['3']) and len(methods) == 3:
        if 'rank' not in srna_m2.keys():
            srna_m2['rank'] = 'NA'
            srna_m2['energy'] = 1000
        srna_m3 = check_method3(srna_m3, srna_m1, method3, srna, srnas)
        target_info = get_target_info(gffs, srna_m1)
        import_merge(merges, name, srna_info, srna_m1, srna_m2, srna_m3, target_info, args_tar.tar_start, args_tar.tar_end, length, len(methods))
        srna_m1['print'] = True
...
def merge_result(srnas, srna_gffs, args_tar, gffs, merges, length, methods):
    ...
    for srna, srna_m1s in srnas[method1].items():
        ...
        for srna_m1 in srna_m1s:
            if 'rank' in srna_m1.keys() and srna_m1['rank'] != 'NA' and 'print' not in srna_m1.keys() and srna_m1['rank'] <= args_tar.top and srna_m1['tar_pos'] != 'NA':
                detect = {'2': False, '3': False}
                srna_m3 = None
                for srna_m2 in srnas[method2][srna]:
                    if srna_m1['detail'] == srna_m2['detail']:
                        if 'rank' in srna_m2.keys() and srna_m2['rank'] != 'NA' and srna_m2['rank'] <= args_tar.top and 'print' not in srna_m2.keys() and srna_m2['tar_pos'] != 'NA':
                            detect['2'] = True
                            srna_m3 = merge_three(srnas, method3, srna, detect, srna_m1, args_tar.top)
                            if len(methods) == 2 or len(methods) == 3 and detect['3']:
                                target_info = get_target_info(gffs, srna_m1)
                                import_merge(overlaps, name, srna_info, srna_m1, srna_m2, srna_m3, target_info, args_tar.tar_start, args_tar.tar_end, length, len(methods))
                                srna_m1['print'] = True
                                srna_m2['print'] = True
                                import_merge(merges, name, srna_info, srna_m1, srna_m2, srna_m3, target_info, args_tar.tar_start, args_tar.tar_end, length, len(methods))
                            break

                check_non_overlap(detect, methods, srna_m1, srna_m2, srna_m3, gffs, merges, name, srna_info, args_tar, length, method3, srna, srnas)
...
def compare_rest(srnas, rest, srna_last, srna):
    ...
    for srna_m3 in srnas[rest][srna]:
        if srna_last['detail'] == srna_m3['detail'] and 'print' not in srna_m3.keys():
            if 'rank' not in srna_m3.keys():
                srna_m3['rank'] = 'NA'
                srna_m3['energy'] = 1000
            return srna_m3
...
def merge_last(srnas, srna_gffs, args_tar, gffs, merges, length, method, ref, num_method, rest, switch):
    ...
    for srna, srnas_last in srnas[method].items():
        ...
        for srna_last in srnas_last:
            if 'rank' in srna_last.keys() and srna_last['rank'] != 'NA' and 'print' not in srna_last.keys() and srna_last['rank'] <= args_tar.top and srna in srnas[ref].keys() and srna_last['tar_pos'] != 'NA':
                for srna_ref in srnas[ref][srna]:
                    if srna_ref['detail'] == srna_last['detail'] and 'print' not in srna_ref.keys():
                        if 'rank' not in srna_ref.keys():
                            srna_ref['rank'] = 'NA'
                            srna_ref['energy'] = 1000
                        else:
                            target_info = get_target_info(gffs, srna_last)
                            if num_method == 2:
                                import_merge(merges, name, srna_info, srna_ref, srna_last, None, target_info, args_tar.tar_start, args_tar.tar_end, length, num_method)
                            elif num_method == 3:
                                srna_m3 = compare_rest(srnas, rest, srna_last, srna)
                                if switch:
                                    import_merge(merges, name, srna_info, srna_ref, srna_m3, srna_last, target_info, args_tar.tar_start, args_tar.tar_end, length, num_method)
                                else:
                                    import_merge(merges, name, srna_info, srna_ref, srna_last, srna_m3, target_info, args_tar.tar_start, args_tar.tar_end, length, num_method)
                                if srna_m3 is not None:
                                    srna_m3['print'] = True
...
def merge_srna_target(rnaplex, rnaup, intarna, args_tar, out_rnaplex, out_rnaup, out_intarna, seq_file, output, out_overlap, srna_gff_file, annotation_gff):
    ...
    if len(args_tar.program) >= 2:
        ...
        print('Merging now...')
        overlaps = merge_result(srnas, srna_gffs, args_tar, gffs, merges, length, methods)
        if len(methods) == 2:
            merge_last(srnas, srna_gffs, args_tar, gffs, merges, length, methods[1], methods[0], 2, None, False)
    else:
        if len(methods) == 3:
            merge_last(srnas, srna_gffs, args_tar, gffs, merges, length, methods[1], methods[0], 3, methods[2], False)
            merge_last(srnas, srna_gffs, args_tar, gffs, merges, length, methods[2], methods[0], 3, methods[1], True)
