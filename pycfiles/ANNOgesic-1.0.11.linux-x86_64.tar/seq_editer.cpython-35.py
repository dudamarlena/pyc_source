...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/seq_editer.py
...
class SeqEditer(object):
    __doc__ = 'Edit the sequence if it is needed'
    ...
    def _import_data(self, mod_table_file, out_name):
        ...
        for row in csv.reader(fh, delimiter='\t'):
            if row[0].startswith('#'):
                ...
                first = False
            elif row[0] == pre_ref_id:
                datas[num_index]['datas'].append({'ref_nt': row[3].strip(), 
                 'tar_nt': row[4].strip(), 
            ...
            if data['ref_id'] + '.fa' in os.listdir(fasta_folder):
                ...
                for change in data['datas']:
                    if change['ref_nt'] == '-':
                        ...
                        if change['tar_nt'] == '-':
                            seq_modifier.remove(int(change['position']), len(change['ref_nt']))
                        else:
