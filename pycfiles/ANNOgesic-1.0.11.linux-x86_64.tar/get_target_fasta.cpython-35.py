...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/annogesiclib/get_target_fasta.py
...
class TargetFasta(object):
    ...
    def get_target_fasta(self, mut_table, tar_folder, ref_files, out_name, out_folder, log):
        new_ref_folder = self.gen_folder(out_folder, ref_files)
        log.write('Running seq_editor.py for updating sequence.\n')
        self.seq_editer.modify_seq(self.folders['tmp_ref'], mut_table, self.folders['tmp_tar'], out_name)
        ...
        for row in csv.reader(mh, delimiter='\t'):
            if not row[0].startswith('#'):
                if pre_strain != row[0]:
                    ...
                    fasta = os.path.join(out_folder, 'fasta_files', tmp_tar_name)
                    if out is not None:
                        out.close()
                    out = open(fasta, 'w')
                    if tmp_tar_name in os.listdir(self.folders['tmp_tar']):
                        with open(os.path.join(self.folders['tmp_tar'], tmp_tar_name)) as (f_h):
                            for line in f_h:
                                out.write(line)

                    else:
                        print('Error: No updated information of {0}.fa'.format(row[0]))
                    pre_strain = row[0]
        ...
        if strain_num == 1:
            ...
            for seq in os.listdir(os.path.join(out_folder, 'fasta_files')):
                if seq.endswith('.fa'):
                    with open(os.path.join(out_folder, 'fasta_files', seq)) as (t_h):
                        for line in t_h:
                            if len(line) != 0:
                                if line.startswith('>'):
                                    o_s.write('>' + out_name + '\n')
