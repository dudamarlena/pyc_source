...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/show/models.py
# Compiled at: 2013-10-10 08:56:07
...
class Credit(models.Model):
    contributor = models.ForeignKey('show.Contributor', related_name='credits')
...
class Show(ModelBase):
    ...
    contributors = models.ManyToManyField('show.Contributor', through='show.Credit')
    start = models.DateTimeField(db_index=True, help_text='If only the time of day is applicable then set the day to today. It will be ignored.')
    end = models.DateTimeField(db_index=True, help_text='If only the time of day is applicable then set the day to today. It will be ignored.')
    repeat = models.CharField(max_length=64, choices=(
     ...
     ('sundays', 'Sundays'),
     ('weekly', 'Weekly'),
     ('monthly_by_day_of_month', 'Monthly By Day Of Month')), default='does_not_repeat', db_index=True)
    repeat_until = models.DateField(blank=True, null=True)
    start_time = models.TimeField(db_index=True, editable=False, auto_now_add=True)
    ...
    def next(self):
        ...
        if now < self.end:
            ...
            if self.repeat != 'does_not_repeat' and (self.repeat_until is None or now.date() <= self.repeat_until):
    ...
    def _next_repeat(self, date):
        if self.repeat == 'daily':
            return date
        if self.repeat == 'monthly_by_day_of_month':
            if date.day > self.start.day:
                date = date.replace(day=1, month=(date.month + 1) % 12, year=date.year + int(math.floor((date.month + 1) / 12)))
            ...
            if self.repeat == 'weekdays':
                ...
                date = date + timedelta(days=self.start.weekday() - weekday) if self.start.weekday() >= weekday else date + timedelta(days=7 - weekday + self.start.weekday())
        return date
...
class CreditOption(models.Model):
    ...
    role_priority = models.IntegerField(blank=True, null=True, help_text='The priority assigned to this role, with lower values being more important.')
...
class Appearance(models.Model):
    ...
    show_contributor = models.ForeignKey(Contributor, related_name='appearances')