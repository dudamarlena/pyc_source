...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/diwnotifier/diwnotifier.py
# Compiled at: 2014-02-05 13:48:16
import pynotify, sys, os, copy
from scraping import Session, WebParser
from settings import Settings
Settings.collector.save()
from utils import MyConfigParser, download_avatar, ShortLinksCollector, bool_print, ThreadsCollector, Thread, which, check_notification_daemon
...
class DIWNotifier(wx.App):
    ...
    def OnInit(self):
        try:
            ...
            if bool_print('Login to ' + Settings.LOGIN_URL, self.session.login, Settings.LOGIN_URL, user, pw):
                ...
                error_msg = 'Error message: ' + self.session.login_status + '.\n' + "Watch <a href='file://" + log_path + "'>" + log_path + '</a> for more info'
                self.notify(Settings.HOME_URL + ' - Login failed ', error_msg, Settings.app_home + 'data/images/dramma.png', Settings.T1)
                return False
    ...
    def counts_refresh(self):
        ...
        self.count['friends'][1] = self.parser.get_notification_count('title="Friends Online"><span class=\'ipsHasNotifications\'>')
        self.count['msgs'][0] = self.tbi.msgc
        self.count['msgs'][1] = self.parser.get_unread_msgs_count()
        self.count['notify'][0] = self.tbi.notifyc
        self.count['notify'][1] = self.parser.get_notification_count('title="Notifiche"><img src="http://clandiw.it/public/style_images/brave/clear.gif" alt="" /><span class=\'ipsHasNotifications\'>')
        self.count['shoutbox'][1] = self.parser.get_notification_count('id="shoutbox-tab-count" class="ipsHasNotifications" style="display: none;">')

    def tbi_refresh(self):
        if self.first:
            self.tbi.profile_link = self.parser.get_link(Settings.MSG_URL, 'Tuo Profilo')
            self.tbi.pca_link = self.parser.get_link(Settings.MSG_URL, 'Pannello di Controllo degli Amministratori (PCA)')
            self.tbi.pcm_link = self.parser.get_link(Settings.MSG_URL, 'Pannello di Controllo dei Moderatori (PCM)')
        self.tbi.set_counters(self.count['friends'][1], self.count['msgs'][1], self.count['notify'][1], self.count['shoutbox'][1])
    ...
    def load_and_notify(self):
        ...
        threads = ThreadsCollector(self.notify_from_msg_page, self.notify_from_friends_page, self.notify_from_notifications_page, self.shoutbox_notify, self.update_new_content())
        threads.make()
        threads.start_all()

    def update_new_content(self):
        bool_print('LOAD ' + self.collector.mirror(Settings.NEW_CONTENT), self.__load_page(Settings.NEW_CONTENT))
    ...
    def notify_from_msg_page(self):
        if self.first:
            self.notify(Settings.HOME_URL, "<a href='" + Settings.FRIENDS_URL + "'>Friends</a>:" + self.count['friends'][1] + "\n<a href='" + Settings.MSG_URL + "'>Unread messages</a>:" + self.count['msgs'][1] + "\n<a href='" + Settings.NOTIFICATIONS_URL + "'>Notifications:</a>" + self.count['notify'][1] + "\n<a href='" + Settings.SHOUTBOX_URL + "'>Shoutbox</a>:" + self.count['shoutbox'][1] + '', Settings.image_uri, Settings.T1)
        ...
        if self.count['msgs'][0] != self.count['msgs'][1] and self.count['msgs'][1] != '0' and msg == 'True':
            ...
            self.notify(Settings.HOME_URL + ' - ' + self.count['msgs'][1] + ' unread messages ', "Last from <a href='" + msg_data['who_link'] + "'>" + msg_data['who'] + '</a>\n' + msg_data['when'] + ": <a href='" + msg_data['what_link'] + "'>" + msg_data['what'] + '</a>', msg_data['avatar'], Settings.T2)

    def notify_from_friends_page(self):
        if self.config.read_field('notifications', 'friends') == 'True':
            bool_print('LOAD ' + self.collector.mirror(Settings.FRIENDS_URL), self.__load_page(Settings.FRIENDS_URL))
            if self.count['friends'][0] != self.count['friends'][1] and self.count['friends'][1] != '0':
                ...
                self.notify(Settings.HOME_URL + ' - ' + self.count['friends'][1] + ' Friends online', string[:-2], Settings.image_uri, Settings.T2)

    def notify_from_notifications_page(self):
        if self.config.read_field('notifications', 'notifications') == 'True':
            bool_print('LOAD ' + self.collector.mirror(Settings.NOTIFICATIONS_URL), self.__load_page(Settings.NOTIFICATIONS_URL))
            if self.count['notify'][0] != self.count['notify'][1] and self.count['notify'][1] != '0':
                self.ln = self.parser.get_last_notification()
                self.notify(Settings.HOME_URL + ' - ' + self.tbi.notifyc + ' new notifications', self.ln['when'] + ':' + self.ln['what'], self.ln['avatar'], Settings.T2)
    ...
    def shoutbox_notify(self):
        if self.config.read_field('notifications', 'shoutbox') == 'True':
            ...
            if flag and self.parser.last_shout['what'] != '':
                self.notify(Settings.HOME_URL + ' - Last Shout', '@' + self.parser.last_shout['who'] + '\n' + "<a href='" + Settings.SHOUTBOX_URL + "'>" + self.parser.last_shout['what'] + '</a>\n' + self.parser.last_shout['when'], self.parser.last_shout['avatar'], Settings.T2)
