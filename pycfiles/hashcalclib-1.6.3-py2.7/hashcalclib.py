...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hashcalclib/hashcalclib.py
# Compiled at: 2015-10-02 05:56:40
...
Note:
    1.If you want to add all files under a directory recursively,
      ...
      `addSrcDirs()`.
"""
import codecs, fnmatch, hashlib, inspect, os, re, stat, sys, time, zlib
...
from commonutil import joinExceptionArgs, getExceptionMsg, str_, unicode_, getAbsPath, Enum, UsageHandler, ProgressBar
__all__ = [
 'BaseHashlib',
 'CRC32',
 'Adler32',
...
class BaseHashlib(object):
    ...
    def update(self, data):
        self._hash = self._hashFunc(data, self._hash) if self._hash is not None else self._hashFunc(data)
        return

    def digest(self):
        hexStr = self.hexdigest()
        pack = __import__('struct').pack
        return ('').join(pack('B', int(hexStr[idx:idx + 2], 16)) for idx in range(0, len(hexStr), 2))
    ...
    def concatenate(self, initialHash, hashSegments):
        ...
        def checkArg(arg, isHash=True):
            if isinstance(arg, (int, long)):
                return arg
            if isHash and isinstance(arg, basestring):
                return int(arg, 16)
            raise TypeError(arg)

        getLowWord = lambda dword: c_uint(dword - 1).value & 65535
        ...
        for nextHash, nextDataSize in hashSegments:
            ...
            lowWord = (1 + getLowWord(currentHash) + getLowWord(nextHash)) % self._PRIME_NUM
            highWord = ((currentHash >> 16) + (nextHash >> 16) + getLowWord(currentHash) * nextDataSize) % self._PRIME_NUM
    ...
    def update(self, data):
        ...
        while dataIdx != dataSize:
            if self._chunkIdx + dataSize - dataIdx > self._CHUNK_SIZE:
                ...
                self._hashObj.update(data[dataIdx:dataIdx + remainingChunkSize])
...
class HashStockUsage(UsageHandler):
    ...
    def __init__(self):
        ...
        messages = {401: 'There are no groups. New a group first.', 
...
')
    ...
    _stdout = sys.stdout
    _newline = '\n'
    _hashRowFormatMap = {'str': ('').join(['*"%(item)s"\t%(hash)s', _newline]), 
    ...
    def _getHashRowText(self, item, hash_, algorithm, type_=_GROUPS_TYPE.STR):
        ...
        return self._hashRowFormatMap[formatType] % {'item': item, 'hash': hash_}

    def newGroup(self, dirHeader='', type_=1):
        """New and return a new group."""
        affirm(isinstance(dirHeader, basestring), self._usage(11, arg='dirHeader', type_='basestring'))
        affirm(isinstance(type_, int), self._usage(11, arg='type_', type_='int'))
        self._stock.append(dict(type_=type_, dirHeader=dirHeader, subGroups=list()))
    ...
    def newSubGroup(self, newOutputHeader=''):
        ...
        affirm(isinstance(newOutputHeader, basestring), self._usage(11, arg='newOutputHeader', type_='basestring'))
        subGroups = self.getCurrentGroup()['subGroups']
        subGroups.append(dict(newOutputHeader=newOutputHeader, items=list()))
    ...
    def addItem(self, item, hashes):
        ...
        affirm(isinstance(item, basestring), self._usage(11, arg='item', type_='basestring'))
        affirm(isinstance(hashes, list) and hashes, self._usage(11, arg='hashes', type_='list'))
    ...
    def print_(self, file_=None):
        ...
        if file_ is not None:
            affirm(isinstance(file_, (file, SpooledTmpFile)), self._usage(11, arg='file_', type_='file or TemporaryFile'))
            output = file_
        else:
            output = self._stdout
                ...
                for item in subGroup['items']:
                    for algorithm, hash_ in item['hashes']:
                        ...
                        if newOutputHeader:
                            ...
                            newOutputHeader = ''
                        text = self._getHashRowText(item['item'], hash_, algorithm, type_)
                        output.write(text)
            ...
            for subGroup in group['subGroups']:
                ...
                for item in subGroup['items']:
                    for algorithm, hash_ in item['hashes']:
                        ...
                        tmpFile.write(self._getHashRowText(item['item'], hash_, algorithm, type_))
...
class HashCalculatorUsage(UsageHandler):
    ...
    def __init__(self):
        ...
        messages = {201: 'Invalid extension name of hash file("%s") and `algorithms` is not given.', 
           ...
           206: 'The algorithm "%(algorithm)s" requires the module "%(module)s" to be installed.', 
           ...
           241: 'OK\t: %s', 
           242: 'FAIL\t: %s', 
           251: '*WARNING* "%s" already exists. Append, Overwrite or Quit(a/o/Q)? ', 
           299: '%s\n%s\n%s' % (DEBUG, '%s', DEBUG)}
        self.MESSAGES.update(messages)
...
class HashCalculator(object):
    ...
    _validAlgorithms = ('crc32', 'md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512',
                        'md4', 'ed2k', 'blake2b', 'blake2s', 'sha3_224', 'sha3_256',
                        'sha3_384', 'sha3_512', 'adler32')
    _validExtnames = ('sfv', 'md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512',
                      'md4', 'ed2k', 'blake2b', 'blake2s', 'sha3224', 'sha3256',
                      'sha3384', 'sha3512', 'sfva')
    _extnameMap = dict(zip(_validExtnames, _validAlgorithms))
    _extnameRegex = re.compile('^(?P<main>.+)(?P<sep>\\.)(?P<ext>[^\\n\\.]+)$', re.U)
    _alnumPattern = '[0-9A-Za-z]'
    _pathPattern = '(?P<path>\\x20*(?P<trimmedPath>[^\\s]+(?:[^\\t\\v\\r\\n\\f]+[^\\s])?)\\x20*)'
    _separatorPattern = '\\t+|\\x20+'
    _CRC32RowPattern = '^%s(?:%s)\\*?(?P<hash>%s{8})(?:\\s*)$' % (
     _pathPattern, _separatorPattern, _alnumPattern)
    _hashRowPattern = '^(?P<hash>%s{%s})(?:%s)\\*?%s(?:\\s*)$' % (
    ...
    _hashRowRegexMap = dict(zip(_validAlgorithms, (
     ...
     _hash8xRowRegex)))
    _pathRowRegex = re.compile('^%s$' % _pathPattern, re.U)
    _strRowRegex = re.compile('^STRING=(?P<string>[^\\r\\n]+)(?:[\\r\\n]*)$', re.U)
    _emptyRowRegex = re.compile('^[\\r\\n]*$', re.U)
    _errorRowRegex = re.compile('^[^\\r\\n]*', re.U)
    _extractionPatternRegex = re.compile('\\(\\?P<hash>.*?\\)')
    _extractionRegex = None
    _newOutputRowRegex = re.compile('\\*\\*\\*\\x20(?P<dir>[^\\t\\v\\r\\n\\f]+)\\x20\\*\\*\\*', re.U)
    _fileFilterRegex = None
    _dirHeaderFormat = '*%(nl)s* %(dir)s%(nl)s*%(nl)s' % {'nl': _newline, 'dir': '%s'}
    ...
    _algorithms = []
    _isUppercase = False
    _extractionPattern = '^.*(?:(?P<hlbk>\\[)|(?P<hlpt>\\())?(?:crc32[\\x20_\\-])?(?P<hash>%(alnum)s{8})(?(hlbk)\\])(?(hlpt)\\))(?:\\[(?:[\\w]{1,5})\\])?\\.%(alnum)s{2,4}' % {'alnum': _alnumPattern}
    ...
    def _getExtname(self, filename, default='', separator='.'):
        ...
        """
        filename = os.path.basename(filename) if isinstance(filename, basestring) else ''
        extname = default
        if separator == '.':
            extnameRegex = self._extnameRegex
        else:
            EscapedSeparator = '\\' + separator
            extnameRegex = re.compile('^(?P<main>.+)(?P<sep>%(sep)s)(?P<ext>[^\\n%(sep)s]+)$' % {'sep': EscapedSeparator})
    ...
    def _computeItemsCount(self):
        ...
        if self._action == 'c' or self._action == 'e':
            self._totalItems += sum([ len(ss[1]) for ss in self._srcStrings ]) + sum([ len(sf[1]) for sf in self._srcFiles ]) + sum([ len(sd[1]) for sd in self._srcDirs ])
        else:
            self._totalItems += sum([ len(vf[(-1)]) for vf in self._filesToBeVerified
                                    ])

    def setSilent(self, isSilent):
        """Set `_isSilent` flag"""
        affirm(isinstance(isSilent, bool), self._usage(11, arg='isSilent', type_='bool'))
    ...
    def setRecursive(self, isRecursive):
        ...
        affirm(isinstance(isRecursive, bool), self._usage(11, arg='isRecursive', type_='bool'))
        self._isRecursive = isRecursive

    def setUppercase(self, isUppercase):
        """Set `_isUppercase` flag"""
        affirm(isinstance(isUppercase, bool), self._usage(11, arg='isUppercase', type_='bool'))
        self._isUppercase = isUppercase

    def setNewOutputMode(self, isNewOutputMode):
        """Set `_isNewOutputMode` flag"""
        affirm(isinstance(isNewOutputMode, bool), self._usage(11, arg='isNewOutputMode', type_='bool'))
        self._isNewOutputMode = isNewOutputMode

    def setDirHeader(self, hasDirHeader):
        """Set `_hasDirHeader` flag"""
        affirm(isinstance(hasDirHeader, bool), self._usage(11, arg='hasDirHeader', type_='bool'))
        self._hasDirHeader = hasDirHeader

    def setExistenceOnly(self, isExistenceOnly):
        """Set `_isExistenceOnly` flag"""
        affirm(isinstance(isExistenceOnly, bool), self._usage(11, arg='isExistenceOnly', type_='bool'))
        self._isExistenceOnly = isExistenceOnly

    def setVerbose(self, isVerbose):
        """Set `_isVerbose` flag"""
        affirm(isinstance(isVerbose, bool), self._usage(11, var='isVerbose', type_='bool'))
        self._isVerbose = isVerbose

    def setTee(self, isTee):
        """Set `_isTee` flag"""
        affirm(isinstance(isTee, bool), self._usage(11, arg='isTee', type_='bool'))
        self._isTee = isTee

    def setEncoding(self, encoding):
        """Set the `_encoding`"""
        affirm(isinstance(encoding, basestring), self._usage(11, arg='encoding', type_='basestring'))
        try:
    ...
    def _selectFileMode(self, filePath):
        affirm(os.access(os.path.dirname(filePath), os.W_OK) and not os.path.isdir(filePath), self._usage(203, self._str(filePath)))
        mode = 'a'
        if os.access(filePath, os.F_OK):
            affirm(os.access(filePath, os.W_OK), self._usage(204, self._str(filePath)))
            input_ = ''
    ...
    def setLogFile(self, logFile):
        ...
        """
        affirm(isinstance(logFile, basestring), self._usage(11, arg='logFile', type_='basestring'))
    ...
    def setFileFilterPattern(self, unixFileFilterPattern=None, regexFileFilterPattern=None):
        """Call this function without args to disable the filter."""
        if regexFileFilterPattern is not None:
            affirm(isinstance(regexFileFilterPattern, basestring), self._usage(11, arg='regexFileFilterPattern', type_='basestring'))
            try:
                self._fileFilterRegex = re.compile(self._unicode(regexFileFilterPattern), re.U)
            except re.error as ree:
                raise Error(self._usage(16, var='regexFileFilterPattern', msg=joinExceptionArgs(ree), value=self._str(regexFileFilterPattern)))

            self._fileFilterPattern = self._fileFilterRegex.pattern
        elif unixFileFilterPattern is not None:
            affirm(isinstance(unixFileFilterPattern, basestring), self._usage(11, arg='unixFileFilterPattern', type_='basestring'))
    ...
    def setExclusiveDirs(self, exclusiveDirs):
        ...
        affirm(isinstance(exclusiveDirs, list), self._usage(11, arg='exclusiveDirs', type_='list'))
        self._exclusiveDirs[:] = []
        for dir_ in exclusiveDirs:
            affirm(isinstance(dir_, basestring), self._usage(12, arg='exclusiveDirs', value=self._str(exclusiveDirs)))
            self._exclusiveDirs.append(self._unicode(dir_))

    def setAction(self, action):
        """Set the `_action`"""
        affirm(action in ('c', 'v', 'e'), self._usage(12, arg='action', value=self._str(action)))
        self._action = action
    ...
    def _checkAlgorithmModule(self, algorithm):
        ...
        if algorithm.startswith('blake'):
            affirm(_isPyblake2Imported, self._usage(206, algorithm=algorithm, module='pyblake2'))
        elif algorithm.startswith('sha3_'):
            affirm(_isSha3Imported, self._usage(206, algorithm=algorithm, module='sha3'))
    ...
    def addAlgorithm(self, algorithm):
        ...
        affirm(algorithm in self._validAlgorithms, self._usage(12, arg='algorithm', value=self._str(algorithm)))
    ...
    def setAlgorithms(self, algorithms=None):
        ...
        if isinstance(algorithms, list) and algorithms:
            ...
            if self._action == 'e' and algorithms is None:
                algorithms = [
                 self._validAlgorithms[0]]
            else:
                raise Error(self._usage(12, arg='algorithms', value=self._str(algorithms)))
    ...
    def setHashFile(self, hashFile):
        ...
        affirm(isinstance(hashFile, basestring), self._usage(11, arg='hashFile', type_='basestring'))
    ...
    def parseHashFile(self, hashFile):
        ...
        affirm(os.path.isfile(filePath) and os.access(filePath, os.R_OK), self._usage(210, self._str(filePath)))
        ...
        if self._algorithms:
            ...
            raise Error(self._usage(201, self._str(os.path.basename(filePath))))
    ...
    def parseHashPathsFile(self, hashPathsFile):
        ...
        affirm(os.path.isfile(hashPathsFile) and os.access(hashPathsFile, os.R_OK), self._usage(210, self._str(hashPathsFile)))
        hashPathsFileDir = os.path.dirname(hashPathsFile)
        hashPaths = []
        with open(hashPathsFile, 'r') as (file_):
            for line in file_:
                line = self._unicode(line)
                if not line.startswith(self._commentChar) and self._emptyRowRegex.search(line) is None:
    ...
    def _getFileList(self, dir_, root=None):
        ...
        affirm(os.path.isdir(dir_) and os.access(dir_, os.R_OK), self._usage(205, self._str(dir_)))
        isRoot = False
        if root is None:
            isRoot = True
            root = dir_
        ...
        for item in items:
            ...
            if os.path.isdir(itemAbsPath):
                if self._isRecursive and not os.path.islink(itemAbsPath) and item not in self._exclusiveDirs:
                ...
                if isRoot:
                    ...
                    files.append(os.path.join(os.path.relpath(dir_, root), item))
    ...
    def addSrcStrings(self, srcStrings):
        ...
        affirm(isinstance(srcStrings, list), self._usage(11, arg='srcStrings', type_='list'))
        srcStrsTmp = []
        for string in srcStrings:
            affirm(isinstance(string, basestring), self._usage(12, arg='srcStrings', value=self._str(srcStrings)))
    ...
    def addSrcFiles(self, srcFiles):
        ...
        affirm(isinstance(srcFiles, list), self._usage(11, arg='srcFiles', type_='list'))
        srcFilesTmp = []
        for file_ in srcFiles:
            affirm(isinstance(file_, basestring), self._usage(12, arg='srcFiles', value=self._str(srcFiles)))
    ...
    def addSrcDirs(self, srcDirs):
        ...
        affirm(isinstance(srcDirs, list), self._usage(11, arg='srcDirs', type_='list'))
        for dir_ in srcDirs:
            affirm(isinstance(dir_, basestring), self._usage(12, arg='srcDirs', value=self._str(srcDirs)))
    ...
    def parseSrcItemsFile(self, srcItemsFile):
        ...
        affirm(isinstance(srcItemsFile, basestring), self._usage(11, arg='srcItemsFile', type_='basestring'))
        ...
        with open(filePath, 'r') as (file_):
            for line in file_:
                line = self._unicode(line)
                if self._emptyRowRegex.search(line) is None and not line.startswith(self._commentChar):
                    strRowMatch = self._strRowRegex.search(line)
                    pathRowMatch = self._pathRowRegex.search(line)
                    affirm(strRowMatch or pathRowMatch, self._usage(233, self._str(line)))
                    if strRowMatch:
                    ...
                    if pathRowMatch:
                        ...
                        if os.path.isfile(itemAbsPath):
                            if self._isFileMatched(os.path.basename(itemAbsPath)):
                                srcFiles.append(itemAbsPath)
                        elif os.path.isdir(itemAbsPath):
                            srcDirs.append(itemAbsPath)
                        elif strRowMatch is None:
                            self._print(self._usage(208, itemAbsPath), self._logTmpFile)
    ...
    def setExtractionPattern(self, extractionPattern=None):
        if extractionPattern is not None:
            affirm(self._extractionPatternRegex.search(extractionPattern), self._usage(12, arg='extractionPattern', value=self._str(extractionPattern)))
            try:
                self._extractionRegex = re.compile(self._unicode(extractionPattern), re.U)
            except re.error as ree:
                raise Error(self._usage(16, var='extractionPattern', msg=joinExceptionArgs(ree), value=self._str(extractionPattern)))

            self._extractionPattern = extractionPattern
        else:
            self._extractionRegex = re.compile(self._extractionPattern, re.U)
        return

    def setFileBufSize(self, fileBufSize):
        affirm(isinstance(fileBufSize, int), self._usage(11, arg='fileBufSize', type_='int'))
    ...
    def __init__(self, action='c', srcStrings=None, srcFiles=None, srcDirs=None, srcItemsFile=None, isRecursive=None, unixFileFilterPattern=None, regexFileFilterPattern=None, exclusiveDirs=None, algorithms=None, isUppercase=None, extractionPattern=None, isNewOutputMode=None, hasDirHeader=None, hashFile=None, hashPathsFile=None, isExistenceOnly=None, encoding=None, logFile=None, isVerbose=None, isTee=None, isSilent=None, fileBufSize=None):
        ...
        self.setFileFilterPattern(unixFileFilterPattern, regexFileFilterPattern)
    ...
    def _calculateHash(self, file_, size=0):
        ...
        fileBufSize = self._fileBufSize if size >> 30 == 0 else self._fileBufSize << 1
        filePosition = 0
        hashObjs = []
        hashes = []
        progress = 0.0
    ...
    def _isFileAccessible(self, srcItem, srcItemText, isReadable=True):
        if os.path.isfile(srcItem) and not (isReadable and not os.access(srcItem, os.R_OK)):
            return True
        else:
    ...
    def _processSrcItem(self, srcItem, srcItemText, groupsType=_GROUPS_TYPE.FILE):
        size = 0
        text = ''
        hashes = {}
        logFile = self._logTmpFile
        ...
        try:
            ...
            self._print(self._usage(242, (', ').join([srcItemText,
    ...
    def _calculate(self, processFunc):
        ...
        srcCollection = (
         self._srcStrings, self._srcFiles, self._srcDirs)
            ...
            for srcGroup in srcCollection[groupsType]:
                ...
                for srcItem in srcGroup[1]:
                    if self._isNewOutputMode:
                        srcItemText = os.path.basename(srcItem) if groupsType != self._GROUPS_TYPE.STR else srcItem
                        newOutputHeader = ''
                        if groupsType == self._GROUPS_TYPE.DIR:
                            relpath = os.path.dirname(srcItem) or None
                            if relpath:
                                newOutputHeader = self._newOutputHeaderFormat % relpath
                    else:
                        srcItemText = os.path.basename(srcItem) if groupsType == self._GROUPS_TYPE.FILE else srcItem
                    ...
                    if relpath != lastRelpath:
                        lastRelpath = relpath
                        hashStock.newSubGroup(self._str(newOutputHeader))
                    hashStock.addItem(self._str(srcItemText), zip(self._algorithms, [ hashes[a] for a in self._algorithms ]))
                    self._itemOK += 1
    ...
    def _verify(self):
        ...
        for hashFileDir, newOutputDir, algorithm, items in self._filesToBeVerified:
            for item in items:
                hashRowMatch = self._hashRowRegexMap[algorithm].search(item)
                if hashRowMatch:
                    hashRowMatch = hashRowMatch.groupdict()
                    fileText = hashRowMatch['path'] if newOutputDir is None else os.path.join(newOutputDir, hashRowMatch['path'])
                    hash_ = hashRowMatch['hash'].upper() if self._isUppercase else hashRowMatch['hash'].lower()
                    ...
                    errorRowMatch = errorRowMatch.group(0) if errorRowMatch else item
    ...
    def act(self):
        ...
        if self._action == 'c':
            ...
            self._algorithms[:] = self._algorithms[:1] if self._algorithms else self._validAlgorithms[:1]
    ...
    def getResult(self):
        ...
        summary = dict(ok=self._itemOK, fail=self._itemFail, found=self._itemFound, notfound=self._itemNotFound, total=self._totalItems)
    ...
    def actAuto(self):
        ...
        try:
            ...
            self._print(self._usage(231, ok=self._itemOK, fail=self._itemFail, found=self._itemFound, notfound=self._itemNotFound, total=self._totalItems, time=elapsedTime), logTmpFile)
        finally:
            self.saveResult()
            self.saveLog()
...
class FileInfoUsage(UsageHandler):
    ...
    def __init__(self):
        ...
        HR = self.HR * 46
        messages = {501: '"%s" is not found, not a directory or no read privilege.', 
           ...
           505: 'Invalid file format: the first line must be the absolute path of the directory that contains all files to be checked.', 
           506: '"%s" is not found, not a file or no read privilege.', 
           507: 'The module "%s" is required. Please make sure that it is installed properly.', 
           530: '*Name\tAttributes\tSize\tFileCount', 
           531: '%(path)s\t%(attributes)s\t%(size)s\t%(count)s', 
           532: '*-------', 
           533: '*Total %(dirs)s Folders, %(files)s Files, and %(bytes_)s B (%(unitBytes)s).', 
           534: '*Mountpoint %(path)s: %(used)s used, %(available)s available.', 
           535: '*Drive %(drive)s (%(label)s): %(used)s used, %(available)s available.', 
           536: 'OK\t: %s', 
           ...
           551: '*WARNING* "%s" already exists. Append, Overwrite or Quit(a/o/Q)? '}
        self.MESSAGES.update(messages)
...
class FileInfo(object):
    ...
    `encoding`: Try to use the specified encoding to decode first.
    """
    _TOTAL = Enum(('DIRS', 'FILES', 'SIZES', 'OK', 'NOTSIZE', 'FOUND', 'NOTFOUND',
                   'COUNT'))
    ...
    def __init__(self, isRecursive=None, isSilent=None, exclusiveDirs=None, isVerbose=None, encoding=None):
        self.setSilent(isSilent or False)
        self.setEncoding(encoding or self._defaultEncoding)
        self.setRecursive(isRecursive or False)
        self.setVerbose(isVerbose or False)
        ...
        if sys.platform.startswith('linux'):
            self._typeMap = (('S_ISDIR', 'd'), ('S_ISCHR', 'c'), ('S_ISBLK', 'b'),
    ...
    def _getAttributesOnWin(self, filePath):
        ...
        for constant, attribute in self._AttributesMap:
            hasAttribute = True if attributesNum & constant == constant else False
            if hasAttribute:
    ...
    def _getDiskUsageOnUnix(self, filePath):
        ...
        resultUsage = self._usage(533, dirs=summary[TOTAL.DIRS], files=summary[TOTAL.FILES], bytes_=summary[TOTAL.SIZES], unitBytes=self.computeSizeUnit(summary[TOTAL.SIZES]))
        diskUsage = self._usage(534, path=mountPath, used=self.computeSizeUnit(usedBytes), available=self.computeSizeUnit(availableBytes))
    ...
    def _getDiskUsageOnWin(self, filePath):
        ...
        summary = self._summary
        resultUsage = self._usage(533, dirs=summary[TOTAL.DIRS], files=summary[TOTAL.FILES], bytes_=summary[TOTAL.SIZES], unitBytes=self.computeSizeUnit(summary[TOTAL.SIZES]))
        diskUsage = self._usage(535, drive=drive, label=label, used=self.computeSizeUnit(usedBytes), available=self.computeSizeUnit(availableBytes))
        return ('\n').join([resultUsage, diskUsage])

    def getFileInfo(self, dir_):
        """Get the file info of all files under the given directory."""
        dir_ = os.path.abspath(self._unicode(dir_))
        affirm(os.path.isdir(dir_) and os.access(dir_, os.R_OK), self._usage(501, self._str(dir_)))
        self.clearResult()
        ...
        while dirs:
            ...
            items.sort()
            fileInfos.append(dict(path=os.path.relpath(currentDir, dir_), size=0, count=0, attributes=self._getAttributes(currentDir)))
            for item in items:
                itemAbsPath = getAbsPath(currentDir, item)
                if os.path.isdir(itemAbsPath):
                    if self._isRecursive and item not in self._exclusiveDirs and not os.path.islink(itemAbsPath):
                    ...
                    fileInfos.append(dict(path=item, attributes=attributes, size=size, count=1))
            ...
            if listDirError:
                self._print(self._usage(541, comment=self._unicode(listDirError)), tmpFile)
                listDirError = None
    ...
    def checkFileInfo(self, filePath):
        ...
        affirm(os.path.isfile(filePath) and os.access(filePath, os.R_OK), self._usage(506, self._str(filePath)))
        self.clearResult()
        tmpFile = self._tmpFile
        COLUMN = self._COLUMN
        TOTAL = self._TOTAL
        ...
        with open(filePath, 'rb') as (file_):
            root = self._unicode(file_.readline()).rstrip(os.linesep).strip(' \t')
            ...
            for line in file_:
                ...
                if os.path.isfile(pathRow):
                    ...
                    if size == int(items[COLUMN.SIZE]):
                        ...
                        self._print(self._usage(537, path=pathRow, filesize=size, infosize=items[COLUMN.SIZE]), tmpFile)
                else:
        ...
        self._print(self._usage(540, **dict(zip(('ok', 'notsize', 'found', 'notfound',
                                                 'total'), summary[TOTAL.OK:]))), tmpFile)

    def _selectFileMode(self, filePath):
        affirm(os.access(os.path.dirname(filePath), os.W_OK) and not os.path.isdir(filePath), self._usage(502, self._str(filePath)))
        mode = 'a'
        if os.access(filePath, os.F_OK):
            affirm(os.access(filePath, os.W_OK), self._usage(503, self._str(filePath)))
            input_ = ''
            while input_.lower() not in ('a', 'o'):
    ...
    def save(self, filePath):
        """Save the hash result to the specified hash file."""
        affirm(isinstance(filePath, basestring), self._usage(11, arg='filePath', type_='basestring'))
