...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/SWHT/ecef.py
...
def geodetic2ecef(lat, lon, alt, degrees=True):
    ...
    if degrees:
        lat = np.deg2rad(lat)
        lon = np.deg2rad(lon)
    xi = np.sqrt(1.0 - esq * np.sin(lat) ** 2.0)
...
def ecef2geodetic(x, y, z, degrees=True):
    ...
    Esq = a * a - b * b
    F = 54.0 * b * b * z * z
    G = r * r + (1.0 - esq) * z * z - esq * Esq
    C = esq * esq * F * r * r / pow(G, 3.0)
    S = cbrt(1.0 + C + np.sqrt(C * C + 2.0 * C))
    P = F / (3.0 * pow(S + 1.0 / S + 1.0, 2.0) * G * G)
    Q = np.sqrt(1.0 + 2.0 * esq * esq * P)
    r_0 = -(P * esq * r) / (1.0 + Q) + np.sqrt(0.5 * a * a * (1.0 + 1.0 / Q) - P * (1.0 - esq) * z * z / (Q * (1.0 + Q)) - 0.5 * P * r * r)
    U = np.sqrt(pow(r - esq * r_0, 2.0) + z * z)
    V = np.sqrt(pow(r - esq * r_0, 2.0) + (1.0 - esq) * z * z)
    Z_0 = b * b * z / (a * V)
    h = U * (1.0 - b * b / (a * V))
...
if __name__ == '__main__':
    ...
    chilbolton = {'lat': 51.143833512, 'lon': -1.433500703, 
       'h': 176.028, 
       'x': 4008438.4572, 
       'y': -100309.724927, 
       'z': 4943735.828}
