...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/SWHT/util.py
...
def sph2cart(theta, phi, r=None):
    ...
    X = np.cos(np.pi / 2.0 - theta) * np.cos(phi - np.pi)
    Y = np.cos(np.pi / 2.0 - theta) * np.sin(phi - np.pi)
    Z = np.sin(np.pi / 2.0 - theta)
    return (
     X, Y, Z)
...
def cart2sph(X, Y, Z):
    ...
    theta = np.pi / 2.0 - np.arctan2(Z, np.sqrt(X ** 2.0 + Y ** 2.0))
    return (
     r, phi, theta)
...
def almVec2array(vec, lmax):
    ...
    for l in np.arange(lmaxp1):
        for m in np.arange(l + 1):
            coeffs[(l, l - m)] = (-1.0) ** m * np.conj(vec[Alm.getidx(lmax, l, m)])
...
if __name__ == '__main__':
    ...
    theta, phi = np.meshgrid(np.linspace(0, np.pi, num=128, endpoint=False), np.linspace(0, 2.0 * np.pi, num=128, endpoint=False))
