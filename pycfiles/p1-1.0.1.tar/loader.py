...
# [GCC 8.4.0]
# Embedded file name: p1/loader.py
# Compiled at: 2015-02-09 06:08:52
...
"""
import optparse, os, time, serial, glob, sys
...
class Loader(object):
    ...
    def _lfsr(self, seed):
        ...
        while True:
            yield seed & 1
            seed = seed << 1 & 254 | (seed >> 7 ^ seed >> 5 ^ seed >> 4 ^ seed >> 1) & 1
    ...
    def _connect(self):
        ...
        self.serial.write(('').join(chr(each | 254) for each in seq[0:LFSR_REQUEST_LEN]))
        self.serial.write(chr(249) * (LFSR_REPLY_LEN + 8))
        for i in range(LFSR_REQUEST_LEN, LFSR_REQUEST_LEN + LFSR_REPLY_LEN):
            if self._read_bit(False, 0.1) != seq[i]:
                raise LoaderError('No hardware found')

        version = 0
        for i in range(8):
            version = version >> 1 & 127 | self._read_bit(False, 0.05) << 7
    ...
    def _bin_to_eeprom(self, code):
        ...
        dbase = ord(code[10]) + (ord(code[11]) << 8)
        if dbase > EEPROM_SIZE:
            raise LoaderError('Invalid binary format')
        code += ('').join(chr(0) * (dbase - 8 - len(code)))
        code += ('').join(chr(each) for each in [255, 255, 249, 255, 255, 255, 249, 255])
        code += ('').join(chr(0) * (EEPROM_SIZE - len(code)))
        return code
    ...
    def _encode_long(self, value):
        ...
        result = []
        for i in range(10):
            result.append(chr(146 | value & 1 | (value & 2) << 2 | (value & 4) << 4))
            value >>= 3

        result.append(chr(242 | value & 1 | (value & 2) << 2))
        return ('').join(result)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--version', action='version', version='%(prog)s 0.1', help='Show the program version and exit.')
    p_comms = argparse.ArgumentParser(add_help=False)
    p_comms.add_argument('-s', '--serial', dest='serial', type=str, metavar='DEVICE', default=None, help='Select the serial port device. The default is %(default)s.')
    p_comms.add_argument('-g', '--gpio_pin', type=int, metavar='GPIO_PIN', default=-1, help='Specify the GPIO pin for GPIO reset.')
    p_upload = argparse.ArgumentParser(add_help=False)
    p_upload.add_argument('filename', type=str, help='Binary file to be uploaded.')
    p_upload.add_argument('-d', '--destination', type=str, default='RAM', choices=[
     'RAM', 'EEPROM'], help='Upload to RAM or to EEPROM.  The default is %(default)s.')
    p_upload.add_argument('-n', '--no-run', action='store_false', dest='run', default=True, help="Don't run the code after upload.")
    ...
    parser_u.add_argument('-t', '--terminal', action='store_true', dest='terminal', default=False, help='Display output from serial after uploading.')
    parser_w = subparsers.add_parser('watch', parents=[p_comms, p_upload])
    parser_w.set_defaults(action=_action_watch)
    parser_w.add_argument('-wait', '--wait', type=float, metavar='DELAY', default=1.0, help='Wait N seconds after detecting a file change before uploading. The default is %(default)s.')
