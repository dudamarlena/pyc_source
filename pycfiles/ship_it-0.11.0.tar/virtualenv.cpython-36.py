...
class VirtualEnvPackager(object):
    ...
    def build_virtualenv(self, virtualenv_path, upgrade_pip, upgrade_wheel):
        ...
        invoke.run('{virtualenv} {location}'.format(virtualenv=(get_virtualenv()), location=quoted_path))
        if upgrade_pip:
            invoke.run('{pip} install --upgrade pip'.format(pip=(quote(path.join(virtualenv_path, 'bin', 'pip')))))
        if upgrade_wheel:
            invoke.run('{pip} install --upgrade wheel'.format(pip=(quote(path.join(virtualenv_path, 'bin', 'pip')))))
    ...
    def run_venv_command(self, command, arg_list):
        ...
        """
        args = ' '.join(arg_list)
        command = quote(path.join(self.virtualenv_path, 'bin', command))
        invoke.run('{command} {args}'.format(command=command, args=args))
    ...
    def copy_package(self, requirements_file_path, package_path):
        ...
        self.run_venv_command('pip', ['install', '-r', req_file])
        invoke.run('find {pkg} -type f -name "*.py[co]" -delete;find {pkg} -type d -name "__pycache__" -delete'.format(pkg=pkg_path))
        invoke.run('cp -r {} {}'.format(quote(package_path.rstrip('/')), quote(self.virtualenv_path)))
    ...
    def patch_virtualenv(self, destination_path):
        ...
        invoke.run('{virtualenv} --relocatable {local_path}'.format(virtualenv=(get_virtualenv()),
          local_path=(quote(self.virtualenv_path))))
        invoke.run('sed -i "s:{local}:{dest}:" {local_activate}'.format(local=(self.virtualenv_path),
    ...
    def remove_prelink_if_applicable(self):
        ...
        """
        try:
            invoke.run('prelink -u {}'.format(quote(path.join(self.virtualenv_path, 'bin', 'python'))))
        except:
