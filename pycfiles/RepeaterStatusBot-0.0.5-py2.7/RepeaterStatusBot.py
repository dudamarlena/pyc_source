...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/RepeaterStatusBot/RepeaterStatusBot.py
# Compiled at: 2016-05-23 15:31:58
...
class RepeaterStatusBot(object):
    ...
    def __init__(self, tg_group, tg_credentials, repeater_info_url, repeater_json_stream_url):
    ...
    def signal_telegram(self, message, retry=3):
        ...
        try:
            ...
            self.logger.error(('Caught exception while transmitting message: {type}: {message}').format(type=type(error), message=str(error)))
            self._init_telegram()
            new_retry = retry - 1
    ...
    def queue_message(self, receiver, enabled):
        ...
        msg = ('"{receiver} is {status}geschakeld"').format(receiver=receiver, status=self.statuses[enabled])
        self.logger.info(('Queueing: {msg}').format(msg=msg))
        result = self.signal_telegram(msg)

    def run(self):
        for line in requests.get(self.repeater_json_stream_url, stream=True).iter_lines(1):
                ...
                try:
                    ...
                    if line_json.get('event') == 'Voter:sql_state':
                        for rx in line_json.get('rx', []).keys():
                            ...
                            if self.rx_status[rx] is not None and self.rx_status[rx] != status:
...
def get_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description='Produces an OTL time sheet for on call')
    parser.add_argument('--verbose', '-v', help='Enable verbose logging', dest='verbose', action='store_true', default=False)
    parser.add_argument('--json-url', '-j', dest='json_url', action='store', help='URL to the json stream', required=True)
    parser.add_argument('--info-url', '-i', dest='info_url', action='store', help='URL to the info stream', required=True)
    parser.add_argument('--telegram-group', '-g', dest='telegram_group', action='store', help='Telegram group to connect to', required=True)
    parser.add_argument('--telegram-credentials', '-c', dest='telegram_credentials', action='store', help='Telegram credentials', required=True)
    parser.add_argument('--logfile', '-l', dest='log_file', action='store', help='Log file to write logs to. When not specified, logs to stdout', required=False)
...
def main():
    ...
    args = get_arguments()
    setup_logging(args)
    bot = RepeaterStatusBot(tg_group=args.telegram_group, tg_credentials=args.telegram_credentials, repeater_info_url=args.info_url, repeater_json_stream_url=args.json_url)
    bot.run()
