...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/irclogger/main.py
# Compiled at: 2014-10-03 22:58:22
"""IRC Logger Daemon
...
def parse_options():
    ...
    parser.add_option('-d', '--daemon', action='store_true', default=False, dest='daemon', help='Enable daemon mode')
    parser.add_option('-c', '--channel', action='append', default=None, dest='channels', help='Channel to join (multiple allowed)')
    parser.add_option('-n', '--nick', action='store', default=environ['USER'], dest='nick', help='Nickname to use')
    parser.add_option('-o', '--output', action='store', default=getcwd(), dest='output', help='Path to store log files')
    parser.add_option('-p', '--pidfile', action='store', default=PIDFILE, dest='pidfile', help='Path to store PID file')
    parser.add_option('-v', '--verbose', action='store_true', default=False, dest='verbose', help='Enable verbose debugging mode')
...
class Bot(Component):
    ...
    def connected(self, host, port):
        ...
        name = ('{0:s} on {1:s} using circuits/{2:s}').format(nick, hostname, circuits.__version__)
    ...
    def part(self, source, channel, reason=None):
        ...
        self.fire(log(('*** {0:s} has left {1:s} ({2:s})').format(source[0], channel, reason or '')), ('logger.{0:s}').format(channel))
