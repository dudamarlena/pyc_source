# uncompyle6 version 3.7.4
# Python bytecode 3.6 (3379)
# Decompiled from: Python 3.6.9 (default, Apr 18 2020, 01:56:04) 
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/firethorn/models/base/base_object.py
# Compiled at: 2019-02-10 20:15:10
# Size of source mod 2**32: 2670 bytes
"""
Created on Feb 8, 2018

@author: stelios
"""
import urllib, json, logging, os, json

class BaseObject(object):
    __doc__ = '\n    classdocs\n    '

    def __init__(self, account, json_object=None, url=None):
        """
        Constructor
        """
        self.json_object = json_object
        self.url = url
        self.account = account
        if self.json_object == None:
            if self.url != None:
                self.json_object = self.get_json(self.url)

    @property
    def url(self):
        return self._BaseObject__url

    @url.setter
    def url(self, url):
        self._BaseObject__url = url
        if self.json_object != None:
            if url == None:
                self._BaseObject__url = self.json_object.get('self', None)

    def name(self):
        if self.json_object == None:
            if self.url != None:
                self.json_object = self.get_json(self.url)
                return self.json_object.get('name', '')
        else:
            return self.json_object.get('name', '')

    def ident(self):
        if self.json_object == None:
            if self.url != None:
                self.json_object = self.get_json(self.url)
                return os.path.basename(self.json_object.get('self', ''))
        else:
            return os.path.basename(self.json_object.get('self', ''))

    def owner(self):
        if self.json_object == None:
            if self.url != None:
                self.json_object = self.get_json(self.url)
                return self.json_object.get('owner', '')
        else:
            return self.json_object.get('owner', '')

    def get_json(self, ident, postparams=None):
        """Select a JSON HTTP resource
        
        Parameters
        ----------
        ident: string, required
            The URL being queried

        """
        data = None
        json_result = {}
        req = None
        if postparams != None:
            data = urllib.parse.urlencode(postparams).encode('utf-8')
        try:
            req = urllib.request.Request(ident, headers=(self.account.get_identity_as_headers()))
            with urllib.request.urlopen(req, data) as (response):
                json_result = json.loads(response.read().decode('utf-8'))
        except Exception as e:
            logging.exception(e)

        return json_result

    def refresh(self):
        self.json_object = self.get_json(self.url)

    def __str__(self):
        """ 
        Print Class as string
        """
        return str(json.dumps((self.json_object), indent=2))