...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ksoftapi\apis\music.py
...
class Music:
    ...
    async def lyrics(self, query: str, text_only: bool=False, clean_up: bool=True, limit: int=10) -> List[LyricResult]:
        ...
        :class:`NoResults`
        """
        r = await self._client.http.get('/lyrics/search', params={'q':query,  'text_only':text_only,  'limit':limit,  'clean_up':clean_up})
        results = r['data']
        if not results:
            raise NoResults
        return [LyricResult(lr) for lr in results]

    async def recommendations(self, tracks: list, provider: str, youtube_token: str=None, limit: int=5, recommend_type: str=None) -> List[Recommendation]:
        """|coro|
