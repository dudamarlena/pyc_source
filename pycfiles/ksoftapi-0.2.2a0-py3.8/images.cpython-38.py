...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ksoftapi\apis\images.py
# Compiled at: 2020-04-19 14:55:43
...
class Images:
    ...
    async def random_reddit(self, subreddit: str, remove_nsfw: bool=False, span: str='day') -> RedditImage:
        ...
        """
        r = await self._client.http.get(('/images/rand-reddit/{}'.format(subreddit)), params={'remove_nsfw':remove_nsfw, 
         'span':span})
        if r['code'] == 404:
            raise NoResults
        return RedditImage(r)
