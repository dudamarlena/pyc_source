...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/sphinxcontrib/erlangdomain.py
# Compiled at: 2010-08-27 01:03:51
"""
...
erl_func_sig_re = re.compile('^ ([\\w.]*:)?             # module name\n          (\\w+)  \\s*             # thing name\n          (?: \\((.*)\\)           # optional: arguments\n           (?:\\s* -> \\s* (.*))?  #           return annotation\n          )? $                   # and nothing more\n          ', re.VERBOSE)
erl_macro_sig_re = re.compile('^ ([\\w.]*:)?             # module name\n          ([A-Z]\\w+) $           # thing name\n          ', re.VERBOSE)
erl_record_sig_re = re.compile('^ ([\\w.]*:)?             # module name\n          (\\#\\w+) $              # thing name\n          ', re.VERBOSE)
erl_paramlist_re = re.compile('([\\[\\],])')
...
class ErlangObject(ObjectDescription):
    ...
    doc_field_types = [
     TypedField('parameter', label=l_('Parameters'), names=('param', 'parameter', 'arg',
                                                       'argument'), typerolename='type', typenames=('type', )),
    ...
    def _resolve_module_name(self, signode, modname, name):
        env_modname = self.options.get('module', self.env.temp_data.get('erl:module', 'erlang'))
        if modname:
            ...
            if self.objtype == 'function':
                ...
                for arity_index in range(first, last + 1):
                    if fname in finv and arity_index in finv[fname]:
                        self.env.warn(self.env.docname, 'duplicate Erlang function descriptionof %s, ' % name + 'other instance in ' + self.env.doc2path(finv[fname][arity_index][0]), self.lineno)
                ...
                if name in oinv:
                    self.env.warn(self.env.docname, 'duplicate Erlang object description of %s, ' % name + 'other instance in ' + self.env.doc2path(oinv[name][0]), self.lineno)
                oinv[name] = (
...
class ErlangDomain(Domain):
    ...
    name = 'erl'
    label = 'Erlang'
    object_types = {'function': ObjType(l_('function'), 'func'), 
       'macro': ObjType(l_('macro'), 'macro'), 
       'record': ObjType(l_('record'), 'record'), 
       'module': ObjType(l_('module'), 'mod')}
    directives = {'function': ErlangObject, 
       'macro': ErlangObject, 
       'record': ErlangObject, 
       'module': ErlangModule, 
       'currentmodule': ErlangCurrentModule}
    roles = {'func': ErlangXRefRole(), 
       'macro': ErlangXRefRole(), 
       'record': ErlangXRefRole(), 
       'mod': ErlangXRefRole()}
    initial_data = {'objects': {}, 'functions': {}, 'modules': {}}
    indices = [
     ErlangModuleIndex]
    ...
    def resolve_xref(self, env, fromdocname, builder, typ, target, node, contnode):
        if typ == 'mod':
            ...
            if not obj:
                ...
                return make_refnode(builder, fromdocname, obj, name, contnode, name)
        return
