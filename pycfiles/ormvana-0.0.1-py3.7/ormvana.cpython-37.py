...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\ormvana.py
# Compiled at: 2019-01-18 07:45:40
# Size of source mod 2**32: 9083 bytes
...
def connection():
    ...
    (bool, SQL connection object)
        The first element of the tuple indicates whether the connection is to be `close`d after every query. The second
...
class Model:
    ...
    def __init__(self, **kwargs):
        self.__dict__['record'] = self.fields.copy()
        for name, field in self.fields.items():
            self.record[name]['value'] = kwargs.pop(name, field.pop('value', ''))

        self._Model__destroy, self.cnx = connection()
    ...
    def save(self):
        ...
        if self.id:
            ...
            query += 'VALUES '
            query += '(' + ', '.join(["'{}'".format(field['value']) if field['type'] == str else str(field['value']) for name, field in self.record.items()]) + '); '
            if debug:
                print('[*] Query: ' + query)
            cursor.execute(query)
            self.cnx.commit()
    ...
    def get(cls, id):
        ...
        """
        _Model__destroy, cnx = connection()
        cursor = cnx.cursor()
