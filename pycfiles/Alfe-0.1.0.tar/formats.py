...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/formats.py
...
class Existence(Format):
    ...
    def word_values(self, value, **kwargs):
        wordset = set(t.text for t in unstopped(self.analyzer(value, **kwargs)))
...
class Characters(Positions):
    ...
    def word_values(self, value, start_pos=0, start_char=0, **kwargs):
        seen = defaultdict(list)
        for t in unstopped(self.analyzer(value, positions=True, chars=True, start_pos=start_pos, start_char=start_char, **kwargs)):
    ...
    def decode_positions(self, valuestring):
        return [ pos for pos, startchar, endchar in self.decode_characters(valuestring)
               ]
...
class PositionBoosts(Positions):
    ...
    def word_values(self, value, start_pos=0, **kwargs):
        ...
        for t in unstopped(self.analyzer(value, positions=True, boosts=True, start_pos=start_pos, **kwargs)):
            pos = t.pos
            boost = t.boost
...
class CharacterBoosts(Characters):
    ...
    def word_values(self, value, start_pos=0, start_char=0, **kwargs):
        seen = defaultdict(iter)
        for t in unstopped(self.analyzer(value, positions=True, characters=True, boosts=True, start_pos=start_pos, start_char=start_char, **kwargs)):
    ...
    def decode_characters(self, valuestring):
        return [ (pos, startchar, endchar) for pos, startchar, endchar, boost in self.decode_character_boosts(valuestring)
               ]

    def decode_position_boosts(self, valuestring):
        return [ (pos, boost) for pos, startchar, endchar, boost in self.decode_character_boosts(valuestring)
               ]