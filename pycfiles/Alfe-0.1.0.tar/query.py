...
# Embedded file name: build/bdist.linux-i686/egg/alfanous/Support/whoosh/query.py
# Compiled at: 2015-06-30 06:52:38
__doc__ = 'This module contains objects that query the search index. These query\nobjects are composable to form complex query trees.\n'
from __future__ import division
__all__ = ('QueryError', 'Term', 'And', 'Or', 'Not', 'DisjunctionMax', 'Prefix', 'Wildcard',
           'FuzzyTerm', 'TermRange', 'Variations', 'Phrase', 'NullQuery', 'Require',
           'AndMaybe', 'AndNot')
import copy
from bisect import bisect_left, bisect_right
import fnmatch, re
from alfanous.Support.whoosh.lang.morph_en import variations
from alfanous.Support.whoosh.postings import QueryScorer, EmptyScorer
from alfanous.Support.whoosh.postings import IntersectionScorer, UnionScorer
from alfanous.Support.whoosh.postings import RequireScorer, AndMaybeScorer, InverseScorer
from alfanous.Support.whoosh.postings import ReadTooFar
...
class CompoundQuery(Query):
    ...
    def __eq__(self, other):
        return other and self.__class__ is other.__class__ and self.subqueries == other.subqueries and self.boost == other.boost
    ...
    def simplify(self, ixreader):
        ...
        if subs:
            subs = self.__class__([ subq.simplify(ixreader) for subq in subs ], boost=self.boost)
            if nots:
    ...
    def _subscorers(self, searcher, exclude_docs):
        ...
        subscorers = [ subquery.scorer(searcher, exclude_docs=exclude_docs) for subquery in subs
                     ]
        return subscorers
...
class MultiTerm(Query):
    ...
    def simplify(self, ixreader):
        return Or([ Term(self.fieldname, word, boost=self.boost) for word in self._words(ixreader)
                  ])
    ...
    def estimate_size(self, ixreader):
        ...
        return sum(ixreader.doc_frequency(fieldnum, text) for text in self._words(ixreader))
...
class Term(Query):
    ...
    class TermScorer(QueryScorer):
        ...
        def __init__(self, postreader, score_fn):
            ...
            for name in ('__cmp__', 'reset', 'all_items', 'all_ids', 'all_as', 'next',
                         'skip_to', 'value', 'value_as'):
...
class And(CompoundQuery):
    ...
    def scorer(self, searcher, exclude_docs=None):
        return IntersectionScorer(self._subscorers(searcher, exclude_docs), boost=self.boost)
...
class Or(CompoundQuery):
    ...
    def scorer(self, searcher, exclude_docs=None):
        return UnionScorer(self._subscorers(searcher, exclude_docs), boost=self.boost, minmatch=self.minmatch)
...
class DisjunctionMax(CompoundQuery):
    ...
    def scorer(self, searcher, exclude_docs=None):
        return self.DisMaxScorer(self._subscorers(searcher, exclude_docs), boost=self.boost, tiebreak=self.tiebreak)
...
class FuzzyTerm(MultiTerm):
    ...
    """
    __inittypes__ = dict(fieldname=str, text=unicode, boost=float, minsimilarity=float, prefixlength=int)
...
class TermRange(MultiTerm):
    ...
    def normalize(self):
        if self.start == self.end:
            ...
            return TermRange(self.fieldname, self.start, self.end, self.startexcl, self.endexcl, boost=self.boost)

    def replace(self, oldtext, newtext):
        if self.start == oldtext:
            return TermRange(self.fieldname, newtext, self.end, self.startexcl, self.endexcl, boost=self.boost)
        else:
            if self.end == oldtext:
                return TermRange(self.fieldname, self.start, newtext, self.startexcl, self.endexcl, boost=self.boost)
            return self
...
class Phrase(MultiTerm):
    ...
    class PostingPhraseScorer(PhraseScorer):
        ...
        def _poses(self):
            return [ scorer.value_as('positions') for scorer in self.intersection.scorers
                   ]
    ...
    def __eq__(self, other):
        return other and self.__class__ is other.__class__ and self.fieldname == other.fieldname and self.words == other.word and self.slop == other.slop and self.boost == other.boost
    ...
    def normalize(self):
        if not self.words:
            ...
            return self.__class__(self.fieldname, words, slop=self.slop, boost=self.boost)
    ...
    def replace(self, oldtext, newtext):
        ...
        return Phrase(self.fieldname, [ rep(w) for w in self.words ], slop=self.slop, boost=self.boost)
    ...
    def scorer(self, searcher, exclude_docs=None):
        ...
        for word in self.words:
            if (
             fieldnum, word) not in ixreader:
                return EmptyScorer()

        wordscorers = [ Term(self.fieldname, word).scorer(searcher, exclude_docs=exclude_docs) for word in self.words
                      ]
        ...
        if intersection.id is None:
            ...
            if field.vector and field.vector.supports('positions'):
                return Phrase.VectorPhraseScorer(ixreader, fieldnum, self.words, intersection, slop=self.slop, boost=self.boost)
...
class Every(Query):
    ...
    def __eq__(self, other):
        return other and self.__class__ is other.__class__ and self.boost == other.boost
    ...
    def scorer(self, searcher, exclude_docs=None):
        ...
        return Every.EveryScorer(searcher.reader().doc_count_all(), exclude_docs, self.boost)
...
class Require(CompoundQuery):
    ...
    def scorer(self, searcher, exclude_docs=None):
        ...
        scorer = RequireScorer(scored.scorer(searcher, exclude_docs=exclude_docs), required.scorer(searcher, exclude_docs=exclude_docs))
        return scorer
...
class AndMaybe(CompoundQuery):
    ...
    def __init__(self, requiredquery, optionalquery, boost=1.0):
        ...
        """
        self.subqueries = (
         requiredquery, optionalquery)
        self.boost = boost

    def scorer(self, searcher, exclude_docs=None):
        required, optional = self.subqueries
        scorer = AndMaybeScorer(required.scorer(searcher, exclude_docs=exclude_docs), optional.scorer(searcher, exclude_docs=exclude_docs))
        return scorer
...
class AndNot(Query):
    ...
    def __eq__(self, other):
        return other and self.__class__ is other.__class__ and self.positive == other.positive and self.negative == other.negative and self.boost == other.boost
    ...
    def replace(self, oldtext, newtext):
        return AndNot(self.positive.replace(oldtext, newtext), self.negative.replace(oldtext, newtext), boost=self.boost)
