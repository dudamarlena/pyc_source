...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pylocator/colors.py
# Compiled at: 2012-04-18 08:44:03
...
class ColorChooser(gtk.Frame):
    ...
    def __init__(self, color=None):
        ...
        if color == None:
            color = gtk.gdk.Color(1.0, 1.0, 1.0)
    ...
    def choose_color(self, *args):
        new_color = choose_one_color('Choose color...', self._color)
...
gobject.signal_new('color_changed', ColorChooser, gobject.SIGNAL_RUN_FIRST, gobject.TYPE_NONE, ())
...
class ColorChooserWithPredefinedColors(gtk.HBox):
    ...
    def __init__(self, colorSeq=colorSeq):
        ...
        self.optmenu = make_option_menu(self.colorNames, self._ddlist_changed)
        self.optmenu.show()
...
gobject.signal_new('color_changed', ColorChooserWithPredefinedColors, gobject.SIGNAL_RUN_FIRST, gobject.TYPE_NONE, ())
...
def tuple2gdkColor(colorTuple):
    return gtk.gdk.Color(int(65535.0 * colorTuple[0]), int(65535.0 * colorTuple[1]), int(65535.0 * colorTuple[2]))


def gdkColor2tuple(color):
    return [ float(x) / 65535.0 for x in (color.red, color.green, color.blue) ]
