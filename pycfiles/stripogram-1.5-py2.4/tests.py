...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/stripogram/tests.py
...
class stripogramTests(TestCase):
 ...
 becomes a space"""
    ...
    def test_html2text10(self):
        """test entities are handled correctly"""
        test(html2text, ('no&lt;space', ), 'no<space')
    ...
    def test_html2safehtml3(self):
        ...
        test(html2safehtml, ('<b>x<br>y<i>z</b>a</i>b<p>', ), '<b>x<br>y<i>z</i></b>ab<p>')

    def test_html2safehtml4(self):
        """test overlapping tags are closed in the correct order 2"""
        test(html2safehtml, ('<p>x<i>y</p>z</i>a<p>b</p>', ), '<p>x<i>y</i></p>za<p>b</p>')
    ...
    def test_html2safehtml6(self):
        ...
        test(html2safehtml, ('Roses <b>are</B> red,<br/>violets <i>are</i> blue', valid_tags2), 'Roses <b>are</b> red,<br>violets <i>are</i> blue')

    def test_html2safehtml7(self):
        """check that unfinished start tags at the end of input are ignored"""
        test(html2safehtml, ('Roses <b>are</B> red,<br/QUACK', valid_tags2), 'Roses <b>are</b> red,')

    def test_html2safehtml8(self):
        """check that unfinished end tags at the end of input are ignored"""
        test(html2safehtml, ('Roses <b>are</B> red,<br/</blink>QUACK<//blink> violets <i>are</i> blue', valid_tags2), 'Roses <b>are</b> red,QUACK violets <i>are</i> blue')

    def test_html2safehtml9(self):
        """test for bug reported by J M Cerqueira Esteves <jmce@artenumerica.com>"""
        test(html2safehtml, ('Roses <b>are</B> red,QUACK<//blink', valid_tags2), 'Roses <b>are</b> red,QUACK')
    ...
    def test_html2safehtml16(self):
        """test image tag"""
        test(html2safehtml, ('<img src="fish.jpg"><img src="monkey.gif"/>', ['img']), '<img src="fish.jpg"><img src="monkey.gif">')

    def test_html2safehtml17(self):
        """test tags that aren't closed"""
        test(html2safehtml, ('<br><wbr><hr><input><isindex><base><meta><img>', ['br', 'wbr', 'hr', 'input', 'isindex', 'base', 'meta', 'img']), '<br><wbr><hr><input><isindex><base><meta><img>')

    def test_html2safehtml18(self):
        """test cross scripting vulnerability"""
        test(html2safehtml, ('<input type="submit" onClick="javascript:alert(\'Die\')">', ['input']), '<input type="submit">')

    def test_html2safehtml19(self):
        """test strict cross scripting vulnerability to handle browser defaults"""
        test(html2safehtml, ('<input type="submit" onClick="alert(\'Die\')">', ['input']), '<input type="submit">')
    ...
    def test_html2safehtml21(self):
        ...
        test(html2safehtml, ('<INPUT TyPe="submit" onClick="alert(\'Die\')"><fOnt></font>', ['INPUT', 'fOnt']), '<input type="submit"><font></font>')

    def test_html2safehtml22(self):
        """test attributes without values"""
        test(html2safehtml, ('<input type=radio name="digest" value="0" CHECKED>', ['input']), '<input type="radio" name="digest" value="0" checked>')

    def test_html2safehtml23(self):
        """test handling of broken table code"""
        test(html2safehtml, ('<table><tr><th><b></th>aaa</tr><tr><td>bbb</td></tr></table>', ['table', 'tr', 'th', 'td', 'b']), '<table><tr><th><b></b></th>aaa</tr><tr><td>bbb</td></tr></table>')
...
try:
    ...
    class ZopeTests(TestCase):
        ...
        def test_ScriptPython_html2safehtml(self):
            self.assertEqual(self._testScript("from Products.stripogram import html2safehtml\nreturn html2safehtml('<i>hello</i>')"), '<i>hello</i>')

        def test_ScriptPython_html2text(self):
            self.assertEqual(self._testScript("from Products.stripogram import html2text\nreturn html2text('<i>hello</i>')"), 'hello')
...
def test_suite():
    return TestSuite((makeSuite(stripogramTests), makeSuite(ZopeTests)))
