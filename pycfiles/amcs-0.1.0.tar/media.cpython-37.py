...
# Embedded file name: /home/phil/repos/python-amcrest/src/amcrest/media.py
...
class Media(object):
    ...
    def factory_close(self, factory_id):
        ret = self.command('mediaFileFind.cgi?action=factory.close&object={0}'.format(factory_id))
        return ret.content.decode('utf-8')

    def factory_destroy(self, factory_id):
        ret = self.command('mediaFileFind.cgi?action=factory.destroy&object={0}'.format(factory_id))
        return ret.content.decode('utf-8')

    def media_file_find_start(self, factory_id, start_time, end_time, channel=0, directories=(), types=(), flags=(), events=(), stream=None):
        """
        ...
        """
        c_dirs = ''.join(['&condition.Dirs[{0}]={1}'.format(k, v) for k, v in enumerate(directories)])
        c_types = ''.join(['&condition.Types[{0}]={1}'.format(k, v) for k, v in enumerate(types)])
        c_flag = ''.join(['&condition.Flag[{0}]={1}'.format(k, v) for k, v in enumerate(flags)])
        c_events = ''.join(['&condition.Events[{0}]={1}'.format(k, v) for k, v in enumerate(events)])
        c_vs = '&condition.VideoStream={0}'.format(stream) if stream else ''
        ret = self.command('mediaFileFind.cgi?action=findFile&object={0}&condition.Channel={1}&condition.StartTime={2}&condition.EndTime={3}{4}{5}{6}{7}{8}'.format(factory_id, channel, start_time, end_time, c_dirs, c_types, c_flag, c_events, c_vs))
        return ret.content.decode('utf-8')

    def media_file_find_next(self, factory_id, count=100):
        ret = self.command('mediaFileFind.cgi?action=findNextFile&object={0}&count={1}'.format(factory_id, count))
        return ret.content.decode('utf-8')

    def find_files(self, start_time, end_time, channel=0, directories=(), types=(), flags=(), events=(), stream=None):
        """
        https://s3.amazonaws.com/amcrest-files/Amcrest+HTTP+API+3.2017.pdf
        ...
        search = self.media_file_find_start(factory_id=factory_id,
    ...
    def download_file(self, file_path, timeout=None, stream=False):
        ...
        """
        ret = self.command(('RPC_Loadfile/{0}'.format(file_path)),
          timeout_cmd=timeout,
          stream=stream)
        return ret.content
    ...
    def download_time(self, start_time, end_time, channel=0, stream=0):
        ...
        '%Y-%m-%d%%20%H:%M:%S'
        """
        ret = self.command('loadfile.cgi?action=startLoad&channel={0}&startTime={1}&endTime={2}&subtype={3}'.format(channel, start_time, end_time, stream))
        return ret.content