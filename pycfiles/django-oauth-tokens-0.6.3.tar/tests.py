...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-oauth-tokens/oauth_tokens/tests.py
# Compiled at: 2016-01-18 08:13:57
...
oauth2_token_mock_response = {'access_token': 's' * 81, 'refresh_token': 'e' * 41, 
   'token_type': 'session', 
   'expires_in': 86301, 
...
class OAuthTokensModelTest(TestCase):
    fixtures = [
     'oauth_tokens.usercredentials.json']

    def test_updating_vk(self):
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_ADDITIONAL=VKONTAKTE_ADDITIONAL, OAUTH_TOKENS_VKONTAKTE_SCOPE=VKONTAKTE_SCOPE, OAUTH_TOKENS_VKONTAKTE_CLIENT_ID=VKONTAKTE_CLIENT_ID, OAUTH_TOKENS_VKONTAKTE_CLIENT_SECRET=VKONTAKTE_CLIENT_SECRET)
    ...
    def test_methods_access_tag(self):
        user = UserCredentialsFactory(provider='vkontakte', username=VKONTAKTE_USERNAME, password=VKONTAKTE_PASSWORD, additional=VKONTAKTE_ADDITIONAL)
    ...
    def test_getting_token_by_tag(self):
        settings_temp = dict(OAUTH_TOKENS_TWITTER_USERNAME=None, OAUTH_TOKENS_TWITTER_PASSWORD=None)
        with self.settings(**settings_temp):
        ...
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_USERNAME=None, OAUTH_TOKENS_VKONTAKTE_PASSWORD=None, OAUTH_TOKENS_VKONTAKTE_ADDITIONAL=None)
        with self.settings(**settings_temp):
            user = UserCredentialsFactory(provider='vkontakte', username=VKONTAKTE_USERNAME, password=VKONTAKTE_PASSWORD, additional=VKONTAKTE_ADDITIONAL)
            user.tags.add(Tag.objects.create(name='ads'))
        ...
        settings_temp = dict(OAUTH_TOKENS_TWITTER_USERNAME=TWITTER_USERNAME, OAUTH_TOKENS_TWITTER_PASSWORD=TWITTER_PASSWORD)
        with self.settings(**settings_temp):
            token = AccessToken.objects.get_token('twitter')
...
class FacebookAccessTokenTest(TestCase):
    ...
    def test_facebook_oauth_access_token(self):
        settings_temp = dict(OAUTH_TOKENS_FACEBOOK_USERNAME=FACEBOOK_USERNAME, OAUTH_TOKENS_FACEBOOK_PASSWORD=FACEBOOK_PASSWORD, OAUTH_TOKENS_FACEBOOK_REDIRECT_URI=None, OAUTH_TOKENS_FACEBOOK_SCOPE=FACEBOOK_SCOPE, OAUTH_TOKENS_FACEBOOK_CLIENT_ID=FACEBOOK_CLIENT_ID, OAUTH_TOKENS_FACEBOOK_CLIENT_SECRET=FACEBOOK_CLIENT_SECRET)
        with self.settings(**settings_temp):
            self.assertFacebookToken(FacebookAccessToken().get())
        return

    def test_facebook_oauth_access_token_user_in_db(self):
        settings_temp = dict(OAUTH_TOKENS_FACEBOOK_USERNAME=None, OAUTH_TOKENS_FACEBOOK_PASSWORD=None, OAUTH_TOKENS_FACEBOOK_REDIRECT_URI=None, OAUTH_TOKENS_FACEBOOK_SCOPE=FACEBOOK_SCOPE, OAUTH_TOKENS_FACEBOOK_CLIENT_ID=FACEBOOK_CLIENT_ID, OAUTH_TOKENS_FACEBOOK_CLIENT_SECRET=FACEBOOK_CLIENT_SECRET)
        with self.settings(**settings_temp):
            self.assertFacebookToken(FacebookAccessToken(username=FACEBOOK_USERNAME, password=FACEBOOK_PASSWORD).get())
        return

    def test_facebook_oauth_wrong_redirect_uri(self):
        settings_temp = dict(OAUTH_TOKENS_FACEBOOK_USERNAME=FACEBOOK_USERNAME, OAUTH_TOKENS_FACEBOOK_PASSWORD=FACEBOOK_PASSWORD, OAUTH_TOKENS_FACEBOOK_REDIRECT_URI='wrong', OAUTH_TOKENS_FACEBOOK_CLIENT_ID=FACEBOOK_CLIENT_ID, OAUTH_TOKENS_FACEBOOK_CLIENT_SECRET=FACEBOOK_CLIENT_SECRET)
        with self.settings(**settings_temp):
    ...
    def test_facebook_authorized_request(self):
        settings_temp = dict(OAUTH_TOKENS_FACEBOOK_USERNAME=FACEBOOK_USERNAME, OAUTH_TOKENS_FACEBOOK_PASSWORD=FACEBOOK_PASSWORD)
        with self.settings(**settings_temp):
...
class TwitterAccessTokenTest(TestCase):
    ...
    def test_twitter_oauth_access_token(self):
        settings_temp = dict(OAUTH_TOKENS_TWITTER_USERNAME=TWITTER_USERNAME, OAUTH_TOKENS_TWITTER_PASSWORD=TWITTER_PASSWORD, OAUTH_TOKENS_TWITTER_CLIENT_ID=TWITTER_CLIENT_ID, OAUTH_TOKENS_TWITTER_CLIENT_SECRET=TWITTER_CLIENT_SECRET)
        with self.settings(**settings_temp):
            token = TwitterAccessToken()
            self.assertTwitterToken(token, token.get())

    def test_twitter_oauth_access_token_user_in_db(self):
        settings_temp = dict(OAUTH_TOKENS_TWITTER_USERNAME=None, OAUTH_TOKENS_TWITTER_PASSWORD=None, OAUTH_TOKENS_TWITTER_CLIENT_ID=TWITTER_CLIENT_ID, OAUTH_TOKENS_TWITTER_CLIENT_SECRET=TWITTER_CLIENT_SECRET)
        with self.settings(**settings_temp):
...
class VkontakteAccessTokenTest(TestCase):
    ...
    def test_vkontakte_oauth_access_token(self):
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_USERNAME=VKONTAKTE_USERNAME, OAUTH_TOKENS_VKONTAKTE_PASSWORD=VKONTAKTE_PASSWORD, OAUTH_TOKENS_VKONTAKTE_ADDITIONAL=VKONTAKTE_ADDITIONAL, OAUTH_TOKENS_VKONTAKTE_SCOPE=VKONTAKTE_SCOPE, OAUTH_TOKENS_VKONTAKTE_CLIENT_ID=VKONTAKTE_CLIENT_ID, OAUTH_TOKENS_VKONTAKTE_CLIENT_SECRET=VKONTAKTE_CLIENT_SECRET)
        with self.settings(**settings_temp):
            self.assertVkontakteToken(VkontakteAccessToken().get())

    def test_vkontakte_oauth_access_token_user_in_db(self):
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_USERNAME=None, OAUTH_TOKENS_VKONTAKTE_PASSWORD=None, OAUTH_TOKENS_VKONTAKTE_ADDITIONAL=None, OAUTH_TOKENS_VKONTAKTE_SCOPE=VKONTAKTE_SCOPE, OAUTH_TOKENS_VKONTAKTE_CLIENT_ID=VKONTAKTE_CLIENT_ID, OAUTH_TOKENS_VKONTAKTE_CLIENT_SECRET=VKONTAKTE_CLIENT_SECRET)
        with self.settings(**settings_temp):
            self.assertVkontakteToken(VkontakteAccessToken(username=VKONTAKTE_USERNAME, password=VKONTAKTE_PASSWORD).get())
        return

    def test_vkontakte_login_password_error(self):
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_USERNAME=VKONTAKTE_USERNAME, OAUTH_TOKENS_VKONTAKTE_PASSWORD='wrong')
        with self.settings(**settings_temp):
    ...
    def test_vkontakte_authorized_request(self):
        settings_temp = dict(OAUTH_TOKENS_VKONTAKTE_USERNAME=VKONTAKTE_USERNAME, OAUTH_TOKENS_VKONTAKTE_PASSWORD=VKONTAKTE_PASSWORD, OAUTH_TOKENS_VKONTAKTE_ADDITIONAL=VKONTAKTE_ADDITIONAL)
        with self.settings(**settings_temp):
            req = VkontakteAuthRequest()
            response = requests.get(url=req.form_action_domain)
            self.assertEqual(response.status_code, 200)
...
class OdnoklassnikiAccessTokenTest(TestCase):
    ...
    def test_odnoklassniki_oauth_access_token(self):
        settings_temp = dict(OAUTH_TOKENS_ODNOKLASSNIKI_USERNAME=ODNOKLASSNIKI_USERNAME, OAUTH_TOKENS_ODNOKLASSNIKI_PASSWORD=ODNOKLASSNIKI_PASSWORD, OAUTH_TOKENS_ODNOKLASSNIKI_SCOPE=ODNOKLASSNIKI_SCOPE, OAUTH_TOKENS_ODNOKLASSNIKI_CLIENT_ID=ODNOKLASSNIKI_CLIENT_ID, OAUTH_TOKENS_ODNOKLASSNIKI_CLIENT_SECRET=ODNOKLASSNIKI_CLIENT_SECRET)
        with self.settings(**settings_temp):
    ...
    def test_odnoklassniki_authorized_request(self):
        settings_temp = dict(OAUTH_TOKENS_ODNOKLASSNIKI_USERNAME=ODNOKLASSNIKI_USERNAME, OAUTH_TOKENS_ODNOKLASSNIKI_PASSWORD=ODNOKLASSNIKI_PASSWORD)
        with self.settings(**settings_temp):
...
class InstagramAccessTokenTest(TestCase):
    ...
    def test_instagram_oauth_access_token(self):
        settings_temp = dict(OAUTH_TOKENS_INSTAGRAM_USERNAME=INSTAGRAM_USERNAME, OAUTH_TOKENS_INSTAGRAM_PASSWORD=INSTAGRAM_PASSWORD, OAUTH_TOKENS_INSTAGRAM_CLIENT_ID=INSTAGRAM_CLIENT_ID, OAUTH_TOKENS_INSTAGRAM_CLIENT_SECRET=INSTAGRAM_CLIENT_SECRET)
        with self.settings(**settings_temp):
            token = InstagramAccessToken()
            self.assertInstagramToken(token, token.get())

    def test_instagram_oauth_access_token_user_in_db(self):
        settings_temp = dict(OAUTH_TOKENS_INSTAGRAM_USERNAME=None, OAUTH_TOKENS_INSTAGRAM_PASSWORD=None, OAUTH_TOKENS_INSTAGRAM_CLIENT_ID=INSTAGRAM_CLIENT_ID, OAUTH_TOKENS_INSTAGRAM_CLIENT_SECRET=INSTAGRAM_CLIENT_SECRET)
        with self.settings(**settings_temp):
    ...
    def test_instagram_authorized_request(self):
        settings_temp = dict(OAUTH_TOKENS_INSTAGRAM_USERNAME=INSTAGRAM_USERNAME, OAUTH_TOKENS_INSTAGRAM_PASSWORD=INSTAGRAM_PASSWORD)
        with self.settings(**settings_temp):
