...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-oauth-tokens/oauth_tokens/api.py
# Compiled at: 2015-12-24 06:13:40
import logging, socket, sys, time, warnings
...
__all__ = [
 'NoActiveTokens', 'ApiAbstractBase', 'Singleton']
...
class ApiAbstractBase(object):
    ...
    def set_context(self, **kwargs):
        ...
        if self.token_tag_arg_name in kwargs:
            warnings.warn('Kwarg `%s` is deprecated, use `OAUTH_TOKENS_API_CALL_CONTEXT` instead.' % self.token_tag_arg_name, DeprecationWarning)
            self.token_tag = kwargs.pop(self.token_tag_arg_name, None)
        if self.user_arg_name in kwargs:
            warnings.warn('Kwarg `%s` is deprecated, use `OAUTH_TOKENS_API_CALL_CONTEXT` instead.' % self.user_arg_name, DeprecationWarning)
    ...
    def handle_error_no_active_tokens(self, e, *args, **kwargs):
        if self.used_access_tokens:
            ...
            self.logger.warning('Suddenly updating tokens, because no active access tokens and used_access_tokens empty, method: %s, recursion count: %d' % (
             self.method, self.recursion_count))
            self.update_tokens()
    ...
    def update_tokens(self):
        ...
        try:
            ...
            updated = False
            while not updated:
                self.logger.info('Updating access tokens, waiting for another execution, method: %s, recursion count: %d' % (
                 self.method, self.recursion_count))
                try:
                    with distributedlock(lock_name, blocking=False):
                        updated = True
