...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-oauth-tokens/oauth_tokens/providers/vkontakte.py
# Compiled at: 2015-01-28 05:41:54
import re
...
class VkontakteAuthRequest(AuthRequestBase):
    ...
    def authorized_request(self, method='get', **kwargs):
        ...
        if '<input name="code" id="code" type="text" class="text"' in response.content:
            m = re.findall("var params = {code: ge\\('code'\\).value, to: '([^']*)', al_page: '([^']+)', hash: '([^']+)'};", response.content)
            if len(m) == 0:
                raise Exception('Impossible to find security check parameters for user %s' % self.username)
            response = self.session.post(self.login_url, headers={'X-Requested-With': 'XMLHttpRequest'}, data={'act': 'security_check', 'code': self.get_setting('additional'), 
               'to': m[0][0], 
               'al_page': m[0][1], 
               'hash': m[0][2]})
        return response
