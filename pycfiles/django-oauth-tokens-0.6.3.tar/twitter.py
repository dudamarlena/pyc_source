...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-oauth-tokens/oauth_tokens/providers/twitter.py
...
class TwitterAccessToken(AccessTokenBase):
    ...
    def get(self):
        ...
        return {'access_token': self.delimeter.join([oauth_token.get('oauth_token'), oauth_token.get('oauth_token_secret')]), 'user_id': oauth_token.get('user_id')}

    def authorization_get_request(self):
        authorization_url = self.oauth.authorization_url(self.authorize_url)
        return self.auth_request.session.get(url=authorization_url)
