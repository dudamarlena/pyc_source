...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-oauth-tokens/oauth_tokens/models.py
# Compiled at: 2016-01-17 13:00:28
...
ACCESS_TOKENS_CLASSES = getattr(settings, 'OAUTH_TOKENS_CLASSES', dict([ (p, 'oauth_tokens.providers.%s.%sAccessToken' % (p, p.title())) for p in PROVIDERS
                                                                       ]))
...
class AccessTokenManager(models.Manager):
    ...
    def filter(self, *args, **kwargs):
        ...
        if tag:
            kwargs['user_credentials__in'] = UserCredentials.objects.filter(tags__name__in=[
             tag]).values_list('pk')
        return super(AccessTokenManager, self).filter(*args, **kwargs)

    def filter_active_tokens_of_provider(self, provider, *args, **kwargs):
        next_hour = datetime(datetime.now().year, datetime.now().month, datetime.now().day, datetime.now().hour) + timedelta(hours=1)
    ...
    def get_token_class(self, provider):
        ...
        try:
            ...
            raise ImproperlyConfigured('Impossible to find access token class with path %s' % ACCESS_TOKENS_CLASSES[provider])
