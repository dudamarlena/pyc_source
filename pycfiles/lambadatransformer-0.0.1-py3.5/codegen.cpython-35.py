...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/lambadalib/codegen.py
# Compiled at: 2017-04-24 15:46:12
# Size of source mod 2**32: 36208 bytes
"""
    codegen
    ~~~~~~~
...
def to_source(node, indent_with='    ', add_line_information=False, correct_line_numbers=False):
...
class SourceGenerator(NodeVisitor):
    __doc__ = 'This visitor is able to transform a well formed syntax tree into python\n    sourcecode.  For more details have a look at the docstring of the\n    `node_to_source` function.\n    '
    COMMA = ', '
    COLON = ': '
    ASSIGN = ' = '
    SEMICOLON = '; '
    ARROW = ' -> '
    BOOLOP_SYMBOLS = {And: (' and ', 5), 
     Or: (' or ', 4)}
    BINOP_SYMBOLS = {Add: (' + ', 12), 
     Sub: (' - ', 12), 
     Mult: (' * ', 13), 
     MatMult: (' @ ', 13), 
     Div: (' / ', 13), 
    ...
    CMPOP_SYMBOLS = {Eq: (' == ', 7), 
     ...
     NotEq: (' != ', 7), 
     NotIn: (' not in ', 7)}
    UNARYOP_SYMBOLS = {Invert: ('~', 14), 
     Not: ('not ', 6), 
     UAdd: ('+', 14), 
     USub: ('-', 14)}
    BLOCK_NODES = (
     If, For, While, With, Try, TryExcept, TryFinally,
    ...
    def visit_ClassDef(self, node):
        ...
        if node.bases or hasattr(node, 'keywords') and node.keywords or hasattr(node, 'starargs') and (node.starargs or node.kwargs):
            self.paren_start()
            sep = Sep(self.COMMA)
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, Num):
            ...
            self.prec_end()
        if len(node.args) == 1 and isinstance(node.args[0], GeneratorExp) and not node.keywords and hasattr(node, 'starargs') and not node.starargs and not node.kwargs:
    ...
    def visit_UnaryOp(self, node):
        ...
        self.write(symbol)
        if not PY3 and isinstance(node.op, USub) and isinstance(node.operand, Num) and (node.operand.n.real or node.operand.n.imag) >= 0:
            self.paren_start()
