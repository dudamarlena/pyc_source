...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/pypomelo/tornadoclient.py
# Compiled at: 2019-01-04 00:02:36
...
class TornadoClient(Client):
    ...
    def send(self, data):
        if not not self.iostream.closed():
            raise AssertionError('iostream has closed')
            data = isinstance(data, bytes) or bytes(data)
