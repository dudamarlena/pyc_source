...
# [GCC 8.4.0]
# Embedded file name: /media/psf/Home/Documents/workspace/rqalpha-mod-ctp/rqalpha_mod_ctp/ctp/trade_gateway.py
# Compiled at: 2017-05-27 01:22:26
...
class TradeGateway(object):
    ...
    def on_trade(self, trade_dict):
        ...
        if self._data_update_date != date.today():
            ...
            trade = Trade.__from_create__(trade_dict.order_id, trade_dict.price, trade_dict.amount, trade_dict.side, trade_dict.position_effect, trade_dict.order_book_id, trade_id=trade_dict.trade_id, commission=commission, frozen_price=trade_dict.price)
            order.fill(trade)
...
class DataCache(object):
    ...
    def cache_ins(self, ins_cache):
        self.ins = ins_cache
        self.future_info = {ins_dict.underlying_symbol:{'speculation': {'long_margin_ratio': ins_dict.long_margin_ratio, 'short_margin_ratio': ins_dict.short_margin_ratio, 'margin_type': ins_dict.margin_type}} for ins_dict in self.ins.values()}

    def cache_commission(self, underlying_symbol, commission_dict):
        self.future_info[underlying_symbol]['speculation'].update({'open_commission_ratio': commission_dict.open_ratio, 
    ...
    def positions(self):
        ...
        for order_book_id, pos_dict in iteritems(self.pos):
            ...
            position._buy_old_holding_list = [
             (
              pos_dict.prev_settle_price, pos_dict.buy_old_quantity)]
    ...
    def account(self):
        ...
        account._frozen_cash = sum([ margin_of(order_dict.order_book_id, order_dict.unfilled_quantity, order_dict.price) for order_dict in self._qry_order_cache.values() if order_dict.status == ORDER_STATUS.ACTIVE
                                   ])
