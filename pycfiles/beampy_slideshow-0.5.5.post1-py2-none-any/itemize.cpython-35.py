...
# [GCC 8.4.0]
# Embedded file name: /home/hugo/developpement/python/libperso/beampy/modules/itemize.py
# Compiled at: 2018-04-20 11:44:33
...
def itemize(items_list, **kwargs):
    ...
    if args['item_layers'] is not None and len(items_list) != len(args['item_layers']):
    ...
    with group(width=args['width'], x=args['x'], y=args['y']) as (groupitem):
        for i, the_item in enumerate(items_list):
            if args['item_style'] == 'bullet':
                ...
                if args['item_style'] == 'number':
                    item_char = str(number) + '.'
                    number += 1
                else:
                ...
                if i == 0:
                    t = text(item_char + ' ' + the_item, x=args['item_indent'], y=0, width=in_width)
                else:
                    t = text(item_char + ' ' + the_item, x=args['item_indent'], y=args['item_spacing'], width=in_width)
