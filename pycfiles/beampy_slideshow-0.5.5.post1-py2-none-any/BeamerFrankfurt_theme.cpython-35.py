...
# Compiled at: 2019-04-18 15:23:36
...
def create_header_bar():
    if len(document._TOC) == 0:
        bluerect = rectangle(x=0, y=0, height=44.800000000000004, color=lead_color, edgecolor=lead_color)
        bluerect.first()
        print('No table of content, their is a bug to set the position of the title')
        ...
        bluerect = rectangle(x=0, y=rtop.bottom + 0, height=44.800000000000004, color=lead_color, edgecolor=lead_color)
        bluerect.first()
        cpt_title = 0
        ...
        for i, toc in enumerate(document._TOC):
            ...
            if toc['level'] == 0:
                if len(elems) > 0:
                    ...
                    groups += [g]
                    elems = []
                t1 = text('\\href{%s}{%s}' % (slidelink, toc['title']), x=0, y=0, size=10, color='white', opacity=selected_opacity, width=group_width)
                elems += [t1]
                t2 = None
            if toc['level'] == 1:
                if t2 is None:
                    ...
                    xt = t2.right + 2
                t2 = circle(x=xt, y=t1.top + 15, color=circle_color, edgecolor='white', opacity=selected_opacity)
                elems += [t2]
...
def theme_maketitle(titlein, author=[], affiliation=None, meeting=None, lead_author=None, date=None):
    ...
    with group(y='center'):
        with box(width='85%', y=0, auto_height_margin=20, background_color=lead_color) as (tg):
            text(titlein, color=args['title_color'], size=args['title_size'], align='center', y=0, x='center', width='90%')
        if len(author) > 0:
            text(author_string, width='80%', y=tg.bottom + '1.5cm', color=args['author_color'], size=args['author_size'], align='center')
        if affiliation is not None:
            text(affiliation, width='90%', y='+1cm', color=args['affiliation_color'], size=args['affiliation_size'], align='center')
        if meeting is not None:
            text(meeting, width='90%', y='+1cm', color=args['meeting_color'], size=args['subtitle_size'], align='center')
        if date is not None:
