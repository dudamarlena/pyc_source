...
class slide(object):
    ...
    def newrender(self):
        ...
        for i, key in enumerate(self.element_keys):
            ...
            if elem.type != 'group':
                elem.rendered or elem.pre_render()
                elem.run_render()
...
class beampy_module(object):
    ...
    def register(self, auto_render=False):
        ...
        auto_render and self.rendered or self.pre_render()
        self.run_render()
        if not self.width.value is not None:
            raise AssertionError
            if not self.height.value is not None:
                raise AssertionError
        return
