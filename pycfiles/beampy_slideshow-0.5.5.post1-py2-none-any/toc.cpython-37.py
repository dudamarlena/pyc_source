...
class tableofcontents(group):
    ...
    def __init__(self, sections=[], subsection=True, subsubsection=True, currentsection=False, currentsubsection=False, hideothersubsection=False, **kwargs):
        ...
        super(tableofcontents, self).__init__(x=(self.x), y=(self.y), width=(self.width),
          height=(self.height),
          opengroup=False)
                    ...
                    if node['level'] == 0:
                        ...
                        if self.section_style in ('round', 'square'):
                            if self.section_style == 'round':
                                c = circle(r=(self.section_decoration_size), x=0,
                                  y=y,
                                  opacity=opacity,
                                  color=(self.section_decoration_color),
                                  edgecolor=(self.section_decoration_color))
                            else:
                                c = rectangle(width=(self.section_decoration_size * 2), height=(self.section_decoration_size * 2),
                                  x=0,
                                  y=y,
                                  opacity=opacity,
                                  color=(self.section_decoration_color),
                                  edgecolor=(self.section_decoration_color))
                            tt = text(('\\textbf{%i}' % cpt_section), x=(c.center + center(0)),
                              y=(c.center + center(0)),
                        ...
                        if self.section_style == 'number':
                            c = text(('\\textbf{%i}' % cpt_section), x=0,
                              y=y,
                              color=(self.section_number_color),
                              size=(self.section_decoration_size))
                            text_elements += [c]
                            deco_x = c.right + 5
                            deco_y = c.center + center(0)
                        prev = text(('\\href{%s}{%s}' % (slidelink, node['title'])), x=deco_x,
                          y=deco_y,
                          color=(self.section_text_color),
                          opacity=opacity,
                          width=(document._slides[self.slide_id].curwidth - (self.section_decoration_size * 2 + 5)))
                        text_elements += [prev]
                        section = prev
                        cpt_section += 1
                if self.show_subsection:
                    if self.show_section:
                        if node['level'] == 1:
                            cpt_subsubsection = 1
                            ...
                            if self.subsection_style == 'number':
                                c = text(('%i-%i' % (cpt_section - 1, cpt_subsection)), x=x,
                                  y=y,
                            ...
                            if self.subsection_style in ('round', 'square'):
                                if self.subsection_style == 'round':
                                    c = circle(r=(self.subsection_decoration_size), x=x,
                                      y=y,
                                      opacity=opacity,
                                      color=(self.subsection_decoration_color),
                                      edgecolor=(self.subsection_decoration_color))
                                else:
                                    c = rectangle(width=(self.subsection_decoration_size * 2), height=(self.subsection_decoration_size * 2),
                                      x=x,
                                      y=y,
                                      opacity=opacity,
                                      color=(self.subsection_decoration_color),
