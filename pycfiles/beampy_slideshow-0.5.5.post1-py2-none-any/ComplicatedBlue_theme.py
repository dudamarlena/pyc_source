...
THEME['title'] = {'size': 28, 
   'font': 'CMR', 
   'color': lead_color, 
   'x': {'shift': 0.5, 'unit': 'cm'}, 'y': {'shift': 1.2, 'unit': 'cm'}, 'reserved_y': '1.5cm', 
   'align': '', 
   'va': 'baseline'}
...
def background_layout():
    ...
    t1 = text('%i/%i' % (cur_slide, N), x={'shift': 0.01, 'align': 'right', 'anchor': 'right'}, y=prog.top - bottom(5), size=13, color='LightBlue')
    return
