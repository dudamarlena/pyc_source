...
# Size of source mod 2**32: 168885 bytes
...
def is_same_direction(x1, y1, x2, y2):
    if is_same_sign(x1, x2):
        if is_same_sign(y1, y2):
            diff = y1 / x1 - y2 / x2
...
def removeUnusedDefs(doc, defElem, elemsToRemove=None, referencedIDs=None):
    ...
    for elem in defElem.childNodes:
        if elem.nodeType == Node.ELEMENT_NODE:
            if elem.getAttribute('id') == '' or elem.getAttribute('id') not in referencedIDs:
                if elem.nodeName == 'g':
                    if elem.namespaceURI == NS['SVG']:
                        elemsToRemove = removeUnusedDefs(doc, elem, elemsToRemove, referencedIDs=referencedIDs)
            if elem.nodeName not in keepTags:
...
def unprotected_ids(doc, options):
    ...
    identifiedElements = findElementsWithId(doc.documentElement)
    if not options.protect_ids_noninkscape:
        if not options.protect_ids_list:
            if not options.protect_ids_prefix:
    ...
    for id in list(identifiedElements):
        protected = False
        if options.protect_ids_noninkscape:
            if not id[(-1)].isdigit():
                protected = True
            elif options.protect_ids_list and id in protect_ids_list:
                protected = True
            if options.protect_ids_prefix:
...
def createGroupsForCommonAttributes(elem):
    ...
    global _num_elements_removed
    num = 0
    for curAttr in ('clip-rule', 'display-align', 'fill', 'fill-opacity', 'fill-rule',
    ...
    for rid, nodes in six.iteritems(findReferencedElements(doc.documentElement)):
        if len(nodes) == 1:
            if rid in identifiedElements:
                ...
                if elem is not None:
                    if elem.nodeType == Node.ELEMENT_NODE:
                        if elem.nodeName in ('linearGradient', 'radialGradient') and elem.namespaceURI == NS['SVG']:
                            refElem = nodes[0]
                        ...
                        for attr in ('gradientUnits', 'spreadMethod', 'gradientTransform'):
                            if refElem.getAttribute(attr) == '':
                                elem.getAttribute(attr) == '' or refElem.setAttributeNS(None, attr, elem.getAttribute(attr))

                        if elem.nodeName == 'radialGradient':
                            if refElem.nodeName == 'radialGradient':
                                for attr in ('fx', 'fy', 'cx', 'cy', 'r'):
                                    if refElem.getAttribute(attr) == '':
                                        elem.getAttribute(attr) == '' or refElem.setAttributeNS(None, attr, elem.getAttribute(attr))

                    if elem.nodeName == 'linearGradient':
                        if refElem.nodeName == 'linearGradient':
...
def _getStyle(node):
    ...
    if node.nodeType == Node.ELEMENT_NODE:
        if len(node.getAttribute('style')) > 0:
            styleMap = {}
    ...
    if styleMap:
        for prop in ('fill', 'stroke'):
            if prop in styleMap:
                ...
                if len(chunk) == 2:
                    if chunk[0][:5] == 'url(#' or chunk[0][:6] == 'url("#' or chunk[0][:6] == "url('#":
                        if chunk[1] == 'rgb(0, 0, 0)':
        ...
        if 'opacity' in styleMap:
            ...
            if opacity == 0.0:
                for uselessStyle in ('fill', 'fill-opacity', 'fill-rule', 'stroke',
                                     'stroke-linejoin', 'stroke-opacity', 'stroke-miterlimit',
                                     'stroke-linecap', 'stroke-dasharray', 'stroke-dashoffset',
                                     'stroke-opacity'):
                    if uselessStyle in styleMap:
                        del (styleInheritedByChild(node, uselessStyle) or styleMap)[uselessStyle]
                        num += 1

        if 'stroke' in styleMap:
            if styleMap['stroke'] == 'none':
                for strokestyle in ('stroke-width', 'stroke-linejoin', 'stroke-miterlimit',
                                    'stroke-linecap', 'stroke-dasharray', 'stroke-dashoffset',
                                    'stroke-opacity'):
                    if strokestyle in styleMap:
                        del (styleInheritedByChild(node, strokestyle) or styleMap)[strokestyle]
                        num += 1
            ...
            if styleMap['fill'] == 'none':
                for fillstyle in ('fill-rule', 'fill-opacity'):
                    if fillstyle in styleMap:
                        del (styleInheritedByChild(node, fillstyle) or styleMap)[fillstyle]
                        num += 1
            ...
            if fillOpacity == 0.0:
                for uselessFillStyle in ('fill', 'fill-rule'):
                    if uselessFillStyle in styleMap:
                        del (styleInheritedByChild(node, uselessFillStyle) or styleMap)[uselessFillStyle]
                        num += 1

        if 'stroke-opacity' in styleMap:
            strokeOpacity = float(styleMap['stroke-opacity'])
            if strokeOpacity == 0.0:
                for uselessStrokeStyle in ('stroke', 'stroke-width', 'stroke-linejoin',
                                           'stroke-linecap', 'stroke-dasharray',
                                           'stroke-dashoffset'):
                    if uselessStrokeStyle in styleMap:
                        del (styleInheritedByChild(node, uselessStrokeStyle) or styleMap)[uselessStrokeStyle]
                        num += 1
        ...
        if not mayContainTextNodes(node):
            for fontstyle in ('font-family', 'font-size', 'font-stretch', 'font-size-adjust',
                              'font-style', 'font-variant', 'font-weight', 'letter-spacing',
                              'line-height', 'kerning', 'text-align', 'text-anchor',
...
def styleInheritedByChild(node, style, nodeIsChild=False):
    ...
    if node.nodeType != Node.ELEMENT_NODE:
        ...
        if nodeIsChild:
            ...
            if style in styles:
                return styles[style] == 'inherit' or False
...
def mayContainTextNodes(node):
    ...
    if node.nodeType != Node.ELEMENT_NODE:
        ...
        if node.namespaceURI != NS['SVG']:
            ...
            if node.nodeName in ('rect', 'circle', 'ellipse', 'line', 'polygon', 'polyline',
                                 'path', 'image', 'stop'):
                result = False
            else:
                if node.nodeName in ('g', 'clipPath', 'marker', 'mask', 'pattern',
...
def removeDefaultAttributeValue(node, attribute):
    ...
    if not node.hasAttribute(attribute.name):
        ...
        if isinstance(attribute.value, str):
            if not node.getAttribute(attribute.name) == attribute.value or attribute.conditions is None or attribute.conditions(node):
                node.removeAttribute(attribute.name)
                return 1
    else:
        nodeValue = SVGLength(node.getAttribute(attribute.name))
        if not ((attribute.value is None or nodeValue.value) == attribute.value):
            if not (attribute.units is None or nodeValue.units == attribute.units or isinstance)(attribute.units, list) or nodeValue.units in attribute.units:
                if not attribute.conditions is None:
                    if attribute.conditions(node):
                        node.removeAttribute(attribute.name)
                        return 1
    return 0
...
def removeDefaultAttributeValues(node, options, tainted=set()):
    ...
    for attribute in attributes:
        if attribute not in tainted and attribute in default_properties:
...
def convertColor(value):
    ...
    if rgbpMatch is not None:
        ...
        if rgbMatch is not None:
            ...
            s = s.lower()
            if len(s) == 7 and s[1] == s[2] and s[3] == s[4]:
                if s[5] == s[6]:
                    s = '#' + s[1] + s[3] + s[5]
        return s
...
def cleanPath(element, options):
    ...
    path = [elem for elem in path if not len(elem[1]) > 0 if elem[0] == 'z']
    newPath = [
     path[0]]
    ...
    for cmd, data in path:
        if prevCmd == '':
            prevCmd = cmd
            prevData = data
        elif cmd != 'm' and not cmd == prevCmd:
            if not cmd == 'l' or prevCmd == 'm':
    ...
    newPath = [path[0]]
    for cmd, data in path[1:]:
        if prevCmd != '' and not cmd != prevCmd:
            if cmd == 'm':
                newPath.append((prevCmd, prevData))
                prevCmd = ''
                prevData = []
...
def scourCoordinates(data, options, force_whitespace=False, control_points=[], flags=[]):
    ...
    if data is not None:
        ...
        for coord in data:
            ...
            scouredCoord = scourUnitlessLength(coord, renderer_workaround=(options.renderer_workaround),
              is_control_point=is_control_point)
            if c > 0:
                if not ((force_whitespace or scouredCoord[0].isdigit() or scouredCoord[0]) == '.'):
                    if c - 1 not in flags or options.renderer_workaround:
                        newData.append(' ')
...
def scourUnitlessLength(length, renderer_workaround=False, is_control_point=False):
    ...
    if (renderer_workaround or len(nonsci)) > 2 and nonsci[:2] == '0.':
        nonsci = nonsci[1:]
    else:
        if len(nonsci) > 3:
            if nonsci[:3] == '-0.':
...
def reducePrecision(element):
    ...
    num = 0
    styles = _getStyle(element)
    for lengthAttr in ('opacity', 'flood-opacity', 'fill-opacity', 'stroke-opacity',
                       'stop-opacity', 'stroke-miterlimit', 'stroke-dashoffset',
                       'letter-spacing', 'word-spacing', 'kerning', 'font-size-adjust',
...
def optimizeTransform(transform):
        ...
        if transform[0][0] == 'matrix':
            ...
            if matrix == [1, 0, 0, 1, 0, 0]:
                ...
                if A1 == 1 and A2 == 0 and B1 == 0 and B2 == 1:
                    ...
                    if A2 == 0 and A3 == 0 and B1 == 0 and B3 == 0:
                        transform[0] = (
                         'scale', [A1, B2])
                    else:
                        if A1 == B2:
                            if -1 <= A1 <= 1:
                                if A3 == 0 and -B1 == A2:
                                    if -1 <= B1 <= 1:
                                        if B3 == 0:
                                            if abs(B1 ** 2 + A1 ** 2 - 1) < Decimal('1e-15'):
                                                sin_A, cos_A = B1, A1
    ...
    while i < len(transform):
        ...
        if currType == prevType == 'translate':
            ...
            if len(prevArgs) == len(currArgs) == 1:
            ...
            if len(prevArgs) == 2 and len(currArgs) == 2:
                ...
                if len(prevArgs) == 1 and len(currArgs) == 2:
                    ...
                    del transform[i]
            if not prevArgs[0] == 1 or len(prevArgs) == 1 or prevArgs[1] == 1:
                i -= 1
                del transform[i]
        else:
            i += 1
    ...
    while i < len(transform):
        ...
        if currType == 'skewX' or currType == 'skewY':
            if len(currArgs) == 1 and currArgs[0] == 0:
                del transform[i]
...
def embedRasters(element, options):
    ...
    try:
        try:
            ...
            try:
                print(("WARNING: Could not open file '" + href + "' for embedding. The raster image will be kept as a reference but might be invalid. (Exception details: " + str(e) + ')'),
...
def properlySizeDoc(docElement, options):
    ...
    if options.renderer_workaround:
        if w.units != Unit.NONE and w.units != Unit.PX or h.units != Unit.NONE:
            if h.units != Unit.PX:
                return
...
def scourString(in_string, options=None):
    ...
    if options.strip_comments:
    ...
    elif options.strip_xml_space_attribute:
                ...
                if removeElem is False:
                    for child in elem.childNodes:
                        if child.nodeType in [Node.ELEMENT_NODE, Node.CDATA_SECTION_NODE, Node.COMMENT_NODE]:
                            ...
                            if child.nodeType == Node.TEXT_NODE and not child.nodeValue.isspace():
                                break
                    else:
                        removeElem = True
        ...
        for type in ('svg', 'image', 'rect', 'circle', 'ellipse', 'line', 'linearGradient',
...
_options_parser = optparse.OptionParser(usage='%prog [INPUT.SVG [OUTPUT.SVG]] [OPTIONS]',
  description='If the input/output files are not specified, stdin/stdout are used. If the input/output files are specified with a svgz extension, then compressed SVG is assumed.',
...
_options_parser.add_option('-q', '--quiet', action='store_true',
  dest='quiet',
  default=False,
  help='suppress non-error output')
_options_parser.add_option('-v', '--verbose', action='store_true',
  dest='verbose',
  default=False,
  help='verbose output (statistics, etc.)')
_options_parser.add_option('-i', action='store',
  dest='infilename',
  metavar='INPUT.SVG',
  help='alternative way to specify input filename')
_options_parser.add_option('-o', action='store',
  dest='outfilename',
  metavar='OUTPUT.SVG',
...
_option_group_optimization.add_option('--set-precision', action='store',
  type=int,
  dest='digits',
  default=5,
  metavar='NUM',
  help='set number of significant digits (default: %default)')
_option_group_optimization.add_option('--set-c-precision', action='store',
  type=int,
  dest='cdigits',
  default=(-1),
  metavar='NUM',
  help="set number of significant digits for control points (default: same as '--set-precision')")
_option_group_optimization.add_option('--disable-simplify-colors', action='store_false',
  dest='simple_colors',
  default=True,
  help="won't convert colors to #RRGGBB format")
_option_group_optimization.add_option('--disable-style-to-xml', action='store_false',
  dest='style_to_xml',
  default=True,
  help="won't convert styles into XML attributes")
_option_group_optimization.add_option('--disable-group-collapsing', action='store_false',
  dest='group_collapse',
  default=True,
  help="won't collapse <g> elements")
_option_group_optimization.add_option('--create-groups', action='store_true',
  dest='group_create',
  default=False,
  help='create <g> elements for runs of elements with identical attributes')
_option_group_optimization.add_option('--keep-editor-data', action='store_true',
  dest='keep_editor_data',
  default=False,
  help="won't remove Inkscape, Sodipodi, Adobe Illustrator or Sketch elements and attributes")
_option_group_optimization.add_option('--keep-unreferenced-defs', action='store_true',
  dest='keep_defs',
  default=False,
  help="won't remove elements within the defs container that are unreferenced")
_option_group_optimization.add_option('--renderer-workaround', action='store_true',
  dest='renderer_workaround',
  default=True,
  help='work around various renderer bugs (currently only librsvg) (default)')
_option_group_optimization.add_option('--no-renderer-workaround', action='store_false',
  dest='renderer_workaround',
  default=True,
...
_option_group_document.add_option('--strip-xml-prolog', action='store_true',
  dest='strip_xml_prolog',
  default=False,
  help="won't output the XML prolog (<?xml ?>)")
_option_group_document.add_option('--remove-titles', action='store_true',
  dest='remove_titles',
  default=False,
  help='remove <title> elements')
_option_group_document.add_option('--remove-descriptions', action='store_true',
  dest='remove_descriptions',
  default=False,
  help='remove <desc> elements')
_option_group_document.add_option('--remove-metadata', action='store_true',
  dest='remove_metadata',
  default=False,
  help='remove <metadata> elements (which may contain license/author information etc.)')
_option_group_document.add_option('--remove-descriptive-elements', action='store_true',
  dest='remove_descriptive_elements',
  default=False,
  help='remove <title>, <desc> and <metadata> elements')
_option_group_document.add_option('--enable-comment-stripping', action='store_true',
  dest='strip_comments',
  default=False,
  help='remove all comments (<!-- -->)')
_option_group_document.add_option('--disable-embed-rasters', action='store_false',
  dest='embed_rasters',
  default=True,
  help="won't embed rasters as base64-encoded data")
_option_group_document.add_option('--enable-viewboxing', action='store_true',
...
_option_group_formatting.add_option('--indent', action='store',
  type='string',
  dest='indent_type',
  default='space',
  metavar='TYPE',
  help='indentation of the output: none, space, tab (default: %default)')
_option_group_formatting.add_option('--nindent', action='store',
  type=int,
  dest='indent_depth',
  default=1,
  metavar='NUM',
  help='depth of the indentation, i.e. number of spaces/tabs: (default: %default)')
_option_group_formatting.add_option('--no-line-breaks', action='store_false',
  dest='newlines',
  default=True,
  help='do not create line breaks in output(also disables indentation; might be overridden by xml:space="preserve")')
_option_group_formatting.add_option('--strip-xml-space', action='store_true',
...
_option_group_ids.add_option('--enable-id-stripping', action='store_true',
  dest='strip_ids',
  default=False,
  help='remove all unreferenced IDs')
_option_group_ids.add_option('--shorten-ids', action='store_true',
  dest='shorten_ids',
  default=False,
  help='shorten all IDs to the least number of letters possible')
_option_group_ids.add_option('--shorten-ids-prefix', action='store',
  type='string',
  dest='shorten_ids_prefix',
  default='',
  metavar='PREFIX',
  help='add custom prefix to shortened IDs')
_option_group_ids.add_option('--protect-ids-noninkscape', action='store_true',
  dest='protect_ids_noninkscape',
  default=False,
  help="don't remove IDs not ending with a digit")
_option_group_ids.add_option('--protect-ids-list', action='store',
  type='string',
  dest='protect_ids_list',
  metavar='LIST',
  help="don't remove IDs given in this comma-separated list")
_option_group_ids.add_option('--protect-ids-prefix', action='store',
  type='string',
...
_option_group_compatibility.add_option('--error-on-flowtext', action='store_true',
...
def start(options, input, output):
    ...
    if not input is sys.stdin:
        hasattr(sys.stdin, 'buffer') and input is sys.stdin.buffer or input.close()
    if not output is sys.stdout:
        hasattr(sys.stdout, 'buffer') and output is sys.stdout.buffer or output.close()
    end = time.time()
    duration = int(round((end - start) * 1000.0))
    oldsize = len(in_string)
    newsize = len(out_string)
    sizediff = newsize / oldsize * 100.0
    options.quiet or print(('Scour processed file "{}" in {} ms: {}/{} bytes new/orig -> {:.1f}%'.format(input.name, duration, newsize, oldsize, sizediff)),
      file=(options.ensure_value('stdout', sys.stdout)))
    if options.verbose:
