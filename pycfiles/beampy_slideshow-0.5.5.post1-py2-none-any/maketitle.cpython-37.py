...
# [GCC 8.4.0]
# Embedded file name: /home/hugo/developpement/python/libperso/beampy/modules/maketitle.py
# Compiled at: 2018-12-13 15:43:19
...
def default_maketitle(titlein, author=None, subtitle=None, date=None, title_width=None, vert_space=None):
    ...
    with group(y='center'):
        text(titlein, width=title_width, y=0, color=(args['title_color']), size=(args['title_size']),
          align='center')
        if author is not None:
            if isinstance(author, str):
                text(author, width=title_width, y=('+' + str(2 * vert_space)), color=(args['author_color']),
                  size=(args['author_size']),
                  align='center')
            else:
                if isinstance(author, list):
                    text((', '.join(author)), width=title_width, y=('+' + str(vert_space)),
                      color=(args['author_color']),
                      size=(args['author_size']),
                      align='center')
        if subtitle is not None:
            text(subtitle, width=title_width, y=('+' + str(vert_space)), color=(args['subtitle_color']),
              size=(args['subtitle_size']))
        if date is not None:
            text(date, width=title_width, y=('+' + str(vert_space)), color=(args['date_color']),
              size=(args['date_size']))
