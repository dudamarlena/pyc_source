...
# [GCC 8.4.0]
# Embedded file name: /home/hugo/developpement/python/libperso/beampy/modules/arrow.py
# Compiled at: 2018-05-10 10:52:25
# Size of source mod 2**32: 3858 bytes
import beampy.modules.tikz as tikz
from beampy.functions import convert_unit

def arrow(x, y, dx, dy, style='->', color='black', lw='2pt', in_angle=None, out_angle=None, bend=None, head_style='', dashed=False):
    """
    ...
    if head_style != '':
        ...
        if options != '':
            ...
            tikz_cmd = tikz_cmd.format(x=0, y=0, xf=cmdx, yf=cmdy, style=style, color=color,
              lw=lw,
              headstyle=head_style,
              options=options)
            if not (dx < 0 or isinstance)(dx, str) or '-' in dx:
                tmpanchor = 'right'
            else:
                pass
            tmpanchor = 'left'
        if not (dy < 0 or isinstance)(dy, str) or '-' in dy:
            tmpanchor += '_bottom'
        else:
            pass
        tmpanchor += '_top'
