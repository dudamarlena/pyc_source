...
# [GCC 8.4.0]
# Embedded file name: /home/hugo/developpement/python/beampy_git/beampy/geometry.py
...
import operator, sys
DEFAULT_X = {'align':'left', 
 'reference':'slide',  'shift':0,  'unit':'width', 
 'anchor':'left'}
...
class positionner:
            ...
            if isinstance(self.y, dict):
                ...
                if isinstance(self.y, str):
                    ...
                    if self.x['shift'] > 1.0 and self.x['unit'] in ('width', 'height'):
                        self.x['unit'] = 'px'
                if self.y['shift'] > 1.0:
                    if self.y['unit'] in ('width', 'height'):
                        self.y['unit'] = 'px'
    ...
    def place(self, available_size, ytop=0):
        ...
        if self.y['align'] == 'center':
            self.y['final'] = element_centering((self.height.value), (available_height - ytop),
