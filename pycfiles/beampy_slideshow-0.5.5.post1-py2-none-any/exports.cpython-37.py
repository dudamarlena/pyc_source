...
# Compiled at: 2019-07-06 08:30:22
# Size of source mod 2**32: 18687 bytes
"""
...
def save(output_file=None, format=None):
    ...
    if 'html' in file_ext or format == 'html5':
        ...
        if 'svg' in file_ext or format == 'svg':
            ...
            if 'pdf' in file_ext or format == 'pdf':
    ...
    for islide in range(document._global_counter['slide'] + 1):
        ...
        if 'glyphs' in document._global_store:
            if py3:
                ...
                glyphs_svg = '<defs>%s</defs>' % ''.join([glyph['svg'] for glyph in document._global_store['glyphs'].values()]).decode('utf-8', errors='replace')
        else:
            glyphs_svg = ''
        if py3:
