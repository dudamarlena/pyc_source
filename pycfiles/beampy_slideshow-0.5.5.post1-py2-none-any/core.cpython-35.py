...
# Compiled at: 2019-04-18 16:15:25
# Size of source mod 2**32: 57036 bytes
"""
...
class slide(object):
    ...
    def add_module(self, module_id, module_content):
        ...
        elif module_content.grouplevel > 0:
    ...
    def build_layout(self):
        ...
        """
        if self.render_layout and self.args['layout'] is not None and 'function' in str(type(self.args['layout'])):
    ...
    def newrender(self):
        ...
        header_template = svg_template.format(width=document._width, height=document._height, bgcolor=self.args['background'])
...
class beampy_module(object):
    ...
    def export_animation_layer(self, layer):
        ...
        out = out.format(slide=self.slide_id, layer=layer, id_anim=self.anim_num, x=self.positionner.x['final'], y=self.positionner.y['final'], fps=self.fps, anim_size=len(self.animout), frame_init=self.animout[0])
        return out
...
class group(beampy_module):
    ...
    def compute_group_size(self):
        ...
        if len(self.autoxid) > 0:
            distribute(self.autoxid, 'hspace', auto_width, offset=self.xoffset, curslide=document._slides[self.slide_id])
        if len(self.autoyid) > 0:
            distribute(self.autoyid, 'vspace', auto_height, offset=self.yoffset, curslide=document._slides[self.slide_id])
