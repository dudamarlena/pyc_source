...
# Compiled at: 2019-07-07 19:09:45
# Size of source mod 2**32: 27937 bytes
"""
...
def unit_operation--- This code section failed: ---
 ...
 L.  44        50  LOAD_STR                 '-'
               52  LOAD_FAST                'value'
               54  COMPARE_OP               in
               56  POP_JUMP_IF_FALSE    98  'to 98'

 L.  45        58  LOAD_FAST                'value'
               60  LOAD_METHOD              split
               62  LOAD_STR                 '-'
               64  CALL_METHOD_1         1  '1 positional argument'
               66  STORE_FAST               'vsplited'

 L.  46        68  SETUP_LOOP           98  'to 98'
               70  LOAD_FAST                'vsplited'
               72  GET_ITER         
               74  FOR_ITER             96  'to 96'
               76  STORE_FAST               'v'

 L.  47        78  LOAD_FAST                'to'
               80  LOAD_GLOBAL              float
               82  LOAD_GLOBAL              convert_unit
               84  LOAD_FAST                'v'
               86  CALL_FUNCTION_1       1  '1 positional argument'
               88  CALL_FUNCTION_1       1  '1 positional argument'
               90  INPLACE_SUBTRACT 
               92  STORE_FAST               'to'
               94  JUMP_BACK            74  'to 74'
               96  POP_BLOCK        
             98_0  COME_FROM_LOOP       68  '68'
             98_1  COME_FROM            56  '56'
             98_2  COME_FROM            48  '48'
             98_3  COME_FROM_LOOP       18  '18'

 L.  49        98  LOAD_STR                 '%0.1f'
              100  LOAD_FAST                'to'
              102  BINARY_MODULO    
              104  RETURN_VALUE     
               -1  RETURN_LAST      

Parse error at or near `COME_FROM_LOOP' instruction at offset 98_3
...
def create_element_id(bpmod, use_args=True, use_name=True, use_content=True, add_slide=True, slide_position=True):
    ...
    if use_args:
        if bpmod.args is not None:
            ct_to_hash += ''.join(['%s:%s' % (k, v) for k, v in bpmod.args.items()])
    if use_name:
        if bpmod.name is not None:
            ct_to_hash += bpmod.name
    if use_content:
        if bpmod.content is not None:
        ...
        for islide in range(len(document._slides) - 1):
            ...
            for cid in document._slides[sid].element_keys:
                ...
                if e.type == 'text' and e.usetex:
                    e.rendered or elements_to_render += [e]
...
PYTHON_XMLFIND_REGEX = re.compile('<\\?xml[^>]+>')
