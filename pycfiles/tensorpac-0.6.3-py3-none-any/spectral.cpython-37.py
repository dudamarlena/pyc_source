...
# [GCC 8.4.0]
# Embedded file name: /run/media/etienne/DATA/Toolbox/tensorpac/tensorpac/spectral.py
...
def spectral(x, sf, f, stype, dcomplex, cycle, width, n_jobs):
    ...
    if dcomplex is 'hilbert':
        ...
        for k in range(n_freqs):
            ...
            _b, a[k] = fir1(forder[k], f[k, :] / (sf / 2.0))
            b += [_b]

        xf = Parallel(n_jobs=n_jobs, **CONFIG['JOBLIB_CFG'])((delayed(filtfilt)((b[k]), (a[k]), x, padlen=(forder[k]), axis=(-1)) for k in range(n_freqs)))
        xd = np.asarray(xf)
        if stype is not None:
            xd = hilbertm(xd)
    else:
        if dcomplex is 'wavelet':
            f = f.mean(1)
            xd = Parallel(n_jobs=n_jobs, **CONFIG['JOBLIB_CFG'])((delayed(morlet)(x, sf, k, width) for k in f))
        if stype is 'pha':
            return np.angle(xd)
        if stype is 'amp':
            return np.abs(xd)
...
def fir1(n, wn):
    ...
    wind = np.hamming(lo)
    b = h * wind
    c = np.exp(complex(0.0, -2.0) * np.pi * (f0 / 2) * np.array(range(lo)))
    b /= abs(c @ b)
    return (
     b, 1)
...
def morlet(x, sf, f, width=7.0):
    ...
    m = a * np.exp(-np.square(t) / (2 * np.square(st))) * np.exp(complex(0.0, 2.0) * np.pi * f * t)
