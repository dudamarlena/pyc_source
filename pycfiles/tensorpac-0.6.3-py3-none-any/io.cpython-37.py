...
# [GCC 8.4.0]
# Embedded file name: /run/media/etienne/DATA/Toolbox/tensorpac/tensorpac/io.py
...
class _Formatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if '*' in msg:
            ...
            to_color = COLOR_SEQ % (30 + RED) + re_search + COLOR_SEQ % (30 + WHITE) + RESET_SEQ
...
def progress_bar(value, endvalue, bar_length=20, pre_st=None):
    ...
    sys.stdout.write('\r{0} [{1}] {2}%'.format(pre_st, arrow + spaces, int(round(percent * 100))))
    sys.stdout.flush()
...
def is_pandas_installed():
    ...
    try:
        ...
        raise IOError('pandas not installed. See https://pandas.pydata.org/pandas-docs/stable/install.html')
...
def is_statsmodels_installed():
    ...
    try:
        ...
        raise IOError('statsmodels not installed. See http://www.statsmodels.org/stable/install.html')