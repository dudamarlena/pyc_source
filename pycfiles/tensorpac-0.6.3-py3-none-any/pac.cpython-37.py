...
# [GCC 8.4.0]
# Embedded file name: /run/media/etienne/DATA/Toolbox/tensorpac/tensorpac/pac.py
# Compiled at: 2019-10-07 10:13:11
# Size of source mod 2**32: 32717 bytes
"""Main PAC class."""
import numpy as np, logging
from tensorpac.spectral import spectral, hilbertm
from tensorpac.methods import get_pac_fcn, pacstr, compute_surrogates, erpac, ergcpac, _ergcpac_perm, preferred_phase, normalize
...
class _PacObj(object):
    __doc__ = 'Main class for relative PAC objects.'

    def __init__(self, f_pha=[
 2, 4], f_amp=[60, 200], dcomplex='hilbert', cycle=(3, 6), width=7):
    ...
    def filter(self, sf, x, ftype='phase', keepfilt=False, edges=None, n_jobs=-1):
        ...
        if not isinstance(sf, (int, float)):
            raise AssertionError('The sampling frequency must be a float number.')
        elif keepfilt is True:
            if self._dcomplex is 'wavelet':
                raise ValueError("Using wavelet for the complex decomposition do not allow to get filtered data only. Set the keepfilt parameter to False or set dcomplex to 'hilbert'.")
            else:
                assert ftype in ('phase', 'amplitude'), "ftype must either be 'phase' or 'amplitude.'"
                mne_epochs_type = CONFIG['MNE_EPOCHS_TYPE']
                if not isinstance(x, np.ndarray):
                    if type(x) in mne_epochs_type:
                        x = x.get_data()
                        sf = x.info['sfreq']
            if x.ndim == 1:
                x = x[np.newaxis, :]
            assert x.ndim == 2, 'x should be a 2d array like (n_epochs, n_times)'
            edges = isinstance(edges, int) or slice(None)
        else:
        ...
        if ftype is 'phase':
            ...
            xfilt = spectral(x, sf, self.f_pha, tosend, self._dcomplex, self._cycle[0], self._width, n_jobs)
        else:
            if ftype is 'amplitude':
                tosend = 'amp' if not keepfilt else None
                xfilt = spectral(x, sf, self.f_amp, tosend, self._dcomplex, self._cycle[1], self._width, n_jobs)
            return xfilt[(..., edges)]
    ...
    def _speccheck(self, dcomplex=None, cycle=None, width=None):
        ...
        if cycle is not None:
            ...
            if not len(cycle) is not 2:
                raise cycle.dtype == int or ValueError('Cycle must be a tuple of two integers.')
            else:
                self._cycle = cycle
        elif dcomplex is not None:
    ...
    def _phampcheck(pha, amp):
        ...
        assert pha.ndim == 3, '`pha` should have a shape of (n_pha, n_epochs, n_times)'
        assert amp.ndim == 3, '`amp` should have a shape of (n_pha, n_epochs, n_times)'
        assert pha.shape[1:] == amp.shape[1:], '`pha` and `amp` must have the same number of trials, channels and time points'
        assert np.ptp(pha) <= 2 * np.pi, 'Your phase is probably in degrees and should be converted in radians using either np.degrees or np.deg2rad.'
        pha = (pha + np.pi) % (2.0 * np.pi) - np.pi
        return (pha, amp)
...
class Pac(_PacObj, _PacPlt):
    ...
    def fit(self, pha, amp, n_perm=200, p=0.05, n_jobs=-1, verbose=None):
        ...
        if self._idpac[1] == 0:
            ...
            self._idpac = n_perm > 0 or (
             self._idpac[0], 0, 0)
            compute_surro = False
        else:
            compute_surro = True
        if self._idpac[0] == 6:
            logger.debug('    copnorm the phase and the amplitude')
        ...
        self._pac = pac.copy()
        if compute_surro:
            logger.info(f"    compute surrogates ({self.str_surro}, {n_perm} permutations)")
            surro = compute_surrogates(pha, amp, self.idpac[1], fcn, n_perm, n_jobs)
            self._surrogates = surro
            self.infer_pvalues(p)
        if self._idpac[2] != 0:
            logger.info(f"    normalize true PAC estimation by surrogates ({self.str_norm})")
            normalize(self.idpac[2], pac, surro)
        return pac

    def filterfit(self, sf, x_pha, x_amp=None, n_perm=200, p=0.05, n_jobs=-1, edges=None, verbose=None):
        ...
        if x_amp is None:
            x_amp = x_pha
        assert x_pha.shape == x_amp.shape, 'Inputs `x_pha` and `x_amp` must have the same shape.'
        logger.info(f"    Extract phases (n_pha={len(self.xvec)}) and amplitudes (n_amps={len(self.yvec)})")
        kw = dict(keepfilt=False, edges=edges, n_jobs=1)
        pha = (self.filter)(sf, x_pha, 'phase', **kw)
        amp = (self.filter)(sf, x_amp, 'amplitude', **kw)
        if self._idpac[0] == 5:
            amp = np.angle(hilbertm(amp))
        return self.fit(pha, amp, p=p, n_perm=n_perm, n_jobs=n_jobs, verbose=verbose)
    ...
    def infer_pvalues(self, p=0.05):
        ...
        pvalues : array_like
            Array of p-values of shape (n_amp, n_pha)
        """
        assert hasattr(self, 'pac'), 'You should compute PAC first. Use the `fit` method'
        ...
        th = np.percentile(max_dist, (100.0 * (1 - p)), axis=0, interpolation='nearest')
    ...
    def _idcheck(self, idpac):
        ...
        if not all([isinstance(k, int) for k in idpac]):
            if len(idpac) != 3:
        ...
        if idpac[2] != 0:
            if idpac[1] == 0:
                logger.warning('If you want to normalize the estimated PAC, you should select a surrogate method (second digit of `idpac`). Normalization ignored.')
                idpac[2] = 0
        self._idpac = idpac
...
class EventRelatedPac(_PacObj, _PacVisual):
 ...
 2, 4], f_amp=[60, 200], dcomplex='hilbert', cycle=(3, 6), width=7, verbose=None):
    ...
    def fit(self, pha, amp, method='circular', smooth=None, n_jobs=-1, n_perm=None, p=0.05, verbose=None):
            ...
            if method == 'gc':
                ...
                if isinstance(n_perm, int):
                    if n_perm > 0:
                        ...
                        self._surrogates = _ergcpac_perm(sco, amp, smooth=smooth, n_jobs=n_jobs,
                          n_perm=n_perm)
                        self.infer_pvalues(p=p)
            return self.erpac

    def filterfit(self, sf, x_pha, x_amp=None, method='circular', smooth=None, n_perm=None, p=0.05, edges=None, n_jobs=-1, verbose=None):
        ...
        return self.fit(pha, amp, method=method, smooth=smooth, n_jobs=n_jobs, n_perm=n_perm,
          p=p,
          verbose=verbose)
    ...
    def infer_pvalues(self, p=0.05):
        ...
        """
        assert hasattr(self, 'erpac'), 'You should compute ERPAC first. Use the `fit` method'
        assert hasattr(self, 'surrogates'), 'No surrogates computed'
        assert all([isinstance(k, np.ndarray) for k in (
         self.erpac, self.surrogates)])
        n_perm = self.surrogates.shape[0]
        logger.info(f"    Infer p-values at p={p}")
        th = np.percentile((self.surrogates.reshape(n_perm, -1).max(1)), (100.0 * (1 - p)),
...
class PreferredPhase(_PacObj, _PolarPlt):
 ...
 2, 4], f_amp=[60, 200], dcomplex='hilbert', cycle=(3, 6), width=7, verbose=None):
    ...
    def filterfit(self, sf, x_pha, x_amp=None, edges=None, n_bins=12, verbose=None):
