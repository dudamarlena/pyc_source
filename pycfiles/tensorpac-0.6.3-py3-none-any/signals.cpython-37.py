...
# [GCC 8.4.0]
# Embedded file name: /run/media/etienne/DATA/Toolbox/tensorpac/tensorpac/signals.py
...
def pac_signals_wavelet(f_pha=10.0, f_amp=100.0, sf=1024.0, n_times=4000.0, n_epochs=10, noise=0.1, pp=0.0, rnd_state=0):
    ...
    dpha = np.exp(complex(-0.0, -1.0) * pp)
    modulation = 1.0 / (1.0 + np.exp(-6.0 * np.real(driver * dpha)))
    xh = xh * modulation
    xl = np.real(driver)
...
def pac_signals_tort(f_pha=10.0, f_amp=100.0, sf=1024, n_times=4000, n_epochs=10, chi=0.0, noise=1.0, dpha=0.0, damp=0.0, rnd_state=0):
    ...
    chi = 0 if (0 <= chi <= 1) else chi
    noise = 0 if (0 <= noise <= 3) else noise
    dpha = 0 if (0 <= dpha <= 100) else dpha
    damp = 0 if (0 <= damp <= 100) else damp
    ...
    if f_pha.ndim == 0:
        apha = [
         f_pha * (1.0 - dpha / 100.0), f_pha * (1.0 + dpha / 100.0)]
    ...
    if f_amp.ndim == 0:
        a_amp = [
         f_amp * (1.0 - damp / 100.0), f_amp * (1.0 + damp / 100.0)]
        ...
        ah = 0.5 * ((1.0 - chi) * xl + 1.0 + chi)
        al = 1.0
        data = ah * xh + al * xl
        data += noise * (rng.rand)(*data.shape)
        return (
         data, time)