...
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/cmislib/browser/binding.py
# Compiled at: 2017-08-27 12:50:57
...
from cmislib.exceptions import CmisException, InvalidArgumentException, NotSupportedException, ObjectNotFoundException
from cmislib.net import RESTService as Rest
from cmislib.util import parsePropValueByType, parseDateTimeValue, safe_quote, safe_urlencode
from cmislib import messages
import json, logging, StringIO, time
...
class RepositoryService(RepositoryServiceIfc):
    ...
    def getRepositories(self, client):
        ...
        for repo in result.itervalues():
            repositories.append({'repositoryId': repo['repositoryId'], 'repositoryName': repo['repositoryName']})
...
class BrowserCmisObject(object):
    ...
    def reload(self, **kwargs):
        ...
        self.data = self._cmisClient.binding.get(byObjectIdUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **self._extArgs)
        self._initData()
    ...
    def getObjectParents(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(byObjectIdUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def updateProperties(self, properties):
        ...
        result = self._cmisClient.binding.post(updateUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
        self.data = result
        self._initData()
        return self
    ...
    def move(self, sourceFolder, targetFolder):
        ...
        props = {'objectId': self.id, 'cmisaction': 'move', 
           'sourceFolderId': sourceFolder.id, 
           'targetFolderId': targetFolder.id}
        self._cmisClient.binding.post(moveUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
    ...
    def delete(self, **kwargs):
        ...
        props = {'objectId': self.id, 'cmisaction': 'delete'}
        self._cmisClient.binding.post(delUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def createRelationship(self, targetObj, relTypeId):
        ...
        props = {'cmisaction': 'createRelationship'}
        setProps({'cmis:sourceId': self.getObjectId(), 
           'cmis:targetId': targetObj.getObjectId(), 
           'cmis:objectTypeId': relTypeId}, props)
        result = self._cmisClient.binding.post(url.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
    ...
    def applyACL(self, acl):
        ...
        if self._repository.getCapabilities()['ACL'] == 'manage':
            ...
            result = self._cmisClient.binding.post(aclUrl.encode('utf-8'), body, contentType, self._cmisClient.username, self._cmisClient.password)
...
class BrowserRepository(object):
    ...
    def getRepositoryInfo(self):
        ...
        if not self._repositoryInfo:
            if self.data is None:
                self.reload()
            repoInfo = {'repositoryId': self.data['repositoryId'], 'repositoryName': self.data['repositoryName'], 'repositoryDescription': self.data['repositoryDescription'], 'vendorName': self.data['vendorName'], 
               'productName': self.data['productName'], 'productVersion': self.data['productVersion'], 
               'rootFolderId': self.data['rootFolderId'], 'latestChangeLogToken': self.data['latestChangeLogToken'], 
    ...
    def getObjectByPath(self, path, **kwargs):
        ...
        result = self._cmisClient.binding.get(byPathUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def getTypeChildren(self, typeId=None, **kwargs):
        """
        ...
        result = self._cmisClient.binding.get(typesUrl, self._cmisClient.username, self._cmisClient.password, **kwargs)
        types = []
        for res in result['types']:
            objectType = BrowserObjectType(self._cmisClient, self, data=res)
    ...
    def getTypeDefinition(self, typeId):
        ...
        """
        typesUrl = self.getRepositoryUrl() + '?cmisselector=typeDefinition' + '&typeId=' + typeId
        result = self._cmisClient.binding.get(typesUrl, self._cmisClient.username, self._cmisClient.password)
        return BrowserObjectType(self._cmisClient, self, data=result)
    ...
    def getCheckedOutDocs(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(typesUrl, self._cmisClient.username, self._cmisClient.password, **kwargs)
        return BrowserResultSet(self._cmisClient, self, data=result)
    ...
    def query(self, statement, **kwargs):
        ...
        result = self._cmisClient.binding.post(queryUrl.encode('utf-8'), None, CMIS_FORM_TYPE, self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def getContentChanges(self, **kwargs):
        ...
        result = self._cmisClient.binding.get(changesUrl, self._cmisClient.username, self._cmisClient.password, **kwargs)
        return BrowserResultSet(self._cmisClient, self, data=result, serializer=ChangeEntrySerializer())
    ...
    def createDocument(self, name, properties={}, parentFolder=None, contentFile=None, contentType=None, contentEncoding=None):
        ...
        props = {'objectId': parentFolder.id, 'cmisaction': 'createDocument', 
        ...
        result = self._cmisClient.binding.post(createDocUrl.encode('utf-8'), body, contentType, self._cmisClient.username, self._cmisClient.password)
    ...
    def createFolder(self, parentFolder, name, properties={}, **kwargs):
        """
...
class BrowserDocument(BrowserCmisObject):
    ...
    def checkout(self):
        ...
        props = {'objectId': self.id, 'cmisaction': 'checkOut'}
        result = self._cmisClient.binding.post(coUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
        return getSpecializedObject(BrowserCmisObject(self._cmisClient, self._repository, data=result))
    ...
    def cancelCheckout(self):
        ...
        """
        coUrl = self._repository.getRootFolderUrl()
        props = {'objectId': self.id, 'cmisaction': 'cancelCheckOut'}
        self._cmisClient.binding.post(coUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
    ...
    def checkin(self, checkinComment=None, contentFile=None, contentType=None, properties=None, **kwargs):
        ...
        if not kwargs.has_key('major'):
            ...
            kwargs['major'] = 'false'
        props = {'checkinComment': checkinComment or ''}
        props.update(kwargs)
        propCount = 0
        ...
        contentType, body = encode_multipart_formdata(props, contentFile, contentType)
        result = self._cmisClient.binding.post(ciUrl.encode('utf-8'), body, contentType, self._cmisClient.username, self._cmisClient.password)
        return getSpecializedObject(BrowserCmisObject(self._cmisClient, self._repository, data=result))
    ...
    def getContentStream(self):
        ...
        if not self.getAllowableActions()['canGetContentStream']:
            ...
            result, content = Rest().get(contentUrl.encode('utf-8'), self._cmisClient.username, self._cmisClient.password, **self._cmisClient.extArgs)
    ...
    def deleteContentStream(self):
        ...
        delUrl = self._repository.getRootFolderUrl()
        props = {'objectId': self.id, 'cmisaction': 'deleteContent'}
        if self.properties.has_key('cmis:changeToken'):
            props['changeToken'] = self.properties['cmis:changeToken']
        self._cmisClient.binding.post(delUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
...
class BrowserFolder(BrowserCmisObject):
    ...
    def createFolder(self, name, properties={}, **kwargs):
        ...
        props = {'objectId': self.id, 'cmisaction': 'createFolder', 
           'propertyId[0]': 'cmis:name', 
           'propertyValue[0]': name}
        props['propertyId[1]'] = 'cmis:objectTypeId'
        if properties.has_key('cmis:objectTypeId'):
        ...
        setProps(properties, props, initialIndex=2)
        result = self._cmisClient.binding.post(createFolderUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def deleteTree(self, **kwargs):
        ...
        """
        delUrl = self._repository.getRootFolderUrl()
        props = {'objectId': self.id, 'cmisaction': 'deleteTree'}
        self._cmisClient.binding.post(delUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password, **kwargs)
    ...
    def addObject(self, cmisObject, **kwargs):
        ...
        props = {'folderId': self.id, 'cmisaction': 'addObjectToFolder', 
           'objectId': cmisObject.id}
        result = self._cmisClient.binding.post(addUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password, **kwargs)
        return getSpecializedObject(BrowserCmisObject(self._cmisClient, self._repository, data=result))
    ...
    def removeObject(self, cmisObject):
        ...
        props = {'folderId': self.id, 'cmisaction': 'removeObjectFromFolder', 
           'objectId': cmisObject.id}
        result = self._cmisClient.binding.post(remUrl.encode('utf-8'), safe_urlencode(props), 'application/x-www-form-urlencoded', self._cmisClient.username, self._cmisClient.password)
        return getSpecializedObject(BrowserCmisObject(self._cmisClient, self._repository, data=result))
...
class BrowserACL(ACL):
    ...
    def getRemovedAces(self):
        ...
        for principalId, original in originalEntries.iteritems():
            ...
            if removedPerms:
                removedAces.append(BrowserACE(principalId=principalId, permissions=list(removedPerms), direct=original.direct))
    ...
    def getAddedAces(self):
        ...
        for principalId, current in entries.iteritems():
            ...
            if addedPerms:
                addedAces.append(BrowserACE(principalId=principalId, permissions=list(addedPerms), direct=current.direct))
...
class ResultsSerializer(object):
    ...
    def fromJSON(self, client, repo, jsonObj):
        ...
        for obj in jsonObj['results']:
            cmisObject = getSpecializedObject(BrowserCmisObject(client, repo, data=obj))
...
class ChildrenSerializer(object):
    ...
    def fromJSON(self, client, repo, jsonObj):
        ...
        for obj in jsonObj['objects']:
            dataObj = obj['object']
            cmisObject = getSpecializedObject(BrowserCmisObject(client, repo, data=dataObj))
            entries.append(cmisObject)
    ...
    def getEntries(self, client, repo, jsonObj):
        ...
        for obj in jsonObj:
            if self.treeType == 'object':
                ...
                cmisThing = getSpecializedObject(BrowserCmisObject(client, repo, data=dataObj))
            else:
                if self.treeType == 'type':
                    dataObj = obj['type']
...
class ChangeEntrySerializer(object):
    ...
    def fromJSON(self, client, repo, jsonObj):
        ...
        for obj in jsonObj['objects']:
            ...
            cmisObject = BrowserChangeEntry(client, repo, data=obj)
