...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/lib/context.py
# Compiled at: 2016-11-22 15:21:45
import hashlib
from contextlib import contextmanager
import json, os, urllib, re, socket, itertools, logging
...
class Context(object):
    ...
    """
    availability_zone_re = re.compile('^([a-z]{2}-[a-z]+-[1-9][0-9]*)([a-z])$')
    name_prefix_re = re.compile('^(/([0-9a-z][0-9a-z._-]*))*')
    ...
    def __init__(self, availability_zone, namespace):
        ...
        if not m:
            raise ValueError("Can't extract region from availability zone '%s'" % availability_zone)
    ...
    def upload_ssh_pubkey(self, ssh_pubkey, fingerprint):
        ...
        if bucket is None:
            bucket = self.s3.create_bucket(self.s3_bucket_name, location=self.s3_location)
        ...
        if ec2_keypair is not None:
            ...
            if ec2_keypair.fingerprint != fingerprint:
                if force:
                    ...
                    raise UserError('Key pair %s already exists in EC2, but its fingerprint %s is different from the fingerprint %s of the key to be imported. Use the force option to overwrite the existing key pair.' % (
    ...
    def download_ssh_pubkey(self, ec2_keypair):
        try:
            ...
            if e.status == 404:
                raise UserError("There is no matching SSH pub key stored in S3 for EC2 key pair %s. Has it been registered, e.g using the cgcloud's register-key command?" % ec2_keypair.name)
            else:
                raise

        fingerprint_len = len(ec2_keypair.fingerprint.split(':'))
        if fingerprint_len == 20:
            pass
        elif fingerprint_len == 16:
            fingerprint = ec2_keypair_fingerprint(ssh_pubkey)
            if ec2_keypair.fingerprint != fingerprint:
                raise UserError("Fingerprint mismatch for key %s! Expected %s but got %s. The EC2 keypair doesn't match the public key stored in S3." % (
                 ec2_keypair.name, ec2_keypair.fingerprint, fingerprint))
        return ssh_pubkey
    ...
    def resolve_me(self, s, drop_hostname=True):
        ...
        if placeholder in s:
            ...
            if not me:
                raise UserError("Can't determine current IAM user name. Be sure to put valid AWS credentials in ~/.boto or ~/.aws/credentials. For details, refer to %s. On an EC2 instance that is authorized via IAM roles, you can set the CGCLOUD_ME environment variable (uncommon)." % 'http://boto.readthedocs.org/en/latest/boto_config_tut.html')
            if drop_hostname:
    ...
    def setup_iam_ec2_role(self, role_name, policies):
        ...
        try:
            self.iam.create_role(aws_role_name, assume_role_policy_document=json.dumps({'Version': '2012-10-17', 
               'Statement': [
                           {'Effect': 'Allow', 
        ...
        self.__setup_entity_policies(aws_role_name, policies, list_policies=self.iam.list_role_policies, delete_policy=self.iam.delete_role_policy, get_policy=self.iam.get_role_policy, put_policy=self.iam.put_role_policy)
    ...
    def setup_iam_user_policies(self, user_name, policies):
        ...
        self.__setup_entity_policies(user_name, policies, list_policies=self.iam.get_all_user_policies, delete_policy=self.iam.delete_user_policy, get_policy=self.iam.get_user_policy, put_policy=self.iam.put_user_policy)

    def __setup_entity_policies(self, entity_name, policies, list_policies, delete_policy, get_policy, put_policy):
        ...
        for policy_name, policy in policies.iteritems():
            current_policy = None
            try:
                current_policy = json.loads(urllib.unquote(get_policy(entity_name, policy_name).policy_document))
            except BotoServerError as e:
    ...
    def agent_topic_arn(self):
        ...
        """
        return self.sns.create_topic(self._agent_topic_name)['CreateTopicResponse']['CreateTopicResult']['TopicArn']
    ...
    def local_security_groups(self):
        return [ sg for sg in self.ec2.get_all_security_groups() if self.try_contains_aws_name(sg.name)
               ]
    ...
    def unused_snapshots(self):
        ...
        :rtype: set[str]
        """
        all_snapshots = self.ec2.get_all_snapshots(owner='self', filters=dict(description='Created by CreateImage*'))
        all_snapshots = set(snapshot.id for snapshot in all_snapshots)
        used_snapshots = set(bdt.snapshot_id for image in self.ec2.get_all_images(owners=['self']) for bdt in image.block_device_mapping.itervalues() if bdt.snapshot_id is not None)
