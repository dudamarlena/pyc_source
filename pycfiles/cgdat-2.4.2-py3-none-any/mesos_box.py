...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/mesos/mesos_box.py
...
def mesos_service(name, *flags):
    ...
    return Service(init_name='mesosbox-' + name, description=fmt('Mesos {name} service'), command=fmt((' ').join(command)))


mesos_services = dict(master=[
 mesos_service('master', '--registry=in_memory', '--ip_discovery_command="hostname -i"', '--credentials=/etc/mesos/credentials')], slave=[
 mesos_service('slave', '--master=mesos-master:5050', '--no-switch_user', '--work_dir=' + work_dir, '--executor_shutdown_grace_period=60secs', '--resources=disk:$(python -c %s)' % quote(heredoc('\n                               import os\n                               df = os.statvfs( "{work_dir}" )\n                               free = df.f_frsize * df.f_bavail >> 20\n                               print max( 0, free - min( free / 2, 5120 ) )').replace('\n', ';')), '$(cat /var/lib/mesos/slave_args)')])
...
class MesosBoxSupport(GenericUbuntuTrustyBox, Python27UpdateUbuntuBox, CoreMesosBox):
    ...
    def _get_iam_ec2_role(self):
        ...
        policies.update(dict(ec2_read_only=ec2_read_only_policy, ec2_mesos_box=dict(Version='2012-10-17', Statement=[
