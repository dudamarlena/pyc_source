...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/package_manager_box.py
...
class PackageManagerBox(Box):
    ...
    def __substitute_package(cls, substitutions, package, history=None):
        ...
        if substitutes is None:
            ...
            if isinstance(substitutes, basestring):
    ...
    def __substitute_packages(cls, substitutions, substitutes, history=None):
        return set(chain.from_iterable(cls.__substitute_package(substitutions, substitute, history) for substitute in substitutes))