...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/lib/ec2.py
...
class EC2VolumeHelper(object):
    ...
    def attach(self, instance_id, device):
        if self.volume.attach_data.instance_id == instance_id:
            ...
            self.ec2.attach_volume(volume_id=self.volume.id, instance_id=instance_id, device=device)
...
def wait_instances_running(ec2, instances):
    ...
    while True:
        ...
        log.info('%i instance(s) pending, %i running, %i other.', *map(len, (pending_ids, running_ids, other_ids)))
        if not pending_ids:
            break
...
def wait_spot_requests_active(ec2, requests, timeout=None, tentative=False):
    ...
    try:
        while True:
            ...
            for r in requests:
                if r.state == 'open':
                    ...
                    if r.status.code == 'pending-evaluation':
                        ...
                        log.info('Request %s entered status %s indicating that it will not be fulfilled anytime soon.', r.id, r.status.code)
                elif r.state == 'active':
            ...
            if batch:
                yield batch
            log.info('%i spot requests(s) are open (%i of which are pending evaluation and %i are pending fulfillment), %i are active and %i are in another state.', *map(len, (open_ids, eval_ids, fulfill_ids, active_ids, other_ids)))
...
def create_spot_instances(ec2, price, image_id, spec, num_instances=1, timeout=None, tentative=False, tags=None):
    ...
    for batch in wait_spot_requests_active(ec2, requests, timeout=timeout, tentative=tentative):
        instance_ids = []
...
def create_ondemand_instances(ec2, image_id, spec, num_instances=1):
    ...
    for attempt in retry_ec2(retry_for=a_long_time, retry_while=inconsistencies_detected):
        with attempt:
            return ec2.run_instances(image_id, min_count=num_instances, max_count=num_instances, **spec).instances
