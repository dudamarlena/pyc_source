...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/ubuntu_box.py
# Compiled at: 2016-11-22 15:21:45
from abc import abstractmethod
from ast import literal_eval
from collections import namedtuple
import contextlib, csv, logging, urllib2
...
class UbuntuBox(AgentBox, CloudInitBox, RcLocalBox):
    ...
    def _base_image(self, virtualization_type):
        ...
        template = self.TemplateDict(release=release, purpose='server', release_type='release', storage_type='ebs', arch='amd64', region=self.ctx.region, hypervisor=virtualization_type)
        url = '%s/query/%s/server/released.current.txt' % (BASE_URL, release)
        matches = []
        with contextlib.closing(urllib2.urlopen(url)) as (stream):
            images = csv.DictReader(stream, fieldnames=[
             'release', 'purpose', 'release_type', 'release_date',
             'storage_type', 'arch', 'region', 'ami_id', 'aki_id',
             'dont_know', 'hypervisor'], delimiter='\t')
            for image in images:
                if template.matches(image):
                    matches.append(image)

        if len(matches) < 1:
            raise self.NoSuchImageException("Can't find Ubuntu AMI for release %s and virtualization type %s" % (
    ...
    def _sync_package_repos(self):
        put(remote_path='/etc/apt/apt.conf.d/99timeout', use_sudo=True, local_path=StringIO(heredoc('\n                Acquire::http::Timeout "10";\n                Acquire::ftp::Timeout "10"; ')))
        for i in range(5):
    ...
    def _get_package_installation_command(self, package):
        return [
         'apt-get', 'install', '-y', '--no-install-recommends', '--force-yes'] + list(self._substitute_package(package))
