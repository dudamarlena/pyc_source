...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/yum_box.py
...
class YumBox(PackageManagerBox, SysvInitdBox):
    ...
    def _get_package_installation_command(self, package):
        return [
         'yum', 'install', '-d', '1', '-y'] + list(self._substitute_package(package))
    ...
    def _yum_local(self, is_update, rpm_urls):
        ...
        sudo(('yum -d 1 -y local{command} {rpms} --nogpgcheck').format(command='update' if is_update else 'install', rpms=(' ').join("'%s'" % rpm for rpm in rpms)))
        for rpm in rpms:
