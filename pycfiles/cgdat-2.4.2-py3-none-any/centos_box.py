...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/core/centos_box.py
...
class CentosBox(YumBox, AgentBox, RcLocalBox):
    ...
    def _base_image(self, virtualization_type):
        ...
        images = self.ctx.ec2.get_all_images(owners=[
         '411009282317'], filters={'name': 'RightImage_CentOS_%s_x64*' % release, 
           'root-device-type': 'ebs', 
           'virtualization-type': virtualization_type})
        if not images:
            raise self.NoSuchImageException("Can't find any candidate AMIs for CentOS release %s and virtualization type %s" % (
             release, virtualization_type))
        max_version = None
        base_image = None
        for image in images:
            match = re.match('RightImage_CentOS_(\\d+(?:\\.\\d+)*)_x64_v(\\d+(?:\\.\\d+)*)(_HVM)?_EBS', image.name)
            if match:
        ...
        if not base_image:
            raise self.NoSuchImageException("Can't find AMI matching CentOS release %s and virtualization type %s" % (
             release, virtualization_type))
        return base_image
    ...
    def __create_admin(self):
        run("sed -i -r 's!^(/usr/bin/)?clear!# \\0!' /etc/skel/.bash_logout ~/.bash_logout")
