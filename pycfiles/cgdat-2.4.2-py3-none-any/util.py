...
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/lib/util.py
# Compiled at: 2016-11-22 15:21:45
import argparse, base64, hashlib, logging, multiprocessing, multiprocessing.pool, os, re, struct, subprocess, sys
...
def camel_to_snake(s, separator='_'):
    ...
    'toil-310-box'
    """
    s = re.sub('([a-z0-9])([A-Z])', '\\1%s\\2' % separator, s)
    s = re.sub('([a-z])([A-Z0-9])', '\\1%s\\2' % separator, s)
    return s.lower()
...
class Application(object):
    ...
    def __init__(self):
        ...
        self.subparsers = self.parser.add_subparsers(help='Application commands', dest='command_name')
        self.commands = {}
        return
...
class Command(object):
    ...
    def __init__(self, application, **kwargs):
        ...
        self.parser = application.subparsers.add_parser(self.name(), formatter_class=ArgParseHelpFormatter, **kwargs)
...
class ArgParseHelpFormatter(argparse.ArgumentDefaultsHelpFormatter):
    try:
        with open(os.devnull, 'a') as (devnull):
            rows, columns = map(int, subprocess.check_output(['stty', 'size'], stderr=devnull).split())
    except:
        rows, columns = (None, None)

    def __init__(self, *args, **kwargs):
        super(ArgParseHelpFormatter, self).__init__(width=min(100, self.columns), max_help_position=30, *args, **kwargs)
