...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/surfcity/edlin.py
# Compiled at: 2019-04-03 08:09:55
# Size of source mod 2**32: 4886 bytes
import re
    ...
    while 1:
        ...
        if len(cmd) == 0:
            ...
            if cmd in ('?', 'h'):
                ...
                if not n < 1:
                    ...
                    if cmd == 'e':
                        if modif:
                            return lines
                        return
                    rng = re.match('([0-9.]+)([^0-9,.])|([0-9.]+),([0-9.]+)([^0-9.])', cmd)
                    if rng:
                        if rng.group(2):
                            cmd = rng.group(2)
                            if rng.group(1) == '.':
                    ...
                    rng = (
                     int(rng.group(1)) - 1, int(rng.group(1)) - 1)
            else:
                cmd = rng.group(5)
                ...
                rng = (a, b)
            if not rng[0] < 0:
                if rng[1] < 0 or rng[0] > rng[1]:
                    print('invalid range')
                    continue
                if cmd == 'd':
                    if not rng or rng[0] >= len(lines) or rng[1] >= len(lines):
                        print('invalid range')
                        continue
                    else:
                        rng = (
