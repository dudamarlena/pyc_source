...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ssb/local/config.py
# Compiled at: 2019-04-03 13:47:16
# Size of source mod 2**32: 8098 bytes
import base64, json, nacl.signing, os, sys
...
def create_new_user_secret(path):
    ...
    s = {'curve':'ed25519', 
     'private':base64.b64encode(sik._signing_key).decode('ascii') + '.ed25519', 
     'public':base64.b64encode(sik.verify_key._key).decode('ascii') + '.ed25519'}
    s['id'] = '@' + s['public']
...
class SSB_SECRET:
    ...
    def unboxPrivateData(self, data):
        nonce = data[:24]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-new', type=str, metavar='USERNAME', help='create new user')
    parser.add_argument('-friends', nargs=2, metavar='NAME', help='make two users follow each other')
    ...
    if args.list:
        ...
        if args.friends:
            ...
            if s[0]['id'] not in fr[1]['value'][s[1]['id']]:
                fr[1]['value'][s[1]['id']][s[0]['id']] = True
            if s[1]['id'] not in fr[0]['value'][s[0]['id']]:
