...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ssb/rpc/feed/models.py
# Compiled at: 2018-09-02 03:18:40
...
OrderedMsg = namedtuple('OrderedMsg', ('previous', 'author', 'sequence', 'timestamp',
                                       'hash', 'content'))
...
class Feed(object):
    ...
    @property
    def id(self):
        return (b'@' + b64encode(bytes(self.public_key)) + b'.ed25519').decode('ascii')
...
class Message(object):
    ...
    def to_dict(self, add_signature=True):
        obj = to_ordered({'previous':self.previous.key if self.previous else None, 
         'author':self.feed.id, 
