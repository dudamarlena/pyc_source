...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/ssb/rpc/tests/test_packet_stream.py
...
MSG_BODY_1 = b'{"previous":"%KTGP6W8vF80McRAZHYDWuKOD0KlNyKSq6Gb42iuV7Iw=.sha256","author":"@1+Iwm79DKvVBqYKFkhT6fWRbAVvNNVH4F2BSxwhYmx8=.ed25519","sequence":116,"timestamp":1496696699331,"hash":"sha256","content":{"type":"post","channel":"crypto","text":"Does anybody know any good resources (e.g. books) to learn cryptography? I\'m not speaking of basic concepts (e.g. what\'s a private key) but the actual mathematics behind the whole thing.\\nI have a copy of the \\"Handbook of Applied Cryptography\\" on my bookshelf but I found it too long/hard to follow. Are there any better alternatives?","mentions":[]},"signature":"hqKePbbTXWxEi1njDnOWFsL0M0AoNoWyBFgNE6KXj//DThepaZSy9vRbygDHX5uNmCdyOrsQrwZsZhmUYKwtDQ==.sig.ed25519"}'
MSG_BODY_2 = b'{"previous":"%iQRhPyqmNLpGaO1Tpm1I22jqnUEwRwkCTDbwAGtM+lY=.sha256","author":"@1+Iwm79DKvVBqYKFkhT6fWRbAVvNNVH4F2BSxwhYmx8=.ed25519","sequence":103,"timestamp":1496674211806,"hash":"sha256","content":{"type":"post","channel":"git-ssb","text":"Is it only me or `git.scuttlebot.io` is timing out?\\n\\nE.g. try accessing %vZCTqraoqKBKNZeATErXEtnoEr+wnT3p8tT+vL+29I4=.sha256","mentions":[{"link":"%vZCTqraoqKBKNZeATErXEtnoEr+wnT3p8tT+vL+29I4=.sha256"}]},"signature":"+i4U0HUGDDEyNoNr2NIROPnT3WQj3RuTaIhY5koWW8f0vwr4tZsYmAkqqMwFWfP+eBIbc7DZ835er6r6h9CwAg==.sig.ed25519"}'
...
async def test_message_decoding(ps_client):
    ...
    ps_client.feed([
     ...
     b'{"name":["createHistoryStream"],"args":[{"id":"@omgyp7Pnrw+Qm0I6T6Fh5VvnKmodMXwnxTIesW2DgMg=.ed25519","seq":10,"live":true,"keys":false}],"type":"source"}'])
    messages = await _collect_messages(ps)
    assert len(messages) == 1
    assert messages[0].type == PSMessageType.JSON
    assert messages[0].body == {'name':[
      'createHistoryStream'], 
     'args':[
...
async def test_message_encoding(ps_client):
    ...
    assert header == b'\n\x00\x00\x00\xa6\x00\x00\x00\x01'
    assert json.loads(body.decode('utf-8')) == {'name':[
      'createHistoryStream'], 
     'args':[
      {'id':'@1+Iwm79DKvVBqYKFkhT6fWRbAVvNNVH4F2BSxwhYmx8=.ed25519', 
       'seq':1,  'live':False,  'keys':False}], 
     'type':'source'}
