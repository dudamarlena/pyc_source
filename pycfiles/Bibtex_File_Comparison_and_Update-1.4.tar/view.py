...
# [GCC 8.4.0]
# Embedded file name: /Users/Ziauddin/Documents/LiClipse Workspace/Bibtex-File-Comparison-and-Update/docs/source/view.py
# Compiled at: 2015-12-13 17:47:24
import Tkinter as tk
from Tkinter import *
import controller, os
from git import *
from git import Repo
import shutil, Tkinter, Tkconstants, tkFileDialog
...
class View(tk.Frame):
    ...
    def list_differences(self, list, list2):
        ...
        if len(list) > 0:
            ...
            Label(window, text='There is a difference in the below properties for the master and local file', font=('Times',
                                                                                                                    16), foreground='Blue').grid(columnspan=2, sticky=W, padx=2, pady=2)
        ...
        line = line + 2
        if len(list2) > 0:
            Label(window, text='Below property is not present in the local file', font=('Times',
                                                                                        16), foreground='Blue').grid(row=line, columnspan=2, sticky=W, padx=1, pady=1)
            Label(window, text='Select properties you want to add', font=('Times',
                                                                          16), foreground='Blue').grid(row=line + 1, columnspan=2, sticky=W, padx=1, pady=1)
            line = line + 2
        ...
        if len(list) == 0 and len(list2) == 0:
            Label(window, text='No differences between the two files', font=('Times',
                                                                             16), foreground='Blue').grid(row=line, columnspan=2, sticky=W, padx=1, pady=1)
            self.update = False
...
def create_local_repo(remote_git, dir, branch):
    ...
    if os.path.exists(dir):
        shutil.rmtree(dir)
    try:
        repo = Repo.clone_from(url=remote_git, to_path=dir, branch=branch)
        if repo.bare:
