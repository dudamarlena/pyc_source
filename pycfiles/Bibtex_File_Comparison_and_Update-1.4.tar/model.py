...
# [GCC 8.4.0]
# Embedded file name: /Users/Ziauddin/Documents/LiClipse Workspace/Bibtex-File-Comparison-and-Update/docs/source/model.py
# Compiled at: 2015-12-13 17:47:24
import bibtexparser, Tkinter as tk
from Tkinter import *
from ttk import *
...
def change_property_db(list, db_coll):
    ...
    for idx, val in enumerate(list):
        if list[idx][2][3].get():
            result = db_coll.update({'ID': list[idx][0]}, {'$set': {list[idx][1][0]: list[idx][2][1]}, '$currentDate': {'lastModified': True}})
...
def add_property_db(list, db_coll):
    ...
    """
    for idx, val in enumerate(list):
        result = db_coll.update({'ID': list[idx][0]}, {'$set': {list[idx][1][0]: list[idx][1][1]}})
