...
# Compiled at: 2018-01-26 02:18:09
# Size of source mod 2**32: 36890 bytes
...
class TestClient(unittest2.TestCase):
    ...
    def test_response_handling(self):
        ...
        for code in (200, 201, 204):
            hp.register_uri(hp.GET, re.compile('.*' + str(code) + '$'), lambda r, u, h: (
             code, h, error_str))
    ...
    def test_create_report(self):
        client = Client(api_key, api_secret)
        report = client.create_report(email='example@coinbase.com',
          type='transactions')
    ...
    def test_deposit(self):
        client = Client(api_key, api_secret)
        send_kwargs = {}
        required_kwargs = {'payment_method':'bar', 
         'amount':'1.0',  'currency':'USD'}
        while required_kwargs:
            with self.assertRaises(ValueError):
...
class TestOauthClient(unittest2.TestCase):
    ...
    def test_refresh(self):
        ...
        errors_in_server = []
        server_response_data = {'access_token':'newaccesstoken', 
    ...
    def test_response_handling(self):
        ...
        header_template = 'Bearer realm="Doorkeeper" error="{error}" error_description="{message}"'

        def resp401_revoked(request, uri, headers):
            error_data = {'error':'revoked_token', 
        ...
        def resp401_expired(request, uri, headers):
            error_data = {'error':'expired_token', 
