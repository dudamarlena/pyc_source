...
# [GCC 8.4.0]
# Embedded file name: /Users/shanedasilva/src/ghc/coinbase/coinbase-python/tests/test_model.py
# Compiled at: 2018-01-26 02:09:43
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
from __future__ import unicode_literals
import six, unittest2, warnings, httpretty as hp
...
mock_item_updated = {b'key1': b'val1-modified', 
   b'key2': b'val2-modified', 
   b'key3': b'newkey'}
mock_collection = [
 mock_item, mock_item]
...
class TestAPIObject(unittest2.TestCase):
    ...
    @mock_response(hp.GET, b'/resource/foo', mock_item_updated)
...
mock_account = {b'id': b'foo', 
   b'resource_path': b'/v2/accounts/foo'}
mock_account_updated = {b'id': b'foo', 
   b'resource_path': b'/v2/accounts/foo', 
   b'newkey': b'present'}

class TestAccount(unittest2.TestCase):

    @mock_response(hp.POST, b'/v2/accounts/foo/primary', mock_account_updated)
    ...
    @mock_response(hp.PUT, b'/v2/accounts/foo', mock_account_updated)
    def test_modify(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        data = account.modify(name=b'New Account Name')
        self.assertEqual(data, mock_account_updated)
        for key, value in six.iteritems(mock_account_updated):
            self.assertEqual(account[key], value)
    ...
    def test_get_address(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        address = account.get_address(b'bar')
        self.assertIsInstance(address, Address)
    ...
    def test_get_address_transactions(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        transactions = account.get_address_transactions(b'bar')
    ...
    def test_get_transaction(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        transaction = account.get_transaction(b'bar')
        self.assertIsInstance(transaction, Transaction)
    ...
    def test_get_report(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        report = account.get_report(b'testreportid')
        self.assertIsInstance(report, Report)
    ...
    def test_create_report(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        report = account.create_report(type=b'transactions', email=b'example@coinbase.com')
    ...
    def test_commit_sell(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        sell = account.commit_sell(b'bar')
        self.assertIsInstance(sell, Sell)
        self.assertEqual(sell, mock_item)
    ...
    def test_get_deposit(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        deposit = account.get_deposit(b'bar')
        self.assertIsInstance(deposit, Deposit)
    ...
    def test_commit_deposit(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        deposit = account.commit_deposit(b'bar')
        self.assertIsInstance(deposit, Deposit)
        self.assertEqual(deposit, mock_item)
    ...
    def test_get_withdrawal(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        withdrawal = account.get_withdrawal(b'bar')
        self.assertIsInstance(withdrawal, Withdrawal)
    ...
    def test_commit_withdrawal(self):
        client = Client(api_key, api_secret)
        account = new_api_object(client, mock_account, Account)
        withdrawal = account.commit_withdrawal(b'bar')
        self.assertIsInstance(withdrawal, Withdrawal)
        self.assertEqual(withdrawal, mock_item)


mock_checkout = {b'id': b'foo', 
   b'resource_path': b'/v2/checkouts/foo'}
...
mock_order = {b'id': b'foo', 
   b'resource_path': b'/v2/orders/foo'}
...
mock_transaction = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/transactions/bar'}
...
mock_buy = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/buys/bar'}
mock_buy_updated = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/buys/bar', 
   b'updated': True}

class TestBuy(unittest2.TestCase):

    @mock_response(hp.POST, b'/v2/accounts/foo/buys/bar/commit', mock_buy_updated)
...
mock_sell = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/sells/bar'}
mock_sell_updated = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/sells/bar', 
   b'updated': True}

class TestSell(unittest2.TestCase):

    @mock_response(hp.POST, b'/v2/accounts/foo/sells/bar/commit', mock_sell_updated)
    def test_commit(self):
        client = Client(api_key, api_secret)
...
mock_deposit = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/deposits/bar'}
mock_deposit_updated = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/deposits/bar', 
   b'updated': True}

class TestDeposit(unittest2.TestCase):

    @mock_response(hp.POST, b'/v2/accounts/foo/deposits/bar/commit', mock_deposit_updated)
    def test_commit(self):
        client = Client(api_key, api_secret)
...
mock_withdrawal = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/withdrawals/bar'}
mock_withdrawal_updated = {b'id': b'bar', 
   b'resource_path': b'/v2/accounts/foo/withdrawals/bar', 
   b'updated': True}

class TestWithdrawal(unittest2.TestCase):

    @mock_response(hp.POST, b'/v2/accounts/foo/withdrawals/bar/commit', mock_withdrawal_updated)
    def test_commit(self):
        client = Client(api_key, api_secret)
...
class TestCurrentUser(unittest2.TestCase):
    ...
    @mock_response(hp.PUT, b'/v2/user', mock_item_updated)
    def test_modify(self):
        client = Client(api_key, api_secret)
        user = new_api_object(client, mock_item, CurrentUser)
        user2 = user.modify(name=b'New Name')
