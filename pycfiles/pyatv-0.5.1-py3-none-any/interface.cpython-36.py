...
# [GCC 8.4.0]
# Embedded file name: /home/postlund/pyatv_dev/pyatv/pyatv/interface.py
# Compiled at: 2019-10-07 03:42:21
# Size of source mod 2**32: 12530 bytes
"""API exposed by the library."""
import re, inspect, hashlib
...
def retrieve_commands(obj):
    ...
    for func in obj.__dict__:
        if not inspect.isfunction(obj.__dict__[func]):
            if not isinstance(obj.__dict__[func], property):
                continue
...
class PairingHandler:
    __doc__ = 'Base class for API used to pair with an Apple TV.'
...
class RemoteControl:
    __doc__ = 'Base class for API used to control an Apple TV.'
    __metaclass__ = ABCMeta
...
class Playing:
    __doc__ = 'Base class for retrieving what is currently playing.'
    __metaclass__ = ABCMeta

    def __str__(self):
        """Convert this playing object to a readable string."""
        output = []
        output.append('Media type: {0}'.format(convert.media_type_str(self.media_type)))
        output.append('Play state: {0}'.format(convert.playstate_str(self.play_state)))
        ...
        if position is not None:
            if total_time is not None:
                if total_time != 0:
                    output.append('  Position: {0}/{1}s ({2:.1%})'.format(position, total_time, float(position) / float(total_time)))
    ...
    def hash(self):
        ...
        """
        base = '{0}{1}{2}{3}'.format(self.title, self.artist, self.album, self.total_time)
...
class Metadata:
    __doc__ = 'Base class for retrieving metadata from an Apple TV.'
    __metaclass__ = ABCMeta
...
class PushUpdater:
    __doc__ = 'Base class for push/async updates from an Apple TV.'
    __metaclass__ = ABCMeta

    def __init__(self):
        """Initialize a new PushUpdater."""
        self._PushUpdater__listener = None

    @property
    def listener(self):
        """Object (PushUpdaterListener) that receives updates."""
        return self._PushUpdater__listener
    ...
    def listener(self, listener):
        ...
        """
        self._PushUpdater__listener = listener
...
class AirPlay:
    __doc__ = 'Base class for AirPlay functionality.'
    __metaclass__ = ABCMeta
...
class AppleTV:
    __doc__ = 'Base class representing an Apple TV.'
    __metaclass__ = ABCMeta
