...
# [GCC 8.4.0]
# Embedded file name: /home/postlund/pyatv_dev/pyatv/pyatv/__main__.py
# Compiled at: 2019-10-07 03:42:42
# Size of source mod 2**32: 17855 bytes
"""CLI application for discovering and controlling Apple TVs."""
import sys, inspect, logging, binascii, asyncio, argparse
...
def _print_commands(title, api):
    ...
    commands = ' - ' + '\n - '.join(map(lambda x: x[0] + ' - ' + x[1], sorted(cmd_list.items())))
...
class GlobalCommands:
    __doc__ = 'Commands not bound to a specific device.'
                    ...
                    if not key.startswith('_'):
                        if key != self.args.command[1]:
                            ...
                            if inspect.isfunction(value):
                                ...
                                signature = ' (property)'
                            print('COMMAND:\n>> {0}{1}\n\nHELP:\n{2}'.format(key, signature, inspect.getdoc(value)))
...
class DeviceCommands:
    ...
    async def pair(self):
        ...
        if protocol == const.PROTOCOL_DMAP:
            await self.atv.pairing.start(zeroconf=(Zeroconf()), name=(self.args.remote_name),
            ...
            if protocol == const.PROTOCOL_MRP:
                ...
                if self.atv.pairing.device_provides_pin:
                    ...
                    if self.args.pin_code is None:
                        print('Use any pin to pair with "{}" (press ENTER to stop)'.format(self.args.remote_name))
                    else:
            ...
            if self.atv.pairing.has_paired:
                ...
                print('You may now use these credentials: {0}'.format(self.atv.pairing.credentials))
            else:
...
class PushListener:
    __doc__ = 'Internal listener for push updates.'
...
def _in_range(lower, upper, allow_none=False):
    ...
    def _checker(value):
        ...
        if int(value) >= lower:
            if int(value) < upper:
                return int(value)
        raise ArgumentTypeError('Must be greater >= {} and < {}'.format(lower, upper))

    return _checker


class TransformProtocol(argparse.Action):
    __doc__ = 'Transform protocol in string format to internal representation.'
...
async def cli_handler(loop):
    ...
    parser.add_argument('--id', help='device identifier', dest='id',
      default=None)
    parser.add_argument('--name', help='apple tv name', dest='name',
      default='Apple TV')
    parser.add_argument('--address', help='device ip address or hostname', dest='address',
      default=None)
    parser.add_argument('--protocol', action=TransformProtocol, help='protocol to use (values: dmap, mrp)',
      dest='protocol',
      default=None)
    parser.add_argument('--port', help='port when connecting', dest='port',
      type=(_in_range(0, 65535)),
      default=0)
    parser.add_argument('-t', '--scan-timeout', help='timeout when scanning', dest='scan_timeout',
      type=(_in_range(1, 100)),
      metavar='TIMEOUT',
      default=3)
    parser.add_argument('--version', action='version', help='version of atvremote and pyatv',
      version=('%(prog)s {0}'.format(const.__version__)))
    pairing = parser.add_argument_group('pairing')
    pairing.add_argument('--remote-name', help='remote pairing name', dest='remote_name',
      default='pyatv')
    pairing.add_argument('-p', '--pin', help='pairing pin code', dest='pin_code',
      metavar='PIN',
      default=1234,
      type=_in_range(0, 9999, allow_none=True))
    pairing.add_argument('--pairing-guid', help='pairing guid (16 chars hex)',
      dest='pairing_guid',
      default=None)
    parser.add_argument('-a', '--autodiscover', action='store_true', help='automatically find a device',
      dest='autodiscover',
      default=False)
    parser.add_argument('--device_credentials', help='credentials to device', dest='device_credentials',
      default=None)
    airplay = parser.add_argument_group('airplay')
    airplay.add_argument('--airplay_credentials', help='credentials for airplay',
      dest='airplay_credentials',
      default=None)
    debug = parser.add_argument_group('debugging')
    debug.add_argument('-v', '--verbose', help='increase output verbosity', action='store_true',
      dest='verbose')
    debug.add_argument('--debug', help='print debug information', action='store_true',
      dest='debug')
    ...
    if args.command[0] in cmds:
        ...
        return await _exec_command(glob_cmds,
          (args.command[0]), print_result=False)
...
async def _autodiscover_device(args, loop):
    atvs = await pyatv.scan_for_apple_tvs(loop,
      timeout=(args.scan_timeout), abort_on_found=True, device_id=(args.id),
    ...
    if not atvs:
        ...
        return
    if len(atvs) > 1:
        logging.error('Found more than one Apple TV; specify one using --address and --device-credentials')
...
async def _handle_commands(args, loop):
    ...
    if args.protocol == const.PROTOCOL_DMAP:
        details.add_service(DmapService((args.device_credentials),
          port=(args.port)))
    else:
        if args.protocol == const.PROTOCOL_MRP:
            details.add_service(MrpService((args.port),
              device_credentials=(args.device_credentials)))
    atv = await pyatv.connect_to_apple_tv(details,
      loop, protocol=(args.protocol))
...
async def _handle_device_command(args, cmd, atv, loop):
    ...
    if cmd in device:
        return await _exec_command(DeviceCommands(atv, loop, args), cmd, False, *cmd_args)
    if cmd in ctrl:
    ...
    if cmd in playing:
        ...
        if cmd in airplay:
            return await _exec_command(atv.airplay, cmd, True, *cmd_args)
