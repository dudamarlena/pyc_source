...
# [GCC 8.4.0]
# Embedded file name: /home/postlund/pyatv_dev/pyatv/pyatv/dmap/daap.py
...
class DaapRequester:
    __doc__ = 'Helper class that makes it easy to perform DAAP requests.\n\n    It will automatically do login and other necesarry book-keeping.\n    '
    ...
    async def login(self):
        ...
        def _login_request():
            return self.http.get_data(self._mkurl('login?[AUTH]&hasFP=1', session=False,
              login_id=True),
    ...
    async def get(self, cmd, daap_data=True, timeout=None, **args):
        ...
        def _get_request():
            return self.http.get_data((self._mkurl)(cmd, *args),
              headers=_DMAP_HEADERS,
              timeout=timeout)
    ...
    async def post(self, cmd, data=None, timeout=None, **args):
        ...
        def _post_request():
            ...
            return self.http.post_data((self._mkurl)(cmd, *args),
              data=data,
              headers=headers,
              timeout=timeout)
    ...
    async def _do(self, action, retry=True, is_login=False, is_daap=True):
        ...
        if retry:
            return await self._do(action,
              False, is_login=is_login, is_daap=is_daap)
    ...
    def _mkurl(self, cmd, *args, session=True, login_id=False):
        ...
        parameters = []
        if login_id:
            if re.match('0x[0-9a-fA-F]{16}', self._login_id):
