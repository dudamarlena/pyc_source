...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/nosexunit/core.py
# Compiled at: 2009-09-22 16:55:25
import os, sys, time, codecs, logging, StringIO, traceback, nosexunit.const as nconst, nosexunit.tools as ntools
...
class StdRecorder:
    ...
    def write(self, value):
        ...
        if isinstance(value, unicode):
            ...
            if self.rec:
                self.record.write(value.decode(self.encoding, 'replace'))
...
class XTest(XTestElmt):
    ...
    def writeXmlOnStream(self, stream):
        ...
        if self.kind in [nconst.TEST_SUCCESS, nconst.TEST_FAIL, nconst.TEST_ERROR]:
            ...
            if self.kind == nconst.TEST_SUCCESS:
                stream.write('/>')
            else:
                stream.write('>')
                if self.kind == nconst.TEST_ERROR:
                    tag = 'error'
                else:
                    tag = 'failure'
