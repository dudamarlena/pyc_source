...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/socketio/namespace.py
# Compiled at: 2014-02-03 00:13:04
import gevent, re, logging, inspect
log = logging.getLogger(__name__)
allowed_event_name_regex = re.compile('^[A-Za-z][A-Za-z0-9_ ]*$')
...
class BaseNamespace(object):
    ...
    def process_event(self, packet):
        ...
        if not allowed_event_name_regex.match(name):
            self.error('unallowed_event_name', 'name must only contains alpha numerical characters')
            return
    ...
    def call_method_with_acl(self, method_name, packet, *args):
        ...
        if not self.is_method_allowed(method_name):
            self.error('method_access_denied', 'You do not have access to method "%s"' % method_name)
            return
    ...
    def call_method(self, method_name, packet, *args):
        ...
        * Otherwise, pass in the arguments as specified by the
        ...
        if method is None:
            self.error('no_such_method', 'The method "%s" was not found' % method_name)
            return
        else:
    ...
    def error(self, error_name, error_message, msg_id=None, quiet=False):
        ...
        :param quiet: specific to error_handlers. The default doesn't send a
                      ...
                      developer console.
        """
        self.socket.error(error_name, error_message, endpoint=self.ns_name, msg_id=msg_id, quiet=quiet)
    ...
    def disconnect(self, silent=False):
        ...
        """
        if not silent:
            packet = {'type': 'disconnect', 'endpoint': self.ns_name}
            self.socket.send_packet(packet)
        try:
