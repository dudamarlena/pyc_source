...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/socketio/server.py
...
class SocketIOServer(WSGIServer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.config = {'heartbeat_timeout': 60, 
           'close_timeout': 60, 
...
def serve(app, **kw):
    ...
    if policy_server in (True, 'True', 'true', 'enable', 'yes', 'on', '1'):
        ...
        policy_server = False
    server = SocketIOServer((host, port), app, resource=_resource, transports=transports, policy_server=policy_server, **kw)
    if not _quiet:
