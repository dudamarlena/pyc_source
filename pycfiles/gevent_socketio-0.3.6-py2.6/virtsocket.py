...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/socketio/virtsocket.py
...
"""
import random, weakref, logging, gevent
...
def default_error_handler(socket, error_name, error_message, endpoint, msg_id, quiet):
    ...
    """
    pkt = dict(type='event', name='error', args=[
     error_name, error_message], endpoint=endpoint)
    if msg_id:
        pkt['id'] = msg_id
    if not quiet:
        socket.send_packet(pkt)
    log.error(('default_error_handler: {}, {} (endpoint={}, msg_id={})').format(error_name, error_message, endpoint, msg_id))
...
class Socket(object):
    ...
    def error(self, error_name, error_message, endpoint=None, msg_id=None, quiet=False):
    ...
    def _receiver_loop(self):
        ...
        while True:
            ...
            try:
                ...
                self.error('invalid_packet', 'There was a decoding error when dealing with packet with event: %s... (%s)' % (
                 rawdata[:20], e))
                continue
            ...
            if endpoint not in self.namespaces:
                ...
                pkt_ns = new_ns_class(self.environ, endpoint, request=self.request)
            ...
            if pkt.get('ack') == 'data' and pkt.get('id'):
                if type(retval) is tuple:
                    ...
                    args = [
                     retval]
                returning_ack = dict(type='ack', ackId=pkt['id'], args=args, endpoint=pkt.get('endpoint', ''))
