...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/rest.py
# Compiled at: 2016-08-29 16:26:48
"""
...
"""
import datetime, decimal, logging, re, projex.text
...
def json2py(json_obj):
    ...
    for key, value in json_obj.items():
        ...
        if re.match('^\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}:\\d+$', value):
            ...
            if re.match('^\\d{2}:\\d{2}:\\d{2}:\\d+$', value):
...
def py2json(py_obj):
    ...
    if method:
        ...
        if type(py_obj) == datetime.date:
            return py_obj.isoformat()
        if type(py_obj) == datetime.time:
            return py_obj.isoformat()
        if type(py_obj) == set:
            return list(py_obj)
        if type(py_obj) == decimal.Decimal:
            return str(py_obj)
...
def xmlresponse(py_data):
    ...
    type_map = {'bool': 'boolean', 'float': 'double', 
       'str': 'string', 
       'unicode': 'string', 
    ...
    def xobj(xparent, py_obj):
        if type(py_obj) in (tuple, list):
            ...
            if isinstance(py_obj, datetime.date) or isinstance(py_obj, datetime.time) or isinstance(py_obj, datetime.datetime):
            ...
            elif type(py_obj) not in (str, unicode):
