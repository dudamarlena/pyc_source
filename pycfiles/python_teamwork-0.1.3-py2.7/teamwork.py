...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/teamwork/teamwork.py
# Compiled at: 2015-12-16 02:16:12
...
class Teamwork(object):
    ...
    def get(self, path=None, params=None, data=None):
        ...
        if params:
            payload = params
        request = requests.get(url, auth=(self._api_key, ''), params=payload)
        return request.json()
    ...
    def post(self, path=None, data=None):
        ...
        if path:
            url = '%s/%s' % (url, path)
        request = requests.post(url, auth=(self._api_key, ''), data=data)
    ...
    def get_project_times(self, project_id, user_id=None, start_date=None, end_date=None):
        """
        ...
        result = self.get('/projects/%i/time_entries.json' % project_id, params=payload)
        return result.get('time-entries')

    def save_project_time_entry(self, project_id, entry_date, duration, user_id, description, start_time):
        """
        ...
        data = {'time-entry': {'description': description, 
                          'person-id': user_id, 
                          ...
                          'isbillable': '1'}}
        result = self.post('/projects/%i/time_entries.json' % project_id, data=json.dumps(data))
        return result
