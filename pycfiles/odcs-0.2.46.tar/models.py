...
class Compose(ODCSBase):
    ...
    @classmethod
    def create(cls, session, owner, source_type, source, results, seconds_to_live, packages=None, flags=0, sigkeys=None, arches=None, multilib_arches=None, multilib_method=None, builds=None, lookaside_repos=None, modular_koji_tags=None, module_defaults_url=None):
        now = datetime.utcnow()
        compose = cls(owner=owner, source_type=source_type, source=source, sigkeys=sigkeys, state='wait', results=results, time_submitted=now, time_to_expire=now + timedelta(seconds=seconds_to_live), packages=packages, flags=flags, arches=arches if arches else (' ').join(conf.arches), multilib_arches=multilib_arches if multilib_arches else '', multilib_method=multilib_method if multilib_method else 0, builds=builds, lookaside_repos=lookaside_repos, modular_koji_tags=modular_koji_tags, module_defaults_url=module_defaults_url)
        session.add(compose)
        return compose
    ...
    def create_copy(cls, session, compose, owner=None, seconds_to_live=None):
        ...
        compose = cls(owner=owner or compose.owner, source_type=compose.source_type, source=compose.source, state='wait', results=compose.results, time_submitted=now, time_to_expire=now + timedelta(seconds=seconds_to_live), packages=compose.packages, builds=compose.builds, flags=compose.flags, koji_event=compose.koji_event, arches=compose.arches, multilib_arches=compose.multilib_arches, multilib_method=compose.multilib_method, sigkeys=compose.sigkeys, lookaside_repos=compose.lookaside_repos, modular_koji_tags=compose.modular_koji_tags, module_defaults_url=compose.module_defaults_url)
        session.add(compose)
        return compose
