...
# Size of source mod 2**32: 13893 bytes
...
class ODCS(object):
    ...
    def _make_request(self, method, resource_path, data=None):
        ...
        if self.auth_mech == AuthMech.OpenIDC:
            ...
            if self.auth_mech == AuthMech.Kerberos:
                request_data['auth'] = HTTPKerberosAuth()
            else:
                if self.auth_mech == AuthMech.SSL:
                    request_data['cert'] = (
    ...
    def new_compose(self, source, source_type, seconds_to_live=None, packages=[], flags=[], sigkeys=None, results=None):
