...
class Compose(ODCSBase):
    ...
    @classmethod
    def create(cls, session, owner, source_type, source, results, seconds_to_live, packages=None, flags=0, sigkeys=None, arches=None):
        now = datetime.utcnow()
    ...
    def result_repo_url(self):
        ...
        if conf.pungi_runroot_enabled:
            if self.source_type in [PungiSourceType.KOJI_TAG,
