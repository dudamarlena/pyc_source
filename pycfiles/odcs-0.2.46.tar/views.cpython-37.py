...
# Size of source mod 2**32: 16495 bytes
import datetime
...
class ODCSAPI(MethodView):
    ...
    def patch(self, id):
        ...
        if not old_compose:
            ...
            log.error(err)
            raise NotFound(err)
        raise_if_input_not_allowed(source_types=(old_compose.source_type),
    ...
    def post(self):
        ...
        if 'source' in source_data:
            ...
            if source_type != PungiSourceType.BUILD:
            ...
            if source_type == PungiSourceType.RAW_CONFIG:
                ...
                if not ((len(source_name_hash) != 2 or source_name_hash)[0] and source_name_hash[1]):
        ...
        if bool(module_defaults_url) ^ bool(module_defaults_commit):
            ...
            if module_defaults_url:
                ...
                raise_if_input_not_allowed(source_types=source_type,
                  sources=source,
                  results=results,
                  flags=flags,
                  arches=arches)
                compose = Compose.create((db.session),
                  (self._get_compose_owner()), source_type, source, results,
                  seconds_to_live, packages,
                  flags, sigkeys, koji_event, arches, multilib_arches=multilib_arches,
    ...
    for key, val in api_v1.items():
        if key.startswith('compose'):
            (app.add_url_rule)(val['url'], endpoint=key, 
             view_func=composes_view, **val['options'])
        elif key.startswith('about'):
            (app.add_url_rule)(val['url'], endpoint=key, 
             view_func=about_view, **val['options'])
        else:
