...
import os, shutil, tempfile, unittest, koji
...
class TestPungiConfig(unittest.TestCase):
    ...
    def test_pungi_config_module(self):
        pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.MODULE, 'testmodule:master:1:1')
    ...
    def test_pungi_config_tag(self):
        pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', packages=['file'], sigkeys='123 456', arches=[
         'ppc64', 's390'])
    ...
    def test_get_pungi_conf_iso(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.MODULE, 'testmodule:master:1:1', results=COMPOSE_RESULTS['iso'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
    ...
    def test_get_pungi_conf_boot_iso(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.MODULE, 'testmodule:master:1:1', results=COMPOSE_RESULTS['boot.iso'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
    ...
    def test_get_pungi_conf_check_deps(self):
        ...
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            ...
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', flags=COMPOSE_FLAGS['check_deps'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
    ...
    def test_get_pungi_conf_multilib(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', multilib_arches=['x86_64', 's390x'], multilib_method=3)
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
    ...
    def test_get_pungi_conf_pkgset_koji_builds(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', builds=['foo-1-1', 'bar-1-1'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
            self.assertEqual(set(cfg['pkgset_koji_builds']), set(['foo-1-1', 'bar-1-1']))
    ...
    def test_get_pungi_conf_modular_koji_tags(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', modular_koji_tags='f26-modules', module_defaults_url='git://localhost.tld/x.git master')
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
            self.assertEqual(set(cfg['pkgset_koji_module_tag']), set(['f26-modules']))
            self.assertEqual(cfg['gather_method'], 'hybrid')
            self.assertEqual(cfg['module_defaults_dir'], {'branch': 'master', 
               'dir': '.', 
    ...
    def test_get_pungi_conf_source_type_build(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.BUILD, 'x', builds=['foo-1-1', 'bar-1-1'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
    ...
    def test_get_pungi_conf_source_type_koji_tag_some_packages(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', packages=[
             'file'])
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
            self.assertEqual(cfg['pkgset_koji_tag'], 'f26')
    ...
    def test_get_pungi_conf_lookaside_repos(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.KOJI_TAG, 'f26', lookaside_repos='foo bar')
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
            self.assertEqual(cfg['gather_lookaside_repos'], [
             (
              '^.*$', {'*': ['foo', 'bar']})])
    ...
    def test_get_pungi_conf_include_devel_modules(self):
        ...
        shutil.copy2(template_path, mock_path)
        with patch('odcs.server.pungi.conf.pungi_conf_path', mock_path):
            pungi_cfg = PungiConfig('MBS-512', '1', PungiSourceType.MODULE, 'foo:1:1:1 foo-devel:1:1:1 bar-devel:1:1:1')
            template = pungi_cfg.get_pungi_config()
            cfg = self._load_pungi_cfg(template)
            self.assertEqual(cfg['include_devel_modules'], {'Temporary': ['foo-devel:1']})
...
class TestPungi(unittest.TestCase):
    ...
    def test_pungi_run(self, execute_cmd):
        ...
        self.makedirs.assert_called_with(AnyStringWith('test_composes/odcs-1-1-'))
        self.makedirs.assert_called_with(AnyStringWith('work/global'))
        self.ci_dump.assert_called_once_with(AnyStringWith('work/global/composeinfo-base.json'))
        execute_cmd.assert_called_once_with([
         'pungi-koji', AnyStringWith('pungi.conf'),
         AnyStringWith('--compose-dir='), '--nightly'], cwd=AnyStringWith('/tmp/'), timeout=3600, stderr=AnyStringWith('pungi-stderr.log'), stdout=AnyStringWith('pungi-stdout.log'))
    ...
    def test_pungi_run_raw_config(self, execute_cmd):
        ...
        fake_raw_config_urls = {'pungi.conf': {'url': 'http://localhost/test.git', 
        ...
        self.clone_repo.assert_called_once_with('http://localhost/test.git', AnyStringWith('/raw_config_repo'), commit='hash')
...
class TestPungiLogs(ModelsBaseTest):
    ...
    def setUp(self):
        ...
        self.compose = Compose.create(db.session, 'me', PungiSourceType.KOJI_TAG, 'tag', COMPOSE_RESULTS['repository'], 3600, packages='ed')
...
class TestPungiRunroot(unittest.TestCase):
    ...
    def test_pungi_run_runroot(self):
        ...
        pungi.run(self.compose)
        conf_topdir = os.path.join(conf.target_dir, 'odcs/unique_path')
        self.koji_session.uploadWrapper.assert_any_call(os.path.join(conf_topdir, 'pungi.conf'), 'odcs/unique_path', callback=None)
        self.koji_session.uploadWrapper.assert_any_call(os.path.join(conf_topdir, 'variants.xml'), 'odcs/unique_path', callback=None)
        self.koji_session.uploadWrapper.assert_any_call(os.path.join(conf_topdir, 'comps.xml'), 'odcs/unique_path', callback=None)
        self.koji_session.runroot.assert_called_once_with('f26-build', 'x86_64', 'cp /mnt/koji/work/odcs/unique_path/* . && cp ./odcs_koji.conf /etc/koji.conf.d/ && pungi-koji --config=./pungi.conf --target-dir=/mnt/koji/compose/odcs --nightly', channel='channel', mounts=['/mnt/odcs-secrets'], packages=['pungi'], weight=3.5)
    ...
    def test_pungi_run_runroot_raw_config(self):
        ...
        fake_raw_config_pungi_koji_args = {'pungi.conf': [
                        '--nightly']}
        with patch.object(conf, 'raw_config_urls', new=fake_raw_config_urls):
            with patch.object(conf, 'raw_config_pungi_koji_args', new=fake_raw_config_pungi_koji_args):
                pungi = Pungi(1, RawPungiConfig('pungi.conf#hash'))
                pungi.run(self.compose)
                self.assertTrue('commit' not in fake_raw_config_urls['pungi.conf'])
        conf_topdir = os.path.join(conf.target_dir, 'odcs/unique_path')
        self.koji_session.uploadWrapper.assert_has_calls([
         call(os.path.join(conf_topdir, 'odcs_koji.conf'), 'odcs/unique_path', callback=None),
         call(os.path.join(conf_topdir, 'pungi.conf'), 'odcs/unique_path', callback=None),
         call(os.path.join(conf_topdir, 'raw_config.conf'), 'odcs/unique_path', callback=None)])
        self.koji_session.runroot.assert_called_once_with('f26-build', 'x86_64', 'cp /mnt/koji/work/odcs/unique_path/* . && cp ./odcs_koji.conf /etc/koji.conf.d/ && pungi-koji --config=./pungi.conf --target-dir=/mnt/koji/compose/odcs --nightly', channel='channel', mounts=['/mnt/odcs-secrets'], packages=['pungi'], weight=3.5)
        self.koji_session.taskFinished.assert_called_once_with(123)
        self.assertEqual(self.compose.koji_task_id, 123)
        return