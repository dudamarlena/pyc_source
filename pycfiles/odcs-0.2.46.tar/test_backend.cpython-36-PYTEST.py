...
# Size of source mod 2**32: 17454 bytes
...
from odcs.server.backend import resolve_compose, get_reusable_compose, generate_compose, generate_pulp_compose, generate_pungi_compose, validate_pungi_compose
...
class TestBackend(ModelsBaseTest):
    ...
    def test_resolve_compose_repo(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.REPO), (os.path.join(thisdir, 'repo')), (COMPOSE_RESULTS['repository']),
          3600, packages='ed')
        db.session.commit()
        resolve_compose(c)
        db.session.commit()
    ...
    def test_resolve_compose_module_no_deps(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.MODULE), 'moduleA-f26 moduleA-f26',
          (COMPOSE_RESULTS['repository']),
          3600, flags=(COMPOSE_FLAGS['no_deps']))
        db.session.commit()
        resolve_compose(c)
        db.session.commit()
        c = db.session.query(Compose).filter(Compose.id == 1).one()
    ...
    def expect_module_lookup_error(self, source, match, flags=0):
        c = Compose.create((db.session),
          'me', (PungiSourceType.MODULE), source,
          (COMPOSE_RESULTS['repository']),
          3600, flags=flags)
        db.session.commit()
    ...
    def test_resolve_compose_module_not_conflict(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.MODULE), 'moduleB-f26 moduleB-f27',
          (COMPOSE_RESULTS['repository']),
          3600, flags=(COMPOSE_FLAGS['no_deps']))
        db.session.commit()
        resolve_compose(c)
    ...
    def test_resolve_compose_module_dep_not_found(self):
        self.expect_module_lookup_error('moduleB-f26 moduleB-f27', 'Failed to find module moduleC-f27 in the PDC.')
    ...
    def test_resolve_compose_repo_no_override_koji_event(self, create_koji_session):
        ...
        c = Compose.create((db.session),
          'me', (PungiSourceType.KOJI_TAG), 'f26', (COMPOSE_RESULTS['repository']),
          3600, packages='ed')
        c.koji_event = 1
        db.session.commit()
        resolve_compose(c)
    ...
    def test_get_reusable_compose(self):
        old_c = Compose.create((db.session),
          'me', (PungiSourceType.REPO), (os.path.join(thisdir, 'repo')), (COMPOSE_RESULTS['repository']),
          3600, packages='ed')
    ...
    def test_generate_pulp_compose_content_set_not_found(self, _write_repo_file, pulp_rest_post):
        ...
        self.assertRegexpMatches(c1.state_reason, 'Error while generating compose: Failed to find all the content_sets.*')
...
class TestGeneratePungiCompose(ModelsBaseTest):
    ...
    def test_generate_pungi_compose(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.KOJI_TAG), 'f26', (COMPOSE_RESULTS['repository']),
          60, packages='pkg1 pkg2 pkg3', arches='x86_64 s390')
        c.id = 1
    ...
    def test_generate_pungi_compose_nodeps(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.KOJI_TAG), 'f26', (COMPOSE_RESULTS['repository']),
          60, packages='pkg1 pkg2 pkg3', flags=(COMPOSE_FLAGS['no_deps']))
        c.id = 1
        generate_pungi_compose(c)
    ...
    def test_generate_pungi_compose_noinheritance(self):
        c = Compose.create((db.session),
          'me', (PungiSourceType.KOJI_TAG), 'f26', (COMPOSE_RESULTS['repository']),
          60, packages='pkg1 pkg2 pkg3', flags=(COMPOSE_FLAGS['no_inheritance']))
        c.id = 1
        generate_pungi_compose(c)
        self.assertEqual(self.pungi_config.gather_method, 'deps')
...
class TestValidatePungiCompose(ModelsBaseTest):
    __doc__ = 'Test validate_pungi_compose'

    def setUp(self):
        super(TestValidatePungiCompose, self).setUp()
        self.c = Compose.create((db.session),
          'me', (PungiSourceType.KOJI_TAG), 'f26', (COMPOSE_RESULTS['repository']),
          60, packages='pkg1 pkg2 pkg3')
        db.session.commit()
