...
from odcs.common.types import COMPOSE_RESULTS, COMPOSE_FLAGS, COMPOSE_STATES, PUNGI_SOURCE_TYPE_NAMES, PungiSourceType, MULTILIB_METHODS
from odcs.server.api_utils import pagination_metadata, filter_composes, validate_json_data, raise_if_input_not_allowed
from odcs.server.auth import requires_role, login_required
from odcs.server.auth import require_scopes
try:
    from odcs.server.celery_tasks import generate_pulp_compose, generate_pungi_compose
...
api_v1 = {'composes': {'url': '/api/1/composes/', 
                'options': {'defaults': {'id': None}, 'methods': [
                                      'GET']}}, 
...
class ODCSAPI(MethodView):
    ...
    def _get_compose_owner(self):
        if conf.auth_backend == 'noauth':
            log.warning("Cannot determine the owner of compose, because 'noauth' auth_backend is used.")
            return 'unknown'
        else:
    ...
    def patch(self, id):
        ...
        if not old_compose:
            ...
            log.error(err)
            raise NotFound(err)
        raise_if_input_not_allowed(source_types=old_compose.source_type, sources=old_compose.source, results=old_compose.results, flags=old_compose.flags, arches=old_compose.arches)
    ...
    def post(self):
        ...
        if source_type == PungiSourceType.RAW_CONFIG:
            ...
            for module_str in source:
                ...
                if len(nsvc) < 2:
                    raise ValueError('Module definition must be in "n:s", "n:s:v" or "n:s:v:c" format, but got %s' % module_str)
                if nsvc[0] in conf.base_module_names:
                    raise ValueError('ODCS currently cannot create compose with base modules, but %s was requested.' % nsvc[0])
        ...
        if bool(module_defaults_url) ^ bool(module_defaults_commit):
            raise ValueError('The "module_defaults_url" and "module_defaults_commit" must be used together.')
        elif module_defaults_url and module_defaults_commit:
            module_defaults = '%s %s' % (module_defaults_url, module_defaults_commit)
        raise_if_input_not_allowed(source_types=source_type, sources=source, results=results, flags=flags, arches=arches)
        compose = Compose.create(db.session, self._get_compose_owner(), source_type, source, results, seconds_to_live, packages, flags, sigkeys, arches, multilib_arches=multilib_arches, multilib_method=multilib_method, builds=builds, lookaside_repos=lookaside_repos, modular_koji_tags=modular_koji_tags, module_defaults_url=module_defaults)
        db.session.add(compose)
        db.session.flush()
        db.session.commit()
        if CELERY_AVAILABLE and conf.celery_broker_url:
...
def register_api_v1():
    ...
    for key, val in api_v1.items():
        if key.startswith('compose'):
            app.add_url_rule(val['url'], endpoint=key, view_func=composes_view, **val['options'])
