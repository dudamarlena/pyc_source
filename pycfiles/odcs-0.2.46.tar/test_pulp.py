...
# [GCC 8.4.0]
# Embedded file name: /home/hanzz/releases/odcs/server/tests/test_pulp.py
...
class TestPulp(ModelsBaseTest):
    ...
    def test_pulp_request(self, pulp_rest_post):
        ...
        pulp_rest_post.assert_called_once_with('repositories/search/', {'criteria': {'fields': [
                                 'notes'], 
                        'filters': {'notes.include_in_download_service': 'True', 
    ...
    def test_generate_pulp_compose_arch_merge(self, pulp_rest_post):
        ...
        pulp_rest_post.return_value = [
         ...
         {'notes': {'relative_url': 'content/1/ppc64le/os', 
                      'content_set': 'foo-1', 
                      'arch': 'ppc64le', 
                      'signatures': 'SIG1,SIG2'}},
        ...
        self.assertEqual(ret, {'foo-1': {'url': 'http://localhost/content/1/$basearch/os', 
                     'arches': set(['x86_64', 'ppc64le']), 
                     'sigkeys': [
                               'SIG1', 'SIG2']}, 
           'foo-2': {'url': 'http://localhost/content/3/ppc64/os', 
                     'arches': set(['ppc64']), 
                     'sigkeys': [
                               'SIG1', 'SIG3']}})
    ...
    def test_pulp_compose_merge_repos(self, download_repodata, lock, makedirs, execute_cmd, pulp_rest_post):
        c = Compose.create(db.session, 'me', PungiSourceType.PULP, 'foo-1', 0, 3600)
        db.session.commit()
        pulp_rest_post.return_value = [
         {'notes': {'relative_url': 'content/1.0/x86_64/os', 
                      'content_set': 'foo-1', 
                      'arch': 'x86_64', 
                      'signatures': 'SIG1,SIG2'}},
         {'notes': {'relative_url': 'content/1.1/x86_64/os', 
                      'content_set': 'foo-1', 
                      'arch': 'x86_64', 
                      'signatures': 'SIG1,SIG2'}},
         {'notes': {'relative_url': 'content/1.0/ppc64le/os', 
                      'content_set': 'foo-1', 
                      'arch': 'ppc64le', 
                      'signatures': 'SIG1,SIG2'}}]
        pulp = Pulp('http://localhost/', 'user', 'pass', c)
        ret = pulp.get_repos_from_content_sets(['foo-1', 'foo-2'])
        self.assertEqual(ret, {'foo-1': {'url': 'http://localhost/odcs/latest-odcs-1-1/compose/Temporary/foo-1/$basearch', 
                     'arches': set(['x86_64', 'ppc64le']), 
                     'sigkeys': [
                               'SIG1', 'SIG2']}})
        ...
        download_repodata.assert_any_call(repo_prefix + '1.0/x86_64/os', 'http://localhost/content/1.0/x86_64/os')
        download_repodata.assert_any_call(repo_prefix + '1.1/x86_64/os', 'http://localhost/content/1.1/x86_64/os')
        download_repodata.assert_any_call(repo_prefix + '1.0/ppc64le/os', 'http://localhost/content/1.0/ppc64le/os')