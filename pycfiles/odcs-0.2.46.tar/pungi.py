...
import copy, os, shutil, tempfile, jinja2, koji, munch, time, random, string
from productmd.composeinfo import ComposeInfo
import odcs.server.utils
from odcs.server import conf, log, db
from odcs.server import comps
from odcs.common.types import PungiSourceType, COMPOSE_RESULTS, MULTILIB_METHODS, INVERSE_PUNGI_SOURCE_TYPE_NAMES, COMPOSE_FLAGS
...
class RawPungiConfig(BasePungiConfig):
    ...
    def __init__(self, compose_source):
        ...
        self.pungi_koji_args = conf.raw_config_pungi_koji_args.get(source_name, conf.pungi_koji_args)
    ...
    def write_config_files(self, topdir):
        ...
        if conf.raw_config_wrapper_conf_path:
            ...
            shutil.copy2(conf.raw_config_wrapper_conf_path, os.path.join(topdir, 'pungi.conf'))
        else:
...
class PungiConfig(BasePungiConfig):
    ...
    def __init__(self, release_name, release_version, source_type, source, packages=None, arches=None, sigkeys=None, results=0, multilib_arches=None, multilib_method=0, builds=None, flags=0, lookaside_repos=None, modular_koji_tags=None, module_defaults_url=None):
    ...
    def get_pungi_config(self):
        try:
            ...
            log.exception(('Failed to render pungi conf template {!r}: {}').format(conf.pungi_conf_path, str(e)))
...
class Pungi(object):
    ...
    def _write_cfgs(self, topdir):
        ...
        if conf.pungi_runroot_koji_conf_path:
            shutil.copy2(conf.pungi_runroot_koji_conf_path, os.path.join(topdir, 'odcs_koji.conf'))
    ...
    def make_koji_session(self):
        ...
        :return: KojiSession
        """
        koji_config = munch.Munch(koji.read_config(profile_name=conf.koji_profile, user_config=conf.koji_config))
        ...
        if authtype == 'kerberos':
            ...
            if keytab and principal:
                koji_session.krb_login(principal=principal, keytab=keytab, ccache=ccache)
            else:
                koji_session.krb_login(ccache=ccache)
        elif authtype == 'ssl':
            koji_session.ssl_login(os.path.expanduser(koji_config.cert), None, os.path.expanduser(koji_config.serverca))
        else:
        ...
        try:
            ...
            with open(log_out_path, 'w') as (log_out):
                with open(log_err_path, 'w') as (log_err):
                    odcs.server.utils.execute_cmd(pungi_cmd, cwd=td, timeout=conf.pungi_timeout, stdout=log_out, stderr=log_err)
        finally:
            try:
                if td is not None:
                    shutil.rmtree(td)
    ...
    def run_in_runroot(self, compose):
        ...
        """
        conf_topdir = os.path.join(conf.target_dir, self._unique_path('runroot_configs'))
        ...
        kwargs = {'channel': conf.pungi_parent_runroot_channel, 
        ...
        task_id = koji_session.runroot(conf.pungi_parent_runroot_tag, conf.pungi_parent_runroot_arch, (' ').join(cmd), **kwargs)
...
class PungiLogs(object):
    ...
    def global_log_path(self):
        ...
        if not toplevel_work_dir:
            ...
            return os.path.join(toplevel_work_dir, 'logs', 'global', 'pungi.global.log')
