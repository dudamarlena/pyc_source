...
# [GCC 8.4.0]
# Embedded file name: /home/hanzz/releases/odcs/server/tests/test_celery_tasks.py
...
class TestCeleryRouter:
    ...
    def test_empty_rule(self, mock_get_compose):
        ...
        mock_conf = {'routing_rules': {'odcs.server.celery_tasks.generate_pungi_compose': {'pungi_composes': {}, 'other_composes': {'source_type': 4}}}, 
        ...
        args = [[1], {}]
        kwargs = {}
        queue = tr.route_for_task('odcs.server.celery_tasks.generate_pungi_compose', *args, **kwargs)
    ...
    def test_default_queue(self, mock_get_compose):
        mock_compose = Mock()
        compose_md = {'source_type': 3}
        mock_conf = {'routing_rules': {'some.other.task': {'pungi_composes': {}, 'other_composes': {'source_type': 4}}}, 
           'cleanup_task': 'odcs.server.celery_tasks.run_cleanup', 
           'default_queue': 'default_queue'}
        tr = TaskRouter()
        tr.config = mock_conf
    ...
    def test_rule_with_single_property(self, mock_get_compose):
        mock_compose = Mock()
        compose_md = {'source_type': 3}
        mock_conf = {'routing_rules': {'odcs.server.celery_tasks.generate_pungi_compose': {'pungi_composes': {'source_type': 3}, 
    ...
    def test_cleanup_queue(self, mock_get_compose):
        ...
        args = [[1], {}]
        kwargs = {}
        queue = tr.route_for_task('odcs.server.celery_tasks.run_cleanup', *args, **kwargs)
    ...
    def test_invalid_rule_property_exception(self, mock_get_compose):
        mock_compose = Mock()
        compose_md = {'source_type': 3}
        mock_conf = {'routing_rules': {'odcs.server.celery_tasks.generate_pungi_compose': {'pungi_composes': {'bad_compose_prop': 3}}}, 
           'cleanup_task': 'odcs.server.celery_tasks.run_cleanup', 
           'default_queue': 'default_queue'}
        tr = TaskRouter()
        tr.config = mock_conf
        ...
        with pytest.raises(ValueError) as (e):
            tr.route_for_task('odcs.server.celery_tasks.generate_pungi_compose', *args, **kwargs)
