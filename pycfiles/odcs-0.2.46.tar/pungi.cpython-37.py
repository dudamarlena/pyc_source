...
# Size of source mod 2**32: 22438 bytes
...
class PungiConfig(BasePungiConfig):
    ...
    def __init__(self, release_name, release_version, source_type, source, packages=None, arches=None, sigkeys=None, results=0, multilib_arches=None, multilib_method=0, builds=None, flags=0, lookaside_repos=None, modular_koji_tags=None, module_defaults_url=None):
        ...
        elif 'boot.iso' in self.results:
            self.bootable = True
        elif source_type == PungiSourceType.KOJI_TAG:
...
class Pungi(object):
    ...
    def make_koji_session(self):
        ...
        :return: KojiSession
        """
        koji_config = munch.Munch(koji.read_config(profile_name=(conf.koji_profile),
        ...
        if authtype == 'kerberos':
            ...
            if keytab and principal:
                koji_session.krb_login(principal=principal,
                  keytab=keytab,
                  ccache=ccache)
            else:
    ...
    def get_pungi_cmd(self, conf_topdir, targetdir, compose_dir=None):
        ...
        if isinstance(self.pungi_cfg, RawPungiConfig):
            ...
            if isinstance(self.pungi_cfg, PungiConfig):
        ...
        try:
            ...
            with open(log_out_path, 'w') as (log_out):
                with open(log_err_path, 'w') as (log_err):
                    odcs.server.utils.execute_cmd(pungi_cmd,
                      cwd=td, timeout=(conf.pungi_timeout), stdout=log_out,
                      stderr=log_err)
        finally:
            try:
                if td is not None:
