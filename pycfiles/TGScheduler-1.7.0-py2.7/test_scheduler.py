...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgscheduler/tests/test_scheduler.py
# Compiled at: 2015-05-15 12:32:43
import unittest
from tgscheduler import start_scheduler, stop_scheduler, add_single_task, add_interval_task, add_monthly_task, add_weekly_task, add_cron_like_task, cancel, get_task, get_tasks, rename_task
...
class TestTasks(unittest.TestCase):
    ...
    def test_weekly_task(self):
        ...
        task = add_weekly_task(functest, [2], (0, 0), taskname='weekdaytest')
        self.assertEqual(task.action, functest)
    ...
    def test_interval_task(self):
        ...
        task = add_interval_task(functest, 600, taskname='intervaltest')
        self.assertEqual(task.action, functest)
    ...
    def test_cron_like_task(self):
        ...
        self.assertEqual(task.minutes, range(0, 60), 'Invalid minutes: %s' % task.minutes)
        self.assertEqual(task.hours, range(0, 24), 'Invalid hours: %s' % task.hours)
        self.assertEqual(task.doms, range(1, 32), 'Invalid doms: %s' % task.doms)
        self.assertEqual(task.months, range(1, 13), 'Invalid months: %s' % task.months)
        self.assertEqual(task.dows, list(range(0, 7)), 'Invalid dows: %s' % task.dows)
    ...
    def test_cron_like_business_hours(self):
        ...
        self.assertEqual(task.minutes, range(0, 60, 30), 'Invalid minutes: %s' % task.minutes)
        self.assertEqual(task.hours, [8, 9, 10, 11, 12, 14, 15, 16, 17, 18], 'Invalid hours: %s' % task.hours)
        self.assertEqual(task.doms, range(1, 32), 'Invalid doms: %s' % task.doms)
        self.assertEqual(task.months, range(1, 13), 'Invalid months: %s' % task.months)
        self.assertEqual(task.dows, list(range(0, 5)), 'Invalid dows: %s' % task.dows)
    ...
    def test_cron_like_complex(self):
        ...
        self.assertEqual(task.minutes, [
         5, 6, 7, 8, 9, 10, 25, 26, 27, 28, 29, 30, 57], 'Invalid minutes: %s' % task.minutes)
        self.assertEqual(task.hours, [8, 9, 10, 11, 12, 14, 15, 16, 17, 18], 'Invalid hours: %s' % task.hours)
        self.assertEqual(task.doms, [2, 25, 26, 27, 28], 'Invalid doms: %s' % task.doms)
        self.assertEqual(task.months, [1, 2, 3, 10, 11, 12], 'Invalid months: %s' % task.months)
        self.assertEqual(task.dows, [0, 1, 2, 6], 'Invalid dows: %s' % task.dows)
    ...
    def test_cron_like_failure(self):
        ...
        an incorrect value: `Jan-12` (mixing names and integers is not
        supported)

        """
        self.assertRaises(ValueError, add_cron_like_task, functest, '* * * Jan-12 *', taskname='crontest3')

    def test_duplicate_task(self):
        """Test adding a task with the same name as an existing task."""
        add_single_task(functest, taskname='duplicate')
        self.assertRaises(ValueError, add_single_task, functest, taskname='duplicate')
