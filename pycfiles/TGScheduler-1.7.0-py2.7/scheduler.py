...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgscheduler/scheduler.py
# Compiled at: 2015-05-15 12:25:47
"""Create the scheduler object and expose its method as functions."""
from tgscheduler import kronos
from tgscheduler.kronos import method
import atexit, logging, six
...
class Scheduler(object):
    ...
    def add_interval_task(self, action, interval, args=None, kw=None, initialdelay=0, processmethod=method.threaded, taskname=None):
        """Add an interval task to the scheduler.

        Pass in initialdelay with a number of seconds to wait before running
        and an interval with the number of seconds between runs.
        ...
        """
        si = self._get_scheduler()
        return si.add_interval_task(action=action, interval=interval, args=args, kw=kw, initialdelay=initialdelay, processmethod=processmethod, taskname=taskname)

    def add_single_task(self, action, args=None, kw=None, initialdelay=0, processmethod=method.threaded, taskname=None):
        ...
        """
        si = self._get_scheduler()
        return si.add_single_task(action=action, args=args, kw=kw, initialdelay=initialdelay, processmethod=processmethod, taskname=taskname)

    def add_weekday_task(self, action, weekdays, timeonday, args=None, kw=None, processmethod=method.threaded, taskname=None):
        ...
        """
        si = self._get_scheduler()
        return si.add_daytime_task(action=action, taskname=taskname, weekdays=weekdays, monthdays=None, timeonday=timeonday, processmethod=processmethod, args=args, kw=kw)

    def add_monthday_task(self, action, monthdays, timeonday, args=None, kw=None, processmethod=method.threaded, taskname=None):
        ...
        """
        si = self._get_scheduler()
        return si.add_daytime_task(action=action, taskname=taskname, weekdays=None, monthdays=monthdays, timeonday=timeonday, processmethod=processmethod, args=args, kw=kw)

    def add_cron_like_task(self, action, cron_str, args=None, kw=None, processmethod=method.threaded, taskname=None):
        """Add a task to the scheduler based on a cron-like syntax.

        @param cron_str: The scheduling information, written in a cron-like
        syntax
        ...
        @param taskname:  Tasks can have a name (stored in task.name), which
        can help if you're trying to keep track of many tasks.
        @param processmethod: By default, each task will be run in a new
        thread.
        You can also pass in turbogears.scheduler.method.sequential or
        turbogears.scheduler.method.forked.
        ...
        """
        si = self._get_scheduler()
        return si.add_cron_like_task(action=action, taskname=taskname, cron_str=cron_str, processmethod=processmethod, args=args, kw=kw)
...
add_monthly_task = add_monthday_task
add_weekday_task = scheduler.add_weekday_task
add_weekly_task = add_weekday_task
