...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgscheduler/kronos.py
# Compiled at: 2015-05-15 14:37:49
...
"""
import os, sys, sched, datetime, time, logging, weakref
try:
...
class Scheduler(object):
    ...
    def add_cron_like_task(self, action, taskname, cron_str, processmethod, args, kw):
...
class CronLikeTask(Task):
    ...
    **Names** can also be used for the "month" and "day of week" fields. Use
    ...
    def __init__(self, name, action, cron_str, args=None, kw=None):
        ...
        Task.__init__(self, name, action, args, kw)
        if not rrule:
            raise ImportError('The dateutil package must be installed if you want to use cron-like tasks.')
        ...
        self.dows = list(set([ (dow - 1) % 7 for dow in self.__process_str(dow_str, DOW_BOUNDS, mapping=DOW_MAPPING)
                             ]))
        self.dows.sort()
        ...
        for item in time_str.split(','):
            if '-' not in item:
                ...
                try:
                    ...
                    if mapping and interval_low.lower() in mapping and interval_high.lower() in mapping:
    ...
    def get_schedule_time(self):
        ...
        next_time = list(rrule.rrule(rrule.SECONDLY, count=1, bysecond=0, byminute=self.minutes, byhour=self.hours, bymonthday=self.doms, bymonth=self.months, byweekday=self.dows, dtstart=now)[0].timetuple())
