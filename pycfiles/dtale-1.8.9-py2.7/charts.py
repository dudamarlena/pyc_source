...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/dash_application/charts.py
# Compiled at: 2020-04-18 12:53:10
import json, math, os, traceback, urllib
from logging import getLogger
import dash_core_components as dcc, dash_html_components as html, numpy as np, pandas as pd, plotly.graph_objs as go
from plotly.io import write_html
from six import PY3, StringIO, string_types
import dtale.dash_application.components as dash_components, dtale.global_state as global_state
from dtale.charts.utils import YAXIS_CHARTS, ZAXIS_CHARTS, build_agg_data, build_base_chart
from dtale.charts.utils import build_formatters as chart_formatters
from dtale.charts.utils import build_group_inputs_filter, check_all_nan, check_exceptions, retrieve_chart_data, valid_chart, weekday_tick_handler
from dtale.dash_application.layout import AGGS, ANIMATE_BY_CHARTS, ANIMATION_CHARTS, build_error, test_plotly_version, update_label_for_freq
from dtale.utils import build_code_export, classify_type, dict_merge, divide_chunks, export_to_csv_buffer, find_dtype, find_dtype_formatter, flatten_lists, get_dtypes, make_list, run_query
    ...
    if final_params['chart_type'] in YAXIS_CHARTS:
        ...
        for y, range in (params.get('yaxis') or {}).items():
            if y not in ((data or {}).get('min') or {}):
                continue
            if not (
             range['min'], range['max']) == (data['min'][y], data['max'][y]):
        ...
        if axis_type == 'multi':
            for i, y2 in enumerate(y, 0):
                ...
                if y2 in axis_data and not (axis_data[y2]['min'], axis_data[y2]['max']) == (mins[y2], maxs[y2]):
                    value['range'] = [
                     axis_data[y2]['min'], axis_data[y2]['max']]
...
def chart_wrapper(data_id, data, url_params=None):
    ...
    if url_params is None:
        ...
        def _chart_wrapper(chart, group_filter=None):
            ...
            popup_link = html.A([
             html.I(className='far fa-window-restore mr-4'), html.Span('Popup Chart')], href=('/charts/{}?{}').format(data_id, querystring), target='_blank', className='mr-5')
            copy_link = html.Div([
             html.A([
              html.I(className='ico-link mr-4'), html.Span('Copy Link')], href=('/charts/{}?{}').format(data_id, querystring), target='_blank', className='mr-5 copy-link-btn'),
             html.Div('Copied to clipboard', className='hoverable__content copy-tt-bottom')], className='hoverable-click')
            code_snippet = html.A([
             html.I(className='ico-code mr-4'), html.Span('Code Export')], href='#', className='code-snippet-btn mr-5')
            export_html_link = html.A([
             html.I(className='fas fa-file-code mr-4'), html.Span('Export Chart')], href=('/dtale/chart-export/{}?{}').format(data_id, querystring), className='export-chart-btn mr-5')
            export_csv_link = html.A([
             html.I(className='fas fa-file-csv mr-4'), html.Span('Export CSV')], href=('/dtale/chart-csv-export/{}?{}').format(data_id, querystring), className='export-chart-btn')
            links = html.Div([
             popup_link, copy_link, code_snippet, export_html_link, export_csv_link], style={'position': 'absolute', 'zIndex': 5})
...
def cpg_chunker(charts, columns=2):
    ...
    return [ html.Div([ _formatter(c) for c in chunk ], className='row') for chunk in divide_chunks(charts, columns)
           ]
...
def scatter_builder(data, x, y, axes_builder, wrapper, group=None, z=None, agg=None, animate_by=None):
    ...
    def layout(axes):
        if z is not None:
            return {'margin': {'l': 0, 'r': 0, 'b': 0}, 'scene': dict_merge(axes, dict(aspectmode='data'))}
        else:
            return axes

    def marker(series):
        if z is not None:
            return {'size': 8, 'color': series[z], 'colorscale': 'Blackbody', 'opacity': 0.8, 'showscale': True, 
    ...
    def _build_final_scatter(y_val):
        figure_cfg = {'data': [ scatter_func(**dict_merge(dict(x=d['x'], y=d[y_val], mode='markers', opacity=0.7, name=series_key, marker=marker(d)), dict(z=d[z]) if z is not None else dict())) for series_key, d in data['data'].items() if y_val in d and (group is None or group == series_key)
                 ], 
           'layout': build_layout(dict_merge(build_title(x, y_val, group, z=z, agg=agg), layout(axes_builder([y_val])[0])))}
            ...
            def build_frame(frame):
                for series_key, series in frame.items():
                    if y_val in series and (group is None or group == series_key):
                        yield scatter_func(**dict(x=series['x'], y=series[y_val], mode='markers', opacity=0.7, name=series_key, marker=marker(series)))

                return

            update_cfg_w_frames(figure_cfg, *build_frames(data, build_frame))
        return wrapper(graph_wrapper(id=('scatter-{}-{}').format(group or 'all', y_val), figure=figure_cfg), group_filter=dict_merge(dict(y=y_val), {} if group is None else dict(group=group)))
...
def surface_builder(data, x, y, z, axes_builder, wrapper, agg=None):
    ...
    return [ wrapper(graph_wrapper(id=('surface-{}').format(y2), figure={'data': [go.Surface(x=x_vals, y=y_vals, z=z_data, opacity=0.8, name='all', colorscale='YlGnBu', colorbar={'title': layout['scene']['zaxis']['title'], 'thickness': 15, 'len': 0.5, 'x': 0.8, 'y': 0.6})], 'layout': build_layout(dict_merge(build_title(x, y2, z=z, agg=agg), layout))}), group_filter=dict(y=y2)) for y2 in y
           ]
    ...
    for i, y2 in enumerate(y, 1):
        for series in series_cfgs:
            ...
            if i == 1 and curr_y is None or yaxis == curr_y:
                ...
                yield dict_merge({k:v for k, v in series.items() if k in ('x', 'name',
                                                                          'type') if k in ('x',
                                                                                           'name',
                                                                                           'type')}, dict(hoverinfo='none', showlegend=False, y=[0]), dict(yaxis=yaxis) if i > 1 else {})
...
def update_cfg_w_frames(cfg, frames, slider_steps):
    ...
    cfg['layout']['updatemenus'] = [
     {'x': 0.1, 
        'y': 0, 'yanchor': 'top', 'xanchor': 'right', 'showactive': False, 'direction': 'left', 
        'type': 'buttons', 'pad': {'t': 87, 'r': 10}, 'buttons': [{'label': 'Play', 'method': 'animate', 'args': [None]},
    ...
    if slider_steps is not None:
        cfg['layout']['sliders'] = [
         {'active': 0, 'steps': [ dict(label=ss, method='animate', args=[[ss], dict(mode='immediate', transition=dict(duration=300), frame=dict(duration=300))]) for ss in slider_steps
                   ], 
            'x': 0.1, 
            'len': 0.9, 'xanchor': 'left', 'y': 0, 'yanchor': 'top', 'pad': {'t': 50, 'b': 10}, 'currentvalue': {'visible': True, 
                             'prefix': 'Year:', 'xanchor': 'right', 'font': {'size': 20, 'color': '#666'}}, 
...
def bar_builder(data, x, y, axes_builder, wrapper, cpg=False, barmode='group', barsort=None, **kwargs):
    ...
    for series_key, series in data['data'].items():
        ...
        if barsort_col != 'x' or kwargs.get('agg') == 'raw':
            ...
            axes['xaxis'] = dict_merge(axes.get('xaxis', {}), build_spaced_ticks(df['x'].values, mode='array'))

    if cpg:
        charts = [ wrapper(graph_wrapper(id=('bar-{}-graph').format(series_key), figure={'data': [ dict_merge({'x': series['x'], 'y': series[y2], 'type': 'bar'}, name_builder(y2, series_key), {} if i == 1 or not allow_multiaxis else {'yaxis': ('y{}').format(i)}, hover_text.get(series_key) or {}) for i, y2 in enumerate(y, 1) ], 'layout': build_layout(dict_merge(build_title(x, y, series_key, agg=kwargs.get('agg')), axes, dict(barmode=barmode or 'group')))}), group_filter=dict(group=series_key)) for series_key, series in data['data'].items()
                 ]
        return cpg_chunker(charts)
    data_cfgs = flatten_lists([ [ dict_merge({'x': series['x'], 'y': series[y2], 'type': 'bar'}, name_builder(y2, series_key), {} if i == 1 or not allow_multiaxis else {'yaxis': ('y{}').format(i)}, hover_text.get(series_key) or {}) for i, y2 in enumerate(y, 1) ] for series_key, series in data['data'].items()
                              ])
    if barmode == 'group' and allow_multiaxis:
        data_cfgs = list(build_grouped_bars_with_multi_yaxis(data_cfgs, y))
    figure_cfg = {'data': data_cfgs, 
       'layout': build_layout(dict_merge(build_title(x, y, agg=kwargs.get('agg')), axes, dict(barmode=barmode or 'group')))}
        ...
        def build_frame(frame):
            ...
            for series_key, series in frame['data'].items():
                ...
                if barsort_col != 'x' or kwargs.get('agg') == 'raw':
                    df = pd.DataFrame(series)
                    df = df.sort_values(barsort_col)
                    series = dict_merge({c:df[c].values for c in df.columns}, {'x': list(range(len(df['x']))), 'hovertext': df['x'].values, 'hoverinfo': 'y+text'})
                    layout['xaxis'] = dict_merge(axes.get('xaxis', {}), build_spaced_ticks(df['x'].values, mode='array'))
                for i, y2 in enumerate(y, 1):
                    data.append(dict_merge({k:v for k, v in series.items() if k in ('x',
                                                                                    'hovertext',
                                                                                    'hoverinfo') if k in ('x',
                                                                                                          'hovertext',
                                                                                                          'hoverinfo')}, {'y': series[y2], 'type': 'bar'}, name_builder(y2, series_key), {} if i == 1 or not allow_multiaxis else {'yaxis': ('y{}').format(i)}))
...
def line_builder(data, x, y, axes_builder, wrapper, cpg=False, **inputs):
    ...
    if cpg:
        charts = [ wrapper(graph_wrapper(id=('line-{}-graph').format(series_key), figure={'data': [ line_func(series)(**dict_merge(line_cfg(series), {'x': series['x'], 'y': series[y2]}, name_builder(y2, series_key), {} if i == 1 or not multi_yaxis else {'yaxis': ('y{}').format(i)})) for i, y2 in enumerate(y, 1) ], 'layout': build_layout(dict_merge(build_title(x, y, group=series_key, agg=inputs.get('agg')), axes))}), group_filter=dict(group=series_key)) for series_key, series in data['data'].items()
                 ]
        return cpg_chunker(charts)
    data_cfgs = flatten_lists([ [ line_func(series)(**dict_merge(line_cfg(series), {'x': series['x'], 'y': series[y2]}, name_builder(y2, series_key), {} if i == 1 or not multi_yaxis else {'yaxis': ('y{}').format(i)})) for i, y2 in enumerate(y, 1) ] for series_key, series in data['data'].items()
                              ])
        ...
        def build_frame(i):
            for series_key, series in data['data'].items():
                for j, y2 in enumerate(y, 1):
                    yield line_func(series)(**dict_merge(line_cfg(series), {'x': series['x'][:i], 'y': series[y2][:i]}, name_builder(y2, series_key), {} if j == 1 or not multi_yaxis else {'yaxis': ('y{}').format(j)}))
    ...
    def build_pies():
        for series_key, series in data['data'].items():
            for y2 in y:
                ...
                chart = wrapper(graph_wrapper(id=('pie-{}-graph').format(series_key), figure={'data': [
                          go.Pie(**dict_merge(dict(labels=series['x'], values=series[y2]), name_builder(y2, series_key)))], 
                   'layout': layout}), group_filter=dict_merge(dict(y=y2), {} if series_key == 'all' else dict(group=series_key)))
                if len(negative_values):
                    error_title = ('The following negative values could not be represented within the {}Pie chart').format('' if series_key == 'all' else ('{} ').format(series_key))
                    error_div = html.Div([
                     html.I(className='ico-error'),
                     html.Span(error_title),
                     html.Div(html.Pre((', ').join(negative_values)), className='traceback')], className='dtale-alert alert alert-danger')
                    if export:
                        yield chart
                    else:
                        yield html.Div([
                         html.Div(error_div, className='col-md-12'), html.Div(chart, className='col-md-12 h-100')], className='row')
                else:
                    yield chart
...
def heatmap_builder(data_id, export=False, **inputs):
    ...
    try:
        if not valid_chart(**inputs):
            ...
            raw_data = run_query(global_state.get_data(data_id), inputs.get('query'), global_state.get_context_variables(data_id))
            ...
            if agg is not None:
                ...
                if agg == 'corr':
                    ...
                    code.append(("chart_data.index.names = ['{x}', '{y}']\nchart_data = chart_data.reset_index()\nchart_data.loc[chart_data['{x}'] == chart_data['{y}'], '{z}'] = np.nan").format(x=x, y=y, z=z))
                    hm_kwargs = dict_merge(hm_kwargs, dict(colorscale=[[0, 'red'], [0.5, 'yellow'], [1.0, 'green']], zmin=-1, zmax=1))
                else:
            ...
            hm_kwargs = dict_merge(hm_kwargs, dict(x=x_data, y=y_data, z=heat_data, colorbar={'title': z_title}, hoverinfo='x+y+z'))
            chart = graph_wrapper(id=('heatmap-graph-{}').format(y), style={'margin-right': 'auto', 'margin-left': 'auto'}, figure=dict(data=[
             go.Heatmapgl(**hm_kwargs)], layout=build_layout(dict_merge(dict(xaxis=x_axis, yaxis=y_axis, xaxis_zeroline=False, yaxis_zeroline=False), build_title(x, y, z=z, agg=agg)))))
            if export:
                return chart
            return (
             wrapper(chart), code)
        ...
        return (
         build_error(e, traceback.format_exc()), code)
...
def map_builder(data_id, export=False, **inputs):
    ...
    try:
        if not valid_chart(**inputs):
            ...
            props = [
             'map_type', 'loc_mode', 'loc', 'lat', 'lon', 'map_val', 'scope', 'proj', 'agg', 'animate_by']
            ...
            if map_type == 'scattergeo':
                ...
                if map_val is not None:
                    ...
                    chart_kwargs['marker'] = dict(color=data[map_val], cmin=data[map_val].min(), cmax=data[map_val].max(), colorscale=inputs.get('colorscale') or 'Reds', colorbar_title=map_val)
                ...
                chart = graph_wrapper(id='scattergeo-graph', style={'margin-right': 'auto', 'margin-left': 'auto', 'height': '95%'}, config=dict(topojsonURL='/maps/'), figure=figure_cfg)
            else:
                ...
                figure_cfg = dict(data=[
                 go.Choropleth(locations=data[loc], locationmode=loc_mode, z=data[map_val], colorscale=inputs.get('colorscale') or 'Reds', colorbar_title=map_val, zmin=data[map_val].min(), zmax=data[map_val].max())], layout=layout)
                if animate_by is not None:
                    ...
                    update_cfg_w_frames(figure_cfg, *build_map_frames(data, animate_by, build_frame))
                chart = graph_wrapper(id='choropleth-graph', style={'margin-right': 'auto', 'margin-left': 'auto'}, config=dict(topojsonURL='/maps/'), figure=figure_cfg)
            if export:
                return chart
            return (
             wrapper(chart), code)
...
def build_figure_data(data_id, chart_type=None, query=None, x=None, y=None, z=None, group=None, group_val=None, agg=None, window=None, rolling_comp=None, animate_by=None, **kwargs):
    """
    ...
    if not valid_chart(**dict(x=x, y=y, z=z, chart_type=chart_type, agg=agg, window=window, rolling_comp=rolling_comp)):
        ...
        chart_kwargs = dict(group_col=group, group_val=group_val, agg=agg, allow_duplicates=chart_type == 'scatter', rolling_win=window, rolling_comp=rolling_comp)
...
def build_raw_figure_data(data_id, chart_type=None, query=None, x=None, y=None, z=None, group=None, agg=None, window=None, rolling_comp=None, **kwargs):
    """
    ...
    """
    chart_params = dict_merge(dict(x=x, y=y, z=z, chart_type=chart_type, agg=agg, window=window, rolling_comp=rolling_comp), kwargs)
    ...
    if chart_type == 'maps':
        ...
        return data
    chart_kwargs = dict(group_col=group, agg=agg, allow_duplicates=chart_type == 'scatter', rolling_win=window, rolling_comp=rolling_comp)
    if chart_type in ZAXIS_CHARTS:
        chart_kwargs['z'] = z
        del chart_kwargs['group_col']
...
def build_chart(data_id=None, **inputs):
    ...
    try:
        ...
        chart_inputs = {k:v for k, v in inputs.items() if k not in ('chart_type', 'x',
                                                                    'y', 'z', 'group') if k not in ('chart_type',
                                                                                                    'x',
                                                                                                    'y',
                                                                                                    'z',
                                                                                                    'group')}
        ...
        if chart_type == 'scatter':
            if inputs['cpg']:
                scatter_charts = flatten_lists([ scatter_builder(data, x, y, axes_builder, chart_builder, group=subgroup, agg=agg) for subgroup in data['data']
                                               ])
            else:
        ...
        if chart_type == '3d_scatter':
            chart = scatter_builder(data, x, y, axes_builder, chart_builder, z=z, agg=agg, animate_by=animate_by)
            return (
             chart, range_data, code)
        ...
        return (
         build_error(e, traceback.format_exc()), None, code)
...
def build_raw_chart(data_id=None, **inputs):
    ...
    def _raw_chart_builder():
        if inputs.get('chart_type') == 'heatmap':
            ...
            z = z if chart_type in ZAXIS_CHARTS else None
            axis_inputs = inputs.get('yaxis') or {}
            chart_builder = chart_builder_passthru
            chart_inputs = {k:v for k, v in inputs.items() if k not in ('chart_type',
                                                                        'x', 'y',
                                                                        'z', 'group') if k not in ('chart_type',
                                                                                                   'x',
                                                                                                   'y',
                                                                                                   'z',
                                                                                                   'group')}
...
def export_chart(data_id, params):
    ...
    post_script_css = ('\n').join([
     ...
     "css.type = 'text/css';",
     "css.appendChild(document.createTextNode('div.modebar > div.modebar-group:last-child,div.modebar > div.modebar-group:first-child { display: none; }'));",
     'document.getElementsByTagName("head")[0].appendChild(css);'])
    html_buffer = StringIO()
    config = dict(topojsonURL='')
    write_html(chart, file=html_buffer, include_plotlyjs=True, auto_open=False, post_script=post_script_css, config=config)
    html_buffer.seek(0)
