...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dtale/dash_application/layout.py
# Compiled at: 2020-04-18 12:53:10
import json, dash_core_components as dcc, dash_daq as daq, dash_html_components as html, plotly
from pkg_resources import parse_version
from dtale.charts.utils import YAXIS_CHARTS, ZAXIS_CHARTS, find_group_vals
from dtale.utils import ChartBuildingError, classify_type, dict_merge, flatten_lists, get_dtypes, inner_build_query, make_list
...
def build_input(label, input, className='col-auto', label_class='input-group-addon', **kwargs):
    ...
    return html.Div([
     html.Div([
      html.Span(label, className=label_class), input], className='input-group mr-3')], className=className, **kwargs)
...
CHART_INPUT_SETTINGS = {'line': dict(x=dict(type='single'), y=dict(type='multi'), z=dict(display=False), group=dict(display=True, type='single')), 
   'bar': dict(x=dict(type='single'), y=dict(type='multi'), z=dict(display=False), group=dict(display=True, type='single')), 
   'scatter': dict(x=dict(type='single'), y=dict(type='multi'), z=dict(display=False), group=dict(display=True, type='single')), 
   'pie': dict(x=dict(type='single'), y=dict(type='multi'), z=dict(display=False), group=dict(display=True, type='single')), 
   'wordcloud': dict(x=dict(type='single'), y=dict(type='multi'), z=dict(display=False), group=dict(display=True, type='single')), 
   'heatmap': dict(x=dict(type='single'), y=dict(type='single'), z=dict(display=True, type='single'), group=dict(display=False)), 
   '3d_scatter': dict(x=dict(type='single'), y=dict(type='single'), z=dict(display=True, type='single'), group=dict(display=True)), 
   'surface': dict(x=dict(type='single'), y=dict(type='single'), z=dict(display=True, type='single'), group=dict(display=False)), 
   'maps': dict(x=dict(display=False), y=dict(display=False), z=dict(display=False), group=dict(display=False), map_group=dict(display=True))}
AGGS = dict(count='Count', nunique='Unique Count', sum='Sum', mean='Mean', rolling='Rolling', corr='Correlation', first='First', last='Last', median='Median', min='Minimum', max='Maximum', std='Standard Deviation', var='Variance', mad='Mean Absolute Deviation', prod='Product of All Items', raw='No Aggregation')
FREQS = [
 'H', 'H2', 'WD', 'D', 'W', 'M', 'Q', 'Y']
FREQ_LABELS = dict(H='Hourly', H2='Hour', WD='Weekday', W='Weekly', M='Monthly', Q='Quarterly', Y='Yearly')
MAP_TYPES = [
 dict(value='choropleth'), dict(value='scattergeo', label='ScatterGeo')]
...
def build_proj_hover_children(proj):
    if proj is None:
        ...
        return [
         ...
         html.Div([
          html.Div(proj), html.Img(src=build_img_src(proj))], className='hoverable__content', style=dict(width='auto'))]


def build_proj_hover(proj):
    return html.Span([
     'Projection',
     html.Div(build_proj_hover_children(proj), className='ml-3 hoverable', style=dict(display='none') if proj is None else dict(borderBottom='none'), id='proj-hover')], className='input-group-addon')


LOC_MODE_INFO = {'ISO-3': dict(url=html.A('ISO-3', href='https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3', target='_blank'), examples=[
           'USA (United States)', 'CAN (Canada)', 'GBR (United Kingdom)']), 
   'USA-states': dict(url=html.A('USA-states (use ISO)', href='https://en.wikipedia.org/wiki/List_of_U.S._state_abbreviations', target='_blank'), examples=[
                'NY (New York)', 'CA (California)', 'MA (Massachusetts)']), 
   'country names': dict(url=html.A('country names (case-insensitive)', href='https://simple.wikipedia.org/wiki/List_of_countries', target='_blank'), examples=[
    ...
    if loc_mode is None:
        ...
        return [
         ...
         html.Div([
          ...
          html.Ul([ html.Li(e, style={'listStyleType': 'disc'}, className='mb-3') for e in loc_mode_cfg['examples'] ], className='pt-3 mb-0')], className='hoverable__content build-code', style=dict(width='auto', whiteSpace='nowrap', left='-2em', top='auto'))]


def build_loc_mode_hover(loc_mode):
    return html.Span([
     html.Span('Location Mode', style=dict(whiteSpace='pre-line')),
     html.Div(build_loc_mode_hover_children(loc_mode), className='ml-3 hoverable', style=dict(display='none') if loc_mode is None else dict(borderBottom='none'), id='loc-mode-hover')], className='input-group-addon pt-1 pb-0', style=dict(height='35.5px'))
...
def build_error(error, tb):
    ...
    if isinstance(error, ChartBuildingError):
        ...
        error = error.error
    return html.Div([
     html.I(className='ico-error'), html.Span(str(error)), html.Div(html.Pre(str(tb)), className='traceback')], className='dtale-alert alert alert-danger')
    ...
    for c in cols:
        if classify_type(dtypes[c]) == 'D':
            for freq in FREQS:
                if freq in FREQ_LABELS:
                    yield (
                     ('{}|{}').format(c, freq), ('{} ({})').format(c, FREQ_LABELS[freq]))
                else:
                    yield (
                     c, c)
...
def build_input_options(df, **inputs):
    ...
    return (
     x_options, y_multi_options, y_single_options, z_options, group_options, barsort_options, yaxis_options)
...
def build_map_options(df, type='choropleth', loc=None, lat=None, lon=None, map_val=None):
    ...
    return (
     loc_options, lat_options, lon_options, val_options)
...
def animate_styles(df, **inputs):
    ...
    return (
     dict(display='none'), dict(display='none'), [])
...
def build_group_val_options(df, group_cols):
    ...
    return [ build_option(json.dumps(gv), ('|').join([ str(gv.get(p, 'NaN')) for p in group_cols ])) for gv in group_vals
           ]
    ...
    def _build_hoverable():
        for t in MAP_TYPES:
            yield html.Div([
             ...
             html.Img(src=build_img_src(t['value'], img_type='map_type'))], className='col-md-6')

    return html.Div([
     dcc.Tabs(id='map-type-tabs', value=map_type or 'choropleth', children=[ build_tab(t.get('label', t['value'].capitalize()), t['value']) for t in MAP_TYPES ], style=dict(height='36px')),
     html.Div(html.Div(list(_build_hoverable()), className='row'), className='hoverable__content map-types')], style=dict(paddingLeft=15, borderBottom='none'), className='pr-5 hoverable')
...
def main_inputs_and_group_val_display(inputs):
    ...
    if show_group and not len(make_list(inputs.get('group'))):
        return (dict(display='none'), 'col-md-12')
    return (
     dict(display='block'), 'col-md-8')
...
def charts_layout(df, settings, **inputs):
    ...
    query_label = html.Div([
     html.Span('Query'),
     html.A(html.I(className='fa fa-info-circle ml-4'), href='https://pandas.pydata.org/pandas-docs/stable/user_guide/indexing.html#indexing-query', target='_blank', style={'color': 'white'})], className='input-group-addon', style={'minWidth': '7em'})
    ...
    loc_options, lat_options, lon_options, map_val_options = build_map_options(df, type=map_type, loc=loc, lat=lat, lon=lon, map_val=map_val)
    ...
    return html.Div([
     ...
     dcc.Store(id='input-data', data={k:v for k, v in inputs.items() if k not in ('cpg',
                                                                             'barmode',
                                                                             'barsort') if k not in ('cpg',
                                                                                                     'barmode',
                                                                                                     'barsort')}),
     dcc.Store(id='chart-input-data', data={k:v for k, v in inputs.items() if k in ('cpg',
                                                                               'barmode',
                                                                               'barsort') if k in ('cpg',
                                                                                                   'barmode',
                                                                                                   'barsort')}),
     dcc.Store(id='map-input-data', data={k:v for k, v in inputs.items() if k in ('map_type',
                                                                             'map_code',
                                                                             'lat',
                                                                             'lon',
                                                                             'map_val',
                                                                             'scope',
                                                                             'proj') if k in ('map_type',
                                                                                              'map_code',
                                                                                              'lat',
                                                                                              'lon',
                                                                                              'map_val',
     ...
     html.Div(html.Div(dcc.Tabs(id='chart-tabs', value=chart_type or 'line', children=[ build_tab(t.get('label', t['value'].capitalize()), t['value']) for t in CHARTS ], style=dict(height='36px')), className='col-md-12'), className='row pt-3 pb-3 charts-filters'),
     html.Div(html.Div([
      html.Div([
       query_label,
       dcc.Input(id='query-input', type='text', placeholder=query_placeholder, className='form-control', value=query_value, style={'lineHeight': 'inherit'})], className='input-group mr-3')], className='col'), className='row pt-3 pb-3 charts-filters'),
     html.Div([
      html.Div([
       html.Div([
        build_input([
         html.Div('X'), html.Small('(Agg By)')], dcc.Dropdown(id='x-dropdown', options=x_options, placeholder='Select a column', value=x, style=dict(width='inherit')), label_class='input-group-addon d-block pt-1 pb-0'),
        build_input('Y', dcc.Dropdown(id='y-multi-dropdown', options=y_multi_options, multi=True, placeholder='Select a column(s)', style=dict(width='inherit'), value=y if show_input('y', 'multi') else None), className='col', id='y-multi-input', style=show_style(show_input('y', 'multi'))),
        build_input([
         html.Div('Y'), html.Small('(Agg By)')], dcc.Dropdown(id='y-single-dropdown', options=y_single_options, placeholder='Select a column', style=dict(width='inherit'), value=y[0] if show_input('y') and len(y) else None), className='col', label_class='input-group-addon d-block pt-1 pb-0', id='y-single-input', style=show_style(show_input('y'))),
        build_input('Z', dcc.Dropdown(id='z-dropdown', options=z_options, placeholder='Select a column', style=dict(width='inherit'), value=z), className='col', id='z-input', style=show_style(show_input('z'))),
        build_input('Group', dcc.Dropdown(id='group-dropdown', options=group_options, multi=True, placeholder='Select a group(s)', value=group, style=dict(width='inherit')), className='col', id='group-input', style=show_style(show_input('group')))], id='non-map-inputs', style={} if not show_map else {'display': 'none'}, className='row pt-3 pb-3 charts-filters'),
       html.Div([
        build_map_type_tabs(map_type),
        html.Div([
         html.Div([
          build_loc_mode_hover(loc_mode),
          dcc.Dropdown(id='map-loc-mode-dropdown', options=[ build_option(v) for v in ['ISO-3', 'USA-states', 'country names'] ], style=dict(width='inherit'), value=loc_mode)], className='input-group mr-3')], id='map-loc-mode-input', style=show_map_style(map_type == 'choropleth'), className='col-auto'),
        build_input([
         html.Div('Locations'), html.Small('(Agg By)')], dcc.Dropdown(id='map-loc-dropdown', options=loc_options, placeholder='Select a column', value=loc, style=dict(width='inherit')), id='map-loc-input', label_class='input-group-addon d-block pt-1 pb-0', style=show_map_style(map_type == 'choropleth')),
        build_input([
         html.Div('Lat'), html.Small('(Agg By)')], dcc.Dropdown(id='map-lat-dropdown', options=lat_options, placeholder='Select a column', value=lat, style=dict(width='inherit')), id='map-lat-input', label_class='input-group-addon d-block pt-1 pb-0', style=show_map_style(map_type == 'scattergeo')),
        build_input([
         html.Div('Lon'), html.Small('(Agg By)')], dcc.Dropdown(id='map-lon-dropdown', options=lon_options, placeholder='Select a column', style=dict(width='inherit'), value=lon), id='map-lon-input', label_class='input-group-addon d-block pt-1 pb-0', style=show_map_style(map_type == 'scattergeo')),
        build_input('Scope', dcc.Dropdown(id='map-scope-dropdown', options=[ build_option(v) for v in SCOPES ], style=dict(width='inherit'), value=map_scope or 'world'), id='map-scope-input', style=show_map_style(map_type == 'scattergeo')),
        html.Div([
         html.Div([
          build_proj_hover(proj),
          dcc.Dropdown(id='map-proj-dropdown', options=[ build_option(v) for v in PROJECTIONS ], style=dict(width='inherit'), value=proj)], className='input-group mr-3')], id='map-proj-input', style=show_map_style(map_type == 'scattergeo'), className='col-auto'),
        build_input('Value', dcc.Dropdown(id='map-val-dropdown', options=map_val_options, placeholder='Select a column', style=dict(width='inherit'), value=map_val)),
        build_input('Group', dcc.Dropdown(id='map-group-dropdown', options=group_options, multi=True, placeholder='Select a group(s)', value=inputs.get('map_group'), style=dict(width='inherit')), className='col', id='map-group-input')], id='map-inputs', className='row pt-3 pb-3 charts-filters', style={} if show_map else {'display': 'none'}),
       html.Div([
        build_input('Aggregation', dcc.Dropdown(id='agg-dropdown', options=[ build_option(v, AGGS[v]) for v in ['count', 'nunique', 'sum', 'mean', 'rolling',
         'corr', 'first', 'last', 'median', 'min', 'max',
         'std', 'var', 'mad', 'prod', 'raw']
                                                                   ], placeholder='Select an aggregation', style=dict(width='inherit'), value=agg or 'raw')),
        html.Div([
         build_input('Window', dcc.Input(id='window-input', type='number', placeholder='Enter days', className='form-control text-center', style={'lineHeight': 'inherit'}, value=inputs.get('window'))),
         build_input('Computation', dcc.Dropdown(id='rolling-comp-dropdown', options=[
          ...
          build_option('var', 'Variance')], placeholder='Select an computation', style=dict(width='inherit'), value=inputs.get('rolling_comp')))], id='rolling-inputs', style=show_style(agg == 'rolling'))], className='row pt-3 pb-3 charts-filters'),
       html.Div([
        build_input('Chart Per\nGroup', html.Div(daq.BooleanSwitch(id='cpg-toggle', on=inputs.get('cpg') or False), className='toggle-wrapper'), id='cpg-input', style=show_style(show_cpg), className='col-auto'),
        build_input('Barmode', dcc.Dropdown(id='barmode-dropdown', options=[
         build_option('group', 'Group'),
         build_option('stack', 'Stack'),
         build_option('relative', 'Relative')], value=inputs.get('barmode') or 'group', placeholder='Select a mode'), className='col-auto addon-min-width', style=bar_style, id='barmode-input'),
        build_input('Barsort', dcc.Dropdown(id='barsort-dropdown', options=barsort_options, value=inputs.get('barsort')), className='col-auto addon-min-width', style=bar_style, id='barsort-input'),
        html.Div(html.Div([
         html.Span('Y-Axis', className='input-group-addon'),
         html.Div(dcc.Tabs(id='yaxis-type', value=yaxis_type, children=get_yaxis_type_tabs(y)), id='yaxis-type-div', className='form-control col-auto pt-3', style=yaxis_type_style),
         ...
         dcc.Input(id='yaxis-min-input', type='number', className='form-control col-auto', style={'lineHeight': 'inherit'}),
         html.Span('Max:', className='input-group-addon col-auto', id='yaxis-max-label'),
         dcc.Input(id='yaxis-max-input', type='number', className='form-control col-auto', style={'lineHeight': 'inherit'})], className='input-group', id='yaxis-min-max-options'), className='col-auto addon-min-width', id='yaxis-input', style=show_style(show_yaxis)),
        build_input('Colorscale', dcc.Dropdown(id='colorscale-dropdown', options=[ build_option(o) for o in COLORSCALES ], value=inputs.get('colorscale') or default_cscale), className='col-auto addon-min-width', style=cscale_style, id='colorscale-input'),
        build_input('Animate', html.Div(daq.BooleanSwitch(id='animate-toggle', on=inputs.get('animate') or False), className='toggle-wrapper'), id='animate-input', style=animate_style, className='col-auto'),
        build_input('Animate By', dcc.Dropdown(id='animate-by-dropdown', options=animate_opts, value=inputs.get('animate_by')), className='col-auto addon-min-width', style=animate_style, id='animate-by-input')], className='row pt-3 pb-5 charts-filters')], id='main-inputs', className=main_input_class),
      build_input('Group(s)', dcc.Dropdown(id='group-val-dropdown', multi=True, placeholder='Select a group value(s)', value=group_val, style=dict(width='inherit')), className='col-md-4 pt-3 pb-5', id='group-val-input', style=group_val_style)], className='row'),
     dcc.Loading(html.Div(id='chart-content', style={'height': '69vh'}), type='circle'),
     dcc.Textarea(id='copy-text', style=dict(position='absolute', left='-110%'))], className='charts-body')