...
# [GCC 8.4.0]
# Embedded file name: /Volumes/OSX_Data/Github/tensorflow-macosx/custom_build_tools/build.py
# Compiled at: 2018-06-01 04:49:08
# Size of source mod 2**32: 4454 bytes
import datetime, fcntl, os, subprocess, sys, time
...
def build_bazel():
    bazel = subprocess.Popen('bazel build --config=cuda --config=opt --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" --action_env PATH --action_env LD_LIBRARY_PATH --action_env DYLD_LIBRARY_PATH //tensorflow/tools/pip_package:build_pip_package',
      shell=True,
...
def build_pip():
    pip = subprocess.Popen('bazel-bin/tensorflow/tools/pip_package/build_pip_package /tmp/tensorflow_pkg', shell=True, stdout=(subprocess.PIPE),
      stderr=(subprocess.PIPE),
      bufsize=128)
...
def install_pip(pip_path):
    ...
    install = subprocess.Popen(('%s install %s' % (pip_path, filepath)), shell=True,
      stdout=(subprocess.PIPE),
      stderr=(subprocess.PIPE),
      bufsize=128)
