...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/unicore.ask/unicore/ask/service/models.py
...
class UUIDMixin(object):
    _uuid = Column(UUIDType(binary=False), name='uuid', default=uuid4, primary_key=True)
...
class QuestionResponse(Base, UUIDMixin):
    ...
    question_option_id = Column(UUIDType(binary=False), ForeignKey('question_options.uuid'), nullable=False)
...
class QuestionOption(Base, UUIDMixin):
    ...
    question_id = Column(UUIDType(binary=False), ForeignKey('questions.uuid'), nullable=False)
    responses = relationship(QuestionResponse, backref='question_options', lazy='dynamic')
...
class Question(Base, UUIDMixin):
    ...
    question_types = ('free_text', 'multiple_choice')
