...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/spacy2folia/spacy2folia.py
# Compiled at: 2019-03-23 07:13:29
# Size of source mod 2**32: 4152 bytes
import argparse, spacy, os
...
def convert(doc: spacy.tokens.doc.Doc, document_id: str='untitled', **kwargs) -> folia.Document:
    ...
    for sentence in doc.sents:
        ...
        for i, word in enumerate(tokens):
            text = word.text
            if text == '\n':
                if foliaword is not None:
                    if i < len(tokens) - 1:
                        foliasentence.append(folia.Linebreak)
                        foliaword.space = True
                    else:
                        if paragraphs:
