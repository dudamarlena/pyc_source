...
# [GCC 8.4.0]
# Embedded file name: /Users/fish/Dropbox/h5dj/h5dj/storage.py
# Compiled at: 2012-07-25 02:47:07
from __future__ import with_statement, print_function
from django.core.files.storage import Storage
from django.core.exceptions import SuspiciousOperation
from datetime import datetime
import os, shutil, types, tempfile, h5py
...
class HDF5Storage(Storage):
    ...
    def __init__(self, hdf5_file, base_url=None, repack=False):
        self.base_url = base_url
        self.hdf5_file = os.path.normpath(os.path.abspath(hdf5_file))
        if repack is True:
    ...
    def _repack(self, reopen=True):
        ...
        if not os.path.isfile(self.hdf5_file):
            raise IOError("Can't repack non-existing file %s" % self.hdf5_file)
        tmp = tempfile.mktemp()
        ...
        if not os.path.isfile(tmp_repacked):
            raise IOError('Repack data not saved in temporary file %s' % tmp_repacked)
    ...
    def save(self, name, content):
        ...
        save_data = directory.require_dataset(filename, shape=ncontent.shape, dtype=ncontent.dtype)
    ...
    def path(self, name):
        ...
        if type(name) in (self.group, self.dataset):
            return name
    ...
    def accessed_time(self, name):
        return datetime.fromtimestamp(os.path.getatime(self.path(name).file.filename))

    def created_time(self, name):
        return datetime.fromtimestamp(os.path.getctime(self.path(name).file.filename))

    def modified_time(self, name):
        return datetime.fromtimestamp(os.path.getmtime(self.path(name).file.filename))