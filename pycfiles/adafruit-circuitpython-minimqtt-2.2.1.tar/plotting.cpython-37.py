...
# [GCC 8.4.0]
# Embedded file name: /home/steven/Documents/Projects/radio/EOR/OthersCodes/21cmFAST/21cmFAST/src/py21cmfast/plotting.py
# Compiled at: 2020-01-09 16:07:28
# Size of source mod 2**32: 7273 bytes
...
eor_colour = colors.LinearSegmentedColormap.from_list('EoR', [
 (0, 'white'),
 (0.21, 'yellow'),
 (0.42, 'orange'),
 (0.63, 'red'),
...
def _imshow_slice(cube, slice_axis=-1, slice_index=0, fig=None, ax=None, fig_kw=None, cbar=True, cbar_horizontal=False, rotate=False, cmap='EoR', **imshow_kw):
    """
        ...
        if ax is None:
            ...
            if fig is None:
                ...
                if slice_index >= cube.shape[slice_axis]:
                    raise IndexError('slice_index is too large for that axis (slice_index=%s >= %s' % (
...
def coeval_sliceplot(struct: [outputs._OutputStruct, Coeval], kind: [str, None]=None, **kwargs):
    """
...
def lightcone_sliceplot(lightcone: LightCone, kind: str='brightness_temp', lightcone2: LightCone=None, vertical: bool=False, **kwargs):
    ...
    if slice_axis == 0:
        ...
        extent = (
         0, lightcone.user_params.BOX_LEN) * 2
        if slice_axis == 1:
            xlabel = 'x-axis [Mpc]'
    ...
    if lightcone2 is None:
        fig, ax = _imshow_slice(getattr(lightcone, kind), extent=extent, 
         slice_axis=slice_axis, 
         rotate=not vertical, 
         cbar_horizontal=not vertical, **kwargs)
    else:
