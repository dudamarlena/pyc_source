...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\index\lib\db.py
# Compiled at: 2013-09-26 16:46:50
    ...
    if engine.name == b'sqlite':
        ...
        if filename:
            ...
            if not os.path.isdir(dirname):
                logging.error((b"Невозможно создать директорию '{0}'").format(dirname))
                return
    if not session:
...
def getDbUri(config={}):
    db_uri = config.get(b'db_uri')
    if not db_uri:
        dbtype = config.get(b'dbtype', b'sqlite')
        if dbtype == b'sqlite':
            dbname = config.get(b'dbname', os.path.expanduser(b'~/default.sqlite'))
            db_uri = (b'{0}:///{1}').format(dbtype, dbname)
        elif dbtype == b'mysql':
            dbtype = config.get(b'dbtype', b'mysql')
            dbname = config.get(b'dbname', b'default')
            host = config.get(b'host', b'localhost')
            user = config.get(b'user', b'root')
            passwd = config.get(b'passwd', b'54321')
            db_uri = (b'{0}://{1}:{2}@{3}/{4}').format(dbtype, user, passwd, host, dbname)
    return db_uri
...
def archiveDb(engine):
    if engine.name == b'sqlite':
        ...
        logging.warning((b'Для данного типа БД архивирование не предусмотрено: {0}, пропускаем архивирование!').format(engine.name))
...
def archiveFile(filename):
    if os.path.isfile(filename):
        ...
        basename_new = (b'{0}_{1}{2}').format(root, timestamp, ext)
        dirname_new = os.path.join(dirname, b'backup')
...
def link_objects(*args):
    args = [ i for i in args if i and hasattr(i, b'_sa_class_manager') ]
    if len(args) < 2:
        return
...
def main():
    ...
    dbconfig = dict(db_uri=b'sqlite://')
    initDb(dbconfig, base=Base)
    initlinks(Base)
    print(b'foreign_keys:')
    for key, value in foreign_keys.items():
        print(key, value)

    print(b'foreign_keys_c:')
