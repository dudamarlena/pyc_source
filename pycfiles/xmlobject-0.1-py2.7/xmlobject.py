...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xmlobject/xmlobject.py
# Compiled at: 2012-01-09 20:10:54
"""
...
"""
import sys, os, xml.dom, xml.dom.minidom
...
class XMLFile(object):
    ...
    def __init__(self, **kw):
        ...
        if raw:
            ...
            if not root:
                raise MissingRootTag('No existing content, so must specify root')
...
class XMLNode(object):
    ...
    def __init__(self, parent, node):
        ...
        if isinstance(node, xml.dom.minidom.Text):
            self._type = 'text'
            self._value = node.nodeValue.strip()
            if self._value == '':
    ...
    def __eq__(self, other):
        """Tests whether a given Node equals this node
        """
        if not isinstance(other, XMLNode):
            return False
        else:
            retval = self._children == other._children and self._value == other._value and self._node.nodeName == other._node.nodeName
            return retval

    def __ne__(self, other):
        """Tests whether a given Node *doesn't* equal this node
        """
    ...
    def _addNode(self, child):
        ...
        if isinstance(child, XMLNode):
            ...
            if not parentDict.has_key(nodeName):
                ...
                if isinstance(parentDict[nodeName], XMLNode):
                    parentDict[nodeName] = self.__dict__[nodeName] = [
                     parentDict[nodeName]]
