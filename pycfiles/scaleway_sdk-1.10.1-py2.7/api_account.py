...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/scaleway/apis/api_account.py
...
class AccountAPI(API):
    ...
    def perm_matches(self, request_perm, effective_perm):
        ...
        if request_perm is None:
            ...
            for request_perm_part, effective_perm_part in zip_longest(request_perm_parts, effective_perm_parts):
                if request_perm_part != effective_perm_part and effective_perm_part != '*' and effective_perm_part is not None:
                    return False

            return True

    def get_resources(self, service=None, name=None, resource=None, include_locked=False):
        ...
        if self.auth_token:
            ...
            if self.auth_jwt:
                ...
                return []
            query_params = {'include_locked': include_locked}
            if resource:
            ...
            ret = []
            for eff_service_name, eff_service_perms in response.get('permissions', {}).items():
                if eff_service_name == service or service is None:
                    for eff_perm_name, eff_perm_resources in eff_service_perms.items():
                        if self.perm_matches(name, eff_perm_name):
                            ret.extend([ eff_perm_resource for eff_perm_resource in eff_perm_resources if self.perm_matches(resource, eff_perm_resource)
                                       ])

        return list(set(ret))

    def has_perm(self, service=None, name=None, resource=None, include_locked=False):
        """ Checks if the token has a permission.
        """
        return bool(self.get_resources(service=service, name=name, resource=resource, include_locked=include_locked))

    @ttl_cache(maxsize=10, ttl=60)
    def get_quotas(self, organization):
        """ Gets a list of quotas for the given organization.
        """
        response = self.query().organizations(organization).quotas.get(per_page=100)
