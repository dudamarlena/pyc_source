...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/commonruntime/DataClayRuntime.py
# Compiled at: 2020-02-04 10:42:18
# Size of source mod 2**32: 40160 bytes
""" Class description goes here. """
import importlib, uuid, six, os, logging, traceback
from abc import ABCMeta, abstractmethod
from logging import TRACE
from lru import LRU
from grpc import RpcError
import dataclay.heap.LockerPool as LockerPool
from dataclay.communication.grpc.messages.common.common_messages_pb2 import LANG_PYTHON
from dataclay.paraver import extrae_tracing_is_enabled, initialize_extrae, get_current_available_task_id, finish_tracing, get_task_id
...
class DataClayRuntime(object):
    ...
    def __init__(self):
        ...
        self._DataClayRuntime__initialized = False
    ...
    def is_initialized(self):
        ...
        """
        return self._DataClayRuntime__initialized
    ...
    def get_object_by_id(self, object_id, class_id=None, hint=None):
        ...
        if o is not None:
            return o
        if not class_id:
            full_name, namespace = self.ready_clients['@LM'].get_object_info(self.get_session_id(), object_id)
            self.logger.debug('Trying to import full_name: %s from namespace %s', full_name, namespace)
    ...
    def get_or_new_volatile_instance_and_load(self, object_id, metaclass_id, hint, obj_with_data, ifacebitmaps):
        """
    ...
    def update_object(self, into_object, from_object):
        ...
        ser_from = SerializationLibUtilsSingleton.serialize_params_or_return(params=parameters,
          iface_bitmaps=None,
        ...
        if vol_objects is not None:
            ...
            for tag in vol_objects:
                ...
                ser_from[3][tag] = (
                 new_ids[cur_oid],) + ser_from[3][tag][1:]
    ...
    def run_remote(self, object_id, backend_id, operation_name, value):
        ...
        value = SerializationLibUtilsSingleton.serialize_params_or_return(params=(
         value,),
          iface_bitmaps=None,
          ...
          hint_volatiles=None,
          runtime=self)
        ret = execution_client.ds_execute_implementation(object_id, implementation_id, session_id, value)
        if ret is not None:
    ...
    def call_execute_to_ds(self, instance, parameters, operation_name, exeenv_id, using_hint):
        ...
        session_id = self.get_session_id()
        implementation_id = self.get_implementation_id(object_id, operation_name)
        serialized_params = SerializationLibUtilsSingleton.serialize_params_or_return(params=parameters,
          iface_bitmaps=None,
          params_spec=(operation.params),
          params_order=(operation.paramsOrder),
        ...
        if serialized_params is not None:
            if serialized_params[3] is not None:
        ...
        for _ in range(max_retry):
            try:
                ...
                self.logger.debug('Not found in cache ExecutionEnvironment {%s}! Starting it at %s:%d', exeenv_id, exeenv.hostname, exeenv.port)
                execution_client = EEClient(exeenv.hostname, exeenv.port)
                self.ready_clients[exeenv_id] = execution_client

            try:
                self.logger.verbose('Calling remote EE %s ', exeenv_id)
                ret = execution_client.ds_execute_implementation(object_id, implementation_id, session_id, serialized_params)
                executed = True
                break
                ...
                try:
                    ...
                    if serialized_params is not None:
                        if serialized_params[4] is not None:
    ...
    def synchronize_federated(self, instance, params, operation_name, dc_info_id):
        self.logger.debug('Calling external dataClay to run %s operation in object %s', operation_name, instance.get_object_id())
        ...
        serialized_params = SerializationLibUtilsSingleton.serialize_params_or_return(params=params,
          iface_bitmaps=None,
          params_spec=(operation.params),
          params_order=(operation.paramsOrder),
        ...
        for i in range(0, len(hosts)):
            try:
                ...
                self.logger.debug('[==JUMP==] Request execution to external dataClay %s with host %s and port %s for object %s', dc_info_id, hosts[i], ports[i], instance.get_object_id())
                lm_client.synchronize_federated_object(self.get_dataclay_id(), instance.get_object_id(), implementation_id, serialized_params)
            except:
                traceback.print_exc()
                if i + 1 == len(hosts):
    ...
    def ensure_object_is_registered(self, session_id, object_id, class_id, hint):
        ...
        """
        reg_info = [
         object_id, class_id, session_id, None]
    ...
    def move_object(self, instance, source_backend_id, dest_backend_id, recursive):
        self.logger.debug('Moving object %r from %s to %s', instance, source_backend_id, dest_backend_id)
        object_id = instance.get_object_id()
        moved_objs = self.ready_clients['@LM'].move_object(self.get_session_id(), object_id, source_backend_id, dest_backend_id, recursive)
    ...
    def get_or_new_persistent_instance(self, object_id, metaclass_id, hint):
        ...
        if metaclass_id is None:
            metadata = self.ready_clients['@LM'].get_metadata_by_oid(self.get_session_id(), object_id)
    ...
    def get_metadata(self, object_id):
        if object_id in self.metadata_cache:
            ...
            self.logger.debug('Received the following MetaDataInfo for object %s: %s', object_id, metadata)
        return metadata
    ...
    def get_execution_environments_info(self):
        if self.ee_info_map is None:
            self.ee_info_map = self.ready_clients['@LM'].get_execution_environments_info(self.get_session_id(), LANG_PYTHON, not self.is_exec_env())
