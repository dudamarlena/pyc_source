...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/commonruntime/Initializer.py
# Compiled at: 2019-11-09 12:00:18
...
def _get_logging_dict_config():
    ...
    if file_config:
        ...
        for h in handlers:
            try:
                ...
                logger.debug('Handler %s is using `%s`, formatter from `%s`', h, new_filename, template)
                try:
    ...
    return {'version':1, 
     ...
     'handlers':{'dclay_console': {'class':'logging.StreamHandler', 
                        'level':level, 
                        'formatter':'simple', 
                        'stream':'ext://sys.stdout'}}, 
     'loggers':{'dataclay': {'level':level, 
                   'handlers':[
                    'dclay_console']}}}
