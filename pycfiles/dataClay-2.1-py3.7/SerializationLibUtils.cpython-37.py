...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/serialization/lib/SerializationLibUtils.py
# Compiled at: 2019-10-28 11:50:26
# Size of source mod 2**32: 16866 bytes
""" Class description goes here. """
from io import BytesIO
import logging, dataclay
...
import dataclay.util.IdentityDict as IdentityDict
...
import dataclay.util.ReferenceCounting as ReferenceCounting
...
class SerializationLibUtils(object):
    ...
    def _create_buffer_and_serialize(self, instance, ignore_user_types, ifacebitmaps, cur_serialized_objs, pending_objs, reference_counting, return_none_if_no_ref_counting=False):
        """
    ...
    def serialize_association(self, io_output, element, cur_serialized_objs, pending_objs, reference_counting):
    ...
    def serialize_params_or_return(self, params, iface_bitmaps, params_spec, params_order, hint_volatiles, runtime, recursive=True, for_update=False):
        """
                ...
                if param is not None:
                    ...
                    try:
                        ...
                        if param.is_persistent():
                            ...
                            obj_with_data = self.serialize_dcobj_with_data(param, pending_objects, not recursive, hint_volatiles, runtime, True, for_update)
            ...
            if recursive:
                while pending_objects:
                    ...
                    if pending_obj.is_persistent():
                        ...
                        obj_with_data = self.serialize_dcobj_with_data(pending_obj, pending_objects, not recursive, hint_volatiles, runtime, True, for_update)
                        vol_params[i] = obj_with_data
                    already_serialized_params.add(oid)
                    i += 1
    ...
    def serialize_dcobj_with_data(self, dc_object, pending_objs, ignore_user_types, hint, runtime, force_pending_to_register, for_update=False):
        """
    ...
    def serialize_for_db_gc(self, instance, ignore_user_types, ifacebitmaps, return_none_if_no_ref_counting=False):
        ...
        obj_data = self._create_buffer_and_serialize(instance, ignore_user_types, ifacebitmaps, cur_serialized_objs, pending_objs, reference_counting,
