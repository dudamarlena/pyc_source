...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/util/FileUtils.py
# Compiled at: 2019-11-09 11:48:01
...
def deploy_class(namespace, full_name, source, imports, source_deploy_path, ds_deploy=False):
    ...
    if not os.path.exists(class_path):
        raise IOError('__init__.py file in package %s should have been already initialized' % package)
    else:
        with open(class_path, 'rt') as (f):
            contents = f.readlines()
        contents = contents[:MAGIC_LINE_NUMBER_FOR_IMPORTS] + [imports] + contents[MAGIC_LINE_NUMBER_FOR_IMPORTS:] + [source]
