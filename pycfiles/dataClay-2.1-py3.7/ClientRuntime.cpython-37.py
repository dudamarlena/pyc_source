...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/commonruntime/ClientRuntime.py
...
import dataclay.heap.ClientHeapManager as ClientHeapManager
import dataclay.loader.ClientObjectLoader as ClientObjectLoader
...
class ClientRuntime(DataClayRuntime):
    ...
    def make_persistent(self, instance, alias, backend_id, recursive):
        ...
        if not instance.is_persistent():
            ...
            serialized_objs = SerializationLibUtilsSingleton.serialize_params_or_return(params=parameters,
              iface_bitmaps=None,
            ...
            try:
                ...
                self.logger.debug('Not found in cache ExecutionEnvironment {%s}! Starting it at %s:%d', location, exeenv.hostname, exeenv.port)
        ...
        if alias is not None:
            reg_info = [
             instance.get_object_id(), instance.get_class_extradata().class_id,
            ...
            self.alias_cache[alias] = (
             instance.get_object_id(), instance.get_class_extradata().class_id, location)
        return location
    ...
    def execute_implementation_aux(self, operation_name, instance, parameters, exeenv_id=None):
        ...
        self.logger.debug('Call is being done into %r with #%d parameters', instance, len(parameters))
        using_hint = False
