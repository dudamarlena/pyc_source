...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/communication/grpc/paraver/ParaverClientInterceptor.py
...
class ParaverClientInterceptor(UnaryUnaryClientInterceptor):
    ...
    def intercept_unary_unary(self, continuation, client_call_details, request):
        ...
        if settings.paraver_tracing_enabled:
            ...
            if not client_port:
                client_port = 892892
            self.prv_manager.add_network_send(send_time, TraceType.SEND_REQUEST, int(client_port), req_id, self.remote_hostaddr, self.remote_port, 0, 0)
            self.prv_manager.add_network_receive(self.remote_hostaddr, self.remote_port, req_id, 0)
            return response
