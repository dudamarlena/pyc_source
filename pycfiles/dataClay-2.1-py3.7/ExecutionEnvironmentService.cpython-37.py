...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dataclay/communication/grpc/server/ExecutionEnvironmentService.py
# Compiled at: 2019-10-28 11:50:26
# Size of source mod 2**32: 19197 bytes
""" Class description goes here. """
from io import BytesIO
import logging, traceback, six
if six.PY2:
...
class DataServiceEE(ds.DataServiceServicer):
    ...
    def get_exception_info(self, ex):
        ...
        return common_messages_pb2.ExceptionInfo(isException=True,
    ...
    def newPersistentInstance(self, request, context):
        try:
            ...
            oid = self.client.ds_new_persistent_instance(Utils.get_id(request.sessionID), Utils.get_id(request.classID), Utils.get_id(request.implementationID), iface_bit_maps, params)
            return dataservice_messages_pb2.NewPersistentInstanceResponse(objectID=(Utils.get_msg_options['object'](oid)))
        except Exception as ex:
            try:
                return dataservice_messages_pb2.NewPersistentInstanceResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
    ...
    def storeObjects(self, request, context):
        try:
            ...
            if request.idsWithAlias is not None:
                if len(request.idsWithAlias) > 0:
    ...
    def executeImplementation(self, request, context):
        ...
        try:
            ...
            try:
                traceback.print_exc()
                return dataservice_messages_pb2.ExecuteImplementationResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
    ...
    def getCopyOfObject(self, request, context):
        try:
            result = self.execution_environment.get_copy_of_object(Utils.get_id(request.sessionID), Utils.get_id(request.objectID), request.recursive)
    ...
    def updateObject(self, request, context):
        try:
            self.execution_environment.update_object(Utils.get_id(request.sessionID), Utils.get_id(request.intoObjectID), Utils.get_param_or_return(request.fromObject))
    ...
    def getObjects(self, request, context):
        try:
            ...
            result = self.execution_environment.get_objects(Utils.get_id(request.sessionID), object_ids, request.recursive, request.moving)
            obj_list = []
            for entry in result:
    ...
    def getReferencedObjectsIDs(self, request, context):
        try:
            ...
            result = self.execution_environment.get_referenced_objects_ids(Utils.get_id(request.sessionID), object_ids)
            obj_list = []
            for entry in result:
            ...
            try:
                return dataservice_messages_pb2.GetReferencedObjectIDsResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
    ...
    def getFederatedObjects(self, request, context):
        try:
            ...
            result = self.execution_environment.get_federated_objects(Utils.get_id(request.extDataClayID), object_ids)
            obj_list = []
            for entry in result:
                obj_list.append(Utils.get_obj_with_data_param_or_return(entry))
            ...
            try:
                return dataservice_messages_pb2.GetFederatedObjectsResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
    ...
    def newVersion(self, request, context):
        try:
            result = self.execution_environment.new_version(Utils.get_id(request.sessionID), Utils.get_id(request.objectID), dataclay_yaml_load(request.metadataInfo))
            vers_ids = dict()
            for k, v in result[1].items():
                vers_ids[Utils.prepare_bytes(str(k))] = Utils.prepare_bytes(str(v))

            return dataservice_messages_pb2.NewVersionResponse(objectID=(Utils.get_msg_options['object'](result[0])),
    ...
    def consolidateVersion(self, request, context):
        try:
            self.execution_environment.consolidate_version(Utils.get_id(request.sessionID), dataclay_yaml_load(request.versionInfo))
            return common_messages_pb2.ExceptionInfo()
        except Exception as ex:
            try:
                return self.get_exception_info(ex)
    ...
    def newReplica(self, request, context):
        try:
            result = self.execution_environment.new_replica(Utils.get_id(request.sessionID), Utils.get_id(request.objectID), request.recursive)
            repl_ids_list = []
            for oid in result:
                repl_ids_list.append(Utils.get_msg_options['object'](oid))

            return dataservice_messages_pb2.NewReplicaResponse(replicatedIDs=repl_ids_list)
        except Exception as ex:
            try:
                traceback.print_exc()
    ...
    def moveObjects(self, request, context):
        try:
            result = self.execution_environment.move_objects(Utils.get_id(request.sessionID), Utils.get_id(request.objectID), Utils.get_id(request.destLocID), request.recursive)
            mov_obj_list = []
            for oid in result:
    ...
    def removeObjects(self, request, context):
        try:
            ...
            result = self.client.ds_remove_objects(Utils.get_id(request.sessionID), object_ids, request.recursive, request.moving, Utils.get_id(request.newHint))
            rem_obj = dict()
            ...
            try:
                return dataservice_messages_pb2.RemoveObjectsResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
    ...
    def migrateObjectsToBackends(self, request, context):
        try:
            ...
            return dataservice_messages_pb2.MigrateObjectsResponse(migratedObjs=migr_obj_res,
              nonMigratedObjs=non_migrated_objs_builder)
        except Exception as ex:
            try:
                return dataservice_messages_pb2.MigrateObjectsResponse(excInfo=(self.get_exception_info(ex)))
            finally:
                ex = None
                del ex
