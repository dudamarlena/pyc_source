...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/twyg/geomutils.py
...
def slice_ellipse(x, y, w, h, ystep, rfactor=1.0):
    ...
    if numlines == 1:
        numlines = 2
    xcorr = x - (cx - r * xscale) - w / 2.0
    py = y + (h - (numlines - 1) * ystep) / 2.0
    while numlines:
...
def arcpath(x, y, w, h, a1, da):
    ...
    a2 = a1 + da
    quadstart = int(math.floor(a1 / 90.0))
    quadend = int(math.floor(a2 / 90.0))
    a1 = math.radians(a1)
    a2 = math.radians(a2)
    ...
    if quadstart == quadend:
        ...
        while quad != quadend:
            ...
            points.append(_calc_arc_segment(cx, cy, p1[0], p1[1], p2[0], p2[1]))
            quad += d
...
def round_corner(p1, p2, p3, r):
    ...
    if s1_vert and s2_horiz or s1_horiz or s2_vert:
        if s1_horiz:
            ...
            dx = p3.x > p2.x
            dy = p2.y > p1.y
        if dx == dy and s1_horiz or dx != dy and not s1_horiz:
            da = -90
        else:
            da = 90
        if s1_horiz:
    ...
    return arcpath(c.x - r, c.y - r, 2 * r, 2 * r, math.degrees(sa), math.degrees(da))
