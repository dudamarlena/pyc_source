...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/twyg/config.py
# Compiled at: 2014-03-08 19:01:46
import math, os, re, sys, operator as _operator
...
rules = [
 ...
 Pattern('(level)', '^{([a-zA-Z][a-zA-Z0-9_]*)}'),
 Pattern('(directive)', '^@([a-zA-Z]+)'),
 Pattern('(float)', '^([0-9]+\\.[0-9]*|[0-9]*\\.[0-9]+)'),
 Pattern('(percent)', '^([0-9][0-9]*%)'),
 Pattern('(integer)', '^([0-9][0-9]*)'),
 Pattern('(operator)', '^(\\+|-|\\*|/|\\^|\\(|\\)|\\[|\\]|\\.|,)'),
 Pattern('(name)', '^([a-zA-Z][a-zA-Z0-9_]*)'),
 Pattern('(hexcolor)', '^(#[a-zA-Z0-9]+)'),
 Pattern('(string)', '^"([^"\\\\]*(?:\\\\.[^"\\\\]*)*)"')]
...
def tokenize(config, file=None, flat=False):
    ...
    for line_nr, line in enumerate(lines):
        ...
        while line:
                ...
                if m:
                    ...
                    if id != '(whitespace)' and id != '(comment)':
                        if id == '(operator)':
                            ...
                            if not sym:
                                raise ConfigError("Syntax error: unknown operator: '%s'" % val, file=file, line=linenum(line_nr, flat), col=col)
                        else:
                            sym = symbol_table[id]
                        s = sym()
                        s.value = val
            ...
            if not found:
                raise ConfigError('Syntax error', file=file, line=linenum(line_nr, flat), col=col)
...
def buildconfig(tokens, cwd=None, state='start', config=None, curr=None, curr_section=None, curr_level=None, section_props=False, prev_configs=[]):
    ...
    def isliteral(id):
        return id in ('(operator)', '(float)', '(percent)', '(integer)', '(name)',
                      '(hexcolor)', '(string)')
    ...
    for t in tokens:
        if state == 'start':
            if t.id == '(newline)':
                ...
                raise ConfigError('Configuration must start with a section definition', t)
        elif state == 'section':
            if t.id == '(newline)':
                state = 'in_section'
            else:
                raise ConfigError("Section definition '%s' must be followed by a newline" % curr_section, t)
            ...
            if t.id == '(newline)':
                ...
                if section in config:
                    raise ConfigError("Duplicate section definition '%s'" % section, t)
                state = 'section'
                ...
                level = t.value
                if section_props:
                    raise ConfigError("Invalid level definition '%s' in section '%s':\n\tlevel definitions are not allowed after section level properties" % (
                     level, curr_section), t)
                if level in config[curr_section]:
                    raise ConfigError("Duplicate level name '%s' in section '%s'" % (
                     level, curr_section), t)
                state = 'level'
                curr_level = level
                ...
                raise ConfigError('Property name, level definition or directive expected', t)
        elif state == 'level':
            if t.id == '(newline)':
                state = 'in_section'
            else:
                raise ConfigError("Level definition '%s' in section '%s' must be followed by a newline" % (
                 curr_level, curr_section), t)
        elif state == 'directive':
            if t.id == '(newline)':
                if not value:
                    p = prevtoken
                    raise ConfigError("Missing parameter for directive '%s'" % name, p)
                state = 'in_section'
                ...
                if name == 'include':
                    try:
                        ...
                        if configpath in prev_configs:
                            raise ConfigError("Error while processing '%s' directive:\n\tCircular reference detected when attempting to include '%s'" % (
                             name, configpath), prevtoken)
                        tokens, cwd = _tokenize_file(configpath, flat=False)
                    except IOError as e:
                        raise ConfigError("Error while processing '%s' directive:\n\t%s: '%s'" % (
                         name, e.strerror, e.filename), prevtoken)

                    prev_configs.append(configpath)
                    buildconfig(tokens, cwd, state, config, curr, curr_section, curr_level, section_props, prev_configs)
                elif name == 'copy':
                    level = param
            ...
            if t.id == '(newline)':
                raise ConfigError("Missing property expressions for property '%s'" % name, prevtoken)
            if t.isoperator('['):
                state = 'array'
                value.append(t)
            elif isliteral(t.id):
                ...
                raise ConfigError("Property expressions cannot start with '%s'" % t.value, t)
            ...
            if isliteral(t.id):
                ...
                raise ConfigError("Syntax error in property expressions '%s'" % name, t)
        elif state == 'array':
            if t.id == '(newline)':
                pass
            elif t.isoperator('['):
            ...
            if t.id == '(newline)':
                ...
                raise ConfigError("End of array symbol ']' must be followedby a newline", t)
        prevtoken = t
...
class SymbolBase(object):
    ...
    def led(self, *args):
        raise ConfigError("Syntax error: unknown operator: '%s'" % self.value, self)
...
def isfunction(o):
    return type(o).__name__ in ('function', 'instancemethod', 'builtin_function_or_method')
...
def eval(self):
    ...
    a = getattr(obj, attr)
    if isfunction(a):
        raise ConfigError("'%s' is a method of '%s'; it cannot be used as a property " % (
         attr, self.first.value), self.second)
    return a
...
class Level(object):
    ...
    def __init__(self, levelname, config={}):
        ...
        properties = {'levelDepthMin': (
           ...
           'levelNumChildrenMax': (
                                 NumberProperty, {'min': 0}), 
           'levelOrientation': (
                              EnumProperty, {'values': Level.orientation})}
        self._props = Properties(properties, 'level.twg', config, extra_prop_warning=False)
        self._eval()
    ...
    def selects(self, node, layout):
        ...
        ok = depth >= self.depth_min and depth <= self.depth_max and numchildren >= self.numchildren_min and numchildren <= self.numchildren_max
...
class StringProperty(Property):
    ...
    def _validate(self):
        if type(self.value) not in (str, unicode):
            raise ConfigError("Property '%s' must evaluate to a string" % self.name, self.expr[0])
...
class NumberProperty(Property):
    ...
    def _validate(self):
        if type(self.value) not in (int, float):
            raise ConfigError("Property '%s' must evaluate to a number" % self.name, self.expr)
        if self.min and self.value < self.min:
            raise ConfigError("Number property '%s' must have a value greater than %s" % (
             self.name, self.min), self.expr)
        if self.max and self.value > self.max:
            raise ConfigError("Number property '%s' must have a value less than %s" % (
             self.name, self.max), self.expr)
...
class ColorProperty(Property):
    ...
    def _validate(self):
        if type(self.value).__name__ != 'Color':
            raise ConfigError("Property '%s' must evaluate to a color" % self.name, self.expr)
...
class EnumProperty(Property):
    ...
    def eval(self, vars):
        ...
        if type(n) not in (int, float):
            raise ConfigError("Enum property '%s' must evaluate to a numeric value" % self.name, self.expr)
        n = int(round(n))
        if n < 0 or n >= len(self.values):
            raise ConfigError("Enum property '%s' evaluated to an invalid numeric value: %s" % (
...
class BooleanProperty(Property):
    ...
    def eval(self, vars):
        ...
        n = eval_expr(self.expr, vars)
        if type(n) not in (int, float):
            raise ConfigError("Boolean property '%s' must evaluate to a numeric value" % self.name, self.expr)
...
class Properties(object):
    ...
    def _warn_extra_props(self, config):
        ...
        for p in extra_props:
            token = config[p][0]
            print >> sys.stderr, "Warning: Unknown property '%s' in configuration file '%s' on line %s" % (
             p, token.file, token.line)
...
def get_stylename(configname, config):
    ...
    if not (type(stylename) == str or type(stylename) == unicode):
        raise ConfigError, format_paramvalue_error(configname, STYLE, stylename, str)
    return stylename