...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/twyg/node.py
# Compiled at: 2014-03-07 19:18:54
import math, os
from twyg.common import textwidth, createpath, brightness
from twyg.config import Properties, StringProperty, NumberProperty, ColorProperty, EnumProperty, BooleanProperty, ArrayProperty
from twyg.geom import Vector2
import twyg.geomutils as geom, twyg.textwrap as textwrap
...
class NodeDrawer(object):
    ...
    def draw(self, node):
        ...
        if E('nodeDrawShadow'):
            _ctx.shadow(dx=E('nodeShadowOffsX'), dy=E('nodeShadowOffsY'), blur=E('nodeShadowBlur'), clr=E('nodeShadowColor'))
        self._draw_gradient_shape(node, path, node.fillcolor)
        _ctx.noshadow()
        if E('textDrawShadow'):
            shadowcolor = E('textShadowColor')
            _ctx.fill(shadowcolor)
            self._drawtext(node, node._textxoffs - E('textShadowOffsX'), node._textyoffs - E('textShadowOffsY'))
    ...
    def _draw_gradient_shape(self, node, path, basecolor):
        E = self._eval_func(node)
        if E('drawGradient'):
            _ctx.gradientfill(path, E('gradientBottomColor'), E('gradientTopColor'), type='linear')
        else:
            _ctx.fill(basecolor)
            _ctx.drawpath(path)
    ...
    def _precalc_text(self, node):
        ...
        if self._wraprect:
            node._textlines, node._textlinewidths, node._textrects, node.textwidth, node.textheight = textwrap.wrap_rect(node.label, lineheight, textwidth_func, node.max_text_width)
        else:
            node._textlines, node._textlinewidths, node._textrects, node.textwidth, node.textheight = textwrap.wrap_shape(node.label, lineheight, textwidth_func, self._shapefunc, hyphenate=node.hyphenate, **self._shapefunc_args)
        ...
        if text_align == 'right':
            ...
            if text_align == 'center':
                ...
                if text_align == 'justify':
                    ...
                    if node.isroot():
                        ...
                        alignment = RIGHT if node.direction() == Direction.Left else LEFT
            ...
            if alignment == JUSTIFY:
                ...
                for i, l in enumerate(node._textlines):
                    ...
                    if numspaces:
                        charswidth = node._textlinewidths[i] - numspaces * spacewidth
                        x_spacing = (w - charswidth) / numspaces
                    if (i == 0 or i == len(node._textlines) - 1) and (x_spacing > spacewidth * 7 or len(words) == 1):
                        x_spacing = spacewidth
                        x += (w - node._textlinewidths[i]) / 2.0
                    for w in words:
                        _ctx.text(w, x, y)
                        x += x_spacing + textwidth(_ctx, w, node.fontname, node.fontsize)
    ...
    def _center_text_vertically(self, node):
        ...
        ystart = (node.textheight - textheight) / 2.0
...
class RectNodeDrawer(NodeDrawer):
    ...
    def __init__(self, config={}):
        properties = {'roundness': (
                       NumberProperty, {'min': 0.0, 'max': 1.0}), 
           'cornerRadius': (
                          NumberProperty, {'min': 0.0}), 
           'roundingStyle': (
                           EnumProperty, {'values': ('screen', 'arc')})}
    ...
    def _calc_shape_path(self, node):
        ...
        if style == 'screen':
            r = E('roundness')
            r = min(max(0, r / 2.0), 1)
            return _ctx.rect(0, 0, node.width, node.height, roundness=r, draw=False)
        if style == 'arc':
            r = E('cornerRadius')
            r = min(r, node.height / 2.0)
            r = min(r, node.width / 2.0)
...
class BoxNodeDrawer(NodeDrawer):
    ...
    def __init__(self, config={}):
        ...
        properties = {'boxOrientation': (
                            EnumProperty, {'values': orientation}), 
           'boxDepth': (
                      NumberProperty, {'min': 0.0}), 
           'horizSideColor': (
                            ColorProperty, {}), 
    ...
    def draw(self, node):
        ...
        if drawstroke:
            ...
            outline = [
             ...
             Vector2(cx5, cy5),
             Vector2(cx6, cy6)]
            offs = geom.offset_poly(outline, strokewidth * 0.5)
...
class PolyNodeDrawer(NodeDrawer):
    ...
    def __init__(self, config={}):
        ...
        self._shapefunc_args = {'numSides': int(round(E('numSides'))), 'rotation': E('rotation')}
        return
    ...
    def precalc_node(self, node):
        ...
        cx = r
        cy = node.height / 2
        node._shape_points = geom.calc_regular_polygon_points(cx, cy, r, int(round(E('numSides'))), E('rotation'))
        connection_points = geom.slice_shape(node._shape_points, cy, node.height, node.height)
...
class OvalNodeDrawer(NodeDrawer):
    ...
    def __init__(self, config={}):
        ...
        self._shapefunc_args = {'aspectRatio': E('aspectRatio'), 'maxWidth': E('maxWidth')}
        return
...
_nodedrawer_map = {'rect': RectNodeDrawer, 
   'box': BoxNodeDrawer, 
   'line': LineNodeDrawer, 
   'poly': PolyNodeDrawer, 
   'oval': OvalNodeDrawer, 
