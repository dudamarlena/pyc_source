...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fanart/core.py
# Compiled at: 2019-03-12 04:24:44
# Size of source mod 2**32: 1752 bytes
import requests, fanart
    ...
    def validate(self):
        for attribute_name in ('ws', 'type', 'sort', 'limit'):
            ...
            if attribute not in choices:
                raise RequestFanartError('Not allowed {}: {} [{}]'.format(attribute_name, attribute, ', '.join(choices)))

    def __str__(self):
        return '{base_url}/{ws}/{id}?api_key={apikey}'.format(base_url=(fanart.BASEURL),
          ws=(self._ws),
          id=(self._id),
          apikey=(self._apikey))
