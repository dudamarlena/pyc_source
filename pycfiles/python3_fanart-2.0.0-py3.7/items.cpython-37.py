...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fanart/items.py
...
class ResourceItem(Immutable):
    ...
    @classmethod
    def get(cls, id):
        map = cls.request_cls(apikey=(os.environ.get('FANART_APIKEY')),
          id=id,
          ws=(cls.WS)).response()
    ...
    def json(self, **kw):
        return (json.dumps)(
 self, default=lambda o: dict([(k, v) for k, v in o.__dict__.items() if not k.startswith('_')]), **kw)
