...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/deepdist/deepdist.py
# Compiled at: 2014-08-24 14:01:49
...
def fetch_model(host='localhost:5000'):
    request = urllib2.Request('http://%s/model' % host, headers={'Content-Type': 'application/deepdist'})
...
def send_gradient(gradient, host='localhost:5000'):
    if not gradient:
        return 'EMPTY'
    request = urllib2.Request('http://%s/update' % host, pickle.dumps(gradient, -1), headers={'Content-Type': 'application/deepdist'})
