...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/taar/recommenders/recommendation_manager.py
# Compiled at: 2018-02-23 09:25:42
...
class RecommendationManager:
    __doc__ = 'This class determines which of the set of recommendation\n    engines will actually be used to generate recommendations.'
    LINEAR_RECOMMENDER_ORDER = [
    ...
    def recommend_linear(self, client_info, client_id, limit, extra_data):
        ...
        for r in self.linear_recommenders:
            if r.can_recommend(client_info, extra_data):
                logger.info('Recommender selected', extra={'client_id': client_id, 'recommender': str(r)})
                recommendations = r.recommend(client_info, limit, extra_data)
                if not recommendations:
                    logger.info('No recommendations', extra={'client_id': client_id, 'recommender': str(r)})
                else:
                    logger.info('Recommendations served', extra={'client_id': client_id, 'recommended_addons': str(recommendations)})
                return recommendations

        logger.info('No recommender can recommend addons', extra={'client_id': client_id})
        return []