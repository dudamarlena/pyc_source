...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ann_solo/spectral_library.py
# Compiled at: 2019-07-25 14:21:13
# Size of source mod 2**32: 21109 bytes
import collections, copy, hashlib, json, logging, multiprocessing, os
from typing import Dict
from typing import Iterator
from typing import List
import faiss, numexpr as ne, numpy as np, tqdm
...
class SpectralLibrary:
    ...
    _hyperparameters = [
     'min_mz', 'max_mz', 'bin_size', 'hash_len', 'num_list']
    _ann_filenames = {}
    ...
    def __init__(self, filename: str) -> None:
        ...
        if self._use_gpu:
            ...
            if self._num_probe > 1024:
                logging.warning('Using num_probe=1024 (maximum supported value on the GPU), %d was supplied', self._num_probe)
                self._num_probe = 1024
            if self._num_candidates > 1024:
                logging.warning('Using num_candidates=1024 (maximum supported value on the GPU), %d was supplied', self._num_candidates)
                self._num_candidates = 1024
        self._current_index = (None, None)
        if config.mode == 'ann':
            verify_file_existence = True
            if self._library_reader.is_recreated:
                logging.warning('ANN indexes were created using non-compatible settings')
                verify_file_existence = False
            base_filename = f"{os.path.splitext(filename)[0]}_{self._get_hyperparameter_hash()[:7]}"
            create_ann_charges = []
            ann_charges = [charge for charge, charge_info in self._library_reader.spec_info['charge'].items() if len(charge_info['id']) >= config.num_list]
            for charge in sorted(ann_charges):
                self._ann_filenames[charge] = f"{base_filename}_{charge}.idxann"
                if verify_file_existence:
                    os.path.isfile(self._ann_filenames[charge]) or create_ann_charges.append(charge)
    ...
    def _create_ann_indexes(self, charges: List[int]) -> None:
        ...
        charge_vectors = {charge:np.zeros((len(self._library_reader.spec_info['charge'][charge]['id']), config.hash_len), np.float32) for charge in charges}
        i = {charge:0 for charge in charge_vectors.keys()}
        for lib_spectrum, _ in tqdm.tqdm((self._library_reader.get_all_spectra()),
          desc='Library spectra added',
          leave=False,
          unit='spectra',
          smoothing=0.1):
            charge = lib_spectrum.precursor_charge
            if charge in charge_vectors.keys():
                spectrum_to_vector(process_spectrum(lib_spectrum, True), config.min_mz, config.max_mz, config.bin_size, config.hash_len, True, charge_vectors[charge][i[charge]])
                i[charge] += 1
        ...
        for charge, vectors in charge_vectors.items():
            ...
            ann_index = faiss.IndexIVFFlat(quantizer, config.hash_len, config.num_list, faiss.METRIC_INNER_PRODUCT)
    ...
    def search(self, query_filename: str) -> List[SpectrumSpectrumMatch]:
        ...
        for query_spectrum in tqdm.tqdm((reader.read_mgf(query_filename)),
          desc='Query spectra read', leave=False,
          unit='spectra',
          smoothing=0.7):
        ...
        logging.info('%d spectra identified after the standard search', len(identifications))
        if config.precursor_tolerance_mass_open is not None:
            if config.precursor_tolerance_mode_open is not None:
                for charge, query_spectra_charge in query_spectra.items():
                    query_spectra[charge] = [spectrum for spectrum in query_spectra_charge if spectrum.identifier not in identifications]

                for ssm in self._search_cascade(query_spectra, 'open'):
                    identifications[ssm.identifier] = ssm

                logging.info('%d spectra identified after the open search', len(identifications))
        return list(identifications.values())

    def _search_cascade(self, query_spectra: Dict[(int, List[MsmsSpectrum])], mode: str) -> Iterator[SpectrumSpectrumMatch]:
        """
        ...
        """
        if mode == 'std':
            logging.debug('Identify the query spectra using a standard search (Δm = %s %s)', config.precursor_tolerance_mass, config.precursor_tolerance_mode)
        else:
            if mode == 'open':
                logging.debug('Identify the query spectra using an open search (Δm = %s %s)', config.precursor_tolerance_mass_open, config.precursor_tolerance_mode_open)
        ssms = {}
        batch_size = config.batch_size
        num_spectra = sum([len(q) for q in query_spectra.values()])
        with tqdm.tqdm(desc='Query spectra processed', total=num_spectra, leave=False,
          unit='spectra',
          smoothing=0.1) as (pbar):
            for charge, query_spectra_charge in query_spectra.items():
                for batch_i in range(0, len(query_spectra_charge), batch_size):
                    query_spectra_batch = query_spectra_charge[batch_i:min(batch_i + batch_size, len(query_spectra_charge))]
                    for ssm in self._search_batch(query_spectra_batch, charge, mode):
                        if ssm is not None:
                            if ssm.identifier not in ssms or ssm.search_engine_score > ssms[ssm.identifier].search_engine_score:
                                ssms[ssm.identifier] = ssm
                        pbar.update(1)

        logging.debug('Filter the spectrum—spectrum matches on FDR (threshold = %s)', config.fdr)
        if mode == 'std':
    ...
    def _search_batch(self, query_spectra: List[MsmsSpectrum], charge: int, mode: str) -> Iterator[SpectrumSpectrumMatch]:
        """
        ...
        Iterator[SpectrumSpectrumMatch]
            ...
            spectrum.
        """
        for query_spectrum, library_candidates in zip(query_spectra, self._get_library_candidates(query_spectra, charge, mode)):
            if library_candidates:
                ...
                yield SpectrumSpectrumMatch(query_spectrum,
                  library_match, score, num_candidates=(len(library_candidates)))

    def _get_library_candidates(self, query_spectra: List[MsmsSpectrum], charge: int, mode: str) -> Iterator[List[MsmsSpectrum]]:
        """
        ...
        if config.mode == 'ann':
            if mode == 'open':
                if charge in self._ann_filenames:
                    ann_index = self._get_ann_index(charge)
                    query_vectors = np.zeros((len(query_spectra), config.hash_len), np.float32)
                    for i, query_spectrum in enumerate(query_spectra):
                        spectrum_to_vector(query_spectrum, config.min_mz, config.max_mz, config.bin_size, config.hash_len, True, query_vectors[i])

                    mask = np.zeros_like(candidate_filters)
                    for mask_i, ann_filter in zip(mask, ann_index.search(query_vectors, self._num_candidates)[1]):
