...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ann_solo/writer.py
...
def natural_sort_key(s: str, _nsre: Pattern[AnyStr]=re.compile('([0-9]+)')) -> List[Union[(str, int)]]:
    """
...
def write_mztab(identifications: List[SpectrumSpectrumMatch], filename: str, lib_reader: SpectralLibraryReader) -> None:
    """
    ...
    metadata = [
     ...
     (
      'description', f'Identification results of file "{os.path.split(config.query_filename)[1]}" against spectral library file "{os.path.split(config.spectral_library_filename)[1]}"'),
     (
     ...
     (
      'ms_run[1]-location',
      pathlib.Path(os.path.abspath(config.query_filename)).as_uri()),
     ('ms_run[1]-id_format', '[MS, MS:1000774, multiple peak list nativeID format,]'),
     ('fixed_mod[1]', '[MS, MS:1002453, No fixed modifications searched,]'),
     ('variable_mod[1]', '[MS, MS:1002454, No variable modifications searched,]'),
     (
      'false_discovery_rate', f"[MS, MS:1002350, PSM-level global FDR, {config.fdr}]")]
    config_keys = [
    ...
    with open(filename, 'w') as (f_out):
        ...
        for ssm in sorted(identifications, key=(lambda ssm: natural_sort_key(ssm.identifier))):
            f_out.write('\t'.join([
             'PSM',
             ssm.sequence,
             str(ssm.identifier),
