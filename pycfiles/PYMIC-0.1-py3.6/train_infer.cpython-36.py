...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymic/train_infer/train_infer.py
# Compiled at: 2020-04-15 21:56:05
# Size of source mod 2**32: 19678 bytes
from __future__ import print_function, division
import sys, time, scipy, torch, torchvision, torchvision.transforms as transforms, numpy as np, torch.nn as nn, torch.optim as optim, torch.nn.functional as F
...
class TrainInferAgent(object):
    ...
    def get_stage_dataset_from_config(self, stage):
        if not stage in ('train', 'valid', 'test'):
        ...
        self.transform_list = [get_transform(name, self.config['dataset']) for name in transform_names]
        csv_file = self.config['dataset'].get(stage + '_csv', None)
        dataset = NiftyDataset(root_dir=root_dir, csv_file=csv_file,
          modal_num=modal_num,
    ...
    def create_dataset(self):
        if self.stage == 'train':
            ...
            self.valid_loader = torch.utils.data.DataLoader((self.valid_set), batch_size=batch_size,
              shuffle=False,
              num_workers=(batch_size * 4))
        else:
            ...
            batch_size = 1
            self.test_loder = torch.utils.data.DataLoader((self.test_set), batch_size=batch_size,
              shuffle=False,
    ...
    def create_optimizer(self):
        self.optimizer = get_optimiser(self.config['training']['optimizer'], self.net.parameters(), self.config['training'])
        last_iter = -1
        if self.checkpoint is not None:
            self.optimizer.load_state_dict(self.checkpoint['optimizer_state_dict'])
            last_iter = self.checkpoint['iteration'] - 1
        self.schedule = optim.lr_scheduler.MultiStepLR((self.optimizer), (self.config['training']['lr_milestones']),
