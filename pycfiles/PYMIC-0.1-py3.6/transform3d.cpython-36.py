...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        if isinstance(self.output_size, (list, tuple)):
            ...
            output_size = [self.output_size * input_shape[(i + 1)] / min_edge for i in range(input_dim)]
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        if isinstance(sample['Rescale_origin_shape'], list) or isinstance(sample['Rescale_origin_shape'], tuple):
        ...
        scale = [(origin_shape[1:][i] + 0.0) / input_shape[2:][i] for i in range(origin_dim)]
        scale = [1.0, 1.0] + scale
...
class RandomFlip(object):
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        if isinstance(sample['RandomFlip_Param'], list) or isinstance(sample['RandomFlip_Param'], tuple):
...
class RandomRotate(object):
    ...
    def __call__(self, sample):
        ...
        sample['image'] = self._RandomRotate__apply_transformation(image, transform_param_list, 1)
        sample['RandomRotate_Param'] = json.dumps(transform_param_list)
        if 'label' in sample:
            sample['label'] = self._RandomRotate__apply_transformation(sample['label'], transform_param_list, 0)
        return sample
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        if isinstance(sample['RandomRotate_Param'], list) or isinstance(sample['RandomRotate_Param'], tuple):
        ...
        sample['predict'] = self._RandomRotate__apply_transformation(sample['predict'], transform_param_list, 1)
        return sample
...
class Pad(object):
    ...
    def __call__(self, sample):
        ...
        input_shape = image.shape
        input_dim = len(input_shape) - 1
        if not len(self.output_size) == input_dim:
            raise AssertionError
        else:
            if self.ceil_mode:
                multiple = [int(math.ceil(float(input_shape[(1 + i)]) / self.output_size[i])) for i in range(input_dim)]
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        if isinstance(predict, tuple) or isinstance(predict, list):
            ...
            for predict_i in predict:
                ...
                crop_max = [predict_shape[2:][i] - margin_upper[i] for i in range(len(margin_lower))]
...
class CropWithBoundingBox(object):
    ...
    def __call__(self, sample):
        ...
        if self.start is None:
            if self.output_size is None:
                ...
                crop_min = [int((bb_min[i] + bb_max[i] + 1) / 2) - int(self.output_size[i] / 2) for i in range(input_dim)]
            ...
            if self.output_size is None:
                assert len(self.output_size) == input_dim
                crop_max = [crop_min[i] + bb_max[i] - bb_min[i] for i in range(input_dim)]
            else:
                crop_max = [crop_min[i] + self.output_size[i] for i in range(input_dim)]
            crop_min = [
             0] + crop_min
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        origin_shape is a 4D or 3D vector as saved in __call__()."""
        if isinstance(sample['CropWithBoundingBox_Param'], list) or isinstance(sample['CropWithBoundingBox_Param'], tuple):
        ...
        output_predict = set_ND_volume_roi_with_bounding_box_range(output_predict, crop_min, crop_max, predict)
        sample['predict'] = output_predict
        return sample


class RandomCrop(object):
    __doc__ = 'Randomly crop the input image (shape [C, D, H, W] or [C, H, W]) \n\n    Args:\n        output_size (tuple or list): Desired output size [D, H, W] or [H, W].\n            the output channel is the same as the input channel.\n    '
    ...
    def __init__(self, output_size, fg_focus=False, fg_ratio=0.0, mask_label=None, inverse=True):
        ...
        if mask_label is not None:
            if not isinstance(mask_label, (list, tuple)):
                raise AssertionError
        self.output_size = output_size
        self.inverse = inverse
    ...
    def __call__(self, sample):
        ...
        if self.fg_focus and random.random() < self.fg_ratio:
            ...
            bb_min, bb_max = bb_min[1:], bb_max[1:]
            crop_min = [random.randint(bb_min[i], bb_max[i]) - int(self.output_size[i] / 2) for i in range(input_dim)]
    ...
    def inverse_transform_for_prediction(self, sample):
        ...
        origin_shape is a 4D or 3D vector as saved in __call__()."""
        if isinstance(sample['RandomCrop_Param'], list) or isinstance(sample['RandomCrop_Param'], tuple):
...
class ChannelWiseThreshold(object):
    __doc__ = 'Threshold the image (shape [C, D, H, W] or [C, H, W]) for each channel\n\n    Args:\n        threshold (tuple/list): The threshold value along each channel.\n    '
            ...
            if name == 'RandomFlip':
                ...
                if name == 'RandomRotate':
                    ...
                    if name == 'Pad':
                        ...
                        if name == 'ChannelWiseGammaCorrection':
                            ...
                            if name == 'ChannelWiseNormalize':
                                ...
                                if name == 'ChannelWiseThresholdWithNormalize':
