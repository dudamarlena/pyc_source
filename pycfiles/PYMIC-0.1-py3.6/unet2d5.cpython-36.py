...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymic/net3d/unet2d5.py
# Compiled at: 2020-04-15 21:24:26
# Size of source mod 2**32: 8005 bytes
"""
...
class ConvBlockND(nn.Module):
    __doc__ = 'for 2D and 3D convolutional blocks'
    ...
    def __init__(self, in_channels, out_channels, dim=2, dropout_p=0.0):
        ...
        if not dim == 2:
            ...
            self.dim = dim
            if self.dim == 2:
                self.conv_conv = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1), nn.BatchNorm2d(out_channels), nn.PReLU(), nn.Dropout(dropout_p), nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1), nn.BatchNorm2d(out_channels), nn.PReLU())
            else:
                self.conv_conv = nn.Sequential(nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1), nn.BatchNorm3d(out_channels), nn.PReLU(), nn.Dropout(dropout_p), nn.Conv3d(out_channels, out_channels, kernel_size=3, padding=1), nn.BatchNorm3d(out_channels), nn.PReLU())
...
class DownBlock(nn.Module):
    __doc__ = 'a convolutional block followed by downsampling'
...
class UpBlock(nn.Module):
    __doc__ = 'Upsampling followed by ConConvBlockNDvBlock'

    def __init__(self, in_channels1, in_channels2, out_channels, dim=2, dropout_p=0.0, bilinear=True):
        super(UpBlock, self).__init__()
        self.bilinear = bilinear
        self.dim = dim
        if bilinear:
            if dim == 2:
                self.up = nn.Sequential(nn.Conv2d(in_channels1, in_channels2, kernel_size=1), nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True))
            else:
                self.up = nn.Sequential(nn.Conv3d(in_channels1, in_channels2, kernel_size=1), nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True))
        else:
            if dim == 2:
...
class UNet2D5(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.up1 = UpBlock((self.ft_chns[4]), (self.ft_chns[3]), (self.ft_chns[3]), (self.dims[3]),
          dropout_p=0.0, bilinear=(self.bilinear))
        self.up2 = UpBlock((self.ft_chns[3]), (self.ft_chns[2]), (self.ft_chns[2]), (self.dims[2]),
          dropout_p=0.0, bilinear=(self.bilinear))
        self.up3 = UpBlock((self.ft_chns[2]), (self.ft_chns[1]), (self.ft_chns[1]), (self.dims[1]),
          dropout_p=0.0, bilinear=(self.bilinear))
        self.up4 = UpBlock((self.ft_chns[1]), (self.ft_chns[0]), (self.ft_chns[0]), (self.dims[0]),
          dropout_p=0.0, bilinear=(self.bilinear))
