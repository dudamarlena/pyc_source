...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymic/train_infer/loss.py
# Compiled at: 2020-04-15 21:24:26
# Size of source mod 2**32: 15321 bytes
...
def get_classwise_dice(predict, soft_y):
    ...
    dice_score = (2.0 * intersect + 1e-05) / (y_vol + p_vol + 1e-05)
    return dice_score
...
def noise_robust_dice_loss(predict, soft_y, softmax, p):
    ...
    y_vol = torch.sum(soft_y, dim=0)
    p_vol = torch.sum(predict, dim=0)
    loss = (numerator + 1e-05) / (y_vol + p_vol + 1e-05)
...
def generalized_dice_loss(predict, soft_y, softmax=True):
    ...
    dice_score = (2.0 * intersect + 1e-05) / (vol_sum + 1e-05)
...
def distance_loss(predict, soft_y, lab_distance, softmax=True):
    ...
    avg_dis = (dis_sum + 1e-05) / (vox_sum + 1e-05)
    ...
    def get_loss_of_single_prediction(self, predict, soft_y, lab_dis=None, softmax=True):
        if self.loss_name == 'noise_robust_dice_loss':
            ...
            if self.loss_name == 'distance_loss':
                ...
                if self.loss_name == 'dice_distance_loss':
