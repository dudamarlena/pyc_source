...
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/canossa/output.py
...
import re, os, select
_dimension_pattern = re.compile('\x1b\\[([0-9]+);([0-9]+)R|\x1b\\[8;([0-9]+);([0-9]+)t')
...
class CSIHandlerTrait():
    ...
    def __init__(self):
        self._csi_map = {_pack('m'): self._handle_sgr, 
    ...
    def dispatch_csi(self, context, parameter, intermediate, final):
        if parameter and parameter[0] > 59:
            key = parameter[0]
        else:
            key = 0
...
class ESCHandlerTrait():
    ...
    def __init__(self):
        self._esc_map = {_pack('7'): self._esc_cursorsave, 
...
class Canossa(tff.DefaultHandler, CSIHandlerTrait, ESCHandlerTrait):
    __cpr = False
    dirty = True

    def __init__(self, screen=None, termenc='UTF-8', termprop=None, visibility=False, resized=True):
        """
        >>> from screen import MockScreenWithCursor
        >>> screen = MockScreenWithCursor()
        >>> screen.getyx()
        ...
        if prefix == 93:
            try:
                ...
                try:
                    ...
                    if num:
                        if num[0] == 48 or num[0] == 50:
                            arg = value[pos + 1:]
                            self.screen.settitle(('').join([ unichr(x) for x in arg ]))
                            s = self.screen.gettitle()
                            if s:
                                value = num + [59] + [ ord(x) for x in s ]
                                new_title = ('').join([ unichr(c) for c in value ])
                                return True
        return False
