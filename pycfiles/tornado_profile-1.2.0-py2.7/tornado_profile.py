...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tornado_profile.py
# Compiled at: 2016-11-23 08:33:53
"""Profile a Tornado application via REST."""
import cProfile, logging, pstats, StringIO, tornado.web, yappi
...
class YappiProfileStatsHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if count <= 0:
            count = None
        strip_dirs = str(strip_dirs).lower() not in ('false', 'no', 'none', 'null',
                                                     '0', '')
        if error:
...
class TornadoProfiler(object):
    ...
    def get_routes(self):
        if self.backend == 'yappi':
            ...
            class UpdatedProfilerHandler(YappiProfilerHandler, self.handler_base_class):
                pass

            class UpdatedProfileStatsHandler(YappiProfileStatsHandler, self.handler_base_class):
                pass
        ...
        if self.backend == 'cprofile' or self.backend == 'cProfile':

            class UpdatedProfilerHandler(CProfileHandler, self.handler_base_class):
                pass

            class UpdatedProfileStatsHandler(CProfileStatsHandler, self.handler_base_class):
                pass

            class UpdatedProfileStatsDumpHandler(CProfileStatsDumpHandler, self.handler_base_class):
                pass
