...
# Embedded file name: /home/mezgrman/projects/pyLCD/pylcd/ks0108.py
# Compiled at: 2015-08-08 07:02:10
__doc__ = '\nLibrary for KS0108 compatible graphical LCDs\n'
import json, math, os, re, time, warnings, PyQRNative as qr
...
class Display:
    ...
    def set_display_enable(self, on=True):
        ...
        self.write_value(124 + (int(on) << 7), chip=1, data=False)
    ...
    def set_start_line(self, line=0):
        self.backend.high(self.backend.PIN_CS2)
        self.write_value(3 + (line << 2), chip=1, data=False)
        self.backend.low(self.backend.PIN_CS2)
    ...
    def text(self, text, x, y, size=10, font='/usr/share/fonts/truetype/freefont/FreeSans.ttf', angle=0, clear=False):
        ...
        if truetype:
            ...
            x_min, x_max, y_min, y_max = (
             0, self.display.columns - 1, 0, self.display.rows - 1)
