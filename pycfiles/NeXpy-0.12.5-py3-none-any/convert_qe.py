...
# [GCC 8.4.0]
# Embedded file name: /Users/rosborn/Documents/Computing/Repositories/nexpy/src/nexpy/examples/plugins/chopper/convert_qe.py
# Compiled at: 2016-07-22 15:37:40
...
class ConvertDialog(BaseDialog):
    ...
    def convert_QE(self):
        ...
        for i in range(0, len(pol)):
            p = pol[i]
            Q[i, :] = np.array(np.sqrt((2 * Ei - en - 2 * np.sqrt(Ei * (Ei - en)) * np.cos(p * np.pi / 180.0)) / 2.0721))
            E[i, :] = np.array(en)
        ...
        Qb = NXfield(Qb[:-1] + dQ / 2.0, name='Q')
        Eb = NXfield(Eb[:-1] + dE / 2.0, name='E')
        result = NXdata(I, (Eb, Qb))
        if self.entry.data.nxerrors:
            result.errors = NXfield(err)
        return result

    def round(self, x, prec=2, base=0.05):
