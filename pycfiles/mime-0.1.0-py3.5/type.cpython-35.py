...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/mime/type.py
...
class Type(object):
    ...
    def encoding(self, enc):
        ...
        if enc is None or enc == 'default':
            ...
            if ENCODING_RE.match(enc):
                self._encoding = enc
            else:
                raise TypeError('The encoding must be None, default, base64, 7bit, 8bit, or quoted-printable.')
    ...
    def to_a(self):
        return [
         self.content_type, self.extensions, self.encoding, self.system,
...
class ItemMeta(type):
    ...
    def __getitem__(cls, type_id):
        if isinstance(type_id, Type):
            ...
            if isinstance(type_id, re._pattern_type):
...
class Types(with_metaclass(ItemMeta, object)):
    ...
    @classmethod
    def match(cls, regex):
        return flatten([v for k, v in iteritems(cls.type_variants) if regex.search(k)])
        ...
        for mime_type in types:
            if isinstance(mime_type, Types):
                ...
                if mts and mime_type in mts:
                    Warning('Type %s already registered as a variant of %s.', mime_type, mime_type.simplified)
