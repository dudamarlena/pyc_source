...
# [GCC 8.4.0]
# Embedded file name: /home/mezgrman/projects/pyLEDSign/examples/ledsign/am03127/communication.py
# Compiled at: 2014-05-30 14:55:40
"""
...
class SerialCommunicator(object):
    ...
    def init_comm(self):
        self.device = serial.serial_for_url(self.port, baudrate=self.baudrate, bytesize=self.BYTESIZE, parity=self.PARITY, stopbits=self.STOPBITS, timeout=self.timeout, writeTimeout=self.timeout)
...
class LEDSign(object):
    ...
    def __init__(self, port=None, baudrate=9600, timeout=None, id=1):
        ...
        self.baudrate = baudrate
        self.timeout = timeout
        self.comm = SerialCommunicator(port=port, baudrate=baudrate, timeout=timeout)
    ...
    def set_clock(self, timedata=None):
        ...
        msg = SetClockMessage(year=timedata.strftime('%y'), weekday=timedata.isoweekday(), month=timedata.month, day=timedata.day, hour=timedata.hour, minute=timedata.minute, second=timedata.second)
        return self.send_message(msg)
    ...
    def send_page(self, content, page='A', line=1, lead=EFFECT_SCROLL_LEFT, speed=SPEED_MEDIUM, method=METHOD_NORMAL, wait=2.0, lag=EFFECT_SCROLL_LEFT):
        ...
        msg = SendPageMessage(line=line, page=page.upper(), lead=lead, method=method, wait=wait.upper(), lag=lag, content=content.render())
        return self.send_message(msg)
    ...
    def send_schedule(self, schedule='A', start=None, end=None, pages='A', recurring=False):
        ...
        msg = SendScheduleMessage(schedule=schedule.upper(), startyear=startyear, startmonth=startmonth, startday=startday, starthour=starthour, startminute=startminute, endyear=endyear, endmonth=endmonth, endday=endday, endhour=endhour, endminute=endminute, pages=pages.upper())
        return self.send_message(msg)
    ...
    def delete_page(self, page, line):
        ...
        if type(page) not in (str, unicode):
            page = self._get_page_char(page)
        msg = DeletePageMessage(page=page, line=line)
        return self.send_message(msg)
    ...
    def delete_schedule(self, schedule):
        ...
        if type(schedule) not in (str, unicode):
            schedule = self._get_schedule_char(schedule)
        msg = DeleteScheduleMessage(schedule=schedule)
        return self.send_message(msg)
    ...
    def set_run_page(self, page):
        ...
        if type(page) not in (str, unicode):
            page = self._get_page_char(page)
        msg = SetRunPageMessage(page=page)
        return self.send_message(msg)
    ...
    def set_brightness(self, level):
        ...
        msg = SetBrightnessMessage(level=level)
        return self.send_message(msg)
