...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tgcombine/combine.py
# Compiled at: 2008-05-21 00:38:17
"""
    TGCombine 
    @version: 1.0.4
...
Scriptbasedir = resource_filename(get_package_name(), 'static')
CacheFolder = config.get('combine.cache', None)
JSFolder = config.get('combine.javascript', 'javascript')
CSSFolder = config.get('combine.css', 'css')
CacheTimeOut = int(config.get('combine.timeout', 10))
MinifyMode = config.get('combine.minify', 'all')
ProjectMode = config.get('server.environment', 'development')
JSPack = config.get('combine.pack_javascript', False)
FileType = re.compile('.*?(css|javascript).*')
...
def cssMinify(css):
    ...
    css = cssComment.sub('', cssWhiteSpace.sub('', css))
