...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/eclipsegen_cli/cmd.py
...
class CreateCommand(cli.Application):
    ...
    archiveSuffix = cli.SwitchAttr(names=[
     '-s', '--archive-suffix'],
      argtype=str,
      mandatory=False,
      requires=[
    ...
    oss = cli.SwitchAttr(names=[
     '-o', '--os'],
      argtype=str,
      mandatory=False,
      list=True,
      help=('Operating system to generate Eclipse for. Choose from: {}. Defaults to OS of this computer: {}'.format(', '.join(Os.keys()), Os.get_current().name)))
    archs = cli.SwitchAttr(names=[
     '-h', '--arch'],
      argtype=str,
      mandatory=False,
      list=True,
      help=('Processor architecture to generate Eclipse for. Choose from: {}. Defaults to architecture of this computer: {}'.format(', '.join(Arch.keys()), Arch.get_current().name)))
    extraRepositories = cli.SwitchAttr(names=[
     '-r', '--repo'],
      argtype=str,
      list=True,
      help='Additional repositories to install units from')
    extraInstallUnits = cli.SwitchAttr(names=[
     '-i', '--install'],
      argtype=str,
      list=True,
    ...
    def main(self, destination):
        if os.path.exists(destination):
            ...
            repositories or print('ERROR: no presets or extra repositories were given')
            return 1
        else:
            if not installUnits:
                print('ERROR: no presets or extra units to install were given')
                return 1
            elif len(oss) == 1 and len(archs) == 1:
                eclipseOs = oss[0]
                eclipseArch = archs[0]
                generator = EclipseGenerator(workingDir=(os.getcwd()), destination=destination, os=eclipseOs, arch=eclipseArch, repositories=repositories,
                  installUnits=installUnits,
                  name=(self.name),
                  fixIni=(self.fixIni),
                  addJre=(self.addJre),
                  archive=(self.archive),
                  archiveJreSeparately=(self.archiveJreSeparately),
                ...
                generator = EclipseMultiGenerator(workingDir=(os.getcwd()), destination=destination, oss=oss, archs=archs, repositories=repositories,
                  installUnits=installUnits,
                  name=(self.name),
                  fixIni=(self.fixIni),
                  addJre=(self.addJre),
