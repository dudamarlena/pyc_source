...
# [GCC 8.4.0]
# Embedded file name: /app/datastore_viewer/__init__.py
# Compiled at: 2020-03-28 06:49:24
# Size of source mod 2**32: 2391 bytes
import os, flask, logging, sys
...
class DatastoreViewer:
    ...
    def __init__(self, emulator_host: Optional[str]=None):
        if emulator_host is not None:
            os.environ['DATASTORE_EMULATOR_HOST'] = emulator_host
        if os.environ.get('DATASTORE_EMULATOR_HOST', '') == '':
            raise RuntimeError('Environment variable "DATASTORE_EMULATOR_HOST" is required.')
    ...
    def run(self, host: Optional[str]=None, port: Optional[str]=None, debug: Optional[bool]=None):
        logger.info(f"DatastoreViewer execute with DATASTORE_EMULATOR_HOST = {self._emulator_host}")
        return self._app.run(host=host,
          port=port,
          debug=debug)
