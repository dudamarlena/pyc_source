...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/twiddler/input/plaintext.py
...
def process(element, source):
    ...
    while True:
        ...
        match = m or n
        if match:
            current = handle_text(element, current, i, source[i:match.start()])
    ...
    current = handle_text(element, current, i, source[i:])
    return