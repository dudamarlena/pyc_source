...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/net6mon/sql.py
# Compiled at: 2006-06-26 03:41:09
import os, time
from env import Environment
from pysqlite2 import dbapi2 as sqlite
from net6mon import uuid
from net6mon import utils
_valid_type = [
 'STRING', 'INTEGER', 'REAL', 'TEXT', 'BLOB']
...
class SQLMapper(object):
    ...
    def __new__(self, dbname, table_name, instance_name, hostid=''):
        ...
        obj._schema = [{'timestamp': 'INTEGER'}, {'instance': 'STRING'}, {'hostid': 'STRING'}]
        obj._con = None
        obj._cur = None
        obj._db = dbname
    ...
    def string_col(self, name):
        ...
        if not self._col_exists_in_schema(name):
            self._schema.append({name: 'STRING'})
    ...
    def int_col(self, name):
        ...
        """
        if not self._col_exists_in_schema(name):
            self._schema.append({name: 'INTEGER'})
    ...
    def insert(self, **values):
        ...
        for i in self._schema:
            cur_col = i.keys()[0]
            c.append(cur_col)
            type = self._get_col_type(cur_col)
            if cur_col in values.keys():
                v.append(values[cur_col])
            elif cur_col == 'timestamp':
                v.append(utils.get_timestamp())
            elif cur_col == 'hostid':
                v.append(self._hostid)
            elif cur_col == 'instance':
                v.append(self._instance)
            else:
                v.append(None)

        print 'cols = %s, vals = %s' % (c, v)
        idx = 0
        cols = ''
        vals = ''
        for col in c:
            coltype = self._get_col_type(col)
            if coltype in ['INTEGER', 'REAL']:
                if v[idx] is None:
                    vals = vals + ' 0 '
                else:
                    vals = vals + ' %s ' % v[idx]
            elif coltype in ['STRING', 'TEXT']:
                str = '%s' % v[idx]
                str = str.replace("'", "''")
                vals = vals + " '%s'" % str
        ...
        query = 'insert into %s (%s) values (%s)' % (self._table, cols, vals)
        self.connect()
        try:
            self._cur.execute(query)
        except sqlite.OperationalError:
