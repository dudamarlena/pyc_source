...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/trainers/scipy_trainer.py
# Compiled at: 2015-01-27 21:45:46
from trainer import SGDTrainer, NeuralTrainer
import logging, theano, numpy as np, theano.tensor as T, scipy
...
class ScipyTrainer(NeuralTrainer):
    ...
    def train(self, train_set, valid_set=None, **kwargs):
        ...
        def display(x):
            ...
            cost_desc = (' ').join('%s=%.2f' % el for el in zip(self.cost_names, costs))
        ...
        for i in range(self.iterations):
            ...
            try:
                res = scipy.optimize.minimize(fun=self.function_at, jac=self.gradient_at, x0=self.arrays_to_flat(self.best_params), args=(
                 train_set,), method=self.method, callback=display, options=dict(maxiter=self.validation_frequency))
            except KeyboardInterrupt:
                logging.info('interrupted!')
                break
