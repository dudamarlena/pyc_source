...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/networks/lstm.py
# Compiled at: 2015-01-20 20:39:45
...
class LSTMLayer(NeuralLayer):
    ...
    def __init__(self, size, target_size=-1, activation='tanh', noise=0.0, dropouts=0.0, update_h0=True, beta=0, optimization='SGD'):
        """
    ...
    def _lstm_step(self, x_t, h_p, c_p):
        i_t_preact = T.dot(x_t, self.W_xi) + T.dot(h_p, self.W_hi) + T.dot(c_p, self.W_ci)
        i_t = self._sigmoid(i_t_preact)
        f_t_preact = T.dot(x_t, self.W_xf) + T.dot(h_p, self.W_hf) + T.dot(c_p, self.W_cf)
        f_t = self._sigmoid(f_t_preact)
        c_t_preact = T.dot(x_t, self.W_xc) + T.dot(h_p, self.W_hc)
        tanh_c_t_right = self._tanh(c_t_preact)
        c_t = f_t * c_p + i_t * tanh_c_t_right
        o_t_preact = T.dot(x_t, self.W_xo) + T.dot(h_p, self.W_ho) + T.dot(c_t, self.W_co)
        ...
        return (
         s_t, h_t, c_t, o_t, f_t, i_t,
         i_t_preact, f_t_preact, c_t_preact, o_t_preact, s_t_preact, tanh_c_t, tanh_c_t_right, h_p, c_p)

    def _lstm_full_gradient_step(self, i, g_h, g_c, xs, ks, s_ts, h_ts, c_ts, o_ts, f_ts, i_ts, i_t_preacts, f_t_preacts, c_t_preacts, o_t_preacts, s_t_preacts, tanh_c_ts, tanh_c_t_rights, h_ps, c_ps):
        ...
        g_o_t = T.dot(g_s_t_preact, T.transpose(self.W_os))
        ...
        return (
         g_h_p, g_c_p, g_wxi, g_whi, g_wci, g_wxf, g_whf, g_wcf, g_wxc, g_whc, g_wxo, g_who, g_wco, g_wos)
    ...
    def _build_gradient_func(self):
        ...
        self._softmax_grad = lambda preact, act: SRG(self._softmax_grad_t, {self._softmax_grad_t: act, self._preact_t: preact})

    def _recurrent_func(self):
        recurrent_vars, _ = theano.scan(fn=self._lstm_step, sequences=[self.x], outputs_info=[
         None, self.h0, self.c0] + [None] * 12)
        self._build_gradient_func()
        gradient_vars, _ = theano.scan(fn=self._lstm_full_gradient_step, sequences=[
         T.arange(self.x.shape[0] - 1, -1, -1)], outputs_info=[
         self.init_h, self.init_c] + [ None for _ in self.params ], non_sequences=[
         self.x, self._vars.k] + recurrent_vars)
        gradient_vars = gradient_vars[2:]
        gradients = [ T.mean(g, axis=0) for g in gradient_vars ]
        updates = optimize_parameters(self.params, gradients, lr=self.learning_rate, method=self.optimization, beta=self.beta, weight_l2=self.weight_l2, clip=True)
        return (
         recurrent_vars[:3], updates)
    ...
    def _setup_params(self):
        ...
        self.B_i = self.create_bias(self.output_n, 'i', value=0.0)
        self.B_c = self.create_bias(self.output_n, 'c', value=0.0)
        self.B_o = self.create_bias(self.output_n, 'o', value=0.0)
        self.W = []
        self.params = [
