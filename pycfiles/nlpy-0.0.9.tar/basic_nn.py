...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/networks/basic_nn.py
# Compiled at: 2015-01-15 19:40:15
import logging as loggers, theano.tensor as T, theano, gzip, cPickle as pickle
...
class NeuralNetwork(object):
    ...
    def setup_layers(self):
        ...
        parameter_count = 0
        z = nnprocessors.add_noise(self.vars.x, self.config.input_noise, self.config.input_dropouts)
    ...
    def monitors(self):
        ...
        for i, h in enumerate(self.hiddens):
            yield (
             ('h{}<0.1').format(i + 1), 100 * (abs(h) < 0.1).mean())
    ...
    def _compile(self):
        if getattr(self, '_compute', None) is None:
            self._compute = theano.function([ x for x in self.inputs if x not in self.target_inputs ], self.hiddens + [self.vars.y], updates=self.updates, allow_input_downcast=True)
        return
    ...
    def J(self, train_conf):
        ...
        if train_conf.contractive_l2 > 0:
            cost += train_conf.contractive_l2 * sum(T.sqr(T.grad(h.mean(axis=0).sum(), self.vars.x)).sum() for h in self.hiddens)
        return cost
