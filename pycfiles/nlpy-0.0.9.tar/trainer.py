...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/trainers/trainer.py
# Compiled at: 2015-02-06 20:21:29
import itertools, logging as loggers, numpy as np, numpy.random as rng, scipy.optimize, theano, theano.tensor as T
from nlpy.deep.conf import TrainerConfig
import sys, gzip, cPickle as pickle
...
class NeuralTrainer(object):
    ...
    def __init__(self, network, config=None):
        ...
        self.evaluation_func = theano.function(network.inputs, self.ev_cost_exprs, updates=network.updates, allow_input_downcast=True, mode=theano.Mode(linker=THEANO_LINKER))
    ...
    def test(self, iteration, test_set):
        costs = list(zip(self.ev_cost_names, np.mean([ self.evaluation_func(*x) for x in test_set ], axis=0)))
        info = (' ').join('%s=%.2f' % el for el in costs)
        logging.info('test    (iter=%i) %s', iteration + 1, info)

    def evaluate(self, iteration, valid_set):
        costs = list(zip(self.ev_cost_names, np.mean([ self.evaluation_func(*x) for x in valid_set ], axis=0)))
        marker = ''
        _, J = costs[0]
...
class SGDTrainer(NeuralTrainer):
    ...
    def __init__(self, network, config=None):
        ...
        self.learning_func = theano.function(network.inputs, self.cost_exprs, updates=update_list, allow_input_downcast=True, mode=theano.Mode(linker=THEANO_LINKER))
...
class RmspropTrainer(SGDTrainer):
    ...
    def learning_updates(self):
        for param in self.params:
            ...
            yield (rms_, rms)
            yield (param, param - self.learning_rate * grad / T.sqrt(rms + 1e-08))