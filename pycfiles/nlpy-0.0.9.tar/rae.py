...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/tasks/paraphrase/rae.py
# Compiled at: 2015-01-21 23:28:07
import time, os, sys
...
from nlpy.deep.functions import FLOATX, monitor_var_sum as MVS, plot_hinton, make_float_vectors, replace_graph as RG, monitor_var as MV, smart_replace_graph as SRG
...
import copy, numpy as np, random as rnd, cPickle as pickle, theano, theano.tensor as T
...
class TreeRAELayer(NeuralLayer):
    ...
    def __init__(self, size, activation='tanh', noise=0.0, dropouts=0.0, beta=0.0, optimization='ADAGRAD', unfolding=True, additional_h=False, max_reg=4, deep=False, batch_size=10, realtime_update=True):
        """
    ...
    def _recursive_func(self):
        ...
        (reps, inter_reps, left_subreps, right_subreps, _, rep_gradients, distances), decoding_updates = theano.scan(self._recursive_step, sequences=[T.arange(seq_len)], outputs_info=[
         None, None, None, None, self.init_registers, None, None], non_sequences=[
         self.x, self._vars.seq, self._vars.back_routes, self._vars.back_lens])
        self.learning_updates.extend(decoding_updates.items())
        (g_we1s, g_we2s, g_bes, g_wee, g_bee, _), _ = theano.scan(self._bpts_step, sequences=[
         T.arange(seq_len - 1, -1, -1)], outputs_info=[
         None, None, None, None, None, self.init_registers], non_sequences=[
         self._vars.seq, reps, inter_reps, left_subreps, right_subreps,
         rep_gradients])
        optimize_params = [
         self.W_e1, self.W_e2, self.B_e]
        ...
        encoding_updates = self.encode_optimizer.setup(optimize_params, optimize_gradients, method=self.optimization, beta=self.beta, count=seq_len)
    ...
    def _bpts_step(self, i, gradient_reg, seqs, reps, inter_reps, left_subreps, right_subreps, rep_gradients):
        ...
        return (
         g_we1, g_we2, g_be, g_wee, g_bee, gradient_reg)
    ...
    def _recursive_step(self, i, regs, tokens, seqs, back_routes, back_lens):
        ...
        gf_W_d1, gf_W_d2, gf_B_d1, gf_B_d2, distance, rep_gradient = self._unfold_gradients(back_reps, lefts, rights, back_routes[i], tokens, back_len)
        return (
         [
          rep, inter_rep, left_rep, right_rep, new_regs, rep_gradient, distance],
    ...
    def _unfold(self, back_route, regs, n, rep=None):
        ...
        (reps, lefts, rights, _), _ = theano.scan(self._decode_step, sequences=[back_route], outputs_info=[
         None, None, None, regs], n_steps=n)
        return (
         reps, lefts, rights)
    ...
    def _decode_step(self, seq, regs):
        ...
        return (
         rep, left_dec, right_dec, regs)

    def _unfold_gradients(self, reps, left_decs, right_decs, back_route, tokens, n):
        (regs, g_wd1s, g_wd2s, g_bd1s, g_bd2s, distances), _ = theano.scan(self._unfold_gradients_step, sequences=[T.arange(n - 1, -1, -1)], outputs_info=[
         self.init_registers, None, None, None, None, None], non_sequences=[
         reps, left_decs, right_decs, back_route, tokens])
        target_reg = back_route[0][2]
        rep_gradient = regs[(-1)][target_reg]
        return (
         T.mean(g_wd1s, axis=0), T.mean(g_wd2s, axis=0), T.mean(g_bd1s, axis=0), T.mean(g_bd2s, axis=0), T.sum(distances), rep_gradient)
    ...
    def _unfold_gradients_func(self, rep, dec, g_dec, target_tok, tok, w, b, unfold_idx=0):
        ...
        return (
         g_dec_rep, g_dec_w, g_dec_b, output_distance)

    def _unfold_gradients_step(self, i, regs, reps, left_decs, right_decs, route, tokens):
        left, right, target = route[i][0], route[i][1], route[i][2]
        g_left_rep, g_wd1, g_bd1, left_distance = self._unfold_gradients_func(reps[i], left_decs[i], regs[left], tokens[(-left)], left, self.W_d1, self.B_d1, 0)
        g_right_rep, g_wd2, g_bd2, right_distance = self._unfold_gradients_func(reps[i], right_decs[i], regs[right], tokens[(-right)], right, self.W_d2, self.B_d2, 1)
        new_regs = T.set_subtensor(regs[target], g_left_rep + g_right_rep)
        distance = left_distance + right_distance
        return (
         new_regs, g_wd1, g_wd2, g_bd1, g_bd2, distance)

    def encode_func(self):
        seq_len = self._vars.seq.shape[0]
        (reps, _), _ = theano.scan(self._encode_step, sequences=[T.arange(seq_len)], outputs_info=[
         None, self.init_registers], non_sequences=[
         self.x, self._vars.seq])
        return reps
    ...
    def get_train_data(self):
        ...
        for j in xrange(len(idx_seq)):
            ...
            if self._train_data[i]['max_reg'] > self.max_reg:
                continue
            yield (
             self._train_data[i]['token_data'], self._train_data[i]['sequence'],
    ...
    def get_valid_data(self):
        ...
        for i in idx_seq:
            if self._valid_data[i]['max_reg'] > self.max_reg:
                continue
            yield (
             self._valid_data[i]['token_data'], self._valid_data[i]['sequence'],
...
if __name__ == '__main__':
    ...
    net_conf = NetworkConfig(input_size=300)
    net_conf.layers = [
     TreeRAELayer(size=300, deep=True, beta=1e-05, optimization='FINETUNING_ADAGRAD', batch_size=10, realtime_update=False)]
    trainer_conf = TrainerConfig()
    trainer_conf.learning_rate = 0.01
    trainer_conf.weight_l2 = 0.0001
    trainer_conf.hidden_l2 = 0.0001
