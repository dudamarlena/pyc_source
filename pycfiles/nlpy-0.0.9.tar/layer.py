...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/networks/layer.py
# Compiled at: 2015-02-03 22:54:28
...
import logging as loggers, math
logging = loggers.getLogger(__name__)

class NeuralLayer(object):

    def __init__(self, size, activation='tanh', noise=0.0, dropouts=0.0, shared_bias=None, disable_bias=True):
        """
    ...
    def _setup_functions(self):
        ...
        bias = (self.shared_bias or self).B if 1 else self._vars.get(self.shared_bias)
        ...
        self.output_func = nnprocessors.add_noise(self._activation_func(self.preact_func), self.noise, self.dropouts)
    ...
    def create_bias(self, output_n=1, suffix='', value=0.0, shape=None):
        if not shape:
            shape = (
             output_n,)
        bs = np.ones(shape)
