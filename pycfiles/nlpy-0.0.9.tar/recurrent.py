...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/networks/recurrent.py
# Compiled at: 2014-12-25 22:59:22
...
class RecurrentLayer(NeuralLayer):
    ...
    def __init__(self, size, target_size=-1, activation='sigmoid', noise=0.0, dropouts=0.0, update_h0=True, bptt=True, bptt_steps=5, beta=1e-07, optimization='ADAGRAD'):
        """
    ...
    def _recurrent_func(self):
        ...
        def recurrent_step(x_t, k_t, h_t):
            ...
            stepping_updates = optimize_parameters([self.W_s], [T.grad(_cost, self.W_s)], method=self.optimization, lr=self.learning_rate, beta=self.beta)
            if not self.bptt:
                stepping_updates += optimize_parameters([self.W_i, self.W_r], [T.grad(_cost, self.W_i), T.grad(_cost, self.W_r)], method=self.optimization, lr=self.learning_rate, beta=self.beta)
        ...
        (h_list, s_list, h_errs), updates = theano.scan(fn=recurrent_step, sequences=[self.x, self._vars.k], outputs_info=[
         self.h0, None, None], n_steps=self.x.shape[0], truncate_gradient=self.bptt_steps)
        ...
        def bptt_step(i, wi, wr, bptt_error, xs, h_list, h_errs):
            ...
            node_map = {_h: h_list[i], _r: ifelse(T.eq(i, 0), self.h0, h_list[(i - 1)]), _x: xs[i], 
               _wi: self.W_i, _wr: self.W_r}
            ...
            w_updates = OrderedDict(optimize_parameters([wi, wr], [g_wi, g_wr], shapes=[self.W_i, self.W_r], method=self.optimization, lr=self.learning_rate, beta=self.beta))
            wi_out = w_updates[wi]
            wr_out = w_updates[wr]
            del w_updates[wr]
            del w_updates[wi]
            ...
            return (
             [
              wi_out, wr_out, bptt_error], w_updates)

        (wi_out, wr_out, _), bptt_updates = theano.scan(fn=bptt_step, sequences=[T.arange(self.x.shape[0] - 1, -1, -1)], non_sequences=[
         self.x, h_list, h_errs], outputs_info=[self.W_i, self.W_r, self.zero_vector])
        if self.bptt:
    ...
    def _predict_func(self):
        ...
        (h_list, s_list), _ = theano.scan(fn=predict_step, sequences=[self.x], outputs_info=[self.h0, None], n_steps=self.x.shape[0])
    ...
    def _setup_params(self):
        ...
        self.B = []
        self.params = [
         self.W_i, self.W_r, self.W_s]
        if not self.disable_bias:
            self.params += [self.B_r, self.B_s]

    def create_params(self, input_n, output_n, suffix, sparse=None):
        ws = np.asarray(global_rand.uniform(low=-np.sqrt(6.0 / (input_n + output_n)), high=np.sqrt(6.0 / (input_n + output_n)), size=(
         input_n, output_n)))
