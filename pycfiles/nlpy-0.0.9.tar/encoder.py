...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/tasks/paraphrase/encoder.py
# Compiled at: 2015-01-22 02:40:34
import numpy as np
...
class ParaphraseEncoder(object):
    ...
    def __init__(self, network, vec=None, parser=None, pooling_size=15, regs_allowed=5):
        ...
        self._parser = (parser or StanfordCFGParser)() if 1 else parser
    ...
    def dynamic_pool(self, reps1, reps2):
        ...
        for i in range(self.pooling_size):
            for j in range(self.pooling_size):
                min_val = self._min_block(sim_matrix, int(i * h_span), int((i + 1) * h_span), int(j * v_span), int((j + 1) * v_span))
