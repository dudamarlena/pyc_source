...
# [GCC 8.4.0]
# Embedded file name: /home/hadoop/nlpy/nlpy/deep/networks/recursive.py
# Compiled at: 2015-01-27 02:21:55
...
class RAELayer(NeuralLayer):
    ...
    def __init__(self, size, target_size=-1, activation='tanh', noise=0.0, dropouts=0.0, beta=1e-07, optimization='ADAGRAD', unfolding=True, additional_h=True):
        """
    ...
    def _recursive_func(self):
        if self.additional_h:
            ...
            h0 = self.x[0]
            start_index = 1
        (reps, distances), _ = theano.scan(self._recursive_step, sequences=[T.arange(start_index, self.x.shape[0])], outputs_info=[
         h0, None], non_sequences=[self.x])
    ...
    def encode_func(self):
        if self.additional_h:
            ...
            h0 = self.x[0]
            start_index = 1
        (reps, _), _ = theano.scan(self._recursive_step, sequences=[T.arange(start_index, self.x.shape[0])], outputs_info=[
         h0, None], non_sequences=[self.x])
        return reps[(-1)]
