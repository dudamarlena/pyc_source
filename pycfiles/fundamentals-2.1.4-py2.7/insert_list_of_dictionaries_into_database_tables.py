...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fundamentals/mysql/insert_list_of_dictionaries_into_database_tables.py
# Compiled at: 2020-04-28 07:11:09
"""
...
def insert_list_of_dictionaries_into_database_tables(dbConn, log, dictList, dbTableName, uniqueKeyList=[], dateModified=False, dateCreated=True, batchSize=2500, replace=False, dbSettings=False):
    ...
    **Usage**
    ...
    global sharedList
    global totalCount
    log.debug('starting the ````insert_list_of_dictionaries_into_database_tables`` function')
    ...
    if len(dictList) == 0:
        ...
        if len(dictList):
            convert_dictionary_to_mysql_table(dbConn=dbConn, log=log, dictionary=dictList[0], dbTableName=dbTableName, uniqueKeyList=uniqueKeyList, dateModified=dateModified, reDatetime=reDate, replace=replace, dateCreated=dateCreated)
            dictList = dictList[1:]
        ...
        if len(dictList):
            ...
            if dbSettings == False:
                fmultiprocess(log=log, function=_insert_single_batch_into_database, inputArray=list(range(len(sharedList))), dbTableName=dbTableName, uniqueKeyList=uniqueKeyList, dateModified=dateModified, replace=replace, batchSize=batchSize, reDatetime=reDate, dateCreated=dateCreated)
            else:
                fmultiprocess(log=log, function=_add_dictlist_to_database_via_load_in_file, inputArray=list(range(len(sharedList))), dbTablename=dbTableName, dbSettings=dbSettings, dateModified=dateModified)
            sys.stdout.write('\x1b[1A\x1b[2K')
            print('%(ltotalCount)s / %(ltotalCount)s rows inserted into %(dbTableName)s' % locals())
        log.debug('completed the ``insert_list_of_dictionaries_into_database_tables`` function')
        return


def _insert_single_batch_into_database(batchIndex, log, dbTableName, uniqueKeyList, dateModified, replace, batchSize, reDatetime, dateCreated):
    """*summary of function*
    ...
    **Usage**
    ...
    while inserted == False:
        ...
        uniKeys = tmp
        myKeys = ('`,`').join(uniKeys)
        vals = [ tuple([ None if d[k] in ('None', None) else d[k] for k in uniKeys ]) for d in batch[0]
               ]
        ...
        message = ''
        try:
            message = writequery(log=log, sqlQuery=insertCommand, dbConn=dbConn, Force=True, manyValueList=vals)
        except:
            theseInserts = []
            for aDict in batch[0]:
                insertCommand, valueTuple = convert_dictionary_to_mysql_table(dbConn=dbConn, log=log, dictionary=aDict, dbTableName=dbTableName, uniqueKeyList=uniqueKeyList, dateModified=dateModified, returnInsertOnly=True, replace=replace, reDatetime=reDate, skipChecks=True)
                theseInserts.append(valueTuple)

            message = ''
            message = writequery(log=log, sqlQuery=insertCommand, dbConn=dbConn, Force=True, manyValueList=theseInserts)

        if message == 'unknown column':
            for aDict in batch:
                convert_dictionary_to_mysql_table(dbConn=dbConn, log=log, dictionary=aDict, dbTableName=dbTableName, uniqueKeyList=uniqueKeyList, dateModified=dateModified, reDatetime=reDate, replace=replace)
...
def _add_dictlist_to_database_via_load_in_file(masterListIndex, dbTablename, dbSettings, dateModified=False):
