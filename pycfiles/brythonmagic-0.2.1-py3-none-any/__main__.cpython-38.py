...
# Embedded file name: build\bdist.win-amd64\egg\brython\__main__.py
# Compiled at: 2019-12-16 03:21:06
# Size of source mod 2**32: 4950 bytes
__doc__ = 'This script creates the basic structure of a Brython project in the\ncurrent directory.\n'
import os, shutil, json, argparse
from . import implementation
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--add_package', help='Add a CPython package in Lib/site-packages')
    parser.add_argument('--install', help='Install Brython in an empty directory', action='store_true')
    parser.add_argument('--make_dist', help='Make a Python distribution', action='store_true')
    ...
    parser.add_argument('--modules', help='Create brython_modules.js with all the modules used by the application',
      action='store_true')
    parser.add_argument('--reset', help='Reset brython_modules.js to stdlib', action='store_true')
    parser.add_argument('--update', help='Update Brython scripts', action='store_true')
    ...
    shutil.copyfile(package_file, os.path.join(dest_dir, os.path.basename(package_file)))
if args.install:
...
if args.reset:
    ...
    shutil.copyfile(os.path.join(os.getcwd(), 'brython_stdlib.js'), os.path.join(os.getcwd(), 'brython_modules.js'))
if args.modules:
...
if args.make_file_system:
    ...
    if len(args_fs) > 2:
        raise ValueError('--make_file_systems expects at most 2 arguments, got ' + str(len(args_fs)))
    vfs_name = args_fs[0]
