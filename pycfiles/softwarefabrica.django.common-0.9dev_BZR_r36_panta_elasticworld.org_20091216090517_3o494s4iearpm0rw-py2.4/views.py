...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/common/views.py
# Compiled at: 2009-11-11 04:20:55
...
class CreateObjectView(crud.CreateObjectView):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._set_form_fields_instance(*args, **kwargs)

    def _set_form_fields_instance(self, *args, **kwargs):
        ...
        if model:
            edit_fields = get_sfapp_field(model, 'edit_fields', get_sfapp_field(model, 'fields', args_fields))
            if edit_fields is not None:
                self.fields = edit_fields
            edit_exclude_fields = get_sfapp_field(model, 'edit_exclude_fields', get_sfapp_field(model, 'exclude_fields', args_exclude))
            if edit_exclude_fields is not None and args_exclude is None:
                self.exclude = edit_exclude_fields
        return self

    def _set_form_fields(self, vcall):
        assert isinstance(vcall, ViewCall)
        model = vcall.c_model or vcall.model or self.model
        args_fields = vcall.fields or self.fields
        args_exclude = vcall.exclude or self.exclude
        args_fieldorder = vcall.fieldorder or self.fieldorder
        if model:
            edit_fields = get_sfapp_field(model, 'edit_fields', get_sfapp_field(model, 'fields', args_fields))
            if edit_fields is not None:
                vcall.fields = edit_fields
            edit_exclude_fields = get_sfapp_field(model, 'edit_exclude_fields', get_sfapp_field(model, 'exclude_fields', args_exclude))
            if edit_exclude_fields is not None and args_exclude is None:
                vcall.exclude = edit_exclude_fields
        return self

    def pre_call(self, vcall, request):
        self._set_form_fields(vcall)
        return super(CreateObjectView, self).pre_call(vcall, request)
...
class UpdateObjectView(crud.UpdateObjectView):
    ...
    def pre_call(self, vcall, request):
        self._set_form_fields(vcall)
        return super(UpdateObjectView, self).pre_call(vcall, request)
    ...
    def __call__(self, request, *args, **kwargs):
        ...
        if 'q' in request.GET:
            if request.GET['q'].strip():
                ...
                raise issubclass(model, db.models.Model) or AssertionError
            fields = search_fields(model)
            queryset = search_queryset(queryset, q, fields)
        kwargs['queryset'] = queryset
        return super(ListObjectView, self).__call__(request, *args, **kwargs)
...
class DetailObjectView(crud.DetailObjectView):
    ...
    def populate_context(self, vcall, request):
        ...
        detail_fields = get_sfapp_field(obj, 'detail_fields', get_sfapp_field(obj, 'fields', None))
        detail_exclude_fields = get_sfapp_field(obj, 'detail_exclude_fields', get_sfapp_field(obj, 'exclude_fields', ()))
                    ...
                    if isinstance(field.rel, models.ManyToOneRel):
                        ...
                        if objs is not None:
                            if hasattr(objs, 'get_absolute_url'):
                                fmt_value = '<a href="%s">%s</a>' % (objs.get_absolute_url(), objs)
                            else:
                                fmt_value = '%s' % objs
                        else:
        ...
        for field in m2m_fields:
            ...
            field_dict = dict(field=field, name=field.name, value=related_objs)
            obj_field_values.append(field_dict)
        ...
        class RelatedItem(object):
            ...
            def __init__(self, instance, rel_object):
                ...
                assert isinstance(self.rel, ManyToManyRel) or isinstance(self.rel, ManyToOneRel)
                ...
                rev_verbose_names = get_sfapp_field(self.model, 'reverse_verbose_names', {})
                r_verbose_name, r_verbose_name_plural = self.opts.verbose_name, self.opts.verbose_name_plural
                ...
                if self.related.get_accessor_name():
                    self.object_list = [ i for i in getattr(self.instance, self.related.get_accessor_name()).all() ]
                return

            def __str__(self):
                return '%r for instance:%r field:%s' % (self.related, self.instance, self.fieldname)
...
def search_fields(model):
    ...
    search_fields = get_sfapp_field(model, 'search_fields', get_sfapp_field(model, 'fields', None))
    search_exclude_fields = get_sfapp_field(model, 'search_exclude_fields', get_sfapp_field(model, 'exclude_fields', ()))
    fields = []
...
def search(request, models, template_name='common/object_search.html'):
    ...
    return render_to_response(request, template_name, {'query_string': q, 'found': found_objects})