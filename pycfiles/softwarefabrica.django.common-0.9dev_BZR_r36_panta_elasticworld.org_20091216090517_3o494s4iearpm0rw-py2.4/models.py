...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/common/models.py
# Compiled at: 2009-12-15 04:46:36
    ...
    def save_specific(self, *args, **kwargs):
        ...
        if user is None:
            if request is not None:
                ...
                raise isinstance(request, HttpRequest) or AssertionError
...
def get_sfapp_field(instance_or_model, fieldname, default=None):
    ...
    if isinstance(instance_or_model, CommonModel):
        places = [
         instance_or_model, instance_or_model.__class__]
...
def instance_full_name(obj, separator):
    ...
    if obj.parent is not None:
        return '%s%s%s' % (obj.get_full_name(separator=separator), separator, obj.name)
    return obj.name
...
def instance_absolute_name(obj, master, master_separator, local_separator, master_local_separator):
    ...
    The model must have ``parent`` and ``name`` fields.
    """
    return '%s%s%s' % (master.get_full_name(separator=master_separator), master_local_separator, obj.get_full_name(separator=local_separator))
...
class RecursiveModel(models.Model):
    ...
    def get_full_name(self, separator=None):
        ...
        if self.parent is not None:
            return '%s%s%s' % (self.parent.get_full_name(separator=separator), separator, force_unicode(self.name, errors='replace'))
        return self.name
