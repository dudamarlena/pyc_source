...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/cctagutils/handler/xmp.py
# Compiled at: 2007-03-15 10:29:40
import os, ccrdf, cctagutils.handler.base
...
class XmpMetadata(cctagutils.handler.base.BaseMetadata):
    ...
    def xmp(self):
        ...
        if self.__xmp is None:
            ...
            for (start_tag, end_tag) in zip(self.START_STRINGS, self.END_STRINGS):
                if file_contents.find(start_tag) == -1:
                    continue
                rdf_string = file_contents[file_contents.find(start_tag):file_contents.find(end_tag) + len(end_tag)]
    ...
    def work_rdfdict(self):
        ...
        return ccrdf.rdfdict.rdfDict(self.__subject(), rdfStore=self.xmp.store)

    def __subject(self):
        """Return the subject (or likely suspect) for this work."""
        subjects = set([ s for s in self.xmp.store.subjects() if str(s)[:2] != '_:' ])
        if len(subjects) == 0:
            return ''
        if len(subjects) == 1:
            return subjects.pop()
    ...
    def getClaim(self):
        return self.__getObjectStr('http://ns.adobe.com/xap/1.0/rights/Copyright')
    ...
    def getMetadataUrl(self):
        ...
        return self.__getObjectStr('http://ns.adobe.com/xap/1.0/rights/WebStatement')
