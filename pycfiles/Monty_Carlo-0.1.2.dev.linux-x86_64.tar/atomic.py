...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/monty-carlo/atomic/atomic.py
# Compiled at: 2011-09-11 12:38:19
...
class Atom(Element):
    ...
    def decay(self):
        ...
        if self.decay_prob == False:
            ...
            self.mass_number = self.mass_number - (self.decay_process[1] + self.decay_process[0])
            self.reinit()
...
class Lattice(State):
    ...
    def __init__(self, vectors, atoms_basis, strech=1, *args, **kwargs):
        ...
        self.unit_vectors = [ self.n_norm_vec / num.linalg.norm(self.n_norm_vec) for self.n_norm_vec in self.unit_vectors_not_norm
                            ]
        self.atoms_basis = atoms_basis
        self.atoms_unit = [ (num.dot(atom.position, self.unit_vectors), atom) for atom in self.atoms_basis
                          ]
        self.atoms = []
        if self.dim == 3:
            symmetries = num.array([ [x_ctr, y_ctr, z_ctr] for x_ctr in range(strech) for y_ctr in range(strech) for z_ctr in range(strech)
                                   ])
        else:
            if self.dim == 2:
                symmetries = num.array([ [x_ctr, y_ctr] for x_ctr in range(strech) for y_ctr in range(strech)
                                       ])
            elif self.dim == 1:
                symmetries = num.array([ [x_ctr] for x_ctr in range(strech)
                                       ])
            print symmetries
            print '  Done!'
            ...
            for atom_coordinate, atom in self.atoms_unit:
                for symmetry_vec in symmetries:
                    ...
                    newcomer, = atom.copy()
                    if any((newcomer_coordinate == coor).all() for coor in coordinates_only):
            ...
            if unit and self.unit != None:
                for atom_coor, atom in self.atoms_unit:
                    if hasattr(atom, 'color') and hasattr(atom, 'size'):
                        ...
                        plot_dict = {}
                    axes.scatter3D(num.array([atom_coor[0]]), num.array([atom_coor[1]]), num.array([atom_coor[2]]), **plot_dict)
                ...
                for atom in self.atoms:
                    if hasattr(atom, 'color') and hasattr(atom, 'size'):
                        ...
                        plot_dict = {}
                    axes.scatter3D(num.array([atom.position[0]]), num.array([atom.position[1]]), num.array([atom.position[2]]), **plot_dict)
            ...
            if unit and self.unit != None:
                for atom_coor, atom in self.atoms_unit:
                    if hasattr(atom, 'color') and hasattr(atom, 'size'):
                        ...
                        plot_dict = {}
                    plt.scatter(num.array([atom_coor[0]]), num.array([atom_coor[1]]), **plot_dict)
                ...
                for atom in self.atoms:
                    if hasattr(atom, 'color') and hasattr(atom, 'size'):
                        ...
                        plot_dict = {}
                    plt.scatter(num.array([atom.position[0]]), num.array([atom.position[1]]), **plot_dict)
