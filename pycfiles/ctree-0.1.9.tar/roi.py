...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/roi.py
# Compiled at: 2016-03-19 20:26:13
import os, wx
from wx import xrc
import motmot.wxvideo.wxvideo as wxvideo, numpy as num, scipy.ndimage.morphology as morph, codedir, imagesk
from params import params
RSRC_FILE = os.path.join(codedir.codedir, 'xrc', 'roi.xrc')
if RSRC_FILE is None:
    RSRC_FILE = os.path.join('xrc', 'roi.xrc')
...
class ROI():
    ...
    def InitializeValues(self, init_polygons):
        ...
        self.img_wind.update_image_and_drawings('fixbg', imblank, format='MONO8')
    ...
    def OnClose(self, evt=None):
        if self.currpolygon.shape[0] > 2:
            ...
            wx.MessageBox('Not enough points selected to close this polygon', 'Select More Points', wx.ICON_ERROR | wx.OK)
        self.currpolygon = num.zeros((0, 2))
        self.InAddPolygonMode()
        self.ShowImage()
    ...
    def ShowImage(self):
        ...
        if self.currpolygon.shape[0] >= 1:
            points = [
             [
              self.currpolygon[(0, 0)], self.currpolygon[(0, 1)]]]
            pointcolors = [
             (currpolygoncolor[0] / 255.0, currpolygoncolor[1] / 255.0,
              currpolygoncolor[2] / 255.0)]
        else:
            points = []
            pointcolors = []
        lines = []
        ...
        self.img_wind.update_image_and_drawings('fixbg', self.get_image_to_show(), format='MONO8', linesegs=lines, lineseg_colors=linecolors, lineseg_widths=linewidths, points=points, point_colors=pointcolors)
    ...
    def MouseDoubleClick(self, evt):
        ...
        if x > self.nc or y > self.nr:
            return
        d = num.sqrt((x - self.currpolygon[(0, 0)]) ** 2 + (y - self.currpolygon[(0,
                                                                                  1)]) ** 2)
        if d > 10:
            return
        self.OnClose()
...
def point_inside_polygon(x, y, poly):
    ...
    for i in range(n + 1):
        p2x, p2y = poly[(i % n)]
        idx = num.logical_and(y > min(p1y, p2y), num.logical_and(y <= max(p1y, p2y), x <= max(p1x, p2x)))
        if p1y != p2y:
