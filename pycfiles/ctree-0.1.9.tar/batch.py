...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/batch.py
# Compiled at: 2016-03-19 20:21:27
import os, sys, wx
from wx import xrc
from wx.lib.mixins.listctrl import TextEditMixin
from numpy import inf
import codedir, movies
from params import params
RSRC_FILE = os.path.join(codedir.codedir, 'xrc', 'batch.xrc')
if RSRC_FILE is None:
    RSRC_FILE = os.path.join('xrc', 'batch.xrc')
AUTODETECT_ARENA = 0
...
class BatchWindow:
    ...
    def __init__(self, parent, directory, default_extension, init_movie=None):
        ...
        if init_movie is not None:
            self.file_list.append({'filename': init_movie.fullpath, 'nframes': init_movie.get_n_frames(), 
               'startframe': 0, 
               'endframe': inf})
        self.dir = directory
    ...
    def ShowWindow(self, parent):
        ...
        self.list_box = EditableListCtrl(parent=self.frame, style=wx.LC_NO_SORT_HEADER | wx.LC_HRULES | wx.LC_SINGLE_SEL | wx.LC_REPORT)
        ...
        self.choice_boxes = [
         self.arena_choice, self.shape_choice, self.bg_model_choice]
    ...
    def should_load_settings(self):
        return self.arena_choice.GetSelection() == LOAD_SETTINGS_ARENA or self.shape_choice.GetSelection() == LOAD_SETTINGS_SHAPE or self.bg_model_choice.GetSelection() == LOAD_SETTINGS_BG or self.settings_checkbox.IsChecked()

    def should_overwrite_arena(self):
        return self.arena_choice.GetSelection() != USE_EXISTING_ARENA and self.arena_choice.GetSelection() != DONT_USE_ARENA
    ...
    def OnButtonAdd(self, evt):
        ...
        try:
            movie = movies.Movie(self.dir, interactive=True, parentframe=self.frame, open_now=True, open_multiple=True, default_extension=self.default_extension)
        except ImportError:
            return
        ...
        if hasattr(movie, 'fullpaths_mult'):
            for fullpath in movie.fullpaths_mult[1:]:
                try:
                    movie_objs.append(movies.Movie(fullpath, interactive=False, open_now=True))
                except ImportError:
                    return
        ...
        for movie in movie_objs:
            ...
            for datum in self.file_list:
                if datum['filename'] == movie.fullpath:
                    wx.MessageBox(movie.fullpath + ' has already been added;\nnot duplicating', 'Duplicate', wx.ICON_WARNING | wx.OK)
                    add_flag = False
                    break

            if add_flag:
                movie_data = {'filename': movie.fullpath, 'nframes': movie.get_n_frames(), 'startframe': 0, 
                   'endframe': inf}
