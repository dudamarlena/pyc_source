...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/bg_settings.py
# Compiled at: 2016-05-01 00:15:23
import os.path, sys, numpy as num, scipy.io, wx
from wx import xrc
import motmot.wxvalidatedtext.wxvalidatedtext as wxvt, codedir
from colormapk import colormap_image
from ellipsesk import find_ellipses
from ellipses_draw import draw_ellipses
import fixbg, imagesk
...
if THRESH_RSRC_FILE is None:
    THRESH_RSRC_FILE = os.path.join('xrc', 'bg_thresh.xrc')
SETTINGS_RSRC_FILE = os.path.join(codedir.codedir, 'xrc', 'bg_settings.xrc')
if SETTINGS_RSRC_FILE is None:
    SETTINGS_RSRC_FILE = os.path.join('xrc', 'bg_settings.xrc')
...
class BackgroundSettings():
    ...
    def ShowBG(self, parent, framenumber, old_thresh, dosub_callback=None):
        ...
        self.thresh_slider.SetScrollbar(self.GetThresholdScrollbar(), 0, 255, 230)
        self.thresh_low_slider = xrc.XRCCTRL(self.frame, 'slider_low_thresh')
        self.thresh_low_slider.SetScrollbar(self.GetThresholdLowScrollbar(), 0, 255, 230)
        ...
        wxvt.setup_validated_integer_callback(self.opening_radius_textinput, xrc.XRCID('opening_radius'), self.OnOpeningRadiusTextEnter, pending_color=params.wxvt_bg)
        ...
        wxvt.setup_validated_integer_callback(self.closing_radius_textinput, xrc.XRCID('closing_radius'), self.OnClosingRadiusTextEnter, pending_color=params.wxvt_bg)
        ...
        wxvt.setup_validated_float_callback(self.min_frac_frames_isback_textinput, xrc.XRCID('min_frac_frames_isback'), self.OnMinFracFramesIsBackTextEnter, pending_color=params.wxvt_bg)
    ...
    def max_thresh(self):
        return 255.0 / self.thresh_mult()

    def SetThreshold(self):
        params.n_bg_std_thresh = min(self.max_thresh(), max(params.n_bg_std_thresh, 0.0))
        params.n_bg_std_thresh_low = min(self.max_thresh(), max(params.n_bg_std_thresh_low, 0.0))
        params.n_bg_std_thresh_low = min(params.n_bg_std_thresh_low, params.n_bg_std_thresh)
    ...
    def ReadScrollbar(self):
        val = 255.0 - float(self.thresh_slider.GetThumbPosition())
        return val / self.thresh_mult()

    def ReadScrollbarLow(self):
        val = 255.0 - float(self.thresh_low_slider.GetThumbPosition())
        return val / self.thresh_mult()
    ...
    def OnBgTypeChoice(self, evt):
        ...
        if new_bg_type is not wx.NOT_FOUND:
            if new_bg_type == 0:
                ...
                self.bg_type = 'other'
            self.DoSub()
            if params.n_bg_std_thresh > 255.0:
                params.n_bg_std_thresh = 255.0
                self.SetThreshold()
                self.DoSub()
    ...
    def OnNormChoice(self, evt):
        ...
        if donormalize:
            params.n_bg_std_thresh *= 255.0 / oldmaxdfore
            params.n_bg_std_thresh_low *= 255.0 / oldmaxdfore
        self.SetThreshold()
        self.DoSub()
    ...
    def UpdateIsArena(self):
        ...
        if hasattr(params, 'min_nonarena') and hasattr(self, 'center'):
            self.isarena = self.isarena & (self.center < params.min_nonarena)
        if params.do_set_circular_arena and hasattr(params, 'arena_center_x') and params.arena_center_x is not None:
            self.isarena = self.isarena & ((params.GRID.X - params.arena_center_x) ** 2.0 + (params.GRID.Y - params.arena_center_y) ** 2 <= params.arena_radius ** 2.0)
        if hasattr(self, 'roi'):
    ...
    def OnThreshSlider(self, evt):
        ...
        if start_std_thresh != params.n_bg_std_thresh or start_std_thresh_low != params.n_bg_std_thresh_low:
            self.DoSub()
    ...
    def get_sub_image(self, sub_type, framenumber):
        ...
        if sub_type == params.SHOW_BACKGROUND:
            ...
            if not params.use_expbgfgmodel or not hasattr(self, 'expbgfgmodel') or self.expbgfgmodel is None:
                resp = wx.MessageBox('No prior model loaded.', 'No prior model loaded', wx.OK)
            elif sub_type == params.SHOW_EXPBGFGMODEL_LLR:
                im, stamp = self.movie.get_frame(int(framenumber))
        ...
        wx.EndBusyCursor()
        return (
         img_8, img_format, line_segs, line_clrs, dfore, bw, cc, ncc, preview_range)

    def DoSub(self):
        """Do background subtraction and draw to screen."""
        img_8, img_format, line_segs, line_clrs, self.dfore, self.bw, self.cc, self.ncc, preview_range = self.get_sub_image(self.show_img_type, self.show_frame)
        line_widths = (
         params.ellipse_thickness,) * len(line_segs)
        self.img_wind.update_image_and_drawings('bg_img', img_8, format=img_format, linesegs=line_segs, lineseg_colors=line_clrs, lineseg_widths=line_widths)
...
class BgSettingsDialog():
    ...
    def __init__(self, parent, bg):
        ...
        wxvt.setup_validated_integer_callback(self.nframes, xrc.XRCID('nframes'), self.OnTextValidatedNFrames, pending_color=params.wxvt_bg)
        self.firstframe_box.SetValue(str(self.bg.bg_firstframe))
        wxvt.setup_validated_integer_callback(self.firstframe_box, xrc.XRCID('bg_firstframe'), self.OnTextValidatedFirstframe, pending_color=params.wxvt_bg)
        self.lastframe_box.SetValue(str(self.bg.bg_lastframe))
        wxvt.setup_validated_integer_callback(self.lastframe_box, xrc.XRCID('bg_lastframe'), self.OnTextValidatedLastframe, pending_color=params.wxvt_bg)
        ...
        wxvt.setup_validated_integer_callback(self.expbgfgmodel_llr_thresh, xrc.XRCID('llr_thresh'), self.OnTextValidatedLLRThresh, pending_color=params.wxvt_bg)
        wxvt.setup_validated_integer_callback(self.expbgfgmodel_llr_thresh_low, xrc.XRCID('llr_thresh_low'), self.OnTextValidatedLLRThresh, pending_color=params.wxvt_bg)
        if 'darwin' in sys.platform:
            fix_text_sizes(self.frame)
