...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/setarena.py
# Compiled at: 2016-03-19 20:26:57
import os.path, sys, numpy as num, scipy.signal as signal
from scipy.misc import imresize
import wx
from wx import xrc
import motmot.wxvideo.wxvideo as wxvideo, motmot.wxvalidatedtext.wxvalidatedtext as wxvt, codedir, colormapk, houghcircles as hough, imagesk
...
if RSRC_FILE is None:
    RSRC_FILE = os.path.join('xrc', 'setarena.xrc')
RESIZE = 500
NTHRESHSTEPS = 30
...
class SetArena():
    ...
    def InitializeValues(self):
        ...
        self.img_wind.update_image_and_drawings('setarena', imblank, format='RGB8')
        ...
        self.edgepoint = [
         self.arena_center_x + self.arena_radius,
         self.arena_center_y]
        ...
        self.maxradius = max(params.movie_size[1] * self.nc_resize_ratio, params.movie_size[0] * self.nr_resize_ratio) / 2.0
        self.maxx = (params.movie_size[1] - 1.0) * self.nc_resize_ratio
        self.maxy = (params.movie_size[0] - 1.0) * self.nr_resize_ratio
    ...
    def BindCallbacks(self):
        ...
        wxvt.setup_validated_float_callback(self.radius_text, xrc.XRCID('radius_text'), self.OnRadiusValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.x_text, xrc.XRCID('x_text'), self.OnXValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.y_text, xrc.XRCID('y_text'), self.OnYValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.minradius_text, xrc.XRCID('minradius_text'), self.OnRadiusBoundsValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.maxradius_text, xrc.XRCID('maxradius_text'), self.OnRadiusBoundsValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.minx_text, xrc.XRCID('minx_text'), self.OnXBoundsValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.maxx_text, xrc.XRCID('maxx_text'), self.OnXBoundsValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.miny_text, xrc.XRCID('miny_text'), self.OnYBoundsValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_float_callback(self.maxy_text, xrc.XRCID('maxy_text'), self.OnYBoundsValidated, pending_color=params.wxvt_bg)
    ...
    def force_range(self, minv, maxv):
        minv = min(max(minv, 0.0), 1.0)
        maxv = min(max(maxv, 0.0), 1.0)
        if maxv < minv:
            maxv = minv
        return (
         minv, maxv)
    ...
    def Detect(self, evt=None):
        wx.BeginBusyCursor()
        wx.Yield()
        self.arena_center_x, self.arena_center_y, self.arena_radius = detectarena(self.edgemag_zero)
        self.set_edgepoint()
    ...
    def Refine(self, evt=None):
        if self.arena_center_x is None:
            ...
            wx.BeginBusyCursor()
            wx.Yield()
            self.arena_center_x, self.arena_center_y, self.arena_radius = detectarena(self.edgemag_zero, approxa=self.arena_center_x, approxb=self.arena_center_y, approxr=self.arena_radius)
            self.set_edgepoint()
            self.force_edgepoint_inbounds()
            self.display_parameters()
            self.ShowImage()
    ...
    def MouseDown(self, evt):
        ...
        dcenter = num.sqrt((x - self.arena_center_x) ** 2.0 + (y - self.arena_center_y) ** 2.0)
        dedge = num.sqrt((x - self.edgepoint[0]) ** 2.0 + (y - self.edgepoint[1]) ** 2.0)
    ...
    def ShowImage(self, evt=None):
        ...
        self.img_wind.update_image_and_drawings('setarena', self.image_shown, format='RGB8', linesegs=linesegs, lineseg_colors=linecolors, points=pointlist, point_colors=pointcolors, point_radii=pointsizes, lineseg_widths=circlewidths)
    ...
    def MouseUp(self, evt):
        ...
        if self.selected_point == 'center':
            ...
            self.edgepoint[0] = x
            self.edgepoint[1] = y
        self.arena_radius = num.sqrt((self.edgepoint[0] - self.arena_center_x) ** 2.0 + (self.edgepoint[1] - self.arena_center_y) ** 2.0)
        self.force_edgepoint_inbounds()
        self.selected_point = 'none'
        self.display_parameters()
        self.ShowImage()
    ...
    def GetArenaParameters(self):
        ...
        r = self.arena_radius * 2.0 / (self.nc_resize_ratio + self.nr_resize_ratio)
        return [
         x, y, r]


def choose_edge_threshold(edgemag, FracPixelsNotEdges=0.95):
...
def doall(im):
    ...
    params.arena_center_x, params.arena_center_y, params.arena_radius = detectarena(edgemag_zero)
    return
...
def detectarena(edgemag, approxa=None, approxb=None, approxr=None):
    ...
    isguessedr = True
    if approxa is None:
        approxa = (params.min_arena_center_x + params.max_arena_center_x) / 2.0 * float(nc)
        isguesseda = False
    if approxb is None:
        approxb = (params.min_arena_center_y + params.max_arena_center_y) / 2.0 * float(nr)
        isguessedb = False
    if approxr is None:
        approxr = (params.min_arena_radius + params.max_arena_radius) / 2.0 * float(min(nr, nc))
        isguessedr = False
    if isguesseda:
        mina = approxa - 0.025 * nc
        maxa = approxa + 0.025 * nc
    else:
        mina = params.min_arena_center_x * float(nc)
        maxa = params.max_arena_center_x * float(nc)
    if isguessedb:
        minb = approxb - 0.025 * nc
        maxb = approxb + 0.025 * nc
    else:
        minb = params.min_arena_center_y * float(nr)
        maxb = params.max_arena_center_y * float(nr)
    if isguessedr:
        minr = max(0.0, approxr - 0.025 * min(nc, nr))
        maxr = approxr + 0.025 * min(nc, nr)
    else:
    ...
    bincentersa = (binedgesa[:-1] + binedgesa[1:]) / 2.0
    binedgesb = num.linspace(minb, maxb, nbinsb + 1)
    bincentersb = (binedgesb[:-1] + binedgesb[1:]) / 2.0
    binedgesr = num.linspace(minr, maxr, nbinsr + 1)
    bincentersr = (binedgesr[:-1] + binedgesr[1:]) / 2.0
    x, y, r = detectcircles(edgemag, binedgesa=binedgesa, bincentersb=bincentersb, bincentersr=bincentersr, peaksnhoodsize=peaksnhoodsize, peaksthreshold=0.0, maxncircles=1)
    t = num.linspace(0, 2.0 * num.pi, 200)
...
def detectcircles(edgemag, binedgesa=None, bincentersb=None, bincentersr=None, nbinsa=10, nbinsb=10, nbinsr=10, mina=0.0, minb=0.0, minr=None, maxa=None, maxb=None, maxr=None, peaksnhoodsize=None, peaksthreshold=None, maxncircles=1):
    binedgesa, bincentersa, bincentersb, bincentersr, nbinsa, nbinsb, nbinsr, peaksnhoodsize, peaksthreshold, maxncircles = detectcircles_setparameters(edgemag, binedgesa, bincentersb, bincentersr, nbinsa, nbinsb, nbinsr, mina, minb, minr, maxa, maxb, maxr, peaksnhoodsize, peaksthreshold, maxncircles)
    bw = edgemag > 0
    r, c = num.where(bw)
    c = c.astype('float')
    r = r.astype('float')
    ...
    if peaksthreshold is None:
        peaksthreshold = num.max(acc) / 2.0
...
def edge(im, sigma=1.0):
    ...
    ssq = sigma ** 2
    width, = num.where(num.exp(-pw ** 2 / (2.0 * ssq)) > GaussianDieOff)
    if len(width) == 0:
        width = 1
    else:
        width = width[(-1)]
    ...
    t = t.astype('float')
    gau = num.exp(-(t * t) / (2.0 * ssq)) / (2.0 * num.pi * ssq)
    y, x = num.mgrid[-width:width + 1, -width:width + 1]
    dgau2D = -x * num.exp(-(x ** 2 + y ** 2) / (2.0 * ssq)) / (num.pi * ssq)
...
def detectcircles_setparameters(im, binedgesa, bincentersb, bincentersr, nbinsa, nbinsb, nbinsr, mina, minb, minr, maxa, maxb, maxr, peaksnhoodsize, peaksthreshold, maxncircles):
    nr = im.shape[0]
    nc = im.shape[1]
    if maxa is None:
        maxa = float(nc - 1)
    ...
    if peaksnhoodsize is None:
        ...
        peaksnhoodsize = max(2 * num.ceil(peaksnhoodsize / 2.0) + 1, 1)
    return (
...
def resize_image_for_speed(im):
    ...
    return (
     im, nr_resize_ratio, nc_resize_ratio)