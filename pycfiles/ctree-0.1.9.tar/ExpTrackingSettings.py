...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/ExpTrackingSettings.py
# Compiled at: 2013-09-26 00:22:11
import numpy as num, scipy.stats as stats
from params import params
import annfiles as annot, matplotlib.pyplot as plt, os.path, pickle, getopt, sys
...
class ExpTrackingSettings():
    ...
    def __init__(self, annnames=None, picklefile=None, nframessample=100, min_major_percentile=1.0, max_major_percentile=99.0, min_minor_percentile=1.0, max_minor_percentile=99.0, min_area_percentile=1.0, max_area_percentile=99.0, min_ecc_percentile=1.0, max_ecc_percentile=99.0, jump_distance_delta=0.1):
    ...
    def update_shape_models(self):
        for ell in self.trxcurr.itervalues():
            self.majors.append(ell.size.height * 4.0)
            self.minors.append(ell.size.width * 4.0)
    ...
    def update_motion_models(self):
        for id, ellcurr in self.trxcurr.iteritems():
            if self.trxprev.hasItem(id) and self.trxprevprev.hasItem(id):
                ...
                dangle = (ellprev.angle - ellprevprev.angle + num.pi / 2.0) % num.pi - num.pi / 2.0
                anglepred = ellprev.angle + dangle
                anglecurr = (ellcurr.angle - anglepred + num.pi / 2.0) % num.pi + anglepred - num.pi / 2
    ...
    def compute_models_permovie(self, i):
        ...
        """
        self.moviei = i
        self.trx = annot.AnnotationFile(self.annnames[i], doreadbgmodel=False, doreadtrx=True)
        self.firstframe = self.trx.firstframetracked + 2
        self.lastframe = self.trx.lastframetracked
        self.framessample = num.unique(num.round(num.linspace(self.firstframe + 1, self.lastframe, self.nframessample)).astype(int))
        if DEBUG:
            print 'Collecting data for movie %d, annfile = %s...' % (self.moviei, self.annnames[self.moviei])
    ...
    def est_motion_parameters(self):
        ...
        alpha = (num.sum((self.xcurrs[self.is_motion_data] - self.xprevs[self.is_motion_data]) * self.dxs[self.is_motion_data]) + num.sum((self.ycurrs[self.is_motion_data] - self.yprevs[self.is_motion_data]) * self.dys[self.is_motion_data])) / (num.sum(self.dxs[self.is_motion_data] ** 2.0) + num.sum(self.dys[self.is_motion_data] ** 2.0))
        alpha = max(alpha, 0.0)
        alpha = min(alpha, 1.0)
        self.center_dampen = 1.0 - alpha
        alpha = num.sum(self.anglecurrs[self.is_motion_data] * self.dangles[self.is_motion_data]) / num.sum(self.dangles[self.is_motion_data] ** 2.0)
        alpha = max(alpha, 0.0)
        alpha = min(alpha, 1.0)
        self.angle_dampen = 1.0 - alpha
        self.xpreds = self.xprevs + self.dxs * (1.0 - self.center_dampen)
        self.ypreds = self.yprevs + self.dys * (1.0 - self.center_dampen)
        self.anglepreds = self.angleprevs + self.dangles * (1.0 - self.angle_dampen)
        self.center_err2s = (self.xpreds - self.xcurrs) ** 2 + (self.ypreds - self.ycurrs) ** 2.0
        self.center_err2 = num.mean(self.center_err2s[self.is_motion_data])
        self.angle_err2s = ((self.anglepreds - self.anglecurrs + num.pi / 2.0) % num.pi - num.pi / 2.0) ** 2.0
        ...
        self.max_jump_distance = num.max(self.dists[self.is_motion_data]) * (1.0 + self.jump_distance_delta)
    ...
    def show_motion(self):
        ...
        self.centers_centererr = (self.edges_centererr[:-1] + self.edges_centererr[1:]) / 2.0
        ...
        self.centers_angleerr = (self.edges_angleerr[:-1] + self.edges_angleerr[1:]) / 2.0
        ...
        self.centers_dist = (self.edges_dist[:-1] + self.edges_dist[1:]) / 2.0
        plt.figure()
        plt.subplot(131)
        plt.plot(self.centers_centererr, self.frac_centererr, 'k.-')
        maxy = num.max(self.frac_centererr)
        ax = num.array([self.edges_centererr[0], self.edges_centererr[(-1)], -maxy * DELTAPLOT, maxy * (1.0 + DELTAPLOT)])
        plt.axis(ax)
        plt.title('center dist')
        plt.subplot(132)
        plt.plot(self.centers_angleerr, self.frac_angleerr, 'k.-')
        maxy = num.max(self.frac_angleerr)
        ax = num.array([self.edges_angleerr[0], self.edges_angleerr[(-1)], -maxy * DELTAPLOT, maxy * (1.0 + DELTAPLOT)])
        plt.axis(ax)
        ...
        ax = num.array([self.edges_dist[0], max(self.max_jump_distance * (1.0 + DELTAPLOT), self.edges_dist[(-1)]), -maxy * DELTAPLOT, maxy * (1.0 + DELTAPLOT)])
    ...
    def show_shape(self):
        ...
        self.centers_area = (self.edges_area[:-1] + self.edges_area[1:]) / 2.0
        plt.figure()
        plt.subplot(221)
        ...
        self.centers_major = (self.edges_major[:-1] + self.edges_major[1:]) / 2.0
        plt.subplot(222)
        ...
        self.centers_minor = (self.edges_minor[:-1] + self.edges_minor[1:]) / 2.0
        plt.subplot(223)
        ...
        self.centers_ecc = (self.edges_ecc[:-1] + self.edges_ecc[1:]) / 2.0
        plt.subplot(224)
...
def genprctile(x, prct):
    if prct < 0 or prct > 100:
        ...
        dx = maxx - minx
        if prct < 0:
            p = minx - -prct / 100.0 * dx
        else:
            p = maxx + (prct - 100.0) / 100 * dx
    else:
...
def read_params(paramsFileName):
    ...
    params['min_major_percentile'] = 1.0
    params['max_major_percentile'] = 99.0
    params['min_minor_percentile'] = 1.0
    params['max_minor_percentile'] = 99.0
    params['min_area_percentile'] = 1.0
    params['max_area_percentile'] = 99.0
    params['min_ecc_percentile'] = 1.0
    params['max_ecc_percentile'] = 99.0
    params['jump_distance_delta'] = 0.1
