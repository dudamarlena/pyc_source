...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/draw.py
# Compiled at: 2016-03-19 19:52:38
import os, pdb, sys, matplotlib
matplotlib.interactive(True)
import matplotlib.backends.backend_wxagg, matplotlib.figure, matplotlib.cm, matplotlib.pyplot as plt
if hasattr(plt, 'tight_layout'):
    HAS_TIGHTLAYOUT = True
else:
    HAS_TIGHTLAYOUT = False
import matplotlib.transforms as mtransforms, numpy as num, wx
from wx import xrc
...
class PlotPanel(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, color=None, dpi=None, style=wx.NO_FULL_REPAINT_ON_RESIZE, **kwargs):
    ...
    def OnMenuSave(self, evt):
        ...
        extensions = {'.eps': 'encapsulated postscript (*.eps)', '.png': 'portable network graphics (*.png)', 
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                wx.MessageBox('Miscellaneous error while saving.', 'Write Error', wx.ICON_ERROR | wx.OK)
    ...
    def post_draw(self, evt):
        ...
        if size.height > 0.8:
            self.figure.subplots_adjust(bottom=margins.bottom + vspace / 2.0, top=margins.top - vspace / 2.0)
        elif size.height < 0.5:
            self.figure.subplots_adjust(bottom=margins.bottom - vspace / 2.0, top=margins.top + vspace / 2.0)
        hspace = size.width - (margins.right - margins.left)
        if size.width > 0.8:
            self.figure.subplots_adjust(left=margins.left + hspace / 2.0, right=margins.right - hspace / 2.0)
        else:
            if size.width < 0.5:
                self.figure.subplots_adjust(left=margins.left - hspace / 2.0, right=margins.right + hspace / 2.0)
    ...
    def __init__(self, frame, data, width, height):
        ...
        for frame in data:
            for fly in frame.itervalues():
                ...
                fly_y = fly.center.y
                try:
                    self.grid[(int(fly_x / const.pos_binsize), int(fly_y / const.pos_binsize))] += 1
                except IndexError:
                    print 'error adding', fly_x, fly_y, 'to bin', fly_x / const.pos_binsize, fly_y / const.pos_binsize, '(size is', self.grid.shape, '\x08)'
...
class DrawSettingsDialog:
    ...
    def __init__(self, parent=None):
        ...
        wxvt.setup_validated_integer_callback(self.vel_bins_box, xrc.XRCID('text_vel_bins'), self.OnTextValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_integer_callback(self.orn_bins_box, xrc.XRCID('text_orn_bins'), self.OnTextValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_integer_callback(self.space_bins_box, xrc.XRCID('text_space_bins'), self.OnTextValidated, pending_color=params.wxvt_bg)
        wxvt.setup_validated_integer_callback(self.pos_binsize_box, xrc.XRCID('text_pos_binsize'), self.OnTextValidated, pending_color=params.wxvt_bg)
        wxvt.Validator(self.vel_axes_box, xrc.XRCID('text_vel_axes'), self.OnTextValidated, self.ParseAxes, pending_color=params.wxvt_bg)
        wxvt.Validator(self.orn_axes_box, xrc.XRCID('text_orn_axes'), self.OnTextValidated, self.ParseAxes, pending_color=params.wxvt_bg)
        wxvt.Validator(self.space_axes_box, xrc.XRCID('text_space_axes'), self.OnTextValidated, self.ParseAxes, pending_color=params.wxvt_bg)
        wxvt.Validator(self.pos_axes_box, xrc.XRCID('text_pos_axes'), self.OnTextValidated, self.ParseAxes, pending_color=params.wxvt_bg)
        self.frame.Show()
    ...
    def ParseAxes(self, string):
        ...
        for vv in range(len(vals)):
            ...
            if vals[vv] == 'a':
                continue
            if vals[vv].isdigit() or vals[vv][0] == '-' and vals[vv][1:].isdigit():
                vals[vv] = int(vals[vv])
                continue
            try:
