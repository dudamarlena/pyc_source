...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/psutil/_psmswindows.py
# Compiled at: 2013-09-24 00:55:58
__doc__ = 'Windows platform implementation.'
import errno, os, sys, platform, warnings, _psutil_mswindows
...
TCP_STATES_TABLE = {_psutil_mswindows.MIB_TCP_STATE_ESTAB: CONN_ESTABLISHED, 
...
from _psutil_mswindows import ABOVE_NORMAL_PRIORITY_CLASS, BELOW_NORMAL_PRIORITY_CLASS, HIGH_PRIORITY_CLASS, IDLE_PRIORITY_CLASS, NORMAL_PRIORITY_CLASS, REALTIME_PRIORITY_CLASS, INFINITE
...
class Process(object):
    ...
    _nt_ext_mem = namedtuple('meminfo', (' ').join(['num_page_faults',
     'peak_wset',
     'wset',
     'peak_paged_pool',
     'paged_pool',
    ...
    def get_connections(self, kind='inet'):
        ...
        rawlist = _psutil_mswindows.get_process_connections(self.pid, families, types)
        ret = []
        for item in rawlist:
    ...
    if hasattr(_psutil_mswindows, 'get_process_io_priority'):
        ...
        def set_process_ionice(self, value, _):
            if _:
                raise TypeError('set_process_ionice() on Windows takes only 1 argument (2 given)')
            if value not in (2, 1, 0):
                raise ValueError('value must be 2 (normal), 1 (low) or 0 (very low); got %r' % value)
