...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/ellipses_draw.py
# Compiled at: 2016-03-19 20:25:24
import os, numpy as num, wx
from wx import xrc
import codedir
...
if ZOOM_RSRC_FILE is None:
    ZOOM_RSRC_FILE = os.path.join('xrc', 'ellipses_zoom.xrc')
SETTINGS_RSRC_FILE = os.path.join(codedir.codedir, 'xrc', 'ellipses_settings.xrc')
if SETTINGS_RSRC_FILE is None:
    SETTINGS_RSRC_FILE = os.path.join('xrc', 'ellipses_settings.xrc')
...
class EllipseWindow:
    ...
    def redraw(self, eraseBackground=False):
        ...
        if self.window.GetRect().GetHeight() < 1:
            return
        else:
            ind = self.spinner.GetValue()
            ...
            if not self.ellipses.hasItem(ind):
                ...
                linesegs = draw_ellipses([self.ellipses[ind]], colors=[
                ...
                self.window.update_image_and_drawings(self.name, im, format='MONO8', linesegs=linesegs, lineseg_colors=linecolors, lineseg_widths=linewidths)
...
def draw_ellipses(targets, step=10 * num.pi / 180.0, colors=None):
...
def draw_ellipses_bmp(img, targets, thickness=params.ellipse_thickness, step=10 * num.pi / 180.0, colors=params.colors, windowsize=None, zoomaxes=None):
    ...
    bmp = imagesk.draw_annotated_image(img, linelists=linelists, windowsize=windowsize, zoomaxes=zoomaxes, linecolors=linecolors, linewidth=thickness)
    return bmp