...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/tracking_settings.py
# Compiled at: 2016-03-19 20:27:41
import copy, os, sys, motmot.wxvideo.wxvideo as wxvideo, motmot.wxvalidatedtext.wxvalidatedtext as wxvt, numpy as num, wx
from wx import xrc
import codedir, params, ellipsesk as ell
...
class StoredObservations:
    ...
    def __init__(self, obs, frame, ellsmall=None, elllarge=None, didlowerthresh=None, didmerge=None, diddelete=None, didsplit=None):
        self.obs = obs
        self.frame = frame
        self.isvalid = True
...
class TrackingSettings:
    ...
    def BindCallbacks(self):
        ...
        self.bindctrl(('min_area', 'mean_area', 'max_area', 'min_major', 'mean_major',
                       'max_major', 'min_minor', 'mean_minor', 'max_minor', 'min_ecc',
                       'mean_ecc', 'max_ecc'), ('float', 'float', 'float', 'float',
                                                'float', 'float', 'float', 'float',
                                                'float', 'float', 'float', 'float'), self.SetManual)
        self.frame.Bind(wx.EVT_CHECKBOX, self.OnEnforceShape, self.enforce_shape_input)
        self.bindctrl(('angle_weight', 'max_jump', 'max_jump_split', 'min_jump', 'center_dampen',
                       'angle_dampen'), ('float', 'float', 'float', 'float', 'float',
                                         'float'), self.SetMotion)
        self.bindctrl(('max_area_delete', 'min_area_ignore', 'max_penalty_merge', 'lower_thresh',
                       'maxclustersperblob', 'max_blobs_detect'), ('float', 'float',
                                                                   'float', 'float',
    ...
    def SetManual(self, evt):
        ...
        if minarea <= meanarea and meanarea <= maxarea and minarea >= 0:
        ...
        if minmajor <= meanmajor and meanmajor <= maxmajor and minmajor >= 0:
        ...
        if minminor <= meanminor and meanminor <= maxminor and minminor >= 0:
        ...
        if minecc <= meanecc and meanecc <= maxecc and minecc >= 0 and maxecc <= 1:
    ...
    def ShowImage(self):
        if DEBUG_TRACKINGSETTINGS:
            print 'ShowImage ' + str(self.show_frame)
        try:
            wx.Yield()
        except:
            pass
        ...
        windowsize = [
         self.img_panel.GetRect().GetHeight(), self.img_panel.GetRect().GetWidth()]
        self.GetBgImage()
        if self.img_chosen == SHOW_UNFILTERED_OBSERVATIONS:
            if DEBUG_TRACKINGSETTINGS:
                print 'SHOW_UNFILTERED_OBSERVATIONS'
            ...
            if DEBUG_TRACKINGSETTINGS:
                print 'SHOW_DELETED_OBSERVATIONS'
            ...
            if DEBUG_TRACKINGSETTINGS:
                print 'SHOW_MERGED_OBSERVATIONS'
            ...
            if DEBUG_TRACKINGSETTINGS:
                print 'SHOW_LOWERED_OBSERVATIONS'
            ...
            for i, obs in enumerate(obs_filtered):
                plot_new_stuff = imagesk.draw_circle(obs.center.x, obs.center.y, params.params.max_jump * 2.0, params.params.colors[(i % len(params.params.colors))])
                plot_linesegs.extend(plot_new_stuff)
                plot_new_stuff = imagesk.draw_circle(obs.center.x, obs.center.y, params.params.min_jump, params.params.colors[(i % len(params.params.colors))])
                plot_linesegs.extend(plot_new_stuff)

        elif self.img_chosen == SHOW_MOTIONMODEL:
            if DEBUG_TRACKINGSETTINGS:
                print 'SHOW_MOTIONMODEL'
    ...
    def GetObsFiltered(self):
        if not (hasattr(self, 'obs_filtered') and self.obs_filtered.issame(self.show_frame)):
            if DEBUG_TRACKINGSETTINGS:
                print 'computing filtered observations'
    ...
    def GetObsUnfiltered(self, *args):
        ...
        if hasattr(self, 'obs_unfiltered') and self.obs_unfiltered.issame(self.show_frame):
            ...
            mustcompute = True
        if DEBUG_TRACKINGSETTINGS:
            print 'mustcompute = ' + str(mustcompute)
        ...
        if len(args) == 0:
            ...
            if mustcompute:
                ...
                obs_unfiltered, ellsmall, elllarge, didlowerthresh, didmerge, diddelete, didsplit = ell.find_ellipses(self.bg_imgs.dfore.copy(), self.bg_imgs.cc.copy(), self.bg_imgs.ncc, return_vals=True)
                if DEBUG_TRACKINGSETTINGS:
                    print 'computed obs_unfiltered = ' + str(obs_unfiltered) + ', len = ' + str(len(obs_unfiltered))
                if DEBUG_TRACKINGSETTINGS:
                    print 'ellsmall = ' + str(ellsmall)
                if DEBUG_TRACKINGSETTINGS:
                    print 'elllarge = ' + str(elllarge)
                if DEBUG_TRACKINGSETTINGS:
                    print 'didlowerthresh = ' + str(didlowerthresh)
                if DEBUG_TRACKINGSETTINGS:
                    print 'didmerge = ' + str(didmerge)
                if DEBUG_TRACKINGSETTINGS:
                    print 'diddelete = ' + str(diddelete)
                if DEBUG_TRACKINGSETTINGS:
                    print 'didsplit = ' + str(didsplit)
                wx.EndBusyCursor()
                self.obs_unfiltered = StoredObservations(obs_unfiltered, self.show_frame, ellsmall, elllarge, didlowerthresh, didmerge, diddelete, didsplit)
                if DEBUG_TRACKINGSETTINGS:
                    print 'stored obs_unfiltered: '
                if DEBUG_TRACKINGSETTINGS:
                    print str(self.obs_unfiltered)
            ret = (
    ...
    def GetBgImage(self):
        if not self.bg_img_frame == self.show_frame:
            self.bg_imgs.dfore, self.bg_imgs.bw, self.bg_imgs.cc, self.bg_imgs.ncc = self.bg_imgs.sub_bg(self.show_frame)
    ...
    def GetTargetMotion(self):
        ...
        targetmotion = (
         target_prev, target_curr, target_pred)
        return targetmotion