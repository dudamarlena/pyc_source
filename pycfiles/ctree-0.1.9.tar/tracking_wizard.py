...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/tracking_wizard.py
# Compiled at: 2016-09-17 16:29:43
import os.path, random, sys
from threading import Timer
import time, numpy as num, wx
from wx import xrc
import bg_settings, codedir
...
RSRC_FILE = os.path.join(codedir.codedir, 'xrc', 'tracking_wizard.xrc')
if RSRC_FILE is None:
    RSRC_FILE = os.path.join('xrc', 'tracking_wizard.xrc')

class TrackingWizard:

    def __init__(self, parent_frame, bg, n_frames, get_frame_callback, recalculate_bg_callback, starttracking_callback, isSBFMF):
        ...
        for i in range(9):
            ...
            panel.Layout()
            self.img_winds.append({'panel': panel, 'stext': stext, 
               'canvas': canvas, 
               'framenumber': i, 
               'n_obs': 0})
        ...
        panel.SetAutoLayout(True)
        panel.Layout()
        self.single_wind = {'panel': panel, 'stext': None, 
           'canvas': canvas, 
           'framenumber': 0, 
           'n_obs': 0}
    ...
    def redraw(self):
        ...
        if winds_to_show is None:
            ...
            line_widths = []
            img_src = 'img'
            if hasattr(self.bg, 'center') and hasattr(self, 'img_size') and self.showing_image_type != params.SHOW_RAW_FRAME and self.showing_image_type != params.SHOW_BACKGROUND:
                for wind_data in winds_to_show:
                    img = num.zeros(self.img_size[:2], dtype=num.uint8) + 127
                    wind_data['canvas'].update_image_and_drawings(wind_data['canvas'].current_source, img, format=fmt)
                    wind_data['n_obs'] = 0
            ...
            for wind_data in winds_to_show:
                if self.showing_image_type == params.SHOW_RAW_FRAME:
                    ...
                    if hasattr(self.bg, 'center'):
                        ...
                        if params.n_bg_std_thresh != initial_high or params.n_bg_std_thresh_low != initial_low:
                        ...
                        img_src = 'blank'
                    try:
                        wind_data['canvas'].update_image_and_drawings(img_src, img, format=fmt, linesegs=line_segs, lineseg_colors=line_clrs, lineseg_widths=line_widths)
    ...
    def remember_button_states(self):
        ...
        if self.__button_reset_timer_running:
            return
    ...
    def OnQuitCircularArena(self, evt):
        ...
        params.arena_center_x, params.arena_center_y, params.arena_radius = self.roi.GetArenaParameters()
        self.bg.UpdateIsArena()
    ...
    def OnLowThreshChanged(self, evt):
        ...
        new_thresh = self.slider.GetThumbPosition()
        if new_thresh <= params.n_bg_std_thresh and new_thresh != params.n_bg_std_thresh_low:
    ...
    def choose_shape_method(self):
        ...
        self.new_control_radio_box(["don't use shape bounds", 'auto-compute shape bounds', 'manually set bounds'])
        self.frame.Layout()
