...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/psutil/_pssunos.py
# Compiled at: 2016-09-17 16:22:08
__doc__ = 'Sun OS Solaris platform implementation.'
...
def swap_memory():
    ...
    p = subprocess.Popen(['/usr/bin/env',
     'PATH=/usr/sbin:/sbin:%s' % os.environ['PATH'], 'swap', '-l'], stdout=subprocess.PIPE)
...
class Process(object):
    ...
    @wrap_exceptions
    def cmdline(self):
        return cext.proc_name_and_args(self.pid, self._procfs_path)[1].split(' ')
    ...
    @wrap_exceptions
    def uids(self):
        real, effective, saved, _, _, _ = cext.proc_cred(self.pid, self._procfs_path)
        return _common.puids(real, effective, saved)

    @wrap_exceptions
    def gids(self):
        _, _, _, real, effective, saved = cext.proc_cred(self.pid, self._procfs_path)
        return _common.puids(real, effective, saved)
    ...
    def terminal(self):
        ...
        hit_enoent = False
        tty = wrap_exceptions(cext.proc_basic_info(self.pid, self._procfs_path)[0])
        if tty != cext.PRNODEV:
            for x in (0, 1, 2, 255):
                try:
