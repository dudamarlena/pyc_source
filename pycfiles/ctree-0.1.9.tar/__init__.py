...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/psutil/__init__.py
# Compiled at: 2016-09-17 16:22:08
__doc__ = 'psutil is a cross-platform library for retrieving information on\nrunning processes and system utilization (CPU, memory, disks, network)\nin Python.\n'
from __future__ import division
import collections, errno, functools, os, signal, subprocess, sys, time, traceback
try:
    import pwd
except ImportError:
    pwd = None
...
if LINUX:
    ...
    if WINDOWS:
        ...
        if int(__version__.replace('.', '')) != getattr(_psplatform.cext, 'version', None):
            msg = 'version conflict: %r C extension module was built for another version of psutil (different than %s)' % (
    ...
    """
        ...
        """
            ...
            def children(self, recursive=False):
                u"""Return the children of this process as a list of Process
        ...
        """
                ...
                fun = self.memory_full_info if memtype in ('uss', 'pss', 'swap') else self.memory_info
                metrics = fun()
...
', None)
    ...
    """
        ...
        _as_dict_attrnames = set([ x for x in dir(Process) if not x.startswith('_') and x not in [
...
def test():
    ...
    print templ % ('USER', 'PID', '%CPU', '%MEM', 'VSZ', 'RSS', 'TTY', 'START', 'TIME',
                   'COMMAND')
    for p in process_iter():
        try:
