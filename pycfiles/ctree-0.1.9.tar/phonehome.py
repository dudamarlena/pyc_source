...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/phonehome.py
# Compiled at: 2013-09-24 00:57:19
import httplib, re, threading
...
class HomePhoner(threading.Thread):
    ...
    def parse_address(self, address, verbose=False):
        ...
        if verbose:
            print 'removed http', address
        if re.search('^s', address) is not None:
            use_ssl = True
            address = re.sub('^s', '', address)
            if verbose:
                print 'removed s', address
        address = re.sub('^\\:\\/\\/', '', address)
        if verbose:
            print 'removed separator', address
