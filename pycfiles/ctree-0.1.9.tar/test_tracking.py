...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Ctrax/test_tracking.py
# Compiled at: 2014-04-26 18:39:36
import multiprocessing, os, os.path
from Queue import Empty as QueueEmpty
import random, shutil, sys, time, traceback, numpy as num, annfiles as ann, bg, bg_pre4, ellipsesk as ell, ellipsesk_pre4 as ell_pre4, hindsight as hs
from params import params, ShapeParams
params.interactive = False
params.enable_feedback(False)
import movies, test_filelist
num.random.seed(0)
TEST_ANN_DIR = '/home/jbender/ann_data_test'
AREA_RANGE = range(1, 10000, 100)
MAJOR_RANGE = num.arange(0.5, 10.0, 0.5)
MINOR_RANGE = MAJOR_RANGE.copy()
ECC_RANGE = num.arange(0.1, 1.001, 0.1)
MINBACKTHRESH_RANGE = num.arange(0.5, 10.0, 1.0)
...
ANGDIST_RANGE = num.arange(1.0, 500.0, 20.0)
MAXJUMP_RANGE = num.arange(10.0, 500.0, 25.0)
MINJUMP_RANGE = num.arange(10.0, 250.0, 25.0)
DAMPEN_RANGE = num.arange(0.0, 1.001, 0.1)
ANGLEDAMP_RANGE = num.arange(0.0, 1.001, 0.1)
PARAM_NAMES = [
 'minbackthresh', 'maxclustersperblob', 'maxpenaltymerge', 'maxareadelete', 'minareaignore', 'max_n_clusters', 'ang_dist_wt', 'max_jump', 'min_jump', 'dampen', 'angle_dampen']
...
def setup_tracking(moviefile, pre4):
    ...
    bg_img_shape = (
     movie.get_height(), movie.get_width())
    ...
    return (
     movie, bg_model, ann_file, hindsight)
...
def run_movie_pre4(moviefile, comms):
    ...
    if quit_val.value > 0:
        ...
        for frame in range(max_frames):
            ...
            if PRINT_COUNTS:
                print 'pre4', frame, 'ncc', ncc
            ellipses = ell_pre4.find_ellipses(dfore, cc, ncc)
            if PRINT_COUNTS:
                print 'pre4', frame, 'found ellipses', len(ellipses)
            try:
            ...
            if len(ann_file) > 1:
                flies = ell.find_flies(ann_file[(-2)], ann_file[(-1)], ellipses, ann_file)
            else:
                if len(ann_file) == 1:
                    flies = ell.find_flies(ann_file[(-1)], ann_file[(-1)], ellipses, ann_file)
                else:
                    ...
                    if PRINT_COUNTS:
                        print 'pre4', frame, 'found flies', len(flies)
        ...
        for frame in range(max_frames):
            ...
            if len(ann_file) > 1:
                ...
                if len(ann_file) == 1:
                    ...
                    if cmp_list is not None:
                        counts.append(len(flies))
                    ann_file.append(flies)
                    if PRINT_COUNTS and (frame == PRINT_FRAME or PRINT_FRAME is None):
                        print frame, 'pre-hindsight', len(ann_file[(-1)])
                    if cmp_list is not None:
                        counts.append(len(ann_file[(-1)]))
                    hindsight.fixerrors()
                    if PRINT_COUNTS and (frame == PRINT_FRAME or PRINT_FRAME is None):
...
def do_all_for_movie(moviefile, quit_vals):
    ...
    comms = {'quit_val': quit_vals[0], 'cmp_list': cmp_list, 
       'shape_dict': shape_dict}
    proc_pre4 = run_func(run_movie_pre4, moviefile, comms)
    comms = {'quit_val': quit_vals[1], 'cmp_list': cmp_list, 
       'shape_dict': shape_dict}
...
def test_tracking_settings(moviefile):
    ...
    if True:
        ...
        for c in range(n_combos):
            minshape = ShapeParams(random.choice(MAJOR_RANGE), random.choice(MINOR_RANGE), random.choice(AREA_RANGE), random.choice(ECC_RANGE))
            meanshape = ShapeParams(random.choice(MAJOR_RANGE), random.choice(MINOR_RANGE), random.choice(AREA_RANGE), random.choice(ECC_RANGE))
            maxshape = ShapeParams(random.choice(MAJOR_RANGE), random.choice(MINOR_RANGE), random.choice(AREA_RANGE), random.choice(ECC_RANGE))
            others = {'minbackthresh': random.choice(MINBACKTHRESH_RANGE), 'maxclustersperblob': random.choice(MAXCLUSTPER_RANGE), 
        ...
        minshape = ShapeParams(7.5, 5.0, 9500.0, 0.9)
        meanshape = ShapeParams(7.5, 4.5, 4800.0, 0.9)
        maxshape = ShapeParams(4.0, 3.5, 3500.0, 0.3)
        others = {'minbackthresh': 9.5, 'maxclustersperblob': 4.0, 
           'maxpenaltymerge': 0.0, 
           'maxareadelete': 5.0, 
           'minareaignore': 9600.0, 
           'max_n_clusters': 190.0, 
           'ang_dist_wt': 121.0, 
           'max_jump': 360.0, 
           'min_jump': 185.0, 
           'dampen': 1.0, 
    ...
    for p in range(n_proc):
        ...
        flags = (multiprocessing.Value('i', 0),
         multiprocessing.Value('i', 0))
        proc = multiprocessing.Process(target=do_combos, args=(
         combos[lo:hi], flags))
        proc.start()
        procs.append(proc)
...
if __name__ == '__main__':
    ...
    print 'total', (time.time() - st) / 60.0, 'min'
    print '\n...all tests passed'