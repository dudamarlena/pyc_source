...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/s3am/operations.py
# Compiled at: 2016-11-03 16:06:47
from __future__ import absolute_import
import base64, functools, random
...
import time, sys, abc, os, pycurl, multiprocessing, itertools
...
from s3am import me, log, ObjectExistsError, UploadExistsError, InvalidSourceURLError, InvalidDestinationURLError, InvalidS3URLError, IncompatiblePartSizeError, InvalidEncryptionKeyError, WorkerException, DownloadExistsError, ObjectExistsError, FileExistsError, MultipleUploadsExistError
from s3am.boto_utils import work_around_dots_in_bucket_names, s3_connect_to_region, bucket_location_to_region, bucket_location_to_http_url
...
class SSEKey(namedtuple('_SSEKey', 'binary is_master')):
    ...
    def resolve_against(self, operation):
        ...
        :rtype: SSEKey
        """
        return self.resolve(bucket_location=operation.bucket_location, bucket_name=operation.bucket_name, key_name=operation.key_name)
...
class Operation(object):
    ...
    def get_bucket_location(self, bucket, headers=None):
        response = bucket.connection.make_request('GET', bucket.name, query_args='location', headers=headers)
        body = response.read()
        ...
        raise bucket.connection.provider.storage_response_error(response.status, response.reason, body)
...
class BucketOperation(Operation):
    ...
    def __init__(self, s3_url, **kwargs):
        ...
        if s3_url.scheme == 's3' and s3_url.netloc and s3_url.path.startswith('/'):
            ...
            raise InvalidS3URLError("The URL '%s' is not a valid S3 URL. An S3 URL must be of the form s3://BUCKET/ or s3://BUCKET/KEY." % s3_url)
    ...
    def _parse_path_or_url(cls, path_or_url):
        ...
        if url.scheme == 'file' and url.netloc not in ('', 'localhost'):
            raise InvalidSourceURLError("'%s' is neither a valid file:// URL nor a path. For absolute paths use file:/ABSOLUTE/PATH/TO/FILE, file:///ABSOLUTE/PATH/TO/FILE or just /ABSOLUTE/PATH/TO/FILE. For relative paths use RELATIVE/PATH/TO/FILE. To refer to a file called FILE in the current working directory, use FILE." % path_or_url)
        return url
...
class BucketModification(BucketOperation):
    ...
    def _get_uploads(self, bucket, limit=max_uploads_per_page, allow_prefix=False):
        ...
        headers = self._get_default_headers()
        uploads = bucket.get_all_multipart_uploads(prefix=self.key_name, max_uploads=limit, headers=headers)
        if len(uploads) == max_uploads_per_page:
            raise RuntimeError("Can't handle more than %i uploads" % max_uploads_per_page)
        uploads = [ MultiPartUploadPlus(bucket=bucket, key_name=upload.key_name, upload_id=upload.id, headers=headers) for upload in uploads if allow_prefix or upload.key_name == self.key_name
                  ]
        return uploads
...
class MultiPartUploadPlus(MultiPartUpload):
    ...
    def get_all_parts(self, max_parts=None, part_number_marker=None, encoding_type=None):
        ...
        response = conn.make_request('GET', self.bucket.name, self.key_name, query_args=query_args, headers=self._headers)
        body = response.read()
        if response.status == 200:
    ...
    def complete_upload(self, headers=None):
        ...
        """
        return self.bucket.complete_multipart_upload(self.key_name, self.id, self.to_xml(), headers=headers)
...
class Cancel(BucketModification):
    ...
    def __init__(self, dst_url, allow_prefix, **kwargs):
        ...
        if (self.key_name.endswith('/') or self.key_name == '') and not allow_prefix:
            raise InvalidDestinationURLError("Make sure the destination URL does not end in / or pass --prefix if you really intend to delete uploads for all objects whose key starts with '%s'." % self.key_name)
...
class Upload(BucketModification):
    ...
    def __init__(self, src_url, dst_url, resume=False, force=False, exists=None, part_size=min_part_size, download_slots=num_cores, upload_slots=num_cores, sse_key=None, src_sse_key=None, **kwargs):
        """
    ...
    def run(self):
        ...
        """
        global done_event
        global download_slots_semaphore
        global error_event
        ...
        for _ in range(num_workers):
            workers.apply_async(functools.partial(time.sleep, random.random() + 0.5))
        ...
        if self.url.startswith('s3:'):
            ...
            with closing(S3Connection()) as (s3):
                ...
                src_key = url.path[1:]
                if self.src_sse_key:
                    self.src_sse_key = self.src_sse_key.resolve(bucket_location=self.get_bucket_location(src_bucket, headers=headers), bucket_name=src_bucket.name, key_name=src_key)
            ...
            kwargs = dict(src_bucket_name=src_bucket.name, src_key_name=src_key.name, size=src_key.size)
        else:
        ...
        try:
            while not done_event.is_set():
                ...
                if part_num in completed_parts:
                    ...
                    workers.apply_async(func=worker_func, args=[
                     upload_id, part_num], kwds=kwargs, callback=complete_part)
            ...
            with closing(s3_connect_to_region(self.bucket_region)) as (s3):
                headers = self._get_default_headers()
                bucket = s3.get_bucket(self.bucket_name, headers=headers)
                upload = MultiPartUploadPlus(bucket=bucket, key_name=self.key_name, upload_id=upload_id, parts=completed_parts.values())
        ...
        with closing(s3_connect_to_region(self.bucket_region)) as (s3):
            ...
            try:
                if bucket.get_key(self.key_name, headers=headers):
                    if self.exists == 'overwrite':
                        log.warn('Key (%s) already exists in bucket (%s). Overwriting the data.', self.key_name, self.bucket_name)
                    elif self.exists == 'skip':
                        log.warn('Key (%s) already exists in bucket (%s). Skipping.', self.key_name, self.bucket_name)
                        sys.exit(0)
                    else:
                ...
                if err.status == 403:
                    if self.sse_key:
                        raise InvalidEncryptionKeyError('The destination URL exists and the input key could not be used to access it (got a 403 error from S3).  This usually means the key differs from the one used for encrypting the object when it was uploaded.')
                    else:
                        raise InvalidEncryptionKeyError('The destination URL exists but could not be accessed (got a 403 error from S3).  This suggests that the remote file is encrypted. This operation cannot be completed without the encryption key that was used when the object was uploaded.')
                else:
                    raise
            ...
            while True:
                if len(uploads) == 0:
                    ...
                    upload_id = bucket.initiate_multipart_upload(key_name=self.key_name, headers=headers).id
                    return (
                     upload_id, {})
                if self.resume:
                    if len(uploads) == 1:
                        ...
                        if len(completed_parts) > 0:
                            ...
                            if self.part_size != previous_part_size:
                                raise IncompatiblePartSizeError('Transfer failed. The part size appears to have changed from %i to %i. Either resume the upload with the previous part size or cancel it before using the new part size.' % (
                                 self.part_size, previous_part_size))
                        return (
                         upload.id, completed_parts)
                    raise MultipleUploadsExistError(("Transfer failed. There are multiple unfinished uploads, so there is ambiguity as to which one of them should be resumed. Consider using '{me} cancel s3://{bucket_name}/{key_name}' to delete all of them before trying the transfer again. Note that unfinished uploads incur storage fees.").format(me=me, **vars(self)))
                elif self.force:
                    log.warn('Cancelling all unfinished uploads before proceeding.')
                    for upload in uploads:
                        bucket.cancel_multipart_upload(upload.key_name, upload.id, headers=self._get_default_headers())

                    uploads = []
                elif len(uploads) == 1:
                    raise UploadExistsError(("Transfer failed. There is an unfinished upload. To resume that upload, run {me} again with --resume. To cancel it, use '{me} cancel s3://{bucket_name}/{key_name}'. Note that unfinished uploads incur storage fees.").format(me=me, **vars(self)))
                else:
                    raise MultipleUploadsExistError(("Transfer failed. Detected unfinished multipart uploads. Consider using '{me} cancel s3://{bucket_name}/{key_name}' to delete all of them before trying the transfer again. Note that pending uploads incur storage fees.").format(me=me, **vars(self)))

    def _guess_part_size(self, completed_parts):
        size_groups = self._part_size_histogram(completed_parts)
        assert len(size_groups) > 0
        if len(size_groups) > 2 or len(size_groups) == 2 and size_groups[1][1] != 1:
            raise RuntimeError("Can't reliably determine previously used part size for this upload. You should probably cancel it and start over.")
    ...
    def _log_progress(self, part_num, task, download_size):
        log.info('part %i: %s %sB (%i bytes)', part_num, task, bytes2human(download_size), download_size)
    ...
    def _download_part(self, part_num):
        ...
        with closing(pycurl.Curl()) as (c):
            ...
            try:
                ...
                if error_code == c.E_BAD_DOWNLOAD_RESUME:
                    pass
    ...
    def _upload_part(self, upload_id, part_num, buf):
        ...
        with closing(s3_connect_to_region(self.bucket_region)) as (s3):
            headers = self._get_default_headers()
            bucket = s3.get_bucket(self.bucket_name, headers=headers)
            upload = MultiPartUploadPlus(bucket=bucket, key_name=self.key_name, upload_id=upload_id, headers=headers)
            if self.sse_key:
        ...
        try:
            ...
            if part_size > 0 or part_num == 0:
                ...
                with closing(s3_connect_to_region(self.bucket_region)) as (s3):
                    ...
                    if part_size == 0:
                        assert part_num == 0
                        key = upload.upload_part_from_file(StringIO(), part_num + 1, headers=headers)
                    else:
                        if self.src_sse_key:
                            self._add_encryption_headers(self.src_sse_key, headers, for_copy=True)
                        key = upload.copy_part_from_key(src_bucket_name=src_bucket_name, src_key_name=src_key_name, part_num=part_num + 1, start=start, end=end - 1, headers=headers)
                        key.size = part_size
                assert key.size == part_size
                self._log_progress(part_num, 'copied', part_size)
                return (
                 part_num, self._part_for_key(bucket, part_num, key))
            done_event.set()
    ...
    def _sanity_check(self, completed_parts):
        ...
        if len(groups) == 1:
            assert xor(sentinel(*groups[0]), one_or_more_full_parts(*groups[0]), one_last_part(*groups[0]))
        elif len(groups) == 2:
    ...
    def _part_size_histogram(self, completed_parts):
        ...
        completed_parts = [ (part_num, part.size) for part_num, part in completed_parts.iteritems()
                          ]
        ...
        return [ (part_size, ilen(group)) for part_size, group in itertools.groupby(completed_parts, by_part_size)
               ]
...
class Verify(Operation):
    ...
    def __init__(self, url, checksum, sse_key=None, part_size=min_part_size, **kwargs):
        ...
        url = urlparse(url)
        if url.scheme != 's3':
            raise NotImplementedError("Only 's3://' URLs are currently supported, not '%s://'." % url.scheme)
        if not url.netloc or not url.path.startswith('/'):
            raise InvalidS3URLError('Invalid S3 URL for destination (%s). Must be of the form s3://BUCKET/ or s3://BUCKET/KEY.' % url)
        self.url = url
    ...
    def run(self):
        ...
        with closing(S3Connection()) as (s3):
            ...
            while start < size:
                ...
                key.open_read(headers=dict(headers, Range='bytes=%i-%i' % (start, end - 1)))
                try:
                    while True:
...
class Download(BucketOperation):
    ...
    def __init__(self, src_url, dst_url, file_exists=None, download_exists=None, part_size=min_part_size, download_slots=num_cores, checksum_slots=num_cores, sse_key=None, **kwargs):
        """
        ...
        if parsed_dst_url.scheme != 'file':
            raise InvalidDestinationURLError("The destination URL must start with file://, '%s' does not" % dst_url)
        if any(parsed_dst_url.path.endswith(suffix) for suffix in ('.partial', '.progress')):
            raise InvalidDestinationURLError("The destination URL can't end in '.partial' or '.progress' since those suffixes are reserved for use by this program.")
    ...
    def run(self):
        ...
        if os.path.exists(self.dst_path):
            if self.file_exists is None:
                raise FileExistsError("The file '%s' already exists. Use --file-exists=overwrite to overwrite the file with the object downloaded from S3 or --file-exists=skip to skip downloading the file." % self.dst_path)
        ...
        with self.DownloadProgress(key) as (progress):
            error_event = multiprocessing.Event()
            download_slots_semaphore = multiprocessing.Semaphore(self.download_slots)
            workers = multiprocessing.Pool(processes=self.download_slots + self.checksum_slots, initializer=_init_worker)
            try:
                start = 0
                ...
                while start < key.size:
                    ...
                    if md5 is None:
                        ...
                        workers.apply_async(self._download, args=(
                         part, start, end), callback=progress.record_progress)
                    else:
                        log.info('part %i: exists, dispatching for verification.', part)
                        workers.apply_async(self._verify, args=(
                         part, start, end, md5), callback=progress.record_progress)
                    start = end
                ...
                if not destination_existed and self.file_exists != 'overwrite':
                    if os.path.exists(self.dst_path):
                        raise FileExistsError("The file '%s' was created while we were downloading the object from S3. Use --file-exists=overwrite to overwrite the file with the object downloaded from S3 if you expect this to happen. Consider adding --download-exists=resume to avoid redownloading the object. " % self.dst_path)
    ...
    def __download(self, part, start, end):
        try:
            ...
            with closing(s3_connect_to_region(self.bucket_region)) as (s3):
                headers = self._get_default_headers()
                bucket = s3.get_bucket(bucket_name=self.bucket_name, validate=False)
                if self.sse_key:
                    self._add_encryption_headers(sse_key=self.sse_key, headers=headers)
    ...
    def _verify(self, part, start, end, md5):
        ...
        try:
            ...
            if hashlib.md5(buf).digest() != md5:
                log.warn('part %i: Destination changed since last download. Downloading part again.', part)
                semaphore = None
                return self.__download(part, start, end)
    ...
    class DownloadProgress(object):
            ...
            try:
                ...
                try:
                    if self.outer.download_exists is None:
                        raise DownloadExistsError("A partial download exists at '%s'. Use --download-exists=discard to discard it and start from scratch or --download-exists=resume to resume it." % self.file_path)
        ...
        def _load(self, f):
            ...
            if part_size != self.outer.part_size:
                raise IncompatiblePartSizeError('Part size changed from %(old)i to %(new)i between download attempts. Resume this download with --part-size=%(old)i or discard the partial download using --download-exists=discard.' % dict(old=part_size, new=self.outer.part_size))
            ...
            if etag == self.etag:
                ...
                for part in xrange(len(self.md5s)):
                    md5 = f.read(16)
                    if md5:
                        if md5 != '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00':
                            self.md5s[part] = md5
                    else:
                        break
                ...
                log.warn("Remote object etag changed from '%s' to '%s' since last download attempt. Downloading file again.", etag, self.etag)
                f.seek(0)
                f.truncate()
                self._save(f)
        ...
        def record_progress(self, (part, md5)):
            assert len(md5), 16
            assert md5 != '\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'
            self.md5s[part] = md5
