...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/edw/logger/events/content.py
...
class ObjectMovedEvent(ObjectEvent):
    ...
    def log_move(self, obj, event):
        ...
        result['OldPath'] = ('{}/{}').format(event.oldParent.absolute_url(), event.oldName)
        if event.oldParent == event.newParent:
            result['Type'] = 'Rename'
        return result

    def _skip(self, obj, event):
        return zci.IObjectRemovedEvent.providedBy(event) or zci.IObjectAddedEvent.providedBy(event)
