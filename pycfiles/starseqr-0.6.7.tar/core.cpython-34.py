...
# [GCC 8.4.0]
# Embedded file name: /mounts/isilon/data/eahome/q804348/ea_code/STAR-SEQR/starseqr_utils/core.py
# Compiled at: 2017-12-07 17:16:00
# Size of source mod 2**32: 14737 bytes
from __future__ import absolute_import, division, print_function
from six import reraise as raise_
import os, sys, re, logging, pandas as pd, numpy as np, warnings
from collections import OrderedDict
import pysam, starseqr_utils as su
try:
    maketrans = ''.maketrans
...
def choose_order(chrL1, posL1, chrL2, posL2):
    ...
    if mychrL1 == mychrL2:
        ...
        if int(chrList.index(mychrL1)) < int(chrList.index(mychrL2)):
            return 1
        if int(chrList.index(mychrL1)) > int(chrList.index(mychrL2)):
            return 2
...
def count_jxns(df):
    ...
    new_df = grouped_df.agg(OrderedDict([
     ('readid',
      OrderedDict([
       ('reads',
        lambda col: ','.join(col)), ('counts', 'count')])), ('overhang_len', 'max')])).reset_index()
...
def get_pairs_func(jxn, dd):
    ...
    forward = dd[chrom1][((dd[chrom1]['chrom2'] == chrom2) & (dd[chrom1]['pos1'] >= pos1left) & (dd[chrom1]['pos1'] <= pos1right) & (dd[chrom1]['pos2'] >= pos2left) & (dd[chrom1]['pos2'] <= pos2right))]
    reverse = dd[chrom2][((dd[chrom2]['chrom2'] == chrom1) & (dd[chrom2]['pos1'] >= pos2left) & (dd[chrom2]['pos1'] <= pos2right) & (dd[chrom2]['pos2'] >= pos1left) & (dd[chrom2]['pos2'] <= pos1right))]
...
def exons2seq(fasta_path, lol_exons, jxn, side, fusion_exons='', decorate='', out_dir='support'):
    ...
    if fusion_exons:
        ...
        for trx_exons in lol_exons:
            seq = []
            for ex_order, chrom, start, end, strand, trx in trx_exons:
                if fa.references:
            ...
            if seq:
                ...
                for fus_exons in fusion_exons:
                    fus_seq = []
                    for fus_ex_order, fus_chrom, fus_start, fus_end, fus_strand, fus_trx in fus_exons:
                        if fa.references:
        ...
        for ntrx_exons in lol_exons:
            ...
            ntrx = ''
            nseq_str = ''
            for nex_order, nchrom, nstart, nend, nstrand, ntrx in ntrx_exons:
                if fa.references:
