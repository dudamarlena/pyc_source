...
# [GCC 8.4.0]
# Embedded file name: /mounts/isilon/data/eahome/q804348/ea_code/STAR-SEQR/starseqr_utils/annotate_sv.py
...
def overlap_exon(interval, pos, side):
    ...
    try:
        ...
        for idx, x in enumerate(starts):
            if pos in range(starts[idx], ends[idx] + 1):
                if interval['strand'] == '+':
                    if side == 1:
                        dist = ends[idx] - pos
                    else:
...
def get_jxnside_anno(jxn, gtree, side):
    ...
    if len(resL) > 0:
        for idx, val in enumerate(resL):
            ann['symbol'].append(resL[idx]['name2'])
    ...
    return [ds['symbol'][0], ann_string, ds['strand'][0], ds['cdslen'][0]]
