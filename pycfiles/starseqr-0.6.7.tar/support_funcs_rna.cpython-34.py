...
# [GCC 8.4.0]
# Embedded file name: /mounts/isilon/data/eahome/q804348/ea_code/STAR-SEQR/starseqr_utils/support_funcs_rna.py
# Compiled at: 2017-12-07 17:16:00
# Size of source mod 2**32: 15892 bytes
from __future__ import absolute_import, division, print_function
import six
from six import reraise as raise_
import os, sys, re, time, logging, subprocess as sp, pysam, collections, numpy as np, starseqr_utils as su
try:
    maketrans = ''.maketrans
...
def find_support_reads(jxn, bam, side, tx, sub_reads, gtree, chimflag):
    ...
    retDict = {}
    lmode = [
     'spanleft', 'spanright', 'jxnleft', 'jxnright', 'hangleft', 'hangright']
    lstrand = ['for', 'rev']
    ...
    for read in bam:
        mymode = ''
        if read.next_reference_name == chrom2 and read.reference_name == chrom1 and not read.flag & 2 and not read.flag & chimflag:
            if int(read.reference_start) > pos1left:
                if int(read.reference_start) < pos1right and int(read.next_reference_start) > pos2left and int(read.next_reference_start) < pos2right:
                    mymode = 'span'
        else:
            if read.next_reference_name == read.reference_name and read.flag & 2 and not read.flag & chimflag:
                if int(read.reference_start) > pos1left and int(read.reference_start) < pos1right or int(read.next_reference_start) > pos2left and int(read.next_reference_start) < pos2right:
                    mymode = 'jxn'
            elif read.next_reference_name == chrom2:
                if read.reference_name == chrom1:
                    if not read.flag & 2 and read.flag & chimflag and (int(read.reference_start) > pos1left and int(read.reference_start) < pos1right or int(read.next_reference_start) > pos2left and int(read.next_reference_start) < pos2right):
                        mymode = 'hang'
        if mymode:
            ...
            if is_subset == 'SUB':
                ...
                if set(tx).intersection(set(read_tx)):
                    if set(tx).intersection(set(read_tx2)):
                        retDict[mytype][(mystrand, myorient)][read.query_name] = (
                         is_subset,
                         read.get_tag('AS'),
                         read.get_tag('nM'),
...
def bam2fastq(in_bam, bam_type, jxn_dir, chimflag):
    ...
    if bam_type == 'span':
        ...
        for read in bamObject.fetch(until_eof=True):
            ...
            out_fq.write(seq + '\n')
            out_fq.write('+\n')
            out_fq.write(''.join(list(map(chr, [x + 33 for x in quals]))) + '\n')
        ...
        for read in bamObject.fetch(until_eof=True):
            if not read.flag & chimflag:
                ...
                out_fq.write('+\n')
                out_fq.write(''.join(list(map(chr, [x + 33 for x in read.query_qualities]))) + '\n')
            elif read.flag & chimflag:
                out_fq2.write('@' + read.query_name + '_' + str(read.flag) + '\n')
                out_fq2.write(read.query_alignment_sequence + '\n')
                out_fq2.write('+\n')
                out_fq2.write(''.join(list(map(chr, [x + 33 for x in read.query_alignment_qualities]))) + '\n')
                continue
...
def get_rna_support(jxn, tx, s_reads, in_bam, gtree, chimflag):
    ...
    for ktype in extracted:
        for ktuple in extracted[ktype]:
            ...
            for kname, val in six.iteritems(extracted[ktype][ktuple]):
                ...
                vun, vas, vmm, vseqlen, vmeanbq = val
    ...
    bam2fastq(splitbam, 'split', jxn_dir, chimflag)
