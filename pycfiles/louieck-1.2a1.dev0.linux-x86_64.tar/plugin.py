...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/louieck/plugin.py
# Compiled at: 2018-05-31 11:06:53
"""Common plugins for louieck."""
from louieck import dispatcher
...
def install_plugin(plugin):
    ...
    for p in dispatcher.plugins:
        if p.__class__ is cls:
            raise error.PluginTypeError(('Plugin of type {0!r} already installed.').format(cls))
...
class Plugin(object):
    """Base class for louieck plugins.

    Plugins are used to extend or alter the behavior of louieck
    in a uniform way without having to modify the louieck code
    itself.
    """
    ...
    def is_live(self, receiver):
        ...
        be live by default louieck semantics.
        """
        return True
...
class QtWidgetPlugin(Plugin):
    """A Plugin for louieck that knows how to handle Qt widgets
    ...
    found that is just an empty shell, it prevents louieck from
    ...
    def is_live(self, receiver):
        ...
        if hasattr(receiver, 'im_self') and isinstance(receiver.im_self, self.qt.QWidget):
            try:
                receiver.im_self.x()
            except RuntimeError:
                return False
...
class TwistedDispatchPlugin(Plugin):
    """Plugin for louieck that wraps all receivers in callables
