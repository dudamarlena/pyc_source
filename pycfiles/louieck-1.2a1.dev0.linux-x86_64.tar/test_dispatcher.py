...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/louieck/test/test_dispatcher.py
# Compiled at: 2018-05-31 10:36:55
import gc, unittest, louie
...
class TestDispatcher(unittest.TestCase):
    ...
    def test_GarbageCollected(self):
        ...
        assert result == expected, ("Send didn't return expected result:\n\texpected:{0}\n\tgot:{1}").format(expected, result)
        assert len(list(louie.get_all_receivers(b, signal))) == 0, ('Remaining handlers: {0}').format(louie.get_all_receivers(b, signal))
        self._isclean()
    ...
    def test_MultipleRegistration(self):
        ...
        assert len(list(louie.get_all_receivers(b, signal))) == 1, ('Remaining handlers: {0}').format(louie.get_all_receivers(b, signal))
        del a
        del b
        del result
        gc.collect()
