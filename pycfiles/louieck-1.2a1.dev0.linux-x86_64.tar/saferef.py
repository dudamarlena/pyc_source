...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/louieck/saferef.py
    ...
    if hasattr(target, 'im_self'):
        if target.im_self is not None:
            if not hasattr(target, 'im_func'):
                raise AssertionError(("safe_ref target {0!r} has im_self, but no im_func, don't know how to create reference").format(target))
                reference = BoundMethodWeakref(target=target, on_delete=on_delete)
                return reference
        if hasattr(target, '__self__') and target.__self__ is not None:
            assert hasattr(target, '__func__'), ("safe_ref target {0!r} has __self__, but no __func__, don't know how to create reference").format(target)
            reference = BoundMethodWeakref(target=target, on_delete=on_delete)
            return reference
...
class BoundMethodWeakref(object):
            ...
            for function in methods:
                try:
                    ...
                    try:
                        ...
                        print ('Exception during saferef {} cleanup function {}: {}').format(self, function, e)
    ...
    def __str__(self):
        ...
        return ('{0}({1}.{2})').format(self.__class__.__name__, self.self_name, self.func_name)
