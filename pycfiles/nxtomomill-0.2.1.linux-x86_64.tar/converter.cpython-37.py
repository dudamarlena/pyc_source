...
# Compiled at: 2020-04-30 05:35:29
# Size of source mod 2**32: 31421 bytes
"""
        ...
        for i_acquisition, acquisition in enumerate(acquisitions):
            if entries is not None:
                if acquisition.initialization_entry.name not in entries:
                    continue
                acquisition.is_valid() or _logger.error('unable to write nexus file for %s' % acquisition.initialization_entry.name)
            else:
                if single_file:
...
class _Acquisition:
 ...
 L. 362        56  LOAD_CONST               False
               58  RETURN_VALUE     
             60_0  COME_FROM            40  '40'

 L. 364        60  LOAD_CONST               True
               62  RETURN_VALUE     
...
Parse error at or near `FOR_ITER' instruction at offset 4
 ...
 L. 626        40  LOAD_FAST                'self'
               42  LOAD_ATTR                n_frames
               44  LOAD_CONST               None
               46  COMPARE_OP               is
               48  POP_JUMP_IF_FALSE    60  'to 60'

 L. 627        50  LOAD_GLOBAL              _logger
 ...
 L. 628        60  LOAD_FAST                'self'
               62  LOAD_ATTR                dim_1
               64  LOAD_CONST               None
               66  COMPARE_OP               is
               68  POP_JUMP_IF_FALSE    80  'to 80'

 L. 629        70  LOAD_GLOBAL              _logger
 ...
 L. 630        80  LOAD_FAST                'self'
               82  LOAD_ATTR                dim_2
               84  LOAD_CONST               None
               86  COMPARE_OP               is
               88  POP_JUMP_IF_FALSE   100  'to 100'

 L. 631        90  LOAD_GLOBAL              _logger
 ...
 L. 634       120  LOAD_GLOBAL              ValueError
 ...
 L. 638       156  LOAD_CONST               0
              158  STORE_FAST               'last'

 L. 639       160  SETUP_LOOP          212  'to 212'
              162  LOAD_GLOBAL              zip
              164  LOAD_FAST                'self'
 ...
 L. 643       212  LOAD_FAST                'detector_node'
