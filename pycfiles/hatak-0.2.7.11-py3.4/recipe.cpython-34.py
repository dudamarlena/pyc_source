...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bael/hatak/recipe.py
# Compiled at: 2015-05-15 05:51:54
...
from .tasks import CreateDataDir, Serve, BaelfireInitFile, ProjectTemplates, Develop, Shell, Tests, Coverage
from .templates import InitPy, Routes, Settings, FrontendIni, TestFixtures, Conftest, Pytestini, TestSettings, RedmeFile, ManagePy, PShell
from .uwsgi import UwsgiStart, UwsgiStop, UwsgiRestart
from .alembic import AlembicData, AlembicMigration, AlembicRevision, AlembicInit
...
class HatakRecipe(Recipe):
    ...
    def create_settings(self):
        ...
        self.set_path('uwsgi:socket', None, '/tmp/%(package:name)s.uwsgi.socket')
        ...
        self.set_path('venv:site-packages', 'virtualenv_path', 'lib/python3.4/site-packages/')
        ...
        self.set_path('exe:manage', 'virtualenv:bin', '%(package:name)s_manage')
        self.set_path('exe:pytest', 'virtualenv:bin', 'py.test')
    ...
    def final_settings(self):
        ...
        self.settings['entry_points'] = '\r\t[paste.app_factory]\n\t\tmain = %(package:name)s.application.init:main\n\t[console_scripts]\n\t\t%(package:name)s_manage = %(package:name)s.application.manage:run\n'
