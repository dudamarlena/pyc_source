...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hatak/application.py
# Compiled at: 2015-03-22 15:44:52
...
class Application(object):
    __doc__ = 'Prepering and configuring project.'
    route_class = Route
    ...
    def get_settings(self, module, settings={}, additional_modules=None):
        ...
        settings, paths = self.factory.make_settings(settings=settings, additional_modules=additional_modules)
        settings['paths'] = paths
        return (settings, paths)

    def create_config(self):
        self.config = Configurator(settings=self.settings.to_dict())
    ...
    def _validate_dependency_plugin(self, plugin):
        if plugin not in self.plugin_types:
