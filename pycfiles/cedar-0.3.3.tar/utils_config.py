...
# Embedded file name: /data/work/tmp/git/ceda-cc/ceda_cc/ceda_cc_config/utils_config.py
# Compiled at: 2017-06-26 09:31:41
__doc__ = 'A set of classes running checks and providing utilities to support checks'
 ...
 'standard_name', 'long_name', 'units', 'cell_methods']):
    ...
    def scan_table(self, ll, log, asDict=False, appendTo=None, lax=False, tag=None):
        ...
        if not asDict:
            ...
            if appendTo != None:
                for k in ff.keys():
                    if not ff[k][1].has_key('standard_name'):
                        raise AssertionError('No standard name in %s:: %s' % (k, str(ff[k][1].keys())))
                        if appendTo.has_key(k):
                            ...
                            assert lax or ff[k][1] == appendTo[k][1], 'Inconsistent entry definitions %s:: %s [%s] --- %s [%s]' % (k, ff[k][1], ff[k][2], appendTo[k][1], appendTo[k][2])
                    else:
                        appendTo[k] = ff[k]
...
class patternControl(object):
    ...
    def __init__(self, tag, pattern, list=None, cls=None, examples=None, badExamples=None, runTest=True):
        if cls is not None:
            ...
            if cls == 'ISO':
                if not pattern in ('ISO8601 duration', ):
                    raise AssertionError('value of pattern [%s] for ISO constraint not recognised' % pattern)
            ...
            self.list = list
            self.cls = cls
            if runTest and examples is not None:
                for e in examples:
