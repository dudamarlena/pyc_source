...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/library/manager.py
    ...
    def item_from_data_type(self, path, data_type):
        ...
        if not self.is_type_registered(data_type):
            ...
            if len(valid_items) > 1:
                qt.logger.warning('Multiple data file supports data type {}:\n{}'.format(data_type, '\n'.join(valid_items)))
    ...
    def get_folder_data_instance(self):
        ...
        if not self.settings:
            ...
            if not self.name:
                ...
                data_type or qt.logger.warning('Impossible to instantiate Data Folder because given Data Type: {} is not valid!'.format(data_type))
                return
