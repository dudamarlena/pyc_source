...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/playblastmanager/widgets/playblastmanager.py
...
class PlayblastManager(artellapipe.ToolWidget, object):
            ...
            for token in tokens:
                for token_name, token_info in token.items():
                    if 'rule' in token_info:
                        artellapipe.PlayblastsMgr().register_token(token_name,
                          (lambda attrs_dict: artellapipe.PlayblastsMgr().get_project_rule_token(token_info['rule'])),
                          label=(token_info['label']))
                    elif 'fn' not in token_info:
                        LOGGER.warning('Impossible to register token "{}" because its function is not defined!'.format(token_name))
                    else:
                        if not hasattr(artellapipe.PlayblastsMgr(), token_info['fn']):
                            LOGGER.warning('Impossible to register token "{}" because PlayblastMgr does not implements its function: "{}"'.format(token_name, token_info['fn']))
                        artellapipe.PlayblastsMgr().register_token(token_name,
                          (lambda attrs_dict: getattr(artellapipe.PlayblastsMgr(), token_info['fn'])()),
                          label=(token_info['label']))
    ...
    def ui(self):
        ...
        self.preset_widget = presets.PlayblastPreset(project=(self._project),
          inputs_getter=(self.get_inputs),
          config=(self.config),
          parent=self)
    ...
    def get_plugins_paths(self):
        ...
        :return: list(str)
        """
        plugins_path = path_utils.clean_path(os.path.join(os.path.dirname(os.path.os.path.dirname(os.path.abspath(__file__))), 'plugins'))
        return [plugins_path]
    ...
    def get_inputs(self, as_preset=False):
        ...
        for playblast_plugin in self._plugins:
            if playblast_plugin.id in inputs:
                LOGGER.warning('Cannot get inputs from plugin "{}" because Playblast widget/plugin has been already processed!'.format(playblast_plugin.id))
            else:
                plugin_inputs = playblast_plugin.get_inputs(as_preset=as_preset)
                if not isinstance(plugin_inputs, dict):
                    LOGGER.warning('Plyablast plugin inputs are not a valid dictionary "{0}" : "{1}"'.format(playblast_plugin.id, plugin_inputs))
                else:
                        ...
                        for cname, obj in inspect.getmembers(mod, inspect.isclass):
                            if issubclass(obj, plugin.PlayblastPlugin):
                                if cname in plugins_found:
                                    LOGGER.warning('Playblast Plugin with name "{}" is already registered! Overriding ...'.format(cname))
            ...
            for plugin_to_register in plugins_to_register:
                for plugin_name, plugin_class in all_plugins.items():
                    if plugin_name in registered_plugins:
                        ...
                        if plugin_class.id == plugin_to_register:
                            if plugin_class.can_be_registered():
    ...
    def _on_capture(self):
        ...
        if playblast_path and os.path.isfile(options['filename']):
            ...
            if do_stamp:
                filename = artellapipe.PlayblastsMgr().stamp_playblast((options['filename']),
                  filename, extra_dict=options)
            else:
                ...
                if sequence_name:
                    if shot_name:
                        if task_name:
                            ...
                            shot_found or LOGGER.warning('No shot found with name: "{}"!'.format(shot_name))
                    else:
                    ...
                    if shots_tasks:
                        for shot_task in shots_tasks:
                            if shot_task.name == task_name:
                                file_uploaded = artellapipe.Tracker().upload_shot_task_preview((shot_task.id),
                                  comment=comment, preview_file_path=file_to_upload)
                                break

                    file_uploaded or LOGGER.warning('It was not possible to upload "{}" to "{}". Please upload it manually!'.format(file_to_upload, artellapipe.Tracker().get_name()))
                    return False
            else:
...
class PlayblastTemplateConfigurationDialog(dialog.ArtellaDialog, object):
    ...
    def __init__(self, project, parent=None, **kwargs):
        ...
        (super(PlayblastTemplateConfigurationDialog, self).__init__)(name='PlayblastTemplateConfigurationDialog', 
