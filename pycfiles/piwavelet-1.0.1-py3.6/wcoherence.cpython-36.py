...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/piwavelet/wcoherence.py
# Compiled at: 2020-03-23 10:49:43
# Size of source mod 2**32: 13325 bytes
...
import pylab, matplotlib.dates as mdates, matplotlib.pyplot as plt
...
RETURN:
    ...
    def plot(self, t, title, units, **kwargs):
        ...
        """
        return (self._Wcoherence__plotWC)(wc=self.Rqs, t=t, coi=self.coi, freqs=self.freqs, 
         signif=self.wtcsig, 
         title=title, 
         units=units, **kwargs)
    ...
    def __plotWC(self, wc, t, coi, freqs, signif, title, units='days', **kwargs):
        ...
        if timeDT:
            ...
            if gray is True:
                cf = ax.contourf(t, (log2(period)), Power, Levels, cmap=(plt.cm.gray),
                  norm=norml,
                  extend=extend)
            else:
                cf = ax.contourf(t, (log2(period)), Power, Levels, cmap=(plt.cm.jet),
                  norm=norml,
                  extend=extend)
        if pSigma:
            ax.contour(t, (log2(period)), sig95, [-99, 1], colors='k', linewidths=2.0)
        else:
            if pArrow:
                ax.quiver((t[::da[1]]), (log2(period)[::da[0]]), (u[::da[0], ::da[1]]),
                  (v[::da[0], ::da[1]]),
                ...
                if zoom:
                    ...
                    ax.fill((concatenate([t[:1] - dt, t, t[-1:] + dt, t[-1:] + dt,
                     t[:1] - dt, t[:1] - dt])),
                      (log2(concatenate([[1e-09], coi, [1e-09],
                     period[-1:], period[-1:], [1e-09]]))),
                      'k',
                      alpha=0.3, hatch='x')
                    Yticks = 2 ** arange(ceil(log2(period.min())), ceil(log2(period.max())))
