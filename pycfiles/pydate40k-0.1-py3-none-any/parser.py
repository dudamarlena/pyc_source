...
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pydatcom/parser.py
# Compiled at: 2012-08-18 16:45:45
import sys, os, re, string, ply.yacc as yacc, ply.lex as lex
...
class Parser(object):
    ...
    def __init__(self, file_name=None, debug=0, keep_parse_tab=False):
        self.debug = debug
        ...
        try:
            ...
            modname = 'parser_' + self.__class__.__name__
...
class DatcomParser(Parser):
    ...
    re_dynamictable = 'DYNAMIC\\ DERIVATIVES\\n(?P<config>.*)\\n(?P<case>.*)\\n(.*\\n){2}(?P<condition_headers>(.*\\n){2})(?P<condition_units>.*)\\n(?P<conditions>.*)\\n(.*\\n){2}0(?P<deriv_headers>.*)\\n0.*\\n(?P<deriv_table>([^\\d\\n].*\\n)+)'
    re_statictable = 'CHARACTERISTICS\\ AT\\ ANGLE\\ OF\\ ATTACK.*\\n(?P<config>.*(\\n.*POWER.*)?)\\n(?P<case>.*)\\n(.*\\n){2}(?P<condition_headers>(.*\\n){2})(?P<condition_units>.*)\\n(?P<conditions>.*)\\n(.*\\n){1}0(?P<deriv_headers>.*)\\n0.*\\n(?P<deriv_table>([^\\d\\n].*\\n)+)'
    re_symflptable = 'CHARACTERISTICS\\ OF\\ HIGH\\ LIFT\\ AND\\ CONTROL\\ DEVICES.*\\n(?P<config>.*)\\n(?P<case>.*)\\n(.*\\n){1}(?P<condition_headers>(.*\\n){2})(?P<condition_units>.*)\\n(?P<conditions>.*)\\n(.*\\n){1}0(?P<deriv_headers>.*)\\n(.*\\n){2}(?P<deriv_table>([^\\d\\n].*\\n)+)(.*\\n){2}.*INDUCED\\ DRAG\\ COEFFICIENT\\ INCREMENT.*\\n.*DELTA\\ =(?P<deflection>.*)\\n(.*\\n){2}(?P<drag_table>([^\\d\\n].*\\n)+)'
    re_asyflptable = 'CHARACTERISTICS\\ OF\\ HIGH\\ LIFT\\ AND\\ CONTROL\\ DEVICES.*\\n(?P<config>.*)\\n(?P<case>.*)\\n(.*\\n){1}(?P<condition_headers>(.*\\n){2})(?P<condition_units>.*)\\n(?P<conditions>.*)\\n(.*\\n){1}.*\\(DELTAL-DELTAR\\)=(?P<deflection>.*)\\n(.*\\n){2}(?P<yaw_table>([^0].*\\n)+)(.*\\n){3}(?P<roll_table>([^1].*\\n)+)'
    states = (
     ('CASE', 'exclusive'),
     ('INPUT', 'exclusive'))
    reserved_INPUT = {'TRIM': 'TRIM', 
       'DIM': 'DIM', 
       'DAMP': 'DAMP', 
       'PART': 'PART', 
       'DERIV': 'DERIV', 
    ...
    tokens = [
     ...
     'BOOL',
     'CASEID',
     'NAMELIST'] + reserved_INPUT.values()
    t_ANY_LPAREN = '\\('
    t_ANY_RPAREN = '\\)'
    t_INITIAL_ignore = ' \t'
    t_ANY_ignore = ' \t'
    ...
    def t_CASE_DYNAMICTABLE(self, t):
        ...
        t.value = {'deriv_table': match.group('deriv_table')}
        return t
    ...
    def t_CASE_ASYFLPTABLE(self, t):
        match = t.lexer.lexmatch
        t.value = {'deflection': match.group('deflection'), 
    ...
    def t_ANY_error(self, t):
        print (
         "Illegal character '%s' at line" % t.value[0], t.lexer.lineno)
        t.lexer.skip(1)
        sys.exit(1)
    ...
    def p_statictable(self, p):
        ...
        self.cases[(-1)]['STATIC'] = self.parse_table1d([
         ...
         [
          'XCP', 9], ['CLA', 13], ['CMA', 13],
         [
          'CYB', 13], ['CNB', 14], ['CLB', 13]], p[1]['deriv_table'])

    def p_symflptable(self, p):
        """statement : SYMFLPTABLE"""
        data = {}
        data['DERIV'] = self.parse_table1d([
         [
          'DELTA', 12], ['D(CL)', 10],
         [
          'D(CM)', 11], ['D(CL MAX)', 10],
         ...
         [
          '(CH)A', 12], ['(CH)D', 11]], p[1]['deriv_table'])
        data['ALPHA'], data['CD'] = self.parse_table2d(9, [
         15, 10, 10, 10, 10, 10, 10, 10, 10], p[1]['drag_table'])
        data['DELTA'] = self.parse_vector(p[1]['deflection'])
        self.cases[(-1)]['SYMFLP'] = data

    def p_asymflptable(self, p):
        """statement : ASYFLPTABLE"""
        data = {}
        data['ALPHA'], data['CN'] = self.parse_table2d(7, [
         18, 12, 12, 12, 12, 12, 12, 12, 12], p[1]['yaw_table'])
        data['ROLL'] = self.parse_table1d([
         [
          'DELTAL', 51], ['DELTAR', 16],
         [
          'CL(ROLL)', 22]], p[1]['roll_table'])
        data['DELTA'] = self.parse_vector(p[1]['deflection'])
