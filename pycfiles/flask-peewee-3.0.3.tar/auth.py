...
# [GCC 8.4.0]
# Embedded file name: ../flask_peewee/auth.py
...
class Auth(object):
    ...
    def __init__(self, app, db, user_model=None, prefix='/accounts', name='auth', clear_session=False, default_next_url='/', db_table='user'):
        self.app = app
        self.db = db
    ...
    def get_logged_in_user(self):
        if session.get('logged_in'):
            if getattr(g, 'user', None):
                return g.user
            try:
                return self.User.select().where(self.User.active == True, self.User.id == session.get('user_pk')).get()
            except self.User.DoesNotExist:
                pass
    ...
    def login(self):
        ...
        return render_template('auth/login.html', error=error, form=form, login_url=url_for('%s.login' % self.blueprint.name), next=next_url)
