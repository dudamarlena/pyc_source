...
# [GCC 8.4.0]
# Embedded file name: ../flask_peewee/admin.py
# Compiled at: 2018-07-09 10:14:19
...
class ModelAdmin(object):
    ...
    base_templates = {'index': 'admin/models/index.html', 
    ...
    def __init__(self, admin, model):
        ...
        self.action_map = dict((action.name, action) for action in self.actions or ())
    ...
    def get_filter_form(self):
        return FilterForm(self.model, self.filter_converter(self), self.filter_mapping(), self.filter_fields, self.filter_exclude)
    ...
    def get_form(self, adding=False):
        ...
        return model_form(self.model, allow_pk=allow_pk, only=self.fields, exclude=self.exclude, converter=self.form_converter(self))
    ...
    def apply_ordering(self, query, ordering):
        if ordering:
            ...
            if self.column_is_sortable(column):
                ...
                query = query.order_by((desc or field.asc)() if 1 else field.desc())
        return query
    ...
    def index(self):
        ...
        return render_template(self.templates['index'], model_admin=self, query=pq, ordering=ordering, filter_form=filter_form, field_tree=field_tree, active_filters=cleaned, **self.get_extra_context())

    def _index_redirect(self):
        url = session.get('%s.index' % self.get_admin_name()) or url_for(self.get_url_name('index'))
        return redirect(url)
    ...
    def dispatch_save_redirect(self, instance):
        if 'save' in request.form:
            ...
            if 'save_add' in request.form:
                return redirect(url_for(self.get_url_name('add')))
            return redirect(url_for(self.get_url_name('edit'), pk=instance._pk))
    ...
    def add(self):
        ...
        if request.method == 'POST':
            ...
            form = Form()
        return render_template(self.templates['add'], model_admin=self, form=form, instance=instance, **self.get_extra_context())
    ...
    def edit(self, pk):
        ...
        return render_template(self.templates['edit'], model_admin=self, instance=instance, form=form, **self.get_extra_context())
    ...
    def delete(self):
        ...
        if request.method == 'GET':
            ...
            return self._index_redirect()
        return render_template(self.templates['delete'], **dict(model_admin=self, query=query, collected=collected, **self.get_extra_context()))
    ...
    def export(self):
        ...
        return render_template(self.templates['export'], model_admin=self, model=query.model, query=query, filter_form=filter_form, field_tree=field_tree, active_filters=cleaned, related_fields=related, sql=query.sql(), **self.get_extra_context())
    ...
    def get_model_field(self, model, field):
        try:
            ...
            try:
                ...
                raise AttributeError('Could not find attribute or method named "%s".' % field)
            else:
                return attr(model)
...
class Admin(object):
    ...
    def __init__(self, app, auth, template_helper=AdminTemplateHelper, prefix='/admin', name='admin', branding='flask-peewee'):
        self.app = app
        self.auth = auth
        self._admin_models = {}
        self._registry = {}
    ...
    def index(self):
        return render_template('admin/index.html', model_admins=self.get_model_admins(), panels=self.get_panels())

    def get_blueprint(self, blueprint_name):
        return Blueprint(blueprint_name, __name__, static_folder=os.path.join(current_dir, 'static'), template_folder=os.path.join(current_dir, 'templates'))
    ...
    def configure_routes(self):
        ...
        for model_admin in self._registry.values():
            ...
            for url, callback in model_admin.get_urls():
                ...
                self.blueprint.add_url_rule(full_url, '%s_%s' % (admin_name, callback.__name__), self.auth_required(callback), methods=[
                 'GET', 'POST'])

        for panel in self._panels.values():
            for url, callback in panel.get_urls():
                full_url = '/%s%s' % (panel.slug, url)
                self.blueprint.add_url_rule(full_url, 'panel_%s_%s' % (panel.slug, callback.__name__), self.auth_required(callback), methods=[
                 'GET', 'POST'])
