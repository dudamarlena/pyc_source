...
# [GCC 8.4.0]
# Embedded file name: ../flask_peewee/filters.py
...
class WithinDaysAgoFilter(QueryFilter):
    ...
    def query(self, value):
        value = int(value)
        return self.field >= datetime.date.today() - datetime.timedelta(days=value)
    ...
    for field_obj in model._meta.sorted_fields:
        ...
        if isinstance(field_obj, ForeignKeyField):
            ...
            if no_explicit_fields:
                ...
                rel_fields = [ rf.replace('%s__' % field_obj.name, '') for rf in fields if rf.startswith('%s__' % field_obj.name) ]
                if not rel_fields and force_recursion:
                    rel_fields = None
            rel_exclude = [ rx.replace('%s__' % field_obj.name, '') for rx in exclude if rx.startswith('%s__' % field_obj.name)
                          ]
...
class FilterForm(object):
    ...
    def get_field_dict(self, node=None, prefix=None):
        ...
        for prefix, node in node.children.items():
            ...
            field_dict['%s%s' % (self.field_relation_prefix, prefix)] = fields.FormField(self.get_form(child_fd), separator=self.separator)

        return field_dict

    def get_form(self, field_dict):
        return type(self.model.__name__ + 'FilterForm', (
         self.base_class,), field_dict)
