...
# [GCC 8.4.0]
# Embedded file name: ../flask_peewee/rest.py
# Compiled at: 2018-02-09 10:01:56
...
class APIKeyAuthentication(Authentication):
    ...
    def get_key(self, k, s):
        try:
            return self.get_query().where(self._key_field == k, self._secret_field == s).get()
...
class RestResource(object):
    paginate_by = 20
    value_transforms = {'False': False, 'false': False, 'True': True, 
       'true': True, 'None': None, 
       'none': None}
    fields = None
    exclude = None
    ...
    def apply_filter(self, query, expr, op, arg_list, negated):
        ...
        if op == 'in':
            ...
            query_clauses = [ constructor({query_expr: val}) for val in arg_list ]
    ...
    def serialize_object(self, obj):
        ...
        return self.prepare_data(obj, s.serialize_object(obj, self._fields, self._exclude))

    def serialize_query(self, query):
        s = self.get_serializer()
        return [ self.prepare_data(obj, s.serialize_object(obj, self._fields, self._exclude)) for obj in query
               ]
    ...
    def get_request_metadata(self, paginated_query):
        ...
        return {'model': self.get_api_name(), 
           'page': current_page, 
           'previous': previous, 
           'next': next}
    ...
    def get_paginate_by(self):
        ...
        if self.paginate_by:
            paginate_by = min(paginate_by, self.paginate_by)
        return paginate_by
    ...
    def paginated_object_list(self, filtered_query):
        ...
        return self.response({'meta': meta_data, 
           'objects': query_dict})
    ...
    def configure_routes(self):
        ...
        for provider in self._registry.values():
            ...
            for url, callback in provider.get_urls():
                ...
                self.blueprint.add_url_rule(full_url, '%s_%s' % (api_name, callback.__name__), self.auth_wrapper(callback, provider), methods=provider.allowed_methods)
