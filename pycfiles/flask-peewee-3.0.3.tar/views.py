...
# [GCC 8.4.0]
# Embedded file name: /home/charles/code/flask-peewee/example/views.py
...
def join():
    if request.method == 'POST' and request.form['username']:
        try:
            ...
            user = User(username=request.form['username'], email=request.form['email'], join_date=datetime.datetime.now())
...
def user_follow(username):
    user = get_object_or_404(User, User.username == username)
    Relationship.get_or_create(from_user=auth.get_logged_in_user(), to_user=user)
