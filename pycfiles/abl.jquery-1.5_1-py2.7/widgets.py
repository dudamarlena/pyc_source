...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/abl/jquery/core/widgets.py
...
from tw.forms import FormFieldRepeater, TextField, TextArea
from abl.jquery.core import jquery_js
from abl.jquery.core.base import jQuery
page_widget_js = JSLink(modname=__name__, filename='static/javascript/jquery.ajax.pager.js', javascript=[
 jquery_js])
...
class AjaxPager(Widget):
    ...
    engine_name = 'genshi'
    params = dict(widget='The widget to be displayed and paginated.\n                            It will be called with the data given to the AjaxPager.', format='The format of the pager, default is\n                            $link_previous ~3~ $link_next . More Info: tg.decorators.paginate.Page', show_pager="Defines where to show the pager. Values can be\n                                'both', 'top', bottom. Default:'bottom'", pager_type="default is a regular 'horizontal' pager. 'vertical' will load more content\n                                instead of switching to the next page", symbol_next="default is 'next'", symbol_previous="default is 'previous'", symbol_more="default is 'more'. Only used if pager_type is 'vertical'")
    widget = Widget()
    show_pager = 'bottom'
    ...
    def update_params(self, d):
        ...
        d.pager = pager(d.format, symbol_next=d.symbol_next, symbol_previous=d.symbol_previous, link_attr=link_attr)
...
growing_js = JSLink(javascript=[
 jquery_js], modname=__name__, filename='static/javascript/jquery.growing.js')
...
class GrowingFormFieldRepeater(FormFieldRepeater):
    ...
    """
    params = [
     'add_link_text', 'delete_link_text', 'undo_link_text']
    add_link_text = 'add'
    ...
    def post_init(self, *args, **kw):
        ...
        removes empty lines.
        """
        self.validator = All(ForEach(self.children[0].validator), StripEmpty())
    ...
    def update_params(self, d):
        ...
        d['output'] = ('\n').join(form_elem_html % (rendered_w, extra_style, self.delete_link_text) for rendered_w, extra_style in outputs if rendered_w)
...
update_content_js = JSLink(modname=__name__, filename='static/javascript/jquery.update.content.js', javascript=[
 jquery_js])
...
default_text_css = CSSLink(modname=__name__, filename='static/css/default_text.css')
default_text_js = JSLink(javascript=[
 jquery_js], modname=__name__, filename='static/javascript/jquery.default_text.js')
