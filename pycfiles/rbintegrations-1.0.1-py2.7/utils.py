...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbintegrations/idonethis/utils.py
...
IDONETHIS_API_BASE_URL = b'https://beta.idonethis.com/api/v2'
TEAM_IDS_CACHE_EXPIRATION = 86400
...
def create_idonethis_request(request_path, api_token, json_payload=None):
    ...
    """
    url = b'%s/%s' % (IDONETHIS_API_BASE_URL, request_path)
    headers = {b'Authorization': b'Token %s' % api_token}
    if json_payload:
...
def get_user_api_token(user):
    ...
    """
    try:
        settings = user.get_profile().settings[b'idonethis']
        return decrypt_password(settings[b'api_token'])
    except KeyError:
        return
...
def get_user_team_ids(user):
    ...
    def _get_user_team_ids_uncached():
        request = create_idonethis_request(request_path=b'teams', api_token=api_token)
        logging.debug(b'IDoneThis: Loading teams for user "%s", request "%s %s"', user.username, request.get_method(), request.get_full_url())
        try:
            teams_data = urlopen(request).read()
        except (HTTPError, URLError) as e:
            if isinstance(e, HTTPError):
                error_info = b'%s, error data: %s' % (e, e.read())
            else:
                error_info = e.reason
            logging.error(b'IDoneThis: Failed to load teams for user "%s", request "%s %s": %s', user.username, request.get_method(), request.get_full_url(), error_info)
            raise

        return set(t[b'hash_id'] for t in json.loads(teams_data))
    ...
    if not api_token:
        ...
        try:
            return set(cache_memoize(_make_user_team_ids_cache_key(user), _get_user_team_ids_uncached, expiration=TEAM_IDS_CACHE_EXPIRATION))
        except Exception as e:
            logging.error(b'IDoneThis: Failed to load teams for user "%s": %s', user.username, e)
            return
...
def delete_cached_user_team_ids(user):
    ...
    """
    logging.debug(b'IDoneThis: Deleting cached team IDs for user "%s"', user.username)
...
def _make_user_team_ids_cache_key(user):
    ...
    """
    return b'idonethis-team_ids-%s' % user.username