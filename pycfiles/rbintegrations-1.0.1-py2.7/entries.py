...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/rbintegrations/idonethis/entries.py
...
default_template_strings = {REPLY_PUBLISHED: 'Replied to review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_REQUEST_COMPLETED: 'Completed review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_REQUEST_DISCARDED: 'Discarded review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_REQUEST_PUBLISHED: 'Published review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_REQUEST_REOPENED: 'Reopened review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_REQUEST_UPDATED: 'Updated review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED: 'Posted review on review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED_ISSUE: 'Posted review (1 issue) on review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED_ISSUES: 'Posted review (${num_issues} issues) on review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED_SHIPIT: 'Posted Ship it! on review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED_SHIPIT_ISSUE: 'Posted Ship it! (1 issue) on review request ${review_request_id}: ${summary} ${url} ${group_tags}', 
   REVIEW_PUBLISHED_SHIPIT_ISSUES: 'Posted Ship it! (${num_issues} issues) on review request ${review_request_id}: ${summary} ${url} ${group_tags}'}
INVALID_TAG_CHARS_RE = re.compile('\\W')
MULTIPLE_WHITESPACE_RE = re.compile('\\s+')

def format_template_string(template_string, num_issues=0, review_request=None, review_request_id=None, summary=None, group_tags=None, url=None):
    ...
    if review_request:
        ...
        if not group_tags:
            ...
            group_tags = (' ').join('#%s' % INVALID_TAG_CHARS_RE.sub('_', group_name) for group_name in group_names)
    result = string.Template(template_string).substitute(num_issues=num_issues, review_request_id=review_request_id, summary=summary, group_tags=group_tags, url=url)
