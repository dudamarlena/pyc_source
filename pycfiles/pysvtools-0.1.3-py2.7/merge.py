...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/pysvtools/merge.py
# Compiled at: 2015-11-16 18:35:32
    ...
    for record in vcf_reader:
        ...
        if SVTYPE == 'bITX':
            try:
                ...
                t = Event(record.CHROM, record.POS, record.CHROM, end, sv_type='TRA', cp_flank=centerpointFlanking, dp=extractDPFromRecord(record), svmethod=svmethod)
                svDB[t.virtualChr] = svDB.get(t.virtualChr, [])
                svDB[t.virtualChr].append(t)

        elif SVTYPE in ('CTX', 'TRA'):
            chrB, chrBpos = extractTXmate(record)
            t = Event(record.CHROM, record.POS, chrB, chrBpos, sv_type='TRA', cp_flank=centerpointFlanking, dp=extractDPFromRecord(record), svmethod=svmethod)
            try:
                svDB[t.virtualChr] = svDB.get(t.virtualChr, [])
            except:
                pass
            ...
            try:
                ...
                t = Event(record.CHROM, record.POS, record.CHROM, end, sv_type=SVTYPE, cp_flank=centerpointFlanking, dp=extractDPFromRecord(record), svmethod=svmethod)
            except:
...
def startMerge(vcf_files, exclusion_regions, output_file, centerpointFlanking, bedoutput, transonly=False, regions_out='regions_out.bed', vcf_output='output.vcf'):
    ...
    for virtualChr in natsorted(commonhits.keys()):
        for s, items in sorted(commonhits[virtualChr].items(), key=lambda hit: hit[1].items()[0][1].chrApos):
            if len(items):
                ...
                tsv_report_output.write(('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n').format(t.chrA, t.chrApos, t.chrB, t.chrBpos, t.sv_type, t.dp, t.size, ('\t').join(locations_found)))
