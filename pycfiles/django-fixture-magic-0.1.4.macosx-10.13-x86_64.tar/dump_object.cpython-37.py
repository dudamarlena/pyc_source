...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.7/site-packages/fixture_magic/management/commands/dump_object.py
...
import json
from fixture_magic.utils import add_to_serialize_list, serialize_me, seen, serialize_fully

class Command(BaseCommand):
    help = 'Dump specific objects from the database into JSON that you can use in a fixture.'
    args = "<[--kitchensink | -k] [--natural]  [--natural-primary]  '    '[--natural-foreign] [--query] object_class id [id ...]>"

    def add_arguments(self, parser):
        """Add command line arguments to parser"""
        parser.add_argument(dest='model', help='Name of the model, with app name first. Eg "app_name.model_name"')
        parser.add_argument(dest='ids', default=None, nargs='*', help='Use a list of ids e.g. 0 1 2 3')
        parser.add_argument('--kitchensink', '-k', action='store_true',
          dest='kitchensink',
          default=False,
        ...
        parser.add_argument('--natural-primary', action='store_true',
          dest='natural_primary',
          default=False,
          help='Use natural primary keys if they are available.')
        parser.add_argument('--natural-foreign', action='store_true',
          dest='natural_foreign',
          default=False,
          help='Use natural foreign keys if they are available.')
        parser.add_argument('--query', dest='query',
          default=None,
          help='Use a json query rather than list of ids e.g. \'{"pk__in": [id, ...]}\'')
        parser.add_argument('--no-follow', action='store_false',
          dest='follow_fk',
          default=True,
          help='does not serialize Foriegn Keys related to object')
        parser.add_argument('--format',
          default='json', dest='format', help='Specifies the output serialization format for fixtures.')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(error_text % 'object_class must be provided in the following format: app_name.model_name')
