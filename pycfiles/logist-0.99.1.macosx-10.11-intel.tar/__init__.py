...
# [GCC 8.4.0]
# Embedded file name: /Users/atm/dimension/lib/python2.7/site-packages/logist/__init__.py
# Compiled at: 2016-06-10 00:36:21
import json, os
...
class Logist(object):
    ...
    def __init__(self, redis_address='localhost', redis_port=6379, flush_count=10000, file_size=1000000000, log_file_name='default', log_folder='', namespace='DEFAULT', compression=True, disable_file_flush=False):
        """
    ...
    def config(self):
        ...
        """
        conf = {'REDIS_ADDRESS': self.REDIS_ADDRESS, 
            ...
            if self.COMPRESSION:
                ...
                if not os.path.isfile(uncompressed_file_name):
                    with open(log_file, 'r') as (f_in):
                        with open(uncompressed_file_name, 'w') as (f_out):
                            shutil.copyfileobj(f_in, f_out)
                            open(log_file, 'w').close()
                    return
            count += 1
    ...
    def count(self, date_from='', date_to='', log_type='', sub_type='', description='', log_source='redis', force_refresh=False):
        """
    ...
    def filter(self, date_from='', date_to='', log_type='', sub_type='', description='', log_source='redis', force_refresh=False):
        """
        Function to count the matching logs in last created file with filters log_type, sub_type and description
        :param date_to: filter logs till date_to - datetime object
        :param date_from: filter logs till date_from - datetime object
    ...
    def _export(self, flush=False, filelocation='', filename='', date_from='', date_to='', log_type='', sub_type='', description='', log_source='redis', force_refresh=False):
        """
        #TODO Flush Data
    ...
    def export(self, flush=False, filelocation='', filename='', date_from='', date_to='', log_type='', sub_type='', description='', log_source='redis', force_refresh=False):
        """
        #TODO Flush Data
        Function to export all availale data to a csv file
        :param flush : False - detemines to flush exported data
