...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/smsaero/tests.py
# Compiled at: 2016-11-05 20:31:25
# Size of source mod 2**32: 4546 bytes
...
class SmsSenderTest(TestCase):
    ...
    def _fake_send(self, params, link):
        return '123=accepted'

    @patch('smsaero.conf.SMSAERO_PASSWORD', 'FAKE')
    @patch('smsaero.utils.SmsSender.send_request', _fake_send)
    ...
    def test_get_encrypted_password(self):
        ...
        m = md5()
        m.update('FAKE'.encode())
        password = m.hexdigest()
        self.assertEqual(hashed, password, 'Encrypted pass not equal with MD5 hash')
...
class SmsAeroAPITest(TestCase):
    ...
    def test_send_sms(self):
        ...
        self._send_sms('+71234567890', 'Message1', signature)
        signature.delete()
