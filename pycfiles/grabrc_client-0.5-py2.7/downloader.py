...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.8-intel/egg/client/downloader.py
# Compiled at: 2013-01-28 15:21:28
"""
Functions for downloading files
"""
import StringIO, urllib2, zipfile, tarfile, shutil, util, glob
...
def _create_grabrc_folder(username, destdir, dirname):
    ...
    if os.path.isfile(repo_dirpath):
        util.warn('Found a file where there should be a git directory.                    Backing up...')
    ...
    if not os.path.exists(repo_dirpath):
        ...
        download_and_untar()
        repofiles = [ os.path.join(tmp_path, filename) for filename in os.listdir(tmp_path)
                    ]
        map(lambda f: shutil.move(f, repo_dirpath), repofiles)
        os.rmdir(tmp_path)
    else:
        util.exit_runtime_error("The repository's target directory exists at %s         but should have been backed up to a different location. Race condition?" % repo_dirpath)
...
def download_repo_nongit(options):
    ...
    if options.replace:
        ...
        util.print_msg('info', "Repository download doesn't support the --append option.                                 Falling back to default behavior of backing up the existing                                 directory")
    _create_grabrc_folder(options.github, options.destdir, options.outfile or Const.DEFAULT_DIRNAME)
...
def download_subdirectory(subdir_name, options):
    ...
    if options.append:
        util.warn("Append option doesn't apply to directories.         Falling to default behavior of backing up         the existing directory")
    ...
    try:
        _create_grabrc_folder(options.github, options.destdir, TMPDIR_NAME)
...
def download_file(filename, options):
    ...
    if not target_file_exists or options.append:
        ...
        util.exit_runtime_error('Please file a bug.', "(File download doesn't seem to cover all option cases)")
