...
# [GCC 8.4.0]
# Embedded file name: /home/seb/git/kipartman/kipartman/frames/buy_frame.py
# Compiled at: 2017-12-13 06:04:32
# Size of source mod 2**32: 37739 bytes
from dialogs.panel_buy import PanelBuy
from frames.order_options_dialog import OrderOptionsDialog
import wx.dataview
from currency.currency import Currency
from configuration import Configuration
import rest, helper.tree, math
...
class DataModelBomProduct(helper.tree.TreeItem):
    ...
    def GetValue(self, col):
        vMap = {0: self.bom_product.bom.filename, 
...
class BuyFrame(PanelBuy):
    ...
    def GetMenus(self):
        return [
         {'title': 'Prices', 'menu': self.menu_prices}]
    ...
    def OnMenuItem(self, event):
        if event.GetId() == self.menu_item_prices_view_all.GetId():
            ...
            if event.GetId() == self.menu_item_prices_select_bestprice.GetId():
    ...
    def onToolOpenBasketClicked(self, event):
        if self.basket.saved == False:
            ...
            dlg = wx.FileDialog(self, message='Choose a basket file', defaultDir=os.getcwd(), defaultFile='', wildcard='Kipartman basket (*.basket)|*.basket', style=wx.FD_OPEN | wx.FD_MULTIPLE | wx.FD_CHANGE_DIR | wx.FD_FILE_MUST_EXIST | wx.FD_PREVIEW)
    ...
    def onButtonAddBomClick(self, event):
        dlg = wx.FileDialog(self, message='Choose a BOM file', defaultDir=os.getcwd(), defaultFile='', wildcard='Kipartman BOM (*.bom)|*.bom', style=wx.FD_OPEN | wx.FD_MULTIPLE | wx.FD_CHANGE_DIR | wx.FD_FILE_MUST_EXIST | wx.FD_PREVIEW)
        dlg.SetFilterIndex(0)
        if dlg.ShowModal() == wx.ID_OK:
