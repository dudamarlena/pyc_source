...
# [GCC 8.4.0]
# Embedded file name: frames/schematic_frame.py
...
class DataModelPart(helper.tree.TreeItem):
    ...
    def GetValue(self, col):
        vMap = {0: self.component.reference, 
...
class SchematicFrame(PanelSchematic):
    ...
    def get_symbol(self, path):
        return re.sub('.*\\.lib/', '', path).replace('.mod', '')
    ...
    def onToolExportBomClicked(self, event):
        path = os.getcwd()
        dlg = wx.FileDialog(self, message='Save a bom file', defaultDir=path, defaultFile='new', wildcard='Kipartman bom (*.bom)|*.bom', style=wx.FD_SAVE | wx.FD_CHANGE_DIR)
