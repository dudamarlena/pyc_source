...
# [GCC 8.4.0]
# Embedded file name: frames/parts_frame.py
...
class PartsFrame(PanelParts):
    ...
    def export_parts(self):
        ...
        export_dialog = wx.FileDialog(self, 'Export Parts', '', '', wildcards, wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
    ...
    def import_parts(self):
        ...
        import_dialog = wx.FileDialog(self, 'Import Parts', '', '', wildcards, wx.FD_OPEN | wx.FD_FILE_MUST_EXIST)
        if import_dialog.ShowModal() == wx.ID_CANCEL:
            ...
            for i, importItem in enumerate(import_items):
                ...
                if progression_frame.Canceled():
                    break
                if isinstance(importItem.footprint, str) and len(importItem.footprint) == 0:
                    part.footprint = None
                else:
                    searchparam = {'search': importItem.footprint}
                    matching_footprints = rest.api.find_footprints(**searchparam)
                    if len(matching_footprints) == 0:
                        try:
                        ...
                        try:
                            ...
                            assert len(m_octopart.data) == 1, ('Import Octopart Lookup found {} matchs {}').format(len(m_octopart.data))
                            octopart_extractor = OctopartExtractor(m_octopart.data[0].json)
                            assert part.name == m_octopart.data[0].item().mpn(), ('Import Octopart Lookup found first part {}').format(m_octopart.data[0].item().mpn())
    ...
    def onTreePartsDropPart(self, x, y, data):
        ...
        if not dest_item.IsOk():
            ...
            if isinstance(dest_obj, DataModelPart):
                try:
                    source_part_id = data['id']
