...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/futurefinity/security.py
...
import io, os, hmac, time, base64, string, struct, random, hashlib
try:
    from cryptography.hazmat.primitives.ciphers import Cipher as AESCipher, algorithms as aes_algorithms, modes as aes_modes
...
def get_random_str(length: int) -> str:
    ...
    for i in range(0, length):
        random_string += random_generator.choice(string.ascii_letters + string.digits)
...
class BaseSecurityContext:
    ...
    def __init__(self, security_secret: str):
        self._security_secret = hashlib.sha256(ensure_bytes(security_secret)).digest()

    def lookup_origin_text(self, secure_text: str, valid_length: Optional[int]=None) -> Optional[str]:
        """
...
class HMACSecurityContext(BaseSecurityContext):
    ...
    def lookup_origin_text(self, secure_text: str, valid_length: Optional[int]=None) -> str:
        try:
...
class AESGCMSecurityContext(BaseSecurityContext):
    ...
    def __init__(self, security_secret: str):
        if None in [AESCipher, aes_algorithms, aes_modes, aes_backend]:
            raise FutureFinityError('Currently, `futurefinity.security.AESGCMSecurityContext` needs Cryptography to work. Please install it before using security features(such as security_secret), or turn aes_security to False in Application Settings.')
    ...
    def lookup_origin_text(self, secure_text: str, valid_length: Optional[int]=None) -> str:
        ...
        decryptor = AESCipher(aes_algorithms.AES(self._security_secret), aes_modes.GCM(iv, tag), backend=aes_backend()).decryptor()
        try:
    ...
    def generate_secure_text(self, origin_text: str) -> str:
        ...
        content = struct.pack('l', int(time.time()))
        content += ensure_bytes(origin_text)
        encryptor = AESCipher(aes_algorithms.AES(self._security_secret), aes_modes.GCM(iv), backend=aes_backend()).encryptor()
