...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/Visualize_Logs/objects/ProcMonCSV.py
# Compiled at: 2016-11-11 19:53:33
# Size of source mod 2**32: 72494 bytes
import networkx, pandas, os
...
class ProcMonCSV(object):
    ...
    def __init__(self, csvlogfile=None):
        ...
        if not os.path.exists(csvlogfile):
            raise Exceptions.VisualizeLogsInvalidFile(csvlogfile)
        else:
            self.csvlogfile = csvlogfile
        try:
            self.csvdata = pandas.read_csv(csvlogfile, low_memory=False)
        except:
            raise Exceptions.VisualizeLogsInvalidFileStructure(csvlogfile)

        if 'Date & Time' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Date & Time')
        if 'Time of Day' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Time of Day')
        if 'Process Name' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Process Name')
        if 'PID' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'PID')
        if 'Operation' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Operation')
        if 'Path' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Path')
        if 'Result' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Result')
        if 'Detail' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Detail')
        if 'TID' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'TID')
        if 'Duration' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Duration')
        if 'Image Path' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Image Path')
        if 'Command Line' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Command Line')
        if 'Parent PID' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Parent PID')
        if 'Event Class' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Event Class')
        if 'User' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'User')
        if 'Session' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Session')
        if 'Category' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Category')
        if 'Architecture' not in self.csvdata:
            raise Exceptions.VisualizeLogsMissingRequiredField(self.csvlogfile, 'Architecture')
        if 'Date & Time' in self.csvdata:
            self.csvdata['Date & Time'] = pandas.to_datetime(self.csvdata['Date & Time'])
        self.csvdata['Time'] = pandas.to_datetime(self.csvdata['Date & Time'].dt.date.map(str) + ' ' + self.csvdata['Time of Day'])
    ...
    def _addnodemetadata(self, i, node):
        ...
        """
        self.nodemetadata[i] = dict({'PID': node['PID'], 
         'Time': node['Time'], 
    ...
    def _addunknownpid(self, unkpid, row=None):
        ...
        if nodename not in self.digraph:
            self.digraph.add_node(nodename, type='Unknown PID', pid=unkpid)
        if row is not None:
            self.unkpidcmdline[nodename] = [
             row['Image Path'],
             row['Command Line']]
        return nodename
    ...
    def _addedgetohost(self, idx, host):
        ...
        if hostname not in self.digraph:
            self.digraph.add_node(hostname, type='host', host='"' + host + '"')
        if not self.digraph.has_edge(idx, hostname):
            self.digraph.add_edge(idx, hostname)
    ...
    def _addfile(self, file):
        ...
        if filename not in self.digraph:
            self.digraph.add_node(filename, type='file', filenum=filenum)
        return filename
    ...
    def _addedgetofile(self, idx, file):
        ...
        if not self.digraph.has_edge(idx, filename):
            self.digraph.add_edge(idx, filename)
    ...
    def _addreg(self, reg):
        ...
        if regname not in self.digraph:
            self.digraph.add_node(regname, type='reg', regnum=regnum)
        return regname
    ...
    def _addedgetoreg(self, idx, reg):
        ...
        if not self.digraph.has_edge(idx, regname):
            self.digraph.add_edge(idx, regname)
    ...
    def _constructgraph(self):
        ...
        for i, row in self.csvdata.iterrows():
            if row['Operation'] != 'Process Start':
                ...
                if row['Operation'] == 'TCP Connect' and self.plottcpconnects is True:
                    ...
                    if m:
                        ...
                        raise Exceptions.VisualizeLogsParseError(row['Path'])
                    if row['Operation'] == 'UDP Receive' and self.plotudprecvs is True:
                        ...
                        if m:
                            ...
                            raise Exceptions.VisualizeLogsParseError(row['Path'])
                        if row['Operation'] == 'UDP Send' and self.plotudpsends is True:
                            ...
                            if m:
                                ...
                                raise Exceptions.VisualizeLogsParseError(row['Path'])
                            if row['Operation'] == 'WriteFile' and self.plotfilewrites is True:
                                nodename = '"* PID {0} Writes File {1}"'.format(pidnode, row['Path'])
                                ...
                                if row['PID'] not in currentprocs:
                                    ...
                                    if not self.digraph.has_edge(currentprocs[row['PID']], realnodename):
                                        self.digraph.add_edge(currentprocs[row['PID']], realnodename)
                                    self._addedgetofile(realnodename, row['Path'])
                                if row['Operation'] == 'ReadFile' and self.plotfilereads is True:
                                    nodename = '"* PID {0} Reads File {1}"'.format(pidnode, row['Path'])
                                    ...
                                    if row['PID'] not in currentprocs:
                                        ...
                                        if not self.digraph.has_edge(currentprocs[row['PID']], realnodename):
                                            self.digraph.add_edge(currentprocs[row['PID']], realnodename)
                                        self._addedgetofile(realnodename, row['Path'])
                                    if row['Operation'] == 'SetDispositionInformationFile' and self.plotfiledeletes is True:
                                        nodename = '"* PID {0} Deletes File {1}"'.format(pidnode, row['Path'])
                                        if nodename not in self.filedeletetable:
                                            ...
                                            self.digraph.add_node(realnodename, type='SetDispositionInformationFile')
                                        else:
                                        ...
                                        if row['PID'] not in currentprocs:
                                            ...
                                            if not self.digraph.has_edge(currentprocs[row['PID']], realnodename):
                                                self.digraph.add_edge(currentprocs[row['PID']], realnodename)
                                            self._addedgetofile(realnodename, row['Path'])
                                        if row['Operation'] == 'SetRenameInformationFile' and self.plotfilerenames is True:
                                            m = re.search('.* FileName: (.*)$', row['Detail'])
                                            if m:
                                                destfile = m.group(1)
                                            else:
                                                raise Exceptions.VisualizeLogsParseError(row['Details'])
                                            nodename = '"* PID {0} Renames File {1} to {2}"'.format(pidnode, row['Path'], destfile)
                                            if nodename not in self.filerenametable:
                                                ...
                                                self.digraph.add_node(realnodename, type='SetRenameInformationFile')
                                            else:
                                            ...
                                            if row['Operation'] == 'RegSetValue' and self.plotregwrites is True:
                                                nodename = '"* PID {0} Writes Reg {1}"'.format(pidnode, row['Path'])
                                                ...
                                                if row['Operation'] == 'RegQueryValue' and self.plotregreads is True:
                                                    nodename = '"* PID {0} Reads Reg {1}"'.format(pidnode, row['Path'])
                                                    ...
                                                    if row['PID'] not in currentprocs:
                                                        ...
                                                        if not self.digraph.has_edge(currentprocs[row['PID']], realnodename):
                                                            self.digraph.add_edge(currentprocs[row['PID']], realnodename)
                                                        self._addedgetoreg(realnodename, row['Path'])
                                                    if row['Operation'] == 'RegDeleteValue' and self.plotregdeletes is True:
                                                        nodename = '"* PID {0} Deletes Reg {1}"'.format(pidnode, row['Path'])
        ...
        if self.graphvizprog is None:
            ...
            self.pos = networkx.drawing.nx_pydot.graphviz_layout(self.digraph, prog=self.graphvizprog)
    ...
    def _generateplot(self):
        ...
        for node in self.digraph:
            if self.digraph.node[node]['type'] == 'Process Start':
                ...
                proctxt.append('PID: {0}<br>Command: {1}<br>Time: {2}<br>User: {3}<br>Architecture: {4}<br>Parent PID: {5}'.format(self.nodemetadata[node]['PID'], self.nodemetadata[node]['Command Line'], self.nodemetadata[node]['Time'], self.nodemetadata[node]['User'], self.nodemetadata[node]['Architecture'], self.nodemetadata[node]['Parent PID']))
            if self.digraph.node[node]['type'] == 'Unknown PID':
                ProcessX.append(self.pos[node][0])
                ProcessY.append(self.pos[node][1])
                if node not in self.unkpidcmdline:
                    proctxt.append('Unknown PID: {0}'.format(self.digraph.node[node]['pid']))
                else:
                    proctxt.append('Unknown PID: {0}<br>Image Path: {1}<br>Command Line: {2}'.format(self.digraph.node[node]['pid'], self.unkpidcmdline[node][0], self.unkpidcmdline[node][1]))
                if self.digraph.node[node]['type'] == 'TCP Connect':
                    TCPConnectX.append(self.pos[node][0])
                    TCPConnectY.append(self.pos[node][1])
                    tcpconntxt.append('Time: {0}<br>Path: {1}<br>PID: {2}<br>Command: {3}<br>User: {4}<br>Architecture: {5}<br>'.format(self.nodemetadata[node]['Time'], self.nodemetadata[node]['Path'], self.nodemetadata[node]['PID'], self.nodemetadata[node]['Command Line'], self.nodemetadata[node]['User'], self.nodemetadata[node]['Architecture']))
                if self.digraph.node[node]['type'] == 'UDP Receive':
                    UDPReceiveX.append(self.pos[node][0])
                    UDPReceiveY.append(self.pos[node][1])
                    udprectxt.append('Time: {0}<br>Path: {1}<br>PID: {2}<br>Command: {3}<br>User: {4}<br>Architecture: {5}<br>'.format(self.nodemetadata[node]['Time'], self.nodemetadata[node]['Path'], self.nodemetadata[node]['PID'], self.nodemetadata[node]['Command Line'], self.nodemetadata[node]['User'], self.nodemetadata[node]['Architecture']))
                if self.digraph.node[node]['type'] == 'UDP Send':
                    UDPSendX.append(self.pos[node][0])
                    UDPSendY.append(self.pos[node][1])
                    udpsendtxt.append('Time: {0}<br>Path: {1}<br>PID: {2}<br>Command: {3}<br>User: {4}<br>Architecture: {5}<br>'.format(self.nodemetadata[node]['Time'], self.nodemetadata[node]['Path'], self.nodemetadata[node]['PID'], self.nodemetadata[node]['Command Line'], self.nodemetadata[node]['User'], self.nodemetadata[node]['Architecture']))
                if self.digraph.node[node]['type'] == 'host':
                    HostX.append(self.pos[node][0])
                    HostY.append(self.pos[node][1])
                    hosttxt.append('{0}'.format(self.digraph.node[node]['host'][1:-1]))
                ...
                if self.digraph.node[node]['type'] == 'SetDispositionInformationFile':
                    FileDeleteX.append(self.pos[node][0])
                    FileDeleteY.append(self.pos[node][1])
                    filedeletetxt.append('DELETE')
                if self.digraph.node[node]['type'] == 'SetRenameInformationFile':
        ...
        for edge in self.digraph.edges():
            if self.digraph.node[edge[1]]['type'] == 'Process Start' or self.digraph.node[edge[1]]['type'] == 'Unknown PID':
            ...
            if self.digraph.node[edge[1]]['type'] == 'TCP Connect' or self.digraph.node[edge[0]]['type'] == 'TCP Connect':
            ...
            if self.digraph.node[edge[1]]['type'] == 'UDP Receive' or self.digraph.node[edge[0]]['type'] == 'UDP Receive':
            ...
            if self.digraph.node[edge[1]]['type'] == 'UDP Send' or self.digraph.node[edge[0]]['type'] == 'UDP Send':
            ...
            if self.digraph.node[edge[1]]['type'] == 'WriteFile' or self.digraph.node[edge[0]]['type'] == 'WriteFile':
            ...
            if self.digraph.node[edge[1]]['type'] == 'ReadFile' or self.digraph.node[edge[0]]['type'] == 'ReadFile':
            ...
            if self.digraph.node[edge[1]]['type'] == 'SetDispositionInformationFile' or self.digraph.node[edge[0]]['type'] == 'SetDispositionInformationFile':
            ...
            if self.digraph.node[edge[0]]['type'] == 'SetRenameInformationFile' or self.digraph.node[edge[1]]['type'] == 'SetRenameInformationFile':
            ...
            if self.digraph.node[edge[0]]['type'] == 'file' and self.digraph.node[edge[1]]['type'] == 'file':
            ...
            if self.digraph.node[edge[1]]['type'] == 'Process Start' and self.digraph.node[edge[0]]['type'] == 'file' or self.digraph.node[edge[1]]['type'] == 'file' and self.digraph.node[edge[0]]['type'] == 'Process Start':
            ...
            if self.digraph.node[edge[1]]['type'] == 'RegSetValue' or self.digraph.node[edge[0]]['type'] == 'RegSetValue':
            ...
            if self.digraph.node[edge[1]]['type'] == 'RegQueryValue' or self.digraph.node[edge[0]]['type'] == 'RegQueryValue':
            ...
            if self.digraph.node[edge[1]]['type'] == 'RegDeleteValue' or self.digraph.node[edge[0]]['type'] == 'RegDeleteValue':
        ...
        ProcEdges = Scatter(x=ProcessXe, y=ProcessYe, mode='lines', line=Line(shape='linear'), name='Process Start', hoverinfo='none')
        nodes.append(ProcNodes)
        edges.append(ProcEdges)
        if self.plottcpconnects is True:
            marker = Marker(symbol='diamond', size=7)
            TCPConnectNodes = Scatter(x=TCPConnectX, y=TCPConnectY, mode='markers', marker=marker, name='TCP Connections', text=tcpconntxt, hoverinfo='text')
            TCPConnectEdges = Scatter(x=TCPConnectXe, y=TCPConnectYe, mode='lines', line=Line(shape='linear'), name='TCP Connect', hoverinfo='none')
        ...
        if self.plotudprecvs is True:
            ...
            UDPReceiveNodes = Scatter(x=UDPReceiveX, y=UDPReceiveY, mode='markers', marker=marker, name='UDP Receives', text=udprectxt, hoverinfo='text')
            UDPReceiveEdges = Scatter(x=UDPReceiveXe, y=UDPReceiveYe, mode='lines', line=Line(shape='linear'), name='UDP Receive', hoverinfo='none')
        ...
        if self.plotudpsends is True:
            ...
            UDPSendNodes = Scatter(x=UDPSendX, y=UDPSendY, mode='markers', marker=marker, name='UDP Sends', text=udpsendtxt, hoverinfo='text')
            UDPSendEdges = Scatter(x=UDPSendXe, y=UDPSendYe, mode='lines', line=Line(shape='linear'), name='UDP Send', hoverinfo='none')
            nodes.append(UDPSendNodes)
            edges.append(UDPSendEdges)
        if self.plottcpconnects is True or self.plotudprecvs is True or self.plotudpsends is True:
            marker = Marker(symbol='square', size=10)
            HostNodes = Scatter(x=HostX, y=HostY, mode='markers', marker=marker, name='Hosts', text=hosttxt, hoverinfo='text')
            nodes.append(HostNodes)
        if self.plotfilewrites is True:
            marker = Marker(symbol='triangle-down', size=7)
            FileWriteNodes = Scatter(x=FileWriteX, y=FileWriteY, mode='markers', marker=marker, name='File Writes', text=filewritetxt, hoverinfo='text')
            FileWriteEdges = Scatter(x=FileWriteXe, y=FileWriteYe, mode='lines', line=Line(shape='linear'), name='File Write', hoverinfo='none')
            nodes.append(FileWriteNodes)
            edges.append(FileWriteEdges)
        if self.plotfilereads is True:
            marker = Marker(symbol='triangle-up', size=7)
            FileReadNodes = Scatter(x=FileReadX, y=FileReadY, mode='markers', marker=marker, name='File Reads', text=filereadtxt, hoverinfo='text')
            FileReadEdges = Scatter(x=FileReadXe, y=FileReadYe, mode='lines', line=Line(shape='linear'), name='File Read', hoverinfo='none')
            nodes.append(FileReadNodes)
            edges.append(FileReadEdges)
        if self.plotfiledeletes is True:
            marker = Marker(symbol='triangle-down', size=7)
            FileDeleteNodes = Scatter(x=FileDeleteX, y=FileDeleteY, mode='markers', marker=marker, name='File Deletes', text=filedeletetxt, hoverinfo='text')
            FileDeleteEdges = Scatter(x=FileDeleteXe, y=FileDeleteYe, mode='lines', line=Line(shape='linear'), name='File Delete', hoverinfo='none')
            nodes.append(FileDeleteNodes)
            edges.append(FileDeleteEdges)
        if self.plotfilerenames is True:
            marker = Marker(symbol='triangle-down', size=7)
            FileRenameNodes = Scatter(x=FileRenameX, y=FileRenameY, mode='markers', marker=marker, name='File Renames', text=filerenametxt, hoverinfo='text')
            FileRenameEdges = Scatter(x=FileRenameXe, y=FileRenameYe, mode='lines', line=Line(shape='linear'), name='File Rename', hoverinfo='none')
            nodes.append(FileRenameNodes)
            edges.append(FileRenameEdges)
        if self.plotfilereads is True or self.plotfilewrites is True or self.plotfiledeletes is True or self.plotfilerenames is True:
            marker = Marker(symbol='hexagon', size=10)
            FileNodes = Scatter(x=FileX, y=FileY, mode='markers', marker=marker, name='Files', text=filetxt, hoverinfo='text')
            FileImageEdges = Scatter(x=FileImageXe, y=FileImageYe, mode='lines', line=Line(shape='linear', dash='dot'), name='Process Load Image', hoverinfo='none')
            FileRenamedEdges = Scatter(x=FileRenamedXe, y=FileRenamedYe, mode='lines', line=Line(shape='linear', dash='dot'), name='File Renamed', hoverinfo='none')
            nodes.append(FileNodes)
            edges.append(FileImageEdges)
            edges.append(FileRenamedEdges)
        if self.plotregwrites is True:
            marker = Marker(symbol='triangle-down', size=7)
            RegWriteNodes = Scatter(x=RegWriteX, y=RegWriteY, mode='markers', marker=marker, name='Registry Writes', text=regwritetxt, hoverinfo='text')
            RegWriteEdges = Scatter(x=RegWriteXe, y=RegWriteYe, mode='lines', line=Line(shape='linear'), name='Registry Write', hoverinfo='none')
            nodes.append(RegWriteNodes)
            edges.append(RegWriteEdges)
        if self.plotregreads is True:
            marker = Marker(symbol='triangle-up', size=7)
            RegReadNodes = Scatter(x=RegReadX, y=RegReadY, mode='markers', marker=marker, name='Registry Reads', text=regreadtxt, hoverinfo='text')
            RegReadEdges = Scatter(x=RegReadXe, y=RegReadYe, mode='lines', line=Line(shape='linear'), name='Registry Read', hoverinfo='none')
            nodes.append(RegReadNodes)
            edges.append(RegReadEdges)
        if self.plotregdeletes is True:
            marker = Marker(symbol='triangle-down', size=7)
            RegDeleteNodes = Scatter(x=RegDeleteX, y=RegDeleteY, mode='markers', marker=marker, name='Registry Deletes', text=regdeletetxt, hoverinfo='text')
            RegDeleteEdges = Scatter(x=RegDeleteXe, y=RegDeleteYe, mode='lines', line=Line(shape='linear'), name='Registry Delete', hoverinfo='none')
            nodes.append(RegDeleteNodes)
            edges.append(RegDeleteEdges)
        if self.plotregreads is True or self.plotregwrites is True or self.plotregdeletes is True:
            marker = Marker(symbol='star', size=10)
            RegNodes = Scatter(x=RegX, y=RegY, mode='markers', marker=marker, name='Registry Values', text=regtxt, hoverinfo='text')
    ...
    def _generateannotations(self):
        ...
        for node in self.digraph:
            if self.digraph.node[node]['type'] == 'Process Start' and self.showproclabels is True:
                annotations.append(Annotation(text='{0}<br>PID: {1}'.format(self.nodemetadata[node]['Process Name'], self.nodemetadata[node]['PID']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
            if self.digraph.node[node]['type'] == 'Unknown PID' and self.showproclabels is True:
                if node not in self.unkpidcmdline:
                    annotations.append(Annotation(text='UNKNOWN<br>PID: {0}'.format(self.digraph.node[node]['pid']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                else:
                    annotations.append(Annotation(text='{1}<br>PID: {0}'.format(self.digraph.node[node]['pid'], self.unkpidcmdline[node][0]), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'TCP Connect' and self.showtcplabels is True:
                    annotations.append(Annotation(text='TCP Connect<br>{0}<br>Time: {1}'.format(self.nodemetadata[node]['Path'], self.nodemetadata[node]['Time']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'UDP Receive' and self.showudplabels is True:
                    annotations.append(Annotation(text='UDP Receive<br>{0}<br>Time: {1}'.format(self.nodemetadata[node]['Path'], self.nodemetadata[node]['Time']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'UDP Send' and self.showudplabels is True:
                    annotations.append(Annotation(text='UDP Send<br>{0}<br>Time: {1}'.format(self.nodemetadata[node]['Path'], self.nodemetadata[node]['Time']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'host' and self.showhostlabels is True:
                    annotations.append(Annotation(text='Host<br>{0}'.format(self.digraph.node[node]['host'][1:-1]), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'WriteFile' and self.showfilelabels is True:
                    annotations.append(Annotation(text='WRITE', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'ReadFile' and self.showfilelabels is True:
                    annotations.append(Annotation(text='READ', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'SetDispositionInformationFile' and self.showfilelabels is True:
                    annotations.append(Annotation(text='DELETE', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'SetRenameInformationFile' and self.showfilelabels is True:
                    annotations.append(Annotation(text='RENAME', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'file' and self.showfilelabels is True:
                    for f in self.filetable:
                        if self.filetable[f] == self.digraph.node[node]['filenum']:
                            filename = f
                            break

                    annotations.append(Annotation(text='File<br>{0}'.format(filename), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'RegSetValue' and self.showreglabels is True:
                    annotations.append(Annotation(text='WRITE', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'RegQueryValue' and self.showreglabels is True:
                    annotations.append(Annotation(text='READ', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'RegDeleteValue' and self.showreglabels is True:
                    annotations.append(Annotation(text='DELETE', x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
                if self.digraph.node[node]['type'] == 'reg' and self.showreglabels is True:
                    for r in self.regtable:
                        if self.regtable[r] == self.digraph.node[node]['regnum']:
                            regname = r
                            break

                    annotations.append(Annotation(text='Registry<br>{0}'.format(regname), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))

        return annotations

    def plotgraph(self, graphvizprog='sfdp', showproclabels=True, showtcplabels=True, showudplabels=True, showfilelabels=True, showhostlabels=True, showreglabels=True, plottcpconnects=True, plotudpsends=True, plotudprecvs=True, plotfilereads=True, plotfilewrites=True, plotfiledeletes=True, plotfilerenames=True, plotregwrites=True, plotregreads=True, plotregdeletes=True, ignorepaths=None, includepaths=None, filename='temp-plot.html', title=None, auto_open=True, image=None, image_filename='plot_image', image_height=600, image_width=800):
        """
        ...
        if ignorepaths is not None and isinstance(ignorepaths, list):
            ...
            if ignorepaths is not None and not isinstance(ignorepaths, list):
