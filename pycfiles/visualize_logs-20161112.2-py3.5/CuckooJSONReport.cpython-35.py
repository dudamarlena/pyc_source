...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/Visualize_Logs/objects/CuckooJSONReport.py
# Compiled at: 2016-11-12 13:31:22
# Size of source mod 2**32: 84536 bytes
import networkx, os, pandas
from plotly.offline import plot
from plotly.graph_objs import Bar, Scatter, Figure, Layout, Line, Marker, Annotations, Annotation, XAxis, YAxis
import re, json
...
class CuckooJSONReport(object):
    ...
    def __init__(self, jsonreportfile=None, jsonreportdict=None, plotnetwork=True, plotfiles=True, plotfilecreates=True, plotfiledeletes=True, plotfilemoves=True, plotfilecopies=True, plotfilewrites=True, plotfilereads=True, plotregistry=True, plotregistrywrites=True, plotregistryreads=True, plotregistrydeletes=True, plotregistrycreates=True, ignorepaths=None, includepaths=None):
        """
    ...
    def _add_processes_recursive(self, processtreedict):
        ...
        self.digraph.add_node(nodename, type='PID', pid=pid, parent_id=ppid)
        ...
        self.nodemetadata[nodename]['module_path'] = processtreedict['module_path']
    ...
    def _add_process_metadata(self):
        ...
        for process in self._processes:
            ...
            self.nodemetadata[nodename]['calls']['timestamp'] = pandas.to_datetime(self.nodemetadata[nodename]['calls']['timestamp'])
            self.nodemetadata[nodename]['calls'] = self.nodemetadata[nodename]['calls'].sort_values(['timestamp'])
    ...
    def _add_file_activity(self):
        ...
        for node in metadata:
            if metadata[node]['node_type'] == 'PID' and 'calls' in metadata[node]:
    ...
    def _connect_file_to_pid(self):
        ...
        for node in self.nodemetadata:
            if self.nodemetadata[node]['node_type'] == 'PID' and 'module_path' in self.nodemetadata[node]:
    ...
    def _add_file_moves(self, node, calls):
        ...
        :returns: Nothing.
        """
        filemoves = calls[(((calls['api'] == 'MoveFileW') | (calls['api'] == 'MoveFileA') | (calls['api'] == 'MoveFileWithProgressW') | (calls['api'] == 'MoveFileWithProgressA')) & (calls['status'] == True))]
        for i, filemove in filemoves.iterrows():
            ...
            if newfilename is not None:
                if self._search_re(newfilename, self.ignorepaths) and not self._search_re(newfilename, self.includepaths):
                    pass
                else:
                    ...
                    self.nodemetadata[fmnodename]['existingfile'] = existingfilename
                    self.nodemetadata[fmnodename]['newfile'] = newfilename
                    self.nodemetadata[fmnodename]['node_type'] = 'FILEMOVE'
                    self.nodemetadata[fmnodename]['timestamp'] = filemove['timestamp']
    ...
    def _add_file_copies(self, node, calls):
        ...
        :returns: Nothing.
        """
        filecopies = calls[(((calls['api'] == 'CopyFileW') | (calls['api'] == 'CopyFileA')) & (calls['status'] == True))]
        for i, filecreate in filecopies.iterrows():
            ...
            if newfilename is not None:
                if self._search_re(newfilename, self.ignorepaths) and not self._search_re(newfilename, self.includepaths):
                    ...
                    if self._search_re(existingfilename, self.ignorepaths) and not self._search_re(existingfilename, self.includepaths):
                        pass
                    else:
                        newfilenodename = self._add_file(newfilename)
                        existingfilenodename = self._add_file(existingfilename)
                        ...
                        self.nodemetadata[fcnodename]['timestamp'] = filecreate['timestamp']
    ...
    def _add_file_deletes(self, node, calls):
        ...
        :returns: Nothing.
        """
        filedeletes = calls[(((calls['api'] == 'DeleteFileW') | (calls['api'] == 'DeleteFileA')) & (calls['status'] == True))]
        for i, filedelete in filedeletes.iterrows():
            ...
            if filename is not None:
                if self._search_re(filename, self.ignorepaths) and not self._search_re(filename, self.includepaths):
                    pass
                else:
                    ...
                    self.nodemetadata[fdnodename]['file'] = filename
                    self.nodemetadata[fdnodename]['node_type'] = 'FILEDELETE'
                    self.nodemetadata[fdnodename]['timestamp'] = filedelete['timestamp']
    ...
    def _add_file_creates(self, node, calls):
        ...
        :returns: Nothing.
        """
        filecreates = calls[((calls['api'] == 'NtCreateFile') & (calls['status'] == True))]
        for i, filecreate in filecreates.iterrows():
            ...
            if filename is not None:
                if self._search_re(filename, self.ignorepaths) and not self._search_re(filename, self.includepaths):
                    ...
                    self.nodemetadata[fcnodename]['createdisposition'] = createdisposition
                    self.nodemetadata[fcnodename]['fileattribtes'] = fileattribtes
                    self.nodemetadata[fcnodename]['timestamp'] = filecreate['timestamp']
    ...
    def _add_file_writes(self, node, calls):
        ...
        :returns: Nothing.
        """
        filewrites = calls[((calls['api'] == 'NtWriteFile') & (calls['status'] == True))]
        for i, filewrite in filewrites.iterrows():
            ...
            if filename is not None:
                if self._search_re(filename, self.ignorepaths) and not self._search_re(filename, self.includepaths):
                    ...
                    self.nodemetadata[fwnodename]['timestamp'] = filewrite['timestamp']
    ...
    def _add_file_reads(self, node, calls):
        ...
        :returns: Nothing.
        """
        filereads = calls[((calls['api'] == 'NtReadFile') & (calls['status'] == True))]
        for i, fileread in filereads.iterrows():
            ...
            if filename is not None:
                if self._search_re(filename, self.ignorepaths) and not self._search_re(filename, self.includepaths):
                    ...
                    self.nodemetadata[frnodename]['timestamp'] = fileread['timestamp']
    ...
    def _add_network_activity(self):
        ...
        for i, dns in self.dns.iterrows():
            self.dns.ix[i]['answers'] = pandas.DataFrame(self.dns.ix[i]['answers'])
    ...
    def _add_sockets(self, node, calls):
        ...
        for i, sock in sockets.iterrows():
            ...
            if socketid is not None:
                ...
                self.nodemetadata[socketname]['opentime'] = sock['timestamp']
                closesockets = calls[((calls['api'] == 'closesocket') & (calls['timestamp'] > sock['timestamp']))]
                try:
                ...
                self._add_tcp_connects(socketname, calls, socketid, sock['timestamp'], closetime)
    ...
    def _add_tcp_connects(self, node, calls, socketid, opentime, closetime):
        ...
        if closetime is not None:
            tcpconnects = calls[((calls['api'] == 'connect') & (calls['timestamp'] >= opentime) & (calls['timestamp'] <= closetime))]
        else:
            tcpconnects = calls[((calls['api'] == 'connect') & (calls['timestamp'] >= opentime))]
        for i, tcpconnect in tcpconnects.iterrows():
            PlotConnect = False
            for arg in tcpconnect['arguments']:
                if arg['name'] == 'socket' and arg['value'] == socketid:
                    PlotConnect = True
            ...
            if PlotConnect is True:
                ...
                if ipaddr is not None:
                    ...
                    self.nodemetadata[connnodename]['node_type'] = 'TCPCONNECT'
                    self.nodemetadata[connnodename]['timestamp'] = tcpconnect['timestamp']
    ...
    def _add_registry_writes(self, node, calls):
        ...
        :returns:  Nothing.
        """
        regwrites = calls[(((calls['api'] == 'RegSetValueExA') | (calls['api'] == 'RegSetValueExW') | (calls['api'] == 'NtSetValueKey')) & (calls['status'] == True))]
        for i, regwrite in regwrites.iterrows():
            ...
            if regname is not None:
                if self._search_re(regname, self.ignorepaths) and not self._search_re(regname, self.includepaths):
                    pass
                else:
                    ...
                    self.nodemetadata[rwnodename]['timestamp'] = regwrite['timestamp']
    ...
    def _add_registry_deletes(self, node, calls):
        ...
        :returns:  Nothing.
        """
        regdeletes = calls[(((calls['api'] == 'RegDeleteValueA') | (calls['api'] == 'RegDeleteValueW') | (calls['api'] == 'NtDeleteKey')) & (calls['status'] == True))]
        for i, regdelete in regdeletes.iterrows():
            ...
            if regname is not None:
                if self._search_re(regname, self.ignorepaths) and not self._search_re(regname, self.includepaths):
                    ...
                    self.nodemetadata[rdnodename]['timestamp'] = regdelete['timestamp']
    ...
    def _add_registry_creates(self, node, calls):
        ...
        :returns:  Nothing.
        """
        regcreates = calls[(((calls['api'] == 'RegCreateKeyExA') | (calls['api'] == 'RegCreateKeyExW') | (calls['api'] == 'NtCreateKey')) & (calls['status'] == True))]
        for i, regcreate in regcreates.iterrows():
            ...
            if regname is not None:
                if self._search_re(regname, self.ignorepaths) and not self._search_re(regname, self.includepaths):
                    ...
                    self.nodemetadata[rcnodename]['timestamp'] = regcreate['timestamp']
    ...
    def _add_registry_reads(self, node, calls):
        ...
        :returns:  Nothing.
        """
        regreads = calls[(((calls['api'] == 'RegQueryValueExA') | (calls['api'] == 'RegQueryValueExW') | (calls['api'] == 'NtQueryValueKey')) & (calls['status'] == True))]
        for i, regread in regreads.iterrows():
            ...
            if regname is not None:
                if self._search_re(regname, self.ignorepaths) and not self._search_re(regname, self.includepaths):
                    ...
                    self.nodemetadata[rrnodename]['timestamp'] = regread['timestamp']
    ...
    def _create_positions_digraph(self):
        ...
        if self.graphvizprog is None:
            ...
            self.pos = networkx.drawing.nx_pydot.graphviz_layout(self.digraph, prog=self.graphvizprog, root=self.rootpid)
    ...
    def _generategraph(self):
        ...
        for node in self.digraph:
            if self.digraph.node[node]['type'] == 'PID':
                ...
                if 'cmdline' in self.nodemetadata[node]:
                    ...
                    cmdline = 'Not Available'
                proctxt.append('PID: {0}<br>Path: {1}<br>Command Line: {2}<br>Parent PID: {3}<br>First Seen: {4}'.format(self.nodemetadata[node]['pid'], self.nodemetadata[node]['module_path'], cmdline, self.nodemetadata[node]['parent_id'], self.nodemetadata[node]['first_seen']))
            if self.digraph.node[node]['type'] == 'HOST':
                HostX.append(self.pos[node][0])
                HostY.append(self.pos[node][1])
                hosttxt.append('HOST: {0}'.format(self.nodemetadata[node]['host']))
            if self.digraph.node[node]['type'] == 'IP':
                IPX.append(self.pos[node][0])
                IPY.append(self.pos[node][1])
                iptxt.append('IP: {0}'.format(self.nodemetadata[node]['ip']))
            if self.digraph.node[node]['type'] == 'SOCKET':
                SocketX.append(self.pos[node][0])
                SocketY.append(self.pos[node][1])
                sockettxt.append('Socket: {0}<br>Protocol: {1}<br>Open Time: {2}<br>Close TIme: {3}'.format(self.nodemetadata[node]['socket'], self.nodemetadata[node]['protocol'], self.nodemetadata[node]['opentime'], self.nodemetadata[node]['closetime']))
            if self.digraph.node[node]['type'] == 'TCPCONNECT':
                TCPConnectX.append(self.pos[node][0])
                TCPConnectY.append(self.pos[node][1])
                tcpconnecttxt.append('TCP Connect:<br>IP: {0}<br>Port: {1}<br>Socket: {2}<br>Time: {3}'.format(self.nodemetadata[node]['ip'], self.nodemetadata[node]['port'], self.nodemetadata[node]['socket'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILE':
                FileX.append(self.pos[node][0])
                FileY.append(self.pos[node][1])
                filetxt.append('File: {0}'.format(self.nodemetadata[node]['file']))
            if self.digraph.node[node]['type'] == 'FILECREATE':
                FileCreateX.append(self.pos[node][0])
                FileCreateY.append(self.pos[node][1])
                filecreatetxt.append('File Create: {0}<br>Existed Before: {1}<br>Desired Access: {2}<br>Create Disposition: {3}<br>File Atributes: {4}<br>Time: {5}'.format(self.nodemetadata[node]['file'], self.nodemetadata[node]['existedbefore'], self.nodemetadata[node]['desiredaccess'], self.nodemetadata[node]['createdisposition'], self.nodemetadata[node]['fileattribtes'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILEWRITE':
                FileWriteX.append(self.pos[node][0])
                FileWriteY.append(self.pos[node][1])
                filewritetxt.append('File Write: {0}<br>Time: {1}'.format(self.nodemetadata[node]['file'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILECOPY':
                FileCopyX.append(self.pos[node][0])
                FileCopyY.append(self.pos[node][1])
                filecopytxt.append('File Copy:<br>Existing File: {0}<br>New File: {1}<br>Existed Before: {2}<br>Time: {3}'.format(self.nodemetadata[node]['existingfile'], self.nodemetadata[node]['newfile'], self.nodemetadata[node]['existedbefore'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILEDELETE':
                FileDeleteX.append(self.pos[node][0])
                FileDeleteY.append(self.pos[node][1])
                filedeletetxt.append('File Delete: {0}<br>Time: {1}'.format(self.nodemetadata[node]['file'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILEMOVE':
                FileMoveX.append(self.pos[node][0])
                FileMoveY.append(self.pos[node][1])
                filemovetxt.append('File Move:<br>Existing File: {0}<br>New File: {1}<br>Time: {2}'.format(self.nodemetadata[node]['existingfile'], self.nodemetadata[node]['newfile'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'FILEREAD':
                FileReadX.append(self.pos[node][0])
                FileReadY.append(self.pos[node][1])
                filereadtxt.append('File Read: {0}<br>Time: {1}'.format(self.nodemetadata[node]['file'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'REGISTRY':
                ...
                registrytxt.append('Registry: {0}'.format(self.nodemetadata[newreg]['registry']))
            if self.digraph.node[node]['type'] == 'REGISTRYWRITE':
                RegistryWriteX.append(self.pos[node][0])
                RegistryWriteY.append(self.pos[node][1])
                registrywritetxt.append('Registry Write: {0}<br>Buffer: {1}<br>Time: {2}'.format(self.nodemetadata[node]['registry'], self.nodemetadata[node]['buffer'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'REGISTRYDELETE':
                ...
                registrydeletetxt.append('Registry Delete: {0}<br>Time: {1}'.format(self.nodemetadata[node]['registry'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'REGISTRYCREATE':
                ...
                registrycreatetxt.append('Registry Create: {0}<br>Time: {1}'.format(self.nodemetadata[node]['registry'], self.nodemetadata[node]['timestamp']))
            if self.digraph.node[node]['type'] == 'REGISTRYREAD':
                RegistryReadX.append(self.pos[node][0])
                RegistryReadY.append(self.pos[node][1])
                registryreadtxt.append('Registry Read: {0}<br>Time: {1}'.format(self.nodemetadata[node]['registry'], self.nodemetadata[node]['timestamp']))

        for edge in self.digraph.edges():
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'PID':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'HOST':
            ...
            if self.digraph.node[edge[0]]['type'] == 'HOST' and self.digraph.node[edge[1]]['type'] == 'IP':
                ...
                DNSYe.append(None)
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'SOCKET':
            ...
            if self.digraph.node[edge[0]]['type'] == 'SOCKET' and self.digraph.node[edge[1]]['type'] == 'TCPCONNECT' or self.digraph.node[edge[0]]['type'] == 'TCPCONNECT' and self.digraph.node[edge[1]]['type'] == 'IP':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILECREATE' or self.digraph.node[edge[0]]['type'] == 'FILECREATE' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILEWRITE' or self.digraph.node[edge[0]]['type'] == 'FILEWRITE' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'FILE' and self.digraph.node[edge[1]]['type'] == 'PID':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILECOPY' or self.digraph.node[edge[0]]['type'] == 'FILECOPY' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILEDELETE' or self.digraph.node[edge[0]]['type'] == 'FILEDELETE' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILEMOVE' or self.digraph.node[edge[0]]['type'] == 'FILEMOVE' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'FILEREAD' or self.digraph.node[edge[0]]['type'] == 'FILEREAD' and self.digraph.node[edge[1]]['type'] == 'FILE':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'REGISTRYWRITE' or self.digraph.node[edge[0]]['type'] == 'REGISTRYWRITE' and self.digraph.node[edge[1]]['type'] == 'REGISTRY':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'REGISTRYDELETE' or self.digraph.node[edge[0]]['type'] == 'REGISTRYDELETE' and self.digraph.node[edge[1]]['type'] == 'REGISTRY':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'REGISTRYCREATE' or self.digraph.node[edge[0]]['type'] == 'REGISTRYCREATE' and self.digraph.node[edge[1]]['type'] == 'REGISTRY':
            ...
            if self.digraph.node[edge[0]]['type'] == 'PID' and self.digraph.node[edge[1]]['type'] == 'REGISTRYREAD' or self.digraph.node[edge[0]]['type'] == 'REGISTRYREAD' and self.digraph.node[edge[1]]['type'] == 'REGISTRY':
        ...
        ProcNodes = Scatter(x=ProcessX, y=ProcessY, mode='markers', marker=marker, name='Process', text=proctxt, hoverinfo='text')
        ProcEdges = Scatter(x=ProcessXe, y=ProcessYe, mode='lines', line=Line(shape='linear', color='rgb(214,39,20)'), name='Process Start', hoverinfo='none')
        nodes.append(ProcNodes)
        edges.append(ProcEdges)
        marker = Marker(symbol='square', size=10)
        HostNodes = Scatter(x=HostX, y=HostY, mode='markers', marker=marker, name='Host', text=hosttxt, hoverinfo='text')
        nodes.append(HostNodes)
        GetNameEdges = Scatter(x=GetNameXe, y=GetNameYe, mode='lines', line=Line(shape='linear', color='rgb(174,199,232)'), name='DNS Query', hoverinfo='none')
        edges.append(GetNameEdges)
        marker = Marker(symbol='square', size=10)
        IPNodes = Scatter(x=IPX, y=IPY, mode='markers', marker=marker, name='IP', text=iptxt, hoverinfo='text')
        nodes.append(IPNodes)
        DNSEdges = Scatter(x=DNSXe, y=DNSYe, mode='lines', line=Line(shape='linear', color='rgb(23,190,207)'), name='DNS Response', hoverinfo='none')
        edges.append(DNSEdges)
        marker = Marker(symbol='diamond', size=7, color='rgb(277,119,194)')
        SocketNodes = Scatter(x=SocketX, y=SocketY, mode='markers', marker=marker, name='Socket', text=sockettxt, hoverinfo='text')
        SocketEdges = Scatter(x=SocketXe, y=SocketYe, mode='lines', line=Line(shape='linear', color='rgb(227,119,194)'), name='Create Socket', hoverinfo='none')
        nodes.append(SocketNodes)
        edges.append(SocketEdges)
        marker = Marker(symbol='diamond', size=7, color='rgb(44,160,44)')
        TCPConnectNodes = Scatter(x=TCPConnectX, y=TCPConnectY, mode='markers', marker=marker, name='TCP Connection', text=tcpconnecttxt, hoverinfo='text')
        TCPConnectEdges = Scatter(x=TCPConnectXe, y=TCPConnectYe, mode='lines', line=Line(shape='linear', color='rgb(44,160,44)'), name='TCP Connect', hoverinfo='none')
        nodes.append(TCPConnectNodes)
        edges.append(TCPConnectEdges)
        marker = Marker(symbol='hexagon', size=10)
        FileNodes = Scatter(x=FileX, y=FileY, mode='markers', marker=marker, name='File', text=filetxt, hoverinfo='text')
        nodes.append(FileNodes)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(123,102,210)')
        FileCreateNodes = Scatter(x=FileCreateX, y=FileCreateY, mode='markers', marker=marker, name='File Create', text=filecreatetxt, hoverinfo='text')
        nodes.append(FileCreateNodes)
        FileCreateEdges = Scatter(x=FileCreateXe, y=FileCreateYe, mode='lines', line=Line(shape='linear', color='rgb(123,102,210)'), name='File Create', hoverinfo='none')
        edges.append(FileCreateEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(255,187,120)')
        FileWriteNodes = Scatter(x=FileWriteX, y=FileWriteY, mode='markers', marker=marker, name='File Write', text=filewritetxt, hoverinfo='text')
        nodes.append(FileWriteNodes)
        FileWriteEdges = Scatter(x=FileWriteXe, y=FileWriteYe, mode='lines', line=Line(shape='linear', color='rgb(255,187,120)'), name='File Write', hoverinfo='none')
        edges.append(FileWriteEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(65,68,81)')
        FileCopyNodes = Scatter(x=FileCopyX, y=FileCopyY, mode='markers', marker=marker, name='File Copy', text=filecopytxt, hoverinfo='text')
        nodes.append(FileCopyNodes)
        FileCopyEdges = Scatter(x=FileCopyXe, y=FileCopyYe, mode='lines', line=Line(shape='linear', color='rgb(65,68,81)'), name='File Copy', hoverinfo='none')
        edges.append(FileCopyEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(255,128,14)')
        FileDeleteNodes = Scatter(x=FileDeleteX, y=FileDeleteY, mode='markers', marker=marker, name='File Delete', text=filedeletetxt, hoverinfo='text')
        nodes.append(FileDeleteNodes)
        FileDeleteEdges = Scatter(x=FileDeleteXe, y=FileDeleteYe, mode='lines', line=Line(shape='linear', color='rgb(255,128,14)'), name='File Delete', hoverinfo='none')
        edges.append(FileDeleteEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(171,171,171)')
        FileMoveNodes = Scatter(x=FileMoveX, y=FileMoveY, mode='markers', marker=marker, name='File Move', text=filemovetxt, hoverinfo='text')
        nodes.append(FileMoveNodes)
        FileMoveEdges = Scatter(x=FileMoveXe, y=FileMoveYe, mode='lines', line=Line(shape='linear', color='rgb(171,171,171)'), name='File Move', hoverinfo='none')
        edges.append(FileMoveEdges)
        marker = Marker(symbol='triangle-up', size=7, color='rgb(207,207,207)')
        FileReadNodes = Scatter(x=FileReadX, y=FileReadY, mode='markers', marker=marker, name='File Read', text=filereadtxt, hoverinfo='text')
        nodes.append(FileReadNodes)
        FileReadEdges = Scatter(x=FileReadXe, y=FileReadYe, mode='lines', line=Line(shape='linear', color='rgb(207,207,207)'), name='File Read', hoverinfo='none')
        edges.append(FileReadEdges)
        LoadImageEdges = Scatter(x=LoadImageXe, y=LoadImageYe, mode='lines', line=Line(shape='linear', dash='dot'), name='Process Load Image', hoverinfo='none')
        edges.append(LoadImageEdges)
        marker = Marker(symbol='star', size=10)
        RegistryNodes = Scatter(x=RegistryX, y=RegistryY, mode='markers', marker=marker, name='Registry', text=registrytxt, hoverinfo='text')
        nodes.append(RegistryNodes)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(255,187,120)')
        RegistryWriteNodes = Scatter(x=RegistryWriteX, y=RegistryWriteY, mode='markers', marker=marker, name='Registry Write', text=registrywritetxt, hoverinfo='text')
        nodes.append(RegistryWriteNodes)
        RegistryWriteEdges = Scatter(x=RegistryWriteXe, y=RegistryWriteYe, mode='lines', line=Line(shape='linear', color='rgb(255,187,120)'), name='Registry Write', hoverinfo='none')
        edges.append(RegistryWriteEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(255,128,14)')
        RegistryDeleteNodes = Scatter(x=RegistryDeleteX, y=RegistryDeleteY, mode='markers', marker=marker, name='Registry Delete', text=registrydeletetxt, hoverinfo='text')
        nodes.append(RegistryDeleteNodes)
        RegistryDeleteEdges = Scatter(x=RegistryDeleteXe, y=RegistryDeleteYe, mode='lines', line=Line(shape='linear', color='rgb(255,128,14)'), name='Registry Delete', hoverinfo='none')
        edges.append(RegistryDeleteEdges)
        marker = Marker(symbol='triangle-down', size=7, color='rgb(123,102,210)')
        RegistryCreateNodes = Scatter(x=RegistryCreateX, y=RegistryCreateY, mode='markers', marker=marker, name='Registry Create', text=registrycreatetxt, hoverinfo='text')
        nodes.append(RegistryCreateNodes)
        RegistryCreateEdges = Scatter(x=RegistryCreateXe, y=RegistryCreateYe, mode='lines', line=Line(shape='linear', color='rgb(123,102,210)'), name='Registry Create', hoverinfo='none')
        edges.append(RegistryCreateEdges)
        marker = Marker(symbol='triangle-up', size=7, color='rgb(207,207,207)')
        RegistryReadNodes = Scatter(x=RegistryReadX, y=RegistryReadY, mode='markers', marker=marker, name='Registry Read', text=registryreadtxt, hoverinfo='text')
        nodes.append(RegistryReadNodes)
        RegistryReadEdges = Scatter(x=RegistryReadXe, y=RegistryReadYe, mode='lines', line=Line(shape='linear', color='rgb(207,207,207)'), name='File Read', hoverinfo='none')
    ...
    def _generateannotations(self):
        ...
        for node in self.digraph:
            if self.digraph.node[node]['type'] == 'PID':
                annotations.append(Annotation(text='{0}<br>PID: {1}'.format(self.nodemetadata[node]['name'], self.nodemetadata[node]['pid']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
            if self.digraph.node[node]['type'] == 'HOST':
                annotations.append(Annotation(text='HOST: {0}'.format(self.nodemetadata[node]['host']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))
            if self.digraph.node[node]['type'] == 'IP':
                annotations.append(Annotation(text='IP: {0}'.format(self.nodemetadata[node]['ip']), x=self.pos[node][0], y=self.pos[node][1], xref='x', yref='y', showarrow=True, ax=-40, ay=-40))

        return annotations

    def plotgraph(self, graphvizprog='sfdp', filename='temp-plot.html', title=None, auto_open=True, image=None, image_filename='plot_image', image_height=600, image_width=800):
        """
        ...
        plotlayout = Layout(showlegend=True, title=title, xaxis=XAxis(axis), yaxis=YAxis(axis), hovermode='closest', annotations=annotations)
        plotfigure = Figure(data=outputdata, layout=plotlayout)
        plot(plotfigure, show_link=False, filename=filename, auto_open=auto_open, image=image, image_filename=image_filename, image_height=image_height, image_width=image_width)