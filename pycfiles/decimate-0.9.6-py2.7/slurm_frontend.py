...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/decimate/slurm_frontend.py
...
if len(cmd_line) >= 2:
    if cmd_line.find(' API ') > -1 or cmd_line.find('API,') > -1 or cmd_line.find(',API') > -1:
        DEBUG = True

class slurm_frontend(decimate):

    def __init__(self):
        decimate.__init__(self, app_name='decimate', decimate_version_required='0.5', app_version='0.1', extra_args=True)
    ...
    def user_filtered_args(self):
        ...
        self.slurm_parser.add_argument('--resume', action='store_true', help='resume the already launched step and workflow in this directory', default=True)
        self.slurm_parser.add_argument('--restart', action='store_true', help='restart the already launched step or workflow in this directory', default=True)
        self.slurm_parser.add_argument('-sc', '--scratch', action='store_true', help='relaunch a new workflow, erasing all from the previous one', default=False)
        self.slurm_parser.add_argument('-xy', '--yalla', action='store_true', help='Use yalla pool', default=False)
        self.slurm_parser.add_argument('-xyp', '--yalla-parallel-runs', type=int, help='# of job to run in parallel in a pool', default=4)
        self.parser.add_argument('-P', '--parameter-file', type=str, help='file listing all parameter combinations to cover')
        self.parser.add_argument('-Pl', '--parameter-list', action='store_true', help='lists all parameters combination to scan and exit', default=False)
        self.parser.add_argument('-PF', '--parameter-filter', type=str, help='filter to apply on combinations to cover')
        self.parser.add_argument('-Pa', '--parameter-range', type=str, help='filtered range to apply on combinations to cover')
    ...
    def user_initialize_parser(self):
        self.parser.add_argument('-e', '--ends', type=int, help=argparse.SUPPRESS, default=-1)
        self.parser.add_argument('--banner', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('-np', '--no-pending', action='store_true', help='do not keep pending the log', default=True)
    ...
    def user_launch_jobs(self):
        ...
        self.log_debug('job submitted:\n%s' % self.print_job(new_job, allkey=False, print_all=True, except_none=True), 4, trace='API')
        if self.args.yalla:
        ...
        if self.slurm_args.check:
            ...
            self.log_debug('*********** final_checking_job **********', 4, trace='SUBMIT,CHECK_FINAL')
            self.log_debug('final_checking_job=%s' % pprint.pformat(final_checking_job), 4, trace='CHECK,FINAL_DETAIL')
            job_id, cmd = self.submit_job(final_checking_job)
        self.log_debug('Saving Job Ids...', 1)
        self.save()

    def check_job_old(self, what, attempt, task_id, running_dir, output_file, error_file, is_done, fix, job_tasks, step_tasks):
        self.log_info(('check_job(what=>%s<, attempt=>%s<, task_id=>%s<, running_dir=>%s<,' + 'output_file=>%s<, error_file=>%s<, is_done=>%s<, fix=>%s<, ' + 'job_tasks=>%s<,step_tasks=>%s<') % (
