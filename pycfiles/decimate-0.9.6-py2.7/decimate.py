...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/decimate/decimate.py
# Compiled at: 2018-04-08 15:51:11
import argparse
from contextlib import contextmanager
import copy
from engine import *
from env import TMPDIR
import fnmatch, itertools, math, numpy, os, pandas, pandas as pd, pprint, shlex
from stat import *
import sys, termios
DECIMATE_VERSION = '0.9.6'
RSYNC_CMD = "timeout 60 bash -c 'until time srun -N ${SLURM_NNODES} --ntasks=${SLURM_NNODES} " + "--ntasks-per-node=1 rsync %s /tmp; do > /dev/null :; done  > /dev/null 2>&1' \n"
SUCCESS = True
FAILURE = False
ABORT = -9999
...
class decimate(engine):
    ...
    def __init__(self, app_name='decimate', app_version='???', decimate_version_required=DECIMATE_VERSION, extra_args=False):
        ...
        engine.__init__(self, engine_version_required='0.23', app_name=app_name, app_version=app_version, extra_args=extra_args)
    ...
    def initialize_parser(self):
        ...
        self.parser.add_argument('-s', '--status', action='store_true', help='list status of jobs and of the workflow')
        self.parser.add_argument('-sa', '--status-all', action='store_true', help='list status of all the jobs of the workflow')
        self.parser.add_argument('-sl', '--status-long', action='store_true', help='list detailed status of jobs of the workflow')
        self.parser.add_argument('-k', '--kill', action='store_true', help='kills job of this workflow')
        self.parser.add_argument('--resume', action='store_true', help='resume the already launched step and workflow in this directory', default=True)
        self.parser.add_argument('--restart', action='store_true', help='restart the already launched step or workflow in this directory', default=True)
        self.parser.add_argument('-sc', '--scratch', action='store_true', help='relaunch a new workflow, erasing all from the previous one', default=False)
        self.parser.add_argument('-x', '--explore', action='store_true', help='start a console to explore the results', default=False)
        self.parser.add_argument('--decimate', action='store_true', help=argparse.SUPPRESS)
        self.parser.add_argument('--check', action='store_true', default=False, help='check at the end of the step')
        self.parser.add_argument('--check-file', type=str, help=argparse.SUPPRESS)
        self.parser.add_argument('-pe', '--print-environment', action='store_true', default=False, help='print environment variables in job output file')
        ...
        self.parser.add_argument('--check-previous-step', action='store_true', help=argparse.SUPPRESS, default=False)
        ...
        self.parser.add_argument('--attempt-initial', type=int, default=0, help=argparse.SUPPRESS)
        ...
        self.parser.add_argument('-ql', '--quick-launch', action='store_true', help='launches jobs quicky only saving state at the end', default=False)
        self.parser.add_argument('--quick-launch-no-load', action='store_true', help='launches jobs quicky only saving state at the end', default=False)
        self.parser.add_argument('--force-check', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('-ncls', '--no-clear-screen', action='store_true', help='do not clear the screen in the console', default=False)
        self.parser.add_argument('-y', '--yes', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--no', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--feed', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--fix', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--ufix', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--forward-mail', action='store_true', help=argparse.SUPPRESS, default=False)
        self.parser.add_argument('--template', action='store_true', help='create template files')
        self.parser.add_argument('--process-templates', type=str, help=argparse.SUPPRESS)
        if not self.user_initialize_parser() == 'default':
            self.parser.add_argument('--create-template', action='store_true', help='Download all the template files in the current directory')
        self.parser.add_argument('--all', action='store_true', default=False, help=argparse.SUPPRESS)
        if not self.user_initialize_parser() == 'default':
            self.parser.add_argument('-r', '--reservation', type=str, help='run in the given reservation name')
            self.parser.add_argument('-p', '--partition', type=str, help='set the default partition')
        self.parser.add_argument('-r2', '--rollback', type=str, help='rollback to step xxx')
        self.parser.add_argument('-rl', '--rollback-list', action='store_true', default=False, help='list available step to rollback to')
        self.parser.add_argument('-xr', '--max-retry', type=int, default=3, help='Number of time a step can fail successively (3 per default)')
        self.parser.add_argument('-xj', '--max-jobs', type=int, default=450, help='Maximimum jobs queued at a time (450 per default)')
        self.parser.add_argument('-sc', '--stripe-count', type=int, default=0, help='stripe count set for new directory created (0 per default)')
        self.parser.add_argument('-xa', '--all-released', action='store_true', help='do release all the job of a step.', default=False)
        self.parser.add_argument('-xy', '--yalla', action='store_true', help='Use yalla pool', default=False)
        self.parser.add_argument('-xyp', '--yalla-parallel-runs', type=int, help='# of job to run in parallel in a pool', default=4)
        self.parser.add_argument('-xyf', '--parameter-file', type=str, help='file listing all parameter combinations to cover')
        self.parser.add_argument('-bbz', '--use-burst-buffer-size', action='store_true', help='Use a non persistent burst buffer space', default=False)
        self.parser.add_argument('-bbs', '--use-burst-buffer-space', action='store_true', help='Use a non persistent burst buffer space', default=False)
        self.parser.add_argument('-xz', '--burst-buffer_size', type=str, help='use a non persistent burst buffer space of size', default='80TiB')
        self.parser.add_argument('-xs', '--burst-buffer-space', type=str, help='name of the persistent burst buffer space used', default='Tst01')
    ...
    def run(self):
        ...
        if self.args.use_burst_buffer_size and not self.args.burst_buffer_size:
            self.error('Wanting to use Burst Buffer? please precise which space ' + 'with --burst-buffer-size argument', exit=True)
        if self.args.use_burst_buffer_space and not self.args.burst_buffer_space:
            self.error('Wanting to use Burst Buffer? please precise which space ' + 'with --burst-buffer-space argument', exit=True)
        if self.args.yalla and not self.args.spawned:
            ...
            if not os.path.exists(yalla_exe):
                if not os.path.isfile(makefile_name):
                    self.error('yalla not available on %s ...                      no makefile %s available for this type of machine' % (
                     self.machine, makefile_name), exit=True)
                self.log_console('Installing pool support...', trace='YALLA')
                cmd = ('mkdir -p %s; cd %s; cp %s/yalla.c .; ' + 'make -f %s > %s/yalla_compile.out 2>&1') % (
            ...
            if not os.path.exists(yalla_srun_wrapper):
                ...
                f.close()
                os.chmod(srun_wrapper_file, 493)
            if not os.path.exists(yalla_srun_wrapper):
            ...
            if not os.path.exists(yalla_srun_debug_wrapper):
                ...
                f.close()
                os.chmod(srun_debug_wrapper_file, 493)
            if not os.path.exists(yalla_srun_debug_wrapper):
        ...
        if self.args.status or self.args.status_long or self.args.status_all:
            ...
            if not self.args.log:
                sys.exit(0)
        if self.args.log:
            self.tail_log_file(keep_probing=True, no_timestamp=True, nb_lines_tailed=5, stop_tailing=[
             'workflow is finishing',
        ...
        if self.args.check_previous_step and self.args.parameter_file and self.args.spawned:
            if self.args.yalla:
                ...
                tasks = [
                 self.TASK_ID]
            self.log_debug('creating parametric files for range [%s]' % self.TASK_IDS, 4, trace='PARAMETRIC,PARAMETRIC_DETAIL,PD')
            for t in tasks:
                ...
                for p in self.parameters.columns:
                    ...
                    if p in self.slurm_vars.keys():
                        if self.slurm_vars[p] == int:
                            self.log_debug('params[%s]:%s of type %s' % (p, params[p], type(params[p])), 4, trace='VAR')
        ...
        if self.args.check_previous_step:
            ...
            self.load()
            if self.args.jobid not in self.JOBS.keys():
                self.error(('strange, while checking if dependency exists, ' + 'my own job-id %s is not known in self.JOBS... ' + 'self.args.check_previous_step=%s') % (
                 self.args.jobid, self.args.check_previous_step), exit=False, exception=True)
            else:
                ...
                self.log_info('status of the current step %s: %s ' % (
                 step['job_name'], self.print_job(step, step.keys())), 2, trace='COMPUTE_CHECK,CURRENT')
                if dep_jobid and my_job['check_previous']:
                    ...
                    self.log_debug('while checking step=%s attempt=%s' % (
                     step_to_check, array_to_check), 4, trace='HEAL,RESTART,CHECK,COMPUTE_CHECK')
                    self.log_debug('self.steps_submitted_attempts=%s' % pprint.pformat(self.steps_submitted_attempts), 4, trace='RESTART,HEAL,CHECK,COMPUTE_CHECK')
                    ...
                    if step in self.steps_submitted_attempts.keys():
                        ...
                        self.log_debug(('should check step %s (%s) attempt %s, ' + 'STEPS.keys=%s self.steps_submitted_attempts=%s') % (
                         step_to_check, array_to_check, attempts_done,
                         (',').join(self.STEPS.keys()),
                         pprint.pformat(self.steps_submitted_attempts)), 4, trace='CHECK_SHORT')
                        self.log_console('\n' + self.system('ls -l %s.*-attempt_* ' % step))
                        self.log_debug('should check step %s (%s) attempt %s: %s' % (
                         step_to_check, array_to_check, attempts_done,
                         pprint.pformat(self.STEPS[step_to_check])), 4, trace='COMPUTE_CHECK,CHECK,WHAT_CHECK')
        ...
        if not self.args.spawned:
            self.log_debug('self.jobs_list = [%s]' % (',').join(map(lambda x: str(x), self.jobs_list)), 4, trace='WORKFLOW')
            if len(self.jobs_list) > 0 and self.args.restart:
                self.log_debug('Workflow has already run in this directory, trying to continue it', 4, trace='WORKFLOW')
    ...
    def print_workflow(self, all_steps=False):
        ...
        for s in steps:
            ...
            try:
                ...
                global_success_percent = 100.0 * len(self.STEPS[s]['global_success']) / total_nb
                global_failure_percent = 100.0 * len(self.STEPS[s]['global_failure']) / total_nb
    ...
    def kill_workflow(self, force=False, exceptMe=False):
        ...
        for jid in self.JOBS.keys():
            self.log_debug("%s:self.JOBS[%s]['completion_percent']=%s and self.JOBS[%s]['status']: %s" % (
             ...
             self.JOBS[jid]['completion_percent'], jid, self.JOBS[jid]['status']), 3, trace='KILL')
            if self.JOBS[jid]['completion_percent'] < 100.0 and not self.JOBS[jid]['status'] == 'ABORTED':
    ...
    def finalize(self):
        if not (self.SCENARIO.find(',%s,' % self.args.step) >= 0 or self.SCENARIO.find(',%s-%s,' % (self.args.step, self.TASK_ID)) >= 0 or self.SCENARIO.find(',%s-%s-%s,' % (self.args.step, self.TASK_ID,
         self.args.attempt)) >= 0):
            ...
            if self.args.check:
                self.check_current_state(self.args.step, self.args.attempt, tasks=[
                 self.TASK_ID], from_finalize=True)
    ...
    def check_current_state(self, what, attempt, tasks=None, checking_from_console=False, from_heal_workflow=False, from_finalize=False):
        ...
        if checking_from_console or from_finalize:
            ...
            if check_it:
                ...
                for i in RangeSet(tasks):
                    ...
                    percent_task_checked = int(100.0 * nb_current_task / nb_tasks_to_check)
                    if nb_tasks_to_check > 100 and percent_task_checked % 5 == 0 and not last_percent_printed == percent_task_checked:
                    ...
                    try:
                        if self.TASKS[step][i]['status'] == 'SUCCESS' and not self.args.force_check:
                            self.log_info('task %s-%s (step %s) found successfull ' % (what, i, step), 2, trace='CHECK')
                            continue
                    except Exception:
                    ...
                    if is_complete and not self.args.force_check:
                        self.log_info('Complete file found for task %s-%s (step=%s) ' % (what, i, step), 2, trace='CHECK')
                    ...
                    user_check = self.prepare_user_defined_check_job(what, i, attempt, is_done, checking_from_console)
                    self.log_info('user_check=%s for task %s-%s (step=%s) ' % (user_check, what, i, step), 2, trace='CHECK')
                    if user_check == SUCCESS:
                        if not is_done and user_check:
                            s = ('ZZZZZZZ step %s-%s was not complete but was validated successfully ' + '+by a check made by a user defined function') % (what, i)
                            self.log_info(s)
                            self.append_mail(s)
                        if self.CanLock:
                            open(filename_complete, 'w')
                        task_status = self.TASKS[step][i]['status'] = 'SUCCESS'
                        self.log_info('set status=SUCCESS for task %s-%s (step=%s) ' % (what, i, step), 2, trace='CHECK')
                        state_has_changed = True
                        continue
                    if not is_done or not user_check == SUCCESS:
                        self.log_debug('adding task %s to tasks_not_complete' % i, 4, trace='CHECK')
                        ...
                        if not user_check == SUCCESS:
                            ...
                            state_has_changed = True
                            self.log_info('set status=%s for task %s-%s (step=%s) ' % (task_status, what, i, step), 2, trace='CHECK')
                    if not user_check == SUCCESS:
                ...
                if not all_complete:
                    ...
                    if checking_from_console:
                        self.args.info_offset = self.args.info_offset - 1
                        return (
                         False, RangeSet(tasks_not_complete))
                    ...
                    if checking_from_console:
                        self.args.info_offset = self.args.info_offset - 1
                        return (
                         True, RangeSet(tasks_not_complete))
                    self.save()
                    ...
                    self.log_debug('at this point for step %s: %s' % (
                     step_with_attempt,
                     self.print_job(step_content, print_only=step_content.keys())), 3, trace='GLOBAL')
                    ...
                    if next_job_id:
                        ...
                        while not next_job['check_previous']:
                            self.log_info('\t\t --> releasing jobs %s (%s)' % (
                             next_job['step'], next_job['array']), 2, trace='RELEASED')
    ...
    def prepare_user_defined_check_job(self, what, task_id, attempt, is_done, checking_from_console=False):
        step = '%s-%s' % (what, attempt)
        job_id = self.TASKS[step][task_id]['job']
        if job_id == 'UNKNOWN':
            return False
            self.error('UNKNOWN key found in self.TASKS[%s][%s][job] \n self.TASKS[%s]=\n%s' % (
             step, task_id, step, pprint.pformat(self.TASKS[step])), exit=False, exception=False)
            return False
        if job_id not in self.JOBS.keys():
            return False
        if 'output' not in self.JOBS[job_id].keys():
            return False
        ...
        s = 'CHECKING step: %s   Task: %s ' % (step, task_id) + ' ' + 'Output file pattern : %s' % output_file_pattern + ' ' + 'Error file pattern : %s' % error_file_pattern + ' ' + 'Running dir : %s' % running_dir + ' '
        ...
        with working_directory(running_dir):
            ...
            if len(output_file_candidates):
                ...
                self.log_debug('ZZZZZZ weird... no output file produced of actual pattern %s' % output_file_pattern, 3, trace='CHECK_PATTERN')
                self.error_add('ZZZZZZ weird... no output file produced of pattern %s' % original_output_pattern, task_id)
            ...
            if len(error_file_candidates):
                ...
                self.log_debug('ZZZZZZ weird... no error file produced of pattern %s' % error_file_pattern, 3, trace='CHECK_PATTERN')
                self.error_add('ZZZZZZ weird... no error file produced of pattern %s' % original_error_pattern, task_id)
        ...
        if self.args.check or checking_from_console:
            if self.args.check_file:
                ...
                self.log_debug('checking via user_script with cmd/%s/' % cmd, 4, trace='USER_CHECK')
                ...
                if return_code == 0:
                    ...
                    user_check = ABORT
                self.log_debug('user_check=/%s/\nresult=/%s/' % (user_check, result), 4, trace='USER_CHECK')
            else:
                try:
                    user_check = self.check_job(what, attempt, task_id, running_dir, output_file, error_file, is_done, fix=not checking_from_console, job_tasks=job_tasks, step_tasks=step_tasks)
                except Exception:
            ...
            s = '%s found while CHECKING step: %s   Task: %s ' % (user_check, step, task_id) + ' ' + 'running_dir : %s' % running_dir + ' ' + 'output_file : %s' % output_file + ' ' + 'error_file : %s' % error_file + ' ' + 'Running dir : %s' % running_dir + ' '
            self.log_info(s, 4, trace='USER_CHECK')
            if not user_check == SUCCESS:
    ...
    def what_to_restart_from_workflow(self):
        ...
        for s in steps:
            ...
            if abs(self.STEPS[s]['global_success_percent'] - 100.0) < 1e-06:
    ...
    def check_workflow_and_start(self, no_restart=False, from_heal_workflow=False):
        ...
        if to_be_restarted == 'FROM_SCRATCH':
            ...
            for task_id in step_to_check_not_done_array:
                ...
                what = ('-').join(items[:-1])
                attempt = items[(-1)]
                res, tasks_uncomplete = self.check_current_state(what, attempt, '%s' % task_id, checking_from_console=True, from_heal_workflow=from_heal_workflow)
                if not res:
            ...
            if len(tasks_todo) == 0:
                ...
                while len(next_steps):
                    ...
                    self.log_info(' Checking actual status of step %s-%s [%s] ...' % (
                     step, attempt, tasks))
                    ret = self.check_current_state(step, attempt, tasks, checking_from_console=True, from_heal_workflow=from_heal_workflow)
                    res, tasks_todo = ret
                    if res:
            ...
            if last_step_uncompleted:
                ...
                self.log_debug('self.steps_submitted=[%s]' % (',').join(self.steps_submitted), trace='RESTART')
                self.steps_to_restart_anyway = next_steps
                self.STEPS_RESTART_STATUS[last_step_uncompleted] = 'TO_RERUN'
                self.log_info('step to restart anyway [%s] ' % self.steps_to_restart_anyway)
            else:
            ...
            self.log_debug('STEPS a restart (%s)' % (',').join(self.STEPS.keys()), 4, trace='RESTART')
            for step_name in [last_step_uncompleted]:
                ...
                self.log_debug('STEPS to restart (%s) = %s ' % (
                 ...
                 self.print_job(step2print, print_only=step2print.keys())), 4, trace='RESTART')
                previous_jobs = self.STEPS[last_step_uncompleted_with_attempt]['jobs']
                if len(previous_jobs) == 0:
                    self.log_info("I don't know how to resubmit step %s" % last_step_uncompleted_with_attempt)
                    return
            ...
            self.log_debug('STEPS after restart (%s)' % (',').join(self.STEPS.keys()), 4, trace='RESTART')
            self.log_debug('self.STEPS_RESTART_STATUS=\n%s' % pprint.pformat(self.STEPS_RESTART_STATUS), 4, trace='RESTART_FAKED')
            self.log_debug('self.STEPS_RESTART_ATTEMPT=\n%s' % pprint.pformat(self.STEPS_RESTART_ATTEMPT), 4, trace='RESTART_FAKED')
            return
    ...
    def heal_workflow(self, what, not_present, from_resubmit=False):
        ...
        s = ('RESTARTING THE WRONG PART PREVIOUS JOB : %s (%s). current_attempt=%s' + ' initial_attempt=%s Extra attempt #%s ( %s out of %s)') % (
         what,
        ...
        if self.args.jobid in self.JOBS.keys():
            ...
            job['array'] = '1-1'
            self.log_info('resubmitting same myself job %s for same attempt' % self.print_job(job), 2, trace='HEAL')
    ...
    def compute_critical_path(self):
        ...
        self.log_debug('computing critical path: self.steps_submitted=[%s]' % pprint.pformat(self.steps_submitted), 4, trace='CRITICAL,RESTART')
        ...
        if len(self.steps_submitted):
            for current_step in self.steps_submitted:
                ...
                if step_alone not in self.steps_addressed:
        ...
        for s in self.steps_addressed:
            for ss in self.steps_submitted:
                if ('-').join(ss.split('-')[:-1]) == s:
                    if ss not in self.steps_in_order:
        ...
        self.log_debug(' steps_critical_path=(%s)' % (',').join(self.steps_critical_path), trace='CRITICAL')
        self.log_debug(' steps_critical_path_with_attempt=(%s)' % (',').join(self.steps_critical_path_with_attempt), trace='CRITICAL')

    def resubmit_step(self, step_name, step_array=False, dep=False, clear_dependency=False, return_all_job_ids=False):
    ...
    def fake_actual_job(self):
        ...
        if self.SCENARIO.find(',%s,' % self.args.step) >= 0 or self.SCENARIO.find(',%s-%s,' % (self.args.step, self.TASK_ID)) >= 0 or self.SCENARIO.find(',%s-%s-%s,' % (self.args.step, self.TASK_ID, self.args.attempt)) >= 0:
    ...
    def additional_tag(self, line):
        matchObj = re.match('^#DECIM\\s*COMBINE\\s*(\\S+|_)\\s*=\\s*(.*)\\s*$', line)
        if matchObj:
            ...
            return True
        matchObj = re.match('^#DECIM\\s*(\\S+|_)\\s*=\\s*(.*)\\s*$', line)
        if matchObj:
            t, v = matchObj.group(1), matchObj.group(2)
    ...
    def eval_tag(self, tag, formula, already_set_variables):
        ...
        try:
            ...
            self.error('error in evalution of the parameters: expression to be evaluted : %s=%s ' % (tag, formula), exception=True, exit=True, where='eval_tag')

        value = locals()[tag]
        self.log_debug('expression to be evaluted : %s  -> value of %s = %s' % (expr, tag, value), 4, trace='PARAMETRIC_DETAIL,PD')
        return value

    def eval_tags(self, formula, already_set_variables):
        eval_expr = already_set_variables + '\n%s' % formula
        self.log_debug('expression to be evaluated : %s  ' % eval_expr, 4, trace='PARAMETRIC_PROG_DETAIL')
        try:
            exec eval_expr
        except Exception:
            self.error('error in evaluation of the parameters (eval_tags): expression to be evaluted : %s ' % eval_expr, where='eval_tags', exception=True, exit=True)
        ...
        for tag, value in variables.items():
            if tag.find('__') == -1 and ('%s' % value).find('<') == -1:
                values[tag] = value
                self.log_debug('value of %s = %s' % (tag, value), 4, trace='PARAMETRIC_DETAIL,PD,PARAMETRIC_PROG')
    ...
    def read_parameter_file(self):
        self.log_debug('reading yalla parameter files %s' % self.args.parameter_file, 2, trace='YALLA,PARAMETRIC_DETAIL,PD')
        ...
        if not self.args.parameter_filter == None or not self.args.parameter_range == None:
            if self.args.parameter_filter:
                self.log_info('the filter %s will be applied... Only following lines will be taken into account : ' % self.args.parameter_filter)
        ...
        for line in lines:
            ...
            if (line.find('#DECIM') > -1 or line_nb == nb_lines) and in_prog:
                ...
                self.direct_tag_ordered = self.direct_tag_ordered + [t]
                nb_prog = nb_prog + 1
                self.log_debug('prog python found in parametric file:\n%s' % prog, 4, trace='PARAMETRIC_PROG,PARAMETRIC_PROG_DETAIL')
                in_prog = False
            matchObj = re.match('^#DECIM\\s*PYTHON\\s*$', line)
            if matchObj:
                in_prog = True
                prog = ''
                continue
                ...
                if self.args.parameter_range and not self.args.parameter_range == nb_case - 1:
                    continue
                self.log_debug('testing : %s\ntags_names:%s' % (line, tags_names), 4, trace='PARAMETRIC_DETAIL,PD')
                tags = shlex.split(line)
                if not len(tags) == len(tags_names):
                    self.error('\tError : pb encountered in reading the test matrix file : %s ' % self.args.parameter_file + 'at  line \n\t\t!%s' % line + '\n\t\tless parameters to read than expected... Those expected are\n' + '\n\t\t\t %s ' % (',').join(tags_names) + '\n\t\tand so far, we read' + '\n\t\t\t %s' % tag, exception=True, exit=True)
                ts = copy.deepcopy(tags_names)
                tag = {}
                self.log_debug('ts:%s\ntags:%s' % (pprint.pformat(ts), pprint.pformat(tags)), 4, trace='PARAMETRIC_DETAIL,PD')
                while len(ts):
                    t = ts.pop(0)
                    tag['%s' % t] = tags.pop(0)
        ...
        self.log_debug('self.parameters: %s ' % pprint.pformat(self.parameters), 4, trace='PARAMETRIC_DETAIL,PD,PARAMETRIC')
        ...
        if len(l):
            ...
            tag = {}
        self.log_debug('%d parameters before functional_tags : \n %s' % (len(l), l), 4, trace='PARAMETRIC_DETAIL,PD')
        self.log_debug('tag before functional_tags : \n %s' % l.columns, 4, trace='PARAMETRIC_DETAIL,PD')
        self.log_debug('prog before functional_tags : \n %s' % prog, 4, trace='PARAMETRIC_DETAIL,PD')
        if len(self.direct_tag):
            self.log_debug('self.direct_tag %s tag:%s' % (
             pprint.pformat(self.direct_tag), pprint.pformat(tag)), 4, trace='PARAMETRIC_DETAIL,PD')
            self.log_debug('self.direct_tag_ordered %s ' % pprint.pformat(self.direct_tag_ordered), 4, trace='PARAMETRIC_DETAIL,PD')
            for t in self.direct_tag_ordered:
                already_set_variables = ''
                if len(l) > 1:
                    values = l.iloc[[1]]
                    self.log_debug('values on first line : \n %s' % values, 4, trace='PARAMETRIC_DETAIL,PD')
                    for c in l.columns:
                        already_set_variables = already_set_variables + '\n' + '%s = %s ' % (c, l.iloc[0][c])

                self.log_debug('already_set_variables : \n %s' % already_set_variables, 4, trace='PARAMETRIC_DETAIL,PD')
                ...
                if t.find('YALLA_prog') == -1:
                    ...
                    self.log_debug('evaluated! %s = %s = %s' % (tag, formula, result), 4, trace='PARAMETRIC_DETAIL,PD')
                    if isinstance(result, list):
                        ...
                        for row in range(1, len(l)):
                            values = l.iloc[[row]]
                            self.log_debug('values on row %s: \n %s' % (row, values), 4, trace='PARAMETRIC_DETAIL,PD')
                            already_set_variables = ''
                            for c in l.columns:
                                already_set_variables = already_set_variables + '\n' + '%s = %s ' % (c, l.iloc[row][c])

                            self.log_debug('about to be revaluated! t=%s results=%s' % (t, results), 4, trace='PARAMETRIC_DETAIL,PD')
                            result = self.eval_tag(t, formula, already_set_variables)
                            results = results + [result]

                        self.log_debug('evaluated! %s = %s = %s' % (t, formula, results), 4, trace='PARAMETRIC_DETAIL,PD')
                    ...
                    for row in range(1, len(l)):
                        values = l.iloc[[row]]
                        self.log_debug('values on row %s: \n %s' % (row, values), 4, trace='PARAMETRIC_PROG_DETAIL')
                        already_set_variables = ''
                        for c in l.columns:
                            already_set_variables = already_set_variables + '\n' + '%s = %s ' % (c, l.iloc[row][c])
                        ...
                        for v in results.keys():
                            ...
                            if v in result_as_column.keys():
                                if not cmp(result_as_column[v], results_for_this_row[v]) == 0:
                                    self.error('mismatch in parameter list  computed from file %s \n\tfor parameter %s: ' % (self.args.parameter_file, v) + '\n\t    returned %s for first combination ' % result_as_column[v] + '\n\tbut returned %s for %dth combination ' % (results_for_this_row[v], row), exit=True, where='read_parameter_file', exception=False)

                        self.log_debug('evaluated! for row %s = %s' % (row, pprint.pformat(results_for_this_row)), 4, trace='PARAMETRIC_PROG_DETAIL')

                    for v in results.keys():
                        if v not in result_as_column.keys():
        ...
        for p in l.columns.get_values():
            if p in self.slurm_vars.keys():
                if self.slurm_vars[p] == int:
                    self.log_debug('casting parameter %s to int' % p, 4, trace='PS,PARAMETRIC_DETAIL,PD,PARAMETRIC_SUMMARY')
        ...
        self.log_debug(parameter_list, 4, trace='PS,PARAMETRIC_DETAIL,PD,PARAMETRIC_SUMMARY')
        ...
        self.log_debug('critera taken : (%s) from %s' % (cluster_keys, l.columns), 4, trace='PARAMETRIC_DETAIL,PD,GATHER_JOBS,GJ')
        if len(clustering_criteria):
            ...
            self.log_debug('cluster (%s) in %s combinations:' % (
             cluster_keys, len(l_per_clusters)), 4, trace='PARAMETRIC_DETAIL,PD,GATHER_JOBS,GJ')
            for n in l_per_clusters.index:
                ...
                self.array_clustered = self.array_clustered + [
                 values]
                self.log_debug('\n%s' % pprint.pformat(subset), 4, trace='PARAMETRIC_DETAIL,PD,GATHER_JOBS,GJ')

            self.log_info('array_clusters: %s' % pprint.pformat(self.array_clustered), 1, trace='PARAMETRIC_DETAIL,PD,GATHER_JOBS,GJ')
        if self.args.parameter_list:
            sys.exit(0)
        self.parameters = l
    ...
    def print_job(self, job, short_format=True, print_only=[
 'check_previous', 'script_file', 'array', 'inital_array',
 'job_name', 'status', 'content', 'job_id', 'step',
 'dependency', 'make_depend', 'attempt', 'global_completion', 'ntasks', 'nodes'], filter=[
 'python /tmp/dart_mitgcm.py'], filtered=['content'], allkey=True, print_all=False, except_none=False):
        fields_to_compress = [
         'jobs', 'success', 'completion', 'failure',
        ...
        if short_format:
            for k in job.keys():
                ...
                if len('%s' % job[k]) > 100:
                    if filter is not None and k in filtered:
    ...
    def submit_and_activate_job(self, job):
        self.log_info('in submit_and_activate job to submit  %s ' % self.print_job(job), 2, trace='SUBMIT_AND_ACTIVATE')
        submitted_job_ids, cmd = self.submit_job(job, registration=False, resubmit=True, return_all_job_ids=True)
    ...
    def submit_job(self, job, registration=True, resubmit=False, take_lock=False, return_all_job_ids=False):
        if job['job_name'] in self.STEPS_RESTART_STATUS:
            step_only = job['job_name']
            if self.STEPS_RESTART_STATUS[step_only] in ('DONE', ):
                self.log_info('step %s not actually submitted as it was marked as DONE use --resubmit to resubmit it again ' % job['job_name'], 0, trace='RESTART_FAKED,SUBMITTED')
                return (None, 'nothing')
        if len(self.array_clustered) == 0:
            forcing_fields = {}
            return self.submit_same_profile_job(job, forcing_fields, registration, resubmit, take_lock, return_all_job_ids)
        else:
            if len(self.array_clustered) == 1:
                forcing_fields = self.array_clustered[0]
                return self.submit_same_profile_job(job, forcing_fields, registration, resubmit, take_lock, return_all_job_ids)
            self.log_info('%d differents job profiles have been detected... They will be submitted as separate job arrays...' % len(self.array_clustered))
            np = 1
            for profile in self.array_clustered:
                forcing_fields = profile
                new_job = copy.deepcopy(job)
                self.log_info('profile %d: %s ' % (np, pprint.pformat(profile)))
                job_id, cmd = self.submit_same_profile_job(new_job, forcing_fields, registration, resubmit, take_lock, return_all_job_ids, profile_nb=np)
                np = np + 1

            return (job_id, cmd)

    def submit_same_profile_job(self, job, forcing_fields={}, registration=True, resubmit=False, take_lock=False, return_all_job_ids=False, profile_nb=False):
        lock_file = self.take_lock(self.LOCK_FILE)
        ...
        job_default_value = {'array': '1-1', 'attempt': 0, 
           'check': None, 
           'initial_attempt': 0, 
           'make_depend': None, 
           'yalla': 0, 
        ...
        self.log_debug('Scanning file %s for additional slurm parameters' % job['script'], 4, trace='PARSE')
        ...
        self.log_debug('after reading job script file %s job =%s' % (
         job['script'], self.print_job(job, print_only=job.keys())), 4, trace='WRAP,PARSE')
        self.log_debug('after reading job script file %s self.args=%s' % (
         job['script'], pprint.pformat(self.args)), 4, trace='WRAP,PARSE,OVERWRITTING')
        if not job['time']:
        ...
        job_keys = job.keys()
        if 'nodes' not in job_keys and 'ntasks' not in job_keys:
        ...
        for n in ['error', 'output']:
            if job[n] == None:
                ...
                self.log_info('empty job %s filename forced to %s' % (n, job[n]), 4, trace='API,WORKFLOW,ACTIVATE_DETAIL,JOB_OUTERR')

        job_script_content, job = self.wrap_job_script(job, original_script_content_lines, job_file_args_overloaded)
        if 'nodes' not in job.keys():
            job['nodes'] = int(math.ceil(job['ntasks'] / 64.0))
        for n in ['error', 'output']:
            if job[n] == None:
                job[n] = '%s.%%j.%s' % (job['job_name'], n[:3])
                self.log_info('empty job %s filename forced to %s' % (n, job[n]), 4, trace='API,WORKFLOW,ACTIVATE_DETAIL,JOB_OUTERR')
        ...
        if 'initial_array' not in job.keys():
        ...
        if job['job_name'] in self.STEPS_RESTART_STATUS.keys():
            if self.STEPS_RESTART_STATUS[job['job_name']] in ('TO_RERUN', 'TO_RESTART'):
                if job['job_name'] not in self.STEPS_RESTART_ATTEMPT.keys():
        ...
        self.log_debug('self.steps_submitted_attempts=%s' % pprint.pformat(self.steps_submitted_attempts), 4, trace='GLOBAL')
        if step_only in self.steps_submitted_attempts.keys():
            ...
            self.log_info(('step already launched before as %s found ' + 'with status %s and completion %s%%') % (
             previously_registred_step, status, completion), 1, trace='RESTART')
        if not resubmit and step_only not in self.steps_to_restart_anyway and step_only in self.steps_submitted_attempts.keys():
            if status in ('SUBMITTED', 'WAITING', 'PENDING', 'RUNNING', 'SUCCESS'):
                ...
                if previously_registred_step not in self.steps_submitted:
                    self.steps_submitted = self.steps_submitted + [previously_registred_step]
                    self.log_debug('submitted_jobs= [%s]' % (',').join(self.steps_submitted), 4, trace='JOBS')
                    self.save(take_lock=False)
                if self.args.decimate:
                    if not self.args.restart:
                        self.error('this step %s has already been submitted as job %s and reached status %s' % (
                         step, job_id, status) + 'Resubmit with --restart or --scratch to force its continuation or reexecution', exit=True)
                self.log_debug('returning an already submitted job %s' % job_id, 4, trace='RESTART,HEAL,RESTART_FAKED')
                self.release_lock(lock_file)
                return (
                 job_id, cmd)
            resubmitted_job = True
        if resubmit or step not in self.STEPS.keys() or step_only in self.steps_to_restart_anyway:
            if resubmit or step_only in self.steps_to_restart_anyway:
                self.log_info('in resubmitting step %s previously_registred_step=%s' % (
                 step, previously_registred_step), 4, trace='RESTART,HEAL')
                self.steps_submitted = self.steps_submitted + [previously_registred_step]
            ...
            if previously_registred_step is None:
                ...
                self.log_debug('previous step of this kind was step %s:%s' % (
                 ...
                 self.print_job(previous_step, short_format=False)), 4, trace='GLOBAL,GLOBAL_JOBS')
                self.log_debug('current_job to be submitted:%s' % self.print_job(job), 4, trace='GLOBAL,GLOBAL_JOBS')
                self.STEPS[step]['global_status'] = 'WAITING'
                ...
                self.STEPS[step]['global_failure'] = []
                self.STEPS[step]['global_failure_percent'] = 0.0
                ...
                if previously_registred_step == step:
                    ...
                    self.log_debug('filtering resubmitted previous jobs %s : tasks (%s) to be removed' % (
                     previous_jobs, job['array']), 4, trace='GLOBAL,GLOBAL_REDUCTION,GLOBAL_JOBS')
                    for task_to_remove in RangeSet(job['array']):
                        ...
                        self.log_debug('filtering resubmitted previous jobs %s : jobs (%s) to be removed' % (
                         previous_jobs, (',').join(jobs_to_remove)), 4, trace='GLOBAL,GLOBAL_REDUCTION,GLOBAL_JOBS,GLOBAL_TASKS')
                        ...
                        if previous_jobs in ('', ',', ',,'):
                            ...
                            self.log_debug('previous_jobs final %s ' % previous_jobs, 4, trace='GLOBAL,GLOBAL_REDUCTION,GLOBAL_JOBS,GLOBAL_TASKS')
                            new_jobs = []
                    ...
                    self.STEPS[step]['jobs'] = []
                    self.STEPS[step]['array'] = job['array']
                global_completion = ',%s,' % (',').join(map(lambda x: str(x), self.STEPS[step]['global_completion']))
                self.log_debug('global_completion initial %s : (%s) to be removed' % (
                 global_completion, job['array']), 4, trace='GLOBAL,GLOBAL_REDUCTION')
                for task in RangeSet(job['array']):
                    if global_completion.find(',%s,' % task) > -1:
                        global_completion = global_completion.replace(',%s,' % task, ',')
                        self.STEPS[step]['global_completion_percent'] -= 100.0 / self.STEPS[step]['global_items']

                if global_completion in ('', ',', ',,'):
                    self.STEPS[step]['global_completion'] = []
                else:
                    self.log_debug('global_completion final %s ' % global_completion, 4, trace='GLOBAL,GLOBAL_REDUCTION')
                    self.STEPS[step]['global_completion'] = map(lambda x: int(x), global_completion[1:-1].split(','))
        self.log_debug('self.STEPS[step][global_completion] %s ' % self.STEPS[step]['global_completion'], 4, trace='GLOBAL,GLOBAL_REDUCTION')
        ...
        if step not in self.TASKS.keys():
        ...
        self.log_debug('just before submitting step %s:%s' % (
         step, self.print_job(self.STEPS[step], short_format=False)), 4, trace='GLOBAL')
        self.log_debug('just before submitting job to be submitted:%s' % self.print_job(job), 4, trace='GLOBAL')
        self.log_debug('just before submitting step %s : TASKS: \n%s' % (
         step, pprint.pformat(self.TASKS[step])), 4, trace='GLOBAL,GLOBAL_JOBS,GLOBAL_TASKS')
        dep = job['dependency']
        step_dep = None
        if dep:
        ...
        self.log_debug(('starting_indices=[%s], index_to_submit=%s, ' + 'len(index_to_submit)=%s chunk_size=%s, job=%s') % (
         (',').join(map(lambda x: str(x), starting_indices)), index_to_submit,
         len(index_to_submit), chunk_splitted, self.print_job(job)), 4, trace='SUBMIT,CHUNK')
        ...
        for i in starting_indices:
            ...
            if dep and len(chunk_size) > 1 and first_one_in_step and not resubmit and not job['yalla']:
                array_items = [
                 RangeSet('%s-%s' % (index_to_submit[i], index_to_submit[i])),
                 RangeSet(index_to_submit[i + 1:i + chunk_splitted])]
            else:
                array_items = [
                 RangeSet(index_to_submit[i:i + chunk_splitted])]
            self.log_debug('starting indice %s : array_items %s' % (
             i, pprint.pformat(array_items)), 4, trace='GLOBAL,GLOBAL_JOBS,GLOBAL_TASKS')
        ...
        self.log_debug('submit_job self.TASKS[%s]: %s ' % (step, pprint.pformat(self.TASKS[step])), 2, trace='GLOBAL_TASKS')
        ...
        if resubmitted_job:
            ...
            self.log_debug('resubmitting job %s [%s]-->Job # %s <-depends-on %s, current_status=%s' % (
             job['job_name'], RangeSet(job['array']), job_id, job['dependency'], status), 0, trace='RESTART,RESTART_FAKED')
        else:
    ...
    def submit_chunk_job(self, job, registration=True):
        self.log_debug('in submit_chunk JOBS start: %s' % (',').join(map(str, self.JOBS.keys())), 4, trace='CHUNK')
        ...
        if not self.args.quick_launch:
            self.save()
        self.log_debug('submit_chunk_job/end submitting job: %s ' % self.print_job(job), 2, trace='SUBMIT_CHUNK_JOB')
    ...
    def activate_job(self, job, registration=True, take_lock=True):
        lock_file = self.take_lock(self.LOCK_FILE)
        self.log_debug('in activate_job self.waiting_job_final_id=%s' % self.waiting_job_final_id, 4, trace='ACTIVATE_DETAIL')
        cmd = job['cmd']
        step = job['step']
        ...
        self.log_debug('waiting_job_final_id : \n%s ' % pprint.pformat(self.waiting_job_final_id), 4, trace='REPLACE,ACTIVATE_DETAIL,CMD,RESTART')
        ...
        if not self.args.dry:
            ...
            cmd = [
             self.SCHED_SUB, job_script_final]
            ...
            try:
                ...
                if len(still_depends):
                    job_still_to_be_activated = still_depends.split('--dependency=afterany:')[1].split(' #')[0]
                    self.log_debug(('problem in activating job=%s\n script %s   content still contains' + ' unsolved _waiting dependency: %s \n putting it back on the pile ') % (
                     self.print_job(job),
                     job_script_final,
                     job_still_to_be_activated))
                    self.log_info(('problem in activating job script %s   content still contains ' + 'unsolved _waiting dependency: %s \n putting it back on the pile ') % (
                     job_script_final,
                     job_still_to_be_activated))
                    self.release_lock(lock_file)
                    return (
                     job_still_to_be_activated, 'UNKNOWN_DEPENDENCY')
                job_still_to_be_activated = 'All dependent jobs activated'
                self.error(('problem in activating job=%s\n script %s content still contains unsolved' + '_waiting dependency: \n  -------------------------  \n %s' + '\n ----------------------------\nwaiting_job_final_id : ' + '\n%s \njobs_queued: \n%s\njobs_waiting: \n%s') % (
                 self.print_job(job),
                 job_script_final,
                 job_still_to_be_activated,
                 pprint.pformat(self.waiting_job_final_id),
                 pprint.pformat(self.jobs_queued),
                 pprint.pformat(self.jobs_waiting)), killall=10, exit=True, exception=True)
            ...
            self.log_debug('job submitted : %s depends on %s' % (job_id, job['dependency']), 1, trace='ACTIVATE_DETAIL,RESTART')
        else:
        ...
        if job_after:
            if job_after in self.JOBS.keys():
                ...
                self.log_info('strange... job_after %s does not exists in self.JOBS!!! EXITING......' % job_after, 0)
                self.log_debug(('strange... job_after %s does not exists in self.JOBS!!! ' + 'EXITING......\n self.JOBS=%s') % (
        ...
        if not self.args.quick_launch:
            self.save()
        self.log_info(('step %s [%s] activating job %s (for %s) --> ' + 'Job # %s (was %s) <-depends-on %s\njobs_waiting=%s') % (
         self.args.step, self.args.taskid, job['job_name'], job['array'],
         job_id, waiting_job_id, job['dependency'], self.jobs_waiting), 4, trace='ACTIVATE_DETAIL,ACTIVATE,ACTIVATE_NOTE')
        self.release_lock(lock_file)
    ...
    def create_slurm_parser(self, DEBUG=False):
        self.slurm_parser = argparse.ArgumentParser(description='Slurm parser', conflict_handler='resolve')
        self.slurm_options = {}
        self.slurm_vars = {}
                ...
                if options[1][0:2] == '--':
                    ...
                    if long_option.find('=') > -1:
                        ...
                        if help_message.find('number') > -1:
                            ...
                            self.slurm_parser.add_argument(short_option, long_option, type=int, help=help_message)
                            self.slurm_vars[long_option[2:].replace('-', '_')] = int
                        else:
                            self.slurm_parser.add_argument(short_option, long_option, type=str, help=help_message)
                            self.slurm_vars[long_option[2:].replace('-', '_')] = str
                    else:
                        self.slurm_parser.add_argument(short_option, long_option, action='store_true', help=help_message)
    ...
    def complete_slurm_args(self, original_script_content_lines, job):
        ...
        self.log_debug('slurm_args before completion: %s' % pprint.pformat(self.slurm_args), 4, trace='PARSE')
        self.log_debug('slurm_vars=%s' % pprint.pformat(self.slurm_vars), 4, trace='VAR')
        job_file_slurm_args, job_file_extra_args = self.slurm_parser.parse_known_args(job_file_args)
        self.log_debug('slurm_args from job file=%s' % pprint.pformat(job_file_slurm_args), 4, trace='PARSE')
        ...
        for k, v in p_job_file_slurm_args.items():
            if p_job_file_slurm_args[k]:
                if not p_slurm_args[k]:
                    p_slurm_args[k] = v
                    self.log_debug('adding from job_file to slurm_args : --%s=%s' % (k, v), 4, trace='PARSE,OVERWRITTING')
                else:
                    job_file_args_overloaded = job_file_args_overloaded + [k]

        self.log_debug('before leaving complete_slurm_args self.slurm_args=%s' % pprint.pformat(self.slurm_args), 4, trace='WRAP,PARSE,OVERWRITTING')
        ...
        self.log_debug('before leaving complete_slurm_args=%s' % pprint.pformat(self.args), 4, trace='WRAP,PARSE,OVERWRITTING')
        return (
         job, job_file_args_overloaded)

    def wrap_job_script(self, job, original_script_content_lines, job_file_args_overloaded):
        self.log_debug('adding prefix and suffix to job %s [%s] : ' % (job['script'], job['array']), 4, trace='WRAP')
        l0 = ''
        ...
        for li in original_script_content_lines:
            if li[:7] == '#SBATCH':
                ...
                if slurm_option in job_file_args_overloaded:
                    ...
                    l = l + '###DECIM###' + li[:-1] + ' ### <-- now OVERLOADED by -%s=%s ###\n' % (slurm_option, args[slurm_option])
                else:
                    l = l + '###DECIM###' + li
            else:
                l = l + li
        ...
        if self.args.print_environment:
            ...
            for env_var in ['PATH', 'LD_LIBRARY_PATH', 'PYTHONPATH',
             'SLURM_ARRAY_TASK_ID', 'SLURM_ARRAY_JOB_ID']:
                environment_variables = environment_variables + 'echo export %s=$%s \n' % (env_var, env_var)

        environment_variables = environment_variables + '\n# Starting from the right directory \n\n'
        environment_variables = environment_variables + 'cd %s \n' % job['submit_dir']
        l0 = l0 + ('sleep 1\npython %s/%s --decimate --step %s --attempt __ATTEMPT__ ' + '--attempt-initial __ATTEMPT_INITIAL__ --log-dir %s  %s %s %s--spawned ') % (
         '/tmp',
        ...
        if job['dependency']:
            ...
            if job['dependency'] not in self.JOBS.keys():
                step_names = map(lambda k: k[0].split('-')[:-1][0], self.STEPS.items())
                step_names.sort()
                if job['dependency'] not in step_names:
                ...
                if not job_found:
                    self.error(('jobs depends on unknown job id or name : %s\n   Weird... ' + 'it was found exisiting in job_names though... \n    ---> (%s)') % (
        ...
        files_to_sync = '%s/*py*' % self.SAVE_DIR + (' ').join(self.files_to_copy_to_tmp)
        prefix = prefix + RSYNC_CMD % files_to_sync
        check_previous = '# Checking the status of previous Jobs\n' + '\n%s  --check-previous-step \n' % l0 + 'if [ $? -ne 0 ] ; then\n' + '   echo "[ERROR] FAILED in precedent step : ' + 'stopping current job and reconnecting..."\n' + '    exit 1\n' + 'fi\n\n'
        if job['yalla']:
            tasks = []
            for t in RangeSet(job['array']):
                tasks = tasks + [t]

            prefix = prefix + check_previous.replace('${SLURM_ARRAY_TASK_ID}', '%s' % tasks[0]).replace('${SLURM_ARRAY_JOB_ID}', '${SLURM_JOB_ID}').replace('--check-previous-step', '--check-previous-step > $output_file.checking.out 2> $error_file.checking.err')
        ...
        if self.args.parameter_file:
            prefix0 = prefix0 + '\n#Sourcing parameters taken from file: %s ' % self.args.parameter_file
            prefix0 = prefix0 + '\n.  %s.${SLURM_ARRAY_TASK_ID}' % self.PARAMETER_FILE
        l = prefix0 + '\n\n# Starting user job\n' + '\n# ---------------- START OF ORIGINAL USER SCRIPT  -------------------\n' + l.replace('#DECIM PROCESS_TEMPLATE_FILES', '# should replace template here returning in the right directory \n\n' + 'HERE_TO_PROCESS=$PWD \n (cd %s;' % job['submit_dir'] + '%s --process-templates $HERE_TO_PROCESS; cd - )' % l0).replace('#DECIM SHOW_PARAMETERS', 'cat  %s.${SLURM_ARRAY_TASK_ID}' % self.PARAMETER_FILE) + '\n# ----------------   END OF ORIGINAL USER SCRIPT  -------------------\n\n'
        ...
        self.log_debug('wrap job before yalla filling =%s' % self.print_job(job), 4, trace='WRAP')
        if job['yalla']:
            if not job['nodes'] and not job['ntasks']:
                self.error('when asking for a Yalla pool, either nodes or ntasks has to be valued', exit=True)
            nb_jobs = len(RangeSet(job['array']))
            job['yalla_parallel_runs'] = min(job['yalla_parallel_runs'], nb_jobs)
            d, h, m, s = ([
             0, 0, 0, 0, 0] + map(lambda x: int(x), job['time'].split(':')))[-4:]
            self.log_debug("job['yalla_parallel_runs']=%s" % job['yalla_parallel_runs'], 4, trace='YALLA,Y')
            factor = math.ceil(nb_jobs / (job['yalla_parallel_runs'] + 0.0))
            whole_time = (((d * 24 + h) * 60 + m) * 60 + s) * factor
            ...
            self.log_debug('yalla related parameters in job:%s' % self.print_job(job, print_only=['time', 'ntasks', 'nodes', 'array',
            ...
            for w in ['output', 'error']:
                s = ('%s/%s' % (job['submit_dir'], job[w])).replace('%J', '\\$SLURM_JOB_ID').replace('%j', '\\$SLURM_JOB_ID').replace('%a', '\\${task}').replace('%04a', '\\${formatted_task}`')
    ...
    def launch_jobs(self, **optional_parameters):
        ...
        if not self.args.quick_launch:
            self.save(take_lock=False)
        self.log_debug('after feed_workflow in launch_jobs, self.STEPS=[%s]' % (',').join(self.STEPS.keys()), 4, trace='FEED,LAUNCH')
        self.release_lock(lock_file)
        if not self.args.decimate:
            self.tail_log_file(nb_lines_tailed=1, no_timestamp=True, stop_tailing=[
             'workflow is finishing', 'workflow is aborting'])
        sys.exit(0)
    ...
    def feed_workflow(self, from_finalize=False):
        ...
        if self.TASK_ID == RangeSet(self.TASK_IDS)[0] or not self.args.spawned:
            ...
            for j in self.jobs_list:
                ...
                elif status not in ('DONE', 'ABORTED', 'WAITING'):
                    nb_jobs_running_or_queued_computed += 1
                    jobs_queued_computed = jobs_queued_computed + [j]

            self.log_debug('feed_workflow: %d jobs_queued' % len(jobs_queued), 1, trace='FEED_DETAIL')
            self.log_debug('feed_workflow: %d jobs_queued:%s' % (
             len(jobs_queued), pprint.pformat(jobs_queued)), 2, trace='FEED_DETAIL')
            self.log_debug('feed_workflow: %d jobs_waiting:%s' % (
             len(jobs_waiting), (',').join(map(lambda x: str(x), jobs_waiting))), 1, trace='FEED_DETAIL,FEED')
            ...
            if nb_jobs_waiting > 0:
                self.log_debug('self.args.max_jobs(%d)-nb_jobs_running_or_queued(%d)=%d' % (
                 ...
                 self.args.max_jobs - nb_jobs_running_or_queued), 1, trace='FEED_DETAIL')
                if from_finalize:
                ...
                self.log_debug('feed_workflow: nb_jobs_to_add=%s' % nb_jobs_to_add, 1, trace='FEED_DETAIL,FEED')
                if nb_jobs_to_add > 0:
                    nb_job_added = 0
                    self.log_debug('feed_workflow: job_waiting=[%s] ' % (',').join(map(lambda x: str(x), self.jobs_waiting)), 1, trace='FEED_DETAIL,ACTIVATE_NOTE')
                    while nb_job_added < nb_jobs_to_add and len(self.jobs_waiting):
                        ...
                        self.log_debug('feed_workflow: activating job %s (%s) ' % (
                         job_to_add, job['array']), 1, trace='FEED_DETAIL,ACTIVATE_NOTE')
                        job_id, cmd = self.activate_job(job, take_lock=False)
                        self.log_debug('feed_workflow: activating job %s (%s) (was %s) -> job # %s' % (
                         job['step'], job['array'], job_to_add, job_id), 1, trace='FEED_DETAIL,FEED')
                        if cmd == 'UNKNOWN_DEPENDENCY':
                            self.jobs_waiting = [
                             job_id] + self.jobs_waiting
                        else:
    ...
    def react_on_key(self, x):
        if x in ('b', 'B'):
            ...
            self.log_debug('searching %s in possible choice:%s' % (current, possibles), 4, trace='KEY')
            ...
            if len(possibles) > 1:
                ...
                while len(possibles) and not item_found:
                    ...
                    self.log_debug('scanning %s, result=%s next=%s previous=%s' % (
                     t, '%s' % current == '%s' % t, next_possible, previous_possible), 4, trace='KEY')
                    if '%s' % current == '%s' % t:
                        if x == '-':
                        ...
                        if x in ('*', '/'):
                            ...
                            try:
                                ...
                                self.error('could not get first job for step: %s ' % self.print_job(step, print_only=step.keys()), exit=True, exception=True)
            ...
            if len(file_list):
                ...
                if len(choices):
                    choice, extra_choices, current_choices, offset = self.choose_in_set(choices, get_choices=True)
                    if choice:
                        self.file_chosen = file_chosen = choice.split(' ')[(-1)].replace(colors.BOLD, '').replace(colors.END, '')
                        ...
                        while more_file_chosen:
                            ...
                            more_what = '-' * first_dashes_length + ' %s ' % file_chosen + '-' * second_dashes_length
                            if False:
                                print more_what
                            ...
                            try:
                                ...
                                f_lines = [
                                 'Problem found while opening file %s' % file_chosen]
                                return
                            ...
                            self.log_debug(('back from more  self.current_key=%s' + ' self.args.inputs=%s extra_choices=%s') % (
                            ...
                            if len(self.args.input):
                                if self.args.input[0] in extra_choices:
                                    ...
                                    self.log_debug('new_choice=%s, position=%s, file_list=[%s], file_chosen=%s' % (
                                     new_choice, position, (',').join(file_list), file_chosen), 4, trace='KEY')
                                    self.general_info()
    ...
    def general_info(self):
        ...
        if False:
            ...
            if self.show_zeroed_files:
                ...
                zeroed_files = 'NO'
            print '\rstep %s from [%s]' % (self.console_current_step_name, self.console_step_filter) + '- jobs %s from %s (j for more)' % (self.console_current_job, len(self.console_current_jobs)) + '- task %s from [%s] ' % (self.console_current_task, self.console_current_tasks) + '  [0 files %s] ' % zeroed_files
    ...
    def choose_in_set(self, choices, on_key_return=None, line_number=True, get_choices=False, extra_choices=''):
        if on_key_return is None:
            on_key_return = self.menu_keys
        on_key_return = on_key_return + extra_choices
        possible_indice = '1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' + '!@#$%^&*()_+=-[]{};:",./<>`~'
        ...
        while not choice_made:
            ...
            if len(current_choices):
                ...
                more_message = 2
                sys.stdout.flush()
                self.log_debug('remaining line in the more session [%s] ' % (',').join(choice_list), 4, trace='MORE,KEY')
                if line_number:
                ...
                if x in current_choices_keys or x in on_key_return:
                    self.current_x = self.last_x
                    self.args.input = x + self.args.input
                    self.log_debug('Choice picked up or command called... leaving the more session ', 4, trace='KEY')
                    choice_made = True
        ...
        while True:
            ...
            if x in on_key_return:
                self.log_debug('key %s hit to be taken in account by general menu ' % x, 4, trace='KEY')
                self.current_x = self.last_x
                self.args.input = x + self.args.input
                if get_choices:
            ...
            if x in indexed_choices.keys():
                if get_choices:
                    ...
                    return (
                     str(indexed_choices[x]), current_choices_keys, current_choices, offset)
                else:
                    self.log_debug('return str(indexed_choices[x])=>%s<' % str(indexed_choices[x]), 4, trace='KEY')
    ...
    def fix(self):
        ...
        for j in self.JOBS.keys():
            ...
            if job['job_name'] == '2' or True:
                print '%s' % self.print_job(job, print_only=['array', 'job_id', 'make_depend', '0kept'], allkey=False)
        ...
        for step in self.STEPS.keys():
            if step[0] not in '01234' or step[0:2] == '10':
