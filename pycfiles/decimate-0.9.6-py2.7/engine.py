...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/decimate/engine.py
# Compiled at: 2018-04-07 11:30:10
import argparse
...
import fcntl, getpass, glob, logging, logging.handlers, os, pickle, pprint, re, subprocess, sys, time, traceback
JOB_POSSIBLE_STATES = (
...
JOB_RUNNING_STATES = (
 ...
 'CONFIGURING', 'RESIZING')
JOB_DONE_STATES = (
 'COMPLETED', 'FAILED', 'SUCCESS', 'FAILURE', 'MIXED')
JOB_ABORTED_STATES = (
 'ABORTED', 'CANCELLED', 'CANCELLED+', 'FAILED',
...
class my_list(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        LOG.log_debug('%s_L_init %s %s' % (self.name_of_dict,
         pprint.pformat(args), pprint.pformat(kwargs)), 3, trace='DIST')
        self.list_content = []

    def __len__(self):
        LOG.log_debug('%s_L_len %s' % (self.name_of_dict, len(self.list_content)), 3, trace='DIST')
        return len(self.list_content)

    def __add__(self, other):
        self.list_content = self.list_content + other
        LOG.log_debug('%s_A_add %s' % (self.name_of_dict, pprint.pformat(self.list_content)), 3, trace='DIST')
        return self

    def __getitem__(self, index):
        LOG.log_debug('%s_L_item[%s]=%s' % (self.name_of_dict, index, self.list_content[index]), 3, trace='DIST')
        result = self.list_content[index]
        return result

    def __repr__(self):
        s = pprint.pformat(self.list_content)
        LOG.log_debug('%s_L_print[%s]' % (self.name_of_dict, s), 3, trace='DIST')
        return s

    def __getslice__(self, i, j):
        LOG.log_debug('%s_L_slice[%s]' % (self.name_of_dict, slice(i, j)), 3, trace='DIST')
        return self.list_content.__getitem__(slice(i, j))

    def update(self, *args, **kwargs):
        LOG.log_debug('%s_D_update %s %s' % (self.name_of_dict,
         pprint.pformat(args),
         pprint.pformat(kwargs)), 3, trace='DIST')
...
class my_dict(dict):
    ...
    def __getitem__(self, key):
        ...
        LOG.log_debug('%s_D_GET[%s]' % (self.name_of_dict, key), 3, trace='DIST')
        return val

    def __setitem__(self, key, val):
        LOG.log_debug('%s_D_SET[%s]=%s' % (self.name_of_dict, key, val), 3, trace='DIST')
...
class engine(object):
    ...
    def __init__(self, app_name='app', app_version='?', app_dir_log=False, engine_version_required=ENGINE_VERSION, extra_args=False):
        global SCHED_TYPE
        self.NewLock = False
        self.CanLock = True
    ...
    def initialize_parser(self):
        ...
        self.parser.add_argument('-f', '--filter', type=str, default='no_filter', help='filtering traces')
        self.parser.add_argument('-m', '--mail-verbosity', action='count', default=0, help='sends a mail tracking the progression of the workflow')
        ...
        self.parser.add_argument('-nfu', '--no-fix-unconsistent', action='store_true', help='do not fix unconsistent steps', default=False)
        LOG = self
    ...
    def initialize_log_files(self):
        ...
        if owner_of_log == os.getuid():
            handler = logging.handlers.RotatingFileHandler(self.log_file_name, maxBytes=20000000, backupCount=5)
    ...
    def dump_exception(self, where='Not known'):
        ...
        if exceptionTraceback or exceptionType or exceptionValue:
            ...
            traceback.print_exception(exceptionType, exceptionValue, exceptionTraceback, limit=None, file=sys.stdout)
        print (
    ...
    def check_if_directory_was_moved(self):
        ...
        if len(already_processed_job_files) == 0:
            self.log_debug('No job was ever processed or submitted yet, everything should be fine', 4, trace='RELOCATE')
        ...
        if not old_save_dir_found:
            self.log_debug('No old self.SAVE_DIR trace found... everything should be fine', 4, trace='RELOCATE')
            return
        if old_save_dir_found == self.SAVE_DIR:
            self.log_debug('No old self.SAVE_DIR same as now... everything should be fine', 4, trace='RELOCATE')
            return
        ...
        self.ask('do you want to relocate the run to the current directory (%s)' % current_root, default='n')
    ...
    def env_init(self):
        ...
        if not (self.args.go_on or self.args.log or self.args.status or self.args.status_long or self.args.status_all):
    ...
    def archive(self, step, attempt, status):
        ...
        self.log_debug('step %s-%s @ status %s archived' % (step, attempt, status), 0, trace='ARCHIVE')
    ...
    def rollback_workflow(self, key='*'):
        ...
        self.ask(('Do you really want to rollback to step %s' + ' and clear status and results for steps followng it? ') % step_attempt, default='n')
        ...
        steps_to_forget = ('__xxx__%s__xxx__' % ('__xxx__').join(steps)).split(step_attempt)[1].split('__xxx__')[1:-1]
    ...
    def forget_steps(self, steps_to_forget):
        self.ask('Do you really want to clear status and results for steps [%s] ' % (',').join(steps_to_forget), default='n')
        jobs_to_forget = []
        ...
        for step in self.steps_submitted:
            if step not in steps_to_forget:
                steps = steps + [step]

        self.steps_submitted = steps
        jobs = []
        for job in self.jobs_list:
            if job not in jobs_to_forget:
                jobs = jobs + [job]
    ...
    def save(self, take_lock=True):
        ...
        try:
            ...
            self.error(workspace_file + '.new    not here anymore', exit=True, exception=True)
        ...
        try:
            if self.args.load:
                ...
                if os.path.exists(saved_workfile):
                    ...
                    self.error('[load] problem while loading saved workfile for trace=/%s/' % self.args.load, exit=True, exception=True)
                ...
                if self.args.load:
                    if not self.already_loaded:
                        ...
                        if os.path.exists(trace_file):
                            ...
                            self.error('[load] problem while loading tracing data for trace=/%s/' % self.args.load, exit=True, exception=True)
        except Exception:
    ...
    def update_task_status(self, what, step, job_id, task_id, attempt, checking_from_console):
        ...
        if not self.TASKS[step][task_id]['counted']:
            ...
            if additional_user_checking:
                task_to_check = [
                 task_id]
                is_user_checked, tasks_not_completed = self.check_current_state(what, attempt, task_to_check, checking_from_console=checking_from_console)
                if task_id in self.STEPS[step]['global_completion']:
                    for stuff in ['global_completion', 'global_success', 'global_failure', 'completion', 'success', 'failure']:
                        if task_id not in self.STEPS[step][stuff]:
                            continue
                        self.STEPS[step][('%s_percent' % stuff)] -= 100.0 / self.STEPS[step]['global_items']
                        i = self.STEPS[step][stuff]
                        previous_stuff = ',%s,' % (',').join(map(lambda x: str(x), i))
                        self.log_debug('for %s before cleaning previous_stuff=/%s/' % (
                         stuff, previous_stuff), 4, trace='CLEAN')
                        ...
                        self.log_debug('for %s after cleaning self.STEPS[step][%s]=/%s/' % (
                         stuff, stuff, self.STEPS[step][stuff]), 4, trace='CLEAN')
                ...
                if is_user_checked is True:
                    ...
                    self.JOBS[job_id]['success_percent'] += 100.0 / self.JOBS[job_id]['items']
                    self.STEPS[step]['success_percent'] += 100.0 / self.STEPS[step]['items']
                    self.STEPS[step]['global_success_percent'] += 100.0 / self.STEPS[step]['global_items']
                    self.TASKS[step][task_id]['status'] = 'SUCCESS'
                    self.JOBS[job_id]['completion'] = self.JOBS[job_id]['completion'] + [task_id]
                    self.JOBS[job_id]['completion_percent'] += 100.0 / self.JOBS[job_id]['items']
                    self.STEPS[step]['completion'] = self.STEPS[step]['completion'] + [task_id]
                    self.STEPS[step]['completion_percent'] += 100.0 / self.STEPS[step]['items']
                    ...
                    self.JOBS[job_id]['failure_percent'] += 100.0 / self.JOBS[job_id]['items']
                    self.STEPS[step]['failure_percent'] += 100.0 / self.STEPS[step]['items']
                    self.STEPS[step]['global_failure_percent'] += 100.0 / self.STEPS[step]['global_items']
                    ...
                    self.STEPS[step]['completion_percent'] += 100.0 / self.STEPS[step]['items']
                    self.STEPS[step]['global_completion_percent'] += 100.0 / self.STEPS[step]['global_items']
                if 100.0 - self.JOBS[job_id]['completion_percent'] < 1e-06:
                    self.JOBS[job_id]['status'] = 'DONE'
                    if 100.0 - self.JOBS[job_id]['success_percent'] < 1e-06:
                        self.JOBS[job_id]['status'] = 'SUCCESS'
                    elif 100.0 - self.JOBS[job_id]['failure_percent'] < 1e-06:
                        self.JOBS[job_id]['status'] = 'FAILURE'
                    else:
                        self.JOBS[job_id]['status'] = 'MIXED'
                else:
                    self.JOBS[job_id]['status'] = 'RUNNING'
                if 100.0 - self.STEPS[step]['completion_percent'] < 1e-06:
                    self.STEPS[step]['status'] = 'DONE'
                    if 100.0 - self.STEPS[step]['success_percent'] < 1e-06:
                        self.STEPS[step]['status'] = 'SUCCESS'
                    elif 100.0 - self.STEPS[step]['failure_percent'] < 1e-06:
                    ...
                    self.STEPS[step]['status'] = 'RUNNING'
                self.TASKS[step][task_id]['counted'] = True
        self.log_debug(("at the end updating task status self.STEPS[step]['status']=%s" + "self.JOBS[job_id]['status']=%s self.TASKS[step][task_id]['status']=%s") % (
         self.STEPS[step]['status'], self.JOBS[job_id]['status'],
         self.TASKS[step][task_id]['status']), 4, trace='CHECK')

    def get_current_jobs_status(self, in_queue_only=False, take_lock=True, from_heal_workflow=True, from_resubmit=False):
        lock_file = self.take_lock(self.LOCK_FILE)
        ...
        for step in self.STEPS.keys():
            self.log_debug("step=>%s<, self.STEPS[step]['status']=>%s<, self.STEPS[step]['completion_percent']=>%s<" % (
             step, self.STEPS[step]['status'], self.STEPS[step]['completion_percent']), 1, trace='STATUS_DETAIL')
            self.log_debug("self.STEPS[step]['jobs']=%s" % pprint.pformat(self.STEPS[step]['jobs']), 4, trace='STATUS_DETAIL')
            status = self.STEPS[step]['status']
            self.log_info('get_current_job_status: status of step %s : %s' % (step, status), 1)
            if status not in ('ABORTED', 'WAITING'):
                steps_to_check[step] = True
                for job_id in self.STEPS[step]['jobs']:
                    if ('%s' % job_id).find('-') == -1:
                        job = self.JOBS[job_id]
                        self.log_debug(('step=>%s<,job_id=>%s<,' + "job['completion_percent']=>%s<,job['status']=>%s<") % (
                         step, job_id, job['completion_percent'], job['status']), 1, trace='STATUS_DETAIL')
                        if job['completion_percent'] < 100.0 and not (job['status'] in JOB_DONE_STATES or job['status'] == 'WAITING'):
                            jobs_to_check[job_id] = True

        self.log_debug('%d jobs to check:  %s' % (
         len(jobs_to_check), (',').join(map(str, jobs_to_check.keys()))), 4, trace='STATUS_DETAIL')
        if len(jobs_to_check) == 0 and in_queue_only:
            self.release_lock(lock_file)
            return []
        if len(jobs_to_check) > 0:
            cmd = 'squeue -r -o "%i %T" -j ' + (',').join(map(lambda x: str(x), jobs_to_check))
            self.log_debug('cmd to get new status via squeue : %s' % ('').join(cmd), 4, trace='STATUS_DETAIL')
            jobs_gone = []
            output = self.system(cmd, force=1)
            self.log_debug('cmd to get new status via squeue : %s' % 'DONE', 4, trace='STATUS_DETAIL')
            ...
            cmd = [
             '/opt/slurm/default/bin/sacct', '-n', '-p', '-j', (',').join(map(lambda x: str(x), jobs_to_check))]
            cmd = (' ').join(cmd)
            self.log_debug('cmd to get new status via sacct : %s' % ('').join(cmd), 1, trace='STATUS_DETAIL,STATUS')
            try:
                sacct_attempt = 0
                error_in_sacct = True
                ...
                while error_in_sacct and sacct_attempt < 3:
                    ...
                    if error_in_sacct:
                        self.log_debug('pb to get sacct,  \n \t\tretrying in %d sec..' % sacct_waiting_time, 1, trace='STATUS_DETAIL,STATUS')
                ...
                for l in output[:-1].split('\n'):
                    ...
                    try:
                        ...
                        if sacct_fields[0].find('.') == -1:
                            ...
                            if task_field not in tasks_checked.keys():
                    ...
                    try:
                        ...
                        if status in JOB_POSSIBLE_STATES:
                            ...
                            for task in tasks:
                                ...
                                if job_id not in self.JOBS.keys():
                                    self.error('job_id=%s received from sacct and not registered\n' % job_id + ('[get_current_job_status] parse job_status with task=%s' + '\n job_id=%s\n job status: l=%s\noutput=>%s< \n cmd=>%s<') % (
                                ...
                                self.log_debug('is_task_counted?=>>%s<<' % self.TASKS[step][task_id]['counted'], 2, trace='STATUS_DETAIL')
                                self.TASKS[step][task_id]['status'] = status
                                self.update_task_status(what, step, job_id, task_id, attempt, checking_from_console=True)
                                self.flush_mail()

                        else:
                            self.log_info('unknown status got for tasks %s' % (',').join(tasks), 1, trace='STATUS_DETAIL')
                    except Exception:
                        self.error(('[get_current_job_status] parse job_status with task=%s ' + '\n job_id = %s \n job status : l=%s') % (task, job_id, l), exception=True, exit=False)
                        status_error = True
        ...
        for step_name in self.STEPS.keys():
            ...
            if self.STEPS[step_name]['status'] == 'DONE':
                self.log_debug('step %s found DONE : %s' % (
                 step_name, self.print_job(step, print_only=step.keys())), 4, trace='UNCONSISTENT')
                if self.args.ends < 0:
                    unconsistent_steps = unconsistent_steps + [step_name]
                elif int(step_name.split('-')[0]) <= int(self.args.ends):
                    unconsistent_steps = unconsistent_steps + [step_name]
                continue
            if not len(step['completion']) == int(step['items']) or abs(step['completion_percent'] - 100.0) > 1e-06:
        ...
        if len(unconsistent_steps) and not self.args.no_fix_unconsistent:
            ...
            for step_name in unconsistent_steps:
                ...
                self.log_debug('self.TASK[%s]=%s' % (step_name, pprint.pformat(self.TASKS[step_name])), 4, trace='UNCONSISTENT')
                ...
                actual_status = self.check_current_state(what, attempt, tasks, checking_from_console=True, from_heal_workflow=from_heal_workflow)
                ...
                for task_id in RangeSet(self.STEPS[step_name]['initial_array']):
                    ...
                    if not job_id == 'UNKNOWN':
                        ...
                        self.update_task_status(what, step_name, job_id, task_id, attempt, checking_from_console=True)
                ...
                self.log_debug('step %s fixed to (status=%s, completion=%s, completion_percent=%s success=%s success_percent=%s failure=%s failure_percent=%s items=%s' % (
                 ...
                 step['items']), 4, trace='UNCONSISTENT')

        nb_jobs_active = 0
        for job_id in jobs_to_check:
            job = self.JOBS[job_id]
            if job['status'] not in JOB_DONE_STATES and job['status'] not in JOB_ABORTED_STATES:
                self.log_debug('active found job  %s  step %s  status %s ' % (
                 job_id, job['step'], job['status']), 1, trace='STATUS_DETAIL,ACTIVE')
                nb_jobs_active = nb_jobs_active + 1

        if nb_jobs_active == 0 and not self.args.spawned and not self.args.decimate and not from_resubmit:
            self.log_info('no active job in the queue, changing all WAITING in ABORTED???')
        if False:
            for step in self.STEPS.keys():
                self.log_debug('step %s  status= %s' % (step, self.STEPS[step]['status']), 2, trace='STATUS_DETAIL')
                if self.STEPS[step]['status'] in ('SUBMITTED', 'WAITING', 'RUNNING'):
                    self.STEPS[step]['status'] = 'ABORTED'
                    for job_id in self.STEPS[step]['jobs']:
                        self.log_debug('step %s/job=%s  status= %s' % (
                         step, job_id, self.STEPS[step]['status']), 2, trace='STATUS_DETAIL')
                        if self.JOBS[job_id]['status'] in ('SUBMITTED', 'WAITING',
                                                           'RUNNING'):
                            self.JOBS[job_id]['status'] = 'ABORTED'
            ...
            for job_id in self.JOBS.keys():
                ...
                if status in ('SUBMITTED', 'WAITING', 'RUNNING'):
                    self.log_debug('job %s  ----> status= ABORTED (was %s)' % (job_id, status), 2, trace='STATUS_DETAIL')
                    self.JOBS[job_id]['status'] = 'ABORTED'
        ...
        if len(steps_to_check):
            ...
            for step in steps_to_check.keys():
                if self.STEPS[step]['status'] in ('WAITING', 'RUNNING', 'SUBMITTED'):
                    ...
                    for job_id in self.STEPS[step]['jobs']:
                        status = self.JOBS[job_id]['status']
                        if status not in jobs_status.keys():
    ...
    def add_step_to_history(self, step, step_dep):
        ...
        if step not in self.steps_submitted:
            ...
            if step_dep:
                ...
                if step_dep not in self.steps_submitted_history.keys():
                    self.error('for step %s on job dependency %s not found in self.steps_submitted_history %s' % (
                     step, step_dep, pprint.pformat(self.steps_submitted_history)), exit=True, exception=True)
    ...
    def release_lock(self, filename):
        if not self.CanLock:
            return
        if filename not in self.lock_taken.keys():
            self.log_debug('release_log: lock was not taken yet on %s ......' % filename, 4, trace='LOCK')
            ...
            if self.lock_taken[filename] == 0:
                ...
                self.log_debug('lock completely released  on %s ' % filename, 4, trace='LOCK')
                self.log_debug('lock completely released  on %s ' % filename, 4, trace='LOCK_DETAIL', stack=True)
                return
        self.log_debug('self.lock_released=%s %s times' % (
         filename, self.lock_taken[filename]), 4, trace='LOCK_DETAIL', stack=True)

    def system(self, cmd, comment='No comment', fake=False, verbosity=1, force=False, trace=True, return_code=False):
        ...
        if trace and self.args.load:
            self.log_info('reading from trace file / output = /%s/' % len(self.SYSTEM_OUTPUTS[self.args.load]), 2, trace='SYSTEM')
    ...
    def tail_log_file(self, filename=None, keep_probing=-999, nb_lines_tailed=20, message=True, no_timestamp=False, stop_tailing=False):
        if keep_probing == -999:
    ...
    def check_for_pattern(self, pattern, file_mask=None, files=None, return_all=False, col=None):
        ...
        if not file_mask and not files:
            self.error("in check_for_pattern('%s') on must add at least file_mask or files parameter" % pattern)
    ...
    def apply_tags(self, file=False, input=False, include_path=['.']):
            ...
            if len(fields) == 2:
                try:
                    ...
                    if expression == '#':
                        ...
                        endif_found = False
                        output_to_add = ''
                        while len(lines_to_process) and not (expression_found and then_present and endif_found) and not (expression_found and not then_present):
                            l = lines_to_process.pop(0)
                            if not re.search('^\\s*#', l):
                                expression_found = True
                                if l.find('___INCLUDE___') > -1:
                                    self.error('ERROR: including file inside and ___IF___ long zone is not yet supported! \n\t check line  (%s)' % l, exit=True)
                            ...
                            elif len(l.split('___ENDIF___')) == 2:
                                endif_found = True
                            else:
                                output_to_add = output_to_add + l
                        ...
                        self.log_debug('IF at the end of line found at %s' % l, 4, trace='TEMPLATE,IF,SHORTIF')
                        l = fields[0]
                        while len(l) > 0 and (l[(-1)] == '#' or l[(-1)] == ' '):
                            l = l[:-1]
                    ...
                    cond = fields[1][:-1]
                    self.log_debug('for l=%s\n evaluating (%s)= %s' % (l, cond, eval(cond)), 4, trace='TEMPLATE')
                    self.log_debug('evaluating (%s)= %s' % (cond, eval(cond)), 4, trace='EVAL')
                    if not eval(cond):
                        continue
                    l = output_to_add
                except Exception:
                    self.error('ERROR: could not evaluate %s for template file %s' % (cond, file), exit=True)
                ...
                if not file_to_include[0] == '/':
                    ...
                    if not include_file_found:
                        self.error('ERROR: could not find included file %s for template file %s in possible directories (%s)' % (
                         file_to_include, file, (',').join(include_dir_candidates)), exit=True)
                output = output + self.apply_tags(included_file)
                continue
            output = output + l

        self.log_debug('result template :\n----- %s -----------\n%s\n-------------------' % (file, output), 4, trace='TEMPLATE,TEMPLATE_RESULT')
        ...
        mo = re.search('(__[\\w.]+__)', output_test)
        if mo:
