...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/pwclip/lib/colortext/colortext.py
...
def colortext(color, text):
    ...
    if osname == 'nt':
        ...
        if __c == 'whi':
            string = '%s37m' % string
        __coloredtext = '%s%s\x1b[0m' % (string, text)
    ...
    if osname == 'nt':
        ...
        if not prompt:
            if 'color' not in kwargs.keys():
                kwargs['color'] = 'grn'
            ynq = '\x1b[0m%s' % colortext(kwargs['color'], '?')
            if yesno:
                ynq = '%s [%s/%s]' % (ynq, y, n)
            args[-1] = '%s%s\n' % (args[(-1)], ynq)
...
def error(*args, **kwargs):
    ...
    if osname == 'nt':
        ...
        errfile = ''
        errline = ''
        end = '\n' if 'end' not in kwargs.keys() else kwargs['end']
        buzzword = 'ERROR:'
        ...
        if 'warn' in kwargs.keys():
            ...
            if 'buzzword' in kwargs.keys():
                buzzword = kwargs['buzzword']
        if 'sep' in kwargs.keys():
            delim = kwargs['sep']
    msgs = [
     errfile + errline + red(buzzword)]
    for arg in args:
        if args.index(arg) % 2 == 0:
            if not arg:
                pass
...
def fatal(*args, **kwargs):
    ...
    if osname == 'nt':
        ...
        if 'sep' in kwargs.keys():
            delim = kwargs['sep']
    msgs = [
     '%s%s%s' % (errfile, errline, bred('FATAL:'))]
    for arg in args:
        if args.index(arg) % 2 == 0:
            if not arg:
                pass
...
def tabs(dat, ind=0, ll=80):
    ...
    return '\n'.join('%s%s' % (' ' * ind, dat[i:int(i + ll)]) for i in range(0, len(dat), ll))
...
def tabl(dats, ind=0, iind=0):
    ...
    for i in dats:
        if isinstance(i, (tuple, list)):
            tabbl = '%s\n%s' % (tabbl,
             tabl(i, ind + iind if iind else 2, iind if iind else 2))
            continue
        else:
