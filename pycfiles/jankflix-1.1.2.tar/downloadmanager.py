...
# [GCC 8.4.0]
# Embedded file name: /home/christian/workspace_py/jankflix-python/jankflixmodules/utils/downloadmanager.py
# Compiled at: 2013-01-14 12:45:03
...
def start_downloads(url_path_pairs):
    ...
    for url, path in url_path_pairs:
        ...
        process = multiprocessing.Process(target=download, args=[url, path, mystatus, 32768])
        process.start()
...
def download(url, path, status, block_size=8192):
    ...
    while True:
        ...
        status.put('%10d  [%3.2f%%]' % (file_size_dl, file_size_dl * 100.0 / file_size))
