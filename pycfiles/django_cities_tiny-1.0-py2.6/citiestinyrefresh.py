...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/cities_tiny/management/commands/citiestinyrefresh.py
# Compiled at: 2012-03-14 13:44:02
import urllib, time, os, logging, zipfile, optparse
...
lang_valid_chars = {'ru': re.compile('^[Ѐ-ӿ-]+$'), 
   'uk': re.compile('^[Ѐ-ӿ-]+$'), 
   'by': re.compile('^[Ѐ-ӿ-]+$')}
...
class Command(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
     optparse.make_option('--force-import-all', action='store_true', default=False, help='Import even if files are up-to-date.'),
     optparse.make_option('--force-all', action='store_true', default=False, help='Download and import if files are up-to-date.'),
     optparse.make_option('--force-import', action='append', default=[], help='Import even if filenames matching FORCE_IMPORT are up-to-date.'),
     optparse.make_option('--force', action='append', default=[], help='Download and import if filenames matching FORCE are up-to-date.'))
    ...
    def handle(self, *args, **options):
        ...
        def prepare_file(url):
            ...
            if posixpath.splitext(basename)[1] == '.zip':
                ...
                if not os.path.exists(dest_path2) or os.stat(dest_path2).st_mtime < os.stat(dest_path).st_mtime:
        ...
        if 'verbosity' in options:
            log_level = min(50, max(10, 10 + 20 * (2 - int(options['verbosity']))))
        ...
        for url in sources:
            (file_name, downloaded) = prepare_file(url)
            force_import = options['force_import_all'] or posixpath.basename(file_name) in options['force_import']
    ...
    def download(self, url, path, force=False):
        ...
        remote_time = time.strptime(remote_file.headers['last-modified'], '%a, %d %b %Y %H:%M:%S %Z')
    ...
    def get_admin_by_code_list(self, country, code_list):
        ...
        try:
            ...
            try:
                self._admin_code_map[country.pk][code_list] = AdminDivision.objects.by_code_list(country, code_list)
            except AdminDivision.DoesNotExist:
                log.warning('Admin division zone with codes %s does not exists', ('.').join(code_list))
                return
    ...
    def country_import(self, file_path, alt_names):
        for row in self.parse(file_path):
            ...
            if country is None:
                try:
                    country = Country.objects.get(geoname_id__isnull=True, code2=code2)
                except Country.DoesNotExist:
    ...
    def admin_import(self, file_path, alt_names):
        for row in self.parse(file_path):
            ...
            try:
                ...
                try:
                    obj = AdminDivision.objects.get(geoname_id__isnull=True, country=country, admin1=admin1, admin2=admin2, admin3=admin3, admin4=admin4)
                except AdminDivision.DoesNotExist:
            ...
            if obj.allow_update or obj.allow_name_update:
                try:
                    ...
                    log.warning('AdminDivision IntegrityError: %s: country=%s, admin1=%s, admin2=%s, admin3=%s, admin4=%s', str(e), repr(obj.country.name_ascii), repr(admin1), repr(admin2), repr(admin3), repr(admin4))
                    transaction.savepoint_rollback(sid)
    ...
    def update_admin_hierarchy(self):
        for obj in AdminDivision.objects.filter(level__gt=1, allow_update=True):
            ...
            level = obj.level - 1
            while code_list:
                obj.parent = self.get_admin_by_code_list(obj.country, code_list)
        ...
        for row in self.parse(file_path):
            ...
            try:
                ...
                if new_admin is not None:
                    ...
                    kw = {}
                try:
                    obj = City.objects.get(geoname_id__isnull=True, country=country, name_ascii=name_ascii, **kw)
                except City.DoesNotExist:
