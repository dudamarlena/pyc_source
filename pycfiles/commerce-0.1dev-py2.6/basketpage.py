...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/commerce/old/basketpage.py
# Compiled at: 2009-03-17 14:41:06
    ...
    def validate(self, field, value):
        if value is None:
            ...
            if value.lower() != self.word.lower():
                raise formal.FieldValidationError("You must enter '%s'" % self.word)
            return
...
class BasketPage(formal.ResourceMixin, common.Page):
    ...
    def helperFactory(self, ctx):
        ...
        return dw_basket.BasketResourceHelper(self.avatar, self.avatar.basket, self.basketURL, self.basketURL.child('checkout'), itemFromId)
    ...
    def _buildSalesOrder(self, storeSession, data):
        ...
        salesOrder = SalesOrder(total_price=basket.getTotalPrice(), customer_id='1', customer_version='1', customer_first_name=checkOutDetails['firstName'], customer_last_name=checkOutDetails['lastName'], customer_address=billingAddress, customer_postcode=checkOutDetails['billingPostcode'], customer_country=checkOutDetails['billingCountry'], customer_phone_number=checkOutDetails['phoneNumber'], customer_email=checkOutDetails['email'], delivery_name=deliveryName, delivery_address=deliveryAddress, delivery_postcode=deliveryPostcode, delivery_country=checkOutDetails['billingCountry'], message=checkOutDetails['message'])
