...
# [GCC 8.4.0]
# Embedded file name: .\src\bots\test.py
# Compiled at: 2019-12-23 10:37:36
# Size of source mod 2**32: 16378 bytes
"""
The purpose of this bot is to test the application and crawlers
"""
import io, os, platform, sys, traceback
...
class TestBot:
    ...
    def start(self):
        try:
            try:
                randomized = sorted((crawler_list.keys()),
                  key=(lambda x: random()))
                        ...
                        try:
                            ...
                            try:
                                ...
                                if link not in self.allerrors:
                                    self.allerrors[link] = []
                                self.allerrors[link].append('> Input: %s\n%s\n%s' % (entry, err, ''.join(traces)))
                            finally:
                                err = None
                                del err
    ...
    def finalize(self):
        if len(self.allerrors):
            ...
            print(message)
            error_count = len([x for x in self.allerrors.keys() if x in self.test_user_inputs if x not in self.allowed_failures])
            if error_count > 0:
    ...
    def error_message(self):
        ...
        for source in sorted(self.allerrors.keys()):
            ...
            if source in self.allowed_failures:
                ...
                if source not in self.test_user_inputs:
                    output += ' (no tests)'
            output += '\n'
    ...
    test_user_inputs = {'http://gravitytales.com/':[
