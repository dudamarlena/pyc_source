...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kafka_logger/handlers.py
# Compiled at: 2019-07-25 07:27:01
"""This module contains logging handler which forwards logs to Kafka."""
import atexit, json, logging
...
class KafkaLoggingHandler(logging.Handler):
    ...
    def __init__(self, hosts_list, topic, security_protocol='SSL', ssl_cafile=None, kafka_producer_args=None, additional_fields={}, flush_buffer_size=None, flush_interval=5.0, unhandled_exception_logger=None, log_preprocess=None):
        """
        ...
        self.max_buffer_size = flush_buffer_size if flush_buffer_size is not None else float('inf')
        ...
        if kafka_producer_args is None:
            kafka_producer_args = {}
        self.producer = KafkaProducer(bootstrap_servers=hosts_list, security_protocol=security_protocol, ssl_cafile=ssl_cafile, value_serializer=(lambda msg: json.dumps(msg).encode('utf-8')), **kafka_producer_args)
        atexit.register(self.at_exit)
        sys.excepthook = self.unhandled_exception
        self.main_process_pid = os.getpid()
        self.mp_log_queue = Queue()
        self.mp_log_handler_flush_lock = Lock()
        self.mp_log_handler_thread = Thread(target=self.mp_log_handler, name='Kafka Logger Multiprocessing Handler')
    ...
    def prepare_record_dict(self, record):
        ...
        for key, value in record.__dict__.items():
            ...
            if key == 'created':
                ...
                rec['timestamp'] = ('{0!s}.{1:03d}Z').format(created_date.strftime('%Y-%m-%dT%H:%M:%S'), int(created_date.microsecond / 1000))
    ...
    def mp_log_handler(self):
        ...
        while True:
            ...
            record_dict = self.mp_log_queue.get(block=True)
    ...
    def at_exit(self):
        ...
        if self.unhandled_exception_logger is not None:
            try:
                ...
                if children:
                    self.unhandled_exception_logger.warning('There are %d child process(es) at the moment of the main process termination. This may cause logs loss.', len(children))
    ...
    def unhandled_exception(self, exctype, exception, traceback):
        ...
        if self.unhandled_exception_logger is not None:
            ...
            self.unhandled_exception_logger.exception('Unhandled top-level exception', exc_info=(
             exctype, exception, traceback))
            if sys.version_info[0] < 3:
