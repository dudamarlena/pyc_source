...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/epscController.py
# Compiled at: 2009-05-29 13:49:21
import os, sys, threading, wx, signal
    ...
    def start_file(self, arg):
        ...
        while fid:
            ...
            if line.find('material data') >= 0:
                ...
                os.system(dictCopy[os.name] + strCopy + matFile + ' "' + epscComp.config.dirEpscCore + matFile + '"')
                self.makeOptTemplate()
            if line.find('shape+texture') >= 0:
                texFile = fid.readline()
                strAll += texFile
                texFile = texFile[0:len(texFile) - 1]
                os.system(dictCopy[os.name] + strCopy + texFile + ' "' + epscComp.config.dirEpscCore + texFile + '"')
            if line.find('diffraction') >= 0:
                strAll += fid.readline()
                diffFile = fid.readline()
                strAll += diffFile
                diffFile = diffFile[0:len(diffFile) - 1]
                os.system(dictCopy[os.name] + strCopy + diffFile + ' "' + epscComp.config.dirEpscCore + diffFile + '"')
            ...
            if line.find('Number of thermomechanical') >= 0:
                ...
                for i in range(processNum):
                    ...
                    os.system(dictCopy[os.name] + strCopy + processFile[i] + ' "' + epscComp.config.dirEpscCore + processFile[i] + '"')
