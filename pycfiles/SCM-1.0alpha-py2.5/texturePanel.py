...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/texturePanel.py
# Compiled at: 2009-05-29 13:49:24
import wx, wx.grid, os, epscComp.config
...
class TexturePanel(wx.Panel, SCMPanel):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.label_random10 = wx.StaticText(self, -1, 'Random generated grains (1000 grains)', size=(300,
                                                                                                     20))
        self.radio_random2916 = wx.RadioButton(self, -1, '')
        self.label_random2916 = wx.StaticText(self, -1, 'Random 10 degree mesh(2916 grains)', size=(300,
                                                                                                    20))
        self.radio_mg1944 = wx.RadioButton(self, -1, '')
        self.label_mg1944 = wx.StaticText(self, -1, 'Magnesium extrusion texture (1944 grains)', size=(300,
                                                                                                       20))
        self.radio_custom = wx.RadioButton(self, -1, '')
        self.label_custom = wx.StaticText(self, -1, 'Custom texture file', size=(300,
                                                                                 20))
    ...
    def OnOK(self, event):
        if os.name == 'nt':
            strCopy = 'copy '
            os.system(strCopy + '"' + epscComp.config.dirEpscCore + epscComp.config.file_Texture[self.controller.epscData.textureFileNum] + '" "' + epscComp.config.dirEpscCore + 'TEXTURE_' + self.dataPhaseNum()[self.kind] + '.tex"')
        else:
            strCopy = 'cp '
            os.system(strCopy + epscComp.config.dirEpscCore + epscComp.config.file_Texture[self.controller.epscData.textureFileNum] + ' ' + epscComp.config.dirEpscCore + 'TEXTURE_' + self.dataPhaseNum()[self.kind] + '.tex')
