...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/plotPanelSimulation.py
# Compiled at: 2009-05-29 13:49:10
import wx, sys, scipy, epscComp.config, epscComp.utility
...
class PlotPanelSimulation(wx.Panel):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.exp_y = [0.0, -73.97, -147.94, -221.91, -295.88, -369.85, -443.82,
         -517.79, -591.75, -668.55, -744.69, -817.95, -894.54, -971.56,
         -1043.3, -1099.13, -1147.84, -1204.84, -1239.9, -1248.05, -1286.99,
         -1311.0, -1319.53, -1336.57, -1341.16, -1349.12, -1359.27, -1359.28]
        voceName = [
         'Modulus(GPa)', 'Sigma Zero(MPa)', 'Sigma One(MPa)', 'Theta Zero(MPa)', 'Theta One(MPa)']
        ...
        for i in range(5):
            ...
            if i == 0:
                self.slider.append(wx.Slider(self, -1, 70, 10, 1000, (10, 10), (250,
                                                                                40), wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS))
            elif i == 2:
                self.slider.append(wx.Slider(self, -1, 400, 10, 1000, (10, 10), (250,
                                                                                 40), wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS))
            elif i == 4:
                self.slider.append(wx.Slider(self, -1, 30, 0, 100, (10, 10), (250,
                                                                              40), wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS))
                ...
                self.slider.append(wx.Slider(self, -1, 700, 10, 1000, (10, 10), (250,
                                                                                 40), wx.SL_HORIZONTAL | wx.SL_AUTOTICKS | wx.SL_LABELS))
            if i != 4:
                self.slider[i].SetTickFreq(5, 1)
                self.text_min.append(wx.TextCtrl(self, -1, '10', size=(50, -1)))
        ...
        self.radio = wx.RadioBox(self, -1, '', wx.DefaultPosition, wx.DefaultSize, ['Compression', 'Tension'])
    ...
    def update_y(self, modulus, tau0, tau1, theta0, theta1):
        model_y = []
        epsilon_0 = float(tau0) / float(modulus) * 0.1
        for x in self.model_x:
            if abs(x) <= abs(epsilon_0):
                ...
                y = tau0 + (tau1 + theta1 * abs(abs(x) - epsilon_0)) * (1 - scipy.exp(-theta0 * abs(abs(x) - epsilon_0) / tau1))
                if self.tension == 0:
                    y = -1 * y
                model_y.append(y)
    ...
    def OnExp(self, event):
        dlg = wx.FileDialog(self, message='Choose an experiment data file', defaultDir=epscComp.config.dirEpscCore, defaultFile='', wildcard='All files (*.*)|*.*', style=wx.OPEN | wx.CHANGE_DIR)
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if epscComp.utility.checkExp(self.expFile, ['Macro_Strain', 'Macro_Stress']) == False:
                ...
                return
            self.collectExpData()
            plotinfos_exp = {'name': 'exp', 'x': list(self.macroExpX), 'y': list(self.macroExpY), 
               'type': 'marker', 'color': 'blue', 'width': 1}
            self.Parent.updatePlot('macro', plotinfos_exp)

    def OnClear(self, event):
        self.flagExpFile = False
        self.expFile = ''
        plotinfos_exp = {'name': 'exp', 'x': [], 'y': [], 'type': 'marker', 'color': 'blue', 'width': 1}
        self.Parent.updatePlot('macro', plotinfos_exp)
        plotinfos_model = {'name': 'model', 'x': [], 'y': [], 'type': 'marker', 'color': 'blue', 'width': 1}
        self.Parent.updatePlot('macro', plotinfos_model)
