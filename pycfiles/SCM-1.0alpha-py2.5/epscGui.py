...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/epscGui.py
...
class EpscGui(wx.Frame):
    ...
    def __init__(self, parent, id=-1, title='Cy-SCM (EPSC)', pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.DEFAULT_FRAME_STYLE | wx.SUNKEN_BORDER | wx.CLIP_CHILDREN):
        ...
        self._mgr.AddPane(self.treePanel, wx.aui.AuiPaneInfo().Name('treePane').Caption('Active Data').MinSize(wx.Size(200.4)).Left().Layer(1).CloseButton(True).MaximizeButton(True))
        self.plotPanel = PlotPanel(self)
        self.plotPanel.controller = self.controller
        self._mgr.AddPane(self.plotPanel, wx.aui.AuiPaneInfo().Name('plotPane').Caption('Plot Options').MinSize(wx.Size(200.4)).Left().Layer(1).Position(1).CloseButton(True).MaximizeButton(True))
        self.centerPanels = {'Phase1': MaterialsNotebook(self), 
        ...
        for key in self.centerPanels:
            self._mgr.AddPane(self.centerPanels[key], wx.aui.AuiPaneInfo().Name(key).MinSize(wx.Size(500, 500)).CenterPane().Hide())
    ...
    def panelsData(self):
        return {201: 'Phase1', 2021: '2Phase1', 2022: '2Phase2', 2031: '2Phase1_BMG', 
           2032: '2Phase2_BMG', 401: 'Opt', 
           402: 'Opt'}

    def phaseSet(self):
        return {201: '1Phase', 2021: '2Phase', 2022: '2Phase', 2031: 'BMG', 2032: 'BMG', 401: 'Opt', 
           402: 'Opt'}
    ...
    def checkForSave(self):
        ...
        if self.controller.epscData.isAltered and self.flagSave == False:
            d = wx.MessageDialog(self, 'Would you like to save this session?', 'Save?', wx.YES_NO | wx.CANCEL)
            code = d.ShowModal()
            if code == wx.ID_YES:
    ...
    def OnOpen(self, event):
        dlg = wx.FileDialog(self, message='Choose a project file', defaultDir=os.getcwd(), defaultFile='', wildcard='SCM project (*.prj)|*.prj|All files (*.*)|*.*', style=wx.OPEN | wx.CHANGE_DIR)
    ...
    def OnSave(self, event):
        if self.project.flag == True:
            ...
            dlg = wx.FileDialog(self, message='Choose a project file', defaultDir=os.getcwd(), defaultFile='project1.prj', wildcard='SCM project (*.prj)|*.prj|All files (*.*)|*.*', style=wx.SAVE | wx.CHANGE_DIR)
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnView(self, event):
        ...
        elif self._mgr.GetPane('outputPane').IsShown() == True:
