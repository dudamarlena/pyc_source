...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/optFCCDataPanel.py
# Compiled at: 2009-05-29 13:49:10
import wx
from hklDialog import HKLDialog
from SCMPanel import SCMPanel

class OptFCCDataPanel(wx.Panel, SCMPanel):
    """
    Optimization data input panel for FCC structure
    ...
    def __init__(self, *args, **kwds):
        ...
        self.chbox_voceFCC = [[], []]
        self.label_name_voceFCC = []
        self.label_initial_voceFCC = []
        self.label_low_voceFCC = []
        self.label_high_voceFCC = []
        self.text_initial_voceFCC = [[], []]
        self.text_low_voceFCC = [[], []]
        self.text_high_voceFCC = [[], []]
        self.nameVoce = ['Tau Zero', 'Tau One', 'Theta Zero', 'Theta One']
        for i in range(2):
            self.label_name_voceFCC.append(wx.StaticText(self, -1, 'Name '))
            self.label_initial_voceFCC.append(wx.StaticText(self, -1, 'Initial '))
            self.label_low_voceFCC.append(wx.StaticText(self, -1, 'Low '))
            self.label_high_voceFCC.append(wx.StaticText(self, -1, 'High '))
            for j in range(4):
                self.chbox_voceFCC[i].append(wx.CheckBox(self, -1, self.nameVoce[j]))
                self.text_initial_voceFCC[i].append(wx.TextCtrl(self, -1, size=(50,
                                                                                -1)))
                self.text_low_voceFCC[i].append(wx.TextCtrl(self, -1, size=(50, -1)))
                self.text_high_voceFCC[i].append(wx.TextCtrl(self, -1, size=(50, -1)))
                self.text_initial_voceFCC[i][j].Enable(False)
    ...
    def disableRanges(self):
        for i in range(2):
            for j in range(4):
                self.text_low_voceFCC[i][j].Enable(False)
                self.text_high_voceFCC[i][j].Enable(False)

    def enableRanges(self):
        for i in range(2):
            for j in range(4):
                self.text_low_voceFCC[i][j].Enable(True)
                self.text_high_voceFCC[i][j].Enable(True)
    ...
    def showData(self):
        ...
        for i in range(2):
            for j in range(4):
                self.chbox_voceFCC[i][j].SetValue(self.controller.epscData.optData.voceFlag[i][j])
                self.text_low_voceFCC[i][j].SetValue(str(self.controller.epscData.optData.lowVoce[i][j]))
                self.text_high_voceFCC[i][j].SetValue(str(self.controller.epscData.optData.highVoce[i][j]))
    ...
    def showVoce(self):
        for i in range(2):
            if i + 1 in self.controller.epscData.matParam['Phase1'].selectedSystems:
                for j in range(4):
                    self.chbox_voceFCC[i][j].Enable(True)
                    self.text_initial_voceFCC[i][j].SetValue(str(self.controller.epscData.matParam['Phase1'].voce[i][j]))

            else:
                for j in range(4):
                    self.chbox_voceFCC[i][j].Enable(False)
            ...
            if i + 1 in self.controller.epscData.matParam['Phase1'].selectedSystems:
                for j in range(4):
                    if self.controller.epscData.optData.voceFlag[i][j] == 1:
                        self.text_initial_voceFCC[i][j].SetValue(str(p[count]))
                        count += 1
    ...
    def OnOK(self, event):
        ...
        for i in range(2):
            for j in range(4):
                self.controller.epscData.optData.voceFlag[i][j] = self.chbox_voceFCC[i][j].GetValue()
                self.controller.epscData.optData.lowVoce[i][j] = self.text_low_voceFCC[i][j].GetValue()
                self.controller.epscData.optData.highVoce[i][j] = self.text_high_voceFCC[i][j].GetValue()
    ...
    def __do_layout(self):
        ...
        for i in range(2):
            ...
            sizer_ph1_voce.Add(self.label_name_voceFCC[i], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
            sizer_ph1_voce.Add(self.label_initial_voceFCC[i], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
            sizer_ph1_voce.Add(self.label_low_voceFCC[i], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
            sizer_ph1_voce.Add(self.label_high_voceFCC[i], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
            for j in range(4):
                sizer_ph1_voce.Add(self.chbox_voceFCC[i][j], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
                sizer_ph1_voce.Add(self.text_initial_voceFCC[i][j], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
                sizer_ph1_voce.Add(self.text_low_voceFCC[i][j], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
                sizer_ph1_voce.Add(self.text_high_voceFCC[i][j], 0, wx.ALL | wx.ADJUST_MINSIZE, 5)
