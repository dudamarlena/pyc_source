...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/deformationPanel.py
...
class DeformationPanel(wx.Panel, SCMPanel):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.label_thetaZero = wx.StaticText(self, -1, 'Theta Zero (GPa)', size=(100,
                                                                                 20))
        self.label_thetaOne = wx.StaticText(self, -1, 'Theta One (GPa)', size=(100,
                                                                               20))
        if self.controller.epscData.phaseNum == 2032:
            self.label_Matrix = wx.StaticText(self, -1, 'Only one slip system for matrix', size=(200,
                                                                                                 20))
            ...
            if self.controller.epscData.matParam[self.kind].typeCrystal == 0:
                ...
                labelBCC = [
                 '{110}<111> Slip', '{112}<111> Slip', '{123}<111> Slip']
                for i in range(3):
                    self.chbox_BCC.append(wx.CheckBox(self, -1, ''))
                    self.label_BCC.append(wx.StaticText(self, -1, labelBCC[i], size=(170,
                                                                                     20)))
            ...
            if self.controller.epscData.matParam[self.kind].typeCrystal == 1:
                self.chbox_FCC = []
                self.label_FCC = []
                ...
                for i in range(1):
                    ...
                    self.label_FCC.append(wx.StaticText(self, -1, labelFCC[i], size=(170,
                                                                                     20)))
            ...
            for i in range(8):
                ...
                self.label_HCP.append(wx.StaticText(self, -1, labelHCP[i], size=(170,
                                                                                 20)))
