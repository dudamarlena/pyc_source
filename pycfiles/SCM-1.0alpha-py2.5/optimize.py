...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/epscApp/epscComp/optimize.py
# Compiled at: 2009-05-29 13:49:10
__all__ = [
 'fmin', 'fmin_powell', 'fmin_bfgs', 'fmin_ncg', 'fmin_cg',
 'fminbound', 'brent', 'golden', 'bracket', 'rosen', 'rosen_der',
 'rosen_hess', 'rosen_hess_prod', 'brute', 'approx_fprime',
 'line_search', 'check_grad', 'simplex']
import numpy
from numpy import atleast_1d, eye, mgrid, argmin, zeros, shape, empty, squeeze, isscalar, vectorize, asarray, absolute, sqrt, Inf, asfarray, isinf
...
def fmin(func, x0, args=(), xtol=0.0001, ftol=0.0001, maxiter=None, maxfun=None, full_output=0, disp=1, retall=0, callback=None):
    ...
    if full_output:
        retlist = (
         x, fval, iterations, fcalls[0], warnflag)
        if retall:
            retlist += (allvecs,)
    else:
        retlist = x
...
def zoom(a_lo, a_hi, phi_lo, phi_hi, derphi_lo, phi, derphi, phi0, derphi0, c1, c2):
    ...
    return (
     a_star, val_star, valprime_star)


def line_search(f, myfprime, xk, pk, gfk, old_fval, old_old_fval, args=(), c1=0.0001, c2=0.9, amax=50):
    ...
    alpha1 = pymin(1.0, 2.02 * (phi0 - old_old_fval) / derphi0)
    if alpha1 == 0:
        alpha_star = None
        fval_star = old_fval
...
def fmin_bfgs(f, x0, fprime=None, args=(), gtol=1e-05, norm=Inf, epsilon=_epsilon, maxiter=None, full_output=0, disp=1, retall=0, callback=None):
    ...
    while gnorm > gtol and k < maxiter:
        pk = -numpy.dot(Hk, gfk)
        (alpha_k, fc, gc, old_fval, old_old_fval, gfkp1) = linesearch.line_search(f, myfprime, xk, pk, gfk, old_fval, old_old_fval)
        if alpha_k is None:
            (alpha_k, fc, gc, old_fval, old_old_fval, gfkp1) = line_search(f, myfprime, xk, pk, gfk, old_fval, old_old_fval)
            if alpha_k is None:
                warnflag = 2
                break
        xkp1 = xk + alpha_k * pk
    ...
    if full_output:
        retlist = (
         xk, fval, gfk, Hk, func_calls[0], grad_calls[0], warnflag)
        if retall:
            retlist += (allvecs,)
    else:
        retlist = xk
...
def fmin_cg(f, x0, fprime=None, args=(), gtol=1e-05, norm=Inf, epsilon=_epsilon, maxiter=None, full_output=0, disp=1, retall=0, callback=None):
    ...
    while gnorm > gtol and k < maxiter:
        ...
        (alpha_k, fc, gc, old_fval, old_old_fval, gfkp1) = linesearch.line_search(f, myfprime, xk, pk, gfk, old_fval, old_old_fval, c2=0.4)
        if alpha_k is None:
            (alpha_k, fc, gc, old_fval, old_old_fval, gfkp1) = line_search(f, myfprime, xk, pk, gfk, old_fval_backup, old_old_fval_backup)
    ...
    if full_output:
        retlist = (
         xk, fval, func_calls[0], grad_calls[0], warnflag)
        if retall:
            retlist += (allvecs,)
    else:
        retlist = xk
...
def fmin_ncg(f, x0, fprime, fhess_p=None, fhess=None, args=(), avextol=1e-05, epsilon=_epsilon, maxiter=None, full_output=0, disp=1, retall=0, callback=None):
    ...
    if full_output:
        retlist = (
         xk, fval, fcalls[0], gcalls[0], hcalls, warnflag)
        if retall:
            retlist += (allvecs,)
    else:
        retlist = xk
...
class Brent:
    ...
    def get_result(self, full_output=False):
        if full_output:
            return (
             self.xmin, self.fval, self.iter, self.funcalls)
        else:
            return self.xmin
...
def fmin_powell(func, x0, args=(), xtol=0.0001, ftol=0.0001, maxiter=None, maxfun=None, full_output=0, disp=1, retall=0, callback=None, direc=None):
    ...
    if full_output:
        retlist = (
         x, fval, direc, iter, fcalls[0], warnflag)
        if retall:
            retlist += (allvecs,)
    else:
        retlist = x
...
class Result:
    ...
    def __init__(self, x, fx, iters, calls, status):
        (self.x, self.fx, self.iters, self.calls) = (
         x, fx, iters, calls)
        self.status = status

    def __str__(self):
        return 'Minimum %g at %s after %d calls' % (self.fx, self.x, self.calls)
...
def simplex(f, x0=None, bounds=None, radius=0.05, xtol=0.0001, ftol=0.0001, maxiter=None, update_handler=None, abort_test=dont_abort):
    """
    ...
    *Notes*
    ...
    while iterations < maxiter:
        if numpy.all(abs(sim[1:] - sim[0]) <= xtol) and max(abs(fsim[0] - fsim[1:])) <= ftol:
            break
