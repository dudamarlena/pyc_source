...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/kedro_viz/server.py
# Compiled at: 2020-02-21 04:43:58
# Size of source mod 2**32: 11100 bytes
""" Kedro-Viz plugin and webserver """
import hashlib, json, multiprocessing, socket, webbrowser
from collections import defaultdict
from contextlib import closing
from pathlib import Path
from typing import Dict
import click, kedro, requests
...
data = None
app = Flask(__name__,
  static_folder=(str(Path(__file__).parent.absolute() / 'html' / 'static')))
ERROR_PROJECT_ROOT = 'Could not find a Kedro project root. You can run `kedro viz` by either providing `--load-file` flag with a filepath to a JSON pipeline representation, or if the current working directory is the root of a Kedro project.'
ERROR_PIPELINE_FLAG_NOT_SUPPORTED = '`--pipeline` flag was provided, but it is not supported in Kedro version {}'.format(kedro.__version__)

@app.route('/')
@app.route('/<path:subpath>')
def root(subpath='index.html'):
    """Serve the non static html and js etc"""
    return send_from_directory((str(Path(__file__).parent.absolute() / 'html')),
      subpath, cache_timeout=0)
...
def _allocate_port(start_at: int, end_at: int=65535) -> int:
    ...
    raise ValueError('Cannot allocate an open TCP port for Kedro-Viz in a range from {} to {}'.format(start_at, end_at))
...
def _load_from_file(load_file: str) -> dict:
    ...
    for key in ('nodes', 'edges', 'tags'):
        if key not in data:
            raise KedroCliError("Invalid file, top level key '{}' not found.".format(key))
...
def _get_pipeline_from_context(context, pipeline_name):
    if match(kedro.__version__, '>=0.15.2'):
        return context._get_pipeline(name=pipeline_name)
    else:
        if pipeline_name:
...
def format_pipeline_data(pipeline, catalog):
    ...
    for namespace, tags in sorted(namespace_tags.items()):
        ...
        nodes.append({'type':'parameters' if is_param else 'data', 
         'id':_hash(namespace), 
...
@click.option('--browser/--no-browser',
  default=True,
  help='Whether to open viz interface in the default browser or not. Defaults to True.')
...
@click.option('--pipeline',
  type=str,
  default=None,
  help='Name of the modular pipeline to visualize. If not set, the default pipeline is visualized')
@click.option('--env',
  '-e',
  type=str,
  default=None,
  multiple=False,
  envvar='KEDRO_ENV',
  help='Kedro configuration environment. If not specified, catalog config in `local` will be used')
...
def _call_viz(host=None, port=None, browser=None, load_file=None, save_file=None, pipeline_name=None, env=None):
    global data
    if load_file:
