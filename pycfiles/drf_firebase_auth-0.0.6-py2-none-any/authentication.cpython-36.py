...
# [GCC 8.4.0]
# Embedded file name: /code/server/drf_firebase_auth/authentication.py
# Compiled at: 2019-02-10 03:42:42
...
"""
import json, uuid, firebase_admin
...
User = get_user_model()
firebase_credentials = firebase_admin.credentials.Certificate(api_settings.FIREBASE_SERVICE_ACCOUNT_KEY)
...
class FirebaseAuthentication(BaseFirebaseAuthentication):
    ...
    def get_token(self, request):
        ...
        if not authorization_header or len(authorization_header) != 2:
            raise exceptions.AuthenticationFailed('Invalid Authorization header format, expecting: JWT <token>.')
        if smart_text(authorization_header[0].lower()) != auth_header_prefix:
            raise exceptions.AuthenticationFailed('Invalid Authorization header prefix, expecting: JWT.')
    ...
    def decode_token(self, firebase_token):
        ...
        try:
            ...
            raise exceptions.AuthenticationFailed('JWT was found to be invalid, or the App’s project ID cannot be determined.')
        except firebase_auth.AuthError as exc:
            if exc.code == 'ID_TOKEN_REVOKED':
                raise exceptions.AuthenticationFailed('Token revoked, inform the user to reauthenticate or signOut().')
            else:
        ...
        try:
            ...
            if api_settings.FIREBASE_AUTH_EMAIL_VERIFICATION:
                if not firebase_user.email_verified:
                    raise exceptions.AuthenticationFailed('Email address of this user has not been verified.')
            return firebase_user
        except ValueError:
            raise exceptions.AuthenticationFailed('User ID is None, empty or malformed')
        except firebase_auth.AuthError:
            raise exceptions.AuthenticationFailed('Error retrieving the user, or the specified user ID does not exist')

    def get_or_create_local_user(self, firebase_user):
        """
        Attempts to return or create a local User from Firebase user data
        """
        email = firebase_user.email if firebase_user.email else firebase_user.provider_data[0].email
        try:
            ...
            if not api_settings.FIREBASE_CREATE_LOCAL_USER:
                raise exceptions.AuthenticationFailed('User is not registered to the application.')
            username = '_'.join(firebase_user.display_name.split(' ') if firebase_user.display_name else str(uuid.uuid4()))
    ...
    def create_local_firebase_user(self, user, firebase_user):
        ...
        if not local_firebase_user:
            new_firebase_user = FirebaseUser(uid=(firebase_user.uid),
              user=user)
        ...
        for provider in firebase_user.provider_data:
            local_provider = FirebaseUserProvider.objects.filter(provider_id=(provider.provider_id),
              firebase_user=local_firebase_user).first()
            if not local_provider:
                new_local_provider = FirebaseUserProvider.objects.create(provider_id=(provider.provider_id),
                  uid=(provider.uid),
                  firebase_user=local_firebase_user)
                new_local_provider.save()

        local_providers = FirebaseUserProvider.objects.filter(firebase_user=local_firebase_user)
