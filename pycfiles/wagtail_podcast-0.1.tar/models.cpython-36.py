...
# [GCC 8.4.0]
# Embedded file name: /home/daniel/podcast/base/wagtail_podcast/wagtail_podcast/models.py
# Compiled at: 2018-10-03 08:27:36
# Size of source mod 2**32: 13980 bytes
import django.http
...
class PodcastParentPage(RoutablePageMixin, Page):
    ...
    languages = (('af-ZA', 'Afrikaans'), ('en-ZA', 'English'))
    language = models.CharField(max_length=5, choices=languages, default='af-ZA', help_text=(_('Language of podcast')), verbose_name=(_('Language')))
    copyright = models.CharField(max_length=255, help_text=(_('Copyright holder of podcast')), verbose_name=(_('Copyright')))
    ...
    owner_name = models.CharField(max_length=255, help_text=(_('Owner of the podcast channel')), verbose_name=(_('Owner name')))
    ...
    category = models.CharField(max_length=255, help_text=(_('Category of the podcast address')), verbose_name=(_('Category')))
    page_text = RichTextField(null=True, help_text=(_('Main text page that appears on the root page')), verbose_name=(_('Page Text')))
    ...
    explicit = models.BooleanField(default=False, help_text=(_('Whether the podcast channel contains explicit content')), verbose_name=(_('Explicit')))
    content_panels = Page.content_panels + [
     ...
     MultiFieldPanel([
      ...
      FieldPanel('blubrry_url')],
       heading=(_('Podcast Directory URLs')), classname='collapsible collapsed'),
     FieldPanel('explicit')]
    show_in_menus = True
    ...
    def podcastFeed(self, request: django.http.HttpRequest, **kwargs) -> django.http.HttpResponse:
        ...
        return render(request, 'wagtail_podcast/feed.xml', {'self':self, 
         'page':self,  'podcasts':podcasts},
    ...
    def archiveMonth(self, request: django.http.HttpRequest, **kwargs) -> django.http.HttpResponse:
        ...
        """
        return render(request, 'wagtail_podcast/podcast_parent_page.html', {'self':self, 
         'page':self,  'podcasts':self.get_pagination(request, kwargs),  'archives':self.get_archives()})
    ...
    def get_pagination(self, request: django.http.request.HttpRequest, kwargs) -> django.core.paginator.Paginator:
        ...
        if 'year' in kwargs:
            if 'month' in kwargs:
                podcasts = PodcastPage.objects.live().filter(first_published_at__year=(kwargs['year']), first_published_at__month=(kwargs['month'])).order_by('-first_published_at')
        else:
            if 'year' in kwargs:
                podcasts = PodcastPage.objects.live().filter(first_published_at__year=(kwargs['year'])).order_by('-first_published_at')
            else:
...
class PodcastPage(Page):
    ...
    episodeTypes = (
     ...
     (
      'bonus', _('Bonus')))
    episodeType = models.CharField(max_length=255, choices=episodeTypes, default='full', help_text=(_('Type of episode')), verbose_name=(_('Episode Type')))
    author = models.CharField(max_length=255, help_text=(_('Author of particular podcast')), verbose_name=(_('Author')))
    subtitle = models.CharField(max_length=255, help_text=(_('Subtitle of particular podcast')), verbose_name=(_('Subtitle')))
    description = models.TextField(help_text=(_('Description of podcast')), verbose_name=(_('Description')))
    recording = models.ForeignKey('wagtaildocs.Document',
      null=True,
      blank=True,
      on_delete=(models.SET_NULL),
      related_name='+',
      verbose_name=(_('Recording')))
    explicit = models.BooleanField(default=False, help_text=(_('Whether the podcast contains explicit content')), verbose_name=(_('Explicit')))
    guid = Page.pk
    duration = models.IntegerField(default=300)
    length = models.IntegerField(default=1024)
    page_text = RichTextField(null=True, help_text=(_('Content that accompanies the podcast')), verbose_name=(_('Page Text')))
    image = models.ForeignKey('wagtailimages.Image',
      null=True,
      blank=True,
      on_delete=(models.SET_NULL),
