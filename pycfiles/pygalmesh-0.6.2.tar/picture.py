...
# Embedded file name: build/bdist.linux-x86_64/egg/pyGallerid/utils/picture.py
# Compiled at: 2012-01-31 10:53:05
__doc__ = '\nProvides helpers for handling and importing image files.\n'
import os, re, dateutil.parser, subprocess, shutil, Image
from ExifTags import TAGS
from ..models.gallery import GalleryContainer, GalleryAlbum, GalleryPicture, GalleryImageFile
...
def create_gallery_image_file(filename, tags=None):
    ...
    image = GalleryImageFile(filename, size=(
     width, height), tags=tags)
    return image


def create_regular_gallery_image_file(filename, output_filename, width, height, tags, use_image_magick=True):
    new_size = get_scaled_size(width, height, MAX_REGULAR_VIEW_WIDTH, MAX_REGULAR_VIEW_HEIGHT)
    if use_image_magick:
        try:
...
def create_small_gallery_image_file(filename, output_filename, width, height, tags, use_image_magick=True):
    new_size = get_scaled_size(width, height, MAX_SMALL_VIEW_WIDTH, MAX_SMALL_VIEW_HEIGHT)
    if use_image_magick:
        try:
            subprocess.check_call(['convert', filename, '-resize',
             '%dx%d!' % new_size,
...
def import_gallery_picture(original_filename, big_filename, regular_filename, small_filename, default_date=None, move_file=True, use_image_magick=True):
    ...
    regular_image = create_regular_gallery_image_file(big_filename, regular_filename, width, height, use_image_magick)
    small_image = create_small_gallery_image_file(big_filename, small_filename, width, height, use_image_magick)
    dates = []
    ...
    description = name
    location = None
    picture = GalleryPicture(name, big_image, regular_image, small_image, description, location, date)
    return picture


def import_gallery_album(album_path, settings, move_files=True, use_image_magick=True, sorting_order='number'):
    rel_album_path = os.path.basename(album_path)
    datematcher = re.compile('^([\\d]{4}) ([a-zA-Z0-9]{1,9}) ([\\d]{1,2})(?:\\-([\\d]{1,2}))? (.+)$')
    try:
        mo = datematcher.match(rel_album_path)
        (year, month, day_from, day_to, album_name) = mo.groups()
        date_from = dateutil.parser.parse('%s %s %s' % (year, month, day_from)).date()
        date_to = dateutil.parser.parse('%s %s %s' % (year, month, day_to)).date()
    ...
    for filename in os.listdir('.'):
        ...
        if os.path.isfile(filename) and fileext.lower() in ('.jpg', '.jpeg', '.png',
                                                            '.tif', '.tiff'):
            ...
            try:
                picture = import_gallery_picture(filename, big_filename, regular_filename, small_filename, move_file=move_files)
            except:
                print 'Failed to import picture:',
                print os.path.join(album_path, filename)
                raise
            else:
    ...
    if len(pictures) == 0:
        ...
        album = GalleryAlbum(album_name, album_name, album_name, None, date_from, date_to, rel_album_path)
...
def import_gallery_container(path, settings, move_files=True, use_image_magick=True, sorting_order='number'):
    cwd = os.getcwd()
    ...
    for filename in os.listdir('.'):
        ...
        container = None
        (filebase, fileext) = os.path.splitext(filename)
        if not picture_found and os.path.isfile(filename) and fileext.lower() in ('.jpg',
                                                                                  '.jpeg',
                                                                                  '.png',
                                                                                  '.tif',
                                                                                  '.tiff'):
            try:
                album = import_gallery_album(path, settings, move_files, use_image_magick)
            except:
            ...
            try:
                container = import_gallery_container(abs_path, settings, move_files, use_image_magick, sorting_order)
            except:
