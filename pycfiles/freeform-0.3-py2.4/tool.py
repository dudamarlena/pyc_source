...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/freeform/tool.py
# Compiled at: 2006-03-28 20:10:08
...
"""
__all__ = [
 'compile_tool', 'compile_aggregate_resources', 'compile_aggregate_files', 'add_options_compile_tool']
import sys, optparse
try:
...
def add_options_compile_tool(parser):
    if pkg_resources:
        parser.add_option('-R', '--requirement', help='Specify the default pkg_resources requirement for the argument freeform resources. This option MUST be specified if -r is set AND ANY of your arguments are not of the form "requirement:resource".', default=None)
        parser.add_option('-r', '--resource-sources', action='store_true', help='Set to indicate the command line arguments are setuptools pkg_resources resources. Default [false]', default=False)
    parser.add_option('-d', '--input-data-file', default=None, help='Read input from file rather than stdin')
    parser.add_option('-I', '--interpreter-shell', action='store_true', default=False, help='a very basic shell')
...
def compile_tool():
        ...
        for n in names:
            ...
            if len(n) == 2:
                ...
                if not options.requirement:
                    print '[ERROR] using pkg_resources arguments requires you either specify -R defaultrequirement, OR you format each of the arguments as "requirement:resource".'
                    raise SystemExit
                requirement, resource = pkg_resources.Requirement(options.requirement), n[0]
    ...
    if options.verbose:
        pprint(ft)
    banner = '\nEnter input corresponding to any of the following forms:\n%s\n\ncompiled from sources:\n%s' % ((' \n').join([ '%s: %s' % describe_form(ft, formid) for formid in range(0, len(ft['form2command'])) ]), s)
...
def match_input(formtable, line):
    ...
    return ('\n').join(["<*** ambiguouse match ***> for:'%s'" % line] + [ '%s: ?= %s' % describe_form(formtable, formid) for formid in details ])
...
def describe_form(formtable, formid, **valuemap):
    ...
    matchdesc = (' ').join([ '%s.%s=%s' % (repr_fieldtype_compact(t), n, t == FIELDTYPE_KEYWORD and n or valuemap.get(n, '?')) for (t, n) in zip(ftypes, fnames) if n ])
    return (
     command, matchdesc)
...
def compile_aggregate_resources(resources, compiler=None):
    ...
    sources = [ pkg_resources.resource_string(*resource) for resource in [ (not isinstance(req, pkg_resources.Requirement) and pkg_resources.Requirement.parse(req) or req, res) for (req, res) in resources ] ]
