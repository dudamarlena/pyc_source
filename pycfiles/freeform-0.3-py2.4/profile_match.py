...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/freeform_tests/profile_match.py
# Compiled at: 2006-03-28 22:30:48
import operator
...
def profile_match_scentence(compiler, profilefile, sourcenames, sources, test_inputs, numruns=4, xmatch_scentence=match_scentence, xformtable_prepare=formtable_prepare):
    global PROFILE
    ...
    allmsgs.append('clock(match_scentence):\n\ttot=%s\n\tpercal=%s\n\t#calls=%s' % (minclocks, minclocks / numinputs, numinputs))
...
def format_results(formtable, results, verbose=True):
    ...
    for (input, expect, match, productions) in results:
        success, msg = True, 'ok'
        if productions:
            productions = [ (formtable['form2command'][formid],
             [formfieldnames[formid],
              map(_fieldtype_reprtab_compact.get, formfieldtypes[formid]), match])
             for formid in productions ]
            if False and verbose:
                msg = ('\n').join(['Words: "%s"' % (' ').join(input), 'Matched: "%s"' % (' ').join(map(str, match))])
            ...
            if len(productions) == 1 and productions[0][0] == expect:
                if expect_match:
                    if not reduce(operator.and_, map(operator.eq, expect_match, productions[0][1][2])):
                        success, msg = False, 'Failed, matched [%s] as expected, but the parameters were not recognised in the expected manner.\n\tgot %s\n\texp %s' % (expect, repr(productions[0][1][2]), repr(expect_match))
                if verbose and success:
                    msg = 'Success, %s' % ('%s <= %s' % (productions[0][0], (' ').join([ '%s:%s=%s' % (t, n, m) for (n, t, m) in zip(*productions[0][1]) ])))
            elif len(productions) == 1:
                success, msg = False, 'Matched [%s], expected [%s]' % (productions[0][0], repr(expect))
            elif isinstance(expect, str) or len(productions) != len(expect):
                success, msg = False, 'Unexpected ambiguous match.%s vs %s' % (repr(expect), repr(productions))
            else:
                matched_cmds = []
                for (i, ((cmd, _), ex)) in enumerate(zip(productions, expect)):
                    ...
                    if cmd != ex:
                        (success, msg) = (
                         False, 'Got an expected ambiguous match but for the wrong forms. %s not in %s. [%s]' % (cmd, (',').join(map(operator.getitem(0), expect))), str(i))
                    elif ex_match and not reduce(operator.and_, map(operator.eq, ex_match, productions[0][1][2])):
                        success, msg = False, 'Failed, ambiguous match [%s] vs [%s] as expected, but the parameters were not recognised in the expected manner. [%s]\n\tgot %s\n\texp %s' % (ex, (',').join(map(operator.getitem(0), expect)), str(i), repr(productions[0][1][2]), repr(ex_match))
                    else:
                        matched_cmds.append(cmd)

                if verbose and success:
                    msg = 'Success, got the expected ambiguous match: %s <= [%s]' % ((',').join(matched_cmds), (' ').join(input))
        elif expect:
            success, msg = False, 'Failed to produce expected match.\n\t%s <!= "%s", productions:%s' % (expect, (' ').join(input), repr(productions))
        elif verbose:
            msg = 'Success, bad scentence [%s] did not match anything.' % (' ').join(input)
