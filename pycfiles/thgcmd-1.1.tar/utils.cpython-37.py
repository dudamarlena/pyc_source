...
# [GCC 8.4.0]
# Embedded file name: /tools/tmp/cmd2/thgcmd/utils.py
# Compiled at: 2019-07-17 15:13:03
# Size of source mod 2**32: 22485 bytes
"""Shared utility functions"""
import collections, glob, os, re, shutil, subprocess, sys, threading, unicodedata
...
def find_editor() -> str:
    ...
    if not editor:
        if sys.platform[:3] == 'win':
            ...
            for editor in ('vim', 'vi', 'emacs', 'nano', 'pico', 'gedit', 'kate', 'subl',
                           'geany', 'atom'):
                if which(editor):
                    break
...
def files_from_glob_pattern(pattern: str, access=os.F_OK) -> List[str]:
    ...
    """
    return [f for f in glob.glob(pattern) if os.path.isfile(f) if os.access(f, access)]
...
class ContextFlag(object):
    __doc__ = 'A context manager which is also used as a boolean flag value within the default sigint handler.\n\n    Its main use is as a flag to prevent the SIGINT handler in thgcmd from raising a KeyboardInterrupt\n    while a critical code section has set the flag to True. Because signal handling is always done on the\n    main thread, this class is not thread-safe since there is no need.\n    '

    def __init__(self) -> None:
        self._ContextFlag__count = 0

    def __bool__(self) -> bool:
        return self._ContextFlag__count > 0

    def __enter__(self) -> None:
        self._ContextFlag__count += 1

    def __exit__(self, *args) -> None:
        self._ContextFlag__count -= 1
        if self._ContextFlag__count < 0:
            raise ValueError('count has gone below 0')


class RedirectionSavedState(object):
    __doc__ = 'Created by each command to store information about their redirection.'

    def __init__(self, self_stdout: Union[(StdSim, TextIO)], sys_stdout: Union[(StdSim, TextIO)], pipe_proc_reader: Optional[ProcReader]) -> None:
