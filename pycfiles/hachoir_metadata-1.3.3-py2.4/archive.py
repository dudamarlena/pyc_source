...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_metadata/archive.py
...
from hachoir_metadata.metadata import RootMetadata, Metadata, MultipleMetadata, registerExtractor
from hachoir_parser.archive import Bzip2Parser, CabFile, GzipParser, TarFile, ZipFile, MarFile
...
def computeCompressionRate(meta):
    ...
    """
    if not meta.has('file_size') or not meta.get('compr_size', 0):
        return
...
class TarMetadata(MultipleMetadata):
    ...
    def extractFile(self, field, meta):
        ...
        meta.author = '%s (uid=%s), group %s (gid=%s)' % (field['uname'].value, field.getOctal('uid'), field['gname'].value, field.getOctal('gid'))
...
class CabMetadata(MultipleMetadata):
    ...
    def extract(self, cab):
        ...
        self.comment = '%s folders, %s files' % (cab['nb_folder'].value, cab['nb_files'].value)
        max_nb = maxNbFile(self)
