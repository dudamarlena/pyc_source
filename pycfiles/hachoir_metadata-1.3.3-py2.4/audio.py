...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_metadata/audio.py
# Compiled at: 2010-07-23 16:44:08
from hachoir_metadata.metadata import registerExtractor, Metadata, RootMetadata, MultipleMetadata
...
def computeComprRate(meta, size):
    if not meta.has('duration') or not meta.has('sample_rate') or not meta.has('bits_per_sample') or not meta.has('nb_channel') or not size:
        return
...
def computeBitRate(meta):
    if not meta.has('bits_per_sample') or not meta.has('nb_channel') or not meta.has('sample_rate'):
        return
    meta.bit_rate = meta.get('bits_per_sample') * meta.get('nb_channel') * meta.get('sample_rate')


VORBIS_KEY_TO_ATTR = {'ARTIST': 'artist', 'ALBUM': 'album', 'TRACKNUMBER': 'track_number', 'TRACKTOTAL': 'track_total', 'ENCODER': 'producer', 'TITLE': 'title', 'LOCATION': 'location', 'DATE': 'creation_date', 'ORGANIZATION': 'organization', 'GENRE': 'music_genre', '': 'comment', 'COMPOSER': 'music_composer', 'DESCRIPTION': 'comment', 'COMMENT': 'comment', 'WWW': 'url', 'WOAF': 'url', 'LICENSE': 'copyright'}
...
class OggMetadata(MultipleMetadata):
    ...
    def theoraHeader(self, header, meta):
        ...
        meta.format_version = 'Theora version %u.%u (revision %u)' % (header['version_major'].value, header['version_minor'].value, header['version_revision'].value)
...
class RealAudioMetadata(RootMetadata):
    __module__ = __name__
    FOURCC_TO_BITRATE = {'28_8': 15200, '14_4': 8000, 'lpcJ': 8000}
...
class RealMediaMetadata(MultipleMetadata):
    __module__ = __name__
    KEY_TO_ATTR = {'generated by': 'producer', 'creation date': 'creation_date', 'modification date': 'last_modification', 'description': 'comment'}
...
class MpegAudioMetadata(RootMetadata):
    __module__ = __name__
    TAG_TO_KEY = {'TP1': 'author', 'COM': 'comment', 'TEN': 'producer', 'TRK': 'track_number', 'TAL': 'album', 'TT2': 'title', 'TYE': 'creation_date', 'TCO': 'music_genre', 'TPE1': 'author', 'COMM': 'comment', 'TENC': 'producer', 'TRCK': 'track_number', 'TALB': 'album', 'TIT2': 'title', 'TYER': 'creation_date', 'WXXX': 'url', 'TCON': 'music_genre', 'TLAN': 'language', 'TCOP': 'copyright', 'TDAT': 'creation_date', 'TRDA': 'creation_date', 'TORY': 'creation_date', 'TIT1': 'title'}
    ...
    def extract(self, mp3):
        if '/frames/frame[0]' in mp3:
            ...
            self.format_version = 'MPEG version %s layer %s' % (frame['version'].display, frame['layer'].display)
    ...
    def computeBitrate(self, frame):
        ...
        if not bit_rate:
            return
        self.bit_rate = (
         bit_rate, _('%s (constant)') % humanBitRate(bit_rate))
    ...
    def computeVariableBitrate(self, mp3):
        ...
        self.bit_rate = (bit_rate, _('%s (Variable bit rate)') % humanBitRate(bit_rate))
