...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_metadata/video.py
# Compiled at: 2010-07-23 17:07:10
...
class MkvMetadata(MultipleMetadata):
    __module__ = __name__
    tag_key = {'TITLE': 'title', 'URL': 'url', 'COPYRIGHT': 'copyright', 'DATE_RECORDED': 'creation_date', 'SUBTITLE': 'subtitle_author'}
    ...
    def trackCommon(self, track, meta):
        ...
        if 'Language/string' in track and track['Language/string'].value not in ('mis',
                                                                                 'und'):
    ...
    def processSimpleTag(self, tag):
        if 'TagName/unicode' not in tag or 'TagString/unicode' not in tag:
            return
    ...
    def processInfo(self, info):
        if 'TimecodeScale/unsigned' in info:
            ...
            if duration is not None:
                try:
                    seconds = duration * info['TimecodeScale/unsigned'].value * 1e-09
...
class AsfMetadata(MultipleMetadata):
    __module__ = __name__
    EXT_DESC_TO_ATTR = {'Encoder': 'producer', 'ToolName': 'producer', 'AlbumTitle': 'album', 'Track': 'track_number', 'TrackNumber': 'track_total', 'Year': 'creation_date', 'AlbumArtist': 'author'}
    SKIP_EXT_DESC = set(('WMFSDKNeeded', 'WMFSDKVersion', 'Buffer Average', 'VBR Peak',
                         'EncodingTime', 'MediaPrimaryClassID', 'UniqueFileIdentifier'))
