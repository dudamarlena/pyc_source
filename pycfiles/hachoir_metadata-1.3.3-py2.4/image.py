...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_metadata/image.py
# Compiled at: 2010-01-20 18:12:53
from hachoir_metadata.metadata import registerExtractor, Metadata, RootMetadata, MultipleMetadata
from hachoir_parser.image import BmpFile, IcoFile, PcxFile, GifFile, PngFile, TiffFile, XcfFile, TargaFile, WMF_File, PsdFile
...
def computeComprRate(meta, compr_size):
    ...
    """
    if not meta.has('width') or not meta.has('height') or not meta.has('bits_per_pixel'):
        return
    if not compr_size:
        return
...
class TiffMetadata(RootMetadata):
    __module__ = __name__
    key_to_attr = {'img_width': 'width', 'img_height': 'width'}
...
class XcfMetadata(RootMetadata):
    ...
    def processProperty(self, prop):
        ...
        if type == XcfProperty.PROP_COMPRESSION:
            self.compression = prop['data/compression'].display
        if type == XcfProperty.PROP_RESOLUTION:
...
class PngMetadata(RootMetadata):
    __module__ = __name__
    TEXT_TO_ATTR = {'software': 'producer'}
    ...
    def extract(self, wmf):
        if wmf.isAPM():
            ...
            if 'description' in emf:
                ...
                if '\x00' in desc:
                    (self.producer, self.title) = desc.split('\x00', 1)
                else:
                    self.producer = desc
