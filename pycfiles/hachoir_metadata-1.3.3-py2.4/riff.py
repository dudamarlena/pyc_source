...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/hachoir_metadata/riff.py
...
class RiffMetadata(MultipleMetadata):
    __module__ = __name__
    TAG_TO_KEY = {'INAM': 'title', 'IART': 'artist', 'ICMT': 'comment', 'ICOP': 'copyright', 'IENG': 'author', 'ISFT': 'producer', 'ICRD': 'creation_date', 'IDIT': 'creation_date'}
    ...
    def extractWAVE(self, wav):
        ...
        if 'nb_sample/nb_sample' in wav and 0 < format['sample_per_sec'].value:
        ...
        if format['codec'].value in UNCOMPRESSED_AUDIO:
            ...
            if not self.has('duration') and 'audio_data/size' in wav and self.has('bit_rate'):
    ...
    def extractAVIVideo(self, header, meta):
        meta.compression = '%s (fourcc:"%s")' % (header['fourcc'].display, makeUnicode(header['fourcc'].value))
    ...
    def extractAVIAudio(self, format, meta):
        ...
        if '../stream_hdr' in format:
            ...
            if header['fourcc'].value != '':
                meta.compression = '%s (fourcc:"%s")' % (format['codec'].display, header['fourcc'].value)
