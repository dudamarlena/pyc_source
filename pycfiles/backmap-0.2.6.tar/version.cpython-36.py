...
# [GCC 8.4.0]
# Embedded file name: /home/fatfrog/Documents/backmap/.eggs/setuptools_scm-2.0.0-py3.6.egg/setuptools_scm/version.py
...
class ScmVersion(object):
    ...
    def __repr__(self):
        return self.format_with('<ScmVersion {tag} d={distance} n={node} d={dirty} b={branch} x={extra}>')

    def format_with(self, fmt, **kw):
        return (fmt.format)(time=self.time, 
         tag=self.tag, 
         distance=self.distance, node=self.node, 
         dirty=self.dirty, extra=self.extra, branch=self.branch, **kw)
...
def simplified_semver_version(version):
    if version.exact:
        return guess_next_simple_semver((version.tag),
          retain=SEMVER_LEN, increment=False)
    else:
        if version.branch is not None:
            if 'feature' in version.branch:
                return version.format_next_version(guess_next_simple_semver,
                  retain=SEMVER_MINOR)
        return version.format_next_version(guess_next_simple_semver,
          retain=SEMVER_PATCH)
...
def _format_local_with_time(version, time_format):
    if version.exact or version.node is None:
        ...
        return version.format_choice('+{node}',
          '+{node}.d{time:{time_format}}', time_format=time_format)
