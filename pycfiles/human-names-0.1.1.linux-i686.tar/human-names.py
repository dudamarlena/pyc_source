...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/human-names/human-names.py
# Compiled at: 2011-06-04 18:12:18
import random, csv
...
class __Name__(object):
    ...
    def as_list(self):
        return [
         self.name, self.freq, self.cum_freq, self.rank]
...
class Names(object):
    ...
    def __load_csv__(self):
        ...
        for row in csvfile:
            ...
            self.last_names.append(__Name__(aname.title(), freq=float(afreq), cum_freq=float(acum_freq), rank=int(arank)))

        csvfile = csv.reader(open('data/male_names.csv', 'r'))
        for row in csvfile:
            aname, afreq, acum_freq, arank = row
            self.male_names.append(__Name__(aname.title(), freq=float(afreq), cum_freq=float(acum_freq), rank=int(arank)))

        csvfile = csv.reader(open('data/female_names.csv', 'r'))
        for row in csvfile:
            aname, afreq, acum_freq, arank = row
            self.female_names.append(__Name__(aname.title(), freq=float(afreq), cum_freq=float(acum_freq), rank=int(arank)))