...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/xio.py
# Compiled at: 2006-11-22 13:14:39
"""IO batteries

$Id: xio.py 390 2006-11-22 18:14:40Z djfroofy $
"""
...
class AggregateFile:
    ...
    def __seek_from_start(self, offset):
        ...
        fd.seek(0)
        fd.seek(voff)
        while offset > amts[self.focus_idx] and self.focus_idx < len(fds) - 1:
    ...
    def __seek_from_curr(self, offset):
        ...
        if offset < 0:
            ...
            voff = offset
            amts = self.seek_amounts
            while self.tell() + offset > amts[self.focus_idx] and self.focus_idx < len(fds) - 1:
    ...
    def read(self, count=-1):
        ...
        if count < 0:
            ...
            buffer.write(bytes)
            while ct + currpos > amts[self.focus_idx] and self.focus_idx < len(fds) - 1:
                self.focus_idx += 1
                ct -= len(bytes)
                bytes = fds[self.focus_idx].read(ct)
                fds[self.focus_idx].seek(0)
