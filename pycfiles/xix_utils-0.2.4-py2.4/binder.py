...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/binder.py
...
class BindingRequest:
    ...
    def __init__(self, name, creator, shared=False, bundlect=1, maxcount=0, initargs=None, initkwargs=None):
        """
...
class BindingFactory:
    ...
    def bind(self, bindingRequest):
        ...
        try:
            ...
            raise BindingException, 'No bindings could be created for %s :reason: %s' % (br.name, ie)

    def __prepbind(self, bindingRequest):
        br = bindingRequest
        if br.shared:
            instances = [ br.creator(*br.initargs, **br.initkwargs) for _ in range(br.bundlect) ]
            br.stack = instances
        else:
            if br.createdct == br.maxcount - 1:
                raise BindingOverflowException, 'Bindings exceeded max count: %d' % br.maxcount
...
def bootstrap(registry=None, request_list=None, context_registry=None):
    ...
    """
    global _binding_context_regsitry
