...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/config.py
# Compiled at: 2006-07-21 09:39:39
"""
...
class ConfigLoader:
    ...
    plugins = {'_chain': None, '_loader': None, 'path': _unixpath, '_': _replace}
        ...
        while line:
            ...
            for node in cfg_chain:
                if not _isvalidname(node):
                    raise ConfigLoaderException('Config node %s does not map to valid Python name' % node)
    ...
    def _updateConfig(self, cfg, cfg_chain, cfg_value=None):
        ...
        while chain:
            ...
            elif chain or cfg_value is None:
                child = Config()
...
def _plugin(strng):
    plugin_patt = re.compile('\\s*[_a-zA-Z]+\\s*\\(.*\\)\\s*$')
