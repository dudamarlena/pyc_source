...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/upgrade.py
# Compiled at: 2007-02-09 11:03:13
import sys, re
...
def _upgrade(vers, namespace, func_pfx='upgrade', sort_func=None, select_func=None):
    p = re.compile('%s(?P<current_vers>\\d+_\\d+(_\\d+)?)' % func_pfx)
...
def upgrade(vers, namespace, func_pfx='upgrade'):
    ...
    """
    _upgrade(vers, namespace, func_pfx=func_pfx, sort_func=lambda l: l.sort(), select_func=lambda cv, v: v >= cv)
...
def downgrade(vers, namespace, func_pfx='downgrade'):
    ...
    _upgrade(vers, namespace, func_pfx=func_pfx, sort_func=sorter, select_func=lambda cv, v: v >= cv)
