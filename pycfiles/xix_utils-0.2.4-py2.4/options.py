...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/tools/options.py
...
class OptionCollection(object):
    ...
    def append(self, option):
        ...
        if hasattr(self, option.dest):
            errmsg = "'%s' already exists as an attribute of '%s'instance" % (option.dest, self.__class__.__name__)
...
def toOptParser(options):
    ...
    for opt in options._data.values():
        ...
        for (name, value) in opt.__dict__.items():
            if name == 'value':
                continue
            if name == 'type' and hasattr(opt, 'action') and opt.action in ('store_true',
                                                                            'store_false'):
                continue
            if name == 'short_desc':
                args[0] = value
