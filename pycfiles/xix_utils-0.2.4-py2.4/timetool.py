...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/timetool.py
...
def setDayOfMonth(dtime, domon):
    ...
    if monmax < domon or domon < 1:
        raise ValueError, '%d does not fall in range for datetime %s' % (domon, dtime)
...
def parse_time(tstamp, *formats):
    ...
    if t is None:
        raise ValueError, 'could not parse timestamp ' + tstamp + ' with given formats: ' + (', ').join(formats)
    return t