...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/xtwisted/sshcmds.py
# Compiled at: 2007-07-30 11:23:50
...
class SSHTransport(transport.SSHClientTransport):
    ...
    def connectionSecure(self):
        self.requestService(SSHUserAuth(self.factory.user, Connection(self.factory.command, self.factory.datahandler), keys_file=self.factory.keys_file, host=self.factory.host))
...
class Connection(connection.SSHConnection):
    ...
    def serviceStarted(self):
        self.openChannel(Channel(self.command, self.datahandler, 2 ** 16, 2 ** 15, self))
...
def main():
    ...
    for a in sys.argv[2:]:
        ...
        reactor.connectTCP(host, 22, SSHTransportFactory(username, host, keys_file='~/.ssh/id_rsa', datahandler=gotdata(getattr(console, 'FG%s' % color.upper()), host), command=cmd))
