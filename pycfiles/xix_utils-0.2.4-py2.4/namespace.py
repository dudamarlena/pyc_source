...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xix/utils/namespace.py
# Compiled at: 2007-02-27 15:02:22
...
class VariablesNS(Namespace):
    ...
    def _isvalid(self, name, *pargs):
        return name[:1].islower() and name[-1:].isalnum() and _uq(lambda c: c.isalnum() or c == '_', name)
...
class ConstantsNS(Namespace):
    ...
    def _isvalid(self, name, *args):
        return name[:1].isupper() and name[-1:].isalnum() and _uq(lambda c: c.isupper() or c == '_' or c.isdigit(), name)
...
class ManyToManyNS(Namespace):
    """A many to many 1-1 mapped namespace.
    ...
    def __init__(self, **kwargs):
        raise NotImplementedError('ManyToManyNS is still an ethreal concept!')
