...
# [GCC 8.4.0]
# Embedded file name: /home/fw/development/AutoBuddy/aioouimeaux/aioouimeaux/device/api/xsd/device.py
# Compiled at: 2017-11-13 02:35:17
# Size of source mod 2**32: 72556 bytes
import sys, getopt, re as re_, base64
    ...
    class GeneratedsSuper(object):
        ...
        class _FixedOffsetTZ(tzinfo):
            ...
            def __init__(self, offset, name):
                self._FixedOffsetTZ__offset = timedelta(minutes=offset)
...
class root(GeneratedsSuper):
    ...
    def hasContent_(self):
        if self.specVersion is not None or self.URLBase is not None or self.device is not None:
            return True
        else:
            return False
...
class DeviceType(GeneratedsSuper):
    ...
    def hasContent_(self):
        if self.deviceType is not None or self.friendlyName is not None or self.manufacturer is not None or self.manufacturerURL is not None or self.modelDescription is not None or self.modelName is not None or self.modelNumber is not None or self.modelURL is not None or self.serialNumber is not None or self.UDN is not None or self.UPC is not None or self.iconList is not None or self.serviceList is not None or self.deviceList is not None or self.presentationURL is not None or self.anytypeobjs_:
            return True
        else:
            return False
...
class iconType(GeneratedsSuper):
    ...
    def hasContent_(self):
        if self.mimetype is not None or self.width is not None or self.height is not None or self.depth is not None or self.url is not None:
            return True
        else:
            return False
...
class serviceType(GeneratedsSuper):
    ...
    def hasContent_(self):
        if self.serviceType is not None or self.serviceId is not None or self.SCPDURL is not None or self.controlURL is not None or self.eventSubURL is not None:
            return True
        else:
            return False
...
GDSClassesMapping = {'serviceList':ServiceListType,  'service':serviceType, 
