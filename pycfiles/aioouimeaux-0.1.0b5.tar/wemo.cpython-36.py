...
# [GCC 8.4.0]
# Embedded file name: /home/fw/development/AutoBuddy/aioouimeaux/aioouimeaux/wemo.py
# Compiled at: 2018-11-10 22:14:04
# Size of source mod 2**32: 8141 bytes
import logging, socket, asyncio as aio
...
class WeMo(object):
    ...
    def start(self):
        ...
        if self._with_discovery:
            ...
            listen = loop.create_datagram_endpoint((partial(UPnP, loop, UPNP_ADDR, self._found_device, self.upnp)),
              sock=sock)
    ...
    def _found_device(self, sender, **kwargs):
        ...
        if usn.startswith('uuid:Socket'):
            if 'Switch' in self._list_of_types:
                klass = Switch
            ...
            if 'Motion' in self._list_of_types:
                klass = Motion
        elif usn.startswith('uuid:Bridge'):
            if 'Bridge' in self._list_of_types:
                klass = Bridge
        elif usn.startswith('uuid:Maker'):
            if 'maker' in self._list_of_types:
                klass = Maker
        else:
