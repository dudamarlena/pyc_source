...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ruamel/yaml/constructor.py
...
try:
    ...
    from ruamel.yaml.compat import utf8, builtins_module, to_str, PY2, PY3, ordereddict, text_type
...
class SafeConstructor(BaseConstructor):
    ...
    def construct_yaml_int(self, node):
        ...
        if value.startswith('0b'):
            ...
            if self.processing_version != (1, 2) and value[0] == '0':
                return sign * int(value, 8)
            if self.processing_version != (1, 2) and ':' in value:
...
class Constructor(SafeConstructor):
    ...
    def find_python_module(self, name, mark):
        ...
        try:
            ...
            raise ConstructorError('while constructing a Python module', mark, 'cannot find module %r (%s)' % (utf8(name), exc), mark)
    ...
    def find_python_name(self, name, mark):
        ...
        if not hasattr(module, object_name):
            raise ConstructorError('while constructing a Python object', mark, 'cannot find %r in the module %r' % (utf8(object_name),
    ...
    def construct_python_module(self, suffix, node):
        value = self.construct_scalar(node)
        if value:
            raise ConstructorError('while constructing a Python module', node.start_mark, 'expected the empty value, but found %r' % utf8(value), node.start_mark)
    ...
    def make_python_instance(self, suffix, node, args=None, kwds=None, newobj=False):
        ...
        if PY3:
            ...
            if newobj and isinstance(cls, type(self.classobj)) and not args and not kwds:
...
class RoundTripConstructor(SafeConstructor):
    ...
    def construct_scalar(self, node):
        ...
        if node.style == '|' and isinstance(node.value, text_type):
            ...
            if self._preserve_quotes and isinstance(node.value, text_type):
                if node.style == "'":
    ...
    def construct_mapping(self, node, maptyp, deep=False):
        ...
        for key_node, value_node in node.value:
            ...
            maptyp._yaml_set_kv_line_col(key, [key_node.start_mark.line, key_node.start_mark.column,
...
RoundTripConstructor.add_constructor('tag:yaml.org,2002:null', RoundTripConstructor.construct_yaml_null)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:bool', RoundTripConstructor.construct_yaml_bool)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:int', RoundTripConstructor.construct_yaml_int)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:float', RoundTripConstructor.construct_yaml_float)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:binary', RoundTripConstructor.construct_yaml_binary)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:timestamp', RoundTripConstructor.construct_yaml_timestamp)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:omap', RoundTripConstructor.construct_yaml_omap)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:pairs', RoundTripConstructor.construct_yaml_pairs)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:set', RoundTripConstructor.construct_yaml_set)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:str', RoundTripConstructor.construct_yaml_str)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:seq', RoundTripConstructor.construct_yaml_seq)
RoundTripConstructor.add_constructor('tag:yaml.org,2002:map', RoundTripConstructor.construct_yaml_map)
