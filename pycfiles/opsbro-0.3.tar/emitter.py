...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ruamel/yaml/emitter.py
...
class Emitter(object):
    ...
    def __init__(self, stream, canonical=None, indent=None, width=None, allow_unicode=None, line_break=None, block_seq_indent=None, top_level_colon_align=None, prefix_colon=None):
        self.stream = stream
        self.encoding = None
        self.states = []
    ...
    def expect_block_sequence_item(self, first=False):
        if not first and isinstance(self.event, SequenceEndEvent):
            ...
            if self.event.comment and self.event.comment[1]:
                self.write_pre_comment(self.event)
            self.write_indent()
            self.write_indicator(' ' * self.block_seq_indent + '-', True, indention=True)
    ...
    def check_simple_key(self):
        ...
        return length < self.MAX_SIMPLE_KEY_LENGTH and (isinstance(self.event, AliasEvent) or isinstance(self.event, ScalarEvent) and not self.analysis.empty and not self.analysis.multiline or self.check_empty_sequence() or self.check_empty_mapping())
    ...
    def choose_scalar_style(self):
        ...
        if self.event.style == '"' or self.canonical:
            return '"'
        if (not self.event.style or self.event.style == '?') and self.event.implicit[0] and not (self.simple_key_context and (self.analysis.empty or self.analysis.multiline)):
    ...
    def analyze_scalar(self, scalar):
        ...
        while index < len(scalar):
            ...
            if ch == ' ':
                ...
                previous_break = False
            index += 1
            preceeded_by_whitespace = ch in '\x00 \t\r\n\x85\u2028\u2029'
    ...
    def write_double_quoted(self, text, split=True):
        ...
        while end <= len(text):
            ...
            if end < len(text):
                ch = text[end]
            if ch is None or ch in '"\\\x85\u2028\u2029\ufeff' or not (' ' <= ch <= '~' or self.allow_unicode and '\xa0' <= ch <= '\ud7ff' or '\ue000' <= ch <= '�'):
                if start < end:
                    data = text[start:end]
                    self.column += len(data)
                    if self.encoding:
        ...
        if comments is None:
            ...
            try:
                for comment in comments:
                    if isinstance(event, MappingStartEvent) and getattr(comment, 'pre_done', None):
                        continue
                    if self.column != 0:
                        self.write_line_break()
