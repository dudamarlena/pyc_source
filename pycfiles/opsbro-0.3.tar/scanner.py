...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ruamel/yaml/scanner.py
# Compiled at: 2017-07-27 07:36:53
from __future__ import absolute_import
from __future__ import print_function
__all__ = [
 'Scanner', 'RoundTripScanner', 'ScannerError']
try:
...
class Scanner(object):
    ...
    def fetch_more_tokens(self):
        ...
        if comment is not None:
            ...
            raise ScannerError('while scanning for the next token', None, 'found character %r that cannot start any token' % utf8(ch), self.get_mark())
            return
    ...
    def scan_directive_name(self, start_mark):
        ...
        if not length:
            raise ScannerError('while scanning a directive', start_mark, 'expected alphabetic or numeric character, but found %r' % utf8(ch), self.get_mark())
    ...
    def scan_yaml_directive_value(self, start_mark):
        ...
        if self.peek() != '.':
            raise ScannerError('while scanning a directive', start_mark, "expected a digit or '.', but found %r" % utf8(self.peek()), self.get_mark())
        self.forward()
    ...
    def scan_yaml_directive_number(self, start_mark):
        ch = self.peek()
        if not '0' <= ch <= '9':
            raise ScannerError('while scanning a directive', start_mark, 'expected a digit, but found %r' % utf8(ch), self.get_mark())
        length = 0
    ...
    def scan_tag_directive_handle(self, start_mark):
        ...
        ch = self.peek()
        if ch != ' ':
            raise ScannerError('while scanning a directive', start_mark, "expected ' ', but found %r" % utf8(ch), self.get_mark())
        return value
    ...
    def scan_directive_ignored_line(self, start_mark):
        ...
        ch = self.peek()
        if ch not in '\x00\r\n\x85\u2028\u2029':
            raise ScannerError('while scanning a directive', start_mark, 'expected a comment or a line break, but found %r' % utf8(ch), self.get_mark())
    ...
    def scan_anchor(self, TokenClass):
        ...
        if not length:
            raise ScannerError('while scanning an %s' % name, start_mark, 'expected alphabetic or numeric character, but found %r' % utf8(ch), self.get_mark())
        value = self.prefix(length)
        self.forward(length)
        ch = self.peek()
        if ch not in '\x00 \t\r\n\x85\u2028\u2029?:,]}%@`':
    ...
    def scan_tag(self):
        ...
        if ch == '<':
            ...
            if self.peek() != '>':
                raise ScannerError('while parsing a tag', start_mark, "expected '>', but found %r" % utf8(self.peek()), self.get_mark())
            self.forward()
        elif ch in '\x00 \t\r\n\x85\u2028\u2029':
            handle = None
            suffix = '!'
        ...
        ch = self.peek()
        if ch not in '\x00 \r\n\x85\u2028\u2029':
            raise ScannerError('while scanning a tag', start_mark, "expected ' ', but found %r" % utf8(ch), self.get_mark())
        value = (
         handle, suffix)
        end_mark = self.get_mark()
    ...
    def scan_block_scalar(self, style):
        ...
        if len(comment) > 0:
            ...
            comment = CommentToken(('').join(comment), end_mark, comment_end_mark)
    ...
    def scan_block_scalar_indicators(self, start_mark):
        ...
        ch = self.peek()
        if ch not in '\x00 \r\n\x85\u2028\u2029':
            raise ScannerError('while scanning a block scalar', start_mark, 'expected chomping or indentation indicators, but found %r' % utf8(ch), self.get_mark())
        return (
         chomping, increment)
    ...
    def scan_block_scalar_ignored_line(self, start_mark):
        ...
        ch = self.peek()
        if ch not in '\x00\r\n\x85\u2028\u2029':
            raise ScannerError('while scanning a block scalar', start_mark, 'expected a comment or a line break, but found %r' % utf8(ch), self.get_mark())
        self.scan_line_break()
        ...
        while True:
            ...
            if not double and ch == "'" and self.peek(1) == "'":
                ...
                if ch in self.ESCAPE_REPLACEMENTS:
                    ...
                    raise ScannerError('while scanning a double-quoted scalar', start_mark, 'found unknown escape character %r' % utf8(ch), self.get_mark())
            else:
                return chunks
    ...
    def scan_plain(self):
        ...
        while True:
            ...
            while True:
                ch = self.peek(length)
                if ch in '\x00 \t\r\n\x85\u2028\u2029' or not self.flow_level and ch == ':' and self.peek(length + 1) in '\x00 \t\r\n\x85\u2028\u2029' or self.flow_level and ch in ',:?[]{}':
                    break
                length += 1

            if self.flow_level and ch == ':' and self.peek(length + 1) not in '\x00 \t\r\n\x85\u2028\u2029,[]{}':
                self.forward(length)
    ...
    def scan_tag_handle(self, name, start_mark):
        ch = self.peek()
        if ch != '!':
            raise ScannerError('while scanning a %s' % name, start_mark, "expected '!', but found %r" % utf8(ch), self.get_mark())
        length = 1
        ch = self.peek(length)
        if ch != ' ':
            while '0' <= ch <= '9' or 'A' <= ch <= 'Z' or 'a' <= ch <= 'z' or ch in '-_':
    ...
    def scan_tag_uri(self, name, start_mark):
        ...
        if length:
            ...
            length = 0
        if not chunks:
            raise ScannerError('while parsing a %s' % name, start_mark, 'expected URI, but found %r' % utf8(ch), self.get_mark())
        return ('').join(chunks)
    ...
    def scan_uri_escapes(self, name, start_mark):
        ...
        while self.peek() == '%':
            ...
            for k in range(2):
                if self.peek(k) not in '0123456789ABCDEFabcdef':
                    raise ScannerError('while scanning a %s' % name, start_mark, 'expected URI escape sequence of 2 hexdecimal numbers, but found %r' % utf8(self.peek(k)), self.get_mark())
...
class RoundTripScanner(Scanner):
    ...
    def get_token(self):
        ...
        self._gather_comments()
        if self.tokens:
            if len(self.tokens) > 1 and isinstance(self.tokens[0], (
             ScalarToken,
             ValueToken,
             FlowSequenceEndToken,
             FlowMappingEndToken)) and isinstance(self.tokens[1], CommentToken) and self.tokens[0].end_mark.line == self.tokens[1].start_mark.line:
                self.tokens_taken += 1
