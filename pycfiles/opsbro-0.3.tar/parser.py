...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/ruamel/yaml/parser.py
# Compiled at: 2017-07-27 07:36:53
from __future__ import absolute_import
__all__ = [
 'Parser', 'RoundTripParser', 'ParserError']
        ...
        while self.check_token(DirectiveToken):
            ...
            if token.name == 'YAML':
                ...
                if handle in self.tag_handles:
                    raise ParserError(None, None, 'duplicate tag handle %r' % utf8(handle), token.start_mark)
        ...
        if self.check_token(AliasToken):
            ...
            if tag is not None:
                ...
                if handle is not None:
                    if handle not in self.tag_handles:
                        raise ParserError('while parsing a node', start_mark, 'found undefined tag handle %r' % utf8(handle), tag_mark)
            ...
            if indentless_sequence and self.check_token(BlockEntryToken):
                ...
                if token.plain and tag is None or tag == '!':
                    ...
                    implicit = (
                     False, False)
                event = ScalarEvent(anchor, tag, implicit, token.value, start_mark, end_mark, style=token.style, comment=token.comment)
                self.state = self.states.pop()
                ...
                event = SequenceStartEvent(anchor, tag, implicit, start_mark, end_mark, flow_style=False, comment=comment)
                self.state = self.parse_block_sequence_first_entry
            elif block and self.check_token(BlockMappingStartToken):
                end_mark = self.peek_token().start_mark
                comment = self.peek_token().comment
                event = MappingStartEvent(anchor, tag, implicit, start_mark, end_mark, flow_style=False, comment=comment)
...
class RoundTripParser(Parser):
    ...
    def transform_tag(self, handle, suffix):
        if handle == '!!' and suffix in ('null', 'bool', 'int', 'float', 'binary',
                                         'timestamp', 'omap', 'pairs', 'set', 'str',
                                         'seq', 'map'):
