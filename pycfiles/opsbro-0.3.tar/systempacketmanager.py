...
# [GCC 8.4.0]
# Embedded file name: /root/opsbro-oss/opsbro/systempacketmanager.py
# Compiled at: 2017-08-24 10:31:10
import time, os, platform, subprocess, threading
...
class AptBackend(object):
    ...
    def has_package(self, package):
        ...
        if apt is None:
            ...
            if not self.deb_cache:
                ...
                elif self.deb_cache_update_time < time.time() - self.DEB_CACHE_MAX_AGE:
                    need_reload = True
                if need_reload:
    ...
    def install_package(package):
        ...
        p = subprocess.Popen(['apt-get', '-q', '--yes', '--no-install-recommends', 'install', '%s' % package], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        stdout, stderr = p.communicate()
...
class SystemPacketMgr(object):
    ...
    def get_distro(self):
        return (
         self.distro, self.distro_version, self.distro_id)
