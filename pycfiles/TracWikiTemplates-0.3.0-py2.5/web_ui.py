...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/WikiTemplates/web_ui.py
# Compiled at: 2008-04-21 11:18:32
...
try:
    ...
    IMPLEMENTS = [
     INavigationContributor, IPermissionRequestor, IRequestHandler,
...
class WikiTemplatesModule(Component):
    ...
    def get_navigation_items(self, req):
        ...
        if not req.perm.has_permission('TEMPLATES_VIEW') or not req.perm.has_permission('TEMPLATES_ADMIN') or not self.env.is_component_enabled('wikitemplates.macros.templates.templatesmacro'):
            return
        yield (
         'mainnav', 'templates',
         html.A('Wiki Templates', href=req.href.templates(), accesskey=9, id='templatesbtn'))

    def match_ctxtnav_add(self, req):
        if not req.perm.has_permission('TEMPLATES_VIEW') or not self.env.is_component_enabled('wikitemplates.macros.templates.templatesmacro'):
            return False
        self._add_js(req, self._make_js(req))
        return len(req.path_info) <= 1 or req.path_info == '/' or req.path_info.startswith('/wiki') or req.path_info.startswith('/templates')

    def get_ctxtnav_adds(self, req):
        yield (
         req.href.templates(), 'Wiki Templates Index')
    ...
    def match_request(self, req):
        match = re.match('^/templates(?:/(.*))?', req.path_info)
        if match:
            if match.group(1):
                req.args['page'] = match.group(1)
                self.env.log.debug("Page Match: '%s'", match.group(1))
                self.env.log.debug("Page Match req.args: '%s'", to_unicode(req.args['page']))
            else:
                req.args['page'] = 'TemplatesIndex'
                self.env.log.debug("Index Match: '%s'", to_unicode(req.args['page']))
            return True
    ...
    def process_request(self, req):
                ...
                if name != 'TemplatesIndex':
                    try:
                        for i in range(1, len(args_re.findall(contents)) + 1):
                            contents = contents.replace('{{%d}}' % i, 'argument_%d' % i)

                        self.env.log.debug('Replace Results: "%s"' % contents)
                    except:
                        self.env.log.debug('No variables replacing needed.')
                    else:
                        previews.append({'name': name, 'contents': wiki_to_html(contents, self.env, req)})
        ...
        if req.method == 'POST':
            if action == 'edit':
                latest_version = WikiTemplate(self.env, pagename, None, db).version
                ...
                req.redirect(req.href.templates(page.name, version=page.version, old_version=req.args.get('old_version'), action='diff'))
        elif action == 'delete':
            ...
            if format:
                Mimeview(self.env).send_converted(req, 'text/x-trac-wiki', page.text, format, page.name)
    ...
    def get_timeline_events(self, req, start, stop, filters):
        if 'templates' in filters:
            ...
            db = self.env.get_db_cnx()
            cursor = db.cursor()
            cursor.execute('SELECT time,name,comment,author,version FROM templates WHERE time>=%s AND time<=%s', (
             start, stop))
            for (t, name, comment, author, version) in cursor:
                title = Markup('<em>%s</em> wiki template edited by %s', template.format_page_name(name), author)
                diff_link = html.A('diff', href=href.templates(name, action='diff', version=version))
                if format == 'rss':
                ...
                yield (
                 'templates', href.templates(name), title, t, author, comment)

            def display(id):
                return Markup('ticket ', html.EM('#', id))

            att = AttachmentModule(self.env)
            for event in att.get_timeline_events(req, db, 'templates', format, start, stop, lambda id: html.EM(id)):
                yield event
    ...
    def _do_delete(self, req, db, page):
        ...
        if version:
            ...
            db.commit()
            page.exists or req.redirect(req.href.templates())
        else:
            req.redirect(req.href.templates(page.name))
        return
    ...
    def _do_save(self, req, db, page):
        ...
        page.save(get_reporter_id(req, 'author'), req.args.get('comment'), req.remote_addr)
        req.redirect(req.href.templates(page.name))
    ...
    def _render_confirm(self, req, db, page):
        ...
        if version is not None:
            ...
            req.hdf['templates'] = {'version': version, 
    ...
    def _render_diff(self, req, db, page):
        ...
        if old_version:
            ...
            if old_version == page.version:
                ...
                old_version, page = page.version, WikiTemplate(self.env, page.name, old_version)
        ...
        for (version, t, author, comment, ipnr) in latest_page.get_history():
            if version == new_version:
                ...
                info['comment'] = wiki_to_html(comment or '--', self.env, req, db)
                info['ipnr'] = ipnr or ''
        ...
        if prev_version:
            add_link(req, 'prev', req.href.templates(page.name, action='diff', version=prev_version), 'Version %d' % prev_version)
        if next_version:
            add_link(req, 'next', req.href.templates(page.name, action='diff', version=next_version), 'Version %d' % next_version)
        ...
        if context < 0:
            context = None
        changes = hdf_diff(oldtext, newtext, context=context, ignore_blank_lines='-B' in diff_options, ignore_case='-i' in diff_options, ignore_space_changes='-b' in diff_options)
    ...
    def _render_editor(self, req, db, page, preview=False):
        ...
        if page.exists:
            info['history_href'] = req.href.templates(page.name, action='history')
            info['last_change_href'] = req.href.templates(page.name, action='diff', version=page.version)
        if preview:
    ...
    def _render_history(self, req, db, page):
        ...
        for (version, t, author, comment, ipnr) in page.get_history():
            history.append({'url': req.href.templates(page.name, version=version), 
               'diff_url': req.href.templates(page.name, version=version, action='diff'), 
               'version': version, 
    ...
    def _render_view(self, req, db, page):
        ...
        if version:
            req.hdf['html.norobots'] = 1
        for conversion in Mimeview(self.env).get_supported_conversions('text/x-trac-wiki'):
            conversion_href = req.href.templates(page.name, version=version, format=conversion[0])
            add_link(req, 'alternate', conversion_href, conversion[1], conversion[3])

        latest_page = WikiTemplate(self.env, page.name)
        req.hdf['templates'] = {'exists': page.exists, 'version': page.version, 
           'latest_version': latest_page.version, 
           'readonly': page.readonly}
        if page.exists:
            req.hdf['templates'] = {'page_html': wiki_to_html(page.text, self.env, req), 'history_href': req.href.templates(page.name, action='history'), 
               'last_change_href': req.href.templates(page.name, action='diff', version=page.version)}
            if version:
                req.hdf['templates'] = {'comment_html': wiki_to_oneliner(page.comment or '--', self.env, db), 
                   'author': page.author, 
            ...
            req.hdf['templates.page_html'] = html.P('Describe "%s" template here' % page_name)
        req.hdf['templates.attachments'] = attachments_to_hdf(self.env, req, db, 'templates', page.name)
    ...
    def _add_js_inc(self, req, file):
        ...
        self._add_js(req, '<script type="text/javascript" src="%s"></script>' % file)
    ...
    def get_search_results(self, req, terms, filters):
        if 'templates' not in filters:
            return
        db = self.env.get_db_cnx()
        (sql_query, args) = search_to_sql(db, ['w1.name', 'w1.author', 'w1.text'], terms)
        cursor = db.cursor()
        cursor.execute('SELECT w1.name,w1.time,w1.author,w1.text FROM templates w1,(SELECT name,max(version) AS ver FROM templates GROUP BY name) w2 WHERE w1.version = w2.ver AND w1.name = w2.name AND ' + sql_query, args)
