...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/plantflow/wonka.py
# Compiled at: 2020-01-26 05:52:35
# Size of source mod 2**32: 29634 bytes
...
class WonkaSimulator:
    ...
    def create(self, products, days=365, transitions=False, transition_matrix_mu=None, transition_matrix_sigma=None, changeover=False, changeover_matrix_mu=None, changeover_matrix_sigma=None, mass_balance_percent_qualities=False, mass_balance_other_qualities=False, down_time_events=None):
        """
            ...
            if i > 0:
                ...
                if transitions:
                    if prev_var:
                        ...
                        transition_loss = int(np.random.normal(loc=mu_transition_loss, scale=(sigma_transition_loss * 0.05)))
                    ...
                    if changeover:
                        if prev_var:
                            ...
                            changeover_loss = int(np.random.normal(loc=mu_changeover_loss, scale=(sigma_changeover_loss * 0.05)))
                        ...
                        changeover_mins = datetime.timedelta(minutes=0)
                    prev_var = var
                    time_delta = template.loc[((template['ITEM'] == 'END TIME') & (template['Scheduled or Actual'] == 'S'))]['VALUE'][1] - template.loc[((template['ITEM'] == 'START TIME') & (template['Scheduled or Actual'] == 'S'))]['VALUE'][0]
                    kg = np.sum(template.loc[((template['Stream Type'] == 'FEED') & (template['Scheduled or Actual'] == 'S'))]['VALUE'])
                    kg_hr_rate = kg / time_delta.total_seconds()
                    ...
                    while 1:
                        campaign_multiplier = np.round(random.randint(1, 10) * random.random(), 3)
                        if campaign_multiplier > 0:
                            break

                    campaign = scheduled * campaign_multiplier
                    total_kg = np.sum(campaign)
                    seconds = int(total_kg / kg_hr_rate)
                    process_time = datetime.timedelta(seconds=seconds)
                    new_percents = self._percent_quality(template, sigma=0.05, mass_balance=mass_balance_percent_qualities)
                    new_others = self._other_quality(template, sigma=0.05, mass_balance=mass_balance_other_qualities)
                    new_yields = self._yield_quality(template, campaign=campaign, campaign_multiplier=campaign_multiplier, transition_loss=transition_loss)
                    if i == 0:
                        new_times = self._time_shift(template, process_time)
                    else:
                        new_times = self._time_shift(template, process_time, new_start_time)
                new_entry = pd.concat([new_times, new_yields, new_percents, new_others]).sort_index()
                if changeover:
                    new_start_time = new_times.loc[((new_times['ITEM'] == 'END TIME') & (new_times['Scheduled or Actual'] == 'A'))]['VALUE'].values[0] + changeover_mins
                else:
                    new_start_time = new_times.loc[((new_times['ITEM'] == 'END TIME') & (new_times['Scheduled or Actual'] == 'A'))]['VALUE'].values[0]
            if down_time_events is not None and event < down_time_events.index[(-1)]:
    ...
    def _percent_quality(self, template, sigma=0.05, mass_balance=False):
        ...
        for i in product_breakdown.index:
            new_value = np.random.normal(loc=(product_breakdown['VALUE'][i]), scale=(product_breakdown['VALUE'][i] * sigma))
    ...
    def _other_quality(self, template, sigma=0.05, mass_balance=False):
        ...
        for i in product_other.index:
            new_value = np.random.normal(loc=(product_other['VALUE'][i]), scale=(product_other['VALUE'][i] * sigma))
    ...
    def _yield_quality(self, template, sigma=0.05, campaign=None, campaign_multiplier=None, transition_loss=0):
        ...
        products = overall_yield.loc[(((overall_yield['Stream Type'] == 'PRODUCT') | (overall_yield['Stream Type'] == 'WASTE')) & ~overall_yield['Stream Type'].str.contains('OFFSPEC') & (overall_yield['Scheduled or Actual'] == 'A'))]
        offspec = overall_yield.loc[overall_yield['Stream Type'].str.contains('OFFSPEC')]
        scheduled = overall_yield.loc[((overall_yield['Stream Type'] == 'FEED') | (overall_yield['Stream Type'] == 'PRODUCT'))].loc[(overall_yield['Scheduled or Actual'] == 'S')]
        ...
        for index, sub_index in enumerate(feeds.index):
            if campaign is not None:
                new_value = np.random.normal(loc=(overall_yield['VALUE'][sub_index] * campaign_multiplier), scale=(overall_yield['VALUE'][sub_index] * campaign_multiplier * sigma))
                overall_yield['VALUE'][sub_index] = new_value
            else:
                new_value = np.random.normal(loc=(overall_yield['VALUE'][sub_index]), scale=(overall_yield['VALUE'][sub_index] * sigma))
                overall_yield['VALUE'][sub_index] = new_value
        ...
        for index, sub_index in enumerate(products.index):
            if sub_index < products.index[(-1)]:
                if campaign is not None:
                    ...
                    if overall_yield.loc[((overall_yield['ITEM'] == overall_yield['ITEM'][sub_index]) & (overall_yield['Scheduled or Actual'] == 'S'))].shape[0] != 0:
                        new_value = np.random.normal(loc=(overall_yield.loc[((overall_yield['ITEM'] == overall_yield['ITEM'][sub_index]) & (overall_yield['Scheduled or Actual'] == 'S'))]['VALUE']),
                          scale=(overall_yield.loc[((overall_yield['ITEM'] == overall_yield['ITEM'][sub_index]) & (overall_yield['Scheduled or Actual'] == 'S'))]['VALUE'] * sigma))[0]
                        overall_yield['VALUE'][sub_index] = new_value
                    else:
                        new_value = np.random.normal(loc=(overall_yield['VALUE'][sub_index]), scale=(overall_yield['VALUE'][sub_index] * sigma))
                        overall_yield['VALUE'][sub_index] = new_value
            else:
                new_value = np.sum(overall_yield.loc[((overall_yield['Stream Type'] == 'FEED') & (overall_yield['Scheduled or Actual'] == 'A'))]['VALUE']) - np.sum(overall_yield.loc[((overall_yield['Scheduled or Actual'] == 'A') & ~overall_yield.index.isin([sub_index]))].loc[(~(overall_yield['Stream Type'] == 'FEED') & ~overall_yield['Stream Type'].str.contains('OFFSPEC'))]['VALUE'])
                overall_yield['VALUE'][sub_index] = new_value

        for index, sub_index in enumerate(offspec.index):
            if campaign is not None:
                new_value = np.random.normal(loc=(overall_yield['VALUE'][sub_index] * campaign_multiplier), scale=(overall_yield['VALUE'][sub_index] * campaign_multiplier * sigma)) + transition_loss
            else:
                new_value = np.random.normal(loc=(overall_yield['VALUE'][sub_index]), scale=(overall_yield['VALUE'][sub_index] * sigma))
                overall_yield['VALUE'][sub_index] = new_value
            overall_yield['VALUE'][sub_index] = new_value
    ...
    def gen_plots(self, color_variable='AUTO', output_stream_type='PRODUCT', size=(15, 5)):
        ...
        time_filter_end = max(df.loc[(df['ITEM'] == 'END TIME')]['VALUE'])
        ...
        for item in distinguished:
            ...
            ids_prev = ids - 1
            dff = df.loc[df['ID'].isin(ids)]
            dff_prev = df.loc[df['ID'].isin(ids_prev)]
            temp = dff[((dff['ITEM'] == 'START TIME') & (dff['Scheduled or Actual'] == 'A'))]['VALUE'] < time_filter_end
            ids = dff.loc[list(temp.index[temp])]['ID']
            ids_prev = ids - 1
            dff = dff.loc[dff['ID'].isin(ids)]
            dff_prev = dff_prev.loc[dff_prev['ID'].isin(ids_prev)]
            temp = dff[((dff['ITEM'] == 'START TIME') & (dff['Scheduled or Actual'] == 'A'))]['VALUE'] > time_filter_start
            ids = dff.loc[list(temp.index[temp])]['ID']
            ids_prev = ids - 1
            dff = dff.loc[dff['ID'].isin(ids)]
            dff_prev = dff_prev.loc[dff_prev['ID'].isin(ids_prev)]
            ...
            if df.iloc[0]['ID'] == dff.iloc[0]['ID']:
                downtime = x1[1:].reset_index(drop=True) - x3
            else:
                downtime = x1 - x3
...
class DataPrep:
    ...
    def load_data(self, product='brownies'):
        ...
        file = self.file
        if product == 'brownies':
            brownie = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='BROWNIES')
            dc_brownie = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='DARK CHOCOLATE BROWNIES')
            almond_brownie = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='ALMOND BROWNIES')
            dc_almond_brownie = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='DARK CHOCOLATE ALMOND BROWNIES')
            brownies = [brownie, dc_brownie, almond_brownie, dc_almond_brownie]
            brownie_dic = {'Brownie':brownie,  'DC Brownie':dc_brownie, 
             'Almond Brownie':almond_brownie, 
             'DC Almond Brownie':dc_almond_brownie}
            changeover_matrix_mu = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Brownie Changeover Matrix Mu',
              index_col=0)
            changeover_matrix_sigma = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Brownie Changeover Matrix Sigma',
              index_col=0)
            transition_matrix_mu = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Brownie Transition Matrix Mu',
              index_col=0)
            transition_matrix_sigma = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Brownie Transition Matrix Sigma',
              index_col=0)
            ...
            if product == 'bars':
                mc_bar = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='MILK CHOCOLATE BAR')
                dc_bar = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='DARK CHOCOLATE BAR')
                mc_almond_bar = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='MILK CHOCOLATE ALMOND BAR')
                dc_almond_bar = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='DARK CHOCOLATE ALMOND BAR')
                bars = [mc_bar, dc_bar, mc_almond_bar, dc_almond_bar]
                bar_dic = {'MC Bar':mc_bar,  'DC Bar':dc_bar, 
                 'MC Almond Bar':mc_almond_bar, 
                 'DC Almond Bar':dc_almond_bar}
                changeover_matrix_mu = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Bars Changeover Matrix Mu',
                  index_col=0)
                changeover_matrix_sigma = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Bars Changeover Matrix Sigma',
                  index_col=0)
                transition_matrix_mu = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Bars Transition Matrix Mu',
                  index_col=0)
                transition_matrix_sigma = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='Bars Transition Matrix Sigma',
                  index_col=0)
                ...
                if product == 'liquor':
                    cote_divoire = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name="COTE D'IVOIRE")
                    ghana = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='GHANA')
                    indonesia = pd.read_excel((os.path.join(os.path.dirname(__file__), '../data/{}.xlsx'.format(file))), sheet_name='INDONESIA')
                    liquors = [cote_divoire, ghana, indonesia]
                    liquor_dic = {"Cote d'Ivoire":cote_divoire,  'Ghana':ghana, 
                     'Indonesia':indonesia}
