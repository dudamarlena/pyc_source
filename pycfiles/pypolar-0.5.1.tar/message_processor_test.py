...
# Embedded file name: build/bdist.macosx-10.12-intel/egg/tests/pypokergui/message_processor_test.py
...
class EngineWrapperTest(BaseUnitTest):
    ...
    def test_update_game_when_game_finished(self):
        ...
        for idx, player in enumerate(engine.current_state['table'].seats.players):
            player.hole_card = [
             Card.from_id(idx * 2), Card.from_id(idx * 2 + 1)]
...
blind_structure = {2: {'small_blind': 20, 'ante': 5}, 3: {'small_blind': 30, 'ante': 10}, 5: {'small_blind': 50, 'ante': 20}}
