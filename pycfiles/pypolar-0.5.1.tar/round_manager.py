...
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokerengine/engine/round_manager.py
...
class RoundManager:
    ...
    def __forward_street(self, state):
        ...
        if table.seats.count_ask_wait_players() <= 1:
            ...
            return (
             state, street_start_msg + messages)
        else:
            next_player_pos = state['next_player']
            next_player = table.seats.players[next_player_pos]
            ask_message = [(next_player.uuid, MessageBuilder.build_ask_message(next_player_pos, state))]
            return (
             state, street_start_msg + ask_message)

    @classmethod
    def __update_state_by_action(self, state, action, bet_amount):
        table = state['table']
        action, bet_amount = ActionChecker.correct_action(table.seats.players, state['next_player'], state['small_blind_amount'], action, bet_amount)
    ...
    def __update_message(self, state, action, bet_amount):
        return (
         -1,
         MessageBuilder.build_game_update_message(state['next_player'], action, bet_amount, state))
    ...
    def __deep_copy_state(self, state):
        ...
        return {'round_count': state['round_count'], 
