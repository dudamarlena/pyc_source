...
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokergui/engine_wrapper.py
...
class EngineWrapper(object):
    ...
    def update_game(self, action, bet_amount):
        ...
        if state['street'] == Const.Street.FINISHED:
            state, new_msgs = self._start_next_round(state['round_count'] + 1, self.config['blind_structure'], state['table'])
            msgs += new_msgs
        self.current_state = state
        return _parse_broadcast_destination(msgs, self.current_state['table'])
...
def gen_game_config(max_round, initial_stack, small_blind, ante, blind_structure=None):
    if not max_round > 0:
        ...
        assert ante >= 0
        blind_structure = blind_structure or {1: {'small_blind': small_blind, 'ante': ante}}
    if 1 not in blind_structure:
        blind_structure[1] = {'small_blind': small_blind, 'ante': ante}
    return {'max_round': max_round, 'initial_stack': initial_stack, 
...
def _get_forced_bet_amount(round_count, blind_structure):
    ...
    return (
     current_structure['small_blind'], current_structure['ante'])
...
def _steal_money_from_poor_player(table, ante, sb_amount):
    ...
    return (
     players.index(sb_player), players.index(bb_player))
...
def _gen_game_result_message(table, config):
    compat_config = {'initial_stack': config['initial_stack'], 
