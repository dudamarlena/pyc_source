...
# Embedded file name: /home/hobs/src/pug-ann/pug/ann/data/weather.py
# Compiled at: 2015-04-10 18:04:19
import os, urllib, datetime, json, warnings, pandas as pd
np = pd.np
...
def hourly(location='Fresno, CA', days=1, start=None, end=None, years=1, use_cache=True, verbosity=1):
    ...
    df = pd.DataFrame()
    for day in days:
        url = ('http://www.wunderground.com/history/airport/{airport_code}/{year}/{month}/{day}/DailyHistory.html?MR=1&format=1').format(airport_code=airport_code, year=day.year, month=day.month, day=day.day)
        if verbosity > 1:
        ...
        if verbosity > 0:
            ...
            print ('Retrieved CSV for airport code "{}" with appox. {} lines and {} columns = {} cells.').format(airport_code, N, int(round(M)), int(round(M)) * N)
        if buf.count('\n') > 2 or buf.count('\n') > 1 and buf.split('\n')[1].count(',') > 0:
            ...
            for rownum, row in enumerate(table):
                try:
                    table[rownum] = [
                     util.make_tz_aware(row[0], tz)] + row[1:]
                except ValueError:
                    pass
...
def api(feature='conditions', city='Portland', state='OR', key=None):
    ...
    url = ('http://api.wunderground.com/api/{key}/{feature}/q/{state}/{city}.json').format(key=key, feature=feature, state=state, city=city)
...
def daily(location='Fresno, CA', years=1, use_cache=True, verbosity=1):
    ...
    df = pd.DataFrame()
    for year in years:
        url = ('http://www.wunderground.com/history/airport/{airport}/{yearstart}/1/1/CustomHistory.html?dayend=31&monthend=12&yearend={yearend}&req_city=&req_state=&req_statename=&reqdb.zip=&reqdb.magic=&reqdb.wmo=&MR=1&format=1').format(airport=airport_code, yearstart=year, yearend=year)
        if verbosity > 1:
            print ('GETing *.CSV using "{0}"').format(url)
        buf = urllib.urlopen(url).read()
        if verbosity > 0:
