...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/bibolamazi_gui/favorites.py
...
default_cmds = [
 FavoriteCmd(name='fixes (standard; for LaTeX)', cmd='filter: fixes -dFixSwedishA -dEncodeUtf8ToLatex -dRemoveTypeFromPhd -sRemoveFullBraces=title -sProtectNames=Einstein,Maxwell,Landauer,Shannon,Neumann,Szilard,Bell,I,II,III,IV,XIV -dRemoveFileField'),
 FavoriteCmd(name='arxiv (for notes & verbose)', cmd='filter: arxiv -sMode=eprint -sUnpublishedMode=eprint -dThesesCountAsPublished -dWarnJournalRef'),
 FavoriteCmd(name='arxiv (for articles & finalizing)', cmd='filter: arxiv -sMode=strip -sUnpublishedMode=unpublished-note'),
 FavoriteCmd(name='url (standard cleanup)', cmd='filter: url -dStripAllIfDoiOrArxiv -dStripDoiUrl -dStripArxivUrl -dKeepFirstUrlOnly'),
 FavoriteCmd(name='orderentries (alphabetically by cite key)', cmd='filter: orderentries -sOrder=alphabetical')]
...
class FavoriteCmdsList(QObject):
    ...
    def loadFromSettings(self, settings):
        ...
        for i in xrange(siz):
            ...
            self.favlist.append(FavoriteCmd(name=str(settings.value('name').toString()), cmd=str(settings.value('cmd').toString())))
    ...
    def moveFavorite(self, oldind, newind):
        ...
        if oldind < newind:
            ...
            self.favlist[newind:(oldind + 1)] = [
             self.favlist[oldind]] + self.favlist[newind:oldind]
        self.favChanged.emit()
...
class FavoritesModel(QAbstractTableModel):
    ...
    def mimeData(self, indexes):
        ...
        if len(indexes) != 1:
            ...
            mimeData = QMimeData()
            mimeData.setData('application/x-bibolamazi-internalmove-favorites', QByteArray(str('%d' % indexes[0].row())))
            logger.longdebug('mimeData=%r', mimeData)
            return mimeData

    def dropMimeData(self, data, action, row, column, parent):
        logger.longdebug('Drop! data=%r (formats=%r), action=%r, row=%r, column=%r, parent=%r (%d,%d)', data, list(data.formats()), action, row, column, parent, parent.row(), parent.column())
...
class FavoritesOverBtns(OverListButtonWidgetBase):
    ...
    def on_btnDelete_clicked(self):
        curidx = self.curIndex()
        if QMessageBox.question(self, 'Confirm deletion', 'Are you sure you want to delete this favorite command?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No) != QMessageBox.Yes:
            return
