...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/mongophile/parser.py
# Compiled at: 2011-02-18 14:20:58
import pymongo, re, cli.log
try:
    import json
except:
...
class ProfilerParser(object):
    ...
    def parseEntry(self, entry):
        ...
        if info.startswith('query'):
            ...
            if info.startswith('update'):
                return self.updateParse(info, entry['ts'], entry['millis'])
            if info.startswith('remove'):
...
mongophile.add_param('-p', '--port', help='MongoDB port to read from', default=27017)
