...
# Embedded file name: build/bdist.macosx-10.6-intel/egg/byu_ws_sdk/core.py
# Compiled at: 2014-06-14 10:50:08
__doc__ = '\nThe code that generates the Authorization HTTP header.\n'
__author__ = 'paul_eden@byu.edu'
import os, xml.dom.minidom, simplejson, hashlib, hmac, base64, sys, time, requests
...
def get_http_authorization_header(apiKey, sharedSecret, keyType, encodingType, url='', requestBody=None, actor='', contentType=None, httpMethod=None, demo=False, actorInHash=False):
    """
...
def authorize_request(requestedUrl, authHeader, apiKey, sharedSecret, actor='', **kwargs):
    """
    ...
    """
    authUrl = 'https://ws.byu.edu/authentication/services/rest/v1/provider/URL-Encoded-API-Key/validate'
    if authHeader:
        ...
        auth = ('Nonce-Encoded-API-Key {0},{1},{2}').format(apiKey, nonce['nonceKey'], nonceDigest)
        if kwargs.get('headers'):
            kwargs['headers']['Authorization'] = auth
        else:
            kwargs['headers'] = {'Authorization': auth}
        response = requests.post(authUrl, data=data, **kwargs)
