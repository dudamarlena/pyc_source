...
# Embedded file name: /Users/paul/dev/byu-ws-sdk-python/build/lib/byu_ws_sdk/core.py
# Compiled at: 2013-07-30 13:55:39
# Size of source mod 2**32: 9654 bytes
__doc__ = '\nThe code that generates the Authorization HTTP header.\n'
import os, xml.dom.minidom
__author__ = 'paul_eden@byu.edu'
import simplejson, hashlib, hmac, base64, time, requests
...
def get_ws_session(casNetId, casPassword, timeout=1):
    ...
    response = requests.post('https://ws.byu.edu/authentication/services/rest/v1/ws/session', data=data, headers=headers, config=requests_config)
    response.raise_for_status()
    body = response.content
    if not body:
        raise Exception('The WsSession-granting web service did not provide a WsSession.  Perhaps the username and password supplied are not valid?')
...
def get_nonce(apiKey, actor=''):
    ...
    if actor:
        actor = '/' + actor
    response = requests.post('https://ws.byu.edu/authentication/services/rest/v1/hmac/nonce/%s%s' % (apiKey, actor), config=requests_config)
    body = response.content
    try:
...
def make_sha512_mac(sharedSecret, string, demo=False):
    ...
    return hmac.new(key=sharedSecret, msg=string, digestmod=hashlib.sha512).digest()


def url_encode(sharedSecret, current_timestamp, url, requestBody='', contentType=None, http_method=None, actor=None, demo=False, actorInHash=False):
    """
    ...
    if requestBody:
        if contentType == exception_ct:
            ...
            if demo:
                print('// There is something in the request body and the content-type of the request is %s' % exception_ct)
        else:
    ...
    mac = make_sha512_mac(sharedSecret, str(item_to_encode).encode('ascii'), demo)
...
def get_http_authorization_header(apiKey, sharedSecret, keyType, encodingType, url='', requestBody=None, actor=None, contentType=None, httpMethod=None, demo=False, actorInHash=False):
    """
    ...
    if encodingType == ENCODING_URL:
        base64encoded_hmac = url_encode(sharedSecret, current_timestamp, url, requestBody, contentType, httpMethod, actor, demo, actorInHash)
    else:
        ...
        if not len(base64encoded_hmac) == 88:
...
def send_ws_request(url, httpMethod, requestBody=None, headers=None):
    ...
    if not valid_http_method(httpMethod):
        raise Exception("The httpMethod passed in (%s) is not one of '%s'" % (httpMethod, "','".join(VALID_HTTP_METHODS)))
    response = getattr(requests, httpMethod.lower())(url, data=requestBody, headers=headers, config=requests_config, verify=False)
    return (
     response.content, response.status_code, response.headers, response)