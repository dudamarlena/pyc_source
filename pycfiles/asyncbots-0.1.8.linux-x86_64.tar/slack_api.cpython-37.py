...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.7/site-packages/asyncbots/slack_api.py
# Compiled at: 2018-08-17 13:08:24
# Size of source mod 2**32: 21209 bytes
...
DEFAULT_CONFIG = SlackConfig(token=None,
  name=None,
  admin_token=None,
  alert='!',
  load_history=False,
...
class Slack:
    ...
    async def connect(self):
        ...
        response = requests.get((BASE_URL + RTM_START),
          params=api_params)
        try:
            body = response.json()
    ...
    async def upload_file(self, f_name, channel, user):
        ...
        with open(f_name, 'rb') as (f):
            ...
            params = {'token':self._config.token,  'filetype':f_name.split('.')[-1], 
             'channels':channel, 
    ...
    async def delete_message(self, channel, user, timestamp, admin_key=True):
        ...
        params = {'token':token,  'ts':str(timestamp), 
         'channel':channel, 
         'as_user':True}
    ...
    def register_handler(self, func, data):
        ...
        if unfiltered:
            uhandler = UnfilteredHandler(name=name, func=func,
              channels=channels,
              doc=doc,
            ...
            handler = Handler(name=name, func=func,
              channels=channels,
              doc=doc,
              admin=admin,
              include_timestamp=include_ts)
    ...
    async def _handle_message(self, event):
        ...
        if is_dm or event['text'][0] == self._config.alert:
            ...
            if is_dm:
                command = parsed and name in self._handlers.filtered or (MessageCommand(channel=None, user=user, text=(self._help_message(user))) if is_dm else None)
            else:
                if parsed:
                    if is_dm or handler.channels is None or channel_name in handler.channels:
                        kwargs = {'timestamp': event['ts']} if handler.include_timestamp else {}
                        if not handler.admin or user in self.admins:
                            command = await (handler.func)(user=user, in_channel=channel_name, 
                             parsed=parsed[name], **kwargs)
                        else:
                            command = MessageCommand(channel=channel_name,
                              user=user,
                              text='That command is admin only.')
                else:
                    command = None
        ...
        if not is_dm:
            if parsed is None:
                for handler in self._handlers.unfiltered:
                    if not handler.channels is None:
                        if channel_name in handler.channels:
                            pass
                        kwargs = {'timestamp': event['ts']} if handler.include_timestamp else {}
                        command = await (handler.func)(user=user, 
                         in_channel=channel_name, 
                         message=event['text'], **kwargs)
                        await self._exhaust_command(command, event)

                if self._config.db:
                    await self.store_message(user=user,
                      channel=channel,
                      text=(event['text']),
                      timestamp=(event['ts']))
    ...
    async def _get_history(self, include_dms=False):
        ...
        for channel in channels:
            channel_name = self.ids.cname if channel[0] in ('C', 'G') else self.ids.dmname(channel)
            logger.info('Getting history for channel: %s', channel_name)
            url = BASE_URL + (CHANNEL_HISTORY if channel[0] == 'C' else GROUP_HISTORY if channel[0] == 'G' else IM_HISTORY)
            latest = float('inf')
            has_more = True
            params = {'token':self._config.token,  'channel':channel, 
             'inclusive':False}
            seen_timestamps = set()
            while has_more:
    ...
    async def _load_history(self):
        ...
        for channel, messages in past_events.items():
            for message in messages:
                try:
                    await self.store_message(channel=channel,
                      user=(message['user']),
                      text=(message['text']),
        ...
        for channel, messages in past_events.items():
            for message in messages:
                ...
                if message['user'] == bot_id:
                    ...
                    if channel[0] != 'D':
                        try:
                            self._parser.parse((message['text']),
                              dm=(channel[0] == 'D'))
                        except ParseException:
                            continue
    ...
    def _make_message(self, text, channel_id, response_callback):
        ...
        if response_callback:
            self._response_callbacks[m_id] = (
             response_callback, channel_id)
        return json.dumps({'id':m_id,  'type':'message', 
         'channel':channel_id, 
         'text':text})
    ...
    def _help_message(self, uid):
        ...
        for handler in self._handlers.filtered.values():
            if handler.doc:
                if not handler.admin or uid in self.admins:
