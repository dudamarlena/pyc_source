...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.8.0-Power_Macintosh/egg/turboentity.py
# Compiled at: 2006-10-28 17:57:38
"""
    turboentity.py
...
"""
import sys, re, sqlalchemy as sa
...
class EntityMeta(type):
    ...
    def __init__(cls, classname, classbases, classdict):
        ...
        te = cls._te = TurboEntityDescriptor(cls, _metadata, getattr(cls, 'turboentity', None))
        ...
        te.tablename = getattr(te.settings, 'tablename', _create_tablename(cls, getattr(te.settings, 'use_shortnames', None)))
        ...
        if not te.primary_key:
            if te.parent is None:
                ...
                te.primary_key = sa.Column(keyname, _DEFAULT_PRIMARY_KEY_TYPE, primary_key=True)
            else:
                parent = te.parent._te
                te.primary_key = sa.Column(parent.primary_key.key, parent.primary_key.type, ForeignKey(parent.primary_key), primary_key=True)
            columns[te.primary_key.key] = te.primary_key
...
def _setup_deferred_classes():
    ...
    for entity in list(EntityMeta.deferred_classes):
        ...
        if te.parent:
            ...
            assign_mapper(session.context, entity, entity.table, properties=relations, inherits=getattr(parent, 'mapper', None), inherit_condition=inherit_condition, order_by=order_by, polymorphic_identity=te.path)
        else:
            te.relations = relations
            if te.deferred_polymorphic_entities:
                te.polymorphic_dict = {te.path: entity.table.select(te.polymorphic_column == te.path)}
                for subentity in te.deferred_polymorphic_entities:
                    onclause = entity._te.primary_key == subentity._te.primary_key
                    te.polymorphic_dict[subentity._te.path] = entity.table.join(subentity.table, onclause=onclause)

                pjoin = sa.polymorphic_union(te.polymorphic_dict, None, 'pjoin')
                assign_mapper(session.context, entity, entity.table, polymorphic_on=te.polymorphic_column, polymorphic_identity=te.path, select_table=pjoin, order_by=order_by, properties=relations)
                subentity.mapper.inherits = entity.mapper
            else:
                assign_mapper(session.context, entity, entity.table, order_by=order_by, properties=relations)
...
def _add_polymorphic_identity(parent, subentity):
    ...
    """
    te = parent._te
    col = sa.Column(_find_name(_DEFAULT_POLYMORPHIC_COLUMN_NAME, te.properties.keys()), _DEFAULT_POLYMORPHIC_COLUMN_TYPE)
    ...
    if hasattr(parent, 'table'):
        if te.polymorphic_dict is None:
            ...
            te.polymorphic_dict = {te.path: parent.table.select(te.polymorphic_column == te.path)}
        ...
        te.polymorphic_dict[subentity._te.path] = parent.table.join(subentity.table, onclause=onclause)
        pjoin = sa.polymorphic_union(te.polymorphic_dict, None, 'pjoin')
...
def _setup_relations(te):
    ...
    for relation in te.relations.values():
        ...
        if isinstance(relation, ManyToMany):
            ...
            results[relation.colname] = sa.relation(relation.target_class, secondary=relation.secondary, backref=sa.backref(relation.backref, uselist=True, order_by=order_by_inv), order_by=order_by, uselist=True)
        else:
            ...
            if isinstance(relation, OneToOne):
                ...
                if relation.foreign_key:
                    ...
                    (foreign_key, primary_key) = _create_foreign_key(from_entity, to_entity, relation.colname)
                primaryjoin = foreign_key == primary_key
                results[relation.colname] = sa.relation(relation.target_class, primaryjoin=primaryjoin, foreignkey=primary_key, backref=sa.backref(relation.backref, uselist=False, primaryjoin=primaryjoin, foreignkey=foreign_key, order_by=order_by_inv), order_by=order_by, post_update=getattr(relation, 'post_update', True), uselist=False)
                ...
                if relation.foreign_key:
                    ...
                    (foreign_key, primary_key) = _create_foreign_key(from_entity, to_entity, relation.colname)
                primaryjoin = foreign_key == primary_key
                results[relation.colname] = sa.relation(relation.target_class, primaryjoin=primaryjoin, foreignkey=primary_key, backref=sa.backref(relation.backref, uselist=True, primaryjoin=primaryjoin, foreignkey=foreign_key, order_by=order_by_inv), order_by=order_by, post_update=getattr(relation, 'post_update', True), uselist=False)
            elif isinstance(relation, OneToMany):
                if relation.foreign_key:
                    foreign_key, primary_key = relation.foreign_key, relation.primary_key
                else:
                    (foreign_key, primary_key) = _create_foreign_key(to_entity, from_entity, relation.backref)
                primaryjoin = foreign_key == primary_key
                results[relation.colname] = sa.relation(relation.target_class, primaryjoin=primaryjoin, foreignkey=foreign_key, backref=sa.backref(relation.backref, uselist=False, primaryjoin=primaryjoin, foreignkey=primary_key, order_by=order_by_inv), order_by=order_by, post_update=getattr(relation, 'post_update', True), uselist=True)
        EntityMeta.deferred_relations.discard(relation)
        EntityMeta.deferred_relations.discard(inverse)
        relation.processed = True
        if inverse:
            inverse.processed = True
...
def _create_foreign_key(from_entity, to_entity, relation_name):
    ...
    colname = _find_name(_DEFAULT_FOREIGN_KEY_SCHEME % (relation_name, primary_key.key), from_entity._te.properties.keys())
...
def _create_secondary(relation, inverse):
    ...
    tablename = _DEFAULT_SECONDARY_TABLE_SCHEME % (entity1._te.tablename, column1, entity2._te.tablename, column2)
    colname1 = entity1._te.tablename + '_' + primary_key1.key
    colname2 = entity2._te.tablename + '_' + primary_key2.key
    relation.secondary = sa.Table(tablename, entity1._te.metadata, sa.Column(colname1, primary_key1.type, ForeignKey(primary_key1)), sa.Column(colname2, primary_key2.type, ForeignKey(primary_key2)))
    if inverse:
...
def _relations_match(rel1, rel2):
    ...
    assert isinstance(rel1, Relationship) and isinstance(rel2, Relationship)
    if rel1.target_class is not rel2.source_class or rel2.target_class is not rel1.source_class or not _relation_types_match(rel1, rel2) or rel1.backref and rel1.backref != rel2.colname or rel2.backref and rel2.backref != rel1.colname:
        return False
    else:
        return True
...
__all__ = [
 'Entity', 'Column', 'OneToOne', 'OneToMany', 'ManyToOne', 'ManyToMany', 'create_all', 'drop_all', 'ForeignKey'] + sa.types.__all__