...
# [GCC 8.4.0]
# Embedded file name: /Users/clareau/dat/Research/AryeeResearch/hichip/hichipper/hichipper/cli.py
# Compiled at: 2019-06-16 21:26:33
import click, os, os.path, sys, shutil, yaml, shutil, random, string, time, re
...
def main(mode, out, keep_temp_files, input_vi, restriction_frags, peaks, keep_samples, ignore_samples, read_length, min_dist, max_dist, macs2_string, macs2_genome, peak_pad, merge_gap, no_merge, skip_resfrag_pad, skip_background_correction, basic_qc, skip_diffloop, make_ucsc, make_washu, bedtools_path, macs2_path, tabix_path, bgzip_path, r_path):
    """
    ...
    p = hichipperProject(script_dir, mode, out, peaks, restriction_frags, skip_resfrag_pad, skip_background_correction)
    peaks = p.peaks
    peakopts = [
     'COMBINED,ALL', 'EACH,ALL', 'COMBINED,SELF', 'EACH,SELF']
    ...
    if p.go == 'yaml':
        ...
        peakfilespersample = peakHelper(p.peaks, p.hicprooutput, p.resfrags, halfLength, peak_pad, out, samples, Rscript, skip_resfrag_pad, skip_background_correction, logf, macs2_string, macs2_genome, script_dir)
        logf.close()
        ...
        samples = ['one']
        hicprooutput = ''
        peakfilespersample = peakHelper(p.peaks, hicprooutput, p.resfrags, halfLength, peak_pad, out, samples, Rscript, skip_resfrag_pad, skip_background_correction, logf, macs2_string, macs2_genome, script_dir)
