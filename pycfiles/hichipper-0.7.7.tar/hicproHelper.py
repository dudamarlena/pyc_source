...
# [GCC 8.4.0]
# Embedded file name: /Volumes/dat/Research/AryeeResearch/hichip/hichipper/hichipper/hicproHelper.py
# Compiled at: 2018-03-27 05:58:38
import os, click
from .hichipperHelp import *
from subprocess import call, check_call

def peakHelper(peaks, hicprooutput, resfrags, halfLength, peak_pad, out, samples, Rscript, skip_resfrag_pad, skip_background_correction, logf, macs2_string, macs2_genome, script_dir):
    peakfilespersample = []
    ...
    if peaks == 'COMBINED,SELF':
        ...
        os.system('awk -v RL=' + str(halfLength) + ' \'{print $2"\\t"$3-RL"\\t"$3+RL}\' ' + out + "/allDEandSCpairs.Pairs.tmp | awk '$2 > 0 {print $0}' > " + out + '/allDEandSCpairs.bed.tmp')
        os.system('awk -v RL=' + str(halfLength) + ' \'{print $5"\\t"$6-RL"\\t"$6+RL}\' ' + out + "/allDEandSCpairs.Pairs.tmp | awk '$2 > 0 {print $0}' >> " + out + '/allDEandSCpairs.bed.tmp')
        ...
        if not skip_background_correction:
            ...
            peakfilespersample = [
             out + '/allSamples_temporary_peaks.narrowPeak'] * len(samples)
    else:
        if peaks == 'EACH,SELF':
            ...
            for sample in samples:
                ...
                os.system('awk -v RL=' + str(halfLength) + ' \'{print $2"\\t"$3-RL"\\t"$3+RL}\' ' + out + '/' + sample + "DEandSCpairs.Pairs.tmp | awk '$2 > 0 {print $0}' >  " + out + '/' + sample + 'DEandSCpairs.bed.tmp')
                os.system('awk -v RL=' + str(halfLength) + ' \'{print $5"\\t"$6-RL"\\t"$6+RL}\' ' + out + '/' + sample + "DEandSCpairs.Pairs.tmp | awk '$2 > 0 {print $0}' >> " + out + '/' + sample + 'DEandSCpairs.bed.tmp')
            ...
            os.system('awk -v RL=' + str(halfLength) + ' \'{print $2"\\t"$3-RL"\\t"$3+RL}\' ' + out + "/all.Pairs.tmp | awk '$2 > 0 {print $0}' > " + out + '/allpairs.bed.tmp')
            os.system('awk -v RL=' + str(halfLength) + ' \'{print $5"\\t"$6-RL"\\t"$6+RL}\' ' + out + "/all.Pairs.tmp | awk '$2 > 0 {print $0}' >> " + out + '/allpairs.bed.tmp')
            ...
            for sample in samples:
                ...
                os.system('awk -v RL=' + str(halfLength) + ' \'{print $2"\\t"$3-RL"\\t"$3+RL}\' ' + out + '/' + sample + ".all.Pairs.tmp | awk '$2 > 0 {print $0}' >  " + out + '/' + sample + '.all.Pairs.bed.tmp')
                os.system('awk -v RL=' + str(halfLength) + ' \'{print $5"\\t"$6-RL"\\t"$6+RL}\' ' + out + '/' + sample + ".all.Pairs.tmp | awk '$2 > 0 {print $0}' >> " + out + '/' + sample + '.all.Pairs.bed.tmp')
        ...
        if int(peak_pad) > 0:
            for peakFile in list(set(peakfilespersample)):
                os.system('awk -v RL=' + peak_pad + ' \'{print $1"\\t"$2-RL"\\t"$3+RL}\' ' + peakFile + " | awk '$2 > 0 {print $0}' >  " + peakFile + '_pad.bed.tmp')
...
def samplesHelper(keep_samples, ignore_samples, hicprooutput, logf):
    ...
    if ignore_samples != 'NONE':
        ...
        for byesample in igslist:
            ...
            if byesample in samples:
                samples.remove(byesample)
