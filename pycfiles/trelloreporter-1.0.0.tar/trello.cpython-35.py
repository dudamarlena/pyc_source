...
# [GCC 8.4.0]
# Embedded file name: /home/philanim/PycharmProjects/trelloreporter/trelloreporter/rest/trello.py
# Compiled at: 2017-12-06 04:32:14
# Size of source mod 2**32: 10606 bytes
import asyncio, logging, uvloop, aiohttp
...
class TrelloRest(object):
    ...
    async def populate_boards(self):
        ...
        url_string = '{0}/organizations/{1}/boards?key={2}&token={3}'.format(self._api_url, self._organisation, self._api_key, self._api_token)
        try:
    ...
    async def populate_lists(self):
        ...
        for board in self._boards.keys():
            try:
                url_string = '{0}/boards/{1}/lists?key={2}&token={3}'.format(self._api_url, board, self._api_key, self._api_token)
                async with aiohttp.ClientSession() as session:
                    async with session.get(url_string) as response_string:
                        self._lists[board] = dict()
                        resp = await response_string.json()
                        for list in resp:
                            list_id = list['id']
    ...
    async def populate_cards(self):
        ...
        for board in self._boards.keys():
            try:
                url_string = '{0}/boards/{1}/cards/?key={2}&token={3}'.format(self._api_url, board, self._api_key, self._api_token)
                async with aiohttp.ClientSession() as session:
                    async with session.get(url_string) as response_string:
                        resp = await response_string.json()
    ...
    async def populate_members(self):
        ...
        url_string = '{0}/organization/{1}/members?key={2}&token={3}'.format(self._api_url, self._organisation, self._api_key, self._api_token)
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with response_string:
                    ...
                    self._logger.info('{0} members populated successfully from Trello'.format(len(resp)))
        except Exception:
