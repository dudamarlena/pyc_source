...
# [GCC 8.4.0]
# Embedded file name: /home/philanim/PycharmProjects/trelloreporter/trelloreporter/rest/elastic.py
...
class ElasticRest(object):
    ...
    def configure_elastic_parameters(self, username, password, endpoint, port, index):
        ...
        self._es_instance = Elasticsearch([endpoint], http_auth=(
         username, password), port=port)
        self._logger.info('Elasticsearch configuration endpoint:{0} port:{1} index:{2}'.format(endpoint, port, index))
    ...
    def write_to_elastic(self, document, type):
        ...
        try:
            self._logger.debug('Attempting to create {0} document: {1}'.format(type, document['Description']))
            res = self._es_instance.index(index=self._elastic_index, doc_type=type, body=document)
            if res['created'] is not 'True':
                self._logger.debug('{0} document written successfully'.format(document['Description']))
                return 0
            else:
                self._logger.debug('{0} document not written'.format(document['Description']))
                return 1
    ...
    def search_latest_document(self, type):
        ...
        try:
            ...
            result = self._es_instance.search(index=self._elastic_index, doc_type=type, sort='writeTime:desc', size=1)
            time_of_insert = datetime.strptime(result['hits']['hits'][0]['_source']['writeTime'], '%Y-%m-%dT%H:%M:%S.%f')
            self._logger.info('Latest time of insert for {0} is {1}'.format(type, time_of_insert))
        except Exception as ex:
            self._logger.error('Unable to obtain latest document in index {0}. ERROR :{1}'.format(self._elastic_index, ex))
