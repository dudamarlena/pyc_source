...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/clustering.py
...
from ..citations import WATTS1998, ONNELA2005, FAGIOLO2007, LANCICHINETTI2012, RUBINOV2010, HUMPHRIES2008, COSTANTINI2014, ZHANG2005
...
def clustering_coef_wu_sign(W, coef_type='default'):
    ...
    if coef_type == 'default':
        W_pos = W * (W > 0)
        K_pos = np.array(np.sum((np.logical_not(W_pos == 0)), axis=1), dtype=float)
        ...
        W_neg = -W * (W < 0)
        K_neg = np.array(np.sum((np.logical_not(W_neg == 0)), axis=1), dtype=float)
    ...
    if coef_type in ('zhang', 'Zhang'):
        ...
        return (
         C_pos, C_neg)
    if coef_type in ('constantini', 'Constantini'):
        cyc3 = np.zeros((n,))
        cyc2 = np.zeros((n,))
        for i in range(n):
            for j in range(n):
...
def get_components(A, no_depend=False):
    ...
    """
    if not np.all(A == A.T):
        raise BCTParamError('get_components can only be computed for undirected matrices.  If your matrix is noisy, correct it with np.around')
    A = binarize(A, copy=True)
    n = len(A)
    np.fill_diagonal(A, 1)
    edge_map = [{u, v} for u in range(n) if A[(u, v)] == 1 for v in range(n)]
    union_sets = []
    ...
    comps = np.array([i + 1 for v in range(n) if v in union_sets[i] for i in range(len(union_sets))])
