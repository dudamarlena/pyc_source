...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/utils/visualization.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 26472 bytes
...
def adjacency_plot_und(A, coor, tube=False):
    ...
    fig = mlab.figure()
    nodesource = mlab.pipeline.scalar_scatter((coor[:, 0]),
      (coor[:, 1]), (coor[:, 2]), figure=fig)
    nodes = mlab.pipeline.glyph(nodesource, scale_mode='none', scale_factor=3.0,
      mode='sphere',
      figure=fig)
    nodes.glyph.color_mode = 'color_by_scalar'
    vectorsrc = mlab.pipeline.vector_scatter((starts[:, 0]),
      (starts[:, 1]), (starts[:,
     2]),
      (vecs[:, 0]), (vecs[:, 1]), (vecs[:, 2]), figure=fig)
    vectorsrc.mlab_source.dataset.point_data.scalars = adjdat
    thres = mlab.pipeline.threshold(vectorsrc, low=0.0001,
      up=(np.max(A)),
      figure=fig)
    vectors = mlab.pipeline.vectors(thres, colormap='YlOrRd', scale_mode='vector',
      figure=fig)
    ...
    vectors.actor.property.opacity = 0.7
    if tube:
...
def align_matrices(m1, m2, dfun='sqrdiff', verbose=False, H=1000000.0, Texp=1, T0=0.001, Hbrk=10):
    """
    ...
    n = len(m1)
    if n < 2:
        raise BCTParamError('align_matrix will infinite loop on a singleton or null matrix.')
    elif dfun in ('absdiff', 'absdff'):
        maxcost = np.sum(np.abs(np.sort(m1.flat) - np.sort(m2.flat)[::-1]))
        lowcost = np.sum(np.abs(m1 - m2)) / maxcost
    else:
        if dfun in ('sqrdiff', 'sqrdff'):
            maxcost = np.sum((np.sort(m1.flat) - np.sort(m2.flat)[::-1]) ** 2)
            lowcost = np.sum((m1 - m2) ** 2) / maxcost
        else:
            if dfun == 'cosang':
                maxcost = np.pi / 2
                lowcost = np.arccos(np.dot(m1.flat, m2.flat) / np.sqrt(np.dot(m1.flat, m1.flat) * np.dot(m2.flat, m2.flat))) / maxcost
            else:
    ...
    while h < H:
        ...
        if dfun in ('absdiff', 'absdff'):
            ...
            if dfun in ('sqrdiff', 'sqrdff'):
                ...
                if dfun == 'cosang':
                    costnew = np.arccos(np.dot(m1.flat, m2atmp.flat) / np.sqrt(np.dot(m1.flat, m1.flat) * np.dot(m2.flat, m2.flat))) / maxcost
...
def backbone_wu(CIJ, avgdeg):
    ...
    if not np.all(CIJ == CIJ.T):
        raise BCTParamError('backbone_wu can only be computed for undirected matrices.  If your matrix is has noise, correct it with np.around')
    else:
...
def reorder_matrix(m1, cost='line', verbose=False, H=10000.0, Texp=10, T0=0.001, Hbrk=10):
    """
    ...
    if n < 2:
        ...
        if cost == 'circ':
            profile = stats.norm.pdf((range(1, n + 1)),
              loc=(n / 2), scale=(n / 4))[::-1]
        else:
...
def reorder_mod(A, ci):
    ...
    u, ci = np.unique(ci, return_inverse=True)
    n = np.size(ci)
    m = np.size(u)
    nm = np.zeros((m,))
    knm = np.zeros((n, m))
