...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/reference.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 60400 bytes
        ...
        while att <= max_attempts:
            ...
            if not (R[(a, d)] or R[(c, b)]):
                if D[(a, b)] * R[(a, b)] + D[(c, d)] * R[(c, d)] >= D[(a, d)] * R[(a, b)] + D[(c, b)] * R[(c, d)]:
                    if not (np.any((R[(a, c)], R[(d, b)], R[(d, c)])) and np.any((R[(c, a)], R[(b, d)], R[(b, a)]))):
                        P = R[(a, c), :].copy()
                        P[(0, b)] = 0
                        P[(0, d)] = 1
                        P[(1, d)] = 0
...
def makerandCIJdegreesfixed(inv, outv, seed=None):
    ...
    for i in range(k):
        if CIJ[(edges[(0, i)], edges[(1, i)])]:
            ...
            while True:
                if len(tried) == k:
                    raise BCTParamError('Could not resolve the given in and out vectors')
                switch = rng.randint(k)
                while switch in tried:
                    switch = rng.randint(k)

                if not CIJ[(edges[(0, i)], edges[(1, switch)])]:
                    if not CIJ[(edges[(0, switch)], edges[(1, i)])]:
...
def maketoeplitzCIJ(n, k, s, seed=None):
    ...
    while np.sum(CIJ) != k:
        ...
        itr += 1
        if itr > 10000:
            raise BCTParamError('Infinite loop was caught generating toeplitz matrix.  This means the matrix could not be resolved with the specified parameters.')

    return CIJ


@due.dcite((BibTeX(RUBINOV2011)), description='Directed signed null model')
def null_model_dir_sign(W, bin_swaps=5, wei_freq=0.1, seed=None):
    """
    ...
    for s in (1, -1):
        if s == 1:
            ...
            Acur = An
            A_rcur = An_r
        Si = np.sum((W * Acur), axis=0)
        So = np.sum((W * Acur), axis=1)
        Wv = np.sort(W[Acur].flat)
        i, j = np.where(A_rcur)
        Lij, = np.where(A_rcur.flat)
        P = np.outer(So, Si)
        if wei_freq == 0:
            Oind = np.argsort(P.flat[Lij])
            W0.flat[Lij[Oind]] = s * Wv
        else:
            wsize = np.size(Wv)
            wei_period = np.round(1 / wei_freq)
...
def randmio_dir_signed(R, itr, seed=None):
    ...
    for it in range(int(itr)):
        ...
        while att <= max_attempts:
            ...
            r0_ad = R[(a, d)]
            r0_cb = R[(c, b)]
            if np.sign(r0_ab) == np.sign(r0_cd) and np.sign(r0_ad) == np.sign(r0_cb):
                if np.sign(r0_ab) != np.sign(r0_ad):
                    R[(a, d)] = r0_ab
                    R[(a, b)] = r0_ad
                    R[(c, b)] = r0_cd
                    R[(c, d)] = r0_cb
...
def randomize_graph_partial_und(A, B, maxswap, seed=None):
    ...
    while nswap < maxswap:
        ...
        if rng.random_sample() > 0.5:
            ...
            c = i[e2]
            d = j[e2]
        A[(a, d)] = A[(a, d)] or A[(c, b)] or B[(a, d)] or B[(c, b)] or A[(a, b)]
        A[(a, b)] = 0
        A[(d, a)] = A[(b, a)]
        A[(b, a)] = 0
        A[(c, b)] = A[(c, d)]
...
def randomizer_bin_und(R, alpha, seed=None):
    ...
    if not np.allclose(R, R.T):
        ...
        if k > nr_poss_edges / 2:
            ...
            swap = False
    fullnodes = np.where(np.sum((np.triu(R, 1)), axis=0) + np.sum((np.triu(R, 1)), axis=1).T == ax - 1)
