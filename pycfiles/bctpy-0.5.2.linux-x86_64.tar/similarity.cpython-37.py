...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/similarity.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 11568 bytes
...
def edge_nei_overlap_bd(CIJ):
    ...
    for e in range(lel):
        neiik = np.setdiff1d(np.union1d(np.where(CIJ[ik[e], :]), np.where(CIJ[:, ik[e]])), (ik[e], jk[e]))
        neijk = np.setdiff1d(np.union1d(np.where(CIJ[jk[e], :]), np.where(CIJ[:, jk[e]])), (ik[e], jk[e]))
...
def dice_pairwise_und(a1, a2):
    ...
    a2 = binarize(a2, copy=True)
    n = len(a1)
    np.fill_diagonal(a1, 0)
    np.fill_diagonal(a2, 0)
    d = np.zeros((n,))
    for i in range(n):
        d[i] = 2 * (np.sum(np.logical_and(a1[:, i], a2[:, i])) / (np.sum(a1[:, i]) + np.sum(a2[:, i])))
