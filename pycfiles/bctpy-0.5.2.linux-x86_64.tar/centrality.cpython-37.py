...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/centrality.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 27712 bytes
...
from ..citations import BRANDES2001, KINTALI2008, SHANNON1948, RUBINOV2011, NEWMAN2016, HONEY2007, HAGMANN2008, GUIMERA2005, MORRISON2005, BOLDI2009, ESTRADA2005, ESTRADA2010
...
def erange(CIJ):
    ...
    eta = np.sum(Erange[np.logical_and(Erange > 0, Erange < np.inf)]) / len(Erange[np.logical_and(Erange > 0, Erange < np.inf)])
    Eshort = Erange > 2
    fs = len(np.where(Eshort)) / K
    return (
     Erange, eta, Eshort, fs)
...
def participation_coef_sparse(W, ci, degree='undirected'):
    ...
    ci += 1
    n = W.shape[0]
    Ko = np.array(W.sum(axis=1)).flatten().astype(float)
