...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/generative.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 20030 bytes
...
def generative_model(A, D, m, eta, gamma=None, model_type='matching', model_var='powerlaw', epsilon=1e-06, copy=True, seed=None):
    """
    ...
    if copy:
        ...
        if len(eta) != len(gamma):
            raise BCTParamError('Eta and gamma hyperparameters must be lists of the same size')
        else:
            nparams = len(eta)
            ...
            def clu_gen(A, K, D, m, eta, gamma, model_var, x_fun):
                ...
                if mv1 in ('powerlaw', 'power_law'):
                    ...
                    if mv1 in ('exponential', ):
                        Fd = np.exp(eta * D)
                    elif mv2 in ('powerlaw', 'power_law'):
                        Fk = K ** gamma
                    else:
                        if mv2 in ('exponential', ):
                            Fk = np.exp(gamma * K)
            ...
            def euclidean_gen(A, D, m, eta, model_var):
                ...
                elif mv1 in ('powerlaw', 'power_law'):
                    Fd = D ** eta
                else:
                    if mv1 in ('exponential', ):
                        Fd = np.exp(eta ** D)
...
def evaluate_generative_model(A, Atgt, D, eta, gamma=None, model_type='matching', model_var='powerlaw', epsilon=1e-06, seed=None):
    """
    ...
    B = generative_model(A, D, m, eta, gamma, model_type=model_type, model_var=model_var,
      epsilon=epsilon,
      copy=True,
      seed=seed)
    nB = len(eta)
