...
# [GCC 8.4.0]
# Embedded file name: /home/aestrivex/anaconda3/lib/python3.7/site-packages/bct/algorithms/core.py
# Compiled at: 2020-04-27 14:47:22
# Size of source mod 2**32: 21133 bytes
...
def assortativity_bin(CIJ, flag=0):
    ...
    term2 = np.square(np.sum(0.5 * (degi + degj)) / K)
    term3 = np.sum(0.5 * (degi * degi + degj * degj)) / K
...
def assortativity_wei(CIJ, flag=0):
    ...
    term2 = np.square(np.sum(0.5 * (stri + strj)) / K)
    term3 = np.sum(0.5 * (stri * stri + strj * strj)) / K
    r = (term1 - term2) / (term3 - term2)
    return r
...
def kcore_bd(CIJ, k, peel=False):
    ...
    if peel:
        return (
         CIJkcore, kn, peelorder, peellevel)
    return (CIJkcore, kn)
...
def local_assortativity_wu_sign(W):
    ...
    for curr_node in range(n):
        ...
        loc_assort_pos[curr_node] = np.sum(np.abs(str_pos[jp] - str_pos[curr_node])) / str_pos[curr_node]
        jn = np.where(W[curr_node, :] < 0)
        loc_assort_neg[curr_node] = np.sum(np.abs(str_neg[jn] - str_neg[curr_node])) / str_neg[curr_node]

    loc_assort_pos = (r_pos + 1) / n - loc_assort_pos / np.sum(loc_assort_pos)
    loc_assort_neg = (r_neg + 1) / n - loc_assort_neg / np.sum(loc_assort_neg)
    return (
     loc_assort_pos, loc_assort_neg)
...
def rich_club_wd(CIJ, klevel=None):
    ...
    for k in range(klevel):
        ...
        if np.size(SmallNodes) == 0:
            Rw[k] = np.nan
            continue
        cutCIJ = np.delete(np.delete(CIJ, SmallNodes, axis=0),
          SmallNodes, axis=1)
        Wr = np.sum(cutCIJ)
