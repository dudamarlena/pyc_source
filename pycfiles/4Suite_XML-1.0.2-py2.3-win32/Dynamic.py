...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\Exslt\Dynamic.py
...
NODE_HANDLERS = {XPathTypes.NodesetType: (None, None), XPathTypes.NumberType: ('exsl:number', Conversions.StringValue), XPathTypes.StringType: ('exsl:string', Conversions.StringValue), XPathTypes.BooleanType: ('exsl:boolean',
                          lambda x: x and 'true' or '')}
...
def Max(context, nodeset, string):
    ...
    return max([ Conversions.NumberValue(n) for n in MapImpl(context, nodeset, expr) ])
...
def Sum(context, nodeset, string):
    ...
    return sum([ Conversions.NumberValue(n) for n in MapImpl(context, nodeset, expr) ])


ExtNamespaces = {EXSL_DYNAMIC_NS: 'dyn'}
ExtFunctions = {(EXSL_DYNAMIC_NS, 'closure'): Closure, (EXSL_DYNAMIC_NS, 'evaluate'): Evaluate, (EXSL_DYNAMIC_NS, 'map'): Map, (EXSL_DYNAMIC_NS, 'max'): Max, (EXSL_DYNAMIC_NS, 'min'): Min, (EXSL_DYNAMIC_NS, 'sum'): Sum}
ExtElements = {}