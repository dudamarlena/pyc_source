...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\XPath\_4xpath.py
...
g_nssBindingPattern = re.compile('([\\d\\D_\\.\\-]*:?[\\d\\D_\\.\\-]+)=(.*)')
...
class XPathCommandLineApp(CommandLineApp.CommandLineApp):
    ...
    options = [
     ...
     Options.Option('N', 'namespace=PREFIX=NAMESPACE', 'Define a namespace/prefix binding'),
     Options.Option('e', 'stacktrace-on-error', 'Display a stack trace when an error occurs'),
     Options.Option(None, 'string', 'Print the string-value of the results')]
    arguments = [
     Arguments.RequiredArgument('source-uri', 'The URI of the XML document to parse, or "-" to indicate standard input. The document\'s root node will be used as the context node.'),
     Arguments.RequiredArgument('expression', 'The XPath expression to evaluate')]

    def validate_arguments(self, args):
        if len(args) < 2:
            raise SystemExit('A source URI argument and an expression argument are required. See "%s -h" for usage info.' % sys.argv[0])
...
def Run(options, args):
    ...
    try:
        ...
        context = XPath.Context.Context(dom, processorNss=processorNss, varBindings=top_level_params, extModuleList=extmodules)
