...
# [GCC 8.4.0]
# Embedded file name: \Ft\Lib\TestSuite\Tester.py
# Compiled at: 2004-11-04 00:33:46
"""
...
"""
import sys, os, time, traceback, linecache
...
class TestItem:
    ...
    def __repr__(self):
        return '<%s, title=%r, compares=%d>' % (self.__class__.__name__, self.title, self.comparisons)
...
class Tester:
    ...
    def __init__(self, stopOnError=1, useColor=1, verbose=VERBOSE_DEBUG, stream=sys.stdout):
    ...
    def testDone(self):
        ...
        if self.test.hasErrors:
            status = '[%sFAILED%s]' % (self.RED, self.NORMAL)
            level = VERBOSE_ERROR
        elif self.test.hasWarnings:
            status = '[%s WARN %s]' % (self.YELLOW, self.NORMAL)
            level = VERBOSE_WARN
        else:
            status = '[%s  OK  %s]' % (self.GREEN, self.NORMAL)
            level = SHOW_TESTS
        if not self.test.comparisons:
            title = '%s (%0.3f secs)' % (self.test.title, self.test.totalTime)
        else:
            title = '%s (%0.3f run, %0.3f total)' % (self.test.title, self.test.runTime, self.test.totalTime)
        spaces = self.tty.columns() - 9
        if len(title) > spaces:
    ...
    def testException(self, func, args, etype, value={}, stackLevel=1, kwargs={}):
        ...
        try:
            ...
            for (attr, expected) in value.items():
                if hasattr(e, attr):
                    self.compare(expected, getattr(e, attr), 'exception attribute %s' % repr(attr), stackLevel=stackLevel)

        except:
            self.exception('Wrong exception raised')
        else:
            self.error("Expected exception '%s' not raised" % etype, stackLevel=stackLevel)
    ...
    def compare(self, expected, actual, msg=None, func=cmp, diff=0, stackLevel=1, funcArgs={}):
        ...
        try:
            ...
            if res:
                ...
                error = '%sExpected:%s %s\n' % (self.GREEN, self.NORMAL, repr(expected))
                error += '%sCompared:%s %s' % (self.RED, self.NORMAL, repr(actual))
    ...
    def compareIn(self, expected, actual, msg=None, stackLevel=1):
        ...
        return self.compare(expected, actual, msg, func, stackLevel=stackLevel + 1)
    ...
    def _displayList(self, title, color, list):
        for (titles, message) in list:
            header = (': ').join(titles)
            self._writetty('\n%s%s: %s%s\n' % (color, title, header, self.NORMAL))
    ...
    def _diff(self, expected, compared):
        ...
        if self._diffCommand:
            ...
            fd.close()
            cmdline = '%s -u %s %s' % (self._diffCommand, expected_file, compared_file)
            self.debug(cmdline)
