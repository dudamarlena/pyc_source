...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\Processor.py
...
XSLT_IMT = [
 'application/xslt+xml', 'application/xslt', 'text/xml', 'application/xml']
...
class Processor(object):
    ...
    def __init__(self, stylesheetAltUris=None, documentReader=None, implementation=None, stylesheetIncPaths=None):
        ...
        if stylesheetIncPaths:
            self.warning("Deprecated 'stylesheetIncPaths' argument was used. Use 'stylesheetAltUris' instead.")
    ...
    def run(self, iSrc, ignorePis=0, topLevelParams=None, writer=None, outputStream=None):
        """
        ...
        return self.execute(src, iSrc, ignorePis, topLevelParams, writer, outputStream)

    def runNode(self, node, sourceUri=None, ignorePis=0, topLevelParams=None, writer=None, outputStream=None, preserveSrc=0, docInputSource=None):
        """
        ...
        if node.nodeType != Node.DOCUMENT_NODE:
            raise ValueError(MessageSource.g_errorMessages[Error.CANNOT_TRANSFORM_FRAGMENT])
        ...
        if not docInputSource:
            docInputSource = InputSource.InputSource(None, sourceUri, processIncludes=1, stripElements=self.getStripElements(), factory=self.inputSourceFactory)
        if not ignorePis and self.__checkStylesheetPis(node, docInputSource):
            self._stripElements(node)
        return self.execute(node, docInputSource, ignorePis=ignorePis, topLevelParams=topLevelParams, writer=writer, outputStream=outputStream)
        return
    ...
    def __checkStylesheetPis(self, node, inputSource):
        ...
        for child in root.childNodes:
            ...
            if child.nodeType == Node.PROCESSING_INSTRUCTION_NODE:
                if child.target == 'xml-stylesheet':
                    ...
                    if pseudo_attrs.has_key('href') and pseudo_attrs.has_key('type'):
                        href = pseudo_attrs['href']
                        imt = pseudo_attrs['type']
                        media = pseudo_attrs.get('media')
                        if media == self.mediaPref and imt in XSLT_IMT:
                            if pseudo_attrs.has_key('alternate') and pseudo_attrs['alternate'] == 'yes':
                                stys.append((1, media, imt, pseudo_attrs['href']))
                            elif found_nonalt:
                                c += 1
                                stys.append((c, media, imt, pseudo_attrs['href']))
                            else:
                                stys.append((0, media, imt, pseudo_attrs['href']))
                                found_nonalt = 1
        ...
        if hrefs:
            ...
            for href in hrefs:
                new_source = inputSource.resolve(href, hint='xml-stylesheet PI')
                self.appendStylesheet(new_source)

        return not not hrefs

    def execute(self, node, docInputSource, ignorePis=0, topLevelParams=None, writer=None, outputStream=None):
        """
        ...
        if not writer:
            ...
            writer = OutputHandler.OutputHandler(self.outputParams, outputStream)
        self.writers = [
         writer]
        ...
        context = XsltContext.XsltContext(node, 1, 1, None, varBindings=variables, processor=self, extFunctionMap=functions)
    ...
    def pushResultTree(self, baseUri, implementation=None):
        ...
        """
        writer = RtfWriter.RtfWriter(self.outputParams, baseUri, implementation or self._domimp)
    ...
    def pushResult(self, handler=None):
        ...
        """
        if handler is None:
            warnings.warn('Use pushResultTree(uri) to create RTFs', DeprecationWarning, stacklevel=2)
            handler = RtfWriter.RtfWriter(self.outputParams, self.stylesheet.baseUri)
        self.writers.append(handler)
        handler.startDocument()
        return
        return
    ...
    def output(self):
        warnings.warn('output() deprecated; use writer', DeprecationWarning, 2)
        return self.writer
        ...
        def to_unicode(s):
            try:
                ...
                try:
                    ...
                    raise ValueError('String parameters must be Unicode objects or strings encoded as UTF-8 or %s.' % DEFAULT_ENCODING)
