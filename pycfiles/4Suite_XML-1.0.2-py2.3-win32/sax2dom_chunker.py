...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\ThirdParty\Xvif\sax2dom_chunker.py
...
class _state_machine:
    ...
    def event(self, is_start, ns, local):
        ...
        if lookup_from.has_key((is_start, ns, local)):
            ...
            if self._state in self._live_states and new_state not in self._live_states:
...
class sax2dom_chunker(sax.ContentHandler):
    ...
    def __init__(self, trim_to_paths=None, chunk_consumer=None, domimpl=xml.dom.minidom.getDOMImplementation(), owner_doc=None):
        self._impl = domimpl
        if owner_doc:
            self._owner_doc = owner_doc
        else:
            dt = self._impl.createDocumentType(DUMMY_DOCELEM, None, '')
            self._owner_doc = self._impl.createDocument(DUMMY_DOCELEM, DUMMY_DOCELEM, dt)
    ...
    def endElementNS(self, name, qname):
        ...
        if not self.state_machine.is_live():
            if self._chunk_consumer and self.state_machine.chunk_completed:
    ...
    def processingInstruction(self, target, data):
        if self.state_machine.is_live():
            pi = self._owner_doc.createProcessingInstruction(target, data)
