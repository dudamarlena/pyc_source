...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\_4xslt.py
...
g_paramBindingPattern = re.compile('([\\d\\D_\\.\\-]*:?[\\d\\D_\\.\\-]+)=(.*)')
g_prefixBindingPattern = re.compile('([\\d\\D_\\.\\-]+)=(.+)')
...
class XsltCommandLineApp(CommandLineApp.CommandLineApp):
    ...
    options = [
     Options.Option('v', 'validate', 'Validate the input file as it is being parsed'),
     Options.Option('i', 'ignore', 'Ignore <?xml-stylesheet ...?> instructions'),
     Options.Option(None, 'media=MEDIA', 'Set media to honor in xml-stylesheet PIs'),
     Options.Option('D', 'define=NAME=VALUE', 'Bind a top-level parameter'),
     Options.Option('P', 'prefix=PREFIX=NSURI', 'Assign a namespace to a prefix used in a top-level parameter'),
     Options.Option('I', 'alt-sty-path=PATH', 'Same as --alt-sty-uri but uses OS path'),
     Options.Option(None, 'alt-sty-uri=URI', "Define an add'l base URI for imports and includes"),
     Options.Option('o', 'outfile=FILE', 'Direct transformation output to FILE (file will be overwritten if it exists)'),
     Options.Option('e', 'stacktrace-on-error', 'Display a stack trace when an error occurs'),
     Options.Option(None, 'noxinclude', 'Do not expand XIncludes in source document and stylesheet'),
     Options.Option(None, 'trace', 'Send execution trace output to stderr or file set by --trace-file'),
     Options.Option(None, 'trace-file=FILE', 'Trace file for execution trace output when using --trace'),
     Options.ExclusiveOptions([
      Options.Option(None, 'reflex', 'Reflexive transform (use the stylesheet as the source document).'),
      Options.Option(None, 'compile', 'Compile an instant stylesheet. The result is written to stdout, unless -o is used.'),
      Options.Option(None, 'instant', 'The stylesheet is "instant" (compiled). Only one stylesheet can be specified with this option.'),
      Options.Option(None, 'chain', 'Chain the stylesheets (result of transforming with the first is the source document for transforming with the second, and so on). Without this option, each extra stylesheet is imported by the preceding one.')]),
     Options.Option(None, 'time', 'Display the elapsed transformation time on stderr'),
     Options.Option(None, 'msg-prefix=STRING', 'Prepend string to xsl:message output'),
     Options.Option(None, 'msg-suffix=STRING', 'Append string to xsl:message output'),
     Options.Option(None, 'no-messages', 'Suppress xsl:message output and warnings')]
    arguments = [
     Arguments.RequiredArgument('source-uri', 'The URI of the XML document to transform, or "-" to indicate standard input. If using --reflex, it is also the stylesheet. If using --compile, it is the stylesheet to compile.'),
     Arguments.ZeroOrMoreArgument('stylesheet-uri', 'The URI(s) of the stylesheet(s) to apply.')]
    ...
    def validate_options(self, options):
        if options.has_key('trace'):
            ...
            if not outputfile and not tracefile:
                msg = 'When using --trace, you must specify an output file for the trace info (--trace-file) and/or for the transformation result (-o or --outfile).'
            else:
...
def ReportFatalException(e, stacktrace_on_error=False):
    ...
    """
    if isinstance(e, XsltException) or isinstance(e, RuntimeException) or isinstance(e, CompiletimeException):
        if stacktrace_on_error:
            traceback.print_exc(1000, sys.stderr)
        raise SystemExit(('').join([ c.encode(sys.getdefaultencoding(), 'ignore') or '&#%d;' % ord(c) for c in e.message ]))
    else:
        if stacktrace_on_error:
            traceback.print_exc(1000, sys.stderr)
            msg = ''
            ...
            msg = 'An unexpected error occurred while processing.\n' + 'The error was: %s: %s\n' % (exceptiontype, str(e)) + 'Use the -e (--stacktrace-on-error) option for a full stack trace.'
        if msg:
            raise SystemExit(('').join([ c.encode(sys.getdefaultencoding(), 'ignore') or '&#%d;' % ord(c) for c in msg ]))
        else:
            sys.exit()
...
def Run(options, args):
    ...
    if options.has_key('compile'):
        ...
        output_flags = 'w'
    out_file = options.has_key('outfile') and open(options['outfile'], output_flags) or sys.stdout
    trace_file = None
    if options.has_key('trace'):
        ...
        trace_file = options.has_key('trace-file') and open(options['trace-file'], 'w') or sys.stderr
    ...
    if os.environ.has_key('XSLTINCLUDE'):
        more_sty_uris = [ OsPathToUri(path, attemptAbsolute=1) for path in os.environ['XSLTINCLUDE'].split(os.pathsep) ]
        alt_sty_uris.extend(more_sty_uris)
    del more_sty_uris
    if options.has_key('trace'):
    ...
    if options.has_key('compile'):
        ...
        try:
            sty_isrcs = map(lambda arg: StySourceArgToInputSource(arg, DefaultFactory, processIncludes=xinclude, stylesheetAltUris=alt_sty_uris), all_source_args)
    ...
    if options.has_key('reflex'):
        ...
        try:
            isrc = StySourceArgToInputSource(source_arg, DefaultFactory, processIncludes=xinclude, stylesheetAltUris=alt_sty_uris)
        ...
        if instant:
            ...
            try:
                sty_isrc = StySourceArgToInputSource(sty_arg, DefaultFactory, processIncludes=xinclude, stylesheetAltUris=alt_sty_uris)
                ...
                source_isrc = SourceArgToInputSource(source_arg, DefaultFactory, processIncludes=xinclude)
            except cPickle.UnpicklingError:
                ReportFatalException(ValueError('%s does not appear to be a compiled stylesheet object.' % sty_arg), stacktrace_on_error)
            except Exception, e:
                ReportFatalException(e, stacktrace_on_error)
            ...
            try:
                sty_isrcs = map(lambda arg: StySourceArgToInputSource(arg, DefaultFactory, processIncludes=xinclude, stylesheetAltUris=alt_sty_uris), sty_args)
        ...
        try:
            ...
            if sty_chain:
                ...
                for sty_isrc in sty_chain[:-1]:
                    ...
                    CloseStream(sty_isrc, quiet=True)
                    processor.run(source_isrc, ignore_pis, topLevelParams=top_level_params, outputStream=resultstream)
                    processor.reset()
                    resultstream.reset()
                    sourcestream = resultstream
                    new_uri = Absolutize('chained-transformation-result-%d' % i, source_isrc.uri)
                    source_isrc = source_isrc.clone(sourcestream, new_uri)
                    resultstream = StringIO()

                processor.appendStylesheet(sty_chain[(-1)])
                processor.run(source_isrc, ignore_pis, topLevelParams=top_level_params, outputStream=out_file)
                if checktime:
                    end = time.time()
