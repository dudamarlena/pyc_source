...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/djangoflash/tests/codec.py
...
class BaseCodecTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.flash.update()
        self.expected = 'eyJfc2Vzc2lvbiI6IHsiaW5mbyI6ICJJbmZvIn0sICJfdXNlZCI6IHsiaW5mbyI6IG51bGx9fWZk\nNDViYTljMmU3MWJlZjBjYjcxOWEwYjdlYzJlZjUx\n'
    ...
    def test_decoded_tampered(self):
        ...
        """
        tampered = 'eyJfc2Vzc2lvbiI6IHsiaW6mbyI6ICJJbmZvIn0sICJfdXNlZCI6IHsiaW5mbyI6IG51bGx9fWZk\nNDViYTljMmU3MWJlZjBjYjcxOWEwYjdlYzJlZjUx\n'
...
class PickleCodecTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.flash['info'] = 'Info'
        self.flash.update()
        self.expected = b'\x80\x02cdjangoflash.models\nFlashScope\nq\x01)\x81q\x02}q\x03(U\x03nowq\x04cdjangoflash.models\n_ImmediateFlashScopeAdapter\nq\x05)\x81q\x06}q\x07U\x08delegateq\x08h\x02sbU\x08_sessionq\t}q\nU\x04infoq\x0bU\x04Infoq\x0csU\x05_usedq\r}q\x0eh\x0bNsub.'
...
class JSONCodecTestCase(TestCase):
    ...
    def setUp(self):
        ...
        """
        self.expected = '{"_session": {"info": "Info"}, "_used": {"info": null}}'
        self.codec = json_impl.CodecClass()
        self.flash = FlashScope()
        self.flash['info'] = 'Info'
        self.flash.update()
...
class JSONZlibCodecTestCase(TestCase):
    ...
    def setUp(self):
        r"""Creates a JSON\zlib-based codec and a sample flash.
        """
        self.expected = b'x\x9c\xabV\x8a/N-.\xce\xcc\xcfS\xb2R\xa8V\xca\xccK\xcb\x072\x94<At\xad\x8e\x82R|iqj\n\xb2T^iNNm-\x00\xf5\xa2\x12\x03'
    ...
    def test_encode(self):
        r"""Codec: JSON\zlib-based codec should return a zlib compressed JSON version of the flash.
        """
        self.assertEqual(self.expected, self.codec.encode(self.flash))

    def test_decode(self):
        r"""Codec: JSON\zlib-based codec should restore the flash from a zlib compressed JSON string.
        """
        flash = self.codec.decode(self.expected)
        self.assertEqual('Info', flash['info'])
        flash.update()
