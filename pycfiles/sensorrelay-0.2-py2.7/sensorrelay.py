...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/sensorrelay/sensorrelay.py
# Compiled at: 2016-08-25 16:57:54
import json
...
class WebsocketPublisher(object):
    ...
    def publish(self, channel, data):
        ...
        """
        self.ws.send(json.dumps({'type': 'data', 
           'channel': channel, 
           'data': data}))
    ...
    def rtt(self):
        ...
        """
        self.ws.send(json.dumps({'type': 'echo', 
           'cmd': 'rtt', 
           'timestamp': time()}))
...
class WebsocketSubscriber(object):
    ...
    def unsubscribe(self, channel):
        ...
        self.ws.send(json.dumps({'type': 'unsubscribe', 
           'channel': channel}))
        while True:
            response = json.loads(self.ws.recv())
