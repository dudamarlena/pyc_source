...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/generate/management/commands/json_loader.py
# Compiled at: 2012-09-18 09:34:13
import cStringIO, hashlib, json, mimetypes, os, random, sys, urllib
...
def load_file(field, source):
    ...
    file_name = '%s.%s' % (
     ('').join([ random.choice(elements) for n in range(8)
          ]), file_name.split('.')[(-1)])
...
def fetch_from_cache(source):
    ...
    if source.startswith('http'):
        destination = '%s/json_loader_cache/%s' % (SCRIPT_PATH,
