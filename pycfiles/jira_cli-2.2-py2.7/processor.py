...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/jiracli/processor.py
...
class ViewCommand(Command):
    ...
    def eval(self):
        if self.args.oneline:
            ...
            for issue in issues:
                print_output(self.jira.format_issue(issue, mode=mode, formatter=self.args.format, comments_only=self.args.comments_only))
...
class ListCommand(Command):
    ...
    def eval(self):
        mappers = {'issue_types': (
                         self.jira.get_issue_types,), 
           'subtask_types': (
                           self.jira.get_subtask_issue_types,), 
           'projects': (
                      self.jira.get_projects,), 
           'priorities': (
                        self.jira.get_priorities,), 
           'statuses': (
                      self.jira.get_statuses,), 
           'resolutions': (
                         self.jira.get_resolutions,), 
           'components': (
                        self.jira.get_components, 'project'), 
           'versions': (
                      self.jira.list_versions, 'project'), 
           'transitions': (
                         self.jira.get_available_transitions, 'issue'), 
           'filters': (
                     self.jira.get_filters,), 
...
class UpdateCommand(Command):
    ...
    def eval(self):
        ...
        if self.args.issue_comment:
            self.jira.add_comment(self.args.issue, self.args.issue_comment if isinstance(self.args.issue_comment, basestring) else get_text_from_editor())
            ...
            if not set(self.args.issue_components).issubset(current_components):
                ...
                print_output(colorfunc('component(s): %s added to %s' % (
        ...
        if self.args.affects_version:
            ...
            print_output(colorfunc('Added affected version(s) %s to %s' % ((',').join(self.args.affects_version), self.args.issue), 'green'))
        if self.args.remove_affects_version:
            self.jira.remove_versions(self.args.issue, self.args.remove_affects_version, 'affects')
            print_output(colorfunc('Removed affected version(s) %s from %s' % ((',').join(self.args.remove_affects_version), self.args.issue), 'blue'))
        if self.args.fix_version:
            self.jira.add_versions(self.args.issue, self.args.fix_version, 'fix')
            print_output(colorfunc('Added fixed version(s) %s to %s' % ((',').join(self.args.fix_version), self.args.issue), 'green'))
        if self.args.remove_fix_version:
            self.jira.remove_versions(self.args.issue, self.args.remove_fix_version, 'fix')
            print_output(colorfunc('Removed fixed version(s) %s from %s' % ((',').join(self.args.remove_fix_version), self.args.issue), 'blue'))
...
class AddCommand(Command):
    ...
    def eval(self):
        ...
        if self.args.issue_type and self.args.issue_type.lower() not in self.jira.get_issue_types().keys() + self.jira.get_subtask_issue_types().keys():
            raise UsageError('invalid issue type: %s (try using jira-cli list issue_types or jira-cli list subtask_types)' % self.args.issue_type)
        if self.args.issue_parent:
            ...
            if self.args.issue_type not in self.jira.get_subtask_issue_types():
                raise UsageError('issues created with parents must be one of {%s}' % (',').join(self.jira.get_subtask_issue_types()))
        components = {}
        if self.args.issue_components:
            ...
            if not set(self.args.issue_components).issubset(valid_components):
                raise UsageError('components for project %s should be one of {%s}' % (
        ...
        print_output(self.jira.format_issue(self.jira.create_issue(self.args.issue_project, self.args.issue_type, self.args.title, description, self.args.issue_priority, self.args.issue_parent, self.args.issue_assignee, self.args.issue_reporter, self.args.labels, components, **extras)))
        return