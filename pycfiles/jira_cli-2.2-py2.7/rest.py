...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/jiracli/bridge/rest.py
# Compiled at: 2017-07-27 23:00:52
"""
...
from jiracli.errors import JiraCliError, JiraAuthenticationError, JiraInitializationError
...
class JiraRestBridge(JiraBridge):
    ...
    def get_issues_by_filter(self, *filters):
        return self.search_issues_jql('filter in (%s)' % (',').join([ '"%s"' % f for f in filters ]))
    ...
    def create_issue(self, project, type='bug', summary='', description='', priority='minor', parent=None, assignee='', reporter='', labels=[], components={}, **extras):
        issue = {'project': {'key': project.upper()}, 'summary': summary, 
           'description': description, 
           'priority': {'id': self.get_priorities()[priority.lower()]['id']}, 'components': [ {'name': k} for k in components.keys() ]}
        if labels:
            issue['labels'] = labels
    ...
    def login(self, username, password):
        try:
            self.jira = JIRA(options={'server': self.base_url, 'check_update': False}, basic_auth=(
             username, password), get_server_info=False, validate=False)
        except JIRAError:
    ...
    def get_issue_comments(self, issue):
        return [ dict(author=comment.author.name, body=comment.body, created=comment.created) for comment in self.jira.comments(issue)
               ]
