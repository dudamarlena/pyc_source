...
def test_filterhits_load_no_filtering(filterhits_file):
    ...
    if not @py_assert1:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py3)s', ), (count, @py_assert2)) % {'py3': @pytest_ar._saferepr(@py_assert2), 'py0': @pytest_ar._saferepr(count) if 'count' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(count) else 'count'}
...
def test_filterhits_load_with_filtering(sources, exp_count, filterhits_file):
    ...
    if not @py_assert7:
        @py_format9 = @pytest_ar._call_reprcompare(('==',), (@py_assert7,), ('%(py6)s\n{%(py6)s = %(py0)s(%(py4)s\n{%(py4)s = %(py1)s(%(py2)s)\n})\n} == %(py8)s',), (@py_assert5, exp_count)) % {'py6': @pytest_ar._saferepr(@py_assert5), 'py1': @pytest_ar._saferepr(list) if 'list' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(list) else 'list', 'py4': @pytest_ar._saferepr(@py_assert3), 'py0': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len', 'py8': @pytest_ar._saferepr(exp_count) if 'exp_count' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(exp_count) else 'exp_count', 'py2': @pytest_ar._saferepr(entries) if 'entries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(entries) else 'entries'}
...
def test_filterhits_load_missing_columns(filterhits_file_missing_columns):
    ...
    if not @py_assert7:
        @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert7,), ('%(py6)s\n{%(py6)s = %(py0)s(%(py4)s\n{%(py4)s = %(py1)s(%(py2)s)\n})\n} == %(py9)s', ), (@py_assert5, @py_assert8)) % {'py6': @pytest_ar._saferepr(@py_assert5), 'py1': @pytest_ar._saferepr(list) if 'list' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(list) else 'list', 'py4': @pytest_ar._saferepr(@py_assert3), 'py0': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len', 'py9': @pytest_ar._saferepr(@py_assert8), 'py2': @pytest_ar._saferepr(entries) if 'entries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(entries) else 'entries'}
