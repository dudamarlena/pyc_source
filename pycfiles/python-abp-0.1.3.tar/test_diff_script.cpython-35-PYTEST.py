...
def test_diff_with_outfile(rootdir, archived_files, diff_dir):
    ...
    if not @py_assert8:
        @py_format11 = @pytest_ar._call_reprcompare(('==', ), (@py_assert8,), ('%(py7)s\n{%(py7)s = %(py0)s(%(py5)s\n{%(py5)s = %(py3)s\n{%(py3)s = %(py1)s.listdir\n}()\n})\n} == %(py10)s', ), (@py_assert6, @py_assert9)) % {'py1': @pytest_ar._saferepr(diff_dir) if 'diff_dir' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(diff_dir) else 'diff_dir', 'py10': @pytest_ar._saferepr(@py_assert9), 'py7': @pytest_ar._saferepr(@py_assert6), 'py0': @pytest_ar._saferepr(len) if 'len' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(len) else 'len', 'py3': @pytest_ar._saferepr(@py_assert2), 'py5': @pytest_ar._saferepr(@py_assert4)}
