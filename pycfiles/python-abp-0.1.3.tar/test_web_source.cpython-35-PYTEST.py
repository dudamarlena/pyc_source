...
def test_fetch_file(web_mock, http_source):
    ...
    if not @py_assert10:
        @py_format13 = @pytest_ar._call_reprcompare(('==', ), (@py_assert10,), ('%(py9)s\n{%(py9)s = %(py0)s(%(py7)s\n{%(py7)s = %(py3)s\n{%(py3)s = %(py1)s.get\n}(%(py5)s)\n})\n} == %(py12)s', ), (@py_assert8, @py_assert11)) % {'py1': @pytest_ar._saferepr(http_source) if 'http_source' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(http_source) else 'http_source', 'py7': @pytest_ar._saferepr(@py_assert6), 'py0': @pytest_ar._saferepr(list) if 'list' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(list) else 'list', 'py12': @pytest_ar._saferepr(@py_assert11), 'py9': @pytest_ar._saferepr(@py_assert8), 'py3': @pytest_ar._saferepr(@py_assert2), 'py5': @pytest_ar._saferepr(@py_assert4)}
