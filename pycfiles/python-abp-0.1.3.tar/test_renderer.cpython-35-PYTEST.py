...
def test_simple_render(head):
    ...
    if not @py_assert1:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == (%(py2)s + %(py4)s)', ), (got, @py_assert5)) % {'py2': @pytest_ar._saferepr(head) if 'head' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(head) else 'head', 'py4': @pytest_ar._saferepr(@py_assert3), 'py0': @pytest_ar._saferepr(got) if 'got' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(got) else 'got'}
...
def test_include(head):
    ...
    if not @py_assert7:
        @py_format9 = ('' + 'assert %(py8)s\n{%(py8)s = %(py2)s\n{%(py2)s = %(py0)s.startswith\n}((%(py3)s + %(py5)s))\n}') % {'py2': @pytest_ar._saferepr(@py_assert1), 'py3': @pytest_ar._saferepr(head) if 'head' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(head) else 'head', 'py5': @pytest_ar._saferepr(@py_assert4), 'py8': @pytest_ar._saferepr(@py_assert7), 'py0': @pytest_ar._saferepr(got) if 'got' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(got) else 'got'}
...
def test_include2(head):
    ...
    if not @py_assert4:
        @py_format6 = ('' + 'assert %(py5)s\n{%(py5)s = %(py2)s\n{%(py2)s = %(py0)s.startswith\n}(%(py3)s)\n}') % {'py2': @pytest_ar._saferepr(@py_assert1), 'py3': @pytest_ar._saferepr(expect) if 'expect' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expect) else 'expect', 'py5': @pytest_ar._saferepr(@py_assert4), 'py0': @pytest_ar._saferepr(got) if 'got' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(got) else 'got'}
...
def test_timestamp(gmtime):
    ...
    if not @py_assert10:
        @py_format12 = @pytest_ar._call_reprcompare(('in',), (@py_assert10,), ('%(py9)s\n{%(py9)s = %(py2)s\n{%(py2)s = %(py0)s.strftime\n}(%(py4)s, %(py7)s\n{%(py7)s = %(py5)s()\n})\n} in %(py11)s',), (@py_assert8, got)) % {'py7': @pytest_ar._saferepr(@py_assert6), 'py4': @pytest_ar._saferepr(@py_assert3), 'py5': @pytest_ar._saferepr(gmtime) if 'gmtime' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(gmtime) else 'gmtime', 'py0': @pytest_ar._saferepr(time) if 'time' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(time) else 'time', 'py2': @pytest_ar._saferepr(@py_assert1), 'py9': @pytest_ar._saferepr(@py_assert8), 'py11': @pytest_ar._saferepr(got) if 'got' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(got) else 'got'}
...
def test_deduplication():
    ...
    if not @py_assert2:
        @py_format4 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py3)s', ), (@py_assert0, got)) % {'py3': @pytest_ar._saferepr(got) if 'got' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(got) else 'got', 'py1': @pytest_ar._saferepr(@py_assert0)}
