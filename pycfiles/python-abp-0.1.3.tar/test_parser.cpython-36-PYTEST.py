...
def test_parse_empty():
    ...
    if not @py_assert3:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.type\n} == %(py5)s', ), (@py_assert1, @py_assert4)) % {'py0':@pytest_ar._saferepr(line) if 'line' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(line) else 'line',  'py2':@pytest_ar._saferepr(@py_assert1),  'py5':@pytest_ar._saferepr(@py_assert4)}
        @py_format8 = 'assert %(py7)s' % {'py7': @py_format6}
        raise AssertionError(@pytest_ar._format_explanation(@py_format8))
    @py_assert1 = @py_assert3 = @py_assert4 = None


@pytest.mark.parametrize('filter_text, expected', {'*asdf*d**dd*': {'selector':{'type':SelType.URL_PATTERN, 
                   'value':'*asdf*d**dd*'}, 
                  'action':FilterAction.BLOCK}, 
 
 '@@|*asd|f*d**dd*|': {'selector':{'type':SelType.URL_PATTERN, 
                        'value':'|*asd|f*d**dd*|'}, 
                       'action':FilterAction.ALLOW}, 
 
 '/ddd|f?a[s]d/': {'selector':{'type':SelType.URL_REGEXP, 
                    'value':'ddd|f?a[s]d'}, 
                   'action':FilterAction.BLOCK}, 
 ...
 'bla$match-case,~script,domain=foo.com|~bar.com,sitekey=foo': {'selector':{'type':SelType.URL_PATTERN, 
                                                                 'value':'bla'}, 
                                                                'action':FilterAction.BLOCK, 
                                                                'options':[
                                                                 (
 ...
 '@@http://bla$~script,~other,sitekey=foo|bar': {'selector':{'type':SelType.URL_PATTERN, 
                                                  'value':'http://bla'}, 
                                                 'action':FilterAction.ALLOW, 
                                                 'options':[
                                                  (
 ...
 "||foo.com^$csp=script-src 'self' * 'unsafe-inline',script,sitekey=foo,other,match-case,domain=foo.com": {'selector':{'type':SelType.URL_PATTERN, 
                                                                                                            'value':'||foo.com^'}, 
                                                                                                           'action':FilterAction.BLOCK, 
                                                                                                           'options':[
                                                                                                            (
 ...
 '@@bla$script,other,domain=foo.com|~bar.foo.com,csp=c s p': {'selector':{'type':SelType.URL_PATTERN, 
                                                               'value':'bla'}, 
                                                              'action':FilterAction.ALLOW, 
                                                              'options':[
                                                               ('script', True),
 ...
 '##ddd': {'selector':{'type':SelType.CSS, 
            'value':'ddd'}, 
           'action':FilterAction.HIDE, 
           'options':[]}, 
 
 '#@#body > div:first-child': {'selector':{'type':SelType.CSS, 
 ...
 'foo,~bar#?#:-abp-properties(abc)': {'selector':{'type':SelType.XCSS, 
                                       'value':':-abp-properties(abc)'}, 
                                      'action':FilterAction.HIDE, 
                                      'options':[
                                       (
                                        FilterOption.DOMAIN, [('foo', True), ('bar', False)])]}, 
 ...
 'foo,~bar#$#abort-on-property-write aaa; abort-on-property-read bbb': {'selector':{'type':SelType.SNIPPET, 
                                                                        ...
                                                                        'options':[
                                                                         (
                                                                          FilterOption.DOMAIN, [('foo', True), ('bar', False)])]}}.items())
def test_parse_filters(filter_text, expected):
    ...
    if not @py_assert3:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.type\n} == %(py5)s', ), (@py_assert1, @py_assert4)) % {'py0':@pytest_ar._saferepr(parsed) if 'parsed' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(parsed) else 'parsed',  'py2':@pytest_ar._saferepr(@py_assert1),  'py5':@pytest_ar._saferepr(@py_assert4)}
        @py_format8 = 'assert %(py7)s' % {'py7': @py_format6}
        raise AssertionError(@pytest_ar._format_explanation(@py_format8))
    @py_assert1 = @py_assert3 = @py_assert4 = None
    ...
    if not @py_assert3:
        @py_format5 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.text\n} == %(py4)s', ), (@py_assert1, filter_text)) % {'py0':@pytest_ar._saferepr(parsed) if 'parsed' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(parsed) else 'parsed',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(filter_text) if 'filter_text' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(filter_text) else 'filter_text'}
    ...
    for attribute, expected_value in expected.items():
        ...
        if not @py_assert5:
            @py_format7 = @pytest_ar._call_reprcompare(('==', ), (@py_assert5,), ('%(py4)s\n{%(py4)s = %(py0)s(%(py1)s, %(py2)s)\n} == %(py6)s', ), (@py_assert3, expected_value)) % {'py0':@pytest_ar._saferepr(getattr) if 'getattr' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(getattr) else 'getattr',  'py1':@pytest_ar._saferepr(parsed) if 'parsed' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(parsed) else 'parsed',  'py2':@pytest_ar._saferepr(attribute) if 'attribute' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(attribute) else 'attribute',  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(expected_value) if 'expected_value' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected_value) else 'expected_value'}
...
def test_parse_start():
    ...
    if not @py_assert9:
        @py_format12 = @pytest_ar._call_reprcompare(('==', ), (@py_assert9,), ('%(py8)s\n{%(py8)s = %(py6)s\n{%(py6)s = %(py0)s(%(py2)s, %(py4)s)\n}.type\n} == %(py11)s', ), (@py_assert7, @py_assert10)) % {'py0':@pytest_ar._saferepr(parse_line) if 'parse_line' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(parse_line) else 'parse_line',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py8':@pytest_ar._saferepr(@py_assert7),  'py11':@pytest_ar._saferepr(@py_assert10)}
...
def test_parse_filterlist():
    ...
    if not @py_assert4:
        @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} == %(py9)s\n{%(py9)s = %(py5)s(%(py7)s)\n}', ), (@py_assert2, @py_assert8)) % {'py0':@pytest_ar._saferepr(next) if 'next' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(next) else 'next',  'py1':@pytest_ar._saferepr(result) if 'result' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(result) else 'result',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(Header) if 'Header' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(Header) else 'Header',  'py7':@pytest_ar._saferepr(@py_assert6),  'py9':@pytest_ar._saferepr(@py_assert8)}
    ...
    if not @py_assert4:
        @py_format12 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} == %(py11)s\n{%(py11)s = %(py5)s(%(py7)s, %(py9)s)\n}', ), (@py_assert2, @py_assert10)) % {'py0':@pytest_ar._saferepr(next) if 'next' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(next) else 'next',  'py1':@pytest_ar._saferepr(result) if 'result' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(result) else 'result',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(Metadata) if 'Metadata' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(Metadata) else 'Metadata',  'py7':@pytest_ar._saferepr(@py_assert6),  'py9':@pytest_ar._saferepr(@py_assert8),  'py11':@pytest_ar._saferepr(@py_assert10)}
        @py_format14 = 'assert %(py13)s' % {'py13': @py_format12}
        raise AssertionError(@pytest_ar._format_explanation(@py_format14))
    ...
    if not @py_assert6:
        @py_format9 = @pytest_ar._call_reprcompare(('==', ), (@py_assert6,), ('%(py5)s\n{%(py5)s = %(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n}.type\n} == %(py8)s', ), (@py_assert4, @py_assert7)) % {'py0':@pytest_ar._saferepr(next) if 'next' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(next) else 'next',  'py1':@pytest_ar._saferepr(result) if 'result' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(result) else 'result',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(@py_assert4),  'py8':@pytest_ar._saferepr(@py_assert7)}
...
def test_exception_timing():
    ...
    if not @py_assert4:
        @py_format10 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} == %(py9)s\n{%(py9)s = %(py5)s(%(py7)s)\n}', ), (@py_assert2, @py_assert8)) % {'py0':@pytest_ar._saferepr(next) if 'next' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(next) else 'next',  'py1':@pytest_ar._saferepr(result) if 'result' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(result) else 'result',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(Comment) if 'Comment' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(Comment) else 'Comment',  'py7':@pytest_ar._saferepr(@py_assert6),  'py9':@pytest_ar._saferepr(@py_assert8)}
        @py_format12 = 'assert %(py11)s' % {'py11': @py_format10}
        raise AssertionError(@pytest_ar._format_explanation(@py_format12))
    @py_assert2 = @py_assert4 = @py_assert6 = @py_assert8 = None
    with pytest.raises(ParseError):
