...
def test_to_blocks(fl_lines):
    ...
    if not @py_assert3:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.description\n} == %(py5)s', ), (@py_assert1, @py_assert4)) % {'py0':@pytest_ar._saferepr(block) if 'block' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(block) else 'block',  'py2':@pytest_ar._saferepr(@py_assert1),  'py5':@pytest_ar._saferepr(@py_assert4)}
    ...
    if not @py_assert2:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert2,), ('%(py1)s == %(py5)s\n{%(py5)s = %(py3)s.URL_PATTERN\n}', ), (@py_assert0, @py_assert4)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(SelectorType) if 'SelectorType' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(SelectorType) else 'SelectorType',  'py5':@pytest_ar._saferepr(@py_assert4)}
        @py_format8 = 'assert %(py7)s' % {'py7': @py_format6}
        raise AssertionError(@pytest_ar._format_explanation(@py_format8))
    ...
    if not @py_assert4:
        @py_format8 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py1)s.action\n} == %(py7)s\n{%(py7)s = %(py5)s.SHOW\n}', ), (@py_assert2, @py_assert6)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(FilterAction) if 'FilterAction' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(FilterAction) else 'FilterAction',  'py7':@pytest_ar._saferepr(@py_assert6)}
...
def test_to_dict(fl_lines, expected_blocks):
    ...
    if not @py_assert1:
        @py_format3 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py2)s', ), (blocks, expected_blocks)) % {'py0':@pytest_ar._saferepr(blocks) if 'blocks' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(blocks) else 'blocks',  'py2':@pytest_ar._saferepr(expected_blocks) if 'expected_blocks' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected_blocks) else 'expected_blocks'}
