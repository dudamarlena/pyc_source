...
def test_line2dict_format(line_type):
    ...
    """
    position = 'start' if line_type in {'metadata', 'header'} else 'body'
    ...
    if not @py_assert1:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py3)s', ), (data, @py_assert2)) % {'py3': @pytest_ar._saferepr(@py_assert2), 'py0': @pytest_ar._saferepr(data) if 'data' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(data) else 'data'}
...
def test_lines2dicts_start_mode():
    ...
    if not @py_assert6:
        @py_format8 = @pytest_ar._call_reprcompare(('==', ), (@py_assert6,), ('%(py5)s\n{%(py5)s = %(py0)s(%(py1)s, %(py3)s)\n} == %(py7)s', ), (@py_assert4, outs)) % {'py7': @pytest_ar._saferepr(outs) if 'outs' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(outs) else 'outs', 'py3': @pytest_ar._saferepr(@py_assert2), 'py5': @pytest_ar._saferepr(@py_assert4), 'py0': @pytest_ar._saferepr(lines2dicts) if 'lines2dicts' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines2dicts) else 'lines2dicts', 'py1': @pytest_ar._saferepr(ins) if 'ins' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(ins) else 'ins'}
...
def test_lines2dicts_default():
    ...
    """
    tests = [t for t in _TEST_EXAMPLES.values() if t['out']['type'] not in {'Header', 'Metadata'}]
    ins = [ex['in'] for ex in tests]
    outs = [ex['out'] for ex in tests]
    ...
    if not @py_assert4:
        @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert4,), ('%(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n} == %(py5)s', ), (@py_assert2, outs)) % {'py3': @pytest_ar._saferepr(@py_assert2), 'py5': @pytest_ar._saferepr(outs) if 'outs' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(outs) else 'outs', 'py0': @pytest_ar._saferepr(lines2dicts) if 'lines2dicts' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(lines2dicts) else 'lines2dicts', 'py1': @pytest_ar._saferepr(ins) if 'ins' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(ins) else 'ins'}
