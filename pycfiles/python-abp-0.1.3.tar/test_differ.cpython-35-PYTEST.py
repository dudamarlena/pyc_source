...
def test_differ():
    ...
    if not @py_assert1:
        @py_format3 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py2)s', ), (gen, exp)) % {'py2': @pytest_ar._saferepr(exp) if 'exp' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(exp) else 'exp', 'py0': @pytest_ar._saferepr(gen) if 'gen' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(gen) else 'gen'}
