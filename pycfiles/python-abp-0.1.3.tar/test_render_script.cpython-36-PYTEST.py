...
def test_render_unicode(rootdir, dstfile):
    ...
    if not @py_assert2:
        @py_format16 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py15)s\n{%(py15)s = %(py11)s\n{%(py11)s = %(py9)s\n{%(py9)s = %(py5)s\n{%(py5)s = %(py3)s.read\n}(mode=%(py7)s)\n}.decode\n}(%(py13)s)\n}', ), (@py_assert0, @py_assert14)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(dstfile) if 'dstfile' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(dstfile) else 'dstfile',  'py5':@pytest_ar._saferepr(@py_assert4),  'py7':@pytest_ar._saferepr(@py_assert6),  'py9':@pytest_ar._saferepr(@py_assert8),  'py11':@pytest_ar._saferepr(@py_assert10),  'py13':@pytest_ar._saferepr(@py_assert12),  'py15':@pytest_ar._saferepr(@py_assert14)}
...
def test_render_with_includes(rootdir, dstfile):
    ...
    if not @py_assert2:
        @py_format8 = @pytest_ar._call_reprcompare(('in', ), (@py_assert2,), ('%(py1)s in %(py7)s\n{%(py7)s = %(py5)s\n{%(py5)s = %(py3)s.read\n}()\n}', ), (@py_assert0, @py_assert6)) % {'py1':@pytest_ar._saferepr(@py_assert0),  'py3':@pytest_ar._saferepr(dstfile) if 'dstfile' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(dstfile) else 'dstfile',  'py5':@pytest_ar._saferepr(@py_assert4),  'py7':@pytest_ar._saferepr(@py_assert6)}
...
def test_render_verbose(rootdir, dstfile):
    code, err, _ = run_script('includer.txt', (str(dstfile)), '-i',
      'inc=inc', '-v', cwd=(str(rootdir)))
    ...
    if not @py_assert1:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py3)s', ), (err, @py_assert2)) % {'py0':@pytest_ar._saferepr(err) if 'err' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(err) else 'err',  'py3':@pytest_ar._saferepr(@py_assert2)}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
...
def test_wrong_include(rootdir, dstfile):
    code, err, _ = run_script('brk.txt', (str(dstfile)), '-i',
      'inc=inc', cwd=(str(rootdir)))
    ...
    if not @py_assert1:
        @py_format3 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py2)s', ), (err, expect)) % {'py0':@pytest_ar._saferepr(err) if 'err' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(err) else 'err',  'py2':@pytest_ar._saferepr(expect) if 'expect' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expect) else 'expect'}
...
def test_circular_includes(rootdir, dstfile):
    code, err, _ = run_script('circ.txt', (str(dstfile)), '-i',
      'inc=inc', cwd=(str(rootdir)))
...
def test_failed_web_include(rootdir, dstfile, webserver_port):
    ...
    if not @py_assert10:
        @py_format12 = 'assert %(py11)s\n{%(py11)s = %(py2)s\n{%(py2)s = %(py0)s.startswith\n}(%(py9)s\n{%(py9)s = %(py6)s\n{%(py6)s = %(py4)s.format\n}(%(py7)s)\n})\n}' % {'py0':@pytest_ar._saferepr(err) if 'err' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(err) else 'err',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py7':@pytest_ar._saferepr(url) if 'url' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(url) else 'url',  'py9':@pytest_ar._saferepr(@py_assert8),  'py11':@pytest_ar._saferepr(@py_assert10)}
