...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pyucsm.py
# Compiled at: 2011-11-02 13:20:45
import httplib, logging, os, socket
from xml.dom import minidom
import xml.dom as dom, threading
...
class UcsmConnection(object):
    ...
    def __init__(self, host, port=None, secure=False, *args, **kwargs):
        ...
        self.__wait_stop_cond = threading.Condition(self.__wait_stop_lock)
        self.__wait_refresh_lock = threading.Lock()
        self.__wait_refresh_cond = threading.Condition(self.__wait_refresh_lock)
        ...
        self.secure = secure
        if secure:
            self._create_connection = lambda : httplib.HTTPSConnection(self.host, self.port, *args, **kwargs)
        else:
            self._create_connection = lambda : httplib.HTTPConnection(self.host, self.port, *args, **kwargs)
        return
    ...
    def _syncronized_request(f, self, *args, **kwargs):
        try:
            ...
            LOG.debug('Current concurrent workers: %s' % self.concurrent_requests)
...
Cookie refresh will be performed automatically."""
    ...
    def login(self, login, password, cookie_timeout=600):
        """Performs authorisation and retrieving cookie from server.
Cookie refresh will be performed automatically."""
        self.__cookie = None
        self.__login = None
        ...
        self.session_id = None
        try:
            reply_xml, conn = self._perform_query('aaaLogin', inName=login, inPassword=password)
            self._check_is_error(reply_xml.firstChild)
            response_atom = reply_xml.firstChild
        ...
        try:
            for pair in buf_res:
                if pair.ucs_class == 'pair':
                    ...
                    raise UcsmFatalError('Wrong reply: non-pair objectin outConfigs section')

            return res
        except IndexError:
            raise UcsmFatalError('Wrong reply: recieved pairdoes not contains value')
        except AttributeError:
            raise UcsmFatalError('Wrong reply: recieved pairdoes not have key')
    ...
    def _get_unresolved_from_response(self, data):
        try:
            ...
            xml_childs = [ child for child in out_config.childNodes if child.nodeType == dom.Node.ELEMENT_NODE and child.nodeName == 'dn'
                         ]
            return map(lambda c: c.attributes['value'].value.encode('utf8'), xml_childs)
        except (KeyError, IndentationError):
            raise UcsmFatalError('No outUnresolved sectionin server response!')
    ...
    def resolve_children(self, class_id='', dn='', hierarchy=False, filter=UcsmFilterOp()):
        ...
        data, conn = self._perform_query('configResolveChildren', filter=filter, cookie=self.__cookie, inDn=dn, inHierarchical=(hierarchy and 'yes' or 'no'), **kwargs)
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)

    @_syncronized_request
    def scope(self, class_id, dn, filter=UcsmFilterOp(), hierarchy=False, recursive=False):
        data, conn = self._perform_query('configScope', filter=filter, cookie=self.__cookie, dn=dn, inClass=class_id, inRecursive=recursive and 'yes' or 'no', inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)

    @_syncronized_request
    def resolve_class(self, class_id, filter=UcsmFilterOp(), hierarchy=False):
        data, conn = self._perform_query('configResolveClass', filter=filter, cookie=self.__cookie, classId=class_id, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)
    ...
    def resolve_classes(self, classes, hierarchy=False):
        ...
        data, conn = self._perform_complex_query('configResolveClasses', data=classes_node, cookie=self.__cookie, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)

    @_syncronized_request
    def resolve_dn(self, dn, hierarchy=False):
        data, conn = self._perform_query('configResolveDn', cookie=self.__cookie, dn=dn, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
...
of unresolved dns."""
        ...
        data, conn = self._perform_complex_query('configResolveDns', data=dns_node, cookie=self.__cookie, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
    ...
    def find_dns_by_class_id(self, class_id, filter=None):
        data, conn = self._perform_query('configFindDnsByClassId', filter=filter, cookie=self.__cookie, classId=class_id)
        self._check_is_error(data.firstChild)
        try:
            out_dns_node = data.getElementsByTagName('outDns')[0]
            dns = [ child.attributes['value'].value.encode('utf8') for child in out_dns_node.childNodes if child.nodeType == dom.Node.ELEMENT_NODE
                  ]
            return dns
    ...
    def resolve_parent(self, dn, hierarchy=False):
        data, conn = self._perform_query('configResolveParent', cookie=self.__cookie, dn=dn, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        res = self._get_single_object_from_response(data)
        return res
...
('created', 'deleted', 'modified')."""
        ...
        data, conn = self._perform_complex_query('configConfMo', data=in_config_node, dn=dn, cookie=self.__cookie, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        res = self._get_single_object_from_response(data)
        return res
...
Possible values: ('created', 'deleted', 'modified')."""
        ...
        data, conn = self._perform_complex_query('configConfMos', cookie=self.__cookie, data=configs_xml)
        self._check_is_error(data.firstChild)
...
configs."""
        ...
        data, conn = self._perform_complex_query('configEstimateImpact', cookie=self.__cookie, data=configs_xml)
        self._check_is_error(data.firstChild)
        try:
    ...
    def conf_mo_group(self, dns, config, hierarchy=False):
        ...
        data, conn = self._perform_complex_query('configConfMoGroup', cookie=self.__cookie, data=[
         dns_xml, config_xml], inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)

    @_syncronized_request
    def clone_profile(self, dn, name, target_org_dn='org-root', hierarchy=True):
        data, conn = self._perform_query('lsClone', cookie=self.__cookie, dn=dn, inTargetOrg=target_org_dn, inServerName=name, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        res = self._get_single_object_from_response(data)
        return res

    @_syncronized_request
    def instantiate_template(self, dn, name, target_org_dn='org-root', hierarchy=False):
        """Returns created profile."""
        data, conn = self._perform_query('lsInstantiateTemplate', cookie=self.__cookie, dn=dn, inTargetOrg=target_org_dn, inServerName=name, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        res = self._get_single_object_from_response(data)
        return res

    @_syncronized_request
    def instantiate_n_template(self, dn, target_org_dn='org-root', prefix='', number=1, hierarchy=False):
        data, conn = self._perform_query('lsInstantiateNTemplate', cookie=self.__cookie, dn=dn, inTargetOrg=target_org_dn, inServerNamePrefixOrEmpty=prefix, inNumberOf=number, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)

    @_syncronized_request
    def instantiate_n_template_named(self, dn, name_set, target_org_dn='org-root', hierarchy=True):
        ...
        data, conn = self._perform_complex_query('lsInstantiateNNamedTemplate', inNames, dn=dn, cookie=self.__cookie, inTargetOrg=target_org_dn, inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_objects_from_response(data)
    ...
    def resolve_elements(self, dn, class_id, single_level=False, hierarchy=False, filter=UcsmFilterOp()):
        ...
        data, conn = self._perform_query('orgResolveElements', filter=filter, dn=dn, cookie=self.__cookie, inClass=class_id, inSingleLevel=single_level and 'yes' or 'no', inHierarchical=hierarchy and 'yes' or 'no')
        self._check_is_error(data.firstChild)
        return self._get_pairs_from_response(data)
...
Yields event id and configuraion."""
    ...
    def _iter_xml_events(self, filter=UcsmFilterOp()):
        request_data = self._instantiate_complex_query('eventSubscribe', child_data=filter.final_xml_node(), cookie=self.__cookie)
        conn = self._create_connection()
        body = request_data
        LOG.debug('>> %s', body)
        try:
...
class UcsmObject(object):
    __slots__ = [
     '__init__', '__getattr__', '__setattr__', '__repr__', 'xml',
     'xml_node', 'pretty_xml',
    ...
    def __init__(self, node_or_class=None, parent=None):
        ...
        if node_or_class is None:
            ...
            if isinstance(node_or_class, UcsmObject):
            ...
            if parent is not None and 'dn' not in self.attributes and 'rn' in self.attributes and 'dn' in parent.attributes:
    ...
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            return self.attributes == other.attributes and self.ucs_class == other.ucs_class and self.children == other.children
        else:
            return False
