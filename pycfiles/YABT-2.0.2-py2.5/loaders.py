...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/YABT/loaders.py
# Compiled at: 2009-07-27 15:40:38
"""
...
class BasicXMLFileLoader(BaseLoader):
    ...
    def _startElement(self, name, attrs):
        ...
        if name == 'char':
            ...
            if iclass not in self.__decisions:
            ...
            if self._path == '.transtable.YABTdata.rule.focus':
                self._focus = ''
            if self._path == '.transtable.YABTdata.rule.bfcontext':
                self._bfcontext = ''
    ...
    def _addRule(self):
        ...
        bfContextObj = self.contextFactory.getContextInstance(bftype, bfcontext, self.contextFactory.BEFORE)
        afContextObj = self.contextFactory.getContextInstance(aftype, afcontext, self.contextFactory.AFTER)
        group = focus[0]
        rule = GeneralContextRule(focus, bfContextObj, afContextObj, trans, fstate)
