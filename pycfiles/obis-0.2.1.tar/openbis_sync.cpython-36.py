...
# [GCC 8.4.0]
# Embedded file name: /Users/vermeul/openbis/obis/src/python/obis/dm/commands/openbis_sync.py
...
class OpenbisSync(OpenbisCommand):
    ...
    def check_configuration(self):
        ...
        if self.object_id() is None:
            if self.object_permId() is None:
                if self.collection_id() is None:
                    if self.collection_permId() is None:
                        missing_config_settings.append('object id or collection id')
        if len(missing_config_settings) > 0:
            return CommandResult(returncode=(-1), output=('Missing configuration settings for {}.'.format(missing_config_settings)))
        else:
    ...
    def create_data_set_code(self):
        try:
            ...
            return (
             CommandResult(returncode=(-1), output=(str(e))), None)
    ...
    def create_data_set(self, data_set_code, external_dms, repository_id, ignore_parent=False):
        ...
        try:
            data_set = self.openbis.new_git_data_set(data_set_type, top_level_path, commit_id, repository_id, (external_dms.code), sample=sample_id,
              experiment=experiment_id,
              properties=properties,
              parents=parent_data_set_id,
