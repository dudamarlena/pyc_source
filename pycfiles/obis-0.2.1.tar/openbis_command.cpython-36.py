...
# [GCC 8.4.0]
# Embedded file name: /Users/vermeul/openbis/obis/src/python/obis/dm/commands/openbis_command.py
# Compiled at: 2019-06-11 04:12:07
# Size of source mod 2**32: 9901 bytes
import getpass, hashlib, os, socket, pybis
...
class OpenbisCommand(object):
    __doc__ = ' Superclass for commands connecting to openBIS.\n    '
    ...
    def __init__(self, dm):
        ...
        if self.openbis is None:
            if dm.openbis_config.get('url') is not None:
    ...
    def get_or_create_external_data_management_system(self):
        ...
        if result.failure():
            ...
            try:
                ...
                try:
                    external_dms = self.openbis.create_external_data_management_system(external_dms_id, external_dms_id, '{}:/{}'.format(hostname, edms_path))
...
class ContentCopySelector(object):
    __doc__ = ' In case a command needs information from a content copy, this class\n    asks the user to pick one if there are multiple. '
    ...
    def select_content_copy_index(self, content_copies):
        if self.content_copy_index is not None:
            if self.content_copy_index >= 0:
                if self.content_copy_index < len(content_copies):
            ...
            while 1:
                ...
                if copy_index_string.isdigit():
                    ...
                    if copy_index_int >= 0:
                        if copy_index_int < len(content_copies):
