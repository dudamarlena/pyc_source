...
# [GCC 8.4.0]
# Embedded file name: /Users/vermeul/openbis/obis/src/python/obis/dm/config.py
# Compiled at: 2019-02-28 10:10:16
# Size of source mod 2**32: 14464 bytes
"""
config.py
...
class ConfigLocation(object):
    __doc__ = 'Path for configuration information.'
...
class ConfigParam(object):
    __doc__ = 'Class for configuration parameters.'
...
class ConfigEnv(object):
    __doc__ = 'The environment in which configurations are constructed.'
...
class RepositoryEnv(ConfigEnv):
    __doc__ = ' These are properties which are not configured by the user but set by obis. '
...
class LocationResolver(object):
    ...
    def __init__(self):
        self.location_roots = {'user_home':os.path.expanduser('~'), 
         'data_set':'./'}
...
class ConfigResolver(object):
    __doc__ = 'Construct a config dictionary.'
    ...
    def set_value_for_parameter(self, name, value, loc, apply_rules=False):
        ...
        :return:
        """
        if name not in self.env.params:
...
class SettingsResolver(object):
    __doc__ = ' This class functions as a wrapper since we have multiple config resolvers. '
...
class SettingRule(object):
    __doc__ = ' Setting rules can react to setting changes and trigger further actions. '
...
class ClearPermIdRule(SettingRule):
    __doc__ = ' When the user sets a new id, the permId might be invalid, so it will be cleared. '
