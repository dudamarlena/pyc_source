...
# [GCC 8.4.0]
# Embedded file name: /Users/vermeul/openbis/obis/src/python/obis/dm/data_mgmt.py
# Compiled at: 2019-06-11 04:12:07
# Size of source mod 2**32: 19501 bytes
"""
data_mgmt.py
...
"""
import abc, json, os, shutil, traceback, pybis, requests, signal, sys
...
class AbstractDataMgmt(metaclass=abc.ABCMeta):
    __doc__ = 'Abstract object that implements operations.\n\n    All operations throw an exepction if they fail.\n    '
...
class NoGitDataMgmt(AbstractDataMgmt):
    __doc__ = 'DataMgmt operations when git is not available -- show error messages.'
...
class GitDataMgmt(AbstractDataMgmt):
    __doc__ = 'DataMgmt operations in normal state.'
    ...
    def config(self, category, is_global, is_data_set_property, prop=None, value=None, set=False, get=False, clear=False):
        ...
        if resolver is None:
            ...
            assert set == True and get == False
            assert clear == False
            assert prop is not None
        if not value is not None:
            raise AssertionError
        else:
            assert get == True and set == False
            assert clear == False
