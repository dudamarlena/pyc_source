...
# [GCC 8.4.0]
# Embedded file name: /Users/vermeul/openbis/obis/src/python/obis/dm/checksum.py
# Compiled at: 2019-05-21 15:45:31
# Size of source mod 2**32: 6918 bytes
import abc, hashlib, json, os
...
def validate_checksum(openbis, files, data_set_id, data_path, metadata_path):
    ...
    for filename in files:
        ...
        if dataset_file['checksumCRC32'] is not None:
            if dataset_file['checksumCRC32'] > 0:
                checksum_generator = ChecksumGeneratorCrc32(data_path, metadata_path)
                expected_checksum = dataset_file['checksumCRC32']
            else:
                if dataset_file['checksumType'] is not None:
...
class ChecksumGeneratorHashlib(ChecksumGenerator):
    ...
    def _get_checksum(self, file):
        return {'checksum':self._checksum(file), 
...
class ChecksumGeneratorWORM(ChecksumGenerator):
    ...
    def _get_checksum(self, file):
        return {'checksum':self.worm(file), 
         'checksumType':'WORM', 
         'fileLength':os.path.getsize(file), 
         'path':file}
...
class ChecksumGeneratorGitAnnex(ChecksumGenerator):
    __doc__ = ' This class generates checksums according to the git annex backend configuration. '
    ...
    def __init__(self, data_path, metadata_path):
        ...
        self.checksum_generator_supplement = get_checksum_generator((self.backend),
          (self.data_path), (self.metadata_path), default=(ChecksumGeneratorCrc32(self.data_path, self.metadata_path)))

    def _get_checksum(self, file):
        if self.checksum_generator_replacement is not None:
            return self.checksum_generator_replacement.get_checksum(file)
        else:
            return self._ChecksumGeneratorGitAnnex__get_checksum(file)
    ...
    def __get_checksum(self, file):
        ...
        if 'Not a valid object name' in annex_result.output:
            ...
            if annex_info['present'] != True:
                return self.checksum_generator_supplement.get_checksum(file)
            return {'checksum':self._get_checksum_from_annex_info(annex_info),  'checksumType':self.backend, 
             'fileLength':os.path.getsize(file), 
             'path':file}
