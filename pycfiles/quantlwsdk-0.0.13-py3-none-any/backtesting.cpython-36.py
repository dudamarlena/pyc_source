...
class Broker(broker.Broker):
    ...
    def _registerOrder(self, order):
        if not order.getId() not in self._Broker__activeOrders:
            raise AssertionError
        elif not order.getId() is not None:
            raise AssertionError
        self._Broker__activeOrders[order.getId()] = order

    def _unregisterOrder(self, order):
        if not order.getId() in self._Broker__activeOrders:
            raise AssertionError
        elif not order.getId() is not None:
            raise AssertionError
    ...
    def setShares(self, instrument, quantity, price):
        ...
        :param price: The price for each share.
        """
        assert not self._Broker__started, "Can't setShares once the strategy started executing"
...
class ChinaFutureBroker(broker.Broker):
    ...
    def _registerOrder(self, order):
        if not order.getId() not in self._ChinaFutureBroker__activeOrders:
            raise AssertionError
        elif not order.getId() is not None:
            raise AssertionError
    ...
    def _unregisterOrder(self, order):
        if not order.getId() in self._ChinaFutureBroker__activeOrders:
            raise AssertionError
        elif not order.getId() is not None:
            raise AssertionError
    ...
    def setShares(self, instrument, quantity, price):
        ...
        :param price: The price for each share.
        """
        assert not self._ChinaFutureBroker__started, "Can't setShares once the strategy started executing"
