...
# [GCC 8.4.0]
# Embedded file name: ..\indicatorModule\highcharts\highstock\common.py
# Compiled at: 2019-10-18 05:04:49
# Size of source mod 2**32: 23433 bytes
""" Python-Highstock common.py
Common Functions For Highstock
"""
from past.builtins import basestring
import datetime, re
FORMATTER_TYPE_MAPPINGS = {'default':'function() { return this.value }', 
...
REGEX_LIST = {'re_funct':re.compile('.*function\\(.*\\)\\{.*\\}', re.I), 
 're_hcharts':re.compile('.*Highcharts.*', re.I)}

class Formatter(object):
    __doc__ = ' Base Formatter Class '
            ...
            if color:
                if isinstance(color, dict):
                    ...
                    for i, item in enumerate(color['stops']):
                        tmp.append([RawJavaScriptText(x) if any([REGEX_LIST[key].search(x) for key in REGEX_LIST.keys()]) else x for x in item if isinstance(x, basestring)])

                    color['stops'] = tmp
                    self.color = color
                else:
                    if any([REGEX_LIST[key].search(color) for key in REGEX_LIST.keys()]):
    ...
    def __init__(self, function=None):
        if function:
            if isinstance(function, basestring):
                ...
                if isinstance(function, JSfunction):
    ...
    def update(self, kwargs):
        for k, v in kwargs.items():
            if k in self.ALLOWED_OPTIONS:
                if isinstance(self.ALLOWED_OPTIONS[k], tuple):
                    if isinstance(self.ALLOWED_OPTIONS[k][0](), CommonObject):
                ...
                if isinstance(self.ALLOWED_OPTIONS[k], tuple) and isinstance(self.ALLOWED_OPTIONS[k][0](), ArrayObject):
                    if isinstance(v, dict):
                ...
                elif isinstance(self.ALLOWED_OPTIONS[k], tuple):
                    if isinstance(self.ALLOWED_OPTIONS[k][0](), CSSObject) or isinstance(self.ALLOWED_OPTIONS[k][0](), SVGObject):
                        if self.__getattr__(k):
                    ...
                    if isinstance(self.ALLOWED_OPTIONS[k][0](), JSfunction) or isinstance(self.ALLOWED_OPTIONS[k][0](), Formatter) or isinstance(self.ALLOWED_OPTIONS[k][0](), ColorObject):
                        if isinstance(v, dict):
                            self.__dict__.update({k: (self.ALLOWED_OPTIONS[k][0])(**v)})
                        else:
                            self.__dict__.update({k: self.ALLOWED_OPTIONS[k][0](v)})
        ...
        for k, v in kwargs.items():
            if k in self.ALLOWED_OPTIONS:
                if self.__validate_options__(k, v, self.ALLOWED_OPTIONS[k]):
                    if isinstance(self.ALLOWED_OPTIONS[k], tuple) and self.ALLOWED_OPTIONS[k][0] in IDV_OBJECT_LIST:
                        if isinstance(v, dict):
                            self.__dict__.update({k: (self.ALLOWED_OPTIONS[k][0])(**v)})
                        else:
...
class Events(CommonObject):
    __doc__ = ' Class for event listener '
    ALLOWED_OPTIONS = {'addSeries':(
...
class ContextButton(CommonObject):
    __doc__ = ' Option class for the export button '
    ALLOWED_OPTIONS = {'align':basestring, 
     'enabled':bool, 
     'height':int, 
...
class Labels(CommonObject):
    ALLOWED_OPTIONS = {'align':basestring, 
     ...
     'format':basestring, 
     'formatter':(
      Formatter, JSfunction, basestring), 
     'inside':bool, 
     'overflow':basestring, 
     'padding':[
      float, int], 
...
class Handles(CommonObject):
    ALLOWED_OPTIONS = {'backgroundColor':(
      ColorObject, basestring, dict), 
     'borderColor':(
      ColorObject, basestring, dict)}
...
class DateTimeLabelFormats(CommonObject):
    ALLOWED_OPTIONS = {'millisecond':basestring, 
     'second':basestring, 
     'minute':basestring, 
     'hour':basestring, 
     'day':basestring, 
...
class Tooltip(CommonObject):
    ALLOWED_OPTIONS = {'dateTimeLabelFormats':(
      DateTimeLabelFormats, dict), 
     'followPointer':bool, 
...
class Items(ArrayObject):
    ALLOWED_OPTIONS = {'html':basestring, 
     'style':(
      CSSObject, dict)}
...
class Buttons(ArrayObject):
    ALLOWED_OPTIONS = {'type':basestring, 
     'count':[
      int, float], 
     'text':basestring}
...
IDV_OBJECT_LIST = [
 JSfunction, Formatter, Halo, Marker, Labels,
