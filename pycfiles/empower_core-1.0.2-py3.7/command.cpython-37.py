...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/empower_core/command.py
# Compiled at: 2020-05-10 06:49:25
# Size of source mod 2**32: 6614 bytes
"""Command Line Interface."""
import sys, base64, getpass, json, pkgutil, importlib
from argparse import ArgumentParser
import requests
import requests.exceptions as RequestConnectionError
import empower_core.cli as help_commands
...
def load_commands(commands):
    """Load commands from specified package."""
    for _, name, is_pkg in pkgutil.walk_packages(commands.__path__):
        if is_pkg and name.endswith('_commands'):
            package = importlib.import_module(commands.__name__ + '.' + name)
...
def connect(gargs, cmd, expected=200, request=None, headers=None, exit_on_err=True):
    """ Run command. """
    if not headers:
...
def parse_global_args(arglist):
    ...
    parser.add_argument('-r', '--host', dest='host', default='127.0.0.1', help="REST server address; default='127.0.0.1'")
    parser.add_argument('-p', '--port', dest='port', default='8888', help='REST server port; default=8888')
    parser.add_argument('-u', '--user', dest='user', default='root', help="EmPOWER admin user; default='root'")
    parser.add_argument('-a', '--auth', dest='auth', default=None, help='EmPOWER admin user:passw')
    parser.add_argument('-n', '--no-passwd', action='store_true', dest='no_passwd',
      default=False,
      help='Run without password; default false')
    parser.add_argument('-f', '--passwd-file', dest='passwdfile', default=None,
      help='Password file; default=none')
    parser.add_argument('-t', '--transport', dest='transport', default='http', help="The transport (http/https); default='http'")
