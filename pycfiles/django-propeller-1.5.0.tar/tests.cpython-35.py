...
# Compiled at: 2017-02-18 10:39:17
# Size of source mod 2**32: 25095 bytes
...
class TestForm(forms.Form):
    ...
    subject = forms.CharField(max_length=100, help_text='my_help_text', required=True, widget=forms.TextInput(attrs={'placeholder': 'placeholdertest'}))
    password = forms.CharField(widget=forms.PasswordInput)
    message = forms.CharField(required=False, help_text='<i>my_help_text</i>')
    sender = forms.EmailField(label='Sender © unicode', help_text='E.g., "me@example.com"')
    secret = forms.CharField(initial=42, widget=forms.HiddenInput)
    cc_myself = forms.BooleanField(required=False, help_text='cc stands for "carbon copy." You will get a copy in your mailbox.')
    ...
    category2 = forms.MultipleChoiceField(choices=RADIO_CHOICES, widget=forms.CheckboxSelectMultiple, help_text='Check as many as you like.')
    category3 = forms.ChoiceField(widget=forms.RadioSelect, choices=MEDIA_CHOICES)
    category4 = forms.MultipleChoiceField(choices=MEDIA_CHOICES, widget=forms.CheckboxSelectMultiple, help_text='Check as many as you like.')
...
class TemplateTest(TestCase):
    ...
    def test_propeller_template(self):
        res = render_template('{% extends "propeller/propeller.html" %}' + '{% block propeller_content %}' + 'test_propeller_content' + '{% endblock %}')
...
class FormTest(TestCase):
    ...
    def test_exclude(self):
        form = TestForm()
        res = render_template_with_form('{% propeller_form form exclude="cc_myself" %}', {'form': form})
        self.assertNotIn('cc_myself', res)

    def test_layout_horizontal(self):
        form = TestForm()
        res = render_template_with_form('{% propeller_form form layout="horizontal" %}', {'form': form})
        self.assertIn('col-md-3', res)
        self.assertIn('col-md-9', res)
        res = render_template_with_form('{% propeller_form form layout="horizontal" ' + 'horizontal_label_class="hlabel" ' + 'horizontal_field_class="hfield" %}', {'form': form})
    ...
    def test_error_class(self):
        ...
        res = render_template_with_form('{% propeller_form form %}', {'form': form})
        self.assertIn('bootstrap3-err', res)
        res = render_template_with_form('{% propeller_form form error_css_class="successful-test" %}', {'form': form})
        self.assertIn('successful-test', res)
        res = render_template_with_form('{% propeller_form form error_css_class="" %}', {'form': form})
    ...
    def test_required_class(self):
        ...
        res = render_template_with_form('{% propeller_form form %}', {'form': form})
        self.assertIn('bootstrap3-req', res)
        res = render_template_with_form('{% propeller_form form required_css_class="successful-test" %}', {'form': form})
        self.assertIn('successful-test', res)
        res = render_template_with_form('{% propeller_form form required_css_class="" %}', {'form': form})
    ...
    def test_bound_class(self):
        ...
        res = render_template_with_form('{% propeller_form form %}', {'form': form})
        self.assertIn('bootstrap3-bound', res)
        res = render_template_with_form('{% propeller_form form bound_css_class="successful-test" %}', {'form': form})
        self.assertIn('successful-test', res)
        res = render_template_with_form('{% propeller_form form bound_css_class="" %}', {'form': form})
...
class FieldTest(TestCase):
    ...
    def test_required_field(self):
        ...
        form_field = 'form.subject'
        rendered = render_template_with_form('{% propeller_field ' + form_field + ' required_css_class="test-required" %}')
    ...
    def test_input_group_addon_button(self):
        res = render_template_with_form('{% propeller_field form.subject addon_before="$" addon_before_class="input-group-btn" addon_after=".00" addon_after_class="input-group-btn" %}')
        self.assertIn('class="input-group"', res)
...
class MessagesTest(TestCase):
    ...
    def test_messages(self):
        ...
        res = render_template_with_form('{% propeller_messages messages %}', {'messages': messages})
...
class ButtonTest(TestCase):
    ...
    def test_button(self):
        ...
        self.assertEqual(res.strip(), '<button class="btn btn-default pmd-ripple-effect btn-lg">button</button>')
        res = render_template_with_form("{% propeller_button 'button' size='lg' href='#' %}")
        self.assertIn(res.strip(), '<a class="btn btn-default btn-lg" href="#">button</a><a href="#" ' + 'class="btn btn-lg">button</a>')
...
class ShowLabelTest(TestCase):
    ...
    def test_show_label(self):
        form = TestForm()
        res = render_template_with_form('{% propeller_form form show_label=False %}', {'form': form})
        self.assertIn('sr-only', res)

    def test_for_formset(self):
        TestFormSet = formset_factory(TestForm, extra=1)
        test_formset = TestFormSet()
        res = render_template_with_form('{% propeller_formset formset show_label=False %}', {'formset': test_formset})
        self.assertIn('sr-only', res)

    def test_button_with_icon(self):
        res = render_template_with_form("{% propeller_button 'test' icon='info-sign' %}")
        self.assertEqual(res.strip(), '<button class="btn btn-default"><span class="glyphicon glyphicon-info-sign"></span> test</button>')
        res = render_template_with_form("{% propeller_button 'test' icon='info-sign' button_class='btn-primary' %}")
        self.assertEqual(res.strip(), '<button class="btn btn-primary"><span class="glyphicon glyphicon-info-sign"></span> test</button>')
        res = render_template_with_form("{% propeller_button 'test' icon='info-sign' button_type='submit' %}")
        self.assertEqual(res.strip(), '<button class="btn btn-default" type="submit"><span class="glyphicon glyphicon-info-sign"></span> test</button>')