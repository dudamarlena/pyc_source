...
# [GCC 8.4.0]
# Embedded file name: /home/thorsten/code/django-propeller/django_propeller/templatetags/propeller.py
...
from ..propeller import css_url, javascript_url, jquery_url, theme_url, get_propeller_setting
from ..components import render_icon, render_alert, render_bootstrap_icon
from ..forms import render_button, render_field, render_field_and_label, render_form, render_form_group, render_formset, render_fab, render_label, render_form_errors, render_formset_errors
...
MESSAGE_LEVEL_CLASSES = {message_constants.DEBUG: b'alert alert-warning', 
   message_constants.INFO: b'alert alert-info', 
   message_constants.SUCCESS: b'alert alert-success', 
   message_constants.WARNING: b'alert alert-warning', 
   message_constants.ERROR: b'alert alert-danger'}
...
def propeller_message_classes(message):
    ...
    try:
        ...
        try:
            ...
            classes.append(b'alert alert-danger')

    return (b' ').join(classes).strip()
...
def propeller_css():
    ...
    **Example**::
    ...
    """
    rendered_urls = [
     render_link_tag(propeller_css_url())]
    if propeller_theme_url():
        rendered_urls.append(render_link_tag(propeller_theme_url()))
    return mark_safe((b'').join([ url for url in rendered_urls ]))
...
def propeller_javascript(jquery=None):
    ...
    **Example**::
    ...
    javascript = b''
    if jquery is None:
        jquery = get_propeller_setting(b'include_jquery', False)
    if jquery:
        url = propeller_jquery_url()
        if url:
            javascript += render_tag(b'script', attrs={b'src': url})
    url = propeller_javascript_url()
    if url:
        attrs = {b'src': url}
        javascript += render_tag(b'script', attrs=attrs)
...
@register.simple_tag
def propeller_alert(content, alert_type=b'info', dismissable=True):
    """
    Render an alert
...
def propeller_buttons(parser, token):
    ...
    **Example**::
    ...
    kwargs[b'nodelist'] = parser.parse(('endbuttons', ))
...
class ButtonsNode(template.Node):
    ...
    def render(self, context):
        ...
        buttons = []
        submit = output_kwargs.get(b'submit', None)
        reset = output_kwargs.get(b'reset', None)
        if submit:
            buttons.append(propeller_button(submit, b'submit'))
        if reset:
            buttons.append(propeller_button(reset, b'reset'))
        buttons = (b' ').join(buttons) + self.nodelist.render(context)
        output_kwargs.update({b'label': None, 
           b'field': buttons})
...
def propeller_messages(context, *args, **kwargs):
    ...
    **Example**::
    ...
    context.update({b'message_constants': message_constants})
    return render_template_file(b'propeller/messages.html', context=context)


@register.inclusion_tag(b'propeller/pagination.html')
...
def get_pagination_context(page, pages_to_show=11, url=None, size=None, extra=None, parameter_name=b'page'):
    """Generate propeller pagination context from a page object"""
    pages_to_show = int(pages_to_show)
    if pages_to_show < 1:
        raise ValueError((b'Pagination pages_to_show should be a positive integer, you specified {pages}').format(pages=pages_to_show))
    ...
    if url:
        url = force_text(url)
        url = re.sub((b'\\?{0}\\=[^\\&]+').format(parameter_name), b'?', url)
        url = re.sub((b'\\&{0}\\=[^\\&]+').format(parameter_name), b'', url)
        if b'?' in url:
            url += b'&'
        else:
    ...
    if extra:
        if not url:
            url = b'?'
        url += force_text(extra) + b'&'
    if url:
        url = url.replace(b'?&', b'?')
    pagination_css_classes = [b'pagination']
    if size == b'small':
        pagination_css_classes.append(b'pagination-sm')
    elif size == b'large':
        pagination_css_classes.append(b'pagination-lg')
    return {b'propeller_pagination_url': url, 
       b'num_pages': num_pages, 
       b'current_page': current_page, 
       b'first_page': first_page, 
       b'last_page': last_page, 
       b'pages_shown': pages_shown, 
       b'pages_back': pages_back, 
       b'pages_forward': pages_forward, 
       b'pagination_css_classes': (b' ').join(pagination_css_classes), 
...
def pmd_muted_text(text, autoescape=True):
    ...
    **Usage**::
    ...
    if autoescape:
        ...
        esc = lambda x: x
    result = b'<span class="text-muted">%s</span>' % esc(text)
...
def pmd_display_text(text, size=1, autoescape=True):
    ...
    **Example**::
    ...
    if autoescape:
        ...
        esc = lambda x: x
    result = b'<span class="pmd-display%d">%s</span>' % (int(size), esc(text))
    return mark_safe(result)
...
def pmd_lead_text(text, autoescape=True):
    ...
    **Usage**::
    ...
    if autoescape:
        ...
        esc = lambda x: x
    result = b'<span class="lead">%s</span>' % esc(text)
    return mark_safe(result)
...
def pmd_bold_text(text, autoescape=True):
    ...
    **Usage**::
    ...
    if autoescape:
        ...
        esc = lambda x: x
    result = b'<strong>%s</strong>' % esc(text)
    return mark_safe(result)
