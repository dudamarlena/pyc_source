...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpPyUtils/enum.py
# Compiled at: 2020-01-16 14:56:47
# Size of source mod 2**32: 28666 bytes
"""
...
class _MetaEnumGroup(type):
    __doc__ = 'An EnumGroup metaclass.\n    '
    ...
    def __repr__(self):
        return '<{mdl}.{cls}({enums})>'.format(mdl=(self._clsDict.get('__module__', 'unknown')),
          cls=(self._clsName),
          enums=(self.join()))
...
class Enum(object):
    ...
    def __repr__(self):
        return '<{mdl}.{cls}.{name}>'.format(mdl=(self.__class__.__module__),
...
class EnumGroup(object):
    ...
    def __init_enums__(cls):
        enums = []
        orderedEnums = sorted([(k, v) for k, v in cls.__dict__.iteritems() if isinstance(v, Enum)], key=(lambda i: i[1]._creationOrder))
