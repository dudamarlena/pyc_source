...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/benchbase/bencher.py
# Compiled at: 2011-09-20 09:02:57
"""Extract information from an FunkLoad result file."""
import os, xml.etree.cElementTree as etree, logging, datetime
...
class Bencher(object):
            ...
            for (events, row) in tree:
                ...
                try:
                    try:
                        ...
                        data.insert(0, bid)
                        db.execute(INSERT_QUERY.format(table=table_name, columns=cols, values=values), data)
                        count += 1
                    except Exception, e:
                        logging.warning(e)
                        error += 1
    ...
    def doImport(self, filename):
        ...
        db = self.db
        logging.info(('Importing {0}: {1} into bid: {2}').format(self._name, filename, bid))
    ...
    def _get_interval_info(self):
        return "SELECT time(interval(?, ?, stamp), 'unixepoch', 'localtime'), COUNT(t), AVG(t)/1000, MAX(t)/1000., MIN(t)/1000.,  STDDEV(t)/1000,   MED(t)/1000, P10(t)/1000, P90(t)/1000, P95(t)/1000, P98(t)/1000, TOTAL(t)/1000, TOTAL(success),  AVG(na) FROM j_sample WHERE bid = ?"
    ...
    def getIntervalInfo(self, bid, start, period, sample, c=None):
        ...
        ret = [
         [
          'time', 'count', 'avg', 'max', 'min', 'stdev', 'med', 'p10', 'p90', 'p95', 'p98', 'total', 'success', 'threads', 'tput', 'error_rate']]
        t = [start, period, bid]
        ...
        c.execute(query, t)
        for row in c:
            error_rate = (row[1] - row[12]) * 100.0 / row[1]
    ...
    def _get_period_info_query(self):
        return 'SELECT COUNT(t), AVG(t), MAX(t), MIN(t),  STDDEV(t),  MED(t), P10(t), P90(t), P95(t), P98(t), TOTAL(t), TOTAL(success) FROM {table} WHERE bid = ? AND stamp >= ? AND stamp < ?'
    ...
    def getPeriodInfo(self, bid, start, period, sample, c=None, total=None):
        ...
        c.execute(query, t)
        row = c.fetchone()
        ret = {'name': sample, 'count': row[0], 'avgt': row[1], 
           'maxt': row[2], 'mint': row[3], 'stddevt': row[4], 
           'medt': row[5], 'p10t': row[6], 'p90t': row[7], 
           'p95t': row[8], 'p98t': row[9], 'total': row[10], 
           'success': row[11], 'tput': row[0] / float(period), 
           'filename': str2id(sample), 
           'title': sample | truncate(20)}
        ret['error'] = int(ret['count'] - ret['success'])
        ret['success_rate'] = 100.0
        if ret['count'] > 0:
            ret['success_rate'] = 100.0 * ret['success'] / ret['count']
        if total is None or not total:
            ret['percent'] = 100.0
        else:
            ret['percent'] = ret['total'] * 100.0 / total
        if close_cursor:
            c.close()
        return ret

    def _get_info_query(self):
        return ("SELECT COUNT(stamp),MIN(stamp), datetime(MIN(stamp), 'unixepoch', 'localtime'), time(MAX(stamp), 'unixepoch', 'localtime'), MAX({cvus}), MAX(stamp) - MIN(stamp) FROM {table} WHERE bid = ?").format(cvus=self._cvus, table=self._table)
    ...
    def getInfo(self, bid):
        ...
        c.close()
        return {'bid': bid, 'count': count, 'start': start, 'end': end, 'filename': os.path.basename(filename), 'start_stamp': start_stamp, 
           'imported': imported[:19], 'comment': comment, 'max_thread': max_thread, 
           'duration': duration, 'generator': generator, 'samples': samples, 
           'all_samples': all_samples, 'error': all_samples['error'], 'extra': extra}