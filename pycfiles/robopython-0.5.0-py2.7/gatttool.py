...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\robopython\pygatt\backends\gatttool\gatttool.py
# Compiled at: 2020-03-04 11:05:09
from __future__ import print_function
import functools, itertools, re, logging, platform, signal, sys, time, threading, subprocess
...
class GATTToolReceiver(threading.Thread):
    ...
    def __init__(self, connection, parent_aliveness):
        ...
        self._event_vector = {'notification': {'patterns': [
           ...
           'char_written': {'patterns': [
                                       'Characteristic value (was )?written successfully']}, 
           'value': {'patterns': [
                                'value: .*? \\r']}, 
           'value/descriptor': {'patterns': [
                                           'value/descriptor: .*? \\r']}, 
           'discover': {'patterns': [
                                   'handle: 0x([a-fA-F0-9]{4}), char properties: 0x[a-fA-F0-9]{2}, char value handle: 0x([a-fA-F0-9]{4}), uuid: ([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\r\n']}, 
           'connect': {'patterns': [
    ...
    def run(self):
        items = sorted(itertools.chain.from_iterable([ [ (pattern, event) for pattern in event['patterns'] ] for event in self._event_vector.values()
                                                     ]))
        ...
        while self._parent_aliveness.is_set():
            try:
                event_index = self._connection.expect(patterns, timeout=0.5)
...
class GATTToolBackend(BLEBackend):
    ...
    def __init__(self, hci_device='hci0', gatttool_logfile=None, cli_options=None, search_window_size=200):
        """
        Initialize.
        ...
        """
        if is_windows():
            raise BLEError('The GATTToolBackend requires BlueZ, which is not available in Windows')
    ...
    def start(self, reset_on_start=True, initialization_timeout=3):
        ...
        self._con = pexpect.spawn(gatttool_cmd, logfile=self._gatttool_logfile, searchwindowsize=self._search_window_size)
        self._con.expect('\\[LE\\]>', timeout=initialization_timeout)
        ...
        for event in ['notification', 'indication']:
            self._receiver.register_callback(event, self._handle_notification_string)
        ...
        try:
            try:
                ...
                for line in scan.before.decode('utf-8').split('\r\n'):
                    if 'sudo' in line:
                        raise BLEError("Enable passwordless sudo for 'hcitool' before scanning")
                    match = re.match('(([0-9A-Fa-f][0-9A-Fa-f]:?){6}) (\\(?.+\\)?)', line)
                    if match is not None:
                        ...
                        if address in devices:
                            if devices[address]['name'] is None and name is not None:
                                log.info('Discovered name of %s as %s', address, name)
                                devices[address]['name'] = name
                        else:
                            log.info('Discovered %s (%s)', address, name)
                            devices[address] = {'address': address, 
                               'name': name}
            ...
            try:
                ...
                log.error('Unable to gracefully stop the scan - BLE adapter may need to be reset.')

        return []

    def connect(self, address, timeout=DEFAULT_CONNECT_TIMEOUT_S, address_type=BLEAddressType.public):
        ...
        try:
            ...
            message = ('Timed out connecting to {0} after {1} seconds.').format(self._address, timeout)
            log.error(message)
    ...
    def clear_bond(self, address=None):
        ...
        try:
            con.expect([
             'Device has been removed', '# '], timeout=0.5)
        except pexpect.TIMEOUT:
    ...
    def _save_charecteristic_callback(self, event):
        ...
        try:
            ...
            self._characteristics[UUID(char_uuid)] = Characteristic(char_uuid, value_handle)
            log.debug('Found characteristic %s, value handle: 0x%x', char_uuid, value_handle)
    ...
    def discover_characteristics(self, timeout=5):
        self._characteristics = {}
        self._receiver.register_callback('discover', self._save_charecteristic_callback)
    ...
    @at_most_one_device
    def char_write_handle(self, handle, value, wait_for_response=False, timeout=1):
        """
        ...
        """
        cmd = ('char-write-{0} 0x{1:02x} {2}').format('req' if wait_for_response else 'cmd', handle, ('').join(('{0:02x}').format(byte) for byte in value))
