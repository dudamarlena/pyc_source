...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\robopython\pygatt\device.py
...
class BLEDevice(object):
    ...
    def char_write(self, uuid, value, wait_for_response=False):
        ...
        Example:
            my_ble_device.char_write('a1e8f5b1-696b-4e4c-87c6-69dfe0b0093b',
                                     bytearray([0x00, 0xFF]))
        """
        return self.char_write_handle(self.get_handle(uuid), value, wait_for_response=wait_for_response)
    ...
    def subscribe(self, uuid, callback=None, indication=False):
        ...
        properties = bytearray([
         2 if indication else 1,
         0])
        with self._lock:
            if callback is not None:
                self._callbacks[value_handle].add(callback)
            if self._subscribed_handlers.get(value_handle, None) != properties:
                self.char_write_handle(characteristic_config_handle, properties, wait_for_response=False)
    ...
    def receive_notification(self, handle, value):
        ...
        """
        log.info('Received notification on handle=0x%x, value=0x%s', handle, hexlify(value))
        with self._lock:
