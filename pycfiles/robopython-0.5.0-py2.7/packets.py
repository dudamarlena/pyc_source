...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\robopython\pygatt\backends\bgapi\packets.py
...
class BGAPICommandPacketBuilder(object):
    ...
    def system_endpoint_tx(endpoint, data):
        return pack(('<4BBB%dB' % len(data)), 0, (2 + len(data)), 0, 9, endpoint, len(data), *data)

    @staticmethod
    def system_whitelist_append(address, address_type):
        return pack('<4B6BB', 0, 7, 0, 10, address[0], address[1], address[2], address[3], address[4], address[5], address_type)

    @staticmethod
    def system_whitelist_remove(address, address_type):
        return pack('<4B6BB', 0, 7, 0, 11, address[0], address[1], address[2], address[3], address[4], address[5], address_type)
    ...
    def flash_ps_save(key, value):
        return pack(('<4BHB%dB' % len(value)), 0, (3 + len(value)), 1, 3, key, len(value), *value)
    ...
    def flash_write_words(address, words):
        return pack(('<4BHB%dB' % len(words)), 0, (3 + len(words)), 1, 7, address, len(words), *words)

    @staticmethod
    def attributes_write(handle, offset, value):
        return pack(('<4BHBB%dB' % len(value)), 0, (4 + len(value)), 2, 0, handle, offset, len(value), *value)
    ...
    def attributes_user_read_response(connection, att_error, value):
        return pack(('<4BBBB%dB' % len(value)), 0, (3 + len(value)), 2, 3, connection, att_error, len(value), *value)
    ...
    @staticmethod
    def connection_update(connection, interval_min, interval_max, latency, timeout):
        return pack('<4BBHHHH', 0, 9, 3, 2, connection, interval_min, interval_max, latency, timeout)
    ...
    def connection_channel_map_set(connection, channel_map):
        return pack(('<4BBB%dB' % len(channel_map)), 0, (2 + len(channel_map)), 3, 5, connection, len(channel_map), *channel_map)
    ...
    def connection_raw_tx(connection, data):
        return pack(('<4BBB%dB' % len(data)), 0, (2 + len(data)), 3, 8, connection, len(data), *data)

    @staticmethod
    def attclient_find_by_type_value(connection, start, end, uuid, value):
        return pack(('<4BBHHHB%dB' % len(value)), 0, (8 + len(value)), 4, 0, connection, start, end, uuid, len(value), *value)
    ...
    @staticmethod
    def attclient_read_by_type(connection, start, end, uuid=[3, 40]):
        return pack(('<4BBHHB%dB' % len(uuid)), 0, (6 + len(uuid)), 4, 2, connection, start, end, len(uuid), *uuid)
    ...
    def attclient_attribute_write(connection, atthandle, data):
        return pack(('<4BBHB%dB' % len(data)), 0, (4 + len(data)), 4, 5, connection, atthandle, len(data), *data)

    @staticmethod
    def attclient_write_command(connection, atthandle, data):
        return pack(('<4BBHB%dB' % len(data)), 0, (4 + len(data)), 4, 6, connection, atthandle, len(data), *data)
    ...
    def attclient_prepare_write(connection, atthandle, offset, data):
        return pack(('<4BBHHB%dB' % len(data)), 0, (6 + len(data)), 4, 9, connection, atthandle, offset, len(data), *data)
    ...
    def attclient_read_multiple(connection, handles):
        return pack(('<4BBB%dB' % len(handles)), 0, (2 + len(handles)), 4, 11, connection, len(handles), *handles)
    ...
    def sm_set_oob_data(oob):
        return pack(('<4BB%dB' % len(oob)), 0, (1 + len(oob)), 5, 6, len(oob), *oob)
    ...
    @staticmethod
    def gap_connect_direct(address, addr_type, conn_interval_min, conn_interval_max, timeout, latency):
        return pack('<4B6BBHHHH', 0, 15, 6, 3, address[(-1)], address[(-2)], address[(-3)], address[(-4)], address[(-5)], address[(-6)], addr_type, conn_interval_min, conn_interval_max, timeout, latency)
    ...
    @staticmethod
    def gap_connect_selective(conn_interval_min, conn_interval_max, timeout, latency):
        return pack('<4BHHHH', 0, 8, 6, 5, conn_interval_min, conn_interval_max, timeout, latency)

    @staticmethod
    def gap_set_filtering(scan_policy, adv_policy, scan_duplicate_filtering):
        return pack('<4BBBB', 0, 3, 6, 6, scan_policy, adv_policy, scan_duplicate_filtering)
    ...
    @staticmethod
    def gap_set_adv_parameters(adv_interval_min, adv_interval_max, adv_channels):
        return pack('<4BHHB', 0, 5, 6, 8, adv_interval_min, adv_interval_max, adv_channels)

    @staticmethod
    def gap_set_adv_data(set_scanrsp, adv_data):
        return pack(('<4BBB%dB' % len(adv_data)), 0, (2 + len(adv_data)), 6, 9, set_scanrsp, len(adv_data), *adv_data)

    @staticmethod
    def gap_set_directed_connectable_mode(address, addr_type):
        return pack('<4B6BB', 0, 7, 6, 10, address[0], address[1], address[2], address[3], address[4], address[5], addr_type)
    ...
    def hardware_adc_read(input, decimation, reference_selection):
        return pack('<4BBBB', 0, 3, 7, 2, input, decimation, reference_selection)
    ...
    @staticmethod
    def hardware_spi_config(channel, polarity, phase, bit_order, baud_e, baud_m):
        return pack('<4BBBBBBB', 0, 6, 7, 8, channel, polarity, phase, bit_order, baud_e, baud_m)

    @staticmethod
    def hardware_spi_transfer(channel, data):
        return pack(('<4BBB%dB' % len(data)), 0, (2 + len(data)), 7, 9, channel, len(data), *data)
    ...
    def hardware_i2c_write(address, stop, data):
        return pack(('<4BBBB%dB' % len(data)), 0, (3 + len(data)), 7, 11, address, stop, len(data), *data)
    ...
    def hardware_timer_comparator(timer, channel, mode, comparator_value):
        return pack('<4BBBBH', 0, 5, 7, 13, timer, channel, mode, comparator_value)
    ...
    @staticmethod
    def test_debug(data):
        return pack(('<4BB%dB' % len(data)), 0, (1 + len(data)), 8, 5, len(data), *data)