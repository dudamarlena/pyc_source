...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\robopython\robo\matrix.py
...
class Matrix(object):
    ...
    def set_display(self, rows, topic=None):
        packet_size = 11
        command_id = 82
        payload_size = 9
        module_id = self.id - 1
    ...
    def display_text(self, text='Test', orientation=0, repeats=1, scroll_rate=8, topic=None):
        ...
        if length > 28:
            ...
            if length < 11:
                command = [
                 packet_size, command_id1, payload_size, self.action_id, module_id, orientation, repeats, scroll_rate, length]
                for char in text:
