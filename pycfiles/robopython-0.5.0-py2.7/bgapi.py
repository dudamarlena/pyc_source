...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\robopython\pygatt\backends\bgapi\bgapi.py
# Compiled at: 2020-03-04 11:05:13
...
import logging, serial, time, threading
...
BLED112_VENDOR_ID = 9304
BLED112_PRODUCT_ID = 1
...
def bgapi_address_to_hex(address):
    address = hexlify(bytearray(list(reversed(address)))).upper().decode('ascii')
...
class BGAPIBackend(BLEBackend):
    ...
    def __init__(self, serial_port=None, receive_queue_timeout=0.1):
        ...
        self._packet_handlers = {ResponsePacketType.sm_get_bonds: self._ble_rsp_sm_get_bonds, 
           EventPacketType.attclient_attribute_value: self._ble_evt_attclient_attribute_value, 
           EventPacketType.attclient_find_information_found: self._ble_evt_attclient_find_information_found, 
           EventPacketType.connection_status: self._ble_evt_connection_status, 
           EventPacketType.connection_disconnected: self._ble_evt_connection_disconnected, 
    ...
    def _detect_device_port(self):
        ...
        detected_devices = find_usb_serial_devices(vendor_id=BLED112_VENDOR_ID, product_id=BLED112_PRODUCT_ID)
        if len(detected_devices) == 0:
            raise BGAPIError('Unable to auto-detect BLED112 serial port')
        log.info('Found BLED112 on serial port %s', detected_devices[0].port_name)
    ...
    def _open_serial_port(self, max_connection_attempts=MAX_CONNECTION_ATTEMPTS):
        ...
        for attempt in range(max_connection_attempts):
            log.debug('Opening connection to serial port (attempt %d)', attempt + 1)
            try:
                serial_port = self._serial_port or self._detect_device_port()
                self._ser = None
                self._ser = serial.Serial(serial_port, baudrate=115200, timeout=1)
                self._ser.read()
                break
            except (BGAPIError, serial.serialutil.SerialException, serial_exception):
                log.debug('Failed to open serial port', exc_info=True)
                if self._ser:
                    self._ser.close()
                elif attempt == 0:
                    raise NotConnectedError('No BGAPI compatible device detected')
                self._ser = None
                time.sleep(0.5)

        else:
            raise NotConnectedError('Unable to reconnect with USB device after rebooting')
    ...
    def set_bondable(self, bondable):
        self.send_command(CommandBuilder.sm_set_bondable_mode(constants.bondable[('yes' if bondable else 'no')]))
        self.expect(ResponsePacketType.sm_set_bondable_mode)

    def disable_advertising(self):
        log.info('Disabling advertising')
        self.send_command(CommandBuilder.gap_set_mode(constants.gap_discoverable_mode['non_discoverable'], constants.gap_connectable_mode['non_connectable']))
    ...
    def scan(self, timeout=10, scan_interval=75, scan_window=50, active=True, discover_mode=constants.gap_discover_mode['observation'], **kwargs):
        """
    ...
    def connect(self, address, timeout=5, address_type=BLEAddressType.public, interval_min=60, interval_max=76, supervision_timeout=100, latency=0):
        """
        ...
        log.info('Connecting to device at address %s (timeout %ds)', address, timeout)
        self.set_bondable(False)
        ...
        self.send_command(CommandBuilder.gap_connect_direct(address_bytes, addr_type, interval_min, interval_max, supervision_timeout, latency))
        try:
            self.expect(ResponsePacketType.gap_connect_direct)
            _, packet = self.expect(EventPacketType.connection_status, timeout=timeout)
            if self._connection_status_flag(packet['flags'], constants.connection_status_flag['connected']):
                device = BGAPIBLEDevice(bgapi_address_to_hex(packet['address']), packet['connection_handle'], self)
                if self._connection_status_flag(packet['flags'], constants.connection_status_flag['encrypted']):
    ...
    def discover_characteristics(self, connection_handle):
        att_handle_start = 1
        att_handle_end = 65535
        log.info('Fetching characteristics for connection %d', connection_handle)
        self.send_command(CommandBuilder.attclient_find_information(connection_handle, att_handle_start, att_handle_end))
        self.expect(ResponsePacketType.attclient_find_information)
        self.expect(EventPacketType.attclient_procedure_completed, timeout=10)
        for char_uuid_str, char_obj in self._characteristics[connection_handle].items():
            log.info('Characteristic 0x%s is handle 0x%x', char_uuid_str, char_obj.handle)
            for desc_uuid_str, desc_handle in char_obj.descriptors.items():
                log.info('Characteristic descriptor 0x%s is handle 0x%x', desc_uuid_str, desc_handle)
    ...
    def _scan_rsp_data(self, data):
        ...
        for b in data:
            if bytes_left_in_field == 0:
                ...
                if bytes_left_in_field == 0:
                    field_name = constants.scan_response_data_type[field_value[0]]
                    field_value = field_value[1:]
                    if field_name == 'complete_local_name' or field_name == 'shortened_local_name':
                        dev_name = bytearray(field_value).decode('utf-8')
                        data_dict[field_name] = dev_name
                    elif field_name == 'complete_list_128-bit_service_class_uuids':
                        if len(field_value) % 16 == 0:
                            data_dict[field_name] = []
                            for i in range(0, int(len(field_value) / 16)):
                                service_uuid = '0x%s' % bgapi_address_to_hex(field_value[i * 16:i * 16 + 16])
                                data_dict[field_name].append(service_uuid)

                        else:
                            log.warning('Expected a service class UUID of 16                                        bytes. Instead received %d bytes', len(field_value))
                    else:
    ...
    def expect_any(self, expected_packet_choices, timeout=None, assert_return_success=True):
        """
        ...
        log.debug('Expecting a response of one of %s within %fs', expected_packet_choices, timeout or 0)
        start_time = None
        ...
        while True:
            packet = None
            try:
                packet = self._receiver_queue.get(timeout=self._receive_queue_timeout)
            except queue.Empty:
                if timeout is not None:
                    if _timed_out(start_time, timeout):
                        exc = ExpectedResponseTimeout(expected_packet_choices, timeout)
                        exc.__cause__ = None
                        raise exc
                    continue
            ...
            log.debug('Received a %s packet: %s', packet_type, get_return_message(return_code))
        ...
        while self._running.is_set():
            ...
            if packet is not None:
                ...
                if packet_type == EventPacketType.attclient_attribute_value:
                    ...
                    device.receive_notification(args['atthandle'], bytearray(args['value']))
    ...
    def _ble_evt_attclient_find_information_found(self, args):
        ...
        if uuid_type != UUIDType.custom:
            uuid = uuid16_to_uuid(int(bgapi_address_to_hex(args['uuid']).replace(':', ''), 16))
        else:
            uuid = UUID(bytes=bytes(raw_uuid))
        if uuid_type == UUIDType.descriptor and self._current_characteristic is not None:
            self._current_characteristic.add_descriptor(uuid, args['chrhandle'])
        elif uuid_type == UUIDType.custom or uuid_type == UUIDType.nonstandard or uuid_type == UUIDType.characteristic:
            if uuid_type == UUIDType.custom:
            ...
            self._characteristics[args['connection_handle']][uuid] = new_char
        return
    ...
    def _ble_evt_connection_status(self, args):
        ...
        if not self._connection_status_flag(args['flags'], constants.connection_status_flag['connected']):
            self._connections.pop(connection_handle, None)
        log.info('Connection status: handle=0x%x, flags=%s, address=0x%s, connection interval=%fms, timeout=%d, latency=%d intervals, bonding=0x%x', connection_handle, args['address'], hexlify(bytearray(args['address'])), args['conn_interval'] * 1.25, args['timeout'] * 10, args['latency'], args['bonding'])
        return
    ...
    def _ble_evt_gap_scan_response(self, args):
        ...
        if dev.address == '':
            dev.address = address
        if packet_type not in dev.packet_data or len(dev.packet_data[packet_type]) < len(data_dict):
