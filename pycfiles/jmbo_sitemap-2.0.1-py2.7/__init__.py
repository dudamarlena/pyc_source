...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jmbo_sitemap/__init__.py
# Compiled at: 2015-06-09 05:01:15
import types
...
class BaseLinkSitemap(Sitemap):
    ...
    def items(self):
        ...
        for obj in self.get_containers():
            linkposition_set = getattr(obj, obj.__class__.__name__.lower() + 'linkposition_set')
            for o in linkposition_set.select_related().all().order_by('position'):
                if o.condition_expression_result(self.request) and o.link.id not in added:
...
if not sitemaps:
    ...
    if 'foundry' in settings.INSTALLED_APPS:
        sitemaps.update({'main-navbar': MainNavbarLinkSitemap, 
