...
# Embedded file name: build\bdist.win32\egg\pycoon\sitemap\manager.py
# Compiled at: 2007-03-10 11:02:28
...
class ComponentManager:
    ...
    def __init__(self):
        ...
        self.components = {'{%(map)s}generate' % ns: ({}, '{%(map)s}generator', '{%(map)s}generators' % ns), '{%(map)s}read' % ns: ({}, '{%(map)s}reader', '{%(map)s}readers' % ns), '{%(map)s}transform' % ns: ({}, '{%(map)s}transformer', '{%(map)s}transformers' % ns), '{%(map)s}serialize' % ns: ({}, '{%(map)s}serializer', '{%(map)s}serializers' % ns), '{%(map)s}match' % ns: ({}, '{%(map)s}matcher', '{%(map)s}matchers' % ns), '{%(map)s}select' % ns: ({}, '{%(map)s}selector', '{%(map)s}selectors' % ns), '{%(map)s}act' % ns: ({}, '{%(map)s}action', '{%(map)s}actions' % ns), '{%(map)s}pipeline' % ns: ({}, '{%(map)s}pipe', '{%(map)s}pipes' % ns), '{%(map)s}aggregate' % ns: ({'default': (self.classLoader.getClass('pycoon.components.cocoon.ContentAggregator'), None)}, None, None), 'input-module': ({}, None, None)}
        return

    def configure(self, element):
        for (classes, name, containterName) in self.components.values():
            if containterName is None:
                continue
