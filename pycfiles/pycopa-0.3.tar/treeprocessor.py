...
# Embedded file name: build\bdist.win32\egg\pycoon\sitemap\treeprocessor.py
# Compiled at: 2007-03-10 13:01:27
__author__ = 'Andrey Nordin <mailto:anrienord@inbox.ru>'
import logging, time, lxml.etree as etree
...
class TreeProcessor(Node):
    ...
    def configure(self, element):
        ...
        def getClassAndElement(e):
            return (
             self.parentComponentManager.classLoader.getClass(e.get('{%(py)s}class' % ns)), e)
...
class TreeBuilder:
    __module__ = __name__
    nodeClasses = {'pipelines': PipelinesNode, 'pipeline': PipelineNode, 'match': MatchNode, 'aggregate': AggregateNode, 'serialize': SerializeNode, 'call': ResourceCall, 'generate': GenerateNode, 'transform': TransformNode, 'select': SelectNode, 'read': ReadNode, 'handle-errors': HandleErrorsNode, 'mount': MountNode, 'act': ActNode, 'redirect-to': RedirectNode, 'component-configurations': ComponentConfigurations}
