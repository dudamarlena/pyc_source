...
# Embedded file name: build\bdist.win32\egg\pycoon\components\cocoon.py
# Compiled at: 2007-03-03 11:13:22
__author__ = 'Andrey Nordin <http://claimid.com/anrienord>'
import re, os, sys, mimetypes, traceback, logging, lxml.etree as etree
    ...
    def serialize(self, env, params):
        ...
        if env.isExternal:
            ...
            if self.doctypePublic is not None:
                response = [
                 '<?xml version="1.0" encoding="%s"?>' % self.encoding, '<!DOCTYPE html PUBLIC "%s" "%s">' % (self.doctypePublic, self.doctypeSystem), body]
                env.response.body = ('\n').join(response)
            else:
...
class ContentAggregator(Generator):
    ...
    def __str__(self):
        return ('\n').join(['<map:aggregate element="%s" ns="%s">' % (self.element, self.ns), ('\n').join([ '  <map:part src="%s"/>' % part for part in self.parts ]), '</map:aggregate>'])
...
class Pipeline(Component):
    ...
    def process(self, env):
        ...
        if self.generator is not None:
            if self.serializer is None:
                raise SitemapException('Serializer is not set')
    ...
    def __str__(self):
        return ('\n').join(['<map:pipeline>', ('\n').join([ '  %s' % line for line in str(self.generator).splitlines() ]), '  ...', '</map:pipeline>'])


class SendmailAction(Action):
    __module__ = __name__
    hmap = {'from': 'From', 'to': 'To', 'replyTo': 'Reply-To', 'cc': 'Cc', 'bcc': 'Bcc', 'subject': 'Subject'}
    ...
    def act(self, objectModel, params):
        ...
        msg = ('\r\n').join([ '%s: %s' % (self.hmap[k], v) for (k, v) in params.items() if k in self.hmap if v ] + ['Content-Type: text/plain; charset=%s; format=flowed' % charset, 'Content-Transfer-Encoding: 8bit', '\r\n%s' % params.get('body')])
        import smtplib
