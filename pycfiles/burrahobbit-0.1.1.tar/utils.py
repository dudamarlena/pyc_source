...
# Embedded file name: /home/ziirish/workspace/burp-ui/burpui/misc/parser/utils.py
...
class OptionInc(Option):
    ...
    def extend(self):
        ...
        for path in glob(root):
            if self.parser._is_secure_path(path) and os.path.isfile(path) and not path.endswith('~') and not path.endswith('.back'):
                paths.append(path)
...
class File(dict):
    ...
    def __init__(self, parser, name=None, mode='srv'):
        ...
        self.types = {'boolean': OrderedDict(), 
           'integer': OrderedDict(), 
           'include': OrderedDict(), 
           'multi': OrderedDict(), 
           'string': OrderedDict()}
    ...
    def flatten(self, typ, listed=True, parse=True):
        ...
        if parse:
            self.clean()
        if listed:
            return [ {'name': key, 'value': val.parse() if parse else val} for key, val in iteritems(self.types[typ])
                   ]
        ret = OrderedDict()
    ...
    @property
    def dirty(self):
        self._dirty = self._dirty or any([ x._dirty for _, x in iteritems(self.options) ])
        return self._dirty
    ...
    def _new_opt(self, key, value=None, typ=None):
        ...
        if typ == 'include':
            key = value
            opt = OptionInc(self.parser, key, value, root=self.name, mode=self.mode)
...
class Config(File):
    ...
    def _refresh(self):
        if self._dirty or any([ x.dirty for _, x in iteritems(self.files)
                              ]):
            self.options = OrderedDict()
            for key in viewkeys(self.types):
                self.types[key] = OrderedDict()
