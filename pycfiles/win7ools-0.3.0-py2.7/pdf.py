...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\win7ools\pdf.py
...
class PDF(object):
    ...
    def add_section(self, text, style='Heading3'):
        section = PDF.Item(_type='SECTION', value=text, order=len(self.story) + 1, data=Paragraph(text, self.styles[style]))
        self.story.append(section)
        return self

    def add_paragraph(self, text, align=4):
        paragraph = PDF.Item(_type='PARAGRAPH', value=text, order=len(self.story) + 1, data=Paragraph(text, self.styles['BodyText']))
        self.story.append(paragraph)
        self.add_spacer(10)
        return self

    def add_spacer(self, H):
        spacer = PDF.Item(_type='SPACER', order=len(self.story) + 1, data=Spacer(0, H))
    ...
    def save(self, savedir='', show=True, auto=False, boundary=0):
        ...
        doc = BaseDocTemplate(savepath, pagesize=self.pagesize, showBoundary=boundary)
        ...
        if self.two_column:
            frame_Top = Frame(doc.leftMargin, doc.bottomMargin + doc.height - preambleHeight, doc.width, preambleHeight, id='top')
            frame_L = Frame(doc.leftMargin, doc.bottomMargin, doc.width / 2 - 6, doc.height - preambleHeight, id='col1')
            frame_R = Frame(doc.leftMargin + doc.width / 2 + 6, doc.bottomMargin, doc.width / 2 - 6, doc.height - preambleHeight, id='col2')
            column_L = Frame(doc.leftMargin, doc.bottomMargin, doc.width / 2 - 6, doc.height, id='colLeft')
            column_R = Frame(doc.leftMargin + doc.width / 2 + 6, doc.bottomMargin, doc.width / 2 - 6, doc.height, id='colRight')
            firstTemplate = PageTemplate(id='FirstPage', frames=[
             frame_Top, frame_L, frame_R])
            laterTemplate = PageTemplate(id='LaterPages', frames=[
             column_L, column_R])
        else:
            frame_Top = Frame(doc.leftMargin, doc.bottomMargin + doc.height - preambleHeight, doc.width, preambleHeight, id='top')
            frame_Bottom = Frame(doc.leftMargin, doc.bottomMargin, doc.width * 2, doc.height - preambleHeight, id='bottom')
            column = Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height, id='column')
            firstTemplate = PageTemplate(id='FirstPage', frames=[
...
class Checklist(PDF):
    ...
    ***Intended for use with letter size paper***    
    ...
    class Todo(PDF.Item):
        ...
        def __init__(self, _type='TODO', value='', order='', data='', complete=False, color='black'):
    ...
    def __init__(self, items=[], title='', pretext=''):
        ...
        FRAMEBREAK = PDF.Item(_type='FRAMEBREAK', order=len(self.story) + 1, data=FrameBreak())
        self.story.append(FRAMEBREAK)
        NEXTPAGETEMPLATE = PDF.Item(_type='TEMPLATE', order=len(self.story) + 1, data=NextPageTemplate('LaterPages'))
    ...
    def add(self, *items, **attr):
        ...
        if complete:
            row_color = 'gray'
        for item in items:
            TODO = Checklist.Todo(value=item, order=len(self.story) + 1, color=row_color)
