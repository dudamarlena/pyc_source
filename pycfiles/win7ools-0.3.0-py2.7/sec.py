...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\win7ools\sec.py
...
import hashlib, hmac, os.path as path, string
...
class BLOB(Structure):
    ...
    _fields_ = [
     (
      'cbData', DWORD), ('pbData', POINTER(c_char))]
...
def crypt_protect_data(ptext, entropy=None, description=''):
    """
    ...
    blob_out = BLOB()
    if CryptProtectData(byref(blob_in), description, blob_entropy, None, None, CRYPTPROTECT_UI_FORBIDDEN, byref(blob_out)):
        ctext = blob_out.data()
    return ctext
...
def crypt_unprotect_data(ctext, entropy=None):
    ...
    if entropy:
        ...
        blob_entropy = entropy
    blob_out = BLOB()
    if CryptUnprotectData(byref(blob_in), None, blob_entropy, None, None, CRYPTPROTECT_UI_FORBIDDEN, byref(blob_out)):
        ptext = blob_out.data()
    return ptext
