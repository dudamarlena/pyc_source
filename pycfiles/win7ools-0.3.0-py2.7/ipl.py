...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\win7ools\ipl.py
...
class IPL(object):
    ...
    def __init__(self, data=None, load=False, keys=RegistryKeys()):
        ...
        if data is not None:
            try:
                self.IPL = {'Programs': data['Programs'], 'Modules': data['Modules']}
            except:
    ...
    def get(cls, keys=RegistryKeys()):
        ...
        temp = []
        try:
            temp = get_values(keys.installer, HKCR=True) + get_values(keys.uninstall) + get_values(keys.uninstall, HKCU=True) + get_values(keys.uninstall_wow6432)
            ...
            for i in temp:
                ...
                data = {'category': Category, 'optional': Optional, 
    ...
    def add_program(self, name='', category='', optional=False, silent_install=None, silent_install_tag=None, backup_required=False, version='', date='', location='', GUID='', data='', replace=False, update=False):
        """
        ...
        modified = False
        if name:
            data = {'category': category, 'optional': optional, 'silent_install': silent_install, 
    ...
    def load(cls, date='', path=''):
        ...
        try:
            try:
                ...
                if not date:
                    date = max(dates)
                c.execute('SELECT data FROM ipl WHERE date=?', date)
                ipl_data = c.fetchone()
            except:
