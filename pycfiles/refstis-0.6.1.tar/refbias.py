...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/refbias.py
...
def flag_hot_pixels(refbias_name):
    ...
    with fits.open(refbias_name, mode='update') as (refbias_hdu):
        ...
        bias_mean, bias_median, bias_std = sigma_clipped_stats(refbias_hdu[('sci',
                                                                            1)].data, sigma=3, iters=30)
        smooth_bias += bias_mean - smooth_bias_mean
        bias_residual = refbias_hdu[('sci', 1)].data - smooth_bias
        resid_mean, resid_median, resid_std = sigma_clipped_stats(bias_residual, sigma=3, iters=30)
        r_five_sigma = resid_mean + 5.0 * resid_std
        print (
         'Updating DQ values of hot pixels above a level of ', r_five_sigma)
        refbias_hdu[('dq', 1)].data = np.where(bias_residual > r_five_sigma, 16, refbias_hdu[('dq',
                                                                                              1)].data)
