...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/pipeline.py
...
import argparse, textwrap, time, shutil, re, numpy as np, yaml, getpass
...
dark_proposals = [
 11844, 11845, 12400, 12401, 12741, 12742, 13131, 13132, 13518, 13519, 13980, 13981, 14412, 14413]
...
def get_new_periods(products_directory, settings):
    ...
    for i in range(len(table_id_all))[::-1]:
        ...
        print '#--------------------------------#'
        products_folder = os.path.join(products_directory, '%d_%s' % (proposal, visit))
        ...
        new_obs = get_new_obs('DARK', ref_begin, ref_end, settings) + get_new_obs('BIAS', ref_begin, ref_end, settings)
        obs_to_get = [ obs for obs in new_obs if obs not in already_retrieved ]
...
def split_files(all_files):
    ...
    """
    all_info = [ (fits.getval(filename, 'EXPSTART', 1), filename) for filename in all_files
               ]
    all_info.sort()
...
def get_anneal_month(proposal_id, anneal_id):
    ...
    c = db.cursor()
    table = 'anneals'
    c.execute(('SELECT id,start FROM {} WHERE proposid={} AND visit={}').format(table, proposal_id, anneal_id))
    all_info = [ row for row in c ]
    if len(all_info) > 1:
...
def make_pipeline_reffiles(root_folder, last_basedark=None, last_basebias=None):
    ...
    bias_threshold = {(1, 1, 1): 98, (1, 1, 2): 25, 
       (1, 2, 1): 25, 
       (1, 2, 2): 7, 
       (1, 4, 1): 7, 
       (1, 4, 2): 4, 
    ...
    for folder in sorted(bias_folders):
        ...
        weekbias_name = os.path.join(folder, 'weekbias_%s_%s_%s_bia.fits' % (proposal, visit, wk))
    ...
    for folder in sorted(dark_folders):
        ...
        proposal, wk, visit = pull_info(folder)
        weekbias_name = os.path.join(root_folder, 'biases/1-1x1', wk, 'weekbias_%s_%s_%s_bia.fits' % (proposal, visit, wk))
        raw_files = glob.glob(os.path.join(folder, '*raw.fits'))
    ...
    for folder in sorted(dark_folders):
        ...
        ybin = functions.get_keyword(raw_files, 'BINAXIS2', 0)
        proposal, wk, visit = pull_info(folder)
        weekdark_name = os.path.join(folder, 'weekdark_%s_%s_%s_drk.fits' % (proposal, visit, wk))
        ...
        weekdark.make_weekdark(raw_files, weekdark_name, basedark_name, weekbias_name)
...
def get_new_obs(file_type, start, end, settings):
    if file_type == 'DARK':
        ...
        if file_type == 'BIAS':
            ...
            print (
             'file type not recognized: ', file_type)
    ...
    for file_type, mode in zip(['BIAS', 'DARK', 'BIAS'], [
     'WK', 'WK', 'BIWK']):
        ...
        if not len(obs_list):
            ...
            print (
             file_type, mode, len(obs_list), 'files to move, ', 'gain = ', gain)
        ...
        for period in range(N_periods):
            ...
            if file_type == 'BIAS':
                output_path = os.path.join(output_path, 'biases/%d-1x1/%s%s/' % (gain,
                 mode.lower(),
                 week))
            else:
                if file_type == 'DARK':
                ...
                obs_to_move = [ item for item in obs_list if begin <= fits.getval(item, 'EXPSTART', ext=1) <= end
                              ]
                if not len(obs_to_move):
                    raise ValueError('error, empty list to move')
                for item in obs_to_move:
                    print (
                     'Moving ', item, ' to:', output_path)
                    shutil.move(item, output_path)
                    if 'IMPHTTAB' not in fits.getheader(os.path.join(output_path, item.split('/')[(-1)]), 0):
                        fits.setval(os.path.join(output_path, item.split('/')[(-1)]), 'IMPHTTAB', ext=0, value='oref$x9r1607mo_imp.fits')
        ...
        for period in range(N_periods):
            ...
            if file_type == 'BIAS':
                ...
                if not os.path.exists(output_path):
                    os.makedirs(output_path)
                print ('week goes from: ', begin, end)
                obs_to_move = [ item for item in obs_list if fits.getval(item, 'EXPSTART', ext=1) >= begin and fits.getval(item, 'EXPSTART', ext=1) < end
                              ]
                if not len(obs_to_move):
...
def move_obs(new_obs, base_output_dir, retrieve_directory):
    if not len(new_obs) > 0:
        raise AssertionError('Empty list of new observations to move.')
        os.path.exists(base_output_dir) or os.makedirs(base_output_dir)
...
def parse_args():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, epilog=textwrap.dedent('Description\n------------------------------------\n  Automated script for producing\n  dark and bias reference files for\n  STIS CCD data.\n\nLocations\n------------------------------------\n  Product directory = EMPTY\n\nProcedure\n------------------------------------\n1: pass\n\n2: pass\n\n------------------------------------\n'))
    parser.add_argument('-r', '--redo_all', action='store_true', dest='redo_all', default=False, help='Re-run analysis on all past anneal months.')
    parser.add_argument('-c', '--no_collect', action='store_false', dest='collect_new', default=True, help='Turn off data collection function.')
    parser.add_argument('-p', '--plots_only', action='store_true', dest='plots_only', default=False, help='Only remake plots and update the website.')
    parser.add_argument('-u', '--user_information', action='store', dest='user_information', default=None, help='info string needed to request data')
