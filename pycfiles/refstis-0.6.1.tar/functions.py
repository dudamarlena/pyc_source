...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/functions.py
...
def update_header_from_input(filename, input_list):
    ...
    if targname == 'BIAS':
        ...
        if gain == 1:
            ...
            hdu_out[0].header.add_history(('Super{} image, combination of {} input {} frames taken in').format(targname.lower(), nimsets, targname.lower()))
            hdu_out[0].header.add_history(('CCDGAIN={}, BINAXIS1={}, BINAXIS2={} mode.').format(hdu_out[0].header['CCDGAIN'], hdu_out[0].header['BINAXIS1'], hdu_out[0].header['BINAXIS2']))
...
def make_resicols_image(residual_image, yfrac=1):
    ...
    ystart = 0
    yend = min(1024, int(np.floor(yfrac * residual_image.shape[0] + 0.5)))
...
def crreject(input_file, workdir=None):
    ...
    if crcorr != 'COMPLETE':
        ...
        if blevcorr != 'COMPLETE':
            ...
            status = basic2d(input_file, output_blev, outblev='', dqicorr='perform', blevcorr='perform', doppcorr='omit', lorscorr='omit', glincorr='omit', lflgcorr='omit', biascorr='omit', darkcorr='omit', flatcorr='omit', photcorr='omit', statflag=False, verbose=False, trailer=trailerfile)
            if status != 0:
                try:
                    print ()
        ...
        status = ocrreject(input=output_blev, output=output_crj, verbose=False, trailer=trailerfile)
        if status != 0:
            try:
                print ()
                with open(trailerfile) as (tr):
...
def get_anneal_month_dates(data_begin, data_end, database_path):
    ...
    anneal_period_start_indx = start_indx[0][(-1)]
    end_indx = np.where(anneal_start_date - data_end > 0)
    anneal_period_end_indx = end_indx[0][0]
...
def divide_anneal_month(data_begin, data_end, database_path, N_period):
    ...
    total_num_days = anneal_month_end.val - anneal_month_start.val
    ...
    if mode == 'WK':
        for n in range(2, number_of_days):
            if number_of_days < 12:
                if number_of_days > MAX_DAYS_IN_WK or number_of_days < MIN_DAYS_IN_WK:
                    number_of_periods = -1
                    break
                else:
...
def bd_calstis(joinedfile, thebiasfile=None):
    ...
    status = calstis(joinedfile, wavecal='', outroot='', savetmp=False, verbose=False, trailer=trailerfile)
    if status != 0:
        try:
            print ()
            with open(trailerfile) as (tr):
...
def refaver(reffiles, combined_name):
    ...
    if not len(all_subfiles) == 2:
        raise AssertionError(('Length of subfiles doesnt equal 2: {}').format(all_subfiles))
        combined_name = combined_name.endswith('.fits') or combined_name + '.fits'
    ...
    with pyfits.open(filename, mode='update') as (ofile):
        if 'tempcorr' not in ofile[0].header:
            ...
            for ext in np.arange(1, nextend, 3):
                ...
                ofile[(ext + 1)].data = np.sqrt(ofile[(ext + 1)].data ** 2 * factor ** 2)
...
def bias_subtract_data(filename, biasfile):
    ...
    status = basic2d(filename, dqicorr='perform', blevcorr='perform', biascorr='perform', doppcorr='omit', lorscorr='omit', glincorr='omit', lflgcorr='omit', darkcorr='omit', flatcorr='omit', photcorr='omit', verbose=False, trailer=trailerfile)
    if status != 0:
        try:
            print ()
            with open(trailerfile) as (tr):
