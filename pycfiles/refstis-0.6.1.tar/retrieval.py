...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/retrieval.py
...
def build_xml_request(datasets, settings):
    ...
    request_string = REQUEST_TEMPLATE.safe_substitute(archive_user=settings['archive_user'], email=settings['email'], ftp_host=settings['host'], ftp_dir=settings['retrieve_directory'], ftp_user=settings['ftp_user'], datasets=datasets, suffix='<suffix name="RAW" />')
...
def submit_xml_request(xml_request, settings):
    ...
    params = urlencode({'dadshost': settings['dads_host'], 'dadsport': 4703, 
       'mission': 'HST', 
