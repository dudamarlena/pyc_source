...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/delivery.py
...
import matplotlib.pyplot as plt, numpy as np, os, glob, shutil, sys
...
def set_descrip(folder):
    ...
    for item in glob.glob(os.path.join(folder, '*.fits')):
        ...
        if '_drk' in item:
            ...
            if '_bia' in item and hdu[0].header['CCDGAIN'] == 1:
...
def regress(folder):
    ...
    reference_files = glob.glob(os.path.join(folder, '*bia.fits')) + glob.glob(os.path.join(folder, '*drk.fits'))
    ...
    for testing_dir in [test_suite, test_dark]:
        for oldfile in glob.glob(os.path.join(testing_dir, '*_drk.fits')) + glob.glob(os.path.join(testing_dir, '*_bia.fits')):
            print (
             'removing', oldfile)
...
def send_forms(folder):
    ...
    for search_string in ('*drk.fits', 'bias_wk*.fits', 'bias_bi*.fits'):
        ...
        for i, name in enumerate(file_list):
            if name != file_list[(-1)]:
                add_str = name + ' is for dates ' + USEAFTER[i] + ' to ' + USEAFTER[(i + 1)] + ' \n '
            else:
...
def remove_products():
    ext_list = [
     '*_crj*', '*_flt*', '*_sx1*', '*_sx2*', '*_x1d*', '*_x2d*', '*_tmp*']
    for ext in ext_list:
...
def move(source, destination):
    ...
    for root, dirs, files in os.walk(source):
        for filename in files:
            ...
            elif 'biases/1-1x1/' in full_path and 'weekbias_' in filename and 'grp' not in filename:
                full_destination = os.path.join(destination, filename)
