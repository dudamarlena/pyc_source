...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/basejoint.py
...
import numpy as np, os, shutil, sys, stistools
    ...
    with fits.open(input_file, mode='update') as (hdu):
        ...
        if crcorr != 'COMPLETE':
            ...
            if blevcorr != 'COMPLETE':
                ...
                stistools.basic2d.basic2d(input=input_file, output=output_blev, outblev='', dqicorr='perform', atodcorr='omit', blevcorr='perform', doppcorr='omit', lorscorr='omit', glincorr='omit', lflgcorr='omit', biascorr='omit', darkcorr='omit', flatcorr='omit', shadcorr='omit', photcorr='omit', statflag=False, verbose=False, trailer='/dev/null')
            else:
                shutil.copy(input_file, output_blev)
            stistools.ocrreject.ocrreject(input=output_blev, output=output_crj, verbose=False, trailer='/dev/null')
...
def replace_hot_pix(mean_bias, median_image):
    ...
    resi_mean, resi_median, resi_std = sigma_clipped_stats(residual_image, sigma=5, iters=40)
...
def make_basebias(input_list, refbias_name='basebias.fits'):
    ...
    replace_hot_cols(mean_bias, median_image, residual_image, yfrac=0.2)
