...
# [GCC 8.4.0]
# Embedded file name: /Users/ely/codebase/refstis/refstis/weekdark.py
...
def create_superdark(crj_filename, basedark):
    ...
    with fits.open(crj_filename, mode='update') as (crj_hdu):
        data_mean, data_median, data_std = sigma_clipped_stats(crj_hdu[('sci', 1)].data, sigma=5, iters=40)
        p_five_sigma = data_median + 5 * data_std
        print ('hot pixels are defined as above: ', p_five_sigma)
        basedark_hdu = fits.open(basedark)
        base_mean, base_median, base_std = sigma_clipped_stats(basedark_hdu[('sci',
                                                                             1)].data, sigma=5, iters=40)
        ...
        only_dark = np.where(basedark_hdu[('sci', 1)].data >= p_five_sigma, basedark_med, basedark_hdu[('sci',
                                                                                                        1)].data)
        crj_hdu[('sci', 1)].data = only_dark + only_hotpix
        crj_hdu[('dq', 1)].data = np.where(only_hotpix >= p_five_sigma, 16, crj_hdu[('dq',
                                                                                     1)].data)
        crj_hdu[('err', 1)].data = np.where(only_hotpix == 0, basedark_hdu[('err',
                                                                            1)].data, crj_hdu[('err',
                                                                                               1)].data)
