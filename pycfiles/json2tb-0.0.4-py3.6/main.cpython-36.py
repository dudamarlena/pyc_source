...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/json2tb/main.py
# Compiled at: 2018-11-22 09:32:56
# Size of source mod 2**32: 4294 bytes
import sys, argparse, codecs, json, numpy as np, tensorboardX
...
class TBWriter(object):
    ...
    def _is_writable(self, key, val):
        if isinstance(key, str):
            if self._is_scalar(val) or self._is_list(val):
                return True
        return False
...
def line2dict(line, ignore_broken_line=False):
    ...
    try:
        ...
        if ignore_broken_line:
            print('Broken line is found and ignored.')
            return
        raise e
