...
# [GCC 8.4.0]
# Embedded file name: /Users/mortonc/Projects/RefET-GEE/geerefet/daily.py
# Compiled at: 2020-01-13 18:08:20
# Size of source mod 2**32: 18153 bytes
import math, ee
from . import calcs
...
class Daily:
    ...
    def __init__(self, tmax, tmin, ea, rs, uz, zw, elev, lat, doy, method='asce', rso_type=None, rso=None):
        ...
        if rso_type is None:
            ...
            if rso_type.lower() not in ('simple', 'full', 'array'):
                ...
                self.es = calcs._sat_vapor_pressure(self.tmax).add(calcs._sat_vapor_pressure(self.tmin)).multiply(0.5)
                ...
                if rso_type is None:
                    if method.lower() == 'asce':
                        ...
                        if method.lower() == 'refet':
                            self.rso = calcs._rso_daily(ea=(self.ea),
                              ra=(self.ra),
                              pair=(self.pair),
                              doy=(self.doy),
                              lat=(self.lat))
            ...
            self.rnl = calcs._rnl_daily(tmax=(self.tmax),
              tmin=(self.tmin),
              ea=(self.ea),
              fcd=(self.fcd))
    ...
    def _etsz(self):
        ...
        """
        return self.tmean.add(273).pow(-1).multiply(self.u2).multiply(self.vpd).multiply(self.cn).multiply(self.psy).add(self.es_slope.multiply(self.rn).multiply(0.408)).divide(self.u2.multiply(self.cd).add(1).multiply(self.psy).add(self.es_slope))
    ...
    def gridmet(cls, input_img, zw=None, elev=None, lat=None, method='asce', rso_type=None):
        ...
        if zw is None:
            zw = ee.Number(10)
        if elev is None:
            elev = ee.Image('projects/earthengine-legacy/assets/projects/climate-engine/gridmet/elevation')
        if lat is None:
            lat = ee.Image('projects/earthengine-legacy/assets/projects/climate-engine/gridmet/elevation').multiply(0).add(ee.Image.pixelLonLat().select('latitude'))
        return cls(tmax=(input_img.select(['tmmx']).subtract(273.15)),
    ...
    def nldas(cls, input_coll, zw=None, elev=None, lat=None, method='asce', rso_type=None):
        ...
        def wind_magnitude(input_img):
            ...
            return ee.Image(input_img.select(['wind_u'])).pow(2).add(ee.Image(input_img.select(['wind_v'])).pow(2)).sqrt().rename([
             'uz'])

        wind_img = ee.Image(ee.ImageCollection(input_coll.map(wind_magnitude)).mean())
        ea_img = calcs._actual_vapor_pressure(pair=(calcs._air_pressure(elev, method)),
          q=(input_coll.select(['specific_humidity']).mean()))
        return cls(tmax=(input_coll.select(['temperature']).max()),
    ...
    def cfsv2(cls, input_coll, zw=None, elev=None, lat=None, method='asce', rso_type=None):
        ...
        cfsv2_crs = 'EPSG:4326'
        cfsv2_transform = [
         0.20454520376789903, 0.0, -180,
         0.0, -0.20442210122586923, 90]
        if zw is None:
            zw = ee.Number(10)
        if elev is None:
        ...
        return cls(tmax=(input_coll.select([
         'Maximum_temperature_height_above_ground_6_Hour_Interval']).max().subtract(273.15)),
          tmin=(input_coll.select([
         'Minimum_temperature_height_above_ground_6_Hour_Interval']).min().subtract(273.15)),
          ea=ea_img,
          rs=(input_coll.select([
         'Downward_Short-Wave_Radiation_Flux_surface_6_Hour_Average']).mean().multiply(0.0864)),
          uz=wind_img,
          zw=zw,
          elev=elev,
          lat=lat,
