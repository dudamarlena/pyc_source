...
# [GCC 8.4.0]
# Embedded file name: /Users/mortonc/Projects/RefET-GEE/geerefet/calcs.py
# Compiled at: 2018-05-11 10:16:20
# Size of source mod 2**32: 22333 bytes
import math, ee
...
def _air_pressure(elev, method='asce'):
    ...
    if method == 'refet':
        return elev.multiply(-0.0065).add(293).divide(293).pow(5.255114352360781).multiply(101.3)
...
def _seasonal_correction(doy):
    ...
    return b.multiply(2).sin().multiply(0.1645).subtract(b.cos().multiply(0.1255)).subtract(b.sin().multiply(0.025))
...
def _ra_daily(lat, doy, method='asce'):
    ...
    if method == 'asce':
        ra = theta.multiply(_dr(doy)).multiply(24.0 / math.pi * 4.92)
    else:
        ra = theta.multiply(_dr(doy)).multiply(24.0 / math.pi * 4.9212)
    return ra
...
def _ra_hourly(lat, lon, doy, time_mid, method='asce'):
    ...
    if method == 'asce':
        ra = theta.multiply(_dr(doy)).multiply(12.0 / math.pi * 4.92)
    else:
        ra = theta.multiply(_dr(doy)).multiply(12.0 / math.pi * 4.9212)
    return ra
...
def _rso_simple(ra, elev):
    ...
    """
    return ra.multiply(elev.multiply(2e-05).add(0.75))
...
def _rnl_daily(tmax, tmin, ea, fcd):
    ...
    """
    return tmax.add(273.16).pow(4).add(tmin.add(273.16).pow(4)).multiply(0.5).multiply(ea.sqrt().multiply(-0.14).add(0.34)).multiply(fcd).multiply(4.901e-09)
