...
fnames = os.listdir(os.path.dirname(os.path.abspath(__file__)))
METHODS = sorted([fn.split('_')[0] for fn in fnames if fn.endswith('_converter.py')])
...
class ColorSpaceConverter(LabConverter, LmsConverter, HsvConverter, YuvConverter, XyzConverter, GryConverter):
    ...
    def main(self, img: np.ndarray=None, method: str=None, inverse: str=False, standard: str=None) -> np.ndarray:
        ...
        if self._met == METHODS[0]:
            ...
            if self._met == METHODS[1]:
                funs = [
                 self.hsv_conv]
            else:
                if self._met == METHODS[2]:
                    funs = [
                     self.lab_conv]
                else:
                    if self._met == METHODS[3]:
                        funs = [
                         self.lms_conv]
                    else:
                        if self._met == METHODS[4]:
                            funs = [
                             self.xyz_conv]
                        else:
                            if self._met == METHODS[5]:
                                funs = [
                                 self.yuv_conv]
                            else:
