...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/frla/controllers/tarihkisi.py
# Compiled at: 2009-03-11 11:05:57
import logging
...
class TarihkisiController(BaseController):
    ...
    def index(self):
        ...
        c.sonuc = [
         _('bilgikisi3') + request.params['tarih'] + _('tarihbilgi2') + request.params['isim'] + _('bilgikisi2') + ungettext('%(num)d time', '%(num)d times', int(baglanmasayisi[0][0])) % {'num': int(baglanmasayisi[0][0])}]
        saat = 0
        for i in bilgiler:
            ...
            if len(ken_ad) == 0:
                ken_ad = [
                 (
                  _('isimverme1') + i[1] + _('isimverme2'),)]
            c.sonuc[(len(c.sonuc)):] = [
             _('saat') + i[0][i[0].find(' '):] + g.bqac + _('kenadres') + i[1] + _('ad') + ken_ad[0][0] + g.bqkapa]
            saat = i[0][i[0].find(' '):]
