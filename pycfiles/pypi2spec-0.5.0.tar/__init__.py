...
# [GCC 8.4.0]
# Embedded file name: /home/pierrey/repos/gitrepo/pypi2spec/pypi2spec/__init__.py
# Compiled at: 2016-02-05 11:11:37
"""
...
"""
import ConfigParser, argparse, logging, os, shutil, tarfile, urllib2
...
def get_rpm_tag(tag):
    u"""" Reads the .rpmmacros and set the values accordingly
...
class Settings(object):
    ...
    def __init__(self):
        ...
        """
        self._dict = {'packager': self.packager, 
           'email': self.email}
...
class Pypi2spec(object):
    ...
    def determine_arch(self):
        ...
        if os.path.exists(self.tardir):
            for root, dirs, files in os.walk(self.tardir):
                for entry in files:
                    if '.' in entry:
                        ...
                        if extension in extensions or 'f' in extension or 'F' in extension:
                            self.arch = True
            ...
            return
        self.log.info('Could not find the extracted source to search the arch')
    ...
    def download(self, force=False):
        ...
        if not force and os.path.exists(sources) and os.path.isfile(sources):
            self.log.info('Sources are already present, no need to re-download')
    ...
    def remove_sources(self):
        ...
        try:
            ...
            self.log.info('Could not remove the extracted sources: "%s"' % self.tardir)
            self.log.debug('ERROR: %s' % err)
    ...
    def retrieve_info(self):
        ...
        try:
            ...
            raise Pypi2specError('Could not retrieve information for theproject "%s". Did you make a typo?' % self.name)

        self.version = data['info']['version']
        self.summary = data['info']['summary']
        self.description = data['info']['description']
        self.source0 = [ url['url'] for url in data['urls'] if url['url'].endswith('tar.gz') or url['url'].endswith('zip')
                       ][0]
        LOG.debug('self.source0 = %s', self.source0)
        if not self.source0 or os.path.splitext(self.source0)[1] not in ('.gz', '.zip',
                                                                         '.bz2'):
            ...
            if url_ext is not False:
                ...
                raise Pypi2specError('No tarball or zip file could be found for this package: %s' % self.name)
        else:
...
class Pypi2specUI(object):
    ...
    def setup_parser(self):
        """ Command line parser. """
        self.parser = argparse.ArgumentParser(usage='%(prog)s [options]', prog='pypi2spec')
        self.parser.add_argument('--version', action='version', version='%(prog)s ' + __version__)
        self.parser.add_argument('package', help='Name of the pypi library to package.')
        self.parser.add_argument('--python3', action='store_true', help='Create a specfile for both ' + 'python2 and python3.')
        self.parser.add_argument('--verbose', action='store_true', help='Give more info about what is ' + 'going on.')
        self.parser.add_argument('--debug', action='store_true', help='Output bunches of debugging info.')
