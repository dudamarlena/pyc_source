...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/kismetexternal/__init__.py
# Compiled at: 2020-04-21 13:36:12
# Size of source mod 2**32: 38019 bytes
"""
...
import asyncio, errno, fcntl, os, select, signal, socket, struct, sys, threading, traceback, time, google.protobuf
if '__version__' not in dir(google.protobuf):
...
class ExternalInterface(object):
    ...
    def __init__(self, infd=-1, outfd=-1, remote=None):
        ...
        self.errorcb = None
        self.handlers = {}
        self.add_handler('HTTPAUTH', self._ExternalInterface__handle_http_auth)
        self.add_handler('HTTPREQUEST', self._ExternalInterface__handle_http_request)
        self.add_handler('PING', self._ExternalInterface__handle_ping)
        self.add_handler('PONG', self._ExternalInterface__handle_pong)
        self.add_handler('SHUTDOWN', self._ExternalInterface__handle_shutdown)
        self.uri_handlers = {}
    ...
    def adler32(data):
        ...
        if len(data) < 4:
            ...
            return (s1 & 65535) + (s2 << 16) & 4294967295

    async def __io_loop(self):
        try:
            try:
                while self.kill_ioloop or not self.last_pong == 0:
                    if time.time() - self.last_pont > 5:
                    ...
                    self._ExternalInterface__recv_packet()
    ...
    async def __asyncio_connect(self):
        ...
        if self.infd is not None and self.infd >= 0 and self.outfd is not None and self.outfd >= 0:
            ...
            self.ext_reader, self.ext_writer = await self._ExternalInterface__async_open_fds()
            if self.debug:
            ...
            if self.remote is not None:
                ...
                self.ext_reader, self.ext_writer = await self._ExternalInterface__async_open_remote(self.remote)
            else:
    ...
    def run(self):
        ...
        try:
            ...
            self.main_io_task = self.loop.create_task(self._ExternalInterface__io_loop())
            if self.debug:
    ...
    def start(self):
        ...
        """
        try:
            self.loop.run_until_complete(self._ExternalInterface__asyncio_connect())
        except RuntimeError:
    ...
    def __handle_ping(self, seqno, packet):
        ...
        self._ExternalInterface__send_pong(seqno)
...
class Datasource(ExternalInterface):
    ...
    def __init__(self, infd=-1, outfd=-1, remote=None):
        ...
        self.add_handler('KDSCONFIGURE', self._Datasource__handle_kds_configure)
        self.add_handler('KDSLISTINTERFACES', self._Datasource__handle_kds_listinterfaces)
        self.add_handler('KDSOPENSOURCE', self._Datasource__handle_kds_opensource)
        self.add_handler('KDSPROBESOURCE', self._Datasource__handle_kds_probesource)
    ...
    def __handle_kds_configure(self, seqno, packet):
        ...
        try:
            ...
            traceback.print_exc(file=(sys.stderr))
            self.send_datasource_configure_report(seqno, success=False, message=('unhandled exception {} in configuresource callback'.format(e)))
            self.spindown()
            return
    ...
    def __handle_kds_opensource(self, seqno, packet):
        ...
        try:
            ...
            traceback.print_exc(file=(sys.stderr))
            self.send_datasource_open_report(seqno, success=False, message=('unhandled exception {} in opensource callback'.format(e)))
            self.spindown()
            return
    ...
    def send_datasource_open_report(self, seqno, success=False, dlt=0, capture_interface=None, channels=None, channel=None, hop_config=None, hardware=None, message=None, spectrum=None, uuid=None, warning=None, **kwargs):
        """
    ...
    def send_datasource_probe_report(self, seqno, success=False, message=None, channels=None, channel=None, spectrum=None, hardware=None, **kwargs):
        """
