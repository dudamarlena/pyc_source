...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fosswallproxy0/haproxy.py
# Compiled at: 2008-02-20 19:24:15
import sys, commands
from base import *
...
class HAProxy(Base):
    ...
    def list_servers(self):
        ...
        return dict(title='Servers', virtuals=servers)
    ...
    def real_modify(self, v_id, r_id, action=None, name=None, ip_address=None, port=None, cookie=None, options=''):
        ...
        if action is None:
            ...
            self.conf.write()
            redirect('real')
        return dict(title='Modify %s real server' % server.name, server=server, virtuals=self.conf.listen, v_id=v_id, r_id=r_id, action=action)
    ...
    def real_new(self, v_id, r_id='', action=None, name=None, ip_address=None, port=None, options=''):
        ...
        if action is None:
            ...
            server = listen.add_server(name=name, ip_address=ip_address, port=port, options=options)
            self.conf.write()
            redirect('real')
        return dict(title='New real server', server=server, virtuals=self.conf.listen, v_id=v_id, r_id=r_id, action=action)
    ...
    def view(self):
        return dict(title='HAProxy Configuration', conf=str(self.conf))
    ...
    def virtual_modify(self, v_id, action=None, name=None, ip_address=None, port=None, mode='http', persistent=False):
        ...
        return dict(title='Modify "%s" Virtual Server' % server.name, virtuals=self.conf.listen, server=server, id=v_id, action='modify')
    ...
    def virtual_new(self, action=None, name=None, ip_address=None, port=None, mode='http', persistent=False, **kw):
        if action is None:
            ...
            self.conf.write()
            redirect('virtual')
        return dict(title='New Virtual Server', server=server, virtuals=self.conf.listen, action='create', id='')
