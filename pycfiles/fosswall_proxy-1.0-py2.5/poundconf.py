...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/poundconf/poundconf.py
# Compiled at: 2008-02-12 02:55:00
import re, time
...
class PoundService(PoundSection):
    _options = ('Address', 'Port')
    _options_class = dict(Address=PoundServiceValue, Port=PoundServiceValue)
...
class PoundListen(PoundSection):
    _options = ('Address', 'Port', 'Cert')
    _options_class = dict(Address=PoundListenValue, Port=PoundListenValue, Cert=PoundListenValue)
...
class PoundConf(PoundSection):
    filename = None
    _options = ('User', 'Group', 'LogLevel', 'ListenHTTPS')
