...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/GNota/model.py
# Compiled at: 2007-09-10 02:42:01
from elixir import *
from gnota_exceptions import *
import os, sqlalchemy, sqlalchemy.exceptions
try:
...
class GNotaConfig(Entity):
    ...
    has_field('dontask_for_gradebook_backups', Boolean)
...
class Grade(Entity):
    ...
    belongs_to('score', of_kind='ScoreSymbolValue', inverse='grades')
    has_field('missed_classes', Integer)

    def assert_grade_type_matches(self):
        if self.activity is not None:
            assert self.score.scoresystem.id == self.activity.scoresystem.id
        elif self.overall_grades_of_class is not None:
            assert self.score.scoresystem.id == self.overall_grades_of_class.scoresystem.id
        else:
...
class Class(Entity):
    ...
    if typechecks_declaration_needed:
        belongs_to('criterion', of_kind='ApprovationCriterion', inverse='classes', enable_typechecks=False)
        belongs_to('scoresystem', of_kind='AbstractScoreSystem', inverse='classes', enable_typechecks=False)
    else:
        belongs_to('criterion', of_kind='ApprovationCriterion', inverse='classes')
...
class ScoreSymbolValue(Entity):
    ...
    if typechecks_declaration_needed:
        has_and_belongs_to_many('approvation_criteria', of_kind='ApprovationCriterion', inverse='passing_score', enable_typechecks=False)
    else:
        has_and_belongs_to_many('approvation_criteria', of_kind='ApprovationCriterion', inverse='passing_score')
...
class DiscreteValuesScoreSystem(AbstractValuesScoreSystem):
    ...
    def fromFractionalValue(self, frac):
        ...
        if frac in l:
            ...
            score_down = self.scores[(frac_down_index - 1)]
...
class ApprovationCriterion(Entity):
    ...
    has_field('description', Unicode)
    has_field('cls', Unicode)
    has_and_belongs_to_many('passing_score', of_kind='ScoreSymbolValue', inverse='approvation_criteria')
