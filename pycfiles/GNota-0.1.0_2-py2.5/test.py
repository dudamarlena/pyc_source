...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/GNota/test.py
# Compiled at: 2007-09-10 02:43:33
import sys, os
...
def test_american_extended_letter_scoresystem():
    ...
    objectstore.flush()
    ExtendedAmericanLetterScoreSystem = DiscreteValuesScoreSystem(name='Extended Letter Grade', description='American Extended Letter Grade', scores=[
...
def test_complete_rangedscoresystem_simpleaverageofbestn():
    ...
    zeroten = gc.create_ranged_scoresystem(name='Zero/Ten scoresystem', desc='Zero/Ten ranged scoresystem', min_ssv=zero, max_ssv=ten)
    objectstore.flush()
    sa_best_2_five = gc.create_simple_average_of_best_N_criterion(name='Simple average of best 2 (Average 5)', min_passing_score=five.symbol, ss=zeroten, N=2)
    ...
    ac = gc.add_activity(name='Arts homework 1', category=homework, activity_class=cls, description='Painting', date=date.today(), scoresystem=zeroten)
...
def test_complete_discretevaluesscoresystem_weightedaverage():
    ...
    foo = gc.add_student(first_name='Foo 1', last_name='Quux', code='1', photograph='/home/lameiro/photo-foo.jpg', notes='Foo student', year=2007, phone='(12) 3456-7890')
    gc.add_class_to_student(cls, foo)
    ac = gc.add_activity(name='Calculus homework 1', category=homework, activity_class=cls, description='Foobar', date=date.today(), scoresystem=ExtendedAmericanLetterScoreSystem, weight=1)
    gc.add_activity_to_student(ac, foo)
    gc.set_student_grade_in_activity(foo, ac, 'B')
    objectstore.flush()
    ac2 = gc.add_activity(name='Calculus project 1', category=project, activity_class=cls, description='Spam', date=date.today(), scoresystem=ExtendedAmericanLetterScoreSystem, weight=15)
