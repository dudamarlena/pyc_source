...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/GNota/controller.py
# Compiled at: 2007-09-10 02:15:22
from model import *
from gnota_exceptions import *
import csv, datetime, tempfile, shutil, os
...
class GNotaController(object):
    ...
    def _get_student_grades_in_class(self, student, cls):
        return [ grade for grade in Grade.select_by(student=student) if grade.activity is not None if grade.activity.activity_class == cls ]
    ...
    def remove_activity(self, ac):
        ...
        ac.activity_class.activities.remove(ac)
        ac.delete()
        objectstore.flush()
        self.set_dirty()
        self.notify_observers()
    ...
    def create_simple_average_of_best_N_criterion(self, name, min_passing_score, ss, N):
        ...
        crit = SimpleAverageOfBestN(name=name, passing_score=[
         score], N=N, cls='SimpleAverageOfBestN')
        objectstore.flush()
        self.set_dirty()
        self.notify_criteria_observers()
        return crit
    ...
    def create_simple_average_criterion(self, name, min_passing_score, ss):
        ...
        score = self.get_scoresymbolvalue(min_passing_score, ss)
        crit = SimpleAverage(name=name, passing_score=[
         score], cls='SimpleAverage')
        objectstore.flush()
        self.set_dirty()
        self.notify_criteria_observers()
        return crit
    ...
    def create_ranged_scoresystem(self, name, desc, min_ssv, max_ssv):
        ...
        ss = RangedValuesScoreSystem(name=name, description=desc, min=min_ssv, max=max_ssv, cls='RangedValuesScoreSystem')
    ...
    def create_discrete_scoresystem(self, name, desc, scores):
        print '[Controller] - Creating scoresystem name =', name
        ss = DiscreteValuesScoreSystem(name=name, description=desc, scores=scores, cls='DiscreteValuesScoreSystem')
        for score in scores:
    ...
    def get_gnota_config(self):
        try:
            ...
            config = GNotaConfig(runs_since_last_backup=0, max_runs_since_last_backup=15, dontask_for_gradebook_backups=False)
            objectstore.flush()
            self.set_dirty()
    ...
    def export_csv(self, filename):
        ...
        for cls in self.classes:
            w.writerow((cls.name,))
            l = [_('Last name'), _('First name')]
            for ac in cls.activities:
                l.append(ac.name)

            l += [_('Overall score'), _('Missed classes'), _('Approved?')]
            w.writerow(l)
