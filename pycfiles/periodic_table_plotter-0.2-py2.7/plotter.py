...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ptplotter/plotter.py
# Compiled at: 2014-04-14 15:44:41
from __future__ import division
import os, os.path, yaml, numpy as np, matplotlib.pylab as plt
from matplotlib.patches import Polygon
from matplotlib.collections import PatchCollection
INSTALL_PATH = os.path.dirname(os.path.abspath(__file__))
__all__ = [
 'ElementDataPlotter', 'plt', 'Square']
...
class ElementDataPlotter(object):
    ...
    def __init__(self, data={}, elements=None, pair_data={}, **kwargs):
        ...
        for elt, value in init_data.items():
            if not isinstance(value, dict):
                ...
                if elements and elt not in elements:
                    continue
    ...
    def cbar_labels(self):
        return [ ('\n').join([ self.functions[j].__doc__ for j in group ]) for group in self.inv.values()
               ]
    ...
    def make_grid(self, xelts=[], yelts=[], functions=[eneg_diff], cmaps='jet', **kwargs):
        """
        ...
        for i, elt1 in enumerate(xelts):
            self._ax.text(i + 0.5, len(yelts) + 0.25, elt1, va='center', ha='right')
    ...
    def ptable(self, functions=[
 atomic_number], cmaps=None, **kwargs):
        """
    ...
    def draw(self, colorbars=True, **kwargs):
        ...
        for coll, cmap, label in zip(self.collections, self.cmaps, self.cbar_labels):
            ...
            if colorbars:
                options = {'orientation': 'horizontal', 'pad': 0.05, 
                   'aspect': 60}
    ...
    def pettifor(self, xaxis=atomic_number, yaxis=atomic_number, label=symbol):
        """
...
class Square(object):
    ...
    def __init__(self, x, y, label=None, data=[], dx=1.0, dy=1.0, **kwargs):
        self.x, self.y = x, y
    ...
    def double_label(self, labels, position='horizontal', **kwargs):
        ...
        if position == 'horizontal':
            ...
            raise ValueError("`position` must be one of:'horizontal', 'vertical'")
        fontdict = kwargs.get('fontdict', {})
