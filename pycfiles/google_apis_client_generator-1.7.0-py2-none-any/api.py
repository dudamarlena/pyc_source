...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/api.py
...
import json, logging, operator, urlparse
...
class Api(template_objects.CodeObject):
  ...
  """
    ...
    def __init__(self, discovery_doc, language=None):
        super(Api, self).__init__(discovery_doc, self, wire_name=discovery_doc['name'])
        ...
        self.SetTemplateValue('versionNoDots', self.values['version'].replace('.', '_'))
        self.SetTemplateValue('versionNoDash', self.values['version'].replace('-', '_'))
        self.SetTemplateValue('dataWrapper', 'dataWrapper' in discovery_doc.get('features', []))
        ...
        self._authscopes = []
        if self.values.get('auth') and self.values['auth'].get('oauth2') and self.values['auth']['oauth2'].get('scopes'):
            for value, auth_dict in sorted(self.values['auth']['oauth2']['scopes'].iteritems()):
    ...
    def _SetupModules(self):
        ...
        self._containing_module = template_objects.Module(package_path=path, owner_name=self.values.get('owner'), owner_domain=self.values.get('ownerDomain'))
        ...
        self._module = template_objects.Module(package_path=base, parent=self._containing_module)
        self.SetTemplateValue('module', self._module)
        self._model_module = template_objects.Module(package_path=None, parent=self._module)
        return
            ...
            for name, info in self._variant_info.iteritems():
                ...
                for prop in schema.values.get('properties'):
                    if prop.values['wireName'] == info['discriminant']:
                        schema.SetTemplateValue('properties', [ p for p in schema.values.get('properties') if p != prop ])
                        break
                else:
                    logging.warn("Variant schema '%s' for base schema '%s' has not the expected discriminant property '%s'.", name, info['schema'].values['wireName'], info['discriminant'])
    ...
    """
        ...
        best_path = base_url or self.values.get('basePath') or self.values.get('restBasePath') or '/%s/%s/' % (self.values['name'], self.values['version'])
        if best_path.find('..') >= 0:
    ...
    def ModelClasses(self):
        ...
        ret = set(s for s in self._schemas.itervalues() if isinstance(s, Schema) or isinstance(s, data_types.MapDataType))
    ...
    def DataTypeFromJson(self, type_dict, default_name, parent=None, wire_name=None):
    ...
    Returns:
      A Schema object.
    """
        schema = Schema.Create(self, default_name, type_dict or {}, wire_name, parent)
        if isinstance(schema, Schema) or isinstance(schema, data_types.MapDataType):
            path = ('.').join([ a.values.get('wireName', '<anon>') for a in schema.full_path ])
    ...
    def SetVariantInfo(self, ref, discriminant, value, schema):
        ...
        if ref in self._variant_info:
            logging.warning("Base type of '%s' changed from '%s' to '%s'. This is an indication that a variant schema is used from multiple base schemas and may result in an inconsistent model.", ref, self._base_type[ref].wireName, schema.wireName)
        self._variant_info[ref] = {'discriminant': discriminant, 'value': value, 'schema': schema}
...
class AuthScope(template_objects.CodeObject):
  ...
  """
    ...
    def constantName(self):
        ...
        return self._language_model.ApplyPolicy('constant', self, self.values['lastPart'])
...
class Method(template_objects.CodeObject):
    ...
    """
        ...
        self.SetTemplateValue('rpcMethod', def_dict.get('rpcMethod') or def_dict['id'])
        ...
        if expected_request:
            request_schema = api.DataTypeFromJson(expected_request, '%sRequestContent' % name, parent=self)
            self.SetTemplateValue('requestType', request_schema)
        expected_response = def_dict.get('response') or def_dict.get('returns')
        if expected_response:
            response_schema = api.DataTypeFromJson(expected_response, '%sResponse' % name, parent=self)
        ...
        req_parameters.sort(lambda x, y: cmp(order.index(x.values['wireName']), order.index(y.values['wireName'])))
    ...
    def _InitMediaUpload(self, parent):
        ...
        if media_upload:
            ...
            self.SetTemplateValue('max_size_bytes', convert_size.ConvertSize(max_size))
            upload_protocols = media_upload['protocols']
            for upload_protocol in upload_protocols:
                self._SetUploadTemplateValues(upload_protocol, upload_protocols[upload_protocol])

    def _InitPageable(self, api):
        response_type = self.values.get('responseType')
        if response_type == api.void_type:
            return
        next_page_token_name = self.FindPageToken(response_type.values.get('properties'))
        ...
        if not page_token_name:
            ...
            if request_type:
                page_token_name = self.FindPageToken(request_type.values.get('properties'))
        if not page_token_name:
            return
        self.SetTemplateValue('isPageable', True)
        self.SetTemplateValue('isPagingStyleStandard', is_page_token_parameter and page_token_name == 'pageToken' and next_page_token_name == 'nextPageToken')
    ...
    """
        ...
        if upload_path:
            ...
            self.SetTemplateValue('%s_upload_multipart' % upload_protocol, protocol_dict.get('multipart', False))
    ...
    """
        if not things:
            ...
            for e in things:
                if e.values['wireName'] == wire_name:
                    return e
...
class Parameter(template_objects.CodeObject):
    ...
    def __init__(self, api, name, def_dict, method):
        super(Parameter, self).__init__(def_dict, api, parent=method, wire_name=name)
        self.ValidateName(name)
        self.schema = api
        self._repeated = self.values.get('repeated', False)
        self._required = self.values.get('required', False)
        self._location = self.values.get('location') or self.values.get('restParameterType') or 'query'
        referenced_schema = self.values.get('$ref')
        if referenced_schema:
            self._data_type = api.SchemaByName(referenced_schema) or data_types.SchemaReference(referenced_schema, api)
        elif def_dict.get('type') == 'array':
            self._data_type = Schema.Create(api, name, def_dict, name, method)
        elif self.values.get('enum'):
            self._data_type = data_types.Enum(def_dict, api, name, self.values.get('enum'), self.values.get('enumDescriptions'), parent=method)
            self.SetTemplateValue('enumType', self._data_type)
        else:
            self._data_type = data_types.PrimitiveDataType(def_dict, api, parent=self)
        if self._repeated:
            self._data_type = data_types.ArrayDataType(name, self._data_type, parent=self)
