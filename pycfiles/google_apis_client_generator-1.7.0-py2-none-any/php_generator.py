...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/php_generator.py
...
import collections, json, operator
...
class PHPGenerator(api_library_generator.ApiLibraryGenerator):
    ...
    """
        super(PHPGenerator, self).__init__(PHPApi, discovery, 'php', language_model=PhpLanguageModel(), options=options)
    ...
    """
        ...
        for r in resource.values['resources']:
            r.values['className'] = resource.values['className'] + r.values['className']
    ...
    """
        list_replacements = {'___models_': [
                        'model', the_api.ModelClasses()], 
           '___resources_': [
                           'resource', the_api.ResourceClasses()], 
           '___topLevelModels_': [
                                'model', the_api.TopLevelModelClasses()]}
        self.WalkTemplateTree('templates', self._path_replacements, list_replacements, self._top_level_defines, source_package_writer)
...
class PhpLanguageModel(language_model.LanguageModel):
    ...
    language = 'php'
    _SCHEMA_TYPE_TO_PHP_TYPE = {'any': 'array', 
       'boolean': 'bool', 
       'integer': 'int', 
       'long': 'string', 
       'number': 'double', 
       ...
       'double': 'double', 
       'float': 'float'}
    PHP_KEYWORDS = frozenset(('abstract', 'and', 'array', 'as', 'break', 'call', 'callable',
                              'case', 'catch', 'cfunction', 'class', 'clone', 'const',
                              'continue', 'declare', 'default', 'do', 'else', 'elseif',
                              'empty', 'enddeclare', 'endfor', 'endforeach', 'endif',
                              'endswitch', 'endwhile', 'extends', 'final', 'finally',
                              'for', 'foreach', 'function', 'global', 'goto', 'if',
                              'implements', 'interface', 'instanceof', 'list', 'namespace',
                              'new', 'old_function', 'or', 'parent', 'private', 'protected',
                              'public', 'return', 'static', 'switch', 'throw', 'trait',
                              'try', 'unset', 'use', 'var', 'while', 'xor', 'yield'))
    PHP_TYPES = frozenset(('bool', 'boolean', 'int', 'integer', 'file', 'float', 'double',
                           'string', 'array', 'object', 'null', 'resource'))
    ...
    member_policy = language_model.NamingPolicy(case_transform=language_model.LOWER_CAMEL_CASE)
    ...
    """
        ...
        php_type = self._SCHEMA_TYPE_TO_PHP_TYPE.get(json_format) or self._SCHEMA_TYPE_TO_PHP_TYPE.get(json_type, json_type)
        return php_type
...
_EXTRA_PROPERTIES = [
 'description', 'enumDescriptions', 'resources', 'pattern',
 'parameterOrder']
