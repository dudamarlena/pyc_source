...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/generator.py
...
import datetime, os, re, StringIO, time, zipfile
...
_GENERATOR_INFORMATION = {'name': 'google-apis-code-generator', 
   'version': '1.6.0', 
...
class TemplateGenerator(object):
  ...
  * Callers can provide a set of replacements to be made to file paths in
  ...
  """
    ...
    """
        variables_dict = {'tool': self._tool_info, 
           'options': self._options, 
           'template_dir': self._template_dir, 
           'features': self._surface_features, 
           'language_model': self._language_model}
        if context_dict:
            variables_dict.update(context_dict)
        return DjangoRenderTemplate(template_path, variables_dict)

    def RenderTemplateToFile(self, template_path, context_dict, package, output_path):
    ...
    def WalkTemplateTree(self, path_to_tree, path_replacements, list_replacements, variables, package, file_filter=None):
    ...
    """
      ...
      """
            ...
            for info in archive.infolist():
                package.WriteDataAsFile(archive.read(info.filename), os.path.join(relative_path, info.filename))

        top_of_tree = os.path.normpath(os.path.join(self._template_dir, path_to_tree))
        ...
        for path in files.IterFiles(top_of_tree):
            ...
            for path_item, call_info in list_replacements.iteritems():
                if file_name.find(path_item) >= 0:
                    self.GenerateListOfFiles(path_item, call_info, path, relative_path, file_name, variables, package, file_filter=file_filter)
            ...
            if file_name.endswith('.tmpl'):
                ...
                if file_filter and not file_filter(full_template_path, full_output_path):
                    continue
    ...
    """
        raise NotImplementedError('GeneratePackage must be implemented by all subclasses')

    def DefaultGeneratePackage(self, package_writer, path_replacements, variables):
    ...
    Args:
      ...
      variables: (dict) The dictionary of variable replacements to pass to the
         templates.
    """
        self.WalkTemplateTree('templates', path_replacements, {}, variables, package_writer)
    ...
    def GenerateListOfFiles(self, path_prefix, call_info, template_path, relative_path, template_file_name, variables, package, file_filter=None):
    ...
    """
        path_and_var_regex = '%s([a-z][A-Za-z]*)___' % path_prefix
        match_obj = re.compile(path_and_var_regex).match(template_file_name)
        if not match_obj:
            raise ValueError('file names which match path item for GenerateListOfFiles must contain a variable for substitution. E.g. "___models_codeName___"')
        variable_name = match_obj.group(1)
        file_name_piece_to_replace = path_prefix + variable_name + '___'
        for element in call_info[1]:
            file_name = template_file_name.replace(file_name_piece_to_replace, element.values[variable_name])
            name_in_zip = file_name[:-5]
            if file_filter and not file_filter(None, name_in_zip):
                continue
            d = dict(variables)
            d[call_info[0]] = element
            self.RenderTemplateToFile(template_path, d, package, os.path.join(relative_path, name_in_zip))
