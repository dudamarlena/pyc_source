...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/objc_generator.py
...
class ObjCGenerator(api_library_generator.ApiLibraryGenerator):
    ...
    _FORMAT_TO_TYPE_COMMENT = {'int16': 'shortValue', 
       'int32': 'intValue', 
       'int64': 'longLongValue', 
    ...
    def __init__(self, discovery, options=None):
        super(ObjCGenerator, self).__init__(ObjCApi, discovery, language='objc', language_model=ObjCLanguageModel(options), options=options)
    ...
    def AnnotateApi(self, api_object):
        ...
        api_name_policy = language_model.NamingPolicy(language_model.UPPER_CAMEL_CASE)
        camel_case_name = self._language_model.TransformString(api_object, name, api_name_policy)
        api_object.SetTemplateValue('serviceName', 'GTLService%s' % camel_case_name)
        ...
        api_object.SetTemplateValue('parameters', [ param for param in api_object.GetTemplateValue('parameters') if param.GetTemplateValue('wireName') not in self._COMMON_PARAMS_TO_SKIP
                                                  ])
        ...
        for method in api_object.GetTemplateValue('methods'):
            for parameter in method.parameters:
                ...
                if not existing_parameter:
                    ...
                    merged_param = api.Parameter(api_object, parameter.GetTemplateValue('wireName'), {'description': '"%s" has different types for some query methods; see the documentation for the right type for each query method.' % parameter.codeName, 
                       'wireName': parameter['wireName'], 
                       'codeName': parameter.codeName}, existing_parameter.parent)
        ...
        api_object.SetTemplateValue('methodParameters', [ method_parameters[key] for key in sorted(method_parameters) ])
        ...
        api_object.SetTemplateValue('allParameters', [ all_parameters[key] for key in sorted(all_parameters) ])

    def AnnotateProperty(self, api_object, prop, schema):
        """Annotate a Property with ObjC specific elements."""
        if prop.codeName == 'items' and isinstance(prop.data_type, data_types.ArrayDataType):
    ...
    def AnnotateEnum(self, api_object, prop, schema, enum_values):
        ...
        enum_elements = [ {'value': enum_values[i], 'description': descriptions and descriptions[i], 'constantName': self.language_model.TransformString(prop, enum_values[i].lower(), self.language_model.constant_policy)} for i in range(len(enum_values))
                        ]
        enum = {'comment': '%s - %s' % (class_name, property_name), 
    ...
    def AnnotateParameterOrProperty(self, prop, schema, api_object):
        ...
        if isinstance(prop.data_type, data_types.ArrayDataType):
            prop.SetTemplateValue('typeComment', 'of %s' % prop.data_type['baseType'].code_type)
            prop.SetTemplateValue('elementCodeType', prop.data_type['baseType'].code_type)
            ...
            if prop.data_type.json_format:
                prop.SetTemplateValue('typeComment', self._FORMAT_TO_TYPE_COMMENT.get(prop.data_type.json_format))
    ...
    def AnnotateMethod(self, api_object, method, resource):
        ...
        method.SetTemplateValue('queryCodeName', self.language_model.TransformString(method, name[name.find('.'):], self.language_model.query_method_policy))

    def AnnotateType(self, data_type, schema):
        if isinstance(data_type, data_types.SchemaReference) or isinstance(data_type, api.Schema):
...
class ObjCLanguageModel(language_model.LanguageModel):
    ...
    constant_policy = language_model.NamingPolicy(case_transform=language_model.UPPER_CAMEL_CASE, format_string='k{schema.className}_{variable_name}_{name}')
    query_method_policy = language_model.NamingPolicy(case_transform=language_model.UPPER_CAMEL_CASE, format_string='queryFor{name}')
    _SCHEMA_TYPE_TO_OBJC_TYPE = {('any', None): 'String', 
    ...
    """
        ...
        native_format = self._SCHEMA_TYPE_TO_OBJC_TYPE.get((json_type, json_format), json_type)
...
class ObjCApi(api.Api):
    ...
    Returns:
      A name suitable for use as a class in the generator's target language.
    """
        if s.lower() in ObjCLanguageModel.RESERVED_CLASS_NAMES or isinstance(element, api.Schema):
