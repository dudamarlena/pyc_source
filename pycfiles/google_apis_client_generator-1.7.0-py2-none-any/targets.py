...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/targets.py
...
class Targets(object):
  ...
  """
    ...
    """
        ...
        self.targets_path = targets_path or os.path.join(self.template_root, 'targets.json')
        if targets_dict:
            self._targets_dict = targets_dict
        else:
            self._targets_dict = json_with_comments.Loads(files.GetFileContents(self.targets_path))
    ...
    _default_template_root = os.path.join(os.path.dirname(__file__), 'languages')
...
class Variations(dict):
    ...
    """
        return os.path.join(self._targets.template_root, self._RelativeTemplateDir(variation))
    ...
    """
        if not variation:
            ...
            features.update(json_expander.ExpandJsonTemplate(json_with_comments.Loads(features_json)))
...
class Features(dict):
    ...
    """
        if not environment and not file_type:
            return []
