...
# [GCC 8.4.0]
# Embedded file name: /Users/dsupplee/dev/apis-client-generator/src/googleapis/codegen/template_helpers.py
...
import contextlib, hashlib, logging, os, re, string, textwrap, threading, django.template as django_template
...
_defaults = {_LINE_BREAK_INDENT: 2, 
   _LINE_WIDTH: 40, 
   _PARAMETER_INDENT: 4, 
   _LEVEL_INDENT: 2, 
   _COMMENT_START: '# ', 
   ...
   _IMPORT_REGEX: '^\\s*import\\s+(?P<import>[a-zA-Z0-9.]+)', 
...
_language_defaults = {'cpp': {_LINE_WIDTH: 80, 
           _PARAMETER_INDENT: 4, 
           _LEVEL_INDENT: 2, 
           _COMMENT_START: '// ', 
           _COMMENT_CONTINUE: '// ', 
           ...
           _DOC_COMMENT_CONTINUE: ' * ', 
           _DOC_COMMENT_END: ' */', 
           _IMPORT_REGEX: '^#include\\s+(?P<import>[\\<\\"][a-zA-Z0-9./_\\-]+[\\>\\"])', 
           _IMPORT_TEMPLATE: '#include %s'}, 
   'csharp': {_LINE_WIDTH: 120, 
              _PARAMETER_INDENT: 4, 
              _LEVEL_INDENT: 4, 
              _COMMENT_START: '// ', 
              _COMMENT_CONTINUE: '// ', 
...
class CachingTemplateLoader(object):
    ...
    UNSTABLE_VARIATION_PATTERN = re.compile('^[^/]+/[^/]*dev/')
    ...
    """
        ...
        if self.UNSTABLE_VARIATION_PATTERN.match(relpath) or os.environ.get('NOCACHE'):
...
def _RenderToString(template_path, context):
  ...
  """
    t = _TEMPLATE_LOADER.GetTemplate(template_path, context.get('template_dir', ''))
...
def _GetArgFromToken(token):
  ...
  """
    try:
        ...
        raise django_template.TemplateSyntaxError('tag requires a single argument: %s' % token.contents)
...
def java_comment_fragment(value, indent):
  ...
  """
    if not indent:
        indent = 0
    prefix = '%s * ' % (' ' * indent)
    wrapper = textwrap.TextWrapper(width=_language_defaults['java'][_LINE_WIDTH], replace_whitespace=False, initial_indent=prefix, subsequent_indent=prefix)
...
def java_parameter_wrap(value):
  ...
  """
    ...
    prefix = ' * %s ' % (' ' * indent)
    wrapper = textwrap.TextWrapper(width=_language_defaults['java'][_LINE_WIDTH], replace_whitespace=False, initial_indent='', subsequent_indent=prefix)
    wrapped = wrapper.fill(value)
    return wrapped
...
def block_comment(value):
  ...
  """
    ...
    wrapper = textwrap.TextWrapper(width=line_width, replace_whitespace=False, initial_indent='%s ' % comment_prefix, subsequent_indent='%s ' % comment_prefix)
    wrapped_blocks = []
...
def DoNoBlank(parser, token):
    ...
    if len(args) > 2:
        raise django_template.TemplateSyntaxError('noblank expects at most one argument')
    if len(args) == 2:
        recursearg = args[1]
        if recursearg not in ('recurse', 'norecurse'):
            raise django_template.TemplateSyntaxError('argument to noblank must be either "norecurse" (the default) or "recurse"')
...
def DoNoEol(parser, token):
    ...
    args = token.split_contents()
    if len(args) > 2:
        raise django_template.TemplateSyntaxError('noeol expects at most one argument')
    if len(args) == 2:
        recursearg = args[1]
        if recursearg not in ('recurse', 'norecurse'):
            raise django_template.TemplateSyntaxError('argument to noeol must be either "norecurse" (the default) or "recurse"')
        recurse = recursearg == 'recurse'
    else:
        recurse = False
...
class DocCommentNode(django_template.Node):
    ...
    def __init__(self, text=None, nodelist=None, comment_type=None, wrap_blocks=True):
        self._text = text
        self._nodelist = nodelist
    ...
    def RenderText(self, text, context):
    ...
    """
        if self._comment_type == 'doc':
            start_prefix = _GetFromContext(context, _DOC_COMMENT_START, _COMMENT_START)
            continue_prefix = _GetFromContext(context, _DOC_COMMENT_CONTINUE, _COMMENT_CONTINUE)
            ...
            begin_tag = ''
            end_tag = ''
        available_width = _GetFromContext(context, _LINE_WIDTH) - context.get(_CURRENT_INDENT, 0)
        return _WrapInComment(text, wrap_blocks=self._wrap_blocks, start_prefix=start_prefix, continue_prefix=continue_prefix, comment_end=comment_end, begin_tag=begin_tag, end_tag=end_tag, available_width=available_width)


def _WrapInComment(text, wrap_blocks, start_prefix, continue_prefix, comment_end, begin_tag, end_tag, available_width):
    if len(text) < available_width and '\n' not in text:
        one_line = '%s%s%s%s%s' % (start_prefix, begin_tag, text, end_tag,
         comment_end)
        if len(one_line) < available_width:
            return one_line
    wrapper = textwrap.TextWrapper(width=available_width, replace_whitespace=False, initial_indent=continue_prefix, subsequent_indent=continue_prefix)
...
def DoDocComment(parser, token):
  ...
  """
    args = token.split_contents()
    if len(args) > 2:
        raise django_template.TemplateSyntaxError('doc_comment expects at most one argument')
    if len(args) == 2:
        wraparg = args[1]
        if wraparg not in ('block', 'noblock'):
            raise django_template.TemplateSyntaxError('argument to doc_comment (wrap_blocks) must be either "block" (the default) or "noblock"')
        wrap_blocks = wraparg == 'block'
    else:
        wrap_blocks = True
    nodelist = parser.parse(('enddoc_comment', ))
    parser.delete_first_token()
    return DocCommentNode(nodelist=nodelist, comment_type='doc', wrap_blocks=wrap_blocks)
  ...
  """
    ...
    def render(self, context):
        ...
        while parent_pointer.parent:
            getter_chain_list.append(lang_model.ToPropertyGetterMethodWithDelim(parent_pointer.safeClassName))
            parent_pointer = parent_pointer.parent

        getter_chain_list.append(lang_model.ToPropertyGetterMethodWithDelim(str(prop.GetTemplateValue('wireName'))))
...
class ImportsNode(django_template.Node):
    ...
    def render(self, context):
        ...
        import_lists = None
        try:
            import_manager = django_template.Variable('%s.importManager' % self._element).resolve(context)
...
def DoParameterList(parser, token):
  ...
  """
    ...
    if len(args) > 1:
        ...
        separator = separator.replace('\\t', '\t')
        separator = separator.replace('\\b', ' ')
    if len(args) > 2:
        if args[2] == 'append':
            append = True
    if len(args) > 3:
        raise django_template.TemplateSyntaxError('parameter_list expects at most two arguments')
    ...
    def render(self, context):
        ...
        for target, source in self._bindings.iteritems():
            try:
                ...
                raise django_template.TemplateSyntaxError('can not resolve %s when calling template %s' % (
...
def CallTemplate(unused_parser, token):
  ...
  """
    ...
    if len(contents) < 2:
        raise django_template.TemplateSyntaxError('tag requires at least 1 argument, the called template')
    ...
    if len(has_equals) != 1:
        raise django_template.TemplateSyntaxError('use either name1=value1 name2=value2 syntax, or name1 value1 name2 value2 syntax, but not both')
    has_equals = has_equals.pop()
    if has_equals:
        bindings = dict(tok.split('=', 1) for tok in toks)
    else:
        if len(toks) % 2 != 0:
            raise django_template.TemplateSyntaxError('odd number of keys and values found')
    ...
    def render(self, context):
        ...
        if hasattr(data, 'GetLanguageModel') and hasattr(data, 'value'):
            ...
            try:
                ...
                raise django_template.TemplateSyntaxError('Variable (%s) with value (%s) is not an accepted DataValue type (%s) as exhibited by: ValueError(%s).' % (
                 self._variable, data.value, data.data_type, e))

        else:
            raise django_template.TemplateSyntaxError('(%s) is not a DataValue object.' % self._variable)
...
class WriteNode(django_template.Node):
  ...
  """
    ...
    """
        ...
        if not file_writer:
            raise ValueError('"write" called in a context where "%s" is not defined.', FILE_WRITER)
