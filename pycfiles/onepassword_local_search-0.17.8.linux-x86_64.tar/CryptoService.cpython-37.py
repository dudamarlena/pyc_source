...
# [GCC 8.4.0]
# Embedded file name: /home/travis/virtualenv/python3.7.1/lib/python3.7/site-packages/onepassword_local_search/services/CryptoService.py
...
class CryptoService:
    ...
    def _get_encrypted_session_file_path(self):
        ...
        if os_path.isfile(filepath) and not os_path.isfile(filepath + '_cached') or self.disable_session_caching:
    ...
    def decrypt(self, key, cipher: Cipher):
        return aes_decrypt(ct=(get_binary_from_string(cipher.data)[:-16]),
    ...
    def _get_vault_key(self, vault_id):
        if not (hasattr(self, 'privateKeyRaw') and self.privateKeyRaw):
            self._get_base_keys()
        account_id = self.storageService.get_account_id_from_user_uuid(self.userUUID)
