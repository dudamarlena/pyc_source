...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nestedrecord/internal.py
# Compiled at: 2009-08-17 16:36:17
...
def encode_error(conversion, list_type=list, dict_type=dict, start_list_part='[', end_list_part=']'):
    ...
    if isinstance(conversion.children, list):
        raise ConversionKitError('Standalone lists are not supported by kerns format')
    return keys


def encode_partially_encoded_data(data, dict_type=dict, start_list_part='[', end_list_part=']', list_item_mapping=None):
    ...
    if isinstance(data, dict):
            ...
            if isinstance(child, list):
                for (i, sub_child) in enumerate(child):
                    if isinstance(sub_child, (unicode, str)):
                        ...
                        for (k, v) in encode_partially_encoded_data(sub_child, dict_type=dict_type, start_list_part=start_list_part, end_list_part=end_list_part, list_item_mapping=list_item_mapping).items():
        ...
        raise ConversionKitError('Objects of type %r are not supported' % type(data))
    return keys
    ...
    if isinstance(data, dict):
        for (name, child) in data.items():
            if isinstance(child, list):
                ...
                raise Exception("The %r key couldn't be converted - you can't have objects of %s nested within objects of %s" % (
                 name,
                 type(child),
                 type(data)))
                for (key, sub_child) in child.items():
...
def _dict_split_next_level(data, dict_type=dict):
    """
...
def _dict_merge_next_level(data, list_type=list, dict_type=dict, start_list_part='-', end_list_part='', strict=False):
    """
    ...
    for (k, v) in data.items():
        if start_list_part in k or end_list_part in k:
            ...
            if not len(parts) == 2:
                raise ConversionKitError('The key %r contained too many %r characters' % (k, start_list_part))
            (name, index) = parts
            try:
                index = index[:len(end_list_part)]
                index = int(index)
            except:
                raise ConversionKitError('The key %r has an invalid index (the part after the %r character)' % start_list_part % name)
            else:
                if k in dict_keys.keys():
                    raise ConversionKitError('There is already a dictionary key with the key %r' % k)
            ...
            if k in list_keys.keys():
                raise ConversionKitError('There is already a list key with the key %r' % k)
            if k in dict_keys.keys():
                raise ConversionKitError('There is already a dictionary key with the key %r' % k)
            dict_keys[k] = v
    ...
    for (k, v) in list_keys.items():
        ...
        for (index, value) in v.items():
            if strict and index != counter:
                raise ConversionKitError('The items in the %r key are not numbered consecutively, item %s is labelled %s' % (k, counter, index))
            values.append(value)
            counter += 1
...
def decode_data(data, list_type=list, dict_type=dict, start_list_part='-', end_list_part=''):
    stripped_data = _dict_split_next_level(data, dict_type=dict_type)
    merged_data = _dict_merge_next_level(stripped_data, list_type=list_type, dict_type=dict_type, start_list_part=start_list_part, end_list_part=end_list_part)
    final = dict_type()
    for (k, v) in merged_data.items():
        if isinstance(v, list):
            new_list = list_type()
            for item in v:
                new_list.append(decode_data(item, list_type=list_type, dict_type=dict_type, start_list_part=start_list_part, end_list_part=end_list_part))

            final[k] = new_list
        elif isinstance(v, dict):
            final[k] = decode_data(v, list_type=list_type, dict_type=dict_type, start_list_part=start_list_part, end_list_part=end_list_part)
        else:
            final[k] = v
...
def decode_data_one_level(data, list_type=list, dict_type=dict, start_list_part='[', end_list_part=']'):
    stripped_data = _dict_split_next_level(data, dict_type=dict_type)
    merged_data = _dict_merge_next_level(stripped_data, list_type=list_type, dict_type=dict_type, start_list_part=start_list_part, end_list_part=end_list_part)
    return merged_data
