...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/obpds/solver.py
...
q = 1.602176565e-19
eps0 = 8.85418781762e-14
k = 8.6173324e-05

def get_fermi_functions(phi_p, phi_n, Ev, Nv, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, nonparabolicity, Vt, approx='kane'):
    if approx == 'boltzmann':
        ...
        def n(psi, buf=None):
            return boltzmann_n3(phi_n, psi, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)

        def dp(psi, buf=None):
            return boltzmann_dp(phi_p, psi, Ev, Nv, Vt, out=buf)

        def dn(psi, buf=None):
            return boltzmann_dn3(phi_n, psi, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)
        ...
        def n(psi, buf=None):
            return parabolic_n3(phi_n, psi, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)

        def dp(psi, buf=None):
            return parabolic_dp(phi_p, psi, Ev, Nv, Vt, out=buf)

        def dn(psi, buf=None):
            return parabolic_dn3(phi_n, psi, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)
        ...
        def n_Gamma(psi, buf=None):
            return nonparabolic_n(phi_n, psi, nonparabolicity, Ec_Gamma, Nc_Gamma, Vt, out=buf)
        ...
        def n(psi, buf=None):
            return nonparabolic_n3(phi_n, psi, nonparabolicity, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)

        def dp(psi, buf=None):
            return parabolic_dp(phi_p, psi, Ev, Nv, Vt, out=buf)

        def dn(psi, buf=None):
            return nonparabolic_dn3(phi_n, psi, nonparabolicity, Ec_Gamma, Ec_L, Ec_X, Nc_Gamma, Nc_L, Nc_X, Vt, out=buf)

    else:
        raise ValueError(('Invalid value for approx: {}').format(approx))
    return (
     p, n_Gamma, n_X, n_L, n, dp, dn)
...
def scalar_charge_neutrality(psi0, p, n, dp, dn, Nnet):
    ...
    def A(psi):
        ...
        if numpy.isnan(drho_dpsi) or drho_dpsi == 0.0:
            drho_dpsi = 1.0
        logger.debug('drho_dpsi = %s', drho_dpsi)
        return drho_dpsi

    result = scalar_newton(G, A, psi0, rtol=1e-07)
    assert result.converged
    return result.value


def charge_neutrality(device, V, phi_p, phi_n, T=300.0, N=1000, approx='kane'):
    parameters = device._get_parameters(T, N)
    flatband = device._get_flatband(T, N)
    Vt = k * T
    p, _n_Gamma, _n_X, _n_L, n, dp, dn = get_fermi_functions(phi_p, phi_n, flatband.Ev, parameters.Nv, flatband.Ec_Gamma, flatband.Ec_L, flatband.Ec_X, parameters.Nc_Gamma, parameters.Nc_L, parameters.Nc_X, parameters.nonparabolicity, Vt, approx)
    psi0 = numpy.zeros(N)
    ...
    for i in xrange(N):
        ...
        if parameters.Nnet[i] < 0.0:
            p0 = -parameters.Nnet[i]
            phi_p0 = iparabolic_p(p0, 0.0, flatband.Ev[i], parameters.Nv[i], Vt)
            psi0[i] = phi_p0 - phi_pi
        elif parameters.Nnet[i] > 0.0:
            n0 = parameters.Nnet[i]
            phi_n0 = iparabolic_n(n0, 0.0, flatband.Ec[i], parameters.Nc[i], Vt)
            psi0[i] = phi_n0 - phi_ni
        elif phi_pi != numpy.inf and phi_ni != -numpy.inf:
...
def _poisson_zero_current(device, psi0, phi_p, phi_n, V, T=300.0, N=1000, approx='kane'):
    """
    ...
    dx = x[1]
    Vt = k * T
    eps = eps0 * parameters.dielectric
    q_over_eps = q / eps
    p, n_Gamma, n_X, n_L, n, dp, dn = get_fermi_functions(phi_p, phi_n, flatband.Ev, parameters.Nv, flatband.Ec_Gamma, flatband.Ec_L, flatband.Ec_X, parameters.Nc_Gamma, parameters.Nc_L, parameters.Nc_X, parameters.nonparabolicity, Vt, approx)
    if isinstance(device._contacts[0], OhmicContact):
        p0, _, _, _, n0, dp0, dn0 = get_fermi_functions(phi_p[0], phi_n[0], flatband.Ev[0], parameters.Nv[0], flatband.Ec_Gamma[0], flatband.Ec_L[0], flatband.Ec_X[0], parameters.Nc_Gamma[0], parameters.Nc_L[0], parameters.Nc_X[0], parameters.nonparabolicity[0], Vt, approx)
        a = scalar_charge_neutrality(psi0[0], p0, n0, dp0, dn0, parameters.Nnet[0])
    ...
    if isinstance(device._contacts[(-1)], OhmicContact):
        p0, _, _, _, n0, dp0, dn0 = get_fermi_functions(phi_p[(-1)], phi_n[(-1)], flatband.Ev[(-1)], parameters.Nv[(-1)], flatband.Ec_Gamma[(-1)], flatband.Ec_L[(-1)], flatband.Ec_X[(-1)], parameters.Nc_Gamma[(-1)], parameters.Nc_L[(-1)], parameters.Nc_X[(-1)], parameters.nonparabolicity[(-1)], Vt, approx)
        b = scalar_charge_neutrality(psi0[(-1)], p0, n0, dp0, dn0, parameters.Nnet[(-1)])
    elif isinstance(device._contacts[(-1)], SchottkyContact):
        wf = device._contacts[(-1)].work_function
        b = parameters.electron_affinity[(-1)] + flatband.Ec[(-1)] - wf
    else:
        raise RuntimeError('unexpected execution path')
    buf1 = numpy.empty(N)
    buf2 = numpy.empty(N)
    ...
    return ZeroCurrentSolution(V, T, N, x, parameters.Na, parameters.Nd, phi_p, phi_n, Ev, Ec_Gamma, Ec_L, Ec_X, Ec, Ei, dEv_dx, dEc_Gamma_dx, dEc_L_dx, dEc_X_dx, dEc_dx, psi, field, n_Gamma(psi), n_L(psi), n_X(psi), n(psi), p(psi))
...
def poisson_eq(device, T=300.0, N=1000, approx='kane'):
    """
    Uses Newton's method to solve the self-consistent electrostatic Poisson
    equation for the given device under equilibrium conditions.
    ...
    zcs = _poisson_zero_current(device, psi0, phi_p, phi_n, 0.0, T, N, approx)
...
def poisson_zero_current(device, V, T=300.0, N=1000, approx='kane'):
    """
    Uses Newton's method to solve the self-consistent electrostatic Poisson
...
def capacitance_zero_current(device, V, dV, T=300.0, N=1000, approx='kane'):
    """
    ...
    drho_positive = numpy.where(drho > 0.0, drho, zeros)
    drho_negative = numpy.where(drho < 0.0, drho, zeros)
