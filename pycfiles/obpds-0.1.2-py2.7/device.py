...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/obpds/device.py
...
class TwoTerminalDevice(object):
        ...
        if contacts is None:
            ...
            for contact in contacts:
                if not isinstance(contact, Contact):
                    raise TypeError('Contacts must be instances of the `Contact` class.')
    ...
    def show_flatband(self, T=300.0):
        """
    ...
    def _calc_Fp_Fn(self, N):
        if self._Fp is None:
            ...
            if self._Fn is None:
                ...
                if not hasattr(self._Fn, '__iter__'):
                    Fn = [
                     self._Fn] * N
                else:
                    layer_xs = []
    ...
    def _calc_parameters(self, T, N):
        ...
        for p in ['VBO', 'CBO_Gamma', 'CBO_L', 'CBO_X', 'CBO', 'Ei', 'ni',
         ...
         'Nc_X', 'Nc', 'Nv', 'nonparabolicity', 'electron_affinity']:
            value = numpy.array([ getattr(m, p)(T=T) for m in materials ], dtype=float)
            setattr(s, p, value)
    ...
    def _calc_flatband(self, T, N):
        ...
        return FlatbandSolution(T=T, N=N, x=x, Ev=p.VBO, Ec_Gamma=p.CBO_Gamma, Ec_L=p.CBO_L, Ec_X=p.CBO_X, Ec=p.CBO, Ei=p.VBO + p.Ei)

    def _get_flatband(self, T, N):
        if (
         T, N) in self._flatband:
    ...
    def _has_equilibrium(self, T=300.0, N=1000, approx='kane'):
        """
        ...
        """
        return (
         T, N, approx) in self._equilibrium

    def get_equilibrium(self, T=300.0, N=1000, approx='kane'):
        """
    ...
    def show_equilibrium(self, T=300.0, N=1000, approx='kane'):
        """
        ...
        _, (ax1, ax2, ax3) = plt.subplots(3, 1, sharex='col', figsize=(10, 10), tight_layout=cfg['plot/tight_layout'])
        ...
        if (solution.Na > 0.0).any():
            ax2.semilogy(x, solution.Na, 'r-', label='$N_A$')
        if (solution.Nd > 0.0).any():
        ...
        self.filtered_autolim(ax3, solution.dEv_dx, solution.dEc_dx, solution.field)
        plt.show()
    ...
    def has_zero_current(self, V, T=300.0, N=1000, approx='kane'):
        """
        ...
        """
        return (
         V, T, N, approx) in self._zero_current

    def get_zero_current(self, V, T=300.0, N=1000, approx='kane'):
        """
    ...
    def show_zero_current(self, V, T=300.0, N=1000, approx='kane'):
        """
        ...
        approx : str (default ='kane')
            ...
            non-parabolicity under the k.p Kane approximation (slow).
        """
        self._zero_current_image(V, path=None, show=True, T=300.0, N=1000, approx='kane')
        return

    def save_zero_current_image(self, path, V, show=False, T=300.0, N=1000, approx='kane'):
        self._zero_current_image(V=V, path=path, show=show, T=300.0, N=1000, approx='kane')

    def _zero_current_image(self, V, path=None, show=False, T=300.0, N=1000, approx='kane'):
    ...
    def interactive_zero_current(self, T=300.0, N=1000, approx='kane'):
        """
        Arguments
        ---------
        T : float (default=300.)
        ...
        approx : str (default ='kane')
            ...
            non-parabolicity under the k.p Kane approximation (slow).
        """
        solution = self.get_zero_current(0.0, T, N, approx)
        x = solution.x * 10000000.0
        import matplotlib.pyplot as plt
        plt.style.use(['ggplot'])
        fig = plt.figure(figsize=(10, 10), tight_layout=cfg['plot/tight_layout'])
        ...
        ax4_V = ax4.axhline(y=0.0, color='k', linestyle='-')
        ax1.set_ymargin(0.1)
        ax2.set_ymargin(0.1)
        ax3.set_ymargin(0.1)
    ...
    def filtered_autolim(cls, ax, *fields):
        ...
        for field in fields:
            for i in xrange(2, field.size - 2):
                ...
                if delta_m1 < delta_m2 * threshold2 and delta_p1 * threshold1 < delta_m1 and delta_p2 > delta_p1 * threshold1:
                    continue
                if delta_m1 * threshold1 < delta_m2 and delta_p1 > delta_m1 * threshold1 and delta_p2 < delta_p1 * threshold2:
                    continue
    ...
    def save_zero_current(self, V, path, show=False, T=300, N=1000, approx='kane'):
        """
        ...
        self._zero_current_image(V=V, path=path + '.png', show=show, T=T, N=N, approx=approx)

    def _calc_capacitance(self, V, dV, T=300, N=1000, approx='kane'):
        return capacitance_zero_current(self, V, dV, T, N, approx)

    def get_capacitance(self, V, dV=0.001, T=300, N=1000, approx='kane'):
        """
        Returns
        -------
        C : float
        ...
        """
        if (
         V, dV, T, N, approx) in self._capacitance:
    ...
    def get_cv(self, Vstart, Vstop, Vnum=100, dV=0.001, T=300, N=1000, approx='kane'):
        """
        Returns
        -------
        C : ndarray
    ...
    def show_cv(self, Vstart, Vstop, Vnum=50, dV=0.001, T=300, N=1000, approx='kane'):
        C, V = self.get_cv(Vstart, Vstop, Vnum, dV, T, N, approx)
        rCs = 1 / C ** 2
        ndV_drCs = -1.0 / numpy.gradient(rCs, V[1] - V[0])
        import matplotlib.pyplot as plt
        plt.style.use(['ggplot'])
        _, (ax1, ax2, ax3) = plt.subplots(3, 1, sharex='col', figsize=(10, 10), tight_layout=cfg['plot/tight_layout'])
        ax1.set_ymargin(0.05)
    ...
    def save_cv(self, path, Vstart, Vstop, Vnum=50, dV=0.001, show=False, T=300, N=1000, approx='kane'):
        C, V = self.get_cv(Vstart, Vstop, Vnum, dV, T, N, approx)
        rCs = 1 / C ** 2
        ndV_drCs = -1.0 / numpy.gradient(rCs, V[1] - V[0])
        if show:
        ...
        with open(path, 'w') as (f):
            ...
            for i in xrange(V.size):
                f.write(template.format(V=V[i], C=C[i], rCs=rCs[i], ndV_drCs=ndV_drCs[i]))