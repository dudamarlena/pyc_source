...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/qetpy/cut/_cut.py
# Compiled at: 2018-12-05 13:45:53
# Size of source mod 2**32: 24068 bytes
import numpy as np, random
from qetpy import ofamp
from scipy.stats import skew
__all__ = [
 'removeoutliers', 'iterstat', 'autocuts', 'get_muon_cut']
...
def get_muon_cut(traces, thresh_pct=0.95, nsatbins=600):
    ...
    ***Traces must have POSITIVE going pulses***
    ...
    for ii, trace in enumerate(traces):
        ...
        if peak_loc + int(nsatbins) < arr.shape[(-1)] and trace[(peak_loc + int(nsatbins))] >= trace_max * thresh_pct:
            muon_cut[ii] = True
