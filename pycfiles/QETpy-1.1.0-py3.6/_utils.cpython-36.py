...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/qetpy/utils/_utils.py
# Compiled at: 2019-02-07 14:38:29
# Size of source mod 2**32: 13775 bytes
import numpy as np
from scipy import interpolate, signal, constants
from scipy.ndimage.interpolation import shift
__all__ = [
 'make_decreasing', 'calc_offset', 'lowpassfilter', 'align_traces', 'get_offset_from_muon',
...
def stdcomplex(x, axis=0):
    ...
    std_complex = rstd + complex(0.0, 1.0) * istd
    return std_complex
...
def lowpassfilter(traces, cut_off_freq=100000, fs=625000.0, order=1):
    """
...
def align_traces(traces, lgcjustshifts=False, n_cut=5000, cut_off_freq=5000.0, fs=625000.0):
    """
    ...
    """
    traces_filt = lowpassfilter((traces[:, :n_cut]), cut_off_freq=5000, fs=625000.0)
    ...
    if lgcjustshifts:
        ...
        return (
         shifts, masked_aligned)


def get_offset_from_muon(avemuon, qetbias, rn, rload, rsh=0.005, nbaseline=6000, lgcfullrtn=True):
    """
...
def energy_absorbed(trace, ioffset, qetbias, rload, rsh, fs=None, baseline=None, time=None, indbasepre=None, indbasepost=None):
    """
