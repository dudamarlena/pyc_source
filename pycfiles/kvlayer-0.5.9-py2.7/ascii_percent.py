...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/encoders/ascii_percent.py
...
class AsciiPercentEncoder(Encoder):
    ...
    def serialize(self, key, spec):
        ...
        if spec is None:
            spec = (None, ) * len(key)
        if len(key) > len(spec):
            raise TypeError(('key too long, wanted {0} parts, got {1}').format(len(spec), len(key)))
        return self.DELIMITER.join(self._serialize_one(frag, typ) for frag, typ in zip(key, spec))
    ...
    def _serialize_one(self, frag, typ):
        ...
        if typ is not None and not isinstance(frag, typ):
            raise TypeError(('expected {0} in key but got {1} ({2!r})').format(typ, type(frag), frag))
        if isinstance(frag, uuid.UUID) or hasattr(frag, 'hex'):
            return frag.hex
        else:
            if isinstance(frag, (int, long)):
                return ('{0:032x}').format(frag)
            if isinstance(frag, basestring):
                return str(frag.replace('%', '%25').replace('\x00', '%00'))
            if typ is None:
                raise TypeError(('could not serialize {0} key fragment ({1!r})').format(type(frag), frag))
            else:
                raise SerializationError(('could not serialize {0} key fragment').format(typ))
            return
    ...
    def deserialize(self, dbkey, spec):
        ...
        return tuple(self._deserialize_one(frag, typ) for frag, typ in zip(parts, spec))
    ...
    def _deserialize_one(self, frag, typ):
        ...
        raise SerializationError(('could not deserialize {0} key fragment').format(typ))