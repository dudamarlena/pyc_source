...
def direct(config_path, namespace_string):
    with yakonfig.defaulted_config([kvlayer], filename=config_path, params={'app_name': 'kvlayer', 'namespace': namespace_string}):
        config = yakonfig.get_global_config('kvlayer')
        conn = Accumulo(host='test-accumulo-1.diffeo.com', port=50096, user=config['username'], password=config['password'])
        yield conn
...
def client(namespace_string, config_path):
    app_name = 'kvlayer'
    with yakonfig.defaulted_config([kvlayer], filename=config_path, params={'app_name': app_name, 'namespace': namespace_string}):
...
def test_string_decrement():
    assert _string_decrement('b') == b'a\xff'
    assert _string_decrement('b\x00') == b'a\xff\xff'
    assert _string_decrement('\x00') == None
    assert _string_decrement('') == None
    assert _string_decrement(b'b\xff') == b'b\xfe\xff'
    return