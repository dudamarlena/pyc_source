...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/tests/performance.py
...
import argparse, cProfile, logging, multiprocessing, os, pstats, Queue
...
import yakonfig, kvlayer
from kvlayer._client import STORAGE_CLIENTS
fifteen_MB_minus_overhead = 15 * 1048576 - 256

def perftest_large_writes(client, item_size=fifteen_MB_minus_overhead, num_items_per_batch=1, num_batches=10):
    client.setup_namespace(dict(t1=2, t2=3))
    u1, u2 = uuid.uuid1(), uuid.uuid1()
    long_string = ' ' * item_size
    num = num_batches * num_items_per_batch
    total_bytes = num * len(long_string)
    print(('Testing large writes ({0} rows of {1} KB)').format(num, len(long_string) / 1024))
    t1 = time.time()
    ...
    print(('{0} single  rows put={1:.1f} sec ({2:.1f} per sec, {3:.1f} KB/s)').format(num, dt, num / dt, total_bytes / (1024 * dt)))
    for _ in xrange(num_batches):
        client.put('t1', *(((u1, u2), long_string) for row in xrange(num_items_per_batch)))

    t3 = time.time()
    dt = t3 - t2
    print(('{0} batched rows put={1:.1f} sec ({2:.1f} per sec, {3:.1f} KB/s)').format(num, dt, num / dt, total_bytes / (1024 * dt)))
...
def perftest_storage_speed(client):
    ...
    t1 = time.time()
    client.put('t1', *[ ((uuid.uuid4(), uuid.uuid4()), '') for i in xrange(num_rows)
                      ])
    t2 = time.time()
    ...
    print(('{0} rows put={1:.1f} sec ({2:.1f} per sec) get={3:.1f} sec ({4:.1f} per sec)').format(num_rows, t2 - t1, put_rate, t3 - t2, get_rate))
...
def run_many(operation, task_generator, timeout, num_workers=1, class_config=None, profile=False):
    ...
    o_queue = manager.Queue()
    pool = multiprocessing.Pool(num_workers, maxtasksperchild=1)
    async_results = []
    for x in range(num_workers):
        async_res = pool.apply_async(worker, args=(
         operation, i_queue, o_queue, tasks_remaining, class_config,
         profile))
    ...
    while async_results:
        ...
        tasks_added = 0
        while o_queue.qsize() < num_workers * 2 and elapsed < timeout and tasks_added < num_workers * 2:
            elapsed = time.time() - start_time
...
def run_local(operation, task_generator, timeout, num_workers=1, class_config=None, profile=False):
...
def worker(operation, i_queue, o_queue, tasks_remaining, class_config=None, profile=False):
    """Simple task runner.
...
class simple_app(object):
    ...
    def __init__(self, config):
        ...
        self.long_string = ' ' * self.item_size
...
class random_inserts(simple_app):
    ...
    def __call__(self, u, o_queue):
        for _ in xrange(self.num_batches):
            keys = [ (u, uuid.uuid4()) for _ in xrange(self.num_items_per_batch)
                   ]
            try:
...
def perftest_throughput_insert_random(num_workers=4, profile=False, item_size=fifteen_MB_minus_overhead, num_items_per_batch=1, num_batches=10, client=None):
    ...
    if client._config.get('storage_type') == 'accumulo':
        ...
        step = 214748365
        splits = [ struct.pack('>I', i) for i in xrange(step, 4294967295, step) ]
    ...
    class_config = dict(item_size=item_size, num_items_per_batch=num_items_per_batch, num_batches=num_batches)
    if num_workers == 1:
        class_config['client'] = client
    start_time = time.time()
    ret_vals = list(run_many(random_inserts, task_generator, timeout=total_inserts * 5, class_config=class_config, num_workers=num_workers, profile=profile))
    elapsed = time.time() - start_time
    ...
    print(('parallel {0} workers, {1} batches, {2} items per batch, {3} bytes per item, {4} inserts ({5:.4f} MB) written in {6:.1f} seconds --> {7:.1f} items/sec, {8:.4f} MB/s').format(num_workers, num_batches, num_items_per_batch, item_size, total_inserts, total_bytes / 1048576, elapsed, rate, total_bytes / (1048576 * elapsed)))
    sys.stdout.flush()
    return (ret_vals, total_bytes / elapsed)


def perftest_throughput_many_gets(ret_vals=[], num_workers=4, item_size=fifteen_MB_minus_overhead, num_items_per_batch=1, num_batches=10, profile=False, client=None):
    ...
    start_time = time.time()
    count = 0
    for found, u in run_many(many_gets, ret_vals, class_config=class_config, num_workers=num_workers, timeout=len(ret_vals) * 5, profile=profile):
        assert found
        count += 1
    ...
    rate = total_inserts / elapsed
    rate = count / elapsed
    print(('parallel {0} workers, {1} batches, {2} items per batch, {3} bytes per item, {4} items ({5:.4f} MB) read in {6:.1f} seconds --> {7:.1f} items/sec, {8:.4f} MB/s').format(num_workers, num_batches, num_items_per_batch, item_size, count, total_bytes / 1048576, elapsed, rate, total_bytes / (1048576 * elapsed)))
    sys.stdout.flush()
    return total_bytes / elapsed


def run_perftests(num_workers=4, item_size=fifteen_MB_minus_overhead, num_items_per_batch=1, num_batches=10, profile=False, splitter='\t', out=None):
    ...
    try:
        ...
        single_put_bps, batch_put_bps = perftest_large_writes(client, item_size=item_size, num_items_per_batch=num_items_per_batch, num_batches=num_batches)
        ...
        if name not in ('filestorage', ):
            ret_vals, insert_bps = perftest_throughput_insert_random(num_workers=num_workers, item_size=item_size, num_items_per_batch=num_items_per_batch, num_batches=num_batches, profile=profile, client=client)
            get_bps = perftest_throughput_many_gets(ret_vals=ret_vals, num_workers=num_workers, item_size=item_size, num_items_per_batch=num_items_per_batch, num_batches=num_batches, profile=profile, client=client)
...
def run_all_perftests(redis_address=None, clientlist=None):
    ...
    for name in STORAGE_CLIENTS.iterkeys():
        if clientlist is not None:
            ...
            logger.info('skipping backend %r', name)
            continue
        config = os.path.join(os.path.dirname(__file__), ('config_{0}.yaml').format(name))
        if not os.path.exists(config):
            continue
        params = {'app_name': 'kvlayer_performance', 'namespace': 'ns' + uuid.uuid1().hex}
        if name == 'filestorage':
...
def main():
    parser = argparse.ArgumentParser(description='Run kvlayer performance tests on a single backend.', conflict_handler='resolve')
    parser.add_argument('--num-workers', action='append', default=[], type=int)
    parser.add_argument('--item-size', action='append', default=[], type=int, help='size of the items to push in the large writes test, defaults to maximum size per record in thrift RPC server example, i.e. 15MB minus a bit of overhead')
    parser.add_argument('--num-items-per-batch', action='append', default=[], type=int, help='number of items per batch in the large writes test, defaults to 1')
    parser.add_argument('--num-batches', default=10, type=int, help='number of batches in the large writes test, defaults to 10')
    ...
    for num_workers in args.num_workers:
        for num_items_per_batch in args.num_items_per_batch:
            for item_size in args.item_size:
                rc = run_perftests(num_workers=num_workers, item_size=item_size, num_items_per_batch=num_items_per_batch, num_batches=args.num_batches, profile=args.profile, out=out)
