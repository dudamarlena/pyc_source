...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/_abstract_storage.py
...
import abc, atexit, collections, itertools, json, operator, struct, time, uuid
...
class AbstractStorage(object):
    ...
    def check_put_key_value(self, key, value, table_name, key_spec=None, value_type=None):
    ...
    def value_to_str(self, value, value_type):
        if value is None:
            ...
            raise ConfigurationError(('unexpected value_type {0!r}').format(value_type))
            return
    ...
    def setup_namespace(self, table_names, value_types=None):
        ...
        for k, v in table_names.iteritems():
            if isinstance(v, (int, long)):
                if v >= 50:
                    raise ConfigurationError(('excessively long tuple size {0!r} for table {1!r}').format(v, k))
                v = (
                 uuid.UUID,) * v
            self._table_names[k] = v
            value_type = value_types.get(k, str)
            if value_type not in (str, int, float, COUNTER, ACCUMULATOR):
                raise ConfigurationError(('invalid value type {0!r} for table {1!r}').format(value_type, k))
    ...
    def scan_keys(self, table_name, *key_ranges, **kwargs):
        ...
        """
        return itertools.imap(operator.itemgetter(0), self.scan(table_name, *key_ranges, **kwargs))
    ...
    def increment(self, table_name, *keys_and_values):
        ...
        new_keys_values = [ (k, (v or 0) + d) for (k, v), d in zip(old_keys_values, deltas)
                          ]
...
class StringKeyedStorage(AbstractStorage):
    ...
    def put(self, table_name, *keys_and_values, **kwargs):
        ...
        ks = [ self._encoder.serialize(k, self._table_names[table_name]) for k, v in keys_and_values ]
        vs = [ self.value_to_str(v, self._value_types[table_name]) for k, v in keys_and_values
             ]
        self._put(table_name, zip(ks, vs), **kwargs)
        end_time = time.time()
        if self._log_stats is not None:
            self._log_stats.put.add(table_name, start_time, end_time, len(ks), sum(len(k) for k in ks), len(vs), sum(len(v) for v in vs))
        return
    ...
    def scan(self, table_name, *key_ranges, **kwargs):
        ...
        key_spec = self._table_names[table_name]
        value_type = self._value_types[table_name]
        new_key_ranges = [ (self._encoder.make_start_key(start, key_spec), self._encoder.make_end_key(end, key_spec)) for start, end in key_ranges
                         ]
    ...
    def get(self, table_name, *keys, **kwargs):
        ...
        for k, v in self._get(table_name, new_keys, **kwargs):
            if v is None:
                ...
                stats.record(len(k), len(v))
            yield (
             self._encoder.deserialize(k, key_spec),
             self.str_to_value(v, value_type))
    ...
    def delete(self, table_name, *keys, **kwargs):
        ...
        if self._log_stats is not None:
            self._log_stats.delete.add(table_name, start_time, time.time(), len(new_keys), sum(len(k) for k in new_keys), 0, 0)
        return
...
class OpStats(object):
    ...
    def add(self, table_name, start_time, end_time, num_keys, keys_size, num_values, values_size):
        self.num_ops += 1
        self.by_table[table_name].add(start_time, end_time, num_keys, keys_size, num_values, values_size)
        self._psto.did_op()

    def add_rec(self, table_name, record):
        """Variant of :meth:`add` taking a :class:`StatRecord`."""
        return self.add(table_name, record.start_time, time.time(), record.num_keys, record.keys_size, record.num_values, record.values_size)
...
class OpStatsPerTable(object):
    ...
    def __str__(self):
        out = ('{t:0.3f}s on {ops} ops ({spops:0.6g} s/op), {k} keys for {kb} bytes ({bpk} B/key)').format(t=self.total_time, ops=self.num_ops, spops=self.num_ops and self.total_time / self.num_ops, k=self.num_keys, kb=self.keys_size, bpk=self.num_keys and 1.0 * self.keys_size / self.num_keys)
        if self.num_values > 0:
            out += ('{v} values for {vb} bytes ({bpv:0.1f} B/val)').format(v=self.num_values, vb=self.values_size, bpv=1.0 * self.values_size / self.num_values)
        if self.total_time > 0:
            out += (' {0:0.1f} (k+v)B/s').format(1.0 * (self.keys_size + self.values_size) / self.total_time)
        return out

    def to_dict(self):
        """return a dict suitable for json.dump()"""
        out = {'t': self.total_time, 
           'ops': self.num_ops, 
           'k': self.num_keys, 
           'kb': self.keys_size}
        if self.num_values > 0:
