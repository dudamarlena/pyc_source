...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/tests/test_batch_size.py
...
import argparse, cProfile, logging, sys, time, pytest, kvlayer
from kvlayer.tests.test_interface import client, backend
import yakonfig
...
def scan_batch_size(client):
    ...
    for batch_size in range(1, 100):
        ...
        print '%d MB written in %.1f seconds --> %.1f MB/sec with batch_size=%d for storage_type=%s' % (
...
def main():
    ...
    if not args.storage_addresses:
        args.storage_addresses = [
         'redis.diffeo.com:6379']
    config['kvlayer'].update({'storage_type': args.storage_type, 
