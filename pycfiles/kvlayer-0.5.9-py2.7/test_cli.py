...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/tests/test_cli.py
...
import logging, uuid, pytest, kvlayer
...
def actions(namespace_string):
    with yakonfig.defaulted_config([kvlayer], params={'app_name': 'diffeo', 
...
def test_get_none(actions, a_key):
    ...
    assert actions.stdout.getvalue() == 'No values for key (' + repr(b_key) + ',).\n'