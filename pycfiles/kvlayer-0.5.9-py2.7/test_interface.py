...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/tests/test_interface.py
...
import uuid, pkg_resources, pytest, yaml
...
@pytest.fixture(scope='module', params=STORAGE_CLIENTS.keys())
def backend(request):
...
def client(backend, request, tmpdir, namespace_string):
    ...
    params = dict(app_name='kvlayer', namespace=namespace_string)
    ...
    with yakonfig.defaulted_config([
     kvlayer], config=file_config, params=params):
        client = kvlayer.client()
        client.delete_namespace()
        yield client
        if client._log_stats is not None:
            client._log_stats.flush()
            logger.info('storage stats (%s %s):\n%s', backend, request.function.__name__, file_config['kvlayer']['log_stats'].getvalue())
        client.delete_namespace()
    return
...
def test_adding_tables(client):
    client.setup_namespace(dict(t1=2, t2=3))
    u1, u2, u3 = uuid.uuid1(), uuid.uuid1(), uuid.uuid1()
    client.put('t1', ((u1, u2), '11'))
    client.put('t2', ((u1, u2, u3), '22'))
...
def test_clear_table(client):
    ...
    client.put('t2', *[ ((uuid.uuid4(), uuid.uuid4(), uuid.uuid4()), '') for i in xrange(num_rows)
                      ])
...
def test_bogus_put(client):
    ...
    with pytest.raises(BadKey):
        client.put('t1', *[ ((uuid.uuid4(), uuid.uuid4(), uuid.uuid4(), uuid.uuid4()), '') for i in xrange(num_rows)
                          ])
    with pytest.raises(BadKey):
        client.put('t2', *[ ((uuid.uuid4(), uuid.uuid4(), uuid.uuid4(), uuid.uuid4()), '') for i in xrange(num_rows)
                          ])
...
def test_binary_clean(client):
    ...
    vala = bytes(('').join([ chr(x) for x in xrange(0, 256) ]))
    client.put('t1', (keya, vala))
    keyb = (uuid.uuid4(), uuid.uuid4())
    valb = bytes(('').join([ chr(random.randint(0, 255)) for x in xrange(1000) ]))
...
def test_scan(client):
    ...
    for x in xrange(0, 100, 10):
        key = (
         uuid.UUID(int=x), uuid.UUID(int=x))
    ...
    expected_results = [
     40, 50, 60, 70]
    actual_results = [ int(v) for k, v in client.scan('t1', ((uuid.UUID(int=40),),
     (
      uuid.UUID(int=75),)))
                     ]
    assert actual_results == expected_results
    ...
    expected_results = [
     80, 90]
    actual_results = [ int(v) for k, v in client.scan('t1', ((uuid.UUID(int=80),), ()))
                     ]
    assert actual_results == expected_results
    expected_results = [
     0, 10]
    actual_results = [ int(v) for k, v in client.scan('t1', ((uuid.UUID(int=0),),
     (
      uuid.UUID(int=15),)))
                     ]
    assert actual_results == expected_results
    ...
    actual_results = [ int(v) for k, v in client.scan('t1', ((), (uuid.UUID(int=75),)))
                     ]
    assert actual_results == expected_results
...
def test_scan_2d(client):
    ...
    def kf(x, y):
        return (
         uuid.UUID(int=x), uuid.UUID(int=y))
...
def test_scan_2d_prefix(client):
    ...
    client.put('ts2', (('a', 'a'), '1'), (('a', 'z'), '2'), (('ab', 'b'), '3'), (('ab', 'y'),
                                                                                 '4'))
...
def test_scan_9042(client):
    ...
    kv = [ ((('k{0:5x}').format(x),), ('v{0:5x}').format(x)) for x in xrange(9042)
         ]
    client.put(tname, *kv)
    count = 0
...
def test_merge_join(client):
    ...
    client.setup_namespace({'t1': (
            str,), 
       't2': (
            str,)})
    client.put('t1', (('a',), 'one'), (('b',), 'two'), (('d',), 'four'))
    client.put('t2', (('a',), 'one'), (('c',), 'three'))
...
def test_partial_scan(client):
    ...
    client.put('t1', (('a',), 'one'), (('b',), 'two'), (('c',), 'three'), (('d',),
                                                                           'four'))
    s1 = client.scan('t1')
...
def test_scan_name_oddity(client):
    ...
    s = client.scan('index', (('NAME', 'a'), ('NAME', b'a\xff')))
    assert list(s) == [row]
    s = client.scan('index', (('NAME', 'al'), ('NAME', b'al\xff')))
    assert list(s) == [row]
    s = client.scan('index', (('NAME', 'ali'), ('NAME', b'ali\xff')))
    assert list(s) == [row]
    s = client.scan('index', (('NAME', 'alis'), ('NAME', b'alis\xff')))
    assert list(s) == [row]
    s = client.scan('index', (('NAME', 'alist'), ('NAME', b'alist\xff')))
    assert list(s) == [row]
