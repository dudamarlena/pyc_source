...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/_client.py
...
import argparse, logging, uuid, pkg_resources
try:
    import dblogger
except ImportError:
    dblogger = None
...
STORAGE_CLIENTS = dict(accumulo=AStorage, local=LocalStorage, filestorage=FileStorage, redis=RedisStorage)
if CStorage:
...
def client(config=None, storage_type=None, *args, **kwargs):
    ...
    if storage_type is None:
        try:
            ...
            raise ConfigurationError('No storage_type in kvlayer configuration')
    ...
    if storage_type not in STORAGE_CLIENTS:
        raise ConfigurationError(('Invalid kvlayer storage_type {0!r}').format(storage_type))
...
class Actions(ArgParseCmd):
    ...
    def args_keys(self, parser):
        ...
        parser.add_argument('schema', help='description of table keys', type=self._schema)
    ...
    def args_get(self, parser):
        parser.add_argument('table', help='name of kvlayer table')
        parser.add_argument('schema', help='description of table keys', type=self._schema)
        parser.add_argument('keys', nargs='+', help='key to fetch')
