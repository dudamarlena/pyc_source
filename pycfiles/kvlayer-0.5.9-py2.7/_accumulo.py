...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/_accumulo.py
...
import logging, random, re, struct
from pyaccumulo import Accumulo, Mutation, Range, BatchWriter
from pyaccumulo.iterators import SummingCombiner
from pyaccumulo.proxy.ttypes import IteratorScope, AccumuloSecurityException, IteratorSetting
...
class AStorage(StringKeyedStorage):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.thrift_framed_transport_size_in_mb = self._config['thrift_framed_transport_size_in_mb']
        self._conn = None
        return
    ...
    def conn(self):
        if not self._conn:
            ...
            logger.debug('connecting to Accumulo %s:%d', host, port)
            self._conn = Accumulo(host, port, self._user, self._password)
    ...
    def _create_table(self, table):
        ...
        self.conn.client.setTableProperty(self.conn.login, ns_table, 'table.bloom.enabled', 'true')
        self.conn.client.setTableProperty(self.conn.login, ns_table, 'table.cache.block.enable', 'true')
    ...
    def delete_namespace(self):
        ...
        logger.debug('finding tables to delete from %s: %r', self._ns(''), tables)
    ...
    def _put(self, table_name, keys_and_values, counter_deletes=True):
        cur_bytes = 0
        max_bytes = self.thrift_framed_transport_size_in_mb * 524288
        batch_writer = BatchWriter(conn=self.conn, table=self._ns(table_name), max_memory=self._max_memory, latency_ms=self._latency_ms, timeout_ms=self._timeout_ms, threads=self._threads)
        try:
            if self._value_types.get(table_name, str) is COUNTER and counter_deletes:
            ...
            for key, blob in keys_and_values:
                if len(key) + len(blob) + cur_bytes >= max_bytes:
                    logger.debug('len(key)=%d + len(blob)=%d + cur_bytes=%d >= thrift_framed_transport_size_in_mb/2 = %d', len(key), len(blob), cur_bytes, max_bytes)
    ...
    def _do_scan(self, table_name, key_ranges, keys_only):
        iterators = []
        if keys_only:
            iterators.append(IteratorSetting(name='SortedKeyIterator', priority=10, iteratorClass='org.apache.accumulo.core.iterators.SortedKeyIterator', properties={}))
        if not key_ranges:
            key_ranges = [
             [
              '', '']]
        for start_key, stop_key in key_ranges:
            ...
            if specific_key_range:
                ...
                key_range = Range(srow=start_key, erow=stop_key, sinclude=True, einclude=True)
                scanner = self.conn.scan(self._ns(table_name), scanrange=key_range, iterators=iterators)
            else:
                scanner = self.conn.scan(self._ns(table_name), iterators=iterators)
            for row in scanner:
                if keys_only:
                    yield row.row
                else:
    ...
    def increment(self, table_name, *keys_and_values):
        ...
        if self._value_types.get(table_name, str) is COUNTER:
            self.put(table_name, counter_deletes=False, *keys_and_values)
        else:
