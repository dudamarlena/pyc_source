...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/tests/test_cassandra_token_range.py
# Compiled at: 2015-07-31 13:31:44
from __future__ import absolute_import
import itertools, logging
from operator import attrgetter
import os, getpass, random, sys, time, uuid
try:
    import cql, pycassa
    from pycassa.pool import ConnectionPool
    from pycassa.system_manager import SystemManager, SIMPLE_STRATEGY, LEXICAL_UUID_TYPE, ASCII_TYPE, BYTES_TYPE
    from pycassa.types import CompositeType, TimeUUIDType, LexicalUUIDType, UUIDType, UTF8Type
except ImportError:
    cql = None
    pycassa = None

import pytest, yaml, kvlayer, yakonfig
...
def client(namespace_string, request):
    ...
    with yakonfig.defaulted_config([kvlayer], filename=config_path, params={'namespace': namespace_string, 'app_name': 'kvltest'}):
...
def test_composite_column_names(client):
    ...
    family = 'test'
    sm.create_column_family(namespace, family, super=False, key_validation_class=ASCII_TYPE, default_validation_class=BYTES_TYPE, comparator_type=CompositeType(UUIDType(), UUIDType()))
    logger.info(sm.describe_schema_versions())
    pool = ConnectionPool(namespace, config['storage_addresses'], max_retries=1000, pool_timeout=10, pool_size=2, timeout=120)
    cf = pycassa.ColumnFamily(pool, family)
    u1, u2, u3, u4 = (uuid.uuid1(), uuid.uuid1(), uuid.uuid1(), uuid.uuid1())
    cf.insert('inbound', {(u1, u2): ''})
    cf.insert('inbound', {(u1, u3): ''})
    cf.insert('inbound', {(u1, u4): ''})
    cf.insert('inbound', {(u1, u1): '45'})
    cf.insert('inbound', {(u3, u3): '42'})
    cf.insert('inbound', {(u3, u3): '43'})
    cf.insert('inbound', {(u3, u2): ''})
    cf.insert('inbound', {(u3, u4): ''})
    cf.insert('inbound', {(u2, u3): ''})
    cf.insert('inbound', {(u2, u4): ''})
    ...
    finish = uuid.UUID(int=340282366920938463463374607431768211455)
    ...
    cf.insert('inbound', {(u3, u3): ('').join(map(lambda u: u.bytes, (u1, u2, u3, u4)))})
    data = cf.get('inbound', column_start=(u3, u3), column_finish=(u3, u3)).items()[0][1]
    assert [
     u1, u2, u3, u4] == map(lambda b: uuid.UUID(bytes=b), grouper(data, 16))
    sm.close()
...
def test_composite_column_names_with_decomposited_keys(client):
    ...
    sm.create_keyspace(namespace, SIMPLE_STRATEGY, {'replication_factor': '1'})
    family = 'test'
    sm.create_column_family(namespace, family, super=False, key_validation_class=ASCII_TYPE, default_validation_class=BYTES_TYPE, comparator_type=UTF8Type())
    logger.info(sm.describe_schema_versions())
    pool = ConnectionPool(namespace, config['storage_addresses'], max_retries=1000, pool_timeout=10, pool_size=2, timeout=120)
    cf = pycassa.ColumnFamily(pool, family)
    u1, u2, u3, u4 = (uuid.uuid1(), uuid.uuid1(), uuid.uuid1(), uuid.uuid1())
    cf.insert('inbound', {join_uuids(u1, u2): ''})
    cf.insert('inbound', {join_uuids(u1, u3): ''})
    cf.insert('inbound', {join_uuids(u1, u4): ''})
    cf.insert('inbound', {join_uuids(u1, u1): '45'})
    cf.insert('inbound', {join_uuids(u3, u3): '42'})
    cf.insert('inbound', {join_uuids(u3, u3): '43'})
    cf.insert('inbound', {join_uuids(u3, u2): ''})
    cf.insert('inbound', {join_uuids(u3, u4): ''})
    cf.insert('inbound', {join_uuids(u2, u3): ''})
    cf.insert('inbound', {join_uuids(u2, u4): ''})
    ...
    assert rec2[1][0] == rec1[0][0]
    assert rec2[1][1] == rec1[0][1] == '43'
    cf.insert('inbound', {join_uuids(u3, u3): ('').join(map(lambda u: u.bytes, (u1, u2, u3, u4)))})
...
def test_composite_column_names_second_level_range_query(client):
    ...
    finish = uuid.UUID(int=u3.int + 1)
    assert start.int < u3.int < finish.int
    rec3 = cf.get('inbound', column_start=(
     u1, start), column_finish=(
     u1, finish)).items()
    assert len(rec3) == 1
    assert rec3[0][0][1] == u3
    for i in xrange(1000):
        cf.insert('inbound', {(u1, uuid.uuid4()): ''})

    step_size = 2 ** 126
    for i in range(4, 0, -1):
        start = uuid.UUID(int=(i - 1) * step_size)
        finish = uuid.UUID(int=min(i * step_size, 340282366920938463463374607431768211455))
        recs = cf.get('inbound', column_start=(
         u1, start), column_finish=(
         u1, finish)).items()
        for key, val in recs:
            assert val == ''
...
def test_composite_column_names_second_level_range_query_with_decomposited_keys(client):
    ...
    finish = uuid.UUID(int=u3.int + 1)
    assert start.int < u3.int < finish.int
    rec3 = cf.get('inbound', column_start=join_uuids(u1, start), column_finish=join_uuids(u1, finish)).items()
    assert len(rec3) == 1
    assert split_uuids(rec3[0][0])[1] == u3
    for i in xrange(1000):
        cf.insert('inbound', {join_uuids(u1, uuid.uuid4()): ''})

    step_size = 2 ** 126
    for i in range(4, 0, -1):
        start = uuid.UUID(int=(i - 1) * step_size)
        finish = uuid.UUID(int=min(i * step_size, 340282366920938463463374607431768211455))
        recs = cf.get('inbound', column_start=join_uuids(u1, start), column_finish=join_uuids(u1, finish)).items()
        for key, val in recs:
            key = split_uuids(key)
            assert val == ''
            assert key[0] == u1
...
def test_cql_token_range(client):
    ...
    for idx in xrange(total_inbound):
        cursor.execute("INSERT INTO data (k, v) VALUES (%d, '%s');" % (idx, ' '))
        count += 1
        if count % 10 == 0:
            elapsed = time.time() - start_time
            rate = float(count) / elapsed
    ...
    lowest_token = -1 * 9223372036854775808
    total_count = 0
    for worker_i in range(2 ** k):
        ...
        end_token = min(9223372036854775807, end_token)
        if worker_i == 2 ** k:
            pass
        limit = 1000
