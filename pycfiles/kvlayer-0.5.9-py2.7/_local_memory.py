...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/_local_memory.py
...
class AbstractLocalStorage(AbstractStorage):
    ...
    def setup_namespace(self, table_names, value_types={}):
        super(AbstractLocalStorage, self).setup_namespace(table_names, value_types)
    ...
    def delete_namespace(self):
        if self._app_name in self._data and self._namespace in self._data[self._app_name]:
...
class LocalStorage(AbstractLocalStorage):
    ...
    def __init__(self, config=None, app_name=None, namespace=None, *args, **kwargs):
        if config is None:
            config = {}
        ...
        super(LocalStorage, self).__init__(config=config, app_name=app_name, namespace=namespace, *args, **kwargs)
        return