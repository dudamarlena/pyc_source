...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kvlayer/_cbor_proxy.py
...
import json, logging, random, socket, struct, time, threading, cbor
...
class CborRpcClient(object):
    ...
    def __init__(self, config=None):
        ...
        if not (self._socket_family == socket.AF_INET and isinstance(self._socket_addr, tuple)):
            tsocket_addr = tuple(self._socket_addr)
            if not len(tsocket_addr) == 2:
                raise AssertionError(('address must be length-2 tuple ("hostname", port number), got {0!r} tuplified to {1!r}').format(self._socket_addr, tsocket_addr))
        ...
        while True:
            try:
                ...
                if not response['id'] == message['id']:
                    ...
                    errormessage = errormessage or repr(response)
                break
...
class CborProxyPooledConnection(object):
    ...
    def conn(self):
        if not self._conn:
            ...
            ok, msg = self._conn._rpc('connect', [
             unicode(zk_addr),
...
class CborProxyStorage(StringKeyedStorage):
    ...
    def setup_namespace(self, table_names, value_types=None):
        ...
        simple_table_names = dict([ (unicode(self._ns(k)), dict()) for k in table_names.iterkeys()
                                  ])
        with self.pooled_conn() as (conn):
            conn._rpc('setup_namespace', [simple_table_names, self._translate_value_config(value_types)])

    def delete_namespace(self):
        simple_table_names = dict([ (self._ns(k), dict()) for k in self._table_names.iterkeys()
                                  ])
        with self.pooled_conn() as (conn):
            conn._rpc('delete_namespace', [simple_table_names])

    def clear_table(self, table_name):
        table_name = self._ns(table_name)
        with self.pooled_conn() as (conn):
            conn._rpc('clear_table', [unicode(table_name)])
    ...
    def _scan(self, table_name, key_ranges):
        ...
        if not key_ranges:
            key_ranges = [
             (None, None)]
        cmd = [
         unicode(table_name), key_ranges]
        with self.pooled_conn() as (conn):
            while cmd is not None:
                resultob = conn._rpc('scan', cmd)
                results = None
                next_command = None
    ...
    def _scan_keys(self, table_name, key_ranges):
        ...
        with self.pooled_conn() as (conn):
            while cmd is not None:
                resultob = conn._rpc('scan_keys', cmd)
                results = None
                next_command = None
                if isinstance(resultob, list):
                    results = resultob
    ...
    def _get(self, table_name, keys):
        table_name = self._ns(table_name)
        with self.pooled_conn() as (conn):
            return conn._rpc('get', [unicode(table_name), keys])
    ...
    def _delete(self, table_name, keys, **kwargs):
        table_name = self._ns(table_name)
        with self.pooled_conn() as (conn):
            conn._rpc('delete', [unicode(table_name), keys])
    ...
    def increment(self, table_name, *keys_and_values):
        ...
        table_name = self._ns(table_name)
        with self.pooled_conn() as (conn):
            conn._rpc('increment', [unicode(table_name), convkv])
