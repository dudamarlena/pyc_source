...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\plugins\layout.py
...
LAYOUT_VAR = '_layout'
DEFAULT_LAYOUTS = {'table': {'key': 'table', 
             'icon': 'fa fa-table', 
             'name': _('Table'), 
...
class GridLayoutPlugin(BaseAdminPlugin):
    ...
    def result_item(self, item, obj, field_name, row):
        if self._current_layout == 'thumbnails':
            if getattr(item.attr, 'is_column', True):
                item.field_label = label_for_field(field_name, self.model, model_admin=self.admin_view, return_attr=False)
    ...
    def block_top_toolbar(self, context, nodes):
        if len(self._active_layouts) > 1:
            context.update({'layouts': self._active_layouts, 
               'current_icon': self._current_icon})
            nodes.append(loader.render_to_string('xadmin/blocks/model_list.top_toolbar.layouts.html', context=get_context_dict(context)))
