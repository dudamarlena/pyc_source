...
# [GCC 8.4.0]
# Embedded file name: E:\python\hhwork\extra_apps\xadmin\filters.py
# Compiled at: 2019-04-17 23:57:58
# Size of source mod 2**32: 22705 bytes
...
class RelatedFieldListFilter(ListFieldFilter):
    ...
    def has_output(self):
        if not (is_related_field(self.field) and self.field.field.null):
            if not hasattr(self.field, 'rel') or self.field.null:
                extra = 1
            else:
                extra = 0
    ...
    def choices(self):
        yield {'selected':self.lookup_exact_val == '' and not self.lookup_isnull_val, 
        ...
        if is_related_field(self.field) and self.field.field.null or hasattr(self.field, 'rel'):
            if self.field.null:
