...
class AdminSite(object):
    ...
    def _get_merge_attrs(self, option_class, plugin_class):
        return dict([ (name, getattr(option_class, name)) for name in dir(option_class) if name[0] != '_' and not callable(getattr(option_class, name)) and hasattr(plugin_class, name)
                    ])
    ...
    def _get_settings_class(self, admin_view_class):
        ...
        if name in self._registry_settings:
            ...
            if name.endswith('admin') and name[0:-5] in self._registry_settings:
                return self._registry_settings[name[0:-5]]
            if name.endswith('adminview') and name[0:-9] in self._registry_settings:
