...
# Size of source mod 2**32: 24268 bytes
...
class UserWidgetAdmin(object):
    ...
    def update_dashboard(self, obj):
        try:
            portal_pos = UserSettings.objects.get(user=(obj.user),
              key=('dashboard:%s:pos' % obj.page_id))
...
class ListWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    def context(self, context):
        ...
        context['results'] = [[o for i, o in enumerate(filter(lambda c: c.field_name in base_fields, r.cells))] for r in list_view.results()]
...
class AddFormWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    def setup(self):
        ...
        self.add_view = self.get_view_class(CreateAdminView,
          (self.model), list_per_page=10)(req)
...
class Dashboard(CommAdminView):
    ...
    def get_widgets(self):
        if self.widget_customiz:
            portal_pos = UserSettings.objects.filter(user=(self.user),
