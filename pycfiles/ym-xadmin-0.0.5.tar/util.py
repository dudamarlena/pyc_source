...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\util.py
# Compiled at: 2019-03-08 01:41:42
...
def lookup_needs_distinct(opts, lookup_path):
    ...
    if hasattr(field, 'rel') and isinstance(field.rel, models.ManyToManyRel) or is_related_field(field) and not field.field.unique:
        return True
    return False
...
def model_format_dict(obj):
    ...
    if isinstance(obj, (models.Model, models.base.ModelBase)):
        ...
        opts = obj
    return {'verbose_name': force_text(opts.verbose_name), 'verbose_name_plural': force_text(opts.verbose_name_plural)}
...
def boolean_icon(field_val):
    return mark_safe('<i class="%s" alt="%s"></i>' % ({True: 'fa fa-check-circle text-success', False: 'fa fa-times-circle text-error', None: 'fa fa-question-circle muted'}[field_val], field_val))
