...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\plugins\chart.py
...
class ChartWidget(ModelBaseWidget):
    ...
    def filte_choices_model(self, model, modeladmin):
        return bool(getattr(modeladmin, 'data_charts', None)) and super(ChartWidget, self).filte_choices_model(model, modeladmin)

    def get_chart_url(self, name, v):
        return self.model_admin_url('chart', name) + '?' + urlencode(self.list_params)

    def context(self, context):
        context.update({'charts': [ {'name': name, 'title': v['title'], 'url': self.get_chart_url(name, v)} for name, v in self.charts.items() ]})
...
class ChartsPlugin(BaseAdminPlugin):
    ...
    def block_results_top(self, context, nodes):
        context.update({'charts': [ {'name': name, 'title': v['title'], 'url': self.get_chart_url(name, v)} for name, v in self.data_charts.items() ]})
        nodes.append(loader.render_to_string('xadmin/blocks/model_list.results_top.charts.html', context=get_context_dict(context)))
...
class ChartsView(ListAdminView):
    ...
    def get(self, request, name):
        ...
        datas = [ {'data': [], 'label': force_text(label_for_field(i, self.model, model_admin=self))} for i in self.y_fields
                ]
        ...
        option = {'series': {'lines': {'show': True}, 'points': {'show': False}}, 'grid': {'hoverable': True, 'clickable': True}}
        try:
...
site.register_modelview('^chart/(.+)/$', ChartsView, name='%s_%s_chart')