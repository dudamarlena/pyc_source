...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        ...
        bookmarks_queryset = Bookmark.objects.filter(content_type=content_type, url_name='xadmin:%s_%s_changelist' % model_info).filter(Q(user=self.user) | Q(is_share=True))
...
class BookmarkView(ModelAdminView):
    ...
    def post(self, request):
        ...
        bookmark = Bookmark(content_type=ContentType.objects.get_for_model(self.model), title=request.POST['title'], user=self.user, query=request.POST.get('query', ''), is_share=request.POST.get('is_share', 0), url_name=url_name)
        bookmark.save()
