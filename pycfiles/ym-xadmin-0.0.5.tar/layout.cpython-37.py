...
# [GCC 8.4.0]
# Embedded file name: E:\python\hhwork\extra_apps\xadmin\layout.py
...
class Row(layout.Div):
    ...
    def __init__(self, *fields, **kwargs):
        ...
        (super(Row, self).__init__)(new_fields, css_class=css_class, **kwargs)

    def convert_field(self, f, counts):
        col_class = 'col-sm-%d' % int(math.ceil(12 / counts))
        if not isinstance(f, Field):
            if not issubclass(f.__class__, Field):
                f = layout.Field(f)
        elif f.wrapper_class:
...
class Col(layout.Column):
    ...
    def __init__(self, id, *fields, **kwargs):
        css_class = [
         ...
         'col col-sm-%d' % kwargs.get('span', 6)]
        if kwargs.get('horizontal'):
            css_class.append('form-horizontal')
        (super(Col, self).__init__)(fields, css_class=' '.join(css_class), **kwargs)
...
class InputGroup(layout.Field):
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, **kwargs):
        ...
        extra_context = {'inputs':self.inputs, 
         'input_size':self.input_size, 
         'classes':classes.replace('form-control', '')}
        if hasattr(self, 'wrapper_class'):
            extra_context['wrapper_class'] = self.wrapper_class
        return render_field(
 self.field, form, form_style, context, template=self.template, attrs=self.attrs, 
