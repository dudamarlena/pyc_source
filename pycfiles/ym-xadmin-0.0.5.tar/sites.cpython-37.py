...
# [GCC 8.4.0]
# Embedded file name: E:\python\hhwork\extra_apps\xadmin\sites.py
# Compiled at: 2019-04-17 23:57:58
# Size of source mod 2**32: 15518 bytes
import sys
...
class AdminSite(object):
    ...
    def check_dependencies(self):
        ...
        if not 'django.contrib.auth.context_processors.auth' in default_template_engine.context_processors:
            if not 'django.core.context_processors.auth' in default_template_engine.context_processors:
    ...
    def get_plugins(self, admin_view_class, *option_classes):
        ...
        for klass in admin_view_class.mro():
            if not klass == BaseAdminView:
                if issubclass(klass, BaseAdminView):
                    merge_opts = []
    ...
    def get_urls(self):
        ...
        urlpatterns = [
         url('^jsi18n/$', wrap((self.i18n_javascript), cacheable=True), name='jsi18n')]
        urlpatterns += [url(path, wrap(self.create_admin_view(clz_or_func)) if (inspect.isclass(clz_or_func) and issubclass(clz_or_func, BaseAdminView)) else (include(clz_or_func(self))), name=name) for path, clz_or_func, name in self._registry_views]
        for model, admin_class in iteritems(self._registry):
            ...
            urlpatterns += [
             url('^%s/%s/' % (model._meta.app_label, model._meta.model_name), include(view_urls))]
    ...
    def i18n_javascript(self, request):
        ...
        if settings.USE_I18N:
            ...
            import django.views.i18n as javascript_catalog
