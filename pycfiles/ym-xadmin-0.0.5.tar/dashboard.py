...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\views\dashboard.py
...
class WidgetTypeSelect(forms.Widget):
    ...
    def render_option(self, selected_choice, widget, id):
        if widget.widget_type == selected_choice:
            ...
            selected_html = ''
        return ('<li%s><a onclick="' + "javascript:$(this).parent().parent().find('>li').removeClass('active');$(this).parent().addClass('active');" + "$('#%s_input').attr('value', '%s')" % (id, widget.widget_type) + '"><h4><i class="%s"></i> %s</h4><p>%s</p></a></li>') % (
         selected_html,
         widget.widget_icon,
...
class UserWidgetAdmin(object):
    ...
    wizard_form_list = (
     ...
     (
      _('Widget Params'),
      {'callback': 'get_widget_params_form', 
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        if db_field.name == 'widget_type':
            ...
            return forms.ChoiceField(choices=[ (w.widget_type, w.description) for w in widgets ], widget=form_widget, label=_('Widget Type'))
...
class BaseWidget(forms.Form):
    ...
    @property
    def widget(self):
        context = {'widget_id': self.id, 'widget_title': self.title, 'widget_icon': self.widget_icon, 'widget_type': self.widget_type, 
           'form': self, 'widget': self}
...
class ModelChoiceField(forms.ChoiceField):
    ...
    def __init__(self, required=True, widget=None, label=None, initial=None, help_text=None, *args, **kwargs):
...
class AddFormWidget(ModelBaseWidget, PartialBaseWidget):
    ...
    def context(self, context):
        ...
        helper.form_tag = False
        helper.include_media = False
        context.update({'addform': self.add_view.form_obj, 
           'addhelper': helper, 
...
class Dashboard(CommAdminView):
    ...
    def get_init_widget(self):
        ...
        UserSettings(user=self.user, key='dashboard:%s:pos' % self.get_page_id(), value=('|').join([ (',').join([ str(w.id) for w in col ]) for col in portal ])).save()
        return portal
    ...
    def get_context(self):
        new_context = {'title': self.get_title(), 
           'icon': self.icon, 
           'portal_key': self.get_portal_key(), 
           'columns': [ ('col-sm-%d' % int(12 / len(self.widgets)), ws) for ws in self.widgets ], 'has_add_widget_permission': self.has_model_perm(UserWidget, 'add') and self.widget_customiz, 
           'add_widget_url': self.get_admin_url('%s_%s_add' % (UserWidget._meta.app_label, UserWidget._meta.model_name)) + '?user=%s&page_id=%s&_redirect=%s' % (self.user.id, self.get_page_id(), urlquote(self.request.get_full_path()))}
    ...
    @filter_hook
    def get_media(self):
        media = super(Dashboard, self).get_media() + self.vendor('xadmin.page.dashboard.js', 'xadmin.page.dashboard.css')
        if self.widget_customiz:
...
class ModelDashboard(Dashboard, ModelAdminView):
    ...
    @filter_hook
    def get_context(self):
        new_context = {'has_change_permission': self.has_change_permission(self.obj), 
           'object': self.obj}
