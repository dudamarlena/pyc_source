...
# [GCC 8.4.0]
# Embedded file name: E:\python\hhwork\extra_apps\xadmin\plugins\inline.py
# Compiled at: 2019-04-17 23:57:58
# Size of source mod 2**32: 18150 bytes
import copy, inspect
...
def replace_field_to_value(layout, av):
    if layout:
        ...
        for i, lo in enumerate(layout.fields):
            if not isinstance(lo, Field):
                if issubclass(lo.__class__, Field):
...
def replace_inline_objects(layout, fs):
    ...
    for i, layout_object in enumerate(layout.fields):
        if isinstance(layout_object, Inline):
            if layout_object.model in fs:
            ...
            for inline_class in self.inlines:
                ...
                if not inline.has_add_permission():
                    if not inline.has_change_permission():
                        if not inline.has_delete_permission():
                            if not inline.has_view_permission():
                                continue
                    else:
                        inline.max_num = inline.has_add_permission() or 0
