...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\views\detail.py
...
class ResultField(object):
    ...
    def init(self):
        self.label = label_for_field(self.field_name, self.obj.__class__, model_admin=self.admin_view, return_attr=False)
        try:
...
def replace_field_to_value(layout, cb):
    ...
    for i, lo in enumerate(layout.fields):
        if isinstance(lo, Field) or issubclass(lo.__class__, Field):
            layout.fields[i] = ShowField(cb, attrs=lo.attrs, wrapper_class=lo.wrapper_class, *lo.fields)
...
class DetailAdminView(ModelAdminView):
    ...
    def get_form_layout(self):
        ...
        if layout is None:
            layout = Layout(Container(Col('full', Fieldset('', css_class='unsort no_title', *self.form_obj.fields.keys()), horizontal=True, span=12)))
        elif type(layout) in (list, tuple) and len(layout) > 0:
            if isinstance(layout[0], Column):
                fs = layout
            elif isinstance(layout[0], (Fieldset, TabHolder)):
                fs = (
                 Col('full', horizontal=True, span=12, *layout),)
            else:
                fs = (
                 Col('full', Fieldset('', css_class='unsort no_title', *layout), horizontal=True, span=12),)
            layout = Layout(Container(*fs))
            if self.detail_show_all:
                rendered_fields = [ i[1] for i in layout.get_field_names() ]
                container = layout[0].fields
                other_fieldset = Fieldset(_('Other Fields'), *[ f for f in self.form_obj.fields.keys() if f not in rendered_fields ])
    ...
    def get_context(self):
        new_context = {'title': _('%s Detail') % force_text(self.opts.verbose_name), 
           'form': self.form_obj, 
           'object': self.obj, 
    ...
    @filter_hook
    def get_media(self):
        return super(DetailAdminView, self).get_media() + self.form_obj.media + self.vendor('xadmin.page.form.js', 'xadmin.form.css')
    ...
    def get_response(self, *args, **kwargs):
        ...
        response = TemplateResponse(self.request, self.detail_template or self.get_template_list('views/model_detail.html'), context)
        return response
