...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\plugins\sortablelist.py
...
class SortableListPlugin(BaseAdminPlugin):
    ...
    def result_row(self, __, obj):
        row = __()
        row.update({b'tagattrs': (b'order-key=order_{}').format(obj.pk)})
        return row

    def result_item(self, item, obj, field_name, row):
        if self.is_list_sortable and field_name == self.list_order_field:
            item.btns.append(b'<a><i class="fa fa-arrows"></i></a>')
        return item

    def get_context(self, context):
        context[b'save_order_url'] = self.get_model_url(self.admin_view.model, b'save_order')
        return context

    def block_top_toolbar(self, context, nodes):
        save_node = render_to_string(b'xadmin/blocks/model_list.top_toolbar.saveorder.html', context_instance=context)
        nodes.append(save_node)

    def get_media(self, media):
        if self.is_list_sortable:
            media = media + self.vendor(b'xadmin.plugin.sortablelist.js')
        return media
...
class SaveOrderView(ModelAdminView):
    ...
    def post(self, request):
        order_objs = request.POST.getlist(b'order[]')
...
site.register_modelview(b'^save-order/$', SaveOrderView, name=b'%s_%s_save_order')