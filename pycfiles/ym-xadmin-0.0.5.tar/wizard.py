...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\plugins\wizard.py
...
class WizardFormPlugin(BaseAdminPlugin):
    ...
    def get_form_list(self):
        init_form_list = hasattr(self, '_form_list') or OrderedDict()
        if not len(self.wizard_form_list) > 0:
            raise AssertionError('at least one form is needed')
    ...
    def get_step_form(self, step=None):
        ...
        if type(attrs) in (list, tuple):
            return modelform_factory(self.model, form=forms.ModelForm, fields=attrs, formfield_callback=self.admin_view.formfield_for_dbfield)
        else:
            if type(attrs) is dict:
                if attrs.get('fields', None):
                    return modelform_factory(self.model, form=forms.ModelForm, fields=attrs['fields'], formfield_callback=self.admin_view.formfield_for_dbfield)
                if attrs.get('callback', None):
                    callback = attrs['callback']
                    if callable(callback):
                        return callback(self)
                    if hasattr(self.admin_view, str(callback)):
    ...
    def get_step_form_obj(self, step=None):
        ...
        return form(prefix=self._get_form_prefix(step), data=self.storage.get_step_data(step), files=self.storage.get_step_files(step))

    def get_form_datas(self, datas):
        datas['prefix'] = self._get_form_prefix()
        if self.request.method == 'POST' and self.wizard_goto_step:
            datas.update({'data': self.storage.get_step_data(self.steps.current), 
    ...
    def block_before_fieldsets(self, context, nodes):
        ...
        context['wizard'] = {'steps': self.steps, 
           'management_form': ManagementForm(prefix=self.prefix, initial={'current_step': self.steps.current})}
