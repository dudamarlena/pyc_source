...
class BaseAdminObject(object):
    ...
    def get_model_url(self, model, name, *args, **kwargs):
        return reverse('%s:%s_%s_%s' % (self.admin_site.app_name, model._meta.app_label,
         model._meta.model_name, name), args=args, kwargs=kwargs, current_app=self.admin_site.name)
    ...
    def log(self, flag, message, obj=None):
        ...
        log = Log(user=self.user, ip_addr=ip_addr, action_flag=flag, message=message)
        if obj:
...
class CommAdminView(BaseAdminView):
    ...
    def get_nav_menu(self):
        ...
        for model, model_admin in self.admin_site._registry.items():
            ...
            elif ('first_icon' not in app_menu or app_menu['first_icon'] == self.default_model_icon) and model_dict.get('icon'):
