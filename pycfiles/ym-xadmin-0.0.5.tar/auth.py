...
# [GCC 8.4.0]
# Embedded file name: G:\python\hhwork\extra_apps\xadmin\plugins\auth.py
...
class UserAdmin(object):
    ...
    def get_form_layout(self):
        if self.org_obj:
            self.form_layout = (
             Main(Fieldset('', 'username', 'password', css_class='unsort no_title'), Fieldset(_('Personal info'), Row('first_name', 'last_name'), 'email'), Fieldset(_('Permissions'), 'groups', 'user_permissions'), Fieldset(_('Important dates'), 'last_login', 'date_joined')),
...
class ModelPermissionPlugin(BaseAdminPlugin):
    ...
    def get_list_display(self, list_display):
        if self.user_can_access_owned_objects_only and not self.user.is_superuser and self.user_owned_objects_field in list_display:
...
site.register_view('^%s/(.+)/password/$' % user_model, ChangePasswordView, name='user_change_password')
