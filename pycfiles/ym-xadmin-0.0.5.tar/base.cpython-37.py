...
# [GCC 8.4.0]
# Embedded file name: E:\python\hhwork\extra_apps\xadmin\views\base.py
# Compiled at: 2019-04-17 23:57:58
# Size of source mod 2**32: 22138 bytes
import copy, functools, datetime, decimal
    ...
    def wrap(func):
        ...
        def method(self, context, nodes, *arg, **kwargs):
            ...
            if isinstance(file_name, Template):
                ...
                if isinstance(file_name, cls_str) or is_iterable(file_name):
...
class BaseAdminObject(object):
    ...
    def get_model_url(self, model, name, *args, **kwargs):
        return reverse(('%s:%s_%s_%s' % (self.admin_site.app_name, model._meta.app_label,
    ...
    def get_form_params(self, new_params=None, remove=None):
        ...
        return mark_safe(''.join(('<input type="hidden" name="%s" value="%s"/>' % (k, v) for k, v in p.items() if v)))
    ...
    def message_user(self, message, level='info'):
        ...
        if hasattr(messages, level):
            if callable(getattr(messages, level)):
    ...
    def log(self, flag, message, obj=None):
        try:
            ip_addr = self.request.META['HTTP_X_FORWARDED_FOR'] if self.request.META['HTTP_X_FORWARDED_FOR'] else self.request.META['REMOTE_ADDR']
...
class BaseAdminView(BaseAdminObject, View):
    __doc__ = ' Base Admin view, support some comm attrs.'
        ...
        def view(request, *args, **kwargs):
            ...
            if hasattr(self, 'get'):
                if not hasattr(self, 'head'):
                    self.head = self.get
            elif self.request_method in self.http_method_names:
...
class CommAdminView(BaseAdminView):
    ...
    def get_nav_menu(self):
        ...
        for model, model_admin in self.admin_site._registry.items():
            if getattr(model_admin, 'hidden_menu', False):
                ...
                app_icon = None
                model_dict = {'title':smart_text(capfirst(model._meta.verbose_name_plural)), 
                ...
                nav_menu[app_key] = {'title':app_title,  'menus':[
                  model_dict]}
            app_menu = nav_menu[app_key]
            if app_icon:
                app_menu['first_icon'] = app_icon
            else:
                if 'first_icon' not in app_menu or app_menu['first_icon'] == self.default_model_icon:
                    if model_dict.get('icon'):
    ...
    def get_context(self):
        ...
        if (settings.DEBUG or 'nav_menu') in self.request.session:
        ...
        context.update({'menu_template':self.menu_template, 
         'nav_menu':nav_menu, 
...
class ModelAdminView(CommAdminView):
    ...
    def get_object_url(self, obj):
        ...
        if self.has_view_permission(obj):
    ...
    def get_model_perms(self):
        ...
        of those actions.
        """
        return {'view':self.has_view_permission(), 
    ...
    def has_view_permission(self, obj=None):
        ...
        return 'view' not in self.remove_permissions and (self.user.has_perm('%s.%s' % (self.app_label, view_codename)) or self.user.has_perm('%s.%s' % (self.app_label, change_codename)))
