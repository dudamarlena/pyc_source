...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/haus/components/genshiview.py
...
class GenshiTemplatingComponent(Component):
    ...
    def __init__(self, wrk):
        ...
        package_text_templates = wrk.pdfilename(config.get('text.templates', 'text'))
        package_markup_templates = wrk.pdfilename(config.get('markup.templates', config.get('templates.dir', 'html')))
        ...
        self.loader = TemplateLoader(template_search_path, auto_reload=config.get('reload', False))
