...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/haus/components/beakerstate.py
...
class BeakerSessionsComponent(Component):
    __module__ = __name__
    provides = [
     'get_session', 'invalidate_session']
    ...
    def __init__(self, wrk):
        ...
        self.environ_key = str(config.get('environ_key', wrk.package_name + '.session'))
    ...
    def __call__(self, wrk, *args, **kwargs):
        ...
        def middleware(app):
            ...
            def proxy(environ, start_response):
                session = self.get_session(environ)
                if session is not None and len(session.keys()) == 0:
                    session.save()
                try:
            ...
            newapp = SessionMiddleware(proxy, environ_key=self.environ_key, config=config)
...
class BeakerCacheComponent(Component):
    ...
    def __init__(self, wrk):
        ...
        self.environ_key = str(config.get('environ_key', wrk.package_name + '.cache'))
    ...
    def __call__(self, wrk, *args, **kwargs):
        ...
        def middleware(app):
            newapp = CacheMiddleware(app, environ_key=self.environ_key, config=config)
            newapp.__name__ = app.__name__
            return newapp
