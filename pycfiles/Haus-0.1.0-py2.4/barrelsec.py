...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/haus/components/barrelsec.py
...
class BarrelSecurityComponent(Component):
    ...
    __module__ = __name__
    consumes = [
     'invalidate_session', 'get_session', 'get_locations', 'render_view', 'valid_user', 'user_roles', 'get_user', 'login', 'logout', 'need_auth', 'denied']
    provides = [
     'valid_user', 'user_roles', 'get_user', 'login', 'logout', 'login_page', 'logout_page', 'need_auth', 'denied']
    ...
    def init_templates(self, wrk):
        ...
        self.template_login = config.get('login', resource_filename(haus_resource, 'haus/html/login.html'))
        self.template_logout = config.get('logout', resource_filename(haus_resource, 'haus/html/logout.html'))
        self.template_denied = config.get('denied', resource_filename(haus_resource, 'haus/html/denied.html'))
    ...
    def __call__(self, wrk, *args, **kwargs):
        ...
        config = wrk.config.get('barrel', {})
        auth_options = dict(realm=config.get('realm', wrk.package_name))
    ...
    def valid_user(self, username, password):
        ...
        return username and password and self.secrets.get(username) == password
    ...
    def need_auth(self, req):
        ...
        return req.redirect(req.uri(self.login_location) + '?uri=' + req.uri(with_qs=True))
    ...
    def login_page(self, req):
        ...
        if _username:
            alternate = req.query.get('uri', req.uri(self.landing_location))
            req.redirect(req.form.get('uri', alternate))
        template_vars = dict(message=message, username=username, layout=self.layout)
    ...
    def logout_page(self, req):
        ...
        template_vars = dict(layout=self.layout)
    ...
    def access_denied(self, req):
        ...
        template_vars = dict(username=get_user(req.environ), layout=self.layout)
        render = req.environ['haus.functions']['render_view']
        req.res.headers['Content-type'] = 'text/html'
