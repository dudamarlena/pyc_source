...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/haus/cli.py
...
from pkg_resources import iter_entry_points, Requirement, resource_filename
...
def run():
    """Run a haus app under wsgiref."""
    parser = OptionParser(usage=run_usage, version='haus run 0.1')
    parser.add_option('-c', '--configs', dest='config', help='Haus configuration files (comma separated).')
    parser.add_option('-r', '--reload', dest='reload', action='store_true', default=False, help='Reload the package being run.')
    parser.add_option('-R', '--reloadlist', dest='reloadlist', help='Reload packages that start with (comma seperated).')
    ...
    if len(args) == 3:
        ...
        try:
            ...
            if options.reload or options.reloadlist:
                ...
                if configs:
                    resolver_expression += ', '
                    resolver_expression += (', ').join(("'%s'" % c for c in configs))
                resolver_expression += ')'
...
def init():
    ...
    builder = Builder(data, filters=[StringTemplateFilter()], ignore=['.hg'])
