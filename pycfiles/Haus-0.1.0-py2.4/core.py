...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/haus/core.py
...
If your package has an ``init()`` method, it will be called
by the :ref:`AppInitComponent`.
...
def make_spec(filename):
    return ConfigObj(filename, encoding='UTF-8', list_values=False)
...
def tack(stack_descriptor, *args, **kwargs):
    ...
    def deco(app):
        app.haus_metadata = (
         stack_descriptor, args, kwargs)
        return app

    return deco


class BadStackDescriptorError(Exception):
    __module__ = __name__
...
class Haus(object):
    ...
    def pdfilename(self, filename):
        ...
        return resource_filename(self.package_resource, self.package_name + '/' + filename)

    def hauspdfilename(self, filename):
        """Use pkg_resources to get haus package data filenames."""
        return resource_filename(self.haus_resource, 'haus/' + filename)
    ...
    def load_config(self, *config_files):
        ...
        self.config = reduce(lambda x, y: x.merge(y) or x, (ConfigObj(f, configspec=config_spec) for f in config_files))
