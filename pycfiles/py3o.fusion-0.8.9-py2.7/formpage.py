...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3o/fusion/formpage.py
# Compiled at: 2018-07-03 11:28:47
import traceback, socket
from py3o.fusion.log import logging
import os, cgi, json, copy
...
class FormPage(Resource):
    ...
    def render_POST(self, request):
        ...
        form = cgi.FieldStorage(fp=request.content, headers=headers, environ={'REQUEST_METHOD': 'POST', 
           'CONTENT_TYPE': headers['content-type']})
        if 'skipfusion' in form:
            targetformat, pdf_options, error, reasons = self.get_target_format(form, [])
    ...
    def rendertarget(self, res):
        ...
        if not self.render_port or not self.render_server:
            newres['error'] = True
            newres['reasons'].append("Renderserver missing config '%s:%s'" % (
            ...
            try:
                try:
                    client.render(res['outname'], newres['outname'], target_format=targetformat, source_format='odt', pdf_options=res.get('pdf_options'))
                    ...
                    if self.clientdebug:
                        newres['reasons'].append('Renderserver traceback: %s' % tb_string)
                    print tb_string
    ...
    def rendered(self, res):
        logging.info('rendering thread finished for target %s' % res['targetformat'])
    ...
    def fusionned(self, res):
        ...
        if res.get('error', False):
            ...
            if res.get('targetformat') not in native_formats:
                d = deferToThread(self.rendertarget, res)
                d.addCallback(self.rendered)
                d.addErrback(self.error)
                return NOT_DONE_YET
    ...
    def reply_success(self, res):
        ...
        fileres = FileAutoDestroy(res['outname'], defaultType=res['outmime'])
    ...
    def get_target_format(self, form, reasons):
        ...
        if targetformat.lower() not in self.allowed_formats:
            error = True
            reasons.append(("target format: '{}' not supported by this server").format(targetformat))
        pdf_options = None
        ...
        return (
         targetformat, pdf_options, error, reasons)
    ...
    def get_image_mapping(self, form, error, reasons):
        ...
        if 'image_mapping' in form and form['image_mapping'].value:
            try:
                image_mapping = json.loads(form['image_mapping'].value)
            except ValueError as e:
                error = True
    ...
    def fusion(self, request, form):
        ...
        error = False
        reasons = []
        targetformat, pdf_options, error, reasons = self.get_target_format(form, reasons)
        image_mapping, error, reasons = self.get_image_mapping(form, error, reasons)
        ignore_undef_vars, error, reasons = self.get_ignore_undef_vars(form, error, reasons)
        escape_false, error, reasons = self.get_escape_false(form, error, reasons)
        fname, error, reasons = self.serialize_template(form, error, reasons)
        config = Py3oTypeConfig.from_odf_file(fname)
        decoder = Py3oJSONDecoder(config=config)
        datadict, error, reasons = self.get_datadict(form, error, reasons, decoder=decoder)
        ...
        if error:
            ...
            return {'request': request, 
               ...
               'targetformat': targetformat, 
               'pdf_options': pdf_options}
        t = Template(fname, outname, ignore_undefined_variables=ignore_undef_vars, escape_false=escape_false)
        tempimages, error, reasons = self.serialize_images(t, image_mapping, form, error, reasons)
        try:
            t.render(datadict)
        except ValueError as e:
            error = True
