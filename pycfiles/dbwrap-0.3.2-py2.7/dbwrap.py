...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dbwrap.py
# Compiled at: 2011-06-17 02:51:40
from cStringIO import StringIO
__all__ = [
 'row', 'rowset', 'table', 'database']
__version__ = '0.3.2'
...
class rowset(object):
    ...
    def select(self, *args):
        ...
        return self.con._query_custom('SELECT %s FROM %s %s' % (
         ...
         self.table.name,
         self.where_clause), self.table.row, self.args)
    ...
    def update(self, _clause='', *args, **values):
        ...
        self.con.execute(('UPDATE %s SET %s %s' % (
         ...
         self.where_clause)), *((args if _clause else tuple(values.values())) + tuple(self.args)))
...
class table(object):
    ...
    def _create_clause(self, _query, args, criteria):
        if _query:
            return _query
        return self._where_clause(*args, **criteria)
...
class database(object):
    ...
    def __getattr__(self, key):
        ...
        if key in self.__dict__:
            ...
            if key in self.table_classes:
                if key in self._accessed_tables:
                    return self._accessed_tables[key]
                return self._accessed_tables.setdefault(key, self.table_classes[key](self))
    ...
    def _query_one_custom(self, query, row_class=row, args=()):
        ...
        while result is None:
            if not (hasattr(cur, 'nextset') and cur.nextset()):
                break
            result = cur.fetchone()
