...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/address.py
...
class Address(object):
    ...
    def setUp(self):
        ...
        for WSA in toplist + WSA_LIST:
            if self.wsAddressURI is not None and self.wsAddressURI != WSA.ADDRESS or _has_type_definition(WSA.ADDRESS, epr) is True:
                break
        else:
        ...
        if pyobj is None:
            ...
            if value != self._addressTo:
                ...
                if scheme == schemeF and path == pathF and query == queryF and fragment == fragmentF:
                    ...
                    if netloc[1] == netlocF[1] and socket.gethostbyname(netloc[0]) == socket.gethostbyname(netlocF[0]):
                        return
