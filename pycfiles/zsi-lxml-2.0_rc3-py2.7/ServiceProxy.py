...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/ServiceProxy.py
...
class ServiceProxy():
    ...
    def __init__(self, wsdl, service=None, port=None, tracefile=None, nsdict=None, transdict=None):
        """
        Parameters:
    ...
    def _call(self, name, *args, **kwargs):
        ...
        binding = Binding(host=host, tracefile=self._tracefile, transport=transport, transdict=self._transdict, port=port, url=url, nsdict=self._nsdict, soapaction=soapAction)
        request, response = self._getTypeCodes(callinfo)
        if len(kwargs):
            args = kwargs
        if request is None:
            request = Any(oname=name)
        binding.Send(url=url, opname=None, obj=args, nsdict=self._nsdict, soapaction=soapAction, requesttypecode=request)
    ...
    def _getTypeCodes(self, callinfo):
        ...
        if request and prefix and callinfo.use == 'encoded':
            request.oname = '%(prefix)s:%(name)s xmlns:%(prefix)s="%(namespaceURI)s"' % {'prefix': prefix, 'name': request.aname, 'namespaceURI': callinfo.namespace}
        return (
         request, response)
    ...
    def _globalElement(self, typeCode, namespaceURI, literal):
        ...
        """
        if literal:
            typeCode.oname = '%(prefix)s:%(name)s xmlns:%(prefix)s="%(namespaceURI)s"' % {'prefix': self._getPrefix(namespaceURI), 'name': typeCode.oname, 'namespaceURI': namespaceURI}
