...
def _get_xsitype(pyclass):
    ...
    if hasattr(pyclass, 'type') and type(pyclass.type) in _seqtypes:
        ...
        if hasattr(pyclass, 'type') and hasattr(pyclass, 'schema'):
...
class TypeCode():
        ...
        if ns == SOAP.ENC:
            if parselist and (
             None, name) not in parselist and (ns, name) not in parselist:
                raise EvaluateException('Element mismatch (got %s wanted %s) (SOAP encoding namespace)' % (
    ...
    def checktype(self, elt, ps):
        ...
        parselist, errorlist = self.get_parse_and_errorlist()
        if not parselist or (
         uri, typeName) in parselist or _is_xsd_or_soap_ns(uri) and (None, typeName) in parselist:
            return (uri, typeName)
        else:
            raise EvaluateException('Type mismatch (%s namespace) (got %s wanted %s)' % (
    ...
    def name_match(self, elt):
        ...
        Parameters:
            elt -- the DOM element being parsed
        """
        return self.pname == elt.localName and self.nspname in [None, elt.namespaceURI]
    ...
    def simple_value(self, elt, ps, mixed=False):
        ...
        return ('').join([ E.nodeValue for E in c if E.nodeType in [
         _Node.TEXT_NODE, _Node.CDATA_SECTION_NODE]
                         ])
...
class Any(TypeCode):
    ...
    def parse(self, elt, ps):
        ...
        if len(_children(elt)) == 0:
            ...
            if not href:
                if self.minOccurs < 1:
                    ...
                    if (
                     ns, type) == (SOAP.ENC, 'Array') or (_find_arraytype(elt) or '').endswith('[0]'):
                        return []
                    return
        ...
        if not type or (ns, type) == (SOAP.ENC, 'Array'):
            if self.aslist or _find_arraytype(elt):
                return [ self.__class__(**self.kwargs).parse(e, ps) for e in _child_elements(elt)
                       ]
            if len(_child_elements(elt)) == 0:
                raise EvaluateException('Any cannot parse untyped element', ps.Backtrace(elt))
    ...
    def get_formatted_content(self, pyobj):
        ...
        if tc == types.InstanceType:
            ...
            if not serializer:
                tc = (
                 types.ClassType, pyobj.__class__.__name__)
                serializer = Any.serialmap.get(tc)
        else:
            serializer = Any.serialmap.get(tc)
        ...
        if hasattr(pyobj, 'typecode') and pyobj.typecode is not self:
            ...
            if tc in _seqtypes:
                if self.aslist:
                    ...
                    for o in pyobj:
                        serializer = getattr(o, 'typecode', self.__class__())
                ...
                if tc == types.InstanceType:
                    ...
                    if not serializer and isinstance(pyobj, time.struct_time):
                        from ZSI.TCtimes import gDateTime
                        serializer = gDateTime()
                if serializer or pyobj is None:
...
class String(SimpleType):
    ...
    def get_formatted_content(self, pyobj):
        if type(pyobj) not in _stringtypes:
            pyobj = str(pyobj)
        if type(pyobj) == types.UnicodeType:
...
class URI(String):
    ...
    """
    parselist = [
     (None, 'anyURI'), (SCHEMA.XSD3, 'anyURI')]
...
class Base64String(String):
    ...
    """
    parselist = [
     (None, 'base64Binary'), (SOAP.ENC, 'base64')]
    ...
    def parse(self, elt, ps):
        ...
        if self.nilled(elt, ps):
            ...
            if type is None:
                ...
                raise EvaluateException('Integer type mismatch; got %s wanted %s' % (
                 type, self.type[1]), ps.Backtrace(elt))
            v = self.simple_value(elt, ps)
...
class Decimal(SimpleType):
    ...
    """
    parselist = [
     (None, 'decimal'), (None, 'float'), (None, 'double')]
    seriallist = _floattypes
    type = None
    ranges = {'float': (7.006492321624085e-46, -3.4028234663852886e+38, 3.4028234663852886e+38), 
        ...
        if not elt:
            ...
            if tag:
                if type is None:
                    ...
                    raise EvaluateException('Floating point type mismatch; got (%s,%s) wanted %s' % (
                     ns, type, tag), ps.Backtrace(elt))
            if self.nilled(elt, ps):
                return Nilled
            v = self.simple_value(elt, ps)
            try:
...
class AnyType(TypeCode):
    ...
    def __init__(self, pname=None, namespaces=['#all'], minOccurs=1, maxOccurs=1, strip=1, **kw):
    ...
    def serialize(self, elt, sw, pyobj, **kw):
        ...
        if what is None:
            ...
            what.serialize(elt, sw, pyobj, name=(
             self.nspname or what.nspname, self.pname or what.pname), **kw)
            return
...
class List(SimpleType):
    ...
    def __init__(self, pname=None, itemType=None, **kw):
        ...
        """
        assert kw.get('maxOccurs', 1) == 1, 'Currently only supporting SimpleType Lists with  maxOccurs=1'
        ...
        if self.nilled(elt, ps):
            ...
            for ce in _child_elements(elt):
                ...
                if name != previousName:
                    if currentElementList != None:
                        groupedElements.append((previousName, currentElementList))
                    currentElementList = list()
                currentElementList.append(ce)
                previousName = name

            if currentElementList != None:
                groupedElements.append((previousName, currentElementList))
            if len(groupedElements) < 1:
                return
            if len(groupedElements) == 1 and len(groupedElements[0][0]) > 1:
                self.aslist = False
...
_get_type_definition, _get_global_element_declaration, Wrap = (
 GTD, GED, WrapImmutable)
