...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/generate/commands.py
...
def SetPyclassMetaclass(option, opt, value, parser, *args, **kwargs):
    ...
    TypesHeaderContainer.imports.append('from %(module)s import %(metaclass)s' % kwargs)
    ServiceHeaderContainer.imports.append('from %(module)s import %(metaclass)s' % kwargs)
...
def wsdl2py(args=None):
    ...
    op.add_option('-b', '--complexType', action='callback', callback=SetPyclassMetaclass, callback_kwargs={'module': 'ZSI.generate.pyclass', 'metaclass': 'pyclass_type'}, help='add convenience functions for complexTypes, including Getters, Setters, factory methods, and properties (via metaclass). *** DONT USE WITH --simple-naming ***')
    op.add_option('-l', '--lazy', action='callback', callback=SetUpLazyEvaluation, callback_kwargs={}, help='EXPERIMENTAL: recursion error solution, lazy evalution of typecodes')
    op.add_option('-w', '--twisted', action='callback', callback=SetUpTwistedClient, callback_kwargs={'module': 'ZSI.generate.pyclass', 'metaclass': 'pyclass_type'}, help='generate a twisted.web client, dependencies python>=2.4, Twisted>=2.0.0, TwistedWeb>=0.5.0')
    ...
    op.add_option('-o', '--output-dir', action='store', dest='output_dir', default='.', type='string', help='Write generated files to OUTPUT_DIR')
    ...
    if options.aname is not None:
        ...
        if not callable(FUNC):
            raise AssertionError('%s must be a callable method with one string parameter' % options.aname)
            from ZSI.generate.containers import TypecodeContainerBase
        ...
        if options.schema is False:
            ...
            options.simple_naming or wsm.writeClient(fd)
        else:
...
def wsdl2dispatch(args=None):
    ...
    op.add_option('-u', '--url', action='store', dest='url', default=None, type='string', help='URL to load wsdl from')
    op.add_option('-a', '--address', action='store_true', dest='address', default=False, help='ws-addressing support, must include WS-Addressing schema.')
    op.add_option('-e', '--extended', action='store_true', dest='extended', default=False, help='Extended code generation.')
    op.add_option('-d', '--debug', action='callback', callback=SetDebugCallback, help='debug output')
    op.add_option('-t', '--types', action='store', dest='types', default=None, type='string', help='Write generated files to OUTPUT_DIR')
    op.add_option('-o', '--output-dir', action='store', dest='output_dir', default='.', type='string', help='file to load types from')
    op.add_option('-s', '--simple-naming', action='store_true', dest='simple_naming', default=False, help='Simplify generated naming.')
    if args is None:
        options, args = op.parse_args()
    else:
