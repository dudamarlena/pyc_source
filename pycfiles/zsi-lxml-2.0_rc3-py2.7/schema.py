...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/schema.py
...
class TypeDefinition:
    ...
    def getSubstituteType(self, elt, ps):
        ...
        if pyclass is None:
            raise EvaluateException('No Type registed for xsi:type=(%s, %s)' % (
        ...
        if subclass is None:
            raise EvaluateException('No registered xsi:type=(%s, %s), substitute for xsi:type=(%s, %s)' % (
        ...
        if not issubclass(subclass, pyclass) and subclass(None) and not issubclass(subclass, pyclass):
            raise TypeError('Substitute Type (%s, %s) is not derived from %s' % (
...
class _Mirage:
    ...
    def _reveal_type(self):
        if self.__cache is None:
            self.__cache = self.klass(pname=self.pname, aname=self.aname, minOccurs=self.minOccurs, maxOccurs=self.maxOccurs, nillable=self.nillable, **self.__kw)
        return self.__cache

    def _reveal_element(self):
        if self.__cache is None:
            self.__cache = self.klass(minOccurs=self.minOccurs, maxOccurs=self.maxOccurs, nillable=self.nillable, **self.__kw)
        return self.__cache
