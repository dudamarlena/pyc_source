...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/generate/wsdl2python.py
...
class MessageWriter:
    ...
    def setUp(self, soc, port, input=False):
        assert isinstance(soc, ServiceOperationContainer), 'expecting a ServiceOperationContainer instance'
        assert isinstance(port, WSDLTools.Port), 'expecting a WSDL.Port instance'
...
class SchemaDescription:
    ...
    def fromSchema(self, schema):
        ...
        assert self.targetNamespace is None or self.targetNamespace == ns, 'SchemaDescription instance represents %s, not %s' % (
