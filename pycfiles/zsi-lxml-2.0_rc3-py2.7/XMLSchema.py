...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/wstools/XMLSchema.py
...
class XMLSchemaComponent(XMLBase, MarkerInterface):
    ...
    def __init__(self, parent=None):
        ...
        if not self.__class__ == XMLSchemaComponent and not (type(self.__class__.required) == type(XMLSchemaComponent.required) and type(self.__class__.attributes) == type(XMLSchemaComponent.attributes) and type(self.__class__.contents) == type(XMLSchemaComponent.contents)):
        ...
        for k in ['memberTypes']:
            if self.attributes.has_key(k):
                ...
                for qname in qnames.split():
                    ...
                    self.attributes['memberTypes'].append(TypeDescriptionComponent((
    ...
    def __checkAttributes(self):
        ...
        for a in self.__class__.required:
            if not self.attributes.has_key(a):
                raise SchemaError, 'class instance %s, missing required attribute %s' % (
                 self.__class__, a)
        ...
        for a, v in self.attributes.items():
            ...
            if a in (XMLSchemaComponent.xmlns, XMLNS.XML):
                continue
            if a not in self.__class__.attributes.keys() and not (self.isAttribute() and self.isReference()):
...
class XMLSchema(XMLSchemaComponent):
    ...
    """
    attributes = {'id': None, 'version': None, 
       'xml:lang': None, 
       ...
       'blockDefault': None, 
       'finalDefault': None}
    contents = {'xsd': ('include', 'import', 'redefine', 'annotation', 'attribute', 'attributeGroup', 'complexType',
 'element', 'group', 'notation', 'simpleType', 'annotation')}
    ...
    def load(self, node, location=None):
        ...
        for childNode in self.getContents(node):
            ...
            if component == 'include':
                ...
                tp.fromDom(childNode)
                import_ns = tp.getAttribute('namespace') or self.__class__.empty_namespace
    ...
    class Import(XMLSchemaComponent):
        """<import> 
           ...
           contents:
               annotation?
        """
        attributes = {'id': None, 'namespace': None, 
           'schemaLocation': None}
        contents = {'xsd': ['annotation']}
        tag = 'import'
    ...
    class Include(XMLSchemaComponent):
        ...
        required = [
         'schemaLocation']
        attributes = {'id': None, 'schemaLocation': None}
        contents = {'xsd': ['annotation']}
        tag = 'include'
...
class AttributeWildCard(XMLSchemaComponent, AttributeMarker, DeclarationMarker, WildCardMarker):
    """<anyAttribute>
       ...
       contents:
           annotation?
    """
    attributes = {'id': None, 'namespace': '##any', 
...
class ElementWildCard(LocalElementDeclaration, WildCardMarker):
    ...
    """
    required = []
    attributes = {'id': None, 'minOccurs': '1', 
       'maxOccurs': '1', 
       'namespace': '##any', 
       'processContents': 'strict'}
    contents = {'xsd': ['annotation']}
    ...
    class Union(XMLSchemaComponent, UnionMarker):
        """<union>
           ...
           contents:
               annotation?, simpleType*
        """
        attributes = {'id': None, 'memberTypes': None}
        contents = {'xsd': ['annotation', 'simpleType']}
        tag = 'union'
    ...
    class List(XMLSchemaComponent, ListMarker):
        """<list>
           ...
           contents:
               annotation?, simpleType?
        """
        attributes = {'id': None, 'itemType': None}
        contents = {'xsd': ['annotation', 'simpleType']}
        tag = 'list'
