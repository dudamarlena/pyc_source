...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/fault.py
...
FaultType.typecode = Struct(FaultType, [
 ...
 Detail.typecode,
 AnyElement(aname='any', minOccurs=0, maxOccurs=UNBOUNDED)], pname=(
 SOAP.ENV, 'Fault'), inline=True, hasextras=0)
...
class ZSIFaultDetailTypeCode(ElementDeclaration, Struct):
    ...
    def __init__(self, **kw):
        Struct.__init__(self, ZSIFaultDetail, [String(pname=(ZSI_SCHEMA_URI, 'string')),
         String(pname=(ZSI_SCHEMA_URI, 'trace'), minOccurs=0)], pname=(
         ZSI_SCHEMA_URI, 'FaultDetail'), **kw)
...
class URIFaultDetailTypeCode(ElementDeclaration, Struct):
    ...
    def __init__(self, **kw):
        Struct.__init__(self, URIFaultDetail, [
         String(pname=(ZSI_SCHEMA_URI, 'URI')), String(pname=(ZSI_SCHEMA_URI, 'localname'))], pname=(
         ZSI_SCHEMA_URI, 'URIFaultDetail'), **kw)
...
class ActorFaultDetailTypeCode(ElementDeclaration, Struct):
    ...
    def __init__(self, **kw):
        Struct.__init__(self, ActorFaultDetail, [String(pname=(ZSI_SCHEMA_URI, 'URI'))], pname=(
         ZSI_SCHEMA_URI, 'ActorFaultDetail'), **kw)
...
def FaultFromException(ex, inheader, tb=None, actor=None):
    ...
    if tb:
        try:
            lines = ('\n').join([ '%s:%d:%s' % (name, line, func) for name, line, func, text in traceback.extract_tb(tb)
                                ])
        except:
            pass
        else:
