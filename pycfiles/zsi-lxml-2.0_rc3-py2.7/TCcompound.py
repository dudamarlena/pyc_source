...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/TCcompound.py
...
from schema import ElementDeclaration, TypeDefinition, _get_substitute_element, _get_type_definition
        ...
        if isinstance(sub, ElementDeclaration):
            if (
             typecode.nspname, typecode.pname) == (sub.nspname, sub.pname):
                raise TypeError('bad usage, failed to serialize element reference (%s, %s), in: %s' % (
                 typecode.nspname, typecode.pname, sw.Backtrace(elt)))
            raise TypeError('failed to serialize (%s, %s) illegal sub GED (%s,%s): %s' % (
             typecode.nspname, typecode.pname, sub.nspname, sub.pname,
             sw.Backtrace(elt)))
        if not isinstance(typecode, AnyType) and not isinstance(sub, typecode.__class__):
            raise TypeError('failed to serialize substitute %s for %s,  not derivation: %s' % (
...
class ComplexType(TypeCode):
            ...
            for i, what in [ (i, self.ofwhat[i]) for i in range(len(self.ofwhat)) ]:
                ...
                for j, c_elt in [ (j, c[j]) for j in crange if c[j] ]:
                    ...
                    if self.inorder is True and i == j:
                        raise EvaluateException('Out of order complexType', ps.Backtrace(c_elt))
                else:
    ...
    def cb(self, elt, sw, pyobj, name=None, **kw):
        ...
        if self.inline:
            ...
            if not self.inline and self.unique:
            ...
            while indx < lenofwhat:
                ...
                if whatTC.maxOccurs > 1 and v is not None:
                    if type(v) not in _seqtypes:
                        raise EvaluateException('pyobj (%s,%s), aname "%s": maxOccurs %s, expecting a %s' % (
                         self.nspname, self.pname, what.aname, whatTC.maxOccurs, _seqtypes), sw.Backtrace(elt))
                    for v2 in v:
                        occurs += 1
                        if occurs > whatTC.maxOccurs:
                            raise EvaluateException('occurances (%d) exceeded maxOccurs(%d) for <%s>' % (
                             occurs, whatTC.maxOccurs, what.pname), sw.Backtrace(elt))
                ...
                raise EvaluateException('Got None for nillable(%s), minOccurs(%d) element (%s,%s), %s' % (
                 what.nillable, what.minOccurs, what.nspname, what.pname, elem), sw.Backtrace(elt))
