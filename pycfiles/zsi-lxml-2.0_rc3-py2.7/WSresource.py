...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/twisted/WSresource.py
...
class WSAddressHandler:
    ...
    def processRequest(self, ps, **kw):
        ...
        if d is None or d.has_key(key) is False:
            raise RuntimeError, 'Error looking for key(%s) in root dictionary(%s)' % (key, str(d))
        self.op_name = d[key]
        ...
        if soap_action and soap_action.strip('\'"') != action:
            raise WSActionException('SOAP Action("%s") must match WS-Action("%s") if specified.' % (
             soap_action, action))
        ps.address = address
        return ps
    ...
    def processResponse(self, sw, **kw):
        if sw is None:
            ...
            key = self.op_name
            if d is None or d.has_key(key) is False:
                raise WSActionNotSpecified, 'Error looking for key(%s) in wsAction dictionary(%s)' % (key, str(d))
            addressRsp = Address(action=d[key])
            if request.transport.TLS == 0:
                addressRsp.setResponseFromWSAddress(self.address, 'http://%s:%d%s' % (
                 request.host.host, request.host.port, request.path))
            else:
                addressRsp.setResponseFromWSAddress(self.address, 'https://%s:%d%s' % (
                 request.host.host, request.host.port, request.path))
...
class WSAddressCallbackHandler:
    ...
    def processRequest(cls, ps, **kw):
        ...
        resource = kw['resource']
        request = kw['request']
        method = getattr(resource, 'wsa_%s' % _get_element_nsuri_name(ps.body_root)[(-1)])
        try:
            req_pyobj, rsp_pyobj = method(ps, ps.address, request=request)
        except TypeError as ex:
            log.err('ERROR: service %s is broken, method MUST return request, response' % self.__class__.__name__)
            raise
        except Exception as ex:
            log.err('failure when calling bound method')
            raise
...
class WSAddressHandlerChainFactory:
    ...
    @classmethod
    def newInstance(cls):
        return cls.protocol(WSAddressCallbackHandler, DataHandler, WSAddressHandler())
