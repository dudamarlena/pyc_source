...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ZSI/generate/containers.py
...
from ZSI.generate import WSISpec, WSInteropError, Wsdl2PythonError, WsdlGeneratorError, WSDLFormatError
ID1 = '    '
ID2 = 2 * ID1
ID3 = 3 * ID1
ID4 = 4 * ID1
        ...
        if not attributes:
            ...
            for a in attributes:
                if a.isWildCard() and a.isDeclaration():
                    atd_list.append('%s[("%s","anyAttribute")] = ZSI.TC.AnyElement()' % (
                     atd, SCHEMA.XSD3))
                    ...
                    if a.getAttribute('form') == 'qualified':
                        ...
                        raise ContainerError, 'attribute form must be un/qualified %s' % a.getAttribute('form')
                    ...
                    if ga is None:
                        ...
                        atd_list.append('%s[%s] = ZSI.TC.String()' % (atd, key))
                    elif tp is None:
                        try:
...
class ContainerBase():
    ...
    def getAttributeName(self, name):
        ...
        if self.func_aname is None:
            ...
            assert callable(self.func_aname), 'expecting callable method for attribute func_aname, not %s' % type(self.func_aname)
            f = self.func_aname
            return f(name)
...
class ServiceLocatorContainer(ServiceContainerBase):
    ...
    def setUp(self, service):
        assert isinstance(service, WSDLTools.Service), 'expecting WDSLTools.Service instance.'
...
class ServiceOperationContainer(ServiceContainerBase):
    ...
    def setUp(self, item):
        ...
        if op is None:
            raise WSDLFormatError('<portType name="%s"/> no match for <binding name="%s"><operation name="%s">' % (
        ...
        if sbody.use == 'encoded':
            if not sbody.encodingStyle == SOAP.ENC:
                raise AssertionError('Supporting encodingStyle=%s, not %s' % (SOAP.ENC, sbody.encodingStyle))
            ...
            if bop.output is not None:
                ...
                raise item.output or WSDLFormatError, 'Operation %s, no match for output binding' % name
    ...
    def _setContent(self):
        ...
        if self.do_extended:
            ...
            try:
                ...
                raise Wsdl2PythonError, 'Extended generation failure: only supports doc/lit, ' + 'and all element attributes (<message><part element=' + '"my:GED"></message>) must refer to single global ' + 'element declaration with complexType content.  ' + '\n\n**** TRY WITHOUT EXTENDED ****\n'
        ...
        if self.outputName:
            ...
            if self.isRPC() and not self.isLiteral():
                response.append('%stypecode = Struct(pname=None, ofwhat=%s.typecode.ofwhat, pyclass=%s.typecode.pyclass)' % (
                 ID2, self.outputName, self.outputName))
                response.append('%sresponse = self.binding.Receive(typecode%s)' % (
                 ID2, responseArgs))
            else:
                response.append('%sresponse = self.binding.Receive(%s.typecode%s)' % (
...
class ServiceOperationsClassContainer(ServiceContainerBase):
    ...
    def setUp(self, port):
        ...
        for bop in port.getBinding().operations:
            soap_bop = bop.findBinding(WSDLTools.SoapOperationBinding)
            if soap_bop is None:
                self.logger.warning('Skip port(%s) operation(%s) no SOAP Binding Operation' % (
                 port.name, bop.name))
                continue
            if bop.input is not None:
                soapBodyBind = bop.input.findBinding(WSDLTools.SoapBodyBinding)
                if soapBodyBind is None:
                    self.logger.warning('Skip port(%s) operation(%s) Bindings(%s) not supported' % (
            ...
            c = self.operationclass(useWSA=self.useWSA, do_extended=self.do_extended)
            c.setUp(bop)
...
class ServiceDocumentLiteralMessageContainer(ServiceContainerBase, MessageContainerInterface):
    ...
    def _setContent(self):
        ...
        kw = KW.copy()
        kw['message'], kw['prefix'], kw['typecode'] = self.content.mName, self.getNSAlias(), element_class_name(self.content.pName)
...
class ServiceRPCEncodedMessageContainer(ServiceContainerBase, MessageContainerInterface):
    ...
    def _setContent(self):
        ...
        assert encodingStyle == SOAP.ENC, 'Supporting encodingStyle=%s, not %s' % (SOAP.ENC, encodingStyle)
        ...
        fdict['typecode'] = 'Struct(pname=("%(nspname)s","%(pname)s"), ofwhat=%(ofwhat)s, pyclass=%(pyclass)s, encoded="%(encoded)s")'
        message = [
         'class %(pyclass)s:',
...
class TypecodeContainerBase(TypesContainerBase):
    ...
    def getvalue(self):
        ...
        for item in self.localTypes:
            content = None
            if not True is item.isElement() is item.isLocal():
                raise AssertionError('expecting local elements only')
                etp = item.content
                qName = item.getAttribute('type')
                etp = qName or item.content
                local = True
            else:
            ...
            if etp is None:
                ...
                out += '\n\n'
                if self.parentClass:
                    content.parentClass = '%s.%s' % (self.parentClass, self.getClassName())
                else:
    ...
    def getPyClassDefinition(self):
        ...
        if self.hasExtPyClass():
            ...
            if self.name is not None:
                kw['name'] = self.name
                definition.append('%(ID3)s%(pyclass)s.__name__ = "%(name)s_Holder"' % kw)
            return definition
    ...
    def _setTypecodeList(self):
        ...
        if type(content) is not tuple:
            mg = content
            if not mg.isModelGroup():
                raise Wsdl2PythonErr('Expecting ModelGroup: %s' % mg.getItemTrace())
            self.logger.debug('ModelGroup(%r) contents(%r): %s' % (
             mg, mg.content, mg.getItemTrace()))
            if mg.isReference():
                raise RuntimeError('Unexpected modelGroup reference: %s' % mg.getItemTrace())
            if mg.isDefinition():
                mg = mg.content
            if mg.isAll():
                flat = mg.content
    ...
    def getBasesLogic(self, indent):
        ...
        bases.append('%sbases = list(%s.%s.__bases__)' % (
         ID1, self.getNSAlias(), self.getClassName()))
        bases.append('%sbases.insert(0, %s.%s)' % (
         ID1, NAD.getAlias(self.sKlassNS), type_class_name(self.sKlass)))
        bases.append('%s%s.%s.__bases__ = tuple(bases)' % (
         ID1, self.getNSAlias(), self.getClassName()))
        s = ''
        for b in bases:
            s += '%s%s\n' % (indent, b)
...
class MessageTypecodeContainer(TypecodeContainerBase):
    ...
    def _setTypecodeList(self):
        ...
        assert type(self.mgContent) is tuple, 'expecting tuple for mgContent not: %s' % type(self.mgContent)
...
class TcListComponentContainer(ContainerBase):
    ...
    def _getvalue(self):
        kw = {'occurs': self._getOccurs(), 'aname': self.getAttributeName(self.name), 
           'klass': self.klass, 
...
class RPCMessageTcListComponentContainer(TcListComponentContainer):
    ...
    def _getvalue(self):
        ...
        if self.style == 'standard':
            ...
            raise RuntimeError('Must set style(%s) for typecode list generation' % self.style)
            return
...
class ElementSimpleTypeContainer(TypecodeContainerBase):
    ...
    def _setContent(self):
        ...
        if pyclass == 'bool':
            pyclass = 'int'
        kw = KW.copy()
        kw.update(dict(aname=aname, ns=self.ns, name=self.name, subclass=self.sKlass, literal=self.literalTag(), schema=self.schemaTag(), init=self.simpleConstructor(), klass=self.getClassName(), element='ElementDeclaration'))
        if self.local:
    ...
    def setUp(self, tp):
        ...
        try:
            ...
            raise Wsdl2PythonError(('Error occured processing element: %s' % tp.getItemTrace()), *ex.args)
...
class ElementLocalSimpleTypeContainer(TypecodeContainerBase):
    ...
    def _setContent(self):
        kw = KW.copy()
        kw.update(dict(aname=self.getAttributeName(self.name), ns=self.ns, name=self.name, subclass=self.sKlass, literal=self.literalTag(), schema=self.schemaTag(), init=self.simpleConstructor(), klass=self.getClassName(), element='ElementDeclaration', baseinit=self.simpleConstructor(self.sKlass)))
        if self.local:
            kw['element'] = 'LocalElementDeclaration'
        element = map(lambda i: i % kw, [
         '%(ID1)sclass %(klass)s(%(subclass)s, %(element)s):',
    ...
    def setUp(self, tp):
        self._item = tp
        assert tp.isElement() is True and tp.content is not None and tp.content.isLocal() is True and tp.content.isSimple() is True, 'expecting local simple type: %s' % tp.getItemTrace()
        self.local = tp.isLocal()
        self.name = tp.getAttribute('name')
        self.ns = tp.getTargetNamespace()
        ...
        if content.isUnion():
            raise Wsdl2PythonError, 'unsupported local simpleType Union: %s' % tp.content.getItemTrace()
...
class ElementLocalComplexTypeContainer(TypecodeContainerBase, AttributeMixIn):
    ...
    def _setContent(self):
        kw = KW.copy()
        try:
            kw.update(dict(klass=self.getClassName(), subclass='ZSI.TCcompound.ComplexType', element='ElementDeclaration', literal=self.literalTag(), schema=self.schemaTag(), init=self.simpleConstructor(), ns=self.ns, name=self.name, aname=self.getAttributeName(self.name), nsurilogic=self.nsuriLogic(), ofwhat=self.getTypecodeList(), atypecode=self.attribute_typecode, pyclass=self.getPyClass()))
        except Exception as ex:
    ...
    def setUp(self, tp):
        ...
        """
        self._item = tp
        assert tp.isElement() is True and (tp.content is None or tp.content.isComplex() is True and tp.content.isLocal() is True), 'expecting element w/local complexType not: %s' % tp.content.getItemTrace()
        self.name = tp.getAttribute('name')
        self.ns = tp.getTargetNamespace()
        self.local = tp.isLocal()
        complex = tp.content
        if complex is None:
            ...
            if is_simple and complex.content.content.isRestriction():
                ...
                return
            if is_simple:
                raise ContainerError, 'not implemented local complexType/simpleContent: %s' % tp.getItemTrace()
            is_complex = complex.content.isComplex()
            if is_complex and complex.content.content is None:
                self.mgContent = ()
                self.attrComponents = self._setAttributes(complex.getAttributeContent())
                return
            if is_complex and complex.content.content.isExtension() and complex.content.content.content is not None and complex.content.content.content.isModelGroup():
                self.mgContent = complex.content.content.content.content
                self.attrComponents = self._setAttributes(complex.content.content.getAttributeContent())
                return
            if is_complex and complex.content.content.isRestriction() and complex.content.content.content is not None and complex.content.content.content.isModelGroup():
                self.mgContent = complex.content.content.content.content
                self.attrComponents = self._setAttributes(complex.content.content.getAttributeContent())
                return
            if is_complex:
...
class ElementGlobalDefContainer(TypecodeContainerBase):
    ...
    def _setContent(self):
        ...
        kw = KW.copy()
        try:
            kw.update(dict(klass=self.getClassName(), element='ElementDeclaration', literal=self.literalTag(), schema=self.schemaTag(), init=self.simpleConstructor(), ns=self.ns, name=self.name, aname=self.getAttributeName(self.name), baseslogic=self.getBasesLogic(ID3), alias=NAD.getAlias(self.sKlassNS), subclass=type_class_name(self.sKlass)))
        except Exception as ex:
            args = ['Failure processing an element w/local complexType: %s' % self._item.getItemTrace()]
            args += ex.args
            ex.args = tuple(args)
...
class ComplexTypeComplexContentContainer(TypecodeContainerBase, AttributeMixIn):
    ...
    def setUp(self, tp):
        ...
        """
        self._item = tp
        assert tp.content.isComplex() is True and (tp.content.content.isRestriction() or tp.content.content.isExtension() is True), 'expecting complexContent/[extension,restriction]'
        self.extType = None
        ...
        if base is None:
            ...
            if base is None:
                raise ContainerError, 'Unsupported derivation: %s' % derivation.getItemTrace()
        ...
        if base == (SOAP.ENC, 'Array'):
            ...
            for a in derivation.getAttributeContent():
                assert a.isAttribute() is True, 'only attribute content expected: %s' % a.getItemTrace()
            ...
            if attr is not None:
                ...
                self.logger.debug('Attribute soapenc:arrayType="%s"' % str(self._kw_array['ofwhat']))
        ...
        if derivation.content is not None and derivation.content.isModelGroup():
    ...
    def _setContent(self):
        ...
        definition.append('%(ID3)sattributes = %(atc)s = attributes or {}' % {'ID3': ID3, 
           'atc': self.attribute_typecode})
        isAnyType = (
         self.sKlassNS, self.sKlass) == (SCHEMA.XSD3, 'anyType')
        if isAnyType:
            del definition[0]
            definition.insert(0, '%sclass %s(ZSI.TC.ComplexType, TypeDefinition):' % (
             ID1, self.getClassName()))
            definition.insert(1, '%s#complexType/complexContent restrict anyType' % ID2)
        ...
        if isAnyType:
            definition.append('%sZSI.TC.ComplexType.__init__(self, None, TClist, pname=pname, **kw)' % ID3)
...
class ComplexTypeContainer(TypecodeContainerBase, AttributeMixIn):
    ...
    def _setContent(self):
        try:
            ...
            args = [
             'Failure processing %s' % self._item.getItemTrace()]
            args += ex.args
            ex.args = tuple(args)
            raise
...
class RestrictionContainer(SimpleTypeContainer):
    ...
    def setUp(self, tp):
        self._item = tp
        assert tp.isSimple() is True and tp.isDefinition() is True and tp.content.isRestriction() is True, 'expecting simpleType restriction, not: %s' % tp.getItemTrace()
            ...
            if sc.content.isRestriction() is True:
                ...
                if item is None:
                    ...
                    if base is not None:
                        ...
                        return
                    raise Wsdl2PythonError, 'Not Supporting simpleType/Restriction w/User-Defined Base: ' % item.getItemTrace()
                raise Wsdl2PythonError, 'Not Supporting simpleType/Restriction w/User-Defined Base: ' % item.getItemTrace()
            if sc.content.isList() is True:
                raise Wsdl2PythonError, 'iction base in subtypes: %s' % sc.getItemTrace()
            if sc.content.isUnion() is True:
                raise Wsdl2PythonError, 'could not get restriction base in subtypes: %s' % sc.getItemTrace()
            return
...
class ComplexTypeSimpleContentContainer(SimpleTypeContainer, AttributeMixIn):
    ...
    def setUp(self, tp):
        ...
        """
        self._item = tp
        if not (tp.isComplex() is True and tp.content.isSimple() is True):
            raise AssertionError('expecting complexType/simpleContent not: %s' % tp.content.getItemTrace())
            simple = tp.content
            dv = simple.content
            assert dv.isExtension() is True or dv.isRestriction() is True, 'expecting complexType/simpleContent/[Extension,Restriction] not: %s' % tp.content.getItemTrace()
            self.name = tp.getAttribute('name')
            self.ns = tp.getTargetNamespace()
            ...
            if base is not None:
                self.sKlass = BTI.get_typeclass(base[1], base[0])
                self.sKlass, self.sKlassNS = self.sKlass or base[1], base[0]
    ...
    def _setContent(self):
        if type(self.sKlass) in (types.ClassType, type):
            ...
            definition = ['%sclass %s(TypeDefinition):' % (ID1, self.getClassName()),
