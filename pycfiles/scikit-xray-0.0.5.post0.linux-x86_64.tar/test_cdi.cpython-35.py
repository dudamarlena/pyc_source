...
# [GCC 8.4.0]
# Embedded file name: /home/edill/mc/lib/python3.5/site-packages/skxray/core/tests/test_cdi.py
...
from skxray.core.cdi import _dist, gauss, find_support, pi_modulus, cal_diff_error, cdi_recon, generate_random_phase_field, generate_box_support, generate_disk_support
...
def test_recon():
    ...
    outv1 = np.abs(outv1)
    outv2, error_dict = cdi_recon(diff_v, init_phase, sup, pi_modulus_flag='Real', sw_flag=True, n_iterations=total_n, sw_step=2)
    outv2 = np.abs(outv2)
...
def test_cdi_plotter():
    ...
    init_phase = generate_random_phase_field(diff_v)
    sup = generate_box_support(sup_radius, diff_v.shape)
    outv, error_d = cdi_recon(diff_v, init_phase, sup, sw_flag=True, n_iterations=total_n, sw_step=2, cb_function=10)