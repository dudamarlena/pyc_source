...
# [GCC 8.4.0]
# Embedded file name: /home/edill/mc/lib/python3.5/site-packages/skxray/io/avizo_io.py
...
def _create_md_dict(clean_header):
    ...
    for header_line in clean_header:
        if 'define' in header_line:
            md_dict['array_dimensions'] = {'x_dimension': int(header_line[(header_line.index('define') + 2)]), 
             
             'y_dimension': int(header_line[(header_line.index('define') + 3)]), 
             
             'z_dimension': int(header_line[(header_line.index('define') + 4)])}
        elif 'Content' in header_line:
            md_dict['data_type'] = header_line[(header_line.index('Content') + 2)]
        elif 'CoordType' in header_line:
            md_dict['coord_type'] = header_line[(header_line.index('CoordType') + 1)]
        elif 'BoundingBox' in header_line:
            md_dict['bounding_box'] = {'x_min': float(header_line[(header_line.index('BoundingBox') + 1)]), 
             
             'x_max': float(header_line[(header_line.index('BoundingBox') + 2)]), 
             
             'y_min': float(header_line[(header_line.index('BoundingBox') + 3)]), 
             
             'y_max': float(header_line[(header_line.index('BoundingBox') + 4)]), 
             
             'z_min': float(header_line[(header_line.index('BoundingBox') + 5)]), 
             
             'z_max': float(header_line[(header_line.index('BoundingBox') + 6)])}
            bbox = [
            ...
            if resolution_list[1] / resolution_list[0] > 0.99 and resolution_list[2] / resolution_list[0] > 0.99 and resolution_list[1] / resolution_list[0] < 1.01 and resolution_list[2] / resolution_list[0] < 1.01:
            ...
            try:
                md_dict['units'] = str(header_line[(header_line.index('Units') + 2)])
            except:
                logging.debug('Units value undefined in source data set. Reverting to default units value of pixels')
                md_dict['units'] = 'pixels'

        elif 'Coordinates' in header_line:
            md_dict['coordinates'] = str(header_line[(header_line.index('Coordinates') + 1)])
