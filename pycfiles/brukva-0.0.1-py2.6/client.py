...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/brukva/client.py
...
def reply_zset(r, *args, **kwargs):
    if not r or 'WITHSCORES' not in args:
        return r
...
class Client(object):
    ...
    def __init__(self, host='localhost', port=6379, io_loop=None):
        ...
        self.REPLY_MAP = dict_merge(string_keys_to_dict('AUTH BGREWRITEAOF BGSAVE DEL EXISTS EXPIRE HDEL HEXISTS HMSET MOVE MSET MSETNX SAVE SETNX', reply_to_bool), string_keys_to_dict('FLUSHALL FLUSHDB SELECT SET SETEX SHUTDOWN RENAME RENAMENX WATCH UNWATCH', make_reply_assert_msg('OK')), string_keys_to_dict('SMEMBERS SINTER SUNION SDIFF', reply_set), string_keys_to_dict('HGETALL', reply_dict_from_pairs), string_keys_to_dict('HGET', reply_str), string_keys_to_dict('SUBSCRIBE UNSUBSCRIBE LISTEN', reply_pubsub_message), string_keys_to_dict('ZRANK ZREVRANK', reply_int), string_keys_to_dict('ZSCORE ZINCRBY', reply_int), string_keys_to_dict('ZRANGE ZRANGEBYSCORE ZREVRANGE', reply_zset), {'PING': make_reply_assert_msg('PONG')}, {'LASTSAVE': reply_datetime}, {'TTL': reply_ttl}, {'INFO': reply_info}, {'MULTI_PART': make_reply_assert_msg('QUEUED')})
        self._pipeline = None
        return
    ...
    def consume_multibulk(self, length, cmd_line, callback):
        ...
        while len(tokens) < length:
            ...
            if not data:
                break
            (error, token) = yield self.process_data(data, cmd_line)
            tokens.append(token)
            if error:
                errors[idx] = error
            idx += 1
...
class Pipeline(Client):
    ...
    def execute(self, callbacks):
        ...
        if self.transactional:
            command_stack = [
             CmdLine('MULTI')] + command_stack + [CmdLine('EXEC')]
        ...
        try:
            ...
            if self.transactional:
                ...
                (errors, tr_responses) = responses[(-1)]
                responses = [ (errors.get(idx, None), tr_responses[idx]) for idx in xrange(len(tr_responses))
                            ]
