...
# [GCC 8.4.0]
# Embedded file name: runkeeper_demo.py
# Compiled at: 2013-01-12 15:27:38
...
"""
import sys, os, optparse, ConfigParser, bottle
...
conf = {'baseurl': 'http://127.0.0.1:8000', 'bindaddr': '127.0.0.1', 
   'bindport': 8000}
defaultConfFilename = 'runkeeper_demo.conf'
sessionOpts = {'session.type': 'file', 
...
def index():
    ...
    if sess.has_key('rk_access_token'):
        ...
        rk_auth_mgr = RunKeeperAuthMgr(conf['client_id'], conf['client_secret'], ('/').join((conf['baseurl'], 'login')))
        rk_auth_uri = rk_auth_mgr.getLoginURL()
        rk_button_img = rk_auth_mgr.getLoginButtonURL('blue', 'black', 300)
        return bottle.template('index.html', {'rk_button_img': rk_button_img, 'rk_auth_uri': rk_auth_uri})
...
def welcome():
    ...
    if access_token is not None:
        ...
        return bottle.template('welcome.html', profile=profile, records=records)
    else:
        bottle.redirect('/')
        return
...
def parse_cmdline(argv=None):
    ...
    parser.add_option('-p', '--bindport', help='Bind to TCP Port. (Default: %d)' % conf['bindport'], dest='bindport', type='int', default=None, action='store')
    parser.add_option('-b', '--bindaddr', help='Bind to IP Address. (Default: %s)' % conf['bindaddr'], dest='bindaddr', default=None, action='store')
    parser.add_option('-u', '--baseurl', help='Base URL. (Default: %s)' % conf['baseurl'], dest='baseurl', default=None, action='store')
    parser.add_option('-D', '--devel', help='Enable development mode.', dest='devel', default=False, action='store_true')
    if argv is None:
...
def parse_conf_files(conf_paths):
    ...
    try:
        ...
        raise ConfigurationError('No valid configuration file (%s) found.' % defaultConfFilename)
...
def main(argv=None):
    ...
    bottle.run(app=app, host=conf['bindaddr'], port=conf['bindport'], reloader=cmd_opts.devel)
    return
