...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pthflops/ops.py
# Compiled at: 2020-01-18 08:36:07
# Size of source mod 2**32: 7635 bytes
import re
...
def string_to_shape(node_string, bias=False):
    """Extract the shape of a given tensor from an onnx string
    ...
    if not isinstance(node_string, str):
        ...
        if bias:
            m = re.search('Float\\((\\d+)\\)', node_string)
        else:
            m = re.search('Float\\(([\\d\\s\\,]+)\\)', node_string)
    if m is None:
        return m
    else:
        return tuple(int(x) for x in m.groups()[0].split(','))


def _count_convNd(node):
    """Estimates the number of FLOPs in conv layer
...
def _count_relu(node):
    """Estimates the number of FLOPs of a  ReLU activation.
    ...
    """
    inp = string_to_shape(list(node.inputs())[0])
    total_ops = 2 * reduce(lambda x, y: x * y, inp)
    return total_ops


def _count_avgpool(node):
    """Estimates the number of FLOPs of an Average Pooling layer.
...
def _count_maxpool(node):
    """Estimates the number of FLOPs of a Max Pooling layer.
...
def _count_bn(node):
    """Estimates the number of FLOPs of a Batch Normalisation operation.
...
def _count_linear(node):
    """Estimates the number of a GEMM or linear layer.
...
def _count_add_mul(node):
    """Estimates the number of FLOPs of a summation op.
...
def _undefined_op(node):
    """Default case for undefined or free (in terms of FLOPs) operations
...
count_operations = defaultdict(lambda : _undefined_op, {'onnx::Conv':_count_convNd, 
...
def count_ops(model, input, custom_ops={}, ignore_layers=[], print_readable=True, verbose=True, *args):
    """Estimates the number of FLOPs of an :class:`torch.nn.Module`
    ...
    if next(model.parameters()).device != input.device:
        ...
        if print_readable:
            ...
            print('{:,} FLOPs or approx. {:,.2f} GFLOPs'.format(ops, ops / 1000000000.0))
        if model_status:
            model.train()
    return (
     ops, all_data)