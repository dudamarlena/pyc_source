...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/at/at.py
# Compiled at: 2009-08-27 19:42:49
...
def parse_atq_stdout_line(stdout_line):
    ...
    return {'number': int(splitted[0]), 
       'datetime': datetime.datetime.strptime((' ').join(splitted[1:-2]), OUT_FORMAT), 
       'queue': splitted[(-2)], 
       'username': splitted[(-1)]}
...
def parse_at_stderr(stderr):
    ...
    return {'number': int(job_number.split()[(-1)]), 
...
def at(_datetime, popen_args):
    ...
    p2 = subprocess.Popen([
     'at'] + _datetime.strftime(IN_FORMAT).split(), stdin=p1.stdout, stderr=subprocess.PIPE)
...
def atrm(jobs):
    if jobs:
        stderr = subprocess.Popen([
         'atrm'] + [ str(job['number']) for job in jobs ], stderr=subprocess.PIPE).communicate()[1]
        if stderr:
