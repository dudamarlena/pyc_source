...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/consumers/xmlwriter/writer.py
# Compiled at: 2010-08-11 04:35:34
...
from pyf.componentized.configuration.fields import TextAreaField, SingleSelectField, InputField
...
class XMLWriter(MultipleFileWriter):
    ...
    def get_template(self):
        ...
        if template_type == 'plugin':
            ...
            return MarkupTemplate(resource_string(template_module, 'static/templates/' + template_node.get('template').strip() + '.xml'))
    ...
    def write(self, values, key, output_filename, target_filename):
        ...
        for (num, markup) in enumerate(self.template.generate(file_name=target_filename, splitkey=key, encoding=encoding, datas=values, get_config_key=self.get_config_key).serialize(method='xml')):
