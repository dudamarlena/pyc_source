...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/minegauler/frontend/minefield.py
...
from PyQt5.QtWidgets import QApplication, QGraphicsScene, QGraphicsView, QSizePolicy, QWidget
from ..core import api
from ..types import CellFlag, CellHitMine, CellImageType, CellMine, CellMineType, CellNum, CellUnclicked, CellWrongFlag
...
def init_or_update_cell_images(cell_images, size, styles, required=CellImageType.ALL):
    ...
    if required & CellImageType.BUTTONS:
        ...
        cell_images['btn_down'] = make_pixmap('buttons', btn_style, 'btn_down.png', size)
    ...
    if required & (CellImageType.BUTTONS | CellImageType.NUMBERS):
        for i in range(1, 19):
            cell_images[CellNum(i)] = make_pixmap('numbers', btn_style, 'btn_down.png', size, 'num%d.png' % i, 0.875)

    if required & (CellImageType.BUTTONS | CellImageType.MARKERS):
        for i in range(1, 4):
            cell_images[CellFlag(i)] = make_pixmap('markers', btn_style, 'btn_up.png', size, 'flag%d.png' % i, 0.625)
            cell_images[CellWrongFlag(i)] = make_pixmap('markers', btn_style, 'btn_up.png', size, 'cross%d.png' % i, 0.625)
            cell_images[CellMine(i)] = make_pixmap('markers', btn_style, 'btn_down.png', size, 'mine%d.png' % i, 0.875)
            cell_images[CellHitMine(i)] = make_pixmap('markers', btn_style, 'btn_down_hit.png', size, 'mine%d.png' % i, 0.875)
...
def make_pixmap(img_subdir, style, bg_fname, size, fg_fname=None, propn=1):
    ...
    if fg_fname:
        image = QImage(bg_path).scaled(size,
          size, transformMode=(Qt.SmoothTransformation))
        fg_size = propn * size
        fg_path = get_path(img_subdir, fg_fname, 'Standard')
        overlay = QPixmap(fg_path).scaled(fg_size,
          fg_size, transformMode=(Qt.SmoothTransformation))
        ...
        image = QPixmap(bg_path).scaled(size,
          size, transformMode=(Qt.SmoothTransformation))
    return image
...
class MinefieldWidget(QGraphicsView):
    ...
    def __init__(self, parent, ctrlr, state_):
    ...
    def mouseDoubleClickEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            if not self.both_mouse_buttons_pressed:
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self._ignore_clicks:
            ...
            coord = self._coord_from_event(event)
            if not event.buttons() & (Qt.LeftButton | Qt.RightButton) or self.await_release_all_buttons or coord == self.mouse_coord:
                return
            ...
            if not self.both_mouse_buttons_pressed or self._state.drag_select:
                if event.buttons() & Qt.LeftButton:
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        ...
        if self.await_release_all_buttons:
            ...
            if event.buttons() & (Qt.LeftButton | Qt.RightButton):
                ...
                if event.button() == Qt.LeftButton:
                    if not self.was_double_left_click:
    ...
    def update_style(self, img_type: CellImageType, style: str) -> None:
        ...
        init_or_update_cell_images(self._cell_images, self.btn_size, self._state.styles, img_type)
