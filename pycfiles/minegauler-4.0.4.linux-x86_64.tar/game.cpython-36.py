...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/minegauler/core/game.py
# Compiled at: 2020-01-26 18:53:06
# Size of source mod 2**32: 20076 bytes
"""
game.py - Game logic
...
import functools, logging, math, time as tm
from typing import Callable, Dict, Iterable, Optional, Tuple, Union
from ..shared import utils
from ..types import CellContentsType, CellFlag, CellHitMine, CellMine, CellMineType, CellNum, CellUnclicked, CellWrongFlag, GameState
...
def _check_coord(method: Callable) -> Callable:
    ...
    def wrapped(self, coord, *args, **kwargs):
        if not 0 <= coord[0] < self.x_size or not 0 <= coord[1] < self.y_size:
            raise ValueError(f"Coordinate is out of bounds, should be between (0,0) and ({self.x_size - 1}, {self.y_size - 1})")
        return method(self, coord, *args, **kwargs)

    return wrapped


def _ignore_if(*, game_state: Optional[Union[(str, GameState, Iterable[str], Iterable[GameState])]]=None, cell_state: Optional[Union[(type, Tuple[(type, ...)])]]=None) -> Callable:
    """
        ...
        if cell_state is None:
            ...
            def wrapped(self, *args, **kwargs):
                if game_state is not None:
                    if self.state == game_state or self.state in game_state:
                        return
            ...
            @functools.wraps(method)
            def wrapped(self, coord, *args, **kwargs):
                if game_state is not None and (self.state == game_state or self.state in game_state) or isinstance(self.board[coord], cell_state):
                    return
                else:
                    return method(self, coord, *args, **kwargs)
...
def _ignore_if_not(*, game_state: Optional[Union[(str, GameState, Iterable[str], Iterable[GameState])]]=None, cell_state: Optional[Union[(type, Tuple[(type, ...)])]]=None) -> Callable:
    """
    Return a decorator which prevents a method from running if any of the given
    parameters are satisfied.
    ...
    def decorator(method):
        if cell_state is None:
            ...
            def wrapped(self, *args, **kwargs):
                if game_state is not None:
                    if self.state != game_state:
                        if self.state not in game_state:
                            return
                return method(self, *args, **kwargs)
            ...
            @functools.wraps(method)
            def wrapped(self, coord, *args, **kwargs):
                if game_state is not None and self.state != game_state and self.state not in game_state or not isinstance(self.board[coord], cell_state):
                    return
                else:
                    return method(self, coord, *args, **kwargs)
...
class GameNotStartedError(Exception):
    __doc__ = 'Game has not been started, so no minefield has been created.'
...
class Game:
    ...
    def __init__(self, *, x_size: Optional[int]=None, y_size: Optional[int]=None, mines: Optional[int]=None, per_cell: int=1, lives: int=1, first_success: bool=False, minefield: Optional[Minefield]=None):
        """
        :param x_size:
        ...
        if minefield:
            ...
            if x_size is None or y_size is None or mines is None:
                raise ValueError('x_size, y_size and mines must be integers if a minefield is not provided')
            Minefield.check_enough_space(x_size=x_size,
              y_size=y_size,
              mines=mines,
              per_cell=per_cell)
            self.mf = None
    ...
    def get_rem_3bv(self) -> int:
        ...
        if self.state == GameState.WON:
            ...
            completed_3bv = len({c for c in self.board.all_coords if type(self.board[c]) is CellNum} - rem_opening_coords)
    ...
    def _create_minefield(self, coord: Coord_T) -> None:
        ...
        if self.first_success:
            ...
            logger.debug('Trying to create minefield with the following safe coordinates: %s', safe_coords)
            try:
                self.mf = Minefield((self.x_size),
                  (self.y_size),
                  mines=(self.mines),
                  per_cell=(self.per_cell),
                  safe_coords=safe_coords)
            except ValueError:
                logger.info('Unable to give opening on the first click, still ensuring a safe click')
                self.mf = Minefield((self.x_size),
                  (self.y_size),
                  mines=(self.mines),
                  per_cell=(self.per_cell),
            ...
            self.mf = Minefield((self.x_size),
              (self.y_size), mines=(self.mines), per_cell=(self.per_cell))
        ...
        if self.mf.cell_contains_mine(coord):
            ...
            if self.lives_remaining == 0:
                ...
                for c in self.mf.all_coords:
                    if self.mf.cell_contains_mine(c) and self.board[c] is CellUnclicked():
                        self._set_cell(c, CellMine(self.mf[c]))
                    else:
                        if type(self.board[c]) is CellFlag and self.board[c] != self.mf.completed_board[c]:
                            self._set_cell(c, CellWrongFlag(self.board[c].num))

            else:
                self.mines_remaining -= self.mf[coord]
        else:
            if self.mf.completed_board[coord] is CellNum(0):
                for full_opening in self.mf.openings:
                    if coord in full_opening:
                        logger.debug('Opening hit: %s', full_opening)
                        break
                else:
                    logger.error('Coordinate %s not found in openings %s', coord, self.mf.openings)

                opening = set()
                check = {coord}
                while check:
                    c = check.pop()
                    unclicked_nbrs = {z for z in self.board.get_nbrs(c, include_origin=True) if self.board[z] is CellUnclicked()}
                    check |= {z for z in unclicked_nbrs - opening if self.mf.completed_board[z] is CellNum(0)}
                ...
                if bad_opening_cells:
                    logger.error('Should only have clicked cells in opening, found: %s', bad_opening_cells)
            else:
    ...
    def _check_for_completion(self) -> None:
        ...
        for c in self.mf.all_coords:
            ...
            if type(exp_val) is CellNum:
                if exp_val != self.board[c]:
                    is_complete = False
                    break
        ...
        if is_complete:
            ...
            for c in self.mf.all_coords:
                if self.mf.cell_contains_mine(c) and type(self.board[c]) is not CellHitMine:
    ...
    def set_cell_flags(self, coord: Coord_T, nr_flags: int) -> Dict[(Coord_T, CellContentsType)]:
        """Set the number of flags in a cell."""
        if nr_flags < 0 or nr_flags > self.per_cell:
            raise ValueError(f"Invalid number of flags ({nr_flags}) - should be between 0 and {self.per_cell}")
        else:
            old_nr_flags = 0 if self.board[coord] is CellUnclicked() else self.board[coord].num
            if nr_flags == 0:
    ...
    def chord_on_cell(self, coord: Coord_T) -> Dict[(Coord_T, CellContentsType)]:
        ...
        num_flagged_nbrs = sum([self.board[c].num for c in nbrs if isinstance(self.board[c], CellMineType)])
        logger.debug('%s flagged mine(s) around clicked cell showing number %s', num_flagged_nbrs, self.board[coord])
