...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/server/bot/formatter.py
...
"""
__all__ = ('format_highscores', 'format_highscore_times', 'format_kwargs', 'format_matchups',
...
def format_player_highscores(highscores: List[hs.HighscoreStruct], difficulty: Optional[str]=None) -> List[str]:
    lines = []
    if highscores:
    ...
    if not difficulty:
        ...
        for h in highscores[:5]:
            line = '{:.2f} ({:.2f} 3bv/s) - {}'.format(h.elapsed, h.bbbvps, format_timestamp(h.timestamp))
            lines.append(line)
...
def format_filters(difficulty: Optional[str], drag_select: Optional[bool], per_cell: Optional[int], *, no_difficulty=False) -> str:
    opts = dict()
...
def format_matchups(matchups: Iterable[Matchup]) -> List[str]:
    return [('{} ({:.2f}) vs {} ({:.2f}) - {:.2f}% difference'.format)(*m) for m in matchups]
