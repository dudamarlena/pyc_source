...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/minegauler/core/board.py
# Compiled at: 2020-01-02 14:29:10
    ...
    def from_2d_array(cls, array):
        ...
        for c in grid.all_coords:
            if type(grid[c]) is int:
                ...
                if grid[c] != CellUnclicked.char:
                    raise ValueError(f"Unknown cell contents representation in cell {c}: {grid[c]}")
...
class Minefield(utils.Grid):
    ...
    def __init__(self, x_size, y_size, *, mines, per_cell=1, safe_coords=None):
        """
        :param x_size:
        ...
        for c in mine_coords:
            if self[c] == self.per_cell:
                raise ValueError(f"Too many mines at coord {c} with max per cell of {self.per_cell}")
            self[c] += 1
    ...
    def from_json(cls, dict_: Dict[(str, Any)]) -> 'Minefield':
        ...
        """
        try:
            return cls((dict_['x_size']),
              (dict_['y_size']),
              mines=[tuple(c) for c in dict_['mine_coords']],
              per_cell=(dict_.get('per_cell', 1)))
        except KeyError as e:
            raise ValueError('Missing key in dictionary when trying to create minefield') from e

    def to_json(self) -> Dict[(str, Any)]:
        """Convert to a JSON-serialisable format."""
        return dict(x_size=(self.x_size),
          y_size=(self.y_size),
          mine_coords=(self.mine_coords),
          per_cell=(self.per_cell))

    def _choose_mine_coords(self, safe_coords: Optional[Iterable[Coord_T]]=None) -> List[Coord_T]:
        """
        ...
        self.check_enough_space(x_size=(self.x_size),
          y_size=(self.y_size),
          mines=(self.nr_mines),
          per_cell=(self.per_cell),
    ...
    @staticmethod
    def check_enough_space(*, x_size: int, y_size: int, mines: int, per_cell: int, nr_safe_cells: int=1) -> None:
        """
        ...
        if mines > mine_spaces * per_cell:
            raise ValueError(f"Number of mines too high ({mines}) for grid with {mine_spaces} spaces and only up to {per_cell} allowed per cell")
    ...
    def _find_openings(self) -> List[List[Coord_T]]:
        ...
        """
        openings = []
        blanks_to_check = {c for c in self.all_coords if self.completed_board[c] == CellNum(0)}
        while blanks_to_check:
            ...
            while check:
                ...
                check |= {c for c in nbrs - opening if self.completed_board[c] == CellNum(0)}
                opening |= nbrs
