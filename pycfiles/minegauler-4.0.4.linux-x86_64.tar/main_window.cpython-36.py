...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/minegauler/frontend/main_window.py
# Compiled at: 2020-01-26 18:53:06
# Size of source mod 2**32: 34555 bytes
"""
...
import logging, os, textwrap, time as tm, traceback
from typing import Callable, Dict, Mapping, Optional, Type
from PyQt5.QtCore import QSize, Qt, pyqtSignal
from PyQt5.QtGui import QFocusEvent, QFont, QIcon, QKeyEvent
from PyQt5.QtWidgets import QAction, QActionGroup, QDialog, QFileDialog, QFrame, QGridLayout, QHBoxLayout, QLabel, QLineEdit, QMainWindow, QMenu, QMenuBar, QPushButton, QSizePolicy, QSlider, QSpinBox, QVBoxLayout, QWidget
...
class _BaseMainWindow(QMainWindow):
    ...
    def __init__(self, parent, title=None):
        ...
        self
        self
        self._icon = QIcon(str(utils.IMG_DIR / 'icon.ico'))
...
class _MinegaulerGUIMeta(type(api.AbstractListener), type(_BaseMainWindow)):
    __doc__ = 'Combined metaclass for the MinegaulerGUI class.'


class MinegaulerGUI(api.AbstractListener, _BaseMainWindow, metaclass=_MinegaulerGUIMeta):
    __doc__ = 'The main Minegauler GUI window.'

    def __init__(self, ctrlr, initial_state):
        """
        :param ctrlr:
            The core controller.
        :param initial_state:
        ...
        self._ctrlr = ctrlr
        self._state = initial_state.deepcopy()
        self
        self._diff_menu_actions = dict()
    ...
    def handle_finished_game(self, info: api.EndedGameInfo) -> None:
        ...
        if info.game_state is GameState.WON and info.difficulty != 'C' and not info.minefield_known:
            highscore = HighscoreStruct(difficulty=(info.difficulty),
            ...
            try:
                new_best = shared.highscores.is_highscore_new_best(highscore, shared.highscores.get_highscores(settings=highscore))
            except Exception:
    ...
    def handle_exception(self, method: str, exc: Exception) -> None:
        logger.error('Error occurred when calling %s() from backend:\n%s\n%s', method, ''.join(traceback.format_exception(None, exc, exc.__traceback__)), exc)
        raise RuntimeError(exc).with_traceback(exc.__traceback__)

    def sizeHint(self) -> QSize:
        width = self._body_frame.sizeHint().width()
        height = self._menubar.sizeHint().height() + self._panel_frame.sizeHint().height() + self._body_frame.sizeHint().height() + self._name_entry_widget.sizeHint().height()
        return QSize(width, height)

    def minimumSizeHint(self) -> QSize:
        width = self._panel_frame.minimumSizeHint().width()
        height = self._menubar.sizeHint().height() + self._panel_frame.sizeHint().height() + 100 + self._name_entry_widget.sizeHint().height()
        return QSize(width, height)
    ...
    def _populate_menubars(self) -> None:
        ...
        info_act = self._game_menu.addAction('Current game info', self._open_current_info_modal)
        info_act.setShortcut('F4')
        highscores_act = self._game_menu.addAction('Highscores', self.open_highscores_window)
        ...
        for diff in ('Beginner', 'Intermediate', 'Expert', 'Master', 'Custom'):
            ...
            diff_act.id = diff[0]
            if diff_act.id == get_difficulty(self._state.x_size, self._state.y_size, self._state.mines):
                diff_act.setChecked(True)
            diff_act.triggered.connect(lambda _: self._change_difficulty(diff_group.checkedAction().id))
        ...
        first_act = QAction('Safe start',
          self, checkable=True, checked=(self._state.first_success))
        self._opts_menu.addAction(first_act)
        first_act.triggered.connect(toggle_first_success)
        drag_act = self._opts_menu.addAction('Drag select', lambda : setattr(self._state, 'drag_select', not self._state.pending_drag_select))
        ...
        rules_act.triggered.connect(lambda : self._open_text_popup('Rules', FILES_DIR / 'rules.txt'))
        tips_act = QAction('Tips', self)
        self._help_menu.addAction(tips_act)
        tips_act.triggered.connect(lambda : self._open_text_popup('Tips', FILES_DIR / 'tips.txt'))
        self._help_menu.addSeparator()
        about_act = QAction('About', self)
        self._help_menu.addAction(about_act)
        about_act.triggered.connect(lambda : self._open_text_popup('About', FILES_DIR / 'about.txt'))
    ...
    def _open_save_board_modal(self) -> None:
        if not (self._state.game_status.finished() or self._state.ui_mode is UIMode.CREATE):
            return
        else:
    ...
    def _open_custom_board_modal(self) -> None:
        ...
        _CustomBoardModal(self, self._state.x_size, self._state.y_size, self._state.mines, self._ctrlr.resize_board).show()
    ...
    def open_highscores_window(self, settings: Optional[HighscoreSettingsStruct]=None, sort_by: Optional[str]=None) -> None:
        """
        ...
        if self._state.difficulty == 'C':
            ...
            if not settings:
                settings = HighscoreSettingsStruct(difficulty=(get_difficulty(self._state.x_size, self._state.y_size, self._state.mines)),
...
class _CurrentInfoModal(QDialog):
    __doc__ = 'A popup window with information about the current game.'

    def __init__(self, parent, info, state_):
        super().__init__(parent)
        self._info = info
        self._state = state_
    ...
    def _setup_ui(self) -> None:
        ...
        info = self._info
        info_text = textwrap.dedent(f"            {info.x_size} x {info.y_size} grid\n            {info.mines} mines\n            Max mines per cell: {info.per_cell}\n            Drag-select: {self._state.drag_select}\n            ")
        if info.finished_info:
            start_time = tm.strftime('%Y-%m-%d %H:%M:%S', tm.localtime(info.finished_info.start_time))
            ...
            if info.game_state is GameState.LOST:
                ...
                if fin_info.prop_complete > 0:
                    info_text += textwrap.dedent(f"                        Predicted completion time: {fin_info.elapsed / fin_info.prop_complete:.2f} seconds\n                        ")
...
class _CustomBoardModal(QDialog):
    __doc__ = 'A popup window to select custom board dimensions.'

    def __init__(self, parent, cols, rows, mines, callback):
        super().__init__(parent)
        self._cols = cols
        self._rows = rows
        self._mines = mines
    ...
    def _setup_ui(self) -> None:
        """Set up the UI."""
        base_layout = QVBoxLayout(self)
        base_layout.addWidget(QLabel('Select the number of rows, columns and mines.', self))
        grid_layout = QGridLayout()
        base_layout.addLayout(grid_layout)

        def make_row(row, label, current, minval, maxval):
...
class _ButtonSizeModal(QDialog):
    __doc__ = 'A popup window to select the button size.'

    def __init__(self, parent, btn_size, callback):
        super().__init__(parent)
...
class _SliderSpinner(QWidget):
    __doc__ = 'A combination of a slider and a spinbox.'
...
class _NameEntryBar(QLineEdit):
    __doc__ = 'Entry bar for entering a name.'
...
class _TextPopup(QWidget):
    __doc__ = 'A popup window containing a block of text.'
