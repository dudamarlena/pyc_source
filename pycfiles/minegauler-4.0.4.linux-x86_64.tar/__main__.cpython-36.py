...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/server/__main__.py
...
"""
import argparse, logging, os, sys, attr
...
def api_v1_highscores():
    ...
    return jsonify([attr.asdict(h) for h in hs.get_highscores((hs.HighscoresDatabases.REMOTE),
...
def api_v1_highscores_ranks():
    ...
    per_cell = int(per_cell)
    drag_select = bool(int(drag_select))
    return jsonify([attr.asdict(h) for h in hs.filter_and_sort(hs.get_highscores((hs.HighscoresDatabases.REMOTE),
      drag_select=drag_select,
      per_cell=per_cell,
      difficulty=difficulty))])
