...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/legaul/venv36/lib/python3.6/site-packages/minegauler/core/api.py
# Compiled at: 2020-02-02 09:05:03
# Size of source mod 2**32: 13815 bytes
"""
...
TODO
"""
__all__ = ('AbstractController', 'AbstractListener', 'AbstractSwitchingController',
           'Caller', 'EndedGameInfo')
...
class GameInfo:
    __doc__ = 'General information about a game.'
    ...
    per_cell: int
    finished_info = None
    finished_info: Optional[FinishedInfo]


@attr.attrs(auto_attribs=True)
class EndedGameInfo:
    __doc__ = 'Information about a finished game.'
    game_state: GameState
    difficulty: str
    per_cell: int
    start_time: float
...
class Caller(AbstractListener):
        ...
        def wrapped(*args, **kwargs):
            ...
            for listener in self._listeners:
                try:
                    ...
                    self._logger.warning(f"Error ocurred calling {func}() on {listener}")
    ...
    def update_cells(self, cell_updates: Dict[(Coord_T, CellContentsType)]) -> None:
        ...
        :param cell_updates:
            Mapping of coordinates that were changed to the new cell state.
        """
        self._logger.debug(f"Calling update_cells() with {len(cell_updates)} updated cells")
...
class AbstractController(metaclass=abc.ABCMeta):
    ...
    def __init__(self, opts: utils.GameOptsStruct, *, notif: Optional[Caller]=None):
        ...
        self._notif = notif if notif else Caller()
    ...
    def register_listener(self, listener: AbstractListener) -> None:
        ...
        :param listener:
            An AbstractListener instance to register.
        """
        self._logger.info('Registering listener: %s.%s', type(listener).__module__, type(listener).__name__)
    ...
    def unregister_listener(self, listener: AbstractListener) -> None:
        ...
        :param listener:
            An AbstractListener instance to unregister.
        """
        self._logger.info('Unregistering listener: %s.%s', type(listener).__module__, type(listener).__name__)
    ...
    def resize_board(self, x_size: int, y_size: int, mines: int) -> None:
        ...
        """
        self._logger.info('Resizing the board to %sx%s with %s mines', x_size, y_size, mines)
...
class AbstractSwitchingController(AbstractController):
    __doc__ = 'An abstract base class for controllers that support switching mode.'
