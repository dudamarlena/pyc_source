...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/picotest.py
# Compiled at: 2011-11-18 08:45:08
__all__ = ('new', 'config', 'skip_when', 'todo')
import sys, os, re, time, types, traceback
sys.STDERR = sys.stderr
python2 = sys.version_info[0] == 2
python3 = sys.version_info[0] == 3
__version__ = ('$Release: 0.2.0 $').split(' ')[1]
__unittest = 1
DEBUG = False
TEST = os.getenv('TEST')
...
config = Config(style='verbose', color=re.match('^(darwin|linux|freebsd|netbsd)', sys.platform) and hasattr(sys.stdout, 'isatty') and sys.stdout.isatty())
_BUILDERS = []
...
class TestBuilder(object):
    ...
    def before_all(self, func):
        self._topic.before_all = func

    def after_all(self, func):
        self._topic.after_all = func
...
class TestTopic(object):
    __module__ = __name__
    before = after = before_all = after_all = None
...
class TestRunner(object):
    ...
    def _run_topic(self, depth, topic, root_p=False):
        ...
        if not root_p:
            self.reporter.enter_topic(depth, topic.name)
        if topic.before_all:
            topic.before_all()
        try:
            ...
            if topic.after_all:
                topic.after_all()
            if not root_p:
                self.reporter.exit_topic(depth, topic.name)
    ...
    def _run_spec(self, depth, desc, func):
        if TEST and TEST != desc:
            return
        self_ = unittest.TestCase('run')
        self_._testMethodName = desc
            ...
            if argname == 'self':
                ...
                if gfunc:
                    ...
                    if not isinstance(g, types.GeneratorType):
                        ex = TypeError('%s(): fixture should yield value instead of return.' % (gfunc.__name__,))
                        break
                    arg = next(g)
                    generators.append(g)
                else:
                    ex = NameError('%s: corresponding fixture not found.' % (argname,))
                    break
            args.append(arg)
...
class TestReporter(object):
    ...
    _pattern = '  File "%s", line ' % re.sub('\\.py[co]?$', '.py', __file__)
...
_reporter_classes = {'verbose': VerboseReporter, 'v': VerboseReporter, 'simple': SimpleReporter, 's': SimpleReporter, 'plain': PlainReporter, 'p': PlainReporter}
...
class Color(object):
    ...
    def bold(self, s):
        return config.color and '\x1b[0;1m%s\x1b[22m' % s or s

    def black(self, s):
        return config.color and '\x1b[1;30m%s\x1b[0m' % s or s

    def red(self, s):
        return config.color and '\x1b[1;31m%s\x1b[0m' % s or s

    def green(self, s):
        return config.color and '\x1b[1;32m%s\x1b[0m' % s or s

    def yellow(self, s):
        return config.color and '\x1b[1;33m%s\x1b[0m' % s or s

    def blue(self, s):
        return config.color and '\x1b[1;34m%s\x1b[0m' % s or s

    def magenta(self, s):
        return config.color and '\x1b[1;35m%s\x1b[0m' % s or s

    def cyan(self, s):
        return config.color and '\x1b[1;36m%s\x1b[0m' % s or s

    def white(self, s):
        return config.color and '\x1b[1;37m%s\x1b[0m' % s or s
...
def _classname(klass):
    ...
    elif hasattr(types, 'ClassType') and isinstance(klass, types.ClassType):
        return klass.__name__
    else:
        return repr(klass)


def assertException(self, fn, exc_class, errmsg=None):
    if not isinstance(fn, types.FunctionType):
        raise TypeError('First argument of assertException() should be a function')
    ex = None
    try:
        fn()
    except Exception:
        ...
        self.fail('%s expected but nothing raised' % (_classname(exc_class),))

    fn.exception = ex
    if not isinstance(ex, exc_class):
        self.fail('%s expected but got %r' % (_classname(exc_class), ex))
    if errmsg is not None:
        if isinstance(errmsg, _rexp_type):
            matched = errmsg.search(str(ex))
        else:
            matched = str(ex) == errmsg
        if not matched:
            self.fail('%r expected but got %r' % (errmsg, str(ex)))
    return ex


unittest.TestCase.assertException = assertException
del assertException
_rexp_type = type(re.compile('x'))

def assertNotException(self, fn, exc_class=Exception):
    if not isinstance(fn, types.FunctionType):
        raise TypeError('First argument of assertNotException() should be a function')
    try:
        fn()
    except Exception:
        ex = sys.exc_info()[1]
        if isinstance(ex, exc_class):
            self.fail('%s should not be raised but %r raised' % (_classname(exc_class), ex))


unittest.TestCase.assertNotException = assertNotException
del assertNotException
...
def _load_file(filepath):
    mod_name = re.sub('\\.py[co]?$', '', os.path.basename(filepath))
