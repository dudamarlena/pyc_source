...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/build/dockerfile.py
# Compiled at: 2020-04-03 17:38:07
# Size of source mod 2**32: 19295 bytes
from __future__ import unicode_literals
import json, collections, os, posixpath, six, tarfile
...
def format_expose(expose):
    ...
    if isinstance(expose, six.string_types):
        return (
         expose,)
    if isinstance(expose, collections.Iterable):
        return map(six.text_type, expose)
    return (
     six.text_type(expose),)
...
class DockerFile(DockerStringBuffer):
    ...
    def add_archive(self, src_file, remove_final=False):
        ...
        with tarfile.open(src_file, 'r') as (tf):
            member_names = [member.name for member in tf.getmembers() if posixpath.sep not in member.name]
