...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/runner/attached.py
...
class AttachedConfigMixin(object):
    ...
    def get_attached_preparation_create_kwargs(self, action, volume_container, volume_alias, kwargs=None):
        ...
        if not cmd:
            return
            c_kwargs = dict(image=(policy.core_image),
              command=cmd,
              user='root',
            ...
            if use_host_config:
                if client_config.features['volumes']:
                    c_kwargs['volumes'] = [
                     PREPARATION_TMP_PATH]
                hc_kwargs = self.get_attached_preparation_host_config_kwargs(action, None, volume_container, kwargs=hc_extra_kwargs)
                if hc_kwargs:
...
class AttachedPreparationMixin(AttachedConfigMixin):
    ...
    def prepare_attached(self, action, a_name, **kwargs):
        ...
        return self.prepare_local and hasattr(client, 'run_cmd') or self._prepare_container(client, action, a_name, v_alias)
        if action.client_config.features['volumes']:
            ...
            if not local_path:
                raise ValueError("Could not locate local path of volume alias '{0}' / path '{1}' in container {2}.".format(action.config_id.instance_name, path, a_name))
        return [client.run_cmd(cmd) for cmd in get_preparation_cmd(user, permissions, local_path)]