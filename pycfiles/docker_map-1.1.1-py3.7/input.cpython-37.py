...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/input.py
# Compiled at: 2019-10-19 14:38:08
# Size of source mod 2**32: 33565 bytes
...
EXEC_POLICY_RESTART = ExecPolicy.RESTART
EXEC_POLICY_INITIAL = ExecPolicy.INITIAL
...
def _get_list(value):
    ...
    if isinstance(value, (list, tuple)):
        return list(value)
    if isinstance(value, six.string_types + (lazy_type,)) or uses_type_registry(value):
        return [
         value]
    raise ValueError('Invalid type; expected a list, tuple, or string type, found {0}.'.format(type(value).__name__))


TIME_PATTERN = re.compile('(\\d+)\\s*([num]s|s|m)?')
...
def _get_nanoseconds(value):
    if value is None:
        ...
        if not value:
            ...
            return isinstance(value, six.string_types) or value
        match = TIME_PATTERN.match(value.strip())
        raise (match or ValueError)('Invalid time format.', value)
...
class NetworkEndpoint(namedtuple('NetworkEndpoint', ('network_name', 'aliases', 'links', 'ipv4_address',
    ...
    def __new__(cls, network_name, aliases=None, links=None, ipv4_address=None, ipv6_address=None, link_local_ips=None):
        return super(NetworkEndpoint, cls).__new__(cls, network_name, _get_list(aliases), ContainerLinkList(links), ipv4_address, ipv6_address, _get_list(link_local_ips))
        ...
        if not test or test == 'NONE':
            ...
            if isinstance(test, (tuple, list)):
                if test[0] not in ('NONE', 'CMD', 'CMD-SHELL'):
                    ...
                    test = test_args
        return super(HealthCheck, cls).__new__(cls, test, _get_nanoseconds(interval), _get_nanoseconds(timeout), retries, _get_nanoseconds(start_period))

    def _asdict(self):
        d = super(HealthCheck, self)._asdict()
        return {k:v for k, v in six.iteritems(d) if v or k == 'test' if v or k == 'test'}


def _get_listed_tuples(value, element_type, conversion_func, **kwargs):
    if value is None:
        return []
    if isinstance(value, element_type) or uses_type_registry(value):
        return [
         value]
    if isinstance(value, six.string_types):
        return [
         conversion_func(value, **kwargs)]
    if isinstance(value, dict):
        return [conversion_func(e, **kwargs) for e in six.iteritems(value)]
    if isinstance(value, Iterable):
        return [conversion_func(e, **kwargs) for e in value]
    raise ValueError('Invalid type; expected {0}, list, tuple, or dict; found {1}.'.format(element_type.__name__, type(value).__name__))
...
def _shared_host_volume_from_tuple(*values):
    ...
    if v_len == 2:
        ...
        if isinstance(v1, (list, tuple)):
            ...
            raise ValueError('Nested list in {0} must have exactly one or two entries; found {1}.'.format(values, sv_len))
        else:
            if isinstance(v1, bool) or v1 in ('ro', 'rw'):
                return SharedVolume(v0, read_only(v1))
        return HostVolume(v0, v1)
    if v_len == 1:
        return SharedVolume(values[0])
    raise ValueError('Invalid element length; only tuples and lists of length 1-3 can be converted to a HostVolume or SharedVolume tuple. Found length {0}.'.format(v_len))
...
class AttachedVolumeList(NamedTupleList):
    ...
    def get_type_item(self, value):
        ...
        if isinstance(value, (list, tuple)):
            ...
            if v_len == 1:
                return SharedVolume(value[0])
            raise ValueError('Invalid element length; only tuples and lists of length 1-2 can be converted to a UsedVolume or SharedVolume tuple; found length {0}.'.format(v_len))
        else:
            if isinstance(value, dict):
                v_len = len(value)
                if v_len == 1:
    ...
    def get_type_item(self, value):
        ...
        if isinstance(value, ContainerLink):
            ...
            if isinstance(value, (list, tuple)):
                ...
                raise ValueError('Invalid element length; only tuples and lists of length 1-2 can be converted to a ContainerLink tuple. Found length {0}.'.format(v_len))
...
class ExecCommandList(NamedTupleList):
    ...
    def get_type_item(self, value):
        ...
        if isinstance(value, ExecCommand):
            return value
            if isinstance(value, six.string_types + (lazy_type,)):
                return ExecCommand(value)
            if isinstance(value, (list, tuple)):
                v_len = len(value)
                if 1 <= v_len <= 3:
                    return ExecCommand(*value)
                raise ValueError('Invalid element length; only tuples and lists of length 1-3 can be converted to a ExecCommand tuple. Found length {0}.'.format(v_len))
        elif isinstance(value, dict):
            return ExecCommand(**value)
        raise ValueError('Invalid type; expected a list, tuple, dict, or string type, found {0}.'.format(type(value).__name__))
...
class PortBindingList(NamedTupleList):
    ...
    def get_type_item(self, value):
        ...
        if isinstance(value, PortBinding):
            return value
            if isinstance(value, sub_types):
                return PortBinding(value)
                if isinstance(value, (list, tuple)):
                    v_len = len(value)
                    if v_len == 1:
                        if isinstance(value[0], sub_types):
                    ...
                    if v_len == 2:
                        ...
                        if isinstance(host_bind, (list, tuple)):
                            s_len = len(host_bind)
                            if s_len in (2, 3):
                                return PortBinding(ex_port, *host_bind)
                        raise ValueError('Invalid sub-element type or length. Needs to be a port number or a tuple / list: (port, interface) or (port, interface, ipv6).')
            elif v_len in (3, 4):
                return PortBinding(*value)
            raise ValueError('Invalid element length; only tuples and lists of length 2 to 4 can be converted to a PortBinding tuple.')
        else:
            if isinstance(value, dict):
                return PortBinding(**value)
            raise ValueError('Invalid type; expected a dict, list, tuple, int, or string type, found {0}.'.format(type(value).__name__))
...
class NetworkEndpointList(NamedTupleList):
    ...
    def get_type_item(self, value):
        ...
        if isinstance(value, (list, tuple)):
            ...
            if 1 <= v_len <= 6:
                return NetworkEndpoint(*value)
            raise ValueError('Invalid element length; only dictionaries, pr tuples and lists of length 1-6 can be converted to a NetworkEndpoint tuple. Found length {0}.'.format(v_len))
        else:
            if isinstance(value, dict):
                d_len = len(value)
                if d_len == 1:
                    ...
                    if isinstance(v, (list, tuple)):
                        if 1 <= v_len <= 5:
                            return NetworkEndpoint(k, *v)
                        raise ValueError('Invalid sub-element length; lists and tuples of length 1-5 can be converted. Found length {0}.'.format(v_len))
                    raise ValueError('Invalid sub-element format; only dicts and tuples of length 1-5 can be converted. Found type {0}.'.format(type(value).__name__))
                return NetworkEndpoint(**value)
            raise ValueError('Invalid type; expected a dict, list, tuple, or string type, found {0}.'.format(type(value).__name__))
...
class InputConfigIdList(NamedTupleList):
            ...
            if isinstance(instances, tuple):
                ...
                if isinstance(instances, list):
                    ...
                    if isinstance(instances, six.string_types):
                        ...
                        raise ValueError('Invalid instances specification; expected string, list, or tuple, found {0}.'.format(type(instances).__name__))
        cls = self.__class__
        if isinstance(seq, cls):
            values = seq
        else:
            if isinstance(seq, MapConfigId):
                ...
                values = [
                 InputConfigId(seq.config_type, seq.map_name, seq.config_name, v_instances)]
            else:
                values = _get_listed_tuples(seq, (cls.element_type), (self.get_type_item), map_name=map_name,
    ...
    def get_type_item(self, value, map_name=None, instances=None):
        ...
        if isinstance(value, InputConfigId):
            ...
            if isinstance(value, six.string_types):
                ...
                if isinstance(value, (tuple, list)):
                    ...
                    if v_len == 3:
                        ...
                        if isinstance(v_instances, six.string_types):
            ...
            if isinstance(value, dict):
                kwargs = {'config_type':ItemType.CONTAINER,  'map_name':map_name, 
