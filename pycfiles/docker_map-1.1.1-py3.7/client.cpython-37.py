...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/client.py
# Compiled at: 2020-04-02 07:00:31
# Size of source mod 2**32: 28253 bytes
...
from state.base import SingleStateGenerator, DependencyStateGenerator, DependentStateGenerator, ImageDependencyStateGenerator
...
class MappingDockerClient(object):
    ...
    generators = {'create':(
      DependencyStateGenerator, simple.CreateActionGenerator), 
     'start':(
      DependencyStateGenerator, simple.StartActionGenerator), 
     'restart':(
      SingleStateGenerator, simple.RestartActionGenerator), 
     'stop':(
      DependentStateGenerator, simple.StopActionGenerator), 
     'remove':(
      DependentStateGenerator, simple.RemoveActionGenerator), 
     'startup':(
      DependencyStateGenerator, simple.StartupActionGenerator), 
     'shutdown':(
      DependentStateGenerator, simple.ShutdownActionGenerator), 
     'update':(
      UpdateStateGenerator, update.UpdateActionGenerator), 
     'script':(
      DependencyStateGenerator, script.ScriptActionGenerator), 
     'signal':(
      SingleStateGenerator, simple.SignalActionGenerator), 
     'pull_images':(
      ImageDependencyStateGenerator, simple.ImagePullActionGenerator)}
    runner_class = DockerClientRunner

    def __init__(self, container_maps=None, docker_client=None, clients=None, map_defaults=None, option_defaults=None):
        if container_maps:
    ...
    def get_policy(self):
        ...
        """
        if not self._policy:
            self._policy = self.policy_class(self._maps, self._clients, self._map_defaults, self._option_defaults)
        return self._policy
    ...
    def get_states(self, action_name, config_name, instances=None, map_name=None, **kwargs):
        ...
        config_ids = get_map_config_ids(config_name, policy.container_maps, map_name or self._default_map, instances)
    ...
    def list_persistent_containers(self, map_name=None):
        ...
        if map_name:
            maps = [
             self._maps[map_name].get_extended_map()]
        else:
        ...
        for c_map in maps:
            ...
            if c_map.use_attached_parent_name:
                c_names.extend([aname_func(m_name, ca, c_name) for c_name, ca in attached])
            else:
                c_names.extend([aname_func(m_name, ca[1]) for ca in attached])
            c_names.extend([cname_func(m_name, c_name, ci) for c_name, ci in persistent])
