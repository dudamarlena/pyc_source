...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/state/base.py
# Compiled at: 2020-04-02 07:00:31
# Size of source mod 2**32: 18025 bytes
...
class ContainerBaseState(AbstractState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not config:
            raise KeyError("Container configuration '{0.config_name}' not found on map '{0.map_name}'.".format(self.config_id))
    ...
    def get_state(self):
        ...
        if c_status['Running']:
            ...
            if c_status['StartedAt'] == INITIAL_START_TIME:
                ...
                if c_status['ExitCode'] in self.options['nonrecoverable_exit_codes']:
            ...
            return (
             base_state, state_flag, extra_data)
...
class NetworkBaseState(AbstractState):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not config:
            raise KeyError("Network configuration '{0.config_name}' not found on map '{0.map_name}'.".format(self.config_id))
    ...
    def get_state(self):
        ...
        if force_update and self.config_id in force_update:
            ...
            state_flag = StateFlags.NONE
        return (
         State.PRESENT, state_flag, {'id':n_id,  'containers':connected_containers})
...
class VolumeBaseState(AbstractState):
    ...
    def inspect(self):
        ...
        if not self.client_config.features['volumes']:
            ...
            volume_name = self.volume_name = self.policy.aname((config_id.map_name), (config_id.instance_name), parent_name=parent_name)
...
class AbstractDependencyStateGenerator(with_metaclass(ABCPolicyUtilMeta, AbstractStateGenerator)):
    ...
    def get_states(self, config_ids):
        ...
        :rtype: collections.Iterable[dockermap.map.state.ConfigState]
        """
        input_paths = [(config_id, list(self.get_dependency_path(config_id))) for config_id in config_ids]
