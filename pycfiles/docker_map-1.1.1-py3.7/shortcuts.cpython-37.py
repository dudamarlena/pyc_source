...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/shortcuts.py
# Compiled at: 2020-04-09 16:25:19
# Size of source mod 2**32: 12652 bytes
...
CMD_ARG_MAPPING = {CmdArgMappings.DEBIAN: {'no_login':'--disabled-login', 
                         'no_create_home':'--no-create-home', 
                         'no_password':'--disabled-password', 
                         'gecos':'--gecos', 
                         'system_user':'--system'}, 
 
 CmdArgMappings.CENTOS: {'no_login':'-s /sbin/nologin', 
                         'no_create_home':'--no-create-home', 
                         'no_password':None, 
                         'gecos':'--comment', 
                         'system_user':'--system'}, 
 
 CmdArgMappings.BUSYBOX: {'no_login':'-s /sbin/nologin', 
                          'no_create_home':'-H', 
                          'no_password':'-D', 
                          'gecos':'-g', 
                          'system_user':'-S'}}
if hasattr(shlex, 'quote'):
    ...
    def str_arg(arg):
        if not arg:
            ...
            return _find_unsafe(arg) or arg
...
chown = lambda user_group, path, recursive=True: _format_cmd('chown', (get_user_group(user_group)), path, _R=(bool(recursive)))
    ...
    if mapped_cmds:
        for arg_key, cmd_arg in six.iteritems(mapped_cmds):
            ...
            if arg_val is not None:
                if arg_val is not False:
                    if cmd_arg is None:
                        raise ValueError('Argument {0} is not supported on {1}.'.format(arg_key, arg_mapping))
                cmd_kwargs[cmd_arg] = arg_val


def adduser(username, uid=None, system=False, no_login=True, no_password=False, gecos=None, arg_mapping=CmdArgMappings.DEBIAN, **kwargs):
    """
    ...
    :rtype: unicode | str
    """
    if arg_mapping == CmdArgMappings.DEBIAN:
        if gecos is None:
            gecos = ''
    cmd_kwargs = dict(system_user=(bool(system)),
      no_password=(bool(no_password)),
      gecos=gecos,
      _u=uid)
    if no_login:
        cmd_kwargs.update(no_create_home=True,
          no_login=True)
...
def assignuser(username, groupnames, arg_mapping=CmdArgMappings.DEBIAN, return_list=False, **kwargs):
    ...
    if arg_mapping == CmdArgMappings.BUSYBOX:
        cmds = [_format_cmd('adduser', username, group_name, **kwargs) for group_name in groupnames]
    else:
        cmds = [
         _format_cmd('usermod', username, _aG=','.join(groupnames), **kwargs)]
    if return_list:
        return cmds
    return ' && '.join(cmds)
...
def get_user_group(user_group):
    ...
    if isinstance(user_group, six.integer_types) or ':' not in user_group:
        return '{0}:{0}'.format(str_arg(user_group))
    return user_group


def addgroupuser(username, groupnames, uid=None, system=False, no_login=True, no_password=False, gecos=None, arg_mapping=CmdArgMappings.DEBIAN, return_list=False, **kwargs):
    """
    ...
    :rtype: unicode | str
    """
    adduser_kwargs = dict(uid=uid, system=system, no_login=no_login, no_password=no_password, gecos=gecos, arg_mapping=arg_mapping, **kwargs)
    if groupnames:
        if arg_mapping == CmdArgMappings.BUSYBOX:
        ...
        usermod = None
    cmds = [
     adduser(username, **adduser_kwargs)]
    if usermod:
        cmds.extend(usermod)
    if return_list:
        return cmds
...
def mkdir_chown(paths, user_group=None, permissions='ug=rwX,o=rX', create_parent=True, check_if_exists=False, recursive=False, return_list=False):
    """
    ...
    def _generate_cmds(path):
        s = [
         mkdir(path, create_parent, check_if_exists)]
        if user_group:
