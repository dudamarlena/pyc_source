...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/state/update/container.py
# Compiled at: 2020-04-02 07:00:31
# Size of source mod 2**32: 15624 bytes
    ...
    if config_cmd:
        ...
        if config_entrypoint:
            ...
            if isinstance(config_entrypoint, six.string_types):
                if [
                 config_entrypoint] != instance_entrypoint:
                    return False
        ...
        if i_key not in instance_ports:
            ...
            if bind_port:
                ...
                if interface:
                    ...
                    bind_addr = '0.0.0.0'
                bind_config = {'HostIp':bind_addr, 
                 'HostPort':six.text_type(bind_port)}
    ...
    for inspect_key, config_key, check_co, input_func in CONTAINER_UPDATE_VARS:
        ...
        if i_value != c_value:
            if c_value is not None:
                ...
                log.debug('Host-config variable %s cannot be reset to default, suggesting container reset.', inspect_key)
                needs_reset = True

    return (
     update_dict, needs_reset)
...
class UpdateContainerState(ContainerBaseState):
    ...
    def _check_commands(self, check_option):
        ...
        if not self.config.exec_commands:
            return
        elif not self.current_commands:
            ...
            return
        return [exec_cmd for exec_cmd in self.config.exec_commands if not _cmd_running(exec_cmd.cmd, exec_cmd.user) if exec_cmd.policy != ExecPolicy.INITIAL]
    ...
    def inspect(self):
        ...
        if self.detail:
            if self.detail['State']['Running']:
                if self.config_id.config_type == ItemType.CONTAINER:
                    check_exec_option = self.options['check_exec_commands']
                    if check_exec_option:
                        if check_exec_option != CmdCheck.NONE:
                            if self.config.exec_commands:
    ...
    def get_state(self):
        ...
        if config_id.config_type == ItemType.VOLUME:
            ...
            if c_image_id != ref_image_id:
                if not self.config.persistent or self.options['update_persistent']:
            ...
            if not (_check_environment(self.config, self.detail) and _check_cmd(self.config, self.detail) and _check_container_network_ports(self.config, self.client_config, self.detail)):
            ...
            if self.endpoint_registry:
                ...
                if not self._check_container_network_mode():
                    state_flags |= StateFlags.MISC_MISMATCH
                ...
                if not self.client_config.features['container_update_restart_policy']:
                    if restart_policy_update:
                        hc_needs_reset = True
                if hc_update:
