...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/client/docker_util.py
...
class DockerUtilityMixin(object):
    ...
    def cleanup_containers(self, include_initial=False, exclude=None, raise_on_error=False, list_only=False):
        ...
        def _stopped_containers():
            for container in self.containers(all=True):
                ...
                if not include_initial or c_status in ('', 'Created') or c_status.startswith('Exited') or c_status == 'Dead':
                    if exclude_names.isdisjoint(c_names):
                        c_id = container['Id']
    ...
    def cleanup_images(self, remove_old=False, keep_tags=None, force=False, raise_on_error=False, list_only=False):
        ...
        :rtype: list[unicode | str]
        """
        used_images = set((self.inspect_container(container['Id'])['Image'] for container in self.containers(all=True)))
        all_images = self.images(all=True)
        image_dependencies = [(image['Id'], image['ParentId']) for image in all_images if image['ParentId']]
        if remove_old:
            check_tags = {
             'latest'}
            if keep_tags:
        ...
        keep_images = {image['Id'] for image in all_images if tag_check(image)} | used_images
        test_images = [image['Id'] for image in all_images if image['Id'] not in keep_images]
        resolver = ImageDependentsResolver(image_dependencies)
        unused_images = [image_id for image_id in test_images if keep_images.isdisjoint(resolver.get_dependencies(image_id))]
        if list_only:
    ...
    def remove_all_containers(self, stop_timeout=10, list_only=False):
        ...
        """
        containers = [(container['Id'], container['Status']) for container in self.containers(all=True)]
        running_containers = [c_id for c_id, status in containers if not status.startswith('Exited') if not status == 'Dead']
        if list_only:
            return (
             running_containers, [c[0] for c in containers])
        stopped_containers = []
        ...
        return (
         stopped_containers, removed_containers)
