...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dockermap/map/state/update/network.py
# Compiled at: 2019-10-19 14:38:08
# Size of source mod 2**32: 9350 bytes
...
def _check_network_driver_opts(network_config, instance_detail):
    ...
    if not driver_opts:
        return True
    opts = {option_key:option_value for option_key, option_value in six.iteritems(instance_detail['Options'])}
...
def _check_network_endpoint(network_name, network_config, linked_names, network_detail, network_endpoint_id):
    ...
    if instance_endpoint_id != network_endpoint_id:
        log.debug('Network endpoint found in %s (%s), but with different id from container (%s).', network_name, network_endpoint_id, instance_endpoint_id)
        return False
    c_alias_set = set(network_config.aliases or ())
    if c_alias_set:
        if not c_alias_set.issubset(network_detail.get('Aliases')):
    ...
    if network_config.ipv4_address:
        ...
        if normalized_ip4 != instance_ip4:
            log.debug('Container IPv4 address %s is different from configuration: %s.', instance_ip4, normalized_ip4)
            return False
    if network_config.ipv6_address:
        ...
        if normalized_ipv6 != instance_ipv6:
            log.debug('Container IPv6 address %s is different from configuration: %s.', instance_ipv6, normalized_ipv6)
            return False
    normalized_ll_ip = {ip_address(six.text_type(addr)).compressed for addr in network_config.link_local_ips or ()}
    instance_ll_ip = set(ipam_config.get('LinkLocalIPs') or ())
    if normalized_ll_ip != instance_ll_ip:
        log.debug('Container link-local addresses %s are different from configuration: %s.', instance_ll_ip, normalized_ll_ip)
        return False
    return True
...
class NetworkEndpointRegistry(object):
    ...
    def check_container_config(self, config_id, c_config, detail):
        ...
        if c_config.networks:
            named_endpoints = [(self._nname(config_id.map_name, cn_config.network_name), cn_config) for cn_config in c_config.networks]
        else:
            if c_net_mode in ('default', 'bridge'):
                named_endpoints = [(d_name, NetworkEndpoint(d_name)) for d_name in self._default_networks]
            else:
                if c_net_mode == 'none':
                    named_endpoints = []
                else:
                    if isinstance(c_net_mode, tuple):
                        ...
                        if cc_id:
                            cn_names = (
                             cc_name_mode, 'container:{0}'.format(cc_id))
                        else:
                            cn_names = (
                             cc_name_mode,)
                    else:
                    ...
                    if i_net_mode not in cn_names:
                        if not any((cn_name in connected_network_names for cn_name in cn_names)):
                            log.debug('Configurations network mode %s not matching instance mode %s. Additional connections: %s.', cn_names, i_net_mode, connected_network_names)
            ...
            for ref_n_name, cn_config in named_endpoints:
                ...
                if ref_n_name not in connected_network_names:
                    ...
                    if not network_endpoint_id:
                        ...
                        continue
                    if cn_config.links:
                        linked_names = {'{0}:{1}'.format(self._cname(config_id.map_name, lc_name), lc_alias or self._hostname(lc_name)) for lc_name, lc_alias in cn_config.links}
                    else:
                        linked_names = set()
                if not _check_network_endpoint(ref_n_name, cn_config, linked_names, network_detail, network_endpoint_id):
        ...
        if left_networks:
            log.debug('Container is connected to the following networks that it is not configured for: %s.', left_networks)
...
class UpdateNetworkState(NetworkBaseState):
    ...
    def get_state(self):
        ...
        if base_state == State.ABSENT or state_flags & StateFlags.NEEDS_RESET:
            return (
             base_state, state_flags, extra)
        self.endpoint_registry.register_network(self.detail)
        if not (self.detail['Driver'] != resolve_value(self.config.driver) or _check_network_driver_opts)(self.config, self.detail) or self.detail['Internal'] != resolve_value(self.config.internal):
