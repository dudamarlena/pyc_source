...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/kipp/aio/http.py
...
class HTTPResponse:
    ...
    def __repr__(self):
        return (b'HTTPResponse({})').format(repr(self.response))

    def __str__(self):
        return (b'HTTPResponse({})').format(str(self.response))

    @property
    def cookies(self):
        """Get cookies as dict"""
        cookies = {}
        for new_cookie in self.response.headers.get_list(b'Set-Cookie'):
...
class HTTPSessionClient:
    ...
    def fetch(self, *args, **kw):
        ...
        self._parse_body(kw)
        get_logger().debug(b'HTTPSessionClient fetch for args %s, kw %s', args, kw)
    ...
    def _parse_url(self, args, kw):
        ...
        params = kw.pop(b'params', {})
        url = kw.pop(b'request', b'') or args[0]
    ...
    def _parse_body(self, kw):
        ...
        data = kw.pop(b'data', {})
        djson = kw.pop(b'json', {})
        assert not (body and (data or djson)), b'you should not specified body with data/json'
        assert not (data and djson), b'you should not specified both data and json'
        if data:
            kw[b'body'] = str(urlencode(data))
            kw[b'headers'][b'Content-Type'] = b'application/x-www-form-urlencoded'
        if djson:
            kw[b'body'] = json_encode(djson)
            kw[b'headers'][b'Content-Type'] = b'application/javascript'

    def _parse_headers(self, kw):
        """Add ``Connection`` & ``Cookie`` into headers"""
        kw[b'headers'] = kw.get(b'headers', {})
        kw[b'headers'][b'Connection'] = b'keep-alive'
        self._parse_cookies(kw)

    def _parse_cookies(self, kw):
        """Add user custom cookies into headers"""
        user_cookies = kw.pop(b'cookies', {})
        user_cookies.update(kw[b'headers'].get(b'Cookie', {}))
        cookies = self._get_cookies(user_cookies)
        if cookies:
            kw[b'headers'][b'Cookie'] = cookies
    ...
    def _get_cookies(self, user_cookies=None):
        ...
        get_logger().debug(b'_get_cookies return cookies: %s', cookies)
        return cookies
    ...
    def _parse_cookies_to_str(self, cookies):
        ...
        return (b';').join([ (b'{}={}').format(n, v) for n, v in cookies.items() ])