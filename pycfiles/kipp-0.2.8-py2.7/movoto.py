...
class RuntimeStats:
    ...
    """
    _sql_to_create_runtime_stats = b'\n        insert into runtime_stats (name, stats)\n        values (%s, %s);\n        '
    _sql_to_update_runtime_stats = b'\n        update runtime_stats\n        set stats=%s\n        where name=%s;\n        '
    _RuntimeStats = namedtuple(b'stats', [b'created_at', b'updated_at', b'stats'])
    _sql_to_get_runtime_stats = b'\n        select created_at, updated_at, stats\n        from runtime_stats\n        where name=%s;\n        '
    _sql_to_delete_runtime_stats = b'\n        delete from runtime_stats\n        where name=%s;\n        '

    def validate_stats(self, stats):
        try:
            assert stats, b'stats should not empty'
            assert isinstance(stats, basestring), b'stats should be ``str``'
            assert len(stats) <= 200, b'the length of stats should shorter than 200'
        except AssertionError as err:
            raise DBValidateError(*err.args)

    def validate_name(self, name):
        try:
            assert name, b'name should not empty'
            assert isinstance(name, basestring), b'name should be ``str``'
            assert len(name) <= 50, b'the length of name should shorter than 50'
        except AssertionError as err:
            raise DBValidateError(*err.args)
    ...
    def create_runtime_stats(self, name, stats=None):
        ...
        except self.get_mysqldb_exception(b'IntegrityError') as err:
            if len(err.args) >= 2 and str(err.args[1]).startswith(b'Duplicate entry '):
                raise DuplicateIndexError((b'Duplicate name in ``runtime_stats`` for {}').format(name))
            else:
                raise
        else:
            return r
    ...
    def get_runtime_stats(self, name, **kwargs):
        ...
        if not r:
            if b'default' in kwargs:
                return kwargs[b'default']
            raise RecordNotFound((b'Can not find stats via name {}').format(name))
