...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/kipp/aio/sqlhelper.py
...
class SqlHelper:
    ...
    def __init__(self, *args, **kw):
        if PY2:
            get_logger().info(b'set SqlHelper for py2')
            from Utilities.movoto import settings
            self.sqlhelper = Py2SqlHelper(*args, **kw)
        elif PY3:
            get_logger().info(b'set SqlHelper for py3')
