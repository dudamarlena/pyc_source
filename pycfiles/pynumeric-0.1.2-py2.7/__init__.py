...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pynumeric/__init__.py
# Compiled at: 2017-12-20 22:31:05
...
class Numeric(object):
    ...
    def __init__(self, ioobj=None, filename=None):
        ...
        for line in filelines:
            ...
            elif key in ('LatCentre', 'LonCentre', 'LatitudeIncrement', 'LongitudeIncrement'):
    ...
    def to_grid(self, filename='out.tif', fmt='GTiff'):
        ...
        maxy = self.metadata['LatCentre'] + resy * (height / 2) - resy / 2
        dsource = gdal.GetDriverByName(str(fmt)).Create(filename, width, height, 1, gdal.GDT_Float64)
...
@click.option('--gdal-version', is_flag=True, is_eager=True, callback=gdal_version_callback, help='Show the GDAL version and exit.')
...
@click.command()
@click.pass_context
@click.option('--file', '-f', 'file_', type=click.Path(exists=True, resolve_path=True), help='Path to Numeric data file')
...
def report(ctx, file_, verbosity):
    ...
    with open(file_) as (fh):
        try:
            ...
            click.echo((' Spatial Extent: {}').format(n.get_data_spatial_extent()))
            data_range = n.get_data_range()
            click.echo((' Range of Values: min={} - max={}').format(data_range[0], data_range[1]))
...
@click.command()
@click.pass_context
@click.option('--file', '-f', 'file_in', type=click.Path(exists=True, resolve_path=True), help='Path to Numeric data file')
@click.option('--output', '-o', 'file_out', type=click.Path(exists=False, resolve_path=True), help='Path to output data file')
...
def export(ctx, file_in, file_out, format_, verbosity):
    ...
    with open(file_in) as (fh):
        try:
            click.echo(('Exporting {} to {} ({})').format(file_in, file_out, format_))
