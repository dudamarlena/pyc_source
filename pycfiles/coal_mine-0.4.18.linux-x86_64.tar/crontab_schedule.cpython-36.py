...
# [GCC 8.4.0]
# Embedded file name: /home/jik/.virtualenvs/coal-mine/lib/python3.6/site-packages/coal_mine/crontab_schedule.py
...
class FastCronTab(CronTab):
    ...
    def next(self, now=None, *args, **kwargs):
        if now is None:
            now = datetime.now()
        elif self.cached_now is not None and now > self.cached_now and now < self.cached_now + self.cached_next:
...
class CronTabSchedule(object):
    ...
    def __init__(self, crontab, delimiter='\n'):
        ...
        """
        self.entries = []
        entry_lines = [s for s in (s.strip() for s in crontab.split(delimiter)) if s if s[0] != '#']
    ...
    def add_entry(self, entry_line):
        ...
        if len(fields) < 6:
            raise CronTabScheduleException('{} does not have six fields'.format(entry_line))
    ...
    def next_minute(self, now=None, multi=True):
        ...
        if len(matches) == 0:
            ...
            if len(matches) > 1:
                if not multi:
                    raise CronTabScheduleException('Multiple schedule matches at {}'.format(now + ONE_MINUTE))
            if not multi:
                return matches[0]
            return matches
    ...
    def next_active(self, now=None, multi=True):
        ...
        while now < end:
            ...
            if matches is not None:
                return (
                 now + ONE_MINUTE, matches)
            now += ONE_MINUTE

        raise CronTabScheduleException('No active schedule matches in the year preceding {}'.format(end))
    ...
    def schedule_iter(self, start=None, end=None, multi=True, endless=False):
        ...
        current_rules = set()
        while not hard_stop and (endless or len(used_rules) < num_rules) or hard_stop and current_end < end:
            new_entries = self.next_minute(current_end, multi)
            if not multi or new_entries is None:
                new_entries = [
                 new_entries]
            new_key = tuple(self.key_of(e) for e in new_entries)
            if new_key != current_key or current_end - current_start > timedelta(days=31):
                if not multi or current_key[0] is None:
                    current_key = current_key[0]
                yield (
                 current_start, current_end, current_key)
