...
# [GCC 8.4.0]
# Embedded file name: /local_home/weigl/.virtualenvs/msml_env/lib/python2.7/site-packages/clictk/model.py
# Compiled at: 2015-06-12 16:53:19
import xml.etree.ElementTree as ET, subprocess
from itertools import chain
__all__ = [
 'Parameter', 'ParameterGroup', 'Executable', 'XMLArgumentNotSupportedByExecutable']
...
class ParameterGroup(object):
    ...
    def __repr__(self):
        return 'ParameterGroup(label=%(label)r, description=%(description)r, advanced=%(advanced)r, parameters=%(parameters)s )' % self.__dict__
    ...
    def __eq__(self, other):
        if isinstance(other, ParameterGroup) and other is not None:
            ...
            return l and self.advanced == other.advanced and self.label == other.label and self.description == other.description
        else:
            return False
            return
...
class Parameter(object):
    ...
    """
    XML_FIELDS = ('name', 'index', 'flag', 'longflag', 'description', 'label', 'default',
                  'channel', 'hidden')

    def __init__(self, name, type, default, description=None, channel=None, values=None, flag=None, index=None, label=None, longflag=None, file_ext=None):
        self.name = name
        self.type = type
    ...
    def __repr__(self):
        return 'Parameter(name=%(name)r, type=%(type)r, default=%(default)r, description=%(description)r, channel=%(channel)r, values=%(values)r,index=%(index)r, label=%(label)r, longflag=%(longflag)r, file_ext=%(file_ext)r)' % self.__dict__
    ...
    def from_xml_node(xml_node):
        ...
        if type in ('label', 'description'):
            ...
            return Parameter(name, type, default, doc, channel, values=values, index=index, label=label, longflag=longflag, file_ext=file_ext)
...
class Executable(object):
    ...
    def __init__(self, xml=None, executable=None, category=None, title=None, description=None, version=None, license=None, contributor=None, acknowledgements=None, documentation_url=None, parameter_groups=None):
    ...
    XML_FIELDS = ('category', 'title', 'description', 'version', 'documentation_url',
    ...
    def __repr__(self):
        return 'Executable(executable=%(executable)r, category=%(category)r, title=%(title)r, description=%(description)r, version=%(version)r,license=%(license)r, contributor=%(contributor)r, acknowledgements=%(acknowledgements)r, documentation_url=%(documentation_url)r,parameter_groups=%(parameter_groups)r)' % self.__dict__
    ...
    def from_etree(tree):
        ...
        for ps in tree.iterfind('parameters'):
            ...
            paras = ParameterGroup(ps.findtext('label'), ps.findtext('description'), ps.attrib.get('advanced', 'false') == 'true', filter(lambda x: x is not None, map(Parameter.from_xml_node, list(ps))))
