...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/azure/facade/appservice.py
...
class AppServiceFacade:
    ...
    async def get_web_apps(self, subscription_id: str):
        try:
            ...
            web_apps = await run_concurrently(lambda : list(client.web_apps.list()))
    ...
    async def _get_and_set_web_app_configuration(self, web_app, api_client):
        ...
        try:
            web_app_config = await run_concurrently(lambda : api_client.web_apps.get_configuration(resource_group_name, web_app.name))
        except Exception as e:
            try:
    ...
    async def _get_and_set_web_app_auth_settings(self, web_app, api_client):
        resource_group_name = get_resource_group_name(web_app.id)
        try:
            web_app_auth_settings = await run_concurrently(lambda : api_client.web_apps.get_auth_settings(resource_group_name=resource_group_name, name=(web_app.name)))
        except Exception as e:
            try:
