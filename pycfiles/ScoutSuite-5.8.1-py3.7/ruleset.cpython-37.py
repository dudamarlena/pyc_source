...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/core/ruleset.py
# Compiled at: 2020-04-02 05:37:10
# Size of source mod 2**32: 9551 bytes
...
class Ruleset:
    ...
    def __init__(self, cloud_provider, environment_name='default', filename=None, name=None, rules_dir=None, rule_type='findings', ip_ranges=None, account_id=None, ruleset_generator=False):
    ...
    def search_ruleset(self, environment_name, no_prompt=False):
        ...
        if environment_name != 'default':
            ...
            if os.path.exists(ruleset_file_path):
                if no_prompt or prompt_yes_no('A ruleset whose name matches your environment name was found in %s. Would you like to use it instead of the default one' % ruleset_file_name):
                    ruleset_found = True
    ...
    def find_file(self, filename, filetype='rulesets'):
        ...
        if filename:
            if not os.path.isfile(filename):
                if not filename.startswith('findings/'):
                    if not filename.startswith('filters/'):
