...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/aws/facade/rds.py
...
class RDSFacade(AWSBaseFacade):
    ...
    async def _cache_instances(self, region: str):
        async with self._regional_instances_cache_locks.setdefault(region, Lock()):
            ...
            for instance in self._instances_cache[region]:
                instance['VpcId'] = instance['DBSubnetGroup']['VpcId'] if ('DBSubnetGroup' in instance and 'VpcId' in instance['DBSubnetGroup'] and instance['DBSubnetGroup']['VpcId']) else ec2_classic

            await get_and_set_concurrently([
             self._get_and_set_instance_clusters, self._get_and_set_instance_tags],
              (self._instances_cache[region]), region=region)
    ...
    async def _get_and_set_instance_tags(self, instance: {}, region: str):
        ...
        try:
            instance_tagset = await run_concurrently(lambda : client.list_tags_for_resource(ResourceName=('arn:aws:rds:' + region + ':' + account_id + ':db:' + instance['DBInstanceIdentifier'])))
    ...
    async def _cache_snapshots(self, region: str):
        async with self._regional_snapshots_cache_locks.setdefault(region, Lock()):
            ...
            await get_and_set_concurrently([
             self._get_and_set_snapshot_attributes],
              (self._snapshots_cache[region]), region=region)

    async def _get_and_set_snapshot_attributes(self, snapshot: {}, region: str):
        client = AWSFacadeUtils.get_client('rds', self.session, region)
        try:
            attributes = await run_concurrently(lambda : client.describe_db_snapshot_attributes(DBSnapshotIdentifier=(snapshot['DBSnapshotIdentifier']))['DBSnapshotAttributesResult'])
    ...
    async def get_parameter_groups(self, region: str):
        try:
            try:
                ...
                await get_and_set_concurrently([
                 self._get_and_set_db_parameters],
                  parameter_groups, region=region)
            except Exception as e:
                try:
    ...
    async def _get_and_set_db_parameters(self, parameter_group: {}, region: str):
        ...
        try:
            parameters = await AWSFacadeUtils.get_all_pages('rds',
              region, (self.session), 'describe_db_parameters', 'Parameters', DBParameterGroupName=name)
