...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/aws/facade/ec2.py
# Compiled at: 2020-04-02 05:37:10
# Size of source mod 2**32: 8881 bytes
import asyncio, base64, boto3, zlib
...
class EC2Facade(AWSBaseFacade):
    ...
    def __init__(self, session, owner_id):
    ...
    async def get_instance_user_data(self, region: str, instance_id: str):
        ...
        try:
            user_data_response = await run_concurrently(lambda : ec2_client.describe_instance_attribute(Attribute='userData', InstanceId=instance_id))
    ...
    async def get_instances(self, region: str, vpc: str):
        ...
        try:
            reservations = await AWSFacadeUtils.get_all_pages('ec2',
              region, (self.session), 'describe_instances', 'Reservations', Filters=filters)
            instances = []
    ...
    async def get_security_groups(self, region: str, vpc: str):
        filters = [{'Name':'vpc-id',  'Values':[vpc]}]
        try:
            return await AWSFacadeUtils.get_all_pages('ec2',
              region, (self.session), 'describe_security_groups', 'SecurityGroups', Filters=filters)
        except Exception as e:
            try:
    ...
    async def get_network_interfaces(self, region: str, vpc: str):
        ...
        try:
            return await AWSFacadeUtils.get_all_pages('ec2',
              region, (self.session), 'describe_network_interfaces', 'NetworkInterfaces', Filters=filters)
        except Exception as e:
            try:
    ...
    async def get_snapshots(self, region: str):
        ...
        try:
            try:
                snapshots = await AWSFacadeUtils.get_all_pages('ec2',
                  region, (self.session), 'describe_snapshots', 'Snapshots', Filters=filters)
            except Exception as e:
                try:
    ...
    async def _get_and_set_snapshot_attributes(self, snapshot: {}, region: str):
        ec2_client = AWSFacadeUtils.get_client('ec2', self.session, region)
        try:
            snapshot['CreateVolumePermissions'] = await run_concurrently(lambda : ec2_client.describe_snapshot_attribute(Attribute='createVolumePermission',
    ...
    async def get_network_acls(self, region: str, vpc: str):
        ...
        try:
            return await AWSFacadeUtils.get_all_pages('ec2',
              region, (self.session), 'describe_network_acls', 'NetworkAcls', Filters=filters)
        except Exception as e:
            try:
    ...
    async def cache_flow_logs(self, region: str):
        async with self.regional_flow_logs_cache_locks.setdefault(region, asyncio.Lock()):
            if region in self.flow_logs_cache:
                return
            self.flow_logs_cache[region] = await AWSFacadeUtils.get_all_pages('ec2', region, self.session, 'describe_flow_logs', 'FlowLogs')
    ...
    async def _get_and_set_subnet_flow_logs(self, subnet: {}, region: str):
        await self.cache_flow_logs(region)
        subnet['flow_logs'] = [flow_log for flow_log in self.flow_logs_cache[region] if not flow_log['ResourceId'] == subnet['SubnetId'] if flow_log['ResourceId'] == subnet['VpcId']]
