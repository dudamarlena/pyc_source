...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/aws/facade/redshift.py
...
class RedshiftFacade(AWSBaseFacade):
    ...
    async def cache_clusters(self, region):
        async with self.regional_cluster_cache_locks.setdefault(region, Lock()):
            if region in self.clusters_cache:
                return
            self.clusters_cache[region] = await AWSFacadeUtils.get_all_pages('redshift', region, self.session, 'describe_clusters', 'Clusters')
            for cluster in self.clusters_cache[region]:
                cluster['VpcId'] = cluster['VpcId'] if ('VpcId' in cluster and cluster['VpcId']) else ec2_classic

    async def get_cluster_parameter_groups(self, region: str):
        try:
            return await AWSFacadeUtils.get_all_pages('redshift', region, self.session, 'describe_cluster_parameter_groups', 'ParameterGroups')
        except Exception as e:
            try:
    ...
    async def get_cluster_security_groups(self, region: str):
        try:
            return await AWSFacadeUtils.get_all_pages('redshift', region, self.session, 'describe_cluster_security_groups', 'ClusterSecurityGroups')
    ...
    async def get_cluster_parameters(self, region: str, parameter_group: str):
        return await AWSFacadeUtils.get_all_pages('redshift',
          region, (self.session), 'describe_cluster_parameters', 'Parameters', ParameterGroupName=parameter_group)