...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/aws/facade/elbv2.py
...
class ELBv2Facade(AWSBaseFacade):
    ...
    async def cache_load_balancers(self, region):
        async with self.regional_load_balancers_cache_locks.setdefault(region, asyncio.Lock()):
            if region in self.load_balancers_cache:
                return
            self.load_balancers_cache[region] = await AWSFacadeUtils.get_all_pages('elbv2', region, self.session, 'describe_load_balancers', 'LoadBalancers')
    ...
    async def _get_and_set_load_balancer_attributes(self, load_balancer: dict, region: str):
        ...
        try:
            load_balancer['attributes'] = await run_concurrently(lambda : elbv2_client.describe_load_balancer_attributes(LoadBalancerArn=(load_balancer['LoadBalancerArn']))['Attributes'])
        except Exception as e:
            try:
    ...
    async def _get_and_set_load_balancer_tags(self, load_balancer: dict, region: str):
        elbv2_client = AWSFacadeUtils.get_client('elbv2', self.session, region)
        try:
            load_balancer['Tags'] = await run_concurrently(lambda : elbv2_client.describe_tags(ResourceArns=[
             load_balancer['LoadBalancerArn']])['TagDescriptions'][0]['Tags'])
        except Exception as e:
            try:
    ...
    async def get_listeners(self, region: str, load_balancer_arn: str):
        return await AWSFacadeUtils.get_all_pages('elbv2',
          region, (self.session), 'describe_listeners', 'Listeners', LoadBalancerArn=load_balancer_arn)