...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/azure/resources/storageaccounts/storage_accounts.py
...
class StorageAccounts(AzureCompositeResources):
    ...
    async def fetch_all(self):
        ...
        await self._fetch_children_of_all_resources(resources=self,
          scopes={storage_account_id:{'resource_group_name':storage_account['resource_group_name'],  'storage_account_name':storage_account['name'],  'subscription_id':self.subscription_id} for storage_account_id, storage_account in self.items()})
    ...
    def _parse_storage_account(self, raw_storage_account):
        ...
        storage_account['access_keys_last_rotation_date'] = self._parse_access_keys_last_rotation_date(raw_storage_account.activity_logs)
        return (
         storage_account['id'], storage_account)
    ...
    def _parse_access_keys_last_rotation_date(self, activity_logs):
        ...
        for log in activity_logs:
            if not log.operation_name.value == 'Microsoft.Storage/storageAccounts/regenerateKey/action' or last_rotation_date is None or last_rotation_date < log.event_timestamp:
