...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/core/fs.py
    ...
    try:
        if prompt_overwrite(filename, force_write):
            with open(filename, 'wt') as (f):
                print_info('%s' % json.dumps(blob, indent=4, separators=(',', ': '), sort_keys=True, cls=CustomJSONEncoder))
