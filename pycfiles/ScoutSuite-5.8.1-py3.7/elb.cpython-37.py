...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/aws/facade/elb.py
...
class ELBFacade(AWSBaseFacade):
    ...
    async def cache_load_balancers(self, region):
        async with self.regional_load_balancers_cache_locks.setdefault(region, asyncio.Lock()):
            if region in self.load_balancers_cache:
                return
            self.load_balancers_cache[region] = await AWSFacadeUtils.get_all_pages('elb', region, self.session, 'describe_load_balancers', 'LoadBalancerDescriptions')
            for load_balancer in self.load_balancers_cache[region]:
                load_balancer['VpcId'] = load_balancer['VPCId'] if ('VPCId' in load_balancer and load_balancer['VPCId']) else ec2_classic

            await get_and_set_concurrently([
             self._get_and_set_load_balancer_attributes],
              (self.load_balancers_cache[region]), region=region)
            await get_and_set_concurrently([
    ...
    async def _get_and_set_load_balancer_attributes(self, load_balancer: {}, region: str):
        ...
        try:
            load_balancer['attributes'] = await run_concurrently(lambda : elb_client.describe_load_balancer_attributes(LoadBalancerName=(load_balancer['LoadBalancerName']))['LoadBalancerAttributes'])
        except Exception as e:
            try:
    ...
    async def _get_and_set_load_balancer_tags(self, load_balancer: {}, region: str):
        elb_client = AWSFacadeUtils.get_client('elb', self.session, region)
        try:
            load_balancer['Tags'] = await run_concurrently(lambda : elb_client.describe_tags(LoadBalancerNames=[
             load_balancer['LoadBalancerName']])['TagDescriptions'][0]['Tags'])
        except Exception as e:
            try:
    ...
    async def _get_policies(self, load_balancer: dict, region: str):
        ...
        elb_client = AWSFacadeUtils.get_client('elb', self.session, region)
        try:
            return await run_concurrently(lambda : elb_client.describe_load_balancer_policies(LoadBalancerName=(load_balancer['LoadBalancerName']),
