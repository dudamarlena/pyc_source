...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/azure/facade/aad.py
...
class AADFacade:
    ...
    def __init__(self, credentials):
        self._client = GraphRbacManagementClient((credentials.aad_graph_credentials), tenant_id=(credentials.get_tenant_id()))
    ...
    async def get_user_groups(self, user_id):
        try:
            return await run_concurrently(lambda : list(self._client.users.get_member_groups(object_id=user_id, security_enabled_only=False)))
        except Exception as e:
            try:
