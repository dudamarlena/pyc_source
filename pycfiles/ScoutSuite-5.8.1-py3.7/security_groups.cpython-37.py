...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/azure/resources/network/security_groups.py
...
class SecurityGroups(AzureResources):
    ...
    def _parse_network_security_group(self, network_security_group):
        ...
        return (
         network_security_group_dict['id'], network_security_group_dict)
    ...
    def _parse_security_rule(self, rule, default=False):
        ...
        source_address_prefixes = self._merge_prefixes_or_ports(rule.source_address_prefix, rule.source_address_prefixes if rule.source_address_prefixes else get_non_provider_id(rule.source_application_security_groups[0].id) if rule.source_application_security_groups else None)
        ...
        destination_address_prefixes = self._merge_prefixes_or_ports(rule.destination_address_prefix, rule.destination_address_prefixes)
        security_rule_dict['destination_address_prefixes'] = destination_address_prefixes
        destination_port_ranges = self._merge_prefixes_or_ports(rule.destination_port_range, rule.destination_port_ranges)
        ...
        return (
         security_rule_dict['id'], security_rule_dict)
    ...
    def _parse_exposed_ports(self, network_security_group):
        ...
        for sr in rules:
            if not sr.direction == 'Inbound' or sr.source_address_prefix == '*' or sr.source_address_prefix == 'Internet':
                port_ranges = self._merge_prefixes_or_ports(sr.destination_port_range, sr.destination_port_ranges)
