...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/providers/gcp/facade/gce.py
...
class GCEFacade(GCPBaseFacade):
    ...
    def metadata_to_dict(self, metadata):
        if 'items' in metadata:
            return dict(((item['key'], item['value']) for item in metadata['items']))
        return {}
    ...
    async def get_project(self, project_id):
        try:
            gce_client = self._get_client()
            return await run_concurrently(lambda : gce_client.projects().get(project=project_id).execute())
        except Exception as e:
            try:
    ...
    async def get_subnetwork(self, project_id, region, subnetwork_id):
        try:
            gce_client = self._get_client()
            return await run_concurrently(lambda : gce_client.subnetworks().get(project=project_id, region=region, subnetwork=subnetwork_id).execute())
        except Exception as e:
            try:
