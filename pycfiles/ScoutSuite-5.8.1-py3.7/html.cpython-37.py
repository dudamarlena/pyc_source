...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ScoutSuite/output/html.py
# Compiled at: 2020-04-02 05:37:10
# Size of source mod 2**32: 6770 bytes
from __future__ import print_function
import datetime, os, shutil, zipfile, dateutil.tz
            ...
            with open(os.path.join(self.html_data_path, 'report.html')) as (f):
                with open(new_file, 'wt') as (nf):
                    for line in f:
                        ...
                        newline = newline.replace('<!-- RESULTS PLACEHOLDER -->', get_filename('RESULTS', (self.report_name),
                          (self.report_dir),
                          relative_path=True)[0])
                        newline = newline.replace('<!-- EXCEPTIONS PLACEHOLDER -->', get_filename('EXCEPTIONS', (self.report_name),
                          (self.report_dir),
                          relative_path=True)[0])
                        newline = newline.replace('<!-- SQLITE JS PLACEHOLDER -->', '{}/sqlite.js'.format(DEFAULT_INCLUDES_DIRECTORY))
                        nf.write(newline)
