...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\picsim.py
# Compiled at: 2019-10-06 18:48:06
# Size of source mod 2**32: 39058 bytes
"""
...
class MaterialStack:
    ...
    def interpolate_points(self, key, num_points):
        ...
        points = []
        for i in np.linspace(1e-08, self.vsize, num_points):
...
def export_component_to_hdf5(filename, component, mstack, boolean_operations):
    ...
    **Note**: that the top-down view of the device is the 'X-Z' plane.  The 'Y' direction specifies the vertical height.
    ...
    for key in polygon_dict.keys():
        ...
        polygons_union = gdspy.fast_boolean(polygons,
          polygons, 'or', max_points=99999, layer=(key[0]), datatype=(key[1]))
        polygon_dict[key] = polygons_union.polygons

    for bo in boolean_operations:
        polygons_bool = gdspy.fast_boolean((polygon_dict[bo[0]]),
          (polygon_dict[bo[1]]),
          (bo[2]),
          layer=(bo[0][0]),
          datatype=(bo[0][1]))
...
def export_timestep_fields_to_png(directory):
    ...
    filename = 'mcts'
    call(('h5topng -S3 -m1 -M4 ' + str(directory) + '/topview-' + str(filename) + '-eps-000000.00.h5'),
      shell=True)
    call(('h5topng -S3 -m1 -M4 ' + str(directory) + '/sideview-' + str(filename) + '-eps-000000.00.h5'),
      shell=True)
...
def compute_mode(wgt, mstack, res, wavelength, sx, sy, plot_mode_number=1, polarization='TE', output_directory='mpb-sim', save_mode_data=True, suppress_window=False):
    ...
    exec_str = "python mcm.py -res %d -wavelength %0.3f -sx %0.3f -sy %0.3f -plot_mode_number %d -polarization %s -epsilon_file '%s/%s' -output_directory '%s/%s' -save_mode_data %r -suppress_window %r > '%s/%s-res%d.out'" % (
     res,
     wavelength,
     sx,
     sy,
...
def compute_transmission_spectra(pic_component, mstack, wgt, ports, port_vcenter, port_height, port_width, res, wl_center, wl_span, boolean_operations=None, norm=False, input_pol='TE', nfreq=100, dpml=0.5, fields=False, plot_window=False, source_offset=0.1, symmetry=None, skip_sim=False, output_directory='meep-sim', parallel=False, n_p=2):
    ...
    **Currently only supports components with port-directions that are `EAST` (0) or `WEST` (pi)**
    ...
    if boolean_operations == None:
        ...
        for port in ports:
            if isinstance(port['direction'], float):
                if abs(port['direction']) < 1e-06:
                    input_directions.append(-1)
                else:
                    if abs(port['direction'] - np.pi) < 1e-06:
                        input_directions.append(1)
                    else:
        ...
        if norm:
            ...
            if norm:
                ...
                norm_ports = [
                 wg2.portlist['input'], wg2.portlist['output']]
                ...
                if parallel:
                    exec_str = "mpirun -np %d python mcts.py -fields %r -input_pol %s -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-norm-res%d.out'" % (
                     int(n_p),
                     False,
                     input_pol,
                     str(os.getcwd()),
                    ...
                    exec_str = "python mcts.py -fields %r -input_pol %s -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-norm-res%d.out'" % (
                     False,
                     input_pol,
                     str(os.getcwd()),
                     str(output_directory),
            ...
            port_string = str(port_string[:-1])
            if parallel:
                exec_str = "mpirun -np %d python mcts.py -fields %r -input_pol %s -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-res%d.out'" % (
                 int(n_p),
                 fields,
                 input_pol,
                 str(os.getcwd()),
    ...
    grep_str = "grep flux1: '%s/%s-res%d.out' > '%s/%s-res%d.dat'" % (
     ...
     res)
    call(grep_str, shell='True')
    norm_data = np.genfromtxt(('%s/%s-norm-res%d.dat' % (str(os.getcwd()), str(output_directory), res)),
      delimiter=',')
    freq, refl0, trans0 = norm_data[:, 1], -norm_data[:, 2], norm_data[:, 3]
    comp_data = np.genfromtxt(('%s/%s-res%d.dat' % (str(os.getcwd()), str(output_directory), res)),
      delimiter=',')
    flux_data = []
