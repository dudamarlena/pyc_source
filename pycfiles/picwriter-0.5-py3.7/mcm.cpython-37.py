...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\mcm.py
# Compiled at: 2019-10-06 18:48:06
# Size of source mod 2**32: 17052 bytes
"""
    ...
    if polarization == 'TE':
        ...
        if polarization == 'TM':
            ...
            if not suppress_window:
                ...
                plt.subplot(2, 3, 5)
                plt.imshow(eps,
                  cmap=cmap_geom, origin='lower', aspect='auto', extent=plt_extent)
            ...
            if save_mode_data:
                ...
                if polarization == 'TE':
                    ...
                    if polarization == 'TM':
                        ...
                        plt.savefig(savetxt)
                if polarization == 'TE':
                    datafilename = '%s/TE_mode%d_data.txt' % (
                     output_directory,
                     plot_mode_number)
                else:
                    if polarization == 'TM':
                        datafilename = '%s/TM_mode%d_data.txt' % (
                         output_directory,
                         plot_mode_number)
                    else:
                        if polarization == 'None':
...
if __name__ == '__main__':
    ...
    parser.add_argument('-wavelength',
      type=float,
      default=1.55,
      help='Wavelength in microns (default=1.55)')
    parser.add_argument('-sx',
      type=float,
      default=4.0,
      help='Size of the simulation region in the x-direction (default=4.0)')
    parser.add_argument('-sy',
      type=float,
      default=4.0,
      help='Size of the simulation region in the y-direction (default=4.0)')
    parser.add_argument('-plot_mode_number',
      type=int,
      default=1,
      help='Which mode to plot (only plots one mode at a time).  Must be a number equal to or less than num_mode (default=1)')
    parser.add_argument('-polarization',
      type=str,
      default=None,
      help='Name of the output directory (for storing the fields) (default=None)')
    parser.add_argument('-epsilon_file',
      type=str,
      default=None,
      help='Filename with the dielectric "block" objects (default=None)')
    parser.add_argument('-output_directory',
      type=str,
      default=None,
      help='Name of the output directory (for storing the fields) (default=None)')
    parser.add_argument('-save_mode_data',
      type=str2bool,
      nargs='?',
      const=True,
      default=True,
      help='Save the mode image and data to a separate file (default=True)')
    parser.add_argument('-suppress_window',
      type=str2bool,
      nargs='?',
      const=True,
      default=False,
      help='Suppress the matplotlib window (default=False)')
