...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\components\adiabaticcoupler.py
...
class AdiabaticCoupler(tk.Component):
    ...
    def __init__(self, wgt, length1, length2, length3, wg_sep, input_wg_sep, output_wg_sep, dw, port=(0, 0), direction='EAST'):
        ...
        self.portlist_output_top = (
         ...
         self.yc + self.output_wg_sep / 2.0)
        self.portlist_output_bot = (
         length1 + length2 + length3,
         self.yc - self.output_wg_sep / 2.0)
        self._AdiabaticCoupler__build_cell()
        self._AdiabaticCoupler__build_ports()
    ...
    def __build_cell(self):
        ...
        input_top_bezier = BBend((self.wgt),
          poles, end_width=(self.wgt.wg_width + self.dw / 2.0))
        ...
        poles = [
         ...
         (
          self.length1, self.yc - self.wg_sep / 2.0)]
        input_bot_bezier = BBend((self.wgt),
          poles, end_width=(self.wgt.wg_width - self.dw / 2.0))
        self.add(input_bot_bezier)
        taper_top = Taper(
 self.wgt,
 self.length2, end_width=self.wgt.wg_width, 
         start_width=self.wgt.wg_width + self.dw / 2.0, **input_top_bezier.portlist['output'])
        self.add(taper_top)
        taper_bot = Taper(
 self.wgt,
 self.length2, end_width=self.wgt.wg_width, 
         start_width=self.wgt.wg_width - self.dw / 2.0, **input_bot_bezier.portlist['output'])
        self.add(taper_bot)
        poles = [
         (
          self.length1 + self.length2, self.yc + self.wg_sep / 2.0),
         (
          self.length1 + self.length2 + self.length3 / 2.0,
          self.yc + self.wg_sep / 2.0),
         (
          self.length1 + self.length2 + self.length3 / 2.0,
          self.yc + self.output_wg_sep / 2.0),
         (
    ...
    def __build_ports(self):
        self.portlist['input_top'] = {'port':self.portlist_input_top, 
         'direction':'WEST'}
        self.portlist['input_bot'] = {'port':self.portlist_input_bot, 
         'direction':'WEST'}
        self.portlist['output_top'] = {'port':self.portlist_output_top, 
         'direction':'EAST'}
        self.portlist['output_bot'] = {'port':self.portlist_output_bot, 
         'direction':'EAST'}
...
if __name__ == '__main__':
 ...
 wgt, length1=30.0, 
     ...
     input_wg_sep=3.0, 
     output_wg_sep=3.0, 
     dw=0.1, **wg1.portlist['output'])
    tk.add(top, ac)
    ac2 = AdiabaticCoupler(
 wgt, length1=20.0, 
     length2=50.0, 
     length3=30.0, 
     wg_sep=1.0, 
     input_wg_sep=3.0, 
     output_wg_sep=3.0, 
     dw=0.1, **ac.portlist['output_bot'])
    tk.add(top, ac2)
    for p in ac.portlist.keys():
        print(str(p) + ': ' + str(ac.portlist[p]['port']))

    gdspy.LayoutViewer()
    gdspy.write_gds('ac.gds', unit=1e-06, precision=1e-09)