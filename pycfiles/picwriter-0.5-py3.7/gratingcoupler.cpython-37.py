...
class GratingCoupler(tk.Component):
    ...
    def __init__(self, wgt, theta=np.pi / 4.0, length=30.0, taper_length=10.0, period=1.0, dutycycle=0.7, ridge=False, ridge_layers=(3, 0), teeth_list=None, port=(0, 0), direction='EAST'):
        ...
        if dutycycle > 1.0 or dutycycle < 0.0:
            raise ValueError('Warning! Dutycycle *must* specify a valid number between 0 and 1.')
        self.dc = dutycycle
        self.ridge = ridge
        ...
        self._GratingCoupler__build_cell()
        self._GratingCoupler__build_ports()
        self._auto_transform_()

    def __build_cell(self):
        """ Create a straight grating GratingCoupler
        """
        taper = (gdspy.Round)((0, 0), radius=self.taper_length, 
         inner_radius=0, 
         initial_angle=-self.theta / 2.0, 
         final_angle=+self.theta / 2.0, 
         number_of_points=self.wgt.get_num_points_curve(self.theta, self.taper_length) + 1, **self.wg_spec)
        if self.ridge:
            ...
            if self.teeth_list == None:
                ...
                for i in range(num_teeth):
                    ...
                    line = (gdspy.Round)((0, 0), radius=outer_rad, 
                     inner_radius=inner_rad, 
                     initial_angle=-self.theta / 2.0, 
                     final_angle=+self.theta / 2.0, 
                     number_of_points=2 * self.wgt.get_num_points_curve(self.theta, outer_rad), **self.wg_spec)
                    self.add(line)
...
class GratingCouplerStraight(tk.Component):
    ...
    def __init__(self, wgt, port=(0, 0), direction='EAST', width=20, length=50, taper_length=20, period=1.0, dutycycle=0.5):
        ...
        self.wg_spec = {'layer':wgt.wg_layer,  'datatype':wgt.wg_datatype}
        self.clad_spec = {'layer':wgt.clad_layer,  'datatype':wgt.clad_datatype}
        self._GratingCouplerStraight__build_cell()
        self._GratingCouplerStraight__build_ports()
        self._auto_transform_()
...
class GratingCouplerFocusing(tk.Component):
    ...
    def __init__(self, wgt, port=(0, 0), direction='EAST', focus_distance=None, width=20, length=50, period=1.0, dutycycle=0.5, wavelength=1.55, sin_theta=np.sin(np.pi * 8 / 180), evaluations=99):
        ...
        self.wg_spec = {'layer':wgt.wg_layer,  'datatype':wgt.wg_datatype}
        self.clad_spec = {'layer':wgt.clad_layer,  'datatype':wgt.clad_datatype}
        self._GratingCouplerFocusing__build_cell()
        self._GratingCouplerFocusing__build_ports()
        self._auto_transform_()
    ...
    def __build_cell(self):
        ...
        w = 0.5 * self.width
        path = gdspy.Path((self.wgt.clad_width),
          (0, 0),
          number_of_paths=2,
 ...
 lambda t: (
  self.width * t - w,
  (c1 + neff * np.sqrt(c2 - c3 * (self.width * t - w) ** 2)) / c3), number_of_evaluations=self.evaluations, 
        ...
        teeth.polygons[0] = np.vstack((
         ...
         [
          (
           0.5 * self.wgt.wg_width, 0), (-0.5 * self.wgt.wg_width, 0)]))
        teeth.fracture()
        clad_path = gdspy.Path(self.wgt.wg_width + 2 * self.wgt.clad_width, (0, 0))
        (clad_path.segment)(
 self.focus_distance, direction='+y', 
         final_width=self.width + 2 * self.wgt.clad_width, **self.clad_spec)
...
if __name__ == '__main__':
 ...
 wgt, theta=np.pi / 5.0, 
     ...
     period=1.0, 
     dutycycle=0.6, 
     ridge=True, **wg1.portlist['output'])
    tk.add(top, gc1)
    teeth_list = [
     (0.2, 0.6),
     (0.25, 0.55),
 ...
 wgt, theta=np.pi / 4.0, 
     ...
     dutycycle=0.6, 
     ridge=True, 
     teeth_list=teeth_list, **wg1.portlist['input'])
    tk.add(top, gc2)
    gdspy.write_gds('gratingcoupler.gds', unit=1e-06, precision=1e-09)