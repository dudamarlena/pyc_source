...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\geomeep.py
# Compiled at: 2018-04-14 23:44:06
# Size of source mod 2**32: 22771 bytes
"""
...
def compute_transmission_spectra(pic_component, mstack, ports, port_vcenter, port_height, port_width, res, wl_center, wl_span, norm=False, wgt=None, nfreq=100, dpml=0.5, fields=False, source_offset=0.1, symmetry=None, convert_component_to_hdf5=True, output_directory='meep-sim', parallel=False, n_p=2):
    ...
    **Currently only supports components with port-directions that are `EAST` (0) or `WEST` (pi)**
    ...
    for port in ports:
        if isinstance(port['direction'], float):
            ...
            elif abs(port['direction'] - np.pi) < 1e-06:
                input_directions.append(1)
            else:
    ...
    if norm:
        ...
        if parallel:
            exec_str = "mpirun -np %d python mcts.py -fields %r -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-norm-res%d.out'" % (
             int(n_p), False, str(os.getcwd()), str(output_directory), str(os.getcwd()),
            ...
            exec_str = "python mcts.py -fields %r -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-norm-res%d.out'" % (
             False, str(os.getcwd()), str(output_directory), str(os.getcwd()),
             eps_norm_input_file, res, nfreq, input_directions[0], float(dpml), float(wl_center),
             float(wl_span), float(port_vcenter), float(port_height), float(port_width),
             float(source_offset), float(center[0]), float(center[1]), float(center[2]),
             float(sx), float(sy), float(sz), port_string, str(os.getcwd()), str(output_directory), res)
        ...
        if parallel:
            exec_str = "mpirun -np %d python mcts.py -fields %r -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-res%d.out'" % (
             int(n_p), fields, str(os.getcwd()), str(output_directory), str(os.getcwd()),
            ...
            exec_str = "python mcts.py -fields %r -output_directory '%s/%s' -eps_input_file '%s/%s' -res %d -nfreq %d -input_direction %d -dpml %0.3f -wl_center %0.3f -wl_span %0.3f -port_vcenter %0.3f -port_height %0.3f -port_width %0.3f -source_offset %0.3f -center_x %0.3f -center_y %0.3f -center_z %0.3f -sx %0.3f -sy %0.3f -sz %0.3f -port_coords %r > '%s/%s-res%d.out'" % (
             fields, str(os.getcwd()), str(output_directory), str(os.getcwd()),
             eps_input_file, res, nfreq, input_directions[0], float(dpml), float(wl_center),
             float(wl_span), float(port_vcenter), float(port_height), float(port_width),
             float(source_offset), float(center[0]), float(center[1]), float(center[2]),
             float(sx), float(sy), float(sz), port_string, str(os.getcwd()), str(output_directory), res)
