...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\meep_compute_transmission_spectra.py
# Compiled at: 2018-04-14 20:47:08
...
def main(args):
    """
    Args:
       * **norm** (boolean): If true, performs a MEEP FDTD simulation of a straight waveguide
    ...
    if len(port_coords) % 2 != 0:
        ...
        fcen = (fmax + fmin) / 2.0
        df = fmax - fmin
        if abs(abs(input_direction) - 1) > 1e-06:
        ...
        flux_plane_objects = []
        for port in ports:
            flux_region = mp.FluxRegion(size=(mp.Vector3(0, float(port_height), float(port_width))), center=(mp.Vector3(float(port[0]) - center_x, float(port_vcenter) - center_y, float(port[1]) - center_z)))
        ...
        if fields:
            sim.run((mp.at_beginning(mp.output_epsilon)), (mp.at_beginning(mp.with_prefix(str('sideview-'), mp.in_volume(sv, mp.output_epsilon)))),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-norm', type=str2bool, nargs='?', const=True, default=False, help='If true, performs a MEEP FDTD simulation of a straight waveguide (default=False)')
    ...
    parser.add_argument('-wl_span', type=float, default=0.3, help='Spectral width of the Gaussian pulse [in um] (default=0.300)')
