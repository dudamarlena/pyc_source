...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\components\waveguide.py
# Compiled at: 2019-10-21 22:45:01
# Size of source mod 2**32: 44067 bytes
    ...
    def __init__(self, wg_type='strip', bend_radius=50.0, waveguide_stack=None, wg_width=2.0, clad_width=10.0, grid=0.001, resist='+', fab='ETCH', slot=0.1, period=0.1, duty_cycle=0.5, wg_layer=1, wg_datatype=0, clad_layer=2, clad_datatype=0, euler_bend=False):
        ...
        if self.wg_type == 'slot':
            ...
            if self.wg_type == 'swg':
                ...
                if resist != '+':
                    if resist != '-':
                        raise ValueError('Warning, invalid input for kwarg resist in WaveguideTemplate')
                    if fab == 'ETCH':
                        self.resist = resist
                else:
            ...
            if self.euler:
                self.scale_factor = self.bend_radius / 0.45015815807855303
            ...
            if self.wg_type == 'swg':
                ...
                straight_path.segment((self.period * self.duty_cycle),
                  direction='+x',
                  layer=(self.wg_layer),
                ...
                bend_path.arc((self.bend_radius),
                  0,
...
class Waveguide(tk.Component):
    ...
    def __init__(self, trace, wgt):
        ...
        self.clad_spec = {'layer':wgt.clad_layer, 
         'datatype':wgt.clad_datatype}
        self._Waveguide__type_check_trace()
        self._Waveguide__build_cell()
        self._Waveguide__build_ports()
    ...
    def __type_check_trace(self):
        ...
        for i in range(len(self.trace) - 1):
            ...
            if prev_dx <= 1e-06 and dx <= 1e-06 or prev_dy <= 1e-06:
                if dy <= 1e-06:
                    raise ValueError('Warning! Unnecessary waypoint specified.  All waypoints must specify a valid bend')
    ...
    def __build_cell(self):
        ...
        if self.wgt.wg_type == 'swg':
            ...
            bends = [[None, None, None] for i in range(len(self.trace) - 2)]
            prev_dl = 0.0
            for i in range(len(self.trace)):
                if i == 0:
                    segments[i][0] = self.trace[i]
                elif i == len(self.trace) - 1:
                    segments[(i - 1)][1] = self.trace[i]
                else:
                    ...
                    if dl + prev_dl > tk.dist(self.trace[(i - 1)], self.trace[i]) + 1e-06:
                        raise ValueError('Warning! The waypoints ' + str(self.trace[(i - 1)]) + ' and ' + str(self.trace[i]) + ' are too close to accommodate  the necessary bend-radius of ' + str(br) + ', the points were closer than ' + str(dl + prev_dl))
                    else:
 ...
 remaining_period - self.wgt.period * (1 - self.wgt.duty_cycle), direction=direction, **self.wg_spec)
                        ...
                        for j in range(num_periods):
                            self.add(gdspy.CellReference((self.wgt.straight_period_cell),
                              origin=curr_point,
                        ...
                        if tk.dist(curr_point, segment[1]) < self.wgt.period * self.wgt.duty_cycle:
                            last_path = gdspy.Path((self.wgt.wg_width),
                              initial_point=curr_point)
                            (last_path.segment)(
 tk.dist(curr_point, segment[1]), direction=direction, **self.wg_spec)
                            self.add(last_path)
                        else:
                            self.add(gdspy.CellReference((self.wgt.straight_period_cell),
                              origin=curr_point,
 ...
 total_dist, direction=direction, **self.wg_spec)
                    ...
                    if abs(angle_change) > remaining_angle:
                        if angle_change > 0:
                            if remaining_angle > angular_period * (1 - self.wgt.duty_cycle):
                                first_path = gdspy.Path((self.wgt.wg_width),
                                  initial_point=(tk.translate_point(bend[0], br, curr_angle)))
                                (first_path.arc)(
                                 br, 
                                 curr_angle, 
                                 (curr_angle + remaining_angle - angular_period * (1 - self.wgt.duty_cycle)), **self.wg_spec)
                                self.add(first_path)
                        ...
                        if angle_change > 0:
                            for j in range(num_periods):
                                self.add(gdspy.CellReference((self.wgt.bend_period_cell),
                                  origin=(bend[0]),
                            ...
                            if abs(tk.normalize_angle(bend[2] - curr_angle)) < angular_period * self.wgt.duty_cycle:
                                last_path = gdspy.Path((self.wgt.wg_width),
                                  initial_point=(tk.translate_point(bend[0], br, bend[2])))
                                (last_path.arc)(
                                 br, 
                                 (bend[2]), 
                                 (bend[2] + abs(tk.normalize_angle(bend[2] - curr_angle))), **self.wg_spec)
                                self.add(last_path)
                            else:
                                self.add(gdspy.CellReference((self.wgt.bend_period_cell),
                                  origin=(bend[0]),
                            ...
                            if abs(angle_change) > remaining_angle - angular_period * (1 - self.wgt.duty_cycle):
                                ...
                                if angle_change > 0:
                                    ...
                                    (first_path.arc)(
                                     br, 
                                     curr_angle, 
                                     (curr_angle + angle_change), **self.wg_spec)
                                    self.add(first_path)
                                else:
                                    first_path = gdspy.Path((self.wgt.wg_width),
                                      initial_point=(tk.translate_point(bend[0], br, curr_angle + angle_change)))
                                    (first_path.arc)(
                                     br, 
            ...
            for i in range(len(self.wgt.waveguide_stack) - 1):
                ...
                if len(self.trace) == 2:
                    ...
                    (path2.segment)(
 tk.dist(self.trace[0], self.trace[1]), direction=tk.get_exact_angle(self.trace[0], self.trace[1]), **cur_spec)
                else:
                    path2 = gdspy.Path(cur_width, self.trace[0])
                    prev_dl = 0.0
                    for i in range(len(self.trace) - 2):
                        ...
                        if dl + prev_dl > tk.dist(self.trace[i], self.trace[(i + 1)]) + 1e-06:
                            raise ValueError('Warning! The waypoints ' + str(self.trace[i]) + ' and ' + str(self.trace[(i + 1)]) + ' are too close to accommodate  the necessary bend-radius of ' + str(br) + ', the points were closer than ' + str(dl + prev_dl))
                        (path2.segment)(
 tk.dist(self.trace[i], self.trace[(i + 1)]) - dl - prev_dl, direction=start_angle, **cur_spec)
                        turnby = tk.normalize_angle(next_angle - start_angle)
                        (path2.turn)(
 br,
 turnby, number_of_points=self.wgt.get_num_points_wg(turnby), **cur_spec)
                        prev_dl = dl

                    (path2.segment)(
 tk.dist(self.trace[(-2)], self.trace[(-1)]) - prev_dl, direction=next_angle, **cur_spec)
                self.add(path2)
            ...
            if len(self.trace) == 2:
                if self.wgt.wg_type == 'strip':
                    ...
                    (path.segment)(
 tk.dist(self.trace[0], self.trace[1]), direction=tk.get_exact_angle(self.trace[0], self.trace[1]), **self.wg_spec)
                else:
                    if self.wgt.wg_type == 'slot':
 ...
 tk.dist(self.trace[0], self.trace[1]), direction=tk.get_exact_angle(self.trace[0], self.trace[1]), **self.wg_spec)
                    ...
                    for c in range(len(self.wgt.waveguide_stack) - 1):
                        ...
                        cur_spec = {'layer':self.wgt.waveguide_stack[(c + 1)][1][0], 
                ...
                if self.wgt.wg_type == 'strip':
                    ...
                    for i in range(len(self.trace) - 2):
                        ...
                        if dl + prev_dl > tk.dist(self.trace[i], self.trace[(i + 1)]) + 1e-06:
                            ...
                            (path.segment)(tk.dist(self.trace[i], self.trace[(i + 1)]) - dl - prev_dl, direction=start_angle, **self.wg_spec)
                            for c in range(len(self.wgt.waveguide_stack) - 1):
                                cur_spec = {'layer':self.wgt.waveguide_stack[(c + 1)][1][0], 
                                 'datatype':self.wgt.waveguide_stack[(c + 1)][1][1]}
 ...
 tk.dist(self.trace[i], self.trace[(i + 1)]) - dl - prev_dl, direction=start_angle, **cur_spec)
                            ...
                            if self.wgt.euler == False:
                                (path.turn)(br,
 turnby, number_of_points=self.wgt.get_num_points_wg(turnby), **self.wg_spec)
                                for c in range(len(self.wgt.waveguide_stack) - 1):
                                    cur_spec = {'layer':self.wgt.waveguide_stack[(c + 1)][1][0], 
                                     'datatype':self.wgt.waveguide_stack[(c + 1)][1][1]}
 ...
 turnby, number_of_points=self.wgt.get_num_points_wg(turnby), **cur_spec)
                    ...
                    (path.segment)(
 tk.dist(self.trace[(-2)], self.trace[(-1)]) - prev_dl, direction=next_angle, **self.wg_spec)
                    for c in range(len(self.wgt.waveguide_stack) - 1):
                        cur_spec = {'layer':self.wgt.waveguide_stack[(c + 1)][1][0], 
                         'datatype':self.wgt.waveguide_stack[(c + 1)][1][1]}
                        (clad_path_list[c].segment)(
 tk.dist(self.trace[(-2)], self.trace[(-1)]) - prev_dl, direction=next_angle, **cur_spec)
    ...
    def __build_ports(self):
        self.portlist['input'] = {'port':(
          self.trace[0][0], self.trace[0][1]), 
...
if __name__ == '__main__':
    ...
    top = gdspy.Cell('top')
    wgt2 = WaveguideTemplate(wg_type='slot',
      wg_width=1.0,
      bend_radius=25,
      slot=0.3,
      resist='+',
      fab='ETCH')
    wgt3 = WaveguideTemplate(wg_type='swg',
      wg_width=1.0,
      bend_radius=25,
      duty_cycle=0.5,
      period=1.0,
    ...
    wg_stack = [
     [
      0.5, (1, 0)], [2.0, (2, 0)], [10, (4, 0)]]
    wgt1 = WaveguideTemplate(wg_type='slot',
      wg_width=1.0,
      bend_radius=25,
      slot=0.3,
    ...
    wg2 = Waveguide([
     (
      0, -space), (140.0, -space), (160.0, 50.0 - space), (300.0, 50.0 - space)], wgt2)
    tk.add(top, wg2)
    wg3 = Waveguide([
     (
      0, -2 * space),
