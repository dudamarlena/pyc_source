...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\components\swgcontradc.py
...
class SWGContraDirectionalCoupler(tk.Component):
    ...
    def __init__(self, wgt, length, gap, period, dc, taper_length, w_phc_bot, top_angle=np.pi / 6.0, width_top=None, width_bot=None, extra_swg_length=0.0, input_bot=False, apodization_top=False, apodization_far_dist=1.0, apodization_curv=None, fins=False, fin_size=(0.2, 0.05), contradc_wgt=None, port=(0, 0), direction='EAST'):
        ...
        self._SWGContraDirectionalCoupler__build_cell()
        self._SWGContraDirectionalCoupler__build_ports()
 ...
 -self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), 
                 final_width=self.width_top, **self.wg_spec)
                (wg_top.turn)(
 self.wgt.bend_radius,
 self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), **self.wg_spec)
                if self.apodization_top:
 ...
 gaussian_top, number_of_evaluations=600, **self.wg_spec)
                ...
                (wg_top.turn)(self.wgt.bend_radius,
 self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), **self.wg_spec)
                (wg_top.turn)(
 self.wgt.bend_radius,
 -self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), 
                ...
                (wg_top_clad.turn)(
 self.wgt.bend_radius,
 -self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), **self.clad_spec)
                (wg_top_clad.turn)(
 self.wgt.bend_radius,
 self.top_angle, number_of_points=self.wgt.get_num_points_wg(self.top_angle), 
 ...
 self.taper_length, final_width=self.width_bot, **self.wg_spec)
                    (wg_bot.segment)(
 angle_x_dist - self.taper_length - self.extra_swg_length, final_width=self.wgt.wg_width, **self.wg_spec)
                else:
                    (wg_bot.segment)(angle_x_dist - self.taper_length - self.extra_swg_length, final_width=self.width_bot, **self.wg_spec)
                    ...
                    wg_bot2.direction = '-x'
                    (wg_bot2.segment)(
 angle_x_dist - self.taper_length - self.extra_swg_length, final_width=self.width_bot, **self.wg_spec)
            ...
            (wg_bot_clad.segment)(
 angle_x_dist, final_width=self.width_bot + 2 * self.wgt.clad_width, **self.clad_spec)
            (wg_bot_clad.segment)((self.length), **self.clad_spec)
            (wg_bot_clad.segment)(
 angle_x_dist, final_width=self.wgt.wg_width + 2 * self.wgt.clad_width, **self.clad_spec)
            ...
            for i in range(int(num_blocks)):
                ...
                if abs(self.w_phc_bot - self.width_bot) > 1e-06:
                    block_list.append((gdspy.Rectangle)(
                     (
                      x, y0 - self.gap / 2.0), 
                     (
                      x + blockx, y0 - self.gap / 2.0 - self.width_bot), **self.wg_spec))
                ...
                for i in range(int(num_fins)):
                    ...
                    block_list.append((gdspy.Rectangle)(
                     (
                      x0, y - disty), 
                     (
                      x0 + self.fin_size[0], y - disty + self.fin_size[1]), **self.fin_spec))
                    block_list.append((gdspy.Rectangle)(
                     (
                      x0 + distx - self.fin_size[0], y), 
                     (
                      x0 + distx, y + self.fin_size[1]), **self.fin_spec))
                    block_list.append((gdspy.Rectangle)(
                     (
                      x0 + distx - self.fin_size[0], y - disty), 
                     (
                      x0 + distx, y - disty + self.fin_size[1]), **self.fin_spec))
    ...
    def __build_ports(self):
        if self.parity == 1:
            ...
            if self.parity == -1:
                ...
                self.portlist['input_bot'] = {'port':self.port_input_straight, 
                 'direction':'WEST'}
