...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\components\ebend.py
# Compiled at: 2019-10-06 18:48:06
# Size of source mod 2**32: 22152 bytes
...
class EBend(tk.Component):
    ...
    def __init__(self, wgt, turnby, start_width=None, end_width=None, port=(0, 0), direction='EAST', vertex=None):
        ...
        if abs(self.turnby) <= np.pi / 2.0:
            ...
            self.scale_factor = self.wgt.bend_radius / self._EBend__get_radius_of_curvature()
            self.output_port = (
             self.scale_factor * output_x_norm,
             self.sign * self.scale_factor * output_y_norm)
            ...
            self.t = 1.0 / np.sqrt(2.0)
            dy, dx = fresnel(self.t)
            self.scale_factor = self.wgt.bend_radius / self._EBend__get_radius_of_curvature()
            self.circle_angle = abs(self.turnby) - np.pi / 2.0
            self.circle_center = (
             dx * self.scale_factor - self.wgt.bend_radius * np.cos(np.pi / 4),
            ...
            self.output_port = (
             self.dist_to_vertex - self.dist_to_vertex * np.cos(np.pi - abs(self.turnby)),
        ...
        if vertex == None:
            ...
            self.port = (
             vertex[0] - self.dist_to_vertex * np.cos(vangle),
             vertex[1] - self.dist_to_vertex * np.sin(vangle))
        self._EBend__build_cell()
        self._EBend__build_ports()
    ...
    def __get_radius_of_curvature(self):
        ...
        """
        t = self.t
        xp = np.cos(np.pi * t ** 2 / 2.0)
        yp = np.sin(np.pi * t ** 2 / 2.0)
        xpp = -np.pi * t * np.sin(np.pi * t ** 2 / 2.0)
        ypp = np.pi * t * np.cos(np.pi * t ** 2 / 2.0)
        return abs((xp ** 2 + yp ** 2) ** 1.5 / (xp * ypp - yp * xpp))
    ...
    def __euler_function(self, t):
        if not t > 1.0:
            ...
            if t < 0.7:
                t0 = (t - 0.3) / 0.4
                return (
                 self.circle_center[0] + self.wgt.bend_radius * np.cos(-self.sign * np.pi / 4 + self.sign * self.circle_angle * t0),
    ...
    def __build_cell(self):
        if self.wgt.wg_type == 'strip':
            ...
            if self.wgt.wg_type == 'slot':
                wg = gdspy.Path((self.wgt.rail),
                  (0, 0), number_of_paths=2, distance=(self.wgt.rail_dist))
        (wg.parametric)(
 self._EBend__euler_function, final_width=self.end_width, 
        ...
        for i in range(len(self.wgt.waveguide_stack) - 1):
            ...
            (clad.parametric)(
 self._EBend__euler_function, tolerance=self.wgt.grid / 2.0, 
...
class EulerSBend(tk.Component):
    __doc__ = " Euler shaped S-Bend Cell class.  Creates an S-shaped Euler waveguide bend that can be used in waveguide routing (in place of the sinusoidal S-Bend).  The number of points is computed based on the waveguide template grid resolution to automatically minimize grid errors.\n\n        Args:\n           * **wgt** (WaveguideTemplate):  WaveguideTemplate object.  Bend radius is extracted from this object.\n           * **height** (float): Height of the Euler S-Bend\n           * **length** (float): Length of the Euler S-Bend\n\n        Keyword Args:\n           * **start_width** (float): If a value is provided, overrides the initial waveguide width (otherwise the width is taken from the WaveguideTemplate object).  Currently only works for strip waveguides.\n           * **end_width** (float): If a value is provided, overrides the final waveguide width (otherwise the width is taken from the WaveguideTemplate object).  Currently only works for strip waveguides.\n           * **port** (tuple): Cartesian coordinate of the input port.  Defaults to (0,0).\n           * **direction** (string): Direction that the component will point *towards*, can be of type `'NORTH'`, `'WEST'`, `'SOUTH'`, `'EAST'`, OR an angle (float, in radians)\n           \n        Members:\n           * **portlist** (dict): Dictionary with the relevant port information\n\n        Portlist format:\n           * portlist['input'] = {'port': (x1,y1), 'direction': 'dir1'}\n           * portlist['output'] = {'port': (x2, y2), 'direction': 'dir2'}\n\n        Where in the above (x1,y1) is the same as the 'port' input, (x2, y2) is the end of the taper, and 'dir1', 'dir2' are of type `'NORTH'`, `'WEST'`, `'SOUTH'`, `'EAST'`, *or* an angle in *radians*.\n        'Direction' points *towards* the waveguide that will connect to it.\n\n    "
    ...
    def __init__(self, wgt, length, height, start_width=None, end_width=None, port=(0, 0), direction='EAST'):
        ...
        if length < 0:
            ...
            self.ls = (height ** 2 + length ** 2) / (4 * length)
            if abs(height) == length:
        ...
        self._EulerSBend__build_cell()
        self._EulerSBend__build_ports()
        self._auto_transform_()
    ...
    def get_radius_of_curvature(self):
        ...
        xpp = -np.pi * t * np.sin(np.pi * t ** 2 / 2.0)
        ypp = np.pi * t * np.cos(np.pi * t ** 2 / 2.0)
        return self.scale_factor * abs((xp ** 2 + yp ** 2) ** 1.5 / (xp * ypp - yp * xpp))
    ...
    def __euler_s_function(self, t):
        ...
        if t < 0.5:
            ...
            return (
             self.output_port[0] / 2 - x * self.scale_factor,
             self.output_port[1] / 2 + self.sign * y * self.scale_factor)
        if t < 0.75:
        ...
        if self.wgt.wg_type == 'strip':
            ...
            if self.wgt.wg_type == 'slot':
                wg = gdspy.Path((self.wgt.rail),
                  (0, 0), number_of_paths=2, distance=(self.wgt.rail_dist))
        (wg.parametric)(
 self._EulerSBend__euler_s_function, final_width=self.end_width, 
         tolerance=self.wgt.grid / 2.0, 
         max_points=199, **self.wg_spec)
        self.add(wg)
        for i in range(len(self.wgt.waveguide_stack) - 1):
            ...
            clad = gdspy.Path(cur_width, (0, 0))
            (clad.parametric)(
 self._EulerSBend__euler_s_function, tolerance=self.wgt.grid / 2.0, 
             max_points=199, **cur_spec)
            self.add(clad)
