...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\picwriter\components\fullcoupler.py
# Compiled at: 2019-10-06 18:48:06
# Size of source mod 2**32: 11615 bytes
...
class FullCoupler(tk.Component):
    ...
    def __init__(self, wgt, length, gap, dw, angle=np.pi / 6.0, parity=1, port=(0, 0), direction='EAST'):
        ...
        self._FullCoupler__build_cell()
        self._FullCoupler__build_ports()
 ...
 -p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), 
         final_width=self.wgt.wg_width + self.dw, **self.wg_spec)
        (wg_top.turn)(
 self.wgt.bend_radius,
 p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), **self.wg_spec)
        (wg_top.segment)(
 self.length, final_width=self.wgt.wg_width - self.dw, **self.wg_spec)
        (wg_top.turn)(
 self.wgt.bend_radius,
 p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), **self.wg_spec)
        (wg_top.turn)(
 ...
 -p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), 
        ...
        (wg_top_clad.turn)(
 self.wgt.bend_radius,
 -p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), **self.clad_spec)
        (wg_top_clad.turn)(
 self.wgt.bend_radius,
 p * self.angle, number_of_points=self.wgt.get_num_points_wg(self.angle), 
    ...
    def __build_ports(self):
        if self.parity == 1:
            ...
            if self.parity == -1:
                ...
                self.portlist['input_bot'] = {'port':self.portlist_input_straight, 
                 'direction':'WEST'}
