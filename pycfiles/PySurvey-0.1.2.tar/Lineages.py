...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/core/Lineages.py
# Compiled at: 2013-04-05 10:21:05
"""
...
class Lineage(object):
    ...
    def __init__(self, id=None, lin_str=None, format='QIIME'):
        ...
        self.lin = dict.fromkeys(_levels, _unassigned_str)
        ...
        if lin_str:
            if format.upper() == 'QIIME':
                ...
                fields = lin_str.strip().split(';')[1:-1]
                n_fields = len(fields)
...
class Lineages(dict):
    ...
    def get_ids(self, level, assignment, best=True, complement=False):
        ...
        if level not in _levels:
            raise ValueError("Level '%s' is not one of the allowed taxonomic leveles: 'k', 'p', 'c', 'o', 'f', 'g' , 's'." % level)
        if complement:
            ids = filter(lambda i: self[i].get_assignment(level=level, best=best) != assignment, self.iterkeys())
        else:
            ids = filter(lambda i: self[i].get_assignment(level=level, best=best) == assignment, self.iterkeys())
        return ids
