...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/analysis/diversity.py
# Compiled at: 2013-04-02 18:46:47
"""
...
def chao1(counts):
    ...
    """
    f0 = len(where(counts == 0)[0])
    f1 = len(where(counts == 1)[0])
    f2 = len(where(counts == 2)[0])
    if not f2:
        f2 += 1
    S_obs = len(counts) - f0
...
def richness(counts, method='ACE'):
    ...
    if method_l == 'ml':
        ...
        return (
         S_obs, 0)
    if method_l in ('chao1', 'ace'):
        temp = vegan.estimateR(counts)
        (S_obs, S_chao1, se_chao1, S_ACE, se_ACE) = array(temp)
        if method_l == 'ace':
            return (S_ACE, se_ACE)
        if method_l == 'chao1':
            return (S_chao1, se_chao1)
    else:
        raise ValueError("Method '%s' is not supported for function Richness." % method)
...
def sample_diversity(counts_mat, indices='Hill_1', **kwargs):
    ...
    n_inds = len(indices)
    methods = kwargs.get('methods', ['ML'] * n_inds)
    ...
    for i in xrange(n_smpls):
        ...
        for j in xrange(n_inds):
            ...
            if ind == 'richness':
                ...
                raise ValueError('%s is not a supported divesity index.' % ind)
            D[(i, j)] = d
