...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/analysis/analysis_methods.py
# Compiled at: 2013-04-05 11:25:09
"""
...
def basis_corr(frame, algo='SparCC', **kwargs):
    ...
    comps = frame.columns
    (cor_med, cov_med) = basecor.main(frame, algo=algo, **kwargs)
...
def correlation(frame, method='pearson', axis=0):
    ...
    if method == 'spearman':
        ...
        if method == 'pearson':
            corr_fun = stats.pearsonr
        elif method == 'kendall':
            corr_fun = stats.kendalltau
        if axis == 0:
            data = frame.T
        elif axis == 1:
            data = frame
...
def discriminating_components(frame1, frame2, method='U'):
    ...
    if method == 'u':
        ...
        for (id, vals1) in frame1.iteritems():
            p1 = frame_pres[id]
            p2 = other_pres[id]
            table = np.array([[p1, n1 - p1], [p2, n2 - p2]])
...
def permute_w_replacement(frame, axis=0):
    ...
    s = frame.shape[axis]
    fun = lambda x: x[randint(0, s, (1, s))][0]
