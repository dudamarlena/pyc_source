...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/core/compositional_methods.py
...
def alr(frame, ref=None, axis=0):
    ...
    if ref is None:
        ...
        label = ref
    if axis == 0:
        norm = 1.0 * frame[label]
    elif axis == 1:
        norm = 1.0 * frame.xs(label)
...
def clr(frame, centrality='mean', axis=0):
    ...
    """
    temp = log(frame)
    if centrality is 'mean':
        f = lambda x: x - x.mean()
    elif centrality is 'median':
...
def replace_zeros(frame, type='multiplicative', e=0.5):
    ...
    for (i, row) in new.iterrows():
        inds_z = (row == 0).nonzero()[0]
        inds = (row > 0).nonzero()[0]
        delta = e * np.min(row[inds])
        row[inds_z] = delta
        if type is 'simple':
            row /= row.sum()
