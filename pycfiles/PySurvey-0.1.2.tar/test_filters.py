...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/tests/test_filters.py
# Compiled at: 2013-04-05 18:08:39
"""
Created on Dec 20, 2011

@author: jonathanfriedman
"""
import unittest, operator, numpy as np
from pysurvey.util import filters
from pandas import Series
from pandas.util.testing import assert_series_equal
expectedComperators = {'==': operator.eq, '!=': operator.ne, 
   '>=': operator.ge, 
   '<=': operator.le, 
   '>': operator.gt, 
   '<': operator.lt, 
   'in': operator.contains}
expectedActors = {'sum': np.sum, 'avg': np.mean, 
   'med': np.median, 
   'var': np.var, 
   'std': np.std, 
   'min': np.min, 
...
class TestLineages(unittest.TestCase):
    ...
    def test_parse_comperator(self):
        for (k, ncs) in filters.namedComperators.iteritems():
            for nc in ncs:
                self.assertEqual(filters.parse_comperator(nc), expectedComperators[k], 'Named Comperator %s failed to parse correctly' % nc)
    ...
    def test_parse_actor(self):
        for (k, nas) in filters.namedActors.iteritems():
            for na in nas:
                self.assertEqual(filters.parse_actor(na), expectedActors[k], 'Named Actor %s failed to parse correctly' % na)
