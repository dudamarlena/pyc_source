...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/plotting/heatmaps.py
# Compiled at: 2013-04-05 15:40:30
"""
...
"""
import scipy, pylab as pl, scipy.cluster.hierarchy as sch, pysurvey.util.distances as distances, numpy as np
...
def clustered_heatmap(matrix, dist_mat=False, **kwargs):
    ...
    if rsort:
        ...
        rdendro_width = 0
        rdendro_gap = 0
    data_width = 1 - 2 * edge_gap - rdendro_width - rdendro_gap - rstrip_width - rstrip_gap - rlabel_width - cbar_gap - cbar_width - cblabel_width
    data_height = 1 - 2 * edge_gap - cdendro_width - cdendro_gap - cstrip_width - cstrip_gap - clabel_width
    drow_left = edge_gap
    ...
    if rsort:
        ...
        Zrow = make_dendrogram(matrix, rdendro_ax, dist_mat, rmetric, **rdendro_kwarg)
        rindex = Zrow['leaves']
    ...
    dcol_left = edge_gap + rdendro_width + rdendro_gap + rstrip_width + rstrip_gap + rlabel_width
    dcol_bottom = 1 - edge_gap - cdendro_width
    dcol_height = cdendro_width
    dcol_width = data_width
    if csort:
        cdendro_pos = [
         dcol_left, dcol_bottom, dcol_width, dcol_height]
        ...
        Zcol = make_dendrogram(matrix.transpose(), cdendro_ax, dist_mat, cmetric, **cdendro_kwarg)
    ...
    matrix_ax = fig.add_axes(mat_pos, sharex=cdendro_ax, sharey=rdendro_ax)
    ...
    if cstrip is not None:
        ...
        cstrip_ax = fig.add_axes(cstrip_pos, frame_on=False, sharex=matrix_ax)
    ...
    if rstrip is not None:
        ...
        rstrip_ax = fig.add_axes(rstrip_pos, frame_on=False, sharey=matrix_ax)
    ...
    if plot_rlabels:
        ...
        plt.setp(matrix_ax.get_yticklabels(), visible=rvisible, fontstretch='ultra-condensed', fontsize=rfontsize)
    else:
        rvisible = False
    ...
    if plot_clabels:
        ...
        plt.setp(matrix_ax.get_xticklabels(), visible=cvisible, rotation='90', fontstretch='ultra-condensed', fontsize=cfontsize)
    else:
        cvisible = False
    ...
    if colorbar:
        ...
        plt.setp(cbar_ax.get_yticklabels(), fontstretch='ultra-condensed', fontsize=cbfontsize)
    ...
    if interactive:
        ...
        HeatmapAnnotator(arange(5, 10 * n, 10), arange(5, 10 * m, 10), rannotes=rannotes_sorted, cannotes=cannotes_sorted, axis=matrix_ax, **interactive_kwargs)
        if cstrip is not None:
            HeatmapAnnotator([25], arange(5, 10 * m, 10), cannotes=cstrip_sotred, axis=cstrip_ax, ytol=1, **cinteractive_kwargs)
        if rstrip is not None:
            HeatmapAnnotator(arange(5, 10 * n, 10), [37.5], rannotes=rstrip_sotred, axis=rstrip_ax, xtol=1, **rinteractive_kwargs)
    return (
     mat, rlabels_sorted, clabels_sorted, axes)
