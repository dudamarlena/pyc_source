...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/tests/test_analysis_methods.py
...
"""
import unittest, numpy as np, pysurvey.analysis.analysis_methods as am
...
class TestFrameMethods(unittest.TestCase):
    ...
    def test_sample_diversity(self):
        ...
        expected = DF(expected_dive, columns=[ ind + '.ML' for ind in inds ], index=[
         self.frame.index])
    ...
    def test_dist_mat(self):
        ...
        expected = DF(D, index=self.frame.index, columns=self.frame.index)
        result = am.dist_mat(self.frame, metric)
        assert_frame_equal(expected, result)
        D = sch.distance.pdist(self.frame.T, metric)
        D = sch.distance.squareform(D)
        expected = DF(D, index=self.frame.columns, columns=self.frame.columns)
