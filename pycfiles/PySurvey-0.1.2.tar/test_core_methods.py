...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/tests/test_core_methods.py
...
"""
import unittest, numpy as np, pysurvey.core.core_methods as cm
...
_lins_d = {'c1': 'k__Bacteria;p__Nitrospirae;c__Nitrospira;o__Nitrospirales;f__Nitrospiraceae;g__Nitrospira;s__', 'c2': 'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Rikenellaceae;g__;s__', 
...
class TestFrameMethods(unittest.TestCase):
    ...
    def test_set_labels(self):
        ...
        self.assertEqual(cm.get_labels(temp, 0), rlabels_list)
        self.assertRaises(Exception, cm.set_labels, (temp, [''], 0))
    ...
    def test_keep(self):
        ...
        result = cm.keep(self.frame, 1, criterion='present', which='last')
        assert_frame_equal(result, expected)
        f = lambda x: len(np.nonzero(x % 4 == 0)[0])
        expected = self.frame.drop(['r1'])
        result = cm.keep(self.frame, 2, criterion=f)
        assert_frame_equal(result, expected)
    ...
    def test_to_fractions(self):
        ...
        exp_vals = [
         [
          3.0 / 12, 5.0 / 12, 4.0 / 12],
         [
          8.0 / 21, 7.0 / 21, 6.0 / 21]]
        expected = DF(exp_vals, columns=_clabels2, index=_rlabels2)
        result = cm.to_fractions(self.frame2, 'pseudo', p_counts=2)
        assert_frame_equal(result, expected)
        exp_vals = [
         [
          2.0 / 8, 5.0 / 10, 3.0 / 8],
        ...
        p_counts = [[1, 2, 1],
         [
          0, 0, 1]]
        expected = DF(exp_vals, columns=_clabels2, index=_rlabels2)
        result = cm.to_fractions(self.frame2, 'pseudo', p_counts=p_counts, axis=1)
        assert_frame_equal(result, expected)
