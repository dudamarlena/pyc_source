...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/io/io.py
# Compiled at: 2013-04-04 16:11:01
"""
...
from pysurvey.core.Lineages import Lineages

def read_txt(file, T=True, lin=None, lin_label='lineage', format='QIIME', verbose=True, **kwargs):
    """
    ...
    if lin is None:
        lin = False
        lin_labels = ('lin', 'lins', 'lineage', 'lineages', 'taxon', 'taxa', 'rdp')
    ...
    scol = tuple([ncol] + list(temp.columns[:3]))
    srow = tuple([nrow] + list(temp.index[:3]))
    s += ['First %d column labels are: %s, %s, %s' % scol,
     'First %d row labels are: %s, %s, %s' % srow]
    table = temp
    if lin:
        if verbose:
            print ('\n').join(s), '\n'
