...
# [GCC 8.4.0]
# Embedded file name: /Users/jonathanfriedman/Dropbox/python_dev_library/PySurvey/pysurvey/core/core_methods.py
# Compiled at: 2013-04-05 19:10:19
"""
...
def filter_by_vals(frame, criteria, axis='cols', verbose=True, how='all', nan_val=None, norm=False):
    """
    ...
    if verbose:
        ...
        s = [
         'Dropped %d %s' % (len(drop), axis_s[axis]),
...
def keep(frame, n, criterion='sum', axis=0, which='first', sort=True):
    ...
    if not sort:
        filtered = filtered.reindex_like(data).dropna(how='all', axis=axis)
    if axis == 0:
        filtered = filtered.T
    return filtered
...
def normalize(frame, axis=0):
    ...
    """
    axis = _get_axis(axis)
    return frame.apply(lambda x: 1.0 * x / x.sum(), axis=1 - axis)
...
def group_taxa(frame, lins, level='p', best=True):
    ...
    """
    old = frame
    new = old.filter(items=[])
    taxa = set(lins.get_assignments(level, best=best))
    for t in taxa:
        otus = lins.get_ids(level, t, best=best)
        temp = old.filter(items=otus)
