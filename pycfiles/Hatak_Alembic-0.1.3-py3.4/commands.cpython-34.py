...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/haplugin/alembic/commands.py
# Compiled at: 2015-04-11 04:42:14
...
class AlembicCommand(Command):
    ...
    def generate_config(self):
        config = ConfigParser()
        config['alembic'] = {'script_location': self.paths['alembic']['versions'], 
         'sqlalchemy.url': self.settings['db']['url']}
        config['loggers'] = {'keys': 'root,sqlalchemy,alembic'}
        config['handlers'] = {'keys': 'console'}
        config['formatters'] = {'keys': 'generic, hatak'}
        config['logger_root'] = {'level': 'WARN', 
         'handlers': 'console', 
         'qualname': ''}
        config['logger_sqlalchemy'] = {'level': 'WARN', 
         'handlers': '', 
         'qualname': 'sqlalchemy.engine'}
        config['logger_alembic'] = {'level': 'INFO', 
         'handlers': '', 
         'qualname': 'alembic'}
        config['handler_console'] = {'class': 'StreamHandler', 
         ...
         'level': 'NOTSET', 
         'formatter': 'hatak'}
        config['formatter_hatak'] = {'format': '[Alembic] %(message)s'}
...
class InitDatabase(AlembicCommand):
    ...
    def run_alembic(self):
        ...
        scan.scan(__import__(self.app.module), ignore=[
