...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/solstice/files/shader.py
# Compiled at: 2020-03-08 13:23:53
# Size of source mod 2**32: 9886 bytes
"""
...
class SolsticeShaderFile(file.ArtellaFile, object):
    ...
    def __init__(self, project, file_name, file_path=None, file_extension=None):
        ...
        super(SolsticeShaderFile, self).__init__(project=project,
          file_name=file_name,
          file_path=file_path,
    ...
    def _export_file(self, file_path, *args, **kwargs):
        ...
        try:
            try:
                exported_shader = shader.ShadingNetwork.write_network(shader_extension=(self.extensions[0]),
                  shaders_path=(os.path.dirname(file_path)),
                  shaders=[
                 self.name],
                  icon_path=temp_file)
...
class SolsticeShaderMappingAssetFile(assetfile.ArtellaAssetFile, object):
    ...
    def _export_file(self, file_path, *args, **kwargs):
        if not tp.is_maya():
            ...
            shaders_to_export = artellapipe.ShadersMgr().get_asset_shaders_to_export(asset=(self._asset),
              return_only_shaders=False)
            locked_file = False
            if os.path.isfile(file_path):
                res = qtutils.show_question(None, 'Exporting Shaders Mapping File', 'Shaders Mapping File "{}" already exists. Do you want to overwrite it?'.format(file_path))
