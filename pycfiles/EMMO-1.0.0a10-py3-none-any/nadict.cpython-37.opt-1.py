...
# [GCC 8.4.0]
# Embedded file name: /emmo/nadict.py
# Compiled at: 2020-04-10 04:40:37
# Size of source mod 2**32: 6421 bytes
"""
...
class NADict:
    ...
    def __setitem__(self, key, value):
        if key in ('clear', 'copy', 'fromkeys', 'get', 'items', 'keys', 'pop', 'popitem',
                   'setdefault', 'update', 'values'):
            raise ValueError('invalid key "%s": must not override supported dict method names' % key)
        else:
            if '.' in key:
                k1, k2 = key.split('.', 1)
