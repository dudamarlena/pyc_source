...
# [GCC 8.4.0]
# Embedded file name: /emmo/utils.py
# Compiled at: 2020-04-26 17:19:22
# Size of source mod 2**32: 7227 bytes
...
def asstring(expr, link='{name}', n=0, exclude_object=False):
    ...
    def fmt(e):
        ...
        if hasattr(e, 'label'):
            if e.label:
                name = e.label.first()
                url = name if re.match('^[a-z]+://', name) else '#' + name
                return link.format(name=name, url=url, lowerurl=(url.lower()))
        if re.match('^[a-z]+://', str(e)):
    ...
    if isinstance(expr, owlready2.Restriction):
        ...
        if isinstance(expr.property, (
         ...
         owlready2.DataPropertyClass)):
            ...
            if isinstance(expr.property, owlready2.Inverse):
                s = 'Inverse(%s)' % asstring(expr.property.property, link, n + 1)
            else:
                print('*** WARNING: unknown restriction property: %r' % expr.property)
                s = fmt(expr.property)
        if not rlabel:
            pass
            ...
            if expr.type in (owlready2.SOME, owlready2.ONLY,
    ...
    if isinstance(expr, owlready2.And):
        s = '%s' if n == 0 else '(%s)'
        return s % ' and '.join([asstring(c, link, n + 1) for c in expr.Classes])
    ...
    if isinstance(expr, owlready2.PropertyClass):
        return fmt(expr)
    if isinstance(expr, owlready2.Thing):
        return fmt(expr)
...
def camelsplit(s):
    ...
    for next in s[1:]:
        if not prev_isspace:
            if not (c.isupper() and next.islower()):
                if prev_lower:
                    if c.isupper():
                        result.append(' ')
                result.append(c)
...
def read_catalog(path, catalog_file='catalog-v001.xml', recursive=False, return_paths=False):
    ...
    def load_catalog(filepath):
        ...
        if gettag(root) != 'catalog':
            raise ValueError('expected root tag of catalog file %r to be "catalog"', filepath)
        for child in root:
            if gettag(child) == 'uri':
...
def infer_version(iri, version_iri):
    ...
    if '/' in version:
        raise ValueError('version IRI %r is not consistent with base IRI %r' % (
         version_iri, iri))
    return version