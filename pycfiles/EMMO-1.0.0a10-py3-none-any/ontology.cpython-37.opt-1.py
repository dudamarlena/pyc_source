...
# [GCC 8.4.0]
# Embedded file name: /emmo/ontology.py
...
"""
import sys, os, itertools, inspect, warnings, uuid
...
class NoSuchLabelError(LookupError, AttributeError):
    __doc__ = 'Error raised when a label cannot be found.'


categories = ('annotation_properties', 'data_properties', 'object_properties', 'classes',
              'individuals')
...
def isinteractive():
    ...
    false otherwise."""
    return bool(hasattr(__builtins__, '__IPYTHON__') or sys.flags.interactive or hasattr(sys, 'ps1'))


class World(owlready2.World):
    __doc__ = 'A subclass of owlready2.World.'
    ...
    def get_ontology(self, base_iri='emmo-inferred'):
        ...
        if base_iri == 'emmo-inferred':
            base_iri = 'https://emmo-repo.github.io/unstable/emmo-inferred.owl'
        else:
            if base_iri == 'emmo':
                ...
                if base_iri + '#' in self.ontologies:
                    ...
                    if base_iri + '/' in self.ontologies:
                        ...
                        if os.path.exists(base_iri):
                            ...
                            if os.path.exists(base_iri + '.owl'):
...
class Ontology(owlready2.Ontology, OntoGraph):
    __doc__ = 'A generic class extending owlready2.Ontology.\n    '
    ...
    dir_labels = property(fget=(lambda self: getattr(self, '_dir_labels', isinteractive())),
      fset=(lambda self, v: setattr(self, '_dir_labels', bool(v))),
      doc='Whether to include entity labels in dir() listing.')
    dir_names = property(fget=(lambda self: getattr(self, '_dir_names', False)),
      fset=(lambda self, v: setattr(self, '_dir_names', bool(v))),
      doc='Whether to entity names in dir() listing.')
    dir_imported = property(fget=(lambda self: getattr(self, '_dir_imported', isinteractive())),
    ...
    def __dir__(self):
        s = set(super().__dir__())
        if self.dir_labels:
            s.update((e.label.first() for e in self.get_entities(imported=(self.dir_imported)) if hasattr(e, 'label')))
        if self.dir_names:
            s.update((e.name for e in self.get_entities(imported=(self.dir_imported)) if hasattr(e, 'name')))
    ...
    def load(self, only_local=False, filename=None, reload=None, reload_if_newer=False, catalog_file='catalog-v001.xml', **kwargs):
        """Load the ontology.
        ...
        """
        dirpath = os.path.normpath(os.path.dirname(filename or self.base_iri.rstrip('/#')))
        if only_local:
            if os.path.exists(os.path.join(dirpath, catalog_file)):
                iris, dirs = read_catalog(dirpath,
                  recursive=True, return_paths=True, catalog_file=catalog_file)
                for d in sorted(dirs, reverse=True):
                    if d not in owlready2.onto_path:
                        owlready2.onto_path.append(d)

        (super().load)(only_local=only_local, fileobj=filename, reload=reload, reload_if_newer=reload_if_newer, **kwargs)
        return self
    ...
    def get_entities(self, imported=True, classes=True, individuals=True, object_properties=True, data_properties=True, annotation_properties=True):
        ...
        categories = set([
         ...
         'annotation_properties' if annotation_properties else None]).difference([
         None])
        for e in itertools.chain.from_iterable((getattr(self, c)() for c in categories)):
            yield e

        if imported:
            for onto in self.get_imported_ontologies(recursive=True):
                for e in itertools.chain.from_iterable((getattr(onto, c)() for c in categories)):
                    yield e

    def get_root_classes(self):
        """Returns a list or root classes."""
        return [cls for cls in self.classes() if not cls.ancestors().difference(set([cls, owlready2.Thing]))]
    ...
    def get_by_label(self, label):
        ...
        d = {'Nothing': owlready2.Nothing}
        if label in d:
            return d[label]
        lst = [cls for cls in itertools.chain.from_iterable((getattr(self, category)() for category in categories)) if hasattr(cls, '__name__') if cls.__name__ == label]
        if len(lst) == 1:
            return lst[0]
        if len(lst) > 1:
            raise NoSuchLabelError('There is more than one Python class with name %r' % label)
        else:
            if label is owlready2.Thing or label == 'Thing':
                return owlready2.Thing
        for onto in self.imported_ontologies:
            onto.__class__ = self.__class__
            try:
        ...
        for onto in [self] + self.imported_ontologies:
            ...
            if len(lst) == 1:
                return lst[0]
                if len(lst) > 1:
                    raise NoSuchLabelError('There is more than one class with name %r' % label)
    ...
    def get_by_label_all(self, label):
        ...
        matching labels.
        """
        return [entity for entity in itertools.chain.from_iterable((getattr(self, c)() for c in categories)) if hasattr(entity, 'label') if label in entity.label]

    def sync_reasoner(self, reasoner='HermiT', include_imported=False, **kwargs):
        ...
        if reasoner == 'Pellet':
            ...
            if reasoner == 'HermiT':
                ...
                raise ValueError('unknown reasoner %r.  Supported reasonersare "Pellet" and "HermiT".', reasoner)
        if include_imported:
            with self:
                sync(**kwargs)
        else:
    ...
    def sync_attributes(self, name_policy=None, name_prefix='', sync_imported=False):
        ...
        updated.
        """
        for cls in itertools.chain(self.classes(), self.object_properties(), self.data_properties(), self.annotation_properties()):
            if not cls.label:
                if hasattr(cls, '__name__'):
                    cls.label.append(cls.__name__)
            if cls.comment or hasattr(cls, '__doc__') and cls.__doc__:
                cls.comment.append(inspect.cleandoc(cls.__doc__))

        chain = itertools.chain(self.classes(), self.individuals(), self.object_properties(), self.data_properties(), self.annotation_properties())
        if name_policy == 'uuid':
            for obj in chain:
                obj.name = name_prefix + str(uuid.uuid4())

        else:
            if name_policy == 'sequential':
                for obj in chain:
                    n = 0
    ...
    def get_relations(self):
        ...
        warnings.warn('Ontology.get_relations() is deprecated.  Use onto.object_properties() instead.', DeprecationWarning)
    ...
    def get_annotations(self, entity):
        ...
        warnings.warn('Ontology.get_annotations(cls) is deprecated.  Use cls.get_annotations() instead.', DeprecationWarning)
        if isinstance(entity, str):
            entity = self.get_by_label(entity)
        d = {'comment': getattr(entity, 'comment', '')}
        for a in self.annotation_properties():
            d[a.label.first()] = [o.strip('"') for s, p, o in self.get_triples(entity.storid, a.storid, None)]

        return d

    def get_branch(self, root, leafs=(), include_leafs=True, strict_leafs=False, exclude=None, sort=False):
        ...
        leafs = set(((self.get_by_label(leaf) if isinstance(leaf, str) else leaf) for leaf in leafs))
        leafs.discard(root)
        if exclude:
            exclude = set(((self.get_by_label(e) if isinstance(e, str) else e) for e in exclude))
        ...
        if strict_leafs:
            ...
            for leaf in leafs:
                if leaf in descendants:
                    branch.difference_update(leaf.descendants(include_self=False))

        if exclude:
            branch.difference_update(exclude)
        if sort:
            branch = sorted(sorted(branch, key=(lambda x: asstring(x))), key=(lambda x: len(x.mro())))
        return branch
    ...
    def get_version(self, as_iri=False):
        ...
        """
        versionIRI_storid = self.world._abbreviate('http://www.w3.org/2002/07/owl#versionIRI')
    ...
    def set_version(self, version=None, version_iri=None):
        ...
        if not version_iri:
            if not version:
                raise TypeError('Either `version` or `version_iri` must be provided')
            version_iri = self.base_iri.rstrip('#/') + '/' + version
        self._add_obj_triple_spo(s=(self.storid),
    ...
    def _number_of_generations(self, descendant, ancestor, n):
        ...
        if descendant.name == ancestor.name:
            return n
        return min((self._number_of_generations(parent, ancestor, n + 1) for parent in descendant.get_parents() if ancestor in parent.ancestors()))
