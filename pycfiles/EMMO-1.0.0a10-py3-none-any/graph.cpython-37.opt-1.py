...
# [GCC 8.4.0]
# Embedded file name: /emmo/graph.py
# Compiled at: 2020-04-10 04:40:37
# Size of source mod 2**32: 30186 bytes
"""
A module for visualising ontologies using graphviz.
"""
import os, re, tempfile
import xml.etree.ElementTree as ET
import owlready2, graphviz
...
class OntoGraph:
    ...
    _default_style = {'graphtype':'Digraph', 
     ...
     'defined_class':{'style':'filled', 
      'fillcolor':'#ffc880'}, 
     'class_construct':{'style':'filled', 
      'fillcolor':'gray'}, 
     'individual':{'shape':'diamond', 
      'style':'filled', 
      'fillcolor':'#874b82', 
      'fontcolor':'white'}, 
     'object_property':{'shape':'box', 
      'style':'filled', 
      'fillcolor':'#0079ba', 
      'fontcolor':'white'}, 
     'data_property':{'shape':'box', 
      'style':'filled', 
      'fillcolor':'green'}, 
     'annotation_property':{'shape':'box', 
      'style':'filled', 
      'fillcolor':'orange'}, 
     'added_node':{'color': 'red'}, 
     'isA':{'arrowhead': 'empty'}, 
     'not':{'color':'gray', 
      'style':'dotted'}, 
     ...
     'default_relation':{'color':'olivedrab', 
      'constraint':'false'}, 
     'relations':{'disconnected':{'color':'red', 
       'style':'dotted',  'arrowhead':'odot'}, 
      'hasPart':{'color': 'blue'}, 
      'hasProperPart':{'color':'blue', 
       'style':'dashed'}, 
      'hasParticipant':{'color': 'red'}, 
      'hasProperParticipant':{'color':'red', 
       'style':'dashed'}, 
      'hasSpatialDirectPart':{'color': 'darkgreen'}, 
      'hasTemporalPart':{'color': 'magenta'}, 
      'hasTemporalDirectPart':{'color':'magenta', 
       'style':'dashed'}, 
      'hasReferenceUnit':{'color':'darkgreen', 
       'style':'dashed'}, 
      'hasSign':{'color': 'orange'}, 
      'hasConvention':{'color':'orange', 
       'style':'dashed'}, 
      'hasProperty':{'color':'orange', 
       'style':'dotted'}}, 
     'inverse':{'arrowhead': 'inv'}, 
     'default_dataprop':{'color':'green', 
      'constraint':'false'}, 
     'node':{},  'edge':{}}

    def __init__(self, ontology, root=None, leafs=None, entities=None, relations='isA', style=None, edgelabels=True, addnodes=False, addconstructs=False, parents=0, excluded_nodes=None, graph=None, **kwargs):
        ...
        if graph is None:
            ...
            if ontology != graph.ontology:
                ValueError('the same ontology must be used when extending a graph')
            self.dot = graph.dot.copy()
            self.nodes = graph.nodes.copy()
            self.edges = graph.edges.copy()
        self.ontology = ontology
        self.relations = set([relations] if isinstance(relations, str) else relations)
        self.style = style
        ...
        if root == ALL:
            self.add_entities(relations=relations,
            ...
            if root:
                self.add_branch(root,
                  leafs, relations=relations,
                  edgelabels=edgelabels,
                  addnodes=addnodes,
                  addconstructs=addconstructs)
                if parents:
                    self.add_parents(root,
                      levels=parents, relations=relations,
                      edgelabels=edgelabels,
                      addnodes=addnodes,
                      addconstructs=addconstructs)
            if entities:
                self.add_entities(entities=entities, relations=relations, edgelabels=edgelabels,
                  addnodes=addnodes,
                  addconstructs=addconstructs)

    def add_entities(self, entities=None, relations='isA', edgelabels=True, addnodes=False, addconstructs=False, nodeattrs=None, **attrs):
        ...
        (self.add_edges)(relations=relations, 
         edgelabels=edgelabels, addnodes=addnodes, 
         addconstructs=addconstructs, **attrs)

    def add_branch(self, root, leafs=None, include_leafs=True, strict_leafs=False, exclude=None, relations='isA', edgelabels=True, addnodes=False, addconstructs=False, **attrs):
        """Adds branch under `root` ending at any entiry included in the
        sequence `leafs`.  If `include_leafs` is true, leafs classes are
        also included."""
        if leafs is None:
            leafs = ()
        classes = self.ontology.get_branch(root=root,
          leafs=leafs,
        ...
        (self.add_entities)(entities=classes, 
         relations=relations, 
         edgelabels=edgelabels, addnodes=addnodes, 
         addconstructs=addconstructs, nodeattrs=nodeattrs, **attrs)

    def add_parents(self, name, levels=1, relations='isA', edgelabels=None, addnodes=False, addconstructs=False, **attrs):
        ...
        (self.add_entities)(entities=parents, 
         relations=relations, 
         edgelabels=edgelabels, addnodes=addnodes, 
         addconstructs=addconstructs, **attrs)
    ...
    def add_edge(self, subject, predicate, object, edgelabel=None, **attrs):
        ...
        predicate = predicate if isinstance(predicate, str) else getlabel(predicate)
        ...
        if subject in self.excluded_nodes or object in self.excluded_nodes:
            return
        if not (isinstance(subject, str) and isinstance(object, str)):
    ...
    def add_source_edges(self, source, relations=None, edgelabels=None, addnodes=None, addconstructs=None, **attrs):
        ...
        addconstructs = self.addconstructs if addconstructs is None else addconstructs
             ...
             owlready2.ObjectPropertyClass)):
                if 'all' in relations or 'isA' in relations:
                    ...
                    if not self.add_missing_node(r, addnodes=addnodes):
                        continue
                    (self.add_edge)(subject=label, 
                     predicate='isA', object=rlabel, edgelabel=edgelabels, **attrs)
                else:
                    if isinstance(r, owlready2.Restriction):
                        ...
                        if 'all' in relations or rname in relations:
                            ...
                            if isinstance(r.value, owlready2.ThingClass):
                                ...
                                if self.addconstructs:
                                    obj = self.add_class_construct(r.value)
                                else:
                                    continue
                            pred = asstring(r, exclude_object=True)
                            (self.add_edge)(label, pred, obj, edgelabel=edgelabels, **attrs)
                    if isinstance(r, owlready2.Inverse) and not 'all' in relations:
                        if 'inverse' in relations:
                            ...
                            if r not in e.get_parents(strict=True):
                                continue
                        (self.add_edge)(subject=label, 
                         predicate='inverse', object=rlabel, edgelabel=edgelabels, **attrs)

    def add_edges(self, sources=None, relations=None, edgelabels=None, addnodes=None, addconstructs=None, **attrs):
 ...
 source, relations=relations, edgelabels=edgelabels, addnodes=addnodes, 
             addconstructs=addconstructs, **attrs)
        ...
        if isinstance(e, owlready2.ThingClass):
            ...
            if isinstance(e, owlready2.Thing):
                ...
                if isinstance(e, owlready2.ObjectPropertyClass):
                    ...
                    if isinstance(e, owlready2.DataPropertyClass):
                        kw = self.style.get('data_property', {})
                    else:
                        if isinstance(e, owlready2.AnnotationPropertyClass):
    ...
    def get_edge_attrs(self, predicate, attrs):
        ...
        if predicate in types:
            ...
            if not isinstance(name, str) or name in self.ontology:
                ...
                rels = set((self.ontology[r] for r in relations.keys() if r in self.ontology))
                for r in e.mro():
                    if r in rels:
                        break
    ...
    def add_legend(self, relations=None):
        ...
        for i, r in enumerate(relations):
            label1.append('<tr><td align="right" port="i%d">%s</td></tr>' % (i, r))
...
def plot_modules(iri, filename=None, format=None, show=False, modules=None, ignore_redundant=True):
    ...
    dot.attr(rankdir='TB')
    dot.node_attr.update(style='filled', fillcolor='lightblue', shape='box', edgecolor='blue')
