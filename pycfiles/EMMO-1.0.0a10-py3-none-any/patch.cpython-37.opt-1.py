...
# [GCC 8.4.0]
# Embedded file name: /emmo/patch.py
...
def _get_parents(self, strict=False):
    ...
    if strict:
        ...
        return s
    if isinstance(self, owlready2.ThingClass):
        return {cls for cls in self.is_a if isinstance(cls, owlready2.ThingClass) if isinstance(cls, owlready2.ThingClass)}
    if isinstance(self, owlready2.ObjectPropertyClass):
        return {cls for cls in self.is_a if isinstance(cls, owlready2.ObjectPropertyClass) if isinstance(cls, owlready2.ObjectPropertyClass)}
    assert 0
...
def get_class_annotations(self, all=False):
    ...
    d = {a.label.first():a._get_values_for_class(self) for a in onto.annotation_properties()}
    d.update({k:v._get_values_for_class(self) for k, v in self.__class__.namespace.world._props.items()})
    if all:
        return d
    return {k:v for k, v in d.items() if v if k != 'label' if k != 'label'}
...
def get_individual_annotations(self, all=False):
    ...
    d = {a.label.first():a._get_values_for_class(self) for a in onto.annotation_properties()}
    d.update({k:v._get_values_for_individual(self) for k, v in props.items()})
    if all:
        return d
    return {k:v for k, v in d.items() if v if k != 'label' if k != 'label'}
