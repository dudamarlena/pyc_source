...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/tornetcd/client.py
# Compiled at: 2016-06-16 04:40:57
"""
tornetcd
...
class Client(object):
    ...
    def __init__(self, host='127.0.0.1:2730', version_prefix='/v2', read_timeout=60, allow_reconnect=True, allow_redirect=True, protocol='http', cert_options=None, username=None, password=None, use_proxies=False, expected_cluster_id=None, ioloop=None, httpclient=None):
        if not ioloop:
        ...
        _log.debug('Machines cache initialised to %s', self._machines_cache)
        ...
        if not use_proxies:
            PeriodicCallback(self.search_machine, 60000).start()
        return
    ...
    def search_machine(self, callback=None):
        ...
        req = HTTPRequest(uri, self._MGET, request_timeout=self.read_timeout, follow_redirects=self.allow_redirect)
        ...
        def _callback(fut):
            ...
            if exc:
                if not isinstance(exc[1], etcdexcept.EtcdException):
                    _log.error('Failed to get list of machines from %s%s: %r and retry it.', uri, self.version_prefix, exc)
                    if self._machines_cache:
                        ...
                        return
                    raise etcdexcept.EtcdException('Could not get the list of servers, maybe you provided the wrong host(s) to connect to?')
            else:
                response = fut.result()
                machines = [ node.strip() for node in self._handle_server_response(response).body.decode('utf-8').split(',')
                           ]
    ...
    def _stats(self, what='self', callback=None):
        ...
        fut = self.api_execute(self.version_prefix + '/stats/' + what, self._MGET)
    ...
    def write(self, key, value, ttl=None, dir=False, append=False, callback=None, **kwdargs):
        ...
        """
        _log.debug('Writing %s to key %s ttl=%s dir=%s append=%s', value, key, ttl, dir, append)
        key = self._sanitize_key(key)
        params = {}
        ...
        if dir:
            if value:
                raise etcdexcept.EtcdException('Cannot create a directory with a value')
            params['dir'] = 'true'
        for k, v in kwdargs.items():
            if k in self._comparison_conditions:
                if type(v) == bool:
    ...
    def update(self, obj, callback=None):
        ...
        """
        if not isinstance(obj, EtcdResult):
            raise AssertionError('obj not a EtcdResult.')
            _log.debug('Updating %s to %s.', obj.key, obj.value)
            kwdargs = {'dir': obj.dir, 
               'ttl': obj.ttl, 
               'prevExist': True}
            kwdargs['prevIndex'] = obj.dir or obj.modifiedIndex
    ...
    def delete(self, key, recursive=None, dir=None, callback=None, **kwdargs):
        ...
        """
        _log.debug('Deleting %s recursive=%s dir=%s extra args=%s', key, recursive, dir, kwdargs)
        key = self._sanitize_key(key)
        kwds = {}
    ...
    def pop(self, key, recursive=None, dir=None, callback=None, **kwdargs):
        ...
        """
        return self.delete(key=key, recursive=recursive, dir=dir, callback=(lambda result: callback(result._prev_node)), **kwdargs)
    ...
    def watch(self, key, index=None, timeout=None, recursive=None, callback=None):
        ...
        if index:
            return self.read(key, wait=True, waitIndex=index, timeout=timeout, recursive=recursive, callback=callback)
        else:
            return self.read(key, wait=True, timeout=timeout, recursive=recursive, callback=callback)
    ...
    def eternal_watch(self, key, index=None, recursive=None, callback=None, timeout=0):
        ...
        def _cb(future):
            ...
            if not exc:
                ...
                watch_again(_cb)
                callback(response)
            elif isinstance(exc, etcdexcept.EtcdWatchTimedOut):
    ...
    def _result_from_response(self, response):
        ...
        try:
            ...
            raise etcdexcept.EtcdException('Server response was not valid JSON: %r' % e)
        ...
        try:
            ...
            raise etcdexcept.EtcdException('Unable to decode server response: %r' % e)
            ...
            def _callback(fut):
                ...
                if exc_obj:
                    if isinstance(exc_obj, HTTPError) and exc_obj.code < 500:
                        ...
                        return
                    if isinstance(exc_obj, HTTPError) and exc_obj.code > 500 or isinstance(exc_obj, socket.error):
                        _log.error('Request to server %s failed: %r', self._base_url, exc_obj)
                        if isinstance(params, dict) and params.get('wait') == 'true' and hasattr(exc_obj, 'code') and exc_obj.code == 599:
                            _log.debug('Watch timed out.')
                            exc = etcdexcept.EtcdWatchTimedOut('Watch timed out: %r' % exc_obj, cause=exc_obj)
                            new_future.set_exception(exc)
                            return
                        if self._allow_reconnect and fut._retry:
                            _log.info('Reconnection allowed, looking for another server.')
                        ...
                        exc = etcdexcept.EtcdConnectionFailed('Connection to etcd failed due to %r' % exc_obj, cause=exc_obj)
                        new_future.set_exception(exc)
                        return
                    else:
    ...
    def api_execute(self, path, method, params=None, timeout=None):
        ...
        if method == self._MGET or method == self._MDELETE:
            ...
            raise etcdexcept.EtcdException(('HTTP method {} not supported').format(method))
        request = HTTPRequest(url, method=method, request_timeout=timeout, headers=self._get_default_headers(method), follow_redirects=self.allow_redirect, body=body, validate_cert=validate_cert, ca_certs=self.cert_options.get('ca_certs', None), client_key=self.cert_options.get('client_key', None), client_cert=self.cert_options.get('client_cert', None), auth_username=self.username, auth_password=self.password)
    ...
    def _check_cluster_id(self, response):
        ...
        if not cluster_id:
            ...
            return
        id_changed = self.expected_cluster_id and cluster_id != self.expected_cluster_id
        old_expected_cluster_id = self.expected_cluster_id
        self.expected_cluster_id = cluster_id
        if id_changed:
            self._base_url = self._choice_machine()
            _log.error(('The UUID of the cluster changed from {} to {}.').format(old_expected_cluster_id, cluster_id))