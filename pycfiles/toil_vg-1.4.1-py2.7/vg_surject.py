...
def run_surjecting(job, context, gam_input_reads_id, output_name, interleaved, xg_file_id, paths):
    ...
    if not context.config.single_reads_chunk:
        reads_chunk_ids = child_job.addChildJobFn(run_split_reads, context, None, 'aln.gam', None, [
         gam_input_reads_id], cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()
    else:
    ...
    return child_job.addFollowOnJobFn(run_whole_surject, context, reads_chunk_ids, output_name, interleaved, xg_file_id, paths, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()
...
def run_whole_surject(job, context, reads_chunk_ids, output_name, interleaved, xg_file_id, paths):
    ...
    for chunk_id, chunk_filename_ids in enumerate(zip(*reads_chunk_ids)):
        chunk_surject_job = child_job.addChildJobFn(run_chunk_surject, context, interleaved, xg_file_id, paths, chunk_filename_ids, ('{}_chunk{}').format(output_name, chunk_id), cores=context.config.alignment_cores, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
        bam_chunk_file_ids.append(chunk_surject_job.rv(0))
        bam_chunk_running_times.append(chunk_surject_job.rv(1))

    return child_job.addFollowOnJobFn(run_merge_bams, output_name, context, bam_chunk_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()
...
def run_merge_bams(job, output_name, context, bam_chunk_file_ids):
    ...
    required_disk = 2 * sum(file_id.size for file_id in flat_ids) + 1073741824
    if job.disk < required_disk:
        ...
        requeued = job.addChildJobFn(run_merge_bams, output_name, context, bam_chunk_file_ids, cores=job.cores, memory=job.memory, disk=required_disk)
        return requeued.rv()
    work_dir = job.fileStore.getLocalTempDir()
...
def surject_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            root_job = Job.wrapJobFn(run_surjecting, context, inputGAMFileID, 'surject', options.interleaved, inputXGFileID, options.paths, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
