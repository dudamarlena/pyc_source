...
import argparse, sys, os, os.path, random, subprocess, shutil, itertools, glob, json, time, timeit, errno
...
def vcfeval_parse_args(parser):
    ...
    parser.add_argument('--vcfeval_opts', type=str, help='Additional options for vcfeval (wrapped in "")', default=None)
    parser.add_argument('--vcfeval_cores', type=int, default=1, help='Cores to use for vcfeval')
    ...
    parser.add_argument('--sv_overlap', type=float, default=0.5, help='minimum reciprical overlap required for bed intersection to count as TP')
    parser.add_argument('--sv_smooth', type=int, default=0, help='mege up svs (in calls and truth) that are at most this many bases apart')
    return
...
def run_extract_sample_truth_vcf(job, context, sample, input_baseline_id, input_baseline_tbi_id):
    ...
    return (
     single_vcf_id, single_vcf_tbi_id)
...
def run_sv_eval(job, context, sample, vcf_tbi_id_pair, vcfeval_baseline_id, vcfeval_baseline_tbi_id, min_sv_len, sv_overlap, sv_region_overlap, sv_smooth=0, bed_id=None, out_name=''):
    ...
    for vcf_name, bed_name in zip([call_vcf_name, vcfeval_baseline_name], [call_bed_name, baseline_bed_name]):
        ...
        with open(os.path.join(work_dir, temp_vcf_name), 'w') as (temp_vcf_file):
            context.runner.call(job, ['bcftools', 'view', '-h', vcf_name], work_dir=work_dir, outfile=temp_vcf_file)
            context.runner.call(job, [['bcftools', 'view', '-H', vcf_name],
             [
              'awk', '-F', '\t', '-v', 'OFS=\t', '{$3="."; print $0;}']], work_dir=work_dir, outfile=temp_vcf_file)
        with open(os.path.join(work_dir, bed_name + '.1del'), 'w') as (bed_file):
            vcf2bed_cmd = [
             [
              'cat', temp_vcf_name], ['vcf2bed']]
         ...
         clipped_calls_name, clipped_baseline_name]):
            with open(os.path.join(work_dir, clipped_bed_name), 'w') as (clipped_bed_file):
                context.runner.call(job, ['bedtools', 'intersect', '-a', bed_name, '-b', regions_bed_name,
                 '-wa', '-f', str(sv_region_overlap), '-u'], work_dir=work_dir, outfile=clipped_bed_file)
    ...
    expand_insertions(os.path.join(work_dir, clipped_calls_name), os.path.join(work_dir, clipped_calls_ins_name), os.path.join(work_dir, clipped_calls_del_name), min_sv_len)
    expand_insertions(os.path.join(work_dir, clipped_baseline_name), os.path.join(work_dir, clipped_baseline_ins_name), os.path.join(work_dir, clipped_baseline_del_name), min_sv_len)
      ...
      tp_del_name, tp_del_rev_name, clipped_calls_del_name, clipped_baseline_del_name, fp_del_name, fn_del_name)]:
        if sv_smooth > 0:
            ...
            with open(os.path.join(work_dir, calls_merge_name), 'w') as (calls_merge_file):
                context.runner.call(job, ['bedtools', 'merge', '-d', str(sv_smooth), '-i', calls_bed_indel_name], work_dir=work_dir, outfile=calls_merge_file)
            with open(os.path.join(work_dir, baseline_merge_name), 'w') as (baseline_merge_file):
                context.runner.call(job, ['bedtools', 'merge', '-d', str(sv_smooth), '-i', baseline_bed_indel_name], work_dir=work_dir, outfile=baseline_merge_file)
        ...
        with open(os.path.join(work_dir, tp_indel_name), 'w') as (tp_file):
            context.runner.call(job, ['bedtools', 'intersect', '-a', calls_bed_indel_name,
             '-b', baseline_bed_indel_name, '-wa', '-f', str(sv_overlap), '-r', '-u'], work_dir=work_dir, outfile=tp_file)
        with open(os.path.join(work_dir, tp_indel_rev_name), 'w') as (tp_file):
            context.runner.call(job, ['bedtools', 'intersect', '-b', calls_bed_indel_name,
             '-a', baseline_bed_indel_name, '-wa', '-f', str(sv_overlap), '-r', '-u'], work_dir=work_dir, outfile=tp_file)
    ...
    results = summarize_sv_results(os.path.join(work_dir, tp_ins_name), os.path.join(work_dir, tp_ins_rev_name), os.path.join(work_dir, fp_ins_name), os.path.join(work_dir, fn_ins_name), os.path.join(work_dir, tp_del_name), os.path.join(work_dir, tp_del_rev_name), os.path.join(work_dir, fp_del_name), os.path.join(work_dir, fn_del_name))
    summary_name = os.path.join(work_dir, ('{}sv_accuracy.tsv').format(out_name))
    with open(summary_name, 'w') as (summary_file):
        header = [
         'Cat', 'TP', 'TP-baseline', 'FP', 'FN', 'Precision', 'Recall', 'F1']
...
def expand_deletions(in_bed_name, out_bed_name):
    ...
    ** We do this before intersection with regions of interest **
    """
    with open(in_bed_name) as (in_bed):
        with open(out_bed_name, 'w') as (out_bed):
            for line in in_bed:
                if line.strip():
...
def expand_insertions(in_bed_name, out_ins_bed_name, out_del_bed_name, min_sv_size):
    ...
    ** We do this after intersection with regions of interest but before comparison **
    """
    with open(in_bed_name) as (in_bed):
        with open(out_ins_bed_name, 'w') as (out_ins):
            with open(out_del_bed_name, 'w') as (out_del):
                for line in in_bed:
                    if line.strip():
                        toks = line.strip().split('\t')
                        ref_len = len(toks[5])
...
def summarize_sv_results(tp_ins, tp_ins_baseline, fp_ins, fn_ins, tp_del, tp_del_baseline, fp_del, fn_del):
    """
        ...
        if not toil.options.restart:
            ...
            if options.vcfeval:
                vcfeval_job = Job.wrapJobFn(run_vcfeval, context, None, (
                 call_vcf_id, call_tbi_id), vcfeval_baseline_id, vcfeval_baseline_tbi_id, options.vcfeval_fasta, fasta_id, bed_id, score_field=options.vcfeval_score_field, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk)
                init_job.addFollowOn(vcfeval_job)
            if options.happy:
                happy_job = Job.wrapJobFn(run_happy, context, None, (
                 call_vcf_id, call_tbi_id), vcfeval_baseline_id, vcfeval_baseline_tbi_id, options.vcfeval_fasta, fasta_id, bed_id, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk)
                init_job.addFollowOn(happy_job)
            if options.sveval:
                sv_job = Job.wrapJobFn(run_sv_eval, context, None, (
                 call_vcf_id, call_tbi_id), vcfeval_baseline_id, vcfeval_baseline_tbi_id, options.min_sv_len, options.sv_overlap, options.sv_region_overlap, options.sv_smooth, bed_id, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk)
