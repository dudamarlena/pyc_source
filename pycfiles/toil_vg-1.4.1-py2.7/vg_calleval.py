...
def calleval_subparser(parser):
    ...
    parser.add_argument('--chroms', nargs='+', required=True, help='Name(s) of reference path in graph(s) (separated by space).')
    ...
    parser.add_argument('--sample_name', type=str, required=True, help='sample name (ex NA12878)')
...
def calleval_parse_args(parser):
    ...
    parser.add_argument('--filter_opts_gt', help='override filter-opts for genotype only')
    ...
    parser.add_argument('--call', action='store_true', help='run vg call (even if --genotype used)')
...
def validate_calleval_options(options):
    ...
    if options.gams or options.gam_names or options.xg_paths:
        require(options.gams and options.gam_names and options.xg_paths and len(options.gam_names) == len(options.xg_paths) == len(options.gams), '--gam_names, --xg_paths, --gams must all contain same number of elements')
        require(options.call or options.genotype or options.surject, '--call and/or --genotype and/or --surject required with --gams')
    ...
    if options.bams or options.bam_names:
        require(options.bams and options.bam_names and len(options.bams) == len(options.bam_names), '--bams and --bam_names must be same length')
    ...
    if options.surject or options.bams:
        require(options.freebayes or options.platypus, '--freebayes and/or --platypus must be used with --bams and --surject')
    return
...
def run_all_bam_caller(job, context, fasta_file_id, bam_file_id, bam_idx_id, sample_name, chroms, offsets, out_name, bam_caller, bam_caller_opts=[]):
    ...
    for chrom, offset in zip(chroms, offsets):
        fb_job = child_job.addChildJobFn(run_bam_caller, context, fasta_file_id, bam_file_id, bam_idx_id, sample_name, chrom, offset, out_name, bam_caller, bam_caller_opts, cores=context.config.calling_cores, memory=context.config.calling_mem, disk=context.config.calling_disk)
...
def run_bam_caller(job, context, fasta_file_id, bam_file_id, bam_idx_id, sample_name, chrom, offset, out_name, bam_caller, bam_caller_opts):
    ...
    return (
     context.write_output_file(job, vcf_fix_path + '.gz'),
    ...
    for roc_type in ['snp', 'non_snp', 'weighted']:
        for mode in ['unclipped', 'clipped']:
            ...
            for subset_number, plot_set in enumerate(plot_sets):
                ...
                roc_plot_ids.append(job.addChildJobFn(run_vcfeval_roc_plot, context, subset_ids, names=plot_conditions, kind=roc_kind, number=subset_number, title=plot_title).rv())
...
def run_calleval_results(job, context, names, vcf_tbi_pairs, eval_results_dict, happy_results_dict, sveval_results_dict, timing_results, plot_sets):
    ...
    calling_labels = [
     'call', 'genotype', 'freebayes', 'platypus']
    ...
    return roc_plot_job.addFollowOnJobFn(run_concat_lists, [
     context.write_output_file(job, stats_path), context.write_output_file(job, times_path)], roc_plot_ids).rv()
...
def run_calleval(job, context, xg_ids, gam_ids, gam_idx_ids, bam_ids, bam_idx_ids, gam_names, bam_names, vcfeval_baseline_id, vcfeval_baseline_tbi_id, caller_fasta_id, vcfeval_fasta_id, bed_id, clip_only, call, genotype, sample_name, chroms, vcf_offsets, vcfeval_score_field, plot_sets, filter_opts_gt, surject, interleaved, freebayes, platypus, happy, sveval, recall):
    """
    ...
    sample_extract_job = head_job.addChildJobFn(run_extract_sample_truth_vcf, context, sample_name, vcfeval_baseline_id, vcfeval_baseline_tbi_id, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk)
    ...
    if surject:
        for xg_id, gam_name, gam_id in zip(xg_ids, gam_names, gam_ids):
            surject_job = head_job.addChildJobFn(run_surjecting, context, gam_id, gam_name + '-surject', interleaved, xg_id, chroms, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    ...
    if bam_ids:
        for bam_id, bam_idx_id, bam_name in zip(bam_ids, bam_idx_ids, bam_names):
            if not bam_idx_id:
                bam_index_job = child_job.addChildJobFn(run_bam_index, context, bam_id, bam_name, cores=context.config.calling_cores, memory=context.config.calling_mem, disk=context.config.calling_disk)
            ...
            for bam_caller, bam_caller_opts, bam_caller_tag in bam_caller_infos:
                ...
                if bed_id:
                    ...
                    if happy:
                        happy_results[bam_caller_out_name]['clipped'] = bam_caller_job.addFollowOnJobFn(run_happy, context, sample_name, bam_caller_vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, bed_id, out_name=bam_caller_out_name, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk).rv()
                    if sveval:
                        sveval_results[bam_caller_out_name]['clipped'] = bam_caller_job.addFollowOnJobFn(run_sv_eval, context, sample_name, bam_caller_vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, min_sv_len=25, sv_overlap=0.5, sv_region_overlap=1, bed_id=bed_id, out_name=bam_caller_out_name, cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk).rv()
                if not clip_only:
                    ...
                    if happy:
                        happy_results[bam_caller_out_name]['unclipped'] = bam_caller_job.addFollowOnJobFn(run_happy, context, sample_name, bam_caller_vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, None, out_name=bam_caller_out_name if not bed_id else bam_caller_out_name + '-unclipped', cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk).rv()
                    if sveval:
                        sveval_results[bam_caller_out_name]['unclipped'] = bam_caller_job.addFollowOnJobFn(run_sv_eval, context, sample_name, bam_caller_vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, min_sv_len=25, sv_overlap=0.5, sv_region_overlap=1, bed_id=None, out_name=bam_caller_out_name if not bed_id else bam_caller_out_name + '-unclipped', cores=context.config.vcfeval_cores, memory=context.config.vcfeval_mem, disk=context.config.vcfeval_disk).rv()
        ...
        for gam_id, gam_idx_id, gam_name, xg_id in zip(gam_ids, gam_idx_ids, gam_names, xg_ids):
            for gt in [False, True]:
                if call and not gt or genotype and gt:
                    ...
                    call_job = child_job.addChildJobFn(run_all_calling, gt_context if gt else context, xg_id, [gam_id], [gam_idx_id], chroms, vcf_offsets, sample_name, genotype=gt, recall=recall, out_name=out_name, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
                    ...
                    if bed_id:
                        eval_results[out_name]['clipped'] = call_job.addFollowOnJobFn(run_vcfeval, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, bed_id, out_name=out_name, score_field=score_field).rv()
                        if happy:
                            happy_results[out_name]['clipped'] = call_job.addFollowOnJobFn(run_happy, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, bed_id, out_name=out_name).rv()
                        if sveval:
                            sveval_results[out_name]['clipped'] = call_job.addFollowOnJobFn(run_sv_eval, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, min_sv_len=25, sv_overlap=0.5, sv_region_overlap=1, bed_id=bed_id, out_name=out_name).rv()
                    if not clip_only:
                        eval_results[out_name]['unclipped'] = call_job.addFollowOnJobFn(run_vcfeval, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, None, out_name=out_name if not bed_id else out_name + '-unclipped', score_field=score_field).rv()
                        if happy:
                            happy_results[out_name]['unclipped'] = call_job.addFollowOnJobFn(run_happy, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, 'ref.fasta', vcfeval_fasta_id, None, out_name=out_name if not bed_id else out_name + '-unclipped').rv()
                        if sveval:
                            sveval_results[out_name]['unclipped'] = call_job.addFollowOnJobFn(run_sv_eval, context, sample_name, vcf_tbi_id_pair, truth_vcf_id, truth_vcf_tbi_id, min_sv_len=25, sv_overlap=0.5, sv_region_overlap=1, bed_id=None, out_name=out_name if not bed_id else out_name + '-unclipped').rv()
                    vcf_tbi_id_pairs.append(vcf_tbi_id_pair)
                    timing_results.append(timing_result)
                    names.append(out_name)

    calleval_results = child_job.addFollowOnJobFn(run_calleval_results, context, names, vcf_tbi_id_pairs, eval_results, happy_results, sveval_results, timing_results, plot_sets, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()
    return (
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            if options.vcfeval_fasta.endswith('.gz'):
                vcfeval_fasta_id = init_job.addChildJobFn(run_unzip_fasta, context, vcfeval_fasta_id, os.path.basename(options.vcfeval_fasta)).rv()
            ...
            if options.chroms:
                vcf_subset_job = init_job.addChildJobFn(run_vcf_subset, context, vcfeval_baseline_id, vcfeval_baseline_tbi_id, options.chroms, disk=context.config.vcfeval_disk)
                vcfeval_baseline_id, vcfeval_baseline_tbi_id = vcf_subset_job.rv(0), vcf_subset_job.rv(1)
            root_job = Job.wrapJobFn(run_calleval, context, inputXGFileIDs, inputGamFileIDs, inputGamIdxIDs, inputBamFileIDs, inputBamIdxIds, options.gam_names, options.bam_names, vcfeval_baseline_id, vcfeval_baseline_tbi_id, caller_fasta_id, vcfeval_fasta_id, bed_id, clip_only, options.call, options.genotype, options.sample_name, options.chroms, options.vcf_offsets, options.vcfeval_score_field, plot_sets, options.filter_opts_gt, options.surject, options.interleaved, options.freebayes, options.platypus, options.happy, options.sveval, options.recall, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
