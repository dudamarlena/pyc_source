...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/vg_call.py
# Compiled at: 2018-11-03 15:09:40
"""
Generate a VCF from a GAM and XG by splitting into GAM/VG chunks.
"""
from __future__ import print_function
import argparse, sys, os, os.path, random, subprocess, shutil, itertools, glob, json, timeit, errno, copy, time
...
def call_subparser(parser):
    ...
    parser.add_argument('xg_path', type=make_url, help='input xg file')
    parser.add_argument('sample_name', type=str, help='sample name (ex NA12878)')
    ...
    parser.add_argument('--chroms', nargs='+', required=True, help='Name(s) of reference path in graph(s) (separated by space). Must be same length/order as --gams')
    parser.add_argument('--gams', nargs='+', required=True, type=make_url, help='GAMs to call.  One per chromosome in the same order as --chroms, or just one.  Indexes (.gai) will be used if found.')
    parser.add_argument('--gam_index_cores', type=int, help='number of threads used for gam indexing')
...
def chunked_call_parse_args(parser):
    ...
    parser.add_argument('--overlap', type=int, help='overlap option that is passed into make_chunks and call_chunk')
    parser.add_argument('--call_chunk_size', type=int, help='chunk size')
    parser.add_argument('--call_opts', type=str, help='arguments to pass to vg call (wrapped in "")')
    parser.add_argument('--genotype', action='store_true', help='use vg genotype instead of vg call')
    parser.add_argument('--no_augment', action='store_true', help='only call existing variants in the graph')
    parser.add_argument('--genotype_opts', type=str, help='arguments to pass to vg genotype (wrapped in "")')
    parser.add_argument('--filter_opts', type=str, help='argument to pass to vg filter (wrapped in "")')
    parser.add_argument('--calling_cores', type=int, help='number of threads during the variant calling step')
    parser.add_argument('--call_chunk_cores', type=int, help='number of threads used for extracting chunks for calling')
    parser.add_argument('--vcf_offsets', nargs='+', default=[], help='offset(s) to apply to output vcfs(s). (order of --chroms)')
    parser.add_argument('--recall', action='store_true', help='only call variants present in the graph')


def validate_call_options(options):
    require(len(options.chroms) == len(options.gams) or len(options.gams) == 1, 'Number of --chroms must be 1 or same as number of --gams')
    require(not options.vcf_offsets or len(options.vcf_offsets) == len(options.chroms), 'Number of --vcf_offsets if specified must be same as number of --chroms')
    require(not options.no_augment or options.genotype, '--no_augment currently only supported with --genotype')
    require(not options.genotype or not options.recall, '--recall not yet supported with --genotype')
...
def sort_vcf(job, drunner, vcf_path, sorted_vcf_path):
    ...
    with open(sorted_vcf_path, 'w') as (outfile):
        drunner.call(job, [['bcftools', 'view', '-h', vcf_name]], outfile=outfile, work_dir=vcf_dir)
    with open(sorted_vcf_path, 'a') as (outfile):
        drunner.call(job, [['bcftools', 'view', '-H', vcf_name],
         [
          'sort', '-k1,1d', '-k2,2n']], outfile=outfile, work_dir=vcf_dir)


def run_vg_call(job, context, sample_name, vg_id, gam_id, xg_id=None, path_names=[], seq_names=[], seq_offsets=[], seq_lengths=[], filter_opts=[], augment_opts=[], call_opts=[], keep_pileup=False, keep_xg=False, keep_gam=False, keep_augmented=False, chunk_name='call', genotype=False, augment=True, recall=False):
    ...
    if not xg_id and defray:
        ...
        context.runner.call(job, ['vg', 'index', os.path.basename(vg_path), '-x',
         os.path.basename(xg_path), '-t', str(context.config.calling_cores)], work_dir=work_dir)
        timer.stop()
        if keep_xg:
    ...
    if filter_opts:
        filter_command = [
         'vg', 'filter', os.path.basename(gam_path), '-t', '1'] + filter_opts
        if defray:
    ...
    if genotype:
        assert not recall
        command = [
         'vg', 'genotype', os.path.basename(vg_path), '-t',
        ...
        try:
            with open(vcf_path, 'w') as (vggenotype_stdout):
                ...
                context.runner.call(job, command, work_dir=work_dir, outfile=vggenotype_stdout)
                timer.stop()
        ...
        augment_command.append([
         'vg', 'augment', os.path.basename(vg_path), aug_gam_input, '-t', str(context.config.calling_cores)] + augment_opts + augment_generated_opts)
        try:
            if augment:
            ...
            if keep_augmented:
                aug_graph_id = context.write_intermediate_file(job, aug_path)
            command = [
             'vg', 'call', os.path.basename(aug_path), '-t',
             str(context.config.calling_cores), '-S', sample_name,
             '-z', os.path.basename(trans_path),
    ...
    return (
     vcf_id, pileup_id, xg_id, gam_id, aug_graph_id, timer)


def run_vg_genotype(job, context, sample_name, vg_id, gam_id, xg_id=None, path_names=[], seq_names=[], seq_offsets=[], seq_lengths=[], genotype_opts=[], filter_opts=[], keep_xg=False, keep_gam=False, keep_augmented=False, index_gam=True, chunk_name='genotype'):
    ...
    if index_gam:
        ...
        command = [
         [
          'vg', 'index', '-A', '-d', os.path.basename(gam_node_index_path)]]
    ...
    try:
        ...
        with open(vcf_path, 'w') as (vgcall_stdout):
            command = [
             'vg', 'genotype', os.path.basename(vg_path)]
            if index_gam:
...
def run_call_chunk(job, context, path_name, chunk_i, num_chunks, chunk_offset, clipped_chunk_offset, xg_file_id, vg_chunk_file_id, gam_chunk_file_id, path_size, vcf_offset, sample_name, genotype, augment, recall):
    """ create VCF from a given chunk """
    child_job = Job()
    job.addChild(child_job)
    RealtimeLogger.info(('Running call_chunk on path {} and chunk {}').format(path_name, chunk_i))
    work_dir = job.fileStore.getLocalTempDir()
    call_job = child_job.addChildJobFn(run_vg_call, context, sample_name, vg_chunk_file_id, gam_chunk_file_id, xg_id=xg_file_id, path_names=[
     path_name], seq_names=[
     path_name], seq_offsets=[
     chunk_offset + vcf_offset], seq_lengths=[
     path_size], filter_opts=context.config.filter_opts, augment_opts=context.config.augment_opts, call_opts=(genotype or context.config).call_opts if 1 else context.config.genotype_opts, chunk_name=('chunk_{}_{}').format(path_name, chunk_offset), genotype=genotype, augment=augment, recall=recall, cores=context.config.calling_cores, memory=context.config.calling_mem, disk=context.config.calling_disk)
    vcf_id, pu_id, xg_id, gam_id, aug_graph_id = [ call_job.rv(i) for i in range(5) ]
    call_timer = call_job.rv(5)
    clip_job = child_job.addFollowOnJobFn(run_clip_vcf, context, path_name, chunk_i, num_chunks, chunk_offset, clipped_chunk_offset, vcf_offset, vcf_id, cores=context.config.calling_cores, memory=context.config.calling_mem, disk=context.config.calling_disk)
...
def run_clip_vcf(job, context, path_name, chunk_i, num_chunks, chunk_offset, clipped_chunk_offset, vcf_offset, vcf_id):
    ...
    with open(clip_path, 'w') as (clip_path_stream):
        offset = vcf_offset + 1
        command = ['bcftools', 'view', '-t',
         ('{}:{}-{}').format(path_name, offset + clipped_chunk_offset + left_clip, offset + clipped_chunk_offset + context.config.call_chunk_size - right_clip - 1),
...
def run_all_calling(job, context, xg_file_id, chr_gam_ids, chr_gam_idx_ids, chroms, vcf_offsets, sample_name, genotype=False, augment=True, out_name=None, recall=False):
    """
    ...
    call_timers_lists = []
    if not len(chr_gam_ids) > 0:
        raise AssertionError
        chr_gam_idx_ids = chr_gam_idx_ids or [
         None] * len(chr_gam_ids)
    if not len(chr_gam_ids) == len(chr_gam_idx_ids):
        raise AssertionError
        for i in range(len(chr_gam_ids)):
            ...
            call_job = child_job.addChildJobFn(run_calling, context, xg_file_id, alignment_file_id, alignment_index_id, chr_label, chr_offset, sample_name, genotype=genotype, augment=augment, recall=recall, cores=context.config.call_chunk_cores, memory=context.config.call_chunk_mem, disk=context.config.call_chunk_disk)
            vcf_tbi_file_id_pair_list.append((call_job.rv(0), call_job.rv(1)))
            call_timers_lists.append(call_job.rv(2))

        out_name = out_name or sample_name
    return child_job.addFollowOnJobFn(run_merge_vcf, context, out_name, vcf_tbi_file_id_pair_list, call_timers_lists, cores=context.config.call_chunk_cores, memory=context.config.call_chunk_mem, disk=context.config.call_chunk_disk).rv()
...
def run_merge_vcf(job, context, out_name, vcf_tbi_file_id_pair_list, call_timers_lists=[]):
    ...
    vcf_idx_file_id = context.write_output_file(job, vcf_file_idx, out_store_path=out_store_key + '.tbi')
    timer.stop()
    ...
    if call_timers_lists:
        ...
        return (
         vcf_file_id, vcf_idx_file_id)


def run_calling(job, context, xg_file_id, alignment_file_id, alignment_index_id, path_names, vcf_offsets, sample_name, genotype, augment, recall):
    """
    ...
    if alignment_index_id:
        ...
        with open(gam_sort_path, 'w') as (gam_sort_stream):
            gamsort_cmd = [
             'vg', 'gamsort', '-i', os.path.basename(gam_index_path), os.path.basename(gam_path),
    ...
    for toks in bed_lines:
        ...
        call_job = child_job.addChildJobFn(run_call_chunk, context, chunk_bed_chrom, chunk_i, len(bed_lines), chunk_bed_start, clipped_chunk_offset, None, vg_chunk_file_id, gam_chunk_file_id, path_size[chunk_bed_chrom], offset_map[chunk_bed_chrom], sample_name, genotype, augment, recall, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
        clip_file_ids.append(call_job.rv(0))
        call_timers.append(call_job.rv(1))

    merge_job = child_job.addFollowOnJobFn(run_merge_vcf_chunks, context, tag, clip_file_ids, cores=context.config.call_chunk_cores, memory=context.config.call_chunk_mem, disk=context.config.call_chunk_disk)
    vcf_out_file_id = merge_job.rv(0)
    tbi_out_file_id = merge_job.rv(1)
    return (
     vcf_out_file_id, tbi_out_file_id, call_timers)
...
def run_merge_vcf_chunks(job, context, path_name, clip_file_ids):
    ...
    for chunk_i, clip_file_id in enumerate(clip_file_ids):
        ...
        if chunk_i == 0:
            with open(vcf_path, 'w') as (outfile):
                context.runner.call(job, ['cat', os.path.basename(clip_path)], outfile=outfile, work_dir=work_dir)
        else:
            with open(vcf_path, 'a') as (outfile):
                context.runner.call(job, ['bcftools', 'view', '-H', os.path.basename(clip_path)], outfile=outfile, work_dir=work_dir)

    vcf_gz_file = vcf_path + '.gz'
    with open(vcf_gz_file, 'w') as (vcf_gz_file_stream):
        command = [
         'bgzip', '-c', ('{}').format(os.path.basename(vcf_path))]
        context.runner.call(job, command, work_dir=work_dir, outfile=vcf_gz_file_stream)
    command = [
     'bcftools', 'tabix', '-f', '-p', 'vcf', ('{}').format(os.path.basename(vcf_path + '.gz'))]
    context.runner.call(job, command, work_dir=work_dir)
    ...
    return (
     vcf_gz_file_id, vcf_tbi_file_id)
...
def call_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            root_job = Job.wrapJobFn(run_all_calling, context, inputXGFileID, inputGamFileIDs, inputGamIndexFileIDs, options.chroms, options.vcf_offsets, options.sample_name, genotype=options.genotype, augment=not options.no_augment, recall=options.recall, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
