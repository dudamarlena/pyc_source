...
def run_sim(job, context, num_reads, gam, fastq_out, seed, sim_chunks, xg_file_ids, xg_annot_file_id, tag_bed_ids=[], paths=[], drop_contigs_matching=[], fastq_id=None, out_name=None):
    """  
    ...
    for xg_i, xg_file_id in enumerate(xg_file_ids):
        ...
        for chunk_i in range(sim_chunks):
            ...
            if chunk_i == sim_chunks - 1:
                chunk_reads += file_reads % sim_chunks
            sim_out_id_info = child_job.addChildJobFn(run_sim_chunk, context, gam, seed_base, xg_file_id, xg_annot_file_id, chunk_reads, chunk_i, xg_i, tag_bed_ids=tag_bed_ids, paths=paths, drop_contigs_matching=drop_contigs_matching, fastq_id=fastq_id, cores=context.config.sim_cores, memory=context.config.sim_mem, disk=context.config.sim_disk).rv()
            sim_out_id_infos.append(sim_out_id_info)

    merge_job = child_job.addFollowOnJobFn(run_merge_sim_chunks, context, gam, sim_out_id_infos, out_name, cores=context.config.sim_cores, memory=context.config.sim_mem, disk=context.config.sim_disk)
    merged_gam_id, true_id = merge_job.rv(0), merge_job.rv(1)
    if fastq_out:
        fastq_job = merge_job.addFollowOnJobFn(run_gam_to_fastq, context, merged_gam_id, False, out_name=out_name if out_name else 'sim', out_store=True, cores=context.config.sim_cores, memory=context.config.sim_mem, disk=context.config.sim_disk)
        merged_fq_id = fastq_job.rv(0)
    return (merged_gam_id, true_id)


def run_sim_chunk(job, context, gam, seed_base, xg_file_id, xg_annot_file_id, num_reads, chunk_i, xg_i, tag_bed_ids=[], paths=[], drop_contigs_matching=[], fastq_id=None):
    """
    ...
    sim_cmd = [
     'vg', 'sim', '-x', os.path.basename(xg_file), '-n', num_reads] + context.config.sim_opts
...
def run_merge_sim_chunks(job, context, gam, sim_out_id_infos, out_name):
    ...
    if not len(sim_out_id_infos) > 0:
        ...
        if out_name:
            ...
            reads_name = 'sim'
            pos_name = 'true'
        merged_reads_file = gam or os.path.join(('{}.reads').format(reads_name))
        ...
        with open(merged_gam_file, 'a') as (out_gam):
            with open(merged_true_file, 'a') as (out_true):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            if options.fastq and options.fastq.endswith('.gz'):
                inputFastqFileID = init_job.addChildJobFn(run_unzip_fasta, context, inputFastqFileID, os.path.basename(options.fastq)).rv()
            root_job = Job.wrapJobFn(run_sim, context, options.num_reads, options.gam, options.fastq_out, options.seed, options.sim_chunks, inputXGFileIDs, inputAnnotXGFileID, tag_bed_ids=tagBEDIDs, paths=options.path, drop_contigs_matching=options.drop_contigs_matching, fastq_id=inputFastqFileID, out_name=options.out_name, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
