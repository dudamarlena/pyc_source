...
from toil_vg.vg_common import require, make_url, remove_ext, add_common_vg_parse_args, add_container_tool_parse_args, get_vg_script, run_concat_lists, parse_plot_sets, title_to_filename
...
def add_mapeval_options(parser):
    ...
    parser.add_argument('--use-gbwt', action='store_true', help='also import <index-base>.gbwt and use it during alignment')
    ...
    parser.add_argument('--fasta', type=make_url, default=None, help='fasta sequence file (required for bwa. if a bwa index exists for this file, it will be used)')
    ...
    parser.add_argument('--ignore-quals', action='store_true', help='never use quality adjusted alignment. necessary if using --multipath on reads not from trained simulator')
    parser.add_argument('--multipath-only', action='store_true', help='run only mpmap and not map (--multipath will run both and using neither will just run map)')
...
def validate_options(options):
    ...
    require(not options.fastq or all(map(lambda x: x.endswith('.gz'), options.fastq)), 'only gzipped fastqs (ending with .gz) accepted by --fastq')
    if options.bwa:
    ...
    if options.use_gbwt:
        require(not options.gams, '--use-gbwt cannot be used with pre-aligned GAMs in --gams')
    require(not options.gbwt_penalties or options.use_gbwt, '--gbwt-penalties requires --use-gbwt')
    ...
    if options.use_snarls:
        require(options.multipath or options.multipath_only, '--use-snarls only affects the multipath mapper (--multipath or --multipath-only)')
    if options.gams:
    ...
    if options.gams:
        require(options.gam_names and len(options.gams) == len(options.gam_names), '--gams and --gam_names must have same number of inputs')
    if options.vg_graphs:
        require(options.gam_names and len(options.vg_graphs) == len(options.gam_names), '--vg-graphs and --gam_names must have same number of inputs')
    if options.index_bases:
        require(options.gam_names and len(options.index_bases) == len(options.gam_names), '--index-bases and --gam_names must have same number of inputs')
    names = []
...
def run_bwa_mem(job, context, fq_reads_ids, bwa_index_ids, paired_mode):
    ...
    if job.disk < required_disk:
        ...
        requeued = job.addChildJobFn(run_bwa_mem, context, fq_reads_ids, bwa_index_ids, paired_mode, cores=job.cores, memory=job.memory, disk=required_disk)
        return requeued.rv()
    work_dir = job.fileStore.getLocalTempDir()
    fq_file_names = []
...
def extract_gam_read_stats(job, context, name, gam_file_id, generate_tags=[]):
    ...
    jq_cmd = [
     'jq', '-c', '-r', tag_generation + '[.name] + if (.annotation.features | length) > 0 then [.annotation.features | join(",")] else ["."] end + if .refpos != null then [.refpos[] | .name, if .offset != null then .offset else 0 end] else [] end + [.score] + if .mapping_quality == null then [0] else [.mapping_quality] end | @tsv',
...
def run_map_eval_index(job, context, xg_file_ids, gcsa_file_ids, gbwt_file_ids, id_range_file_ids, snarl_file_ids, vg_file_ids):
    ...
    if vg_file_ids:
        for vg_file_id in vg_file_ids:
            index_job = job.addChildJobFn(run_indexing, context, [vg_file_id], ['default.vg'], 'index', ['default'], cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
...
def run_map_eval_align(job, context, index_ids, xg_comparison_ids, gam_names, gam_file_ids, reads_fastq_single_ids, reads_fastq_paired_ids, reads_fastq_paired_for_vg_ids, fasta_file_id, bwa_index_ids, matrix, ignore_quals, surject):
    """
    ...
    condition_steps = [
     aligner_conditions, multipath_conditions, multipath_opts_conditions, paired_conditions, gbwt_conditions]
    ...
    bwa_bam_file_ids, bwa_mem_times = [
     None, None], [None, None]
    bwa_start_job = None
    surjected_results = {'bam_file_ids': [], 'pe_bam_file_ids': [], 'bam_names': [], 'pe_bam_names': []}
    read_chunk_jobs = {}
    ...
    for condition in condition_generator([{}]):
        if condition['aligner'] == 'vg' and do_vg_mapping:
            ...
            if condition['paired']:
                tag_string += '-pe'
                if not reads_fastq_paired_for_vg_ids:
                    raise AssertionError
                    fastq_ids = reads_fastq_paired_for_vg_ids
                    interleaved = len(reads_fastq_paired_for_vg_ids) == 1
                    assert interleaved or len(reads_fastq_paired_for_vg_ids) == 2
            else:
            ...
            for i, indexes in enumerate(index_ids):
                ...
                if not read_chunk_jobs.has_key(tuple(fastq_ids)):
                    read_chunk_jobs[tuple(fastq_ids)] = job.addChildJobFn(run_split_reads_if_needed, context, fq_names(fastq_ids), None, None, fastq_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
                read_chunk_job = read_chunk_jobs[tuple(fastq_ids)]
                map_jobs.append(read_chunk_job.addFollowOnJobFn(run_mapping, mapping_context, fq_names(fastq_ids), None, None, ('aligned-{}{}').format(gam_names[i], tag_string), interleaved, condition['multipath'], indexes, reads_chunk_ids=read_chunk_job.rv(), bam_output=False, surject=surject, gbwt_penalty=gbwt_penalty, cores=mapping_context.config.misc_cores, memory=mapping_context.config.misc_mem, disk=mapping_context.config.misc_disk))
            ...
            if bwa_start_job is None:
                ...
                bwa_index_job = bwa_start_job.addChildJobFn(run_bwa_index, context, fasta_file_id, bwa_index_ids=bwa_index_ids, intermediate=True, cores=context.config.bwa_index_cores, memory=context.config.bwa_index_mem, disk=context.config.bwa_index_disk)
                bwa_index_ids = bwa_index_job.rv()
            if condition['paired']:
                assert reads_fastq_paired_ids
                bwa_mem_job = bwa_start_job.addFollowOnJobFn(run_bwa_mem, context, reads_fastq_paired_ids, bwa_index_ids, True, cores=context.config.alignment_cores, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
                bwa_bam_file_ids[1] = bwa_mem_job.rv(0)
                bwa_mem_times[1] = bwa_mem_job.rv(1)
            else:
                assert reads_fastq_single_ids
                bwa_mem_job = bwa_start_job.addFollowOnJobFn(run_bwa_mem, context, reads_fastq_single_ids, bwa_index_ids, False, cores=context.config.alignment_cores, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
    ...
    return (
     out_gam_names, gam_file_ids, out_xg_ids, map_times, bwa_bam_file_ids,
     bwa_mem_times, surjected_results)


def run_map_eval_comparison(job, context, xg_file_ids, gam_names, gam_file_ids, bam_names, bam_file_ids, pe_bam_names, pe_bam_file_ids, bwa_bam_file_ids, surjected_results, true_read_stats_file_id, mapeval_threshold, score_baseline_name=None, original_read_gam=None, downsample_portion=None, gbwt_usage_tag_gam_name=None):
    """
    ...
    for bam_name, surjected_bam_file_id in zip(surjected_results['bam_names'], surjected_results['bam_file_ids']):
    ...
    for pe_bam_name, surjected_pe_bam_file_id in zip(surjected_results['pe_bam_names'], surjected_results['pe_bam_file_ids']):
    ...
    for bam_i, bam_id in enumerate(bam_file_ids):
        ...
        if downsample_portion is not None and downsample_portion != 1.0:
            parent_job = job.addChildJobFn(downsample_bam, context, bam_id, downsample_portion, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=bam_id.size * 2)
            bam_id = parent_job.rv()
        name = ('{}-{}.bam').format(bam_names[bam_i], bam_i)
        bam_stats_jobs.append(parent_job.addChildJobFn(extract_bam_read_stats, context, name, bam_id, False, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk))
    ...
    for bam_i, bam_id in enumerate(pe_bam_file_ids):
        ...
        bam_stats_jobs.append(parent_job.addChildJobFn(extract_bam_read_stats, context, name, bam_id, True, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk))
    ...
    for gam_i, gam_id in enumerate(gam_file_ids):
        ...
        parent_job = job
        if downsample_portion is not None and downsample_portion != 1.0:
            parent_job = job.addChildJobFn(downsample_gam, context, gam, downsample_portion, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=gam.size * 2)
            gam = parent_job.rv()
        annotate_job = parent_job.addChildJobFn(annotate_gam, context, xg_file_ids[gam_i], gam, cores=context.config.misc_cores, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
        generate_tags = []
        if gam_names[gam_i] == gbwt_usage_tag_gam_name:
            generate_tags.append('haplotype_score_used')
        gam_stats_jobs.append(annotate_job.addFollowOnJobFn(extract_gam_read_stats, context, name, annotate_job.rv(), generate_tags=generate_tags, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk))
        gam_stats_file_ids.append(gam_stats_jobs[(-1)].rv())

    position_comparison_job = job.addChildJobFn(run_map_eval_compare_positions, context, true_read_stats_file_id, gam_names, gam_stats_file_ids, bam_names, bam_stats_file_ids, pe_bam_names, pe_bam_stats_file_ids, mapeval_threshold, gbwt_usage_tag_gam_name=gbwt_usage_tag_gam_name, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    ...
    if score_baseline_name is not None:
        ...
        score_comp_job = job.addChildJobFn(run_map_eval_compare_scores, context, score_baseline_name, baseline_stats_id, gam_names, gam_stats_file_ids, bam_names, bam_stats_file_ids, pe_bam_names, pe_bam_stats_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
        for dependency in itertools.chain(gam_stats_jobs, bam_stats_jobs):
    ...
    if original_read_gam is not None:
        stats_job = job.addChildJobFn(extract_gam_read_stats, context, name, original_read_gam, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk)
        score_comp_job = stats_job.addFollowOnJobFn(run_map_eval_compare_scores, context, 'input', stats_job.rv(), gam_names, gam_stats_file_ids, bam_names, bam_stats_file_ids, pe_bam_names, pe_bam_stats_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
        for dependency in itertools.chain(gam_stats_jobs, bam_stats_jobs):
            dependency.addFollowOn(score_comp_job)
...
def run_map_eval_compare_positions(job, context, true_read_stats_file_id, gam_names, gam_stats_file_ids, bam_names, bam_stats_file_ids, pe_bam_names, pe_bam_stats_file_ids, mapeval_threshold, gbwt_usage_tag_gam_name=None):
    """
    ...
    if gbwt_usage_tag_gam_name is not None:
        ...
        for other_id in stats_file_ids:
            propagate_job = job.addChildJobFn(propagate_tag, context, tag_stats_id, other_id, 'haplotype_score_used', cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk)
    ...
    for name, stats_file_id in zip(names, stats_file_ids):
        compare_ids.append(root.addChildJobFn(compare_positions, context, true_read_stats_file_id, name, stats_file_id, mapeval_threshold, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk).rv())

    position_comp_file_id = root.addFollowOnJobFn(run_process_position_comparisons, context, names, compare_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.alignment_disk).rv(1)
    return (
...
def run_process_position_comparisons(job, context, names, compare_ids):
    ...
    with open(results_file, 'w') as (out_stream):
        ...
        for name, compare_id in itertools.izip(names, compare_ids):
            ...
            map_stats.append([
             job.addChildJobFn(run_acc, context, name, compare_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv(),
             job.addChildJobFn(run_auc, context, name, compare_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv(),
             job.addChildJobFn(run_qq, context, name, compare_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv(),
             job.addChildJobFn(run_max_f1, context, name, compare_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()])

    position_results_id = context.write_output_file(job, results_file)
    return (
     job.addFollowOnJobFn(run_write_position_stats, context, names, map_stats).rv(), position_results_id)
...
def run_map_eval_compare_scores(job, context, baseline_name, baseline_stats_file_id, gam_names, gam_stats_file_ids, bam_names, bam_stats_file_ids, pe_bam_names, pe_bam_stats_file_ids):
    """
    ...
    compare_ids = []
    for name, stats_file_id in zip(names, stats_file_ids):
        compare_ids.append(job.addChildJobFn(compare_scores, context, baseline_name, baseline_stats_file_id, name, stats_file_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv())

    stats_job = job.addFollowOnJobFn(run_process_score_comparisons, context, baseline_name, names, compare_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    return (
...
def run_process_score_comparisons(job, context, baseline_name, names, compare_ids):
    ...
    with open(results_file, 'w') as (out_results_file):
        ...
        for name, compare_id in itertools.izip(names, compare_ids):
            ...
            write_tsv(compare_file, name)
            map_stats.append([
             job.addChildJobFn(run_portion_worse, context, name, compare_id, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()])
...
def run_mapeval(job, context, options, xg_file_ids, xg_comparison_ids, gcsa_file_ids, gbwt_file_ids, id_range_file_ids, snarl_file_ids, vg_file_ids, gam_file_ids, reads_gam_file_id, reads_xg_file_id, reads_bam_file_id, reads_fastq_file_ids, fasta_file_id, bwa_index_ids, bam_file_ids, pe_bam_file_ids, true_read_stats_file_id):
    """
    ...
    """
    index_job = job.addChildJobFn(run_map_eval_index, context, xg_file_ids, gcsa_file_ids, gbwt_file_ids, id_range_file_ids, snarl_file_ids, vg_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    if not true_read_stats_file_id and reads_gam_file_id:
        annotate_job = index_job.addChildJobFn(annotate_gam, context, reads_xg_file_id, reads_gam_file_id, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
        true_read_stats_file_id = annotate_job.addFollowOnJobFn(extract_gam_read_stats, context, 'truth', annotate_job.rv(), disk=context.config.alignment_disk).rv()
    elif not true_read_stats_file_id and reads_bam_file_id:
        true_read_stats_file_id = index_job.addChildJobFn(extract_bam_read_stats, context, 'truth', reads_bam_file_id, True, disk=context.config.alignment_disk).rv()
    fq_reads_ids_bwa = reads_fastq_file_ids
    if reads_fastq_file_ids and options.bwa:
        fq_reads_ids_bwa = index_job.addChildJobFn(run_strip_fq_ext, context, reads_fastq_file_ids, disk=context.config.alignment_disk, cores=context.config.alignment_cores).rv()
    ...
    if len(fq_reads_ids) == 2 and not options.paired_only:
        fq_reads_ids = [index_job.addChildJobFn(run_concat_fastqs, context, fq_reads_ids, disk=context.config.alignment_disk).rv()]
    if reads_gam_file_id or reads_bam_file_id:
        if not options.paired_only and not fq_reads_ids:
            fq_reads_ids = index_job.addChildJobFn(fastq_fn, context, reads_gam_file_id if reads_gam_file_id else reads_bam_file_id, False, disk=context.config.alignment_disk).rv()
        if not options.single_only and not fq_paired_reads_ids:
            fq_paired_reads_ids = index_job.addChildJobFn(fastq_fn, context, reads_gam_file_id if reads_gam_file_id else reads_bam_file_id, True, disk=context.config.alignment_disk).rv()
            fq_paired_reads_for_vg_ids = index_job.addChildJobFn(fastq_fn, context, reads_gam_file_id if reads_gam_file_id else reads_bam_file_id, True, True, disk=context.config.alignment_disk).rv()
    matrix = {'aligner': [
                 'vg'], 
       'paired': [], 'multipath': [], 'gbwt': []}
    ...
    alignment_job = index_job.addFollowOnJobFn(run_map_eval_align, context, index_job.rv(), xg_comparison_ids, options.gam_names, gam_file_ids, fq_reads_ids, fq_paired_reads_ids, fq_paired_reads_for_vg_ids, fasta_file_id, bwa_index_ids, matrix, options.ignore_quals, options.surject)
    ...
    if options.skip_eval:
        ...
        comparison_job = comparison_parent_job.addChildJobFn(run_map_eval_comparison, context, xg_ids, gam_names, gam_file_ids, options.bam_names, bam_file_ids, options.pe_bam_names, pe_bam_file_ids, bwa_bam_file_ids, surjected_results, true_read_stats_file_id, options.mapeval_threshold, options.compare_gam_scores, reads_gam_file_id, downsample_portion=options.downsample, gbwt_usage_tag_gam_name=options.gbwt_baseline, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
        ...
        summarize_job = lookup_job.addFollowOnJobFn(run_map_eval_summarize, context, position_stats_file_id, plot_sets, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
...
def run_map_eval_summarize(job, context, position_stats_file_id, plot_sets):
    ...
    plot_job = job.addChildJobFn(run_map_eval_plot, context, position_stats_file_id, plot_sets, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    table_job = job.addChildJobFn(run_map_eval_table, context, position_stats_file_id, plot_sets, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    merge_job = plot_job.addFollowOnJobFn(run_concat_lists, plot_job.rv(), table_job.rv(), cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
...
def mapeval_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            main_job = Job.wrapJobFn(run_mapeval, context, options, plan.xg_file_ids, plan.xg_comparison_ids, plan.gcsa_file_ids, plan.gbwt_file_ids, plan.id_range_file_ids, plan.snarl_file_ids, plan.vg_file_ids, plan.gam_file_ids, plan.reads_gam_file_id, plan.reads_xg_file_id, plan.reads_bam_file_id, plan.reads_fastq_file_ids, plan.fasta_file_id, plan.bwa_index_ids, plan.bam_file_ids, plan.pe_bam_file_ids, plan.true_read_stats_file_id)
