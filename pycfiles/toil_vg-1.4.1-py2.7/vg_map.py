...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_vg/vg_map.py
...
import argparse, sys, os, os.path, errno, random, subprocess, shutil, itertools, glob, tarfile, doctest, re, json, collections, time, timeit, logging, logging.handlers, SocketServer, struct, socket, threading, string, urlparse, getpass, pdb, gzip, logging
...
def map_subparser(parser):
    ...
    parser.add_argument('xg_index', type=make_url, help='Path to xg index')
    parser.add_argument('gcsa_index', type=make_url, help='Path to GCSA index')
    ...
    parser.add_argument('--id_ranges', type=str, default=None, help='Path to file with node id ranges for each chromosome in BED format.')
    parser.add_argument('--kmer_size', type=int, help='size of kmers to use in gcsa-kmer mapping mode')
...
def map_parse_args(parser, stand_alone=False):
    ...
    parser.add_argument('--gbwt_index', type=make_url, help='Path to GBWT haplotype index')
    parser.add_argument('--snarls_index', type=make_url, help='Path to snarls file')
    parser.add_argument('--fastq', nargs='+', type=make_url, help='Input fastq (possibly compressed), two are allowed, one for each mate')
    parser.add_argument('--gam_input_reads', type=make_url, default=None, help='Input reads in GAM format')
    parser.add_argument('--bam_input_reads', type=make_url, default=None, help='Input reads in BAM format')
    ...
    parser.add_argument('--interleaved', action='store_true', default=False, help='treat fastq as interleaved read pairs.  overrides map-args')
    parser.add_argument('--map_opts', type=str, help='arguments for vg map (wrapped in "")')
    parser.add_argument('--multipath', action='store_true', help='use vg mpmap instead of vg map')
    parser.add_argument('--mpmap_opts', type=str, help='arguments for vg mpmap (wrapped in "")')
    parser.add_argument('--bam_output', action='store_true', help='write BAM output directly')
    parser.add_argument('--surject', action='store_true', help='surject output, producing BAM in addition to GAM alignments')
    return
...
def validate_map_options(context, options):
    ...
    require(options.multipath or options.snarls_index is None, 'snarls cannot be used with the single-path mapper')
    if options.multipath:
        require('-S' in context.config.mpmap_opts or '--single-path-mode' in context.config.mpmap_opts, '-S must be used with multipath aligner to produce GAM output')
        require(not options.bam_output, '--bam_output not currently supported with multipath aligner (--multipath)')
    require(not options.bam_output or not options.surject, '--bam_output cannot be used in combination with --surject')
    require(not options.id_ranges or not options.surject, '--surject not currently supported with --id_ranges')
    return
...
def run_split_reads_if_needed(job, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids):
    ...
    if not context.config.single_reads_chunk:
        reads_chunk_ids = job.addChildJobFn(run_split_reads, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk).rv()
    else:
...
def run_mapping(job, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved, multipath, indexes, reads_file_ids=None, reads_chunk_ids=None, bam_output=False, surject=False, gbwt_penalty=None):
    """
    ...
    if reads_chunk_ids is None:
        chunk_job = job.addChildJobFn(run_split_reads_if_needed, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
        reads_chunk_ids = chunk_job.rv()
    align_job = Job.wrapJobFn(run_whole_alignment, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved, multipath, indexes, reads_chunk_ids, bam_output=bam_output, surject=surject, gbwt_penalty=gbwt_penalty, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
...
def run_split_reads(job, context, fastq, gam_input_reads, bam_input_reads, reads_file_ids):
    ...
    if fastq and len(fastq):
        for fastq_i, reads_file_id in enumerate(reads_file_ids):
            reads_chunk_ids.append(job.addChildJobFn(run_split_fastq, context, fastq, fastq_i, reads_file_id, cores=context.config.fq_split_cores, memory=context.config.fq_split_mem, disk=context.config.fq_split_disk).rv())

    elif gam_input_reads:
        assert len(reads_file_ids) == 1
        reads_chunk_ids.append(job.addChildJobFn(run_split_gam_reads, context, gam_input_reads, reads_file_ids[0], cores=context.config.fq_split_cores, memory=context.config.fq_split_mem, disk=context.config.fq_split_disk).rv())
    else:
        assert bam_input_reads
        assert len(reads_file_ids) == 1
        reads_chunk_ids.append(job.addChildJobFn(run_split_bam_reads, context, bam_input_reads, reads_file_ids[0], cores=context.config.fq_split_cores, memory=context.config.fq_split_mem, disk=context.config.fq_split_disk).rv())
    return reads_chunk_ids
...
def run_split_fastq(job, context, fastq, fastq_i, sample_fastq_id):
    ...
    if fastq_gzipped:
        cmd = [
         [
          'gzip', '-d', '-c', os.path.basename(fastq_path)]]
    else:
        cmd = [
         [
          'cat', os.path.basename(fastq_path)]]
...
def run_split_bam_reads(job, context, bam_input_reads, bam_reads_file_id):
    ...
    cmd = [
     [
      'samtools', 'view', '-h', os.path.basename(bam_path)]]
    cmd.append(['split', '-l', str(chunk_lines),
...
def run_whole_alignment(job, context, fastq, gam_input_reads, bam_input_reads, sample_name, interleaved, multipath, indexes, reads_chunk_ids, bam_output=False, surject=False, gbwt_penalty=None):
    """
    ...
    for chunk_id, chunk_filename_ids in enumerate(zip(*reads_chunk_ids)):
        chunk_alignment_job = child_job.addChildJobFn(run_chunk_alignment, context, gam_input_reads, bam_input_reads, sample_name, interleaved, multipath, chunk_filename_ids, chunk_id, indexes, bam_output=bam_output, gbwt_penalty=gbwt_penalty, cores=context.config.alignment_cores, memory=context.config.alignment_mem, disk=context.config.alignment_disk)
        if not bam_output:
    ...
    if not bam_output:
        merge_gams_job = child_job.addFollowOnJobFn(run_merge_gams, context, sample_name, indexes.get('id_ranges'), gam_chunk_file_ids, gam_chunk_running_times, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
    ...
    if surject:
        ...
        bam_chrom_ids = [
         zip_job.addFollowOnJobFn(run_whole_surject, context, zip_job.rv(), sample_name + '-surject', interleaved_surject, xg_id, []).rv()]
...
def run_chunk_alignment(job, context, gam_input_reads, bam_input_reads, sample_name, interleaved, multipath, chunk_filename_ids, chunk_id, indexes, bam_output=False, gbwt_penalty=None, always_check_population=True):
    """
    ...
    change mapping behavior.
    """
    RealtimeLogger.info(('Starting {}alignment on {} chunk {}').format('multipath ' if multipath else '', sample_name, chunk_id))
    run_time = None
    work_dir = job.fileStore.getLocalTempDir()
    ...
    with open(output_file, 'w') as (alignment_file):
        ...
        if bam_output is True and '--surject-to' not in vg_parts:
            ...
            if bam_output is False and '--surject-to' in vg_parts:
            ...
            RealtimeLogger.info(('Running VG for {} against {}: {}').format(sample_name, graph_file, (' ').join(vg_parts)))
            start_time = timeit.default_timer()
            command = vg_parts
            try:
    ...
    RealtimeLogger.info(('Aligned {}. Process took {} seconds with {} vg-{}').format(output_file, run_time, 'paired-end' if paired_end else 'single-end', 'mpmap' if multipath else 'map'))
...
def split_gam_into_chroms(job, work_dir, context, xg_file, id_ranges_file, gam_file):
    ...
    with open(cid_ranges_file, 'w') as (ranges):
    ...
    chunk_cmd = [
     'vg', 'chunk', '-x', os.path.basename(xg_file),
...
def run_merge_gams(job, context, sample_name, id_ranges_file_id, gam_chunk_file_ids, gam_chunk_running_times):
    ...
    for i, chr in enumerate(chroms):
        ...
        chr_gam_id = job.addChildJobFn(run_merge_chrom_gam, context, sample_name, chr, shard_ids, cores=context.config.fq_split_cores, memory=context.config.fq_split_mem, disk=context.config.fq_split_disk).rv()
    ...
    return (
     chr_gam_ids, total_running_time)
...
def map_main(context, options):
    ...
    with context.get_toil(options.jobStore) as (toil):
        if not toil.options.restart:
            ...
            root_job = Job.wrapJobFn(run_mapping, context, options.fastq, options.gam_input_reads, options.bam_input_reads, options.sample_name, options.interleaved, options.multipath, indexes, reads_file_ids=inputReadsFileIDs, bam_output=options.bam_output, surject=options.surject, cores=context.config.misc_cores, memory=context.config.misc_mem, disk=context.config.misc_disk)
