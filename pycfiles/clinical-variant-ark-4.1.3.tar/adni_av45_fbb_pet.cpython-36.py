...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_av45_fbb_pet.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 11716 bytes
...
def compute_av45_fbb_pet_paths(source_dir, csv_dir, dest_dir, subjs_list):
    ...
    for subj in subjs_list:
        ...
        if subject_pet_meta.shape[0] < 1:
            ...
            for visit in list(amyloid_qc_subj.VISCODE2.unique()):
                ...
                original_pet_meta = subject_pet_meta[((subject_pet_meta['Orig/Proc'] == 'Original') & (subject_pet_meta['Image ID'] == int_image_id) & subject_pet_meta.Sequence.map(lambda s: s.lower().find('early') < 0))]
                if original_pet_meta.shape[0] < 1:
                    original_pet_meta = subject_pet_meta[((subject_pet_meta['Orig/Proc'] == 'Original') & (subject_pet_meta['Scan Date'] == qc_visit.EXAMDATE) & subject_pet_meta.Sequence.map(lambda s: s.lower().find('early') < 0))]
                    if original_pet_meta.shape[0] < 1:
                ...
                final_pet_meta = subject_pet_meta[(subject_pet_meta.Sequence.map(lambda x: x.find('Coreg, Avg, Std Img and Vox Siz, Uniform Resolution') > 0) & (subject_pet_meta['Series ID'] == original_image['Series ID']))]
                if final_pet_meta.shape[0] < 1:
                    final_pet_meta = subject_pet_meta[(subject_pet_meta.Sequence.map(lambda x: x.find('Coreg, Avg, Std Img and Vox Siz, Uniform Resolution') > 0) & (subject_pet_meta['Scan Date'] == original_image['Scan Date']))]
                    if final_pet_meta.shape[0] < 1:
                        cprint('No "Coreg, Avg, Std Img and Vox Siz, Uniform Resolution" Amyloid PET image metadata for subject ' + subj + ' for visit ' + qc_visit.VISCODE2)
                        continue
                processed_sequence = final_pet_meta.iloc[0].Sequence
                if processed_sequence.startswith('AV45'):
                    tracer = 'AV45'
                else:
                    if processed_sequence.startswith('FBB'):
                        tracer = 'FBB'
                    else:
                        cprint('Unknown tracer for Amyloid PET image metadata for subject ' + subj + ' for visit ' + qc_visit.VISCODE2)
                        continue
                    averaged_pet_meta = subject_pet_meta[((subject_pet_meta['Sequence'] == '%s Co-registered, Averaged' % tracer) & (subject_pet_meta['Series ID'] == original_image['Series ID']))]
    ...
    for conv_error in conversion_errors:
        error_indices.append((pet_amyloid_df.Subject_ID == conv_error[0]) & (pet_amyloid_df.VISCODE == conv_error[1]))
