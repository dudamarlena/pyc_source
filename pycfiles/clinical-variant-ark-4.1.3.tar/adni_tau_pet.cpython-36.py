...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_tau_pet.py
...
def compute_tau_pet_paths(source_dir, csv_dir, dest_dir, subjs_list):
        ...
        if subject_pet_meta.shape[0] < 1:
            ...
            for visit in list(tau_qc_subj.VISCODE2.unique()):
                if str(visit) == 'nan':
                    ...
                    original_pet_meta = subject_pet_meta[((subject_pet_meta['Orig/Proc'] == 'Original') & (subject_pet_meta['Image ID'] == int_image_id))]
                    if original_pet_meta.shape[0] < 1:
                        original_pet_meta = subject_pet_meta[((subject_pet_meta['Orig/Proc'] == 'Original') & subject_pet_meta.Sequence.map(lambda x: (x.lower().find('tau') > -1) | (x.lower().find('av-1451') > -1) | (x.lower().find('av1451') > -1)) & (subject_pet_meta['Scan Date'] == qc_visit.SCANDATE))]
                        if original_pet_meta.shape[0] < 1:
                            cprint('No TAU-PET images metadata for subject - ' + subj + ' for visit ' + qc_visit.VISCODE2)
                            continue
                    original_image = original_pet_meta.iloc[0]
                    averaged_pet_meta = subject_pet_meta[((subject_pet_meta['Sequence'] == 'AV1451 Co-registered, Averaged') & (subject_pet_meta['Series ID'] == original_image['Series ID']))]
    ...
    for conv_error in conversion_errors:
        error_indices.append((pet_tau_df.Subject_ID == conv_error[0]) & (pet_tau_df.VISCODE == conv_error[1]))
