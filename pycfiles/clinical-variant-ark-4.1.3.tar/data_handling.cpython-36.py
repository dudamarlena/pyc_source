...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/utils/data_handling.py
# Compiled at: 2020-01-31 03:01:13
# Size of source mod 2**32: 47685 bytes
...
def create_merge_file(bids_dir, out_tsv, caps_dir=None, tsv_file=None, pipelines=None, **kwargs):
    ...
    while i_subject < n_sessions:
        ...
        for i_session in loc_sessions:
            ...
            if os.path.isfile(path.join(bids_dir, sub_name, 'ses-' + session_id, sub_name + '_' + 'ses-' + session_id + '_scans.tsv')):
                scans_df = pd.read_csv((path.join(bids_dir, sub_name, 'ses-' + session_id, sub_name + '_' + 'ses-' + session_id + '_scans.tsv')),
                  sep='\t')
    ...
    if caps_dir is not None:
        ...
        pipeline_options = {'t1-volume':t1_volume_pipeline, 
            ...
            if len(ses_path_avail) == 0:
                ...
                if 'func' in mods_avail_bids:
                    for m in mods_avail_dict['func']:
                        tokens = m.split('_')
                        task_name = tokens[1]
                        task_avail_list = glob(path.join(ses_path, 'func', '*' + task_name + '*'))
...
def create_subs_sess_list(input_dir, output_dir, file_name=None, is_bids_dir=True, use_session_tsv=False):
    """
...
def center_nifti_origin(input_image, output_image):
    ...
    if not error_str:
        ...
        if not isfile(output_image):
            error_str = Fore.RED + '[Error] NIfTI file created but Clinica could not save it to ' + output_image + '. Please check that the output folder has the correct permissions.' + Fore.RESET
    return (
     output_image, error_str)
...
def center_all_nifti(bids_dir, output_dir, modality, center_all_files=False):
    ...
    if bids_dir == output_dir:
        ...
        raise AssertionError('modality arg must be a list of str')
    ...
    for f in listdir(bids_dir):
        if isdir(join(bids_dir, f)) and not isdir(join(output_dir, f)):
            ...
            if isfile(join(bids_dir, f)) and not isfile(join(output_dir, f)):
                copy2(join(bids_dir, f), output_dir)

    pattern = join(output_dir, '**/*.nii*')
    nifti_files = glob(pattern, recursive=True)
    nifti_files_filtered = [f for f in nifti_files if any(elem.lower() in basename(f).lower() for elem in modality)]
    ...
    if len(all_errors) > 0:
        final_error_msg = Fore.RED + '[Error] Clinica encoutered ' + str(len(all_errors)) + ' error(s) while trying to center all NIfTI images.\n'
...
def write_list_of_files(file_list, output_file):
    ...
    if not isinstance(file_list, list):
        ...
        raise AssertionError('Second argument must be a str')
...
def check_relative_volume_location_in_world_coordinate_system(label_1, nifti_list1, label_2, nifti_list2, bids_dir, modality):
    """
    ...
    if len(pairs_with_problems) > 0:
        warning_message = Fore.YELLOW + '[Warning] It appears that ' + str(len(pairs_with_problems)) + ' pairs of files' + ' have an important relative offset. SPM coregistration has a high probability to fail ' + 'on these files:\n\n'
        file1_width = max(3 + len(label_1), 3 + max(len(basename(file)) for file in [nifti_list1[k] for k in pairs_with_problems]))
        file2_width = max(3 + len(label_2), 3 + max(len(basename(file)) for file in [nifti_list2[k] for k in pairs_with_problems]))
        norm_width = len('Relative distance')
        warning_message += ('%-' + str(file1_width) + 's%-' + str(file2_width) + 's%-' + str(norm_width) + 's') % (label_1,
         label_2,
         'Relative distance')
        warning_message += '\n' + '-' * (file1_width + file2_width + norm_width) + '\n'
        for file1, file2, norm in zip([nifti_list1[k] for k in pairs_with_problems], [nifti_list2[k] for k in pairs_with_problems], [l2_norm[k] for k in pairs_with_problems]):
            warning_message += ('%-' + str(file1_width) + 's%-' + str(file2_width) + 's%-' + str(norm_width) + '.2f\n') % (str(basename(file1)),
             str(basename(file2)),
             norm)

        warning_message += '\nClinica provides a tool to counter this problem by replacing the center of the volume at the origin of the world coordinates.\nUse the following command line to correct the header of the faulty NIFTI volumes in a new folder:\n' + Fore.RESET + Fore.BLUE + '\nclinica iotools center-nifti ' + abspath(bids_dir) + ' ' + abspath(bids_dir) + '_centered --modality "' + modality + '"\n\n' + Fore.YELLOW + 'You will find more information on the command by typing ' + Fore.BLUE + 'clinica iotools center-nifti' + Fore.YELLOW + ' in the console.\nDo you still want to ' + 'launch the pipeline now?' + Fore.RESET
...
def check_volume_location_in_world_coordinate_system(nifti_list, bids_dir, modality='t1w'):
    ...
    if len(list_non_centered_files) > 0:
        ...
        center_width = max(len('Coordinate of center') + 3, 3 + max(len(str(center)) for center in centers))
        warning_message = Fore.YELLOW + '[Warning] It appears that ' + str(len(list_non_centered_files)) + ' files ' + 'have a center way out of the origin of the world coordinate system. SPM has a high prob' + 'ability to fail on these files (for coregistration or segmentation):\n\n'
        ...
        warning_message += '\nClinica provides a tool to counter this problem by replacing the center of the volume at the origin of the world coordinates.\nUse the following command line to correct the header of the faulty NIFTI volumes in a new folder:\n' + cmd_line + 'You will find more information on the command by typing ' + Fore.BLUE + 'clinica iotools center-nifti' + Fore.YELLOW + ' in the console.\nDo you still want to ' + 'launch the pipeline now?' + Fore.RESET
        cprint(warning_message)
        while 1:
            cprint('Your answer [yes/no]:')
            answer = input()
...
def get_world_coordinate_of_center(nii_volume):
    ...
    from colorama import Fore
    import numpy as np
    if not isinstance(nii_volume, str):
        raise AssertionError('input argument nii_volume must be a str')
    elif not isfile(nii_volume):
        raise AssertionError('input argument must be a path to a file')
    try:
        orig_nifti = nib.load(nii_volume)
    except nib.filebasedimages.ImageFileError:
        print(Fore.RED + '[Error] ' + nii_volume + ' could not be read by nibabel. Is it a valid NIfTI file ?' + Fore.RESET)
        return np.nan
    else:
...
def vox_to_world_space_method_1(coordinates_vol, header):
    ...
    """
    import numpy as np
    return np.array(coordinates_vol) * np.array(header['pixdim'][1], header['pixdim'][2], header['pixdim'][3])
