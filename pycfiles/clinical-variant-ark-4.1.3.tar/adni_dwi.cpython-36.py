...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_dwi.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 15295 bytes
...
def compute_dwi_paths(source_dir, csv_dir, dest_dir, subjs_list):
    ...
    for conv_error in conversion_errors:
        error_indices.append((dwi_df.Subject_ID == conv_error[0]) & (dwi_df.VISCODE == conv_error[1]))
...
def dwi_paths_to_bids(images, dest_dir, mod_to_update=False):
    ...
    counter = Value('i', 0)
    total = len(images)
    partial_generate_subject_files = partial(generate_subject_files, images=images,
      dest_dir=dest_dir,
...
def generate_subject_files(subj, images, dest_dir, mod_to_update):
    import clinica.iotools.bids_utils as bids, clinica.iotools.converters.adni_to_bids.adni_utils as adni_utils
    from clinica.utils.stream import cprint
    ...
    for ses in sess_list:
        with counter.get_lock():
            counter.value += 1
        cprint('[DWI] Processing subject ' + str(subj) + ' - session ' + ses + ', ' + str(counter.value) + ' / ' + str(len(images)))
                ...
                if dwi_subj['Path'] != '':
                    if not os.path.exists(path.join(ses_path, 'dwi')):
                        ...
                        if os.path.exists(adc_image):
                            os.remove(adc_image)
                        if not os.path.exists(path.join(bids_dest_dir, bids_name + '.nii.gz')) or not os.path.exists(path.join(bids_dest_dir, bids_name + '.bvec') or not os.path.exists(path.join(bids_dest_dir, bids_name + '.bval'))):
                            ...
                            if os.path.exists(bvec_file) and os.path.exists(bval_file):
                                ...
                                cprint('WARNING: bvec and bval not generated by dcm2nii' + ' for subject ' + subj + ' and session ' + ses)
                    if os.path.exists(nii_file):
                        os.rename(nii_file, path.join(bids_dest_dir, bids_name + '.nii.gz'))
                    else:
                        cprint('WARNING: CONVERSION FAILED...' + ' for subject ' + subj + ' and session ' + ses)
