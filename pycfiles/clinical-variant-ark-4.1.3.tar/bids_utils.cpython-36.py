...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/bids_utils.py
# Compiled at: 2019-10-10 04:46:11
# Size of source mod 2**32: 39358 bytes
            ...
            if location == prev_location and sheet == prev_sheet:
                ...
                for j in range(0, len(file_to_read)):
                    ...
                    if participant_fields_bids[i] == 'alternative_id_1':
                        if value_to_read.dtype == np.float64 or value_to_read.dtype == np.int64:
            ...
            for r in range(0, len(file_to_read.values)):
                ...
                if len(subj_bids) == 0:
                    ...
                    if subj_bids in sessions_dict:
                        ...
                        sessions_dict.update({subj_bids: {'M00': {'session_id': 'ses-M00', sessions_fields_bids[i]: row[sessions_fields[i]]}}})
    ...
    for i in range(0, len(fields_dataset)):
        ...
        for bids_id in bids_ids:
            ...
            if len(row_to_extract) > 0:
                ...
                scans_dict[bids_id][fields_mod[i]].update({fields_bids[i]: file_to_read.iloc[row_to_extract][fields_dataset[i]]})
            else:
...
def get_supported_dataset():
    ...
    """
    return [
     'ADNI', 'CLINAD', 'PREVDEMALS', 'INSIGHT', 'OASIS', 'AIBL']
...
def has_fixed(subj_id, ses, mod, special_list):
    ...
    """
    if subj_id in special_list and mod in special_list[subj_id]:
...
def convert_fieldmap(folder_input, folder_output, name, fixed_file=[
 False, False]):
    """
    ...
    if len(map_ph) == 0:
        ...
        if len(map_ph) > 0:
            ...
            if map_ph_name not in files_to_skip:
                if map_name not in files_to_skip:
                    ...
                    if dim_map_ph == 2:
                        if dim_map == 2:
                            ...
                            os.rename(path.join(folder_output, bids_name_mag + '0000.nii.gz'), path.join(folder_output, bids_name_mag + '1.nii.gz'))
                            os.rename(path.join(folder_output, bids_name_mag + '0001.nii.gz'), path.join(folder_output, bids_name_mag + '2.nii.gz'))
                            os.rename(path.join(folder_output, bids_name_ph + '0000.nii.gz'), path.join(folder_output, bids_name_ph + '1.nii.gz'))
                            os.rename(path.join(folder_output, bids_name_ph + '0001.nii.gz'), path.join(folder_output, bids_name_ph + '2.nii.gz'))
        else:
            if mag_missing:
                if map_ph_missing:
                    return -1
