...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_flair.py
...
def compute_flair_paths(source_dir, csv_dir, dest_dir, subjs_list):
    ...
    for conv_error in conversion_errors:
        error_indices.append((flair_df.Subject_ID == conv_error[0]) & (flair_df.VISCODE == conv_error[1]))
...
def flair_paths_to_bids(images, dest_dir, mod_to_update=False):
    ...
    subjs_list = [sub for sub in images['Subject_ID'].unique() if sub == sub if 'S' in sub]
    counter = Value('i', 0)
...
def generate_subject_files(subj, images, dest_dir, mod_to_update):
    ...
    for ses in sess_list:
        with counter.get_lock():
            counter.value += 1
        cprint('[FLAIR] Processing subject ' + str(subj) + ' - session ' + ses + ', ' + str(counter.value) + ' / ' + str(len(images)))
        ...
        for j in range(len(flair_info)):
            ...
            if type(flair_subj['Path']) != float:
                if flair_subj['Path'] != '':
                    ...
                    os.path.exists(path.join(bids_dest_dir, bids_name + '.nii.gz')) or cprint('\tConversion with dcm2niix failed, trying with dcm2nii')
