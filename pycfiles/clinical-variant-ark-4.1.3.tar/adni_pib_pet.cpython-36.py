...
# Embedded file name: /Users/mdiazmel/code/aramis/clinica/clinica/iotools/converters/adni_to_bids/adni_modalities/adni_pib_pet.py
...
def compute_pib_pet_paths(source_dir, csv_dir, dest_dir, subjs_list):
    ...
    from clinica.utils.stream import cprint
    pet_pib_col = [
     'Phase', 'Subject_ID', 'VISCODE', 'Visit', 'Sequence', 'Scan_Date', 'Study_ID',
        ...
        if subject_pet_meta.shape[0] < 1:
            ...
            for visit in list(pet_qc_subj.VISCODE.unique()):
                ...
                if original_pet_meta.shape[0] < 1:
                    original_pet_meta = subject_pet_meta[((subject_pet_meta['Orig/Proc'] == 'Original') & subject_pet_meta.Sequence.map(lambda x: x.lower().find('pib') > -1) & (subject_pet_meta['Scan Date'] == qc_visit.EXAMDATE))]
                    if original_pet_meta.shape[0] < 1:
                        cprint('No PIB-PET images metadata for subject - ' + subj + ' for visit ' + qc_visit.VISCODE)
                        continue
                original_image = original_pet_meta.iloc[0]
                averaged_pet_meta = subject_pet_meta[((subject_pet_meta['Sequence'] == 'PIB Co-registered, Averaged') & (subject_pet_meta['Series ID'] == original_image['Series ID']))]
    ...
    for conv_error in conversion_errors:
        error_indices.append((pet_pib_df.Subject_ID == conv_error[0]) & (pet_pib_df.VISCODE == conv_error[1]))
