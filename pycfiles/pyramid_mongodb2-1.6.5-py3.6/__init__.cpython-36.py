...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pyramid_mongodb2\__init__.py
# Compiled at: 2019-12-28 08:58:06
...
def includeme(config):
    debug = 'pyramid_mongodb2:MongoToolbar' in config.registry.settings.get('debugtoolbar.includes', '') and 'pyramid_debugtoolbar' in config.registry.settings.get('pyramid.includes', '')
    ...
    mongo_client = MongoClient(db_url, serverSelectionTimeoutMS=timeout,
    ...
    for db_name in mongo_dbs.splitlines():
        if '=' in db_name or ':' in db_name:
            db_name, nice_name = (x.strip() for x in re.split('[=:]', db_name))
        else:
            nice_name = db_name
