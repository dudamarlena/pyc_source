...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cookiecutter/hooks/post_gen_project.py
...
if __name__ == '__main__':
    ...
    for path in (Path('HISTORY.rst'),
     ...
     Path('docs') / 'conf.py'):
        ...
        tox_envlist = []
        for py, on in (('py26', '{{ cookiecutter.py26 }}'), ('py27', '{{ cookiecutter.py27 }}'),
                       ('py33', '{{ cookiecutter.py33 }}'), ('py34', '{{ cookiecutter.py34 }}'),
                       ('py35', '{{ cookiecutter.py35 }}'), ('py36', '{{ cookiecutter.py36 }}'),
                       ('py37', '{{ cookiecutter.py37 }}'), ('pypy', '{{ cookiecutter.pypy }}'),
                       ('pypy3', '{{ cookiecutter.pypy3 }}')):
            if on == 'yes':
                tox_envlist.append(py)
            replace_contents(Path('tox.ini'), '@TOX_ENVLIST@', ', '.join(tox_envlist))
            ...
            for f in Path('licenses').iterdir():
                ...
                if '{{ cookiecutter.src_dir }}' != '.':
                    ...
                    copytree('{{ cookiecutter.py_module }}', str(src_d / '{{ cookiecutter.py_module }}'))
