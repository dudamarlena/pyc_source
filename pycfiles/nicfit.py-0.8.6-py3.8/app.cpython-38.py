...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nicfit/app.py
# Compiled at: 2020-04-04 15:31:52
...
class Application:
    ...
    def __init__(self, main_func=None, *, name=None, description=None, logging_args=True, config_opts=None, version=None, atexit=None, pdb_opt=True, extra_arg_parser_opts=None, gettext_domain=None):
        ...
        parser = ArgumentParser(add_log_args=logging_args, config_opts=config_opts, 
         prog=name, 
         description=description, **extra_arg_parser_opts)
        self.name = parser.prog
        if version:
            parser.add_argument('--version', action='version', version=version)
        if pdb_opt:
            parser.add_argument('--pdb', action='store_true', dest='debug_pdb', help="Drop into 'pdb' for unhandled exceptions")
        ...
        if gettext_domain:
            ...
            if translation is None:
                raise ValueError("Unable to find gettext message catalog for gettext_domain='{}'".format(gettext_domain))
...
class AsyncApplication(Application):
    __doc__ = 'A base for asynchronous apps.\n    See :mod:`nicfit.aio.Application`.\n    '
