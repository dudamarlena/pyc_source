...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/dccs/maya/managers/dependencies.py
# Compiled at: 2020-03-08 12:27:22
# Size of source mod 2**32: 8669 bytes
"""
        ...
        def on_set_attr(self, name, value, attr_type):
            if name == 'cfn':
                ...
                if value not in self._gpu_cache_paths:
                    if os.path.isfile(value) or value not in self._invalid_paths:
                ...
                if name == 'fn':
                    value = artellapipe.FilesMgr().fix_path(value)
                    if value not in self._alembic_paths and (os.path.isfile(value) or value not in self._invalid_paths):
                        self._invalid_paths.append(value)
                else:
...
class ArtellaMayaDependenciesManager(dependencies.ArtellaDependenciesManager, object):
    ...
    def get_dependencies(self, file_path, parent_path=None, found_files=None):
        ...
        if parent_path:
            ...
            file_path = os.path.isfile(file_path) or artellapipe.FilesMgr().fix_path(file_path)
            ...
            with open(file_path, 'r') as (open_file):
                ...
                for ref in parser.references:
                    ...
                    self.get_dependencies(file_path=ref,
                      parent_path=file_path,
                      found_files=found_files)

            return (
             found_files, invalid_paths)
    ...
    def fix_file_paths(self, file_path, invalid_paths=None):
        ...
        if not self._project:
            LOGGER.warning('Impossible to fix file paths from "{}" because project is not defined!'.format(file_path))
            return
        else:
                            ...
                            for line in lines:
                                for invalid_path in invalid_paths:
                                    if invalid_path in line:
                                        ...
                                        split_path or LOGGER.warning('Impossible to fix path "{}" because is not in project folder!'.format(invalid_path))
                                        continue
