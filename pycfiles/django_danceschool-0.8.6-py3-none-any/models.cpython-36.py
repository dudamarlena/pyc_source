...
# [GCC 8.4.0]
# Embedded file name: /c/Users/Lee/Sync/projects/django-danceschool/currentmaster/django-danceschool/danceschool/vouchers/models.py
# Compiled at: 2019-04-03 22:56:33
# Size of source mod 2**32: 13941 bytes
...
class Voucher(models.Model):
    ...
    def getHasExpired(self):
        if self.expirationDate:
            if timezone.now() > self.expirationDate:
                return True
        return False
    ...
    def getIsValidForAnyCustomer(self):
        isvalid = len(CustomerVoucher.objects.filter(voucher=self)) + len(CustomerGroupVoucher.objects.filter(voucher=self)) == 0
        return isvalid
            ...
            if len(VoucherUse.objects.filter(voucher=self)) > 0:
                ...
                if not self.isValidForAnyCustomer:
                    ...
                    if cvs.exists() and not cvs.filter(customer=customer).exists():
                        ...
                        if not cgvs.filter(group__in=(customer.groups.all())).exists():
                            raise ValidationError(_('This voucher is associated with a specific customer group.'))
        else:
            if self.forFirstTimeCustomersOnly:
                if customer:
                    if customer.numClassSeries > 0:
                        raise ValidationError(_('This voucher can only be used by first time customers.'))
            if self.forPreviousCustomersOnly:
                if not customer or customer.numClassSeries == 0:
