...
# [GCC 8.4.0]
# Embedded file name: /c/Users/Lee/Sync/projects/django-danceschool/currentmaster/django-danceschool/danceschool/core/management/commands/setupschool.py
# Compiled at: 2019-02-24 22:45:17
# Size of source mod 2**32: 27849 bytes
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        current_domain = self.pattern_input("Enter the domain name of this installation, with no protocol or trailing slashes (e.g. 'bostonlindyhop.com') [localhost:8000]",
        ...
        this_site.save()
        school_name = self.pattern_input('Give your dance school a name [Dance School]',
        ...
        school_email = self.pattern_input('Provide a default school email address',
        ...
        school_city = self.pattern_input('Enter school city [Boston]',
          message='Invalid city (no special characters allowed)',
          pattern='^[a-zA-Z0-9 ]+$',
          default='Boston')
        prefs['contact__businessCity'] = school_city
        school_state = self.pattern_input('Enter school state [MA]',
        ...
        school_country = self.pattern_input('Enter two-letter school country code [US]',
          message='Invalid state (only two uppercase letters allowed)',
          pattern='^[A-Z][A-Z]$',
          default='US')
        prefs['contact__businessCountryCode'] = school_country
        currency_code = self.pattern_input('Enter three-letter currency code for transactions [USD]',
        ...
        error_email_address = self.pattern_input(('Email address to send error messages to [%s]' % school_email),
          message='Invalid email address.',
          pattern='^[^@]+@[^@]+\\.[^@]+$',
        ...
        initial_dancetype = self.pattern_input('Name of your initial default dance type [Lindy Hop]',
        ...
        initial_pricing_tier_name = self.pattern_input('Give your initial pricing tier a name [Default Pricing]',
          message='Invalid dance type name (no special characters)',
        ...
        PricingTier.objects.get_or_create(name=initial_pricing_tier_name,
          defaults={'onlinePrice':initial_online_price, 
        ...
        if not sp_public.cmsplugin_set.all():
            address_string = '{}{}{}, {} {}<br />'.format(school_address1 + '<br />\n' if school_address1 else '', school_address2 + '<br />\n' if school_address2 else '', school_city, school_state, school_postal)
        ...
        if registration_first:
            home_page = create_page('Registration',
              'cms/home.html', initial_language, menu_title='Registration', apphook='RegistrationApphook',
              in_navigation=True,
              published=True)
        ...
        if add_registration_link:
            registration_link_page = create_page('Registration',
              'cms/home.html', initial_language, menu_title='Register',
              slug='register',
              overwrite_url=(reverse('registration')),
              in_navigation=True,
              published=True)
        ...
        if add_instructor_page:
            ...
            add_plugin(content_placeholder,
              'StaffMemberListPlugin', initial_language, orderChoice=(StaffMemberListPluginModel.OrderChoices.random),
              bioRequired=True,
              template='core/staff_list.html')
            add_plugin(sidebar_placeholder,
              'StaffMemberListPlugin', initial_language, orderChoice=(StaffMemberListPluginModel.OrderChoices.random),
              photoRequired=True,
        ...
        if apps.is_installed('danceschool.private_lessons'):
            if allow_public_privatelesson_booking:
                ...
                if add_privatelesson_link:
                    privatelesson_link_page = create_page('Schedule Private Lessons',
                      'cms/home.html', initial_language, menu_title='Private Lessons',
        ...
        if apps.is_installed('danceschool.news'):
            ...
            if add_news_page:
                ...
                create_page('Latest News',
                  'cms/twocolumn_rightsidebar.html', initial_language, menu_title='News',
                  apphook='NewsApphook',
                  in_navigation=True,
                  published=True)
        ...
        if apps.is_installed('danceschool.stats'):
            ...
            if add_stats_page:
                stats_page = create_page('School Performance Stats',
                  'cms/admin_home.html', initial_language, menu_title='Stats',
                  slug='stats',
                  apphook='StatsApphook',
                  in_navigation=False,
                  published=False)
        ...
        if add_login_link:
            create_page('Login',
              'cms/home.html', initial_language, menu_title='Login',
              slug='login',
              overwrite_url=(reverse('account_login')),
              in_navigation=True,
              limit_visibility_in_menu=VISIBILITY_ANONYMOUS,
              published=True)
            self.stdout.write('Login link added.\n')
            create_page('My Account',
              'cms/home.html', initial_language, menu_title='My Account',
              slug='profile',
              overwrite_url=(reverse('accountProfile')),
              in_navigation=True,
              limit_visibility_in_menu=VISIBILITY_USERS,
              published=True)
            self.stdout.write("'My Account' link added.\n")
            create_page('Logout',
              'cms/home.html', initial_language, menu_title='Logout',
              slug='logout',
              overwrite_url=(reverse('account_logout')),
              in_navigation=True,
              limit_visibility_in_menu=VISIBILITY_USERS,
              published=True)
