...
# [GCC 8.4.0]
# Embedded file name: /c/Users/Lee/Sync/projects/django-danceschool/currentmaster/django-danceschool/danceschool/vouchers/dynamic_preferences_registry.py
...
class GiftCertTemplate(ModelChoicePreference):
    ...
    def get_default(self):
        ...
        with open(initial_template.origin.name, 'r') as (infile):
            content = infile.read()
            infile.close()
        return EmailTemplate.objects.get_or_create(name=(_('Gift Certificate Purchase Confirmation Email')),
          defaults={'subject':_('Gift Certificate Purchase Confirmation'), 
         'content':content, 
...
class GiftCertPDFTemplate(ModelChoicePreference):
    ...
    def get_default(self):
        ...
        with open(initial_template.origin.name, 'r') as (infile):
            content = infile.read()
            infile.close()
        return EmailTemplate.objects.get_or_create(name=(_('Gift Certificate Purchase PDF Text')),
          defaults={'subject':_("You've Been Given the Gift of Dance!"), 
         'content':content, 
         'defaultFromAddress':get_defaultEmailFrom(), 
         'defaultFromName':get_defaultEmailName(), 
         'defaultCC':'', 
