...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/django-danceschool/currentmaster/django-danceschool/danceschool/vouchers/helpers.py
...
def createReferreeVoucher(name, amountPerUse):
    ...
    expirationDate = None
    disabled = False
    voucher = Voucher(voucherId=voucherId, name=name, category=category, originalAmount=originalAmount, maxAmountPerUse=maxAmountPerUse, singleUse=singleUse, forFirstTimeCustomersOnly=forFirstTimeCustomersOnly, expirationDate=expirationDate, disabled=disabled)
    voucher.save()
...
def ensureReferralVouchersExist(customer):
    ...
    if vrd:
        ...
        vrd = VoucherReferralDiscount.objects.get_or_create(referrerVoucher=referrerVoucher, referreeVoucher=referreeVoucher, referrerBonus=referrerDiscount)
    return vrd
...
def awardReferrers(voucherUse):
    ...
    for rd in rds:
        vc = VoucherCredit(voucher=rd.referrerVoucher, amount=rd.referrerBonus, description=_('Referral from ' + str(rd.referreeVoucher)))
        vc.save()
        vrdu = VoucherReferralDiscountUse(voucherReferralDiscount=rd, voucherUse=voucherUse, voucherCredit=vc)
        vrdu.save()