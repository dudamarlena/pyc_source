...
# [GCC 8.4.0]
# Embedded file name: /c/Users/Lee/Sync/projects/django-danceschool/currentmaster/django-danceschool/danceschool/financial/migrations/0012_auto_20181216_1615.py
# Compiled at: 2019-04-03 22:56:30
# Size of source mod 2**32: 6595 bytes
...
def getFullName(obj):
    ...
    in the migration.
    """
    if hasattr(obj, 'firstName'):
        if hasattr(obj, 'lastName'):
...
def createPartyFromName(apps, name):
    ...
    members = StaffMember.objects.filter(firstName__istartswith=firstName,
      lastName__istartswith=lastName)
    users = User.objects.filter(first_name__istartswith=firstName,
      last_name__istartswith=lastName)
    if members.count() == 1:
        this_member = members.first()
        party = TransactionParty.objects.get_or_create(staffMember=this_member,
          defaults={'name':getFullName(this_member), 
         'staffMember':this_member.userAccount})[0]
    else:
        if users.count() == 1:
            this_user = users.first()
            party = TransactionParty.objects.get_or_create(user=this_user,
              defaults={'name':getFullName(this_user), 
             'staffMember':getattr(this_user, 'staffmember', None)})[0]
        else:
            party = TransactionParty.objects.get_or_create(name=name)[0]
    return party
...
def update_payTo(apps, schema_editor):
    ...
    for item in chain(ExpenseItem.objects.filter(Q(payToUser__isnull=False) | Q(payToLocation__isnull=False) | Q(payToName__isnull=False)), GenericRepeatedExpense.objects.filter(Q(payToUser__isnull=False) | Q(payToLocation__isnull=False) | Q(payToName__isnull=False))):
        if getattr(item, 'payToUser', None):
            party = TransactionParty.objects.get_or_create(user=(item.payToUser),
              defaults={'name':getFullName(item.payToUser), 
             'staffMember':getattr(item.payToUser, 'staffmember', None)})[0]
        else:
            if getattr(item, 'payToLocation', None):
                party = TransactionParty.objects.get_or_create(location=(item.payToLocation),
                  defaults={'name': item.payToLocation.name})[0]
            else:
                if getattr(item, 'payToName', None):
...
class Migration(migrations.Migration):
    ...
    operations = [
     migrations.CreateModel(name='TransactionParty',
       fields=[
      (
      ...
      (
       ...
       options={'verbose_name':'Transaction party', 
     ...
     migrations.AddField(model_name='genericrepeatedexpense',
       name='payTo',
       field=models.ForeignKey(null=True, on_delete=(django.db.models.deletion.SET_NULL), to='financial.TransactionParty', verbose_name='Pay to')),
