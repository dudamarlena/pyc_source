...
# [GCC 8.4.0]
# Embedded file name: /c/Users/Lee/Sync/projects/django-danceschool/currentmaster/django-danceschool/danceschool/core/mixins.py
# Compiled at: 2019-04-09 02:10:05
...
class EmailRecipientMixin(object):
    ...
    def email_recipient(self, subject, content, **kwargs):
        ...
        if kwargs.pop('send_html', False):
            if kwargs.get('html_message'):
                email_kwargs['html_content'] = render_to_string('email/html_email_base.html',
                  context={'html_content':kwargs.get('html_message'), 
                 'subject':subject})
...
class GroupRequiredByFieldMixin(GroupRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not required_group or required_group == ['']:
            ...
            if in_group or self.raise_exception:
...
class PluginTemplateMixin(object):
    ...
    def templateChoiceFormFactory(self, request, choices):
        ...
        class PluginTemplateChoiceForm(ModelForm):
            ...
            def __init__(self, *args, **kwargs):
                ...
                all_choices = choices
                if self.instance:
                    if self.instance.template not in [x[0] for x in choices]:
                        all_choices = [
                         (
                          self.instance.template, self.instance.template)] + all_choices
                elif self.request and self.request.user.has_perm('core.choose_custom_plugin_template'):
            ...
            def _media(self):
                ...
                if self.request:
                    if self.request.user.has_perm('core.choose_custom_plugin_template'):
                        return Media(css={'all': ('autocomplete_light/select2.css', )},
    ...
    def get_template_choices(self):
        if hasattr(self, 'template_choices'):
            if self.template_choices:
...
class EventOrderMixin(object):
    ...
    def get_annotations(self):
        ...
        annotations = {'nullParam':Case(default_value=None, output_field=IntegerField()), 
        ...
        if rule == 'SessionFirst':
            annotations.update({'nullParam':Case(When(session__startTime__isnull=False, then=0),
            ...
            if rule == 'SessionAlphaFirst':
                annotations.update({'nullParam':Case(When(session__name__isnull=False, then=0),
                   When(month__isnull=False, then=1),
                   default_value=2,
                   output_field=IntegerField()), 
                ...
                if rule == 'Month':
                    annotations.update({'nullParam':Case(When(month__isnull=False, then=0),
                       default_value=1,
                       output_field=IntegerField()), 
                    ...
                    if rule == 'Session':
                        ...
                        if rule == 'SessionAlpha':
                            annotations.update({'nullParam':Case(When(session__name__isnull=False, then=0),
                               default_value=1,
                               output_field=IntegerField()), 
                             'paramOne':F('session__name')})
                        else:
                            if rule == 'SessionMonth':
                                annotations.update({'nullParam':Case(When((Q(session__startTime__isnull=False) & Q(month__isnull=False)), then=0),
                                ...
                                if rule == 'SessionAlphaMonth':
                                    annotations.update({'nullParam':Case(When((Q(session__name__isnull=False) & Q(month__isnull=False)), then=0),
                                    ...
                                    if rule == 'Weekday':
                                        annotations.update({'nullParam':Case(When(startTime__week_day__isnull=False, then=0),
                                           default_value=1,
                                           output_field=IntegerField()), 
                                         'paramOne':ExtractWeekDay('startTime')})
                                    else:
                                        if rule == 'MonthWeekday':
                                            annotations.update({'nullParam':Case(When((Q(month__isnull=False) & Q(startTime__week_day__isnull=False)), then=0),
                                               default_value=1,
                                               output_field=IntegerField()), 
                                             'paramOne':ExpressionWrapper(12 * F('year') + F('month'), output_field=IntegerField()), 
...
class SiteHistoryMixin(object):
    ...
    def set_return_page(self, namedUrl, pageName='', **kwargs):
        ...
        if list(siteHistory.get('returnPage', [])) != list(updates['returnPage']):
            updates.update({'priorPage':siteHistory.get('returnPage', (None, {})), 
