...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/perseuspy/dependent_peptides.py
...
import numpy as np
_index_columns = [
 'DP Proteins', 'DP Base Sequence', 'DP Cluster Index', 'DP Modification']
...
def read_dependent_peptides(filename):
    ...
    """
    df = pd.read_perseus(filename, usecols=_cols).dropna(subset=[
     'DP Ratio mod/base'])
    df['DP Ratio mod/base'] = df['DP Ratio mod/base'].astype(float)
    dep = df.pivot_table('DP Ratio mod/base', index=_index_columns, columns='Raw file', aggfunc=np.median)
