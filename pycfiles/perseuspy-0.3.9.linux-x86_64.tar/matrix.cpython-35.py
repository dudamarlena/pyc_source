...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/perseuspy/io/perseus/matrix.py
# Compiled at: 2019-11-19 08:17:46
...
def annotation_rows(prefix, annotations):
    ...
    return {name.replace(prefix, '', 1):values + [''] * (ncol - len(values)) for name, values in annotations.items() if name.startswith(prefix)}
...
def to_perseus(df, path_or_file, main_columns=None, separator=separator, convert_bool_to_category=True, numerical_annotation_rows=set([])):
    """
    ...
    annotations['Type'] = ['E' if column_names[i] in main_columns else dtype_to_perseus(dtype) for i, dtype in enumerate(_df.dtypes)]
