...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/perseuspy/test/test_io.py
...
class TestReading(TestCase):
    ...
    def test_reading_example3(self):
        ...
        self.assertIsNot(df, None)
        self.assertIsNot(to_string(df), '')
        self.assertEqual('#!{Type}E\tE\tE\tC', '\t'.join(type_row(df).split('\t')[:4]))

    def test_reading_numerical_annotation_row(self):
        df = pd.read_perseus(path.join(TEST_DIR, 'matrix5.txt'))
        self.assertIsNot(df, None)
        self.assertIsNot(to_string(df), '')
        self.assertEqual('#!{C:Quantity1}1.0\t2.0\t3.0\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan\tnan', to_string(df).splitlines()[2])
