...
# [GCC 8.4.0]
# Embedded file name: /Users/jacob/Projects/python-hdcloud/tests/fakeserver.py
# Compiled at: 2010-04-16 11:40:58
"""
...
class FakeClient(HDCloudClient):
    ...
    def get_encoding_profiles(self, **kw):
        return (
         200,
         {'encoding_profiles': [
                                {'id': 1, 'encoded_filename_format_string': '%f_%p_%b_%wx%h.%e', 
    ...
    def get_encoding_profiles_1(self, **kw):
        return (
         200, {'encoding_profile': self.get_encoding_profiles(**kw)[1]['encoding_profiles'][0]})

    def get_jobs(self, **kw):
        return (
         200,
         [
          {'job': {'current_step': 'encoding', 'source_filename': 'beer-drinking-pig.mpg', 
    ...
    def get_jobs_1(self, **kw):
        return (
         200, self.get_jobs(**kw)[1][0])

    def get_jobs_1_frames(self, **kw):
        return (
         200,
         {'frames': {'filenames': ['one.jpg', 'two.jpg'], 'page': 1, 
                       'per_page': 15, 
                       'total_entries': 0, 
                       'total_pages': 0}})
    ...
    def get_jobs_completed(self, **kw):
        return (
         200,
         [
          {'job': {'current_step': None, 'source_filename': 'beer-drinking-pig.mpg', 
                     'encoding_profile_id': None, 
                     'resolution': None, 
                     'status_url': 'http://example.com/api/v1/jobs/1.json', 
                     'id': 1, 
    ...
    def get_jobs_failed(self, **kw):
        return (
         200,
         [
          {'job': {'current_step': 'failed', 'source_filename': 'beer-drinking-pig.mpg', 
                     'encoding_profile_id': None, 
                     'resolution': None, 
                     'status_url': 'http://example.com/api/v1/jobs/1.json', 
                     'failed?': True, 
    ...
    def post_jobs(self, body, **kw):
        ...
        required_keys = [
         'job[source_id]', 'job[destination_id]', 'files[]', 'encoding_profile_ids[]']
        for k in required_keys:
            assert_in(k, post_keys)

        optional_keys = [
         'job[priority]', 'job[use_file_cache]', 'job[callback_url]', 'job[remote_id]']
    ...
    def get_stores_1(self, **kw):
        return (
         200, {'store': self.get_stores(**kw)[1]['stores'][0]})