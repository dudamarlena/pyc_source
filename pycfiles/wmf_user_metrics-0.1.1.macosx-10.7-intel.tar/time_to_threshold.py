...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/time_to_threshold.py
...
class TimeToThreshold(um.UserMetric):
    """
        ...
        is -1.
    """
    _param_types = {'init': {'threshold_type_class': [
                                       'str', 'Type of threshold to use.',
    ...
    class EditCountThreshold:
        ...
        """
        _param_types = {'init': {'first_edit': [
                                 'int',
                                 'Event that initiates measurement period.',
                                 REGISTRATION], 
                    'threshold_edit': [
                                     'int', 'Threshold event.', 1]}, 
           'process': {}}
        ...
        def __init__(self, **kwargs):
            ...
            """
            try:
                self._first_edit_ = int(kwargs['first_edit']) if 'first_edit' in kwargs else self._param_types['init']['first_edit'][2]
                self._threshold_edit_ = int(kwargs['threshold_edit']) if 'threshold_edit' in kwargs else self._param_types['init']['threshold_edit'][2]
            except ValueError:
        ...
        def process(self, user_handle, threshold_obj, **kwargs):
            ...
            for user in user_handle:
                sql = user_revs_SQL % {'user_handle': str(user), 
...
if __name__ == '__main__':
    for i in TimeToThreshold(threshold_type_class='edit_count_threshold', first_edit=0, threshold_edit=1).process([13234584]):
        print i