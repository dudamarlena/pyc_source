...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/etl/log_parser.py
...
import sys, urlparse, re, logging, json, gzip, config.settings as projSet
...
class LineParseMethods:
    ...
    def e3_acux_log_parse_client_event(line, version=1):
        ...
        regex_str = 'ext.accountCreationUX.*@.*_%s' % version
    ...
    def e3_acux_log_parse_server_event(line, version=1):
        ...
        num_fields = len(line_bits)
        server_event_regex = 'account_create.*userbuckets.*ACUX'
        if num_fields == 1:
            line_bits = line.split()
            try:
                if re.search(server_event_regex, line):
                    ...
                    if query_vars['self_made'][0] and query_vars['?event_id'][0] == 'account_create' and str(version) in userbuckets['ACUX'][0]:
                        campaign = userbuckets['campaign'][0] if 'campaign' in userbuckets else ''
                        return [
                         line_bits[0], query_vars['username'][0], query_vars['user_id'][0],
    ...
    def e3_cta4_log_parse_client(line, version=1):
        ...
        line_bits = line.split('\t')
        num_fields = len(line_bits)
        regex_1 = 'ext.articleFeedbackv5@10-option6X-cta_signup_login-impression'
        regex_2 = 'ext.articleFeedbackv5@10-option6X-cta_signup_login-button_signup_click'
    ...
    def e3_cta4_log_parse_server(line, version=1):
        ...
        if num_fields == 1:
            ...
            query_vars = urlparse.parse_qs(line_bits[1])
            try:
                if query_vars['self_made'][0] and query_vars['?event_id'][0] == 'account_create' and re.search('userbuckets', line) and 'campaign' in json.loads(query_vars['userbuckets'][0]):
                    return [
                     line_bits[0], query_vars['username'][0], query_vars['user_id'][0],
                     query_vars['timestamp'][0], query_vars['?event_id'][0], query_vars['self_made'][0],
