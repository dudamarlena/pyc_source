...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/metrics_manager.py
# Compiled at: 2013-01-30 18:42:14
"""
...
import user_metric as um, threshold as th
...
import src.etl.data_loader as dl, src.etl.aggregator as agg, src.etl.time_series_process_methods as tspm
from config import logging
INTERVALS_PER_THREAD = 10
MAX_THREADS = 5
USER_THREADS = 100
REVISION_THREADS = 100
metric_dict = {'threshold': th.Threshold, 
   'survival': Survival, 
...
aggregator_dict = {'sum+bytes_added': agg.list_sum_indices, 
...
def process_data_request(metric_handle, users, **kwargs):
    ...
    if aggregator_func:
        if time_series:
            ...
            logging.info('Metrics Manager: Initiating time series for %(metric)s with %(agg)s from %(start)s to %(end)s.' % {'metric': metric_class.__name__, 
               'agg': aggregator_func.__name__, 
               'start': str(start), 
               'end': str(end)})
            out = tspm.build_time_series(start, end, interval, metric_class, aggregator_func, users, num_threads=time_threads, metric_threads='{"num_threads" : %(user_threads)s, "rev_threads" : %(rev_threads)s}' % {'user_threads': USER_THREADS, 'rev_threads': REVISION_THREADS}, log=True)
            count = 1
            for row in out:
                results['metric'][count] = (' ').join(to_string([row[0][:10] + 'T' + row[0][11:13]] + row[3:]))
                count += 1

        else:
            logging.info('Metrics Manager: Initiating aggregator for %(metric)s with %(agg)s from %(start)s to %(end)s.' % {'metric': metric_class.__name__, 
               'agg': aggregator_func.__name__, 
               'start': str(start), 
               'end': str(end)})
        ...
        logging.info('Metrics Manager: Initiating user data for %(metric)s from %(start)s to %(end)s.' % {'metric': metric_class.__name__, 
           'start': str(start), 
           'end': str(end)})
        metric_obj.process(users, num_threads=USER_THREADS, rev_threads=REVISION_THREADS, log_progress=True, **kwargs)
