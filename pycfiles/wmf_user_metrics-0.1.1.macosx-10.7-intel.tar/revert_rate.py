...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/revert_rate.py
# Compiled at: 2013-01-30 18:15:28
__author__ = "Ryan Faulkner (adapted from Aaron Halfaker's implementation)"
__date__ = 'October 29th, 2012'
__license__ = 'GPL (version 2 or later)'
import user_metric as um, src.etl.data_loader as dl, collections, os, src.utils.multiprocessing_wrapper as mpw
from src.etl.aggregator import decorator_builder, weighted_rate
from config import logging
RevertRateArgsClass = collections.namedtuple('RevertRateArgs', 'project log_progress look_ahead look_back date_start date_end rev_threads')
...
class RevertRate(um.UserMetric):
    ...
    REV_SHA1_IDX = 2
    REV_USER_TEXT_IDX = 1
    _param_types = {'init': {'look_ahead': [
                             'int', 'Number of revisions to look ahead when computing revert.', 15], 
                'look_back': [
                            'int', 'Number of revisions to look back when computing revert.', 15]}, 
       'process': {'log_progress': [
                                  'bool', 'Enable logging for processing.', False], 
                   'num_threads': [
                                 'int', 'Number of worker processes over users.', 1], 
                   'rev_threads': [
                                 'int', 'Number of worker processes over revisions.', 1]}}
    ...
    def header():
        return [
         'user_id', 'revert_rate', 'total_revisions']
    ...
    def process(self, user_handle, **kwargs):
        ...
        args = [
         self._project_, log_progress, self.look_ahead, self.look_back, self._start_ts_, self._end_ts_, k_r]
...
def _process_help(args):
    ...
    if thread_args.log_progress:
        logging.info(__name__ + '::PID %s complete.' % str(os.getpid()))
    return results_agg
