...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/etl/data_loader.py
# Compiled at: 2013-01-30 18:27:28
r"""
...
import sys, MySQLdb, logging, operator, config.settings as projSet
        ...
        if 'instance' in kwargs:
            ...
            while retries:
                try:
                    ...
                    logging.debug(__name__ + '::Connection dropped. Reopening MySQL connection. %s retries left' % retries)
                    retries -= 1
...
class DataLoader(object):
        ...
        if clause_type == self.AND:
            ...
            for row in elems:
                if isinstance(row[index], str):
                    ...
                    value = str(row[index])
                clause = ('').join([clause,
                 '%(field_name)s = %(value)s %(clause_op)s ' % {'field_name': field_name, 'clause_op': clause_op, 
                    'value': value}])
        ...
        if hasattr(nested_list, '__iter__'):
            for elem in nested_list:
                ...
                try:
                    ...
                    if log:
                        logging.error('Could not write: "%s"' % str(line_in.strip()))

        else:
            logging.error('Expected an iterable to write to file.')
        file_obj.close()

    def create_table_from_list(self, l, create_sql, table_name, conn=Connector(instance='slave'), max_records=10000, user_db=projSet.connections['slave']['db']):
        """
    ...
    def create_xsv_from_SQL(self, sql, conn=Connector(instance='slave'), outfile='sql_to_xsv.out', separator='\t'):
        """
    ...
    def format_condition_between(self, field_name, val_1, val_2, include_quotes=False):
        ...
        if include_quotes:
            val_1 = '"' + val_1 + '"'
            val_2 = '"' + val_2 + '"'
        ts_cond = '%(field_name)s BETWEEN %(val_1)s AND %(val_2)s' % {'field_name': field_name, 
           'val_1': val_1, 
           'val_2': val_2}
        return ts_cond