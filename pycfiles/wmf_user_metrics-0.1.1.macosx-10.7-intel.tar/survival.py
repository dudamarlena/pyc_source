...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/survival.py
...
class Survival(um.UserMetric):
    ...
    """
    _param_types = {'init': {'t': [
                    'int', 'The time in minutes registration after which survival is measured.', 24]}, 
       'process': {'log_progress': [
                                  'bool', 'Enable logging for processing.', False], 
                   'num_threads': [
                                 'int', 'Number of worker processes over users.', 0], 
                   'restrict': [
...
if __name__ == '__main__':
    for r in Survival().process([13234584, 156171], num_threads=2, log_progress=True).__iter__():
        print r