...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/etl/time_series_process_methods.py
...
import operator, json, src.metrics.revert_rate as rr, src.metrics.user_metric as um
...
def build_time_series(start, end, interval, metric, aggregator, cohort, **kwargs):
    ...
    processes = list()
    if log:
        logging.info('Spawning procs, %s - %s, interval = %s, threads = %s ... ' % (
         str(start), str(end), interval, k))
    for i in xrange(len(time_series)):
        p = Process(target=time_series_worker, args=(
...
def time_series_worker(time_series, metric, aggregator, cohort, kwargs, q):
    ...
    while 1:
        ...
        if log:
            logging.info(__name__ + ' :: Processing thread %s, %s - %s ...' % (
        ...
        if log:
            logging.info(__name__ + ' :: Processing complete %s, %s - %s ...' % (
             os.getpid(), str(ts_s), str(ts_e)))
...
if __name__ == '__main__':
    ...
    print build_time_series('20120101000000', '20120201000000', 24, metric, aggregator, cohort, num_threads=4, metric_threads='{"num_threads" : 20, "rev_threads" : 50}', log=True)