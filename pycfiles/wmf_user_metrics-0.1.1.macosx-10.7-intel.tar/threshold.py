...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/threshold.py
# Compiled at: 2013-01-30 20:47:24
__author__ = 'Ryan Faulkner'
__date__ = 'December 6th, 2012'
__license__ = 'GPL (version 2 or later)'
from datetime import timedelta
import collections, os, src.utils.multiprocessing_wrapper as mpw, user_metric as um
...
class Threshold(um.UserMetric):
    ...
    """
    _param_types = {'init': {'t': [
                    'int', 'The time in minutes until the threshold.', 24], 
                'n': [
                    'int', 'Revision threshold that is to be exceeded in time `t`.',
                    1]}, 
       'process': {'log_progress': [
                                  'bool', 'Enable logging for processing.', False], 
                   'num_threads': [
                                 'int', 'Number of worker processes over users.',
                                 1], 
                   'survival': [
                              'bool', 'Indicates whether this is to be processed as the survival metric.',
                              False], 
                   'restrict': [
                              'bool', 'Restrict threshold calculations to those users registered between `date_start` and `date_end`',
                              False]}}
    _data_model_meta = {'id_fields': [
                   0], 
       'date_fields': [], 'float_fields': [], 'integer_fields': [], 'boolean_fields': [
                        1]}
    _agg_indices = {'list_sum_indices': _data_model_meta['boolean_fields']}
    ...
    def process(self, user_handle, **kwargs):
        ...
        args = [
         self._project_, self._namespace_, self._n_,
...
def _process_help(args):
    ...
    ThresholdArgsClass = collections.namedtuple('ThresholdArgs', 'project namespace n t log_progress survival restrict ts_start ts_end')
    user_data = args[0]
    state = args[1]
    thread_args = ThresholdArgsClass(state[0], state[1], state[2], state[3], state[4], state[5], state[6], state[7], state[8])
    if thread_args.log_progress:
        logging.info(__name__ + ' :: Processing revision data ' + '(%s users) by user... (PID = %s)' % (len(user_data), os.getpid()))
    ...
    for r in user_data:
        try:
            threshold_ts = um.UserMetric._get_timestamp(um.date_parse(r[1]) + timedelta(hours=thread_args.t))
            uid = long(r[0])
            rev_query = threshold_rev_query(uid, thread_args.survival, thread_args.namespace, thread_args.project, thread_args.restrict, thread_args.ts_start, thread_args.ts_start, threshold_ts)
    ...
    if thread_args.log_progress:
        logging.info(__name__ + '::Processed PID = %s.  Dropped users = %s.' % (
...
if __name__ == '__main__':
    for r in Threshold(namespace=[0, 4]).process([13234584, 156171], num_threads=0, log_progress=True).__iter__():
        print r