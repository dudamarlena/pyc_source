...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/src/metrics/blocks.py
...
class Blocks(um.UserMetric):
    ...
    """
    _param_types = {'init': {}, 'process': {'is_id': [
                           'bool', 'Are user ids or names being passed.', True], 
                   'log_progress': [
    ...
    def header():
        return [
         'user_id', 'block_count', 'block_first', 'block_last', 'ban']
    ...
    def process(self, user_handle, **kwargs):
        ...
        if log:
            logging.info(__name__ + '::Processing blocks for %s users.' % len(user_handle))
