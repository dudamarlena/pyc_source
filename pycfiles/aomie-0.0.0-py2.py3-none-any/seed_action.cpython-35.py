...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/seed_action.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 7172 bytes
__doc__ = ' The aomi "seed" loop '
...
from aomi.model.backend import CHANGED, ADD, DEL, OVERWRITE, NOOP, CONFLICT, VaultBackend
...
def seed(vault_client, opt):
    ...
    Context.load(get_secretfile(opt), opt).fetch(vault_client).sync(vault_client, opt)
    if opt.thaw_from:
        rmtree(opt.secrets)
...
def export(vault_client, opt):
    ...
    ctx = Context.load(get_secretfile(opt), opt).fetch(vault_client)
    for resource in ctx.resources():
...
def details_dict(obj, existing, ignore_missing, opt):
    ...
    for ex_k, ex_v in iteritems(existing):
        ...
        if ex_k in obj and og_value != new_value:
            print(maybe_colored('-- %s: %s' % (ex_k, og_value), 'red', opt))
            print(maybe_colored('++ %s: %s' % (ex_k, new_value), 'green', opt))
    ...
    for ob_k, ob_v in iteritems(obj):
        ...
        if ob_k not in existing:
            print(maybe_colored('++ %s: %s' % (ob_k, val), 'green', opt))
...
def maybe_details(resource, opt):
    ...
    if is_unicode(existing) and is_unicode(obj):
        a_diff = difflib.unified_diff(existing.splitlines(), obj.splitlines(), lineterm='')
        for line in a_diff:
