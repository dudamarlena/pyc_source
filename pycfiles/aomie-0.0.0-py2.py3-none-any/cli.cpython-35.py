...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/cli.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 17683 bytes
__doc__ = ' The CLI interface for aomi '
from __future__ import print_function
import os, sys, logging
from argparse import ArgumentParser
import aomi.vault, aomi.render, aomi.template, aomi.validation, aomi.util, aomi.filez, aomi.seed_action
...
def help_me(parser, opt):
    ...
    if opt.verbose == 2:
        ...
        appre_str = 'App Role Env,' if 'VAULT_ROLE_ID' in os.environ and 'VAULT_SECRET_ID' in os.environ else ''
        appe_str = 'AppID Env,' if 'VAULT_USER_ID' in os.environ and 'VAULT_APP_ID' in os.environ else ''
        LOG.info(('Auth Hints Present : %s%s%s%s%s%s' % (
         tf_str, app_str, approle_str, tfe_str,
         appre_str, appe_str))[:-1])
        LOG.info('Vault Server %s' % os.environ['VAULT_ADDR'] if 'VAULT_ADDR' in os.environ else '??')
    parser.print_help()
    sys.exit(0)
...
def extract_file_args(subparsers):
    ...
    extract_parser = subparsers.add_parser('extract_file', help='Extract a single secret fromVault to a local file')
    extract_parser.add_argument('vault_path', help='Full path (including key) to secret')
    extract_parser.add_argument('destination', help='Location of destination file')
    base_args(extract_parser)


def mapping_args(parser):
    """Add various variable mapping command line options to the parser"""
    parser.add_argument('--add-prefix', dest='add_prefix', help='Specify a prefix to use when generating secret key names')
    parser.add_argument('--add-suffix', dest='add_suffix', help='Specify a suffix to use when generating secret key names')
    parser.add_argument('--merge-path', dest='merge_path', action='store_true', default=True, help='merge vault path and key name')
    parser.add_argument('--no-merge-path', dest='merge_path', action='store_false', default=True, help='do not merge vault path and key name')
    parser.add_argument('--key-map', dest='key_map', action='append', type=str, default=[])


def export_arg(parser):
    """Add the export argument to a parser"""
    parser.add_argument('--export', dest='export', help='Export declared variables', action='store_true')
...
def template_args(subparsers):
    ...
    template_parser.add_argument('template', help='Template source', nargs='?')
    template_parser.add_argument('destination', help='Path to write rendered template', nargs='?')
    template_parser.add_argument('vault_paths', help='Full path(s) to secret', nargs='*')
    template_parser.add_argument('--builtin-list', dest='builtin_list', help='Display a list of builtin templates', action='store_true', default=False)
    template_parser.add_argument('--builtin-info', dest='builtin_info', help='Display information on a particular builtin template')
...
def secretfile_args(parser):
    ...
    parser.add_argument('--secrets', dest='secrets', help='Path where secrets are stored', default=os.path.join(os.getcwd(), '.secrets'))
    parser.add_argument('--policies', dest='policies', help='Path where policies are stored', default=os.path.join(os.getcwd(), 'vault', ''))
    parser.add_argument('--secretfile', dest='secretfile', help='Secretfile to use', default=os.path.join(os.getcwd(), 'Secretfile'))
    parser.add_argument('--tags', dest='tags', help='Tags of things to seed', default=[], type=str, action='append')
    parser.add_argument('--include', dest='include', help='Specify paths to include', default=[], type=str, action='append')
    parser.add_argument('--exclude', dest='exclude', help='Specify paths to exclude', default=[], type=str, action='append')
...
def generic_args(parser):
    ...
    parser.add_argument('--verbose', dest='verbose', help='Verbose output', action='count', default=0)
...
def export_args(subparsers):
    ...
    export_parser.add_argument('directory', help='Path where secrets will be exported into')
...
def render_args(subparsers):
    ...
    render_parser.add_argument('directory', help='Path where Secrefile and accoutrement will be rendered into')
...
def seed_args(subparsers):
    ...
    seed_parser.add_argument('--mount-only', dest='mount_only', help='Only mount paths if needed', default=False, action='store_true')
    thaw_from_args(seed_parser)
    seed_parser.add_argument('--remove-unknown', dest='remove_unknown', action='store_true', help='Remove mountpoints that are not defined in the Secretfile')
...
def thaw_from_args(parser):
    ...
    of icefiles"""
    parser.add_argument('--thaw-from', dest='thaw_from', help='Thaw an ICE file containing secrets')
    parser.add_argument('--gpg-password-path', dest='gpg_pass_path', help='Vault path of GPG passphrase location')


def archive_args(parser):
    """Add the command line options for archive related operations"""
    parser.add_argument('icefile', help='Path to the encrypted archivefile of frozen secrets')


def thaw_args(subparsers):
    """Add command line options for the thaw operation"""
    thaw_parser = subparsers.add_parser('thaw')
    thaw_parser.add_argument('--gpg-password-path', dest='gpg_pass_path', help='Vault path of GPG passphrase location')
    thaw_parser.add_argument('--ignore-missing', dest='ignore_missing', help='Warn when secrets are missing from icefilesinstead of exiting', action='store_true', default=False)
...
def freeze_args(subparsers):
    ...
    freeze_parser.add_argument('--icefile-prefix', dest='icefile_prefix', help='Prefix of icefilename')
...
def password_args(subparsers):
    ...
    password_parser.add_argument('vault_path', help='Path which contains passwordsecret to be udpated')
...
def vars_args(parser):
    ...
    parser.add_argument('--extra-vars', dest='extra_vars', help='Extra template variables', default=[], type=str, action='append')
    parser.add_argument('--extra-vars-file', dest='extra_vars_file', help='YAML files full of variables', default=[], type=str, action='append')
...
def parser_factory(fake_args=None):
    ...
    return (
     parser, parser.parse_args(fake_args))
...
def template_runner(client, parser, args):
    ...
    if args.builtin_list:
        ...
        if args.builtin_info:
            aomi.template.builtin_info(args.builtin_info)
        else:
            if args.template and args.destination and args.vault_paths:
                aomi.render.template(client, args.template, args.destination, args.vault_paths, args)
            else:
                parser.print_usage()
                sys.exit(2)
    sys.exit(0)
...
def action_runner(parser, args):
    ...
    if args.operation == 'extract_file':
        aomi.render.raw_file(client.connect(args), args.vault_path, args.destination, args)
        sys.exit(0)
    else:
        if args.operation == 'environment':
            aomi.render.env(client.connect(args), args.vault_paths, args)
            sys.exit(0)
        else:
            if args.operation == 'aws_environment':
                aomi.render.aws(client.connect(args), args.vault_path, args)
                sys.exit(0)
            else:
                            ...
                            if args.operation == 'diff':
                                ...
                                if args.operation == 'template':
                                    ...
                                    if args.operation == 'token':
                                        ...
                                        if args.operation == 'set_password':
