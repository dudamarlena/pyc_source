...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/util.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 4304 bytes
__doc__ = 'Utilities which are generally that are tied into actual command line\noperations processing but do not really fit under seed or render.'
import os, logging
from aomi.helpers import get_password, path_pieces, backend_type, mount_for_path, abspath
import aomi.validation, aomi.exceptions
...
def vault_time_to_s(time_string):
    ...
    if not time_string or len(time_string) < 2:
        raise aomi.exceptions.AomiData('Invalid timestring %s' % time_string)
    ...
    if last_char == 'd':
        cur = int(time_string[0:len(time_string) - 1])
        return cur * 86400
    raise aomi.exceptions.AomiData('Invalid time scale %s' % last_char)