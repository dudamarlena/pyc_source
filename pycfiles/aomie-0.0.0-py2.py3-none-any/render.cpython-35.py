...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/render.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 6715 bytes
__doc__ = ' Secret rendering '
...
from aomi.helpers import merge_dicts, cli_hash, path_pieces, abspath
...
def template(client, src, dest, paths, opt):
    ...
    for path in paths:
        ...
        for s_k, s_v in response['data'].items():
            ...
            if s_k in key_map:
                o_key = key_map[s_k]
            k_name = secret_key_name(path, o_key, opt).lower().replace('-', '_')
            obj[k_name] = s_v
...
def env(client, paths, opt):
    ...
    if old_prefix:
        LOG.warning('the prefix option is deprecated but being used due to not passing in new options')
    elif opt.prefix:
        LOG.warning('the prefix option is deprecated but not being used due to passing in new options')
    key_map = cli_hash(opt.key_map)
    for path in paths:
...
def aws(client, path, opt):
    ...
    try:
        ...
        if vault_exception.errors[0].find('unsupported path') > 0:
            emsg = 'Invalid AWS path. Did you forget the credential type and role?'
