...
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/validation.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 5055 bytes
__doc__ = 'Some validation helpers for aomi'
from __future__ import print_function
import sys, os, re, platform, stat, logging
...
def secret_file(filename):
    ...
    if stat.S_ISREG(filestat.st_mode) == 0 and stat.S_ISLNK(filestat.st_mode) == 0:
        e_msg = 'Secret file %s must be a real file or symlink' % filename
        raise aomi.exceptions.AomiFile(e_msg)
    if platform.system() != 'Windows' and (filestat.st_mode & stat.S_IROTH or filestat.st_mode & stat.S_IWOTH or filestat.st_mode & stat.S_IWGRP):
...
def specific_path_check(path, opt):
    ...
    if opt.exclude and path in opt.exclude:
        return False
    if opt.include and path not in opt.include:
        return False
    return True
...
def gpg_fingerprint(key):
    ...
    if len(key) == 8 and re.match('^[0-9A-F]{8}$', key) or len(key) == 40 and re.match('^[0-9A-F]{40}$', key):
        return
...
def is_vault_time(time_string):
    ...
    return re.match('^[0-9]+[sdmh]$', time_string)