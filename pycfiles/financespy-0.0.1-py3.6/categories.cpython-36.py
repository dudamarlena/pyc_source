...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/financespy/categories.py
...
class Utilities(Expense):
    subtypes = [
     'Internet', 'Electricity', 'Cellphone_Balance']


class Travel(Expense):
    subtypes = [
     'Plane_Ticket', 'Hotel_Reservation', 'Train_Ticket']
...
class Shopping(Expense):
    subtypes = [
     'Electronics', 'Clothes', 'Sports', 'Home_Goods', 'Furnishing', 'Shopping_Misc', 'Shoes', 'Bag']


class Transport(Expense):
    subtypes = [
     'Monthly_Ticket', 'Single_Ticket', 'Day_Ticket']


class Home(Expense):
    subtypes = [
     'Rent', 'Home_Repair', 'Home_Service']


class Education(Expense):
    subtypes = [
     'Course', 'School_Book', 'School_Suplies']
...
class Eating_Out(Food):
    subtypes = [
     'Restaurant', 'Fast_Food', 'Icecream', 'Drink', 'Street_Food']
...
class Body_And_Hygiene(Expense):
    subtypes = [
     'Perfume', 'Hair_Product', 'Hairdresser', 'Nails']