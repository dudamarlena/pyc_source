...
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/Dates.py
# Compiled at: 2009-11-24 21:09:47
import time, datetime, re, Csys.DateTime as DateTime
_tzPatterns = (
 (
  re.compile('^(\\d{4})-(\\d\\d)-(\\d\\d)'), '\\1/\\2/\\3'),
 (
  re.compile('^(.*[-+]\\d+)(.*)$'), '\\1'),
 (
 ...
 (
  re.compile('(.*)\\s+([a-z/]+)\\s+(\\d{4})', re.IGNORECASE), '\\1 \\3 \\2'))
...
_str2secondTable = (
 ...
 (
  'm', minute),
 ('s', 1))
_str2secondsPattern = re.compile('[yMwdhm]')
...
def rfcDate(mytime=None):
    ...
    return time.strftime(mailFmt, loctime) + '%05d' % ((time.mktime(loctime) - time.mktime(utctime)) / 36)
...
_yyyy_mm_dd_re = re.compile('^([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})$')
_mm_dd_year_re = re.compile('^([0-9]{1,2})/([0-9]{1,2})/([0-9]{2,4})$')
_mmddyear_re = re.compile('^([0-9]{1,2})([0-9]{2})([0-9]{4})$')
_9999_re = re.compile('^(-{0,1}[0-9]{1,4})$')
...
def strdate(date_in, fmt=DF_SQLDATE):
    ...
    while True:
        ...
        if not date_in or date_in == '**/**/**' or date_in == '0000-00-00':
            return
        r = _mm_dd_year_re.search(date_in)
        if not r:
            r = _mmddyear_re.search(date_in)
        if r:
            _month, _day, _year = int(r.group(1)), int(r.group(2)), int(r.group(3))
            if _year < 70:
                _year += 2000
            elif _year < 100:
                _year += 1900
