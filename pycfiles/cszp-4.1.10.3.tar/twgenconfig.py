...
# Embedded file name: /home/bill/py3/.virtualenvs/pytest/lib/python2.7/site-packages/Csys/twgenconfig.py
# Compiled at: 2013-06-11 15:55:09
...
def main():
    ...
    def setOptions():
        ...
        parser.add_option('-f', '--force', action='store_true', dest='force', default=False, help='Force replace')
        parser.add_option('-o', '--outfile', action='store', type='string', dest='outfile', default=None, help='Output File (default stdout)')
        parser.add_option('-r', '--rerun', action='store_true', dest='rerun', default=False, help='Rerun from existing tw.config file')
        parser.add_option('-x', '--exclude', action='append', dest='exclude', default=[], help='Exclude Directory')
        return parser
    ...
    openPKGInstances = [ os.path.realpath(s.strip()) for s in open('/etc/openpkg') if s.strip() ]
    configDir = os.path.join(csoft, 'var/tripwire')
    configFiles = Csys.CSClassBase({'tw_config': os.path.join(configDir, 'tw.config')})
    if options.rerun:
        if not options.outfile:
            options.outfile = configFiles.tw_config
        commandPattern = re.compile('^# COMMAND:')
    ...
    if amq:
        ...
        amdPattern = re.compile('\\b(toplvl)\\b')
        for line in fhamd:
    ...
    for entry in rootentries:
        ...
        if entry not in exclude_dirs and os.path.exists(entry):
    ...
    for mountpoint in mounted:
        if mountpoint not in search_dirs:
    ...
    try:
        ...
        for ignore in ('files.list', 'files.list.gz', 'ls-Rl.gz', 'ls-lR.gz', 'ls-tRl.gz',
                       'ls-tlR.gz'):
