...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\binpackp\bins.py
# Compiled at: 2016-07-10 03:17:55
...
class SimpleBin(object):
    __doc__ = 'An object to represent a bin in the Bin Packing Problem.'
    ...
    def __repr__(self):
        return '<{}>(Volume: {}/{}, Items: {})'.format(self.__class__.__name__, self._volume - self._available_volume, self._volume, reprlib.repr(self._items))
    ...
    def _fits(self, value):
        required = self.size(value)
        if required > self._available_volume:
            raise VolumeError('Item of size {} is unable to fit in available volume {}.'.format(required, self._available_volume))
        return required
