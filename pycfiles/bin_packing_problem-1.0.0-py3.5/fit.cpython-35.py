...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\binpackp\fit.py
# Compiled at: 2016-07-10 04:27:21
# Size of source mod 2**32: 10988 bytes
...
class Fit(object):
    ...
    @classmethod
    def fit(cls, bin_cls, bin_volume, items, sort=False, sort_reversed=True, starting_bins=1, max_open_bins=0, bin_sort=BinOrdering.none, bin_reduction=BinReduction.min_volume, method_name=None, method_detection=True):
    ...
    def _detect_method(cls, sort, sort_reversed, max_open_bins, bin_ordering, bin_reduction):
        ...
        if sort and sort_reversed:
            code += 'd'
        if (max_open_bins > 0 or sort and not sort_reversed) and code != 'cv' and code != 'Nf' and max_open_bins == 1:
            code += 'v'
...
class BPResult(object):
    ...
    def __repr__(self):
        return '<BPResult (Total Bins: {}, Total Volume: {}, Unused Volume: {})>'.format(self.total_bins, self.total_volume, self.unused_volume)