...
# [GCC 8.4.0]
# Embedded file name: ..\indicatorModule\pyalgotrade\technical\atr.py
# Compiled at: 2019-07-03 05:38:18
...
class ATRSmaEventWindow(technical.EventWindow):
    ...
    def __init__(self, period, useAdjustedValues):
        ...
        self._ATRSmaEventWindow__useAdjustedValues = useAdjustedValues
        self._ATRSmaEventWindow__prevClose = None
        self._ATRSmaEventWindow__value = None

    def _calculateTrueRange(self, value):
        ret = None
        if self._ATRSmaEventWindow__prevClose is None:
            ret = value.getHigh(self._ATRSmaEventWindow__useAdjustedValues) - value.getLow(self._ATRSmaEventWindow__useAdjustedValues)
        else:
            tr1 = value.getHigh(self._ATRSmaEventWindow__useAdjustedValues) - value.getLow(self._ATRSmaEventWindow__useAdjustedValues)
            tr2 = abs(value.getHigh(self._ATRSmaEventWindow__useAdjustedValues) - self._ATRSmaEventWindow__prevClose)
            tr3 = abs(value.getLow(self._ATRSmaEventWindow__useAdjustedValues) - self._ATRSmaEventWindow__prevClose)
            ret = max(max(tr1, tr2), tr3)
        return ret
    ...
    def onNewValue(self, dateTime, value):
        ...
        self._ATRSmaEventWindow__prevClose = value.getClose(self._ATRSmaEventWindow__useAdjustedValues)
        if value is not None:
            if self.windowFull():
                self._ATRSmaEventWindow__value = commonHelpBylw.round_up(self.getValues().mean(), 2)

    def getValue(self):
        return self._ATRSmaEventWindow__value
