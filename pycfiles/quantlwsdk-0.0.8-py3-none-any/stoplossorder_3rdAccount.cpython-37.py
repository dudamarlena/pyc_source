...
# Compiled at: 2020-04-13 02:55:34
# Size of source mod 2**32: 26761 bytes
"""
...
class riskStopOrder:
    ...
    def getOrderInfo(self):
        ...
        infodict['posiNum'] = self.target_order_position.volume
        return infodict
    ...
    def init_excute_fun(self):
        ...
        if self.target_order_position.positionSide == 1:
            if self._order_type == 2:
                self._excute_fun = partial(gm3HelpBylw.gmOrder.clearLong, self._targetSymbol, self.target_order_position.volume, self._signalName)
        ...
        if self.target_order_position.positionSide == 2:
            if self._order_type == 2:
                self._excute_fun = partial(gm3HelpBylw.gmOrder.clearShort, self._targetSymbol, self.target_order_position.volume, self._signalName)
            if self._order_type == 1:
        ...
        if self._status == STOP_PROFIT_LOSS_ORDER_STATUS.ACTIVE:
            if self.target_order_position.positionSide == 2:
                ...
                if bar_.high >= clear_price:
                    ...
                    self.setStatus(STOP_PROFIT_LOSS_ORDER_STATUS.ORDER_SENDED)
                elif self.target_order_position.positionSide == 1:
                    if self._stop_type == 'percent':
                        clear_long_price = self._hh * (1 - self._stop_gap)
