...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/azure_translator/__init__.py
# Compiled at: 2017-02-15 19:19:52
"""Azure Translator module."""
import xml.etree.ElementTree as ET, requests
from .errors import AzureApiError, AzureApiBadFormatError, AzureCannotGetTokenError, AzureApiTimeoutError
...
class Translator(object):
    ...
    def get_access_token(self):
        ...
        """
        try:
            resp = requests.post(self.TOKEN_API, headers={'Content-Type': 'application/json', 
               'Accept': 'application/jwt', 
               'Ocp-Apim-Subscription-Key': self.api_key}, timeout=self.HTTP_TIMEOUT)
            resp.raise_for_status()
        except requests.exceptions.Timeout as error:
            raise AzureApiTimeoutError(unicode(error), request=error.request)
        except requests.exceptions.HTTPError as error:
            raise AzureCannotGetTokenError(unicode(error), response=error.response, request=error.request)
    ...
    def translate(self, text, to=DEFAULT_LANGUAGE, source_language=None):
        ...
        try:
            resp = requests.get(self.TRANSLATE_API, headers={'Authorization': ('Bearer {}').format(self.get_access_token()), 
               'Accept': 'application/xml'}, params=params, timeout=self.HTTP_TIMEOUT)
            resp.raise_for_status()
        except requests.exceptions.Timeout as error:
            raise AzureApiTimeoutError(unicode(error), request=error.request)
        except requests.exceptions.HTTPError as error:
            raise AzureApiError(unicode(error), response=error.response, request=error.request)
