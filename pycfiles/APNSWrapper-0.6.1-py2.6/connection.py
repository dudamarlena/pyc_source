...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/APNSWrapper/connection.py
# Compiled at: 2010-05-19 14:38:53
...
class OpenSSLCommandLine(APNSConnectionContext):
    ...
    def _command(self):
        command = '%(executable)s s_client -ssl3 -cert %(cert)s -connect %(host)s:%(port)s' % {'executable': self.executable, 
           'cert': self.certificate, 
           'host': self.host, 
           'port': self.port}
        return subprocess.Popen(command.split(' '), shell=False, bufsize=256, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
class SSLModuleConnection(APNSConnectionContext):
    ...
    def context(self):
        ...
        if self.connectionContext != None:
            ...
            self.connectionContext = self.ssl_module.wrap_socket(self.socket, ssl_version=self.ssl_module.PROTOCOL_SSLv3, certfile=self.certificate)
            return self
...
class APNSConnection(APNSConnectionContext):
    ...
    def __init__(self, certificate=None, ssl_command='openssl', force_ssl_command=False, disable_executable_search=False, debug=False):
        self.connectionContext = None
        self.debug = debug
