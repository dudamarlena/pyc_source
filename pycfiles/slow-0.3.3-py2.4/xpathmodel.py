...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/model/xpathmodel.py
# Compiled at: 2006-03-22 06:38:58
"""
...
from lxml.etree import ElementTree, SubElement, Element, ElementBase, ETXPath, XPathEvaluator, RelaxNG
...
class AugmenterMetaClass(type):
    ...
    def __new__(mcls, cls, bases, class_dict):
        complete_dict = dict((item for base in reversed(bases) for c in reversed(type.mro(base))))
    ...
    def __augment(mcls, class_dict, complete_dict):
        ...
        RE_VARIABLE_REFERENCE = re.compile('\\$([a-zA-Z_][:a-zA-Z0-9_.-]*)')
            ...
            if argnames:
                ...
                def mget(self, *args, **kwargs):
                    if var_names and (args or kwargs):
                        variables = _dict((var for var in _chain(_izip(argnames, args), kwargs.iteritems()) if var[0] in var_names))
        ...
        for name in attribute_names:
            ...
            if isinstance(mget, (str, unicode)):
                ...
                getter_xpath = None
            if getter_xpath:
                mget = build_xpath_getter(getter_call, getter_xpath, getter_argnames, converters, get_autoconstruct)
            elif converters:
            ...
            if setter_xpath:
                if setter_argnames[:2] == ('self', '_xpath_result'):
                    mset = build_xpath_call(mset, setter_xpath, setter_argnames[2:], set_autoconstruct)
            ...
            if isinstance(mdel, (str, unicode)):
                ...
                if deller_xpath:
                    ...
                    if deller_argnames[:2] == ('self', '_xpath_result'):
                        ...
                        deller_argnames = deller_argnames[1:]
                        mdel = default_del
            if deller_xpath:
...
class XPathModelHelper(object):
    ...
    def _build_referenced_access(el_name, ref_attribute, parent_expression=''):
        xpath_all = '.%s/%s' % (parent_expression, el_name)
        xpath_set = '(%s[@%s = $ref])[1]' % (xpath_all, ref_attribute)
        xpath_get = xpath_set + '/*'
        ...
        return (
         _get_one, _set_one, _del_one, _get_all, _del_all)

    @staticmethod
    def _build_tree_node(el_tag, parent_expression=''):
        xpath_set = '.%s/%s[1]' % (parent_expression, el_tag)
        xpath_get = xpath_set + '/*'
...
if __name__ == '__main__':
    ...
    class Test(XPathModel):
        ...
        def _get_etext(self, t):
            """string(./bla2[@t = $t]/e/text()[1])"""
            pass

        _get_etexts = './bla2/e/text()'
        ...
        _compiled_xpath_bla2 = './bla2[@t = $t]'

        def _set_t(self, t, value):
            self._update_xpath_attribute(self._compiled_xpath_bla2, 't', unicode(value), t=t)
