...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/pyexec/pydb/viewspec.py
# Compiled at: 2006-01-10 04:15:20
...
__all__ = ('parse_view_spec', 'UnknownReferenceError', 'ViewSpecification', 'ViewFunctions')
...
def parse_view_spec(statement, user_functions={}):
    ...
    return ViewSpecification(StatementParser.parse(statement), user_functions)
...
class ViewObjectSpec(object):
    ...
    def __init__(self, name, attribute_value_select, user_functions={}, known_variables={}):
        self.name = name
        self.attributes = attributes = {}
        self.reverse_deps = reverse_deps = {}
        for (attr, value) in attribute_value_select.iteritems():
            if value:
                expr = CompiledExpression(value, user_functions=user_functions, known_variables=known_variables)
...
class ViewFunctions(object):
    ...
    def _select(self, key_expr, objects, count, var_values, distinct=False, reverse=False):
        ...
        if count == 1:
            ...
            iter_result = imap(heapq.heappop, repeat(decorated_objects, len(decorated_objects)))
    ...
    def _absolute(self, count, expression, distinct=False, reverse=False):
        evaluate = expression.evaluate
        select = self._select

        def _absolute_rank(objects, var_values={}):
            return select(evaluate, objects, count, var_values, distinct=distinct, reverse=reverse)

        return _absolute_rank

    def _relative(self, count, rel_value, expression, distinct=False, reverse=False):
        _abs = abs
        evaluate = expression.evaluate
        ...
        def _relative_rank(objects, var_values={}):
            return select(_eval_expression, objects, count, var_values, distinct=distinct, reverse=reverse)
...
class ViewSpecification(object):
    ...
    def __init__(self, parsed_viewdef, user_functions={}, new_name=None, new_parents=None, **variable_defaults):
        ...
        self.loops = [ (Variable(str(var.name)), values) for (var, values) in parsed_viewdef.loops ]
        ...
        for variable in parsed_viewdef.variable_options:
            ...
            if variable.name in variable_defaults:
                ...
                val_exp = CompiledExpression(variable.default_value, user_functions=user_functions, known_variables=self.__all_known_variables)
        ...
        self.view_object = ViewObjectSpec(self.view_name, parsed_viewdef.view_attributes, user_functions=user_functions, known_variables=self.__all_known_variables)
        ...
        self.where_expression = CompiledExpression(parsed_viewdef.where_expression, user_functions=user_functions, known_variables=self.__all_known_variables)
        self.having_expression = CompiledExpression(parsed_viewdef.having_expression, user_functions=user_functions, known_variables=self.__all_known_variables)
        ...
        self.__hash = hash((str(self.view_object), self.select_distinct, self.select_buckets, select_string, self.view_parents, self.where_expression))
        return
    ...
    def __compile_function(self, function):
        ...
        for param in function.function_parameters:
            cparam = CompiledExpression(param, user_functions=self.user_functions, known_variables=self.__all_known_variables)
...
class Bucket(dict):
    ...
    def select_nodes(self, select_node, filter_node=None, bucket=None):
        ...
        def nodefilter(nodelist):
            return [ filter_node(node) for node in nodelist if select_node(node) ]

        return self.filter_nodelists(nodefilter, bucket)

    def filter_select(self, select_node=None, filter_node=None, filter_list=None, prefilter_list=None, bucket=None):
        ...
        def listfilter(nodelist):
            return filter_list((filter_node(node) for node in prefilter_list(nodelist) if select_node(node)))
