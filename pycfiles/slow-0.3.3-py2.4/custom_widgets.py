...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/custom_widgets.py
...
class SelectiveCustomWidget(object):
    ...
    def __new__(cls, parent, name, *args):
        ...
        if hasattr(cls, '_EXCLUDED_WIDGET_NAMES'):
            use_class = use_class and name not in cls._EXCLUDED_WIDGET_NAMES
...
class DotWriter(object):
    ...
    def generate_for_callback(self, call_back, edsm_model, **kwargs):
        dot_out = ProcessManager(self.parent, call_back, self.prog, '-T%s' % self.format)
    ...
    def write_graph(self, f, edsm_model, **kwargs):
        write = f.write
        write(('').join(('%s = "%s"\n' % item for item in kwargs.iteritems())).encode('utf-8'))
        dispatch = {'subgraph': self.subgraph_to_dot, 'state': self.state_to_dot}
        for state_or_subgraph in edsm_model.states:
            dispatch[state_or_subgraph.type_name](f, state_or_subgraph)

        write(('\n').join(('%s\n' % c for c in imap(self.connection_to_dot, edsm_model.transitions))).encode('utf-8'))
    ...
    def attributes(self, **attributes):
        return (', ').join(('%s = "%s"' % attr for attr in attributes.iteritems()))
    ...
    def connection_to_dot(self, connection):
        ...
        attributes = self.Attributes(color=colour, fontcolor='gray', fontsize='6')
        ...
        if from_queue and from_queue != 'output':
            attributes.add(taillabel=from_queue, labelfontcolor='black')
        to_queue = connection.to_queue
        if to_queue and to_queue != 'input':
            attributes.add(headlabel=to_queue, labelfontcolor='black')
        return '"%s" -> "%s" [%s]' % (connection.from_state.id, connection.to_state.id, attributes)
...
class DotGraphWidget(qt.QWidget):
    ...
    def rebuild_graph(self):
        ...
        if not self._editor:
            return
        self._graph_generator.generate_for_callback(self.set_image_data, self._editor.edsm_model, splines=self._use_splines)
...
class IterableListView(SelectiveCustomWidget, qt.QListView):
    ...
    def iterSelectedItems(self):
        return (item for item in self.iterColumnItems() if item.isSelected())
...
class EDSMIconView(SelectiveCustomWidget, qt.QIconView):
    ...
    def iterconnections(self, m_type=None):
        if m_type:
            return (connection for connection in self.connections if connection.type == m_type)
        else:
    ...
    def connections_at(self, pos, prefer_type=None, restrict_type=None):
        if restrict_type is None:
            ...
            connections = (c for c in self.connections if c.type == restrict_type)
        sqrt = math.sqrt
        ...
        close_connections = [ c[(-1)] for c in sorted(takewhile(lambda c: c[0] <= max_dist, candidates), key=operator.itemgetter(1))
                            ]
        if prefer_type is not None:
            best_connections = [ c for c in close_connections if connection.type == prefer_type ]
            if best_connections:
