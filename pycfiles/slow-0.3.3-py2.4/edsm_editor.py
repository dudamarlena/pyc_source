...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/edsm_editor.py
# Compiled at: 2006-01-10 04:15:18
from qt import Qt, QWidget, QComboBox, QListBox, QIconViewItem, QPopupMenu, SIGNAL, PYSIGNAL
...
from slow.model.edsm_model import EDSMState, EDSMTransition, buildTransition, buildState, buildSubgraph, buildEmptyModel
...
class AbstractEDSMDialog(object):
    ...
    def __init__(self, parent=None, name=None, modal=0, fl=0):
        ...
        self.field_dict = dict(((attr_name, (getattr(self, fieldname), getattr(self, '%s_textlabel' % fieldname))) for (attr_name, fieldname) in self.FIELD_MAP if hasattr(self, fieldname)))
        self.code_field_dict = dict(((attr_name, (getattr(self, fieldname), getattr(self, '%s_textlabel' % fieldname))) for (attr_name, fieldname) in self.CODE_FIELD_MAP if hasattr(self, fieldname)))
        self._code_dict = {}
        self._selected_language = None
        return

    FIELD_MAP = (('readable_name', 'object_name'), ('message_type', 'message_type'), ('from_queue', 'from_queue'), ('to_queue', 'to_queue'), ('output_queues', 'output_queues'), ('input_queues', 'input_queues'))
    CODE_FIELD_MAP = (
     ('language', 'language_name'), ('code', 'init_code'), ('class_name', 'class_name'))
    ...
    def activate_fields(self):
        ...
        for (field, label) in chain(self.field_dict.itervalues(), self.code_field_dict.itervalues()):
    ...
    def setModel(self, dialog_model):
        ...
        for queue_type in ('to', 'from'):
            try:
                ...
                queues = getattr(state, queue_type == 'to' and 'input_queues' or 'output_queues')
            except AttributeError:
                continue
    ...
    def accept(self):
        ...
        for (attrname, fieldname) in self.FIELD_MAP:
            ...
            if isinstance(field, QListBox):
                values = [ qstrpy(field.text(i)) for i in range(field.count()) ]
...
class TransitionDialog(AbstractTransitionDialog, EDSMTransitionDialog):
    ...
    FIELDS = {'message': ['object_name', 'message_type', 'to_queue'], 'event': ['object_name', 'event_type', 'to_queue'], 'outputchain': ['object_name', 'from_queue', 'to_queue'], 'transition': ['object_name']}
...
class StateDialog(AbstractEDSMDialog, EDSMStateDialog):
    ...
    FIELDS = ('object_name', 'class_name', 'language_name', 'init_code', 'output_queues',
              'input_queues')
...
class EDSMIconViewIcon(EDSMEditorItem, QIconViewItem):
    ...
    def add_connection(self, from_state, model):
        ...
        connection = ConnectionItem(iconview, self._edsm_editor, from_state, self, model)
...
class IconViewSubgraphIcon(EDSMIconViewIcon):
    ...
    def _populate_popup_menu(self, menu):
        ...
        menu.insertItem(self._edsm_editor.delete_icon, tr('delete'), self.menu_delete)
...
class IconViewStateIcon(EDSMIconViewIcon):
    ...
    def _populate_popup_menu(self, menu):
        ...
        if self.FLAG_NAMES:
            menu.insertSeparator()
            self._build_flag_menu_area(menu, self._edsm_editor, self._model, self.FLAG_NAMES, False)
        menu.insertSeparator()
...
class ConnectionItem(EDSMEditorItem):
    __module__ = __name__
    CONNECTION_COLOURS = dict(izip(sorted(EDSMTransition.VALID_TYPES), chain((Qt.black, Qt.blue, Qt.red, Qt.green, Qt.magenta), repeat(Qt.black))))
    ...
    def description(self):
        model = self._model
        return '%s: %s -> %s' % (self.tr(model.TYPENAMES[model.type]), model.from_state.readable_name, model.to_state.readable_name)
...
class EDSMEditor(EDSMEditorItem):
    ...
    def __init__(self):
        ...
        self.EDSM_CLICK_FUNCTIONS = {'state': self._put_state_icon, 'subgraph': self._put_state_icon}
    ...
    def __init_tools(self):
        ...
        self.TOOL_BUTTONS_BY_TYPE = dict(((types[transition_name], tool_buttons.find(button_id)) for (button_id, transition_name) in islice(enumerate(self.TOOL_BUTTONS), 3, None)))
    ...
    def __init_icons(self):
        ...
        class StateIcon(IconViewStateIcon):
            __module__ = __name__
            FLAG_NAMES = (('inherit_context', tr('inherit context')), ('long_running', tr('long running')))

            def __init__(self, iconview, model):
                IconViewStateIcon.__init__(self, iconview, editor, model, state_pixmap)
        ...
        class SubgraphIcon(IconViewSubgraphIcon):
            ...
            def __init__(self, iconview, model):
                IconViewSubgraphIcon.__init__(self, iconview, editor, model, subgraph_pixmap)
                self.setRenameEnabled(True)
    ...
    def __reset_iconview(self, iconview, subgraph_model, static_state_names):
        ...
        class SpecialStateIcon(IconViewStateIcon):
            ...
            def __init__(self, model):
                ...
                IconViewStateIcon.__init__(self, iconview, editor, model, pixmap)
        ...
        for state_name in static_state_names:
            ...
            if model is None:
                model = buildState(subgraph_model.states, state_name, readable_name)
    ...
    def __init_dot(self):
        if hasattr(self, 'edsm_dot_graph'):
            ...
            colours = dict(((type_names[key], str(qcolour.name())) for (key, qcolour) in ConnectionItem.CONNECTION_COLOURS.iteritems()))
    ...
    def add_iconview_tab(self, name, model, icon_positions={}):
        ...
        self.connect(child_iconview, SIGNAL('contextMenuRequested(QIconViewItem*,const QPoint&)'), self.edsm_iconview_contextMenuRequested)
        self.connect(child_iconview, SIGNAL('doubleClicked(QIconViewItem*)'), self.edsm_edit_state)
        self.connect(child_iconview, SIGNAL('itemRenamed(QIconViewItem*)'), child_iconview.updateContents)
