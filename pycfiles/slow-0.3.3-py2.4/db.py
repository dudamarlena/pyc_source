...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/pyexec/pydb/db.py
# Compiled at: 2006-01-31 15:37:47
try:
...
class NodeDB(ReflectiveObservable):
    ...
    def __init__(self, name, local_node, pyattributes, attribute_defaults=None):
        ReflectiveObservable.__init__(self)
        self.name = name
        self.local_node = local_node
        self.__identifiers = sorted((attribute.name for attribute in pyattributes.values() if attribute.identifier))
    ...
    def updateNodeAttribute(self, node, attr):
        self._notify(self.NOTIFY_UPDATE_NODES, node, attr, getattr(node, attr))
...
class DBNode(object):
    ...
    def __repr__(self):
        return 'node[%s]' % (',').join(('%s=%r' % (name, getattr(self, name, None)) for name in self._getIDs()))
...
class BackedNode(object):
    ...
    def __new__(cls, property_names, back_object):
        property_names = tuple(sorted(set((name for name in property_names if name if name[0] != '_'))))
        key = (
         cls,) + property_names
        try:
...
class ObservableBackedNode(BackedNode):
    ...
    def _build_proxies(self, property_names):
        return iter(((name, ObservableDescriptorProxy(observable, name)) for (name, observable) in self.__notifiers.iteritems()))
...
class Node(object):
    ...
    def _iterProperties(cls, pnames=None):
        return ((name, getattr(cls, name)) for name in pnames or dir(cls) if name if name[0] != '_')
