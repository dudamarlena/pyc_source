...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/message_editor.py
...
class AddItemPopupAction(qt.QAction):
    ...
    def __init__(self, editor, title, item_parent, item_class, type_name=None, item_name=None):
        ...
        def action_method():
            model = buildMessageElement(item_parent.model, model_type, type_name=type_name or model_type, readable_name=item_name or readable_name)
...
class ShowMessagePopupAction(qt.QAction):
    ...
    def __init__(self, editor, title, item):
        ...
        def action_method():
            ...
            dialog = TextDialog(editor, 'message', self.tr("Message '%1'").arg(item.type_name))
            dialog.setText(xml)
...
class MListViewItem(qt.QListViewItem, MenuProvider):
    ...
    ORDER = 9
    ALLOWS_DELETE = True
    READABLE_NAME = MODEL_TYPE = PIXMAP = None
    ...
    def __init__(self, editor, parent, model, popup_checkable=False, *args):
        ...
        MenuProvider.__init__(self, editor, title=coltext1, checkable=popup_checkable or bool(self.FLAG_NAMES))
    ...
    def new_reference(self, new_parent):
        if self.REF_MODEL_TYPE and self.REF_ITEM_CLASS:
            ...
            ref_model = buildMessageElement(new_parent.model, self.REF_MODEL_TYPE, type_name=type_name, access_name=self.build_access_name(type_name, new_parent.model), readable_name='>' + ref_item_class.READABLE_NAME)
    ...
    def _populate_popup_menu(self, menu):
        if self.FLAG_NAMES:
            ...
            self._build_flag_menu_area(menu, self.editor, self.model, self.FLAG_NAMES, True)
...
class ContainerListViewItem(MListViewItem):
    ...
    def __setupClassMenuEntries(self):
        ...
        ContainerListViewItem.MENU_ENTRIES = (
         (
          tr('Add Content'), ContentItem), (tr('Add View Data'), ViewDataItem), (tr('Add Container'), ContainerItem), (tr('Add Container'), PredefContainerItem), (tr('Add Header'), HeaderItem), (tr('Add Message'), MessageItem))
    ...
    def acceptDrop(self, data):
        ...
        return data.provides(self.ATTRIBUTE_MIME_TYPE) or data.provides(self.FIELD_MIME_TYPE)
    ...
    def dropped(self, data):
        ...
        if source == listview:
            ...
            if hasattr(item, 'text'):
                ...
                new_model = buildMessageElement(model, 'attribute', readable_name=qstrpy(self.tr('Attribute')), type_name=name, access_name=access_name)
    ...
    def _populate_popup_menu(self, menu):
        ...
        for (text, item_class) in self.MENU_ENTRIES:
            if self._accepts_child_class(item_class):
                action = AddItemPopupAction(self.editor, text, self, item_class)
                action.addTo(menu)
...
class ViewDataItem(MListViewItem):
    ...
    def __init__(self, editor, *args):
        self.FLAG_NAMES = (('structured', editor.tr('bucket structure')), ('bucket', editor.tr('single bucket')), ('list', editor.tr('node list')))
...
class ContainerItem(ContainerListViewItem):
    ...
    def __init__(self, editor, *args):
        self.FLAG_NAMES = (('list', editor.tr('list')),)
        MListViewItem.__init__(self, editor, popup_checkable=True, *args)
...
class MessageItem(ContainerListViewItem):
    ...
    class ProtocolSet(object):
        ...
        def to_set(self):
            return frozenset((protocol for protocol in self.PROTOCOLS if getattr(self, protocol)))
    ...
    def protocol_menu(self):
        ...
        for protocol in protocol_set.PROTOCOLS:
            action = FlagMaintainerAction(parent, protocol_set, protocol, protocol)
            action.addTo(menu)
    ...
    def _populate_popup_menu(self, menu):
        menu.insertSeparator()
        menu.insertItem(qt.QIconSet(self.PROTOCOLS_PIXMAP), self.PROTOCOLS_NAME, self.protocol_menu())
        if 'message_builder' in STYLESHEETS:
            action = ShowMessagePopupAction(self.editor, self.tr('Show XML'), self)
            action.addTo(menu)
...
class MessageEditor(MenuProvider):
    ...
    __ITEM_CLASSES = (HeaderItem, MessageItem, ContainerItem, ViewDataItem, ContentItem, AttributeContentItem)
    __ITEM_TYPE_DICT = dict(((cls.MODEL_TYPE, cls) for cls in chain(__ITEM_CLASSES, (ContainerRefItem,))))
    ...
    def __setup_icons(self):
        ...
        for (item, item_class) in izip(islice(items, 1, None), chain(self.__TOP_LEVEL_CLASSES, self.__ITEM_CLASSES)):
        ...
        listview.takeItem(self.__orig_items)
        return
