...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/vis/view_snapshot.py
# Compiled at: 2006-01-10 04:15:16
from itertools import *
from math import pi, cos, sin, sqrt
import colorsys, Image, ImageDraw
try:
...
class ViewCircleSnapshot(ViewGraph):
    ...
    def _spread_nodes(self, _):
        ...
        for (i, node) in enumerate(nodes):
            ...
            sin_val = sin(angle)
            cos_val = cos(angle)
            pos[node.name] = (middle + radius * sin_val + name_width // 2 * decluster(angle, cos_val ** 8), middle - radius * cos_val)
    ...
    def snapshot(self):
        ...
        for node in graph.nodes:
            ...
            ellipse_pos = (
             pos[0] - half_text_size[0], pos[1] - text_size[1], pos[0] + half_text_size[0], pos[1] + text_size[1])
            draw.ellipse(ellipse_pos, fill=white, outline=black)
            text_pos = (
             pos[0] - half_text_size[0], pos[1] - half_text_size[1])
