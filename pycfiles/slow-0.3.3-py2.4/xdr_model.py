...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/model/xdr_model.py
...
class XdrDataTypes(object):
    __module__ = __name__
    TYPES = (('boolean', bool), ('char', str), ('short', int), ('int', int), ('long', long), ('float', float), ('double', float), ('enumeration', EnumType), ('structure', StructType), ('string', str), ('farray', ArrayType), ('varray', ArrayType), ('union', UnionType), ('opaque', str))
    FLAT_TYPES = tuple((t for t in TYPES if t[0] not in ('structure', 'union')))
    SIMPLE_TYPES = tuple((t for t in FLAT_TYPES if t[0] not in ('farray', 'varray',
                                                                'enumeration')))
    TYPE_NAMES = [ t[0] for t in TYPES ]
    FLAT_TYPE_NAMES = [ t[0] for t in FLAT_TYPES ]
    SIMPLE_TYPE_NAMES = [ t[0] for t in SIMPLE_TYPES ]
    TYPE_DICT = dict(TYPES)
    XDR_TYPE_DICT = dict((item for item in TYPES if issubclass(item[1], XdrDataType)))
...
class XdrEnumerationAttributeModel(XdrAttributeModel):
    ...
    def removeValue(self, name):
        try:
            self._type_description.remove(name)
        except:
            pass
