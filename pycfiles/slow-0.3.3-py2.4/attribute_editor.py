...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/attribute_editor.py
...
class AttributeEditor(object):
    ...
    def __init_type_selectors(self):
        length_attrs = self.__LENGTH_ATTRIBUTES = dict(chain([('decimal', 'bits'), ('text', 'maxlength')], ((name, 'maxval') for name in ('smallint',
                                                                                                                                          'integer',
                                                                                                                                          'bigint')), ((name, 'length') for name in ('char',
                                                                                                                                                                                     'array',
                                                                                                                                                                                     'bytea'))))
        length_frame = self.attribute_type_length_frame
        configurators = self.TYPE_CONFIGURATORS = {'array': [self.attribute_type_subtype_frame], 'composite': [self.attribute_type_composite_table]}
    ...
    def __init_views(self):
        self.attribute_type_range_frame.hide()
    ...
    class HighlightedFirstColumnListItem(AttributeListItem):
        ...
        def paintCell(self, painter, colour_group, column, width, align):
            ...
            FlagMaintainerListItem.paintCell(self, painter, colour_group, column, width, align)

    def _build_attribute_list_item(self, attribute):
        item = self.HighlightedFirstColumnListItem(self.node_attribute_list, attribute, 'selected', attribute.name, attribute.type_name)
    ...
    def copy_type_to_model(self):
        ...
        def tag_name(element_type):
            ...
            return '{%s}%s' % (namespace, element_type)
        ...
        value_dict = {'type_name': type_name}
        ...
        if child_type:
            ...
            for row in xrange(composite_table.numRows()):
                ...
                if cname:
                    if ctype and (ctype == type_name or ctype not in all_types):
    ...
    def copy_type_from_model(self, attribute_type, model=None):
        ...
        if attribute_type in ('array', 'composite'):
            if self.attribute_type_subtype_frame.isEnabled():
                subtype = len(model) and model[0].base_type or ''
                self.attribute_type_subtype_select.setCurrentText(subtype)
            elif self.attribute_type_composite_table.isEnabled():
                table = self.attribute_type_composite_table
                for (row, child) in izip(range(table.numRows()), chain(model, repeat(None))):
                    if child:
        ...
        if bits:
            spinbox.setSuffix(' %s' % self.tr('bits'))
        else:
            spinbox.setSuffix('')
        return