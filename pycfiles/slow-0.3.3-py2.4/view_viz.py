...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/vis/view_viz.py
# Compiled at: 2006-01-10 04:15:16
...
def build_colour_table():
    ...
    for power in xrange(7):
        power_step *= 2.0
        colours.extend(('%.3f,1.0,0.8' % (step / power_step) for step in xrange(1, int(power_step), 2)))
...
class ViewGraph(object):
    ...
    def _build_graph(self, node_views_tuples, **kwargs):
        ...
        next_colour = chain(COLOUR_TABLE, imap(random.choice, repeat(COLOUR_TABLE))).next
        colour_map = {}
        build_edges_from_views = self.build_edges_from_views
        for (node, views) in node_views_tuples:
            build_edges_from_views(str(node), views, colour_map, next_colour)
    ...
    def build_edges_from_views(self, node_name, views, colour_map, next_colour):
        ...
        def build_bucket_edges(bucket_iterator, view_name, node_name):
            for (weight, (var_values, nodelist)) in enumerate(bucket_iterator):
                ...
                build_edges(graph, node_name, nodelist, color=colour, weight=weight)
    ...
    def parse_options(cls, options):
        ...
        for (opt, value) in cls.re_options.findall(options):
            if opt not in cls._searched_options:
                continue
            parts = tuple((tuple((convert(e) for e in part.split(','))) for part in value.split(' ')))
            try:
                while len(parts) == 1:
                    parts = parts[0]
...
class Node(object):
    ...
    def __repr__(self):
        return '"%s" [%s]' % (self.name, (', ').join(('%s="%s"' % item for item in self.args.iteritems())))
...
class Edge(object):
    ...
    def __eq__(self, other):
        return (
         other is not None and other.from_node, other.to_node == self.from_node, self.to_node)

    def __repr__(self):
        return '"%s" -> "%s" [%s]' % (self.from_node, self.to_node, (', ').join(('%s="%s"' % item for item in self.args.iteritems())))