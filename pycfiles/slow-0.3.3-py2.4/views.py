...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/pyexec/pydb/views.py
# Compiled at: 2006-03-22 06:41:51
...
ATTRIBUTE_XPATH = ETXPath("//{http://www.w3.org/1998/Math/MathML}ci[contains('node.', text())]")
DEPENDENCY_XPATH = ETXPath('//{http://www.w3.org/1998/Math/MathML}ci')
...
class PySlosl(object):
    ...
    class PyRanked(object):
        ...
        def __init__(self, ranked, _math_compile):
            ...
            self.parameters = [ _math_compile(param, '<rankparam%d>' % i) for (i, param) in enumerate(ranked.parameters) ]

    def __init__(self, slosl):
        self.xml_slosl = slosl
        self.attribute_dependencies = set((a.name() for a in ATTRIBUTE_XPATH.evaluate(slosl)))
...
class NodeView(ReflectiveObservable):
    ...
    def __init__(self, pyslosl, viewreg, name_override=None, **variable_initializations):
        ...
        self.parents = tuple((viewreg[parent] for parent in pyslosl.parents))
    ...
    class BackedAttributeDescriptor(object):
        ...
        def __new__(cls, attr_name):
            try:
                return cls.__instances[attr_name]
            except KeyError:
    ...
    def _select_nodes_into_buckets(self, spec, all_nodes, buckets, tuple_prefix=()):
        ...
        if node_rank:
            for value_tuple in self._iter_foreach_values(spec.foreachs, loop_value_dict):
                ...
                matches = [ build_view_node(node, loop_value_dict) for node in node_rank(candidate_iterator, loop_value_dict)
                          ]
                buckets[tuple_prefix + value_tuple] = matches

        for value_tuple in self._iter_foreach_values(spec.foreachs, loop_value_dict):
            candidate_iterator = ifilter(having, candidates)
            matches = [ build_view_node(node, loop_value_dict) for node in candidate_iterator ]
            buckets[tuple_prefix + value_tuple] = matches
...
class ViewFunctions(object):
    ...
    def _relative(self, count, rel_value, expression, distinct=False, reverse=False):
        ...
        if constant:
            ...
            def _eval_expression(var_values):
                return _abs(_pyeval(rel_value, _globals, var_values) - _pyeval(expression, _globals, var_values))
