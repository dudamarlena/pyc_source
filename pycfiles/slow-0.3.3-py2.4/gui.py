...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/gui.py
# Compiled at: 2006-03-24 08:45:14
try:
...
class RegExps(object):
    __module__ = __name__
    __SIMPLE_RE_DICT = {}
    RE_CLASS_NAME = __SIMPLE_RE_DICT['classname'] = '[A-Z][_a-zA-Z0-9]*'
    RE_IDENTIFIER = __SIMPLE_RE_DICT['identifier'] = '[a-z][_a-z0-9]*'
    RE_ASSIGNMENT = __SIMPLE_RE_DICT['assignment'] = '\\s*%(identifier)s\\s*(?:=\\s*(?:[^\n]+)\\s*)?' % __SIMPLE_RE_DICT
...
class MenuFunctions(object):
    ...
    def fileOpen(self):
        filename = qt.QFileDialog.getOpenFileName(qt.QString.null, self.FILE_FILTER, self, 'open file dialog', self.tr('Open file ...'))
    ...
    def fileSaveAs(self):
        filename = qt.QFileDialog.getSaveFileName(None, self.FILE_FILTER, self, 'save file dialog', self.tr('Save to file ...'))
        filename = qstrpy(filename)
        if not filename:
            return
    ...
    def exportFlat(self):
        ...
        filename = qt.QFileDialog.getSaveFileName(None, self.FLAT_FILE_FILTER, self, 'export file dialog', self.tr('Export to flat file ...'))
        filename = qstrpy(filename)
        if not filename:
            return
...
class OverlayDesigner_gui(MenuFunctions, AttributeEditor, MessageEditor, SLOSLEditor, EDSMEditor, TestEditor, OverlayDesignerMainWindow):
    __module__ = __name__
    ...
    def __init__(self, parent=None, name=None, fl=0):
        ...
        self.pref_dialog = PreferenceDialog(self, self.storePreferences, self.preferences)
        ...
        for supertype in (MenuFunctions, AttributeEditor, MessageEditor, SLOSLEditor, EDSMEditor, TestEditor):
...
def run():
    ...
    opparser.add_option('-l', '--logfile', dest='logfile', help='write log to FILE', metavar='FILE')
    opparser.add_option('-d', '--loglevel', dest='loglevel', help='set log level', default='ERROR')
    opparser.add_option('-L', '--language', dest='language', help='set user interface language')
    opparser.add_option('-P', '--no-psyco', dest='psyco_off', action='store_true', default=False, help='set user interface language')
    ...
    if options.logfile:
        logging.basicConfig(level=loglevel, filename=options.logfile, filemode='w')
    else:
