...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/pyexec/pydb/expression.py
...
import math
from specparser import Attribute, Variable, Function, XRange, Operator, BoolOperator, StatementElement
...
class UnknownReferenceError(Exception):
    ...
    def __init__(self, ref, valid):
        Exception.__init__(self, self.__ERROR_LINE % (ref, (',').join((str(v) for v in valid)), ('was', 'were')[(len(valid) > 1)]))
...
class CompiledExpression(object):
    ...
    BEGIN_TERM = Bracket('(')
    END_TERM = Bracket(')')
    __BASIC_GLOBALS = {'__builtins__': None, 'True': True, 'False': False, 'xrange': xrange}
    __allowed_builtins = ('abs', 'len', 'max', 'min')
    __supported_functions = [((mname, getattr(math, mname)) for mname in dir(math)), ((bname, __builtins__.get(bname, None)) for bname in __allowed_builtins),
     [
      ('round',
       lambda val, digits=0: int(round(val, digits)))]]
    __GLOBALS = dict(((fname, function) for (fname, function) in chain(*__supported_functions) if callable(function) if fname[0] != '_'))
    __valid_functions = set(__GLOBALS.keys())
    __GLOBALS.update(__BASIC_GLOBALS)

    def __init__(self, expression_tree, user_functions={}, all_functions={}, known_variables={}):
        ...
        self.expression = tuple(self._prefix2infix(expression_tree, known_variables))
    ...
    def substituted(self, constant_dict):
        ...
        for op in self.expression:
            if isinstance(op, Attribute):
                try:
                    op = repr(getattr(constant_dict[op.object_name], op.name))
                except (KeyError, AttributeError):
                    pass

            elif isinstance(op, Variable):
                try:
                    op = repr(constant_dict[op.name])
                except KeyError:
                    pass
                ...
                op = '%s ( %s )' % (op.name, (' , ').join((param.substituted(constant_dict) for param in op.function_parameters)))
            term.append(str(op))
    ...
    def _prefix2infix(self, prefix_term, known_variables):
        ...
        if isinstance(prefix_term, Attribute):
            ...
            if hasattr(prefix_term, 'clone'):
                ...
                yield prefix_term
        elif len(prefix_term) == 0:
            yield True
        elif len(prefix_term) == 1:
            for elem in self._prefix2infix(prefix_term[0], known_variables):
                yield elem

        elif len(prefix_term) == 2:
            ...
            if operator == 'in' and isinstance(prefix_term[2], XRange):
                ...
                params = tuple((self._prefix2infix(term, known_variables) for term in xrange_obj.function_parameters))
                cmp_val = tuple(self._prefix2infix(prefix_term[1], known_variables))
                for elem in chain((b_open, b_open), params[0], (Operator('<='),), cmp_val, (
                 b_close, BoolOperator('and'), b_open), cmp_val, (Operator('<'),), params[1], (
                 b_close, b_close)):
                    yield elem
    ...
    def _collect_dependencies_and_compile_functions(self, expression, known_variables):
        ...
        for elem in expression:
            ...
            if hasattr(elem, 'function_parameters'):
                ...
                for param in elem.function_parameters:
                    cparam = CompiledExpression(param, all_functions=self.functions, known_variables=known_variables)
        ...
        return {Attribute: frozenset(attributes), Function: frozenset(functions), Variable: frozenset(variables)}
