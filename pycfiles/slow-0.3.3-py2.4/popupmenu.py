...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/popupmenu.py
...
class MenuProvider(object):
    __module__ = __name__
    POPUP_TITLE_TEMPLATE = '<b>%s</b>'

    def __init__(self, parent=None, title=None, title_template=POPUP_TITLE_TEMPLATE, checkable=False):
        self.__title = title
    ...
    def _build_flag_menu_area(self, menu, editor, model, flag_list, exclusive=True):
        ...
        actions = [ FlagMaintainerAction(parent, model, flag_name, translation) for (flag_name, translation) in flag_list ]
        if len(actions) == 1:
