...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/slow/qtgui/slosl_editor.py
...
term_parsers.register_converter('slosl_foreach_list', SloslForeachListParser().p_foreach_list())
...
tree_converters.register_converter('slosl_foreach_list', SloslForeachListSerializer())
...
class SLOSLEditor(object):
    ...
    def copy_slosl_from_model(self, model=None):
        ...
        def build_assign(t):
            name = unicode(t[0])
            if t[1]:
                return '%s=%s' % (name, build_term(t[1]))
            else:
                return name
        ...
        self.slosl_with.setText(pyqstr((', ').join(imap(build_assign, model.withs))))
        ranked = model.ranked
        ...
        for (variable_name, variable_declaration) in foreachs:
            QListViewItem(foreach_list, variable_name, build_qterm(variable_declaration, 'slosl_foreach_list'))
    ...
    def copy_slosl_to_model(self, model=None):
        ...
        try:
            ...
            parents = filter(None, (name.strip() for name in qstrpy(self.slosl_from.text()).split(',')))
            if parents:
            ...
            if ranking_function_name:
                ...
                for (i, field) in islice(enumerate((self.slosl_rank_count, self.slosl_rank_expression, self.slosl_rank_compare)), ranked.function_parameter_count()):
    ...
    def slosl_foreach_apply_button_clicked(self):
        ...
        if item:
            ...
            item = QListViewItem(self.slosl_foreach_list, name, declaration)
