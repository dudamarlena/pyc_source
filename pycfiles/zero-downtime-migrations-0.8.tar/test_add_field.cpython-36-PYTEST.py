...
# Size of source mod 2**32: 17022 bytes
...
def test_sqlmigrate_add_field_working():
    ...
    with schema_editor(connection=connection, collect_sql=True) as (editor):
        ...
        if not @py_assert3:
            @py_format6 = @pytest_ar._call_reprcompare(('==', ), (@py_assert3,), ('%(py2)s\n{%(py2)s = %(py0)s.collected_sql\n} == %(py5)s', ), (@py_assert1, @py_assert4)) % {'py0':@pytest_ar._saferepr(editor) if 'editor' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(editor) else 'editor',  'py2':@pytest_ar._saferepr(@py_assert1),  'py5':@pytest_ar._saferepr(@py_assert4)}
...
def test_add_bool_field_no_existed_objects_success():
    ...
    if not @py_assert1:
        @py_format3 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py2)s', ), (queries, expected_queries)) % {'py0':@pytest_ar._saferepr(queries) if 'queries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(queries) else 'queries',  'py2':@pytest_ar._saferepr(expected_queries) if 'expected_queries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(expected_queries) else 'expected_queries'}
