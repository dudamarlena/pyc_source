...
# Compiled at: 2018-01-17 06:14:16
...
def test_retry_with_drop_working(add_column):
    ...
    @py_assert2 = ["SELECT IS_NULLABLE, DATA_TYPE, COLUMN_DEFAULT from information_schema.columns where table_name = 'test_app_testmodel' and column_name = 'bool_field';", 'ALTER TABLE "test_app_testmodel" DROP COLUMN "bool_field" CASCADE', 'ALTER TABLE "test_app_testmodel" ADD COLUMN "bool_field" boolean NULL', 'ALTER TABLE "test_app_testmodel" ALTER COLUMN "bool_field" SET DEFAULT true', "SELECT reltuples::BIGINT FROM pg_class WHERE relname = 'test_app_testmodel';", 'SELECT COUNT(*) FROM test_app_testmodel;', 'ALTER TABLE "test_app_testmodel" ALTER COLUMN "bool_field" SET NOT NULL', 'ALTER TABLE "test_app_testmodel" ALTER COLUMN "bool_field" DROP DEFAULT']
    @py_assert1 = queries == @py_assert2
    if not @py_assert1:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py3)s', ), (queries, @py_assert2)) % {'py0':@pytest_ar._saferepr(queries) if 'queries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(queries) else 'queries',  'py3':@pytest_ar._saferepr(@py_assert2)}
...
def test_retry_with_choice_working(add_column):
    ...
    calls = [
     call('It look like column "bool_field" in table "test_app_testmodel" already exist with following parameters: TYPE: "boolean", DEFAULT: "None", NULLABLE: "YES".', ('abort migration',
    ...
    @py_assert2 = ["SELECT IS_NULLABLE, DATA_TYPE, COLUMN_DEFAULT from information_schema.columns where table_name = 'test_app_testmodel' and column_name = 'bool_field';", 'ALTER TABLE "test_app_testmodel" ALTER COLUMN "bool_field" SET NOT NULL', 'ALTER TABLE "test_app_testmodel" ALTER COLUMN "bool_field" DROP DEFAULT']
    @py_assert1 = queries == @py_assert2
    if not @py_assert1:
        @py_format4 = @pytest_ar._call_reprcompare(('==', ), (@py_assert1,), ('%(py0)s == %(py3)s', ), (queries, @py_assert2)) % {'py0':@pytest_ar._saferepr(queries) if 'queries' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(queries) else 'queries',  'py3':@pytest_ar._saferepr(@py_assert2)}
        @py_format6 = 'assert %(py5)s' % {'py5': @py_format4}
