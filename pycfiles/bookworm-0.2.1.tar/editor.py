...
# Embedded file name: build/bdist.linux-x86_64/egg/booki/channels/editor.py
...
def getAttachments(book_version):
    ...
    attachments = [ {'id': att.id, 'dimension': _getDimension(att), 'status': att.status.id, 'name': os.path.split(att.attachment.name)[1], 'created': str(att.created.strftime('%d.%m.%Y %H:%M:%S')), 'size': att.attachment.size} for att in book_version.getAttachments().order_by('attachment') if att.attachment
                  ]
    return attachments
...
def remote_init_editor(request, message, bookid, version):
    ...
    if request.user.username not in _onlineUsers:
        ...
        if profile:
            ...
            moodMessage = ''
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'user_add', 'username': request.user.username, 
           'mood': moodMessage})
    ...
    return {'licenses': licenses, 'chapters': chapters, 
       'metadata': metadata, 
       'hold': holdChapters, 
       'users': users, 
       'locks': locks, 
...
def remote_attachments_delete(request, message, bookid, version):
    ...
    if bookSecurity.isAdmin():
        for att_id in message['attachments']:
            ...
            import os.path
            log.logBookHistory(book=book, version=book_version, args={'filename': os.path.split(att.attachment.name)[1]}, user=request.user, kind='attachment_delete')
            att.delete()
...
def remote_chapter_status(request, message, bookid, version):
    ...
    sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_status', 'chapterID': message['chapterID'], 
...
def remote_change_status(request, message, bookid, version):
    ...
    try:
        chapter.save()
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'change_status', 'chapterID': message['chapterID'], 
           'statusID': int(message['statusID']), 
           'username': request.user.username})
        sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
...
def remote_chapter_save(request, message, bookid, version):
    ...
    if message.get('minor', False) != True:
        history = logChapterHistory(chapter=chapter, content=message['content'], user=request.user, comment=message.get('comment', ''), revision=chapter.revision + 1)
        logBookHistory(book=chapter.book, version=book_version, chapter=chapter, chapter_history=history, user=request.user, args={'comment': message.get('comment', ''), 'author': message.get('author', ''), 
           'authorcomment': message.get('authorcomment', '')}, kind='chapter_save')
        chapter.revision += 1
...
def remote_chapter_rename(request, message, bookid, version):
    ...
    try:
        ...
        logBookHistory(book=chapter.book, version=book_version, chapter=chapter, user=request.user, args={'old': oldTitle, 'new': message['chapter']}, kind='chapter_rename')
        sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
           'message': 'User %s has renamed chapter "%s" to "%s".' % (request.user.username, oldTitle, message['chapter'])}, myself=True)
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_status', 'chapterID': message['chapterID'], 
           'status': 'normal', 
           'username': request.user.username})
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_rename', 'chapterID': message['chapterID'], 
...
def remote_chapters_changed(request, message, bookid, version):
    ...
    book_version = getVersion(book, version)
    weight = len(lst)
    logBookHistory(book=book, version=book_version, user=request.user, kind='chapter_reorder')
    for chap in lst:
        if chap[0] == 's':
            ...
            try:
                ...
                m = models.BookToc(book=book, version=book_version, name='SOMETHING', chapter=chptr, weight=weight, typeof=1)
                m.save()
    ...
    if message['kind'] == 'remove':
        if type(message['chapter_id']) == type(' ') and message['chapter_id'][0] == 's':
            ...
            m.delete()
    sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapters_changed', 'ids': lst, 
       'hold_ids': lstHold, 
       'kind': message['kind'], 
...
def remote_chapter_split(request, message, bookid, version):
    ...
    book = models.Book.objects.get(id=bookid)
    book_version = getVersion(book, version)
    logBookHistory(book=book, version=book_version, user=request.user, kind='chapter_split')
    allChapters = []
    try:
    ...
    for chap in message['chapters']:
        chapter = models.Chapter(book=book, url_title=bookiSlugify(chap[0]), title=chap[0], status=s, content='<h1>%s</h1>%s' % (chap[0], chap[1]), created=datetime.datetime.now(), modified=datetime.datetime.now())
        chapter.save()
        if tocChapter:
            m = models.BookToc(book=book, chapter=chapter, name=chap[0], weight=0, typeof=1)
            m.save()
    ...
    chapters = getTOCForBook(book_version)
    holdChapters = getHoldChapters(book_version)
    sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_split', 'chapterID': message['chapterID'], 
       'chapters': chapters, 
       'hold': holdChapters, 
       'username': request.user.username}, myself=True)
    transaction.commit()
    return {}
...
def remote_create_chapter(request, message, bookid, version):
    ...
    chapter = models.Chapter(book=book, version=book_version, url_title=url_title, title=message['chapter'], status=s, content=content, created=datetime.datetime.now(), modified=datetime.datetime.now())
    try:
        chapter.save()
    except:
        transaction.rollback()
    ...
    tc = models.BookToc(version=book_version, book=book, name=message['chapter'], chapter=chapter, weight=1, typeof=1)
    try:
        tc.save()
    except:
        transaction.rollback()
    ...
    history = logChapterHistory(chapter=chapter, content=content, user=request.user, comment=message.get('comment', ''), revision=chapter.revision)
    logBookHistory(book=book, version=book_version, chapter=chapter, chapter_history=history, user=request.user, kind='chapter_create')
    transaction.commit()
    result = (
     chapter.id, chapter.title, chapter.url_title, 1, s.id)
    sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
       'message': 'User %s has created new chapter "%s".' % (request.user.username, message['chapter'])}, myself=True)
...
def copy_attachment(attachment, target_book):
    ...
    """
    import os.path
    att = models.Attachment(book=target_book, version=target_book.version, status=target_book.status)
...
def remote_clone_chapter(request, message, bookid, version):
    ...
    if len(list(ch)) > 0:
        ...
        chapter = models.Chapter(book=book, version=book_version, url_title=url_title, title=title, status=s, content=source_chapter.content, created=datetime.datetime.now(), modified=datetime.datetime.now())
        try:
            chapter.save()
        except:
            transaction.rollback()
        ...
        tc = models.BookToc(version=book_version, book=book, name=title, chapter=chapter, weight=1, typeof=1)
        try:
            tc.save()
        except:
            transaction.rollback()
        ...
        try:
            history = logChapterHistory(chapter=chapter, content=chapter.content, user=request.user, comment=message.get('comment', ''), revision=chapter.revision)
        except:
            transaction.rollback()
            import traceback
        ...
        try:
            logBookHistory(book=book, version=book_version, chapter=chapter, chapter_history=history, user=request.user, kind='chapter_clone')
        except:
            transaction.rollback()
        ...
        try:
            ...
            target_attachmentnames = dict([ (att.getName(), att) for att in target_attachments
                                          ])
            name2copy = {}
            ...
            for e in tree.iter():
                ...
                if src is not None:
                    ...
                    if dirname == 'static':
                        ...
                        if att and name not in name2copy:
        ...
        sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
           'message': 'User %s has cloned chapter "%s" from book "%s".' % (request.user.username, chapter.title, source_book.title)}, myself=True)
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_create', 'chapter': result}, myself=True)
        return {'created': True}
...
def remote_publish_book(request, message, bookid, version):
    ...
    book = models.Book.objects.get(id=bookid)
    sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
       'message': '"%s" is being published.' % (book.title,)}, myself=True)
    import urllib2, urllib
    publishMode = message.get('publish_mode', 'epub')
    destination = 'nowhere'
    if message.get('is_archive', False):
        destination = 'archive.org'
    args = {'book': book.url_title.encode('utf8'), 'project': 'export', 
       'mode': publishMode, 
    ...
    def _isSet(name, default=None):
        if message.get(name, None):
            if name == 'grey_scale':
                args['grey_scale'] = 'yes'
            elif type(message.get(name)) == type(' '):
...
def remote_create_section(request, message, bookid, version):
    ...
    book = models.Book.objects.get(id=bookid)
    book_version = getVersion(book, version)
    ch = models.BookToc.objects.filter(book=book, version=book_version, name=message['chapter'], typeof=0)
    if len(list(ch)) > 0:
        return {'created': False}
    else:
        c = models.BookToc(book=book, version=book_version, name=message['chapter'], chapter=None, weight=0, typeof=0)
        result = True
        try:
            c.save()
        except:
            ...
            logBookHistory(book=book, version=book_version, user=request.user, args={'title': message['chapter']}, kind='section_create')
            transaction.commit()
            result = (
             's%s' % c.id, c.name, None, c.typeof)
            sputnik.addMessageToChannel(request, '/chat/%s/' % bookid, {'command': 'message_info', 'from': request.user.username, 
               'message': 'User %s has created new section "%s".' % (request.user.username, message['chapter'])}, myself=True)
            sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_create', 'chapter': result, 
               'typeof': c.typeof}, myself=True)
...
def remote_get_history(request, message, bookid, version):
    ...
    for entry in book_history:
        if entry.kind in (1, 2, 3) and entry.chapter:
            history.append({'chapter': entry.chapter.title, 'chapter_url': entry.chapter.url_title, 
            ...
            history.append({'modified': entry.modified.strftime('%d.%m.%Y %H:%M:%S'), 'version': parseJSON(entry.args), 
               'user': entry.user.username, 
               'kind': temp.get(entry.kind, '')})
        elif entry.kind in (13, 14):
            history.append({'modified': entry.modified.strftime('%d.%m.%Y %H:%M:%S'), 'args': parseJSON(entry.args), 
               'user': entry.user.username, 
               'kind': temp.get(entry.kind, '')})
        else:
            history.append({'modified': entry.modified.strftime('%d.%m.%Y %H:%M:%S'), 'description': entry.args, 
               'user': entry.user.username, 
               'kind': temp.get(entry.kind, '')})
...
def remote_revert_revision(request, message, bookid, version):
    ...
    history = logChapterHistory(chapter=chapter, content=revision.content, user=request.user, comment='Reverted to revision %s.' % message['revision'], revision=chapter.revision + 1)
    logBookHistory(book=book, version=book_ver, chapter=chapter, chapter_history=history, user=request.user, args={}, kind='chapter_save')
    chapter.revision += 1
...
def remote_get_chapter_revision(request, message, bookid, version):
    ...
    try:
        ...
        return {'chapter': revision.chapter.title, 'chapter_url': revision.chapter.url_title, 
...
def create_new_version(book, book_ver, message, major, minor):
    ...
    """
    new_version = models.BookVersion(book=book, major=major, minor=minor, name=message.get('name', ''), description=message.get('description', ''))
    new_version.save()
    for toc in book_ver.getTOC():
        nchap = None
        if toc.chapter:
            chap = toc.chapter
            nchap = models.Chapter(version=new_version, book=book, url_title=chap.url_title, title=chap.title, status=chap.status, revision=chap.revision, content=chap.content)
            nchap.save()
        ntoc = models.BookToc(version=new_version, book=book, name=toc.name, chapter=nchap, weight=toc.weight, typeof=toc.typeof)
        ntoc.save()

    for chap in book_ver.getHoldChapters():
        c = models.Chapter(version=new_version, book=book, url_title=chap.url_title, title=chap.title, status=chap.status, revision=chap.revision, content=chap.content)
        c.save()

    for att in book_ver.getAttachments():
        a = models.Attachment(version=new_version, book=book, status=att.status)
...
def remote_create_major_version(request, message, bookid, version):
    ...
    try:
        ...
        logBookHistory(book=book, version=new_version, chapter=None, chapter_history=None, user=request.user, args={'version': new_version.getVersion()}, kind='major_version')
        transaction.commit()
...
def remote_create_minor_version(request, message, bookid, version):
    ...
    logBookHistory(book=book, version=new_version, chapter=None, chapter_history=None, user=request.user, args={'version': new_version.getVersion()}, kind='minor_version')
    transaction.commit()
    return {'version': new_version.getVersion()}
    return
    ...
    while True:
        ...
        if n == -1:
            ...
            if j == 0:
                ...
                tag = l[n:j]
                if tag.replace(' ', '').replace('/', '').lower() not in ('<br>', '<hr>'):
                    if n != 0:
                        out += '</span>'
...
def remote_settings_options(request, message, bookid, version):
    ...
    transaction.commit()
    return {'licenses': licenses, 'permission': book.permission, 
...
def remote_license_attributions(request, message, bookid, version):
    ...
    transaction.commit()
    return {'users': users, 'users_exclude': users_exclude}
...
def remote_settings_language_save(request, message, bookid, version):
    ...
    try:
        ...
        rtl = models.Info(book=book, name='{http://booki.cc/}dir', kind=0, value_string=rtl_value)
        rtl.save()
...
def _remote_roles_save(request, message, bookid, version):
    ...
    for userName in newUsers:
        try:
            u = User.objects.get(username=userName)
            up = models.BookiPermission(book=book, user=u, permission=1)
            up.save()
...
def remote_roles_add(request, message, bookid, version):
    ...
    try:
        ...
        up = models.BookiPermission(book=book, user=u, permission=message['role'])
        up.save()
    except User.DoesNotExist:
        pass
...
def remote_roles_delete(request, message, bookid, version):
    ...
    book_ver = getVersion(book, version)
    try:
        for up in models.BookiPermission.objects.filter(book=book, user__username=message['username'], permission=message['role']):
            up.delete()
...
def remote_book_status_create(request, message, bookid, version):
    ...
    if not bookSecurity.isAdmin():
        ...
        try:
            bs = models.BookStatus(book=book, name=strip_tags(message['status_name'].strip()), weight=0)
            bs.save()
        except:
            transaction.rollback()
        else:
        ...
        sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_status_changed', 'statuses': allStatuses}, myself=False)
        return {'status': True, 'status_id': status_id, 
...
def remote_book_status_remove(request, message, bookid, version):
    ...
    try:
        ...
        if len(list(models.Chapter.objects.filter(status=up, book=book))) == 0 and len(list(models.Attachment.objects.filter(status=up, version__book=book))) == 0 and book.status != up:
            up.delete()
        else:
            result = False
...
def remote_book_status_order(request, message, bookid, version):
    ...
    allStatuses = [ (status.id, status.name) for status in models.BookStatus.objects.filter(book=book).order_by('-weight') ]
    sputnik.addMessageToChannel(request, '/booki/book/%s/%s/' % (bookid, version), {'command': 'chapter_status_changed', 'statuses': allStatuses}, myself=False)
    return {'status': True, 'statuses': allStatuses}
...
def remote_get_wizzard(request, message, bookid, version):
    ...
    options = []
    try:
        pw = models.PublishWizzard.objects.get(book=book, user=request.user, wizz_type=message['wizzard_type'])
        try:
...
def remote_set_wizzard(request, message, bookid, version):
    ...
    pw, created = models.PublishWizzard.objects.get_or_create(book=book, user=request.user, wizz_type=message.get('wizzard_type', 'book'))
...
def remote_publish_book2(request, message, bookid, version):
    ...
    options = []
    try:
        pw = models.PublishWizzard.objects.get(book=book, user=request.user, wizz_type=message.get('publish_mode', 'epub'))
        try:
            options = simplejson.loads(pw.wizz_options)
        except:
            options = PUBLISH_OPTIONS[message.get('wizzard_type', 'book')]
    ...
    publishOptions = {'ebook': 'epub', 'book': 'book', 
       'odt': 'openoffice', 
       'newpaper': 'pdf', 
       'lulu': 'book', 
       'pdf': 'web'}
    licenses = {'PD': 'public domain', 'MIT': 'MIT', 
       'CC0': 'CC-BY', 
       'CC BY': 'CC-BY', 
       'CC BY-SA': 'CC-BY', 
       'CC BY-ND': 'CC BY', 
    ...
    destination = 'nowhere'
    args = {'book': book.url_title.encode('utf8'), 'license': licenses.get(book.license.abbrevation, 'GPL'), 
       'project': 'export', 
       'mode': publishMode, 
       'server': THIS_BOOKI_SERVER, 
       'destination': destination, 
