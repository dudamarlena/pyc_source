...
# [GCC 8.4.0]
# Embedded file name: /home/trhura/Code/pythonenv/lib/python3.7/site-packages/myanmar/converter.py
...
encoders = {'unicode':encodings.UnicodeEncoding(), 
...
def convert(text, fromenc, toenc):
    ...
    for syllable in iterator:
        ...
        if full_syllable in fromencoder.reverse_table:
            ...
            key = key[:key.find('_')] if '_' in key else key
...
def has_lower_marks(syllable, filters=[]):
    MAKRS = [
     'stack', 'wasway', 'yapin', 'yayit', 'hatoh', 'uVowel']
...
def has_upper_marks(syllable, filters=[]):
    MAKRS = [
     'kinzi', 'yapin', 'iVowel', 'aiVowel', 'anusvara']
    for mark in [m for m in MAKRS if m not in filters]:
        if mark in syllable:
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert between various Myanmar encodings')
    ...
    if args.fro not in get_supported_encodings():
        print('%s is not a supported encoding. Should be any of %s.' % (
