...
# Embedded file name: e:\01_work\15_phd\11_python_neural_mass_models\pyrates\pyrates\utility\grid_search.py
# Compiled at: 2019-07-11 07:42:40
# Size of source mod 2**32: 45959 bytes
__doc__ = 'Functions for performing parameter grid simulations with pyrates model_templates.\n'
import pandas as pd, numpy as np
from typing import Optional, Union
import os, sys, json, time as t, glob, getpass
...
def grid_search(circuit_template: Union[(CircuitTemplate, str)], param_grid: Union[(dict, pd.DataFrame)], param_map: dict, dt: float, simulation_time: float, inputs: dict, outputs: dict, sampling_step_size: Optional[float]=None, permute_grid: bool=False, init_kwargs: dict=None, **kwargs) -> pd.DataFrame:
    ...
    if type(circuit_template) is str:
        ...
        results = (net.run)(simulation_time=simulation_time, inputs=inputs, 
         outputs=outputs, 
         sampling_step_size=sampling_step_size, **kwargs)
...
class ClusterCompute:
    ...
    def cluster_connect(self, nodes: list):
        ...
        for node in nodes:
            ...
            if client is not None:
                ...
                self.clients.append({'paramiko_client':client, 
                 'node_name':node, 
                 'hardware':hw, 
    ...
    def spawn_thread(self, client, thread_kwargs):
        ...
        """
        t_ = Thread(name=(client['node_name']),
          target=(self.thread_master),
          args=(
         client, thread_kwargs))
        t_.start()
        return t_
...
class ClusterGridSearch(ClusterCompute):
    ...
    def run(self, circuit_template: str, params: Union[(dict, pd.DataFrame)], param_map: dict, dt: float, simulation_time: float, inputs: dict, outputs: dict, sampling_step_size: float, chunk_size: (int, list), worker_env: str, worker_file: str, result_kwargs: dict={}, config_kwargs: dict={}, add_template_info: bool=False, permute: bool=False, **kwargs) -> str:
        ...
        if not self.check_key_consistency(params, param_map):
            ...
            self.create_cgs_config(fp=config_file, circuit_template=circuit_template,
              param_map=param_map,
              dt=dt,
            ...
            with h5py.File(global_res_file, 'a') as (file):
                ...
                file.create_dataset('/Config/config_file', data=config_file)
                file.create_dataset('/Config/circuit_template', data=circuit_template)
                file.create_dataset('/Config/simulation_time', data=simulation_time)
                file.create_dataset('/Config/dt', data=dt)
                file.create_dataset('/Config/sampling_step_size', data=sampling_step_size)
            ...
            print(f"Done. Elapsed time: {t.time() - t0:.3f} seconds")
            print('')
            thread_kwargs = {'worker_env':worker_env, 
            ...
            print('')
            print('***WRITING RESULTS TO GLOBAL RESULT FILE***')
            t0 = t.time()
        ...
        while True:
            with self.lock:
                ...
                if remaining_params.empty:
                    ...
                    subgrid_idx += 1
                    local_res_file = f"{grid_res_dir}/CGS_result_{grid_name}_chunk_{chunk_idx:02}_idx_{param_idx[0]}-{param_idx[(-1)]}_temp.h5"
                    self.res_file_collection[chunk_idx] = local_res_file
                    t0 = t.time()
                    try:
                        print(f"[T]'{thread_name}': Starting remote computation...")
                        stdin, stdout, stderr = pm_client.exec_command((command + f" --config_file={config_file} --subgrid={subgrid_fp} --local_res_file={local_res_file} &>> {logfile}"),
                          get_pty=True)
    ...
    @staticmethod
    def create_cgs_config(fp: str, circuit_template: str, param_map: dict, dt: float, simulation_time: float, inputs: dict, outputs: dict, sampling_step_size: float, add_kwargs: dict):
        ...
        """
        config_dict = {'circuit_template':circuit_template, 
         'param_map':param_map, 
         'dt':dt, 
         'simulation_time':simulation_time, 
         'outputs':outputs, 
...
class StreamTee(object):
    ...
    def __init__(self, stream1, stream2fp):
        ...
        self._StreamTee__missing_method_name = None

    def __getattribute__(self, name):
        return object.__getattribute__(self, name)

    def __getattr__(self, name):
        self._StreamTee__missing_method_name = name
        return getattr(self, '__methodmissing__')

    def __methodmissing__(self, *args, **kwargs):
        callable2 = getattr(self.stream2, self._StreamTee__missing_method_name)
        callable2(*args, **kwargs)
        callable1 = getattr(self.stream1, self._StreamTee__missing_method_name)
