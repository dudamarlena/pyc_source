...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/tests/test_factory.py
...
class FactoryTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.form_data = {'subscribe-form-uuid': self.form_fields['uuid'].initial, 
        ...
        self.form_files = {'subscribe-form-id-copy': SimpleUploadedFile('test.txt', 'Test')}
        self.upload_path = os.path.join(settings.MEDIA_ROOT, self.form_data['subscribe-form-upload-to'])
        ...
        for value in self.simpleformfield_data.values():
            ...
            for k, v in value.items():
                if k in ('label', 'help_text', 'required'):
                    self.assertEqual(v, getattr(self.form_factory.fields[value['slug']], k))

        form_factory = self.simpleform.as_form(data=self.form_data, files=self.form_files)
    ...
    def test_save(self):
        ...
        self.assertTrue(form_factory.is_valid())
        form_factory.save()
        form_store = models.FormData.objects.get(uuid=form_factory.fields['uuid'].initial)
        ...
        for file_field in self.form_files:
            self.assertTrue(os.path.exists(os.path.join(self.upload_path, self.form_files[file_field].name)))
    ...
    def test_incremental_filenames(self):
        ...
        for file_field in self.form_files:
            self.assertTrue(os.path.exists(os.path.join(self.upload_path, self.form_files[file_field].name)))
            self.assertTrue(os.path.exists(os.path.join(self.upload_path, 'test_1.txt')))
