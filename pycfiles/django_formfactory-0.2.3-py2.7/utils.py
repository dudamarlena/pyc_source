...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/utils.py
...
def order_by_through(queryset, through_model_name, filter_on, filter_instance, ordered_object_type, order_by='order'):
    ...
    return_set = None
    try:
        return_set = queryset.order_by(through_model_name.lower())
        len(return_set)
    except AttributeError as e:
        from formfactory import models
        model = getattr(models, through_model_name)
        filter_data = {filter_on: filter_instance}
        return_set = [ getattr(instance, ordered_object_type) for instance in model.objects.filter(**filter_data).order_by(order_by)
                     ]
