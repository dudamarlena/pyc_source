...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/tests/test_views.py
...
class ViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.form_postdata = {'subscribe-form-uuid': self.form_fields['uuid'].initial, 
        ...
        self.loginform_postdata = {'login-form-uuid': self.form_fields['uuid'].initial, 
    ...
    def test_detail(self):
        ...
        response = self.client.post(self.simpleform.get_absolute_url(), data=self.form_postdata, follow=True)
        original_form_field = response.context['form'].fields
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Success')
        self.assertNotContains(response, 'Failure')
        self.assertNotContains(response, 'This field is required.')
        form_store = models.FormData.objects.get(uuid=original_form_field['uuid'].initial)
...
class LoginViewDetailTestCase(TestCase):
    ...
    def test_detail(self):
        ...
        response = self.client.post(self.loginform.get_absolute_url(), data=self.loginform_postdata, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Success')
        self.assertNotContains(response, 'Failure')
        self.assertNotContains(response, 'This field is required.')
        self.assertContains(response, 'Login Templete Override')
        self.assertContains(response, self.loginform_data['submit_button_text'])
...
class WizardViewTestCase(TestCase):
    ...
    def get_first_step(self):
        response = self.client.get(reverse('formfactory:wizard-detail', kwargs={'slug': self.wizard.slug, 
           'step': 'subscribe-form'}))
        return response

    def post_first_step(self):
        simple_form_uuid_field = self.simpleform.as_form().fields['uuid'].initial
        post_data = {'FactoryWizardView-test-wizard-current_step': 'subscribe-form', 
           'subscribe-form-salutation': 'Mr', 
           ...
           'subscribe-form-upload-to': 'uploads/test', 
           'subscribe-form-id-copy': SimpleUploadedFile('test.txt', 'Test')}
        response = self.client.post(reverse('formfactory:wizard-detail', kwargs={'slug': self.wizard.slug, 
           'step': 'subscribe-form'}), data=post_data, follow=True)
        return response

    def post_second_step(self):
        login_form_uuid_field = self.loginform.as_form().fields['uuid'].initial
        post_data = {'FactoryWizardView-test-wizard-current_step': 'login-form', 
        ...
        response = self.client.post(reverse('formfactory:wizard-detail', kwargs={'slug': self.wizard.slug, 
           'step': 'login-form'}), data=post_data, follow=True)
        return response
    ...
    def test_form_actions_at_done_step(self):
        ...
        """
        action_data = {'action': 'formfactory.tests.actions.dummy_wizard_action'}
        action = models.Action.objects.create(**action_data)
        wizard_actionthrough_data = {'action': action, 
           'wizard': self.wizard, 
           'order': 0}
        ...
        self.assertTrue(models.FormDataItem.objects.filter(form_data=form_data, value='Mr').exists())
        self.assertTrue(models.FormDataItem.objects.filter(form_data=form_data, value='Tester').exists())
        self.assertTrue(models.FormDataItem.objects.filter(form_data=form_data, value='tester@example.com').exists())
...
class FormHasRequestObjectTestCase(TestCase):
    ...
    def test_request_is_passed_to_forms(self):
        response = self.client.get(reverse('formfactory:form-detail', args=[self.simpleform.slug]))
