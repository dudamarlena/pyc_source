...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/factory.py
# Compiled at: 2017-11-29 05:18:58
...
class FormFactory(forms.Form):
    ...
    """
    uuid = forms.UUIDField(initial=unicode(uuid4()), widget=forms.HiddenInput)
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_group in defined_field_groups:
            fields = utils.order_by_through(field_group.fields.all(), 'FieldGroupThrough', 'field_group', field_group, 'field')
            ...
            for field in fields:
                ...
                for validator in field.additional_validators.all():
                    additional_validators.append(validator.as_function)
                ...
                field_args.update({'label': field.label, 
                   'initial': field.initial or self.initial.get(field.slug), 
                   'required': field.required, 
                   'disabled': field.disabled, 
                   'help_text': field.help_text, 
                   'validators': additional_validators, 
                   'error_messages': dict((m.key, m.value) for m in field.error_messages.all())})
                if field.widget:
                ...
                if field.choices.exists():
                    choices = tuple((c.value, c.label) for c in field.choices.all())
                    try:
                        self.fields[field.slug].choices = choices
                    except TypeError:
                        pass

                if field.model_choices and hasattr(field.model_choices, 'items') and field.model_choices.items.exists():
                    choices = tuple((c.value, c.label) for c in field.model_choices.items.all())
                    try:
            ...
            for name in fieldnames:
                ...
                if not bf.is_hidden:
                    ...
                    if field.help_text:
                        ...
                        help_text = ''
                    output.append(normal_row % {'errors': force_text(bf_errors), 
    ...
    def save(self, *args, **kwargs):
        ...
        actions = utils.order_by_through(self.actions, 'FormActionThrough', 'form', form, 'action')
