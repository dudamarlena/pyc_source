...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/views.py
...
class FactoryFormView(generic.FormView):
    ...
    @generic_deprecation('The form_detail_<slug>.html pattern will be deprecated in the upcoming version 1.0, make use of paragraph fields on forms')
    ...
    def form_valid(self, form):
        ...
        messages.add_message(self.request, messages.SUCCESS, self.form_object.success_message)
        return super(FactoryFormView, self).form_valid(form)

    def form_invalid(self, form):
        messages.add_message(self.request, messages.ERROR, self.form_object.failure_message)
        return super(FactoryFormView, self).form_invalid(form)

    def get_form(self, form_class=None):
        self.form_object = get_object_or_404(Form, slug=self.kwargs.get('slug', self.form_slug))
    ...
    def get_success_url(self):
        ...
        if ajax == 'true':
            url = '%s?ajax=true'
        redirect_url = self.form_object.redirect_to or self.request.GET.get(SETTINGS['redirect-url-param-name']) or self.redirect_to
        if redirect_url:
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'form_object': self.form_object})
...
class FactoryWizardView(NamedUrlSessionWizardView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.form_list_map = {}
        self.form_object_list = utils.order_by_through(self.wizard_object.forms.all(), 'WizardFormThrough', 'wizard', self.wizard_object, 'form')
        form_list = []
    ...
    def done(self, form_list, form_dict, **kwargs):
        ...
        for action in self.wizard_object.actions.all():
            ...
            action_params.update(dict((obj.key, obj.value) for obj in action.params.all()))
    ...
    def get(self, *args, **kwargs):
        ...
        if step_name is None:
            self.storage.extra_data[redirect_name] = self.request.GET.get(redirect_name)
        return super(FactoryWizardView, self).get(*args, **kwargs)

    def get_context_data(self, form, **kwargs):
        context = super(FactoryWizardView, self).get_context_data(form, **kwargs)
    ...
    @generic_deprecation('The wizard_detail_<slug>.html pattern will be deprecated in the upcoming version 1.0, make use of paragraph fields on forms')
    def get_template_names(self):
        return [
    ...
    def get_success_url(self):
        stored_redirect = self.storage.extra_data.get(SETTINGS['redirect-url-param-name'])
        redirect_url = self.wizard_object.redirect_to or stored_redirect or self.redirect_to
        if redirect_url:
            return redirect_url
