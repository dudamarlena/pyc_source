...
# [GCC 8.4.0]
# Embedded file name: /home/altus/gitArchives/django/_instances/django-formfactory/formfactory/tests/test_models.py
...
class ModelTestCase(TestCase):
    ...
    def test_field_constant(self):
        ...
        self.assertIn(self.action_data['action'], [ a[0] for a in models.FORM_ACTIONS ])
        self.assertIn(self.dummy_validator, [ v[0] for v in models.ADDITIONAL_VALIDATORS ])
    ...
    def test_form(self):
        ...
        self.assertEqual(self.form.get_absolute_url(), '/formfactory/%s/' % self.form.slug)
        self.assertEqual(unicode(self.form), 'Form 1')
    ...
    def test_wizard(self):
        ...
        if django.get_version() in ('1.9', '1.9.1'):
            forms = [ instance.form for instance in models.WizardFormThrough.objects.filter(wizard=self.wizard).order_by('order')
                    ]
            self.assertQuerysetEqual(forms, [
             repr(self.simpleform), repr(self.loginform)])
        else:
            self.assertQuerysetEqual(self.wizard.forms.all().order_by('wizardformthrough'), [
             repr(self.simpleform), repr(self.loginform)])
        self.assertEqual(self.wizard.get_absolute_url(), '/formfactory/wizard/%s/' % self.wizard.slug)