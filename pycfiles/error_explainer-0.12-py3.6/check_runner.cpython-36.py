...
# Compiled at: 2020-04-27 11:43:51
# Size of source mod 2**32: 16669 bytes
import ast
from collections import defaultdict
from typing import NoReturn, Any, List, Callable
from error_explainer.check import Check
from error_explainer.checks import check_quote_error, check_invalid_function_def, check_invalid_function_name, check_missing_function_def_parts, check_missing_brackets, check_miss_matched_bracket_type, check_print_missing_brackets, check_missing_colon, check_invalid_assignment_expr, BracketErrorType, check_docstring_quote_error, check_invalid_indentation, check_coma_used_instead_of_period
    ...
    if results is not None:
        if len(results) == 2:
            if any(leaf for leaf in results if leaf.get_code().strip() == "'"):
                if any(leaf for leaf in results if leaf.get_code().strip() == '"'):
                    add_message('miss_matched_quotes',
...
def missing_brackets_check(filename: str) -> NoReturn:
    ...
    for error in found_errors:
        ...
        if normal_brackets + square_brackets + curly_brackets != 0 and check_miss_matched_bracket_type(filename) is None:
    ...
    for error in found_errors:
        ...
        if normal_brackets + square_brackets + curly_brackets != 0:
            ...
            if miss_matched_bracket_type_res == BracketErrorType.NORMAL_SQUARE:
                ...
                if miss_matched_bracket_type_res == BracketErrorType.NORMAL_CURLY:
