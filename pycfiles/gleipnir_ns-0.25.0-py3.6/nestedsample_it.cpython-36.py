...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gleipnir/pysb_utilities/nestedsample_it.py
# Compiled at: 2019-07-02 17:26:43
# Size of source mod 2**32: 23637 bytes
...
class NestedSampleIt(object):
    ...
    def __init__(self, model, observable_data, timespan, solver=pysb.simulator.ScipyOdeSimulator, solver_kwargs=None, nest_it=None, builder=None):
        ...
        if solver_kwargs is None:
            ...
            for observable_key in observable_data.keys():
                self._like_data[observable_key] = norm(loc=(observable_data[observable_key][0]), scale=(observable_data[observable_key][1]))
    ...
    def __call__(self, ns_version='gleipnir-classic', ns_population_size=1000, ns_kwargs=None, log_likelihood_type='logpdf'):
        ...
        if ns_kwargs is None:
            ...
            if log_likelihood_type == 'mse':
                ...
                if ns_version == 'gleipnir-classic':
                    ...
                    sampler = MetropolisComponentWiseHardNSRejection(iterations=10, burn_in=10,
                      tuning_cycles=1)
                    stopping_criterion = NumberOfIterations(10 * population_size)
                    nested_sampler = NestedSampling(sampled_parameters=(self._sampled_parameters), loglikelihood=loglikelihood,
                      sampler=sampler,
                    ...
                    if ns_version == 'multinest':
                        ...
                        nested_sampler = MultiNestNestedSampling(sampled_parameters=self._sampled_parameters, loglikelihood=loglikelihood, 
                         population_size=population_size, **self._ns_kwargs)
                    else:
                        if ns_version == 'polychord':
                            from gleipnir.polychord import PolyChordNestedSampling
                            nested_sampler = PolyChordNestedSampling(sampled_parameters=(self._sampled_parameters), loglikelihood=loglikelihood,
                              population_size=population_size)
                        elif ns_version == 'dnest4':
                            from gleipnir.dnest4 import DNest4NestedSampling
                            if 'num_steps' not in list(self._ns_kwargs.keys()):
                                self._ns_kwargs['num_steps'] = 100 * population_size
                            nested_sampler = DNest4NestedSampling(sampled_parameters=sampled_parameters, loglikelihood=loglikelihood, 
                             population_size=population_size, **self._ns_kwargs)
...
if __name__ == '__main__':
    ...
    with open(model_file, 'r') as (file_obj):
        for line in file_obj:
            words = line.split()
            if len(words) > 1 and words[0] == '#NESTEDSAMPLE_IT':
