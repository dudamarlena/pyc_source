...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/emulator/paho/mqtt/client.py
# Compiled at: 2018-09-18 06:57:31
"""
This is an MQTT v3.1 client module. MQTT is a lightweight pub/sub messaging
protocol that is easy to implement and suitable for low powered devices.
"""
import collections, errno, platform, random, select, socket
try:
    import ssl
except ImportError:
    ssl = None

import struct, sys, threading, time, uuid, base64, string, hashlib, logging
try:
    ...
    * Use disconnect() to disconnect from the broker
    ...
    def reconnect(self):
        ...
        if self._ssl:
            ...
            try:
                sock = self._ssl_context.wrap_socket(sock, server_hostname=self._host, do_handshake_on_connect=False)
            except ssl.CertificateError:
                raise
            except ValueError:
                sock = self._ssl_context.wrap_socket(sock, do_handshake_on_connect=False)

            if hasattr(self._ssl_context, 'check_hostname') and self._ssl_context.check_hostname:
                verify_host = False
    ...
    def loop_forever(self, timeout=1.0, max_packets=1, retry_first_connection=False):
        ...
        while run:
            ...
            while rc == MQTT_ERR_SUCCESS:
                ...
                if self._thread_terminate is True and self._current_out_packet is None and len(self._out_packet) == 0 and len(self._out_messages) == 0:
                    rc = 1
                    run = False
        ...
        while self._current_out_packet:
            ...
            if write_length > 0:
                ...
                if packet['to_process'] == 0:
                    if packet['command'] & 240 == PUBLISH and packet['qos'] == 0:
                        with self._callback_mutex:
                            if self.on_publish:
                                with self._in_callback:
    ...
    def _filter_wildcard_len_check(sub):
        if len(sub) == 0 or len(sub) > 65535 or any('+' in p or '#' in p for p in sub.split('/') if len(p) > 1) or '#/' in sub:
            return MQTT_ERR_INVAL
        return MQTT_ERR_SUCCESS
    ...
    def _send_publish(self, mid, topic, payload='', qos=0, retain=False, dup=False, info=None):
        assert not isinstance(topic, unicode) and not isinstance(payload, unicode) and payload is not None
        if self._sock is None:
            return MQTT_ERR_NO_CONN
        else:
            command = PUBLISH | (dup & 1) << 3 | qos << 1 | retain
            packet = bytearray()
            packet.append(command)
            payloadlen = len(payload)
            remaining_length = 2 + len(topic) + payloadlen
            if payloadlen == 0:
                self._easy_log(MQTT_LOG_DEBUG, "Sending PUBLISH (d%d, q%d, r%d, m%d), '%s' (NULL payload)", dup, qos, retain, mid, topic)
            else:
                self._easy_log(MQTT_LOG_DEBUG, "Sending PUBLISH (d%d, q%d, r%d, m%d), '%s', ... (%d bytes)", dup, qos, retain, mid, topic, payloadlen)
            if qos > 0:
                remaining_length += 2
    ...
    def _send_connect(self, keepalive, clean_session):
        ...
        protocol = 'MQTT' if proto_ver >= MQTTv311 else 'MQIsdp'
            ...
            for m in messages:
                if m.timestamp + self._message_retry < now:
                    if m.state == mqtt_ms_wait_for_puback or m.state == mqtt_ms_wait_for_pubrec:
                        m.timestamp = now
                        m.dup = True
                        self._send_publish(m.mid, m.topic.encode('utf-8'), m.payload, m.qos, m.retain, m.dup)
    ...
    def _handle_connack(self):
        if self._in_packet['remaining_length'] != 2:
            ...
            if result == CONNACK_REFUSED_PROTOCOL_VERSION and self._protocol == MQTTv311:
                ...
                return self.reconnect()
            if result == CONNACK_REFUSED_IDENTIFIER_REJECTED and self._client_id == '':
            ...
            if result == 0:
                ...
                with self._out_message_mutex:
                    for m in self._out_messages:
                        ...
                        if m.qos == 0:
                            with self._in_callback:
                                rc = self._send_publish(m.mid, m.topic.encode('utf-8'), m.payload, m.qos, m.retain, m.dup)
                            if rc != 0:
                                return rc
                        elif m.qos == 1:
    ...
    def _reconnect_wait(self):
        ...
        while self._state != mqtt_cs_disconnecting and not self._thread_terminate and remaining > 0:
...
class WebsocketWrapper(object):
            ...
            while True:
                ...
                if byte == '\n':
                    if len(self._readbuffer) > 2:
                        if 'connection' in str(self._readbuffer).lower().encode('utf-8'):
                            if 'upgrade' not in str(self._readbuffer).lower().encode('utf-8'):
                                raise WebsocketConnectionError('WebSocket handshake error, connection not upgraded')
                            else:
                                has_upgrade = True
                        if 'sec-websocket-accept' in str(self._readbuffer).lower().encode('utf-8'):
    ...
    def _create_frame(self, opcode, data, do_masking=1):
        ...
        elif length < 9223372036854775809:
