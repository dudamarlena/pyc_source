...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/rad/rules.py
...
class RuleList(list):
    ...
    def __init__(self, names, config_dir=CONFIG, ext='.yaml'):
        ...
        if not os.path.exists(config_dir):
            os.mkdir(config_dir)
        self.configs = [ os.path.join(config_dir, f) for f in os.listdir(config_dir) if f.endswith(ext)
                       ]
    ...
    def print_listing(self):
        ...
        colorers = [ (self.path_to_name(config), self.get_rules(self.path_to_name(config))) for config in self.configs
                   ]
