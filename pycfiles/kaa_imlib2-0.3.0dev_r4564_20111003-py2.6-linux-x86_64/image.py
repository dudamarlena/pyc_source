...
# [GCC 8.4.0]
# Embedded file name: /data/home/tack/projects/kaa/imlib2/build/lib.linux-x86_64-2.6/kaa/imlib2/image.py
# Compiled at: 2010-11-10 09:25:50
__all__ = [
...
import types, math, os, errno, _Imlib2
...
def _open_with_error_handle(open, *args):
    ...
    try:
        ...
        if len(exc.args) != 1 or not exc.args[0].isdigit():
            raise
        map = [
         None, errno.ENOENT, errno.EISDIR, errno.EACCES, None, errno.ENAMETOOLONG,
...
class Image(Object):
    ...
    def __getstate__(self):
        return (
         self.size, str(self.get_raw_data()), self._font, self.has_alpha)
    ...
    def blend(self, src, src_pos=(0, 0), src_size=(-1, -1), dst_pos=(0, 0), dst_size=(-1, -1), alpha=255, merge_alpha=True):
        """
        ...
        if src_size[0] == -1:
            src_size = (
             src.width, src_size[1])
        if src_size[1] == -1:
            src_size = (
             src_size[0], src.height)
        if dst_size[0] == -1:
            dst_size = (
             src_size[0], dst_size[1])
        if dst_size[1] == -1:
            dst_size = (
             dst_size[0], src_size[1])
    ...
    def draw_text(self, (x, y), text, color=None, font_or_fontname=None, style=None, shadow=None, outline=None, glow=None, glow2=None):
        """
        ...
        if style == TEXT_STYLE_PLAIN or style == None and font.style == TEXT_STYLE_PLAIN:
            ...
            metrics = self._image.draw_text_with_style(font._font, int(x), int(y), utf8(text), style, color, shadow, outline, glow, glow2)
        self._changed()
        return metrics
    ...
    def as_gdk_pixbuf(self):
        ...
        return gtk.gdk.pixbuf_new_from_data(data, gtk.gdk.COLORSPACE_RGB, True, 8, self.width, self.height, self.width * 4)