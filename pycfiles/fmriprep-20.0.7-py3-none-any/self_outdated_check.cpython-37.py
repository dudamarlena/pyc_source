...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-vxs8l7gu/pip/pip/_internal/self_outdated_check.py
# Compiled at: 2020-05-05 12:41:36
# Size of source mod 2**32: 8009 bytes
from __future__ import absolute_import
import datetime, hashlib, json, logging, os.path, sys
from pip._vendor import pkg_resources
import pip._vendor.packaging as packaging_version
...
def make_link_collector(session, options, suppress_no_index=False):
    """
...
class SelfCheckState(object):
    ...
    def __init__(self, cache_dir):
        ...
        if cache_dir:
            self.statefile_path = os.path.join(cache_dir, 'selfcheck', _get_statefile_name(self.key))
            try:
...
def pip_self_version_check(session, options):
    ...
    try:
        ...
        if pypi_version is None:
            link_collector = make_link_collector(session,
              options=options,
              suppress_no_index=True)
            selection_prefs = SelectionPreferences(allow_yanked=False,
              allow_all_prereleases=False)
            finder = PackageFinder.create(link_collector=link_collector,
