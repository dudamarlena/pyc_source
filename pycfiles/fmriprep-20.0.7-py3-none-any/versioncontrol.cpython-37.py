...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-vxs8l7gu/pip/pip/_internal/vcs/versioncontrol.py
# Compiled at: 2020-05-05 12:41:36
# Size of source mod 2**32: 23591 bytes
...
from pip._internal.utils.misc import ask_path_exists, backup_dir, display_path, hide_url, hide_value, rmtree
from pip._internal.utils.subprocess import call_subprocess, make_command
from pip._internal.utils.typing import MYPY_CHECK_RUNNING
from pip._internal.utils.urls import get_url_scheme
if MYPY_CHECK_RUNNING:
    from typing import Any, Dict, Iterable, Iterator, List, Mapping, Optional, Text, Tuple, Type, Union
...
class VersionControl(object):
    ...
    def get_url_rev_and_auth(cls, url):
        ...
        if '+' not in scheme:
            raise ValueError('Sorry, {!r} is a malformed VCS url. The format is <vcs>+<protocol>://<url>, e.g. svn+http://myrepo/svn/MyApp#egg=MyApp'.format(url))
        ...
        if '@' in path:
            ...
            if not rev:
                raise InstallationError('The URL {!r} has an empty revision (after @) which is not supported. Include a revision after @ or remove @ from the URL.'.format(url))
    ...
    def get_url_rev_options(self, url):
        ...
        return (
         hide_url(secret_url), rev_options)
    ...
    @classmethod
    def run_command(cls, cmd, show_stdout=True, cwd=None, on_returncode='raise', extra_ok_returncodes=None, command_desc=None, extra_environ=None, spinner=None, log_failed_cmd=True):
        """
        ...
        try:
            return call_subprocess(cmd, show_stdout, cwd, on_returncode=on_returncode,
            ...
            try:
                if e.errno == errno.ENOENT:
                    raise BadCommand(('Cannot find command {cls.name!r} - do you have {cls.name!r} installed and in your PATH?'.format)(**locals()))
                else:
                    raise
            finally:
                e = None
