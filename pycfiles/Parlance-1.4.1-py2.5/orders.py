...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/orders.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance unit orders
...
class UnitOrder(Comparable):
    ...
    def is_moving(self):
        return self.order_type in (MTO, CTO, RTO)

    def is_holding(self):
        return self.order_type == HLD

    def is_supporting(self):
        return self.order_type == SUP

    def is_convoying(self):
        return self.order_type == CVY

    def is_convoyed(self):
        return self.order_type == CTO

    def is_leaving(self):
        return self.order_type in (MTO, CTO, RTO, DSB, REM)
    ...
    def order_note(self, power, phase, past_orders=None):
        ...
        if not self.order_type.value() & phase:
            ...
            note = NYU
        elif not self.unit.coast.province.exists():
            note = NSP
...
class MovementPhaseOrder(UnitOrder):
    ...
    def convoy_note(convoyed, destination, routes, route_valid=bool):
        result = FAR
        if not (convoyed.exists() and convoyed.can_be_convoyed()):
            result = NSA
        elif not destination.exists():
            result = CST
        elif destination.province.is_coastal():
            if any((route_valid(route) for route in routes)):
                result = MBV
        return result
    ...
    def get_routes(self, convoyers, ignore_foreign):
        if self.unit.can_be_convoyed():
            ...
            if path_list:
                ...
                all_routes = [ path for path in path_list if all((available(place) for place in path))
                             ]
                if ignore_foreign:
                    ...
                    solo_routes = [ path for path in all_routes if all((countryman(place) for place in path))
                                  ]
                else:
                    solo_routes = None
    ...
    def supported_name(self):
        ...
        if self.unit.nation == power:
            return name
        else:
            return '%s %s' % (power.adjective, name)
...
class HoldOrder(MovementPhaseOrder):
    ...
    def __repr__(self):
        return 'HoldOrder(%r)' % self.unit

    def __str__(self):
        return self.unit.coast.prefix + ' HOLD'
...
class MoveOrder(MovementPhaseOrder):
    ...
    def order_note(self, power, phase, past_orders=None):
        ...
        if note == MBV:
            if not self.unit.can_move_to(self.destination):
                note = FAR
    ...
    def create(klass, order, nation, board, datc):
        ...
        if unit.can_be_convoyed():
            if datc.datc_4a3 != 'f':
                ...
                result = routes or klass(unit, dest)
...
class ConvoyingOrder(MovementPhaseOrder):
    ...
    def matches(self, order_set):
        ...
        return self.supported.exists() and counterpart and counterpart.is_convoyed() and self.destination.matches(counterpart.destination.key)

    def order_note(self, power, phase, past_orders=None):
        note = self.__super.order_note(power, phase, past_orders)
        if note == MBV:
            if not self.unit.coast.province.can_convoy():
                note = NAS
...
class ConvoyedOrder(MovementPhaseOrder):
    ...
    def __init__(self, unit, destination, path=None):
        ...
        if path:
            ...
            self.key = (
             unit.key, CTO, destination.province.key)
        return
    ...
    def matches(self, order_set):
        ...
        def matching(fleet):
            ...
            if counterpart:
                return counterpart.is_convoying() and counterpart.supported == self.unit and counterpart.destination.matches(self.destination.key)
            return False

        if self.path:
            return all((matching(unit) for unit in self.path))
        else:
            return True
    ...
    def order_note(self, power, phase, past_orders=None):
        ...
        if note == MBV:
            ...
            if note == MBV and self.path:
                ...
                def real_prov(fleet):
                    return fleet.coast.province.exists()

                def at_sea(fleet):
                    return fleet.coast.province.can_convoy()

                def check(f):
                    return not all((f(unit) for unit in self.path))

                if check(real_prov):
                    note = NSP
                elif check(Unit.exists):
                    note = NSF
                elif check(at_sea):
                    note = NAS
                elif check(Unit.can_convoy):
                    note = NSF
                elif self.path_key not in self.routes:
                    note = FAR
        return note
    ...
    def create(klass, order, nation, board, datc):
        ...
        if len(order) > 4 and datc.datc_4a6 != 'a':
            path = [ board.ordered_unit(nation, prov, datc) for prov in order[4] ]
        else:
            path = None
...
class SupportOrder(MovementPhaseOrder):
    ...
    def order_note(self, power, phase, past_orders=None):
        note = self.__super.order_note(power, phase, past_orders)
        if note == MBV:
            if not self.supported.coast.province.exists():
                note = NSP
...
class SupportHoldOrder(SupportOrder):
    ...
    def matches(self, order_set):
        ...
        return self.supported.exists() and not order_set.is_moving(self.supported) and self.supported.coast.province != self.unit.coast.province
...
class SupportMoveOrder(SupportOrder):
    ...
    def __init__(self, unit, mover, destination, legal_coast=True):
        self.key = (
         unit.key, SUP, mover.key, MTO, destination.province.key)
        self.unit = unit
        self.supported = mover
        self.destination = destination
    ...
    def matches(self, order_set):
        """ Whether the order is matched by the supported unit."""
        counterpart = order_set.get_order(self.supported)
        return self.supported.nation and counterpart.is_moving() and self.destination.matches(counterpart.destination.key)
...
class RetreatPhaseOrder(UnitOrder):
    ...
    def order_note(self, power, phase, past_orders=None):
        note = self.__super.order_note(power, phase, past_orders)
        if note == MBV and not self.unit.dislodged:
            note = NRN
        return note
...
class DisbandOrder(RetreatPhaseOrder):
    ...
    def __str__(self):
        return self.unit.coast.prefix + ' DISBAND'
...
class BuildPhaseOrder(UnitOrder):
    ...
    def required(self, power, past_orders):
        if past_orders:
            surplus = -past_orders.builds_remaining(power)
        else:
...
class BuildOrder(BuildPhaseOrder):
    ...
    def order_note(self, power, phase, past_orders=None):
        ...
        if note == NSU:
            ...
            if not coast.exists():
                note = CST
            elif not coast.province.is_supply():
                note = NSC
            elif self.unit.nation != coast.province.owner:
                note = YSC
            elif self.unit.nation.key not in coast.province.homes:
                note = HSC
            elif coast.province.units:
                note = ESC
            elif old_order:
                note = ESC
            elif not self.required(power, past_orders):
...
class RemoveOrder(BuildPhaseOrder):
    ...
    def order_note(self, power, phase, past_orders=None):
        note = self.__super.order_note(power, phase, past_orders)
        if note == MBV and not self.required(power, past_orders):
            note = NMR
        return note


_class_types = {HLD: HoldOrder, 
   MTO: MoveOrder, 
   SUP: SupportOrder, 
   CVY: ConvoyingOrder, 
   CTO: ConvoyedOrder, 
...
class OrderSet(defaultdict):
    ...
    def __str__(self):
        ...
        return '{ %s }' % ('; ').join([ '%s: %s' % (nation, (', ').join(map(str, orders))) for (nation, orders) in nations.iteritems()
                                      ])
    ...
    def create_SUB(self, nation=None):
        ...
        if result:
            return SUB % sorted(result)
        return

    def order_list(self, nation=None):
        if nation:
            return [ o for o in self if o.__author == nation ]
        else:
            return list(self)
    ...
    def holding(self):
        return [ o for o in self if o.is_holding() ]

    def moving_into(self, province):
        return [ order for order in self if order.is_moving() if order.destination.province.key == province.key ]
    ...
    def builds_remaining(self, power):
        ...
        if surplus > 0:
            return -max(0, surplus - len(removes))
        elif surplus < 0:
            return -min(0, surplus + len(builds) + waives)
        return 0
    ...
    def missing_orders(self, phase, nation=None):
        ...
        if phase == Turn.move_phase:
            result = [ unit.key for unit in power.units if not self.get_order(unit) ]
            if result:
                return MIS % result
        elif phase == Turn.retreat_phase:
            result = [ unit for unit in power.units if unit.dislodged if not self.get_order(unit) ]
            if result:
                return MIS % result
    ...
    def complete_set(self, board):
        ...
        if phase == Turn.move_phase:
            ...
            for power in board.powers.itervalues():
                ...
                if builds > 0:
                    self.waive(builds, power)
                elif builds < 0:
                    self.default_removes(-builds, power, board)

        else:
            raise UserWarning('Unknown phase %d' % phase)
