...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/player.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance client framework and simple clients
...
class Observer(VerboseObject):
    ...
    def handle_message(self, message):
        ...
        if self.validator:
            if message[0] is HLO:
                try:
                    level = message[(message.index(LVL) + 1)].value()
    ...
    def apply_handler(self, method, message):
        ...
        try:
            ...
            self.handle_invalid(message, error=e)

    def handle_invalid(self, message, error=None, reply=None):
        response = self.options.response
        if response in ('print', 'warn', 'carp', 'croak'):
            if error:
                self.log_debug(1, 'Error processing command: ' + str(message))
            else:
                self.log_debug(1, 'Invalid server command: ' + str(message))
        if response in ('huh', 'complain', 'carp', 'croak') and ERR not in message:
            self.send(reply or HUH(ERR + message))
        if response in ('die', 'close', 'carp', 'croak'):
            self.close()
        if error and response not in ('print', 'close', 'huh', 'carp', 'ignore'):
            raise
    ...
    def handle_HUH_SEL(self, message):
        self.send_identity()

    def handle_YES_SEL(self, message):
        self.send_identity()
    ...
    def handle_MDF(self, message):
        ...
        if self.map:
            if self.map.valid and self.process_map():
                ...
                self.reject(MAP(self.map.name))
        else:
            raise ValueError('MDF before MAP')
    ...
    def phase(self):
        return self.map.current_turn.phase()
...
class Player(Observer):
    ...
    def request_draws(self):
        ...
        for power_set in self.draws:
            if power_set == current:
                self.send(DRW)
            elif len(power_set) <= 1:
                self.log_debug(11, 'Skipping power set in draws: %s', power_set)
    ...
    def handle_MIS(self, message):
        if len(message) > 1:
            self.log_debug(7, 'Missing orders for %s: %s; expected %s', self.power, message, self.missing_orders())
    ...
    def handle_FRM(self, message):
        ...
        try:
            ...
            try:
                ...
                self.log_debug(7, 'Exception in %s(%s, %s): %s', method_name, sender, press, err)

    def send_press(self, recips, press):
        if not (self.in_game and self.power):
            return
        if not isinstance(recips, list):
            recips = [recips]
        for recipient in recips:
            if self.bcc_list.has_key(recipient) and press[0] != FRM:
                self.send_press(self.bcc_list[recipient], FRM(self.power)(recips)(press))
    ...
    def handle_ADM(self, message):
        ...
        if self.admin_state == 0:
            ...
            if '?' in s:
                ...
                elif re.match('[Rr]eally', s):
                    self.send_admin('Yup.')
                elif re.match('[Ww]ho', s):
                    self.send_admin('Eric.')
...
class Sizes(AutoObserver):
    ...
    def handle_SCO(self, message):
        self.log_debug(1, 'Supply Centres: ' + ('; ').join([ '%s, %d' % (dist[0], len(dist) - 1) for dist in message.fold()[1:]
                                                           ]))
...
class Clock(AutoObserver):
    ...
    def handle_HLO(self, message):
        ...
        if max_time > 0:
            for seconds in range(5, max_time, 5):
...
class Ladder(AutoObserver):
    ...
    def store_scores(self, scores):
        try:
            dump(scores, open(self.options.score_file, 'w'))
        except IOError:
            pass
