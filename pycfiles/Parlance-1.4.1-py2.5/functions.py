...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/functions.py
# Compiled at: 2009-08-25 22:30:53
"""Parlance general utility functions
...
def expand_list(items, conjunction='and'):
    ...
    if items:
        ...
        if len(items) > 1:
            result += ','
        while len(items) > 1:
            result += ' %s,' % (items.pop(0),)

        if items:
            result += ' %s %s' % (conjunction, items[0])
        return result
    else:
        return 'nothing'
...
def sublists(series):
    ...
    subs = [[]]
    for tail in series:
        subs += [ head + [tail] for head in subs ]
...
def version_string(extra=None):
    ...
    """
    items = [
     'Parlance', extra, parlance.__version__]
...
def num2name(number):
    ...
    result = names[number]
    if tens:
        result = result and tens + '-' + result or tens
    if hundreds:
        result = result and hundreds + ' ' + result or hundreds
...
def instances(number, name, article=True):
    if article and number == 1:
        ...
        prefix = '%s instances of ' % num2name(number)
    else:
        prefix = ''
    return prefix + name
...
def fails(test_function):
    ...
    def test_wrapper(test_case):
        ...
        except test_case.failureException:
            pass
        else:
            test_case.fail('Test unexpectedly passed')
...
def static(**kwargs):
    """Initializes static variables of a function.
...
def timestamp(seconds=None):
    ...
    now = gmtime(seconds)
    year = now[0] // 2 % 20 + 10
    month = now[1] + now[0] % 2 * 12
    day = now[2] - 1
    hour = now[3]
    minute = now[4] // 2
    second = now[5] // 2
    result = chars[year] + chars[month] + chars[day] + chars[hour] + chars[minute] + chars[second]
