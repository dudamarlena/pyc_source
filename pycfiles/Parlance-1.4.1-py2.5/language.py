...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/language.py
# Compiled at: 2009-08-25 22:30:53
"""Parlance language classes
...
class Message(list):
    ...
    def __init__(self, *message):
        ...
        """
        for item in message:
            list.extend(self, self.to_tokens(item))
    ...
    def to_tokens(value, wrap=False):
        ...
        if hasattr(value, 'tokenize'):
            ...
            try:
                return sum([ Message.to_tokens(item, True) for item in value ], [])
            except TypeError:
                raise TypeError('Cannot tokenize ' + str(value))
    ...
    def wrap(value):
        ...
        """
        return [
         BRA] + Message.to_tokens(value) + [KET]
    ...
    def __iand__(self, other):
        ...
        list.extend(self, self.wrap(other))
        return self
    ...
    def __setslice__(self, from_index, to_index, value):
        ...
        """
        try:
            list.__setslice__(self, from_index, to_index, self.to_tokens(value))
        except TypeError:
            raise TypeError('must assign list (not "%s") to slice' % type(value).__name__)
...
class _integer_Token(int):
    ...
    def __setattr__(self, name, value):
        if hasattr(self, name):
            ...
            super(_integer_Token, self).__setattr__(name, value)
...
class Token(_integer_Token):
    ...
    def __new__(klass, name, number):
        ...
        if result is None:
            ...
            result.category = (number & 65280) >> 8
            Token.cache[key] = result
        return result

    def __reduce_ex__(self, proto):
        """Enables pickling tokens.
        ...
        """
        return (
         _fetch_token, (Token, self.text, self.number))
    ...
    def is_coastal(self):
        ...
        """
        return self.category_name() in ('Coastal SC', 'Coastal non-SC', 'Bicoastal SC',
                                        'Bicoastal non-SC')
    ...
    def __repr__(self):
        ...
        if self.is_integer() and self.text == str(self.value()):
            ...
            return 'BRA'
        elif protocol.default_rep.get(self.text) == self:
            return self.text
        elif len(self.text) == 1 and StringToken(self.text) == self:
            return name + '(' + repr(self.text) + ')'
        else:
            return name + '(' + repr(self.text) + ', ' + '0x%04X' % self.number + ')'
...
class StringToken(Token):
    ...
    def __reduce_ex__(self, proto):
        """Ensures that unpickled StringTokens work correctly.
            >>> from pickle import dumps, loads
            >>> five = StringToken('5')
            >>> s = dumps(five)
            >>> t = loads(s)
        ...
        """
        return (
         _fetch_token, (StringToken, self.text))
...
class IntegerToken(Token):
    ...
    def __reduce_ex__(self, proto):
        """Ensures that unpickled IntegerTokens work correctly.
            >>> from pickle import dumps, loads
            >>> five = base_rep[5]
            >>> s = dumps(five)
            >>> t = loads(s)
        ...
        """
        return (
         _fetch_token, (IntegerToken, self.number))
...
class Time(Comparable):
    ...
    def __init__(self, seconds, hours=0):
        if isinstance(seconds, Token):
            seconds = seconds.value()
        if isinstance(hours, Token):
            hours = hours.value()
        ...
        if hours >= 0 <= seconds:
            ...
            raise ValueError('Invalid Time(%r, %r)' % (seconds, hours))
    ...
    def __int__(self):
        return int(self.seconds + 3600 * self.hours)

    def __str__(self):
        return str(Message(self))

    def __repr__(self):
        return 'Time(%s, %s)' % (self.seconds, self.hours)
...
class Representation(Configurable):
        ...
        if result is None:
            if isinstance(key, Token):
                ...
                try:
                    ...
                    elif number & 65280 == protocol.quot_prefix:
                        result = StringToken(chr(number & 255))
    ...
    def has_key(self, key):
        ...
        return key in self.names or key in self.numbers or self.base and self.base.has_key(key)
    ...
    def __eq__(self, other):
        """Compares two Representations, or a Representation and a dict.
    ...
    def translate(self, text):
        r""" Translates diplomacy message strings into Messages,
        ...
        if self.options.input_escape == self.options.quot_char:
            ...
            return self.translate_backslashed(text)
    ...
    def translate_doubled_quotes(self, text):
        ...
        for piece in fragments[1:-1]:
            ...
            if in_text:
                ...
                append(StringToken(self.options.quot_char))

        if len(fragments) > 1:
            in_text = not in_text
            if in_text:
                addmsg(quoted(fragments[(-1)]))
            else:
                addmsg(normal(fragments[(-1)]))
        if in_text:
    ...
    def translate_backslashed(self, text):
        ...
        for piece in fragments:
            ...
            if slashes % 2:
                ...
                if in_text:
                    addmsg(quoted(saved + piece))
                else:
                    addmsg(normal(saved + piece))
                in_text = not in_text
                saved = ''
    ...
    def parse_dcsp(self, dcsp_file):
        ...
        for line in dcsp_file:
            ...
            if err_type:
                ...
                if match:
                    setattr(self, match.group(1), err_type)
                ...
                if match:
                    ...
                    old_line = line[line.rfind('>'):].strip()
            elif msg_name:
                if line.find('Message Type =') > 0:
                    type_num = int(re.search('Type = (\\d+)', line).group(1))
                    self.message_types[msg_name] = type_num
                    msg_name = ''
            elif line.find(' (0x') > 0:
                match = re.search('[> ](\\w[\\w ]+) \\((0x\\w\\w)', line)
                descrip = match.group(1)
                start_cat = int(match.group(2), 16)
                match = re.search(' (0x\\w\\w)\\)', line)
                if match:
                ...
                if line.find('category =') > 0:
                    match = re.search('>([\\w -]+) category = (0x\\w\\w)<', line)
                    if match:
                    ...
                    if match:
                        ...
                        if rep_item:
                            default_tokens[number] = name
                        else:
                            token_names[number] = name
            elif line.find('M)') > 0:
                match = re.search('The (\\w+) Message', line)
                if match:
                ...
                if match:
                    self.magic = int(match.group(1), 16)
                else:
                    self.log_debug(1, 'Invalid magic number: ' + line)

        self.base_rep = Representation(token_names, None)
        self.default_rep = Representation(default_tokens, self.base_rep)
        if not self.magic:
            self.log_debug(1, 'Missing magic number')
        if not self.version:
            self.log_debug(1, 'Missing version number')
        return
