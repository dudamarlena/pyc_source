...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/judge.py
# Compiled at: 2009-08-25 22:30:53
"""Parlance adjudication system
...
from orders import DisbandOrder, HoldOrder, OrderSet, RemoveOrder, WaiveOrder, createUnitOrder
from tokens import *
...
class JudgeInterface(VerboseObject):
    ...
    def handle_MAP(self, client, message):
        client.send(MAP(self.map_name))

    def handle_VAR(self, client, message):
        client.send(VAR(self.variant_name))

    def handle_MDF(self, client, message):
        client.send(self.mdf)
    ...
    def players(self):
        return self.map.powers.keys()

    def player_name(self, country):
        return self.map.powers[country].name

    def score(self, player):
        return len(self.map.powers[player].centers)

    def turn(self):
        return self.map.current_turn
...
class Judge(JudgeInterface):
    ...
    def handle_NOW(self, client, message):
        client.send(self.map.create_NOW())

    def handle_SCO(self, client, message):
        client.send(self.map.create_SCO())

    def handle_ORD(self, client, message):
        client.send_list(self.last_orders)
    ...
    def handle_NOT_SUB(self, client, message):
        ...
        country = client.country
        if country:
            if self.phase and not self.eliminated(country):
                orders = self.next_orders
                if len(message) > 4:
                    order = createUnitOrder(message.fold()[1][1], country, self.map, self.datc)
                    orders.remove(order, country) or client.reject(message)
                    return
            else:
                orders.clear(country)
    ...
    def get_draw_parties(self, message):
        if len(message) == 1:
            return frozenset(self.map.current_powers())
    ...
    def eliminated(self, country=None):
        ...
        if country:
            ...
            return [ country for (country, power) in self.map.powers.iteritems() if power.eliminated
                   ]
    ...
    def create_SETs(self, turn):
        return [ SET(nation)(turn) % [ ([order], [order.__note]) for order in self.next_orders.order_list(nation) ] for nation in self.map.powers.values() if not nation.eliminated
               ]
    ...
    def init_turn(self):
        ...
        self.unready.update([ country for country in self.map.powers if self.missing_orders(country)
                            ])
    ...
    def check_draw(self):
        ...
        for (winners, voters) in self.draws.iteritems():
            if voters >= in_game:
                if self.game_opts.PDA:
                    return DRW(winners)
                elif self.options.full_DRW:
                    return DRW(in_game)
                else:
                    return +DRW
    ...
    def check_solo(self, growing):
        ...
        if growing:
            ...
            for (token, power) in self.map.powers.iteritems():
                ...
                if strength == max_seen:
                    country = None
                elif strength > max_seen:
                    max_seen = strength
                    country = token

            self.log_debug(11, 'Checking solo, with (%s, %s, %d, %d).', country, growing, max_seen, self.win_condition)
            ...
            self.static += 1
        year = self.map.current_turn.year
        self.log_debug(11, 'Now in %s, with %d static year%s.', year, self.static, s(self.static))
    ...
    def move_algorithm(self):
        ...
        for unit in self.map.units:
            ...
            if order.is_moving():
                ...
                if order.matches(self.next_orders) and not self.illegal(order):
                    convoyers[unit.coast.province.key] = (
                     order.supported.key, unit.nation.key)
                elif not order.__result:
                    order.__result = NSO
        ...
        if self.datc.datc_4a2 == 'b':
            ...
            decision_list = [ choice for choice in decision_list if choice not in resolved
                            ]
        while decision_list:
            ...
            for choice in decision_list:
                ...
                for dep in choice.depends:
                    self.log_debug(15, '- ' + str(dep))

            remaining = [ choice for choice in decision_list if not choice.calculate()
                        ]
            if len(remaining) == len(decision_list):
                decision_list = self.resolve_paradox(remaining)
            else:
                decision_list = remaining

        turn = self.map.current_turn
        orders = [ ORD(turn)(unit.current_order.strict)(self.process_results(unit)) for unit in self.map.units
                 ]
        for choice in decisions:
            del choice.depends
        ...
        if order.is_convoyed():
            ...
            if order.maybe_overland():
                if not routes:
                    ...
                    if key not in convoyers.itervalues():
                        routes = None
                elif self.datc.datc_4a3 == 'a':
                    pass
                elif heads:
                    if self.datc.datc_4a3 == 'c':
                        try_overland = True
                else:
                    routes = None
        else:
            routes = None
        self.log_debug(11, "Path_Decision(%s, %s, %s, %s) from '%s' for 4.A.1 and '%s' for 4.A.3", order, routes and [ [ s.key for s in p ] for p in routes ], not disrupt_any, try_overland, self.datc.datc_4a1, self.datc.datc_4a3)
    ...
    def illegal(self, order):
        ...
        if option == 'a':
            ...
            return order.__note == NSP
        elif option == 'c':
            return order.__note in (NSP, FAR, NAS, CST)
        elif option == 'd':
            return order.__note != MBV
        else:
            raise NotImplementedError
    ...
    def resolve_paradox(self, decisions):
        ...
        resolved = None
        if convoy:
            if self.datc.datc_4a2 == 'd':
                resolved = self.Szykman(core)
            elif self.datc.datc_4a2 == 'f':
                resolved = self.dptg(core)
        elif moving_to and moving_to == moving_from:
            resolved = self.circular(core)
        if not resolved:
            resolved = self.fallback(core)
    ...
    def dptg(self, decisions):
        ...
        def confused(choice):
            result = choice.type == Decision.SUPPORT and choice.order.supported.current_order.is_convoying()
            if result:
                self.log_debug(11, '* Confused: %s', choice)
            return result

        if any((confused(d) for d in decisions)):
            return self.fallback(decisions)
        else:
            return self.Szykman(decisions)
    ...
    def get_core(self, decisions):
        choices = {}
        for choice in decisions:
            choices[choice] = set([ dep for dep in choice.depends if dep if not dep.decided()
                                  ])
            self.log_debug(8, '%s:', choice)
            for dep in choice.depends:
                self.log_debug(11, '- %s', dep)
    ...
    def process_results(self, unit):
        ...
        if not result:
            ...
            if order.is_moving():
                ...
                if unit.decisions[Decision.SUPPORT].passed:
                    result = SUC
                else:
                    result = CUT
            elif order.is_convoying():
        ...
        if unit.decisions[Decision.DISLODGE].passed:
            if not unit.dislodged:
                unit.retreat(self.collect_retreats(unit))
            if result:
                return (result, RET)
            else:
                return RET
        else:
            return result or SUC

    def collect_retreats(self, unit):
        self.log_debug(8, 'Collecting retreats for %s, dislodged by %s', unit, unit.dislodger)
        return [ coast.maybe_coast for coast in [ self.map.coasts[key] for key in unit.coast.borders_out ] if self.valid_retreat(coast, unit.dislodger)
               ]
    ...
    def valid_retreat(self, retreat, dislodger):
        ...
        if retreat.province.entering:
            if retreat.province.hold.max_value > 0:
                return False
...
class Decision_Set(defaultdict):
    ...
    def __init__(self):
        super(Decision_Set, self).__init__(list)

    def add(self, decision):
        self[decision.type].append(decision)

    def __iter__(self):
        return chain(*self.itervalues())

    def sorted(self):
        return self[Decision.PATH] + self[Decision.HEAD] + self[Decision.ATTACK] + self[Decision.SUPPORT] + self[Decision.DEFEND] + self[Decision.PREVENT] + self[Decision.HOLD] + self[Decision.MOVE] + self[Decision.DISLODGE]
    ...
    def __init__(self, order):
        ...
        if self.type != Decision.HOLD:
            ...
            if order.destination:
                self.into = order.destination.province
        return
...
class Tristate_Decision(Decision):
    ...
    def minmax(self, decision_list):
        ...
        for decision in decision_list:
            if decision.min_value > min_found:
                min_found = decision.min_value
            if decision.max_value > max_found:
                max_found = decision.max_value
...
class Move_Decision(Tristate_Decision):
    ...
    def init_deps(self):
        ...
        self.depends.extend([ unit.decisions[Decision.DEFEND] for unit in self.battles()
                            ])
        self.depends.extend([ unit.decisions[Decision.PREVENT] for unit in self.into.entering if unit != self.order.unit
                            ])
    ...
    def calculate(self):
        ...
        if self.passed:
            for unit in self.order.destination.province.units:
                unit.dislodger = (
                 self.order.unit.coast.province, self.order.path)
...
class Support_Decision(Tristate_Decision):
    ...
    def init_deps(self):
        self.depends = [self.order.unit.decisions[Decision.DISLODGE]] + [ u.decisions[Decision.ATTACK] for u in self.order.unit.coast.province.entering if u.coast.province != self.into
                                                                        ]
...
class Support_Decision_Cuttable(Tristate_Decision):
    ...
    def init_deps(self):
        ...
        self.depends = [
         self.order.unit.decisions[Decision.DISLODGE]] + attacks + paths.keys() + paths.values()
    ...
    def calculate(self):
        ...
        if self.paths:
            ...
            for decision in self.attacks + actual:
                if decision.min_value > min_oppose:
                    min_oppose = decision.min_value

            for decision in self.attacks + possible:
                if decision.max_value > max_oppose:
                    max_oppose = decision.max_value

        else:
            (min_oppose, max_oppose) = self.minmax(self.attacks)
        self.passed = dislodge.failed and max_oppose == 0
        self.failed = dislodge.passed or min_oppose >= 1
        return self.decided()
...
class Dislodge_Decision(Tristate_Decision):
    ...
    def init_deps(self):
        if self.order.is_moving():
            ...
            my_move = None
        self.depends = [
         my_move] + [ unit.decisions[Decision.MOVE] for unit in self.order.unit.coast.province.entering
                    ]
        return
...
class Path_Decision(Tristate_Decision):
    ...
    def __init__(self, order, routes, disrupt_all, try_overland):
        ...
        self.routes = routes and [ sum([ [ unit.decisions[Decision.DISLODGE] for unit in prov.units ] for prov in path ], []) for path in routes
                                 ]

    def init_deps(self):
        if self.routes:
            self.depends = set(sum(self.routes, []))
        else:
            self.depends = []
    ...
    def calculate(self):
        if self.routes:
            ...
            if self.failed and self.backup:
                self.routes = None
            else:
        ...
        if not self.routes:
            if self.routes is None:
                ...
                if routes:
                    self.order.set_path([ prov.unit for prov in routes[0] ])
                self.passed = False
...
class Head_Decision(Tristate_Decision):
    ...
    def calculate(self):
        ...
        if path.failed:
            self.failed = True
        elif all((head.failed for head in heads)):
            self.failed = True
        elif path.passed:
            if path.routes:
                self.failed = True
            elif any((head.overland() for head in heads)):
                self.passed = True
        return self.decided()
...
class Attack_Decision(Numeric_Decision):
    ...
    def calculate(self):
        ...
        def minimal_test(choice):
            return choice and choice.passed

        def maximal_test(choice):
            return choice and not choice.failed
    ...
    def calc_attack(self, path, heads, moves, supports, valid, valid_head):
        if valid(path):
            ...
            powers = set([ choice.order.unit.nation.key for choice in heads if valid_head(choice)
                         ] + [ unit.nation.key for (unit, choice) in moves if not valid(choice) ])
            if self.order.unit.nation.key in powers:
                return 0
            return 1 + len([ choice for choice in valid_supports if choice.order.unit.nation.key not in powers
                           ])
        else:
            return 0
...
class Prevent_Decision(Numeric_Decision):
    ...
    def calculate(self):
        ...
        if path.failed:
            ...
            head = self.depends[1]
            moves = [ choice for choice in self.depends if choice if choice.type == Decision.MOVE
                    ]
            supports = [ choice for choice in self.depends if choice if choice.type == Decision.SUPPORT
                       ]
            ...
            if head and not head.failed and any((not choice.failed for choice in moves)):
                self.min_value = 0
                if head.passed and any((choice.passed for choice in moves)):
                    self.max_value = 0
            elif not path.passed:
                self.min_value = 0
        return self.decided()
