...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/main.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance command-line interface
...
class ThreadManager(VerboseObject):
    ...
    """
    flags = getattr(select, 'POLLIN', 0) | getattr(select, 'POLLERR', 0) | getattr(select, 'POLLHUP', 0) | getattr(select, 'POLLNVAL', 0)
    __options__ = (
     (
    ...
    def clients(self):
        return [ item[1] for item in chain(self.polled.iteritems(), self.timed, self.threaded) ] + self.dynamic + self.queue
            ...
            while not self.closed:
                if self.clients():
                    ...
                    if self.polled or self.timed or self.closed:
                        ...
                        self.log_debug(7, 'sleep()ing for %.3f seconds', self.options.sleep_time)
    ...
    def process(self, wait_time=1):
        ...
        while not self.closed:
            ...
            if self.polled:
                ...
                self.log_debug(7, '%s()ing for %.3f seconds', method.__name__, poll_time)
    ...
    def attempt(self, client):
        ...
        try:
            ...
            self.log_debug(1, 'Exception running %s: %s %s', client.prefix, e.__class__.__name__, e.args)
            if not client.closed:
                client.close()
            if not self.options.block_exceptions:
                raise
    ...
    def add_polled(self, client):
        ...
        if self.polling:
            self.polling.register(fd, self.flags)

    def remove_polled(self, fd):
        self.log_debug(11, 'Removing polled client: %s', self.polled.pop(fd).prefix)
        if self.polling:
    ...
    def add_input(self, input_handler, close_handler):
        ...
        if self.polling:
            self.add_polled(waiter)
        else:
            self.add_looped(waiter)

    def select(self, timeout):
        self.clean_polled()
        try:
            ready = select.select(self.polled.values(), [], [], timeout)[0]
    ...
    def poll(self, timeout):
        self.clean_polled()
        try:
            ready = self.polling.poll(timeout * 1000)
        except select.error, e:
    ...
    def get_timeout(self):
        now = time()
        times = [ t for t in (client.time_left(now) for client in self.dynamic if not client.closed) if t is not None
                ]
        when = [ t for (t, c) in self.timed if not c.closed ]
        if when:
            times.append(max(0, 0.005 + min(when) - now))
        if times:
            result = min(times)
        else:
            result = None
    ...
    def check_dynamic(self):
        ...
        for client in self.dynamic:
            ...
            elif None is not client.time_left(now) <= 0:
                self.attempt(client)

        for client in removals:
            self.dynamic.remove(client)
    ...
    class ThreadClient(object):
        ...
        def __init__(self, target, *args, **kwargs):
            ...
            self.kwargs = kwargs
            self.closed = False
            arguments = chain((repr(arg) for arg in args), ('%s=%r' % (name, value) for (name, value) in kwargs.iteritems()))
            self.prefix = target.__name__ + '(' + str.join(', ', arguments) + ')'
    ...
    def wait_threads(self):
        for (thread, client) in self.threaded:
            while thread.isAlive():
                try:
                    thread.join(self.options.sleep_time)
                except KeyboardInterrupt:
                    if not client.closed:
                        client.close()
    ...
    def add_queued(self, client):
        ...
        if run_queue:
            self.new_thread(self.check_queue)
    ...
    def run_queue(self):
        if self.queue:
            self.log_debug(11, 'Checking all queued clients')
...
def run_player(player_class, allow_multiple=True, allow_country=True):
    ...
    for arg in remainder:
        if arg.isdigit():
            ...
            if allow_country:
                ...
                usage('%s does not accept country codes.', name)
        elif host is None:
    ...
    while num > 0 or countries:
        ...
        if countries:
            ...
            result = manager.add_client(player_class, power=nation, passcode=pcode)
        else:
...
def run_server(server_class, default_verbosity):
    ...
    if remainder:
        if remainder[0] in variants:
            ...
            usage('Unknown variant %r', remainder[0])
    manager = ThreadManager()
    ...
    if server.open():
        ...
        server.log_debug(1, 'Failed to open the server.')
    return
