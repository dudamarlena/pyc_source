...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/server.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance game server
...
from functions import defaultdict, expand_list, instances, num2name, s, timestamp, version_string
...
class DelayedAction(object):
    ...
    def veto(self, client):
        ...
        if block:
            ...
            if self.veto_line:
                client.game.admin('%s has vetoed %s', client.full_name(), self.veto_line)
...
class Server(VerboseObject):
    ...
    def broadcast_admin(self, text):
        if self.options.snd_admin:
            self.broadcast(ADM('Server')(text))
    ...
    def handle_message(self, client, message):
        ...
        handlers = [
         self, client.game, client.game.judge]
    ...
    def handle_HUH(self, client, message):
        self.log_debug(7, 'Client #%d complained about message: %s', client.client_id, message)

    def handle_ADM(self, client, message):
        line = message.fold()[2][0]
        text = line.lower()
        if text[0:7] == 'server:':
            if self.options.admin_cmd:
                self.seek_command(client, text[7:])
            else:
        ...
        for pattern in self.commands:
            ...
            for pattern in client.game.commands:
                ...
                for pattern in self.local_commands:
                    ...
                    client.admin('Unrecognized command: "%s"', text.strip())
    ...
    def handle_SEL(self, client, message):
        if len(message) > 3:
            ...
            client.send(SEL(client.game.game_id))

    def handle_PNG(self, client, message):
    ...
    def join_game(self, client, game_id):
        ...
        if client.game.game_id == game_id:
            ...
            try:
                ...
                self.log_debug(1, 'Exception while loading a saved game: %s %s', e.__class__.__name__, e.args)
            else:
                if result:
    ...
    def start_game(self, client=None, match=None):
        ...
        try:
            ...
            if client:
                client.admin('New game started, with id %s.', game_id)
    ...
    def list_help(self, client, match):
        for line in [
         'Begin an admin message with "Server:" to use the following commands, all of which are case-insensitive:'] + [ pattern.description for pattern in self.commands ] + [ pattern.description for pattern in client.game.commands ]:
            client.admin(line)
    ...
    def list_status(self, client, match):
        for game in self.games.itervalues():
            if game.closed:
                ...
                message = 'Forming'
            client.admin('Game %s: %s; %s', game.game_id, message, game.has_need())

    def list_powers(self, client, match):
        for player in client.game.players.values():
            if player.client:
                client.admin('%s (%d): %s, from %s', player.pname, player.passcode, player.full_name(), player.client.address)
            else:
                client.admin('%s (%d): None', player.pname, player.passcode)
    ...
    def close(self, client=None, match=None):
        ...
        if not self.closed:
            ...
            self.log_debug(11, 'Duplicate close() call')

    commands = [
     Command('help', list_help, '  help - Lists admin commands recognized by the server'),
     Command('new game', start_game, '  new game - Starts a new game of Standard Diplomacy'),
     Command('(new|start) (\\w+) game', start_game, '  new <variant> game - Starts a new game, with the <variant> map'),
     Command('list variants', list_variants, '  list variants - Lists known map variants'),
     Command('list bots', list_bots, '  list bots - Lists bots that can be started by the server'),
     Command('status', list_status, '  status - Displays the status of each game')]
    local_commands = [
     Command('shutdown', close, '  shutdown - Stops the server'),
     Command('powers', list_powers, '  powers - Displays the power assignments for this game')]
...
class Historian(VerboseObject):
    ...
    def save(self, stream):
        ...
        def write(token):
            write_message(self.messages[token])
        ...
        if self.judge.game_result:
            write_message(self.judge.game_result)
        if self.started:
            if self.closed:
                write(SMR)
            else:
    ...
    def load(self, stream):
        ...
        for line in stream:
            ...
            if first in (ORD, SET):
                ...
                history.setdefault(turn, {SET: [], ORD: [], SCO: sco, 
                   'new_SCO': False, 
                   NOW: None})[first].append(message)
            elif first is NOW:
                ...
                if first is MAP:
                    ...
                    try:
                        ...
                        self.log_debug(7, 'Variant %r not found among %r', variant_name, variants.keys())
                        return False
                    else:
    ...
    def handle_REJ_MAP(self, client, message):
        self.disconnect(client)

    def handle_YES_SVE(self, client, message):
        pass

    def handle_REJ_SVE(self, client, message):
        pass

    def handle_YES_LOD(self, client, message):
        pass

    def handle_REJ_LOD(self, client, message):
        self.disconnect(client)
    ...
    def handle_YES_MAP(self, client, message):
        ...
        if self.judge.game_result:
            client.send(self.judge.game_result)
    ...
    def get_history(self, key, always_sco):
        ...
        if turn:
            for message in sorted(turn[SET]):
                result.append(message)

            for message in sorted(turn[ORD]):
                result.append(message)
...
class Game(Historian):
    ...
    class Player_Struct(object):
        ...
        def client_ready(self):
            return self.client and self.ready
    ...
    def set_limits(self):
        ...
        self.press_in = {Turn.move_phase: press_limit < move_limit or not move_limit, 
    ...
    def open_position(self, country):
        ...
        if self.closed:
            ...
            if not self.judge.eliminated(country):
                if self.game_options.DSD:
                    self.pause()
        elif self.limbo:
            self.offer_power(country, *self.limbo.popitem())
        return
    ...
    def players_unready(self):
        ...
        return [ country for (country, struct) in self.players.iteritems() if not (struct.client_ready() or self.judge.eliminated(country))
               ]
    ...
    def disconnect(self, client):
        ...
        if client in self.clients:
            ...
            opening = client.country
            client.country = None
        elif self.limbo.has_key(client):
            del self.limbo[client]
        ...
        for (country, player) in self.players.iteritems():
            if not self.judge.eliminated(country):
                if not player.client:
                    disconnected[country] = player.passcode
                elif player.robotic:
                    robotic[country] = player.passcode

        msg = None
        slate = None
        if disconnected:
            if len(disconnected) > 1:
                msg = 'have been disconnected'
            else:
            ...
            if len(robotic) > 1:
                ...
                msg = 'seems to be a bot'
            slate = robotic
        if msg:
            client.admin('%s %s.', expand_list([ '%s (%s)' % kv for kv in slate.iteritems() ]), msg)
            return True
        return False
    ...
    def unpause(self):
        ...
        if self.time_stopped:
            self.set_deadlines(self.time_stopped)
    ...
    def run(self):
        ...
        if self.ready():
            ...
            times = reversed(sorted((sec for sec in self.timers if remain < sec < self.time_checked)))
            for second in times:
    ...
    def run_judge(self):
        ...
        if self.deadline:
            self.log_debug(10, 'Running the judge with %f seconds left', self.deadline - time())
        else:
            self.log_debug(10, 'Running the judge')
        key = self.judge.turn().key
        self.history[key] = turn = {SET: [], ORD: [], SCO: None, NOW: None, 'new_SCO': False}
        for message in self.judge.run():
            self.broadcast(message)
            if message[0] in (ORD, SET):
                turn[message[0]].append(message)
            elif message[0] in (SCO, NOW):
                turn[message[0]] = message

        if not turn[SCO]:
            turn[SCO] = self.judge.map.create_SCO()
        else:
            turn['new_SCO'] = True
        if self.judge.phase:
            self.set_deadlines()
    ...
    def queue_action(self, client, action_callback, action_line, veto_callback, veto_line, veto_terms, *args):
        delay = self.options.veto_time
        self.admin('%s is %s', client.full_name(), action_line)
        if delay > 0:
            self.actions.append(DelayedAction(action_callback, veto_callback, veto_line, veto_terms, delay, *args))
    ...
    def send_hello(self, client):
        ...
        if country:
            passcode = self.players[country].passcode
        else:
            country = UNO
            passcode = 0
    ...
    def listing(self, result=None):
        ...
        return LST(self.game_id)(need, result or self.status())(self.variant.name)(self.game_options)
        ...
        if country and self.press_allowed and country not in eliminated:
            ...
            for nation in recips:
                if not self.players[nation].client:
                    ...
                    self.log_debug(7, 'Client #%d is sending press to itself.', client.client_id)
            else:
    ...
    def handle_TME(self, client, message):
        if self.deadline and not self.paused:
            remain = self.deadline - time()
        else:
            remain = 0
        if len(message) == 1:
            if remain:
                client.send(TME(Time(remain)))
            else:
                client.reject(message)
        elif len(message) >= 4:
            try:
    ...
    def handle_IAM(self, client, message):
        ...
        if not self.started:
                ...
                if old_client:
                    for new_country in self.p_order:
                        ...
                        if not new_slot.client:
                            self.log_debug(6, 'Reassigning client #%d to %s', old_client.client_id, new_country)
                            old_client.country = new_country
                            new_slot.copy_client(slot)
                            break
                    else:
                        self.log_debug(1, 'No place to put old client #%d!', old_client.client_id)
            ...
            if old_client is client and country == client.country:
                ...
                if good:
                    self.broadcast(NOT(CCD(country)))
            ...
            if good:
                ...
                if client not in self.clients:
                    self.clients.append(client)
                if client.country:
                    self.open_position(client.country)
                client.country = country
                ...
                if unready:
                    self.log_debug(9, 'Still waiting for %s', expand_list(unready))
                elif self.paused:
                    self.resume()
            else:
                client.reject(message)
    ...
    def find_players(self, name):
        ...
        for (key, struct) in self.players.iteritems():
            names = (
             struct.name, struct.version)
            if self.started:
                names += (struct.pname, key.text)
            for item in names:
                if name == item:
    ...
    def eject(self, client, match):
        ...
        if len(players) == 1 or players and not self.started:
            ...
            itemlist.sort()
            self.queue_action(client, self.boot_players, '%sing %s from the game.' % (
             verb,
             expand_list([ instances(num, nam, False) for (num, nam) in itemlist
             ])), veto_action, 'the player %s.' % veto_verb, terms, [ c for (c, n) in players ])
        else:
    ...
    def block_boot(self, client, players):
        if client in players:
            return "You can't veto your own booting."
    ...
    def list_players(self, client, match):
        ...
        if observers:
            ...
            observers.sort()
            for line in observers:
                client.admin(line)

        if observing:
            client.admin('%s anonymous observer%s', num2name(observing).capitalize(), s(observing))
    ...
    def set_time_limit(self, client, match):
        ...
        if phase[0] in 'mrbp':
            ...
            if seconds and not self.started:
                ...
                self.admin('%s has set the %s time limit to %d seconds.', client.full_name(), phase, value)
            else:
                value = getattr(self.game_options, attribute)
                client.admin('The %s time limit is %d seconds.', phase, value)
        else:
            client.admin('Unknown phase %r; try move, build, retreat, or press.', phase)

    def stop_time(self, client, match):
        if self.paused:
            client.admin('The game is already paused.')
        else:
            self.pause()
    ...
    def resume(self, client=None, match=None):
        if self.paused:
            self.queue_action(client or self, self.unpause, 'resuming the game.', None, 'resuming the game.', ('resume',
                                                                                                               'unpause'))
        elif client:
            client.admin('The game is not currently paused.')
        return
    ...
    def start_bot(self, client, match):
        ...
        if bot_name in bots:
            ...
            try:
                ...
                if num < 1:
                    num += self.players_needed()
                name = bot_class.__name__
                self.queue_action(client, self.start_bot_class, 'starting %s%s.' % (
                 instances(num, name), power and ' as %s' % pname or ''), None, 'the %s%s.' % (name, s(num)), (
                 'start', 'bot', 'bots', name, name + 's'), bot_class, num, power, pcode)
        else:
            client.admin('Unknown bot: %s', bot_name)
        return

    def start_bot_class(self, bot_class, number, power, pcode):
        self.log_debug(11, 'Attempting to start %s %s%s', num2name(number), bot_class.__name__, s(number))
        failure = 0
        for dummy in range(number):
            client = self.server.manager.add_client(bot_class, game_id=self.game_id, power=power, passcode=pcode)
            if not client:
                failure += 1

        if failure:
            self.admin('%s bot%s failed to start', num2name(failure).capitalize(), s(failure))

    def num_players(self):
        return len(set([ p.client.address for p in self.players.values() if p.client
                       ]))
    ...
    def set_press_level(self, client, match):
        ...
        if specific:
            try:
                new_level = self.validator.press_levels[level]
            except KeyError:
        ...
        if new_level == old_level:
            ...
            self.admin('%s has set the press level to %d (%s).', client.full_name(), new_level, self.validator.press_levels[new_level])

    def end_game(self, client, match):
        if self.closed:
            client.admin('The game is already over.')
        else:
            self.queue_action(client, self.close, 'ending the game.', None, 'ending the game.', ('end',
                                                                                                 'close'))
        return
    ...
    def veto_admin(self, client, match):
        word = match.group(2)
        if word:
            actions = [ a for a in self.actions if word in a.terms ]
        else:
            actions = list(self.actions)
        if actions:
            for vetoed in actions:
                vetoed.veto(client)

        else:
            client.admin('%s to %s.', word and 'No %s commands' % word or 'Nothing', match.group(1))

    commands = [
     Command('who|list players', list_players, '  list players - Lists the player names (but not power assignments)'),
     Command('(veto|cancel|reject) *(\\w*)', veto_admin, '  veto [<command>] - Cancels recent admin commands'),
     Command('(en|dis)able +press *(level +(\\d+|[a-z ]+))?', set_press_level, '  enable/disable press - Allows or blocks press between powers'),
     Command('pause', stop_time, '  pause - Stops deadline timers and phase transitions'),
     Command('resume|unpause', resume, '  resume - Resumes deadline timers and phase transitions'),
     Command('\\b(\\w+) time limit( \\d+|)', set_time_limit, '  <phase> time limit [<seconds>] - Set or display the time limit for <phase>'),
     Command('(eject|boot) +(.+)', eject, '  eject <player> - Disconnect <player> (either name or country) from the game'),
     Command('end game', end_game, '  end game - Ends the game (without a winner)'),
     Command('start (an? )?(\\w+) as (\\w+)', start_bot, '  start <bot> as <country> - Start a copy of <bot> to play <country>'),
     Command('start (an? |\\d+ )?(\\w+)()', start_bot, '  start <number> <bot> - Invites <number> copies of <bot> into the game')]
