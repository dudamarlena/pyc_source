...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/validation.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance message validation
...
class Validator(VerboseObject):
    ...
    def parse_syntax_file(klass, stream):
        ...
        """
        tla = re.compile('[A-Z0-9]{3}$')
        strings = re.compile("'\\w+'")
        repeats = re.compile('(.*) +\\1 *\\.\\.\\.')
        lev_mark = re.compile('<h2><a name="level(\\d+)">.*: (\\w[a-zA-Z ]*)<')
        bnf_rule = re.compile('<(?:h4|li><b)>({\\w\\w\\w} |)(?:<a name="\\w+">|)(\\w+) (\\+|-|)= (.*?)<(?:/h4|/b|sup)>')
        syntax_level = 0
        ...
        def parse_rule(level, rule):
            ...
            while ']' in words:
                ...
                for (index, word) in enumerate(words):
                    if word == ']':
                        words[start:(index + 1)] = subrule(level, words[start:index]).split()
                        break
                    elif word == 'optional':
                        start = index
    ...
    def validate(self, msg, base_expression):
        ...
        if msg.count(BRA) != msg.count(KET):
            if msg[0] == PRN:
                ...
                self.log_debug(17, 'Bracket counts differ: %d != %d', msg.count(BRA), msg.count(KET))
                return PRN(msg)
        else:
            (index, valid) = self.validate_expression(msg, base_expression)
            if valid and index == len(msg):
                return False
            else:
                if index < len(msg) and msg[index] == KET:
                    ...
                    if submsg.count(BRA) < submsg.count(KET):
                        if msg[0] == PRN:
                            ...
                            self.log_debug(17, 'Bracket counts: %d < %d', submsg.count(BRA), submsg.count(KET))
                            return PRN(msg)
                result = HUH(msg)
    ...
    def validate_expression(self, msg, sub):
        ...
        for (level, sub_list) in self.syntax[sub]:
            if level <= self.syntax_level:
                self.spaces += 1
                self.log_debug(16, '%sChecking "%s" against %s', ' ' * self.spaces, msg, sub_list)
                (result, good) = self.validate_option(msg, sub_list)
                self.log_debug(16, '%sResult: %s, %s', ' ' * self.spaces, result, good)
                self.spaces -= 1
                if good == valid and result > best:
                    best = result
                    if valid and best == length:
                        break
                    ...
                    if in_sub:
                        ...
                        if protocol.token_cats.has_key(opt):
                            ...
                            if isinstance(num, tuple):
                                ...
                                check = lambda x: x.category == num
                        elif opt == 'Token':
                            check = lambda x: x not in (BRA, KET, ERR)
                        else:
                            raise ValueError('unknown category "%s"' % opt)
...
def count_valid(msg, func, repeat):
    ...
    if repeat:
        index = 0
        length = len(msg)
        while index < length and func(msg[index]):
            index += 1

        return index
    elif msg and func(msg[0]):
        return 1
    else:
        return 0
