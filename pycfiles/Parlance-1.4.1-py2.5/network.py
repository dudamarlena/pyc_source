...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/network.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance client/server communications
...
class Connection(SocketWrapper):
    ...
    def process_message(self, type_code, data):
        ...
        if first is not None and type_code not in (first, self.FM, self.EM):
            if self.is_server:
                self.send_error(self.proto.NotIMError)
            else:
                self.send_error(self.proto.NotRMError)
            ...
            if len(data) == 2:
                self.send_error(unpack('!H', data)[0], True)
            else:
                self.send_error(self.proto.LengthError)
        else:
        ...
        if len(data) == 4:
            ...
            if magic == self.proto.magic:
                if version == self.proto.version:
                    ...
                    self.send_error(self.proto.VersionError)
            elif unpack('<HH', data)[1] == self.proto.magic:
                self.send_error(self.proto.EndianError)
            else:
                self.send_error(self.proto.MagicError)
        else:
            self.send_error(self.proto.LengthError)
    ...
    def unpack_message(self, data):
        ...
        """
        try:
            result = Message([ self.rep[x] for x in unpack('!' + 'H' * (len(data) // 2), data)
                             ])
        except ValueError:
    ...
    def send_RM(self):
        ...
        if self.options.null_rm and self.rep == self.proto.default_rep:
            ...
            data = str.join('', (pack('!H3sx', token.number, name) for (name, token) in self.rep.items()))
    ...
    def send_dcsp(self, msg_type, data):
        ...
        if self.sock and not self.broken:
            ...
            try:
                self.sock.sendall(pack('!BxH', msg_type, len(data)) + data)
            except socket.error, e:
            ...
            self.log_debug(7, 'Trying to send on a closed connection')
...
class Client(Connection):
    ...
    @property
    def prefix(self):
        return (self.player and self.player.prefix or self.pclass.__name__) + ' Client'
    ...
    def open(self):
        ...
        port = (
         self.options.host or 'localhost', self.options.port)
        ...
        try:
            ...
            self.log_debug(7, 'Could not set SO_KEEPALIVE')
    ...
    def run(self):
        ...
        if msg and not self.rep:
            ...
            self.player = self.pclass(send_method=self.send, representation=self.rep, **self.kwargs)
...
class Service(Connection):
    ...
    def full_name(self):
        return self.name and '%s (%s)' % (self.name, self.version) or 'An observer'
    ...
    def run(self):
        ...
        if msg:
            ...
            try:
                ...
                self.log_debug(1, '%s handling "%s": %s', e.__class__.__name__, msg, e)
                self.server.broadcast_admin('An error has occurred.  The server may be unreliable until it is restarted.')
                self.errors += 1
                if self.errors > 3:
                    self.admin('You have caused too much trouble.  Good-bye.')
                    self.boot()
                else:
                    self.admin("Please don't do that again, whatever it was.")
    ...
    def admin(self, line, *args):
        self.send(ADM('Server')(str(line) % args))
...
class ServerSocket(SocketWrapper):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_debug(6, 'Connection from %s as client #%d', addr, self.next_id)
            try:
