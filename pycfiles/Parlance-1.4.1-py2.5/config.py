...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/parlance/config.py
# Compiled at: 2009-08-22 22:50:09
"""Parlance configuration management
...
from functions import any, autosuper, defaultdict, expand_list, s, settable_property
...
def boolean(value):
    if str(value).lower() in ('yes', 'true', 'on'):
        ...
        raise ValueError('Unrecognized boolean value; try "yes" or "no"')
    return result
...
class Configuration(object):
            ...
            def attempt_arg(option):
                ...
                if self._args.has_key(option):
                    ...
                    try:
                        ...
                        print 'Warning: Illegal value for argument %s: %r (%s)' % (
                         option, val, str(err))
            ...
            def attempt_file(option):
                ...
                if self._user_config.has_option(section, option):
                    ...
                    try:
                        ...
                        print 'Warning: Illegal value for %s, in section %s of the configuration file: %r (%s)' % (
    ...
    _validators = {bool: boolean, float: number, 
       int: integer, 
       str: string, 
       file: string, 
       list: stringlist}
    ...
    def parse_message(self, message):
        ...
        for var_opt in message.fold()[(-1)]:
            ...
            if len(var_opt) == 1:
                ...
                setattr(self, text, int(Time(*var_opt[1:])))
            elif len(var_opt) == 2:
                setattr(self, text, var_opt[1])
            else:
                raise ValueError('Unknown variant option in %s message' % message[0].text)

    def sanitize(self):
        """ Performs a few sanity checks on the options."""
        if self.LVL >= 10:
            if self.MTL > 0:
                if self.PTL >= self.MTL:
                    self.PTL = self.MTL
            else:
                self.PTL = 0
        else:
    ...
    def get_params(self):
        ...
        params = [
         (
          LVL, self.LVL)]
        if self.MTL:
            params.append((MTL, Time(self.MTL)))
        if self.RTL:
            params.append((RTL, Time(self.RTL)))
        if self.BTL:
            params.append((BTL, Time(self.BTL)))
        if self.AOA:
            params.append((AOA,))
        if self.DSD:
            params.append((DSD,))
        if self.LVL >= 10:
            ...
            if self.NPB:
                params.append((NPB,))
            if self.PTL:
                params.append((PTL, Time(self.PTL)))
        return params
...
def read_representation_file(stream):
    ...
    if num_tokens > 0:
        ...
        if num_tokens == 0:
            return Representation(rep, protocol.base_rep)
        else:
            raise ValueError('Wrong number of lines in representation file')
    else:
        return protocol.default_rep


class EntryPointContainer(object):
    """Wrapper for a set of entry points, to be used like a dict.
...
class ConfigPrinter(Configuration):
    ...
    def __init__(self):
        self.names = {'str': 'string', 
           'int': 'integer', 
           'bool': 'boolean', 
           'float': 'number', 
           'intlist': 'list of integers'}
        self.intro = ('Example configuration file for Parlance, a Diplomacy framework',
                      ...
                      'those in the former.', '', 'Lines starting with hash marks (#) or semicolons (;) are ignored,',
                      'and may be used for comments.  In this sample file, semicolons are',
                      'used to show the default setting for each option.', '', 'The sections are ordered in approximate likelihood of customization.')
        ...
        headers['syntax'] = ('Syntax files and special tokens.', 'Note: File names are relative to the current working directory,',
    ...
    def walk(self, package):
        ...
        for section in sorted(self.modules.keys()):
            header = (
             'Options for the %s module.' % section,)
            if section not in self.headers:
                self.print_section(section, header)


def run():
    """Print out a configuration file template.
