...
# [GCC 8.4.0]
# Embedded file name: /Users/Newville/Codes/xraylarch/examples/io/AthenaProjects/athena.py
# Compiled at: 2018-05-03 15:55:08
# Size of source mod 2**32: 23118 bytes
"""
...
def make_athena_args(group, hashkey=None, **kws):
    ...
    args = {}
    for k, v in (('annotation', ''), ('beamline', ''), ('beamline_identified', '0'),
                 ('bft_dr', '0.0'), ('bft_rmax', '3'), ('bft_rmin', '1'), ('bft_rwindow', 'hanning'),
                 ('bkg_algorithm', 'autobk'), ('bkg_cl', '0'), ('bkg_clamp1', '0'),
                 ('bkg_clamp2', '24'), ('bkg_delta_eshift', '0'), ('bkg_dk', '1'),
                 ('bkg_e0_fraction', '0.5'), ('bkg_eshift', '0'), ('bkg_fixstep', '0'),
                 ('bkg_flatten', '1'), ('bkg_former_e0', '0'), ('bkg_funnorm', '0'),
                 ('bkg_int', '7.'), ('bkg_kw', '1'), ('bkg_kwindow', 'hanning'),
                 ('bkg_nclamp', '5'), ('bkg_rbkg', '1.0'), ('bkg_slope', '-0.0'),
                 ('bkg_stan', 'None'), ('bkg_tie_e0', '0'), ('bkg_nc0', '0'), ('bkg_nc1', '0'),
                 ('bkg_nc2', '0'), ('bkg_nc3', '0'), ('bkg_rbkg', '1.0'), ('bkg_slope', '0'),
                 ('prjrecord', 'athena.prj, 1'), ('chi_column', ''), ('chi_string', ''),
                 ('collided', '0'), ('columns', ''), ('daq', ''), ('denominator', '1'),
                 ('display', '0'), ('energy', ''), ('energy_string', ''), ('epsk', ''),
                 ('epsr', ''), ('fft_dk', '4'), ('fft_edge', 'k'), ('fft_kmax', '15.'),
                 ('fft_kmin', '2.00'), ('fft_kwindow', 'kaiser-bessel'), ('fft_pc', '0'),
                 ('fft_pcpathgroup', ''), ('fft_pctype', 'central'), ('forcekey', '0'),
                 ('from_athena', '1'), ('from_yaml', '0'), ('frozen', '0'), ('generated', '0'),
                 ('i0_scale', '1'), ('i0_string', '1'), ('importance', '1'), ('inv', '0'),
                 ('is_col', '1'), ('is_fit', '0'), ('is_kev', '0'), ('is_merge', ''),
                 ('is_nor', '0'), ('is_pixel', '0'), ('is_special', '0'), ('is_xmu', '1'),
                 ('ln', '0'), ('mark', '0'), ('marked', '0'), ('maxk', '15'), ('merge_weight', '1'),
                 ('multiplier', '1'), ('nidp', '5'), ('nknots', '4'), ('numerator', ''),
                 ('plot_scale', '1'), ('plot_yoffset', '0'), ('plotkey', ''), ('plotspaces', 'any'),
                 ('provenance', ''), ('quenched', '0'), ('quickmerge', '0'), ('read_as_raw', '0'),
                 ('rebinned', '0'), ('recommended_kmax', '1'), ('recordtype', 'mu(E)'),
                 ('referencegroup', ''), ('rmax_out', '10'), ('signal_scale', '1'),
                 ('signal_string', '-1'), ('trouble', ''), ('tying', '0'), ('unreadable', '0'),
                 ('update_bft', '1'), ('update_bkg', '1'), ('update_columns', '0'),
                 ('update_data', '0'), ('update_fft', '1'), ('update_norm', '1'),
                 ('xdi_will_be_cloned', '0'), ('xdifile', ''), ('xmu_string', '')):
        args[k] = v
...
def parse_perlathena(text, filename):
                        ...
                        if key == 'journal':
                            ...
                            if key == 'args':
                                ...
                                if key == 'xdi':
                                    ...
                                    if key in ('x', 'y', 'i0', 'signal', 'stddev'):
    ...
    for dat in athenagroups:
        ...
        this = Group(athena_id=label, energy=(dat['x']), mu=(dat['y']), bkg_params=(Group()),
          fft_params=(Group()),
          athena_params=(Group()))
        if 'i0' in dat:
            this.i0 = dat['i0']
...
def parse_jsonathena(text, filename):
    ...
    for key, val in jsdict.items():
        if key.startswith('_____head'):
            ...
            if key.startswith('_____journ'):
                journal = val
            else:
                if key.startswith('_____order'):
                    athena_names = val
    ...
    for name in athena_names:
        ...
        this = Group(athena_id=name, energy=x, mu=y, bkg_params=(Group()),
          fft_params=(Group()),
          athena_params=(Group()))
        if 'i0' in dat:
...
class AthenaProject(object):
    ...
    def read(self, filename=None, match=None, do_preedge=True, do_bkg=True, do_fft=True, use_hashkey=False):
        """
        ...
        for gname in data.group_names:
            ...
            if do_preedge or do_bkg:
                if self._larch is not None:
                    ...
                    pre_edge(this, e0=(float(pars.e0)), pre1=(float(pars.pre1)),
                      ...
                      _larch=(self._larch))
                    if do_bkg:
                        if hasattr(pars, 'rbkg'):
                            autobk(this, _larch=(self._larch), e0=(float(pars.e0)), rbkg=(float(pars.rbkg)),
                            ...
                            if do_fft:
                                ...
                                if hasattr(pars, 'kw'):
                                    kweight = float(pars.kw)
                                xftf(this, _larch=(self._larch), kmin=(float(pars.kmin)), kmax=(float(pars.kmax)),
                                  kweight=kweight,
                                  window=(pars.kwindow),
                                  dk=(float(pars.dk)))
...
def read_athena(filename, match=None, do_preedge=True, do_bkg=True, do_fft=True, use_hashkey=False, _larch=None):
    ...
    aprj.read(filename, match=match, do_preedge=do_preedge, do_bkg=do_bkg, do_fft=do_fft,
...
def registerLarchPlugin():
    return (
     '_io',
     {'read_athena':read_athena,  'create_athena':create_athena})