...
# [GCC 8.4.0]
# Embedded file name: /Users/Newville/Codes/xraylarch/tests/test_interpreter.py
# Compiled at: 2017-04-05 21:43:24
"""
Base TestCase for asteval
"""
import ast, os, textwrap, time, unittest
...
class TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.outfile = '_stdout_'
        self.interp.writer = NamedTemporaryFile('w', delete=False, prefix='larchtest')
        if not HAS_NUMPY:
...
class TestEval(TestCase):
    ...
    def test_assignment(self):
        ...
        if HAS_NUMPY:
            ...
            self.isnear('a', np.array([0.0, 1.0, 1.5, 2.0, 2.5, 5.0, 6.0, 7.0, 8.0, 9.0]))
    ...
    def test_syntaxerrors_1(self):
        ...
        for expr in ('class = 1', 'for = 1', 'if = 1', 'raise = 1', '1x = 1', '1.x = 1',
                     '1_x = 1'):
            self.interp(expr)
    ...
    def test_reservedwords(self):
        """test reserved words"""
        for w in ('and', 'as', 'while', 'raise', 'else', 'class', 'del', 'def', 'import',
                  'None', 'True', 'False', 'eval'):
            self.interp.error = []
            self.interp('%s=2' % w)
            self.check_error('SyntaxError')
