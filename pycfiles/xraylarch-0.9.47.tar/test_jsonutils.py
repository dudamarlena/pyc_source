...
# [GCC 8.4.0]
# Embedded file name: /Users/Newville/Codes/xraylarch/tests/test_jsonutils.py
...
import unittest, nose, numpy as np
...
class EncodeDecode4Json_Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.group1 = Group(a=1.030405, label='a string', opts={'a': 1, 'b': 2}, x=np.linspace(0, 10, 21), y=np.sqrt(np.arange(3) / 5.0))
        self.group2 = Group(par1=Parameter(name='p1', value=3.0, min=0.0, vary=False), par2=Parameter(name='p2', value=1.0, vary=True), sub=Group(label='a label', x=np.linspace(0, 10, 21)))
        self.group3 = Group(par1=Parameter(name='p1', value=3.0, min=0.0), dx={'a': complex(1.3, 0.2), 'blist': [1, 2, 3.0], 'tup': (0, None)})
        return
    ...
    def test_encode_array1(self):
        ...
        assert_allclose(out['value'][1], 0.142857, rtol=0.0001)

    def test_eval_array1(self):
        testval = np.arange(10) / 7.2
        assert_allclose(deval(testval), testval, rtol=0.0001)
    ...
    def test_encode_array2(self):
        ...
        assert_allclose(arr[1][2], 3.5, rtol=0.0001)

    def test_eval_array2(self):
        testval = (np.arange(20) / 7.2).reshape((4, 5))
        out = deval(testval)
        assert out.shape == (4, 5)
        assert_allclose(out, testval, rtol=0.0001)

    def test_encode_array3(self):
        out = encode(np.arange(10) / 2.0 - complex(0.0, 1.0) * np.arange(10))
        assert isinstance(out, dict)
        assert out['__class__'] == 'Array'
        assert out['__shape__'] == (10, )
        ...
        assert len(val) == 2
        assert len(val[0]) == 10
        assert_allclose(val[0][4], 2.0, rtol=0.0001)
        assert_allclose(val[1][3], -3.0, rtol=0.0001)

    def test_eval_array3(self):
        testval = np.arange(10) / 2.0 - complex(0.0, 1.0) * np.arange(10)
        out = deval(testval)
    ...
    def test_encode_group1(self):
        ...
        assert_allclose(out['opts']['a'], 1.0, rtol=0.0001)
        assert_allclose(out['x']['value'][:3], [0, 0.5, 1.0], rtol=0.0001)
    ...
    def test_eval_group1(self):
        ...
        assert out.opts['a'] == 1
        assert_allclose(out.a, 1.03, rtol=0.01)
        assert_allclose(out.x, np.linspace(0, 10, 21), rtol=0.01)
    ...
    def test_eval_group3(self):
        ...
        assert isgroup(out)
        assert out.dx['a'] == complex(1.3, 0.2)
