...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\musicbutler\musicbutler.py
# Compiled at: 2008-08-18 16:04:45
import speech, random, thread, time, math, mp3play
...
class MusicButler():
    ...
    def _rebuild_commands(self):
        ...
        commands['%s, are you there' % name] = (
         self._ping,)
        commands['%s, what bands do i have' % name] = (
         self._listbands,)
        ...
        numbers = ('zero ten twenty thirty fourty fifty sixty seventy' + 'eighty ninety').split()
        ...
        self._addaction(commands, (self._volume, 100), [
         'volume one hundred',
         'volume a hundred'], ())
        commands['%s, louder' % name] = (
         self._volumechange, True, False)
    ...
    def startlistening(self):
        ...
        if self.islistening():
            self.stoplistening()
        self._speechlistener = speech.listenfor(self._commands.keys(), self._respond_to_command)
    ...
    def _sayname(self):
        self.say('My name is %s.  Always say my name first to get my attention.' % self.name)
    ...
    def _listalbums(self, bandOfChoice=None):
        ...
        if bandOfChoice:
            ...
            albums = [ '%s by %s' % (album, band) for band in collection for album in collection[band]
                     ]
    ...
    def _listsongs(self, bandOfChoice=None):
        collection = self._collection
        if bandOfChoice:
            titles = [ title for (album, songs) in collection[bandOfChoice].items() for (title, locator) in songs
                     ]
        else:
            titles = [ '%s by %s' % (title, band) for band in collection for (album, songs) in collection[band].items() for (title, locator) in songs
                     ]
...
class ID3Reader(object):
    ...
    def read(self):
        ...
        for locator in self._searcher.search():
            ...
            if 'album' not in data or 'artist' not in data or 'title' not in data:
                continue
            artist, album, title = data['artist'], data['album'], data['title']
