...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/looker_client/looker_client.py
# Compiled at: 2016-05-18 12:11:55
import json, urllib
...
class LookerClient(object):
    ...
    def get_access_token_to_looker(self):
        ...
        :return:
        """
        res = requests.post(('{0}:{1}/api/3.0/login?client_id={2}&client_secret={3}').format(self.host, self.port, self.token, self.secret))
    ...
    def get_df_from_slug(self, slug):
        ...
        """
        slug = slug.split('/')[(-1)]
        get_slug = requests.get(('{0}:{1}/api/3.0/queries/slug/{2}?access_token={3}').format(self.host, self.port, slug, self.access_token))
        query_id = json.loads(get_slug.text)['id']
        url = ('{0}:{1}/api/3.0/queries/{2}/run/csv?access_token={3}').format(self.host, self.port, query_id, self.access_token)
    ...
    def parse_looker_link_url(self, share_url):
        ...
        if len(slashsplit) > 5:
            ...
            if fields:
                ...
                return [
                 explore, models, dimensions, filters, pivots]
            self.flag = 0
    ...
    def get_useful_part(input_url):
        ...
        """
        test_list = [
         '&sorts', '&limit', '&column_limit', '&vis', '&filter_config', 'dynamic_fields', '&show', 'origin']
