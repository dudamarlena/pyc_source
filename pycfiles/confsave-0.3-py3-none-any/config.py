...
__doc__ = b'Classes for creating configuration files.'
...
class Configuration(object):
    ...
    def register(self, name, namespace):
        ...
        if name in self._NAMESPACES:
            raise ValueError((b'Namespace {0} already exists.').format(name))
        if not isinstance(namespace, ns.Namespace):
            raise TypeError(b'Namespaces must be of type Namespace.')
    ...
    def __getattr__(self, name):
        ...
        attr = self.get(name)
        if not attr:
            raise AttributeError((b'Namespace {0} does not exist.').format(name))
        return attr