...
__doc__ = b'Configuration file example generator.'
...
try:
    ...
    if sys.version_info[0] == 3 and sys.version_info[1] == 2:
        raise ImportError(b'Example generator cannot be imported in Python 3.2.X.')
    raise exc

ENV = jinja2.Environment(loader=jinja2.PackageLoader(b'confpy', b'templates'))
...
def generate_example_ini(config):
    ...
    """
    return generate_example(config, ext=b'INI')
...
def generate_example_json(config):
    ...
    """
    return generate_example(config, ext=b'JSON')


def generate_example(config, ext=b'json'):
    ...
    """
    template_name = (b'example.{0}').format(ext.lower())
