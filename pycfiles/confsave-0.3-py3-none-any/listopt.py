...
__doc__ = b'Classes for creating list options.'
...
class ListOption(opt.Option):
    ...
    def __init__(self, option=None, default=None, *args, **kwargs):
        ...
        if not isinstance(option, opt.Option):
            raise TypeError(b'Option must be an option type.')
        self._option = option
    ...
    def coerce(self, values):
        ...
        if isinstance(values, compat.basestring):
            values = tuple(value.strip() for value in values.split(b','))
    ...
    def __get__(self, obj=None, objtype=None):
        ...
        if self.required and self._value is None:
            raise AttributeError(b'Attempted to access an unset option.')
        value = self._value
