...
__doc__ = b'Classes for creating string options.'
...
class PatternOption(option.Option):
    ...
    def __init__(self, pattern=None, *args, **kwargs):
        ...
        if pattern is None:
            raise ValueError(b'The pattern cannot be None.')
    ...
    def coerce(self, value):
        ...
        if not self._re.match(value):
            raise ValueError((b'The value {0} does not match the pattern {1}').format(value, self.pattern))
        return value
