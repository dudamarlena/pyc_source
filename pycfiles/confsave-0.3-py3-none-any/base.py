...
__doc__ = b'Base classes for all config file formats.'
...
class ConfigurationFile(object):
    ...
    def config(self):
        ...
        for namespace in self.namespaces:
            if not hasattr(conf, namespace):
                if not self._strict:
                    continue
                raise exc.NamespaceNotRegistered((b'The namespace {0} is not registered.').format(namespace))
            name = getattr(conf, namespace)
            for item, value in compat.iteritems(self.items(namespace)):
                if not hasattr(name, item):
                    if not self._strict:
                        continue
                    raise exc.OptionNotRegistered((b'The option {0} is not registered.').format(item))
    ...
    def _read(self):
        ...
        with open(self.path, b'r') as (file_handle):
