...
# Embedded file name: /home/kc/code/kevinconway/confpy/confpy/parser.py
# Compiled at: 2019-08-24 21:12:06
__doc__ = b'Configuration file parser API.'
...
FILE_TYPES = {b'ini': ini.IniFile, 
   b'json': json.JsonFile, 
   b'py': pyfile.PythonFile}
...
def configfile_from_path(path, strict=True):
    ...
    """
    extension = path.split(b'.')[(-1)]
    conf_type = FILE_TYPES.get(extension)
    if not conf_type:
        raise exc.UnrecognizedFileExtension((b'Cannot parse file of type {0}. Choices are {1}.').format(extension, FILE_TYPES.keys()))
...
def set_environment_var_options(config, env=None, prefix=b'CONFPY'):
    ...
    for section_name, section in config:
        for option_name, _ in section:
            var_name = (b'{0}_{1}_{2}').format(prefix.upper(), section_name.upper(), option_name.upper())
...
def set_cli_options(config, arguments=None):
    ...
    for section_name, section in config:
        for option_name, _ in section:
            var_name = (b'{0}_{1}').format(section_name.lower(), option_name.lower())
            parser.add_argument((b'--{0}').format(var_name))
    ...
    for section_name, section in config:
        for option_name, option in section:
            if option.required and option.value is None:
                raise exc.MissingRequiredOption((b'Option {0} in namespace {1} is required.').format(option_name, section_name))

    return config


def parse_options(files, env_prefix=b'CONFPY', strict=True):
