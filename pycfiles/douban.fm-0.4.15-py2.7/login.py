...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/doubanfm/API/login.py
...
def request_token():
    ...
    while True:
        ...
        if req_json['r'] == 0:
            post_data = {'liked': req_json['user_info']['play_record']['liked'], 
...
def get_capthca_pic(captcha_id=None):
    options = {'size': 'm', 
       'id': captcha_id}
    r = requests.get('https://douban.fm/misc/captcha', params=options, headers=HEADERS)
