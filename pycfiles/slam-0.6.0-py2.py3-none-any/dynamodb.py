...
# [GCC 8.4.0]
# Embedded file name: /Users/migu7781/Documents/dev/flask/slam/slam/plugins/dynamodb.py
# Compiled at: 2017-05-24 02:37:10
"""DynamoDB Plugin
...
@climax.command()
@climax.argument('--dynamodb-tables', help='Comma-separated list of table names to create for each stage.')
def init(config, dynamodb_tables):
    tables = [ s.strip() for s in dynamodb_tables.split(',') ] if dynamodb_tables is not None else []
    if not tables:
        return
    else:
        table_config = {}
        for table in tables:
            table_config[table] = {'attributes': {'id': 'S'}, 'key': 'id', 
               'read_throughput': 1, 
               'write_throughput': 1}
        ...
        for name, table in config['dynamodb_tables'].items():
            resources.append({'Fn::Join': [
                          ...
                          [
                           ...
                           ':', {'Ref': 'AWS::AccountId'},
                           ':table/',
                           {'Ref': ('{}{}DynamoDBTable').format(stage.title(), name.title())}]]})

    policy = {'PolicyName': 'DynamoDBPolicy', 
...
def _get_dynamodb_key_schema(key):
    ...
    if isinstance(key, list):
        for k in key:
            key_schema.append({'AttributeName': k, 
...
def _get_dynamodb_projection(project):
    if not project:
        p = {'ProjectionType': 'KEYS_ONLY'}
    elif project == 'all':
        p = {'ProjectionType': 'ALL'}
    else:
        p = {'ProjectionType': 'INCLUDE', 'NonKeyAttributes': project}
    return p
...
def cfn_template(config, template):
    ...
    for stage in config['stage_environments'].keys():
        for name, table in config.get('dynamodb_tables', {}).items():
            res[('{}{}DynamoDBTable').format(stage.title(), name.title())] = _get_table_resource(config, stage, name)

    res['FunctionExecutionRole']['Properties']['Policies'].extend(_get_dynamodb_policies(config))
    return template