...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/tinylogan.py
# Compiled at: 2011-10-04 17:14:00
import sys, re, optparse, logging, os.path, ConfigParser
from datetime import datetime, date, time, timedelta
PATTERN = '.*?\\[(?P<date>.*?)\\:(?P<time>\\d\\d\\:\\d\\d\\:\\d\\d).*?\\] "(?:GET|POST) (?P<url>.*?)(?P<querystring>\\?.*?)? HTTP\\/.*?" (?P<code>\\d\\d\\d).*(?P<sec>\\d+)\\/(?P<micros>\\d+)'
logLine = re.compile(PATTERN, re.I)
DAY_PATTERN = '^(?P<day>today|yesterday|tomorrow|week|month|year)(?:(?P<modifier>\\+|\\-)(?P<qty>\\d+))?$'
...
def str2datetime(st):
    ...
    (dd, mmm, yyyy, hh, mm, ss) = (
     st[:2], st[3:6], st[7:11], st[12:14], st[15:17], st[18:20])
...
def analyze(options, logfile):
        ...
        for l in log:
            ...
            if url.endswith('/'):
                ...
                if first:
                    ...
                    first = False
                registry[url] = registry.get(url) or {'micros': curMicros, 'times': 1, 'url': url}
            else:
    ...
    diff_seconds = (td_diff.microseconds + (td_diff.seconds + td_diff.days * 24 * 3600) * 1000000) / 1000000
    ...
    for x in topTotal:
        ...
        print '  %04d - %s %0.3f (%d times, average %0.3f, %0.2f%% of the total)' % (
         cnt,
         x['url'],
         float(x['micros']) / 1000000,
         x['times'],
         float(x['micros']) / x['times'] / 1000000,
         float(x['micros']) / 1000000 * 100 / float(diff_seconds))
    ...
    for x in topAverage:
        ...
        print '  %04d - %s %0.3f (%d times, %d total)' % (
         cnt,
         x['url'],
         float(x['average']) / 1000000,
         x['times'],
         float(x['average']) / 1000000 * x['times'])
...
def main():
    args = sys.argv[1:]
    defaults = {'size': 50, 'keep-query': False, 'min-time': 0, 'max-time': 0, 'min-times': 0, 'start-date': None, 
       'end-date': None, 'skip-time-start': None, 'skip-time-end': None, 'skip-timeperiod-start': None, 
       'skip-timeperiod-end': None, 'includes': [], 'excludes': [], 'skip-days': []}
    if '-U' not in args:
        if os.path.exists(os.path.join(HOME, '.tinylogan')):
            ...
            if '-c' in args:
                ...
                print config.has_section(config_profile) or 'Section "%s" not found in %s' % (config_profile, os.path.join(HOME, '.tinylogan'))
                sys.exit(1)
        else:
    ...
    p.add_option('--size', '-s', type='int', dest='size', default=defaults['size'], help='choose the number of record to store in every log')
    p.add_option('--keep-query', '-q', dest='keep_query', default=defaults['keep-query'], action='store_true', help='keep query strings in URLs instead of cutting them. Using this an URL with different query string is treat like different URLs.')
    p.add_option('--include', '-i', dest='includes', default=defaults['includes'], action='append', metavar='INCLUDE_REGEX', help='a regexp expression that an URLs must match of will be discarded. Can be called multiple times, expanding the set')
    p.add_option('--exclude', '-e', dest='excludes', default=defaults['excludes'], action='append', metavar='EXCLUDE_REGEX', help='a regexp expression that an URLs must not match of will be discarded. Can be called multiple times, reducing the set')
    p.add_option('--skip-day', dest='skip_days', default=defaults['skip-days'], action='append', metavar='SKIP_DAY', help='A regexp that repr specific whole day or a set of dates that must be ignored. Can be called multiple times')
    p.add_option('--min-time', type='int', dest='min_time', default=defaults['min-time'], metavar='MIN_TIME_MILLIS', help='ignore all entries that require less than this amount of millisecs')
    p.add_option('--max-time', type='int', dest='max_time', default=defaults['max-time'], metavar='MAX_TIME_MILLIS', help='ignore all entries that require more than this amount of millisecs')
    p.add_option('--min-times', type='int', dest='min_times', default=defaults['min-times'], metavar='MIN_TIMES', help='set a minimum number of times that a entry must be found to be used in the "Top average time" statistic')
    group = optparse.OptionGroup(p, 'Date filters', 'For those kind of filters you need to specify a date.\nYou are free to use a specific date in the format dd/mmm/aaaa, like "24/May/2011", but also some keywords for relative date like "today", "yesterday", "tomorrow", "week" and "month".\nUse of "week" and "month" mean referring to first day of the current week or month.\nYou can also provide a numerical modifier using "+" or "-" followed by a day quantity\n(example: "week-5" for going back of 5 days from the start of the week).')
    group.add_option('--start-date', dest='start_date', default=defaults['start-date'], help='date where to start analyze and record')
    group.add_option('--end-date', dest='end_date', default=defaults['end-date'], help='date where to end analyze and record')
    p.add_option_group(group)
    group = optparse.OptionGroup(p, 'Time filters', 'When a time is needed, you must enter it in the format hh:mm:ss or simply hh:mm, like "09:21:30" or "09:21".\nThose filter are used for skip record that are registered "too late at night" or "too early in the morning".')
    group.add_option('--skip-timeperiod-start', dest='skip_time_start', default=defaults['skip-timeperiod-start'], help='do not analyse records before the given time')
    group.add_option('--skip-timeperiod-end', dest='skip_time_end', default=defaults['skip-timeperiod-end'], help='do not analyse records later the given time')
    p.add_option_group(group)
    group = optparse.OptionGroup(p, 'Default configuration profiles', 'You can read a set of default configuration options from a ".tinylogan" file placed in the user\'s home directory.\nIf this file is found, parameters from the "DEFAULT" section are read, but you can also add other sections.\nYou can always override those options from the command line.')
    group.add_option('-c', dest='foo', default='default', metavar='PROFILE', help='read a different profile section than DEFAULT')
    group.add_option('-U', dest='foo', default=False, action='store_true', help='Ignore the user default profile file (if exists)')
    group.add_option('--example-profile', dest='example_profile', default=False, action='store_true', help='Print out an example profile file, then exit. You can put this output in a ".tinylogan" file in your home, then customize it')
    p.add_option_group(group)
