...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/abiosgaming/base_client.py
# Compiled at: 2015-12-06 21:22:47
...
class BaseAbiosClient(object):
    ...
    def __init__(self, retries=3, client_id=None, secret=None, access_token=None, endpoint=DEFAULT_ENDPOINT):
        ...
        retry = Retry(total=retries, status_forcelist=[
         500, 401], method_whitelist=frozenset(['POST', 'GET', 'PUT']), backoff_factor=1)
    ...
    def _post(self, url, data):
        ...
        """
        try:
            response = self.session.post(url, data=data, verify=True)
    ...
    def _call(self, url, **parameters):
        ...
        try:
            response = self.session.get(url, params=parameters, verify=True)
        except RetryError as e:
            log.exception(e)
            raise e
