...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/gse/__main__.py
# Compiled at: 2014-02-10 01:51:38
from __init__ import *
import sys, csv, json, cPickle
...
def prettyPrint(obj, fmt='pretty'):
    ...
    if fmt == 'pretty':
        ...
        for ser_md in obj:
            if ser_md == 'samples' or ser_md[0] == '_':
                continue
...
def main():
    ...
    config.add_option('--url', '-U', action='store', type='string', default=':memory:', help='URL of the databse to be created and loaded')
    config.add_option('--separator', '-s', action='store', default='\t', help='Column SEPARATOR string')
    config.add_option('--input', '-i', action='store', type='string', default='', help='Input file in GEO format.')
    config.add_option('--output', '-o', action='store', type='string', default='', help='Output file in TSV format.')
    config.add_option('--group-by', '-g', action='store', type='string', default='0', help='group columns at the specified index level')
    config.add_option('--log2', '-2', action='store_true', default=False, help='expression values will be converted to log2')
    config.add_option('--show-levels', action='store_true', default=False, help='Output an enumerated list of column group levels.')
    config.add_option('--show-metadata', action='store_true', default=False, help='Outpiut metadata.')
    config.add_option('--metadata-format', action='store', type='string', default='pretty', help="Output format for metadata.  Default is 'pretty' print.  Othere format(s): json, html")
    ...
    if opts.output != '':
        ...
        gmd.to_tsv(sys.stdout if opts.output == '-' else open(opts.output, 'w'), level=lev, sep=opts.separator)
    return
