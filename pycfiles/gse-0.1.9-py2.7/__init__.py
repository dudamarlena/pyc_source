...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/gse/__init__.py
# Compiled at: 2014-02-10 01:49:40
import logging
...
"""
    ...
    def __init__(self, geo_file):
        ...
        for r in rows:
            if len(r) < 2 or r[0][0] != '!':
                continue
            if r[0].lower().startswith('!series_matrix_table_begin'):
                break
            self.parse(r)
...
"""
        geo_file.seek(0)
        while not geo_file.readline().startswith('!series_matrix_table_begin'):
            pass
        ...
        if multi_index:
            ...
            level_values = [ [s.sample_id] + [ s.characteristics.get(k, '') for k in level_names ] for s in self._meta.samples if s.sample_id in col_names
                           ]
