...
# [GCC 8.4.0]
# Embedded file name: /home/rahul/.pyenv/versions/py3.6.2/lib/python3.6/site-packages/pwmodel/models.py
# Compiled at: 2018-02-25 12:16:14
# Size of source mod 2**32: 10052 bytes
import heapq, os
from collections import defaultdict
import operator, dawg
from . import helper
...
def create_model(modelfunc, fname='', listw=[], outfname=''):
    ...
    pws = []
    if fname:
        pws = helper.open_get_line(fname, limit=3000000.0)
...
class PwModel(object):
    ...
    def __init__(self, pwfilename=None, **kwargs):
        ...
        self._modelf = get_data_path('{}-{}.dawg.gz'.format(self._leak, self.modelname))
        if freshall:
            os.remove(self._modelf)
        try:
            self._T = read_dawg(self._modelf)
        except IOError as ex:
            print('I could not find the file ({}).\nHang on I am creating the {} model for you!\nex={}'.format(self._modelf, self.modelname, ex))
            self._T = create_model(fname=pwfilename,
    ...
    def qth_pw(self, q):
        ...
        """
        return heapq.nlargest((q + 2), (self._T.items()), key=(operator.itemgetter(1)))[(-1)]
...
class PcfgPw(PwModel):
    __doc__ = 'Creates a pcfg model from the password in @pwfilename. \n    '
    ...
    def pcfgtokensofw(self, word):
        ...
        sym = ['__{0}{1}__'.format(helper.whatchar(w), len(w)) for w in tok]
...
class NGramPw(PwModel):
    ...
    def cprob(self, c, history):
        ...
        if len(history) >= self._n:
            ...
            assert d != 0, 'ERROR: Denominator zero!\nd={} n={} history={!r} c={!r} ({})'.format(d, n, hist, c, self._n)
        return n / d
    ...
    def prob(self, pw):
        ...
        pw = helper.START + pw
        try:
            return helper.prod(self.cprob(pw[i], pw[:i]) for i in range(1, len(pw)))
