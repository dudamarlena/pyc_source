...
# [GCC 8.4.0]
# Embedded file name: C:\Users\pfrias\__src\robolink-blockly-web\src\app\blockly-lib\i18n\common.py
# Compiled at: 2019-07-17 16:03:13
    ...
    for unit in units:
        if not first_entry:
            ...
            if write_key_file:
                key_file.write(',\n')
            qqq_file.write(',\n')
        lang_file.write(('\t"{0}": "{1}"').format(unit['meaning'], unit['source'].replace('"', "'")))
        if write_key_file:
            key_file.write(('"{0}": "{1}"').format(unit['meaning'], unit['key']))
        qqq_file.write(('\t"{0}": "{1}"').format(unit['meaning'], unit['description'].replace('"', "'").replace('{lb}', '{').replace('{rb}', '}')))
        first_entry = False
