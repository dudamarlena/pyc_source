...
# [GCC 8.4.0]
# Embedded file name: /Users/val/Projects/workon/mpro/doozdev/backend/restful-backend/apps/geoware/management/utils/downloader.py
# Compiled at: 2017-01-27 09:53:00
# Size of source mod 2**32: 6252 bytes
import os, sys, time, mmap, requests, zipfile, tarfile, logging, resource, progressbar
...
class FileDownloader(object):
    ...
    def _get_progress_widgets(self):
        ...
        """
        format_custom_text = progressbar.FormatCustomText('Fetching [ %(file)s ] :: ', dict(file=self.remote_file_name))
        widgets = [
         format_custom_text,
         progressbar.ETA(),
    ...
    def download(self, force=False):
        ...
        self.stdout.write('Downloading file ({file}) from ({host})\n'.format(file=self.remote_file_name, host=urlparse(self.remote_file_path).hostname))
    ...
    def extract(self):
        ...
        if filetype.endswith('.zip'):
            ...
            if filetype.endswith('.tar.gz') or file_path.endswith('.tgz'):
                extractor, mode = tarfile.open, 'r:gz'
            else:
                if filetype.endswith('.tar.bz2') or file_path.endswith('.tbz'):
