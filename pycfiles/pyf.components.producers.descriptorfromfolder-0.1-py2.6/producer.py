...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/producers/descriptorfromfolder/producer.py
...
"""
import codecs, decimal, fnmatch, logging, os, datetime
...
class DescriptorFromFolder(Producer):
    ...
    configuration = [SimpleKey('input_folder', label='Input Folder'),
     SimpleKey('pattern', label='Pattern matching the files names (optional)'),
     SimpleKey('encoding', label='Encoding of the files', default='UTF-8'),
     SimpleKey('descriptor', label='Name of the descriptor to use')]
    ...
    def launch(self, progression_callback=None, message_callback=None, params=None):
        ...
        self.pattern = substitute(self.pattern, lambda x: eval(x), regex='\\|([^|]+?)\\|')
        if not self.pattern:
            self.pattern = '*'
        filelist = self.__get_files()
        d100 = decimal.Decimal('100')
        if bool(filelist):
            descriptor = Descriptor.by_name(self.get_config_key('descriptor')).get_descriptor_object(encoding=self.encoding)
