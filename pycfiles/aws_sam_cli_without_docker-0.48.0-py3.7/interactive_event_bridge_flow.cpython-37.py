...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/init/interactive_event_bridge_flow.py
...
"""
import click
from samcli.lib.schemas.schemas_cli_message_generator import construct_cli_display_message_for_schemas, construct_cli_display_message_for_registries
...
from samcli.lib.schemas.schemas_constants import SCHEMAS_REGISTRY, SCHEMA_NAME, EVENT_BRIDGE_SOURCE, EVENT_BRIDGE_SOURCE_DETAIL_TYPE, PAGE_LIMIT, SCHEMA_ROOT
...
def get_schema_template_details(schemas_api_caller):
    ...
    return {'registry_name':registry_name, 
...
def _get_registry_cli_choice(schemas_api_caller):
    ...
    if len(registry_pages) == 1:
        if len(registry_pages.get(0)) == 1:
    ...
    return _prompt_for_registry_choice(schemas_api_caller,
      registry_pages, 0, next_token, is_last_page, last_page_number=None)
...
def _get_schema_cli_choice(schemas_api_caller, registry_name):
    ...
    if len(schema_pages) == 1:
        if len(schema_pages.get(0)) == 1:
            return schema_pages.get(0)[0]
    click.echo('\nWhich Schema would you like to use?')
    next_token = schemas.get('next_token')
    is_last_page = next_token is None
    return _prompt_for_schemas_choice(schemas_api_caller,
      registry_name, schema_pages, 0, next_token, is_last_page, last_page_number=None)
...
def get_schemas_template_parameter(schema_template_details):
    ...
    return {SCHEMAS_REGISTRY: schema_template_details['registry_name'], 
