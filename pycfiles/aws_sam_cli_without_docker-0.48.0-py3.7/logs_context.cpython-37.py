...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/logs/logs_context.py
...
class LogsCommandContext:
    ...
    def __init__(self, function_name, stack_name=None, filter_pattern=None, start_time=None, end_time=None, output_file=None):
        """
    ...
    def log_group_name(self):
        ...
        if self._stack_name:
            ...
            LOG.debug("Function with LogicalId '%s' in stack '%s' resolves to actual physical ID '%s'", self._function_name, self._stack_name, function_id)
    ...
    def _get_resource_id_from_stack(cfn_client, stack_name, logical_id):
        ...
        """
        LOG.debug("Getting resource's PhysicalId from AWS CloudFormation stack. StackName=%s, LogicalId=%s", stack_name, logical_id)
        try:
            ...
            try:
                LOG.debug('Unable to fetch resource name from CloudFormation Stack: StackName=%s, ResourceLogicalId=%s, Response=%s', stack_name, logical_id, ex.response)
