...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/init/init_templates.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 10848 bytes
"""
Manages the set of application templates.
"""
import itertools, json, os, logging, platform, shutil, subprocess
...
class InitTemplates:
    ...
    def prompt_for_location(self, runtime, dependency_manager):
        ...
        if len(options) == 1:
            ...
            template_md = options[(int(choice) - 1)]
        if template_md.get('init_location') is not None:
            return (
             template_md['init_location'], 'hello-world')
        if template_md.get('directory') is not None:
            return (
             os.path.join(self.repo_path, template_md['directory']), template_md['appTemplate'])
    ...
    def _shared_dir_check(self, shared_dir):
        try:
            shared_dir.mkdir(mode=448, parents=True, exist_ok=True)
            return True
    ...
    def _replace_app_templates(self, temp_path, dest_path):
        try:
            ...
            raise AppTemplateUpdateException('Unstable state when updating app templates. Check that you have permissions to create/delete files in the AWS SAM shared directory or file an issue at https://github.com/awslabs/aws-sam-cli/issues')
    ...
    def _git_executable(self):
        ...
        if platform.system().lower() == 'windows':
            options = [
             execname, '{}.cmd'.format(execname), '{}.exe'.format(execname), '{}.bat'.format(execname)]
        else:
            options = [
             execname]
        for name in options:
