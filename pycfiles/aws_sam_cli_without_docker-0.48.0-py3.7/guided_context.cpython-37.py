...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/deploy/guided_context.py
...
class GuidedContext:
    ...
    def __init__(self, template_file, stack_name, s3_bucket, s3_prefix, region=None, profile=None, confirm_changeset=None, capabilities=None, parameter_overrides=None, save_to_config=True, config_section=None):
    ...
    def guided_prompts(self, parameter_override_keys):
        ...
        stack_name = prompt(f"\t{self.start_bold}Stack Name{self.end_bold}",
          default=default_stack_name, type=(click.STRING))
        ...
        confirm_changeset = confirm(f"\t{self.start_bold}Confirm changes before deploy{self.end_bold}",
        ...
        capabilities_confirm = confirm(f"\t{self.start_bold}Allow SAM CLI IAM role creation{self.end_bold}",
          default=True)
        if not capabilities_confirm:
            input_capabilities = prompt(f"\t{self.start_bold}Capabilities{self.end_bold}",
    ...
    def prompt_parameters(self, parameter_override_keys, start_bold, end_bold):
        ...
        if parameter_override_keys:
            for parameter_key, parameter_properties in parameter_override_keys.items():
                ...
                if no_echo:
                    parameter = prompt(f"\t{start_bold}Parameter {parameter_key}{end_bold}",
                      type=(click.STRING), hide_input=True)
                    _prompted_param_overrides[parameter_key] = {'Value':parameter, 
                     'Hidden':True}
                else:
                    parameter = prompt(f"\t{start_bold}Parameter {parameter_key}{end_bold}",
    ...
    def run(self):
        ...
        if self.save_to_config:
            guided_config.save_config((self._parameter_overrides),
