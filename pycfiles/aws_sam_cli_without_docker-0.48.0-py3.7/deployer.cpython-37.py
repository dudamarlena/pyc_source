...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/lib/deploy/deployer.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 21205 bytes
"""
...
from samcli.commands.deploy.exceptions import DeployFailedError, ChangeSetError, DeployStackOutPutFailedError, DeployBucketInDifferentRegionError
from samcli.commands._utils.table_print import pprint_column_names, pprint_columns, newline_per_item, MIN_OFFSET
import samcli.commands.deploy as deploy_exceptions
...
DESCRIBE_STACK_EVENTS_FORMAT_STRING = '{ResourceStatus:<{0}} {ResourceType:<{1}} {LogicalResourceId:<{2}} {ResourceStatusReason:<{3}}'
DESCRIBE_STACK_EVENTS_DEFAULT_ARGS = OrderedDict({'ResourceStatus':'ResourceStatus', 
...
DESCRIBE_CHANGESET_DEFAULT_ARGS = OrderedDict({'Operation':'Operation', 
 'LogicalResourceId':'LogicalResourceId',  'ResourceType':'ResourceType'})
...
class Deployer:
    ...
    def create_changeset(self, stack_name, cfn_template, parameter_values, capabilities, role_arn, notification_arns, s3_uploader, tags):
        """
        ...
        if not self.has_stack(stack_name):
            ...
            parameter_values = [x['ParameterKey'] not in existing_parameters or x for x in parameter_values if x.get('UsePreviousValue', False)]
        kwargs = {'ChangeSetName':self.changeset_prefix + str(int(time.time())), 
        ...
        if s3_uploader:
            with mktempfile() as (temporary_file):
                ...
                parts = parse_s3_url((s3_uploader.upload_with_dedup(temporary_file.name, 'template')),
    ...
    @pprint_column_names(format_string=DESCRIBE_CHANGESET_FORMAT_STRING,
    ...
    def describe_changeset(self, change_set_id, stack_name, **kwargs):
        ...
        for item in response_iterator:
            ...
            for change in cf_changes:
                ...
                changes[action].append({'LogicalResourceId':resource_props.get('LogicalResourceId'), 
                 'ResourceType':resource_props.get('ResourceType')})

        for k, v in changes.items():
            for value in v:
                row_color = self.deploy_color.get_changeset_action_color(action=k)
                pprint_columns(columns=[
                 changes_showcase.get(k, k), value['LogicalResourceId'], value['ResourceType']],
                  width=(kwargs['width']),
    ...
    def wait_for_changeset(self, changeset_id, stack_name):
        ...
        try:
            ...
            try:
                ...
                if status == 'FAILED' and "The submitted information didn't contain changes." in reason or 'No updates are to be performed' in reason:
                    raise deploy_exceptions.ChangeEmptyError(stack_name=stack_name)
                raise ChangeSetError(stack_name=stack_name,
                  msg=('ex: {0} Status: {1}. Reason: {2}'.format(ex, status, reason)))
            finally:
                ex = None
                del ex
    ...
    def get_last_event_time(self, stack_name):
        ...
        """
        try:
            return utc_to_timestamp(self._client.describe_stack_events(StackName=stack_name)['StackEvents'][0]['Timestamp'])
        except KeyError:
            return time.time()

    @pprint_column_names(format_string=DESCRIBE_STACK_EVENTS_FORMAT_STRING,
    ...
    def create_and_wait_for_changeset(self, stack_name, cfn_template, parameter_values, capabilities, role_arn, notification_arns, s3_uploader, tags):
        try:
            result, changeset_type = self.create_changeset(stack_name, cfn_template, parameter_values, capabilities, role_arn, notification_arns, s3_uploader, tags)
    ...
    @pprint_column_names(format_string=OUTPUTS_FORMAT_STRING,
      format_kwargs=OUTPUTS_DEFAULTS_ARGS,
      table_header=OUTPUTS_TABLE_HEADER_NAME)
            ...
            for k, v in [
             ...
             (
              'Value', output.get('OutputValue'))]:
                pprint_columns(columns=[
                 '{k:<{0}}{v:<{0}}'.format(MIN_OFFSET, k=k, v=v)],
                  width=(kwargs['width']),
                  margin=(kwargs['margin']),
