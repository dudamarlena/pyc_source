...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/local/apigw/local_apigw_service.py
# Compiled at: 2020-03-22 09:26:57
# Size of source mod 2**32: 20440 bytes
"""API Gateway Local Service"""
import io, json, logging, base64, requests
...
class Route:
    ...
    def __eq__(self, other):
        return isinstance(other, Route) and sorted(self.methods) == sorted(other.methods) and self.function_name == other.function_name and self.path == other.path
...
class LocalApigwService(BaseLocalService):
        ...
        * If the same key-value pair is specified in both, the value will only appear once.
        ...
        for header in headers:
            if header in multi_headers:
                if headers[header] in multi_headers[header]:
                    continue
    ...
    def _query_string_params(flask_request):
        ...
        return (
         query_string_dict, multi_value_query_string_dict)
