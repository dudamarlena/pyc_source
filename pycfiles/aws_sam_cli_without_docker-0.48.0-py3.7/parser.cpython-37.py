...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/local/lib/swagger/parser.py
...
class SwaggerParser:
    ...
    _BINARY_MEDIA_TYPES_EXTENSION_KEY = 'x-amazon-apigateway-binary-media-types'
    _ANY_METHOD = 'ANY'
        ...
        for full_path, path_config in paths_dict.items():
            for method, method_config in path_config.items():
                ...
                if not function_name:
                    LOG.debug("Lambda function integration not found in Swagger document at path='%s' method='%s'", full_path, method)
                    continue
