...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/lib/schemas/schemas_api_caller.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 5677 bytes
...
class SchemasApiCaller:
    ...
    def list_registries(self, next_token=None, limit=10):
        ...
        page_iterator = paginator.paginate(PaginationConfig={'StartingToken':next_token, 
    ...
    def list_schemas(self, registry_name, next_token=None, limit=10):
        ...
        page_iterator = paginator.paginate(RegistryName=registry_name,
    ...
    def get_schema_metadata(self, registry_name, schema_name):
        describe_schema_response = self._schemas_client.describe_schema(RegistryName=registry_name,
          SchemaName=schema_name)
        try:
            ...
            return {'event_source':event_source,  'event_source_detail_type':event_source_detail_type, 
             'schema_root_name':schema_root_name, 
             'schemas_package_hierarchy':schemas_package_hierarchy}
        except JSONDecodeError:
            raise SchemasApiException('Parse error reading the content from Schemas response. This should not be possible, please raise an issue.')

    def download_source_code_binding(self, runtime, registry_name, schema_name, schema_version, download_location):
        response = self._schemas_client.get_code_binding_source(Language=runtime,
    ...
    def put_code_binding(self, runtime, registry_name, schema_name, schema_version):
        try:
            self._schemas_client.put_code_binding(Language=runtime,
              RegistryName=registry_name,
              SchemaName=schema_name,
              SchemaVersion=schema_version)
    ...
    def poll_for_code_binding_status(self, schemas_runtime, registry_name, schema_name, schema_version):
        ...
        waiter.wait(Language=schemas_runtime,
          RegistryName=registry_name,
          SchemaName=schema_name,
          SchemaVersion=schema_version)