...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/lib/providers/api_collector.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 7094 bytes
"""
...
class ApiCollector:
    ...
    def __iter__(self):
        ...
        for logical_id, _ in self._route_per_resource.items():
            yield (
             logical_id, self._get_routes(logical_id))
    ...
    def normalize_cors_methods(routes, cors):
        ...
        if not cors:
            return routes
        return [add_options_to_route(route) for route in routes]
