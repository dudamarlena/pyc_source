...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/_utils/options.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 7680 bytes
"""
...
def guided_deploy_stack_name(ctx, param, provided_value):
    ...
    if not guided:
        if not provided_value:
            raise click.BadOptionUsage(option_name=(param.name),
              ctx=ctx,
              message="Missing option '--stack-name', 'sam deploy --guided' can be used to provide and save needed parameters for future deploys.")
    if provided_value:
...
def template_click_option(include_build=True):
    ...
    """
    return click.option('--template-file',
      '--template',
      '-t',
      ...
      show_default=True,
      is_eager=True,
      help=('AWS SAM template which references built artifacts for resources in the template. (if applicable)' if include_build else 'AWS SAM template file.'))
...
def docker_click_options():
    return [
     ...
     click.option('--docker-network',
       ...
       help='Specifies the name or id of an existing docker network to lambda docker containers should connect to, along with the default bridge network. If not specified, the Lambda containers will only connect to the default bridge docker network.')]
...
def parameter_override_click_option():
    return click.option('--parameter-overrides',
      ...
      default={},
      help="Optional. A string that contains AWS CloudFormation parameter overrides encoded as key=value pairs.For example, 'ParameterKey=KeyPairName,ParameterValue=MyKey ParameterKey=InstanceType,ParameterValue=t1.micro' or KeyPairName=MyKey InstanceType=t1.micro")
...
def capabilities_click_option():
    return click.option('--capabilities',
      cls=OptionNargs,
      required=False,
      type=FuncParamType(func=_space_separated_list_func_type),
      help="A list of  capabilities  that  you  must  specifybefore  AWS  Cloudformation  can create certain stacks. Some stack tem-plates might include resources that can affect permissions in your  AWSaccount,  for  example, by creating new AWS Identity and Access Manage-ment (IAM) users. For those stacks,  you  must  explicitly  acknowledgetheir  capabilities by specifying this parameter. The only valid valuesare CAPABILITY_IAM and CAPABILITY_NAMED_IAM. If you have IAM resources,you  can specify either capability. If you have IAM resources with cus-tom names, you must specify CAPABILITY_NAMED_IAM. If you don't  specifythis  parameter, this action returns an InsufficientCapabilities error.")
...
def tags_click_option():
    return click.option('--tags',
      cls=OptionNargs,
      type=(CfnTags()),
      required=False,
      help='A list of tags to associate with the stack that is created or updated.AWS CloudFormation also propagates these tags to resources in the stack if the resource supports it.')
...
def notification_arns_click_option():
    return click.option('--notification-arns',
      cls=OptionNargs,
      type=FuncParamType(func=_space_separated_list_func_type),
      required=False,
      help='Amazon  Simple  Notification  Service  topicAmazon  Resource  Names  (ARNs) that AWS CloudFormation associates withthe stack.')
