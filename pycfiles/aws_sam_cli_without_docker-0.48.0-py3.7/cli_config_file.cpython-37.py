...
class TomlProvider:
    ...
    def __call__(self, config_dir, config_env, cmd_names):
        ...
        try:
            ...
            try:
                LOG.debug('Error reading configuration file at %s with config_env=%s, command=%s, section=%s %s', samconfig.path(), config_env, cmd_names, self.section, str(ex))
            finally:
                ex = None
                del ex
...
def configuration_callback(cmd_name, option_name, config_env_name, saved_callback, provider, ctx, param, value):
    ...
    if saved_callback:
        return saved_callback(ctx, param, config_env_name)
...
def configuration_option(*param_decls, **attrs):
    ...
    def decorator(f):
        ...
        partial_callback = functools.partial(configuration_callback, None, option_name, config_env_name, saved_callback, provider)
