...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/commands/deploy/deploy_context.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 6907 bytes
"""
Deploy a SAM stack
"""
import logging, os, boto3, click
...
class DeployContext:
    ...
    def __init__(self, template_file, stack_name, s3_bucket, force_upload, s3_prefix, kms_key_id, parameter_overrides, capabilities, no_execute_changeset, role_arn, notification_arns, fail_on_empty_changeset, tags, region, profile, confirm_changeset):
    ...
    def run(self):
        ...
        if not isinstance(template_dict, dict):
            raise deploy_exceptions.DeployFailedError(stack_name=(self.stack_name),
              msg=('{} not in required format'.format(self.template_file)))
        ...
        cloudformation_client = boto3.client('cloudformation',
          region_name=(self.region if self.region else None), config=boto_config)
        s3_client = None
        if self.s3_bucket:
        ...
        return self.deploy(self.stack_name, template_str, parameters, self.capabilities, self.no_execute_changeset, self.role_arn, self.notification_arns, self.s3_uploader, [{'Key':key,  'Value':value} for key, value in self.tags.items()] if self.tags else [], region, self.fail_on_empty_changeset, self.confirm_changeset)

    def deploy(self, stack_name, template_str, parameters, capabilities, no_execute_changeset, role_arn, notification_arns, s3_uploader, tags, region, fail_on_empty_changeset=True, confirm_changeset=False):
        try:
            result, changeset_type = self.deployer.create_and_wait_for_changeset(stack_name=stack_name,
