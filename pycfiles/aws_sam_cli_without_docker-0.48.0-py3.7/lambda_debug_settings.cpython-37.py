...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/samcli/local/docker/lambda_debug_settings.py
# Compiled at: 2020-03-21 12:32:11
# Size of source mod 2**32: 8516 bytes
"""
...
class LambdaDebugSettings:
    ...
    def get_debug_settings(debug_port, debug_args_list, runtime, options):
        ...
        """
        entrypoint_mapping = {Runtime.java8.value: DebugSettings(entrypoint=([
                               '/usr/bin/java'] + debug_args_list + [
         ...
         Runtime.java11.value: DebugSettings(None,
                                 debug_env_vars={'_JAVA_OPTIONS': f"-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,quiet=y,address=*:{debug_port} -XX:MaxHeapSize=2834432k -XX:MaxMetaspaceSize=163840k -XX:ReservedCodeCacheSize=81920k -XX:+UseSerialGC -XX:-TieredCompilation -Djava.net.preferIPv4Stack=true" + ' '.join(debug_args_list)}), 
         
         Runtime.dotnetcore20.value: DebugSettings(([
                                      '/var/lang/bin/dotnet'] + debug_args_list + ['/var/runtime/MockBootstraps.dll', '--debugger-spin-wait']),
                                       debug_env_vars={}), 
         ...
         Runtime.go1x.value: DebugSettings(([
                              '/var/runtime/aws-lambda-go'] + debug_args_list + ['-debug=true', '-delvePort=' + str(debug_port), '-delvePath=' + options.get('delvePath')]),
                               debug_env_vars={}), 
         
         Runtime.nodejs.value: DebugSettings(([
                                '/usr/bin/node'] + debug_args_list + [
         ...
         Runtime.nodejs43.value: DebugSettings(([
                                  '/usr/local/lib64/node-v4.3.x/bin/node'] + debug_args_list + [
                                  '--debug-brk=' + str(debug_port),
                                  '--nolazy',
         ...
         Runtime.nodejs610.value: DebugSettings(([
                                   '/var/lang/bin/node'] + debug_args_list + [
                                   '--debug-brk=' + str(debug_port),
                                   '--nolazy',
                                   '--max-old-space-size=2547',
                                   '--max-semi-space-size=150',
         ...
         Runtime.nodejs10x.value: DebugSettings([
                                   ...
                                   json.dumps(debug_args_list + [
                                    ...
                                    debug_env_vars={'NODE_PATH': '/opt/nodejs/node_modules:/opt/nodejs/node10/node_modules:/var/runtime/node_modules'}), 
         ...
         Runtime.nodejs12x.value: DebugSettings([
                                   ...
                                   json.dumps(debug_args_list + [
                                    ...
                                    '81920',
                                    '/var/runtime/index.js'])],
                                    debug_env_vars={'NODE_PATH': '/opt/nodejs/node_modules:/opt/nodejs/node12/node_modules:/var/runtime/node_modules'}), 
         
         Runtime.python27.value: DebugSettings(([
                                  '/usr/bin/python2.7'] + debug_args_list + ['/var/runtime/awslambda/bootstrap.py']),
                                   debug_env_vars={}), 
