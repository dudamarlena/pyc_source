...
# [GCC 8.4.0]
# Embedded file name: /home/steven/Documents/Projects/radio/EOR/OthersCodes/21cmFAST/21cmFAST/src/py21cmfast/cli.py
# Compiled at: 2020-01-09 16:07:28
# Size of source mod 2**32: 27981 bytes
"""Module that contains the command line app."""
import builtins, inspect, logging, warnings
from os import path
from os import remove
import click
import matplotlib.pyplot as plt
import numpy as np, powerbox, yaml
from . import _cfg
...
def init(ctx, config, regen, direc, seed):
    ...
    lib.initial_conditions(user_params=user_params,
...
def perturb(ctx, redshift, config, regen, direc, seed):
    ...
    cosmo_params = (lib.CosmoParams)(**cfg.get('cosmo_params', {}))
    _override(ctx, user_params, cosmo_params)
    lib.perturb_field(redshift=redshift,
...
def spin(ctx, redshift, prev_z, config, regen, direc, z_step_factor, z_heat_max, seed):
    ...
    if z_step_factor is None:
        if 'z_step_factor' in cfg:
            z_step_factor = cfg['z_step_factor']
    if z_heat_max is None:
        if 'z_heat_max' in cfg:
            z_heat_max = cfg['z_heat_max']
    lib.spin_temperature(redshift=redshift,
...
def ionize(ctx, redshift, prev_z, config, regen, direc, z_step_factor, z_heat_max, seed):
    ...
    if z_heat_max is None:
        if 'z_heat_max' in cfg:
            z_heat_max = cfg['z_heat_max']
    lib.ionize_box(redshift=redshift,
      astro_params=astro_params,
      flag_options=flag_options,
...
def coeval(ctx, redshift, config, regen, direc, z_step_factor, z_heat_max, seed):
    ...
    if z_heat_max is None:
        if 'z_heat_max' in cfg:
            z_heat_max = cfg['z_heat_max']
    lib.run_coeval(redshift=redshift,
      astro_params=astro_params,
      flag_options=flag_options,
      z_step_factor=z_step_factor,
      z_heat_max=z_heat_max,
...
def lightcone(ctx, redshift, config, regen, direc, max_z, z_step_factor, z_heat_max, seed):
    ...
    if z_heat_max is None:
        if 'z_heat_max' in cfg:
            z_heat_max = cfg['z_heat_max']
    lib.run_lightcone(redshift=redshift,
      max_redshift=max_z,
      astro_params=astro_params,
      flag_options=flag_options,
      z_step_factor=z_step_factor,
...
def _query(direc=None, kind=None, md5=None, seed=None, clear=False):
    cls = list(cache_tools.query_cache(direc=direc, kind=kind, hsh=md5, seed=seed, show=False))
    if not clear:
...
@click.option('-z',
  '--redshift', type=float, default=6.0, help='redshift of the comparison boxes')
@click.option('-Z',
  '--max-redshift',
  type=float,
  default=30,
...
def pr_feature(param, value, struct, vtype, lightcone, redshift, max_redshift, random_seed, verbose, regenerate):
    """
    ...
    structs = {'user_params':{'HII_DIM':128, 
      'BOX_LEN':250}, 
     'flag_options':{'USE_TS_FLUCT': True}, 
     'cosmo_params':{},  'astro_params':{}}
    if lightcone:
        print('Running default lightcone...')
        lc_default = (lib.run_lightcone)(redshift=redshift, 
         max_redshift=max_redshift, 
         random_seed=random_seed, 
         regenerate=regenerate, **structs)
        structs[struct][param] = value
        print('Running lightcone with new feature...')
        lc_new = (lib.run_lightcone)(redshift=redshift, 
         max_redshift=max_redshift, 
         random_seed=random_seed, 
         regenerate=regenerate, **structs)
        ...
        for field in ('brightness_temp', ):
            ...
            plotting.lightcone_sliceplot(lc_default,
              ax=(ax[0]), fig=fig, vmin=vmin, vmax=vmax)
            ax[0].set_title('Default')
            plotting.lightcone_sliceplot(lc_new,
              ax=(ax[1]), fig=fig, cbar=False, vmin=vmin, vmax=vmax)
            ax[1].set_title('New')
            plotting.lightcone_sliceplot(lc_default,
              lightcone2=lc_new, cmap='bwr', ax=(ax[2]), fig=fig)
        ...
        ax[0].set_ylabel('$x_{\\rm HI}$')
        ax[0].legend()
        ax[1].plot((lc_default.node_redshifts),
        ...
        ax[3].set_xlabel('z')
        rms_diff = rms((lc_default.brightness_temp), axis=(0, 1)) - rms((lc_new.brightness_temp),
          axis=(0, 1))
        ax[2].plot((lc_default.lightcone_redshifts), rms_diff, label='RMS')
        ax[2].plot((lc_new.node_redshifts),
        ...
        diff_rms = rms((lc_default.brightness_temp - lc_new.brightness_temp), axis=(0,
                                                                                    1))
        ...
        while start + ncells <= lc_new.shape[(-1)]:
            pd, k = powerbox.get_power(lc_default.brightness_temp[:, :, start:start + ncells], lc_default.lightcone_dimensions[:2] + (chunk_size,))
            p_default.append(pd)
            pn, k = powerbox.get_power(lc_new.brightness_temp[:, :, start:start + ncells], lc_new.lightcone_dimensions[:2] + (chunk_size,))
            p_new.append(pn)
        ...
        for i, (pdef, pnew, kk) in enumerate(zip(p_default[inds], p_new[inds], k[inds])):
            ax[0].plot(z,
              pdef, ls='--', label=('k={:.2f}'.format(kk)), color=('C{}'.format(i)))
            ax[0].plot(z, pnew, ls='-', color=('C{}'.format(i)))
            ax[1].plot(z, (np.log10(pdef / pnew)), ls='-', color=('C{}'.format(i)))

        ax[1].set_xlabel('z')
        ax[0].set_ylabel('$\\Delta^2 [{\\rm mK}^2]$')
        ax[1].set_ylabel('log ratio of $\\Delta^2 [{\\rm mK}^2]$')
        ax[0].legend()
