...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.4-x86_64/egg/wtftz/parser.py
...
def _from(query):
    ...
    """
    pattern = re.compile('from ([A-Za-z/ ]+)')
    matches = pattern.findall(query)
    if len(matches) > 0:
        fromz = matches[0]
        query = pattern.sub('', query, 1)
        return (
         query.strip(), fromz.strip())
...
def _implicit_from(query):
    ...
    for count, token in enumerate(tokens[::-1]):
        ...
        if len(matches) > 0:
            ...
            if len(fromz) <= 1:
                continue
            query = (' ').join(tokens[0:len(tokens) - count - 1] + tokens[len(tokens) - count:])
            return (
             query.strip(), fromz.strip())
