...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fellowiki/controllers/wiki.py
...
import turbogears, turbogears.widgets as w, turbogears.validators as v, copy
...
from wikiparser import macros, tables, lists, headlines, special_characters, escaped_text, links, special_markup, text_style, text_style_simple, structure_modifiers
extensions = [
...
def extract_ressource_key(num_args):
    ...
    def erp(f):
        ...
        def new_f(self, *args, **kw_args):
            return f(self, ressource_key=self.get_ressource_key(args[num_args:]), *args[0:num_args], **kw_args)
...
text_types = [ (t.mime_type, t.name) for t in WikiItemType.select(order_by=[desc(WikiItemType.c.wiki_text), WikiItemType.c.name]) if t.text ]
binary_types = [ (t.mime_type, t.name) for t in WikiItemType.select(order_by=WikiItemType.c.name) if not t.text ]
try:
...
wiki_edit_form = SubmitPreviewTableForm(fields=[w.TextArea(name='wiki_text', label='My Wiki Text', validator=v.NotEmpty), w.TextField(name='comment', label='My Comment', validator=v.NotEmpty), w.CheckBox(name='minor_change', label='This is a minor change.'), w.SingleSelectField(name='data type', label='My Data Type', options=text_types, default=default_text_type), w.HiddenField(name='version', validator=v.All(v.Int, v.NotEmpty))], submit_text='Save', preview_text='Preview', reset_text='Reset')
del text_types
del binary_types
del default_text_type
del default_binary_type
...
class WikiInstance(Controller, identity.SecureResource):
    ...
    def view(self, ressource_key, version=None, tg_errors={}):
        ...
        try:
            if version is None:
                current_version = WikiItem.selectone(and_(WikiItem.c.name == ressource_key, WikiItem.c.wiki_id == self.wiki_id), order_by=desc(WikiItem.c.version), limit=1)
            else:
                current_version = WikiItem.selectone(and_(WikiItem.c.name == ressource_key, WikiItem.c.version == version, WikiItem.c.wiki_id == self.wiki_id))
        ...
        return dict(title='FelloWiki - %s' % ressource_key, page_name=ressource_key, main_content=content, edit_link=edit_link, page_is_editable=page_is_editable)
    ...
    def source(self, ressource_key, version=None, tg_errors={}):
        ...
        if len(tg_errors) > 0:
            raise cherrypy.NotFound
        try:
            selected_version = WikiItem.selectone(and_(WikiItem.c.name == ressource_key, WikiItem.c.version == version, WikiItem.c.wiki_id == self.wiki_id))
        except InvalidRequestError:
            raise cherrypy.NotFound
    ...
    def edit(self, ressource_key, **input_data):
        ...
        preview = None
        try:
            prev_version = WikiItem.selectone(and_(WikiItem.c.name == ressource_key, WikiItem.c.wiki_id == self.wiki_id), order_by=desc(WikiItem.c.version), limit=1)
        except InvalidRequestError:
            prev_version = None
        ...
        if len(input_data) == 0:
            ...
            if len(tg_errors) == 0:
                item = WikiItem(ressource_key, input_data['version'] + 1, self.wiki_id, 'wiki text', text_content=input_data['wiki_text'], cache=parsed_content)
