...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fellowiki/model/model_classes.py
...
from schema import *
__all__ = [
 'DBVersion', 'Visit', 'VisitIdentity', 'Group', 'User', 'Permission', 'Wiki', 'WikiItemType', 'WikiItem', 'Registry', 'initialize']
...
class DBVersion(object):
    ...
    def __repr__(self):
        return 'DBVersion(%s, %s)' % (repr(self.prj), repr(self.db_version))
...
assign_mapper(sc, User, user, properties={'groups': relation(Group, secondary=user_group, backref='users')})
...
assign_mapper(sc, Permission, permission, properties={'groups': relation(Group, secondary=group_permission, backref='permissions')})

class Wiki(object):
    __module__ = __name__

    def __init__(self, name, read_permission=None, write_permission=None, admin_permission=None, protect_read_history=False):
        attributes_from_dict(locals())


assign_mapper(sc, Wiki, wiki, properties={'read_permission': relation(Permission, primaryjoin=wiki.c.read_permission_id == permission.c.permission_id, backref='wiki_read_access'), 'write_permission': relation(Permission, primaryjoin=wiki.c.write_permission_id == permission.c.permission_id, backref='wiki_write_access'), 'admin_permission': relation(Permission, primaryjoin=wiki.c.admin_permission_id == permission.c.permission_id, backref='wiki_admin_access')})
...
assign_mapper(sc, WikiItem, wiki_item, properties={'wiki': relation(Wiki), 'item_type': relation(WikiItemType)})

class Registry(object):
    __module__ = __name__
    TYPES = {'S': 'value_string', 'I': 'value_integer', 'B': 'value_boolean', 'D': 'value_datetime'}
...
assign_mapper(sc, Registry, wiki_registry, properties={'wiki': relation(Wiki, backref='registry_items')})
...
def initialize():
    ...
    wiki_private = Wiki('private', read_permission=private_read, write_permission=private_write, admin_permission=private_admin)
    wiki_protected = Wiki('protected', write_permission=protected_write, admin_permission=protected_admin)
    wiki_public = Wiki('public', admin_permission=public_admin)
