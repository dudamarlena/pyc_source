...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/fellowiki/controllers/wikiparser/tests/test_parser.py
# Compiled at: 2006-11-21 20:30:38
...
from fellowiki.controllers.wikiparser.macros import SPLIT, ESCAPE, MACRO, DEFERRED, BLOCK_LEVEL
from fellowiki.controllers.wikiparser import macros, tables, lists, headlines, special_characters, escaped_text, links, special_markup, text_style, text_style_simple, structure_modifiers
extensions = [
 macros, tables, lists, headlines, special_characters, escaped_text, links, special_markup, text_style, text_style_simple, structure_modifiers]
_xhtml_dtd = xmldtd.load_dtd(join(dirname(abspath(__file__)), '..', '..', '..', 'third_party', 'w3c', 'xhtml1-strict.dtd'))
...
def wiki_ut(fname, tc_extension, parser1, parser2):
    ...
    xhtml_validator.val.dtd = xhtml_validator.dtd = xhtml_validator.ent = _xhtml_dtd
    try:
        ...
        xhtml_tree_wiki_ext = Element('html', xmlns='http://www.w3.org/1999/xhtml')
        ...
        xhtml_validator.parse_resource(xhtml_fn)
    except AssertionError, ex_val:
        ex_tb = sys.exc_info()[(-1)]
        try:
            gen_file = codecs.open('%s.%s.generated.xml' % (fname, tc_extension), 'w+', 'utf8')
...
def make_ut_wiki(tc_extension, parser1, parser2, tc_path=join(dirname(abspath(__file__)), 'tests'), **kwargs):
    ...
    fnames = [ file_ for file_ in fnames if file_[0:1] != '.' ]
    fnames = [ join(tc_path, file_) for file_ in fnames ]
    files = [ splitext(file_)[0] for file_ in fnames if splitext(file_)[1] == '.wiki' ]
    dirs = [ file_ for file_ in fnames if isdir(file_) ]
    for file_ in files:
        yield (
         wiki_ut, file_, tc_extension, parser1, parser2)
...
def test_parser_typical():
    ...
    def _link_test_proc(element, target):
        if target in ['link1', 'link 2', 'link   3', 'Ärger']:
    ...
    def _image_link_test_proc(element, image, description, new_subelement):
        if image in ['link1', 'link 2', 'link   3', 'äöü', 'look out for éèê']:
            if new_subelement:
    ...
    procs = {MACRO + 'macro default': {PROC: _macro_test_proc('default macro'), SPLIT: True, ESCAPE: True}, MACRO + 'macro no split': {PROC: _macro_test_proc('no split'), ESCAPE: True}, MACRO + 'macro no escape': {PROC: _macro_test_proc('no escape'), SPLIT: True}, MACRO + 'macro w/o s/e': {PROC: _macro_test_proc('no split, escape')}, MACRO + 'macro deferred': {PROC: _macro_test_proc('deferred'), SPLIT: True, ESCAPE: True, DEFERRED: True}, MACRO + 'macro block level': {PROC: _macro_test_proc('block level'), SPLIT: True, ESCAPE: True, BLOCK_LEVEL: True}, MACRO + 'macro b/l, def': {PROC: _macro_test_proc('block level, deferred'), SPLIT: True, ESCAPE: True, DEFERRED: True, BLOCK_LEVEL: True}, LINK: {PROC: _link_test_proc}, IMAGE_LINK: {PROC: _image_link_test_proc}}
    for (num, name) in enumerate(['testmacro', 'test macro2', 'Test: -Macro3-', 'testmacro4', 'testmacro5', 'testmacro6', 'testmacro7', 'testmacro8', 'testmacro9', 'test macro 10', 'test macro | 11 ', 'test macro \\', 'test macro \\ ', 'test macro \\\\', 'test macro \\\\ ']):
        procs[MACRO + name] = {PROC: _macro_test_proc('testmacro %i' % num), SPLIT: True, ESCAPE: True}

    procs2 = copy(procs)
    procs2[MACRO + 'macro default'] = ({PROC: _macro_test_proc('default macro DEFERRED'), SPLIT: True, ESCAPE: True},)
    procs2[MACRO + 'macro deferred'] = {PROC: _macro_test_proc('DEFERRED'), SPLIT: True, ESCAPE: True, DEFERRED: True}
    procs2[MACRO + 'macro b/l, def'] = {PROC: _macro_test_proc('DEFERRED and block level'), SPLIT: True, ESCAPE: True, DEFERRED: True, BLOCK_LEVEL: True}
