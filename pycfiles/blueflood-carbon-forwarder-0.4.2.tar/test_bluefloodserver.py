...
# [GCC 8.4.0]
# Embedded file name: /Users/vinny.ly/workspace/cloud-metrics/blueflood-carbon-forwarder/tests/test_bluefloodserver.py
# Compiled at: 2016-05-31 16:44:01
import StringIO, json, mock, pytest, twisted.plugins.graphite_blueflood_plugin as plugin
...
def test_send_blueflood():
    ...
    factory.protocol = plugin.MetricLineReceiver
    plugin.MetricService(protocol_cls=factory.protocol, endpoint='', interval=5, blueflood_url='http://bluefloodurl:190', tenant='tenant', ttl=30)._setup_blueflood(factory, agent)
    proto = factory.buildProtocol(('127.0.0.1', 0))
    tr = proto_helpers.StringTransport()
    proto.makeConnection(tr)
    proto.dataReceived('foo.bar.baz 123 123456789.0\n')
