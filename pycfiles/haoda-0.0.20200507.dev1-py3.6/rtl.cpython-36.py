...
class ReportXoUtil(backend.Vivado):
    ...
    """
        ...
        with zipfile.ZipFile(xo_file) as (xo_zip):
            ...
            xo_zip.extractall(path=(self.tmpdir.name), members=[name for name in xo_zip.namelist() if name.startswith(hdl_dir_prefix)])
            if part_num is None:
                for hdl_file_format in ('{}.v', '{0}_{0}.v'):
                    try:
                        with open(os.path.join(hdl_dir, hdl_file_format.format(kernel_name))) as (hdl_file):
        ...
        report_util_args = ' '.join(('-{} {}'.format)(*kv) for kv in report_util_kwargs.items())
        kwargs = {'output_dir':os.path.join(self.tmpdir.name, 'output'), 
         'hdl_dir':hdl_dir, 
        ...
        if self.job_server_fd is not None:
            new_fd = os.open('/proc/self/fd/%d' % self.job_server_fd, os.O_RDWR | os.O_NONBLOCK)
            self.num_jobs = 1
            try:
                for i in range(7):
...
RTL_HLS_INFO_REGEX = '\\(\\* CORE_GENERATION_INFO\\s*=\\s*".*,\\{(.*)\\}" \\*\\)'
