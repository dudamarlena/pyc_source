...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/haoda/ir/arithmetic/base.py
# Compiled at: 2020-05-07 15:49:31
# Size of source mod 2**32: 8299 bytes
import collections, functools, logging
from collections import OrderedDict
from typing import Callable, Dict, Iterable, List, Mapping, Tuple, TypeVar, overload
...
@overload
def simplify(expr: NodeT, logger: Callable[(Ellipsis, None)]=None) -> NodeT:
    pass


@overload
def simplify(expr: Iterable[NodeT], logger: Callable[(Ellipsis, None)]=None) -> Iterable[NodeT]:
    pass
    ...
    def visitor(node: ir.Node, args=None) -> ir.Node:
        if isinstance(node, ir.BinaryOp):
            if len(node.operand) == 1:
                ...
                new_operator = []
                new_operand = []
                for child_operator, child_operand in zip((*(None, ), *node.operator), node.operand):
                    if child_operator is not None:
                        new_operator.append(child_operator)
                    if child_operator in (*(None, '||', '&&'), *'|&+*') and type(child_operand) is type(node):
                ...
                if isinstance(node, ir.Unary):
                    ...
                    if not_count % 2 == 0:
                        if not_count == len(node.operator):
                            return flatten(node.operand)
        else:
                ...
                if operator in ir.REDUCTION_FUNCS:
                    ...
                    for operand in getattr(node, 'arg'):
                        if isinstance(operand, ir.Call):
                            if getattr(operand, 'name') == operator:
  ...
  """
    ...
    """
        if isinstance(node, ir.AddSub):
            ...
            new_operators = []
            new_operands = []
            for operator, operand in zip(('+', ) + getattr(node, 'operator'), getattr(node, 'operand')):
                if operator == '+' and isinstance(operand, ir.MulDiv) and getattr(operand, 'operator') == ('*', ):
                    if left_distribute:
            ...
            if len(new_operands) > 1:
                ...
                new_node = ir.AddSub(operator=(tuple(new_operators[1:])), operand=(tuple(new_operands)))
                if new_node != node:
                    return new_node
            elif new_operands:
                if new_operands[0] != node:
...
def print_tree(node: NodeT, printer: Callable[([str], None)]=_logger.debug) -> NodeT:
  ...
  """
    ...
    if not isinstance(node, ir.Node):
        ...
        printer('root')
        return node.visit(visitor, args=[
         1],
