...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/backends/hg/repository.py
# Compiled at: 2015-06-08 07:05:19
"""
    managevcs.backends.hg.repository
    ~~~~~~~~~~~~~~~~~~~~~~~~~~
...
"""
import os, time, urllib, urllib2, datetime
from managevcs.backends.base import BaseRepository, CollectionGenerator
from managevcs.exceptions import BranchDoesNotExistError, ChangesetDoesNotExistError, EmptyRepositoryError, RepositoryError, VCSError, TagAlreadyExistError, TagDoesNotExistError
from managevcs.utils import author_email, author_name, date_fromtimestamp, makedate, safe_unicode
from managevcs.utils.lazy import LazyProperty
from managevcs.utils.ordered_dict import OrderedDict
from managevcs.utils.paths import abspath
from managevcs.utils.hgcompat import ui, nullid, match, patch, diffopts, clone, get_contact, pull, localrepository, RepoLookupError, Abort, RepoError, hex, scmutil, hg_url, httpbasicauthhandler, httpdigestauthhandler
...
class MercurialRepository(BaseRepository):
    ...
    def __init__(self, repo_path, create=False, baseui=None, src_url=None, update_after_clone=False, bare=False):
        ...
        if not isinstance(repo_path, str):
            raise VCSError('Mercurial backend requires repository path to be instance of <str> got %s instead' % type(repo_path))
    ...
    def get_diff(self, rev1, rev2, path='', ignore_whitespace=False, context=3):
        """
        ...
        if path:
            ...
            file_filter = None
        return ('').join(patch.diff(self._repo, rev1, rev2, match=file_filter, opts=diffopts(git=True, ignorews=ignore_whitespace, context=context)))
    ...
    def contact(self):
        ...
        return safe_unicode(get_contact(self._repo.ui.config) or undefined_contact)
    ...
    def _get_revision(self, revision):
        ...
        try:
            ...
            raise ChangesetDoesNotExistError('Revision %s does not exist for this repository' % revision)
    ...
    def _get_archives(self, archive_name='tip'):
        ...
        for i in [('zip', '.zip'), ('gz', '.tar.gz'), ('bz2', '.tar.bz2')]:
            if i[0] in allowed or self._repo.ui.configbool('web', 'allow' + i[0], untrusted=True):
    ...
    def get_changesets(self, start=None, end=None, start_date=None, end_date=None, branch_name=None, reverse=False):
        ...
        if None not in [start, end] and start_pos > end_pos:
            raise RepositoryError("Start revision '%s' cannot be after end revision '%s'" % (
             start, end))
        if branch_name and branch_name not in self.allbranches.keys():
            raise BranchDoesNotExistError('Branch %s not found in this repository' % branch_name)
