...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_git.py
# Compiled at: 2015-06-08 06:35:03
from __future__ import with_statement
import os, mock, datetime
from managevcs.backends.git import GitRepository, GitChangeset
from managevcs.exceptions import RepositoryError, VCSError, NodeDoesNotExistError
from managevcs.nodes import NodeKind, FileNode, DirNode, NodeState
from managevcs.utils.compat import unittest
from managevcs.tests.base import BackendTestMixin
from managevcs.tests.conf import TEST_GIT_REPO, TEST_GIT_REPO_CLONE, get_new_dir

class GitRepositoryTest(unittest.TestCase):

    def __check_for_existing_repo(self):
        if os.path.exists(TEST_GIT_REPO_CLONE):
            self.fail('Cannot test git clone repo as location %s already exists. You should manually remove it first.' % TEST_GIT_REPO_CLONE)
    ...
    def test_repo_clone(self):
        ...
        repo_clone = GitRepository(TEST_GIT_REPO_CLONE, src_url=TEST_GIT_REPO, create=True, update_after_clone=True)
    ...
    def test_repo_clone_without_create(self):
        self.assertRaises(RepositoryError, GitRepository, TEST_GIT_REPO_CLONE + '_wo_create', src_url=TEST_GIT_REPO)

    def test_repo_clone_with_update(self):
        repo = GitRepository(TEST_GIT_REPO)
        clone_path = TEST_GIT_REPO_CLONE + '_with_update'
        repo_clone = GitRepository(clone_path, create=True, src_url=TEST_GIT_REPO, update_after_clone=True)
        self.assertEqual(len(repo.revisions), len(repo_clone.revisions))
        fpath = os.path.join(clone_path, 'MANIFEST.in')
        self.assertEqual(True, os.path.isfile(fpath), 'Repo was cloned and updated but file %s could not be found' % fpath)

    def test_repo_clone_without_update(self):
        repo = GitRepository(TEST_GIT_REPO)
        clone_path = TEST_GIT_REPO_CLONE + '_without_update'
        repo_clone = GitRepository(clone_path, create=True, src_url=TEST_GIT_REPO, update_after_clone=False)
        self.assertEqual(len(repo.revisions), len(repo_clone.revisions))
        fpath = os.path.join(clone_path, 'MANIFEST.in')
        self.assertFalse(repo_clone._repo.bare)
        self.assertEqual(False, os.path.isfile(fpath), 'Repo was cloned and updated but file %s was found' % fpath)

    def test_repo_clone_into_bare_repo(self):
        repo = GitRepository(TEST_GIT_REPO)
        clone_path = TEST_GIT_REPO_CLONE + '_bare.git'
        repo_clone = GitRepository(clone_path, create=True, src_url=repo.path, bare=True)
    ...
    def test_branches(self):
        for name, id in self.repo.branches.items():
            self.assertTrue(isinstance(self.repo.get_changeset(id), GitChangeset))
    ...
    def test_initial_changeset(self):
        ...
        self.assertEqual(init_chset.author, 'Marcin Kuzminski <marcin@python-blog.com>')
    ...
    def test_not_existing_changeset(self):
        self.assertRaises(RepositoryError, self.repo.get_changeset, 'f' * 40)
...
class GitChangesetTest(unittest.TestCase):
    ...
    def test_nodes_with_changeset(self):
        ...
        self.assertTrue(index is chset.get_node('docs').get_node('api').get_node('index.rst'))
    ...
    def _test_slices(self, limit, offset):
        ...
        for changeset in changesets:
            ...
            if idx > limit:
                self.fail('Exceeded limit already (getting revision %s, there are %s total revisions, offset=%s, limit=%s)' % (
        ...
        self.failUnlessEqual(result, sliced, msg='Comparison failed for limit=%s, offset=%s(get_changeset returned: %s and sliced: %s' % (
    ...
    def test_file_history(self):
        ...
        for path, revs in files.items():
            ...
            self.assertTrue(set(revs).issubset(set(node_revs)), 'We assumed that %s is subset of revisions for which file %s has been changed, and history of that node returned: %s' % (
             revs, path, node_revs))

    def test_file_annotate(self):
        files = {'vcs/backends/__init__.py': {'c1214f7e79e02fc37156ff215cd71275450cffc3': {'lines_no': 1, 
                                                                                     'changesets': [
                                                                                                  'c1214f7e79e02fc37156ff215cd71275450cffc3']}, 
                                        '16fba1ae9334d79b66d7afed2c2dfbfa2ae53647': {'lines_no': 21, 
                                                                                     'changesets': [
                                                                                                  '49d3fd156b6f7db46313fac355dca1a0b94a0017',
                                                                                                  ...
                                                                                                  '16fba1ae9334d79b66d7afed2c2dfbfa2ae53647',
                                                                                                  '16fba1ae9334d79b66d7afed2c2dfbfa2ae53647']}, 
                                        'e29b67bd158580fc90fc5e9111240b90e6e86064': {'lines_no': 32, 
                                                                                     'changesets': [
                                                                                                  '16fba1ae9334d79b66d7afed2c2dfbfa2ae53647',
                                                                                                  '16fba1ae9334d79b66d7afed2c2dfbfa2ae53647',
                                                                                                  '5eab1222a7cd4bfcbabc218ca6d04276d4e27378',
        ...
        for fname, revision_dict in files.items():
            for rev, data in revision_dict.items():
                ...
                self.assertTrue(l1 == l2, 'The lists of revision for %s@rev %sfrom annotation list should match each other, got \n%s \nvs \n%s ' % (
                 fname, rev, l1, l2))
    ...
    def test_files_state(self):
        ...
        """
        node = self.repo.get_changeset('e6ea6d16e2f26250124a1f4b4fe37a912f9d86a0').get_node('vcs/utils/diffs.py')
        ...
        node = self.repo.get_changeset('33fa3223355104431402a888fa77a4e9956feb3e').get_node('.hgignore')
        self.assertTrue(node.state, NodeState.CHANGED)
        self.assertFalse(node.added)
        self.assertTrue(node.changed)
        self.assertFalse(node.not_changed)
        self.assertFalse(node.removed)
        node = self.repo.get_changeset('e29b67bd158580fc90fc5e9111240b90e6e86064').get_node('setup.py')
        self.assertTrue(node.state, NodeState.NOT_CHANGED)
        self.assertFalse(node.added)
        self.assertFalse(node.changed)
        self.assertTrue(node.not_changed)
        self.assertFalse(node.removed)
        chset = self.repo.get_changeset('fa6600f6848800641328adbf7811fd2372c02ab2')
        ...
        chset = self.repo.get_changeset('54386793436c938cff89326944d4c2702340037d')
    ...
    def test_author_email(self):
        self.assertEqual('marcin@python-blog.com', self.repo.get_changeset('c1214f7e79e02fc37156ff215cd71275450cffc3').author_email)
        self.assertEqual('lukasz.balcerzak@python-center.pl', self.repo.get_changeset('ff7ca51e58c505fec0dd2491de52c622bb7a806b').author_email)
        self.assertEqual('none@none', self.repo.get_changeset('8430a588b43b5d6da365400117c89400326e7992').author_email)

    def test_author_username(self):
        self.assertEqual('Marcin Kuzminski', self.repo.get_changeset('c1214f7e79e02fc37156ff215cd71275450cffc3').author_name)
        self.assertEqual('Lukasz Balcerzak', self.repo.get_changeset('ff7ca51e58c505fec0dd2491de52c622bb7a806b').author_name)
        self.assertEqual('marcink', self.repo.get_changeset('8430a588b43b5d6da365400117c89400326e7992').author_name)
...
class GitSpecificWithRepoTest(BackendTestMixin, unittest.TestCase):
        ...
        return [
         {'message': 'Initial', 
            ...
            'added': [
                    ...
                    FileNode('foobar/static/admin', content='admin', mode=40960),
    ...
    def test_paths_slow_traversing(self):
        ...
        self.assertEqual(cs.get_node('foobar').get_node('static').get_node('js').get_node('admin').get_node('base.js').content, 'base')

    def test_paths_fast_traversing(self):
        cs = self.repo.get_changeset()
        self.assertEqual(cs.get_node('foobar/static/js/admin/base.js').content, 'base')
    ...
    def test_get_diff_runs_git_command_with_hashes(self):
        ...
        self.repo.run_git_command.assert_called_once_with('diff -U%s --full-index --binary -p -M --abbrev=40 %s %s' % (
    ...
    def test_get_diff_runs_git_command_with_str_hashes(self):
        ...
        self.repo.run_git_command.assert_called_once_with('show -U%s --full-index --binary -p -M --abbrev=40 %s' % (
         3, self.repo._get_revision(1)))

    def test_get_diff_runs_git_command_with_path_if_its_given(self):
        self.repo.run_git_command = mock.Mock(return_value=['', ''])
        self.repo.get_diff(0, 1, 'foo')
        self.repo.run_git_command.assert_called_once_with('diff -U%s --full-index --binary -p -M --abbrev=40 %s %s -- "foo"' % (
         3, self.repo._get_revision(0), self.repo._get_revision(1)))
