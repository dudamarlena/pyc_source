...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/utils/helpers.py
...
from managevcs.exceptions import RepositoryError
...
def get_scms_for_path(path):
    ...
    """
    from managevcs.backends import get_backend
...
def parse_changesets(text):
    ...
    CID_RE = '[a-zA-Z0-9]+'
    if '..' not in text:
        m = re.match('^(?P<cid>%s)$' % CID_RE, text)
        if m:
            return {'start': None, 
               'main': text, 
               'end': None}
    else:
        RE = '^(?P<start>%s)?\\.{2,3}(?P<end>%s)?$' % (CID_RE, CID_RE)
        m = re.match(RE, text)
        if m:
            result = m.groupdict()
            result['main'] = None
...
def parse_datetime(text):
    ...
    INPUT_FORMATS = ('%Y-%m-%d %H:%M:%S', '%Y-%m-%d %H:%M', '%Y-%m-%d', '%m/%d/%Y %H:%M:%S',
    ...
    days = 0
    matched = re.match('^((?P<weeks>\\d+) ?w(eeks?)?)? ?((?P<days>\\d+) ?d(ays?)?)?$', text)
    if matched:
...
def get_total_seconds(timedelta):
    ...
    """
    return (timedelta.microseconds + (timedelta.seconds + timedelta.days * 24 * 60 * 60) * 1000000) / 1000000