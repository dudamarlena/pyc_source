...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_inmemchangesets.py
# Compiled at: 2015-06-08 06:25:02
"""
Tests so called "in memory changesets" commit API of vcs.
"""
from __future__ import with_statement
import time, datetime, managevcs
from managevcs.tests.conf import SCM_TESTS, get_new_dir
from managevcs.exceptions import EmptyRepositoryError
from managevcs.exceptions import NodeAlreadyAddedError
from managevcs.exceptions import NodeAlreadyExistsError
from managevcs.exceptions import NodeAlreadyRemovedError
from managevcs.exceptions import NodeAlreadyChangedError
from managevcs.exceptions import NodeDoesNotExistError
from managevcs.exceptions import NodeNotChangedError
from managevcs.nodes import DirNode
...
class InMemoryChangesetTestMixin(object):
    ...
    - ``backend_alias``: alias of used backend (see ``managevcs.BACKENDS``)
    - ``repo_path``: path to the repository which would be created for set of
      tests
    """

    def get_backend(self):
        return managevcs.get_backend(self.backend_alias)
    ...
    def setUp(self):
        ...
        self.nodes = [
         ...
         FileNode('foo/bar/file.bin', content=b'\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00;\x00\x03\x00\xfe\xff\t')]

    def test_add(self):
        rev_count = len(self.repo.revisions)
        to_add = [ FileNode(node.path, content=node.content) for node in self.nodes
                 ]
        for node in to_add:
            self.imc.add(node)
    ...
    def test_add_actually_adds_all_nodes_at_second_commit_too(self):
        self.imc.add(FileNode('foo/bar/image.png', content='\x00'))
        ...
        self.assertEqual(changeset.get_node('foo/bar/image.png').content, '\x00')
    ...
    def test_check_integrity_raise_already_exist(self):
        node = FileNode('foobar', content='baz')
        self.imc.add(node)
        self.imc.commit(message='Added foobar', author=unicode(self))
        self.imc.add(node)
        self.assertRaises(NodeAlreadyExistsError, self.imc.commit, message='new message', author=str(self))
    ...
    def test_change(self):
        ...
        self.assertEqual(newtip.get_node('foo/bar/baz').content, 'My **changed** content')
    ...
    def test_check_integrity_change_raise_node_does_not_exist(self):
        ...
        self.imc.change(node)
        self.assertRaises(NodeDoesNotExistError, self.imc.commit, message='Changed not existing node', author=str(self))
    ...
    def test_check_integrity_change_raise_node_not_changed(self):
        ...
        self.imc.change(node)
        self.assertRaises(NodeNotChangedError, self.imc.commit, message='Trying to mark node as changed without touching it', author=unicode(self))
    ...
    def test_remove(self):
        ...
        self.imc.commit(message='Removed %s' % node.path, author=unicode(self))
        newtip = self.repo.get_changeset()
        self.assertNotEqual(tip, newtip)
        self.assertNotEqual(tip.id, newtip.id)
    ...
    def test_remove_raise_node_does_not_exist(self):
        ...
        self.assertRaises(NodeDoesNotExistError, self.imc.commit, message='Trying to remove node at empty repository', author=str(self))

    def test_check_integrity_remove_raise_node_does_not_exist(self):
        self.test_add()
        node = FileNode('no-such-file')
        self.imc.remove(node)
        self.assertRaises(NodeDoesNotExistError, self.imc.commit, message='Trying to remove not existing node', author=unicode(self))
    ...
    def test_multiple_commits(self):
        ...
        for x in xrange(N):
            ...
            self.imc.add(node)
            commit = self.imc.commit('Commit no. %s' % (x + 1), author='managevcs')
    ...
    def test_date_attr(self):
        ...
        commit = self.imc.commit('Committed at time when I was born ;-)', author='lb', date=date)
...
class BackendBaseTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for commit in self.get_commits():
            ...
            self.imc.commit(message=unicode(commit['message']), author=unicode(commit['author']), date=commit['date'])
