...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_nodes.py
# Compiled at: 2015-06-08 06:25:02
from __future__ import with_statement
import stat
from managevcs.nodes import DirNode
from managevcs.nodes import FileNode
from managevcs.nodes import Node
from managevcs.nodes import NodeError
from managevcs.nodes import NodeKind
...
class NodeBasicTest(unittest.TestCase):
    ...
    def _test_parent_path(self, node_path, expected_parent_path):
        ...
        self.assertTrue(parent_path.endswith('/') or node.is_root() and parent_path == '')
        self.assertEqual(parent_path, expected_parent_path, "Node's path is %r and parent path is %r but should be %r" % (
    ...
    def test_file_node_is_executable(self):
        node = FileNode('foobar', 'empty... almost', mode=33261)
        self.assertTrue(node.is_executable())
        node = FileNode('foobar', 'empty... almost', mode=33088)
        self.assertTrue(node.is_executable())
        node = FileNode('foobar', 'empty... almost', mode=33188)
