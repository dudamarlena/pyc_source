...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/backends/hg/changeset.py
# Compiled at: 2015-06-08 06:25:00
import os, posixpath
from managevcs.conf import settings
from managevcs.backends.base import BaseChangeset
from managevcs.exceptions import ChangesetDoesNotExistError, ChangesetError, ImproperArchiveTypeError, NodeDoesNotExistError, VCSError
from managevcs.nodes import AddedFileNodesGenerator, ChangedFileNodesGenerator, DirNode, FileNode, NodeKind, RemovedFileNodesGenerator, RootNode, SubModuleNode
from managevcs.utils import safe_str, safe_unicode, date_fromtimestamp
from managevcs.utils.lazy import LazyProperty
from managevcs.utils.paths import get_dirs_for_path
from managevcs.utils.hgcompat import archival, hex
...
class MercurialChangeset(BaseChangeset):
    ...
    def parents(self):
        ...
        """
        return [ self.repository.get_changeset(parent.rev()) for parent in self._ctx.parents() if parent.rev() >= 0
               ]
    ...
    def next(self, branch=None):
        if branch and self.branch != branch:
            raise VCSError('Branch option used on changeset not belonging to that branch')
    ...
    def diff(self, ignore_whitespace=True, context=3):
        return ('').join(self._ctx.diff(git=True, ignore_whitespace=ignore_whitespace, context=context))
    ...
    def _get_kind(self, path):
        ...
        if path in self._dir_paths:
            return NodeKind.DIR
        raise ChangesetError("Node does not exist at the given path '%s'" % path)

    def _get_filectx(self, path):
        path = self._fix_path(path)
        if self._get_kind(path) != NodeKind.FILE:
            raise ChangesetError("File does not exist for revision %s at  '%s'" % (
             self.raw_id, path))
    ...
    def fill_archive(self, stream=None, kind='tgz', prefix=None, subrepos=False):
        """
        ...
        if kind not in allowed_kinds:
            raise ImproperArchiveTypeError('Archive kind not supported use oneof %s', allowed_kinds)
        if stream is None:
        ...
        archival.archive(self.repository._repo, stream, self.raw_id, kind, prefix=prefix, subrepos=subrepos)
    ...
    def get_nodes(self, path):
        ...
        if self._get_kind(path) != NodeKind.DIR:
            raise ChangesetError("Directory does not exist for revision %s at  '%s'" % (
             self.revision, path))
        path = self._fix_path(path)
        filenodes = [ FileNode(f, changeset=self) for f in self._file_paths if os.path.dirname(f) == path
                    ]
        dirs = path == '' and '' or [ d for d in self._dir_paths if d and posixpath.dirname(d) == path
                                    ]
        dirnodes = [ DirNode(d, changeset=self) for d in dirs if os.path.dirname(d) == path
                   ]
        als = self.repository.alias
        for k, vals in self._extract_submodules().iteritems():
            loc = vals[0]
            cs = vals[1]
            dirnodes.append(SubModuleNode(k, url=loc, changeset=cs, alias=als))
    ...
    def get_node(self, path):
        ...
        if path not in self.nodes:
            if path in self._file_paths:
                ...
                raise NodeDoesNotExistError("There is no file nor directory at the given path: '%s' at revision %s" % (
                 path, self.short_id))
