...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/subprocessio.py
...
from managevcs.utils.compat import deque, Event, Thread, _bytes, _bytearray
...
class StreamFeeder(Thread):
    ...
    def __init__(self, source):
        ...
        filelike = False
        self.bytes = _bytes()
        if type(source) in (type(''), _bytes, _bytearray):
            self.bytes = _bytes(source)
        else:
            if type(source) in (int, long):
                try:
...
class SubprocessIOChunker(object):
    ...
    def __init__(self, cmd, inputstream=None, buffer_size=65536, chunk_size=4096, starting_values=[], **kwargs):
        """
        ...
        kwargs['shell'] = _shell
        _p = subprocess.Popen(cmd, bufsize=(-1), stdin=inputstream, stdout=subprocess.PIPE, stderr=subprocess.PIPE, **kwargs)
