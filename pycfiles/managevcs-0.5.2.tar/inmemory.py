...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/backends/hg/inmemory.py
# Compiled at: 2015-06-08 06:25:01
import datetime, errno
from managevcs.backends.base import BaseInMemoryChangeset
from managevcs.exceptions import RepositoryError
from managevcs.utils.hgcompat import memfilectx, memctx, hex, tolocal
    ...
    def commit(self, message, author, parents=None, branch=None, date=None, **kwargs):
        ...
        def filectxfn(_repo, memctx, path):
            ...
            for node in self.added:
                if node.path == path:
                    return memfilectx(path=node.path, data=(node.is_binary or node.content.encode)('utf8') if 1 else node.content, islink=False, isexec=node.is_executable, copied=False, repo=_repo)
