...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/utils/diffs.py
# Compiled at: 2015-06-08 06:25:01
import re, difflib, logging, itertools
from difflib import unified_diff
from managevcs.exceptions import VCSError
from managevcs.nodes import FileNode, NodeError
from managevcs.utils import safe_unicode
...
def get_udiff(filenode_old, filenode_new, show_whitespace=True):
    ...
    for filenode in (filenode_old, filenode_new):
        if not isinstance(filenode, FileNode):
            raise VCSError('Given object should be FileNode object, not %s' % filenode.__class__)

    if filenode_old_date and filenode_new_date:
        if not filenode_old_date < filenode_new_date:
            logging.debug('Generating udiff for filenodes with not increasing dates')
    vcs_udiff = unified_diff(filenode_old.content.splitlines(True), filenode_new.content.splitlines(True), filenode_old.name, filenode_new.name, filenode_old_date, filenode_old_date)
    return vcs_udiff
...
def get_gitdiff(filenode_old, filenode_new, ignore_whitespace=True):
    ...
    vcs_gitdiff = repo.get_diff(old_raw_id, new_raw_id, filenode_new.path, ignore_whitespace)
    return vcs_gitdiff
...
class DiffProcessor(object):
    ...
    def __init__(self, diff, differ='diff', format='udiff'):
        ...
        if isinstance(self.__udiff, basestring):
            ...
            self.lines = itertools.imap(self.escaper, self._parse_gitdiff(udiff_copy))
        else:
            udiff_copy = self.copy_iterator()
    ...
    def _extract_rev(self, line1, line2):
        ...
        try:
            if line1.startswith('--- ') and line2.startswith('+++ '):
                ...
                filename = old_filename if old_filename != 'dev/null' else new_filename
                return (
                 filename, new_rev, old_rev)
    ...
    def _highlight_line_difflib(self, line, next):
        ...
        if line['action'] == 'del':
            ...
            old, new = next, line
        oldwords = re.split('(\\W)', old['line'])
        newwords = re.split('(\\W)', new['line'])
            ...
            while 1:
                ...
                while line:
                    ...
                    while old_line < old_end or new_line < new_end:
                        ...
                        lines.append({'old_lineno': affects_old and old_line or '', 
    ...
    def _safe_id(self, idstring):
        ...
        """
        idstring = re.sub('\\s', '_', '%s' % idstring)
        idstring = re.sub('(?!-)\\W', '', idstring).lower()
        return idstring
    ...
    def as_html(self, table_class='code-difftable', line_class='line', new_lineno_class='lineno old', old_lineno_class='lineno new', code_class='code'):
        """
        ...
        def _link_to_if(condition, label, url):
            ...
            """
            if condition:
                return '<a href="%(url)s">%(label)s</a>' % {'url': url, 'label': label}
            else:
                return label
        ...
        for diff in diff_lines:
            for line in diff['chunks']:
                ...
                for change in line:
                    ...
                    anchor_old_id = ''
                    anchor_new_id = ''
                    anchor_old = '%(filename)s_o%(oldline_no)s' % {'filename': self._safe_id(diff['filename']), 'oldline_no': change['old_lineno']}
                    anchor_new = '%(filename)s_n%(oldline_no)s' % {'filename': self._safe_id(diff['filename']), 'oldline_no': change['new_lineno']}
                    cond_old = change['old_lineno'] != '...' and change['old_lineno']
                    cond_new = change['new_lineno'] != '...' and change['new_lineno']
                    if cond_old:
                        anchor_old_id = 'id="%s"' % anchor_old
                    if cond_new:
                        anchor_new_id = 'id="%s"' % anchor_new
                    _html.append('\t<td %(a_id)s class="%(old_lineno_cls)s">' % {'a_id': anchor_old_id, 'old_lineno_cls': old_lineno_class})
                    _html.append('<pre>%(link)s</pre>' % {'link': _link_to_if(cond_old, change['old_lineno'], '#%s' % anchor_old)})
                    _html.append('</td>\n')
                    _html.append('\t<td %(a_id)s class="%(new_lineno_cls)s">' % {'a_id': anchor_new_id, 'new_lineno_cls': new_lineno_class})
                    _html.append('<pre>%(link)s</pre>' % {'link': _link_to_if(cond_new, change['new_lineno'], '#%s' % anchor_new)})
                    _html.append('</td>\n')
                    _html.append('\t<td class="%(code_class)s">' % {'code_class': code_class})
                    _html.append('\n\t\t<pre>%(code)s</pre>\n' % {'code': change['line']})
                    _html.append('\t</td>')
                    _html.append('\n</tr>\n')
