...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_utils_progressbar.py
# Compiled at: 2015-06-08 06:25:01
from __future__ import with_statement
import sys, datetime
from StringIO import StringIO
from managevcs.utils.helpers import get_total_seconds
from managevcs.utils.progressbar import AlreadyFinishedError
from managevcs.utils.progressbar import ProgressBar
...
class TestProgressBar(unittest.TestCase):
    ...
    def test_get_eta(self):
        ...
        self.assertEquals(get_total_seconds(bar.get_eta(now)), int(get_total_seconds(delta) * 0.5))
        bar.render(75)
        now = datetime.datetime.now()
        delta = now - bar.started
        self.assertEquals(get_total_seconds(bar.get_eta(now)), int(get_total_seconds(delta) * 0.25))
    ...
    def test_get_rendered_transfer(self):
        ...
        GB = MB * 1024
        bar = ProgressBar()
        self.assertEqual(bar.get_rendered_transfer(12, 100), 'Transfer: 12 B / 100 B')
        self.assertEqual(bar.get_rendered_transfer(KB * 5, MB), 'Transfer: 5 KB / 1.0 MB')
        self.assertEqual(bar.get_rendered_transfer(GB * 2.3, GB * 10), 'Transfer: 2.30 GB / 10.00 GB')
