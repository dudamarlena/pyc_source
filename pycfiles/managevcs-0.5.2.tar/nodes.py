...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/nodes.py
# Compiled at: 2015-06-08 06:25:02
import stat, posixpath, mimetypes
from pygments import lexers
from managevcs.backends.base import EmptyChangeset
from managevcs.exceptions import NodeError, RemovedFileNodeError
...
class Node(object):
    ...
    def __init__(self, path, kind):
        if path.startswith('/'):
            raise NodeError('Cannot initialize Node objects with slash at the beginning as only relative paths are supported')
...
class FileNode(Node):
    ...
    def content(self):
        ...
        if bool(content and '\x00' in content):
            return content
    ...
    def size(self):
        ...
        raise NodeError('Cannot retrieve size of the file without related changeset attribute')

    @LazyProperty
    def message(self):
        if self.changeset:
            return self.last_changeset.message
        raise NodeError('Cannot retrieve message of the file without related changeset attribute')

    @LazyProperty
    def last_changeset(self):
        if self.changeset:
            return self.changeset.get_file_changeset(self.path)
        raise NodeError('Cannot retrieve last changeset of the file without related changeset attribute')
    ...
    def state(self):
        if not self.changeset:
            raise NodeError("Cannot check state of the node if it's not linked with changeset")
        else:
            if self.path in (node.path for node in self.changeset.added):
                return NodeState.ADDED
            else:
                if self.path in (node.path for node in self.changeset.changed):
...
class RemovedFileNode(FileNode):
    ...
    def __getattribute__(self, attr):
        ...
        raise RemovedFileNodeError('Cannot access attribute %s on RemovedFileNode' % attr)
...
class DirNode(Node):
    ...
    @LazyProperty
    def content(self):
        raise NodeError('%s represents a dir and has no ``content`` attribute' % self)
...
class RootNode(DirNode):
    ...
    def __init__(self, nodes=(), changeset=None):
        super(RootNode, self).__init__(path='', nodes=nodes, changeset=changeset)
