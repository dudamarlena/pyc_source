...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_utils.py
# Compiled at: 2015-06-08 06:25:01
from __future__ import with_statement
import os, mock, time, shutil, tempfile, datetime
from managevcs.utils.compat import unittest
from managevcs.utils.paths import get_dirs_for_path
from managevcs.utils.helpers import get_dict_for_attrs
from managevcs.utils.helpers import get_scm
from managevcs.utils.helpers import get_scms_for_path
from managevcs.utils.helpers import get_total_seconds
from managevcs.utils.helpers import parse_changesets
from managevcs.utils.helpers import parse_datetime
from managevcs.utils import author_email, author_name
from managevcs.utils.paths import get_user_home
from managevcs.exceptions import VCSError
from managevcs.tests.conf import TEST_HG_REPO, TEST_GIT_REPO, TEST_TMP_PATH
...
class PathsTest(unittest.TestCase):
    ...
    def _test_get_dirs_for_path(self, path, expected):
        ...
        self.assertEqual(result, expected, msg='%s != %s which was expected result for path %s' % (
    ...
    def test_get_scms_for_path(self):
        ...
        new = os.path.join(dirpath, 'managevcs-scms-for-path-%s' % time.time())
        os.mkdir(new)
...
class TestParseChangesets(unittest.TestCase):
    ...
    def test_main_is_returned_correctly(self):
        self.assertEqual(parse_changesets('123456'), {'start': None, 
           'main': '123456', 
           'end': None})
        return

    def test_start_is_returned_correctly(self):
        self.assertEqual(parse_changesets('aaabbb..'), {'start': 'aaabbb', 
           'main': None, 
           'end': None})
        return

    def test_end_is_returned_correctly(self):
        self.assertEqual(parse_changesets('..cccddd'), {'start': None, 
           'main': None, 
           'end': 'cccddd'})
        return
...
class TestParseDatetime(unittest.TestCase):
    ...
    def test_datetime_text(self):
        self.assertEqual(parse_datetime('2010-04-07 21:29:41'), datetime.datetime(2010, 4, 7, 21, 29, 41))

    def test_no_seconds(self):
        self.assertEqual(parse_datetime('2010-04-07 21:29'), datetime.datetime(2010, 4, 7, 21, 29))

    def test_date_only(self):
        self.assertEqual(parse_datetime('2010-04-07'), datetime.datetime(2010, 4, 7))

    def test_another_format(self):
        self.assertEqual(parse_datetime('04/07/10 21:29:41'), datetime.datetime(2010, 4, 7, 21, 29, 41))

    def test_now(self):
        self.assertTrue(parse_datetime('now') - datetime.datetime.now() < datetime.timedelta(seconds=1))

    def test_today(self):
        today = datetime.date.today()
        self.assertEqual(parse_datetime('today'), datetime.datetime(*today.timetuple()[:3]))

    def test_yesterday(self):
        yesterday = datetime.date.today() - datetime.timedelta(days=1)
        self.assertEqual(parse_datetime('yesterday'), datetime.datetime(*yesterday.timetuple()[:3]))
    ...
    def test_weeks(self):
        timestamp = datetime.datetime.today() - datetime.timedelta(days=21)
        args = timestamp.timetuple()[:3] + (0, 0, 0, 0)
        expected = datetime.datetime(*args)
    ...
    def test_mixed(self):
        timestamp = datetime.datetime.today() - datetime.timedelta(days=17)
        args = timestamp.timetuple()[:3] + (0, 0, 0, 0)
        expected = datetime.datetime(*args)
...
class TestGetDictForAttrs(unittest.TestCase):
    ...
    def test_returned_dict_has_expected_attrs(self):
        ...
        self.assertEqual(get_dict_for_attrs(obj, ['CONST', 'foo', 'attrs',
         'date', 'count']), {'CONST': True, 
           'foo': 'aaa', 
           'attrs': {'foo': 'bar'}, 'date': datetime.datetime(2010, 12, 31), 
           'count': 1001})
...
class TestGetTotalSeconds(unittest.TestCase):
    ...
    def assertTotalSecondsEqual(self, timedelta, expected_seconds):
        ...
        self.assertEqual(result, expected_seconds, 'We computed %s seconds for %s but expected %s' % (
...
class TestGetUserHome(unittest.TestCase):
    ...
    @mock.patch.object(os, 'environ', {'HOME': '/home/foobar', 'USERPROFILE': '/Users/foobar'})
