...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_hg.py
# Compiled at: 2015-06-08 06:33:25
from __future__ import with_statement
import os
from managevcs.backends.hg import MercurialRepository, MercurialChangeset
from managevcs.exceptions import RepositoryError, VCSError, NodeDoesNotExistError
from managevcs.nodes import NodeKind, NodeState
from managevcs.tests.conf import TEST_HG_REPO, TEST_HG_REPO_CLONE, TEST_HG_REPO_PULL
...
class MercurialRepositoryTest(unittest.TestCase):
    ...
    def __check_for_existing_repo(self):
        if os.path.exists(TEST_HG_REPO_CLONE):
            self.fail('Cannot test mercurial clone repo as location %s already exists. You should manually remove it first.' % TEST_HG_REPO_CLONE)
    ...
    def test_unicode_path_repo(self):
        self.assertRaises(VCSError, lambda : MercurialRepository('iShouldFail'))

    def test_repo_clone(self):
        self.__check_for_existing_repo()
        repo = MercurialRepository(TEST_HG_REPO)
        repo_clone = MercurialRepository(TEST_HG_REPO_CLONE, src_url=TEST_HG_REPO, update_after_clone=True)
    ...
    def test_repo_clone_with_update(self):
        repo = MercurialRepository(TEST_HG_REPO)
        repo_clone = MercurialRepository(TEST_HG_REPO_CLONE + '_w_update', src_url=TEST_HG_REPO, update_after_clone=True)
        self.assertEqual(len(repo.revisions), len(repo_clone.revisions))
        self.assertEqual(os.path.isfile(os.path.join(TEST_HG_REPO_CLONE + '_w_update', 'MANIFEST.in')), True)

    def test_repo_clone_without_update(self):
        repo = MercurialRepository(TEST_HG_REPO)
        repo_clone = MercurialRepository(TEST_HG_REPO_CLONE + '_wo_update', src_url=TEST_HG_REPO, update_after_clone=False)
        self.assertEqual(len(repo.revisions), len(repo_clone.revisions))
        self.assertEqual(os.path.isfile(os.path.join(TEST_HG_REPO_CLONE + '_wo_update', 'MANIFEST.in')), False)

    def test_pull(self):
        if os.path.exists(TEST_HG_REPO_PULL):
            self.fail('Cannot test mercurial pull command as location %s already exists. You should manually remove it first' % TEST_HG_REPO_PULL)
    ...
    def test_branches(self):
        ...
        for name, id in self.repo.branches.items():
            self.assertTrue(isinstance(self.repo.get_changeset(id), MercurialChangeset))
    ...
    def test_initial_changeset(self):
        ...
        self.assertEqual(sorted(init_chset._file_paths), sorted([
         'vcs/__init__.py',
         'vcs/backends/BaseRepository.py',
         'vcs/backends/__init__.py']))
        self.assertEqual(sorted(init_chset._dir_paths), sorted(['', 'vcs', 'vcs/backends']))
    ...
    def test_not_existing_changeset(self):
        self.assertRaises(RepositoryError, self.repo.get_changeset, 'abcd' * 10)
        self.assertRaises(RepositoryError, self.repo.get_changeset, 'erroerroerroerro')
        self.assertRaises(RepositoryError, self.repo.get_changeset, self.repo.count() + 1)
        revision = pow(2, 30)
...
class MercurialChangesetTest(unittest.TestCase):
    ...
    def test_file_history(self):
        files = {'setup.py': [
                      7, 18, 45, 46, 47, 69, 77], 
           'vcs/nodes.py': [
                          7, 8, 24, 26, 30, 45, 47, 49, 56, 57, 58, 59, 60,
                          61, 73, 76], 
           'vcs/backends/hg.py': [
                                4, 5, 6, 11, 12, 13, 14, 15, 16, 21, 22, 23,
    ...
    def test_file_annotate(self):
        files = {'vcs/backends/__init__.py': {89: {'lines_no': 31, 'changesets': [
                                                          32, 32, 61, 32, 32, 37, 32, 32, 32, 44,
                                                          37, 37, 37, 37, 45, 37, 44, 37, 37, 37,
                                                          32, 32, 32, 32, 37, 32, 37, 37, 32,
                                                          32, 32]}, 
                                        20: {'lines_no': 1, 'changesets': [
                                                          4]}, 
                                        55: {'lines_no': 31, 'changesets': [
                                                          32, 32, 45, 32, 32, 37, 32, 32, 32, 44,
                                                          37, 37, 37, 37, 45, 37, 44, 37, 37, 37,
                                                          32, 32, 32, 32, 37, 32, 37, 37, 32,
                                                          32, 32]}}, 
           'vcs/exceptions.py': {89: {'lines_no': 18, 'changesets': [
                                                   16, 16, 16, 16, 16, 16, 16, 16, 16, 16,
                                                   16, 16, 17, 16, 16, 18, 18, 18]}, 
                                 20: {'lines_no': 18, 'changesets': [
                                                   16, 16, 16, 16, 16, 16, 16, 16, 16, 16,
                                                   16, 16, 17, 16, 16, 18, 18, 18]}, 
                                 55: {'lines_no': 18, 'changesets': [16, 16, 16, 16, 16, 16,
                                                   16, 16, 16, 16, 16, 16,
                                                   17, 16, 16, 18, 18, 18]}}, 
           'MANIFEST.in': {89: {'lines_no': 5, 'changesets': [
                                             7, 7, 7, 71, 71]}, 
                           20: {'lines_no': 3, 'changesets': [
                                             7, 7, 7]}, 
                           55: {'lines_no': 3, 'changesets': [
                                             7, 7, 7]}}}
    ...
    def test_changeset_state(self):
        ...
        self.assertEqual(set(node.path for node in chset88.changed), set(['.hgignore']))
        ...
        self.assertEqual(set(node.path for node in chset85.removed), set(['vcs/utils/web.py']))
    ...
    def test_author_email(self):
        self.assertEqual('marcin@python-blog.com', self.repo.get_changeset('b986218ba1c9').author_email)
        self.assertEqual('lukasz.balcerzak@python-center.pl', self.repo.get_changeset('3803844fdbd3').author_email)
        self.assertEqual('', self.repo.get_changeset('84478366594b').author_email)

    def test_author_username(self):
        self.assertEqual('Marcin Kuzminski', self.repo.get_changeset('b986218ba1c9').author_name)
        self.assertEqual('Lukasz Balcerzak', self.repo.get_changeset('3803844fdbd3').author_name)
        self.assertEqual('marcink', self.repo.get_changeset('84478366594b').author_name)