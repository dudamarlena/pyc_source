...
# [GCC 8.4.0]
# Embedded file name: /Users/dzhiltsov/Development/vcslib/managevcs/tests/test_changesets.py
...
from managevcs.nodes import FileNode, AddedFileNodesGenerator, ChangedFileNodesGenerator, RemovedFileNodesGenerator
from managevcs.exceptions import BranchDoesNotExistError, ChangesetDoesNotExistError, RepositoryError
...
class TestBaseChangeset(unittest.TestCase):
    ...
    def test_as_dict(self):
        ...
        changeset.changed = []
        changeset.removed = []
        self.assertEqual(changeset.as_dict(), {'id': 'ID', 
           'raw_id': 'RAW_ID', 
           'short_id': 'SHORT_ID', 
           'revision': 1009, 
...
class ChangesetsWithCommitsTestCaseixin(BackendTestMixin):
    ...
    def test_get_changesets_respects_branch_name(self):
        tip = self.repo.get_changeset()
        self.imc.add(managevcs.nodes.FileNode('docs/index.txt', content='Documentation\n'))
        doc_changeset = self.imc.commit(message='New branch: docs', author='joe', branch='docs')
        self.imc.add(managevcs.nodes.FileNode('newfile', content=''))
        self.imc.commit(message='Back in default branch', author='joe', parents=[
         tip])
        default_branch_changesets = self.repo.get_changesets(branch_name=self.repo.DEFAULT_BRANCH_NAME)
...
class ChangesetsTestCaseMixin(BackendTestMixin):
    ...
    def test_get_changesets_is_ordered_by_date(self):
        ...
        ordered_by_date = sorted(changesets, key=lambda cs: cs.date)
    ...
    def test_get_changesets_respects_both_start_and_end(self):
        ...
        changesets = list(self.repo.get_changesets(start=second_id, end=third_id))
        self.assertEqual(len(changesets), 2)
    ...
    def test_get_changesets_respects_start_date_and_end_date(self):
        ...
        for cs in self.repo.get_changesets(start_date=start_date, end_date=end_date):
            self.assertGreaterEqual(cs.date, start_date)
            self.assertLessEqual(cs.date, end_date)
    ...
    def test_get_changesets_respects_start_date_and_end_date_and_branch_name(self):
        start_date = datetime.datetime(2010, 1, 1)
        end_date = datetime.datetime(2010, 1, 3)
        for cs in self.repo.get_changesets(start_date=start_date, end_date=end_date, branch_name=self.repo.DEFAULT_BRANCH_NAME):
            self.assertGreaterEqual(cs.date, start_date)
            self.assertLessEqual(cs.date, end_date)

    def test_get_changesets_respects_reverse(self):
        changesets_id_list = [ cs.raw_id for cs in self.repo.get_changesets(reverse=True)
                             ]
    ...
    def test_author(self):
        tip = self.repo.get_changeset()
        self.assertEqual(tip.author, 'Joe Doe <joe.doe@example.com>')

    def test_author_name(self):
        tip = self.repo.get_changeset()
        self.assertEqual(tip.author_name, 'Joe Doe')

    def test_author_email(self):
        tip = self.repo.get_changeset()
        self.assertEqual(tip.author_email, 'joe.doe@example.com')
    ...
    def _get_commits(cls):
        return [
         ...
         {'message': 'Massive changes', 
            ...
            'added': [
                    FileNode('fallout', content='War never changes')], 
            'changed': [
