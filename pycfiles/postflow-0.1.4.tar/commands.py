...
# [GCC 8.4.0]
# Embedded file name: /Users/shawn/Workspace/PostFlow/postflow/config/commands.py
# Compiled at: 2017-05-06 04:04:59
import os, sys, click
...
def prompt_config_path(config_path):
    ...
    while True:
        if os.path.exists(config_path) and click.confirm(click.style(('Config {cfg} exists. Do you want to overwrite it?').format(cfg=config_path), fg='magenta')):
            break
...
def generate_config(output=None, force=None, theme=None):
    config_env = Environment(loader=FileSystemLoader(os.path.join(current_app.root_path, 'config')))
    ...
    default_conf = {'secret_key': gen_salt(24), 
    ...
    if force and os.path.exists(config_path):
        click.secho(('Overwriting existing config file: {}').format(config_path), fg='yellow')
    write_config(default_conf, config_template, config_path)
    return config_path


@config.command('generate')
@click.option('--output', '-o', required=False, help="The path where the config file will be saved at. Defaults to the PostFlow's root folder.")
@click.option('--force', '-f', default=False, is_flag=True, help='Overwrite any existing config file if one exists.')
def generate(output, force):
    """Generates a PostFlow configuration file."""
    config_path = generate_config(output, force)
    click.secho(('The configuration file has been saved to:\n{cfg}').format(cfg=config_path), fg='blue')
    click.secho(('Usage: postflow --config {cfg} run\nFeel free to adjust it as needed.').format(cfg=config_path), fg='green')
    sys.exit(0)