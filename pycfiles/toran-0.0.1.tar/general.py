...
# [GCC 8.4.0]
# Embedded file name: /Users/ricardovegas/Documents/dev/toran/handlers/general.py
# Compiled at: 2016-04-23 07:33:00
...
def access_refused(environ, start_response, type, transport, error):
    ...
    result = util_transport.prepare_error(transport=transport, message=str(error), error='Access refused of type: ' + type + '', code='access_refused', status_code='401')
...
def access_failed(environ, start_response, type, transport, error):
    """Called if not authorized for action"""
    result = util_transport.prepare_error(transport=transport, message=str(error), error='Access unauthorized of type: ' + type + '', code='invalid_token', status_code='401')
    start_response('401 UNAUTHORIZED', [('Content-Type', 'application/json')])
    return [json.dumps(result)]
...
def url_not_found(environ, start_response, transport):
    ...
    result = util_transport.prepare_error(transport=transport, message='Service Not Found', error='', code='endpoint_not_found', status_code='404')
...
def bad_method(environ, start_response, transport):
    """Called if no URL matches."""
    result = util_transport.prepare_error(transport=transport, message='Method not allowed', error='', code='bad_method', status_code='405')
...
def bad_request(environ, start_response, transport, error, params=None):
    ...
    if params is None:
        ...
        join_params = params
        code = 'missing_param'
    result = util_transport.prepare_error(transport=transport, message='Bad request ' + (', ').join(join_params), error=error, code=code, status_code='400')
    start_response('400 BAD REQUEST', [('Content-Type', 'application/json')])
    return [json.dumps(result)]
...
def rpc_not_found(environ, start_response, name, transport, error, logger):
    ...
    logger.log_gateway_error(message='Exchange ' + name + ' Not Found', detail=str(error), request_id='', environ=environ)
    result = util_transport.prepare_error(transport=transport, message='Exchange ' + name + ' Not Found', error=error, code='not_found', status_code='500')
...
def gateway_timeout(environ, start_response, name, transport, error, logger):
    """Called if no rpc queue matches."""
    logger.log_gateway_error(message='Exchange ' + name + ' Not Found', detail=str(error), request_id='', environ=environ)
    result = util_transport.prepare_error(transport=transport, message='Could not connect to Broker', error='', code='internal_error', status_code='504')
...
def service_unavailable(environ, start_response, transport, logger):
    ...
    logger.log_gateway_error(message=str(main_error['detail']), detail=main_error['title'], request_id=request_id, environ=environ)
    result = util_transport.prepare_error(transport=transport, message=str(main_error['detail']), error=Exception(main_error['title']), code='internal_error', status_code=status_code)
...
def internal_error(environ, start_response, error, transport, logger, request_id):
    ...
    logger.log_gateway_error(message='Gateway Fatal Error', detail=trace_message.replace('\n', ' ').replace('\r', ''), request_id=request_id, environ=environ)
    result = util_transport.prepare_error(transport=transport, message=error, error=str(error), code='internal_error', status_code='500')
    start_response('500 INTERNAL ERROR', [('Content-Type', 'application/json')])
    return [json.dumps(result)]
...
def request_timeout(environ, start_response, transport, error, logger, request_id):
    ...
    logger.log_gateway_error(message='Gateway Timeout Error', detail=str(error), request_id=request_id, environ=environ)
    result = util_transport.prepare_error(transport=transport, message='rpc could not complete', error=error, code='request_timeout', status_code='408', request_id=request_id)
