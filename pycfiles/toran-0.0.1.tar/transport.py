...
# [GCC 8.4.0]
# Embedded file name: /Users/ricardovegas/Documents/dev/toran/handlers/util/transport.py
...
def prepare_transport(message, query, paths, request_id, verbose):
    ...
    if verbose is False:
        transport = {'@id': request_id, 'meta': {'request': {'params': query, 
                                'path': paths_dict}}}
    else:
        transport = {'@id': request_id, 
           'meta': {'request': {'params': query, 
                                'path': paths_dict}, 
                    'message': message, 
                    'depth': 0}}
    return transport
...
def prepare_error(transport, message, error, code=None, status_code=None, request_id=None):
    ...
    errors = {'error': {'status': status_code, 
                 'errors': error_list}}
