...
# [GCC 8.4.0]
# Embedded file name: /Users/ricardovegas/Documents/dev/toran/src/server.py
# Compiled at: 2016-04-23 07:33:51
...
import cgi, re, mmap, ast, ConfigParser, socket, uuid, json, urllib
rules = {}
auth_rules = {}
alias_rules = False
actions = {}
...
def calculate_route(rule, pathinfo, method):
    ...
    alias = calculate_alias(resource=first_entity_name, method=method, requested_action=resource_name, requested_id=resource_id)
    if alias == 'removed':
        return False
    if alias is not False:
        return {'resource': first_entity_name, 
           'id': resource_id, 
           'action': alias}
...
def dispatch(environ, start_response):
    ...
    destination = calculate_redirect(pathinfo=pathinfo)
    if destination is not False:
        ...
        route = calculate_route(rule=general_rule, pathinfo=pathinfo, method=method)
        if route == 'wrong_url':
            logger.log_http_response(environ, '404', request_id)
            return general_handler.bad_request(environ=environ, start_response=start_response, transport=transport, error=Exception('Endpoint is not available'))
        if route == 'invalid_method':
            logger.log_http_response(environ, '405', request_id)
            return general_handler.bad_method(environ=environ, start_response=start_response, transport=transport)
        if route is False:
            logger.log_http_response(environ, '404', request_id)
            return general_handler.url_not_found(environ=environ, start_response=start_response, transport=transport)
        ...
        try:
            for key in form_data.keys():
                if type(form_data[key]).__name__ != 'list' and form_data[key].filename:
                    ...
                    if content_type == 'application/x-www-form-urlencoded' or content_type == 'application/x-www-form-urlencoded; charset=utf-8':
        ...
        transport = util_transport.prepare_transport(message=message, query=query, paths=readable_routes, request_id=request_id, verbose=verbose)
        try:
            try:
            ...
            return general_handler.handle(environ=environ, start_response=start_response, transport=transport, resource=resource, version=version, action=action, request_id=request_id, logger=logger)
        except (NameError, AttributeError, TypeError, Exception) as e:
            return general_handler.internal_error(environ=environ, start_response=start_response, error=e, transport=transport, logger=logger, request_id=request_id)
...
def load_actions():
    ...
    for action in config.sections():
        action_config = {'method': config.get(action, 'method'), 'id': config.getboolean(action, 'id')}
...
def load_rules():
    ...
    for resource in config.sections():
        entity_rules = {'actions': config.get(resource, 'actions')}
...
# global auth ## Warning: Unused global