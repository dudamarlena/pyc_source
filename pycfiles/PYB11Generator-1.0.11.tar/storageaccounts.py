...
# Embedded file name: build\bdist.win32\egg\pyazure\storageaccounts.py
...
class StorageAccounts(ServiceManagementEndpoint):
    ...
    @property
    def base_url(self):
        return super(StorageAccounts, self).base_url + '/services/storageservices'
    ...
    def list_storage_accounts(self, just_names=True):
        ...
        for storage in storages:
            service_name = storage.findtext('{%s}ServiceName' % NAMESPACE_MANAGEMENT)
            if just_names:
                yield service_name
            else:
    ...
    def get_storage_account_properties(self, name):
        ...
        props['ServiceName'] = storage.findtext('.//{%s}ServiceName' % NAMESPACE_MANAGEMENT)
        props['Description'] = storage.findtext('.//{%s}Description' % NAMESPACE_MANAGEMENT)
        affinitygroup = storage.find('.//{%s}AffinityGroup' % NAMESPACE_MANAGEMENT)
        if affinitygroup:
            props['AffinityGroup'] = affinitygroup.text
        else:
            props['Location'] = storage.findtext('.//{%s}Location' % NAMESPACE_MANAGEMENT)
        label = storage.findtext('.//{%s}Label' % NAMESPACE_MANAGEMENT)
        props['Label'] = base64.b64decode(label)
        props['Status'] = storage.findtext('.//{%s}Status' % NAMESPACE_MANAGEMENT)
    ...
    def create_storage_account(self, name, label, location_or_affinity_group, description=''):
        """The Create Storage Account (async-)operation creates a new
        storage account in Windows Azure."""
        if not re.match('^[a-z0-9]{3,24}$', name):
            raise ValueError(name, 'name must be between 3 and 24 characters in length and use ' + 'numbers and lower-case letters only.')
        if not label:
            raise ValueError(label, 'label must be set')
        if len(label) > 100:
            raise ValueError(label, 'label must be <= 100 chars')
        if description and len(description) > 1024:
            raise ValueError(description, 'description must be <= 1024 chars')
