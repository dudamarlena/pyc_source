...
# Embedded file name: build\bdist.win32\egg\pyazure\hostedservices.py
...
class HostedServices(ServiceManagementEndpoint):
    ...
    @property
    def base_url(self):
        return super(HostedServices, self).base_url + '/services/hostedservices'

    @property
    def locations(self):
        if not self._locations:
            self._locations = list(Locations(self.cert, self.sub_id, self.key).list_locations())
    ...
    def list_services(self, just_names=True):
        ...
        for service in services:
            service_name = service.findtext('{%s}ServiceName' % NAMESPACE_MANAGEMENT)
            if just_names:
                yield service_name
            else:
    ...
    def create_service(self, service_name, label, location_or_affinity_group, description=''):
        ...
        try:
            i = [ l.lower() for l in self.locations ].index(location_or_affinity_group.lower().strip())
    ...
    def get_service_properties(self, service_name, embed_detail=False):
        ...
        info['ServiceName'] = service.findtext('.//{%s}ServiceName' % NAMESPACE_MANAGEMENT)
        props = OrderedDict()
        info['HostedServiceProperties'] = props
        props['Description'] = service.findtext('.//{%s}Description' % NAMESPACE_MANAGEMENT)
        props['Location'] = service.findtext('.//{%s}Location' % NAMESPACE_MANAGEMENT)
        affinitygroup = service.findtext('.//{%s}AffinityGroup' % NAMESPACE_MANAGEMENT)
        if affinitygroup:
            props['AffinityGroup'] = affinitygroup
        else:
            props['Location'] = service.findtext('.//{%s}Location' % NAMESPACE_MANAGEMENT)
        props['Label'] = base64.b64decode(service.findtext('.//{%s}Label' % NAMESPACE_MANAGEMENT))
        if embed_detail:
            deployments = service.findall('.//{%s}Deployment' % NAMESPACE_MANAGEMENT)
            info['Deployments'] = []
    ...
    def create_deployment(self, service_name, deployment_slot, name, package_url, label, configuration, start_deployment=False, treat_warnings_as_error=False):
        ...
        if deployment_slot.lower() not in ('staging', 'production'):
            raise ValueError(deployment_slot, 'deployment_slot must be ' + '"staging" or "production"')
        if len(label) > 100:
            raise ValueError(label, 'label exceeds 100 char limit')
        ...
        if start_deployment:
            req_body['StartDeployment'] = 'true'
        if treat_warnings_as_error:
            req_body['TreatWarningsAsError'] = 'true'
    ...
    def _parse_deployment(self, deployment_element):
        ...
        info['DeploymentSlot'] = de.findtext('.//{%s}DeploymentSlot' % NAMESPACE_MANAGEMENT)
        info['PrivateID'] = de.findtext('.//{%s}PrivateID' % NAMESPACE_MANAGEMENT)
        info['Status'] = de.findtext('.//{%s}Status' % NAMESPACE_MANAGEMENT)
        info['Label'] = base64.b64decode(de.findtext('.//{%s}Label' % NAMESPACE_MANAGEMENT))
        info['Url'] = de.findtext('.//{%s}Url' % NAMESPACE_MANAGEMENT)
        info['Configuration'] = base64.b64decode(de.findtext('.//{%s}Configuration' % NAMESPACE_MANAGEMENT))
        role_instances = de.findall('.//{%s}RoleInstance' % NAMESPACE_MANAGEMENT)
        info['RoleInstanceList'] = []
        for ri in role_instances:
            ri_info = OrderedDict()
            ri_info['RoleName'] = ri.findtext('.//{%s}RoleName' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceName'] = de.findtext('.//{%s}InstanceName' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceStatus'] = de.findtext('.//{%s}InstanceStatus' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceUpgradeDomain'] = de.findtext('.//{%s}InstanceUpgradeDomain' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceFaultDomain'] = de.findtext('.//{%s}InstanceFaultDomain' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceSize'] = de.findtext('.//{%s}InstanceSize' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceStateDetails'] = de.findtext('.//{%s}InstanceStateDetails' % NAMESPACE_MANAGEMENT)
            ri_info['InstanceErrorCode'] = de.findtext('.//{%s}InstanceErrorCode' % NAMESPACE_MANAGEMENT)
            info['RoleInstanceList'].append(ri_info)

        upgrade_status = de.find('.//{%s}UpgradeStatus' % NAMESPACE_MANAGEMENT)
        if upgrade_status:
            info['UpgradeType'] = de.findtext('.//{%s}UpgradeType' % NAMESPACE_MANAGEMENT)
            info['CurrentUpgradeDomainState'] = de.findtext('.//{%s}CurrentUpgradeDomainState' % NAMESPACE_MANAGEMENT)
            info['CurrentUpgradeDomain'] = de.findtext('.//{%s}CurrentUpgradeDomain' % NAMESPACE_MANAGEMENT)
        info['UpgradeDomainCount'] = de.findtext('.//{%s}UpgradeDomainCount' % NAMESPACE_MANAGEMENT)
        roles = de.findall('.//{%s}Role' % MANAGEMENT_VERSION)
        info['RoleList'] = []
        for role in roles:
            role_info = OrderedDict()
            role_info['RoleName'] = role.findtext('.//{%s}RoleName' % NAMESPACE_MANAGEMENT)
            role_info['OsVersion'] = role.findtext('.//{%s}OsVersion' % NAMESPACE_MANAGEMENT)
            info['RoleList'].append(role_info)

        info['SdkVersion'] = de.findtext('.//{%s}SdkVersion' % NAMESPACE_MANAGEMENT)
        input_endpoints = de.findall('.//{%s}InputEndpoint' % MANAGEMENT_VERSION)
        info['InputEndpointList'] = []
        for endpoint in input_endpoints:
            endpoint_info = OrderedDict()
            endpoint_info['RoleName'] = endpoint.findtext('.//{%s}RoleName' % NAMESPACE_MANAGEMENT)
            endpoint_info['Vip'] = endpoint.findtext('.//{%s}Vip' % NAMESPACE_MANAGEMENT)
            endpoint_info['Port'] = endpoint.findtext('.//{%s}Port' % NAMESPACE_MANAGEMENT)
            info['InputEndpointList'].append(endpoint_info)

        info['Locked'] = de.findtext('.//{%s}Locked' % NAMESPACE_MANAGEMENT)
        info['RollbackAllowed'] = de.findtext('.//{%s}RollbackAllowed' % NAMESPACE_MANAGEMENT)
        return info
    ...
    def delete_deployment(self, service_name, deployment_slot_or_name):
        ...
        deployment."""
        log.debug('Deleting deployment: %s - %s', service_name, deployment_slot_or_name)
    ...
    def change_deployment_configuration(self, service_name, deployment_slot_or_name, configuration, treat_warnings_as_error=False, mode='Auto'):
        ...
        log.debug('Changing deployment config: %s - %s', service_name, deployment_slot_or_name)
        ...
        if deployment_slot_or_name.lower() in ('staging', 'production'):
            req = RequestWithMethod('POST', '%s/%s/deploymentslots/%s/?comp=config' % (self.base_url,
             service_name, deployment_slot_or_name.lower()))
        else:
            req = RequestWithMethod('POST', '%s/%s/deployments/%s/?comp=config' % (self.base_url,
    ...
    def update_deployment_status(self, service_name, deployment_slot_or_name, status):
        ...
        log.debug('Updating deployment: %s - %s to %s', service_name, deployment_slot_or_name, status)
        if deployment_slot_or_name.lower() in ('staging', 'production'):
            req = RequestWithMethod('POST', '%s/%s/deploymentslots/%s/?comp=status' % (self.base_url,
             service_name, deployment_slot_or_name.lower()))
        else:
            req = RequestWithMethod('POST', '%s/%s/deployments/%s/?comp=status' % (self.base_url,
             service_name, deployment_slot_or_name))
        req_body = OrderedDict()
...
class ServiceConfiguration(object):
    ...
    def update_connections(self, data_connection_string):
        ...
        self.update_setting('DiagnosticsConnectionString', data_connection_string)
        self.update_setting('Microsoft.WindowsAzure.Plugins.Diagnostics.ConnectionString', data_connection_string)
