...
# Embedded file name: build/bdist.linux-x86_64/egg/PYB11Generator/PYB11class.py
# Compiled at: 2020-04-15 00:03:10
...
def PYB11generateModuleClasses(modobj, ss):
    ...
    for kname, klass in klasses:
        if isinstance(klass, PYB11TemplateClass):
            ...
            if not klassattrs['ignore'] and (klass not in mods or mods[klass] == modobj.PYB11modulename):
...
class PYB11TemplateClass:
    ...
    def __init__(self, klass_template, template_parameters, cppname=None, pyname=None, docext=''):
        ...
        if isinstance(template_parameters, str):
            ...
            if isinstance(template_parameters, tuple):
...
def PYB11generateClass(klass, klassattrs, ssout):
    ...
    special_operators = {'__init__': (ignore, ''), '__call__': (
                  call_operator, ''), 
       '__add__': (
                 binary_operator, '+'), 
       '__sub__': (
    ...
    if klassattrs['singleton']:
        ...
        if doc:
            ...
            PYB11docstring(doc, ss)
            ss(';\n')
        allmethods = [ (mname, meth) for mname, meth in PYB11ThisClassMethods(klass) if not PYB11attrs(meth)['ignore']
                     ]
        ss('\n    // Constructors\n')
        kills = []
        ...
        def newVirtualMethod(mname, meth, allmethods, klassattrs):
            ...
            if new_virtual:
                for othername, othermeth in allmethods:
                    othermethattrs = PYB11attrs(othermeth)
                    otherargs = extractArgs(othermeth)
                    if othermethattrs['virtual'] and methattrs['cppname'] == othermethattrs['cppname'] and otherargs == args:
                        return False
        ...
        if klassattrs['exposeBaseOverloads']:
            ...
            for bklass in inspect.getmro(klass)[1:]:
                ...
                for mname, meth in PYB11ThisClassMethods(bklass):
                    if not PYB11attrs(meth)['ignore'] and mname[:6] != 'pyinit' and newOverloadedMethod(meth, allmethods, klassattrs):
                        methattrs = PYB11attrs(meth)
        ...
        for bklass in inspect.getmro(klass)[1:]:
            ...
            if bklassattrs['template']:
                ...
                bklassattrs['cppname'] = bcppname
            for mname, meth in PYB11ThisClassMethods(bklass):
                if not PYB11attrs(meth)['ignore'] and newVirtualMethod(mname, meth, allmethods, klassattrs):
                    methattrs = PYB11attrs(meth)
                    PYB11generic_class_method(bklass, bklassattrs, meth, methattrs, ss)
