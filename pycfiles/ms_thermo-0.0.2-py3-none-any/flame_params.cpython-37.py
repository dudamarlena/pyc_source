...
# [GCC 8.4.0]
# Embedded file name: /Users/rossi/gitlab/ms_thermo/src/ms_thermo/flame_params.py
# Compiled at: 2020-02-25 09:22:47
...
class FlameTable:
    ...
    def print_param_space(self):
        ...
        self.nparam = len(self.phi_list) * len(self.temperature_list) * len(self.pressure_list)
        ...
        print(str('temperature').ljust(20) + ': ' + str(np.mean(self.temperature_list)) + '\xa0[K]')
        print(str('pressure').ljust(20) + ': ' + str(np.mean(self.pressure_list)) + ' [Pa]')
    ...
    def read_table_hdf(self, hdf_filename):
        ...
        *Read the flame data table from an hdf format*
        ...
        self.datanames_list = [dataset for dataset in hdf_file['Data'].keys() if 'CLASS' not in hdf_file['Data'][dataset].attrs.keys()]
        for dataset in self.datanames_list:
            self.data_dict[dataset] = hdf_file['Data'][dataset][:]

        dims = [hdf_file['Data'].attrs['DIM1_NAME'].decode(),
         hdf_file['Data'].attrs['DIM2_NAME'].decode(),
         hdf_file['Data'].attrs['DIM3_NAME'].decode()]
        if dims[0] != 'EQUIVALENCE_RATIO':
            raise IOError('The first dimension of the table should be EQUIVALENCE_RATIO')
        if dims[1] != 'TEMPERATURE':
            raise IOError('The second dimension of the table should be TEMPERATURE')
        if dims[2] != 'PRESSURE':
    ...
    def check_bounds(self, equivalence_ratio, temperature, pressure):
        ...
        if pressure < self.pressure_list[0] or pressure > self.pressure_list[(-1)]:
            raise IOError('Input pressure ' + str(pressure) + ' is out of database bounds: ' + str(self.pressure_list[0]) + ' ' + str(self.pressure_list[(-1)]))
        if temperature < self.temperature_list[0] or temperature > self.temperature_list[(-1)]:
            raise IOError('Input temperature ' + str(temperature) + ' is out of database bounds: ' + str(self.temperature_list[0]) + ' ' + str(self.temperature_list[(-1)]))
        if equivalence_ratio < self.phi_list[0] or equivalence_ratio > self.phi_list[(-1)]:
            raise IOError('Input equivalence_ratio ' + str(equivalence_ratio) + ' is out of database bounds: ' + str(self.phi_list[0]) + ' ' + str(self.phi_list[(-1)]))
    ...
    def get_params(self, equivalence_ratio=1.0, temperature=600, pressure=101325):
        ...
        index, target_point = squeeze_interp((self.phi_list,
         self.temperature_list,
         self.pressure_list), [
         equivalence_ratio, temperature, pressure])
        for dataname in self.data_dict:
            self.interpolated_data_dict[dataname] = interpn(index, (np.squeeze(self.data_dict[dataname])),
              target_point,
              bounds_error=True)
