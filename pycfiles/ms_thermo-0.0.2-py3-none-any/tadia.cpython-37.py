...
# [GCC 8.4.0]
# Embedded file name: /Users/rossi/gitlab/ms_thermo/src/ms_thermo/tadia.py
# Compiled at: 2020-02-28 05:19:35
...
def tadia_table(t_fresh_gases, p_fresh_gases, phi, fuel=None):
    """
        *Compute the adiabatic flame temperature of a premixed kero/air mixture from tables*
        ...
        """
    case = FlameTable()
    mass_molar = {'C':0.0120107, 
     'H':0.00100797, 
     'O2':0.0319988, 
     'N2':0.0280134}
    if fuel == None:
        fuel_table = pkg_resources.resource_filename(__name__, '../INPUT/2S_KERO_BFER.h5')
        c_x = 10
        h_y = 20
        ...
        if phi <= 1:
            ...
            yk_burnt['H2O'] = 1 / (0.5 + 2 * c_x / h_y) * yk_fresh['O2'] * (0.5 * mass_molar['O2'] + 2 * mass_molar['H']) / mass_molar['O2']
    else:
        fuel_table = pkg_resources.resource_filename(__name__, '../INPUT/' + fuel + '.h5')
