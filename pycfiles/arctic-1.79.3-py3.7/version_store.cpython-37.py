...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/store/version_store.py
# Compiled at: 2019-07-09 17:30:49
# Size of source mod 2**32: 56986 bytes
import logging
from datetime import datetime as dt, timedelta
import bson, pymongo, six
...
def register_version(version, numerical):
    global ARCTIC_VERSION
    global ARCTIC_VERSION_NUMERICAL
...
class VersionStore(object):
    ...
    def list_versions(self, symbol=None, snapshot=None, latest_only=False):
        ...
        for symbol in symbols:
            ...
            for version in self._versions.find(query, projection=['symbol', 'version', 'parent', 'metadata.deleted'], sort=[('version', -1)]):
                if latest_only:
                    if version['symbol'] in seen_symbols:
                        continue
                seen_symbols.add(version['symbol'])
                meta = version.get('metadata')
                versions.append({'symbol':version['symbol'],  'version':version['version'],  'deleted':meta.get('deleted', False) if meta else False, 
    ...
    def read(self, symbol, as_of=None, date_range=None, from_version=None, allow_secondary=None, **kwargs):
        ...
        try:
            ...
            return (self._do_read)(symbol, _version, from_version, date_range=date_range, 
            ...
            try:
                ...
                return (self._do_read_retry)(symbol, _version, from_version, date_range=date_range, 
    ...
    def _do_read(self, symbol, version, from_version=None, **kwargs):
        ...
        if self._with_strict_handler_match:
            if kwargs.get('date_range'):
                if not self.handler_supports_read_option(handler, 'date_range'):
        ...
        return VersionedItem(symbol=symbol, library=(self._arctic_lib.get_name()), version=(version['version']), metadata=(version.pop('metadata', None)),
          data=data,
    ...
    def read_metadata(self, symbol, as_of=None, allow_secondary=None):
        ...
        return VersionedItem(symbol=symbol, library=(self._arctic_lib.get_name()), version=(_version['version']), metadata=(_version.pop('metadata', None)),
          data=None,
          host=(self._arctic_lib.arctic.mongo_host))
            ...
            if isinstance(as_of, six.string_types):
                ...
                if snapshot:
                    ...
                    if isinstance(as_of, dt):
                        ...
                        _version = versions_coll.find_one({'symbol':symbol,  '_id':{'$lt': bson.ObjectId.from_datetime(as_of + timedelta(seconds=1))}},
                          sort=[
                         (
                          'symbol', pymongo.DESCENDING), ('version', pymongo.DESCENDING)])
                    else:
                        _version = versions_coll.find_one({'symbol':symbol,  'version':as_of})
            else:
                assert _version, 'No data found for %s in library %s' % (symbol, self._arctic_lib.get_name())
            metadata = _version.get('metadata', None)
            if metadata is not None:
                if metadata.get('deleted', False) is True:
                    raise NoDataFoundException('No data found for %s in library %s' % (symbol, self._arctic_lib.get_name()))
            return _version
    ...
    def append(self, symbol, data, metadata=None, prune_previous_version=True, upsert=True, **kwargs):
        ...
        spec = {'symbol': symbol}
        previous_version = self._versions.find_one(spec, sort=[
         (
          'version', pymongo.DESCENDING)])
        if len(data) == 0:
            if previous_version is not None:
                return VersionedItem(symbol=symbol, library=(self._arctic_lib.get_name()), version=(previous_version['version']), metadata=(version.pop('metadata', None)),
                  data=None,
                  host=(self._arctic_lib.arctic.mongo_host))
        if upsert:
        ...
        dirty_append = False
        next_ver = self._version_nums.find_one_and_update({'symbol': symbol}, {'$inc': {'version': 1}},
          upsert=False,
          new=True)['version']
        ...
        if upsert:
            if previous_metadata is not None:
                if previous_metadata.get('deleted', False) is True:
                    return self.write(symbol=symbol, data=data, prune_previous_version=prune_previous_version, metadata=metadata)
        handler = self._read_handler(previous_version, symbol)
        if metadata is not None:
            version['metadata'] = metadata
        else:
            if 'metadata' in previous_version:
    ...
    def write(self, symbol, data, metadata=None, prune_previous_version=True, **kwargs):
        ...
        version['arctic_version'] = ARCTIC_VERSION_NUMERICAL
        version['symbol'] = symbol
        version['version'] = self._version_nums.find_one_and_update({'symbol': symbol}, {'$inc': {'version': 1}},
          upsert=True,
          new=True)['version']
        version['metadata'] = metadata
        previous_version = self._versions.find_one({'symbol':symbol,  'version':{'$lt': version['version']}}, sort=[
         (
          'version', pymongo.DESCENDING)])
    ...
    def _add_new_version_using_reference(self, symbol, new_version, reference_version, prune_previous_version):
        ...
        if last_look is None or last_look.get('deleted'):
            ...
            raise OperationFailure('Failed to write metadata for symbol %s. The previous version (%s, %d) has been removed during the update' % (
        ...
        return VersionedItem(symbol=symbol, library=(self._arctic_lib.get_name()), version=(new_version['version']), metadata=(new_version.get('metadata')),
          data=None,
          host=(self._arctic_lib.arctic.mongo_host))
    ...
    def write_metadata(self, symbol, metadata, prune_previous_version=True, **kwargs):
        ...
        try:
            ...
            new_version_num = self._version_nums.find_one_and_update({'symbol': symbol}, {'$inc': {'version': 1}},
              upsert=True,
              new=True)['version']
    ...
    def restore_version(self, symbol, as_of, prune_previous_version=True):
        ...
        if self._last_version_seqnum(symbol) == version_to_restore['version']:
            return VersionedItem(symbol=symbol, library=(self._arctic_lib.get_name()), version=(version_to_restore['version']),
              host=(self._arctic_lib.arctic.mongo_host),
              metadata=(version_to_restore.pop('metadata', None)),
              data=None)
        item = self.read(symbol, as_of=as_of)
        new_item = self.write(symbol, data=(item.data),
    ...
    def _find_prunable_version_ids(self, symbol, keep_mins):
        ...
        query = {'symbol':symbol,  '$or':[
          {'parent': {'$exists': False}}, {'parent': []}], 
         '_id':{'$lt': bson.ObjectId.from_datetime(dt.utcnow() + timedelta(seconds=1) - timedelta(minutes=keep_mins))}}
        cursor = versions.find(query, sort=[
         (
          'version', pymongo.DESCENDING)],
          skip=1,
          projection={'_id': 1, FW_POINTERS_REFS_KEY: 1, FW_POINTERS_CONFIG_KEY: 1})
        return {v['_id']:(
         [bson.binary.Binary(x) for x in v.get(FW_POINTERS_REFS_KEY, [])], get_fwptr_config(v)) for v in cursor}
    ...
    def _find_base_version_ids(self, symbol, version_ids):
        ...
        """
        cursor = self._versions.find({'symbol':symbol,  '_id':{'$nin': version_ids}, 
    ...
    def _prune_previous_versions(self, symbol, keep_mins=120, keep_version=None, new_version_shas=None):
        ...
        shas_to_delete = [sha for v in prunable_ids_to_shas.values() if sha not in new_version_shas for sha in v[0]]
        mongo_retry(cleanup)((self._arctic_lib), symbol, version_ids, (self._versions), shas_to_delete=shas_to_delete,
    ...
    def _delete_version(self, symbol, version_num, do_cleanup=True):
        ...
        if do_cleanup:
            cleanup((self._arctic_lib), symbol, [version['_id']], (self._versions), shas_to_delete=(tuple((bson.binary.Binary(s) for s in version.get(FW_POINTERS_REFS_KEY, [])))),
              pointers_cfgs=(
             get_fwptr_config(version),))
    ...
    def _cleanup_unreachable_shas(self, dry_run):
        ...
        for symbol in chunks_coll.distinct('symbol'):
            ...
            if FwPointersCfg.DISABLED not in all_symbol_pointers_cfgs:
                ...
                id_time_constraint = dry_run or {'$lt': bson.ObjectId.from_datetime(dt.now() - timedelta(days=1))}
                chunks_coll.delete_many({'_id':id_time_constraint, 
                 'symbol':symbol, 
