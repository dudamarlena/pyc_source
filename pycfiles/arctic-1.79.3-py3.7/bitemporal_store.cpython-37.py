...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/store/bitemporal_store.py
...
class BitemporalStore(object):
    ...
    def read(self, symbol, as_of=None, raw=False, **kwargs):
        ...
        if raw:
            return BitemporalItem(symbol=symbol, library=(self._store._arctic_lib.get_name()), data=(item.data), metadata=(item.metadata),
              last_updated=last_updated)
        index_names = list(item.data.index.names)
        index_names.remove(self.observe_column)
        return BitemporalItem(symbol=symbol, library=(self._store._arctic_lib.get_name()), data=groupby_asof((item.data), as_of=as_of, dt_col=index_names, asof_col=(self.observe_column)),
    ...
    def update(self, symbol, data, metadata=None, upsert=True, as_of=None, **kwargs):
        ...
        if not as_of:
            ...
            if upsert:
                df = self._store.has_symbol(symbol) or data
            else:
