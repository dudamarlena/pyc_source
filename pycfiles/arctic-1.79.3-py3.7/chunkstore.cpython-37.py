...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/chunkstore/chunkstore.py
# Compiled at: 2019-08-24 20:25:44
# Size of source mod 2**32: 28526 bytes
...
class ChunkStore(object):
    ...
    def _ensure_index(self):
        self._symbols.create_index([(SYMBOL, pymongo.ASCENDING)], unique=True,
          background=True)
        ...
        self._collection.create_index([(SYMBOL, pymongo.ASCENDING),
         ...
         (
          ...
          unique=True,
          background=True)
        self._collection.create_index([(SEGMENT, pymongo.ASCENDING)], unique=False,
          background=True)
    ...
    def delete(self, symbol, chunk_range=None, audit=None):
        ...
        if chunk_range is not None:
            ...
            row_adjust = len(df)
            df = df.empty or CHUNKER_MAP[sym[CHUNKER]].exclude(df, chunk_range)
            query = {SYMBOL: symbol}
    ...
    def read(self, symbol, chunk_range=None, filter_data=True, **kwargs):
        ...
        for _, segments in groupby(segment_cursor, key=(lambda x: (x[START], x[SYMBOL]))):
            ...
            mdata = self._mdata.find_one({SYMBOL: segments[0][SYMBOL], 
             ...
             END: segments[0][END]})
            chunk_data = ''.join([doc[DATA] for doc in segments])
        ...
        if skip_filter:
            return deser((chunks[symbol[0]]), **kwargs)
        return chunker.filter(deser((chunks[symbol[0]]), **kwargs), chunk_range)
    ...
    def write(self, symbol, item, metadata=None, chunker=DateChunker(), audit=None, **kwargs):
        ...
        mongo_retry(self._symbols.update_one)({SYMBOL: symbol}, {'$set': doc},
          upsert=True)
        if audit is not None:
            audit['symbol'] = symbol
            audit['action'] = 'write'
    ...
    def append(self, symbol, item, upsert=False, metadata=None, audit=None, **kwargs):
        ...
        if audit is not None:
            audit['symbol'] = symbol
            audit['action'] = 'append'
        self._ChunkStore__update(sym, item, metadata=metadata, combine_method=(SER_MAP[sym[SERIALIZER]].combine), audit=audit)
    ...
    def update(self, symbol, item, metadata=None, chunk_range=None, upsert=False, audit=None, **kwargs):
        ...
        if not sym:
            ...
            if chunk_range is not None:
                ...
                self._ChunkStore__update(sym, item, metadata=metadata, combine_method=(self.serializer.combine), chunk_range=chunk_range, audit=audit)
            else:
                self._ChunkStore__update(sym, item, metadata=metadata, combine_method=(lambda old, new: new), chunk_range=chunk_range, audit=audit)
    ...
    def get_chunk_ranges(self, symbol, chunk_range=None, reverse=False):
        ...
        for x in self._collection.find(spec, projection=[
         START, END],
          sort=[
         (
          START, pymongo.ASCENDING if not reverse else pymongo.DESCENDING)]):
            yield (
             c.chunk_to_str(x[START]), c.chunk_to_str(x[END]))
