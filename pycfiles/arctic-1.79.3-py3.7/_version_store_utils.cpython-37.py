...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/store/_version_store_utils.py
# Compiled at: 2019-12-06 10:13:42
# Size of source mod 2**32: 15797 bytes
import functools, hashlib, logging, pickle, numpy as np, pandas as pd, pymongo, six
...
def _cleanup_parent_pointers(collection, symbol, version_ids):
    for v in version_ids:
        collection.delete_many({'symbol':symbol,  'parent':[
          v]})
...
def cleanup(arctic_lib, symbol, version_ids, versions_coll, shas_to_delete=None, pointers_cfgs=None):
    ...
    if not (all_symbol_pointers_cfgs == {FwPointersCfg.DISABLED} or all_symbol_pointers_cfgs):
        _cleanup_parent_pointers(collection, symbol, version_ids)
        return
    if FwPointersCfg.DISABLED not in all_symbol_pointers_cfgs:
        _cleanup_fw_pointers(collection, symbol, version_ids, versions_coll, shas_to_delete=shas_to_delete,
          do_clean=True)
        return
...
def analyze_symbol(l, sym, from_ver, to_ver, do_reads=False):
    ...
    for v in l._versions.find({'symbol':sym,  'version':{'$gte':from_ver,  '$lte':to_ver}}, sort=[
     (
      'version', pymongo.ASCENDING)]):
        ...
        if is_deleted:
            ...
            spec = {'symbol':sym, 
             'parent':v.get('base_version_id', v['_id']),  'segment':{'$lt': v.get('up_to', 0)}}
...
def _fast_check_corruption(collection, sym, v, check_count, check_last_segment, check_append_safe):
    if v is None:
        ...
        if check_append_safe:
            ...
            spec = {'symbol':sym, 
             'parent':v.get('base_version_id', v['_id']),  'segment':{'$lt': v['up_to']}}
    try:
        if check_count:
...
def is_safe_to_append(l, sym, input_v):
    ...
    return not _fast_check_corruption((l._collection), sym, input_v, check_count=True,
...
def fast_is_corrupted(l, sym, input_v):
    ...
    """
    input_v = l._versions.find_one({'symbol':sym,  'version':input_v}) if isinstance(input_v, int) else input_v
    return _fast_check_corruption((l._collection), sym, input_v, check_count=True,
      check_last_segment=True,
...
def is_corrupted(l, sym, input_v):
    """
        ...
        """
    input_v = l._versions.find_one({'symbol':sym,  'version':input_v}) if isinstance(input_v, int) else input_v
    if not _fast_check_corruption((l._collection), sym, input_v, check_count=True,
      check_last_segment=True,
      check_append_safe=False):
        try:
