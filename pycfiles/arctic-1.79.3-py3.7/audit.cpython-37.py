...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/store/audit.py
...
class ArcticTransaction(object):
    ...
    def __init__(self, version_store, symbol, user, log, modify_timeseries=None, audit=True, *args, **kwargs):
        ...
        try:
            ...
            versions.append(0)
            self.base_ts = VersionedItem(symbol=(self._symbol), library=None, version=(versions[0]),
              metadata=None,
              data=None,
              host=None)
        ...
        if modify_timeseries is not None:
            if not are_equals(modify_timeseries, self.base_ts.data):
    ...
    def write(self, symbol, data, prune_previous_version=True, metadata=None, **kwargs):
        ...
        """
        if data is not None:
            if not (self.base_ts.data is None or are_equals)(data, self.base_ts.data) or metadata != self.base_ts.metadata:
                self._do_write = True
