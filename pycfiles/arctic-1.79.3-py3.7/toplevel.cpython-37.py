...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/arctic/tickstore/toplevel.py
# Compiled at: 2019-02-02 17:02:31
# Size of source mod 2**32: 9728 bytes
import bisect, logging, re
...
from ..exceptions import NoDataFoundException, UnhandledDtypeException, OverlappingDataException, LibraryNotFoundException
...
class TopLevelTickStore(object):
    ...
    def add(self, date_range, library_name):
        ...
        if not (date_range.start and date_range.end):
            raise AssertionError('Date range should have start and end properties {}'.format(date_range))
        ...
        if not (start.time() == time.min and end.time() == end_time_min):
            raise AssertionError('Date range should fall on UTC day boundaries {}'.format(date_range))
        library_metadata = self._get_library_metadata(date_range)
        if (len(library_metadata) > 1 or len(library_metadata)) == 1:
            if library_metadata[0] != library_name:
        ...
        self._collection.update_one({'library_name': library_name}, {'$set': {'start':start,  'end':end}},
          upsert=True)
    ...
    def read(self, symbol, date_range, columns=None, include_images=False):
        ...
        for l in libraries:
            try:
                df = l.library.read(symbol, (l.date_range.intersection(date_range)), columns, include_images=include_images)
                dfs.append(df)
    ...
    def _get_libraries(self, date_range):
        ...
        rtn = [TickStoreLibrary(self._arctic_lib.arctic[library.library], library.date_range) for library in libraries]
        if rtn:
            current_start = rtn[(-1)].date_range.end
        else:
            if date_range.end.tzinfo:
        ...
        if date_range.end is None or current_start < date_range.end:
            ...
            try:
                rtn.append(TickStoreLibrary(self._arctic_lib.arctic[current_lib], DateRange(current_start, None, OPEN_OPEN)))
    ...
    def _get_library_metadata(self, date_range):
        ...
        query = {'$or': [{'start':{'$lte': start},  'end':{'$gte': start}},
                 ...
                 {'start':{'$lte': end}, 
                  'end':{'$gte': end}}]}
        cursor = self._collection.find(query, projection={'library_name':1, 
         'start':1,  'end':1},
          sort=[
         (
