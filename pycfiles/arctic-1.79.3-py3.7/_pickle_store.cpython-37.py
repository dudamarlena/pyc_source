    ...
    def read(self, mongoose_lib, version, symbol, **kwargs):
        ...
        if blob is not None:
            if blob == _MAGIC_CHUNKEDV2:
                ...
                data = (b'').join((decompress(x['data']) for x in sorted((collection.find({'symbol':symbol,  'parent':version_base_or_id(version)})),
                  key=(itemgetter('segment')))))
            else:
                if blob == _MAGIC_CHUNKED:
                    collection = mongoose_lib.get_top_level_collection()
                    data = (b'').join((x['data'] for x in sorted((collection.find({'symbol':symbol,  'parent':version_base_or_id(version)})),
                      key=(itemgetter('segment')))))
