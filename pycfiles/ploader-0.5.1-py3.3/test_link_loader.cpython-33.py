...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ploader/tests/test_link_loader.py
# Compiled at: 2014-01-10 20:07:03
...
class TestLinkLoaderSpecialPloaderFile(TestCase):
    ...
    def test_get_unstarted_download(self):
        sample_dw = Download('bla', [{'link': 'http://foo.bar.baz/other_stuff.avi',  'filename': '',  'status': 'not started'}, {'link': 'http://foo.bar.baz/other_stuff2.avi',  'filename': '',  'status': 'not started'}], 'blub')
        self.assertEqual(repr(self.link_loader.get_unstarted_download()), repr(sample_dw))
...
class TestLinkLoader(TestCase):
    ...
    def test_link_list_parser(self):
        link_list = [
         'http://foo.bar.baz', 'http://www.google.com/']
