...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/holdem/table.py
# Compiled at: 2018-01-07 12:52:54
# Size of source mod 2**32: 17485 bytes
...
class TexasHoldemEnv(Env, utils.EzPickle):
    BLIND_INCREMENTS = [
     [
      10, 25], [25, 50], [50, 100], [75, 150], [100, 200],
     [
      150, 300], [200, 400], [300, 600], [400, 800], [500, 10000],
     [
      600, 1200], [800, 1600], [1000, 2000]]

    def __init__(self, n_seats, max_limit=1000000000.0, debug=False):
        n_suits = 4
        n_ranks = 13
        n_community_cards = 5
        n_pocket_cards = 2
          ...
          spaces.Tuple([
           spaces.MultiDiscrete([
            [
             -1, n_suits],
            [
             -1, n_ranks]])] * n_pocket_cards)] * n_seats),
         spaces.Tuple([
          spaces.Discrete(n_seats - 1),
    ...
    def _ready_players(self):
        if len([p for p in self._seats if not p.emptyplayer if p.sitting_out]) == len(self._seats):
    ...
    def _resolve_sidepots(self, players_playing):
        ...
        try:
            ...
            smallest_players_allin = [p for p, bet in zip(players, [p.currentbet for p in players]) if bet == smallest_bet if p.isallin]
            for p in players:
    ...
    def _output_state(self, current_player):
        return {'players':[player.player_state() for player in self._seats],  'community':self.community, 
