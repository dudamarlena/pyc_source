...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/holdem/env.py
# Compiled at: 2018-01-07 14:49:39
...
class TexasHoldemEnv(Env, utils.EzPickle):
    ...
    def __init__(self, n_seats, max_limit=1000000000000.0, debug=False):
        n_suits = 4
        n_ranks = 13
        n_community_cards = 5
        n_pocket_cards = 2
    ...
    def _first_to_act(self, players):
        if self._round == 0:
            if len(players) == 2:
                return self._next(sorted((players + [self._seats[self._button]]),
