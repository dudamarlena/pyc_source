...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/holdem/player.py
# Compiled at: 2018-01-07 14:45:39
...
class Player(object):
    ...
    def player_state(self):
        return (
         self.get_seat(), self.stack, self.playing_hand, self.betting, self.player_id)
    ...
    def player_move(self, table_state, action):
        ...
        if tocall == 0:
            ...
            if action_idx == Player.RAISE:
                ...
                if action_idx == Player.CALL:
                    move_tuple = (
                     'call', tocall)
                else:
                    if choice == Player.FOLD:
                        move_tuple = ('fold', -1)
                    else:
                        raise error.Error('invalid action ({}) must be raise (2), call (1), or fold (3)'.format(action_idx))
            return move_tuple