...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/qi/Goban/browser/recent.py
# Compiled at: 2008-05-05 07:35:00
...
class RecentGamesView(BrowserView):
    ...
    def num_games(self):
        ...
        results = catalog(object_provides='qi.Goban.interfaces.IGoGame', path=('/').join(self.context.getPhysicalPath()))
        return len(results)

    def results(self, limit=20, offset=0):
        catalog = self.portal_catalog
        results = catalog(object_provides='qi.Goban.interfaces.IGoGame', sort_on='modified', sort_order='reverse', sort_limit=offset + limit, path=('/').join(self.context.getPhysicalPath()))[offset:offset + limit]
    ...
    def _buildDict(self, ob):
        ...
        return {'Title': ob.title_or_id(), 'Description': ob.Description(), 'absolute_url': ob.absolute_url(), 'review_state_normalized': wfstate, 'creator': creator, 'is_new': self._is_new(ob.modified()), 'blackPlayer': ob.getBlackPlayer(), 'blackRating': ob.getBlackRating(), 'whitePlayer': ob.getWhitePlayer(), 'whiteRating': ob.getWhiteRating(), 'datePlayed': ob.getDatePlayed(), 'place': ob.getPlace(), 'handicap': ob.getHandicap()}
