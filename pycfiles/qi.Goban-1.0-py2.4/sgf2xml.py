...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/qi/Goban/lib/sgf2xml.py
# Compiled at: 2008-04-06 04:02:05
import libxml2, re, copy
...
class sgf2xml:
    ...
    def parseNodeForMoves(self, node, board):
        ...
        if node.has_key('AE'):
            ...
            for stone in toRemove:
                ...
                if board[(coords[1], coords[0])] == 1:
                    self.addXMLMove(root, coords, False)
                elif board[(coords[1], coords[0])] == -1:
                    self.addXMLMove(root, coords, False, False)
    ...
    def translateCoordsInverse(self, sCoord):
        return (
         translCoordDictInverse[sCoord[0]], translCoordDictInverse[sCoord[1]])
