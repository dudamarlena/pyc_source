...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/handlers/crack.py
# Compiled at: 2020-01-19 08:02:16
...
from ..plugins.hashcat import Hashcat, HcxPcapTool
from ..plugins.john import John

class CrackHelper:
    __doc__ = 'Manages handshake retrieval, selection, and running the cracking commands.'
    TYPES = {'4-WAY':'4-Way Handshake', 
     'PMKID':'PMKID Hash'}
    ...
    def run(cls):
        ...
        for tool, dependencies in available_tools.items():
            missing = [dep for dep in dependencies if not Process.exists(dep.dependency_name)]
            if len(missing) > 0:
        ...
        if all_pmkid:
            ...
            Color.p('\n{+} Enter the {C}cracking tool{W} to use ({C}%s{W}): {G}' % '{W}, {C}'.join(available_tools.keys()))
            tool_name = raw_input()
                ...
                if hs_file.count('_') != 3:
                    ...
                    if cls.is_cracked(hs_file):
                        ...
                        if hs_file.endswith('.cap'):
                            ...
                            hours = hours.replace('-', ':')
                            date = '%s %s' % (days, hours)
                            handshake = {'filename':os.path.join(hs_dir, hs_file), 
                             'bssid':bssid.replace('-', ':'), 
                             'essid':essid, 
                             'date':date, 
                             'type':hs_type}
    ...
    def crack_4way(cls, hs, tool):
        handshake = Handshake((hs['filename']), bssid=(hs['bssid']),
          essid=(hs['essid']))
        try:
