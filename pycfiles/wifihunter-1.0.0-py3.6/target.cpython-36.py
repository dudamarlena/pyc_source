...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/handlers/target.py
# Compiled at: 2020-01-19 08:08:15
...
class Target(object):
    ...
    def __init__(self, fields):
        ...
        if self.essid == '\\x00' * self.essid_len or self.essid == 'x00' * self.essid_len or self.essid.strip() == '':
            self.essid = None
    ...
    def validate(self):
        ...
        if self.channel == '-1':
            ...
            bssid_broadcast = re.compile('^(ff:ff:ff:ff:ff:ff|00:00:00:00:00:00)$', re.IGNORECASE)
            if bssid_broadcast.match(self.bssid):
                raise Exception('Ignoring target with Broadcast BSSID (%s)' % self.bssid)
            bssid_multicast = re.compile('^(01:00:5e|01:80:c2|33:33)', re.IGNORECASE)
        ...
        if 'WEP' in encryption:
            ...
            if self.power > 50:
                ...
                if self.wps == WPSState.UNLOCKED:
                    ...
                    if self.wps == WPSState.NONE:
                        wps = Color.s('{O}  no')
                    else:
                        if self.wps == WPSState.LOCKED:
                            wps = Color.s('{R}lock')
