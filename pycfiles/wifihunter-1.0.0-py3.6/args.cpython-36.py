...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/handlers/args.py
# Compiled at: 2020-01-19 07:50:35
# Size of source mod 2**32: 19355 bytes
import sys, argparse
from .color import Color

class Arguments(object):
    __doc__ = ' Holds arguments '
    ...
    def get_arguments(self):
        ...
        parser = argparse.ArgumentParser(usage=(argparse.SUPPRESS), formatter_class=(lambda prog: argparse.HelpFormatter(prog,
          max_help_position=80, width=130)))
    ...
    def _add_global_args(self, glob):
        ...
        glob.add_argument('--pillage', help=(argparse.SUPPRESS), action='store', dest='scan_time',
          nargs='?',
          const=10,
          type=int)
        glob.add_argument('--kill', action='store_true',
          dest='kill_conflicting_processes',
          help=(Color.s('Kill processes that conflict with Airmon/Airodump (default: {G}off{W})')))
        glob.add_argument('-b', action='store',
          dest='target_bssid',
          metavar='[bssid]',
          type=str,
          help=(self._verbose('BSSID (e.g. {GR}AA:BB:CC:DD:EE:FF{W}) of access point to attack')))
        glob.add_argument('--bssid', help=(argparse.SUPPRESS), action='store', dest='target_bssid',
          type=str)
        glob.add_argument('-e', action='store',
          dest='target_essid',
          metavar='[essid]',
          type=str,
          help=(self._verbose('ESSID (e.g. {GR}NETGEAR07{W}) of access point to attack')))
        glob.add_argument('--essid', help=(argparse.SUPPRESS), action='store', dest='target_essid',
          type=str)
        glob.add_argument('-E', action='store',
          dest='ignore_essid',
          metavar='[text]',
        ...
        glob.add_argument('--ignore-essid', help=(argparse.SUPPRESS), action='store', dest='ignore_essid',
          type=str)
        ...
        glob.add_argument('--showb', action='store_true',
          dest='show_bssids',
          help=(self._verbose('Show BSSIDs of targets while scanning')))
        glob.add_argument('--nodeauths', action='store_true',
          dest='no_deauth',
        ...
        glob.add_argument('--num-deauths', action='store',
          type=int,
          dest='num_deauths',
          metavar='[num]',
          default=None,
    ...
    def _add_wep_args(self, wep):
        wep.add_argument('--wep', action='store_true',
          dest='wep_filter',
        ...
        wep.add_argument('--require-fakeauth', action='store_true',
        ...
        wep.add_argument('-nofakeauth', help=(argparse.SUPPRESS), action='store_true', dest='require_fakeauth')
        wep.add_argument('--keep-ivs', action='store_true',
          dest='wep_keep_ivs',
          default=False,
          help=(Color.s('Retain .IVS files and reuse when cracking (default: {G}off{W})')))
        wep.add_argument('--pps', action='store',
          dest='wep_pps',
          metavar='[pps]',
          type=int,
          help=(self._verbose('Packets-per-second to replay (default: ' + '{G}%d pps{W})' % self.config.wep_pps)))
        wep.add_argument('-pps', help=(argparse.SUPPRESS), action='store', dest='wep_pps',
          type=int)
        wep.add_argument('--wept', action='store',
          dest='wep_timeout',
          metavar='[seconds]',
          type=int,
          help=(self._verbose('Seconds to wait before failing (default: ' + '{G}%d sec{W})' % self.config.wep_timeout)))
        wep.add_argument('-wept', help=(argparse.SUPPRESS), action='store', dest='wep_timeout',
          type=int)
        wep.add_argument('--wepca', action='store',
          dest='wep_crack_at_ivs',
          metavar='[ivs]',
          type=int,
          help=(self._verbose('Start cracking at this many IVs (default: ' + '{G}%d ivs{W})' % self.config.wep_crack_at_ivs)))
        wep.add_argument('-wepca', help=(argparse.SUPPRESS), action='store', dest='wep_crack_at_ivs',
          type=int)
        wep.add_argument('--weprs', action='store',
        ...
        wep.add_argument('-weprs', help=(argparse.SUPPRESS), action='store', dest='wep_restart_stale_ivs',
          type=int)
        wep.add_argument('--weprc', action='store',
        ...
        wep.add_argument('-weprc', help=(argparse.SUPPRESS), action='store', dest='wep_restart_aircrack',
          type=int)
        wep.add_argument('--arpreplay', action='store_true',
        ...
        wep.add_argument('--fragment', action='store_true',
        ...
        wep.add_argument('--chopchop', action='store_true',
        ...
        wep.add_argument('--caffelatte', action='store_true',
        ...
        wep.add_argument('--p0841', action='store_true',
        ...
        wep.add_argument('--hirte', action='store_true',
    ...
    def _add_wpa_args(self, wpa):
        wpa.add_argument('--wpa', action='store_true',
          dest='wpa_filter',
        ...
        wpa.add_argument('--hs-dir', action='store',
          dest='wpa_handshake_dir',
          metavar='[dir]',
          type=str,
          help=(self._verbose('Directory to store handshake files ' + '(default: {G}%s{W})' % self.config.wpa_handshake_dir)))
        wpa.add_argument('-hs-dir', help=(argparse.SUPPRESS), action='store', dest='wpa_handshake_dir',
          type=str)
        wpa.add_argument('--new-hs', action='store_true',
          dest='ignore_old_handshakes',
          help=(Color.s('Captures new handshakes, ignores existing handshakes ' + 'in {C}%s{W} (default: {G}off{W})' % self.config.wpa_handshake_dir)))
        wpa.add_argument('--dict', action='store',
          dest='wordlist',
          metavar='[file]',
          type=str,
          help=(Color.s('File containing passwords for cracking (default: {G}%s{W})') % self.config.wordlist))
        wpa.add_argument('--wpadt', action='store',
        ...
        wpa.add_argument('-wpadt', help=(argparse.SUPPRESS), action='store', dest='wpa_deauth_timeout',
          type=int)
        wpa.add_argument('--wpat', action='store',
        ...
        wpa.add_argument('-wpat', help=(argparse.SUPPRESS), action='store', dest='wpa_attack_timeout',
          type=int)
        wpa.add_argument('-strip', help=(argparse.SUPPRESS), action='store_true', dest='wpa_strip_handshake')

    def _add_wps_args(self, wps):
        wps.add_argument('--wps', action='store_true',
          dest='wps_filter',
        ...
        wps.add_argument('--no-wps', action='store_true',
          dest='no_wps',
          help=(self._verbose('{O}Never{W} use {O}WPS PIN{W} & {O}Pixie-Dust{W}attacks on targets (default: {G}off{W})')))
        wps.add_argument('--wps-only', action='store_true',
          dest='wps_only',
          help=(Color.s('{O}Only{W} use {C}WPS PIN{W} & {C}Pixie-Dust{W} attacks (default: {G}off{W})')))
        wps.add_argument('--pixie', action='store_true', dest='wps_pixie', help=(self._verbose('{O}Only{W} use {C}WPS Pixie-Dust{W} attack (do not use {O}PIN attack{W})')))
        wps.add_argument('--no-pixie', action='store_true', dest='wps_no_pixie', help=(self._verbose('{O}Never{W} use {O}WPS Pixie-Dust{W} attack (use {G}PIN attack{W})')))
        wps.add_argument('--bully', action='store_true',
          dest='use_bully',
          help=(Color.s('Use {G}bully{W} program for WPS PIN & Pixie-Dust attacks (default: {G}reaver{W})')))
        wps.add_argument('-bully', help=(argparse.SUPPRESS), action='store_true', dest='use_bully')
        wps.add_argument('--ignore-locks', action='store_true', dest='wps_ignore_lock', help=(Color.s('Do {O}not{W} stop WPS PIN attack if AP becomes {O}locked{W}  (default: {G}stop{W})')))
        wps.add_argument('--wps-time', action='store',
        ...
        wps.add_argument('-wpst', help=(argparse.SUPPRESS), action='store', dest='wps_pixie_timeout',
          type=int)
        wps.add_argument('--wps-fails', action='store',
        ...
        wps.add_argument('-wpsf', help=(argparse.SUPPRESS), action='store', dest='wps_fail_threshold',
          type=int)
        wps.add_argument('--wps-timeouts', action='store',
        ...
        wps.add_argument('-wpsto', help=(argparse.SUPPRESS), action='store', dest='wps_timeout_threshold',
          type=int)

    def _add_pmkid_args(self, pmkid):
        pmkid.add_argument('--pmkid', action='store_true',
          dest='use_pmkid_only',
          help=(Color.s('{O}Only{W} use {C}PMKID capture{W}, avoids other WPS & WPA attacks (default: {G}off{W})')))
        pmkid.add_argument('-pmkid', help=(argparse.SUPPRESS), action='store_true', dest='use_pmkid_only')
        pmkid.add_argument('--pmkid-timeout', action='store',
          dest='pmkid_timeout',
          metavar='[sec]',
          type=int,
          help=(Color.s('Time to wait for PMKID capture ' + '(default: {G}%d{W} seconds)' % self.config.pmkid_timeout)))

    def _add_command_args(self, commands):
        commands.add_argument('--cracked', action='store_true',
          dest='cracked',
        ...
        commands.add_argument('--check', action='store',
          metavar='file',
          nargs='?',
          const='<all>',
          dest='check_handshake',
          help=(Color.s('Check a {C}.cap file{W} (or all {C}hs/*.cap{W} files) for WPA handshakes')))
        commands.add_argument('-check', help=(argparse.SUPPRESS), action='store', nargs='?',
          const='<all>',
          dest='check_handshake')
