...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/handlers/result.py
# Compiled at: 2020-01-20 06:28:48
# Size of source mod 2**32: 12853 bytes
import os, time
...
class CrackResult(object):
    __doc__ = ' Abstract class containing results from a crack session '
    ...
    def display(cls):
        ...
        results = sorted([cls.load(item) for item in cracked_targets], key=(lambda x: x.date),
          reverse=True)
    ...
    def load(json):
        ...
        if json['type'] == 'WPA':
            result = CrackResultWPA(json['bssid'], json['essid'], json['handshake_file'], json['key'])
        else:
            if json['type'] == 'WEP':
                result = CrackResultWEP(json['bssid'], json['essid'], json['hex_key'], json['ascii_key'])
            else:
                if json['type'] == 'WPS':
                    result = CrackResultWPS(json['bssid'], json['essid'], json['pin'], json['psk'])
                else:
                    if json['type'] == 'PMKID':
                        result = CrackResultPMKID(json['bssid'], json['essid'], json['pmkid_file'], json['key'])
