...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/config.py
# Compiled at: 2020-01-19 08:29:21
# Size of source mod 2**32: 20551 bytes
import os
from .handlers.color import Color
from .plugins.macchanger import Macchanger

class Configuration(object):
    __doc__ = ' Stores configuration variables and functions '
    version = '1.0.0'
    initialized = False
    temp_dir = None
    interface = None
    ...
    def initialize(cls, load_interface=True):
        ...
        cls.wpa_handshake_dir = 'wifihunter-workspace'
    ...
    def parse_wpa_args(cls, args):
        ...
        if args.wpa_filter:
            ...
            if args.wordlist:
                cls.wordlist = os.path.exists(args.wordlist) or None
                ...
                if os.path.isfile(args.wordlist):
                    ...
                    if os.path.isdir(args.wordlist):
                        cls.wordlist = None
    ...
    def parse_wps_args(cls, args):
        ...
        if args.wps_filter:
            ...
            if args.wps_only:
                cls.wps_only = True
                cls.wps_filter = True
                Color.pl('{+} {C}option:{W} will *only* attack WPS networks with {G}WPS attacks{W} (avoids handshake and PMKID)')
            else:
                if args.no_wps:
    ...
    def parse_wep_attacks(cls):
        ...
        if len(cls.wep_attacks) == 0:
            ...
            Color.pl('{+} {C}option:{W} using {G}%s{W} WEP attacks' % '{W}, {G}'.join(cls.wep_attacks))
    ...
    def exit_gracefully(cls, code=0):
        ...
        if cls.interface is not None:
            if Airmon.base_interface is not None:
    ...
    def dump(cls):
        ...
        for key, val in sorted(cls.__dict__.items()):
            if not key.startswith('__'):
                if type(val) in [classmethod, staticmethod] or val is None:
                    pass
                else:
