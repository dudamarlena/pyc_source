...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/src/plugins/airmon.py
# Compiled at: 2020-01-19 08:11:24
# Size of source mod 2**32: 15699 bytes
import re, os, signal
...
class Airmon(Dependency):
    __doc__ = " Wrapper around the 'airmon-ng' program "
    ...
    def start(iface):
        ...
        if enabled_iface is None:
            if driver in Airmon.BAD_DRIVERS:
    ...
    def _parse_airmon_start(airmon_output):
        ...
        enabled_re = re.compile('.*\\(mac80211 monitor mode (?:vif )?enabled (?:for [^ ]+ )?on (?:\\[\\w+\\])?(\\w+)\\)?.*')
    ...
    def stop(iface):
        ...
        if not disabled_iface:
            if iface in Airmon.BAD_DRIVERS:
                Color.p('{O}"bad driver" detected{W} ')
        ...
        return (
         disabled_iface, enabled_iface)

    @staticmethod
    def _parse_airmon_stop(airmon_output):
        """Find the interface taken out of into monitor mode (if any)"""
        disabled_re = re.compile('\\s*\\(mac80211 monitor mode (?:vif )?disabled for (?:\\[\\w+\\])?(\\w+)\\)\\s*')
        removed_re = re.compile('([a-zA-Z0-9]+).*\\(removed\\)')
        enabled_re = re.compile('\\s*\\(mac80211 station mode (?:vif )?enabled on (?:\\[\\w+\\])?(\\w+)\\)\\s*')
        disabled_iface = None
        enabled_iface = None
        for line in airmon_output.split('\n'):
    ...
    def terminate_conflicting_processes():
        ...
        pid_pnames = []
        pid_pname_re = re.compile('^\\s*(\\d+)\\s*([a-zA-Z0-9_\\-]+)\\s*$')
        for line in airmon_output.split('\n'):
        ...
        if not Configuration.kill_conflicting_processes:
            names_and_pids = ', '.join(['{R}%s{O} (PID {R}%s{O})' % (pname, pid) for pid, pname in pid_pnames])
            Color.pl('{!} {O}Conflicting processes: %s' % names_and_pids)
            Color.pl('{!} {O}If you have problems: {R}kill -9 PID{O} or re-run wifihunter with {R}--kill{O}){W}')
            return
