...
# Embedded file name: build/bdist.linux-x86_64/egg/couchable/core.py
# Compiled at: 2018-01-31 20:25:05
import logging
...
import base64, collections, copy, cPickle as pickle, cStringIO, datetime, gzip, hashlib, inspect, itertools, os, pprint, random, re, string, subprocess, sys, tempfile, time, traceback, uuid, weakref, couchdb, couchdb.client, couchdb.design, couchdb.http, couchdb.json, couchdb.multipart, requests
    ...
    def func(func_):
        ...
        def func__(self, parent_doc, data, *args, **kwargs):
            if id(data) in self._cycle_set:
                raise ValueError(('Object {} is cycle root: {!r}, {}').format(id(data), args[1], type(data)))
            try:
...
class CouchableDb(object):
    ...
    def __init__(self, url=None, db=None, exists=None, timeout=None):
        ...
        if db is not None:
            ...
            if exists or requests.head(self.url).status_code != 200:
    ...
    def _pack_object(self, parent_doc, data, attachment_dict, name, isKey, topLevel=False):
        ...
        if base_cls and not topLevel:
            if self._additiveOnly and getattr(data, '_id', None) is not None and getattr(data, '_rev', None) is not None and getattr(data, '_couchableMultipartPending', None) is None:
                pass
    ...
    def _pack_native(self, parent_doc, data, attachment_dict, name, isKey):
        ...
        if '\x00' in data or highBytes or len(data) > self._maxStrLen:
            return self._pack_pickle(parent_doc, data, attachment_dict, name, isKey)
        else:
    ...
    def _pack_consargs_keyAsKey(self, parent_doc, data, attachment_dict, name, isKey):
        ...
        if isKey:
            ...
            if isinstance(data, tuple) and type(data) != tuple and type(data).__new__ != tuple.__new__:
    ...
    def _pack_dict_keyMeansObject(self, parent_doc, data, attachment_dict, name, isObjDict, topLevel=False):
        ...
        if type(data) is not dict:
            assert not isObjDict, ('{}: {}').format(name, str(type(data)))
            return self._pack_object(parent_doc, data, attachment_dict, name, False)
        if isObjDict:
            nameFormat_str = '{}.{}'
        else:
            nameFormat_str = '{}[{}]'
        if topLevel:
            private_keys = {k for k in data.keys() if k.startswith('_') and k not in ('_id',
                                                                                      '_rev',
                                                                                      '_attachments',
                                                                                      '_cdb',
                                                                                      '_couchableMultipartPending') if k.startswith('_') and k not in ('_id',
                                                                                                                                                       '_rev',
                                                                                                                                                       '_attachments',
                                                                                                                                                       '_cdb',
                                                                                                                                                       '_couchableMultipartPending')}
        else:
            private_keys = set()
        doc = {}
        ...
        if topLevel and private_keys:
            ...
            parent_doc.setdefault(FIELD_NAME, {})
            parent_doc[FIELD_NAME]['private'] = {self._pack(parent_doc, k, attachment_dict, ('{}>{}').format(name, str(k)), True):self._pack(parent_doc, v, attachment_dict, ('{}.{}').format(name, str(k)), False) for k, v in data.items() if k in private_keys}
        return doc
        ...
        try:
            if isinstance(doc, (str, unicode)):
                if doc.startswith(FIELD_NAME):
                    ...
                    if method_str == 'module':
                        return importstr(data)
                    if method_str == 'pickle':
...
def registerDocType(type_, preStore_func=lambda obj, cdb: None, postLoad_func=lambda obj, cdb: None):
    ...
    """
    _couchable_types[type_] = (
     preStore_func, postLoad_func)
...
def doGzip(data, compresslevel=1):
    ...
    for offset in range(0, len(data), 1073741824):
        gz_file.write(data[offset:offset + 1073741824])
...
def doGunzip(data):
    ...
    while True:
        uncompressed_data = gz_file.read(1073741824)
...
def registerAttachmentType(type_, serialize_func=doPickle, deserialize_func=doUnpickle, content_type='application/octet-stream', gzip=True):
    """
    @type  type_: type
    ...
    if gzip:
        handler_tuple = (
         lambda data: doGzip(serialize_func(data)), lambda data: deserialize_func(doGunzip(data)), content_type)
    else:
        handler_tuple = (
         serialize_func, deserialize_func, content_type)
...
registerAttachmentType(CouchableAttachment, lambda obj: CouchableAttachment.pack(obj), lambda data: CouchableAttachment.unpack(data), 'application/octet-stream')
