...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/ldap/auth/group.py
# Compiled at: 2013-04-24 12:48:52
from persistent import Persistent
from zope.app.authentication.groupfolder import IGroupInformation
from zope.app.authentication.interfaces import IAuthenticatorPlugin, IQueriableAuthenticator, IQuerySchemaSearch, IPrincipalInfo, IAuthenticatedPrincipalCreated
...
from ztfy.ldap.auth.interfaces import ILDAPGroupsFolder, ILDAPGroupsMailInfo, ILDAPGroupSearchSchema, ILDAPGroupInfo
...
class LDAPGroupsFolder(Persistent, Contained):
    ...
    implements(IAuthenticatorPlugin, ILDAPGroupsFolder, ILDAPGroupsMailInfo, IQueriableAuthenticator, IQuerySchemaSearch)
    ...
    def principalInfo(self, id):
        ...
        if id.startswith(self.groupIdPrefix):
            ...
            res = self._search({auth.groupIdAttribute: id}, attrs=[
             auth.groupIdAttribute,
             self.titleAttribute,
             self.descriptionAttribute], queryType='group')
    ...
    def getGroupsForPrincipal(self, principal_id):
        ...
        if auth is None:
            ...
            try:
                ...
                res = self._search({auth.idAttribute: internal_id}, attrs=attrs, queryType='principal')
            ...
            if '%(dn)s' in value:
                ...
                value = value % locals()
                if self.queryMode == 'group':
                    res = self._search({attr: value}, attrs=[
    ...
    def getPrincipalsForGroup(self, group_id):
        ...
        if auth is None:
            ...
            if self.queryMode == 'group':
                res = self._search({auth.groupIdAttribute: internal_id}, attrs=[
                 self.groupPrincipalsAttribute], queryType='group')
                try:
                    _dn, entry = res[0]
                    for dn in entry[self.groupPrincipalsAttribute]:
                        res = self._search({'objectClass': '*'}, base=dn, attrs=[
                         auth.idAttribute], queryType='principal')
                        for _dn, entry in res:
                ...
                res = self._search({self.principalGroupsAttribute: internal_id}, attrs=[
                 auth.idAttribute], queryType='principal')
                for dn, entry in res:
                    principals.append(auth.principalIdPrefix + entry[auth.idAttribute][0])
    ...
    def _getGroupInfoFromEntry(self, id, entry):
        return LDAPGroupInformation(self, id, entry[self.titleAttribute][0], self.descriptionAttribute and entry.get(self.descriptionAttribute, [''])[0] or '')
            ...
            if plugin.cacheProxyName:
                ...
                if cache_proxy is not None:
                    ...
                    if cache is not None:
                        cached_groups = cache.query('%s::%s' % (LDAP_CACHE_NAMESPACE, name), principal_id, LDAP_CACHE_MARKER)
                        if cached_groups is not LDAP_CACHE_MARKER:
                            principal.groups.extend(cached_groups)
                            continue
            cached_groups = [ auth.prefix + p_id for p_id in plugin.getGroupsForPrincipal(principal.id)
                            ]
            if cache is not None:
