...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/toughbt/radius_benchmark.py
# Compiled at: 2016-03-17 10:06:09
...
def run():
    ...
    parser.add_argument('-auth', '--auth', action='store_true', default=False, dest='auth', help='Run radius auth test')
    parser.add_argument('-acct', '--acct', action='store_true', default=False, dest='acct', help='Run radius acct test')
    parser.add_argument('-s', '--server', type=str, default='127.0.0.1', dest='server', help='Radius server address')
    parser.add_argument('-P', '--port', type=int, default=0, dest='port', help='Radius server auth port or acct port')
    parser.add_argument('-e', '--secret', type=str, default='secret', dest='secret', help='Radius testing share secret')
    parser.add_argument('-u', '--username', type=str, default='', dest='username', help='Radius testing username')
    parser.add_argument('-p', '--password', type=str, default='', dest='password', help='Radius testing password')
    parser.add_argument('-n', '--requests', type=int, default=1, dest='requests', help='Number of requests to perform')
    parser.add_argument('-c', '--concurrency', type=int, default=10, dest='concurrency', help='Number of multiple requests to make at a time')
    parser.add_argument('-v', '--verbosity', action='store_true', default=False, dest='verbosity', help='How much troubleshooting info to print')
    parser.add_argument('-t', '--timeout', type=int, default=120, dest='timeout', help='Seconds to max. wait for all response')
    ...
    if args.auth:
        ...
        start_auth = lambda : auth_test(args.server, args.port, args.secret, args.requests, args.concurrency, args.username, args.password, args.verbosity, args.timeout)
