...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/skin/form.py
# Compiled at: 2014-10-10 05:17:44
__docformat__ = 'restructuredtext'
from z3c.form.interfaces import IActions, IButtonForm, ISubForm, IHandlerForm, IWidgets, DISPLAY_MODE
from z3c.json.interfaces import IJSONWriter
from z3c.language.switch.interfaces import II18n
from zope.container.interfaces import IContainer
from zope.contentprovider.interfaces import IContentProvider
from zope.dublincore.interfaces import IZopeDublinCore
from ztfy.baseskin.interfaces.form import IBaseForm, IGroupsBasedForm, IViewletsBasedForm, IForm, IWidgetsGroup, ISubFormViewlet, ICustomUpdateSubForm, IFormObjectCreatedEvent
from ztfy.skin.interfaces import IDialog, IEditFormButtons, IDialogAddFormButtons, IDialogDisplayFormButtons, IDialogEditFormButtons, IDialogTitle
...
class AjaxForm(ajax.AJAXRequestHandler):
    ...
    def getAjaxErrors(self):
        ...
        for error in self.errors:
            ...
            if hasattr(error, 'widget'):
                ...
                if widget is not None:
                    errors.setdefault('errors', []).append({'id': widget.id, 'widget': translate(widget.label, context=self.request), 
        ...
        return {'output': 'ERRORS', 'errors': errors}
...
class WidgetsGroup(object):
    ...
    def __init__(self, id, widgets=(), legend=None, help=None, cssClass='', switch=False, hide_if_empty=False, checkbox_switch=False, checkbox_on=False, checkbox_field=None):
        self.id = id
...
def NamedWidgetsGroup(id, widgets, names=(), legend=None, help=None, cssClass='', switch=False, hide_if_empty=False, checkbox_switch=False, checkbox_on=False, checkbox_field=None):
    """Create a widgets group based on widgets names"""
    return WidgetsGroup(id, [ widgets.get(name) for name in names ], legend, help, cssClass, switch, hide_if_empty, checkbox_switch, checkbox_on, checkbox_field)
...
class BaseAddForm(form.AddForm, GroupsBasedForm):
    ...
    def getForms(self, with_self=True):
        if not hasattr(self, 'subforms'):
            self.subforms = [ form for form in self.createSubForms() if form is not None ]
        if not hasattr(self, 'tabforms'):
            tabforms = self.tabforms = [ form for form in self.createTabForms() if form is not None ]
            if tabforms:
                jquery_tools.need()
        if with_self:
...
class BaseDialogAddForm(AjaxForm, BaseAddForm):
    ...
    def getOutput(self, writer, parent):
        if self.parent_view is not None:
            ...
            view.update()
            return '<textarea>%s</textarea>' % writer.write({'output': '<!-- OK -->\n' + view.output()})
        else:
...
class BaseDialogDisplayForm(AjaxForm, BaseDisplayForm):
    ...
    def updateActions(self):
        self.actions = getMultiAdapter((self, self.request, self.getContent()), IActions)
        self.actions.update()
