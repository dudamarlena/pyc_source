...
# [GCC 8.4.0]
# Embedded file name: /home/putzw/Documents/Projects/Source/jinjamator/jinjamator/plugins/content/cisco/aci.py
# Compiled at: 2020-04-10 09:12:01
# Size of source mod 2**32: 17763 bytes
import jinja2
...
def connect_apic(subscription_enabled=False):
    if not self._parent.configuration['apic_url']:
        ...
        if not self._parent.configuration['apic_username']:
            ...
            del (self._parent.configuration['apic_cert_name'] or self._parent.configuration)['apic_cert_name']
...
def is_dn_in_use(dn, ignore_children=False):
    ...
    if int(retval['totalCount']) != len(retval['imdata']):
        log.warning('ACI API bug detected -> totalCount is {0} but {1} objects retured -> fixing up'.format(retval['totalCount'], len(retval['imdata'])))
...
def version(node_id=1):
    ...
    data = query('/api/node/class/topology/pod-{0}/node-{1}/firmwareCtrlrRunning.json'.format(pod_id, node_id))
...
def get_access_aep_name_by_vlan_id(vlan_id, dn_filter='Access'):
    try:
        access_aep = query('/api/node/class/infraRsFuncToEpg.json?query-target-filter=and(wcard(infraRsFuncToEpg.dn,"{1}"),eq(infraRsFuncToEpg.encap,"vlan-{0}"),eq(infraRsFuncToEpg.mode,"untagged"))&order-by=infraRsFuncToEpg.dn'.format(vlan_id, dn_filter))['imdata'][0]['infraRsFuncToEpg']['attributes']
