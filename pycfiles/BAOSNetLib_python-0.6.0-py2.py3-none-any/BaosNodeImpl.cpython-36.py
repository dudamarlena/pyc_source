...
# [GCC 8.4.0]
# Embedded file name: ../../script/BaosNodeImpl.py
# Compiled at: 2020-05-05 11:11:49
# Size of source mod 2**32: 13486 bytes
...
class BaosNodeImpl(object):
    ...
    def publish_msg(self, topic, msg):
        ...
        sendbyte = bytes(frame_start, encoding='utf-8') + len(class_name).to_bytes(1, 'big') + bytes(frame_end, encoding='utf-8') + msg
        try:
            self.m_pub.send(sendbyte)
    ...
    def subscribeTopic_Msg(self, topic, msgcb):
        ...
        if self.m_connManager.findTopic(topic, endpoints) == False:
            msgCB = MsgCallBack()
            msgCB.msgCb = msgcb
            msgCB.msgSub = self.subscribeTopic_Msg
