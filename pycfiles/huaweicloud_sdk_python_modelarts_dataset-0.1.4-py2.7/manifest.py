...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/modelarts/manifest.py
# Compiled at: 2019-06-28 23:00:22
import json
from modelarts import field_name
from modelarts.field_name import prefix_text, label_separator, property_start_index, property_end_index, property_content, sound_classification, audio_classification
from modelarts.file_util import __is_local, save
from modelarts.file_util import __read
from obs import ObsClient

def get_sample_list(manifest_path, task_type, exactly_match_type=False, access_key=None, secret_key=None, end_point=None, usage=field_name.default_usage, ssl_verify=False, max_retry_count=3, timeout=60):
    """
  ...
  """
    data_set = parse_manifest(manifest_path, access_key=access_key, secret_key=secret_key, end_point=end_point, ssl_verify=ssl_verify, max_retry_count=max_retry_count, timeout=timeout)
    ...
    for sample in sample_list:
        ...
        if str(sample_usage).lower().__eq__(str(usage).lower()) or str(usage).lower().__eq__(field_name.default_usage):
            ...
            for annotation in annotations:
                ...
                if not exactly_match_type:
                    if str(type).endswith('/' + task_type):
                        flag = True
                        if task_type == field_name.image_classification or task_type == field_name.audio_classification or task_type == field_name.text_classification:
                            label_list.append(annotation.get_name())
                        elif task_type == field_name.text_entity:
                            annotation_property = annotation.get_property()
                            label_list.append(annotation.get_name() + label_separator + str(annotation_property[property_start_index]) + label_separator + str(annotation_property[property_end_index]))
                    ...
                    if type == task_type:
                        flag = True
                        if str(task_type).endswith('/' + field_name.image_classification) or str(task_type).endswith('/' + field_name.audio_classification) or str(task_type).endswith('/' + field_name.text_classification):
                            label_list.append(annotation.get_name())
            ...
            continue
        if str(task_type).endswith(field_name.text_classification) or str(task_type).endswith(field_name.text_entity):
...
def __getAnnotationsInternal(text):
    ...
    if annotations is not None:
        for annotation in annotations:
            ...
            annotation_creation_time = annotation.get(field_name.annotation_creation_time) or annotation.get(field_name.annotation_creation_time2)
            annotation_property = annotation.get(field_name.annotation_property)
            annotation_format = annotation.get(field_name.annotation_format) or annotation.get(field_name.annotation_format2)
            annotation_confidence = annotation.get(field_name.annotation_confidence)
            annotated_by = annotation.get(field_name.annotation_annotated_by) or annotation.get(field_name.annotation_annotated_by2)
            annotations_list.append(Annotation(name=annotation_name, type=annotation_type, loc=annotation_loc, property=annotation_property, confidence=annotation_confidence, creation_time=annotation_creation_time, annotated_by=annotated_by, annotation_format=annotation_format))
...
def __getDataSet(lines):
    ...
    for line in lines:
        if line != '':
            ...
            sample_list.append(Sample(source=source, usage=usage, annotations=annotations_list, inference_loc=inference_loc, id=id, source_type=source_type, source_property=source_property))
...
def parse_manifest(manifest_path, obs_client=None, access_key=None, secret_key=None, end_point=None, ssl_verify=False, max_retry_count=3, timeout=60):
    """
...
class DataSet(object):
  ...
  """
    ...
    def save(self, path, access_key=None, secret_key=None, end_point=None, saveMode='w', ssl_verify=False, max_retry_count=3, timeout=60):
        """
    ...
    """
        if access_key is None and secret_key is None and end_point is None:
            ...
            save(manifest_json, path, access_key=access_key, secret_key=secret_key, end_point=end_point, saveMode=saveMode, ssl_verify=ssl_verify, max_retry_count=max_retry_count, timeout=timeout)
        return


class Sample(object):

    def __init__(self, source, annotations=None, usage=None, inference_loc=None, id=None, source_type=None, source_property=None):
        self._source = source
...
class Annotation:
    ...
    def __init__(self, name=None, type=None, loc=None, property=None, confidence=None, creation_time=None, annotated_by=None, annotation_format=None, hard=None):
        self._name = name
        self._type = type
