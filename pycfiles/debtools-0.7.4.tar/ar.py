...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/DebTools/debtools/ar.py
# Compiled at: 2015-07-27 08:10:47
...
class ArFile(object):
    ...
    def __init__(self, name=None, mode=b'r', fileobj=None, bufsize=10240):
        self.name = name
        self.mode = mode[0]
        self.fileobj = fileobj
        self.bufsize = bufsize
        if fileobj is None:
            self.fileobj = open(name, self.mode + b'b')
        if self.mode == b'r':
    ...
    def next(self):
        ...
        if magic != b'`\n':
            ...
            ar_info = ArInfo(data[0:16].decode(b'utf-8').strip())
            ar_info.filename = data[0:16].decode(b'utf-8').strip()
    ...
    def extractall(self, path=b'.', members=None):
    ...
    def extract(self, member, path=b''):
        self.__reset()
