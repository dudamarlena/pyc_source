...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/DebTools/debtools/multideb.py
# Compiled at: 2015-07-28 01:47:46
...
def main():
    ...
    if config_parser.has_option('multideb', 'exclude'):
        excluded_packages = {x for x in config_parser.get('multideb', 'exclude').splitlines() if x.strip() if x.strip()}
    else:
    ...
    if only_packages:
        packages_to_create = {package_name:package_version for package_name, package_version in packages_to_create.items() if package_name in set(only_packages)}
    cwd = os.getcwd()
