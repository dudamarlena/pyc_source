...
# Compiled at: 2016-09-11 08:54:08
...
ubuntu_distribs = [
 'precise', 'precise-updates', 'trusty', 'trusty-updates', 'wily', 'wily-updates', 'xenial',
...
class EnvironmentBuilder(object):
    ...
    def get_available_package_version_in_url(self, base_url, debian_package):
        ...
        if title is None:
            return
        title = title.replace('\xa0', ' ').partition(' [')[0]
        matcher = re.match('^\\w+\\s*:[^(]*\\((\\d:|)(.*)-.*\\).*$', title)
        if not matcher:
            return
...
def main():
    parser = argparse.ArgumentParser(description='Build a Python virtual env using the versions also available as official Debian or Ubuntu packages')
    parser.add_argument('-u', '--URL', default=[], action='append', help='"wheezy", "xenial-updates", ... or any URL like https://packages.debian.org/stretch/. Known distributions: "%s", "%s"' % (
     '", "'.join(ubuntu_distribs), '", "'.join(debian_distribs)))
    parser.add_argument('-M', '--defaultmap', help='Use name mapping for well-known packages', action='store_true', default=False)
    parser.add_argument('-P', '--only-python-version', help='Only print the available Python version', action='store_true', default=False)
    parser.add_argument('-m', '--mapfile', help='mapping file between Python package names and Debian ones: each line is like "python-package-name=debian-package-name".Otherwise, use the default Debianized name ("python[3]-package-name"). Add"python-package-name=" to ignore this package', default=None)
    parser.add_argument('-p', '--python', help='Python version: "2" or "3" (default: "%s")' % sys.version_info[0], default=str(sys.version_info[0]))
    parser.add_argument('-r', '--requirements', help='Requirements file (otherwise use "pip list")', default=None)
