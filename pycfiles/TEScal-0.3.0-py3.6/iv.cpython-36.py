...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tescal/detcal/iv.py
    ...
    def calc_iv(self):
        ...
        for t in range(ntemps):
            for ch in range(nch):
                ...
                x, xcov = curve_fit(fitfunc, (self.vb[(t, ch, normalinds)]), (self.dites[(t, ch, normalinds)]), sigma=(self.dites_err[(t, ch, normalinds)]),
                  absolute_sigma=True)
                jac = np.zeros((2, 2))
                jac[(0, 0)] = 1
                jac[(1, 1)] = -1 / x[1] ** 2
