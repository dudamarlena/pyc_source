...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thevpncompany/consumer_orchestration.py
# Compiled at: 2020-01-06 23:51:25
...
if CONFIGURATION_REPOSITORY is None or REPOSITORY_PRIVATE_KEY is None:
    log.error('You need to provide CONFIGURATION_REPOSITORY and REPOSITORY_PRIVATE_KEY. It allows to access to the configuration of OpenVPN.')
    exit(1)
...
def process_single_server(request) -> bool:
    ...
    if request['supplier_id'] == 1:
        ...
        if not result:
            ...
            return False
        mac_id = mac.create_server(hardware='s-1vcpu-1gb', cookbook_tag='openvpn_28581', location=request['location_code'], servername=str(request['server_id']) + '-' + request['location_code'], provider='digitalocean2', repository_conf=CONFIGURATION_REPOSITORY, repository_private_key=REPOSITORY_PRIVATE_KEY)
        ...
        vpn.action_server_completed(server_id=str(request['server_id']), id_external=server['id'], ipv4=server['ipv4'])
        return True
