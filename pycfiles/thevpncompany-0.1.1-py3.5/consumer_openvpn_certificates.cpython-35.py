...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thevpncompany/consumer_openvpn_certificates.py
# Compiled at: 2020-01-06 23:48:36
# Size of source mod 2**32: 6844 bytes
"""
...
"""
import greenstalk, logging, os, json
...
if not os.path.exists('/var/thevpncompany/data/conf'):
    log.error('OpenVPN Server Configuration with the private key is expected at /var/thevpncompany/data/conf. This folder has not been found.')
    exit(1)
...
def main():
    ...
    try:
        with greenstalk.Client(host=VPN_QUEUE_SERVER, port=VPN_QUEUE_PORT, watch=VPN_QUEUE_TUBE, use=VPN_QUEUE_TUBE) as (queue):
        ...
        log.error('Unable to connect to ip %s:%s. Is beanstalkd the daemon running?' % (VPN_QUEUE_SERVER, VPN_QUEUE_PORT), exc_info=True)
...
def process_single_certificate(request) -> bool:
    ...
    cmd = 'docker run  -i -v /var/thevpncompany/data/conf:/etc/openvpn --rm ' + VPN_OPENVPN_CONTAINER + ' easyrsa build-client-full "' + cn + '" nopass '
    ...
    prepare_certs_response = execute('docker run -v /var/thevpncompany/data/conf:/etc/openvpn --rm kylemanna/openvpn ovpn_getclient ' + cn + ' separated')
    if prepare_certs_response.rc != 0:
        log.error('Error executing command, aborting certificate ' + cn)
        return False
    client_private_file_response = execute('docker run -v /var/thevpncompany/data/conf:/etc/openvpn --rm kylemanna/openvpn cat /etc/openvpn/clients/' + cn + '/' + cn + '.key')
    client_public_file_response = execute('docker run -v /var/thevpncompany/data/conf:/etc/openvpn --rm kylemanna/openvpn cat /etc/openvpn/clients/' + cn + '/' + cn + '.crt')
    ca_file_response = execute('docker run -v /var/thevpncompany/data/conf:/etc/openvpn --rm kylemanna/openvpn cat /etc/openvpn/clients/' + cn + '/ca.crt')
    static_key_file_response = execute('docker run -v /var/thevpncompany/data/conf:/etc/openvpn --rm kylemanna/openvpn cat /etc/openvpn/clients/' + cn + '/ta.key')
    save_cert_response = vpn.action_save_certificates(cn, client_private_file_response.stdout, client_public_file_response.stdout, ca_file_response.stdout, static_key_file_response.stdout)
