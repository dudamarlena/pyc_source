...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/archer/projects/views.py
# Compiled at: 2013-08-20 06:18:05
import logging, os
...
def index(request):
    debug_info = {'user': pformat(request.user.__dict__), 'environ': pformat(dict(os.environ.items())), 
       'meta': pformat(dict(request.META))}
    projects = get_objects_for_user(request.user, 'projects.view_project')
    package_sets = [ (project, request.user.has_perm('projects.upload_package', project), [ package for package in project.package_set.order_by('-id') ]) for project in projects
                   ]
...
class UploadView(View):
    ...
    def get(self, request, project_id):
        form = UploadFileForm()
        return render_to_response('projects/upload.html', {'form': form, 'project_id': project_id}, context_instance=RequestContext(request))
...
def deploy(request, project_id, path):
    ...
    if request.POST:
        ...
        if not request.POST['package'].isdigit():
            messages.add_message(request, messages.ERROR, 'package with id = "%s" does not exist!' % request.POST['package'])
        ...
        if package.can_deploy():
            if request.user.has_perm('packages.deploy_package', package):
                try:
                    ...
                    if deployed:
                        messages.add_message(request, messages.SUCCESS, 'Package "%s" successfully deployed in "%s" directory' % (package, path))
                    else:
                    ...
                    messages.add_message(request, messages.ERROR, 'Error while deploying a package "%s": "%s"' % (package, e))
        ...
        packages = [ package for package in projects_packages if request.user.has_perm('packages.deploy_package', package) and package.can_deploy()
                   ]
        return render_to_response('projects/deploy.html', {'packages': packages, 'project_id': project_id, 
           'path': path}, context_instance=RequestContext(request))


class Remove(ModifyDirectory):

    def _get(self, request, project, path, to_remove, form_class=RemoveDirectoryForm, template='projects/rmdir.html', text='directory'):
        form = form_class(to_remove)
        try:
            return render_to_response(template, {'project_id': project.id, 'form': form, 
               'path': path}, context_instance=RequestContext(request))
    ...
    def _post(self, request, project, path, to_remove, form_class=RemoveDirectoryForm, template='', text='directory'):
        ...
        try:
            ...
            messages.add_message(request, messages.SUCCESS, '%s "%s" successfully removed.' % (text.title(), to_remove))
        except (exceptions.ApplicationError, OSError) as e:
            messages.add_message(request, messages.ERROR, 'Could not delete %s "%s": %s' % (text, to_remove, e))
...
class RemoveFile(Remove):
    ...
    def get(self, request, project_id, path):
        ...
        return super(RemoveFile, self)._get(request, project, path, to_remove, form_class=RemoveFileForm, template='projects/rm.html', text='file')

    def post(self, request, project_id, path):
        project, to_remove = self.validate_file(project_id, path)
        return super(RemoveFile, self)._post(request, project, path, to_remove, form_class=RemoveFileForm, template='projects/rm.html', text='file')
...
class RemoveDirectory(Remove):
    ...
    def get(self, request, project_id, path):
        ...
        return super(RemoveDirectory, self)._get(request, project, path, to_remove, form_class=RemoveDirectoryForm, template='projects/rmdir.html', text='directory')

    def post(self, request, project_id, path):
        project, to_remove = self.validate_directory(project_id, path)
        return super(RemoveDirectory, self)._post(request, project, path, to_remove, form_class=RemoveDirectoryForm, template='projects/rmdir.html', text='directory')
...
class MakeDirectory(ModifyDirectory):
    ...
    def get(self, request, project_id, path):
        ...
        return render_to_response('projects/mkdir.html', {'project_id': project.id, 'form': form, 
           'path': path}, context_instance=RequestContext(request))
    ...
    def post(self, request, project_id, path):
        ...
        if form.is_valid():
            try:
                ...
                os.mkdir(dir_full_path, 493)
                messages.add_message(request, messages.SUCCESS, 'Directory "%s" successfully created.' % dir_full_path)
            except OSError as e:
                messages.add_message(request, messages.ERROR, 'Could not create directory "%s": %s' % (dir_full_path, e))

            return HttpResponseRedirect(reverse('projects:show', args=[project_id]))
        return render_to_response('projects/mkdir.html', {'form': form, 'path': path, 
           'project_id': project.id}, context_instance=RequestContext(request))
...
def show(request, project_id, path=''):
        ...
        def _index(root):
            ...
            for mfile in files:
                ...
                if os.path.isdir(t):
                    ...
                    yield loader.render_to_string('tree/_folder.html', {'file': mfile + '/', 'subfiles': _index(os.path.join(root, t)), 
            ...
            if len(files_only) > 0:
                ...
                if pre:
                    files_only = ('\n').join(files_only)
                yield loader.render_to_string('tree/_files.html', {'files': files_only, 'project_id': project_id, 
                   'path': relative_path, 
                   'pre': pre})
    ...
    return render_to_response('projects/show.html', {'project': project, 'can_upload': can_upload, 
       'can_deploy': can_deploy, 
       'packages': [ package for package in project.package_set.order_by('-id') ], 'files': index_maker(), 
       'path': path}, context_instance=RequestContext(request))