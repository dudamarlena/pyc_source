...
# [GCC 8.4.0]
# Embedded file name: /home/rnix/workspace/yafowil.demo/devsrc/yafowil.widget.dict/src/yafowil/widget/dict/widget.py
...
factory.register('dict_actions', edit_renderers=[
 actions_renderer])
factory.doc['blueprint']['dict_actions'] = UNSET
...
def dict_builder(widget, factory):
    ...
    if callable(key_label):
        key_label = key_label()
    row['key'] = factory('th', props={'structural': True, 
       'label': key_label})
    ...
    row['value'] = factory('th', props={'structural': True, 
       'label': value_label})
    if not widget.attrs['static']:
        row['actions'] = factory('th:dict_actions', props={'structural': True, 
           'add': True, 
           'class': 'actions'})
...
def dict_edit_renderer(widget, data):
    ...
    for key, val in value.items():
        ...
        row['value'] = factory('td:text', value=val, name='value', props={'td.class': 'value', 
           'text.class': attr_value('value_class', widget, data)})
        if not static:
            row['actions'] = factory('td:dict_actions', props={'add': True, 
               'remove': True, 
               'up': True, 
               'down': True, 
               'class': 'actions'})
...
def extract_static(data, basename):
    ...
    while True:
        ...
        if valuename in request:
            if index >= len(keys):
                message = _('invalid_number_static_values', default='Invalid number of static values')
...
def dict_display_renderer(widget, data):
    ...
    if key_label and value_label:
        head = ('{}: {}').format(data.tag.translate(key_label), data.tag.translate(value_label))
        head = data.tag('h5', head)
    return head + data.tag('dl', *values)


factory.register('dict', extractors=[
 dict_extractor], edit_renderers=[
 dict_edit_renderer, compound_renderer], display_renderers=[
 dict_display_renderer], builders=[
 dict_builder])
factory.doc['blueprint']['dict'] = 'Add-on widget `yafowil.widget.dict\n<http://github.com/bluedynamics/yafowil.widget.dict/>`_.\n'
...
factory.doc['props']['dict.table_class'] = 'CSS classes rendered on dict table.\n'
factory.defaults['dict.key_class'] = 'keyfield'
factory.doc['props']['dict.key_class'] = 'CSS classes rendered on key input fields.\n'
factory.defaults['dict.value_class'] = 'valuefield'
factory.doc['props']['dict.value_class'] = 'CSS classes rendered on value input fields.\n'
factory.defaults['dict.key_label'] = UNSET
factory.doc['props']['dict.key_label'] = 'Label for dict keys column.\n'
factory.defaults['dict.value_label'] = UNSET
factory.doc['props']['dict.value_label'] = 'Label for dict values column.\n'
factory.defaults['dict.head'] = {}
factory.doc['props']['dict.head'] = 'B/C Labels for dict keys and values columns. Expect a dict containing\n``key`` and ``value`` keys.\n'
factory.defaults['dict.static'] = False
factory.doc['props']['dict.static'] = 'Makes keys immutable.\n'