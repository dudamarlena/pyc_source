...
# [GCC 8.4.0]
# Embedded file name: /home/paulina/pangtree/pangtreebuild/tests/tests_serialization/tests_to_fasta.py
...
class ToFASTATests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        poagraph_sequences = {msa.SequenceID('seq0'): graph.Sequence(msa.SequenceID('seq0'), [
         ...
         msa.SequenceID('seq2'): graph.Sequence(msa.SequenceID('seq2'), [
                                  ...
                                  graph.SeqPath([*map(nid, [14, 17])])], graph.SequenceMetadata({'group': '1'})), 
         
         msa.SequenceID('seq3'): graph.Sequence(msa.SequenceID('seq3'), [
                                  graph.SeqPath([*map(nid, [11])]),
                                  graph.SeqPath([*map(nid, [13, 14, 15])])], graph.SequenceMetadata({'group': '1'}))}
    ...
    def test_2_consensuses_tree_fasta(self):
        ...
        affinity_tree.nodes = [
         tree.AffinityNode(id_=(tree.AffinityNodeID(0)), parent=(tree.AffinityNodeID(-1)),
           children=[
          tree.AffinityNodeID(1), tree.AffinityNodeID(2)],
           sequences=[
          msa.SequenceID('seq0'),
          msa.SequenceID('seq1'),
          msa.SequenceID('seq2'),
         ...
         tree.AffinityNode(id_=(tree.AffinityNodeID(1)), parent=(tree.AffinityNodeID(0)),
           sequences=[
          msa.SequenceID('seq0'),
          msa.SequenceID('seq1'),
          msa.SequenceID('seq2')],
