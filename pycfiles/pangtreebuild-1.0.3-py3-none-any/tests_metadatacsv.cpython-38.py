...
# [GCC 8.4.0]
# Embedded file name: /home/paulina/pangtree/pangtreebuild/tests/tests_pangenome/input_types/tests_metadatacsv.py
...
def sid(x):
    return msa.SequenceID(x)


def sm(x):
    return graph.SequenceMetadata(x)
...
class MetadataCSVTests(unittest.TestCase):
    ...
    def test_2_no_seqid(self):
        ...
        self.assertEqual("No 'seqid' column in metadata csv.", str(err.exception))
    ...
    def test_3_empty_file(self):
        ...
        with self.assertRaises(Exception) as (err):
            _ = msa.MetadataCSV(csv_content, csv_path)
        self.assertEqual('Empty csv file.', str(err.exception))
    ...
    def test_6_incorrect_commas_number(self):
        ...
        with self.assertRaises(Exception) as (err):
            _ = msa.MetadataCSV(csv_content, csv_path)
        self.assertEqual('CSV metadata error. Different number of columns in line 0 than in header line.', str(err.exception))
    ...
    def test_10_metadata_feed_to_alignment_from_csv(self, test_name, maf_name, csv_name, po_name, expected_metadata):
        ...
        poagraph, _ = builder.build_from_dagmaf(msa.Maf(pathtools.get_file_content_stringio(maf_path), maf_path), self.fasta_provider, msa.MetadataCSV(pathtools.get_file_content_stringio(csv_path), csv_path))
        actual_metadata = {seq.seqmetadata:seq_id for seq_id, seq in poagraph.sequences.items()}
        self.assertEqual(expected_metadata, actual_metadata)
        poagraph = builder.build_from_maf(msa.Maf(pathtools.get_file_content_stringio(maf_path), maf_path), msa.MetadataCSV(pathtools.get_file_content_stringio(csv_path), csv_path))
        actual_metadata = {seq.seqmetadata:seq_id for seq_id, seq in poagraph.sequences.items()}
        self.assertEqual(expected_metadata, actual_metadata)
        poagraph = builder.build_from_po(msa.Po(pathtools.get_file_content_stringio(po_path), maf_path), msa.MetadataCSV(pathtools.get_file_content_stringio(csv_path), csv_path))
        actual_metadata = {seq.seqmetadata:seq_id for seq_id, seq in poagraph.sequences.items()}
        self.assertEqual(expected_metadata, actual_metadata)
