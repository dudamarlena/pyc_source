...
# [GCC 8.4.0]
# Embedded file name: c:\users\павел\pycharmprojects\segmentation_training_pipeline\build\lib\segmentation_pipeline\segmentation.py
# Compiled at: 2019-09-16 11:13:06
# Size of source mod 2**32: 11571 bytes
import imgaug, segmentation_models, numpy as np, tqdm, classification_models
from segmentation_models.utils import set_trainable
import keras
from segmentation_models import backbones
from musket_core import configloader, datasets
import os, musket_core.losses
from musket_core.datasets import DataSet, WriteableDataSet, DirectWriteableDS, CompressibleWriteableDS
import imageio, inspect
...
custom_models = {'DeepLabV3': dlm.Deeplabv3}
    ...
    def createNet(self):
        ...
        if self.architecture in custom_models:
            ...
            if self.architecture in configloader.load('layers').catalog:
                ...
                if self.architecture not in dir(segmentation_models):
        ...
        if self.backbone.lower() not in classification_models.Classifiers.names():
            print('Unknown backbone:' + self.backbone)
            print('Known backbones:', classification_models.Classifiers.names())
            raise ValueError('Unknown backbone')
        t = configloader.loaded['segmentation'].catalog['PipelineConfig']
        ...
        for arg in self.all:
            pynama = t.alias(arg)
            if arg not in r and pynama in sig.parameters:
                cleaned[pynama] = self.all[arg]

        self.clean(cleaned)
        if self.crops is not None:
            cleaned['input_shape'] = (
             cleaned['input_shape'][0] // self.crops, cleaned['input_shape'][1] // self.crops, cleaned['input_shape'][2])
            ...
            try:
                for f in test_g():
                    ...
                    for i in range(0, len(z)):
                        ...
                        augmented = imgaug.augmenters.Scale({'height':orig.shape[0], 
                         'width':orig.shape[1]}).augment_segmentation_maps(segmentation_maps_aug)
                        ids.append(id)
                        augs = augs + augmented
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in iter():
            ...
            if self.train:
                ...
                self.drawingFunction(i, os.path.join(dr, 't_epoch_' + str(epoch) + '.' + str(num) + '.jpg'))
            num = num + 1
