...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/mpgutils/utils.py
# Compiled at: 2009-07-19 21:41:58
"""LOTS OF CONSTANTS, MMMM KAY?"""
import os, fileinput, glob, numpy, subprocess, sys, time, re
LIB_DIR = 'LIB_DIR'
CHIP_TYPE = 'CHIP_TYPE'
...
V6 = 'V6'
iNUM_CHROMOSOMES = 24
dctV5Defaults = {LIB_DIR: '/humgen/affy_info/GAPProduction/', 
   CHIP_TYPE: 'BI_SNP', 
...
dctV6Defaults = {EXEC_DIR: '/fg/software/Affymetrix/1chip/', 
...
def raiseExceptionWithFileInput(fileInput, strFileType, strMessage):
    raise Exception('Error reading ' + strFileType + " '" + fileInput.filename() + "' at line " + str(fileInput.lineno()) + ': ' + strMessage + '.')
...
def repmat(mat, rowMultiplier, colMultiplier):
    if hasattr(numpy, 'repmat'):
        ...
        return numpy.tile(mat, (rowMultiplier, colMultiplier))
...
def loadSpecialProbes(dctRet, strPath):
    ...
    for strLine in fIn:
        ...
        dctRet[lstFields[0]] = (
         int(lstFields[3]), int(lstFields[2]))
...
def load_tfam(strTfamPath, lstIndicesToLoad=None):
    ...
    for (iIndex, strLine) in enumerate(fIn):
        ...
        tupIndividual = (
         lstFields[iTFAM_FAMILY_ID_INDEX], lstFields[iTFAM_INDIVIDUAL_ID_INDEX])
    ...
    for tupIndividual in lstRet:
        ...
        if tupIndividual[iTFAM_PATERNAL_ID_INDEX] is not None:
            iPaternalIndex = dctIndividualMap.get((tupIndividual[iTFAM_FAMILY_ID_INDEX],
             tupIndividual[iTFAM_PATERNAL_ID_INDEX]), None)
        iMaternalIndex = None
        if tupIndividual[iTFAM_MATERNAL_ID_INDEX] is not None:
            iMaternalIndex = dctIndividualMap.get((tupIndividual[iTFAM_FAMILY_ID_INDEX],
             tupIndividual[iTFAM_MATERNAL_ID_INDEX]), None)
...
def iterateProbeLocus(strProbeLocusPath):
    ...
    for lstFields in iterateWhitespaceDelimitedFile(strProbeLocusPath, iNumFieldsExpected=3):
        if lstFields[1] == 'MT':
            continue
        yield [
         lstFields[0], convertChromosomeStr(lstFields[1]), int(lstFields[2])]
