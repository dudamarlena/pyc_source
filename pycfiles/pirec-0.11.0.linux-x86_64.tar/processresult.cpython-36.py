...
# [GCC 8.4.0]
# Embedded file name: /home/jstutters/.virtualenvs/pirec/lib/python3.6/site-packages/pirec/processresult.py
# Compiled at: 2017-11-07 07:45:36
# Size of source mod 2**32: 13803 bytes
"""Main pirec module containing the Pipeline class and function recording methods."""
from __future__ import print_function
from collections import Mapping
import datetime, json, os, os.path, shutil
from subprocess import check_output, STDOUT, CalledProcessError
import tarfile, tempfile, traceback, wrapt, pirec.environment, pirec.artefacts

class Pipeline(object):
    __doc__ = 'Main class managing the recording of a processing pipeline.'
    ...
    def run(self, name, pipeline_func, base_dir, *inputs, **kwargs):
        ...
        if sentry is not None:
            sentry.user_context({'pipeline_name':self.name, 
        ...
        if results is not None:
            if not hasattr(results, '__iter__'):
                ...
                if len(result_names) != len(results):
                    raise ValueError
            self.results = dict(zip(result_names, results))
    ...
    def save(self, exception=None, report_name='report.json'):
        ...
        basename = self.filename.format(metadata=(self.metadata),
          name=(self.name),
...
class OutputRecorder(object):
    __doc__ = 'Holds commands used via the call function and their resulting output.'
    ...
    def process_recorder(wrapped, _, args, kwargs):
        ...
        try:
            ...
            result = ProcessOutput(func=wrapped, 
             ...
             exception=exception, 
             started=started, 
             finished=finished, **named_returns)
...
class ProcessOutput(Mapping):
    ...
    def as_dict(self):
        ...
        d = {'function':self.function.__name__, 
