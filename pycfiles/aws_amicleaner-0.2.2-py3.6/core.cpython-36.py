...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/amicleaner/core.py
# Compiled at: 2018-05-21 06:07:00
# Size of source mod 2**32: 7860 bytes
...
class OrphanSnapshotCleaner(object):
    __doc__ = ' Finds and removes ebs snapshots left orphaned '
    ...
    def fetch(self):
        ...
        used_snaps = [ebs.get('Ebs', {}).get('SnapshotId') for image in resp.get('Images') for ebs in image.get('BlockDeviceMappings')]
...
class AMICleaner(object):
    ...
    def remove_amis_from_ids(self, ami_ids):
        ...
        if not ami_ids:
            ...
            my_custom_images = self.ec2.describe_images(Owners=[
             'self'],
              ImageIds=ami_ids)
            amis = []
        ...
        if not candidates_amis:
            ...
            for ami in candidates_amis:
                if mapping_strategy.get('key') == 'name':
                    ...
                    if mapping_strategy.get('key') == 'tags':
                        mapping_value = self.tags_values_to_string(ami.tags, mapping_strategy.get('values'))
        ...
        if tags is None:
            ...
            for tag in tags:
                if not filters:
                    ...
                    if tag.key in filters_to_string:
                        tag_values.append(tag.value)
