...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/amicleaner/utils.py
# Compiled at: 2018-05-21 06:07:00
...
class Printer(object):
    __doc__ = ' Pretty table prints methods '
...
def parse_args(args):
    parser = argparse.ArgumentParser(description='Clean your AMIs on your AWS account. Your AWS credentials must be sourced')
    parser.add_argument('-v', '--version', dest='version',
      action='store_true',
      help='Prints version and exits')
    parser.add_argument('--from-ids', dest='from_ids',
      nargs='+',
      help='AMI id(s) you simply want to remove')
    parser.add_argument('--full-report', dest='full_report',
      action='store_true',
      help='Prints a full report of what to be cleaned')
    parser.add_argument('--mapping-key', dest='mapping_key',
      help='How to regroup AMIs : [name|tags]')
    parser.add_argument('--mapping-values', dest='mapping_values',
      nargs='+',
      help='List of values for tags or name')
    parser.add_argument('--excluded-mapping-values', dest='excluded_mapping_values',
      nargs='+',
      help='List of values to be excluded from tags')
    parser.add_argument('--keep-previous', dest='keep_previous',
      type=int,
      default=KEEP_PREVIOUS,
      help='Number of previous AMI to keep excluding those currently being running')
    parser.add_argument('-f', '--force-delete', dest='force_delete',
      action='store_true',
      help='Skip confirmation')
    parser.add_argument('--check-orphans', dest='check_orphans',
      action='store_true',
      help='Check and clean orphaned snapshots')
    parser.add_argument('--ami-min-days', dest='ami_min_days',
      type=int,
      default=AMI_MIN_DAYS,
      help='Number of days AMI to keep excluding those currently being running')
