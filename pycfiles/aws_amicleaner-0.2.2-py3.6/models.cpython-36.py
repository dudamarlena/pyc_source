...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/amicleaner/resources/models.py
# Compiled at: 2018-05-21 06:07:00
# Size of source mod 2**32: 4921 bytes
...
class AMI(object):
    ...
    def object_with_json(json):
        if json is None:
            ...
            ebs_snapshots = [AWSBlockDevice.object_with_json(block_device) for block_device in json.get('BlockDeviceMappings', [])]
            o.block_device_mappings = [f for f in ebs_snapshots if f]
            return o

    def __repr__(self):
        return '{0}: {1} {2}'.format(self.__class__.__name__, self.id, self.creation_date)
...
class AWSBlockDevice(object):
    ...
    def __str__(self):
        return str({'device_name':self.device_name, 
