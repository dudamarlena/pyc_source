...
# Embedded file name: build/bdist.linux-i686/egg/cspace/hgsite/site.py
# Compiled at: 2013-06-09 10:57:05
__doc__ = "'site' web command handling\nCopyright (C) 2012 Remy Blank\n"
import re, sys, time
from urlparse import urlunparse
from mercurial import config, error, revset, util
from mercurial.hgweb import common, webcommands, webutil
from .util import exists_in_wc, find_line_no, get_ext, guess_mimetype, hex_node, href_builder, http_date
webcmd = 'hgsite'
node_prefix = '~'
...
class Config(config.config):
    ...
    def ext_search_order(self):
        ...
        return dict((ext, i) for i, ext in enumerate(ext.strip() for ext in value.split(',')))
    ...
    def render_info(self):
        ...
        return [ (patterns[name], config[name].pop('renderer', 'static'), config[name]) for name in sorted(patterns) ]
...
class Request(object):
    ...
    def send(self, content, mimetype=None, encoding=None, headers=None, status=None):
...
def render_static(req, path, name, config, **kwargs):
    ...
    mimetype = guess_mimetype(config.get('mimetype'), path, 'application/octet-stream')
    if req.url_node:
        headers = [
         ('Cache-Control', 'public'),
         (
          'Expires', http_date(time.time() + 31536000))]
    else:
        headers = cache_must_revalidate
    return req.send(req.ctx[path].data(), mimetype=mimetype, encoding=config.get('encoding'), headers=headers)
...
def render(req, url_path, kwargs=None):
    ...
    def match_key(p):
        ext = get_ext(p)
        return (
         not p == path, ext_order.get(ext, sys.maxint), ext)

    match = req.ctx.match(['re:%s(?:\\.[^/]*)?$' % re.escape(path)])
    try:
    ...
    if renderer is None or not file_exists and getattr(renderer, 'file_must_exist', True):
...
def prepare_request(req, node):
    ...
    def build_hist_href(node, *args, **kwargs):
        ...
        return req.repo_href((node_prefix + node if node is not None else None), *args, **kwargs)
...
def handle_default_webcmd(web, req, tmpl):
    ...
    try:
        ...
        if not (prepare_request(req, None) and req.cfg.error_page):
            raise
        req.status = exc_info[1].code if isinstance(exc_info[1], common.ErrorResponse) else common.HTTP_SERVER_ERROR
...
def wc_filectx(orig, repo, req, *args, **kwargs):
    ...
    if req.form['cmd'][0] != 'file' or req.form.get('node', ['']) != [node_prefix] or not repo.ui.configbool('hgsite', 'allow_wc', False):
        ...
        try:
            ...
            if fctx.node() is None:
                if not exists_in_wc(repo, fctx.path()):
                    raise error.LookupError(node_prefix, fctx.path(), 'not found')
