...
# Embedded file name: build/bdist.linux-i686/egg/cspace/hgsite/genshi.py
# Compiled at: 2013-06-09 10:57:05
__doc__ = 'Genshi rendering\nCopyright (C) 2012 Remy Blank\n'
from __future__ import absolute_import
import os.path, weakref
try:
    from genshi import QName, Stream
    from genshi.core import Attrs, END, START, TEXT
    from genshi.template import Context, MarkupTemplate, NewTextTemplate, TemplateLoader
...
def get_template_loader(repo):
    ...
    def load_template(filename):
        ...
        return (
         filename, filename, fctx.data(), uptodate)

    loader = _loaders[repo] = Loader(load_template, auto_reload=True, max_cache_size=100, default_encoding='utf-8', variable_lookup='strict')
    return loader
...
invalid_control_chars = ('').join(chr(i) for i in range(32) if i not in (9, 10, 13))
...
def render_markup_template(req, path, name, config, **kwargs):
    ...
    template = loader.load('%s@%s' % (path, hex_node(req.ctx)), cls=MarkupTemplate)
    ...
    return req.send(text, mimetype=mimetype, encoding=encoding, headers=cache_must_revalidate)
...
def render_text_template(req, path, name, config, **kwargs):
    ...
    encoding = config.get('encoding', 'utf-8')
    loader = get_template_loader(req.repo)
    template = loader.load('%s@%s' % (path, hex_node(req.ctx)), cls=NewTextTemplate)
...
def render_pygments_css(req, path, name, config, **kwargs):
    ...
    return req.send(content, mimetype='text/css', encoding='utf-8', headers=cache_must_revalidate)
...
def generate_highlight_markup(tokens):
    ...
    for cls, text_chunk in _group_by_class(tokens):
        if cls:
            yield (
             START, (span, Attrs([(class_, cls)])), pos)
...
def highlight(text, lexer, tabsize=0):
    ...
    if has_pygments:
        try:
            lexer = get_lexer_by_name(lexer, stripnl=False, ensurenl=False, tabsize=tabsize)
