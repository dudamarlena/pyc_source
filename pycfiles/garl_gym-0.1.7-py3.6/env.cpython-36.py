...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/garl_gym/scenarios/env.py
# Compiled at: 2019-05-27 14:32:25
# Size of source mod 2**32: 18045 bytes
...
class SimplePopulationDynamics(BaseEnv):
        ...
        for predator in self.predators.values():
            ...
            if len(agent_indice[0]) == 0:
                ...
                for local_x, local_y in zip(agent_indice[0], agent_indice[1]):
                    ...
                    if candidate_agent.predator and np.random.rand() < crossover_prob:
                        child = Agent()
    ...
    def render(self):
        ...
        pool.close()
        batch_obs = []
        for i in range(int(np.ceil(1.0 * len(self.agents) / self.batch_size))):
