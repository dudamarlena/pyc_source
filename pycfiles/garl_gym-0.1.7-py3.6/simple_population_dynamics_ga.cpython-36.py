...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/garl_gym/scenarios/simple_population_dynamics_ga.py
# Compiled at: 2019-06-10 11:29:04
# Size of source mod 2**32: 26191 bytes
...
class SimplePopulationDynamicsGA(BaseEnv):
            ...
            if not len(agent_indice[0]) == 0:
                if predator.crossover:
                    ...
                    for local_x, local_y in zip(agent_indice[0], agent_indice[1]):
                        ...
                        if candidate_agent.predator and not candidate_agent.crossover and predator.id != candidate_agent.id and np.random.rand() < crossover_rate:
    ...
    def crossover_prey(self, crossover_scope=3, crossover_rate=0.001):
        ...
        for prey in list(self.preys.values()):
            ...
            for local_x, local_y in zip(agent_indice[0], agent_indice[1]):
                candidate_id = local_map[(local_x, local_y)]
                candidate_agent = self.agents[candidate_id]
                if not candidate_agent.predator and not candidate_agent.crossover and candidate_agent.id != prey.id and np.random.rand() < crossover_rate:
                    candidate_agent.crossover = True
                    prey.crossover = True
                    child = Agent()
                    child.id = self.max_id
    ...
    def get_predator_reward(self, agent):
        ...
        return (
         (
          agent.id, reward), (agent.id, killed_id))
