...
# [GCC 8.4.0]
# Embedded file name: /Users/apryor/projects/flaskerize/flaskerize/parser.py
# Compiled at: 2019-09-23 08:06:38
...
class FzArgumentParser(argparse.ArgumentParser):
    __doc__ = 'Flaskerize argument parser with default common options'

    def __init__(self, schema=None, xtra_schema_files=None):
        import json
        super().__init__()
        cfgs = []
        if schema:
        ...
        for cfg in cfgs:
            for option in cfg['options']:
                switches = [
                 option.pop('arg')] + option.pop('aliases', [])
...
class Flaskerize(object):
    ...
    def __init__(self, args):
        ...
        parser = FzArgumentParser(os.path.join(os.path.dirname(__file__), 'global/schema.json'))
    ...
    def bundle(self, args):
        ...
        arg_parser.add_argument('--output-file',
          '-o', type=str, help='Name of output file')
        arg_parser.add_argument('--source',
          '--from', type=str, help='Path of input static site to bundle')
        arg_parser.add_argument('--to',
          type=str,
          required=True,
          help='Flask app factory function to attach blueprint')
        arg_parser.add_argument('--with-wsgi',
          action='store_true',
          help='Also generate a wsgi.py for gunicorn')
        arg_parser.add_argument('--with-dockerfile',
          action='store_true', help='Also generate a Dockerfile')
        arg_parser.add_argument('--dry-run',
          action='store_true',
          help="Dry run -- don't actually create any files.")
        ...
        if parsed.source:
            if not parsed.source.endswith('/'):
                print(f"Input source {parsed.source} does not end with trailing /, adding for you")
                parsed.source += '/'
    ...
    def generate(self, args):
        import os
        arg_parser = FzArgumentParser(schema=(os.path.join(os.path.dirname(__file__), 'global/generate.json')))
        ...
        self._check_render_schematic(schematic,
    ...
    def _split_pkg_schematic(self, pkg_schematic: str, delim: str=':') -> Tuple[(str, str)]:
        if delim not in pkg_schematic:
            pkg = 'flaskerize'
            schematic = pkg_schematic
        else:
            pkg, _, schematic = pkg_schematic.rpartition(delim)
            raise pkg and schematic or ValueError(f"Unable to parse schematic '{pkg_schematic}.'Correct syntax is <package_name>:<schematic_name>")
        return (
         pkg, schematic)
    ...
    def _check_get_schematic_dirname(self, pkg_path: str) -> str:
        ...
        if not path.isdir(schematic_dirname):
            raise ValueError(f"Unable to locate directory 'schematics/' in path {schematic_dirname}")
    ...
    def _check_get_schematic_path(self, schematic_dirname: str, schematic: str) -> str:
        ...
        if not path.isdir(schematic_path):
            raise ValueError(f"Unable to locate schematic '{schematic}' in path {schematic_path}")
    ...
    def _check_render_schematic(self, pkg_schematic: str, render_dirname: str, src_path: str, name: str, args: List[Any], dry_run: bool=False, delim: str=':') -> None:
        from os import path
        from flaskerize import generate
        ...
        self.render_schematic(schematic_path,
          render_dirname=render_dirname,
          src_path=src_path,
          name=name,
          dry_run=dry_run,
    ...
    def render_schematic(self, schematic_path: str, render_dirname: str, name: str, args: List[Any], src_path: str='.', dry_run: bool=False) -> None:
        from flaskerize.render import SchematicRenderer
        SchematicRenderer(schematic_path,
          src_path=src_path,
          output_prefix=render_dirname,
          dry_run=dry_run).render(name, args)
