...
# [GCC 8.4.0]
# Embedded file name: /Users/apryor/projects/flaskerize/flaskerize/render.py
# Compiled at: 2019-09-08 08:53:32
# Size of source mod 2**32: 8572 bytes
import os, argparse
...
class SchematicRenderer:
    ...
    def __init__(self, schematic_path: str, src_path: str='.', output_prefix: str='', dry_run: bool=False):
        from jinja2 import Environment
        from flaskerize.fileio import StagedFileSystem
        self.src_path = src_path
        self.output_prefix = output_prefix
        self.schematic_path = schematic_path
        self.schematic_files_path = os.path.join(self.schematic_path, self.DEFAULT_FILES_DIRNAME)
        self.schema_path = self._get_schema_path()
        self._load_schema()
        self.arg_parser = self._check_get_arg_parser()
        self.env = Environment()
        self.fs = StagedFileSystem(src_path=(self.src_path),
    ...
    def _check_get_arg_parser(self, schema_path: Optional[str]=None) -> FzArgumentParser:
    ...
    def copy_from_sch(self, src_path: str, dst_path: str=None) -> None:
        ...
        return fs.copy.copy_file(self.sch_fs, src_path, self.fs.render_fs, dst_path or src_path)
    ...
    def get_template_files(self) -> List[str]:
        ...
        for pattern in patterns:
            filenames.extend([str(p) for p in Path(self.schematic_files_path).glob(pattern)])
    ...
    def _get_ignore_files(self) -> List[str]:
        ...
        for pattern in ignore_patterns:
            ignore_filenames.extend([str(p) for p in Path(self.schematic_path).glob(pattern)])

        return ignore_filenames

    def _generate_outfile(self, template_file: str, root: str, context: Optional[Dict]=None) -> str:
    ...
    def render(self, name: str, args: List[Any]) -> None:
        ...
        if 'name' in context:
            raise ValueError(f"Collision between Flaskerize-reserved parameter 'name' and parameter found in schema.json corresponding to {self.schematic_path}")
        context = {**context, **{'name': name}}
        self._load_custom_functions(path=(os.path.join(self.schematic_path, 'custom_functions.py')))
        try:
            run = self._load_run_function(path=(os.path.join(self.schematic_path, 'run.py')))
