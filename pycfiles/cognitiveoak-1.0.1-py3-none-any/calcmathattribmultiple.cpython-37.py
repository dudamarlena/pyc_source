...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\calcmathattribmultiple.py
...
class calcmathattribmultiple(object):
    ...
    def clickBtnCalcMathAttribMultiple(self):
        ...
        if len(_property1) < 1:
            vis_msg.print('ERROR in CalcMathAttribMultiple: No property 1 selected for attribute analysis', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Math Attribute between Properties', 'No property 1 selected for attribute analysis')
            return
        _property2 = self.lwgproperty2.selectedItems()
        if len(_property2) < 1:
            vis_msg.print('ERROR in CalcMathAttribMultiple: No property 2 selected for attribute analysis', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Math Attribute between Properties', 'No property 2 selected for attribute analysis')
            return
        if len(self.ldtname.text()) < 1:
            vis_msg.print('ERROR in CalcMathAttribMultiple: No name specified for output attribute', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Math Attribute between Properties', 'No name specified for output attribute')
            return
        if self.ldtname.text() in self.seisdata.keys():
            if self.checkSeisData(self.ldtname.text()):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Calculate Math Attribute between Properties', self.ldtname.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        self.refreshLwgProperty(self.lwgproperty1)
        self.refreshLwgProperty(self.lwgproperty2)
        QtWidgets.QMessageBox.information(self.msgbox, 'Calculate Math Attribute between Properties', 'Math attribute ' + self.mathattriblist[self.cbbattrib.currentIndex()] + ' calculated successfully')
