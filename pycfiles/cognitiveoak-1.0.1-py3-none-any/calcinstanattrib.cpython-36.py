...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\calcinstanattrib.py
...
class calcinstanattrib(object):
    ...
    dialog = None
    instanattriblist = [
     'Envelop', 'Quadrature', 'Phase', 'Frequency', 'Cosine of Phase']
    ...
    def clickBtnCalcInstanAttrib(self):
        ...
        if len(_propertylist) < 1:
            ...
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Instantaneous Attribute', 'No property selected for attribute analysis')
            return
        if len(self.ldtname.text()) < 1:
            vis_msg.print('Error in CalcInstanAttrib: No name specified for output attribute', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Calculate Instantaneous Attribute', 'No name specified for output attribute')
            return
        if self.ldtname.text() in self.seisdata.keys():
            if self.checkSeisData(self.ldtname.text()):
                reply = QtWidgets.QMessageBox.question(self.msgbox, 'Calculate Instantaneous Attribute', self.ldtname.text() + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
        ...
        self.refreshLwgProperty()
        QtWidgets.QMessageBox.information(self.msgbox, 'Calculate Instantaneous Attribute', self.instanattriblist[self.cbbattrib.currentIndex()] + ' calculated successfully')
