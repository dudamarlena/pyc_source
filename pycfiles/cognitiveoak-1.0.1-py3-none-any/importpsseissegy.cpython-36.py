...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\importpsseissegy.py
# Compiled at: 2019-12-16 00:14:23
# Size of source mod 2**32: 13928 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import numpy as np, sys, os
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-13])
from cognitivegeo.src.psseismic.inputoutput import inputoutput as psseis_io
from cognitivegeo.src.psseismic.analysis import analysis as psseis_ays
from cognitivegeo.src.gui.viewsegytextualheader import viewsegytextualheader as gui_viewsegytextualheader
from cognitivegeo.src.gui.viewsegybinaryheader import viewsegybinaryheader as gui_viewsegybinaryheader
from cognitivegeo.src.gui.viewsegytraceheader import viewsegytraceheader as gui_viewsegytraceheader
...
class importpsseissegy(object):
    ...
    def clickBtnSegy(self):
        ...
        _file = _dialog.getOpenFileNames(None, 'Select Pre-stack Seismic SEG-Y(s)', (self.rootpath), filter='SEG-Y files (*.segy; *.sgy);;All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnTextualHeader(self):
        ...
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportPsSeisSegy: No SEG-Y selected for viewing textual header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic SEG-Y', 'No SEG-Y selected for viewing textual header')
            return
    ...
    def clickBtnBinaryHeader(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportPsSeisSegy: No SEG-Y selected for viewing binary header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic SEG-Y', 'No SEG-Y selected for viewing binary header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnTraceHeader(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportPsSeisSegy: No SEG-Y selected for viewing trace header', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import SEG-Y', 'No SEG-Y selected for viewing trace header')
            return
        _viewheader = QtWidgets.QDialog()
    ...
    def clickBtnImport(self):
        ...
        _nfile = len(self.segylist)
        if _nfile <= 0:
            vis_msg.print('ERROR in ImportPsSeisSegy: No SEG-Y selected for import', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Pre-stack Seismic SEG-Y', 'No SEG-Y selected for import')
            return
        _traceheaderformat = psseis_io.defSegyTraceHeaderFormat(record_byte=(self.cbbrecord.currentIndex() + 1), trace_byte=(self.cbbtrace.currentIndex() + 1))
        ...
        for i in range(_nfile):
            ...
            _psseisdata[_filenamemain] = psseis_io.readPsSeisFromSegy(_filename, traceheaderformat=_traceheaderformat,
              qpgsdlg=_pgsdlg)

        for key in _psseisdata.keys():
            if key in self.psseisdata.keys():
                if checkPsSeisData(self.psseisdata[key]):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Pre-stack Seismic SEG-Y', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                    if reply == QtWidgets.QMessageBox.No:
                        return
            self.psseisdata[key] = _psseisdata[key]

        QtWidgets.QMessageBox.information(self.msgbox, 'Import Pre-stack Seismic SEG-Y', str(_nfile) + ' SEG-Y imported successfully')
