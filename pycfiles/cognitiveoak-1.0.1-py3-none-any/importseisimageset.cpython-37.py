...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\importseisimageset.py
...
import cognitivegeo.src.seismic.visualization as seis_vis
...
class importseisimageset(object):
    ...
    def clickBtnImage(self):
        ...
        _file = _dialog.getOpenFileNames(None, 'Select Seismic Image(s)', (self.rootpath), filter='Image files (*.jpg; *.png);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnImportSeisImageSet(self):
        ...
        if _nimage <= 0:
            vis_msg.print('ERROR in ImageSeisImageSet: No image selected for import', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic ImageSet', 'No image selected for import')
            return
        ...
        if _ninl is False or _nxl is False or _nz is False:
            vis_msg.print('ERROR in ImageSeisImageSet: Non-integer survey dimensions', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic ImageSet', 'Non-integer dimensions')
            return
        if _ninl <= 0 or _nxl <= 0 or _nz <= 0:
            vis_msg.print('ERROR in ImageSeisImageSet: Zero survey dimensions', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Import Seismic ImageSet', 'Zero survey dimensions')
            return
        ...
        if self.cbbtype.currentIndex() == 0:
            _imagedata = seis_vis.loadSeisILSliceTo3DMat((self.imagelist), inlsls=np.linspace(0, (_ninl - 1), _ninl, dtype=int),
              ispref=False,
              xlnum=_nxl,
              znum=_nz,
              qpgsdlg=_pgsdlg)
        if self.cbbtype.currentIndex() == 1:
            _imagedata = seis_vis.loadSeisXLSliceTo3DMat((self.imagelist), xlsls=np.linspace(0, (_nxl - 1), _nxl, dtype=int), ispref=False,
              inlnum=_ninl,
              znum=_nz,
              qpgsdlg=_pgsdlg)
        if self.cbbtype.currentIndex() == 2:
            _imagedata = seis_vis.loadSeisZSliceTo3DMat((self.imagelist), zsls=np.linspace(0, (-_nz + 1), _nz, dtype=int),
              ispref=False,
              inlnum=_ninl,
              xlnum=_nxl,
              qpgsdlg=_pgsdlg)
        ...
        for key in _seisdata.keys():
            if key in self.seisdata.keys():
                if checkSeisData(self.seisdata[key], self.survinfo):
                    reply = QtWidgets.QMessageBox.question(self.msgbox, 'Import Seismic ImageSet', key + ' already exists. Overwrite?', QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No, QtWidgets.QMessageBox.No)
                    if reply == QtWidgets.QMessageBox.No:
                        return
            self.seisdata[key] = _seisdata[key]

        QtWidgets.QMessageBox.information(self.msgbox, 'Import Seismic ImageSet', str(_nimage) + ' image(s) imported as Seismic successfully')
