...
# Size of source mod 2**32: 101130 bytes
                ...
                if 'pointsetdata' in _proj.keys():
                    ...
                    for key in _proj['pointsetdata'].keys():
                        if os.path.exists(os.path.join(self.projpath, self.projname + '.proj.data/PointSet/' + key + '.pts.npy')):
                            try:
                                _pointdata = np.load(os.path.join(self.projpath, self.projname + '.proj.data/PointSet/' + key + '.pts.npy')).item()
                            except ValueError:
                                _pointdata = {}

                            if checkPointData(_pointdata[key]):
                                self.pointsetdata[key] = _pointdata[key]
...
def saveProject(survinfo={}, seisdata={}, psseisdata={}, pointsetdata={}, settings={}, savepath='', savename='gpy'):
    ...
    for key in pointsetdata.keys():
        ...
        _tmp = {}
        _tmp[key] = pointsetdata[key]
        np.save(os.path.join(savepath, savename + '.proj.data/PointSet/' + key + '.pts.npy'), _tmp)
