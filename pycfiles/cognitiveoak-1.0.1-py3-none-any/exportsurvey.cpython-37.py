...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\exportsurvey.py
...
class exportsurvey(object):
    ...
    def clickBtnSave(self):
        ...
        _file = _dialog.getSaveFileName(None, 'Select Survey Numpy', (self.rootpath), filter='Survey Numpy files (*.srv.npy);; All files (*.*)')
        if len(_file[0]) > 0:
    ...
    def clickBtnExportSurvey(self):
        ...
        if self.checkSurvInfo() is False:
            vis_msg.print('ERROR in ExportSurvey: No survey found', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Survey', 'No survey found')
            return
        if len(self.ldtsave.text()) < 1:
            vis_msg.print('ERROR in ExportSurvey: No name specified for survey', type='error')
            QtWidgets.QMessageBox.critical(self.msgbox, 'Export Survey', 'No name specified for survey')
            return
        print('ExportSurvey: Export survey as %s' % self.ldtsave.text())
        np.save(self.ldtsave.text(), self.survinfo)
        QtWidgets.QMessageBox.information(self.msgbox, 'Export Survey', 'Survey exported successfully')
