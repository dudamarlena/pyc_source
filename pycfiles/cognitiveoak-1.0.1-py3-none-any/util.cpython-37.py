...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\segpy\util.py
...
def round_up(integer, multiple):
    ...
    if integer % multiple == 0:
        return integer
    return integer + multiple - integer % multiple
...
def ensure_contains(collection, item):
    if item in collection:
        return collection
    return conjoin(collection, item)
