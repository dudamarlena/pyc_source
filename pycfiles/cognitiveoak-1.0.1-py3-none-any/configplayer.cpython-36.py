...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\configplayer.py
    ...
    def retranslateGUI(self, ConfigPlayer):
        ...
        for play in sorted(self.playerconfig.keys()):
            ...
            for prop in vis_player.PlayerPropertyList[:-1]:
                ...
                item.setCurrentIndex(list.index(vis_player.PlayerKeyList, self.playerconfig[play][prop]))
            ...
            item.setCurrentIndex(list.index(vis_player.PlayerIntervalList, self.playerconfig[play][prop]))
            self.twgplayer.setCellWidget(_idx, _idxprop + 1, item)
            _idx = _idx + 1
