...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\gui\settingsviewer.py
# Compiled at: 2019-12-13 23:25:58
# Size of source mod 2**32: 8858 bytes
from PyQt5 import QtCore, QtGui, QtWidgets
import os, sys
sys.path.append(os.path.dirname(__file__)[:-4][:-4][:-13])
from cognitivegeo.src.core.settings import settings as core_set
from cognitivegeo.src.vis.viewer3d import viewer3d as vis_viewer3d
from cognitivegeo.src.vis.player import player as vis_player
...
class settingsviewer(object):
    ...
    def retranslateGUI(self, SettingsViewer):
        ...
        for _i in vis_viewer3d.ViewFromPropertyList:
            _idx = list.index(vis_viewer3d.ViewFromPropertyList, _i)
            self.twgviewfrom.cellWidget(0, _idx).setCurrentIndex(list.index(vis_viewer3d.ViewFromKeyList, self.settings['Viewer3D']['ViewFrom'][_i]))

        for _i in vis_player.PlayerPropertyList[:-1]:
            _idx = list.index(vis_player.PlayerPropertyList, _i)
            self.twgplayer.cellWidget(0, _idx).setCurrentIndex(list.index(vis_player.PlayerKeyList, self.settings['Player'][_i]))

        _i = vis_player.PlayerPropertyList[(-1)]
        _idx = list.index(vis_player.PlayerPropertyList, _i)
        self.twgplayer.cellWidget(0, _idx).setCurrentIndex(list.index(vis_player.PlayerIntervalList, self.settings['Player'][_i]))
    ...
    def clickBtnApply(self):
        ...
        for _prop in vis_viewer3d.ViewFromPropertyList:
            ...
            self.settings['Viewer3D']['ViewFrom'][_prop] = vis_viewer3d.ViewFromKeyList[self.twgviewfrom.cellWidget(0, _idx).currentIndex()]

        for _prop in vis_player.PlayerPropertyList[:-1]:
            _idx = list.index(vis_player.PlayerPropertyList, _prop)
            self.settings['Player'][_prop] = vis_player.PlayerKeyList[self.twgplayer.cellWidget(0, _idx).currentIndex()]

        _prop = vis_player.PlayerPropertyList[(-1)]
        _idx = list.index(vis_player.PlayerPropertyList, _prop)
        self.settings['Player'][_prop] = vis_player.PlayerIntervalList[self.twgplayer.cellWidget(0, _idx).currentIndex()]
        self.dialog.close()
