...
# Embedded file name: C:/Users/HDi/Google Drive/ProgramCodes/Released/PyPI/cognitivegeo\cognitivegeo\src\basic\matdict.py
# Compiled at: 2019-12-15 16:23:09
# Size of source mod 2**32: 15036 bytes
...
def isDictConstantRow(dict):
    ...
    for key in dict.keys():
        if nrow != len(dict[key]):
            vis_msg.print(('ERROR in isDictConstantRow: ' + key + ' of inconstant row number'), type='error')
            return False
...
def maxDictConstantRow(dict):
    ...
    """
    if len(dict.keys()) <= 0:
        vis_msg.print('WARNING in maxMatDictConstantRow: Empty dictionary', type='error')
        return 0
    firstkey = list(dict.keys())[0]
    nrow = len(dict[firstkey])
    for key in dict.keys():
...
def retrieveDictByKey(dict, key_list):
    ...
    for key in key_list:
        if key in dict.keys():
            ...
            vis_msg.print(('WARNING in retrieveDictByKeys: ' + key + ' Not found in the input dictionary'), type='warning')
...
def retrieveDictByIndex(dict, index_list):
    ...
    if len(dict.keys()) <= 0:
        ...
        sys.exit()
    if isDictConstantRow(dict) is False:
        vis_msg.print('ERROR in retrieveDictByIndex: Inconstant row number found in input dictionary', type='error')
        sys.exit()
...
def retrieveDictRandom(dict, batch_size=10, unique=True):
    ...
    if len(dict.keys()) <= 0:
        ...
        sys.exit()
    if isDictConstantRow(dict) is False:
        vis_msg.print('ERROR in retrieveDictBatchRandom: Inconstant row number found in input dictionary', type='error')
        sys.exit()
    maxnrow = len(dict[list(dict.keys())[0]])
...
def exportMatDict(dict, key_list=None):
    ...
    for key in key_list:
        if key in dict.keys():
            if len(mat) == 0:
                mat = dict[key]
            elif np.shape(mat)[0] == np.shape(dict[key])[0]:
