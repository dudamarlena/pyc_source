...
# [GCC 8.4.0]
# Embedded file name: build/lib/iprir/database.py
...
class DB:
    ...
    def add_records(self, records):
        try:
            self.conn.cursor().executemany('INSERT INTO iprir VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (self.record_to_tuple(r) for r in records if self.filter_record(r)))
            self.conn.commit()
        except sqlite3.Error:
    ...
    def filter_record(record: RIRRecord):
        ...
        return record.status in ('allocated', 'assigned') and record.type in ('ipv4',
                                                                              'ipv6')
    ...
    def by_country(self, type_: str, country: str):
        cur = self.conn.cursor()
        cur.execute('SELECT * from iprir WHERE type = :type AND country = :country', dict(type=type_, country=country))
        return [self.tuple_to_record(tup) for tup in cur.fetchall()]
