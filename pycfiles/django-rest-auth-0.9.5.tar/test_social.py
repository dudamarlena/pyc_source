...
# [GCC 8.4.0]
# Embedded file name: /Users/mario/work/django-rest-auth/rest_auth/tests/test_social.py
# Compiled at: 2017-11-12 04:10:41
import json
...
class TestSocialAuth(TestsMixin, TestCase):
    ...
    def setUp(self):
        self.init()
        social_app = SocialApp.objects.create(provider='facebook', name='Facebook', client_id='123123123', secret='321321321')
    ...
    def test_failed_social_auth(self):
        responses.add(responses.GET, self.graph_api_url, body='', status=400, content_type='application/json')
        payload = {'access_token': 'abc123'}
        response = self.post(self.fb_login_url, data=payload, status_code=400)

    @responses.activate
    def test_social_auth(self):
        resp_body = {'id': '123123123123', 
           'first_name': 'John', 
           'gender': 'male', 
           'last_name': 'Smith', 
           ...
           'username': 'john.smith', 
           'verified': True}
        responses.add(responses.GET, self.graph_api_url, body=json.dumps(resp_body), status=200, content_type='application/json')
    ...
    def _twitter_social_auth(self):
        resp_body = {'id': '123123123123'}
        responses.add(responses.GET, 'https://api.twitter.com/1.1/account/verify_credentials.json', body=json.dumps(resp_body), status=200, content_type='application/json')
        users_count = get_user_model().objects.all().count()
        payload = {'access_token': 'abc123', 
    ...
    def test_twitter_social_auth_request_error(self):
        resp_body = {'id': '123123123123'}
        responses.add(responses.GET, 'https://api.twitter.com/1.1/account/verify_credentials.json', body=json.dumps(resp_body), status=400, content_type='application/json')
        users_count = get_user_model().objects.all().count()
        payload = {'access_token': 'abc123', 
           'token_secret': '1111222233334444'}
    ...
    @responses.activate
    @override_settings(ACCOUNT_EMAIL_VERIFICATION='mandatory', ACCOUNT_EMAIL_REQUIRED=True, REST_SESSION_LOGIN=False, ACCOUNT_EMAIL_CONFIRMATION_HMAC=False)
    def test_email_clash_with_existing_account(self):
        ...
        email_confirmation = new_user.emailaddress_set.get(email=self.EMAIL).emailconfirmation_set.order_by('-created')[0]
        self.post(self.verify_email_url, data={'key': email_confirmation.key}, status_code=status.HTTP_200_OK)
        self._login()
        self._logout()
        payload = {'access_token': 'abc123'}
    ...
    @responses.activate
    @override_settings(ACCOUNT_EMAIL_VERIFICATION='mandatory', ACCOUNT_EMAIL_REQUIRED=True, REST_SESSION_LOGIN=False, ACCOUNT_EMAIL_CONFIRMATION_HMAC=False)
    def test_email_clash_with_existing_social_account(self):
        resp_body = {'id': '123123123123', 
           'first_name': 'John', 
           'gender': 'male', 
           'last_name': 'Smith', 
        ...
        self.assertIn('key', self.response.json.keys())
        self.assertEqual(get_user_model().objects.all().count(), users_count + 1)
        response = self.post(self.register_url, data=self.REGISTRATION_DATA, status_code=201)
        import pdb
        pdb.set_trace()
        new_user = get_user_model().objects.latest('id')
        self.assertEqual(new_user.username, self.REGISTRATION_DATA['username'])
    ...
    def test_jwt(self):
        ...
        responses.add(responses.GET, self.graph_api_url, body=resp_body, status=200, content_type='application/json')
        users_count = get_user_model().objects.all().count()
        payload = {'access_token': 'abc123'}
        self.post(self.fb_login_url, data=payload, status_code=200)
