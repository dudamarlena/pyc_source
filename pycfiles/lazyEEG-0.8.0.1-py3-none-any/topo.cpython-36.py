...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\IPython Notebooks\experiment\lazyEEG\algorithms\topo.py
# Compiled at: 2017-12-25 13:14:11
...
def topography(self, step_size='1ms', win_size='1ms', sample='mean', sig_limit=0.05):
    ...
    def to_topo(case_raw_data):
        ...
        if len(data_with_subject.index.get_level_values('condition_group').unique()) == 2:
            ...
            stats_result = roll_on_levels(data_with_subject, (stats_methods.t_test), levels=['time', 'channel'], between='condition_group',
              in_group='subject',
              prograssbar=False)
    ...
    default_plot_params = dict(title='Topography', plot_type=['matrix', 'topograph'], zlim=minmax, color=(plt.cm.jet), chan_locs=(self.info['xy_locs']),
      sig_limit=sig_limit,
      x_title='time',
...
def significant_channels_count(self, step_size='1ms', win_size='1ms', sample='mean', sig_limit=0.05):
    ...
    default_plot_params = dict(title='significant_channels_count', plot_type=['direct', 'heatmap'], x_len=12, color=sns.cubehelix_palette(light=1, as_cmap=True),
      x_title='time',
      y_title='condition_group',
      cbar_title='Count')
