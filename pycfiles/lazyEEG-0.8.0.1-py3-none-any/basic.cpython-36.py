...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\IPython Notebooks\experiment\lazyEEG\algorithms\basic.py
# Compiled at: 2017-12-24 20:39:48
...
def point_sampling(df, step_size):
    if step_size[-2:] in ('ms', 'Ms', 'MS'):
        if step_size[:-2].isdigit():
...
def window_sampling(df, win_size, sample='mean'):
    if win_size[-2:] in ('ms', 'Ms', 'MS'):
        if win_size[:-2].isdigit():
    ...
    for idx, group_data in df.groupby(axis=1, level='time_group'):
        ...
        group_data_new.columns = group_data_new.columns + pd.Timedelta(win_size[:-1] + 'ns') / 2
...
def roll_on_levels(df, func, arguments_dict=None, levels='time', between='condition_group', in_group='subject', prograssbar=True):
    ...
    for group_id, group_data in df.groupby(level=levels):
        data_subjects = [group_data.mean(level=in_group).mean(axis=1) for group_id, group_data in group_data.groupby(level=between)]
...
def stats_compare(df, comparison, levels='time', between='condition_group', in_group='subject'):
    ...
    if comparison['sig_limit'] > 0 and len(condition_groups) == 2:
        ...
        if comparison['need_fdr']:
            pval_result = stats_methods.fdr(pval_result)
        return pval_result
    else:
        return
