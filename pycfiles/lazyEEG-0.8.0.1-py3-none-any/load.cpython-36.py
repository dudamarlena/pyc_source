...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\IPython Notebooks\experiment\lazyEEG\io\load.py
# Compiled at: 2017-12-25 11:17:57
# Size of source mod 2**32: 27950 bytes
from ..default import *
...
def from_mne_epochs(subj_datas, subjIDs=[], montage_path='standard-10-5-cap385'):
    ...
    """
    if type(subj_datas) == mne.epochs.Epochs:
        subj_datas = [
         subj_datas]
    if subjIDs == []:
    ...
    for subjID, epoch_raw in zip(subjIDs, subj_datas):
        ...
        index_new = [(subjID, condition, trial, channel) for condition, trial, channel in epochs_data_each_subj.index.tolist()]
...
def load_epochs(files=[], path='', subjIDs=[], montage_path='standard-10-5-cap385', baseline_time=[], extremum_in_all=None, extremum_in_baseline=None, events_dict=dict(), drop_channels=[], compress=False):
    ...
    if filepaths[0].endswith('.h5'):
        return load_dataframe_hdf5(files, path, subjIDs, montage_path, compress)
    if filepaths[0].endswith('.fif'):
        return load_mne_fif(files, path, subjIDs, montage_path)
    if filepaths[0].endswith('.mat'):
        return load_eeglab_mat(files, path, subjIDs, montage_path, baseline_time, extremum_in_all, extremum_in_baseline, events_dict, drop_channels)
        ...
        try:
            ...
            if type(df) is pd.DataFrame:
                if compress:
                    ...
                    df_averaged.index = pd.MultiIndex.from_tuples([(subject, condition, conditions_list.index(condition), channel) for channel, condition, subject in df_averaged.index],
                      names=[
        ...
        try:
            ...
            for sub_id in store.keys():
                ...
                if type(df) is pd.DataFrame:
                    ...
                    print(f'{sub_id} in "{filepath}" is not a DataFrame')

            store.close()
        except:
            print('"%s" cannot be loaded as a hdf5 file' % filename)
...
def load_eeglab_mat(files=[], path='', subjIDs=[], montage_path='standard-10-5-cap385', baseline_time=[], extremum_in_all=None, extremum_in_baseline=None, events_dict=dict(), drop_channels=[]):
    filepaths = infer_paths(files, path)
    if subjIDs == []:
...
def load_raw_eeg(files=[], path='', subjIDs=[], montage_path='standard-10-5-cap385', events_dict=dict(), default_reference_channels='Cz', drop_channels=[], filter_range=(0.1, 30.0), ref_channels='average', epoch_range=(-0.2, 1), reject=None):
    filepaths = infer_paths(files, path)
    if subjIDs == []:
        subjIDs = [i + 1 for i in range(len(filepaths))]
    epochs_data = []
    for filepath in filepaths:
        ...
        EEG.set_eeg_reference(ref_channels=ref_channels)
        EEG.apply_proj()
