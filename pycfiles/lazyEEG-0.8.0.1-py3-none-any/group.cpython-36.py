...
# [GCC 8.4.0]
# Embedded file name: D:\Coding\py\IPython Notebooks\experiment\lazyEEG\group.py
# Compiled at: 2017-12-20 10:13:21
...
def parsing(collection_script, epochs_data):
    ...
    for batch_script in collection_script:
        if type(batch_script) is str:
            ...
            if type(batch_script) is not dict:
...
def extract(epochs, collection_script, average=False, preload=False):
    ...
    for batch_name, batch_frame in collection_frame:
        batch_data = (generate_case_data(case_frame, epochs_data_todo) for case_frame in batch_frame)
        if preload:
