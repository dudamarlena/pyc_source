...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/blast/results/BlastHitPartitions.py
...
class BlastHitPartitions:
    ...
    def append(self, hit: AMRHitHSP) -> None:
        ...
        if hit.get_genome_contig_start() > hit.get_genome_contig_end():
            if hit.get_genome_contig_strand() == 'plus':
                raise InvalidPositionException('Unsupported condition: strand=plus and contig start > contig end for hit (contig=' + hit.get_genome_contig_id() + ', start=' + str(hit.get_genome_contig_start()) + ', end=' + str(hit.get_genome_contig_end()) + ')')
        else:
