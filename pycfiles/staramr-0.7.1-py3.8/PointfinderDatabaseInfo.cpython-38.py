...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/blast/pointfinder/PointfinderDatabaseInfo.py
...
class PointfinderDatabaseInfo:
    ...
    def _resistance_table_hacks(self, table):
        ...
        """
        if self._file:
            if 'salmonella' in str(self._file):
                if path.exists(path.join(path.dirname(self._file), '16S_rrsD.fsa')):
    ...
    def _get_resistance_codon_match(self, gene, codon_mutation):
        ...
        matches = table[((table['#Gene_ID'] == gene) & (table['Codon_pos'] == codon_mutation.get_mutation_position()) & (table['Ref_codon'] == codon_mutation.get_database_amr_gene_mutation()) & table['Res_codon'].str.contains((codon_mutation.get_input_genome_mutation()), regex=False))]
