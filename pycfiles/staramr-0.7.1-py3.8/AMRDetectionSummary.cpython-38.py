...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/results/AMRDetectionSummary.py
# Compiled at: 2019-12-17 17:26:02
# Size of source mod 2**32: 10708 bytes
import logging
from os import path
...
class AMRDetectionSummary:
    ...
    def __init__(self, files, resfinder_dataframe: DataFrame, quality_module_dataframe: DataFrame, pointfinder_dataframe=None, plasmidfinder_dataframe=None, mlst_dataframe=None) -> None:
        """
    ...
    def _compile_results(self, resistance_frame: DataFrame) -> DataFrame:
        df_summary = resistance_frame.sort_values(by=['Gene']).groupby(['Isolate ID']).aggregate(lambda x: {'Gene': (self.SEPARATOR + ' ').join(x['Gene'])})
        return df_summary[['Gene']]

    def _compile_plasmids(self, plasmid_frame: DataFrame) -> DataFrame:
        ds_summary = plasmid_frame.sort_values(by=['Gene']).groupby(['Isolate ID']).aggregate(lambda x: {'Gene': (self.SEPARATOR + ' ').join(x['Gene'])})
    ...
    def _include_negatives(self, resistance_frame: DataFrame) -> DataFrame:
        ...
        negative_entries = pd.DataFrame([[x, 'None'] for x in negative_names_set], columns=('Isolate ID',
                                                                                            'Gene')).set_index('Isolate ID')
        return resistance_frame.append(negative_entries, sort=True)

    def _get_detailed_negative_columns(self):
        return [
         'Isolate ID', 'Gene', 'Start', 'End']
    ...
    def _get_negative_resistance_entries(self, names_set: Set, resistance_frame: DataFrame) -> DataFrame:
        ...
        if len(negative_res_names_set) != len(names_set) or resistance_frame.empty:
            if self._include_phenotype():
                ...
                negative_resistance_entries = pd.DataFrame([[x, 'None', '', ''] for x in negative_res_names_set], columns=negative_columns).set_index('Isolate ID')
    ...
    def _include_detailed_negatives(self, resistance_frame: DataFrame, plasmid_frame: DataFrame=None) -> DataFrame:
        ...
        if plasmid_frame is not None:
            ...
            negative_plasmid_entries = pd.DataFrame([[x, 'None'] for x in set_used], columns=('Isolate ID',
                                                                                              'Gene')).set_index('Isolate ID')
    ...
    def create_summary(self, include_negatives: bool=False) -> DataFrame:
        ...
        if plasmid_frame is not None:
            ...
            if resistance_frame.empty:
                ...
                resistance_frame = resistance_frame.merge(plasmid_frame, on='Isolate ID', how='left').fillna(value={'Plasmid': 'None'})
    ...
    def _get_detailed_summary_columns(self):
        return [
         'Gene', 'Data Type', '%Identity', '%Overlap', 'HSP Length/Total Length', 'Contig', 'Start', 'End', 'Accession']
    ...
    def create_detailed_summary(self, include_negatives: bool=True) -> DataFrame:
        ...
        if self._resfinder_dataframe is None:
            ...
            resistance_frame = resistance_frame.round({'%Identity':self.FLOAT_DECIMALS, 
             '%Overlap':self.FLOAT_DECIMALS})
        ...
        if mlst_merging_frame is not None:
            if resistance_frame is not None:
