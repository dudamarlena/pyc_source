...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/tests/unit/results/test_AMRDetectionSummary.py
# Compiled at: 2019-12-17 17:26:02
# Size of source mod 2**32: 42121 bytes
import logging, unittest, pandas as pd
from os import path
...
class AMRDetectionSummaryTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.columns_resfinder = ('Isolate ID', 'Gene', '%Identity', '%Overlap', 'HSP Length/Total Length',
        ...
        self.columns_plasmidfinder = ('Isolate ID', 'Gene', '%Identity', '%Overlap',
                                      'HSP Length/Total Length', 'Contig', 'Start',
                                      'End', 'Accession')
        self.columns_quality_module = ('Isolate ID', 'Genome Length', 'N50 value',
                                       'Number of Contigs Under 1000 bp', 'Quality Module',
                                       'Quality Module Feedback')
        self.detailed_summary = ('Isolate ID', 'Gene', 'Predicted Phenotype', '%Identity',
                                 '%Overlap', 'HSP Length/Total Length', 'Contig',
                                 'Start', 'End', 'Accession', 'Data Type')
        self.resfinder_table_empty = pd.DataFrame([], columns=(self.columns_resfinder))
        self.resfinder_table1 = pd.DataFrame([
         [
          'file1', 'blaIMP-42', 99.73, 100.0, '741/741', 'blaIMP-42_1_AB753456', 1, 741,
          'AB753456']],
        ...
        self.resfinder_table_mult_resistance = pd.DataFrame([
         [
          ...
          'AB753456'],
         [
          'file1', 'newGene', 99.73, 100.0, '741/741', 'newGene', 1, 741,
          'AB753456']],
          columns=(self.columns_resfinder))
        ...
        self.resfinder_table_mult_same_gene_same_resistance = pd.DataFrame([
         [
          ...
          'AB753456'],
         [
          'file1', 'blaIMP-42', 99.73, 100.0, '741/741', 'newGene', 1, 741,
          'AB753456']],
          columns=(self.columns_resfinder))
        ...
        self.resfinder_table_mult_file = pd.DataFrame([
         ...
         [
          ...
          'AB753456'],
         [
          'file2', 'blaIMP-42', 99.73, 100.0, '741/741', 'newGene', 1, 741,
          'AB753456']],
          columns=(self.columns_resfinder))
        self.resfinder_table_mult_file_files = ['file1', 'file2']
        self.plasmidfinder_table_empty = pd.DataFrame([], columns=(self.columns_plasmidfinder))
        self.plasmidfinder_table1 = pd.DataFrame([
         [
          'file4', 'IncFIB(S)', 100.0, 100.0, '643/643', 'ref|NC_003277.2|', 17653, 17011,
          'FN432031']],
        ...
        self.plasmidfinder_table_mult_resistance = pd.DataFrame([
         [
          ...
          'FN432031'],
         [
          'file4', 'IncFII(S)', 100.0, 100.0, '262/262', 'ref|NC_003277.2|', 1665, 1926,
          'CP000858']],
          columns=(self.columns_plasmidfinder))
        self.plasmidfinder_table_mult_resistance_files = ['file4']
        self.plasmidfinder_table_mult_gene_same_resistance_files = [
         'file4']
        ...
        self.plasmidfinder_table_mult_same_gene_same_resistance = pd.DataFrame([
         ...
         [
          ...
          'CP000858']],
          columns=(self.columns_plasmidfinder))
        self.plasmidfinder_table_mult_file_files = [
         'file4', 'file5']
        self.plasmidfinder_table_mult_file = pd.DataFrame([
         ...
         [
          ...
          'CP000858'],
         [
          'file5', 'IncFIB(K)', 98.93, 100.0, '560/560', 'ref|NC_006856.1|', 118238, 117679,
          'JN233704']],
          columns=(self.columns_plasmidfinder))
        ...
        self.pointfinder_table_empty = pd.DataFrame([], columns=(self.columns_pointfinder))
        self.quality_module_table_single_file = pd.DataFrame([['file1', 6000000, 11000, 0, 'Pass', '']], columns=(self.columns_quality_module)).set_index('Isolate ID')
    ...
    def testResfinderSingleGene(self):
        amr_detection_summary = AMRDetectionSummary(self.resfinder_table1_files, self.resfinder_table1, self.quality_module_table_single_file, self.plasmidfinder_table_empty)
    ...
    def testResfinderMultipleGeneResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.resfinder_table_mult_resistance_files, self.resfinder_table_mult_resistance, self.quality_module_table_single_file, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testResfinderMultipleGeneSameResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.resfinder_table_mult_gene_same_resistance_files, self.resfinder_table_mult_gene_same_resistance, self.quality_module_table_single_file, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
        self.assertEqual('blaIMP-42, newGene', summary['Genotype'].iloc[0], 'Genes not equal')
    ...
    def testResfinderMultipleSameGeneSameResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.resfinder_table_mult_same_gene_same_resistance_files, self.resfinder_table_mult_same_gene_same_resistance, self.quality_module_table_single_file, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testResfinderMultipleFile(self):
        amr_detection_summary = AMRDetectionSummary(self.resfinder_table_mult_file_files, self.resfinder_table_mult_file, self.quality_module_table_two_files, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(2, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
        self.assertEqual('blaIMP-42, newGene', summary['Genotype'].iloc[0], 'Genes not equal')
    ...
    def testPlasmidfinderSingleGene(self):
        amr_detection_summary = AMRDetectionSummary(self.plasmidfinder_table1_files, self.resfinder_table_empty, self.quality_module_table_file_4, self.plasmidfinder_table1)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
    ...
    def testPlasmidfinderMultipleGeneResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.plasmidfinder_table_mult_resistance_files, self.resfinder_table_empty, self.quality_module_table_file_4, self.plasmidfinder_table_mult_resistance)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file4', summary.index[0], 'File name not equal')
    ...
    def testPlasmidfinderMultipleGeneSameResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.plasmidfinder_table_mult_gene_same_resistance_files, self.resfinder_table_empty, self.quality_module_table_file_4, self.plasmidfinder_table_mult_gene_same_resistance)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file4', summary.index[0], 'File name not equal')
        self.assertEqual('IncFIB(S), IncFII(S)', summary['Genotype'].iloc[0], 'Genes not equal')
    ...
    def testPlasmidfinderMultipleSameGeneSameResistance(self):
        amr_detection_summary = AMRDetectionSummary(self.plasmidfinder_table_mult_same_gene_same_resistance_files, self.resfinder_table_empty, self.quality_module_table_file_4, self.plasmidfinder_table_mult_same_gene_same_resistance)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file4', summary.index[0], 'File name not equal')
        self.assertEqual('IncFIB(S), IncFII(S)', summary['Genotype'].iloc[0], 'Genes not equal')
    ...
    def testPlasmidfinderMultipleFile(self):
        amr_detection_summary = AMRDetectionSummary(self.plasmidfinder_table_mult_file_files, self.resfinder_table_empty, self.quality_module_table_file_4_and_5, self.plasmidfinder_table_mult_file)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(2, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file4', summary.index[0], 'File name not equal')
        self.assertEqual('IncFIB(S), IncFII(S)', summary['Genotype'].iloc[0], 'Genes not equal')
    ...
    def testPointfinderSingleGene(self):
        amr_detection_summary = AMRDetectionSummary(self.pointfinder_table_files, self.resfinder_table_empty, self.quality_module_table_single_file, self.pointfinder_table, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testPointfinderSingleMultipleGene(self):
        amr_detection_summary = AMRDetectionSummary(self.pointfinder_table_multiple_gene_files, self.resfinder_table_empty, self.quality_module_table_single_file, self.pointfinder_table_multiple_gene, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testPointfinderSingleMultipleGeneSame(self):
        df = pd.DataFrame([
         ...
         [
          ...
          columns=(self.columns_pointfinder))
        files = ['file1']
        amr_detection_summary = AMRDetectionSummary(files, self.resfinder_table_empty, self.quality_module_table_single_file, df, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testPointfinderResfinderSingleGene(self):
        files = [
         'file1']
        amr_detection_summary = AMRDetectionSummary(files, self.resfinder_table1, self.quality_module_table_single_file, self.pointfinder_table, self.plasmidfinder_table_empty)
        summary = amr_detection_summary.create_summary()
        self.assertEqual(1, len(summary.index), 'Invalid number of rows in results')
        self.assertEqual('file1', summary.index[0], 'File name not equal')
    ...
    def testDetailedSummary_noPlasmid_noPoint(self):
        amr_detection_summary = AMRDetectionSummaryResistance(self.resfinder_table1_files, self.resfinder_table1.set_index('Isolate ID'), self.quality_module_table_file_1)
    ...
    def testDetailedSummary_noRes(self):
        ...
        plasmid_table['Isolate ID'] = 'file1'
        plasmid_table = plasmid_table.set_index('Isolate ID')
        amr_detection_summary = AMRDetectionSummaryResistance((self.resfinder_table1_files), (self.resfinder_table_empty.set_index('Isolate ID')),
          (self.quality_module_table_file_1),
          pointfinder_dataframe=point_table,
          plasmidfinder_dataframe=plasmid_table)
        detailed_summary = amr_detection_summary.create_detailed_summary()
    ...
    def testDetailedSummary_noFinders(self):
        amr_detection_summary = AMRDetectionSummaryResistance(self.resfinder_table1_files, self.resfinder_table_empty.set_index('Isolate ID'), self.quality_module_table_file_1, self.pointfinder_table_empty.set_index('Isolate ID'), self.plasmidfinder_table_empty.set_index('Isolate ID'))
        detailed_summary = amr_detection_summary.create_detailed_summary()
        self.assertEqual(2, len(detailed_summary.index), 'Invalid number of rows, expected 2')
        self.assertEqual('file1', detailed_summary.index[0], 'File name not equal')
        self.assertEqual('file1', detailed_summary.index[1], 'File name not equal')
    ...
    def testDetailedSummary_multiFiles(self):
        amr_detection_summary = AMRDetectionSummaryResistance(self.detailed_summary_multi_files, self.resfinder_table_mult_file.set_index('Isolate ID'), self.quality_module_table_file_1_and_2_and_4_and_5, self.pointfinder_table_multiple_gene.set_index('Isolate ID'), self.plasmidfinder_table_mult_file.set_index('Isolate ID'))
        detailed_summary = amr_detection_summary.create_detailed_summary()
