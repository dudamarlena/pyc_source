...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/blast/results/BlastResultsParser.py
# Compiled at: 2019-12-17 17:26:02
# Size of source mod 2**32: 6361 bytes
import abc, logging, os
from typing import List
import Bio.SeqIO, numpy as np, pandas as pd
import staramr.blast.JobHandler as JobHandler
import staramr.blast.results.BlastHitPartitions as BlastHitPartitions
...
class BlastResultsParser:
    ...
    def _handle_blast_hit(self, in_file, database_name, blast_file, results, hit_seq_records):
        blast_table = pd.read_csv(blast_file, sep='\t', header=None, names=(JobHandler.BLAST_COLUMNS), index_col=False).astype(dtype={'qseqid':np.unicode_, 
         'sseqid':np.unicode_})
        partitions = BlastHitPartitions()
        blast_table['plength'] = blast_table.length / blast_table.qlen * 100.0
        blast_table = blast_table[((blast_table.pident >= self._pid_threshold) & (blast_table.plength >= self._plength_threshold) & ~blast_table.qseqid.isin(self._genes_to_exclude))]
    ...
    def _select_hits_to_include(self, hits):
        ...
        if len(hits) >= 1:
            sorted_hits_pid_first = sorted(hits, key=(lambda x: (
             x.get_pid(), x.get_plength(), x.get_amr_gene_length(), x.get_amr_gene_id())),
              reverse=True)
            sorted_hits_length_first = sorted(hits, key=(lambda x: (
             x.get_amr_gene_length(), x.get_pid(), x.get_plength(), x.get_amr_gene_id())),
              reverse=True)
            if self._report_all:
                ...
                if first_hit_pid == first_hit_length:
                    ...
                    if first_hit_length.get_amr_gene_length() - first_hit_pid.get_amr_gene_length() > 10 and first_hit_length.get_pid() - first_hit_pid.get_pid() > -1 and first_hit_length.get_plength() - first_hit_pid.get_plength() > -1:
                        hits_to_include.append(first_hit_length)
                    else:
