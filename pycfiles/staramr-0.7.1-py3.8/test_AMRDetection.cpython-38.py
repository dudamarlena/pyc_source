...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/staramr/tests/integration/detection/test_AMRDetection.py
# Compiled at: 2019-12-17 17:26:02
# Size of source mod 2**32: 64885 bytes
import logging, os, tempfile, unittest
from os import path
import pandas as pd
...
class AMRDetectionIT(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.blast_handler = JobHandler({'resfinder':self.resfinder_database, 
         'pointfinder':self.pointfinder_database}, 2, self.blast_out.name)
        ...
        self.drug_key_pointfinder_invalid_file = path.join(self.test_data_dir, 'gene-drug-tables', 'drug_key_pointfinder_invalid.tsv')
    ...
    def testResfinderAminoglycosideNameSuccess(self):
        ...
        self.assertAlmostEqual((result['%Identity'].iloc[0]), 100.0, places=2, msg='Wrong pid')
    ...
    def testResfinderBetaLactam2MutationsSuccess(self):
        ...
        self.assertEqual(result['Predicted Phenotype'].iloc[0], 'ampicillin, amoxicillin/clavulanic acid, cefoxitin, ceftriaxone, meropenem', 'Wrong phenotype')
    ...
    def testResfinderBetaLactam2MutationsSuccessNoPredictedPhenotype(self):
        amr_detection = AMRDetection((self.resfinder_database), (self.blast_handler), (self.pointfinder_database), output_dir=(self.outdir.name))
        file = path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-mut-2.fsa')
        files = [file]
    ...
    def testResfinderBetaLactam2MutationsSuccessNoMatchDrugTable(self):
        ...
        self.amr_detection = AMRDetectionResistance((self.resfinder_database), resfinder_drug_table, (self.blast_handler),
          (self.pointfinder_drug_table), (self.pointfinder_database),
          output_dir=(self.outdir.name))
        file = path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-mut-2.fsa')
        files = [file]
    ...
    def testResfinderBetaLactam2MutationsFail(self):
        files = [
         path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-mut-2.fsa')]
    ...
    def testResfinderBetaLactamDelStartSuccess(self):
        ...
        self.assertEqual(len(result.index), 1, 'Wrong number of results detected')
        self.assertAlmostEqual((result['%Identity'].iloc[0]), 100.0, places=2, msg='Wrong pid')
        self.assertAlmostEqual((result['%Overlap'].iloc[0]), 91.9, places=2, msg='Wrong overlap')
    ...
    def testResfinderBetaLactamDelStartFail(self):
        files = [
         path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-del-start.fsa')]
    ...
    def testResfinderBetaLactamDelMiddleReverseComplementSuccess(self):
        ...
        self.assertEqual(len(result.index), 1, 'Wrong number of results detected')
        self.assertAlmostEqual((result['%Identity'].iloc[0]), 99.33, places=2, msg='Wrong pid')
        self.assertAlmostEqual((result['%Overlap'].iloc[0]), 100.0, places=2, msg='Wrong percent overlap')
        self.assertEqual((result['HSP Length/Total Length'].iloc[0]), '741/741', msg='Wrong lengths')
        self.assertEqual(result['Predicted Phenotype'].iloc[0], 'ampicillin, amoxicillin/clavulanic acid, cefoxitin, ceftriaxone, meropenem', 'Wrong phenotype')
    ...
    def testResfinderBetaLactamInsMiddleSuccess(self):
        ...
        records = SeqIO.to_dict(SeqIO.parse(hit_file, 'fasta'))
        self.assertEqual(len(records), 1, 'Wrong number of hit records')
        expected_records = SeqIO.to_dict(SeqIO.parse(path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-ins-middle.fsa'), 'fasta'))
        logger.debug('expected_seq=%s', expected_records['blaIMP-42_1_AB753456'].seq)
        logger.debug('actual_seq=%s', records['blaIMP-42_1_AB753456'].seq)
        self.assertEqual(expected_records['blaIMP-42_1_AB753456'].seq.upper(), records['blaIMP-42_1_AB753456'].seq, "records don't match")
    ...
    def testResfinderBetaLactamTwoCopies(self):
        ...
        self.assertAlmostEqual((result['%Overlap']), 100.0, places=2, msg='Wrong percent overlap')
        ...
        self.assertEqual(result['Predicted Phenotype'], 'ampicillin, amoxicillin/clavulanic acid, cefoxitin, ceftriaxone, meropenem', 'Wrong phenotype')
    ...
    def testPointfinderSalmonellaA67PSuccess(self):
        ...
        blast_handler = JobHandler({'resfinder':self.resfinder_database,  'pointfinder':pointfinder_database}, 2, self.blast_out.name)
        amr_detection = AMRDetectionResistance((self.resfinder_database), (self.resfinder_drug_table), blast_handler, (self.pointfinder_drug_table),
          pointfinder_database, output_dir=(self.outdir.name))
        ...
        self.assertEqual(result['Predicted Phenotype'].iloc[0], 'ciprofloxacin I/R, nalidixic acid', 'Wrong phenotype')
    ...
    def testPointfinderSalmonellaA67PFailPID(self):
        ...
        amr_detection = AMRDetectionResistance((self.resfinder_database), (self.resfinder_drug_table), blast_handler, (self.pointfinder_drug_table),
          pointfinder_database, output_dir=(self.outdir.name))
        files = [
         path.join(self.test_data_dir, 'gyrA-A67P.fsa')]
    ...
    def testPointfinderSalmonellaA67TFail(self):
        ...
        amr_detection = AMRDetectionResistance((self.resfinder_database), (self.resfinder_drug_table), blast_handler, (self.pointfinder_drug_table),
          pointfinder_database, output_dir=(self.outdir.name))
        files = [
         path.join(self.test_data_dir, 'gyrA-A67T.fsa')]
        amr_detection.run_amr_detection(files, 99, 99, 90, 90, 0, 0, 0, 0, 0)
        pointfinder_results = amr_detection.get_pointfinder_results()
        self.assertEqual(len(pointfinder_results.index), 0, 'Wrong number of rows in result')
        self.assertEqual(len(os.listdir(self.outdir.name)), 0, 'File found where none should exist')
    ...
    def testPointfinderSalmonella_16S_rrSD_C1065T_Success(self):
        ...
        self.assertEqual(result['Predicted Phenotype'].iloc[0], 'spectinomycin', 'Wrong phenotype')
    ...
    def testResfinderExcludeNonMatches(self):
        amr_detection = AMRDetectionResistance((self.resfinder_database), (self.resfinder_drug_table), (self.blast_handler), (self.pointfinder_drug_table),
          (self.pointfinder_database), include_negative_results=False,
          output_dir=(self.outdir.name))
    ...
    def testResfinderIncludeNonMatches(self):
        amr_detection = AMRDetectionResistance((self.resfinder_database), (self.resfinder_drug_table), (self.blast_handler), (self.pointfinder_drug_table),
          (self.pointfinder_database), include_negative_results=True,
          output_dir=(self.outdir.name))
        file_beta_lactam = path.join(self.test_data_dir, 'beta-lactam-blaIMP-42-mut-2.fsa')
        file_non_match = path.join(self.test_data_dir, 'non-match.fsa')
        files = [file_beta_lactam, file_non_match]
