...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Nomensa/django-scrub-pii/scrubpii/utils.py
# Compiled at: 2016-01-29 11:56:06
import uuid
...
OBFUSCATED_SOURCES = {'CharField': {'postgresql': 'substring(md5(random()::text) FROM 0 FOR 10)', 
                 'default': "'###########'"}, 
   'TextField': {'default': "'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce urna lorem, condimentum ut pretium commodo, iaculis eget lacus. In eget lorem varius erat auctor tempor. Pellentesque lacinia varius erat, sed eleifend magna consectetur et. Maecenas porttitor varius enim quis tincidunt. Maecenas feugiat ipsum a augue iaculis, a volutpat urna egestas. Cras molestie mauris non arcu tempor efficitur. Donec at felis lacus. Nulla posuere pretium magna at elementum.'"}, 
   'EmailField': {'sqlite': "'user-' || id || '@example.net'", 
                  'default': "concat('user-', id, '@example.net')"}, 
   'DateField': {'postgresql': "date '1990-06-02'", 
                 'sqlite': "date('1990-06-02')"}, 
   'DateTimeField': {'default': "timestamp '1990-06-02 10:00:00'", 
                     'sqlite': "datetime('1990-06-02 10:00:00')"}, 
   'GenericIPAddressField': {'postgresql': "CONCAT(\n                             '2001:db8:',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM  1 FOR 4), ':',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM  5 FOR 4), ':',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM  9 FOR 4), ':',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM 13 FOR 4), ':',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM 17 FOR 4), ':',\n                             SUBSTRING(md5(concat({field_name}, '{salt}')) FROM 21 FOR 4)\n                         )::inet", 
                             'default': "'2001:db8::1'"}, 
   'username': {'sqlite': "'user-' || id", 
                'default': "concat('user-', id)"}, 
   'password': {'default': ("'{0}'").format(make_password('password', hasher='md5'))}}
