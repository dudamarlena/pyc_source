...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\xicam\plugins\DataHandlerPlugin.py
...
class DataHandlerPlugin(IPlugin):
    ...
    def ingest(cls, paths):
        ...
        return {'start':cls._setTitle(cls.getStartDoc(paths, start_uid), paths),  'descriptors':list(cls.getDescriptorDocs(paths, start_uid, descriptor_uids)), 
...
def event_doc(data_uid: str, descriptor_uid: str, metadata: dict=None):
    if not metadata:
        metadata = {}
    metadata.update({'descriptor':descriptor_uid,  'time':datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Y'), 
     'uid':str(uuid.uuid4()), 
     'data':{'primary': data_uid}})
    return metadata


def embedded_local_event_doc(descriptor_uid: str, field: str, handler: type, handler_args: tuple=None, handler_kwargs: dict=None, metadata: dict=None):
    if not handler_args:
        handler_args = tuple()
    else:
    ...
    metadata.update(FillableDict({'descriptor':descriptor_uid,  'time':datetime.datetime.now(), 
     'uid':str(uuid.uuid4()), 
     'data':datafield}))
    return metadata
...
def descriptor_doc(start_uid: str, descriptor_uid: str, metadata: dict=None):
    if not metadata:
        metadata = {}
    metadata.update({'run_start':start_uid,  'name':'primary', 
     'uid':descriptor_uid})
    return metadata
...
def stop_doc(start_uid: str, metadata: dict=None):
    if not metadata:
        metadata = {}
    metadata.update({'run_start':start_uid,  'time':0, 
     'uid':str(uuid.uuid4()), 
     'exit_status':'success'})
    return metadata
