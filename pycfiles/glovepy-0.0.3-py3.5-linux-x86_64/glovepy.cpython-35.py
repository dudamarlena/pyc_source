...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/glovepy/glovepy.py
# Compiled at: 2017-08-18 15:49:51
...
import threading, numpy as np, logging, scipy, pyximport
...
class Glove(object):
    ...
    def _similarity_query(self, word_vec, topn):
        ...
        """
        dst = np.dot(self.W, word_vec) / np.linalg.norm(self.W, axis=1) / np.linalg.norm(word_vec)
    ...
    def most_similar(self, positive=[], negative=[], topn=5, word_norm=None):
        ...
        if isinstance(positive, str) and not negative:
            positive = [positive]
        positive = [(word, 1.0) if isinstance(word, (str, np.ndarray)) else word for word in positive]
        negative = [(word, -1.0) if isinstance(word, (str, np.ndarray)) else word for word in negative]
    ...
    def most_similar_cosmul(self, positive=[], negative=[], topn=5, word_norm=None):
        ...
        if isinstance(positive, str) and not negative:
            positive = [positive]
        all_words = set([self.word2id[word] for word in positive + negative if not isinstance(word, np.ndarray) and word in self.word2id])
        if word_norm is not None:
            logger.info('Provided normalized word embedding matrix')
            word_vec = word_norm
        else:
            ...
            word_vec = self.word_vectors
        positive = [self.word_vec(word, word_vec) if isinstance(word, str) else word for word in positive]
        negative = [self.word_vec(word, word_vec) if isinstance(word, str) else word for word in negative]
        if not positive:
            raise ValueError('cannot compute similarity with no input')
        ...
        dists = np.prod(pos_dists, axis=0) / (np.prod(neg_dists, axis=0) + 1e-06)
        if not topn:
            return dists
        best = np.argsort(dists)[::-1][:topn + len(all_words)]
        result = [(self.id2word[sim], float(dists[sim])) for sim in best if sim not in all_words][:topn]
