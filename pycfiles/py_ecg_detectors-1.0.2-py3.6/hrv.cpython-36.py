...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hrv.py
# Compiled at: 2019-12-20 05:38:30
# Size of source mod 2**32: 9858 bytes
"""
...
class HRV:
    ...
    def SDANN(self, rr_samples, average_period=5.0, normalise=False):
        ...
        for i in range(sections):
            idx = np.where((rr_samples >= i * average_period_samples) & (rr_samples < (i + 1) * average_period_samples))
            idx = idx[0]
    ...
    def fAnalysis(self, rr_samples):
        ...
        nsamp = 1000
        self.t_hr_linear = np.linspace((self.t_hr_discrete[1]), (self.t_hr_discrete[(len(self.t_hr_discrete) - 2)]),
          num=nsamp)
        duration = self.t_hr_discrete[(len(self.t_hr_discrete) - 2)] - self.t_hr_discrete[1]
        self.hr_linear = self.hr_func(self.t_hr_linear)
        model = LombScargleFast().fit(self.t_hr_discrete, self.hr_discrete, 0.01)
        fmax = 1
        fmin = 0.01
        df = (fmax - fmin) / nsamp
        ...
        for i in range(0, int(nsamp / 2)):
            if self.f_hr_axis[i] >= 0.04:
                if self.f_hr_axis[i] <= 0.15:
