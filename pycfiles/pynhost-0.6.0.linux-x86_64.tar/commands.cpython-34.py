...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynhost/commands.py
# Compiled at: 2015-07-19 13:40:13
        ...
        while self.remaining_words:
            ...
            if rule_match is not None:
                ...
                if action_list.contains_non_repeat_actions():
                    action_list.actions = gram_handler.triggered['match']['before'] + action_list.actions + gram_handler.triggered['match']['after']
                self.remaining_words = rule_match.remaining_words
                utilities.log_message(log_handler, 'info', 'Input "{}" matched rule {} in grammar {}'.format(' '.join(rule_match.matched_words), rule_match.rule, rule_match.rule.grammar))
            else:
...
class ActionList:
    ...
    def add_string(self, text, gram_handler):
        if self.command.action_lists:
            if self.command.action_lists[(-1)].rule_match is None:
                self.actions.append(' ')
