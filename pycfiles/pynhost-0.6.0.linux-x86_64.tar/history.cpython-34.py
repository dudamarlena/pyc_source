...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynhost/history.py
# Compiled at: 2015-07-19 13:35:00
...
class CommandHistory:
            ...
            if not self.execute_string_or_func(action):
                if isinstance(action, int):
                    ...
                    if isinstance(action, dynamic.RepeatCommand):
                        count = action.count.evaluate(action_list.rule_match) if isinstance(action.count, dynamic.Num) else count
    ...
    def execute_string_or_func(self, action):
        if isinstance(action, str):
            ...
            return True
        if isinstance(action, commands.CallableWrapper):
