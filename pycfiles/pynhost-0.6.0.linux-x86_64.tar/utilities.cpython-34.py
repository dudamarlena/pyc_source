...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynhost/utilities.py
# Compiled at: 2015-07-12 13:51:34
# Size of source mod 2**32: 5961 bytes
import configparser, argparse, os, shutil, re, sys, copy, pynhost, logging
...
def get_cl_args():
    ...
    parser.add_argument('-d', '--debug', help='Enable text input for grammar debugging', action='store_true')
    parser.add_argument('--debug_delay', help='Delay (seconds) in debug mode between text being entered and run', type=check_negative, default=constants.DEFAULT_DEBUG_DELAY)
...
def create_logging_handler(verbal_mode, logging_dir):
    ...
    my_handler = RotatingFileHandler(log_file, mode='a', maxBytes=5242880, backupCount=2, encoding=None, delay=0)
