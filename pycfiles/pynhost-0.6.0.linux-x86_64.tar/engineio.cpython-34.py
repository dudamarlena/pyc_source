...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.4/dist-packages/pynhost/engineio.py
# Compiled at: 2015-07-19 15:02:22
# Size of source mod 2**32: 5173 bytes
import subprocess, os, sys, time, re, socket, socketserver, threading, copy
...
class SphinxEngine(BaseEngine):
    ...
    def get_lines(self):
        ...
        commands = {'-hmm': self.hmm_directory, 
         '-lm': self.lm_filename, 
        ...
        with subprocess.Popen(full_command, stdout=subprocess.PIPE, stderr=null, bufsize=1, universal_newlines=True) as (p):
            for line in p.stdout:
                split_line = line.rstrip('\n').split(' ')
                if split_line[0] == 'READY....':
                    if not self.loaded:
                        self.loaded = True
                        print('Ready!')
...
class SharedDirectoryEngine(BaseEngine):
    ...
    def get_buffer_lines(self):
        files = sorted([f for f in os.listdir(self.shared_dir) if not os.path.isdir(f) and re.match('o\\d+$', f)])
        lines = []
        for fname in files:
    ...
    def filter_duplicate_letters(self, line):
        ...
        for word in line.split():
            ...
            for i, char in enumerate(word):
                if char.islower() or i in [0, len(word) - 1] or char.lower() != word[(i + 1)] or not char.isalpha():
                    new_word += char
                    continue
