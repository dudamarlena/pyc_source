...
# Compiled at: 2019-12-13 02:39:46
...
class MailboxHandler(Mailbox):
    ...
    def handle_message(self, message):
        if message['Date'] is None:
            message.set_date(dt.datetime.now())
        local_message_id = self.mailbox.add(message)
        mailbox_message = self.mailbox[local_message_id]
        mailbox_message.add_flag('S')
        self.mailbox[local_message_id] = mailbox_message
