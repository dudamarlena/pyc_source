...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/examples/router/test_router.py
# Compiled at: 2013-05-10 12:28:11
"""
...
from ginsfsm.globals import list_globals
...
def ac_message(self, event):
    ...
    self.counter += 1
    if self.counter > 4:
        ret = self.gaplic.unsubscribe_event_outside('TOTO', '', 'sample2', 'EV_MESSAGE', self, __subscription_reference__=event.__subscription_reference__)
        if not ret:
            print 'Eiiiiiiiiiiiiiiiiiiiiii something wrong!'
    self.gaplic.send_event_outside('TOTO', '', 'sample2', 'EV_SET_TIMEOUT', self, timeout=2)


SAMPLE1_FSM = {'event_list': ('EV_MESSAGE: top input', ), 
   'state_list': ('ST_IDLE', ), 
   'machine': {'ST_IDLE': (
                         (
...
class Sample1(GObj):
    ...
    *Output-Events:*
    ...
    def start_up(self):
        """
        """
        ret = self.gaplic.subscribe_event_outside('TOTO', '', 'sample2', 'EV_MESSAGE', self)
        if not ret:
            print 'Eiiiiiiiiiiiiiiiiiiiiii something wrong!'
        self.gaplic.send_event_outside('TOTO', '', 'sample2', 'EV_SET_TIMEOUT', self, timeout=2)
...
SAMPLE2_FSM = {'event_list': ('EV_MESSAGE: top output', 'EV_TIMEOUT: bottom input', 'EV_SET_TIMEOUT: top input'), 
   'state_list': ('ST_IDLE', ), 
   'machine': {'ST_IDLE': (
                         (
...
def gaplic1(global_config, **local_conf):
    ...
    ga.create_gobj('sample1', Sample1, ga, __unique_name__=True)
    return ga
...
def gaplic2(global_config, **local_conf):
    ...
    ga.create_gobj('sample2', Sample2, ga, __unique_name__=True)
    return ga
