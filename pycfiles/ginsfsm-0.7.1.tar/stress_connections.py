...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/examples/essential_gobjs/stress_connections.py
# Compiled at: 2013-04-19 11:50:15
"""
...
SERVER_FSM = {'event_list': ('EV_SET_TIMER: bottom output', 'EV_TIMEOUT: bottom input', 'EV_CONNECTED: bottom input',
...
SERVER_GCONFIG = {'verbose': [
             int, 0, 0, None, 'Increase output verbosity. Values [0,1,2]'], 
   'connections': [
                 int, 0, 0, None, 'Limit of connections to be reached.']}
...
class OnServer(GObj):
    ...
    *Output-Events:*
    ...
    def start_up(self):
        ...
        self.server = self.create_gobj(None, GServerSock, self, host='127.0.0.1', port=8000, rx_data_event_name=None, transmit_ready_event_name=None)
        self.set_timeout(5)
        return
...
def ac_client_timeout(self, event):
    if self.connex is None:
        ...
        for i in self.connex:
            self.connex[i] = self.create_gobj('client-%02d' % i, GConnex, self, destinations=[
             ('127.0.0.1', 8000)], rx_data_event_name=None, transmit_ready_event_name=None)
...
CLIENT_FSM = {'event_list': ('EV_SET_TIMER: bottom output', 'EV_TIMEOUT: bottom input', 'EV_CONNECTED: bottom input',
 'EV_DISCONNECTED: bottom input'), 
   'state_list': ('ST_IDLE', ), 
   'machine': {'ST_IDLE': (
                         (
...
if __name__ == '__main__':
    ...
    parser.add_argument('connections', type=int, nargs='?', default=400, help='Limit of connections to be reached.')
    parser.add_argument('--run-as-process', action='store_true', help='Run the server as subprocess. By default it runs as thread.')
    parser.add_argument('-v', '--verbose', help='Increase output verbosity', type=int, choices=[
     0, 1, 2], default=0)
    args = parser.parse_args()
    run_as_process = args.run_as_process
    local_conf = {'GObj.trace_mach': True if args.verbose else False, 
       'GObj.logger': logging}
    if run_as_process:
        ga_srv = GAplic(name='Server', roles='', **local_conf)
        ga_srv.create_gobj('server', OnServer, ga_srv, connections=args.connections, verbose=args.verbose)
        srv_worker = setup_gaplic_process(ga_srv)
        srv_worker.start()
        ga_cli = GAplic(name='Client', roles='', **local_conf)
        ga_cli.create_gobj('client', OnClient, ga_cli, connections=args.connections, verbose=args.verbose)
        try:
            ga_cli.start()
