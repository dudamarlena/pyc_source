...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/examples/essential_gobjs/ongconnex.py
...
ONGCONNEX_FSM = {'event_list': ('EV_SET_TIMER: bottom output', 'EV_TIMEOUT:bottom input', 'EV_RX_DATA:bottom input'), 
   'state_list': ('ST_IDLE', ), 
   'machine': {'ST_IDLE': (
                         (
...
ONGCONNEX_GCONFIG = {'verbose': [
             int, 0, 0, None, 'Increase output verbosity. Values [0,1,2]'], 
   'url1': [
          str, 'www.google.com', 0, None, 'Url one to connect.'], 
   'url2': [
          str, 'www.twitter.com', 0, None, 'Url two to connect.']}
...
class OnGConnex(GObj):
    ...
    *Output-Events:*
    ...
    def start_up(self):
        ...
        self.connex = self.create_gobj('connex', GConnex, self, destinations=[
         (
          self.config.url1, 80),
         (
          self.config.url2, 80)], timeout_inactivity=5, connected_event_name=None, disconnected_event_name=None, transmit_ready_event_name=None)
        self.set_timeout(5)
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('url1', nargs='?', default='www.google.es', help='First url to connect.')
    parser.add_argument('url2', nargs='?', default='www.twitter.es', help='Second url to connect.')
    ...
    ga.create_gobj('ongconnex', OnGConnex, ga, verbose=args.verbose, url1=args.url1, url2=args.url2)
    try:
        ga.start()
