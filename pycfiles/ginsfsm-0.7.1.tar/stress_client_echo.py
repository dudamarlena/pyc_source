...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/examples/essential_gobjs/stress_client_echo.py
...
def ac_client_timeout(self, event):
    ...
    if self.n_clients < self.config.connections:
        for i in list(range(self.n_clients, self.config.connections)):
            self.n_clients += 1
            self.connex[i] = self.create_gobj('client-%d' % i, GConnex, self, destinations=[
             (
              self.config.host, self.config.port)], transmit_ready_event_name=None)
            self.connex[i].idx = i
    ...
    if self.n_connected_clients == self.config.connections:
        ...
        for cli in self.connex:
            if cli.gsock.connected:
                ...
                self.send_event(cli, 'EV_SEND_DATA', data=query)
...
CLIENT_FSM = {'event_list': ('EV_SET_TIMER: bottom output', 'EV_TIMEOUT: bottom input', 'EV_CONNECTED: bottom input',
 'EV_DISCONNECTED: bottom input', 'EV_RX_DATA:bottom input'), 
   'state_list': ('ST_IDLE', ), 
   'machine': {'ST_IDLE': (
                         (
...
CLIENT_GCONFIG = {'verbose': [
   ...
   'host': [
          str, '127.0.0.1', 0, None, 'Port.'], 
   'port': [
          int, 8000, 0, None, 'Port.']}
...
if __name__ == '__main__':
    ...
    parser.add_argument('host', nargs='?', default='127.0.0.1', help='Host.')
    parser.add_argument('port', type=int, nargs='?', default=8000, help='Port.')
    parser.add_argument('connections', type=int, nargs='?', default=100, help='Limit of connections to be reached.')
    ...
    ga_cli.create_gobj('client', OnClient, ga_cli, host=args.host, port=args.port, connections=args.connections)
    try:
        ga_cli.start()
