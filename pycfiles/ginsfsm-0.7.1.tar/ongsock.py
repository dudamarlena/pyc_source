...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/examples/essential_gobjs/ongsock.py
# Compiled at: 2013-04-21 14:11:19
"""
GObj :class:`OnGSock`
=====================
...
ONGSOCK_FSM = {'event_list': ('EV_SET_TIMER: bottom output', 'EV_TIMEOUT:bottom input', 'EV_CONNECTED:bottom input',
 'EV_DISCONNECTED:bottom input', 'EV_RX_DATA:bottom input', 'EV_TRANSMIT_READY:bottom input'), 
   'state_list': ('ST_DISCONNECTED', 'ST_WAIT_CONNECTED', 'ST_CONNECTED'), 
   'machine': {'ST_DISCONNECTED': (
                                 (
...
ONGSOCK_GCONFIG = {'verbose': [
             int, 0, 0, None, 'Increase output verbosity. Values [0,1,2]'], 
   'seconds': [
             int, 5, 0, None, 'Seconds to repeat the connection.'], 
   'url': [
         str, 'www.google.com', 0, None, 'Url to connect.']}
...
class OnGSock(GObj):
    ...
    *Output-Events:*
    ...
    def start_up(self):
        """ Initialization zone."""
        self.timer = self.create_gobj(None, GTimer, self, timeout_event_name='EV_TIMEOUT')
        self.timer.subscribe_event('EV_TIMEOUT', self)
        self.gsock = self.create_gobj(None, GSock, self, connected_event_name='EV_CONNECTED', disconnected_event_name='EV_DISCONNECTED', host=self.config.url, port=80)
        self.set_timeout(1)
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('seconds', type=int, nargs='?', default=2, help='Seconds to repeat the url.')
    parser.add_argument('url', nargs='?', default='www.google.com', help='Url to connect.')
    ...
    ga.create_gobj('ongsock', OnGSock, ga, verbose=args.verbose, seconds=args.seconds, url=args.url)
    try:
        ga.start()
