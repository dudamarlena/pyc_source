...
# [GCC 8.4.0]
# Embedded file name: /home/gines/midesarrollo/hg-projects/ginsfsm/ginsfsm/scaffolds/copydir.py
# Compiled at: 2012-07-11 15:59:09
# Size of source mod 2**32: 10536 bytes
import os, sys, pkg_resources
from ginsfsm.compat import input_, native_, url_quote as compat_url_quote, escape
...
def copy_dir(source, dest, vars, verbosity, simulate, indent=0, sub_vars=True, interactive=False, overwrite=True, template_renderer=None, out_=sys.stdout):
    ...
    for name in names:
        ...
        if use_pkg_resources and pkg_resources.resource_isdir(source[0], full):
            ...
            copy_dir((source[0], full), dest_full, vars, verbosity, simulate, indent=indent + 1, sub_vars=sub_vars, interactive=interactive, template_renderer=template_renderer, out_=out_)
            continue
        else:
            if not use_pkg_resources and os.path.isdir(full):
                if verbosity:
                    out('%sRecursing into %s' % (pad, os.path.basename(full)))
                copy_dir(full, dest_full, vars, verbosity, simulate, indent=indent + 1, sub_vars=sub_vars, interactive=interactive, template_renderer=template_renderer, out_=out_)
                continue
            else:
                if use_pkg_resources:
                ...
                if already_exists:
                    ...
                    if old_content == content:
                        ...
                        continue
                    if interactive:
                        if not query_interactive(native_(full, fsenc), native_(dest_full, fsenc), native_(content, fsenc), native_(old_content, fsenc), simulate=simulate, out_=out_):
                            continue
                    elif not overwrite:
                        continue
                if verbosity and use_pkg_resources:
                    ...
                    out('%sCopying %s to %s' % (pad, os.path.basename(full),
                     dest_full))
        if not simulate:
            f = open(dest_full, 'wb')
            f.write(content)
...
def query_interactive(src_fn, dest_fn, src_content, dest_content, simulate, out_=sys.stdout):
    global all_answer
