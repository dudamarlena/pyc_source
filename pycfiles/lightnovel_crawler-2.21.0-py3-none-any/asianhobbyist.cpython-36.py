...
# [GCC 8.4.0]
# Embedded file name: ./src/sources/asianhobbyist.py
# Compiled at: 2020-03-25 09:13:24
# Size of source mod 2**32: 1950 bytes
import logging, re
from ..utils.crawler import Crawler
logger = logging.getLogger('ASIAN_HOBBYIST')
...
class AsianHobbyistCrawler(Crawler):
    ...
    def read_novel_info(self):
        ...
        self.novel_title = soup.select_one('#content article .post-title.entry-title a').text
        logger.info('Novel title: %s', self.novel_title)
        self.novel_author = 'N/A'
        logger.info('Novel author: %s', self.novel_author)
        self.novel_cover = self.absolute_url(soup.select_one('#content article p img')['src'])
