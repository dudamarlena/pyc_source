...
# [GCC 8.4.0]
# Embedded file name: .\src\sources\myoniyonitrans.py
# Compiled at: 2020-03-25 09:13:24
# Size of source mod 2**32: 4090 bytes
import logging, re
from urllib.parse import urlparse
from utils.crawler import Crawler
logger = logging.getLogger('MYONIYONI_TRANSLATION')
...
class MyOniyOniTranslation(Crawler):
    ...
    def read_novel_info(self):
        ...
        if not soup.select_one('article.page.type-page'):
            ...
            self.novel_title = soup.select_one('header.entry-header h1.entry-title').text
            logger.info('Novel title: %s', self.novel_title)
            possible_authors = []
            for p in soup.select('.x-container .x-column.x-1-2 p'):
                if re.match('author|trans|edit', p.text, re.I):
                    possible_authors.append(p.text.strip())
                if len(possible_authors):
                    self.novel_author = ', '.join(possible_authors)
                logger.info('Novel author: %s', self.novel_author)
                self.novel_cover = self.absolute_url(soup.select_one('.x-container img.x-img')['src'])
                ...
                if accordian:
                    for div in accordian:
                        ...
                        for chap in div.select('.x-accordion-body a'):
                            self.chapters.append({'volume':vol_id, 
