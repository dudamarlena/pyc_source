...
# Embedded file name: build/bdist.linux-x86_64/egg/pynats2/client.py
# Compiled at: 2019-10-24 22:38:16
# Size of source mod 2**32: 19394 bytes
import json, logging, queue, re, socket, ssl
...
from pynats2.exceptions import NATSConnectionError, NATSInvalidResponse, NATSInvalidSchemeError, NATSInvalidUrlError, NATSRequestTimeoutError, NATSTCPConnectionRequiredError, NATSTLSConnectionRequiredError, NATSUnexpectedResponse
...
DEFAULT_WORKERS = 3
INFO_RE = re.compile('^INFO\\s+([^\\r\\n]+)\\r\\n')
PING_RE = re.compile('^PING\\r\\n')
PONG_RE = re.compile('^PONG\\r\\n')
MSG_RE = re.compile('^MSG\\s+(?P<subject>[^\\s\\r\\n]+)\\s+(?P<sid>[^\\s\\r\\n]+)\\s+(?P<reply>([^\\s\\r\\n]+)[^\\S\\r\\n]+)?(?P<size>\\d+)\\r\\n')
OK_RE = re.compile('^\\+OK\\s*\\r\\n')
ERR_RE = re.compile("^-ERR\\s+('.+')?\\r\\n")
_CRLF_ = '\r\n'
_SPC_ = ' '
COMMANDS = {INFO_OP: INFO_RE, 
 PING_OP: PING_RE, 
 PONG_OP: PONG_RE, 
 MSG_OP: MSG_RE, 
 OK_OP: OK_RE, 
 ERR_OP: ERR_RE}
INBOX_PREFIX = bytearray('_INBOX.')
...
class NATSSubscription:
    ...
    def is_wasted(self):
        return self.max_messages is not None and self.received_messages == self.max_messages
...
class NATSNoSubscribeClient:
    __slots__ = ('_conn_options', '_socket', '_socket_buffer', '_socket_options', '_subs_queue',
                 '_ssid', '_subs', '_nuid', '_ping_interval', '_waiter', '_waiter_enabled',
                 '_vhost_name', '_vhost_len')

    def __init__(self, url: str='nats://127.0.0.1:4222', *, name: str='pynats2', verbose: bool=False, pedantic: bool=False, tls_cacert: Optional[str]=None, tls_client_cert: Optional[str]=None, tls_client_key: Optional[str]=None, tls_verify: bool=False, socket_timeout: float=DEFAULT_SOCKET_TIMEOUT, socket_keepalive: bool=False, ping_interval: float=DEFAULT_PING_INTERVAL) -> None:
        try:
            parsed = urlparse(url)
        except ValueError:
            raise NATSInvalidUrlError(url)

        self._conn_options = {'hostname':parsed.hostname,  'port':parsed.port, 
        ...
        if parsed.scheme == 'nats':
            ...
            if parsed.scheme == 'tls':
                ...
                raise NATSInvalidSchemeError('got unsupported URI scheme: %s' % parsed.scheme)
        vhost = parsed.path.strip('/').replace('/', '.')
        if len(vhost) > 0:
            vhost += '.'
        self._vhost_name = vhost
        self._vhost_len = len(vhost)
        self
        self._socket_buffer = ''
        self._socket_options = {'timeout':socket_timeout, 
    ...
    def _send_connect_command(self) -> None:
        options = {'name':self._conn_options['name'], 
        ...
        if self._conn_options['username']:
            if self._conn_options['password']:
    ...
    def _connect_tls(self) -> None:
        ...
        if self._conn_options['tls_verify']:
            ...
            if self._conn_options['tls_client_cert'] is not None:
                if self._conn_options['tls_client_key'] is not None:
                    ctx.load_cert_chain(certfile=(str(self._conn_options['tls_client_cert'])),
    ...
    def publish(self, subject: str, *, payload: bytes='', reply: str='') -> None:
        self._send(PUB_OP, self._vhost(subject), self._vhost(reply), len(payload))
        self._send(payload)

    def request(self, subject: str, *, payload: bytes='', timeout: float=DEFAULT_REQUEST_TIMEOUT) -> Optional[NATSMessage]:
    ...
    def _recv(self, *commands: Pattern[bytes]) -> Union[(Tuple[(Pattern[bytes], Match[bytes])], Tuple[(None, None)])]:
        try:
            line = self._readline()
    ...
    def _get_command(self, line: bytes) -> Optional[Pattern[bytes]]:
        values = self._strip(line).split(' ', 1)
    ...
    def _recv_message(self, result: Match[bytes]) -> NATSMessage:
        ...
        message = NATSMessage(sid=(int(message_data['sid'].decode())),
          subject=(self._del_vhost(message_data['subject'].decode())),
          reply=(self._del_vhost(message_data['reply'].decode()) if message_data['reply'] else ''),
...
class NATSClient(NATSNoSubscribeClient):
    __slots__ = ('_conn_options', '_socket', '_socket_buffer', '_socket_options', '_subs_queue',
                 '_send_lock', '_ssid', '_subs', '_nuid', '_ping_interval', '_waiter',
                 '_waiter_enabled', '_vhost_name', '_vhost_len', '_pinger', '_pinger_timer',
                 '_workers')

    def __init__(self, url='nats://127.0.0.1:4222', *, name='pynats2', verbose=False, pedantic=False, tls_cacert=None, tls_client_cert=None, tls_client_key=None, tls_verify=False, socket_timeout=DEFAULT_SOCKET_TIMEOUT, socket_keepalive=False, ping_interval=DEFAULT_PING_INTERVAL, workers=DEFAULT_WORKERS):
        super().__init__(url,
          name=name,
          verbose=verbose,
          pedantic=pedantic,
        ...
        self._subs = {}
        self._subs_queue = queue.Queue()
        self._send_lock = RLock()
        self._waiter = None
        self._waiter_enabled = False
        self._pinger = None
        self._pinger_timer = Event()
        self._workers = None
    ...
    def _start_workers(self):
        self._workers = ThreadPoolExecutor(max_workers=(self._conn_options['workers']),
    ...
    def subscribe(self, subject: str, *, callback: Callable, queue: str='', max_messages: Optional[int]=None) -> NATSSubscription:
        sub = NATSSubscription(sid=(self._ssid),
          subject=subject,
          queue=queue,
          callback=callback,
    ...
    def request(self, subject: str, *, payload: bytes='', timeout: float=DEFAULT_REQUEST_TIMEOUT) -> NATSMessage:
        next_inbox = INBOX_PREFIX[:]
        next_inbox.extend(self._nuid.next_())
        reply_subject = next_inbox.decode()
