...
class Interface(object):
    ...
    def connect(self, database_type, source):
        ...
        if connection.type == self.db.ODBC():
            ...
            if connection.type == self.db.POSTGRES() and connection.dsn.engine.upper() == self.db.POSTGRES():
                ...
                if connection.type == self.db.MYSQL() and connection.dsn.engine.upper() == self.db.MYSQL():
                    ...
                    if connection.type == self.db.SQLITE() and connection.dsn.engine.upper() == self.db.SQLITE():
                        ...
                        if connection.type == self.db.ORACLE():
