...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ast2000tools/solar_system.py
...
class SolarSystem(object):
    ...
    def semi_major_axes(self):
        """1-D :class:`numpy.ndarray`: Array of shape (`number_of_planets`,) containing the semi-major axis of each planet in astronomical units."""
    ...
    def verify_planet_positions(self, simulation_duration, planet_positions, filename=None, number_of_output_points=None):
        ...
        filename : str, optional
            ...
            They are stored as a tuple (times, planet_positions) in the binary NumPy .npy format.
            By default, no file is generated.
        number_of_output_points : int, optional
            The number of exact trajectory points to include in the output .npy file (if applicable).
        ...
        """
        planet_positions = np.asfarray(planet_positions)
        ...
        if filename is not None:
            ...
            with open(filename, 'wb') as (f):
                np.save(f, (times, correct_planet_positions))
            if self.verbose:
    ...
    def generate_orbit_video(self, times, planet_positions, number_of_frames=None, reduce_other_periods=True, filename='orbit_video.xml'):
        ...
        system_xml = _SolarSystemXML(self, adjust_moon_orbital_speeds=reduce_other_periods, adjust_moon_rotational_speeds=reduce_other_periods, visualization_program='SSView')
        objects = system_xml._create_objects_element(output_times, output_planet_positions, output_rotation_angles, planet_indices)
        system_xml._write_to_file(filename, [objects], verbose=self.verbose)
        return
    ...
    def generate_binary_star_orbit_video(self, times, planet_positions, star_1_positions, star_2_positions, number_of_frames=1000, filename='binary_star_video.xml'):
        ...
        star_1_positions = np.asfarray(star_1_positions)
        ...
        star_2_positions = np.asfarray(star_2_positions)
    ...
    def generate_landing_video(self, times, lander_positions, planet_idx, initial_system_time=0, number_of_frames=1000, filename='landing_video.xml'):
        ...
        if number_of_times < 100:
            raise ValueError('The number of provided times is %d but must be at least 100.' % number_of_times)
        lander_positions = np.asfarray(lander_positions)
        ...
        output_lander_positions = output_planet_trajectories[:, planet_idx, :] + resampled_lander_positions * self._compute_lander_position_scales(resampled_lander_positions)
        system_xml = _SolarSystemXML(self, visualization_program='SSView')
    ...
    def _compute_lander_position_scales(self, positions):
        ...
        """
        distances = np.linalg.norm(positions * 0.001, axis=0)
        scales = 0.001 * distances ** 0.75 / (25000 * distances)
    ...
    def _init_star(self):
        ...
        self._star_mass = np.amax([self.star_temperature ** 2 / 33640000.0 * self._random_state.normal(1, 0.1), 0.1])
        self._star_radius = np.abs(self.star_mass ** 0.724 * 700000.0 * self._random_state.normal(1, 0.1))
        self._star_color = utils._convert_temperature_to_RGB(self.star_temperature)

    def _init_planets(self):
        eps = 0.25
        self._number_of_planets = self._random_state.randint(7, 9)
        self._semi_major_axes = np.zeros(self.number_of_planets)
        self._eccentricities = np.zeros(self.number_of_planets)
        self._masses = np.zeros(self.number_of_planets)
        self._radii = np.zeros(self.number_of_planets)
        self._initial_orbital_angles = np.zeros(self.number_of_planets)
        self._aphelion_angles = np.zeros(self.number_of_planets)
        self._rotational_periods = np.zeros(self.number_of_planets)
        self._initial_positions = np.zeros((2, self.number_of_planets))
        self._initial_velocities = np.zeros((2, self.number_of_planets))
        self._atmospheric_densities = np.zeros(self.number_of_planets)
        self._mean_molecular_masses = np.zeros(self.number_of_planets)
        self._types = []
        for i in range(self.number_of_planets):
            if i == 0:
                ...
                densi = self._random_state.uniform(4500.0, 6500)
                self._rotational_periods[i] = self._random_state.uniform(0.8, 1.3)
                self._masses[i] = self._random_state.uniform(1.0, 11) * 1e-06
                self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                ...
                densi = self._random_state.uniform(3000.0, 6500)
                ...
                if self.initial_orbital_angles[i] > self.aphelion_angles[i]:
                    if self.initial_orbital_angles[i] > self.aphelion_angles[i] + const.pi:
                        ...
                        vParr = -np.sqrt(v ** 2 - vPerp ** 2)
                elif self.initial_orbital_angles[i] < self.aphelion_angles[i] - const.pi:
                    vParr = -np.sqrt(v ** 2 - vPerp ** 2)
                else:
                    vParr = np.sqrt(v ** 2 - vPerp ** 2)
                self._initial_velocities[(0, i)] = -vPerp * np.sin(self.initial_orbital_angles[i]) + vParr * np.cos(self.initial_orbital_angles[i])
                ...
                self._semi_major_axes[i] = self._random_state.uniform(0.2 * self.semi_major_axes[0], 8 * self.semi_major_axes[1])
                ok = True
                count = 1.0
                while ok:
                    ok = False
                    ...
                    for k in range(i):
                        while np.abs(self.semi_major_axes[i] - self.semi_major_axes[k]) < eps * (self.semi_major_axes[i] + self.semi_major_axes[k]) * 0.5:
                            self.semi_major_axes[i] = self._random_state.uniform(0.2 * self.semi_major_axes[0], 8 * count * self.semi_major_axes[1])
                            ok = True
                ...
                self._initial_positions[(1, i)] = R0 * np.sin(self.initial_orbital_angles[i])
                r_vect = (
                 utils.AU_to_m(self.initial_positions[(0, i)]), utils.AU_to_m(self.initial_positions[(1, i)]))
                T_planet = self.star_temperature * np.sqrt(self.star_radius * 1000 / (2 * np.linalg.norm(r_vect)))
                earth_radius = 6371 * 1000
                if T_planet < 170:
                    R = (self.star_temperature / T_planet) ** 2 * (self.star_radius / 2)
                    R_max = (self.star_temperature / 170) ** 2 * (self.star_radius / 2) * 4
                    if R < R_max:
                        prob = self._random_state.uniform(0, 1)
                        if prob < 0.3:
                            self._types.append('rock')
                            self._masses[i] = 10 ** (-self._random_state.uniform(7.5, 8.5))
                            densi = self._random_state.uniform(2000.0, 6500)
                            self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                            self._atmospheric_densities[i] = self._random_state.uniform(1, 1.5)
                        elif prob < 0.31:
                            self._types.append('gas')
                            self._masses[i] = 10 ** (-self._random_state.uniform(5, 6.5))
                            densi = self._random_state.uniform(400.0, 2000)
                            self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                            ...
                            densi = self._random_state.uniform(500.0, 1000) + 1000 * max(np.log10(3500 * self.masses[i]), 0)
                            self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                            self._atmospheric_densities[i] = self._random_state.uniform(18, 25)
                    else:
                        prob = self._random_state.uniform(0, 1)
                    ...
                    if prob < 0.96:
                        ...
                        densi = self._random_state.uniform(4000.0, 6500)
                        self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                        self._atmospheric_densities[i] = self._random_state.uniform(1, 1.5)
                    else:
                        self._types.append('gas')
                        self._masses[i] = 10 ** (-self._random_state.uniform(2 - np.log10(self.star_mass), 4.5))
                        densi = self._random_state.uniform(600.0, 2500)
                        self._radii[i] = (3 * self.masses[i] * const.m_sun / (4 * const.pi * densi)) ** 0.3333333333333333 * 0.001
                        self._atmospheric_densities[i] = self._random_state.uniform(18, 25)
    ...
    def _compute_gravitational_acceleration(self, displacement, mass):
        ...
        """
        displacement = np.asfarray(displacement)
    ...
    def _compute_single_planet_trajectory(self, times, planet_idx):
        ...
        distances = self.semi_major_axes[planet_idx] * (1 - self.eccentricities[planet_idx] ** 2) / (1 - self.eccentricities[planet_idx] * np.cos(orbital_angles - self.aphelion_angles[planet_idx]))
    ...
    def _compute_single_planet_velocity(self, time, planet_idx, dt=1e-08):
    ...
    def _compute_planet_velocities(self, time, dt=1e-08):
    ...
    def _compute_single_planet_orbital_angle_evolution(self, times, planet_idx):
        ...
        iterations = 1
        x0 = times * sqt + self.initial_orbital_angles[planet_idx]
        x00 = x0
        ff = self._compute_angle_integral(x0 - self.aphelion_angles[planet_idx], self.eccentricities[planet_idx])
        ffo = self._compute_angle_integral(self.initial_orbital_angles[planet_idx] - self.aphelion_angles[planet_idx], self.eccentricities[planet_idx])
        lenmask = 1
        try:
    ...
    def _compute_planet_rotational_angle_evolutions(self, times, speed_adjustments=1):
        ...
        """
        times = np.asfarray(times)
        rotation_frequencies = speed_adjustments / utils.day_to_s(self.rotational_periods)
    ...
    def _generate_spacecraft_video(self, times, positions, camera_angles, planet_idx=None, last_valid_camera_time=None, filename='spacecraft_video.xml'):
        ...
        """
        times = np.asfarray(times)
        camera_angles = np.asfarray(camera_angles)
        positions = np.asfarray(positions)
        if last_valid_camera_time is None:
            last_valid_camera_time = np.amax(times) + 1
        ...
        if planet_idx is None:
            ...
            assert 0 <= planet_idx < self.number_of_planets, 'invalid planet index %d' % planet_idx
    ...
    def _generate_spacecraft_picture(self, time, position, camera_angle, planet_idx=None, filename='spacecraft_picture.xml'):
        ...
        """
        self._generate_spacecraft_video(np.asfarray([time]), np.asfarray(position)[:, np.newaxis], np.asfarray(camera_angle)[:, np.newaxis], planet_idx=planet_idx, filename=filename)
...
class _Moons(object):
    ...
    def __init__(self, system, planet_idx):
        ...
        self._radii = np.zeros(number_of_moons)
        self._orbital_frequencies = np.zeros(number_of_moons)
        self._rotational_frequencies = np.zeros(number_of_moons)
        self._initial_orbital_angles = np.zeros(number_of_moons)
        self._distances = np.zeros(number_of_moons)
        for moon_idx in range(number_of_moons):
            self._radii[moon_idx] = system._random_state.uniform(400.0, min(4000.0, 0.4 * system.radii[planet_idx]))
            self._initial_orbital_angles[moon_idx] = system._random_state.uniform(0.0, 2.0 * const.pi)
            self._distances[moon_idx] = utils.km_to_AU(system._random_state.uniform(3.0, 100.0) * system.radii[planet_idx])
    ...
    def _compute_positions(self, time, speed_adjustments=1.0):
        ...
        """
        theta = self._initial_orbital_angles + speed_adjustments * self._orbital_frequencies * time
        x = self._distances * np.cos(theta)
        y = self._distances * np.sin(theta)
        z = x * self._inclinations
...
class _SolarSystemXML():
    ...
    def __init__(self, system, adjust_moon_orbital_speeds=False, adjust_moon_rotational_speeds=False, visualization_program='MCAst'):
        self._system = system
    ...
    def _create_cameras_element(self, times, camera_positions, camera_angles, last_valid_camera_time, field_of_view=70):
        ...
        for frame_idx in range(n_frames):
            ...
            dirvec = [
             np.sin(thetai) * np.cos(phii), np.sin(thetai) * np.sin(phii), np.cos(thetai)]
            upvec = [0.0, 0.0, 1.0]
    ...
    def _create_spacecraft_subelement(self, parent, times, spacecraft_positions):
        ...
        if spacecraft_positions.shape[0] == 3:
            etree.SubElement(spacecraft, 'pos_y').text = str(spacecraft_positions[(2,
                                                                                   0)])
        else:
    ...
    def _create_planet_subelement(self, parent, times, planet_positions, rotation_angles, planet_idx):
        ...
        for frame_idx in range(n_frames):
            ...
            etree.SubElement(frame, 'pos_y').text = str(0)
            etree.SubElement(frame, 'rot_y').text = str(-rotation_angles[(planet_idx, frame_idx)])
    ...
    def _create_moon_objects_subelement(self, parent, times, planet_idx):
        ...
        for moon_idx in range(self._system._moons[planet_idx]._number_of_moons):
            ...
            for frame_idx in range(n_frames):
                ...
                etree.SubElement(frame, 'rot_y').text = str(-moonRots[frame_idx][moon_idx])
