...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ast2000tools/shortcuts.py
# Compiled at: 2019-11-12 17:41:01
"""Module containing the SpaceMissionShortcuts class."""
from __future__ import division, print_function, absolute_import
from six.moves import range, map
import numpy as np, ast2000tools.constants as const, ast2000tools.utils as utils
...
class SpaceMissionShortcuts(object):
    ...
    def place_spacecraft_on_escape_trajectory(self, rocket_thrust, rocket_mass_loss_rate, time, height_above_surface, direction_angle, remaining_fuel_mass):
        ...
        if height_above_surface < 0 or height_above_surface > 40000000.0:
            raise ValueError('Argument "height_above_surface" is %g but must be in the range [0, %g] m.' % 40000000.0)
        ...
        planet_mass = self.system.masses[0]
        direction_angle_rad = utils.deg_to_rad(direction_angle)
        direction_of_motion = np.asfarray([np.cos(direction_angle_rad), np.sin(direction_angle_rad)])
        distance_from_center = planet_radius + utils.m_to_AU(height_above_surface)
        spacecraft_position = self.system._compute_single_planet_position(time, 0) + distance_from_center * direction_of_motion
        escape_speed = np.sqrt(2 * const.G_sol * planet_mass / distance_from_center)
    ...
    def place_spacecraft_in_unstable_orbit(self, time, planet_idx):
        ...
        -------
        float
            The eccentricity of the resulting orbit.
        ...
        spacecraft_distance = max(utils.m_to_AU(self.system.radii[planet_idx] * 1000.0 + 10000.0), spacecraft_distance)
    ...
    def place_spacecraft_in_stable_orbit(self, time, orbital_height, orbital_angle, planet_idx):
        ...
        planet_radius = self.system.radii[planet_idx] * 1000.0
        spacecraft_distance = utils.m_to_AU(planet_radius + orbital_height)
        max_spacecraft_distance = self.system._compute_force_balance_distance(planet_idx, k=10)
        if spacecraft_distance >= max_spacecraft_distance:
            raise ValueError('Argument "orbital_height" is %g but must be smaller than %g meters.' % (utils.AU_to_m(max_spacecraft_distance) - planet_radius))
