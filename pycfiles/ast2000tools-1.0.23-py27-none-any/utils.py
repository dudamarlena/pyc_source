...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ast2000tools/utils.py
# Compiled at: 2019-11-12 17:41:01
"""Module containing various utility functions."""
from __future__ import division, print_function, absolute_import
from six.moves import urllib
from distutils.version import StrictVersion
import sys, os, subprocess, json, hashlib, numpy as np, ast2000tools.constants as const
...
def check_for_newer_version(verbose=False):
    ...
    def has_pip_command(pip):
        try:
            subprocess.check_call([sys.executable, '-m', pip, '--version'], stdout=open(os.devnull, 'wb'), stderr=open(os.devnull, 'wb'))
...
def yr_to_s(yr):
    ...
    """
    yr = float(yr) if np.isscalar(yr) else np.asfarray(yr)
    return yr * const.yr
...
def s_to_yr(s):
    ...
    """
    s = float(s) if np.isscalar(s) else np.asfarray(s)
    return s / const.yr
...
def day_to_s(day):
    ...
    float
        The corresponding time in seconds.
    """
    day = float(day) if np.isscalar(day) else np.asfarray(day)
...
def AU_to_m(AU):
    ...
    """
    AU = float(AU) if np.isscalar(AU) else np.asfarray(AU)
    return AU * const.AU
...
def m_to_AU(m):
    ...
    """
    m = float(m) if np.isscalar(m) else np.asfarray(m)
    return m / const.AU
...
def km_to_AU(km):
    ...
    float
        The corresponding distance in astronomical units.
    """
    km = float(km) if np.isscalar(km) else np.asfarray(km)
    return km / (const.AU * 0.001)
...
def kg_to_m(kg):
    ...
    """
    kg = float(kg) if np.isscalar(kg) else np.asfarray(kg)
...
def AU_pr_yr_to_m_pr_s(AU_pr_yr):
    ...
    """
    AU_pr_yr = float(AU_pr_yr) if np.isscalar(AU_pr_yr) else np.asfarray(AU_pr_yr)
...
def m_pr_s_to_AU_pr_yr(m_pr_s):
    ...
    """
    m_pr_s = float(m_pr_s) if np.isscalar(m_pr_s) else np.asfarray(m_pr_s)
...
def rad_to_deg(rad):
    ...
    """
    rad = float(rad) if np.isscalar(rad) else np.asfarray(rad)
...
def deg_to_rad(deg):
    ...
    """
    deg = float(deg) if np.isscalar(deg) else np.asfarray(deg)
