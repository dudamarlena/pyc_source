...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ast2000tools/space_mission.py
...
class SpaceMission(object):
    ...
    def __init__(self, seed, data_path=None, has_moons=True, verbose=True):
        ...
        self._spacecraft_mass = 1100.0
        self._spacecraft_area = 16.0
        self._lander_mass = 90.0
        self._lander_area = 0.3
    ...
    def star_direction_angles(self):
        r"""tuple: Tuple (:math:`\phi_0`, :math:`\phi_1`) containing the azimuthal angles of the reference stars relative to the solar system x-axis, in degrees."""
        return self._star_direction_angles

    @property
    def star_doppler_shifts_at_sun(self):
        r"""tuple: Tuple (:math:`\Delta\lambda_0`, :math:`\Delta\lambda_1`) containing the Doppler shifts of the stars relative to your sun, in nanometers."""
    ...
    def set_launch_parameters(self, thrust_per_box, mass_loss_rate_per_box, number_of_boxes, initial_fuel_mass, estimated_launch_duration, launch_position, time_of_launch):
        ...
        Parameters
        ----------
        thrust_per_box : float
            The thrust of a single simulation box in Newtons.
        mass_loss_rate_per_box : float
            The rate of mass loss of a single simulation box in kilograms per second.
        number_of_boxes : int or float
            The total number of simulation boxes making up the combustion chamber.
        ...
        """
        thrust_per_box = float(thrust_per_box)
        if thrust_per_box < 0:
            raise ValueError('Argument "thrust_per_box" is %g but must be a positive value.' % thrust_per_box)
        mass_loss_rate_per_box = float(mass_loss_rate_per_box)
        if mass_loss_rate_per_box < 0:
            raise ValueError('Argument "mass_loss_rate_per_box" is %g but must be a positive value.' % mass_loss_rate_per_box)
        number_of_boxes = float(number_of_boxes)
        if number_of_boxes < 0:
            raise ValueError('Argument "number_of_boxes" is %g but must be a positive value.' % number_of_boxes)
        ...
        if estimated_launch_duration < 0.01 or estimated_launch_duration > const.day:
        ...
        launch_position = np.asfarray(launch_position)
        ...
        planet_position_at_launch = self.system._compute_single_planet_position(time_of_launch, 0)
        launch_displacement_from_center = utils.AU_to_m(launch_position - planet_position_at_launch)
        launch_distance_from_center = np.linalg.norm(launch_displacement_from_center)
        planet_radius = self.system.radii[0] * 1000.0
        deviation = launch_distance_from_center - planet_radius
        max_deviation = planet_radius * 0.01
        ...
        self._rocket_thrust = thrust_per_box * number_of_boxes
        self._rocket_mass_loss_rate = mass_loss_rate_per_box * number_of_boxes
    ...
    def launch_rocket(self, time_step=0.01):
        ...
        if time_step < 0 or time_step > 0.01:
            raise ValueError('Argument "time_step" is %g but must be a positive value not exceeding 0.01 s.' % time_step)
        n_time_steps = int(np.ceil(self.estimated_launch_duration / time_step))
        dt = self.estimated_launch_duration / n_time_steps
        ...
        current_mass = self.spacecraft_mass + self.initial_fuel_mass
        ...
        for i in range(n_time_steps):
            ...
            if accelerating_upward == True:
                kin_energy = 0.5 * current_mass * current_speed ** 2
                ...
                if kin_energy >= pot_energy:
                    ...
                    time_to_reach_escape_velocity = dt * (i + 1)
                    break
        ...
        if reached_escape_velocity == True:
            if time_to_reach_escape_velocity < 300:
                print('Reached escape velocity in only %g s. Realistically it should take at least %g s (5 minutes).' % (time_to_reach_escape_velocity, 300))
        ...
        planet_position = utils.AU_to_m(planet_position_AU)
        planet_velocity = utils.AU_pr_yr_to_m_pr_s(self.system._compute_single_planet_velocity(self.time_of_launch, 0))
        planet_radius = self.system.radii[0] * 1000.0
        launch_direction = launch_position_relative_to_planet / planet_radius
        final_displacement_from_center = current_dist_from_center * launch_direction
        final_radial_velocity = current_speed * launch_direction
        planet_angular_velocity = 2.0 * const.pi / utils.day_to_s(self.system.rotational_periods[0])
        surface_tangent_vector = np.array([-launch_direction[1], launch_direction[0]])
        surface_velocity = planet_angular_velocity * planet_radius * surface_tangent_vector
        correct_final_velocity = utils.m_pr_s_to_AU_pr_yr(planet_velocity + surface_velocity + final_radial_velocity)
        correct_final_position = utils.m_to_AU(planet_position + (planet_velocity + surface_velocity) * time_to_reach_escape_velocity + final_displacement_from_center)
    ...
    def verify_launch_result(self, position_after_launch):
        ...
        position_after_launch = np.asfarray(position_after_launch)
        ...
        displacement = position_after_launch - self._position_after_launch
        deviation = np.linalg.norm(displacement)
        max_deviation = 0.015 * utils.km_to_AU(self.system.radii[0])
    ...
    def verify_planet_positions(self, simulation_duration, planet_positions, filename='planet_trajectories.npy'):
        ...
        ----
            ...
            By default, this wrapper causes the trajectories to be stored in the file "planet_trajectories.npy" in the working directory.
            The syntax for loading the data is:

            >>> times, planet_positions = np.load(`filename`)
        """
    ...
    def get_sky_image_pixel(polar_angle, azimuth_angle):
        ...
        za = np.abs(z)
        tt = azimuth_angle % (2.0 * const.pi) / (const.pi / 2.0)
        ipix = 0
        if za <= 0.6666666666666666:
            temp1 = nside * (0.5 + tt)
            temp2 = nside * 0.75 * z
            jp = int(np.floor(temp1 - temp2))
            jm = int(np.floor(temp1 + temp2))
            ir = nside + 1 + jp - jm
            kshift = 1 - ir % 2
            nl4 = 4 * nside
            ip = int(np.floor((jp + jm - nside + kshift + 1) // 2))
            if ip >= nl4:
                ip = ip - nl4
            ipix = 2 * nside * (nside - 1) + nl4 * (ir - 1) + ip
        else:
            tp = tt - int(np.floor(tt))
            tmp = nside * np.sqrt(3.0 * (1.0 - za))
            jp = int(np.floor(tp * tmp))
            jm = int(np.floor((1.0 - tp) * tmp))
            ir = jp + jm + 1
            ip = int(np.floor(tt * ir))
            if ip >= 4 * ir:
                ip = ip - 4 * ir
            if z > 0.0:
                ipix = 2 * ir * (ir - 1) + ip
    ...
    def measure_star_doppler_shifts(self):
        r"""Returns the Doppler shifts of the two reference stars as measured by your spacecraft directly after launch.
        ...
        v = -self._velocity_after_launch[0] * np.cos(utils.deg_to_rad(self.star_direction_angles[0])) - self._velocity_after_launch[1] * np.sin(utils.deg_to_rad(self.star_direction_angles[0])) + self._reference_star_radial_speeds[0]
        v = utils.AU_pr_yr_to_m_pr_s(v)
        dl1 = utils._convert_relative_speed_to_doppler_shift(self.reference_wavelength, v)
        v = -self._velocity_after_launch[0] * np.cos(utils.deg_to_rad(self.star_direction_angles[1])) - self._velocity_after_launch[1] * np.sin(utils.deg_to_rad(self.star_direction_angles[1])) + self._reference_star_radial_speeds[1]
        v = utils.AU_pr_yr_to_m_pr_s(v)
        dl2 = utils._convert_relative_speed_to_doppler_shift(self.reference_wavelength, v)
        return (
         dl1, dl2)
    ...
    def verify_manual_orientation(self, position_after_launch, velocity_after_launch, angle_after_launch):
        ...
        if position_after_launch.shape != (2, ):
            raise ValueError('Argument "position_after_launch" has shape %s but must have shape (2,).' % str(position_after_launch.shape))
        velocity_after_launch = np.asfarray(velocity_after_launch)
    ...
    def _init_mean_molecular_masses(self):
        muList = [
         31.333333, 27.6, 27.6, 31.0, 30.8, 22.0, 23.5, 27.5, 30.333333, 30.8, 30.333333, 26.5, 26.5, 30.333333, 22.0, 30.333333, 23.5, 27.6, 23.5, 25.333333, 27.5, 31.333333, 30.333333, 27.6, 27.6, 30.5, 25.0, 27.6, 31.333333, 23.5, 27.6, 34.666667, 30.333333, 23.5, 30.333333, 30.333333, 27.6, 22.0, 30.333333, 30.333333, 27.6, 34.5, 30.0, 30.333333, 30.5, 30.333333, 30.0, 27.6, 26.0, 30.333333, 27.6, 27.6, 30.333333, 30.333333, 27.6, 27.6, 27.6, 27.6, 30.333333, 40.0, 30.5, 27.6, 31.0, 31.333333, 30.8, 27.6, 30.666667, 26.0, 30.333333, 27.6, 30.333333, 30.333333, 27.5, 30.0, 27.5, 27.5, 30.333333, 30.5, 34.5, 30.333333, 30.333333, 30.8, 27.6, 30.8, 31.333333, 30.333333, 25.0, 31.333333, 27.6, 27.6, 27.5, 30.333333, 30.333333, 30.333333, 30.333333, 27.5, 30.333333, 30.333333, 27.6, 30.5]
    ...
    def _init_orientation_data(self):
        self._angle_after_launch = self.system._random_state.randint(0, 360)
        phi_1 = self.system._random_state.uniform(0, 360)
        phi_2 = (phi_1 + self.system._random_state.uniform(40, 140)) % 360
        self._star_direction_angles = (phi_2, phi_1)
        self._reference_wavelength = 656.3
        self._reference_star_radial_speeds = [self.system._random_state.uniform(-2, 2), self.system._random_state.uniform(-2, 2)]
    ...
    def _get_launch_results(self):
        return (
         self._fuel_needed_for_launch, self.time_after_launch, self._position_after_launch, self._velocity_after_launch)

    def _get_orientation_data(self):
        return (
         self._position_after_launch, self._velocity_after_launch, self._angle_after_launch)
    ...
    def _compute_fuel_mass_needed_for_boost(self, remaining_fuel_mass, delta_v):
        ...
        delta_v_magnitude = utils.AU_pr_yr_to_m_pr_s(np.linalg.norm(delta_v))
    ...
    def _generate_picture(azimuth_angle, filename, full_sky_image_path):
        ...
        xymaxmin = 2 * np.sin(FOV / 2.0) / (1.0 + np.cos(FOV / 2.0))
        ...
        for i in range(xresolution):
            ...
            for j in range(yresolution):
                ...
                theta = const.pi / 2.0 - np.arcsin(y * np.sin(c) / rho)
...
class _Atmosphere(object):
    ...
    def __init__(self, planet_mass, planet_radius, surface_density, surface_temp, mean_molecular_mass, isothermal_temp_frac=0.5, adiabatic_index=1.4):
        self._gamma = adiabatic_index
        self._mu = mean_molecular_mass
        self._M = planet_mass
        self._R = planet_radius
        self._rho0 = surface_density
        self._T0 = surface_temp
        self._P0 = self._ideal_gas_pressure(surface_density, surface_temp)
        self._T_iso = isothermal_temp_frac * surface_temp
        self._h_iso = self._isothermal_height()
        self._P_iso = self._adiabatic_pressure(self._h_iso)
    ...
    def _pressure(self, h):
        if h < self._h_iso:
            return self._adiabatic_pressure(h)
        return self._isothermal_pressure(h)
    ...
    def _temperature(self, h):
        if h < self._h_iso:
            return self._adiabatic_temperature(h)
        return self._T_iso
...
class _Camera(object):
    ...
    def _set_n_dims(self, n_dims):
        ...
        assert self._n_dims == 2 or self._n_dims == 3, 'The number of dimensions is %d but must be 2 or 3' % self._n_dims
    ...
    def _get_up_vector(self):
        return (
         self._up_vector[0], self._up_vector[1], self._up_vector[2])
...
class _SpacecraftJourneyEvents(object):
    ...
    def _update_data(self, name, data):
        assert name in self._events, 'Trying to update data of non-existing event %s.' % name
    ...
    def _get_history(self, start_time):
        return sorted([ (name, event) for name, event in self._events.items() if event[0] >= start_time ], key=lambda item: item[1][0])
...
class _SpacecraftJourney(object):
    ...
    def __init__(self, mission, verbose=True):
        if not isinstance(mission, SpaceMission):
            raise ValueError('Argument "mission" is an instance of %s but must be a SpaceMission instance.' % mission.__class__.__name__)
        self._absolute_error_tolerance = 1e-06
    ...
    def take_picture(self, filename='journey_picture.xml'):
        ...
        self.system._generate_spacecraft_picture(self._convert_to_output_time(self._current_time), self._convert_to_output_position(self._current_time, self._current_position), (
         theta, phi), planet_idx=self._planet_idx, filename=filename)
        if self.verbose:
    ...
    def finish_video(self, filename='journey_video.xml', number_of_frames=1000, radial_camera_offset=0):
        ...
        if n_trajectory_segments == 0:
            ...
            all_times = np.concatenate([self._recorded_trajectories[0]['times']] + [ trajectory['times'][1:] for trajectory in self._recorded_trajectories[1:] ])
            all_positions = np.concatenate([self._recorded_trajectories[0]['positions']] + [ trajectory['positions'][:, 1:] for trajectory in self._recorded_trajectories[1:] ], axis=1)
            all_velocities = np.concatenate([self._recorded_trajectories[0]['velocities']] + [ trajectory['velocities'][:, 1:] for trajectory in self._recorded_trajectories[1:] ], axis=1)
            ...
            for segment_idx in range(n_trajectory_segments):
                ...
                polar_angles, azimuth_angles = self._recorded_trajectories[segment_idx]['camera']._compute_orientations(frame_times[segment_start_frame_idx:segment_end_frame_idx], frame_positions[:, segment_start_frame_idx:segment_end_frame_idx], frame_velocities[:, segment_start_frame_idx:segment_end_frame_idx])
            ...
            self.system._generate_spacecraft_video(frame_times, frame_positions, frame_orientations, planet_idx=self._planet_idx, filename=filename)
            if self.verbose:
    ...
    def look_in_fixed_direction(self, polar_angle=const.pi / 2, azimuth_angle=0):
        ...
        if polar_angle < 0 or polar_angle > const.pi:
            raise ValueError('Argument "polar_angle" is %g but must be in the range [0, pi] rad.' % polar_angle)
        azimuth_angle = float(azimuth_angle)
        self._camera = _Camera(self.system, self._n_dims, relative_polar_angle=polar_angle, relative_azimuth_angle=azimuth_angle)
        if self.verbose:
    ...
    def look_in_direction_of_planet(self, planet_idx=None, relative_polar_angle=0, relative_azimuth_angle=0):
        ...
        self._camera = _PlanetRelativeCamera(planet_idx, self.system, self._n_dims, relative_polar_angle=relative_polar_angle, relative_azimuth_angle=relative_azimuth_angle)
        if self.verbose:
    ...
    def look_in_direction_of_motion(self, relative_polar_angle=0, relative_azimuth_angle=0):
        ...
        if relative_polar_angle < 0 or relative_polar_angle > const.pi:
            raise ValueError('Argument "relative_polar_angle" is %g but must be in the range [0, pi] rad.' % relative_polar_angle)
        relative_azimuth_angle = float(relative_azimuth_angle)
        self._camera = _MotionRelativeCamera(self.system, self._n_dims, relative_polar_angle=relative_polar_angle, relative_azimuth_angle=relative_azimuth_angle)
        if self.verbose:
    ...
    def _compute_trajectory(self, start_time, start_position, start_velocity, end_time, tolerance=1e-09):
        start_pos_vel = np.concatenate((start_position, start_velocity))
        result = integrate.solve_ivp(self._compute_derivatives, (
         start_time, end_time), start_pos_vel, method='BDF', atol=self._absolute_error_tolerance, rtol=tolerance)
        ...
        return (result.t,
         result.y[:self._n_dims, :],
         result.y[self._n_dims:, :])

    def _advance_until_time(self, end_time, tolerance=1e-09):
        ...
        output_times, output_positions, output_velocities = self._compute_trajectory(self._current_time, self._current_position, self._current_velocity, end_time, tolerance=tolerance)
        ...
        if self.video_active:
            self._recorded_trajectories.append({'times': output_times, 'positions': output_positions, 
...
class InterplanetaryTravel(_SpacecraftJourney):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._current_position = np.zeros(2)
    ...
    def coast(self, duration, tolerance=1e-09):
    ...
    def coast_until_time(self, end_time, tolerance=1e-09):
    ...
    def _compute_derivatives(self, time, pos_vel):
        ...
        """
        planet_positions = self.system._compute_planet_positions(time)
    ...
    def _process_new_events(self, start_time):
        ...
        for name, event in history:
            ...
            data = event[1]
            if name == 'crashed':
                print('FAILURE: You collided with planet %d at time %g.' % (data['planet'], time))
            elif name == 'burned':
                print('FAILURE: You were swallowed by the sun at time %g.' % time)
            else:
...
class LandingSequence(_SpacecraftJourney):
    ...
    def __init__(self, mission, assume_uniform_gravity_atmosphere=True, verbose=True):
        ...
        self._max_impact_speed = 3.0
        self._max_drag_for_orbital_decay = 1000.0
        self._max_drag_for_parachute_failure = 250000.0
        self._max_drag_per_area_for_burn = 10000000.0
        self._initial_time_in_system = 0
        self._initial_position_in_system = np.zeros(2)
        self._initial_velocity_in_system = np.zeros(2)
        self._initial_position = np.zeros(3)
        self._initial_velocity = np.zeros(3)
        self._current_time = 0
        self._current_position = np.zeros(3)
        self._current_velocity = np.zeros(3)
        self._lander_launched = False
        self._parachute = {'area': 0, 'min_height': 0}
        self._landing_thruster = {'force': 0, 'min_height': 0}
        self._move_to_initial_state()
    ...
    def fall(self, duration, tolerance=1e-09):
    ...
    def fall_until_time(self, end_time, tolerance=1e-09):
    ...
    def finish_video(self, filename='landing_video.xml', number_of_frames=1000, radial_camera_offset=100000.0):
        """Ends the recording and generates a corresponding video viewable in MCAst.
    ...
    def _convert_to_output_times(self, times):
        ...
        """
        return self._initial_time_in_system + utils.s_to_yr(np.asfarray(times))
    ...
    def _convert_to_output_position(self, time, position):
        ...
        return np.pad(planet_position, (0, 1), 'constant') + utils.m_to_AU(np.asfarray(position))
    ...
    def _compute_planet_properties(self):
        ...
        """
        self._planet_mass = self.system.masses[self._planet_idx] * const.m_sun
        self._planet_radius = self.system.radii[self._planet_idx] * 1000.0
        self._surface_density = self.system.atmospheric_densities[self._planet_idx]
        self._mean_molecular_mass = self.system._mean_molecular_masses[self._planet_idx]
        self._rotation_period = utils.day_to_s(self.system.rotational_periods[self._planet_idx])
        self._surface_gravity = self._planet_mass * const.G / self._planet_radius ** 2
        self._escape_velocity = np.sqrt(2 * self._planet_mass * const.G / self._planet_radius)
        self._surface_temperature = self.system._compute_single_planet_temperature(self._planet_idx)
        ...
        self._atmosphere = _UniformGravityAtmosphere(*atmosphere_parameters) if self._assume_uniform_gravity_atmosphere else _Atmosphere(*atmosphere_parameters)
    ...
    def _compute_derivatives(self, time, pos_vel):
        ...
        1-D :class:`numpy.ndarray`
            Array of shape (4,) with concatenated velocity and acceleration at the given time.
        """
        distance_from_center = np.linalg.norm(pos_vel[0:3])
        height = distance_from_center - self._planet_radius
        atm_vel = 2 * const.pi / self._rotation_period * np.array([-pos_vel[1], pos_vel[0], 0])
        rel_vel = pos_vel[3:6] - atm_vel
        radial_vel = np.dot(pos_vel[3:6], pos_vel[0:3] / distance_from_center)
        self._events._remove_all_outdated(time)
        if height < 0 and not self.reached_surface:
            self._events._add('reached_surface', time)
            if abs(radial_vel) > self._max_impact_speed:
                self._events._add_if_absent('crashed', time, data={'rel_vel': np.linalg.norm(rel_vel)}, tag='failure')
            elif abs(radial_vel) < self._escape_velocity:
                self._events._add_if_tag_absent('failure', 'landed_successfully', time, data={'position': pos_vel[0:3], 'radial_vel': abs(radial_vel)})
            elif radial_vel < 0:
    ...
    def _process_new_events(self, start_time):
        ...
        for name, event in history:
            ...
            tag = event[2]
            if name == 'crashed':
                print('FAILURE: You collided with planet %d at time %g s with velocity %g m/s.' % (
                 self._planet_idx, time, data['rel_vel']))
            elif name == 'orbital_decay':
                print('FAILURE: Orbital decay of the spacecraft due to air resistance at time %g s!' % time)
            elif name == 'broke_parachute':
                print('WARNING: Parachute failed due to extreme drag at time %g s!' % time)
            elif name == 'burned':
                print('FAILURE: Lander module burned up due to reentry heating at time %g s!' % time)
            elif name == 'bounced':
