...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/utils/calculations.py
...
def double_gaussian(x, amp1, peak1, sigma1, amp2, peak2, sigma2):
    ...
    """
    y_values = amp1 * np.exp(-(x - peak1) ** 2.0 / (2.0 * sigma1 ** 2.0)) + amp2 * np.exp(-(x - peak2) ** 2.0 / (2.0 * sigma2 ** 2.0))
    return y_values
