...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/tests/test_monitor_mast.py
...
def test_astroquery_mast():
    ...
    params = {'columns':'COUNT_BIG(*)',  'filters':[],  'pagesize':1,  'page':1}
...
def test_instrument_inventory_filtering():
    ...
    filt = 'GR150R'
    data = mm.instrument_inventory('niriss', add_filters={'filter': filt},
      return_data=True)
...
def test_instrument_dataproduct_filtering():
    ...
    by data product"""
    dp = 'spectrum'
    data = mm.instrument_inventory('nirspec', dataproduct=dp, caom=True, return_data=True)
