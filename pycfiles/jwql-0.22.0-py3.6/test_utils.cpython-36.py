...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/tests/test_utils.py
...
def test_copy_files():
    ...
    if not os.path.exists(original_file):
        raise AssertionError('Failed to create original test file.')
    else:
...
def test_validate_config():
    ...
    if not 'Provided config.json does not match the required JSON schema' in str(excinfo.value):
        raise AssertionError('Failed to reject incorrect JSON dict.')
    else:
        good_config_dict = {'connection_string':'',  'database':{'engine':'', 
          'name':'', 
          'user':'', 
          'password':'', 
          'host':'', 
