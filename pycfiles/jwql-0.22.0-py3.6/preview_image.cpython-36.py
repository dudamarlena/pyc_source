...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/utils/preview_image.py
# Compiled at: 2019-08-26 11:08:03
# Size of source mod 2**32: 14973 bytes
"""
...
class PreviewImage:
    ...
    def make_figure(self, image, integration_number, min_value, max_value, scale, maxsize=8, thumbnail=False):
        """
        ...
        if scale == 'log':
            ...
            if thumbnail:
                fig = plt.imshow(shiftdata, norm=colors.LogNorm(vmin=shiftmin, vmax=shiftmax),
                  cmap=(self.cmap))
                ...
                cax = ax.imshow(shiftdata, norm=colors.LogNorm(vmin=shiftmin, vmax=shiftmax),
                  cmap=(self.cmap))
                plt.gca().invert_yaxis()
    ...
    def make_image(self, max_img_size=8):
        ...
        for i in range(nint):
            ...
            self.make_figure(frame, i, minval, maxval, (self.scaling.lower()), maxsize=max_img_size,
              thumbnail=False)
