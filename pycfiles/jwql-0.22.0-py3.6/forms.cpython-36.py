...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/website/apps/jwql/forms.py
...
class AnomalySubmitForm(forms.Form):
    __doc__ = 'A multiple choice field for specifying flagged anomalies.'
...
class FileSearchForm(forms.Form):
    __doc__ = 'Single-field form to search for a proposal or fileroot.'
    search = forms.CharField(label='', max_length=500, required=True, empty_value='Search')
    fileroot_dict = None
    search_type = None
    instrument = None
    ...
    def clean_search(self):
        ...
        if search.isnumeric():
            if 1 < int(search) < 99999:
                self.search_type = 'proposal'
        else:
            if self._search_is_fileroot(search):
        ...
        if self.search_type == 'proposal':
            ...
            search_string = os.path.join(FILESYSTEM_DIR, 'jw{}'.format(proposal_string), '*{}*.fits'.format(proposal_string))
...
class MnemonicSearchForm(forms.Form):
    __doc__ = 'A single-field form to search for a mnemonic in the DMS EDB.'
    search = forms.CharField(label='', max_length=500, required=True, empty_value='Search',
    ...
    def clean_search(self):
        ...
        if not self.logged_in:
            raise forms.ValidationError('Could not log into MAST. Please login or provide MAST token in environment variable or config.json.')
        else:
            search = self.cleaned_data['search']
            if is_valid_mnemonic(search):
                self.search_type = 'mnemonic'
            else:
                raise forms.ValidationError('Invalid search term {}. Please enter a valid DMS EDB mnemonic.'.format(search))
        return self.cleaned_data['search']


class MnemonicQueryForm(forms.Form):
    __doc__ = 'A triple-field form to query mnemonic records in the DMS EDB.'
    ...
    search = forms.CharField(label='mnemonic', max_length=500, required=True, initial=default_mnemonic_identifier,
      empty_value='Search',
      help_text='Mnemonic identifier')
    start_time = forms.CharField(label='start', max_length=500, required=False, initial=(default_start_time.iso),
      help_text='Start time')
    end_time = forms.CharField(label='end', max_length=500, required=False, initial=(default_end_time.iso),
      help_text='End time')
    search_type = None
    ...
    def clean_start_time(self):
        ...
        try:
            ...
            raise forms.ValidationError('Invalid start time {}. Please enter a time in iso format, e.g. {}'.format(start_time, self.default_start_time))
    ...
    def clean_end_time(self):
        ...
        try:
            ...
            raise forms.ValidationError('Invalid end time {}. Please enter a time in iso format, e.g. {}.'.format(end_time, self.default_end_time))

        if 'start_time' in self.cleaned_data.keys():
            if self.cleaned_data['end_time'] <= self.cleaned_data['start_time']:
                raise forms.ValidationError('Invalid time inputs. End time is required to be after Start time.')
        return self.cleaned_data['end_time']


class MnemonicExplorationForm(forms.Form):
    __doc__ = 'A sextuple-field form to explore the EDB mnemonic inventory.'
    default_description = 'centroid data'
    description = forms.CharField(label='description', max_length=500, required=False, initial=default_description,
      help_text='Description')
    sql_data_type = forms.CharField(label='sqlDataType', max_length=500, required=False, help_text='sqlDataType')
    subsystem = forms.CharField(label='subsystem', max_length=500, required=False, help_text='subsystem')
    tlm_identifier = forms.CharField(label='tlmIdentifier', max_length=500, required=False, help_text='Numerical ID (tlmIdentifier)')
    tlm_mnemonic = forms.CharField(label='tlmMnemonic', max_length=500, required=False, help_text='String ID (tlmMnemonic)')
    unit = forms.CharField(label='unit', max_length=500, required=False, help_text='unit')