...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/tests/test_dark_monitor.py
...
def test_find_hot_dead_pixels():
    ...
    mean_image[(6, 6)] = 0.06
    mean_image[(7, 3)] = 0.09
    hot, dead = monitor.find_hot_dead_pixels(mean_image, comparison_image, hot_threshold=2.0, dead_threshold=0.1)
    if not len(hot) == 2:
        raise AssertionError
    else:
        if not np.all(hot[0] == np.array([1, 7])):
            raise AssertionError
        else:
            if not np.all(hot[1] == np.array([1, 7])):
                raise AssertionError
            elif not len(dead) == 2:
                raise AssertionError
...
def test_shift_to_full_frame():
    ...
    monitor.y0 = 512
    coordinates = (
     np.array([6, 7]), np.array([6, 3]))
    new_coords = monitor.shift_to_full_frame(coordinates)
    if not np.all(new_coords[0] == np.array([518, 519])):
        raise AssertionError
    elif not np.all(new_coords[1] == np.array([518, 515])):
        raise AssertionError