...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/utils/instrument_properties.py
...
def amplifier_info(filename, omit_reference_pixels=True):
    ...
    sample_time = header['TSAMPLE'] * 1e-06
    frame_time = header['TFRAME']
    subarray_name = header['SUBARRAY']
    aperture = '{}_{}'.format(detector, subarray_name)
    if instrument.lower() == 'miri' or x_dim == 2048 and y_dim == 2048 or subarray_name in FOUR_AMP_SUBARRAYS:
        num_amps = 4
        ...
        if subarray_name not in NIRCAM_SUBARRAYS_ONE_OR_FOUR_AMPS:
            ...
            amp4_time = calc_frame_time(instrument, aperture, x_dim, y_dim, 4,
              sample_time=sample_time)
            amp1_time = calc_frame_time(instrument, aperture, x_dim, y_dim, 1,
              sample_time=sample_time)
            if np.isclose(amp4_time, frame_time, atol=0.001, rtol=0):
                ...
                for amp_num in ('1', '2', '3', '4'):
                    newdims = (
                     amp_bounds[amp_num][1][0], y_dim, 1)
                    amp_bounds[amp_num][1] = newdims

            else:
                if np.isclose(amp1_time, frame_time, atol=0.001, rtol=0):
                    num_amps = 1
                    amp_bounds = {'1': [(0, x_dim, 1), (0, y_dim, 1)]}
                else:
                    raise ValueError('Unable to determine number of amps used for exposure. 4-amp frametimeis {}. 1-amp frametime is {}. Reported frametime is {}.'.format(amp4_time, amp1_time, frame_time))
    if omit_reference_pixels:
        ...
        for key in amp_bounds:
            ...
            amp_bounds[key] = [
             (
              new_xmin, new_xmax, prev_xstep), (new_ymin, new_ymax, prev_ystep)]

    return (
     num_amps, amp_bounds)


def calc_frame_time(instrument, aperture, xdim, ydim, amps, sample_time=1e-05):
