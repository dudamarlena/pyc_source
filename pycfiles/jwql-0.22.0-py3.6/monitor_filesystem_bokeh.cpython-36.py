...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/website/apps/jwql/monitor_pages/monitor_filesystem_bokeh.py
...
class MonitorFilesystem(BokehTemplate):
    ...
    def pre_init(self):
        ...
        self.allowed_types = [
         'fits_files', 'uncal', 'cal', 'rate', 'rateints',
    ...
    def initial_load(self):
        ...
        self.ftypes = Table(names=(['timestamp'] + self.allowed_types), dtype=([
         Time] + [int] * 11))
        ...
        self.stypes = Table(names=(['timestamp'] + self.allowed_types), dtype=([
         Time] + [float] * 11))
    ...
    def update_plots(self, full=False):
        if full:
            self.refs['source_filecount'].data = {'dates':self.statistics['timestamp'].datetime64, 
             'filecount':self.statistics['file_count'].data}
            self.refs['source_stats'].data = {'dates':self.statistics['timestamp'].datetime64, 
             'systemsize':self.statistics['total'].data.astype(float) / 1073741824.0, 
             'freesize':self.statistics['available'].data.astype(float) / 1073741824.0, 
             'usedsize':self.statistics['used'].data.astype(float) / 1073741824.0}
            ftype_dict = {'dates': self.ftypes['timestamp'].datetime64}
            ftype_dict.update({x:self.ftypes[y].data for x, y in zip(self.types_y, self.allowed_types)})
            self.refs['source_files'].data = ftype_dict
            stype_dict = {'dates': self.stypes['timestamp'].datetime64}
            stype_dict.update({x:self.stypes[y].data for x, y in zip(self.types_y, self.allowed_types)})
            ...
            if new_stats:
                ...
                self.refs['source_stats'].stream({'dates':new_stats['timestamp'].datetime64, 
            ...
            if new_files:
                ...
                ftype_dict.update({x:new_files[y].data for x, y in zip(self.types_y, self.allowed_types)})
                self.refs['source_files'].stream(ftype_dict)
            if new_sizes:
                stype_dict = {'dates': new_sizes['timestamp'].datetime64}
                stype_dict.update({x:new_sizes[y].data / 1073741824.0 for x, y in zip(self.types_y, self.allowed_types)})
                self.refs['source_sizes'].data = stype_dict
            if not self.statistics:
                self.latest_timestamp = Time(0.0, format='unix')
            else:
    ...
    def read_new_data(self):
        ...
        new_stats = Table(names=(self.statistics.colnames), dtype=(self.statistics.dtype))
        new_files = Table(names=(self.ftypes.colnames), dtype=(self.ftypes.dtype))
        new_sizes = Table(names=(self.stypes.colnames), dtype=(self.stypes.dtype))
