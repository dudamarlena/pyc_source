...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/jwql_monitors/monitor_cron_jobs.py
...
def create_table(status_dict):
    ...
    data = dict(name=(list(status_dict.keys())), filename=filenames, date=dates, missing=missings, result=results)
...
def get_cadence(filenames):
    ...
    if len(times) >= minimum_log_num:
        ...
        mean_delta = 31556736.0
        stdev_delta = 31556736.0
    return (
    ...
    for subdir, subsubdir, filenames in generator:
        ...
        if len(filenames) > 0:
            ...
            if monitor_name != 'monitor_cron_jobs':
                ...
                if missing_file:
                    logging.warning('Expected a more recent {} logfile than {}'.format(monitor_name, os.path.basename(latest_log)))
