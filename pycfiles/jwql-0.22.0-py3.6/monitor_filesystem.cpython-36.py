...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/jwql_monitors/monitor_filesystem.py
# Compiled at: 2019-08-26 11:08:03
# Size of source mod 2**32: 19759 bytes
...
import datetime, itertools, logging, os, subprocess
    ...
    for dirpath, _, files in os.walk(FILESYSTEM):
        ...
        for filename in files:
            ...
            if filename.endswith('.fits'):
                ...
                instrument_results_dict[instrument][filetype]['size'] += os.path.getsize(file_path) / 1099511627776

    general_results_dict['total_file_size'] = general_results_dict['total_file_size'] / 1099511627776
    general_results_dict['fits_file_size'] = general_results_dict['fits_file_size'] / 1099511627776
    logging.info('{} fits files found in filesystem'.format(general_results_dict['fits_file_count']))
    return (
     general_results_dict, instrument_results_dict)
...
def get_global_filesystem_stats(general_results_dict):
    ...
    general_results_dict['used'] = int(stats[0]) / 1073741824
    general_results_dict['available'] = int(stats[1]) / 1073741824
...
def get_area_stats(central_storage_dict):
    ...
    for area in arealist:
        ...
        stats = subprocess.check_output(command, shell=True).split()
        total = int(stats[0]) / 1073741824
        free = int(stats[2]) / 1073741824
...
def initialize_results_dicts():
    ...
    return (
     general_results_dict, instrument_results_dict, central_storage_dict)
...
def plot_by_filetype(plot_type, instrument):
    ...
    for filetype, color in zip(FILE_SUFFIX_TYPES, colors):
        results = session.query(FilesystemInstrument.date, getattr(FilesystemInstrument, plot_type)).filter(FilesystemInstrument.filetype == filetype)
        if instrument == 'all':
            results = results.all()
        else:
...
def plot_filesystem_size():
    ...
    plot : bokeh.plotting.figure.Figure object
        ``bokeh`` plot of total file counts versus date
    """
    results = session.query(FilesystemGeneral.date, FilesystemGeneral.total_file_size, FilesystemGeneral.used, FilesystemGeneral.available).all()
...
def plot_total_file_counts():
    ...
    plot = figure(tools='pan,box_zoom,reset,wheel_zoom,save',
      x_axis_type='datetime',
