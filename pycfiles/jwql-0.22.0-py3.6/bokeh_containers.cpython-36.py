...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jwql/website/apps/jwql/bokeh_containers.py
# Compiled at: 2019-08-26 11:08:03
# Size of source mod 2**32: 13750 bytes
        ...
        for fname in file_list:
            if 'component' in fname:
                ...
                with open(os.path.join(output_dir, js_file)) as (f):
                    script = f.read()
                dashboard_components[name_dict[monitor_name]][name_dict[plot_name]] = [
                 div, script]
...
def split_files(file_list, page_type):
    ...
    i_cutoff = int(len(exp_times) * 0.1)
...
def thumbnails(inst, proposal=None):
    ...
    if proposal is not None:
        ...
        for i, file_id in enumerate(full_ids):
            ...
            if file_dict['detector'] not in detectors:
                if not file_dict['detector'].startswith('f'):
