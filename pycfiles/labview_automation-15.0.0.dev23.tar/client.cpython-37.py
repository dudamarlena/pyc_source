...
# [GCC 8.4.0]
# Embedded file name: c:\users\group\documents\repos\python_labview_automation\labview_automation\client.py
# Compiled at: 2020-03-05 13:11:02
...
class LabVIEWClient(object):
    ...
    def _check_for_error(self, return_dict):
        ...
        if return_dict['RunVIState_Status']:
            ...
            raise Error(return_dict['RunVIState_Code'], return_dict['RunVIState_Source'], f"{self.describe_error(error)}\n\nReturn dict: {return_dict}")
    ...
    def _recv_dict(self):
        if self.connection:
            ...
            while len(packet) < packet_size_int:
                partial_packet = self.connection.recv(packet_size_int - len(packet))
