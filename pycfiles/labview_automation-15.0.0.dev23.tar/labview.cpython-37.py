...
# [GCC 8.4.0]
# Embedded file name: C:\Users\group\Documents\Repos\python_labview_automation\labview_automation\labview.py
# Compiled at: 2020-03-03 04:44:48
# Size of source mod 2**32: 20082 bytes
import configparser
from distutils.dir_util import copy_tree
from hoplite.remote_enabler import remotify
import logging, os, platform, psutil, stat, subprocess, tempfile, time
...
class LabVIEW(object):
    ...
    def __init__(self, host='localhost', version=None, bitness=None, start_with_server=True, server_port=2552, server_log_path=None, server_error_log_path=None, server_tcp_timeout_s=60):
        ...
        if bitness is not None:
            if bitness.lower() not in ('x86', 'x64'):
                raise ValueError("Bitness must be one of 'x86', 'x64', or None.  Specfied {}".format(bitness))
        self._bitness = bitness
        self._pid = None
        self.ini = _LVOptions()
        self._server_cfg = ServerConfiguration(start_with_server, server_port, server_log_path, server_error_log_path, server_tcp_timeout_s)
    ...
    def path(self):
        if self._path is None:
            if self._version is None:
                ...
                if self._bitness == 'x64':
                    if not os_64bit:
                        raise ValueError("Bitness cannot be 'x64' if the OS is 32-bit")
                possible_canidates = self._helpers.get_labview_paths()
                for candidate in possible_canidates:
                    if os_64bit:
                        if self._bitness == 'x86':
                            if 'program files (x86)' not in candidate.lower():
                                continue
                        elif self._bitness == 'x64' and 'program files (x86)' in candidate.lower():
                            continue
    ...
    def client(self):
        ...
        if not self.server_cfg.start:
            raise NotStartedError('You must start LabVIEW with the automation server to use the LabVIEW Client.')
    ...
    def add_to_search_path(self, path, append=False):
        ...
        try:
            ...
            ini_paths = '<topvi>:\\*;<foundvi>:\\;<vilib>:\\*;<userlib>:\\*;<instrlib>:\\*'
        ...
        ini_paths = '"{}"'.format((';' if len(ini_paths) > 2 else '').join(ini_paths))
    ...
    def start(self, wait_until_open=True, timeout_s=900):
        ...
        if self.server_cfg.start:
            ...
            if wait_until_open:
                self.wait_until_server_loaded(timeout_s,
    ...
    def memory_usage(self):
        return self._helpers.get_process_memory_usage(self._pid, self.executable)
        ...
        while waiting:
            try:
                ...
                if time.time() - start_time > timeout_s:
                    raise TimeoutError('Timed out while waiting for LabVIEW server to load')
            else:
                waiting = False
...
class LabVIEWHelpers(object):
    ...
    def get_labview_paths(self):
        ...
        raise NotImplementedError('This command is only supported on Windows platforms')

    def _get_labview_paths_windows(self):
        import winreg
        reg = winreg.ConnectRegistry(None, winreg.HKEY_LOCAL_MACHINE)
        installed_lv_paths = []
        for root_key in ('SOFTWARE\\Wow6432Node\\National Instruments\\LabVIEW', 'SOFTWARE\\National Instruments\\LabVIEW'):
            try:
                key = self._open_windows_native_key(reg, root_key)
                i = 0
                current_lv_version = winreg.EnumKey(key, i)
                while current_lv_version.find('.') != -1:
                    try:
                        try:
                            current_lv_reg_key = self._open_windows_native_key(reg, root_key + '\\' + current_lv_version)
                            value, key_type = winreg.QueryValueEx(current_lv_reg_key, 'PATH')
    ...
    def _open_windows_native_key(self, key, sub_key):
        ...
        if python_bitness == '32bit':
            if platform.machine().endswith('64'):
                return winreg.OpenKey(key, sub_key, 0, winreg.KEY_READ | winreg.KEY_WOW64_64KEY)
    ...
    def _get_active_labview_windows(self):
        ...
        reg = winreg.ConnectRegistry(None, winreg.HKEY_LOCAL_MACHINE)
        try:
            key = self._open_windows_native_key(reg, 'SOFTWARE\\National Instruments\\LabVIEW\\CurrentVersion')
            value, key_type = winreg.QueryValueEx(key, 'PATH')
        except WindowsError:
            key = self._open_windows_native_key(reg, 'SOFTWARE\\Wow6432Node\\National Instruments\\LabVIEW\\CurrentVersion')
            value, key_type = winreg.QueryValueEx(key, 'PATH')

        return value

    def get_listener_vi_path(self):
        return os.path.realpath(os.path.join(os.path.dirname(__file__), '..', 'lv_listener', 'Listener', 'Listener Launcher', 'Splash Screen.vi'))
    ...
    def create_temp_ini(self, options={}):
        ...
        for section, tokens in options.items():
...
class RemoteLabVIEWHelpers(object):
    ...
    def process_is_running(self, pid, executable):
        return self.helpers.remote_process_is_running(self.host, pid, executable)

    def get_process_memory_usage(self, pid, executable):
        return self.helpers.remote_get_process_memory_usage(self.host, pid, executable)
