...
# [GCC 8.4.0]
# Embedded file name: /home/lxsameer/src/daarmaan/daarmaan/server/forms.py
# Compiled at: 2012-10-23 10:15:46
import urllib
...
class AjaxWidget(forms.TextInput):
    ...
    def _media(self):
        a = forms.Media(css={'all': ('ajaxwidget.css', )}, js=(
         reverse('gauth.views.statics.ajax_widget_jsonp', args=[]) + '?validator=%s' % urllib.urlencode(self.url),))
        print ']]]] ', a
        return a
...
class LoginForm(forms.Form):
    ...
    password = forms.CharField(max_length=64, label=_('password'), widget=forms.PasswordInput())
    remember_me = forms.BooleanField(label=_('Remember me'))


class PreRegistrationForm(forms.Form):
    username = forms.CharField(max_length=30, label=_('Username'), widget=AjaxWidget('asd', 'asdasD'))
...
class PostRegistrationForm(forms.Form):
    ...
    password1 = forms.CharField(max_length=250, label=_('Password'), widget=forms.PasswordInput())
    password2 = forms.CharField(max_length=250, label=_('Enter again'), widget=forms.PasswordInput())
    ...
    def save(self, user):
        ...
        if len(password) < 6:
            raise self.PasswordError(_('Password length should be more than six charachters'))
...
class NewUserForm(forms.Form):
    ...
    password2 = forms.CharField(label=_('repead password'), max_length=32, widget=forms.PasswordInput())
    verification_code = forms.CharField(widget=forms.HiddenInput(), max_length=40)
...
class EditBasicProfile(forms.ModelForm):
    ...
    def __init__(self, user=None, *args, **kwargs):
        if user:
            ...
            super(EditBasicProfile, self).__init__(initial=data, *args, **kwargs)
        else:
            super(EditBasicProfile, self).__init__(*args, **kwargs)
