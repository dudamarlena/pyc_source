...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/__plugin__/livedesk/upgrade.py
...
def upgradeUser():
    ...
    try:
        session.execute('ALTER TABLE user DROP COLUMN deleted_on')
    except (ProgrammingError, OperationalError):
        pass
...
def upgradeLiveBlog14End():
    ...
    assert isinstance(session, Session)
    try:
        session.execute('ALTER TABLE `post` CHANGE COLUMN `meta` `meta` TEXT NULL DEFAULT NULL, CHANGE COLUMN `content_plain` `content_plain` TEXT NULL DEFAULT NULL, CHANGE COLUMN `content` `content` TEXT NULL DEFAULT NULL')
    except (ProgrammingError, OperationalError):
        return
...
def upgradeMediaArchiveDeleteFix():
    ...
    try:
        ...
        session.execute('ALTER TABLE `archive_audio_data` ADD CONSTRAINT `archive_audio_data_ibfk_1` FOREIGN KEY (`fk_metadata_id` ) REFERENCES `archive_meta_data` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass

    try:
        session.execute('ALTER TABLE `archive_audio_info` DROP FOREIGN KEY `archive_audio_info_ibfk_1`')
        session.execute('ALTER TABLE `archive_audio_info` ADD CONSTRAINT `archive_audio_info_ibfk_1` FOREIGN KEY (`fk_metainfo_id` ) REFERENCES `archive_meta_info` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass

    try:
        session.execute('ALTER TABLE `archive_image_data` DROP FOREIGN KEY `archive_image_data_ibfk_1`')
        session.execute('ALTER TABLE `archive_image_data` ADD CONSTRAINT `archive_image_data_ibfk_1` FOREIGN KEY (`fk_metadata_id` ) REFERENCES `archive_meta_data` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass

    try:
        session.execute('ALTER TABLE `archive_image_info` DROP FOREIGN KEY `archive_image_info_ibfk_1`')
        session.execute('ALTER TABLE `archive_image_info` ADD CONSTRAINT `archive_image_info_ibfk_1` FOREIGN KEY (`fk_metainfo_id` ) REFERENCES `archive_meta_info` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass

    try:
        session.execute('ALTER TABLE `archive_video_data` DROP FOREIGN KEY `archive_video_data_ibfk_1`')
        session.execute('ALTER TABLE `archive_video_data` ADD CONSTRAINT `archive_video_data_ibfk_1` FOREIGN KEY (`fk_metadata_id` ) REFERENCES `archive_meta_data` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass

    try:
        session.execute('ALTER TABLE `archive_video_info` DROP FOREIGN KEY `archive_video_info_ibfk_1`')
        session.execute('ALTER TABLE `archive_video_info` ADD CONSTRAINT `archive_video_info_ibfk_1` FOREIGN KEY (`fk_metainfo_id` ) REFERENCES `archive_meta_info` (`id` ) ON DELETE CASCADE ON UPDATE RESTRICT')
    except (ProgrammingError, OperationalError):
        pass
...
def upgradeBlogSourceDeleteFix():
    ...
    session.execute('ALTER TABLE `livedesk_blog_source` ADD CONSTRAINT `livedesk_blog_source_ibfk_2` FOREIGN KEY (`fk_source` ) REFERENCES `source` (`id` ) ON DELETE RESTRICT ON UPDATE RESTRICT')
...
def upgradeSourceUnicityFix():
    ...
    session.execute('ALTER TABLE `source` ADD CONSTRAINT `uix_source_type_name` UNIQUE KEY (`name`, `fk_type_id`, `uri`)')