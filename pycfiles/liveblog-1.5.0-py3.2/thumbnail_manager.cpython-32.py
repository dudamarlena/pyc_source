...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/thumbnail_manager.py
...
from superdesk.media_archive.core.spec import IThumbnailManager, IThumbnailProcessor
...
class ThumbnailManagerAlchemy(SessionSupport, IThumbnailManager):
    ...
    def __init__(self):
        ...
        assert isinstance(self.thumbnailProcessor, IThumbnailProcessor), 'Invalid thumbnail processor %s' % self.thumbnailProcessor
    ...
    def putThumbnail(self, thumbnailFormatId, imagePath, metaData=None):
        ...
        try:
            thumbTimestamp = self.cdmThumbnail.getTimestamp(thumbPath)
        except PathNotFound:
            thumbTimestamp = None
        ...
        if not thumbTimestamp or thumbTimestamp < timestampURI(imagePath):
            ...
            if imageExt != thumbExt:
                thumbPath = thumbName + imageExt
    ...
    def deleteThumbnail(self, thumbnailFormatId, metaData):
        ...
        if format.find('id') == -1:
            return
        try:
            self.cdmThumbnail.remove(thumbPath)
        except PathNotFound:
            return
    ...
    def populate(self, metaData, scheme, size=None):
        ...
        try:
            self.cdmThumbnail.getTimestamp(thumbPath)
        except PathNotFound:
            original = self.thumbnailPath(metaData.thumbnailFormatId, metaData)
            original = self.cdmThumbnail.getURI(original, 'file')
            if size:
                if size not in self.thumbnailSizes:
                    raise InputError(_("Unknown size '%s'") % size)
