...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/impl/video_persist.py
...
from superdesk.media_archive.meta.video_data import META_TYPE_KEY, VideoDataEntry
...
class VideoPersistanceAlchemy(SessionSupport, IMetaDataHandler):
    ...
    def process(self, metaDataMapped, contentPath):
        ...
        p = Popen((self.ffmpeg_path, '-i', contentPath, '-vframes', '1', '-an', '-ss', '2', thumbnailPath), stdin=PIPE, stdout=PIPE, stderr=STDOUT)
        if p.wait() != 0:
            return False
    ...
    def populateThumbnail(self):
        ...
        """
        self.thumbnailManager.putThumbnail(self.defaultThumbnailFormatId(), abspath(join(pythonPath(), 'resources', 'video.jpg')))
