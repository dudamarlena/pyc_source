...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/thumbnail_processor_gm.py
...
class ThumbnailProcessorGM(IThumbnailProcessor):
    ...
    """
    command_transform = '"%(gm)s" convert "%(source)s" "%(destination)s"'
    ...
    gm_path = join('workspace', 'tools', 'gm', 'bin', 'gm.exe')
    wire.config('gm_path', doc='\n    The path where the gm is found')
