...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/url_info/impl/url_info.py
...
class URLInfoService(IURLInfoService):
    ...
    @see IURLInfoService
    """
    html_fixes = [{'from': '<DOCTYPE html PUBLIC "-//W3C//DTD XHTML',  'to': '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML'}]
    wire.config('html_fixes', doc='\n    Web page repairing: list of "from -> to" string pairs')
            ...
            with urlopen(url) as (conn):
                ...
                if not contentType or contentType != 'text/html':
                    ...
                    if selector:
                        ...
                        if parts:
                            urlInfo.Title = parts[(len(parts) - 1)]
                    else:
...
class HTMLInfoExtractor(HTMLParser):
            ...
            if tag == LINK:
                ...
                if 'src' in attrs:
                    if isinstance(self.urlInfo.Picture, list):
                        ...
                        self.urlInfo.Picture = [
                         self._fullURL(self.urlInfo.URL, attrs['src'])]
        if self._done():
            self.reset()
    ...
    def _done(self):
        ...
        """
        return self.urlInfo.Title and not isclass(self.urlInfo.Title) and self.urlInfo.Description and not isclass(self.urlInfo.Description) and self.urlInfo.SiteIcon and not isclass(self.urlInfo.SiteIcon) and self.urlInfo.Picture and not isclass(self.urlInfo.Picture) and len(self.urlInfo.Picture) == self.maxPictures