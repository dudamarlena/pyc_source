...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/security/core/impl/processor/user_persistence_filter.py
...
from ally.design.processor.handler import Handler, HandlerBranchingProceed, HandlerProcessorProceed
    ...
    def processPermissions(self, permissions, userId):
        ...
        for permission in permissions:
            ...
            if permission.modelsAuthenticated is not None:
                assert isinstance(permission.modelsAuthenticated, set), 'Invalid model authenticated %s' % permission.modelsAuthenticated
                for propertyType in permission.modelsAuthenticated:
                    ...
                    if propertyType.parent.clazz == User or issubclass(propertyType.parent.clazz, User):
                        if permission.putHeaders is None:
                            permission.putHeaders = {}
...
class InvokingFilterHandler(HandlerBranchingProceed, AuthenticatedUserConfigurations):
    ...
    acl = Acl
    wire.entity('acl')
    assemblyPermissions = Assembly
    wire.entity('assemblyPermissions')
    ...
    def __init__(self):
        ...
        HandlerBranchingProceed.__init__(self, Using(self.assemblyPermissions, Permission=PermissionFilter, ModelFilter=ModelFilter, solicitation=SolicitationFilter))
        AuthenticatedUserConfigurations.__init__(self)
    ...
    def process(self, processing, request: Request, response: Response, **keyargs):
        ...
        if solFilter.permissions is None:
            return
        else:
            permissions = []
