...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/thumbnail_processor_avconv.py
...
class ThumbnailProcessorAVConv(IThumbnailProcessor):
    ...
    """
    command_transform = '"%(avconv)s" -i "%(source)s" "%(destination)s"'
    wire.config('command_transform', doc='\n    The command used to transform the thumbnails')
    ...
    avconv_dir_path = join('workspace', 'tools', 'avconv')
    wire.config('avconv_dir_path', doc='\n    The path where the avconv is placed in order to be used, if empty will not place the contained avconv')
    avconv_path = join(avconv_dir_path, 'bin', 'avconv')
    wire.config('avconv_path', doc='\n    The path where the avconv is found')
    ...
    def __init__(self):
        ...
        if self.avconv_dir_path:
            synchronizeURIToDir(join(pythonPath(), 'resources', 'avconv'), self.avconv_dir_path)
    ...
    def processThumbnail(self, source, destination, width=None, height=None):
        ...
        if width and height:
            ...
            if height:
                assert isinstance(height, int), 'Invalid height %s' % height
                width = int(1.7777777777777777 * height)
                params.update(width=width, height=height)
                command = self.command_resize % params
            else:
