...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/solr_search.py
...
from superdesk.media_archive.core.impl.query_service_creator import QMetaDataInfo, ISearchProvider
...
from ally.api.criteria import AsBoolean, AsLike, AsEqual, AsDate, AsDateTime, AsRange, AsTime, AsOrdered
    ...
    def processQuery(self, session, scheme, qa=None, qi=None, qd=None):
        ...
        if orClauses:
            ...
            for clause in orClauses:
                if extend:
                    extend = extend | clause
                else:
                    extend = clause

            if solrQuery is None:
                solrQuery = si.query(extend)
            else:
                solrQuery = solrQuery.query(extend)
        if solrQuery is None:
            solrQuery = si.query()
            ...
            if isinstance(crt, AsBoolean):
                ...
                if isinstance(crt, AsLike):
                    if AsLike.like in crt:
                        if solrQuery is None:
                            solrQuery = si.query(**{field: crt.like})
                        else:
                            solrQuery = solrQuery.query(**{field: crt.like})
                    ...
                    if isinstance(crt, AsEqual):
                        ...
                        if isinstance(crt, (AsDate, AsTime, AsDateTime, AsRange)):
