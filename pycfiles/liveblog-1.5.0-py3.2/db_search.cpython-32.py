...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/db_search.py
...
class SqlSearchProvider(ISearchProvider):
    ...
    def buildQuery(self, session, scheme, offset=None, limit=1000, qa=None, qi=None, qd=None):
                ...
                if criteria is AsLikeExpression or criteria is AsLikeExpressionOrdered:
                    ...
                    if MetaInfoMapped not in criteriaMetaInfos:
                        for metaInfo in criteriaMetaInfos:
                            if self.queryIndexer.typesByMetaInfo[metaInfo.__name__] in types:
                                metaInfos.add(metaInfo)
                                continue

                    else:
                        if self.queryIndexer.typesByMetaInfo[getattr(MetaInfoMapped, '__name__')] in types:
                ...
                if criteria is AsLikeExpression or criteria is AsLikeExpressionOrdered:
                    ...
                    if MetaDataMapped not in criteriaMetaDatas:
                        for metaData in criteriaMetaDatas:
                            if self.queryIndexer.typesByMetaData[metaData.__name__] in types:
                                metaDatas.add(metaData)
                                continue

                    else:
                        if self.queryIndexer.typesByMetaData[getattr(MetaDataMapped, '__name__')] in types:
        ...
        if qi is not None:
            ...
            for name in namesForQuery(qi):
                if getattr(self.QMetaInfo, name) not in qi:
                    continue
                criteriaMetaInfos = self.queryIndexer.metaInfoByCriteria.get(name)
        ...
        if qd is not None:
            ...
            for name in namesForQuery(qd):
                if getattr(self.QMetaData, name) not in qd:
                    continue
                criteriaMetaDatas = self.queryIndexer.metaDataByCriteria.get(name)
        ...
        if not metaInfos and not metaDatas:
            ...
            if metaInfos and not metaDatas:
                ...
                if not metaInfos and metaDatas:
    ...
    def buildSubquery(self, session, metaInfo, metaData, qa, qi, qd, types):
        ...
        if qi:
            sql = buildExpressionQuery(sql, qi, metaInfo, qa)
        if qd:
            sql = buildExpressionQuery(sql, qd, metaData, qa)
        if qi and metaInfo != MetaInfoMapped:
        ...
        if qa and qa.all:
            assert isinstance(qa, QMetaDataInfo), 'Invalid query %s' % qa
            sql = buildAllQuery(sql, qa.all, self.queryIndexer.queryByInfo[metaInfo.__name__], metaInfo, self.queryIndexer.queryByData[metaData.__name__], metaData)
        return sql
...
def buildExpressionQuery(sql, query, mapped, qa):
    ...
    if qa:
        all = qa.all
    columns = {cp.key.lower():getattr(mapper.c, cp.key) for cp in  if isinstance(cp, ColumnProperty)}
    columns = {criteria:columns.get(criteria.lower()) for criteria in }
...
def buildAllQuery(sql, all, qMetaInfo, metaInfo, qMetaData, metaData):
    ...
    assert isinstance(dataMapper, Mapper)
    infoProperties = {cp.key.lower():getattr(infoMapper.c, cp.key) for cp in  if isinstance(cp, ColumnProperty)}
    dataProperties = {cp.key.lower():getattr(dataMapper.c, cp.key) for cp in  if isinstance(cp, ColumnProperty)}
    baseInfoProperties = {cp.key.lower():getattr(baseInfoMapper.c, cp.key) for cp in  if isinstance(cp, ColumnProperty)}
    baseDataProperties = {cp.key.lower():getattr(baseDataMapper.c, cp.key) for cp in  if isinstance(cp, ColumnProperty)}
