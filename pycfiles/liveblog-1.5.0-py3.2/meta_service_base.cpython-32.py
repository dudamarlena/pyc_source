...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/superdesk/media_archive/core/impl/meta_service_base.py
...
class MetaDataServiceBaseAlchemy(SessionSupport, IMetaDataService):
    ...
    def __init__(self, MetaDataClass, QMetaDataClass, referencer, cdmArchive, thumbnailManager):
        ...
        """
        assert isclass(MetaDataClass) and issubclass(MetaDataClass, MetaDataMapped), 'Invalid meta data class %s' % MetaDataClass
        assert isclass(QMetaDataClass) and issubclass(QMetaDataClass, QMetaData), 'Invalid meta data query class %s' % QMetaDataClass
    ...
    def getById(self, id, scheme, thumbSize=None):
        ...
        if metaData is None:
            raise InputError(Ref(_('Unknown meta data'), ref=self.MetaData.Id))
    ...
    def buildSql(self, typeId, q):
        ...
        if typeId:
            sql = sql.filter(self.MetaData.typeId == typeId)
        if q:
...
class MetaInfoServiceBaseAlchemy(EntityGetCRUDServiceAlchemy):
    ...
    def __init__(self, MetaInfoClass, QMetaInfoClass, MetaDataClass, QMetaDataClass, searchProvider, metaDataService, type):
        ...
        """
        assert isclass(MetaInfoClass) and issubclass(MetaInfoClass, MetaInfo), 'Invalid meta info class %s' % MetaInfoClass
        assert isclass(QMetaInfoClass) and issubclass(QMetaInfoClass, QMetaInfo), 'Invalid meta info query class %s' % QMetaInfoClass
        assert isclass(MetaDataClass) and issubclass(MetaDataClass, MetaDataMapped), 'Invalid meta data class %s' % MetaDataClass
        assert isclass(QMetaDataClass) and issubclass(QMetaDataClass, QMetaData), 'Invalid meta data query class %s' % QMetaDataClass
    ...
    def buildSql(self, dataId, languageId, qi, qd):
        ...
        if dataId:
            sql = sql.filter(self.MetaInfo.MetaData == dataId)
        if languageId:
            sql = sql.filter(self.MetaInfo.Language == languageId)
        if qi:
...
def metaTypeFor(session, type):
    ...
    try:
        metaType = session.query(MetaTypeMapped).filter(MetaTypeMapped.Type == type).one()
...
def thumbnailFormatFor(session, format):
    ...
    try:
        thumbnail = session.query(ThumbnailFormat).filter(ThumbnailFormat.format == format).one()
    except NoResultFound:
