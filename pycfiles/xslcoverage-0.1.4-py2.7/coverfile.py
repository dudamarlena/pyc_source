...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/coverfile.py
...
class XmlCoverFile(XmlCoverFileBase):
    ...
    def line_status(self, linenum):
        if self.unused_lines.has_key(linenum):
            ...
            if self.data.lines.has_key(linenum):
                return 'covered'
            return 'uncovered'
    ...
    def get_stats(self):
        return dict(payload_covered=self.payload_covered, payload_total=self.payload_total, covered_linecount=self.covered_linecount, total_linecount=self.total_linecount)
