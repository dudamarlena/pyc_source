...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/xslcoverage.py
# Compiled at: 2016-11-26 11:29:20
import os, sys, glob, traceback, runner, runcover, xmlcover
...
class RunCoverageCmd(BasicCmd):
    ...
    def setup_parser(self, parser):
        parser.add_argument('runner', help='Trace generator. Known runners: %s' % (', ').join(self.runners))
        parser.add_argument('--no-snapshot', action='store_false', dest='snapshot', default=True, help='Do not create a dated subdirectory containing the traces')
    ...
    def run(self, parser, args):
        ...
        rc, cover_runner = runcover.cmdline_runargs(command, args, args.remain_args)
...
class ReportCoverageCmd(BasicCmd):
    ...
    def setup_parser(self, parser):
        parser.add_argument('-f', '--from-log', help='Trace report of the traces')
        parser.add_argument('--show-stats', action='store_true', help='Show coverage statistics on console')
        parser.add_argument('-O', '--html-dir', help='Directory containing the HTML output')
    ...
    def run(self, parser, args):
        ...
        if self.coverage.cover_runner:
            ...
            r.build_coverage_report(html_dir=args.html_dir, print_stats=args.show_stats)
        else:
...
class XslCoverageCommand:
    ...
    def setup_options(self, parser):
        parser.add_argument('--trace-dir', default='', help='Base directory where the traces are stored')
        parser.add_argument('--dump-stack', action='store_true', help='Dump error stack (debug purpose)')
