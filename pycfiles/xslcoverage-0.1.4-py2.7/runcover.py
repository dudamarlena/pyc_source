...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/runcover.py
...
class CoverageRunner:
    ...
    def __init__(self, snapshot=True, trace_dir='', command=None, write_report=False):
...
def create_filename(dirname, basename, max_files=1000, try_basename=True):
    ...
    for i in range(1, max_files):
        filename_candidate = os.path.join(dirname, '%s-%04d%s' % (corebasename, i, ext))
        if not os.path.exists(filename_candidate):
            break
...
def cmdline_parser(parser=None, description=''):
    ...
    parser.add_argument('--trace-dir', help='Directory containing the traces')
    ...
    parser.add_argument('--report', action='store_true', help='Build the HTML coverage report from the run')
    return parser
...
def cmdline_runargs(command, options, args):
    ...
    runner = CoverageRunner(command=command, snapshot=options.snapshot, trace_dir=options.trace_dir, write_report=options.report)
    rc = runner.run(args)
    return (rc, runner)
