...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/xmlcover.py
# Compiled at: 2016-11-26 11:29:20
import os, sys, re, textwrap, shutil, glob, xml.etree.ElementTree as ET, runner
...
class TraceLog:
    ...
    def fromfile(self, tracelog):
        ...
        if root.tag != self.root_tag:
            ...
            if node is not None:
                for trace_file in node.findall('file'):
                    self.add_stylesheet(trace_file.get('path'), trace_file.get('md5sum'))
...
class CoverAnalyzer:
    ...
    def print_stats(self):
        title = '%30s %10s %6s %10s %6s' % ('FILE', 'PCOV / PTOT', '%PCOV', 'LCOV / LTOT',
                                            '%LCOV')
        ...
        def sort_coverage_pct(o):
            return 100.0 * float(o.payload_covered) / o.payload_total
        ...
        for xcover in coverages:
            ...
            stats['payload_pct'] = 100.0 * float(pc) / pt
            stats['linecount_pct'] = 100.0 * float(lc) / lt
            print pattern % stats
...
def main():
    ...
    parser.add_option('-f', '--from-log', help='Trace report of the traces')
    parser.add_option('-r', '--trace-dir', help='Directory containing the traces')
    parser.add_option('-s', '--show-stats', action='store_true', help='Show coverage statistics on console')
    parser.add_option('-O', '--html-dir', help='Directory containing the HTML output')
