...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/htmlreport.py
...
class HtmlCoverageWriter:
    ...
    def __init__(self, cssdir='htdocs'):
        ...
        self.formatter = HtmlFormatterCoverage(linenos=True, cssclass='source', cssfile=self.cssfile)
    ...
    def _push_covering(self, covering_file):
        if covering_file not in self.covering_files:
    ...
    def write_covering(self):
        ...
        for filename in self.covering_files:
            ...
            formatter = HtmlFormatter(linenos=True, cssclass='source', cssfile=self.cssfile, noclobber_cssfile=True, lineanchors='line', full=True, title=os.path.basename(filename))
    ...
    def write_file(self, coverfile, outfile, title=''):
        ...
        header = self.DOC_HEADER % dict(cssfile=self.cssfile, csscover=self.csscover, encoding=self.formatter.encoding, script=script, body_attr='onload="fill_popups()"', title=title)
        footer = self.DOC_FOOTER
        popups = self.write_popups(coverfile)
        table = highlight(code, self.lexer, self.formatter)
        outfile.write(header + popups + table + footer)

    def write_chunk(self, coverfile):
        filename = os.path.basename(coverfile.filepath())
        coverage_pct = 100.0 * float(coverfile.payload_covered) / float(coverfile.payload_total)
        title = '%s coverage = %d/%d (%5.2f%%)' % (
         filename, coverfile.payload_covered, coverfile.payload_total,
         coverage_pct)
        self.summary.append(dict(href=filename + '.html', filename=filename, covered=coverfile.payload_covered, total=coverfile.payload_total, pct=coverage_pct))
    ...
    def write_index(self, tracelog):
        row = '<tr><td><a href="%(href)s">%(filename)s</a></td><td class="lcount">%(covered)4d / %(total)4d</td><td class="percent">%(pct)5.2f%%</td></tr>\n'
        table = '<table class="sortable">\n'
        table += '<thead><tr><th>File</th>                             <th>Covered / Total</th>                             <th>Rate</th></tr></thead><tbody>'
        for data in self.summary:
            table += row % data

        table += '</tbody></table>'
        script = '<script type="text/javascript" src="sorttable.js"></script>'
        header = self.DOC_HEADER % dict(cssfile=self.cssfile, csscover=self.csscover, encoding='utf-8', script=script, body_attr='', title='Coverage Summary')
        footer = self.DOC_FOOTER
        body = ''
        if tracelog:
            if tracelog.command:
                cmd = textwrap.fill(tracelog.command, width=90, break_long_words=False, break_on_hyphens=False)
