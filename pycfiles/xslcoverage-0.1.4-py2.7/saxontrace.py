...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/xslcover/saxontrace.py
...
class TreeLineBuilder:
    ...
    def end(self, tag):
        ...
        m2 = re.search('<' + self.tag_pattern % tag + '(\\s.*?)?/>', buf, re.M | re.DOTALL)
        if not (m1 and m2):
            m = m1 or m2
        elif m1.end() < m2.end():
            m = m1
...
class SaxonCoverFile(XmlCoverFile):
    ...
    def _fix_closing_tag(self, linenum, elements):
        end_of_covered = [ e for e in elements.ending if self.line_status(e.start.linepos) == 'covered'
                         ]
