...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/no_cloud/cli.py
# Compiled at: 2017-03-11 19:46:43
import os, re, sys, click, string, datetime, subprocess
...
from .fs import find_in_path, load_configuration, is_encrypted, list_files, test_mode, fix_mode, asset_path
...
def push(paths):
    ...
    """
    for path in paths:
        root, filename = find_in_path(path, '.no-cloud.yml.crypt', '.no-cloud.yml')
...
def encrypt(dry_run, keep, paths):
    ...
    """
    password = None if dry_run else get_password('Encryption password', confirm=True)
...
@cli.command()
@click.option('-d', '--dry-run', is_flag=True, help='Do not perform anything.')
@click.option('-k', '--keep', is_flag=True, help='Leave encrypted files behind.')
@click.argument('paths', nargs=-1)
...
@cli.command()
@click.option('-s', '--service', help='', default='Service to generate a password for.')
@click.option('-u', '--username', help='', default='User name to generate a password for.')
@click.option('-i', '--iterations', help='Number of iterations for the SHA512 algorithm (defaults to 100000).', default=100000)
@click.option('-c', '--characters', help='Characters classes to use for the digest; `l` for lowercase, `u` for uppercase, `d` for digits and `p` for punctuation (defaults to `ludp`).', default='ludp')
@click.option('-l', '--length', help='Length of the digest (defaults to 32).', default=32)
@click.option('-f', '--filename', help='YAML file to read the above information from.', default=None)
@click.option('-v', '--version', help='YAML document starting at zero (defaults to 0).', default=0)
@click.option('-n', '--no-clipboard', is_flag=True, help='Disable clipboard copy, password is printed to stdout.')
def password(service, username, iterations, characters, length, filename, version, no_clipboard):
...
@cli.command()
@click.option('-d', '--dry-run', is_flag=True, help='Do not perform anything.')
@click.option('-f', '--force', is_flag=True, help='Force renaming, possibly overwriting existing files.')
...
def rename(dry_run, force, pattern, paths):
    ...
    for filename in filenames:
        files.append({'path': os.path.dirname(filename), 
    ...
    for file in files:
        ...
        if not (dry_run or force):
            assert not os.path.isfile(file['dst']), 'destination exists `%s`' % file['dst']
...
@cli.command()
@click.option('-d', '--dry-run', is_flag=True, help='Do not perform anything (ie.: not file mode fixing).')
@click.argument('paths', nargs=-1)
def audit(dry_run, paths):
    ...
    for filename in list_files(paths):
        ...
        status = '' + ('c' if clear else ' ') + ('m' if mode else ' ')
...
@cli.command()
@click.option('-p', '--preview', is_flag=True, help='Automatically preview document.')
...
def render(preview, timestamp, stylesheet, paths):
    ...
    for filename in list_files(paths):
        ...
        echo(filename)
        HTML(string=html).write_pdf(filename, stylesheets=[
         CSS(stylesheet)])
        if preview:
