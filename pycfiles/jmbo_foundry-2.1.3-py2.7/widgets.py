...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/widgets.py
# Compiled at: 2015-12-02 04:11:30
import types, datetime, re
from django.forms.widgets import CheckboxSelectMultiple, MultiWidget, Select, HiddenInput, FileInput, RadioInput, RadioFieldRenderer, RadioSelect
...
class DragDropOrderingWidget(MultiWidget):
    ...
    def format_output(self, rendered_widgets):
        ...
        return '%s<ul class="dragdrop">%s</ul>%s' % (rendered_widgets[0], para,
         DragDropOrderingWidget.script % re.search('id="(?P<id>\\w+)"', rendered_widgets[0]).group('id'))
...
class RadioImageInput(RadioInput):
    ...
    def tag(self):
        ...
        if self.image_path:
            ...
            return mark_safe(result + '<img src="%s%s" %s />' % (
