...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/views.py
# Compiled at: 2015-12-02 04:11:30
...
from foundry.models import Listing, Page, ChatRoom, BlogPost, Notification, Member, FoundryComment, CommentReport, Country
from foundry.forms import JoinForm, JoinFinishForm, AgeGatewayForm, TestForm, SearchForm, CreateBlogPostForm
...
def join_finish(request):
    ...
    warnings.warn('join_finish will be deprecated in jmbo-foundry 1.2.', RuntimeWarning)
    if request.method == 'POST':
...
def search(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('search-results') + '?search_term=' + param)
    else:
        form = SearchForm()
    extra = dict(form=form)
...
def search_results(request):
    ...
    extra = dict(items_per_page=10, search_term=search_term, queryset=queryset)
    ...
    if ctype is None or object_pk is None:
        ...
        if form.errors:
            ...
            template_name = [
             ...
             'comments/form.html']
            return render_to_response(template_name, {'comment': form.data.get('comment', ''), 
               'form': form, 
               'next': next}, RequestContext(request, {}))
        if isinstance(target, ModelBase):
            can_comment, reason = target.can_comment(request)
            if not can_comment:
                raise RuntimeError('Commenting on target is not allowed (reason = %s)' % reason)
        ...
        context['request'].GET = QueryDict('paginate_by=%s&paginate_offset=%s' % (request.POST['paginate_by'], request.POST.get('paginate_offset', -1)))
...
def comment_reply_form(request):
    obj = ContentType.objects.get(id=request.GET['content_type_id']).get_object_for_this_type(id=request.GET['oid'])
...
def report_comment(request, comment_id):
    ...
    if comment.can_report(request):
        ...
        if comment.commentreport_set.count() == 3:
            ...
            msg = EmailMultiAlternatives('Flagged comment on %s' % site.name, strip_tags(content), settings.DEFAULT_FROM_EMAIL, preferences.NaughtyWordPreferences.email_recipients.split())
            msg.attach_alternative(content, 'text/html')
            msg.send()
    next = request.REQUEST.get('next')
    response = HttpResponseRedirect(next or comment.content_object.get_absolute_url())
    response.set_cookie('comment_report_%s' % comment.id, value=1, max_age=604800)
    return response
...
def fetch_new_comments_ajax(request, content_type_id, oid, last_comment_id):
    context = RequestContext(request)
    context['object'] = ContentType.objects.get(id=content_type_id).get_object_for_this_type(id=oid)
