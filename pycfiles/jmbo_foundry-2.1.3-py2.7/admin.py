...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/admin.py
# Compiled at: 2015-12-07 04:34:20
import inspect
...
from foundry.models import Listing, Link, MenuLinkPosition, Menu, NavbarLinkPosition, Navbar, GeneralPreferences, GeneralPreferences, RegistrationPreferences, LoginPreferences, Member, DefaultAvatar, PasswordResetPreferences, Country, Page, ChatRoom, BlogPost, Notification, FoundryComment, CommentReport, PageView, NaughtyWordPreferences, ViewProxy, ListingContent, ListingPinned
...
class LinkAdminForm(forms.ModelForm):
    view_name = forms.ChoiceField(label='View Name', help_text='View name to which this link will redirect.', required=False)
    target = forms.ModelChoiceField(ModelBase.objects.all().order_by('title'), required=False, label='Target')

    class Meta:
        model = Link
        fields = ('title', 'subtitle', 'view_name', 'category', 'target', 'url')

    def __init__(self, *args, **kwargs):
        self.declared_fields['view_name'].choices = [
         ('', '-- Select --')] + get_view_choices()
    ...
    def clean(self):
        ...
        for fieldname in ('view_name', 'category', 'target', 'url'):
            if cleaned_data[fieldname]:
                if n:
                    raise forms.ValidationError('You may set at most one of view_name, category, target or URL.')
                n += 1
...
class MenuAdminForm(forms.ModelForm):
    ...
    def clean(self):
        for site in self.cleaned_data['sites']:
            ...
            if q.exists():
                raise forms.ValidationError(_('The slug is already in use by menu %s. To use the same                     slug the pages menu not have overlapping sites.' % q[0]))
    ...
    def clean(self):
        for site in self.cleaned_data['sites']:
            ...
            if self.instance.id:
                q = q.exclude(id=self.instance.id)
            if q.exists():
                raise forms.ValidationError(_('The slug is already in use by navbar %s. To use the same                     slug the navbars may not have overlapping sites.' % q[0]))
...
class ListingAdminForm(forms.ModelForm):
    content_helper = forms.models.ModelMultipleChoiceField(label=_('Content'), queryset=ModelBase.objects.all().order_by('title'), required=False, help_text=_('Individual items to display. Setting this will ignore any setting for <i>Content Type</i>, <i>Categories</i> and <i>Tags</i>.'))
    pinned_helper = forms.models.ModelMultipleChoiceField(label=_('Pinned'), queryset=ModelBase.objects.all().order_by('title'), required=False, help_text=_('Individual items to pin to the top of the listing. These items are visible across all pages when navigating the listing.'))

    class Meta:
        model = Listing
        fields = ('title', 'slug', 'subtitle', 'content_type', 'categories', 'tags',
                  'content_helper', 'pinned_helper', 'style', 'count', 'items_per_page',
                  'display_title_tiled', 'enable_syndication', 'sites')
        widgets = {'sites': SitesGroupsWidget, 
           'style': RadioImageSelect(choices=(
                   ('x', 'x'), ('y', 'y')), attrs={'image_attrs': {'style': 'max-width: 128px;'}})}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            if 'initial' not in kwargs:
                kwargs['initial'] = {}
            kwargs['initial']['content_helper'] = [ o.modelbase_obj for o in ListingContent.objects.filter(listing=instance).order_by('position') ]
            kwargs['initial']['pinned_helper'] = [ o.modelbase_obj for o in ListingPinned.objects.filter(listing=instance).order_by('position')
                                                 ]
    ...
    def clean(self):
        ...
        for site in self.cleaned_data['sites']:
            ...
            if self.instance.id:
                q = q.exclude(id=self.instance.id)
            if q.exists():
                raise forms.ValidationError(_('The slug is already in use by listing %s. To use the same                     slug the listings may not have overlapping sites.' % q[0]))
    ...
    def save(self, commit=True):
        ...
        def save_m2m():
            ...
            for n, obj in enumerate(self.cleaned_data['content_helper']):
                ListingContent.objects.create(modelbase_obj=obj, listing=instance, position=n)

            ListingPinned.objects.filter(listing=instance).delete()
            for n, obj in enumerate(self.cleaned_data['pinned_helper']):
                ListingPinned.objects.create(modelbase_obj=obj, listing=instance, position=n)
...
class RegistrationPreferencesAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {'raw_display_fields': SelectCommaWidget, 
    ...
    def clean(self):
        ...
        for fieldname in unique_fields:
            ...
            if li and li[0]['dcount'] > 1:
                raise forms.ValidationError('Cannot set %s to be unique since there is more than one member with the same %s %s.' % (fieldname, fieldname, li[0][fieldname]))
    ...
    def clean(self):
        for site in self.cleaned_data['sites']:
            ...
            if self.instance.id:
                q = q.exclude(id=self.instance.id)
            if q.exists():
                raise forms.ValidationError(_('The slug is already in use by page %s. To use the same                     slug the pages may not have overlapping sites.' % q[0]))
...
class BlogPostAdmin(ModelBaseAdmin):
    list_display = ('title', 'preview', 'publish_on', 'retract_on', '_get_absolute_url',
                    'owner', 'created', '_actions')
