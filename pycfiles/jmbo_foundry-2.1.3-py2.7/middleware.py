...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/middleware.py
...
AG_TOKEN_MAX_TIME_TO_EXPIRY = 60
...
class AgeGateway:
    ...
    def process_response(self, request, response):
        ...
        if request.is_ajax():
            ...
            if exempted_urls and re.match(('|').join(exempted_urls.split()), request.META['PATH_INFO']) is not None:
                return response
            exempted_ips = get_preference('GeneralPreferences', 'exempted_ips')
            if exempted_ips and re.match(('|').join(exempted_ips.split()), request.META['REMOTE_ADDR']) is not None:
                return response
            exempted_user_agents = get_preference('GeneralPreferences', 'exempted_user_agents')
            if exempted_user_agents and re.match(('|').join(exempted_user_agents.split()), request.META.get('HTTP_USER_AGENT', '')) is not None:
                return response
            user = getattr(request, 'user', None)
            if user is not None and user.is_anonymous():
                if private_site:
                    return redirect_to_login(request.path_info, login_url=reverse('login'))
                else:
                    ...
                    if ag_values and expires:
                        ...
                        if Country.objects.filter(country_code__iexact=ag_values[:2], minimum_age__lte=get_age(dob)).exists():
                            response.set_cookie('age_gateway_passed', value=1, expires=expires)
                            response.set_cookie('age_gateway_values', value=ag_values, expires=expires)
                            return response
                    return redirect_to_login(request.path_info, login_url=reverse('age-gateway'))
    ...
    def get_partner_age_gateway_values(self, request):
        ...
        if AG_TOKEN_PARAMETER_NAME in request.GET:
            ...
            partner_config = get_preference('GeneralPreferences', 'partner_site_configuration')
            try:
                domain_key_map = dict(line.split(' ', 1) for line in partner_config.strip('\n').split('\n'))
                jwt_shared_secret = domain_key_map[ref_domain]
                payload = jwt.decode(token, jwt_shared_secret)
                from_expiry = timegm(datetime.utcnow().utctimetuple()) - int(payload['exp'])
                ...
                return (
                 payload['v'],
                 datetime.strptime(payload['e'], '%Y-%m-%dT%H:%M:%S'))
