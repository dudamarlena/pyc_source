...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/admin_forms.py
...
class RowEditAjaxForm(forms.ModelForm):
    ...
    class Meta:
        model = Row
        fields = ('block_name', 'class_name', 'enable_caching', 'cache_type', 'cache_timeout')
...
class ColumnCreateAjaxForm(forms.ModelForm):
    ...
    class Meta:
        model = Column
        fields = ('row', 'width', 'title', 'designation', 'class_name', 'enable_caching',
                  'cache_type', 'cache_timeout')
        widgets = {'row': forms.widgets.HiddenInput}
...
class TileEditAjaxForm(forms.ModelForm):
    target = forms.ChoiceField(choices=[], required=False, help_text='A navbar, menu or listing.')

    class Meta:
        model = Tile
        fields = ('column', 'target', 'view_name', 'class_name', 'enable_ajax', 'condition_expression',
                  'enable_caching', 'cache_type', 'cache_timeout')
        widgets = {'column': forms.widgets.HiddenInput, 
    ...
    def __init__(self, *args, **kwargs):
        ...
        choices = []
        tileproviders = [ m for m in models.get_models() if 'ITileProvider' in [ interface.getName() for interface in implementedBy(m) ]
                        ]
        ...
        self.fields['view_name'].widget.choices = [
