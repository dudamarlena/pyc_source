...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/admin_views.py
...
from foundry.admin_forms import RowEditAjaxForm, ColumnCreateAjaxForm, ColumnEditAjaxForm, TileEditAjaxForm

@staff_member_required
def row_create_ajax(request):
    row = Row(page_id=int(request.REQUEST['page_id']))
    row.save()
    di = dict(id=row.id, page_render_height=row.page.render_height, block_name=row.block_name)
...
def row_edit_ajax(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            row = form.save()
            di = dict(id=row.id, block_name=row.block_name)
            return HttpResponse(simplejson.dumps(di))
    else:
        form = RowEditAjaxForm(instance=instance)
    extra = dict(form=form)
    return render_to_response('admin/foundry/page/row_edit_ajax.html', extra, context_instance=RequestContext(request))
...
def column_create_ajax(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            column = form.save()
            di = dict(id=column.id, width=column.width, title=column.title, row_id=column.row.id, row_render_height=column.row.render_height, page_render_height=column.row.page.render_height)
            return HttpResponse(simplejson.dumps(di))
    else:
        form = ColumnCreateAjaxForm(initial={'row': int(request.GET['row_id'])})
    extra = dict(form=form)
    return render_to_response('admin/foundry/page/column_create_ajax.html', extra, context_instance=RequestContext(request))
...
def column_edit_ajax(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            column = form.save()
            di = dict(id=column.id, width=column.width, title=column.title)
            return HttpResponse(simplejson.dumps(di))
    else:
        form = ColumnEditAjaxForm(instance=instance)
    extra = dict(form=form)
    return render_to_response('admin/foundry/page/column_edit_ajax.html', extra, context_instance=RequestContext(request))
...
def tile_create_ajax(request):
    ...
    tile.save()
    di = dict(id=tile.id, column_id=tile.column.id, column_render_height=tile.column.render_height, row_id=tile.column.row.id, row_render_height=tile.column.row.render_height, page_render_height=tile.column.row.page.render_height)
    return HttpResponse(simplejson.dumps(di))
...
def tile_edit_ajax(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            tile = form.save()
            di = dict(id=tile.id, label=tile.label)
            return HttpResponse(simplejson.dumps(di))
    else:
