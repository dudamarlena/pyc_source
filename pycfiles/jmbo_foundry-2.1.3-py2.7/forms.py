...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/forms.py
    ...
    def clean(self):
        ...
        if username and password:
            ...
            if self.user_cache is None:
                v = preferences.LoginPreferences.raw_login_fields
                if v == 'email':
                    msg = 'Please enter a correct email address and password. Note that both fields are case-sensitive.'
                    raise forms.ValidationError(_(msg))
                elif v == 'mobile_number':
                    msg = 'Please enter a correct mobile number and password. Note that both fields are case-sensitive.'
                    raise forms.ValidationError(_(msg))
                elif v == 'username,email':
                    msg = 'Please enter a correct username or email address and password. Note that both fields are case-sensitive.'
                    raise forms.ValidationError(_(msg))
                elif v == 'username,mobile_number':
                    msg = 'Please enter a correct username or mobile number and password. Note that both fields are case-sensitive.'
                    raise forms.ValidationError(_(msg))
                else:
...
class JoinForm(UserCreationForm):
        ...
        for name in unique_fields:
            ...
            if value is not None:
                ...
                if models.Member.objects.filter(**di).count() > 0:
                    ...
                    message = ugettext('The %(pretty_name)s is already in use. Please supply a different %(pretty_name)s.') % {'pretty_name': pretty_name}
        ...
        if self.show_age_gateway:
            ...
            if country and dob:
                ...
                if dob > today.replace(today.year - country.minimum_age):
                    msg = 'You must be at least %s years of age to use this site.' % country.minimum_age
                    raise forms.ValidationError(_(msg))
        return cleaned_data
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.fields.has_key('mobile_number'):
            ...
            self.fields['mobile_number'].help_text = _('The number must be in international format and may start with a + sign. All other characters must be numbers. No spaces allowed. An example is +27821234567.')
        ...
        self.fields.keyOrder = sorted(field_order, key=lambda key: field_order[key])
    ...
    def clean_image(self):
        ...
        if image:
            ...
            try:
                ...
                raise forms.ValidationError(_('The image is either invalid or unsupported.'))
        ...
        for name in unique_fields:
            ...
            if value is not None:
                ...
                if models.Member.objects.filter(**di).exclude(id=self.instance.id).exists():
                    pretty_name = self.fields[name].label.lower()
                    message = _('The %(pretty_name)s is already in use. Please supply a different %(pretty_name)s.' % {'pretty_name': pretty_name})
                    self._errors[name] = self.error_class([message])
...
class PasswordResetForm(BasePasswordResetForm):
    ...
    def clean_mobile_number(self):
        ...
        mobile_number = self.cleaned_data['mobile_number']
        self.users_cache = models.Member.objects.filter(mobile_number__iexact=mobile_number, is_active=True)
    ...
    def save(self, **kwargs):
        ...
        for user in self.users_cache:
            ...
            if self.fields.has_key('email'):
                send_mail(_('Password reset on %s') % site_name, content, from_email, [user.email])
            else:
                sms = AmbientSMS(settings.FOUNDRY['sms_gateway_api_key'], settings.FOUNDRY['sms_gateway_password'])
                try:
