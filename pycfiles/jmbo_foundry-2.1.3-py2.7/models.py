...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/foundry/models.py
# Compiled at: 2016-03-30 10:09:29
import inspect, re, cPickle
...
from foundry.profile_models import AbstractAvatarProfile, AbstractSocialProfile, AbstractPersonalProfile, AbstractContactProfile, AbstractSubscriptionProfile, AbstractLocationProfile
...
SCRIPT_REGEX = re.compile('(<script[^>]*>)|(<[^>]* on[a-z]+=[\'"].*?[\'"][^>]*)', flags=re.DOTALL)
...
class Link(models.Model):
    ...
    subtitle = models.CharField(max_length=256, blank=True, null=True, help_text='Some titles may be the same. A subtitle makes a distinction. It is not displayed on the site.')
    view_name = models.CharField(max_length=256, help_text='View name to which this link will redirect.', blank=True, null=True)
    category = models.ForeignKey('category.Category', help_text='Category to which this link will redirect.', blank=True, null=True)
    target_content_type = models.ForeignKey(ContentType, blank=True, null=True, related_name='link_target_content_type')
    target_object_id = models.PositiveIntegerField(blank=True, null=True)
    target = generic.GenericForeignKey('target_content_type', 'target_object_id')
    url = models.CharField(max_length=256, help_text='URL to which this link will redirect.', blank=True, null=True)
...
class Menu(CachingMixin):
    ...
    subtitle = models.CharField(max_length=256, blank=True, null=True, help_text='Some titles may be the same. A subtitle makes a distinction. It is not displayed on the site.')
    slug = models.SlugField(editable=True, max_length=32, db_index=True)
    display_title = models.BooleanField(default=True)
    sites = models.ManyToManyField('sites.Site', blank=True, null=True, help_text='Sites that this page will appear on.')
...
class Listing(models.Model):
    ...
    subtitle = models.CharField(max_length=256, blank=True, null=True, help_text='Some titles may be the same. A subtitle makes a distinction. It is not displayed on the site.')
    slug = models.SlugField(editable=True, max_length=32, db_index=True)
    content_type = models.ManyToManyField(ContentType, help_text='Content types to display, eg. post or gallery.', blank=True, null=True)
    content = models.ManyToManyField('jmbo.ModelBase', help_text='Individual items to display. Setting this will ignore any setting for <i>Content Type</i>, <i>Categories</i> and <i>Tags</i>.', blank=True, null=True, related_name='listing_content', through='ListingContent')
    categories = models.ManyToManyField('category.Category', help_text='Categories for which to collect items.', blank=True, null=True, related_name='listing_categories')
    tags = models.ManyToManyField('category.Tag', help_text='Tags for which to collect items.', blank=True, null=True, related_name='listing_tags')
    pinned = models.ManyToManyField('jmbo.ModelBase', help_text='Individual items to pin to the top of the listing. These\nitems are visible across all pages when navigating the listing.', blank=True, null=True, related_name='listing_pinned', through='ListingPinned')
    count = models.IntegerField(help_text='Number of items to display (excludes any pinned items).\nSet to zero to display all items.')
    style = models.CharField(choices=[ (klass.__name__, klass.__name__) for klass in LISTING_CLASSES ], max_length=64)
    items_per_page = models.PositiveIntegerField(default=0, help_text='Number of items displayed on a page (excludes any pinned items). Set to zero to disable paging.')
    view_modifier = models.CharField('Ordering and filtering', max_length=255, help_text="A set of links to order or filter the listing,\neg. 'most liked' or 'most commented'. DefaultViewModifier provided by Jmbo works for\nall listings; however, others may be very specific and not work with the listing.", blank=True, null=True, default='')
    display_title_tiled = models.BooleanField('Display title if in a tile', default=True, help_text='Display the title if used as a tile within a more\ncomplex page.')
    enable_syndication = models.BooleanField(default=False)
    sites = models.ManyToManyField('sites.Site', blank=True, null=True, help_text='Sites that this listing will appear on.')
    objects = DefaultManager()
    permitted = PermittedManager()
    implements(ITileProvider)
    ...
    def queryset(self, request=None):
        ...
        if 'oracle' in connection.vendor.lower():
            ...
            q = q.distinct('publish_on', 'created', 'id').order_by('-publish_on', '-created')
        if self.count:
            q = q[:self.count]
        return q
    ...
    def set_pinned(self, iterable):
        for n, obj in enumerate(iterable):
            ListingPinned.objects.create(modelbase_obj=obj, listing=self, position=n)

    def set_content(self, iterable):
        for n, obj in enumerate(iterable):
            ListingContent.objects.create(modelbase_obj=obj, listing=self, position=n)

    @property
    def pinned_queryset(self):
        li = [ o for o in ModelBase.permitted.filter(listing_pinned=self) ]
        order = [ o.modelbase_obj.id for o in ListingPinned.objects.filter(listing=self).order_by('position')
                ]
    ...
    def content_queryset(self):
        li = [ o for o in ModelBase.permitted.filter(listing_content=self).exclude(id__in=self.pinned.all())
             ]
        order = [ o.modelbase_obj.id for o in ListingContent.objects.filter(listing=self).order_by('position')
                ]
        li.sort(lambda a, b: cmp(order.index(a.id), order.index(b.id)))
        return AttributeWrapper(li, exists=len(li))
...
class AbstractLinkPosition(models.Model):
    ...
    name = models.CharField(max_length=200, null=True, blank=True, help_text='A name that is applied to the link tag.')
    class_name = models.CharField(max_length=200, null=True, blank=True, help_text='One or more CSS classes that are applied to the link tag.')
    condition_expression = models.CharField(max_length=255, null=True, blank=True, help_text='A python expression. Variable request is in scope.')
...
class GeneralPreferences(Preferences):
    ...
    site_description = models.CharField(max_length=512, null=True, blank=True, help_text='A sentence describing the site.')
    ...
    private_site = models.BooleanField(default=False, help_text=_('A private site requires a visitor to be logged in to view any content.'))
    show_age_gateway = models.BooleanField(default=False)
    exempted_urls = models.TextField(blank=True, default='', help_text='URL patterns that are exempted from the Private Site and Age Gateway. Certain URLs like /login are already protected and do not need to be listed. One entry per line. Matches are wildcard by default, eg. /my-page will match /my-pages/the-red-one.')
    exempted_ips = models.TextField('Exempted IP addresses', blank=True, default='', help_text='IP address patterns that are exempted from the Private Site and Age Gateway. Matches are wildcard by default, eg. 192.168.0 will match 192.168.0.5.')
    analytics_tags = models.TextField(null=True, blank=True, help_text='May contain tags and javascript. Set this even if the\npreference is for a mobile site.')
    exempted_user_agents = models.TextField('Exempted user agents', blank=True, default='Googlebot\nTwitterbot\nfacebookexternalhit\n', help_text='User agents patterns that are exempted from only the Age Gateway.\nThis is useful when wanting to share content that is protected by an age\ngateway with eg. Facebook. Matches are wildcard by default, eg. my-user-agent will match my-user-agent-version-two.')
    analytics_tags = models.TextField(null=True, blank=True, help_text='May contain tags and javascript. Set this even if the\npreference is for a mobile site.')
    partner_site_configuration = models.TextField(null=True, blank=True, help_text='A list of partner domains and JWT keys, one per line. A line has the format &lt;domain&gt; &lt;key&gt;, e.g. www.example.com this_is_a_key. Partner sites use the JWT key to encode age gateway values, etc.')
...
class RegistrationPreferences(Preferences):
    __module__ = 'preferences.models'
    raw_display_fields = models.CharField('Display fields', max_length=256, default='', help_text=_('Fields to display on the registration form.'))
    raw_required_fields = models.CharField('Required fields', max_length=256, default='', blank=True, help_text=_('Set fields which are not required by default as required on the registration form.'))
    raw_unique_fields = models.CharField('Unique fields', max_length=256, default='', blank=True, help_text=_('Set fields which must be unique on the registration form.'))
    raw_field_order = models.CharField('Field order', max_length=1024, default='{}', blank=True, help_text=_('Set the ordering of fields on the registration form (opt-in fields are always last).'))
    ...
    def save(self, *args, **kwargs):
        for fieldname in self.unique_fields:
            li = Member.objects.exclude(**{fieldname: None}).exclude(**{fieldname: ''}).values(fieldname).annotate(dcount=Count(fieldname)).order_by('-dcount')
            if li and li[0]['dcount'] > 1:
                raise RuntimeError('Cannot set %s to be unique since there is more than one member with the same %s %s.' % (fieldname, fieldname, li[0][fieldname]))
...
class LoginPreferences(Preferences):
    __module__ = 'preferences.models'
    raw_login_fields = models.CharField('Login fields', max_length=32, default='username', choices=(
     (
     ...
     (
      'username,mobile_number', _('Username or mobile number'))), help_text=_('Users may log in with more than one identifier.'))
...
class PasswordResetPreferences(Preferences):
    __module__ = 'preferences.models'
    lookup_field = models.CharField(max_length=32, default='email', choices=(
     (
      'email', _('Email address')),
     (
      'mobile_number', _('Mobile number'))), help_text=_('The field a user must enter to retrieve his password.'))
...
class NaughtyWordPreferences(Preferences):
    __module__ = 'preferences.models'
    entries = models.TextField(default='', help_text='Each line has format "word,weight", eg. "bomb,8". Weight must be a value from 1 to 10.')
    threshold = models.PositiveIntegerField(default=5, help_text='An item is deemed suspect if its weighted score exceeds this value.')
    email_recipients = models.TextField(default='', help_text='Reports are sent to these email addresses. One email address per line.')
...
class Country(models.Model):
    ...
    slug = models.SlugField(editable=True, max_length=50, unique=True)
    minimum_age = models.PositiveIntegerField(default=18)
    country_code = models.CharField(max_length=2, null=True, blank=False, unique=True, db_index=True)
...
class Page(models.Model):
    title = models.CharField(max_length=200, help_text='A title that may appear in the browser window caption.')
    subtitle = models.CharField(max_length=256, blank=True, null=True, help_text='Some titles may be the same. A subtitle makes a distinction. It is not displayed on the site.')
    description = models.TextField(help_text=_('A short description. More verbose than the title but limited to one or two sentences.'), blank=True, null=True)
    slug = models.SlugField(editable=True, max_length=32, db_index=True)
    is_homepage = models.BooleanField(default=False, help_text="Tick if you want this page to be the site's homepage.")
    css = models.TextField(blank=True, null=True, help_text='Additional styling to be applied to the page. This is useful when using a page as a campaign.')
    sites = models.ManyToManyField('sites.Site', blank=True, null=True, help_text='Sites that this page will appear on.')
    objects = DefaultManager()
    permitted = PermittedManager()
...
class PageView(models.Model):
    ...
    view_name = models.CharField(max_length=200, help_text='A view that uses the target page to render itself.')
...
class Row(CachingMixin):
    ...
    block_name = models.CharField(max_length=32, default='content', choices=(
     (
      'header', _('Header')),
     (
      'content', _('Content')),
     (
      'footer', _('Footer'))), help_text='The Django base template block that this row is rendered             within. It is only applicable if the page is set to be the home             page.')
    has_left_or_right_column = models.BooleanField(default=False, editable=False, db_index=True)
    class_name = models.CharField(max_length=200, null=True, blank=True, help_text='One or more CSS classes that are applied to the row.')
...
class Column(CachingMixin):
    ...
    title = models.CharField(max_length=256, null=True, blank=True, help_text='The title is rendered at the top of a column.')
    designation = models.CharField(max_length=32, null=True, blank=True, default='', choices=(
     (
      'left', _('Left')),
     (
      'right', _('Right'))), help_text='Applicable to content (green) rows. Used to display columns to the left and right of the content block.')
    class_name = models.CharField(max_length=200, null=True, blank=True, help_text='One or more CSS classes that are applied to the column.')
...
class Tile(CachingMixin):
    ...
    target = generic.GenericForeignKey('target_content_type', 'target_object_id')
    view_name = models.CharField(max_length=200, null=True, blank=True, help_text='A view to be rendered in this tile. This view is typically a snippet of a larger page. If you are unsure test and see if it works - you cannot break anything.')
    class_name = models.CharField(max_length=200, null=True, blank=True, help_text='One or more CSS classes that are applied to the tile.')
...
class ViewProxy(ModelBase):
    ...
    view_name = models.CharField(max_length=256, help_text=_('View name to which this link will redirect.'))
...
def check_slug(sender, **kwargs):
    ...
    if kwargs['action'] == 'post_add' and sender.__name__.endswith('_sites') and isinstance(instance, (Navbar, Menu, Listing, Page)):
