...
# [GCC 8.4.0]
# Embedded file name: /Users/amandeep/Github/dig-sandpaper/test/coarse/preprocess/test_preprocessing.py
...
class TestCoarsePreprocessing(unittest.TestCase):
    ...
    def test_basic_coarse_preprocessing(self):
        ...
        self.assertEqual(result['SPARQL']['where']['clauses'][1]['constraint'], 'toronto, ontario')
        self.assertEqual(result['SPARQL']['where']['clauses'][1]['isOptional'], True)
        self.assertEqual(result['SPARQL']['where']['clauses'][1]['type'], 'Location')
        self.assertEqual(result['SPARQL']['where']['clauses'][3]['type'], 'Title')
        self.assertEqual(result['SPARQL']['where']['clauses'][4]['type'], 'HairColor')
        self.assertEqual(result['SPARQL']['where']['clauses'][4]['constraint'], 'blonde')
        self.assertEqual(result['SPARQL']['where']['clauses'][5]['constraint'], 'straw')
        self.assertEqual(result['SPARQL']['where']['filters'][0]['type'], 'Title')
    ...
    def test_basic_coarse_preprocessing_with_compound_filter(self):
        ...
        self.assertEqual(f['clauses'][0]['type'], 'Title')
        self.assertEqual(f['clauses'][1]['type'], 'owl:Thing')
    ...
    def test_basic_coarse_preprocessing_with_no_type_mappings(self):
        ...
        result = preprocessor.preprocess(query)
        f = result['SPARQL']['where']['filters'][0]
        self.assertEqual(f['clauses'][0]['type'], 'owl:Thing')
    ...
    def test_basic_coarse_preprocessing_with_date_filter(self):
        ...
        result = preprocessor.preprocess(query)
        f = result['SPARQL']['where']['filters'][0]
        self.assertEqual(f['clauses'][0]['type'], 'PostingDate')
    ...
    def test_nested_filter_constraint_expansion(self):
        ...
        query = {'SPARQL': {'where': {'clauses': [], 'filters': [{'operator': 'or', 'clauses': [{'operator': '=', 'constraint': 'blonde', 'variable': '?hair-color', 'type': 'HairColor'}, {'operator': '=', 'constraint': 'brown', 'variable': '?hair-color', 'type': 'HairColor'}]}]}}}
