...
# [GCC 8.4.0]
# Embedded file name: /Users/amandeep/Github/dig-sandpaper/test/coarse/execute/test_executing.py
# Compiled at: 2018-07-13 17:14:37
# Size of source mod 2**32: 10781 bytes
import unittest, json, codecs
from digsandpaper.coarse.execute.executor import Executor
import os, test.test_utils
_location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))

def load_json_file(file_name):
    with codecs.open(os.path.join(_location__, file_name), 'r', 'utf-8') as (json_file):
...
class TestCoarseExecuting(unittest.TestCase):
    ...
    def test_basic_coarse_executing(self):
        ...
        test.test_utils.initialize_elasticsearch([document], config['components'][0])
    ...
    def test_basic_coarse_executing_compound_filters_no_type_mapping(self):
        ...
        for query in queries:
            ...
            self.assertEqual('2015-03-01T14:02:00', result.hits[0]['fields']['posting-date']['strict']['name'])
    ...
    def test_basic_coarse_executing_sparql_unnesting(self):
        ...
        docs_by_type = {'ads':[ad_document],  'clusters':[
          cluster_document]}
        test.test_utils.initialize_elasticsearch_doc_types(docs_by_type, config['components'][0])
        executor = Executor(config)
        for query in queries:
            result = executor.execute(query)
    ...
    def test_basic_coarse_executing_aggregation(self):
        ...
        test.test_utils.initialize_elasticsearch([document], config['components'][0], mapping)
        executor = Executor(config)
        for query in queries:
            result = executor.execute(query)
            ...
            self.assertEqual(1, result_dict['aggregations']['?ethnicity']['buckets'][0]['doc_count'])
    ...
    def test_basic_coarse_executing_union_and_not_exists(self):
        ...
        test.test_utils.initialize_elasticsearch([document, document2, document3], config['components'][0])
        executor = Executor(config)
        for query in queries:
            result = executor.execute(query)
            result_dict = result.to_dict()
            self.assertEqual(len(result.hits), 1)
            self.assertEqual('QPONMLKJIHGFEDCBA', result_dict['hits']['hits'][0]['_source']['doc_id'])
    ...
    def test_basic_coarse_executing_network_expansion(self):
        ...
        test.test_utils.initialize_elasticsearch([document, document2, document3,
         document4, document5, document6], config['components'][0], mapping)
        executor = Executor(config)
        try:
            for query in queries:
                ...
                self.assertEqual('JKLMNOPQABCDEFGHI', result2_dict['hits']['hits'][4]['_source']['doc_id'])
    ...
    def test_basic_coarse_order_by(self):
        ...
        try:
            for query in queries:
                ...
                self.assertEqual('PQABCDEFGHIJKLMNO', result_dict['hits']['hits'][0]['_source']['doc_id'])
                self.assertEqual('DEFGHIJKLMNOPQABCD', result_dict['hits']['hits'][1]['_source']['doc_id'])
                self.assertEqual('GHIJKLMNOPQABCDEF', result_dict['hits']['hits'][2]['_source']['doc_id'])
                self.assertEqual('ABCDEFGHIJKLMNOPQ', result_dict['hits']['hits'][3]['_source']['doc_id'])
    ...
    def test_basic_coarse_rank_scoring_coefficient(self):
        ...
        test.test_utils.initialize_elasticsearch([document, document2, document3, document4], config['components'][0])
        executor = Executor(config)
        try:
            for query in queries:
                result = executor.execute(query)
                result_dict = result.to_dict()
                self.assertEqual(len(result.hits), 4)
                self.assertEqual('ABCDEFGHIJKLMNOPQ', result_dict['hits']['hits'][0]['_source']['doc_id'])
                self.assertEqual('DEFGHIJKLMNOPQABCD', result_dict['hits']['hits'][1]['_source']['doc_id'])
                self.assertEqual('GHIJKLMNOPQABCDEF', result_dict['hits']['hits'][2]['_source']['doc_id'])
                self.assertEqual('JKLMNOPQABCDEFGHI', result_dict['hits']['hits'][3]['_source']['doc_id'])
