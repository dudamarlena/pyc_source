...
# Embedded file name: build/bdist.linux-x86_64/egg/aiogcd/connector/client_token.py
# Compiled at: 2019-09-11 07:21:09
# Size of source mod 2**32: 7112 bytes
__doc__ = 'client_token.py\n\nCreated on: May 19, 2017\n    Author: Jeroen van der Heijden <jeroen@transceptor.technology>\n'
import os, time, json, asyncio, logging, aiohttp
...
class Token:
    ...
    def __init__(self, client_id, client_secret, token_file, scopes):
    ...
    def _update_refresh_ts(self):
        ...
        :return: None
        """
        self._refresh_ts = None if self._token is None else self._token['timestamp'] + self._token['expires_in'] // 2
    ...
    def _read_token_file(self):
        ...
        if not self._token_file.endswith('.json'):
            raise ValueError('Invalid token file: {}, expecting a .json file.'.format(self._token_file))
        if os.path.isfile(self._token_file):
            with open(self._token_file, 'r') as (f):
                token = json.load(f)
            missing = self._REQUIRED_TOKEN_FIELDS - set(token.keys())
            if missing:
                raise ValueError('Invalid token file: {} (fields are missing: {}).'.format(self._token_file, ', '.join(missing)))
            scopes = set(token['scopes'])
            if scopes ^ self._scopes:
                raise ValueError('Invalid scopes defined in token file: {file}.\nExpecting "{expecting}" but found "{found}".'.format(file=(self._token_file),
    ...
    async def connect(self):
        ...
        async with self._lock:
            if self._token is None:
                ...
                assert self._refresh_ts is not None, 'When having a token we should also have a refresh_ts value.'
                if self._refresh_ts < time.time():
                    await self._refresh_token()
                else:
    ...
    async def _refresh_token(self):
        logging.info('Token has exceeded half of the expiration time, starting a token refresh.')
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(TOKEN_URL, data=data) as resp:
                ...
                if resp.status == 200:
                    ...
                    raise ValueError('Got an error while asking for a token refresh: {} ({})'.format(content.get('error', 'unknown'), resp.status))
    ...
    async def _ask(self):
        url = '{}?{}'.format(AUTH_URL, urlencode({'response_type':'code', 
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(TOKEN_URL, data=data) as resp:
                    ...
                    if resp.status == 200:
                        ...
                        return content
                    print('Got an error: {} ({})'.format(content.get('error', 'unknown'), resp.status))