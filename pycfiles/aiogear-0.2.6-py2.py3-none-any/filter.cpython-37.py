...
# Embedded file name: build/bdist.linux-x86_64/egg/aiogcd/orm/filter.py
...
class Filter(dict):
    ...
    def __init__(self, model, *filters, has_ancestor=None, key=None):
        ...
        if has_ancestor is not None:
            ...
            filters.append({'property':{'name': '__key__'}, 
            ...
            if len(filters) == 1:
                filter_dict['query']['filter'] = {'propertyFilter': filters[0]}
            elif len(filters) > 1:
                filter_dict['query']['filter'] = {'compositeFilter': {'op':'AND', 
    ...
    def _set_start_cursor(self, start_cursor):
        if start_cursor:
            if not isinstance(start_cursor, str):
                raise TypeError('start_cursor is expected to be str, {} passed'.format(type(start_cursor)))
            self['query']['startCursor'] = start_cursor

    def _set_offset(self, offset):
        if offset:
            if not isinstance(offset, int):
                raise TypeError('offset is expected to be int, {} passed'.format(type(offset)))
            self['query']['offset'] = offset

    def _set_limit(self, limit):
        if limit:
            if not isinstance(limit, int):
                raise TypeError('limit is expected to be int, {} passed'.format(type(limit)))
    ...
    def order_by(self, *order):
        self['query']['order'] = [{'property':{'name': p[0]},  'direction':p[1]} if isinstance(p, tuple) else {'property':{'name': p.name},  'direction':'DIRECTION_UNSPECIFIED'} for p in order]
        return self
    ...
    async def get_keys(self, gcd: GcdConnector, offset=None, limit=None) -> list:
