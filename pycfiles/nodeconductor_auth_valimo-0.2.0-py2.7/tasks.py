...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_auth_valimo/tasks.py
...
class AuthTask(tasks.StateTransitionTask):
    ...
    def execute(self, auth_result):
        response = client.SignatureRequest.execute(transaction_id=auth_result.uuid.hex, phone=auth_result.phone, message=auth_result.message)
...
class PollTask(tasks.Task):
    ...
    def execute(self, auth_result):
        response = client.StatusRequest.execute(transaction_id=auth_result.uuid.hex, backend_transaction_id=auth_result.backend_transaction_id)
