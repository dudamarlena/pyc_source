...
# Embedded file name: build/bdist.linux-x86_64/egg/camsaxs/remesh_bbox.py
...
def remesh(image: np.ndarray, geometry: Geometry, reflection: bool, alphai: float, bins: Tuple[(int, int)]=None, q_h_range: Tuple[(float, float)]=None, q_v_range: Tuple[(float, float)]=None, out_range=None, res=None, coord_sys='qp_qz'):
    """
    ...
    """
    if not image.shape == geometry.detector.shape:
    ...
    if q_v_range is None:
        q_v_range = (
         q_v.min(), q_v.max())
    I, q_h, q_v, _, _ = splitBBox.histoBBox2d(weights=image, pos0=q_h,
...
if __name__ == '__main__':
    ...
    g = Geometry(wavelength=1.23984e-10)
    (g.setFit2D)(**fit2d)
    g.detector = det()
    ...
    I, q_h, q_v = remesh(data, g, False, 0)
    test_show(I)