...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pepg/pepg.py
# Compiled at: 2020-03-31 16:53:52
...
def utility_function(population_size: int) -> List[float]:
    utility = np.clip(np.log(population_size / 2 + 1) - np.log(np.arange(population_size, 0, -1)), 0, None)
    utility /= np.sum(np.clip(np.log(population_size / 2 + 1) - np.log(np.arange(1, population_size + 1)), 0, None))
...
class PEPG:
    ...
    def __init__(self, population_size: int, theta_size: int, mu_init: float, sigma_init: float, mu_lr: float, sigma_lr: float, l2_coeff: float=0.005, optimizer=Adam, optimizer_kwargs={}):
    ...
    def save_checkpoint(self):
        import time, pathlib, os, inspect
        frame = inspect.stack()[1]
        module = inspect.getmodule(frame[0])
        folder = pathlib.Path((f"{os.path.join(os.path.dirname(module.__file__), 'checkpoints')}"))
