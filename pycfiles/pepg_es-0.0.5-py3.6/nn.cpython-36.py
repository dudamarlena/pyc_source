...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pepg/nn.py
# Compiled at: 2020-03-31 10:51:15
...
class NeuralNetwork(object):
    ...
    def __init__(self, input_size: int, output_size: int, hidden_sizes: List[int]=[], hidden_activation: Callable[([float], float)]=lambda x: x, output_activation: Callable[([float], float)]=lambda x: x, bias: bool=True):
    ...
    def __call__(self, value):
        ...
        w = weights[index:]
        w = w.reshape(self.sizes[(len(self.sizes) - 2)] + (1 if self.bias else 0), self.sizes[(len(self.sizes) - 1)])
