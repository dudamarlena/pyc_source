...
# [GCC 8.4.0]
# Embedded file name: /home/threebean/devel/datagrepper/tests/test_cors.py
# Compiled at: 2018-02-21 15:09:41
...
class TestCORS(unittest.TestCase):
    ...
    @patch.dict(datagrepper.app.app.config, CORS_DOMAINS=[re.compile('http://safe\\.site'),
     re.compile('http://.*\\.honest')])
    @patch('datagrepper.app.dm.Message.grep', return_value=(0, 0, []))
    ...
    def test_preflight_cors_methods(self, grep):
        resp = self.client.options('/raw', headers={'Origin': 'http://preflight.example.com', 'Access-Control-Request-Method': 'GET'})
        self.assertEqual(resp.status_code, 200)
        self.assertTrue('Access-Control-Allow-Origin' in resp.headers)
        self.assertEqual(resp.headers['Access-Control-Allow-Origin'], 'http://preflight.example.com')
    ...
    @patch.dict(datagrepper.app.app.config, CORS_HEADERS=[re.compile('Content-.*'),
     re.compile('X-Other-.*')])
    @patch('datagrepper.app.dm.Message.grep', return_value=(0, 0, []))
