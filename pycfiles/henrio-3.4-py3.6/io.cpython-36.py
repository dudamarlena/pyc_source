...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\henrio\io.py
# Compiled at: 2017-12-19 23:22:40
# Size of source mod 2**32: 9002 bytes
import typing, socket, errno, os
...
try:
    ...
    WantRead = (
     BlockingIOError, InterruptedError, SSLWantReadError)
...
yerrlist = [
 ...
 'EINVAL',
 'WSAEINVAL']
yerrors = {getattr(errno, name) for name in yerrlist if hasattr(errno, name) if hasattr(errno, name)}
...
def _async_connect(sock, host):
    ...
    while True:
        ...
        if err in yerrors:
            ...
            if err in (getattr(errno, 'EISCONN'), getattr(errno, 'WSAEISCONN')):
                break
            else:
...
async def open_connection(hostpair: tuple, timeout=None, *, ssl=False, source_addr=None, server_hostname=None, alpn_protocols=None):
    """Open a new asynchronous connection (like `socket.create_connection`) and returns a new `henrio.AsyncSocket` instance"""
    if timeout is not None:
        async with _timeout(timeout):
            open_connection(hostpair, timeout, ssl=ssl, source_addr=source_addr, server_hostname=server_hostname, alpn_protocols=alpn_protocols)
        return
    else:
        sock = socket.socket()
        if source_addr:
...
class AsyncFile(BaseSocket):
    __doc__ = 'A wrapped file object with all methods run in a threadpool'

    def __init__(self, file, mode='r', *args, **kwargs):
        self.file = open(file, *args, mode=mode, **kwargs)
