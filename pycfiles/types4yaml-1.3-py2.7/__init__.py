...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/types4yaml/__init__.py
# Compiled at: 2017-04-27 19:45:19
"""
Types4YAML
==========
...
import datetime, re, StringIO, yaml
    ...
    def check_atom_time(self, x, ctx):
        if not isinstance(x, basestring):
            ...
            if m.group(4) is not None:
                us = int(1000000.0 * float('0' + m.group(4)))
            ok = True
            if hh < 0 or hh >= 24:
                ok = False
            if mm < 0 or mm >= 60:
            ...
            if ss < 0 or ss >= 60:
                ok = False
            if not (0 <= us and us < 1000000):
                raise AssertionError
                ok or ctx.error('invalid time ' + x)
                return
