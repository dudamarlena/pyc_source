...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/sockspy/core/engine.py
# Compiled at: 2017-07-30 05:12:50
# Size of source mod 2**32: 7521 bytes
import enum, selectors, socket, time, abc, six
...
class StatusHandlerEngine(EventHandlerEngine):
    ...
    def handle_event(self, endpoint):
        ...
        if selectors.EVENT_READ & endpoint.event:
            ...
            if self.get_status(endpoint) == ProtocolStatus.Connecting:
                pass
            else:
                endpoint.write()
...
class SocksEngine(StatusHandlerEngine):
    ...
    def get_handler_by_status(self, status, endpoint):
        if status == ProtocolStatus.ProtocolValidated:
            return {selectors.EVENT_READ: self.handle_read, 
             selectors.EVENT_WRITE: self.handle_write}.get(endpoint.event)
        else:
