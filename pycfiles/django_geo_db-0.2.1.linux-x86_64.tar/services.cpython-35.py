...
# [GCC 8.4.0]
# Embedded file name: /home/trjg/code/venv/django-geo-db/lib/python3.5/site-packages/django_geo_db/services.py
# Compiled at: 2018-03-10 23:59:04
...
from django_geo_db.models import UserLocation, Zipcode, Location, Country, State, LocationMap, LocationBounds, LocationMapType
import os
...
class LocationMapGenerator:
    ...
    def get_or_generate_location_map(self, type, location):
        ...
        if not map:
            url, base_map = self._LocationMapGenerator__get_base_map(type, location)
            coord_obj = None
            ...
            map.type = type
            if coord_obj:
                combined_image = self._LocationMapGenerator__generate_detail_map_image(base_map, type, location, location_bounds, coord_obj)
                new_url = self._LocationMapGenerator__save_map_and_return_url_of_detailed_map(type, location, combined_image)
    ...
    def __generate_detail_map_image(self, base_map, type, location, location_bounds, coord_obj):
        ...
        coord_star = self._LocationMapGenerator__get_star()
    ...
    def __save_map_and_return_url_of_detailed_map(self, location_type, location, combined_image):
        url = self._LocationMapGenerator__get_url(location_type, location, is_base_map=False)
        x = default_storage.save(url, combined_image)
        return url

    def __get_url(self, type, location, is_base_map=True):
        url = 'django_geo_db/maps/{type}/{country}/'.format(type=type, country=location.country.name.lower().replace(' ', '-'))
        if location.geocoordinate and not is_base_map:
            url += str(location.geocoordinate)
        else:
            if location.zipcode and not is_base_map:
                url += str(location.zipcode.zipcode)
            else:
                if location.city and not is_base_map:
                    state = location.state.name.lower().replace(' ', '-')
                    url += state + '-'
                    url += location.city.name.lower().replace(' ', '-')
                else:
                    if location.county and not is_base_map:
                        state = location.state.name.lower().replace(' ', '-')
                        url += state + '-'
    ...
    def __get_base_map(self, type, location):
        url = 'img/' + self._LocationMapGenerator__get_url(type, location)
        url = static(url)
        if 'http' not in url:
            url = self.domain + url
