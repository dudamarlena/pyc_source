...
# [GCC 8.4.0]
# Embedded file name: /home/jeanphix/www/Flask-Dashed/flask_dashed/ext/sqlalchemy.py
# Compiled at: 2012-06-26 16:19:49
...
class ModelAdminModule(ObjectAdminModule):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not cls.list_fields:
            ...
            for column in cls.model.__table__._columns:
                cls.list_fields[column.name] = {'label': column.name, 'column': getattr(cls.model, column.name)}
    ...
    def get_object_list(self, search=None, order_by_name=None, order_by_direction=None, offset=None, limit=None):
        ...
        if not (order_by_name and order_by_direction) and self.order_by is not None:
            order_by_name = self.order_by[0]
            order_by_direction = self.order_by[1]
        if order_by_name and order_by_direction:
            try:
                query = query.order_by(getattr(self.list_fields[order_by_name]['column'], order_by_direction)())
            except KeyError:
                raise Exception('Order by field must be provided in ' + 'list_fields with a column key')
    ...
    def get_actions_for_object(self, object):
        ...
        return [
         (
          ...
          url_for('%s.%s_edit' % (self.admin.blueprint.name, self.endpoint), pk=object.id)),
         (
          'delete', 'delete', 'Delete object',
          url_for('%s.%s_delete' % (self.admin.blueprint.name, self.endpoint), pk=object.id))]
    ...
    def _get_filtered_query(self, query, search=None):
        ...
        if search and self.searchable_fields:
            ...
            for field in self.searchable_fields:
                if field in self.list_fields and 'column' in self.list_fields[field]:
                    if condition is None:
                        condition = self.list_fields[field]['column'].contains(search)
                    else:
                        condition = or_(condition, self.list_fields[field]['column'].contains(search))
                else:
                    raise Exception('Searchables fields must be in ' + 'list_fields with specified column.')
