...
# [GCC 8.4.0]
# Embedded file name: /home/jeanphix/www/Flask-Dashed/flask_dashed/views.py
...
class DashboardView(MethodView, AdminModuleMixin):
    ...
    def get(self):
        return render_template('flask_dashed/dashboard.html', admin=self.admin_module.admin, module=self.admin_module)
...
class ObjectListView(MethodView, AdminModuleMixin):
    ...
    def get(self, page=1):
        ...
        return render_template(self.admin_module.list_template, admin=self.admin_module.admin, module=self.admin_module, objects=self.admin_module.get_object_list(search=search, offset=self.admin_module.list_per_page * (page - 1), limit=self.admin_module.list_per_page, order_by_name=order_by, order_by_direction=order_direction), count=count, current_page=page, pages=self.iter_pages(count, page), compute_args=compute_args)

    def iter_pages(self, count, current_page, left_edge=2, left_current=2, right_current=5, right_edge=2):
        per_page = self.admin_module.list_per_page
        pages = int(ceil(count / float(per_page)))
        last = 0
        for num in xrange(1, pages + 1):
            if num <= left_edge or num > current_page - left_current - 1 and num < current_page + right_current or num > pages - right_edge:
                if last + 1 != num:
                    yield
                yield num
                last = num
    ...
    def object(self):
        ...
        if not hasattr(self, '_object'):
            if 'pk' in request.view_args:
                self._object = self.admin_module.get_object(request.view_args['pk'])
            else:
