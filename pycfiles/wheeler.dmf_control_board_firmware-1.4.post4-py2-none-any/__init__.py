...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ryan\Documents\dev\python\dmf-control-board-firmware\dmf_control_board_firmware\__init__.py
# Compiled at: 2017-01-03 14:10:02
"""
...
import decorator, logging, math, os, re, time, types, warnings
...
import matplotlib.mlab as mlab, numpy as np, pandas as pd, serial_device as sd
...
def savgol_filter(x, window_length, polyorder, deriv=0, delta=1.0, axis=-1, mode='interp', cval=0.0):
    ...
    try:
        ...
        x[ind:] = signal.savgol_filter(x[ind:], window_length, polyorder, deriv, delta, axis, mode, cval)
    except IndexError:
        pass
...
RETURN_ATTRS = [ attr for attr in dir(Base) if attr.startswith('RETURN_')
               ]
RETURN_CODES_BY_NAME = pd.Series([ getattr(Base, attr) for attr in RETURN_ATTRS
                                 ], index=[ attr[len('RETURN_'):] for attr in RETURN_ATTRS
                                          ])
RETURN_CODES_BY_NAME.sort_values(inplace=True)
NAMES_BY_RETURN_CODE = pd.Series(RETURN_CODES_BY_NAME.index, index=RETURN_CODES_BY_NAME)
COMMAND_ATTRS = [ attr for attr in dir(Base) if attr.startswith('CMD_')
                ]
COMMAND_CODES_BY_NAME = pd.Series([ getattr(Base, attr) for attr in COMMAND_ATTRS
                                  ], index=[ attr[len('CMD_'):] for attr in COMMAND_ATTRS
                                           ])
COMMAND_CODES_BY_NAME.sort_values(inplace=True)
NAMES_BY_COMMAND_CODE = pd.Series(COMMAND_CODES_BY_NAME.index, index=COMMAND_CODES_BY_NAME)
CRE_REMOTE_ERROR = re.compile('Error sending command\\s+0x(?P<command_hex>[0-9a-fA-F]+)\\s+\\((?P<command_int>\\d+)\\).\\s+Return code=(?P<return_code_int>\\d+).')
CRE_REMOTE_COMMAND_ERROR = re.compile('[Cc]ommand\\s+0x(?P<command_hex>[0-9a-fA-F]+)\\s+\\((?P<command_int>\\d+)\\)')
...
class FirmwareError(Exception):
    ...
    def __str__(self):
        return '%s [command=%s]' % (
         NAMES_BY_RETURN_CODE.get(self.return_code, self.return_code),
         NAMES_BY_COMMAND_CODE.get(self.command_code, self.command_code))
...
def feedback_results_to_measurements_frame(feedback_result):
    ...
    `pandas.DataFrame`.
    """
    index = pd.Index(feedback_result.time * 0.001, name='seconds')
    df_feedback = pd.DataFrame(np.column_stack([feedback_result.V_fb,
     feedback_result.V_hv,
     feedback_result.fb_resistor,
     feedback_result.hv_resistor]), columns=[
     'V_fb', 'V_hv', 'fb_resistor',
     'hv_resistor'], index=index)
...
def feedback_results_to_impedance_frame(feedback_result):
    ...
    """
    index = pd.Index(feedback_result.time * 0.001, name='seconds')
    df_feedback = pd.DataFrame(np.column_stack([
     feedback_result.V_actuation().filled(np.NaN),
     feedback_result.capacitance().filled(np.NaN),
     feedback_result.Z_device().filled(np.NaN)]), columns=[
     'V_actuation', 'capacitance',
...
class FeedbackResults():
    ...
    def __init__(self, voltage, frequency, dt_ms, V_hv, hv_resistor, V_fb, fb_resistor, calibration, area=0, amplifier_gain=None, vgnd_hv=None, vgnd_fb=None):
    ...
    def _upgrade(self):
        ...
        if version > Version.fromstring(self.class_version):
            ...
            if version < Version(0, 6):
                ...
                logging.info('[FeedbackResults] upgrade to version %s' % self.version)
        return
    ...
    def V_total(self):
        ...
        V1.fill(np.nan)
        V1[ind] = compute_from_transfer_function(self.calibration.hw_version.major, 'V1', V2=self.V_hv[ind], R1=10000000.0, R2=self.calibration.R_hv[self.hv_resistor[ind]], C2=self.calibration.C_hv[self.hv_resistor[ind]], f=self.frequency)
        V1 = np.ma.masked_invalid(pd.Series(V1, pd.to_datetime(self.time, unit='s')).interpolate(method='time').values)
    ...
    def Z_device(self, filter_order=None, window_size=None, tol=0.05):
        ...
        Z1[ind] = compute_from_transfer_function(self.calibration.hw_version.major, 'Z1', V1=self.V_total()[ind], V2=self.V_fb[ind], R2=R2, C2=C2, f=self.frequency)
        Z1 = np.ma.masked_invalid(pd.Series(Z1, pd.to_datetime(self.time, unit='s')).interpolate(method='time').values)
        ...
        if filter_order is None or window_size is None or window_size < filter_order + 2:
            pass
        elif window_size and window_size < len(Z1) / 2:
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                Z1 = savgol_filter(Z1, window_size, filter_order)
        else:
            result = self.mean_velocity(tol=tol)
            if result['dt'] and result['dt'] > 0.1 * self.time[(-1)] and result['p'][0] > 0:
                ...
                C = self.area * (x * (c_drop - c_filler) / np.sqrt(self.area) + c_filler)
                Z1 = 1.0 / (2.0 * math.pi * self.frequency * C)
                Z1[(mlab.find(self.time == result['t_end'])[0] + 1):] = Z1[mlab.find(self.time == result['t_end'])[0]]
            else:
    ...
    def capacitance(self, filter_order=None, window_size=None, tol=0.05):
        ...
        """
        C = np.ma.masked_invalid(1.0 / (2.0 * math.pi * self.frequency * self.Z_device(filter_order=filter_order, window_size=window_size, tol=tol)))
        C.fill_value = np.nan
    ...
    def mean_velocity(self, tol=0.05, Lx=None):
        ...
        if self.area == 0:
            ...
            if len(ind) >= 2:
                dx = np.diff(x[ind])[0]
                dt = np.diff(self.time[ind])[0]
                with warnings.catch_warnings():
                    warnings.simplefilter('ignore')
                    p = np.polyfit(self.time[ind[0]:ind[1]], x[ind[0]:ind[1]], 1)
                ind_stop = mlab.find(self.time > (x[ind_last] - p[1]) / p[0])
                if len(ind_stop):
    ...
    def _get_window_size(self, tol=0.05):
        ...
        if result['dt'] and result['dt'] > 0.1 * self.time[(-1)] and result['p'][0] > 0:
            ...
            window_size = np.round(0.5 / (mean_dxdt / np.sqrt(self.area)) / dt / 2.0) * 2.0 + 1
        return window_size
    ...
    def to_frame(self, filter_order=3):
        ...
        if velocity_results and velocity_results['dx']:
            mean_velocity = velocity_results['p'][0] * 1000.0
            dx = velocity_results['dx']
            dt = velocity_results['dt'] * 0.001
            t, dxdt = self.dxdt(Lx=L)
            dxdt = np.interp(self.time, t, dxdt) * 1000.0
            dxdt = np.ma.masked_invalid(dxdt)
            t, dxdt_filtered = self.dxdt(filter_order=filter_order, Lx=L)
            dxdt_filtered = np.interp(self.time, t, dxdt_filtered) * 1000.0
            dxdt_filtered = np.ma.masked_invalid(dxdt_filtered)
            peak_velocity = np.max(dxdt_filtered)
        index = pd.Index(self.time * 0.001, name='step_time')
        df = pd.DataFrame({'target_voltage': self.voltage, 'voltage': self.V_actuation(), 
           'force': self.force(Ly=1.0) * 1000000.0, 
           'Z_device_filtered': self.Z_device(filter_order=filter_order), 
           'capacitance_filtered': self.capacitance(filter_order=filter_order), 
           'x_position_filtered': self.x_position(filter_order=filter_order), 
           'dxdt_filtered': dxdt_filtered, 
           'Z_device': self.Z_device(), 
           'capacitance': self.capacitance(), 
           'x_position': self.x_position(), 
           'dxdt': dxdt}, index=index)
        df['frequency'] = self.frequency
        df['area'] = self.area
        df['dx'] = dx
        df['dt'] = dt
        df['mean_velocity'] = mean_velocity
        df['peak_velocity'] = peak_velocity
        df['window_size'] = window_size
        df['filter_order'] = filter_order
        return df[['frequency', 'target_voltage', 'voltage', 'force', 'area',
         'Z_device_filtered', 'capacitance_filtered', 'x_position_filtered',
         'dxdt_filtered', 'Z_device', 'capacitance', 'x_position', 'dxdt',
         'dx', 'dt', 'mean_velocity', 'peak_velocity',
...
class FeedbackResultsSeries():
    ...
    def _upgrade(self):
        ...
        version = Version.fromstring(self.version)
        logging.debug('[FeedbackResultsSeries] version=%s, class_version=%s', str(version), self.class_version)
        if version > Version.fromstring(self.class_version):
...
class FeedbackCalibration():
    ...
    def __init__(self, R_hv=None, C_hv=None, R_fb=None, C_fb=None, c_drop=None, c_filler=None, hw_version=None):
        if R_hv:
            self.R_hv = np.array(R_hv)
        else:
            self.R_hv = np.array([87000.0, 640000.0])
        if C_hv:
        ...
        if R_fb:
            ...
            self.R_fb = np.array([1140.0, 10000.0, 93000.0, 650000.0])
        if C_fb:
    ...
    def _upgrade(self):
        ...
        version = Version.fromstring(self.version)
        logging.debug('[FeedbackCalibration] version=%s, class_version=%s', str(version), self.class_version)
        if version > Version.fromstring(self.class_version):
            ...
            if version < Version(0, 2):
                self.hw_version = Version(1)
                self.version = str(Version(0, 2))
                logging.info('[FeedbackCalibration] upgrade to version %s', self.version)
            if version < Version(0, 2):
                self.hw_version = Version(1)
                self.version = str(Version(0, 2))
                logging.info('[FeedbackCalibration] upgrade to version %s', self.version)
...
def safe_series_resistor_index_read(f, self, channel, resistor_index=None):
    ...
    if resistor_index is not None and resistor_index != original_resistor_index:
        self.set_series_resistor_index(channel, original_resistor_index)
    return value


@decorator.decorator
def safe_series_resistor_index_write(f, self, channel, value, resistor_index=None):
    """
    This decorator checks the resistor-index from the current context _(i.e.,
    the result of `self.series_resistor_index`)_.  If the resistor-index
    specified by the `resistor_index` keyword argument is different than the
...
def remote_command(function, self, *args, **kwargs):
    ...
    try:
        ...
        if match:
            ...
            raise RuntimeError(CRE_REMOTE_COMMAND_ERROR.sub(command_name, error_message))
        raise
...
class DMFControlBoard(Base):
    ...
    def config_version(self):
        return tuple(self.persistent_read_multibyte(self.PERSISTENT_CONFIG_SETTINGS, count=3, dtype=np.uint16))
    ...
    def connect(self, port=None, baud_rate=115200):
        ...
        for comport_i in ports:
            ...
            raise RuntimeError('Could not connect to control board on any of the following ports: %s' % ports)
        ...
        logger.info('Poll control board for series resistors and capacitance values.')
        ...
        for channel in channels:
            try:
                ...
                logger.info('A%d VGND = %.2f V (%.2f%% of Aref)', channel, self.__aref__ * v[channel] / 1024, 100.0 * v[channel] / 1024)
                if np.abs(v[channel] - expected) / expected > 0.1:
        ...
        if damaged:
            ...
            raise BadVGND(msg + ' to be damaged. You may need to replace the op-amp on the control board.')
    ...
    def _read_calibration_data(self):
        ...
        self.calibration = FeedbackCalibration(R_hv, C_hv, R_fb, C_fb, hw_version=Version.fromstring(self.hardware_version()))
    ...
    @property
    def baud_rate(self):
        return self.persistent_read_multibyte(self.PERSISTENT_BAUD_RATE_ADDRESS, dtype=np.uint32)

    @baud_rate.setter
    def baud_rate(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_BAUD_RATE_ADDRESS, np.array([value], dtype=np.uint32), True)
        self.__baud_rate = value

    @property
    def serial_number(self):
        return self.persistent_read_multibyte(self.PERSISTENT_SERIAL_NUMBER_ADDRESS, dtype=np.uint32)

    @serial_number.setter
    def serial_number(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_SERIAL_NUMBER_ADDRESS, np.array([value], dtype=np.uint32), True)
        self.__serial_number = value

    @property
    def voltage_tolerance(self):
        if not hasattr(self, '__voltage_tolerance'):
            self.__voltage_tolerance = self.persistent_read_multibyte(self.PERSISTENT_VOLTAGE_TOLERANCE, dtype=np.float32)
        return self.__voltage_tolerance

    @voltage_tolerance.setter
    def voltage_tolerance(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_VOLTAGE_TOLERANCE, np.array([value], dtype=np.float32), True)
    ...
    def use_antialiasing_filter(self, value):
        return self.persistent_write(self.PERSISTENT_USE_ANTIALIASING_FILTER, value, True)

    @property
    def min_waveform_frequency(self):
        if not hasattr(self, '__min_waveform_frequency'):
            self.__min_waveform_frequency = self.persistent_read_multibyte(self.PERSISTENT_MIN_WAVEFORM_FREQUENCY, dtype=np.float32)
        return self.__min_waveform_frequency

    @min_waveform_frequency.setter
    def min_waveform_frequency(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_MIN_WAVEFORM_FREQUENCY, np.array([value], dtype=np.float32), True)
        self.__min_waveform_frequency = value

    @property
    def max_waveform_frequency(self):
        if not hasattr(self, '__max_waveform_frequency'):
            self.__max_waveform_frequency = self.persistent_read_multibyte(self.PERSISTENT_MAX_WAVEFORM_FREQUENCY, dtype=np.float32)
        return self.__max_waveform_frequency

    @max_waveform_frequency.setter
    def max_waveform_frequency(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_MAX_WAVEFORM_FREQUENCY, np.array([value], dtype=np.float32), True)
        self.__max_waveform_frequency = value

    @property
    def max_waveform_voltage(self):
        if not hasattr(self, '__max_waveform_voltage'):
            self.__max_waveform_voltage = self.persistent_read_multibyte(self.PERSISTENT_MAX_WAVEFORM_VOLTAGE, dtype=np.float32)
        return self.__max_waveform_voltage

    @max_waveform_voltage.setter
    def max_waveform_voltage(self, value):
        self.persistent_write_multibyte(self.PERSISTENT_MAX_WAVEFORM_VOLTAGE, np.array([value], dtype=np.float32), True)
    ...
    def default_pin_modes(self, pin_modes):
        for i in range(0, 53 / 8 + 1):
            ...
            self.persistent_write(self.PERSISTENT_PIN_MODE_ADDRESS + i, ~mode & 255, True)
    ...
    def default_pin_states(self, pin_states):
        for i in range(0, 53 / 8 + 1):
            ...
            self.persistent_write(self.PERSISTENT_PIN_STATE_ADDRESS + i, ~state & 255, True)
    ...
    @remote_command
    def measure_impedance_non_blocking(self, sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, rms, state):
        state_ = uint8_tVector()
        for i in range(0, len(state)):
            state_.append(int(state[i]))

        Base.measure_impedance_non_blocking(self, sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, rms, state_)

    @remote_command
    def sweep_channels_non_blocking(self, sampling_window_ms, n_sampling_windows_per_channel, delay_between_windows_ms, interleave_samples, rms, channel_mask):
        ...
        Base.sweep_channels_non_blocking(self, sampling_window_ms, n_sampling_windows_per_channel, delay_between_windows_ms, interleave_samples, rms, channel_mask_)
    ...
    def measure_impedance_buffer_to_feedback_result(self, buffer):
        ...
        dt_ms = buffer[(-4)]
        buffer = buffer[:-4]
        V_hv = buffer[0::4] / 65472.0 * self.__aref__ / 2.0 / np.sqrt(2)
        hv_resistor = buffer[1::4].astype(int)
        V_fb = buffer[2::4] / 65472.0 * self.__aref__ / 2.0 / np.sqrt(2)
        fb_resistor = buffer[3::4].astype(int)
        voltage = self.waveform_voltage()
        frequency = self.waveform_frequency()
        return FeedbackResults(voltage, frequency, dt_ms, V_hv, hv_resistor, V_fb, fb_resistor, self.calibration, amplifier_gain=amplifier_gain, vgnd_hv=vgnd_hv, vgnd_fb=vgnd_fb)
    ...
    def sweep_channels_buffer_to_feedback_result(self, buffer):
        ...
        for channel_i, active_i in enumerate(self._channel_mask_cache):
            if not active_i:
                continue
            V_hv_i = V_hv[i * n_samples_per_channel:(i + 1) * n_samples_per_channel]
            hv_resistor_i = hv_resistor[i * n_samples_per_channel:(i + 1) * n_samples_per_channel]
            V_fb_i = V_fb[i * n_samples_per_channel:(i + 1) * n_samples_per_channel]
            fb_resistor_i = fb_resistor[i * n_samples_per_channel:(i + 1) * n_samples_per_channel]
            feedback_results_i = FeedbackResults(voltage, frequency, dt_ms, V_hv_i, hv_resistor_i, V_fb_i, fb_resistor_i, self.calibration, amplifier_gain=amplifier_gain, vgnd_hv=vgnd_hv, vgnd_fb=vgnd_fb)
            df_impedances_i = feedback_results_to_impedance_frame(feedback_results_i)
            df_impedances_i.insert(2, 'channel_i', channel_i)
            frames.append(df_impedances_i)
            i += 1

        df_impedances = pd.concat(frames)
        index = pd.Index(dt_ms * np.arange(df_impedances.shape[0]) * 0.001, name='seconds')
    ...
    @remote_command
    def measure_impedance(self, sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, rms, state):
        """
        ...
        buffer = np.array(Base.measure_impedance(self, sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, rms, state_))
        return self.measure_impedance_buffer_to_feedback_result(buffer)

    @remote_command
    def sweep_channels(self, sampling_window_ms, n_sampling_windows_per_channel, delay_between_windows_ms, interleave_samples, rms, channel_mask):
        """
        Measure voltage across load of each of the following control board
        feedback circuits:
        ...
        max_channels_per_call = (self.MAX_PAYLOAD_LENGTH - 16) / 6 / n_sampling_windows_per_channel
        self._channel_mask_cache = np.array(channel_mask)
        buffer = np.zeros(4)
        for i in range(int(math.ceil(n_channels_in_mask / max_channels_per_call))):
            ind = np.logical_and(channel_cumsum >= i * max_channels_per_call, channel_cumsum < (i + 1) * max_channels_per_call)
            ...
            buffer = np.concatenate((buffer,
             np.array(Base.sweep_channels(self, sampling_window_ms, n_sampling_windows_per_channel, delay_between_windows_ms, interleave_samples, rms, channel_mask_uint8))))

        return self.sweep_channels_buffer_to_feedback_result(buffer)

    @remote_command
    def sweep_channels_slow(self, sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, use_rms, channel_mask):
        """
        Measure voltage across load of each of the following control board
        feedback circuits:
        ...
        **N.B.,** Use one firmware call per channel, as opposed to scanning all
        ...
        for channel_i, state_i in enumerate(channel_mask):
            if state_i:
                scan_count_i += 1
                print ('\rMeasure impedance: {} ({}/{})').format(channel_i, scan_count_i, scan_count),
                channel_states_i = [0] * channel_count
                channel_states_i[channel_i] = 1
                start_time_i = datetime.utcnow()
                feedback_results_i = self.measure_impedance(sampling_window_ms, n_sampling_windows, delay_between_windows_ms, interleave_samples, use_rms, channel_states_i)
                df_result_i = feedback_results_to_impedance_frame(feedback_results_i)
    ...
    def i2c_send_command(self, address, cmd, data, delay_ms=100):
        ...
        return np.array(Base.i2c_send_command(self, address, cmd, data_, delay_ms))
    ...
    def flash_firmware(self, hardware_version=None):
        ...
        try:
            hex_path = package_path().joinpath('firmware', 'mega2560', '%s_%s' % (
             ...
             hardware_version.minor), 'dmf_control_board.hex')
            logger.info('hex_path=%s' % hex_path)
            context = auto_context()
            board = Board(context, 'mega2560')
            uploader = Uploader(board)
            logger.info('flashing firmware: hardware version %s' % hardware_version)
    ...
    def PERSISTENT_AREF_ADDRESS(self):
        ...
        if hardware_version == '1.0' or hardware_version == '1.1' or hardware_version == '1.2':
    ...
    @aref.setter
    def aref(self, value):
        return self.persistent_write(self.PERSISTENT_AREF_ADDRESS, value, True)

    @property
    def switching_board_i2c_address(self):
        return self.persistent_read(self.PERSISTENT_SWITCHING_BOARD_I2C_ADDRESS)

    @switching_board_i2c_address.setter
    def switching_board_i2c_address(self, value):
        return self.persistent_write(self.PERSISTENT_SWITCHING_BOARD_I2C_ADDRESS, value, True)
    ...
    def waveout_gain_1(self, value):
        return self.persistent_write(self.PERSISTENT_WAVEOUT_GAIN_1_ADDRESS, value, True)
    ...
    def signal_generator_board_i2c_address(self):
        return self.persistent_read(self.PERSISTENT_SIGNAL_GENERATOR_BOARD_I2C_ADDRESS)

    @signal_generator_board_i2c_address.setter
    def signal_generator_board_i2c_address(self, value):
        return self.persistent_write(self.PERSISTENT_SIGNAL_GENERATOR_BOARD_I2C_ADDRESS, value, True)
    ...
    def write_all_series_channel_values(self, read_f, write_f, channel, values):
        """
        Return all values for the specified channel of the type corresponding
        to the function `f`, where `f` is either `self.series_resistance` or
        `self.series_capacitance`.
    ...
    def a0_series_resistance(self, values):
        return self.write_all_series_channel_values(self.series_resistance, self.set_series_resistance, 0, values)
    ...
    def a0_series_capacitance(self, values):
        return self.write_all_series_channel_values(self.series_capacitance, self.set_series_capacitance, 0, values)
    ...
    def a1_series_resistance(self, values):
        return self.write_all_series_channel_values(self.series_resistance, self.set_series_resistance, 1, values)
    ...
    def a1_series_capacitance(self, values):
        return self.write_all_series_channel_values(self.series_capacitance, self.set_series_capacitance, 1, values)
    ...
    def read_config(self):
        except_types = (
         PersistentSettingDoesNotExist,)
        return OrderedDict([ (a, safe_getattr(self, a, except_types)) for a in self.config_attribute_names
                           ])
    ...
    def write_config(self, config):
        ...
        for k in device_config.keys():
            if k in common_keys and device_config[k] is not None and config[k] is not None:
