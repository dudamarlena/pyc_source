...
# [GCC 8.4.0]
# Embedded file name: c:\users\ryan\documents\dev\python\dmf-control-board-firmware\dmf_control_board_firmware\gui\impedance.py
# Compiled at: 2016-11-30 18:21:49
import pkg_resources, numpy as np, pandas as pd, gtk
...
from dmf_control_board_firmware.calibrate.impedance import run_experiment, fit_fb_calibration, apply_calibration, update_fb_calibration
from dmf_control_board_firmware.calibrate.impedance_benchmarks import plot_stat_summary
...
class AssistantView(WindowView):
    ...
    def __init__(self, control_board, impedance_readings=None, calibration=None):
        self.control_board = control_board
        self.settings = {}
        if control_board is not None:
            self.settings['frequency'] = self.control_board.waveform_frequency()
            self.settings['channel_states'] = self.control_board.state_of_all_channels
            self.settings['voltage'] = self.control_board.waveform_voltage()
            self.settings['amplifier_gain'] = self.control_board.amplifier_gain
            self.settings['auto_adjust_amplifier_gain'] = self.control_board.auto_adjust_amplifier_gain
    ...
    def restore_settings(self):
        if self.control_board is not None:
            ...
            self.control_board.set_state_of_all_channels(self.settings['channel_states'])
        return
    ...
    def create_ui(self):
        ...
        content = 'This wizard will guide you through the process of calibrating the device load feedback measurement circuit. This feedback circuit is used to measure the impedance between the actuated area and ground.  This impedance is related to the volume of liquid between the actuated area\n\nSee <a href="http://microfluidics.utoronto.ca/trac/dropbot/wiki/Control board calibration#device-load-impedance-calibration">here</a> for more details.'
        label = gtk.Label(content)
        label.set_use_markup(True)
        label.set_line_wrap(True)
        image = gtk.Image()
        img_path = pkg_resources.resource_filename('dmf_control_board_firmware', 'gui/impedance_feedback_intro.png')
        ...
        image = gtk.Image()
        img_path = pkg_resources.resource_filename('dmf_control_board_firmware', 'gui/impedance_feedback_setup.png')
        image.set_from_file(str(img_path))
        label.set_line_wrap(True)
        label.set_use_markup(True)
        box.pack_start(label, True, False, padding=15)
        box.pack_start(image, True, True, padding=5)
        self.widget.set_page_complete(box, True)
        form = Form.of(Integer.named('start_frequency').using(default=self.control_board.min_waveform_frequency, optional=True, validators=[
         ValueAtLeast(minimum=self.control_board.min_waveform_frequency),
         ValueAtMost(maximum=self.control_board.max_waveform_frequency)]), Integer.named('end_frequency').using(default=self.control_board.max_waveform_frequency, optional=True, validators=[
         ValueAtLeast(minimum=self.control_board.min_waveform_frequency),
         ValueAtMost(maximum=self.control_board.max_waveform_frequency)]), Integer.named('number_of_steps').using(default=10, optional=True, validators=[
         ValueAtLeast(minimum=2)]), Integer.named('RMS_voltage').using(default=min(100, self.control_board.max_waveform_voltage), optional=True, validators=[
         ValueAtLeast(minimum=10),
         ValueAtMost(maximum=self.control_board.max_waveform_voltage)]))
        box = gtk.HBox()
        ...
        label = gtk.Label('Calibration of device load feedback circuit is complete.  The impedance between actuated device area and ground should now be measured accurately by the control board.')
        label.set_line_wrap(True)
    ...
    def assistant_prepared(self, assistant, *args):
        ...
        if assistant.get_current_page() == 3:
            settings = dict([ (f, self.form_view.form.fields[f].proxy.get_widget_value()) for f in ('start_frequency',
                                                                                                    'number_of_steps',
                                                                                                    'end_frequency',
                                                                                                    'RMS_voltage')
                            ])
            ...
            frequencies = np.floor(np.logspace(np.log10(start_frequency), np.log10(end_frequency), number_of_steps))
            self.measurements_label.set_label('Reading measurements (this might take a few minutes)...')
            gtk.idle_add(self.read_measurements, settings['RMS_voltage'], frequencies)
            ...
            apply_calibration(fitted_readings, self.fitted_params, self.calibration)
    ...
    def read_measurements(self, rms_voltage, frequencies):
            ...
            try:
                try:
                    ...
                    def on_update(frequency, C1, channel, i, group_count, data):
                        ...
                        self.measurements_label.set_label('Frequency=%.2fkHz, C=%.2fpF' % (
                         frequency / 1000.0, C1 * 1000000000000.0))
                    ...
                    self.impedance_readings = run_experiment(self.control_board, rms_voltage, frequencies=frequencies, on_update=on_update)
                except:
                    raise
        ...
        gtk.gdk.threads_enter()
        self.measurements_label.set_label('Done.\n\nClick "Forward" to fit the data (this can take a few minutes).')
    ...
    def fit_feedback_params(self):
        self.fitted_params = fit_fb_calibration(self.impedance_readings, self.calibration)
    ...
    def to_hdf(self, output_path, complib='zlib', complevel=6):
        ...
        df['aref'] = str(proxy.__aref__)
        for address, desc in sorted(proxy._i2c_devices.items()):
            df['i2c address %d' % address] = desc

        pd.Series(df).to_hdf(str(output_path), '/feedback/impedance/control_board_info', format='t', complib=complib, complevel=complevel)
        self.impedance_readings.to_hdf(str(output_path), '/feedback/impedance/measurements', format='t', data_columns=self.impedance_readings.columns, complib=complib, complevel=complevel)
        pd.Series([self.calibration]).to_hdf(str(output_path), '/feedback/impedance/calibration')
        self.fitted_params.to_hdf(str(output_path), '/feedback/impedance/fitted_params', format='t', data_columns=self.fitted_params.columns, complib=complib, complevel=complevel)


if __name__ == '__main__':
    if False:
        impedance_readings = pd.read_hdf('/tmp/dropbot-impedance-calibration3NXvK0/2014-12-05T11h21m44-calibration.h5', '/feedback/impedance/measurements')
        calibration = pd.read_hdf('../../../dropbot-calibration/dropbot_calibration-002-2014-11-18.h5', '/impedance/calibration')[0]
        view = AssistantView(None, impedance_readings=impedance_readings, calibration=calibration)
    else:
