...
# [GCC 8.4.0]
# Embedded file name: C:\Users\ryan\Documents\dev\python\dmf-control-board-firmware\dmf_control_board_firmware\calibrate\feedback.py
...
import pandas as pd, sympy as sp

def limit_default(equation, symbol_names, default, **kwargs):
    return swap_default('limit_default', equation, symbol_names, default, **kwargs)
...
def _limit(eq, *args, **kwargs):
    if isinstance(eq, sp.Eq):
        return sp.Eq(sp.limit(eq.lhs, *args, **kwargs), sp.limit(eq.rhs, *args, **kwargs))
    else:
...
def rc_transfer_function(eq, Zs=None):
    ...
    result = eq
    cre_Z = re.compile('Z(?P<suffix>.*)')
    sub_params = [ (z, sp.symbols('R%s C%s omega' % (s, s))) for z, s in [ (Z, cre_Z.match(Z).group('suffix')) for Z in Zs
                                                                         ]
                 ]
...
def compute_from_transfer_function(hardware_major_version, solve_for, **kwargs):
    """
    Conventions:
    ...
    **kwargs
    ...
    if not Zs:
        Zs = None
    result = get_transfer_function(hardware_major_version, solve_for=solve_for, Zs=Zs)
    Rs = set([ s.name for s in result.atoms(sp.Symbol) if s.name.startswith('R')
             ])
    Cs = set([ s.name for s in result.atoms(sp.Symbol) if s.name.startswith('C')
             ])
