...
# [GCC 8.4.0]
# Embedded file name: c:\users\ryan\documents\dev\python\dmf-control-board-firmware\dmf_control_board_firmware\gui\reference.py
# Compiled at: 2016-11-30 18:21:49
import os
from datetime import datetime
import tempfile, pkg_resources
from path_helpers import path
import numpy as np, pandas as pd, gtk
...
from dmf_control_board_firmware.calibrate.hv_attenuator import resistor_max_actuation_readings, fit_feedback_params, update_control_board_calibration, plot_feedback_params
from dmf_control_board_firmware.calibrate.oscope import VISA_AVAILABLE, AgilentOscope, read_oscope as read_oscope_
...
class AssistantView(WindowView):
    ...
    def create_ui(self):
        ...
        content = 'This wizard will guide you through the process of calibrating the high-voltage reference load feedback measurement circuit.  This feedback circuit is used to measure the output voltage of the amplifier on the controlboard.\n\nSee <a href="http://microfluidics.utoronto.ca/trac/dropbot/wiki/Control board calibration#high-voltage-attenuation-calibration">here</a> for more details.'
        ...
        form = Form.of(Integer.named('start_frequency').using(default=minimum, optional=True, validators=[
         ValueAtLeast(minimum=minimum)]), Integer.named('end_frequency').using(default=maximum, optional=True, validators=[
         ValueAtLeast(minimum=minimum)]), Integer.named('number_of_steps').using(default=10, optional=True, validators=[
        ...
        label = gtk.Label('Calibration of reference load feedback circuit is complete.  The high-voltage output from amplifier should now be measured accurately by the control board.')
        label.set_line_wrap(True)
    ...
    def assistant_prepared(self, assistant, *args):
        ...
        if assistant.get_current_page() == 3:
            ...
            frequencies = np.logspace(np.log10(start_frequency), np.log10(end_frequency), number_of_steps)
            ...
            hw_version = Version.fromstring(self.control_board.hardware_version()).major
            plot_feedback_params(hw_version, self.hv_readings, self.fitted_params, axis=self.axis)
        ...
        try:
            try:
                if VISA_AVAILABLE:
                    ...
                    self._read_oscope = read_oscope_
                self.hv_readings = resistor_max_actuation_readings(self.control_board, frequencies, self.read_oscope)
                gtk.gdk.threads_enter()
    ...
    def read_oscope(self):
        ...
        self.measurements_label.set_label('Measuring amplifier voltage readings...')
        self.measure_progress.set_fraction(float(self.measurement_i) / self.measurement_count)
    ...
    def apply_button_clicked(self, assistant):
        update_control_board_calibration(self.control_board, self.fitted_params)
    ...
    def cancel_button_clicked(self, assistant):
        ...
        self.calibration_file = output_dir.joinpath('%s-calibration.h5' % timestamp)

    def fit_feedback_params(self):
        self.fitted_params = fit_feedback_params(self.control_board.calibration, self.hv_readings)
    ...
    def to_hdf(self, output_path, complib='zlib', complevel=6):
        ...
        pd.Series(df).to_hdf(str(output_path), '/feedback/reference/control_board_info', format='t', complib=complib, complevel=complevel)
        self.hv_readings.to_hdf(str(output_path), '/feedback/reference/measurements', format='t', data_columns=self.hv_readings.columns, complib=complib, complevel=complevel)
        self.fitted_params.to_hdf(str(output_path), '/feedback/reference/fitted_params', format='t', data_columns=self.fitted_params.columns, complib=complib, complevel=complevel)
