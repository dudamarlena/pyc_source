...
# [GCC 8.4.0]
# Embedded file name: c:\users\ryan\documents\dev\python\dmf-control-board-firmware\dmf_control_board_firmware\gui\channels.py
...
import gtk, matplotlib.pyplot as plt, numpy as np, pandas as pd

def sweep_channels(proxy, test_loads, voltage=10, frequency=None, on_update=None):
    if frequency is None:
        frequency = 0.5 * (proxy.max_waveform_frequency + proxy.min_waveform_frequency)

    def default_on_update(df):
        item = df.iloc[0]
        print (',').join((k, item[k]) for k in ('channel', 'expected capacitance'))
    ...
    def measure_load(channel, expected_load):
        ...
        states[channel] = 1
        results = proxy.measure_impedance(5, n_samples, 0, True, True, states)
        df = pd.DataFrame(results.capacitance(), columns=[
         'measured capacitance'])
        df['measured impedance'] = results.Z_device()
        df['actuation voltage'] = results.V_actuation()
        df['expected capacitance'] = expected_load
        df['channel'] = channel
        df['RMS error'] = np.abs((df['measured capacitance'] - df['expected capacitance']) / df['expected capacitance'])
        df['voltage'] = voltage
...
def plot_channel_sweep(proxy, start_channel):
    ...
    normalized_measurements = results['measured capacitance'] / results['expected capacitance']
    fig, axis = plt.subplots(figsize=(10, 8))
    axis.bar(normalized_measurements.index - 0.3, normalized_measurements, width=0.6, edgecolor='none', facecolor='limegreen')
    axis.set_xlim(left=test_loads.index.min() - 0.5, right=test_loads.index.max() + 0.5)
    axis.set_xlabel('channel')
    axis.set_ylabel('$\\frac{C_{\\tt{measured}}}{C_{\\tt{expected}}}$', fontsize=28)
    return results
...
class AssistantView(WindowView):
    ...
    def create_ui(self):
        ...
        content = 'This wizard will guide you through the process of testing the channels of a single switching board.'
        label = gtk.Label(content)
        label.set_use_markup(True)
        label.set_line_wrap(True)
        image = gtk.Image()
        img_path = pkg_resources.resource_filename('dmf_control_board_firmware', 'gui/channels_intro.png')
        ...
        box.pack_start(image, True, True, padding=5)
        self.widget.set_page_complete(box, True)
        self.radio_buttons = [ gtk.RadioButton(label='Channels %d-%d' % (c, c + self.channels_per_board - 1)) for c in xrange(0, self.channel_count, self.channels_per_board)
                             ]
    ...
    def selected_channels(self):
        return [ r for r in self.radio_buttons[0].get_group() if r.get_active()
               ][0].get_label()
    ...
    def assistant_prepared(self, assistant, *args):
        ...
        if assistant.get_current_page() == 3:
            ...
            start_channel = int(re.match('Channels (?P<start_channel>\\d+)-\\d+', channels_str).group('start_channel'))
            ...
            normalized_measurements = self.readings['measured capacitance'] / self.readings['expected capacitance']
            self.figure.clf()
            axis = self.figure.add_subplot(111)
            axis.bar(normalized_measurements.index - 0.3, normalized_measurements, width=0.6, edgecolor='none', facecolor='limegreen')
            axis.set_xlim(left=self.test_loads.index.min() - 0.5, right=self.test_loads.index.max() + 0.5)
            axis.set_xlabel('channel')
            axis.set_ylabel('$\\frac{C_{\\tt{measured}}}{C_{\\tt{expected}}}$', fontsize=28)
    ...
    def read_measurements(self, start_channel):
        try:
            ...
            def on_update(df):
                ...
                self.measurements_label.set_label('channel=%d' % df.iloc[0]['channel'])
                i = int(df.iloc[0]['channel'] % self.channels_per_board)
                self.measure_progress.set_fraction(float(i + 1) / self.channels_per_board)
            ...
            self.readings = sweep_channels(self.control_board, self.test_loads, on_update=on_update)
        finally:
    ...
    def to_hdf(self, output_path, complib='zlib', complevel=6):
        ...
        def write_results(output_path, df, append=True):
            df.to_hdf(str(output_path), '/channel_sweeps', format='t', append=append, data_columns=self.readings.columns, complib=complib, complevel=complevel)
