...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/menorah/menorah.py
# Compiled at: 2015-10-26 17:10:49
import os, errno, csv, json, confluencefactory
...
class Menorah(object):
    ...
    def _createConfluence(self):
        if self._confluence is not None:
            ...
            self._confluence = confluencefactory.create(self._streamIds, since=self._since, limit=self._limit, debug=self._debug)
    ...
    """
        ...
        swarmDescriptionPath = os.path.join(workingDirPath, 'swarm_description.json')
        self.writeSwarmDescription(csvPath, swarmDescriptionPath, predictedField=predictedField, swarmParams=swarmParams)
    ...
    :return:
    """
        self.prepareSwarm(predictedField=predictedField, swarmParams=swarmParams)
        self.runSwarm(self._workingDir)

    def runModel(self, plot=False):
        self.resetConfluence()
        handler, whenDone = modelrunner.getRowHandlers(self._workingDir, self._predictedField, modelParams=self._modelParams, plot=plot)
