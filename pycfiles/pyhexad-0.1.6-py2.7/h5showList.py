...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\pyhexad\h5showList.py
# Compiled at: 2015-03-06 08:22:54
from functools import partial
import logging, posixpath, h5py, numpy as np
from pyxll import xl_func
from config import Limits
from file_helpers import file_exists
from h5_helpers import path_is_valid_wrt_loc
import renderer
from type_helpers import is_supported_h5array_type, is_supported_h5table_type, dtype_to_hexad
...
def render_row(grp, name):
    ...
    if obj_type == h5py.Group:
        return {'OBJ_TYPE': 'GROUP', 
           'NAME': path, 
           '#ATTR': len(obj.attrs.keys()), 
           '#LNK': len(obj.keys())}
    else:
        if obj_type == h5py.Dataset:
            flavor = 'DATASET'
            dty = obj.dtype
            if is_supported_h5array_type(dty):
                ...
                flavor = 'TABLE'
            return {'OBJ_TYPE': flavor, 
               'NAME': name.split('/')[(-1)], 
               '#ATTR': len(obj.attrs.keys()), 
               'DTYPE': dtype_to_hexad(obj.dtype), 
               'RANK': len(obj.shape), 
               'DSPACE': tuple_to_excel(obj.shape)}
        if obj_type == h5py.Datatype:
            return {'OBJ_TYPE': obj_type, 
               'NAME': name.split('/')[(-1)], 
               '#ATTR': len(obj.attrs.keys()), 
               'DTYPE': dtype_to_hexad(obj.dtype)}
        return {'OBJ_TYPE': str(obj_type), 
           'NAME': name.split('/')[(-1)]}
...
def h5showList(filename, location):
    ...
    with h5py.File(filename, 'r') as (f):
        ...
        if len(lines) >= Limits.EXCEL_MAX_ROWS:
            return 'The number objects in the file exceeds the maximum numberof rows of an Excel worksheet.'
