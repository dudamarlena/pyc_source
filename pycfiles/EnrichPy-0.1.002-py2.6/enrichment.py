...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/enrichpy/enrichment.py
# Compiled at: 2010-10-26 10:11:35
"""Classes and routines to calculate enrichment and ionization histories.
"""
import copy, numpy, scipy, scipy.constants.codata as scc, cosmolopy.reionization as cr, cosmolopy.utils as utils
...
def get_omega_CIV_data(filename):
    data = numpy.loadtxt(filename, skiprows=1, delimiter=',', usecols=range(5))
    return data
...
class LFIonHistory(Saveable):
    ...
    def __init__(self, z, lf_params, cosmo, lf_args={}, lf_maglim=None, MStar_z_slope=None, alpha=None, f_esc_gamma=None, clump_fact=None, clump_fact_func=None):
        """
        ...
        if MStar_z_slope is not None:
            ...
            if 'extrap_args' not in lf_args:
                lf_args['extrap_args'] = {}
            lf_args['extrap_args']['slopes'] = [
             0, MStar_z_slope]
    ...
    def make_ion_func(self):
        ...
        ion_func = utils.Extrapolate1d(self.z, self.xEsc, bounds_behavior=[
         0.0,
         'extrapolate'])
        return ion_func
    ...
    def integrate_ion_recomb(self):
        ...
        (x, w, t) = cr.integrate_ion_recomb(self.z, ion_func=ion_func, clump_fact_func=self.clump_fact_func, **self.cosmo)
        self.xr = x
        self.w = w
        self.t = t
        return (x, w, t)
    ...
    def optical_depth(self, use_recomb=True, store=True):
        ...
        xPhys[xPhys > 1.0] = 1.0
        tau_later = tau_z0 + cr.integrate_optical_depth(xPhys[..., ::-1], xPhys[..., ::-1], self.z[::-1], **self.cosmo)
        tau = tau_later[..., ::-1]
        if store:
            if use_recomb:
                self.tau = tau
...
class EnrichmentHistory(Saveable):
    ...
    def __init__(self, ionHist, f_x_Z, z_change=None, f_change=None, delay_kernel=None):
    ...
    def calculate_Omega_Z(self):
        if self.z_change is not None:
            ...
            xfunc = scipy.interpolate.interp1d(self.ionHist.z[::-1], self.ionHist.xTot[::-1])
            x_change = xfunc(self.z_change)
            xTot = self.ionHist.xTot
            f_early = self.f_x_Z
            f_late = self.f_x_Z * self.f_change
            self.Omega_Z = (self.z > self.z_change) * f_early * xTot + (self.z <= self.z_change) * (f_late * xTot + (f_early - f_late) * x_change)
        else:
    ...
    def convolve(self):
        ...
        """
        self.Omega_Z_convolved = padconvolve.padded_convolve(self.Omega_Z, self.delay_kernel, origin=0, value=0.0)
        return self.Omega_Z_convolved

    def binned_Omega_Z(self, zmin, zmax, useConvolved=True):
        if useConvolved:
            o = self.Omega_Z_convolved
        elif hasattr(self, 'Omega_Z_unconvolved'):
            o = self.Omega_Z_unconvolved
        else:
            o = self.Omega_Z
        mask = numpy.logical_and(self.z >= zmin, self.z < zmax)
        return numpy.mean(o[mask])

    def fit_f_change(self, z_bin, Omega_Z_obs, min_f_change=0.0, max_f_change=None, useConvolved=True):
        ...
        def fitfunc(fc):
            ...
            if useConvolved:
                self.convolve()
            return self.binned_Omega_Z(z_bin[0], z_bin[1], useConvolved) - Omega_Z_obs

        if fitfunc(min_f_change) > Omega_Z_obs:
            print "Warning: can't match Omega_Z with f_change > %.3g" % min_f_change
            self.f_change = min_f_change
            return min_f_change
        else:
            if max_f_change is None:
                max_f_change = 3.0 * self.original_f_change
                ff = fitfunc(max_f_change)
                while ff < 0:
                    print ' max_f_change = %.3g too small (diff = %.3g)...' % (
                     max_f_change, ff)
                    max_f_change = 3.0 * max_f_change
                    ff = fitfunc(max_f_change)

            print ' considering f_change between %.3g and %.3g' % (min_f_change,
             max_f_change)
            f_change_fit = scipy.optimize.minpack.bisection(fitfunc, min_f_change, max_f_change, xtol=self.original_f_change * 0.001)
            if useConvolved:
    ...
    def fit_f_x_Z(self, z_bin, Omega_Z_obs, min_f_x_Z=0.0, max_f_x_Z=None, useConvolved=True):
        ...
        if fitfunc(min_f_x_Z) > Omega_Z_obs:
            print "Warning: can't match Omega_Z with f_x_Z > %.3g" % min_f_x_Z
            self.f_x_Z = min_f_x_Z
            return min_f_x_Z
        else:
            if max_f_x_Z is None:
                max_f_x_Z = 3.0 * self.original_f_x_Z
                ff = fitfunc(max_f_x_Z)
                while ff < 0:
                    print ' max_f_x_Z = %.3g too small (diff = %.3g)...' % (
                     max_f_x_Z, ff)
                    max_f_x_Z = 3.0 * max_f_x_Z
                    ff = fitfunc(max_f_x_Z)

            print ' considering f_x_Z between %.3g and %.3g' % (min_f_x_Z,
             max_f_x_Z)
            f_x_Z_fit = scipy.optimize.minpack.bisection(fitfunc, min_f_x_Z, max_f_x_Z, xtol=self.original_f_x_Z * 0.001)
...
class EnrichmentHistoryCollection(Saveable):
    ...
    def __init__(self, ionHists, delay_kernels, f_x_Zs, z_changes, f_changes, lf_names=None, dl_names=None, en_names=None):
        ...
        for iLF in range(self.nLF):
            ...
            for iDl in range(self.nDl):
                ...
                for iEn in range(self.nEn):
                    ...
                    line.append(EnrichmentHistory(ionHists[iLF], f_x_Zs[iEn], z_changes[iEn], f_changes[iEn], delay_kernels[iDl]))
    ...
    def fit_f_change(self, z_bin, Omega_Z_obs, min_f_change=0.0, max_f_change=None):
        f_changes = []
        ...
        for iLF in range(self.nLF):
            ...
            for iDl in range(self.nDl):
                ...
                for iEn in range(self.nEn):
                    if iDl == 0:
                        line_unc.append(self.EnHists[iLF][iDl][iEn].fit_f_change(z_bin, Omega_Z_obs, min_f_change, max_f_change, useConvolved=False))
                    line.append(self.EnHists[iLF][iDl][iEn].fit_f_change(z_bin, Omega_Z_obs, min_f_change, max_f_change, useConvolved=True))
    ...
    def fit_f_x_Z(self, z_bin, Omega_Z_obs, min_f_x_Z=0.0, max_f_x_Z=None, useConvolved=True):
        ...
        for iLF in range(self.nLF):
            ...
            for iDl in range(self.nDl):
                line = []
                for iEn in range(self.nEn):
                    line.append(self.EnHists[iLF][iDl][iEn].fit_f_x_Z(z_bin, Omega_Z_obs, min_f_x_Z, max_f_x_Z, useConvolved))
    ...
    def convolve(self):
        ...
        for iLF in range(self.nLF):
            ...
            for iDl in range(self.nDl):
                line = []
                for iEn in range(self.nEn):
                    line.append(self.EnHists[iLF][iDl][iEn].convolve())
