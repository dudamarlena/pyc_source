...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/enrichpy/lifetime.py
# Compiled at: 2010-10-26 10:11:35
"""Stellar lifetimes as a function of stellar mass, and related functions.
"""
import math, numpy, scipy.interpolate
...
def mass_from_main_sequence_life(life):
    ...
    mass in solar masses.
    """
    return (life / 10000000000.0) ** (-1.0 / 2.5)
...
def invert_function_log(function, logxmin, logxmax, logxstep, bounds_error=False):
    """Invert function y = F(x) using interpolation.
    ...
    """
    logxi = numpy.arange(logxmin, logxmax + logxstep, logxstep)
    logyi = numpy.log10(function(10.0 ** logxi))
    logifunc = scipy.interpolate.interp1d(logyi, logxi, bounds_error=bounds_error)
    ifunc = lambda y: 10.0 ** logifunc(numpy.log10(y))
    return ifunc


def invert_function_semilogy(function, xmin, xmax, xstep, bounds_error=False):
    """Invert function y = F(x) using interpolation.
    ...
    ifunclog = scipy.interpolate.interp1d(logyi, xi, bounds_error=bounds_error)
    ifunc = lambda y: ifunclog(numpy.log10(y))
    return ifunc


def mass_from_main_sequence_life_function(function, mmin=0.0, mmax=1000.0, mstep=0.01):
    ...
    """
    return invert_function_semilogy(function, mmax, mmin, -1.0 * mstep)


def main_sequence_life_MM89(mass):
    r"""Lifetime in years from Maeder & Meynet (1989) via Romano et al. (2005). 
    ...
    constant = numpy.array([[1.0, 1.35, 0.77, 0.17, -0.94]]).transpose()
    mlimit = numpy.array([[0.0, 1.3, 3.0, 7.0, 15.0, 60.0]]).transpose()
    t_star = numpy.sum(10 ** (logmfactor * logm + constant) * ((mass > mlimit[0:-1]) * (mass <= mlimit[1:])), axis=0)
    t_star[mass > 60.0] = 1.2 * mass[(mass > 60.0)] ** (-1.85) + 0.003
    return 1000000000.0 * t_star


main_sequence_life_MM89.minlife = 3000000.0
...
def main_sequence_life_K97(mass):
    ...
    t[imask] = 10.0 ** ((0.334 - numpy.sqrt(1.79 - 0.2232 * (7.764 - numpy.log10(mass[imask])))) / 0.1116)
    t[mass > 6.6] = 1.2 * mass[(mass > 6.6)] ** (-1.85) + 0.003
    t[t > 50.0] = 50.0
    return 1000000000.0 * t


main_sequence_life_K97.minlife = 3000000.0
main_sequence_life_K97.maxlife = 50000000000.0
...
def test_conversion(lifetime_function):
    ...
    mass = numpy.arange(0.0, 200.0, 0.1)
    time_from_mass = lifetime_function(mass)
    mass_from_time_func = mass_from_main_sequence_life_function(lifetime_function)
    ...
    if False:
        threshold = 1e-09
        ntest.assert_array_less(numpy.abs(frac_diff[goodmask]), threshold * numpy.ones(len(frac_diff[goodmask])), err_msg='Error in mass recovery exceeds %s' % threshold)
