...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/enrichpy/delay.py
# Compiled at: 2010-10-26 10:11:35
"""Class Delay represents the delay distribution for metals emerging from stars.

"""
import math, sys, cPickle, os, scipy, scipy.integrate, scipy.optimize, numpy, cosmolopy.utils as utils, yields, initialmassfunction, lifetime
...
class Delay():
    ...
    def __init__(self, metallicity, imfFunc=initialmassfunction.imf_number_Chabrier, yieldClass=yields.Data_GBM, ejections=False, lifetimeFunc=lifetime.main_sequence_life_K97, lowMass=None, highMass=None, minTime=None, maxTime=None):
        ...
        self._m_star_func = lifetime.mass_from_main_sequence_life_function(self.lifetimeFunc)
        self._yields = yieldClass()
        self._yieldFunc_2d = self._yields.interpolate_function(ejections=ejections)
        self._fCnorm = 1.0
        ...
        if lowMass is None and highMass is None:
            ...
            marray = numpy.linspace(self.lowMass, self.highMass, 1000.0)
    ...
    def mass_limit_error(self, m_max=None, imf_index=-2.3):
        ...
        if m_max is None:
            m_max = self.highMass
        (exfuncHigh, exfuncLow, highSlope, highIntercept, lowSlope, lowIntercept) = yields.extrapolate_function(self._yieldFunc_2d, self.metallicity, return_coeffs=True)
        phi0 = self.imf(m_max) * m_max ** 2.3
        phi0 *= self._fCnorm
        plustwo = imf_index + 2.0
        plusone = imf_index + 1.0
        error = highSlope * phi0 * m_max ** plustwo / (-1.0 * plustwo) + highIntercept * phi0 * m_max ** plusone / (-1.0 * plusone)
        return error
    ...
    def metal_mass_dist_postinterp(self, m):
        ...
        if not hasattr(self, '_fCfunc_2d'):
            self._fCfunc_2d = self.yieldClass(interp_args={'weight_function': self.imf}).interpolate_function(ejections=self.ejections)
    ...
    def metal_mass_dist(self, m):
        ...
        """
        dist = self.imf(m) * self._yieldFunc_2d(m, self.metallicity).flatten() / self._fCnorm
        return dist
    ...
    def metal_mass_CCDF(self, m, highMass=None, method='romberg', **kwargs):
        ...
        ccdf = utils.ccumulate(self.metal_mass_dist, m, max=highMass, method=method, **kwargs)
    ...
    def metal_mass_dist_binned(self, m, ccdf=None, **kwargs):
        ...
        """
        if ccdf is None:
            cdf = 1.0 - numpy.nan_to_num(self.metal_mass_CCDF(m, **kwargs))
        else:
            cdf = 1.0 - numpy.nan_to_num(ccdf)
    ...
    def stats(self, meantol=0.001):
        """Calculate median and mean delay."""
        npieces = 1.0 / meantol
        tarray = numpy.logspace(numpy.log10(self.minTime), numpy.log10(self.maxTime), npieces)
    ...
    def printstats(self):
        ...
        print 'Median and mean delays are %.4g and %.4g Gyr' % (median / 1000000000.0,
         mean / 1000000000.0)
        ...
        print '   Age from mean mass is %.3g Gyr' % (self.t_life(meanMass) / 1000000000.0)


class MockDelay(Delay):
    _fCnorm = numpy.nan

    def __init__(self, binmasses, fractions, lifetimeFunc=lifetime.main_sequence_life_K97, maxTime=None, **args):
        ...
        assert len(fractions) == len(binmasses) - 1, 'len(fractions) must be one less than len(splitmasses).'
        assert numpy.all(numpy.sort(binmasses) == binmasses), 'binmasses must be ordered from low to high.'
    ...
    def metal_mass_CCDF(self, mass):
        ...
        for ibin in range(len(self.fractions) - 1, -1, -1):
            ...
            binmask = numpy.logical_and(mass >= self.binmasses[ibin], mass < self.binmasses[(ibin + 1)])
            binwidth = self.binmasses[(ibin + 1)] - self.binmasses[ibin]
            ccdf[binmask] += self.fractions[ibin] * (self.binmasses[(ibin + 1)] - mass[binmask]) / binwidth
...
def test_plot_binned_delay_function():
    ddist = Delay(0.02)
    t = numpy.arange(0.0, 50000000000.0, 1330000.0)
...
def test_plot_delay_function(dlogt=0.001):
    Z = 0.02
    ...
    m_star_func = lifetime.mass_from_main_sequence_life_function(lifetimeFunc, logmmin=10.0)
    m = m_star_func(t[:-1])
...
def test_plot_converge_delay_function():
    test_plot_delay_function(0.0001)
    test_plot_delay_function(0.001)
    test_plot_delay_function(0.01)


def plot_delay_function(metallicity, imfFunc=initialmassfunction.imf_number_Chabrier, yieldClass=yields.Data_GBM, lifetimeFunc=lifetime.main_sequence_life_K97):
    """ Plot delay function, yield, IMF, and lifetime.
    
    """
    dm = 0.01
    mass = numpy.arange(0, 110.0, dm)
    time = 10 ** numpy.arange(6.0, 10.5, 0.01)
    dt = time[1:] - time[:-1]
    time = time[1:]
    m_star_func = lifetime.mass_from_main_sequence_life_function(lifetimeFunc)
    ...
    ddist = Delay(0.02, imfFunc=imfFunc, yieldClass=yieldClass, lifetimeFunc=lifetimeFunc)
    ...
    pylab.xlabel('mass (Msun)')
    pylab.ylabel('$m_C$ unweighted (--)')
    pylab.gca().set_yscale('log')
    pylab.twinx()
    pylab.plot(mass, mC_weighted, label='IMF weighted', ls=':')
    pylab.ylabel('$m_C$ weighted (:)')
    pylab.gca().set_yscale('log')
    pylab.subplot(223)
    ...
    pylab.gca().set_yscale('log')
    pylab.twinx()
    pylab.plot(mass[1:], -1.0 * dmdt, label='-dmdt', ls=':')
    pylab.gca().set_yscale('log')
    pylab.ylabel('dmdt(:)')
    pylab.subplot(224)
    ...
    pylab.gca().set_xscale('log')
    pylab.twinx()
    pylab.plot(tLife[1:], -1.0 * tLife[1:] * imf[1:] * dmdt, ls=':')
    ...
    pylab.gca().set_xscale('log')
    pylab.twinx()
    pylab.plot(tLife[1:], -1.0 * dmdt, label='-dmdt', ls=':')
    pylab.gca().set_yscale('log')
    pylab.ylabel('-dmdt(:)')
    pylab.subplot(224)
...
def test_Delay(plots=True, skip_asserts=False, delayfunc=None):
    ...
    npoints = 1000
    (mass, dmass) = numpy.linspace(delayfunc.lowMass, delayfunc.highMass, npoints, retstep=True)
    ...
    (metal_time_dist_unbinned, dtime_ubin) = delayfunc.metal_time_dist_unbinned(time)
    imf = delayfunc.imf(mass)
    yield_mass = delayfunc.yield_mass(mass)
    import scipy.integrate
    test_mass_CCDF = scipy.integrate.cumtrapz((dmass * metal_mass_dist)[::-1])[::-1]
    diff_mass_CCDF = test_mass_CCDF - metal_mass_CCDF[:-1]
    sum_metal_time_dist = numpy.cumsum(numpy.nan_to_num(metal_time_dist_binned))
    ...
    if not skip_asserts:
        ...
        ntest.assert_almost_equal(metal_time_CDF_time_from_mass, metal_mass_CCDF, 4)
        ...
        ntest.assert_approx_equal(ccdf[0], 1.0, 4)
        ntest.assert_approx_equal(cdf[0], 1.0, 4)
        ntest.assert_approx_equal(sum_metal_time_dist[(-1)], 1.0, 4)
        ntest.assert_almost_equal(numpy.cumsum(numpy.nan_to_num(metal_time_dist_binned)), numpy.nan_to_num(metal_time_CDF))
        ntest.assert_almost_equal(numpy.nan_to_num(metal_time_CDF), test_time_CDF, 10)
        ntest.assert_almost_equal(metal_mass_dist, test_metal_mass_dist, 10)
        ntest.assert_almost_equal(metal_mass_dist, metal_mass_dist_postinterp, 2)
    if plots:
        ...
        pylab.legend(loc='best')
        pylab.subplot(212)
        pylab.plot(mass, metal_mass_dist - test_metal_mass_dist, ':', label='PDF - c*(IMF * yield)')
        pylab.plot(mass, metal_mass_dist - metal_mass_dist_postinterp, '-', label='PDF - postinterp')
        pylab.axvline(x=delayfunc.lowMass)
        pylab.axvline(x=delayfunc.highMass)
        pylab.legend(loc='best')
        ...
        pylab.axvline(x=maxTime)
        pylab.subplot(224)
        pylab.plot(mass_from_time, sum_metal_time_dist - metal_time_CDF, label='sum-CDF')
        pylab.plot(mass_from_time, test_time_CDF - metal_time_CDF, label='int-CDF')
        pylab.legend(loc='best')
        pylab.axvline(x=delayfunc.lowMass)
        pylab.axvline(x=delayfunc.highMass)
        ...
        pylab.legend(loc='best')
        pylab.subplot(212)
        pylab.plot(mass, metal_time_CDF_time_from_mass - metal_mass_CCDF, ':', label='CDF - CCDF')
        pylab.legend(loc='best')
...
def compare_dist_times():
    ...
    timelist.append(time.time())
    m = numpy.linspace(delayfunc.lowMass, delayfunc.highMass, 1000.0)
    ...
    timelist.append(time.time())
    utils.ccumulate(delayfunc.metal_mass_dist, m, max=delayfunc.highMass, method='romberg')
    names.append('ccumulate postintepr')
    timelist.append(time.time())
    utils.ccumulate(delayfunc.metal_mass_dist_postinterp, m, max=delayfunc.highMass)
    names.append('ccumulate postintepr romberg')
    timelist.append(time.time())
    utils.ccumulate(delayfunc.metal_mass_dist_postinterp, m, max=delayfunc.highMass, method='romberg')
    timelist.append(time.time())
    timearray = numpy.asarray(timelist)
    time_diffs = 1000.0 * (timearray[1:] - timearray[:-1])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        filename = None
    test_Delay(skip_asserts=False, delayfunc=Delay(0.02, maxTime=2000000000.0, ejections=False))
    test_Delay(skip_asserts=False, delayfunc=Delay(0.02, maxTime=2000000000.0, ejections=True))
    if filename is None:
        pylab.show()
