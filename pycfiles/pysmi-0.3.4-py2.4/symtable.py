...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysmi/codegen/symtable.py
...
class SymtableCodeGen(AbstractCodeGen):
    ...
    constImports = {'SNMPv2-SMI': ('iso', 'Bits', 'Integer32', 'TimeTicks', 'Counter32', 'Counter64', 'NOTIFICATION-TYPE', 'Gauge32', 'MODULE-IDENTITY', 'OBJECT-TYPE', 'OBJECT-IDENTITY', 'Unsigned32', 'IpAddress', 'MibIdentifier'), 'SNMPv2-TC': ('DisplayString', 'TEXTUAL-CONVENTION'), 'SNMPv2-CONF': ('MODULE-COMPLIANCE', 'NOTIFICATION-GROUP')}
    baseTypes = [
    ...
    def allParentsExists(self, parents):
        ...
        for parent in parents:
            if not (parent in self._out or parent in self._importMap or parent in self.baseTypes or parent in ('MibTable',
                                                                                                               'MibTableRow',
                                                                                                               'MibTableColumn') or parent in self._rows):
                parentsExists = False
                break
    ...
    def regSym(self, symbol, symProps, parents=()):
        if symbol in self._out or symbol in self._postponedSyms:
    ...
    def genObjectType(self, data, classmode=False):
        ...
        pysmiName = self.transOpers(origName)
        symProps = {'type': 'ObjectType', 'oid': oid, 'syntax': syntax, 'origName': origName}
        parents = [
         syntax[0][0]]
        if augmention:
        ...
        if index and index[1]:
            ...
            for (fakeIdx, fakeSyntax) in zip(fakeIndexes, fakeSymSyntax):
                ...
                fakeSymProps = {'type': 'fakeColumn', 'oid': oid + (fakeIdx,), 'syntax': fakeSyntax, 'origName': fakeName}
    ...
    def genTypeDeclaration(self, data, classmode=False):
        ...
        if declaration:
            ...
            if parentType:
                symProps = {'type': 'TypeDeclaration', 'syntax': declaration, 'origName': origName}
    ...
    def genDefVal(self, data, classmode=False):
        defval = data[0]
        ...
        elif defval in self._out or defval in self._importMap:
            val = defval + '.getName()'
        else:
            val = dorepr(defval)
        return val
    ...
    def genIndex(self, data, classmode=False):
        ...
        return (
         fakeIdxName, fakeIndexes, fakeSymsSyntax)
    ...
    def genCode(self, ast, symbolTable, **kwargs):
        ...
        return (
         MibInfo(oid=None, name=self.moduleName[0], revision=self._moduleRevision, imported=tuple([ x for x in importedModules ])), self._out)