...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pysmi/codegen/jsondoc.py
...
class JsonCodeGen(AbstractCodeGen):
    ...
    """
    __module__ = __name__
    constImports = {'SNMPv2-SMI': ('iso', 'NOTIFICATION-TYPE', 'MODULE-IDENTITY', 'OBJECT-TYPE', 'OBJECT-IDENTITY'), 'SNMPv2-TC': ('DisplayString', 'TEXTUAL-CONVENTION'), 'SNMPv2-CONF': ('MODULE-COMPLIANCE', 'NOTIFICATION-GROUP')}
    fakeMibs = (
     'ASN1', 'ASN1-ENUMERATION', 'ASN1-REFINEMENT') + AbstractCodeGen.baseMibs
    baseTypes = [
     'Integer', 'Integer32', 'Bits', 'ObjectIdentifier', 'OctetString']
    typeClasses = {'NetworkAddress': 'IpAddress', 'nullSpecific': 'zeroDotZero', 'ipRoutingTable': 'ipRouteTable', 'snmpEnableAuthTraps': 'snmpEnableAuthenTraps'}
    smiv1IdxTypes = [
    ...
    def genImports(self, imports):
        ...
        return (
         OrderedDict(imports=outDict), tuple(sorted(imports)))
    ...
    def genObjectType(self, data):
        ...
        outDict['oid'] = oidStr
        if syntax[0]:
            nodetype = syntax[0] == 'Bits' and 'scalar' or syntax[0]
    ...
    def genOid(self, data):
        ...
        return (
         ('.').join([ str(x) for x in self.genNumericOid(out) ]), parent)
    ...
    def genRow(self, data):
        ...
        return row in self.symbolTable[self.moduleName[0]]['_symtable_rows'] and ('row',
    ...
    def genCode(self, ast, symbolTable, **kwargs):
        ...
        debug.logger & debug.flagCodegen and debug.logger('canonical MIB name %s (%s), imported MIB(s) %s, Python code size %s bytes' % (self.moduleName[0], moduleOid, (',').join(importedModules) or '<none>', len(outDict)))
        return (
         MibInfo(oid=moduleOid, identity=self._moduleIdentityOid, name=self.moduleName[0], revision=self._moduleRevision, oids=self._oids, enterprise=self._enterpriseOid, compliance=self._complianceOids, imported=tuple([ x for x in importedModules if x not in self.fakeMibs ])), json.dumps(outDict, indent=2))

    def genIndex(self, processed, **kwargs):
        outDict = {'meta': {}, 'identity': {}, 'enterprise': {}, 'compliance': {}, 'oids': {}}
        ...
        if 'comments' in kwargs:
            outDict['meta']['comments'] = kwargs['comments']
        debug.logger & debug.flagCodegen and debug.logger('OID->MIB index built, %s entries' % len(processed))
