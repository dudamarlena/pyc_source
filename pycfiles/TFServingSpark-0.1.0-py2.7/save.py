...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/tss/save.py
# Compiled at: 2018-04-30 11:37:42
import logging, os, time, tensorflow as tf
...
def export_saved_model(inputs, outputs, output_dir, model_version, graph, sess=None, method_name='tensorflow/serving/classify'):
    ...
    with graph.as_default():
        with sess.as_default():
            ...
            signature = tf.saved_model.signature_def_utils.build_signature_def(inputs={k:tf.saved_model.utils.build_tensor_info(v) for k, v in inputs.items()}, outputs={k:tf.saved_model.utils.build_tensor_info(v) for k, v in outputs.items()}, method_name=method_name)
