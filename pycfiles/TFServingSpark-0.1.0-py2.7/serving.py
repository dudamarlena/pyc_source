...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/tss/serving.py
# Compiled at: 2018-05-01 15:21:11
...
def load_meta_graph(model_path, tags, graph, signature_def_key=None):
    ...
    try:
        signature_def = signature_def_utils.get_signature_def_by_key(the_meta_graph, signature_def_key)
        ...
        try:
            ...
            raise ValueError(('Got signature_def_key "{}". Available signatures are {}. Original error:\n{}').format(signature_def_key, list(the_meta_graph.signature_def), ex))

    input_names = {k:v.name for k, v in signature_def.inputs.items()}
    output_names = {k:v.name for k, v in signature_def.outputs.items()}
    feed_tensors = {k:graph.get_tensor_by_name(v) for k, v in input_names.items()}
    fetch_tensors = {k:graph.get_tensor_by_name(v) for k, v in output_names.items()}
