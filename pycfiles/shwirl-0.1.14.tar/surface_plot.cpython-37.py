...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/visuals/surface_plot.py
                ...
                if z is not None:
                    ...
                    if self._y is not None:
                        if z.shape[1] != len(self._y):
                            raise TypeError('Z values must have shape (len(x), len(y))')
                    self._z = z
                    if self._SurfacePlotVisual__vertices is not None:
                        if self._z.shape != self._SurfacePlotVisual__vertices.shape[:2]:
                            self._SurfacePlotVisual__vertices = None
            if self._z is None:
                return
            update_mesh = False
