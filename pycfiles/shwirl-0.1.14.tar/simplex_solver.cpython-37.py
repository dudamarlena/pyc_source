...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/ext/_bundled/cassowary/simplex_solver.py
...
class SimplexSolver(Tableau):
    ...
    def remove_constraint(self, cn):
        ...
        col = self.rows.get(marker) or self.columns[marker]
        exit_var = None
        min_ratio = 0.0
        for v in col:
            if v.is_restricted:
            ...
            if cn.is_stay_constraint:
                if e_vars:
                    ...
                    while self.stay_error_vars:
                        ...
                        found or remaining.append((p_evar, m_evar))
    ...
    def choose_subject(self, expr):
        ...
        for v, c in expr.terms.items():
            if found_unrestricted and not v.is_restricted:
                if v not in self.columns:
                    retval_found = True
                    retval = v
                    break
                elif v.is_restricted:
                    if not found_new_restricted:
                        if not v.is_dummy:
                            if c < 0:
                                col = self.columns.get(v)
                                if not col == None:
                                    if not len(col) == 1 or self.objective in self.columns:
                                        subject = v
    ...
    def optimize(self, z_var):
        ...
        while 1:
            ...
            if not objective_coeff >= -EPSILON:
