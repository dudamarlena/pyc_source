...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/geometry/triangulation.py
# Compiled at: 2018-10-01 14:58:41
# Size of source mod 2**32: 39587 bytes
...
class Triangulation(object):
            ...
            for direction in (-1, 1):
                while True:
                    ...
                    if angle > np.pi / 2.0 or np.isnan(angle):
                        break
                    if not (i != front[ind1] and front[ind1] != front[ind2] and front[ind2] != i):
    ...
    def _finalize(self):
        ...
        while k < l - 1:
            if self._iscounterclockwise(front[k], front[(k + 1)], front[(k + 2)]):
                self._add_tri((front[k]), (front[(k + 1)]), (front[(k + 2)]), legal=False, source='fill_hull')
                front.pop(k + 1)
                l -= 1
                continue
            k += 1
        ...
        while tris:
            ...
            for t in tris:
                ...
                for i in (0, 1, 2):
                    ...
                    t2a = t2[1:3] + t2[0:1]
                    t2b = t2[2:3] + t2[0:2]
                    if not t2 in tri_state:
                        if t2a in tri_state or t2b in tri_state:
                            continue
                        elif self._is_constraining_edge(edge):
                            tri_state[t2] = 1 - v
                        else:
                            tri_state[t2] = v
                        next_tris.append(t2)
    ...
    def _edge_event(self, i, j):
        ...
        front = self._front
        if (
         i, j) in self._edges_lookup or (j, i) in self._edges_lookup:
            return
        else:
            upper_polygon = [
             i]
            ...
            if mode == 1:
                if j in next_tri:
                    ...
                    if lower_polygon[(-1)] == last_edge[0]:
                        ...
                        if lower_polygon[(-1)] == last_edge[1]:
                            upper_polygon.append(last_edge[0])
                        else:
                            if upper_polygon[(-1)] == last_edge[0]:
                                lower_polygon.append(last_edge[1])
                            else:
                                if upper_polygon[(-1)] == last_edge[1]:
                ...
                next_edge = (
                 front[front_index], front[(front_index + front_dir)])
    ...
    def _adjacent_tri(self, edge, i):
        ...
        try:
            ...
            if pt2 == i:
                return (
                 edge[1], edge[0], pt1)
            raise RuntimeError('Edge %s and point %d do not form a triangle in this mesh.' % (
             edge, i))
    ...
    def _distances_from_line(self, edge, points):
        ...
        if not (distances[0] == 0 and distances[(-1)] == 0):
            raise AssertionError
        return distances
    ...
    def _intersection_matrix(self, lines):
        ...
        *lines* must be an array of point locations with shape (N, 2, 2), where
        ...
        """
        return self._intersect_edge_arrays(lines[:, np.newaxis, ...], lines[(np.newaxis, ...)])
    ...
    def _remove_tri(self, a, b, c):
        ...
        if self._edges_lookup.get((a, b), -1) == c:
            ...
            if self._edges_lookup.get((b, a), -1) == c:
