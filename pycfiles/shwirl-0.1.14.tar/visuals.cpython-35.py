...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/code/shwirl/extern/vispy/scene/visuals.py
        ...
        if re.search('\\w', line):
            ...
            if params_started:
                if indent < param_indent:
                    break
                elif indent == param_indent and re.match('\\s*[a-zA-Z0-9_]+\\s*:\\s*\\S+', line) is None:
                    break
                param_end = i + 1
            else:
                if re.match('\\s*Parameters\\s*', line):
                    params_started = True
                    param_indent = indent
                    if first_blank is None:
                        first_blank = i
                else:
                    if first_blank is None and line.strip() == '':
                        first_blank = i
                    i += 1
...
__all__ = [name for name, obj in globals().items() if isinstance(obj, type) and issubclass(obj, VisualNode)]