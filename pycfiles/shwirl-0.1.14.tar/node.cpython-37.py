...
class Node(object):
    ...
    def parent(self, parent):
        if not isinstance(parent, (Node, type(None))):
            raise ValueError('Parent must be Node instance or None (got %s).' % parent.__class__.__name__)
        else:
            prev = self.parent
            if parent is prev:
                return
    ...
    def node_path_to_child(self, node):
        ...
        if node is self:
            ...
            assert path2, '%r is not a child of %r' % (node, self)
    ...
    def node_path(self, node):
        ...
        if cp is None:
            raise RuntimeError('No single-path common parent between nodes %s and %s.' % (
             self, node))
        p1 = p1[:p1.index(cp) + 1]
        p2 = p2[:p2.index(cp)][::-1]
        return (p1, p2)
