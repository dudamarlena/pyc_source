...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/code/shwirl/extern/vispy/ext/cocoapy.py
# Compiled at: 2017-04-05 22:12:59
# Size of source mod 2**32: 53492 bytes
from ctypes import cdll, util, Structure, cast, byref, POINTER, CFUNCTYPE, c_int, c_long, c_ulong, c_ushort, c_wchar, c_uint32, c_double, c_uint, c_float, c_void_p, c_char_p, c_bool, c_buffer, c_ubyte, c_byte, c_int8, c_int16, c_int32, c_int64, c_short, c_longlong, c_size_t, sizeof, c_uint8, c_longdouble, c_char, c_ulonglong, py_object, alignment, ArgumentError
...
def send_message(receiver, selName, *args, **kwargs):
    ...
    if should_use_fpret(restype):
        ...
        if x86_should_use_stret(restype):
            objc.objc_msgSend_stret.argtypes = [
             POINTER(restype), c_void_p,
             c_void_p] + argtypes
            result = restype()
            ...
            if c == b'}':
                ...
                if c == b'[':
                    ...
                    if c == b']':
                        ...
                        if brace_count or bracket_count:
                            typecode += c
                        else:
...
def cfunctype_for_encoding(encoding):
    ...
    for code in parse_type_encoding(encoding):
        if code in typecodes:
            ...
            if code[0:1] == b'^' and code[1:] in typecodes:
...
class ObjCMethod(object):
    ...
    def ctype_for_encoding(self, encoding):
        ...
        if encoding[0:1] == b'^' and encoding[1:] in [CGImageEncoding,
         NSZoneEncoding]:
            return c_void_p
