...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/Documents/Etudes/Doctorat/Australie/code/shwirl/extern/vispy/visuals/ellipse.py
...
class EllipseVisual(PolygonVisual):
    ...
    def _update(self):
        if self._center is None:
            ...
            vertices = self._generate_vertices(center=self._center, radius=self._radius, start_angle=self._start_angle, span_angle=self._span_angle, num_segments=self._num_segments)
            if not self._color.is_blank:
                self._mesh.set_data(vertices=vertices, color=self._color.rgba)
            if not self._border_color.is_blank:
                border_pos = vertices[:-1]
                self._border.set_data(pos=border_pos, color=self._border_color.rgba, width=self._border_width, connect='strip')
            return