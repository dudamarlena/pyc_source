...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/gloo/buffer.py
...
class Buffer(GLObject):
    ...
    def set_subdata(self, data, offset=0, copy=False):
        ...
        if offset < 0:
            ...
            elif nbytes == self._nbytes and offset == 0:
        ...
        if isinstance(key, string_types):
            ...
            if isinstance(key, int):
                ...
                if isinstance(key, slice):
                    ...
                    if step != 1:
                        raise ValueError('Cannot set non-contiguous data on buffer')
                    data = isinstance(data, np.ndarray) or np.array(data, dtype=(self.dtype), copy=False)
        ...
        if data.dtype.isbuiltin:
            if convert is True:
                ...
                if data.dtype in (np.float64, np.int64):
                    ...
                    if c in (2, 3, 4):
                        data.flags['C_CONTIGUOUS'] or logger.warning('Copying discontiguous data for struct dtype:\n%s' % _last_stack_str())
                        data = data.copy()
                    else:
                        c = 1
