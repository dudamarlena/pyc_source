...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/code/shwirl/extern/vispy/gloo/texture.py
...
class BaseTexture(GLObject):
    ...
    _inv_formats = {'luminance': 1, 
     ...
     'rgb': 3, 
     'rgba': 4}
    _inv_internalformats = dict([(base + suffix, channels) for base, channels in [
     ('r', 1), ('rg', 2), ('rgb', 3), ('rgba', 4)] for suffix in [
     '8', '16', '16f', '32f']] + [
     ('luminance', 1),
     ('alpha', 1),
     ('red', 1),
    ...
    def _set_data(self, data, offset=None, copy=False):
        ...
        if offset is None:
            ...
            if all([i == 0 for i in offset]) and data.shape == self._shape:
                self._resize(data.shape)
            offset = offset or tuple([0 for i in range(self._ndim)])
            if not len(offset) == self._ndim:
