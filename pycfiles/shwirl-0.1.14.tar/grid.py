...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/Documents/Etudes/Doctorat/Australie/code/shwirl/extern/vispy/scene/widgets/grid.py
# Compiled at: 2016-11-03 01:40:19
from __future__ import division
import numpy as np
from .widget import Widget
from ...ext.cassowary import SimplexSolver, expression, Variable, WEAK, REQUIRED, STRONG, RequiredFailure
    ...
    **kwargs : dict
    ...
    def __getitem__(self, idxs):
        ...
        if existing.any():
            ...
            if len(existing) != 1 or (layout == ii).sum() != np.prod(np.diff(lims)):
                raise ValueError('Cannot add widget (collision)')
            return self._grid_widgets[ii][(-1)]
        else:
            spans = np.diff(lims)[:, 0]
            item = self.add_widget(self._default_class(), row=lims[(0, 0)], col=lims[(1,
                                                                                      0)], row_span=spans[0], col_span=spans[1])
            return item
    ...
    def remove_widget(self, widget):
        ...
        """
        self._grid_widgets = dict((key, val) for key, val in self._grid_widgets.items() if val[(-1)] != widget)
        self._need_solver_recreate = True
    ...
    def __repr__(self):
        return '<Grid at %s:\n' % hex(id(self)) + str(self.layout_array + 1) + '>'
    ...
    def _add_widget_dim_constraints(solver, width_grid, height_grid, total_var_w, total_var_h, grid_widgets):
        ...
        for _, val in grid_widgets.items():
            ...
            for ws in width_grid[y:y + ys]:
                total_w = np.sum(ws[x:x + xs])
                solver.add_constraint(total_w >= widget.width_min, strength=REQUIRED)
                if widget.width_max is not None:
                    solver.add_constraint(total_w <= widget.width_max, strength=REQUIRED)
                else:
                    solver.add_constraint(total_w <= total_var_w)

            for hs in height_grid[x:x + xs]:
                total_h = np.sum(hs[y:y + ys])
                solver.add_constraint(total_h >= widget.height_min, strength=REQUIRED)
                if widget.height_max is not None:
                    solver.add_constraint(total_h <= widget.height_max, strength=REQUIRED)
                else:
    ...
    def _recreate_solver(self):
        ...
        self._height_stay = None
        self._width_stay = None
        self._width_grid = np.array([ [ Variable('width(x: %s, y: %s)' % (x, y)) for x in range(0, xmax) ] for y in range(0, ymax)
                                    ])
        self._height_grid = np.array([ [ Variable('height(x: %s, y: %s' % (x, y)) for y in range(0, ymax) ] for x in range(0, xmax)
                                     ])
        ...
        Grid._add_total_width_constraints(self._solver, self._width_grid, self._var_w)
        Grid._add_total_height_constraints(self._solver, self._height_grid, self._var_h)
        try:
        ...
        Grid._add_stretch_constraints(self._solver, self._width_grid, self._height_grid, self._grid_widgets, self._widget_grid)
        Grid._add_widget_dim_constraints(self._solver, self._width_grid, self._height_grid, self._var_w, self._var_h, self._grid_widgets)
        return
    ...
    def _update_child_widget_dim(self):
        ...
        if rect.height != self._var_h.value:
            ...
            self._height_stay = self._solver.add_stay(self._var_h, strength=STRONG)
        if rect.width != self._var_w.value:
            if self._width_stay:
                self._solver.remove_constraint(self._width_stay)
            self._var_w.value = rect.width
            self._width_stay = self._solver.add_stay(self._var_w, strength=STRONG)
    ...
    def _widget_grid(self):
        ymax, xmax = self.grid_size
        widget_grid = np.array([ [ None for _ in range(0, ymax) ] for _ in range(0, xmax)
                               ])
        for _, val in self._grid_widgets.items():
            y, x, ys, xs, widget = val
