...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/util/logs.py
...
class use_log_level(object):
    ...
    def __init__(self, level, match=None, record=False, print_msg=True):
        ...
        if match is not None:
            if not isinstance(match, string_types):
                raise TypeError('match must be None or str')

    def __enter__(self):
        old_level, old_match = set_log_level((self._new_level), (self._new_match),
          return_old=True)
