...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/visuals/shaders/variable.py
...
class Variable(ShaderObject):
    ...
    def __init__(self, name, value=None, vtype=None, dtype=None):
        ...
        if ' ' in name:
            ...
            if len(fields) == 3:
                ...
                if len(fields) == 4 and fields[0] == 'const':
                    vtype, dtype, name, value = fields
                else:
                    raise ValueError('Variable specifications given by string must be of the form "vtype dtype name" or "const dtype name value".')
        ...
        if value is not None:
            self.value = value
        if self._vtype:
            if self._vtype not in VARIABLE_TYPES:
    ...
    def value(self, value):
        if isinstance(value, (tuple, list)):
            if 1 < len(value) < 5:
                ...
                if isinstance(value, np.ndarray):
                    ...
                    raise ValueError('Cannot make uniform value for %s from array of shape %s.' % (
                     self.name, value.shape))
        else:
            if np.isscalar(value):
                ...
                if getattr(value, 'glsl_type', None) is not None:
        ...
        if self._type_locked and not dtype != self._dtype:
            if vtype != self._vtype:
                raise TypeError('Variable is type "%s"; cannot assign value %r.' % (
                 self.dtype, value))
            return
            changed = False
...
class Varying(Variable):
    ...
    def link(self, var):
        ...
        """
        if not self._dtype is not None:
