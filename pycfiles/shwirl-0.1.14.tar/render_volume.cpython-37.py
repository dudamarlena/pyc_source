...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/visual/render_volume.py
# Compiled at: 2018-10-02 03:28:55
# Size of source mod 2**32: 47056 bytes
...
FRAG_GENERAL_SECTIONS = dict(frag_init_uniforms=FRAG_INIT_UNIFORMS,
  frag_init_varyings=FRAG_INIT_VARYINGS,
  frag_init_consts=FRAG_INIT_CONSTS,
  frag_init_variables=FRAG_INIT_VARIABLES,
  smoothing_kernels=SMOOTHING_KERNELS,
  edge_detection_kernel=EDGE_DETECTION_KERNEL,
  calculate_color_kernel=CALCULATE_COLOR_KERNEL,
  frag_init_ray=FRAG_INIT_RAY,
  smooth_value=SMOOTH_VALUE,
  filter_value=FILTER_VALUE,
  before_loop='<(tf_before_loop)>',
  in_loop='<(tf_in_loop)>',
  after_loop='<(tf_after_loop)>')
FRAG_SHADER = ('\n{frag_init_uniforms}\n{frag_init_varyings}\n{frag_init_consts}\n{frag_init_variables}\n\n{smoothing_kernels}\n{calculate_color_kernel}\n\nvoid main() {{\n    \n    {frag_init_ray}\n    \n    {before_loop}\n    \n    vec3 loc = start_loc;  \n    \n    for (int iter=0; iter<nsteps; iter++)\n    {{\n        color = $sample(u_volumetex, loc);\n        // Get sample color\n        if (u_filter_size == 1)\n            color = $sample(u_volumetex, loc);\n        else {{\n            color = movingAverageFilter_line_of_sight(loc, step);\n        }}\n\n        if (u_use_gaussian_filter==1) {{\n            {smooth_value}\n        }}\n\n        val = color.g;\n        \n        {filter_value}\n        {in_loop}\n        \n        // Advance location deeper into the volume\n        loc += step;\n    }}\n\n    /*\n    {after_loop}\n    */\n    \n    gl_FragColor = $cmap(maxval);\n    \n    //gl_FragColor = color;\n}}\n\n'.format)(**FRAG_GENERAL_SECTIONS)
FRAG_SHADER = FRAG_SHADER.replace('{', '{{').replace('}', '}}').replace('<(', '{').replace(')>', '}')
MIP_SNIPPETS = dict(tf_before_loop='\n        float maxval = -99999.0; // The maximum encountered value\n        int maxi = 0;  // Where the maximum value was encountered\n        ',
  tf_in_loop='\n        if( val > maxval ) {\n            maxval = val;\n            maxi = iter;\n        }\n        ',
...
class RenderVolumeVisual(Visual):
    ...
    def set_data(self, vol, clim=None):
        ...
        if not isinstance(vol, np.ndarray):
            ...
            if not vol.ndim == 3:
                raise vol.ndim == 4 and vol.shape[(-1)] <= 4 or ValueError('Volume visual needs a 3D image.')
            if clim is not None:
            ...
            vol = np.array((np.flipud(vol)), dtype='float32', copy=False)
    ...
    def density_factor(self, density_factor=1):
