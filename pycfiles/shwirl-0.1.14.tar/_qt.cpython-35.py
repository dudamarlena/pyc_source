...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/code/shwirl/extern/vispy/app/backends/_qt.py
...
if sys.platform.startswith('linux'):
    ...
    class QtBaseCanvasBackend(BaseCanvasBackend):
        ...
        def mousePressEvent(self, ev):
            if self._vispy_canvas is None:
                return
            self._vispy_mouse_press(native=ev, pos=(
             ev.pos().x(), ev.pos().y()), button=BUTTONMAP.get(ev.button(), 0), modifiers=self._modifiers(ev))

        def mouseReleaseEvent(self, ev):
            if self._vispy_canvas is None:
                return
            self._vispy_mouse_release(native=ev, pos=(
             ev.pos().x(), ev.pos().y()), button=BUTTONMAP[ev.button()], modifiers=self._modifiers(ev))

        def mouseDoubleClickEvent(self, ev):
            if self._vispy_canvas is None:
                return
            self._vispy_mouse_double_click(native=ev, pos=(
             ev.pos().x(), ev.pos().y()), button=BUTTONMAP.get(ev.button(), 0), modifiers=self._modifiers(ev))

        def mouseMoveEvent(self, ev):
            if self._vispy_canvas is None:
                return
            self._vispy_mouse_move(native=ev, pos=(
             ev.pos().x(), ev.pos().y()), modifiers=self._modifiers(ev))
        ...
        def wheelEvent(self, ev):
            ...
            self._vispy_canvas.events.mouse_wheel(native=ev, delta=(
             deltax, deltay), pos=(
             ev.pos().x(), ev.pos().y()), modifiers=self._modifiers(ev))
            ...
            if t == QtCore.QEvent.Gesture:
                ...
                if gesture:
                    ...
                    self._vispy_canvas.events.touch(type='pinch', pos=(
                     x, y), last_pos=None, scale=scale, last_scale=last_scale, rotation=rotation)
                ...
                self._vispy_canvas.events.touch(type='touch', pos=pos, last_pos=lpos)
            return out
