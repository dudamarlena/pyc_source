...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/util/event.py
# Compiled at: 2018-10-01 14:58:41
# Size of source mod 2**32: 29428 bytes
"""
...
class EventEmitter(object):
    ...
    def print_callback_errors(self, val):
        if val not in ('first', 'reminders', 'always', 'never'):
            raise ValueError('print_callback_errors must be "first", "reminders", "always", or "never"')
    ...
    def connect(self, callback, ref=False, position='first', before=None, after=None):
        ...
        for ri, criteria in enumerate((before, after)):
            if criteria is None or criteria == []:
                ...
                matches = [ci for ci, (cn, cc) in enumerate(zip(callback_refs, callbacks)) if not cc in criteria if cn in criteria]
                bounds.append(matches[0] if ri == 0 else matches[(-1)] + 1)

        if bounds[0] < bounds[1]:
            raise RuntimeError('cannot place callback before "%s" and after "%s" for callbacks: %s' % (
             before, after, callback_refs))
        idx = bounds[1] if position == 'first' else bounds[0]
    ...
    def _normalize_cb(self, callback):
        if inspect.ismethod(callback):
            callback = (
             callback.__self__, callback.__name__)
        if isinstance(callback, tuple):
            if not isinstance(callback[0], weakref.ref):
                callback = (
                 weakref.ref(callback[0]),) + callback[1:]
        return callback
    ...
    def _invoke_callback(self, cb, event):
        try:
            ...
            _handle_exception((self.ignore_callback_errors), (self.print_callback_errors),
              self,
              cb_event=(cb, event))
    ...
    def _prepare_event(self, *args, **kwargs):
        if len(args) == 1:
            if not kwargs:
                ...
                args = args or self.default_args.copy()
                args.update(kwargs)
    ...
    def unblock(self, callback=None):
        ...
        if callback not in self._blocked or self._blocked[callback] == 0:
            raise RuntimeError('Cannot unblock %s for callback %s; emitter was not previously blocked.' % (
             self, callback))
...
class WarningEmitter(EventEmitter):
    ...
    def _warn(self, cb):
        if self._warned:
            return
        if isinstance(cb, tuple):
            if getattr(cb[0], cb[1], None) is None:
                return
    ...
    def add(self, auto_connect=None, **kwargs):
        ...
        for name, emitter in kwargs.items():
            ...
            if inspect.isclass(emitter):
                if issubclass(emitter, Event):
                    emitter = EventEmitter(source=(self.source),
                      type=name,
                      event_class=emitter)
                else:
                    if not isinstance(emitter, EventEmitter):
                        raise Exception('Emitter must be specified as either an EventEmitter instance or Event subclass. (got %s=%s)' % (
                         name, emitter))
                    else:
...
class EventBlocker(object):
    __doc__ = " Represents a block for an EventEmitter to be used in a context\n    manager (i.e. 'with' statement).\n    "
