...
# [GCC 8.4.0]
# Embedded file name: /Users/danyvohl/Documents/Etudes/Doctorat/Australie/code/shwirl/extern/vispy/visuals/colorbar.py
# Compiled at: 2017-02-09 01:58:44
import numpy as np
...
class ColorBarVisual(CompoundVisual):
 ...
 0, 0], label_str='', label_color='black', clim=(0.0, 1.0), border_width=1.0, border_color='black', **kwargs):
    ...
    def _update_positions(self):
        ...
        x, y = self._pos
        halfw, halfh = self._halfdim
        label_anchors = ColorBarVisual._get_label_anchors(center=self._pos, halfdim=self._halfdim, orientation=self._orientation, transforms=self.label.transforms)
        self._label.anchors = label_anchors
        ticks_anchors = ColorBarVisual._get_ticks_anchors(center=self._pos, halfdim=self._halfdim, orientation=self._orientation, transforms=self.label.transforms)
        self._ticks[0].anchors = ticks_anchors
        self._ticks[1].anchors = ticks_anchors
        label_pos, ticks_pos = ColorBarVisual._calc_positions(center=self._pos, halfdim=self._halfdim, border_width=self.border_width, orientation=self._orientation, transforms=self.transforms)
