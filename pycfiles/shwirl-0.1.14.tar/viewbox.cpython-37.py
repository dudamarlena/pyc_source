...
# [GCC 8.4.0]
# Embedded file name: /Users/vohl/Documents/code/shwirl/shwirl/extern/vispy/scene/widgets/viewbox.py
...
class ViewBox(Widget):
    ...
    def __init__(self, camera=None, **kwargs):
        ...
        if camera is None:
            camera = 'base'
        elif isinstance(camera, string_types):
            self.camera = make_camera(camera, parent=(self.scene))
        else:
            if isinstance(camera, BaseCamera):
                self.camera = camera
            else:
