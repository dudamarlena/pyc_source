...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_bridge_base/jet_bridge_base/views/base/api.py
# Compiled at: 2019-10-22 13:08:20
import platform
...
class APIView(object):
    ...
    def error_response(self, exc_type, exc, traceback):
        if isinstance(exc, PermissionDenied):
            return TemplateResponse('403.html', status=403, data={'path': self.request.path})
        else:
            if isinstance(exc, NotFound):
                return TemplateResponse('404.html', status=404, data={'path': self.request.path})
            if settings.DEBUG:
                ctx = {'path': self.request.path, 'full_path': self.request.protocol + '://' + self.request.host + self.request.path, 
                ...
                if exc:
                    ctx.update({'exception_type': exc_type.__name__, 
                ...
                if traceback:
                    ...
                    ctx.update({'exception_last_traceback_line': line_number, 
