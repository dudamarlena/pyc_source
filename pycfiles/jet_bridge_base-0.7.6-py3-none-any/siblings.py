...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_bridge_base/jet_bridge_base/utils/siblings.py
# Compiled at: 2019-03-27 02:10:07
...
def get_row_number(Model, queryset, instance):
    ...
    subqquery = queryset.with_entities(mapper.primary_key[0].label('__inner__pk'), func.row_number().over(order_by=queryset._order_by).label('__inner__row')).subquery()
...
def get_row_siblings(Model, queryset, row_number):
    ...
    return {'prev': map_row(rows[prev_index]) if prev_index is not None else None, 
