...
# [GCC 8.4.0]
# Embedded file name: /Users/f1nal/Dropbox/python/jet-bridge/src/packages/jet_bridge_base/jet_bridge_base/db.py
# Compiled at: 2019-11-26 08:18:15
...
def build_engine_url(DATABASE_ENGINE, DATABASE_HOST, DATABASE_PORT, DATABASE_NAME, DATABASE_USER, DATABASE_PASSWORD, DATABASE_EXTRA=''):
    if not DATABASE_ENGINE or not DATABASE_NAME:
        ...
        if DATABASE_EXTRA:
            ...
            if DATABASE_ENGINE == 'mysql':
                ...
                if DATABASE_ENGINE == 'mssql+pyodbc':
                    url.append('?driver=SQL+Server+Native+Client+11.0')
        return ''.join(url)


def build_engine_url_from_settings():
    return build_engine_url(settings.DATABASE_ENGINE, settings.DATABASE_HOST, settings.DATABASE_PORT, settings.DATABASE_NAME, settings.DATABASE_USER, settings.DATABASE_PASSWORD, settings.DATABASE_EXTRA)
...
def database_connect():
    ...
    def only(table, meta):
        if settings.DATABASE_ONLY is not None:
            if table not in settings.DATABASE_ONLY:
                return False
        if settings.DATABASE_EXCEPT is not None:
            if table in settings.DATABASE_EXCEPT:
                return False
        return True
