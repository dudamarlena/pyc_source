...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/genconf/dispatch.py
# Compiled at: 2011-04-12 12:37:37
"""
...
def dispatch(argv):
    ...
    genconf = GenConf(manifest_path=args.manifest, overrides_path=args.overrides, templatedir=args.templatedir, targetdir=args.targetdir)
...
def parse_args(argv):
    ...
    parser.add_argument('-m', '--manifest', dest='manifest', action='store', default=os.path.join(os.curdir, 'genconf-manifest.yaml'), help='the genconf-YAML manifest to use (default: ./genconf-manifest.yaml)')
    parser.add_argument('-o', '--manifest-overrides', dest='overrides', action='store', default=os.path.expanduser('~/.genconf-overrides.yaml'), help='the genconf-overrides file to use (default: ~/.genconf-overrides.yaml)')
    parser.add_argument('-t', '--target-directory', dest='targetdir', action='store', default=os.curdir, help='the target directory into which files will be generated (default: the current directory)')
    parser.add_argument('-T', '--template-directory', dest='templatedir', action='store', default=os.curdir, help='the directory in which the templates reside (default: the current directory)')
    parser.add_argument('-v', '--verbose', dest='verbose', action='store_true', default=False, help='Whether to print additional progress messages')
