...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/genconf/manifest/_parser.py
# Compiled at: 2011-04-12 12:32:03
"""
...
class ManifestParser(object):
    ...
    def parse(self, stream, overrides={}):
        ...
        def create_output_file(profile_provider):
            ...
            def f(data):
                return OutputFile(profile_provider, target_path=data['target'], template_path=data['template'], template_format=data['template_format'])
        ...
        def do_create_profile(data):
            p = Profile(name=data['name'], is_abstract=data['abstract'], extends=map(get_profile, data['extends']), properties=data['properties'], output_files=map(create_output_file(lambda : p), data['output_files']), overrides=project_overrides.get(data['name'], {}))
