...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/MFEprimer/chilli/chilli.py
...
def parse_blast7_best(fh):
    ...
    for line in fh:
        ...
        if query_id not in records:
            records[query_id] = {'sbjct_id': sbjct_id, 'identity': float(fields[2]), 
...
def parse_blastn7(fh):
    ...
    for line in fh:
        ...
        records[query_id][sbjct_id].append({'identity': float(fields[2]), 
           'aln_len': int(fields[3]), 
           'mismatches': int(fields[4]), 
           'gap_opens': int(fields[5]), 
           'q_start': int(fields[6]), 
...
def parse_blastn7_to_array(fh):
    ...
    for line in fh:
        ...
        if query_id not in records:
            records[query_id] = {}
        records[query_id].append({'sbjct_id': sbjct_id, 
