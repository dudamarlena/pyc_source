...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/MFEprimer/chilli/DegenerateSeqConvetor.py
...
import FastaFormatParser as FFP, chilli
iupac_dict = {'a': 'a', 
   'A': 'A', 
   't': 't', 
   'T': 'T', 
...
def convert(records):
    ...
    for record in records:
        ...
        for normal_seq in normal_seq_list:
            sn += 1
            result_fasta_array.append({'id': '%s_%s' % (id, sn), 
               'desc': desc, 
               'seq': normal_seq, 
               'size': len(seq)})
