...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/MFEprimer/chilli/MFEprimerParser.py
    ...
    while True:
        ...
        if re.match('>\\d+', line):
            ...
            while True:
                ...
                if not line:
                    amp.append(AmpCreator(amp_id, fp_id, rp_id, float(ppc), int(size), float(gc), float(fp_tm), float(fp_dg), float(rp_tm), float(rp_dg), ('').join(seq), hit_id))
                    break
                else:
                    seq.append(line)
