...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/MFEprimer/chilli/ve.py
# Compiled at: 2017-07-16 05:34:29
from __future__ import division
__version__ = '1.0'
__author__ = 'Wubin Qu <quwubin@gmail.com>'
__license__ = 'GPL v3'
import os, time, CairoNumberWidth as CNW, cairo, GelMobility
...
def draw_virtual_elec(ctx, height, width, idname_size, agarose, product_number):
    ...
    for i in range(product_number + 2):
        ...
        if i != 0 and i <= product_number:
            ...
            ctx.line_to(x + 60, y1)
            x2 = 110 + 90 * (product_number + 1) + 10
            ctx.move_to(x2, y1)
            ctx.set_line_width(6)
            ctx.line_to(x2 + 50, y1)
            ctx.stroke()
            x3 = 110 + 90 * (product_number + 2)
            ctx.move_to(x3, y1 + 4)
...
def main():
    size_list = [
     100, 114, 128, 144, 161, 179, 198, 219, 242, 266, 292, 320, 350, 383, 418, 456, 497, 541, 588, 639, 694, 753, 816, 885, 958, 1038, 1123, 1215, 1314, 1421, 1536, 1660, 1793]
    a = [
     90, 104, 119, 135, 151, 169, 189, 209, 231, 255, 283, 313, 346, 381, 416, 454, 495, 543, 595, 647, 708, 768, 833, 910, 993, 1076, 1173, 1269, 1383, 1507, 1641, 1787, 1944]
