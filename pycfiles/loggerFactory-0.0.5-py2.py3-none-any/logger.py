...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/loggerFactory-project/loggerFactory/logger.py
# Compiled at: 2019-03-01 11:11:57
"""
Implements
"""
...
class BaseLogger(object):
    ...
    def show(self, msg, indent=0, style='', **kwargs):
        ...
        if self.enable_verbose:
            new_msg = self.MessageTemplate.with_style.format(indent=self.tab * indent, style=style, msg=msg)
...
class StreamOnlyLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, rand_name=False, stream_level=logging.INFO, stream_format=DEFAULT_STREAM_FORMAT):
...
class SingleFileLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, rand_name=False, path=None, logging_level=logging.DEBUG, stream_level=logging.INFO, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, reset=False):
        if path is None:
...
class FileRotatingLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, rand_name=False, path=None, logging_level=logging.DEBUG, stream_level=logging.INFO, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, max_bytes=1000000000, backup_count=10):
        if path is None:
            raise ValueError('Please specify a log file in ``path``!')
        super(FileRotatingLogger, self).__init__(name, rand_name)
        self.logger.setLevel(logging_level)
        file_handler = RotatingFileHandler(path, mode='a', maxBytes=max_bytes, backupCount=backup_count, encoding='utf-8')
        file_handler.setFormatter(logging.Formatter(logging_format))
        self.logger.addHandler(file_handler)
        set_stream_handler(self.logger, stream_level, stream_format)
        return
...
class TimeRotatingLogger(BaseLogger):
    ...
    **中文文档**
    ...
    def __init__(self, name=None, rand_name=False, path=None, logging_level=logging.DEBUG, stream_level=logging.INFO, logging_format=DEFAULT_LOG_FORMAT, stream_format=DEFAULT_STREAM_FORMAT, rotate_on_when='D', interval=1, backup_count=30):
        if path is None:
            raise ValueError('Please specify a log file in ``path``!')
        super(TimeRotatingLogger, self).__init__(name, rand_name)
        self.logger.setLevel(logging_level)
        file_handler = TimedRotatingFileHandler(path, when=rotate_on_when, interval=interval, backupCount=backup_count, encoding='utf-8')
        file_handler.setFormatter(logging.Formatter(logging_format))
        self.logger.addHandler(file_handler)
        set_stream_handler(self.logger, stream_level, stream_format)
        return