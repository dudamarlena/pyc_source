...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/uforgecli/commands/org/org_targetFormat.py
...
import pyxb, datetime, shlex, os.path
...
class Org_TargetFormat_Cmd(Cmd, CoreGlobal):
    ...
    def arg_create(self):
        ...
        optional.add_argument('--credAccountType', dest='credAccountType', type=str, required=False, help='Credential account type of the target format to create in the organization (aws, azure, ews, google, abiquo, cloudcom, flexiant, nimbula, openstack, vclouddirector, vsphere).')
    ...
    def do_create(self, args):
        try:
            ...
            printer.out('In Arguments: ' + str(e), printer.ERROR)
            self.help_create()
        except pyxb.SimpleFacetValueError, e:
    ...
    def arg_update(self):
        ...
        optional.add_argument('--credAccountType', dest='credAccountType', type=str, required=False, help='(New) Credential account type of the target format to update in the organization (aws, azure, ews, google, abiquo, cloudcom, flexiant, nimbula, openstack, vclouddirector, vsphere).')
