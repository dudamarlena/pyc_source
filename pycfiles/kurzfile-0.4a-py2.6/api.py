...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kurzfile/api.py
# Compiled at: 2011-02-04 07:00:18
...
def parse_header(data):
    ...
    """
    if len(data) < 32:
        raise ValueError('Header size too small (%i bytes). Must be 32 bytes.' % len(data))
...
class Kurzfile(object):
    ...
    def parse_file(self, filename):
        ...
        try:
            try:
                ...
                log.debug("File '%s', size %i", self.filename, self.filesize)
                ...
                if obj_data_size < hdr.object_data_size:
                    msg = 'Truncated object data. Expected %i bytes, read only %i bytes before end-of-file.' % (
        ...
        while True:
            if offset + 4 > len(data):
                if len(data) < data_len - 32:
                    msg = 'Truncated object data. Expected %i bytes, got only %i bytes.' % (
                     data_len, len(data))
                    if self.strict:
                        raise ParseError(msg)
                    log.warning(msg)
            ...
            if blocksize >= len(remaining_data):
                msg = 'Truncated block data. Expected %i bytes, read only %i bytes before end of data.' % (
                 blocksize, len(remaining_data))
                if self.strict:
                    raise ParseError(msg)
                log.warning(msg)
                blocksize = len(remaining_data)
            yield KurzfileBlock(self, blocksize, data[offset + 4:offset + blocksize])
            offset += blocksize
            offset += offset % 4
    ...
    def list_objects(self, stream=None, listformat='pretty', format=None, fields=None, sortby=None):
        ...
        if listformat == 'pretty':
            print >> stream, '%-16s %-3s %-12s %12s' % ('Type', 'ID', 'Name', 'Size (bytes)')
            print >> stream, '-' * 46
        elif listformat == 'csv':
            fileinfo = dict(filename=basename(self.filename), filesize=self.filesize, path=dirname(self.filename))
            if not fields:
                fields = ('type_name', 'id', 'name', 'size', 'filename', 'filesize',
                          'path')
...
class KurzfileBlock(object):
    ...
    def parse(self, data):
        ...
        while True:
            ...
            log.debug('Object with typeid %s,%s (type=%s, ID %i) found.', hex(typeid[0]), hex(typeid[1]), type, id)
            ...
            if d_offset >= size - 2:
                ...
                obj_count += 1
                yield KurzfileObject(type, id, name, size, data[offset + 4 + d_offset:offset + size])
            offset += size
            offset += offset % 4
