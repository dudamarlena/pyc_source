...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/project/importfiles.py
# Compiled at: 2019-10-17 01:45:10
# Size of source mod 2**32: 20159 bytes
import tkinter as tk
from tkinter import ttk, filedialog, messagebox
from noval import _, GetApp
import os, noval.util.apputils as sysutilslib, noval.util.fileutils as fileutils, threading, time, noval.util.strutils as strutils, noval.project.wizard as projectwizard, noval.consts as consts, noval.ttkwidgets.checkboxtreeview as checkboxtreeview, noval.ttkwidgets.checklistbox as checklistbox, noval.ttkwidgets.treeviewframe as treeviewframe, noval.syntax.lang as lang, noval.project.baseconfig as baseconfig, queue, noval.util.utils as utils, noval.ui_base as ui_base, noval.constants as constants, noval.project.document as projectdocument
...
class ImportfilesPage(projectwizard.BitmapTitledContainerWizardPage):
    ...
    def CreateContent(self, content_frame, **kwargs):
        ...
        self.file_filter_btn = ttk.Button(sizer_frame, text=_('File Filters'), command=self.SetFilters)
        self.file_filter_btn.grid(column=0, row=0, sticky='nsew', pady=(consts.DEFAUT_CONTRL_PAD_Y, 0))
        self.select_all_btn = ttk.Button(sizer_frame, text=_('Select All'), command=self.SelectAll)
        self.select_all_btn.grid(column=1, row=0, sticky='nsew', padx=(consts.DEFAUT_CONTRL_PAD_X, 0), pady=(consts.DEFAUT_CONTRL_PAD_Y, 0))
        self.unselect_all_btn = ttk.Button(sizer_frame, text=_('UnSelect All'), command=self.UnselectAll)
    ...
    def ListDirFiles(self, item, checked=True):
        ...
        for f in files:
            file_path = os.path.join(path, f)
            if os.path.isfile(file_path) and not fileutils.is_file_path_hidden(file_path) and strutils.get_file_extension(file_path) not in self.rejects:
    ...
    def Finish(self):
        ...
        if GetApp().GetDefaultLangId() == lang.ID_LANG_PYTHON and self._is_wizard and prev_page._new_project_configuration.PythonpathMode == baseconfig.NewProjectConfiguration.PROJECT_ADD_SRC_PATH:
