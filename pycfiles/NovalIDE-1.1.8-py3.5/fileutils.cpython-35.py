...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/util/fileutils.py
# Compiled at: 2019-10-17 03:45:04
# Size of source mod 2**32: 22414 bytes
from noval import _
import logging, copy, os, shutil, sys, zipfile, noval.util.logger as logger, noval.util.apputils as apputils, noval.util.utillang as utillang, subprocess, noval.util.strutils as strutils, chardet, noval.util.txtutils as txtutils, noval.syntax.lang as lang, noval.syntax.syntax as syntax
        ...
        if len(line1) == 0:
            ...
            if ignore != None:
                ...
                if line1 != line2:
                    len1 = len(line1)
                    len2 = len(line2)
                    if abs(len1 - len2) == 1 and len1 > 0 and len2 > 0 and line1[(-1)] == '\n' and line2[(-1)] == '\n':
                        if len1 > len2:
                            longer = line1
                            shorter = line2
                        else:
...
def convertSourcePath(path, to, otherdir=None):
    ...
    if ix < 0:
        ...
        if ix < 0 or len(path) <= ix + 7 or path[ix] not in ('\\', '/') or path[(ix + 7)] not in ('\\',
                                                                                                  '/'):
...
def zip(zipfilepath, basedir=None, files=None):
    ...
    if files is None:
        logger.debug(fileutilsLogger, 'Looking for files to zip in %s' % basedir)
...
def unzip(zipfilepath, extractdir):
    ...
    for info in z.infolist():
        ...
        if os.path.isdir(filename):
            ...
            logger.debug(fileutilsLogger, 'Writing arcfile %s to %s' % (info.filename, filename))
            f = open(filename, 'w')
...
def replaceToken(infilepath, tokens={}, outfilepath=None, delim='@@', useEnv=False):
