...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/parser/intellisence.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 14888 bytes
from noval import _, GetApp
import noval.util.appdirs as appdirs, noval.python.interpreter.interpreter as pythoninterpreter, noval.python.interpreter.interpretermanager as interpretermanager, subprocess, noval.util.apputils as apputils
...
import noval.util.utils as utils, datetime, copy, noval.consts as consts
...
class IntellisenceDataLoader(object):
    ...
    def __init__(self, data_location, _builtin_data_location, manager):
        ...
        self._IntellisenceDataLoader__builtin_data_location = _builtin_data_location
        self.module_dicts = {}
        self.import_list = []
    ...
    def LodBuiltInData(self, interpreter):
        if interpreter.IsV2():
            builtin_data_path = os.path.join(self._IntellisenceDataLoader__builtin_data_location, '2')
        else:
            builtin_data_path = os.path.join(self._IntellisenceDataLoader__builtin_data_location, '3')
    ...
    def LoadIntellisenceDirData(self, data_path):
        ...
        for name in name_sets:
            d = dict(members=os.path.join(data_path, name + config.MEMBERS_FILE_EXTENSION), member_list=os.path.join(data_path, name + config.MEMBERLIST_FILE_EXTENSION))
    ...
    def LoadImportList(self):
        for key in self.module_dicts.keys():
            if key.find('.') == -1 and key not in self.import_list:
...
class IntellisenceManager(object):
    ...
    def IsInterpreterNeedUpdateDatabase(self, interpreter):
        ...
        if update_interval_option == consts.UPDATE_ONCE_WEEK:
            return now_datetime > last_datetime + datetime.timedelta(days=7)
        if update_interval_option == consts.UPDATE_ONCE_MONTH:
            return now_datetime > last_datetime + datetime.timedelta(days=30)
        if update_interval_option == consts.NEVER_UPDATE_ONCE:
            return False
    ...
    def generate_default_intellisence_data(self):
        ...
        try:
            ...
            utils.get_logger().error('load interpreter name %s path %s version %s intellisence data path %s error: %s', current_interpreter.Name, current_interpreter.Path, current_interpreter.Version, os.path.join(self.data_root_path, str(current_interpreter.Id)), e)
            utils.get_logger().exception('')
    ...
    def GetModule(self, name):
        if name in self._loader.module_dicts:
            return ModuleLoader(name, self._loader.module_dicts[name][ModuleLoader.MEMBERS_KEY], self._loader.module_dicts[name][ModuleLoader.MEMBER_LIST_KEY], self)
    ...
    def AddUnfinishModuleFile(self, module_file):
        ...
        if interpreter.Path not in self.unfinish_files:
