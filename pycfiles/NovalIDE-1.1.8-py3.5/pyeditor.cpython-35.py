...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/pyeditor.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 33887 bytes
import os
from tkinter import messagebox
from noval import _, GetApp, NewId
import noval.editor.code as codeeditor, string, keyword, sys, codecs, noval.python.parser.scope as scope, noval.python.interpreter.interpreter as pythoninterpreter, intellisence, nodeast, noval.util.strutils as strutils
from noval.python.parser.utils import CmpMember
import noval.python.interpreter.interpretermanager as interpretermanager, noval.util.fileutils as fileutils, noval.python.parser.utils as parserutils, noval.consts as consts
from noval.util import utils
import noval.python.analyzer as analyzer, noval.syntax.lang as lang, noval.menu as tkmenu, noval.constants as constants, noval.python.project.viewer as projectviewer, noval.python.project.runconfig as runconfig, noval.ui_utils as ui_utils
from noval.python.project.rundocument import *
import noval.python.pyutils as pyutils, io as StringIO, noval.python.debugger.debugger as pythondebugger, noval.python.debugger.watchs as watchs
...
class PythonDocument(codeeditor.CodeDocument):
    ...
    def DoSave(self):
        ...
        if interpreter is None or interpreter.IsV2():
            is_v2 = True
        if is_v2 and None == declare_encoding and self.file_encoding != self.ASC_FILE_ENCODING:
            ...
            if ret == True and GetApp().InsertEncodingDeclare(view):
                lines = view.GetCtrl().GetTopLines(consts.ENCODING_DECLARE_LINE_NUM)
                declare_encoding = self.get_coding_spec(lines)
        ...
        elif self.IsDocEncodingChanged(declare_encoding):
            self.file_encoding = declare_encoding
    ...
    def GetDocEncoding(self, encoding):
        ...
        if lower_encoding == 'gbk' or lower_encoding == 'gb2312' or lower_encoding == 'gb18030' or lower_encoding == self.ANSI_FILE_ENCODING:
            return self.GBK_ENCODING
        if lower_encoding == self.ASC_FILE_ENCODING:
...
class PythonView(codeeditor.CodeView):
            ...
            while parent is not None:
                if parent.Parent is None:
                    ...
                    if is_cls_scope and parent == scope.Parent:
                        ...
                        if is_self_scope and parent == scope.Parent:
                            member_list = [
                             'self']
                        else:
    ...
    def LoadOutLine(self, outlineView, force=False, lineNum=-1):
        ...
        force = force or self._checkSum != newCheckSum
        if callback_view and callback_view is self and self._checkSum == newCheckSum:
    ...
    def UpdateUI(self, command_id):
        if command_id in [constants.ID_INSERT_DECLARE_ENCODING, constants.ID_UNITTEST, constants.ID_RUN, constants.ID_DEBUG, constants.ID_SET_EXCEPTION_BREAKPOINT, constants.ID_STEP_INTO, constants.ID_STEP_NEXT, constants.ID_RUN_LAST,
         constants.ID_CHECK_SYNTAX, constants.ID_SET_PARAMETER_ENVIRONMENT, constants.ID_DEBUG_LAST, constants.ID_START_WITHOUT_DEBUG, constants.ID_TOGGLE_BREAKPOINT]:
            return True
        if command_id == constants.ID_GOTO_DEFINITION:
...
class PythonCtrl(codeeditor.CodeCtrl):
    ...
    def DoIndent(self):
        ...
        eol_char = '\n'
        if caretPos == 0 or len(text.strip()) == 0:
                ...
                for i in range(caretPos - 1, -1, -1):
                    if i >= len(text):
                        ...
                        if self._tokenizerChars[i] == 1:
                            ...
                            if self._tokenizerChars[i] == 2:
                                startQuoteCount = startQuoteCount + 1
                            else:
                                if self._tokenizerChars[i] == 3:
    ...
    def OnChar(self, event):
        ...
        if event.char == '(':
            ...
            if event.char == self.TYPE_POINT_WORD:
    ...
    def GetCurdirImports(self):
        ...
        for file_name in os.listdir(dir_path):
            if not parserutils.ComparePath(cur_file_name, file_name):
                if not not fileutils.is_python_file(file_name):
                    if file_name.find(' ') != -1:
                        pass
                    else:
    ...
    def IsCaretLocateInWord(self, pos=None):
        ...
        if at == '':
            return False
        if at in self.DEFAULT_WORD_CHARS:
            return True
        return False
