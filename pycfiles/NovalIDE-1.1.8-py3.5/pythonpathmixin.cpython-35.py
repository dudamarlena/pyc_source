...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/pythonpathmixin.py
...
import noval.util.fileutils as fileutils, noval.util.apputils as sysutils, noval.python.parser.utils as parserutils, locale, noval.imageutils as imageutils, noval.consts as consts, noval.ttkwidgets.treeviewframe as treeviewframe, noval.menu as tkmenu
ID_GOTO_PATH = NewId()
...
class PythonpathMixin:
    ...
    def InitUI(self, hide_tree_root=False):
        ...
        self.add_file_btn = ttk.Menubutton(right_frame, text=_('Add File...'), state='pressed')
    ...
    def AddNewFilePath(self, id):
        ...
        path = filedialog.askopenfilename(title=title, filetypes=filetypes, master=self)
        if not path:
            return
