...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/plugins/pyshell.py
# Compiled at: 2019-10-07 21:26:51
# Size of source mod 2**32: 49357 bytes
...
import noval.syntax.lang as lang, noval.syntax.syntax as syntax, noval.util.strutils as strutils, noval.util.utils as utils, noval.python.pyutils as pyutils
from noval.util import hiscache
import re, noval.editor.text as texteditor, tkinter.font as tk_font, six.moves.builtins as builtins
    ...
    def perform_return1111(self, event):
        ...
        try:
            ...
            for context in roughparse.NUM_CONTEXT_LINES:
                ...
                y.set_str(rawtext)
                bod = y.find_good_parse_start(False, roughparse._build_char_in_string_func(startatindex))
...
class ShellText(PythonText):
    ...
    def __init__(self, master, cnf={}, **kw):
        ...
        self.tag_configure('io', lmargin1=io_indent, lmargin2=io_indent, rmargin=io_indent, font='IOFont')
        self.tag_configure('prompt', font='PyShellBoldEditorFont')
        ...
        self.tag_configure('io_hyperlink', underline=False, font=io_hyperlink_font)
    ...
    def _init_menu(self):
        ...
        self._menu.add_command(label='Clear shell', command=self._clear_shell, accelerator=sequence_to_accelerator(clear_seq))
    ...
    def _handle_toplevel_response(self, msg):
        ...
        if value_repr != 'None':
            ...
            self.tag_bind(object_tag, sequence, lambda _: get_workbench().event_generate('ObjectSelect', object_id=msg['value_info'].id))
    ...
    def intercept_delete(self, index1, index2=None, **kw):
        if index1 == 'sel.first' and index2 == 'sel.last' and not self.has_selection():
            return
        if self._editing_allowed() and self._in_current_input_range(index1) and (index2 is None or self._in_current_input_range(index2)):
    ...
    def _insert_text_directly(self, txt, tags=()):
        ...
        if 'stderr' in tags or 'error' in tags:
            for line in txt.splitlines(True):
                parts = re.split('(File .* line \\d+.*)$', line, maxsplit=1)
    ...
    def _code_is_ready_for_submission(self, source, tail=''):
        ...
        if len(lines) > 0:
            ...
            if first_word in compound_keywords and not source.replace(' ', '').replace('\t', '').endswith('\n\n'):
                pass
            return False
        return True
        ...
        if get_runner().is_waiting_toplevel_command():
            ...
            try:
                if cmd_line.startswith('%'):
                    ...
                    get_runner().send_command(ToplevelCommand(command_name, args=argv[1:], args_str=args_str, cmd_line=cmd_line))
                else:
                    if cmd_line.startswith('!'):
                        argv = parse_cmd_line(cmd_line[1:])
                        get_workbench().event_generate('SystemCommand', cmd_line=text_to_be_submitted)
                        get_runner().send_command(ToplevelCommand('execute_system_command', argv=argv, cmd_line=cmd_line))
                    else:
    ...
    def _arrow_down(self, event):
        ...
        if insert_line != last_line:
            return
        if len(self._command_history) == 0 or self._command_history_current_index == len(self._command_history) - 1:
            return 'break'
        if self._command_history_current_index is None:
            self._command_history_current_index = len(self._command_history) - 1
        else:
    ...
    def _handle_hyperlink(self, event):
        try:
            ...
            matches = re.findall('File "([^"]+)", line (\\d+)', line)
...
class PythonInteractiveInterpreter(InteractiveInterpreter):
    __doc__ = 'Interpreter based on code.InteractiveInterpreter.'
    ...
    def push(self, command, astMod=None):
        ...
        if utils.is_py2() and type(command) == unicode:
            try:
...
class PyShell(ttk.Frame):
    ...
    def __init__(self, mater, introText='', locals=None, InterpClass=PythonInteractiveInterpreter, startupScript=None, execStartupScript=True, *args, **kwds):
        ...
        self.fonts = [
         tk_font.Font(name='PyShellEditorFont', family=utils.profile_get(consts.EDITOR_FONT_FAMILY_KEY, default_editor_family)),
         tk_font.Font(name='PyShellBoldEditorFont', family=utils.profile_get(consts.EDITOR_FONT_FAMILY_KEY, default_editor_family), weight='bold')]
        self.text = ShellText(self, font='PyShellEditorFont', highlightthickness=0, borderwidth=0, yscrollcommand=self.vert_scrollbar.set, padx=4, insertwidth=2, height=10, undo=True)
        ...
        self.interp = InterpClass(*args, **kwds)
        self.more = False
    ...
    def prompt(self):
        ...
        if pos > 0:
            ...
            if pstrip[(-1)] == ':' and first_word in ('if', 'else', 'elif', 'for',
                                                      'while', 'def', 'class', 'try',
                                                      'except', 'finally'):
                indent += '    '
            self.write(indent)
    ...
    def addHistory(self, command):
        ...
        self.historyIndex = -1
        if command != '' and (self.history.GetSize() == 0 or command != self.history[0]):
...
class PyshellViewLoader(plugin.Plugin):
    ...
    def Load(self):
        GetApp().MainFrame.AddView(consts.PYTHON_INTERPRETER_VIEW_NAME, PyShell, _('Python Interpreter'), 's', image_file='python/interpreter.ico', default_position_key=1)