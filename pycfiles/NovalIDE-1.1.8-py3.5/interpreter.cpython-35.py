...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/interpreter.py
# Compiled at: 2019-10-17 05:02:06
# Size of source mod 2**32: 21693 bytes
from noval import GetApp
import os, locale, noval.util.apputils as apputils, noval.util.strutils as strutils, six.moves.builtins as builtins, threading
...
import py_compile, noval.util.fileutils as fileutils
...
class PythonInterpreter(BuiltinPythonInterpreter):
    ...
    def __init__(self, name, executable_path, id=None, is_valid_interpreter=False):
        if apputils.is_windows():
            ...
            if os.path.basename(executable_path) == PythonInterpreter.CONSOLE_EXECUTABLE_NAME:
    ...
    def GetPipPath(self):
        ...
        if self.IsV2() or apputils.is_windows():
            pip_path_list = [
             os.path.join(python_location, 'Scripts', pip_name), os.path.join(python_location, pip_name)]
        if self.IsV3():
    ...
    def GetDocPath(self):
        if self._help_path == '' and apputils.is_windows():
            python_location = os.path.dirname(self.Path)
