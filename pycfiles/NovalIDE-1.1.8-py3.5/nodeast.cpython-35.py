...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/parser/nodeast.py
...
class ClassDef(Node):
    ...
    def GetClassMembers(self, sort=True):
        member_list = []
        for child in self.Childs:
            if child.Type == config.NODE_FUNCDEF_TYPE and child.IsClassMethod or child.Type == config.NODE_ASSIGN_TYPE:
                member_list.append(child.Name)
