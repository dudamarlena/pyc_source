...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/editor/text.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 66194 bytes
import tkinter as tk
from tkinter import messagebox, filedialog
from noval import GetApp, _
import noval.core as core, noval.filewatcher as filewatcher, codecs, noval.python.parser.utils as parserutils, noval.consts as consts, os, noval.util.fileutils as fileutils, sys, noval.util.utils as utils, noval.ui_base as ui_base
from tkinter import ttk
import time, noval.menu as tkmenu, noval.syntax.lang as lang, noval.util.strutils as strutils, shutil
from noval.filewatcher import FileEventHandler
import noval.ui_common as ui_common, noval.misc as misc, tkinter.font as tkfont, noval.docposition as docposition, noval.constants as constants, noval.python.pyutils as pyutils, noval.ui_utils as ui_utils, noval.syntax.syntax as syntax, noval.find.findtext as findtext, noval.editor.format as textformat
...
class TextDocument(core.Document):
    ...
    def SaveAs(self):
        ...
        filename = filedialog.asksaveasfilename(master=GetApp(), filetypes=descrs, defaultextension=default_ext, initialdir=os.getcwd(), initialfile=os.path.basename(self.GetFilename()))
        if filename == '':
            return False
        ...
        try:
            ...
            if os.path.exists(filename):
                if not os.access(filename, os.W_OK):
                    messagebox.showerror(msgTitle, "Could not save '%s'.  No write permission to overwrite existing file." % fileutils.get_filename_from_path(filename))
                    return False
...
class TextView(misc.AlarmEventView):
    ...
    def OnCreate(self, doc, flags):
        ...
        self._text_frame = ui_base.TextviewFrame(frame, text_class=self.GetCtrlClass(), font='EditorFont', horizontal_scrollbar_class=ui_common.AutoScrollbar, wrap=tk.NONE, undo=True, use_edit_tester=True, use_edit_image=True)
        ...
        self._text_frame.home_widget = frame
    ...
    def CheckEol(self, value):
        ...
        for cur in range(1, end_line):
            ...
            if txt.endswith(consts.EOL_DIC[consts.EOL_CR]) and txt2.endswith(consts.EOL_DIC[consts.EOL_LF]):
                ...
                if txt2.endswith(consts.EOL_DIC[consts.EOL_LF]) and cur != end_line - 1:
    ...
    def check_for_external_changes(self):
        ...
        if self._alarm_event == FileEventHandler.FILE_MODIFY_EVENT:
            ret = messagebox.askyesno(_('Reload..'), _('File "%s" has already been modified outside,Do You Want to reload it?') % self.GetDocument().GetFilename(), parent=self.GetFrame())
            if ret == True:
                document = self.GetDocument()
                document.OnOpenDocument(document.GetFilename())
        else:
            if self._alarm_event == FileEventHandler.FILE_MOVED_EVENT or self._alarm_event == FileEventHandler.FILE_DELETED_EVENT:
                ret = messagebox.askyesno(_('Keep Document..'), _('File "%s" has already been moved or deleted outside,Do You Want to keep it in Editor?') % self.GetDocument().GetFilename(), parent=self.GetFrame())
                document = self.GetDocument()
                if ret == True:
    ...
    def UpdateUI(self, command_id):
        ...
        if command_id in [constants.ID_INSERT_COMMENT_TEMPLATE, constants.ID_INSERT_DECLARE_ENCODING, constants.ID_UNITTEST, constants.ID_COMMENT_LINES, constants.ID_UNCOMMENT_LINES,
         ...
         constants.ID_CHECK_SYNTAX, constants.ID_SET_PARAMETER_ENVIRONMENT, constants.ID_DEBUG_LAST, constants.ID_START_WITHOUT_DEBUG, constants.ID_AUTO_COMPLETE, constants.ID_TOGGLE_BREAKPOINT]:
            return False
        if command_id == constants.ID_UNDO:
            return self.GetCtrl().CanUndo()
        if command_id == constants.ID_REDO:
            return self.GetCtrl().CanRedo()
        if command_id in [constants.ID_CUT, constants.ID_COPY, constants.ID_CLEAR]:
            return self.GetCtrl().CanCopy()
        if command_id == constants.ID_PASTE:
            return True
        if command_id in [constants.ID_UPPERCASE, constants.ID_LOWERCASE, constants.ID_CLEAN_WHITESPACE, constants.ID_TAB_SPACE, constants.ID_SPACE_TAB]:
            return self.GetCtrl().HasSelection()
        if command_id in [constants.ID_EOL_MAC, constants.ID_EOL_UNIX, constants.ID_EOL_WIN]:
            GetApp().MainFrame.GetNotebook().eol_var.set(self.GetCtrl().eol)
        return True


class TextCtrl(ui_base.TweakableText):
    __doc__ = 'Text widget with extra navigation and editing aids. \n    Provides more comfortable deletion, indentation and deindentation,\n    and undo handling. Not specific to Python code.\n    \n    Most of the code is adapted from idlelib.EditorWindow.\n    use_edit_tester表示文本窗口弹出编辑菜单是否使用主编辑菜单的tester函数,默认为False\n    use_edit_image表示是否使用主编辑菜单的编辑图标,默认为False\n    tag_current_line:表示是否高亮当前行\n    '

    def __init__(self, master=None, style='Text', tag_current_line=True, indent_with_tabs=False, replace_tabs=False, cnf={}, use_edit_tester=False, use_edit_image=False, **kw):
    ...
    def perform_page_down(self, event):
        try:
            ...
            if row == line_count or row == line_count - 1:
        ...
        if event.state & 1 == 0:
            ...
            if not self.index_sel_first():
                ...
                if self.compare(self.index_sel_first(), '<', self.index('insert')):
                    self.mark_set('my_anchor', 'sel.first')
                else:
                    self.mark_set('my_anchor', 'sel.last')
    ...
    def move_to_edge_if_selection(self, edge_index):
        ...
        def move_at_edge(event):
            if self.has_selection() and event.state & 5 == 0:
                try:
    ...
    def _log_keypress_for_undo(self, e):
        ...
        if event_kind != self._last_event_kind or e.char in ('\r', '\n', ' ', '\t') or e.keysym in ('Return',
                                                                                                    'KP_Enter') or time.time() - self._last_key_time > 2:
            self.edit_separator()
    ...
    def CreatePopupMenu(self):
        ...
        if not self._use_edit_tester:
            common_kwargs.update({'tester': default_tester})
        if not self._use_edit_image:
            common_kwargs.update({'image': None})
        self._popup_menu = tkmenu.PopupMenu(self, **misc.get_style_configuration('Menu'))
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_UNDO), handler=GetApp().MainFrame.CreateEditCommandHandler('<<Undo>>'), **common_kwargs)
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_REDO), handler=GetApp().MainFrame.CreateEditCommandHandler('<<Redo>>'), **common_kwargs)
        self._popup_menu.add_separator()
        args = {}
        if not self._use_edit_image:
            args.update({'image': None})
        if not self._use_edit_tester:
            args.update({'tester': self.CanCut})
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_CUT), handler=GetApp().MainFrame.CreateEditCommandHandler('<<Cut>>'), **args)
        if not self._use_edit_tester:
            args.update({'tester': self.CanCopy})
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_COPY), handler=GetApp().MainFrame.CreateEditCommandHandler('<<Copy>>'), **args)
        if not self._use_edit_tester:
            args.update({'tester': self.CanPaste})
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_PASTE), handler=GetApp().MainFrame.CreateEditCommandHandler('<<Paste>>'), **args)
        if not self._use_edit_tester:
            args.update({'tester': self.CanDelete})
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_CLEAR), handler=self.OnDelete, **args)
        sel_args = {}
        if not self._use_edit_tester:
            sel_args['tester'] = None
        self._popup_menu.AppendMenuItem(GetApp().Menubar.GetEditMenu().FindMenuItem(consts.ID_SELECTALL), handler=GetApp().MainFrame.SelectAll, **sel_args)
    ...
    def _insert_untypable_characters_on_windows(self, event):
        if event.state == 131084:
            lang_id = get_keyboard_language()
            char = _windows_altgr_chars_by_lang_id_and_keycode.get(lang_id, {}).get(event.keycode, None)
            if char is not None:
    ...
    def direct_insert(self, index, chars, tags=None, **kw):
        try:
            ...
            GetApp().event_generate('TextInsert', index=concrete_index, text=chars, tags=tags, text_widget=self)
    ...
    def direct_delete(self, index1, index2=None, **kw):
        try:
            ...
            return ui_base.TweakableText.direct_delete(self, index1, index2=index2, **kw)
        finally:
            GetApp().event_generate('TextDelete', index1=concrete_index1, index2=concrete_index2, text_widget=self)
    ...
    def check_convert_tabs_to_spaces(self, chars):
        ...
        if not self.replace_tabs or tab_count == 0:
            ...
            if messagebox.askyesno(_('Convert tabs to spaces?'), _('NovalIDE (according to Python recommendation) uses spaces for indentation, ') + _('but the text you are about to insert/open contains %d tab characters. ') % tab_count + _("To avoid confusion, it's better to convert them into spaces (unless you know they should be kept as tabs).\n\n") + _('Do you want me to replace each tab with %d spaces?\n\n') % self.indent_width, parent=tk._default_root):
