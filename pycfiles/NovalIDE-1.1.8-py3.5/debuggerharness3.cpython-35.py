...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/debugger/debuggerharness3.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 32884 bytes
import bdb, sys, threading, os, types, traceback, inspect
from xml.dom.minidom import getDOMImplementation
import atexit, pickle, bz2
from xmlrpc.server import SimpleXMLRPCServer
import xmlrpc.client as xmlrpclib, queue as Queue, io as cStringIO, importlib
...
class Adb(bdb.Bdb):
    ...
    def user_line(self, frame):
        if self.in_debugger_code(frame):
            self.set_step()
            return
        message = self._Adb__frame2message(frame)
    ...
    def frame2message(self, frame):
        return self._Adb__frame2message(frame)
...
class DebuggerHarness(object):
    ...
    def isPrimitive(self, item):
        tp = type(item)
        return tp is types.IntType or tp is types.LongType or tp is types.FloatType or tp is types.BooleanType or tp is types.ComplexType or tp is types.StringType
    ...
    def addAny(self, top_element, name, item, doc, ply):
        ...
        if tp in DebuggerHarness.cantIntro or ply < 1:
            ...
            if tp is tuple or tp is list:
                ...
                if tp is dict or tp is types.MappingProxyType:
                    self.addDict(top_element, name, item, doc, ply - 1)
                else:
                    if inspect.ismodule(item):
                        self.addModule(top_element, name, item, doc, ply - 1)
                    else:
                        if inspect.isclass(item) or inspect.isclass(item.__class__):
    ...
    def addDictAttr(self, root_node, name, thing, document, ply):
        ...
        self.addDict(dict_node, '', thing.__dict__, document, ply)
