...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/plugins/outline.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 13634 bytes
from noval import GetApp, _
import tkinter as tk
from tkinter import ttk
import noval.ui_base as ui_base, noval.iface as iface, noval.plugin as plugin, noval.consts as consts, noval.imageutils as imageutils, noval.python.parser.config as parserconfig, noval.syntax.lang as lang, noval.util.utils as utils, noval.menu as tkmenu, noval.constants as constants, noval.python.pyeditor as pyeditor, noval.preference as preference, noval.ui_utils as ui_utils
...
class PythonOutlineView(ui_base.OutlineView):
    ...
    def LoadOutLine(self, currView, lineNum=-1):
        ...
        if currView and self.IsValidViewType(currView):
    ...
    def TranverseItem(self, module_analyzer, node, parent=''):
        ...
        for child in node.Childs:
            ...
            if child.Type == parserconfig.NODE_FUNCDEF_TYPE:
                ...
                current = self.tree.insert(parent, 'end', text=display_name, values=(child.Line, child.Col, child.Type), image=self.func_image)
            elif child.Type == parserconfig.NODE_CLASSDEF_TYPE:
                if self._display_item_flag & self.DISPLAY_ITEM_CLASS_BASE and len(child.Bases) > 0:
                ...
                if self._display_item_flag & self.DISPLAY_ITEM_LINE:
                    display_name = '%s[%d]' % (display_name, child.Line)
                if child.Type == parserconfig.NODE_CLASS_PROPERTY or child.Type == parserconfig.NODE_ASSIGN_TYPE:
                    current = self.tree.insert(parent, 'end', text=display_name, values=(child.Line, child.Col, child.Type), image=self.property_image)
                else:
                    if child.Type == parserconfig.NODE_IMPORT_TYPE:
                        display_name = child.Name
                        if child.AsName is not None:
                            display_name = child.AsName
                        if self._display_item_flag & self.DISPLAY_ITEM_LINE:
                            display_name = '%s[%d]' % (display_name, child.Line)
                        current = self.tree.insert(parent, 'end', text=display_name, values=(child.Line, child.Col, child.Type), image=self.import_image)
                    else:
                        if child.Type == parserconfig.NODE_FROMIMPORT_TYPE:
                            current = self.tree.insert(parent, 'end', text=display_name, values=(child.Line, child.Col, child.Type), image=self.from_import_image)
                            for node_import in child.Childs:
                                name = node_import.Name
                                if node_import.AsName is not None:
                                    name = node_import.AsName
                                current = self.tree.insert(current, 'end', text=name, values=(child.Line, child.Col, child.Type), image=self.import_image)

                        elif child.Type == parserconfig.NODE_MAIN_FUNCTION_TYPE:
                            current = self.tree.insert(parent, 'end', text=display_name, values=(child.Line, child.Col, child.Type), image=self.mainfunction_image)
...
def OutlineSort(outline_sort_id):
    ...
    if outline_sort_id == constants.ID_SORT_BY_LINE:
        ...
        if outline_sort_id == constants.ID_SORT_BY_TYPE:
...
class PythonOutlineViewLoader(plugin.Plugin):
    ...
    def Load(self):
        ...
        GetApp().AddMenuCommand(constants.ID_SORT_BY_NONE, outline_menu, _('Unsorted'), lambda : OutlineSort(constants.ID_SORT_BY_NONE), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, variable=self.outline_sort_var, value=ui_base.OutlineView.SORT_BY_NONE)
        GetApp().AddMenuCommand(constants.ID_SORT_BY_LINE, outline_menu, _('Sort By Line'), lambda : OutlineSort(constants.ID_SORT_BY_LINE), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, variable=self.outline_sort_var, value=ui_base.OutlineView.SORT_BY_LINE)
        GetApp().AddMenuCommand(constants.ID_SORT_BY_TYPE, outline_menu, _('Sort By Type'), lambda : OutlineSort(constants.ID_SORT_BY_TYPE), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, variable=self.outline_sort_var, value=ui_base.OutlineView.SORT_BY_TYPE)
        GetApp().AddMenuCommand(constants.ID_SORT_BY_NAME, outline_menu, _('Sort By Name(A-Z)'), lambda : OutlineSort(constants.ID_SORT_BY_NAME), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, variable=self.outline_sort_var, value=ui_base.OutlineView.SORT_BY_NAME)
