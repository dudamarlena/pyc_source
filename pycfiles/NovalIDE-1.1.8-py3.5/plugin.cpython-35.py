...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/plugin.py
# Compiled at: 2019-10-17 05:35:18
# Size of source mod 2**32: 30727 bytes
from noval import GetApp, _
import os, sys, shutil, noval.util.strutils as strutils, noval.python.parser.utils as parserutils, noval.util.utils as utils, noval.consts as consts, six, pkg_resources
try:
...
class Interface(object):
    __doc__ = 'Base class for defining interfaces. Interface classes are\n    used to define the method/contract from which the plugin must\n    conform to.\n    \n    '


class ExtensionPoint(property):
    __doc__ = 'Declares what L{Interface} a plugin is extending'
...
class PluginMeta(type):
    ...
    def __new__(mcs, name, bases, d):
        ...
        init = d.get('__init__')
        if not init:
            for init in [b.__init__._original for b in new_obj.mro() if issubclass(b, Plugin) and '__init__' in b.__dict__]:
                break
...
class Plugin(six.with_metaclass(PluginMeta, object)):
    __doc__ = 'Base class for all plugin type objects'
...
class PluginData(object):
    __doc__ = 'A storage class for representing data about a Plugin\n    @see: L{Plugin}\n\n    '
...
class PluginManager(object):
    ...
    def __init__(self, pi_path=None):
        ...
        self.cfg = None
        self._pi_path = []
        self._config = self.LoadPluginConfig()
        if not pi_path:
    ...
    def CreateEnvironment(self, path):
        ...
        if pkg_resources != None:
            if utils.is_py2():
                paths = [strutils.EncodeString(pname, sys.getfilesystemencoding()) for pname in path]
                        ...
                        try:
                            if cls not in self._pdata:
                                ...
                                if not minv or not parserutils.CompareCommonVersion(minv, utils.get_app_version()):
                                    ...
                                    pdata = PluginData(egg.project_name, desc.strip(), auth.strip(), egg.version)
                                    pdata.SetDist(egg)
    ...
    def RefreshConfig(self):
        ...
        """
        plugins = [plugin.GetInstance().__module__ for plugin in self._pdata.values()]
        config = dict()
