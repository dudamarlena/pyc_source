...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/debugger/commandui.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 47058 bytes
...
try:
    ...
    import xmlrpclib, Queue, SocketServer
except ImportError:
    import xmlrpc.client as xmlrpclib, queue as Queue, socketserver as SocketServer
    from xmlrpc.server import SimpleXMLRPCServer

import os, threading, types
from xml.dom.minidom import parse, parseString
import bz2, shutil, noval.python.interpreter.interpreter as pythoninterpreter, noval.python.parser.utils as parserutils, copy, noval.util.appdirs as appdirs, noval.util.utils as utils, noval.python.project.runconfig as runconfig, noval.python.debugger.watchs as watchs, pickle, noval.util.timer as pytimer, noval.constants as constants, noval.ui_base as ui_base, noval.util.fileutils as fileutils, noval.consts as consts
...
class BaseDebuggerUI(RunCommandUI):
    ...
    def DebuggerInWait():
        for debugger in BaseDebuggerUI.debuggers:
            if debugger._executor and debugger._callback._waiting:
                return True
    ...
    def DebuggerPastAutoContinue():
        for debugger in BaseDebuggerUI.debuggers:
            if debugger._executor and debugger._callback._waiting and not debugger._callback._autoContinue:
                return True
...
class PythonDebuggerUI(BaseDebuggerUI):
    ...
    def CreateExecutor(self):
        ...
        self._executor = PythonDebuggerExecutor(path, self._run_parameter, self, self._debuggerHost, self._debuggerPort, self._debuggerBreakPort, self._guiHost, self._guiPort, self._run_parameter.FilePath, callbackOnExit=self.ExecutorFinished)
...
class RequestHandlerThread(threading.Thread):
    ...
    def __init__(self, debuggerUI, queue, address):
        ...
        self._input_text = ''
        if _VERBOSE:
            print('RequestHandlerThread on fileno %s' % str(self._server.fileno()))
    ...
    def get_input_text(self):
        text = tkSimpleDialog.askstring(_('Enter input'), _('Enter the input text:'))
        if text:
