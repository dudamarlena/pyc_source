...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/interpreterconfigruation.py
# Compiled at: 2019-10-17 04:12:34
# Size of source mod 2**32: 30350 bytes
from noval import NewId, _, GetApp
import tkinter as tk
from tkinter import ttk
from tkinter import messagebox, filedialog
import noval.python.interpreter.interpreter as pythoninterpreter, intellisence, noval.util.apputils as sysutils, os, noval.python.interpreter.pythonbuiltins as pythonbuiltins, noval.python.interpreter.environment as pythonenvironment, noval.python.interpreter.pythonpackages as pythonpackages, noval.python.interpreter.pythonpath as pythonpath, noval.python.interpreter.interpretermanager as interpretermanager, threading, noval.outputthread as outputthread, subprocess, noval.util.which as whichpath, noval.util.strutils as strutils, noval.util.fileutils as fileutils, noval.util.utils as utils, noval.ui_base as ui_base, noval.imageutils as imageutils, noval.consts as consts, noval.ttkwidgets.treeviewframe as treeviewframe, noval.misc as misc, noval.constants as constants, noval.menu as tkmenu, noval.ui_utils as ui_utils, noval.preference as preference
...
class NewVirtualEnvProgressDialog(ui_base.GenericProgressDialog):
    ...
    def __init__(self, parent):
        ui_base.GenericProgressDialog.__init__(self, parent, _('New Virtual Env'), mode='indeterminate', info=_('Please wait a minute for end New Virtual Env'))
        self.KeepGoing = True
    ...
    def ChooseExecutablePath(self):
        if sysutils.is_windows():
            ...
            descrs = [
             (
              _('All Files'), '.*')]
        path = filedialog.askopenfilename(master=self, filetypes=descrs, title=_('Select Executable Path'))
        if not path:
            return
        self.path_var.set(fileutils.opj(path))
    ...
    def _ok(self, event=None):
        ...
        if not self._id_modify_dlg:
            if 0 == len(self.path_var.get()):
                ...
                return
            if not os.path.exists(self.path_ctrl.get()):
...
class InterpreterConfigurationPanel(ui_utils.BaseConfigurationPanel):
    ...
    def __init__(self, parent):
        ...
        self.listview = treeviewframe.TreeViewFrame(row, columns=columns, displaycolumns=(1,
                                                                                          2,
                                                                                          3,
                                                                                          4), show='headings', height=7, borderwidth=1, relief='solid')
    ...
    def ProcessEvent(self, id):
        ...
        if id == ID_COPY_INTERPRETER_NAME:
            ...
            return True
        if id == ID_COPY_INTERPRETER_VERSION:
            sysutils.CopyToClipboard(interpreter.Version)
            return True
        if id == ID_COPY_INTERPRETER_PATH:
            sysutils.CopyToClipboard(interpreter.Path)
            return True
        if id == ID_MODIFY_INTERPRETER_NAME:
            self.ModifyInterpreterNameDlg()
            return True
        if id == ID_REMOVE_INTERPRETER:
            self.RemoveInterpreter()
            return True
        if id == ID_NEW_INTERPRETER_VIRTUALENV:
            ...
            if status == constants.ID_OK:
                ...
                return True
        if id == ID_GOTO_INTERPRETER_PATH:
    ...
    def CreatePythonVirtualEnv(self, name, location, include_site_packages, interpreter, progress_dlg):
        ...
        if 'virtualenv' not in interpreter.Packages:
...
class AnalyseProgressDialog(ui_base.GenericProgressDialog):
    ...
    def __init__(self, parent):
        ui_base.GenericProgressDialog.__init__(self, parent, _('Interpreter Smart Analyse'), mode='indeterminate', info=_('Please wait a minute for end analysing'))
        self.KeepGoing = True
        self.mpb.start()
