...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/environment.py
# Compiled at: 2019-08-16 02:55:35
# Size of source mod 2**32: 3727 bytes
from noval import _
import tkinter as tk
from tkinter import ttk, messagebox
import noval.ui_base as ui_base, os, noval.ttkwidgets.linklabel as linklabel, noval.ui_common as ui_common, noval.ui_utils as ui_utils, noval.ttkwidgets.treeviewframe as treeviewframe
...
class EnvironmentPanel(ui_utils.BaseEnvironmentUI):
    ...
    def IsEnvironChanged(self, dct):
        ...
        for name in dct:
            if not self.interpreter.Environ.Exist(name):
                return True
            if dct[name] != self.interpreter.Environ[name]:
                return True
    ...
    def CheckAndRemoveKeyitem(self, key):
        if not ui_utils.BaseEnvironmentUI.CheckAndRemoveKeyitem(self, key):
            return False
        if self.include_chkvar.get() and key.upper() in os.environ:
