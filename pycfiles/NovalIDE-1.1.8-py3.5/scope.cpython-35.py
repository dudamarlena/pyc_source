...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/parser/scope.py
# Compiled at: 2019-08-20 20:31:02
# Size of source mod 2**32: 21087 bytes
import config, nodeast
from utils import CmpMember, py_sorted
import intellisence, noval.util.utils as logger_utils
...
class ModuleScope(Scope):
    ...
    def MakeScopes(self, node, parent_scope):
        for child in node.Childs:
            if child.Type == config.NODE_FUNCDEF_TYPE or type(child) == nodeast.FuncDef:
            ...
            elif child.Type == config.NODE_CLASS_PROPERTY or child.Type == config.NODE_ASSIGN_TYPE:
                ...
                if child.Type == config.NODE_MAIN_FUNCTION_TYPE:
                    MainFunctionScope(child, parent_scope, self)
                else:
                    if child.Type == config.NODE_RETURN_TYPE:
    ...
    def GetClassMemberList(self, sort=True):
        ...
        for base in self.Node.Bases:
            ...
            if base_scope is not None:
                if base_scope.Node.Type == config.NODE_IMPORT_TYPE or base_scope.Node.Type == config.NODE_BUILTIN_IMPORT_TYPE:
                    member_list.extend(base_scope.GetImportMemberList(base))
                else:
