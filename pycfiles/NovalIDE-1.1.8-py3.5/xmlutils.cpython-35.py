...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/util/xmlutils.py
# Compiled at: 2019-08-16 02:55:35
# Size of source mod 2**32: 23488 bytes
import os, time, urllib, logging, noval.util.objutils as objutils, noval.util.xmlmarshaller as xmlmarshaller, noval.util.logger as logger
from noval.consts import PROJECT_NAMESPACE_URL
import noval.util.compat as compat, noval.util.utils as utils
...
def addNSAttribute(xmlDoc, shortNamespace, longNamespace):
    if not hasattr(xmlDoc, '__xmlnamespaces__'):
        ...
        if hasattr(xmlDoc.__class__, '__xmlnamespaces__') and xmlDoc.__xmlnamespaces__ is xmlDoc.__class__.__xmlnamespaces__:
...
def genShortNS(xmlDoc, longNamespace=None):
    if not hasattr(xmlDoc, '__xmlnamespaces__'):
        return 'ns1'
    if longNamespace != None and longNamespace in xmlDoc.__xmlnamespaces__.items():
...
def getAgVersion(fileName):
    ...
    if i2 >= 0:
        ...
        if xml.find('<project version="10"') >= 0:
            return '10'
        else:
            return
    version = None
...
def getAgXsdToClassName():
    ...
    if agXsdToClassName == None:
        agXsdToClassName = {'ag:append': 'activegrid.model.processmodel.AppendOperation', 
...
def getAgKnownTypes():
    ...
    if agKnownTypes == None:
        try:
            tmpAgKnownTypes = {}
            import activegrid.model.processmodel, activegrid.model.schema, activegrid.server.deployment, activegrid.model.wsdl, activegrid.data.dataservice
        ...
        if len(agKnownTypes) == 0:
            noKnownNamespaces = {}
            return (agKnownTypes, noKnownNamespaces)
        return (
         agKnownTypes, KNOWN_NAMESPACES)