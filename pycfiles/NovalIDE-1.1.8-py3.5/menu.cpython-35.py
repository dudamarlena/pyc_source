...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/menu.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 30340 bytes
...
import noval.util.utils as utils, copy, os, noval.misc as misc
MenuItem = collections.namedtuple('MenuItem', ['id', 'label', 'accelerator', 'image', 'tester'])

class KeyBinder(object):
    __doc__ = 'Class for managing keybinding configurations'
    cprofile = None
    key_binds = copy.copy(DEFAULT_KEY_BINDS)
...
class PopupMenu(tk.Menu):
    __doc__ = 'Custom wxMenu class that makes it easier to customize and access items.\n\n    '
    ...
    def GetMenuData(self, id_, text, handler, img, accelerator, kind, variable, tester):
        ...
        if (kind == consts.CHECK_MENU_ITEM_KIND or kind == consts.RADIO_MENU_ITEM_KIND) and variable is not None:
            kwargs.update(dict(variable=variable))
        return (
         menu_item, kwargs)

    def Append(self, id_, text, helpstr='', handler=None, img=None, accelerator=None, kind=consts.NORMAL_MENU_ITEM_KIND, variable=None, tester=None, **extra_args):
        """Append a MenuItem
        ...
        if kind == consts.NORMAL_MENU_ITEM_KIND:
            ...
            if kind == consts.CHECK_MENU_ITEM_KIND:
    ...
    def InsertMenu(self, pos, id_, text, menu):
        ...
        self.insert(pos, 'cascade', label=text, menu=menu)
    ...
    def Insert(self, pos, id_, text, helpstr='', handler=None, img=None, accelerator=None, kind=consts.NORMAL_MENU_ITEM_KIND, variable=None, tester=None):
        ...
        self.insert(pos, 'checkbutton' if kind == consts.CHECK_MENU_ITEM_KIND else 'command', **kwargs)
        return menu_item

    def InsertAfter(self, item_id, id_, text, helpstr='', handler=None, img=None, accelerator=None, kind=consts.NORMAL_MENU_ITEM_KIND, variable=None, tester=None):
    ...
    def InsertBefore(self, item_id, id_, text, helpstr='', handler=None, img=None, accelerator=None, kind=consts.NORMAL_MENU_ITEM_KIND, variable=None, tester=None):
    ...
    def InsertAlpha(self, id_, label='', helpstr='', after=0, use_bmp=True):
...
class CustomMenubar(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        ttk.Style().map('CustomMenubarLabel.TLabel', background=[
         (
          '!active', misc.lookup_style_option('Menubar', 'background', 'gray')),
         (
          'active',
          misc.lookup_style_option('Menubar', 'activebackground', 'LightYellow'))], foreground=[
         (
    ...
    def add_cascade(self, label, menu):
        label_widget = ttk.Label(self, style='CustomMenubarLabel.TLabel', text=label, padding=[
         6, 3, 6, 2], font='TkDefaultFont')
        ...
        def click(event):
            try:
                ...
                menu.tk_popup(label_widget.winfo_rootx(), label_widget.winfo_rooty() + label_widget.winfo_height())
            finally:
                self._opened_menu = None
...
class MenubarMixin:
    __doc__ = 'Custom menubar to allow for easier access and updating\n    of menu components.\n    @todo: redo all of this\n\n    '
    keybinder = KeyBinder()
