...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/core.py
# Compiled at: 2019-10-17 01:45:10
# Size of source mod 2**32: 82627 bytes
from __future__ import print_function
from noval import GetApp, _, NewId
import tkinter as tk
from tkinter import filedialog, messagebox, ttk
import tempfile, mmap, noval, sys, noval.util.apputils as apputils, noval.util.strutils as strutils, noval.util.check as check, noval.util.timer as timer, pickle, time, noval.util.utils as utils, os, noval.util.fileutils as fileutils, noval.consts as consts, noval.ui_base as ui_base, shutil, noval.newTkDnD as newTkDnD, noval.ui_utils as ui_utils
if utils.is_py2():
...
class App(tk.Tk):
    ...
    def OnInit(self):
        ...
        if not hasattr(self, '_debug'):
            self._debug = False
        if not hasattr(self, '_singleInstance'):
            self._singleInstance = True
        ...
        if self._singleInstance:
            ...
            if self._singleInstanceChecker.IsAnotherRunning():
                ...
                while True:
                    ...
                    if marker == '\x00' or marker == ord('\x00') or marker == '*' or marker == ord('*'):
                        self._sharedMemory.seek(0)
                        if type(marker) == int:
                            self._sharedMemory.write_byte(ord('-'))
    ...
    def DoBackgroundListenAndLoad(self):
        ...
        if byte == '+' or byte == ord('+'):
            data = self._sharedMemory.read(1023)
            self._sharedMemory.seek(0)
            if utils.is_py2():
                self._sharedMemory.write_byte('*')
        else:
...
class DocManager(object):
    ...
    def SelectDocumentPath(self, templates, flags, save):
        ...
        paths = filedialog.askopenfilename(master=GetApp(), filetypes=descrs, multiple=True)
        path_templates = []
        if paths:
            for path in paths:
    ...
    def CreateDocument(self, path, flags=0):
        ...
        if len(self.GetDocuments()) >= self._maxDocsOpen:
            ...
            return self.CloseDocument(doc, False) or []
        if flags & DOC_NEW:
    ...
    def CreateView(self, doc, flags=0):
        ...
        templates = []
        for temp in self._templates:
            if temp.IsVisible() and temp.GetDocumentName() == doc.GetDocumentName():
                templates.append(temp)
    ...
    def SelectViewType(self, temps, sort=False):
        ...
        strings = []
        for temp in temps:
            if temp.IsVisible() and temp.GetViewTypeName() and temp.GetViewName() not in strings:
                templates.append(temp)
        ...
        if sort:
            ...
            templates.sort(tempcmp)
        res = wx.GetSingleChoiceIndex(_('Select a document view:'), _('Views'), strings, self.FindSuitableParent())
        if res == -1:
            return
        return templates[res]
...
class Document(object):
    ...
    def SaveAs(self):
        ...
        if not docTemplate:
            return False
        filename = filedialog.asksaveasfilename(master=GetApp(), filetypes=[
         strutils.get_template_filter(docTemplate)], defaultextension=docTemplate.GetDefaultExtension(), initialdir=docTemplate.GetDirectory(), initialfile=os.path.basename(self.GetFilename()))
        if filename == '':
            return False
    ...
    def OnOpenDocument(self, filename):
        ...
        try:
            ...
            if fileObject:
                fileObject.close()
            wx.MessageBox("Could not open '%s'.  %s" % (FileNameFromPath(filename), sys.exc_value), msgTitle, wx.OK | wx.ICON_EXCLAMATION, self.GetDocumentWindow())
            return False
    ...
    def AddView(self, view):
        ...
        """
        if view not in self._documentViews:
    ...
    def OnChangedViewList(self):
        ...
        """
        if len(self._documentViews) == 0 and self.OnSaveModified():
            pass
