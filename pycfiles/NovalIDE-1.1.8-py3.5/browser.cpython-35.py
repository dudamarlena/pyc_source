...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/project/browser.py
...
import noval.consts as consts, os, noval.python.project.viewer as projectviewer, noval.python.project.rundocument as runprojectdocument
...
class ProjectBrowser(BaseProjectbrowser):
    ...
    def GetProjectTreectrl(self, **tree_kw):
        return PythonProjectTreeCtrl(self, yscrollcommand=self.vert_scrollbar.set, **tree_kw)
    ...
    def ProcessEvent(self, id):
        if id == constants.ID_SET_PROJECT_STARTUP_FILE:
            ...
            if id == constants.ID_RUN:
                self.Run()
                return True
            if id == constants.ID_DEBUG:
                self.DebugRun()
                return True
            if id == constants.ID_BREAK_INTO_DEBUGGER:
                self.BreakintoDebugger()
                return True
            if id == constants.ID_ADD_PACKAGE_FOLDER:
    ...
    def _InitCommands(self):
        ...
        GetApp().InsertCommand(constants.ID_ADD_FOLDER, constants.ID_ADD_PACKAGE_FOLDER, _('&Project'), _('New Package Folder'), image=GetApp().GetImage('project/python/package.png'), handler=lambda : self.ProcessEvent(constants.ID_ADD_PACKAGE_FOLDER), tester=lambda : self.GetView().UpdateUI(constants.ID_ADD_PACKAGE_FOLDER))
