...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/project/pages/debugrun.py
# Compiled at: 2019-10-17 01:45:10
# Size of source mod 2**32: 38022 bytes
from noval import _, GetApp
import tkinter as tk
from tkinter import ttk, filedialog, messagebox
import os, noval.util.fileutils as fileutils, noval.python.interpreter.interpretermanager as interpretermanager, noval.project.variables as variablesutils, noval.python.project.runconfiguration as runconfiguration
from noval.util import utils
import noval.consts as consts, noval.ui_utils as ui_utils, noval.project.property as projectproperty, noval.ui_base as ui_base, noval.imageutils as imageutils, noval.editor.text as texteditor, noval.ttkwidgets.treeviewframe as treeviewframe, noval.ttkwidgets.listboxframe as listboxframe, noval.ttkwidgets.textframe as textframe, noval.python.pyutils as pyutils, noval.constants as constants, copy, noval.ui_utils as ui_utils
...
class StartupPage(BasePage):
    ...
    def __init__(self, parent, run_configuration):
        ...
        if self.MainModuleFile is not None:
            main_module_path = self.ProjectDocument.GetModel().GetRelativePath(self.MainModuleFile)
            main_module_path = os.path.join(variablesutils.FormatVariableName(variablesutils.PROJECT_DIR_VARIABLE), main_module_path)
    ...
    def BrowseMainModule(self):
        ...
        if dlg.ShowModal() == constants.ID_OK:
            main_module_path = os.path.join(variablesutils.FormatVariableName(variablesutils.PROJECT_DIR_VARIABLE), self.ProjectDocument.GetModel().GetRelativePath(dlg.module_file))
            self.main_module_var.set(main_module_path)
    ...
    def GetConfiguration(self):
        return runconfiguration.StartupConfiguration(self.ProjectDocument, self.MainModuleFile, self._startup_path_pattern, self.other_var.get().strip())
...
class ArgumentsPage(BasePage):
    ...
    def GetConfiguration(self):
        ...
        return runconfiguration.AugumentsConfiguration(self.ProjectDocument, main_module_file, self.interpreter_option_textctrl.GetValue(), self.program_argument_textctrl.GetValue())
...
class InterpreterConfigurationPage(BasePage):
    ...
    def GetConfiguration(self):
        main_module_file = self.master.master.master.GetMainModuleFile()
        return runconfiguration.InterpreterConfiguration(self.ProjectDocument, main_module_file, self.interpreter_var.get().strip())
...
class EnvironmentPage(BasePage, ui_utils.BaseEnvironmentUI):
    ...
    def GetConfiguration(self):
        ...
        main_module_file = self.master.master.master.GetMainModuleFile()
        return runconfiguration.EnvironmentConfiguration(self.ProjectDocument, main_module_file, environ)
...
class RunConfigurationDialog(ui_base.CommonModaldialog):
    ...
    def _ok(self):
        ...
        args = {runconfiguration.StartupConfiguration.CONFIGURATION_NAME: self.startup_panel.GetConfiguration(), 
...
class DebugRunPanel(ui_utils.BaseConfigurationPanel):
    ...
    def IsPythonFile(self):
        if self.select_project_file is not None and not fileutils.is_python_file(self.select_project_file):
            return False
        return True
    ...
    def SetStartupFile(self):
        ...
        if constants.ID_OK == dlg.ShowModal():
            startup_path = os.path.join(variablesutils.FormatVariableName(variablesutils.PROJECT_DIR_VARIABLE), self.current_project_document.GetModel().GetRelativePath(dlg.module_file))
