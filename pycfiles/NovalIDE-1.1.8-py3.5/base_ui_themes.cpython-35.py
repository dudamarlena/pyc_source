...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/plugins/base_ui_themes.py
# Compiled at: 2019-10-07 21:21:23
# Size of source mod 2**32: 23349 bytes
from noval import GetApp
import noval.consts as consts
from tkinter import ttk
import noval.iface as iface, noval.plugin as plugin, noval.util.utils as utils
...
def _treeview_settings():
    ...
    return {'Treeview': {'configure': {'font': 'TreeviewFont'}, 
                  'map': {'background': [
                                         (
...
def _menubutton_settings():
    return {'TMenubutton': {'configure': {'padding': scale(14)}, 
                     'layout': [
                                (
...
def _text_settings():
    return {'Text': {'configure': {'background': 'SystemWindow' if utils.is_windows() else 'white', 
...
def _button_notebook_settings():
    return {'closebutton': {'element create': (
                                        'image',
                                        'img_close',
     ...
     'ButtonNotebook.TNotebook.Tab': {'layout': [
                                                 (
                                                  'Notebook.tab',
                                                  {'sticky': 'nswe', 
                                                   'children': [
...
def clam():
    ...
    return {'.': {'configure': {'background': frame, 
           ...
           'map': {'background': [('disabled', frame), ('active', lighter)], 
     ...
     'Toolbutton': {'configure': {'anchor': 'center', 'padding': scale(2), 'relief': 'flat'}, 
                    'map': {'relief': [
                                       ('disabled', 'flat'),
                                       ('selected', 'sunken'),
                                       ('pressed', 'sunken'),
     ...
     'TCheckbutton': {'configure': {'indicatorbackground': '#ffffff', 
                                    'indicatormargin': [scale(1), scale(1), scale(6), scale(1)], 
                                    'padding': scale(2)}, 
                      
                      'map': {'indicatorbackground': [
                                                      (
                                                       'pressed', frame),
                                                      (
     ...
     'TRadiobutton': {'configure': {'indicatorbackground': '#ffffff', 
                                    'indicatormargin': [scale(1), scale(1), scale(6), scale(1)], 
                                    'padding': scale(2)}, 
     ...
     'TEntry': {'configure': {'padding': scale(1), 'insertwidth': scale(1)}, 
                'map': {'background': [('readonly', frame)], 
                        'bordercolor': [('focus', selectbg)], 
                        'lightcolor': [('focus', '#6f9dc6')], 
                        'darkcolor': [('focus', '#6f9dc6')]}}, 
     
     'TCombobox': {'configure': {'padding': [scale(4), scale(2), scale(2), scale(2)], 
                                 'insertwidth': scale(1)}, 
                   
                   'map': {'background': [('active', lighter), ('pressed', lighter)], 
                           'fieldbackground': [
                                               (
     ...
     'TSpinbox': {'configure': {'arrowsize': scale(10), 
                                'padding': [scale(2), 0, scale(10), 0]}, 
                  
                  'map': {'background': [('readonly', frame)], 
                          'arrowcolor': [('disabled', disabledfg)]}}, 
     
     'TNotebook.Tab': {'configure': {'padding': [scale(6), scale(2), scale(6), scale(2)]}, 
                       'map': {'padding': [('selected', [scale(6), scale(4), scale(6), scale(4)])], 
                               'background': [('selected', frame), ('', darker)], 
                               'lightcolor': [('selected', lighter), ('', dark)]}}, 
     
     'Treeview': {'configure': {'background': window}, 
                  'map': {'background': [
                                         (
                                          'disabled', frame),
                                         (
...
def xpnative():
    return {'.': {'configure': {'background': 'SystemButtonFace', 
     ...
     'TEntry': {'configure': {'padding': [scale(2), scale(2), scale(2), scale(4)]}, 
                'map': {'selectbackground': [('!focus', 'SystemWindow')], 
                        'selectforeground': [('!focus', 'SystemWindowText')]}}, 
     
     'TCombobox': {'configure': {'padding': scale(2)}, 
                   'map': {'selectbackground': [('!focus', 'SystemWindow')], 
                           'selectforeground': [('!focus', 'SystemWindowText')], 
                           'foreground': [
     ...
     'TSpinbox': {'configure': {'padding': [scale(2), 0, scale(14), 0]}, 
                  'map': {'selectbackground': [('!focus', 'SystemWindow')], 
                          'selectforeground': [('!focus', 'SystemWindowText')]}}, 
     
     'TNotebook': {'configure': {'tabmargins': [scale(2), scale(2), scale(2), 0]}}, 
     'TNotebook.Tab': {'map': {'expand': [('selected', [scale(2), scale(2), scale(2), scale(2)])]}}, 
     
     'Treeview': {'configure': {'background': 'SystemWindow'}, 
                  'map': {'background': [
     ...
     'Heading': {'configure': {'font': 'TkHeadingFont', 'relief': 'raised'}}, 
     
     'TLabelframe.Label': {'configure': {'foreground': '#0046d5'}}}


def aqua():
    return {'.': {'configure': {'font': 'TkDefaultFont', 
     ...
     'TNotebook': {'configure': {'tabmargins': [10, 0], 
                                 'tabposition': 'n', 
                                 'padding': [18, 8, 18, 17]}}, 
     ...
     'Heading': {'configure': {'font': 'TkHeadingFont'}}, 
     'Treeview': {'configure': {'rowheight': 18, 'background': 'white'}, 
                  'map': {'background': [
...
def windows():
    return [
     ...
     {'TNotebook': {'configure': {'tabmargins': [scale(2), scale(2), scale(2), scale(2)]}}, 
      
      'Tab': {'configure': {'padding': [scale(3), scale(1), scale(3), 0]}}, 
      'ButtonNotebook.TNotebook.Tab': {'configure': {'padding': (scale(4), scale(1), scale(1), 0)}}, 
      
      'TCombobox': {'map': {'selectbackground': [
      ...
      'Listbox': {'configure': {'background': 'SystemWindow', 
                                'foreground': 'SystemWindowText', 
      ...
      'ViewBody.TFrame': {'configure': {'background': 'SystemButtonFace'}}, 
      
      'ViewToolbar.TFrame': {'configure': {'background': 'SystemWindow'}}, 
      'ViewToolbar.Toolbutton': {'configure': {'background': 'SystemWindow'}}, 
      'ViewTab.TLabel': {'configure': {'background': 'SystemWindow', 'padding': [scale(5), 0]}}, 
      
      'ViewToolbar.TLabel': {'configure': {'background': 'SystemWindow', 'padding': [scale(5), 0]}}, 
      
      'Active.ViewTab.TLabel': {'configure': {'relief': 'sunken', 
                                              'borderwidth': scale(1)}}, 
...
def enhanced_clam():
    return [
     ...
     _label_settings(),
     _button_notebook_settings(),
     {'ButtonNotebook.Tab': {'configure': {'padding': (scale(6), scale(4), scale(2), scale(3))}}, 
      
      'TScrollbar': {'configure': {'gripcount': 0, 
                                   'arrowsize': scale(14)}}, 
      
      'TCombobox': {'configure': {'arrowsize': scale(14)}, 
                    'map': {'selectbackground': [('readonly', '!focus', '#dcdad5')], 
...
def load_plugin():
    ...
    for name in original_themes:
        ...
        if name == consts.CLAM_UI_THEME_NAME:
            ...
            if name == consts.XPNATIVE_UI_THEME_NAME:
                settings = xpnative
            elif name == consts.AQUA_UI_THEME_NAME:
                settings = aqua
        GetApp().add_ui_theme(name, None, settings)

    GetApp().add_ui_theme(consts.ENHANCED_CLAM_UI_THEME_NAME, consts.CLAM_UI_THEME_NAME, enhanced_clam, {'tab-close': 'tab-close-clam', 'tab-close-active': 'tab-close-active-clam'})
