...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/pythonpackages.py
# Compiled at: 2019-10-17 03:55:52
# Size of source mod 2**32: 33253 bytes
from noval import _, GetApp
from tkinter import ttk
from tkinter import messagebox, filedialog
import tkinter as tk, noval.ui_base as ui_base, noval.python.interpreter.interpretermanager as interpretermanager, os, subprocess, noval.outputthread as outputthread, threading, noval.util.strutils as strutils, noval.util.apputils as sysutils, noval.util.utils as utils, time
try:
    from urlparse import urlparse
except ImportError:
    from urllib.parse import urlparse

import noval.consts as consts, noval.ttkwidgets.treeviewframe as treeviewframe, noval.editor.text as texteditor, noval.util.urlutils as urlutils, noval.python.parser.utils as parserutils, noval.constants as constants, noval.ttkwidgets.textframe as textframe, noval.util.compat as compat, noval.util.fileutils as fileutils
...
class CommonManagePackagesDialog(ui_base.CommonModaldialog):
    ...
    def BrowsePath(self):
        ...
        path = filedialog.askopenfilename(master=self, title=title, filetypes=descrs, initialfile='requirements.txt')
        if not path:
            return
        self.GetInterpreter()
...
class InstallPackagesDialog(CommonManagePackagesDialog):
    ...
    def __init__(self, parent, interpreter, interpreters=interpretermanager.InterpreterManager().interpreters, pkg_name='', install_args='', autorun=False, install_update=False, call_back=None):
        ...
        self.CreateWidgets(1, _('We will download and install it in the interpreter:'), _('Type the name of package or args to install:'), _('To install the specific version,type "xxx==1.0.1"\nTo install more packages,please specific the path of requirements.txt'))
...
class UninstallPackagesDialog(CommonManagePackagesDialog):
    ...
    def __init__(self, parent, interpreter, interpreters=interpretermanager.InterpreterManager().interpreters, pkg_name='', uninstall_args='', autorun=False, call_back=None):
        ...
        self.CreateWidgets(0, _('We will uninstall it in the interpreter:'), _('Type the name of package or args to uninstall:'), _('To uninstall more packages,please specific the path of requirements.txt'))
        self.auto_run()
...
class PackagePanel(ttk.Frame):
    ...
    def RemovePackage(self, name, interpreter):
        ...
        if package_name not in interpreter.Packages:
    ...
    def FreezePackage(self):
        ...
        filename = filedialog.asksaveasfilename(master=self, filetypes=[
         descrs], defaultextension=default_ext, initialdir=text_docTemplate.GetDirectory(), initialfile='requirements.txt')
        if filename == '':
            return
        try:
