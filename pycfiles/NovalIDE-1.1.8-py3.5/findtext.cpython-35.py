...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/find/findtext.py
...
class FindTextEngine:
    ...
    def getcookedpat(self):
        ...
        if not self._find_option.regex:
            path = re.escape(path)
    ...
    def search_reverse(self, prog, chars, col):
        r"""Search backwards and return an re match object or None.
        ...
        if self.last_processed_index:
            if self._find_option.down:
                if self.last_processed_index[0] > line:
                    ...
                    if self.last_processed_index[0] == line and self.last_processed_index[1] >= col:
                        wrap_search = True
                    if wrap_search:
                        info_label_var.set(_('search from the document begging!'))
            elif self.last_processed_index[0] < line:
                wrap_search = True
