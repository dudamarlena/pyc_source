...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/editor/notebook.py
# Compiled at: 2019-09-09 02:52:13
# Size of source mod 2**32: 28823 bytes
import tkinter as tk
from noval import GetApp, _, constants, NewId
import noval.ui_base as ui_base, noval.util.utils as utils, noval.misc as misc, noval.menu as tkmenu, noval.consts as consts, noval.imageutils as imageutils, noval.core as core, noval.syntax.syntax as syntax, noval.util.fileutils as fileutils
from tkinter import messagebox, filedialog
import os, noval.util.strutils as strutils
try:
    import tkSimpleDialog
except ImportError:
    import tkinter.simpledialog as tkSimpleDialog

import noval.find.find as findtext, noval.find.findindir as findindir
from noval.editor import text as texteditor
import noval.docposition as docposition, noval.editor.format as textformat, datetime, noval.project.variables as variablesutils, noval.ui_utils as ui_utils
...
class EditorNotebook(ui_base.ClosableNotebook):
    ...
    def load_startup_files(self):
        ...
        if len(cmd_line_filenames) > 0:
            ...
            if get_workbench().get_option('file.reopen_all_files'):
                ...
                if get_workbench().get_option('file.current_file'):
                    filenames = [
                     get_workbench().get_option('file.current_file')]
                else:
                    filenames = []
            if len(filenames) > 0:
    ...
    def GotoLine(self, event=None):
        ...
        lineno = tkSimpleDialog.askinteger(_('Go to Line'), _('Enter line number to go to:(1-%d)') % line_count, parent=self.get_current_editor(), minvalue=1, maxvalue=line_count)
        if lineno is None:
            return 'break'
        if lineno <= 0:
    ...
    def _InitCommands(self):
        ...
        GetApp().AddCommand(constants.ID_COMMENT_LINES, _('&Format'), _('Comment &Lines'), lambda : self.ProcessFormatterEvent(constants.ID_COMMENT_LINES), image=GetApp().GetImage('comment.png'), default_tester=True, default_command=True, extra_sequences=['<<CtrlKInText>>'])
        GetApp().AddCommand(constants.ID_UNCOMMENT_LINES, _('&Format'), _('&Uncomment Lines'), lambda : self.ProcessFormatterEvent(constants.ID_UNCOMMENT_LINES), image=GetApp().GetImage('uncomment.png'), default_tester=True, default_command=True)
        GetApp().AddCommand(constants.ID_INDENT_LINES, _('&Format'), _('&Indent Lines'), lambda : self.ProcessFormatterEvent(constants.ID_INDENT_LINES), image=GetApp().GetImage('indent.png'), default_tester=True, default_command=True)
        GetApp().AddCommand(constants.ID_DEDENT_LINES, _('&Format'), _('&Dedent Lines'), lambda : self.ProcessFormatterEvent(constants.ID_DEDENT_LINES), image=GetApp().GetImage('dedent.png'), default_tester=True, default_command=True)
        ...
        GetApp().AddMenuCommand(constants.ID_EOL_MAC, lineformat_menu, _(textformat.EOLFormatDlg.EOL_CHOICES[0]), lambda : self.ProcessFormatterEvent(constants.ID_EOL_MAC), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, value=consts.EOL_CR, variable=self.eol_var)
        GetApp().AddMenuCommand(constants.ID_EOL_UNIX, lineformat_menu, _(textformat.EOLFormatDlg.EOL_CHOICES[1]), lambda : self.ProcessFormatterEvent(constants.ID_EOL_UNIX), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, value=consts.EOL_LF, variable=self.eol_var)
        GetApp().AddMenuCommand(constants.ID_EOL_WIN, lineformat_menu, _(textformat.EOLFormatDlg.EOL_CHOICES[2]), lambda : self.ProcessFormatterEvent(constants.ID_EOL_WIN), default_command=True, default_tester=True, kind=consts.RADIO_MENU_ITEM_KIND, value=consts.EOL_CRLF, variable=self.eol_var)
        ...
        GetApp().AddCommand(constants.ID_PRE_POS, _('&View'), _('Previous Position'), self.GotoPrePos, image=GetApp().GetImage('toolbar/go_prev.png'), tester=lambda : docposition.DocMgr.CanNavigatePrev(), include_in_toolbar=True)
        GetApp().AddCommand(constants.ID_NEXT_POS, _('&View'), _('Next Position'), self.GotoNextPos, image=GetApp().GetImage('toolbar/go_next.png'), tester=lambda : docposition.DocMgr.CanNavigateNext(), include_in_toolbar=True)
    ...
    def ProcessFormatterEvent(self, event_id):
        ...
        if event_id == constants.ID_COMMENT_LINES:
            ...
            if event_id == constants.ID_UNCOMMENT_LINES:
                opj.UncommentRegion()
            else:
                if event_id == constants.ID_INDENT_LINES:
                    opj.IndentRegion()
                else:
                    if event_id == constants.ID_DEDENT_LINES:
                        opj.DedentRegion()
                    else:
                        if event_id == constants.ID_UPPERCASE:
                            opj.UpperCase()
                        else:
                            if event_id == constants.ID_LOWERCASE:
                                opj.LowerCase()
                            else:
                                if event_id == constants.ID_FIRST_UPPERCASE:
                                    opj.FirstUppercase()
                                else:
                                    if event_id == constants.ID_TAB_SPACE:
                                        opj.untabify_region()
                                    else:
                                        if event_id == constants.ID_SPACE_TAB:
                                            opj.tabify_region()
                                        else:
                                            if event_id == constants.ID_CLEAN_WHITESPACE:
                                                opj.do_rstrip()
                                            else:
                                                if event_id in [constants.ID_EOL_MAC, constants.ID_EOL_WIN, constants.ID_EOL_UNIX]:
                                                    opj.do_eol(event_id)
    ...
    def InsertFileContent(self):
        path = filedialog.askopenfilename(master=self, filetypes=[
         (
          _('All Files'), '.*')], initialdir=os.getcwd())
        if not path:
            return
        with open(path) as (f):
