...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/util/parser.py
# Compiled at: 2019-08-16 02:55:35
# Size of source mod 2**32: 14277 bytes
        ...
        while self.textIndex < self.textLen:
            ...
            if c not in string.whitespace:
                ...
                if c == '<' or c == '>' or c == '!' or c == '=':
                    ...
                    if c == '%' and endIndex < self.textLen:
                        c = self.text[endIndex]
                        if c in ('d', 'i', 'o', 'u', 'x', 'X', 'e', 'E', 'f', 'F',
                                 'g', 'G', 'c', 'r', 's', '%'):
                            endIndex += 1
                        self.nextTokenVal = self.text[self.textIndex:endIndex]
                        self.textIndex = endIndex
...
def isXPathNonVar(var):
    ...
    if var.startswith("'") and var.endswith("'") or var.startswith('"') and var.endswith('"'):
        return True
    if var.lower() in ('count', 'empty', 'true', 'false', 'null', 'and', 'or', 'like',
                       'not'):
        return True
    try:
        t = int(var)
    ...
    for xpath in xpaths:
        ...
        while t.nextToken != None:
            ...
            if t.token == Tokenizer.TOKEN_WS:
                ...
                if t.token == Tokenizer.TOKEN_OP:
                    if t.tokenVal == '=':
                        expr.append('==')
                    else:
                        if t.tokenVal == '[' and convertBracket:
                            expr.append('(')
                        else:
                            if t.tokenVal == ']' and convertBracket:
                                expr.append(')')
                    ...
                    if t.token == Tokenizer.TOKEN_IDENT:
                        if t.tokenVal == 'and':
                            expr.append(' and ')
                        else:
                            if t.tokenVal == 'or':
