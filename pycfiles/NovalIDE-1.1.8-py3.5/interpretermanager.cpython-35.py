...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/interpreter/interpretermanager.py
# Compiled at: 2019-10-17 05:45:01
# Size of source mod 2**32: 16900 bytes
import sys, os
from noval import GetApp, _, NewId
from noval.util.singleton import Singleton
import noval.util.apputils as apputils, pickle
from noval.python.interpreter import interpreter as pythoninterpreter
import json, noval.util.utils as utils
from tkinter import messagebox
from noval.ui_base import GetSingleChoiceIndex
import noval.constants as constants, noval.python.explain_environment as explain_environment
...
class InterpreterManager:
    ...
    def LoadPythonInterpreters(self):
        if apputils.is_windows():
            ...
            ROOT_KEY_LIST = [
             registry.HKEY_LOCAL_MACHINE, registry.HKEY_CURRENT_USER]
            ROOT_KEY_NAMES = ['LOCAL_MACHINE', 'CURRENT_USER']
            for k, root_key in enumerate(ROOT_KEY_LIST):
                try:
                    with registry.Registry(root_key).Open('SOFTWARE\\Python\\Pythoncore') as (open_key):
        ...
        if apputils.is_windows():
            ...
            if utils.is_py3():
                ...
                for l in lst:
                    ...
                    utils.get_logger().debug('load python interpreter from app config name:%s path:%s,version:%s,builtin:%s', interpreter.Name, path, interpreter.Version, interpreter.IsBuiltIn)
    ...
    def ConvertInterpretersToDictList(self):
        lst = []
        for interpreter in self.interpreters:
            d = dict(id=interpreter.Id, name=interpreter.Name, version=interpreter.Version, path=interpreter.Path, default=interpreter.Default, sys_path_list=interpreter.SysPathList, python_path_list=interpreter.PythonPathList, builtins=interpreter.Builtins, help_path=interpreter.HelpPath, environ=interpreter.Environ.environ, packages=interpreter.DumpPackages(), is_builtin=interpreter.IsBuiltIn, minor_version=interpreter.MinorVersion)
            lst.append(d)
    ...
    def CheckInterpreterExist(self, interpreter):
        for kb in self.interpreters:
            if kb.Name.lower() == interpreter.Name.lower():
                return True
            if kb.Path.lower() == interpreter.Path.lower():
                return True
