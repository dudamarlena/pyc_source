...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/util/objutils.py
# Compiled at: 2019-08-16 02:55:35
# Size of source mod 2**32: 17366 bytes
import logging, traceback, sys, os, types, noval.util.utillang as utillang, noval.util.datetimeparser as datetimeparser
from types import *
...
def newInstance(className, objargs=None):
    ...
    if className == 'bool':
        ...
        if className == 'str' or className == 'unicode':
            if len(objargs) > 0:
                try:
                    if utils.is_py2():
...
def typeToString(obj, options=0):
    ...
    if options & PRINT_OBJ_DIFFABLE > 0 and ds.startswith('<class '):
        ix = ds.rfind('.')
        if ix < 0:
            ix = 8
...
def printObject(out, object, name=None, indent=0, flags=0, exclude=None, remove=None, maxIndent=30):
    ...
    if flags & (PRINT_OBJ_COMPACT | PRINT_OBJ_HIDE_EXCLUDED) > 0:
        ...
        if object is None:
            ...
            if name.startswith('_') and flags & PRINT_OBJ_HIDE_INTERNAL > 0 and not name.startswith('_v_'):
                finalNewLine = False
                printed = False
            else:
                if isinstance(object, (list, tuple)):
                    if exclude != None and object in exclude:
                        ...
                        if exclude != None and name in exclude:
                            ...
                            if exclude != None and len(object) > 0:
        ...
        if len(name) > 0:
            ...
            for key in keys:
                if key != None:
                    ...
                    if (not n.startswith('_') or flags & PRINT_OBJ_HIDE_INTERNAL == 0) and printObject(out, object[key], name=n, indent=indent + 2, flags=flags | PRINT_OBJ_INTERNAL, exclude=exclude, remove=remove, maxIndent=maxIndent):
                        if flags & PRINT_OBJ_COMPACT == 0:
                            print >> out
                        else:
                            (
            ...
            if hasattr(object, '__dict__'):
                if name.startswith('_'):
                    ...
                    if exclude != None and (object in exclude or object.__dict__ in exclude):
                        (
                         print >> out, ' ' * indent, name, ' : ', typeToString(object, flags), ' (already printed)')
                    else:
                        if exclude != None:
