...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/python/plugins/pip_gui.py
# Compiled at: 2019-10-17 05:42:34
# Size of source mod 2**32: 64431 bytes
from noval import _, NewId, GetApp
import json, logging, os, re, subprocess, sys, tkinter as tk, webbrowser
from distutils.version import LooseVersion, StrictVersion
from logging import exception
from os import makedirs
from tkinter import messagebox, ttk, filedialog
from tkinter.messagebox import showerror
import noval.ui_base as ui_base, noval.iface as iface, noval.plugin as plugin, noval.constants as constants, noval.util.utils as utils, noval.misc as misc, noval.ui_utils as ui_utils, noval.ui_common as ui_common, noval.consts as consts
from dummy.userdb import UserDataDb
import noval.util.fileutils as fileutils, noval.util.strutils as strutils, site, noval.python.interpreter.interpretermanager as interpretermanager, intellisence, pkg_resources, noval.python.pyutils as pyutils, noval.util.downutils as downutils, shutil, noval.python.parser.utils as parserutils, noval.python.interpreter.pythonpackages as pythonpackages, noval.plugins.update as updateutils, noval.editor.text as texteditor, threading
...
class PipDialog(ui_base.CommonModaldialog):
    ...
    def _create_widgets(self, parent, install_label_text, button_text, label_text, tip_text):
        ...
        self.search_button = ttk.Button(header_frame, text=button_text, command=self._on_search, width=25)
        self.search_button.grid(row=1, column=1, sticky='nse', padx=(10, 0))
        main_pw = tk.PanedWindow(parent, orient=tk.HORIZONTAL, background=misc.lookup_style_option('TPanedWindow', 'background'), sashwidth=15)
        ...
        self.listbox = ui_utils.ThemedListbox(listframe, activestyle='dotbox', width=20, height=20, selectborderwidth=0, relief='flat', borderwidth=0)
        self.listbox.insert('end', ' <INSTALL>')
        self.listbox.bind('<<ListboxSelect>>', self._on_listbox_select, True)
        self.listbox.grid(row=1, column=0, sticky='nsew')
        list_scrollbar = ui_common.AutoScrollbar(listframe, orient=tk.VERTICAL, style=None)
        ...
        info_text_frame = ui_base.TextviewFrame(info_frame, read_only=True, horizontal_scrollbar=False, background=misc.lookup_style_option('TFrame', 'background'), vertical_scrollbar_class=ui_common.AutoScrollbar, vertical_scrollbar_style=None, horizontal_scrollbar_style=None, width=70, height=10, text_class=texteditor.TextCtrl)
        info_text_frame.configure(borderwidth=0)
        info_text_frame.grid(row=1, column=0, columnspan=4, sticky='nsew', pady=(0,
                                                                                 10))
        ...
        self.info_text.tag_bind('install_reqs', '<ButtonRelease-1>', self._handle_install_requirements_click)
        self.info_text.tag_bind('install_reqs', '<Enter>', lambda e: self.info_text.config(cursor='hand2'))
        self.info_text.tag_bind('install_reqs', '<Leave>', lambda e: self.info_text.config(cursor=''))
        ...
        self.install_button = ttk.Button(self.command_frame, text=_(' Upgrade '), command=self._on_click_install)
        self.install_button.grid(row=0, column=0, sticky='w', padx=0)
        self.uninstall_button = ttk.Button(self.command_frame, text=_('Uninstall'), command=lambda : self._perform_action('uninstall'))
    ...
    def _install_pip(self):
        ...
        self.info_text.direct_insert('end', _('pip, a required module for managing packages is missing or too old.\n\n' + 'Downloading pip installer (about 1.5 MB), please wait ...\n'))
        self.update()
    ...
    def _provide_pip_install_instructions(self, error):
        ...
        self.info_text.direct_insert('end', _('You seem to have problems with pip\n\n'), ('caption', ))
        self.info_text.direct_insert('end', _('pip, a required module for managing packages is missing or too old for Noval.\n\n' + "If your system package manager doesn't provide recent pip (9.0.0 or later), " + 'then you can install newest version by downloading '))
        self.info_text.direct_insert('end', PIP_INSTALLER_URL, ('url', ))
        self.info_text.direct_insert('end', _(' and running it with ') + self._get_interpreter().Name + _(' (probably needs admin privileges).\n\n'))
    ...
    def _instructions_for_command_line_install(self):
        return _('Alternatively, if you have an older pip installed, then you can install packages ' + 'on the command line (Tools → Open terminator...)')
    ...
    def _start_show_package_info(self, name):
        ...
        if active_dist is not None:
            ...
            if not os.path.exists(dist_location):
                self.info_text.direct_insert('end', _('path is not exist...'))
            else:
                self.info_text.direct_insert('end', strutils.normpath_with_actual_case(dist_location), ('url', ))
    ...
    def _show_package_info(self, name, data, error_code=None):
        ...
        if error_code is not None:
            if error_code == 404:
                ...
                write(_('Could not find the package info from PyPI. Error code: ') + str(error_code))
            return
        info = data
        self.name_label['text'] = info['name']
        ...
        if self._get_active_version(name) != latest_stable_version or not self._get_active_version(name):
    ...
    def _handle_install_file_click(self, event):
        if self._get_state() != 'idle':
            return
        filename = filedialog.askopenfilename(master=self, filetypes=[
         ('Package', '.whl .zip .tar.gz'), ('all files', '.*')])
        if filename:
    ...
    def _targets_virtual_environment(self):
        return hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix or hasattr(sys, 'real_prefix') and getattr(sys, 'real_prefix') != sys.prefix
...
class PluginsPipDialog(PipDialog):
    ...
    def _show_instructions(self):
        ...
        self.info_text.direct_insert('end', _("If you don't know where to get the plugin from, " + "then most likely you'll want to search the plugin Package Index. " + 'Start by entering the name of the plugin in the search box above and pressing ENTER.\n\n'))
        ...
        self.info_text.direct_insert('end', _(' to locate and install the plugin file (usually with .egg extension).\n\n'))
        self.info_text.direct_insert('end', _('Upgrade or uninstall\n'), ('caption', ))
        self.info_text.direct_insert('end', _('Start by selecting the package from the left.\n\n'))
        if self._get_target_directory():
            ...
            self.info_text.direct_insert('end', _('This dialog lists all available plugins,' + ' but allows upgrading and uninstalling only plugins from '))
            ...
            self.info_text.direct_insert('end', _('. New plugin will be also installed into this directory.' + ' Other locations must be managed by alternative means.'))
        self._select_list_item(0)
    ...
    def _get_latest_stable_version(self, version_strings):
        ...
        versions = []
        for s in version_strings:
            versions.append(LooseVersion(s['version']))
    ...
    def _start_update_list(self, name_to_show=None, refresh=False):
        ...
        self._active_distributions = {plugin.GetName():{'project_name': plugin.GetName(), 'key': plugin.GetName(), 'location': plugin.GetDist().location, 'version': plugin.GetVersion(), 'enabled': plugin.IsEnabled()} for plugin in pdata if plugin.GetName() not in self._uninstall_plugins}
    ...
    def InstallEgg(self, name, egg_path, version):
        ...
        elif utils.is_py3_plus() and sys.version_info.minor != 6:
        ...
        self._install_plugins[name] = {'project_name': name, 
         'key': name, 
    ...
    def _perform_install(self, package_data):
        ...
        payload = dict(new_version=package_data['version'], app_version=app_version, lang=lang, os_name=sys.platform, plugin_id=package_data['id'])
    ...
    def create_advance_buttons(self):
        self.enabled_button = ttk.Button(self.command_frame, text=_('Enabled'), command=lambda : self._perform_enable_action(self.current_package_data))
...
class PyPiPipDialog(PipDialog):
    ...
    def _show_instructions(self):
        ...
        if self._read_only():
            ...
            self.info_text.direct_insert('end', _('With current interpreter you can only browse the packages here.\n' + "Use 'Tools → Plugin Manager' for installing, upgrading or uninstalling.\n\n"))
            if self._get_target_directory():
            ...
            self.info_text.direct_insert('end', _("If you don't know where to get the package from, " + "then most likely you'll want to search the Python Package Index. " + 'Start by entering the name of the package in the search box above and pressing ENTER.\n\n'))
            self.info_text.direct_insert('end', _('Install from requirements file\n'), ('caption', ))
            self.info_text.direct_insert('end', _('Click '))
            self.info_text.direct_insert('end', _('here'), ('install_reqs', ))
            self.info_text.direct_insert('end', _(' to locate requirements.txt file and install the packages specified in it.\n\n'))
            self.info_text.direct_insert('end', _('Install from local file\n'), ('caption', ))
            self.info_text.direct_insert('end', _('Click '))
            self.info_text.direct_insert('end', _('here'), ('install_file', ))
            self.info_text.direct_insert('end', _(' to locate and install the package file (usually with .whl, .tar.gz or .zip extension).\n\n'))
            self.info_text.direct_insert('end', _('Upgrade or uninstall\n'), ('caption', ))
            self.info_text.direct_insert('end', _('Start by selecting the package from the left.\n\n'))
        if self._get_target_directory():
            self.info_text.direct_insert('end', _('Target:  '), ('caption', ))
            ...
            self.info_text.direct_insert('end', _('This dialog lists all available packages,' + ' but allows upgrading and uninstalling only packages from '))
            self.info_text.direct_insert('end', self._get_target_directory(), 'url')
            self.info_text.direct_insert('end', _('. New packages will be also installed into this directory.' + ' Other locations must be managed by alternative means.'))
        self._select_list_item(0)
    ...
    def _start_update_list(self, name_to_show=None, refresh=False):
        ...
        if interpreter.IsBuiltIn:
            ...
            self._active_distributions = {dist.key:{'project_name': dist.project_name, 'key': dist.key, 'location': dist.location, 'version': dist.version} for dist in pkg_resources.working_set}
        else:
    ...
    def _perform_uninstall(self, package_data):
        ...
        """
        name = package_data['name']
        if name in ('pip', 'setuptools') and not messagebox.askyesno(_('Really uninstall?'), _("Package '{}' is required for installing and uninstalling other packages.\n\n").format(name) + _('Are you sure you want to uninstall it?'), parent=self):
            return
    ...
    def _confirm_install(self, package_data):
        ...
        other_version_text = 'NB! There may be another version available ' + 'which is compatible with current Thonny version. ' + "Click on '...' button to choose the version to install."
        if name.lower().startswith('thonny-') and not reqs:
            showerror('Thonny plugin without requirements', 'Looks like you are trying to install an outdated Thonny\n' + "plug-in (it doesn't specify required Thonny version).\n\n" + 'If you still want it, then please install it from the command line.' + '\n\n' + other_version_text, parent=get_workbench())
            return False
        if reqs:
            conflicts = self._conflicts_with_thonny_version(reqs)
            if conflicts:
                showerror('Unsuitable requirements', 'This package requires different Thonny version:\n\n  ' + '\n  '.join(conflicts) + '\n\nIf you still want it, then please install it from the command line.' + '\n\n' + other_version_text, parent=get_workbench())
                return False
        return True
    ...
    def _perform_advanced(self, package_data):
        """
            其它操作
        """
        details = _ask_installation_details(self, data, _get_latest_stable_version(list(data['releases'].keys())))
        if details is None:
            return
...
class PluginManagerGUI(plugin.Plugin):
    ...
    def PlugIt(self, parent):
        ...
        mitem = tools_menu.InsertBefore(constants.ID_PREFERENCES, constants.ID_PLUGIN, _('Plugin Manager'), 'Plugin Manager GUI', handler=self.ShowPluginManagerDlg, img=GetApp().GetImage('plugin.png'))
