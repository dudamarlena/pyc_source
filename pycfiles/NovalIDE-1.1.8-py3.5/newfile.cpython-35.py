...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/project/newfile.py
# Compiled at: 2019-08-16 02:55:35
# Size of source mod 2**32: 7601 bytes
from noval import GetApp, _, Locale
from tkinter import messagebox
import os, noval.util.apputils as sysutilslib, xml.etree.ElementTree as ET, noval.generalopt as generalopt
from bz2 import BZ2File
import noval.util.fileutils as fileutils, noval.syntax.syntax as syntax, noval.util.appdirs as appdirs, copy, uuid, tarfile, tempfile, noval.ui_base as ui_base, noval.consts as consts, noval.util.utils as utils, noval.util.compat as compat
...
class NewFileDialog(ui_base.SingleChoiceDialog):
                    ...
                    with BZ2File(content_zip_path, 'r') as (f):
                        for i, line in enumerate(f):
                            if i == 0:
                                ...
                                fp.write(line.strip('\x00').strip('\r').strip('\n'))
                                fp.write('\n')
    ...
    def LoadFileTemplates(self):
        ...
        if os.path.exists(user_template_path):
            ...
            if os.path.exists(sys_template_path):
            ...
            if element.tag == lang:
                for node in element.getchildren():
                    ...
                    for child in node.getchildren():
                        ...
                        file_type.update({'Category': value_type})
