...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/noval/util/strutils.py
# Compiled at: 2019-10-17 01:45:10
# Size of source mod 2**32: 8286 bytes
import os, re, noval.python.parser.utils as parserutils
...
def get_python_coding_declare(lines):
    CODING_REG_STR = re.compile('^[ \\t\\f]*#.*coding[:=][ \\t]*([-\\w.]+)')
    BLANK_REG_STR = re.compile('^[ \\t\\f]*(?:[#\\r\\n]|$)')
    lst = lines[:2]
    hit_line = 0
    for line in lst:
...
def normpath_with_actual_case(name):
    ...
    if not os.path.isabs(name):
        assert os.path.ismount(name), 'Not abs nor mount: ' + name
