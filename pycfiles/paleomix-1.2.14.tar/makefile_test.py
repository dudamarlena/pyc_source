...
from paleomix.common.makefile import DEFAULT_NOT_SET, REQUIRED_VALUE, MakefileError, MakefileSpec, read_makefile, process_makefile, WithoutDefaults, IsInt, IsUnsignedInt, IsFloat, IsBoolean, IsStr, IsNone, ValueLT, ValueLE, ValueGE, ValueGT, ValueIn, ValuesIntersect, ValuesSubsetOf, ValueMissing, And, Or, Xor, Not, StringIn, StringsIntersect, StringsSubsetOf, StringIsUppercase, StringStartsWith, StringEndsWith, IsListOf, IsDictOf, PreProcessMakefile
...
def test_string_is_uppercase__rejects_not_uppercase_str():
    ...
    yield (
     _reject_not_uppercase_str, 'AcEf')
    yield (_reject_not_uppercase_str, 1)
    for value in _COMMON_INVALID_VALUES:
        yield (
         _reject_not_uppercase_str, value)
...
def test_string_starts_with__rejects_not_uppercase_str():
    ...
    yield (
     _reject_not_str_with_prefix, 1)
    for value in _COMMON_INVALID_VALUES:
        yield (
         _reject_not_str_with_prefix, value)
...
def test_string_ends_with__rejects_not_uppercase_str():
    ...
    yield (
     _reject_not_str_with_postfix, 1)
    for value in _COMMON_INVALID_VALUES:
        yield (
         _reject_not_str_with_postfix, value)
...
def test_specs__path_is_displayed_in_exception():
    ...
    yield (
     _path_is_displayed_in_exception, IsInt(), 'foo')
...
def test_process_makefile__dict_keys_found():
    ...
    yield (
     _dict_keys_found, {'B': 7}, {'A': IsInt, 'B': IsInt})
    yield (
     _dict_keys_found, {1: 'Abc'}, {IsStr: IsInt, IsInt: IsStr})
    yield (
     _dict_keys_found, {1: 'Abc'}, {IsStr(): IsInt, IsInt: IsStr()})
    yield (
     _dict_keys_found, {3: 14}, {IsInt: IsInt, 'A': IsInt})
    yield (
     _dict_keys_found, {'A': 23}, {IsInt: IsInt, 'A': IsInt})
...
def test_process_makefile__dict_keys_not_found():
    ...
    yield (
     _dict_keys_missing, {'C': 7}, {'A': IsInt, 'B': IsInt})
    yield (
     _dict_keys_missing, {1.3: 'Abc'}, {IsStr: IsInt, IsInt: IsStr})
    yield (
     _dict_keys_missing, {1.3: 'Abc'}, {IsStr(): IsInt, IsInt: IsStr()})
    yield (
     _dict_keys_missing, {'C': 14}, {IsInt: IsInt, 'A': IsInt})
...
def test_process_makefile__sets_missing_keys():
    current = {'A': 1}
    specs = {'A': IsInt(default=0), 'B': IsInt(default=-1), 
       'C': IsInt(default=-2)}
...
def test_process_makefile__mixed_keys():
    current = {'A': 1}
    specs = {IsStr: IsInt, 'B': IsInt(default=-1), 
       'C': IsInt(default=-2)}
    expected = {'A': 1, 'B': -1, 'C': -2}
    result = process_makefile(current, specs)
    assert_equal(result, expected)
...
def test_process_makefile__sets_missing_recursive():
    current = {'A': 1, 'B': {'C': 2}}
    specs = {'A': IsInt(default=0), 'B': {'C': IsInt(default=-1), 'D': IsInt(default=-2)}}
...
def test_process_makefile__path_shown_in_exception_for_list():
    assert_raises_regexp(MakefileError, _DUMMY_PATH_STR, process_makefile, {}, [], _DUMMY_PATH)
...
def test_read_makefile__simple_file():
    ...
    expected = {'Makefile': {'Defaults': {'First': 0.0001, 'Second': 'a string'}}, 
       'Statistics': {'Filename': test_file('simple.yaml'), 
