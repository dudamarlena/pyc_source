...
import os, stat, errno, nose
from nose.tools import assert_is, assert_in, assert_equal, assert_raises
import paleomix
from paleomix.common.testing import with_temp_folder, Monkeypatch, SetWorkingDirectory, set_file_contents, get_file_contents
from paleomix.common.fileutils import add_postfix, swap_ext, reroot_path, create_temp_dir, missing_files, modified_after, is_executable, which_executable, executable_exists, missing_executables, make_dirs, move_file, copy_file, open_ro, try_rmdir, try_remove, try_rmtree, describe_files, describe_paired_files
...
def test_create_temp_dir__permissions(temp_folder):
    ...
    assert_equal(stats.st_mode & 511, 488)
...
def test_modified_after__modified_after():
    assert modified_after(test_file('timestamp_a_younger'), test_file('timestamp_a_older'))
    assert modified_after(test_file('timestamp_a_younger'), test_file('timestamp_b_older'))
    assert modified_after(test_file('timestamp_b_younger'), test_file('timestamp_a_older'))


def test_modified_after__not_modified_after():
    assert not modified_after(test_file('timestamp_a_older'), test_file('timestamp_a_younger'))
    assert not modified_after(test_file('timestamp_a_older'), test_file('timestamp_b_younger'))
    assert not modified_after(test_file('timestamp_b_older'), test_file('timestamp_a_younger'))


def test_modified_after__same_file():
    assert not modified_after(test_file('timestamp_a_older'), test_file('timestamp_a_older'))
    assert not modified_after(test_file('timestamp_a_older'), test_file('timestamp_b_older'))
    assert not modified_after(test_file('timestamp_b_older'), test_file('timestamp_a_older'))
...
def test_make_dirs__permissions(temp_folder):
    ...
    assert make_dirs(work_dir, mode=329)
    stats = os.stat(work_dir)
    assert_equal(oct(stats.st_mode & 511), oct(329))
...
def test_open_ro__uncompressed():
    ...
    try:
        assert_equal(handle.read(), '>This_is_FASTA!\nACGTN\n>This_is_ALSO_FASTA!\nCGTNA\n')
    finally:
        handle.close()
...
def test_open_ro__gz():
    ...
    try:
        assert_equal(handle.read(), '>This_is_GZipped_FASTA!\nACGTN\n>This_is_ALSO_GZipped_FASTA!\nCGTNA\n')
    finally:
        handle.close()
...
def test_open_ro__bz2():
    ...
    try:
        assert_equal(handle.read(), '>This_is_BZ_FASTA!\nCGTNA\n>This_is_ALSO_BZ_FASTA!\nACGTN\n')
    finally:
        handle.close()
