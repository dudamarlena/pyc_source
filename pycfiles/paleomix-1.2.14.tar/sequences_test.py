...
import itertools, nose.tools
from nose.tools import assert_equal
from paleomix.common.sequences import complement, reverse_complement, encode_genotype, count_nts, count_gc_diploid, split
...
def test_count_nts__complex():
    assert_equal(count_nts('NNBKVGRVMM'), {'N': 2, 'B': 1, 'K': 1, 'V': 2, 'G': 1, 'R': 1, 
       'M': 2})
...
def test_count_nts__bad_nt():
    count_nts('TTAPGTGTCT')


@nose.tools.raises(ValueError)
def test_count_nts__bad_nt_lowercase():
    count_nts('atATNcoGCC')
