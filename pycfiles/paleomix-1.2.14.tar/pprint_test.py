...
def test_pformat__sets__simple():
    ...
    def _test_pformat__sets__simple(cls, description):
        ...
        cmd_2 = AtomicCmd('gzip', IN_STDIN=cmd_1)
        cmd = cls((cmd_1, cmd_2))
        assert_equal(pformat(cmd), template.format(description=description, cmd_1_id=id(cmd_1), cmd_2_id=id(cmd_2)))

    yield (
     _test_pformat__sets__simple, ParallelCmds, 'Parallel processes')
