...
# [GCC 8.4.0]
# Embedded file name: /home/mischu/devel/paleomix/tests/common_tests/versions_tests.py
# Compiled at: 2019-10-27 09:55:00
import pickle, operator
from nose.tools import assert_is, assert_is_not, assert_in, assert_equal, assert_not_equal, assert_raises
...
def test_and__check_version__truncated():
    ...
    yield (
     _do_and_check_truncated, versions.GE(1, 2), versions.LT(2, 0))
...
def test_and__check_version__insufficient_number_of_values():
    ...
    yield (
     _do_and_check_num_values, versions.GE(1, 2, 2), versions.LT(2, 0))
...
def test_or__check_version__truncated():
    ...
    yield (
     _do_or_check_truncated, versions.GE(1, 2), versions.LT(2, 0))
...
def test_or__check_version__insufficient_number_of_values():
    ...
    yield (
     _do_or_check_num_values, versions.GE(1, 2, 2), versions.LT(2, 0))
...
def test_requirementobj__init__defaults():
    obj = versions.RequirementObj(call=('echo', 'foo'), search='(\\d+)\\.(\\d+)', checks=versions.Any())
    assert_equal(obj.name, 'echo')
    assert_equal(obj.priority, 0)


def test_requirementobj__init__non_defaults():
    obj = versions.RequirementObj(call=('bash', 'foo'), search='(\\d+)\\.(\\d+)', checks=versions.Any(), name='A name', priority=17)
...
def test_requirementobj__version__call():
    ...
    for pipe in _PIPES:
        yield (_do_test_version__single_digit, pipe, 'v(\\d+)', (3, ))
        yield (_do_test_version__single_digit, pipe, 'v(\\d+)\\.(\\d+)', (3, 5))
        yield (_do_test_version__single_digit, pipe, 'v(\\d+)\\.(\\d+)\\.(\\d+)',
         (3, 5, 2))


def test_requirementobj__version__version_str_not_found():
    call = _echo_version('A typical error\n')
    obj = versions.RequirementObj(call=call, search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    assert_raises(versions.VersionRequirementError, getattr, obj, 'version')


def test_requirementobj__version__command_not_found():
    obj = versions.RequirementObj(call=('xyzabcdefoo', ), search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    try:
        obj.version
        assert False
...
def test_requirementobj__version__command_not_executable():
    obj = versions.RequirementObj(call=('./README.rst', ), search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    try:
        obj.version
        assert False
    except versions.VersionRequirementError as error:
...
def test_requirementobj__version__return_code_is_ignored():
    obj = versions.RequirementObj(_echo_version('v1.2.3', returncode=1), search='v(\\d+)\\.(\\d+)', checks=versions.Any())
...
def test_requirementobj__version__func_call():
    ...
    obj = versions.RequirementObj(call=_return_version, search='v(\\d+)\\.(\\d+)', checks=versions.Any())
...
def test_requirementobj__version__func_call_with_arguments():
    ...
    obj = versions.RequirementObj(call=(_return_version, 2, 'foo'), search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    assert_equal(obj.version, (5, 3))
...
def test_requirementobj__version__outdated_jre__with_or_without_version_str():
    ...
    def _do_test_outdated_jre(message):
        obj = versions.RequirementObj(call=lambda : message, search='v(\\d+)\\.(\\d+)', checks=versions.Any())
        try:
            obj.version
            assert False
        except versions.VersionRequirementError as error:
...
def test_requirementobj__executable__no_cli_args():
    obj = versions.RequirementObj(call=['samtools'], search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    assert_equal(obj.executable, 'samtools')


def test_requirementobj__executable__with_cli_arguments():
    obj = versions.RequirementObj(call=['samtools', '--version'], search='v(\\d+)\\.(\\d+)', checks=versions.Any())
    assert_equal(obj.executable, 'samtools')


def test_requirementobj__executable__function():
    obj = versions.RequirementObj(call=lambda : 'v1.1', search='v(\\d+)\\.(\\d+)', checks=versions.Any())
...
def test_requirementobj__call__result_is_cached():
    ...
    obj = versions.RequirementObj(call=lambda : 'v1.1.3', search='(\\d)\\.(\\d)', checks=counter)
    obj()
    obj()
...
def test_requirementobj__call__check_fails__function():
    ...
    obj = versions.RequirementObj(call=lambda : 'v1.0.3', search='(\\d)\\.(\\d)', checks=versions.GE(1, 1), name='test#1')
    try:
        obj()
        assert False
    except versions.VersionRequirementError as error:
...
def test_requirementobj__call__check_fails():
    ...
    obj = versions.RequirementObj(call=_echo_version('v1.0.2'), search='(\\d)\\.(\\d)', checks=versions.GE(1, 1), name='test#1')
    try:
        obj()
        assert False
    except versions.VersionRequirementError as error:
...
def test_requirementobj__call__check_fails__jre_outdated():
    ...
    obj = versions.RequirementObj(call=_echo_version(value), search='(\\d)\\.(\\d)', checks=versions.GE(1, 1), name='test#1')
    try:
        obj()
        assert False
    except versions.VersionRequirementError as error:
...
def test_check__can_pickle():
    ...
    yield (
     _do_test_can_pickle, versions.EQ(1, 2, 3))
