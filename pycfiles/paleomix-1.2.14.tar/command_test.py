...
import os, signal, weakref
from flexmock import flexmock
import nose
from nose.tools import assert_in, assert_equal, assert_raises
from paleomix.common.testing import with_temp_folder, Monkeypatch, get_file_contents, set_file_contents
import paleomix.atomiccmd.command, paleomix.common.fileutils as fileutils
...
def test_atomiccmd__set_cwd():
    ...
    def _do_test_atomiccmd__set_cwd(temp_folder, set_cwd):
        cwd = os.getcwd()
        cmd = AtomicCmd(('bash', '-c', 'echo -n ${PWD}'), TEMP_OUT_STDOUT='result.txt', set_cwd=set_cwd)
        cmd.run(temp_folder)
    ...
    yield (
     _do_test_atomiccmd__set_cwd, False)
...
def test_atomiccmd__set_cwd__temp_in_out():
    ...
    def _test_atomiccmd__paths_temp_in(temp_folder, set_cwd, kwargs):
        cmd = AtomicCmd(('echo', '-n', '%%(%s)s' % tuple(kwargs.keys())), TEMP_OUT_STDOUT='result.txt', set_cwd=set_cwd, **kwargs)
        cmd.run(temp_folder)
        assert_equal(cmd.join(), [0])
    ...
    yield (
     _test_atomiccmd__paths_temp_in, True, {'TEMP_IN_FOO': 'test_file'})
...
def test_atomiccmd__paths():
    cmd = AtomicCmd('ls', IN_AAA='/a/b/c', IN_AAB='/x/y/z', TEMP_IN_ABB='tmp_in', OUT_AAA='/out/foo', OUT_BBC='foo/bar', TEMP_OUT_A='xyb', EXEC_OTHER='true', AUX_WAT='wat/wat', CHECK_FUNC=bool, OUT_STDERR='/var/log/pipe.stderr', TEMP_OUT_STDOUT='pipe.stdout')
...
def test_atomiccmd__pipes_out():
    ...
    yield (
     _test_atomiccmd__pipes_out,
     None, None,
     {'OUT_STDOUT': AtomicCmd.DEVNULL, 'OUT_STDERR': AtomicCmd.DEVNULL})
    yield (_test_atomiccmd__pipes_out,
...
def test_atomiccmd__paths__malformed_keys():
    ...
    yield (
     _do_test_atomiccmd__paths__malformed, {'IN': '/var/foo'})
    yield (
     _do_test_atomiccmd__paths__malformed, {'IN_': '/var/foo'})
    yield (
     _do_test_atomiccmd__paths__malformed, {'TEMP_OUT': '/var/foo'})
    yield (
     _do_test_atomiccmd__paths__malformed, {'TEMP_OUT_': '/var/foo'})
    yield (
     _do_test_atomiccmd__paths__malformed, {'TEMP_OUX_FOO': 'foo'})
    yield (
     _do_test_atomiccmd__paths__malformed, {'INS_BAR': 'foo'})
...
def test_atomiccmd__paths__invalid_temp_paths():
    ...
    yield (
     test_function, {'TEMP_IN_FOO': 'sub/infile'})
    ...
    yield (
     test_function, {'TEMP_IN_FOO': '/tmp/sub/infile'})
...
def test_atomiccmd__paths__overlapping_output():
    ...
    yield (
     test, 'OUT_FILE_1', '/foo/bar/outfile', 'OUT_FILE_2', '/var/outfile')
    ...
    yield (
     test, 'OUT_FILE_1', '/foo/bar/outfile', 'OUT_STDOUT', '/var/outfile')
    ...
    yield (
     test, 'OUT_FILE_1', '/foo/bar/outfile', 'OUT_STDERR', '/var/outfile')
...
def test_atomiccmd__pipes__duplicates():
    ...
    yield (
     _do_test_atomiccmd__pipes__duplicates, 'IN_STDIN')
...
def test_atomiccmd__exec__invalid():
    ...
    yield (
     _test_atomiccmd__exec__invalid, str)
...
def test_atomiccmd__piping__wrong_pipe():
    ...
    yield (
     _test_atomiccmd__piping__wrong_pipe, 'IN_STDIN')
...
def test_atomiccmd__join_wait():
    ...
    yield (
     _do_test_atomiccmd__join_wait, AtomicCmd.join, 'true', [None], [0])
    yield (_do_test_atomiccmd__join_wait, AtomicCmd.join, 'false', [None], [1])
    yield (
     _do_test_atomiccmd__join_wait, AtomicCmd.wait, 'true', None, 0)
...
def test_atomiccmd__commit_temp_out(temp_folder):
    ...
    cmd = AtomicCmd(('echo', 'foo'), OUT_STDOUT=os.path.join(dest, 'foo.txt'), TEMP_OUT_FOO='bar.txt')
    cmd.run(temp)
    assert_equal(cmd.join(), [0])
...
def test_atomiccmd__commit_missing_files(temp_folder):
    ...
    cmd = AtomicCmd(('touch', '%(OUT_FOO)s'), OUT_FOO=os.path.join(destination, '1234'), OUT_BAR=os.path.join(destination, '4567'))
    cmd.run(temp_folder)
    cmd.join()
...
def test_atomiccmd__commit_failure_cleanup(temp_folder):
    ...
    destination, temp_folder = _setup_for_commit(temp_folder, False)
    command = AtomicCmd(('touch', '%(OUT_FILE_1)s', '%(OUT_FILE_2)s', '%(OUT_FILE_3)s'), OUT_FILE_1=os.path.join(destination, 'file_1'), OUT_FILE_2=os.path.join(destination, 'file_2'), OUT_FILE_3=os.path.join(destination, 'file_3'))
    try:
...
def test_atomiccmd__commit_with_pipes(temp_folder):
    ...
    command_2 = AtomicCmd(('gzip', ), IN_STDIN=command_1, OUT_STDOUT=os.path.join(destination, 'foo.gz'))
...
def test_atomiccmd__cleanup_proc():
    ...
    yield (
     _do_test_atomiccmd__cleanup_proc, _do_commit)
    yield (
     _do_test_atomiccmd__cleanup_proc, lambda _cmd, _temp_folder: None)
...
def test_atomiccmd__cleanup_sigterm():
    ...
    def _do_test_atomiccmd__cleanup_sigterm(kill_at):
        ...
        assert_equal(sigs_sent, {7913: (signal.SIGTERM, kill_at == 0), 12345: (
                 signal.SIGTERM, kill_at == 1)})
        return
...
def test_atomiccmd__cleanup_sigterm__dead_weakrefs():
    exit_called = []
    procs_wrapper = [
     weakref.ref(Monkeypatch('sys.exit', None))]
