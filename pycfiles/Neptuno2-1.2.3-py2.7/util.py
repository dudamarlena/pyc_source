...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neptuno/util.py
# Compiled at: 2013-04-22 04:32:15
import re, simplejson, datetime as dt
...
def get_usuario(form):
    return (
     get_param(form, 'id_usuario', int), get_param(form, 'id_sesion', str))
...
def strtodate(s, fmt='%d/%m/%Y', no_exc=False):
    ...
    OUT
      <date>
    """
    regex = '^\\s*%s\\s*$' % fmt.replace('%d', '(?P<day>\\d{1,2})').replace('%m', '(?P<month>\\d{1,2})').replace('%Y', '(?P<year>\\d{4})')
...
def strtotime(s):
    m = re.search('^(\\d{1,2}):(\\d{1,2}):?(\\d{1,2})?', s)
...
def format_float(value, thousands_sep=',', decimal_sep='.', show_sign=False, n_dec=2, format=None):
    """
    IN
      value          <float> / <str>
    ...
    """
    if format:
        m_format = re.search('^(\\+)?[1-9](\\.|,)?[1-9]{3}(\\.|,)(0*)$', format)
        if m_format:
    ...
    if int_part:
        ...
        result = '0'
    return '%(sign)s%(int_part)s%(sep)s%(dec_part)s' % dict(sign=sign or '', int_part=result, sep=decimal_sep, dec_part=dec_part)
