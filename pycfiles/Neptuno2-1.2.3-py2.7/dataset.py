...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neptuno/dataset.py
# Compiled at: 2013-09-03 10:52:27
...
from sqlalchemy.types import INTEGER, NUMERIC, DATE, TIME, BOOLEAN, BIGINT, DATETIME
    ...
    def _formatdt(self, fmt, value):
        if isinstance(fmt, (str, unicode)):
            if dt.date == type(value) and value >= dt.date(1900, 1, 1) or dt.datetime == type(value) and value >= dt.datetime(1900, 1, 1) or isinstance(value, dt.time):
                ...
                def _strftime(d, fmt):
                    return fmt.replace('%d', '%.2d' % d.day).replace('%m', '%.2d' % d.month).replace('%Y', '%.4d' % d.year)
                    if isinstance(d, (dt.datetime, dt.time)):
                        return fmt.replace('%H', '%.2d' % d.hour).replace('%M', '%.2d' % d.minute).replace('%S', '%.2d' % d.second)
