...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/neptuno/postgres/search.py
# Compiled at: 2013-05-16 08:21:05
...
class Busqueda(object):
    ...
    def __init__(self, tabla, texto_busqueda, columnas_trans=None, strtodatef=None):
        ...
        if not columnas_trans:
            self.cols_trans = [ re.sub('[^a-z0-9]*', '', self.quitar_acentos(col)) for col in self.cols ]
        else:
    ...
    def process_date_constants(self, termino):
        ...
        return re.sub('\\{\\s*(\\w+)(\\s+([\\+\\-])\\s*(\\d+)\\s*(\\w{0,1}))?\\s*\\}', _sub, termino)
    ...
    def busqueda_campos(self, busqueda):
        ...
        for termino in terminos:
            ...
            if termino != '':
                ...
                if operador in ('#', '!'):
                    if isinstance(col.type, DATE) or isinstance(col.type, TIME) or isinstance(col.type, DATETIME):
                        if operador == '#':
                        ...
                        sql.append('TRIM(COALESCE(CAST("%s" as Text), \'\')) <> \'\'' % c.encode('utf-8'))
                    elif operador == '!':
                        sql.append('TRIM(COALESCE(CAST("%s" as Text), \'\')) = \'\'' % c.encode('utf-8'))
                    elif operador == '==':
                    ...
                    explicitos = re.findall('"([^"]+)"', campos[1], re.I)
                    texto = self.quitar_especiales(re.sub('"[^"]+"', '', campos[1], re.I))
                    ...
                    for exp in explicitos:
                        if not (isinstance(col.type, DATE) or isinstance(col.type, TIME) or isinstance(col.type, DATETIME)):
                            if operador == '=':
                    ...
                    expresion_novacio = '%s AND \n(' % expresion_novacio if expresion_novacio else ''
    ...
    def condicion_busqueda(self, busqueda):
        ...
        for filtro in filtros:
            ...
            if m:
                ...
                sql_termino = []
                explicitos = re.findall('"([^"]+)"', filtro, re.I)
                filtro = re.sub('"[^"]+"', '', filtro, re.I | re.U)
                filtro = filtro.replace('(', ' ').replace(')', ' ').replace('?', ' ').replace('¿', ' ').replace('@', ' ')
                ...
                for item in search_items:
                    ...
                    if item.strip():
                        ...
                        for col_name, tipo in zip(self.cols, self.types):
                            ...
                            elif col_name != 'id' and not col_name.startswith('id_') and not col_name.startswith('_') and not is_date and not is_time:
                                try:
                                    args_ = (
                                     col_name, col_name, self.sin_acentos(item.encode('utf-8')))
...
def search(session, table_name, q=None, rp=100, offset=0, show_ids=False, strtodatef=None, filters=None, collection=None):
    """
    IN
    ...
    if collection:
        ...
        sel = select([child_table.c.id], from_obj=child_table, whereclause=and_(child_table.c[child_attr] == parent_id, child_table.c.id == tbl.c.id)).correlate(tbl)
