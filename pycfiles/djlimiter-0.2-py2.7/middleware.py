...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djlimiter/middleware.py
# Compiled at: 2015-12-19 20:09:51
...
class Limiter(object):
    ...
    def __init__(self):
        ...
        if conf_limits:
            self.global_limits = [
             LimitWrapper(list(parse_many(conf_limits)), self.key_function, None, False)]
        self.header_mapping = {HEADERS.RESET: getattr(settings, C.HEADER_RESET, 'X-RateLimit-Reset'), 
        ...
        if isinstance(callback, six.string_types):
            ...
            try:
                ...
                self.logger.error('Unable to load callback function %s. Rate limiting disabled', callback)
                self.enabled = False
    ...
    def process_request(self, request):
        ...
        if not self.enabled or func in EXEMPT or not name:
            return
        ...
        for lim in limits:
            ...
            for cur_limit in cur_limits:
                if not limit_for_header or cur_limit < limit_for_header[0]:
                    limit_for_header = (
                     cur_limit, (lim.key_func or self.key_function)(request), limit_scope)
                if lim.per_method:
