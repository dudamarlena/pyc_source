...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/djlimiter/decorators.py
# Compiled at: 2015-01-08 23:22:02
...
def limit(limit_value, key_function=None, per_method=False):
    ...
    def __inner(fn):
        ...
        if callable(limit_value):
            DECORATED.setdefault(_inner, []).append(LimitWrapper(limit_value, key_function, None, per_method))
        else:
            DECORATED.setdefault(_inner, []).extend([
             LimitWrapper(list(parse_many(limit_value)), key_function, None, per_method)])
        return _inner
...
def shared_limit(limit_value, scope, key_function=None):
    ...
    def __inner(fn):
        ...
        if fn in DECORATED:
            DECORATED.setdefault(_inner, DECORATED.pop(fn))
        if callable(limit_value):
            DECORATED.setdefault(_inner, []).append(LimitWrapper(limit_value, key_function, scope))
        else:
            DECORATED.setdefault(_inner, []).extend([
             LimitWrapper(list(parse_many(limit_value)), key_function, scope)])
        return _inner
