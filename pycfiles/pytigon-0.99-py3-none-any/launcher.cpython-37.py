...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-76h68wr6/pyppeteer/pyppeteer/launcher.py
# Compiled at: 2020-04-19 04:11:09
# Size of source mod 2**32: 13760 bytes
"""Chromium process launcher module."""
import asyncio, atexit, json
...
class Launcher(object):
    __doc__ = 'Chrome process launcher class.'
    ...
    def __init__(self, options: Dict[(str, Any)]=None, **kwargs: Any) -> None:
        ...
        if self.options.get('appMode', False):
            ...
            if not self.options.get('ignoreDefaultArgs', False):
                ...
                if (self.options.get('ignoreDefaultArgs') or isinstance)(self.options.get('args'), list):
                    if _is_default_url():
    ...
    def _parse_args(self) -> None:
        if not (isinstance(self.options.get('args'), list) and any((opt for opt in self.options['args'] if opt.startswith('--user-data-dir')))):
            if 'userDataDir' not in self.options:
                if not CHROME_PROFILE_PATH.exists():
                    CHROME_PROFILE_PATH.mkdir(parents=True)
                self._tmp_user_data_dir = tempfile.mkdtemp(dir=(str(CHROME_PROFILE_PATH)))
            self.chrome_args.append('--user-data-dir={}'.format(self.options.get('userDataDir', self._tmp_user_data_dir)))
    ...
    def _get_ws_endpoint(self) -> str:
        ...
        while 1:
            ...
            raise BrowserError('Browser closed unexpectedly:\n{}'.format(self.proc.stdout.read().decode()))
    ...
    def waitForChromeToClose(self) -> None:
        """Terminate chrome."""
        if self.proc.poll() is None:
            self.chromeClosed = self.chromeClosed or True
            try:
                self.proc.terminate()
                self.proc.wait()
            except Exception:
