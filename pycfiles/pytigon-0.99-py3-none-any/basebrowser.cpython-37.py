...
# [GCC 8.4.0]
# Embedded file name: /home/sch/prj/pytigon/pytigon/appdata/plugins/standard/webview/basebrowser.py
# Compiled at: 2019-12-31 05:46:00
# Size of source mod 2**32: 17474 bytes
...
class BaseWebBrowser(object):
    ...
    def _get_http_file(self, uri):
        ...
        if uri.startswith('http://127.0.0.2'):
            if uri.startswith('http://127.0.0.2/data'):
                if '?' in uri:
                    data = split2(uri, '?')
                    ...
                    if uri.startswith('http://127.0.0.2/file/'):
                        return (
                         None, uri.replace('http://127.0.0.2/file/', ''))
                    uri2 = uri
                if '/images/ui' in uri2:
                    uri2 = '/static/themes/bootstrap/images/ui' + uri2.split('/images/ui')[1].split('#')[0]
            else:
                if uri.startswith('http://127.0.0.2/static'):
                    if '?' not in uri:
                        path = uri.replace('http://127.0.0.2', '')
                        return (None, path)
                if '/site_media/' in uri and '?' not in uri:
