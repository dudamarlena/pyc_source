...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-76h68wr6/pyppeteer/pyppeteer/browser.py
...
class Browser(EventEmitter):
    ...
    def browserContexts(self) -> List['BrowserContext']:
        ...
        """
        return [
         self._defaultContext] + [context for context in self._contexts.values()]
    ...
    async def create(connection: Connection, contextIds: List[str], ignoreHTTPSErrors: bool, appMode: bool, process: Optional[Popen]=None, closeCallback: Callable[([], Awaitable[None])]=None, **kwargs: Any) -> 'Browser':
        ...
        browser = Browser(connection, contextIds, ignoreHTTPSErrors, appMode, process, closeCallback)
    ...
    async def _targetCreated(self, event: Dict) -> None:
        ...
        target = Target(targetInfo, context, lambda : self._connection.createSession(targetInfo), self._ignoreHTTPSErrors, self._setDefaultViewport, self._screenshotTaskQueue, self._connection._loop)
