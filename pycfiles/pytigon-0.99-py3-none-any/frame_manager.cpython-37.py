...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-76h68wr6/pyppeteer/pyppeteer/frame_manager.py
# Compiled at: 2020-04-19 04:11:09
# Size of source mod 2**32: 35279 bytes
...
class FrameManager(EventEmitter):
    __doc__ = 'FrameManager class.'
    Events = SimpleNamespace(FrameAttached='frameattached',
      FrameNavigated='framenavigated',
      FrameDetached='framedetached',
      LifecycleEvent='lifecycleevent',
      FrameNavigatedWithinDocument='framenavigatedwithindocument')

    def __init__(self, client, frameTree, page):
    ...
    def _onExecutionContextCreated(self, contextPayload: Dict) -> None:
        ...
        context = ExecutionContext(self._client, contextPayload, lambda obj: self.createJSHandle(contextPayload['id'], obj), frame)
...
class Frame(object):
 ...
 pageFunction, *args, **{'force_expr': force_expr})
    ...
    def _waitForSelectorOrXPath(self, selectorOrXPath: str, isXPath: bool, options: dict=None, **kwargs: Any) -> 'WaitTask':
        ...
        return WaitTask(self, predicate, f"""{'XPath' if isXPath else 'selector'} "{selectorOrXPath}"""", polling, timeout, self._client._loop, selectorOrXPath, isXPath, waitForVisible, waitForHidden)
...
class WaitTask(object):
    ...
    async def rerun(self) -> None:
        ...
        try:
            if not error:
                if success:
                    if await self._frame.evaluate('s => !s', success):
                        await success.dispose()
                        return
        except NetworkError:
            ...
            if isinstance(error, NetworkError):
                if 'Execution context was destroyed' in error.args[0]:
                    return
            if isinstance(error, NetworkError):
                if 'Cannot find context with specified id' in error.args[0]:
                    return
            if error:
