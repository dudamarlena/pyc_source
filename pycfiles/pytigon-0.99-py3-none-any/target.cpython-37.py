...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-76h68wr6/pyppeteer/pyppeteer/target.py
...
class Target(object):
    __doc__ = "Browser's target class."

    def __init__(self, targetInfo: Dict, browserContext: 'BrowserContext', sessionFactory: Callable[([], Coroutine[(Any, Any, CDPSession)])], ignoreHTTPSErrors: bool, setDefaultViewport: bool, screenshotTaskQueue: List, loop: asyncio.AbstractEventLoop) -> None:
    ...
    async def page(self) -> Optional[Page]:
        ...
        if self._targetInfo['type'] in ('page', 'background_page'):
            if self._page is None:
                ...
                new_page = await Page.create(client, self, self._ignoreHTTPSErrors, self._setDefaultViewport, self._screenshotTaskQueue)
                self._page = new_page
                return new_page
        return self._page
    ...
    def _targetInfoChanged(self, targetInfo: Dict) -> None:
        ...
        if not self._isInitialized:
            if not self._targetInfo['type'] != 'page':
                if self._targetInfo['url'] != '':
