...
# Embedded file name: build\bdist.win-amd64\egg\accasim\base\simulator_class.py
# Compiled at: 2018-12-13 10:38:46
# Size of source mod 2**32: 32781 bytes
...
class SimulatorBase(ABC):
    ...
    def additional_data_init(self, _additional_data):
        ...
        for ad in _additional_data:
            ...
            if isinstance(ad, AdditionalData):
                ...
                if issubclass(ad, AdditionalData):
    ...
    def generate_enviroment(self, config_path):
        ...
        return (
         ResourceManager(resources), equiv, start_time)
    ...
    def show_config(self):
        ...
        self._logger.info('\tResults folder: {}{}.'.format(self.constants.RESULTS_FOLDER_PATH, ', Overwrite previous files' if self.constants.OVERWRITE_PREVIOUS else ''))
        self._logger.info('\t\t ({}) Dispatching Plan Output. Prefix: {}'.format(self.on_off(self.constants.SCHEDULING_OUTPUT), self.constants.SCHED_PREFIX))
        self._logger.info('\t\t ({}) Statistics Output. Prefix: {}'.format(self.on_off(self.constants.STATISTICS_OUTPUT), self.constants.STATISTICS_PREFIX))
        self._logger.info('\t\t ({}) Dispatching Plan. Pretty Print Output. Prefix: {}'.format(self.on_off(self.constants.PPRINT_OUTPUT), self.constants.PPRINT_PREFIX))
        self._logger.info('\t\t ({}) Benchmark Output. Prefix: {}'.format(self.on_off(self.constants.BENCHMARK_OUTPUT), self.constants.BENCHMARK_PREFIX))
    ...
    def remove_previous(self):
        ...
        _paths = [path.join(self.constants.RESULTS_FOLDER_PATH, _prefix + self.constants.WORKLOAD_FILENAME) for state, _prefix in _wouts if state]
    ...
    def _save_parameters(self, _parameters, filename='simulator_parameters.json'):
        ...
        _dict = {_param:getattr(self.constants, _param) for _param in _parameters}
    ...
    def _generated_filepaths(self):
        ...
        return {f[1]:f[2] for f in possible_filepaths if f[0] if f[0]}


class Simulator(SimulatorBase):
    """Simulator"""

    def __init__(self, workload, sys_config, dispatcher, resource_manager=None, reader=None, job_factory=None, additional_data=[], simulator_config=None, overwrite_previous=True, scheduling_output=True, pprint_output=False, benchmark_output=False, statistics_output=True, save_parameters=None, show_statistics=True, **kwargs):
        r"""
        ...
        """
        if not version_info >= (3, 5):
            raise AssertionError('Unsupported python version. Try with 3.5 or newer.')
        else:
            ...
            if not job_factory:
                ...
                _jf_arguments = [
                 'job_class', 'job_attrs', 'job_mapper']
            ...
            if not isinstance(additional_data, list):
                if not isinstance(additional_data, AdditionalData):
                    if not issubclass(additional_data, AdditionalData):
                        raise AssertionError('Only subclasses of AdditionalData class are acepted as additional_data argument ')
                additional_data = [
                 additional_data]
            ...
            if benchmark_output:
                self._usage_writer = AsyncWriter(path=(path.join(self.constants.RESULTS_FOLDER_PATH, self.constants.BENCHMARK_PREFIX + self.constants.WORKLOAD_FILENAME)),
    ...
    def start_simulation(self, system_status=False, **kwargs):
        ...
        """
        if system_status:
            functions = {'usage_function':self.mapper.usage,  'availability_function':self.mapper.availability, 
             'simulated_status_function':self.mapper.simulated_status, 
             'current_time_function':self.mapper.simulated_current_time}
            self.daemons['system_status'] = {'class':SystemStatus, 
             'args':[
              self.constants.WATCH_PORT, functions], 
             'object':None}
        else:
    ...
    def start_hpc_simulation(self, **kwargs):
        ...
        while events or self.mapper.has_events():
            ...
            if events:
                ...
                assert queued_len == dispatched_len + rejected_len, 'Some queued jobs ({}/{}) were not included in the dispatching decision.'.format(dispatched_len + rejected_len, queued_len)
            ...
            if self.constants.BENCHMARK_OUTPUT:
                ...
                benchMemUsage = self._process_obj.memory_info().rss / float(1048576)
            ...
            if self.timeout:
                if self.timeout <= int(time() - init_sim_time):
                    ontime = False
                    break

        self.end_simulation_time = clock()
        if not self.timeout or self.timeout and ontime:
            if not self.loaded_jobs == self.dispatched_jobs + self.rejected_jobs:
                raise AssertionError('Loaded {} != dispatched + rejected {} jobs'.format(self.loaded_jobs, self.dispatched_jobs + self.rejected_jobs))
    ...
    def statics_write_out(self, show, save):
        ...
        if not (show or save):
            ...
            if save:
                _filepath = path.join(self.constants.RESULTS_FOLDER_PATH, self.constants.STATISTICS_PREFIX + self.constants.WORKLOAD_FILENAME)
    ...
    def daemon_init(self):
        ...
        """
        _iter_func = --- This code section failed: ---

 L. 696         0  LOAD_GLOBAL              isinstance
                2  LOAD_FAST                'act'
                4  LOAD_GLOBAL              dict
                6  CALL_FUNCTION_2       2  ''
...
Parse error at or near `RETURN_END_IF_LAMBDA' instruction at offset 44
    ...
    def default_job_description(self):
        ...
        return [
         total_cores, total_mem, expected_duration, queue, user_id]
