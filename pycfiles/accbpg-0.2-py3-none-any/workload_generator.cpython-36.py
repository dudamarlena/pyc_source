...
# Embedded file name: build\bdist.win-amd64\egg\accasim\experimentation\workload_generator.py
# Compiled at: 2018-06-11 05:03:13
# Size of source mod 2**32: 32955 bytes
...
class Generator(ABC):
    ...
    def _generate_dist_params(self, data, save=False, filepath=None):
        ...
        params = {'dist_name':dist_name,  'dist_param':param[:-2], 
         'optional':{'loc':param[-2], 
          'scale':param[-1]}}
        return (
         params, bins, hist)
...
class JobGenerator(Generator):
    ...
    def __init__(self, total_nodes, resources_types, serial_prob, parallel_prob, parallel_node_prob, performance, min_request, max_request, params, max_opers_serial, max_parallel_duration, distributions=[
 'gamma', 'expon', 'erlang', 'beta', 'arcsine']):
        """
        ...
        self.resources = list(min_request.keys())
    ...
    def _init_probabilities(self, serial_prob, parallel_prob, par_node_prob):
        ...
        :return:
        """
        if not serial_prob + parallel_prob == 1:
            raise AssertionError('Jobs types (serial, parallel) probabilities dont sum 1')
        elif not round(sum([p for p in par_node_prob.values()]), 10) in (0, 1):
            raise AssertionError('Parallel node probabilities dont sum 1')
    ...
    def _generate_request(self, gflops, job_profiles=None):
        ...
        r = random()
        if r <= self.p_serial:
            if gflops <= self.max_opers_serial:
                nodes = 1
                type = self.SERIAL
            else:
    ...
    def _job_request(self, type, nodes, request):
        ...
        for res in request:
            ...
            if type == self.PARALLEL:
                if nodes == 1:
                    if res == 'core':
                        _min = 2
            if type == self.SERIAL:
                if res not in ('core', 'mem'):
                    _max = 0
                if res == 'core':
                    _max = 1
...
class ArriveGenerator(Generator):
    ...
    def __init__(self, initial_time, hour_prob, day_prob, month_prob, param, total_jobs, cyclic_day_start=0, max_arrive_time=432000, distributions=[]):
        """
        ...
        self.initial_bucket = int(round(str_datetime(self.initial_time).get_hours() * self.BUCKETS / self.HOURS_PER_DAY))
    ...
    def add_sample(self, submission_times, rush_hours=(8, 17), save=False):
        ...
        _bucket_number = lambda _dtime: _dtime.get_hours() * (self.BUCKETS // self.HOURS_PER_DAY) + _dtime.get_minutes() // 30
        ...
        for i, (cur_time, next_time) in enumerate(zip(submission_times[:-1], submission_times[1:])):
            ia_time = abs(next_time - cur_time)
    ...
    def next_time(self, generation_stats):
        ...
        if not len(self.params) > 0:
            raise AssertionError('Data samples must be added before try to generate a next time arrive.')
        else:
            if not self.initialized:
                ...
                if current_jobs == 0:
                    ...
                    cur = [
                     generation_stats['current_h'][(current_hour - 1)] / current_jobs, generation_stats['current_d'][(current_day - 1)] / current_jobs, generation_stats['current_m'][(current_month - 1)] / current_jobs]
            ...
            assert cur_max_ia_time <= self.TOO_MUCH_ARRIVE_TIME, 'wrong cur max ia time {} > {} ({} days)'.format(cur_max_ia_time, self.TOO_MUCH_ARRIVE_TIME, exp(cur_max_ia_time) / 86400)
        while 1:
    ...
    def _date_info(self, timestamp):
        ...
        return (
         dtime.get_hours(), dtime.get_weekday(), dtime.get_month())
    ...
    def _initialize(self):
        ...
        for _name, dist_params in self.params.items():
            dist = {'dist_name':dist_params['dist_name'], 
             'dist_param':dist_params['dist_param'],  'optional':dist_params['optional']}
            self.weights[_name] = [(self.dist_cdf)((i + 0.5), **dist) - (self.dist_cdf)((i - 0.5), **dist) for i in range(self.BUCKETS)]
...
class WorkloadGenerator:
    ...
    def __init__(self, workload, sys_config, performance, request_limits, reader_class=None, resources_target=None, user_behavior=None, walltime_calculation=None, non_processing_resources=['mem'], **kwargs):
        ...
        total_jobs, _submissiont_times, _job_total_opers, serial_prob, nodes_parallel_prob, hour_prob, day_prob, month_prob, max_opers_serial, max_parallel_duration = self._initialize(reader_class, performance, resources, non_processing_resources)
    ...
    def _initialize(self, base_reader, performance, resources, non_processing_resources):
        ...
        proc_units = [res_type for res_type in resource_types if res_type not in non_processing_resources]
        _job_runtimes = []
        ...
        while True:
            ...
            if _job_type == 'serial':
                if _max_opers_serial < _job_total_oper:
            ...
            assert requested_nodes < total_nodes, 'More nodes than available have been requested. ({} > {})'.format(requested_nodes, total_nodes)
            total_jobs += 1
        ...
        return (
         total_jobs, _job_submission_times, _job_total_opers, _job_types['serial'], nodes_parallel_prob, _job_hours, _job_days, _job_months, _max_opers_serial, _max_parallel_duration)
    ...
    def generate_jobs(self, n, filename, overwrite=True, writer=None):
        ...
        if not writer:
            ...
            obj_assertion(writer, WorkloadWriter, 'Received {} type as system resource. resources_class type expected.', [
             WorkloadWriter.__class__.__name__])
        generation_stats = {'current_jobs':0, 
         'total_jobs':n, 
    ...
    def _set_resources(self, resources_obj, sys_config):
        ...
        """
        if resources_obj:
            obj_assertion(resources_obj, Resources, 'Received {} type as system resource. resources_class type expected.', [
