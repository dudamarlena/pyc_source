...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/tools/renamer/widgets/renamer.py
# Compiled at: 2020-04-11 22:50:40
# Size of source mod 2**32: 40272 bytes
"""
...
class RenamerWidget(base.BaseWidget, object):
    ...
    def _setup_categories(self):
        if not self._config:
            logger.warning('Impossible to setup categories because tpDcc-tools-renamer configuration file is not available!')
            return
        else:
            categories = self._config.get('categories', default=(list()))
            if not categories:
                logger.warning('Impossible to setup categories because categories property is not defined in tpDcc-tools-renamer configuration file!')
                return
    ...
    def _find_manual_available_name(self, items, name, prefix=None, suffix=None, side='', index=-1, padding=0, letters=False, capital=False, remove_first=0, remove_last=0, find_str=None, replace_str=None, joint_end=False):
        ...
        if tp.Dcc.object_exists(test_name) or test_name in item_names:
            ...
            return self._find_manual_available_name(items,
              name, prefix=prefix, index=new_index, padding=padding, letters=letters,
              capital=capital,
        ...
        if self.rename_tab.currentIndex() == 0:
            ...
            for item in items:
                ...
                if generate_preview_name:
                    if base_name == item_obj:
                        if prefix or suffix or side:
                            index = None
                    else:
                        index = duplicated_names[base_name]
                    preview_name = self._find_manual_available_name(items,
                      base_name, prefix=prefix, side=side, suffix=suffix, index=index,
                      padding=padding,
                      letters=naming_method,
                      capital=upper,
                      joint_end=joint_end,
                      remove_first=remove_first,
                      remove_last=remove_last,
                    ...
                    while preview_name in generated_names:
                        duplicated_names[base_name] += 1
                        preview_name = self._find_manual_available_name(items,
                          base_name, prefix=prefix, side=side, suffix=suffix, index=(duplicated_names[base_name]),
                          padding=padding,
                          letters=naming_method,
                          capital=upper,
                          joint_end=joint_end,
    ...
    def _get_filter_vars(self):
        ...
        return (
         search_hierarchy, only_selection)
    ...
    def _on_refresh_category(self, category_widget=None):
        if not category_widget:
            ...
            if not category_widget:
                return
        category_widget.refresh(selected_objects=(self._selected_radio.isChecked()),
          hierarchy=(self._hierarchy_cbx.isChecked()))
                ...
                if hasattr(item, 'handle'):
                    if tp.is_maya():
                        ...
                        try:
                            ...
                            logger.warning('Impossible to rename: {} > {} | {}'.format(full_name, item.preview_name, exc))
    ...
    def _on_auto_rename(self, rule_name, tokens_dict):
            ...
            if auto_suffix:
                ...
                for i, obj_name in enumerate(objs_to_rename):
                    ...
                    if obj_uuid in solved_names:
                        tp.logger.warning('Node with name: "{} and UUID "{}" already renamed to "{}"! Skipping ...'.format(obj_name, obj_uuid, solved_names[obj_name]))
                        continue
                        obj_type = maya.cmds.objectType(obj_name)
                        if obj_type == 'transform':
                            shape_nodes = maya.cmds.listRelatives(obj_name, shapes=True, fullPath=True)
                            obj_type = shape_nodes or 'group'
                        else:
                        ...
                        if 'node_type' in token_dict:
                            if token_dict['node_type']:
                                node_type = token_dict.pop('node_type')
                        node_name = tp.Dcc.node_short_name(obj_name)
                        description = token_dict['description'] if ('description' in token_dict and token_dict['description']) else node_name
                        side = token_dict.get('side', None)
                        solved_name = self._name_lib.solve(description,
                          id=(len(objs_to_rename) - 1 - i), side=side, node_type=node_type)
                        if not solved_name:
                            pass
                        else:
        ...
        if solved_names:
            ...
            for obj_name in objs_to_rename:
                ...
                if not solve_name:
                    tp.logger.warning('Impossible to rename "{}" with rule "{}" | "{}"'.format(obj_name, rule_name, token_dict))
                else:
                    try:
    ...
    def _on_add_prefix(self, prefix_text):
        ...
        if not prefix_text:
            ...
            return tp.Dcc.add_name_prefix(prefix=prefix_text,
              filter_type=dcc_type,
    ...
    def _on_add_suffix(self, suffix_text):
        ...
        if not suffix_text:
            ...
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.add_name_suffix(suffix=suffix_text,
    ...
    def _on_remove_prefix(self):
        ...
        if not search_hierarchy and not selection_only:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.remove_name_prefix(filter_type=filter_type,
              search_hierarchy=search_hierarchy,
              selection_only=selection_only,
              rename_shape=rename_shape)
    ...
    def _on_remove_suffix(self):
        ...
        if not search_hierarchy and not selection_only:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.remove_name_suffix(filter_type=filter_type,
              search_hierarchy=search_hierarchy,
              selection_only=selection_only,
              rename_shape=rename_shape)
    ...
    def _on_remove_first(self, num_to_remove):
        ...
        if not num_to_remove or not num_to_remove > 0:
            return
        search_hierarchy, selection_only = self._get_filter_vars()
        if not search_hierarchy:
            if not selection_only:
                tp.logger.warning('Remove Suffix must be used with "Selected" option not with "All"')
                return
        filter_type = self._get_filter_type()
        rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
        filtered_obj_list = tp.Dcc.filter_nodes_by_type(filter_type=filter_type,
          search_hierarchy=search_hierarchy,
          selection_only=selection_only)
    ...
    def _on_replace_padding(self, pad):
        ...
        if not search_hierarchy and not selection_only:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.renumber_objects(filter_type=filter_type,
    ...
    def _on_change_padding(self, pad):
        ...
        if not search_hierarchy and not selection_only:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.change_suffix_padding(filter_type=filter_type,
              padding=(int(pad)),
              add_underscore=True,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
    ...
    def _on_side(self, side):
        if not side:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return tp.Dcc.add_name_suffix(suffix=side,
              filter_type=filter_type,
              search_hierarchy=search_hierarchy,
              add_underscore=True,
              selection_only=selection_only,
    ...
    def _on_add_replace_namespace(self, namespace_to_add):
        if not tp.is_maya() or not namespace_to_add:
            ...
            filter_type = self._get_filter_type()
            rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
            return namespace.assign_namespace_to_object_by_filter(namespace=namespace_to_add,
              filter_type=filter_type,
              force_create=True,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
    ...
    def _on_remove_namespace(self, namespace_to_remove=None):
        if not tp.is_maya():
            ...
            return namespace.remove_namespace_from_object_by_filter(namespace=namespace_to_remove,
              filter_type=filter_type,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
              selection_only=selection_only,
    ...
    def _on_auto_suffix(self):
        ...
        filter_type = self._get_filter_type()
        rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
        return tp.Dcc.auto_name_suffix(filter_type=filter_type,
          rename_shape=rename_shape,
          search_hierarchy=search_hierarchy,
          selection_only=selection_only)
    ...
    def _on_unique_name(self):
        ...
        filter_type = self._get_filter_type()
        rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
        return tp.Dcc.find_unique_name(filter_type=filter_type,
          do_rename=True,
          rename_shape=rename_shape,
          search_hierarchy=search_hierarchy,
          selection_only=selection_only)
    ...
    def _on_remove_all_numbers(self):
        ...
        filter_type = self._get_filter_type()
        rename_shape = self._auto_rename_shapes_cbx.isChecked() if self._auto_rename_shapes_cbx else True
        return tp.Dcc.remove_name_numbers(filter_type=filter_type,
          rename_shape=rename_shape,
          search_hierarchy=search_hierarchy,
          selection_only=selection_only,
          trailing_only=False)
