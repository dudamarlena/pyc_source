...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django_tlsauth/views.py
# Compiled at: 2013-04-03 12:01:29
...
def renderUserForm(request):
    ...
    if form.is_valid():
        return HttpResponse(settings.TLS_CA.gencert(str(form.cleaned_data['name']), str(form.cleaned_data['email']), str(form.cleaned_data['org'])), mimetype='application/x-pkcs12')
...
def renderCSRForm(request):
    ...
    if settings.TLS_BLINDSIGN:
        if not settings.TLS_SCRUTINIZER or settings.TLS_SCRUTINIZER(str(form.cleaned_data['csr'])):
            return HttpResponse(settings.TLS_CA.signcsr(str(form.cleaned_data['csr'])), mimetype='text/plain')
    settings.TLS_CA.submit(str(form.cleaned_data['csr']))
    return HttpResponse('Thank you. If all goes well you should soon receive your signed certificate.')
...
def showcsrs(request):
    ...
    if err:
        return err
    return render_to_response('csrs.html', {'certs': [ (todn(cert.get_subject()), datetime.datetime.fromtimestamp(os.stat(path).st_mtime), os.path.basename(path)) for cert, path in settings.TLS_CA.incoming()
              ]})
