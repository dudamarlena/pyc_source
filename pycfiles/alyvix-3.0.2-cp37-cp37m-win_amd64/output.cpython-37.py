...
# [GCC 8.4.0]
# Embedded file name: D:\python\envs\alyvix\alyvix_py37\lib\site-packages\alyvix\core\output.py
# Compiled at: 2019-11-11 12:27:44
# Size of source mod 2**32: 5650 bytes
import cv2, json, base64, copy, time, os
...
class OutputManager:
    ...
    def _build_json(self, json_object, chunk, object_list, exit, duration):
        ...
        for object in object_list:
            ...
            object_dict['measure'] = {'perfomance_ms':int(object.performance_ms), 
             ...
             'records':object.records,  'resolution':{'width':w, 
              'height':h}, 
        ...
        json_object['run'] = {'host':chunk['host'], 
         'user':chunk['user'],  'test':chunk['test'], 
         'code':chunk['code'],  'duration_s':round(duration, 3), 
         'exit':exit}
        return json_object
    ...
    def save(self, filename, json_object, chunk, object_list, exit, duration):
        with open(filename, 'w') as (f):
            json.dump((self._build_json(json_object, chunk, object_list, exit, duration)), f, indent=4, sort_keys=True,
              ensure_ascii=False)