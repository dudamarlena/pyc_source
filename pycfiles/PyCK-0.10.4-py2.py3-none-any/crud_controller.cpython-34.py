...
# Compiled at: 2015-11-12 15:14:02
# Size of source mod 2**32: 28760 bytes
import os.path
...
class CRUDController(object):
    ...
    list_actions = [{'link_text': 'Add {friendly_name}',  'link_url': 'add',  'css_class': 'btn btn-success'}]
    list_per_record_actions = [{'link_text': 'Details',  'link_url': 'details/{PK}'}, {'link_text': 'Edit',  'link_url': 'edit/{PK}'}, {'link_text': 'Delete',  'link_url': 'delete/{PK}',  'css_class': 'text-danger'}]
    detail_actions = [{'link_text': 'Edit',  'link_url': '../edit/{PK}'}, {'link_text': 'Delete',  'link_url': '../delete/{PK}',  'css_class': 'btn btn-danger'}]
    field_translations = {}
    ...
    def _get_search_condition(self, col, val, case_sensitive=True, partial_match=False):
        ...
        if col_type in (Time, TIME):
            ...
            if col_type in (DATETIME, DateTime, TIMESTAMP):
                try:
    ...
    def _list_csv_common_code(self, return_only_records=False, return_all_records=False):
        ...
        if return_only_records:
            ...
            return {'columns': columns, 
             'primary_key_columns': primary_key_columns,  'records': records, 
             'pages': pages,  'current_page': p,  'total_records': total_recs}
    ...
    def list(self):
        ...
        ret_dict = {'base_template': self.base_template, 
         'friendly_name': self.friendly_name,  'records_per_page': self.list_recs_per_page, 
         'list_field_args': self.list_field_args,  'field_translations': self.field_translations, 
        ...
        for R in records:
            ...
            for column in columns:
                if column in self.list_field_args and 'display_field' in self.list_field_args[column]:
                    ...
                    if self.field_translations:
                        if column in self.field_translations:
    ...
    def add(self):
        ...
        ret_dict = {'base_template': self.base_template,  'friendly_name': self.friendly_name,  'model_record_counts': self._models_rec_count_if_needed(), 
         'form': f, 
         'action_type': 'add'}
        return dict(list(ret_dict.items()) + list(self.template_extra_params.items()))
    ...
    def edit(self):
        ...
        ret_dict = {'base_template': self.base_template,  'model_record_counts': self._models_rec_count_if_needed(), 
         'friendly_name': self.friendly_name, 
         'form': f,  'action_type': 'edit'}
        return dict(list(ret_dict.items()) + list(self.template_extra_params.items()))
    ...
    def details(self):
        ...
        ret_dict = {'base_template': self.base_template,  'R': R,  'friendly_name': self.friendly_name, 
         'model_record_counts': self._models_rec_count_if_needed(), 
         'columns': columns, 
         'primary_key_columns': primary_key_columns,  'actions': self.detail_actions, 
