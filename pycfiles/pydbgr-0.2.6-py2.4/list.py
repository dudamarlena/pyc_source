...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pydbgr/processor/command/list.py
# Compiled at: 2013-03-17 12:02:14
            ...
            if args[0] == '-':
                ...
                if first == None:
                    if modfunc == None:
                        return (None, None, None)
                    if len(args) == 1:
                        if first is None and modfunc is not None:
                            first = 1
                        ...
                        if modfunc:
                            if first is None:
                                ...
                                if len(args) == 3 and modfunc:
                                    msg = 'last or count parameter expected, ' + 'got: %s.' % args[2]
                        ...
                        modfunc or self.errmsg(('At most 2 parameters allowed when no module' + ' name is found/given. Saw: %d parameters') % len(args))
                        return (None, None, None)
                else:
                    self.errmsg(('At most 3 parameters allowed when a module' + ' name is given. Saw: %d parameters') % len(args))
                    return (None, None, None)
                ...
                if line is None:
                    ...
                    if (
                     canonic_filename, lineno) in list(bplist.keys()):
                        ...
                        s += ' '
                        a_pad = '  '
                    if curframe and lineno == inspect.getlineno(curframe) and filename == curframe.f_code.co_filename:
                        s += '->'
                    else:
                        s += a_pad
                    self.msg(s + '\t' + line)
