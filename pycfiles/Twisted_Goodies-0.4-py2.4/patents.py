...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twisted_goodies/webfetch/patents.py
...
class Imagerator(object):
    ...
    def __init__(self, pubNum, dirPath, initialSchemePart, numPages, getter):
        ...
        (self.left, self.right) = re.split('PageNum\\=1', initialSchemePart.replace('\n', ''))
        self.pageFiles = []
...
class FetcherBase(object):
    ...
    re_tiff_src = re.compile('<embed\\ src\\="(/\\.DImg\\?Docid\\=[uU][sS]\\d+[ki\n]*&PageNum\\=1&\n?' + 'IDKey=[0-9A-F]+\n?&ImgFormat\\=tif)')
    ...
    def _getImageTools(self, schemePart):
        ...
        def second(m):
            if m is None:
                return FetchFailure('Error isolating image server and scheme part ' + 'from fulltext page')
    ...
    def _getImagerator(self, imageTools, validatedPubNumber):
        ...
        def gotImagePage(html):
            ...
            tiff_src = m.group(1)
            return Imagerator(validatedPubNumber, self.dirPath, tiff_src, numPages, getter)
...
class PatentFetcher(FetcherBase):
    ...
    """
    __module__ = __name__
    spec = ('patft.uspto.gov', '/netacgi/nph-Parser?patentnumber=%d', 'URL\\=(/netacgi/nph-Parse.+?)">',
            'a\\ href\\=http://(patimg.+)(/\\.piw.+?)>')

    @staticmethod
    def parsePubNumber(pubNumberString):
        """
...
class AppFetcher(FetcherBase):
    ...
    __module__ = __name__
    spec = (
     'appft1.uspto.gov', '/netacgi/nph-Parser?Sect1=PTO2&Sect2=HITOFF' + '&u=%%2Fnetahtml%%2FPTO%%2Fsearch-adv.html&r=0&p=1&' + 'f=S&l=50&Query=DN%%2F%d&d=PG01', 'HREF\\=(/netacgi/nph-Parse.+?)"?>', 'a\\s+href\\=http://(aiw.+)(/\\.aiw\\?.+?idkey\\=[0-9A-F]+)')
