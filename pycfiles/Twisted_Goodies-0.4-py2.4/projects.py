...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twisted_goodies/simpleserver/http/resources/projects.py
# Compiled at: 2007-08-01 15:09:33
"""
...
class APIDocResource(Mixin, Resource):
    ...
    """
    __module__ = __name__
    re_rev = re.compile('Revision:\\s+(\\d+)')
    ...
    def locateChild(self, request, segments):
        ...
        d = defer.maybeDeferred(self.ensureDocsPresent, projectName, apiFile)
        d.addCallback(ready)
        return d
    ...
    def svnExport(self, tmpDir, projectName):
        ...
        packageDir = os.path.realpath(os.path.join(tmpDir, self._packageName(projectName)))
        args = [
         'export', '-q', '--force', repo, packageDir]
    ...
    def pydoctor(self, tmpDir, projectName):
        ...
        args = ['--make-html', '--project-name=%s' % projectName, '--add-package=%s' % self._packageName(projectName), '--html-output=%s/api/%s' % (self.vhostPath, projectName), '--project-url=%s' % url]
        return utils.getProcessValue(self._executable('pydoctor'), args=args, path=tmpDir)
...
class DownloadResource(Mixin, StanResource):
    ...
    style = {'H1': {'margin-bottom': '0px'}, 'H2': {'margin-bottom': '0px'}, 'code': {'padding': '0.3em', 'margin-left': '0.5em', 'font-size': 'larger', 'font-style': 'bold', 'font-family': 'monospace'}}
    ...
    def _sectionEasyInstall(self):
        if '%s.ez_setup' % self.projectName in self._pertinentFiles()[1]:
            return [
             T.h2['Setup Tools (the easiest way)'], T.p(id='code')[('easy_install %s' % self.projectName)]]
        return []

    def _sectionSVN(self):
        url = self.projectRepoProto % self.projectName
        return [T.h2['SVN Checkout'], T.p(id='code')[('svn co %s %s' % (url, self.projectName))], T.p[('The current source code ("trunk") will appear in the ' + '"%s" subdirectory. ' % self.projectName + 'You can go there and install with')], T.p(id='code')['sudo python setup.py install']]
    ...
    def _sectionInstallFiles(self):
        ...
        if not listing:
            return []
        return [
         T.h2['Install Files'], T.p[('You can download one of the following files and use it to ' + 'install %s, either by ' % self.projectName + 'executing them (for Windows ' + '".exe" files) or extracting their archived files and ' + 'using the setup.py script.')], T.ul[listing]]

    def render(self, request):
        """
        Returns the body of my HTML.
        """
        msg = 'There are a couple of different ways to get up and running ' + 'with a copy of %s:' % self.projectName
        body = [
         T.img(src='/banner.png'), T.hr, T.h1[self.title], T.p[msg]]
