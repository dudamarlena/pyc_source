...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twisted_goodies/simpleserver/service.py
...
"""
import pwd, grp, os, os.path, imp, configobj, pkg_resources
...
class MasterService(service.Service):
    ...
    def __init__(self, configPath):
        ...
        self.application = service.Application('SIMPLESERVER', uid=self.uid, gid=self.gid)
            ...
            for portName in ('tcp', 'ssl'):
                ...
                if portName == 'tcp':
                    ...
                    if 'ctx' not in locals():
                        ctx = ssl.DefaultOpenSSLContextFactory(self.config['private key'], self.config['certificate'])
                    serviceObject = internet.SSLServer(port, factory, ctx)
                serviceName = '%s-%s' % tuple([ x.upper() for x in (key, portName) ])
