...
# Embedded file name: build/bdist.macosx-10.6-universal/egg/ctler.py
# Compiled at: 2012-03-13 16:44:48
import os, re, time, datetime, operator, numpy as np
        ...
        for line in self.ctl.split('\n'):
            ...
            if read:
                ...
                if levels[0] > 0:
                    ...
                    size = self.dimensions['time'] * self.dimensions['levels'] * (SPACE + 2)
                else:
                    var.dimensions = ('time', 'latitude', 'longitude')
                    size = self.dimensions['time'] * (SPACE + 2)
                var.shape = tuple(self.dimensions[dim] for dim in var.dimensions)
                var.data = self.data[i:i + size].reshape(-1, SPACE + 2)[:, 1:-1].reshape(var.shape)
                if self.yrev:
                    var.data = var.data[..., ::-1, :]
                i += size
                units = int(m.group(3))
                ...
                var.attributes = {'long_name': m.group(4).strip(), 
