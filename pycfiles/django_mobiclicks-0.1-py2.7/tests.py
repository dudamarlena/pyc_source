...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mobiclicks/tests.py
...
class MiddlewareTestCase(RequestFactoryTestCase):
    ...
    def test_store_token_default_settings(self):
        ...
        self.assertEquals(self.session[conf.CPA_TOKEN_SESSION_KEY], self.cpatoken)

    def test_store_token_custom_settings(self):
        with self.settings(MOBICLICKS={'CPA_TOKEN_SESSION_KEY': 'foo_session_key', 
    ...
    def test_click_confirmation(self, mock_get):
        click_ref = 'foo'
        with self.settings(MOBICLICKS={'CONFIRM_CLICKS': True, 
           'CPA_SECURITY_TOKEN': 'foo'}):
            request = self.make_request('/?pollen8_click_ref=%s' % click_ref)
            self.middleware.process_request(request)
            mock_get.assert_called_once_with(conf.CLICK_CONFIRMATION_URL, params={'action': 'clickReceived', 
               'clickRef': click_ref, 
               'authKey': conf.CPA_SECURITY_TOKEN})
        with self.settings(MOBICLICKS={'CONFIRM_CLICKS': False, 
           'CPA_SECURITY_TOKEN': 'foo'}):
            mock_get.reset()
            self.middleware.process_request(request)
    ...
    def test_click_confirmation_duplicate_on_redirect(self):
        with self.settings(MOBICLICKS={'CONFIRM_CLICKS': True, 
           'CPA_SECURITY_TOKEN': 'foo'}, LANGUAGES=(('en-us', 'English'), ), ROOT_URLCONF=i18n_patterns('', url('', lambda r: r))):
            ...
            response = locale_middleware.process_response(request, HttpResponseNotFound())
...
class RegistrationTrackingTestCase(RequestFactoryTestCase):
    ...
    def create_user(self, username='foo'):
        try:
            return User.objects.create_user(username, '%s@example.com' % username, 'password')
        except TypeError:
    ...
    def test_track_registration_on_login(self, mock_get):
        with self.settings(MOBICLICKS={'TRACK_REGISTRATIONS': True, 
           'CPA_SECURITY_TOKEN': 'foo'}):
            request = self.make_request('/join/')
            user = self.create_user()
            user_logged_in.send(sender=User, user=user, request=request)
            mock_get.assert_called_once_with(conf.ACQUISITION_TRACKING_URL, params={'cpakey': conf.CPA_SECURITY_TOKEN, 
               'code': self.cpatoken})
            self.assertNotIn(conf.CPA_TOKEN_SESSION_KEY, self.session)
