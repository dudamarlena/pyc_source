...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/formish/tests/testish/testish/lib/forms.py
# Compiled at: 2010-02-16 11:28:18
import logging, os.path, subprocess, formish, schemaish, validatish
...
def form_MinLengthCheckboxMultiChoice(request):
    ...
    """
    schema = schemaish.Structure()
    schema.add('multiChoice', schemaish.Sequence(schemaish.String(validator=validatish.Length(min=2))))
    options = [
     (1, 'a'), (2, 'b'), (3, 'c')]
    form = formish.Form(schema, 'form')
...
def form_All(request):
    ...
    """
    schema = schemaish.Structure()
    schema.add('minAge', schemaish.Integer(validator=validatish.All(validatish.Required(), validatish.Integer(), validatish.Range(min=18))))
    form = formish.Form(schema, 'form')
    return form
...
def form_Any(request):
    ...
    """
    schema = schemaish.Structure()
    schema.add('noTeenagers', schemaish.Integer(validator=validatish.Any(validatish.Range(max=12), validatish.Range(min=20))))
    form = formish.Form(schema, 'form')
    return form
...
def form_SequenceOfDateParts(request):
    ...
    """
    schema = schemaish.Structure()
    schema.add('birthdays', schemaish.Sequence(schemaish.Structure([
     (
