...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/formish/safefilename.py
# Compiled at: 2009-02-11 06:25:22
    ...
    while i < input_length:
        ...
        if c in safe_characters:
            ...
            if i == input_length:
                ...
                continue
            if input[i] != ')':
                handle_problematic_characters(errors, input, i, i + 1, "invalid character '%s' in parentheses sequence" % input[i])
                continue
        elif c == '{':
            end_position = input.find('}', i)
            if end_position == -1:
                end_position = i + 1
                while end_position < input_length and input[end_position] in '0123456789abcdef' and end_position - i <= 8:
                    end_position += 1

                output += handle_problematic_characters(errors, input, i, end_position, 'open backet was never closed')
                i = end_position
                continue
            else:
                try:
                    ...
                    output += handle_problematic_characters(errors, input, i, end_position + 1, 'invalid data between brackets')
