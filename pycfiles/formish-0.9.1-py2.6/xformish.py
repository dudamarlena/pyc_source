...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/formish/tests/testish/testish/lib/xformish.py
...
class ApproximateDateParts(widgets.DateParts):
    ...
    def from_request_data(self, field, data):
        parts = (
         data['year'][0].strip(), data['month'][0], data['day'][0])
...
class ReCAPTCHA(widgets.Input):
    ...
    def pre_parse_incoming_request_data(self, field, request_data):
        ...
        return {'recaptcha_challenge_field': full_request_data['recaptcha_challenge_field'], 'recaptcha_response_field': full_request_data['recaptcha_response_field']}

    def from_request_data(self, field, data):
        print 'in from ', data
        params = urllib.urlencode({'privatekey': self.privatekey, 
           'remoteip': self.remoteip, 
           'challenge': data['recaptcha_challenge_field'][0], 
           'response': data['recaptcha_response_field'][0]})
        request = urllib2.Request(url='http://%s/verify' % self.VERIFY_SERVER, data=params, headers={'Content-type': 'application/x-www-form-urlencoded', 
