...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/formish/tests/unittests/test_html.py
# Compiled at: 2010-03-01 05:16:45
import unittest, webob, schemaish, formish
...
class TestHTML(unittest.TestCase):
    ...
    def test_complex_form(self):
        ...
        f['two.a'].widget = formish.SelectChoice([('opt1', 'Options 1'), ('opt2', 'Option 2')], none_option=('-select option-',
                                                                                                             None))
    ...
    def test_complex_error_all(self):
        schema = schemaish.Structure([
         ('one',
          schemaish.Integer(validator=v.All(v.Required(), v.Integer(), v.Range(min=18), v.Range(min=20))))])
        f = formish.Form(schema, name='form', add_default_action=False)
        f.add_action('submit', 'Submit Me')
        r = webob.Request.blank('http://localhost/', environ={'REQUEST_METHOD': 'POST'})
        r.POST['__formish_form__'] = 'form'
    ...
    def test_complex_error_all_required_2(self):
        schema = schemaish.Structure([
         ('one',
          schemaish.Integer(validator=v.All(v.Required(), v.Integer(), v.Any(v.Range(min=18), v.Range(min=20)))))])
        f = formish.Form(schema, name='form', add_default_action=False)
        f.add_action('submit', 'Submit Me')
        r = webob.Request.blank('http://localhost/', environ={'REQUEST_METHOD': 'POST'})
        r.POST['__formish_form__'] = 'form'
