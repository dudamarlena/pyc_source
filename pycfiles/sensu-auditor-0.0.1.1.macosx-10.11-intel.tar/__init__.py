...
# [GCC 8.4.0]
# Embedded file name: /Users/benwaters/sensu-auditor/lib/python2.7/site-packages/sensu_auditor/__init__.py
# Compiled at: 2016-09-05 08:12:08
...
import gzip, logging
from defaults import CONFIG_PATH, SENSU_LOG_FOLDER, LOG_LOCATION, LOG_TYPES, MESSAGES, DEFAULT_CONFIG_FIELDS, DEFAULT_SECTIONS
...
class SensuAuditor(object):
            ...
            for entry in log_file:
                ...
                if message == MESSAGES['PUBLISHED_MESSAGE']:
                    if (self.today - timestamp).days < int(self.days):
                        ...
                        if status != 0 and check_name in self.checks and check_name not in self.unhealthy_checks.names():
                            ...
                            entry = RawReportableEntry(check_name=done.check_name, start_time=done.start_time, end_time=timestamp, category=self.report.get_group_by_check(check_name))
    ...
    def upload(self):
        ...
        try:
            es.index(index=index, doc_type='sensu-audit', body={'hostname': self.report.hostname, 'total_downtime': self.report.total_downtime, 
               'timestamp': str(self.today.strftime('%Y-%m-%dT%H:%M:%S.%fZ')), 
               'affected': self.report.content}, timestamp=self.today)
