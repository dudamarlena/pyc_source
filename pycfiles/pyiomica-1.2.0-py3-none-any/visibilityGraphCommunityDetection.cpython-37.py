...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_MSU_NASA\VS\pyiomica\pyiomica\visibilityGraphCommunityDetection.py
# Compiled at: 2020-02-24 11:04:11
    ...
    for i in range(dimension):
        if i < dimension - 1:
            if weight == 'time':
                ...
                if weight == 'tan':
                    A[(i, i + 1)] = A[(i + 1, i)] = abs((data[i] - data[(i + 1)]) / (times[i] - times[(i + 1)])) + 1e-08
                else:
                    if weight == 'distance':
        ...
        for j in range(i + 2, dimension):
            if np.max(data[i + 1:j]) < min(data[i], data[j]):
                if weight == 'time':
                    ...
                    A[(i, j)] = A[(j, i)] = abs((data[i] - data[j]) / (times[i] - times[j])) + 1e-08
...
def communityDetectByPathLength(G, direction=None, cutoff=None):
    ...
    PL_dict = dict(nx.all_pairs_dijkstra_path_length(G))
    value_PL_list = []
    for node1 in range(len(PL_dict)):
        pl_node1 = PL_dict[node1]
        for node2 in range(node1 + 1, len(PL_dict)):
            value_PL_list.append(pl_node1[node2])

    nodelist = list(G.nodes)
    nodelist.sort()
    PL_node0_node_end = nx.dijkstra_path(G, nodelist[0], nodelist[(-1)])
    community = {}
