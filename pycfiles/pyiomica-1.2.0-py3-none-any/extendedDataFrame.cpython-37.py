...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_MSU_NASA\VS\pyiomica\pyiomica\extendedDataFrame.py
# Compiled at: 2020-02-28 11:06:39
# Size of source mod 2**32: 27165 bytes
...
class DataFrame(pd.DataFrame):
    ...
    def filterOutFractionZeroSignals(self, min_fraction_of_non_zeros, inplace=False):
       ...
       """
        print('Filtering out low-quality signals (with more than %s%% zero points)' % np.round(100.0 * (1.0 - min_fraction_of_non_zeros), 3))
    ...
    def filterOutFractionMissingSignals(self, min_fraction_of_non_missing, inplace=False):
       ...
       """
        print('Filtering out low-quality signals (with more than %s%% missing points)' % np.round(100.0 * (1.0 - min_fraction_of_non_missing), 3))
    ...
    def quantileNormalize(self, output_distribution='original', averaging=np.mean, ties=np.mean, inplace=False):
        ...
        if output_distribution == 'original':
            ...
            def rankTransform(series, weights):
                se_temp = pd.Series(index=scipy.stats.rankdata((series.values), method='min'), data=(weights[(scipy.stats.rankdata((series.values), method='ordinal') - 1)]))
            ...
            if not output_distribution == 'normal':
...
def getRandomSpikesCutoffs(df_data, p_cutoff, NumberOfRandomSamples=1000):
    ...
    df_data_random.removeConstantSignals(0.0, inplace=True)
...
def getRandomAutocorrelations(df_data, NumberOfRandomSamples=100000, NumberOfCPUs=4, fraction=0.75, referencePoint=0):
    ...
    results = utilityFunctions.runCPUs(NumberOfCPUs, coreFunctions.pAutocorrelation, [(df_data_random.iloc[i].index.values.copy(), df_data_random.iloc[i].values.copy(), df_data.columns.values.copy()) for i in range(df_data_random.shape[0])])
    return pd.DataFrame(data=(results[1::2]), columns=(results[0]))


def getRandomPeriodograms(df_data, NumberOfRandomSamples=100000, NumberOfCPUs=4, fraction=0.75, referencePoint=0):
