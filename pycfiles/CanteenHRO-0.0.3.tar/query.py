...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/model/query.py
# Compiled at: 2014-10-03 07:13:34
__doc__ = '\n\n  model queries\n  ~~~~~~~~~~~~~\n\n  :author: Sam Gammon <sg@samgammon.com>\n  :copyright: (c) Sam Gammon, 2014\n  :license: This software makes use of the MIT Open Source License.\n      A copy of this license is included as ``LICENSE.md`` in\n      the root of the project.\n\n'
__version__ = 'v5'
import abc, base64, operator
from canteen.util.struct import EMPTY, Sentinel
...
AND = Sentinel('AND')
OR = Sentinel('OR')
_operator_map = {EQUALS: operator.eq, 
...
class QueryOptions(object):
    ...
    def __init__(self, **kwargs):
        ...
        map(lambda bundle: self._set_option(*bundle), map(lambda slot: (slot, kwargs.get(slot, EMPTY)), self.option_names))
    ...
    def __repr__(self):
        ...
        return 'QueryOptions(%s)' % (', ').join(map(lambda (k, v): ('=').join((k, str(v))), properties))
    ...
    def _set_option(self, name, value=EMPTY, _setter=False):
        ...
        if not isinstance(name, basestring):
            raise ValueError('Argument `name` of `_set_option` must be a string internal propery name. Got: "%s".' % str(name))
        name = '_' + unicode(name) if name[0] != '_' else unicode(name)
        if name not in self.__slots__:
            raise AttributeError('`QueryOptions` object has no option by the name "%s".' % name)
    ...
    def _get_option(self, name, default=EMPTY):
        ...
        :returns: Configuration value at ``name``, or ``default`` if no value
          was found. """
        if not isinstance(name, basestring):
            raise ValueError('Argument `name` of `_get_option` must be a string internal property name. Got: "%s".' % name)
        name = '_%s' % name
        if name not in self.__slots__:
            raise AttributeError('`QueryOptions` object has no option by the name "%s".' % name)
    ...
    def pack(self, encode=True):
        ...
        items = tuple(items)
        if encode:
            return base64.b64encode((':').join(map(unicode, items)))
        else:
            return items
    ...
    plan = property(lambda self: self._get_option('plan'), lambda self, v: self._set_option('plan', v, _setter=True))
    cursor = property(lambda self: self._get_option('cursor'), lambda self, v: self._set_option('cursor', v, _setter=True))
...
class GraphQueryOptions(QueryOptions):
    """ Specifies Graph-related query options in addition to standard ``Query``
      properties. """
    magic_symbol = 1
    options = QueryOptions.options | frozenset(('_base', ))
    __slots__ = QueryOptions.options | options
    option_names = frozenset(('_').join(opt.split('_')[1:]) for opt in options)
    _defaults = dict(QueryOptions._defaults, **{'_graph_base': None})
...
class Query(AbstractQuery):
    ...
    def __init__(self, kind=None, filters=None, sorts=None, **kwargs):
        ...
        filters = filters and ([filters] if not isinstance(filters, (list, tuple)) else filters) or None
        sorts = sorts and ([sorts] if not isinstance(sorts, (list, tuple)) else sorts) or None
    ...
    def _execute(self, options=None, adapter=None, **kwargs):
        ...
        if self.adapter:
            return self.adapter._execute_query(self)
        if self.kind:
    ...
    def filter(self, expression):
        ...
        if isinstance(expression, Filter):
            ...
            raise NotImplementedError('Query method `filter` does not yet support non-`Filter` component types.')
        return self
    ...
    def sort(self, expression):
        ...
        if isinstance(expression, Sort):
            ...
            raise NotImplementedError('Query method `sort` does not yet support non-`Sort` component types.')
        return self
    ...
    def fetch(self, adapter=None, **options):
        ...
        return self._execute(options=QueryOptions(**options) if options else None, adapter=adapter)
    ...
    def pack(self, encode=True):
        ...
        if encode:
            return base64.b64encode((',').join(bundles))
        return bundles
...
class Filter(QueryComponent):
    ...
    def __init__(self, prop, value, AND=None, OR=None, type=PROPERTY, operator=EQUALS):
        ...
        from canteen import model
        value = model.AbstractModel._PropertyValue(value)
        ...
        if AND or OR:
            for var in (AND, OR):
                self.chain += var if isinstance(var, (list, tuple, set, frozenset)) else [var]
    ...
    def __repr__(self):
        ...
        :returns: """
        return 'Filter(%s %s %s)' % (
         self.sub_operator.name + ' ' if self.sub_operator else '' + getattr(self.target, 'name', self.generic_name),
...
class KeyFilter(Filter):
    ...
    def __init__(self, value, AND=None, OR=None, _type=KEY_KIND):
        ...
        from canteen import model
        if not (isinstance(value, (model.Key, model.Model)) or isinstance(value, type) and issubclass(value, model.Model) or value is None):
            raise ValueError('`KeyFilter` value must be a `Key`, model class, or `None`. Instead, got: "%s".' % repr(value))
        elif _type is self.KIND:
            value = value and ((isinstance(value, model.Key) or value.kind)() if 1 else value.kind)
        elif _type is self.ANCESTOR:
            value = value and (value.urlsafe() if isinstance(value, model.Key) else value.key.urlsafe())
...
class EdgeFilter(Filter):
    ...
    magic_symbol = 4
    items = ('kind', 'operator', 'tails', 'value')
    EDGES = EDGES
    NEIGHBORS = NEIGHBORS
    tails = None

    def __init__(self, value, tails=None, AND=None, OR=None, type=EDGES):
