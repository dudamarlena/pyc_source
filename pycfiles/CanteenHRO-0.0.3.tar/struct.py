...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/util/struct.py
...
_ENUM_TYPES = (
 int, long, float, basestring, bool)
...
class Sentinel(object):
    ...
    __eq__ = lambda self, other: isinstance(other, self.__class__) and other.hash == self.hash
...
EMPTY, _TOMBSTONE = Sentinel('EMPTY', True), Sentinel('TOMBSTONE', True)
...
class UtilStruct(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if cls.__name__ is 'UtilStruct':
            raise NotImplementedError('Cannot construct `UtilStruct` directly as it is an abstract class.')
    ...
    def fillStructure(self, struct, case_sensitive=False, **kwargs):
        ...
        :raises:
        :returns: """
        raise NotImplementedError('`UtilStruct.fillStructure` is abstract and must be implemented by a subclass.')
...
class ObjectProxy(UtilStruct):
    ...
    def __getitem__(self, name):
        ...
        if filtered not in self._entries and name not in self._entries:
            raise KeyError("Cannot locate name '%s' in ObjectProxy '%s'." % (
             name, self))
...
class CallbackProxy(ObjectProxy):
    ...
    def __getitem__(self, item):
        ...
        if self._entries and item not in self._entries:
            raise KeyError("Could not retrieve item '%s' from CallbackProxy '%s'." % (
             item, self))
...
class BidirectionalEnum(object):
    ...
    class __metaclass__(type):
        ...
        def __new__(mcs, name, chain, _map):
          ...
          :returns: Constructed ``BidirectionalEnum`` subclass. """
            _pmap, _keys, _plookup = _map['_pmap'], _map['_keys'], _map['_plookup'] = ({}, [], set())
            for key, value in _map.iteritems():
                if not key.startswith('_') and isinstance(value, _ENUM_TYPES):
                    if value in _plookup:
                        raise RuntimeError('Cannot map non-unique (double-validated) `BidirectionalEnum` value %s.' % value)
                    if key in _plookup:
                        raise RuntimeError('Cannot map non-unique (double-validated) `BidirectionalEnum` key %s.' % value)
                    _keys.append(key)
                    _pmap[key] = value
                    _pmap[value] = key
                    _plookup.add(key)
        ...
        def __setitem__(cls, item, value):
          ...
          :raises NotImplementedError: Always, as ``BidirectionalEnum`` is an
            immutable type. """
            raise NotImplementedError('`%s` is an immutable type and cannot be written to with item syntax. ' % cls.__name__)
        ...
        def __setattr__(cls, attr, value):
          ...
          :raises NotImplementedError: Always, as ``BidirectionalEnum`` is an
            immutable type. """
            raise NotImplementedError('`%s` is an immutable type and cannot be written to with attribute syntax. ' % cls.__name__)
    ...
    def __new__(cls):
        ...
        raise TypeError('`%s` objects are abstract and only usable as type objects.' % cls.__name__)

    reverse_resolve = classmethod(lambda cls, code: cls._pmap.get(code, False))
    forward_resolve = resolve = classmethod(lambda cls, flag: cls.__getattr__(flag, False))
    ...
    __repr__ = classmethod(lambda cls: ('::').join([
     '<%s' % cls.__name__,
     (',').join([ block for block in (('=').join([str(k), str(v)]) for k, v in cls.__serialize__().items()) ]),
     'BiDirectional>']))