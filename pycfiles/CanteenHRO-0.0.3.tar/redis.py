...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/model/adapter/redis.py
# Compiled at: 2014-10-11 12:14:38
__doc__ = '\n\n  redis model adapter\n  ~~~~~~~~~~~~~~~~~~~\n\n  :author: Sam Gammon <sg@samgammon.com>\n  :copyright: (c) Sam Gammon, 2014\n  :license: This software makes use of the MIT Open Source License.\n            A copy of this license is included as ``LICENSE.md`` in\n            the root of the project.\n\n'
import json, base64, datetime, collections
from operator import itemgetter
from . import abstract
from .abstract import IndexedModelAdapter, DirectedGraphAdapter
...
try:
    import fakeredis
    _support.fakeredis = True
except ImportError:
    fakeredis, _support.fakeredis = None, False

try:
    import gevent
    _support.gevent = gevent
except ImportError:
...
class RedisMode(object):
    ...
    hashkey_hash = 'hashkey'
    hashkey_blob = 'hashblob'
    hashkind_blob = 'hashkind'
    toplevel_blob = 'toplevel'
...
class RedisAdapter(DirectedGraphAdapter):
    ...
    def __repr__(cls):
        ...
        return '%s(mode=%s, serializer=%s, compression=%s)' % (
         ...
         cls.EngineConfig.serializer and cls.EngineConfig.serializer.__name__,
         cls.EngineConfig.compression and cls.EngineConfig.compression.__name__)
    ...
    def __init__(self):
        ...
        if not _default_profile:
            ...
            if not _default_profile:
                ...
                _server_profiles['__default__'] = {'host': '127.0.0.1', 
                   'port': 6379}
    ...
    def channel(cls, kind):
        ...
        if not cls.adapter or not _support.redis:
            raise RuntimeError('No support detected in the current environment for Python Redis. Please `pip install redis`.')
        if not (__debug__ and cls.__testing__):
            impl, pool, profile = cls.adapter.StrictRedis, cls.adapter.ConnectionPool, None
            ...
            if kind in _profiles_by_model.get('index', set()):
                ...
                if 'unix_socket_path' in profile:
                    client = _client_connections[kind] = impl(**profile)
                    return client
                max_connections = profile.get('max_connections', 1000)
                if 'max_connections' in profile:
                    del profile['max_connections']
                client = _client_connections[kind] = pool(max_connections=max_connections, **profile)
    ...
    def execute(cls, operation, kind, *args, **kwargs):
        ...
        try:
            ...
            if isinstance(target, (_redis_client.client.Pipeline,
             _redis_client.client.StrictPipeline)) or fakeredis and isinstance(target, fakeredis.FakePipeline):
    ...
    def get(cls, key, pipeline=None, _entity=None):
        ...
        if key:
            ...
            if cls.EngineConfig.mode == RedisMode.toplevel_blob:
                result = _entity or cls.execute(cls.Operations.GET, flattened[1], encoded, target=pipeline)
                ...
                result = _entity or cls.execute(target=pipeline, *(cls.Operations.HASH_GET, flattened[1], cls.encode_key(j_kinded, f_kinded), cls.encode_key(tail, flattened)))
                ...
                result = _entity or cls.execute(target=pipeline, *(cls.Operations.HASH_GET, flattened[1], cls.encode_key(*root.flatten(True)), cls.encode_key(tail, flattened)))
        ...
        if keys:
            ...
            results, calls, bundles, handler = ({}, [], [],
             {RedisMode.toplevel_blob: cls.Operations.MULTI_GET, 
                ...
                RedisMode.hashkey_hash: cls.Operations.HASH_GET_ALL}.get(cls.EngineConfig.mode))
            for _k in keys:
                ...
                if cls.EngineConfig.mode == RedisMode.toplevel_blob:
                    ...
                    j_kinded, f_kinded = model.Key(flattened[1]).flatten(True)
                    tail = joined.replace(j_kinded, '')
                    encoded_root, encoded_tail = cls.encode_key(j_kinded, f_kinded), cls.encode_key(tail, flattened)
                    ...
                    root = (ancestor for ancestor in desired_key.ancestry).next()
                    tail = desired_key.flatten(True)[0].replace(root.flatten(True)[0], '') or '__root__'
                    encoded_root, encoded_tail = cls.encode_key(*root.flatten(True)), cls.encode_key(tail, flattened)
                    bundles.append((flattened[1], encoded_root, encoded_tail, _k))
                elif cls.EngineConfig.mode == RedisMode.hashkey_hash:
                    raise NotImplementedError('Redis mode not implemented: "hashkey_hash.')
            ...
            for read in bundles:
                ...
                elif cls.EngineConfig.mode == RedisMode.hashkind_blob or cls.EngineConfig.mode == RedisMode.hashkey_blob:
            ...
            with pipeline as (pipe):
                if cls.EngineConfig.mode == RedisMode.toplevel_blob:
                    for kind in kinds:
                        ...
                        cls.execute(handler, kind, target=pipe, *[ i for i in keys[kind] ])

                else:
                    if cls.EngineConfig.mode == RedisMode.hashkind_blob or cls.EngineConfig.mode == RedisMode.hashkey_blob:
                        for root in keys:
                            _expected, tails = [ r for r in requested[root] ], [ tail for tail in keys[root] ]
                            ...
                            cls.execute(_target_handler, '__meta__', root, target=pipe, *tails)

                    resultset = pipe.execute()
                    for keygroup, item in zip(expected, resultset):
                        if not isinstance(item, (tuple, list)):
                            item = (
                             item,)
                        for key, entity in zip(keygroup, item):
                            results[key] = cls.inflate(entity) if isinstance(entity, basestring) else entity
                    ...
                    for key in requested_keys:
                        ...
                        if not entity:
                            ...
                            encoded, flattened = key
                            entity['key'] = cls.registry[flattened[1]].__keyclass__.from_raw(base64.b64decode(encoded))
                            inflated_results.append(cls.registry[flattened[1]](_persisted=True, **entity))
    ...
    def _put(self, entity, **kwargs):
        ...
        if 'pipeline' in kwargs:
            ...
            pipeline = self.channel(entity.kind()).pipeline(transaction=True)
        if not entity.key.id:
            entity.key.id = self.allocate_ids(entity.__keyclass__, entity.kind())
        with pipeline as (pipe):
            written_key = super(IndexedModelAdapter, self)._put(entity, pipeline=pipe, **kwargs)
            origin, meta, property_map, graph = self.generate_indexes(entity.key, entity, _indexed_properties)
            self.write_indexes((origin, meta, property_map), graph, pipeline=pipe, **kwargs)
            pipe.execute()
            return written_key
    ...
    def put(cls, key, entity, model, pipeline=None):
        ...
        serialized = entity if isinstance(entity, dict) else entity.to_dict(convert_datetime=False, convert_keys=True, convert_models=True)
        ...
        if cls.EngineConfig.mode == RedisMode.toplevel_blob:
            if cls.execute(target=pipeline, *(cls.Operations.SET, flattened[1], joined, serialized)):
        ...
        if cls.EngineConfig.mode == RedisMode.hashkind_blob:
            ...
            if cls.execute(target=pipeline, *(cls.Operations.HASH_SET, flattened[1], cls.encode_key(*kinded), cls.encode_key(tail, flattened), serialized)):
                entity._set_persisted(True)
                return entity.key
            raise RuntimeError('Failed to write entity "%s" to key "%s".' % (
             str(entity), str(key) or '<none>'))
            ...
            if cls.execute(target=pipeline, *(cls.Operations.HASH_SET, flattened[1], cls.encode_key(*root), cls.encode_key(tail, flattened), serialized)):
                entity._set_persisted(True)
                return entity.key
            raise RuntimeError('Failed to write entity "%s" to key "%s".' % (
             str(entity), str(key) or '<none>'))
    ...
    def delete(cls, key, pipeline=None):
        ...
        if cls.EngineConfig.mode == RedisMode.toplevel_blob:
            return cls.execute(target=pipeline, *(cls.Operations.DELETE, flattened[1], cls.encode_key(joined, flattened)))
        if cls.EngineConfig.mode == RedisMode.hashkind_blob:
            kinded = model.Key(flattened[1]).flatten(True)
            tail = joined.replace(kinded[0], '')
            return cls.execute(target=pipeline, *(cls.Operations.HASH_DELETE, flattened[1], cls.encode_key(*kinded), cls.encode_key(tail, flattened)))
        if cls.EngineConfig.mode == RedisMode.hashkey_blob:
            desired_key = model.Key.from_raw(joined)
            root = (ancestor for ancestor in desired_key.ancestry).next()
            tail = desired_key.flatten(True)[0].replace(root.flatten(True)[0], '') or '__root__'
            return cls.execute(target=pipeline, *(cls.Operations.HASH_DELETE, flattened[1], cls.encode_key(*root.flatten(True)), cls.encode_key(tail, flattened)))
        if cls.EngineConfig.mode == RedisMode.hashkey_hash:
            raise NotImplementedError('Redis mode not implemented: "hashkey_hash".')
        raise NotImplementedError("Unknown storage mode: '%s'." % cls.EngineConfig.mode)
    ...
    def allocate_ids(cls, key_class, kind, count=1, pipeline=None):
        ...
        if cls.EngineConfig.mode == RedisMode.toplevel_blob:
            ...
            value = cls.execute(target=pipeline, *(cls.Operations.HASH_INCREMENT, kinded_key.kind, key_root_id, cls._id_prefix, count))
        else:
            if cls.EngineConfig.mode in (
             RedisMode.hashkind_blob,
             RedisMode.hashkey_blob):
                tail = cls._magic_separator.join([cls._meta_prefix, 'id'])
                value = cls.execute(target=pipeline, *(cls.Operations.HASH_INCREMENT, flattened[1], cls.encode_key(joined, flattened), cls.encode_key(tail, flattened), count))
            elif cls.EngineConfig.mode == RedisMode.hashkey_hash:
                raise NotImplementedError('Redis mode not implemented: "hashkey_hash".')
            else:
                raise NotImplementedError("Unknown storage mode: '%s'." % cls.EngineConfig.mode)
    ...
    @classmethod
    def encode_key(cls, joined, flattened=None):
    ...
    def write_indexes(cls, w, g, pipeline=None, execute=True):
        ...
        handler, ekey_encoder, vkey_encoder = cls.Operations.SET_ADD, cls._index_basetypes[EdgeKey], cls._index_basetypes[VertexKey]
        for bundle in g:
            bundle_args = []
            if 0 < len(bundle) < 3:
                ...
                for _item in bundle:
                    if isinstance(_item, EdgeKey):
                        ...
                        raise ValueError('Got invalid value in graph index bundle that was neither a `basestring` or `VertexKey` or `EdgeKey`. Instead, got: "%s" of type "%s".' % (
                         repr(_item), type(_item)))
            ...
            for element in bundle:
                ...
                if not isinstance(value, bool) and isinstance(value, _SERIES_BASETYPES):
                    if converter is None:
                        raise RuntimeError('Illegal non-string value passed in for a `meta` index value. Write bundle: "%s".' % write)
        ...
        if execute:
            ...
            return indexer_calls

    @classmethod
    def clean_indexes(cls, writes, pipeline=None):
    ...
    def execute_query(cls, kind, spec, options, **kwargs):
        ...
        if isinstance(options.ancestor, basestring):
            ...
            if isinstance(options.ancestor, model.Key):
                ancestry_parent = options.ancestor
            else:
                if isinstance(options.ancestor, model.Model):
                    ancestry_parent = options.ancestor.key
                _and_filters, _or_filters, kinded_key = [], [], model.Key(kind, parent=ancestry_parent)
                if ancestry_parent:
                    filters.insert(0, query.KeyFilter(ancestry_parent, _type=query.KeyFilter.ANCESTOR))
                if not kind and not filters:
                    filters.append(query.KeyFilter(None))
                if kind and not filters:
                    filters.append(query.KeyFilter(kind))
                _filters, _filter_i_lookup = {}, set()
                for _f in filters:
                    if isinstance(_f, query.EdgeFilter):
                        _data = _f.value.data if isinstance(_f.value, model.Model._PropertyValue) else _f.value
                        _index_key = [
                         cls._graph_prefix,
                        ...
                        _filter_key = (
                         'S', cls._magic_separator.join(_index_key))
                        ...
                        if _f.kind is query.KeyFilter.KIND:
                            if _f.value is None:
                                ...
                                if isinstance(_filter_kind, basestring) and _filter_kind in frozenset(('Edge',
                                                                                                       'Vertex',
                                                                                                       'Model')):
                                    _filter_key = (
                                     'S',
                                    ...
                                    _filter_key = (
                                     'S',
                                     cls._magic_separator.join((
                                      cls._kind_prefix, _filter_kind)))
                        elif _f.kind is query.KeyFilter.ANCESTOR:
                            if _f.value is None:
                                raise RuntimeError('Root entity filters are not yet supported in Redis.')
                            else:
                                _filter_key = (
                                 'S',
                                 cls._magic_separator.join((cls._group_prefix,
                                  _f.value.data)))
                        if _filter_key not in _filter_i_lookup:
                            _filter_i_lookup.add(_filter_key)
                            _filters[_filter_key] = [(_f.EQUALS, _f.value, _f.chain)]
                    else:
                        origin, meta, property_map, graph_indexes = cls.generate_indexes(*(
                        ...
                        if (
                         _flag, _index_key) not in _filter_i_lookup:
            ...
            if sorted_indexes:
                for prop, _directives in sorted_indexes.iteritems():
                    ...
                    if len(_filters[prop]) == 1:
                        _operator, _value = ((operator, value) for operator, value, chain in _directives).next()
                        _, prop = prop
                        if _operator is query.EQUALS:
                            ...
                            _data_frame.append(cls.execute(cls.Operations.SORTED_RANGE_BY_SCORE, None, prop, '-inf', (isinstance(_value, float) or float)(_value) if 1 else _value))
                            continue
                        elif _operator is query.GREATER_THAN:
                            _data_frame.append(cls.execute(cls.Operations.SORTED_RANGE_BY_SCORE, None, prop, (isinstance(_value, float) or float)(_value) if 1 else _value, '+inf'))
                            continue
            ...
            if _data_frame:
                ...
                for frame in _data_frame:
                    if not len(_result_window):
                        ...
                        continue
                    _result_window &= (isinstance(frame, set) or set)(frame) if 1 else frame
                ...
                for k in matching_keys:
                    if 0 < options.limit <= _seen_results:
                        ...
                        if vanilla.kind != _base_kind.kind():
                            ...
                            results.append(_base_kind.__keyclass__.from_urlsafe(k, _persisted=True))
                        else:
                            results.append(vanilla)

                return results
            result_entities, bundles = [], []
            _queued = collections.deque()
            for key in matching_keys:
                decoded_k, _base_kind = model.Key.from_urlsafe(key, _persisted=True), None
        ...
        if bundles:
            ...
            for entity in cls.get_multi(bundles):
                ...
                if _and_filters or _or_filters:
                    if _and_filters and not all(_filter.match(entity) for _filter in _and_filters):
                        continue
                    if _or_filters and not any(_filter.match(entity) for _filter in _or_filters):
                        continue
                result_entities.append(entity.key if options.keys_only else entity)
                _seen_results += 1
                if 0 < options.limit <= _seen_results:
                    break
            ...
            if sorts:
                if len(sorts) == 1:
                    sorted_results, sort_chain, sort = [], sorted(result_entities, key=itemgetter(sorts[0].target.name)), sorts[0]
                    if sort.target.basetype in (basestring, unicode, str) and sort.operator is sort.ASCENDING or sort.operator is sort.DESCENDING and sort.target.basetype not in (basestring, unicode, str):
