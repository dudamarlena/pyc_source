...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen_tests/test_util/test_struct.py
...
class ObjectProxyTests(test.FrameworkTest):
    ...
    def test_construct(self):
        ...
        st_struct = struct.ObjectProxy({'hi': True, 
           'iam': False, 
           'astruct': None})
        st_kwargs = struct.ObjectProxy(hi=1, iam=2, astruct=3)
        st_both = struct.ObjectProxy({'hi': True, 
           'iam': False, 
    ...
    def test_fill_case_sensitive(self):
        ...
        st_struct = struct.ObjectProxy({'HelloThere': True, 
           'IamA': False, 
    ...
    def test_values(self):
        ...
        st_struct = struct.ObjectProxy({'HelloThere': 1, 
           'IamA': 2, 
           'StRuct': 3})
        ref = (1, 2, 3)
...
class BidirectionalEnumTests(test.FrameworkTest):
    ...
    def test_contains(self):
        ...
        assert 1 in enum
        assert 2 in enum
        assert 3 not in enum
        assert 4 not in enum

    def test_getattr(self):
        """ Test `x.y` syntax against `BidirectionalEnum` """
        enum = self.test_construct()
        assert enum.BLUE is enum.BLUE is 0
        assert enum.RED is enum.RED is 1
        assert enum.GREEN is enum.GREEN is 2
        with self.assertRaises(AttributeError):
            assert enum.BLACK
    ...
    def test_reverse_resolve(self):
        ...
        enum = self.test_construct()
        assert enum.reverse_resolve(0) == 'BLUE'
        assert enum.reverse_resolve(1) == 'RED'
        assert enum.reverse_resolve(2) == 'GREEN'
        assert not enum.reverse_resolve(3)
        assert not enum.reverse_resolve(3)
