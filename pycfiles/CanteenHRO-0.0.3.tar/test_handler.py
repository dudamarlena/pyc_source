...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen_tests/test_base/test_handler.py
...
class BaseHandlerTest(test.FrameworkTest):
    ...
    def _make_handler(self, valid=False, _impl=handler.Handler, _environ=None):
        ...
        if not valid:
            ...
            canteen_style = _impl(*(
             environ, callback, _runtime, request, response))
            return (
             canteen_style, request, response, _runtime)
        result, environ = {}, wtest.create_environ('/sample', 'http://localhost:8080/')
        if _environ:
            environ.update(_environ)
        callback, _runtime, request, response = (
         lambda status, headers: result.__setitem__('status', status) and result.__setitem__('headers', headers),
    ...
    def test_template_context(self):
        ...
        for entry in ('handler', 'config', 'runtime', 'http', 'wsgi', 'cache', 'asset',
                      'services', 'output', 'link', 'route'):
    ...
    def test_invalid_method(self):
        ...
        _handler, request, response, runtime = self._make_handler(True, SubHandler, _environ={'REQUEST_METHOD': 'POST'})
