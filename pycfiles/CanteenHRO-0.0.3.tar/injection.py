...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/core/injection.py
...
class Delegate(object):
    ...
    __target__, __bridge__ = (None, None)
    ...
    class __metaclass__(type):
              ...
              ``key``. """
                    try:
                        ...
                        if isinstance(bridge[key], tuple):
                            return getattr(*bridge[key])
                        return bridge[key]
                    except KeyError:
                        raise AttributeError("Could not resolve attribute '%s' on item '%s'." % (
                         key, klass))

                return type.__new__(mcs.__class__, 'Delegate', (object,), {'__bridge__': None, 
        ...
        def __repr__(cls):
          ...
          :returns: String representation of a bound or root delegate. """
            if not cls.__target__:
                return '<delegate root>'
            return "<delegate '%s'>" % cls.__target__.__name__
...
class Compound(type):
    ...
    def mro(cls):
        ...
        for base in cls.__bases__:
            ...
            return [
             origin[0]] + [ i for i in filter(lambda x: x not in (object, type), origin[1:])
