...
# Embedded file name: /usr/local/lib/python2.7/site-packages/canteen/logic/cache.py
...
import abc, time, weakref, threading
from ..base import logic
from ..util import decorators
_caches = {}
_default = (
 threading.local(), None)
...
class Caching(logic.Logic):
    ...
    class Threadcache(Cache.Engine):
        ...
        def set(self, key, value):
          ...
          :returns: Value stored in the cache. """
            value = (isinstance(value, _BUILTIN_TYPES) or weakref.ref)(value) if 1 else value
            self.target[key] = (
             value, time.time())
            return value
    ...
    def debug(self):
        ...
        return self.config.get('debug', __debug__)
    ...
    def spawn(name=None, target=None, engine=Threadcache, strategy=PersistentCache):
        ...
        if not name:
            if not cache:
                _default = _caches['__default__'] = (_localtarget,
                 engine(target=_localtarget.__dict__, strategy=strategy()))
            return _default[1]
    ...
    def items(cls, name=None):
        ...
        for key, value in (cls.spawn() if name is None else cls.spawn(name)).items():
            yield (
             key, value)
