...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datascienceutils/imageProcessingUtils.py
...
def bitwiseOps(img1, img2=None, ops=None):
    if not ops:
        raise AssertionError('Please pass a bit operation')
    else:
        if ops == 'not':
            return cv2.bitwise_not(img)
        else:
            assert ops == 'add' and img1 and img2
...
def perspectiveTransform(img, orig_pt, pers_pt):
    ...
    if not len(orig_pt) == 4:
        raise AssertionError('Please pass 4 points in original image')
    elif not len(pers_pt) == 4:
        raise AssertionError('Please pass 4 points in perspective image')
