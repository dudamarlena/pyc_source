...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/test/database-test.py
...
def test_create_table_users():
    with database.Database() as (db):
        ...
        db.execute('create table if not exists users(id int primary key, name varchar(255), birth date, gender char)')
...
def test_insert_users():
    with database.Database() as (db):
        db.insert('users').set('id', 1).set('name', 'User 1').set('birth', '2018-03-20').set('gender', 'M').execute()
        db.insert('users').set('id', 2).set('name', 'User 2').set('birth', '2018-02-20').set('gender', 'F').execute()
        db.insert('users').setall({'id':3, 
         'name':'User 3',  'birth':'2018-01-20'}).set('gender', 'M').execute()
        db.insert('users').setall({'id':4, 
         'name':'Usuario 4',  'birth':'2017-01-20'}).set('gender', 'M').execute()
...
def test_update_user():
    with database.Database() as (db):
        c = db.update('users').set('name', 'Usuario 3').where('id', 3).execute()
...
def test_paging_user_by_select():
    with database.Database() as (db):
        page = db.select('users').fields('id', 'name').where('id', '10', '<').order_by('id').paging(0, 3)
...
def test_paging_complex_query():
    with database.Database() as (db):
        page = db.paging('select * from users where id < %(id)s', {'id': 10}, 0, 3)
        assert type(page) == database.Page
        assert len(page.data) == 3
        assert not page.last_page
        page = db.paging('select * from users where id < %(id)s', {'id': 10}, 1, 3)
        assert type(page) == database.Page
        assert len(page.data) == 1
        assert page.last_page
...
def test_group_by():
    with database.Database() as (db):
        result = db.select('users').fields('gender sexo', 'count(1) contagem').where('id', 10, '<').group_by('gender').order_by('contagem desc').execute().fetchall()
...
def test_update_user_whereall():
    with database.Database() as (db):
        try:
            db.update('users').set('name', 'Usuario 3').whereall({'id':1, 
             'name':'User1'}).execute()
        except () as e:
            try:
                assert e is None
...
def test_rollback_with():
    try:
        with database.Database() as (db):
            db.insert('users').set('id', 11).set('name', 'User 11').set('birth', '2018-03-20').set('gender', 'M').execute()
            raise Exception()
    except:
        with database.Database() as (db):
