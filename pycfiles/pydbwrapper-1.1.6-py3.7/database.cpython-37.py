...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pydbwrapper/database.py
# Compiled at: 2020-01-30 14:30:52
# Size of source mod 2**32: 8796 bytes
"""Database access module"""
import os, psycopg2, psycopg2.extras, errno
from pydbwrapper.config import Config
QUERIES_DIR = os.path.realpath(os.path.curdir) + '/sql/'

class DictWrapper(dict):
    __doc__ = 'Dict wrapper to access dict attributes with . operator'
    ...
    def __getattr__(self, name):
        if name in self:
            if isinstance(self[name], dict):
                if not isinstance(self[name], DictWrapper):
...
class SelectBuilder(SQLBuilder):
    ...
    def sql(self):
        ...
        return 'SELECT {} FROM {} {} {} {} {}'.format(', '.join(self.fields_to_select), self.table, self.build_where(), group_by_str, order_by_str, self.page_str)

    def paging(self, page=0, page_size=10):
        self.page_str = 'LIMIT {} OFFSET {}'.format(page_size + 1, page * page_size)
...
class Database(object):
    __doc__ = 'Facade to access database using psycopg2'
    ...
    def execute(self, name_or_sql, parameters=None, skip_load_query=False):
        ...
        if skip_load_query:
            ...
            sql = self._Database__load_query(name_or_sql)
        if self.print_sql:
    ...
    def paging(self, name_or_sql, parameters=None, page=0, page_size=10, skip_load_query=True):
        if skip_load_query:
            ...
            sql = self._Database__load_query(name_or_sql)
        sql = '{} LIMIT {} OFFSET {}'.format(sql, page_size + 1, page * page_size)
