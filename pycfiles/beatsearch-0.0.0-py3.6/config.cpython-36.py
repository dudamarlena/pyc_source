...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\config.py
# Compiled at: 2018-03-19 11:48:48
# Size of source mod 2**32: 12952 bytes
import os, uuid, configparser, typing as tp
...
class BSConfigSettingHandle(object):
    ...
    def __init__(self, section: str, name: str, default_value: tp.Any, to_type: tp.Callable, to_str: tp.Callable=None, validate: tp.Callable=None):
        if validate:
    ...
    def default_value(self, default_value: tp.Any):
        if not self._is_representable_as_string(default_value):
            raise ValueError('given default value "%s" is not representable as a string without information loss' % default_value)
...
class BSConfig(object):
    ...
    def __init__(self, config_dir: str=None):
        ...
        self.midi_root_directory = BSConfigSettingHandle((BSConfigSection.DEFAULT),
          'midi_root_dir', '', str, validate=(lambda val: not val or os.path.isdir(val)))
        self.rhythm_resolution = BSConfigSettingHandle((BSConfigSection.DEFAULT),
          'rhythm_resolution', 240, int, validate=(lambda val: val >= 0))
        self.mapping_reducer = BSConfigSettingHandle(BSConfigSection.DEFAULT, 'midi_mapping_reducer', None, lambda name: None if name == 'None' else get_drum_mapping_reducer_implementation(name), lambda reducer: 'None' if reducer is None else reducer.__name__)
...
__all__ = [
 'BSConfigSettingHandle', 'BSConfigSection', 'BSConfig']