...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\feature_extraction.py
# Compiled at: 2018-03-19 11:46:30
# Size of source mod 2**32: 22969 bytes
import enum, math, itertools, numpy as np, typing as tp
...
class RhythmFeatureExtractorBase(RhythmFeatureExtractor, metaclass=ABCMeta):
    ...
    def __init__(self, unit='eighths'):
        self._pre_processors = tuple(self.init_pre_processors())
        self._unit = None
        self.unit = unit
...
class MonophonicRhythmFeatureExtractor(RhythmFeatureExtractorBase, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __process__(self, rhythm: MonophonicRhythm, unit: tp.Union[(int, str)], pre_processor_results: tp.List[tp.Any]):
...
class PolyphonicRhythmFeatureExtractor(RhythmFeatureExtractorBase, metaclass=ABCMeta):
    ...
    @abstractmethod
    def __process__(self, rhythm: PolyphonicRhythm, unit: tp.Union[(int, str)], *pre_processor_results: tp.List[tp.Any]):
...
class IOIVector(MonophonicRhythmFeatureExtractor, QuantizableRhythmFeatureExtractorMixin):
    ...
    def __process__(self, rhythm: MonophonicRhythm, unit: tp.Union[(int, str)], pre_processor_results: tp.List[tp.Any]):
        ...
        if self.mode == self.Mode.POST_NOTE:
    ...
    def __process__(self, rhythm: MonophonicRhythm, unit: tp.Union[(int, str)], pre_processor_results: tp.List[tp.Any]):
        ...
        def get_syncopations():
            for step, curr_step_is_onset in enumerate(binary_vector):
                ...
                if not not curr_step_is_onset:
                    if next_step_is_onset:
                        pass
                    else:
