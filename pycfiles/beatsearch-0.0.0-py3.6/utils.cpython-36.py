...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\utils.py
# Compiled at: 2018-03-19 11:52:03
# Size of source mod 2**32: 10069 bytes
import os, sys, collections, typing as tp
...
def get_beatsearch_dir(mkdir=True):
    ...
    if not os.path.isdir(beatsearch_dir):
        if mkdir:
...
def type_check_and_instantiate_if_necessary(thing, thing_base_type, allow_none=True, *args, **kwargs):
    ...
    if allow_none and thing is None or isinstance(thing, thing_base_type):
        ...
        if not isclass(thing) or not issubclass(thing, thing_base_type):
            raise TypeError('Expected either a "%s" (sub)-class or an instance but got "%s"' % (
             thing_base_type, str(thing)))
...
def color_variant(color, brightness_offset=0):
    ...
    new_brightness = max(-1.0, min(curr_brightness + brightness_offset, 1.0))
    new_hsv = (
     hsv[0], hsv[1], new_brightness)
