...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\rhythm.py
# Compiled at: 2018-03-19 11:47:03
# Size of source mod 2**32: 96828 bytes
import os, enum, inspect, textwrap, itertools
...
def convert_time(time, unit_from, unit_to, quantize=False):
    ...
    if unit_from == unit_to:
        ...
        if quantize:
            return int(round(converted_time))
    ...
    def concretize_unit_decorator(func):
        ...
        try:
            ...
            if not arg_spec.keywords:
                raise ValueError("Function %s doesn't have a 'unit' argument, neither does it accept **kwargs" % func.__name__)
...
class TimeSignature(object):
    ...
    def to_midi_event(self, metronome=24, thirty_seconds=8):
        return midi.TimeSignatureEvent(numerator=(self.numerator),
    ...
    def get_meter_tree(self, unit='eighths'):
        ...
        while curr_branch > 1:
            for quotient in (2, 3):
                ...
                raise Exception('No context-sensitive meters allowed. Branch of %i units not equally divisible into binary or ternary sub-units' % curr_branch)
...
class Rhythm(object, metaclass=ABCMeta):
 ...
 tp.Sequence[int], None)]) -> None:
    ...
    @resolution.setter
    def resolution(self, resolution: tp.Union[(float, int)]):
    ...
    @bpm.setter
    def bpm(self, bpm: tp.Union[(float, int)]):
        self.set_bpm(bpm)
 ...
 tp.Sequence[int], None)]) -> None:
    ...
    def duration_in_ticks(self, duration: int) -> None:
        self.set_duration_in_ticks(duration)


class RhythmFactory(object, metaclass=ABCMeta):
    __doc__ = 'Interface for rhythm factory utility classes'
    ...
    @classmethod
    @abstractmethod
    def from_string(cls, onset_string: str, time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, onset_character: str='x', **kwargs) -> Rhythm:
        """Creates and returns a rhythm, given a string representation of its onsets"""
        raise NotImplementedError

    @classmethod
    @abstractmethod
    def from_binary_vector(cls, binary_vector: tp.Iterable[tp.Any], time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, **kwargs) -> Rhythm:
...
class RhythmBase(Rhythm, metaclass=ABCMeta):
    __doc__ = 'Rhythm abstract base class\n\n    This class extends the Rhythm interface and adds state for all of its properties (resolution, bpm, time_signature\n    and duration_in_ticks). It also implements the getters and setters of these properties.\n\n    Note that this class does not add onset state and does not implement onset-related functionality. Rhythm.    get_last_onset_tick, Rhythm.get_onset_count and Rhythm.__rescale_onset_ticks__ remain abstract and should be\n    implemented in subclasses.\n    '
...
class MonophonicRhythm(Rhythm, metaclass=ABCMeta):
    __doc__ = 'Monophonic rhythm interface\n\n    Interface for monophonic rhythms.\n    '
    ...
    class create(RhythmFactory):

        @classmethod
        def from_string(cls, onset_string: str, time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, onset_character='x', **kwargs):
            """
            ...
            """
            return cls.from_binary_vector(binary_vector=(cls.__string_to_binary_onset_vector__(onset_string, onset_character)),
              time_signature=time_signature,
              velocity=velocity,
              resolution=resolution)

        @classmethod
        def from_binary_vector(cls, binary_vector: tp.Sequence[tp.Any], time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, **kwargs):
            """
        ...
        def validate_onsets_generator():
            ...
            for onset in onsets:
                try:
                    ...
                    raise ValueError('onset should be iterable of at least two elements (tick, velocity) but got %s' % str(onset))
...
class MonophonicRhythmImpl(RhythmBase, MonophonicRhythmBase):
    __doc__ = 'Implements both rhythm base and monophonic rhythm base'

    def __init__(self, onsets: tp.Union[(tp.Iterable[Onset], tp.Iterable[tp.Tuple[(int, int)]], tp.Iterable[tp.Sequence[int]])]=None, **kwargs):
        """
...
class SlavedRhythmBase(Rhythm, metaclass=ABCMeta):
    ...
    def get_resolution(self) -> int:
        ...
        return self._SlavedRhythmBase__check_and_get_parent().get_resolution()

    def get_bpm(self) -> float:
        """Returns the bpm of the parent"""
        return self._SlavedRhythmBase__check_and_get_parent().get_bpm()

    def get_time_signature(self) -> tp.Union[(TimeSignature, None)]:
        """Returns the time signature of the parent"""
        return self._SlavedRhythmBase__check_and_get_parent().get_time_signature()

    def get_duration_in_ticks(self) -> int:
        """Returns the tick duration of the parent"""
        return self._SlavedRhythmBase__check_and_get_parent().get_duration_in_ticks()
    ...
    def set_time_signature(self, time_signature: tp.Union[(TimeSignature, tp.Tuple[(int, int)], tp.Sequence[int], None)]) -> None:
        """Raises a ParentPropertyAccessError exception"""
...
class Track(MonophonicRhythmBase, SlavedRhythmBase):
    ...
    def __init__(self, onsets: tp.Union[(tp.Iterable[Onset], tp.Iterable[tp.Tuple[(int, int)]], tp.Iterable[tp.Sequence[int]])]=None, track_name: str='', parent: Rhythm=None):
        ...
        self._track_name = str(track_name)
        self._parent = parent
...
class PolyphonicRhythm(Rhythm, metaclass=ABCMeta):
    ...
    class TrackNameError(Exception):
        __doc__ = "Thrown if there's something wrong with a track name"

    class EquallyNamedTracksError(TrackNameError):
        __doc__ = 'Thrown by set_tracks when given multiple tracks with same name'

    class IllegalTrackName(TrackNameError):
        __doc__ = 'Thrown by set_tracks if __validate_track_name__ returns False'
    ...
    class create(RhythmFactory):
        ...
        @classmethod
        def from_string(cls, input_string: str, time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, onset_character: str='x', *_, track_separator_char: str='\n', name_separator_char: str=':', **kwargs):
            '''
...
" and onset character "x", to create a simple
            ...
            return cls.from_binary_vector(binary_vector_tracks=track_onset_vectors,
              time_signature=time_signature,
              velocity=velocity,
              resolution=resolution,
        ...
        @classmethod
        def from_binary_vector(cls, binary_vector_tracks: tp.Sequence[tp.Sequence[tp.Any]], time_signature: tp.Optional[tp.Union[(tp.Tuple[(int, int)], TimeSignature)]]=None, velocity: int=100, resolution: int=4, *_, track_names: tp.Sequence[str]=None, **kwargs):
            """
            ...
            track_names = track_names or cls._create__track_name_generator(n_tracks)
...
class PolyphonicRhythmImpl(RhythmBase, PolyphonicRhythm):
    ...
    def __init__(self, tracks=tuple(), resolution=0, **kwargs):
        """
...
class FrequencyBand(enum.Enum):
    __doc__ = 'Enumeration containing three drum sound frequency bands (low, mid and high)'
    LOW = enum.auto()
    MID = enum.auto()
    HIGH = enum.auto()
...
class DecayTime(enum.Enum):
    __doc__ = 'Enumeration containing three drum sound decay times (short, normal and long)'
    SHORT = enum.auto()
    NORMAL = enum.auto()
    LONG = enum.auto()
...
class MidiDrumMappingImpl(MidiDrumMapping):
    ...
    def __init__(self, name: str, keys: tp.Sequence[MidiDrumMapping.MidiDrumKey]):
        ...
        for k in keys:
            if not k.midi_pitch not in keys_by_midi_key:
                raise AssertionError('multiple keys on midi key %i' % k.midi_pitch)
            elif not k.id not in keys_by_id:
                raise AssertionError('multiple keys with id "%s"' % k.id)
...
class RhythmLoop(PolyphonicRhythmImpl):
    __doc__ = 'Rhythm loop with a name and duration always snapped to a downbeat'
    ...
    def set_time_signature(self, time_signature):
        ...
        if new_time_signature:
            if new_time_signature != old_time_signature:
...
class MidiRhythm(RhythmLoop):
    ...
    def __init__(self, midi_file='', midi_pattern=None, midi_mapping=GMDrumMapping, midi_mapping_reducer_cls=None, name='', preserve_midi_duration=False, **kwargs):
        (super().__init__)(**kwargs)
        ...
        self._midi_metronome = -1
        self._prototype_midi_pitches = dict()
        if midi_pattern:
    ...
    def as_midi_pattern(self, note_length: int=0, midi_channel: int=9, midi_format: int=0, midi_keys: tp.Optional[tp.Dict[(str, int)]]=None) -> midi.Pattern:
        """
        ...
        :return: MIDI pattern
        """
        midi_track = midi.Track(tick_relative=False)
        midi_track.append(midi.TrackNameEvent(text=(self.get_name())))
        midi_metronome = 24 if self._midi_metronome is None else self._midi_metronome
        midi_track.append(self.get_time_signature().to_midi_event(midi_metronome))
        if self.bpm:
            midi_track.append(midi.SetTempoEvent(bpm=(self.bpm)))
    ...
    def load_midi_pattern(self, pattern: midi.Pattern, preserve_midi_duration: bool=False) -> None:
        ...
        if n_tracks_containing_note_events > 1:
            raise ValueError('Given MIDI pattern has multiple tracks with note events (%i)', n_tracks_containing_note_events)
        else:
        ...
        track = list((itertools.chain)(*pattern))
        track = midi.Track(sorted(track, key=(lambda event: event.tick)))
        bpm = 0
        midi_note_events = defaultdict(lambda : [])
        ts_midi_event = None
        eot_event = None
        for msg in track:
            if isinstance(msg, midi.NoteOnEvent):
                midi_pitch = msg.get_pitch()
                ...
                if mapping_key is None:
                    ...
                    if isinstance(msg, midi.TimeSignatureEvent):
                        if ts_midi_event is None:
                            ...
                            raise ValueError('Time signature changes are not supported (from %s to %s)' % (
                             ts_midi_event, msg))
                    else:
        ...
        midi_metronome = ts_midi_event.get_metronome()
