...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\test_metrics.py
# Compiled at: 2018-03-13 11:10:12
# Size of source mod 2**32: 19384 bytes
import inspect, typing as tp
...
from beatsearch.metrics import HammingDistanceMeasure, EuclideanIntervalVectorDistanceMeasure, IntervalDifferenceVectorDistanceMeasure, SwapDistanceMeasure, ChronotonicDistanceMeasure
...
class TestMonophonicRhythmDistanceMeasureBase(TestCase):
    ...
    def test_all_concrete_subclasses_have_unit_and_length_policy_as_first_arguments_and_set_the_props_accordingly(self, _):

        def patch_prop(prop_name):
            return patch.object(MonophonicRhythmDistanceMeasure, prop_name, new_callable=PropertyMock)

        for clazz in get_concrete_monophonic_measure_implementations():
            with patch_prop('length_policy') as (length_policy_mock):
                with patch.object(clazz, 'set_unit') as (set_unit_mock):
    ...
    def test_length_policy_setter_raises_unknown_length_policy_exception(self):
        with patch.object(MonophonicRhythmDistanceMeasure, 'length_policy'):
            m = self.create_measure()
        self.assertRaises(MonophonicRhythmDistanceMeasure.UnknownLengthPolicy, setattr, m, 'length_policy', 'foo')
    ...
    def test_get_distance(self):
        ppq = 123
        distance = 321
        rhythm_a = MagicMock(PolyphonicRhythm)
        rhythm_b = MagicMock(PolyphonicRhythm)
        for r in [rhythm_a, rhythm_b]:
            r.get_resolution = MagicMock(return_value=ppq)

        m = self.create_measure(unit='ticks')
        m.__get_iterable__ = MagicMock(side_effect=['iterable_a', 'iterable_b'])
    ...
    def test_iterables_meet_len_policy_check_raises_unknown_length_policy_error(self):
        ...
        self.assertRaises(MonophonicRhythmDistanceMeasure.UnknownLengthPolicy, MonophonicRhythmDistanceMeasure.check_if_iterables_meet_len_policy, length_policy, 'iter_a', 'iter_b')
...
class TestMonophonicRhythmDistanceMeasureImplementation(metaclass=ABCMeta):
    ...
    def test_compute_distance(self):
        ...
        for secondary_iterable, secondary_cookie, expected_d, secondary_name in zipped:
            ...
            actual_d = measure_class.__compute_distance__(max_len, test_iterables.main, secondary_iterable, test_cookies.main, secondary_cookie)
...
class TestHammingDistanceMeasure(TestCase, TestMonophonicRhythmDistanceMeasureImplementation):
    ...
    @classmethod
    def get_test_iterables(cls) -> MonophonicDistanceTestInfo:
        main = '1001000100101000'
        exct = '1001001000101001'
        mult = '10010001'
        fill = '10010001001010'
        return MonophonicDistanceTestInfo(main=(tuple(int(c) for c in main)),
    ...
    @classmethod
    def get_expected_distances_to_main(cls) -> MonophonicDistanceTestInfo:
        return MonophonicDistanceTestInfo(main=0,
          secondary_exact=3,
    ...
    def test_get_iterable(self, mock_binary_onset_vector_process, *args):
        rhythm = MagicMock(MonophonicRhythm)
...
class TestEuclideanIntervalVectorDistanceMeasure(TestCase, TestMonophonicRhythmDistanceMeasureImplementation):
    ...
    @classmethod
    def get_expected_distances_to_main(cls) -> MonophonicDistanceTestInfo:
        return MonophonicDistanceTestInfo(main=0.0,
          secondary_exact=2.0,
...
class TestIntervalDifferenceVectorDistanceMeasure(TestCase, TestMonophonicRhythmDistanceMeasureImplementation):
    ...
    def get_test_iterables(cls) -> MonophonicDistanceTestInfo:
        main = [
         1.33, 0.75, 0.67, 1.0, 1.0, 1.5]
        exct = [1.0, 1.33, 0.5, 1.5, 0.33, 3.0]
        mult = [0.5, 2.0, 2.0]
        fill = [1.33, 0.75, 1.0]
