...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\metrics.py
# Compiled at: 2018-03-19 11:46:36
# Size of source mod 2**32: 25919 bytes
import inspect, math, typing as tp
from collections import OrderedDict
from abc import abstractmethod, ABCMeta
from beatsearch.rhythm import MonophonicRhythm, MonophonicRhythmImpl, PolyphonicRhythm, Unit
from beatsearch.feature_extraction import BinaryOnsetVector, IOIVector, IOIDifferenceVector, OnsetPositionVector, ChronotonicChain
from beatsearch.utils import friendly_named_class, Quantizable

class DistanceMeasure(object):
    __doc__ = 'Abstract base class for distance measures'
...
class MonophonicRhythmDistanceMeasure(DistanceMeasure, metaclass=ABCMeta):
    ...
    class UnknownLengthPolicy(Exception):
        ...
        def __init__(self, given_length_policy):
            super(MonophonicRhythmDistanceMeasure.UnknownLengthPolicy, self).__init__('Given %s, please choose between: %s' % (
             given_length_policy, MonophonicRhythmDistanceMeasure.LENGTH_POLICIES))

    class LengthPolicyNotMet(Exception):
        MESSAGES = {'exact':'When length policy is set to "exact", both iterables should have the same number of elements', 
         'multiple':'When length policy is set to "multiple", the length of the largest iterable should be a multiple of all the other iterable lengths'}
    ...
    def get_distance(self, rhythm_a: MonophonicRhythm, rhythm_b: MonophonicRhythm):
        ...
        if unit_in_ticks:
            if res_a != res_b:
                raise ValueError("%s unit set to 'ticks', but given rhythms have different resolutions (%i != %i)" % (
    ...
    def check_if_iterables_meet_len_policy(cls, length_policy, iterable_a, iterable_b):
        ...
        """
        l = [
         len(iterable_a), len(iterable_b)]
        if length_policy == 'exact':
            return l[0] == l[1]
        if length_policy == 'multiple':
            return all(x % l[0] == 0 or l[0] % x == 0 for x in l)
        if length_policy == 'fill':
            return l[0] > 0 and l[1] > 0
        raise cls.UnknownLengthPolicy(length_policy)

    __measures_by_friendly_name__ = {}
    __measures_by_class_name__ = {}
    ...
    def get_measures(cls, friendly_name=True):
        ...
        """
        if len(MonophonicRhythmDistanceMeasure.__measures_by_friendly_name__) != len(MonophonicRhythmDistanceMeasure.__subclasses__()):
            by_friendly_name = OrderedDict()
            by_class_name = OrderedDict()
            for dm in cls.__subclasses__():
                name = dm.__name__
                if friendly_name:
                    try:
...
def rhythm_pair_track_iterator(rhythm_a: PolyphonicRhythm, rhythm_b: PolyphonicRhythm, tracks: tp.Union[(str, tp.Iterable[tp.Any])]):
    """
    ...
    try:
        ...
        try:
            ...
            raise ValueError('Excepted an iterable or one of %s but got %s' % (
             TRACK_WILDCARDS, tracks))
...
class SummedMonophonicRhythmDistance(PolyphonicRhythmDistanceMeasure):
    ...
    def monophonic_measure(self, track_distance_measure: tp.Union[(MonophonicRhythmDistanceMeasure, tp.Type[MonophonicRhythmDistanceMeasure])]):
        """
        ...
        """
        if inspect.isclass(track_distance_measure):
            if issubclass(track_distance_measure, MonophonicRhythmDistanceMeasure):
                track_distance_measure = track_distance_measure()
        if not isinstance(track_distance_measure, MonophonicRhythmDistanceMeasure):
            raise ValueError("Expected a MonophonicRhythmDistanceMeasure subclass or instance, but got '%s'" % track_distance_measure)
    ...
    def get_distance(self, rhythm_a, rhythm_b):
        ...
        if self.normalize:
            return average_distance / duration
        else:
