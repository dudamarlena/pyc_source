...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\beatsearch\test_rhythm.py
# Compiled at: 2018-03-19 08:16:20
# Size of source mod 2**32: 57707 bytes
import enum, textwrap, typing as tp
...
class ITestRhythmBase(object):
    ...
    def get_rhythm_class_with_mocked_setters(cls) -> tp.Type[RhythmMockedSetters]:
        ...
        class RhythmImplWithMockedSetters(rhythm_class):
    ...
    def get_rhythm_class_with_mocked_getters(cls) -> tp.Type[RhythmMockedGetters]:
        ...
        class RhythmImplWithMockedGetters(rhythm_class):
    ...
    def get_rhythm_class_with_mocked_post_init(cls) -> tp.Type[RhythmMockedPostInit]:
        ...
        class RhythmImplWithMockedPostInit(rhythm_class):

            def __init__(self, **kw):
                mocked_post_init_mixin.__init__(self)
                (super().__init__)(**kw)
    ...
    def get_rhythm_class_with_mocked_setters_and_getters(cls) -> tp.Type[RhythmMockedSettersAndGetters]:
        ...
        class RhythmImplWithMockedSettersAndGetters(rhythm_class):
...
class inject_rhythm(object):
    ...
    def __get_rhythm_class(cls, test: ITestRhythmBase, mock_type: Mock):
        if mock_type is cls.Mock.NO_MOCK:
            ...
            if mock_type is cls.Mock.MOCK_GETTERS:
                return test.get_rhythm_class_with_mocked_getters()
            else:
                if mock_type is cls.Mock.MOCK_SETTERS:
                    return test.get_rhythm_class_with_mocked_setters()
                if mock_type is cls.Mock.MOCK_POST_INIT:
                    return test.get_rhythm_class_with_mocked_post_init()
            if mock_type is cls.Mock.MOCK_GETTERS_AND_SETTERS:
    ...
    def __create_decorator(mock_type: Mock, **constructor_kwargs):
        ...
        def decorator(func):
            ...
            def wrapper(self, *args, **kwargs):
                rhythm_class = inject_rhythm._inject_rhythm__get_rhythm_class(self, mock_type)
    ...
    no_mock = partial(_inject_rhythm__create_decorator, mock_type=(Mock.NO_MOCK))
    mocked_getters = partial(_inject_rhythm__create_decorator, mock_type=(Mock.MOCK_GETTERS))
    mocked_setters = partial(_inject_rhythm__create_decorator, mock_type=(Mock.MOCK_SETTERS))
    mocked_setters_and_getters = partial(_inject_rhythm__create_decorator, mock_type=(Mock.MOCK_GETTERS_AND_SETTERS))
    mocked_post_init = partial(_inject_rhythm__create_decorator, mock_type=(Mock.MOCK_POST_INIT))
    ...
    def type(func: tp.Callable[([ITestRhythmBase, tp.Type[Rhythm], tp.Any], tp.Any)]):
        ...
        @wraps(func)
        def wrapper(self, *args, **kwargs):
            rhythm_class = inject_rhythm._inject_rhythm__get_rhythm_class(self, inject_rhythm.Mock.NO_MOCK)
...
class TestRhythmBase(ITestRhythmBase, TestCase):
    ...
    def test_constructor_calls_post_init_with_correct_arguments(self, rhythm):
        rhythm.post_init.assert_called_once_with(bpm=123,
          resolution=234,
          time_signature=(5, 4),
    ...
    def test_beat_duration(self, rhythm_class):
        ...
        rhythm_info = {'quarter_beat_unit':[
          create_rhythm(TimeSignature(5, 4)), resolution], 
         'eighth_beat_unit':[
          create_rhythm(TimeSignature(5, 8)), resolution // 2]}
...
class TestMonophonicRhythmImpl(TestRhythmBase):
    ...
    def test_onsets_setter_raises_value_error_on_invalid_input(self, rhythm):
        onset_inputs = {'on_missing_velocity':((1, 0), (2, 0), (3, 0), 4), 
    ...
    def test_rescale_onsets_scales_onsets(self, rhythm, _):
        ...
        for i, (original_onset, expected_onset, actual_onset) in enumerate(zip(original_onsets, expected_scaled_onsets, actual_scaled_onsets)):
...
class TestPolyphonicRhythmImpl(TestRhythmBase):
    ...
    def test_set_tracks_adds_all_tracks(self, rhythm):
        kick, snare, tom = tuple(self.create_fake_named_track(name) for name in ('kick',
                                                                                 'snare',
                                                                                 'tom'))
    ...
    def test_set_tracks_parents_tracks_to_rhythm(self, rhythm):
        tracks = tuple(self.create_fake_named_track(name) for name in ('kick', 'snare',
                                                                       'tom'))
        rhythm.__get_track_naming_error__ = MagicMock(return_value='')
    ...
    def test_set_resolution_calls_rescaling(self, rhythm):
        ...
        rhythm.__get_track_naming_error__ = MagicMock(return_value='')
        rhythm.set_tracks(tuple(self.create_fake_named_track(name) for name in ('kick',
                                                                                'snare',
                                                                                'tom')), 16)
...
class TestRhythmLoop(TestPolyphonicRhythmImpl):
    ...
    def test_duration_in_ticks_setter_permissive_when_no_downbeat_can_be_computed(self):
        rhythms = {'no_ts-no_res':self._create_rhythm_mocked_ts_and_res(time_signature=None, resolution=0), 
    ...
    def test_non_zero_duration_is_updated_as_soon_as_downbeat_is_computable(self):
        ...
        for r in [with_ts_without_res, with_res_without_ts]:
            ...
            r.get_duration_in_ticks = MagicMock(return_value=16)
...
class TestMidiRhythm(TestRhythmLoop):
    ...
    def test_load_midi_pattern(self, rhythm, _):
        ...
        assert isinstance(rhythm.midi_mapping, MagicMock), 'something went wrong in the constructor, midi_mapping was not set'
        ...
        with self.subTest('track_onsets_tick_positions_are_integers'):
            for kick_onset, snare_onset in zip(kick_track.get_onsets(), snare_track.get_onsets()):
                self.assertIsInstance(kick_onset.tick, int, 'kick onset tick position "%s" is not an int' % str(kick_onset.tick))
                self.assertIsInstance(snare_onset.tick, int, 'snare onset tick position "%s" is not an int' % str(snare_onset.tick))
    ...
    def test_as_midi_pattern(self, rhythm):
        ...
        snare_key_mock = self._create_midi_drum_key_mock(snare, 'mid', 'normal', 'snare', 'snr')
        assert isinstance(rhythm.midi_mapping, MagicMock), 'something went wrong in the constructor, midi_mapping was not set'
        rhythm.midi_mapping.get_key_by_id = lambda key_id: kick_key_mock if key_id == kick_key_mock.id else snare_key_mock
        kick_onsets = (
         mock_onset(0, 80), mock_onset(3, 40), mock_onset(7, 80), mock_onset(12, 80))
        ...
        actual_track_name_event, actual_time_signature_event, actual_tempo_event, *actual_note_events, actual_eot_event = midi_track
...
class TestMonophonicRhythmFactory(TestCase):
    ...
    def test_from_binary_vector(self, mock_monophonic_rhythm_impl_constructor):
        ...
        MonophonicRhythm.create.from_binary_vector(binary_onset_vector, time_signature=(1,
                                                                                        2), velocity=87, resolution=2)
        mock_monophonic_rhythm_impl_constructor.assert_called_once_with(onsets=((0, 87),
                                                                                (3, 87),
                                                                                (6, 87)),
          time_signature=(1, 2),
          resolution=2,
    ...
    def test_from_string(self, mock_from_binary_chain):
        ...
        mock_from_binary_chain.assert_called_once_with(binary_vector=(True, False,
                                                                      False, True,
                                                                      False, False,
                                                                      True, True),
          time_signature=(7, 8),
...
class TestPolyphonicRhythmFactory(TestCase):
    ...
    def test_from_binary_vector_track_creation(self, mock_track_constructor):
        ...
        with patch.object(PolyphonicRhythmImpl, 'set_tracks', return_value=None) as (mock_set_tracks):
            PolyphonicRhythm.create.from_binary_vector((self.track_onset_vectors),
              track_names=(self.track_names), velocity=velocity)
            mock_set_tracks.assert_called_once()
            tracks_iter = mock_set_tracks.call_args_list[0][0][0]
            _ = tuple(tracks_iter)
    ...
    def test_from_binary_vector(self, mock_track_constructor, mock_polyphonic_rhythm_impl_constructor):
        ...
        n_tracks = 2
        rhythm_kwargs = {'time_signature':(7, 8), 
         'resolution':123}
        (PolyphonicRhythm.create.from_binary_vector)('fake-onset-vectors', velocity=87, track_names=['fake-name'] * n_tracks, **rhythm_kwargs)
        ...
        with self.subTest('tracks'):
            self.assertSequenceEqual(tuple(constructor_call_args[0]), [mock_track_constructor.return_value] * n_tracks)
    ...
    def test_from_string(self, mock_from_binary_chain):
        kwargs = dict(time_signature=(4, 4), velocity=123, resolution=2, onset_character='O',
          track_separator_char='\n',
          name_separator_char='=')
        (PolyphonicRhythm.create.from_string)(
         (textwrap.dedent('\n            kick   =  O---O-O-\n            snare  =  -OO--O-O\n            hi-hat =  OO-OO-OO\n        ')), **kwargs)
        track_names = [
         'kick', 'snare', 'hi-hat']
        binary_vectors = [
        ...
        mock_from_binary_chain.assert_called_once_with(binary_vector_tracks=binary_vectors,
...
class TestOnset(TestCase):
    ...
    def test_scale_tick_rounds_down(self):
        onset = Onset(2, 75).scale(10, 16)
        self.assertEqual(onset.tick, 3)

    def test_scale_tick_rounds_up(self):
        onset = Onset(3, 75).scale(10, 16)
        self.assertEqual(onset.tick, 5)
