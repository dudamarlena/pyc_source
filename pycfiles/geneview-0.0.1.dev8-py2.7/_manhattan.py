...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/geneview/gwas/_manhattan.py
# Compiled at: 2016-02-18 21:56:03
"""
...
def manhattanplot(data, ax=None, xlabel=None, ylabel=None, color=None, kind='scatter', xtick_label_set=None, CHR=None, alpha=0.8, mlog10=True, **kwargs):
    ...
    if not x:
        msg = 'zero-size array to reduction operation minimum which has no identity. This could be caused by zero-size array of ``x`` in the ``manhattanplot(...)`` function.'
        raise ValueError(msg)
    c = np.array(c)
    x = np.array(x)
    ...
    if CHR is None:
        xs_by_id = [ (k, xs_by_id[k]) for k in sorted(xs_by_id.keys(), cmp=chr_id_cmp) if k in xtick_label_set
                   ]
