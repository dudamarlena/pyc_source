...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/geneview/util/_dataset.py
# Compiled at: 2016-02-18 11:26:58
"""
...
def get_dataset_names():
    ...
    return [ l.text.replace('.csv', '') for l in gh_list.find_all('a', {'class': 'js-directory-link'}) if l.text.endswith('.csv')
           ]
...
def load_dataset(name, cache=True, data_home=None, **kw):
    ...
    if cache:
        cache_path = os.path.join(_get_data_home(data_home), os.path.basename(full_path))
