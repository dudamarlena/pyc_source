...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/KeplerOrbit/KeplerOrbit.py
# Compiled at: 2020-04-11 15:08:35
# Size of source mod 2**32: 10485 bytes
import numpy as np
...
def cart2kep(X, Y, Z, vx, vy, vz, m1, m2):
    ...
    nmag = np.sqrt(nx ** 2.0 + ny ** 2.0 + nz ** 2.0)
    asc_node = np.where(inc == 0.0, 0.0, np.arccos(dot(ivec[0], ivec[1], ivec[2], nx, ny, nz) / nmag))
    asc_node[dot(nx, ny, nz, jvec[0], jvec[1], jvec[2]) < 0.0] = 2.0 * np.pi - asc_node[(dot(nx, ny, nz, jvec[0], jvec[1], jvec[2]) < 0.0)]
    omega = np.where(inc == 0.0, np.arctan2(evecy / e, evecx / e), np.arccos(dot(nx, ny, nz, evecx, evecy, evecz) / (nmag * e)))
    omega[dot(evecx, evecy, evecz, kvec[0], kvec[1], kvec[2]) < 0.0] = 2.0 * np.pi - omega[(dot(evecx, evecy, evecz, kvec[0], kvec[1], kvec[2]) < 0.0)]
    theta = np.arccos(dot(evecx, evecy, evecz, X, Y, Z) / (e * magr))
    theta = np.where(dot(X, Y, Z, vx, vy, vz) < 0.0, 2 * np.pi - theta, theta)
    E = np.arccos((e + np.cos(theta)) / (1 + e * np.cos(theta)))
    E = np.where(np.logical_and(theta > np.pi, theta < 2 * np.pi), 2 * np.pi - E, theta)
    M = E - e * np.sin(E)
    return (
     a, e, inc, asc_node % (2 * np.pi), omega, M)
...
def kep2cart(a, ecc, inc, Omega, omega, M, mass, m_central):
    ...
    E = nr(M, ecc)
    X = a * (np.cos(E) - ecc)
    Y = a * np.sqrt(1.0 - ecc ** 2.0) * np.sin(E)
    mu = m_central + mass
    n = np.sqrt(mu / a ** 3.0)
...
def orb_params(snap):
    ...
    pl = snap[1:]
    m2 = pl['mass']
    pl['a'], pl['e'], pl['inc'], pl['asc_node'], pl['omega'], pl['M'] = cart2kep(x_h[:, 0], x_h[:, 1], x_h[:, 2], v_h[:, 0], v_h[:, 1], v_h[:, 2], m1, m2)
    return pl
...
def lap(j, s, alpha):
    ...
    def int_func(x):
        return np.cos(j * x) / (1.0 - 2.0 * alpha * np.cos(x) + alpha ** 2.0) ** s

    integral = integrate.quad(int_func, 0.0, 2.0 * np.pi)[0]
    return 1.0 / np.pi * integral


def d_lap(j, s, alpha):
    """ First alpha derivative of Laplace coefficient """
    return s * (lap(j - 1, s + 1, alpha) - 2.0 * alpha * lap(j, s + 1, alpha) + lap(j + 1, s + 1, alpha))
...
def f_d2(j, alpha):
    ...
    l = lap(j, 0.5, alpha)
    dl = d_lap(j, 0.5, alpha)
    return 0.125 * (-5 * j * l + 4 * j ** 2 * l - 2 * alpha * dl + 4 * j * alpha * dl + alpha ** 2 * dl ** 2)


def f_s1(alpha):
    """ Secular terms in disturbing function (from table 8.4 in M+D) """
    c1 = 0.25 * alpha * d_lap(0, 0.5, alpha)
    c2 = 0.0625 * alpha ** 2 * d_lap(1, 1.5, alpha)
    c3 = -0.125 * alpha ** 3 * d_lap(0, 1.5, alpha)
    c4 = 0.0625 * alpha ** 2 * d_lap(1, 1.5, alpha)
    c5 = -0.125 * alpha ** 2 * lap(0, 1.5, alpha)
    return c1 + c2 + c3 + c4 + c5


def C_r(m_pert, m_c, a, j1, j2):
    """ Constant from resonant part of disturbing function (M+D equation 8.32) """
    alpha = (j2 / (j2 - 1)) ** 0.6666666666666666
...
def res_width(m, m_c, ecc, j1, j2):
    ...
    alpha = (-j2 / j1) ** 0.6666666666666666
    if j2 == 1 - j1:
        ...
        if j2 == 2 - j1:
            ...
            if j2 == 1 - j1:
                da_a = np.sqrt(5.333333333333333 * Cr_n * ecc) * np.sqrt(1.0 + 1.0 / (27.0 * j2 ** 2.0 * ecc ** 3) * Cr_n) - 2.0 / (9.0 * j2 * ecc) * Cr_n
            else:
                if j2 == 2 - j1:
                    da_a = np.sqrt(5.333333333333333 * Cr_n * ecc)
        return da_a
