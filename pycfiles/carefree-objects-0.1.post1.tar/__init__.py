...
# Embedded file name: build/bdist.linux-x86_64/egg/careerjet_api_client/__init__.py
# Compiled at: 2015-05-20 10:15:18
import platform, pkg_resources, requests, json
...
class CareerjetAPIClient(object):
    ...
    def search(self, search_params):
        ...
        for field in search_params:
            if field not in Constants.ALLOWED_FIELDS:
                raise Exception("Unknown param key '" + field + "'")

        for field in Constants.MANDATORY_FIELDS:
            if field not in search_params or not search_params[field]:
