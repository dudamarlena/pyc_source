...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/synthesis/hmiscsv30writer.py
...
class HmisCsv30Writer(dbobjects.DatabaseObjects):
    ...
    def getHistoryRelatedColumnData(self, phIndex, table, *columns):
        query = 'self.session.query(dbobjects.%s)' % table + '.filter(dbobjects.%s.person_historical_index_id == phIndex)' % table + '.first()'
        row = eval(query)
        if self.debug:
            print '\n* %s = %s' % (table, row)
        retVal = []
    ...
    def getSchoolBarrier(self, cesIndex):
        barrier = self.session.query(dbobjects.ChildEnrollmentStatusBarrier).filter(dbobjects.ChildEnrollmentStatusBarrier.child_enrollment_status_index_id == cesIndex).first()
        if not barrier:
            return
        else:
            if self.debug:
    ...
    def getRelationshipToHeadData(self, hhId):
        members = self.session.query(dbobjects.Household, dbobjects.Members).filter(and_(or_(dbobjects.Household.household_id_num == hhId, dbobjects.Household.household_id_str == hhId), dbobjects.Household.id == dbobjects.Members.household_index_id)).first()
        if not members:
            return
        else:
            if self.debug:
    ...
    def getPriorZipCodeData(self, phIndex):
        address = self.session.query(dbobjects.PersonAddress).filter(and_(dbobjects.PersonAddress.person_historical_index_id == phIndex, dbobjects.PersonAddress.is_last_permanent_zip == 1)).first()
        if not address:
            return (None, None)
        else:
            if self.debug:
    ...
    def getReasonForLeavingData(self, sspIndex):
        reason = self.session.query(dbobjects.ReasonsForLeaving).filter(dbobjects.ReasonsForLeaving.site_service_participation_index_id == sspIndex).first()
        if not reason:
            return
        else:
            if self.debug:
    ...
    def getPersonHistoricalIndexData(self, sspIndex):
        historical = self.session.query(dbobjects.PersonHistorical).filter(dbobjects.PersonHistorical.site_service_index_id == sspIndex).first()
        if not historical:
            return
        else:
            if self.debug:
    ...
    def getRacesData(self, personIndex):
        races = self.session.query(dbobjects.Races).filter(dbobjects.Races.person_index_id == personIndex)
        primaryRace = None
        secondaryRace = None
        try:
    ...
    def getReleaseGrantedData(self, personIndex):
        roi = self.session.query(dbobjects.ReleaseOfInformation).filter(dbobjects.ReleaseOfInformation.person_index_id == personIndex).first()
        if not roi:
            return
        else:
            try:
    ...
    def getReceivesMcKinneyFundingData(self, serviceIndex):
        funding = self.session.query(dbobjects.FundingSource).filter(dbobjects.FundingSource.service_index_id == serviceIndex).first()
        if not funding:
            return
        else:
            try:
    ...
    def getFundingSourceData(self, seIndex):
        funding = self.session.query(dbobjects.FundingSource).filter(dbobjects.FundingSource.service_event_index_id == seIndex).first()
        if not funding:
            return
        else:
            faAmt = None
            ...
            return (
             faAmt, grantId, advArrears)

    def getNonCashBenefitsData(self, phIndex):
        print 'in gncbd'
        nonCashBens = self.session.query(dbobjects.NonCashBenefits).filter(dbobjects.NonCashBenefits.person_historical_index_id == phIndex)
    ...
    def getIncomeAndSourcesData(self, phIndex):
        print 'in gisd'
        incomes = self.session.query(dbobjects.IncomeAndSources).filter(dbobjects.IncomeAndSources.person_historical_index_id == phIndex)
    ...
    def getPersonHistoricalData(self, personIndex, personId):
        historicals = self.session.query(dbobjects.PersonHistorical).filter(dbobjects.PersonHistorical.person_index_id == personIndex)
        if not historicals.count():
            print 'Warning: no data in person_historical table for person %s.' % personId
            return
        self.historicalFile = self.openFile(HmisCsv30Writer.files['historical'])
        self.openFiles.append(self.historicalFile)
        self.historicalWriter = csv.writer(self.historicalFile, quoting=csv.QUOTE_NONNUMERIC)
    ...
    def getServiceEventData(self, personIndex, personId):
        serviceEvents = self.session.query(dbobjects.ServiceEvent).filter(dbobjects.ServiceEvent.person_index_id == personIndex)
        ...
        self.serviceEventWriter = csv.writer(self.serviceEventFile, quoting=csv.QUOTE_NONNUMERIC)
    ...
    def getParticipationData(self, personIndex, personId):
        participations = self.session.query(dbobjects.SiteServiceParticipation).filter(dbobjects.SiteServiceParticipation.person_index_id == personIndex)
        if not participations.count():
            print 'Warning: no data in site_service_participation table for person %s.' % personId
            return
        self.participationFile = self.openFile(HmisCsv30Writer.files['participation'])
        self.openFiles.append(self.participationFile)
        self.participationWriter = csv.writer(self.participationFile, quoting=csv.QUOTE_NONNUMERIC)
    ...
    def getPersonData(self, exportId):
        persons = self.session.query(dbobjects.Person).filter(dbobjects.Person.export_id == exportId)
        if exportId == None:
            print 'listen, bub, you cant select on null export id'
        if not persons.count():
            print "Warning: there's no data in person table for export %s." % exportId
            return
        else:
    ...
    def getInventoryData(self, siteServiceIndex):
        inventories = self.session.query(dbobjects.Inventory).filter(dbobjects.Inventory.site_service_index_id == siteServiceIndex)
        if not inventories.count():
            print 'Warning: no data in inventory for site_service_id %s.' % siteServiceIndex
            return
    ...
    def getRegionData(self, siteServiceId):
        regions = self.session.query(dbobjects.Region).filter(dbobjects.Region.site_service_id == siteServiceId)
    ...
    def getSiteServiceData(self, siteIndex):
        siteServices = self.session.query(dbobjects.SiteService).filter(dbobjects.SiteService.site_index_id == siteIndex)
        if not siteServices.count():
            print 'Warning: no data in site_service for site index %s.' % siteIndex
            return
    ...
    def getAgencyProgramData(self, exportIndex):
        agencyPrograms = self.session.query(dbobjects.Agency, dbobjects.Service, dbobjects.Site).filter(and_(dbobjects.Agency.export_index_id == exportIndex, dbobjects.Service.export_index_id == exportIndex, dbobjects.Site.export_index_id == exportIndex, dbobjects.Agency.airs_key == dbobjects.Service.airs_key, dbobjects.Agency.id == dbobjects.Site.agency_index_id))
        if not agencyPrograms.count():
            print 'Warning: no data in (agency x service x site) for export %s.' % self.exportId
            return
    ...
    def getSourceData(self, exportId):
        sources = self.session.query(dbobjects.Source).filter(dbobjects.Source.export_id == exportId).first()
        if not sources:
            print "Warning: there's no data in source table for export %s." % exportId
            return
        else:
            try:
                if self.debug:
    ...
    def getIncomeAndNonCashBenefitsData(self, phIndex):
        ...
        for ias in self.getIncomeAndSourcesData(phIndex):
            yield (
             IB_TYPE_INCOME, ias.income_source_code, ias.income_source_other,
    ...
    def createServiceEventRecs(self, personIndex, personId, phIndex):
        for se in self.getServiceEventData(personIndex, personId):
            try:
                ...
                clientEngaged = self.getHistoryRelatedColumnData(phIndex, 'EngagedDate', 'id') != None
                (contTime, contSite) = self.getHistoryRelatedColumnData(phIndex, 'ContactMade', 'contact_date', 'contact_site')
            except:
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['serviceEvent']
                raise

        return

    def createIncomeBenefitsRecs(self, phIndex, personId):
        for (ibType, srcCode, srcOther, monthlyAmt, sourceDate) in self.getIncomeAndNonCashBenefitsData(phIndex):
            ...
            try:
                if not getattr(self, 'incBensFile', None):
                    ...
                    self.incBensWriter = csv.writer(self.incBensFile, quoting=csv.QUOTE_NONNUMERIC)
                print '\n* DataRow (IncomeBenefits)= ', dataRow
                self.incBensWriter.writerow(dataRow)
            except:
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['incBens']
                raise
    ...
    def createParticipationRecs(self, personIndex, personId):
        for participation in self.getParticipationData(personIndex, personId):
            try:
                ...
                hhId = self.chooseId(participation.household_idid_num, participation.household_idid_str)
                phIndex = self.getPersonHistoricalIndexData(sspIndex)
                reasonForLeaving = self.getReasonForLeavingData(sspIndex)
                vetStatus = self.getHistoryRelatedColumnData(phIndex, 'VeteranVeteranStatus', 'veteran_status')
                disCond = self.getHistoryRelatedColumnData(phIndex, 'DisablingCondition', 'disabling_condition')
                priorRes = self.getHistoryRelatedColumnData(phIndex, 'PriorResidence', 'prior_residence_code')
                lengthPriorStay = self.getHistoryRelatedColumnData(phIndex, 'LengthOfStayAtPriorResidence', 'length_of_stay_at_prior_residence')
                dest = self.getHistoryRelatedColumnData(phIndex, 'Destinations', 'destination_code')
                chronicHomeless = self.getHistoryRelatedColumnData(phIndex, 'HudChronicHomeless', 'hud_chronic_homeless')
                housingEntry = self.getHistoryRelatedColumnData(phIndex, 'HousingStatus', 'housing_status')
                housingExit = self.getHistoryRelatedColumnData(phIndex, 'HousingStatus', 'housing_status')
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['participation']
                raise
    ...
    def createClientHistoricalRecs(self, personIndex, personId):
        for historical in self.getPersonHistoricalData(personIndex, personId):
            try:
                phIndex = historical.id
                monthlyIncome = self.getHistoryRelatedColumnData(phIndex, 'IncomeTotalMonthly', 'income_total_monthly')
                income30 = self.getHistoryRelatedColumnData(phIndex, 'IncomeLast30Days', 'income_last_30_days')
                noncash30 = self.getHistoryRelatedColumnData(phIndex, 'NonCashBenefitsLast30Days', 'income_last_30_days')
                (physDis, recvPhysDis) = self.getHistoryRelatedColumnData(phIndex, 'PhysicalDisability', 'has_physical_disability', 'receive_physical_disability_services')
                (devDis, recvDevDis) = self.getHistoryRelatedColumnData(phIndex, 'DevelopmentalDisability', 'has_developmental_disability', 'receive_developmental_disability')
                (chronicCond, recvChronic) = self.getHistoryRelatedColumnData(phIndex, 'ChronicHealthCondition', 'has_chronic_health_condition', 'receive_chronic_health_services')
                (hivAids, recvHivAids) = self.getHistoryRelatedColumnData(phIndex, 'HivAidsStatus', 'has_hiv_aids', 'receive_hiv_aids_services')
                (mental, mentalIndef, recvMental) = self.getHistoryRelatedColumnData(phIndex, 'MentalHealthProblem', 'has_mental_health_problem', 'mental_health_indefinite', 'receive_mental_health_services')
                (substance, substanceIndef, recvSubstance) = self.getHistoryRelatedColumnData(phIndex, 'SubstanceAbuseProblem', 'has_substance_abuse_problem', 'substance_abuse_indefinite', 'receive_substance_abuse_services')
                (violence, violenceOccured) = self.getHistoryRelatedColumnData(phIndex, 'DomesticViolence', 'domestic_violence_survivor', 'dv_occurred')
                (employed, hoursLastWk, tenure, looking) = self.getHistoryRelatedColumnData(phIndex, 'Employment', 'currently_employed', 'hours_worked_last_week', 'employment_tenure', 'looking_for_work')
                inSchool = self.getHistoryRelatedColumnData(phIndex, 'CurrentlyInSchool', 'currently_in_school')
                vocational = self.getHistoryRelatedColumnData(phIndex, 'VocationalTraining', 'vocational_training')
                highestSchool = self.getHistoryRelatedColumnData(phIndex, 'HighestSchoolLevel', 'highest_school_level')
                (degreeNum, degreeStr) = self.getHistoryRelatedColumnData(phIndex, 'Degree', 'degree_id_id_num', 'degree_id_id_str')
                degree = self.chooseId(degreeNum, degreeStr)
                healthStatus = self.getHistoryRelatedColumnData(phIndex, 'HealthStatus', 'health_status')
                (pregnant, dueDate) = self.getHistoryRelatedColumnData(phIndex, 'Pregnancy', 'pregnancy_status', 'due_date')
                dueDate = dueDate
                serviceEra = self.getHistoryRelatedColumnData(phIndex, 'VeteranServiceEra', 'service_era')
                serviceDur = self.getHistoryRelatedColumnData(phIndex, 'VeteranMilitaryServiceDuration', 'military_service_duration')
                servedInWz = self.getHistoryRelatedColumnData(phIndex, 'VeteranServedInWarZone', 'served_in_war_zone')
                (wzNum, wzStr, wzMonths, wzFire) = self.getHistoryRelatedColumnData(phIndex, 'VeteranWarzonesServed', 'war_zone_id_id_id_num', 'war_zone_id_id_id_str', 'months_in_war_zone', 'received_fire')
                warZone = wzNum
                (branch, discharge) = self.getHistoryRelatedColumnData(phIndex, 'VeteranMilitaryBranches', 'military_branch', 'discharge_status')
                (cesIndex, childInSchool, school, mvLiason, schoolType, lastSchoolDt) = self.getHistoryRelatedColumnData(phIndex, 'ChildEnrollmentStatus', 'id', 'child_currently_enrolled_in_school', 'child_school_name', 'child_mckinney_vento_liason', 'child_school_type', 'child_last_enrolled_date')
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['historical']
                raise
    ...
    def createClientRecs(self, exportId):
        for person in self.getPersonData(exportId):
            try:
                ...
                personId = self.chooseId(person.person_id_id_num, person.person_id_id_str)
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['client']
                raise
    ...
    def createBedInventoryRecs(self, siteService, orgId):
        for inventory in self.getInventoryData(siteService.id):
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['inventory']
                raise
    ...
    def createRegionsRecs(self, siteService, orgId):
        for region in self.getRegionData(siteService.key):
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['regions']
                raise
    ...
    def createSiteInformationRecs(self, site, orgId):
        for siteService in self.getSiteServiceData(site.id):
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['siteInfo']
                raise
    ...
    def createAgencyProgramRecs(self, exportIndex):
        ...
        for (agency, service, site) in self.getAgencyProgramData(exportIndex):
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['agency']
                raise
    ...
    def createExportRecs(self):
        ...
        for export in self.getExportData():
            ...
            try:
                ...
                print 'Unable to write record to CSV file %s!' % HmisCsv30Writer.files['export']
                raise
