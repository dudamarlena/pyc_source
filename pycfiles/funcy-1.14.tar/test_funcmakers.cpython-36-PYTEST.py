...
def test_callable():
    ...
    if not @py_assert9:
        @py_format12 = @pytest_ar._call_reprcompare(('==', ), (@py_assert9,), ('%(py8)s\n{%(py8)s = %(py4)s\n{%(py4)s = %(py0)s(%(py2)s)\n}(%(py6)s)\n} == %(py11)s', ), (@py_assert7, @py_assert10)) % {'py0':@pytest_ar._saferepr(make_func) if 'make_func' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(make_func) else 'make_func',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py8':@pytest_ar._saferepr(@py_assert7),  'py11':@pytest_ar._saferepr(@py_assert10)}
...
def test_slice():
    ...
    if not @py_assert14:
        @py_format17 = @pytest_ar._call_reprcompare(('==', ), (@py_assert14,), ('%(py13)s\n{%(py13)s = %(py9)s\n{%(py9)s = %(py0)s(%(py7)s\n{%(py7)s = %(py1)s(%(py3)s, %(py5)s)\n})\n}(%(py11)s)\n} == %(py16)s', ), (@py_assert12, @py_assert15)) % {'py0':@pytest_ar._saferepr(make_func) if 'make_func' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(make_func) else 'make_func',  'py1':@pytest_ar._saferepr(slice) if 'slice' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(slice) else 'slice',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(@py_assert4),  'py7':@pytest_ar._saferepr(@py_assert6),  'py9':@pytest_ar._saferepr(@py_assert8),  'py11':@pytest_ar._saferepr(@py_assert10),  'py13':@pytest_ar._saferepr(@py_assert12),  'py16':@pytest_ar._saferepr(@py_assert15)}
...
def test_str():
    ...
    if not @py_assert9:
        @py_format12 = @pytest_ar._call_reprcompare(('is', ), (@py_assert9,), ('%(py8)s\n{%(py8)s = %(py4)s\n{%(py4)s = %(py0)s(%(py2)s)\n}(%(py6)s)\n} is %(py11)s', ), (@py_assert7, @py_assert10)) % {'py0':@pytest_ar._saferepr(make_func) if 'make_func' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(make_func) else 'make_func',  'py2':@pytest_ar._saferepr(@py_assert1),  'py4':@pytest_ar._saferepr(@py_assert3),  'py6':@pytest_ar._saferepr(@py_assert5),  'py8':@pytest_ar._saferepr(@py_assert7),  'py11':@pytest_ar._saferepr(@py_assert10)}
        @py_format14 = 'assert %(py13)s' % {'py13': @py_format12}
        raise AssertionError(@pytest_ar._format_explanation(@py_format14))
    @py_assert1 = @py_assert3 = @py_assert5 = @py_assert7 = @py_assert9 = @py_assert10 = None
    @py_assert1 = '\\d+'
...
def test_set():
    ...
    if not @py_assert8:
        @py_format11 = @pytest_ar._call_reprcompare(('is', ), (@py_assert8,), ('%(py7)s\n{%(py7)s = %(py3)s\n{%(py3)s = %(py0)s(%(py1)s)\n}(%(py5)s)\n} is %(py10)s', ), (@py_assert6, @py_assert9)) % {'py0':@pytest_ar._saferepr(make_func) if 'make_func' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(make_func) else 'make_func',  'py1':@pytest_ar._saferepr(s) if 's' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(s) else 's',  'py3':@pytest_ar._saferepr(@py_assert2),  'py5':@pytest_ar._saferepr(@py_assert4),  'py7':@pytest_ar._saferepr(@py_assert6),  'py10':@pytest_ar._saferepr(@py_assert9)}
        @py_format13 = 'assert %(py12)s' % {'py12': @py_format11}
        raise AssertionError(@pytest_ar._format_explanation(@py_format13))
    @py_assert2 = @py_assert4 = @py_assert6 = @py_assert8 = @py_assert9 = None
    @py_assert2 = make_func(s)
