...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/opensearchsdk/utils/prepare_url.py
# Compiled at: 2015-12-07 07:33:57
import base64, hashlib, hmac, six, sys, time, uuid
try:
...
def get_common_params(key_id):
    params = dict(Version='v2', SignatureMethod='HMAC-SHA1', SignatureVersion='1.0', Timestamp=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), SignatureNonce=str(uuid.uuid4()), AccessKeyId=key_id)
    return params
