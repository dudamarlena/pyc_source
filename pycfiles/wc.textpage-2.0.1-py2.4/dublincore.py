...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/wc/textpage/dublincore.py
...
class ITextMetadata(Interface):
    ...
    """
    __module__ = __name__
    title = TextLine(title='Title', description='Title of the text document.')
    description = TextLine(title='Description', description='Abstract or description of the text, usually the first paragraph.')


class ReSTMetadata(object):
    r"""Extract title from ReST source.
    ...
    def __init__(self, text):
        ...
        if titles:
            title = (' ').join([ node.nodeValue for node in titles[0].childNodes if node.nodeValue ])
            self.title = unicode(title)
        paras = dom.getElementsByTagName('paragraph')
        if paras:
            description = (' ').join([ node.nodeValue for node in paras[0].childNodes if node.nodeValue ])
            self.description = unicode(description)


class PlainTextMetadata(object):
    r"""Extract title from a plain text source.
...
class STXMetadata(object):
    r"""Extract title from a StructuredText source.
