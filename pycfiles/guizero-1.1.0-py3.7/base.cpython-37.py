...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\guizero\base.py
# Compiled at: 2019-10-24 09:39:32
# Size of source mod 2**32: 21433 bytes
"""
Abstract classes for guizero.
"""
from .tkmixins import ScheduleMixin, DestroyMixin, EnableMixin, FocusMixin, DisplayMixin, TextMixin, ColorMixin, SizeMixin, LayoutMixin, EventsMixin
...
class Component(Base, ScheduleMixin, DestroyMixin, FocusMixin, ColorMixin, EventsMixin):
...
class Container(Component):
            ...
            if widget.width == 'fill':
                ...
                if widget.height == 'fill':
                    ...
                    if widget.align is not None:
                        ...
                        if pack_params.get('side') is None:
                            if pack_params.get('fill') == Y:
                                pack_params['expand'] = YES
    ...
    def _grid_widget(self, widget):
        grid_params = {'column':widget.grid[0], 
         'row':widget.grid[1]}
...
class Widget(Component, EnableMixin, DisplayMixin, SizeMixin, LayoutMixin):
...
class TextWidget(Widget, TextMixin):
...
class ContainerWidget(Container, EnableMixin, DisplayMixin, SizeMixin, LayoutMixin):
    ...
    def _set_propagation(self, width, height):
        ...
        if width is None:
            ...
            if isinstance(height, int):
                if height > 0:
                    propagate_value = False
            if isinstance(width, int) and isinstance(height, int) and (width == 0 and height > 0 or height == 0):
                if width > 0:
                    utils.error_format('You must specify a width and a height for {}'.format(self.description))
        propagate_function(propagate_value)


class ContainerTextWidget(ContainerWidget, TextMixin):
