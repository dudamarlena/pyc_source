...
# [GCC 8.4.0]
# Embedded file name: C:\fyp-media-recommender\mediarecommender\recommender\recommender.py
# Compiled at: 2019-04-08 00:57:39
...
class Recommender:
    ...
    def __init__(self, user_favorites_dict):
        ...
        self.user_vector = self._Recommender__user_favorites_to_user_vector()

    def generate_k_recommendations(self, media_type, k):
        item_vectors = []
        all_items_without_favorites = []
        if media_type == Media.MOVIE:
            favorite_movies_id = self._Recommender__get_favorites_id('Movie')
            ...
            if media_type == Media.GAME:
                favorite_games_id = self._Recommender__get_favorites_id('Game')
                ...
                if media_type == Media.BOOK:
                    favorite_books_id = self._Recommender__get_favorites_id('Book')
        ...
        recommended_movies = []
        for i in indices[0]:
            recommended_movie_dict = {'id':all_items_without_favorites[i].id, 
