...
# [GCC 8.4.0]
# Embedded file name: /home/sam/git/Kanban/django-autotask/djautotask/tests/test_sync.py
# Compiled at: 2020-05-12 14:17:57
# Size of source mod 2**32: 50232 bytes
from django.test import TestCase
from atws.wrapper import Wrapper
import random
from copy import deepcopy
from djautotask.models import Ticket, Status, Resource, SyncJob, TicketSecondaryResource, Priority, Queue, Account, Project, ProjectType, ProjectStatus, TicketCategory, Source, IssueType, SubIssueType, TicketType, DisplayColor, LicenseType, Task, TaskSecondaryResource, Phase, TimeEntry, TicketNote, TaskNote, TaskTypeLink, UseType, AllocationCode, Role, Department, ResourceRoleDepartment, ResourceServiceDeskRole, Contract, AccountType, ServiceCall, ServiceCallStatus, AccountPhysicalLocation
...
class TestAssignNullRelationMixin:
    ...
    def test_sync_assigns_null_relation(self):
        ...
        fixture_instance = deepcopy(getattr(fixtures, 'API_{}'.format(model_type.upper())))
        fixture_instance.pop('AssignedResourceID')
        object_instance = fixture_utils.generate_objects(model_type, [fixture_instance])
        _, patch = mocks.create_mock_call(mocks.WRAPPER_QUERY_METHOD, object_instance)
...
class TestTicketNoteSynchronizer(TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.create_date_time, object_data['CreateDateTime'])
        self.assertEqual(instance.last_activity_date, object_data['LastActivityDate'])
        self.assertEqual(instance.ticket.id, object_data['TicketID'])
        self.assertEqual(instance.creator_resource.id, object_data['CreatorResourceID'])
...
class TestTicketSynchronizer(TestAssignNullRelationMixin, TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.estimated_hours, object_data['EstimatedHours'])
        self.assertEqual(instance.last_activity_date, object_data['LastActivityDate'])
        self.assertEqual(instance.status.id, object_data['Status'])
        self.assertEqual(instance.assigned_resource.id, object_data['AssignedResourceID'])
    ...
    def test_sync_ticket_related_records(self):
        ...
        time_mock, time_patch = mocks.create_mock_call('djautotask.sync.TimeEntrySynchronizer.fetch_records', None)
        note_mock, note_patch = mocks.create_mock_call('djautotask.sync.TicketNoteSynchronizer.fetch_records', None)
        resource_mock, resource_patch = mocks.create_mock_call('djautotask.sync.TicketSecondaryResourceSynchronizer.fetch_records', None)
...
class AbstractPicklistSynchronizer(object):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.is_default_value, object_data['IsDefaultValue'])
...
class TestServiceCallStatusSynchronizer(AbstractPicklistSynchronizer, TestCase):
...
class TestTicketCategorySynchronizer(TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.display_color.id, object_data['DisplayColorRGB'])
...
class FilterProjectTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.complete_status = ProjectStatus.objects.create(label=ProjectStatus.COMPLETE, is_active=True)
        cls.inactive_status = ProjectStatus.objects.create(label='New (Inactive)', is_active=False)
...
class TestProjectSynchronizer(FilterProjectTestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.completed_date, object_data['CompletedDateTime'].date())
        self.assertEqual(instance.completed_percentage, object_data['CompletedPercentage'])
        self.assertEqual(instance.duration, object_data['Duration'])
        self.assertEqual(instance.start_date, object_data['StartDateTime'].date())
        self.assertEqual(instance.end_date, object_data['EndDateTime'].date())
        self.assertEqual(instance.estimated_time, object_data['EstimatedTime'])
        self.assertEqual(instance.last_activity_date_time, object_data['LastActivityDateTime'])
        self.assertEqual(instance.project_lead_resource.id, object_data['ProjectLeadResourceID'])
        self.assertEqual(instance.account.id, object_data['AccountID'])
        self.assertEqual(instance.status.id, object_data['Status'])
    ...
    def test_sync_filters_projects_in_inactive_status(self):
        ...
        project_instance = fixture_utils.generate_objects('Project', [project_fixture, fixtures.API_PROJECT_LIST[0]])
        _, patch = mocks.create_mock_call(mocks.WRAPPER_QUERY_METHOD, project_instance)
        synchronizer = sync.ProjectSynchronizer(full=True)
        synchronizer.sync()
...
class TestTaskSynchronizer(TestAssignNullRelationMixin, FilterProjectTestCase):
    model_class = Task
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.completed_date, object_data['CompletedDateTime'])
        ...
        self.assertEqual(instance.description, object_data['Description'])
        self.assertEqual(instance.remaining_hours, object_data['RemainingHours'])
        self.assertEqual(instance.estimated_hours, object_data['EstimatedHours'])
        self.assertEqual(instance.last_activity_date, object_data['LastActivityDateTime'])
        self.assertEqual(instance.status.id, object_data['Status'])
    ...
    def test_sync_filters_tasks_on_inactive_project(self):
        ...
        task_instance = fixture_utils.generate_objects('Task', [task_fixture, fixtures.API_TASK])
        _, patch = mocks.create_mock_call(mocks.WRAPPER_QUERY_METHOD, task_instance)
        synchronizer = sync.TaskSynchronizer(full=True)
        synchronizer.sync()
...
class TestTimeEntrySynchronizer(TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.start_date_time, object_data['StartDateTime'])
    ...
    def test_sync_time_entry(self):
        ...
        """
        time_entries = fixture_utils.generate_objects('TimeEntry', fixtures.API_TIME_ENTRY_LIST)
    ...
    def test_batch_queries_creates_multiple_batches(self):
        ...
        _, _patch = mocks.create_mock_call('django.db.models.query.QuerySet.values_list', object_ids)
    ...
    def test_batch_queries_returns_query_list(self):
        ...
        """
        _, _patch = mocks.create_mock_call('django.db.models.query.QuerySet.values_list', [])
        synchronizer = sync.TimeEntrySynchronizer()
        sync_job = SyncJob.objects.filter(entity_name='TimeEntry')
        batch_query_list = synchronizer.build_batch_queries(sync_job)
...
class TestResourceRoleDepartmentSynchronizer(TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.department_lead, object_data['DepartmentLead'])
        self.assertEqual(instance.resource.id, object_data['ResourceID'])
    ...
    def test_sync_resource_role_department(self):
        ...
        """
        resource_role_department = fixture_utils.generate_objects('ResourceRoleDepartment', fixtures.API_RESOURCE_ROLE_DEPARTMENT_LIST)
...
class TestResourceServiceDeskRoleSynchronizer(TestCase):
    ...
    def test_sync_resource_service_desk_role(self):
        ...
        Test to ensure synchronizer saves an instance locally.
        """
        resource_service_desk_role = fixture_utils.generate_objects('ResourceServiceDeskRole', fixtures.API_RESOURCE_SERVICE_DESK_ROLE_LIST)
...
class TestContractSynchronizer(TestCase):
    ...
    def test_sync_contract(self):
        ...
        """
        contract = fixture_utils.generate_objects('Contract', fixtures.API_CONTRACT_LIST)
...
class TestServiceCallSynchronizer(TestCase):
    ...
    def _assert_sync(self, instance, object_data):
        ...
        self.assertEqual(instance.start_date_time, object_data['StartDateTime'])
        self.assertEqual(instance.end_date_time, object_data['EndDateTime'])
        self.assertEqual(instance.canceled_date_time, object_data['CanceledDateTime'])
        self.assertEqual(instance.last_modified_date_time, object_data['LastModifiedDateTime'])
        self.assertEqual(instance.status.id, object_data['Status'])
        self.assertEqual(instance.account.id, object_data['AccountID'])
        self.assertEqual(instance.creator_resource.id, object_data['CreatorResourceID'])
        self.assertEqual(instance.canceled_by_resource.id, object_data['CanceledByResource'])

    def test_sync_service_call(self):
        """
        Test to ensure synchronizer saves a service call instance locally.
        """
        service_call = fixture_utils.generate_objects('ServiceCall', fixtures.API_SERVICE_CALL_LIST)
