...
# [GCC 8.4.0]
# Embedded file name: /home/sam/git/Kanban/django-autotask/djautotask/api.py
...
def get_errors(response):
    ...
    try:
        ...
        error = 'An Autotask API error occurred. The error was: {}'.format(response.errors[0])
        errors.append(error)
...
def init_api_connection(**kwargs):
    ...
    kwargs['integrationcode'] = settings.AUTOTASK_CREDENTIALS['integration_code']
...
def get_connection_url(**kwargs):
    ...
    logger.debug('Cached Autotask {} was: {}'.format(cache_key, wsdl_url_from_cache))
    if not wsdl_url_from_cache:
        try:
            url = connection.get_connection_url(**kwargs)
            cache.set(cache_key, url)
        except transport.TransportError as e:
            raise AtwsTransportError('Failed to get webservices URL: {}'.format(e))
...
def get_web_url():
    ...
    if not web_url_from_cache:
        try:
            url = connection.get_zone_info(settings.AUTOTASK_CREDENTIALS['username'], settings.AUTOTASK_CREDENTIALS['api_version'])['WebUrl']
...
class AutotaskRequestsTransport(transport.Transport):
    ...
    def __init__(self):
        ...
        self.session.mount('https://', requests.adapters.HTTPAdapter(max_retries=self.request_settings.get('max_attempts')))
    ...
    def send(self, request):
        for attempt in range(1, self.max_attempts + 1):
            try:
                resp = self.session.post(request.url, data=request.message, headers=request.headers, timeout=self.timeout)
                break
            except RequestException as e:
                if attempt == self.max_attempts:
                    response = self.format_error_message(e)
...
def delete_objects(entity_type, objects):
    ...
    for instance in objects:
        objects_to_delete.append(fetch_object(entity_type, instance.id, at))
