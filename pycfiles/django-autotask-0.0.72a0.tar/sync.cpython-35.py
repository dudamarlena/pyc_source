...
# [GCC 8.4.0]
# Embedded file name: /home/sam/git/Kanban/django-autotask/djautotask/sync.py
...
class Synchronizer:
    ...
    def set_relations(self, instance, object_data):
        for object_field, value in self.related_meta.items():
            ...
            self._assign_relation(instance, object_data, object_field, model_class, field_name)
    ...
    def _assign_relation(self, instance, object_data, object_field, model_class, field_name):
        ...
        try:
            ...
            logger.warning('Failed to find {} {} for {} {}.'.format(object_field, relation_id, type(instance), instance.id))

    def _instance_ids(self, filter_params=None):
        if not filter_params:
            ids = self.model_class.objects.all().order_by(self.db_lookup_key).values_list('id', flat=True)
        else:
            ids = self.model_class.objects.filter(filter_params).order_by(self.db_lookup_key).values_list('id', flat=True)
        return set(ids)

    def build_base_query(self, sync_job_qset):
        query = Query(self.model_class.__name__)
        if sync_job_qset.exists() and self.last_updated_field and not self.full:
            last_sync_job_time = sync_job_qset.last().start_time
            query.WHERE(self.last_updated_field, query.GreaterThanorEquals, last_sync_job_time)
        else:
    ...
    def get(self, results):
        ...
        """
        sync_job_qset = models.SyncJob.objects.filter(entity_name=self.model_class.__name__)
...
class PicklistSynchronizer(Synchronizer):
    ...
    def sync(self):
        ...
        self.at_api_client = api.init_api_connection()
        field_info = helpers.get_field_info(self.at_api_client, self.entity_type)
        try:
            field_picklist = picklist.get_field_picklist(self.picklist_field, field_info)
            picklist_objects = field_picklist.PicklistValues[0]
        except KeyError as e:
            logger.warning('Failed to find {} picklist field. {}'.format(self.picklist_field, e))
...
class ParentSynchronizer:
    ...
    def sync_children(self, *args):
        for synchronizer, query_params in args:
            created_count, updated_count, deleted_count = synchronizer.callback_sync(query_params)
...
class ChildSynchronizer:
    ...
    def _child_instance_ids(self, query_params):
        ids = self.model_class.objects.filter(ticket__id=query_params[1]).order_by(self.db_lookup_key).values_list('id', flat=True)
        return set(ids)

    def _child_query_condition(self, query, query_params):
        parent_field, parent_id = query_params
        query.open_bracket('AND')
        query.WHERE(parent_field, query.Equals, parent_id)
        query.close_bracket()
        self._get_query_conditions(query)
...
class QueryConditionMixin:
    ...
    def _get_query_conditions(self, query):
        ...
        query.open_bracket('AND')
        query.WHERE('Status', query.NotEqual, models.Status.COMPLETE_ID)
        query.open_bracket('OR')
        query.WHERE(self.completed_date_field, query.GreaterThan, (timezone.now() - timezone.timedelta(hours=request_settings.get('keep_completed_hours'))).isoformat())
        query.close_bracket()
        query.close_bracket()
        return query
...
class BatchQueryMixin:
    ...
    def _build_fk_batch(self, model_class, object_id_field, sync_job_qset):
        """
        Generic batching method for batching based off of the fk relation of
        an object currently present in the local DB.
        """
        queries = []
        object_ids = list(model_class.objects.order_by(self.db_lookup_key).values_list('id', flat=True))
        while object_ids:
            query = self.build_base_query(sync_job_qset)
            query.open_bracket('AND')
...
class TicketSynchronizer(QueryConditionMixin, Synchronizer, ParentSynchronizer):
    model_class = models.Ticket
    last_updated_field = 'LastActivityDate'
    completed_date_field = 'CompletedDate'
    related_meta = {'Status': (models.Status, 'status'), 
    ...
    def _assign_field_data(self, instance, object_data):
        ...
        instance.first_response_date_time = object_data.get('FirstResponseDateTime')
        instance.first_response_due_date_time = object_data.get('FirstResponseDueDateTime')
        instance.resolution_plan_date_time = object_data.get('ResolutionPlanDateTime')
        instance.resolution_plan_due_date_time = object_data.get('ResolutionPlanDueDateTime')
        instance.resolved_date_time = object_data.get('ResolvedDateTime')
        instance.resolved_due_date_time = object_data.get('ResolvedDueDateTime')
...
class SubIssueTypeSynchronizer(TicketPicklistSynchronizer):
    ...
    related_meta = {'parentValue': (models.IssueType, 'parent_value')}
...
class ResourceSynchronizer(Synchronizer):
    ...
    last_updated_field = None
    related_meta = {'LicenseType': (models.LicenseType, 'license_type'), 
...
class TicketCategorySynchronizer(Synchronizer):
    ...
    last_updated_field = None
    related_meta = {'DisplayColorRGB': (models.DisplayColor, 'display_color')}
...
class TicketSecondaryResourceSynchronizer(SecondaryResourceSyncronizer, ChildSynchronizer):
    last_updated_field = None
    model_class = models.TicketSecondaryResource
    model_name = 'TicketSecondaryResource'
    id_type = 'TicketID'
    related_meta = {'ResourceID': (models.Resource, 'resource'), 
...
class AccountSynchronizer(Synchronizer):
    ...
    last_updated_field = 'LastActivityDate'
    related_meta = {'AccountType': (models.AccountType, 'type')}
...
class AccountPhysicalLocationSynchronizer(Synchronizer):
    ...
    related_meta = {'AccountID': (models.Account, 'account')}
...
class FilterProjectStatusMixin:
    ...
    def fetch_records(self, query, results):
        ...
        for record in self.at_api_client.query(query):
            ...
            if active_object_ids and object_id and object_id not in active_object_ids:
                logger.info('Project with ID: {} is set to an inactive status. Skipping this {}.'.format(object_id, self.model_class.__name__))
                continue
                self.persist_record(record, results)
...
class ProjectSynchronizer(FilterProjectStatusMixin, Synchronizer):
    ...
    related_meta = {'ProjectLeadResourceID': (models.Resource, 'project_lead_resource'), 
     'AccountID': (models.Account, 'account'), 
    ...
    def get_active_ids(self):
        active_project_statuses = models.ProjectStatus.objects.exclude(is_active=False).values_list('id', flat=True).order_by(self.db_lookup_key)
        return active_project_statuses

    def _get_query_conditions(self, query):
        try:
            status = models.ProjectStatus.objects.get(label=models.ProjectStatus.COMPLETE)
            query.open_bracket('AND')
            query.WHERE('Status', query.NotEqual, status.id)
            query.close_bracket()
        except models.ProjectStatus.DoesNotExist as e:
            logger.warning('Failed to find project status - {}. {}'.format(models.ProjectStatus.COMPLETE, e))
    ...
    def _assign_field_data(self, instance, object_data):
        ...
        instance.last_activity_date_time = object_data.get('LastActivityDateTime')
...
class PhaseSynchronizer(Synchronizer):
    ...
    last_updated_field = 'LastActivityDateTime'
    related_meta = {'ProjectID': (models.Project, 'project'), 
...
class TaskSynchronizer(QueryConditionMixin, FilterProjectStatusMixin, Synchronizer):
    model_class = models.Task
    last_updated_field = 'LastActivityDateTime'
    object_filter_field = 'ProjectID'
    completed_date_field = 'CompletedDateTime'
    related_meta = {'AssignedResourceID': (models.Resource, 'assigned_resource'), 
     'ProjectID': (models.Project, 'project'), 
    ...
    def get_active_ids(self):
        active_projects = models.Project.objects.exclude(Q(status__is_active=False) | Q(status__label=models.ProjectStatus.COMPLETE)).values_list('id', flat=True).order_by(self.db_lookup_key)
...
class NoteSynchronizer(Synchronizer):
    ...
    def _get_query_conditions(self, query):
        query.open_bracket('AND')
        query.WHERE('NoteType', query.NotEqual, models.NoteType.WORKFLOW_RULE_NOTE_ID)
        query.close_bracket()
        return query
    ...
    def create(self, **kwargs):
        ...
        """
        description = '{}\n\nNote was added by {} {}.'.format(kwargs['description'], kwargs['resource'].first_name, kwargs['resource'].last_name)
...
class TicketNoteSynchronizer(BatchQueryMixin, NoteSynchronizer, ChildSynchronizer):
    model_class = models.TicketNote
    last_updated_field = 'LastActivityDate'
    related_model_field = 'TicketID'
    model_name = 'TicketNote'
    related_meta = {'NoteType': (models.NoteType, 'note_type'), 
     'CreatorResourceID': (models.Resource, 'creator_resource'), 
     'TicketID': (models.Ticket, 'ticket')}

    def build_batch_queries(self, sync_job_qset):
        batch_query_list = self._build_fk_batch(models.Ticket, 'TicketID', sync_job_qset)
        return batch_query_list


class TaskNoteSynchronizer(BatchQueryMixin, NoteSynchronizer):
    ...
    def build_batch_queries(self, sync_job_qset):
        batch_query_list = self._build_fk_batch(models.Task, 'TaskID', sync_job_qset)
        return batch_query_list
...
class TimeEntrySynchronizer(BatchQueryMixin, Synchronizer, ChildSynchronizer):
    ...
    def build_batch_queries(self, sync_job_qset):
        batch_query_list = []
        batch_query_list.extend(self._build_fk_batch(models.Ticket, 'TicketID', sync_job_qset))
        batch_query_list.extend(self._build_fk_batch(models.Task, 'TaskID', sync_job_qset))
        return batch_query_list
...
class AllocationCodeSynchronizer(Synchronizer):
    ...
    last_updated_field = None
    related_meta = {'UseType': (models.UseType, 'use_type')}
...
class ServiceCallSynchronizer(Synchronizer):
    ...
    def _assign_field_data(self, instance, object_data):
        ...
        instance.last_modified_date_time = object_data.get('LastModifiedDateTime')
        self.set_relations(instance, object_data)
        return instance
...
class ServiceCallTicketSynchronizer(BatchQueryMixin, Synchronizer):
    ...
    related_meta = {'ServiceCallID': (models.ServiceCall, 'service_call'), 
     'TicketID': (models.Ticket, 'ticket')}
...
class ServiceCallTicketResourceSynchronizer(BatchQueryMixin, Synchronizer):
    ...
    related_meta = {'ServiceCallTicketID': (
    ...
    def build_batch_queries(self, sync_job_qset):
        batch_query_list = self._build_fk_batch(models.ServiceCallTicket, 'ServiceCallTicketID', sync_job_qset)
        return batch_query_list


class ServiceCallTaskResourceSynchronizer(BatchQueryMixin, Synchronizer):
    model_class = models.ServiceCallTaskResource
    related_meta = {'ServiceCallTaskID': (
    ...
    def build_batch_queries(self, sync_job_qset):
        batch_query_list = self._build_fk_batch(models.ServiceCallTask, 'ServiceCallTaskID', sync_job_qset)
        return batch_query_list