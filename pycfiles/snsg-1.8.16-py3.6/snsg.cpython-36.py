...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\snsg\snsg.py
# Compiled at: 2018-03-19 04:57:42
...
class SNSG(object):
    ...
    def read_ls(self, ls):
        ...
        for word in ls:
            self.split(words=self._SNSG__clean(data=word))
            ...
            for j in range(1, self.split_num + 1):
                if i + j < lens:
                    ...
                    if key in self.sns_words:
                        ...
                        self.sns_words[key] = [
                         1, 1 / self.all_lens, 1, 0, [words[(i - 1)]], [words[(i + 1)]]]
    ...
    def filter(self, filter_cond=10, filter_free=0.5, flag=False):
        ...
        for max_key in keys:
            ...
            if lens > 4:
                ...
                split_flag = 0
                while i <= lens:
                    if max_key[split_flag:i] in keys:
                        if max_key[split_flag:i] != max_key:
                            split_flag = i
                    i += 1
