...
# Embedded file name: C:\symlinks\repos\boilerplate_dcc_pyside_widget\boilerplate_dcc_pyside_widget\lib\third_party\pysideuic\properties.py
...
class Properties(object):
    ...
    def _string(self, prop, notr=None):
        ...
        if text is None:
            ...
            if prop.get('notr', notr) == 'true':
                return text
            return QtGui.QApplication.translate(self.uiname, text, prop.get('comment'), QtGui.QApplication.UnicodeUTF8)
        ...
        if gtype == 'LinearGradient':
            ...
            if gtype == 'ConicalGradient':
                ...
                gradient = self.factory.createQObject('QRadialGradient', name, (
                 centralx, centraly, radius, focalx, focaly), is_attribute=False)
            else:
    ...
    def orientation(self, widget, prop):
        if widget.metaObject().className() == 'QFrame':
            widget.setFrameShape({'Qt::Horizontal': QtGui.QFrame.HLine, 'Qt::Vertical': QtGui.QFrame.VLine}[prop[0].text])
