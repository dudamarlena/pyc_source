...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/imgur_scraper/imgur_scraper.py
# Compiled at: 2019-09-16 06:25:39
...
def get_viral_posts_from(start_date: str, end_date: str) -> json:
    ...
    for days_ago in reversed(range(end, start + 1)):
        counter = 0
        r = HTMLSession().get(f"https://imgur.com/gallery/hot/viral/page/{days_ago}/hit?scrolled&set={counter}")
        if r.html.find('.images-header-main'):
            print('Grabbing ' + ' '.join(r.html.find('.images-header-main')[0].full_text.split()))
        while not r.html.find('#nomore'):
            for entries in r.html.find('.post'):
                yield {'title':entries.find('.hover > p')[0].full_text, 
...
def main():
    parser = argparse.ArgumentParser(prog='imgur-scraper',
    ...
    parser.add_argument('--path',
      ...
      type=str,
      dest='path_to_save',
      help='path to save the csv file in             (defaults to the current working directory)')
    ...
    if to_csv:
        try:
            ...
            with open(file_name, 'x', newline='', encoding='utf-8') as (csvfile):
                fieldnames = [
                 'title', 'url', 'points', 'tags', 'type', 'views']
