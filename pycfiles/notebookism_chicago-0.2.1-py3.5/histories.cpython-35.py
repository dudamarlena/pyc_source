...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/notebookism/histories.py
# Compiled at: 2016-10-01 22:45:51
# Size of source mod 2**32: 6541 bytes
...
__notebooks = (__x('http://localhost:8888/api/contents') | requests.get | _this().json()['content']._) + __x().get('type').eq('notebook').__
__contents = __notebooks | pluck('name') | list | map(read_json) | list
__notebook_imports = __x(__contents.copy().pluck('cells').first.__()) + _this()['cell_type'].copy().eq('code')._ | pluck('source') | concat | '\n'.join | (lambda x: x.split(';')) | map(lambda x: first(x.split('#', 1))) | '\n'.join | str.splitlines
__notebook_imports = __notebook_imports * str.lstrip + (lambda x: x.startswith('from ') or x.startswith('import '))
__notebook_imports = (__notebook_imports * (lambda x: second(x.split(' ', 1))) * (lambda x: first(x.split(' import', 1))) * (lambda x: first(x.split(' as', 1))) * (lambda x: x.split(',')) | concat) * (lambda x: first(x.split('.', 1))) | unique
__notebook_imports = __notebook_imports * str.strip | list
contents = __contents.copy().pluck('cells').list.__()
imports = __x(contents).map(__notebook_imports.__).zip(__notebooks * (
 _this()['created']._, _this()['name']._) > identity).dict.valmap(__x() * [identity, lambda x: 1] | dict > compose) | pandas.DataFrame > _this().fillna(0).transpose()._
imports = imports.set_index(imports.index.set_levels(pandas.to_datetime(imports.index.get_level_values(0)), level=0))
...
imports.groupby([
 ...
 imports.index.get_level_values(0).month // 3]).sum().transpose().sort_values((2016,
                                                                               2), ascending=False).head(15).transpose()
