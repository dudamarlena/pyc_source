...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tw/fusioncharts/elements.py
# Compiled at: 2008-05-20 19:38:56
try:
...
class XElem(object):
    ...
    def __validate(self):
        if self.valid_attrs:
            for a in self.attrs:
                assert a in self.valid_attrs, 'Invalid attribute %s on element %s' % (a, self.name)

        if self.valid_elems:
            for e in self.elems:
                assert e.name in self.valid_elems, 'Invalid element name %s in %s' % (e.name, self.name)
...
class Gantt(object):
    ...
    class datatable(XElem):
        pass

    class datacolumn(XElem):
        pass
