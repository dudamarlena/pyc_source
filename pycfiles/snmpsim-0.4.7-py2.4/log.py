...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim/log.py
# Compiled at: 2019-01-25 01:02:20
...
class SyslogLogger(AbstractLogger):
    ...
    def init(self, *priv):
        if len(priv) < 1:
            raise SnmpsimError('Bad syslog params, need at least facility, also accept host, port, socktype (tcp|udp)')
        if len(priv) < 2:
            priv = [
             priv[0], 'debug']
        if len(priv) < 3:
    ...
    def init(self, *priv):
        ...
        if len(priv) > 1 and priv[1]:
            try:
                ...
                raise SnmpsimError('Error in timed log rotation specification. Use <NNN>k,m,g for size or <NNN>S,M,H,D for time limits')
        ...
        try:
            ...
            raise SnmpsimError('Failure configure logging: %s' % sys.exc_info()[1])
...
METHODS_MAP = {'syslog': SyslogLogger, 'file': FileLogger, 'stdout': StdoutLogger, 'stderr': StderrLogger, 'null': NullLogger}
LEVELS_MAP = {'debug': LOG_DEBUG, 'info': LOG_INFO, 'error': LOG_ERROR}
msg = lambda x: None
logLevel = LOG_INFO
