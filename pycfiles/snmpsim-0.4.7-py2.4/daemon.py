...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim/daemon.py
# Compiled at: 2019-02-09 11:13:51
import sys
...
if sys.platform[:3] == 'win':
    ...
    class PrivilegesOf(object):
        ...
        def __enter__(self):
            if os.getuid() != 0:
                if self._uname and self._uname != pwd.getpwnam(self._uname).pw_name or self._gname and self._gname != grp.getgrnam(self._gname).gr_name:
                    raise error.SnmpsimError('Process is running under different UID/GID')
                else:
                    return
            elif not self._uname or not self._gname:
            ...
            try:
                ...
                raise error.SnmpsimError('getpwnam()/getgrnam() failed for %s/%s: %s' % (self._uname, self._gname, sys.exc_info()[1]))
            ...
            try:
                ...
                raise error.SnmpsimError('%s failed for %s/%s: %s' % (self._final and 'setgid()/setuid()' or 'setegid()/seteuid()', runningGid, runningUid, sys.exc_info()[1]))
        ...
        def __exit__(self, *args):
            ...
            try:
                ...
                raise error.SnmpsimError('setegid()/seteuid() failed for %s/%s: %s' % (self._oldgid, self._olduid, sys.exc_info()[1]))
