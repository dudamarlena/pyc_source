...
# [GCC 8.4.0]
# Embedded file name: /Users/jsvine/.virtualenvs/bikeshares/lib/python2.7/site-packages/bikeshares/programs/chicago.py
...
class Divvy(bikeshares.program.BikeShareProgram):
    ...
    def parse_trips(self, data_path):
        parsed = pd.read_csv(data_path, dtype=dict(gender='O'), usecols=[
         'starttime', 'stoptime', 'tripduration',
         'from_station_id', 'to_station_id',
         'bikeid', 'usertype', 'gender', 'birthday'], parse_dates=[
         'starttime', 'stoptime'])
    ...
    def parse_stations(self, data_path):
        parsed = pd.read_csv(data_path, usecols=[
         'id', 'name', 'dpcapacity',
         'online date', 'latitude', 'longitude'], parse_dates=[
         'online date'])
        mapped = pd.DataFrame({'id': parsed['id'], 
           ...
           'install_date': parsed['online date']}).groupby('id').first().reset_index()
        return mapped