...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/gazehound/shapes.py
...
class Shape(object):
    ...
    def __contains__(self, point):
        raise NotImplementedError('__contains__ must be overridden by subclass')


class Rectangle(Shape):

    def __init__(self, x1=None, y1=None, x2=None, y2=None, name='', description=''):
        super(Rectangle, self).__init__(name=name, description=description)
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
    ...
    def to_matrix(self, type_str='f', fill_value=1.0, bkg_value=0.0):
        ...
        matrix = [ array.array(type_str, [fill_value] * h) for c in range(0, w)
                 ]
        return matrix
...
class Ellipse(Shape):
    ...
    def __init__(self, cx=None, cy=None, semix=None, semiy=None, name='', description=''):
        super(Ellipse, self).__init__(name=name, description=description)
        self.cx = cx
        self.cy = cy
        self.semix = semix
        self.semiy = semiy
    ...
    def __contains__(self, point):
        ...
        """
        (x, y) = point
        return (x - self.cx) ** 2 / float(self.semix ** 2) + (y - self.cy) ** 2 / float(self.semiy ** 2) <= 1
    ...
    def to_matrix(self, type_str='f', fill_value=1.0, bkg_value=0.0):
        h = self.height()
        w = self.width()
        matrix = [ array.array(type_str, [bkg_value] * h) for c in range(0, w)
                 ]
        for i in range(0, w):
            for j in range(0, h):
...
class ShapeParser(object):
    ...
    def __init__(self):
        ...
        self.OBT_MAP = {'1': self.__parse_rectangle, 
    ...
    def __parse_rectangle(self, str, name):
        ...
        return Rectangle(x1, y1, x2, y2, description=description, name=name)
    ...
    def __parse_ellipse(self, str, name):
        ...
        return Ellipse(x, y, semi_x, semi_y, description=description, name=name)
...
class ShapeReader(object):
    ...
    def shapes_from_config_section(self, shape_tuples):
        parser = ShapeParser()
        return [ parser.parse_obt_str(st[1], name=st[0]) for st in shape_tuples ]
...
class TimelineDecorator(object):
    ...
    def find_file_and_add_shapes_to_presentation(self, presentation):
        presentation.shapes = self.shape_reader.find_file_and_create_shapes(presentation.name)
        return presentation