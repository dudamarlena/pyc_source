...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/gazehound/runners/fixation_statistics.py
# Compiled at: 2010-07-20 11:00:05
...
class FixationStatisticsOptionParser(object):
    ...
    def __init__(self, argv, err=sys.stderr):
        ...
        self.argv = argv
        parser = OptionParser()
        parser.add_option('--stimuli', dest='stim_file', help='Read stimulus timings from FILE', metavar='FILE')
        parser.add_option('--obt-dir', dest='object_dir', help='Find .OBT files in PATH. Requires --stimuli', metavar='PATH')
        parser.add_option('--recenter-on', dest='recenter_on', help='Recenter when stimuli named NAME are shown', metavar='NAME')
        (self.options, self.args) = parser.parse_args(argv[1:])
        if len(self.args) == 0:
            parser.error('No fixation file specified')
        if self.options.recenter_on is not None and self.options.stim_file is None:
...
class FixationStatsRunner(object):
    ...
    def __init__(self, argv):
        ...
        if op.options.stim_file is not None:
            ...
            if op.options.object_dir is not None:
                ...
                self.timeline = dec.find_shape_files_and_add_to_timeline(self.timeline)
            if op.options.recenter_on is not None:
                rx = 400
                ry = 300
                limiter = shapes.Rectangle(300, 200, 500, 400)
                self.timeline = self.timeline.recenter_on(op.options.recenter_on, rx, ry, limiter)
        self.analyzer = FixationStatisticsAnalyzer(pointpath=self.pointpath, timeline=self.timeline)
        return
    ...
    def __build_timeline(self, filename):
        ...
        timeline_with_points = viewing.Combiner(timeline=reader.timeline, pointpath=self.pointpath).viewings()
        return timeline_with_points


class FixationStatisticsAnalyzer(object):

    def __init__(self, pointpath=None, strict_valid_fun=None, pointpath_meta=None, timeline=[]):
        ...
        def default_valid(x_width, y_width, slop_frac, point):
            ...
            max_y = y_width + yslop
            min_y = 0 - yslop
            return point.x >= min_x and point.x < max_x and point.y >= min_y and point.y < max_y and not (point.x == 0 and point.y == 0)

        def svf(point):
            return default_valid(MAX_X, MAX_Y, 0, point)

        def lvf(point):
            return default_valid(MAX_X, MAX_Y, 0.1, point)
        ...
        def in_fun(shape, point):
            return (
             point.x, point.y) in shape

        self.in_fun = in_fun

    def general_stats(self):
        """Return a FixationStats containing basic data about the pointpath"""
        data = FixationStats(presented='screen', area='all', start_ms=self.pointpath[0].time, end_ms=self.pointpath[(-1)].time + self.pointpath[(-1)].duration, total_fixations=len(self.pointpath), time_fixating=self.__time_fixating(self.pointpath), fixations_per_second=self.__fixations_per_second(self.pointpath), distance_between_fixations=self.__distance_between_fixations(self.pointpath))
    ...
    def timeline_stats(self):
        ...
        for pres in self.timeline:
            if len(pres.pointpath) == 0:
                stats = FixationStats(presented=pres.name, area='all')
            else:
                stats = FixationStats(presented=pres.name, area='all', start_ms=pres.pointpath[0].time, end_ms=pres.pointpath[(-1)].time + pres.pointpath[(-1)].duration, total_fixations=len(pres.pointpath), time_fixating=self.__time_fixating(pres.pointpath), fixations_per_second=self.__fixations_per_second(pres.pointpath), distance_between_fixations=self.__distance_between_fixations(pres.pointpath))
    ...
    def shape_stats(self, pres):
        ...
        if pres.shapes is None:
            return [
             FixationStats(presented=pres.name, area="Can't read shape file")]
        else:
            for s in pres.shapes:
                if len(pres.pointpath) == 0:
                    stats = FixationStats(presented=pres.name, area=s.name)
                else:
                    stats = FixationStats(presented=pres.name, area=s.name, start_ms=pres.pointpath[0].time, end_ms=pres.pointpath[(-1)].time + pres.pointpath[(-1)].duration, total_fixations=len(pres.pointpath), time_fixating=self.__time_fixating(pres.pointpath), fixations_per_second=self.__fixations_per_second(pres.pointpath), distance_between_fixations=self.__distance_between_fixations(pres.pointpath))
    ...
    def __fixations_per_second(self, pointpath):
        ...
        if tf == 0:
            return 0
        return float(self.__total_fixations(pointpath)) / (self.__pp_duration(pointpath) / 1000.0)
...
class FixationStats(object):
    ...
    def __init__(self, presented=None, area=None, start_ms=0, end_ms=0, total_fixations=0, time_fixating=0, time_in=0, time_out=0, fixations_per_second=0, distance_between_fixations=0):
