...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/gazehound/runners/gaze_statistics.py
# Compiled at: 2010-07-20 11:26:53
import sys, os.path
...
class GazeStatsRunner(object):
    ...
    def __init__(self, argv):
        ...
        self.analyzer = GazeStatisticsAnalyzer(pointpath=self.pointpath, timeline=self.timeline)
        return
...
class GazeStatisticsAnalyzer(object):
    ...
    def general_stats(self):
        ...
        data = GazeStats(presented='screen', area='all', total_points=len(self.pointpath), start_ms=self.pointpath[0].time, end_ms=self.pointpath[(-1)].time, valid_strict=len(self.pointpath.valid_points(self.strict_valid_fun)), valid_lax=len(self.pointpath.valid_points(self.lax_valid_fun)))
    ...
    def timeline_stats(self):
        ...
        for pres in self.timeline:
            stats = GazeStats(presented=pres.name, area='all', total_points=len(pres.pointpath), start_ms=pres.start, end_ms=pres.end, valid_strict=len(pres.pointpath.valid_points(self.strict_valid_fun)), valid_lax=len(pres.pointpath.valid_points(self.lax_valid_fun)))
    ...
    def shape_stats(self, pres):
        ...
        if pres.shapes is None:
            return [
             GazeStats(presented=pres.name, area="Can't read shape file")]
        else:
            for s in pres.shapes:
                stats = GazeStats(presented=pres.name, area=s.name, total_points=len(pres.pointpath), start_ms=pres.start, end_ms=pres.end, valid_strict=len(pres.pointpath.valid_points(self.strict_valid_fun)), valid_lax=len(pres.pointpath.valid_points(self.lax_valid_fun)))
...
class GazeStats(object):
    ...
    def __init__(self, presented=None, area=None, start_ms=0, end_ms=0, total_points=0, points_in=0, points_out=0, valid_strict=0, valid_lax=0):
