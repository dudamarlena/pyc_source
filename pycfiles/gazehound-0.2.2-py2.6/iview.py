...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/gazehound/readers/iview.py
# Compiled at: 2010-07-20 17:19:08
import re
...
class IViewReader(DelimitedReader):
    ...
    def __init__(self, header_map, file_data=None, skip_comments=True, comment_char='#', opts_for_parser={}, filename=None):
        super(IViewReader, self).__init__(file_data, skip_comments, comment_char, opts_for_parser, filename)
        self.header_map = header_map

    def header_pairs(self):
        coms = [ re.sub('^%s' % self.comment_char, '', l).strip() for l in self.comment_lines
               ]
...
class IView2ScanpathReader(IViewReader):
    ...
    def __init__(self, file_data=None, skip_comments=True, comment_char='#', opts_for_parser={}, filename=None):
        super(IView2ScanpathReader, self).__init__(self.__header_map(), file_data, skip_comments, comment_char, opts_for_parser, filename)

    def pointpath(self):
        """Return a list of Points representing the scan path."""
        fact = gazepoint.IView2PointFactory()
        points = fact.from_component_list(self)
        return gazepoint.IViewPointPath(points=points, samples_per_second=self.header()['sample_rate'])
...
class IView3PointPathReader(IViewReader):
    ...
    def __init__(self, file_data=None, skip_comments=True, comment_char='##', opts_for_parser={}, filename=None, column_mapping=standard_column_mapping):
        super(IView3PointPathReader, self).__init__(self.__header_map, file_data, skip_comments, comment_char, opts_for_parser, filename)
        self.column_mapping = column_mapping

    def pointpath(self):
        fact = gazepoint.IView3PointFactory(self.measure_mapping)
        points = fact.from_component_list(self)
        return gazepoint.IView3Pointpath(points=points, samples_per_second=self.header()['sample_rate'])
    ...
    def _build_measure_map(self):
        ...
        for (measure_name, cols, fx) in self.column_mapping:
            self._meas_map[measure_name] = (
             self._col_index(cols), fx)

    def _col_index(self, cols):
        if type(cols) == str:
            cols = [
             cols]
        return [ self._column_headers.index(c) for c in cols if c in self._column_headers
               ][0]
    ...
    def __header_map(self):
        return {'Version': (
           ...
           'Stimulus Dimension [mm]': (
                                     'stimulus_dimension', lambda x: [ int(e) for e in x.split('\t') ] + ['mm']), 
           'Head Distance [mm]': (
                                'head_distance',
                                lambda x: [
                                 x, 'mm']), 
...
class IViewFixationReader(IViewReader):
    ...
    def __init__(self, file_data=None, skip_comments=True, comment_char='#', opts_for_parser={}, filename=None):
        super(IViewFixationReader, self).__init__(self.__header_map(), file_data, skip_comments, comment_char, opts_for_parser, filename)
