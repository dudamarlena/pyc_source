...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/gazehound/gazepoint.py
...
class Point(object):
    ...
    def __repr__(self):
        return '<gazehound.gazepoint.Point(x: %s, y: %s, time: %s, duration: %s)' % (
         self.x, self.y, self.time, self.duration)


class IViewPoint(Point):
    """ A point from the iView system. """
    interp_attrs = ('x', 'y', 'pupil_h', 'pupil_v', 'corneal_reflex_h', 'corneal_reflex_v',
                    'diam_h', 'diam_v')

    def __init__(self, x=None, y=None, time=None, duration=1 / 60.0, set='', pupil_h=0, pupil_v=0, corneal_reflex_h=0, corneal_reflex_v=0, diam_h=0, diam_v=0):
...
class PointPath(object):
    ...
    def valid_points(self, criterion):
        return PointPath([ point for point in self.points if criterion(point) ])
    ...
    def constrain_to(self, min_x_const=(0, 0), min_y_const=(0, 0), max_x_const=(1000, 1000), max_y_const=(1000, 1000)):
        plist = copy.deepcopy(self)
        for p in plist:
            if p.x < min_x_const[0]:
                p.x = min_x_const[1]
            if p.x > max_x_const[0]:
                p.x = max_x_const[1]
            if p.y < min_y_const[0]:
                p.y = min_y_const[1]
            if p.y > max_y_const[0]:
                p.y = max_y_const[1]
    ...
    def as_array(self, measures=None, dtype=np.float32):
        """ Turns our list of points into a numpy ndarray. """
        if measures is None:
            measures = self.measures
        return np.array([ [ getattr(point, m) for m in measures ] for point in self.points
                        ], dtype=dtype)
...
class IViewPointPath(UniformelySampledPointPath):
    ...
    def __init__(self, *args, **kwargs):
        self.measures = ('x', 'y', 'time', 'duration', 'pupil_h', 'pupil_v', 'corneal_reflex_h',
                         'corneal_reflex_v', 'diam_h', 'diam_v')
...
class IView3Pointpath(IViewPointPath):
    ...
    def __init__(self, *args, **kwargs):
        self.measures = ('x', 'y', 'time', 'timestamp', 'duration', 'pupil_h', 'pupil_v',
                         'corneal_reflex_1_h', 'corneal_reflex_1_v', 'corneal_reflex_2_h',
                         'corneal_reflex_2_v', 'diam_h', 'diam_v')
...
class IView2PointFactory(PointFactory):
    ...
    def from_component_list(self, components):
        return super(IView2PointFactory, self).from_component_list(components, self.data_map)
...
class IViewFixationFactory(PointFactory):
    ...
    def from_component_list(self, components):
        return super(IViewFixationFactory, self).from_component_list(components, self.data_map)