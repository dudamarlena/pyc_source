...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/gwc/gwc_scripts.py
# Compiled at: 2019-08-14 03:33:51
from __future__ import print_function
import argparse, json, os, platform, pydoc, re, sys, time, urlparse, gddriver, gddriver.models as models, gdpy, requests, yaml
...
def read_configuration(configfile):
    ...
    if not read_ok:
        ...
        print('Using: "gwc config" or "gwc config -e [endpoint] -i [access_id] -k [access_key]".', file=sys.stderr)
        sys.exit(1)
    ...
    if args.ls:
        ...
        try:
            ...
            if switch_choice == '':
                ...
                if switch_choice in ENDPOINT_DICT.keys():
                ...
                while switch_choice == '' or switch_choice not in ('Y', 'YES', 'N',
                                                                   'NO'):
                    switch_choice = input('Please confirm the configuration is correctly?[ Y/N ]:').upper()
            ...
            if switch_choice in ('YES', 'Y'):
                ...
                print(('It is saved successfully!\nAccount {}\nUser {}\nEndpoint {}').format(account_name, user_name, endpoint), file=sys.stdout)
            else:
                sys.exit(1)
        ...
        with open(globals.CONFIGFILE, 'w') as (f):
            config.write(f)
        _logger.info(('Saving the configuration to the file "{}" successfully.').format(globals.CONFIGFILE))
        print(('It is saved successfully!\nAccount {}\nUser {}\nEndpoint {}').format(account_name, user_name, args.endpoint), file=sys.stdout)
...
def get_version(operation, name, res_account, project_name=PROJECTNAME, tool_id=None):
    ...
    if operation == 'workflow':
        ...
        for item in gdpy.utils.json_loader(resp.response.text).get('tools'):
            ...
            elif operation_version > version:
                version = operation_version
...
def get_job_integral_log(log_downloadurl, job_id, out_put_file=None):
    ...
    if log_downloadurl.startswith('http'):
        ...
        connection = driver.create_connection_manager(store_host=res.hostname, store_port=res.port, credential=models.Credential(user=None, password=None), anonymous=True)
        request = models.FTPDownloadRequest(container_name='/', file_path=file_path, object_name=res.path)
        request.threads_count = 4
        request.checksum_type = 'md5'
        request.progress_callback = percentage
        res = driver.download_file(connection=connection, request=request)
        connection.close()
        print(res)
        return file_path
    return
...
def list_task(args):
    ...
    if args.todate:
        try:
            to_date = gdpy.utils.iso8601_to_unixtime(args.todate + 'T0:0:0.000Z') - 28800
    ...
    if args.fromdate:
        try:
            from_date = gdpy.utils.iso8601_to_unixtime(args.fromdate + 'T0:0:0.000Z') - 28800
        except ValueError as e:
...
def _get_tool(tool_name, version, output, isreturnid=False):
    ...
    if len(tools) > 0 and tools[0].get('config', {}).get('type') == 'system':
        print(("Permission denied to get app: {}, it's a SYSTEM APP ... pass").format(tool_config['tools'][0]['configs']['_id']))
        return
    else:
        ...
        if output:
            with open(output, 'w') as (f):
                f.write(tool_config)
            _logger.info('Get tool successfully! name: %s, version: %s. The output was saved to the file "%s"!', tool_name, tool_version, output)
            print(('Get tool: {} (version: {}) successfully! The output was saved to the file "{}"!').format(tool_name, tool_version, output))
            return True
...
def _list_tool(account, is_totimestamp=False, isreturn_tools_info=False):
    ...
    if account is None:
        tool_res_accounts = [
         account_name]
        resp = utility.catch_exceptions(retry_times=RETRY_TIMES)(authpolicy.list_user_authorized_policy)(endpoint, account_name, user_name, gd_auth)
    ...
    if tool_template and isreturn_tools_info:
        ...
        for tool in tool_template:
            for t in tool:
                tool_info = {'tool_version': t['tool_version'], 
...
def active_workflow(args):
    ...
    if args.version:
        ...
        try:
            resp = utility.catch_exceptions(retry_times=RETRY_TIMES)(task.active_workflow)(task_name + '_tmp.yml', workflow_name, workflow_version, workflow_owner)
        except gdpy.exceptions.ServerError as e:
            if e.status // 100 == 4:
                print('Please check the input "workflow name", "workflow version", "the inputs data name in parameters file" or "resource account" (if authoried by others).', file=sys.stderr)
                sys.exit(1)

    if resp.status == 200:
        _logger.info(('Run the workflow successfully! name: {}, version: {}. task_name: {}, task_id: {}').format(workflow_name, workflow_version, resp.task_name, resp.task_id))
        print(('Run the workflow successfully!\ntask_name: {}, task_id: {}').format(resp.task_name, resp.task_id), file=sys.stdout)
    else:
        _logger.info(('Run the workflow successfully! name: {}, version: {}. split: {}, splitting: {}').format(workflow_name, workflow_version, resp.split, resp.splitting))
        print('Run the workflow successfully!\nFastq file is spliting！', file=sys.stdout)
...
def _get_workflow(workflow_name, version, output):
    ...
    if output:
        ...
        _logger.info(('Get workflow successfully! name: {}, version: {}. The output was saved to the file "{}"!').format(workflow_name, workflow_version, output))
    ...
    return (
     workflow_config_dic['workflow']['nodelist'], account_name)
...
def _get_workflow_param(workflow_name, account, output, version):
    ...
    if version:
        ...
        try:
            ...
            if e.status // 100 == 4:
                print('Please check the input "workflow name" or "resource account" (if authoried by others).', file=sys.stderr)
                sys.exit(1)

        workflow = gdpy.Workflows(gd_auth, endpoint, workflow_owner, PROJECTNAME)
        try:
            resp = utility.catch_exceptions(retry_times=RETRY_TIMES)(workflow.get_exc_workflow)(workflow_name, workflow_version)
        except gdpy.exceptions.ServerError as e:
            if e.status // 100 == 4:
                print('Please check the input "workflow name", "workflow version" or "resource account" (if authoried by others).', file=sys.stderr)
                sys.exit(1)
    ...
    if output:
        with open(output, 'w') as (f):
            f.write(workflow_param.decode('utf-8'))
        _logger.info('Get workflow parameters successfully! name: %s, version: %s. The output was saved to the file %s!', workflow_name, workflow_version, output)
        print(('Get workflow parameters successfully! The output was saved to the file "{}"!').format(output))
    else:
        _logger.info('Get workflow parameters successfully! name: %s, version: %s.', workflow_name, workflow_version)
...
def _set_workflow_param(workflow_name, configs, version):
    ...
    try:
        ...
        if e.status // 100 == 4:
            print('Please check "workflow name", "workflow version", "workflow status"(need checked, gwc workflow ls | grep <workflow name>) or the content of the input "config file".', file=sys.stderr)
            sys.exit(1)

    _logger.info(('Setting workflow parameters successfully! name: {}, version: {}.').format(workflow_name, version))
    print(('Setting workflow parameters successfully! name: {}, version: {}.').format(workflow_name, version), file=sys.stdout)
...
def _get_account_and_public_tool_name_and_version_and_id(account_name, object_name):
    u"""获取指定 account 和 public 用户的工具名字/版本/id
    :param account_name:
    ...
    tools = {'public': tools_public, 
       account_name: tools_account_name}
    if object_name == 'id':
        dict_appid2name = {}
        for account, tools in tools.iteritems():
            for onetool in tools:
                dict_appid2name[onetool['tool_id']] = ('/{0}/{1}/{2}').format(account, onetool['tool_name'], onetool['tool_version'])

        return dict_appid2name
    if object_name == 'name_version':
        dict_name2appid = {}
        for account, tools in tools.iteritems():
            for onetool in tools:
                dict_name2appid[('/{0}/{1}/{2}').format(account, onetool['tool_name'], onetool['tool_version'])] = onetool['tool_id']
...
def push_all_conf(args):
    ...
    if not os.path.exists(args.dir + '/Workflow') or not os.path.exists(args.dir + '/APP'):
        ...
        if len(os.listdir(args.dir + '/Workflow')) == 0:
        ...
        for item in os.listdir(args.dir + '/APP'):
            ...
            with open(tool_path, 'r') as (f):
                ...
                print(('\nStart to push apps: {} (verison: {})').format(app_info['app']['name'], app_info['app']['version']))
                ...
                if tool_id:
                    ...
                    print(('The app: {} (version: {}) already exists!').format(app_info['app']['name'], app_info['app']['version']))
                else:
                    _create_tool(app_info['app']['name'], tool_path, app_info['app']['version'])
                    tool_id = _get_tool(app_info['app']['name'], app_info['app']['version'], None, isreturnid=True)
                    app_id_list.append((re.sub('_app.yml', '', item), tool_id))
                    print(('Push app: {} (verision: {}) successfully!').format(app_info['app']['name'], app_info['app']['version']))
            f.close()
    ...
    with open(old_workflow_path, 'r') as (old_f):
        with open(new_workflow_path, 'w') as (new_f):
    ...
    with open(new_workflow_path, 'r') as (f):
        ...
        _create_workflow(workflow_info['workflow']['name'], new_workflow_path, workflow_info['workflow']['version'])
    ...
    _set_workflow_param(workflow_info['workflow']['name'], workflow_para_path, workflow_info['workflow']['version'])
...
def pull_all_conf(args):
    ...
    for node in nodelists:
        ...
        if node['name'] in SYSTERMTOOL:
            print(("Permission denied to get app: {} ({}), it's a SYSTEM APP... pass").format(node['name'], node['app_id']))
            continue
        try:
            tool_version = get_version('tool', node['name'], account_name, project_name=PROJECTNAME, tool_id=node['app_id'])
        except gdpy.exceptions.ServerError as e:
            if e.status // 100 == 4:
                print(("Permission denied to get app: {} ({}), it's a PUBLIC APP... pass").format(node['name'], node['app_id']))
    ...
    if len(public_tools) != 0:
        ...
        for tool in all_public_tools:
            if tool['tool_id'] in public_tools_id:
                tool_name = ('{name}_{version}').format(name=tool['tool_name'], version=tool['tool_version'])
...
def main():
    ...
    subparsers_workflow_appid2name = subparser_workflow.add_parser('appid2name', help='Change the app id of workflow yml file to /<account>/<tool_name>/<tool_version>', prog='gdtools workflow appid2name', formatter_class=argparse.RawTextHelpFormatter)
    subparsers_workflow_appid2name.add_argument('-f', '--file', help='Specify the parameter file of Workflow', required=True)
    subparsers_workflow_appid2name.set_defaults(func=workflow_appid_to_appname)
    subparsers_workflow_name2appid = subparser_workflow.add_parser('name2appid', help='Change /<account>/<tool_name>/<tool_version> of workflow yml file to app id', prog='gdtools workflow name2appid', formatter_class=argparse.RawTextHelpFormatter)
    subparsers_workflow_name2appid.add_argument('-f', '--file', help='Specify the parameter file of Workflow', required=True)
    ...
    parsers_pull = subparser.add_parser('pull', help='Pull All Workflow Configuration File into a Local Dictionary')
