...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-intel/egg/gwc/utility.py
# Compiled at: 2019-08-14 03:31:02
from __future__ import print_function
import functools, random, time, os, sys, gdpy
...
def retry_exception(exc, func):
    ...
    if isinstance(exc, gdpy.exceptions.RequestError):
        _logger.warning('Failed to %s! status: %s, error: %s\nNetWork Connection Error, please check your network and retry.', func.__name__, exc.status, exc.error_message.decode('utf-8'))
        print(('Failed to {}!\nstatus: {}\nerror: {}\n').format(func.__name__, exc.status, exc.error_message.decode('utf-8')), file=sys.stderr)
...
def no_retry_exception(exc, func):
    ...
    if isinstance(exc, ConfigParser.NoSectionError):
        _logger.warning(('Can not find section: "{}" in config file: "{}".').format(exc.section, globals.CONFIGFILE))
        ...
        _logger.warning(('Can not find option: "{}" in section: "{}" in config file: "{}".').format(exc.option, exc.section, globals.CONFIGFILE))
        print('Please config your account information.', file=sys.stderr)
        print('Using: "gwc config" or "gwc config -e [endpoint] -i [access_id] -k [access_key]".', file=sys.stderr)
        sys.exit(-1)
        ...
        _logger.warning('Failed to %s! status: %s, error: %s, 错误: %s', func.__name__, exc.status, exc.error_message.decode('utf-8'), exc.error_message_chs.decode('utf-8'))
        print(('Failed to {}!\nstatus: {}\nerror: {}\n错误: {}').format(func.__name__, exc.status, exc.error_message.decode('utf-8'), exc.error_message_chs.decode('utf-8')), file=sys.stderr)
        os._exit(-1)