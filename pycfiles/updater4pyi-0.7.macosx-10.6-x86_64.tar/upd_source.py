...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/updater4pyi/upd_source.py
# Compiled at: 2014-12-07 12:16:31
"""
...
"""
import sys, re, os, os.path, logging, copy, json, inspect, urllib2
from . import util
...
class BinReleaseInfo(object):
    ...
    def __init__(self, version=None, filename=None, url=None, reltype=RELTYPE_UNKNOWN, platform=None, **kwargs):
        """
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(' + (', ').join([ '%s=%r' % (k, v) for k, v in self.__dict__.iteritems() ]) + ')'
...
def relpattern(re_pattern, reltype=RELTYPE_UNKNOWN, platform=None, **kwargs):
    ...
    return (
     re_pattern,
     lambda m, filename, url, version=None, _fix_plat=platform, _fix_rtyp=reltype, _fix_kwargs=copy.deepcopy(kwargs), **innerkwargs: _make_bin_release_info(m, [
      (
       'version', version)] + _fix_kwargs.items() + [
      (
       'filename', filename),
      (
       'url', url),
...
def _guess_plat(m, d, default=None):
    ...
    if relfile_label is None or not len(relfile_label):
        if default is not None:
            return default
        return IgnoreArgument
    if re.search('mac\\s*os\\s*x', relfile_label, re.I):
        return 'macosx'
    else:
        if re.search('linux', relfile_label, re.I):
            return 'linux'
        if re.search('windows', relfile_label, re.I):
            return 'win'
        if default is not None:
            return default
        return IgnoreArgument
...
_RX_VER = '-(?P<version>\\d+[\\w.]+)'
_RX_VER_OPT = '(' + _RX_VER + ')?'
_RX_PLAT = '-(?P<platform>macosx|linux|win)'
_RX_PLAT_OPT = '(' + _RX_PLAT + ')?'
_default_naming_strategy_patterns = (
 relpattern(_RX_VER_OPT + _RX_PLAT_OPT + '(?P<onedir>-(onedir|dir|dist))?\\.(tar(\\.gz|\\.bz(ip)?2?|\\.Z)|tgz|tbz2?|zip)$', version=lambda m: m.group('version') if m.group('version') else IgnoreArgument, platform=_guess_plat, reltype=lambda m, d, x: _guess_reltype(m, d, x, default=RELTYPE_ARCHIVE)),
 relpattern(_RX_VER_OPT + _RX_PLAT_OPT + '\\.exe$', version=lambda m: m.group('version') if m.group('version') else IgnoreArgument, platform=lambda m, d: _guess_plat(m, d, default='win'), reltype=RELTYPE_EXE),
 relpattern(_RX_VER_OPT + _RX_PLAT_OPT + '(\\.(bin|run))?$', version=lambda m: m.group('version') if m.group('version') else IgnoreArgument, platform=lambda m, d: _guess_plat(m, d, default='linux'), reltype=RELTYPE_EXE))
...
class UpdateSourceDevelopmentReleasesFilter(object):
    ...
    def __init__(self, include_devel_releases=False, regexname=None):
        ...
        if self.regexname is None:
            self.regexname = re.compile('(beta|alpha|rc)', re.IGNORECASE)
        return
...
class UpdateLocalDirectorySource(UpdateSource):
    ...
    def get_releases(self, newer_than_version=None, **kwargs):
        try:
            versiondirs = sorted([ vdir for vdir in os.listdir(self.source_directory) if os.path.isdir(os.path.join(self.source_directory, vdir))
                                 ], key=util.parse_version, reverse=True)
        except OSError:
        ...
        for ver in versiondirs:
            ...
            if newer_than_version_parsed is not None and util.parse_version(ver) <= newer_than_version_parsed:
                break
            ...
            try:
                for fn in os.listdir(base):
                    ...
                    inf = self.naming_strategy.get_release_info(filename=fn, url=fnurl, version=ver)
        ...
        logger.debug('Found releases:\n' + ('\n').join([ '\t* %s, %s (%r)' % (r.get_filename(), r.get_version(), r.__dict__) for r in inf_list
                                                       ]))
        return inf_list
    ...
    def get_releases(self, newer_than_version=None, **kwargs):
        ...
        if isinstance(data, dict):
            ...
            for relinfo in data:
                ...
                if newer_than_version_parsed is not None and util.parse_version(relver) <= newer_than_version_parsed:
                ...
                for relfile in relfiles:
                    ...
                    inf = self.naming_strategy.get_release_info(filename=relfn, url=relurl, version=relver, rel_name=rel_name, relfile_label=rellabel, rel_description=rel_desc, relfile_content_type=relcontenttype, rel_tag_name=tag_name, rel_html_url=html_url)
