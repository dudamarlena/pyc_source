...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/lindypy/TupleSpace.py
...
"""
import os, sys, select, signal, traceback, multiprocessing

class MessageException(Exception):
    pass


class MessageNotUnderstoodError(MessageException):
    pass
...
class Client(object):
    ...
    def __init__(self, manager=None, pipe=None):
        ...
        if manager is None and pipe is None:
            ...
            self.manager = multiprocessing.Process(target=Manager, args=(
             self.__class__, os.getpid(), managerpipe))
            self.manager.start()
            self.ismain = True
