...
# [GCC 8.4.0]
# Embedded file name: /opt/devel/durian/durian/event.py
# Compiled at: 2009-09-12 10:38:59
...
class Hook(object):
    ...
    def __init__(self, name=None, task_cls=None, timeout=None, async=None, retry=None, max_retries=None, fail_silently=False, config_form=None, provides_args=None, match_forms=None, **kwargs):
        ...
        self.match_forms = match_forms or self.match_forms or create_match_forms(form_name, self.provides_args)
        return
    ...
    def get_match_forms(self, **kwargs):
        ...
        """
        return [ match_form(**kwargs) for match_form in self.match_forms.values()
               ]
    ...
    def get_listeners(self, sender, payload):
        ...
        return [ target for target in possible_targets if self.event_filter(sender, payload, target.match)
               ]
    ...
    def add_listener_by_form(self, form, match=None):
        ...
        url = config.pop('url')
        return Listener.objects.create(hook=self.name, url=url, match=match, config=config)

    def add_listener(self, url, match={}, **config):
        r"""Add listener for this signal.
        ...
        """
        return Listener.objects.create(hook=self, url=url, match=match, **dict(config))
...
class SignalHook(Hook):
    ...
    def connect(self, sender):
        self.signal.connect(self.send, sender=sender, dispatch_uid=self.__class__._dispatch_uid)

    def disconnect(self, sender):
        self.signal.disconnect(self.send, sender=sender, dispatch_uid=self.__class__._dispatch_uid)
...
class ModelHook(SignalHook):
    ...
    def get_model_default_fields(self):
        return [ field.name for field in self.model._meta.fields if field.name != self.model._meta.pk.name
               ]

    def prepare_payload(self, sender, payload):
        instance = payload.pop('instance')
        payload.pop('signal', None)
        model_data = dict((field_name, getattr(instance, field_name, None)) for field_name in self.provides_args)
