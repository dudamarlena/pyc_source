...
# [GCC 8.4.0]
# Embedded file name: /opt/devel/durian/durian/tests/test_hooks.py
...
testhook = Hook(name='__durian__.unittest.testhook', provides_args=[
 'name', 'address', 'phone', 'email'], task_cls=TestWebhookSignal, async=False)
hooks.register(testhook)
testmhook = ModelHook(model=User, signal=signals.post_save, name='__durian__.unittest.testmhook', provides_args=[
 'username', 'email', 'first_name',
 'last_name'], task_cls=TestWebhookSignal, async=False)
...
class TestHook(unittest.TestCase):
    ...
    def test_match_forms(self):
        ...
        matchdict = testhook.apply_match_forms({'name_cond': match.CONDITION_EXACT, 
        ...
        self.assertTrue(match.deepmatch(matchdict, {'name': 'George Constanza', 
    ...
    def test_trigger_event(self):
        ...
        testhook.send(sender=self, name='Joe', address='foo', phone='123', email='joe@example.com')
        self.assertTrue(TestWebhookSignal.scratchpad.get(url))
        del TestWebhookSignal.scratchpad[url]
        testhook.send(sender=self, name='Simon', address='bar', phone='456', email='simon@example.com')
...
class TestModelHook(unittest.TestCase):
    ...
    def test_trigger_event(self):
        ...
        self.assertTrue(lis.count())
        self.assertTrue(lis[0].url == url)
        u = User.objects.create_user(username='joe', email='joe@example.com', password='joe')
