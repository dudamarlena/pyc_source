...
# [GCC 8.4.0]
# Embedded file name: /opt/devel/durian/durian/forms.py
# Compiled at: 2009-09-12 10:47:16
...
class HookConfigForm(forms.Form):
    ...
    """
    url = forms.URLField(label=_('Listener URL'), required=True)
...
def create_match_forms(name, provides_args):
    ...
    def gen_field_for_name(name):
        return {'%s_cond' % name: forms.ChoiceField(label=name, choices=MATCHABLE_CHOICES, widget=forms.Select()), 
           '%s_query' % name: forms.CharField(label='', required=False, initial='')}
    ...
    return dict((field, gen_form_for_field(field)) for field in provides_args)