...
# [GCC 8.4.0]
# Embedded file name: /opt/devel/durian/durian/tests/test_match.py
...
class TestMtuplelist(unittest.TestCase):
    ...
    def test_conversion(self):
        mtuplelist = [
         (
          'name', match.CONDITION_ENDSWITH, 'Constanza'),
         (
        ...
        matches = {'name': 'George Constanza', 'zipcode': '70312', 
           'address': 'Milkyway', 
           'work': 'Vandeley Industries'}
        notmatches = {'name': 'Jerry Seinfeld', 'zipcode': '70314', 
           'address': 'Milkyway', 
           'work': 'Comedian'}
...
class TestStrategy(unittest.TestCase):
    ...
    def test_deepmatch(self):
        self.assertTrue(deepmatch({'name': Like('Constanza')}, {'name': 'George Constanza'}))
        self.assertFalse(deepmatch({'name': Like('Seinfeld')}, {'name': 'George Constanza'}))
        self.assertTrue(deepmatch({'name': Startswith('George')}, {'name': 'George Constanza'}))
        self.assertFalse(deepmatch({'name': Startswith('Cosmo')}, {'name': 'George Constanza'}))
        self.assertTrue(deepmatch({'name': {'first_name': Startswith('El'), 
                    'last_name': Endswith('es')}}, {'name': {'first_name': 'Elaine', 
                    'last_name': 'Benes'}}))
        x = {'foo': 'xuzzy', 
           'baz': 'xxx', 
           'mooze': {'a': 'b', 
        ...
        self.assertFalse(deepmatch(x, {'foo': 'xuzzy', 'baz': 'xxx', 
           'mooze': {'a': 'b', 
                     'c': {'x': 'y'}}}))
        self.assertTrue(deepmatch({'foo': 'bar'}, {'foo': 'bar'}))
        self.assertFalse(deepmatch({'foo': 1}, {'foo': '1'}))
        self.assertFalse(deepmatch({'foo': 'bar', 'baz': {'x': 'x'}}, {'foo': 'bar'}))