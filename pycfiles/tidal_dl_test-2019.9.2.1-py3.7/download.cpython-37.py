...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tidal_dl\download.py
# Compiled at: 2019-09-04 12:56:17
# Size of source mod 2**32: 25185 bytes
"""
@File    :   download.py
@Time    :   2019/02/27
@Author  :   Yaron Huang 
...
class Download(object):
    ...
    def __thradfunc_dl(self, paraList):
        ...
        if 'redownload' in paraList:
            ...
            if redownload is False:
                needDl = self._Download__isNeedDownload(paraList['path'], paraList['url'])
            showprogress = False
            if int(self.config.threadnum) <= 1:
                if self.showpro:
                    showprogress = True
                else:
    ...
    def downloadAlbum(self, album_id=None):
        ...
        while while_count > 0:
            ...
            targetDir = self._Download__creatAlbumDir(aAlbumInfo)
            ...
            for item in aAlbumTracks['items']:
                ...
                filePath = self._Download__getAlbumSongSavePath(targetDir, aAlbumInfo, item, fileType)
                paraList = {'album':aAlbumInfo,  'redownload':redownload, 
                 'title':item['title'], 
                 'trackinfo':item, 
                 'url':streamInfo['url'], 
                 'path':filePath, 
                 ...
                 'coverpath':coverPath}
                self.thread.start(self._Download__thradfunc_dl, paraList)
            ...
            for item in aAlbumVideos:
                ...
                selectIndex = self._Download__getVideoResolutionIndex(resolutionList)
    ...
    def downloadTrack(self, track_id=None):
        ...
        while while_count > 0:
            ...
            if self.tool.errmsg != '':
                ...
                continue
            fileType = self._getSongExtension(streamInfo['url'])
            filePath = self._Download__getAlbumSongSavePath(targetDir, aAlbumInfo, aTrackInfo, fileType)
        ...
        while flag:
            ...
            if video_id is None:
                ...
                if self.tool.errmsg != '':
                    ...
                    continue
                index = self._Download__getVideoResolutionIndex(resolutionList)
                path = targetDir + '/' + pathHelper.replaceLimitChar(aVideoInfo['title'], '-') + '.mp4'
                path = os.path.abspath(path)
                if os.access(path, 0):
                    os.remove(path)
                if self.ffmpeg.mergerByM3u8_Multithreading(urlList[int(index)], path, True, True):
            ...
            while bBreakFlag is False:
                ...
                for item in aItemInfo:
                    ...
                    if bFirstTime is False:
                        ...
                        os.path.isfile(filePath) or self.thread.start(self._Download__thradfunc_dl, paraList)
