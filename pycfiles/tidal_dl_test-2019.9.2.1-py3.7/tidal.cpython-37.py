...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\tidal_dl\tidal.py
# Compiled at: 2019-09-04 12:41:15
# Size of source mod 2**32: 19702 bytes
"""
@File    :   tidal.py
@Time    :   2019/02/27
@Author  :   Yaron Huang 
...
@Desc    :   Tidal API
"""
import os, re, uuid, requests, json
...
class TidalTool(object):
    ...
    def covertMp4toM4a(self, file_path):
        if self.config.onlym4a != 'True':
            ...
            if '.mp4' not in file_path:
                return file_path
            return self.ffmpeg.enable or file_path
    ...
    def getPlaylist(self, playlist_id):
        ...
        list = self._TidalTool__getItemsList('playlists/' + playlist_id + '/items')
    ...
    def getAlbumVideos(self, album_id):
        info = self._TidalTool__getItemsList('albums/' + str(album_id) + '/items')
        if self.errmsg != '':
            return []
        ret = []
        for item in info:
    ...
    def getFavorite(self, user_id):
        trackList = self._TidalTool__getItemsList('users/' + str(user_id) + '/favorites/tracks')
        videoList = self._TidalTool__getItemsList('users/' + str(user_id) + '/favorites/videos')
        return (trackList, videoList)

    def getArtistAlbum(self, artist_id):
        items1 = self._TidalTool__getItemsList('artists/' + str(artist_id) + '/albums', {'filter': 'EPSANDSINGLES'})
        items3 = self._TidalTool__getItemsList('artists/' + str(artist_id) + '/albums')
    ...
    def getVideoResolutionList(self, video_id):
        ...
        resolutionList, urlList = self._TidalTool__parseVideoMasterAll(str(content))
        return (resolutionList, urlList)

    def getVideoMediaPlaylist(self, url):
        urlList = self._TidalTool__parseVideoMediaPlaylist(url)
        return urlList
    ...
    def parseLink(self, link):
        ...
        if link.find('http') < 0:
            return (None, None)
        stype = re.findall('tidal.com/(.+?)/', link)
        if len(stype) <= 0 or stype[0] not in TYPE_ARR:
            return (None, None)
        sid = re.findall('tidal.com/' + stype[0] + '/(.+)/', link)
        if len(sid) <= 0:
            sid = re.findall('tidal.com/' + stype[0] + '/(.+)', link)
        if len(sid) <= 0:
            return (None, None)
...
class TidalAccount(object):
    ...
    def getSessionID(self, password):
        postParams = {'username':self.username, 
         'password':password, 
         'token':self.token, 
