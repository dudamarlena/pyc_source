...
# [GCC 8.4.0]
# Embedded file name: /Users/tonra/ohm2/Clients/ohm2/Entwicklung/ohm2-dev/application/website/apps/ohm2_handlers/countries/apisdispatcher.py
# Compiled at: 2016-12-27 16:48:37
...
import simplejson as json, os, time, random, datetime, mimetypes
...
def flag_code(params):
    p = h_utils.cleaned(params, (('request', 'request', ''), ('string', 'version', 1),
                                 ('string', 'size', 2), ('string', 'filename', 2)))
...
def get_country(params):
    ...
    if country is None:
        return {'error': CountriesRunException(**country_errors.NO_COUNTRY_FOUND).regroup()}
...
def create_user_address(params):
    p = h_utils.cleaned(params, (('request', 'request', ''), ('string', 'version', 1),
                                 ('string', 'region', 0), ('string', 'commune', 0),
                                 ('string', 'street', 1), ('string', 'number', 1),
                                 ('string', 'apartment', 0), ('string', 'extra_information', 0),
                                 ('string', 'latitude', 0), ('string', 'longitude', 0),
                                 ('string', 'next', 0)))
    request = p['request']
    user = User.objects.get(username=request.user.get_username())
    kwargs = {'apartment': p['apartment'], 
