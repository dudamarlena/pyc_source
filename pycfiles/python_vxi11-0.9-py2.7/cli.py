...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vxi11/cli.py
# Compiled at: 2017-01-18 12:58:14
import time, sys, logging, readline
...
LOCAL_COMMANDS = {'%SLEEP': (
            1, 1, lambda a: time.sleep(float(a[0]) / 1000))}
...
def main():
    ...
    parser.add_option('-d', action='store_true', dest='debug', help='enable debug messages')
    ...
    parser.add_option('--always-check-esr', action='store_true', dest='check_esr', help='Check the error status register after every command')
