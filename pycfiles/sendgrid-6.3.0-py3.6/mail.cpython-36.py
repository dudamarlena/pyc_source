...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sendgrid/helpers/mail/mail.py
# Compiled at: 2020-04-29 15:57:11
# Size of source mod 2**32: 33230 bytes
...
class Mail(object):
    __doc__ = 'Creates the response body for v3/mail/send'

    def __init__(self, from_email=None, to_emails=None, subject=None, plain_text_content=None, html_content=None, global_substitutions=None, is_multiple=False):
        """
    ...
    def add_personalization(self, personalization, index=0):
        ...
        :type index: int
        """
        self._personalizations = self._ensure_append(personalization, self._personalizations, index)
    ...
    def add_cc(self, cc_email, global_substitutions=None, is_multiple=False, p=0):
        ...
        self._set_emails(cc_email,
          global_substitutions, is_multiple=is_multiple, p=p)
    ...
    def add_substitution(self, substitution):
        ...
        if substitution.personalization:
            ...
            if not has_internal_personalization:
                self.add_personalization(personalization,
                  index=(substitution.personalization))
        else:
            if isinstance(substitution, list):
                for s in substitution:
                    for p in self.personalizations:
    ...
    def add_custom_arg(self, custom_arg):
        ...
        if custom_arg.personalization is not None:
            ...
            if not has_internal_personalization:
                self.add_personalization(personalization,
                  index=(custom_arg.personalization))
        else:
            if isinstance(custom_arg, dict):
                k, v = list(custom_arg.items())[0]
        ...
        if isinstance(content, str):
            ...
            if content.mime_type == 'text/plain':
                ...
                if self._contents:
                    ...
                    index = 0
                self._contents = self._ensure_append(content,
                  (self._contents), index=index)
    ...
    def get(self):
        ...
        :rtype: dict
        """
        mail = {'from':self._get_or_none(self.from_email), 
        ...
        return {key:value for key, value in mail.items() if value is not None if value != [] if value != {} if value != {}}
    ...
    def from_EmailMessage(cls, message):
        ...
        :rtype: Mail
        """
        mail = cls(from_email=(Email(message.get('From'))),
