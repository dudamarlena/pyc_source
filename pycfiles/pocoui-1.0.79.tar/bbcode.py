...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/bbcode.py
...
class TokenList(list):
    ...
    def flatten(self):
        return ('').join((token.raw for token in self))
...
class NodeList(list):
    ...
    def render(self, ctx):
        return ('').join((node.render(ctx) for node in self))
    ...
    def get_editor_options(self, req, signature):
        ...
        for comp in self.ctx.get_components(BBCodeTagProvider):
            for tagname in comp.tags:
                if signature and signature_tags is not None and tagname not in signature_tags:
                    continue
                btn = comp.get_button(req, tagname)
                if btn is not None:
                    buttons.append(btn)

        return {'buttons': buttons, 'smilies': get_smiley_buttons(req.ctx)}


class BasicBBCodeTagProvider(BBCodeTagProvider):
    __module__ = __name__
    tags = ['b', 'i', 'u', 's', 'url', 'email', 'color', 'size', 'code', 'quote', 'list']

    def get_node(self, token, parser):
        ctx = self.ctx
        if token.name == 'b':
            nodelist = parser.parse('/b')
            return '<strong>%s</strong>' % nodelist.render(ctx)
        if token.name == 'i':
            nodelist = parser.parse('/i')
            return '<em>%s</em>' % nodelist.render(ctx)
        if token.name == 'u':
            nodelist = parser.parse('/u')
            return '<ins>%s</ins>' % nodelist.render(ctx)
        if token.name == 's':
            nodelist = parser.parse('/s')
            return '<del>%s</del>' % nodelist.render(ctx)
        if token.name == 'url':
            if token.attr:
            ...
            if url.startswith('javascript:'):
                url = url[11:]
            return '<a href="%s" rel="nofollow">%s</a>' % (url, content)
        if token.name == 'email':
            if token.attr:
            ...
            return '<a href="mailto:%s">%s</a>' % (mail, content)
        if token.name == 'color':
            ...
            try:
                ...
                return '<span style="color: %s">%s</span>' % (color, content)
        if token.name == 'size':
            nodelist = parser.parse('/size')
            content = nodelist.render(ctx)
            if not token.attr or not token.attr.isdigit() or len(token.attr) > 2:
                return token.raw
            return '<span style="font-size: %spx">%s</span>' % (token.attr, content)
        if token.name == 'img':
        ...
        if token.name == 'code':
            ...
            return '<pre>%s</pre>' % tokenlist.flatten()
        if token.name == 'quote':
            nodelist = parser.parse('/quote')
            if token.attr:
                _ = frozen_translation(token.attr)
                preamble = '<div class="written_by">%s</div>' % _('%s has written:')
            else:
                preamble = ''
            return '<blockquote>%s%s</blockquote>' % (preamble, nodelist.render(ctx))
        if token.name == 'list':
            nodelist = parser.parse('/list')
            lines = []
            for line in re.split('^\\s*\\[\\*\\](?m)', '\n%s\n' % nodelist.render(ctx)):
                line = line.strip()
    ...
    def get_button(self, req, tagname):
        ...
        if tagname == 'b':
            ...
            labels = [_('Black'), _('Blue'), _('Brown'), _('Cyan'), _('Gray'), _('Green'), _('Magenta'), _('Purple'), _('Red'), _('White'), _('Yellow')]
            values = [
             'black', 'blue', 'brown', 'cyan', 'gray', 'green', 'magenta', 'purple', 'red', 'white', 'yellow']
        elif tagname == 's':
        ...
        return {'name': str(name), 'description': str(description), 'icon': icon and str(make_url(iconurl % icon)), 'insert': str(insert), 'values': values, 'labels': labels}