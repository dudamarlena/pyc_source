...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/user.py
...
from pocoo.utils.text import gen_password, gen_activation_key, gen_pwhash, check_pwhash
...
def get_user_list(ctx, order_by=users.c.username, order='asc', letter=None, hide_internal=False):
    ...
    try:
        order = {'desc': meta.desc, 'asc': meta.asc}[order]
    except KeyError:
        raise ValueError("order must be either 'asc' or 'desc'")

    result = ctx.engine.execute(meta.select([u.user_id, u.username, u.email, u.profile, u.last_login, u.register_date, u.post_count], meta.and_(*q), order_by=[order(order_by)]))
...
def get_group_list(ctx, order_by=groups.c.name, order='asc', letter=None, show_hidden=True):
    ...
    result = ctx.engine.execute(meta.select([g.group_id, g.name, g.public, g.hidden], meta.and_(*q), order_by=[order(order_by)]))
...
def get_user(req, user_id_or_name):
    ...
    result = ctx.engine.execute(meta.select([u.user_id, u.username, u.email, u.profile, u.settings, u.last_login, u.register_date, u.post_count], q))
    row = result.fetchone()
    if row is None:
        return
    user = dict(row)
...
def get_id_by_name(ctx, name):
    u = users.c
    result = ctx.engine.execute(meta.select([u.user_id], u.username == name))
    row = result.fetchone()
    if not row:
        raise UserNotFound
    return row[0]
...
def check_login_data(ctx, username, password):
    ...
    """
    u = users.c
    result = ctx.engine.execute(meta.select([u.user_id, u.pwhash, u.act_key], users.c.username == username))
    row = result.fetchone()
    if row is not None and not row['act_key'] and check_pwhash(row['pwhash'], password):
        return row['user_id']
    return
...
def sync_post_count(ctx, user_id):
    ...
    def do(con):
        result = con.execute(meta.select([meta.func.count(posts.c.post_id)], posts.c.author_id == user_id))
        con.execute(users.update(users.c.user_id == user_id), post_count=result.fetchone()[0])
...
def reset_password(ctx, username, email):
    ...
    result = ctx.engine.execute(users.update((users.c.username == username) & (users.c.email == email)), pwhash=gen_pwhash(password))
    if result.rowcount:
        return password
...
class User(DatabaseModel, LinkableMixin):
    ...
    def create(ctx, username, password, email, activate=False):
        ...
        if activate:
            ...
            act_key = None
        result = ctx.engine.execute(users.insert(), username=username, email=email, act_key=act_key, pwhash=gen_pwhash(password), register_date=datetime.utcnow(), post_count=0)
    ...
    def iter_groups(self):
        ...
        """
        result = self.ctx.engine.execute(meta.select([groups.c.group_id], (users.c.user_id == self.user_id) & (group_members.c.group_id == groups.c.group_id) & (group_members.c.user_id == users.c.user_id)))
        for row in result:
    ...
    def __repr__(self):
        return '<%s %d: %r>' % (self.__class__.__name__, self.user_id, self.username)
...
class Group(DatabaseModel):
    ...
    def create(ctx, name, public=True, hidden=False):
        ...
        result = ctx.engine.execute(groups.insert(), name=name, public=public, hidden=hidden)
    ...
    def iter_members(self):
        ...
        result = self.ctx.engine.execute(meta.select([users.c.user_id], (groups.c.group_id == self.group_id) & (group_members.c.group_id == groups.c.group_id) & (group_members.c.user_id == users.c.user_id)))
        for row in result:
    ...
    def add_member(self, user):
        ...
        result = self.ctx.engine.execute(meta.select([group_members.c.user_id], (group_members.c.user_id == user_id) & (group_members.c.group_id == self.group_id)))
        if result.fetchone() is not None:
            raise ValueError('The user %d is alreay a member of this group' % user_id)
        self.ctx.engine.execute(group_members.insert(), user_id=user_id, group_id=self.group_id)
        return
    ...
    def remove_member(self, user):
        ...
        if result.fetchone() is None:
            raise ValueError('The user %d is not a member of this group' % user_id)
        self.ctx.engine.execute(group_members.delete((group_members.c.group_id == self.group_id) & (group_members.c.user_id == user_id)))
        return
    ...
    def __repr__(self):
        return '<%s %d: %r>' % (self.__class__.__name__, self.group_id, self.name)