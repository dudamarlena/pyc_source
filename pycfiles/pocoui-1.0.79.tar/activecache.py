...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/activecache.py
...
class CallbackNode(Node):
    ...
    def __repr__(self):
        return '%s(%r, %s)' % (self.__class__.__name__, self.callback, (', ').join((repr(x) for x in self.data)))
...
class NodeList(Node, list):
    ...
    def render(self, req, comp):
        return ('').join((node.render(req, comp) for node in self))
    ...
    def __repr__(self):
        ...
        for node in self:
            ...
            if type(node) is Node:
                ...
                if tmp:
                    newnodes.append(repr(Node(('').join(tmp))))
                    tmp[:] = ()
        ...
        if len(newnodes) == 1:
            return newnodes[0]
        return '%s(%s)' % (self.__class__.__name__, (', ').join(newnodes))
...
def get_cache_namespace():
    ...
    """
    return {'datetime': datetime, 'Node': Node, 'CallbackNode': CallbackNode, 'NodeList': NodeList}
...
def load_cache(cache):
    ...
    return (
     eval(code, get_cache_namespace()), syntax_parser)