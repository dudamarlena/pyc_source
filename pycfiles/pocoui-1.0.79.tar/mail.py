...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/mail.py
...
class Email(object):
    ...
    def __init__(self, ctx, subject=None, to_addrs=None, text=None):
        ...
        if not self.from_addr:
            ...
            self.from_addr = '%s <noreply@%s>' % (ctx.cfg.get('board', 'title'), p[1].split(':')[0])
        self.to_addrs = []
    ...
    def send(self):
        ...
        if self.ctx.cfg.get('email', 'user', ''):
            try:
                try:
                    smtp.login(self.ctx.cfg.get('email', 'user'), self.ctx.cfg.get('email', 'pass', ''))
