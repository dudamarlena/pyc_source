...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/tests.py
...
class InteractivePocoo(object):
    ...
    def __repr__(self):
        return '<%s %r>' % (self.__class__.__name__, self.ctx)
...
class BufferedServer(object):
    ...
    def start_response(self, url='/', params=None, query_string='', cookie='', remote_addr='127.0.0.1', request_method='POST', input_stream=None):
        ...
        if ':' in host:
            ...
            servername = host
            serverport = '80'
        environ = {'wsgi.input': input_stream, 'wsgi.errors': error_stream, 'wsgi.version': (1, 0), 'wsgi.multithread': False, 'wsgi.multiprocess': True, 'wsgi.run_once': False, 'wsgi.url_scheme': scheme, 'QUERY_STRING': query_string, 'SCRIPT_NAME': str(self.ctx.scriptname), 'PATH_INFO': url, 'HTTP_COOKIE': cookie, 'HTTP_HOST': host, 'SERVER_NAME': servername, 'SERVER_PORT': serverport, 'SERVER_ADDR': str(IP.by_hostname(servername)), 'REMOTE_ADDR': remote_addr, 'REQUEST_METHOD': request_method}
        stream = StringIO()
        _headers_set = []
        _headers_sent = []
...
class InteractiveController(object):
    ...
    def __repr__(self):
        return '<%s %r>' % (self.__class__.__name__, self.position)
...
class UnboundRequest(object):
    ...
    def __repr__(self):
        return '<%s %r%s>' % (self.__class__.__name__, self.controller.req.path, self.alive and ' *' or '')
