...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/utils/decorators.py
# Compiled at: 2006-12-26 17:18:07
__doc__ = '\n    pocoo.utils.decorators\n    ~~~~~~~~~~~~~~~~~~~~~~\n\n    Decorators and decorator utilities.\n\n    :copyright: 2006 by Armin Ronacher.\n    :license: GNU GPL, see LICENSE for more details.\n'
import sys, inspect, new, itertools, threading
...
class decorator(object):
    ...
    def _getinfo(self, func):
        ...
        fullsign = inspect.formatargspec(regargs, varargs, varkwargs, defaults, formatvalue=lambda value: '=defarg[%i]' % counter.next())[1:-1]
        shortsign = inspect.formatargspec(regargs, varargs, varkwargs, defaults, formatvalue=lambda value: '')[1:-1]
        dic = dict((('arg%s' % n, name) for (n, name) in enumerate(argnames)))
        dic.update(name=func.__name__, argnames=argnames, shortsign=shortsign, fullsign=fullsign, defarg=func.func_defaults or ())
        return dic
    ...
    def _decorate(self, func, caller):
        ...
        if '__call_' in infodict['argnames'] or '__func_' in infodict['argnames']:
        ...
        if func.__name__ == '<lambda>':
            lambda_src = 'lambda %(fullsign)s: __call_(__func_, %(shortsign)s)' % infodict
...
def copyfunc(func):
    ...
    """
    return new.function(func.func_code, func.func_globals, func.func_name, func.func_defaults, func.func_closure)
...
def tail_call_optimized(func, *args):
    ...
    """
    f = sys._getframe()
    if f.f_back and f.f_back.f_back and f.f_back.f_back.f_code == func.func_code:
