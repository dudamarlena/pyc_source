...
# Embedded file name: build/bdist.linux-i686/egg/pocoo/pkg/core/textfmt.py
...
frozen_translation_re = re.compile('<trans(?: value="(.*?)")?>(.*?)</trans>')
...
class PlainText(MarkupFormat):
    ...
    def quote_text(self, req, text, username=None):
        ...
        if username is not None:
            _ = req.gettext
            lines.insert(0, (_('%s has written') + ':') % username)
        return ('\n').join(lines)
...
def frozen_translation(value=None):
    ...
    def proxy(text):
        return '<trans%s>%s</trans>' % (attr, escape_html(text))
...
def parse_and_translate(ctx_or_request, text, signature=False, syntax_parser=None):
    """
