...
# [GCC 8.4.0]
# Embedded file name: M:\Programming\Project\django-auth\auth\CustomAuth\models\abstract_user.py
# Compiled at: 2020-03-20 12:37:06
...
class AbstractUser(AbstractBaseUser, PermissionMixin, DateMixin, FinanceMixin):
    ...
    def profile(self):
        if not (hasattr(self, '_profile') and self._profile):
            self._profile = None
    ...
    def generate_jwt_token(self):
        ...
        token = jwt.encode(payload={'id':self.pk, 
         'exp':dt},
    ...
    def digital_sign(self, days: int=None):
        if days:
            ...
            timed_sign = jwt.encode(payload={'sign':signing.dumps(self.id), 
             'exp':dt},
              key=(settings.SECRET_KEY),
              algorithm='HS256')
            return timed_sign.decode('utf-8')
        sign = jwt.encode(payload={'sign': signing.dumps(self.id)},
          key=(settings.SECRET_KEY),
          algorithm='HS256')
