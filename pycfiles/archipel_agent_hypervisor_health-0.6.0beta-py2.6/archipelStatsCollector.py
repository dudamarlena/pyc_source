...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/archipelagenthypervisorhealth/archipelStatsCollector.py
# Compiled at: 2013-03-20 13:50:16
import datetime, sqlite3, subprocess, time, json
...
class TNThreadedHealthCollector(Thread):
    ...
    def get_collected_stats(self, limit=1):
        ...
        if limit > 1:
            ...
            aload.reverse()
            anetwork.reverse()
        return {'cpu': acpu, 'memory': amem, 'disk': adisk, 'totaldisk': totalDisk, 'load': aload, 'uptime': uptime_stats, 'uname': self.uname_stats, 'network': anetwork}
    ...
    def get_cpu_stats(self):
        ...
        """
        dt = self.deltaTime(1)
        cpuPct = dt[(len(dt) - 1)] * 100.0 / sum(dt)
