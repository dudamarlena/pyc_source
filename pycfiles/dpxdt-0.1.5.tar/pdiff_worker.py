...
# [GCC 8.4.0]
# Embedded file name: /Users/danvk/github/dpxdt/dpxdt/client/pdiff_worker.py
# Compiled at: 2014-07-25 17:09:30
"""Background worker that does perceptual diffs, possibly from a queue."""
import Queue, json, os, shutil, subprocess, sys, tempfile, threading, time, urllib2, re, gflags
FLAGS = gflags.FLAGS
...
gflags.DEFINE_integer('pdiff_task_max_attempts', 3, 'Maximum number of attempts for processing a pdiff task.')
gflags.DEFINE_integer('pdiff_wait_seconds', 3, 'Wait this many seconds between repeated invocations of pdiff subprocesses. Can be used to spread out load on the server.')
gflags.DEFINE_string('pdiff_compare_binary', 'compare', 'Path to the compare binary used for generating perceptual diffs.')
gflags.DEFINE_string('pdiff_composite_binary', 'composite', 'Path to the composite binary used for resizing images.')
gflags.DEFINE_integer('pdiff_threads', 1, 'Number of perceptual diff threads to run')
gflags.DEFINE_integer('pdiff_timeout', 60, 'Seconds until we should give up on a pdiff sub-process and try again.')
...
class ResizeWorkflow(process_worker.ProcessWorkflow):
    ...
    def __init__(self, log_path, ref_path, run_path, resized_ref_path):
        ...
        """
        process_worker.ProcessWorkflow.__init__(self, log_path, timeout_seconds=FLAGS.pdiff_timeout)
...
class DoPdiffQueueWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id=None, release_name=None, release_number=None, run_name=None, reference_sha1sum=None, run_sha1sum=None, heartbeat=None):
        ...
        try:
            ...
            yield [
             release_worker.DownloadArtifactWorkflow(build_id, reference_sha1sum, result_path=ref_path),
             release_worker.DownloadArtifactWorkflow(build_id, run_sha1sum, result_path=run_path)]
            max_attempts = FLAGS.pdiff_task_max_attempts
            yield heartbeat('Resizing reference image')
            returncode = yield ResizeWorkflow(log_path, ref_path, run_path, ref_resized_path)
            if returncode != 0:
                raise PdiffFailedError(max_attempts, 'Could not resize reference image to size of new image')
            yield heartbeat('Running perceptual diff process')
            returncode = yield PdiffWorkflow(log_path, ref_resized_path, run_path, diff_path)
            diff_failed = True
            distortion = None
            ...
            yield release_worker.ReportPdiffWorkflow(build_id, release_name, release_number, run_name, diff_path, log_path, diff_failed, distortion)
            if diff_failed:
                raise PdiffFailedError(max_attempts, 'Comparison failed. returncode=%r' % returncode)
        finally:
