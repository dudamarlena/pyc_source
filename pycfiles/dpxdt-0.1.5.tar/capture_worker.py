...
# [GCC 8.4.0]
# Embedded file name: /Users/danvk/github/dpxdt/dpxdt/client/capture_worker.py
# Compiled at: 2014-07-25 17:49:27
"""Background worker that screenshots URLs, possibly from a queue."""
import Queue, json, os, shutil, subprocess, sys, threading, tempfile, time, urllib2, gflags
FLAGS = gflags.FLAGS
...
gflags.DEFINE_integer('capture_threads', 5, 'Number of website screenshot threads to run')
gflags.DEFINE_integer('capture_task_max_attempts', 3, 'Maximum number of attempts for processing a capture task.')
gflags.DEFINE_integer('capture_wait_seconds', 3, 'Wait this many seconds between repeated invocations of capture subprocesses. Can be used to spread out load on the server.')
gflags.DEFINE_string('phantomjs_binary', None, 'Path to the phantomjs binary')
gflags.DEFINE_string('phantomjs_script', None, 'Path to the script that drives the phantomjs process')
gflags.DEFINE_integer('phantomjs_timeout', 120, 'Seconds until giving up on a phantomjs sub-process and trying again.')
...
class CaptureWorkflow(process_worker.ProcessWorkflow):
    ...
    def __init__(self, log_path, config_path, output_path):
        ...
        """
        process_worker.ProcessWorkflow.__init__(self, log_path, timeout_seconds=FLAGS.phantomjs_timeout)
...
class DoCaptureQueueWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id=None, release_name=None, release_number=None, run_name=None, url=None, config_sha1sum=None, baseline=None, heartbeat=None):
        ...
        try:
            ...
            yield release_worker.DownloadArtifactWorkflow(build_id, config_sha1sum, result_path=config_path)
            yield heartbeat('Running webpage capture process')
            try:
                returncode = yield CaptureWorkflow(log_path, config_path, image_path)
            except (process_worker.TimeoutError, OSError) as e:
            ...
            yield release_worker.ReportRunWorkflow(build_id, release_name, release_number, run_name, image_path=image_path, log_path=log_path, baseline=baseline, run_failed=capture_failed)
            if capture_failed:
                raise CaptureFailedError(FLAGS.capture_task_max_attempts, failure_reason)
        finally:
...
def register(coordinator):
    ...
    item = queue_worker.RemoteQueueWorkflow(constants.CAPTURE_QUEUE_NAME, DoCaptureQueueWorkflow, max_tasks=FLAGS.capture_threads, wait_seconds=FLAGS.capture_wait_seconds)
    item.root = True
