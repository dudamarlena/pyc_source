...
# [GCC 8.4.0]
# Embedded file name: /Users/danvk/github/dpxdt/dpxdt/client/release_worker.py
# Compiled at: 2014-07-21 13:34:50
"""Background worker that uploads new release candidates."""
import hashlib, os, gflags
FLAGS = gflags.FLAGS
from dpxdt.client import fetch_worker
from dpxdt.client import workers
gflags.DEFINE_string('release_server_prefix', None, 'URL prefix of where the release server is located, such as "https://www.example.com/here/is/my/api". This should use HTTPS if possible, since API requests send credentials using HTTP basic auth.')
gflags.DEFINE_string('release_client_id', None, 'Client ID of the API key to use for requests to the release server.')
gflags.DEFINE_string('release_client_secret', None, 'Client secret of the API key to use for requests to the release server.')
...
class CreateReleaseWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, release_name, url):
        call = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/create_release', post={'build_id': build_id, 
           'release_name': release_name, 
           'url': url}, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
...
class UploadFileWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, file_path):
        try:
            ...
            upload = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/upload', post={'build_id': build_id, 'file': handle}, timeout_seconds=120, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
...
class FindRunWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, run_name):
        call = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/find_run', post={'build_id': build_id, 
           'run_name': run_name}, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
        if call.json and call.json.get('error'):
...
class RequestRunWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, release_name, release_number, run_name, url=None, config_data=None, ref_url=None, ref_config_data=None):
        ...
        if ref_url and ref_config_data:
            post.update(ref_url=ref_url, ref_config=ref_config_data)
        call = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/request_run', post=post, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
        if call.json and call.json.get('error'):
...
class ReportRunWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, release_name, release_number, run_name, image_path=None, log_path=None, url=None, config_path=None, ref_url=None, ref_image=None, ref_log=None, ref_config=None, baseline=None, run_failed=False):
        if baseline and (ref_url or ref_image or ref_log or ref_config):
            raise ReportRunError('Cannot specify "baseline" along with any "ref_*" arguments.')
        upload_jobs = [
        ...
        call = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/report_run', post=post, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
        if call.json and call.json.get('error'):
...
class ReportPdiffWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, release_name, release_number, run_name, diff_path=None, log_path=None, diff_failed=False, distortion=None):
        diff_id = None
        log_id = None
        if isinstance(diff_path, basestring) and os.path.isfile(diff_path) and isinstance(log_path, basestring) and os.path.isfile(log_path):
...
class RunsDoneWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, release_name, release_number):
        call = yield fetch_worker.FetchItem(FLAGS.release_server_prefix + '/runs_done', post={'build_id': build_id, 
           'release_name': release_name, 
           'release_number': release_number}, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
        if call.json and call.json.get('error'):
...
class DownloadArtifactWorkflow(workers.WorkflowItem):
    ...
    def run(self, build_id, sha1sum, result_path):
        ...
        call = yield fetch_worker.FetchItem(download_url, result_path=result_path, username=FLAGS.release_client_id, password=FLAGS.release_client_secret)
