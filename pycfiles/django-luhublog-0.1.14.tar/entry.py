...
# [GCC 8.4.0]
# Embedded file name: /home/johsanca/Projects/luhu-blog-app/luhublog/models/entry.py
# Compiled at: 2015-10-22 06:16:21
import os, bleach
...
class EntryCategory(TimeStampedModel):
    blog = models.ForeignKey(Blog, verbose_name='Site', blank=True, editable=False)
    name = models.CharField(verbose_name='Nombre', max_length=60)
    slug = models.SlugField(verbose_name='ULR Slug')
    description = models.CharField(verbose_name='Descripción', max_length=255, blank=True)
    image_cover = ImageField(verbose_name='Imagen de portada', upload_to=rename_filename, blank=True, null=True)
...
class Entry(TimeStampedModel):
    ...
    category = models.ForeignKey(EntryCategory, verbose_name='Categoría', blank=True, null=True)
    title = models.CharField(verbose_name='title', max_length=100)
    slug = models.SlugField(verbose_name='slug', unique=True, help_text="Used to build the entry's URL.")
    lead_entry = models.CharField(verbose_name='Encabezado', max_length=255, blank=True)
    image_header = ImageField(verbose_name='Imagen de encabezado', upload_to=rename_filename, blank=True, null=True)
    image_caption = ImageField(verbose_name='Imagen de la entrada', upload_to=rename_filename, blank=True, null=True)
    content = FroalaField(verbose_name='Cuerpo de la entrada')
    status = StatusField()
    author = models.ForeignKey(Author, verbose_name='Autor de la entrada', related_name='entries')
    start_publication = models.DateTimeField(verbose_name='Fecha de publicación', db_index=True, blank=True, null=True)
    related = models.ManyToManyField('self', blank=True, verbose_name=_('related entries'))
    seo_title = models.CharField(verbose_name='Meta Title', max_length=60, blank=True, default='', help_text='Google typically displays the first 50-60 characters of a title tag')
    seo_description = models.CharField(verbose_name='Meta Description', max_length=155, default='', blank=True, help_text='The description should optimally be between 10-155 characters')
    seo_keywords = models.CharField(verbose_name='Meta Keywords', max_length=400, default='', blank=True, help_text='Opcional')
