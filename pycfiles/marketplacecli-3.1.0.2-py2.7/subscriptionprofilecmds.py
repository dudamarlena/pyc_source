...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/subscriptionprofilecmds/subscriptionprofilecmds.py
...
class SubscriptionProfileCmds(Cmd, CoreGlobal):
    ...
    def arg_list(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' list', add_help=True, description='List all the subscription profiles for a given organization.  If no organization is provided the default organization is used.')
    ...
    def arg_info(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' info', add_help=True, description='Get detailed information on a subscription profile within an organization.')
    ...
    def arg_create(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' create', add_help=True, description='Create a new subscription profile within an organization. A subscription profile is mandatory in order to create a user. It must also be activated. By default, the subscription profiles you create are not active.')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        ...
        optional = do_parser.add_argument_group('optional arguments')
        optional.add_argument('--description', dest='description', required=False, help='the description of the subscription profile to create')
        optional.add_argument('--active', dest='active', required=False, help='flag to make the subscription profile active.')
    ...
    def arg_delete(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' delete', add_help=True, description='Delete a subscription profile from an organization.')
        mandatory = do_parser.add_argument_group('mandatory arguments')
    ...
    def arg_update(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' update', add_help=True, description='Updates a new subscription profile')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        mandatory.add_argument('--name', dest='name', required=True, help='the name of the subscription profile to update')
        optional = do_parser.add_argument_group('optional arguments')
        optional.add_argument('--description', dest='description', required=False, help='the description of the subscription profile to update')
        optional.add_argument('--active', dest='active', required=False, help='flag to make the subscription profile active.')
        optional.add_argument('--org', dest='org', required=False, help='the organization name. If no organization is provided, then the default organization is used.')
        return do_parser
