...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/subscriptionprofilecmds/subscriptionprofile_roles.py
...
class SubscriptionProfileRoles(Cmd, CoreGlobal):
    ...
    def arg_add(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' add', add_help=True, description='Add roles to a subscription profile')
    ...
    def arg_remove(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' remove', add_help=True, description='Remove one or several roles from a subscription profile')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        mandatory.add_argument('--name', dest='name', required=True, help='the name of the subscription profile')
        mandatory.add_argument('--roles', dest='roles', nargs='+', required=True, help='the roles to add to the subscription profile')
        optional = do_parser.add_argument_group('optional arguments')
