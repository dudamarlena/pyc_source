...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/usercmds/usercmds.py
...
class UserCmds(Cmd, CoreGlobal):
    ...
    def arg_info(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' info', add_help=True, description='Display informations of provided user')
        return do_parser
    ...
    def arg_create(self):
        ...
        mandatory.add_argument('--code', dest='code', required=True, help='the creation code (subscription profile) to be used to create the user account')
        optional = do_parser.add_argument_group('optional arguments')
        optional.add_argument('--accountPassword', dest='accountPassword', required=False, help='the new user account password')
        optional.add_argument('--org', dest='org', required=False, help='the organization in which the user should be created')
        optional.add_argument('--disable', dest='disable', required=False, help='flag to de-activate the account during the creation')
        return do_parser
    ...
    def arg_list(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' list', add_help=True, description='List the users registered to the platform')
        return do_parser
