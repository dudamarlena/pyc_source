...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/usercmds/user_admin_cmds.py
...
class UserAdminCmds(Cmd, CoreGlobal):
    ...
    def arg_promote(self):
        ...
        optionnal.add_argument('--org', dest='org', type=str, required=False, help='the organization where the user is/will be a member/administrator (depending on command context). If no organization is provided, then the default organization is used.')
        return doParser
    ...
    def do_promote(self, args):
        try:
            ...
            if adminUser == None:
                ...
                printer.out('User [' + doArgs.account + '] has been promoted in [' + orgSpecified.name + '] :', printer.OK)
    ...
    def do_demote(self, args):
        try:
            ...
            if adminUser == None:
                ...
                self.api.Orgs(orgSpecified.dbId).Members(adminUser.loginName).Change(Admin=False, body=adminUser)
                printer.out('User [' + doArgs.account + '] has been demoted in [' + orgSpecified.name + '] :', printer.OK)
            if adminUser.active == True:
                active = 'X'
            else:
                active = ''
