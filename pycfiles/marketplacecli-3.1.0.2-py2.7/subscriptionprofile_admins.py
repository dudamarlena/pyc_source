...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/subscriptionprofilecmds/subscriptionprofile_admins.py
...
class SubscriptionProfileAdmins(Cmd, CoreGlobal):
    ...
    def arg_add(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' add', add_help=True, description='Add a user as a subscription profile administrator.')
    ...
    def arg_remove(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' remove', add_help=True, description='Remove a user as an administrator of subscription profiles')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        mandatory.add_argument('--name', dest='name', required=True, help='the name of the subscription profile')
