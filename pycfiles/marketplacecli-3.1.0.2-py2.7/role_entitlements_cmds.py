...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/rolecmds/role_entitlements_cmds.py
...
class RoleEntitlementCmds(Cmd, CoreGlobal):
    ...
    def arg_add(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' add', add_help=True, description='Add one or more entitlements to a role within the specified organization')
        ...
        mandatory.add_argument('--entitlements', dest='entitlements', nargs='+', required=True, help='the entitlements to the role')
    ...
    def arg_remove(self):
        ...
        mandatory = do_parser.add_argument_group('mandatory arguments')
        mandatory.add_argument('--name', dest='name', required=True, help='the name of role')
        mandatory.add_argument('--entitlements', dest='entitlements', nargs='+', required=True, help='the login name to remove as a new subscription profile administrator.')
        optional = do_parser.add_argument_group('optional arguments')
        optional.add_argument('--org', dest='org', required=False, help='the organization name. If no organization is provided, then the default organization is used.')
        return do_parser
