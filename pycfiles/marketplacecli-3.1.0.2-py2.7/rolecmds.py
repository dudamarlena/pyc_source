...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/marketplacecli/commands/rolecmds/rolecmds.py
...
class RoleCmds(Cmd, CoreGlobal):
    ...
    def arg_list(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' list', add_help=True, description='List all the roles for a given organization.  If not organization is provided the default organization is used.')
    ...
    def arg_info(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' info', add_help=True, description='Prints out all the details of a specified role within an organization')
    ...
    def arg_create(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' create', add_help=True, description='Create a new empty role in the specified organization')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        ...
        optional.add_argument('--entitlements', dest='entitlements', nargs='+', required=False, help='a list of entitlements to be added to this role during creation (example: --entitlements ent1 ent2 ent3). For a list of available entitlements, run the command: uforge entitlement list --org <org name>')
        optional.add_argument('--org', dest='org', required=False, help='the organization name. If no organization is provided, then the default organization is used.')
        return do_parser
    ...
    def arg_delete(self):
        do_parser = ArgumentParser(prog=self.cmd_name + ' delete', add_help=True, description='Delete a role from the specified organization')
        mandatory = do_parser.add_argument_group('mandatory arguments')
        mandatory.add_argument('--name', dest='name', required=True, help='name of the role')
        optional = do_parser.add_argument_group('optional arguments')
        optional.add_argument('--org', dest='org', required=False, help='the organization name. If no organization is provided, then the default organization is used.')
