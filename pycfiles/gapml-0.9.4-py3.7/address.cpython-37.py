...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gapml/address.py
...
class Address(object):
        ...
        if self._pob is not None:
            ...
            if self.index < self.length:
                if self.words[self.index]['word'] == ',':
                    self.index += 1
            if self._debug:
                print('POB', self._pob, self.idx_pob)
            return True
        return False
    ...
    def parse_pob(self):
        ...
        if self.words[index]['word'] == 'pob' or self.words[index]['word'] == 'pmb':
            ...
            if self.words[index]['word'] == '.':
                return (
                 self.words[(index + 1)]['word'], 3)
            return (self.words[index]['word'], 2)
        else:
            if self.words[index]['word'] == 'p':
                index += 1
                if index == self.length:
                    return (None, 0)
                if self.words[index]['word'] == '.':
                ...
                if self.words[index]['word'] == '.':
                    ...
                    if index == self.length:
                        return (None, 0)
                return (
                 self.words[index]['word'], index - start + 1)
        ...
        if self._stn is not None:
            ...
            if self.index < self.length:
                if self.words[self.index]['word'] == ',':
                    self.index += 1
            if self._debug:
                print('STN', self._stn, self.idx_stn)
            return True
        return False
    ...
    def streetnum(self):
        ...
        if self._num is not None:
            ...
            self.index += n
            if self._debug:
                print('NUM', self._num, self.idx_num)
            return True
        return False
    ...
    def parse_streetnum(self):
        ...
        index = self.index
        start = self.index
        if len(self.words[index]['word']) == 1:
            if not self.words[index]['word'][0].isdigit():
                return (None, 0)
        else:
            ...
            return word[0].isdigit() or (None, 0)
        index += 1
        if index == self.length:
            return (None, 0)
        if self.words[index]['word'] == '-':
            ...
            if self.words[index]['word'][0] in ('n', 's', 'w', 'e') and self.words[index]['word'][1:].isdigit():
                ...
                if len(self.words[index]['word']) == 1 and self.words[index]['word'].isalpha():
                    pass
                elif not self.words[index]['word'].isdigit():
                    index -= 2
            number += self.words[index]['word']
        ...
        if self._dir is not None:
            ...
            if self.index < self.length:
                if self.words[self.index]['word'] == ',':
                    self.index += 1
            if self._debug:
                print('DIR', self._dir, self.idx_dir)
            return True
        return False
    ...
    def streetname(self):
        ...
        if self._nam is not None:
            ...
            self.index += n
            if self._debug:
                print('NAM', self._nam, self.idx_nam)
            return True
        return False
        ...
        if self._typ is not None:
            ...
            if self.index < self.length:
                if self.words[self.index]['word'] == ',':
                    self.index += 1
            if self._debug:
                print('TYP', self._typ, self.idx_typ)
            self.isaddr = True
            return True
        return False
        ...
        if self._sac is not None:
            ...
            if self.index < self.length:
                if self.words[self.index]['word'] == ',':
                    self.index += 1
            if self._debug:
                print('SAC', self._sac, self.idx_sac)
            return True
        return False
        ...
        if self.words[index]['word'] == ',':
            ...
            if n > 0:
                ...
                if index == self.length:
                    index -= 1
                return (
                 city, state, index - start + 1, index)
    ...
    def state_hack(self, index):
        ...
        if self.words[index]['word'] == 'medical doctor':
            ...
            if self.words[index]['word'] == 'd':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'c':
                        return ('ISO3166-2:US-DC', 1)
            if self.words[index]['word'] == 'rhode':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'island':
                        return ('ISO3166-2:US-RI', 1)
            if self.words[index]['word'] == 'virgin':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'islands':
                        return ('ISO3166-2:US-VI', 1)
            if self.words[index]['word'] == 'puerto':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'rico':
                        return ('ISO3166-2:US-PR', 1)
            if self.words[index]['word'] == 'american':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'samoa':
                        return ('ISO3166-2:US-AS', 1)
            if self.words[index]['word'] == 'marshall':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'islands':
                        return ('ISO3166-2:US-FM', 1)
            if self.words[index]['word'] == 'northern':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'marianas':
                        return ('ISO3166-2:US-FM', 1)
            if self.words[index]['word'] == 'british':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'columbia':
                        return ('ISO3166-2:CA-BC', 1)
            if self.words[index]['word'] == 'newfoundland':
                if index + 2 < self.length:
                    if self.words[(index + 1)]['word'] == 'and':
                        return ('ISO3166-2:CA-NL', 1)
            if self.words[index]['word'] == 'nova':
                if index + 1 < self.length:
                    if self.words[(index + 1)]['word'] == 'scotia':
                        return ('ISO3166-2:CA-NS', 1)
        else:
            if self.words[index]['word'] == 'prince':
                if index + 2 < self.length:
                    if self.words[(index + 1)]['word'] == 'edward':
            ...
            if self.words[index]['word'] in ('new', 'north', 'south', 'west', 'northwest'):
                ...
                try:
                    return (
                     self._state_dict[word], 1)
                except:
                    pass

        return (None, 0)

    _state_dict = {'al':'ISO3166-2:US-AL', 
    ...
    def postal(self):
        ...
        if self._pst is not None:
            ...
            self.index += n
            if self._debug:
                print('PST', self._pst, self.idx_pst)
            return True
        return False
    ...
    def parse_postalUS(self):
        ...
        index = self.index
        return len(self.words[index]['word']) != 5 or self.words[index]['word'].isdigit() or (None,
                                                                                              0)
        ...
        if index + 1 < self.length:
            if self.words[(index + 1)]['word'] == '-':
                ...
                if index == self.length:
                    return (None, 0)
                if len(self.words[index]['word']) == 4:
                    if self.words[index]['word'].isdigit():
                        postal += '-' + self.words[index]['word']
                        return (postal, 3)
                return (
                 postal, 1)
