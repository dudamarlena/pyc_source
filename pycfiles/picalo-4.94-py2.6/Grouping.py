...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Grouping.py
# Compiled at: 2011-02-17 12:03:13
"""
...
__functions__ = [
 ...
 'summarize_by_step',
 'summarize_by_date']
import Simple, math, sys, types, datetime
  ...
  """
    if isinstance(table, TableArray):
        ...
        if len(table) == 0:
            ...
            assert isinstance(step, datetime.timedelta) and table.column(col).get_type() in (Date, DateTime), 'The column to group by must be DateTime or Date type.'
