...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Simple.py
# Compiled at: 2011-02-17 12:01:00
"""
...
"""
__functions__ = ('col_join', 'col_left_join', 'col_right_join', 'col_match', 'col_match_same',
                 'col_match_diff', 'compare_records', 'custom_match', 'custom_match_same',
                 'custom_match_diff', 'describe', 'expression_match', 'find_duplicates',
                 'find_gaps', 'fuzzysearch', 'fuzzymatch', 'fuzzycoljoin', 'get_unordered',
                 'join', 'left_join', 'regex_match', 'right_join', 'select', 'select_by_value',
                 'select_outliers', 'select_outliers_z', 'select_nonoutliers', 'select_nonoutliers_z',
                 'select_records', 'soundex', 'soundexcol', 'sort', 'transpose',
                 'wildcard_match')
import lib.stats, math, types, re
from picalo import Table, TableList, TableArray, show_progress, clear_progress, check_valid_table
from picalo import run_tablearray, ensure_unique_colname, ensure_unique_list_value, make_unique_colnames, make_valid_variable
from Column import _ColumnLoader
...
def select_by_regex(table, **col_value_pairs):
    r"""Selects record from a table given colname=pattern pairs and returns a new
...
def col_match_same(table1, table2, *match_cols):
  ...
  """
    ...
    for i in [0, 1]:
        for recno in dict(zip(matches.column(i), matches.column(i))).keys():
...
def regex_match(pattern, value, ignorecase=False):
    r"""Returns whether the given value matches the given regular
...
def wildcard_match(pattern, value, ignorecase=False, fullmatch=True):
  ...
  """
    ...
    regex = []
    for ch in pattern:
        if ch not in '01234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-*?# ':
            regex.append('\\')
        regex.append(ch)
