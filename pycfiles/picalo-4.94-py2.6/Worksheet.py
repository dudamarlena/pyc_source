...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/lib/pyExcelerator/Worksheet.py
...
import BIFFRecords, Bitmap, Formatting, Style
from Deco import *
...
class Worksheet(object):
    ...
    def __init__(self, sheetname, parent_book):
        ...
        self.__row_default_height = 255
        self.__col_default_width = 8
        self.__calc_mode = 1
        self.__calc_count = 100
        ...
        self.__left_margin = 0.3
        self.__right_margin = 0.3
        self.__top_margin = 0.61
        self.__bottom_margin = 0.37
        self.__paper_size_code = 9
        ...
        self.__print_hres = 300
        self.__print_vres = 300
    ...
    def __wsbool_rec(self):
        options = 0
        options |= (self.__show_auto_page_breaks & 1) << 0
        options |= (self.__dialogue_sheet & 1) << 4
        options |= (self.__auto_style_outline & 1) << 5
        options |= (self.__outline_below & 1) << 6
        options |= (self.__outline_right & 1) << 7
        options |= (self.__fit_num_pages & 1) << 8
        options |= (self.__show_row_outline & 1) << 10
        options |= (self.__show_col_outline & 1) << 11
        options |= (self.__alt_expr_eval & 1) << 14
        options |= (self.__alt_formula_entries & 1) << 15
    ...
    def __window2_rec(self):
        options = 0
        options |= (self.__show_formulas & 1) << 0
        options |= (self.__show_grid & 1) << 1
        options |= (self.__show_headers & 1) << 2
        options |= (self.__panes_frozen & 1) << 3
        options |= (self.__show_empty_as_zero & 1) << 4
        options |= (self.__auto_colour_grid & 1) << 5
        options |= (self.__cols_right_to_left & 1) << 6
        options |= (self.__show_outline & 1) << 7
        options |= (self.__remove_splits & 1) << 8
        options |= (self.__selected & 1) << 9
        options |= (self.__hidden & 1) << 10
        options |= (self.__page_preview & 1) << 11
        return BIFFRecords.Window2Record(options, self.__first_visible_row, self.__first_visible_col, self.__grid_colour, self.__preview_magn, self.__normal_magn).get()
    ...
    def __calc_settings_rec(self):
        result = ''
        result += BIFFRecords.CalcModeRecord(self.__calc_mode & 1).get()
        result += BIFFRecords.CalcCountRecord(self.__calc_count & 65535).get()
        result += BIFFRecords.RefModeRecord(self.__RC_ref_mode & 1).get()
        result += BIFFRecords.IterationRecord(self.__iterations_on & 1).get()
        result += BIFFRecords.DeltaRecord(self.__delta).get()
        result += BIFFRecords.SaveRecalcRecord(self.__save_recalc & 1).get()
        return result
    ...
    def __print_settings_rec(self):
        ...
        setup_page_options = (self.__print_in_rows & 1) << 0
        setup_page_options |= (self.__portrait & 1) << 1
        setup_page_options |= 0
        setup_page_options |= (self.__print_not_colour & 1) << 3
        setup_page_options |= (self.__print_draft & 1) << 4
        setup_page_options |= (self.__print_notes & 1) << 5
        setup_page_options |= 0
        setup_page_options |= 128
        setup_page_options |= (self.__print_notes_at_end & 1) << 9
        setup_page_options |= (self.__print_omit_errors & 3) << 10
        result += BIFFRecords.SetupPageRecord(self.__paper_size_code, self.__print_scaling, self.__start_page_number, self.__fit_width_to_pages, self.__fit_height_to_pages, setup_page_options, self.__print_hres, self.__print_vres, self.__header_margin, self.__footer_margin, self.__copies_num).get()
        return result
