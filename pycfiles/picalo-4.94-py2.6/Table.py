...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Table.py
# Compiled at: 2011-03-14 13:29:23
import os, sys, time, types, codecs, os.path, tempfile, inspect, re, codecs, xml.dom.minidom
...
import picalo.lib.pyExcelerator, picalo.lib.pyExcelerator.CompoundDoc, picalo.lib.delimitedtext
__all__ = [
 'Table',
 'load_delimited',
 'load_csv',
...
class Table(persistent.Persistent):
    """The primary data structure used in Picalo modules.  This is the first object a user
     ...
     Example 7:
       ...
       +----+----+
  """
    TRANSIENT_VARIABLE_NAMES = [
     '_listeners', '_indexes', 'changed', 'filterindex', 'filterexpression']
    ...
    """
        ...
        if isinstance(columns, (types.IntType, types.LongType)):
    ...
    """
        ...
        elif len(a) > 0 and isinstance(a[0], (types.ListType, types.TupleType, Record)):
            for row in a:
        ...
        elif len(a) > 0 and isinstance(a[0], types.DictType):
            for row in a:
    ...
    """
        if not isinstance(col, int):
            if col not in self.columns_map_names:
    ...
    """
        coldef = self.column(column)
        if new_index != self.column_count():
    ...
    """
        ...
        if isinstance(index, types.SliceType):
            ...
            if step == None:
                step = 1
            if stop == None:
                stop = self.record_count(respect_filter)
    ...
    """
        ...
        table = Table(self.get_columns(), self)
        table.extend(other)
        return table
    ...
    """
        ...
        for record in self.iterator(respect_filter):
            ...
            for i in range(self.column_count()):
                ...
                elif isinstance(field, (types.IntType, types.LongType, types.FloatType)):
...
bommap = {(0, 0, 254, 255): 'utf_32_be', 
   (255, 254, 0, 0): 'utf_32_le', 
   (239, 187, 191): 'utf_8', 
   (254, 255): 'utf_16_be', 
   (255, 254): 'utf_16_le'}
    ...
    try:
        ...
        if encoding != None:
            ...
            if bomDetection == None:
                encoding = 'utf_8'
                filename = codecs.EncodedFile(filename, 'utf_8', 'utf_8', errors)
            else:
