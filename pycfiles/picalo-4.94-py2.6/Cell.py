...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/lib/pyExcelerator/Cell.py
# Compiled at: 2008-03-17 12:58:02
__rev_id__ = '$Id: Cell.py,v 1.3 2005/08/11 08:53:48 rvk Exp $'
import struct, BIFFRecords

class StrCell(object):
    __slots__ = [
     '__init__', 'get_biff_data',
...
class NumberCell(object):
    ...
    def get_biff_data(self):
        ...
        if w0 == 0 and w1 == 0 and w2 & 65532 == w2:
            rk_encoded = w3 << 16 | w2
            return BIFFRecords.RKRecord(self.__parent.get_index(), self.__idx, self.__xf_idx, rk_encoded).get()
        if abs(self.__number) < 1073741824 and int(self.__number) == self.__number:
        ...
        if w0 == 0 and w1 == 0 and w2 & 65532 == w2:
            ...
            return BIFFRecords.RKRecord(self.__parent.get_index(), self.__idx, self.__xf_idx, rk_encoded).get()
        if abs(temp) < 1073741824 and int(temp) == temp:
