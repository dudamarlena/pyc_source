...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Calendar.py
# Compiled at: 2011-02-17 12:17:44
...
for (repattern, timepattern) in DATE_FORMATS:
...
def _parse(klass, subklass, stdate, format):
    ...
    m = NATIVE_FORMAT_PARSER.search(stdate)
...
def _create(klass, subklass, *args, **kargs):
    ...
    if len(args) == 1:
        if isinstance(args[0], (datetime.datetime, datetime.date)):
            if klass == datetime.datetime and type(args[0]) in (DateTime, datetime.datetime):
                return args[0]
...
class Date(datetime.date):
    ...
    def __reduce_ex__(self, pickle_protocol):
        ...
        return (
         Date, (self.strftime('%Y-%m-%d %H:%M:%S'),))

    def __add__(self, other):
        return datetime.date.__add__(self, __coerce_value__(other))

    def __radd__(self, other):
        return datetime.date.__radd__(self, __coerce_value__(other))

    def __sub__(self, other):
        return datetime.date.__sub__(self, __coerce_value__(other))

    def __rsub__(self, other):
        return datetime.date.__rsub__(self, __coerce_value__(other))
...
class DateTime(datetime.datetime):
    ...
    def __reduce_ex__(self, pickle_protocol):
        """Helps in saving and loading the object"""
        return (
         DateTime, (self.strftime('%Y-%m-%d %H:%M:%S'),))

    def __add__(self, other):
        return datetime.datetime.__add__(self, __coerce_value__(other))

    def __radd__(self, other):
        return datetime.datetime.__radd__(self, __coerce_value__(other))

    def __sub__(self, other):
        return datetime.datetime.__sub__(self, __coerce_value__(other))

    def __rsub__(self, other):
        return datetime.datetime.__rsub__(self, __coerce_value__(other))
