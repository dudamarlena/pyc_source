...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/lib/pyExcelerator/antlr.py
...
class NoViableAltException(RecognitionException):
    ...
    def __str__(self):
        if self.token:
            ...
            raise self.node or AssertionError
...
class NoViableAltForCharException(RecognitionException):
    ...
    def __init__(self, *args):
        ...
        if len(args) == 2:
            ...
            scanner = args[1]
            RecognitionException.__init__(self, 'NoViableAlt', scanner.getFilename(), scanner.getLine(), scanner.getColumn())
        elif len(args) == 4:
            self.foundChar = args[0]
            fileName = args[1]
            line = args[2]
            ...
            RecognitionException.__init__(self, 'NoViableAlt', '', -1, -1)
        return
...
class MismatchedCharException(RecognitionException):
    ...
    def __init__(self, *args):
        ...
        if len(args) == 5:
            ...
            self.upper = args[2]
            self.scanner = args[4]
            RecognitionException.__init__(self, 'Mismatched char range', self.scanner.getFilename(), self.scanner.getLine(), self.scanner.getColumn())
            ...
            self.expecting = args[1]
            self.scanner = args[3]
            RecognitionException.__init__(self, 'Mismatched char', self.scanner.getFilename(), self.scanner.getLine(), self.scanner.getColumn())
            ...
            self.set = args[1]
            self.scanner = args[3]
            RecognitionException.__init__(self, 'Mismatched char set', self.scanner.getFilename(), self.scanner.getLine(), self.scanner.getColumn())
        else:
    ...
    def __init__(self, *args):
        ...
        if len(args) >= 2:
            if isinstance(args[1], Token):
                ...
                RecognitionException.__init__(self, 'Mismatched Token', self.fileName, self.token.getLine(), self.token.getColumn())
            elif isinstance(args[1], AST):
                self.node = args[1]
                self.tokenText = str(self.node)
                RecognitionException.__init__(self, 'Mismatched Token', '<AST>', self.node.getLine(), self.node.getColumn())
            else:
                self.tokenText = '<empty tree>'
                RecognitionException.__init__(self, 'Mismatched Token', '<AST>', -1, -1)
        return
...
class TokenStreamSelector(TokenStream):
    ...
    def select(self, arg):
        ...
        if isinstance(arg, str):
            ...
            return
        raise TypeError('TokenStreamSelector.select requires ' + 'TokenStream or string argument')
...
class TokenStreamBasicFilter(TokenStream):
    ...
    def discard(self, arg):
        ...
        if isinstance(arg, BitSet):
            ...
            return
        raise TypeError('TokenStreamBasicFilter.discard requires' + 'integer or BitSet argument')
...
class TokenStreamHiddenTokenFilter(TokenStreamBasicFilter):
    ...
    def consumeFirst(self):
        self.consume()
        p = None
        while not self.hideMask.member(self.LA(1).getType()):
            if self.discardMask.member(self.LA(1).getType()):
    ...
    def nextToken(self):
        ...
        self.consume()
        p = monitored
        while self.hideMask.member(self.LA(1).getType()) or self.discardMask.member(self.LA(1).getType()):
            if self.hideMask.member(self.LA(1).getType()):
                p.setHiddenAfter(self.LA(1))
                if p != monitored:
                    self.LA(1).setHiddenBefore(p)
...
class CharScanner(TokenStream):
    ...
    def testForLiteral(self, token):
        if not token:
            ...
            if not _text:
                return
            raise isinstance(_text, str) or isinstance(_text, unicode) or AssertionError
...
class BitSet(object):
    ...
    def __init__(self, data=None):
        ...
        if not isinstance(data, list):
            raise TypeError('BitSet requires integer, long, or ' + 'list argument')
        for x in data:
            if not isinstance(x, long):
                raise TypeError(self, 'List argument item is ' + 'not a long: %s' % x)
...
def illegalarg_ex(func):
    raise ValueError('%s is only valid if parser is built for debugging' % func.func_name)


def runtime_ex(func):
    raise RuntimeException('%s is only valid if parser is built for debugging' % func.func_name)
...
class Parser(object):
    ...
    def _match_int(self, t):
        if self.LA(1) != t:
            raise MismatchedTokenException(self.tokenNames, self.LT(1), t, False, self.getFilename())
        else:
            self.consume()

    def _match_set(self, b):
        if not b.member(self.LA(1)):
            raise MismatchedTokenException(self.tokenNames, self.LT(1), b, False, self.getFilename())
        else:
            self.consume()
    ...
    def matchNot(self, t):
        if self.LA(1) == t:
            raise MismatchedTokenException(tokenNames, self.LT(1), t, True, self.getFilename())
        else:
            self.consume()
    ...
    def addASTChild(self, currentAST, child):
        if not child:
            return
            currentAST.root = currentAST.root or child
...
class LLkParser(Parser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(arg1, TokenStream):
            ...
            self.set_k(1, *args)
            return
        raise TypeError('LLkParser requires integer, ' + 'ParserSharedInputStream or TokenStream argument')
...
class TreeParser(object):
    ...
    def traceIn(self, rname, t):
        self.traceDepth += 1
        self.traceIndent()
        print '> ' + rname + '(' + ifelse(t, str(t), 'null') + ')' + ifelse(self.inputState.guessing > 0, '[guessing]', '')
...
class BaseAST(AST):
    ...
    def equalsTree(self, t):
        return self.equals(t) and cmptree(self.getFirstChild(), t.getFirstChild(), partial=False)

    def equalsTreePartial(self, t):
        if not t:
            return True
        return self.equals(t) and cmptree(self.getFirstChild(), t.getFirstChild(), partial=True)
    ...
    def findAllPartial(self, sub):
        ...
        if not sub:
            ...
            self.doWorkForFindAll(roots, sub, True)
            return roots
    ...
    def initialize(self, *args):
        ...
        if args:
            if isinstance(args[0], Token):
                ...
                raise isinstance(args[0], CommonHiddenStreamToken) or AssertionError
    ...
    def create(self, *args):
        if not args:
            ...
            if isinstance(arg0, Token) and not arg1:
                ...
                raise isinstance(ttype, int) or AssertionError
            t = self.create(ttype)
            if t:
                t.initialize(arg0)
            return t
            ...
            if isinstance(arg0, Token):
                ...
                if isinstance(arg0, type):
                    ...
                    raise isinstance(t, AST) or AssertionError
                return t
            return
