...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/lib/stats.py
# Compiled at: 2011-02-08 20:46:38
"""
stats.py module
...
"""
    ...
    def __call__(self, arg1, *args, **kw):
        for (t, func) in self._dispatch.items():
            if isinstance(arg1, t):
                return apply(func, (arg1,) + args, kw)
...
"""
    (hist, smallest, binsize, extras) = histogram(inlist, numbins)
    cumhist = cumsum(hist)
...
"""
        ...
        if type(x) == N.ArrayType:
            frozen = N.ones(x.shape, N.Float) * -1
        else:
            arrayflag = 0
            frozen = N.array([-1])
            x = N.array([x])
...
"""
        ...
        b = N.dot(N.dot(LA.inverse(N.dot(N.transpose(x), x)), N.transpose(x)), data)
