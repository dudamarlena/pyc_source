...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Benfords.py
# Compiled at: 2011-02-17 12:01:31
"""
...
"""
__functions__ = ('analyze', 'calc_benford', 'get_expected')
import math, types
...
def calc_benford(position, digit, base=10):
  ...
  """
    ...
    if position == 0:
        ...
        k = math.pow(base, position - 1)
    k_stop = math.pow(base, position) - 1
    part2 = 0.0
    while k <= k_stop:
...
def get_expected(number, number_of_significant_digits=1):
  ...
  """
    ...
    for pos in range(0, number_of_significant_digits):
        probability *= calc_benford(pos, int(numst[pos]))
