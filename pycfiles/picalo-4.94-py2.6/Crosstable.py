...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/picalo/Crosstable.py
# Compiled at: 2011-02-17 12:01:47
from picalo import Table, TableArray, Simple, show_progress, clear_progress, check_valid_table
from picalo import run_tablearray, ensure_valid_variables, ensure_unique_list_value, make_unique_colnames, make_valid_variable
...
__functions__ = ('pivot', 'pivot_table', 'pivot_map', 'pivot_map_detail')
...
def pivot(table, col_fields, row_fields, expressions, delimiter='_'):
  ...
  """
    ...
    try:
        ...
        columns = [make_valid_variable(('_').join(_ensure_tuple(row_fields)))]
  ...
  """
    ...
    try:
        ...
        for (k, rowlabel) in enumerate(rowlabels):
            show_progress('Running pivot...', float(k) / len(rowlabels))
            columns = [
             ('_').join([ unicode(name) for name in _ensure_tuple(rowlabel) ])]
            for collabel in collabels:
                if len(ct.expressions) == 1:
                    columns.append(ct.get_cell_calculations(rowlabel, collabel)[0])
                else:
                    columns.append(ct.get_cell_calculations(rowlabel, collabel))

            if len(ct.expressions) == 1:
                columns.append(ct.get_row_calculations(rowlabel)[0])
            else:
                columns.append(ct.get_row_calculations(rowlabel))
            table.append(columns)
        ...
        for collabel in collabels:
            if len(ct.expressions) == 1:
                columns.append(ct.get_col_calculations(collabel)[0])
            else:
                columns.append(ct.get_col_calculations(collabel))

        if len(ct.expressions) == 1:
            columns.append(ct.get_table_calculations()[0])
        else:
            columns.append(ct.get_table_calculations())
        table.append(columns)
        return table
    finally:
        clear_progress()
...
class Crosstabler:
  ...
  """
    ...
    def _get_records(self, indexfields, key):
        ...
        index = self.table.index(*indexfields)
    ...
    def _get_calculations(self, indexfields=None, key=None):
        ...
        if indexfields == None or key == None:
            group = self.table
        else:
