...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rede/rede.py
...
class Setor(Arvore):
    ...
    def __init__(self, nome, vizinhos, nos_de_carga, prioridade=0):
        assert isinstance(nome, str), 'O parâmetro nome da classeSetor deve ser do tipo string'
        assert isinstance(vizinhos, list), 'O parâmetro vizinhos da classe Setor deve ser do tipo list'
        assert isinstance(nos_de_carga, list), 'O parâmetro nos_de_carga da classeSetor deve ser do tipo list'
        assert prioridade >= 0 and prioridade <= 10, 'O valo de prioridadedeve estar entre 0-10'
        self.nome = nome
...
class NoDeCarga(object):
    ...
    def __init__(self, nome, vizinhos, potencia=Fasor(real=0.0, imag=0.0, tipo=Fasor.Potencia), tensao=Fasor(real=0.0, imag=0.0, tipo=Fasor.Tensao), chaves=None):
        assert isinstance(nome, str), 'O parâmetro nome da classe NoDeCarga deve ser do tipo string'
        assert isinstance(vizinhos, list), 'O parâmetro vizinhos da classe Barra deve ser do tipo string'
        self.nome = nome
        self.vizinhos = vizinhos
        ...
        if chaves is not None:
            assert isinstance(chaves, list), 'O parâmetro chaves da classe NoDeCarga deve ser do tipo list'
            self.chaves = chaves
        else:
            self.chaves = list()
        self.setor = None
...
class Subestacao(object):
    ...
    def __init__(self, nome, alimentadores, transformadores, impedancia_positiva=0, impedancia_zero=0):
        assert isinstance(nome, str), 'O parâmetro nome da classe Subestacao deve ser do tipo str'
        assert isinstance(alimentadores, list), 'O parâmetro alimentadores da classe deve ser do tipo list'
        assert isinstance(transformadores, list), 'O parâmetro alimentadores da classe deve ser do tipo list'
        self.nome = nome
    ...
    def calculacurto(self, tipo):
        if tipo == 'trifasico':
            ...
            self.curto_bifasico = [
             [
              'Trecho 2fasico', 'Curto pu', 'Curto A']]
            for alimentador_atual, r in self.alimentadores.iteritems():
                for i in self.alimentadores[alimentador_atual].trechos.values():
            ...
            self.curto_monofasico_minimo = [
             [
              'Trecho 1fasico min', 'Curto pu', 'Curto A']]
            for alimentador_atual, r in self.alimentadores.iteritems():
                for i in self.alimentadores[alimentador_atual].trechos.values():
    ...
    def _calculaimpedanciaeq(self, trecho_anterior, no_atual, alimentador_atual, trechosvisitados):
        for i in self.alimentadores[alimentador_atual].trechos.values():
            if i not in trechosvisitados and (i.n1 or i.n2) == no_atual:
    ...
    def _busca_trecho(self, alimentador, n1, n2):
        u"""Função que busca trechos em um alimendador entre os nós/chaves
          n1 e n2"""
    ...
    def _atribuir_tensao_a_subestacao(self, tensao):
        u""" Função que atribui tensão à subestação
        ...
        for alimentador in self.alimentadores.values():
            for no in alimentador.nos_de_carga.values():
                no.tensao = Fasor(real=tensao.real, imag=tensao.imag, tipo=Fasor.Tensao)

    def _varrer_alimentador(self, alimentador):
        u""" Função que varre os alimentadores pelo
        ...
        prof = prof_max
        while prof >= 0:
            nos = [ alimentador.nos_de_carga[no_prof[1]] for no_prof in rnp_alimentador.transpose() if int(no_prof[0]) == prof
                  ]
            prof -= 1
            for no in nos:
                ...
                if vizinhos_jusante == []:
                    ...
                    for no_jus in vizinhos_jusante:
                        ...
                        trecho = self._busca_trecho(alimentador, no.nome, no_jus.nome)
        ...
        prof = 0
        while prof <= prof_max:
            nos = [ alimentador.nos_de_carga[col_no_prof[1]] for col_no_prof in rnp_alimentador.transpose() if int(col_no_prof[0]) == prof + 1
                  ]
            for no in nos:
                vizinhos = arvore_nos_de_carga[no.nome]
                no_prof = [ col_no_prof for col_no_prof in rnp_alimentador.transpose() if col_no_prof[1] == no.nome
                          ]
                ...
                print ('Tensao do no {nome}: {tens}').format(nome=no.nome, tens=no.tensao.mod * np.sqrt(3) / 1000.0)
                corrente = no.tensao.real - no_mon.tensao.real
                corrente += (no.tensao.imag - no_mon.tensao.imag) * complex(0.0, 1.0)
                corrente /= r + x * complex(0.0, 1.0)
                if not isinstance(trecho, Trecho):
                    trecho[0].fluxo = Fasor(real=corrente.real, imag=corrente.imag, tipo=Fasor.Corrente)
                    trecho[1].fluxo = Fasor(real=corrente.real, imag=corrente.imag, tipo=Fasor.Corrente)
                else:
                    trecho.fluxo = Fasor(real=corrente.real, imag=corrente.imag, tipo=Fasor.Corrente)

            prof += 1

    def calcular_fluxo_de_carga(self):
        f1 = Fasor(mod=13800.0 / np.sqrt(3), ang=0.0, tipo=Fasor.Tensao)
        ...
        for alimentador in self.alimentadores.values():
            ...
            while iter <= max_iteracaoes and converg > criterio_converg:
                ...
                tensao_nos = dict()
                for no in alimentador.nos_de_carga.values():
                    tensao_nos[no.nome] = Fasor(real=no.tensao.real, imag=no.tensao.imag, tipo=Fasor.Tensao)
...
class Trecho(Aresta):
    ...
    def __init__(self, nome, n1, n2, fluxo=None, condutor=None, comprimento=None, resistenciacontato=100):
        assert isinstance(nome, str), 'O parâmetro nome da classe Trecho deve ser do tipo str'
        assert isinstance(n1, NoDeCarga) or isinstance(n1, Chave), 'O parâmetro n1 da classe Trecho deve ser do tipo No de carga ou do tipo Chave'
        assert isinstance(n2, NoDeCarga) or isinstance(n2, Chave), 'O parâmetro n2 da classe Trecho deve ser do tipo No de carga ou do tipo Chave'
        ...
        self.impedancia_positiva = (self.condutor.rp + self.condutor.xp * complex(0.0, 1.0)) * self.comprimento
        self.impedancia_zero = (self.condutor.rz + self.condutor.xz * complex(0.0, 1.0)) * self.comprimento
    ...
    def calcula_impedancia(self):
        return (
         self.comprimento * self.condutor.rp,
    ...
    def calcula_curto_bifasico(self):
        curto2 = 1.7320508075688772 * self.base.corrente / (2 * self.impedancia_equivalente_positiva)
...
class Alimentador(Arvore):
    ...
    def __init__(self, nome, setores, trechos, chaves):
        assert isinstance(nome, str), 'O parâmetro nome da classe Alimentadordeve ser do tipo string'
        assert isinstance(setores, list), 'O parâmetro setores da classeAlimentador deve ser do tipo list'
        assert isinstance(chaves, list), 'O parâmetro chaves da classeAlimentador deve ser do tipo list'
        self.nome = nome
        self.setores = dict()
        ...
        for i in vizinhos:
            if i not in visitados and i in self.setores.keys():
                ...
                self.arvore_nos_de_carga.inserir_ramo(no_insersao.nome, (
                 rnp_insersao,
                 arvore_insersao), no_raiz=rnp_insersao[(1, 0)])
                break
            else:
                continue
        else:
                    ...
                    if setor_1 != setor_2:
                        for chave in self.chaves.values():
                            if chave.n1 in (setor_1, setor_2) and chave.n2 in (setor_1, setor_2):
                                self.trechos[n_1 + n_2] = Trecho(nome=n_1 + n_2, n1=self.nos_de_carga[n_1], n2=self.nos_de_carga[n_2], chave=chave)

                    else:
                        self.trechos[n_1 + n_2] = Trecho(nome=n_1 + n_2, n1=self.nos_de_carga[n_1], n2=self.nos_de_carga[n_2])
    ...
    def inserir_ramo(self, no, poda, no_raiz=None):
        setores, arvore_setores, rnp_setores, nos_de_carga, arvore_nos_de_carga, rnp_nos_de_carga, chaves, trechos = poda
        if no_raiz is None:
...
if __name__ == '__main__':
    ...
    s1 = NoDeCarga(nome='S1', vizinhos=[
     'A2'], potencia=Fasor(real=0.0, imag=0.0, tipo=Fasor.Potencia), chaves=[
     '1'])
    a1 = NoDeCarga(nome='A1', vizinhos=[
     'A2'], potencia=Fasor(real=160000.0, imag=120000.0, tipo=Fasor.Potencia))
    a2 = NoDeCarga(nome='A2', vizinhos=[
     'S1', 'A1', 'A3', 'C1'], potencia=Fasor(real=150000.0, imag=110000.0, tipo=Fasor.Potencia), chaves=[
     '1', '3'])
    a3 = NoDeCarga(nome='A3', vizinhos=[
     'A2', 'B1'], potencia=Fasor(real=100000.0, imag=80000.0, tipo=Fasor.Potencia), chaves=[
     '2'])
    b1 = NoDeCarga(nome='B1', vizinhos=[
     'B2', 'A3'], potencia=Fasor(real=200000.0, imag=140000.0, tipo=Fasor.Potencia), chaves=[
     '2'])
    b2 = NoDeCarga(nome='B2', vizinhos=[
     'B1', 'B3', 'E2'], potencia=Fasor(real=150000.0, imag=110000.0, tipo=Fasor.Potencia), chaves=[
     '4'])
    b3 = NoDeCarga(nome='B3', vizinhos=[
     'B2', 'C3'], potencia=Fasor(real=100000.0, imag=80000.0, tipo=Fasor.Potencia), chaves=[
     '5'])
    c1 = NoDeCarga(nome='C1', vizinhos=[
     'C2', 'C3', 'A2'], potencia=Fasor(real=200000.0, imag=140000.0, tipo=Fasor.Potencia), chaves=[
     '3'])
    c2 = NoDeCarga(nome='C2', vizinhos=[
     'C1'], potencia=Fasor(real=150000.0, imag=110000.0, tipo=Fasor.Potencia))
    c3 = NoDeCarga(nome='C3', vizinhos=[
     'C1', 'E3', 'B3'], potencia=Fasor(real=100000.0, imag=80000.0, tipo=Fasor.Potencia), chaves=[
     '5', '8'])
    s2 = NoDeCarga(nome='S2', vizinhos=[
     'D1'], potencia=Fasor(real=0.0, imag=0.0, tipo=Fasor.Potencia), chaves=[
     '6'])
    d1 = NoDeCarga(nome='D1', vizinhos=[
     'S2', 'D2', 'D3', 'E1'], potencia=Fasor(real=200000.0, imag=160000.0, tipo=Fasor.Potencia), chaves=[
     '6', '7'])
    d2 = NoDeCarga(nome='D2', vizinhos=[
     'D1'], potencia=Fasor(real=90000.0, imag=40000.0, tipo=Fasor.Potencia))
    d3 = NoDeCarga(nome='D3', vizinhos=[
     'D1'], potencia=Fasor(real=100000.0, imag=80000.0, tipo=Fasor.Potencia))
    e1 = NoDeCarga(nome='E1', vizinhos=[
     'E3', 'E2', 'D1'], potencia=Fasor(real=100000.0, imag=40000.0, tipo=Fasor.Potencia), chaves=[
     '7'])
    e2 = NoDeCarga(nome='E2', vizinhos=[
     'E1', 'B2'], potencia=Fasor(real=110000.0, imag=70000.0, tipo=Fasor.Potencia), chaves=[
     '4'])
    e3 = NoDeCarga(nome='E3', vizinhos=[
     'E1', 'C3'], potencia=Fasor(real=150000.0, imag=80000.0, tipo=Fasor.Potencia), chaves=[
     '8'])
    ...
    st1 = Setor(nome='S1', vizinhos=[
     'A'], nos_de_carga=[
     s1])
    stA = Setor(nome='A', vizinhos=[
     'S1', 'B', 'C'], nos_de_carga=[
     a1, a2, a3])
    stB = Setor(nome='B', vizinhos=[
     'A', 'C', 'E'], nos_de_carga=[
     b1, b2, b3])
    stC = Setor(nome='C', vizinhos=[
     'A', 'B', 'E'], nos_de_carga=[
     c1, c2, c3])
    st2 = Setor(nome='S2', vizinhos=[
     'D'], nos_de_carga=[
     s2])
    stD = Setor(nome='D', vizinhos=[
     'S2', 'E'], nos_de_carga=[
     d1, d2, d3])
    stE = Setor(nome='E', vizinhos=[
     'D', 'B', 'C'], nos_de_carga=[
     e1, e2, e3])
    ch1.n1 = st1
    ch1.n2 = stA
    ...
    ch8.n1 = stC
    ch8.n2 = stE
    sub_1_al_1 = Alimentador(nome='S1_AL1', setores=[
     st1, stA, stB, stC], trechos=[
     s1_ch1, ch1_a2, a2_a1,
     a2_a3, a2_ch3, ch3_c1,
     c1_c2, c1_c3, c3_ch5,
    ...
    sub_2_al_1 = Alimentador(nome='S2_AL1', setores=[
     st2, stD, stE], trechos=[
     s2_ch6, ch6_d1, d1_d2,
     d1_d3, d1_ch7, ch7_e1,
     e1_e2, e2_ch4, e1_e3,
     e3_ch8], chaves=[
     ch6, ch7, ch4, ch8])
    t1 = Transformador(nome='S1_T1', tensao_primario=Fasor(mod=69000.0, ang=0.0, tipo=Fasor.Tensao), tensao_secundario=Fasor(mod=13800.0, ang=0.0, tipo=Fasor.Tensao), potencia=Fasor(mod=10000000.0, ang=0.0, tipo=Fasor.Potencia), impedancia=Fasor(real=0.5, imag=0.2, tipo=Fasor.Impedancia))
    t2 = Transformador(nome='S2_T1', tensao_primario=Fasor(mod=69000.0, ang=0.0, tipo=Fasor.Tensao), tensao_secundario=Fasor(mod=13800.0, ang=0.0, tipo=Fasor.Tensao), potencia=Fasor(mod=10000000.0, ang=0.0, tipo=Fasor.Potencia), impedancia=Fasor(real=0.5, imag=0.2, tipo=Fasor.Impedancia))
