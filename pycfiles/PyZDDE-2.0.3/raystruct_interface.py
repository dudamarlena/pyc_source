...
# [GCC 8.4.0]
# Embedded file name: C:\PROGRAMSANDEXPERIMENTS\PYTHON\OPTICS\PyZDDE\pyzdde\arraytrace\raystruct_interface.py
# Compiled at: 2016-09-25 01:34:04
...
import os as _os, sys as _sys, ctypes as _ct, collections as _co
...
class DdeArrayData(_ct.Structure):
    _fields_ = [
     (
      'x', _ct.c_double), ('y', _ct.c_double), ('z', _ct.c_double),
     (
...
def _is64bit():
    ...
    """
    return _sys.maxsize > 2147483647
...
def getRayDataArray(numRays, tType=0, mode=0, startSurf=None, endSurf=-1, **kwargs):
    ...
    """
    fields = {
     'x', 'y', 'z', 'l', 'm', 'n', 'opd', 'intensity', 'Exr', 'Exi',
     'Eyr', 'Eyi', 'Ezr', 'Ezi', 'wave', 'error', 'vigcode', 'want_opd'}
    assert kwargs and set(kwargs).issubset(fields), 'Received one or more unexpected kwargs '
...
def zGetTraceArray(numRays, hx=None, hy=None, px=None, py=None, intensity=None, waveNum=None, mode=0, surf=-1, want_opd=0, timeout=5000):
    ...
    if ret == 0:
        reals = [
         'x', 'y', 'z', 'l', 'm', 'n', 'l2', 'm2', 'n2', 'opd',
         'intensity']
        ints = ['error', 'vigcode']
        for r in reals:
            exec r + ' = [0.0] * numRays' in locals(), d

        for i in ints:
            exec i + ' = [0] * numRays' in locals(), d
...
def zGetTraceDirectArray(numRays, x=None, y=None, z=None, l=None, m=None, n=None, intensity=None, waveNum=None, mode=0, startSurf=0, lastSurf=-1, timeout=5000):
    ...
    """
    rd = getRayDataArray(numRays, tType=1, mode=mode, startSurf=startSurf, endSurf=lastSurf)
...
def zGetPolTraceArray(numRays, hx=None, hy=None, px=None, py=None, Exr=None, Exi=None, Eyr=None, Eyi=None, Ezr=None, Ezi=None, Ex=0, Ey=0, Phax=0, Phay=0, intensity=None, waveNum=None, mode=0, surf=-1, timeout=5000):
    ...
    """
    rd = getRayDataArray(numRays, tType=2, mode=mode, endSurf=surf, x=Ex, y=Ey, z=Phax, l=Phay)
    hx = hx if hx else [0.0] * numRays
    hy = hy if hy else [0.0] * numRays
    px = px if px else [0.0] * numRays
    py = py if py else [0.0] * numRays
    ...
    if ret == 0:
        reals = [
         'intensity', 'Exr', 'Exi', 'Eyr', 'Eyi', 'Ezr', 'Ezi']
        ints = ['error']
        for r in reals:
            exec r + ' = [0.0] * numRays' in locals(), d
...
def zGetPolTraceDirectArray(numRays, x=None, y=None, z=None, l=None, m=None, n=None, Exr=None, Exi=None, Eyr=None, Eyi=None, Ezr=None, Ezi=None, Ex=0, Ey=0, Phax=0, Phay=0, intensity=None, waveNum=None, mode=0, startSurf=0, lastSurf=-1, timeout=5000):
    ...
    """
    rd = getRayDataArray(numRays, tType=3, mode=mode, startSurf=startSurf, endSurf=lastSurf, x=Ex, y=Ey, z=Phax, l=Phay)
    x = x if x else [0.0] * numRays
    y = y if y else [0.0] * numRays
    z = z if z else [0.0] * numRays
    l = l if l else [0.0] * numRays
...
def zGetNSCTraceArray(x=0.0, y=0.0, z=0.0, l=0.0, m=0.0, n=1.0, Exr=0.0, Exi=0.0, Eyr=0.0, Eyi=0.0, Ezr=0.0, Ezi=0.0, intensity=1.0, waveNum=0, surf=1, insideOf=0, usePolar=0, split=0, scatter=0, nMaxSegments=50, timeout=5000):
    ...
    rd = getRayDataArray(numRays=nMaxSegments + 10, x=x, y=y, z=z, l=l, m=m, n=n, opd=nMaxSegments + 5, intensity=intensity, Exr=Exr, Exi=Exi, Eyr=Eyr, Eyi=Eyi, Ezr=Ezr, Ezi=Ezi, wave=waveNum, error=surf, vigcode=vigcode, want_opd=insideOf)
    ret = zArrayTrace(rd, timeout)
    if ret == 0:
        segData = _co.namedtuple('segment', ['segment_level', 'segment_parent',
         ...
         'y', 'z', 'l', 'm', 'n', 'intensity',
         'opl'])
        nNumRaySegments = rd[0].want_opd
        rayData = [''] * nNumRaySegments
        for i in xrange(1, nNumRaySegments + 1):
            rayData[i - 1] = segData(rd[i].wave, rd[i].want_opd, rd[i].vigcode, rd[i].error, rd[i].x, rd[i].y, rd[i].z, rd[i].l, rd[i].m, rd[i].n, rd[i].intensity, rd[i].opd)
