...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_MSU_NASA\VS\PyIOmica GitHub\pyiomica\pyiomica\pyiomica.py
# Compiled at: 2019-07-15 17:50:40
# Size of source mod 2**32: 158017 bytes
"""
...
import matplotlib.patches, matplotlib.collections
from matplotlib import cm
import numpy as np, pandas as pd, networkx as nx, appdirs, zipfile, os, pickle, gzip, copy, multiprocessing, urllib.request, shutil, h5py, pymysql, datetime, json, numba, scipy, scipy.signal, scipy.stats
...
def write(data, fileName, withPKLZextension=True, hdf5fileName=None, jsonFormat=False):
    ...
    if hdf5fileName != None and type(data) in [pd.DataFrame, pd.Series]:
        ...
        pd.DataFrame(data=(data.values.copy().astype(float)), index=(data.index),
        ...
        if hdf5fileName != None and type(data) is np.ndarray:
            createDirectories(hdf5fileName)
            hdf5file = h5py.File(hdf5fileName, 'a')
            key = 'arrays/' + fileName.split('/')[(-1)]
            data = data.astype(float)
            if key not in hdf5file:
                hdf5file.create_dataset(key, data=data, maxshape=(tuple([None] * len(data.shape))), dtype=(data.dtype), compression='gzip',
                  compression_opts=6)
            else:
...
def read(fileName, withPKLZextension=True, hdf5fileName=None, jsonFormat=False):
    ...
    if hdf5fileName != None:
        if not os.path.isfile(hdf5fileName):
            ...
            key = 'arrays/' + fileName.split('/')[(-1)]
            if key in hdf5file and hdf5file[key].attrs['gtype'] == 'np':
...
def GeneTranslation--- This code section failed: ---
 ...
 L. 884         0  LOAD_FAST                'InputID'
                2  LOAD_CONST               None
                4  COMPARE_OP               !=
                6  POP_JUMP_IF_FALSE    94  'to 94'

 L. 885         8  BUILD_LIST_0          0 
               10  STORE_FAST               'listOfKeysToUse'

 L. 886        12  LOAD_GLOBAL              type
 ...
 L. 889        54  LOAD_FAST                'listOfKeysToUse'
 ...
 L. 891        82  LOAD_FAST                'listOfKeysToUse'
 ...
 L. 895       110  BUILD_MAP_0           0 
              112  STORE_FAST               'returning'

 L. 897   114_116  SETUP_LOOP          412  'to 412'
 ...
 L. 901       176  SETUP_LOOP          300  'to 300'
              178  LOAD_FAST                'InputList'
              180  GET_ITER         
              182  FOR_ITER            298  'to 298'
              184  STORE_FAST               'item'

 L. 902       186  LOAD_GLOBAL              np
 ...
 L. 910       346  BUILD_LIST_0          0 
              348  STORE_FAST               'tempList'

 L. 911       350  SETUP_LOOP          388  'to 388'
              352  LOAD_FAST                'listOfKeysToUse'
              354  GET_ITER         
              356  FOR_ITER            386  'to 386'
              358  STORE_FAST               'key'

 L. 912       360  LOAD_FAST                'tempList'
...
def ExportEnrichmentReport(data, AppendString='', OutputDirectory=None):
 ...
 L.1365         0  BUILD_MAP_0           0 
                2  STORE_FAST               'returning'

 L.1367         4  LOAD_GLOBAL              type
                6  LOAD_FAST                'data'
 ...
 L.1383       196  SETUP_LOOP          424  'to 424'
 ...
 L.1384       214  SETUP_LOOP          272  'to 272'
 ...
 L.1387       292  SETUP_LOOP          434  'to 434'
 ...
 L.1389       310  SETUP_LOOP          418  'to 418'
 ...
 L.1390       332  SETUP_LOOP          412  'to 412'
 ...
 L.1395       434  LOAD_FAST                'returning'
              436  RETURN_VALUE     
               -1  RETURN_LAST      
...
def getSpikesCutoffs(df_data, p_cutoff, NumberOfRandomSamples=1000):
    ...
    df_data_random = removeConstantSignalsDataframe(df_data_random, 0.0)
    ...
    for i in list(range(data.shape[1] + 1)):
        ...
        if len(idata) > 0:
            cutoffs.update({i: (np.quantile(np.max(idata, axis=1), (1.0 - p_cutoff), interpolation='lower'),
...
def getRandomAutocorrelations(df_data, NumberOfRandomSamples=100000, NumberOfCPUs=4):
...
def getRandomPeriodograms(df_data, NumberOfRandomSamples=100000, NumberOfCPUs=4):
...
def get_optimal_number_clusters_from_linkage_Elbow(Y):
    ...
    """
    if len(Y) >= 5:
        return np.diff(np.array([[nc, Y[(-nc + 1)][2]] for nc in range(2, min(50, len(Y)))]).T[1], 2).argmax() + 1
    return 1
...
def makeClusteringObject(df_data, df_data_autocorr, significance='Elbow'):
    ...
    def getSubgroups(df_data, metric, significance):
        ...
        return (
         {cluster:df_data.index[leaves][(clusters == cluster)] for cluster in np.unique(clusters)}, Y)
    ...
    for group in sorted([item for item in list(ClusteringObject.keys()) if not item == 'linkage']):
        for subgroup in sorted([item for item in list(ClusteringObject[group].keys()) if not item == 'linkage']):
            ...
            if includeData == True and includeAutocorr == True:
                ...
                if includeData == True and includeAutocorr == False:
                    df = df_data
                else:
                    if includeData == False and includeAutocorr == True:
                        df = df_dataAutocorr
                    else:
...
def addVisibilityGraph(data, times, dataName='G1S1', coords=[0.05, 0.95, 0.05, 0.95], numberOfVGs=1, groups_ac_colors=['b'], fig=None, numberOfCommunities=6, printCommunities=False, fontsize=None, nodesize=None, level=0.55, commLineWidth=0.5, lineWidth=1.0, withLabel=True, withTitle=False, layout='circle', radius=0.07, noplot=False):
    ...
    if fontsize is None:
        fontsize = 52.0 / (numberOfVGs + 5.0)
    if nodesize is None:
        nodesize = 390.0 / (numberOfVGs + 5.0)
    x1, x2, y1, y2 = coords
    ...
    if layout == 'line':
        for edge in graph_nx.edges:
            ...
            if edge in shortest_path_edges:
                axisVG.add_artist(matplotlib.patches.Wedge(((l + r) / 2.0), (0.5 * np.sqrt((l - r)[0] * (l - r)[0] + (l - r)[1] * (l - r)[1])), 0, 180, fill=False, edgecolor='y', linewidth=(1.5 * lineWidth), alpha=0.7, width=0.001))
            axisVG.add_artist(matplotlib.patches.Wedge(((l + r) / 2.0), (0.5 * np.sqrt((l - r)[0] * (l - r)[0] + (l - r)[1] * (l - r)[1])), 0, 180, fill=False, edgecolor='k', linewidth=(0.5 * lineWidth), alpha=0.7, width=0.001))

        nx.draw_networkx(graph_nx, pos=pos, ax=axisVG, node_color='y', edge_color='y', node_size=(nodesize * 1.7), width=0.0, nodelist=shortest_path, edgelist=shortest_path_edges, with_labels=False)
        nx.draw_networkx(graph_nx, pos=pos, ax=axisVG, node_color=data, cmap=cmap, alpha=1.0, font_size=fontsize, width=0.0, font_color='k', node_size=nodesize)
    else:
    ...
    X, Y = np.meshgrid(np.arange(xmin, xmax, (xmax - xmin) / 300.0), np.arange(ymin, ymax, (ymax - ymin) / 300.0))

    def smooth(Z, N=7.0):
        for ix in range(1, Z.shape[0] - 1, 1):
            Z[ix] = ((N - 1.0) * Z[ix] + (Z[(ix - 1)] + Z[(ix + 1)]) / 2.0) / N

        return Z

    for icommunity, community in enumerate(communities):
        Z = np.exp(X ** 2 - Y ** 2) * 0.0
        ...
        for i in range(len(community) - 1):
            ...
            for j in range(-2, 32):
                pm = p1 + (p2 - p1) * float(j) / 30.0
                Z[np.where((X - pm[0]) ** 2 + (Y - pm[1]) ** 2 <= radius ** 2)] = 1.0
    ...
    if layout == 'line':
        axisVG.set_xlim(-0.1, 1.0)
        axisVG.set_ylim(-0.1, 1.0)
...
def makeDendrogramHeatmap(ClusteringObject, saveDir, dataName, AutocorrNotPeriodogr=True, textScale=1.0):
    ...
    def addAutocorrelationDendrogramAndHeatmap(ClusteringObject, groupColors, fig, AutocorrNotPeriodogr=AutocorrNotPeriodogr):
        ...
        hierarchy.set_link_color_palette(groupColors[:n_clusters])
        ...
        for i in range(n_clusters - 1):
            posB = cluster_line_positions[(i + 1)] * (axisDendro.get_ylim()[1] / len(Z_ac['leaves'])) - 0.0
            axisDendro.plot([posA, axisDendro.get_xlim()[1]], [posB, posB], '--', color='black', linewidth=1.0)

        axisDendro.plot([posA, axisDendro.get_xlim()[1]], [0.0, 0.0], '--', color='k', linewidth=1.0)
        axisDendro.plot([posA, axisDendro.get_xlim()[1]], [-0.0 + axisDendro.get_ylim()[1], -0.0 + axisDendro.get_ylim()[1]], '--', color='k', linewidth=1.0)
        axisMatrixAC = fig.add_axes([0.8500000000000001, 0.1, 0.105, 0.8])
        cmap = plt.cm.bwr
        ...
        axisColorAC = fig.add_axes([0.9650000000000001, 0.55, 0.01, 0.35])
    ...
    def addGroupDendrogramAndShowSubgroups(ClusteringObject, groupSize, bottom, top, group, groupColors, fig):
        ...
        hierarchy.set_link_color_palette([matplotlib.colors.rgb2hex(rgb[:3]) for rgb in cm.nipy_spectral(np.linspace(0, 0.5, n_clusters + 1))])
        origLineWidth = matplotlib.rcParams['lines.linewidth']
        matplotlib.rcParams['lines.linewidth'] = 0.5
        ...
        cluster_line_positions = np.where(clusters - np.roll(clusters, 1) != 0)[0]
        for i in range(n_clusters - 1):
            posB = cluster_line_positions[(i + 1)] * (axisDendro.get_ylim()[1] / len(Z['leaves'])) - 0.0
            axisDendro.plot([posA, axisDendro.get_xlim()[1]], [posB, posB], '--', color='black', linewidth=1.0)

        axisDendro.plot([posA, axisDendro.get_xlim()[1]], [0.0, 0.0], '--', color='k', linewidth=1.0)
        axisDendro.plot([posA, axisDendro.get_xlim()[1]], [-0.0 + axisDendro.get_ylim()[1], -0.0 + axisDendro.get_ylim()[1]], '--', color='k', linewidth=1.0)
        axisDendro.text((axisDendro.get_xlim()[0]), (0.5 * axisDendro.get_ylim()[1]), ('G%s:' % group + str(groupSize)),
          fontsize=(14 * textScale)).set_path_effects([path_effects.Stroke(linewidth=1, foreground=(groupColors[(group - 1)])), path_effects.Normal()])
        return (
         n_clusters, clusters, cluster_line_positions)
    ...
    def addGroupHeatmapAndColorbar(data_loc, n_clusters, clusters, cluster_line_positions, bottom, top, group, groupColors, fig):
        ...
        def add_label(pos, labelText):
            return axisMatrix.text((-1.0), pos, labelText, ha='right', va='center', fontsize=(12.0 * textScale)).set_path_effects([path_effects.Stroke(linewidth=0.4, foreground=(groupColors[(group - 1)])), path_effects.Normal()])

        order = clusters[np.sort(np.unique(clusters, return_index=True)[1])] - 1
        for i in range(n_clusters - 1):
            if len(data_loc[(clusters == i + 1)]) >= 5.0:
                try:
        ...
        if len(data_loc[(clusters == n_clusters)]) >= 5.0:
        ...
        axisColor = fig.add_axes([0.535, current_bottom + 0.01, 0.01, max(0.01, current_top - current_bottom - 0.02)])
    ...
    for indVG, (dataVG, dataNameVG) in enumerate(zip(data_list, data_names_list)):
        ...
        height = min((y_max - y_min) / numberOfVGs, (x_max - x_min) * 1.5)
        x_displacement = (x_max - x_min - height / 1.5) * 0.5
        y_displacement = (y_max - y_min - numberOfVGs * height) / numberOfVGs
        coords = [
         x_min + x_displacement, x_min + x_displacement + height / 1.5, y_min + indVG * height + (0.5 + indVG) * y_displacement, y_min + (indVG + 1) * height + (0.5 + indVG) * y_displacement]
...
def PlotVisibilityGraph(A, data, times, fileName, id):
    ...
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            if i > j and A[(i, j)] == 1:
                ax.annotate(s='', xy=(times[i], data[i]), xytext=(times[j], data[j]), arrowprops=dict(arrowstyle='<->', shrinkA=0, shrinkB=0, linestyle='--'))
    ...
    for i in range(A.shape[0]):
        for j in range(A.shape[1]):
            if i > j and A[(i, j)] == 1:
                ...
                ax.annotate(s='', xy=(times[i], level), xytext=(times[j], level), arrowprops=dict(arrowstyle='<->', shrinkA=0, shrinkB=0, linestyle='--'))
...
def filterOutFractionZeroSignalsDataframe(df, max_fraction_of_allowed_zeros):
   ...
   """
    print('Filtering out low-quality signals (with more than %s%% missing points)...' % (100.0 * (1.0 - max_fraction_of_allowed_zeros)))
    init = df.shape[0]
...
def compareTwoTimeSeriesDataframe(df1, df2, function=np.subtract, compareAllLevelsInIndex=True, mergeFunction=np.mean):
    ...
    index = pd.MultiIndex.from_tuples((list(set(df1_grouped.index.values).intersection(set(df2_grouped.index.values)))), names=(df1_grouped.index.names))
...
def timeSeriesClassification(df_data, dataName, saveDir, hdf5fileName=None, p_cutoff=0.05, NumberOfRandomSamples=100000, NumberOfCPUs=4, frequencyBasedClassification=False, calculateAutocorrelations=False, calculatePeriodograms=False):
    ...
    df_data = filterOutFractionZeroSignalsDataframe(df_data, 0.75)
    df_data = tagMissingValuesDataframe(df_data)
    df_data = tagLowValuesDataframe(df_data, 1.0, 1.0)
    df_data = removeConstantSignalsDataframe(df_data, 0.0)
    ...
    if frequencyBasedClassification:
        ...
        QP = [1.0]
        QP.extend([np.quantile((df_randomClassifier.values.T[i]), (1.0 - p_cutoff), interpolation='lower') for i in range(1, df_classifier.shape[1])])
...
def visualizeTimeSeriesClassification(dataName, saveDir, numberOfLagsToDraw=3, hdf5fileName=None, exportClusteringObjects=False, writeClusteringObjectToBinaries=False, AutocorrNotPeriodogr=True):
    ...
    def internalDraw(className):
        ...
        clusteringObject = makeClusteringObject(df_data_selected, df_classifier_selected, significance='Elbow')
