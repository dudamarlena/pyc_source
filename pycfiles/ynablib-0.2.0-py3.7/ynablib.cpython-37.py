...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/personal/python/ynablib/ynablib/ynablib.py
# Compiled at: 2019-09-08 05:52:41
...
class Ynab:
    __doc__ = 'Models the ynab service.'
    ...
    def budgets(self):
        ...
        if self._budgets is None:
            ...
            if not response.ok:
                self._logger.error('Error retrieving budgets, response was : %s with status code : %s', response.text, response.status_code)
                return []
            self._budgets = [Budget(self, budget) for budget in response.json().get('data', {}).get('budgets', [])]
    ...
    def upload_transactions(self, budget_id, payloads):
        ...
        if not isinstance(payloads, (list, tuple, set)):
            ...
            if not response.ok:
                self._logger.error('Unsuccessful attempt to upload to budget "%s", response was %s with status code %s', budget_id, response.text, response.status_code)
            else:
...
class Budget:
    __doc__ = 'Models the YNAB budget object.'
    ...
    def accounts(self):
        ...
        if self._accounts is None:
            ...
            if not response.ok:
                self._logger.error('Error retrieving accounts, response was : %s with status code : %s', response.text, response.status_code)
                return []
            self._accounts = [Account(self, account) for account in response.json().get('data', {}).get('accounts', [])]
    ...
    def get_account_by_id(self, id_):
        ...
        """
        return next((account for account in self.accounts if account.id == id_), None)
    ...
    def transactions(self):
        ...
        response = self._ynab.session.get(url)
        if not response.ok:
            self._logger.error('Error retrieving transactions, response was : %s with status code : %s', response.text, response.status_code)
            return []
        return [Transaction(self._ynab, transaction) for transaction in response.json().get('data', {}).get('transactions', [])]


class Account:
    __doc__ = 'Models the account of a YNAB Budget.'
...
class Transaction:
    ...
    def account(self):
        if self._account is None:
            self._account = next((budget.get_account_by_id(self._data.get('account_id')) for budget in self._ynab.budgets), None)
