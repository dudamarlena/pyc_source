...
# [GCC 8.4.0]
# Embedded file name: /Users/guillem/repos_and_code/TACA/tests/test_deliver.py
# Compiled at: 2015-04-22 09:13:54
""" Unit tests for the deliver commands """
import mock, os, shutil, tempfile, unittest
...
SAMPLECFG = {'deliver': {'analysispath': '_ROOTDIR_/ANALYSIS', 
...
class TestDeliverer(unittest.TestCase):
    ...
    def setUp(self, dbmock):
        ...
        self.dbmock = dbmock
        self.deliverer = deliver.Deliverer(self.projectid, self.sampleid, rootdir=self.casedir, **SAMPLECFG['deliver'])
    ...
    def create_content(self, parentdir, level=0, folder=0):
        ...
        for nf in xrange(self.nfiles):
            open(os.path.join(parentdir, ('level{}_folder{}_file{}').format(level, folder, nf)), 'w').close()
    ...
    @mock.patch.object(deliver.db.CharonSession, 'sample_get', return_value='mocked return value')
    def test_fetch_sample_db_entry(self, dbmock):
        """ retrieving sample entry from db and caching result """
        self.assertEquals(self.deliverer.sample_entry(), 'mocked return value')
        dbmock.assert_called_with(self.projectid, self.sampleid)
        dbmock.reset_mock()
        self.assertEquals(self.deliverer.sample_entry(), 'mocked return value')
        self.assertFalse(dbmock.called, 'sample_get method should not have been called for cached result')

    def test_update_sample_delivery(self):
        with self.assertRaises(NotImplementedError):
            self.deliverer.update_delivery_status()

    @mock.patch.object(deliver.db.CharonSession, 'project_create', return_value='mocked return value')
    def test_wrap_database_query(self, dbmock):
        self.assertEqual(self.deliverer.wrap_database_query(self.deliverer.dbcon().project_create, 'funarg1', funarg2='funarg2'), 'mocked return value')
        dbmock.assert_called_with('funarg1', funarg2='funarg2')
        dbmock.side_effect = db.CharonError('mocked error')
        with self.assertRaises(deliver.DelivererDatabaseError):
            self.deliverer.wrap_database_query(self.deliverer.dbcon().project_create, 'funarg1', funarg2='funarg2')

    def test_gather_files1(self):
        """ Gather files in the top directory """
        expected = [ ('level0_folder0_file{}').format(n) for n in xrange(self.nfiles)
                   ]
        pattern = SAMPLECFG['deliver']['files_to_deliver'][0]
        self.deliverer.files_to_deliver = [pattern]
        self.assertItemsEqual([ os.path.basename(p) for p, _, _ in self.deliverer.gather_files() ], expected)

    def test_gather_files2(self):
        """ Gather a folder in the top directory """
        expected = [ f for _, _, files in os.walk(os.path.join(self.deliverer.expand_path(self.deliverer.analysispath), 'level0_folder2')) for f in files
                   ]
    ...
    def test_gather_files3(self):
        ...
        expected = [ ('level2_folder{}_file{}').format(m, n) for m in xrange(self.nfolders) for n in xrange(self.nfiles)
                   ]
    ...
    def test_gather_files4(self):
        ...
        expected = [ ('level1_folder{}_file0').format(n) for n in xrange(self.nfolders)
                   ]
    ...
    def test_stage_delivery1(self):
        ...
        """
        gathered_files = (
         os.path.join(self.deliverer.expand_path(self.deliverer.analysispath), 'level0_folder1', 'level1_folder0', 'level2_folder0_file1'),
         os.path.join(self.deliverer.expand_path(self.deliverer.stagingpath), 'level0_folder1_link', 'level1_folder0_link', 'level2_folder0_file1_link'),
        ...
        with mock.patch.object(deliver.Deliverer, 'gather_files', return_value=[gathered_files]):
            self.deliverer.stage_delivery()
            expected = os.path.join(self.deliverer.expand_path(self.deliverer.stagingpath), gathered_files[1])
            self.assertTrue(os.path.exists(expected), 'Expected staged file does not exist')
            self.assertTrue(os.path.islink(expected), 'Staged file is not a link')
            self.assertTrue(os.path.exists(self.deliverer.staging_digestfile()), 'Digestfile does not exist in staging directory')
            with mock.patch.object(deliver.transfer.SymlinkAgent, 'transfer', side_effect=SymlinkError('mocked error')):
...
class TestProjectDeliverer(unittest.TestCase):
    ...
    def setUp(self, dbmock):
        self.casedir = tempfile.mkdtemp(prefix='case_', dir=self.rootdir)
        self.projectid = 'NGIU-P001'
        self.deliverer = deliver.ProjectDeliverer(self.projectid, rootdir=self.casedir, **SAMPLECFG['deliver'])
    ...
    @mock.patch.object(deliver.db.CharonSession, 'project_update', return_value='mocked return value')
    def test_update_delivery_status(self, dbmock):
        """ Updating the delivery status for a project """
        self.assertEquals(self.deliverer.update_delivery_status(), 'mocked return value')
        dbmock.assert_called_with(self.projectid, delivery_status='DELIVERED')
...
class TestSampleDeliverer(unittest.TestCase):
    ...
    def setUp(self, dbmock):
        ...
        self.projectid = 'NGIU-P001'
        self.sampleid = 'NGIU-S001'
        self.deliverer = deliver.SampleDeliverer(self.projectid, self.sampleid, rootdir=self.casedir, **SAMPLECFG['deliver'])
    ...
    @mock.patch.object(deliver.db.CharonSession, 'sample_update', return_value='mocked return value')
    def test_update_delivery_status(self, dbmock):
        """ Updating the delivery status for a sample """
        self.assertEquals(self.deliverer.update_delivery_status(), 'mocked return value')
        dbmock.assert_called_with(self.projectid, self.sampleid, delivery_status='DELIVERED')