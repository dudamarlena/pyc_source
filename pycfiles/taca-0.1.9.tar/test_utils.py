...
# [GCC 8.4.0]
# Embedded file name: /Users/guillem/repos_and_code/TACA/tests/test_utils.py
# Compiled at: 2015-04-22 09:13:54
""" Unit tests for the utils helper functions """
import hashlib, mock, os, shutil, subprocess, tempfile, unittest
...
class TestMisc:
    ...
    def setUpClass(self):
        ...
        self.hashfile_digests = {'SHA256': '4f075ae76b480bb0200dab01cd304f4045e04cd2b73e88b89549e5ac1627f222', 
           'MD5': 'c8498fc299bc3e22690045f1b62ce4e9', 
           'SHA1': '098fb272dfdae2ea1ba57c795dd325fa70e3c3fb'}
...
class TestFilesystem(unittest.TestCase):
    ...
    def test_crete_folder1(self):
        ...
        self.assertTrue(filesystem.create_folder(target_folder), 'A non-existing target folder could not be created')
        self.assertTrue(os.path.exists(target_folder), 'A non-existing target folder was not created             but method returned True')

    def test_crete_folder2(self):
        """ Ensure that an existing folder is detected """
        self.assertTrue(filesystem.create_folder(self.rootdir), 'A pre-existing target folder was not detected')

    def test_crete_folder3(self):
        """ Ensure that a non-existing parent folder is created """
        target_folder = os.path.join(self.rootdir, 'parent-non-existing', 'target-non-existing')
        self.assertTrue(filesystem.create_folder(target_folder), 'A non-existing parent and target folder could not be created')
        self.assertTrue(os.path.exists(target_folder), 'A non-existing parent folder was not created             but method returned True')
    ...
    def test_crete_folder3(self):
        ...
        with mock.patch.object(filesystem.os, 'makedirs', side_effect=OSError):
            self.assertFalse(filesystem.create_folder(os.path.join(self.rootdir, 'target-non-existing')), 'A raised exception was not handled properly')
...
class TestTransferAgent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.agent = transfer.TransferAgent(src_path=self.rootdir, dest_path=self.destdir)
    ...
    def test_transfer_validate_src_path(self):
        ...
        with self.assertRaises(transfer.TransferError):
            self.agent.validate_src_path()
        self.agent.src_path = os.path.join(self.rootdir, 'this-file-does-not-exist')
        with self.assertRaises(transfer.TransferError):
            self.agent.validate_src_path()
        return
...
class TestSymlinkAgent(unittest.TestCase):
    ...
    def setUp(self):
        self.targetdir = tempfile.mkdtemp(prefix='test_taca_filesystem_symlink_dest')
    ...
    def test_symlink_file2(self):
        ...
        target = os.path.join(self.targetdir, 'these', 'folders', 'should', 'be', 'created')
        self.assertTrue(transfer.SymlinkAgent(src, target).transfer())
    ...
    def test_symlink_file4(self):
        ...
        target = os.path.join(self.targetdir, os.path.basename(src))
        open(target, 'w').close()
        self.assertFalse(transfer.SymlinkAgent(src, target, overwrite=False).transfer())
    ...
    def test_symlink_file6(self):
        ...
        open(target, 'w').close()
        with mock.patch.object(transfer.os, 'unlink', side_effect=OSError('Mocked error')):
    ...
    def test_symlink_folder6(self):
        ...
        with mock.patch.object(transfer.shutil, 'rmtree', side_effect=OSError('Mocked error')):
            with self.assertRaises(transfer.SymlinkError):
                transfer.SymlinkAgent(src, target).transfer()
    ...
    def test_symlink_folder7(self):
        ...
        src = self.rootdir
        target = os.path.join(self.targetdir, os.path.basename(src))
        with mock.patch.object(transfer.os, 'symlink', side_effect=OSError('Mocked error')):
            with self.assertRaises(transfer.SymlinkError):
                transfer.SymlinkAgent(src, target).transfer()
    ...
    def test_symlink_folder8(self):
        ...
        src = self.rootdir
        target = self.targetdir
        with mock.patch.object(transfer.os.path, 'exists', side_effect=Exception('Mocked error')):
...
class TestRsyncAgent(unittest.TestCase):
    ...
    def setUpClass(self):
        self.rootdir = tempfile.mkdtemp(prefix='test_taca_transfer_src')
        fh, self.testfile = tempfile.mkstemp(prefix='test_taca_transfer_file')
    ...
    def setUp(self):
        self.destdir = tempfile.mkdtemp(prefix='test_taca_transfer_dest')
        self.agent = transfer.RsyncAgent(self.rootdir, dest_path=self.destdir, validate=False)
    ...
    def test_rsync_agent1(self):
        ...
        self.assertEqual(self.destdir, self.agent.remote_path(), 'Destination path was not correct for empty remote user and empty destination host')
        self.agent.remote_host = 'localhost'
        self.assertEqual(('localhost:{}').format(self.destdir), self.agent.remote_path(), 'Destination path was not correct for empty remote user')
        self.agent.remote_user = 'user'
        self.assertEqual(('user@localhost:{}').format(self.destdir), self.agent.remote_path(), 'Destination path was not correct for non-empty remote user')
        self.agent.dest_path = None
        self.assertEqual('user@localhost:', self.agent.remote_path(), 'Destination path was not correct for empty destination path')
        return
    ...
    def test_rsync_agent2(self):
        """ An error thrown by the rsync subprocess should be wrapped and 
            propagated 
        """
        with mock.patch.object(transfer.subprocess, 'check_call', side_effect=subprocess.CalledProcessError(cmd='mocked subprocess', returncode=-1)):
    ...
    def test_rsync_agent3(self):
        ...
        self.assertTrue(self.agent.transfer(), 'transfer a single file failed')
        self.assertTrue(self.validate_files(self.agent.src_path, os.path.join(self.destdir, os.path.basename(self.agent.src_path))), 'test file was not properly transferred')

    def test_rsync_agent4(self):
        """ rsync transfer of a folder """
        self.agent.src_path = os.path.join(self.rootdir, 'folder0')
        self.assertTrue(self.agent.transfer(), 'transfer a folder failed')
        self.assertTrue(self.validate_folders(self.agent.src_path, os.path.join(self.destdir, os.path.basename(self.agent.src_path))), 'folder was not properly transferred')
    ...
    def test_rsync_agent5(self):
        ...
        self.assertTrue(self.agent.transfer(), 'transfer a folder containing a symlink failed')
        self.assertEqual(misc.hashfile(self.testfile, hasher='sha1'), misc.hashfile(os.path.join(self.destdir, 'folder0', 'link1'), hasher='sha1'), 'symlink was not properly transferred')
        return
    ...
    def validate_files(self, src, dst):
        return os.path.exists(src) and os.path.isfile(src) and os.path.exists(dst) and os.path.isfile(dst) and misc.hashfile(src) == misc.hashfile(dst)