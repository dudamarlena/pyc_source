...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pagingish/tests/createdata.py
# Compiled at: 2009-04-23 06:02:05
...
def create_items(dbname='test', num_items=50, force_create=False, verbose=False, model_type='test', items=None):
    ...
    if items is None:
        ...
        for n in xrange(num_items):
            data = {'model_type': model_type, 'url': chars[(n % 10)], 'title': chars[(n % 20)].title()}
            if verbose == True:
                print ' %s - %r' % (n, data)
