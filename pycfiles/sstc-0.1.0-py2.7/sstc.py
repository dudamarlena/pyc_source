...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sstc/sstc.py
# Compiled at: 2015-03-28 13:27:57
import os, time, threading, socket, libtorrent as lt, requests, logging
...
class TorrentClient(object):
    ...
    def __init__(self, download_path='./', port_start=6881, port_ends=6891, interface=None, proxy_type=None, proxy_host=None, proxy_port=None, anonymous_mode=False, download_rate_limit=-1, upload_rate_limit=-1, download_limit=-1, upload_limit=-1, user_agent=None, alert_mask=ALERT_MASK_STORAGE | ALERT_MASK_STATUS | ALERT_MASK_STATS | ALERT_MASK_ERROR):
    ...
    def _add_torrent_content(self, bdecoded, is_paused=False, download_path=None, download_limit=0, upload_limit=0):
        ...
        params = {'save_path': download_path or self.download_path, 
    ...
    def add(self, what, alert_handler=None, *args, **kwargs):
        if what.startswith('magnet:'):
            return self.add_magnet(what, alert_handler=alert_handler, *args, **kwargs)
        if os.path.isfile(what):
            return self.add_torrent(what, alert_handler=alert_handler, *args, **kwargs)
        if what.startswith('http://') or what.startswith('https://'):
            return self.add_url(what, alert_handler=alert_handler, *args, **kwargs)
