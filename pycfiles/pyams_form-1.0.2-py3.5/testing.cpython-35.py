...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_form/testing.py
# Compiled at: 2020-02-23 12:53:51
# Size of source mod 2**32: 20311 bytes
"""PyAMS_form.testing module

This module provides several testing helpers.
"""
import base64, os, pprint, re, sys
...
if sys.argv[(-1)].endswith('/bin/test'):
    ...
    OUTPUT_CHECKER = outputchecker.OutputChecker(patterns=(
     (
      re.compile("u('.*?')"), '\\1'),
     (
      re.compile("b('.*?')"), '\\1'),
    ...
    class TestingFileUploadDataConverter(FileUploadDataConverter):
        __doc__ = 'A special file upload data converter that works with testing.'
        adapts(IBytes, IFileWidget)

        def to_field_value(self, value):
            if value is None or value == '':
                value = self.widget.request.params.get(self.widget.name + '.testing', '')
                encoding = self.widget.request.params.get(self.widget.name + '.encoding', 'plain')
                if encoding == 'base64':
    ...
    class SimpleSecurityPolicy:
        ...
        def checkPermission(self, permission, object):
            ...
            if self.logged_in and permission in self.allowed_permissions:
                return True
            return False
    ...
    def text_of_with_optional_title(node, add_title=False, show_tooltips=False):
        ...
        if isinstance(node, (list, tuple)):
            return '\n'.join(text_of_with_optional_title(child, add_title, show_tooltips) for child in node)
        text = []
        if node is None:
            return
        if node.tag == 'br':
        ...
        if node.tag == 'textarea' and add_title:
            title = node.get('name') or ''
            title += ' '
            text.append(title)
        if node.tag == 'select':
    ...
    class IMySubObject(Interface):
        __doc__ = 'Sub-object interface'
        foofield = Int(title='My foo field', default=1111, max=9999, required=True)
        barfield = Int(title='My dear bar', default=2222, required=False)


    @implementer(IMySubObject)
    class MySubObject:
        __doc__ = 'Sub-object class'
        foofield = FieldProperty(IMySubObject['foofield'])
        barfield = FieldProperty(IMySubObject['barfield'])


    class IMySecond(Interface):
        __doc__ = 'Second interface'
        subfield = Object(title='Second-subobject', schema=IMySubObject)
        moofield = TextLine(title='Something')


    @implementer(IMySecond)
    class MySecond:
        __doc__ = 'Second object class'
        subfield = FieldProperty(IMySecond['subfield'])
        moofield = FieldProperty(IMySecond['moofield'])


    class IMyObject(Interface):
        __doc__ = 'Object interface'
    ...
    class IMyComplexObject(Interface):
        __doc__ = 'Complex object interface'
        subobject = Object(title='my object', schema=IMySecond)
        name = TextLine(title='name')


    class IMySubObjectMulti(Interface):
        __doc__ = 'Multi sub-object interface'
        foofield = Int(title='My foo field', default=None, max=9999, required=True)
        barfield = Int(title='My dear bar', default=2222, required=False)


    @implementer(IMySubObjectMulti)
    class MySubObjectMulti:
        __doc__ = 'Multi sub-object class'
    ...
    class IMyMultiObject(Interface):
        __doc__ = 'Multi-object interface'
        list_of_objects = List(title='My list field', value_type=Object(title='my object widget', schema=IMySubObjectMulti))
        name = TextLine(title='name')


    @implementer(IMyMultiObject)
    class MyMultiObject:
        __doc__ = 'Multi-object class'
    ...
    class IntegrationBase:
        ...
        def __repr__(self):
            ...
            items.sort()
            return '<' + self.__class__.__name__ + '\n  ' + '\n  '.join(['%s: %s' % (key, pprint.pformat(value)) for key, value in items]) + '>'


    class IObjectWidgetSingleSubIntegration(Interface):
        __doc__ = 'Sub-integration interface'
        single_int = Int(title=INT_LABEL)
        single_bool = Bool(title=BOOL_LABEL)
        single_choice = Choice(title=CHOICE_LABEL, values=('one', 'two', 'three'))
        single_choice_opt = Choice(title=CHOICE_OPT_LABEL, values=('four', 'five',
                                                                   'six'), required=False)
    ...
    class ObjectWidgetSingleSubIntegration(IntegrationBase):
        ...
        single_choice_opt = FieldProperty(IObjectWidgetSingleSubIntegration['single_choice_opt'])
        single_textline = FieldProperty(IObjectWidgetSingleSubIntegration['single_textline'])
        single_date = FieldProperty(IObjectWidgetSingleSubIntegration['single_date'])
        single_readonly = FieldProperty(IObjectWidgetSingleSubIntegration['single_readonly'])


    class IObjectWidgetSingleIntegration(Interface):
        __doc__ = 'Single integration interface'
        subobj = Object(title=OBJECT_LABEL, schema=IObjectWidgetSingleSubIntegration)


    @implementer(IObjectWidgetSingleIntegration)
    class ObjectWidgetSingleIntegration:
        __doc__ = 'Single integration class'
        subobj = FieldProperty(IObjectWidgetSingleIntegration['subobj'])


    class IObjectWidgetMultiSubIntegration(Interface):
        __doc__ = 'Multi sub-integration interface'
        multi_int = Int(title=INT_LABEL)
        multi_bool = Bool(title=BOOL_LABEL)
        multi_choice = Choice(title=CHOICE_LABEL, values=('one', 'two', 'three'))
        multi_choice_opt = Choice(title=CHOICE_OPT_LABEL, values=('four', 'five', 'six'), required=False)
    ...
    class ObjectWidgetMultiSubIntegration(IntegrationBase):
        __doc__ = 'Multi sub-integration class'
        ...
        multi_choice_opt = FieldProperty(IObjectWidgetMultiSubIntegration['multi_choice_opt'])
        multi_textline = FieldProperty(IObjectWidgetMultiSubIntegration['multi_textline'])
        multi_date = FieldProperty(IObjectWidgetMultiSubIntegration['multi_date'])


    class IObjectWidgetMultiIntegration(Interface):
        __doc__ = 'Multi-integration interface'
        subobj = Object(title=OBJECT_LABEL, schema=IObjectWidgetMultiSubIntegration)


    @implementer(IObjectWidgetMultiIntegration)
    class ObjectWidgetMultiIntegration:
        __doc__ = 'Multi-integration class'
        subobj = FieldProperty(IObjectWidgetMultiIntegration['subobj'])


    class IMultiWidgetListIntegration(Interface):
        __doc__ = 'Multi-list interface'
        list_of_int = List(title='ListOfInt label', value_type=Int(title=INT_LABEL))
        list_of_bool = List(title='ListOfBool label', value_type=Bool(title=BOOL_LABEL))
        list_of_choice = List(title='ListOfChoice label', value_type=Choice(title=CHOICE_LABEL, values=('one',
                                                                                                        'two',
                                                                                                        'three')))
        list_of_textline = List(title='ListOfTextLine label', value_type=TextLine(title=TEXTLINE_LABEL))
        list_of_date = List(title='ListOfDate label', value_type=Date(title=DATE_LABEL))
    ...
    class IMultiWidgetDictIntegration(Interface):
        __doc__ = 'Multi-dict interface'
        dict_of_int = Dict(title='DictOfInt label', key_type=Int(title='Int key'), value_type=Int(title=INT_LABEL))
        dict_of_bool = Dict(title='DictOfBool label', key_type=Bool(title='Bool key'), value_type=Bool(title=BOOL_LABEL))
        dict_of_choice = Dict(title='DictOfChoice label', key_type=Choice(title='Choice key', values=('key1',
                                                                                                      'key2',
                                                                                                      'key3')), value_type=Choice(title=CHOICE_LABEL, values=('one',
                                                                                                                                                              'two',
                                                                                                                                                              'three')))
        dict_of_textline = Dict(title='DictOfTextLine label', key_type=TextLine(title='TextLine key'), value_type=TextLine(title=TEXTLINE_LABEL))
        dict_of_date = Dict(title='DictOfDate label', key_type=Date(title='Date key'), value_type=Date(title=DATE_LABEL))
    ...
    class IAJAXTestContent(Interface):
        __doc__ = 'AJAX test content interface'
        name = TextLine(title='Name')
    ...
    class ITestAddForm(IAJAXForm):
        __doc__ = 'Test add form marker interface'


    @ajax_form_config(name='test-add-form.html', ajax_implements=ITestAddForm, ajax_method=None, ajax_xhr=None)
    class TestAddForm(AddForm):
        __doc__ = 'Test add form'
    ...
    class ITestEditForm(IAJAXForm):
        __doc__ = 'Test edit form marker interface'


    @ajax_form_config(name='test-edit-form.html', context=IAJAXTestContent, ajax_implements=ITestEditForm, ajax_method=None, ajax_xhr=None)
    class TestEditForm(EditForm):
        __doc__ = 'Test edit form'
        fields = field.Fields(IAJAXTestContent)