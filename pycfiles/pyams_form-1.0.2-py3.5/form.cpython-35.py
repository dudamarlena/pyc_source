...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_form/form.py
# Compiled at: 2020-02-23 12:53:51
# Size of source mod 2**32: 18734 bytes
...
from pyams_form.interfaces.form import IActionForm, IAddForm, IButtonForm, IDisplayForm, IEditForm, IFieldsForm, IForm, IFormAware, IFormContextPermissionChecker, IGroup, IGroupManager, IHandlerForm, IInnerSubForm, IInnerTabForm, IInputForm
...
def apply_changes(form, content, data):
    ...
    for name, field in form.fields.items():
        ...
        if new_value is NOT_CHANGED:
            pass
        elif changed_field(field.field, new_value, context=content):
...
def handle_action_error(event):
    ...
    error_view = registry.getMultiAdapter((event.error.error, action.request, widget,
     ...
     form.get_content()), IErrorViewSnippet)
    error_view.update()
    if widget:
...
class BaseForm(ContextRequestAdapter):
    ...
    def required_info(self):
        ...
        if self.label_required is not None and self.widgets is not None and self.widgets.has_required_fields:
    ...
    def subforms(self):
        ...
        registry = self.request.registry
        return sorted((adapter for name, adapter in registry.getAdapters((self.context, self.request, self), IInnerSubForm)), key=get_form_weight)

    @reify
    def tabforms(self):
        """Get list of internal tab-forms"""
        registry = self.request.registry
        return sorted((adapter for name, adapter in registry.getAdapters((self.context, self.request, self), IInnerTabForm)), key=get_form_weight)
    ...
    def json(self):
        ...
        data = {'errors': [error.message for error in self.widgets.errors or [] if error.field is None], 
...
class Form(BaseForm):
    ...
    def get_form_options(self):
        ...
        if self.ajax_form_options:
            return json.dumps(self.ajax_form_options)


@implementer(IAddForm)
class AddForm(Form):
    __doc__ = 'A field and button based add form.'
    ignore_context = True
...
class EditForm(Form):
    __doc__ = 'A simple edit form with an apply button.'
    ...
    def apply_changes(self, data):
        ...
        if changes:
            for content_hash, content_changes in changed_contents.items():
                ...
                self.request.registry.notify(ObjectModifiedEvent(contents[content_hash], *descriptions))

        return changes


class FormSelector:
    __doc__ = 'Form event selector\n\n    This predicate can be used by subscribers to filter form events\n    '
