...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyams_form/util.py
...
def create_css_id(name):
    """Create CSS id"""
    return str(''.join([char if char in _ACCEPTABLE_CHARS else hexlify(char.encode('utf-8')).decode() for char in name]))
...
def get_specification(spec, force=False):
    ...
    """
    if force or spec is not None and not ISpecification.providedBy(spec) and not isinstance(spec, class_types):
        iface_name = 'IGeneratedForObject_%i' % id(spec)
        existing_interfaces = [i for i in directlyProvidedBy(spec) if i.__name__ == iface_name]
...
class Manager(OrderedDict):
    __doc__ = 'Non-persistent IManager implementation.'
...
class SelectionManager(Manager):
    __doc__ = 'Non-persistent ISelectionManager implementation.'
    ...
    def omit(self, *names):
        """See interfaces.ISelectionManager"""
        return self.__class__(*[item for name, item in self.items() if name not in names])
