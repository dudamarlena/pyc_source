...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\AngularJSLibrary\__init__.py
# Compiled at: 2019-07-31 21:09:47
...
class ngElementFinder(ElementFinder):
    ...
    def find(self, locator, tag=None, first_only=True, required=True, parent=None):
        ...
        if not self.ignore_implicit_angular_wait:
            try:
                WebDriverWait(self._sldriver, timeout, 0.2).until_not(lambda x: self._sldriver.execute_script(js_wait_for_angular, self.root_selector))
...
class AngularJSLibrary:
    ...
    def wait_for_angular(self, timeout=None, error=None):
        ...
        timeout = timestr_to_secs(timeout)
        error = error or 'Timed out waiting for AngularJSLibrary to synchronize with ' + 'the page after specified timeout.'
        try:
            WebDriverWait(self._sldriver, timeout, 0.2).until_not(lambda x: self._sldriver.execute_script(js_wait_for_angular, self.root_selector))
        except TimeoutException:
            pass
    ...
    def set_ignore_implicit_angular_wait(self, ignore):
        ...
        if not is_boolean(ignore):
            raise TypeError('Ignore must be boolean, got %s.' % type_name(ignore))
    ...
    def _find_by_ng_repeater(self, parent, criteria, tag, constraints):
        ...
        elements = self._sldriver.execute_script(js_repeater_min + 'var ng_repeat = new byRepeaterInner(true);' + 'return ng_repeat%s.getElements();' % js_repeater_str, arg0, arg1)
        if len(elements):
            return ElementFinder(self._s2l)._filter_elements(elements, tag, constraints)
        raise ValueError("Element locator '" + criteria + "' did not match any elements.")
