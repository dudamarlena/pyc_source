...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/txportal/simulator/portald.py
...
from txportal.simulator.handlers import auth_handler, chellenge_handler, base_handler, reqinfo_handler, logout_handler
...
class TpSimWorker(protocol.DatagramProtocol):
    ...
    def __init__(self, config, logger=None):
        ...
        self.ac_handlers = {cmcc.REQ_CHALLENGE: chellenge_handler.ChellengeHandler(self.config, self.logger), 
    ...
    def sendtoPortald(self, msg):
        portal_addr = (
         self.config.get('portal_host', '127.0.0.1'), int(self.config.get('portal_listen', 50100)))
        if self.debug:
