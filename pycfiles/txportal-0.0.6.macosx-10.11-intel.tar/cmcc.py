...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/txportal/packet/cmcc.py
# Compiled at: 2016-03-18 12:19:39
import struct, random, hashlib
from txportal.packet import pktutils
import binascii, six, copy, itertools
...
class Portal(object):
    ...
    def get_text_info(self):
        ...
        长度至少为3字节，但不超过253字节。内容可以为任意字符串，但是不包括字符串结束符‘\x00’。
    ...
    def get_port(self):
        ...
        描述: 本属性只能用在REQ_INFO（Type＝9）和ACK_INFO（Type＝0x0a）报文中，数量不能超过1。
        当是REQ_INFO报文时，长度为2字节。
        当是ACK_INFO报文时，变长，但不能超过253字节。内容为一个字符串（无'\x00'结束符）。
    ...
    def newMessage(typ, userIp, serialNo, reqId, secret, basip=None, chap=False):
        return Portal(type=typ, isChap=chap and 0 or 1, userIp=pktutils.EncodeAddress(userIp), serialNo=serialNo, reqId=reqId, secret=six.b(secret))

    @staticmethod
    def newReqChallenge(userIp, secret, basip=None, serialNo=None, chap=True):
        """0x01"""
        pkt = Portal.newMessage(REQ_CHALLENGE, userIp, serialNo or CurrentSN(), 0, secret, chap=chap)
        return pkt

    @staticmethod
    def newReqAuth(userIp, username, password, reqId, challenge, secret, basip=None, serialNo=None, chap=True):
        """0x03"""
...
if __name__ == '__main__':
    pkt = Portal(packet=b'\x02\x02\x00\x00\x1b@\x00\x00\xac\x10\x01*\x00\x00\x01\x01\x06\xf2\xc3\xa0\xfe\xf1\x0c8\xea\xfb\x9aM\x86\xb4E\x06&\x06S\xf7O\xf6')
    print repr(pkt)