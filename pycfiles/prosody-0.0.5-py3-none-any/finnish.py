...
# Embedded file name: /Users/ryan/DH/prosodic/dicts/fi/finnish.py
# Compiled at: 2017-04-15 22:50:35
...
def get(token, config={}):
    ...
    for ij in range(len(Annotation.syllables)):
        ...
        for x in [onsetStr, nucleusStr, codaStr]:
            ...
            if not x:
                continue
            if x not in orth2phon:
                for y in x:
                    y = y.strip()
                    if not y:
                        continue
                    if y not in orth2phon:
                        wordbroken = True
                    else:
