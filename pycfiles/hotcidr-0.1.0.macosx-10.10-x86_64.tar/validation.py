...
# [GCC 8.4.0]
# Embedded file name: /Users/skemper/workspace/hotcidr/venv/lib/python2.7/site-packages/hotcidr/validation.py
# Compiled at: 2015-05-15 12:08:18
import difflib, inflect, itertools, logging, netaddr, os, re, toposort, yaml, hotcidr.state
...
def validate_aws_instance_id(self):
    for name in self.boxes.keys():
        if not re.match('^i\\-[0-9a-f]{8}$', name):
...
def validate_aws_group_id(self):
    ...
    for group_name, group in self.groups.items():
        if 'id' in group:
            name = group['id']
            if not re.match('^sg\\-[0-9a-f]{8}$', name):
                ...
                if rule['location'] not in valid_groups:
                    try:
                        ...
                        if str(ip.cidr) != rule['location']:
                            self.warn('Location for rule %d in %s will be interpreted as %s' % (
