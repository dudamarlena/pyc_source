...
# [GCC 8.4.0]
# Embedded file name: /Users/skemper/workspace/hotcidr/venv/lib/python2.7/site-packages/hotcidr/apply.py
# Compiled at: 2014-09-11 23:30:12
import boto.ec2, collections, itertools, sys
...
class ModifyRule(Action):
    ...
    def run(self, conn, f):
        ...
        if self.rule.ports:
            ...
            fromport = -1
            toport = -1
        k = {'group_id': util.get_id_for_group(conn, self.group), 
           'ip_protocol': proto, 
           'from_port': fromport, 
           'to_port': toport}
        if util.is_cidr(loc):
...
def changes(actions, unauthorized=0):
    objs = dict(zip([
     ...
     (
      RemoveRule, '%d rule(s) removed')], itertools.repeat(0)))
    r = []
...
def main(git_repo, region_code, vpc_id, aws_key, aws_secret, dry_run, expected_repo=None):
    with fetch.vpc(region_code, vpc_id, aws_key, aws_secret) as (aws_dir):
        with util.repo(git_repo) as (git_dir):
            ...
            conn = util.get_connection(vpc_id, region_code, aws_access_key_id=aws_key, aws_secret_access_key=aws_secret)
            if not conn:
