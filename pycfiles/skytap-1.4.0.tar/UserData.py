...
# [GCC 8.4.0]
# Embedded file name: skytap/models/UserData.py
...
class UserData(SkytapResource):
    ...
    def add(self, key, value):
        ...
        if add_key:
            Utils.info('Adding key "' + key + '" with value "' + value + '"')
            api = ApiClient()
            ...
            Utils.info('Key "' + key + '" with value "' + value + '"already exists.')
            return '{}'
    ...
    def _get_values(self, contents):
        ...
        for i in lines:
            ...
            if len(tokens) < 2:
                continue
            if tokens[0].endswith(':') and '#' not in tokens[0] and len(tokens) > 1 and '#' not in tokens[1]:
                try:
