...
# [GCC 8.4.0]
# Embedded file name: skytap/Labels.py
...
class Labels(SkytapGroup):
    ...
    def __init__(self, labels_json=None, url=None):
        ...
        if not labels_json:
            self.load_list_from_api('/v2/label_categories', Label, {'scope': 'company'})
        else:
            self.load_list_from_json(labels_json, Label, url + '/labels', {'scope': 'company'})
    ...
    def create(self, name, single_value):
        ...
        Utils.info('Creating Label: ' + name + '. Single-value: ' + str(single_value))
        api = ApiClient()
