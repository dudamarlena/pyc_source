...
# [GCC 8.4.0]
# Embedded file name: skytap/models/User.py
...
class User(SkytapResource):
            ...
            if isinstance(self.data['configurations'], list):
                if len(self.data['configurations']) > 0:
                    if isinstance(self.data['configurations'][0], dict):
                        self.data['configurations'] = Environments(self.data['configurations'])
    ...
    def delete(self, transfer_user):
        ...
        Utils.info('Deleting user: ' + str(self.id) + ' (' + self.name + ') and transferring ' + 'resources to user id: ' + str(transfer_user))
        api = ApiClient()
