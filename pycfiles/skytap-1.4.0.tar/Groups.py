...
# [GCC 8.4.0]
# Embedded file name: skytap/Groups.py
...
class Groups(SkytapGroup):
    ...
    def add(self, group, description=''):
        ...
        api = ApiClient()
        data = {'name': group, 'description': description}
        url = self.url + '.json'
        ...
        Utils.warning('Trying to create group (' + group + '), but ' + 'got an unexpected return from Skytap. Response:\n' + response)
        return 0
