...
# [GCC 8.4.0]
# Embedded file name: skytap/framework/ApiClient.py
# Compiled at: 2017-01-04 22:10:48
"""Functions needed to access the skytap api."""
import json, requests, six, time
...
class ApiClient(object):
    ...
    def _check_response(self, resp, attempts=1):
        ...
        if 200 <= resp.status_code < 300:
            return True
        if attempts >= Config.max_http_attempts or resp.status_code != 429 and resp.status_code != 423:
        ...
        if resp.status_code == 423:
            if 'Retry-After' in resp.headers:
                Utils.info('Received HTTP 423. Retry-After set to ' + resp.headers['Retry-After'] + ' sec. Waiting to retry.')
                time.sleep(int(resp.headers['Retry-After']) + 1)
            else:
                Utils.info('Received HTTP 429. Too many requests. Waiting to retry.')
    ...
    def _dict_to_query_params(param_dict):
        ...
        if param_dict is None or len(param_dict) == 0:
            return ''
        param_list = [ param + '=' + (str(value).lower() if type(value) == bool else str(value)) for param, value in six.iteritems(param_dict) if value is not None
                     ]
    ...
    def _rest(self, req, url, params=None, data=None, attempts=0):
        ...
        response = self.cmds[cmd](url, headers=self.headers, auth=self.auth, params=data)
