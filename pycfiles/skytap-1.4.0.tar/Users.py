...
# [GCC 8.4.0]
# Embedded file name: skytap/Users.py
...
class Users(SkytapGroup):
    ...
    def add(self, login_name, email=None):
        ...
        if email is None:
            email = login_name
        api = ApiClient()
        data = {'login_name': login_name, 'email': email}
        ...
        if 'id' in new_user:
            ...
            Utils.warning('Trying to create user (' + login_name + '), but ' + 'got an unexpected return from Skytap. Response:\n' + response)
            return 0
