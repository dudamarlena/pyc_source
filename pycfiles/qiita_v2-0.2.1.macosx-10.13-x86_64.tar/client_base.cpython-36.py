...
# [GCC 8.4.0]
# Embedded file name: /Users/komurasaki/.anyenv/envs/pyenv/versions/3.6.3/lib/python3.6/site-packages/qiita_v2/client_base.py
...
class QiitaClientBase:
    __doc__ = ' Python Client Base for Qiita API v2\n    '
    HOST = 'qiita.com'
    ...
    def header(self):
        ...
        if self.access_token:
            self.HEADER.update({'Authorization': 'Bearer {}'.format(self.access_token)})
        return self.HEADER
    ...
    def post(self, path, params=None, headers=None):
        ...
        >>> now = datetime.today()
        >>> res = client.post('/items',                         params={                             'body': 'python api client test:',                             'coediting': False,                             'gist': False,                             'private': False,                             'tags': [{'name': 'python', 'versions': ['3.4.1']}],                             'title': 'test for python client {}'.format(now),                             'tweet': False,                         })
        >>> res.status
        201
        >>> id = res.to_json()['id']
...
def test(access_token):
    import doctest
    doctest.testmod(extraglobs={'client': QiitaClientBase(access_token=access_token)})
