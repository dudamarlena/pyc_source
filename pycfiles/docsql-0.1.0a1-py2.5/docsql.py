...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/docsql.py
# Compiled at: 2007-10-02 10:51:22
...
class Select(Operation):
    ...
    def _cursor_and_row_cls(self):
        cursor = self()
        row_cls_dict = dict(((column_description[0], _ordered_property(index)) for (index, column_description) in enumerate(cursor.description)))
...
class _MemoizeMetaclass(type):
    ...
    def __new__(cls, name, bases, dict):
        if name != 'MemoizeMixin' and bases[0] is not MemoizeMixin:
            other_bases = tuple((base for base in bases if base is not MemoizeMixin))
