...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tkfontchooser.py
# Compiled at: 2018-08-07 08:54:06
"""
...
__version__ = '2.0.2'
EN = {'Cancel': 'Cancel', 'Bold': 'Bold', 'Italic': 'Italic', 'Underline': 'Underline', 
   'Overstrike': 'Strikethrough'}
FR = {'Cancel': 'Annuler', 'Bold': 'Gras', 'Italic': 'Italique', 'Underline': 'Souligné', 
   'Overstrike': 'Barré'}
IT = {'Cancel': 'Annulla', 'Bold': 'Grassetto', 'Italic': 'Corsivo', 'Underline': 'Sottolineato', 
   'Overstrike': 'Barrato'}
...
class FontChooser(Toplevel):
    ...
    def __init__(self, master, font_dict={}, text='Abcd', title='Font Chooser', **kwargs):
        ...
        b_bold = Checkbutton(options_frame, text=TR['Bold'], command=self.toggle_bold, variable=self.var_bold)
        b_bold.grid(row=0, sticky='w', padx=4, pady=(4, 2))
        self.var_italic = BooleanVar(self, font_dict['slant'] == 'italic')
        b_italic = Checkbutton(options_frame, text=TR['Italic'], command=self.toggle_italic, variable=self.var_italic)
        b_italic.grid(row=1, sticky='w', padx=4, pady=2)
        self.var_underline = BooleanVar(self, font_dict['underline'])
        b_underline = Checkbutton(options_frame, text=TR['Underline'], command=self.toggle_underline, variable=self.var_underline)
        b_underline.grid(row=2, sticky='w', padx=4, pady=2)
        self.var_overstrike = BooleanVar(self, font_dict['overstrike'])
        b_overstrike = Checkbutton(options_frame, text=TR['Overstrike'], variable=self.var_overstrike, command=self.toggle_overstrike)
        b_overstrike.grid(row=3, sticky='w', padx=4, pady=(2, 4))
        self.var_size = StringVar(self)
        self.entry_family = Entry(self, width=max_length, validate='key', validatecommand=(
         self._validate_family, '%d', '%S',
         '%i', '%s', '%V'))
        self.entry_size = Entry(self, width=4, validate='key', textvariable=self.var_size, validatecommand=(
         self._validate_size, '%d', '%P', '%V'))
        self.list_family = Listbox(self, selectmode='browse', listvariable=self.font_family, highlightthickness=0, exportselection=False, width=max_length)
        self.list_size = Listbox(self, selectmode='browse', listvariable=self.font_size, highlightthickness=0, exportselection=False, width=4)
        ...
        self.entry_family.grid(row=0, column=0, sticky='ew', pady=(10, 1), padx=(10,
                                                                                 0))
        self.entry_size.grid(row=0, column=2, sticky='ew', pady=(10, 1), padx=(10,
                                                                               0))
        self.list_family.grid(row=1, column=0, sticky='nsew', pady=(1, 10), padx=(10,
                                                                                  0))
        self.list_size.grid(row=1, column=2, sticky='nsew', pady=(1, 10), padx=(10,
                                                                                0))
        ...
        self.preview.grid(row=2, column=0, columnspan=5, sticky='eswn', padx=10, pady=(0,
                                                                                       10), ipadx=4, ipady=4)
