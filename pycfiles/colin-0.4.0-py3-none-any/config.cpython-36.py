...
# [GCC 8.4.0]
# Embedded file name: /src/colin/core/config/config.py
        ...
        for g in self._get_check_groups(group):
            ...
            for sev, files in iteritems(self.config_dict[g]):
                if not severity or severity == sev:
                    check_files += Config.get_check_files(group=g, names=files,
                      severity=sev)
...
def get_config_file(config=None):
    ...
    if os.path.exists(config_file):
        if os.path.isfile(config_file):
            ...
            return config_file
    logger.warning("Configuration with the name '{}' cannot be found at '{}'.".format(config, config_file))
...
def get_config_directory():
    ...
    :return: str
    """
    local_share = os.path.join(os.path.expanduser('~'), '.local', CONFIG_DIRECTORY)
    ...
    if os.path.isdir(usr_local_share):
        if os.path.exists(usr_local_share):
