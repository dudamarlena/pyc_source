...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-8.11.1-i386/egg/serverdownmacro/macro.py
...
class ServerDownMacro(WikiMacroBase):
    ...
    __module__ = __name__
    default_timeout = IntOption('serverdown', 'default_timeout', default=60, doc='Default refresh timeout for the ServerDown macro')
    default_port = IntOption('serverdown', 'default_port', default=80, doc='Default por for the ServerDown macro')
    ...
    def upgrade_environment(self, db):
        ...
        for tbl in db_default.tables:
            ...
            if tbl.name in old_data:
                ...
                sql = 'INSERT INTO %s (%s) VALUES (%s)' % (tbl.name, (',').join(data[0]), (',').join(['%s'] * len(data[0])))
                for row in data[1]:
                    try:
