...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/kinopoiskpy/kinopoisk/utils.py
# Compiled at: 2019-07-02 23:08:30
...
class Manager(object):
    ...
    def search(self, query):
        ...
        if len(response.history):
            if '/film/' in response.url or '/name/' in response.url:
                instance = self.kinopoisk_object()
                instance.get_source_instance('main_page',
                  instance=instance, content=content, request=(self.request)).parse()
                return [instance]
    ...
    def get_url_with_params(self, query):
        return (
         'http://www.kinopoisk.ru/index.php', {'kp_query': query})
...
class KinopoiskImagesPage(KinopoiskPage):
    ...
    def get(self, page=1):
        ...
        if re.findall('<h1 class="main_title">', content):
            return False
    ...
    def parse(self):
        ...
        for link in links:
            img_id = re.compile('/picture/(\\d+)/').findall(link['href'])
