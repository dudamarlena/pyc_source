...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyclamd/pyclamd.py
# Compiled at: 2017-08-27 17:50:21
"""
pyclamd.py
...
class _ClamdGeneric(object):
    ...
    def scan_file(self, file):
        ...
        """
        if not isstr(file):
            raise AssertionError(('Wrong type for [file], should be a string [was {0}]').format(type(file)))
            try:
                self._init_socket()
            ...
            while result:
                ...
                if len(result) > 0:
                    ...
                    if status == 'ERROR':
                        dr[filename] = (
                         'ERROR', ('{0}').format(reason))
                        return dr
                    if status == 'FOUND':
                        dr[filename] = (
                         'FOUND', ('{0}').format(reason))
    ...
    def scan_stream(self, stream, chunk_size=4096):
        ...
        if sys.version_info[0] <= 2:
            if not (hasattr(stream, 'read') or isinstance(stream, str)):
                raise AssertionError(('Wrong type for [stream], should be str/file-like [was {0}]').format(type(stream)))
            else:
    ...
    def _send_command(self, cmd):
        ...
        terminated strings, as python<->clamd has some problems with \x00x00
        """
        try:
            cmd = str.encode(('n{0}\n').format(cmd))
