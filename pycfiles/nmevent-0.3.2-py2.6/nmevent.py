...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/nmevent.py
# Compiled at: 2010-11-28 11:39:41
...
class InstanceEvent(object):
    ...
    def __call__(self, *args, **keywords):
        ...
        if sender is None:
            if len(args) < 1:
                raise TypeError, 'Unbound event must be called with at least 1 positional argument representing the sender.'
            if type(args[0]) is not self.im_class:
                raise TypeError, 'This unbound event must be called with %s instance as the first argument.' % self.im_class.__name__
            sender = args[0]
            args = args[1:]
