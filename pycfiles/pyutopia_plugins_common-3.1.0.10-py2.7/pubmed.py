...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/utopia/plugins/common/pubmed.py
# Compiled at: 2017-06-19 05:34:47
import re, utopia.citation, utopia.tools.pubmed
            ...
            if pubmed_id is None:
                ...
                if title is not None:
                    ...
                    if len(pubmed_title) > 0:
                        ...
                        if re.sub('[^\\w]+', ' ', title).strip().lower() == re.sub('[^\\w]+', ' ', pubmed_title).strip().lower():
                            matched = True
                        elif document is not None:
                            matches = document.findInContext('', pubmed_title, '')
                            if len(matches) > 0:
                                matched = True
...
class PubMedResolver(utopia.citation.Resolver):
    ...
    def resolve(self, citations, document=None):
        ...
        if pmid is not None and not utopia.citation.has_link(citations, {'type': 'abstract'}, {'whence': 'pubmed'}):
            citation['links'] = [
             {'url': ('http://www.ncbi.nlm.nih.gov/pubmed/{0}').format(pmid), 'mime': 'text/html', 
                'type': 'abstract', 
