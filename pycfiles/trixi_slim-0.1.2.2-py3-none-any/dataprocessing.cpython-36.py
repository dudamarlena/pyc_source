...
# [GCC 8.4.0]
# Embedded file name: /home/jens/repos/trixi/trixi/experiment_browser/dataprocessing.py
# Compiled at: 2019-06-14 09:35:56
# Size of source mod 2**32: 8708 bytes
import os
from collections import defaultdict
import re, colorlover as cl, numpy as np, plotly.graph_objs as go
from flask import Markup
from plotly.offline import plot
from scipy.signal import savgol_filter
from trixi.experiment_browser.experimentreader import ExperimentReader
from trixi.util import Config
IGNORE_KEYS = ('name', 'experiment_dir', 'work_dir', 'config_dir', 'log_dir', 'checkpoint_dir',
               'img_dir', 'plot_dir', 'save_dir', 'result_dir', 'time', 'state')
...
def process_base_dir(base_dir, view_dir='', default_val='-', short_len=25, ignore_keys=IGNORE_KEYS):
    ...
    for exp in exps:
        ...
        state = exp.exp_info.get('state', default_val) if 'state' in exp.exp_info else exp.config.get('state', default_val)
        epoch = exp.exp_info.get('epoch', default_val) if 'epoch' in exp.exp_info else exp.config.get('epoch', default_val)
...
def make_graphs(results, trace_options=None, layout_options=None, color_map=COLORMAP):
    ...
    if layout_options is None:
        layout_options = {'legend': dict(orientation='v',
                     xanchor='left',
                     x=0,
                     yanchor='top',
                     y=(-0.1),
    ...
    for group in sorted(results):
        ...
        for r, result in enumerate(sorted(results[group])):
            ...
            opacity = 0.2 if do_filter else 1.0
            if 'min' in results[group][result] and 'max' in results[group][result]:
                ...
                upper_bound = (go.Scatter)(x=x, y=max_, name=result, legendgroup=result, showlegend=False, mode='lines', 
                 line=dict(width=0), hoverinfo='none', fillcolor=fill_color, **trace_options)
                lower_bound = (go.Scatter)(x=x, y=min_, name=result, legendgroup=result, showlegend=False, mode='lines', 
                 fill='tonexty', line=dict(width=0), hoverinfo='none', fillcolor=fill_color, **trace_options)
                traces.append(upper_bound)
                traces.append(lower_bound)
                traces.append((go.Scatter)(x=x, y=y, opacity=opacity, name=result, legendgroup=result, line=dict(color=(color_map[(r % len(color_map))])), **trace_options))
            else:
                if do_filter:
                    ...
                    traces.append((go.Scatter)(x=x, y=y, opacity=opacity, name=result, legendgroup=result, showlegend=False, line=dict(color=(color_map[(r % len(color_map))])), **trace_options))
                    traces.append((go.Scatter)(x=x, y=filter_(y), name=result, legendgroup=result, line=dict(color=(color_map[(r % len(color_map))])), **trace_options))
                else:
                    traces.append((go.Scatter)(x=x, y=y, opacity=opacity, name=result, legendgroup=result, line=dict(color=(color_map[(r % len(color_map))])), **trace_options))

        trace_counters.append(len(results[group]))
        graphs.append(Markup(plot({'data':traces,  'layout':layout}, output_type='div',
