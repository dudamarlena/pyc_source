...
# [GCC 8.4.0]
# Embedded file name: /home/jens/repos/trixi/trixi/experiment_browser/experimentreader.py
# Compiled at: 2019-04-26 11:22:50
# Size of source mod 2**32: 17447 bytes
import copy, itertools, json, os, time, warnings
...
class ExperimentReader(object):
    ...
    def __init__(self, base_dir, exp_dir='', name=None, decode_config_clean_str=True):
        ...
        if os.path.exists(exp_info_file):
            ...
            self._ExperimentReader__results_dict = None
            self.meta_name = None
    ...
    def get_results(self):
        ...
        """
        if self._ExperimentReader__results_dict is None:
            self._ExperimentReader__results_dict = {}
            results_file = os.path.join(self.result_dir, 'results.json')
            if os.path.exists(results_file):
                try:
                    with open(results_file, 'r') as (f):
                        self._ExperimentReader__results_dict = json.load(f)
                except Exception as e:
                    pass

        return self._ExperimentReader__results_dict
    ...
    def update_meta_info(self, name=None, star=None, ignore=None):
        ...
        meta_dict = {'name':self.meta_name,  'star':self.meta_star, 
...
class CombiExperimentReader(ExperimentReader):
    ...
    def __init__(self, base_dir, exp_dirs=(), name=None, decode_config_clean_str=True):
        ...
        for exp_dir in exp_dirs:
            self.experiments.append(ExperimentReader(base_dir=base_dir, exp_dir=exp_dir, decode_config_clean_str=decode_config_clean_str))
        ...
        self._CombiExperimentReader__results_dict = None
        self.work_dir = None
    ...
    def get_result_log_dict(self):
        ...
        for tag, key_result_dict in results_dict.items():
            for s_key, s_key_result_dict in key_result_dict.items():
                for cnt, val, max_, min_ in zip(s_key_result_dict['counter'], s_key_result_dict['data'], s_key_result_dict['max'], s_key_result_dict['min']):
    ...
    def save(self, target_dir=None):
        ...
        self.elog.save_result(results, 'results', encoder_cls=None, default=_CombiExperimentReader__default)
        self.elog.save_result(results_combi, 'results-combi', encoder_cls=None, default=_CombiExperimentReader__default)
        self.elog.save_result((self.get_result_log_dict()), 'results-log', encoder_cls=None, default=_CombiExperimentReader__default)
        self.elog.text_logger.log_to('\n'.join(['\n'] + [e.work_dir for e in self.experiments]), 'combi_exps')


def group_experiments_by(exps, group_by_list):
    configs_flat = [e.config.flat() for e in exps]
