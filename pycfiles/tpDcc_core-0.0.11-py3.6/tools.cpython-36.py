...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/managers/tools.py
# Compiled at: 2020-04-26 16:42:29
# Size of source mod 2**32: 27689 bytes
"""
...
class ToolsManager(plugin.PluginsManager, object):
    ...
    def register_plugin(self, package_name, package_loaders, environment, root_package_name=None, config_dict=None, load=True):
        """
        ...
        if not package_loaders:
            ...
            if not config_dict:
                config_dict = dict()
            config_dict.update({'join':os.path.join, 
            ...
            for sub_module in loader.walk_packages([package_loader.filename]):
                ...
                for cname, obj in inspect.getmembers(mod, inspect.isclass):
                    if issubclass(obj, self.INTERFACE):
                        ...
                        if not plugin_id:
                            tp.logger.warning('Impossible to register plugin "{}" because its ID is not defined!'.format(plugin_path))
                            continue
                        if not plugin_name:
                            tp.logger.warning('Impossible to register plugin "{}" because its name is not defined!'.format(plugin_path))
                            continue
                        if plugin_id in self._plugins[package_name]:
                            tp.logger.warning('Impossible to register plugin "{}" because its ID "{}" its already defined!'.format(plugin_path, plugin_id))
                            continue
            ...
            if len(plugins_found) > 1:
                tp.logger.warning('Multiple plugins found ({}) in module "{}". Loading first one. {} ...'.format(len(plugins_found), plugin_path, plugins_found[(-1)]))
            ...
            plugin_config = tp.ConfigsMgr().get_config(config_name=plugin_config_name,
              package_name=package_name,
              root_package_name=root_package_name,
              environment=environment,
              config_dict=config_dict,
              extra_data=plugin_config_dict)
            if dcc_loader:
                dcc_path = dcc_loader.fullname
                dcc_config = tp.ConfigsMgr().get_config(config_name=(dcc_path.replace('.', '-')),
                  package_name=package_name,
                  environment=environment,
                  config_dict=config_dict)
            ...
            self._plugins[package_name][plugin_id] = {'name':plugin_name, 
             'icon':plugin_icon, 
             'package_name':package_name, 
    ...
    def reload_plugin(self, plugin_id, debug=False):
        ...
        import_order = ['{}.{}'.format(package_loader.fullname, mod) for mod in plugin_config.data.get('import_order', list())]
        skip_modules = ['{}.{}'.format(package_loader.fullname, mod) for mod in plugin_config.data.get('skip_modules', list())]
        ...
        if dcc_loader:
            ...
            if dcc_config:
                dcc_import_order = ['{}.{}'.format(package_loader.fullname, mod) for mod in dcc_config.data.get('import_order', list())]
                dcc_skip_modules = ['{}.{}'.format(package_loader.fullname, mod) for mod in dcc_config.data.get('skip_modules', list())]
    ...
    def get_plugin_data_from_plugin_instance(self, plugin, as_dict=False, package_name=None):
        ...
        :return: dict
        """
        plugin_data = super(ToolsManager, self).get_plugin_data_from_plugin_instance(plugin=plugin,
          as_dict=as_dict,
          package_name=package_name)
        if plugin_data:
            return plugin_data
    ...
    def load_package_tools(self, package_name, root_package_name=None, tools_to_load=None, dev=True, config_dict=None):
        ...
        for pkg_loaders in tools_to_register.values():
            self.register_plugin(package_name=package_name,
              root_package_name=root_package_name,
              package_loaders=pkg_loaders,
              environment=environment,
              load=True,
              config_dict=config_dict)
    ...
    def get_tool_by_id(self, tool_id, package_name=None, do_reload=False, debug=False, *args, **kwargs):
        ...
        if not package_name:
        ...
        elif package_name not in self._plugins:
            ...
            if tool_config:
                import_order = ['{}.{}'.format(pkg_loader.fullname, mod) for mod in tool_config.data.get('import_order', list())]
                skip_modules = ['{}.{}'.format(pkg_loader.fullname, mod) for mod in tool_config.data.get('skip_modules', list())]
        ...
        if dcc_loader:
            ...
            dcc_import_order = ['{}.{}'.format(pkg_loader.fullname, mod) for mod in dcc_config.data.get('import_order', list())]
            dcc_skip_modules = ['{}.{}'.format(pkg_loader.fullname, mod) for mod in dcc_config.data.get('skip_modules', list())]
            ...
            tool_found or tp.logger.error('Error while launching tool: {}'.format(tool_fullname))
            return
        else:
            ...
            tool_inst = tool_found(self, *args, config=tool_config, settings=tool_settings, **kwargs)
    ...
    def launch_tool_by_id(self, tool_id, package_name=None, do_reload=False, debug=False, *args, **kwargs):
        ...
        tool_inst = (self.get_tool_by_id)(args, tool_id=tool_id, package_name=package_name, do_reload=do_reload, debug=debug, **kwargs)
        if not tool_inst:
            return
    ...
    def get_tool_config(self, tool_id, package_name=None):
        ...
        if not package_name:
            package_name = tool_id.replace('.', '-').split('-')[0]
        if package_name not in self._plugins:
            tp.logger.warning('Impossible to retrieve tool config for "{}" in package "{}"! Package not registered.'.format(tool_id, package_name))
            return
        else:
            if tool_id not in self._plugins[package_name]:
                tp.logger.warning('Impossible to retrieve tool config for "{}" in package "{}"! Tool not found'.format(tool_id, package_name))
                return
