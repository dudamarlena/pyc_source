...
# [GCC 8.4.0]
# Embedded file name: /Users/cbrand/projects/python-sqlalchemy-filterparams/build/lib/sqlalchemy_filterparams/util.py
...
from sqlalchemy import Integer, Date, DateTime, Numeric, Float, Time
...
def parse_datetime(dt_string):
    ...
    if isinstance(dt_string, date) and not isinstance(dt_string, datetime):
        dt_string = datetime(dt_string.year, dt_string.month, dt_string.day)
    return dt_string
...
DEFAULT_CONVERTERS = {Integer: int, 
...
def is_type(column_type, to_check_type):
    return isinstance(column_type, to_check_type) or inspect.isclass(to_check_type) and inspect.isclass(column_type) and issubclass(column_type, to_check_type) or column_type is to_check_type
