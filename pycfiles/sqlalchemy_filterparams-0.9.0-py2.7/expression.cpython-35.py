...
# [GCC 8.4.0]
# Embedded file name: /Users/cbrand/projects/python-sqlalchemy-filterparams/build/lib/sqlalchemy_filterparams/expression.py
# Compiled at: 2016-02-20 19:36:52
...
class ExpressionHandler:
    ...
    def _base_obj(self, name):
        return ExpressionItem(self.lookup[name])

    def get_filter_expression(self, query, name):
        join_path = self.path_to(name)
        query = self._evaluate_join_path(query, join_path)
    ...
    def _evaluate_join_path(self, query, path):
        ...
        for path_item in path:
            ...
            if current_path not in self._join_cache:
                relation = getattr(current_base, path_item, None)
                if not hasattr(relation, 'property') or not hasattr(relation.property, 'mapper') or not hasattr(relation.property.mapper, 'class_'):
                    raise ValueError('The specified join path from the base obj %s has a join configuration in %s which is not a relationship.' % (
