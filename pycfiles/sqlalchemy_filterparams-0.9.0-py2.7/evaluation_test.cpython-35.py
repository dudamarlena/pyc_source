...
# [GCC 8.4.0]
# Embedded file name: /Users/cbrand/projects/python-sqlalchemy-filterparams/build/lib/sqlalchemy_filterparams_tests/evaluation_test.py
# Compiled at: 2016-02-20 18:54:39
# Size of source mod 2**32: 8711 bytes
from expects import *
from datetime import date, datetime
from filterparams.obj import Query, And, Or, Not, Order, BindingOperation
from sqlalchemy_filterparams.expression import ExpressionHandler
from sqlalchemy_filterparams.evaluation import Evaluation
from sqlalchemy_filterparams.query_config import QueryConfig
from sqlalchemy_filterparams_tests.models import Domain, EMail, User
...
class EvaluationTest(BaseDatabaseTest):
    ...
    def setUp(self):
        super().setUp()
        self.session.add(User(name='user', fullname='The User', date_of_birth=date(1985, 10, 26), created_at=datetime(2015, 10, 21)))
        self.session.commit()
        self.sql_query = self.session.query(User)
        self.query_config = QueryConfig()
        self.query_config.model = User
        self.query_config.expressions = ExpressionHandler({'name': User.name, 
         ...
         'mail.domain': {'param': Domain.domain, 
                         'join': ('email', 'domain')}, 
         
         'fullname.not_existing': {'param': 'not_existing', 
                                   'join': ('fullname', )}}, User)
        self.query = Query()
    ...
    def test_filter_and(self):
        ...
        self.query.param_order = And(self.query.get_param('fullname'), self.query.get_param('name'))
        expect(self.evaluated_qry.first()).to(be_none)

    def test_filter_or(self):
        self.query.add('fullname', filter='eq', value='The User')
        self.query.add('name', filter='eq', value='user2')
        self.query.param_order = Or(self.query.get_param('fullname'), self.query.get_param('name'))
        expect(self.evaluated_qry.first()).to_not(be_none)
    ...
    def add_user_order(self):
        self.session.add(User(name='abc', fullname='The User', date_of_birth=date(1985, 10, 26), created_at=datetime(2015, 10, 21)))
        self.session.commit()
    ...
    def test_join(self):
        ...
        user.email = EMail(mail='test@example.com', domain=Domain(domain='example.com'))
        self.session.commit()
    ...
    def test_fullname_not_existing_relationship(self):
        self.query.add('fullname.not_existing', filter='eq', value='test@example.com')
