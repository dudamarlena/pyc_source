...
# [GCC 8.4.0]
# Embedded file name: /Users/cbrand/projects/python-sqlalchemy-filterparams/build/lib/sqlalchemy_filterparams_tests/util_test.py
# Compiled at: 2016-02-20 09:55:18
# Size of source mod 2**32: 2330 bytes
from expects import *
from datetime import datetime, date, time
from decimal import Decimal
from sqlalchemy import Date, DateTime, DECIMAL, Float, Integer, Numeric, Unicode, Time
...
def test_convert_date_unparseable():
    expect(lambda : convert('2016-ab-01', Date)).to(raise_error(ValueError))


def test_convert_datetime():
    expect(convert('2016-01-01 18:00:00', DateTime)).to(equal(datetime(2016, 1, 1, 18, 0, 0)))


def test_convert_datetime_reverse():
    expect(convert('18:00:00 2016-01-01', DateTime)).to(equal(datetime(2016, 1, 1, 18, 0, 0)))


def test_convert_datetime_unparseable():
    expect(lambda : convert('2016-01-01 ab:cd:ef', DateTime)).to(raise_error(ValueError))
...
def test_custom_converter():
    ...
    expect(convert('random', SomeStuff, {SomeStuff: lambda data: '%s stuff' % data})).to(equal('random stuff'))