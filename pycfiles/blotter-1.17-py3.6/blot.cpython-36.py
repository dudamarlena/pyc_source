...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/blotter/blot.py
# Compiled at: 2019-06-26 09:58:22
# Size of source mod 2**32: 5934 bytes
...
class Blotter:
    ...
    def __init__(self, ticker, contract_multiplier=1, tick_value=12.5, tick_size=0.0025):
        self.ticker = ticker
        self.net_position = 0
    ...
    def __str__(self):
        if self.avg_open_price:
            ...
            avg_open_price = 'None'
        return f">{self.__class__.__name__.upper()}|" + f"{self.net_direction.name}|" + f"{abs(self.net_position)}|" + f"{self.ticker}|" + f"{avg_open_price}|" + f"{round(self.total_pnl, 2)}|" + '|'
    ...
    def get_open_positions(self):
        ...
        for t in not_booked:
            ...
            if not position:
                ...
                price = (position.PriceLevel * abs(position.OpenQuantity) + t.PriceLevel * abs(t.OpenQuantity)) / (abs(position.OpenQuantity) + abs(t.OpenQuantity))
    ...
    def close_existing_positions(self, trade):
        ...
        if trade.Booked or not closing_trade:
            ...
            pnl = calc_pnl(closing_trade.OpenQuantity, closing_trade.PriceLevel, trade.OpenQuantity, trade.PriceLevel) * self.contract_multiplier / self.tick_size * self.tick_value
    ...
    def update(self, fill):
        ...
        if LOGGING_ENABLED:
            ...
            if is_closing_trade:
                if abs(self.net_position) < abs(fill.OpenQuantity):
                    self.avg_open_price = fill.PriceLevel
