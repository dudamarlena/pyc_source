...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mapachemon\mapachemon.py
# Compiled at: 2010-07-12 10:05:35
...
import os.path, re, ConfigParser, time
...
class MicroApache:
    ...
    def start(self):
        if not findPidsByName(os.path.split(self.mapache)[1]):
            ...
            Popen(self.mapache, cwd=os.path.split(self.mapache)[0], stdout=self.logfile, stderr=STDOUT)
...
def findPidsByName(processName):
    p = Popen('tasklist /FI "IMAGENAME eq ' + processName + '" /NH /FO CSV', stdout=PIPE, stderr=PIPE)
    if p.stderr.read():
        return []
    pattern = re.compile('^"([^"]*)","([^"]*)')
    nameList = []
