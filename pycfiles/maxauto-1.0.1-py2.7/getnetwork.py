...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/max/monkey/getnetwork.py
# Compiled at: 2019-04-27 11:18:34
"""
Author  : xinxi
    ...
    def real_network(self):
        ...
        try:
            try:
                ...
                total = format(float(updata + downdata) / float(1048576), '.3f')
            ...
            try:
                ...
                for index in pipe.readlines():
                    if 'eth0' in index:
                        down = index.split()[1]
                        send = index.split()[9]
                        total = (int(down) + int(send)) / 1048576
