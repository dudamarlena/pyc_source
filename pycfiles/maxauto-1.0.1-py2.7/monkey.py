...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/max/monkey/monkey.py
...
class Monkey:
    ...
    def start_monkey(self):
        ...
        try:
            self.make_env()
            cmd = ('adb -s {} shell CLASSPATH=/sdcard/monkey.jar:/sdcard/framework.jar exec app_process /system/bin tv.panda.test.monkey.Monkey -p {} --{} --imagepolling --running-minutes {} --throttle {} > {}').format(self.device, self.pkg, self.runmodel, self.runtime, self.throttle, self.monkeylog)
