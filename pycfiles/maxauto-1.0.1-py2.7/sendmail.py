...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/max/report/sendmail.py
# Compiled at: 2019-04-27 11:20:37
"""
@Author  : xinxi
...
class SendMail:
    ...
    def send_mail(self):
        ...
        message['From'] = self._format_addr('发件人<%s>' % self.mail_user)
        subject = 'Android稳定性测试报告'
        message['Subject'] = Header(subject, 'utf-8')
        message.attach(MIMEText(content, 'plain', 'utf-8'))
        att1 = MIMEText(open(self.report_path).read())
        att1['Content-Type'] = 'application/octet-stream'
        att1['Content-Disposition'] = ('attachment; filename="{}"').format(('Android稳定性测试报告.html').encode('gb2312'))
        message.attach(att1)
        try:
            s = smtplib.SMTP()
