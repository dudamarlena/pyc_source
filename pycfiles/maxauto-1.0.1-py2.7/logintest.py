...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/max/logintest/logintest.py
...
class AppiumDriver(object):
    ...
    def start_appium(self):
        ...
        """
        self.kill_appium()
        args = ('appium --log {} --session-override --udid {} -a {} -p {}').format(self.appium_log, self.device_name, self.url, self.port)
        logger.info(('appium启动命令:{}').format(args))
        appium = subprocess.Popen(args, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1, close_fds=True)
        while True:
