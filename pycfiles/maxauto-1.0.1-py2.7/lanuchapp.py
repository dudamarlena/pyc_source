...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/max/lanuchtest/lanuchapp.py
        ...
        try:
            try:
                while self.lanuch_loop > 0:
                    self.clear_app()
                    lanuch_cmd = ('adb -s {} shell am start -W {}/{}').format(self.device_name, self.pck_name, self.lanuch_activity)
                    lanuch_info = subprocess.Popen(lanuch_cmd, shell=True, stdout=subprocess.PIPE).stdout.readlines()
