...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/api.py
# Compiled at: 2016-07-21 17:01:29
"""
...
def _get_page(url):
    ...
    try:
        ...
        raise errors.APIGetError(str(e))
...
def _post_data(data, url):
    ...
    err = None
    try:
        req = requests.post(url, data=data, headers={'User-Agent': config.USER_AGENT})
    except Exception as e:
...
def _get_data_obj(page):
    ...
    try:
        ...
        if 'This website is under heavy load' in page.text:
            raise errors.SiteLoadError('API call failed. ' + 'The site is under heavy load.')
