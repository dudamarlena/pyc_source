...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/post.py
...
def search(query, limit=75):
    ...
    if not limit >= 0:
        limit = 75
    url = config.BASE_URL + 'post/index.json?tags=' + str(query) + '&limit=' + (str(limit) if limit > 0 else '100')
    result = 0
    page = 1
    end = False
    while not end:
...
def from_file(dir, filename):
    ...
    try:
        ...
        raise errors.BadPostError('An error occured loading the ' + "specified file's metadata.")

    try:
        data = json.load(open(dir + '.metadata/' + md5))
    except:
...
class Post(object):
    ...
    def __init__(self, post_id=None, post_data=None):
        ...
        if post_id is not None:
            try:
                data = api._fetch_data(config.BASE_URL + 'post/show.json?id=' + str(post_id))
                for prop in data:
                    self._data[prop] = data[prop]

            except (errors.APIGetError, errors.JSONError):
                raise errors.PostNotFoundError('The requested post could ' + 'not be found.')

        if post_data is not None:
            for prop in post_data:
                self._data[prop] = post_data[prop]
    ...
    def tag_history(self):
        ...
        url = config.BASE_URL + 'post_tag_history/index.json?post_id=' + str(self.id)
        try:
            data = api._fetch_data(url)
    ...
    def flag_history(self):
        ...
        url = config.BASE_URL + 'post_flag_history/index.json?post_id=' + str(self.id)
        try:
            data = api._fetch_data(url)
        except errors.APIGetError:
            return
    ...
    def vote(self, vote):
        ...
        if not config.USERNAME and not config.PASSWORD:
            raise errors.APIUnauthorizedError('You must be logged in to ' + 'vote on posts.')
        data = {'id': str(self.id), 'score': str(vote), 
           'login': str(config.USERNAME), 
           'password_hash': str(config.PASSWORD)}
        send_vote = api._get_data_obj(api._post_data(data, config.BASE_URL + 'post/vote.json'))
        return send_vote
    ...
    def download_metadata(self, dest, comments=False, pretty=False):
        ...
        data['comments'] = []
        if comments:
            for c in reversed(list(self.comments)):
                data['comments'].append(c.dump_data())
    ...
    def download(self, dest='./', name_format='{md5}.{file_ext}', overwrite=False, write_metadata=False):
        ...
        if file and (not os.path.isfile(dest + filename) or overwrite):
            if file.headers['Content-Type'].lower() == 'text/html':
                raise errors.FileDownloadError('An error occured attempting ' + 'to download the image.')
            ...
            if write_metadata:
                self.download_metadata(dest + '.metadata/')
            return True
        return False