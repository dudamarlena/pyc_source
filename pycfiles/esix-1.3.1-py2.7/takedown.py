...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/takedown.py
...
class Takedown(object):
    ...
    def __init__(self, takedown_id=None, takedown_data=None):
        ...
        if takedown_id is not None:
            try:
                data = api._fetch_data(config.BASE_URL + 'takedown/show.json?id=' + str(takedown_id))
                for prop in data:
                    self._data[prop] = data[prop]

            except errors.JSONError:
                raise errors.TakedownNotFoundError('The requested takedown ' + 'could not be found.')

        if takedown_data is not None:
            for prop in takedown_data:
                self._data[prop] = takedown_data[prop]
