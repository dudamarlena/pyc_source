...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/tag.py
...
class Tag(object):
    ...
    def __init__(self, tag_id=None, tag_data=None):
        ...
        if tag_id is not None:
            ...
            if len(tag_list) == 0:
                raise errors.TagNotFoundError('The requested tag ' + 'could not be found.')
            else:
                data = tag_list[0]
                for prop in data:
        ...
        if tag_data is not None:
            for prop in tag_data:
                self._data[prop] = tag_data[prop]
