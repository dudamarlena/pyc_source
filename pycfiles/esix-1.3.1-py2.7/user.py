...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/user.py
...
def login(username, password):
    ...
    """
    result = api._get_data_obj(api._post_data({'name': str(username), 
       'password': str(password)}, config.BASE_URL + 'user/login.json'))
    if 'success' in result and result['success'] in ('failed', False):
        if 'reason' in result:
            raise errors.APILoginError('Login failed: ' + str(result['reason']))
        else:
            raise errors.APILoginError('Login failed, ' + 'incorrect username or password.')
...
class User(object):
    ...
    def __init__(self, user_id=None, user_data=None):
        ...
        if user_id is not None:
            ...
            if len(user_list) == 0:
                raise errors.UserNotFoundError('User ' + str(user_id) + ' not found.')
            else:
                data = user_list[0]
                for prop in data:
        ...
        if user_data is not None:
            for prop in user_data:
                self._data[prop] = user_data[prop]
    ...
    def tag_history(self):
        ...
        url = config.BASE_URL + 'post_tag_history/index.json?' + 'user_id=' + str(self.id)
    ...
    def flag_history(self):
        ...
        url = config.BASE_URL + 'post_flag_history/index.json?' + 'user_id=' + str(self.id)
