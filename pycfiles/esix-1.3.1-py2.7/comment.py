...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/comment.py
...
class Comment(object):
    ...
    def __init__(self, comment_id=None, comment_data=None):
        ...
        if comment_id is not None:
            try:
                data = api._get_data_obj(api._post_data({'id': str(comment_id)}, config.BASE_URL + 'comment/show.json'))
                for prop in data:
                    self._data[prop] = data[prop]

            except (errors.APIPostError, errors.JSONError):
                raise errors.CommentNotFoundError('The requested comment ' + 'could not be found.')

        if comment_data is not None:
            for prop in comment_data:
                self._data[prop] = comment_data[prop]
    ...
    def submit(self):
        ...
        data = {'comment[post_id]': str(self.post_id), 'comment[body]': str(self.body), 
           'login': str(config.USERNAME), 
           'password_hash': str(config.PASSWORD)}
        return api._get_data_obj(api._post_data(data, config.BASE_URL + 'comment/create.json'))
