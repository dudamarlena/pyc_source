...
# [GCC 8.4.0]
# Embedded file name: build/bdist.cygwin-2.4.1-x86_64/egg/esix/ticket.py
...
class Ticket(object):
    ...
    def __init__(self, ticket_id=None, ticket_data=None):
        ...
        if ticket_id is not None:
            try:
                data = api._fetch_data(config.BASE_URL + 'ticket/show.json?id=' + str(ticket_id))
            except (errors.APIGetError, errors.JSONError):
                raise errors.TicketNotFoundError('The requested ticket ' + 'could not be found.')

            for prop in data:
                self._data[prop] = data[prop]

        if ticket_data is not None:
            for prop in ticket_data:
                self._data[prop] = ticket_data[prop]
