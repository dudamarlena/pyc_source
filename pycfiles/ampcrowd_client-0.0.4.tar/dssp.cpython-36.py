...
# Embedded file name: /home/cw12401/code/work/ampal/src/ampal/dssp.py
# Compiled at: 2018-09-27 07:51:17
# Size of source mod 2**32: 7861 bytes
__doc__ = 'This module provides an interface to the program DSSP.\n\nFor more information on DSSP see [4]_.\n\nReferences\n----------\n.. [4] Kabsch W, Sander C (1983) "Dictionary of protein\n   secondary structure: pattern recognition of hydrogen-bonded\n   and geometrical features", Biopolymers, 22, 2577-637.\n'
...
def dssp_available():
    ...
    try:
        ...
        print('DSSP has not been found on your path. If you have already installed DSSP but are unsure how to add it to your path, check out this: https://stackoverflow.com/a/14638025')
...
def find_ss_regions(dssp_residues, loop_assignments=(' ', 'B', 'S', 'T')):
    ...
    for ele in dssp_residues:
        if previous_ele is None:
            ...
            if ele[2] != previous_ele[2]:
                fragments.append(fragment)
                fragment = [ele]
            else:
                if previous_ele[1] in loops:
                    if ele[1] in loops:
                        fragment.append(ele)
                    else:
                        fragments.append(fragment)
...
def tag_dssp_data(assembly, loop_assignments=(' ', 'B', 'S', 'T')):
    ...
    for record in dssp_data:
        ...
        assembly[chid][str(rnum)].tags['dssp_data'] = {'ss_definition':sstype, 
    ...
    for region in ss_regions:
        ...
        if 'ss_regions' not in assembly[chain].tags:
...
def get_ss_regions(assembly, ss_types):
    ...
    if not any(map(lambda x: 'ss_regions' in x.tags, assembly)):
        raise ValueError('This assembly does not have any tagged secondary structure regions. Use `ampal.dssp.tag_dssp_data` to add the tags.')
    fragments = Assembly()
    ...
    if not fragments:
        raise ValueError('No regions matching that secondary structure type have been found. Use standard DSSP labels.')
    return fragments
