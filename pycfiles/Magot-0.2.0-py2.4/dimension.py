...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/magot/dimension.py
# Compiled at: 2006-10-29 14:05:08
...
class StandardRootAccount(StandardAccounts, RootAccount):
    ...
    __module__ = __name__
    accNames = {TYPE_ASSET: 'Asset', TYPE_EXPENSE: 'Expense', TYPE_INCOME: 'Income', TYPE_LIABILITY: 'Liability', TYPE_EQUITY: 'Equity', TYPE_PROFITS: 'Profits & Loss', TYPE_NET_ASSETS: 'Net Assets'}
...
class PropertyScopedStandardAccounts(StandardAccounts):
    ...
    def __init__(self, standardParent, scopingProperty):
        ...
        self.accParents = {TYPE_ASSET: p.asset, TYPE_EXPENSE: p.expense, TYPE_INCOME: p.income, TYPE_LIABILITY: p.liability, TYPE_EQUITY: p.equity, TYPE_PROFITS: p.profits, TYPE_NET_ASSETS: p.netAssets}
...
class HierarchyViewer(object):
    ...
    def groupAccountsByOneDimension(self, dimensions, accounts, root, isFirstLevel=False):
        if not dimensions:
            ...
            r = self.root
            Transaction(self.endYear, 'Profit & Loss', r.equity, root.profits, root.income.balance - root.expense.balance)
            Transaction(self.endYear, 'Net Assets', r.equity, root.netAssets, root.profits.balance + root.equity.balance)
            return
