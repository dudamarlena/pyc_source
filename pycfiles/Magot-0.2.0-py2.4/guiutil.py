...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/magot/guiutil.py
# Compiled at: 2006-05-10 20:18:05
...
class MoneyEditor(gridlib.PyGridCellEditor):
    ...
    def Create(self, parent, id, evtHandler):
        self._tc = masked.NumCtrl(parent, id, fractionWidth=2, autoSize=False, selectOnEntry=False, groupDigits=False)
        self.SetControl(self._tc)
        if evtHandler:
            self._tc.PushEventHandler(evtHandler)
...
class OppositeAccountEditor(gridlib.PyGridCellEditor):
    ...
    def EndEdit(self, row, col, grid):
        ...
        changed = False
        if selectedAcc is not self.oldOppositeAcc:
            changed = True
