...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/magot/model.py
# Compiled at: 2006-11-05 14:26:09
...
class Entry(elements.Element):
    ...
    def __str__(self):
        return '%s\t%s\t%s\t%s' % (str(self.date), str(self.isDebit), str(self.amount), str(self.balance))
...
class Account(RootAccount):
    ...
    class dimensionMembers(features.Collection):
        ...
        def _onLink(self, account, member, posn):
            if member.dimension not in account._dimensionToMember:
                account._dimensionToMember[member.dimension] = [
                 member]
...
class Transaction(elements.Element):
    ...
    def post(self, *modifiedEntries):
        ...
        self._update(date=entry.getModifiedAttr('date'), nb=entry.getModifiedAttr('number'), desc=entry.getModifiedAttr('description'), amount=entry.getModifiedAttr('amount'))
        entry._update(isReconciled=entry.getModifiedAttr('isReconciled'), isDebit=entry.getModifiedAttr('isDebit'))
