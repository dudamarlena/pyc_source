...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/magot/gui.py
# Compiled at: 2006-11-05 10:50:01
import sys, wx
...
class MainFrame(wx.Frame):
    ...
    def __init__(self, parent, title, ctx):
        ...
        self.ctx = ctx
        menuFile = wx.Menu()
        self.bindMenuItemToHandler(menuFile, 'E&dit account\tAlt-E', 'Configure account details', self.OnConfigureAccount)
        self.bindMenuItemToHandler(menuFile, '&Save\tAlt-S', 'Save modified data', self.OnSave)
        self.bindMenuItemToHandler(menuFile, '&Jump to opposite account\tAlt-J', 'Jump to the opposite account', self.OnJumpToOppositeAccount)
        self.bindMenuItemToHandler(menuFile, '&Close account ledger\tAlt-C', 'Close the selected account ledger', self.OnCloseAccount)
        self.bindMenuItemToHandler(menuFile, 'E&xit\tAlt-X', 'Exit application', self.OnExit)
        menuReport = wx.Menu()
        self.bindMenuItemToHandler(menuReport, 'View under &dimensions\tAlt-D', 'View account hierarchy under selected dimensions', self.OnViewHierarchyUnderDimensions)
        menuBar = wx.MenuBar()
    ...
    def OnConfigureAccount(self, evt):
        ...
        win = AccountEditor(self, account, -1, size=wx.Size(500, 150), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        win.CenterOnScreen()
    ...
    def OnViewHierarchyUnderDimensions(self, evt):
        if self.isActionRefused():
            return
        win = DimensionalViewer(self, self.accRoot, -1, size=wx.Size(500, 300), style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        win.CenterOnScreen()
        if win.ShowModal() == wx.ID_OK:
            dimensions = win.getSelectedDimensions()
            frame = DimensionalReportFrame(self, self.accRoot, dimensions, 'Grouping by ' + (', ').join(map(str, dimensions)))
            frame.Show()

    def isCurrentSelectionAnAccountLedger(self):
        return self.nb.GetCurrentPage() is not self.nb.hierarchy
    ...
    def isActionRefused(self):
        return self.isCurrentSelectionAnAccountLedger() and not self.nb.GetCurrentPage().CheckTransactionModification()
...
class AccountEditor(wx.Dialog):
    ...
    def __init__(self, parent, account, ID, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.DEFAULT_DIALOG_STYLE):
...
class DimensionalViewer(wx.Dialog):
    ...
    def __init__(self, parent, rootAccount, ID, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.DEFAULT_DIALOG_STYLE):
        wx.Dialog.__init__(self, parent, ID, 'Dimension selection', pos, size, style)
        sizer = wx.BoxSizer(wx.VERTICAL)
        comment = wx.StaticText(self, -1, 'Select dimensions/members to group accounts by:', (15,
                                                                                              10))
    ...
    def getSelectedDimensions(self):
        return [ line.selected for line in self.lines if hasattr(line, 'selected') if line.selected is not None ]
