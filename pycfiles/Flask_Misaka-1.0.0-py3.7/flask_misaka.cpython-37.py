...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/flask_misaka.py
# Compiled at: 2019-01-17 05:31:10
...
from misaka import EXT_AUTOLINK, EXT_FENCED_CODE, EXT_NO_INTRA_EMPHASIS, EXT_SPACE_HEADERS, EXT_STRIKETHROUGH, EXT_SUPERSCRIPT, EXT_TABLES, HTML_ESCAPE, HTML_HARD_WRAP, HTML_SKIP_HTML, HTML_USE_XHTML, TABLE_ALIGNMASK, TABLE_HEADER, TABLE_ALIGN_CENTER, TABLE_ALIGN_LEFT, TABLE_ALIGN_RIGHT, EXT_MATH, EXT_FOOTNOTES, EXT_UNDERLINE, EXT_MATH_EXPLICIT, EXT_DISABLE_INDENTED_CODE, EXT_HIGHLIGHT, EXT_QUOTE
ALIAS_EXT = {'autolink':EXT_AUTOLINK, 
...
def make_flags(**options):
    ...
    for name, val in ALIAS_EXT.items():
        if options.get(name):
            ext = ext | val
        if name.startswith('no_') and options.get(name[3:]) is False:
            ext = ext | val
