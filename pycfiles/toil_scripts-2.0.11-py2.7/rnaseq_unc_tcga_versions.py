...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/rnaseq_unc/rnaseq_unc_tcga_versions.py
...
import glob, hashlib, os, subprocess, errno, multiprocessing, shutil, tarfile
...
def build_parser():
    ...
    group.add_argument('--config', default=None, help='Path to config. One sample per line, with the format: uuid,url_to_sample.tar')
    group.add_argument('--input', default=None, help='Accepts a local sample: /path/to/sample.tar. Take note! TheUUID for this sample is derived from the name. So samplesshould be in the form of uuid.tar.')
    group.add_argument('-f', '--config_fastq', default=None, help='Path to CSV. One sample per line with the format: uuid,file:///path/to/R_1.fastq,file:///path/to/R_2.fastq')
    parser.add_argument('--single_end_reads', default=False, action='store_true', help='Set this flag if input data is non-paired (single end reads).')
    parser.add_argument('--unc', help='URL to unc_hg19.bed', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/unc_hg19.bed')
    parser.add_argument('--fasta', help='URL to hg19_M_rCRS_ref.transcripts.fa', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/hg19_M_rCRS_ref.transcripts.fa')
    parser.add_argument('--composite_exons', help='URL to composite_exons.bed', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/composite_exons.bed')
    parser.add_argument('--normalize', help='URL to normalizeBedToolsExonQuant.pl', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/normalizeBedToolsExonQuant.pl')
    parser.add_argument('--rsem_ref', help='RSEM_REF URL', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/rsem_ref.zip')
    parser.add_argument('--chromosomes', help='Chromosomes Directory', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/chromosomes.zip')
    parser.add_argument('--ebwt', help='EBWT Directory', default='https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/rna-seq/ebwt.zip')
    ...
    parser.add_argument('--upload_bam_to_s3', default=False, action='store_true', help='uploads alignment bam to S3 directory specified.')
    parser.add_argument('--s3_dir', default=None, help='S3 Directory, starting with bucket name. e.g.: cgl-driver-projects/ckcc/rna-seq-samples/')
    parser.add_argument('--sudo', dest='sudo', action='store_true', default=False, help='Docker usually needs sudo to execute locally, but not when running Mesos or when the user is a member of a Docker group.')
    return parser
...
def download_shared_files(job, input_args):
    ...
    """
    shared_files = [
     'unc.bed', 'hg19.transcripts.fa', 'composite_exons.bed', 'normalize.pl', 'rsem_ref.zip',
...
def mapsplice(job, job_vars):
    ...
    docker_call(tool='quay.io/ucsc_cgl/mapsplice:2.0.1.9--2296da365ead6b12ded9d9c7b7798fbc927cd66b', tool_parameters=parameters, work_dir=work_dir, sudo=sudo)
...
def mapping_stats(job, job_vars):
    ...
    output_files = [
     ('{}_stats2.txt').format(uuid), ('{}_stats_all.txt').format(uuid), ('{}_mapping.tab').format(uuid)]
...
def add_read_groups(job, job_vars):
    ...
    docker_call(tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e', tool_parameters=parameter, work_dir=work_dir, sudo=sudo)
...
def bamsort_and_index(job, job_vars):
    ...
    cmd1 = [
     'sort', docker_path(rg_alignments), docker_path('sorted')]
    cmd2 = ['index', docker_path(output)]
    docker_call(tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e', tool_parameters=cmd1, work_dir=work_dir, sudo=sudo)
    docker_call(tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e', tool_parameters=cmd2, work_dir=work_dir, sudo=sudo)
...
def rseq_qc(job, job_vars):
    ...
    docker_call(tool='jvivian/qc', tool_parameters=['/opt/cgl-docker-lib/RseqQC_v2.sh', '/data/sorted.bam', uuid], work_dir=work_dir, sudo=sudo)
...
def sort_bam_by_reference(job, job_vars):
    ...
    cmd = [
     'samtools', 'cat', '-o', output] + sorted_files
...
def exon_count(job, job_vars):
    ...
    cmd_2 = ['perl',
     ...
     composite_bed]
    popen_docker = [
     'docker', 'run', '-v', ('{}:/data').format(work_dir), tool]
    if sudo:
        popen_docker = [
         'sudo'] + popen_docker
...
def transcriptome(job, job_vars):
    ...
    work_dir = job.fileStore.getLocalTempDir()
    sudo = input_args['sudo']
    sort_by_ref, bed, hg19_fa = return_input_paths(job, work_dir, ids, 'sort_by_ref.bam', 'unc.bed', 'hg19.transcripts.fa')
    ...
    parameters = [
     ...
     '--xgtag',
     '--reverse']
    docker_call(tool='quay.io/ucsc_cgl/ubu:1.0--ce6807e937a7a29138f56ea1b8fc077528ee8180', tool_parameters=parameters, work_dir=work_dir, java_opts='-Xmx30g', sudo=sudo)
...
def filter_bam(job, job_vars):
    ...
    parameters = [
     ...
     '--out', docker_path(output)]
    docker_call(tool='quay.io/ucsc_cgl/ubu:1.0--ce6807e937a7a29138f56ea1b8fc077528ee8180', tool_parameters=parameters, work_dir=os.path.dirname(output), java_opts='-Xmx30g', sudo=sudo)
...
def rsem(job, job_vars):
    ...
    docker_call(tool='quay.io/ucsc_cgl/rsem:1.1.13--be66304ff7fcd6fb0babcd1884ed289eabedc655', tool_parameters=parameters, work_dir=work_dir, sudo=sudo)
...
def rsem_postprocess(job, job_vars):
    ...
    output_files = [
     'rsem.genes.norm_counts.tab', 'rsem.genes.raw_counts.tab', 'rsem.genes.norm_fpkm.tab',
...
def main():
    ...
    inputs = {'config': args.config, 'config_fastq': args.config_fastq, 
       'input': args.input, 
       'unc.bed': args.unc, 
