...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/spark_utils/spawn_cluster.py
...
def start_spark_hdfs_cluster(job, numWorkers, executorMemory, jFn, jArgs=[], jCores=None, jMemory=None, jDisk=None):
    jReqs = {}
    if jCores:
        jReqs['cores'] = jCores
    if jMemory:
    ...
    job.addChildJobFn(start_spark_hdfs_workers, masterIP, numWorkers, executorMemory, jFn, jArgs, jReqs)
...
class MasterService(Job.Service):
    ...
    def start(self, fileStore):
        ...
        self.sparkContainerID = docker_call(tool='quay.io/ucsc_cgl/apache-spark-master:1.5.2', docker_parameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw',
         '-e', 'SPARK_MASTER_IP=' + self.IP,
         '-e', 'SPARK_LOCAL_DIRS=/ephemeral/spark/local',
         '-e', 'SPARK_WORKER_DIR=/ephemeral/spark/work'], rm=False, check_output=True, mock=False)[:-1]
        _log.info('Started HDFS Datanode.')
        self.hdfsContainerID = docker_call(tool='quay.io/ucsc_cgl/apache-hadoop-master:2.6.2', docker_parameters=[
         '--net=host',
         '-d'], parameters=[
         self.IP], rm=False, check_output=True, mock=False)[:-1]
        return self.IP
    ...
    def check(self):
        ...
        return self.sparkContainerID in containers and self.hdfsContainerID in containers
...
class WorkerService(Job.Service):
    ...
    def start(self, fileStore):
        ...
        fileStore: Unused
        """
        self.sparkContainerID = docker_call(tool='quay.io/ucsc_cgl/apache-spark-worker:1.5.2', docker_parameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw',
         ...
         self.masterIP + ':' + SPARK_MASTER_PORT], rm=False, check_output=True, mock=False)[:-1]
    ...
    def __start_datanode(self):
        ...
        """
        self.hdfsContainerID = docker_call(tool='quay.io/ucsc_cgl/apache-hadoop-worker:2.6.2', docker_parameters=[
         '--net=host',
         '-d',
         '-v', '/mnt/ephemeral/:/ephemeral/:rw'], parameters=[
         self.masterIP], rm=False, check_output=True, mock=False)[:-1]
