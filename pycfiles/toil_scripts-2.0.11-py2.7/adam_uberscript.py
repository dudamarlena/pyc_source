...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/adam_uberscript/adam_uberscript.py
# Compiled at: 2016-10-04 14:54:20
import logging
from contextlib import contextmanager
from subprocess import check_call, CalledProcessError, check_output
log = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO, format='%(asctime)-15s:%(levelname)s:%(name)s:%(message)s', datefmt='%m-%d %H:%M:%S')
import argparse, csv, errno, os, threading, time
from StringIO import StringIO
from collections import namedtuple
from datetime import datetime, timedelta
from pipes import quote
from uuid import uuid4
import boto, boto.ec2.cloudwatch, boto.sdb
...
def launch_cluster(params):
    ...
    check_call([
     'cgcloud', 'create-cluster', '--zone', params.zone, '--cluster-name', params.cluster_name, '--leader-instance-type', params.leader_type, '--instance-type', params.instance_type, '--num-workers', '1', '--ssh-opts', '"StrictHostKeyChecking=no"'] + role_options(params) + spot_options(params) + [
     'toil'])
    check_call(['cgcloud',
     'rsync',
     '--zone', params.zone,
...
def launch_pipeline(params):
    ...
    try:
        ...
        pipeline_command = [
         'PYTHONPATH=$PYTHONPATH:~/toil-scripts/src',
        ...
        if mock_mode():
            pipeline_command = [
             'TOIL_SCRIPTS_MOCK_MODE=1'] + pipeline_command
        pipeline_command.append('{r} 2>&1 | tee toil_output\n')
        pipeline_command = (' ').join(pipeline_command)
        pipeline_command = pipeline_command.format(j=jobstore, r=restart, region=region_of_zone(params.zone), config='~/' + os.path.basename(params.config_path))
        chunk_size = 500
...
def get_metric(cw, metric, instance_id, start, stop):
    ...
    metric_object = cw.get_metric_statistics(namespace=namespace, metric_name=metric_name, dimensions={'InstanceId': instance_id}, start_time=start, end_time=stop, period=300, statistics=[
     'Average'])
...
def get_desired_cluster_size(domain):
    ...
    log.info('Samples are currently requesting %d nodes: %s', sample_request, nodes_per_sample.value)
...
def grow_cluster(num_nodes, instance_type, cluster_name, cluster_type='toil', options=None):
    ...
    while num_nodes_left > 0:
        ...
        cmd = [
         'cgcloud', 'grow-cluster', '--list', '--instance-type', instance_type, '--num-workers', str(num_nodes_left), '--cluster-name', cluster_name] + options + [
         cluster_type]
        try:
        ...
        with throttle(cluster_scaling_interval_in_seconds):
            with cluster_size_lock:
                ...
                if cluster_size < desired_cluster_size:
                    log.info('Cluster size (%d) is smaller than requested (%d).', cluster_size, desired_cluster_size)
...
def manage_standalone_spark_cluster(params, domain):
    semaphore = Semaphore.create(domain=domain, name=standalone_spark_semaphore_name, value=params.spark_sample_slots)
    while True:
        with throttle(cluster_scaling_interval_in_seconds):
            ...
            if num_samples:
                ...
                if num_masters == 0:
                    ...
                    output = check_output([
                     'cgcloud', 'create-cluster', '--list', '--zone', params.zone, '--cluster-name', params.cluster_name, '--leader-instance-type', params.spark_master_type or params.leader_type, '--instance-type', params.spark_instance_type or params.instance_type, '--num-workers', str(num_desired_workers), '--ssh-opts', '"StrictHostKeyChecking=no"'] + role_options(params) + [
                     'spark'])
                    ...
                    if num_desired_workers > num_workers:
                        grow_cluster(num_nodes=num_desired_workers - num_workers, instance_type=params.instance_type, cluster_name=params.cluster_name, cluster_type='spark')
...
def collect_realtime_metrics(params, threshold=0.5):
    ...
    timestamps = {}
    log.info(('Metric collection has started. Waiting {} seconds before initial collection.').format(metric_initial_wait_period_in_seconds))
            ...
            for instance_id in tqdm(ids):
                ...
                for metric in list_of_metrics:
                    ...
                    if metric == 'AWS/EC2/CPUUtilization':
                        averages = [ x.value for x in sorted(datapoints, key=lambda x: x.timestamp)
                                   ][-6:]
                        if len(averages) == 6:
                            if max(averages) < threshold:
                                idle = True
                                log.info(('Flagging {} to be killed. Max CPU {} for last 30 minutes.').format(instance_id, max(averages)))
                            else:
                ...
                if idle:
                    try:
                        with cluster_size_lock:
                            ...
                            desired_cluster_size = get_desired_cluster_size(domain)
                            if cluster_size > desired_cluster_size:
                                log.info('Cluster size (%d) is larger than requested (%d).Terminating idle instance %s.', cluster_size, desired_cluster_size, instance_id)
                                cmd = [
                                 'cgcloud',
                                 'terminate',
                                ...
                                try:
                                    check_call(cmd)
                                    log.info('Successfully terminated instance via %s.', (' ').join(cmd))
                                except:
                                    log.error('Terminating instance with %s failed.', (' ').join(cmd))
                                    raise
...
def main():
    ...
    cwd = '/home/ubuntu/toil-scripts/src/toil_scripts/adam_uberscript/'
    default_manifest_path = cwd + ('adam-gatk-manifest.csv' if not mock_mode() else 'adam-gatk-mock-manifest.csv')
    default_config_path = cwd + ('GRCh38.config' if not mock_mode() else 'adam-gatk-mock.config')
    cluster_sp = subparsers.add_parser('launch-cluster', help='Launches EC2 cluster via CGCloud')
    cluster_sp.add_argument('-S', '--share', required=True, help='Full path to directory containing pipeline script, launch script, and master key.')
    cluster_sp.add_argument('-T', '--leader-type', default='m3.medium', help='Sets leader instance type.')
    cluster_sp.add_argument('-b', '--boto-path', default='/home/mesosbox/.boto', type=str, help='Path to local .boto file to be placed on leader.')
    cluster_sp.add_argument('-M', '--manifest-path', default=default_manifest_path, required=not mock_mode(), help='Path to manifest file.')
    pipeline_sp = subparsers.add_parser('launch-pipeline', help='Launches pipeline')
    pipeline_sp.add_argument('-j', '--jobstore', default=None, help='Name of jobstore. Defaults to UUID-Date if not set')
    pipeline_sp.add_argument('--restart', default=None, action='store_true', help='Attempts to restart pipeline, requires existing job store.')
    metric_sp = subparsers.add_parser('launch-metrics', help='Launches metric collection thread')
    metric_sp.add_argument('-j', '--jobstore', required=True, help='Name of jobstore')
    metric_sp.add_argument('--namespace', default=os.environ.get('CGCLOUD_NAMESPACE', '/'), help='CGCloud NameSpace')
    metric_sp.add_argument('--spark-sample-slots', required=False, default=0, type=int, help='The maximum number of samples to be computed concurrently on a standalone Spark cluster managed by this script. To be used in conjunction with the --master_ip=auto option of the launch-cluster command. The default of 0 disables the standalone Spark cluster.')
    cgcloud_zone = os.environ.get('CGCLOUD_ZONE')
    for sp in (cluster_sp, pipeline_sp, metric_sp):
        sp.add_argument('-c', '--cluster-name', required=True, help='The CGCloud cluster name for Toil leader and workers.')
        sp.add_argument('-z', '--zone', required=cgcloud_zone is None, default=cgcloud_zone, help="The EC2 availability zone in which to place on-demand instances like the leaders of the Toil and standalone Spark clusters. Also determines the region of the S3 bucket and SDB domain for Toil's job store. The availability zone for spot instances may be chosen independently from all zones in the region containing the specified zone.")

    for sp in (cluster_sp, metric_sp):
        sp.add_argument('-t', '--instance-type', default='r3.8xlarge' if not mock_mode() else 'c3.large', help='Worker instance type, e.g. m4.large or c3.8xlarge. Defaults to r3.8xlarge in production mode. Will always use c3.large in mock mode, regardless of input value.')
        sp.add_argument('--spot-price', default=None, required=False, help='Instance spot price if desired.')

    for sp in (cluster_sp, pipeline_sp):
        sp.add_argument('-C', '--config-path', default=default_config_path, required=not mock_mode(), help='Path to config file.')

    for sp in (metric_sp, cluster_sp):
        sp.add_argument('-etc', '--add-to-etc-hosts', default=None, required=False, help='Deprecated. Optional entry to add to /etc/hosts on Toil workers. This should *not* be used to communicate the address of a standalone Spark master to driver jobs running on Toil nodes. Use --master_ip=auto instead.')

    params = parser.parse_args()
    if params.command == 'launch-pipeline':
        print params
        parsed_config = {x.replace('-', '_'):y for x, y in yaml.load(open(params.config_path).read()).iteritems()}
