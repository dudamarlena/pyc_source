...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/tools/preprocessing.py
...
def run_realigner_target_creator(job, cores, bam, bai, ref, ref_dict, fai, phase, mills, mem, unsafe=False):
    ...
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs={'sample.intervals': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Xmx{}').format(mem)))
...
def run_indel_realignment(job, intervals, bam, bai, ref, ref_dict, fai, phase, mills, mem, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs={'sample.indel.bam': None, 'sample.indel.bai': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Xmx{}').format(mem)))
...
def run_base_recalibration(job, cores, indel_bam, indel_bai, ref, ref_dict, fai, dbsnp, mem, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs={'sample.recal.table': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Xmx{}').format(mem)))
...
def run_print_reads(job, cores, table, indel_bam, indel_bai, ref, ref_dict, fai, mem, unsafe=False):
    ...
    if unsafe:
        parameters.extend(['-U', 'ALLOW_SEQ_DICT_INCOMPATIBILITY'])
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs={'sample.bqsr.bam': None, 'sample.bqsr.bai': None}, work_dir=work_dir, parameters=parameters, env=dict(JAVA_OPTS=('-Xmx{}').format(mem)))
