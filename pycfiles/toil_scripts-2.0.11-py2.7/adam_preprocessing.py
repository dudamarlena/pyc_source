...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/adam_pipeline/adam_preprocessing.py
...
"""
import argparse, logging, multiprocessing, os, textwrap
...
def adam_transform(master_ip, inputs, in_file, snp_file, hdfs_dir, out_file, spark_on_toil):
    ...
    call_adam(master_ip, [
     ...
     '-limit_projection',
     '-mark_duplicate_reads'], memory=inputs['memory'])
    in_file_name = in_file.split('/')[(-1)]
    remove_file(master_ip, in_file_name + '*', spark_on_toil)
    ...
    call_adam(master_ip, [
     ...
     hdfs_dir + '/ri.adam',
     '-realign_indels'], memory=inputs['memory'])
    ...
    call_adam(master_ip, [
     ...
     '-known_snps', snp_file], memory=inputs['memory'])
    ...
    call_adam(master_ip, [
     ...
     hdfs_dir + '/bqsr.adam',
     out_file,
     '-sort_reads', '-single'], memory=inputs['memory'])
...
def static_adam_preprocessing_dag(job, inputs, sample, output_dir, suffix=''):
    ...
    if inputs.master_ip:
        ...
        start_cluster = job.wrapJobFn(start_spark_hdfs_cluster, inputs.num_nodes - 1, inputs.memory, download_run_and_upload, jArgs=(
         inputs, spark_on_toil), jCores=cores, jMemory='%s G' % inputs.memory).encapsulate()
...
def main():
    ...
    parser_run.add_argument('--config', default='adam_preprocessing.config', type=str, help='Path to the (filled in) config file, generated with "generate-config". \nDefault value: "%(default)s"')
    parser_run.add_argument('--sample', help='The full s3 url of the input SAM or BAM file')
    parser_run.add_argument('--output-dir', default=None, help='full path where final results will be output')
    ...
    if args.command == 'generate-config':
        ...
        require(not (inputs.master_ip and inputs.num_nodes), 'Only one of master_ip and num_nodes can be provided.')
        ...
        for arg in [inputs.dbsnp, inputs.memory]:
            ...
            Job.Runner.startToil(Job.wrapJobFn(static_adam_preprocessing_dag, inputs, args.sample, args.output_dir), args)
