...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/exome_variant_pipeline/exome_variant_pipeline.py
# Compiled at: 2016-10-04 14:54:20
import argparse, multiprocessing, os, sys, tarfile, textwrap
...
def download_sample(job, sample, config):
    ...
    config.normal_bam = job.addChildJobFn(download_url_job, url=config.normal, s3_key_path=config.ssec, cghub_key_path=config.gtkey, disk=disk).rv()
    config.tumor_bam = job.addChildJobFn(download_url_job, url=config.tumor, s3_key_path=config.ssec, cghub_key_path=config.gtkey, disk=disk).rv()
...
def preprocessing_declaration(job, config):
    ...
    if config.preprocessing:
        ...
        processed_normal = job.wrapJobFn(run_gatk_preprocessing, config.cores, config.normal_bam, config.normal_bai, config.reference, config.dict, config.fai, config.phase, config.mills, config.dbsnp, mem, cores=1, memory=mem, disk=disk)
        processed_tumor = job.wrapJobFn(run_gatk_preprocessing, config.cores, config.tumor_bam, config.tumor_bai, config.reference, config.dict, config.fai, config.phase, config.mills, config.dbsnp, mem, cores=1, memory=mem, disk=disk)
        static_workflow = job.wrapJobFn(static_workflow_declaration, config, processed_normal.rv(0), processed_normal.rv(1), processed_tumor.rv(0), processed_tumor.rv(1))
        job.addChild(processed_normal)
        job.addChild(processed_tumor)
        job.addFollowOn(static_workflow)
    else:
        job.addFollowOnJobFn(static_workflow_declaration, config, config.normal_bam, config.normal_bai, config.tumor_bam, config.tumor_bai)
...
def static_workflow_declaration(job, config, normal_bam, normal_bai, tumor_bam, tumor_bai):
    ...
    if config.run_mutect:
        mutect_results = job.addChildJobFn(run_mutect, normal_bam, normal_bai, tumor_bam, tumor_bai, config.reference, config.dict, config.fai, config.cosmic, config.dbsnp, cores=1, memory=memory, disk=disk).rv()
    if config.run_pindel:
        pindel_results = job.addChildJobFn(run_pindel, config.cores, normal_bam, normal_bai, tumor_bam, tumor_bai, config.reference, config.fai, cores=config.cores, memory=memory, disk=disk).rv()
    if config.run_muse:
        muse_results = job.addChildJobFn(run_muse, config.cores, normal_bam, normal_bai, tumor_bam, tumor_bai, config.reference, config.dict, config.fai, config.dbsnp, cores=config.cores, memory=memory, disk=disk).rv()
    ...
    with open(path_to_manifest, 'r') as (f):
        for line in f.readlines():
            if not line.isspace() and not line.startswith('#'):
                ...
                require(len(sample) == 3, ('Bad manifest format! Expected 3 tab separated columns, got: {}').format(sample))
...
def main():
    ...
    parser_run.add_argument('--config', default='config-toil-exome.yaml', type=str, help='Path to the (filled in) config file, generated with "generate-config". \nDefault value: "%(default)s"')
    parser_run.add_argument('--manifest', default='manifest-toil-exome.tsv', type=str, help='Path to the (filled in) manifest file, generated with "generate-manifest". \nDefault value: "%(default)s"')
    parser_run.add_argument('--normal', default=None, type=str, help='URL for the normal BAM. URLs can take the form: http://, file://, s3://, and gnos://. The UUID for the sample must be given with the "--uuid" flag.')
    parser_run.add_argument('--tumor', default=None, type=str, help='URL for the tumor BAM. URLs can take the form: http://, file://, s3://, and gnos://. The UUID for the sample must be given with the "--uuid" flag.')
    parser_run.add_argument('--uuid', default=None, type=str, help='Provide the UUID of a sample when using the"--tumor" and "--normal" option')
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
    ...
    if args.command == 'generate-manifest' or args.command == 'generate':
        ...
        if config.preprocessing:
            require(config.reference and config.phase and config.mills and config.dbsnp, 'Missing inputs for preprocessing, check config file.')
        if config.run_mutect:
            require(config.reference and config.dbsnp and config.cosmic, 'Missing inputs for MuTect, check config file.')
        if config.run_pindel:
            require(config.reference, 'Missing input (reference) for Pindel.')
        if config.run_muse:
            require(config.reference and config.dbsnp, 'Missing inputs for MuSe, check config file.')
