...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/adam_gatk_pipeline/align_and_call.py
...
from toil_scripts.bwa_alignment.bwa_alignment import *
...
def static_dag(job, uuid, rg_line, inputs):
    ...
    args = {'uuid': uuid, 's3_bucket': inputs.s3_bucket, 
    ...
    bwa = job.wrapJobFn(download_reference_files, inputs, [
     uuid,
     ('s3://{s3_bucket}/{sequence_dir}/{uuid}_1.fastq.gz').format(**args),
     ('s3://{s3_bucket}/{sequence_dir}/{uuid}_2.fastq.gz').format(**args)]).encapsulate()
    adam_preprocess = job.wrapJobFn(static_adam_preprocessing_dag, inputs, ('s3://{s3_bucket}/alignment{dir_suffix}/{uuid}.bam').format(**args), ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}').format(**args), suffix='.adam').encapsulate()
    gatk_preprocess = job.wrapJobFn(download_gatk_files, inputs, [
     uuid, ('s3://{s3_bucket}/alignment{dir_suffix}/{uuid}.bam').format(**args)], ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}').format(**args), suffix='.gatk').encapsulate()
    adam_call_inputs = inputs
    gatk_call_inputs = copy.deepcopy(inputs)
    adam_call_inputs.indexed = False
    gatk_call_inputs.indexed = True
    gatk_adam_call = job.wrapJobFn(batch_start, adam_call_inputs, [
     uuid, ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}/{uuid}.adam.bam').format(**args)], ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}').format(**args), suffix='.adam').encapsulate()
    gatk_gatk_call = job.wrapJobFn(batch_start, gatk_call_inputs, [
     uuid, ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}/{uuid}.gatk.bam').format(**args)], ('s3://{s3_bucket}/analysis{dir_suffix}/{uuid}').format(**args), suffix='.gatk').encapsulate()
    nodes_needed_after_alignment = -1
    if inputs.pipeline_to_run == 'adam' or inputs.pipeline_to_run == 'both':
        if inputs.master_ip:
            nodes_needed_after_alignment += 1
        else:
            nodes_needed_after_alignment += inputs.num_nodes + 1
    if inputs.pipeline_to_run == 'gatk' or inputs.pipeline_to_run == 'both':
        nodes_needed_after_alignment += 1
    autoscale_after_alignment = nodes_needed_after_alignment > 1 and inputs.autoscale_cluster
    if autoscale_after_alignment:
        increase_nodes_after_alignment = job.wrapJobFn(increase_node_count, nodes_needed_after_alignment - 1, uuid)
        decrease_nodes_after_adam_preprocess = job.wrapJobFn(decrease_node_count, inputs.num_nodes, uuid)
