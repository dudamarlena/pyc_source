...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/gatk_processing/gatk_preprocessing.py
...
12 = ouput bqsr file
"""
import argparse, base64, collections, errno, hashlib, logging, multiprocessing, os, shutil, subprocess, sys, textwrap, yaml
...
def upload_or_move(job, work_dir, output_dir, output, ssec=None):
    if output_dir.startswith('s3://'):
        s3am_upload(fpath=os.path.join(work_dir, output), s3_dir=output_dir, s3_key_path=ssec)
    else:
...
def create_reference_index(job, ref_id):
    ...
    docker_call(work_dir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:0.1.19--dd5ac549b95eb3e5d166a5e310417ef13651994e', inputs=[
     'ref.fa'], outputs={'ref.fa.fai': None})
...
def create_reference_dict(job, ref_id, input_args):
    ...
    docker_call(work_dir=work_dir, parameters=command, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e', inputs=[
     'ref.fa'], outputs={'ref.dict': None})
...
def remove_supplementary_alignments(job, shared_ids, input_args):
    ...
    command = [
     ...
     '-F', '0x800',
     '/data/sample.bam']
    docker_call(work_dir=work_dir, parameters=command, tool='quay.io/ucsc_cgl/samtools:1.3--256539928ea162949d8a65ca5c79a72ef557ce7c', inputs=[
     'sample.bam'], outputs={'sample.nosuppl.bam': None})
...
def sort_sample(job, shared_ids, input_args):
    ...
    command = [
     ...
     'CREATE_INDEX=true']
    docker_call(work_dir=work_dir, parameters=command, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e', inputs=[
     'sample.nosuppl.bam'], outputs={'sample.sorted.bam': None, 'sample.sorted.bai': None})
...
def mark_dups_sample(job, shared_ids, input_args):
    ...
    command = [
     ...
     'CREATE_INDEX=true']
    docker_call(work_dir=work_dir, parameters=command, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, tool='quay.io/ucsc_cgl/picardtools:1.95--dd5ac549b95eb3e5d166a5e310417ef13651994e', inputs=[
     'sample.sorted.bam'], outputs={'sample.mkdups.bam': None, 'sample.mkdups.bai': None})
...
def realigner_target_creator(job, shared_ids, input_args):
    ...
    """
    work_dir = job.fileStore.getLocalTempDir()
    read_from_filestore(job, work_dir, shared_ids, 'ref.fa', 'sample.mkdups.bam', 'ref.fa.fai', 'ref.dict', 'sample.mkdups.bam.bai', 'phase.vcf', 'mills.vcf')
    ...
    parameters = [
     ...
     '-o', 'sample.intervals']
    docker_call(work_dir=work_dir, parameters=parameters, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=[
     'ref.fa', 'sample.mkdups.bam', 'ref.fa.fai', 'ref.dict',
     'sample.mkdups.bam.bai', 'phase.vcf', 'mills.vcf'], outputs={'sample.intervals': None}, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory})
...
def indel_realignment(job, shared_ids, input_args):
    ...
    """
    work_dir = job.fileStore.getLocalTempDir()
    return_input_paths(job, work_dir, shared_ids, 'ref.fa', 'sample.mkdups.bam', 'phase.vcf', 'mills.vcf', 'sample.intervals', 'ref.fa.fai', 'ref.dict', 'sample.mkdups.bam.bai')
    ...
    parameters = [
     ...
     '-o', 'sample.indel.bam']
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', work_dir=work_dir, parameters=parameters, inputs=[
     'ref.fa', 'sample.mkdups.bam', 'phase.vcf', 'mills.vcf',
     'sample.intervals', 'ref.fa.fai', 'ref.dict',
     'sample.mkdups.bam.bai'], outputs={'sample.indel.bam': None, 'sample.indel.bai': None}, env={'JAVA_OPTS': '-Xmx10g'})
...
def base_recalibration(job, shared_ids, input_args):
    ...
    """
    work_dir = job.fileStore.getLocalTempDir()
    return_input_paths(job, work_dir, shared_ids, 'ref.fa', 'sample.indel.bam', 'dbsnp.vcf', 'ref.fa.fai', 'ref.dict', 'sample.indel.bam.bai')
    ...
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', work_dir=work_dir, parameters=parameters, inputs=[
     'ref.fa', 'sample.indel.bam', 'dbsnp.vcf', 'ref.fa.fai',
     'ref.dict', 'sample.indel.bam.bai'], outputs={'sample.recal.table': None}, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory})
...
def print_reads(job, shared_ids, input_args):
    ...
    work_dir = job.fileStore.getLocalTempDir()
    return_input_paths(job, work_dir, shared_ids, 'ref.fa', 'sample.indel.bam', 'ref.fa.fai', 'ref.dict', 'sample.indel.bam.bai', 'sample.recal.table')
    ...
    parameters = [
     ...
     '-o', outfile]
    docker_call(tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', work_dir=work_dir, parameters=parameters, inputs=[
     'ref.fa', 'sample.indel.bam', 'ref.fa.fai', 'ref.dict',
     'sample.indel.bam.bai', 'sample.recal.table'], outputs={outfile: None, gatk_outfile_idx: None}, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory})
...
def main():
    ...
    parser_run.add_argument('--config', default='gatk_preprocessing.config', type=str, help='Path to the (filled in) config file, generated with "generate-config".')
    group.add_argument('--manifest', default='bwa-alignment-manifest.tsv', type=str, help='Path to the (filled in) manifest file, generated with "generate-manifest". \nDefault value: "%(default)s".')
    group.add_argument('--sample', default=None, nargs='2', type=str, help='Space delimited sample UUID and BAM file in the format: uuid url.')
    parser_run.add_argument('--output-dir', default=None, help='Full path to directory or filename where final results will be output')
    ...
    if options.command == 'generate-manifest' or options.command == 'generate':
        ...
        if not options.sample:
            require(os.path.exists(options.manifest), ('{} not found and no sample provided. Please run "generate-manifest"').format(options.manifest))
    ...
    inputs.cpu_count = multiprocessing.cpu_count()
    inputs.memory = '15'
