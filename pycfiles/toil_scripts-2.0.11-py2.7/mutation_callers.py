...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/tools/mutation_callers.py
...
def run_mutect(job, normal_bam, normal_bai, tumor_bam, tumor_bai, ref, ref_dict, fai, cosmic, dbsnp):
    ...
    parameters = ['--analysis_type', 'MuTect',
     ...
     '--vcf', 'mutect.vcf']
    docker_call(work_dir=work_dir, parameters=parameters, tool='quay.io/ucsc_cgl/mutect:1.1.7--e8bf09459cf0aecb9f55ee689c2b2d194754cbd3')
    output_file_names = [
     'mutect.vcf', 'mutect.cov', 'mutect.out']
...
def run_muse(job, cores, normal_bam, normal_bai, tumor_bam, tumor_bai, ref, ref_dict, fai, dbsnp):
    ...
    parameters = ['--mode', 'wxs',
     ...
     '--cpus', str(cores)]
    docker_call(tool='quay.io/ucsc_cgl/muse:1.0--6add9b0a1662d44fd13bbc1f32eac49326e48562', work_dir=work_dir, parameters=parameters)
...
def run_pindel(job, cores, normal_bam, normal_bai, tumor_bam, tumor_bai, ref, fai):
    ...
    parameters = [
     ...
     '-o', 'pindel']
    docker_call(tool='quay.io/ucsc_cgl/pindel:0.2.5b6--4e8d1b31d4028f464b3409c6558fb9dfcad73f88', work_dir=work_dir, parameters=parameters)
