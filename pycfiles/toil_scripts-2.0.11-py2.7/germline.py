...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/gatk_germline/germline.py
...
import argparse, errno, multiprocessing, os, shutil, subprocess, sys, textwrap
...
def create_reference_index_hc(job, shared_ids, input_args):
    ...
    docker_call(work_dir=work_dir, parameters=faidx_command, tool='quay.io/ucsc_cgl/samtools', inputs=inputs, outputs=outputs)
...
def create_reference_dict_hc(job, shared_ids, input_args):
    ...
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, parameters=command, tool='quay.io/ucsc_cgl/picardtools', inputs=inputs, outputs=outputs)
...
def index(job, shared_ids, input_args):
    ...
    docker_call(work_dir=work_dir, parameters=parameters, tool='quay.io/ucsc_cgl/samtools', inputs=inputs, outputs=outputs)
...
def haplotype_caller(job, shared_ids, input_args):
    ...
    try:
        inputs = input_files
        outputs = {output: None}
        docker_call(work_dir=work_dir, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs=outputs)
    except:
...
def vqsr_snp(job, shared_ids, input_args):
    ...
    docker_call(work_dir=work_dir, env={'JAVA_OPTS': '-Xmx%sg' % input_args.memory}, parameters=command, tool='quay.io/ucsc_cgl/gatk:3.5--dba6dae49156168a909c43330350c6161dc7ecc2', inputs=inputs, outputs=outputD)
...
if __name__ == '__main__':
    ...
    parser_run.add_argument('--config', default='gatk_germline.config', type=str, help='Path to the (filled in) config file, generated with "generate-config".')
    group.add_argument('--manifest', default='gatk-germline-manifest.tsv', type=str, help='Path to the (filled in) manifest file, generated with "generate-manifest". \nDefault value: "%(default)s".')
