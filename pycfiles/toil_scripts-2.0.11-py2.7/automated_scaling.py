...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/adam_uberscript/automated_scaling.py
...
class Sample(object):
    ...
    def to_attribute(self):
        return (
         self.sample_id, '%i:%i' % (self.index, self.nodes))
...
class Samples(Model):
    ...
    def _reindex(cls, samples):
        return OrderedDict((sample.sample_id, sample) for sample in sorted(samples, key=lambda sample: (sample.index, sample.sample_id)))

    def save(self):
        attributes = dict((sample.to_attribute() for sample in self.value.itervalues()), version=str(self.version + 1))
        try:
            self.domain.put_attributes('nodes_per_sample', attributes, expected_value=(
             'version', str(self.version) if self.version else False))
...
class SingletonModel(Model):
    ...
    def save(self, force=False):
        ...
        try:
            ...
            self.domain.put_attributes(item_name=item_name, expected_value=None if force else ('version', str(self.version) if self.version else False), attributes=dict(value=self._from_string(self.value), version=str(self.version + 1)))
        except SDBResponseError as e:
            if e.error_code == 'ConditionalCheckFailed':
...
class Semaphore(Model):
    ...
    def _load(self):
        attributes = self.domain.get_attributes(item_name=self._item_name, consistent_read=True)
        self.version = int(attributes['version'])
        self.value = int(attributes['value'])

    def _save(self, **put_attributes_kwargs):
        attributes = dict(version=str(self.version), value=str(self.value))
        self.domain.put_attributes(item_name=self._item_name, attributes=attributes, **put_attributes_kwargs)
