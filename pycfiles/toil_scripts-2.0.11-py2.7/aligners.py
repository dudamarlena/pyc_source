...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/tools/aligners.py
...
def run_star(job, cores, r1_id, r2_id, star_index_url, wiggle=False):
    ...
    if wiggle:
        ...
        return (
         transcriptome_id, sorted_id, wiggle_id)
    else:
        return (
         transcriptome_id, sorted_id)
...
def run_bwakit(job, config, threads, sort=True, trim=False):
    ...
    docker_call(tool='quay.io/ucsc_cgl/bwakit:0.7.12--528bb9bf73099a31e74a7f5e6e3f2e0a41da486e', parameters=parameters, inputs=file_names, outputs=outputs, work_dir=work_dir)
