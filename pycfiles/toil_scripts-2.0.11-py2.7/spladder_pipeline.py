...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/spladder_pipeline/spladder_pipeline.py
...
"""
import argparse, fnmatch, multiprocessing, os, shutil, subprocess, tarfile
    ...
    if inputs.config:
        with open(inputs.config, 'r') as (f):
            for line in f.readlines():
                sample = line.isspace() or line.strip().split(',')
                if not len(sample) == 2:
                    raise AssertionError('Error: Config file is inappropriately formatted.')
...
def star(job, inputs, r1_cutadapt, r2_cutadapt):
    ...
    docker_call(tool='quay.io/ucsc_cgl/star:2.4.2a--bcbd5122b69ff6ac4ef61958e47bde94001cfe80', work_dir=work_dir, parameters=parameters)
    index_command = [
     'index', '/data/rnaAligned.sortedByCoord.out.bam']
    docker_call(work_dir=work_dir, parameters=index_command, tool='quay.io/ucsc_cgl/samtools:1.3--256539928ea162949d8a65ca5c79a72ef557ce7c')
...
def variant_calling_and_qc(job, inputs, bam_id, bai_id):
    ...
    input_info = [
     (
      inputs.genome, 'genome.fa'), (inputs.positions, 'positions.tsv'),
     (
    ...
    docker_call(work_dir=work_dir, parameters=variant_command, sudo=inputs.sudo, tool='quay.io/ucsc_cgl/samtools:1.3--256539928ea162949d8a65ca5c79a72ef557ce7c')
    qc_command = [
     '-o', 'qc',
     '-n', 'alignment.bam',
     '-a', 'annotation.gtf',
     '-m', 'annotation.m53']
    docker_call(work_dir=work_dir, parameters=qc_command, tool='jvivian/checkbias:612f129--b08a1fb6526a620bbb0304b08356f2ae7c3c0ec3')
...
def consolidate_output_tarballs(job, inputs, vcqc_id, spladder_id):
    ...
    fname = (inputs.improper_pair or uuid) + '.tar.gz' if 1 else 'IMPROPER_PAIR' + uuid + '.tar.gz'
    ...
    if inputs.output_s3_dir:
        ...
        job.addChildJobFn(s3am_upload_job, file_id=out_id, s3_dir=inputs.output_s3_dir, num_cores=inputs.cores, file_name=fname, key_path=inputs.ssec, cores=inputs.cores)
...
def main():
    ...
    parser.add_argument('--config', required=True, help='Path to configuration file for samples, one per line. UUID,URL_to_bamfile. The URL may be a standard "http://", a "file://<abs_path>", or "s3://<bucket>/<key>"')
    parser.add_argument('--gtf', help='URL to annotation GTF file', default=url_prefix + 'rnaseq_cgl/gencode.v23.annotation.gtf')
    parser.add_argument('--gtf-pickle', help='Pickled GTF file', default=url_prefix + 'spladder/gencode.v23.annotation.gtf.pickle')
    parser.add_argument('--gtf-m53', help='M53 preprocessing annotation table', default=url_prefix + 'spladder/gencode.v23.annotation.gtf.m53')
    parser.add_argument('--positions', help='URL to SNP positions over genes file (TSV)', default=url_prefix + 'spladder/positions_fixed.tsv')
    parser.add_argument('--genome', help='URL to Genome fasta', default=url_prefix + 'rnaseq_cgl/hg38_no_alt.fa')
    parser.add_argument('--genome-index', help='Index file (fai) of genome', default=url_prefix + 'spladder/hg38_no_alt.fa.fai')
    ...
    parser.add_argument('--sudo', action='store_true', default=False, help='Set flag if sudo is required to run Docker.')
    parser.add_argument('--star-index', help='URL to download STAR Index built from HG38/gencodev23 annotation.', default=url_prefix + 'rnaseq_cgl/starIndex_hg38_no_alt.tar.gz')
    ...
    assert args.config and os.path.isfile(args.config), ('Config not found at: {}').format(args.config)
    if args.ssec:
        assert os.path.isfile(args.ssec), ('Encryption key not found at: {}').format(args.config)
        assert args.output_s3_dir and args.output_s3_dir.startswith('s3://'), 'Wrong format for output s3 directory'
