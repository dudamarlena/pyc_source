...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_scripts/adam_uberscript/boto_lib.py
...
"""
import base64, hashlib, logging, os, subprocess, boto, boto.ec2, time, datetime
...
def calculate_ec2_spot_instance_cost(instance_id, instance_type='c3.8xlarge', avail_zone='us-west-2a'):
    ...
    def calculate_cost(instance_type, start_time, end_time, avail_zone, region='us-west-2'):
        ...
        total, n = (0.0, 0)
        conn = boto.ec2.connect_to_region(region)
        prices = conn.get_spot_price_history(instance_type=instance_type, start_time=start_time, end_time=end_time, availability_zone=avail_zone)
        for price in prices:
            total += price.price
            n += 1
...
def apply_alarm_to_instance(instance_id, namespace='AWS/EC2', metric='CPUUtilization', statistic='Average', comparison='<', threshold=0.5, period=300, evaluation_periods=1, region='us-west-2'):
    """
    ...
    alarm = boto.ec2.cloudwatch.MetricAlarm(name=('CPUAlarm_{}').format(instance_id), description='Terminate instance after low CPU.', namespace=namespace, metric=metric, statistic=statistic, comparison=comparison, threshold=threshold, period=period, evaluation_periods=evaluation_periods, dimensions={'InstanceId': [instance_id]}, alarm_actions=[
     ('arn:aws:automate:{}:ec2:terminate').format(region)])
