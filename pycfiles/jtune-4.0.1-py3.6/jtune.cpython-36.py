...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/jtune/jtune.py
# Compiled at: 2019-01-27 20:22:02
# Size of source mod 2**32: 83777 bytes
"""
...
"""
import atexit, datetime, getpass, locale, logging, math, os, re, resource, shlex, socket, subprocess as sp, sys, textwrap, time
...
class GCRecord(object):
    __doc__ = 'Object definition for a single gc record.'
    _version = '1.0'
    ...
    def _parse_record(self):
        ...
        if any('CMS Initial Mark' in line for line in record_array):
            match = re.search(', ([\\d\\.]+) secs\\] ', record_array[(-1)])
            if match:
        ...
        if not self.is_stw_gc:
            for line in record_array:
                if 'CMS-concurrent-sweep: ' in line:
                    match = re.match('^\\d+-\\d+-\\d+T\\d+:\\d+:[\\d\\.]+[+-]\\d+: ([\\d\\.]+): \\[CMS-concurrent-sweep: [\\d\\.]+/([\\d\\.]+) secs', line)
                    if match:
                        self.is_cms_gc = True
                        self.valid_record = True
                        self.gc_type = 'CMS'
                ...
                if not (self.jvm_running_time or self.gc_type):
                    match = re.match('^\\d+-\\d+-\\d+T\\d+:\\d+:[\\d\\.]+[+-]\\d+: ([\\d\\.]+): .*\\[(\\S+)', line)
                    if match:
                        self.jvm_running_time = float(match.group(1))
                        self.gc_type = match.group(2)
                    if not (self.desired_survivor_size or self.curr_threshold or self.max_threshold):
                        match = re.match('^Desired survivor size (\\d+) bytes, new threshold (\\d+) \\(max (\\d+)\\)', line)
                        if match:
                            self.valid_record = True
                    ...
                    if self.jvm_running_time < 300:
                        self.valid_record = False
                        break
                    match = re.match('^- age\\s+(\\d+):\\s+(\\d+) bytes,\\s+(\\d+) total', line)
                    if match:
                        age = int(match.group(1))
                        curr_size = int(match.group(2))
                        max_size = int(match.group(3))
                        self.ages[age - 1] = (
                         age, curr_size, max_size)
                    else:
                        match = re.match('^: (\\d+)\\w->(\\d+)\\w\\((\\d+)\\w\\), ([\\d\\.]+) secs\\] (\\d+)\\w->(\\d+)\\w\\((\\d+)\\w\\), ([\\d\\.]+) secs\\]', line)
                        if match:
...
def percentile(values=None, pct=None):
    ...
    """
    watermark_index = int(round(float(pct) / 100 * len(values) + 0.5))
...
def _run_analysis(gc_data=None, jmap_data=None, jstat_data=None, proc_details=None, optimized_for_ygcs_rate=None):
    ...
    if not gc_data:
        ...
        for survivor_num, pct_list in enumerate(zip_longest(*gc_survivor_death_rates, **{'fillvalue': 0}), 1):
            ...
            survivor_info[survivor_num] = (
             min_pct, mean_pct, max_pct)
            display.render('   Age {0}: {1:>5} / {2:>5} / {3:>5} / {4:>5} (min/mean/max/cuml alive %)\n'.format(survivor_num, '{0:0.1%}'.format(min_pct), '{0:0.1%}'.format(mean_pct), '{0:0.1%}'.format(max_pct), '{0:0.1%}'.format(cuml_pct)))
        ...
        display.render('\n')
        if og_rates:
            display.render('Est. Time Between FGCs (min/mean/max):    {0:>10} {1:>10} {2:>10}\n'.format(reduce_seconds(jvm_mem_cfg['og_size'] / min_og_rate), reduce_seconds(jvm_mem_cfg['og_size'] / mean_og_rate), reduce_seconds(jvm_mem_cfg['og_size'] / max_og_rate)))
        else:
            display.render('Est. Time Between FGCs (min/mean/max):    {0:>10} {1:>10} {2:>10}\n'.format('n/a', 'n/a', 'n/a'))
        ...
        display.render('\n')
        _show_recommendations(death_ages, young_gc_times, full_gc_times, fgc_rate, ygc_rate, yg_alloc_rates, og_rates, jvm_mem_cfg, jmap_data, jstat_data, gc_data, cms_times, survivor_info, optimized_for_ygcs_rate, proc_details)
        display.render('~~~\n')
    ...
    if max_survivor_age < 15:
        if longest_used_ratio == max_survivor_age:
            if age_objects_still_alive[(-1)] > (100 - survivor_watermark) / 100.0:
                error_msg = 'The survivor ratio of {0} is too small as {1:0.1f}% of the objects are still alive. Try increasing the MaxTenuringThreshold (Max: 15) parameter, and running this analysis again.'.format(longest_used_ratio, age_objects_still_alive[(-1)])
            else:
                error_msg = survivor_info or 'For the examined sample period, I could not retrieve any meaningful survivor statistics from the gc.log. This JVM is either sick, or the sample period was too short.'
        else:
            error_msg = survivor_info or 'For the examined sample period, I could not retrieve any meaningful survivor statistics from the gc.log. This JVM is either sick, or the sample period was too short.'
    else:
        if not survivor_info:
        ...
        return (
         adj_ng_size, survivor_ratio, max_tenuring_size, max_tenuring_age)


def _show_recommendations(death_ages=None, young_gc_times=None, full_gc_times=None, fgc_rate=None, ygc_rate=None, yg_alloc_rates=None, og_rates=None, jvm_mem_cfg=None, jmap_data=None, jstat_data=None, gc_data=None, cms_times=None, survivor_info=None, optimized_for_ygcs_rate=None, proc_details=None):
    ...
    if len(young_gc_times) < ygc_count_goal:
        ...
        if ygc_stdev > ygc_stdev_goal * 4:
            ...
            if ygc_stdev > ygc_stdev_goal * 2:
                comment = 'pretty inconsistent'
            else:
                if ygc_stdev > ygc_stdev_goal:
                    ...
                    ready_for_g1 = True
        messages = list()
        if optimized_for_ygcs_rate > ygc_rate:
            if ygc_stdev > ygc_stdev_goal or ygc_mean_ms > ygc_time_goal_ms:
                ...
                messages.append('- With a mean YGC time goal of {0:0.0f}ms, the suggested (optimized for a YGC rate of {1:0.2f}/min) size of NewGen (including adjusting for calculated max tenuring size) considering the above criteria should be {2:0.0f} MiB (currently: {3:0.0f} MiB).'.format(ygc_time_goal_ms, optimized_for_ygcs_rate, float(adj_ng_size) / 1024.0 / 1024.0, float(curr_ng_size) / 1024.0 / 1024.0))
                if new_adj_ng_size < curr_ng_size:
                    messages.append("- Because we're decreasing the size of NewGen, it can have an impact on system load due to increased memory management requirements. There's not an easy way to predict the impact to the application, so watch this after it's tuned.")
            else:
                if ygc_mean_ms > ygc_time_goal_ms:
                    ...
                    messages.append('- With a mean YGC time goal of {0:0.0f}ms, the suggested (optimized for YGC time) size of NewGen (including adjusting for calculated max tenuring size) considering the above criteria should be {1:0.0f} MiB (currently: {2:0.0f} MiB).'.format(ygc_time_goal_ms, float(adj_ng_size) / 1024.0 / 1024.0, float(curr_ng_size) / 1024.0 / 1024.0))
                    if new_adj_ng_size < curr_ng_size:
                        messages.append("- Because we're decreasing the size of NewGen, it can have an impact on system load due to increased memory management requirements. There's not an easy way to predict the impact to the application, so watch this after it's tuned.")
                else:
                    adj_ng_size = curr_ng_size
    ...
    try:
            ...
            for blob in _file:
                for line in blob.split('\x00'):
                    if '-Xloggc' in line:
                        ...
                        if '-XX:SurvivorRatio=' in line:
                            details['survivor_ratio'] = line.split('SurvivorRatio=')[1]
                        else:
                            if '-XX:+UseConcMarkSweepGC' in line:
                                details['use_cms'] = True
                            else:
                                if '-XX:+UseParNewGC' in line:
        ...
        for line in liverun('{0}/java -version'.format(details['java_path'])):
            ...
            if 'java version' in line:
                ...
                match = re.match('^(\\d+)\\.(\\d+)\\.(\\d+)', details['java_build_version'])
...
def run_jstat(pid=None, java_path=None, no_jstat_output=None, fgc_stop_count=None, max_count=None, ygc_stop_count=None):
    ...
    ordered_fields = [
     'EC', 'EP', 'EU', 'S0C/S1C', 'S0C', 'S1C', 'S0U', 'S1U', 'OC', 'OP', 'OU', 'MC', 'MU', 'PC', 'PU', 'YGC', 'YGCD', 'FGC', 'FGCD']
                ...
                if line_num > 0:
                    ...
                    if not displayed_output:
                        ...
                        for field in ordered_fields:
                            if combined_survivors and field != 'S0C' and field != 'S1C' and field in field_widths:
...
def _at_exit(raw_gc_log=None, jmap_data=None, jstat_data=None, proc_details=None, optimized_for_ygcs_rate=None):
    ...
    for line in raw_gc_log:
        match = re.match('^(\\d+)-(\\d+)-(\\d+)T(\\d+):(\\d+):([\\d\\.]+)[+-]\\d+: ([\\d\\.]+):', line)
        if match:
            in_stanza = True
            if date_time:
                if entry:
                    ...
                    if gc_data:
                        ...
                        if gc_record.jvm_running_time:
                            if prev_gc_record.jvm_running_time > gc_record.jvm_running_time:
...
def get_rotated_log_file(gc_log_file):
    ...
    if log_number:
        ...
        logger.debug('\n'.join(textwrap.wrap('Was not able to find a rotated GC log for this process, defaulting to gc log from process.', display.textwrap_offset)))
    return gc_log_file
...
def main():
    ...
    if cmd_args.no_jstat_output:
        if not (cmd_args.ygc_stop_count or cmd_args.stop_count or cmd_args.fgc_stop_count):
    ...
    if not cmd_args.gc_stdin:
        try:
            ...
            if proc_details.get('min_heap_size', 0) != proc_details.get('max_heap_size', 1):
                config_error = True
                logger.error("It looks like either you didn't specify your min and max heap size (-Xms & -Xmx respectively), or they are set to two different sizes. They need to be set to the same for jtune.py to work properly.")
