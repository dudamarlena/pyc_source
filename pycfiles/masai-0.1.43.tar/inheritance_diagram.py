...
# [GCC 8.4.0]
# Embedded file name: /Users/christian/Dropbox/workspace/masai/doc/sphinxext/inheritance_diagram.py
...
LaTeX.
"""
import inspect, os, re, subprocess
try:
...
class InheritanceGraph(object):
    ...
    def _import_class_or_module(self, name):
        ...
        try:
            ...
            raise ValueError("Could not import class or module '%s' specified for inheritance diagram" % name)
        ...
        try:
            ...
            raise ValueError("Could not find class or module '%s' specified for inheritance diagram" % name)

        if inspect.isclass(todoc):
            return [todoc]
        else:
            if inspect.ismodule(todoc):
                classes = []
    ...
    def run_dot(self, args, name, parts=0, urls={}, graph_options={}, node_options={}, edge_options={}):
        ...
        *urls* is a dictionary mapping class names to http urls
        ...
        """
        try:
            dot = subprocess.Popen(['dot'] + list(args), stdin=subprocess.PIPE, stdout=subprocess.PIPE, close_fds=True)
        except OSError:
...
def inheritance_diagram_directive(name, arguments, options, content, lineno, content_offset, block_text, state, state_machine):
    ...
    for name in graph.get_all_class_names():
        refnodes, x = xfileref_role('class', ':class:`%s`' % name, name, 0, state)
...
def setup(app):
    ...
    app.add_node(inheritance_diagram, latex=(
     visit_inheritance_diagram(latex_output_graph), do_nothing), html=(
     visit_inheritance_diagram(html_output_graph), do_nothing))
    app.add_directive('inheritance-diagram', inheritance_diagram_directive, False, (1,
                                                                                    100,
                                                                                    0), parts=directives.nonnegative_int)