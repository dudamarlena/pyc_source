...
# Compiled at: 2020-05-11 11:09:11
...
def nms_ml(batch, main_class_num, multilabel_dict, confidence_threshold=0.5, overlap_threshold=0.5):
    """[summary]
 ...
 [
  int, int, int, int]]):
    ...
    for i in bboxes:
        ...
        end_point = (int(i[0]), int(i[1] + i[3] / 3.0))
        image = cv2.line(image, start_point, end_point, (rg, rb, 125), thickness=6)
        end_point = (int(i[0] + i[2] / 3.0), int(i[1]))
        image = cv2.line(image, start_point, end_point, (rg, rb, 125), thickness=6)
