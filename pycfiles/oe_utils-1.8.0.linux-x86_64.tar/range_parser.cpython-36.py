...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/range_parser.py
...
class RangeParseException(Exception):
    __doc__ = 'Used for handling errors during range parsing.'
...
class ItemRange(Range):
        ...
        if max_end == DEFAULT_MAX_END:
            ...
            if range_header:
                if range_header.startswith('items='):
                    ...
                    raise RangeParseException("range header '{}' does not match expected format".format(range_header))
            if page_param in request.params:
    ...
    def set_link_headers(self, request, total_count):
        ...
        response.headers['Link'] = self._make_link_headers(links, page_param, queryparams, url)

    def _make_link_headers(self, links, page_param, queryparams, url):
        link_headers = []
        queryparams = MultiDict([(key, val.encode('utf8') if isinstance(val, basestring) else val) for key, val in queryparams.items()])
        for name, page in links.items():
            queryparams[page_param] = page
            link_headers.append('<{url}?{queryparams}>; rel="{name}"'.format(url=url,
