...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/qr/pdf.py
...
class QrPdf(object):
    ...
    def __init__(self, request, id, onderwerp, base_url, route, uri, form_data, pagesize=A4, qr_size=5 * cm):
    ...
    def _add_text(self, canvas):
        ...
        text = 'Onderwerp: {subject}<br/>URI: {uri}'.format(subject=(self.onderwerp),
          uri=(self.uri))
        style = ParagraphStyle('Normal')
        style.fontName = 'Courier'
        paragraph = Paragraph(text, style)
        paragraph.wrap(width - 4 * cm, 10 * cm)
        paragraph.drawOn(canvas, 2 * cm, height - 2 * cm)
    ...
    def _add_qr_code(self, canvas):
        ...
        drawing.add(qr)
        renderPDF.draw(drawing, canvas, width / 2 - self.qr_size / 2, height / 2 - self.qr_size / 2)