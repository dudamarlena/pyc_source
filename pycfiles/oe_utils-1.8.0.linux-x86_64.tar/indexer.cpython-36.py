...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/search/indexer.py
            ...
            if not any((
             ...
             session.index_deleted[self.cls_name])):
                ...
                if session.redis is not None:
                    queue_job(session.redis, self.settings['redis.queue_name'], self.index_operation_name, session.index_new[self.cls_name], session.index_dirty[self.cls_name], session.index_deleted[self.cls_name], self.settings)
                else:
                    log.info('Redis not found, falling back to indexing synchronously without redis')
                    self.index_operation(session.index_new[self.cls_name], session.index_dirty[self.cls_name], session.index_deleted[self.cls_name], self.settings)
...
def _add_to_session_list(target, operation):
    ...
    try:
        ...
        log.warning('Trying to register a ' + str(target) + ' for indexing ' + operation + ', but indexing sets are not present.')