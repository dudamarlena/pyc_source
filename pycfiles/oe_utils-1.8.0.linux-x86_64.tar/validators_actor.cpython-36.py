...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/validation/validators_actor.py
...
class RRNSchemaNode(colander.SchemaNode):
    ...
    def validator(node, rrn):
        ...
        if len(rrn) != 11:
            raise colander.Invalid(node, 'Een rijksregisternummer moet 11 cijfers lang zijn.')
        else:
            valid_rrn = _valid(rrn) or False
...
class TelefoonSchemaNode(colander.MappingSchema):
    ...
    def preparer(telefoon):
        ...
        if telefoon is None or telefoon == colander.null:
            ...
            if 'landcode' in telefoon:
                if telefoon.get('landcode') is not None:
                    landcode = telefoon.get('landcode')
                    value = re.sub('[\\s./,-]', '', landcode).lstrip('0')
                    telefoon['landcode'] = '+' + value if value[0] != '+' else value
            if 'nummer' in telefoon:
                if telefoon.get('nummer') is not None:
                    nummer = telefoon.get('nummer')
                    value = re.sub('[\\s./,-]', '', nummer).lstrip('0')
    ...
    def validator(node, telefoon):
        ...
        if telefoon.get('nummer', '') != '':
            ...
            if landcode[0] != '+' or len(landcode[1:]) > 4 or not is_number(landcode[1:]):
                raise colander.Invalid(node, msg + ': Een geldige landcode begint met een + gevolgd door maximaal 4 cijfers')
            if landcode[0] != '+' or len(landcode[1:]) + len(nummer) > 15 or not is_number(nummer):
                raise colander.Invalid(node, msg + ': Een geldige nummer begint met een + gevolgd door maximaal 15 cijfers')
            if landcode == '+32':
