...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/search/searchengine.py
...
class ISearchEngine(Interface):
    ...
    def bulk_add_to_index(self, system_token, object_type, data, id_field='id', batch_size=5000):
    ...
    def query(system_token, object_type=None, query_params=None, sort=None, result_range=None, mapper=default_mapper, load_searchquery_param_func=load_searchquery_parameters, aggregations=None, settings=None, user_acls=None, min_score=None, principals=NOT_SET, source=None):
...
class SearchEngine(object):
    ...
    def add_to_index(self, system_token, object_type, object_id, object_data):
        ...
        if object_type:
            if object_type != '_doc':
                log.info('The use of object types is discouraged by Elasticsearch and will be depecreated in future versions: https://www.elastic.co/guide/en/elasticsearch/reference/current/removal-of-types.html ')
    ...
    def _send_batch(self, system_token, bulk_url, batch_data):
        ...
        if res.json().get('errors'):
            raise BulkException('Result from bulk operation contains errors: ' + res.text)
    ...
    def build_query(self, query_params=None, sort=None, result_range=None, load_searchquery_param_func=load_searchquery_parameters, aggregations=None, settings=None, user_acls=None, min_score=None, source=None):
        """
    ...
    def run_query(self, system_token, es_query, params, object_type=None, mapper=default_mapper, settings=None, principals=NOT_SET):
        """
        Execute a query on the search engine.
        ...
        res = requests.post(search_url,
          data=(json.dumps(es_query)), params=params, headers=headers)
        self.assert_response_ok(res)
        result = json.loads(res.text)
        mapper_args = [result, settings]
        if principals != NOT_SET:
            mapper_args.append(principals)
        return ResultDTO(mapper(*mapper_args), result['hits']['total'] if 'hits' in result else 0, result['aggregations'] if 'aggregations' in result else None)

    def query(self, system_token, object_type=None, query_params=None, sort=None, result_range=None, mapper=default_mapper, load_searchquery_param_func=load_searchquery_parameters, aggregations=None, settings=None, user_acls=None, min_score=None, principals=NOT_SET, source=None):
        """
        Execute a query on the search engine.
        ...
        :return: ResultDTO with data, total, and aggregations.
        """
        query_build = self.build_query(query_params=query_params,
          sort=sort,
          ...
          min_score=min_score,
          source=source)
        return self.run_query(system_token,
    ...
    def create_index(self, system_token, data):
        headers = {'OpenAmSSOID': system_token} if system_token else {}
        headers.update(self.content_header)
        res = requests.put((self.baseurl + '/' + self.index_name),
