...
# [GCC 8.4.0]
# Embedded file name: /home/claeyswo/Envs/env_oe_utils/lib/python3.6/site-packages/oe_utils/audit.py
# Compiled at: 2020-02-12 07:50:31
# Size of source mod 2**32: 3864 bytes
import functools, hashlib, json, logging
    ...
    def wrap(fn):
        ...
        def advice(parent_object, *args, **kw):
                ...
                if request.user is not None:
                    if 'actor' in request.user:
                        ...
                        if actor.get('uri') == actor.get('instantie_actor_uri'):
                            wijziging.updated_by_omschrijving = attributes.get('displayname') or attributes.get('mail') or actor.get('omschrijving')
                        else:
            ...
            if result is not None:
                try:
                    renderer_name = request.registry.settings.get('audit.pyramid.json.renderer', 'jsonrenderer')
                    json_string = renderers.render(renderer_name,
