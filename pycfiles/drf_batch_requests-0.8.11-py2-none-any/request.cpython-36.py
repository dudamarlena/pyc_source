...
# [GCC 8.4.0]
# Embedded file name: /home/th13f/dev/drf-batch-requests/drf_batch_requests/request.py
# Compiled at: 2019-03-01 05:15:59
...
class BatchRequestsFactory(object):
    response_variable_regex = re.compile('({result=(?P<name>[\\w\\d_]+):\\$\\.(?P<value>[\\w\\d_.*]+)})')
    ...
    def update_soft_dependencies(self):
        for request_data in self.request_serializer.validated_data['batch']:
            ...
            for part in request_data.values():
                params = re.findall(self.response_variable_regex, force_text(part))
    ...
    def _prepare_formdata_body(self, data, files=None):
        if not data:
            ...
            match = re.search('boundary=(?P<boundary>.+)', self.request.content_type)
            assert match
            ...
            if files:
                for key, attachment in files.items():
                    attachment.seek(0)
                    body += '--{}\r\nContent-Disposition: form-data; name="{}"; filename="{}"\r\nContent-Type: {}\r\nContent-Transfer-Encoding: binary\r\n\r\n{}\r\n'.format(boundary, key, attachment.name, attachment.content_type, attachment.read())
    ...
    def _process_attr(self, attr):
        params = re.findall(self.response_variable_regex, attr)
        if not params:
            return attr
        else:
            for url_param in params:
                if url_param[1] not in self.named_responses:
                    ...
                    result = get_attribute(self.named_responses[url_param[1]]['_data'], url_param[2].split('.'))
                    if result is None:
    ...
    def generate_request(self, request_data):
        ...
        if self.request.content_type.startswith('multipart/form-data'):
            ...
            if self.request.content_type.startswith('application/x-www-form-urlencoded'):
