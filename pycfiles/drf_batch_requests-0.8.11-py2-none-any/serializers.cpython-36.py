...
# [GCC 8.4.0]
# Embedded file name: /home/th13f/dev/drf-batch-requests/drf_batch_requests/serializers.py
# Compiled at: 2018-02-16 09:53:55
    ...
    def get_files(self, attrs):
        if 'attached_files' not in attrs:
            ...
            if isinstance(attached_files, dict):
                return {key:self.context['parent'].get_files()[attrs['attached_files'][key]] for key in attrs['attached_files']}
            if isinstance(attached_files, list):
                return {key:self.context['parent'].get_files()[key] for key in attrs['attached_files']}
...
class BatchRequestSerializer(serializers.Serializer):
    ...
    def get_files(self, attrs=None):
        return {fn:f for fn, f in self.initial_data.items() if isinstance(f, File) if isinstance(f, File)}
