...
# [GCC 8.4.0]
# Embedded file name: /home/th13f/dev/drf-batch-requests/drf_batch_requests/graph.py
# Compiled at: 2018-02-19 04:16:56
...
class RequestGraphNode(object):
    ...
    def can_be_performed(self):
        return self.status == RequestGraphNode.STATUS_NOT_STARTED and all(map(lambda parent: parent.status == RequestGraphNode.STATUS_COMPLETED, self.parents))
...
class RequestGraph(object):
    ...
    def __init__(self, requests):
        ...
        self._named_requests = {node.request['name']:node for node in filter(lambda n: n.request.get('name'), self.nodes)}
