...
# [GCC 8.4.0]
# Embedded file name: /home/chathuranga/Documents/Repository/Python_Projects/opsworks-cli/modules/common_functions.py
# Compiled at: 2019-07-19 06:31:12
import boto3, time, prettytable, modules.colour
...
def get_status_instances_main(region, deploymentid, instances, success_count, skipped_count, failed_count):
    if deploymentid == '2e7f6dd5e4a34389bc95b4bacc234df0':
        ...
        describe_deployment = client.describe_commands(DeploymentId=deploymentid)
...
def get_names(stack, layer, region, name):
    if stack == '2e7f6dd5-e4a3-4389-bc95-b4bacc234df0':
        ...
        client = boto3.client('opsworks', region_name=region)
        stack_details = client.describe_stacks(StackIds=[
         stack])
        stack_name = stack_details['Stacks'][0]['Name']
        if layer is not None:
            layer_details = client.describe_layers(LayerIds=[
             layer])
