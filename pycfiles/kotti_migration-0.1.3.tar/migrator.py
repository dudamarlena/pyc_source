...
# [GCC 8.4.0]
# Embedded file name: /home/oshane/Workspace/osoobe/packages/kotti/src/kotti_migration/scripts/migrator.py
# Compiled at: 2017-05-22 19:35:26
import sys, json, re, csv, uuid, time, operator, random, transaction, gzip, click, ConfigParser, subprocess
...
@click.option('--login/--no-login', default=True, help='Use the SQL URL found in the configuration file to create the new site if --login is used, else use createdb and dropdb to carry out this task.')
@click.option('--yes', is_flag=True, callback=abort_if_false, help='Skip the prompt message and proceed with dropping and create a new site', expose_value=False, prompt='This will erase all pervious data, are you sure you want to create a new site?')
def clone_db(config_file, postgres_url, login, **kwargs):
    ...
    if login:
        ...
        subprocess.call([
         ('pg_dump {} | psql {}').format(postgres_url, sql_url)], shell=True)
    else:
