...
# [GCC 8.4.0]
# Embedded file name: /home/oshane/Workspace/osoobe/packages/kotti/src/kotti_migration/kotti_migration/util.py
# Compiled at: 2017-05-22 15:21:19
import time, uuid, json, re, subprocess, transaction
...
def json_value(value):
    ...
    if isinstance(value, LocalGroup):
        return {'principal_name': value.principal_name, 
           'id': value.id, 
...
def export_base(context):
    ...
    context_data['__class__'] = ('{0}.{1}').format(context.__class__.__module__, context.__class__.__name__)
    context_data['__foreign_keys__'] = {}
    for fk in context.__table__.foreign_keys:
        context_data['__foreign_keys__'][fk.parent.name] = ('{}.{}').format(str(fk.column.table.name), str(fk.column.name))
...
def import_data(base, context_data, recursively=True):
    ...
    if ('.').join(_class_path) in config.ignore_content_types:
        return
    _class = get_content_type(content_type=_class_path[1], package=_class_path[0])
    default_attrs = {k:v for k, v in context_data.iteritems() if not k.startswith('_') and k not in config.ignore_attrs if not k.startswith('_') and k not in config.ignore_attrs}
