...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vasctrees/viewGraph.py
# Compiled at: 2014-03-03 13:20:07
import os
from mpl_toolkits.mplot3d import axes3d, Axes3D
from matplotlib import cm
from matplotlib.ticker import LinearLocator, FixedLocator, FormatStrFormatter
import matplotlib as mpl_connect, matplotlib.colors as colors, matplotlib.pyplot as pp, numpy as np

def get3DPlotUndirected(fig, og, verbose=True, degree=None):
    ax = Axes3D(fig)
...
def getGraphWithMapping(fig, og, mapping, alpha=0.05, subsample=4, degree=None, showSurface=True, root=None, colormap='jet'):
    ax = Axes3D(fig)
...
def get3DPlot(fig, og, labelNodes=False, alpha=0.05, subsample=4, verbose=True, degree=None, showSurface=True, showMidPlane=True, root=None):
    ax = Axes3D(fig)
    edges = og.edges(data=True)
    nodes = og.nodes(data=True)
    narray = np.zeros((len(nodes), 3), np.float32)
...
def viewUndirectedGraph(og, fignum=1, subsample=4, verbose=True, degree=None, fileName=None, view=True):
...
def viewSurfaceGraph(og, fignum=1, labelNodes=False, alpha=0.05, subsample=4, verbose=True, degree=None, root=None, fileName=None, view=True, theta=100, phi=120):
    """view an ordered graph generated using the SkeltonGraph class"""
...
def viewGraphWithMapping(og, mapping, fignum=1, alpha=0.05, subsample=4, colormap='jet', verbose=True, degree=None, showSurface=True, root=None, fileName=None, view=True, theta=100, phi=120):
    """view an ordered graph generated using the SkeltonGraph class"""
    fig1 = pp.figure(fignum)
    ax1 = getGraphWithMapping(fig1, og, mapping, alpha=alpha, subsample=subsample, degree=degree, showSurface=showSurface, root=root, colormap=colormap)
    ax1.view_init(theta, phi)
    if fileName:
...
def viewGraph2(og, fignum=1, labelNodes=False, alpha=0.05, subsample=4, verbose=True, degree=None, showSurface=True, showMidPlane=True, root=None, fileName=None, view=True, theta=100, phi=120):
    """view an ordered graph generated using the SkeltonGraph class"""
    print 'generating surface view'
    fig1 = pp.figure(fignum)
