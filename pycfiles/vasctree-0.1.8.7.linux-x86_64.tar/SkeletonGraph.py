...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vasctrees/SkeletonGraph.py
# Compiled at: 2014-03-17 17:08:50
import networkx as nx, numpy as np, cPickle, cmvtg, scipy.ndimage as ndi, matplotlib.pyplot as plt, math
...
class SkeletonGraph(object):
    ...
    def __init__(self, img=None, spacing=None, origin=None, orientation=None, label='', pool=None):
        if spacing != None:
    ...
    def getGraphsFromSkeleton(self, verbose=False):
    ...
    def saveCompressedGraphs(self, name, protocol=0):
        ...
        cPickle.dump({'imgShape': self.img.shape, 'skelGraphs': self.graphs, 'orderedGraphs': self.orderedGraphs, 
           'roots': self.roots}, fo)
        fo.close()
        ...
        for e in edges:
            if e[2].has_key('d0'):
                ...
                try:
                    ...
                    cmds = [ ((d0[0][i], d0[1][i], d0[2][i]), (d1[0][i], d1[1][i], d1[2][i]), i) for i in xrange(numPoints)
                           ]
    ...
    def mapVoxelsToGraph(self, points_toMap, key, mp_key='mappedPoints', worldCoordinates=False, verbose=False):
        ...
        cg = self.orderedGraphs[key]
        mapVoxelsToGraph(cg, points_toMap, self.pool, mp_key=mp_key, worldCoordinates=worldCoordinates, origin=self.origin, spacing=self.spacing, verbose=verbose)
...
def pruneUndirectedBifurcations(cg, bifurcations, verbose=False):
...
def fitEdge(og, e, VERBOSE=False):
    ...
    if ae.shape[0] > 3:
        ...
        u = np.array(range(ae.shape[0] + 1)).astype(np.float64) / ae.shape[0]
