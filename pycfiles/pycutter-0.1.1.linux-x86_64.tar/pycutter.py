...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/pycutter/pycutter.py
# Compiled at: 2013-05-26 15:02:36
"""
...
class CutterWindow(QWidget):
    ...
    def adjust_label(self):
        ...
        self.shot_label.move(rect.width() * (1 - self.SCALE_RATIO) / 2.0, rect.height() * (1 - self.SCALE_RATIO) / 2.0)

    def error_slot(self, error):
        """
        General error slot
        """
        QMessageBox.warning(self, QString('Error'), error.message)
        self.close()
...
class ShotLabel(QLabel):
    ...
    def save_shot(shot):
        ...
        """
        save_path = path.join(DEFAULT_PATH, str(int(time.time())) + '.png')
        succ = shot.save(save_path, 'PNG', QUALITY)
        if not succ:
            raise IOError('Error to save file to %s' % save_path)
    ...
    def _in_selected_range(self, pos_x, pos_y):
        ...
        if not self.pos_x1:
            return False
        min_x, max_x = min(self.pos_x1, self.pos_x2), max(self.pos_x1, self.pos_x2)
        min_y, max_y = min(self.pos_y1, self.pos_y2), max(self.pos_y1, self.pos_y2)
    ...
    def _get_sub_shot(self):
        ...
        """
        return self.full_shot.copy(min(self.pos_x1, self.pos_x2) / CutterWindow.SCALE_RATIO, min(self.pos_y1, self.pos_y2) / CutterWindow.SCALE_RATIO, abs(self.pos_x1 - self.pos_x2) / CutterWindow.SCALE_RATIO, abs(self.pos_y1 - self.pos_y2) / CutterWindow.SCALE_RATIO)
    ...
    def _scale_pixmap(shot, ratio):
        ...
        desktop = QApplication.desktop()
        rect = desktop.screenGeometry()
        return shot.scaled(rect.width() * ratio, rect.height() * ratio, transformMode=Qt.SmoothTransformation)
