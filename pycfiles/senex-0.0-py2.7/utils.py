...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/senex/utils.py
# Compiled at: 2016-04-30 10:16:31
import platform, os, sys
from subprocess import Popen, PIPE, STDOUT
from uuid import uuid4
from .installold import which, shell, mysql_python_installed, old_installed, importlib_installed, pil_installed, get_python_path
...
def validate_mysql_credentials(params):
    ...
    if not params.get('mysql_user') or not params.get('mysql_pwd'):
        return 'A MySQL username and password must be provided in order to create and manage OLDs.'
    ...
    if 'Access denied' in stdout:
        return 'Sorry, we cannot access MySQL with user %s and the provided password.' % params['mysql_user']
    else:
        if "GRANT ALL PRIVILEGES ON *.* TO '%s'" % params['mysql_user'] not in stdout:
            return 'Sorry, user %s does not have sufficient MySQL privileges to build an OLD.' % params['mysql_user']
        else:
            return
...
def get_old_version(params):
    ...
    """
    stmts = 'import onlinelinguisticdatabase;print getattr(onlinelinguisticdatabase, "__version__", "")'
