...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/senex/installold.py
# Compiled at: 2016-04-29 19:16:42
"""
================================================================================
  Install OLD
================================================================================
...
"""
import re, os, sys, shutil, urllib, optparse, getpass, pprint, json, datetime, tarfile
...
def add_optparser_options(parser):
    ...
    """
    parser.add_option('--env-dir', dest='env_dir', metavar='ENV_DIR', help="The name of the virtual environment directory that the OLD is/will be installed in (in your home directory). Defaults to 'env'.")
...
def get_params():
    ...
    params = {'env_dir': options.env_dir or 'env'}
    return params
...
def get_system_python_version():
    ...
    if maj == '2':
        if min not in ('7', '6'):
            sys.exit('%sWarning, the OLD was developed on Python 2.6 and 2.7. Your system Python is version %s. Please install Python 2.6 or 2.7 using .pyenv prior to using this install script. Aborting.%s' % (
             ANSI_FAIL, version, ANSI_ENDC))
    else:
        sys.exit('%sWarning, the OLD was developed on Python 2.6 and 2.7. Your system Python is version %s. Please install Python 2.6 or 2.7 using .pyenv prior to using this install script. Aborting.%s' % (
         ANSI_FAIL, version, ANSI_ENDC))
...
def create_env(params):
    ...
    if os.path.isfile(os.path.join(path, 'bin', 'python')):
        ...
        sys.exit('%sFailed to create a new virtual environment in %s.%s' % (
         ANSI_FAIL, path, ANSI_ENDC))
...
def install_PIL(params):
    ...
    fname, headers = urllib.urlretrieve('http://effbot.org/downloads/Imaging-1.1.7.tar.gz', pilpath)
    ...
    stdout = shell([get_python_path(params), 'setup.py', 'build_ext', '-i'], pildirpath)
    logtext.append(stdout)
    logtext.append('\n\nRan `selftext.py` in PIL\n\n')
    stdout = shell([get_python_path(params), 'selftest.py'], pildirpath)
    logtext.append(stdout)
    stdout = shell([get_python_path(params), 'setup.py', 'install'], pildirpath)
    logtext.append(stdout)
    log('install-PIL.log', ('\n').join(logtext))
    if pil_installed(params):
        print 'Done.'
...
def test_PIL(params):
    ...
    if pil_installed(params):
        ...
        try:
            ...
            print '%sWarning: the PIL installation does not seem to be able to correctly reduce the size of .jpg, .png and/or .gif files.%s' % (
...
def test_FFmpeg():
    ...
    try:
        ...
        print '%sWarning: the FFmpeg install does not appear to be able to convert .wav files to .mp3 and/or .ogg formats.%s' % (
         ANSI_WARNING, ANSI_ENDC)
...
def install_m4():
    ...
    fname, headers = urllib.urlretrieve('ftp://ftp.gnu.org/gnu/m4/m4-1.4.10.tar.gz', m4path)
...
def install_bison():
    ...
    fname, headers = urllib.urlretrieve('http://ftp.gnu.org/gnu/bison/bison-2.3.tar.gz', bisonpath)
...
def install_foma():
    ...
    stdout = shell(['svn', 'co', 'http://foma.googlecode.com/svn/trunk/foma/'], get_tmp_path())
    logtext.append(stdout)
...
def install_mitlm():
    ...
    fname, headers = urllib.urlretrieve('https://mitlm.googlecode.com/files/mitlm-0.4.1.tar.gz', mitlmpath)
    ...
    if which('estimate-ngram') and which('evaluate-ngram'):
        ...
        print 'MITLM was not installed correctly. Please see https://code.google.com/p/mitlm/ for instructions on how to install it on your system.'
