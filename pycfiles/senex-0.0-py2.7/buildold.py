...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/senex/buildold.py
# Compiled at: 2016-04-27 15:13:22
"""
================================================================================
  Build OLD
================================================================================
...
"""
import re, os, sys, shutil, optparse, getpass, pprint, json, datetime
...
def catcherror(func):
    ...
    def new_func(params):
        try:
            ...
            print '%sError: attempted to create a directory where a file already existed. Aborting.%s' % (
             ANSI_FAIL, ANSI_ENDC)
            abort(params)
            sys.exit('Goodbye.')
        except DBCheckError:
            print '%sError occurred when attempting to check if the MySQL database %s already exists. Aborting.%s' % (
             ANSI_FAIL, db_name,
             ANSI_ENDC)
            abort(params)
            sys.exit('Goodbye.')
...
def write_updated_virtual_hosts_file_to_tmp(params):
    ...
    proxy_lines = {'ProxyPass': {}, 'ProxyPassReverse': {}}
    ...
    proxy_lines['ProxyPass'][params['old_dir_name']] = 'ProxyPass /%s/ http://localhost:%s/ retry=5' % (
     params['old_dir_name'], params['old_port'])
    proxy_lines['ProxyPassReverse'][params['old_dir_name']] = 'ProxyPassReverse /%s/ http://localhost:%s/' % (
     params['old_dir_name'], params['old_port'])
    proxy_lines = '%s\n    %s' % (
...
def add_virtual_host(params):
    ...
    if os.path.isfile(params['vh_path']):
        ...
        print '%sThe virtual hosts file %s already existed; we moved the pre-modified version of it to %s_bk.%s' % (
         ANSI_WARNING,
    ...
    try:
        ...
        abort(params)
        if vhs_content:
            sys.exit('%sUnable to configure the virtual host. Maybe you can do it manually. The file at %s should look like this:\n\n%s%s' % (
             ANSI_FAIL, params['vh_path'], vhs_content, ANSI_ENDC))
        else:
...
def enable_virtual_hosts_config(params):
    ...
    """
    fail_msg = '%sUnable to enable your Apache virtual hosts config file. Do it manually by running `sudo a2ensite ` followed by the name of your virtual hosts config file.%s' % (
     ANSI_WARNING, ANSI_ENDC)
    if not os.path.isfile(params['vh_path'].replace('sites-available', 'sites-enabled')):
        try:
            print 'Enabling the Apache virtual hosts config file.'
            vh_name = os.path.split(params['vh_path'])[1]
            enableconfig = Popen(['sudo', 'a2ensite', vh_name], stdout=PIPE, stderr=STDOUT)
...
def restore_virtual_hosts_file(params):
    ...
    try:
        ...
        fail_msg = '%sUnable to restore virtual hosts file. You should manually remove any lines that contain ProxyPass /%s/ or ProxyPassReverse /%s/ from the file %s and then run `sudo /etc/init.d/apache2 restart`.%s' % (
        ...
        print '%sSomething may have gone wrong when attempting to restore the virtual hosts file %s. Please check that it is no longer proxying requests to %s.%s' % (
         ANSI_WARNING, params['vh_path'],
...
def restart_apache(params):
    ...
    apacherestart = Popen(['sudo', '/etc/init.d/apache2', 'restart'], stdout=PIPE, stderr=STDOUT)
    stdout, nothing = apacherestart.communicate()
    try:
        assert '...done.' in stdout
    except:
        print '%sUnable to restart Apache. Do it manually by running `sudo /etc/init.d/apache2 restart`.%s' % (
         ANSI_WARNING, ANSI_ENDC)
...
def add_optparser_options(parser):
    ...
    """
    parser.add_option('--mysql-username', dest='mysql_user', metavar='MYSQL_USER', help='The username of a MySQL user that can create and drop databases, alter them, and perform CRUD operations on tables.')
    parser.add_option('--mysql-password', dest='mysql_pwd', metavar='MYSQL_PWD', help='The password of the MySQL user.')
    parser.add_option('--config-file', dest='config_file', metavar='CONFIG_FILE', help="Path to a JSON file containing an object with any of the following keys: 'mysql_user', 'paster_path', 'apps_path', 'vh_path', 'host', 'ssl_crt_path', 'ssl_key_path', or 'ssl_pem_path'. If present, these values will be used when their corresponding options are not supplied.")
    parser.add_option('--paster_path', dest='paster_path', metavar='PASTER_PATH', help="The path to the paster program. Defaults to 'paster'.")
    parser.add_option('--apps-path', dest='apps_path', metavar='APPS_PATH', help='Path of the directory where you want the OLD app to be installed.')
    parser.add_option('--virtual-hosts-path', dest='vh_path', metavar='VH_PATH', help="Path to the Apache virtual hosts config file. This file may exist but it doesn't need to; this script will create it, if needed. Defaults to a file with the same name as HOST located in /etc/apache2/sites-available/.")
    parser.add_option('--host', dest='host', metavar='HOST', help='The host name of the URL of this OLD. E.g., www.myoldurl.com')
    parser.add_option('--destroy', dest='destroy', action='store_true', default=False, metavar='DESTROY', help='Use this option to change this program from a builder to a destroyer. If buildold.py has created the target OLD, it will be destroyed: it will stop being served, its directories will be deleted, and its MySQL database will be dropped. USE WITH EXTREME CAUTION.')
    parser.add_option('--list', dest='list', action='store_true', default=False, metavar='LIST', help='Print a list of all OLDs that have been installed here by buildold.py.')
    parser.add_option('--dative-servers', dest='dative_servers', metavar='DATIVE_SERVERS', help="Specify the path for a JSON file that this script should produce; that JSON file will summarize the OLD instances that are being served by this script; this JSON summary can be served by Dative as servers.json so that Dative's default servers match those being served here.")
    parser.add_option('--ssl-crt-path', dest='ssl_crt_path', metavar='SSL_CRT_PATH', help='Path to your SSL .crt file.')
    parser.add_option('--ssl-pem-path', dest='ssl_pem_path', metavar='SSL_PEM_PATH', help='Path to your SSL .pem file, i.e., the intermediate certificate, the one that Apache calls the SSLCertificateChainFile.')
    parser.add_option('--ssl-key-path', dest='ssl_key_path', metavar='SSL_KEY_PATH', help='Path to your SSL .key file.')
...
def prompt_for_name(p):
    ...
    """
    if not p['old_name']:
        prompt, fail_msg = {True: (
                '%sPlease enter the name of the OLD that you want to destroy:%s ' % (
                 ANSI_WARNING, ANSI_ENDC),
                '%sYou must provide the name of the OLD to destroy.%s' % (
                 ANSI_FAIL, ANSI_ENDC)), 
           False: (
                 '%sPlease enter a name for the OLD that you want to build (use only letters, numbers and/or the underscore):%s ' % (
                  ANSI_WARNING, ANSI_ENDC),
                 '%sYou must provide a name for the OLD that you want to build.%s' % (
                  ANSI_FAIL, ANSI_ENDC))}[p['destroy']]
...
def prompt_for_mysql_password(p):
    ...
    """
    if not p['mysql_pwd']:
        p['mysql_pwd'] = getpass.getpass('%sPlease enter the password for the MySQL user %s:%s ' % (
         ANSI_WARNING, p['mysql_user'], ANSI_ENDC))
        if not p['mysql_pwd']:
...
def validate_mysql_credentials(params):
    ...
    mysql_show_grants = Popen(['mysql', '-u', params['mysql_user'],
     '-p%s' % params['mysql_pwd'], '-e', 'show grants;'], stdout=PIPE, stderr=STDOUT)
    stdout, stderr = mysql_show_grants.communicate()
    if 'Access denied' in stdout:
        sys.exit('%sSorry, we cannot access MySQL with user %s and the provided password.%s' % (
         ANSI_FAIL, params['mysql_user'], ANSI_ENDC))
    elif "GRANT ALL PRIVILEGES ON *.* TO '%s'" % params['mysql_user'] not in stdout:
        print 'INSUFF PRIV'
        sys.exit('%sSorry, user %s does not have sufficient MySQL privileges to build an OLD.%s' % (
         ANSI_FAIL, params['mysql_user'], ANSI_ENDC))
...
def get_params():
    ...
    if p['list'] or p['dative_servers']:
        ...
        if p['destroy']:
            return (p, global_state)
        if not p['apps_path']:
            p['apps_path'] = raw_input("%sPlease enter the path of the directory where you want your OLD's files to be written. This script will create the directory, if needed:%s " % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['apps_path']:
                sys.exit("%sYou must specify the directory where the OLD app's directory will be located.%s" % (
                 ANSI_FAIL, ANSI_ENDC))
        if not re.search('^\\w+$', p['old_name'].strip()):
            sys.exit('%sYour OLD name can only contain letters, numbers and/or the underscore.%s' % (
             ANSI_FAIL, ANSI_ENDC))
        if old_already_exists(p):
            sys.exit('%sThere is already an OLD with the name %s installed here. Please try again with a different name.%s' % (
             ANSI_FAIL,
             p['old_name'], ANSI_ENDC))
        if not p['mysql_user']:
            p['mysql_user'] = raw_input('%sPlease enter the username of a MySQL user with sufficient privileges:%s ' % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['mysql_user']:
        ...
        if not p['paster_path']:
            p['paster_path'] = raw_input("%sPlease tell us where the paste executable is; type nothing if it's in your PATH and can be accessed with 'paster':%s" % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['paster_path']:
                p['paster_path'] = conf.get('paster_path', 'paster')
        if not which(p['paster_path']):
            sys.exit('%sSorry, there is no (paster) executable at %s. Please install Paste and tell us where it is.%s' % (
             ANSI_FAIL, p['paster_path'], ANSI_ENDC))
        p['old_dir_name'] = get_dir_name_from_old_name(p['old_name'])
        if database_already_exists(p['old_dir_name'], p['mysql_user'], p['mysql_pwd']):
            sys.exit('%sThere is already a MySQL database with the name %s. Please choose a name for your OLD other than %s.%s' % (
             ANSI_FAIL,
             p['old_dir_name'], p['old_name'], ANSI_ENDC))
        exists = len([ old_p for old_p in global_state if old_p['old_dir_name'] == p['old_dir_name']
                     ]) > 0
        if exists:
            sys.exit('%sThis script has already installed an OLD with the name %s. Please try again with a different name.%s' % (
             ANSI_FAIL,
             p['old_name'], ANSI_ENDC))
        if not p['host']:
            p['host'] = raw_input('%sPlease enter the host name of the URL where your OLD will be served, e.g., something like "www.myoldurl.com":%s ' % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['host']:
        ...
        if not p['vh_path']:
            ...
            p['vh_path'] = raw_input('%sPlease enter the path for the Apache virtual hosts path for this OLD. We will use %s if you enter nothing:%s ' % (
             ANSI_WARNING, default, ANSI_ENDC))
            if not p['vh_path']:
                p['vh_path'] = default
        if not p['ssl_crt_path']:
            p['ssl_crt_path'] = raw_input('%sPlease enter the absolute path to your SSL .crt file:%s ' % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['ssl_crt_path']:
                sys.exit('%sYou must provide a SSL .crt file path%s' % (ANSI_FAIL,
                 ANSI_ENDC))
        if not p['ssl_key_path']:
            p['ssl_key_path'] = raw_input('%sPlease enter the absolute path to your SSL .key file:%s ' % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['ssl_key_path']:
                sys.exit('%sYou must provide a SSL .key file path%s' % (ANSI_FAIL,
                 ANSI_ENDC))
        if not p['ssl_pem_path']:
            p['ssl_pem_path'] = raw_input('%sPlease enter the absolute path to your SSL .pem file:%s ' % (
             ANSI_WARNING, ANSI_ENDC))
            if not p['ssl_pem_path']:
...
def old_already_exists(params):
    ...
    """
    return get_dir_name_from_old_name(params['old_name']) in get_old_dirs(params)
...
def get_old_dirs(params):
    ...
    if os.path.isdir(params['apps_path']):
        existing_old_dirs = [ o for o in os.listdir(params['apps_path']) if os.path.isdir(os.path.join(params['apps_path'], o)) ]
...
def get_next_available_port(params):
    ...
    abort(params)
    sys.exit('%sAborting: no more ports available; you are already using ports %s through %s.%s' % (
     ANSI_FAIL, PORT_START, PORT_END,
     ANSI_ENDC))
...
def get_serve_command(params):
    ...
    cnf_pth = os.path.join(params['old_path'], 'production.ini')
    return [
     params['paster_path'], 'serve', '--daemon',
...
def serve(params):
    ...
    try:
        ...
        print resp
        abort(params)
        sys.exit('%sSomething went wrong when attempting to serve the OLD. Aborting.%s' % (
...
def stop_serving(params):
    ...
    try:
        ...
        try:
            ...
            print 'Error: the output from stopping the paster server is not empty:'
            print e
            print '%sSomething may have gone wrong when attempting to stop the paster server.%s' % (
             ANSI_HEADER, ANSI_ENDC)
...
def create_cronjob(params):
    ...
    if crontab:
        ...
        if True == job.is_valid():
            ...
            print '%sUnable to enable the OLD restart cronjob. We suggest you put the following line in your crontab: "*/1 * * * * %s".%s' % (
             ANSI_WARNING, cmd, ANSI_ENDC)
    else:
        print '%sPython-crontab is not installed. You should probably install it (e.g., via `easy_install python-crontab`) before you run this script again. For the OLD that has just been built, we suggest you put the following line in your crontab: "*/1 * * * * %s".%s' % (
         ANSI_WARNING, cmd, ANSI_ENDC)
...
def destroy_cronjob(params):
    ...
    try:
        ...
        if crontab:
            ...
            print '%sPython-crontab is not installed. You should probably install it (e.g., via `easy_install python-crontab`) before you run this script again. For now, we suggest that you manually remove the following line from your crontab: "* * * * * %s".%s' % (
             ANSI_WARNING, cmd, ANSI_ENDC)
    except:
        print '%sSorry, something went wrong when attempting to destroy the cronjob. Try to do it yourself.%s' % (
         ANSI_WARNING, ANSI_ENDC)
...
def setup_app(params):
    ...
    resp = os.popen(cmd).read()
    try:
        assert resp.strip() == 'Running setup_app() from onlinelinguisticdatabase.websetup'
    except:
        abort(params)
        sys.exit('%sSomething went wrong when attempting to set up the OLD. Aborting.%s' % (
         ANSI_HEADER, ANSI_ENDC))
...
def edit_config(params):
    ...
    try:
        new_config_file = []
        cnf_pth = os.path.join(params['old_path'], 'production.ini')
        mysql_line = 'sqlalchemy.url = mysql://%s:%s@localhost:3306/%s?charset=utf8' % (
         params['mysql_user'],
        ...
        print e
        abort(params)
        sys.exit("%sSomething went wrong when attempting to edit the OLD's config file. Aborting.%s" % (
         ANSI_HEADER, ANSI_ENDC))
...
def create_database(params):
    ...
    cmd = 'mysql -u %s -p%s -e "create database %s default character set utf8;"' % (
     params['mysql_user'], params['mysql_pwd'],
     params['db_name'])
    resp = os.popen(cmd).read()
    try:
        resp = resp.strip()
        ...
        if resp:
            ...
            if 'database exists' in resp:
                sys.exit('%sThe MySQL database %s already exists; please drop it manually or choose a different name for your OLD.%s' % (
                 ANSI_FAIL, params['db_name'], ANSI_ENDC))
            else:
                sys.exit('%sAn error occurred when attempting to create the MySQL database %s. Aborting.%s' % (
                 ANSI_FAIL,
                 params['db_name'], ANSI_ENDC))
    except:
        abort(params)
        sys.exit('%sAn error occurred when attempting to create the MySQL database %s. Aborting.%s' % (
         ANSI_FAIL, params['db_name'],
         ANSI_ENDC))
...
def drop_database(params):
    ...
    """
    fail_msg = '%sSomething may have gone wrong when attempting to drop the MySQL database %s. Please check to ensure that it has been dropped.%s' % (
     ANSI_WARNING, params['db_name'], ANSI_ENDC)
    try:
...
def database_already_exists(db_name, mysql_user, mysql_pwd):
    ...
    """
    cmd = 'mysql -u %s -p%s -e "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = \'%s\';"' % (
...
def fix_tag_name_col(params):
    ...
    cmd = 'mysql -u %s -p%s -e "use %s; alter table tag modify name varchar(255) collate utf8_bin;"' % (
     params['mysql_user'],
     params['mysql_pwd'], params['db_name'])
    resp = os.popen(cmd).read()
    try:
        ...
        abort(params)
        sys.exit('%sAn error occurred when attempting to create the MySQL database %s. %s. Aborting.%s' % (
         ANSI_FAIL,
...
def destroy_old_directory(params):
    ...
    try:
        ...
        print '%sSomething may have gone wrong when attempting to destroy the directory %s. Check to ensure that it has been destroyed.%s' % (
...
def save_state(params):
    ...
    try:
        ...
        print '%sWarning: an error occurred when attempting to save state to %s. Because of this, this script cannot be used in the future to destroy or shut down this OLD.%s' % (
         ANSI_WARNING, STORE,
         ANSI_ENDC)
...
def get_state():
    ...
    if os.path.isfile(STORE):
        try:
            ...
            print "%sWarning: unable to retrieve buildold's state at %s. The file exists but is not properly formatted. If this build succeeds, we will overwrite it.%s" % (
             ANSI_WARNING, STORE,
             ANSI_ENDC)
            global_state = []
...
def destroy(params, global_state):
    ...
    try:
        destroyee_params = [ o for o in global_state if o['old_name'] == params['old_name']
                           ][0]
    except:
        sys.exit('%sSorry, this script has no record of an OLD named %s. If it exists, you will need to destroy it manually.%s' % (
         ANSI_FAIL,
         params['old_name'], ANSI_ENDC))

    proceed = raw_input("%sAre you sure that you want to destroy the OLD named %s? THIS CANNOT BE UNDONE. Enter 'y' or 'Y' to proceed with the destruction. You may want to backup its database and files, prior to destruction.%s" % (
     ANSI_WARNING, params['old_name'], ANSI_ENDC))
    ...
    global_state = [ o for o in global_state if o['old_name'] != params['old_name']
                   ]
    with open(STORE, 'w') as (fo):
        json.dump(global_state, fo, indent=4, sort_keys=True)
    actions_taken = params['actions']
...
def build(params):
    ...
    params['old_path'] = os.path.join(params['apps_path'], params['old_dir_name'])
    ...
    init_script(params)
    save_state(params)
    print 'The %s OLD is being served at %shttps://%s/%s%s.\nIts files are stored at %s%s%s.' % (
     params['old_name'], ANSI_OKGREEN, params['host'],
...
def init_script(params):
    ...
    with open(tmp_pth, 'w') as (f):
        f.write(script)
    fail_msg = 'Something went wrong when attempting to install the init script. You can try doing it yourself. See http://pylonsbook.com/en/1.1/deployment.html. Also check to see if there is a file at %s and whether it contains errors.' % (
     initd_pth,)
    ...
    chmod = Popen(['sudo', 'chmod', 'o+x', initd_pth], stdout=PIPE, stderr=STDOUT)
...
def remove_init_script(params):
    ...
    init_name = '%s_init' % params['old_dir_name']
    initd_pth = '/etc/init.d/%s' % init_name
    fail_msg = '%sSomething may have gone wrong when attempting to remove the init script.%s' % (
     ANSI_WARNING, ANSI_ENDC)
    if os.path.isfile(initd_pth):
        ...
        update = Popen(['sudo', 'update-rc.d', '-f', init_name, 'remove'], stdout=PIPE, stderr=STDOUT)
        stdout, nothing = update.communicate()
        try:
...
def create_dative_servers_file(params, global_state):
    ...
    if os.path.isfile(path):
        r = raw_input('There is already a file at %s. Do you want to overwrite it? ' % path)
        if r not in ('y', 'Y'):
    ...
    with open(path, 'w') as (fo):
        servers = []
        for old in global_state:
            old_dict = {'name': '%s OLD' % old['old_name'].capitalize(), 
               'type': 'OLD', 
