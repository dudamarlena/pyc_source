...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-x86_64/egg/senex/views.py
# Compiled at: 2016-05-02 10:42:08
import cgi, re, pprint, json, datetime, os
from .utils import get_server, get_dependencies, validate_mysql_credentials, generate_salt
...
from .models import DBSession, OLD, SenexState
from worker import worker_q
setting_labels_human = {'mysql_user': 'MySQL username', 
...
setting_tooltips = {'mysql_user': 'The username of a MySQL user that can create, alter and drop databases and tables.', 
   'mysql_pwd': 'The password corresponding to the MySQL username.', 
   'env_dir': "The name of the directory (in your user's home directory) where the Python virtual environment will be, or has been, created. Where the OLD and its Python dependencies are installed.", 
   'apps_path': 'The full path to the directory which contains a directory for each installed OLD.', 
   'host': 'The host name of the URL at which the OLDs will be served, e.g., www.myoldurl.com. OLDs will be served at a path relative to this host.', 
   'vh_path': 'The full path to the Apache virtual hosts file for proxying requests to specific OLDs.', 
...
core_dependencies = ('Python', 'OLD', 'MySQL', 'easy_install', 'virtualenv', 'MySQL-python',
                     'importlib', 'Apache')
...
def get_state():
    ...
    return (
     server_state, dependency_state, settings, senex_state.installation_in_progress)
...
def get_warnings(server_state, dependency_state, settings):
    ...
    """
    warnings = {}
    if server_state.get('os') != 'Ubuntu Linux' or server_state.get('os_version', '')[:5] not in ('14.04',
                                                                                                  '10.04'):
        warnings['server'] = 'Senex is only known to work with Ubuntu Linux 14.04 and 10.04.'
    for dependency in dependency_state:
        if dependency['name'] in core_dependencies and not dependency['installed']:
            warnings['core_dependencies'] = "Some of the OLD's core dependencies are not installed."
            break

    if not warnings.get('core_dependencies'):
        try:
            py_ver = [ d for d in dependency_state if d['name'] == 'Python'
                     ][0].get('version', '')
            if py_ver.strip()[:3] not in ('2.6', '2.7'):
                warnings['core_dependencies'] = 'The OLD only works with Python 2.6 and 2.7'
        except:
            pass
...
def validate_settings(settings, warnings):
    ...
    for ext in ('crt', 'key', 'pem'):
        if settings.get('ssl_%s_path' % ext) and not os.path.isfile(settings[('ssl_%s_path' % ext)]):
...
def install_old():
    ...
    settings = senex_state.get_settings()
    worker_q.put({'id': generate_salt(), 
       'func': 'install_old', 
       'args': settings})


@view_config(route_name='return_status', renderer='json', permission='view')
def return_status(request):
    ...
    if logged_in:
        senex_state = get_senex_state_model()
        return {'installation_in_progress': senex_state.installation_in_progress}
    else:
        return {'logged_in': False}


@view_config(route_name='view_main_page', renderer='templates/main.pt', permission='view')
def view_main_page(request):
    logged_in = request.authenticated_userid
    if logged_in:
        if 'form.submitted' in request.params and 'edit.settings' in request.params:
        ...
        if settings.get('mysql_pwd'):
            settings['mysql_pwd'] = '********************'
        return dict(edit_settings_url=request.route_url('view_main_page'), validate_settings_url='%s?validate_settings=true' % request.route_url('view_main_page'), return_status_url=request.route_url('return_status'), install_old_deps_url='%s?install_old_deps=true' % request.route_url('view_main_page'), logged_in=logged_in, olds=olds, server=server_state, dependencies=dependency_state, core_dependencies=[ d for d in dependency_state if d['name'] in core_dependencies ], soft_dependencies=[ d for d in dependency_state if d['name'] not in core_dependencies ], settings=settings, setting_labels_human=setting_labels_human, setting_tooltips=setting_tooltips, old_installed=old_installed, installation_in_progress=installation_in_progress, warnings=warnings)
    else:
        return dict(logged_in=logged_in)


@view_config(route_name='view_old', renderer='templates/view_old.pt', permission='view')
def view_old(request):
    ...
    if old is None:
        ...
        return dict(old=old, logged_in=request.authenticated_userid, edit_url=request.route_url('edit_old', oldname=oldname), login_url=request.route_url('login'), logout_url=request.route_url('logout'))


@view_config(route_name='add_old', renderer='templates/edit_old.pt', permission='edit')
def add_old(request):
    oldname = request.matchdict['oldname']
    if 'form.submitted' in request.params:
        old = OLD(name=oldname)
        DBSession.add(old)
        return HTTPFound(location=request.route_url('view_old', oldname=oldname))
    old = OLD(name='')
    return dict(old=old, logged_in=request.authenticated_userid, save_url=request.route_url('add_old', oldname=oldname), login_url=request.route_url('login'), logout_url=request.route_url('logout'))


@view_config(route_name='edit_old', renderer='templates/edit_old.pt', permission='edit')
def edit_old(request):
    ...
    if 'form.submitted' in request.params:
        ...
        DBSession.add(old)
        return HTTPFound(location=request.route_url('view_old', oldname=oldname))
    return dict(old=old, logged_in=request.authenticated_userid, save_url=request.route_url('edit_old', oldname=oldname), login_url=request.route_url('login'), logout_url=request.route_url('logout'))
...
def login(request):
    ...
    return dict(message=message, url=request.application_url + '/login', came_from=came_from, login=login, password=password)


@view_config(route_name='logout')
def logout(request):
    headers = forget(request)
    return HTTPFound(location=request.route_url('view_main_page'), headers=headers)