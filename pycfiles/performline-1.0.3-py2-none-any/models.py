...
# [GCC 8.4.0]
# Embedded file name: /home/ghetzel/src/github.com/PerformLine/python-performline-client/build/lib.linux-x86_64-2.7/performline/embedded/stdlib/clients/rest/models.py
...
class RestModel(object):
    ...
    def __init__(self, client, data=None, metadata={}, rest_root=None, primary_key=None, secondary_key=None):
        """
        ...
        if not hasattr(self, 'rest_root') or self.rest_root is None:
            raise AttributeError("Cannot create RestModel instance without a valid 'rest_root' attribute")
        if not hasattr(self, 'primary_key') or self.primary_key is None:
            raise AttributeError("Cannot create RestModel instance without a valid 'primary_key' attribute")
    ...
    def get(cls, client, pk):
        ...
        if cls.secondary_key is not None:
            if not isinstance(pk, (list, tuple)) or len(pk) == 1:
                raise AttributeError('Multiple components are required to retrieve this item, 1 given.')
            return cls(client, {cls.primary_key: pk[0], 
               cls.secondary_key: pk[1]}).retrieve()
        else:
            return cls(client, {cls.primary_key: pk}).retrieve()
            return
    ...
    def formatted_path(self, additional_args={}):
        ...
        Returns:
            str
        """
        kwa = {'rest_root': self.rest_root.rstrip('/')}
    ...
    def save(self, refresh=True, compact=False, update=False):
        ...
        if self.rest_create_method is None and self.rest_update_method is None:
            raise UnsupportedOperation(('Persistence is not supported on {0}').format(self.__class__))
        data = self._data
        if compact:
        ...
        self.client.request(method, self.formatted_path(), data)
        if refresh:
    ...
    def __setattr__(self, name, value):
        ...
        if isinstance(self._data, dict):
            if key in [ u(camelize(k, True)) for k in self._data.keys() ] or key in [ u(camelize(k, True)) for k in self.fields ]:
