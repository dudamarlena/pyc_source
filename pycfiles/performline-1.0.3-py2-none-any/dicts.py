...
# [GCC 8.4.0]
# Embedded file name: /home/ghetzel/src/github.com/PerformLine/python-performline-client/build/lib.linux-x86_64-2.7/performline/embedded/stdlib/utils/dicts.py
...
def camelize_dict(inValue, upperFirst=False):
    ...
    """
    return mutate_dict(inValue, keyFn=camelize, keyTypes=(
     str, unicode), upperFirst=upperFirst)
...
def underscore_dict(inValue, joiner='_'):
    ...
    Returns:
        The `dict` with changes applied.
    """
    return mutate_dict(inValue, keyFn=underscore, keyTypes=(
     str, unicode), joiner=joiner)


def mutate_dict(inValue, keyFn=lambda k: k, valueFn=lambda v: v, keyTypes=None, valueTypes=None, **kwargs):
    """
    ...
    def recurse(value):
        return mutate_dict(value, keyFn=keyFn, valueFn=valueFn, keyTypes=keyTypes, valueTypes=valueTypes, **kwargs)
    ...
    if isinstance(inValue, dict):
        ...
        return outputDict
    if isinstance(inValue, list) and len(inValue) > 0:
...
def compact(inDict, keep_if=lambda k, v: v is not None):
    ...
    if isinstance(inDict, dict):
        return {k:v for k, v in inDict.items() if keep_if(k, v)}
