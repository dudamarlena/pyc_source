...
# [GCC 8.4.0]
# Embedded file name: /home/ghetzel/src/github.com/PerformLine/python-performline-client/build/lib.linux-x86_64-2.7/performline/embedded/stdlib/utils/time.py
# Compiled at: 2018-05-17 16:01:23
from __future__ import absolute_import
import datetime
from dateutil import tz, parser
import pytz, time, re
RX_PANDAS_TIMESTR = re.compile('(?P<value>\\d+)\\s*(?P<unit>(?:YEARS?|YRS?|MONTHS?|DAYS?|HOURS?|HRS?|MINUTES?|MINS?|SECONDS?|SECS?|MS|US|NS|[AYMDHTSLUN]))')
PANDAS_UNITS = 'AMDHTSLUN'
RX_SPACE = re.compile('\\s+')
PANDAS_TO_ATTR = {'A': 'from_years', 
   'Y': 'from_years', 
   'YR': 'from_years', 
   'YRS': 'from_years', 
   'YEAR': 'from_years', 
...
class Time(object):
    ...
    def __init__(self, tm=None):
        ...
        if isinstance(tm, self.NATIVE_DATETIME_TYPES):
            ...
            self._nanoseconds = time.time() * int(1000000000.0)
        else:
    ...
    def _parse(self, dt):
        ...
        if isinstance(dt, Time):
            ...
            self._nanoseconds = int(Time.from_seconds(epoch)) + int(Time.from_microseconds(dt.microsecond))
    ...
    def __add__(self, other):
        if isinstance(other, Time):
            ...
            if isinstance(other, (basestring, datetime.datetime, datetime.timedelta)):
                return Time(self._nanoseconds + Time(other)._nanoseconds)
            return TypeError(("'+' operator not supported for type {}").format(type(other)))
    ...
    def total_seconds(self):
        return int(int(self) / int(1000000000.0))
    ...
    def as_datetime(self):
        epoch = int(self._nanoseconds / long(1000000000.0))
        ns = float(self._nanoseconds - epoch * int(1000000000.0))
        result = datetime.datetime.utcfromtimestamp(epoch) + datetime.timedelta(microseconds=int(ns / int(1000.0)))
    ...
    def from_microseconds(cls, val):
        return long(val) * long(1000.0)

    @classmethod
    def from_milliseconds(cls, val):
        return long(val) * long(1000000.0)

    @classmethod
    def from_seconds(cls, val):
        return long(val) * long(1000000000.0)
    ...
    def to_microseconds(cls, nanoseconds):
        return long(nanoseconds) / 1000.0

    @classmethod
    def to_milliseconds(cls, nanoseconds):
        return long(nanoseconds) / 1000000.0
