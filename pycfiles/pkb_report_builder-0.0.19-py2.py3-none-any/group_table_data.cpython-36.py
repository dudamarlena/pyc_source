...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\Github\PKBReportBuilder\PKBReportBuilder\modules\group_data_modules\group_table_data.py
# Compiled at: 2019-02-04 09:36:07
# Size of source mod 2**32: 8572 bytes
import logging, copy, modules.table_processing.not_classificated_records as not_classificated_records, models.root_models.root_model as root_model, modules.root_processor.root_processor as root_processor
...
def check_roots(values, value):
    try:
        ...
        for _value in values:
            if str(_value) != str(value):
                if str(_value).startswith(value):
                    return True
...
def check_result_roots(values, value):
    try:
        for _value in values:
            if _value != value:
                if str(value).startswith(_value):
                    return False
...
def get_to_single_contracts(b_values, roots):
    try:
        ...
        for b_value in b_values:
            if len(b_value) > 1 and b_value[1] == False and b_value[0] != '':
...
def calculate_root_elements(table):
    try:
        column_names = [
         'TotalAmount', 'OutstandingAmount', 'MonthlyInstalmentAmount']
    ...
    try:
        ...
        for order_root in order_roots:
            ...
            if grouping == True:
                ...
                current_group_id += 1
                row_index, root_row = get_table_row_by_aggreement_number(table, cell_index, root_element.aggreement_value)
                ...
                for child_element in child_elements:
                    row_index, child_row = get_table_row_by_aggreement_number(table, cell_index, child_element.aggreement_value)
                    if child_row != None:
                ...
                if start_row_group_index != -1:
                    if end_row_group_index != -1:
