...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pycovjson/write.py
...
class Writer(object):
    __doc__ = 'Writer class'

    def __init__(self, output_name: object, dataset_path: object, vars_to_write: object, tiled=False,
                 tile_shape=[]) -> object:
        """
       ...
       """
        for variable in self.vars_to_write:
            ...
            if self.tiled:
                ...
                url_template = tile_set_obj.generate_url_template(base_url='localhost:8080', axis_names=[
                 't'])
                tileset = TileSet(variable_shape, url_template).create_tileset(self.tile_shape)
                range = Range('TiledNdArray', data_type=variable_type, variable_name=variable, axes=axis_names, tile_sets=tileset, shape=variable_shape)
                return range
            else:
                ...
                range = Range(range_type='NdArray', data_type=data_type, values=values, shape=shape, variable_name=variable, axes=axis_names)
                return range
...
class CustomEncoder(json.JSONEncoder):
    __doc__ = 'Custom Json Encoder class - Allows Json to be saved using custom format (no_indent, compact)'
