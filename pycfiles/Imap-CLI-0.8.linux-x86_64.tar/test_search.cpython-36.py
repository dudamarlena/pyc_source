...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/imap_cli/tests/test_search.py
# Compiled at: 2018-06-03 05:22:40
# Size of source mod 2**32: 9756 bytes
"""Test helpers"""
import datetime, imaplib, sys, unittest
...
class SearchTests(unittest.TestCase):
    ...
    def test_combine_search_criterion(self):
        ...
        if not imap_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")':
            raise AssertionError
        else:
            imap_request = search.combine_search_criterion(search_criterion, operator='AND')
            assert imap_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")'
            imap_request = search.combine_search_criterion(search_criterion, operator='OR')
            assert imap_request == 'OR BEFORE 03-Jan-1989 TO "user@example.tld"'
            imap_request = search.combine_search_criterion(search_criterion, operator='NOT')
            assert imap_request == 'NOT BEFORE 03-Jan-1989 TO "user@example.tld"'
            imap_request = search.combine_search_criterion(search_criterion, operator='WRONG')
            assert imap_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")'
    ...
    def test_create_search_criteria_by_date(self):
        ...
        if not search_criterion == ['SINCE 03-Jan-1989']:
            raise AssertionError
        else:
            search_criterion = search.create_search_criterion_by_date(date,
              relative='BEFORE')
    ...
    def test_create_search_criterion_by_mail_address(self):
        ...
        if not search_criterion == ['FROM "user@example.tld"']:
            raise AssertionError
        else:
            search_criterion = search.create_search_criterion_by_mail_address(mail_address,
              header_name='WRONG')
    ...
    def test_create_search_criteria_by_size(self):
        ...
        if not search_criterion == ['LARGER "3141592"']:
            raise AssertionError
        else:
            search_criterion = search.create_search_criterion_by_size(size,
              relative='SMALLER')
    ...
    def test_create_search_criterion_by_subject(self):
        ...
        if not search_criterion == ['SUBJECT "subject searched"']:
            raise AssertionError
        else:
    ...
    def test_fetch_mails_info(self):
        ...
        reference_mails_info = [
         {'from':'exampleFrom <example@from.org>', 
          ...
          'uid':'1'}]
        if not list(search.fetch_mails_info(self.imap_account)) == reference_mails_info:
            raise AssertionError
        else:
    ...
    def test_fetch_threads(self):
        self.imap_account = imaplib.IMAP4_SSL()
        if not list(search.fetch_threads(self.imap_account)) == [[[1], [2]],
         [
          3, 4]]:
            raise AssertionError
        else:
            threads = search.fetch_threads((self.imap_account),
              search_criterion=[
             'BODY "CONTENT"'])
    ...
    def test_search_cli_tools(self):
        ...
        if not search.main() == 0:
            raise AssertionError
        else:
            sys.argv = [
             'imap-cli-search', '--thread', '-c', 'wrong-config.ini']
            assert search.main() == 1
            sys.argv = [
             'imap-cli-search', '--thread', '-c', 'config-example.ini']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-t testTag -T testText', '-c',
             'config-example.ini']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-t testTag,test2Tag -T testText', '-c',
             'config-example.ini']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-d', '2014-03-13', '-c',
             'config-example.ini']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-d', '201403-13', '-c',
             'config-example.ini']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-a', 'user@exemple.org']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-f', '{from}']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-l', '10']
            assert search.main() == 0
            sys.argv = [
             'imap-cli-search', '-l', '0']
            assert search.main() == 1