...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/imap_cli/flag.py
...
def flag(imap_account, message_set, flags, unset=False):
    if message_set is None or len(message_set) == 0:
        ...
        status, result = imap_account.uid('STORE', request_message_set, '+FLAGS' if unset is False else '-FLAGS', '({})'.format(' '.join(flags)))
