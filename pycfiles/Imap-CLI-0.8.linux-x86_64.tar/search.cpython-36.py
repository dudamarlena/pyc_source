...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/imap_cli/search.py
# Compiled at: 2018-06-03 05:22:40
# Size of source mod 2**32: 16971 bytes
"""Functions searching in IMAP account"""
import ast, codecs, datetime, email
from email import header
import logging, re, sys, docopt, six, imap_cli
...
FLAGS_RE = '.*FLAGS \\((?P<flags>[^\\)]*)\\)'
MAIL_ID_RE = '^(?P<mail_id>\\d+) \\('
UID_RE = '.*UID (?P<uid>[^ ]*)'
...
def create_search_criterion_by_mail_address(mail_address, header_name='FROM'):
    ...
    :param header_name: Specify in wich header address must be searched.                         Possible values are "FROM", "CC", "BCC" and "TO"
    """
    ...
    for idx, thread in enumerate(threads):
        if isinstance(thread, int):
            ...
            if mail_info_by_uid.get(thread) is None:
                ...
                yield '{}{}'.format('  ' * indent, (format_thread.format)(**mail_info_by_uid[thread]))[0:140]
        else:
            for output in display_mail_tree(imap_account,
              thread,
...
def fetch_mails_info(imap_account, mail_set=None, decode=True, limit=None):
    ...
    if mail_set is None:
        ...
        if isinstance(mail_set, six.string_types):
    ...
    for mail_data in mails_data:
        ...
        if not mail_id_match is None:
            if flags_match is None or uid_match is None:
                pass
            else:
                        ...
                        for value, encoding in header.decode_header(header_value):
                            if value is None:
                                ...
                                try:
                                    ...
                                    log.debug("Can't decode {} with {} encoding".format(value, encoding))
                                    decoded_value = value
...
def fetch_uids(imap_account, charset=None, limit=None, search_criterion=None):
    ...
    if imap_account.state != 'SELECTED':
        log.warning('No directory specified, selecting {}'.format(const.DEFAULT_DIRECTORY))
        imap_cli.change_dir(imap_account, const.DEFAULT_DIRECTORY)
    status, data_bytes = imap_account.uid('SEARCH', charset, request_search_criterion)
...
def parse_thread_response(thread_string):
    ...
    """
    return ast.literal_eval('[{}]'.format(thread_string.decode('utf-8').replace(' ', ', ').replace('(', '[').replace(')', '], ')))
...
def main():
    ...
    if connect_conf is None:
        ...
        try:
            ...
            search_criterion = create_search_criterion(address=(args['--address']),
              date=date,
            ...
            if args['--thread'] is False:
                ...
                for output in display_mail_tree(imap_account,
                  mail_tree, format_thread=(display_conf['format_thread'])):
