...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/imap_cli/tests/test_config.py
...
class SASLAuthConfigTest(unittest.TestCase):
    ...
    def test_xoauth2_with_bearer_access_token(self):
        ...
        self.conf = config.new_context_from_file((self.config_file.name), section='imap')
        if not self.conf['hostname'] == 'imap.example.org':
            raise AssertionError
        else:
            if not self.conf['username'] == 'username':
                raise AssertionError
            elif not self.conf['sasl_auth'] == 'XOAUTH2':
                raise AssertionError
            assert self.conf['sasl_ir'] == 'user=username\x01auth=Bearer 12345abcde\x01\x01'
    ...
    def test_other_sasl_auth_with_initial_response(self):
        ...
        if not self.conf['hostname'] == 'imap.example.org':
            ...
            if not self.conf['username'] == 'username':
                raise AssertionError
            elif not self.conf['sasl_auth'] == 'OAUTHBEARER':
                raise AssertionError
    ...
    def test_sasl_auth_no_initial_response(self):
        self.config_file.write('sasl_auth = XOAUTH2\n')
        self.config_file.seek(SEEK_SET, 0)
        self.assertRaises((configparser.NoOptionError), (config.new_context_from_file),
