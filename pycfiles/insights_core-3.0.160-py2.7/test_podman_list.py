...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/tests/test_podman_list.py
...
PODMAN_LIST_IMAGES_NO_DATA = '\nREPOSITORY                           TAG                 DIGEST              IMAGE ID                                                           CREATED             SIZE\n'

def test_podman_list_images():
    result = podman_list.PodmanListImages(context_wrap(PODMAN_LIST_IMAGES))
    ...
    assert result.rows[0].get('SIZE') == '459.5 MB'
...
def test_podman_list_containers():
    result = podman_list.PodmanListContainers(context_wrap(PODMAN_LIST_CONTAINERS))
...
def test_podman_documentation():
    failed_count, tests = doctest.testmod(podman_list, globs={'images': podman_list.PodmanListImages(context_wrap(PODMAN_LIST_IMAGES)), 'containers': podman_list.PodmanListContainers(context_wrap(PODMAN_LIST_CONTAINERS))})
