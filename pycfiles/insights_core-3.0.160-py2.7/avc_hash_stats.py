...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/avc_hash_stats.py
...
class AvcHashStats(CommandParser, LegacyItemAccess):
    ...
    def parse_content(self, content):
        ...
        self.buckets_used, self.buckets = map(lambda x: int(x.strip()), self.data['buckets used'].split('/')) if 'buckets used' in self.data else [None, None]
