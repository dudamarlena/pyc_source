...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/ifcfg.py
...
JSON_FIELDS = [
 'TEAM_CONFIG', 'TEAM_PORT_CONFIG']
QUOTES = '"\''
bond_mode_map = {'balance-rr': 0, 
   'active-backup': 1, 
   'balance-xor': 2, 
   'broadcast': 3, 
   '802.3ad': 4, 
        ...
        for line in get_active_lines(content):
            ...
            if key == 'BONDING_OPTS':
                ...
                for key_value_pair in value.split():
                    sub_key, sub_value = [ s.strip() for s in key_value_pair.split('=', 1) ]
