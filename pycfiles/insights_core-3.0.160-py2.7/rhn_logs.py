...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/rhn_logs.py
...
class TaskomaticDaemonLog(LogFileOutput):
    ...
    def parse_content(self, content):
        ...
        for l in reversed(self.lines):
            ...
            if len(l_sp) >= 3:
                try:
                    self.last_log_date = datetime.strptime(l_sp[2].strip(), self.time_format)
                    break
                except Exception:
                    continue
...
class ServerXMLRPCLog(LogFileOutput):
    ...
    """
    time_format = '%Y/%m/%d %H:%M:%S'
    _LINE_STR = '^(?P<timestamp>\\d{4}/\\d{2}/\\d{2} \\d{2}:\\d{2}:\\d{2} ' + '[+-]?\\d{2}:\\d{2}) (?P<pid>\\d+) (?P<client_ip>\\S+): ' + '(?P<module>\\w+)/(?P<function>[\\w.-]+)' + '(?:\\((?:(?P<client_id>\\d+), ?)?(?P<args>.*?),?\\))?$'
