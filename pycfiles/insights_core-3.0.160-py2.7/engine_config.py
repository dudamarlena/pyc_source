...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/engine_config.py
...
class EngineConfigAll(CommandParser):
    ...
    def parse_content(self, content):
        ...
        for line in get_active_lines(content):
            try:
                ...
                self.fields.append(self.keyvalue(key.strip(':'), val, ver, key.strip(':').lower()))
            except:
                pass
    ...
    def get(self, keyword):
        ...
        if kw in self.keywords:
            return [ kv.value for kv in self.fields if kv.kw_lower == kw and kv.value ]
        return []
    ...
    def get_version(self, keyword):
        ...
        kw = keyword.lower()
        if kw in self.keywords:
            return [ kv.version for kv in self.fields if kv.kw_lower == kw and kv.version ]
        return []