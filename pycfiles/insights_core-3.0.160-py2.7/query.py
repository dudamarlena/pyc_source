...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/tools/query.py
...
import argparse, importlib, inspect, logging, pydoc, re, sys, yaml
from six.moves import StringIO
from insights import apply_default_enabled, apply_configs, dr, get_filters, load_default_plugins, load_packages, parse_plugins
...
def matches(d, path):
    ...
    if isinstance(d, sf.glob_file):
        for pat in d.patterns:
            ...
            if (search_tgt.startswith(path) or re.match(glob2re(search_tgt), path)) and not d.ignore_func(path):
                return True
