...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/contrib/pyparsing.py
...
import copy, sys, warnings, re, sre_constants, collections, pprint, functools, itertools, traceback
__all__ = [
...
class ParseResults(object):
    ...
    def __getnewargs__(self):
        return (
         self.__toklist, self.__name, self.__asList, self.__modal)
...
def oneOf(strs, caseless=False, useRegex=True):
    ...
    if isinstance(strs, basestring):
        ...
        if isinstance(strs, collections.Sequence):
            symbols = list(strs[:])
        else:
            if isinstance(strs, _generatorType):
                symbols = list(strs)
            else:
...
class pyparsing_common():
    ...
    signedInteger = Regex('[+-]?\\d+').setName('signed integer').setParseAction(convertToInteger)
    real = Regex('[+-]?\\d+\\.\\d*').setName('real number').setParseAction(convertToFloat)
    sciReal = Regex('[+-]?\\d+([eE][+-]?\\d+|\\.\\d*([eE][+-]?\\d+)?)').setName('real number with scientfic notation').setParseAction(convertToFloat)
    numeric = (sciReal | real | signedInteger).streamline()
    number = Regex('[+-]?\\d+\\.?\\d*([eE][+-]?\\d+)?').setName('number').setParseAction(convertToFloat)
