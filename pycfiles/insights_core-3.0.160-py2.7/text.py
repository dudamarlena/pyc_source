...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/formats/text.py
# Compiled at: 2020-03-25 13:10:41
from __future__ import print_function
import six, sys, inspect
...
class HumanReadableFormat(Formatter):
    ...
    def __init__(self, broker, missing=False, tracebacks=False, dropped=False, fail_only=False, stream=sys.stdout):
        self.broker = broker
    ...
    def preprocess(self):
        ...
        self.responses = {'pass': response(color=Fore.GREEN, label='PASS', intl='P', title='Passed      : '), 
    ...
    def show_description(self):
        ...
        for c in sorted(self.broker.get_by_type(rule), key=dr.get_name):
            ...
            if _type in self.responses:
                self.counts[_type] += 1
            if _type and (self.fail_only and _type == 'rule' or self.missing and _type == 'skip' or not self.fail_only and _type != 'skip'):
                printit(c, v)
...
class HumanReadableFormatAdapter(FormatterAdapter):
    ...
    def preprocess(self, broker):
        self.formatter = HumanReadableFormat(broker, self.missing, self.tracebacks, self.dropped, self.fail_only)
