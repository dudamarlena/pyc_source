...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/archive/tool.py
# Compiled at: 2019-05-16 13:41:33
import json, atexit, hashlib, os, re, shutil, subprocess, shlex, tempfile
...
class BaseArchive(object):
    ...
    def __init__(self, name, validate=True):
        self.name = name
        root_prefix = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), REPO_PATH, self.root_prefix))
...
class Transform(object):
    ...
    def grep(self, pattern, keep=True):
        ...
        def _grep(path):
            with file_replacer(path) as (fp, tf):
                for line in fp:
                    if bool(pattern.match(line)) == keep:
                        tf.write(line)
                    else:
                        continue
...
class TestArchive(BaseArchive):
    ...
    def __init__(self, name, base_archive='rhel7', overlays=None, transforms=None, machine_id=None, removals=None, compression='gz', hostname=None, **kwargs):
        ...
        self.base_archive = base_archive if isinstance(base_archive, BaseArchive) else BaseArchive(base_archive)
    ...
    def get_context(self):
        return ctx.Context(hostname=self.hostname, machine_id=self.machine_id, **self.extra)
    ...
    def export(self, dest='.', nested_root=True):
        ...
        if nested_root:
            ...
            root_name = '.'
        subprocess.check_call(shlex.split(('tar {0} {1}.{2} -C {3} {4}').format(options, os.path.join(dest, self.name), ext, dir_root, root_name)))
    ...
    def apply_input_data(self, input_data):
        ...
        if input_data.version != ['-1', '-1'] and not [ t.path for t in self.transforms if t.path == 'uname' ]:
...
class MultiArchive(TestArchive):
    ...
    def build_metadata(self, sub_archives):
        ...
        for system in sub_archives:
            ...
            systems.append({'product': product.name, 
...
def build_links(target_archive, all_archives, parent):
    ...
    if is_parent:
        for archive in all_archives:
            ...
            links.append({'system_id': ctx.machine_id, 
