...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/installed_rpms.py
...
class RpmList(object):
    ...
    def is_hypervisor(self):
        ...
        if rpm and rpm.release.endswith(('.el6ev', '.el7ev')):
            return True
        return False
...
class InstalledRpm(object):
    ...
    def _arch_sep(package_string):
        ...
        Returns:
            str: arch separator
        """
        if package_string.rfind('.') > package_string.rfind('-'):
            return '.'
        return '-'
    ...
    def __eq__(self, other):
        ...
        if self.name != other.name:
            raise ValueError(('Cannot compare packages with differing names {0} != {1}').format(self.name, other.name))
