...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/saphostctrl.py
...
class SAPHostCtrlInstances(CommandParser):
    ...
    """
    REQUIRED_DIRECTIVES = ('CreationClassName', 'SID', 'SystemNumber', 'InstanceName',
                           'Hostname', 'SapVersionInfo', 'FullQualifiedHostname')
    ...
    def parse_content(self, content):
        ...
        def _update_instance(inst):
            ...
            if not inst['InstanceName'].endswith(inst['SystemNumber']):
                raise ParseException(('InstanceName: "{0}" missing match with SystemNumber: "{0}"').format(inst['InstanceName'], inst['SystemNumber']))
