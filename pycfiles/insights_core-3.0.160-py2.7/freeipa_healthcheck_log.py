...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/freeipa_healthcheck_log.py
...
class FreeIPAHealthCheckLog(JSONParser):
    ...
    def get_results(self, source, check):
        ...
        for entry in self.data:
            if entry.get('source') == source and entry.get('check') == check and entry.get('result') in ('ERROR',
                                                                                                         'CRITICAL'):
