...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/util/subproc.py
# Compiled at: 2019-05-16 13:41:33
import logging, os, shlex, signal, six, sys
...
class Pipeline(object):
    ...
    def __init__(self, *cmds, **kwargs):
        ...
        cmds = [ (isinstance(c, list) or shlex.split)(c) if 1 else c for c in cmds ]
...
def call(cmd, timeout=None, signum=signal.SIGKILL, keep_rc=False, encoding='utf-8', env=os.environ):
    ...
    if not isinstance(cmd, list):
        cmd = [
         cmd]
    p = Pipeline(timeout=timeout, signum=signum, env=env, *cmd)
