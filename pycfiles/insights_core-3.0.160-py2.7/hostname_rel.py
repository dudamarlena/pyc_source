...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/examples/rules/hostname_rel.py
...
CONTENT = {ERROR_KEY_1: 'This release is RHEL\nHostname: {{ hostname }}\nRelease: {{ release }}', 
...
def report(hostname, release):
    if release and release.is_rhel:
        return make_pass(ERROR_KEY_1, hostname=hostname.fqdn, release=release.version)
    else:
        if release:
            return make_fail(ERROR_KEY_2, hostname=hostname.fqdn, release=release.raw)
