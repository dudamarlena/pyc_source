...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/tests/test_qpid_stat.py
...
def test_qpid_stat_q_docs():
    env = {'qpid_stat_g': qpid_stat.QpidStatG(context_wrap(QPID_STAT_G_DOCS)), 
...
def test_qpid_stat_g():
    ...
    assert [ d.get('Messages') for d in qpid_list ] == [
     ...
     '0', '0', '0', '0']
    assert qpid_list.by_queue['<standalone>'] == {'uptime': '97d 0h 16m 24s', 
       'sessions': '37', 
       'queues': '33', 
       'connections': '23', 
       'cluster': '<standalone>', 
