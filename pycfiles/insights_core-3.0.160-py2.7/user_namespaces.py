...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/combiners/user_namespaces.py
...
class UserNamespaces(object):
    ...
    def enabled(self):
        ...
        """
        return '1' in self.cmdline.get('user_namespaces.enable', []) or '1' in self.cmdline.get('user-namespaces.enable', [])
    ...
    def enabled_configs(self):
        ...
        """
        return [ entry.name for entry in self.grub_cmdline if 'user_namespaces.enable=1' in entry.cmdline or 'user-namespaces.enable=1' in entry.cmdline
               ]