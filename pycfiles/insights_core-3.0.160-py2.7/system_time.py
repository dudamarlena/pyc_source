...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/system_time.py
...
class NtpTime(CommandParser):
    ...
    * ``flags`` - the flags in brackets after the status code.
    ...
    def parse_content(self, content):
        result = {}
        return_code_re = re.compile('(?P<func>ntp_(?:get|adj)time)\\(\\) returns code (?P<code>\\d+) ')
        status_code_re = re.compile('status (?P<status>0x[0-9a-f]+) \\((?P<flags>.*)\\)')
        time_data_re = re.compile('time (?P<timecode>[0-9a-f]{8}\\.[0-9a-f]{8}) +(?P<timestamp>\\w{3}, \\w{3} \\d+ .*),')
        value_re = re.compile('(?P<keyword>[A-Za-z][A-Za-z ]+) (?P<value>\\d+(?:\\.\\d+|x[0-9a-f]+)?) ?(?P<units>us|ppm|s|\\(.*\\))?')
        for line in content:
