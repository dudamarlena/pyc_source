...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/initscript.py
...
CHKCONFIG_REGEX = re.compile('^#\\s+chkconfig:\\s+.*$')
LSB_REGEX = re.compile('^#\\s+Provides:\\s+.*$')
SHEBANG_REGEX = re.compile('^#!\\s*/.*$')
COMMENT_REGEX = re.compile('^\\s*#.*$')
STARTSTOPSTATUS_REGEX = re.compile('\\b(start|stop|status)\\b')
