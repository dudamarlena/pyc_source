...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/insights/parsers/rhn_schema_stats.py
...
class DBStatsLog(CommandParser):
    ...
    def parse_content(self, content):
        ...
        if tables:
            self.data = {'table_summary': TableSummary(tables[0]) if len(tables) > 0 else None, 'constraint_table': ConstraintTable(tables[1]) if len(tables) > 1 else None, 
