...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/genmechanics/unidimensional.py
# Compiled at: 2020-03-26 14:17:33
# Size of source mod 2**32: 19840 bytes
"""
...
class Body:
    ...
    def __init__(self, initial_position, y_position=0.0, name=''):
...
class Load:
    ...
    def Plot(self, bodies_position=None, ax=None, width=0.008, intensity_factor=0.0001):
        ...
        ax.add_patch(Arrow(x, y, self.value * intensity_factor, 0, width))
        ax.text((x + 0.5 * self.value * intensity_factor), (y + 0.2 * width), ('{} N'.format(round(self.value))),
          color='k',
...
class ImposedDisplacement:
    ...
    def Plot(self, bodies_positions=None, strains=None, ax=None, width=0.01, color='k', intensity_factor=1e-06):
        ...
        if self.value != 0:
            ...
            ax.text((x0 + 0.5 * self.value), (y + 0.2 * h), ('{} mm'.format(round(1000 * self.value))),
              horizontalalignment='center',
              verticalalignment='bottom')
...
class Spring(Linkage):
    ...
    def StiffnessMatrices(self):
        return (
         self.stiffness * npy.array([[1, -1], [-1, 1]]),
         self.stiffness * self.free_length * npy.array([-1, 1]))

    def ConstraintsMatrices(self):
        return (None, None)

    def Plot(self, bodies_positions=None, strains=None, ax=None, linkages_width=0.01, color='k', intensity_factor=1e-06):
        if bodies_positions is None:
        ...
        xm = 0.5 * (x1 + x2)
        ym = 0.5 * (y1 + y2)
        ax.text(xm, (ym - 0.5 * linkages_width), ('{} #{}'.format(self.__class__.__name__, str(self.__hash__())[-5:])),
          horizontalalignment='center')
...
class UnilateralContact(Linkage):
    ...
    def ConstraintsMatrices(self):
        return (
         npy.array([[1.0, -1.0]]), npy.array([0.0]))

    def ActivationCondition(self, positions, reactions):
        return positions[1] - positions[0] <= self.contact_distance - 1e-08
    ...
    def Plot(self, bodies_positions=None, strains=None, ax=None, linkages_width=0.01, color='k', intensity_factor=1e-06):
        ...
        ax.text((0.5 * (x1 + x2)), (ym - 0.5 * linkages_width), ('Unilateral contact #{}'.format(str(self.__hash__())[-5:])),
          horizontalalignment='center')
        if strains is not None:
            if self in strains:
                strain = strains[self][0]
                if strain < 1e-06:
                    ax.add_patch(Arrow(xplate1, ym, (-strain * intensity_factor),
                      0, linkages_width, color='r'))
                    ax.add_patch(Arrow(xplate2, ym, (strain * intensity_factor),
                      0, linkages_width, color='r'))
                else:
                    if strain > -1e-06:
                        ax.add_patch(Arrow((xplate1 - strain * intensity_factor), ym, (strain * intensity_factor),
                          0, linkages_width, color='r'))
                        ax.add_patch(Arrow((xplate2 + strain * intensity_factor), ym, (-strain * intensity_factor),
                          0, linkages_width, color='r'))
                    if abs(strain) > 1e-06:
                        ax.text(xplate1, (ym + 0.5 * linkages_width), ('{} N'.format(round(strain, 3))),
...
class UnidimensionalModel:
    ...
    def __init__(self, bodies, linear_linkages, nonlinear_linkages, loads, imposed_displacements):
        self.bodies = bodies
    ...
    def LinearSolve(self, activated_nonlinear_linkages=[], strains_tol=1.0):
        ...
        if abs(linalg.det(A)) < 1e-10:
            ...
            for ires, res in enumerate(npy.abs(residue)):
                if ires < ndof and res > strains_tol:
    ...
    def Solve(self, max_iters=100):
        ...
        while set(new_activated_nonlinear_linkages) != set(activated_nonlinear_linkages) and iters < max_iters:
            ...
            while not self.IsModelValid(activated_nonlinear_linkages + additional_linkages):
                disactivated_linkages = [l for l in self.nonlinear_linkages if l not in activated_nonlinear_linkages]
            ...
            for linkage in self.nonlinear_linkages:
                positions = (
                 result.positions[linkage.body1], result.positions[linkage.body2])
                ...
                elif linkage.ActivationCondition(positions, reactions):
                    new_activated_nonlinear_linkages.append(linkage)
    ...
    def Plot(self, activated_nonlinear_linkages=None, bodies_positions=None, strains={}, ax=None, color='k', linkages_width=0.006, intensity_factor=1e-06):
        ...
        for linear_linkages in self.linear_linkages + activated_nonlinear_linkages:
            linear_linkages.Plot(bodies_positions, strains=strains, ax=ax, color=color, linkages_width=linkages_width,
              intensity_factor=intensity_factor)

        for load in self.loads:
            load.Plot(bodies_positions, ax=ax, width=linkages_width, intensity_factor=intensity_factor)

        for imposed_displacement in self.imposed_displacements:
            imposed_displacement.Plot(bodies_positions, ax=ax, width=linkages_width, intensity_factor=intensity_factor)
...
class UnidimensionalModelResults:
    ...
    def Plot(self, intensity_factor=1e-06):
        fig, ax = plt.subplots()
        self.model.Plot((self.activated_nonlinear_linkages), bodies_positions=(self.positions),
