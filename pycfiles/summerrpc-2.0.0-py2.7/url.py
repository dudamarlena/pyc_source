...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/summerrpc/helper/url.py
# Compiled at: 2018-07-31 10:42:31
__all__ = [
 'URLBuilder', 'parse_query']
...
class URLBuilder(object):
    ...
    def with_port(self, port):
        if not isinstance(port, int):
            raise TypeError('expect int, not %s' % type(port).__name__)
        if port < 0:
            raise ValueError('port should be more than 0')
        self._port = port
        return self

    def with_path(self, path):
        if not isinstance(path, str):
            raise TypeError('expect str, not %s' % type(path).__name__)
