...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/summerrpc/helper/lru_cache.py
...
class Entry(object):
    ...
    def next(self, next):
        if not isinstance(next, self.__class__):
            raise TypeError('expect %s, not %s' % (
             self.__class__.__name__, type(next).__name__))
        self._next = next
...
class LRUCache(object):
    ...
    @head.setter
    def head(self, head):
        raise RuntimeError('head can not be overriden')
