...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/eggspacker/eggspacker.py
# Compiled at: 2011-12-01 06:38:48
import pkg_resources, os, shutil, weakref, urllib, urllib2, re, zipfile, tempfile
from urlparse import urlparse
index_pkg_link_re = re.compile('<a href="(?P<link>[^"\\\']+)">(?P<name>\\S+)</a>')
...
class EggFinder(object):
    ...
    def __init__(self, packer):
        ...
        self.env = MyEnvironment(search_path=[], platform=self.packer.platform, python=self.packer.python)
        ...
        if self.obtain_recurse_lock:
            ...
            try:
                if requirement.key in self.index_packages:
                    for name, link in self.read_links(self.index_packages[requirement.key]['link']):
    ...
    def index_packages(self):
        if self._index_packages is None:
            self._index_packages = dict((name.lower(), dict(name=name, link=link)) for name, link in self.read_links(self.packer.index))
...
class EggsPacker(object):
    ...
    def __init__(self, index=None, basket=None, python=None, platform=None, unzip=[]):
        self.index = index
        self.basket = basket
        self.python = python
        ...
        self.env = pkg_resources.Environment(search_path=self.basket, platform=self.platform, python=self.python)
    ...
    def obtain(self, requirement):
        ...
        if url:
            ...
            fname = os.path.join(self.downloaddir, os.path.basename(parsed_url.path))
    ...
    def need_unzip(self, dist):
        if 'all' in self.unzip or 'auto' in self.unzip and not dist.has_metadata('zip-safe') or dist.key in self.unzip:
            return True
        return False

    def add(self, filename):
        dist = pkg_resources.Distribution.from_filename(filename, metadata=pkg_resources.EggMetadata(pkg_resources.get_importer(filename)))
    ...
    def pack(self, egg):
        if isinstance(egg, basestring) and os.path.exists(egg):
            egg_dist = pkg_resources.Distribution.from_filename(egg, metadata=pkg_resources.EggMetadata(pkg_resources.get_importer(egg)))
