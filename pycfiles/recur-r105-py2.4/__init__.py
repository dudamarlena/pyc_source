...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/recur/__init__.py
# Compiled at: 2007-06-14 13:57:27
"""
...
def eachminute(startDate, seconds=0, endDate=None):
    ...
    if hasattr(startDate, 'time'):
        (days, zerotime) = sane_time(0, startDate.hour, startDate.minute, seconds)
        if days < 0 or zerotime < startDate.time():
            (days, zerotime) = sane_time(0, startDate.hour, startDate.minute + 1, seconds)
    else:
        (days, zerotime) = sane_time(0, 0, 0, seconds)
    startDate = sane_date(startDate.year, startDate.month, startDate.day + days)
    ...
    if hasattr(startDate, 'time'):
        ...
        if zerotime < startDate.time():
            if zerotime.hour < 23:
                ...
                startDate = sane_date(startDate.year, startDate.month, startDate.day + 1)
    else:
        zerotime = datetime.time(0, minutes, seconds)
    startDate = datetime.datetime.combine(startDate, zerotime)
    while endDate is None or startDate <= endDate:
...
def time_from_str(timeofday):
    atoms = timeofday.split(':')

    def pop_or_zero():
        try:
            return int(atoms.pop(0))
        except TypeError:
            raise ValueError("The supplied time '%s' could not be parsed." % timeofday)
        except IndexError:
            return 0
...
class Locale(object):
    ...
    """
    __module__ = __name__
    patterns = {byunits: ['([0-9]+) sec', '([0-9]+) min', '([0-9]+) hour', '([0-9]+) day', '([0-9]+) week', '([0-9]+) month', '([0-9]+) year'], eachday: '([\\S]+) (?:every|each) day', eachweekday: ['([\\S]+) (?:every|each) mon(?!th)', '([\\S]+) (?:every|each) tue', '([\\S]+) (?:every|each) wed', '([\\S]+) (?:every|each) thu', '([\\S]+) (?:every|each) fri', '([\\S]+) (?:every|each) sat', '([\\S]+) (?:every|each) sun'], eachweek: ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'], eachmonth: '(-?\\d+) (?:every|each) month', eachyear: ['^(dummy entry to line up indexing)$', '(?<!\\d\\d[/ \\-])(?:jan(?:uary)?|0?1)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:febr?(?:uary)?|0?2)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:mar(?:ch)?|0?3)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:apr(?:il)?|0?4)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:may|0?5)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:june?|0?6)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:july?|0?7)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:aug(?:ust)?|0?8)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:sept?(?:ember)?|0?9)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:oct(?:ober)?|10)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:nov(?:ember)?|11)[/ \\-]([0-9]+)', '(?<!\\d\\d[/ \\-])(?:dec(?:ember)?|12)[/ \\-]([0-9]+)'], singledate: '(\\d\\d\\d\\d)[/ \\-]([01]?\\d)[/ \\-]([0123]?\\d)'}
    regexes = {}

    def __init__(self):
        for (key, regSet) in self.patterns.items():
            if isinstance(regSet, list):
                self.regexes[key] = [ re.compile(x, re.IGNORECASE) for x in regSet ]
            else:
    ...
    def __call__(self, description):
        ...
        raise ValueError("The supplied description ('%s') could not be parsed." % description)
...
class Recurrence(object):
    ...
    def __init__(self, startDate=None, description='', endDate=None, locale=localeEnglish):
        """
