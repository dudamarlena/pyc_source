...
# [GCC 8.4.0]
# Embedded file name: /home/mw/dev/pixelcms-shop-server/shop/sale/serializers.py
# Compiled at: 2017-01-05 10:12:03
# Size of source mod 2**32: 10357 bytes
...
from .models import Cart, CartProduct, CartProductOptionValue, PaymentMethod, ShippingMethod, OrderProductOption, OrderProduct, Order, Customer
...
class CartDataProductSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CartProduct
        fields = ('pk', 'name', 'route', 'image', 'options', 'unit_price', 'quantity',
                  'subtotal')
    ...
    def get_image(self, obj):
        try:
            return obj.product.main_image.get_image(request=self.context['request'], version=shop_settings.SHOP_IMAGES_VERSIONS['product']['cart'])
...
class CartDataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Cart
        fields = ('pk', 'products', 'products_count', 'total', 'timestamp')
...
class AddToCartSerializer(serializers.Serializer):
    product = serializers.PrimaryKeyRelatedField(queryset=Product.objects.filter(published=True, category__published_with_parents=True, available=True))
    options = CartProductOptionValueSerializer(many=True)
    quantity = serializers.IntegerField(min_value=1, max_value=shop_settings.SHOP_QUANTITY_LIMIT)

    def validate(self, data):
        product_options_groups = data['product'].options.all()
        for o in data['options']:
            if o['options_group'] not in product_options_groups or o['option'] not in o['options_group'].options.all():
...
class CartChangeQuantitySerializer(serializers.Serializer):
    ...
    def __init__(self, *args, **kwargs):
        self.fields['product'].queryset = CartProduct.objects.filter(cart__customer=kwargs['context']['customer'], cart__pk=kwargs['context']['cart_pk'])
...
class PaymentMethodSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PaymentMethod
        fields = ('pk', 'name', 'price', 'has_gateway')
...
class ShippingMethodSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('pk', 'name', 'price', 'payment_methods')
...
class PlaceOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ('shipping_method', 'payment_method', 'bd', 'shipping_data_form',
                  'sd', 'additional_comment')
...
class OrdersOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ('order_id', 'created', 'total', 'status')
...
class OrdersSerializer(serializers.Serializer):
    ...
    def _pagination(self):
        if shop_settings.SHOP_ORDERS_PAGINATION_ON_PAGE:
            ...
            try:
                paginator = Paginator(Order.objects.filter(customer=self.instance), shop_settings.SHOP_ORDERS_PAGINATION_ON_PAGE)
    ...
    def get_orders(self, obj):
        ...
        return OrdersOrderSerializer(orders, many=True, context={'request': self.context['request']}).data

    def get_pagination(self, obj):
        if self._pagination and self._pagination.paginator.num_pages > 1:
            return {'count': self._pagination.paginator.count, 
...
class OrderPaymentMethodSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = PaymentMethod
        fields = ('name', 'price', 'additional_info')
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ('order_id', 'created', 'status', 'create_payment_eligible', 'shipping_method',
                  'payment_method', 'total', 'billing_data', 'shipping_data', 'additional_comment',
                  'products')
    ...
    def get_products(self, obj):
        return OrderProductSerializer(obj.products, many=True, context={'request': self.context['request']}).data