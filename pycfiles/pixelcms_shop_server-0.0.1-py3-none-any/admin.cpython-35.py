...
# [GCC 8.4.0]
# Embedded file name: /home/mw/dev/pixelcms-shop-server/shop/sale/admin.py
# Compiled at: 2017-01-10 17:57:10
# Size of source mod 2**32: 12329 bytes
from django.contrib import admin
from django.utils.translation import ugettext_lazy as _
from django.core.urlresolvers import reverse
from polymorphic.admin import PolymorphicInlineSupportMixin
from shop.payments.models import Payment
from .models import Customer, Cart, CartProduct, Order, OrderProduct, OrderBillingData, OrderShippingData
...
class CustomerOrderInline(admin.TabularInline):
    ...
    def order_link(self, obj):
        return '<a href="{}">{}</a>'.format(reverse('admin:shop_order_change', args=[obj.pk]), obj.order_id)

    order_link.short_description = _('Order ID')
    order_link.allow_tags = True
    model = Order
    extra = 0
    fields = ('order_link', 'created', 'total', 'shipping_method', 'payment_method',
              'status')
    readonly_fields = ('order_link', 'created', 'total', 'shipping_method', 'payment_method',
                       'status')
...
class CustomerCartInline(admin.TabularInline):
    ...
    def cart_link(self, obj):
        return '<a href="{}">{}</a>'.format(reverse('admin:shop_cart_change', args=[obj.pk]), str(obj.total))
...
class CustomerAdmin(admin.ModelAdmin):
    ...
    def user_link(self, obj):
        return '<a href="{}">{}</a>'.format(reverse('admin:auth_user_change', args=[obj.user.pk]), obj.user)
    ...
    fieldsets = (
     (
      None,
      {'fields': ('user_link', 'email')}),)
    inlines = (
     CustomerOrderInline, CustomerCartInline)
...
class CartProductInline(admin.TabularInline):
    ...
    def product_link(self, obj):
        return '<a href="{}">{}</a>'.format(reverse('admin:shop_product_change', args=[obj.product.pk]), obj.product)
    ...
    def product_image(self, obj):
        try:
            return '<img src="{}" alt="" />'.format(obj.product.main_image.image.original.version_generate('admin_thumbnail').url)
        ...
        for o in obj.options.all():
            ...
            if o.option.price_mod:
                ...
                if o.option.price_mod_percentage:
                    ...
                    suffix = ''
                output += ' ({}{}{})'.format(prefix, o.option.price_mod, suffix)
    ...
    model = CartProduct
    extra = 0
    fields = ('product_link', 'product_image', 'options', 'unit_price', 'quantity',
              'subtotal')
    readonly_fields = ('product_link', 'product_image', 'options', 'unit_price', 'quantity',
                       'subtotal')
...
class CartAdmin(admin.ModelAdmin):
    ...
    def customer_link(self, obj):
        if obj.customer:
            return '<a href="{}">{}</a>'.format(reverse('admin:shop_customer_change', args=[obj.customer.pk]), obj.customer)
        else:
            return ''
    ...
    search_fields = ('customer__user__username', 'customer__user__email', 'customer__user__first_name',
                     'customer__user__last_name')
    readonly_fields = ('id', 'created', 'customer_link', 'products_count', 'total')
    fieldsets = (
     (
      None,
      {'fields': ('id', 'created', 'customer_link')}),
     (
      None,
...
class OrderProductInline(admin.TabularInline):
        ...
        for o in obj.options.all():
            ...
            if o.price_mod:
                ...
                if o.price_mod_percentage:
                    ...
                    suffix = ''
                output += ' ({}{}{})'.format(prefix, o.price_mod, suffix)
            options_list.append(output)
    ...
    model = OrderProduct
    extra = 0
    fields = ('product_or_product_name', 'product_image', 'options', 'unit_price',
              'quantity', 'subtotal')
    readonly_fields = ('product_or_product_name', 'product_image', 'options', 'unit_price',
                       'quantity', 'subtotal')
...
class PaymentInlineAdmin(admin.TabularInline):
    ...
    def details_link(self, obj):
        return '<a href="{}">{}</a>'.format(reverse('admin:shop_payment_change', args=[obj.pk]), _('Details'))
    ...
    model = Payment
    extra = 0
    fields = ('created', 'total', 'success', 'gateway_name', 'internal_id', 'internal_status',
              'details_link')
    readonly_fields = ('created', 'total', 'success', 'gateway_name', 'internal_id',
...
class OrderAdmin(PolymorphicInlineSupportMixin, admin.ModelAdmin):
    ...
    list_display = ('order_id', 'customer_link', 'created', 'total', 'shipping_method',
                    'payment_method', 'status')
    list_filter = ('shipping_method', 'payment_method')
    readonly_fields = ('order_id', 'customer_link', 'created', 'products_total', 'customer_link',
                       'total')
    ...
    fieldsets = (
     (
      None,
      {'fields': ('order_id', 'customer_link', 'created', 'products_total', 'shipping_method', 'payment_method',
 'total', 'status')}),
     (
      None,
      {'classes': ('placeholder products-group', ), 
    ...
    class Media:
        js = [
         'admin/shop/sale/orderBillingShippingData.js']