...
# [GCC 8.4.0]
# Embedded file name: /home/mw/dev/pixelcms-shop-server/shop/sale/views_api.py
# Compiled at: 2017-01-12 16:28:47
# Size of source mod 2**32: 7601 bytes
...
from .models import Cart, CartProduct, CartProductOptionValue, Order, OrderProduct, OrderProductOption, OrderBillingData, OrderShippingData
...
def cart_change_quantity_view(request, cart_pk):
    if request.user.is_authenticated:
        ...
        customer = None
    serializer = CartChangeQuantitySerializer(data=request.data, context={'customer': customer, 
     'cart_pk': cart_pk})
    if serializer.is_valid(raise_exception=True):
...
def remove_from_cart_view(request, cart_pk):
    if request.user.is_authenticated:
        ...
        customer = None
    serializer = RemoveFromCartSerializer(data=request.data, context={'customer': customer, 
     'cart_pk': cart_pk})
    if serializer.is_valid(raise_exception=True):
        cart_product = serializer.validated_data['product']
...
def place_order_view(request, cart_pk):
    cart = get_object_or_404(Cart, customer=request.user.customer, pk=cart_pk)
    ...
    if serializer.is_valid(raise_exception=True):
        ...
        if serializer.validated_data['shipping_data_form']:
            ...
            sd = {'name': bd['name'], 
             ...
             'place': bd['place'], 
             'phone': bd.get('phone')}
        order = Order.objects.create(customer=cart.customer, shipping_method=shipping_method, payment_method=payment_method, status=order_status, additional_comment=serializer.validated_data.get('additional_comment'))
        for p in cart.products.all():
            order_product = OrderProduct.objects.create(order=order, product=p.product, product_name=p.product.name, unit_price=p.unit_price, quantity=p.quantity, subtotal=p.subtotal)
            for o in p.options.all():
                OrderProductOption.objects.create(order_product=order_product, options_group=o.options_group.name, option=o.option.name, price_mod=o.option.price_mod, price_mod_percentage=o.option.price_mod_percentage)
