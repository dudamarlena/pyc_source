...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/berrymq/berrymq.py
# Compiled at: 2009-10-16 09:53:03
import re, new, sys, copy, uuid, Queue as queue, heapq, types, weakref, traceback, threading, xmlrpclib, itertools
try:
    import functools
except ImportError:
    functools = None
...
class Identifier(object):
    ...
    _split_name_action = re.compile('(.*):(.*)')
    _split_namespace = re.compile('(.*)@(.*)')
    ...
    def __str__(self):
        return '<berrymq.Identifier object at %d: id=%s>' % (id(self), self.id_str())
    ...
    def _(func):
        ...
        def __(*args, **kwargs):
            try:
                ...
                _mqas.twitter(id_obj, [message], {'type': t, 'value': v, 'traceback': tb})
                if reraise:
                    raise
    ...
    def regist_queue(cls, filter_id, shared):
        ...
        if shared:
            if queues['shared'] is None:
                ...
                RootTransporter.regist_follower(id_obj, _weakmethod_ref(queue_obj._receive_message))
