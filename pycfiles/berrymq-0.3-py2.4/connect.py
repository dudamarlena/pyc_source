...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-i386/egg/berrymq/connect.py
# Compiled at: 2009-10-08 08:50:20
import time, uuid, socket, berrymq, jsonrpc.client, jsonrpc.server
...
class UnInitializedServer(RuntimeError):
    __module__ = __name__
    ...
    def send_get(cls, block, timeout):
        for connection in cls._connections.values():
            if isinstance(connection, QueueClientConnection):
                ...
                return berrymq.Message(berrymq.Identifier(idstr), args, kwargs, 100)
