...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/pymcaspec/pymcaspec.py
# Compiled at: 2019-10-03 15:36:51
# Size of source mod 2**32: 12899 bytes
...
class scan:
    __doc__ = 'Container for scan in spec file'
    ...
    def get_description(self):
        ...
        for dataobject in self.dataobjects:
            ...
            if type(BaselineNames) is not list:
                BaselineNames = [
                 'no baseline motors']
            motors_description += 'Key: {}\n'.format(dataobject.info['Key']) + 'Scanned Motors are:\n{}\n'.format('\t'.join(PrimaryNames)) + 'Baseline Motors are:\n{}\n\n'.format('\t'.join(BaselineNames))
    ...
    def index(self, key):
        ...
        """
        try:
            datacol = np.hstack([dataobject.data[:, key] for dataobject in self.dataobjects])
        except IndexError:
            PrimaryNames = self.dataobjects[0].info['LabelNames']
            try:
                index = [i for i, k in enumerate(PrimaryNames) if key == k][0]
                datacol = np.hstack([dataobject.data[:, index] for dataobject in self.dataobjects])
            except IndexError:
    ...
    def get_baseline(self, key):
        ...
        if len(self.dataobjects) > 1:
            motor_val = [dataobject.info['MotorValues'][index] for dataobject in self.dataobjects]
        else:
    ...
    def plot(self, ax=None, xkey=0, ykey=-1, monitor=None, **kwargs):
        ...
        for s in self:
            try:
                leg, art, ax = (s.plot_combined)(ax=ax, xkey=xkey, ykey=ykey, monitor=monitor, **kwargs)
            except IndexError:
    ...
    def plot_combined(self, ax=None, xkey=0, ykey=-1, monitor=None, **kwargs):
        ...
        art = (ax.plot)(xdata, ydata, **kwargs, **{'label': '{}'.format(label)})
...
class specfile:
    __doc__ = 'Container for specfile'
        ...
        try:
            ...
            try:
                ...
                if len(found_keys) == 1:
                    ...
                    if len(found_keys) > 1:
                        msg = 'key {} matches multiple '.format(key) + 'entries in file:\n{}'.format(found_keys)
    ...
    import PyMca5.PyMcaCore.SpecFileLayer as SpecFileLayer
