...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/managers/shaders.py
# Compiled at: 2020-05-03 00:26:57
# Size of source mod 2**32: 28770 bytes
"""
...
class ShadersManager(object):
    ...
    def set_project(self, project):
        ...
        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-shaders',
    ...
    def get_shaders_paths(self):
        ...
        return [path_utils.clean_path(os.path.join(artellapipe.AssetsMgr().get_assets_path(), p)) for p in shaders_paths]
        ...
        if asset:
            ...
            if not shaders_asset_file_type:
                ...
                if not shader_asset_file_class:
                    LOGGER.warning('Impossible to get shader path: {} | {} | {}'.format(shader_name, shaders_paths, shaders_asset_file_type))
                    return
    ...
    def load_shader(self, shader_name, shader_path=None, asset=None, apply=True, status=defines.ArtellaFileStatus.WORKING):
        """
        ...
        if not tp.is_maya():
            ...
            if shader_file:
                ...
                if not valid_shaders_paths:
                    LOGGER.debug('{} Shaders Library folder is not synchronized in your PC. Synchronize it please!'.format(self._project.name.title()))
                    return False
        ...
        if not tp.is_maya():
            ...
            for shape in shaded_shapes:
                if shape in updated_shapes:
                    LOGGER.warning('Shape "{}" already checked. Multiple shapes with same name in Asset Shaders File. Reexport shader again!'.format(shape))
                elif tp.Dcc.object_exists(shape):
                    updated_shapes[shape] = shape
                else:
                    shape_ns = tp.Dcc.node_namespace(shape, check_node=False)
                    if shape_ns:
                        LOGGER.warning('Shape "{}" already has a namespace defined in Asset Shaders File. Reexport shader without namespace!'.format(shape))
                    else:
            ...
            for shader_name in shader_names:
                valid_load = tp.Dcc.object_exists(shader_name) or self.load_shader(shader_name, asset=(asset_node.asset), apply=apply, status=status)
                if not valid_load:
            ...
            if apply_shaders:
                ...
                for i, (original_shape, updated_shape) in enumerate(updated_shapes.items()):
                    ...
                    if not asset_shape_operator:
                        ...
                        if not asset_shape_operator:
                            LOGGER.warning('Impossible to create Asset Shape Operator for "{} | {}"'.format(asset_node.id, updated_shape))
                            return False
                    for j, shader_to_apply in enumerate(shaders_to_apply):
                        if not tp.Dcc.object_exists(shader_to_apply):
                            ...
                            if shader_type == 'displacementShader':
                                asset_node.add_shape_operator_assignment(shape_name=updated_shape,
                                  assignment_value=("displacement = '{}'".format(shader_to_apply)))
                            else:
                                asset_node.add_shape_operator_assignment(shape_name=updated_shape,
                                  assignment_value=("shader = '{}'".format(shader_to_apply)))
    ...
    def unload_asset_shaders(self, asset_node):
        ...
        if not tp.is_maya():
            ...
            if not shaders_mapping_file:
                ...
                shader_names or LOGGER.warning('No shaders to unload found!')
                return False
        found_shaders = list()
            ...
            for shader_name in shaders_list:
                for set_node in arnold_set_parameters:
                    ...
                    for index in indices:
                        ...
                        if assign_value:
                            if shader_name in assign_value:
                            ...
                            valid_remove = asset_node.remove_shape_operator_assignment(assign_value,
                              shape_name=asset_shape)
                            if valid_remove:
    ...
    def get_asset_shaders_to_export(self, asset, return_only_shaders=True, skip_standard_shaders=True):
        ...
        if not valid_open:
            ...
            asset_shaders = self.get_asset_shaders(asset=asset,
        ...
        if not asset:
            ...
            if new_version:
                if os.path.isfile(file_path):
                    ...
                    valid_upload = artellapipe.FilesMgr().upload_working_version(file_path=file_path,
                      skip_saving=True,
                      comment=comment)
                    if valid_upload:
                        artellapipe.FilesMgr().unlock_file(file_path, warn_user=False)

    def export_asset_shaders(self, asset, comment=None, new_version=False, shader_swatch=None, shaders_to_export=None):
        """
        ...
        if not asset:
            ...
            return self.export_shaders(shaders_names=valid_shaders,
              export_path=shaders_path,
              comment=comment,
              new_version=new_version,
              asset=asset,
              shader_swatch=shader_swatch)

    def export_shader(self, shader_name, export_path=None, comment=None, new_version=False, shader_swatch=None, asset=None):
        """
        Exports shaders
        ...
        if not export_path:
            ...
            if not export_path or not os.path.exists(export_path):
                LOGGER.warning('Shader Export Path "{}" does not exists. Aborting shader "{}"  export!'.format(export_path, shader_name))
                return
            else:
                if not os.path.exists(export_path):
                    ...
                    if not tp.Dcc.object_exists(shader_name):
                        ...
                        shader_file or LOGGER.warning('Impossible to export shader "{}"!'.format(shader_name))
                        return
                exported_shader = shader_file.export_file(shader_swatch=shader_swatch, status=(defines.ArtellaFileStatus.WORKING))
                exported_shader or LOGGER.warning('Impossible to export shader "{}"'.format(shader_name))
            return
        else:
            if new_version:
    ...
    def export_shaders(self, shaders_names, export_path=None, comment=None, new_version=False, asset=None, shader_swatch=None):
        """
        ...
        if not shaders_names:
            ...
            for shader_name in shaders_names:
                exported_shader = self.export_shader(shader_name=shader_name,
