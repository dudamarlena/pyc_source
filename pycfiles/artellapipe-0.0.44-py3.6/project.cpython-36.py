...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/project.py
# Compiled at: 2020-05-03 00:26:57
# Size of source mod 2**32: 36967 bytes
"""
...
import os, sys, time, logging, tempfile, datetime, importlib, traceback, webbrowser
...
class ArtellaProject(object):
    ...
    def __init__(self, name, settings=None):
        ...
        try:
            ...
            tp.ConfigsMgr().register_package_configs(clean_name, os.path.dirname(project_config_mod.__file__))
        except RuntimeError:
            artellapipe.logger.warning('No Configuration Module found for project: {}'.format(clean_name))

        self._config = tp.ConfigsMgr().get_config(config_name='artellapipe-project',
          ...
          config_dict={'title':clean_name.title(), 
    ...
    def get_changelog_path(self):
        ...
        :return: str
        """
        return path_utils.clean_path(os.path.join(self.get_project_path(), defines.ARTELLA_PROJECT_CHANGELOG_FILE_NAME))
    ...
    def get_shelf_path(self):
        ...
        :return: str
        """
        return path_utils.clean_path(os.path.join(self.get_configurations_folder(), defines.ARTELLA_PROJECT_SHELF_FILE_NAME))
    ...
    def get_menu_path(self):
        ...
        :return: str
        """
        return path_utils.clean_path(os.path.join(self.get_configurations_folder(), defines.ARTELLA_PROJECT_MENU_FILE_NAME))
    ...
    def get_version_path(self):
        ...
        try:
            ...
            mod_path = os.path.join(os.path.dirname(os.path.abspath(mod.__file__)), defines.ARTELLA_PROJECT_DEFAULT_VERSION_FILE_NAME)
            if os.path.isfile(mod_path):
                return mod_path
            else:
                return path_utils.clean_path(os.path.join(self.get_configurations_folder(), defines.ARTELLA_PROJECT_DEFAULT_VERSION_FILE_NAME))
    ...
    def set_environment_variables(self):
        ...
        try:
            if tp.Dcc.get_name() == tp.Dccs.Unknown:
                ...
                if artella_var and os.path.exists(artella_var):
                    os.environ[self.env_var] = '{}{}/{}/{}/'.format(artella_var, production_folder, self.id_number, self.id)
                else:
                    LOGGER.warning('Impossible to set Artella environment variable!')
        except Exception as e:
            LOGGER.debug('Error while setting {0} Environment Variables. {0} Pipeline Tools may not work properly!'.format(self.name.title()))
        ...
        project_temp_folder = path_utils.clean_path(os.path.join(tempfile.gettempdir(), '{}_temp'.format(self.get_clean_name())))
    ...
    def get_drive(self):
        ...
        if not project_path or not os.path.isdir(project_path):
            LOGGER.warning('Impossible to retrieve project path drive because project path does not exists: "{}"'.format(project_path))
            return
        else:
    ...
    def update_project(self):
        ...
        try:
            if tp.is_maya():
                ...
                if project_folder:
                    if project_folder != 'folder-not-defined':
    ...
    def get_progress_bar(self):
        ...
        new_progress_bar.setStyleSheet('QProgressBar {border: 0px solid grey; border-radius:4px; padding:0px} QProgressBar::chunk {background: qlineargradient(x1: 0, y1: 1, x2: 1, y2: 1, stop: 0 rgb(' + self.progress_bar.color0 + '), stop: 1 rgb(' + self.progress_bar.color1 + ')); }')
            ...
            if tp.Dcc != tp.Dccs.Unknown:
                ...
                if main_win:
                    if hasattr(main_win, 'menuBar'):
                        ...
                        menubar.setStyleSheet('QMenuBar { background: qlineargradient(x1: 0, y1: 1, x2: 1, y2: 1, stop: 0 rgb(' + self.dev_color0 + '), stop: 1 rgb(' + self.dev_color1 + ')); }')
