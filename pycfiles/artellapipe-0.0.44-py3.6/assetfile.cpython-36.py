...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/core/assetfile.py
...
class ArtellaAssetFile(file.ArtellaFile, object):
    ...
    def get_file(self, status=defines.ArtellaFileStatus.WORKING, extension=None, fix_path=False, version=None, **kwargs):
        ...
        return self._asset.get_file(file_type=(self.FILE_TYPE),
          status=status,
          extension=extension,
          fix_path=fix_path,
          version=version,
          extra_dict=template_dict)
    ...
    def get_file_paths(self, return_first=False, fix_path=True, **kwargs):
        if self.FILE_TYPE not in self._asset.FILES:
            LOGGER.warning('FileType "{}" is not a valid file for Assets of type "{}"'.format(self.FILE_TYPE, self._asset.FILE_TYPE))
            return list()
        else:
            file_paths = (super(ArtellaAssetFile, self).get_file_paths)(return_first=return_first, fix_path=fix_path, **kwargs)
            if file_paths:
                return file_paths
