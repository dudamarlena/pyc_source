...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snurtle/bundles/common.py
# Compiled at: 2012-11-02 10:13:30
...
PERFORMANCE_TEMPLATE = "<%   \n    COMMAND_LENGTH_LIMIT = 30 \n    command = report[ 'command' ]\n    if len( command ) > COMMAND_LENGTH_LIMIT:\n       command = command[ :COMMAND_LENGTH_LIMIT - 1 ]\n       command += '_'\n    command   = '{:<30}'.format( command )\n    counter   = '{:>8}'.format( report[ 'counter' ] )\n    errors    = '{:>5}'.format( report[ 'errors' ] )\n    maxtime   = '{:>8}'.format( round( report[ 'max' ], 2) )\n    mintime   = '{:>6}'.format( round( report[ 'min' ], 2) )\n    totaltime = '{:>10}'.format( round( report[ 'total' ], 2) )\n    average = round( report[ 'total' ] / report[ 'counter' ], 2 )\n    average   = '{:>6}'.format( average )\n%>${command} ${counter} ${errors} ${maxtime} ${mintime} ${totaltime} ${average}"
...
class CommonCLIBundle(CLIBundle):
    ...
    def do_set_property(self, arg, opts=None):
        ...
        if response:
            prop = {'entityName': 'objectProperty', 'parentobjectid': opts.objectid, 'namespace': opts.namespace, 
    ...
    def do_set_acl(self, arg, opts=None):
        ...
        if response:
            ...
            payload = {'parentObjectId': opts.objectid, 'entityName': 'acl', 
    ...
    def do_lock(self, arg, opts=None):
        ...
        if opts.delete:
            operations += 'd'
        e = {'targetObjectId': opts.objectid, 'entityName': 'lock', 
    ...
    def do_unlock(self, arg, opts=None):
        ...
        callid = self.server.delete_object({'entityName': 'lock', 'targetObjectId': opts.objectid}, callback=self.callback)
        response = self.get_response(callid)
        if response:
            self.set_result(response, template=LOCK_TEMPLATE)
