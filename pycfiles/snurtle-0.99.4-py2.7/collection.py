...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snurtle/bundles/collection.py
...
class CollectionCLIBundle(CLIBundle):
    ...
    def do_list_collections(self, arg, opts=None):
        """List collections"""
        if opts.favorite:
            callid = self.server.get_favorites(entity_name='Collection', detail_level=0, callback=self.callback)
        else:
            callid = self.server.search_for_objects(entity='Collection', criteria='list', detail=0, callback=self.callback)
    ...
    def do_create_collection(self, arg, opts=None):
        ...
        if opts.projectid:
            values['projectid'] = opts.projectid
        if opts.kind:
            values['kind'] = opts.kind
        if opts.davenabled:
            values['davenabled'] = True
    ...
    def do_collect(self, arg, opts=None):
        ...
        if response:
            values = {'objectid': 0, 'entityname': 'collectionAssignment', 'collectionObjectId': opts.collectionid, 
    ...
    def do_uncollect(self, arg, opts=None):
        response = self._get_entity(opts.objectid)
        if response:
            values = {'entityname': 'collectionAssignment', 'collectionObjectId': opts.collectionid, 'assignedObjectId': opts.objectid}
