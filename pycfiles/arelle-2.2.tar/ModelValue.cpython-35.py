...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ModelValue.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 27686 bytes
"""
...
def qname(value, name=None, noPrefixIsNoNamespace=False, castException=None, prefixException=None):
    ...
    if namespaceDict and prefix in namespaceDict:
...
def dateTime(value, time=None, addOneDay=None, type=None, castException=None):
    ...
    if isinstance(value, ModelObject):
        ...
        if isinstance(value, DateTime) and not addOneDay and value.dateOnly == (type == DATE):
            return value
    ...
    if type == DATE:
        ...
        if castException and not isinstance(value, _STR_BASE):
...
def dateunionDate(datetimeValue, subtractOneDay=False):
    ...
    d = datetimeValue
    if (isDate or d.hour == 0 and d.minute == 0 and d.second == 0) and subtractOneDay and not isDate:
...
def isoDuration(value):
    ...
    return IsoDuration(years=groups['years'], months=groups['months'], days=groups['days'], hours=groups['hours'], minutes=groups['minutes'], seconds=groups['seconds'], weeks=groups['weeks'], negate=groups['sign'] == '-', sourceValue=value)
    ...
    def __lt__(self, other):
        ...
        if self.avgdays == other.avgdays:
            if self.tdelta.seconds < other.tdelta.seconds:
                return True
            if self.tdelta.seconds == other.tdelta.seconds and self.tdelta.microseconds < other.tdelta.microseconds:
                pass
            return True
        return False
