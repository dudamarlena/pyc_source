...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ValidateFormula.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 89050 bytes
"""
            ...
            for modelRel in relsSet.modelRelationships:
                ...
                if fromQname:
                    if fromMdlObj is None or not val.modelXbrl.isInSubstitutionGroup(fromMdlObj.elementQname, fromQname) and fromMdlObj.elementQname.namespaceURI in val.modelXbrl.namespaceDocs:
                        val.modelXbrl.log(level, fromErrCode, _('Relationship from %(xlinkFrom)s to %(xlinkTo)s should have an %(element)s source'), modelObject=modelRel, xlinkFrom=modelRel.fromLabel, xlinkTo=modelRel.toLabel, element=fromQname)
                    elif notFromQname and val.modelXbrl.isInSubstitutionGroup(fromMdlObj.elementQname, notFromQname):
                        val.modelXbrl.log(level, notFromErrCode, _('Relationship from %(xlinkFrom)s to %(xlinkTo)s should not have an %(element)s source'), modelObject=modelRel, xlinkFrom=modelRel.fromLabel, xlinkTo=modelRel.toLabel, element=fromQname)
                    if toQname:
                        if toMdlObj is None or not val.modelXbrl.isInSubstitutionGroup(toMdlObj.elementQname, toQname) and toMdlObj.elementQname.namespaceURI in val.modelXbrl.namespaceDocs:
                            val.modelXbrl.log(level, toErrCode, _('Relationship from %(xlinkFrom)s to %(xlinkTo)s should have an %(element)s target'), modelObject=modelRel, xlinkFrom=modelRel.fromLabel, xlinkTo=modelRel.toLabel, element=toQname)
                        elif notToQname and val.modelXbrl.isInSubstitutionGroup(fromMdlObj.elementQname, notToQname):
                            val.modelXbrl.log(level, notFromErrCode, _('Relationship from %(xlinkFrom)s to %(xlinkTo)s should not have an %(element)s target'), modelObject=modelRel, xlinkFrom=modelRel.fromLabel, xlinkTo=modelRel.toLabel, element=fromQname)

        if arcrole == XbrlConst.functionImplementation:
            for relFrom, rels in relsSet.fromModelObjects().items():
                if len(rels) > 1:
                    val.modelXbrl.error('xbrlcfie:tooManyCFIRelationships', _('Function-implementation relationship from signature %(name)s has more than one implementation target'), modelObject=[
                     relFrom] + rels, name=relFrom.name)

            for relTo, rels in relsSet.toModelObjects().items():
                if len(rels) > 1:
                    val.modelXbrl.error('xbrlcfie:tooManyCFIRelationships', _('Function implementation %(xlinkLabel)s must be the target of only one function-implementation relationship'), modelObject=[
                     relTo] + rels, xlinkLabel=relTo.xlinkLabel)

    if arcrole == XbrlConst.assertionUnsatisfiedSeverity:
        for relFrom, rels in relsSet.fromModelObjects().items():
            if len(rels) > 1:
                val.modelXbrl.error('seve:multipleSeveritiesForAssertionError', _('Assertion-unsatisfied-severity relationship from %(xlinkLabel)s has more than one severity target'), modelObject=[
                 relFrom] + rels, xlinkLabel=relFrom.xlinkLabel)
        ...
        for relTo, rels in relsSet.toModelObjects().items():
            if relTo.modelDocument.basename != 'severities.xml' or relTo.getparent().qname != XbrlConst.qnGenLink or relTo.getparent().getparent().qname != XbrlConst.qnLinkLinkbase:
                val.modelXbrl.error('seve:assertionSeverityTargetError', _('Target of assertion-unsatisfied-severity relationship must be a severity element in the published severities linkbase.'), modelObject=[
                 relTo] + rels)
...
def executeCallTest(val, name, callTuple, testTuple):
    if callTuple:
        ...
        try:
            ...
            val.modelXbrl.info('formula:trace', _('%(name)s result %(result)s'), modelObject=callTuple[1], name=name, result=str(result))
            if testTuple:
                ...
                if testResult:
                    val.modelXbrl.info('cfcn:testPass', _('Test %(name)s result %(result)s'), modelObject=testTuple[1], name=name, result=str(testResult))
            else:
                val.modelXbrl.error('cfcn:testFail', _('Test %(name)s result %(result)s'), modelObject=testTuple[1], name=name, result=str(testResult))
            xpathContext.close()
        except XPathContext.XPathException as err:
            val.modelXbrl.error(err.code, _('%(name)s evaluation error: %(error)s \n%(errorSource)s'), modelObject=callTuple[1], name=name, error=err.message, errorSource=err.sourceErrorIndication)
...
def validate(val, xpathContext=None, parametersOnly=False, statusMsg='', compileOnly=False):
    for e in ('xbrl.5.1.4.3:cycles', 'xbrlgene:violatedCyclesConstraint'):
        if e in val.modelXbrl.errors:
            val.modelXbrl.info('info', _('Formula validation skipped due to %(error)s error'), modelObject=val.modelXbrl, error=e)
            return
    ...
    for paramQname in parameterQnames:
        if paramQname not in dependencyResolvedParameters:
            ...
            if len(undefinedVars) > 0:
                val.modelXbrl.error('xbrlve:unresolvedDependency', _('Undefined dependencies in parameter %(name)s, to names %(dependencies)s'), modelObject=val.modelXbrl.qnameParameters[paramQname], name=paramQname, dependencies=', '.join(str(v) for v in undefinedVars))
            if len(paramsCircularDep) > 0:
                val.modelXbrl.error('xbrlve:parameterCyclicDependencies', _('Cyclic dependencies in parameter %(name)s, to names %(dependencies)s'), modelObject=val.modelXbrl.qnameParameters[paramQname], name=paramQname, dependencies=', '.join(str(d) for d in paramsCircularDep))
    ...
    for custFnSig in val.modelXbrl.modelCustomFunctionSignatures.values():
        if custFnSig is not None:
            ...
            if custFnQname.namespaceURI == XbrlConst.xfi:
                val.modelXbrl.error('xbrlve:noProhibitedNamespaceForCustomFunction', _('Custom function %(name)s has namespace reserved for functions in the function registry %(namespace)s'), modelObject=custFnSig, name=custFnQname, namespace=custFnQname.namespaceURI)
            ...
            if _outputType and _outputType.namespaceURI == XbrlConst.xsd and not FunctionXs.isXsType(_outputType.localName):
                val.modelXbrl.error('xbrlve:invalidDatatypeInCustomFunctionSignature', _('Custom Function Signature %(name)s output type %(type)s is not valid'), modelObject=custFnSig, name=custFnQname, type=_outputType)
            for _inputType in custFnSig.inputTypes:
                if _inputType and _inputType.namespaceURI == XbrlConst.xsd and not FunctionXs.isXsType(_inputType.localName):
                    val.modelXbrl.error('xbrlve:invalidDatatypeInCustomFunctionSignature', _('Custom Function Signature %(name)s input type %(type)s is not valid'), modelObject=custFnSig, name=custFnQname, type=_inputType)
            ...
            for modelRel in val.modelXbrl.relationshipSet(XbrlConst.functionImplementation).fromModelObject(custFnSig):
                ...
                if len(custFnImpl.inputNames) != len(custFnSig.inputTypes):
                    val.modelXbrl.error('xbrlcfie:inputMismatch', _('Custom function %(name)s signature has %(parameterCountSignature)s parameters but implementation has %(parameterCountImplementation)s, must be matching'), modelObject=custFnSig, name=custFnQname, parameterCountSignature=len(custFnSig.inputTypes), parameterCountImplementation=len(custFnImpl.inputNames))
    ...
    for custFnImpl in val.modelXbrl.modelCustomFunctionImplementations:
        if not val.modelXbrl.relationshipSet(XbrlConst.functionImplementation).toModelObject(custFnImpl):
            val.modelXbrl.error('xbrlcfie:missingCFIRelationship', _('Custom function implementation %(xlinkLabel)s has no relationship from any custom function signature'), modelObject=custFnSig, xlinkLabel=custFnImpl.xlinkLabel)
        custFnImpl.compile()
    ...
    for paramQname in orderedParameters:
        ...
        if not isinstance(modelParameter, ModelInstance):
            ...
            if asType and asType.namespaceURI == XbrlConst.xsd and not FunctionXs.isXsType(asType.localName):
                val.modelXbrl.error('xbrlve:parameterTypeMismatch', _('Parameter %(name)s type %(type)s is not valid'), modelObject=modelParameter, name=paramQname, type=asType)
                ...
                if val.parameters and paramQname in val.parameters:
                    ...
                    if isinstance(paramDataType, str):
                        ...
                        if formulaOptions.traceParameterInputValue:
                            val.modelXbrl.info('formula:trace', _('Parameter %(name)s input value %(input)s'), modelObject=modelParameter, name=paramQname, input=result)
                    xpathContext.inScopeVars[paramQname] = result
                else:
                    if modelParameter.isRequired:
                        val.modelXbrl.error('xbrlve:missingParameterValue', _('Parameter %(name)s is required but not input'), modelObject=modelParameter, name=paramQname)
                    else:
                        if not modelParameter.selectProg:
                            val.modelXbrl.error('xbrlve:missingParameterValue', _('Parameter %(name)s does not have a select attribute'), modelObject=modelParameter, name=paramQname)
                        else:
                            result = modelParameter.evaluate(xpathContext, asType)
                            if formulaOptions.traceParameterExpressionResult:
                                val.modelXbrl.info('formula:trace', _('Parameter %(name)s select result %(result)s'), modelObject=modelParameter, name=paramQname, result=result)
                            xpathContext.inScopeVars[paramQname] = result
            except XPathContext.XPathException as err:
                val.modelXbrl.error('xbrlve:parameterTypeMismatch' if err.code == 'err:FORG0001' else err.code, _('Parameter \n%(name)s \nException: \n%(error)s'), modelObject=modelParameter, name=paramQname, error=err.message, messageCodes=('xbrlve:parameterTypeMismatch',
                                                                                                                                                                                                                                                  'err:FORG0001'))
    ...
    for modelRel in val.modelXbrl.relationshipSet(XbrlConst.equalityDefinition).modelRelationships:
        ...
        if typedDomainElt in val.modelXbrl.modelFormulaEqualityDefinitions:
            val.modelXbrl.error('xbrlve:multipleTypedDimensionEqualityDefinitions', _('Multiple typed domain definitions from %(typedDomain)s to %(equalityDefinition1)s and %(equalityDefinition2)s'), modelObject=modelRel.arcElement, typedDomain=typedDomainElt.qname, equalityDefinition1=modelEqualityDefinition.xlinkLabel, equalityDefinition2=val.modelXbrl.modelFormulaEqualityDefinitions[typedDomainElt].xlinkLabel)
        else:
    ...
    for modelVariableSet in val.modelXbrl.modelVariableSets:
        ...
        if isinstance(modelVariableSet, ModelFormula):
            ...
            for modelRel in val.modelXbrl.relationshipSet(XbrlConst.formulaInstance).fromModelObject(modelVariableSet):
                ...
                if isinstance(instance, ModelInstance):
                    if instanceQname is None:
                        ...
                        val.modelXbrl.info('arelle:multipleOutputInstances', _('Multiple output instances for formula %(xlinkLabel)s, to names %(instanceTo)s, %(instanceTo2)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, instanceTo=instanceQname, instanceTo2=instance.instanceQname)
            ...
            if getattr(val, 'validateSBRNL', False):
                val.modelXbrl.error('SBR.NL.2.3.9.03', _('Formula:formula %(xlinkLabel)s is not allowed'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel)
        else:
            instanceQname = None
        ...
        if modelVariableSet.aspectModel not in ('non-dimensional', 'dimensional'):
            val.modelXbrl.error('xbrlve:unknownAspectModel', _('Variable set %(xlinkLabel)s, aspect model %(aspectModel)s not recognized'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, aspectModel=modelVariableSet.aspectModel)
        ...
        for modelRel in val.modelXbrl.relationshipSet(XbrlConst.variableSet).fromModelObject(modelVariableSet):
            ...
            if varqname:
                ...
                if varqname not in nameVariables:
                    ...
                    val.modelXbrl.error('xbrlve:duplicateVariableNames', _('Multiple variables named %(xlinkLabel)s in variable set %(name)s'), modelObject=toVariable, xlinkLabel=modelVariableSet.xlinkLabel, name=varqname)
                ...
                if fromInstanceQnames is None:
                    ...
                    val.modelXbrl.error('xbrlve:variableNameResolutionFailure', _('Variables name %(name)s cannot be determined on arc from %(xlinkLabel)s'), modelObject=modelRel, xlinkLabel=modelVariableSet.xlinkLabel, name=modelRel.variablename)
        ...
        for modelRel in val.modelXbrl.relationshipSet(XbrlConst.variableSet).fromModelObject(modelVariableSet):
            ...
            if isinstance(variable, (ModelParameter, ModelVariable)):
                ...
                if len(depVars) > 0 and formulaOptions.traceVariablesDependencies:
                    val.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s, variable %(name)s, dependences %(dependencies)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, name=varqname, dependencies=depVars)
                definedNamesSet.add(varqname)
                if isinstance(variable, ModelFactVariable):
                    ...
                    for depVar in XPathParser.variableReferencesSet(variable.fallbackValueProg, variable):
                        if depVar in qnameRels and isinstance(qnameRels[depVar].toModelObject, ModelVariable):
                            val.modelXbrl.error('xbrlve:fallbackValueVariableReferenceNotAllowed', _("Variable set %(xlinkLabel)s fallbackValue '%(fallbackValue)s' cannot refer to variable %(dependency)s"), modelObject=variable, xlinkLabel=modelVariableSet.xlinkLabel, fallbackValue=variable.fallbackValue, dependency=depVar)
        ...
        for varqname, depVars in variableDependencies.items():
            if varqname not in orderedNameSet:
                ...
                if len(undefinedVars) > 0:
                    val.modelXbrl.error('xbrlve:unresolvedDependency', _('Undefined variable dependencies in variable set %(xlinkLabel)s, from variable %(nameFrom)s to %(nameTo)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, nameFrom=varqname, nameTo=undefinedVars)
                if len(varsCircularDep) > 0:
                    val.modelXbrl.error('xbrlve:cyclicDependencies', _('Cyclic dependencies in variable set %(xlinkLabel)s, from variable %(nameFrom)s to %(nameTo)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, nameFrom=varqname, nameTo=varsCircularDep)
        ...
        for varSetDepVarQname in modelVariableSet.variableRefs():
            if varSetDepVarQname not in definedNamesSet and varSetDepVarQname not in parameterQnames:
                val.modelXbrl.error('xbrlve:unresolvedDependency', _('Undefined variable dependency in variable set %(xlinkLabel)s, %(name)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, name=varSetDepVarQname)
        ...
        if formulaOptions.traceVariablesOrder:
            val.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s, variables order: %(dependencies)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, dependencies=orderedNameList)
        if formulaOptions.traceVariablesDependencies and len(varSetInstanceDependencies) > 0 and varSetInstanceDependencies != {XbrlConst.qnStandardInputInstance}:
            val.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s, instance dependences %(dependencies)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, dependencies=varSetInstanceDependencies)
        ...
        if isinstance(modelVariableSet, ModelExistenceAssertion):
            for depVar in XPathParser.variableReferencesSet(modelVariableSet.testProg, modelVariableSet):
                if depVar in qnameRels and isinstance(qnameRels[depVar].toModelObject, ModelVariable):
                    val.modelXbrl.error('xbrleae:variableReferenceNotAllowed', _('Existence Assertion %(xlinkLabel)s, cannot refer to variable %(name)s'), modelObject=modelVariableSet, xlinkLabel=modelVariableSet.xlinkLabel, name=depVar)
        ...
        for modelRel in val.modelXbrl.relationshipSet(XbrlConst.variableSetFilter).fromModelObject(modelVariableSet):
            ...
            if modelRel.isCovered:
                val.modelXbrl.warning('arelle:variableSetFilterCovered', _('Variable set %(xlinkLabel)s, filter %(filterLabel)s, cannot be covered'), modelObject=varSetFilter, xlinkLabel=modelVariableSet.xlinkLabel, filterLabel=varSetFilter.xlinkLabel)
                modelRel._isCovered = False
            for depVar in varSetFilter.variableRefs():
                if depVar in qnameRels and isinstance(qnameRels[depVar].toModelObject, ModelVariable):
                    val.modelXbrl.error('xbrlve:factVariableReferenceNotAllowed', _('Variable set %(xlinkLabel)s, filter %(filterLabel)s, cannot refer to variable %(name)s'), modelObject=varSetFilter, xlinkLabel=modelVariableSet.xlinkLabel, filterLabel=varSetFilter.xlinkLabel, name=depVar)
    ...
    for modelTable in val.modelXbrl.modelRenderingTables:
        ...
        if modelTable.aspectModel not in ('non-dimensional', 'dimensional'):
            val.modelXbrl.error('xbrlte:unknownAspectModel', _('Table %(xlinkLabel)s, aspect model %(aspectModel)s not recognized'), modelObject=modelTable, xlinkLabel=modelTable.xlinkLabel, aspectModel=modelTable.aspectModel)
        modelTable.compile()
        ...
        if instqname not in orderedInstancesSet:
            ...
            if instqname:
                if missingDependentInstances:
                    val.modelXbrl.error('xbrlvarinste:instanceVariableRecursionCycle', _('Cyclic dependencies of instance %(name)s produced by a formula, with variables consuming instances %(dependencies)s'), modelObject=val.modelXbrl, name=instqname, dependencies=missingDependentInstances)
        elif instqname == XbrlConst.qnStandardOutputInstance:
            orderedInstancesSet.add(instqname)
            orderedInstancesList.append(instqname)
            continue
            if instqname in depInsts:
                val.modelXbrl.error('xbrlvarinste:instanceVariableRecursionCycle', _('Cyclic dependencies of instance %(name)s produced by its own variables'), modelObject=val.modelXbrl, name=instqname)

    if formulaOptions.traceVariablesOrder and len(orderedInstancesList) > 1:
        val.modelXbrl.info('formula:trace', _('Variable instances processing order: %(dependencies)s'), modelObject=val.modelXbrl, dependencies=orderedInstancesList)
    for modelRel in val.modelXbrl.relationshipSet(XbrlConst.consistencyAssertionFormula).modelRelationships:
        if isinstance(modelRel.fromModelObject, ModelConsistencyAssertion) and isinstance(modelRel.toModelObject, ModelFormula):
            ...
            if consisAsser.hasProportionalAcceptanceRadius and consisAsser.hasAbsoluteAcceptanceRadius:
                val.modelXbrl.error('xbrlcae:acceptanceRadiusConflict', _('Consistency assertion %(xlinkLabel)s has both absolute and proportional acceptance radii'), modelObject=consisAsser, xlinkLabel=consisAsser.xlinkLabel)
            ...
            for consisParamRel in val.modelXbrl.relationshipSet(XbrlConst.consistencyAssertionParameter).fromModelObject(consisAsser):
                if isinstance(consisParamRel.toModelObject, ModelVariable):
                    val.modelXbrl.error('xbrlcae:variablesNotAllowed', _('Consistency assertion %(xlinkLabel)s has relationship to a %(elementTo)s %(xlinkLabelTo)s'), modelObject=consisAsser, xlinkLabel=consisAsser.xlinkLabel, elementTo=consisParamRel.toModelObject.localName, xlinkLabelTo=consisParamRel.toModelObject.xlinkLabel)
            ...
            for varScopeRel in val.modelXbrl.relationshipSet(XbrlConst.variablesScope).toModelObject(modelVariableSet):
                if isinstance(varScopeRel.fromModelObject, ModelVariableSet):
                    ...
                    if sourceVariableSet.outputInstanceQname != instanceQname:
                        val.modelXbrl.error('xbrlvarscopee:differentInstances', _('Variable set %(xlinkLabel1)s in instance %(instance1)s has variables scope relationship to varaible set %(xlinkLabel2)s in instance %(instance2)s'), modelObject=modelVariableSet, xlinkLabel1=sourceVariableSet.xlinkLabel, instance1=sourceVariableSet.outputInstanceQname, xlinkLabel2=modelVariableSet.xlinkLabel, instance2=modelVariableSet.outputInstanceQname)
                    if sourceVariableSet.aspectModel != modelVariableSet.aspectModel:
                        val.modelXbrl.error('xbrlvarscopee:conflictingAspectModels', _('Variable set %(xlinkLabel1)s aspectModel (%(aspectModel1)s) differs from varaible set %(xlinkLabel2)s aspectModel (%(aspectModel2)s)'), modelObject=modelVariableSet, xlinkLabel1=sourceVariableSet.xlinkLabel, aspectModel1=sourceVariableSet.aspectModel, xlinkLabel2=modelVariableSet.xlinkLabel, aspectModel2=modelVariableSet.aspectModel)
    ...
    for instanceQname in instanceQnames:
        if instanceQname == XbrlConst.qnStandardInputInstance:
            ...
            if val.parameters and instanceQname in val.parameters:
                ...
                uri = uri + '.xml'
                namedInstance = ModelXbrl.create(val.modelXbrl.modelManager, newDocumentType=ModelDocument.Type.INSTANCE, url=uri, schemaRefs=schemaRefs, isEntry=True)
                ValidateXbrlDimensions.loadDimensionDefaults(namedInstance)
    ...
    if runIDs:
        val.modelXbrl.info('formula:trace', _('Formua/assertion IDs restriction: %(ids)s'), modelXbrl=val.modelXbrl, ids=', '.join(runIDs))
    try:
                ...
                if not val.modelXbrl.relationshipSet(XbrlConst.variablesScope).toModelObject(modelVariableSet):
                    if not runIDs or modelVariableSet.id in runIDs or modelVariableSet.hasConsistencyAssertion and any(modelRel.fromModelObject.id in runIDs for modelRel in val.modelXbrl.relationshipSet(XbrlConst.consistencyAssertionFormula).toModelObject(modelVariableSet) if isinstance(modelRel.fromModelObject, ModelConsistencyAssertion)):
                        try:
                            ...
                            val.modelXbrl.error(err.code, _('Variable set \n%(variableSet)s \nException: \n%(error)s'), modelObject=modelVariableSet, variableSet=str(modelVariableSet), error=err.message)

        if maxFormulaRunTimeTimer:
            maxFormulaRunTimeTimer.cancel()
    except XPathContext.RunTimeExceededException:
        val.modelXbrl.info('formula:maxRunTime', _('Formula execution ended after %(mins)s minutes'), modelObject=val.modelXbrl, mins=val.maxFormulaRunTime)
    ...
    for modelRel in val.modelXbrl.relationshipSet(XbrlConst.consistencyAssertionFormula).modelRelationships:
        if isinstance(modelRel.fromModelObject, ModelConsistencyAssertion) and isinstance(modelRel.toModelObject, ModelFormula) and (not runIDs or modelRel.fromModelObject.id in runIDs):
            ...
            if formulaOptions.traceAssertionResultCounts:
                val.modelXbrl.info('formula:trace', _('Consistency Assertion %(id)s evaluations : %(satisfiedCount)s satisfied, %(notSatisfiedCount)s not satisfied'), modelObject=consisAsser, id=consisAsser.id, satisfiedCount=consisAsser.countSatisfied, notSatisfiedCount=consisAsser.countNotSatisfied)
...
def checkFormulaRules(val, formula, nameVariables):
    if not (formula.hasRule(Aspect.CONCEPT) or formula.source(Aspect.CONCEPT)):
        if XmlUtil.hasDescendant(formula, XbrlConst.formula, 'concept'):
            val.modelXbrl.error('xbrlfe:incompleteConceptRule', _('Formula %(xlinkLabel)s concept rule does not have a nearest source and does not have a child element'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
        else:
            val.modelXbrl.error('xbrlfe:missingConceptRule', _('Formula %(xlinkLabel)s omits a rule for the concept aspect'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
        if not isinstance(formula, ModelTuple):
            if not (formula.hasRule(Aspect.SCHEME) or formula.source(Aspect.SCHEME)) or not (formula.hasRule(Aspect.VALUE) or formula.source(Aspect.VALUE)):
                if XmlUtil.hasDescendant(formula, XbrlConst.formula, 'entityIdentifier'):
                    val.modelXbrl.error('xbrlfe:incompleteEntityIdentifierRule', _('Formula %(xlinkLabel)s entity identifier rule does not have a nearest source and does not have either a @scheme or a @value attribute'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
                else:
                    val.modelXbrl.error('xbrlfe:missingEntityIdentifierRule', _('Formula %(xlinkLabel)s omits a rule for the entity identifier aspect'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
                if not (formula.hasRule(Aspect.PERIOD_TYPE) or formula.source(Aspect.PERIOD_TYPE)):
                    if XmlUtil.hasDescendant(formula, XbrlConst.formula, 'period'):
                        val.modelXbrl.error('xbrlfe:incompletePeriodRule', _('Formula %(xlinkLabel)s period rule does not have a nearest source and does not have a child element'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
        else:
            val.modelXbrl.error('xbrlfe:missingPeriodRule', _('Formula %(xlinkLabel)s omits a rule for the period aspect'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
        ...
        if concept is not None:
            if concept.isNumeric:
                if not (formula.hasRule(Aspect.MULTIPLY_BY) or formula.hasRule(Aspect.DIVIDE_BY) or formula.source(Aspect.UNIT)):
                    if XmlUtil.hasDescendant(formula, XbrlConst.formula, 'unit'):
                        val.modelXbrl.error('xbrlfe:missingSAVForUnitRule', _('Formula %(xlinkLabel)s unit rule does not have a source and does not have a child element'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
                    else:
                        val.modelXbrl.error('xbrlfe:missingUnitRule', _('Formula %(xlinkLabel)s omits a rule for the unit aspect'), modelObject=formula, xlinkLabel=formula.xlinkLabel)
        else:
            if formula.hasRule(Aspect.MULTIPLY_BY) or formula.hasRule(Aspect.DIVIDE_BY) or formula.source(Aspect.UNIT, acceptFormulaSource=False):
                val.modelXbrl.error('xbrlfe:conflictingAspectRules', _('Formula %(xlinkLabel)s has a rule for the unit aspect of a non-numeric concept %(concept)s'), modelObject=formula, xlinkLabel=formula.xlinkLabel, concept=concept.qname)
            ...
            if concept.periodType == 'duration' and aspectPeriodType == 'instant' or concept.periodType == 'instant' and aspectPeriodType in ('duration',
                                                                                                                                              'forever'):
                val.modelXbrl.error('xbrlfe:conflictingAspectRules', _('Formula %(xlinkLabel)s has a rule for the %(aspectPeriodType)s period aspect of a %(conceptPeriodType)s concept %(concept)s'), modelObject=formula, xlinkLabel=formula.xlinkLabel, concept=concept.qname, aspectPeriodType=aspectPeriodType, conceptPeriodType=concept.periodType)
        for eltName, dim, badUsageErr, missingSavErr in (('explicitDimension', 'explicit', 'xbrlfe:badUsageOfExplicitDimensionRule', 'xbrlfe:missingSAVForExplicitDimensionRule'),
            ...
            for dimElt in XmlUtil.descendants(formula, XbrlConst.formula, eltName):
                ...
                if dimQname and (dimConcept is None or (not dimConcept.isExplicitDimension if dim == 'explicit' else not dimConcept.isTypedDimension)):
                    val.modelXbrl.error(badUsageErr, _('Formula %(xlinkLabel)s dimension attribute %(dimension)s on the %(dimensionType)s dimension rule contains a QName that does not identify an (dimensionType)s dimension.'), modelObject=formula, xlinkLabel=formula.xlinkLabel, dimensionType=dim, dimension=dimQname, messageCodes=('xbrlfe:badUsageOfExplicitDimensionRule',
                    ...
                    val.modelXbrl.error(missingSavErr, _('Formula %(xlinkLabel)s %(dimension)s dimension rule does not have any child elements and does not have a SAV for the %(dimensionType)s dimension that is identified by its dimension attribute.'), modelObject=formula, xlinkLabel=formula.xlinkLabel, dimensionType=dim, dimension=dimQname, messageCodes=('xbrlfe:missingSAVForExplicitDimensionRule',
        ...
        if formula.aspectModel == 'non-dimensional':
            unexpectedElts = XmlUtil.descendants(formula, XbrlConst.formula, ('explicitDimension',
                                                                              'typedDimension'))
            if unexpectedElts:
                val.modelXbrl.error('xbrlfe:unrecognisedAspectRule', _('Formula %(xlinkLabel)s aspect model, %(aspectModel)s, includes an rule for aspect not defined in this aspect model: %(undefinedAspects)s'), modelObject=formula, xlinkLabel=formula.xlinkLabel, aspectModel=formula.aspectModel, undefinedAspects=', '.join([elt.localName for elt in unexpectedElts]))
                ...
                if sourceElt.get('source') is not None:
                    ...
                    if qnSource == XbrlConst.qnFormulaUncovered:
                        if formula.implicitFiltering != 'true':
                            val.modelXbrl.error('xbrlfe:illegalUseOfUncoveredQName', _('Formula %(xlinkLabel)s, not implicit filtering element has formulaUncovered source: %(name)s'), modelObject=formula, xlinkLabel=formula.xlinkLabel, name=sourceElt.localName)
                elif qnSource not in nameVariables:
                    val.modelXbrl.error('xbrlfe:nonexistentSourceVariable', _('Variable set %(xlinkLabel)s, source %(name)s is not in the variable set'), modelObject=formula, xlinkLabel=formula.xlinkLabel, name=qnSource)
                else:
                    ...
                    if isinstance(factVariable, ModelVariableSet):
                        ...
                        val.modelXbrl.error('xbrlfe:nonexistentSourceVariable', _('Variable set %(xlinkLabel)s, source %(name)s not a factVariable but is a %(element)s'), modelObject=formula, xlinkLabel=formula.xlinkLabel, name=qnSource, element=factVariable.localName)
                    elif factVariable.fallbackValue is not None:
                        val.modelXbrl.error('xbrlfe:bindEmptySourceVariable', _('Formula %(xlinkLabel)s: source %(name)s is a fact variable that has a fallback value'), modelObject=formula, xlinkLabel=formula.xlinkLabel, name=qnSource)
                    elif sourceElt.localName == 'formula' and factVariable.bindAsSequence == 'true':
                        val.modelXbrl.error('xbrlfe:defaultAspectValueConflicts', _('Formula %(xlinkLabel)s: formula source %(name)s is a fact variable that binds as a sequence'), modelObject=formula, xlinkLabel=formula.xlinkLabel, name=qnSource)
...
def checkDefinitionNodeRules(val, table, parent, arcrole, xpathContext):
    for rel in val.modelXbrl.relationshipSet(arcrole).fromModelObject(parent):
        ...
        if isinstance(axis, ModelFilterDefinitionNode):
        ...
        elif isinstance(axis, ModelRuleDefinitionNode):
            ...
            for elt in XmlUtil.descendants(axis, XbrlConst.formula, '*'):
                try:
                    ...
                    val.modelXbrl.error('xbrlte:incompleteAspectRule', _('RuleAxis %(xlinkLabel)s includes an incomplete rule aspect: %(incompleteAspect)s'), modelObject=axis, xlinkLabel=axis.xlinkLabel, incompleteAspect=elt.qname)

                if elt.localName == 'occFragments' and XmlUtil.children(elt, XbrlConst.xbrldi, '*'):
                    val.modelXbrl.error('xbrlfe:badSubsequentOCCValue', _('Formula %(label)s OCC element in rule aspect %(occ)s covers a dimensional aspect'), modelObject=axis, label=axis.xlinkLabel, occ=elt.qname)

            for aspect, rules in rulesByAspect.items():
                if len(rules) > 1:
                    val.modelXbrl.error('xbrlte:multipleValuesForAspect', _('RuleAxis %(xlinkLabel)s has %(count)s rules for aspect: %(multipleAspect)s'), modelObject=axis, xlinkLabel=axis.xlinkLabel, count=len(rules), multipleAspect=aspect)
            ...
            if table.aspectModel == 'non-dimensional':
                ...
                for sourceElt in [axis] + XmlUtil.descendants(axis, XbrlConst.formula, '*', 'source', '*'):
                    if sourceElt.get('source') is not None:
                        qnSource = qname(sourceElt, sourceElt.get('source'), noPrefixIsNoNamespace=True)
                        val.modelXbrl.info('table:info', _('RuleAxis rule %(xlinkLabel)s contains a @source attribute %(qnSource)s which is not applicable to table rule axes.'), modelObject=axis, xlinkLabel=axis.xlinkLabel, qnSource=qnSource)

                for childElt in XmlUtil.children(axis, '*', '*'):
                    if childElt.namespaceURI not in (XbrlConst.formula, axis.namespaceURI):
                        val.modelXbrl.error('xbrlte:unrecognisedAspectRule', _('RuleAxis %(xlinkLabel)s aspect model includes an unrecognized rule aspect: %(unrecognizedAspect)s'), modelObject=axis, xlinkLabel=axis.xlinkLabel, unrecognizedAspect=childElt.qname)
                ...
                if isinstance(axis, ModelRelationshipDefinitionNode):
                    for qnameAttr in ('relationshipSourceQname', 'arcQname', 'linkQname',
                        ...
                        if eltQname and eltQname not in val.modelXbrl.qnameConcepts:
                            val.modelXbrl.info('table:info', _('%(axis)s rule %(xlinkLabel)s contains a %(qnameAttr)s QName %(qname)s which is not in the DTS.'), modelObject=axis, axis=axis.localName.title(), xlinkLabel=axis.xlinkLabel, qnameAttr=qnameAttr, qname=eltQname)
...
def checkMessageExpressions(val, message):
    if isinstance(message, ModelMessage) and not hasattr(message, 'expressions'):
            ...
            if expression is not None and c in ("'", '"'):
                ...
                if lastC == c and c in ('{', '}'):
                    ...
                    if lastC == '{':
                        ...
                        if c == '}' and expression is not None:
        ...
        if lastC == '}':
            bracketNesting -= 1
        if bracketNesting:
            val.modelXbrl.error('xbrlmsge:missingLeftCurlyBracketInMessage' if bracketNesting < 0 else 'xbrlmsge:missingRightCurlyBracketInMessage', _('Message %(xlinkLabel)s: unbalanced %(character)s character(s) in: %(text)s'), modelObject=message, xlinkLabel=message.xlinkLabel, character='{' if bracketNesting < 0 else '}', text=message.text, messageCodes=('xbrlmsge:missingLeftCurlyBracketInMessage',
        ...
        if not message.xmlLang:
            val.modelXbrl.error('xbrlmsge:xbrlmsge:missingMessageLanguage', _('Message %(xlinkLabel)s is missing an effective value for xml:lang: %(text)s.'), modelObject=message, xlinkLabel=message.xlinkLabel, text=message.text)
        ...
        for modelRel in val.modelXbrl.relationshipSet(msgRelationship).fromModelObject(modelVariableSet):
            ...
            for msgVarQname in message.variableRefs():
                if msgVarQname not in varNames and msgVarQname not in varSetVars and msgVarQname not in paramNames:
                    val.modelXbrl.error('err:XPST0008', _('Undefined variable dependency in message %(xlinkLabel)s, %(name)s'), modelObject=message, xlinkLabel=message.xlinkLabel, name=msgVarQname)
                elif msgVarQname in varNames and isinstance(modelVariableSet, ModelExistenceAssertion) and isinstance(varNames[msgVarQname].toModelObject, ModelVariable):
                    val.modelXbrl.error('err:XPST0008', _('Existence Assertion depends on evaluation variable in message %(xlinkLabel)s, %(name)s'), modelObject=message, xlinkLabel=message.xlinkLabel, name=msgVarQname)