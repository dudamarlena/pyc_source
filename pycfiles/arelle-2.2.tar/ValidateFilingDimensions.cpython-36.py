...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ValidateFilingDimensions.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 29510 bytes
"""
...
def checkDimensions(val, drsELRs):
        ...
        for hasHypercubeArcrole in (XbrlConst.all, XbrlConst.notAll):
            ...
            for hasHcRels in hasHypercubeRelationships.values():
                for hasHcRel in hasHcRels:
                    ...
                    if hasHypercubeArcrole == XbrlConst.all:
                        ...
                        if not hasHcRel.isClosed:
                            val.modelXbrl.error('SBR.NL.2.3.6.04', (_("All hypercube %(hypercube)s in DRS role %(linkrole)s, does not have closed='true'")),
                              modelObject=hasHcRel,
                              hypercube=(hc.qname),
                              linkrole=ELR)
                    else:
                        if hasHypercubeArcrole == XbrlConst.notAll:
                            if hasHcRel.isClosed:
                                val.modelXbrl.error(('EFM.6.16.06', 'GFM.1.08.06'), (_("Not all hypercube %(hypercube)s in DRS role %(linkrole)s, does not have closed='false'")),
                                  modelObject=hasHcRel,
                                  hypercube=(hc.qname),
                                  linkrole=ELR,
                                  primaryItem=(sourceConcept.qname))
                            if hc in positiveHypercubes:
                                val.modelXbrl.error(('EFM.6.16.08', 'GFM.1.08.08'), (_('Not all hypercube %(hypercube)s in DRS role %(linkrole)s, is also the target of a positive hypercube')),
                                  modelObject=hasHcRel,
                                  hypercube=(hc.qname),
                                  linkrole=ELR,
                            ...
                            if dimTargetRequired:
                                if len(hcDimRels) == 0:
                                    val.modelXbrl.error(('EFM.6.16.09', 'GFM.1.08.09'), (_('Table %(hypercube)s in DRS role %(linkrole)s, missing targetrole consecutive relationship')),
                                      modelObject=hasHcRel,
                                      hypercube=(hc.qname),
                                      fromConcept=(sourceConcept.qname),
                                      toConcept=(hc.qname),
                                      linkrole=ELR,
                                      linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
                                      arcroleURI=(hasHcRel.arcrole),
                                ...
                                if not domELR:
                                    if dim.isExplicitDimension:
                                        ...
                                        if val.validateSBRNL:
                                            val.modelXbrl.error('SBR.NL.2.3.5.04', (_('Hypercube %(hypercube)s in DRS role %(linkrole)s, missing targetrole to dimension %(dimension)s consecutive relationship')),
                                              modelObject=hcDimRel,
                                              hypercube=(hc.qname),
                                              linkrole=ELR,
                                              dimension=(dim.qname))
                                else:
                                    if dim.isTypedDimension:
                                        if val.validateSBRNL:
                                            val.modelXbrl.error('SBR.NL.2.3.5.07', (_('Typed dimension %(dimension)s in DRS role %(linkrole)s, has targetrole consecutive relationship')),
                                              modelObject=hcDimRel,
                                              dimension=(dim.qname),
                                              linkrole=ELR)
                                    if hasHypercubeArcrole == XbrlConst.all:
                                        ...
                                        if hasHypercubeArcrole == XbrlConst.notAll:
                                            if dim not in positiveAxisTableSources or not commonAncestor(domainMemberRelationshipSet, sourceConcept, positiveAxisTableSources[dim]):
                                                val.modelXbrl.error(('EFM.6.16.07',
                                                                     'GFM.1.08.08'), (_('Negative table axis %(dimension)s in DRS role %(linkrole)s, not in any positive table in same role')),
                                                  modelObject=hcDimRel,
                                                  dimension=(dim.qname),
                                                  linkrole=ELR,
                                                  linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
                            ...
                            if domTargetRequired:
                                if len(dimDomRels) == 0:
                                    val.modelXbrl.error(('EFM.6.16.09', 'GFM.1.08.09'), (_('Axis %(dimension)s in DRS role %(linkrole)s, missing targetrole consecutive relationship')),
                                      modelObject=hcDimRel,
                                      dimension=(dim.qname),
                                      fromConcept=(hc.qname),
                                      toConcept=(dim.qname),
                                      linkrole=ELR,
                                      linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
                                      arcroleURI=(hasHcRel.arcrole),
                                      arcrole=(os.path.basename(hcDimRel.arcrole)))
                                if val.validateEFMorGFM:
                                    ...
                                    if cycleCausingConcept is not None:
                                        ...
                                        val.modelXbrl.error(('EFM.6.16.04', 'GFM.1.08.04'), (_('Dimension relationships have an undirected cycle in DRS role %(linkrole)s \nstarting from table %(hypercube)s, \naxis %(dimension)s, \npath %(path)s')),
                                          modelObject=([
                                         hc, dim] + [rel for rel in cycleCausingConcept if not isinstance(rel, bool)]),
                                          linkrole=ELR,
                                          linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
                                          hypercube=(hc.qname),
                                          dimension=(dim.qname),
                                          conceptFrom=(dim.qname),
                ...
                if hasHypercubeArcrole == XbrlConst.all and len(hasHcRels) > 1:
                    val.modelXbrl.error(('EFM.6.16.05', 'GFM.1.08.05'), (_('Multiple tables (%(hypercubeCount)s) DRS role %(linkrole)s, source %(concept)s, only 1 allowed')),
                      modelObject=([
                     sourceConcept] + hasHcRels),
                      hypercubeCount=(len(hasHcRels)),
                      linkrole=ELR,
                      linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
            ...
            if relFrom in primaryItems:
                ...
                if cycleCausingConcept is not None:
                    val.modelXbrl.error(('EFM.6.16.04', 'GFM.1.08.04'), (_('Domain-member primary-item relationships have an undirected cycle in DRS role %(linkrole)s \nstarting from %(conceptFrom)s, \npath %(path)s')),
                      modelObject=([
                     relFrom] + [rel for rel in cycleCausingConcept if not isinstance(rel, bool)]),
                      linkrole=ELR,
                      conceptFrom=(relFrom.qname),
                      path=(cyclePath(relFrom, cycleCausingConcept)))
                fromConceptELRs.clear()
            for rel in rels:
                ...
                if isinstance(toMbr, ModelConcept) and toELR and len(val.modelXbrl.relationshipSet(XbrlConst.domainMember, toELR).fromModelObject(toMbr)) == 0:
                    val.modelXbrl.error(('EFM.6.16.09', 'GFM.1.08.09'), (_('Domain member %(concept)s in DRS role %(linkrole)s, missing targetrole consecutive relationship')),
                      modelObject=rel,
                      concept=(fromMbr.qname),
                      fromConcept=(toMbr.qname),
                      toConcept=(fromMbr.qname),
                      linkrole=ELR,
                      linkroleDefinition=(val.modelXbrl.roleTypeDefinition(ELR)),
                      arcroleURI=(hasHcRel.arcrole),
                      arcrole=(os.path.basename(rel.arcrole)))
    ...
    if val.validateSBRNL:
        for hc in hypercubes:
            for priHcRel in val.modelXbrl.relationshipSet(XbrlConst.all).toModelObject(hc):
                ...
                if priHcRel.contextElement == 'segment':
                    val.modelXbrl.error('SBR.NL.2.3.5.06', (_('hypercube %(hypercube)s in segment not allowed, ELR role %(linkrole)s')),
                      modelObject=priHcRel,
                      linkrole=ELR,
                      hypercube=(hc.qname))

        for notAllRel in val.modelXbrl.relationshipSet(XbrlConst.notAll).modelRelationships:
            val.modelXbrl.error('SBR.NL.2.3.5.05', (_('Notall from primary item %(primaryItem)s in ELR role %(linkrole)s to %(hypercube)s')),
              modelObject=(val.modelXbrl),
              primaryItem=(notAllRel.fromModelObject.qname),
              linkrole=(notAllRel.linkrole),
              hypercube=(notAllRel.toModelObject.qname))
                ...
                for arcrole in (XbrlConst.parentChild, 'XBRL-dimensions'):
                    for modelRel in val.modelXbrl.relationshipSet(arcrole, ELR).modelRelationships:
                        if modelRel.fromModelObject != hc and modelRel.toModelObject != hc and modelRel.fromModelObject not in domains and modelRel.toModelObject not in domains:
                            val.modelXbrl.error('SBR.NL.2.2.3.05', (_('ELR role %(linkrole)s, has hypercube %(hypercube)s and a %(arcrole)s relationship not involving the hypercube or primary domain, from %(fromConcept)s to %(toConcept)s')),
                              modelObject=modelRel,
                              linkrole=ELR,
                              hypercube=(hc.qname),
                              arcrole=(os.path.basename(modelRel.arcrole)),
        ...
        for rel in val.modelXbrl.relationshipSet(XbrlConst.dimensionDomain).modelRelationships:
            ...
            if isinstance(relFrom, ModelConcept):
            ...
            if rel.isUsable:
                if val.modelXbrl.relationshipSet(XbrlConst.domainMember, rel.targetRole).fromModelObject(relTo):
                    val.modelXbrl.error('SBR.NL.2.3.7.05', (_('Dimension %(dimension)s in DRS role %(linkrole)s, has usable domain with members %(domain)s')),
                      modelObject=rel,
                      dimension=(relFrom.qname),
                      linkrole=(rel.linkrole),
                      domain=(relTo.qname))
                else:
                    if not relTo.isAbstract:
                        val.modelXbrl.error('SBR.NL.2.3.7.02', (_('Dimension %(dimension)s in DRS role %(linkrole)s, has nonAbsract domain %(domain)s')),
                          modelObject=rel,
                          dimension=(relFrom.qname),
                          linkrole=(rel.linkrole),
                          domain=(relTo.qname))
                    if relTo.substitutionGroupQname.localName not in ('domainItem',
                                                                      'domainMemberItem'):
                        val.modelXbrl.error('SBR.NL.2.2.2.19', (_('Domain item %(domain)s in DRS role %(linkrole)s, in dimension %(dimension)s is not a domainItem')),
                          modelObject=rel,
                          domain=(relTo.qname),
                          linkrole=(rel.linkrole),
                          dimension=(relFrom.qname))
                if not rel.targetRole and relTo.substitutionGroupQname.localName == 'domainItem':
                    val.modelXbrl.error('SBR.NL.2.3.6.03', (_('Dimension %(dimension)s in DRS role %(linkrole)s, missing targetrole to consecutive domain relationship')),
                      modelObject=rel,
                      dimension=(relFrom.qname),
                      linkrole=(rel.linkrole))
        ...
        for linkrole, domains in domainsInLinkrole.items():
            if linkrole and len(domains) > 1:
                val.modelXbrl.error('SBR.NL.2.3.7.04', (_('Linkrole %(linkrole)s, has multiple domains %(domains)s')),
                  modelObject=(val.modelXbrl),
                  linkrole=linkrole,
                  domains=(', '.join([str(dom.qname) for dom in domains])))
        ...
        for dimDomMems, linkroles in linkrolesByDimDomMems.items():
            if len(linkroles) > 1:
                val.modelXbrl.error('SBR.NL.2.3.6.02', (_('Dimension %(dimension)s  usable members same in linkroles %(linkroles)s')),
                  modelObject=(val.modelXbrl),
                  dimension=(dimDomMems[0].qname),
                  linkroles=(', '.join(l for l in linkroles)))
        ...
        for rel in val.modelXbrl.relationshipSet(XbrlConst.domainMember).modelRelationships:
            if val.modelXbrl.relationshipSet(XbrlConst.domainMember, rel.targetRole).fromModelObject(rel.toModelObject):
                val.modelXbrl.error('SBR.NL.2.3.7.03', (_('Domain member %(member)s in DRS role %(linkrole)s, has nested members')),
                  modelObject=rel,
                  member=(rel.toModelObject.qname if isinstance(rel.toModelObject, ModelConcept) else None),
                  linkrole=(rel.linkrole))
            ...
            if isinstance(relTo, ModelConcept):
                if relFrom.substitutionGroupQname.localName == 'domainItem':
                    if relTo.substitutionGroupQname.localName != 'domainMemberItem':
                        val.modelXbrl.error('SBR.NL.2.2.2.19', (_('Domain member item %(member)s in DRS role %(linkrole)s is not a domainMemberItem')),
                          modelObject=rel,
                          member=(relTo.qname),
                          linkrole=(rel.linkrole))
                else:
                    if relTo.substitutionGroupQname.localName == 'domainMemberItem':
                        val.modelXbrl.error('SBR.NL.2.2.2.19', (_('Domain item %(domain)s in DRS role %(linkrole)s is not a domainItem')),
                          modelObject=rel,
                          domain=(relFrom.qname),
                          linkrole=(rel.linkrole))
                        break
                    elif relFrom.isAbstract:
                        if relFrom.substitutionGroupQname.localName != 'primaryDomainItem':
                            val.modelXbrl.error('SBR.NL.2.2.2.19', (_('Abstract domain item %(domain)s in DRS role %(linkrole)s is not a primaryDomainItem')),
                              modelObject=rel,
                              domain=(relFrom.qname),
                              linkrole=(rel.linkrole))
                            break
        ...
        for hc, DRSdims in hypercubeDRSDimensions.items():
            ...
            for ELR, mutableDims in DRSdims.items():
                ...
                if dims not in hcELRdimSets:
                    ...
                    val.modelXbrl.error('SBR.NL.2.3.5.02', (_('Hypercube %(hypercube)s has same dimensions in ELR roles %(linkrole)s and %(linkrole2)s: %(dimensions)s')),
                      modelObject=hc,
                      hypercube=(hc.qname),
                      linkrole=ELR,
                      linkrole2=(hcELRdimSets[dims]),
...
def undirectedFwdCycle(val, fromELR, rels, drsELR, drsRelsFrom, drsRelsTo, fromConceptELRs, ELRsVisited=None):
    ...
    for rel in rels:
        if rel.linkrole == fromELR:
            ...
            if isinstance(relTo, ModelConcept):
                toELR = rel.targetRole
                toELR = toELR or fromELR
        if relTo in fromConceptELRs:
            if toELR in fromConceptELRs[relTo]:
                return [
                 rel, True]
            else:
    ...
    for arcrole in (XbrlConst.domainMember, XbrlConst.dimensionDomain):
        ...
        for rel in mbrDomRels:
            if not rel.isIdenticalTo(turnbackRel):
                relFrom = rel.fromModelObject
                relELR = rel.linkrole
                if relFrom in fromConceptELRs:
                    if relELR in fromConceptELRs[relFrom]:
                        return [
                         rel, False]
    ...
    for rel in rels:
            ...
            if isDomMbr or not relTo.isAbstract:
                if relTo in members:
                    ...
                    if isDomMbr:
                        ...
                        val.modelXbrl.error('SBR.NL.2.3.5.01', (_('Primary items for hypercube %(hypercube)s ELR %(ELR)s, have non-unique (inheritance) member %(concept)s in DRS role %(linkrole)s')),
                          modelObject=relTo,
                          hypercube=(hc.qname),
                          ELR=domELR,
                          concept=(relTo.qname),
        ...
        if not isDomMbr:
            if relTo.isAbstract and not relFrom.isAbstract or relFrom.substitutionGroupQname.localName != 'primaryDomainItem':
                val.modelXbrl.error('SBR.NL.2.3.7.01', (_('Primary item %(concept)s in DRS role %(linkrole)s for hypercube %(hypercube)s, has parent %(concept2)s which is not a primaryDomainItem')),
                  modelObject=rel,
                  concept=(relTo.qname),
                  linkrole=ELR,
                  hypercube=(hc.qname),
                  concept2=(relFrom.qname))
