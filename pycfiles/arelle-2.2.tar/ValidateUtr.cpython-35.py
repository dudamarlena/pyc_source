...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ValidateUtr.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 11910 bytes
"""
...
class UtrEntry:
    __slots__ = ('id', 'unitId', 'nsUnit', 'itemType', 'nsItemType', 'isSimple', 'numeratorItemType',
                 'nsNumeratorItemType', 'denominatorItemType', 'nsDenominatorItemType',
                 'symbol', 'status')

    def __repr__(self):
        return 'utrEntry({})'.format(', '.join('{}={}'.format(n, getattr(self, n)) for n in self.__slots__))


def loadUtr(modelXbrl):
    ...
    try:
        ...
        for unitElt in xmldoc.iter(tag='{http://www.xbrl.org/2009/utr}unit'):
            ...
            u.nsUnit = unitElt.findtext('{http://www.xbrl.org/2009/utr}nsUnit') or None
            ...
            unitDupKey = (
             u.unitId, u.nsUnit, u.status)
            if unitDupKey in unitDupCheck:
                modelXbrl.error('arelleUtrLoader:entryDuplication', 'Unit Type Registry entry duplication: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s', modelObject=modelXbrl, id=u.id, unitId=u.unitId, nsUnit=u.nsUnit, status=u.status)
            unitDupCheck.add(unitDupKey)
            if u.isSimple:
                if not u.itemType:
                    modelXbrl.error('arelleUtrLoader:simpleDefMissingField', 'Unit Type Registry simple unit definition missing item type: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s', modelObject=modelXbrl, id=u.id, unitId=u.unitId, nsUnit=u.nsUnit, status=u.status)
                if u.numeratorItemType or u.denominatorItemType or u.nsNumeratorItemType or u.nsDenominatorItemType:
                    modelXbrl.error('arelleUtrLoader', 'Unit Type Registry simple unit definition may not have complex fields: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s', modelObject=modelXbrl, id=u.id, unitId=u.unitId, nsUnit=u.nsUnit, status=u.status)
            else:
                if u.symbol:
                    modelXbrl.error('arelleUtrLoader:complexDefSymbol', 'Unit Type Registry complex unit definition may not have symbol: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s', modelObject=modelXbrl, id=u.id, unitId=u.unitId, nsUnit=u.nsUnit, status=u.status)
                if not u.numeratorItemType or not u.denominatorItemType:
                    modelXbrl.error('arelleUtrLoader:complexDefMissingField', 'Unit Type Registry complex unit definition must have numerator and denominator fields: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s', modelObject=modelXbrl, id=u.id, unitId=u.unitId, nsUnit=u.nsUnit, status=u.status)

    except (EnvironmentError,
     etree.LxmlError) as err:
        modelManager.modelXbrl.error('arelleUtrLoader:error', 'Unit Type Registry Import error: %(error)s', modelObject=modelXbrl, error=err)
...
class ValidateUtr:
        ...
        if modelXbrl.modelDocument.type in (ModelDocument.Type.INSTANCE, ModelDocument.Type.INLINEXBRL):
            ...
            for f in modelXbrl.facts:
                ...
                if concept is not None and concept.isNumeric:
                    unit = f.unit
                    typeMatched = False
                    if f.unitID is not None and unit is not None:
                        unitMatched = False
                        _type = concept.type
                        while _type is not None:
                            unitMatched, typeMatched, _utrEntry = self.measuresMatch(False, unit.measures[0], unit.measures[1], _type.name, _type.modelDocument.targetNamespace)
                            if typeMatched:
                                break
            ...
            for fact in utrInvalidFacts:
                modelXbrl.log(self.messageLevel, self.messageCode, _('Unit %(unitID)s disallowed on fact %(element)s of type %(typeName)s'), modelObject=fact, unitID=fact.unitID, element=fact.qname, typeName=fact.concept.type.name, messageCodes=('utre:error-NumericFactUtrInvalid', ))
    ...
    def measuresMatch(self, typeMatched, mulMeas, divMeas, typeName=None, typeNS=None, *divArgs):
        ...
        if typeName and typeName not in self.utrItemTypeEntries:
            if divMeas or divArgs:
                return self.measuresMatch(typeMatched, divMeas, (), *divArgs)
            return (
             typeMatched, typeMatched, None)
            ...
            if typeNS is None or u.nsItemType is None or typeNS == u.nsItemType:
                ...
                if u.isSimple:
                    for i, m in enumerate(mulMeas):
                        if m.localName == u.unitId and (u.nsUnit is None or u.nsUnit == m.namespaceURI) and self.measuresMatch(True, divMeas, mulMeas[:i] + mulMeas[i + 1:], *divArgs)[0]:
                            return (
                             True, True, u)

                elif self.measuresMatch(True, mulMeas, divMeas, u.numeratorItemType, u.nsNumeratorItemType, u.denominatorItemType, u.nsDenominatorItemType, None, DIVISOR, *divArgs)[0]:
                    return (
                     True, True, u)

        return (
         False, typeMatched, None)
    ...
    def utrEntries(self, modelType, unit):
        ...
        _type = modelType
        while _type is not None:
            unitMatched, typeMatched, utrEntry = self.measuresMatch(False, unit.measures[0], unit.measures[1], _type.name, _type.modelDocument.targetNamespace)
            if typeMatched:
                if unitMatched:
    ...
    def utrSymbol(self, multMeasures, divMeasures):
        if not divMeasures:
            ...
            if len(multMeasures) == 1:
                ...
                if m in self.modelXbrl.qnameConcepts:
                    return self.modelXbrl.qnameConcepts[m].label(fallbackToQname=False) or m.localName
                return m.localName

            def symbols(measures, wrapMult=True):
                measuresString = ' '.join(sorted(self.utrSymbol([measure], None) for measure in measures))
