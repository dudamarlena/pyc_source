...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ValidateXbrlDimensions.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 38281 bytes
"""
        ...
        for modelRel in relsSet.modelRelationships:
            ...
            if fromConcept is not None and toConcept is not None:
                if not isinstance(fromConcept, ModelConcept) or not fromConcept.isHypercubeItem:
                    val.modelXbrl.error('xbrldte:HypercubeDimensionSourceError', _('Hypercube-dimension relationship from %(source)s to %(target)s in link role %(linkrole)s must have a hypercube declaration source'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                if not isinstance(toConcept, ModelConcept) or not toConcept.isDimensionItem:
                    val.modelXbrl.error('xbrldte:HypercubeDimensionTargetError', _('Hypercube-dimension relationship from %(source)s to %(target)s in link role %(linkrole)s must have a dimension declaration target'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                ...
                for hasHcRel in hcRels:
                    ...
                    if priItemConcept is not None and hcConcept is not None:
                        if not isinstance(priItemConcept, ModelConcept) or not priItemConcept.isPrimaryItem:
                            val.modelXbrl.error('xbrldte:HasHypercubeSourceError', _('HasHypercube %(arcroleType)s relationship from %(source)s to %(target)s in link role %(linkrole)s must have a primary item source'), modelObject=hasHcRel, arcroleType=os.path.basename(arcrole), source=priItemConcept.qname, target=hcConcept.qname, linkrole=ELR)
                        if not isinstance(hcConcept, ModelConcept) or not hcConcept.isHypercubeItem:
                            val.modelXbrl.error('xbrldte:HasHypercubeTargetError', _('HasHypercube %(arcroleType)s relationship from %(source)s to %(target)s in link role %(linkrole)s must have a hypercube declaration target'), modelObject=hasHcRel, arcroleType=os.path.basename(arcrole), source=priItemConcept.qname, target=hcConcept.qname, linkrole=ELR)
                        hcContextElement = hasHcRel.contextElement
                        if hcContextElement not in ('segment', 'scenario'):
                            val.modelXbrl.error('xbrldte:HasHypercubeMissingContextElementAttributeError', _('HasHypercube %(arcroleType)s relationship from %(source)s to %(target)s in link role %(linkrole)s must have a context element'), modelObject=hasHcRel, arcroleType=os.path.basename(arcrole), source=priItemConcept.qname, target=hcConcept.qname, linkrole=ELR)
                        dimELR = hasHcRel.targetRole
                        if not dimELR:
                            dimELR = ELR
                        hcDimRels = val.modelXbrl.relationshipSet(XbrlConst.hypercubeDimension, dimELR).fromModelObject(hcConcept)
                        for hcDimRel in hcDimRels:
                            ...
                            if dimConcept is not None:
                                if arcrole == XbrlConst.all:
                                    ...
                                    if not domELR:
                                        domELR = dimELR
                                    dimDomRels = val.modelXbrl.relationshipSet(XbrlConst.dimensionDomain, domELR).fromModelObject(dimConcept)
                                    cycle = xdtCycle(val, domainTargetRoles(val, domELR, dimDomRels), dimDomRels, {hcConcept, dimConcept})
                                    if cycle is not None:
                                        if cycle is not None:
                                            cycle.append(hcDimRel)
                                            path = str(hcConcept.qname) + ' ' + ' - '.join('{0}:{1} {2}'.format(rel.modelDocument.basename, rel.sourceline, rel.toModelObject.qname if isinstance(rel.toModelObject, ModelObject) else None) for rel in reversed(cycle))
                                        val.modelXbrl.error('xbrldte:DRSDirectedCycleError', _('Dimension relationships have a directed cycle in DRS role %(linkrole)s \nstarting from hypercube %(hypercube)s, \ndimension %(dimension)s, \npath %(path)s'), modelObject=[
                                         hcConcept] + cycle, hypercube=hcConcept.qname, dimension=dimConcept.qname, linkrole=ELR, path=path)
                                    cycle = drsPolymorphism(val, domELR, dimDomRels, drsPriItems(val, ELR, priItemConcept))
                                    if cycle is not None:
                                        if cycle is not None:
                                            cycle.append(hcDimRel)
                                            path = str(priItemConcept.qname) + ' ' + ' - '.join('{0}:{1} {2}'.format(rel.modelDocument.basename, rel.sourceline, rel.toModelObject.qname if isinstance(rel.toModelObject, ModelObject) else None) for rel in reversed(cycle))
                                        val.modelXbrl.error('xbrldte:PrimaryItemPolymorphismError', _('Dimension relationships have a polymorphism cycle in DRS role %(linkrole)s \nstarting from hypercube %(hypercube)s, \ndimension %(dimension)s, \npath %(path)s'), modelObject=[
                                         hcConcept] + cycle, hypercube=hcConcept.qname, dimension=dimConcept.qname, linkrole=ELR, path=path)
            ...
            if arcrole == XbrlConst.dimensionDomain:
                for modelRel in relsSet.modelRelationships:
                    ...
                    if fromConcept is not None and toConcept is not None:
                        if not isinstance(fromConcept, ModelConcept) or not fromConcept.isDimensionItem:
                            val.modelXbrl.error('xbrldte:DimensionDomainSourceError', _('Dimension-domain relationship from %(source)s to %(target)s in link role %(linkrole)s must have a dimension declaration source'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                        else:
                            if fromConcept.get('{http://xbrl.org/2005/xbrldt}typedDomainRef') is not None:
                                val.modelXbrl.error('xbrldte:DimensionDomainSourceError', _('Dimension-domain relationship from %(source)s to %(target)s in link role %(linkrole)s has a typed dimension source'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                            if not isinstance(toConcept, ModelConcept) or not toConcept.isDomainMember:
                                val.modelXbrl.error('xbrldte:DimensionDomainTargetError', _('Dimension-domain relationship from %(source)s to %(target)s in link role %(linkrole)s must have a domain member target'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                ...
                if arcrole == XbrlConst.dimensionDefault:
                    for modelRel in relsSet.modelRelationships:
                        ...
                        if fromConcept is not None and toConcept is not None:
                            if not isinstance(fromConcept, ModelConcept) or not fromConcept.isDimensionItem:
                                val.modelXbrl.error('xbrldte:DimensionDefaultSourceError', _('Dimension-default relationship from %(source)s to %(target)s in link role %(linkrole)s must have a dimension declaration source'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                            else:
                                if fromConcept.get('{http://xbrl.org/2005/xbrldt}typedDomainRef'):
                                    val.modelXbrl.error('xbrldte:DimensionDefaultSourceError', _('Dimension-default relationship from %(source)s to %(target)s in link role %(linkrole)s has a typed dimension source'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                                if not isinstance(toConcept, ModelConcept) or not toConcept.isDomainMember:
                                    val.modelXbrl.error('xbrldte:DimensionDefaultTargetError', _('Dimension-default relationship from %(source)s to %(target)s in link role %(linkrole)s must have a domain member target'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, linkrole=ELR)
                                if fromConcept in val.modelXbrl.dimensionDefaultConcepts and toConcept != val.modelXbrl.dimensionDefaultConcepts[fromConcept]:
                                    val.modelXbrl.error('xbrldte:TooManyDefaultMembersError', _('Dimension %(source)s has multiple defaults %(target)s and %(target2)s'), modelObject=modelRel, source=fromConcept.qname, target=toConcept.qname, target2=val.modelXbrl.dimensionDefaultConcepts[fromConcept].qname)
                                else:
                        ...
                        for domMbrRel in rels:
                            ...
                            if toConcept is not None:
                                if not isinstance(priItemConcept, ModelConcept) or not priItemConcept.isDomainMember:
                                    val.modelXbrl.error('xbrldte:DomainMemberSourceError', _('Domain-Member relationship from %(source)s to %(target)s in link role %(linkrole)s must have a domain primary item or domain member source'), modelObject=domMbrRel, source=priItemConcept.qname, target=toConcept.qname, linkrole=ELR)
                                if not isinstance(toConcept, ModelConcept) or not toConcept.isDomainMember:
                                    val.modelXbrl.error('xbrldte:DomainMemberTargetError', _('Domain-Member relationship from %(source)s to %(target)s in link role %(linkrole)s must have a domain primary item or domain member target'), modelObject=domMbrRel, source=priItemConcept.qname, target=toConcept.qname, linkrole=ELR)
...
def xdtCycle(val, ELRs, rels, fromConcepts):
    for rel in rels:
        relTo = rel.toModelObject
        if rel.isUsable and relTo in fromConcepts:
            return [rel]
        fromConcepts.add(relTo)
        for ELR in ELRs:
...
def drsPolymorphism(val, fromELR, rels, priItems, visitedMbrs=None):
    ...
    for rel in rels:
        ...
        if not toELR:
            toELR = fromELR
        if rel.isUsable and relTo in priItems:
            return [rel]
...
def checkConcept(val, concept):
    ...
    if concept.isDimensionItem:
        ...
        if typedDomainElement is None:
            ...
            if len(id) == 0:
                val.modelXbrl.error('xbrldte:TypedDimensionURIError', _('Concept %(concept)s typedDomainRef has no fragment identifier'), modelObject=concept, concept=concept.qname)
            else:
                val.modelXbrl.error('xbrldte:OutOfDTSSchemaError', _('Concept %(concept)s typedDomainRef is not resolved'), modelObject=concept, concept=concept.qname)
        elif not isinstance(typedDomainElement, ModelConcept) or not typedDomainElement.isGlobalDeclaration or typedDomainElement.abstract == 'true':
            val.modelXbrl.error('xbrldte:TypedDimensionError', _('Concept %(concept)s typedDomainRef must identify a non-abstract element'), modelObject=concept, concept=concept.qname)
    else:
        val.modelXbrl.error('xbrldte:TypedDomainRefError', _('Concept %(concept)s is not a dimension item but has a typedDomainRef'), modelObject=concept, concept=concept.qname)
...
def checkContext(val, cntx):
    ...
    for modelDimValues in (cntx.segDimValues.values(), cntx.scenDimValues.values(), cntx.errorDimValues):
        for modelDimValue in modelDimValues:
            ...
            if dimensionConcept is None or not dimensionConcept.isDimensionItem or modelDimValue.isTyped != (dimensionConcept.get('{http://xbrl.org/2005/xbrldt}typedDomainRef') is not None):
                val.modelXbrl.error('xbrldie:TypedMemberNotTypedDimensionError' if modelDimValue.isTyped else 'xbrldie:ExplicitMemberNotExplicitDimensionError', _('Context %(contextID)s %(dimension)s %(value)s is not an appropriate dimension item'), modelObject=logDimAndFacts(modelDimValue), contextID=cntx.id, dimension=modelDimValue.prefixedName, value=modelDimValue.dimensionQname, messageCodes=('xbrldie:TypedMemberNotTypedDimensionError',
                    ...
                    if isinstance(element, ModelObject):
                        ...
                        if typedDomainConcept is None:
                            ...
                            if element.localName != typedDomainConcept.name or element.namespaceURI != typedDomainConcept.modelDocument.targetNamespace:
                                ...
                                problem = None
                                if val.validateEnum and typedDomainConcept.isEnumeration and getattr(element, 'xValid', 0) == 4 and element.get('{http://www.w3.org/2001/XMLSchema-instance}nil') not in ('true',
                                                                                                                                                                                                          '1'):
                                    qnEnums = element.xValue
                                    if not isinstance(qnEnums, list):
                                        qnEnums = (
                                         qnEnums,)
                                    if not all(enumerationMemberUsable(val, typedDomainConcept, val.modelXbrl.qnameConcepts.get(qnEnum)) for qnEnum in qnEnums):
                                        val.modelXbrl.error('enum2ie:InvalidDimensionSetValue', _('Dimension value %(dimensionMember)s context %(contextID)s enumeration %(value)s is not in the domain of %(concept)s'), modelObject=element, dimensionMember=element.qname, contextID=cntx.id, value=element.xValue, concept=element.qname)
                                    if len(qnEnums) > len(set(qnEnums)):
                                        val.modelXbrl.error('enum2ie:RepeatedDimensionSetValue', _('Dimension value %(dimensionMember)s context %(contextID)s enumeration has non-unique values %(value)s'), modelObject=element, dimensionMember=element.qname, contextID=cntx.id, value=element.xValue, concept=element.qname)
                                    if any(qnEnum < qnEnums[i] for i, qnEnum in enumerate(qnEnums[1:])):
                                        val.modelXbrl.error('enum2ie:InvalidDimensionSetOrder', _('Dimension value %(dimensionMember) context %(contextID)s enumeration is not in lexicographical order %(value)s'), modelObject=element, dimensionMember=element.qname, contextID=cntx.id, value=element.xValue, concept=element.qname)

                if problem:
                    val.modelXbrl.error('xbrldie:IllegalTypedDimensionContentError', _('Context %(contextID)s typed dimension %(dimension)s has %(error)s'), modelObject=logDimAndFacts(modelDimValue), contextID=cntx.id, dimension=modelDimValue.dimensionQname, error=problem)
            if modelDimValue.isExplicit:
                ...
                if memberConcept is None or not memberConcept.isGlobalDeclaration:
                    val.modelXbrl.error('xbrldie:ExplicitMemberUndefinedQNameError', _('Context %(contextID)s explicit dimension %(dimension)s member %(value)s is not a global member item'), modelObject=logDimAndFacts(modelDimValue), contextID=cntx.id, dimension=modelDimValue.dimensionQname, value=modelDimValue.memberQname)
                elif val.modelXbrl.dimensionDefaultConcepts.get(dimensionConcept) == memberConcept:
                    val.modelXbrl.error('xbrldie:DefaultValueUsedInInstanceError', _('Context %(contextID)s explicit dimension %(dimension)s member %(value)s is a default member item'), modelObject=logDimAndFacts(modelDimValue), contextID=cntx.id, dimension=modelDimValue.dimensionQname, value=modelDimValue.memberQname)
    ...
    for modelDimValue in cntx.errorDimValues:
        ...
        if dimensionConcept is not None and (dimensionConcept in cntx.segDimValues or dimensionConcept in cntx.scenDimValues):
            val.modelXbrl.error('xbrldie:RepeatedDimensionInInstanceError', _('Context %(contextID)s dimension %(dimension)s is a repeated dimension value'), modelObject=logDimAndFacts(modelDimValue), contextID=cntx.id, dimension=modelDimValue.dimensionQname)
...
def checkFact(val, f, otherFacts=None):
    if not isFactDimensionallyValid(val, f, otherFacts):
        val.modelXbrl.error('xbrldie:PrimaryItemDimensionallyInvalidError', _('Fact %(fact)s context %(contextID)s dimensionally not valid'), modelObject=f, fact=f.qname, contextID=f.context.id)
...
def checkFactElrHcs(val, f, ELR, hcRels, setPrototypeContextElements=False):
    ...
    for hasHcRel in hcRels:
        ...
        if hcIsClosed and len(modelNonDimValues) > 0:
            ...
            for hcDimRel in val.modelXbrl.relationshipSet(XbrlConst.hypercubeDimension, dimELR).fromModelObject(hcConcept):
                dimConcept = hcDimRel.toModelObject
                if isinstance(dimConcept, ModelConcept):
                    ...
                    if dimConcept in modelDimValues:
                        ...
                        if dimConcept in val.modelXbrl.dimensionDefaultConcepts:
                            ...
                            if setPrototypeContextElements and isinstance(context, ContextPrototype) and dimConcept in oppositeContextDimValues:
...
def dimensionMemberUsable(val, dimConcept, memConcept, domELR):
    ...
    try:
        ...
        findUsableMembersInDomainELR(val, val.modelXbrl.relationshipSet(XbrlConst.dimensionDomain, domELR).fromModelObject(dimConcept), domELR, usableMembers, unusableMembers, defaultdict(set))
    ...
    if enumConcept is None:
        ...
        try:
            ...
            if domConcept is None:
                ...
                findUsableMembersInDomainELR(val, val.modelXbrl.relationshipSet(XbrlConst.domainMember, domELR).fromModelObject(domConcept), domELR, usableMembers, unusableMembers, defaultdict(set))
                usableMembers -= unusableMembers
                return usableMembers
