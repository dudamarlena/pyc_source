...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\FunctionUtil.py
...
def testTypeCompatiblity(xc, p, op, a1, a2):
    if isinstance(a1, ModelValue.DateTime) and isinstance(a2, ModelValue.DateTime):
        ...
        if type(a1) == type(a2) or isinstance(a1, _NUM_TYPES) and isinstance(a2, _NUM_TYPES) or isinstance(a1, _STR_BASE) and isinstance(a2, _STR_BASE):
            return
    if op in ('+', '-'):
        ...
        if isinstance(a1, datetime.date):
            if isinstance(a2, datetime.date):
                return
