...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ValidateInfoset.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 15880 bytes
"""
...
def validate(val, modelXbrl, infosetModelXbrl):
    ...
    if infoset.type == Type.INSTANCE:
        ...
        if len(modelXbrl.factsInInstance) != len(infosetModelXbrl.factsInInstance):
            modelXbrl.error('arelle:infosetTest', (_('Fact counts mismatch, testcase instance %(foundFactCount)s, infoset instance %(expectedFactCount)s')),
              modelObject=(
             modelXbrl.modelDocument, infosetModelXbrl.modelDocument),
            ...
            for i, instFact in enumerate(modelXbrl.facts):
                ...
                for fact in infosetFacts[instFact.qname]:
                    if fact.isTuple and fact.isDuplicateOf(instFact, deemP0Equal=True):
                        ...
                        if fact.isItem:
                            if fact.isVEqualTo(instFact, deemP0Equal=True):
                                infosetFact = fact
                                break
                ...
                if infosetFact is None:
                    ...
                    modelXbrl.error('arelle:infosetTest', (_('Fact %(factNumber)s mismatch %(concept)s')),
                    ...
                    if ptvPeriodType:
                        if ptvPeriodType != instFact.concept.periodType:
                            modelXbrl.error('arelle:infosetTest', (_('Fact %(factNumber)s periodType mismatch %(concept)s expected %(expectedPeriodType)s found %(foundPeriodType)s')),
                              modelObject=(
                             instFact, infosetFact),
                              factNumber=(i + 1),
                              concept=(instFact.qname),
                              expectedPeriodType=ptvPeriodType,
                              foundPeriodType=(instFact.concept.periodType))
                    if ptvBalance:
                        if ptvBalance != instFact.concept.balance:
                            modelXbrl.error('arelle:infosetTest', (_('Fact %(factNumber)s balance mismatch %(concept)s expected %(expectedBalance)s found %(foundBalance)s')),
                              modelObject=(
                             instFact, infosetFact),
                              factNumber=(i + 1),
                              concept=(instFact.qname),
                    ...
                    if ptvDecimals:
                        if ptvDecimals != str(inferredDecimals(fact)):
                            modelXbrl.error('arelle:infosetTest', (_('Fact %(factNumber)s inferred decimals mismatch %(concept)s expected %(expectedDecimals)s found %(inferredDecimals)s')),
                              modelObject=(
                             instFact, infosetFact),
                              factNumber=(i + 1),
                              concept=(instFact.qname),
                        ...
                        if ptvPrecision and ptvPrecision != str(inferredPrecision(fact)):
                            modelXbrl.error('arelle:infosetTest', (_('Fact %(factNumber)s inferred precision mismatch %(concept)s expected %(expectedPrecision)s found %(inferredPrecision)s')),
                              modelObject=(
                             instFact, infosetFact),
                              factNumber=(i + 1),
                              concept=(instFact.qname),
        ...
        for arcElt in XmlUtil.children(infoset.xmlRootElement, 'http://www.xbrl.org/2003/ptv', 'arc'):
            ...
            if fromObj is None:
                modelXbrl.error('arelle:infosetTest', (_('Arc fromPath not found: %(fromPath)s')),
                  modelObject=arcElt,
            ...
            if linkType in ('label', 'reference'):
                ...
                if linkType == 'label':
                    ...
                    if foundLabel != expectedLabel:
                        modelXbrl.error('arelle:infosetTest', (_("Label expected='%(expectedLabel)s', found='%(foundLabel)s'")),
                          modelObject=arcElt,
                        ...
                        for refrel in modelXbrl.relationshipSet(XbrlConst.conceptReference, extRole).fromModelObject(fromObj):
                            ...
                            if resRole == ref.role:
                                ...
                                if foundRef != expectedRef:
                                    modelXbrl.error('arelle:infosetTest', (_("Reference inner text expected='%(expectedRef)s, found='%(foundRef)s'")),
                                      modelObject=arcElt,
                        ...
                        if referenceFound:
                            continue
                modelXbrl.error('arelle:infosetTest', (_("%(linkType)s not found containing '%(text)s' linkRole %(linkRole)s")),
                  modelObject=arcElt,
                ...
                if toObj is None:
                    modelXbrl.error('arelle:infosetTest', (_('Arc toPath not found: %(toPath)s')),
                      modelObject=arcElt,
                ...
                if not found:
                    modelXbrl.error('arelle:infosetTest', (_('Arc not found: from %(fromPath)s, to %(toPath)s, role %(arcRole)s, linkRole $(extRole)s')),
                      modelObject=arcElt,
                      fromPath=(arcElt.get('fromPath')),
                      toPath=(arcElt.get('toPath')),
                      arcRole=arcRole,
        ...
        for itemElt in XmlUtil.children(infoset.xmlRootElement, None, 'item'):
            try:
                ...
                if factElt.qname != factQname:
                    modelXbrl.error('arelle:infosetTest', (_('Fact %(sPointer)s mismatch Qname, expected %(qnElt)s, observed %(factQname)s')),
                      modelObject=itemElt,
                      sPointer=sPointer,
                      qnElt=factQname,
                    ...
                    if not factElt.isItem or factElt.context is None:
                        modelXbrl.error('arelle:infosetTest', (_('Fact %(sPointer)s has no context: %(qnElt)s')),
                          modelObject=(
                         itemElt, factElt),
                          sPointer=sPointer,
                          qnElt=factQname)
                        ...
                        for memberElt in memberElts:
                            ...
                            if not (qnDim in context.qnameDims and not isDefault or qnDim in factElt.modelXbrl.qnameDimensionDefaults and isDefault):
                                modelXbrl.error('arelle:infosetTest', (_('Fact %(sPointer)s (qnElt)s dimension mismatch %(qnDim)s')),
                                  modelObject=(
                                 itemElt, factElt, context),
                                  sPointer=sPointer,
                                  qnElt=factQname,
                                  qnDim=qnDim)

                if numNonDefaults != len(context.qnameDims):
                    modelXbrl.error('arelle:infosetTest', (_('Fact %(sPointer)s (qnElt)s dimensions count mismatch')),
                      modelObject=(
                     itemElt, factElt, context),
                      sPointer=sPointer,
                      qnElt=factQname)
            except (IndexError, ValueError, AttributeError) as err:
                modelXbrl.error('arelle:infosetTest', (_('Invalid entity fact dimensions infoset sPointer: %(test)s, error details: %(error)s')),
                  modelObject=itemElt,
    ...
    try:
        ...
        while sourceElt is not None and comparisonElt is not None:
            ...
            if sourceEltTag != comparisonEltTag:
                modelXbrl.error('arelle:tableModelElementMismatch', (_('Table model expecting %(elt1)s found %(elt2)s source line %(elt1line)s comparison line %(elt2line)s')),
                  modelObject=modelXbrl,
                  elt1=sourceEltTag,
                  elt2=comparisonEltTag,
                  elt1line=(sourceElt.sourceline),
                  elt2line=(comparisonElt.sourceline))
            else:
                text1 = (sourceElt.text or '').strip() or '(none)'
                text2 = (comparisonElt.text or '').strip() or '(none)'
                if text1 != text2:
                    modelXbrl.error('arelle:tableModelTextMismatch', (_('Table model comparison element %(elt)s expecting text %(text1)s found %(text2)s source line %(elt1line)s comparison line %(elt2line)s')),
                      modelObject=modelXbrl,
                      elt=(sourceElt.tag),
                      text1=text1,
                      text2=text2,
            ...
            if attrs1 != attrs2:
                modelXbrl.error('arelle:tableModelAttributesMismatch', (_('Table model comparison element %(elt)s expecting attributes %(attrs1)s found %(attrs2)s source line %(elt1line)s comparison line %(elt2line)s')),
                  modelObject=modelXbrl,
                  elt=(sourceElt.tag),
        ...
        modelXbrl.error('arelle:tableModelFileError', (_('Table model comparison file %(xmlfile)s error %(error)s')),
          modelObject=modelXbrl,
          xmlfile=comparisonFile,
          error=(str(err)))