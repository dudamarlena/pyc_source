...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ValidateHmrc.py
            ...
            if modelXbrl.modelDocument.type == ModelDocument.Type.INSTANCE or modelXbrl.modelDocument.type == ModelDocument.Type.INLINEXBRL:
                ...
                def checkFacts(facts):
                    for f1 in facts:
                        ...
                        if getattr(f1, 'xValid', 0) >= 4:
                            ...
                            if busNamespacePattern.match(factNamespaceURI):
                                if factLocalName in busLocalNames:
                                    busItems[factLocalName] = f1
                            if gaapNamespacePattern.match(factNamespaceURI):
                                if factLocalName in gaapLocalNames:
                                    gaapItems[factLocalName] = f1
                            if ifrsNamespacePattern.match(factNamespaceURI):
                                if factLocalName in ifrsLocalNames:
                                    ifrsItems[factLocalName] = f1
                            if direpNamespacePattern.match(factNamespaceURI):
                                if factLocalName in direpLocalNames:
                            ...
                            if context is not None:
                                if f1 in uniqueFacts:
                                    ...
                                    if f1.effectiveValue != f2.effectiveValue:
                                        modelXbrl.error('HMRC.14', (_('Inconsistent duplicate facts %(fact)s context %(contextID)s and %(contextID2)s.')),
                                          modelObject=(
                                         f1, f2),
                                          fact=(f1.qname),
                        ...
                        if f1.isNumeric:
                            if f1.precision:
                                modelXbrl.error('HMRC.5.4', (_("Numeric fact %(fact)s of context %(contextID)s has a precision attribute '%(precision)s'")),
                                  modelObject=f1,
                                  fact=(f1.qname),
                                  contextID=(f1.contextID),
                            ...
                            try:
                                if f1.xValue < 0:
                                    ...
                                    if not labelHasNegativeTermPattern.match(label):
                                        modelXbrl.error('HMRC.5.3', (_("Numeric fact %(fact)s of context %(contextID)s has a negative value '%(value)s' but label does not have a bracketed negative term (using parentheses): %(label)s")),
                                          modelObject=f1,
                                          fact=(f1.qname),
                                          contextID=(f1.contextID),
                                          value=(f1.value),
                ...
                if isAccounts:
                    if 'StartDateForPeriodCoveredByReport' not in busItems:
                        modelXbrl.error('HMRC.02', (_('Period Start Date (uk-bus:StartDateForPeriodCoveredByReport) is missing.')),
                          modelObject=modelXbrl)
                    else:
                        if busItems['StartDateForPeriodCoveredByReport'].value < '2008-04-06':
                            modelXbrl.error('HMRC.02', (_('Period Start Date (uk-bus:StartDateForPeriodCoveredByReport) must be 6 April 2008 or later.')),
                              modelObject=modelXbrl)
                        else:
                            ...
                            if 'DateApprovalAccounts' not in gaapItems:
                                if 'DateAuthorisationFinancialStatementsForIssue' not in ifrsItems:
                                    modelXbrl.error('HMRC.07', (_('Balance Sheet Date of Approval (uk-gaap:DateApprovalAccounts) is missing OR Balance Sheet Date of Approval (uk-ifrs:DateAuthorisationFinancialStatementsForIssue) is missing.')),
                                      modelObject=modelXbrl)
                            if 'NameDirectorSigningAccounts' not in gaapItems:
                                if 'ExplanationOfBodyOfAuthorisation' not in ifrsItems:
                                    modelXbrl.error('HMRC.08', (_('Name of Director Approving Balance Sheet (uk-gaap:NameDirectorSigningAccounts) is missing OR Name of Director Approving Balance Sheet (ifrs:ExplanationOfBodyOfAuthorisation) is missing.')),
                                      modelObject=modelXbrl)
                            if 'ProfitLossForPeriod' not in gaapItems:
                                if 'ProfitLoss' not in ifrsItems:
                                    modelXbrl.error('HMRC.11', (_('Profit or Loss for the period (uk-gaap:ProfitLossForPeriod OR ifrs:ProfitLoss) is missing.')),
                                      modelObject=modelXbrl)
                        if companyReferenceNumberContexts:
                            if 'UKCompaniesHouseRegisteredNumber' not in busItems:
                                modelXbrl.error('HMRC.16.1', (_('Company Reference Number (uk-bus:UKCompaniesHouseRegisteredNumber) is missing.')),
                                  modelObject=modelXbrl)
                            else:
                                ...
                                for compRefNbr, contextIds in companyReferenceNumberContexts.items():
                                    if compRefNbr != factCompNbr:
                                        modelXbrl.error('HMRC.16.2', (_('Context entity identifier (%(entityIdentifier)s) does not match Company Reference Number (uk-bus:UKCompaniesHouseRegisteredNumber) Location: Accounts (context id %(contextID)s).')),
                                          modelObject=modelXbrl,
