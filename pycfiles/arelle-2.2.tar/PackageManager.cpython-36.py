...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\PackageManager.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 29652 bytes
"""
...
def parsePackage(cntlr, filesource, metadataFile, fileBase, errors=[]):
    ...
    if ArchiveFileIOError is None:
        ...
        try:
            ...
            cntlr.addToLog((_('Package catalog syntax error %(error)s')), messageArgs={'error': str(err)},
            ...
            for n in root.iter(tag=(nsPrefix + '*')):
                for eltName in ('name', 'description', 'publisher'):
                    ...
                    for lang, elts in langElts.items():
                        if not lang:
                            cntlr.addToLog((_('Multi-lingual element %(element)s has no in-scope xml:lang attribute')), messageArgs={'element': eltName},
                            ...
                            cntlr.addToLog((_('Multi-lingual element %(element)s has multiple (%(count)s) in-scope xml:lang %(lang)s elements')), messageArgs={'element':eltName, 
                             'lang':lang,  'count':len(elts)},
            ...
            if prefixValue:
                if replaceValue is not None:
                    if prefixValue not in remappings:
                        ...
                        if replaceValue.endswith(os.sep):
                            if not _normedValue.endswith(os.sep):
                                _normedValue += os.sep
                        remappings[prefixValue] = _normedValue
                    else:
                        cntlr.addToLog((_('Package catalog duplicate rewrite start string %(rewriteStartString)s')), messageArgs={'rewriteStartString': prefixValue},
...
def init(cntlr, loadPackagesConfig=True):
    ...
    if packagesConfig is None:
        packagesConfig = {'packages':[],  'remappings':{}}
        packagesConfigChanged = False
            ...
            if openFileSource is None:
                ...
                if filesource.isZip:
                    ...
                    if len(topLevelDirectories) != 1:
                        ...
                        any('/META-INF/' in f for f in _dir) or cntlr.addToLog((_('Taxonomy package does not contain a subdirectory META-INF')), messageCode='tpe:metadataDirectoryNotFound',
                          file=(os.path.basename(packageFilename)),
                          level=(logging.ERROR))
                    ...
                    if any(f.endswith('/META-INF/taxonomyPackage.xml') for f in _dir):
                        ...
                        cntlr.addToLog((_('Taxonomy package does not contain a metadata file */META-INF/taxonomyPackage.xml')), messageCode='tpe:metadataFileNotFound',
                          file=(os.path.basename(packageFilename)),
                          level=(logging.ERROR))
                        errors.append('tpe:metadataFileNotFound')
                _metaInf = packageFiles or '{}/META-INF/'.format(os.path.splitext(os.path.basename(packageFilename))[0])
                if packageManifestName:
                    ...
                    if _metaInf + 'taxonomyPackage.xml' in _dir:
                        ...
                        if any(pf.startswith('_metaInf') for pf in packageFiles):
                            if any(not pf.startswith(_metaInf) for pf in packageFiles):
                                packageFiles = [pf for pf in packageFiles if pf.startswith(_metaInf)]
                            else:
                                if any(pf.startswith('META-INF/') for pf in packageFiles):
                                    if any(not pf.startswith('META-INF/') for pf in packageFiles):
                            ...
                            cntlr.addToLog((_('Taxonomy package is not a zip file.')), messageCode='tpe:invalidArchiveFormat',
                              file=(os.path.basename(packageFilename)),
                              level=(logging.ERROR))
                ...
                for packageFileUrl, packageFilePrefix, packageFile in packages:
                    ...
                    for prefix, remapping in parsedPackage['remappings'].items():
                        if prefix not in remappings:
                            ...
                            cntlr.addToLog('Package mapping duplicate rewrite start string %(rewriteStartString)s', messageArgs={'rewriteStartString': prefix},
        ...
        for j in range(i - 1, -1, -1):
            ...
            if _packageURL != _packageURL2 and _prefix and _prefix2 and (_prefix.startswith(_prefix2) or _prefix2.startswith(_prefix)):
                ...
                if _url1 == _url2:
                    _url1 = _packageURL
                    _url2 = _packageURL2
                cntlr.addToLog((_('Packages overlap the same rewrite start string %(rewriteStartString)s') if _prefix == _prefix2 else _('Packages overlap rewrite start strings %(rewriteStartString)s and %(rewriteStartString2)s')),
                  messageArgs={'rewriteStartString':_prefix, 
        ...
        if newPackageInfo.get('name'):
            ...
            for i, _packageInfo in enumerate(packagesList):
                if _packageInfo['name'] == name:
                    if _packageInfo['version'] == version:
                        j = i
                        break
