...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/FunctionXfi.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 63767 bytes
"""
...
def unit_numerator(xc, p, args):
    ...
    unit = args[0][0]
    if isinstance(unit, ModelObject) and unit.localName == 'unit' and unit.namespaceURI == XbrlConst.xbrli:
...
def measure_name(xc, p, args):
    ...
    unit = args[0][0]
    if isinstance(unit, ModelObject) and unit.localName == 'measure' and unit.namespaceURI == XbrlConst.xbrli:
...
def parent_child(args, parentName, childName, findDescendant=False):
    ...
    parent = args[0][0]
    if isinstance(parent, ModelObject) and parent.localName == parentName and parent.namespaceURI == XbrlConst.xbrli:
...
def is_period_type(args, periodElement):
    ...
    period = args[0][0]
    if isinstance(period, ModelObject) and period.localName == 'period' and period.namespaceURI == XbrlConst.xbrli:
...
def period_datetime(p, args, periodElement):
    ...
    if len(args[0]) != 1:
        raise XPathContext.FunctionArgType(1, 'xbrl:period')
    period = args[0][0]
    if isinstance(period, ModelObject) == 1 and period.localName == 'period' and period.namespaceURI == XbrlConst.xbrli:
...
def uncovered_aspects(xc, p, args, dimensionAspects=False):
    ...
    return [a if dimensionAspects else aspectToToken.get(a) for a in uncoveredAspects if a != Aspect.DIMENSIONS and isinstance(a, QName) == dimensionAspects]
...
def s_equal_test(node1, node2):
    if isinstance(node1, (ModelFact, ModelInlineFact)) and node1.isItem and isinstance(node2, (ModelFact, ModelInlineFact)) and node2.isItem:
    ...
    if isinstance(node1, ModelObject):
        if isinstance(node2, ModelObject):
            ...
            return False
    elif isinstance(node1, ModelAttribute) and isinstance(node2, ModelAttribute):
...
def concepts_from_local_name(xc, p, args):
    ...
    if len(args) == 2:
        ...
        return [c.qname for c in xc.modelXbrl.nameConcepts.get(localName, ()) if (c.isItem or c.isTuple) and bool(nsPattern.search(c.qname.namespaceURI))]
    else:
...
def concepts_from_local_name_pattern(xc, p, args):
    ...
    if len(args) == 2:
        nsPattern = re.compile(stringArg(xc, args, 1, 'xs:string'))
        return [c.qname for c in xc.modelXbrl.qnameConcepts.values() if (c.isItem or c.isTuple) and bool(localNamePattern.search(c.name)) and bool(nsPattern.search(c.qname.namespaceURI))]
    else:
        return [c.qname for c in xc.modelXbrl.qnameConcepts.values() if (c.isItem or c.isTuple) and bool(localNamePattern.search(c.name))]
...
def fact_dimension_s_equal2(xc, p, args):
    ...
    if context1 is not None:
        if context2 is not None:
            ...
            if dimValue2 is not None and isinstance(dimValue2, ModelDimensionValue):
...
def fact_footnotes(xc, p, args):
    ...
    if not footnoteroleURI:
        footnoteroleURI = XbrlConst.footnote
...
def concept_relationships(xc, p, args, nestResults=False):
    ...
    if not linkroleURI:
        ...
        if linkroleURI == 'XBRL-all-linkroles':
            linkroleURI = None
        arcroleURI = stringArg(xc, args, 2, 'xs:string')
        axis = stringArg(xc, args, 3, 'xs:string')
        if axis not in ('descendant', 'child', 'ancestor', 'parent', 'sibling', 'sibling-or-self'):
...
def create_element(xc, p, args):
    ...
    if scratchpadXmlDocUrl in xc.modelXbrl.urlDocs:
        ...
        modelDocument = ModelDocument.create(xc.modelXbrl, ModelDocument.Type.UnknownXML, scratchpadXmlDocUrl, initialXml="<xfc:dummy xmlns:xfc='http://www.xbrl.org/2012/function/creation'/>")
    newElement = XmlUtil.addChild(modelDocument.xmlRootElement, qn, attributes=attrParam, text=value)
    if childElements:
