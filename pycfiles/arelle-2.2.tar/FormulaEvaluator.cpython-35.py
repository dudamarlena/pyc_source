...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/FormulaEvaluator.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 77414 bytes
"""
    ...
    try:
        ...
        if isinstance(varSet, ModelExistenceAssertion):
            ...
            if xpCtx.formulaOptions.traceSatisfiedAssertions and result or (xpCtx.formulaOptions.traceUnsatisfiedAssertions or xpCtx.formulaOptions.errorUnsatisfiedAssertions) and not result:
                xpCtx.modelXbrl.log('ERROR' if xpCtx.formulaOptions.errorUnsatisfiedAssertions and not result else 'INFO', 'formula:assertionSatisfied' if result else 'formula:assertionUnsatisfied', _('%(label)s'), modelObject=varSet, label=varSet.logLabel(), messageCodes=('formula:assertionSatisfied',
                                                                                                                                                                                                                                                                                  'formula:assertionUnsatisfied'))
            if xpCtx.formulaOptions.traceVariableSetExpressionResult:
                xpCtx.modelXbrl.info('formula:trace', _('Existence Assertion %(xlinkLabel)s \nResult: %(result)s'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, result=result)
            msg = varSet.message(result)
            if msg is not None:
                xpCtx.inScopeVars[XbrlConst.qnEaTestExpression] = varSet.test
                xpCtx.modelXbrl.info('message:' + (varSet.id or varSet.xlinkLabel or _('unlabeled variableSet')), msg.evaluate(xpCtx), modelObject=varSet, messageCodes=('message:{variableSetID|xlinkLabel}', ))
        ...
        if xpCtx.formulaOptions.traceVariableSetExpressionResult and initialTraceCount == xpCtx.modelXbrl.logCount.get(logging._checkLevel('INFO'), 0):
            xpCtx.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s had no xpCtx.evaluations'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel)
        if xpCtx.formulaOptions.timeVariableSetEvaluation:
            xpCtx.modelXbrl.info('formula:time', _('Variable set %(xlinkLabel)s time for %(count)s evaluations: %(time)s'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, count=varSet.evaluationNumber, time=format_string(xpCtx.modelXbrl.modelManager.locale, '%.3f', time.time() - timeEvaluationsStarted))
        xpCtx.variableSet = None
    except XPathContext.XPathException as err:
        xpCtx.modelXbrl.error(err.code, _('Variable set %(label)s \nException: %(error)s'), modelObject=varSet, label=varSet.logLabel(), error=err.message)
        xpCtx.variableSet = None

    if xpCtx.formulaOptions.traceVariableSetExpressionResult:
        xpCtx.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s evaluations: %(evaluations)s x %(variables)s'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, evaluations=len(xpCtx.evaluations), variables=max(len(e) for e in xpCtx.evaluations) if xpCtx.evaluations else 0)
...
def evaluateVar(xpCtx, varSet, varIndex, cachedFilteredFacts, uncoveredAspectFacts):
    if varIndex == len(varSet.orderedVariableRelationships):
        ...
        if xpCtx.varBindings and anyFactVar and not anyBoundFactVar:
            if xpCtx.formulaOptions.traceVariableSetExpressionResult:
                xpCtx.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s skipped evaluation, all fact variables have fallen back'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel)
            return
        ...
        if evaluationIsUnnecessary(thisEvaluation, xpCtx):
            if xpCtx.formulaOptions.traceVariableSetExpressionResult:
                xpCtx.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s skipped non-different or fallback evaluation, duplicates another evaluation'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel)
            varSet.evaluationNumber += 1
            if xpCtx.formulaOptions.timeVariableSetEvaluation:
                now = time.time()
                xpCtx.modelXbrl.info('formula:time', _('Variable set %(xlinkLabel)s skipped evaluation %(count)s: %(time)s sec'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, count=varSet.evaluationNumber, time=format_string(xpCtx.modelXbrl.modelManager.locale, '%.3f', now - varSet.timeEvaluationStarted))
        ...
        for vQn, vBoundFact in thisEvaluation.items():
            if vQn not in xpCtx.evaluationHashDicts:
                xpCtx.evaluationHashDicts[vQn] = defaultdict(set)
            xpCtx.evaluationHashDicts[vQn][hash(vBoundFact)].add(len(xpCtx.evaluations))

        xpCtx.evaluations.append(thisEvaluation)
        for precondition in varSet.preconditions:
            result = precondition.evalTest(xpCtx)
            if xpCtx.formulaOptions.traceVariableSetExpressionResult:
                xpCtx.modelXbrl.info('formula:trace', _('Variable set %(xlinkLabel)s \nPrecondition %(precondition)s \nResult: %(result)s'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, precondition=precondition.xlinkLabel, result=result)
            if not result:
                if xpCtx.formulaOptions.timeVariableSetEvaluation:
                    varSet.evaluationNumber += 1
                    now = time.time()
                    xpCtx.modelXbrl.info('formula:time', _('Variable set %(xlinkLabel)s precondition blocked evaluation %(count)s: %(time)s sec'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, count=varSet.evaluationNumber, time=format_string(xpCtx.modelXbrl.modelManager.locale, '%.3f', now - varSet.timeEvaluationStarted))
                    varSet.timeEvaluationStarted = now
                if xpCtx.isRunTimeExceeded:
                    raise XPathContext.RunTimeExceededException()
                return
        ...
        if isinstance(varSet, ModelExistenceAssertion):
            ...
            if isinstance(varSet, ModelTuple):
                ...
                if isinstance(varSet, ModelFormula):
                    ...
                    if msg is not None:
                        ...
                        xpCtx.modelXbrl.info('message:' + (varSet.id or varSet.xlinkLabel or _('unlabeled variableSet')), msg.evaluate(xpCtx), modelObject=varSet, label=varSet.logLabel(), messageCodes=('message:{variableSetID|xlinkLabel}', ))
                    ...
                    if xpCtx.formulaOptions.traceSatisfiedAssertions and result or (xpCtx.formulaOptions.traceUnsatisfiedAssertions or xpCtx.formulaOptions.errorUnsatisfiedAssertions) and not result:
                        ...
                        xpCtx.modelXbrl.log('ERROR' if xpCtx.formulaOptions.errorUnsatisfiedAssertions and not result else 'INFO', 'formula:assertionSatisfied' if result else 'formula:assertionUnsatisfied', _('%(label)s%(factVarBindings)s'), modelObject=_modelObjects, label=varSet.logLabel(), factVarBindings=''.join(factVarBindings) + ('\n' if factVarBindings else ''), messageCodes=('formula:assertionSatisfied',
                                                                                                                                                                                                                                                                                                                                                                                                  'formula:assertionUnsatisfied'))
                        del _modelObjects[:]
                ...
                if xpCtx.formulaOptions.traceVariableSetExpressionResult:
                    ...
                    xpCtx.modelXbrl.info('formula:trace', _('%(variableSetType)s %(xlinkLabel)s{0} \nExpression: %(expression)s \nEvaluated: %(evaluatedExpression)s \nResult: %(result)s').format(' \n%(label)s' if label else ''), modelObject=varSet, variableSetType=traceOf, xlinkLabel=varSet.xlinkLabel, label=label, result=result, expression=expression, evaluatedExpression=''.join(xpCtx.traceEffectiveVariableValue(varSet, expr) for grp in expressionVariablesPattern.findall(expression) for expr in grp))
            ...
            if xpCtx.formulaOptions.timeVariableSetEvaluation:
                varSet.evaluationNumber += 1
                now = time.time()
                xpCtx.modelXbrl.info('formula:time', _('Variable set %(xlinkLabel)s completed evaluation %(count)s: %(time)s sec'), modelObject=varSet, xlinkLabel=varSet.xlinkLabel, count=varSet.evaluationNumber, time=format_string(xpCtx.modelXbrl.modelManager.locale, '%.3f', now - varSet.timeEvaluationStarted))
                varSet.timeEvaluationStarted = now
            if xpCtx.isRunTimeExceeded:
                raise XPathContext.RunTimeExceededException()
            for varScopeRel in xpCtx.modelXbrl.relationshipSet(XbrlConst.variablesScope).fromModelObject(varSet):
                try:
                    ...
                    if resultQname:
                        ...
                        if overriddenInScopeVar is not None:
                    ...
                    xpCtx.modelXbrl.error(err.code, _('Variable set chained in scope of variable set %(variableset)s \nException: \n%(error)s'), modelObject=(
                     varSet, varScopeRel.toModelObject), variableSet=varSet.logLabel(), error=err.message)
        ...
        var = vb.var
        if vb.isFactVar:
            vb.aspectsDefined = set(aspectModels[varSet.aspectModel])
            vb.values = None
            ...
            if varHasNoVariableDependencies and varQname in cachedFilteredFacts:
                ...
                if xpCtx.formulaOptions.traceVariableFilterWinnowing:
                    xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s: start with %(factCount)s facts previously cached after explicit filters'), modelObject=var, variable=varQname, factCount=len(facts))
            else:
                if var.fromInstanceQnames:
                    ...
                    if groupFilteredFactsKey in cachedFilteredFacts:
                        ...
                        if xpCtx.formulaOptions.traceVariableFilterWinnowing:
                            xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s: start with %(factCount)s facts previously cached before variable filters'), modelObject=var, variable=varQname, factCount=len(facts))
                    else:
                        facts = set.union(*[inst.factsInInstance if varHasNilFacts else inst.nonNilFactsInInstance for inst in vb.instances])
                        if xpCtx.formulaOptions.traceVariableFilterWinnowing:
                            xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s filtering: start with %(factCount)s facts'), modelObject=var, variable=varQname, factCount=len(facts))
                        checkVarSetFilterInfo(varSet)
                        facts = trialFilterFacts(xpCtx, vb, facts, varSet.groupFilterRelationships, 'group', varSet=varSet)
                        vb.aspectsCovered.clear()
                    ...
                    if varHasNoVariableDependencies:
                        cachedFilteredFacts[varQname] = (
                         facts, vb.aspectsDefined, vb.aspectsCovered)
            considerFallback = bool(var.fallbackValueProg)
            if varSet.implicitFiltering == 'true' and any(_vb.isFactVar and not _vb.isFallback for _vb in xpCtx.varBindings.values()):
                pass
        factCount = len(facts)
        facts = implicitFilter(xpCtx, vb, facts, uncoveredAspectFacts)
        if considerFallback and varHasNoVariableDependencies and factCount and len(facts) > 0 and len(xpCtx.varBindings) > 1:
            ...
            if trial_for_svc_311_1_like:
                ...
                if xpCtx.formulaOptions.traceVariableFiltersResult:
                    xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s: filters result %(result)s'), modelObject=var, variable=varQname, result=str(vb.facts))
                if considerFallback:
                    vb.values = xpCtx.evaluate(var.fallbackValueProg)
                    if xpCtx.formulaOptions.traceVariableExpressionResult:
                        xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s: fallbackValue result %(result)s'), modelObject=var, variable=varQname, result=str(vb.values))
                else:
                    if vb.isGeneralVar:
                        if var.fromInstanceQnames:
                            contextItem = [inst.modelDocument.xmlRootElement for qn in var.fromInstanceQnames for instSeq in (
                             xpCtx.inScopeVars[qn],) for inst in instSeq if isinstance(instSeq, (list, tuple)) else (instSeq,)]
                        else:
                            contextItem = xpCtx.modelXbrl.modelDocument.xmlRootElement
                        vb.values = xpCtx.flattenSequence(xpCtx.evaluate(var.selectProg, contextItem=contextItem))
                        if xpCtx.formulaOptions.traceVariableExpressionResult:
                            xpCtx.modelXbrl.info('formula:trace', _('General Variable %(variable)s: select result %(result)s'), modelObject=var, variable=varQname, result=str(vb.values))
                    elif vb.isParameter:
            ...
            for evaluationResult in vb.evaluationResults:
                ...
                if xpCtx.formulaOptions.traceVariableFiltersResult:
                    xpCtx.modelXbrl.info('formula:trace', _('%(variableType)s %(variable)s: bound value %(result)s'), modelObject=var, variableType=vb.resourceElementName, variable=varQname, result=str(evaluationResult))
                if xpCtx.isRunTimeExceeded:
                    raise XPathContext.RunTimeExceededException()
...
def trialFilterFacts(xpCtx, vb, facts, filterRelationships, filterType, var=None, varSet=None):
    ...
    if filterInfo is not None and filterInfo and len(noComplHandledFilterRels) > 0:
        ...
        if isinstance(_filter, ModelFilter):
            if filterType is None and len(facts) == 0:
                ...
                if xpCtx.formulaOptions.traceVariableFilterWinnowing:
                    ...
                    (
                     xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s %(filterType)s %(filter)s filter %(xlinkLabel)s passes %(factCount)s facts %(allFacts)s'), modelObject=vb.var, variable=vb.qname, filterType=typeLbl, filter=_filter.localName, xlinkLabel=_filter.xlinkLabel, factCount=len(result), allFacts=allFacts),)
                if orFilter:
                    factSet |= result
                else:
                    facts = result
    ...
    if orFilter:
        facts = factSet
    if filterInfo is not None and filterInfo and len(complHandledFilterRels) > 0:
...
def filterFacts(xpCtx, vb, facts, filterRelationships, filterType):
    ...
    for varFilterRel in filterRelationships:
        ...
        if isinstance(_filter, ModelFilter):
            ...
            if xpCtx.formulaOptions.traceVariableFilterWinnowing:
                (
                 xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s %(filterType)s %(filter)s filter %(xlinkLabel)s passes %(factCount)s facts'), modelObject=vb.var, variable=vb.qname, filterType=typeLbl, filter=_filter.localName, xlinkLabel=_filter.xlinkLabel, factCount=len(result)),)
            if orFilter:
                factSet |= result
            else:
                facts = result
    ...
    for varFilterRel in filterRelationships:
        ...
        if isinstance(_filter, ModelAspectCover):
            if varFilterRel.isCovered:
                ...
                if isinstance(_filter, ModelBooleanFilter) and varFilterRel.isCovered:
...
def implicitFilter(xpCtx, vb, facts, uncoveredAspectFacts):
    ...
    if len(xpCtx.varBindings) == 1:
        ...
        if isinstance(f, ModelFact) and f.isTuple and all(f is None for a, f in uncoveredAspectFacts.items() if a != Aspect.DIMENSIONS):
    ...
    if xpCtx.formulaOptions.traceVariableFilterWinnowing:
        ...
        for aspect in aspects:
            if uncoveredAspectFacts.get(aspect, 'none') is not None:
                _facts = [fact for fact in _facts if aspectMatches(xpCtx, uncoveredAspectFacts.get(aspect), fact, aspect)]
                a = str(aspect) if isinstance(aspect, QName) else Aspect.label[aspect]
                xpCtx.modelXbrl.info('formula:trace', _('Fact Variable %(variable)s implicit filter %(aspect)s passes %(factCount)s facts'), modelObject=vb.var, variable=vb.qname, aspect=a, factCount=len(facts))
                if len(_facts) == 0:
                    break
        ...
        if testableAspectFacts:
            _facts = [fact for fact in facts if all(aspectMatches(xpCtx, uncoveredAspectFact, fact, aspect) for aspect, uncoveredAspectFact in testableAspectFacts)]
        else:
            _facts = facts
    return _facts
        ...
        if isinstance(dimValue1, ModelDimensionValue):
            if dimValue1.isExplicit:
                ...
                if isinstance(dimValue2, (ModelDimensionValue, DimValuePrototype)):
                    ...
                    if not XbrlUtil.nodesCorrespond(fact1.modelXbrl, dimValue1.typedMember, dimValue2.typedMember, dts2=fact2.modelXbrl):
                        return False
                elif dimValue2 is None:
                    pass
                return False
                ...
                if isinstance(dimValue2, QName):
                    ...
                    if isinstance(dimValue2, (ModelDimensionValue, DimValuePrototype)):
                        ...
                        if dimValue1 != dimValue2.memberQname:
                            return False
                    elif dimValue2 is None and fact1.modelXbrl == fact2.modelXbrl:
                        return False
            else:
                if dimValue1 is None:
                    if isinstance(dimValue2, QName) and fact1.modelXbrl == fact2.modelXbrl:
                        return False
...
def evaluationIsUnnecessary(thisEval, xpCtx):
    ...
    if otherEvals:
        ...
        if all(e is None for e in thisEval.values()):
            return True
        otherEvalSets = [otherEvalHashDicts[vQn][hash(vBoundFact)] for vQn, vBoundFact in thisEval.items() if vBoundFact is not None and vQn in otherEvalHashDicts and hash(vBoundFact) in otherEvalHashDicts[vQn]]
        if otherEvalSets:
            matchingEvals = [otherEvals[i] for i in set.intersection(*otherEvalSets)]
        else:
            matchingEvals = otherEvals
        varBindings = xpCtx.varBindings
        vQnDependentOnOtherVarFallenBackButBoundInOtherEval = set(vQn for vQn, vBoundFact in thisEval.items() if vBoundFact is not None and any(varBindings[varRefQn].isFallback and any(m[varRefQn] is not None for m in matchingEvals) for varRefQn in varBindings[vQn].var.variableRefs()))
        return any(all([vBoundFact == matchingEval[vQn] for vQn, vBoundFact in thisEval.items() if vBoundFact is not None and vQn not in vQnDependentOnOtherVarFallenBackButBoundInOtherEval]) for matchingEval in matchingEvals)
    return False
...
def produceOutputFact(xpCtx, formula, result):
    ...
    if isinstance(conceptQname, VariableBindingError):
        xpCtx.modelXbrl.error(conceptQname.err, _('Formula %(label)s concept: %(concept)s'), modelObject=formula, label=formula.logLabel(), concept=conceptQname.msg)
        modelConcept = None
    else:
    ...
    if modelConcept is None or (not modelConcept.isTuple if isTuple else not modelConcept.isItem):
        xpCtx.modelXbrl.error('xbrlfe:missingConceptRule', _('Formula %(label)s concept %(concept)s is not a %(element)s'), modelObject=formula, label=formula.logLabel(), concept=conceptQname, element=formula.localName)
    ...
    if not isTuple:
        ...
        if isinstance(entityIdentScheme, VariableBindingError):
            pass
        xpCtx.modelXbrl.error(str(entityIdentScheme), _('Formula %(label)s entity identifier scheme: %(scheme)s'), modelObject=formula, label=formula.logLabel(), scheme=entityIdentScheme.msg)
    ...
    if isinstance(entityIdentValue, VariableBindingError):
        xpCtx.modelXbrl.error(str(entityIdentScheme), _('Formula %(label)s entity identifier value: %(entityIdentifier)s'), modelObject=formula, label=formula.logLabel(), entityIdentifier=entityIdentValue.msg)
    ...
    if isinstance(periodType, VariableBindingError):
        xpCtx.modelXbrl.error(str(periodType), _('Formula %(label)s period type: %(periodType)s'), modelObject=formula, label=formula.logLabel(), periodType=periodType.msg)
    else:
        if periodType == 'instant':
            ...
            if isinstance(periodEndInstant, VariableBindingError):
                xpCtx.modelXbrl.error(str(periodEndInstant), _('Formula %(label)s period end: %(period)s'), modelObject=formula, label=formula.logLabel(), period=periodEndInstant.msg)
            ...
            if isinstance(periodStart, VariableBindingError):
                xpCtx.modelXbrl.error(str(periodStart), _('Formula %(label)s period start: %(period)s'), modelObject=formula, label=formula.logLabel(), period=periodStart.msg)
    ...
    if modelConcept is not None and modelConcept.isNumeric:
        ...
        if isinstance(multDivBy, VariableBindingError):
            pass
        xpCtx.modelXbrl.error(str(multDivBy) if isinstance(multDivBy, VariableBindingError) else 'xbrlfe:missingUnitRule', _('Formula %(label)s unit: %(unit)s'), modelObject=formula, label=formula.logLabel(), unit=multDivBy.msg)
        multiplyBy = ()
        divideBy = ()
    else:
    ...
    if isinstance(divMultBy, VariableBindingError):
        xpCtx.modelXbrl.error(str(multDivBy) if isinstance(divMultBy, VariableBindingError) else 'xbrlfe:missingUnitRule', _('Formula %(label)s unit: %(unit)s'), modelObject=formula, label=formula.logLabel(), unit=divMultBy.msg)
        multiplyBy = ()
        divideBy = ()
    else:
        ...
        if len(multiplyBy) == 0:
            if Aspect.MULTIPLY_BY not in formula.aspectValues and Aspect.MULTIPLY_BY not in formula.aspectProgs and Aspect.DIVIDE_BY not in formula.aspectValues and Aspect.DIVIDE_BY not in formula.aspectProgs:
                xpCtx.modelXbrl.error('xbrlfe:missingUnitRule', _('Formula %(label)s'), modelObject=formula, label=formula.logLabel())
            multiplyBy = (
        ...
        if formula.aspectModel == 'dimensional':
            ...
            if dimQnames:
                for dimQname in dimQnames:
                    ...
                    if isinstance(dimValue, VariableBindingError):
                        xpCtx.modelXbrl.error(dimErr, _('Formula %(label)s dimension %(dimension)s: %(value)s'), modelObject=formula, label=formula.logLabel(), dimension=dimQname, value=dimValue.msg)
                    elif dimConcept.isTypedDimension:
                        if isinstance(dimValue, list):
                            if len(dimValue) != 1 or not isinstance(dimValue[0], ModelObject):
                                xpCtx.modelXbrl.error('xbrlfe:wrongXpathResultForTypedDimensionRule', _('Formula %(label)s dimension %(dimension)s value is not a node: %(value)s'), modelObject=formula, label=formula.logLabel(), dimension=dimQname, value=dimValue)
                                continue
                                dimValue = dimValue[0]
                            dimAspects[dimQname] = dimValue
                        else:
                            if dimValue is not None and xpCtx.modelXbrl.qnameDimensionDefaults.get(dimQname) != dimValue:
                                dimAspects[dimQname] = dimValue
            ...
            for occElt in xpCtx.flattenSequence((segOCCs, scenOCCs)):
                if isinstance(occElt, ModelObject) and occElt.namespaceURI == XbrlConst.xbrldi:
                    xpCtx.modelXbrl.error('xbrlfe:badSubsequentOCCValue', _('Formula %(label)s OCC element %(occ)s covers a dimensional aspect'), modelObject=(
                     formula, occElt), label=formula.logLabel(), occ=occElt.elementQname)
    ...
    if isTuple:
        ...
        if prevCntx is not None:
            ...
            newCntxElt = outputXbrlInstance.createContext(entityIdentScheme, entityIdentValue, periodType, periodStart, periodEndInstant, conceptQname, dimAspects, segOCCs, scenOCCs, afterSibling=xpCtx.outputLastContext.get(outputInstanceQname), beforeSibling=xpCtx.outputFirstFact.get(outputInstanceQname))
        ...
        if modelConcept.isNumeric:
            ...
            if prevUnit is not None:
                ...
                newUnitElt = outputXbrlInstance.createUnit(multiplyBy, divideBy, afterSibling=xpCtx.outputLastUnit.get(outputInstanceQname), beforeSibling=xpCtx.outputFirstFact.get(outputInstanceQname))
            ...
            if valueSeqLen > 1:
                xpCtx.modelXbrl.error('xbrlfe:nonSingletonOutputValue', _('Formula %(label)s value is a sequence of length %(valueSequenceLength)s'), modelObject=formula, label=formula.logLabel(), valueSequenceLength=valueSeqLen)
    if valueSeqLen == 0:
        attrs.append((XbrlConst.qnXsiNil, 'true'))
        v = None
    elif modelConcept.isNumeric and not modelConcept.isFraction:
        ...
        if isinstance(x, Decimal):
            ...
            if isinstance(x, QName):
                ...
                if isinstance(x, datetime.datetime):
                    ...
                    v = xsString(xpCtx, None, x)
            newFact = outputXbrlInstance.createFact(conceptQname, attributes=attrs, text=v, parent=outputLocation, afterSibling=xpCtx.outputLastFact.get(outputInstanceQname))
        ...
        for vb in xpCtx.varBindings.values():
            if vb.isFactVar and not vb.isFallback:
                if aspect == Aspect.DIMENSIONS and formulaUncovered:
                    ...
                    if formulaUncovered and vb.hasAspectValueUncovered(aspect):
                        aspectSourceValue = vb.aspectValue(aspect)
                        break
