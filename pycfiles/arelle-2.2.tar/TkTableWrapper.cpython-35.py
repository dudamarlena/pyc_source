...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/TkTableWrapper.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 32277 bytes
"""
...
def _setup_master(master):
    if master is None:
        if tkinter._support_default_root:
            ...
            raise RuntimeError('No master specified and Tkinter is configured to not support default master')
        return master


class ArrayVar(tkinter.Variable):
    __doc__ = 'Class for handling Tcl arrays.\n\n    An array is actually an associative array in Tcl, so this class supports\n    some dict operations.\n    '
...
class Table(tkinter.Widget):
    __doc__ = 'Create and manipulate tables.'
    _switches = ('holddimensions', 'holdselection', 'holdtags', 'holdwindows', 'keeptitles',
                 '-')
    _tabsubst_format = ('%c', '%C', '%i', '%r', '%s', '%S', '%W')
    _tabsubst_commands = ('browsecommand', 'browsecmd', 'command', 'selectioncommand',
                          'selcmd', 'validatecommand', 'valcmd')
...
def sample_test():
    ...
    test = Table(root, rows=numrows, cols=numcols, state='normal', width=6, height=6, titlerows=2, titlecols=2, roworigin=0, colorigin=0, selectmode='extended', selecttype='cell', rowstretch='last', colstretch='last', rowheight=-26, colwidth=15, browsecmd=browsecmd, flashmode='off', anchor='e', usecommand=1, background='#fffffffff', relief='sunken', command=test_cmd, takefocus=False, rowseparator='\n')
    ...
    for y in range(2, numrows):
        ...
        combobox.bind(sequence='<<ComboboxSelected>>', func=comboValueChanged, add='+')
    ...
    def cellRight(event, *args):
        ...
        if row < top:
            ...
            if col < left:
                ...
                if col < maxCols - titleCols - 1:
                    widget.moveCell(0, 1)
                else:
                    if row < maxRows - titleCols - 1:
