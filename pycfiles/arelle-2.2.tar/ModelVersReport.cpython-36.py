...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelVersReport.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 75116 bytes
"""
...
authoritiesEquivalence = {'http://xbrl.iasb.org':'IFRS', 
 'http://xbrl.ifrs.org':'IFRS',  'http://xbrl.us':'XBRL-US', 
 'http://fasb.org':'XBRL-US',  'http://xbrl.sec.gov':'XBRL-US'}
...
class ModelVersReport(ModelDocument.ModelDocument):
                ...
                if isinstance(modelObject, ModelObject):
                    ...
                    if ns == XbrlConst.ver:
                        if ln == 'action':
                            ...
                            if (ln == 'fromDTS' or ln == 'toDTS') and not getattr(self, ln):
                                ...
                                if schemaRefElts:
                                    if len(schemaRefElts) == 1:
                                        if schemaRefElts[0].get('{http://www.w3.org/1999/xlink}href') is not None:
                                            DTSmodelXbrl = ModelXbrl.load((self.modelXbrl.modelManager), (schemaRefElts[0].get('{http://www.w3.org/1999/xlink}href')),
                                              'loading validation report',
                                              base=(self.baseForElement(schemaRefElts[0])))
                                    else:
                                        DTSmodelXbrl = ModelXbrl.create((self.modelXbrl.modelManager), newDocumentType=(ModelDocument.Type.DTSENTRIES),
                                          url=(self.uri[:-4] + '-' + ln + '.dts'),
                                          isEntry=True)
                                        ...
                                        for schemaRefElt in schemaRefElts:
                                            if schemaRefElt.get('{http://www.w3.org/1999/xlink}href') is not None:
                                                doc = ModelDocument.load(DTSmodelXbrl, (schemaRefElt.get('{http://www.w3.org/1999/xlink}href')),
                            ...
                            elif ln in ('namespaceRename', 'roleChange') and modelAction is not None:
                    ...
                    if self.toDTS is None:
                        ...
                        if ns in (XbrlConst.vercu, XbrlConst.vercb):
                            ...
                            if ns in (XbrlConst.vercd, XbrlConst.verce):
                                if ln in frozenset({'conceptTypeChange', 'conceptNillableChange', 'conceptAttributeChange', 'conceptAttributeDelete', 'conceptPeriodTypeChange', 'conceptBlockChange', 'tupleContentModelChange', 'conceptReferenceAdd', 'conceptAttributeAdd', 'conceptBalanceChange', 'conceptFinalChange', 'conceptAbstractChange', 'conceptReferenceChange', 'conceptLabelAdd', 'conceptDefaultChange', 'conceptLabelDelete', 'conceptLabelChange', 'conceptIDChange', 'conceptReferenceDelete', 'conceptSubstitutionGroupChange', 'conceptFixedChange'}) and modelAction is not None:
                                    modelAction.events.append(modelObject)
                            else:
                                if ns == XbrlConst.verrels:
                                    if ln in frozenset({'relationshipSetModelDelete', 'relationshipSetModelAdd', 'relationshipSetModelChange'}):
                                        if modelAction is not None:
                                            modelAction.events.append(modelObject)
                                    ...
                                    if ln in ('aspectModelChange', 'aspectModelAdd',
                                              'aspectModelDelete'):
                                        if modelAction is not None:
                                            modelAction.events.append(modelObject)
                                        ...
                                        if ln in ('fromAspects', 'toAspects'):
                                            ...
                                            if ln in ('concepts', 'explicitDimension',
                                                      'typedDimension', 'segment',
                                                      'scenario', 'entityIdentifier',
                                                      'period', 'location', 'unit'):
    ...
    def diffConcepts(self):
        ...
        for fromConceptQname, fromConcept in self.fromDTS.qnameConcepts.items():
            if not fromConcept.isItem:
                if not fromConcept.isTuple:
                    continue
            ...
            if toConceptQname in self.toDTS.qnameConcepts:
                ...
                if fromConcept.isTuple:
                    if toConcept.isTuple:
                        fromType = fromConcept.type
                        toType = toConcept.type
                        if fromType is not None:
                            if toType is not None:
                                if not XbrlUtil.nodesCorrespond(self.fromDTS, fromType, toType, self.toDTS):
                ...
                for concept, attrs in ((fromConcept, fromCustAttrs), (toConcept, toCustAttrs)):
                    for attrName, attrValue in concept.items():
                        ...
                        if attrName not in ('abstract', 'block', 'default', 'final',
                                            'fixed', 'form', 'id', 'maxOccurs', 'minOccurs',
                                            'name', 'nillable', 'ref', 'substitutionGroup',
                                            'type') and attrQname.namespaceURI != XbrlConst.xbrli and attrQname.namespaceURI != XbrlConst.xbrldt:
                ...
                if not (linkRoleUri in otherRoleUris or linkRoleUri in roleChanges):
                    ...
                    if isFrom:
                        ...
                        for fromRoot in fromRoots:
                            ...
                            if toRootConcept is not None:
                                if toRootConcept not in toRoots:
                        ...
                        for toRoot in toRoots:
                            ...
                            if fromRootConcept is not None:
                                if fromRootConcept not in fromRoots:
    ...
    def diffRelationships(self, fromConcept, toConcept, fromRelationshipSet, toRelationshipSet):
        ...
        for i, fromRel in enumerate(fromRels):
            ...
            if toRel is not None and isinstance(toRel.toModelObject, ModelConcept) and toRel.toModelObject.qname == toTgtQname:
                ...
                toRelAttrs = XbrlUtil.attributes((self.modelXbrl), (toRel.arcElement), exclusions=relationshipSetArcAttributesExclusion,
                ...
                if not otherDTSpriItemDRSrels:
                    ...
                    for dimRel, isNotAll, isClosed in self.DRSdimRels(dts, priItemDRSrels):
                        ...
                        explDim = self.createInstanceAspectsEvent(('typedDimension' if dimConcept.isTypedDimension else 'explicitDimension'), ((
                         (
                          'name', dimConcept.qname),) + ((('excluded', 'true'), ) if isNotAll else ())),
                                  ...
                                  otherDTSpriItemQname, toDimRel, 'toAspects', False)):
                                    if dimRel is not None:
                                        ...
                                        if mbrDiffs:
                                            ...
                                            for domRel in self.DRSdomRels(dts, dimRel):
                                                domHasMemRels = dts.relationshipSet(XbrlConst.domainMember, linkrole).fromModelObject(priItemConcept)
                                                self.createInstanceAspectsEvent('member', ((('name', domRel.toModelObject.qname),) + ((('linkrole', domRel.linkrole), ('arcrole', XbrlConst.domainMember), ('axis', 'DRS-descendant-or-self')) if domHasMemRels else ())),
                                                  eventParent=explDim)
    ...
    def DRShcDiff(self, fromDTS, fromPriItemDRSrels, toDTS, toPriItemDRSrels):
        ...
        for i, fromHcRelKey in enumerate(fromHcRels.keys()):
            ...
            try:
                ...
                toRelAttrs = XbrlUtil.attributes((self.modelXbrl), (toHcRel.arcElement), exclusions=relationshipSetArcAttributesExclusion,
                  ns2ns1Tbl=(self.namespaceRenameToURI))
                if fromRelAttrs != toRelAttrs:
    ...
    def typedDomainIsDifferent(self, fromDimConcept, toDimConcept):
        try:
            ...
            isCorresponding = fromTypedDomain is not None and toTypedDomain is not None and XbrlUtil.sEqual((self.fromDTS), fromTypedDomain, toTypedDomain, excludeIDs=(XbrlUtil.ALL_IDs_EXCLUDED),
              dts2=(self.toDTS),
              ns2ns1Tbl=(self.namespaceRenameToURI))
        ...
        for i, fromDimKey in enumerate(fromDims.keys()):
            ...
            if toDimRel is not None:
                ...
                if fromDimConcept.isTypedDimension:
                    ...
                    dimsCorrespond = False
            elif mbrDiffs or not dimsCorrespond:
    ...
    def toDTSqname(self, fromDTSqname):
        if fromDTSqname is not None:
            if fromDTSqname.namespaceURI in self.namespaceRenameFromURI:
    ...
    def fromDTSqname(self, toDTSqname):
        if toDTSqname:
            if toDTSqname.namespaceURI in self.namespaceRenameToURI:
