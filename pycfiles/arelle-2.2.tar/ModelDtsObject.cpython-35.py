...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ModelDtsObject.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 86863 bytes
"""
    ...
    def instanceOfType(self, typeqname):
        ...
        if isinstance(typeqname, (tuple, list, set)):
            if self.typeQname in typeqname:
                return True
        elif self.typeQname == typeqname:
            return True
        type = self.type
        if type is not None and type.isDerivedFrom(typeqname):
            return True
        subs = self.substitutionGroup
        if subs is not None:
...
class ModelType(ModelNamableTerm):
    ...
    def isDomainItemType(self):
        ...
        if self.name == 'domainItemType' and ('/us-types/' in self.modelDocument.targetNamespace or self.modelDocument.targetNamespace.startswith(XbrlConst.dtrTypesStartsWith)):
            return True
        qnameDerivedFrom = self.qnameDerivedFrom
        if not isinstance(qnameDerivedFrom, ModelValue.QName) or qnameDerivedFrom.namespaceURI in (XbrlConst.xsd, XbrlConst.xbrli):
            return False
    ...
    def isMultiLanguage(self):
        ...
        return self.baseXbrliType in {'stringItemType', 'string', 'normalizedString', 'normalizedStringItemType'}
        ...
        if isinstance(qnamesDerivedFrom, (tuple, list)):
            if isinstance(typeqname, (tuple, list)):
                if any(t in qnamesDerivedFrom for t in typeqname):
                    return True
            elif typeqname in qnamesDerivedFrom:
                return True
        else:
            if isinstance(typeqname, (tuple, list)):
...
class ModelResource(ModelObject):
    ...
    def viewText(self, labelrole=None, lang=None):
        ...
        return ' '.join([XmlUtil.text(resourceElt) for resourceElt in self.iter() if isinstance(resourceElt, ModelObject) and not resourceElt.localName.startswith('URI')])
...
class ModelRelationship(ModelObject):
    ...
    def equivalenceKey(self):
        ...
        return (
         ...
         self.order,
         self.weight,
         self.preferredLabel) + XbrlUtil.attributes(self.modelXbrl, self.arcElement, exclusions=arcCustAttrsExclusions, keyByTag=True)
