...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ValidateUtr.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 12114 bytes
"""
    ...
    try:
        ...
        for unitElt in xmldoc.iter(tag='{http://www.xbrl.org/2009/utr}unit'):
            ...
            if unitDupKey in unitDupCheck:
                modelXbrl.error('arelleUtrLoader:entryDuplication', 'Unit Type Registry entry duplication: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s',
            ...
            if u.isSimple:
                if not u.itemType:
                    modelXbrl.error('arelleUtrLoader:simpleDefMissingField', 'Unit Type Registry simple unit definition missing item type: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s',
                      modelObject=modelXbrl,
                      id=(u.id),
                      unitId=(u.unitId),
                      nsUnit=(u.nsUnit),
                ...
                if u.numeratorItemType or u.denominatorItemType or u.nsNumeratorItemType or u.nsDenominatorItemType:
                    modelXbrl.error('arelleUtrLoader', 'Unit Type Registry simple unit definition may not have complex fields: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s',
                      modelObject=modelXbrl,
                      id=(u.id),
                      unitId=(u.unitId),
                      nsUnit=(u.nsUnit),
                    ...
                    if u.symbol:
                        modelXbrl.error('arelleUtrLoader:complexDefSymbol', 'Unit Type Registry complex unit definition may not have symbol: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s',
                          modelObject=modelXbrl,
                          id=(u.id),
                          unitId=(u.unitId),
                          nsUnit=(u.nsUnit),
                    ...
                    if not u.numeratorItemType or not u.denominatorItemType:
                        modelXbrl.error('arelleUtrLoader:complexDefMissingField', 'Unit Type Registry complex unit definition must have numerator and denominator fields: id %(id)s unit %(unitId)s nsUnit %(nsUnit)s status %(status)s',
                          modelObject=modelXbrl,
                          id=(u.id),
                          unitId=(u.unitId),
                          nsUnit=(u.nsUnit),
     ...
     etree.LxmlError) as err:
        modelManager.modelXbrl.error('arelleUtrLoader:error', 'Unit Type Registry Import error: %(error)s',
          modelObject=modelXbrl,
          error=err)
    ...
    def validateFacts(self):
        ...
        if modelXbrl.modelDocument.type in (ModelDocument.Type.INSTANCE, ModelDocument.Type.INLINEXBRL):
            ...
            for fact in utrInvalidFacts:
                modelXbrl.log((self.messageLevel), (self.messageCode),
            ...
            if len(multMeasures) == 1:
                ...
                for utrItemTypeEntry in self.utrItemTypeEntries.values():
                    for utrEntry in utrItemTypeEntry.values():
                        if utrEntry.unitId == m.localName:
                            if utrEntry.nsUnit == m.namespaceURI:
