...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelDtsObject.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 88913 bytes
"""
...
class ModelRoleType(ModelObject):
    ...
    def usedOns(self):
        ...
        try:
            ...
            self._usedOns = set(usedOn.xValue for usedOn in self.iterdescendants('{http://www.xbrl.org/2003/linkbase}usedOn') if isinstance(usedOn, ModelObject))
            return self._usedOns
    ...
    def __repr__(self):
        return '{0}[{1}, uri: {2}, definition: {3}, {4} line {5}])'.format('modelArcroleType' if self.isArcrole else 'modelRoleType', self.objectIndex, self.arcroleURI if self.isArcrole else self.roleURI, self.definition, self.modelDocument.basename, self.sourceline)
...
class ModelNamableTerm(ModelObject):
    ...
    def schemaNameQname(self, prefixedName, isQualifiedForm=True, prefixException=None):
        ...
        if prefixedName:
            ...
            if qn:
                if not qn.namespaceURI:
                    if self.modelDocument.noTargetNamespace:
                        if not isQualifiedForm:
...
class ParticlesList(list):
    __doc__ = 'List of particles which can provide string representation of contained particles'
    ...
    def __repr__(self):
        ...
        for particle in self:
            if isinstance(particle, ModelConcept):
                ...
                if isinstance(particle, ModelAny):
                    p = 'any'
                else:
                    p = '{0}({1})'.format(particle.localName, getattr(particle.dereference(), 'particles', ''))
            particlesList.append(p + ('' if particle.minOccurs == particle.maxOccurs == 1 else '{{{0}:{1}}}'.format(particle.minOccursStr, particle.maxOccursStr)))

        return ', '.join(particlesList)


class ModelParticle:
    __doc__ = 'Represents a particle (for multi-inheritance subclasses of particles)'
...
class ModelConcept(ModelNamableTerm, ModelParticle):
    ...
    def niceType(self):
        ...
        """
        if self.isHypercubeItem:
            return 'Table'
        else:
            if self.isDimensionItem:
                return 'Axis'
            if self.typeQname:
    ...
    def instanceOfType(self, typeqname):
        ...
        type = self.type
        if type is not None:
            if type.isDerivedFrom(typeqname):
                return True
        subs = self.substitutionGroup
        if subs is not None:
    ...
    def isQualifiedForm(self):
        ...
        if self.get('form') is not None:
            return self.get('form') == 'qualified'
        else:
            return getattr(self.modelDocument, 'isQualifiedElementFormDefault', False)
    ...
    def isPrimaryItem(self):
        ...
        try:
            ...
            self._isPrimaryItem = self.isItem and not (self.substitutesForQname(XbrlConst.qnXbrldtHypercubeItem) or self.substitutesForQname(XbrlConst.qnXbrldtDimensionItem))
            return self._isPrimaryItem

    @property
    def isDomainMember(self):
        """(bool) -- Same as isPrimaryItem (same definition in XDT)"""
        return self.isPrimaryItem
    ...
    def propertyView(self):
        ...
        _labels = tuple(('{} ({})'.format(os.path.basename(label.role), label.xmlLang), label.stringValue) for labelRel in self.modelXbrl.relationshipSet(XbrlConst.conceptLabel).fromModelObject(self) for label in (
         labelRel.toModelObject,))
        if _labels:
            _labelProperty = (
             'label', _labelDefault, sorted(_labels))
        else:
            _labelProperty = (
             'label', _labelDefault)
        _refT = tuple((self.modelXbrl.roleTypeDefinition(_ref.role, _lang), ' ',
         tuple((_refPart.localName, _refPart.stringValue.strip()) for _refPart in _ref.iterchildren())) for _refRel in sorted((self.modelXbrl.relationshipSet(XbrlConst.conceptReference).fromModelObject(self)), key=(lambda r: r.toModelObject.roleRefPartSortKey())) for _ref in (
         _refRel.toModelObject,))
        _refsStrung = ' '.join(_refPart.stringValue.strip() for _refRel in self.modelXbrl.relationshipSet(XbrlConst.conceptReference).fromModelObject(self) for _refPart in _refRel.toModelObject.iterchildren())
        _refProperty = ('references', _refsStrung, _refT) if _refT else ()
        _facets = ('facets', ', '.join(sorted(self.facets.keys())),
         tuple(((_name, sorted(_value.keys()),
          tuple((eVal, eElt.genLabel()) for eVal, eElt in sorted((_value.items()), key=(lambda i: i[0])))) if isinstance(_value, dict) else (_name, _value)) for _name, _value in sorted((self.facets.items()), key=(lambda i: i[0])))) if self.facets else ()
    ...
    def __repr__(self):
        return 'modelConcept[{0}, qname: {1}, type: {2}, abstract: {3}, {4}, line {5}]'.format(self.objectIndex, self.qname, self.typeQname, self.abstract, self.modelDocument.basename, self.sourceline)
...
class ModelType(ModelNamableTerm):
    ...
    def init(self, modelDocument):
        ...
        self.modelXbrl.qnameTypes.setdefault(self.qname, self)
    ...
    def qnameDerivedFrom(self):
        ...
        if isinstance(typeOrUnion, list):
        ...
        *XBRLI_DECIMALSUNION*, *XBRLI_PRECISIONUNION*, *XBRLI_NONZERODECIMAL*.
        ...
        try:
            ...
            if self.qname == XbrlConst.qnXbrliDateUnion:
                ...
                if self.qname == XbrlConst.qnXbrliDecimalsUnion:
                    self._baseXsdType = 'XBRLI_DECIMALSUNION'
                else:
                    if self.qname == XbrlConst.qnXbrliPrecisionUnion:
                        self._baseXsdType = 'XBRLI_PRECISIONUNION'
                    else:
                        if self.qname == XbrlConst.qnXbrliNonZeroDecimalUnion:
                            ...
                            if qnameDerivedFrom is None:
                                ...
                                if isinstance(qnameDerivedFrom, list):
                                    ...
                                    if qnameDerivedFrom.namespaceURI == XbrlConst.xsd:
                                        self._baseXsdType = qnameDerivedFrom.localName
                                    else:
                                        typeDerivedFrom = self.modelXbrl.qnameTypes.get(qnameDerivedFrom)
                                        self._baseXsdType = typeDerivedFrom.baseXsdType if typeDerivedFrom is not None else 'anyType'
                            if self._baseXsdType == 'anyType':
                                if XmlUtil.emptyContentModel(self):
                                    self._baseXsdType = 'noContent'
            return self._baseXsdType
        ...
        try:
            ...
            if self.qname == XbrlConst.qnXbrliDateUnion:
                ...
                if isinstance(qnameDerivedFrom, list):
                    ...
                    if isinstance(qnameDerivedFrom, ModelValue.QName):
                        if qnameDerivedFrom.namespaceURI == XbrlConst.xbrli:
                            self._baseXbrliTypeQname = qnameDerivedFrom
                        else:
                            if qnameDerivedFrom.namespaceURI == XbrlConst.xsd:
                                self._baseXbrliTypeQname = qnameDerivedFrom
    ...
    def isTextBlock(self):
        ...
        if self.name == 'textBlockItemType':
            if '/us-types/' in self.modelDocument.targetNamespace:
                return True
        if self.name == 'escapedItemType':
            if self.modelDocument.targetNamespace.startswith(XbrlConst.dtrTypesStartsWith):
                return True
        qnameDerivedFrom = self.qnameDerivedFrom
        if not isinstance(qnameDerivedFrom, ModelValue.QName) or qnameDerivedFrom.namespaceURI in (XbrlConst.xsd, XbrlConst.xbrli):
            return False
        else:
            typeDerivedFrom = self.modelXbrl.qnameTypes.get(qnameDerivedFrom)
    ...
    def isDomainItemType(self):
        ...
        if self.name == 'domainItemType':
            if '/us-types/' in self.modelDocument.targetNamespace or self.modelDocument.targetNamespace.startswith(XbrlConst.dtrTypesStartsWith):
                return True
        qnameDerivedFrom = self.qnameDerivedFrom
        if not isinstance(qnameDerivedFrom, ModelValue.QName) or qnameDerivedFrom.namespaceURI in (XbrlConst.xsd, XbrlConst.xbrli):
            return False
    ...
    def isMultiLanguage(self):
        ...
        return self.baseXbrliType in frozenset({'normalizedString', 'normalizedStringItemType', 'string', 'stringItemType'})
    ...
    def isDerivedFrom(self, typeqname):
        ...
        qnamesDerivedFrom = self.qnameDerivedFrom
        if qnamesDerivedFrom is None:
            return typeqname is None or not typeqname
        else:
            if isinstance(qnamesDerivedFrom, (tuple, list)):
                if isinstance(typeqname, (tuple, list)):
                    ...
                    if isinstance(typeqname, (tuple, list)):
                        if qnamesDerivedFrom in typeqname:
                            return True
                    elif qnamesDerivedFrom == typeqname:
                        return True
            else:
                qnamesDerivedFrom = (
                 qnamesDerivedFrom,)
            for qnameDerivedFrom in qnamesDerivedFrom:
                typeDerivedFrom = self.modelXbrl.qnameTypes.get(qnameDerivedFrom)
                if typeDerivedFrom is not None:
                    if typeDerivedFrom.isDerivedFrom(typeqname):
                        return True
    ...
    def constrainingFacets(self, facetValues=None):
        ...
        if facetValues is None:
            ...
            for facetElt in XmlUtil.schemaFacets(self, ('{http://www.w3.org/2001/XMLSchema}length',
                ...
                if facetName not in facetValues and facetValue is not None:
    ...
    def __repr__(self):
        return 'modelType[{0}, qname: {1}, derivedFrom: {2}, {3}, line {4}]'.format(self.objectIndex, self.qname, self.qnameDerivedFrom, self.modelDocument.basename, self.sourceline)
            ...
            if self._isAny:
                ...
                if namespaceURI == self.modelDocument.targetNamespace:
                    if '##targetNamespace' in self._namespaces:
                        return True
                elif '##other' in self._namespaces:
                    return True
            return False
        except AttributeError:
...
class ModelResource(ModelObject):
    ...
    def viewText(self, labelrole=None, lang=None):
        ...
        if self.qname == XbrlConst.qnLinkFootnote:
            return XmlUtil.innerText(self, ixEscape='html', strip=True)
        else:
            return ' '.join([XmlUtil.text(resourceElt) for resourceElt in self.iter() if isinstance(resourceElt, ModelObject) if not resourceElt.localName.startswith('URI')])

    def roleRefPartSortKey(self):
        return '{} {}'.format(self.role, ' '.join('{} {}'.format(_refPart.localName, _refPart.stringValue.strip()) for _refPart in self.iterchildren()))[:200]
...
class ModelLocator(ModelResource):
    ...
    def propertyView(self):
        ...
        if isinstance(hrefObj, (ModelFact, ModelConcept)):
            ...
            if isinstance(hrefObj, ModelResource):
                return (
                 (
                  'href', hrefObj.viewText()),)
...
arcCustAttrsExclusions = {
 XbrlConst.xlink, 'use', 'priority', 'order', 'weight', 'preferredLabel'}
...
class ModelRelationship(ModelObject):
    ...
    def fromLocator(self):
        ...
        for fromResource in self.arcElement.getparent().labeledResources[self.fromLabel]:
            if isinstance(fromResource, ModelLocator):
                if self.fromModelObject is fromResource.dereference():
                    return fromResource
    ...
    def toLocator(self):
        ...
        for toResource in self.arcElement.getparent().labeledResources[self.toLabel]:
            if isinstance(toResource, ModelLocator):
                if self.toModelObject is toResource.dereference():
                    return toResource

    def locatorOf(self, dereferencedObject):
        """(ModelLocator) -- Value of locator surrogate of relationship target, if any"""
        fromLocator = self.fromLocator
        if fromLocator is not None:
            if fromLocator.dereference() == dereferencedObject:
                return fromLocator
        toLocator = self.toLocator
        if toLocator is not None:
            if toLocator.dereference() == dereferencedObject:
                return toLocator
    ...
    def variableQname(self):
        ...
        if varName:
            return ModelValue.qname((self.arcElement), varName, noPrefixIsNoNamespace=True)
    ...
    def isClosed(self):
        ...
        try:
            ...
            self._isClosed = self.get('{http://xbrl.org/2005/xbrldt}closed') in ('true',
                                                                                 '1')
            return self._isClosed
    ...
    def axisDisposition(self):
        ...
        try:
            ...
            aType = self.get('axis') or self.get('axisDisposition') or self.get('axisType')
            if aType in ('xAxis', 'x'):
                self._axisDisposition = 'x'
            else:
                if aType in ('yAxis', 'y'):
                    self._axisDisposition = 'y'
                else:
                    if aType in ('zAxis', 'z'):
                        self._axisDisposition = 'z'
                    else:
                        self._axisDisposition = None
            return self._axisDisposition

    @property
    def equivalenceHash(self):
    ...
    def equivalenceKey(self):
        ...
        return (
         ...
         self.order,
         self.weight,
         self.preferredLabel) + XbrlUtil.attributes((self.modelXbrl), (self.arcElement), exclusions=arcCustAttrsExclusions,
          keyByTag=True)
    ...
    def isIdenticalTo(self, otherModelRelationship):
        ...
        return otherModelRelationship is not None and self.arcElement == otherModelRelationship.arcElement and self.fromModelObject is not None and otherModelRelationship.fromModelObject is not None and self.toModelObject is not None and otherModelRelationship.toModelObject is not None and self.fromModelObject == otherModelRelationship.fromModelObject and self.toModelObject == otherModelRelationship.toModelObject
    ...
    @property
    def propertyView(self):
        if isinstance(self.fromModelObject, ModelObject):
            return self.toModelObject.propertyView + (('arcrole', self.arcrole), ('weight', self.weight) if self.arcrole == XbrlConst.summationItem else (), ('preferredLabel', self.preferredLabel) if (self.arcrole == XbrlConst.parentChild and self.preferredLabel) else (), ('contextElement', self.contextElement) if self.arcrole in (XbrlConst.all, XbrlConst.notAll) else (), ('typedDomain', self.toModelObject.typedDomainElement.qname) if (self.arcrole == XbrlConst.hypercubeDimension and isinstance(self.toModelObject, ModelConcept) and self.toModelObject.isTypedDimension and self.toModelObject.typedDomainElement is not None) else (), ('closed', self.closed) if self.arcrole in (XbrlConst.all, XbrlConst.notAll) else (), ('usable', self.usable) if self.arcrole == XbrlConst.domainMember else (), ('targetRole', self.targetRole) if self.arcrole.startswith(XbrlConst.dimStartsWith) else (), ('order', self.order), ('priority', self.priority)) + (('from', self.fromModelObject.qname),)
        else:
            return ()

    def __repr__(self):
        return 'modelRelationship[{0}, linkrole: {1}, arcrole: {2}, from: {3}, to: {4}, {5}, line {6}]'.format(self.objectIndex, os.path.basename(self.linkrole), os.path.basename(self.arcrole), self.fromModelObject.qname if isinstance(self.fromModelObject, ModelObject) else '??', self.toModelObject.qname if isinstance(self.toModelObject, ModelObject) else '??', self.modelDocument.basename, self.sourceline)

    @property
    def viewConcept(self):
        if isinstance(self.toModelObject, ModelConcept):
            return self.toModelObject
        if isinstance(self.fromModelObject, ModelConcept):
...
elementSubstitutionModelClass.update((
 ...
 (
  XbrlConst.qnLinkLabelLink, ModelLink),
 (
