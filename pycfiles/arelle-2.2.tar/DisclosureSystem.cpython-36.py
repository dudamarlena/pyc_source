...
# [GCC 8.4.0]
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\DisclosureSystem.py
    ...
    def dirlist(self, listFormat):
        ...
        try:
         ...
         etree.LxmlError) as err:
            self.modelManager.cntlr.addToLog((_('Disclosure System listing, import error: %(error)s')), messageCode='arelle:disclosureSystemListingError',
            ...
            try:
                if name:
                    ...
                    if result:
                        ...
                        self.modelManager.cntlr.addToLog((_('Disclosure System "%(name)s" not recognized (a plug-in may be needed).')), messageCode='arelle:disclosureSystemName',
             ...
             etree.LxmlError) as err:
                ...
                result = False
                self.modelManager.cntlr.addToLog((_('Disclosure System "%(name)s" loading error: %(error)s')), messageCode='arelle:disclosureSystemLoadingError',
                  messageArgs={'error':str(err), 
                 'name':name},
                  level=(logging.ERROR))
    ...
    def loadStandardTaxonomiesDict(self):
        if self.selection:
            ...
            try:
             ...
             etree.LxmlError) as err:
                self.modelManager.cntlr.addToLog((_('Disclosure System "%(name)s" import %(importFile)s, error: %(error)s')), messageCode='arelle:disclosureSystemImportError',
                  messageArgs={'error':str(err), 
                 'name':self.name,  'importFile':basename},
                  level=(logging.ERROR))
                etree.clear_error_log()
        ...
        if self.standardTaxonomiesUrl:
            if namespaceUri in self.standardTaxonomiesDict:
                if href in self.standardTaxonomiesDict[namespaceUri]:
                    return False
            if namespaceUri in self.standardLocalHrefs:
                if not isHttpUrl(href):
