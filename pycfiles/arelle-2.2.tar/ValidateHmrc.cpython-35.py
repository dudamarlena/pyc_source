...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ValidateHmrc.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 12215 bytes
"""
...
from arelle import ModelObject, ModelDocument, ModelValue, ValidateXbrl, ModelRelationshipSet, XmlUtil, XbrlConst, UrlUtil, ValidateFilingDimensions, ValidateFilingDTS, ValidateFilingText
...
class ValidateHmrc(ValidateXbrl.ValidateXbrl):
    ...
    def validate(self, modelXbrl, parameters=None):
        if not hasattr(modelXbrl.modelDocument, 'xmlDocument'):
            return
        busNamespacePattern = re.compile('^http://www\\.xbrl\\.org/uk/cd/business')
        gaapNamespacePattern = re.compile('^http://www\\.xbrl\\.org/uk/gaap/core')
        ifrsNamespacePattern = re.compile('^http://www\\.iasb\\.org/.*ifrs')
        direpNamespacePattern = re.compile('^http://www\\.xbrl\\.org/uk/reports/direp')
        labelHasNegativeTermPattern = re.compile('.*[(].*\\w.*[)].*')
        ...
        isAccounts = XmlUtil.hasAncestor(modelXbrl.modelDocument.xmlRootElement, 'http://www.govtalk.gov.uk/taxation/CT/3', 'Accounts')
        isComputation = XmlUtil.hasAncestor(modelXbrl.modelDocument.xmlRootElement, 'http://www.govtalk.gov.uk/taxation/CT/3', 'Computation')
        if parameters:
            ...
            if modelXbrl.modelDocument.type == ModelDocument.Type.INSTANCE or modelXbrl.modelDocument.type == ModelDocument.Type.INLINEXBRL:
                ...
                busLocalNames = {'EntityCurrentLegalOrRegisteredName',
                ...
                def checkFacts(facts):
                    for f1 in facts:
                        ...
                        if ifrsNamespacePattern.match(factNamespaceURI) and factLocalName in ifrsLocalNames:
                        ...
                        dupKey = (f1.concept,
                         ...
                         f1.xmlLang)
                        if context is not None and f1 in uniqueFacts:
                            f2 = uniqueFacts[f1]
                            if f1.effectiveValue != f2.effectiveValue:
                                modelXbrl.error('HMRC.14', _('Inconsistent duplicate facts %(fact)s context %(contextID)s and %(contextID2)s.'), modelObject=(
                                 f1, f2), fact=f1.qname, contextID=f1.contextID, contextID2=f2.contextID)
                            uniqueFacts[dupKey] = f1
                            if f1.isNumeric:
                                if f1.precision:
                                    modelXbrl.error('HMRC.5.4', _("Numeric fact %(fact)s of context %(contextID)s has a precision attribute '%(precision)s'"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, precision=f1.precision)
                                try:
                                    if f1.xValue < 0:
                                        label = f1.concept.label(lang='en')
                                        if not labelHasNegativeTermPattern.match(label):
                                            modelXbrl.error('HMRC.5.3', _("Numeric fact %(fact)s of context %(contextID)s has a negative value '%(value)s' but label does not have a bracketed negative term (using parentheses): %(label)s"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, value=f1.value, label=label)
                ...
                if isAccounts:
                    if 'StartDateForPeriodCoveredByReport' not in busItems:
                        modelXbrl.error('HMRC.02', _('Period Start Date (uk-bus:StartDateForPeriodCoveredByReport) is missing.'), modelObject=modelXbrl)
        else:
            if busItems['StartDateForPeriodCoveredByReport'].value < '2008-04-06':
                modelXbrl.error('HMRC.02', _('Period Start Date (uk-bus:StartDateForPeriodCoveredByReport) must be 6 April 2008 or later.'), modelObject=modelXbrl)
             ...
             (
              ...
              '13')):
                if name not in items:
                    modelXbrl.error('HMRC.{0}'.format(ref), msg, modelObject=modelXbrl, messageCodes=('HMRC.01',
                                                                                                      'HMRC.03',
                                                                                                      'HMRC.06',
                                                                                                      'HMRC.09',
                                                                                                      'HMRC.10',
            ...
            if 'DateApprovalAccounts' not in gaapItems and 'DateAuthorisationFinancialStatementsForIssue' not in ifrsItems:
                modelXbrl.error('HMRC.07', _('Balance Sheet Date of Approval (uk-gaap:DateApprovalAccounts) is missing OR Balance Sheet Date of Approval (uk-ifrs:DateAuthorisationFinancialStatementsForIssue) is missing.'), modelObject=modelXbrl)
            if 'NameDirectorSigningAccounts' not in gaapItems and 'ExplanationOfBodyOfAuthorisation' not in ifrsItems:
                modelXbrl.error('HMRC.08', _('Name of Director Approving Balance Sheet (uk-gaap:NameDirectorSigningAccounts) is missing OR Name of Director Approving Balance Sheet (ifrs:ExplanationOfBodyOfAuthorisation) is missing.'), modelObject=modelXbrl)
            if 'ProfitLossForPeriod' not in gaapItems and 'ProfitLoss' not in ifrsItems:
                modelXbrl.error('HMRC.11', _('Profit or Loss for the period (uk-gaap:ProfitLossForPeriod OR ifrs:ProfitLoss) is missing.'), modelObject=modelXbrl)
            if companyReferenceNumberContexts:
                if 'UKCompaniesHouseRegisteredNumber' not in busItems:
                    modelXbrl.error('HMRC.16.1', _('Company Reference Number (uk-bus:UKCompaniesHouseRegisteredNumber) is missing.'), modelObject=modelXbrl)
            else:
                ...
                for compRefNbr, contextIds in companyReferenceNumberContexts.items():
                    if compRefNbr != factCompNbr:
                        modelXbrl.error('HMRC.16.2', _('Context entity identifier (%(entityIdentifier)s) does not match Company Reference Number (uk-bus:UKCompaniesHouseRegisteredNumber) Location: Accounts (context id %(contextID)s).'), modelObject=modelXbrl, entityIdentifier=compRefNbr, contextID=','.join(contextIds))
