...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/Validate.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 42147 bytes
"""
...
class Validate:
    ...
    def validate(self):
        if self.modelXbrl.modelDocument is None:
            self.modelXbrl.info('arelle:notValdated', _('Validation skipped, document not successfully loaded: %(file)s'), modelXbrl=self.modelXbrl, file=self.modelXbrl.modelDocument.basename)
            ...
            try:
                ...
                if _disclosureSystem.name:
                    self.modelXbrl.info('info', _('Disclosure system %(disclosureSystemName)s, version %(disclosureSystemVersion)s'), modelXbrl=self.modelXbrl, disclosureSystemName=_disclosureSystem.name, disclosureSystemVersion=_disclosureSystem.version)
                ...
                self.modelXbrl.error('exception:' + type(err).__name__, _('Testcase validation exception: %(error)s, testcase: %(testcase)s'), modelXbrl=self.modelXbrl, testcase=self.modelXbrl.modelDocument.basename, error=err, exc_info=True)
        ...
        if self.modelXbrl.modelDocument.type == Type.VERSIONINGREPORT:
            try:
                ...
                self.modelXbrl.error('exception:' + type(err).__name__, _('Versioning report exception: %(error)s, testcase: %(reportFile)s'), modelXbrl=self.modelXbrl, reportFile=self.modelXbrl.modelDocument.basename, error=err, exc_info=True)
            ...
            if self.modelXbrl.modelDocument.type == Type.RSSFEED:
                ...
                try:
                    ...
                    self.modelXbrl.error('exception:' + type(err).__name__, _('Instance validation exception: %(error)s, instance: %(instance)s'), modelXbrl=self.modelXbrl, instance=self.modelXbrl.modelDocument.basename, error=err, exc_info=True)
    ...
    def validateRssFeed(self):
        ...
        for rssItem in self.modelXbrl.modelDocument.rssItems:
            if getattr(rssItem, 'skipRssItem', False):
                self.modelXbrl.info('info', _('skipping RSS Item %(accessionNumber)s %(formType)s %(companyName)s %(period)s'), modelObject=rssItem, accessionNumber=rssItem.accessionNumber, formType=rssItem.formType, companyName=rssItem.companyName, period=rssItem.period)
                continue
                self.modelXbrl.info('info', _('RSS Item %(accessionNumber)s %(formType)s %(companyName)s %(period)s'), modelObject=rssItem, accessionNumber=rssItem.accessionNumber, formType=rssItem.formType, companyName=rssItem.companyName, period=rssItem.period)
                modelXbrl = None
                try:
                    modelXbrl = ModelXbrl.load(self.modelXbrl.modelManager, openFileSource(rssItem.zippedUrl, self.modelXbrl.modelManager.cntlr, reloadCache=reloadCache), _('validating'), rssItem=rssItem)
                    ...
                    self.modelXbrl.error('exception:' + type(err).__name__, _('RSS item validation exception: %(error)s, instance: %(instance)s'), modelXbrl=(
                     self.modelXbrl, modelXbrl), instance=rssItem.zippedUrl, error=err, exc_info=True)
                    try:
                        self.instValidator.close()
    ...
    def validateTestcase(self, testcase):
        ...
        if testcase.type == Type.TESTCASESINDEX:
            ...
            for modelTestcaseVariation in testcase.testcaseVariations:
                ...
                self.modelXbrl.info('info', _('Variation %(id)s %(name)s: %(expected)s - %(description)s'), modelObject=modelTestcaseVariation, id=modelTestcaseVariation.id, name=modelTestcaseVariation.name, expected=modelTestcaseVariation.expected, description=modelTestcaseVariation.description)
                ...
                for readMeFirstUri in modelTestcaseVariation.readMeFirstUris:
                    if isinstance(readMeFirstUri, tuple):
                        ...
                        if resultIsVersioningReport:
                            ...
                            dtsName = None
                    if resultIsVersioningReport and dtsName:
                        if dtsName in inputDTSes:
                            dtsName = inputDTSes[dtsName]
                        else:
                            modelXbrl = ModelXbrl.create(self.modelXbrl.modelManager, Type.DTSENTRIES, self.modelXbrl.modelManager.cntlr.webCache.normalizeUrl(readMeFirstUri[:-4] + '.dts', baseForElement), isEntry=True, errorCaptureLevel=errorCaptureLevel)
                        ...
                        if resultIsTaxonomyPackage:
                            ...
                            if self.useFileSource.isArchive:
                                modelXbrl = ModelXbrl.load(self.modelXbrl.modelManager, readMeFirstUri, _('validating'), base=baseForElement, useFileSource=self.useFileSource, errorCaptureLevel=errorCaptureLevel)
                            else:
                                ...
                                modelXbrl = ModelXbrl.load(self.modelXbrl.modelManager, filesource, _('validating'), base=baseForElement, errorCaptureLevel=errorCaptureLevel)
                            modelXbrl.isTestcaseVariation = True
                    if modelXbrl.modelDocument is None:
                        modelXbrl.error('arelle:notLoaded', _('Variation %(id)s %(name)s readMeFirst document not loaded: %(file)s'), modelXbrl=testcase, id=modelTestcaseVariation.id, name=modelTestcaseVariation.name, file=os.path.basename(readMeFirstUri))
                        ...
                        try:
                            ...
                            modelXbrl.error('exception:' + type(err).__name__, _('Testcase variation validation exception: %(error)s, instance: %(instance)s'), modelXbrl=modelXbrl, instance=modelXbrl.modelDocument.basename, error=err, exc_info=True)
                ...
                if resultIsVersioningReport and modelXbrl.modelDocument:
                    ...
                    if os.path.exists(versReportFile):
                        ...
                        if len(inputDTSes) == 2:
                            ...
                            modelXbrl.error('arelle:notLoaded', _('Variation %(id)s %(name)s input DTSes not loaded, unable to generate versioning report: %(file)s'), modelXbrl=testcase, id=modelTestcaseVariation.id, name=modelTestcaseVariation.name, file=os.path.basename(readMeFirstUri))
                    ...
                    if resultIsTaxonomyPackage:
                        ...
                        if expectedDataFiles - foundDataFiles:
                            modelXbrl.error('arelle:testcaseDataNotUsed', _('Variation %(id)s %(name)s data files not used: %(missingDataFiles)s'), modelObject=modelTestcaseVariation, name=modelTestcaseVariation.name, id=modelTestcaseVariation.id, missingDataFiles=', '.join(sorted(os.path.basename(f) for f in expectedDataFiles - foundDataFiles)))
                        if foundDataFiles - expectedDataFiles:
                            modelXbrl.warning('arelle:testcaseDataUnexpected', _('Variation %(id)s %(name)s files not in variation data: %(unexpectedDataFiles)s'), modelObject=modelTestcaseVariation, name=modelTestcaseVariation.name, id=modelTestcaseVariation.id, unexpectedDataFiles=', '.join(sorted(os.path.basename(f) for f in foundDataFiles - expectedDataFiles)))
                if modelXbrl.hasTableRendering or modelTestcaseVariation.resultIsTable:
                    try:
                        RenderingEvaluator.init(modelXbrl)
                    except Exception as err:
                        modelXbrl.error('exception:' + type(err).__name__, _('Testcase RenderingEvaluator.init exception: %(error)s, instance: %(instance)s'), modelXbrl=modelXbrl, instance=modelXbrl.modelDocument.basename, error=err, exc_info=True)
                    ...
                    if modelXbrlHasFormulae:
                        try:
                            ...
                            modelXbrl.error('exception:' + type(err).__name__, _('Testcase formula variation validation exception: %(error)s, instance: %(instance)s'), modelXbrl=modelXbrl, instance=modelXbrl.modelDocument.basename, error=err, exc_info=True)
                        ...
                        if modelTestcaseVariation.resultIsInfoset and self.modelXbrl.modelManager.validateInfoset:
                            ...
                            infoset = ModelXbrl.load(self.modelXbrl.modelManager, modelTestcaseVariation.resultInfosetUri, _('loading result infoset'), base=baseForElement, useFileSource=self.useFileSource, errorCaptureLevel=errorCaptureLevel)
                            if infoset.modelDocument is None:
                                modelXbrl.error('arelle:notLoaded', _('Variation %(id)s %(name)s result infoset not loaded: %(file)s'), modelXbrl=testcase, id=modelTestcaseVariation.id, name=modelTestcaseVariation.name, file=os.path.basename(modelTestcaseVariation.resultXbrlInstance))
                        ...
                        if modelXbrl.hasTableRendering or modelTestcaseVariation.resultIsTable:
                            ...
                            if not any(alternativeValidation(modelXbrl, resultTableUri) for alternativeValidation in pluginClassMethods('Validate.TableInfoset')):
                                try:
                                    ...
                                    modelXbrl.error('exception:' + type(err).__name__, _('Testcase table linkbase validation exception: %(error)s, instance: %(instance)s'), modelXbrl=modelXbrl, instance=modelXbrl.modelDocument.basename, error=err, exc_info=True)
                                ...
                                if resultIsXbrlInstance and formulaOutputInstance and formulaOutputInstance.modelDocument:
                                    ...
                                    if expectedInstance.modelDocument is None:
                                        ...
                                        if len(expectedInstance.facts) != len(formulaOutputInstance.facts):
                                            formulaOutputInstance.error('{}:resultFactCounts'.format(errMsgPrefix), _('Formula output %(countFacts)s facts, expected %(expectedFacts)s facts'), modelXbrl=modelXbrl, countFacts=len(formulaOutputInstance.facts), expectedFacts=len(expectedInstance.facts), messageCodes=('formula:resultFactCounts',
                                                                                                                                                                                                                                                                                                                           'ix:resultFactCounts'))
                                        else:
                                            ...
                                            for expectedInstanceFact in expectedInstance.facts:
                                                ...
                                                if formulaOutputFact is None:
                                                    ...
                                                    formulaOutputInstance.error('{}:expectedFactMissing'.format(errMsgPrefix), _('Output missing expected fact %(fact)s'), modelXbrl=missingFact, fact=missingFact.qname, messageCodes=('formula:expectedFactMissing',
                                                    ...
                                                    if len(expectedInstanceFactFootnotes) != len(formulaOutputFactFootnotes) or set(expectedInstanceFactFootnotes.values()) != set(formulaOutputFactFootnotes.values()):
                                                        formulaOutputInstance.error('{}:expectedFactFootnoteDifference'.format(errMsgPrefix), _('Output expected fact %(fact)s expected footnotes %(footnotes1)s produced footnotes %(footnotes2)s'), modelXbrl=(
                                                         formulaOutputFact, expectedInstanceFact), fact=expectedInstanceFact.qname, footnotes1=sorted(expectedInstanceFactFootnotes.items()), footnotes2=sorted(formulaOutputFactFootnotes.items()), messageCodes=('formula:expectedFactFootnoteDifference',
                ...
                if expected is None and numErrors == 0:
                    ...
                    if isinstance(expected, (QName, _STR_BASE, dict)):
                        ...
                        for testErr in _errors:
                            if isinstance(expected, QName) and isinstance(testErr, _STR_BASE):
                                ...
                                if not sep and errLocalName == expected.localName or expected == qname(XbrlConst.errMsgPrefixNS.get(errPrefix) or errPrefix == expected.prefix and expected.namespaceURI, errLocalName) or expected.namespaceURI == XbrlConst.xdtSchemaErrorNS and errPrefix == 'xmlSchema':
                                    _passCount += 1
                            elif type(testErr) == type(expected) and (testErr == expected or isinstance(expected, _STR_BASE) and (expected == 'EFM.6.03.04' and testErr.startswith('xmlSchema:') or expected == 'EFM.6.03.05' and (testErr.startswith('xmlSchema:') or testErr == 'EFM.5.02.01.01') or expected == 'EFM.6.04.03' and (testErr.startswith('xmlSchema:') or testErr.startswith('utr:') or testErr.startswith('xbrl.') or testErr.startswith('xlink:')) or expected == 'EFM.6.05.35' and testErr.startswith('utre:') or expected.startswith('EFM.') and testErr.startswith(expected) or expected == 'vere:invalidDTSIdentifier' and testErr.startswith('xbrl'))):
                                _passCount += 1
                        ...
                        if _passCount > 0:
                            ...
                            status = 'pass'
                        if status == 'fail' and isinstance(expected, _STR_BASE) and ' ' in expected and all(any(testErr == e for testErr in _errors) for e in expected.split()):
                            status = 'pass'
                        if not _errors and status == 'fail':
                            if modelTestcaseVariation.assertions:
                                if modelTestcaseVariation.assertions == expected:
                                    status = 'pass'
                        elif isinstance(expected, dict) and all(countSatisfied == 0 and countNotSatisfied == 0 for countSatisfied, countNotSatisfied in expected.values()):
                            status = 'pass'
                    else:
                        status = 'fail'
