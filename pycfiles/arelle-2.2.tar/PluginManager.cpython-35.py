...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/PluginManager.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 24958 bytes
"""
...
def moduleModuleInfo(moduleURL, reload=False, parentImportsSubtree=False):
    ...
    if moduleFilename:
        ...
        try:
                        ...
                        if item.level == 1:
                            if item.module is None:
                                for importee in item.names:
                                    ...
                                    elif os.path.isfile(os.path.join(moduleDir, importee.name + '.py')) and importee.name not in moduleImports:
                                        moduleImports.append(importee.name)

                            else:
                                modulePkgs = item.module.split('.')
                                modulePath = os.path.join(*modulePkgs)
                                if os.path.isfile(os.path.join(moduleDir, modulePath) + '.py') and modulePath not in moduleImports:
            ...
            _msg = _('Exception obtaining plug-in module info: {error}\n{traceback}').format(error=err, traceback=traceback.format_tb(sys.exc_info()[2]))
            if PLUGIN_TRACE_FILE:
                with open(PLUGIN_TRACE_FILE, 'at', encoding='utf-8') as (fh):
                    fh.write(_msg + '\n')
            else:
                    ...
                    try:
                        ...
                        if elementSubstitutionClasses:
                            try:
                                ...
                                _msg = _('Exception loading plug-in {name}: processing ModelObjectFactory.ElementSubstitutionClasses').format(name=name, error=err)
                                if PLUGIN_TRACE_FILE:
                                    with open(PLUGIN_TRACE_FILE, 'at', encoding='utf-8') as (fh):
                                        fh.write(_msg + '\n')
                                else:
                        ...
                        _msg = _('Exception loading plug-in {name}: {error}\n{traceback}').format(name=name, error=err, traceback=traceback.format_tb(sys.exc_info()[2]))
                        if PLUGIN_TRACE_FILE:
                            with open(PLUGIN_TRACE_FILE, 'at', encoding='utf-8') as (fh):
                                fh.write(_msg + '\n')
                        else:
