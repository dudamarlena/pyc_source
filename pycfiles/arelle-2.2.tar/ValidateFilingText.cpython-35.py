...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ValidateFilingText.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 43977 bytes
"""
            ...
            for match in docCheckPattern.finditer(line):
                ...
                if text.startswith('&'):
                    if text not in xhtmlEntities:
                        modelXbrl.error(('EFM.5.02.02.06', 'GFM.1.01.02'), _('Disallowed entity code %(text)s in file %(file)s line %(line)s column %(column)s'), modelDocument=filepath, text=text, file=os.path.basename(filepath), line=lineNum, column=match.start())
                elif isEFM and not _isTestcase:
                    if len(text) == 1:
                        modelXbrl.error('EFM.5.02.01.01', _("Disallowed character '%(text)s' (%(unicodeIndex)s) in file %(file)s at line %(line)s col %(column)s"), modelDocument=filepath, text=text, unicodeIndex='U+{:04X}'.format(ord(text)), file=os.path.basename(filepath), line=lineNum, column=match.start())
                    else:
                        modelXbrl.error('EFM.5.02.01.01', _("Disallowed character '%(text)s' in file %(file)s at line %(line)s col %(column)s"), modelDocument=filepath, text=text, file=os.path.basename(filepath), line=lineNum, column=match.start())
        ...
        if f1.xsiNil != 'true' and concept is not None and concept.isTextBlock and XMLpattern.match(f1.value):
            for match in namedEntityPattern.finditer(f1.value):
                ...
                if entity not in xhtmlEntities:
                    modelXbrl.error(('EFM.6.05.16', 'GFM.1.2.15'), _('Fact %(fact)s contextID %(contextID)s has disallowed entity %(entity)s'), modelObject=f1, fact=f1.qname, contextID=f1.contextID, entity=entity, error=entity)
                ...
                try:
                    ...
                    if not edbodyDTD.validate(textblockXml):
                        ...
                        modelXbrl.error('EFM.6.05.16' if htmlError else ('EFM.6.05.15.dtdError',
                                                                         'GFM.1.02.14'), _('Fact %(fact)s contextID %(contextID)s has text which causes the XML error %(error)s'), modelObject=f1, fact=f1.qname, contextID=f1.contextID, error=', '.join(e.message for e in errors), messageCodes=('EFM.6.05.16',
                    ...
                    for elt in textblockXml.iter():
                        ...
                        if isinstance(elt, ModelObject) and elt.namespaceURI == xhtml:
                            ...
                            if isInlineDTD and eltTag in efmBlockedInlineHtmlElements:
                                modelXbrl.error('EFM.5.02.05.disallowedElement', _('%(validatedObjectLabel)s has disallowed element <%(element)s>'), modelObject=elt, validatedObjectLabel=f1.qname, element=eltTag)
                        for attrTag, attrValue in elt.items():
                            if isInlineDTD and attrTag in efmBlockedInlineHtmlElementAttributes.get(eltTag, ()):
                                modelXbrl.error('EFM.5.02.05.disallowedAttribute', _('%(validatedObjectLabel)s has disallowed attribute on element <%(element)s>: %(attribute)s="%(value)s"'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, element=eltTag, attribute=attrTag, value=attrValue)
                            if attrTag == 'href' and eltTag == 'a' or attrTag == 'src' and eltTag == 'img':
                                if 'javascript:' in attrValue:
                                    modelXbrl.error('EFM.6.05.16.activeContent', _("Fact %(fact)s of context %(contextID)s has javascript in '%(attribute)s' for <%(element)s>"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrTag, element=eltTag)
                            if attrValue.startswith('http://www.sec.gov/Archives/edgar/data/') and eltTag == 'a':
                                pass
                            else:
                                if scheme(attrValue) in ('http', 'https', 'ftp'):
                                    modelXbrl.error('EFM.6.05.16.externalReference', _("Fact %(fact)s of context %(contextID)s has an invalid external reference in '%(attribute)s' for <%(element)s>"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrTag, element=eltTag)
                                if attrTag == 'src' and attrValue not in checkedGraphicsFiles:
                                    if scheme(attrValue) == 'data':
                                        modelXbrl.error('EFM.6.05.16.graphicDataUrl', _("Fact %(fact)s of context %(contextID)s references a graphics data URL which isn't accepted '%(attribute)s' for <%(element)s>"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrValue[:32], element=eltTag)
                                    else:
                                        if attrValue.lower()[-4:] not in ('.jpg', '.gif'):
                                            modelXbrl.error('EFM.6.05.16.graphicFileType', _("Fact %(fact)s of context %(contextID)s references a graphics file which isn't .gif or .jpg '%(attribute)s' for <%(element)s>"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrValue, element=eltTag)
                                        else:
                                            try:
                                                if validateGraphicFile(f1, attrValue) != attrValue.lower()[-3:]:
                                                    modelXbrl.error('EFM.6.05.16.graphicFileContent', _("Fact %(fact)s of context %(contextID)s references a graphics file which doesn't have expected content '%(attribute)s' for <%(element)s>"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrValue, element=eltTag)
                                            except IOError as err:
                                                modelXbrl.error('EFM.6.05.16.graphicFileError', _("Fact %(fact)s of context %(contextID)s references a graphics file which isn't openable '%(attribute)s' for <%(element)s>, error: %(error)s"), modelObject=f1, fact=f1.qname, contextID=f1.contextID, attribute=attrValue, element=eltTag, error=err)

                                            checkedGraphicsFiles.add(attrValue)

                        if eltTag == 'table' and any(a is not None for a in elt.iterancestors('table')):
                            modelXbrl.error('EFM.6.05.16.nestedTable', _('Fact %(fact)s of context %(contextID)s has nested <table> elements.'), modelObject=f1, fact=f1.qname, contextID=f1.contextID)

                except (XMLSyntaxError,
                 UnicodeDecodeError) as err:
                    modelXbrl.error(('EFM.6.05.15', 'GFM.1.02.14'), _('Fact %(fact)s contextID %(contextID)s has text which causes the XML error %(error)s'), modelObject=f1, fact=f1.qname, contextID=f1.contextID, error=err)
...
def validateFootnote(modelXbrl, footnote):
    ...
    try:
        ...
        if not edbodyDTD.validate(footnoteHtml):
            modelXbrl.error('EFM.6.05.34.dtdError', _('%(validatedObjectLabel)s causes the XML error %(error)s'), modelObject=footnote, validatedObjectLabel=validatedObjectLabel, error=', '.join(e.message for e in edbodyDTD.error_log.filter_from_errors()))
        ...
        modelXbrl.error('EFM.6.05.34', _('%(validatedObjectLabel)s causes the XML error %(error)s'), modelObject=footnote, validatedObjectLabel=validatedObjectLabel, error=edbodyDTD.error_log.filter_from_errors())
    ...
    for elt in htmlEltTree.iter():
        ...
        if isInline:
            if eltTag in efmBlockedInlineHtmlElements:
                modelXbrl.error('EFM.5.02.05.disallowedElement', _('%(validatedObjectLabel)s has disallowed element <%(element)s>'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, element=eltTag)
            if eltTag == 'a' and 'href' not in elt.keys() and any(a.tag not in _anchorAncestorTags for a in elt.iterancestors()):
                modelXbrl.warning('EFM.5.02.05.anchorElementPosition', _('If element <a> does not have attribute @href, it should not have any ancestors other than html, body, or div.  Disallowed ancestors: %(disallowedAncestors)s'), modelObject=elt, disallowedAncestors=', '.join(a.tag.rpartition('}')[2] for a in elt.iterancestors() if a.tag not in _anchorAncestorTags))
            for attrTag, attrValue in elt.items():
                if isInline:
                    if attrTag in efmBlockedInlineHtmlElementAttributes.get(eltTag, ()):
                        modelXbrl.error('EFM.5.02.05.disallowedAttribute', _('%(validatedObjectLabel)s has disallowed attribute on element <%(element)s>: %(attribute)s="%(value)s"'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, element=eltTag, attribute=attrTag, value=attrValue)
                    elif attrTag == '{http://www.w3.org/XML/1998/namespace}base':
                        modelXbrl.error('EFM.5.02.05.xmlBaseDisallowed', _('%(validatedObjectLabel)s has disallowed attribute on element <%(element)s>: xml:base="%(value)s"'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, element=eltTag, value=attrValue)
                elif attrTag == '{http://www.w3.org/2001/XMLSchema-instance}schemaLocation':
                    modelXbrl.warning('EFM.5.02.05.schemaLocationDisallowed', _('%(validatedObjectLabel)s has disallowed attribute on element <%(element)s>: xsi:schemaLocation="%(value)s"'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, element=eltTag, value=attrValue)
                if attrTag == 'href' and eltTag == 'a' or attrTag == 'src' and eltTag == 'img':
                    if 'javascript:' in attrValue:
                        modelXbrl.error(messageCodePrefix + 'activeContent', _("%(validatedObjectLabel)s has javascript in '%(attribute)s' for <%(element)s>"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrTag, element=eltTag, messageCodes=('EFM.6.05.34.activeContent',
                                                                                                                                                                                                                                                                             'EFM.5.02.05.activeContent'))
                if attrValue.startswith('http://www.sec.gov/Archives/edgar/data/') and eltTag == 'a':
                    pass
                else:
                    if scheme(attrValue) in ('http', 'https', 'ftp'):
                        modelXbrl.error(messageCodePrefix + 'externalReference', _("%(validatedObjectLabel)s has an invalid external reference in '%(attribute)s' for <%(element)s>: %(value)s"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrTag, element=eltTag, value=attrValue, messageCodes=('EFM.6.05.34.externalReference',
                                                                                                                                                                                                                                                                                                                                'EFM.5.02.05.externalReference'))
                    if attrTag == 'src' and attrValue not in checkedGraphicsFiles:
                        if scheme(attrValue) == 'data':
                            modelXbrl.error(messageCodePrefix + 'graphicDataUrl', _("%(validatedObjectLabel)s references a graphics data URL which isn't accepted '%(attribute)s' for <%(element)s>"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrValue[:32], element=eltTag)
                        else:
                            if attrValue.lower()[-4:] not in ('.jpg', '.gif'):
                                modelXbrl.error(messageCodePrefix + 'graphicFileType', _("%(validatedObjectLabel)s references a graphics file which isn't .gif or .jpg '%(attribute)s' for <%(element)s>"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrValue, element=eltTag, messageCodes=('EFM.6.05.34.graphicFileType',
                                                                                                                                                                                                                                                                                                                           'EFM.5.02.05.graphicFileType'))
                            else:
                                try:
                                    if validateGraphicFile(referenceElt, attrValue) != attrValue.lower()[-3:]:
                                        modelXbrl.error(messageCodePrefix + 'graphicFileContent', _("%(validatedObjectLabel)s references a graphics file which doesn't have expected content '%(attribute)s' for <%(element)s>"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrValue, element=eltTag, messageCodes=('EFM.6.05.34.graphicFileContent',
                                                                                                                                                                                                                                                                                                                                                 'EFM.5.02.05.graphicFileContent'))
                                except IOError as err:
                                    modelXbrl.error(messageCodePrefix + 'graphicFileError', _("%(validatedObjectLabel)s references a graphics file which isn't openable '%(attribute)s' for <%(element)s>, error: %(error)s"), modelObject=elt, validatedObjectLabel=validatedObjectLabel, attribute=attrValue, element=eltTag, error=err, messageCodes=('EFM.6.05.34.graphicFileError',
                                                                                                                                                                                                                                                                                                                                                         'EFM.5.02.05.graphicFileError'))

                                checkedGraphicsFiles.add(attrValue)
                            if eltTag == 'meta' and attrTag == 'content' and not attrValue.startswith('text/html'):
                                modelXbrl.error(messageCodePrefix + 'disallowedMetaContent', _('%(validatedObjectLabel)s <meta> content is "%(metaContent)s" but must be "text/html"'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, metaContent=attrValue, messageCodes=('EFM.6.05.34.disallowedMetaContent',
                                                                                                                                                                                                                                                                                         'EFM.5.02.05.disallowedMetaContent'))

            if eltTag == 'table' and any(a.tag in _tableTags for a in elt.iterancestors()):
                modelXbrl.error(messageCodePrefix + 'nestedTable', _('%(validatedObjectLabel)s has nested <table> elements.'), modelObject=elt, validatedObjectLabel=validatedObjectLabel, messageCodes=('EFM.6.05.34.nestedTable',
...
def referencedFiles(modelXbrl, localFilesOnly=True):
    ...
    def addReferencedFile(docElt, elt):
        if elt.tag in ('a', 'img', '{http://www.w3.org/1999/xhtml}a', '{http://www.w3.org/1999/xhtml}img'):
            for attrTag, attrValue in elt.items():
                if attrTag in ('href', 'src') and scheme(attrValue) not in ('data',
                                                                            'javascript') and (not localFilesOnly or not isHttpUrl(attrValue) and not os.path.isabs(attrValue)):
