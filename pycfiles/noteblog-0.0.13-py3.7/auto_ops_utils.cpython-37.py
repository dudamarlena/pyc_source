...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/fzutils/auto_ops_utils.py
...
def judge_whether_file_exists(connect_object: Connection, file_path):
    ...
    """
    result = False
    _ = connect_object.run(command=('[ -e "{0}" ] && echo 1 || echo 0'.format(file_path)),
      hide=True)
...
def local_compress_folders(father_folders_path, folders_name, default_save_path='/Users/afa/myFiles/tmp', exclude_folders=None):
    ...
    if not exists(father_folders_path):
        ...
        if not exists(default_save_path):
            ...
            cmd = 'cd {0} && zip -r {1}.zip {2} {3} && mv {4}.zip {5}'.format(father_folders_path, folders_name, './' + folders_name + '/*', exclude_str, folders_name, default_save_path)
            try:
                system(cmd)
...
def remote_decompress_folders(connect_object: Connection, folders_path, target_decompress_path):
    ...
    if not exists(folders_path):
        ...
        _ = False
        cmd = 'cd {0} && rm -rf {1} && unzip -o -O CP936 {2} && rm {3}'.format(target_decompress_path, basename(folders_path).split('.')[0], folders_path, folders_path)
        try:
...
def auto_git(path):
    ...
    now_time = str(re.compile('\\..*').sub('', now_time))
