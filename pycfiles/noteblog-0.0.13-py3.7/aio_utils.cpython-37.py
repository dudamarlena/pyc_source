...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/fzutils/aio_utils.py
# Compiled at: 2019-05-05 05:26:25
# Size of source mod 2**32: 9447 bytes
"""
aio 异步utils
"""
import re
...
from spider.fz_requests import Requests, PROXY_TYPE_HTTP, PROXY_TYPE_HTTPS
from .internet_utils import get_base_headers
from spider.fz_driver import BaseDriver, PC, PHANTOMJS
from spider.fz_phantomjs import PHANTOMJS_DRIVER_PATH
__all__ = [
 'Asyncer',
 'get_async_execute_result',
 'async_wait_tasks_finished',
 'TasksParamsListObj',
 'unblock_request',
 'unblock_get_driver_obj',
 'unblock_request_by_driver']
...
def get_async_execute_result(obj=Asyncer, obj_method_name='get_async_requests_body', **kwargs):
    """
    获取异步执行结果
    :param obj: 对象的类
    ...
    result = loop.run_until_complete(future=method_callback(**kwargs))
    return result
...
async def unblock_request(url, use_proxy=True, headers: dict=get_base_headers(), params=None, data=None, cookies=None, had_referer=False, encoding='utf-8', method='get', timeout=12, num_retries=1, high_conceal=True, ip_pool_type=ip_proxy_pool, verify=None, _session=None, get_session=False, proxies=None, proxy_type=PROXY_TYPE_HTTP, logger=None) -> str:
    """
    非阻塞的request请求
    :param url:
    :param use_proxy:
...
async def unblock_get_driver_obj(type=PHANTOMJS, load_images=False, executable_path=PHANTOMJS_DRIVER_PATH, logger=None, high_conceal=True, headless=False, driver_use_proxy=True, user_agent_type=PC, driver_obj=None, ip_pool_type=ip_proxy_pool, extension_path=None, driver_cookies=None):
    """
    异步获取一个driver obj
    :return:
    """

    async def _get_init_args():
        """获取args"""
        return [
         type,
         load_images,
...
async def unblock_request_by_driver(url, type=PHANTOMJS, load_images=False, executable_path=PHANTOMJS_DRIVER_PATH, logger=None, high_conceal=True, headless=False, driver_use_proxy=True, user_agent_type=PC, driver_obj=None, ip_pool_type=ip_proxy_pool, extension_path=None, driver_cookies=None, css_selector='', exec_code='', timeout=20) -> str:
    """
    非阻塞的driver 的请求
    :return:
    """

    async def _get_request_args():
        return [
         url,
         css_selector,
         exec_code,
    ...
    try:
        try:
            driver = await unblock_get_driver_obj(type=type,
