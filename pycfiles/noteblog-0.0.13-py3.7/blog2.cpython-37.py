...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/blog2.py
...
import re, mimetypes, os.path, hashlib
...
def to_html(path, target):
    ...
    if not f:
        raise Exception('do not know how to make html from %s' % path)
...
class MetaWeblog:
    __doc__ = 'works with www.cnblogs.com atleast'

    def __init__(self, serviceUrl, appKey, usr, passwd):
        self.serviceUrl, self.appKey, self.usr, self.passwd = (
         serviceUrl, appKey, usr, passwd)
