...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/fzutils/spider/bloom_utils.py
# Compiled at: 2019-05-05 05:26:25
# Size of source mod 2**32: 14477 bytes
"""
@author = super_fazai
@File    : bloom_utils.py
...
class BloomFilter(object):
    ...
    def __init__(self, capacity, error_rate=0.0001):
        """Implements a space-efficient probabilistic data structure
        capacity
            容量个数, 同时保持不超过*错误率*错误概率
        error_rate
            过滤器返回误报的错误率。这个确定筛选器容量。插入超过容量元素大大增加了误报的机会。
    ...
    def tofile(self, f):
        ...
        f.write(self.bitarray.tobytes()) if is_string_io(f) else self.bitarray.tofile(f)
    ...
    def fromfile(cls, f, n=-1):
        ...
        if 0 < n < headerlen:
            ...
            if n > 0:
                filter.bitarray.frombytes(f.read(n - headerlen)) if is_string_io(f) else filter.bitarray.fromfile(f, n - headerlen)
            else:
                filter.bitarray.frombytes(f.read()) if is_string_io(f) else filter.bitarray.fromfile(f)
        if filter.num_bits != filter.bitarray.length():
            if filter.num_bits + (8 - filter.num_bits % 8) != filter.bitarray.length():
