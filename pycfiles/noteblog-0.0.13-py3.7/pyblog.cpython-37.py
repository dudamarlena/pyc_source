...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/pyblog.py
# Compiled at: 2019-04-29 11:26:56
# Size of source mod 2**32: 25180 bytes
...
class MetaWeblog(Blog):
    ...
    def get_recent_posts(self, num_posts=10, blog_id=None):
        ...
        Args:
            blog_id (int): Blog ID
            num_posts (int): Number of posts to be returned [optional]
        """
        if blog_id is None:
        ...
        if isinstance(new_object, str):
            try:
                if os.path.exists(new_object):
                    media_bits = Binary(open(new_object).read())
                    if name is not None:
                        media_name = name
                    else:
                        media_name = os.path.basename(new_object)
                    new_object = {'bits':media_bits, 
                     'name':media_name}
            except ValueError:
                pass

        else:
            if type(getattr(new_object, 'read', None)).__name__ == 'function':
    ...
    def get_template(self, template_type, blog_id=None):
        ...
        if blog_id is None:
            if self.default_blog_id is None:
                raise BlogError('No blog_id passed')
            blog_id = self.default_blog_id
        return self.execute('metaWeblog.getTemplate', self.app_key, blog_id, self.username, self.password, template_type)
    ...
    def set_template(self, template, template_type, blog_id=None):
        ...
        if blog_id is None:
            if self.default_blog_id is None:
                raise BlogError('No blog_id passed')
            blog_id = self.default_blog_id
        return self.execute('metaWeblog.setTemplate', self.app_key, blog_id, self.username, self.password, template, template_type)
...
class MovableType(MetaWeblog):
    ...
    def _parse_custom_fields(self, content):
        if not isinstance(content, dict):
            raise BlogError("Invalid type for field 'content': excepted dict, got %s" % type(content)._MovableType__name_)
        custom_fields = []
        ...
        if len(custom_fields) > 0:
            if 'mt_text_more' not in content:
                ...
                if not isinstance(content['mt_text_more'], str):
                    content['mt_text_more'] = ''
            content['mt_text_more'] += '\n' + '\n'.join(custom_fields)
        return content
