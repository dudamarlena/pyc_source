...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/fzutils/spider/fz_requests.py
...
from ..ip_pools import MyIpPools, ip_proxy_pool, fz_ip_pool, sesame_ip_pool, tri_ip_pool
...
class MyRequests(object):
    ...
    @classmethod
    def get_url_body(cls, url, use_proxy=True, headers: dict=get_base_headers(), params=None, data=None, cookies=None, had_referer=False, encoding='utf-8', method='get', timeout=12, num_retries=1, high_conceal=True, ip_pool_type=ip_proxy_pool, verify=None, _session=None, get_session=False, proxies=None, proxy_type=PROXY_TYPE_HTTP):
        """
        根据url得到body
        :param url:
        ...
        def _get_one_proxies_obj():
            ...
            if proxies is None:
                if use_proxy:
                    tmp_proxies = cls._get_proxies(ip_pool_type=ip_pool_type,
                      high_conceal=high_conceal,
                      proxy_type=proxy_type)
                    if not tmp_proxies != {}:
                        raise AssertionError('获取代理失败, 此处跳过!')
            else:
        ...
        tmp_headers = headers
        tmp_headers['Host'] = re.compile('://(.*?)/').findall(url)[0]
        if had_referer:
            if re.compile('https').findall(url) != []:
        ...
        with requests.session() if _session is None else _session as (s):
            try:
                ...
                try:
                    if num_retries > 1:
                        return cls.get_url_body(url=url,
                          use_proxy=use_proxy,
                          headers=tmp_headers,
                          params=params,
                          data=data,
    ...
    def _get_proxies(cls, ip_pool_type=ip_proxy_pool, high_conceal=True, proxy_type=PROXY_TYPE_HTTP):
        ...
        proxies = ip_obj.get_proxy_ip_from_ip_pool()
        ...
        try:
            ...
            if proxy_type == PROXY_TYPE_HTTP:
                return {'http': proxy}
            if proxy_type == PROXY_TYPE_HTTPS:
                return {'https': proxy}
            raise ValueError('未知的proxy_type值!请检查!')

    @classmethod
    def _download_file(cls, url, file_save_path, headers=None, params=None, cookies=None, use_proxy=True, ip_pool_type=ip_proxy_pool, high_conceal=True) -> bool:
        """
        下载文件
        :param url:
