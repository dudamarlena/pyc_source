...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/noteblog/fzutils/free_api_utils.py
# Compiled at: 2019-05-05 05:26:25
# Size of source mod 2**32: 16942 bytes
"""
@author = super_fazai
...
__all__ = [
 ...
 'get_gd_input_prompt_info',
 'get_gd_reverse_geocode_info',
 'get_gd_map_shop_info_list_by_lng_and_lat_and_keyword',
 'get_gd_map_shop_info_list_by_gd_id']
...
def get_bd_map_shop_info_list_by_keyword_and_area_name(ak: str, keyword: str, area_name: str, page_num: int, page_size: int=20, use_proxy=True, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, logger=None) -> list:
    """
...
def get_gd_map_shop_info_list_by_keyword_and_area_name(gd_key: str, keyword: str, area_name: str, page_num: int, page_size: int=20, use_proxy=True, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, children=0, extensions='all', poi_type='', logger=None) -> list:
    """
    ...
    body = Requests.get_url_body(use_proxy=use_proxy,
      url=url,
      headers=headers,
      params=params,
      ip_pool_type=ip_pool_type,
...
def get_gd_input_prompt_info(gd_key: str, keyword, city_name: str, poi_type='', lng: float=0.0, lat: float=0.0, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, use_proxy=True, logger=None) -> list:
    """
    ...
    headers.update({'Connection':'keep-alive', 
     ...
     'accept':'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3', 
     'accept-language':'zh-CN,zh;q=0.9,en;q=0.8'})
    location = ','.join([str(lng), str(lat)]) if (lng != 0.0 or lat != 0.0) else ''
    params = (
     (
      'key', str(gd_key)),
     (
...
def get_gd_reverse_geocode_info(gd_key: str, address: str, city_name: str, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, use_proxy=True, logger=None) -> list:
    """
...
def get_gd_map_shop_info_list_by_lng_and_lat_and_keyword(gd_key: str, lng: float, lat: float, keyword: str='', radius: int=1000, page_num: int=1, page_size: int=20, poi_type='', extensions='all', use_proxy=True, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, logger=None) -> list:
    """
...
def get_gd_map_shop_info_list_by_gd_id(gd_key: str, gd_id: str, use_proxy=True, ip_pool_type=tri_ip_pool, num_retries=6, timeout=20, logger=None) -> list:
    """
