...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/icsemantic/core/content/multilingual.py
...
from icsemantic.core.interfaces import IContentTypesMultilingualPatcher, IMultilingualContentMarker, IMultilingualGettersMarker, IFieldEmptiness
NOT_FALLBACK_FIELDS = [
 'id', 'language']
FALLBACK_TYPES = [
 'string', 'text', 'lines']
...
class ContentTypesMultilingualPatcher(object):
    ...
    def patch(self, klass, replace_accessors=False):
        ...
        def getMultilingualField(self, field_name, fallback=True):
            ...
            if not IMultilingualContentMarker.providedBy(self) or field.isLanguageIndependent(self) or not fallback or field_name in NOT_FALLBACK_FIELDS:
                return value
            translations = self.getTranslations()
            current_lang = self.getLanguage()
            if fallback and current_lang and len(translations) > 1:
                try:
                    icsemantic_lang_util = queryUtility(IUserPreferredLanguages, name='icsemantic_preferred_languages')
        ...
        for field in fields:
            name = field.getName()
            if field.type in FALLBACK_TYPES and name not in NOT_FALLBACK_FIELDS and not field.isLanguageIndependent(field):
                ...
                if replace_accessors:
                    ...
                    if hasattr(klass, getName) and hasattr(klass, getMultilingualName):
...
def makeMethod(klass, field):
    ...
    method = function(method.func_code, method.func_globals, methodName, method.func_defaults, method.func_closure)
