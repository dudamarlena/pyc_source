...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/event_bus_utils/packet_dumper.py
# Compiled at: 2018-06-28 11:55:51
# Size of source mod 2**32: 12497 bytes
...
def launch_amqp_data_to_pcap_dumper(dump_dir, log_level, filename, dlt, amqp_url, amqp_exchange, topics):
    ...
    pcap_dumper = AmqpDataPacketDumper(dump_dir=dump_dir,
      log_level=log_level,
      filename=filename,
      dlt=dlt,
      topics=topics,
...
class AmqpDataPacketDumper:
    ...
    def dumper_init(self):
        ...
        self.pcap_dumper = Dumper(filename=(os.path.join(self.dump_dir, self.pcap_filename_wr)),
          snaplen=2000,
          network=(self.dlt))
        shutil.copyfile(os.path.join(self.dump_dir, self.pcap_filename_wr), os.path.join(self.dump_dir, self.pcap_filename))

    def __init__(self, dump_dir=DEFAULT_DUMP_DIR, log_level=DEFAULT_LOG_LEVEL, filename=DEFAULT_FILENAME, dlt=DLT_RAW, topics=DEFAULT_TOPICS, amqp_url=None, amqp_exchange=None):
        if not dlt in ALLOWED_DATA_LINK_TYPES:
            raise AssertionError('not accepted dlt %s' % dlt)
        else:
            self.COMPONENT_ID = 'capture_dumper_%s' % str(uuid.uuid4())[:8]
            self.dlt = dlt
        ...
        for t in self.topics:
            self.channel.queue_bind(exchange=(self.exchange), queue=(self.data_queue_name),
              routing_key=t)

        self.channel.queue_bind(exchange=(self.exchange), queue=(self.data_queue_name),
          routing_key=(MsgTestingToolTerminate.routing_key))
        msg = MsgTestingToolComponentReady(component=(self.COMPONENT_ID),
          description=('%s READY to start test suite.' % self.COMPONENT_ID))
        self.channel.basic_publish(body=(msg.to_json()),
    ...
    def dump_packet(self, message):
        try:
            ...
            if 'serial' in message.interface_name:
                self.logger.info('Dumping packet found in %s interface of %s bytes' % (message.interface_name, len(message.data)))
                raw_packet = bytes(message.data)
                packet_slip = bytes(message.data_slip)
                pcap_packet_header = Pkthdr(ts_sec=t_s,
                  ts_usec=t_u_delta,
                ...
                if 'tun' in message.interface_name:
                    self.logger.info('Dumping packet found in %s interface of %s bytes' % (message.interface_name, len(message.data)))
                    raw_packet = bytes(message.data)
                    pcap_packet_header = Pkthdr(ts_sec=t_s,
                      ts_usec=t_u_delta,
    ...
    def dumps_rotate(self):
        ...
        if os.path.isfile(full_path):
            ...
            self.logger.info(full_path)
            self.logger.info(os.path.join(self.dump_dir, datetime.now().strftime('%Y%m%d_%H%M%S_') + self.pcap_filename))
            self.logger.info(self.pcap_filename)
            shutil.copyfile(full_path, os.path.join(self.dump_dir, datetime.now().strftime('%Y%m%d_%H%M%S_') + self.pcap_filename))
        ...
        try:
            ...
            if isinstance(m, MsgTestingToolTerminate):
                ...
                if isinstance(m, MsgSniffingStop):
                    ...
                    self.dump_packet(m)
                    try:
                        if self.messages_dumped != 0:
                            if self.messages_dumped % self.QUANTITY_MESSAGES_PER_PCAP == 0:
                                self.dumps_rotate()
                                self.dumper_init()
                    except Exception as e:
                        self.logger.error(e)
