...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/ioppytest_cli/ioppytest_cli.py
# Compiled at: 2018-11-08 11:00:35
# Size of source mod 2**32: 47252 bytes
import os
from os.path import expanduser
import sys, pika, errno, base64, logging, datetime, threading, traceback
...
session_profile = OrderedDict({'user_name':'Walter White', 
 'protocol':'coap', 
 'node':'both', 
 'amqp_url':'amqp://{0}:{1}@{2}/{3}'.format('guest', 'guest', 'localhost', '/'), 
 'amqp_exchange':'amq.topic'})
state_lock = threading.RLock()
state = {'testcase_id':None, 
 'step_id':None, 
 'last_message':None, 
 'suggested_cmd':None, 
 'connection':None, 
 'channel':None}
profile_choices = {'protocol':[
  'coap', '6lowpan', 'onem2m'], 
 'node':[
  'coap_client', 'coap_server']}
UI_suggested_actions = {MsgTestingToolConfigured: 'ts_start', 
...
def repl():
    ...
    prompt_kwargs = {'history': FileHistory(history_path_file)}
    ...
    _echo_session_helper('\nNote that <action suggested> will help you navigate through the session by executing the actions the backend normally expects for a standard session flow :)\n')
...
@cli.command()
@click.option('-ll', '--lazy-listener', is_flag=True,
  default=False,
...
def _handle_testcase_select():
    ...
    msg = MsgTestCaseSelect(testcase_id=(ls[(resp - 1)]['testcase_id']))
    _publish_message(msg)
...
def _handle_action_stimuli():
    ...
    if isinstance(state['last_message'], MsgStepStimuliExecute):
        ...
        if resp:
            msg = MsgStepStimuliExecuted(node=(session_profile['node']),
...
def _handle_action_verify():
    ...
    msg = MsgStepVerifyExecuted(response_type='bool',
...
def action(api_call):
    ...
    if api_call == 'suggested':
        ...
        if api_call in message_handles_options:
...
ignorable_message_types = {'dissections':[
  MsgDissectionAutoDissect], 
 'packets':[
  MsgPacketSniffedRaw, MsgPacketInjectRaw], 
 'ui':[
...
def enter_debug_context():
    ...
    """
    dummy_messages_dict = {k:v for k, v in globals().items() if 'Msg' in k if 'Msg' in k}
    ...
    def _results_store_insert_result():
        tc_results_example = [
         ...
         {'verdict':'None', 
          'description':'Testcase TD_COAP_CORE_04 was skipped.',  'testcase_id':'TD_COAP_CORE_04', 
          'partial_verdicts':[]}]
        m = MsgInsertResultRequest(data=tc_results_example)
        _publish_message(m)
    ...
    def _agent_serial_message_inject():
        ...
        m = MsgPacketInjectRaw(timestamp=1488586183.45,
    ...
    def _execute_stimuli_step(step_id, target_address, node):
        ...
        msg = MsgStepStimuliExecute(node=node,
          step_id=step_id,
    ...
    def _execute_verify_step(step_id, node):
        ...
        """
        _echo_session_helper('Executing test message function %s' % sys._getframe().f_code.co_name)
        msg = MsgStepVerifyExecute(node=node,
          step_id=step_id,
          description='',
          step_type='verify',
          step_info=[
    ...
    def _sniffer_start(testcase_id):
        ...
        _echo_session_helper('Executing test message function %s' % sys._getframe().f_code.co_name)
        msg = MsgSniffingStart(capture_id=testcase_id, filter_if='tun0',
          filter_proto='udp')
        _publish_message(msg)
    ...
    def _configure_perf_tt():
        ...
        message = MsgSessionConfiguration(**PERF_TT_CONFIGURATION)
    ...
    def _configure_comi_tt():
        ...
        message = MsgSessionConfiguration(**COMI_TT_CONFIGURATION)
        _publish_message(message)
    ...
    def _configure_6lowpan_tt():
        ...
        message = MsgSessionConfiguration(**SIXLOWPAN_TT_CONFIGURATION)
        _publish_message(message)
    ...
    def _configure_coap_tt():
        ...
        message = MsgSessionConfiguration(**COAP_TT_CONFIGURATION)
        _publish_message(message)
    ...
    def _testcase_skip(testcase_id):
        ...
        """
        _echo_session_helper('Executing test message function %s' % sys._getframe().f_code.co_name)
        msg = MsgTestCaseSkip(testcase_id=testcase_id)
        _publish_message(msg)
...
def _test_automated_iut_reaches_another_other_implementation(target_host, origin_node):
    ...
    """
    _echo_session_helper('Executing test message function %s' % sys._getframe().f_code.co_name)
    msg = MsgAutomatedIutTestPing(node=origin_node,
...
def chat(message):
    ...
    c = MsgSessionChat(description=m, user_name=(session_profile['user_name']),
...
def _set_up_connection(create_listener=True, lazy_listener=False):
    ...
    if 'amqp_listener_thread' in state and state['amqp_listener_thread'] is not None:
        ...
        if create_listener is False:
            return
        if lazy_listener:
            amqp_listener_thread = AmqpListener(amqp_url=(session_profile['amqp_url']),
...
def _pre_configuration():
    for key, _ in session_profile.items():
        if key in profile_choices.keys():
            ...
            selection_type = str
        value = click.prompt(('Please type %s ' % key), type=selection_type,
...
def _message_handler(msg):
    ...
    if isinstance(msg, Message):
        ...
        if type(msg) in UI_suggested_actions:
            ...
            _echo_session_helper('Suggested following action to execute: <action %s> or or <action suggested>' % state['suggested_cmd'])
    else:
        if isinstance(msg, MsgTestCaseVerdict):
            json_file = os.path.join(TEMP_DIR, msg.testcase_id + '_verdict.json')
            with open(json_file, 'w') as (f):
                f.write(msg.to_json())
        else:
            if isinstance(msg, (MsgStepStimuliExecute, MsgStepVerifyExecute)):
...
def _exit():
    _quit_callback()
    if 'amqp_listener_thread' in state:
        if state['amqp_listener_thread'] is not None:
...
def _echo_dispatcher(msg):
    ...
    if type(msg) is str:
        ...
        if isinstance(msg, MsgSessionLog):
            ...
            if isinstance(msg, MsgPacketSniffedRaw):
                _echo_data_message(msg)
            else:
                if isinstance(msg, MsgSessionChat):
                    _echo_chat_message(msg)
                else:
                    if isinstance(msg, MsgSessionConfiguration):
                        msg.configuration = ['...ommited fields...']
                        _echo_backend_message(msg)
                    else:
                        if isinstance(msg, dict):
                            click.echo(click.style((repr(msg)), fg=COLOR_DEFAULT))
                        else:
                            if isinstance(msg, (MsgUiDisplay, MsgUiDisplayMarkdownText, MsgUiRequestConfirmationButton)):
...
def _echo_backend_message(msg):
    if not isinstance(msg, Message):
        ...
        if isinstance(msg, MsgTestCaseReady):
            ...
            if isinstance(msg, MsgDissectionAutoDissect):
            ...
            if isinstance(msg, MsgTestCaseVerdict):
                ...
                return
            if isinstance(msg, MsgTestSuiteReport):
                _echo_report_as_table(msg.tc_results)
                return
    if isinstance(msg, MsgTestingToolComponentReady):
        pass
    else:
        if isinstance(msg, MsgTestingToolComponentShutdown):
            pass
...
def _echo_chat_message(msg: MsgSessionChat):
    if msg.iut_node == session_profile['node']:
        click.echo(click.style(('[Chat message sent] %s' % list_to_str(msg.description)), fg=COLOR_CHAT_MESSAGE_ECHO))
    else:
        click.echo(click.style(('[Chat message from %s] %s' % (msg.user_name, list_to_str(msg.description))), fg=COLOR_CHAT_MESSAGE))


def _echo_data_message(msg):
    assert isinstance(msg, (MsgPacketInjectRaw, MsgPacketSniffedRaw))
    click.echo(click.style(('[agent] Packet captured on %s. Routing key: %s' % (msg.interface_name, msg.routing_key)),
      fg=COLOR_SESSION_LOG))


def _echo_gui_message(msg):
    click.echo(click.style(('[UI message]\n\tMessage: %s \n\ttags: %s\n\tFields: %s \n\tR_key: %s \n\tcorr_id: %s' % (
     repr(msg)[:70],
     str(msg.tags),
     str(msg.fields)[:70],
     msg.routing_key,
...
def _amqp_request(request_message, component_id=COMPONENT_ID, timeout=10):
    ...
    try:
        ...
        while retries_left > 0:
            ...
            if method:
                ...
                if hasattr(props, 'correlation_id'):
                    if props.correlation_id == request_message.correlation_id:
                        break
            retries_left -= 1
        ...
        if retries_left > 0:
            ...
            raise Exception('Response timeout! rkey: %s , request type: %s' % (
    ...
    for i in range(1, 4):
        try:
            try:
                ...
                channel.basic_publish(exchange=(session_profile['amqp_exchange']),
