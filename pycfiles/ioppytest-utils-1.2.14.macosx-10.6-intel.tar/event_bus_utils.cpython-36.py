...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/event_bus_utils/event_bus_utils.py
# Compiled at: 2018-07-09 09:00:24
# Size of source mod 2**32: 11692 bytes
import os, pika, logging, threading, traceback
try:
...
class AmqpListener(threading.Thread):
    ...
    def amqp_connect(self):
        ...
        if not self.queue_is_declared:
            self.channel.queue_declare(queue=(self.services_queue_name), auto_delete=True,
              arguments={'x-max-length': 200})
            for t in self.topics:
                self.channel.queue_bind(exchange=(self.exchange), queue=(self.services_queue_name),
                  routing_key=t)

        m = MsgTestingToolComponentReady(component=(self.COMPONENT_ID),
          description=('%s is READY' % self.COMPONENT_ID))
        self.channel.basic_publish(body=(m.to_json()),
...
def amqp_request(connection, request_message, component_id, retries=10, time_between_retries=0.5, use_message_typing=False):
    ...
    elif not request_message.correlation_id:
        raise AssertionError
    else:
        if not retries > 0:
            raise AssertionError
            ...
            if retries_left > 0:
                ...
                channel.close()
                raise AmqpSynchCallTimeoutError('Response timeout! rkey: %s , request type: %s' % (
