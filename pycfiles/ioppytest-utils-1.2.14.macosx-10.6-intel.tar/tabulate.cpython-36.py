...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/tabulate.py
# Compiled at: 2017-09-12 14:13:03
# Size of source mod 2**32: 57381 bytes
...
def _moin_row_with_attrs(celltag, cell_values, colwidths, colaligns, header=''):
    alignment = {'left':'', 
     ...
     'center':'<style="text-align: center;">', 
     'decimal':'<style="text-align: right;">'}
    values_with_attrs = ['{0}{1} {2} '.format(celltag, alignment.get(a, ''), header + c + header) for c, a in zip(cell_values, colaligns)]
    return ''.join(values_with_attrs) + '||'


def _latex_line_begin_tabular(colwidths, colaligns, booktabs=False):
    alignment = {'left':'l', 
     'right':'r',  'center':'c',  'decimal':'r'}
...
def _rst_escape_first_column(rows, headers):
    ...
    def escape_empty(val):
        if isinstance(val, (_text_type, _binary_type)):
            if val.strip() is '':
                return '..'
        return val
...
_table_formats = {'simple':TableFormat(lineabove=Line('', '-', '  ', ''), linebelowheader=Line('', '-', '  ', ''),
   ...
   with_header_hide=[
  'lineabove', 'linebelow']), 
 'plain':TableFormat(lineabove=None, linebelowheader=None, linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('', '  ', ''),
   datarow=DataRow('', '  ', ''),
   padding=0,
   with_header_hide=None), 
 'grid':TableFormat(lineabove=Line('+', '-', '+', '+'), linebelowheader=Line('+', '=', '+', '+'),
   ...
   padding=1,
   with_header_hide=None), 
 'fancy_grid':TableFormat(lineabove=Line('â•’', 'â•\x90', 'â•¤', 'â••'), linebelowheader=Line('â•ž', 'â•\x90', 'â•ª', 'â•¡'),
   linebetweenrows=Line('â”œ', 'â”€', 'â”¼', 'â”¤'),
   linebelow=Line('â•˜', 'â•\x90', 'â•§', 'â•›'),
   headerrow=DataRow('â”‚', 'â”‚', 'â”‚'),
   datarow=DataRow('â”‚', 'â”‚', 'â”‚'),
   padding=1,
   with_header_hide=None), 
 'pipe':TableFormat(lineabove=_pipe_line_with_colons, linebelowheader=_pipe_line_with_colons,
   linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('|', '|', '|'),
   datarow=DataRow('|', '|', '|'),
   ...
   with_header_hide=[
  'lineabove']), 
 'orgtbl':TableFormat(lineabove=None, linebelowheader=Line('|', '-', '+', '|'),
   linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('|', '|', '|'),
   datarow=DataRow('|', '|', '|'),
   padding=1,
   with_header_hide=None), 
 'jira':TableFormat(lineabove=None, linebelowheader=None,
   linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('||', '||', '||'),
   datarow=DataRow('|', '|', '|'),
   padding=1,
   with_header_hide=None), 
 'presto':TableFormat(lineabove=None, linebelowheader=Line('', '-', '+', ''),
   linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('', '|', ''),
   datarow=DataRow('', '|', ''),
   padding=1,
   with_header_hide=None), 
 'psql':TableFormat(lineabove=Line('+', '-', '+', '+'), linebelowheader=Line('|', '-', '+', '|'),
   linebetweenrows=None,
   linebelow=Line('+', '-', '+', '+'),
   headerrow=DataRow('|', '|', '|'),
   datarow=DataRow('|', '|', '|'),
   padding=1,
   with_header_hide=None), 
 'rst':TableFormat(lineabove=Line('', '=', '  ', ''), linebelowheader=Line('', '=', '  ', ''),
   linebetweenrows=None,
   linebelow=Line('', '=', '  ', ''),
   headerrow=DataRow('', '  ', ''),
   datarow=DataRow('', '  ', ''),
   padding=0,
   with_header_hide=None), 
 'mediawiki':TableFormat(lineabove=Line('{| class="wikitable" style="text-align: left;"', '', '', '\n|+ <!-- caption -->\n|-'),
   ...
   padding=0,
   with_header_hide=None), 
 'moinmoin':TableFormat(lineabove=None, linebelowheader=None,
   linebetweenrows=None,
   linebelow=None,
   ...
   padding=1,
   with_header_hide=None), 
 'youtrack':TableFormat(lineabove=None, linebelowheader=None,
   linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('|| ', ' || ', ' || '),
   datarow=DataRow('| ', ' | ', ' |'),
   padding=1,
   with_header_hide=None), 
 'html':TableFormat(lineabove=_html_begin_table_without_header, linebelowheader='',
   linebetweenrows=None,
 ...
 'latex':TableFormat(lineabove=_latex_line_begin_tabular, linebelowheader=Line('\\hline', '', '', ''),
   linebetweenrows=None,
   ...
   padding=1,
   with_header_hide=None), 
 'latex_raw':TableFormat(lineabove=_latex_line_begin_tabular, linebelowheader=Line('\\hline', '', '', ''),
   linebetweenrows=None,
   linebelow=Line('\\hline\n\\end{tabular}', '', '', ''),
   ...
   padding=1,
   with_header_hide=None), 
 'latex_booktabs':TableFormat(lineabove=partial(_latex_line_begin_tabular, booktabs=True), linebelowheader=Line('\\midrule', '', '', ''),
   linebetweenrows=None,
   ...
   padding=1,
   with_header_hide=None), 
 'tsv':TableFormat(lineabove=None, linebelowheader=None, linebetweenrows=None,
   linebelow=None,
   headerrow=DataRow('', '\t', ''),
   datarow=DataRow('', '\t', ''),
   padding=0,
   with_header_hide=None), 
 'textile':TableFormat(lineabove=None, linebelowheader=None, linebetweenrows=None,
   linebelow=None,
...
multiline_formats = {'fancy_grid':'fancy_grid', 
 'grid':'grid', 
 'simple':'simple_multiline'}
_multiline_codes = re.compile('\\r|\\n|\\r\\n')
_multiline_codes_bytes = re.compile(b'\r|\n|\r\n')
_invisible_codes = re.compile('\\x1b\\[\\d+[;\\d]*m|\\x1b\\[\\d*\\;\\d*\\;\\d*m')
...
def simple_separated_format(separator):
    ...
    """
    return TableFormat(None, None, None, None, headerrow=(DataRow('', separator, '')),
...
def _isint(string, inttype=int):
    ...
    False
    """
    return type(string) is inttype or (isinstance(string, _binary_type) or isinstance(string, _text_type)) and _isconvertible(inttype, string)
...
def _isbool(string):
    ...
    False
    """
    return type(string) is _bool_type or isinstance(string, (_binary_type, _text_type)) and string in ('True',
                                                                                                       'False')
...
def _format(val, valtype, floatfmt, missingval='', has_invisible=True):
    ...
    if val is None:
        return missingval
    elif valtype in [int, _text_type]:
        return '{0}'.format(val)
    else:
        if valtype is _binary_type:
            try:
    ...
    * pandas.DataFrame (usually used with headers="keys")
    ...
    if headers == 'keys':
        if not rows:
            headers = []
    if headers == 'keys':
        if hasattr(tabular_data, 'dtype'):
            if getattr(tabular_data.dtype, 'names'):
    ...
    if isinstance(showindex, Iterable) and not showindex_is_a_str:
        rows = _prepend_row_index(rows, list(showindex))
    else:
        if showindex == 'always' or _bool(showindex) and not showindex_is_a_str:
            if index is None:
                index = list(range(len(rows)))
            rows = _prepend_row_index(rows, index)
        else:
            if showindex == 'never' or not _bool(showindex):
                if not showindex_is_a_str:
                    pass
    if headers:
        if len(rows) > 0:
            nhs = len(headers)
...
def tabulate(tabular_data, headers=(), tablefmt='simple', floatfmt=_DEFAULT_FLOATFMT, numalign='decimal', stralign='left', missingval=_DEFAULT_MISSINGVAL, showindex='default', disable_numparse=False):
    ...
    â•’â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•¤â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â••
    â”‚ strings   â”‚   numbers â”‚
    â•žâ•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•ªâ•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•\x90â•¡
    â”‚ spam      â”‚   41.9999 â”‚
    ...
    if tabular_data is None:
        ...
        list_of_lists, headers = _normalize_tabular_data(tabular_data,
          headers, showindex=showindex)
        if tablefmt == 'rst':
            ...
            if tablefmt in multiline_formats:
                if _is_multiline(plain_text):
        ...
        if isinstance(floatfmt, basestring):
        ...
        if isinstance(missingval, basestring):
            ...
            if headers:
                ...
                headers = [_align_header(h, a, minw, width_fn(h), is_multiline) for h, a, minw in zip(headers, t_aligns, minwidths)]
                rows = list(zip(*cols))
            else:
                minwidths = [max(width_fn(cl) for cl in c) for c in cols]
                rows = list(zip(*cols))
        tablefmt = isinstance(tablefmt, TableFormat) or _table_formats.get(tablefmt, _table_formats['simple'])
...
def _expand_numparse(disable_numparse, column_count):
    ...
    if isinstance(disable_numparse, Iterable):
        ...
        return [
         not disable_numparse] * column_count
...
def _format_table(fmt, headers, rows, colwidths, colaligns, is_multiline):
    ...
    lines = []
    if headers:
        hidden = fmt.with_header_hide if fmt.with_header_hide else []
        pad = fmt.padding
...
def _main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'h1o:s:F:f:', [
        ...
        for f in files:
            if f == '-':
                ...
                if _is_file(f):
                    ...
                    with open(f) as (fobj):
                        _pprint_file(fobj, headers=headers, tablefmt=tablefmt, sep=sep,
                          floatfmt=floatfmt,
                          file=out)
