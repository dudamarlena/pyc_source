...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dap/helper.py
...
import sys, re, operator, itertools, copy
...
def build(dapvar, new, fields, queries, ids, names):
    ...
    for var in dapvar.walk():
        ...
        if not vars_ or candidate.id in vars_ or candidate.name in vars_ and candidate.name not in ids:
            ...
            if vars_ and candidate.id not in vars_:
                if candidate.name in names:
                    raise ConstraintExpressionError('Ambiguous shorthand notation request: %s' % candidate.name)
                names.append(candidate.name)
            if vars_:
                if candidate.id in vars_:
                    index = vars_.index(candidate.id)
                else:
                    index = vars_.index(candidate.name)
        ...
        if slice_:
            candidate = slicevar(candidate, slice_)
...
def order(dataset, fields):
    ...
    for var in dataset.walk():
        ...
        if var.id in fields_:
            index = fields_.index(var.id)
        elif var.name in fields:
            index = fields.index(var.name)
        else:
            index = n + dataset._keys.index(var.name)
...
def getslice(hyperslab, shape=None):
    ...
    """
    if not isinstance(hyperslab, basestring):
        return hyperslab or slice(None)
    if hyperslab:
        output = []
...
def escape_dods(dods, pad=''):
    ...
    out = ('').join(out)
    out = out.replace('\\x5a\\x00\\x00\\x00', '<start of sequence>')
    out = out.replace('\\xa5\\x00\\x00\\x00', '<end of sequence>\n')
    return dds + out
