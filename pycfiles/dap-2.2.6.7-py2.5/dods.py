...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/dap/responses/dods.py
...
def _dispatch(dapvar, data=None):
    ...
    """
    func = {dtypes.DatasetType: _dataset, dtypes.StructureType: _structure, 
       dtypes.SequenceType: _sequence, 
       dtypes.BaseType: _base, 
       dtypes.ArrayType: _array, 
       dtypes.GridType: _grid}[type(dapvar)]
...
def _sequence(dapvar, data=None):
    for struct_ in dapvar:
        yield 'Z\x00\x00\x00'
        for line in _dispatch(struct_):
            yield line

    yield b'\xa5\x00\x00\x00'
