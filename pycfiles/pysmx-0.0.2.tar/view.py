...
# Embedded file name: build/bdist.linux-x86_64/egg/pysmvt/view.py
# Compiled at: 2010-12-06 14:17:34
from decorator import decorator
from os import path
from traceback import format_exc
import logging
from pysmvt import settings, user, ag, _getview, rg, appimportauto
from pysmvt.utils import reindent, auth_error, bad_request_error, fatal_error, urlslug, markdown, registry_has_object
from pysmvt.utils.html import strip_tags
from pysmvt.templates import JinjaHtmlBase, JinjaBase
from pysmvt.exceptions import ActionError, UserError, ProgrammingError, ViewCallStackAbort
...
class ViewBase(object):
    ...
    def call_methods(self):
        ...
        rg_has_context = registry_has_object(rg)
        if rg_has_context and rg.request.is_xhr and hasattr(self, 'xhr'):
            retval = self.xhr(**argsdict)
        elif rg_has_context and rg.request.method == 'GET' and hasattr(self, 'get'):
            retval = self.get(**argsdict)
        elif rg_has_context and rg.request.method == 'POST' and hasattr(self, 'post'):
...
class RespondingViewBase(ViewBase):
    ...
    def __init__(self, modulePath, endpoint, args):
        ...
        if hasattr(rg, 'respview'):
            raise ProgrammingError('Responding view (%s) intialized but one already exists (%s).  Only one responding view is allowed per request.' % (
             endpoint, rg.respview._endpoint))
        rg.respview = self
        self._init_response()
