...
# Embedded file name: build/bdist.linux-x86_64/egg/pysmvt/script.py
...
def prep_app():
    if not _is_application_context():
        raise UsageError("this command must be run from inside an application's directory")
    app_name = _app_name()
...
class ProfileLast(object):
    ...
    def __call__(self, caller, func=None):
        ...
        if func is None:
            ...
            return fun.make(src, dict(caller=caller, _call_=self.__call__), undecorated=caller)
        else:
