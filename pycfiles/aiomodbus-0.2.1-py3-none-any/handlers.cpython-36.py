...
# Embedded file name: /Users/denny/project/picme/aiommy/build/lib/aiommy/auth/handlers.py
# Compiled at: 2017-11-20 03:41:06
# Size of source mod 2**32: 6693 bytes
...
class OAuthProfile(object):
    ...
    def __init__(self, oauth_id=None, first_name=None, last_name=None, email=None, photo=None, thumbnail=None):
        if not oauth_id:
    ...
    def to_dict(self):
        return dict(oauth_id=(self.oauth_id),
...
class FacebookDispatcher(AbstractOAuthDispatcher):
    ...
    def normalize(self, profile):
        thumbnail = profile.get('picture').get('data').get('url') or f"{self.url}{profile.get('id')}/picture?type=normal"
        return OAuthProfile(oauth_id=(profile.get('id')),
    ...
    def _fetch_id(self, access_token):
        query_string = parse.urlencode(dict(access_token=access_token,
    ...
    def _fetch_profile(self, access_token):
        ...
        url = f"{self.url}{id}/?fields={fields}&access_token={access_token}&appsecret_proof={self._make_secret_proof(access_token)}"
    ...
    def __init__(self, facebook=None, google=None):
        if facebook:
            if not isinstance(facebook, FacebookDispatcher):
                raise TypeError('`facebook` argument should be instance of `FacebookDispatcher`')
            self.facebook = facebook
        if google:
            if not isinstance(google, GoogleDispatcher):
                raise TypeError('`google` argument should be instance of `GoogleDispatcher`')
            self.google = google
