...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/fedmsg_atomic_composer/composer.py
# Compiled at: 2017-03-17 10:41:05
import os, copy, json, glob, time, shutil, logging, tempfile, traceback, subprocess, pkg_resources
...
class AtomicComposer(object):
    ...
    def update_configs(self, release):
        ...
        git_dir = release['git_dir'] = os.path.join(release['tmp_dir'], os.path.basename(git_repo))
    ...
    def mock_cmd(self, release, *cmd, **kwargs):
        ...
        return self.call(fmt.format(**release).split() + list(cmd))
    ...
    def ostree_compose(self, release):
        ...
        self.log.info('rpm-ostree compose complete (%s), ref %s, commitid %s', datetime.utcnow() - start, ref, commitid)
        return (ref, commitid)
