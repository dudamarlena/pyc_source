...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/lib/context.py
# Compiled at: 2020-05-07 00:32:15
# Size of source mod 2**32: 19989 bytes
from six import iteritems
import json, os, urllib, re, logging, inspect
try:
...
class Context(object):
    ...
    name_prefix_re = re.compile('^(/([0-9a-zA-Z.-][_0-9a-zA-Z.-]*))*')
    ...
    def __init__(self, availability_zone, namespace):
        ...
        self._Context__iam = None
        self._Context__vpc = None
        self._Context__s3 = None
        self._Context__sns = None
        self._Context__sqs = None
    ...
    def iam(self):
        ...
        :rtype: IAMConnection
        """
        if self._Context__iam is None:
            self._Context__iam = self._Context__aws_connect(iam, 'universal')
        return self._Context__iam
    ...
    def vpc(self):
        ...
        :rtype: VPCConnection
        """
        if self._Context__vpc is None:
            self._Context__vpc = self._Context__aws_connect(vpc)
        return self._Context__vpc
    ...
    def s3(self):
        ...
        :rtype: S3Connection
        """
        if self._Context__s3 is None:
            self._Context__s3 = S3Connection()
        return self._Context__s3
    ...
    def sns(self):
        ...
        :rtype: SNSConnection
        """
        if self._Context__sns is None:
            self._Context__sns = self._Context__aws_connect(sns)
        return self._Context__sns
    ...
    def sqs(self):
        ...
        :rtype: SQSConnection
        """
        if self._Context__sqs is None:
            self._Context__sqs = self._Context__aws_connect(sqs)
        return self._Context__sqs
    ...
    def close(self):
        if self._Context__vpc is not None:
            self._Context__vpc.close()
        else:
            if self._Context__s3 is not None:
                self._Context__s3.close()
            else:
                if self._Context__iam is not None:
                    self._Context__iam.close()
                if self._Context__sns is not None:
                    self._Context__sns.close()
            if self._Context__sqs is not None:
                self._Context__sqs.close()
    ...
    def setup_iam_ec2_role(self, role_name, policies):
        ...
        self._Context__setup_entity_policies(aws_role_name, policies, list_policies=(self.iam.list_role_policies),
    ...
    def __setup_entity_policies(self, entity_name, policies, list_policies, delete_policy, get_policy, put_policy):
        ...
        for policy_name, policy in iteritems(policies):
            current_policy = None
            try:
                current_policy = json.loads(unquote(get_policy(entity_name, policy_name).policy_document))
            except BotoServerError as e:
                if e.status == 404:
