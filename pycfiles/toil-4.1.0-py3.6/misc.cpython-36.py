...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/lib/misc.py
# Compiled at: 2020-05-07 00:32:15
# Size of source mod 2**32: 10408 bytes
import random
from math import sqrt
import logging, errno, os, shutil, sys, time, uuid, socket
...
def atomic_tmp_file(final_path):
    ...
    final_dir = os.path.dirname(os.path.normpath(final_path))
    if final_dir == '/dev':
        return final_path
    else:
...
class CalledProcessErrorStderr(subprocess.CalledProcessError):
    __doc__ = 'Version of CalledProcessError that include stderr in the error message if it is set'
