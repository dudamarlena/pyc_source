...
# Size of source mod 2**32: 37753 bytes
import ast, logging, os, pwd, socket, time, sys, getpass, json, traceback, addict
...
class MesosBatchSystem(BatchSystemLocalSupport, AbstractScalableBatchSystem, Scheduler):
    ...
    def frameworkMessage(self, driver, executorId, agentId, message):
        ...
        for k, v in message.items():
            if k == 'nodeInfo':
                ...
                resources = [taskData for taskData in self.runningJobMap.values() if taskData.executorID == executorId.value]
    ...
    def getNodes(self, preemptable=None, timeout=600):
        ...
        return {nodeAddress:executor.nodeInfo for nodeAddress, executor in self.executors.items() if preemptable is None or preemptable == (executor.agentId not in self.nonPreemptableNodes)}
