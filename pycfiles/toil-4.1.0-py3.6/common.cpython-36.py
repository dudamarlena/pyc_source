...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/common.py
# Compiled at: 2020-05-07 00:32:15
# Size of source mod 2**32: 68164 bytes
...
import logging, os, re, sys, tempfile, time, uuid, requests
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.deadlockWait = 60
        self.deadlockCheckInterval = 30
        self.statePollingWait = 1
    ...
    def setOptions(self, options):
        ...
        if self.workDir is not None:
            ...
            if not os.path.exists(self.workDir):
                raise RuntimeError('The path provided to --workDir (%s) does not exist.' % self.workDir)
        ...
        if self.stats:
            if self.clean != 'never':
                if self.clean is not None:
                    raise RuntimeError("Contradicting options passed: Clean flag is set to %s despite the stats flag requiring the jobStore to be intact at the end of the run. Set clean to 'never'" % self.clean)
            self.clean = 'never'
        else:
        ...
        if not 0.0 <= self.betaInertia <= 0.9:
            ...
            if not 0.0 <= self.preemptableCompensation <= 1.0:
                raise RuntimeError('preemptableCompensation (%f) must be between 0.0 and 1.0!' % self.preemptableCompensation)
            ...
            assert not (self.writeLogs and self.writeLogsGzip), 'Cannot use both --writeLogs and --writeLogsGzip at the same time.'
        if not not self.writeLogsFromAllJobs:
            if not self.writeLogs:
                assert self.writeLogsGzip, 'To enable --writeLogsFromAllJobs, either --writeLogs or --writeLogsGzip must be set.'
...
jobStoreLocatorHelp = 'A job store holds persistent information about the jobs and files in a workflow. If the workflow is run with a distributed batch system, the job store must be accessible by all worker nodes. Depending on the desired job store implementation, the location should be formatted according to one of the following schemes:\n\nfile:<path> where <path> points to a directory on the file systen\n\naws:<region>:<prefix> where <region> is the name of an AWS region like us-west-2 and <prefix> will be prepended to the names of any top-level AWS resources in use by job store, e.g. S3 buckets.\n\n google:<project_id>:<prefix> TODO: explain\n\nFor backwards compatibility, you may also specify ./foo (equivalent to file:./foo or just file:foo) or /bar (equivalent to file:/bar).'

def _addOptions(addGroupFn, config):
    addOptionFn = addGroupFn('toil core options', 'Options to specify the location of the Toil workflow and turn on stats collation about the performance of jobs.')
    addOptionFn('jobStore', type=str, help=('The location of the job store for the workflow. ' + jobStoreLocatorHelp))
    addOptionFn('--workDir', dest='workDir', default=None, help='Absolute path to directory where temporary files generated during the Toil run should be placed. Standard output and error from batch system jobs (unless --noStdOutErr) will be placed in this directory. A cache directory may be placed in this directory. Temp files and folders will be placed in a directory toil-<workflowID> within workDir. The workflowID is generated by Toil and will be reported in the workflow logs. Default is determined by the variables (TMPDIR, TEMP, TMP) via mkdtemp. This directory needs to exist on all machines running jobs; if capturing standard output and error from batch system jobs is desired, it will generally need to be on a shared file system. When sharing a cache between containers on a host, this directory must be shared between the containers.')
    addOptionFn('--noStdOutErr', dest='noStdOutErr', action='store_true', default=None, help='Do not capture standard output and error from batch system jobs.')
    addOptionFn('--stats', dest='stats', action='store_true', default=None, help="Records statistics about the toil workflow to be used by 'toil stats'.")
    addOptionFn('--clean', dest='clean', choices=['always', 'onError', 'never', 'onSuccess'], default=None,
      help="Determines the deletion of the jobStore upon completion of the program. Choices: 'always', 'onError','never', 'onSuccess'. The --stats option requires information from the jobStore upon completion so the jobStore will never be deleted withthat flag. If you wish to be able to restart the run, choose 'never' or 'onSuccess'. Default is 'never' if stats is enabled, and 'onSuccess' otherwise")
    addOptionFn('--cleanWorkDir', dest='cleanWorkDir', choices=[
     'always', 'never', 'onSuccess', 'onError'],
      default='always',
      help="Determines deletion of temporary worker directory upon completion of a job. Choices: 'always', 'never', 'onSuccess'. Default = always. WARNING: This option should be changed for debugging only. Running a full pipeline with this option could fill your disk with intermediate data.")
    addOptionFn('--clusterStats', dest='clusterStats', nargs='?', action='store', default=None,
      const=(os.getcwd()),
      help='If enabled, writes out JSON resource usage statistics to a file. The default location for this file is the current working directory, but an absolute path can also be passed to specify where this file should be written. This options only applies when using scalable batch systems.')
    addOptionFn = addGroupFn('toil options for restarting an existing workflow', 'Allows the restart of an existing workflow')
    addOptionFn('--restart', dest='restart', default=None, action='store_true', help='If --restart is specified then will attempt to restart existing workflow at the location pointed to by the --jobStore option. Will raise an exception if the workflow does not exist')
    addOptionFn = addGroupFn('toil options for specifying the batch system', 'Allows the specification of the batch system, and arguments to the batch system/big batch system (see below).')
    addBatchOptions(addOptionFn, config)
    addOptionFn = addGroupFn('toil options for autoscaling the cluster of worker nodes', 'Allows the specification of the minimum and maximum number of nodes in an autoscaled cluster, as well as parameters to control the level of provisioning.')
    addOptionFn('--provisioner', dest='provisioner', choices=['aws', 'gce'], help=("The provisioner for cluster auto-scaling. The currently supported choices are'gce', or 'aws'. The default is %s." % config.provisioner))
    addOptionFn('--nodeTypes', default=None, help="List of node types separated by commas. The syntax for each node type depends on the provisioner used. For the cgcloud and AWS provisioners this is the name of an EC2 instance type, optionally followed by a colon and the price in dollars to bid for a spot instance of that type, for example 'c3.8xlarge:0.42'.If no spot bid is specified, nodes of this type will be non-preemptable.It is acceptable to specify an instance as both preemptable and non-preemptable, including it twice in the list. In that case,preemptable nodes of that type will be preferred when creating new nodes once the maximum number of preemptable-nodes has beenreached.")
    addOptionFn('--nodeOptions', default=None, help='Options for provisioning the nodes. The syntax depends on the provisioner used. Neither the CGCloud nor the AWS provisioner support any node options.')
    addOptionFn('--minNodes', default=None, help='Mininum number of nodes of each type in the cluster, if using auto-scaling. This should be provided as a comma-separated list of the same length as the list of node types. default=0')
    addOptionFn('--maxNodes', default=None, help=('Maximum number of nodes of each type in the cluster, if using autoscaling, provided as a comma-separated list. The first value is used as a default if the list length is less than the number of nodeTypes. default=%s' % config.maxNodes[0]))
    addOptionFn('--targetTime', dest='targetTime', default=None, help=('Sets how rapidly you aim to complete jobs in seconds. Shorter times mean more aggressive parallelization. The autoscaler attempts to scale up/down so that it expects all queued jobs will complete within targetTime seconds. default=%s' % config.targetTime))
    addOptionFn('--betaInertia', dest='betaInertia', default=None, help=('A smoothing parameter to prevent unnecessary oscillations in the number of provisioned nodes. This controls an exponentially weighted moving average of the estimated number of nodes. A value of 0.0 disables any smoothing, and a value of 0.9 will smooth so much that few changes will ever be made.  Must be between 0.0 and 0.9.  default=%s' % config.betaInertia))
    addOptionFn('--scaleInterval', dest='scaleInterval', default=None, help=('The interval (seconds) between assessing if the scale of the cluster needs to change. default=%s' % config.scaleInterval))
    addOptionFn('--preemptableCompensation', dest='preemptableCompensation', default=None,
      help=('The preference of the autoscaler to replace preemptable nodes with non-preemptable nodes, when preemptable nodes cannot be started for some reason. Defaults to %s. This value must be between 0.0 and 1.0, inclusive. A value of 0.0 disables such compensation, a value of 0.5 compensates two missing preemptable nodes with a non-preemptable one. A value of 1.0 replaces every missing pre-emptable node with a non-preemptable one.' % config.preemptableCompensation))
    addOptionFn('--nodeStorage', dest='nodeStorage', default=50, help='Specify the size of the root volume of worker nodes when they are launched in gigabytes. You may want to set this if your jobs require a lot of disk space. The default value is 50.')
    addOptionFn('--metrics', dest='metrics', default=False,
      action='store_true',
      help='Enable the prometheus/grafana dashboard for monitoring CPU/RAM usage, queue size, and issued jobs.')
    if not config.cwl:
        addOptionFn = addGroupFn('toil options for limiting the number of service jobs and detecting service deadlocks', 'Allows the specification of the maximum number of service jobs in a cluster. By keeping this limited  we can avoid all the nodes being occupied with services, so causing a deadlock')
        addOptionFn('--maxServiceJobs', dest='maxServiceJobs', default=None, type=int, help=('The maximum number of service jobs that can be run concurrently, excluding service jobs running on preemptable nodes. default=%s' % config.maxServiceJobs))
        addOptionFn('--maxPreemptableServiceJobs', dest='maxPreemptableServiceJobs', default=None, type=int, help=('The maximum number of service jobs that can run concurrently on preemptable nodes. default=%s' % config.maxPreemptableServiceJobs))
        addOptionFn('--deadlockWait', dest='deadlockWait', default=None, type=int, help=('Time, in seconds, to tolerate the workflow running only the same service jobs, with no jobs to use them, before declaring the workflow to be deadlocked and stopping. default=%s' % config.deadlockWait))
        addOptionFn('--deadlockCheckInterval', dest='deadlockCheckInterval', default=None, type=int, help=('Time, in seconds, to wait between checks to see if the workflow is stuck running only service jobs, with no jobs to use them. Should be shorter than --deadlockWait. May need to be increased if the batch system cannot enumerate running jobs quickly enough, or if polling for running jobs is placing an unacceptable load on a shared cluster. default=%s' % config.deadlockCheckInterval))
        addOptionFn('--statePollingWait', dest='statePollingWait', default=1, type=int, help='Time, in seconds, to wait before doing a scheduler query for job state. Return cached results if within the waiting period.')
    else:
        addOptionFn = addGroupFn('toil options for cores/memory requirements', 'The options to specify default cores/memory requirements (if not specified by the jobs themselves), and to limit the total amount of memory/cores requested from the batch system.')
        addOptionFn('--defaultMemory', dest='defaultMemory', default=None, metavar='INT', help=('The default amount of memory to request for a job. Only applicable to jobs that do not specify an explicit value for this requirement. Standard suffixes like K, Ki, M, Mi, G or Gi are supported. Default is %s' % bytes2human((config.defaultMemory), symbols='iec')))
        addOptionFn('--defaultCores', dest='defaultCores', default=None, metavar='FLOAT', help=('The default number of CPU cores to dedicate a job. Only applicable to jobs that do not specify an explicit value for this requirement. Fractions of a core (for example 0.1) are supported on some batch systems, namely Mesos and singleMachine. Default is %.1f ' % config.defaultCores))
        addOptionFn('--defaultDisk', dest='defaultDisk', default=None, metavar='INT', help=('The default amount of disk space to dedicate a job. Only applicable to jobs that do not specify an explicit value for this requirement. Standard suffixes like K, Ki, M, Mi, G or Gi are supported. Default is %s' % bytes2human((config.defaultDisk), symbols='iec')))
    ...
    addOptionFn('--maxCores', dest='maxCores', default=None, metavar='INT', help=('The maximum number of CPU cores to request from the batch system at any one time. Standard suffixes like K, Ki, M, Mi, G or Gi are supported. Default is %s' % bytes2human((config.maxCores), symbols='iec')))
    addOptionFn('--maxMemory', dest='maxMemory', default=None, metavar='INT', help=('The maximum amount of memory to request from the batch system at any one time. Standard suffixes like K, Ki, M, Mi, G or Gi are supported. Default is %s' % bytes2human((config.maxMemory), symbols='iec')))
    addOptionFn('--maxDisk', dest='maxDisk', default=None, metavar='INT', help=('The maximum amount of disk space to request from the batch system at any one time. Standard suffixes like K, Ki, M, Mi, G or Gi are supported. Default is %s' % bytes2human((config.maxDisk), symbols='iec')))
    addOptionFn = addGroupFn('toil options for rescuing/killing/restarting jobs', 'The options for jobs that either run too long/fail or get lost                              (some batch systems have issues!)')
    addOptionFn('--retryCount', dest='retryCount', default=None, help=('Number of times to retry a failing job before giving up and labeling job failed. default=%s' % config.retryCount))
    addOptionFn('--enableUnlimitedPreemptableRetries', dest='enableUnlimitedPreemptableRetries', action='store_true',
      default=False,
      help='If set, preemptable failures (or any failure due to an instance getting unexpectedly terminated) would not count towards job failures and --retryCount.')
    addOptionFn('--maxJobDuration', dest='maxJobDuration', default=None, help=('Maximum runtime of a job (in seconds) before we kill it (this is a lower bound, and the actual time before killing the job may be longer). default=%s' % config.maxJobDuration))
    addOptionFn('--rescueJobsFrequency', dest='rescueJobsFrequency', default=None, help=('Period of time to wait (in seconds) between checking for missing/overlong jobs, that is jobs which get lost by the batch system. Expert parameter. default=%s' % config.rescueJobsFrequency))
    addOptionFn = addGroupFn('Toil Miscellaneous Options', 'Miscellaneous Options')
    addOptionFn('--disableCaching', dest='disableCaching', type='bool',
      nargs='?',
      const=True,
      default=True,
      help='Disables caching in the file store. This flag must be set to use a batch system that does not support caching such as Grid Engine, Parasol, LSF, or Slurm')
    addOptionFn('--disableChaining', dest='disableChaining', action='store_true', default=False, help="Disables chaining of jobs (chaining uses one job's resource allocation for its successor job if possible).")
    addOptionFn('--disableJobStoreChecksumVerification', dest='disableJobStoreChecksumVerification', default=False,
      action='store_true',
      help='Disables checksum verification for files transferred to/from the job store. Checksum verification is a safety check to ensure the data is not corrupted during transfer. Currently only supported for non-streaming AWS files.')
    addOptionFn('--maxLogFileSize', dest='maxLogFileSize', default=None, help=('The maximum size of a job log file to keep (in bytes), log files larger than this will be truncated to the last X bytes. Setting this option to zero will prevent any truncation. Setting this option to a negative value will truncate from the beginning.Default=%s' % bytes2human(config.maxLogFileSize)))
    addOptionFn('--writeLogs', dest='writeLogs', nargs='?', action='store', default=None,
      const=(os.getcwd()),
      help="Write worker logs received by the leader into their own files at the specified path. Any non-empty standard output and error from failed batch system jobs will also be written into files at this path. The current working directory will be used if a path is not specified explicitly. Note: By default only the logs of failed jobs are returned to leader. Set log level to 'debug' or enable '--writeLogsFromAllJobs' to get logs back from successful jobs, and adjust 'maxLogFileSize' to control the truncation limit for worker logs.")
    addOptionFn('--writeLogsGzip', dest='writeLogsGzip', nargs='?', action='store', default=None,
      const=(os.getcwd()),
      help='Identical to --writeLogs except the logs files are gzipped on the leader.')
    addOptionFn('--writeLogsFromAllJobs', dest='writeLogsFromAllJobs', action='store_true', default=False,
      help="Whether to write logs from all jobs (including the successful ones) without necessarily setting the log level to 'debug'. Ensure that either --writeLogs or --writeLogsGzip is set if enabling this option.")
    addOptionFn('--realTimeLogging', dest='realTimeLogging', action='store_true', default=False, help='Enable real-time logging from workers to masters')
    addOptionFn('--sseKey', dest='sseKey', default=None, help='Path to file containing 32 character key to be used for server-side encryption on awsJobStore or googleJobStore. SSE will not be used if this flag is not passed.')
    addOptionFn('--setEnv', '-e', metavar='NAME=VALUE or NAME', dest='environment',
      default=[],
      action='append',
    ...
    addOptionFn('--servicePollingInterval', dest='servicePollingInterval', default=None, help=('Interval of time service jobs wait between polling for the existence of the keep-alive flag (defailt=%s)' % config.servicePollingInterval))
    addOptionFn('--forceDockerAppliance', dest='forceDockerAppliance', action='store_true', default=False,
    ...
    addOptionFn('--disableProgress', dest='disableProgress', action='store_true', default=False, help='Disables the progress bar shown when standard error is a terminal.')
    addOptionFn = addGroupFn('toil debug options', 'Debug options')
    addOptionFn('--debugWorker', default=False, action='store_true', help='Experimental no forking mode for local debugging. Specifically, workers are not forked and stderr/stdout are not redirected to the log.')
    addOptionFn('--disableWorkerOutputCapture', default=False, action='store_true', help="Let worker output go to worker's standard out/error instead of per-job logs.")
    addOptionFn('--badWorker', dest='badWorker', default=None, help=("For testing purposes randomly kill 'badWorker' proportion of jobs using SIGKILL, default=%s" % config.badWorker))
    addOptionFn('--badWorkerFailInterval', dest='badWorkerFailInterval', default=None, help=("When killing the job pick uniformly within the interval from 0.0 to 'badWorkerFailInterval' seconds after the worker starts, default=%s" % config.badWorkerFailInterval))
...
def addOptions(parser, config=Config()):
    ...
    if isinstance(parser, ArgumentParser):
        ...
        raise RuntimeError('Unanticipated class passed to addOptions(), %s. Expecting argparse.ArgumentParser' % parser.__class__)
    ...
    for idSourceFile in ('/var/lib/dbus/machine-id', '/proc/sys/kernel/random/boot_id'):
        ...
        if len(nodeIDs) == 2:
            if nodeIDs[0] == nodeIDs[1]:
                ...
                logger.warning('Different node IDs {} received from repeated calls to uuid.getnode(). You should use another method to generate node ID.'.format(nodeIDs))
            logger.debug('Obtained node ID {} from uuid.getnode()'.format(nodeID))

    if not nodeID:
        logger.warning('Failed to generate stable node ID, returning empty string. If you see this message with a work dir on a shared file system when using workers running on multiple nodes, you might experience cryptic job failures')
    return nodeID
...
class Toil(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        """
        try:
            if exc_type is not None and self.config.clean == 'onError' or exc_type is None and self.config.clean == 'onSuccess' or self.config.clean == 'always':
                try:
    ...
    def restart(self):
        ...
        if not self.config.restart:
            raise ToilRestartException('A Toil workflow must be initiated with Toil.start(), not restart().')
    ...
    def _setProvisioner(self):
        if self.config.provisioner is None:
            ...
            self._provisioner = clusterFactory(provisioner=(self.config.provisioner), clusterName=None,
              zone=None,
    ...
    def createBatchSystem(config):
        ...
        """
        kwargs = dict(config=config, maxCores=(config.maxCores),
        ...
        if not config.disableCaching:
            if not batchSystemClass.supportsWorkerCleanup():
                raise RuntimeError('%s currently does not support shared caching.  Set the --disableCaching flag if you want to use this batch system.' % config.batchSystem)
        logger.debug('Using the %s' % re.sub('([a-z])([A-Z])', '\\g<1> \\g<2>', batchSystemClass.__name__).lower())
    ...
    def _setupAutoDeployment(self, userScript=None):
        ...
        if userScript is not None:
            if userScript.belongsToToil:
                ...
                if self._batchSystem.supportsAutoDeployment():
                    if not self.config.disableAutoDeployment:
                ...
                if not isinstance(self._batchSystem, SingleMachineBatchSystem):
                    logger.warning('Batch system does not support auto-deployment. The user script %s will have to be present at the same location on every worker.', userScript)
                userScript = None
        else:
    ...
    def getToilWorkDir(configWorkDir=None):
        ...
        if not os.path.exists(workDir):
            raise RuntimeError('The directory specified by --workDir or TOIL_WORKDIR (%s) does not exist.' % workDir)
        return workDir
    ...
    def _runMainLoop(self, rootJob):
        ...
        with RealtimeLogger((self._batchSystem), level=(self.options.logLevel if self.options.realTimeLogging else None)):
            ...
            return Leader(config=(self.config), batchSystem=(self._batchSystem),
...
class ToilContextManagerException(Exception):
    ...
    def __init__(self):
        super(ToilContextManagerException, self).__init__('This method cannot be called outside the "with Toil(...)" context manager.')
        ...
        try:
            ...
            try:
                for attempt in retry(delays=(0, 1, 1, 4, 16), predicate=requestPredicate):
                    with attempt:
                        requests.post('http://localhost:3000/api/datasources', auth=('admin',
                                                                                     'admin'), data='{"name":"DS_PROMETHEUS","type":"prometheus",                                   "url":"http://localhost:9090", "access":"direct"}',
...
def getDirSizeRecursively(dirPath):
    ...
    """
    try:
        return int(subprocess.check_output(['du', '-s', dirPath], env=dict((os.environ), BLOCKSIZE='512')).decode('utf-8').split()[0]) * 512
    except subprocess.CalledProcessError:
        return 0
