...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil/lib/ec2.py
...
def retry_ec2(t=a_short_time, retry_for=10 * a_short_time, retry_while=not_found):
    return retry(delays=(t, t, t * 2, t * 4), timeout=retry_for,
            ...
            for r in requests:
                if r.state == 'open':
                    ...
                    if r.status.code == 'pending-evaluation':
                        ...
                        if r.status.code == 'pending-fulfillment':
...
def create_spot_instances(ec2, price, image_id, spec, num_instances=1, timeout=None, tentative=False, tags=None):
    ...
    for batch in wait_spot_requests_active(ec2, requests,
      timeout=timeout,
      tentative=tentative):
        instance_ids = []
...
def create_ondemand_instances(ec2, image_id, spec, num_instances=1):
    ...
    for attempt in retry_ec2(retry_for=a_long_time, retry_while=inconsistencies_detected):
        with attempt:
            return (ec2.run_instances)(image_id, min_count=num_instances, 
