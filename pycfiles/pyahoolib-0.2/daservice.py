...
# Embedded file name: build\bdist.win32\egg\pyahooapis\jlp\daservice.py
# Compiled at: 2011-05-16 12:39:56
from pyahooapis.service import Service, BaseObject

class DAService(Service):
    u"""Yahoo!テキスト解析 日本語係り受け解析APIのPythonラッパー
    ...
    def get_chunks(self, sentence, json=False):
        ...
        for c in dom.getElementsByTagName('Chunk'):
            morphems = [ Morphem(self._get_text(m, 'Surface'), self._get_text(m, 'Reading'), self._get_text(m, 'Baseform'), self._get_text(m, 'POS'), self._get_text(m, 'Feature')) for m in c.getElementsByTagName('Morphem')
                       ]
            chunks.append(Chunk(int(self._get_text(c, 'Id')), int(self._get_text(c, 'Dependency')), morphems))
