...
# Embedded file name: build\bdist.win32\egg\pyahooapis\jlp\kouseiservice.py
# Compiled at: 2011-05-16 12:42:50
from pyahooapis.service import Service, BaseObject
FILTER_GROUP_MISS = 1
FILTER_GROUP_SIMPLIFY = 2
FILTER_GROUP_TO_BETTER = 4
FILTER_GROUPS = {FILTER_GROUP_MISS: 1, 
   FILTER_GROUP_SIMPLIFY: 2, 
...
NO_FILTERS = {NO_FILTER_FALSE_CONVERSION: 1, 
   NO_FILTER_MISUSE: 2, 
...
class KouseiService(Service):
    ...
    def get_results(self, sentence, filter_group=None, no_filter=None, json=False):
        ...
        results = [ Result(self._get_text(r, 'StartPos'), self._get_text(r, 'Length'), self._get_text(r, 'Surface'), self._get_text(r, 'ShitekiWord'), self._get_text(r, 'ShitekiInfo')) for r in dom.getElementsByTagName('Result')
                  ]
