...
# Embedded file name: build\bdist.win32\egg\pyahooapis\jlp\jimservice.py
# Compiled at: 2011-05-16 12:59:16
...
RESPONSES = {RESPONSE_KATAKANA: 'katakana', 
...
DICTIONARIES = {DICTIONARY_DEFAULT: 'default', 
...
class JIMService(Service):
    ...
    def get_segments(self, sentence, format=None, mode=None, response=None, dictionary=None, results=999, json=False):
        params = {}
        ...
        segmentlist = [ Segment(self._get_text(s, 'SegmentText'), self._get_text(s, 'Alphanumeric'), self._get_text(s, 'HalfAlphanumeric'), self._get_text(s, 'Katakana'), self._get_text(s, 'HalfKatakana'), self._get_text(s, 'Hiragana'), [ c.firstChild.nodeValue for c in s.getElementsByTagName('Candidate') ]) for s in dom.getElementsByTagName('Segment')
                      ]
...
class Segment(BaseObject):
    ...
    def __init__(self, segment_text, alphanumeric=None, half_alphanumeric=None, katakana=None, half_katakana=None, hiragana=None, candidate_list=None):
