...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ohm/server.py
...
from ohm.validators import JSONConverter, SimplePostConverter, to_python_headers, from_python_headers, SimplePostIdentity
...
class ApplicationWrapper(object):
    ...
    def __classinit__(cls, new_attrs):
        if '_attribute_apps' not in new_attrs:
    ...
    def __repr__(self):
        return '<%s wrapping %s>' % (self.__class__.__name__, repr(self.object).strip('<>'))

    def __call__(self, environ, start_response):
        path_info = environ.get('PATH_INFO', '')
        for (prefix, app) in self._attribute_apps:
            if prefix == path_info or path_info.startswith(prefix + '/'):
    ...
    def not_found(self, environ, start_response):
        exc = httpexceptions.HTTPNotFound('No handler for %r (need one of attributes: %s)' % (self, (', ').join([ repr(path) for (path, app) in self._attribute_apps ])))
    ...
    def add_attribute_app(cls, prefix, app):
        ...
        cls._attribute_apps.sort(key=lambda i: -len(i[0]))
...
class Setter(object):
    ...
    def __init__(self, unicode=False, content_type=None, uri_path=None, parent_app=None, attr=None, validator=None, POST=None, getter=None, setter=None, deleter=None):
    ...
    def __addtoclass__(self, cls, name):
        assert self.parent_app is None and self.attr is None, 'Attribute %r bound multiple times to different classes (first %r as %r, not %r as %r)' % (self.parent_app, self.attr, cls, name)
        self.parent_app = cls
        self.attr = name
        if self.uri_path is None:
            self.uri_path = name
        assert self.uri_path not in cls._attribute_apps, 'Setter already registered at %r: %r' % (self.uri_path, cls._attribute_apps[self.uri_path])
    ...
    def getter(self, obj):
        try:
            ...
            raise httpexceptions.HTTPNotFound('You cannot read this resource (attribute %r: %s)' % (self.attr, e))

    def setter(self, obj, value):
        try:
            setattr(obj, self.attr, value)
        except AttributeError, e:
            raise httpexceptions.HTTPMethodNotAllowed('You cannot PUT to this resource (attribute %r: %s)' % (self.attr, e), headers=[('Allow', 'GET,POST')])

    def deleter(self, obj):
        try:
            delattr(obj, self.attr)
        except AttributeError, e:
            raise httpexceptions.HTTPMethodNotAllowed('You cannot DELETE this resource (attribute %r: %s)' % (self.attr, e), headers=[('Allow', 'GET,PUT,POST')])
    ...
    def __call__(self, environ, start_response):
        ...
        if self_method is None:
            exc = httpexceptions.HTTPNotImplemented('The method %r is not implemented' % method)
            return exc(environ, start_response)
        try:
    ...
    def method_GET(self, obj, environ, start_response):
        try:
            ...
            exc = httpexceptions.HTTPNotFound('Cannot retrieve: %s' % e)
            return exc(environ, start_response)
        ...
        extra_ct = ''
        if self.unicode:
            assert isinstance(data, unicode), 'Did not get unicode data as expected; got: %r' % data
            data = data.encode(self.default_encoding)
            extra_ct += '; charset=%s' % self.default_encoding
        else:
            assert isinstance(data, str), 'Did not get str data as expected; got: %r' % data
    ...
    def method_POST(self, obj, environ, start_response):
        if not self.POST:
            exc = httpexceptions.HTTPNotImplemented('No POST methods have been defined')
            return exc(environ, start_response)
        qs = cgi.parse_qsl(environ.get('QUERY_STRING', ''), keep_blank_values=True)
        command = ''
        if qs:
            if not qs[0][1]:
                command = qs[0][0]
            ...
            if command not in self.POST:
                ...
                exc = httpexceptions.HTTPBadRequest('No POST method %s defined (need one of %s)' % (command_desc, (', ').join([ repr(k) for k in self.POST.keys() ])))
                return exc(environ, start_response)
    ...
    def call_POST(self, obj, command, request):
        if isinstance(command, basestring):
            if self.default_validator:
                command = (
                 self.default_validator, command)
            else:
                command = (
                 SimplePostConverter(), command)
        if not isinstance(command, (list, tuple)) or not len(command) == 2:
            raise TypeError('Commands must be in the form (validator, method) not %r' % command)
    ...
    _text_re = re.compile('^[^\\x00-\\x1f]+$')
    _mime_re = re.compile('^[a-z]+/[a-zA-Z0-9._-]_$')
    ...
    def _coerce_POST_response(self, response):
        if isinstance(response, basestring):
            ...
            elif isinstance(response, unicode) or self._text_re.search(response):
        ...
        if isinstance(response, tuple) and len(response) == 2 and isinstance(response[0], basestring) and self._mime_re.search(response[0]):
            return (
             [
              (
               'Content-Type', response[0])], response[1])
        if isinstance(response, tuple) and len(response) == 2 and (hasattr(response[0], 'items') or isinstance(response[0], list)):
            if not isinstance(response[0], list):
                response = (
                 response[0].items(), response[1])
            return response
        raise ValueError("I don't know how to turn %r into a WSGI response" % response)
...
class MethodNotAllowed(object):
    ...
    def __call__(self, *args, **kw):
        raise httpexceptions.HTTPMethodNotAllowed(self.msg, headers=[('Allow', self.allow)])
