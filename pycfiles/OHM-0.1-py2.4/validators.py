...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ohm/validators.py
...
class XMLRPCConverter(FancyValidator):
    ...
    def _unwrap(self, value, state):
        start = value.find('>')
        if start == -1:
            raise Invalid('Not valid XML: %r' % value, value, state)
        value = value[start + 1:]
        end = value.rfind('<')
        if end == -1:
            raise Invalid('Not valid XML: %r' % value, value, state)
    ...
    def _wrap_xmlrpc(self, value, state):
        return '<methodResponse><params><param><value>%s</value></param></params></methodResponse>' % value
...
class ToFrom(Validator):
    ...
    def __init__(self, *args, **kw):
        ...
        if not self.to_python_validator:
            raise ValueError('You must give a value for to_python_validator')
        if not self.from_python_validator:
            raise ValueError('You must give a value for from_python_validator')
        if isinstance(self.to_python_validator, type) and issubclass(self.to_python_validator, Validator):
            self.to_python_validator = self.to_python_validator()
        if isinstance(self.from_python_validator, type) and issubclass(self.from_python_validator, Validator):
...
class FormPostConverter(FancyValidator):
    ...
    def _to_python(self, value, state=None):
        ...
        fs = cgi.FieldStorage(fp=StringIO(body), environ=environ, keep_blank_values=1)
        if not dict:
            return fs
        result = MultiDict()
    ...
    def _from_python(self, value, state):
        if not isinstance(value, basestring):
            if hasattr(value, '__unicode__'):
                ...
                value = str(value)
        if isinstance(value, unicode):
            value = value.encode(self.encoding or self.default_encoding)
        return value
