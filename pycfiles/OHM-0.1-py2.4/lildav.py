...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ohm/lildav.py
...
class LilDAV(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if os.path.isdir(full) and environ['PATH_INFO']:
            subapp = self.__class__(full, root_directory=self.root_directory)
...
class PuttableFileApp(FileApp):
    ...
    def directory_index(self, environ, start_response):
        ...
        filenames.sort()
        parts = ['<html><head><title>Directory Listing</title></head>\n<body><h1>Directory Listing</h1>\n<ul>\n']
        for filename in filenames:
            parts.append('<li><a href="%s">%s</a></li>\n' % (cgi.escape(urllib.quote(filename), 1), cgi.escape(filename, 1)))
