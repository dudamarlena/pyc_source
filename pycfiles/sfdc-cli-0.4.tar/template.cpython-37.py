...
# [GCC 8.4.0]
# Embedded file name: D:\Workspace\sfdc-cli\sfdc_cli\templates\template.py
# Compiled at: 2020-03-08 03:41:21
# Size of source mod 2**32: 8015 bytes
...
class AntConfig:
    ...
    def build_migration_tools(self, save_path, config_data, template_name='MigrationTools'):
        ...
        build_xml_src = build_xml_src.replace('{jar_path}', config_data['jar_path']).replace('{target_proxy_body}', self._get_ant_proxy_body(config_data)).replace('{jar_url_path}', config_data['jar_url_path'])
    ...
    def build_ant_dataloader(self, save_path, config_data):
        ...
        build_xml_src = build_xml_src.replace('{ant_export_xml}', config_data['ant_export_xml']).replace('{dataloader_jar_name}', config_data['dataloader_jar_name'])
        self._save_file(os.path.join(save_path, 'build.xml'), build_xml_src)

    def _get_ant_proxy_body(self, config_data):
        proxy_config = config_data['proxy']
        xml_str_list = []
        if 'use_proxy' in proxy_config:
            if proxy_config['use_proxy']:
                if 'nonproxyhosts' in proxy_config:
                    if proxy_config['nonproxyhosts']:
                        xml_str_list.append('nonproxyhosts="' + proxy_config['nonproxyhosts'] + '"')
                    else:
                        if 'proxyhost' in proxy_config:
                            if proxy_config['proxyhost']:
                                xml_str_list.append('proxyhost="' + proxy_config['proxyhost'] + '"')
                        if 'proxypassword' in proxy_config:
                            if proxy_config['proxypassword']:
                    ...
                    if 'proxyuser' in proxy_config:
                        if proxy_config['proxyuser']:
                            xml_str_list.append('proxyuser="' + proxy_config['proxyuser'] + '"')
                else:
                    if 'socksproxyhost' in proxy_config:
                        if proxy_config['socksproxyhost']:
