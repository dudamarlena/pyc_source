...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/error_explainer/utils.py
...
def is_correct_assignment_signature(code: str) -> bool:
    ...
    try:
        ...
        if code[(equals_index + 1)] != '=':
            ...
            return should_be_var_names is None or all(is_correct_variable_name_token(token) for token in should_be_var_names)
    except ValueError:
        return False
...
def is_bad_coma_usage(code: str) -> bool:
    ...
    if len(should_be_var_names) == 1:
        return any(token.type == tokenize.OP and token.string == ',' for token in tokenize_line(other))
...
def remove_irrelevant_tokens_in_var_names(tokens: List[tokenize.TokenInfo]) -> Optional[List[TokenInfo]]:
    ...
    if any(token for token in tokens if token.type == tokenize.COMMENT):
        ...
        striped = [token for token in tokens if token.type != tokenize.ENDMARKER if token.type != tokenize.NEWLINE if token.string != ',' if token.string != '.']
        if any(token.string == '[' for token in striped):
            striped = [token for token in striped if token.type != tokenize.NUMBER if token.string != '[' if token.string != ']' if token.string != ':']
...
def count_brackets(string: str) -> Tuple[(int, int, int)]:
    ...
    return (
     brackets_normal, brackets_square, brackets_curly)


def find_nodes_of_type(root_node: parso.python.tree.Module, node_type: parso.python.tree.PythonBaseNode) -> List[parso.python.tree.PythonBaseNode]:
    """
