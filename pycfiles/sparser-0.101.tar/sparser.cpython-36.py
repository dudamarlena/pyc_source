...
# [GCC 8.4.0]
# Embedded file name: ../sparser/sparser.py
# Compiled at: 2017-06-18 18:39:09
# Size of source mod 2**32: 21240 bytes
...
OP_TOKENS = (
 OPENLOOP, CLOSELOOP, VAR, OPENCASE, CLOSECASE, OPENSWITCH, CLOSESWITCH, INCLUDE)
...
def _tokenize(raw, includes_dict):
    ...
    tokens = []
    while True:
        match = re.match('^(.*?)({{.*?}}|{\\*.*?\\*})(.*?)\\Z', remaining, re.DOTALL | re.MULTILINE)
        if not match:
            break
...
def _make_switch(tokens, ctx):
    ...
    return (
     tokens, Switch(switch_tokens, ctx))
    ...
    def __init__(self, tokens, ctx):
        ...
        while tokens:
            if isinstance(tokens[0], OPENLOOP):
                ...
                if isinstance(tokens[0], VAR):
                    token = tokens.pop(0)
                    all_members.append(Var([token], ctx))
                else:
                    if isinstance(tokens[0], TEXT):
                        token = tokens.pop(0)
...
class Loop(SIS):
    ...
    def __init__(self, tokens, ctx):
        ...
        :param SparserCompilationContext ctx:
        """
        if not isinstance(tokens[0], OPENLOOP):
            ...
            raise self.cases or SparserSyntaxError('{*loop*} tags must contain at least one {*case*}')
...
class Switch(SIS):
    ...
    def __init__(self, tokens, ctx):
        ...
        :param SparserCompilationContext ctx:
        """
        if not isinstance(tokens[0], OPENSWITCH):
            raise AssertionError
        else:
            ...
            raise self.cases or SparserSyntaxError('{*switch*} tags must contain at least one {*case*}')
    ...
    def translate(self):
        ...
        """
        return (
         '(?P<%s>.*?)' % self.switch_name, self.switch_name, self.cb)
...
class Var(SIS):
        ...
        if not len(tokens) == 1:
            ...
            if len(elements) == 2:
                ...
                if var_type in ctx.type_map:
                    ...
                    if QUOTE_HUGGED_STRING.match(var_type):
    ...
    def translate(self):
        ...
        """
        return (
         self.patt, self.var_name, self.cb)
...
def _main(args, out=sys.stdout):
    ...
    pattern_group.add_argument('--pattern-string',
      dest='pattern_string', help='The sparser pattern as a string')
    pattern_group.add_argument('--pattern-file',
      dest='pattern_file', help='The sparser pattern as a text file')
    input_group = arg_parser.add_mutually_exclusive_group(required=True)
    input_group.add_argument('--input-string',
      dest='input_string', help='The string to be matched against')
    input_group.add_argument('--input-file',
      dest='input_file', help='The string to be matched against as a text file')
