...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\GPS_Tools\loc_utils.py
...
def transform_to_gps(cart_point):
    ...
    proj = partial(pyproj.transform, pyproj.Proj(init='epsg:2180'), pyproj.Proj(init='epsg:4326'))
    out = transform(proj, point)
...
class GPS:
    __doc__ = 'Base class responsible for holding gps coordinates of single point\n    '
...
def get_photos_names_from_dir(images_path, extentions=[
 '*.JPG', '*.png']):
