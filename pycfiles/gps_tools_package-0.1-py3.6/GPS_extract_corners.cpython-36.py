...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\GPS_Tools\GPS_extract_corners.py
# Compiled at: 2019-04-12 01:16:34
# Size of source mod 2**32: 35223 bytes
import math, os, pdb
...
from GPS_Tools.loc_calculations import calculate_azimuth_points, calculate_point_geographic_coordinates
from GPS_Tools.loc_utils import GPS, deg_to_radians, get_photos_names_from_dir, radians_to_deg

class DroneCoordinateSystemQuadrant(Enum):
    __doc__ = 'Enum defining on which quadrant in regard to drone movement direction (azimuth), center image point is placed\n    '
    INVALID_STATUS = 1
    I_QUADRANT = 2
    II_QADRANT = 3
    III_QUADRANT = 4
    IV_QUADRANT = 5
    CENTER = 6
    ROLL_AXIS_NEGATIVE = 7
    ROLL_AXIS_POSITIVE = 8
    PITCH_AXIS_NEGATIVE = 9
    PITCH_AXIS_POSITIVE = 10
...
class RotationDataHolder:
    __doc__ = 'Class for holding rotation and azimuth\n    '
...
class GPSCornersHolder:
    __doc__ = 'Class holding GPS corners\n    '
...
class CameraParams:
    __doc__ = 'Static class holding information about camera durring flight\n    '
    sensorWidth = 12.8
    sensorHeight = 9.6
    focalLength = 8.8
    xResolution = 4864
...
class GPSDataManager(RotationDataHolder):
    __doc__ = 'Class holding gps whole information for one image\n    \n    :param RotationDataHolder: class holding data rotation\n    '
    ...
    def _calculate_image_center_azimuth_change(self, drone_azimuth):
        ...
        if status == DroneCoordinateSystemQuadrant.I_QUADRANT:
            ...
            if status == DroneCoordinateSystemQuadrant.II_QADRANT:
                image_center_from_drone_direction_angle = 360 - angle_absolute_value
            else:
                if status == DroneCoordinateSystemQuadrant.III_QUADRANT:
                    image_center_from_drone_direction_angle = 180 + angle_absolute_value
                else:
                    if status == DroneCoordinateSystemQuadrant.IV_QUADRANT:
                        image_center_from_drone_direction_angle = 180 - angle_absolute_value
                    else:
                        if status == DroneCoordinateSystemQuadrant.ROLL_AXIS_NEGATIVE:
                            image_center_from_drone_direction_angle = 270
                        else:
                            if status == DroneCoordinateSystemQuadrant.ROLL_AXIS_POSITIVE:
                                image_center_from_drone_direction_angle = 90
                            else:
                                if status == DroneCoordinateSystemQuadrant.PITCH_AXIS_POSITIVE:
                                    image_center_from_drone_direction_angle = 0
                                else:
                                    if status == DroneCoordinateSystemQuadrant.PITCH_AXIS_NEGATIVE:
                                        image_center_from_drone_direction_angle = 180
                                    else:
                                        if status == DroneCoordinateSystemQuadrant.CENTER:
                                            image_center_from_drone_direction_angle = 0
    ...
    def calculate_image_corner_coordinates(self):
        ...
        self.gps_corrners.right_up = calculate_point_geographic_coordinates(self._calculate_corner_azimuth(angle_to_right_up_corner), self.gps.latitude, self.gps.longitude, real_distace_to_corner)
        self.gps_corrners.right_down = calculate_point_geographic_coordinates(self._calculate_corner_azimuth(angle_to_right_downner), self.gps.latitude, self.gps.longitude, real_distace_to_corner)
        self.gps_corrners.left_down = calculate_point_geographic_coordinates(self._calculate_corner_azimuth(angle_to_left_downner), self.gps.latitude, self.gps.longitude, real_distace_to_corner)
        self.gps_corrners.left_up = calculate_point_geographic_coordinates(self._calculate_corner_azimuth(angle_to_left_up_corner), self.gps.latitude, self.gps.longitude, real_distace_to_corner)
        return self
    ...
    def calculate_fov_for_height(self, sensor_width, sensor_height, focal_length):
        ...
        return (
         self.horizontalFieldOfView, self.verticalFieldOfView)
...
class FlightGPSManipulator:
    ...
    def _convert_to_geoJSON(self, path, filename):
        ...
        for key, item in self.gps_dict.items():
            ...
            features.append(Feature(geometry=center, properties={'shapely_type':'Center',  'Azimuth':item.azimuth}))
    ...
    def _convert_to_native_geoJSON(self, path, filename, save=True):
        ...
        for key, item in self.gps_dict.items():
            ...
            features.append(Feature(geometry=center, properties={'shapely_type':'Center',  'Azimuth':item.azimuth, 
             'Filename':key, 
             'Name':key + '_Center'}))
            features.append(Feature(geometry=right_up, properties={'shapely_type':'Right_up',  'Name':key + '_Right_up'}))
            features.append(Feature(geometry=right_down, properties={'shapely_type':'Right_down',  'Name':key + '_Right_down'}))
            features.append(Feature(geometry=left_down, properties={'shapely_type':'Left_down',  'Name':key + '_Left_down'}))
            features.append(Feature(geometry=left_up, properties={'shapely_type':'Left_up',  'Name':key + '_Left_down'}))
            features.append(Feature(geometry=center_corr, properties={'shapely_type':'Center_corected',  'Name':key + 'Center_corected'}))
            polygon_corners = Polygon([
             [(item.gps_corrners.right_up.longitude, item.gps_corrners.right_up.latitude),
              (
               item.gps_corrners.right_down.longitude, item.gps_corrners.right_down.latitude),
              ...
              (
               item.gps_corrners.right_up.longitude, item.gps_corrners.right_up.latitude)]])
            features.append(Feature(geometry=polygon_corners, properties={'shapely_type':'Corner_polygon',  'Name':key + '_Corner_polygon'}))
    ...
    def save_corner_metadata(self, path, filename):
        ...
        """
        columns_list = [
         'Img_name', 'Center_lat', 'Center_lon', 'Right_up_lat', 'Right_up_lon', 'Right_down_lat', 'Right_down_lon', 'Left_down_lat', 'Left_down_lon', 'Left_up_lat', 'Left_up_lon', 'Azimuth', 'Center_lat_cor', 'Center_lon_cor']
...
def generate_corners_for_images_native_GEOJSON(gps_base_file, sensorWidth, sensorHeight, focalLength, xResolution, yResolution, calculate_azimuth=True):
    ...
    corners_data = CorGen.generate_corners_for_images_GEOJSON(r"D:\\Praca\\KSM\\Repos\\GPS_Tools\\data@m_data_pres\x818-05-08_original_deg0_100m_part1_wiadukt.txt", 12.8, 9.6, 8.8, 4864, 3648)
    corsers_data_test = CorGen.generate_corners_for_images(r"D:\\Praca\\KSM\\Repos\\GPS_Tools\\data@m_data_pres\x818-05-08_original_deg0_100m_part1_wiadukt.txt", 12.8, 9.6, 8.8, 4864, 3648)
