...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\GPS_Tools\GPS_danger_zone_calc.py
# Compiled at: 2019-04-12 01:16:34
# Size of source mod 2**32: 19567 bytes
from shapely.geometry import Polygon, mapping
import math, numpy as np, cv2, matplotlib.pyplot as plt, os, sys, pandas as pd, json, pdb
from GPS_Tools.loc_calculations import calculate_azimuth, calculate_azimuth_points, calculate_distance, calculate_distance_points, calculate_pix_meter_coef, calculate_pix_meter_coef_points, calculate_point_geographic_coordinates, calculate_side_point_azimuth, calculate_object_gps
from GPS_Tools.loc_utils import GPS, GPS_extended, deg_to_radians, get_photos_names_from_dir, radians_to_deg, transform_to_cartesian, transform_to_gps, visualize_image_group, create_polygon_from_gps_points
import geojson

class Piping:
    __doc__ = 'Class responsible for piping and global danger zone generation based on gps positions. Class offers functionality to\n    find common area between danger zone and images polygon.\n    \n    '
    ...
    def _calculate_gps_side_points(self, distance):
        ...
        for point, azim in zip(list(self.piping_raw_gps_points.values()), self.azimuths):
            right_point = calculate_point_geographic_coordinates(calculate_side_point_azimuth(right_absolute_angle, azim), point.latitude, point.longitude, distance)
            left_point = calculate_point_geographic_coordinates(calculate_side_point_azimuth(left_absolute_angle, azim), point.latitude, point.longitude, distance)
...
def generate_danger_zone(photo_gps, piping_data, resolution, visualize=False, danger_zone_dist=10):
    ...
    if visualize:
        data = [
         inter_sec, photo_gps.get_polygon()]
