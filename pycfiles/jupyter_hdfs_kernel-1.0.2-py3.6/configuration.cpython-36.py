...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/hdfs_kernel/utils/configuration.py
# Compiled at: 2020-01-16 03:10:17
# Size of source mod 2**32: 2804 bytes
import getpass, copy, sys, base64
...
def logging_config():
    return {'version':1, 
     'formatters':{'hdfsFormatter': {'format':'%(asctime)s\t%(levelname)s\t%(message)s', 
                        'datefmt':''}}, 
     'handlers':{'hdfsHandler': {'class':LOGGING_CONFIG_CLASS_NAME, 
                      'formatter':'hdfsFormatter', 
                      'home_path':HOME_PATH}}, 
     'loggers':{HDFS_LOGGER_NAME: {'handlers':[
                          'hdfsHandler'], 
                         'level':'WARNING', 
                         'propagate':0}}}
