...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\aux_functions.py
# Compiled at: 2020-03-23 15:40:39
# Size of source mod 2**32: 19508 bytes
"""
...
import time as time_package, os
    ...
    if not bool_val:
        ...
        for inval_key in inv_keys:
            ...
            if warn_print is True:
                warn((RuntimeWarning('At least one of the specified kwargs is not applied in numpy.loadtxt function' + str_inval_keys)),
                  stacklevel=2)
                break
...
def _is_a_url(input_element):
    ...
    if type(input_element) is str:
        if any((mark in input_element for mark in ('http://', 'https://', 'www.', '.pt',
                                                   '.com', '.org', '.net'))):
            return True
        return False
    else:
