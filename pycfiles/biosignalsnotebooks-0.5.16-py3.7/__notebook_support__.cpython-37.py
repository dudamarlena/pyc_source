...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\__notebook_support__.py
# Compiled at: 2020-03-31 07:47:47
# Size of source mod 2**32: 99591 bytes
"""
...
/"""
import os.path as pathexist
import numpy, requests, math, os
...
def plot_sample_rate_compare(data_dict, file_name=None):
    ...
    list_figures = plot(([[]] * nbr_rows * 2), ([[]] * nbr_rows * 2), y_axis_label='Raw Data', x_axis_label='Time (s)',
      grid_lines=nbr_rows,
      grid_columns=2,
      grid_plot=True,
      get_fig_list=True,
    ...
    for iter, sample_rate in enumerate(list(data_dict.keys())):
        ...
        (list_figures[(2 * iter)].line)((data_dict[sample_rate]['time'][:10 * int(sample_rate)]), 
         (data_dict[sample_rate]['data'][:10 * int(sample_rate)]), **opensignals_kwargs('line'))
        ...
        (list_figures[(2 * iter + 1)].line)((data_dict[sample_rate]['time'][:int(sample_rate)]), 
         (data_dict[sample_rate]['data'][:int(sample_rate)]), **opensignals_kwargs('line'))
    ...
    for iter_nbr, sample_rate in enumerate(['4000', '3000', '1000', '500', '200', '100']):
        if not iter_nbr == 0:
            ...
            (list_figures[(-1)][(-1)].line)((data_interp[sample_rate]['time'][:int(sample_rate)]), 
             (data_interp[sample_rate]['data'][:int(sample_rate)]), **opensignals_kwargs('line'))
...
def plot_ecg_pan_tompkins_steps(time, orig_ecg, pre_process_ecg, sampling_rate, titles):
    ...
    if len(titles) == 2:
        list_figures_1 = [[]]
        list_figures_1[(-1)].append(figure(x_axis_label='Time (s)', y_axis_label='Raw Data', title=titles[0], **opensignals_kwargs('figure')))
        (list_figures_1[(-1)][(-1)].line)((time[:sampling_rate]), (orig_ecg[:sampling_rate]), **opensignals_kwargs('line'))
        list_figures_1[(-1)].append(figure(x_axis_label='Time (s)', y_axis_label='Raw Data', title=titles[1], **opensignals_kwargs('figure')))
...
def plot_ecg_pan_tompkins_peaks(time, orig_ecg, integrated_ecg, sampling_rate, possible_peaks, probable_peaks, definitive_peaks):
    """
    -----
    Brief
    -----
...
def plot_emg_graphical_statistical(time, signal, max_sample_value, min_sample_value, avg_sample_value, std_sample_value, y_range=[
 -1.1, 1]):
    """
    -----
    Brief
    ...
    """
    list_figures = []
    list_figures.append(figure(x_axis_label='Time (s)', y_axis_label='Electric Tension (mV)', x_range=(0, time[(-1)] + 0.5 * time[(-1)]), y_range=y_range, **opensignals_kwargs('figure')))
    ...
    for parameter in parameter_list:
        ...
        if parameter == 'Maximum':
            list_figures[(-1)].circle(find_time_max, max_sample_value, radius=0.5, fill_color=(opensignals_color_pallet()), legend_label=(parameter + ' EMG'))
        elif parameter == 'Minimum':
            list_figures[(-1)].circle(find_time_min, min_sample_value, radius=0.5, fill_color=(opensignals_color_pallet()), legend_label=(parameter + ' EMG'))
        elif parameter == 'Average':
            (list_figures[(-1)].line)([0, time[(-1)]], [avg_sample_value, avg_sample_value], legend_label=parameter + ' EMG Sample', **opensignals_kwargs('line'))
        elif parameter == 'Standard Deviation':
            box_annotation = BoxAnnotation(left=0, right=(time[(-1)]), top=(avg_sample_value + std_sample_value), bottom=(avg_sample_value - std_sample_value),
              fill_color='black',
              fill_alpha=0.3)
            list_figures[(-1)].rect(find_time_min, std_sample_value, width=0, height=0, fill_color='black', fill_alpha=0.3, legend_label='Average + Standard Deviation Zone')
...
def plot_emg_rms_area(time, signal, rms, area):
    ...
    """
    list_figures = []
    list_figures.append(figure(x_axis_label='Frequency (Hz)', y_axis_label='Electric Tension (mV)', x_range=[0, time[(-1)]], y_range=[
 -1, 1], **opensignals_kwargs('figure')))
    ...
    (list_figures[(-1)].line)(time[1:], area, legend_label='Area along time (cumulative)', y_range_name='Area', **opensignals_kwargs('line'))
    opensignals_style(list_figures)
    show(list_figures[(-1)])
...
def plot_emg_spect_freq(freq_axis, power_axis, max_freq, median_freq):
    ...
    list_figures[(-1)].patch((list(freq_axis) + list(freq_axis)[::-1]), (list(power_axis) + list(numpy.zeros(len(power_axis)))), fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='Area Under Curve')
    (list_figures[(-1)].line)([median_freq, median_freq], [0, power_axis[numpy.where(freq_axis == median_freq)[0][0]]], legend_label='Median Frequency', **opensignals_kwargs('line'))
    (list_figures[(-1)].line)([max_freq, max_freq], [0, power_axis[numpy.where(freq_axis == max_freq)[0][0]]], legend_label='Maximum Power Frequency', **opensignals_kwargs('line'))
    opensignals_style(list_figures)
    show(list_figures[(-1)])


def plot_eeg_signal_wind(time, signal, time_range, time_windows_evt_1, time_windows_evt_2, y_axis_label='Electric Voltage', legend=['Eyes Opened', 'Eyes Closed']):
    """
    -----
    Brief
    -----
    ...
    for i in range(0, len(eyes_open_begin)):
        box_annotations.append(BoxAnnotation(left=(eyes_open_begin[i]), right=(eyes_open_end[i]), fill_color=color_open_eyes,
          fill_alpha=0.1))
    ...
    if len(time_windows_evt_2) != 0:
        for i in range(0, len(eyes_closed_begin)):
            box_annotations.append(BoxAnnotation(left=(eyes_closed_begin[i]), right=(eyes_closed_end[i]), fill_color=color_closed_eyes, fill_alpha=0.1))
            list_figures[(-1)].add_layout(box_annotations[(-1)])
...
def plot_eeg_alpha_band(freq_axis_evt1, power_axis_evt1, freq_axis_evt2, power_axis_evt2, freq_range):
    ...
    list_figures = plot([freq_axis_eyes_closed, freq_axis_eyes_opened], [
     power_spect_eyes_closed, power_spect_eyes_opened],
      title=[
     'Estimated Power Spectral Density - Closed Eyes',
     'Estimated Power Spectral Density - Opened Eyes'],
      legend_label=[
     'Bandpass Filtered Freq. Band', 'Bandpass Filtered Freq. Band'],
      x_axis_label='Frequency (Hz)',
      y_axis_label='PSD (uV^2/Hz)',
      grid_plot=True,
      grid_lines=1,
      grid_columns=2,
      x_range=freq_range,
      show_plot=False,
      get_fig_list=True)
    alpha_band_indexes_eyes_closed = numpy.where((numpy.array(freq_axis_eyes_closed) >= 8) & (numpy.array(freq_axis_eyes_closed) <= 12))[0]
    alpha_band_indexes_eyes_opened = numpy.where((numpy.array(freq_axis_eyes_opened) >= 8) & (numpy.array(freq_axis_eyes_opened) <= 12))[0]
    ...
    list_figures[0].patch((list(freq_axis_eyes_closed[alpha_band_indexes_eyes_closed]) + list(freq_axis_eyes_closed[alpha_band_indexes_eyes_closed])[::-1]),
      (list(power_spect_eyes_closed[alpha_band_indexes_eyes_closed]) + list(numpy.zeros(len(power_spect_eyes_closed[alpha_band_indexes_eyes_closed])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='Alpha Band 8-12 Hz')
    list_figures[1].y_range = Range1d(0, 1.2 * maxPower)
    list_figures[1].patch((list(freq_axis_eyes_opened[alpha_band_indexes_eyes_opened]) + list(freq_axis_eyes_opened[alpha_band_indexes_eyes_opened])[::-1]),
      (list(power_spect_eyes_opened[alpha_band_indexes_eyes_opened]) + list(numpy.zeros(len(power_spect_eyes_opened[alpha_band_indexes_eyes_opened])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='Alpha Band 8-12 Hz')
...
def plot_post_ecto_rem_tachogram(tachogram_time, tachogram_data, tachogram_time_nn, tachogram_data_nn):
    ...
    """
    list_figures = []
    list_figures.append(figure(x_axis_label='Time (s)', y_axis_label='Cardiac Cycle (s)', x_range=(0, tachogram_time[(-1)]), **opensignals_kwargs('figure')))
    (list_figures[(-1)].line)(tachogram_time, tachogram_data, legend_label='Original Tachogram', **opensignals_kwargs('line'))
    (list_figures[(-1)].line)(tachogram_time_nn, tachogram_data_nn, legend_label='Post Ectopy Removal Tachogram', line_dash='dashed', **opensignals_kwargs('line'))
    opensignals_style(list_figures)
    show(list_figures[(-1)])
...
def plot_hrv_parameters(tachogram_time, tachogram_data, time_param_dict):
    ...
    list_figures.append(figure(x_axis_label='Time (s)', y_axis_label='Cardiac Cycle (s)', x_range=(0, tachogram_time[(-1)] + 0.3 * tachogram_time[(-1)]), y_range=(0.6, 1), **opensignals_kwargs('figure')))
    (list_figures[(-1)].line)(tachogram_time, tachogram_data, legend_label='Original Tachogram', **opensignals_kwargs('line'))
    ...
    (list_figures[(-1)].line)(tachogram_time, bpm_data, legend_label='Heart Rate (BPM)', y_range_name='BPM', **opensignals_kwargs('line'))
    dict_keys = time_param_dict.keys()
    for key in dict_keys:
        if not 'Maximum' in key:
            if 'Minimum' in key:
                if 'BPM' not in key:
                    find_time = tachogram_time[numpy.where(tachogram_data == time_param_dict[key])[0][0]]
                    list_figures[(-1)].circle(find_time, (time_param_dict[key]), radius=5, fill_color=(opensignals_color_pallet()),
                      legend_label=key)
            if 'Maximum' in key or 'Minimum' in key:
                if 'BPM' in key:
            ...
            if 'Average' in key:
                if 'BPM' not in key:
                    (list_figures[(-1)].line)([0, tachogram_time[(-1)]], [time_param_dict[key], time_param_dict[key]], legend_label='Average RR', **opensignals_kwargs('line'))
            if 'SDNN' in key:
                box_annotation = BoxAnnotation(left=0, right=(tachogram_time[(-1)]), top=(time_param_dict['Average RR'] + time_param_dict['SDNN']), bottom=(time_param_dict['Average RR'] - time_param_dict['SDNN']),
                  fill_color='black',
                  fill_alpha=0.1)
                list_figures[(-1)].rect(find_time, (time_param_dict[key]), width=0, height=0, fill_color='black', fill_alpha=0.1,
                  legend_label='SDNN')
                list_figures[(-1)].add_layout(box_annotation)
...
def plot_hrv_power_bands(freqs, power_spect):
    ...
    freq_bands = {'ulf_band':[
      0.0, 0.003], 
     'vlf_band':[0.003, 0.04],  'lf_band':[0.04, 0.15],  'hf_band':[
      0.15, 0.4]}
    power_band = {}
    total_power = 0
    ...
    for band in band_keys:
        ...
        freq_samples_inside_band = [freq for freq in freqs if freq >= freq_band[0] if freq <= freq_band[1]]
        power_samples_inside_band = [p for p, freq in zip(power_spect, freqs) if freq >= freq_band[0] if freq <= freq_band[1]]
    ...
    list_figures = []
    list_figures.append(figure(x_axis_label='Frequency (Hz)', y_axis_label='Power Spectral Density (s² / Hz)', **opensignals_kwargs('figure')))
    (list_figures[(-1)].line)(freqs, power_spect, **opensignals_kwargs('line'))
    list_figures[(-1)].patch((power_band['ulf_band']['Freqs'] + power_band['ulf_band']['Freqs'][::-1]), (power_band['ulf_band']['Power'] + list(numpy.zeros(len(power_band['ulf_band']['Power'])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='ULF Band')
    list_figures[(-1)].patch((power_band['vlf_band']['Freqs'] + power_band['vlf_band']['Freqs'][::-1]), (power_band['vlf_band']['Power'] + list(numpy.zeros(len(power_band['vlf_band']['Power'])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='VLF Band')
    list_figures[(-1)].patch((power_band['lf_band']['Freqs'] + power_band['lf_band']['Freqs'][::-1]), (power_band['lf_band']['Power'] + list(numpy.zeros(len(power_band['lf_band']['Power'])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='LF Band')
    list_figures[(-1)].patch((power_band['hf_band']['Freqs'] + power_band['hf_band']['Freqs'][::-1]), (power_band['hf_band']['Power'] + list(numpy.zeros(len(power_band['hf_band']['Power'])))),
      fill_color=(opensignals_color_pallet()),
      fill_alpha=0.5,
      line_alpha=0,
      legend_label='HF Band')
    opensignals_style(list_figures)
    show(list_figures[(-1)])
...
def plot_compare_act_config(signal, sample_rate, file_name=None):
    ...
    muscular_activation_begin_20_10, muscular_activation_end_20_10, smooth_signal_20_10, threshold_level_20_10 = detect_emg_activations((list(signal)), sample_rate, smooth_level=20, threshold_level=10)
    threshold_level_20_10 = numpy.max(signal) * threshold_level_20_10 / numpy.max(smooth_signal_20_10)
    smooth_signal_20_10 = numpy.max(signal) * numpy.array(smooth_signal_20_10) / numpy.max(smooth_signal_20_10)
    activation_20_10 = numpy.zeros(len(time))
    for activation in range(0, len(muscular_activation_begin_20_10)):
        activation_20_10[muscular_activation_begin_20_10[activation]:muscular_activation_end_20_10[activation]] = numpy.max(signal)

    muscular_activation_begin_20_50, muscular_activation_end_20_50, smooth_signal_20_50, threshold_level_20_50 = detect_emg_activations((list(signal)), sample_rate, smooth_level=20, threshold_level=50)
    threshold_level_20_50 = numpy.max(signal) * threshold_level_20_50 / numpy.max(smooth_signal_20_50)
    smooth_signal_20_50 = numpy.max(signal) * numpy.array(smooth_signal_20_50) / numpy.max(smooth_signal_20_50)
    activation_20_50 = numpy.zeros(len(time))
    for activation in range(0, len(muscular_activation_begin_20_50)):
        activation_20_50[muscular_activation_begin_20_50[activation]:muscular_activation_end_20_50[activation]] = numpy.max(signal)

    muscular_activation_begin_70_10, muscular_activation_end_70_10, smooth_signal_70_10, threshold_level_70_10 = detect_emg_activations((list(signal)), sample_rate, smooth_level=70, threshold_level=10)
    threshold_level_70_10 = numpy.max(signal) * threshold_level_70_10 / numpy.max(smooth_signal_70_10)
    ...
    for activation in range(0, len(muscular_activation_begin_70_10)):
        activation_70_10[muscular_activation_begin_70_10[activation]:muscular_activation_end_70_10[activation]] = numpy.max(signal)
    ...
    plot_list = plot([list([]), list([]), list([]), list([]), list([]), list([])], [
     list([]), list([]), list([]), list([]), list([]), list([])],
      ...
      show_plot=False)
    combination = [
     'Smooth Level: 20 %  Threshold Level: 10 %', 'Smooth Level: 20 %  Threshold Level: 50 %',
    ...
    detect_dict = {'Smooth Level: 20 %  Threshold Level: 10 %':[smooth_signal_20_10,
      ...
      activation_20_10], 
     'Smooth Level: 20 %  Threshold Level: 50 %':[
      smooth_signal_20_50,
    ...
    for plot_aux in range(0, len(plot_list)):
        ...
        if plot_aux % 2 == 0:
            ...
            (plot_list[plot_aux].line)(time, (detect_dict[combination_temp][0]), **opensignals_kwargs('line'))
            (plot_list[plot_aux].line)(time, (detect_dict[combination_temp][1]), **opensignals_kwargs('line'))
            plot_list[plot_aux].yaxis.axis_label = 'Raw Data'
        else:
            title.text = 'Result for ' + combination_temp
            (plot_list[plot_aux].line)(time, (detect_dict[combination_temp][2]), **opensignals_kwargs('line'))
        if plot_aux in (4, 5):
            plot_list[plot_aux].xaxis.axis_label = 'Time (s)'
        plot_list[plot_aux].title = title

    grid_plot_ref = gridplot(
...
def plot_median_freq_evol(time_signal, signal, time_median_freq, median_freq, activations_begin, activations_end, sample_rate, file_name=None):
    """
    -----
    Brief
    -----
    ...
    list_figures_1 = plot([list(time_signal), list(time_median_freq)], [
     list(signal), list(median_freq)],
      title=[
     'EMG Acquisition highlighting muscular activations',
     'Median Frequency Evolution'],
      grid_plot=True,
      grid_lines=2,
      grid_columns=1,
      open_signals_style=True,
      x_axis_label='Time (s)',
      yAxisLabel=[
     'Raw Data', 'Median Frequency (Hz)'],
      x_range=[
     0, 125],
      get_fig_list=True,
      show_plot=False)
    for activation in range(0, len(activations_begin)):
        ...
        box_annotation = BoxAnnotation(left=(activations_begin[activation] / sample_rate), right=(activations_end[activation] / sample_rate),
          fill_color=color,
          fill_alpha=0.1)
        box_annotation_copy = BoxAnnotation(left=(activations_begin[activation] / sample_rate), right=(activations_end[activation] / sample_rate),
          fill_color=color,
          fill_alpha=0.1)
...
def plot_informational_band(freqs, power, signal, sr, band_begin, band_end, legend='Signal Power Spectrum', x_lim=[], y_lim=[], show_plot=False, file_name=None):
    """
    -----
    Brief
    -----
    ...
    if type(x_lim) is list:
        if type(y_lim) is list:
            ...
            raise RuntimeError("The inputs arguments 'x_lim' and 'y_lim', when explicitly specified, must be formed by two elements (defining the lower and upper limits of the x and y axis).")
    else:
        raise RuntimeError('At least one of the input arguments (x_lim or y_lim) does not have a valid type. The inputs must be lists.')
    list_figures = []
    list_figures.append(figure(x_axis_label='Frequency (Hz)', y_axis_label='Relative Weight', x_range=(x_lim[0], x_lim[(-1)]), y_range=(
 y_lim[0], y_lim[1]), **opensignals_kwargs('figure')))
    (list_figures[(-1)].line)(freqs, power, legend_label=legend_label, **opensignals_kwargs('line'))
    color = opensignals_color_pallet()
    box_annotation = BoxAnnotation(left=band_begin, right=band_end, fill_color=color, fill_alpha=0.1)
    list_figures[(-1)].circle([-100], [0], fill_color=color, fill_alpha=0.1, legend_label='Informational Band')
    list_figures[(-1)].add_layout(box_annotation)
...
def plot_before_after_filter(signal, sr, band_begin, band_end, order=1, x_lim=[], y_lim=[], orientation='hor', show_plot=False, file_name=None):
    """
    -----
    Brief
    -----
    ...
    for i in range(0, order + 1):
        if i == 0:
            ...
            figure_after = plot_informational_band(freqs_after, power_after, signal, sr, band_begin,
              band_end, legend='Signal Power Spectrum',
              x_lim=x_lim,
              y_lim=y_lim)
            list_figures = [
             [
            ...
            if orientation != 'same':
                figure_after = plot_informational_band(freqs_after, power_after, filter_signal, sr, band_begin,
                  band_end, legend=('Filtered FFT (Order ' + str(i) + ')'),
                  x_lim=x_lim,
                  y_lim=y_lim)
                if orientation == 'hor':
                ...
                (list_figures[(-1)][0].line)(freqs_after, power_after, legend_label='Filtered FFT (Order ' + str(i) + ')', **opensignals_kwargs('line'))
...
def plot_low_pass_filter_response(show_plot=False, file_name=None):
    ...
    fig_list = plot(([freqs / cutoff_freq] * len(gain_functions)), gain_functions, legend_label=legend_strs, title='Filter Response',
      ...
      get_fig_list=True)
    color = opensignals_color_pallet()
    box_annotation = BoxAnnotation(left=0.1, right=1, top=0, bottom=(-120), fill_color=color,
      fill_alpha=0.3)
...
def plot_ecg_tachogram(time, signal, tachogram_time, tachogram, time_r_peaks, show_plot=True):
    ...
    """
    list_figures_1 = [[], []]
    list_figures_1[0].append(figure(x_axis_label='Time (s)', y_axis_label='Cardiac Cycle (s)', title='Tachogram', x_range=(0, time[(-1)]), **opensignals_kwargs('figure')))
    (list_figures_1[0][(-1)].line)(tachogram_time, tachogram, **opensignals_kwargs('line'))
    list_figures_1[1].append(figure(x_axis_label='Time (s)', y_axis_label='Raw Data', title='ECG Acquisition', x_range=(0, time[(-1)]), **opensignals_kwargs('figure')))
...
def plot_poincare(tachogram_data, show_plot=True):
    ...
    list_figures[(-1)].line([numpy.min(rr_i), numpy.max(rr_i)], [numpy.min(rr_i_plus_1), numpy.max(rr_i_plus_1)], line_color='black',
      line_dash='dashed')
    list_figures[(-1)].line([numpy.min(rr_i), numpy.max(rr_i)], [
     -numpy.min(rr_i) + 2 * avg_rr, -numpy.max(rr_i) + 2 * avg_rr],
      line_color='black', line_dash='dashed')
    list_figures[(-1)].add_layout(Arrow(start=VeeHead(size=15, line_color=color_1, fill_color=color_1), end=VeeHead(size=15, line_color=color_1, fill_color=color_1),
      ...
      y_end=(avg_rr + SD2 * math.cos(math.radians(45))),
      line_color=color_1,
      line_width=3))
    list_figures[(-1)].add_layout(Arrow(start=VeeHead(size=15, line_color=color_2, fill_color=color_2), end=VeeHead(size=15, line_color=color_2, fill_color=color_2),
      ...
      y_end=(avg_rr - SD1 * math.cos(math.radians(45))),
      line_color=color_2,
      line_width=3))
...
def plot_compare_resolutions(time, signal_res_1, signal_res_2, sampling_rate, y_axis_label='Temperature (ºC)', time_start=110, time_end=150, y_range=[38, 40.5]):
    """
    -----
    Brief
    -----
    ...
    wind_start = int(sampling_rate * time_start)
    wind_end = int(sampling_rate * time_end)
    figure_8 = figure(x_axis_label='Time (s)', y_axis_label=y_axis_label, title='8 Bits Acquisition', y_range=y_range, **opensignals_kwargs('figure'))
    (figure_8.line)((time[wind_start:wind_end]), (signal_res_1[wind_start:wind_end]), **opensignals_kwargs('line'))
    figure_16 = figure(x_axis_label='Time (s)', y_axis_label=y_axis_label, title='16 Bits Acquisition', y_range=y_range, **opensignals_kwargs('figure'))
    (figure_16.line)((time[wind_start:wind_end]), (signal_res_2[wind_start:wind_end]), **opensignals_kwargs('line'))
...
def plot_resp_slow(signal, rect_signal, sample_rate):
    ...
    for inhal_exhal in range(0, len(inhal_begin)):
        if inhal_exhal == 0:
            ...
            legend_label = [
             None, None]
        plot_aux.line((time[inhal_begin[inhal_exhal]:inhal_end[inhal_exhal]]), (rect_signal_rev[inhal_begin[inhal_exhal]:inhal_end[inhal_exhal]]),
          line_width=2,
          line_color=inhal_color,
          legend_label=(legend_label[0]))
        if inhal_exhal != len(inhal_begin) - 1:
            plot_aux.line((time[exhal_begin[inhal_exhal]:exhal_end[inhal_exhal]]), (rect_signal_rev[exhal_begin[inhal_exhal]:exhal_end[inhal_exhal]]),
              line_width=2,
              line_color=exhal_color,
              legend_label=(legend_label[1]))
        else:
            plot_aux.line((time[exhal_begin[inhal_exhal]:]), (rect_signal_rev[exhal_begin[inhal_exhal]:]), line_width=2,
              line_color=exhal_color,
              legend_label=(legend_label[1]))
...
def plot_resp_diff(signal, rect_signal, sample_rate):
    ...
    figure_list = plot([list([0]), list([0]), list([0])], [
     list([0]), list([0]), list([0])],
      gridPlot=True, gridLines=3, gridColumns=1,
      showPlot=False)
    title = Title()
    ...
    _inhal_exhal_segments(figure_list[0], list(time), list(rect_signal_rev), inhal_begin, inhal_end, exhal_begin, exhal_end)
    ...
    _inhal_exhal_segments(figure_list[1], list(time), list(scaled_rect_signal), inhal_begin, inhal_end, exhal_begin, exhal_end)
    ...
    _inhal_exhal_segments(figure_list[2], list(time), list(norm_rect_signal), inhal_begin, inhal_end, exhal_begin, exhal_end)
    figure_list[2].yaxis.axis_label = 'Normalized Data'
    figure_list[2].xaxis.axis_label = 'Time (s)'
    grid_plot_ref = gridplot([[figure_list[0]], [figure_list[1]], [figure_list[2]]], **opensignals_kwargs('gridplot'))
    show(grid_plot_ref)
...
def plot_simple_threshold_algorithm(time, signal_smooth, binary_signal, sr, thresh_1, thresh_2):
    ...
    for index, i in enumerate(signal_smooth):
        if i >= thresh_1:
            if signal_smooth[(index - 1)] < thresh_1:
                intersect_on.append(thresh_1)
                intersect_index_on.append(index)
        if i <= thresh_2 and signal_smooth[(index - 1)] > thresh_2:
    ...
    fig_list = plot([list(time), list(time), list(time), list(time)], [
     ...
     list(thresh2)],
      legend_label=['Activation Signal', 'Smoothed Signal', 'Threshold 1', 'Threshold 2'], grid_plot=False,
      opensignals_style=True,
      x_axis_label='Time (s)',
      y_axis_label='RAW Data Samples',
      x_range=(11, 13),
      get_fig_list=True,
      show_plot=False)
    fig_list[0].circle((numpy.array(intersect_index_on) / sr), intersect_on, size=10, color='yellowgreen', legend_label='Onset')
    fig_list[0].circle((numpy.array(intersect_index_off) / sr), intersect_off, size=10, color='darkorange', legend_label='Offset')
    show(fig_list[0])
...
def _inhal_exhal_segments(fig, time, signal, inhal_begin, inhal_end, exhal_begin, exhal_end):
    ...
    for inhal_exhal in range(0, len(inhal_begin)):
        if inhal_exhal == 0:
            legend_label = [
             'Respiration Suspension', 'Normal Breath']
        else:
            legend_label = [
             None, None]
        fig.line((time[inhal_begin[inhal_exhal]:inhal_end[inhal_exhal]]), (signal[inhal_begin[inhal_exhal]:inhal_end[inhal_exhal]]),
          line_width=2, line_color=inhal_color,
          legend_label=(legend_label[0]))
        if inhal_exhal != len(inhal_begin) - 1:
            fig.line((time[exhal_begin[inhal_exhal]:exhal_end[inhal_exhal]]), (signal[exhal_begin[inhal_exhal]:exhal_end[inhal_exhal]]),
              line_width=2, line_color=exhal_color,
              legend_label=(legend_label[1]))
            if inhal_exhal == 0:
                fig.line((time[:inhal_begin[inhal_exhal]]), (signal[:inhal_begin[inhal_exhal]]), line_width=2,
                  line_color=exhal_color,
                  legend_label=(legend_label[1]))
        else:
            fig.line((time[exhal_begin[inhal_exhal]:]), (signal[exhal_begin[inhal_exhal]:]), line_width=2,
              line_color=exhal_color,
              legend_label=(legend_label[1]))