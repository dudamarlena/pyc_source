...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\detect.py
# Compiled at: 2020-03-31 07:47:47
# Size of source mod 2**32: 28033 bytes
"""
...
def detect_r_peaks(ecg_signal, sample_rate, time_units=False, volts=False, resolution=None, device='biosignalsplux', plot_result=False):
    ...
    if plot_result is True:
        ...
        fig.circle((time_int[definitive_peaks]), (integrated[definitive_peaks]), size=30, color='#00893E',
          legend_label='Definitive Peaks')
        fig.circle((time_int[probable_peaks]), (integrated[probable_peaks]), size=20, color='#009EE3', legend_label='Probable Peaks')
        fig.circle((time_int[possible_peaks]), (integrated[possible_peaks]), size=10, color='#302683', legend_label='Possible Peaks')
        ...
        fig2.circle((time[definitive_peaks_rephase]), (numpy.array(ecg_signal)[definitive_peaks_rephase]),
          size=30,
...
def detect_emg_activations(emg_signal, sample_rate, smooth_level=20, threshold_level=10, time_units=False, volts=False, resolution=None, device='biosignalsplux', plot_result=False):
    """
    -----
    Brief
    -----
    ...
    for i, signal_sample in enumerate(pre_pro_signal):
        if i in (0, len(pre_pro_signal) - 1):
            ...
            tkeo.append(numpy.power(signal_sample, 2) - pre_pro_signal[(i + 1)] * pre_pro_signal[(i - 1)])
    ...
    threshold_level = avg_pre_pro_signal + _thres_norm_reg(threshold_level, smooth_signal, pre_pro_signal) * std_pre_pro_signal
    binary_signal = []
    ...
    if time_units is True:
        ...
        time_end = act_end
    if plot_result is True:
        plot([list(time), list(time)], [list(emg_signal),
         list(numpy.array(binary_signal) * numpy.max(emg_signal))],
          yAxisLabel=([
         'Data Samples (' + units + ')'] * 2),
          x_axis_label=time_units_str,
          legend_label=['EMG Signal', 'Activation Signal'])
...
def _ecg_band_pass_filter(data, sample_rate):
    ...
    out : list
        Filtered signal.
    """
    nyquist_sample_rate = sample_rate / 2.0
...
def _integration(data, sample_rate):
    ...
    """
    wind_size = int(0.08 * sample_rate)
...
def _detects_peaks(ecg_integrated, sample_rate):
    ...
    possible_peaks = [i for i in range(0, len(ecg_integrated) - 1) if ecg_integrated[(i - 1)] < ecg_integrated[i] if ecg_integrated[i] > ecg_integrated[(i + 1)]]
    ...
    for peak_i, peak_amp in zip(possible_peaks, possible_amplitudes):
        if peak_i - peak_candidate_i <= min_rr:
            if peak_amp > peak_candidate_amp:
                peak_candidate_i = peak_i
                peak_candidate_amp = peak_amp
        if peak_i - peak_candidate_i > min_rr:
            chosen_peaks += [peak_candidate_i - 6]
            peak_candidate_i = peak_i
            peak_candidate_amp = peak_amp
            continue

    return (
     chosen_peaks, possible_peaks)
...
def _checkup(peaks, ecg_integrated, sample_rate, rr_buffer, spk1, npk1, threshold):
    ...
    for i, peak in enumerate(peaks):
        amp = peaks_amp[i]
        if amp > threshold:
            definitive_peaks, spk1, rr_buffer = _acceptpeak(peak, amp, definitive_peaks, spk1, rr_buffer)
        else:
            if amp > threshold / 2:
                if list(definitive_peaks) and len(peaks) > i + 1:
...
def tachogram(data, sample_rate, signal=False, in_seconds=False, out_seconds=False):
    ...
    if signal is False:
        ...
        data_copy = detect_r_peaks(data, sample_rate, time_units=out_seconds, volts=False, resolution=None,
          plot_result=False)[0]
        time_axis = data_copy
