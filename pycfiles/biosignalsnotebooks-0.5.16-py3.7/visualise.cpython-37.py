...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\visualise.py
# Compiled at: 2020-04-03 09:52:59
# Size of source mod 2**32: 43640 bytes
"""
...
COLOR_LIST = itertools.cycle(('#009EE3', '#302683', '#00893E', '#94C11E', '#FDC400',
                              '#E84D0E', '#CF0272', '#F199C1'))

def _plot_future(time, data, legend_label=None, title=None, y_axis_label=None, hor_lines=None, hor_lines_leg=None, vert_lines=None, vert_lines_leg=None, apply_opensignals_style=True, show_plot=True, warn_print=False, **kwargs):
    """
    Plotting function intended for an easy representation of OpenSignals acquired data.
    ...
    **kwargs : dict
    ...
    if apply_opensignals_style is True:
        style_figure = {**(opensignals_kwargs('figure')), **(_filter_keywords(FigureOptions, kwargs, is_class=True,
          warn_print=warn_print))}
        style_line = {**(opensignals_kwargs('line')), **(_filter_keywords(Line, kwargs, warn_print=warn_print))}
        style_gridplot = {**(opensignals_kwargs('gridplot')), **(_filter_keywords(gridplot, kwargs, warn_print=warn_print))}
    else:
    ...
    if len(list(numpy.shape(data))) == 3 and len(list(numpy.shape(time))) == 3:
        ...
        if len(list(numpy.shape(data))) == 1:
            if len(list(numpy.shape(time))) == 1:
                grid_plot = False
            else:
                raise RuntimeError("'time' and 'data' fields must have the same shape, which would be a list with 1 dimension or a list of lists with 3 levels, such as [[[time_0_0, time_0,1, time_0_2], [time_1_0, time_1_1, time_1_2]]]. In the previous example the output will be a gridplot with 2 rows and three columns.")
        else:
            ...
            y_axis_label = _check_validity_of_inputs(data, y_axis_label, 'y_axis_label', grid_plot, dimension=2)
            hor_lines = _check_validity_of_inputs(data, hor_lines, 'hor_lines', grid_plot, dimension=2)
            hor_lines_leg = _check_validity_of_inputs(data, hor_lines_leg, 'hor_lines_leg', grid_plot, dimension=2)
            vert_lines = _check_validity_of_inputs(data, vert_lines, 'vert_lines', grid_plot, dimension=2)
            vert_lines_leg = _check_validity_of_inputs(data, vert_lines_leg, 'vert_lines_leg', grid_plot, dimension=2)
            fig_list = []
                    ...
                    for row in range(0, len(data)):
                        ...
                        for column in range(0, len(data[row])):
                            for plt in range(0, len(data[row][column])):
                                if _is_instance(Number, (data[row][column][plt]), condition='all', deep=True) and not _is_instance(bool, (data[row][column][plt]), condition='any'):
                                    if _is_instance(Number, (time[row][column][0]), condition='all'):
                                        if not _is_instance(bool, (time[row][column][0]), condition='any'):
                                            fig_list.append([])
                                            fig_list[(-1)][(-1)].append(figure(title=title[row][column][0], y_axis_label=y_axis_label[row][column][0], **style_figure))
                                            (fig_list[(-1)][(-1)][(-1)].line)(time[row][column][0], data[row][column][plt], legend_label=legend_label[row][column][plt], **style_line)
                                    raise RuntimeError('At least one of the list elements, specified in data or time, is not numeric.')

                            if hor_lines is not None:
                                for hor_line_nbr, hor_line in enumerate(hor_lines[row][column]):
                                    if hor_lines_leg is not None:
                                        (fig_list[(-1)][(-1)][(-1)].line)([time[row][column][0], time[row][column][(-1)]],
 [
  hor_line, hor_line], legend_label=hor_lines_leg[row][hor_line_nbr], **opensignals_kwargs('line'))
                                    else:
                                        (fig_list[(-1)][(-1)][(-1)].line)(
                                         [time[row][column][0],
                                          time[row][column][(-1)]], 
                                         [
                                          hor_line, hor_line], **opensignals_kwargs('line'))
 ...
 [numpy.min(data[row][column][0]),
  numpy.max(data[row][column][0])], legend_label=vert_lines_leg[row][vert_line_nbr], **opensignals_kwargs('line'))
                                    else:
                                        (fig_list[(-1)][(-1)][(-1)].line)([vert_line, vert_line], 
                                         [
                                          numpy.min(data[row][column][0]),
                                          numpy.max(data[row][column][0])], **opensignals_kwargs('line'))
                    ...
                    raise RuntimeError('At least one of the list elements, specified in data, is not a sublist.')
            else:
                if _is_instance(Number, data, condition='all') and _is_instance(bool, data, condition='any') or _is_instance(Number, time, condition='all'):
                    _is_instance(bool, time, condition='any') or fig_list.append(figure(title=title, y_axis_label=y_axis_label[0], **style_figure))
    ...
    if grid_plot is True:
        ...
        if show_plot is True:
            ...
            raise RuntimeError('The specified number of lines and columns for the grid plot is not compatible.')
    else:
        if show_plot is True:
            show(fig_list[(-1)])
        return fig_list
...
def plot(*args, legend_label=None, title=None, x_axis_label='Time (s)', y_axis_label=None, grid_plot=False, grid_lines=None, grid_columns=None, hor_lines=None, hor_lines_leg=None, vert_lines=None, vert_lines_leg=None, apply_opensignals_style=True, show_plot=True, save_plot=False, warn_print=False, get_fig_list=False, file_name='plot', scatter=False, **kwargs):
    ...
    **kwargs : dict
    ...
    if len(args) == 1:
        time = [
         list(numpy.linspace(1, len(args[0][0]) + 1, len(args[0][0])))] * len(args[0])
        data = args[0]
    else:
        if len(args) == 2:
            time = list(args[0])
            ...
            raise RuntimeError('biosignalsnotebooks plot function only accepts 1 or 2 arguments in *args input. If only 1 input is given it should be a list with data samples,otherwise if 2 inputs are given then the first one defines the timeaxis and the second one data values.')
    if _is_instance(Number, data, condition='all') and _is_instance(bool, data, condition='any') or _is_instance(Number, time, condition='all') and not _is_instance(bool, time, condition='any'):
        time = [
         time]
        data = [data]
        if y_axis_label is not None:
            ...
            if legend_label is not None:
                ...
                if _is_instance((numpy.ndarray), data, condition='any') or _is_instance((numpy.ndarray), time, condition='any'):
    ...
    if legend_label is not None:
        if isinstance(legend_label, list):
            if len(legend_label) != len(time) or len(legend_label) != len(data):
                ...
                raise RuntimeError('The specified data type of legend field is not valid. Input must be a list.')
        elif title is not None:
            if not isinstance(title, list) or len(title) != len(time) or len(title) != len(data):
                raise RuntimeError('The shape of title does not match with time input.')
            else:
                if isinstance(title, str):
                    if grid_plot is True:
                        raise RuntimeError('Each figure of the gridplot must have a title, i.e., the shape of time, data and title inputs needs to match.')
                else:
                    title = [
                     title] * len(time)
        else:
            if grid_plot is False and len(title) != 1:
                raise RuntimeError('The number of titles is not compatible with the number of figures (only one title is needed).')
            else:
                raise RuntimeError('The specified data type of title field is not valid. Input must be a list.')
    else:
        title = [
         None] * len(time)
    if y_axis_label is not None:
        if isinstance(y_axis_label, list):
            if len(y_axis_label) != len(time) or len(y_axis_label) != len(data):
                ...
                if isinstance(y_axis_label, str):
                    y_axis_label = [
                     y_axis_label] * len(time)
                else:
                    if grid_plot is False and len(y_axis_label) != 1:
                        raise RuntimeError('The number of y axis labels is not compatible with the number of figures.')
                    else:
                        raise RuntimeError('The specified data type of y_axis_label field is not valid. Input must be a list or a string when grid_plot field is False.')
        else:
            y_axis_label = [
             None] * len(time)
        if grid_lines is not None or grid_columns is not None:
            if grid_plot is not True:
                raise RuntimeError('When grid_lines and grid_columns inputs are used the field grid_plot must be True.')
    else:
        raise isinstance(grid_lines, int) and isinstance(grid_columns, int) or RuntimeError('At least one of the grid_lines or grid_columns values is not an integer.')
    if hor_lines is not None:
        if not isinstance(hor_lines, list) or len(hor_lines) != len(time) or len(hor_lines) != len(data):
            raise RuntimeError('The shape of hor_lines does not match with time input.')
        else:
            raise RuntimeError('The specified data type of hor_lines field is not valid. Input must be a list of lists.')
        for cell in hor_lines:
            if not _is_instance(Number, cell, condition='all') or _is_instance(bool, cell, condition='any'):
                raise RuntimeError('At least one of the list elements, specified in hor_lines, is not numeric.')
    ...
    if vert_lines is not None:
        if isinstance(vert_lines, list):
            if len(vert_lines) != len(time) or len(vert_lines) != len(data):
                ...
                raise RuntimeError('The specified data type of vert_lines field is not valid. Input must be a list of lists.')
            for cell in vert_lines:
                if not _is_instance(Number, cell, condition='all') or _is_instance(bool, cell, condition='any'):
                    raise RuntimeError('At least one of the list elements, specified in vert_lines, is not numeric.')

        else:
            fig_list = []
            if _is_instance(list, data, condition='all'):
                if _is_instance(list, time, condition='all'):
                    for list_entry in range(0, len(time)):
                        if _is_instance(Number, (data[list_entry]), condition='all'):
                            if not _is_instance(bool, (data[list_entry]), condition='any'):
                                if _is_instance(Number, (time[list_entry]), condition='all') and not _is_instance(bool, (time[list_entry]), condition='any'):
                                    if len(time[list_entry]) == len(data[list_entry]):
                                        if grid_plot is True:
                                            fig_list.append(figure(title=title[list_entry], y_axis_label=y_axis_label[list_entry], 
                                             x_axis_label=x_axis_label, **style_figure))
                                        else:
                                            if grid_plot is False:
                                                if list_entry == 0:
                                                    fig_list.append(figure(title=title[list_entry], y_axis_label=y_axis_label[list_entry], 
                                                     x_axis_label=x_axis_label, 
                                                     sizing_mode='scale_both', **style_figure))
                                            else:
                                                if legend_label is not None:
                                                    if not scatter:
                                                        (fig_list[(-1)].line)(time[list_entry], data[list_entry], legend_label=legend_label[list_entry], **style_line)
                                                    else:
                                                        (fig_list[(-1)].circle)(time[list_entry], data[list_entry], legend_label=legend_label[list_entry], **style_line)
                                                else:
                                                    if not scatter:
                                                ...
                                                if hor_lines is not None:
                                                    for hor_line_nbr, hor_line in enumerate(hor_lines[list_entry]):
                                                        if hor_lines_leg is not None:
                                                            (fig_list[(-1)].line)([time[list_entry][0], time[list_entry][(-1)]], [hor_line, hor_line], legend_label=hor_lines_leg[list_entry][hor_line_nbr], **opensignals_kwargs('line'))
                                                        else:
 ...
 [numpy.min(data[list_entry]),
  numpy.max(data[list_entry])], legend_label=vert_lines_leg[list_entry][vert_line_nbr], **opensignals_kwargs('line'))
                                                        else:
                                                            (fig_list[(-1)].line)([vert_line, vert_line], 
                                                             [
                                                              numpy.min(data[list_entry]),
                                                              numpy.max(data[list_entry])], **opensignals_kwargs('line'))
                    ...
                    if _is_instance(Number, data, condition='all') and _is_instance(bool, data, condition='any') or _is_instance(Number, time, condition='all') and not _is_instance(bool, time, condition='any'):
                        ...
                        if _is_instance(Number, data, condition='all') and not _is_instance(bool, data, condition='any'):
                            if _is_instance(Number, time, condition='all'):
                                if not _is_instance(bool, time, condition='any'):
                                    if len(time) == len(data):
                                        fig_list.append(figure(title=title[0], y_axis_label=y_axis_label[0], x_axis_label=x_axis_label, **style_figure))
                                        if legend_label is not None:
                                ...
                                raise RuntimeError("The input 'data' or/and 'time' does not have a valid format. It should be a list of numbers or a list of lists.")
                if apply_opensignals_style is True:
...
def dispersion(x_axis, y_axis, x_axis_label, y_axis_label, show_plot=True):
    ...
    """
    list_figures_1 = []
    list_figures_1.append(figure(x_axis_label=x_axis_label, y_axis_label=y_axis_label, **opensignals_kwargs('figure')))
...
def opensignals_kwargs(obj):
    ...
    if obj == 'figure':
        ...
        if obj == 'gridplot':
            out = {'toolbar_options':{'logo': None}, 
             'sizing_mode':'scale_width'}
        else:
            if obj == 'line':
                out = {'line_width':2, 
                 'line_color':opensignals_color_pallet()}
    return out
        ...
        if grid_plot is True:
            if isinstance(input_arg, list):
                if numpy.shape(input_arg)[:dimension] != numpy.shape(data)[:dimension]:
                    raise RuntimeError('The shape of ' + input_name + ' does not match with data input.')
            else:
                raise RuntimeError('The specified data type of ' + input_name + ' field is not valid. Input must be a list.')
        elif not isinstance(input_arg, str):
            raise RuntimeError("Taking into account that only one time-series had been specified at 'data', the " + input_name + ' field must be a string')
