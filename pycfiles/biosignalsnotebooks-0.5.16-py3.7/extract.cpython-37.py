...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\extract.py
...
def hrv_parameters(data, sample_rate, signal=False, in_seconds=False):
    ...
    """
    out_dict = {}
    tachogram_data, tachogram_time = tachogram(data, sample_rate, signal=signal, in_seconds=in_seconds,
      out_seconds=True)
    ...
    max_hr = 1 / out_dict['MinRR']
    out_dict['MaxBPM'] = max_hr * 60
    min_hr = 1 / out_dict['MaxRR']
    out_dict['MinBPM'] = min_hr * 60
    avg_hr = 1 / out_dict['AvgRR']
    out_dict['AvgBPM'] = avg_hr * 60
    ...
    out_dict['SD2'] = numpy.sqrt(2 * numpy.power(out_dict['SDNN'], 2) - numpy.power(out_dict['SD1'], 2))
    ...
    out_dict['pNN20'] = int(float(out_dict['NN20']) / len(tachogram_diff_abs) * 100)
    out_dict['NN50'] = sum((1 for i in tachogram_diff_abs if i > 0.05))
    out_dict['pNN50'] = int(float(out_dict['NN50']) / len(tachogram_diff_abs) * 100)
    freqs, power_spect = psd(tachogram_time, tachogram_data)
    freq_bands = {'ulf_band':[
      0.0, 0.003], 
    ...
    for band in band_keys:
        ...
        power_samples_inside_band = [power_val for power_val, freq in zip(power_spect, freqs) if freq_band[0] <= freq <= freq_band[1]]
...
def psd(tachogram_time, tachogram_data):
    ...
    freq_axis, power_axis = scisignal.welch(nn_tachogram_even, interpolation_rate, window=(scisignal.get_window('hanning', min(len(nn_tachogram_even), 1000))),
...
def emg_parameters(data, sample_rate, raw_to_mv=True, device='biosignalsplux', resolution=16):
    ...
    burst_begin, burst_end = detect_emg_activations(data, sample_rate, smooth_level=20, threshold_level=10,
      time_units=True)[:2]
    ...
    freq_axis, power_axis = scisignal.welch(data, fs=sample_rate, window='hanning', noverlap=0, nfft=(int(256.0)))
    area_power_spect = integr.cumtrapz(power_axis, freq_axis, initial=0)
    out_dict['Total Power Spect'] = area_power_spect[(-1)]
    out_dict['Median Frequency'] = freq_axis[numpy.where(area_power_spect >= out_dict['Total Power Spect'] / 2)[0][0]]
    out_dict['Maximum Power Frequency'] = freq_axis[numpy.argmax(power_axis)]
    return out_dict


def fatigue_eval_med_freq(data, sample_rate, time_units=True, raw_to_mv=True, device='biosignalsplux', resolution=16, show_plot=False):
    """
    -----
    Brief
    -----
    ...
    for burst in range(0, len(burst_begin)):
        ...
        freqs, power = scisignal.welch(processing_window, fs=sample_rate, window='hanning', noverlap=0,
          nfft=(int(256.0)))
    ...
    if show_plot is True:
        list_figures_1 = plot([list(time), list(median_freq_time)], [
         list(data), list(median_freq_data)],
          title=[
         'EMG Acquisition highlighting bursts',
         'Median Frequency Evolution'],
          gridPlot=True,
          gridLines=2,
          gridColumns=1,
          openSignalsStyle=True,
          x_axis_label='Time (s)',
          yAxisLabel=[
        ...
        for burst in range(0, len(burst_begin)):
            ...
            box_annotation = BoxAnnotation(left=(burst_begin[burst] / sample_rate), right=(burst_end[burst] / sample_rate),
              fill_color=color,
              fill_alpha=0.1)
            box_annotation_copy = BoxAnnotation(left=(burst_begin[burst] / sample_rate), right=(burst_end[burst] / sample_rate),
              fill_color=color,
              fill_alpha=0.1)
        ...
        show(gridplot_1)
    return pandas.DataFrame({'Time (s)':median_freq_time,  'Median Frequency (Hz)':median_freq_data},
      columns=[
