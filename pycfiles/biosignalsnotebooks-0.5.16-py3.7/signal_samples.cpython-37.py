...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\biosignalsnotebooks\signal_samples.py
# Compiled at: 2020-03-23 15:40:40
# Size of source mod 2**32: 17989 bytes
"""
...
SIGNAL_PATH = (os.path.abspath(__file__).split(os.path.basename(__file__))[0] + 'notebook_files\\osf_files\\signal_samples\\').replace('\\', '/')
FILE_EXTENSION = '.h5'
...
def load_signal(signal_handler, get_header=False):
    ...
    """
    primordial_list_signals = [
     'ecg_4000_Hz', 'ecg_5_min', 'ecg_sample', 'ecg_20_sec_10_Hz',
    ...
    if any((mark in signal_handler for mark in ('http://', 'https://', 'www.', '.pt',
                                                '.com', '.org', '.net'))):
        ...
        if signal_handler in primordial_list_signals or signal_handler in list_files:
            ...
            raise RuntimeError('The signal name defined as input does not correspond to any of the signal samples contained in the package.')
