...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/rbmozui/fields.py
    ...
    def as_html(self):
        ...
        if not is_squashed:
            ...
            try:
                ...
                logging.error('Could not retrieve child review request with commit id %s because it does not appear to exist, or the user does not have read access to it.' % identifier)
                return ''

        template = get_template('rbmozui/commits.html')
        return template.render(Context({'current_id': current_id, 
           'root_rr': rr}))