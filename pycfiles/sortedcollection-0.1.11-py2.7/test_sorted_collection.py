...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sortedcollection/test_sorted_collection.py
...
def test_sorted_collection_found_items_is_index_0():
    sc = SortedCollection([1, 2])
    assert sc.find_le(1.5) == 1
    with pytest.raises(ValueError):
        sc.find_le(0.5)


def test_sorted_collection_items_with_same_key():
    sc = SortedCollection([1, 2, 3, 3, 3, 3, 4, 5, 7])
    assert list(sc.find_le_iter(3.5)) == [3, 3, 3, 3]


def test_sorted_collection_iter():
    data = [
     8, 6, 3, 1, 2, 3, 3, 3, 3, 4, 5, 7]
    sc = SortedCollection(data)
    data2 = copy.deepcopy(data)
    data2.sort()
    assert list(sc) == data2
    data2.sort(reverse=True)
    print data2
    assert list(sc.reverse_iter()) == data2