...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sortedcollection/__init__.py
# Compiled at: 2014-12-16 15:50:06
"""
SortedCollection class that abstracts bisect

MIT/X11 licensed recipe from
...
class SortedCollection(object):
    ...
    less-than-or-equal to a key, or the first item
    ...
    def __init__(self, iterable=(), key=None):
        """
        :param iterable: items to put into this SortedCollection
        :param key: callable that returns an orderable hash key for any item
        """
        self._given_key = key
    ...
    def clear(self):
        """
        empty this SortedCollection
        """
        self.__init__([], self._key)

    def copy(self):
        """
        return a full clone of this SortedCollection
        """
    ...
    def __setitem__(self, key, item):
        """
        alias for insert
        asserts that key matches key_func(item)
        """
        assert key == self._key(item)
        self.insert(item)
    ...
    def find_le_iter(self, k):
        """
        Yield possibly multiple items with the first key <= k.  
        Raise ValueError if not found.
        """
        i = bisect_right(self._keys, k)
        if not i:
            raise ValueError('No item found with key at or below: %r' % (k,))
        i -= 1
        found_k = self._keys[i]
        while i > 0:
            if not self._keys[i] == found_k:
                break
            yield self._items[i]
            i -= 1
    ...
    def find_range(self, k1, k2):
        """Generate all items with a k1 <= key < k2.  Could be no items."""
        i = bisect_left(self._keys, k1)
        while i < bisect_left(self._keys, k2):
            yield self._items[i]
            i += 1

    def reverse_iter(self):
        for i in xrange(1, len(self._items) + 1):
            yield self._items[(-i)]