...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/config.py
...
class PackageInstaller(_HTMLParser):
    ...
    def __init__(self, name, importmod=None, version=None, buildargs='', instargs='', extrainfo=None, verbose=True):
        """
            ...
            if 'download_url' in rd:
                if any([ ext in rd['download_url'] for ext in ('.tar.gz', '.tgz', '.zip') ]):
                    ...
                    for pathpiece in urlparse(url).path.split('/'):
                        if any([ pathpiece.endswith(ext) for ext in ('.tar.gz', '.tgz',
                                                                     '.zip') ]):
                            fn = pathpiece
                            break
...
_recpkgs = [
 PackageInstaller('ipython', 'IPython'),
...
_guipkgs = [
 PackageInstaller('Traits', 'enthought.traits'),
