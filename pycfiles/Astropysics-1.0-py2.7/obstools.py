...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/obstools.py
# Compiled at: 2014-05-05 14:40:58
"""
    ...
    **Examples**
    ...
    if output == 'fracarray':
        ...
        if rounding == 1000000:
            ...
            msec = (dec * 86400000000.0).astype('int64')
            if rounding > 0:
        ...
        if output == 'datetime':
            ...
            if msec is None:
                ts = (
                 year, month, day, hr % 24, min % 60, sec % 60)
            else:
                ts = (
                 year, month, day, hr % 24, min % 60, sec % 60, msec % 1000000)
...
def calendar_to_jd(caltime, tz=None, gregorian=True, mjd=False):
    ...
    **Examples**
    ...
    if isinstance(tz, basestring) or isinstance(tz, tzinfo):
        ...
        for t in zip(yr, month, day, hr, min, sec, msec):
            ...
            if utcdt is None:
                ...
                utcoffset.append(utcdt.days * 24 + (utcdt.seconds + utcdt.microseconds * 1e-06) / 3600)
    ...
    if gregorian is True:
        ...
        gregoffset[~gmask] = 0
    jdn = (365.25 * (yr + 4716)).astype(int) + (30.6001 * (month + 1)).astype(int) + day + gregoffset - 1524.5
...
def delta_AT(jdutc, usett=False):
    ...
    delat = cdelat[i]
    if i < drift.shape[0]:
        fd = dt.hour / 24 + dt.minute / 24 / 60 + dt.second / 24 / 3600.0 + dt.microsecond / 24 / 3600000000.0
...
class Site(object):
        ...
        * 'datetime'
            ...
            if hasattr(args[0], 'year'):
                if hasattr(args[0], 'hour'):
                    if args[0].tzinfo is None:
                        dtobj = datetime.datetime(args[0].year, args[0].month, args[0].day, args[0].hour, args[0].minute, args[0].second, args[0].microsecond, self.tz)
                    else:
                        dtobj = args[0]
                else:
                    dtobj = datetime.datetime(args[0].year, args[0].month, args[0].day, tzinfo=self.tz)
        ...
        if rettype is None or rettype == 'hours':
            ...
            if rettype == 'datetime':
                ...
                sec = 60 * (min - int(min))
                min = int(min)
                msec = int(1000000.0 * (sec - int(sec)))
                sec = int(sec)
    ...
    def localTime(self, lsts, date=None, apparent=True, returntype=None, utc=False):
        ...
        * 'datetime'
        ...
        if isinstance(lsts, datetime.datetime):
            ...
            date = lsts.date()
            lsts = datetime.time()
            lsts = lsts.hour + lsts.minute / 60 + (lsts.second + lsts.microsecond * 1000000.0) / 3600
        else:
    ...
    def equatorialToHorizontal(self, eqpos, lsts, epoch=None):
        ...
        if eqpos.decerr is not None or eqpos.raerr is not None:
            ...
            dazdH = (cHA * cdec / (cHA * cdec * slat - clat * sdec) + cdec * cdec * sHA * sHA * slat * (cHA * cdec * slat - clat * sdec) ** (-2)) / dtanaz
            dazddec = (sHA * sdec / (clat * sdec - cHA * cdec * slat) + (cdec * clat + cHA * sdec * slat) * cdec * sHA * (cHA * cdec * slat - clat * sdec) ** (-2)) / dtanaz
    ...
    def riseSetTransit(self, eqpos, date=None, alt=-0.5667, timeobj=False, utc=False):
        """
        ...
        if timeobj:
            ...
            def hrtodatetime(t):
                if t is None:
                    ...
                    t = (t - sec) * 1000000.0
                    msec = np.floor(t)
                    timeobj = datetime.time(int(hr), int(min), int(sec), int(msec), tzinfo=tz.tzutc() if utc else tz.tzlocal())
    ...
    def nextRiseSetTransit(self, eqpos, dtime=None, alt=-0.5667):
        """
        ...
        i.e. returns rise, set, transit such that rise < now < set;
        ...
        in the future, so now < rise < transit < set.
        ...
        *returns*
        ...
        rise1, set1, transit1 = self.riseSetTransit(eqpos, today, alt, timeobj=True, utc=True)
        rise2, set2, transit2 = self.riseSetTransit(eqpos, tomorrow, alt, timeobj=True, utc=True)
        ...
        elif transit1 is None:
            return (rise1, set1, transit1)
        yesterday = today - timedelta(days=1)
        rise0, set0, transit0 = self.riseSetTransit(eqpos, yesterday, alt, timeobj=True, utc=True)
        if dtime < set0:
    ...
    def onSky(self, eqpos, dtime=None, alt=-0.5667):
        """
    ...
    def apparentCoordinates(self, coords, datetime=None, precess=True, refraction=True):
        ...
        *returns*
        ...
        if datetime is None:
            ...
            if hasattr(datetime, 'year') or isSequenceType(datetime) and hasattr(datetime[0], 'year'):
            ...
            if refraction:
                ...
                for this_res in res_list:
                    ...
                    this_res.alt._decval += radians(R / 60.0)
    ...
    def nightTable(self, coord, date=None, strtablename=None, localtime=True, hrrange=(18, 6, 25)):
        """
        ...
        timehr = (jds - np.round(np.mean(jds)) + 0.5) * 24 + utcoffset
    ...
    def nightPlot(self, coords, date=None, plottype='altam', onlynight=False, clf=True, utc=False, colors=None, plotkwargs=None):
        """
        ...
        with _mpl_context(clf=clf) as (plt):
            ...
            if plottype == 'altam' or plottype == 'alt' or plottype == 'am':
                ...
                if 'alt' in plottype:
                    ...
                    plt.ylabel('${\\rm altitude} [{\\rm degrees}]$')
                elif 'am' == plottype:
                    if plt.ylim()[0] < 1:
                        lowery = 1
                    else:
                ...
                if utc:
                    ...
                    plt.xlabel('$\\rm UTC (hours)$')
                else:
                    plt.xticks(xtcks, [ xt % 24 for xt in xtcks ])
                    plt.xlabel('$\\rm Local Time (hours)$')
                if onlynight:
                ...
                plt.ylim(0, 90)
                plt.title(str(ra.sitedate))
                plt.xlabel('${\\rm azimuth} [{\\rm degrees}]$')
                plt.ylabel('${\\rm altitude} [{\\rm degrees}]$')
                if not nonames:
                    plt.legend(loc=0)
            elif plottype == 'sky':
                ...
                xtlabs = ['N', '$45^\\circ$', 'E', '$135^\\circ$', 'S', '$225^\\circ$', 'W', '$315^\\circ$']
                plt.xticks(np.radians(xticks), xtlabs)
                plt.ylim(0, 90)
                yticks = [15, 30, 45, 60, 75]
                plt.yticks(yticks, [ ('${0}^\\circ$').format(int(90 - yt)) for yt in yticks ])
                plt.title(str(ra.sitedate))
                if not nonames:
                    plt.legend(loc=0)
            else:
    ...
    def yearPlot(self, coords, startdate=None, n=13, months=12, utc=False, clf=True, colors=None):
        """
        ...
        """
        import datetime
        from matplotlib.dates import MonthLocator, WeekdayLocator, DateFormatter, MONDAY, DayLocator, YearLocator
        from operator import isMappingType, isSequenceType
...
class Extinction(object):
    ...
    __balmerratios = {'Hab': (2.86, 6562.82, 4861.33), 
...
class CalzettiExtinction(Extinction):
    ...
    def f(self, lamb):
        return -2.5 / np.log(10) * self._poly(10000.0 / lamb)
...
class FMExtinction(_EBmVExtinction):
    ...
    def f(self, lamb):
        x = 10000.0 / np.array(lamb, copy=False)
...
class CardelliExtinction(_EBmVExtinction):
    ...
    def f(self, lamb):
        ...
        x = 10000.0 / np.array(lamb, ndmin=1)
        ...
        a[opts] = np.polyval((0.32999, -0.7753, 0.01979, 0.72085, -0.02427, -0.50447,
                              0.17699, 1), x[opts] - 1.82)
        b[opts] = np.polyval((-2.09002, 5.3026, -0.62251, -5.38434, 1.07233, 2.28305,
                              1.41338, 0), x[opts] - 1.82)
        ...
        Fb = -0.213 * y ** 2 - 0.1207 * y ** 3
        a[nuv2s] = 1.752 - 0.316 * x[nuv2s] - 0.104 / ((x[nuv2s] - 4.67) ** 2 + 0.341) + Fa
        b[nuv2s] = -3.09 + 1.825 * x[nuv2s] + 1.206 / ((x[nuv2s] - 4.62) ** 2 + 0.263) + Fb
        a[fuvs] = np.polyval((-0.07, 0.137, -0.628, -1.073), x[fuvs] - 8)
        b[fuvs] = np.polyval((0.374, -0.42, 4.257, 13.67), x[fuvs] - 8)
        AloAv = a + b / self.Rv
        if scalar:
            return AloAv[0]
        else:
...
class LMCExtinction(FMExtinction):
    ...
    def __init__(self, EBmV=0.3, Rv=3.41):
        super(LMCExtinction, self).__init__(-0.89, 0.998, 2.719, 0.4, 4.579, 0.934, EBmV, Rv)
...
class SMCExtinction(FMExtinction):
    ...
    def __init__(self, EBmV=0.2, Rv=2.74):
...
def extinction_correction(lineflux, linewl, EBmV, Rv=3.1, exttype='MW'):
    ...
    if exttype == 'LMC':
        ...
        if exttype == 'SMC':
            ...
            a[irs] = 0.574 * x[irs] ** 1.61
            b[irs] = -0.527 * x[irs] ** 1.61
            a[opts] = polyval((0.32999, -0.7753, 0.01979, 0.72085, -0.02427, -0.50447,
                               0.17699, 1), x[opts] - 1.82)
            b[opts] = polyval((-2.09002, 5.3026, -0.62251, -5.38434, 1.07233, 2.28305,
                               1.41338, 0), x[opts] - 1.82)
            a[nuv1s] = 1.752 - 0.316 * x[nuv1s] - 0.104 / ((x[nuv1s] - 4.67) ** 2 + 0.341)
            b[nuv1s] = -3.09 + 1.825 * x[nuv1s] + 1.206 / ((x[nuv1s] - 4.62) ** 2 + 0.263)
            y = x[nuv2s] - 5.9
            ...
            a[nuv2s] = 1.752 - 0.316 * x[nuv2s] - 0.104 / ((x[nuv2s] - 4.67) ** 2 + 0.341) + Fa
            b[nuv2s] = -3.09 + 1.825 * x[nuv2s] + 1.206 / ((x[nuv2s] - 4.62) ** 2 + 0.263) + Fb
            a[fuvs] = polyval((-0.07, 0.137, -0.628, -1.073), x[fuvs] - 8)
            b[fuvs] = polyval((0.374, -0.42, 4.257, 13.67), x[fuvs] - 8)
            AloAv = a + b / Rv
            Al = AloAv * Rv * EBmV
            finalval = Al
...
def extinction_from_flux_ratio(frobs, frexpect, outlambda=None, Rv=3.1, tol=0.0001):
    """
