...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/utils/io.py
...
def _readrem(remote, reportprogress=False):
    ...
    if reportprogress:
        if reportprogress is True:
            ...
            def reportprogress(progress, started, ended, url):
                ...
                if started is False:
                    ...
                    kb = progress / 1000.0
                    mb = kb / 1000.0 if kb > 1000 else None
                    gb = mb / 1000.0 if mb > 1000 else None
                    if gb is not None:
        ...
        if 'content-length' in remote.info():
            ...
            if totbytes > 102400:
                nbytesper = int(ceil(totbytes / 100.0))
            else:
                nbytesper = 1024
        else:
            totbytes = 1
...
def get_data(dataurl, asfile=False, localfn=None):
    ...
    if store:
        ...
        with contextlib.closing(shelve.open(os.path.join(datadir, 'urlmap'))) as (url2fn):
            if dataurl not in url2fn or store == 'refresh' or not os.path.exists(os.path.join(datadir, url2fn[dataurl])):
...
def set_data_download_reporter(reportprogress=_data_reporter):
    ...
    """
    global _data_reporter
    if not (callable(reportprogress) or reportprogress is True or reportprogress is False):
...
class FixedColumnDataParser(object):
    ...
    def addColumnsFromFile(self, fn, columnlinestart=None, sep=None, useskiprows=True, maxcols=100):
        """
...
def loadtxt_fixed_column_fields(fn, fncol=None, skiprows=0, comments='#', columnlinestart='#:', columnsep=' ', maxcols=None, oneindexed=True, maskedarray=False, updatedict=None):
    """
...
def load_deimos_spectrum(fn, plot=False, extraction='horne', retdata=False, smoothing=None):
    ...
    with open_with_pyfits(fn) as (f):
        ...
        if extname + '-B' in extd and extname + '-R' in extd:
            ...
            if extname + '-B' in extd or extname + '-R' in extd:
...
def load_all_deimos_spectra(dir='.', pattern='spec1d*', extraction='horne', smoothing=None, verbose=True):
    """
