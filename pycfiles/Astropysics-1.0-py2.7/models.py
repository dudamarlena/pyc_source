...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/models.py
...
class BlackbodyModel(FunctionModel1DAuto, _HasSpecUnits):
    ...
    def xaxisname(self):
        if self._phystype == 'wavelength':
            return 'lambda'
        if self._phystype == 'frequency':
            return 'nu'
        if self._phystype == 'energy':
            return 'E'
...
class EinastoModel(FunctionModel1DAuto):
    ...
    def f(self, r, A=1, rs=1, alpha=0.17):
...
class GaussHermiteModel(FunctionModel1DAuto):
    ...
    def f(self, v, A=1, v0=0, sig=1, *hj3):
        ...
        return A * alpha / sig * (1 + np.sum(hj3arr * self._Hjs(w, len(hj3) + 3, exclude=(0,
                                                                                          1,
                                                                                          2)), axis=0))
...
class NFWModel(FunctionModel1DAuto):
    ...
    def getV(self, r, **kwargs):
        ...
        return (GMspc / 1000 * M / r) ** 0.5

    _xmin = 2.1625815870646097
    ...
    def setC(self, c, Rvir=None, Mvir=None, z=0):
        ...
        if Rvir is None:
            ...
            Rvir = 0.001 * (3 * Mvir / (4 * pi * rhov)) ** 0.3333333333333333
        elif Mvir is None:
            rhov = get_cosmology().rhoC(z, 'cosmological') * 1e-18 * self.deltavir(z)
            Mvir = 4 * pi * (Rvir * 1000.0) ** 3 / 3 * rhov
        else:
            self._c = c
        self.rc = Rvir / c
        self.rho0 = 1
    ...
    def Rvir_to_Mvir(Rvir, z=0):
        ...
        from .constants import get_cosmology
        c = get_cosmology()
        return 1000000000000.0 / c.h * (c.omega * c.deltavir(z) / 97.2) * (Rvir * c.h * (1 + z) / 203.4) ** 3
    ...
    def Mvir_to_Rvir(Mvir, z=0):
        ...
        from .constants import get_cosmology
        c = get_cosmology()
        return 203.4 / c.h * (c.omega * c.deltavir(z) / 97.2) ** (-0.3333333333333333) * (Mvir * c.h / 1000000000000.0) ** 0.3333333333333333 / (1 + z)
    ...
    def Mvir_to_Vvir(Mvir, z=0):
        ...
        from .constants import get_cosmology
        c = get_cosmology()
        return 143.8 * (c.omega * c.deltavir(z) / 97.2) ** 0.16666666666666666 * (Mvir * c.h / 1000000000000.0) ** 0.3333333333333333 * (1 + z) ** 0.5
    ...
    def Vvir_to_Mvir(Vvir, z=0):
        ...
        from .constants import get_cosmology
        c = get_cosmology()
        return (c.omega * c.deltavir(z) / 97.2) ** (-0.5) * (1 + z) ** (-1.5) * 1000000000000.0 * (Vvir / 143.8) ** 3 / c.h
    ...
    def Vvir_to_Vmax(Vvir):
        ...
        """
        return (Vvir / 0.468) ** 0.9090909090909091
    ...
    def Mvir_to_Cvir(Mvir, z=0):
        ...
        cvir = 9.6 * (Mvir * (H0 / 72.0) / 10000000000000.0) ** (-0.13) * (1 + z) ** (-1)
        if cvir > 4:
            return cvir
        return 4.0
    ...
    def Cvir_to_Mvir(Cvir, z=0):
        ...
        """
        return 10000000000000.0 / (c.h / 0.72) * (Cvir / 9.6 * (1 + z)) ** (-7.692307692307692)
    ...
    def Vvir_to_Tvir(Vvir, mu=0.59):
        ...
        return mu * mp * Vvir ** 2 / (2 * kb) * 10000000000.0
...
class PlummerModel(FunctionModel1DAuto):
    ...
    def f(self, r, rp=1.0, M=1.0):
        return 3 * M / (4.0 * pi * rp ** 3) * (1 + (r / rp) ** 2) ** (-2.5)
...
class SchechterLumModel(FunctionModel1DAuto):
    ...
    def f(self, L, Lstar=10000000000.0, alpha=-1.0, phistar=1.0):
        x = L / Lstar
...
class SersicModel(FunctionModel1DAuto):
    ...
    _bnpoly1 = np.poly1d([-7.151012295891972e-05, 0.0001140941058636532, 0.0018028610621203215, 0.009876543209876543, -0.3333333333333333])
    ...
    def bn_estimate(n):
        ...
        n = float(n)
        if n > 0.36:
            return 2 * n + SersicModel._bnpoly1(1 / n)
        return SersicModel._bnpoly2(n)
