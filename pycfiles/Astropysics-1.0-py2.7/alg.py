...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/utils/alg.py
...
def sigma_clip(data, sig=3, iters=1, cenfunc='median', varfunc=np.var, maout=False):
    ...
    if maout:
        ...
        return (
         data[mask], mask.reshape(oldshape))
        return
...
def angle_axis(matrix, degrees=True):
    ...
    denom = sqrt(2 * (m[(2, 1)] - m[(1, 2)] + (m[(0, 2)] - m[(2, 0)]) + (m[(1, 0)] - m[(0,
                                                                                        1)])))
