...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/gui/spectarget.py
...
if traitsflat:
    from traits.api import HasTraits, on_trait_change, Instance, Float, String, ListStr, ListFloat, Button, Bool, Property, Event, Array, Enum, TraitError
    from traitsui.api import View, Group, HGroup, HFlow, Item, Handler
    from traitsui.menu import ModalButtons
    from chaco.api import Plot, ArrayPlotData, HPlotContainer, ColorBar, LinearMapper, jet, LassoOverlay, ColormappedSelectionOverlay
    from chaco.tools.api import PanTool, ZoomTool, LassoSelection
    from enable.component_editor import ComponentEditor
else:
    from enthought.traits.api import HasTraits, on_trait_change, Instance, Float, String, ListStr, ListFloat, Button, Bool, Property, Event, Array, Enum, TraitError
    ...
    from enthought.chaco.api import Plot, ArrayPlotData, HPlotContainer, ColorBar, LinearMapper, jet, LassoOverlay, ColormappedSelectionOverlay
...
class OffsetDialog(HasTraits):
    ...
    locscale = Float
    view = View(Group(Item('offsetbands'), Item('offsetscales'), Item('locscale'), layout='normal'), resizable=True, title='Offset Settings', handler=ODHandler(), buttons=ModalButtons, kind='modal')
...
class SpecTarget(HasTraits):
    ...
    updatelocs = Event
    updategastars = Event
    view = View(Group(Item('plotcontainer', editor=ComponentEditor(size=(800, 400)), show_label=False), Group(Group(Item('distance', label='Distance (kpc)'), Item('distmod', label='Distance Modulus'), Item('xax', label='CMD x-axis'), Item('yax', label='CMD y-axis'), columns=2), Group(Item('agband', label='Alignment/Guide Star Band'), Item('gstarcut', label='Guide Star Cutoff'), Item('astarcut', label='Alignment Star Cutoff'), Item('astarcut2', label='Alignment Star Cutoff 2'), Item('agigpri', label='A/G Ignore priority'), Item('agigg1', label='A/G Ignore if group 1?'), columns=6), HGroup(Item('priband', label='Priority Band'), Item('pribandw', label='Band Priority Weight'), Item('offsetset', label='Offset Settings'), Item('pripower', label='Power for converting offsets into priorities'), Item('groupcutoff', label='Last Group Cutoff'), Item('hideg0', label='Set Group 0 priority to 0?')), Group(Item('masktype', label='Mask Type'), Item('maskfn', label='Mask File(base)'), Item('makemaskbutton', label='Make Mask'), Item('maskshortname', label='Mask Name(short)'), Item('masklongname', label='Mask Name(long)'), columns=3)), layout='normal'), resizable=True, title='Spectra Targeter')
    ...
    def __init__(self, *args, **kwargs):
        ...
        * SpecTarget(fiducialdict,data)
        ...
        self.priband = self.yax
        self._cmda_late_changed()
        self._masktype_changed(self.masktype)
        return
    ...
    def _selection_completed(self, obj, name, new):
        cds = (
         self.cmplot.plots['target_mags'][0].color_data, self.locplot.plots['target_locs'][0].color_data)
        datasource = obj.selection_datasource
        targds = cds[1] if cds[0] == datasource else cds[0]
...
class DEIMOSMaskMaker(MaskMaker):
    ...
    def writeInFile(self, fn, pa=None):
        ...
        with open(fn, 'w') as (f):
            for i, n in enumerate(self.sto.cmda.datanames):
                ...
                if samp[i] != -9 or pri[i] == -1 or pri[i] == -2:
                    if pa is None:
                        t = (
                         n[:16], ras, decs, 2000, mags[i], magband, int(pri[i]), samp[i], presel[i])
                        f.write('%s\t%s %s %i %2.2f %s %4i %i %i INDEF\n' % t)
                    else:
                        t = (
                         n[:16], ras, decs, 2000.0, mags[i], magband, int(pri[i]), samp[i], presel[i], pa[i])
                ...
                for l in fr:
                    if 'PA' in l:
                        ...
                        if ls.startswith('#') or ls == '':
                            ...
                            t = (
                             name, ra, dec, epoch, mag, band, pri, grp, presel, panum + degoffset)
    ...
    def doneToG0(fn, cmda, forceradec=False):
        ...
        if forceradec or objs.max() >= len(g):
            ...
            mask = match_coords(rao, deco, ras, decs, 0.0001388888888888889)[0]
            g[mask] = 0
        elif len(objs) > 0:
            g[objs] = 0
        if not all(g):
