...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/ccd.py
# Compiled at: 2013-11-27 17:30:35
"""
...
class CCDImage(object):
    ...
    * band
    ...
    def __init__(self, range=None, scaling=None):
        ...
        self.applyChangesOnActivate = False
        self._directaccess = False
        self._rng = range
        self.setScaling(scaling)
        if scaling is None:
            self.activateRange(None)
        return
    ...
    def activateRange(self, range):
        ...
        if range is None:
            ...
            range = [
             xcen - rad, xcen + rad, ycen - rad, ycen + rad]
    ...
    def applyChanges(self):
        ...
        if self.scaling.name == 'linear' and self._directaccess:
            pass
        else:
    ...
    def plotImage(self, valrange='p99', flipaxis=None, invert=False, cb=True, scalebar=None, axes='image', clickinspect=True, clf=True, colormap='gray', **kwargs):
        """
        ...
        * 'i#,#,#,#' to ignore the specified values in the calculation of the range
        ...
        if valrange:
            if 's' in valrange:
                if 'g' in valrange:
                    ...
                    vm = np.median(vals)
                valrange = (
                 vm - valrange, vm + valrange)
            elif 'n' in valrange or 'p' in valrange:
                if 'g' in valrange and valrange.replace('g', '') in self._fstatd:
                    ...
                    if niglow + nigup >= sortval.size:
                        ...
                        self._fstatd[valrange.replace('g', '')] = valrange = (
                         sortval[niglow], sortval[(-1 if nigup == 0 else -nigup)])
                    else:
                        valrange = (
                         sortval[niglow], sortval[(-1 if nigup == 0 else -nigup)])
            elif 'i' in valrange:
        ...
        with mpl_context(clf=clf) as (plt):
            ...
            if scalebar:
                ...
                xlscal = ext[0] + 0.5 * scalsz
                xuscal = xlscal + scalsz
                yscal = ext[2] + yr / 6
                plt.plot([xlscal, xuscal], [yscal, yscal], '-r')
                yscal = plt.gca().transData.transform([0, yscal])
                yscal += [0, 3]
                yscal = plt.gca().transData.inverted().transform(yscal)
                yscal = yscal[1]
                if axes == 'image':
                    scalestr = 'px'
                else:
    ...
    def plotHist(self, perc=None, gauss=None, clf=True, **kwargs):
        ...
        if 'bins' not in kwargs:
            kwargs['bins'] = 100 if vals.size > 100000.0 else 10
...
class LogScaling(DataScaling):
    ...
    def transform(self, imagearray):
        if self.base is None:
            ...
            return np.log(imagearray) * (self.upper - self.lower) + self.lower
        else:
            if self._base == 10:
...
class FitsImage(CCDImage):
    ...
    def __init__(self, fnordata, range=None, scaling=None, hdu=0, memmap=0):
        ...
        try:
            if isinstance(fnordata, basestring):
                ...
                hdr, arr = fnordata
                if isSequenceType(hdr) and not isMappingType(hdr):
                    if len(hdr) != len(arr):
                        raise ValueError('headers and arrays not the same size in FitsImage constructor')
                    hdus = [
                     pyfits.PrimaryHDU(data=arr[0], header=FitsImage._hdr_conv(hdr[0]))]
                    self._chdu = 0
                    ...
                    hdus = [
                     pyfits.PrimaryHDU(data=arr, header=FitsImage._hdr_conv(hdr))]
                    self._chdu = 0
                    self._applyArray(None, arr)
                    self.fitsfile[self._chdu].update_header()
                self.fitsfile = pyfits.HDUList(hdus)
    ...
    def _hdr_conv(hdr):
        ...
        if isinstance(hdr, pyfits.Header):
            ...
            if isinstance(hdr, pyfits.CardList) or isinstance(hdr, list):
    ...
    def combineImages(self, images):
        ...
        if self.shifts:
            ...
            def shift_coords(coords, shift):
                return (
                 coords[0] - shift[0], coords[1] - shift[1])

            shiftedimages = []
            for i, im in enumerate(images):
                newim = geometric_transform(im, partial(shift_coords, shift=shifts[i]), output_shape=outshape, order=self.shiftorder)
...
def mosaic_objects(xcens, ycens, radii, images, row=None, titles=None, noticks=True, clf=True, logify=False, imdict=None, **kwargs):
    """
    ...
    if isSequenceType(xcens):
        ...
        if isSequenceType(ycens):
            onev = np.ones(len(ycens))
        else:
            if isSequenceType(radii):
                onev = np.ones(len(radii))
            else:
                if isSequenceType(images) and type(images) is not str:
...
def kcorrect_images(images, bands, z, range=None, zeropoints=None, pixelareas=None, retdict=False, offset=None, **kckwargs):
    ...
    targetshape = tuple(np.r_[(nbands, dshape)])
