...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/data/project_template.py
...
def make_plots(plots, argd=None, figs=None, save=False, overwrite=True, showfigs=True):
    ...
    if plots is None:
        ...
        if isinstance(plots, basestring):
            if '*' in plots:
                import re
                plots = plots.replace('*', '.*')
        ...
        for p, fnum in zip(plots, figs):
            ...
            plotd[p] = (
             _plotreg[p][0], fnum, _plotreg[p][1], _plotreg[p][2], _plotreg[p][3])
...
def main():
    ...
    try:
        ...
        p.add_argument('-m', '--main-figs', dest='mainfigs', const=True, default=False, nargs='?', help='Shows all main figures even if they are not included in the command line.' + ' Can also provide a mainfig group name to create only mainfigs associated with that group.')
        ...
        op.add_option('-m', '--main-figs', dest='mainfigs', action='store_true', default=False, help='Shows all main figures even if they are not included in the command line.' + ' Can also provide a mainfig group name to create only mainfigs associated with that group.')
