...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/phot.py
...
def choose_magnitude_system(system):
    ...
    * 'asinh'
    ...
    """
    global _flux_to_mag
    global _fluxerr_to_magerr
    global _mag_to_flux
    global _magerr_to_fluxerr
    global _magsys
...
class GaussianBand(Band):
    ...
    def _getFWHM(self):
        return self.unittrans[0](self._fwhm)
    ...
    if cs is None:
        ...
        if len(sortednames) == 5:
            if sortednames[:3] == ['U', 'B', 'V']:
                cs = [
                 'c', 'b', 'g', 'r', 'm']
            elif np.all([ n in sortednames[i] for i, n in enumerate(('u', 'g', 'r',
                                                                     'i', 'z')) ]):
                cs = [
                 'c', 'g', 'r', 'm', 'k']
    kwargs['clf'] = False
...
class PhotObservation(object):
    ...
    def plot(self, includebands=True, fluxtype=None, unit='angstroms', clf=True, **kwargs):
        ...
        * 'fluxden'
        ...
        rng = y.max() - y.min()
        plt.ylim(y.min() * 1.05 - y.max() * 0.05, y.min() * -0.05 + y.max() * 1.05)
        return
    ...
    def _dataAndBandsToDicts(bandsin, arr):
        ...
        for i, c in enumerate(colors):
            if c is not None:
                ...
                if b1 in bd and b2 not in bd:
                    bd[b2] = bands[b2]
                    datad[b2] = datad[b1] - arr[i]
                    maskd[b2] = True
                elif b2 in bd and b1 not in bd:
                    bd[b1] = bands[b1]
        ...
        if self._offbands is None:
            ...
            for b in self._offbands:
                if isinstance(b, tuple):
                    ...
                    if not (isinstance(i2, int) and i2 >= 0 and i2 < self._nb):
                        raise AssertionError((i2, b2))
                        raise (self._fidmask[i1] and self._datamask[i1] or self._fidmask[i2] and self._datamask[i2] or ValueError)('masks incompatible with bands at positions %i and %i' % (i1, i2))
                else:
                    i = lbns.index(b)
                    if not (isinstance(i, int) and i >= 0 and i < self._nb):
                        raise AssertionError((i, b))
                        if not self._datamask[i]:
                            raise ValueError('data mask incompatible with band at position %i' % i)
                        raise (self._fidmask[i] or ValueError)('fiducial mask incompatible with band at position %i' % i)
...
class ModelPointSpreadFunction(PointSpreadFunction):
    ...
    def convolve(self, arr2d, background=False):
        ...
        if self.convsize is None:
            nx, ny = arr2d.shape
        elif np.isscalar(self.convsize):
            nx = ny = self.convsize
        else:
            nx, ny = self.convsize
...
class SExtractor(object):
    ...
    def _getSexDefaults():
        ...
        SExtractor._optorder = optorder
        SExtractor._parinfo = parinfo
        SExtractor._parorder = parorder
        return
...
def ML_ratio_from_color(c, color='B-V'):
    ...
    """
    bdj01table1 = np.array([[-0.994, 1.804, -0.734, 1.404, -0.66, 1.222, -0.627, 1.075, -0.621, 0.794, -0.663, 0.704, -0.692, 0.652],
     [
      -1.224, 1.251, -0.916, 0.976, -0.82, 0.851, -0.768, 0.748, -0.724, 0.552, -0.754, 0.489, -0.776, 0.452],
     [
      -1.919, 2.214, -1.476, 1.747, -1.314, 1.528, -1.204, 1.347, -1.04, 0.987, -1.03, 0.87, -1.027, 0.8],
     [
      -1.903, 1.138, -1.477, 0.905, -1.319, 0.794, -1.209, 0.7, -1.029, 0.505, -1.014, 0.442, -1.005, 0.402],
     [
      -2.181, 0.978, -1.7, 0.779, -1.515, 0.684, -1.383, 0.603, -1.151, 0.434, -1.12, 0.379, -1.1, 0.345],
     [
      -2.156, 0.895, -1.683, 0.714, -1.501, 0.627, -1.37, 0.553, -1.139, 0.396, -1.108, 0.346, -1.087, 0.314]], [
     -0.942, 1.737, -0.628, 1.305, -0.52, 1.094, -0.399, 0.824, -0.261, 0.433, -0.209, 0.21, -0.206, 0.135], [
     -0.976, 1.111, -0.633, 0.816, -0.523, 0.683, -0.405, 0.518, -0.289, 0.297, -0.262, 0.18, -0.264, 0.138])
...
def M_star_from_mags(B, V, R, I, color='B-V'):
    ...
    if color == 'B-V':
        ...
        if color == 'mean':
            ...
            return (
             np.mean((bv[0], br[0], vi[0])), np.mean((bv[1], br[1], vi[1]), axis=0))
        raise ValueError('Unknown color')
    mstar = []
...
def ML_ratio_from_color_SDSS(c, color='g-r'):
    ...
    """
    b03table = np.array([
     [-0.221, 0.485, -0.099, 0.345, -0.053, 0.268, -0.105, 0.226,
     ...
     [
      ...
      -0.079, 0.65, -0.148, 0.437, -0.186, 0.349],
     [
...
def M_star_from_mags_SDSS(u, g, r, i, z, J=None, H=None, K=None, color='mean'):
    ...
    if color == 'mean':
        ...
        return (
         mstars.mean(axis=0).mean(axis=0), mstars)
    else:
        if '-' in color:
            c1, c2 = color.split('-')
...
def mag_to_lum(M, Mzpt=4.83, Lzpt=1, Merr=None):
    ...
    dictin = isMappingType(M) and not np.isscalar(M)
    if dictin:
        dkeys = Mzpt = M.keys()
        M = np.array(M.values())
...
def lum_to_mag(L, Mzpt=4.83, Lzpt=1, Lerr=None):
    ...
    """
    from operator import isMappingType, isSequenceType
    dictin = isMappingType(L) and not np.isscalar(L)
    if dictin:
        dkeys = Mzpt = L.keys()
        L = np.array(L.values())
...
def kcorrect(mags, zs, magerr=None, filterlist=['U', 'B', 'V', 'R', 'I']):
    ...
    fd = {'U': 'bessell_U.par', 'B': 'bessell_B.par', 
       'V': 'bessell_V.par', 
       'R': 'bessell_R.par', 
       'I': 'bessell_I.par', 
       'u': 'sdss_u0.par', 
...
def color_to_Teff(c, colorbands='g-r'):
    ...
    if colorbands == 'g-r':
        ...
        logteff = np.polyval([0.0283, 0.0488, -0.316, 3.882], c)
        return 10 ** logteff
    if colorbands == 'B-V':
        teffsf = [
         7078, 6621, 6214, 5852, 5532, 5248, 4996, 4770, 4567, 4382, 4209, 4044, 3882]
        BmVsf = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.4, 1.5]
...
def teff_to_color(teff, colorbands='g-r'):
    ...
    if colorbands == 'g-r':
        (0.9, -0.2)
        ps = np.poly1d([0.0283, 0.0488, -0.316, 3.882 - np.log10(teff)])
        roots = ps.r
        goodroot = None
        for r in roots:
            if r.imag == 0 and -0.2 <= r.real <= 0.9:
