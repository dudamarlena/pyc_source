...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/gui/spylot.py
...
try:
    from traits.api import HasTraits, List, Instance, Property, Int, Range, Float, Event, Bool, DelegatesTo, Dict, Str, List, Button, cached_property, on_trait_change, Enum, Tuple, Array, Trait, File
    from traitsui.api import View, Item, Label, RangeEditor, Group, HGroup, VGroup, HFlow, SetEditor, spring, Handler, TextEditor, ColorTrait, TabularEditor, Include
    from traitsui.key_bindings import KeyBinding, KeyBindings
    from traitsui.tabular_adapter import TabularAdapter
    from chaco.api import Plot, ArrayPlotData, PlotAxis, LinePlot, DataRange1D, DataLabel, ArrayDataSource, AbstractOverlay, LinearMapper, TextBoxOverlay
    from chaco.tools.api import PanTool, ZoomTool, BroadcasterTool, RangeSelection, RangeSelectionOverlay, DragTool
    from enable.api import ComponentEditor, BaseTool, Interactor, Line, Component
except ImportError:
    from enthought.traits.api import HasTraits, List, Instance, Property, Int, Range, Float, Event, Bool, DelegatesTo, Dict, Str, List, Button, cached_property, on_trait_change, Enum, Tuple, Array, Trait, File
    from enthought.traits.ui.api import View, Item, Label, RangeEditor, Group, HGroup, VGroup, HFlow, SetEditor, spring, Handler, TextEditor, ColorTrait, TabularEditor, Include
    from enthought.traits.ui.key_bindings import KeyBinding, KeyBindings
    from enthought.traits.ui.tabular_adapter import TabularAdapter
    from enthought.chaco.api import Plot, ArrayPlotData, PlotAxis, LinePlot, DataRange1D, DataLabel, ArrayDataSource, AbstractOverlay, LinearMapper, TextBoxOverlay
    from enthought.chaco.tools.api import PanTool, ZoomTool, BroadcasterTool, RangeSelection, RangeSelectionOverlay, DragTool
    from enthought.enable.api import ComponentEditor, BaseTool, Interactor, Line, Component
...
class LineListEditor(HasTraits):
    ...
    major_view = View(Item('selectednames', editor=SetEditor(name='candidatenames', left_column_title='Candidate Lines', right_column_title='Selected Lines'), show_label=False), Item('lineplot', show_label=False, style='custom'), title='Major Lines')
    minor_view = View(Item('selectednames', editor=SetEditor(name='candidatenames', left_column_title='Candidate Lines', right_column_title='Selected Lines'), show_label=False), Item('lineplot', show_label=False, style='custom'), title='Minor Lines')
...
spylotkeybindings = KeyBindings(KeyBinding(binding1='<', binding2='Ctrl-,', description='Previous spectrum', method_name='prevSpec'), KeyBinding(binding1='>', binding2='Ctrl-.', description='Next spectrum', method_name='nextSpec'))
...
class Spylot(HasTraits):
    ...
    titlegroup = HGroup(Item('specleft', show_label=False, enabled_when='currspeci>0'), spring, Label('Spectrum #', height=0.5), Item('currspecip1', show_label=False, editor=RangeEditor(low_name='lowerspecip1', high_name='upperspecip1', mode='spinner')), Item('_titlestr', style='readonly', show_label=False), spring, Item('specright', show_label=False, enabled_when='currspeci<(len(specs)-1)'))
    speclistgroup = HGroup(Label('Spectrum List:'), spring, Item('delcurrspec', show_label=False, enabled_when='len(specs)>1'), Item('saveloadfile', show_label=False), Item('savespeclist', show_label=False), Item('loadspeclist', show_label=False, enabled_when='os.path.exists(saveloadfile)'), Item('loadaddfile', show_label=False), Item('loadaddspec', show_label=False, enabled_when='os.path.exists(saveloadfile)'), Item('loadaddspectype', show_label=False), spring)
    plotformatgroup = HGroup(spring, Item('fluxformat', show_label=False), Item('errformat', show_label=False), Item('scaleerr', label='Scale Error?'), Item('scaleerrfraclow', label='Lower', enabled_when='scaleerr', editor=TextEditor(evaluate=float)), Item('scaleerrfrachigh', label='Upper', enabled_when='scaleerr'), Item('showgrid', label='Grid?'), Item('showcoords', label='Coords?'), spring)
    featuregroup = HGroup(spring, Item('showmajorlines', label='Show major?'), Item('editmajor', show_label=False), Item('showlabels', label='Labels?'), Item('showminorlines', label='Show minor?'), Item('editminor', show_label=False), spring, Item('editfeatures', show_label=False), Item('featureselmode', show_label=False), spring)
    continuumgroup = HGroup(spring, Item('contsub', show_label=False), Item('contclear', show_label=False), Item('showcont', label='Continuum line?'), Item('contformat', show_label=False), Item('dosmoothing', label='Smooth?'), Item('smoothing', show_label=False, enabled_when='dosmoothing'), spring)
    zgroup = VGroup(HGroup(Item('zviewtype', show_label=False), Item('zview', editor=RangeEditor(low_name='lzview', high_name='uzview', format='%5.4g ', mode='slider'), show_label=False, springy=True)), HGroup(Item('lzview', show_label=False), Item('coarserz', show_label=False), spring, Item('zqual', style='custom', label='Z quality', editor=RangeEditor(cols=_zqh - _zql + 1, low=_zql, high=_zqh)), spring, Item('finerz', show_label=False), Item('uzview', show_label=False)))
    features_view = View(VGroup(HGroup(Item('showfeatures', label='Show'), Item('featurelocsmooth', label='Locator Smoothing'), Item('featurelocsize', label='Locator Window size')), Item('featurelist', editor=TabularEditor(adapter=FeaturesAdapter(), selected_row='selectedfeatureindex'), show_label=False), HGroup(Item('deletefeature', show_label=False, enabled_when='len(featurelist)>0 and selectedfeatureindex>=0'), Item('idfeature', show_label=False, enabled_when='len(featurelist)>0 and selectedfeatureindex>=0'), Item('recalcfeature', show_label=False, enabled_when='len(featurelist)>0 and selectedfeatureindex>=0'), Item('clearfeatures', show_label=False, enabled_when='len(featurelist)>0'))), resizable=True, title='Spylot Features')
    traits_view = View(VGroup(Include('titlegroup'), Include('speclistgroup'), Include('plotformatgroup'), Include('featuregroup'), Include('continuumgroup'), Item('plot', editor=ComponentEditor(), show_label=False, width=768), Include('zgroup')), resizable=True, title='Spectrum Plotter', handler=SpylotHandler(), key_bindings=spylotkeybindings)
    ...
    def __init__(self, specs, **traits):
        ...
        """
        pd = ArrayPlotData(x=[1], flux=[1], err=[1])
        pd.set_data('majorx', [1, 1])
        pd.set_data('majory', [0, 1])
        pd.set_data('minorx', [1, 1])
        pd.set_data('minory', [0, 1])
        pd.set_data('continuum', [0, 0])
        ...
        del plot.x_mapper.range.sources[-1]
    ...
    def _get_lzview(self):
        if self.zviewtype == 'redshift':
            return self.lowerz
        return 300000.0 * self.lowerz

    def _set_lzview(self, val):
        self.lowerz = val if self.zviewtype == 'redshift' else val / 300000.0

    def _get_uzview(self):
        if self.zviewtype == 'redshift':
            return self.upperz
        return 300000.0 * self.upperz

    def _set_uzview(self, val):
        self.upperz = val if self.zviewtype == 'redshift' else val / 300000.0

    def _get_zview(self):
        if self.zviewtype == 'redshift':
            return self.z
        return 300000.0 * self.z

    def _set_zview(self, val):
        self.z = val if self.zviewtype == 'redshift' else val / 300000.0

    def _scaleerr_changed(self, new):
        if new:
            self._scaleerrfrac_changed()
            self._update_errmapper_screen()
        self.spechanged = True
    ...
    def _rebuild_labels(self):
        ...
        if self.showlabels:
            ...
            for s, l in zip(linestrs, linelocs):
                x = l * lfact
                y = np.interp(x, xd, yd)
                dl = DataLabel(self.plot, data_point=(x, y), lines=[s], label_position='top', padding_bottom=10, arrow_visible=False)
    ...
    def _featureselmode_changed(self, old, new):
        ...
        if new == 'No Selection':
            ...
            rstool = RangeSelectionBugfix(component=fplot, left_button_selects=True, enable_resize=False, mapper=self.plot.x_mapper)
            rstool.plot = fplot
            rstool.component = pl
            rsoverlay = RangeSelectionOverlay(component=fplot, mapper=self.plot.x_mapper)
    ...
    def _update_featurelist(self):
        self.linehighlighter.extents = [ f.extent for f in self.featurelist ] if len(self.featurelist) > 0 else np.ndarray((0,
                                                                                                                            2))
...
def _get_default_lines(linetypes):
    ...
    if linetypes == 'galaxy':
        major = [
         'Ly_alpha', 'H_alpha', 'H_beta', 'H_gamma', '[OIII]_5007', '[OIII]_4959', '[OII]_3727', 'Balmer_limit', '[SII]_6717', '[SII]_6731']
        return (candidates, [ str(c) for c in candidates if c.name in major ], [ str(c) for c in candidates if c.name not in major ])
    if linetypes == 'stellar':
        major = [
         'H_alpha', 'H_beta', 'CaII_a', 'CaII_b', 'CaII_c', 'NaI_a', 'NaI_b']
