...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/coords/coordsys.py
...
class AngularCoordinate(object):
    ...
    __acregex = _re.compile('(?:([+-])?(\\d+(?:[.]\\d*)?)(hours|h|degrees|d|radians|rads|rad|r| |:(?=\\d+:\\d+[.]?\\d*$)))?(?:(\\d+(?:[.]\\d*)?)(m|\\\'|[:]| ))?(?:(\\d+(?:[.]\\d*)?)(s|"|$))?$')
    __decregex = _re.compile('[+-]?\\d+([.]\\d*)?$')
            ...
            if inpt is None:
                ...
                if decm:
                    ...
                    if acm:
                        ...
                        val = (0 if dec1 is None else float(dec1)) + (0 if dec2 is None else float(dec2) / 60) + (0 if dec3 is None else float(dec3) / 3600)
                        if sgn == '-':
                            val *= -1
                        if mark1 == ':' or mark1 == ' ':
                            if sghms is None:
    ...
    def _setDegminsec(self, dms):
        ...
        self.degrees = abs(dms[0]) + abs(dms[1]) / 60.0 + abs(dms[2]) / 3600.0
        if dms[0] < 0:
            self._decval *= -1
    ...
    def _getDegminsec(self):
        ...
        min = int(fracpart * 60.0)
        sec = fracpart * 3600.0 - min * 60.0
    ...
    def _setHrsminsec(self, dms):
        ...
        self.degrees = 15 * (dms[0] + dms[1] / 60.0 + dms[2] / 3600.0)

    def _getHrsminsec(self):
        factorized = self.degrees / 15.0
        hrs = int(factorized)
        mspart = factorized - hrs
        min = int(mspart * 60.0)
        sec = mspart * 3600.0 - min * 60.0
        return (hrs, min, sec)
    ...
    def getDmsStr(self, secform='%05.2f', sep=(unichr(176), "'", '"'), sign=True, canonical=False, inclzero=True):
        """
    ...
    def getHmsStr(self, secform=None, sep=('h', 'm', 's'), canonical=False, inclzero=True):
        """
...
class AngularSeparation(AngularCoordinate):
    ...
    def separation3d(self, zord1, zord2, usez=False, **kwargs):
        ...
        if usez:
            d1 = cosmo_z_to_dist(zord1, disttype=2, **kwargs) * 1000000.0
            d2 = cosmo_z_to_dist(zord1, disttype=2, **kwargs) * 1000000.0
        else:
            if len(kwargs) > 0:
...
class CoordinateSystem(object):
    ...
    * The :attr:`transweight` class variable can be set to determine the
    ...
    _converters = _defaultdict(dict)
    _transtypes = dict()

    @staticmethod
    def registerTransform(fromclass, toclass, func=None, transtype=None, overwrite=True):
        """
    ...
    def getTransformGraph():
        ...
        if CoordinateSystem._transgraph is None:
            ...
            for a, b in CoordinateSystem.listAllTransforms():
                avgweight = (getattr(a, 'transweight', 1) + getattr(b, 'transweight', 1)) / 2
    ...
    *Subclassing*
    ...
    def _setDistanceau(self, val):
        ...
        if val is None:
            ...
            try:
                self._dpc = (
                 float(val[0]) * pcperau, float(val[1]) * pcperau)
            except (TypeError, IndexError):
                self._dpc = (
                 float(val) * pcperau, 0)
    ...
    def matrixRotate(self, matrix, apply=True, fixrange=True, unitarycheck=False):
        ...
        if unitarycheck:
            mdagger = m.H
            rtol = 1e-05 if unitarycheck is True else unitarycheck
        ...
        if laterr != 0 or longerr != 0:
            ...
            dlongp = sqrt((dxp * yp * xp ** (-2)) ** 2 + (dyp / xp) ** 2) / (1 + (yp / xp) ** 2)
        else:
            laterr = None
        if fixrange:
        ...
        * EquatorialCoordinatesBase(ra,dec,raerr,decerr,epoch,distancepc)
        ...
        if len(args) == 1:
            if isinstance(args[0], EquatorialCoordinatesBase):
                EpochalLatLongCoordinates.__init__(self, args[0].ra, args[0].dec, args[0].raerr, args[0].decerr, args[0].epoch, args[0].distancepc)
                return
...
class ICRSCoordinates(EquatorialCoordinatesBase):
    ...
    def __makeFrameBias():
        ...
        xi0 = -16.617
        eta0 = -6.8192
        return rotation_matrix(-eta0 / 3600000, axis='x') * rotation_matrix(xi0 / 3600000, axis='y') * rotation_matrix(da0 / 3600000, axis='z')
...
def _nutation_components2000B(intime, asepoch=True):
    ...
    masecperrad = asecperrad * 1000.0
    dpsipl = -0.135 / masecperrad
    depspl = 0.388 / masecperrad
    return (
     epsa, dpsils + dpsipl, depsls + depspl)
...
def _nutation_matrix(epoch):
    ...
    return rotation_matrix(-(epsa + deps), 'x', False) * rotation_matrix(-dpsi, 'z', False) * rotation_matrix(epsa, 'x', False)
...
class CIRSCoordinates(EquatorialCoordinatesBase):
    ...
    def _CIOLocator(epoch):
        ...
        """
        from ..constants import asecperrad
        from .ephems import _mean_anomaly_of_moon, _mean_anomaly_of_sun, _mean_long_of_moon_minus_ascnode, _long_earth, _mean_elongation_of_moon_from_sun, _long_venus, _mean_long_asc_node_moon, _long_prec
        B = ICRSCoordinates.frameBiasJ2000
        P = _precession_matrix_J2000_Capitaine(epoch)
        N = _nutation_matrix(epoch)
        x, y, z = (N * P * B).A[2]
...
class EquatorialCoordinatesEquinox(EquatorialCoordinatesBase):
    ...
    def transformToEpoch(self, newepoch):
        ...
        """
        if self.epoch is not None and newepoch is not None:
            B = _nutation_matrix(self.epoch) * _precession_matrix_J2000_Capitaine(self.epoch).T
            A = _nutation_matrix(newepoch) * _precession_matrix_J2000_Capitaine(newepoch)
    ...
    def _toCIRS(eqsys):
        if eqsys.epoch is None:
            ...
            eqo = equation_of_the_origins(jd) * 15.0
...
class ITRSCoordinates(EpochalLatLongCoordinates):
    ...
    def _WMatrix(epoch):
        ...
        if ITRSCoordinates.polarmotion is None:
            ...
            if callable(yp):
                yp = yp(epoch)
        return rotation_matrix(-yp, 'x') * rotation_matrix(-xp, 'y') * rotation_matrix(sp, 'z')
    ...
    def _fromEqE(eqe):
        ...
        if epoch is not None:
            ...
            try:
                gst = greenwich_sidereal_time(jd, True) * 15.0
            except Exception as e:
                from warnings import warn
                warn('temporarily bypassing problem with greenwich_sidereal_time:%s' % e)
                gst = greenwich_sidereal_time(jd, 'simple') * 15.0
...
class FK5Coordinates(EquatorialCoordinatesEquinox):
    ...
    def _precessionMatrixJ(epoch1, epoch2):
        ...
        pz = (0.018203, -6.6e-05, 1.09468)
        z = dt * (temp + dt * (pz[2] + pz[1] * T + dt * pz[0])) / 3600
        ptheta = (-0.041833, -0.000217, -0.42665, -0.000217, -0.8533, 2004.3109)
    ...
    def _fromICRS(icrsc):
        ...
        """
        from ..utils import rotation_matrix
        eta0 = -5.527777777777777e-06
        xi0 = 2.5277777777777778e-06
        da0 = -6.3611111111111105e-06
        B = rotation_matrix(-eta0, 'x') * rotation_matrix(xi0, 'y') * rotation_matrix(da0, 'z')
        epoch = icrsc.epoch
...
class FK4Coordinates(EquatorialCoordinatesEquinox):
    ...
    def _toFK5(fk4c):
        ...
        B = np.mat([[0.9999256794956877, -0.0111814832204662, -0.0048590038153592],
         [
          0.0111814832391717, 0.9999374848933135, -2.71625947142e-05],
         [
          0.0048590037723143, -2.7170293744e-05, 0.9999881946023742]])
        if fk4c.epoch is not None and fk4c.epoch != 1950:
            jd = epoch_to_jd(fk4c.epoch, False)
            jepoch = jd_to_epoch(jd)
            T = (jepoch - 1950) / 100
            B[(0, 0)] += -2.6455262e-09 * T
            B[(0, 1)] += -1.1539918689e-06 * T
            B[(0, 2)] += 2.111134619e-06 * T
            B[(1, 0)] += 1.1540628161e-06 * T
            B[(1, 1)] += -1.29042997e-08 * T
            B[(1, 2)] += 2.36021478e-08 * T
            B[(2, 0)] += -2.1112979048e-06 * T
            B[(2, 1)] += -5.6024448e-09 * T
            B[(2, 2)] += 1.02587734e-08 * T
...
class EclipticCoordinatesCIRS(EpochalLatLongCoordinates):
    ...
    def __init__(self, lamb=0, beta=0, lamberr=None, betaerr=None, epoch=2000, distanceau=None):
        """
        See the associated attribute docstrings for the meaning of the inputs.  
        """
...
class GalacticCoordinates(EpochalLatLongCoordinates):
    ...
    def _fromFK5(fk5coords):
        ...
        mat = rotation_matrix(180 - GalacticCoordinates._long0_J2000.d, 'z') * rotation_matrix(90 - GalacticCoordinates._ngp_J2000.dec.d, 'y') * rotation_matrix(GalacticCoordinates._ngp_J2000.ra.d, 'z') * FK5Coordinates._precessionMatrixJ(epoch, 2000)
        mat.nocache = True
        return mat
    ...
    def _fromFK4(fk4coords):
        ...
        mat = rotation_matrix(180 - GalacticCoordinates._long0_B1950.d, 'z') * rotation_matrix(90 - GalacticCoordinates._ngp_B1950.dec.d, 'y') * rotation_matrix(GalacticCoordinates._ngp_B1950.ra.d, 'z') * FK4Coordinates._precessionMatrixB(epoch, 1950)
        mat.nocache = True
        return mat
...
class SupergalacticCoordinates(EpochalLatLongCoordinates):
    ...
    _longrange_ = (0, 360)
    _nsgp_gal = GalacticCoordinates(47.37, 6.32)
    ...
    def _fromGal(galcoords):
        ...
        z2r = rotation_matrix(180 - SupergalacticCoordinates._sg0_gal.l.d + SupergalacticCoordinates._nsgp_gal.l.d, 'z')
        return z2r * yr * z1r
...
try:
    ...
    graph.graph_attr.update(dict(size='12.0, 12.0', fontsize=12))
