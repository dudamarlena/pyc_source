...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/coords/ephems.py
...
class ProperMotionObject(EphemerisObject):
    ...
    def __init__(self, name, ra0, dec0, dra=0, ddec=0, epoch0=2000, distpc0=None, rv=0, coordclass=None):
        """
    ...
    def distancepc(self):
        ...
        if self.distpc0 is None:
            ...
            return self.distpc0 + self._dyr * self.rv * secperyr * 100000.0 / cmperpc
            return
    ...
    def _getCoordObj(self):
        ...
        if self.distancepc is None:
            ...
            return self.coordclass(self.ra, self.dec, distancepc=self.distancepc, epoch=jd_to_epoch(self.jd))
            return
...
class KeplerianObject(EphemerisObject):
    ...
    * :attr:`dperi`
    ...
    def getPhase(self, viewobj='Earth', illumobj='Sun'):
        ...
        xR, yR, zR = x - xv, y - yv, z - zv
        xs, ys, zs = xv - xi, yv - yi, zv - zi
        xr, yr, zr = x - xi, y - yi, z - zi
...
def _ecl_to_icrs(x, y, z, jd):
    ...
    """
    seps = 0.3977769780087639
    ceps = 0.9174821392082875
    yp = ceps * y - seps * z
    zp = seps * y + ceps * z
    return (
     x, yp, zp)
...
class Moon(KeplerianObject):
    ...
    def __init__(self):
        ...
        auperkm = aupercm * 100000.0
        degperasec = degrees(1.0 / asecperrad)
        jd4000b = calendar_to_jd((-3999, 1, 1))
        jd8000 = calendar_to_jd((8000, 1, 1))
        kw = {'name': 'Moon', 'validjdrange': (jd4000b, jd8000), 'outcoords': RectangularGCRSCoordinates, 
           'outtransfunc': _ecl_to_icrs}
        kw['a'] = (
         383397.7725 * auperkm, 0.004 * auperkm)
        kw['e'] = (0.055545526, -1.6e-08)
        kw['i'] = (5.15668983,
         -8e-05 * degperasec,
         0.02966 * degperasec,
         -4.2e-05 * degperasec,
        ...
        kw['Lp'] = (83.35324312,
         ...
         -38.2702 * degperasec,
         -0.045047 * degperasec,
         0.00021301 * degperasec)
        kw['Lan'] = (125.04455501,
         -6967919.3631 * degperasec,
         6.3602 * degperasec,
         0.007625 * degperasec,
         3.586e-05 * degperasec)
        kw['L'] = (218.31664563,
         1732559343.4847 * degperasec,
         -6.391 * degperasec,
         0.006588 * degperasec,
         -3.169e-05 * degperasec)
    ...
    def getPhase(self, viewobj=None, illumobj=None):
        ...
        if viewobj is None and illumobj is None:
            from math import sqrt
            c = self()
            xg, yg, zg = c.x, c.y, c.z
            xe, ye, ze = earth_pos_vel(self.jd, False)[0]
            xm = xe + xg
            ym = ye + yg
            zm = ze + zg
...
_shortelems = _load_jpl_orb_elems('ss_elems_1.dat')
_longelems = _load_jpl_orb_elems('ss_elems_2.dat')
_longelemsb = _load_jpl_orb_elems('ss_elems_2b.dat')

def _ecl_to_gcrs(x, y, z, jd):
    from math import sin, cos, radians
    sine = 0.3977769780087639
    cose = 0.917482139208
    xp = x
    yp = cose * y - sine * z
    zp = sine * y + cose * z
...
def _keplerian_ephems():
    ...
    for n, oes in _shortelems.iteritems():
        kw = {'name': n, 'validjdrange': (jd1800, jd2050), 'outcoords': RectangularGCRSCoordinates, 
    ...
    for n, oes in _longelems.iteritems():
        n = n + '-long'
        kw = {'name': n, 'validjdrange': (jd3000bce, jd3000ce), 'outcoords': RectangularGCRSCoordinates, 
           'outtransfunc': _ecl_to_gcrs}
        for oen, oe in zip(('a', 'e', 'i', 'L', 'Lp', 'Lan'), oes.T):
            kw[oen] = oe
...
_mean_anomaly_of_moon_poly = np.poly1d([-0.0002447,
 0.051635,
 31.8792,
 1717915923.2178,
 485868.249036])
...
_mean_anomaly_of_sun_poly = np.poly1d([-1.149e-05,
 0.000136,
 -0.5532,
 129596581.0481,
 1287104.793048])
...
_mean_long_of_moon_minus_ascnode_poly = np.poly1d([4.17e-06,
 -0.001037,
 -12.7512,
 1739527262.8478,
 335779.526232])
...
_mean_elongation_of_moon_from_sun_poly = np.poly1d([-3.169e-05,
 0.006593,
 -6.3706,
 1602961601.209,
 1072260.703692])
...
_mean_long_ascnode_moon_poly = np.poly1d([-5.939e-05,
 0.007702,
 7.4722,
 -6962890.5431,
 450160.398036])
...
def _long_prec(T):
    ...
    """
    return (0.02438175 + 5.38691e-06 * T) * T
