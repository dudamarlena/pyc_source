...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/plotting.py
...
def logerrplot(x, y, xerr=None, yerr=None, logaxes='y', boundscale=0.5, **kwargs):
    """
...
def scatter4d(x, y, c, s, xe=None, ye=None, logax=(False, False, False, False), scaling=(1, 1, 1, 1), outlines=True, cb=True, errkwargs={}, **kwargs):
    """
...
def scatter_panel(p, plims, x, y, c=None, s=20, xe=None, ye=None, logax=(False, False, False, False), scaling=(1, 1, 1, 1), pcfg=None, outlines=True, cb=True, xlab='', ylab='', plab='', xls=None, yls=None, errkwargs={}, fitline=False, tsize=14, **kwargs):
    """
    ...
    with mpl_context() as (plt):
        ...
        plims.append(max(p) + (max(p) - min(p)) / 1000)
        scats = []
        try:
        ...
        for i in range(len(plims) - 1):
            ...
            xi, yi = x[ind], y[ind]
            scats.append(scatter4d(xi, yi, ci, si, xei, yei, logax=logax, errkwargs=errkwargs, scaling=scaling, outlines=outlines, cb=False, **kwargs))
            if fitline:
                if xls:
                ...
                if logax[0] and logax[1]:
                    mask = np.logical_and(mask, np.logical_and(xi > 0, yi > 0))
                    maski = np.where(mask)
...
def scatter_density(x, y, bins=20, threshold=None, ncontours=None, contf=False, cb=False, clf=True, bingrid=False, xlog=False, ylog=False, points=False, lims=None, skwargs={}, ckwargs={}):
    """
...
def cumulative_plot(data, Nlt=True, frac=False, xlabel='x', edges=(None, None), logx=False, logy=False, doplot=True, perr=False, **kwargs):
    """
        ...
        with mpl_context() as (plt):
            ...
            if perr:
                ...
                if frac:
                    N1 = y * finalN / frac
                    N2 = finalN
                    ferr = (1.0 / N1 + 1.0 / N2) ** 0.5
                else:
                    ferr = y ** (-0.5)
                perr.setdefault('zorder', pltres.get_zorder() - 0.1)
...
def split_histograms(vals, edgevals, edges, bins=None, clf=True, colors=None, styles=None, edgename='', **kwargs):
    """
    ...
    for i in range(len(edges) - 1):
        ...
        if edgename == '':
            ...
            hstrs.append('$%.3f \\leq %s<%.3f$' % (l, edgename, u))
    ...
    lsmap = {'-': 'solid', '--': 'dashed', '-.': 'dashdot', ':': 'dotted', 'solid': 'solid', 
       'dashed': 'dashed', 'dashdot': 'dashdot', 'dotted': 'dotted'}
    with mpl_context(clf=clf) as (plt):
        maxh = -1
...
def plot_histogram(nbin, binedges, **kwargs):
    ...
    with mpl_context(clf=clf) as (plt):
        ...
        plt.plot(np.concatenate((binedges, [binedges[(-1)]])), np.concatenate(([0], nbin, [0])), **kwargs)


def ax3d_animate(azels, nframes, fn, fig=None, initazel=None, relative=True, predelete=False, verbose=False, savekwargs={}):
    """
...
def mlab_animate_rotzoom(fnbase, azs=None, els=None, rolls=None, dists=None, relative=True, fps=None, ui=False, figsize=None):
    """
...
def make_movie(frfn, moviefn, framerate=30, bitrate='200k', inops=None, outops=None, run=True):
    """
    ...
    callt = (
     inops, framerate, bitrate, ffmpeginstr, outops, moviefn)
