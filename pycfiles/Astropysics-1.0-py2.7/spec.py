...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/astropysics/spec.py
...
class HasSpecUnits(object):
    ...
    def _convertType(self, oldtype, newtype, oldscale, newscale):
        ...
        x, flux = self._unitGetX() * oldscale, self._unitGetY() * oldscale
        if newtype == 'energy':
            if oldtype == 'energy':
                fluxscale = 1
    ...
    def __xftrans(self, x, f):
        ...
        return (
         newx / self.__newscale, newf * self.__newscale)
...
class Spectrum(HasSpecUnits):
    ...
    def __getstate__(self):
        state = self.__dict__
        if type(state['_features']) is not list:
            state = dict(state)
    ...
    def getUnitFlux(self, units, err=False):
        ...
        try:
            ...
            if err == 'ivar':
                res = (
                 self.x.copy(), self.flux.copy(), self.ivar.copy())
            elif err:
                res = (
                 self.x.copy(), self.flux.copy(), self.err.copy())
            else:
                res = (
                 self.x.copy(), self.flux.copy())
        finally:
            self.unit = oldunit
    ...
    def rgbEyeColor(self):
        ...
        eyefluxes = eyefluxes[::-1]
        eyefluxes /= self._rgbsensitivity
        maxe = eyefluxes.max()
        eyefluxes /= maxe
        return tuple(eyefluxes)

    def fitContinuum(self, model=None, weighted=False, evaluate=False, interactive=False, **kwargs):
        """
    ...
    def plot(self, fmt=None, ploterrs=0.1, plotcontinuum=True, smoothing=None, step=True, clf=True, colors=('b', 'g', 'r', 'k'), restframe=True, xrng=None, **kwargs):
        """
        ...
        if ploterrs and np.any(e):
            ...
            if isMappingType(ploterrs) and 'scale' in ploterrs:
                ...
                scale = 0.1 * np.mean(y) / np.mean(e[m])
...
ABSpec = FunctionSpectrum(np.linspace(50000000000000.0, 3000000000000000.0, 1024), lambda x: np.ones_like(x) * 10 ** (-19.44), unit='hz')
...
class SpectralFeature(HasSpecUnits):
    ...
    def _applyUnits(self, xtrans, xitrans, xftrans, xfinplace):
        ...
        if self.extent is not None:
            self.extent = (
             xtrans(self.extent[0]), xtrans(self.extent[1]))
        return
        ...
        if edges == 'tweak':
            ...
            if fit:
                ...
                if xi2 == 0:
                    ...
                    dxi2 = xi2 + 1
                dx = np.convolve(spec.x[dxi1:dxi2], [0.5, 0, -0.5], mode='valid')
            ...
            mcont, mconterr = np.mean(cont), np.mean(conterr)
...
def air_to_vacuum(airwl, nouvconv=True):
    ...
    if isscal:
        airwl = airwl.ravel()
    sig2 = (10000.0 / airwl) ** 2
    convfact = 1.000064328 + 0.0294981 / (146.0 - sig2) + 0.0002554 / (41.0 - sig2)
    newwl = airwl.copy()
    if nouvconv:
        convmask = newwl >= 2000
...
def vacuum_to_air(vacwl, nouvconv=True):
    ...
    if isscal:
        vacwl = vacwl.ravel()
    wave2 = vacwl * vacwl
    convfact = 1.0002735182 + 131.4182 / wave2 + 276249000.0 / (wave2 * wave2)
...
def zfind(specobj, templates, lags=(
 0, 200), checkspec=True, checktemplates=True, verbose=True, interpolation=None):
    """
