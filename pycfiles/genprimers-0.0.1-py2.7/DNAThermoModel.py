...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/lib/Probe/Thermo/DNAThermoModel.py
# Compiled at: 2017-03-25 16:33:25
...
def _count_overlapping(subseq, seq):
    ...
    """
    return len(re.findall('(?=(%s))' % re.escape(subseq), seq))
...
class DNAThermoModel(NNModel):
    ...
    def __init__(self):
        ...
        self.dh = {'AA': -7.6, 'AT': -7.2, 'AC': -8.4, 'AG': -7.8, 'TA': -7.2, 
           'TT': -7.6, 'TC': -8.2, 'TG': -8.5, 'CA': -8.5, 
           'CT': -7.8, 'CC': -8.0, 'CG': -10.6, 'GA': -8.2, 
           'GT': -8.4, 'GC': -9.8, 'GG': -8.0}
        self.ds = {'AA': -21.3, 'AT': -20.4, 'AC': -22.4, 'AG': -21.0, 'TA': -21.3, 
           'TT': -21.3, 'TC': -22.2, 'TG': -22.7, 'CA': -22.7, 
           'CT': -21.0, 'CC': -19.9, 'CG': -27.2, 'GA': -22.2, 
           'GT': -22.4, 'GC': -24.4, 'GG': -19.9}
        self.dg = {'AA': -1.0, 'AT': -0.88, 'AC': -1.44, 'AG': -1.28, 'TA': -0.58, 
           'TT': -1.0, 'TC': -1.3, 'TG': -1.45, 'CA': -1.45, 
           'CT': -1.28, 'CC': -1.84, 'CG': -2.17, 'GA': -1.3, 
           'GT': -1.44, 'GC': -2.24, 'GG': -1.84}
        self.init_dh = 0.2
        self.init_ds = -5.7
        self.init_dg = 1.96
        self.sym_ds = -1.4
        ...
        self.end_ds = 6.9
        self.end_dg = 0.05
        self.dangle5_dh = {'AA': +0.2, 'CA': +0.6, 'GA': -1.1, 'TA': -6.9, 'AC': -6.3, 
           'CC': -4.4, 'GC': -5.1, 'TC': -4.0, 'AG': -3.7, 
           'CG': -4.0, 'GG': -3.9, 'TG': -4.9, 'AT': -2.9, 
           'CT': -4.1, 'GT': -4.2, 'TT': -0.2}
        self.dangle5_dg = {'AA': -0.51, 'CA': -0.42, 'GA': -0.62, 'TA': -0.71, 'AC': -0.96, 
           'CC': -0.52, 'GC': -0.72, 'TC': -0.58, 'AG': -0.58, 
           'CG': -0.34, 'GG': -0.56, 'TG': -0.61, 'AT': -0.5, 
           'CT': -0.02, 'GT': +0.48, 'TT': -0.1}
        self.dangle5_ds = {k:(self.dangle5_dh[k] - self.dangle5_dg[k]) / self.Tref for k in self.pairs}
        self.dangle3_dh = {'AA': -0.5, 'AC': +4.7, 'AG': -4.1, 'AT': -3.8, 'CA': -5.9, 
           'CC': -2.6, 'CG': -3.2, 'CT': -5.2, 'GA': -2.1, 
           'GC': -0.2, 'GG': -3.9, 'GT': -4.4, 'TA': -0.7, 
           'TC': +4.4, 'TG': -1.6, 'TT': +2.9}
        self.dangle3_dg = {'AA': -0.12, 'AC': +0.28, 'AG': -0.01, 'AT': -0.13, 'CA': -0.82, 
           'CC': -0.31, 'CG': -0.01, 'CT': -0.52, 'GA': -0.92, 
           'GC': -0.23, 'GG': -0.44, 'GT': -0.35, 'TA': -0.48, 
           'TC': -0.19, 'TG': -0.5, 'TT': -0.29}
        self.dangle3_ds = {k:(self.dangle3_dh[k] - self.dangle3_dg[k]) / self.Tref for k in self.pairs}

    def calc_energies(self, s, T=310.15, saltc=None):
        ...
        if str(s) != str(cs.reverse_complement()):
            ...
            if cs.endswith('-'):
                ...
                dS += self.dangle5_ds[pair]
                s = s[1:]
        nncounts = {pair:_count_overlapping(pair, str(s)) for pair in self.pairs}
    ...
    def calc_homodimer(self, seq, na, mg, c, temp):
        ...
        """
        return calcHomodimer(seq, mv_conc=na * 1000, dv_conc=mg * 1000, dna_conc=c * 1000000000.0, temp_c=temp).dg / 1000
    ...
    def calc_heterodimer(self, seq1, seq2, na, mg, c, temp):
        ...
        module. The results are in Kcal/mol
        """
        return calcHeterodimer(seq1, seq2, mv_conc=na * 1000, dv_conc=mg * 1000, temp_c=temp, dna_conc=c * 1000000000.0).dg / 1000
    ...
    def calc_hairpin(self, seq, na, mg, c, temp):
        ...
        module. The results are in Kcal/mol
        """
        return calcHairpin(seq, mv_conc=na * 1000, dv_conc=mg * 1000, temp_c=temp, dna_conc=c * 1000000000.0).dg / 1000

    def calc_sdss(self, dna_seq, f_threshold=0.01, cpo=2e-07, temp=310.15, saltc=None):
...
class DNAThermoModelUnittest(unittest.TestCase):
    ...
    def test_calc_energies(self):
        ...
        self.assertEqual(dmodel.calc_energies(Seq('GTCTACC')), (-47.8, -0.1349, -5.960764999999995))
        self.assertEqual(dmodel.calc_energies(Seq('GTCTACCA')), (-54.09999999999999,
                                                                 -0.1507, -7.36039499999999))
        self.assertEqual(dmodel.calc_energies(Seq('GTCTAGAC')), (-55.79999999999999,
                                                                 -0.1596, -6.300059999999995))
        self.assertEqual(dmodel.calc_energies(Seq('TGTCTAGACA')), (-68.39999999999999,
                                                                   -0.19119999999999998,
                                                                   -9.099319999999999))
        self.assertEqual(dmodel.calc_energies(Seq('GTCTACC'), saltc=0.5), (-47.8, -0.13643046897467637,
                                                                           -5.486090047504128))

    def test_calc_melting(self):
        dmodel = DNAThermoModel()
        self.assertEqual(dmodel.calc_melting(Seq('GTCTACC')), -3.9650586677994397)

    def test_calc_homodimer(self):
        dmodel = DNAThermoModel()
        self.assertEqual(dmodel.calc_homodimer('GTCTACC', 0.05, 0, 2e-09, 37), -0.3060264978145642)

    def test_calc_heterodimer(self):
        dmodel = DNAThermoModel()
        self.assertEqual(dmodel.calc_heterodimer('GTCTACC', 'ATCTAGG', 0.05, 0, 2e-09, 37), -1.1131764978176615)

    def test_calc_hairpin(self):
        dmodel = DNAThermoModel()
        self.assertEqual(dmodel.calc_hairpin('GTCTACC', 0.05, 0, 2e-09, 37), 0.0)
    ...
    def test_get_sdss_region(self):
        ...
        for i in range(0, len(seqs)):
            self.assertEqual(dmodel.calc_sdss(seqs[i], temp=333.15, saltc=1), starts[i])