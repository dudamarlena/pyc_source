...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/lib/Report/Reporter.py
# Compiled at: 2017-03-26 21:40:45
import plotly.offline as po, plotly.graph_objs as go, os.path as op
...
class ReportGenerator:
    ...
    def __init__(self, args, primers_set, targets_records):
        ...
        self.report_template_folder = op.join(main_path, 'templates')
        ...
        copy_tree(op.join(self.report_template_folder, 'site_libs'), op.join(self.report_dir, 'site_libs'))
    ...
    def generate_settings_report(self):
        ...
        thermo_info = [
         [
          'NaCl contentration:', str(self.args.na) + ' M'],
         [
        ...
        for pos in range(1, len(thermo_info) + 1):
            if pos % 2 == 0:
                ...
                row = 'odd'
            thermo.append(dict(tr_class=row, parameter=thermo_info[(pos - 1)][0], value=thermo_info[(pos - 1)][1]))
        ...
        for pos in range(1, len(filt_info) + 1):
            if pos % 2 == 0:
                ...
                row = 'odd'
            filt.append(dict(tr_class=row, parameter=filt_info[(pos - 1)][0], value=filt_info[(pos - 1)][1]))

        pairing_info = [
         ['Edit distance for primer alignment:',
        ...
        for pos in range(1, len(pairing_info) + 1):
            if pos % 2 == 0:
                ...
                row = 'odd'
            pairing.append(dict(tr_class=row, parameter=pairing_info[(pos - 1)][0], value=pairing_info[(pos - 1)][1]))

        output_info = [
         ['Maximum number of primers pairs reported:',
        ...
        for pos in range(1, len(output_info) + 1):
            if pos % 2 == 0:
                ...
                row = 'odd'
            output.append(dict(tr_class=row, parameter=output_info[(pos - 1)][0], value=output_info[(pos - 1)][1]))

        info_sets.append(dict(id='thermodynamic-settings', name='Thermodynamic settings', info=thermo))
        info_sets.append(dict(id='primers-filtering', name='Primers filtering settings', info=filt))
        info_sets.append(dict(id='primers-pairing', name='Primers pairing settings', info=pairing))
        info_sets.append(dict(id='output-options', name='Output settings', info=output))
        items = dict(output_name=output_name, universe_file=universe_file, targets=targets, info_sets=info_sets)
    ...
    def generate_pairs_report(self):
        ...
        tmp_res = [
         [
          'Total number of primers pairs generated:',
          str(self.n_total_pairs)],
         [
          "Pairs filtered because they don't detect" + ' the minimum fraction of targets:',
        ...
        for pair in self.ppairs:
            ...
            if flag == 0:
                ...
                display = 'none'
            tmp_pair = dict(id=pair.id, a=pa.id, b=pb.id, mean_amplicon=str(int(pair.mean_amp_len)), inter_energy=pair.inter_energy, melt_diff=pair.met_diff, display=display, figures=[])
            for record in self.targets_records:
                ...
                if len(pa_algs) != 0 and len(pb_algs) != 0:
                    ...
                    pa_desc = None
                    pb_desc = None
                output_html = op.join(self.figures_dir, pair.id + '_' + record.id + '.html')
                self.draw_primers(pa_desc, pb_desc, len(record), output_html)
                tmp_figure = dict(target=record.id, path=op.join('figures', op.basename(output_html)))
                tmp_pair['figures'].append(tmp_figure)

            pairs_info.append(tmp_pair)

        items = dict(pairs_info=pairs_info, pairs_res=pairs_res, output_name=output_name)
        html_out = template.render(items)
        return html_out
    ...
    def draw_primers(self, pa_desc, pb_desc, seq_len, output_file):
        ...
        if pa_desc is not None and pb_desc is not None:
            ...
            layout['shapes'].append({'type': 'path', 
               'path': a_path, 
        ...
        trace0 = go.Scatter(x=[
         a_lab_x_pos, b_lab_x_pos, amp_len_pos], y=[
         arrow_height * 1.2, arrow_height * 1.2, arrow_height / 2 * 1.3], text=[
         pa_desc[0],
         pb_desc[0],
         'amp size: ' + str(amp_len)], mode='text', hoverinfo='none')
        data = [
         trace0]
        fig = {'data': data, 
           'layout': layout}
        ...
        with open(url.replace('file://', ''), 'r') as (file):
            ...
            tempHTML = tempHTML.replace('modeBarButtonsToRemove:[]', 'modeBarButtonsToRemove:["' + 'sendDataToCloud", "pan2d",' + ' "select2d", "lasso2d",' + ' "hoverClosestCartesian", ' + '"hoverCompareCartesian"]')
            tempHTML = tempHTML.replace('"linkText": "Export to plot.ly",' + ' "showLink": true', '"linkText": "", "showLink": false')
    ...
    def get_svg_path(self, p_desc, arrow_height):
        ...
        if p_desc[3] == '+':
            ...
            head_end = p_desc[1]
            mid_arrow = arrow_height / 2
        path = 'M ' + str(body_start) + ',0 L' + str(body_end) + ',0 L' + str(head_end) + ',' + str(mid_arrow) + ' L' + str(body_end) + ',' + str(arrow_height) + ' L' + str(body_start) + ',' + str(arrow_height) + ' Z'
        return path
    ...
    def generate_primers_report(self):
        ...
        tmp_res = [
         [
          'Total number of primers generated:',
        ...
        for primer in primers:
            ...
            for i in range(len(tmp)):
                ...
                if row % 2 == 0:
                    ...
                    row_class = 'odd'
                info.append(dict(row_class=row_class, parameter=tmp[i][0], value=tmp[i][1]))

            primers_info.append(dict(id=primer_ob.id, info=info, display=display))

        final_info = dict(output_name=output_name, primers_res=primers_res, primers_info=primers_info)
