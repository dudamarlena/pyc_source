...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/lib/Probe/ProbeSet.py
# Compiled at: 2017-03-25 17:57:08
...
class ProbeSet(dict):
    ...
    def __init__(self, probes_set):
        ...
        self.pb_filtered = {'keep': 0, 'n': 0, 
           'mt': 0, 
           'gc': 0, 
           'lcc': 0, 
           'ghm': 0, 
           'gss': 0}
        self.pp_filtered = {'keep': 0, 'frac': 0, 
           'amp': 0, 
           'int': 0}
    ...
    def filter_probes(self, na, mg, op_tp, min_lcc, min_mt, max_mt, min_gc, max_gc, thr_gss, thr_ghm, prim_con):
        ...
        for probe_id in self.probes_set:
            ...
            if lcc < min_lcc:
                ...
                continue
            dghm = self.thermo_model.calc_homodimer(str(tmp_probe.seq), na, mg, prim_con, op_tp)
            if dghm < thr_ghm:
                self.pb_filtered['ghm'] += 1
                continue
            dgss = self.thermo_model.calc_hairpin(str(tmp_probe.seq), na, mg, prim_con, op_tp)
            if dgss < thr_gss:
        ...
        logger.info('  ' + str(self.pb_filtered['n']) + ' were filtered by' + 'N bases')
        logger.info('  ' + str(self.pb_filtered['mt']) + ' were filtered by' + ' melting temperature')
        logger.info('  ' + str(self.pb_filtered['gc']) + ' were filtered by' + ' GC percentage')
        logger.info('  ' + str(self.pb_filtered['lcc']) + ' were filtered by' + ' low complexity')
        logger.info('  ' + str(self.pb_filtered['ghm']) + ' were filtered by' + ' homodimer formation')
        logger.info('  ' + str(self.pb_filtered['gss']) + ' were filtered by' + ' hairpin formation')
    ...
    def get_sdss_regions(self, op_tm, na, sdss_t, con):
        ...
        for probe_id in self.probes_set:
            ...
            pb_obj.sdss_start = self.thermo_model.calc_sdss(pb_obj.seq, f_threshold=sdss_t, cpo=con, temp=op_tm + 273.15, saltc=na)
            if pb_obj.sdss_start is None:
                logger.warning('SDSS could not be defined for probe ' + pb_obj.id)
    ...
    def assign_coords(self, fasta_indx, mm_threshold):
        ...
        logger.info('aligning SDSS region of each filtered primer against ' + ' the universe of sequences')
    ...
    def def_primers_pairs(self, targets_records, min_targets_frac, min_amp_size, max_amp_size, dghm_threshold, na, mg, prim_con, temp, max_pairs):
        ...
        for i in range(0, n_probes - 1):
            ...
            for j in range(i + 1, n_probes):
                ...
                tmp_pp = ProbePair('pair_' + tmp_probe_a.id + '_' + tmp_probe_b.id, tmp_probe_a, tmp_probe_b)
                candidates = tmp_pp.are_candidates(targets_ids, min_targets_frac, min_amp_size, max_amp_size, na, mg, prim_con, temp, dghm_threshold)
                if candidates == 0:
        ...
        logger.info('  ' + str(self.pp_filtered['frac']) + ' pairs were' + ' discarded beacuse they did not cover the minimum' + ' fraction of targets')
        logger.info('  ' + str(self.pp_filtered['amp']) + ' pairs were discarded' + ' because they generated weird amplicons')
        logger.info('  ' + str(self.pp_filtered['int']) + ' pairs were discarded' + ' because they were able to interact one each other')
        logger.info('  ' + str(self.pp_filtered['keep']) + ' pairs were kept,' + ' but the best ' + str(max_pairs) + ' pairs will be reported' + ' due to the -r flag')
        if len(all_pairs) > 0:
...
class ProbePairUnittest(unittest.TestCase):
    ...
    def test_filter_probes(self):
        ...
        self.assertEqual(ps.filter_probes(0.05, 0, 37, 1.5, -18, -20, 30, 60, 0, -2.5, 5e-08), {'n': 0, 'mt': 2, 'gc': 2, 
           'lcc': 1, 'ghm': 2, 
           'gss': 0})

    def test_get_sdss_region(self):
        seqs = [
         Seq('GACGCATGCTCCTGATACTTCCAATAAT'),
        ...
        ps = ProbeSet(probes_set)
        self.assertEqual(ps.get_sdss_regions(60, 1, 0.01, 2e-07), [
         12, 12, 11, 11, 10, 10, 10])

    def test_assign_coords(self):
        from Probes import Probe
        index = '/genoma/homes/ddiaz/diego_projects/mathomics_software/' + 'genprimers/test/sequence.fa'
        seqs = [Seq('CCTGCA'),
         Seq('ATGCATGC'),
         Seq('TTGGGGCAT')]
        probes_set = dict()
