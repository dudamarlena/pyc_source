...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/lib/Probe/Probes.py
# Compiled at: 2017-03-25 17:35:48
...
class Probe:
    ...
    def add_alignment(self, seq_id, start, end, strand, mm):
        ...
        if start >= end:
            ...
            raise ValueError('Number of mismatches grater than sequence' + +' length (%s:%d > %s:%d)' % (seq_id, mm,
             self.id,
             self.seq_len))
        else:
    ...
    def to_fasta(self, add_info=True, sdss=False):
        ...
        """
        if add_info:
            fasta = '>' + self.id + ' melt_tmp=' + str(self.mt) + ' gc_cont=' + str(self.gc) + ' lcc=' + str(self.lcc) + ' dghm=' + str(self.dghm) + ' dgss=' + str(self.dgss) + '\n'
        else:
            fasta = '>' + self.id + '\n'
        flag = True
        bg = 0
...
class ProbesUnittest(unittest.TestCase):
    ...
    def test_to_fasta(self):
        ...
        pb.sdss_start = 3
        self.assertEqual(pb.to_fasta(), '>id melt_tmp=None gc_cont=None' + ' lcc=None dghm=None dgss=None\nATCGATTACG')
        self.assertEqual(pb.to_fasta(add_info=False), '>id\nATCGATTACG')
        self.assertEqual(pb.to_fasta(sdss=True), '>id melt_tmp=None gc_cont=None' + ' lcc=None dghm=None dgss=None\nGATTACG')
        self.assertEqual(pb.to_fasta(add_info=False, sdss=True), '>id\nGATTACG')

    def test_to_tsv(self):
        pb = Probe('id', 'ATCGATTACG')
        self.assertEqual(pb.to_tsv(), 'id\tATCGATTACG\tNone' + '\tNone\tNone\tNone\tNone\tNone')