...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pinocchio/spec.py
...
"""
import doctest, inspect, os, re, types, unittest
try:
...
def camel2word(string):
    ...
    return string[0] + re.sub('([A-Z])', wordize, string[1:])
...
def testContext(test):
    if isinstance(test.test, nose.case.FunctionTestCase) and test.test.descriptor is not None:
...
def in_color(color, text):
    ...
    """
    return ('').join('%s%s%s' % (colors[color], line, color_end) for line in text.splitlines(True))
...
class Spec(Plugin):
    ...
    def options(self, parser, env=os.environ):
        ...
        parser.add_option('--spec-color', action='store_true', dest='spec_color', default=env.get('NOSE_SPEC_COLOR'), help='Show coloured (red/green) output for specifications [NOSE_SPEC_COLOR]')
        parser.add_option('--spec-doctests', action='store_true', dest='spec_doctests', default=env.get('NOSE_SPEC_DOCTESTS'), help='Include doctests in specifications [NOSE_SPEC_DOCTESTS]')
