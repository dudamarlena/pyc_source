...
# [GCC 8.4.0]
# Embedded file name: /usr/home/sasha/workspace/toCSS/Python version/toCSS.py
# Compiled at: 2012-01-24 22:36:59
"""
- toCSS
    ...
    def format_colors(self, color):
        ...
        def to_short_hex(text):
            """
                        ...
                        - @return {string} str
                        """
            return ''.join('#' + i[1::2] for i in re.findall('#[\\da-fA-F]{6}\\b', text) if i[1::2] == i[2::2]) or text
        ...
        def rgb_to_hex(text):
            """
                        ...
                        - @return {string} str
                        """
            return re.sub('rgb\\s*\\({0},{0},{0}\\)'.format('\\s*(\\d{1,3})\\s*'), lambda match: '#%s' % ''.join('%02X' % int(i) for i in match.groups()), text)
