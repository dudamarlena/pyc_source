...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pathlib_mate-project/pathlib_mate/pathlib2.py
# Compiled at: 2020-03-14 13:51:38
# Size of source mod 2**32: 59799 bytes
try:
    import typing
except:
    pass

import ctypes, fnmatch, functools, io, ntpath, os, posixpath, re, six, sys
...
def _py2_fsencode(parts):
    assert six.PY2
    return [part.encode('ascii') if isinstance(part, six.text_type) else part for part in parts]
...
def _win32_get_unique_path_id(path):
    ...
    if success == 0:
        raise WinError()
    return (
     info.volume, info.index_hi, info.index_lo)
...
class _WindowsFlavour(_Flavour):
    ...
    reserved_names = set(['CON', 'PRN', 'AUX', 'NUL']) | set(['COM%d' % i for i in range(1, 10)]) | set(['LPT%d' % i for i in range(1, 10)])
...
class PurePath(object):
    __doc__ = "PurePath represents a filesystem path and offers operations which\n    don't imply any actual filesystem I/O.  Depending on your system,\n    instantiating a PurePath will return either a PurePosixPath or a\n    PureWindowsPath object.  You can also instantiate either of these classes\n    directly, regardless of your system.\n    "
        ...
        for a in args:
            if isinstance(a, PurePath):
                ...
                if isinstance(a, str):
                    ...
                    parts.append(a.encode(sys.getfilesystemencoding() or 'ascii'))
                else:
    ...
    def relative_to(self, *other):
        ...
        if root or drv if n == 0 else cf(abs_parts[:n]) != cf(to_abs_parts):
            ...
            raise ValueError('{0!r} does not start with {1!r}'.format(str(self), str(formatted)))
...
class Path(PurePath, AttrAccessor, HashesMethods, PathFilters, MutateMethods, ToolBox):
    ...
    def open(self, mode='r', buffering=-1, encoding=None, errors=None, newline=None):
        ...
        if self._closed:
            self._raise_closed()
        if sys.version_info >= (3, 3):
            return io.open((str(self)),
              mode, buffering, encoding, errors, newline, opener=(self._opener))
        else:
    ...
    def write_bytes(self, data):
        ...
        if not isinstance(data, six.binary_type):
            raise TypeError('data must be %s, not %s' % (
    ...
    def replace(self, target):
        ...
        """
        if sys.version_info < (3, 3):
            raise NotImplementedError('replace() is only available with Python 3.3 and later')
        if self._closed:
            self._raise_closed()
