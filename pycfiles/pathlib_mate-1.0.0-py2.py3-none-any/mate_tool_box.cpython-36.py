...
# [GCC 8.4.0]
# Embedded file name: /Users/sanhehu/Documents/GitHub/pathlib_mate-project/pathlib_mate/mate_tool_box.py
# Compiled at: 2020-03-14 12:17:18
# Size of source mod 2**32: 10338 bytes
import os, six, hashlib, autopep8
...
class ToolBox(ToolBoxZip):
    ...
    def print_big_dir(self, top_n=5):
        ...
        size_table = sorted([(p, p.dirsize) for p in self.select_dir(recursive=False)],
          key=(lambda x: x[1]),
          reverse=True)
    ...
    def print_big_file(self, top_n=5):
        ...
        """
        self.assert_is_dir_and_exists()
        size_table = sorted([(p, p.size) for p in self.select_file(recursive=True)],
          key=(lambda x: x[1]),
          reverse=True)
        for p, size in size_table[:top_n]:
            print('{:<9}    {:<9}'.format(repr_data_size(size), p.abspath))
    ...
    def print_big_dir_and_big_file(self, top_n=5):
        ...
        """
        self.assert_is_dir_and_exists()
        size_table1 = sorted([(p, p.dirsize) for p in self.select_dir(recursive=False)],
          key=(lambda x: x[1]),
          reverse=True)
        for p1, size1 in size_table1[:top_n]:
            print('{:<9}    {:<9}'.format(repr_data_size(size1), p1.abspath))
            size_table2 = sorted([(p, p.size) for p in p1.select_file(recursive=True)],
              key=(lambda x: x[1]),
              reverse=True)
