...
# Compiled at: 2020-04-25 14:39:52
from .forms import *
from .models import *
    ...
    if request.method == 'POST':
        if form_busqueda.is_valid():
            ...
            if tipo_documento == 'F':
                ...
                if ventas_ligas:
                    ventas = VentasDocumento.objects.filter(Q(estado='F', folio__in=pedidos) | Q(id__in=ventas_ligas)).exclude(estado__in=['C']).only('id', 'descripcion', 'folio', 'cliente', 'vendedor')
                else:
                    ventas = VentasDocumento.objects.filter(estado='F', folio__in=pedidos).exclude(estado__in=['C']).only('id', 'descripcion', 'folio', 'cliente', 'vendedor')
            elif tipo_documento == 'R':
                ventas_surtidas = VentasDocumento.objects.filter(folio__in=pedidos, estado='S')
                ventas_ligas = VentasDocumentoLiga.objects.filter(factura__in=ventas_surtidas, devolucion__estado='F').values_list('factura__id')
                ligas = [ ven[0] for ven in ventas_ligas ]
                if ventas_ligas:
                    ventas = VentasDocumento.objects.filter(folio__in=pedidos, estado='S').exclude(estado__in=['C']).exclude(id__in=ventas_ligas).only('id', 'descripcion', 'folio', 'cliente', 'vendedor')
                else:
                    ventas = VentasDocumento.objects.filter(folio__in=pedidos, estado='S').exclude(estado__in=['C']).only('id', 'descripcion', 'folio', 'cliente', 'vendedor')
            else:
                ventas = VentasDocumento.objects.filter(folio__in=pedidos).exclude(estado__in=['C', 'S', 'F']).only('id', 'descripcion', 'folio', 'cliente', 'vendedor')
            if busqueda:
                if len(busqueda) >= 8:
...
def get_pedido(folio):
    pedido = first_or_none(Pedidos_Crm.objects.filter(folio=folio))
    return pedido
