...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/hcpre/workflows.py
# Compiled at: 2014-06-10 20:29:48
import nipype.interfaces.io as nio, nipype.interfaces.utility as util, nipype.pipeline.engine as pe
...
class HCPrepWorkflow(pe.Workflow):
    ...
    def update_nodes_from_config(self):
        ...
        self.hc_pre_fs.inputs.full_command, self.hc_fs.inputs.full_command, self.hc_post_fs.inputs.full_command, self.hc_volume.inputs.full_command, self.hc_surface.inputs.full_command = get_hcp_commands_for_config(self.hc_config)
    ...
    def subjects_node(self):
        if not getattr(self, '_subjects_node', None):
            self._subjects_node = pe.Node(name='subs_node', interface=util.IdentityInterface(fields=[
             'subject']))
    ...
    def dicom_grabber(self):
        if not getattr(self, '_dicom_grabber', None):
            self._dicom_grabber = pe.Node(name='dicom_source_1', interface=nio.DataGrabber(infields=[
             'subject'], outfields=[
             'dicom']))
    ...
    def hc_volume(self):
        if not getattr(self, '_hc_volume', None):
            self._hc_volume = pe.MapNode(name='volume', interface=VolumeProcessing(), iterfield=[
             'bold_name',
             'bold_img',
             'bold_scout',
             'se_fieldmap_pos',
    ...
    def hc_surface(self):
        if not getattr(self, '_hc_surface', None):
            self._hc_surface = pe.MapNode(name='surface', interface=SurfaceProcessing(), iterfield=[
             'bold_name',
             'subject'])
