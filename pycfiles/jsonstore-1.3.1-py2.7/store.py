...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/jsonstore/store.py
# Compiled at: 2013-03-01 04:39:47
...
class EntryManager(object):
    ...
    def update(self, entry=None, old=None, condition=lambda existing: True, **kwargs):
        """
        ...
        id_ = entry['__id__']
        with self.lock[id_]:
            existing = self._load_entry(id_)
            if old is not None and old != existing or not condition(existing):
    ...
    def search(self, obj=None, size=None, offset=0, count=False, **kwargs):
        ...
        conn = self.pool.connect()
        if count:
            result = conn.execute('SELECT COUNT(*) FROM (%s) AS ITEMS' % query, tuple(params)).fetchone()[0]
            conn.close()
            return result
        else:
    ...
    def _store_entry(self, entry):
        ...
        indexes = [ (entry['__id__'], entry['__updated__'], k, v) for k, v in flatten(entry) if k != '__id__'
                  ]
        conn = self.pool.connect()
