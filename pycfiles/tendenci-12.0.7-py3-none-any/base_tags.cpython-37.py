...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/base/templatetags/base_tags.py
# Compiled at: 2020-03-30 17:48:03
# Size of source mod 2**32: 26567 bytes
import re
...
class GoogleCMapsURL(Node):
    ...
    def render(self, context):
        ...
        if self.origin_point:
            ...
            origin = None
        url = '{base_url}?center={lat}%2C{lng}&size={size}&markers={markers}%7C{lat}%2C{lng}'.format(base_url=GOOGLE_SMAPS_BASE_URL,
        ...
        if origin:
            url = url + '&markers={markers_origin}%7C{origin_lat}%2C{origin_lng}'.format(markers_origin=(self.markers_origin.replace(':', '%3A').replace('|', '%7C')),
...
class PhotoImageURL(Node):
    ...
    def render(self, context):
        ...
        if self.crop:
            ...
            return photo.pk or static(settings.DEFAULT_IMAGE_URL)
...
class NonHashedTagsNode(TagsForObjectNode):
    ...
    def render(self, context):
        context[self.context_var] = Tag.objects.get_for_object(self.obj.resolve(context)).exclude(name__contains='#')
        return ''
...
class HashedTagsNode(TagsForObjectNode):
    ...
    def render(self, context):
        context[self.context_var] = Tag.objects.get_for_object(self.obj.resolve(context)).filter(name__contains='#')
        return ''
