...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/locations/templatetags/locations_tags.py
...
def location_options(context, user, location):
    context.update({'opt_object':location, 
     'user':user})
    return context
...
def location_nav(context, user, location=None):
    context.update({'nav_object':location, 
     'user':user})
    return context
...
def location_current_app(context, user, location=None):
    context.update({'app_object':location, 
     'user':user})
    return context
