...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/photos/templatetags/photo_tags.py
# Compiled at: 2020-02-26 14:48:40
# Size of source mod 2**32: 8059 bytes
...
class PublicPhotosNode(Node):
    ...
    def render(self, context):
        ...
        if use_pool:
            queryset = Pool.objects.filter(photo__is_public=True).select_related('photo')
        else:
...
def photo_options(context, user, photo):
    context.update({'opt_object':photo, 
     'user':user})
    return context
...
def photo_nav(context, user, photo=None):
    context.update({'nav_object':photo, 
     'user':user})
    return context
...
def photo_current_app(context, user, photo=None):
    context.update({'app_object':photo, 
     'user':user})
    return context
...
def photo_set_options(context, user, photo_set):
    context.update({'opt_object':photo_set, 
     'user':user})
    return context
...
def photo_set_nav(context, user, photo_set=None):
    context.update({'nav_object':photo_set, 
     'user':user})
    return context
