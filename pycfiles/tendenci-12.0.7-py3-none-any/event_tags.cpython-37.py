...
# [GCC 8.4.0]
# Embedded file name: /Users/jennyq/.pyenv/versions/venv_t12/lib/python3.7/site-packages/tendenci/apps/events/templatetags/event_tags.py
# Compiled at: 2020-02-26 14:48:40
# Size of source mod 2**32: 18097 bytes
...
from tendenci.apps.events.utils import registration_earliest_time, registration_has_started, registration_has_ended
...
def event_options(context, user, event):
    context.update({'opt_object':event, 
     'user':user})
    return context
...
def event_nav(context, user, event=None):
    ...
    context.update({'nav_object':event, 
     'user':user, 
...
def event_current_app(context, user, event=None):
    context.update({'app_object':event, 
     'user':user})
    return context
...
def registrant_options(context, user, registrant):
    context.update({'opt_object':registrant, 
     'user':user})
    return context
...
def registration_pricing_and_button(context, event, user):
    ...
    if hasattr(user, 'registrant_set'):
        is_registrant = user.registrant_set.filter(registration__event=event).exists()
...
def file_detail(context, attachment):
    context.update({'file': attachment})
    return context
...
class IsRegisteredUserNode(Node):
    ...
    def render(self, context):
        ...
        if isinstance(user, AnonymousUser):
            ...
            exists = Registrant.objects.filter(registration__event=event,
              email=(user.email),
              cancel_dt=None).exists()
...
class ListEventsNode(ListNode):
    ...
    def render(self, context):
        ...
        if 'user' in self.kwargs:
                ...
                if isinstance(context['user'], User):
                    ...
                    if 'limit' in self.kwargs:
                        ...
                        if user.is_authenticated:
                            items = user.profile.is_superuser or items.distinct()
                    if event_type:
                        if ',' in event_type:
            ...
            elif hasattr(self.model, 'groups'):
                if group:
                    ...
                    if order:
                        if order == 'next_upcoming':
                            ...
                            if order == 'current_and_upcoming':
                                if not start_dt:
                                    now = datetime.now().replace(second=0, microsecond=0)
                                    items = items.filter(Q(start_dt__gt=now) | Q(end_dt__gt=now))
                                items = items.order_by('start_dt', '-priority')
                            else:
                                if order == 'current_and_upcoming_by_hour':
                                    now = datetime.now().replace(second=0, microsecond=0)
