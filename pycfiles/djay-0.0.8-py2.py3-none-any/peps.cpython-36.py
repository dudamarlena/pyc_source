...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/transforms/peps.py
    ...
    def apply(self):
            ...
            for field in header:
                if field[0].astext().lower() == 'pep':
                    ...
                    try:
                        ...
                        pep = value
                        cvs_url = None
                        msg = self.document.reporter.warning(('"PEP" header must contain an integer; "%s" is an invalid value.' % pep),
                          base_node=field)
        ...
        for field in header:
            ...
            if len(body) > 1:
                ...
                if len(body) == 1:
                    if not isinstance(body[0], nodes.paragraph):
                        raise DataError('PEP header field body may only contain a single paragraph:\n%s' % field.pformat(level=1))
            ...
            if name == 'author':
                ...
                if name in ('replaces', 'replaced-by', 'requires'):
                    newbody = []
                    space = nodes.Text(' ')
                    for refpep in re.split(',?\\s+', body.astext()):
                        pepno = int(refpep)
                        newbody.append(nodes.reference(refpep,
                          refpep, refuri=(self.document.settings.pep_base_url + self.pep_url % pepno)))
        ...
        if self.pep_table:
            if self.entry == 2:
                if len(node) == 1:
                    node['classes'].append('num')
                    p = node[0]
                    if isinstance(p, nodes.paragraph):
                        if len(p) == 1:
                            text = p.astext()
                            try:
                                pep = int(text)
...
def mask_email(ref, pepno=None):
    ...
    if ref.hasattr('refuri'):
        if ref['refuri'].startswith('mailto:'):
