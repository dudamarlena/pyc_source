...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pluggy/pluggy.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 29862 bytes
"""
...
class HookspecMarker:
    __doc__ = ' Decorator helper class for marking functions as hook specifications.\n\n    You can instantiate it with a project_name to get a decorator.\n    Calling PluginManager.add_hookspecs later will discover all marked functions\n    if the PluginManager uses the same project_name.\n    '
...
class HookimplMarker:
    __doc__ = ' Decorator helper class for marking functions as hook implementations.\n\n    You can instantiate with a project_name to get a decorator.\n    Calling PluginManager.register later will discover all marked functions\n    if the PluginManager uses the same project_name.\n    '
    ...
    def __call__(self, function=None, hookwrapper=False, optionalhook=False, tryfirst=False, trylast=False):
        ...
        def setattr_hookimpl_opts(func):
            setattr(func, self.project_name + '_impl', dict(hookwrapper=hookwrapper, optionalhook=optionalhook, tryfirst=tryfirst,
              trylast=trylast))
            return func
...
if not _py3:
    ...
    class PluginManager(object):
        ...
        def parse_hookimpl_opts(self, plugin, name):
            ...
            if res is None:
                if self._implprefix:
                    if name.startswith(self._implprefix):
                        res = {}
            return res
                    ...
                    if not hook.has_spec():
                        for hookimpl in hook._wrappers + hook._nonwrappers:
                            if not hookimpl.optionalhook:
                                raise PluginValidationError('unknown hook %r in plugin %r' % (
        ...
        Return the number of loaded plugins. """
            ...
            for ep in iter_entry_points(entrypoint_name):
                if not self.get_plugin(ep.name):
    ...
    class _HookRelay:
        __doc__ = ' hook holder object for performing 1:N hook calls where N is the number\n    of registered plugins.\n\n    '
    ...
    class _HookCaller(object):
        ...
        methods using the specified kwargs as call parameters. """
            old = (
             list(self._nonwrappers), list(self._wrappers))
