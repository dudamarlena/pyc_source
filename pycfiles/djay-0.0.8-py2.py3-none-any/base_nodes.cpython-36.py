...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/redbaron/redbaron/base_nodes.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 74036 bytes
from __future__ import absolute_import
import re, os, sys, ast, inspect, itertools
...
class Path(object):
    ...
    def get_holder(class_, node):
        if node.on_attribute is not None:
            if isinstance(node.parent, Node):
                ...
                elif possible_parent is not node:
...
class GenericNodesUtils(object):
    ...
    def at(self, line_no):
        ...
        if node.absolute_bounding_box.top_left.line == line_no:
            if hasattr(node.parent, 'absolute_bounding_box'):
                if node.parent.absolute_bounding_box.top_left.line == line_no:
                    if node.parent.parent is not None:
                        return node.parent
            return node
        else:
            if node is not None:
                if hasattr(node, 'next_rendered'):
    ...
    def _iter_in_rendering_order(self, node):
        ...
        for kind, key, display in node._render():
            if isinstance(display, string_instance):
                if not getattr(node, display):
                    continue
            if kind == 'constant':
                yield node
            else:
...
class NodeList(UserList, GenericNodesUtils):
    ...
    def from_fst(klass, node_list, parent=None, on_attribute=None):
        return klass((map(lambda x: Node.from_fst(x, parent=parent, on_attribute=on_attribute), node_list)), parent=parent,
    ...
    def __getattr__(self, key):
        if key not in redbaron.ALL_IDENTIFIERS:
            raise AttributeError("%s instance has no attribute '%s' and '%s' is not a valid identifier of another node" % (
    ...
    def _bytes_repr_html_(self):
        ...
        return (b'').join(_NodeList__repr_html(self))
...
class Node(GenericNodesUtils):
    ...
    def previous_intuitive(self):
        ...
        if previous_:
            if previous_.type == 'ifelseblock':
                return previous_.value[(-1)]
        if previous_ and previous_.type == 'try':
            ...
            if previous_:
                if previous_.type in ('for', 'while'):
                    if previous_.else_:
                        return previous_.else_
        return previous_
    ...
    def indentation(self):
        ...
        if endl_node is not None:
            return endl_node.indent
        else:
            return ''
    ...
    def _get_list_attribute_is_member_off(self):
        ...
        if self.parent is None:
            ...
            if self.on_attribute is 'root':
                ...
                if self.on_attribute is not None:
    ...
    def __getattr__(self, key):
        if key.endswith('_'):
            if key[:-1] in self._dict_keys + self._list_keys + self._str_keys:
                return getattr(self, key[:-1])
        else:
            if key != 'value':
                if hasattr(self, 'value'):
                    if isinstance(self.value, ProxyList):
    ...
    def _node_match_query(self, node, identifier, *args, **kwargs):
        if not self._attribute_match_query(node.generate_identifiers(), identifier.lower() if (isinstance(identifier, string_instance) and not identifier.startswith('re:')) else identifier):
            ...
            if args:
                if isinstance(args[0], (string_instance, RE_PATTERN_FIELD, list, tuple)):
    ...
    def _attribute_match_query(self, attribute_names, query):
        ...
        if isinstance(query, string_instance):
    ...
    def generate_identifiers(klass):
        return sorted(set(map(lambda x: x.lower(), [
         ...
         redbaron_classname_to_baron_type(klass.__name__) + '_'] + klass._other_identifiers)))
    ...
    def _get_helpers(self):
        not_helpers = set([
         ...
         'replace',
         'to_python'])
        return [x for x in dir(self) if not x.startswith('_') if x not in not_helpers if inspect.ismethod(getattr(self, x))]
    ...
    def __help__(self, deep=2, with_formatting=False):
        ...
        if not deep:
            ...
            to_join += ['%s=%s' % (key, repr(getattr(self, key))) for key in self._str_keys if key != 'type' if 'formatting' not in key]
        ...
        if deep:
            if with_formatting:
                to_join += ['%s=%s' % (key, repr(getattr(self, key))) for key in self._str_keys if key != 'type' if 'formatting' in key]
...
class CodeBlockNode(Node):
        ...
        if self.on_attribute == 'root' and self.next or not self.next and self.parent and self.parent.next:
            if not all(map(lambda x: x.type == 'endl', result[-1:])):
                ...
                if not all(map(lambda x: x.type == 'endl', result[-2:])):
                    ...
                    all(map(lambda x: x.type == 'endl', result[-3:])) or result.append(endl_base_node.copy())
                    result.append(endl_base_node.copy())
                    result.append(endl_base_node.copy())
        else:
            if self.next:
                if not all(map(lambda x: x.type == 'endl', result[-2:])):
                    ...
                    if not all(map(lambda x: x.type == 'endl', result[-3:])):
                        result.append(endl_base_node.copy())
                        result.append(endl_base_node.copy())
    ...
    def __setattr__(self, key, value):
        ...
        if key == 'value':
            if not isinstance(self.value, LineProxyList):
                setattr(self, 'value', LineProxyList((self.value), on_attribute='value'))
        if key == 'decorators':
            if not isinstance(self.decorators, LineProxyList):
    ...
    def _convert_input_to_one_indented_member(self, indented_type, string, parent, on_attribute):
        ...
        if not string:
            ...
            if re.match('^\\s*%s' % indented_type, string):
                ...
                fst = {'first_formatting':[],  'second_formatting':[],  'type':indented_type, 
                 'value':[
                  {'type': 'pass'},
                  {'formatting':[],  'indent':'', 
                   'type':'endl', 
            ...
            if self.next:
                ...
                if isinstance(last_member.value, ProxyList):
                    ...
                    last_member.value.append(redbaron.nodes.EndlNode({'type':'endl',  'indent':'',  'formatting':[],  'value':'\n'}, parent=last_member,
                      on_attribute='value'))
                if self.indentation:
                    node.value.node_list.append(redbaron.nodes.EndlNode({'type':'endl', 
                     'indent':self.indentation,  'formatting':[],  'value':'\n'},
                      parent=node, on_attribute='value'))
                else:
                    node.value.node_list.append(redbaron.nodes.EndlNode({'type':'endl',  'indent':'',  'formatting':[],  'value':'\n'}, parent=node,
                      on_attribute='value'))
...
class ProxyList(object):
    ...
    def __init__(self, node_list, on_attribute='value'):
        ...
        self.middle_separator = redbaron.nodes.CommaNode({'type':'comma', 
         'first_formatting':[],  'second_formatting':[{'type':'space',  'value':' '}]})
        self.on_attribute = on_attribute
    ...
    def _bytes_repr_html_(self):
        ...
        return (b'').join(_ProxyList__repr_html(self))
    ...
    def _get_middle_separator(self):
        if self.style == 'indented':
            return redbaron.nodes.CommaNode({'type':'comma',  'first_formatting':[],  'second_formatting':[
              {'type':'endl', 
               'indent':self.parent.indentation + '    ',  'formatting':[],  'value':'\n'}]})
        else:
            return redbaron.nodes.CommaNode({'type':'comma', 
             'first_formatting':[],  'second_formatting':[{'type':'space',  'value':' '}]})
        ...
        if not self.data:
            ...
            if expected_list:
                if self.has_trailing:
                    if self.style == 'indented':
                        raise expected_list[(-1)].second_formatting.endl or Exception("It appears that you have indentation in your CommaList, for now RedBaron doesn't know how to handle this situation (which requires a lot of work), sorry about that. You can find more information here https://github.com/PyCQA/redbaron/issues/100")
                elif expected_list[(-1)].second_formatting.endl.indent != self.parent.indentation:
                    expected_list[(-1)].second_formatting.endl.indent = self.parent.indentation
            return expected_list
...
class LineProxyList(ProxyList):
    ...
    def __init__(self, node_list, on_attribute='value'):
        ...
        self.middle_separator = redbaron.nodes.DotNode({'type':'endl', 
         'formatting':[],  'value':'\n',  'indent':'    '})
    ...
    def _get_separator_indentation(self):
        if self.node_list.filtered():
            return self.node_list.filtered()[0].indentation
        else:
    ...
    def _generate_expected_list(self):
        ...
        if expected_list:
            ...
            if expected_list:
                if self.data:
                    if self.data[0][0].type == 'endl':
                        if not expected_list[(-1)].formatting.comment:
                            log("first_blank_lines doesn't has comments, reset indentation")
                            expected_list[(-1)].indent = ''
            else:
                for position, i in enumerate(self.data):
                    log('[%s] %s', position, i)
                    if might_need_separator:
                        if i[0].type != 'endl':
                            if not previous or previous.type != 'endl':
                                if not isinstance(previous, (
                    ...
                    if i[1] is not None:
                        ...
                        if not is_last:
                            if not i[1]:
                                if not isinstance(i[0], CodeBlockNode):
