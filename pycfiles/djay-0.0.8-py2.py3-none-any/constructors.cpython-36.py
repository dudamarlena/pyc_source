...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_internal/req/constructors.py
...
def install_req_from_editable(editable_req, comes_from=None, use_pep517=None, isolated=False, options=None, wheel_cache=None, constraint=False):
    ...
    return InstallRequirement(req,
      comes_from, source_dir=source_dir, editable=True,
      link=(Link(url)),
...
def install_req_from_line(name, comes_from=None, use_pep517=None, isolated=False, options=None, wheel_cache=None, constraint=False, line_source=None):
        ...
        if req_as_string is not None:
            try:
                ...
                if os.path.sep in req_as_string:
                    ...
                    if not any(op in req_as_string for op in operators):
                ...
                msg = 'Invalid requirement: {!r}{}'.format(req_as_string, source)
                if add_msg:
                    msg += '\nHint: {}'.format(add_msg)
                raise InstallationError(msg)
            ...
            req = None
    return InstallRequirement(req,
      comes_from, link=link, markers=markers, use_pep517=use_pep517,
      isolated=isolated,
      options=(options if options else {}),
      wheel_cache=wheel_cache,
      constraint=constraint,
            ...
            if comes_from.link:
                if comes_from.link.netloc in domains_not_allowed:
                    raise InstallationError('Packages installed from PyPI cannot depend on packages which are not also hosted on PyPI.\n%s depends on %s ' % (
                     comes_from.name, req))
    return InstallRequirement(req,
      comes_from, isolated=isolated, wheel_cache=wheel_cache, use_pep517=use_pep517)