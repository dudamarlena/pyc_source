...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/venv.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 15135 bytes
from __future__ import with_statement
import os, sys, re, codecs, py, tox
from .config import DepConfig, hookimpl

class CreationConfig:

    def __init__(self, md5, python, version, sitepackages, usedevelop, deps):
        self.md5 = md5
        self.python = python
    ...
    def matches(self, other):
        return other and self.md5 == other.md5 and self.python == other.python and self.version == other.version and self.sitepackages == other.sitepackages and self.usedevelop == other.usedevelop and self.deps == other.deps
...
class VirtualEnv(object):
            ...
            if os.path.split(name)[0] == '.':
                ...
                if venv:
                    if not self.is_allowed_external(p):
                        self.session.report.warning('test command found but not installed in testenv\n  cmd: %s\n  env: %s\nMaybe you forgot to specify a dependency? See also the whitelist_externals envconfig setting.' % (
    ...
    def update(self, action):
        ...
        if not self.envconfig.recreate:
            if rconfig:
                if rconfig.matches(self._getliveconfig()):
    ...
    def _needs_reinstall(self, setupdir, action):
        ...
        output = action.popen(args, cwd=setupdir, redirect=False, returnout=True)
        name = output.strip()
        egg_info = setupdir.join('.'.join((name, 'egg-info')))
        for conf_file in (setup_py, setup_cfg):
            if not egg_info.check() or conf_file.check() and conf_file.mtime() > egg_info.mtime():
                return True
    ...
    def _install(self, deps, extraopts=None, action=None):
        ...
        for dep in deps:
            if isinstance(dep, (str, py.path.local)):
                ...
                if not isinstance(dep, DepConfig):
                ...
                if ixserver not in l:
                    l.append(ixserver)
            if not ixserver.url is None:
                assert isinstance(ixserver.url, str)
            ...
            for i, argv in enumerate(self.envconfig.commands):
                ...
                try:
                    ...
                    if self.envconfig.ignore_outcome:
                        self.session.report.warning('command failed but result from testenv is ignored\n  cmd: %s' % (
                         str(err),))
    ...
    def _pcall(self, args, cwd, venv=True, testcommand=False, action=None, redirect=True, ignore_ret=False):
