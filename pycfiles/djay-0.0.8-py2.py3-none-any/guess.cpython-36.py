...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/requests-toolbelt/requests_toolbelt/auth/guess.py
...
class GuessAuth(auth.AuthBase):
    __doc__ = 'Guesses the auth type by the WWW-Authentication header.'
    ...
    def _handle_digest_auth_401(self, r, kwargs):
        ...
        if hasattr(self.auth, 'num_401_calls'):
            if self.auth.num_401_calls is None:
...
class GuessProxyAuth(GuessAuth):
    ...
    def _handle_digest_auth_407(self, r, kwargs):
        self.proxy_auth = http_proxy_digest.HTTPProxyDigestAuth(username=(self.proxy_username),
