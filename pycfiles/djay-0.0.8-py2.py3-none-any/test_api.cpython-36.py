...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/pyflakes/pyflakes/test/test_api.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 25951 bytes
"""
Tests for L{pyflakes.scripts.pyflakes}.
"""
import contextlib, os, sys, shutil, subprocess, tempfile
...
class TestIterSourceCode(TestCase):
    ...
    def test_shebang(self):
        ...
        self.assertEqual(sorted(iterSourceCode([self.tempdir])), sorted([python, python2, python3, pythonw]))
...
class TestReporter(TestCase):
    ...
    def test_syntaxError(self):
        ...
        reporter.syntaxError('foo.py', 'a problem', 3, 8 if sys.version_info >= (3,
                                                                                 8) else 7, 'bad line of source')
...
class CheckTests(TestCase):
    ...
    def test_eofSyntaxError(self):
        ...
        with self.makeTempFile('def foo(') as (sourcePath):
            if PYPY:
                result = '%s:1:7: parenthesis is never closed\ndef foo(\n      ^\n' % (sourcePath,)
            else:
                result = '%s:1:9: unexpected EOF while parsing\ndef foo(\n        ^\n' % (sourcePath,)
            self.assertHasErrors(sourcePath, [
             result])
    ...
    def test_checkRecursive(self):
        ...
        try:
            ...
            self.assertEqual(sorted(log), sorted([('flake', str(UnusedImport(file1, Node(1), 'baz'))),
             (
              'flake',
...
class IntegrationTests(TestCase):
    ...
    def test_errors_syntax(self):
        ...
        d = self.runPyflakes([self.tempfilepath])
        error_msg = '{0}:1:{2}: invalid syntax{1}import{1}    {3}^{1}'.format(self.tempfilepath, os.linesep, 5 if PYPY else 7, '' if PYPY else '  ')
        self.assertEqual(d, ('', error_msg, 1))
