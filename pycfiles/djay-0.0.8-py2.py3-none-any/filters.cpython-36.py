...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/jinja2/jinja2/filters.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 36528 bytes
"""
    jinja2.filters
    ~~~~~~~~~~~~~~
...
from jinja2.utils import Markup, escape, pformat, urlize, soft_unicode, unicode_urlencode, htmlsafe_json_dumps
from jinja2.runtime import Undefined
from jinja2.exceptions import FilterArgumentError
from jinja2._compat import imap, string_types, text_type, iteritems, PY2
_word_re = re.compile('\\w+', re.UNICODE)
_word_beginning_split_re = re.compile('([-\\s\\(\\{\\[\\<]+)', re.UNICODE)
...
def do_urlencode(value):
    ...
    if itemiter is None:
        ...
        return '&'.join(unicode_urlencode(k) + '=' + unicode_urlencode(v, for_qs=True) for k, v in itemiter)
...
def do_xmlattr(_eval_ctx, d, autospace=True):
    ...
    """
    rv = ' '.join('%s="%s"' % (escape(key), escape(value)) for key, value in iteritems(d) if value is not None if not isinstance(value, Undefined))
    if autospace:
        if rv:
            rv = ' ' + rv
...
def do_sort(environment, value, reverse=False, case_sensitive=False, attribute=None):
    ...
    """
    key_func = make_attrgetter(environment,
...
def do_unique(environment, value, case_sensitive=False, attribute=None):
    ...
    """
    getter = make_attrgetter(environment,
      attribute, postprocess=(ignore_case if not case_sensitive else None))
    seen = set()
    for item in value:
        key = getter(item)
...
def _min_or_max(environment, value, func, case_sensitive, attribute):
    ...
    try:
        ...
        key_func = make_attrgetter(environment, attribute, ignore_case if not case_sensitive else None)
...
def do_indent(s, width=4, first=False, blank=False, indentfirst=None):
    ...
    if indentfirst is not None:
        warnings.warn((DeprecationWarning('The "indentfirst" argument is renamed to "first".')),
          stacklevel=2)
        first = indentfirst
    else:
        s += '\n'
...
def do_wordwrap(environment, s, width=79, break_long_words=True, wrapstring=None):
    ...
    import textwrap
    return wrapstring.join(textwrap.wrap(s, width=width, expand_tabs=False, replace_whitespace=False,
...
def prepare_map(args, kwargs):
    ...
    if len(args) == 2:
        ...
        func = lambda item: context.environment.call_filter(name,
          item, args, kwargs, context=context)
    return (seq, func)
