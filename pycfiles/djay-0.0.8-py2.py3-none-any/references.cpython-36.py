...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/transforms/references.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 36464 bytes
"""
...
class PropagateTargets(Transform):
    ...
    def apply(self):
        for target in self.document.traverse(nodes.target):
            if not isinstance(target.parent, nodes.TextElement):
                if target.hasattr('refid') or target.hasattr('refuri') or target.hasattr('refname'):
                    pass
                else:
                    assert len(target) == 0, 'error: block-level target has children'
                    next_node = target.next_node(ascend=True)
                    if next_node is not None and (not isinstance(next_node, nodes.Invisible) and not isinstance(next_node, nodes.Targetable) or isinstance(next_node, nodes.target)):
