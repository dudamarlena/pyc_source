...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/urllib3/urllib3/connectionpool.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 35307 bytes
...
class HTTPConnectionPool(ConnectionPool, RequestMethods):
    ...
    def _new_conn(self):
        ...
        conn = (self.ConnectionCls)(host=self.host, port=self.port, timeout=self.timeout.connect_timeout, 
        ...
        if headers is None:
            ...
            try:
                try:
                    ...
                    httplib_response = self._make_request(conn, method, url, timeout=timeout_obj,
                      body=body,
                      headers=headers,
                      chunked=chunked)
                    response_conn = conn if not release_conn else None
                    response_kw['request_method'] = method
                    response = (self.ResponseCls.from_httplib)(httplib_response, pool=self, 
 ...
 redirect, assert_same_host, timeout=timeout, 
        ...
        def drain_and_release_conn(response):
            try:
                response.read()
            except (TimeoutError, HTTPException, SocketError, ProtocolError, BaseSSLError, SSLError):
                pass
        ...
        if redirect_location:
            ...
            return (self.urlopen)(
 method, redirect_location, body, headers, retries=retries, 
            ...
            if retries.is_retry(method, response.status, has_retry_after):
                try:
                    ...
                    return (self.urlopen)(
 method, url, body, headers, retries=retries, 
                     redirect=redirect, assert_same_host=assert_same_host, 
                     timeout=timeout, 
                     pool_timeout=pool_timeout, release_conn=release_conn, 
                     body_pos=body_pos, **response_kw)
...
class HTTPSConnectionPool(HTTPConnectionPool):
    ...
    def __init__(self, host, port=None, strict=False, timeout=Timeout.DEFAULT_TIMEOUT, maxsize=1, block=False, headers=None, retries=None, _proxy=None, _proxy_headers=None, key_file=None, cert_file=None, cert_reqs=None, key_password=None, ca_certs=None, ssl_version=None, assert_hostname=None, assert_fingerprint=None, ca_cert_dir=None, **conn_kw):
        (HTTPConnectionPool.__init__)(self, host, port, strict, timeout, maxsize, 
         block, headers, retries, _proxy, _proxy_headers, **conn_kw)
    ...
    def _prepare_conn(self, conn):
        ...
        if isinstance(conn, VerifiedHTTPSConnection):
            conn.set_cert(key_file=(self.key_file), key_password=(self.key_password),
    ...
    def _new_conn(self):
        ...
        conn = (self.ConnectionCls)(host=actual_host, port=actual_port, timeout=self.timeout.connect_timeout, 
         strict=self.strict, 
