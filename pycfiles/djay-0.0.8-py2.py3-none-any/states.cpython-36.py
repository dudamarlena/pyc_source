...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/parsers/rst/states.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 132221 bytes
"""
    ...
    def nested_parse(self, block, input_offset, node, match_titles=False, state_machine_class=None, state_machine_kwargs=None):
        ...
        if state_machine_class is None:
            ...
            if state_machine_kwargs is None:
                ...
                state_machine = state_machine or state_machine_class(debug=self.debug, **state_machine_kwargs)
...
class Inliner:
        ...
        if endmatch and endmatch.start(1):
            ...
            if rawsource[-1:] == '_':
                if role:
                    ...
                    prb = self.problematic(text, text, msg)
                    return (
                     string[:rolestart], [prb], string[textend:], [msg])
                else:
    ...
    def phrase_ref(self, before, after, rawsource, escaped, text):
        ...
        if match:
            ...
            if aliastext.endswith('_'):
                if not (underscore_escaped or self.patterns.uri.match(aliastext)):
                    aliastype = 'name'
    ...
    def standalone_uri(self, match, lineno):
        if not match.group('scheme') or match.group('scheme').lower() in urischemes.schemes:
            ...
            reference = nodes.reference(rawsource, unescaped, refuri=(addscheme + unescaped))
    ...
    def implicit_inline(self, text, lineno):
        ...
        if not text:
            ...
            return [
             nodes.Text((unescape(text)), rawsource=(unescape(text, True)))]

    dispatch = {'*':emphasis,  '**':strong, 
...
class Body(RSTState):
    ...
    def parse_option_marker(self, match):
        ...
        for optionstring in optionstrings:
            ...
            if len(firstopt) > 1:
                ...
                if len(tokens[0]) > 2:
                    if tokens[0].startswith('-') and not tokens[0].startswith('--') or tokens[0].startswith('+'):
        ...
        if not block:
            ...
            for node in substitution_node.traverse(nodes.Element):
                if self.disallowed_inside_substitution_definitions(node):
                    ...
                    msg = self.reporter.error(('Substitution definition contains illegal element <%s>:' % node.tagname),
                      pformat,
    ...
    def disallowed_inside_substitution_definitions(self, node):
        if node['ids'] or isinstance(node, nodes.reference) and node.get('anonymous') or isinstance(node, nodes.footnote_reference) and node.get('auto'):
            return True
        else:
            return False
    ...
    def parse_directive_block(self, indented, line_offset, directive, option_presets):
        ...
        if indented:
            if directive.required_arguments or directive.optional_arguments or option_spec:
        ...
        if column_width(title) > len(underline):
            if len(underline) < 4:
                ...
                msg = self.reporter.warning('Title underline too short.', (nodes.literal_block(blocktext, blocktext)),
                  line=lineno)
                messages.append(msg)
            blocktext = self.state_machine.match_titles or context[0] + '\n' + self.state_machine.line
            src, srcline = self.state_machine.get_source_and_line()
