...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/modula2.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 52561 bytes
"""
...
class Modula2Lexer(RegexLexer):
    ...
    tokens = {'whitespace':[
     ...
     'dialecttags':[
      (
       '\\(\\*!m2pim\\*\\)', Comment.Special),
      (
       '\\(\\*!m2iso\\*\\)', Comment.Special),
      (
       '\\(\\*!m2r10\\*\\)', Comment.Special),
      (
       '\\(\\*!objm2\\*\\)', Comment.Special),
      (
       '\\(\\*!m2iso\\+aglet\\*\\)', Comment.Special),
      (
       '\\(\\*!m2pim\\+gm2\\*\\)', Comment.Special),
      (
       '\\(\\*!m2iso\\+p1\\*\\)', Comment.Special),
      (
       '\\(\\*!m2iso\\+xds\\*\\)', Comment.Special)], 
     'identifiers':[
      (
       '([a-zA-Z_$][\\w$]*)', Name)], 
    ...
    common_reserved_words = ('AND', 'ARRAY', 'BEGIN', 'BY', 'CASE', 'CONST', 'DEFINITION',
                             'DIV', 'DO', 'ELSE', 'ELSIF', 'END', 'EXIT', 'FOR',
                             'FROM', 'IF', 'IMPLEMENTATION', 'IMPORT', 'IN', 'LOOP',
                             'MOD', 'MODULE', 'NOT', 'OF', 'OR', 'POINTER', 'PROCEDURE',
                             'RECORD', 'REPEAT', 'RETURN', 'SET', 'THEN', 'TO', 'TYPE',
                             'UNTIL', 'VAR', 'WHILE')
    common_builtins = ('ABS', 'BOOLEAN', 'CARDINAL', 'CHAR', 'CHR', 'FALSE', 'INTEGER',
                       'LONGINT', 'LONGREAL', 'MAX', 'MIN', 'NIL', 'ODD', 'ORD',
                       'REAL', 'TRUE')
    common_pseudo_builtins = ('ADDRESS', 'BYTE', 'WORD', 'ADR')
    pim_lexemes_to_reject = ('!', '`', '@', '$', '%', '?', '\\', '==', '++', '--',
                             '::', '*.', '+>', '->', '<<', '>>', '|#', '##')
    pim_additional_reserved_words = ('EXPORT', 'QUALIFIED', 'WITH')
    pim_additional_builtins = ('BITSET', 'CAP', 'DEC', 'DISPOSE', 'EXCL', 'FLOAT',
                               'HALT', 'HIGH', 'INC', 'INCL', 'NEW', 'NIL', 'PROC',
                               'SIZE', 'TRUNC', 'VAL')
    pim_additional_pseudo_builtins = ('SYSTEM', 'PROCESS', 'TSIZE', 'NEWPROCESS', 'TRANSFER')
    iso_lexemes_to_reject = ('`', '$', '%', '?', '\\', '==', '++', '--', '::', '*.',
                             '+>', '->', '<<', '>>', '|#', '##')
    iso_additional_reserved_words = ('EXCEPT', 'EXPORT', 'FINALLY', 'FORWARD', 'PACKEDSET',
                                     'QUALIFIED', 'REM', 'RETRY', 'WITH', 'ABSTRACT',
                                     'AS', 'CLASS', 'GUARD', 'INHERIT', 'OVERRIDE',
                                     'READONLY', 'REVEAL', 'TRACED', 'UNSAFEGUARDED')
    iso_additional_builtins = ('BITSET', 'CAP', 'CMPLX', 'COMPLEX', 'DEC', 'DISPOSE',
                               'EXCL', 'FLOAT', 'HALT', 'HIGH', 'IM', 'INC', 'INCL',
                               'INT', 'INTERRUPTIBLE', 'LENGTH', 'LFLOAT', 'LONGCOMPLEX',
                               'NEW', 'PROC', 'PROTECTION', 'RE', 'SIZE', 'TRUNC',
                               'UNINTERRUBTIBLE', 'VAL', 'CREATE', 'DESTROY', 'EMPTY',
                               'ISMEMBER', 'SELF')
    iso_additional_pseudo_builtins = ('SYSTEM', 'BITSPERLOC', 'LOCSPERBYTE', 'LOCSPERWORD',
                                      'LOC', 'ADDADR', 'SUBADR', 'DIFADR', 'MAKEADR',
                                      'ADR', 'ROTATE', 'SHIFT', 'CAST', 'TSIZE',
                                      'COROUTINES', 'ATTACH', 'COROUTINE', 'CURRENT',
                                      'DETACH', 'HANDLER', 'INTERRUPTSOURCE', 'IOTRANSFER',
                                      'IsATTACHED', 'LISTEN', 'NEWCOROUTINE', 'PROT',
                                      'TRANSFER', 'EXCEPTIONS', 'AllocateSource',
                                      'CurrentNumber', 'ExceptionNumber', 'ExceptionSource',
                                      'GetMessage', 'IsCurrentSource', 'IsExceptionalExecution',
                                      'RAISE', 'TERMINATION', 'IsTerminating', 'HasHalted',
                                      'M2EXCEPTION', 'M2Exceptions', 'M2Exception',
                                      'IsM2Exception', 'indexException', 'rangeException',
                                      'caseSelectException', 'invalidLocation', 'functionException',
                                      'wholeValueException', 'wholeDivException',
                                      'realValueException', 'realDivException', 'complexValueException',
                                      'complexDivException', 'protException', 'sysException',
                                      'coException', 'exException')
    m2r10_lexemes_to_reject = ('!', '`', '@', '$', '%', '&', '<>')
    m2r10_additional_reserved_words = ('ALIAS', 'ARGLIST', 'BLUEPRINT', 'COPY', 'GENLIB',
                                       'INDETERMINATE', 'NEW', 'NONE', 'OPAQUE',
                                       'REFERENTIAL', 'RELEASE', 'RETAIN', 'ASM',
                                       'REG')
    m2r10_additional_builtins = ('CARDINAL', 'COUNT', 'EMPTY', 'EXISTS', 'INSERT',
                                 'LENGTH', 'LONGCARD', 'OCTET', 'PTR', 'PRED', 'READ',
                                 'READNEW', 'REMOVE', 'RETRIEVE', 'SORT', 'STORE',
                                 'SUBSET', 'SUCC', 'TLIMIT', 'TMAX', 'TMIN', 'TRUE',
                                 'TSIZE', 'UNICHAR', 'WRITE', 'WRITEF')
    m2r10_additional_pseudo_builtins = ('TPROPERTIES', 'PROPERTY', 'LITERAL', 'TPROPERTY',
                                        'TLITERAL', 'TBUILTIN', 'TDYN', 'TREFC',
                                        'TNIL', 'TBASE', 'TPRECISION', 'TMAXEXP',
                                        'TMINEXP', 'CONVERSION', 'TSXFSIZE', 'SXF',
                                        'VAL', 'UNSAFE', 'CAST', 'INTRINSIC', 'AVAIL',
                                        'ADD', 'SUB', 'ADDC', 'SUBC', 'FETCHADD',
                                        'FETCHSUB', 'SHL', 'SHR', 'ASHR', 'ROTL',
                                        'ROTR', 'ROTLC', 'ROTRC', 'BWNOT', 'BWAND',
                                        'BWOR', 'BWXOR', 'BWNAND', 'BWNOR', 'SETBIT',
                                        'TESTBIT', 'LSBIT', 'MSBIT', 'CSBITS', 'BAIL',
                                        'HALT', 'TODO', 'FFI', 'ADDR', 'VARGLIST',
                                        'VARGC', 'ATOMIC', 'INTRINSIC', 'AVAIL',
                                        'SWAP', 'CAS', 'INC', 'DEC', 'BWAND', 'BWNAND',
                                        'BWOR', 'BWXOR', 'COMPILER', 'DEBUG', 'MODNAME',
                                        'PROCNAME', 'LINENUM', 'DEFAULT', 'HASH',
                                        'ASSEMBLER', 'REGISTER', 'SETREG', 'GETREG',
                                        'CODE')
    objm2_lexemes_to_reject = ('!', '$', '%', '&', '<>')
    objm2_additional_reserved_words = ('BYCOPY', 'BYREF', 'CLASS', 'CONTINUE', 'CRITICAL',
                                       'INOUT', 'METHOD', 'ON', 'OPTIONAL', 'OUT',
    ...
    aglet_additional_builtins = ('BITSET8', 'BITSET16', 'BITSET32', 'CARDINAL8', 'CARDINAL16',
                                 'CARDINAL32', 'INTEGER8', 'INTEGER16', 'INTEGER32')
    aglet_additional_pseudo_builtins = ()
    gm2_additional_reserved_words = ('ASM', '__ATTRIBUTE__', '__BUILTIN__', '__COLUMN__',
                                     '__DATE__', '__FILE__', '__FUNCTION__', '__LINE__',
                                     '__MODULE__', 'VOLATILE')
    gm2_additional_builtins = ('BITSET8', 'BITSET16', 'BITSET32', 'CARDINAL8', 'CARDINAL16',
                               'CARDINAL32', 'CARDINAL64', 'COMPLEX32', 'COMPLEX64',
                               'COMPLEX96', 'COMPLEX128', 'INTEGER8', 'INTEGER16',
                               'INTEGER32', 'INTEGER64', 'REAL8', 'REAL16', 'REAL32',
                               'REAL96', 'REAL128', 'THROW')
    ...
    xds_additional_builtins = ('ASH', 'ASSERT', 'DIFFADR_TYPE', 'ENTIER', 'INDEX',
                               'LEN', 'LONGCARD', 'SHORTCARD', 'SHORTINT')
    xds_additional_pseudo_builtins = ('PROCESS', 'NEWPROCESS', 'BOOL8', 'BOOL16', 'BOOL32',
                                      'CARD8', 'CARD16', 'CARD32', 'INT8', 'INT16',
                                      'INT32', 'REF', 'MOVE', 'FILL', 'GET', 'PUT',
                                      'CC', 'int', 'unsigned', 'size_t', 'voidCOMPILER',
                                      'OPTION', 'EQUATION')
    pim_stdlib_module_identifiers = ('Terminal', 'FileSystem', 'InOut', 'RealInOut',
                                     'MathLib0', 'Storage')
    pim_stdlib_type_identifiers = ('Flag', 'FlagSet', 'Response', 'Command', 'Lock',
                                   'Permission', 'MediumType', 'File', 'FileProc',
                                   'DirectoryProc', 'FileCommand', 'DirectoryCommand')
    pim_stdlib_proc_identifiers = ('Read', 'BusyRead', 'ReadAgain', 'Write', 'WriteString',
                                   'WriteLn', 'Create', 'Lookup', 'Close', 'Delete',
                                   'Rename', 'SetRead', 'SetWrite', 'SetModify',
                                   'SetOpen', 'Doio', 'SetPos', 'GetPos', 'Length',
                                   'Reset', 'Again', 'ReadWord', 'WriteWord', 'ReadChar',
                                   'WriteChar', 'CreateMedium', 'DeleteMedium', 'AssignName',
                                   'DeassignName', 'ReadMedium', 'LookupMedium',
                                   'OpenInput', 'OpenOutput', 'CloseInput', 'CloseOutput',
                                   'ReadString', 'ReadInt', 'ReadCard', 'ReadWrd',
                                   'WriteInt', 'WriteCard', 'WriteOct', 'WriteHex',
                                   'WriteWrd', 'ReadReal', 'WriteReal', 'WriteFixPt',
                                   'WriteRealOct', 'sqrt', 'exp', 'ln', 'sin', 'cos',
                                   'arctan', 'entier', 'ALLOCATE', 'DEALLOCATE')
    ...
    m2r10_stdlib_adt_identifiers = ('BCD', 'LONGBCD', 'BITSET', 'SHORTBITSET', 'LONGBITSET',
                                    'LONGLONGBITSET', 'COMPLEX', 'LONGCOMPLEX', 'SHORTCARD',
                                    'LONGLONGCARD', 'SHORTINT', 'LONGLONGINT', 'POSINT',
                                    'SHORTPOSINT', 'LONGPOSINT', 'LONGLONGPOSINT',
                                    'BITSET8', 'BITSET16', 'BITSET32', 'BITSET64',
                                    'BITSET128', 'BS8', 'BS16', 'BS32', 'BS64', 'BS128',
                                    'CARDINAL8', 'CARDINAL16', 'CARDINAL32', 'CARDINAL64',
                                    'CARDINAL128', 'CARD8', 'CARD16', 'CARD32', 'CARD64',
                                    'CARD128', 'INTEGER8', 'INTEGER16', 'INTEGER32',
                                    'INTEGER64', 'INTEGER128', 'INT8', 'INT16', 'INT32',
                                    'INT64', 'INT128', 'STRING', 'UNISTRING')
    m2r10_stdlib_blueprint_identifiers = ('ProtoRoot', 'ProtoComputational', 'ProtoNumeric',
                                          'ProtoScalar', 'ProtoNonScalar', 'ProtoCardinal',
                                          'ProtoInteger', 'ProtoReal', 'ProtoComplex',
                                          'ProtoVector', 'ProtoTuple', 'ProtoCompArray',
                                          'ProtoCollection', 'ProtoStaticArray',
                                          'ProtoStaticSet', 'ProtoStaticString',
                                          'ProtoArray', 'ProtoString', 'ProtoSet',
                                          'ProtoMultiSet', 'ProtoDictionary', 'ProtoMultiDict',
                                          'ProtoExtension', 'ProtoIO', 'ProtoCardMath',
                                          'ProtoIntMath', 'ProtoRealMath')
    m2r10_stdlib_module_identifiers = ('ASCII', 'BooleanIO', 'CharIO', 'UnicharIO',
                                       'OctetIO', 'CardinalIO', 'LongCardIO', 'IntegerIO',
                                       'LongIntIO', 'RealIO', 'LongRealIO', 'BCDIO',
                                       'LongBCDIO', 'CardMath', 'LongCardMath', 'IntMath',
                                       'LongIntMath', 'RealMath', 'LongRealMath',
                                       'BCDMath', 'LongBCDMath', 'FileIO', 'FileSystem',
                                       'Storage', 'IOSupport')
    ...
    dialects = ('unknown', 'm2pim', 'm2iso', 'm2r10', 'objm2', 'm2iso+aglet', 'm2pim+gm2',
                'm2iso+p1', 'm2iso+xds')
    lexemes_to_reject_db = {'unknown':(),  'm2pim':(
      pim_lexemes_to_reject,), 
     'm2iso':(
      iso_lexemes_to_reject,), 
     'm2r10':(
    ...
    stdlib_adts_db = {'unknown':(),  'm2pim':(),  'm2iso':(),  'm2r10':(
      m2r10_stdlib_adt_identifiers,), 
     'objm2':(
      m2r10_stdlib_adt_identifiers,), 
     'm2iso+aglet':(),  'm2pim+gm2':(),  'm2iso+p1':(),  'm2iso+xds':()}
    stdlib_modules_db = {'unknown':(),  'm2pim':(
    ...
    stdlib_types_db = {'unknown':(),  'm2pim':(
     ...
     'm2iso+xds':(
      iso_stdlib_type_identifiers,)}
    stdlib_procedures_db = {'unknown':(),  'm2pim':(
     ...
     'm2iso+xds':(
      iso_stdlib_proc_identifiers,)}
    stdlib_variables_db = {'unknown':(),  'm2pim':(
     ...
     'm2iso+xds':(
      iso_stdlib_var_identifiers,)}
    stdlib_constants_db = {'unknown':(),  'm2pim':(
    ...
    def get_dialect_from_dialect_tag(self, dialect_tag):
        ...
        if len(dialect_tag) > left_tag_delim_len + right_tag_delim_len:
            if dialect_tag.startswith(left_tag_delim):
                if dialect_tag.endswith(right_tag_delim):
    ...
    def get_tokens_unprocessed(self, text):
        for index, token, value in RegexLexer.get_tokens_unprocessed(self, text):
            if not self.dialect_set_by_tag:
                if token == Comment.Special:
            ...
            if token is Name:
                if value in self.reserved_words:
                    ...
                    if value in self.builtins:
                        ...
                        if value in self.pseudo_builtins:
                            token = Name.Builtin.Pseudo
                            if self.algol_publication_mode:
                                value = value.lower()
                        else:
                            if value in self.adts:
                                token = self.treat_stdlib_adts_as_builtins or Name.Namespace
                            else:
                                token = Name.Builtin.Pseudo
                                if self.algol_publication_mode:
                                    value = value.lower()
                        ...
                        if value in self.procedures:
                            ...
                            if value in self.variables:
                                ...
                                if value in self.constants:
                                    ...
                                    if token in Number:
                                        if self.dialect not in ('unknown', 'm2r10',
                                                                'objm2'):
                                            if "'" in value or value[0:2] in ('0b',
                                                                              '0x',
                                                                              '0u'):
                                                token = Error
                                            ...
                                            if token is Number.Oct:
                                                ...
                                                if 'H' in value:
                                                    token = Error
                                                if token is Number.Float and 'E' in value:
                                                    token = Error
                                    else:
                                        if token in Comment:
                                            if token is Comment.Single:
                                                if self.dialect not in ('unknown',
                                                                        'm2r10',
                                                                        'objm2'):
                                                    token = Error
                                                if token is Comment.Preproc:
                                                    if value.startswith('<*'):
                                                        if self.dialect.startswith('m2pim'):
                                                            token = Error
                                                    if value.startswith('(*$') and self.dialect != 'unknown' and not self.dialect.startswith('m2pim'):
