...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/baron/baron/render.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 38536 bytes
import sys, json
...
def render_node(node, strict=False):
    ...
    for key_type, render_key, dependent in nodes_rendering_order[node['type']]:
        if not dependent:
            ...
            if key_type == 'bool':
                ...
                if isinstance(dependent, str):
                    if not node.get(dependent):
                        continue
        if isinstance(dependent, list):
            if not all([node.get(x) for x in dependent]):
                continue
        if strict:
            try:
                if key_type == 'key':
                    if not isinstance(node[render_key], (dict, type(None))):
                        raise AssertionError("Key '%s' is expected to have type of 'key' (dict/None) but has type of '%s' instead" % (render_key, type(node[render_key])))
                else:
                    if key_type == 'string':
        ...
        if key_type in ('key', 'string', 'list', 'formatting'):
            yield (
             key_type, node[render_key], render_key)
        else:
            if key_type in ('constant', 'string'):
                yield (
                 key_type, render_key, render_key)
            else:
