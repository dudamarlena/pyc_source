...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/parsers/rst/directives/tables.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 22842 bytes
"""
    ...
    def run(self):
        if not self.content:
            ...
            if len(node) != 1 or not isinstance(node[0], nodes.table):
                error = self.state_machine.reporter.error(('Error parsing content block for the "%s" directive: exactly one table expected.' % self.name),
                  (nodes.literal_block(self.block_text, self.block_text)),
                  line=(self.lineno))
                return [error]
            table_node = node[0]
...
class CSVTable(Table):
    ...
    def get_csv_data(self):
        ...
        if self.content:
            if 'file' in self.options or 'url' in self.options:
                error = self.state_machine.reporter.error(('"%s" directive may not both specify an external file and have content.' % self.name),
                  (nodes.literal_block(self.block_text, self.block_text)),
                  line=(self.lineno))
                raise SystemMessagePropagation(error)
            ...
            if 'file' in self.options:
                if 'url' in self.options:
                    error = self.state_machine.reporter.error(('The "file" and "url" options may not be simultaneously specified for the "%s" directive.' % self.name),
                      (nodes.literal_block(self.block_text, self.block_text)),
                      line=(self.lineno))
                    raise SystemMessagePropagation(error)
                ...
                if 'url' in self.options:
                    ...
                    csv_file = io.StringInput(source=csv_text,
                      source_path=source,
                      encoding=encoding,
                      error_handler=(self.state.document.settings.input_encoding_error_handler))
                    csv_data = csv_file.read().splitlines()
                else:
                    error = self.state_machine.reporter.warning(('The "%s" directive requires content; none supplied.' % self.name),
                      (nodes.literal_block(self.block_text, self.block_text)),
                      line=(self.lineno))
                    raise SystemMessagePropagation(error)
        return (
    ...
    def check_list_content(self, node):
        ...
        for item_index in range(len(list_node)):
            ...
            if len(item) != 1 or not isinstance(item[0], nodes.bullet_list):
                error = self.state_machine.reporter.error(('Error parsing content block for the "%s" directive: two-level bullet list expected, but row %s does not contain a second-level bullet list.' % (
