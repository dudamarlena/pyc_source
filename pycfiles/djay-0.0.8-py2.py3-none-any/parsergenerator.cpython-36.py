...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/rply/rply/parsergenerator.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 22255 bytes
import errno, hashlib, json, os, sys, tempfile, warnings
...
class ParserGenerator(object):
    ...
    def serialize_table(self, table):
        return {'lr_action':table.lr_action, 
    ...
    def build(self):
        ...
        if self.cache_id is not None:
            ...
            cache_file = os.path.join(cache_dir, '%s-%s-%s.json' % (
        ...
        if table.sr_conflicts:
            warnings.warn(('%d shift/reduce conflict%s' % (
             len(table.sr_conflicts),
             's' if len(table.sr_conflicts) > 1 else '')),
              ParserGeneratorWarning,
              stacklevel=2)
        if table.rr_conflicts:
            warnings.warn(('%d reduce/reduce conflict%s' % (
    ...
    def _write_cache(self, cache_dir, cache_file, table):
        if not os.path.exists(cache_dir):
            try:
                os.makedirs(cache_dir, mode=448)
            except OSError as e:
...
class LRTable(object):
    ...
    def from_cache(cls, grammar, data):
        lr_action = [dict([(str(k), v) for k, v in iteritems(action)]) for action in data['lr_action']]
        lr_goto = [dict([(str(k), v) for k, v in iteritems(goto)]) for goto in data['lr_goto']]
                        ...
                        for a in laheads:
                            if a in st_action:
                                ...
                                if r > 0:
                                    ...
                                    rprec, rlevel = grammar.precedence.get(a, ('right',
                                                                               0))
                                    if slevel < rlevel or slevel == rlevel and rprec == 'left':
                                        ...
                                        if not slevel:
                                            ...
                                            slevel == rlevel and rprec == 'nonassoc' or rlevel or sr_conflicts.append((st, repr(a), 'shift'))
                                else:
                                    if r < 0:
                            ...
                            if a in st_action:
                                ...
                                if r > 0:
                                    ...
                                    if r < 0:
                                        ...
                                        sprec, slevel = grammar.precedence.get(a, ('right',
                                                                                   0))
    ...
    def dr_relation(cls, grammar, C, trans, nullable, add_count, goto_cache):
        ...
        if state == 0:
            if N == grammar.productions[0].prod[0]:
                terms.append('$end')
        return terms
