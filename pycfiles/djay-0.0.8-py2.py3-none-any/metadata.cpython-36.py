...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/wheel/wheel/metadata.py
# Compiled at: 2019-07-30 18:46:56
# Size of source mod 2**32: 4699 bytes
"""
Tools for converting old- to new-style metadata.
"""
import os.path, re, textwrap, pkg_resources
...
def generate_requirements(extras_require):
    ...
    for extra, depends in extras_require.items():
        ...
        for new_req in convert_requirements(depends):
            yield (
             'Requires-Dist', new_req + condition)
...
def pkginfo_to_metadata(egg_info_path, pkginfo_path):
    ...
    if os.path.exists(requires_path):
        ...
        parsed_requirements = sorted((pkg_resources.split_sections(requires)), key=(lambda x: x[0] or ''))
        for extra, reqs in parsed_requirements:
            for key, value in generate_requirements({extra: reqs}):
                if (
                 key, value) not in pkg_info.items():
                    pkg_info[key] = value
...
def pkginfo_unicode(pkg_info, field):
    ...
    if not isinstance(text, str):
        if not hasattr(pkg_info, 'raw_items'):
            return str(text)
...
def dedent_description(pkg_info):
    ...
    description_dedent = '\n'.join((
     ...
     '\n'))
    if surrogates:
        description_dedent = description_dedent.encode('utf8').decode('ascii', 'surrogateescape')
