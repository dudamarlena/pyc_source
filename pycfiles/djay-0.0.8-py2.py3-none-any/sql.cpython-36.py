...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/sql.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 31574 bytes
"""
    pygments.lexers.sql
    ~~~~~~~~~~~~~~~~~~~
...
from pygments.token import Punctuation, Whitespace, Error, Text, Comment, Operator, Keyword, Name, String, Number, Generic
...
__all__ = [
 'PostgresLexer', 'PlPgsqlLexer', 'PostgresConsoleLexer',
...
def language_callback(lexer, match):
    ...
    if m:
        ...
        yield (
         match.start(1), String, match.group(1))
...
class PostgresLexer(PostgresBase, RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       '(' + '|'.join(s.replace(' ', '\\s+') for s in DATATYPES + PSEUDO_TYPES) + ')\\b', Name.Builtin),
      (
      ...
      (
       '\\$\\d+', Name.Variable),
      (
       '([0-9]*\\.[0-9]*|[0-9]+)(e[+-]?[0-9]+)?', Number.Float),
      (
       '[0-9]+', Number.Integer),
      (
       "((?:E|U&)?)(')", bygroups(String.Affix, String.Single), 'string'),
      (
       '((?:U&)?)(")', bygroups(String.Affix, String.Name), 'quoted-ident'),
      (
       '(?s)(\\$)([^$]*)(\\$)(.*?)(\\$)(\\2)(\\$)', language_callback),
      (
       '[a-z_]\\w*', Name),
      (
       ':([\'"]?)[a-z]\\w*\\b\\1', Name.Variable),
      (
       '[;:()\\[\\]{},.]', Punctuation)], 
     'multiline-comments':[
      (
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens['root']):
        if pattern[1] == Keyword:
            tokens['root'][i] = (words((KEYWORDS + PLPGSQL_KEYWORDS), suffix='\\b'),
             Keyword)
            del i
            break
    else:
    ...
    tokens['root'][:0] = [
     (
      '\\%[a-z]\\w*\\b', Name.Builtin),
     (
      ':=', Operator),
     (
      '\\<\\<[a-z]\\w*\\>\\>', Name.Label),
     (
      '\\#[a-z]\\w*\\b', Keyword.Pseudo)]
...
class lookahead(object):
    __doc__ = 'Wrap an iterator and allow pushing back an item.'
        ...
        while True:
            ...
            for line in lines:
                ...
                if mmsg is not None:
                    ...
                    yield (
                     mmsg.start(1), Generic.Strong, mmsg.group(1))
...
class SqlLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       '/\\*', Comment.Multiline, 'multiline-comments'),
      (
       words(('ABORT', 'ABS', 'ABSOLUTE', 'ACCESS', 'ADA', 'ADD', 'ADMIN', 'AFTER', 'AGGREGATE',
       ...
       'BITVAR', 'BIT_LENGTH', 'BOTH', 'BREADTH', 'BY', 'C', 'CACHE', 'CALL', 'CALLED',
       'CARDINALITY', 'CASCADE', 'CASCADED', 'CASE', 'CAST', 'CATALOG', 'CATALOG_NAME',
       'CHAIN', 'CHARACTERISTICS', 'CHARACTER_LENGTH', 'CHARACTER_SET_CATALOG', 'CHARACTER_SET_NAME',
       'CHARACTER_SET_SCHEMA', 'CHAR_LENGTH', 'CHECK', 'CHECKED', 'CHECKPOINT', 'CLASS',
       'CLASS_ORIGIN', 'CLOB', 'CLOSE', 'CLUSTER', 'COALSECE', 'COBOL', 'COLLATE',
       ...
       'CONSTRUCTOR', 'CONTAINS', 'CONTINUE', 'CONVERSION', 'CONVERT', 'COPY', 'CORRESPONTING',
       'COUNT', 'CREATE', 'CREATEDB', 'CREATEUSER', 'CROSS', 'CUBE', 'CURRENT', 'CURRENT_DATE',
       ...
       'DATETIME_INTERVAL_PRECISION', 'DAY', 'DEALLOCATE', 'DECLARE', 'DEFAULT',
       'DEFAULTS', 'DEFERRABLE', 'DEFERRED', 'DEFINED', 'DEFINER', 'DELETE', 'DELIMITER',
       'DELIMITERS', 'DEREF', 'DESC', 'DESCRIBE', 'DESCRIPTOR', 'DESTROY', 'DESTRUCTOR',
       'DETERMINISTIC', 'DIAGNOSTICS', 'DICTIONARY', 'DISCONNECT', 'DISPATCH', 'DISTINCT',
       'DO', 'DOMAIN', 'DROP', 'DYNAMIC', 'DYNAMIC_FUNCTION', 'DYNAMIC_FUNCTION_CODE',
       'EACH', 'ELSE', 'ELSIF', 'ENCODING', 'ENCRYPTED', 'END', 'END-EXEC', 'EQUALS',
       'ESCAPE', 'EVERY', 'EXCEPTION', 'EXCEPT', 'EXCLUDING', 'EXCLUSIVE', 'EXEC',
       'EXECUTE', 'EXISTING', 'EXISTS', 'EXPLAIN', 'EXTERNAL', 'EXTRACT', 'FALSE',
       'FETCH', 'FINAL', 'FIRST', 'FOR', 'FORCE', 'FOREIGN', 'FORTRAN', 'FORWARD',
       'FOUND', 'FREE', 'FREEZE', 'FROM', 'FULL', 'FUNCTION', 'G', 'GENERAL', 'GENERATED',
       'GET', 'GLOBAL', 'GO', 'GOTO', 'GRANT', 'GRANTED', 'GROUP', 'GROUPING', 'HANDLER',
       'HAVING', 'HIERARCHY', 'HOLD', 'HOST', 'IDENTITY', 'IF', 'IGNORE', 'ILIKE',
       ...
       'INTERSECT', 'INTO', 'INVOKER', 'IS', 'ISNULL', 'ISOLATION', 'ITERATE', 'JOIN',
       'KEY', 'KEY_MEMBER', 'KEY_TYPE', 'LANCOMPILER', 'LANGUAGE', 'LARGE', 'LAST',
       'LATERAL', 'LEADING', 'LEFT', 'LENGTH', 'LESS', 'LEVEL', 'LIKE', 'LIMIT',
       'LISTEN', 'LOAD', 'LOCAL', 'LOCALTIME', 'LOCALTIMESTAMP', 'LOCATION', 'LOCATOR',
       'LOCK', 'LOWER', 'MAP', 'MATCH', 'MAX', 'MAXVALUE', 'MESSAGE_LENGTH', 'MESSAGE_OCTET_LENGTH',
       'MESSAGE_TEXT', 'METHOD', 'MIN', 'MINUTE', 'MINVALUE', 'MOD', 'MODE', 'MODIFIES',
       'MODIFY', 'MONTH', 'MORE', 'MOVE', 'MUMPS', 'NAMES', 'NATIONAL', 'NATURAL',
       'NCHAR', 'NCLOB', 'NEW', 'NEXT', 'NO', 'NOCREATEDB', 'NOCREATEUSER', 'NONE',
       'NOT', 'NOTHING', 'NOTIFY', 'NOTNULL', 'NULL', 'NULLABLE', 'NULLIF', 'OBJECT',
       'OCTET_LENGTH', 'OF', 'OFF', 'OFFSET', 'OIDS', 'OLD', 'ON', 'ONLY', 'OPEN',
       'OPERATION', 'OPERATOR', 'OPTION', 'OPTIONS', 'OR', 'ORDER', 'ORDINALITY',
       'OUT', 'OUTER', 'OUTPUT', 'OVERLAPS', 'OVERLAY', 'OVERRIDING', 'OWNER', 'PAD',
       'PARAMETER', 'PARAMETERS', 'PARAMETER_MODE', 'PARAMATER_NAME', 'PARAMATER_ORDINAL_POSITION',
       'PARAMETER_SPECIFIC_CATALOG', 'PARAMETER_SPECIFIC_NAME', 'PARAMATER_SPECIFIC_SCHEMA',
       'PARTIAL', 'PASCAL', 'PENDANT', 'PLACING', 'PLI', 'POSITION', 'POSTFIX', 'PRECISION',
       'PREFIX', 'PREORDER', 'PREPARE', 'PRESERVE', 'PRIMARY', 'PRIOR', 'PRIVILEGES',
       'PROCEDURAL', 'PROCEDURE', 'PUBLIC', 'READ', 'READS', 'RECHECK', 'RECURSIVE',
       ...
       'ROUTINE_SCHEMA', 'ROW', 'ROWS', 'ROW_COUNT', 'RULE', 'SAVE_POINT', 'SCALE',
       'SCHEMA', 'SCHEMA_NAME', 'SCOPE', 'SCROLL', 'SEARCH', 'SECOND', 'SECURITY',
       'SELECT', 'SELF', 'SENSITIVE', 'SERIALIZABLE', 'SERVER_NAME', 'SESSION', 'SESSION_USER',
       'SET', 'SETOF', 'SETS', 'SHARE', 'SHOW', 'SIMILAR', 'SIMPLE', 'SIZE', 'SOME',
       'SOURCE', 'SPACE', 'SPECIFIC', 'SPECIFICTYPE', 'SPECIFIC_NAME', 'SQL', 'SQLCODE',
       'SQLERROR', 'SQLEXCEPTION', 'SQLSTATE', 'SQLWARNINIG', 'STABLE', 'START',
       'STATE', 'STATEMENT', 'STATIC', 'STATISTICS', 'STDIN', 'STDOUT', 'STORAGE',
       'STRICT', 'STRUCTURE', 'STYPE', 'SUBCLASS_ORIGIN', 'SUBLIST', 'SUBSTRING',
       'SUM', 'SYMMETRIC', 'SYSID', 'SYSTEM', 'SYSTEM_USER', 'TABLE', 'TABLE_NAME',
       ' TEMP', 'TEMPLATE', 'TEMPORARY', 'TERMINATE', 'THAN', 'THEN', 'TIMESTAMP',
       ...
       'TRUSTED', 'TYPE', 'UNCOMMITTED', 'UNDER', 'UNENCRYPTED', 'UNION', 'UNIQUE',
       'UNKNOWN', 'UNLISTEN', 'UNNAMED', 'UNNEST', 'UNTIL', 'UPDATE', 'UPPER', 'USAGE',
       'USER', 'USER_DEFINED_TYPE_CATALOG', 'USER_DEFINED_TYPE_NAME', 'USER_DEFINED_TYPE_SCHEMA',
       'USING', 'VACUUM', 'VALID', 'VALIDATOR', 'VALUES', 'VARIABLE', 'VERBOSE',
       'VERSION', 'VIEW', 'VOLATILE', 'WHEN', 'WHENEVER', 'WHERE', 'WITH', 'WITHOUT',
       'WORK', 'WRITE', 'YEAR', 'ZONE'),
         suffix='\\b'),
       Keyword),
      (
       words(('ARRAY', 'BIGINT', 'BINARY', 'BIT', 'BLOB', 'BOOLEAN', 'CHAR', 'CHARACTER',
       'DATE', 'DEC', 'DECIMAL', 'FLOAT', 'INT', 'INTEGER', 'INTERVAL', 'NUMBER',
       'NUMERIC', 'REAL', 'SERIAL', 'SMALLINT', 'VARCHAR', 'VARYING', 'INT8', 'SERIAL8',
       'TEXT'),
         suffix='\\b'),
       Name.Builtin),
      (
...
class TransactSqlLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       words((_tsql_builtins.OPERATOR_WORDS), suffix='\\b'), Operator.Word),
      (
       words((_tsql_builtins.TYPES), suffix='\\b'), Name.Class),
      (
       words((_tsql_builtins.FUNCTIONS), suffix='\\b'), Name.Function),
      (
       '(goto)(\\s+)(\\w+\\b)', bygroups(Keyword, Whitespace, Name.Label)),
      (
       words((_tsql_builtins.KEYWORDS), suffix='\\b'), Keyword),
      (
      ...
      (
       '@\\w+', Name.Variable),
      (
       '(\\w+)(:)', bygroups(Name.Label, Punctuation)),
      (
       '#?#?\\w+', Name),
      (
       '\\?', Name.Variable.Magic)], 
    ...
    def analyse_text(text):
        ...
        if tsql_declare_re.search(text):
            ...
            if dialect_name_count >= 1:
                if name_between_bracket_count >= 2 * name_between_backtick_count:
                    rating += 0.5
            if name_between_bracket_count > name_between_backtick_count:
                rating += 0.2
            else:
                if name_between_bracket_count > 0:
                    rating += 0.1
    ...
    def analyse_text(text):
        ...
        if dialect_name_count >= 1:
            if name_between_backtick_count >= 2 * name_between_bracket_count:
                rating += 0.5
        if name_between_backtick_count > name_between_bracket_count:
            rating += 0.2
        else:
            if name_between_backtick_count > 0:
                rating += 0.1
        return rating
...
class RqlLexer(RegexLexer):
    ...
    tokens = {'root': [
              (
               '\\s+', Text),
              (
               '(DELETE|SET|INSERT|UNION|DISTINCT|WITH|WHERE|BEING|OR|AND|NOT|GROUPBY|HAVING|ORDERBY|ASC|DESC|LIMIT|OFFSET|TODAY|NOW|TRUE|FALSE|NULL|EXISTS)\\b',
               Keyword),
              (
               '[+*/<>=%-]', Operator),
              (
               '(Any|is|instance_of|CWEType|CWRelation)\\b', Name.Builtin),
              (
               '[0-9]+', Number.Integer),
              (
               '[A-Z_]\\w*\\??', Name),
