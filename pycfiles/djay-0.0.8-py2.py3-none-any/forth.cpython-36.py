...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/forth.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 7179 bytes
"""
...
class ForthLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       '\\([\\s].*?\\)', Comment.Single),
      (
       '(:|variable|constant|value|buffer:)(\\s+)',
       bygroups(Keyword.Namespace, Text), 'worddef'),
      (
       '([.sc]")(\\s+?)', bygroups(String, Text), 'stringdef'),
      (
      ...
      (
       '(\\#|%|&|\\-|\\+)?[0-9.]+', Keyword.Type),
      (
       '(@i|!i|@e|!e|pause|noop|turnkey|sleep|itype|icompare|sp@|sp!|rp@|rp!|up@|up!|>a|a>|a@|a!|a@+|a@-|>b|b>|b@|b!|b@+|b@-|find-name|1ms|sp0|rp0|\\(evaluate\\)|int-trap|int!)' + delimiter,
       Name.Constant),
      (
       '(do-recognizer|r:fail|recognizer:|get-recognizers|set-recognizers|r:float|r>comp|r>int|r>post|r:name|r:word|r:dnum|r:num|recognizer|forth-recognizer|rec:num|rec:float|rec:word)' + delimiter, Name.Decorator),
      (
       '(Evalue|Rvalue|Uvalue|Edefer|Rdefer|Udefer)(\\s+)',
       bygroups(Keyword.Namespace, Text), 'worddef'),
      (
