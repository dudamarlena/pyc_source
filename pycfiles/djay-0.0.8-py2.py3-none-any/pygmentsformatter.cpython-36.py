...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/writers/odf_odt/pygmentsformatter.py
...
"""
import pygments, pygments.formatter
    ...
    def format(self, tokensource, outfile):
        ...
        for ttype, value in tokensource:
            ...
            if ttype == tokenclass.Keyword:
                ...
                if ttype == tokenclass.Literal.String:
    ...
    def format(self, tokensource, outfile):
        ...
        for ttype, value in tokensource:
            ...
            if ttype == tokenclass.Keyword:
                ...
                if ttype in (tokenclass.Literal.String,
                 tokenclass.Literal.String.Backtick):
                    s2 = self.rststyle('codeblock-string')
                    s1 = '<text:span text:style-name="%s">%s</text:span>' % (
                     s2, value)
                else:
                    if ttype == tokenclass.Name.Attribute:
                        s2 = self.rststyle('codeblock-operator')
                        s1 = '<text:span text:style-name="%s">%s</text:span>' % (
                         s2, value)
                    else:
                        if ttype == tokenclass.Comment:
                            ...
                            if ttype == tokenclass.Name.Builtin:
                                s2 = self.rststyle('codeblock-name')
                                s1 = '<text:span text:style-name="%s">%s</text:span>' % (
                                 s2, value)
                            else:
