...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/requests-toolbelt/requests_toolbelt/adapters/x509.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 7324 bytes
...
class X509Adapter(HTTPAdapter):
    ...
    def _check_version(self):
        if PyOpenSSLContext is None:
            raise exc.VersionMismatchError('The X509Adapter requires at least Requests 2.12.0 to be installed. Version {0} was found instead.'.format(requests.__version__))
...
def check_cert_dates(cert):
    ...
    if cert.not_valid_after < now or cert.not_valid_before > now:
        raise ValueError('Client certificate expired: Not After: {0:%Y-%m-%d %H:%M:%SZ} Not Before: {1:%Y-%m-%d %H:%M:%SZ}'.format(cert.not_valid_after, cert.not_valid_before))
