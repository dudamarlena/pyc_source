...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/session.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 24710 bytes
"""
...
import tox, py, os, sys, subprocess
...
def show_help(config):
    ...
    tw.line('TOX_TESTENV_PASSENV: space-separated list of extra environment variables to be passed into test command environments')
...
class Action(object):
    ...
    def popen(self, args, cwd=None, env=None, redirect=True, returnout=False, ignore_ret=False):
        ...
        if cwd is None:
            cwd = py.path.local()
        try:
            popen = self._popen(args, cwd, env=env, stdout=stdout,
              stderr=STDOUT)
        except OSError as e:
    ...
    def _rewriteargs(self, cwd, args):
        ...
        if sys.platform == 'win32':
            ...
            if ext == '.py':
                if self.venv:
                    newargs = [
                     str(self.envconfig.envpython)] + newargs
        return newargs
    ...
    def _popen(self, args, cwd, stdout, stderr, env=None):
        ...
        if env is None:
            env = os.environ.copy()
        return self.session.popen(args, shell=False, cwd=(str(cwd)), universal_newlines=True,
          stdout=stdout,
          stderr=stderr,
          env=env)
...
class Reporter(object):
    ...
    def logaction_finish(self, action):
        ...
        self.verbosity2(('%s finish: %s after %.2f seconds' % (
         action.venvname, action.msg, duration)),
          bold=True)
...
class Session:
    ...
    def runcommand(self):
        ...
        if self.config.minversion:
            ...
            if toxversion < minversion:
                self.report.error('tox version is %s, required is at least %s' % (
            ...
            if self.config.option.showconfig:
                ...
                if self.config.option.listenvs:
                    self.showenvs()
                else:
        ...
        with action:
            ...
            try:
                ...
                if not ''.join(data).strip():
                    ...
                    raise SystemExit(1)
                self.report.error('No dist directory found. Please check setup.py, e.g with:\n     python setup.py sdist')
                raise SystemExit(1)
    ...
    def get_installpkg_path(self):
        ...
        :rtype: py.path.local
        """
        if not self.config.option.sdistonly:
            if self.config.sdistsrc or self.config.option.installpkg:
            ...
            for venv in self.venvlist:
                if self.setupenv(venv):
                    if venv.envconfig.usedevelop:
                        ...
                        if self.config.skipsdist or venv.envconfig.skip_install:
                    ...
                    with action:
                        ...
                        output = venv._pcall([str(pip), 'freeze'], cwd=(self.config.toxinidir),
                          action=action)
                        output = output.split('\n\n')[(-1)]
