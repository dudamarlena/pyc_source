...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pytest/_pytest/core.py
# Compiled at: 2019-07-30 18:47:09
# Size of source mod 2**32: 17988 bytes
"""
pytest PluginManager, basic initialization and tracing.
"""
import os, sys, inspect, py
assert py.__version__.split('.')[:2] >= ['1', '4'], "installation problem: %s is too old, remove or upgrade 'py'" % py.__version__
...
class CallOutcome:
    __doc__ = ' Outcome of a function call, either an exception or a proper result.\n    Calling the ``get_result`` method will return the result or reraise\n    the exception raised when the function was called. '
    excinfo = None
...
class PluginManager(object):
        ...
        try:
            ...
            for ep in iter_entry_points('pytest11'):
                ...
                if name.startswith('pytest_'):
                    name = name[7:]
                if not ep.name in self._name2plugin:
    ...
    def consider_pluginarg(self, arg):
        ...
        elif self.getplugin(arg) is None:
            self.import_plugin(arg)

    def consider_conftest(self, conftestmodule):
        if self.register(conftestmodule, name=(conftestmodule.__file__), conftest=True):
    ...
    def call_plugin(self, plugin, methname, kwargs):
        return MultiCall(methods=self.listattr(methname, plugins=[plugin]), kwargs=kwargs,
          firstresult=True).execute()
...
class MultiCall:
    __doc__ = ' execute a call into multiple python functions/methods. '
    ...
    def _addhooks(self, hookspec, prefix):
        ...
        for name, method in vars(hookspec).items():
            if name.startswith(prefix):
                ...
                hc = HookCaller(self, name, firstresult=firstresult, argnames=varnames(method, startindex=isclass))
                    ...
                    if not isgenerichook(name) and not is_optional:
                        ...
                        for arg in varnames(method):
                            if arg not in hook.argnames:
                                fail('argument %r not available\nactual definition: %s\navailable hookargs: %s', arg, formatdef(method), ', '.join(hook.argnames))
...
class HookCaller:
    ...
    def new_cached_caller(self, methods):
        return HookCaller((self.hookrelay), (self.name), (self.firstresult), argnames=(self.argnames),
          methods=methods)
...
class PluginValidationError(Exception):
    __doc__ = ' plugin failed validation. '


def isgenerichook(name):
    return name == 'pytest_plugins' or name.startswith('pytest_funcarg__')
