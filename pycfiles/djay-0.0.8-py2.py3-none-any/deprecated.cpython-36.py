...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/requests-toolbelt/requests_toolbelt/utils/deprecated.py
...
find_charset = re.compile(b'<meta.*?charset=["\\\']*(.+?)["\\\'>]',
  flags=(re.I)).findall
find_pragma = re.compile(b'<meta.*?content=["\\\']*;?charset=(.+?)["\\\'>]',
  flags=(re.I)).findall
find_xml = re.compile(b'^<\\?xml.*?encoding=["\\\']*(.+?)["\\\'>]').findall
