...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_vendor/ipaddress.py
# Compiled at: 2019-07-30 18:46:55
# Size of source mod 2**32: 79852 bytes
...
def _compat_to_bytes(intval, length, endianess):
    if not isinstance(intval, _compat_int_types):
        ...
        assert endianess == 'big'
        if length == 4:
            if intval < 0 or intval >= 4294967296:
...
class AddressValueError(ValueError):
    __doc__ = 'A Value Error related to the address.'


class NetmaskValueError(ValueError):
    __doc__ = 'A Value Error related to the netmask.'
...
def ip_address(address):
    ...
    if isinstance(address, bytes):
        raise AddressValueError('%r does not appear to be an IPv4 or IPv6 address. Did you pass in a bytes (str in Python 2) instead of a unicode object?' % address)
...
def ip_network(address, strict=True):
    ...
    if isinstance(address, bytes):
        raise AddressValueError('%r does not appear to be an IPv4 or IPv6 network. Did you pass in a bytes (str in Python 2) instead of a unicode object?' % address)
...
class _IPAddressBase(_TotalOrderingMixin):
    ...
    def _check_packed_address(self, address, expected_len):
        ...
        if address_len != expected_len:
            msg = '%r (len %d != %d) is not permitted as an IPv%d address. Did you pass in a bytes (str in Python 2) instead of a unicode object?'
...
class _BaseNetwork(_IPAddressBase):
    __doc__ = 'A generic IP network object.\n\n    This IP class contains the version independent methods which are\n    used by networks.\n\n    '
    ...
    def _is_subnet_of(a, b):
        try:
            ...
            raise TypeError('Unable to test subnet containment between %s and %s' % (
             a, b))
...
class IPv4Address(_BaseV4, _BaseAddress):
    __doc__ = 'Represent and manipulate single IPv4 Addresses.'
...
class IPv4Network(_BaseV4, _BaseNetwork):
    __doc__ = "This class represents and manipulates 32-bit IPv4 network + addresses..\n\n    Attributes: [examples for IPv4Network('192.0.2.0/27')]\n        .network_address: IPv4Address('192.0.2.0')\n        .hostmask: IPv4Address('0.0.0.31')\n        .broadcast_address: IPv4Address('192.0.2.32')\n        .netmask: IPv4Address('255.255.255.224')\n        .prefixlen: 27\n\n    "
...
class _BaseV6(object):
            ...
            if skip_index is not None:
                ...
                if len(parts) != cls._HEXTET_COUNT:
                    ...
                    if not parts[0]:
                        msg = "Leading ':' only permitted as part of '::' in %r"
                        raise AddressValueError(msg % ip_str)
                    msg = parts[(-1)] or "Trailing ':' only permitted as part of '::' in %r"
                    raise AddressValueError(msg % ip_str)
                parts_hi = len(parts)
                parts_lo = 0
                parts_skipped = 0
    ...
    def _compress_hextets(cls, hextets):
        ...
        for index, hextet in enumerate(hextets):
            ...
            elif doublecolon_len > best_doublecolon_len:
...
class IPv6Address(_BaseV6, _BaseAddress):
    __doc__ = 'Represent and manipulate single IPv6 Addresses.'
    __slots__ = ('_ip', '__weakref__')
...
class IPv6Network(_BaseV6, _BaseNetwork):
    __doc__ = "This class represents and manipulates 128-bit IPv6 networks.\n\n    Attributes: [examples for IPv6('2001:db8::1000/124')]\n        .network_address: IPv6Address('2001:db8::1000')\n        .hostmask: IPv6Address('::f')\n        .broadcast_address: IPv6Address('2001:db8::100f')\n        .netmask: IPv6Address('ffff:ffff:ffff:ffff:ffff:ffff:ffff:fff0')\n        .prefixlen: 124\n\n    "
