...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/_pytestplugin.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 10280 bytes
import py, pytest, tox, os, sys
...
class ReportExpectMock:
    ...
    def getnext(self, cat):
        ...
        raise LookupError('looking for %r, no reports found at >=%d in %r' % (
    ...
    def expect(self, cat, messagepattern='*', invert=False):
        ...
        if not messagepattern.startswith('*'):
            ...
            assert invert, 'looking for %s(%r), no reports found at >=%d in %r' % (
...
def mocksession(request):
    ...
    class MockSession(Session):
        ...
        def popen(self, args, cwd, shell=None, universal_newlines=False, stdout=None, stderr=None, env=None):
...
class Cmd:
    ...
    def run(self, *argv):
        ...
        f2 = p2.open('wb')
        now = time.time()
        popen = self.popen(argv, stdout=f1, stderr=f2, close_fds=(sys.platform != 'win32'))
        ret = popen.wait()
        f1.close()
        f2.close()
        out = p1.read('rb')
    ...
    def initproj(nameversion, filedefs=None):
        if filedefs is None:
            ...
            if name not in filedefs:
                create_files(base, {name: {'__init__.py': '__version__ = %r' % version}})
        manifestlines = []
