...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/requests-toolbelt/requests_toolbelt/downloadutils/stream.py
...
_QUOTED_STRING_RE = '"[^"\\\\]*(?:\\\\.[^"\\\\]*)*"'
_OPTION_HEADER_PIECE_RE = re.compile(';\\s*(%s|[^\\s;=]+)\\s*(?:=\\s*(%s|[^;]+))?\\s*' % (_QUOTED_STRING_RE,
 _QUOTED_STRING_RE))
