...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/py/py/_path/local.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 36697 bytes
"""
local path implementation.
"""
from __future__ import with_statement
from contextlib import contextmanager
import sys, os, atexit, io, uuid, py
...
class LocalPath(FSBase):
    __doc__ = ' object oriented interface to os.path and other local filesystem\n        related information.\n    '

    class ImportMismatchError(ImportError):
        __doc__ = " raised on pyimport() if there is a mismatch of __file__'s"
    ...
    def remove(self, rec=1, ignore_errors=False):
        ...
        if self.check(dir=1, link=0):
            if rec:
                ...
                import shutil
                py.error.checked_call((shutil.rmtree),
                  (self.strpath), ignore_errors=ignore_errors)
            else:
        ...
        try:
            ...
            if ext:
                if not ext.startswith('.'):
                    ext = '.' + ext
                kw['basename'] = pb + ext
            elif 'dirname' in kw and not kw['dirname']:
                kw['dirname'] = drive
            else:
        ...
        if ensure:
            ...
            if 'b' in mode:
                ...
                if not py.builtin._istext(data):
                    data = py.builtin._isbytes(data) or str(data)
                else:
        ...
        if not self.check():
            ...
            if ensuresyspath == 'importlib':
                if modname is None:
                    ...
                    if spec is None:
                        raise ImportError("Can't find module %s at location %s" % (
                         modname, str(self)))
    ...
    def sysexec(self, *argv, **popen_opts):
        ...
        if ret != 0:
            ...
            raise py.process.cmdexec.Error(ret, ret, str(self), stdout, stderr)
        return stdout
        ...
        if rootdir is None:
            ...
            def create_lockfile(path):
                ...
                if hasattr(lockfile, 'mksymlinkto'):
                    ...
                    fd = py.error.checked_call(os.open, str(lockfile), os.O_WRONLY | os.O_CREAT | os.O_EXCL, 420)
