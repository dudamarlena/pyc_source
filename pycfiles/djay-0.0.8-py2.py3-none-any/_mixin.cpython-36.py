...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/urllib3/urllib3/packages/rfc3986/_mixin.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 13214 bytes
...
class URIMixin(object):
    __doc__ = 'Mixin with all shared methods for URIs and IRIs.'
        ...
        if not self.authority:
            ...
            if host:
                if misc.IPv4_MATCHER.match(host):
                    if not validators.valid_ipv4_host_address(host):
                        raise exc.InvalidAuthority(self.authority.encode(self.encoding))
            return matches
    ...
    def is_valid(self, **kwargs):
        ...
        :rtype: bool
        """
        warnings.warn('Please use rfc3986.validators.Validator instead. This method will be eventually removed.', DeprecationWarning)
        validators = [
         (
    ...
    def authority_is_valid(self, require=False):
        ...
        try:
            ...
            return validators.authority_is_valid((self.authority),
              host=(self.host),
              require=require)
    ...
    def resolve_with(self, base_uri, strict=False):
        ...
        if not isinstance(base_uri, URIMixin):
        ...
        elif not base_uri.is_absolute():
            raise exc.ResolutionError(base_uri)
        else:
            base_uri = base_uri.normalize()
            resolving = self
            if not strict:
                if resolving.scheme == base_uri.scheme:
            ...
            if resolving.scheme is not None:
                ...
                if resolving.authority is not None:
                    target = resolving.copy_with(scheme=(base_uri.scheme),
    ...
    def copy_with(self, scheme=misc.UseExisting, authority=misc.UseExisting, path=misc.UseExisting, query=misc.UseExisting, fragment=misc.UseExisting):
