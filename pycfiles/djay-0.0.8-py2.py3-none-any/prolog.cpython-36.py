...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/prolog.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 12031 bytes
"""
...
class LogtalkLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       'logtalk_(compile|l(ibrary_path|oad|oad_context)|make)(?=[(])', Keyword),
      (
      ...
      (
       '(fa(il|lse)|true)\\b', Keyword),
      (
       '((bag|set)of|f(ind|or)all)(?=[(])', Keyword),
      (
       'threaded(_(call|once|ignore|exit|peek|wait|notify))?(?=[(])', Keyword),
      (
     ...
     'directive':[
      ...
      (
       '(alias|d(ynamic|iscontiguous)|m(eta_(non_terminal|predicate)|ode|ultifile)|s(et_(logtalk|prolog)_flag|ynchronized))(?=[(])',
       Keyword, 'root'),
      (
       'op(?=[(])', Keyword, 'root'),
      (
