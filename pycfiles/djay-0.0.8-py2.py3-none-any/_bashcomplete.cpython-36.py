...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/click/click/_bashcomplete.py
# Compiled at: 2019-07-30 18:47:04
# Size of source mod 2**32: 11014 bytes
import copy, os, re
...
def get_completion_script(prog_name, complete_var, shell):
    ...
    return (script % {'complete_func':'_%s_completion' % cf_name, 
     'script_names':prog_name, 
     'autocomplete_var':complete_var}).strip() + ';'
...
def is_incomplete_option(all_args, cmd_param):
    ...
    if not isinstance(cmd_param, Option):
        ...
        if last_option:
            if last_option in cmd_param.opts:
                return True
        return False
...
def is_incomplete_argument(current_params, cmd_param):
    ...
    if not isinstance(cmd_param, Argument):
        ...
        if isinstance(current_param_values, abc.Iterable):
            if cmd_param.nargs > 1:
                if len(current_param_values) < cmd_param.nargs:
                    return True
        return False
...
def get_user_autocompletions(ctx, args, incomplete, cmd_param):
    ...
    if isinstance(cmd_param.type, Choice):
        ...
        if cmd_param.autocompletion is not None:
            dynamic_completions = cmd_param.autocompletion(ctx=ctx, args=args,
...
def get_choices(cli, prog_name, args, incomplete):
    ...
    if ctx is None:
        ...
        if start_of_option(incomplete):
            if WORDBREAK in incomplete:
                partition_incomplete = incomplete.partition(WORDBREAK)
                all_args.append(partition_incomplete[0])
                incomplete = partition_incomplete[2]
        else:
            if incomplete == WORDBREAK:
                incomplete = ''
            completions = []
            if start_of_option(incomplete):
...
def bashcomplete(cli, prog_name, complete_var, complete_instr):
    if complete_instr.startswith('source'):
        ...
        if complete_instr == 'complete' or complete_instr == 'complete_zsh':
