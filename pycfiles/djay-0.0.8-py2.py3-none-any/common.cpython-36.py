...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/py/py/_path/common.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 14626 bytes
"""
"""
import warnings, os, sys, posixpath, fnmatch, py
...
class PathBase(object):
    __doc__ = ' shared implementation for filesystem path objects.'
    Checkers = Checkers
    ...
    def _sortlist(self, res, sort):
        if sort:
            if hasattr(sort, '__call__'):
                warnings.warn((DeprecationWarning('listdir(sort=callable) is deprecated and breaks on python3')),
                  stacklevel=3)
                res.sort(sort)
            else:
                res.sort()
...
class Visitor:
    ...
    def gen(self, path):
        try:
            ...
            rec = self.rec
            dirs = self.optsort([p for p in entries if p.check(dir=1) if rec is None or rec(p)])
...
class FNMatcher:
    ...
    def __call__(self, path):
        ...
        if pattern.find(path.sep) == -1:
            if iswin32:
                if pattern.find(posixpath.sep) != -1:
