...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/py/py/_xmlgen.py
...
class NamespaceMetaclass(type):
    ...
    def __getattr__(self, name):
        if name[:1] == '_':
            ...
            if tagspec is not None:
                if name not in tagspec:
                    raise AttributeError(name)
            classattr = {}
...
Namespace = NamespaceMetaclass('Namespace', (object,), {'__tagspec__':None, 
 '__tagclass__':Tag, 
...
class raw(object):
    __doc__ = 'just a box that can contain a unicode string that will be\n    included directly in the output'
...
class SimpleUnicodeVisitor(object):
    __doc__ = ' recursive visitor to write unicode. '
    ...
    def visit(self, node):
        ...
        try:
            ...
            for subclass in cls.__mro__:
                ...
                visitmethod = self._SimpleUnicodeVisitor__object
    ...
    def Tag(self, tag):
        if not id(tag) not in self.visited:
