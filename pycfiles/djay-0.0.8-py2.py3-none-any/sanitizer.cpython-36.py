...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_vendor/html5lib/filters/sanitizer.py
# Compiled at: 2019-07-30 18:46:56
# Size of source mod 2**32: 26248 bytes
...
attr_val_is_uri = frozenset((
 ...
 (
  namespaces['xml'], 'base')))
svg_attr_val_allows_ref = frozenset(((None, 'clip-path'), (None, 'color-profile'),
                                     (None, 'cursor'), (None, 'fill'), (None, 'filter'),
                                     (None, 'marker'), (None, 'marker-start'), (None, 'marker-mid'),
                                     (None, 'marker-end'), (None, 'mask'), (None, 'stroke')))
svg_allow_local_href = frozenset(((None, 'altGlyph'), (None, 'animate'), (None, 'animateColor'),
                                  (None, 'animateMotion'), (None, 'animateTransform'),
                                  (None, 'cursor'), (None, 'feImage'), (None, 'filter'),
                                  (None, 'linearGradient'), (None, 'pattern'), (None, 'radialGradient'),
                                  (None, 'textpath'), (None, 'tref'), (None, 'set'),
                                  (None, 'use')))
...
class Filter(base.Filter):
    __doc__ = 'Sanitizes token stream of XHTML+MathML+SVG and of inline style attributes'
    ...
    def allowed_token(self, token):
        if 'data' in token:
            ...
            if token['name'] in self.svg_allow_local_href:
                if (
                 namespaces['xlink'], 'href') in attrs:
                    if re.search('^\\s*[^#\\s].*', attrs[(namespaces['xlink'], 'href')]):
            ...
            for prop, value in re.findall('([-\\w]+)\\s*:\\s*([^:;]*)', style):
                if not value:
                    ...
                    if prop.lower() in self.allowed_css_properties:
                        ...
                        if prop.split('-')[0].lower() in ('background', 'border', 'margin',
                                                          'padding'):
                            for keyword in value.split():
                                if keyword not in self.allowed_css_keywords:
                                    if not re.match('^(#[0-9a-fA-F]+|rgb\\(\\d+%?,\\d*%?,?\\d*%?\\)?|\\d{0,2}\\.?\\d{0,2}(cm|em|ex|in|mm|pc|pt|px|%|,|\\))?)$', keyword):
                                        break
                            else:
                                clean.append(prop + ': ' + value + ';')

                        else:
                            if prop.lower() in self.allowed_svg_properties:
                                clean.append(prop + ': ' + value + ';')
