...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/_verlib.py
...
class IrrationalVersionError(Exception):
    __doc__ = 'This is an irrational version.'


class HugeMajorVersionNumError(IrrationalVersionError):
    __doc__ = 'An irrational version because the major version number is huge\n    (often because a year or date was used).\n\n    See `error_on_huge_major_num` option in `NormalizedVersion` for details.\n    This guard can be disabled by setting that option False.\n    '
...
class NormalizedVersion(object):
    ...
    def _parse(self, s, error_on_huge_major_num=True):
        ...
        if error_on_huge_major_num:
            if self.parts[0][0] > 1980:
    ...
    def parts_to_str(cls, parts):
        ...
        if prerel is not FINAL_MARKER:
            ...
            if postdev:
                if postdev is not FINAL_MARKER:
                    if postdev[0] == 'f':
                        postdev = postdev[1:]
                    i = 0
                    while i < len(postdev):
...
def suggest_normalized_version(s):
    ...
    rs = s.lower()
    for orig, repl in (('-alpha', 'a'), ('-beta', 'b'), ('alpha', 'a'), ('beta', 'b'),
                       ('rc', 'c'), ('-final', ''), ('-pre', 'c'), ('-release', ''),
                       ('.release', ''), ('-stable', ''), ('+', '.'), ('_', '.'),
                       (' ', ''), ('.final', ''), ('final', '')):
