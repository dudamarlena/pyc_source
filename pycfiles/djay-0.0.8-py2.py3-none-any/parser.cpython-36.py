...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/pyyaml/yaml/parser.py
# Compiled at: 2019-07-30 18:47:04
# Size of source mod 2**32: 25495 bytes
__all__ = [
 'Parser', 'ParserError']
...
class Parser:
    ...
    def parse_document_start(self):
        ...
        if not self.check_token(StreamEndToken):
            ...
            event = DocumentStartEvent(start_mark, end_mark, explicit=True,
              version=version,
              tags=tags)
            self.states.append(self.parse_document_end)
    ...
    def process_directives(self):
        ...
        if self.tag_handles:
            value = (
             self.yaml_version, self.tag_handles.copy())
        else:
            value = (
             self.yaml_version, None)
        ...
        if self.check_token(AliasToken):
            ...
            if self.check_token(AnchorToken):
                ...
                if self.check_token(ScalarToken):
                    ...
                    event = ScalarEvent(anchor, tag, implicit, (token.value), start_mark,
                      end_mark, style=(token.style))
                    self.state = self.states.pop()
                else:
                    if self.check_token(FlowSequenceStartToken):
                        end_mark = self.peek_token().end_mark
                        event = SequenceStartEvent(anchor, tag, implicit, start_mark,
                          end_mark, flow_style=True)
                        self.state = self.parse_flow_sequence_first_entry
                    else:
                        if self.check_token(FlowMappingStartToken):
                            end_mark = self.peek_token().end_mark
                            event = MappingStartEvent(anchor, tag, implicit, start_mark,
                              end_mark, flow_style=True)
    ...
    def parse_flow_sequence_entry(self, first=False):
        if not (self.check_token(FlowSequenceEndToken) or first):
            ...
            if self.check_token(KeyToken):
                token = self.peek_token()
                event = MappingStartEvent(None, None, True, (token.start_mark),
                  (token.end_mark), flow_style=True)
                self.state = self.parse_flow_sequence_entry_mapping_key
                return event
            self.check_token(FlowSequenceEndToken) or self.states.append(self.parse_flow_sequence_entry)
    ...
    def parse_flow_mapping_key(self, first=False):
        if not self.check_token(FlowMappingEndToken):
            if not first:
                ...
                if self.check_token(KeyToken):
                    token = self.get_token()
                    self.check_token(ValueToken, FlowEntryToken, FlowMappingEndToken) or self.states.append(self.parse_flow_mapping_value)
                    return self.parse_flow_node()
                else:
                    self.state = self.parse_flow_mapping_value
                    return self.process_empty_scalar(token.end_mark)
            else:
                self.check_token(FlowMappingEndToken) or self.states.append(self.parse_flow_mapping_empty_value)
            return self.parse_flow_node()
        else:
            token = self.get_token()
            event = MappingEndEvent(token.start_mark, token.end_mark)
