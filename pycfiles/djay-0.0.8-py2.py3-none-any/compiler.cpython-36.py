...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/jinja2/jinja2/compiler.py
# Compiled at: 2019-07-30 18:47:04
# Size of source mod 2**32: 65386 bytes
"""
    jinja2.compiler
    ~~~~~~~~~~~~~~~
...
def optimizeconst(f):
    ...
    def new_func(self, node, frame, **kwargs):
        if self.optimized:
            if not frame.eval_ctx.volatile:
...
class Frame(object):
    __doc__ = 'Holds compile time information for us.'
...
class VisitorExit(RuntimeError):
    __doc__ = 'Exception used by the `UndeclaredNameVisitor` to signal a stop.'


class DependencyFinderVisitor(NodeVisitor):
    __doc__ = 'A visitor that collects filter and test calls.'
...
class UndeclaredNameVisitor(NodeVisitor):
    __doc__ = 'A visitor that checks if a name is accessed without being\n    declared.  This is different from the frame visitor as it will\n    not stop at closure frames.\n    '
...
class CompilerExit(Exception):
    __doc__ = "Raised if the compiler encountered a situation where it just\n    doesn't make sense to further process the code.  Any block that\n    raises such an exception is not further processed.\n    "
...
class CodeGenerator(NodeVisitor):
    ...
    def newline(self, node=None, extra=0):
        ...
        if node is not None:
            if node.lineno != self._last_line:
    ...
    def enter_frame(self, frame):
        ...
        for target, (action, param) in iteritems(frame.symbols.loads):
            ...
            if action == VAR_LOAD_RESOLVE:
                ...
                if action == VAR_LOAD_ALIAS:
                    self.writeline('%s = %s' % (target, param))
                else:
                    if action == VAR_LOAD_UNDEFINED:
    ...
    def macro_body(self, node, frame):
        ...
        if 'kwargs' in undeclared:
            if 'kwargs' not in skip_special_params:
                args.append(frame.symbols.declare_parameter('kwargs'))
                macro_ref.accesses_kwargs = True
        if 'varargs' in undeclared:
            if 'varargs' not in skip_special_params:
    ...
    def macro_def(self, macro_ref, frame):
        ...
        if len(macro_ref.node.args) == 1:
            arg_tuple += ','
        self.write('Macro(environment, macro, %r, (%s), %r, %r, %r, context.eval_ctx.autoescape)' % (
    ...
    def visit_Template(self, node, frame=None):
        ...
        for name, block in iteritems(self.blocks):
            self.writeline('%s(context, missing=missing%s):' % (
             self.func('block_' + name), envenv), block, 1)
            self.indent()
            self.write_commons()
            ...
            if 'super' in undeclared:
                ...
                self.writeline('%s = context.super(%r, block_%s)' % (
                 ref, name, name))
        ...
        self.writeline(('blocks = {%s}' % ', '.join('%r: block_%s' % (x, x) for x in self.blocks)),
          extra=1)
        ...
        if node.ignore_missing:
            ...
            if isinstance(node.template, nodes.Const):
                ...
                if node.with_context:
                    loop = self.environment.is_async and 'async for' or 'for'
                    self.writeline('%s event in template.root_render_func(template.new_context(context.get_all(), True, %s)):' % (
    ...
    def visit_Import(self, node, frame):
        ...
        if frame.toplevel:
            if not node.target.startswith('_'):
    ...
    def visit_FromImport(self, node, frame):
        ...
        for name in node.names:
            if isinstance(name, tuple):
                ...
                alias = name
            self.writeline('%s = getattr(included_template, %r, missing)' % (
             frame.symbols.ref(alias), name))
            self.writeline('if %s is missing:' % frame.symbols.ref(alias))
            self.indent()
            self.writeline('%s = undefined(%r %% included_template.__name__, name=%r)' % (
             frame.symbols.ref(alias),
             'the template %%r (imported on %s) does not export the requested name %s' % (
              self.position(node),
              repr(name)),
             name))
            self.outdent()
        ...
        if self.has_known_extends:
            ...
            if self.environment.finalize:
                ...
                if getattr(func, 'contextfunction', False) or getattr(func, 'evalcontextfunction', False):
                    ...
                    if getattr(func, 'environmentfunction', False):
                ...
                for argument in arguments:
                    ...
                    if self.environment.finalize is not None:
                        ...
                        if getattr(self.environment.finalize, 'contextfunction', False):
                            ...
                            if getattr(self.environment.finalize, 'evalcontextfunction', False):
                                self.write('context.eval_ctx, ')
                            else:
                                if getattr(self.environment.finalize, 'environmentfunction', False):
    ...
    def binop(operator, interceptable=True):
        ...
        def visitor(self, node, frame):
            if self.environment.sandboxed:
                if operator in self.environment.intercepted_binops:
    ...
    def uaop(operator, interceptable=True):
        ...
        def visitor(self, node, frame):
            if self.environment.sandboxed:
                if operator in self.environment.intercepted_unops:
            ...
            if getattr(func, 'contextfilter', False):
                ...
                if getattr(func, 'evalcontextfilter', False):
                    ...
                    if node.node is not None:
                        ...
                        if frame.eval_ctx.volatile:
                            self.write('(context.eval_ctx.autoescape and Markup(concat(%s)) or concat(%s))' % (
