...
class CovController(object):
    __doc__ = 'Base class for different plugin implementations.'
    ...
    def summary(self, stream):
        ...
        if self.failed_slaves:
            ...
            stream.write('The following slaves failed to return coverage data, ensure that pytest-cov is installed on these slaves.\n')
            for node in self.failed_slaves:
                stream.write('%s\n' % node.gateway.id)


class Central(CovController):
    __doc__ = 'Implementation for centralised operation.'

    def start(self):
        """Erase any previous coverage data and start coverage."""
        self.cov = coverage.coverage(source=(self.cov_source), data_file=(self.cov_data_file),
...
class DistMaster(CovController):
    __doc__ = 'Implementation for distributed master.'
    ...
    def testnodedown(self, node, error):
        ...
        if 'cov_slave_lines' in node.slaveoutput:
            cov = coverage.coverage(source=(self.cov_source), data_file=(self.cov_data_file),
...
class DistSlave(CovController):
    __doc__ = 'Implementation for distributed slaves.'

    def start(self):
        """Determine what data file and suffix to contribute to and start coverage."""
        self.is_collocated = bool(socket.gethostname() == self.config.slaveinput['cov_master_host'] and self.topdir == self.config.slaveinput['cov_master_topdir'])
