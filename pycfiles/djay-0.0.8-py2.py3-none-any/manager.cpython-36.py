...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/flake8/flake8/plugins/manager.py
...
class Plugin(object):
    __doc__ = 'Wrap an EntryPoint from setuptools and other logic.'
    ...
    def __repr__(self):
        ...
        return 'Plugin(name="{0}", entry_point="{1}")'.format(self.name, self.entry_point)
    ...
    def _load(self):
        ...
        if not callable(self._plugin):
            msg = 'Plugin %r is not a callable. It might be written for an older version of flake8 and might not work with this version' % self._plugin
            LOG.critical(msg)
            raise TypeError(msg)
    ...
    def load_plugin(self):
        ...
        if self._plugin is None:
            ...
            try:
                ...
                failed_to_load = exceptions.FailedToLoadPlugin(plugin=self,
    ...
    def enable(self, optmanager, options=None):
        ...
        try:
            ...
            LOG.debug('Attempted to remove %s from the ignore list but it was not a member of the list.', self.name)
    ...
    def register_options(self, optmanager):
        ...
        if add_options is not None:
            LOG.debug('Registering options from plugin "%s" on OptionManager %r', self.name, optmanager)
...
class PluginManager(object):
    __doc__ = 'Find and manage plugins consistently.'
    ...
    def _load_entrypoint_plugins(self):
        ...
        for entry_point in entrypoints.get_group_all(self.namespace):
            if entry_point.name == 'per-file-ignores':
                LOG.warning('flake8-per-file-ignores plugin is incompatible with flake8>=3.7 (which implements per-file-ignores itself).')
            else:
...
class PluginTypeManager(object):
    __doc__ = 'Parent class for most of the specific plugin types.'
    namespace = None
    ...
    def register_options(self, optmanager):
        ...
        self.load_plugins()
        call_register_options = self._generate_call_function('register_options', optmanager)
        list(self.manager.map(call_register_options))

    def provide_options(self, optmanager, options, extra_args):
        """Provide parsed options and extra arguments to the plugins."""
        call_provide_options = self._generate_call_function('provide_options', optmanager, options, extra_args)
        list(self.manager.map(call_provide_options))


class Checkers(PluginTypeManager):
    __doc__ = 'All of the checkers registered through entry-points or config.'
    ...
    def to_dictionary(self):
        ...
        return {'ast_plugins':[plugin.to_dictionary() for plugin in self.ast_plugins], 
         'logical_line_plugins':[plugin.to_dictionary() for plugin in self.logical_line_plugins], 
...
class ReportFormatters(PluginTypeManager):
    __doc__ = 'All of the report formatters registered through entry-points/config.'
