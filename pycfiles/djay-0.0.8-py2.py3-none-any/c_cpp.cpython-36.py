...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/c_cpp.py
    ...
    tokens = {'whitespace':[
     ...
     'statements':[
      ...
      (
       '[()\\[\\],.]', Punctuation),
      (
       words(('asm', 'auto', 'break', 'case', 'const', 'continue', 'default', 'do', 'else',
       'enum', 'extern', 'for', 'goto', 'if', 'register', 'restricted', 'return',
      ...
      (
       ...
       Keyword.Type),
      (
       words(('inline', '_inline', '__inline', 'naked', 'restrict', 'thread', 'typename'),
        ...
        for index, token, value in RegexLexer.get_tokens_unprocessed(self, text):
            if token is Name:
                if self.stdlibhighlighting:
                    if value in self.stdlib_types:
                        token = Keyword.Type
                if self.c99highlighting and value in self.c99_types:
                    token = Keyword.Type
                elif self.platformhighlighting:
                    if value in self.linux_types:
                        token = Keyword.Type
            yield (
             index, token, value)
...
class CppLexer(CFamilyLexer):
    ...
    tokens = {'statements':[
      (
       words(('catch', 'const_cast', 'delete', 'dynamic_cast', 'explicit', 'export', 'friend',
       'mutable', 'namespace', 'new', 'operator', 'private', 'protected', 'public',
       'reinterpret_cast', 'restrict', 'static_cast', 'template', 'this', 'throw',
       'throws', 'try', 'typeid', 'typename', 'using', 'virtual', 'constexpr', 'nullptr',
       'decltype', 'thread_local', 'alignas', 'alignof', 'static_assert', 'noexcept',
       'override', 'final'),
         suffix='\\b'), Keyword),
      (
      ...
      (
       '(u8|u|U)(")', bygroups(String.Affix, String), 'string'),
      inherit], 
     'root':[
      inherit,
      (
