...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/py/py/_path/svnwc.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 43825 bytes
"""
...
"""
import os, sys, time, re, calendar, py, subprocess
...
class SvnPathBase(common.PathBase):
    __doc__ = ' Base implementation for SvnPath implementations. '
    sep = '/'
...
class PropListDict(dict):
    __doc__ = ' a Dictionary which fetches values (InfoSvnCommand instances) lazily'
...
_reg_allow_disk = re.compile('^([a-z]\\:\\\\)?[^:]+$', re.I)
...
class SvnAuth(object):
    __doc__ = ' container for auth information for Subversion '
...
rex_blame = re.compile('\\s*(\\d+)\\s*(\\S+) (.*)')

class SvnWCCommandPath(common.PathBase):
    __doc__ = ' path implementation offering access/modification to svn working copies.\n        It has methods similar to the functions in os.path and similar to the\n        commands of the svn client.\n    '
    sep = os.sep
    ...
    def __new__(cls, wcpath=None, auth=None):
        ...
        if _check_for_bad_chars(str(wcpath), ALLOWED_CHARS):
    ...
    def _geturl(self):
        if getattr(self, '_url', None) is None:
            info = self.info()
            self._url = info.url
        elif not isinstance(self._url, py.builtin._basestring):
    ...
    def remove(self, rec=1, force=1):
        ...
        """
        if not rec:
            raise AssertionError('svn cannot remove non-recursively')
        else:
    ...
    def blame(self):
        ...
        for i, (blameline, line) in enumerate(zip(blamelines, reallines)):
            ...
            if not m:
                raise ValueError('output line %r of svn blame does not match expected format' % (
                 line,))
            rev, name, _ = m.groups()
        ...
        if not info:
            try:
                ...
                if e.err.find('Path is not a working copy directory') != -1:
                    ...
                    if e.err.find('is not under version control') != -1:
                        raise py.error.ENOENT(self, e.err)
                raise
...
class WCStatus:
    attrnames = ('modified', 'added', 'conflict', 'unchanged', 'external', 'deleted',
                 'prop_modified', 'unknown', 'update_available', 'incomplete', 'kindmismatch',
                 'ignored', 'locked', 'replaced')
            ...
            if not line.strip():
                ...
                if c0 in '?XI':
                    ...
                    if c0 == '?':
                        ...
                        if c0 == 'X':
                            wcpath = rootwcpath.__class__(rootwcpath.localpath.join(fn, abs=1),
    ...
    def fromstring(data, rootwcpath, rev=None, modrev=None, author=None):
        ...
        for entryel in doc.getElementsByTagName('entry'):
            ...
            if itemstatus == 'unversioned':
                ...
                if itemstatus == 'external':
                    wcpath = rootwcpath.__class__(rootwcpath.localpath.join(path, abs=1),
                      auth=(rootwcpath.auth))
                    rootstatus.external.append(wcpath)
                    continue
                else:
                    if itemstatus == 'ignored':
                        wcpath = rootwcpath.join(path, abs=1)
                        rootstatus.ignored.append(wcpath)
                        continue
                    else:
            ...
            wcpath = rootwcpath.join(path, abs=1)
            if not itemstatus != 'modified':
                if not wcpath.check(file=1):
                    raise AssertionError("did't expect a directory with changed content here")
            itemattrname = {'normal':'unchanged',  'unversioned':'unknown', 
             'conflicted':'conflict', 
             'none':'added'}.get(itemstatus, itemstatus)
        ...
        for lpart in filter(None, logentry.childNodes):
            if lpart.nodeType == lpart.ELEMENT_NODE:
                if lpart.nodeName == 'author':
                    ...
                    if lpart.nodeName == 'msg':
                        if lpart.firstChild:
                            self.msg = lpart.firstChild.nodeValue
                        else:
                            self.msg = ''
                    else:
                        if lpart.nodeName == 'date':
