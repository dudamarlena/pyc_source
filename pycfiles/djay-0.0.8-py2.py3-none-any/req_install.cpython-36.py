...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_internal/req/req_install.py
# Compiled at: 2019-07-30 18:46:55
# Size of source mod 2**32: 40296 bytes
...
class InstallRequirement(object):
    ...
    def remove_temporary_source(self):
        ...
        for deletion"""
        if self.source_dir:
            if os.path.exists(os.path.join(self.source_dir, PIP_DELETE_MARKER_FILENAME)):
    ...
    def run_egg_info(self):
        if self.name:
            logger.debug('Running setup.py (path:%s) egg_info for package %s', self.setup_py_path, self.name)
        else:
            logger.debug('Running setup.py (path:%s) egg_info for package from %s', self.setup_py_path, self.link)
    ...
    def install_editable(self, install_options, global_options=(), prefix=None):
        ...
        with indent_log():
            with self.build_env:
                call_subprocess((make_setuptools_shim_args(self.setup_py_path) + list(global_options) + ['develop', '--no-deps'] + list(install_options)),
                  cwd=(self.setup_py_dir))
            ...
            if create_archive:
                ...
                for dirpath, dirnames, filenames in os.walk(dir):
                    ...
                    for dirname in dirnames:
                        dir_arcname = self._get_archive_name(dirname, parentdir=dirpath,
                          rootdir=dir)
                    ...
                    for filename in filenames:
                        if filename == PIP_DELETE_MARKER_FILENAME:
                            ...
                            file_arcname = self._get_archive_name(filename, parentdir=dirpath,
                              rootdir=dir)
    ...
    def get_install_args(self, global_options, record_filename, root, prefix, pycompile):
        install_args = make_setuptools_shim_args((self.setup_py_path), unbuffered_output=True)
