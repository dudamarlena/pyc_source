...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pytest/_pytest/python.py
# Compiled at: 2019-07-30 18:47:09
# Size of source mod 2**32: 80816 bytes
""" Python test discovery, setup and run of test functions. """
import fnmatch, py, inspect, sys, pytest
...
class FixtureFunctionMarker:
    ...
    def __init__(self, scope, params, autouse=False, yieldctx=False, ids=None):
        self.scope = scope
        self.params = params
...
def yield_fixture(scope='function', params=None, autouse=False, ids=None):
    ...
    if callable(scope):
        if params is None:
            if autouse == False:
                return FixtureFunctionMarker('function',
                  params, autouse, yieldctx=True)(scope)
...
def pytest_addoption(parser):
    ...
    group.addoption('--fixtures', '--funcargs', action='store_true',
      dest='showfixtures',
      default=False,
...
def pytest_configure(config):
    ...
    config.addinivalue_line('markers', 'usefixtures(fixturename1, fixturename2, ...): mark tests as needing all of the specified fixtures. see http://pytest.org/latest/fixture.html#usefixtures ')
...
def pytest_namespace():
    ...
    return {'fixture':fixture, 
     ...
     'collect':{'Module':Module, 
      'Class':Class,  'Instance':Instance,  'Function':Function, 
      'Generator':Generator,  '_fillfuncargs':fillfixtures}}
...
def pytest_pycollect_makeitem(collector, name, obj):
    ...
    if res is not None:
        ...
        elif collector.funcnamefilter(name):
            if hasattr(obj, '__call__'):
                if getfixturemarker(obj) is None:
                    obj = getattr(obj, '__func__', obj)
                    if not isfunction(obj):
                        collector.warn(code='C2', message=('cannot collect %r because it is not a function.' % name))
...
def is_generator(func):
    try:
        return py.code.getrawcode(func).co_flags & 32
    except AttributeError:
        return False
...
class PyCollector(PyobjMixin, pytest.Collector):
    ...
    def _matches_prefix_or_glob_option(self, option_name, name):
        ...
        for option in self.config.getini(option_name):
            if name.startswith(option):
                return True
            if '*' in option or '?' in option or '[' in option:
                if fnmatch.fnmatch(name, option):
                    return True
    ...
    def _genfunctions(self, name, funcobj):
        ...
        metafunc = Metafunc(funcobj, fixtureinfo, (self.config), cls=cls,
          module=module)
        try:
            methods = [
             module.pytest_generate_tests]
        except AttributeError:
            methods = []
        ...
        if hasattr(cls, 'pytest_generate_tests'):
            ...
            if not metafunc._calls:
                ...
                for callspec in metafunc._calls:
                    ...
                    yield Function(name=subname, parent=self, callspec=callspec,
                      callobj=funcobj,
...
def add_funcarg_pseudo_fixture_def(collector, metafunc, fixturemanager):
    ...
    for argname, valuelist in arg2params.items():
        ...
        if node and argname in node._name2pseudofixturedef:
            arg2fixturedefs[argname] = [
             node._name2pseudofixturedef[argname]]
        else:
            fixturedef = FixtureDef(fixturemanager, '', argname, get_direct_param_fixture_func, arg2scope[argname], valuelist, False, False)
...
class Module(pytest.File, PyCollector):
    __doc__ = ' Collector for test classes and functions. '
    ...
    def _importtestmodule(self):
        try:
            ...
            raise self.CollectError(py.code.ExceptionInfo().getrepr(style='short'))
        except self.fspath.ImportMismatchError:
            e = sys.exc_info()[1]
            raise self.CollectError('import file mismatch:\nimported module %r has this __file__ attribute:\n  %s\nwhich is not the same as the test file we want to collect:\n  %s\nHINT: remove __pycache__ / .pyc files and/or use a unique basename for your test file modules' % e.args)
...
class Class(PyCollector):
    __doc__ = ' Collector for test methods. '

    def collect(self):
        if hasinit(self.obj):
            self.warn('C1', 'cannot collect test class %r because it has a __init__ constructor' % self.obj.__name__)
            return []
        else:
            return [
             self._getcustomclass('Instance')(name='()', parent=self)]
...
class FunctionMixin(PyobjMixin):
    __doc__ = ' mixin for the code common to Function and Generator.\n    '
...
class CallSpec2(object):
    ...
    def setmulti(self, valtype, argnames, valset, id, keywords, scopenum, param_index):
...
class FuncargnamesCompatAttr:
    __doc__ = ' helper class so that Metafunc, Function and FixtureRequest\n    don\'t need to each define the "funcargnames" compatibility attribute.\n    '
    ...
    def parametrize(self, argnames, argvalues, indirect=False, ids=None, scope=None):
        ...
        for callspec in self._calls or [CallSpec2(self)]:
            for param_index, valset in enumerate(argvalues):
                ...
                newcallspec.setmulti(valtype, argnames, valset, ids[param_index], newkeywords.get(param_index, {}), scopenum, param_index)
...
def _idvalset(idx, valset, argnames, idfn):
    this_id = [_idval(val, argname, idx, idfn) for val, argname in zip(valset, argnames)]
    return '-'.join(this_id)


def idmaker(argnames, argvalues, idfn=None):
    ids = [_idvalset(valindex, valset, argnames, idfn) for valindex, valset in enumerate(argvalues)]
...
def raises(ExpectedException, *args, **kwargs):
    ...
    if isinstance(ExpectedException, tuple):
        ...
        if not inspect.isclass(ExpectedException):
...
class Function(FunctionMixin, pytest.Item, FuncargnamesCompatAttr):
    __doc__ = ' a Function Item is responsible for setting up and executing a\n    Python test function.\n    '
    _genid = None

    def __init__(self, name, parent, args=None, config=None, callspec=None, callobj=NOTSET, keywords=None, session=None, fixtureinfo=None):
        ...
        if fixtureinfo is None:
            fixtureinfo = self.session._fixturemanager.getfixtureinfo((self.parent),
              (self.obj), (self.cls), funcargs=(not self._isyieldedfunction()))
    ...
    def _initrequest(self):
        ...
        if self._isyieldedfunction():
            assert not hasattr(self, 'callspec'), 'yielded functions (deprecated) cannot have funcargs'
        else:
...
class FixtureRequest(FuncargnamesCompatAttr):
    __doc__ = ' A request for a fixture from a test or fixture function.\n\n    A request object gives access to the requesting test context\n    and has an optional ``param`` attribute in case\n    the fixture is parametrized indirectly.\n    '
    ...
    def _getnextfixturedef(self, argname):
        ...
        if fixturedefs is None:
            fixturedefs = self._fixturemanager.getfixturedefs(argname, self._pyfuncitem.parent.nodeid)
    ...
    def _addfinalizer(self, finalizer, scope):
        ...
        self._pyfuncitem.session._setupstate.addfinalizer(finalizer=finalizer,
          colitem=colitem)
    ...
    def cached_setup(self, setup, teardown=None, scope='module', extrakey=None):
        ...
        cachekey = (
         self.fixturename, self._getscopeitem(scope), extrakey)
    ...
    def _getfuncargvalue(self, fixturedef):
        ...
        try:
            ...
            try:
                ...
                self.session._setupstate.addfinalizer(fixturedef.finish, subrequest.node)
...
class SubRequest(FixtureRequest):
    __doc__ = ' a sub request for handling getting a fixture from a\n    test function/fixture. '
...
class ScopeMismatchError(Exception):
    __doc__ = ' A fixture function tries to use a different fixture function which\n    which has a lower scope (e.g. a Session one calls a function one)\n    '
...
class FixtureLookupError(LookupError):
    __doc__ = ' could not return a requested Fixture (missing or invalid). '
...
class FixtureManager:
    ...
    def getfixtureclosure(self, fixturenames, parentnode):
        ...
        return (
         fixturenames_closure, arg2fixturedefs)
        ...
        for name in dir(holderobj):
            ...
            if not callable(obj):
                ...
                if marker is None:
                    ...
                    fixturedef = FixtureDef(self, nodeid, name, obj, (marker.scope),
                      (marker.params), yieldctx=(marker.yieldctx),
                      unittest=unittest,
                      ids=(marker.ids))
...
def call_fixture_func(fixturefunc, request, kwargs, yieldctx):
    if yieldctx:
        if not is_generator(fixturefunc):
            fail_fixturefunc(fixturefunc, msg='yield_fixture requires yield statement in function')
        ...
        def teardown():
            try:
                ...
                fail_fixturefunc(fixturefunc, "yield_fixture function has more than one 'yield'")

        request.addfinalizer(teardown)
    else:
        if is_generator(fixturefunc):
            fail_fixturefunc(fixturefunc, msg='pytest.fixture functions cannot use ``yield``. Instead write and return an inner function/generator and let the consumer call and iterate over it.')
        res = fixturefunc(**kwargs)
    return res


class FixtureDef:
    __doc__ = ' A container for a factory definition. '

    def __init__(self, fixturemanager, baseid, argname, func, scope, params, yieldctx, unittest=False, ids=None):
    ...
    def execute(self, request):
        ...
        if cached_result is not None:
            ...
            if my_cache_key == cache_key:
                if err is not None:
                    ...
                    return result
            self.finish()
            if not not hasattr(self, 'cached_result'):
                raise AssertionError
        if self.unittest:
        ...
        try:
            result = call_fixture_func(fixturefunc, request, kwargs, self.yieldctx)
        except Exception:
            self.cached_result = (
...
def num_mock_patch_args(function):
    ...
    if not patchings:
        ...
        if mock is not None:
            return len([p for p in patchings if not p.attribute_name if p.new is mock.DEFAULT])
...
def getfuncargnames(function, startindex=None):
    ...
    defaults = getattr(function, 'func_defaults', getattr(function, '__defaults__', None)) or ()
...
def reorder_items_atscope(items, ignore, argkeys_cache, scopenum):
    ...
    items_done = []
    while True:
        items_before, items_same, items_other, newignore = slice_items(items, ignore, argkeys_cache[scopenum])
        items_before = reorder_items_atscope(items_before, ignore, argkeys_cache, scopenum + 1)
        ...
        for i, item in enumerate(it):
            ...
            if argkeys is not None:
                ...
                if argkeys:
                    ...
                    for item in it:
                        argkeys = scoped_argkeys_cache.get(item)
                        if argkeys and slicing_argkey in argkeys and slicing_argkey not in ignore:
        ...
        for argname, param_index in cs.indices.items():
            if cs._arg2scopenum[argname] != scopenum:
                ...
                if scopenum == 0:
                    ...
                    if scopenum == 1:
                        key = (
                         argname, param_index, item.fspath)
                    else:
                        if scopenum == 2:
                            key = (
                             argname, param_index, item.fspath, item.cls)
                yield key
