...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/robotframework.py
...
class RowTokenizer(object):
    ...
    def __init__(self):
        ...
        self._tables = {'settings':settings,  'setting':settings,  'metadata':settings, 
         'variables':variables, 
         'variable':variables,  'testcases':testcases, 
         'testcase':testcases,  'keywords':keywords, 
         'keyword':keywords,  'userkeywords':keywords, 
...
class Setting(Tokenizer):
    _tokens = (
     SETTING, ARGUMENT)
    _keyword_settings = ('suitesetup', 'suiteprecondition', 'suiteteardown', 'suitepostcondition',
                         'testsetup', 'testprecondition', 'testteardown', 'testpostcondition',
                         'testtemplate')
    ...
    def _tokenize(self, value, index):
        ...
        if index == 0:
            ...
            if normalized in self._keyword_settings:
                ...
                if normalized in self._import_settings:
                    self._custom_tokenizer = ImportSetting()
                else:
                    if normalized not in self._other_settings:
                        return ERROR
        else:
...
class SettingTable(_Table):
    ...
    def _tokenize(self, value, index):
        if index == 0:
            if normalize(value) == 'testtemplate':
...
class VariableSplitter:
    ...
    def _split(self, string):
        ...
        for index, char in enumerate(string[start_index:]):
            ...
            if index == max_index:
                if not self._scanning_list_variable_index():
                    return
