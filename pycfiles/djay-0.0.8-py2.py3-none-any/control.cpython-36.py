...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/coverage/coverage/control.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 29646 bytes
...
class coverage(object):
    __doc__ = "Programmatic access to coverage.py.\n\n    To use::\n\n        from coverage import coverage\n\n        cov = coverage()\n        cov.start()\n        #.. call your code ..\n        cov.stop()\n        cov.html_report(directory='covhtml')\n\n    "
    ...
    def __init__(self, data_file=None, data_suffix=None, cover_pylib=None, auto_data=False, timid=None, branch=None, config_file=True, source=None, omit=None, include=None, debug=None, debug_file=None):
        ...
        if config_file:
            ...
            self.collector = Collector((self._should_trace),
              timid=(self.config.timid), branch=(self.config.branch),
              warn=(self._warn))
            ...
            if filename.startswith('<'):
                ...
                if self.source_match:
                    ...
                    if self.include_match:
                        ...
                        if self.pylib_match:
                            if self.pylib_match.match(canonical):
                                return (None, 'is in the stdlib')
                if self.cover_match:
                    if self.cover_match.match(canonical):
                        return (None, 'is part of coverage.py')
                if self.omit_match:
                    if self.omit_match.match(canonical):
                        return (None, 'is inside an --omit pattern')
            return (
             canonical, 'because we love you')
    ...
    def _harvest_data(self):
        ...
        if not self._measured:
            ...
            if not summary:
                if self._warn_no_data:
                    self._warn('No data was collected.')
        for src in self.source:
            for py_file in find_python_files(src):
                py_file = self.file_locator.canonical_filename(py_file)
                if self.omit_match:
                    if self.omit_match.match(py_file):
                        continue
    ...
    def sysinfo(self):
        ...
        import coverage as covmod, platform, re
        try:
        ...
        info = [
         ...
         (
          'environment',
          sorted(['%s = %s' % (k, v) for k, v in iitems(os.environ) if re.search('^COV|^PY', k)])),
         (
