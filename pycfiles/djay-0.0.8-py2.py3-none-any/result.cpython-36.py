...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/result.py
...
class ResultLog:
    ...
    def set_header(self, installpkg):
        ...
        """
        self.dict['installpkg'] = dict(md5=(installpkg.computehash('md5')),
...
class EnvLog:
    ...
    def set_python_info(self, pythonexecutable):
        ...
        out = pythonexecutable.sysexec('-c', 'import sys; print (sys.executable);print (list(sys.version_info)); print (sys.version)')
        lines = out.splitlines()
        executable = lines.pop(0)
        version_info = eval(lines.pop(0))
        version = '\n'.join(lines)
        self.dict['python'] = dict(executable=executable,
