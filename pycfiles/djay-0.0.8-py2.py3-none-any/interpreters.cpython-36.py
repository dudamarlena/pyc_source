...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/tox/tox/interpreters.py
# Compiled at: 2019-07-30 18:47:10
# Size of source mod 2**32: 5711 bytes
import sys, py, re, inspect
...
def run_and_get_interpreter_info(name, executable):
    ...
    try:
        ...
        val = sys.exc_info()[1]
        return NoInterpreterInfo(name, executable=(val.executable), out=(val.out),
          err=(val.err))
    else:
...
if sys.platform != 'win32':
    ...
    def tox_get_python_executable(envconfig):
        ...
        if p:
            return p
        actual = None
        m = re.match('python(\\d)\\.(\\d)', name)
        if m:
            actual = 'c:\\python%s%s\\python.exe' % m.groups()
        if not actual:
    ...
    win32map = {'python':sys.executable, 
...
def pyinfo():
    import sys
    return dict(version_info=(tuple(sys.version_info)), sysplatform=(sys.platform))
