...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/pycodestyle/pycodestyle.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 100217 bytes
"""
...
import inspect, keyword, os, re, sys, time, tokenize, warnings, bisect
try:
...
MAX_LINE_LENGTH = 79
BLANK_LINES_CONFIG = {'top_level':2, 
 'method':1}
MAX_DOC_LENGTH = 72
REPORT_FORMAT = {'default':'%(path)s:%(row)d:%(col)d: %(code)s %(text)s', 
...
STARTSWITH_INDENT_STATEMENT_REGEX = re.compile('^\\s*({0})\\b'.format('|'.join(s.replace(' ', '\\s+') for s in ('def',
                                                                                                                'async def',
                                                                                                                'for',
                                                                                                                'async for',
                                                                                                                'if',
...
def register_check(check, codes=None):
    ...
    if inspect.isfunction(check):
        ...
        if args:
            if args[0] in ('physical_line', 'logical_line'):
                if codes is None:
...
def tabs_or_spaces(physical_line, indent_char):
    ...
    for offset, char in enumerate(indent):
        if char != indent_char:
            return (
             offset, 'E101 indentation contains mixed spaces and tabs')
...
def tabs_obsolete(physical_line):
    ...
    if '\t' in indent:
        return (
         indent.index('\t'), 'W191 indentation contains tabs')
        ...
        if not noqa:
            if line_number == 1:
                if line.startswith('#!'):
                    return
                chunks = line.split()
                if len(chunks) == 1 and multiline or len(chunks) == 2 and chunks[0] == '#':
                    if len(line) - len(chunks[(-1)]) < max_line_length - 7:
                        return
            else:
...
def blank_lines(logical_line, blank_lines, indent_level, line_number, blank_before, previous_logical, previous_unindented_logical_line, previous_indent_level, lines):
    ...
    if line_number < top_level_lines + 1:
    ...
    if previous_logical.startswith('@'):
        ...
        if blank_lines > top_level_lines or indent_level and blank_lines == method_lines + 1:
            yield (0, 'E303 too many blank lines (%d)' % blank_lines)
        else:
            if STARTSWITH_TOP_LEVEL_REGEX.match(logical_line):
                prev_line = lines[(line_number - 2)] if line_number >= 2 else ''
                next_line = lines[line_number] if line_number < len(lines) else ''
                if expand_indent(prev_line) <= indent_level:
                    if expand_indent(next_line) <= indent_level:
                        return
                if indent_level:
                    ...
                    if blank_before != top_level_lines:
                        yield (
                         0,
                ...
                if not indent_level:
                    if blank_before != top_level_lines:
                        if previous_unindented_logical_line.startswith(('def ', 'class ')):
                            yield (0,
...
def whitespace_around_keywords(logical_line):
    ...
    for match in KEYWORD_REGEX.finditer(logical_line):
        ...
        if '\t' in before:
            yield (
             match.start(1), 'E274 tab before keyword')
        else:
            if len(before) > 1:
                yield (
                 match.start(1), 'E272 multiple spaces before keyword')
        if '\t' in after:
            yield (
             match.start(2), 'E273 tab after keyword')
        else:
            if len(after) > 1:
                yield (
                 match.start(2), 'E271 multiple spaces after keyword')
...
def missing_whitespace(logical_line):
    ...
    for index in range(len(line) - 1):
        char = line[index]
        if char in ',;:':
            if line[(index + 1)] not in WHITESPACE:
                before = line[:index]
                if char == ':':
                ...
                if char == ',':
                    if line[(index + 1)] == ')':
                        continue
            yield (
             index, "E231 missing whitespace after '%s'" % char)
...
def indentation(logical_line, previous_logical, indent_char, indent_level, previous_indent_level):
    ...
    if indent_level % 4:
        yield (
         0, tmpl % (1 + c, 'indentation is not a multiple of four'))
    indent_expect = previous_logical.endswith(':')
    if indent_expect:
        if indent_level <= previous_indent_level:
            yield (
             0, tmpl % (2 + c, 'expected an indented block'))
    if not indent_expect:
        if indent_level > previous_indent_level:
            yield (
             0, tmpl % (3 + c, 'unexpected indentation'))
    expected_indent_level = previous_indent_level + 4
    if indent_expect:
        if indent_level > expected_indent_level:
            yield (
             0, tmpl % (7, 'over-indented'))
    ...
    if noqa or nrows == 1:
        ...
        for token_type, text, start, end, line in tokens:
                    ...
                    if close_bracket:
                        if indent[depth]:
                            if start[1] != indent[depth]:
                                yield (
                                 start, 'E124 closing bracket does not match visual indentation')
                        elif close_bracket and not hang:
                            if hang_closing:
                                yield (
                                 start, 'E133 closing bracket is missing indentation')
                        elif indent[depth] and start[1] < indent[depth]:
                            if visual_indent is not True:
                                yield (start, 'E128 continuation line under-indented for visual indent')
                        else:
                            if hanging_indent or indent_next and rel_indent[row] == 8:
                                if close_bracket:
                                    if not hang_closing:
                                        yield (
                                         start, "E123 closing bracket does not match indentation of opening bracket's line")
                                hangs[depth] = hang
                            else:
                                if visual_indent is True:
                                    indent[depth] = start[1]
                                else:
                                    if visual_indent in (text, str):
                                        pass
                                    else:
                                        if hang <= 0:
                                            ...
                                            if indent[depth]:
                                                ...
                                                if not close_bracket and hangs[depth]:
                        ...
                        yield (
                         start, '%s continuation line %s' % error)
            if parens[row] and token_type not in (tokenize.NL, tokenize.COMMENT) and not indent[depth]:
                ...
                if token_type in (tokenize.STRING, tokenize.COMMENT) or text in ('u',
                                                                                 'ur',
                                                                                 'b',
                                                                                 'br'):
                    indent_chances[start[1]] = str
                else:
                    if not indent_chances:
                        if not row:
                            if not depth:
                                if text == 'if':
                                    indent_chances[end[1] + 1] = True
            if text == ':':
                if line[end[1]:].isspace():
        ...
        if indent_next:
            if expand_indent(line) == indent_level + 4:
                pos = (
                 start[0], indent[0] + 4)
                if visual_indent:
                ...
                yield (
                 pos, '%s with same indent as next logical line' % code)
...
def whitespace_before_parameters(logical_line, tokens):
    ...
    for index in range(1, len(tokens)):
        ...
        if token_type == tokenize.OP:
            if text in '([':
                if start != prev_end:
                    if prev_type == tokenize.NAME or prev_text in '}])':
                        if index < 2 or tokens[(index - 2)][1] != 'class':
                            if not keyword.iskeyword(prev_text):
                                yield (
                                 prev_end, "E211 whitespace before '%s'" % text)
...
def whitespace_around_operator(logical_line):
    ...
    for match in OPERATOR_REGEX.finditer(logical_line):
        ...
        if '\t' in before:
            yield (
             match.start(1), 'E223 tab before operator')
        else:
            if len(before) > 1:
                yield (
                 match.start(1), 'E221 multiple spaces before operator')
        if '\t' in after:
            yield (
             match.start(2), 'E224 tab after operator')
        else:
            if len(after) > 1:
                yield (
                 match.start(2), 'E222 multiple spaces after operator')
                ...
                if need_space is True or need_space[1]:
                    ...
                    if prev_text != '**':
                        ...
                        if prev_text == '%':
                            ...
                            if prev_text not in ARITHMETIC_OP:
                                code, optype = ('E227', 'bitwise or shift')
                            yield (
                             need_space[0],
                             '%s missing whitespace around %s operator' % (
                              code, optype))
                need_space = False
        else:
            if token_type == tokenize.OP:
                if prev_end is not None:
                    if text == '=' and parens:
                        ...
                        if text in WS_NEEDED_OPERATORS:
                            need_space = True
                        else:
                            if text in UNARY_OPERATORS:
                                ...
                                if need_space is None:
                                    ...
                                    if start == prev_end:
                                        yield (
                                         prev_end, 'E225 missing whitespace around operator')
                                        need_space = False
            prev_type = token_type
            prev_text = text
            prev_end = end
...
def whitespace_around_comma(logical_line):
    ...
    for m in WHITESPACE_AFTER_COMMA_REGEX.finditer(line):
        ...
        if '\t' in m.group():
            yield (
             found, "E242 tab after '%s'" % m.group()[0])
        else:
            yield (
             found, "E241 multiple spaces after '%s'" % m.group()[0])
    ...
    for token_type, text, start, end, line in tokens:
        if token_type == tokenize.COMMENT:
            ...
            if inline_comment:
                ...
                if bad_prefix != '!' or start[0] > 1:
                    if bad_prefix != '#':
                        yield (
                         start, "E265 block comment should start with '# '")
                    elif comment:
                        yield (
                         start, "E266 too many leading '#' for block comment")
        else:
            if token_type != tokenize.NL:
                prev_end = end
...
def imports_on_separate_lines(logical_line):
    ...
    if line.startswith('import '):
        found = line.find(',')
        if -1 < found:
            if ';' not in line[:found]:
                yield (
                 found, 'E401 multiple imports on one line')
...
def compound_statements(logical_line):
    ...
    while -1 < found < last_char:
        ...
        if counts['{'] <= counts['}'] and counts['['] <= counts[']'] and counts['('] <= counts[')']:
            ...
            if lambda_kw:
                ...
                if before[-1:] == '=':
                    if isidentifier(before[:-1].strip()):
            ...
            if STARTSWITH_DEF_REGEX.match(line):
                ...
                yield (
                 found, 'E701 multiple statements on one line (colon)')
        prev_found = found
    ...
    while -1 < found:
        if found < last_char:
            yield (
             found, 'E702 multiple statements on one line (semicolon)')
        else:
            yield (
             found, 'E703 statement ends with a semicolon')
        found = line.find(';', found + 1)
        ...
        if start[0] != prev_start:
            if parens:
                if backslash:
                    if not comment:
                        yield (
                         backslash, 'E502 the backslash is redundant between brackets')
            else:
                if end[0] != prev_end:
                    if line.rstrip('\r\n').endswith('\\'):
                        backslash = (
                         end[0], len(line.splitlines()[(-1)]) - 1)
                    else:
                        backslash = None
...
def break_before_binary_operator(logical_line, tokens):
    ...
    for context in _break_around_binary_operators(tokens):
        ...
        if _is_binary_operator(token_type, text) and line_break and not unary_context and not _is_binary_operator(previous_token_type, previous_text):
            yield (
             start, 'W503 line break before binary operator')
...
def break_after_binary_operator(logical_line, tokens):
    ...
    for context in _break_around_binary_operators(tokens):
        ...
        if _is_binary_operator(previous_token_type, previous_text):
            if line_break:
                if not unary_context:
                    if not _is_binary_operator(token_type, text):
                        yield (
                         prev_start, 'W504 line break after binary operator')
        prev_start = start
...
def comparison_negative(logical_line):
    ...
    if match:
        ...
        if match.group(2) == 'in':
            yield (
             pos, "E713 test for membership should be 'not in'")
        else:
            yield (
             pos, "E714 test for object identity should be 'is not'")
...
def comparison_type(logical_line, noqa):
    ...
    if match:
        if not noqa:
            inst = match.group(1)
            if inst:
                if isidentifier(inst):
                    if inst not in SINGLETONS:
                        return
            yield (
             match.start(), "E721 do not compare types, use 'isinstance()'")
...
def bare_except(logical_line, noqa):
    ...
    if noqa:
        return
    regex = re.compile('except\\s*:')
    match = regex.match(logical_line)
    if match:
        yield (
         match.start(), "E722 do not use bare 'except'")
...
def ambiguous_identifier(logical_line, tokens):
    ...
    for token_type, text, start, end, line in tokens[1:]:
        ...
        if prev_text == 'class':
            if text in idents_to_avoid:
                yield (
                 start, "E742 ambiguous class definition '%s'" % text)
        if prev_text == 'def':
            if text in idents_to_avoid:
                yield (
                 start, "E743 ambiguous function definition '%s'" % text)
        if ident:
            yield (
             pos, "E741 ambiguous variable name '%s'" % ident)
        prev_text = text
        prev_start = start
...
def python_3000_has_key(logical_line, noqa):
    ...
    if pos > -1:
        if not noqa:
            yield (
             pos, "W601 .has_key() is deprecated, use 'in'")
...
def python_3000_raise_comma(logical_line):
    ...
    if match:
        if not RERAISE_COMMA_REGEX.match(logical_line):
            yield (
             match.end() - 1, 'W602 deprecated form of raising exception')
...
def python_3000_backticks(logical_line):
    ...
    if pos > -1:
        yield (
         pos, "W604 backticks are deprecated, use 'repr()'")
        ...
        if token_type in (tokenize.STRING, tokenize.COMMENT):
            if prev_token is None or prev_token in SKIP_TOKENS:
                ...
                for line_num, physical_line in enumerate(lines):
                    if hasattr(physical_line, 'decode'):
                        try:
                        ...
                        if token_type == tokenize.COMMENT:
                            if len(chunks) == 2:
                                if length - len(chunks[(-1)]) < MAX_DOC_LENGTH:
                                    continue
                        if len(chunks) == 1:
                        ...
                        if length > max_doc_length:
                            doc_error = (
                             start[0] + line_num, max_doc_length)
                            yield (doc_error,
...
noqa = lru_cache(512)(re.compile('# no(?:qa|pep8)\\b', re.I).search)
...
def parse_udiff(diff, patterns=None, parent='.'):
    ...
    return {os.path.join(parent, filepath):rows for filepath, rows in rv.items() if filename_match(filepath, patterns)}
...
class Checker(object):
    __doc__ = 'Load a Python source file, tokenize it, check coding style.'
...
class BaseReport(object):
    __doc__ = 'Collect the results of the checks.'
...
class FileReport(BaseReport):
    __doc__ = 'Collect the results of the checks and print the filenames.'
    print_filename = True


class StandardReport(BaseReport):
    __doc__ = 'Collect and print the results of the checks.'
    ...
    def error(self, line_number, offset, text, check):
        ...
        if code:
            if self.counters[code] == 1 or self._repeat:
    ...
    def get_file_results(self):
        ...
        for line_number, offset, code, text, doc in self._deferred_print:
            print(self._fmt % {'path':self.filename, 
...
class DiffReport(StandardReport):
    __doc__ = 'Collect and print the results for the changed lines only.'
...
class StyleGuide(object):
    __doc__ = 'Initialize a PEP-8 instance with few options.'
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not options.reporter:
            ...
            if not (options.select or options.ignore or options.testsuite or options.doctest):
                if DEFAULT_IGNORE:
    ...
    def ignore_code(self, code):
        ...
        """
        if len(code) < 4:
            if any(s.startswith(code) for s in self.options.select):
                return False
...
def get_parser(prog='pycodestyle', version=__version__):
    ...
    parser.add_option('--max-line-length', type='int', metavar='n', default=MAX_LINE_LENGTH,
      help='set maximum allowed line length (default: %default)')
    parser.add_option('--max-doc-length', type='int', metavar='n', default=None,
      help='set maximum allowed doc line length and perform these checks (unchecked if not set)')
        ...
        for opt in config.options(pycodestyle_section):
            if opt.replace('_', '-') not in parser.config_options:
                ...
                if opt_type in ('int', 'count'):
                    ...
                    if opt_type in ('store_true', 'store_false'):
...
def process_options(arglist=None, parse_argv=False, config_file=None, parser=None, verbose=None):
    ...
    if not parser:
        ...
        if not parser.has_option('--config'):
            group = parser.add_option_group('Configuration', description=('The project options are read from the [%s] section of the tox.ini file or the setup.cfg file located in any parent folder of the path(s) being processed.  Allowed options are: %s.' % (
