...
def pytest_addoption(parser):
    ...
    group.addoption('--no-cov-on-fail', action='store_true', default=False, dest='no_cov_on_fail',
...
class CovPlugin(object):
    ...
    def start(self, controller_cls, config=None, nodeid=None):
        if config is None:
            ...
            config = Config()
        self.cov_controller = controller_cls(self.options.cov_source, self.options.cov_report or ['term'], self.options.cov_config, config, nodeid)
