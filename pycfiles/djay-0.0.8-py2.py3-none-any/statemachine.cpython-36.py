...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/statemachine.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 57489 bytes
"""
...
class StringList(ViewList):
    ...
    def get_indented(self, start=0, until_blank=False, strip_indent=True, block_indent=None, first_indent=None):
        ...
        while end < last:
            line = self.data[end]
            if line:
                if line[0] != ' ' or block_indent is not None and line[:block_indent].strip():
                    blank_finish = end > start and not self.data[(end - 1)].strip()
                    break
            stripped = line.lstrip()
            if stripped or until_blank:
                blank_finish = 1
                break
            else:
