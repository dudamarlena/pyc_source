...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/urllib3/urllib3/packages/rfc3986/builder.py
...
class URIBuilder(object):
    ...
    def __repr__(self):
        ...
        formatstr = 'URIBuilder(scheme={b.scheme}, userinfo={b.userinfo}, host={b.host}, port={b.port}, path={b.path}, query={b.query}, fragment={b.fragment})'
    ...
    def add_credentials(self, username, password):
        ...
        if password is not None:
            userinfo = '{}:{}'.format(userinfo, normalizers.normalize_password(password))
        return URIBuilder(scheme=(self.scheme),
          userinfo=userinfo,
          host=(self.host),
          port=(self.port),
          path=(self.path),
    ...
    def add_port(self, port):
        ...
        port_int = int(port)
        if port_int < 0:
            raise ValueError('ports are not allowed to be negative. You provided {}'.format(port_int))
        if port_int > 65535:
            raise ValueError('ports are not allowed to be larger than 65535. You provided {}'.format(port_int))
        return URIBuilder(scheme=(self.scheme),
          userinfo=(self.userinfo),
          host=(self.host),
    ...
    def finalize(self):
        ...
        """
        return uri.URIReference(self.scheme, normalizers.normalize_authority((
         self.userinfo, self.host, self.port)), self.path, self.query, self.fragment)