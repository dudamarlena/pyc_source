...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/Pygments/pygments/lexers/business.py
# Compiled at: 2019-07-30 18:47:12
# Size of source mod 2**32: 27665 bytes
"""
...
class CobolLexer(RegexLexer):
    ...
    tokens = {'root':[
     ...
     'core':[
      (
       '(^|(?<=[^\\w\\-]))(ALL\\s+)?((ZEROES)|(HIGH-VALUE|LOW-VALUE|QUOTE|SPACE|ZERO)(S)?)\\s*($|(?=[^\\w\\-]))',
       Name.Constant),
      (
       ...
       'UNLOCK', 'UNSTRING', 'USE', 'VALIDATE', 'WORKING-STORAGE', 'WRITE'),
         prefix='(^|(?<=[^\\w\\-]))', suffix='\\s*($|(?=[^\\w\\-]))'),
       Keyword.Reserved),
      (
       ...
       'VALUES', 'VARYING', 'WAIT', 'WHEN', 'WITH', 'WORDS', 'YYYYDDD', 'YYYYMMDD'),
         prefix='(^|(?<=[^\\w\\-]))',
         suffix='\\s*($|(?=[^\\w\\-]))'),
       Keyword.Pseudo),
      (
       ...
       Error),
      (
       '(^|(?<=[^\\w\\-]))(PIC\\s+.+?(?=(\\s|\\.\\s))|PICTURE\\s+.+?(?=(\\s|\\.\\s))|(COMPUTATIONAL)(-[1-5X])?|(COMP)(-[1-5X])?|BINARY-C-LONG|BINARY-CHAR|BINARY-DOUBLE|BINARY-LONG|BINARY-SHORT|BINARY)\\s*($|(?=[^\\w\\-]))',
       Keyword.Type),
      (
       '(\\*\\*|\\*|\\+|-|/|<=|>=|<|>|==|/=|=)', Operator),
      (
      ...
      (
       ...
       Name.Function),
      (
       '(^|(?<=[^\\w\\-]))(true|false)\\s*($|(?=[^\\w\\-]))', Name.Builtin),
      (
       '(^|(?<=[^\\w\\-]))(equal|equals|ne|lt|le|gt|ge|greater|less|than|not|and|or)\\s*($|(?=[^\\w\\-]))',
       Operator.Word)], 
     'strings':[
      (
       '"[^"\\n]*("|\\n)', String.Double),
...
class ABAPLexer(RegexLexer):
    ...
    tokens = {'common':[
     ...
     'root':[
      include('common'),
      (
       'CALL\\s+(?:BADI|CUSTOMER-FUNCTION|FUNCTION)',
       Keyword),
      (
      ...
      (
       '[0-9]+', Number.Integer),
      (
       '(?<=(\\s|.))(AND|OR|EQ|NE|GT|LT|GE|LE|CO|CN|CA|NA|CS|NOT|NS|CP|NP|BYTE-CO|BYTE-CN|BYTE-CA|BYTE-NA|BYTE-CS|BYTE-NS|IS\\s+(NOT\\s+)?(INITIAL|ASSIGNED|REQUESTED|BOUND))\\b',
       Operator.Word),
...
class OpenEdgeLexer(RegexLexer):
    ...
    keywords = words(OPENEDGEKEYWORDS, prefix='(?i)(^|(?<=[^\\w\\-]))',
      suffix='\\s*($|(?=[^\\w\\-]))')
    tokens = {'root':[
      (
...
class MaqlLexer(RegexLexer):
    ...
    tokens = {'root':[
      ...
      (
       ...
       'ORDER', 'ASC', 'DESC', 'HYPERLINK', 'INCLUDE', 'TEMPLATE', 'MODIFY'),
         suffix='\\b'),
       Keyword),
      (
