...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/py/py/_log/log.py
...
class Producer(object):
    __doc__ = " (deprecated) Log producer API which sends messages to be logged\n        to a 'consumer' object, which then prints them to stdout,\n        stderr, files, etc. Used extensively by PyPy-1.1.\n    "
    Message = Message
    keywords2consumer = {}
...
class KeywordMapper:
    ...
    def setconsumer(self, keywords, consumer):
        ...
        if isinstance(keywords, str):
            ...
            if hasattr(keywords, '_keywords'):
                keywords = keywords._keywords
            else:
                if not isinstance(keywords, tuple):
                    raise TypeError('key %r is not a string or tuple' % (keywords,))
        if consumer is not None:
            if not py.builtin.callable(consumer):
...
class File(object):
    __doc__ = ' log consumer wrapping a file(-like) object '
...
class Path(object):
    __doc__ = ' log consumer that opens and writes to a Path '

    def __init__(self, filename, append=False, delayed_create=False, buffering=False):
        self._append = append
...
class Syslog:
    __doc__ = ' consumer that writes to the syslog daemon '
