...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/jinja2/jinja2/runtime.py
# Compiled at: 2019-07-30 18:47:05
# Size of source mod 2**32: 27755 bytes
"""
    jinja2.runtime
    ~~~~~~~~~~~~~~
...
from jinja2.utils import Markup, soft_unicode, escape, missing, concat, internalcode, object_type_repr, evalcontextfunction, Namespace
...
class ContextMeta(type):
    ...
    def __new__(cls, name, bases, d):
        ...
        if bases == ():
            ...
            if resolve is not default_resolve:
                if resolve_or_missing is default_resolve_or_missing:
                    rv._legacy_resolve_mode = True
            if resolve is default_resolve:
                if resolve_or_missing is default_resolve_or_missing:
...
class Context(with_metaclass(ContextMeta)):
    ...
    def super(self, name, current):
        ...
        try:
            ...
            return self.environment.undefined(('there is no parent block called %r.' % name),
              name='super')
        else:
    ...
    def call(_Context__self, _Context__obj, *args, **kwargs):
        ...
        if hasattr(_Context__obj, '__call__'):
            fn = _Context__obj.__call__
            for fn_type in ('contextfunction', 'evalcontextfunction', 'environmentfunction'):
                if hasattr(fn, fn_type):
                    _Context__obj = fn
                    break

        if isinstance(_Context__obj, _context_function_types):
            if getattr(_Context__obj, 'contextfunction', 0):
                args = (
                 _Context__self,) + args
            else:
                if getattr(_Context__obj, 'evalcontextfunction', 0):
                    args = (
                     _Context__self.eval_ctx,) + args
                elif getattr(_Context__obj, 'environmentfunction', 0):
                    args = (
                     _Context__self.environment,) + args
        try:
            return _Context__obj(*args, **kwargs)
        except StopIteration:
            return _Context__self.environment.undefined('value was undefined because a callable raised a StopIteration exception')
...
class BlockReference(object):
    __doc__ = 'One block on a template reference.'
    ...
    def super(self):
        ...
        if self._depth + 1 >= len(self._stack):
            return self._context.environment.undefined(('there is no parent block called %r.' % self.name),
              name='super')
        else:
...
class LoopContextBase(object):
    __doc__ = 'A loop context for dynamic iteration.'
...
class LoopContextIterator(object):
    __doc__ = 'The iterator for a loop context.'
    __slots__ = ('context', )
...
class Macro(object):
    __doc__ = 'Wraps a macro function.'
    ...
    def __call__(self, *args, **kwargs):
        if args:
            if isinstance(args[0], EvalContext):
        ...
        if self.catch_kwargs:
            ...
            if kwargs:
                if 'caller' in kwargs:
                    raise TypeError('macro %r was invoked with two values for the special caller argument.  This is most likely a bug.' % self.name)
...
class Undefined(object):
    __doc__ = "The default undefined type.  This undefined type can be printed and\n    iterated over, but every other access will raise an :exc:`jinja2.exceptions.UndefinedError`:\n\n    >>> foo = Undefined(name='foo')\n    >>> str(foo)\n    ''\n    >>> not foo\n    True\n    >>> foo + 42\n    Traceback (most recent call last):\n      ...\n    jinja2.exceptions.UndefinedError: 'foo' is undefined\n    "
    ...
    def _log_message(undef):
        if undef._undefined_hint is None:
            if undef._undefined_obj is missing:
                ...
                if not isinstance(undef._undefined_name, string_types):
...
class DebugUndefined(Undefined):
    __doc__ = "An undefined that returns the debug info when printed.\n\n    >>> foo = DebugUndefined(name='foo')\n    >>> str(foo)\n    '{{ foo }}'\n    >>> not foo\n    True\n    >>> foo + 42\n    Traceback (most recent call last):\n      ...\n    jinja2.exceptions.UndefinedError: 'foo' is undefined\n    "
    __slots__ = ()
...
class StrictUndefined(Undefined):
    __doc__ = "An undefined that barks on print and iteration as well as boolean\n    tests and all kinds of comparisons.  In other words: you can do nothing\n    with it except checking if it's defined using the `defined` test.\n\n    >>> foo = StrictUndefined(name='foo')\n    >>> str(foo)\n    Traceback (most recent call last):\n      ...\n    jinja2.exceptions.UndefinedError: 'foo' is undefined\n    >>> not foo\n    Traceback (most recent call last):\n      ...\n    jinja2.exceptions.UndefinedError: 'foo' is undefined\n    >>> foo + 42\n    Traceback (most recent call last):\n      ...\n    jinja2.exceptions.UndefinedError: 'foo' is undefined\n    "
    __slots__ = ()
