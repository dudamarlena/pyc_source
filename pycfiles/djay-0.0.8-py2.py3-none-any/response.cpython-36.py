...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/urllib3/urllib3/response.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 27171 bytes
from __future__ import absolute_import
from contextlib import contextmanager
import zlib, io, logging
...
from .exceptions import BodyNotHttplibCompatible, ProtocolError, DecodeError, ReadTimeoutError, ResponseNotChunked, IncompleteRead, InvalidHeader
...
class HTTPResponse(io.IOBase):
    ...
    def __init__(self, body='', headers=None, status=0, version=0, reason=None, strict=0, preload_content=True, decode_content=True, original_response=None, pool=None, connection=None, msg=None, retries=None, enforce_content_length=False, request_method=None, request_url=None):
        ...
        if body:
            if isinstance(body, (basestring, bytes)):
                self._body = body
        self._pool = pool
    ...
    def stream(self, amt=65536, decode_content=None):
        ...
        """
        if self.chunked:
            if self.supports_chunked_reads():
    ...
    def read_chunked(self, amt=None, decode_content=None):
        ...
        if not self.supports_chunked_reads():
            raise BodyNotHttplibCompatible('Body should be httplib.HTTPResponse like. It should have have an fp attribute which returns raw chunks.')
        with self._error_catcher():
            if self._original_response:
