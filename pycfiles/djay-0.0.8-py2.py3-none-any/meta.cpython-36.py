...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/jinja2/jinja2/meta.py
...
class TrackingCodeGenerator(CodeGenerator):
    __doc__ = 'We abuse the code generator for introspection.'
...
def find_referenced_templates(ast):
    ...
    for node in ast.find_all((nodes.Extends, nodes.FromImport, nodes.Import,
     nodes.Include)):
        if isinstance(node.template, nodes.Const) or isinstance(node.template, (nodes.Tuple, nodes.List)):
