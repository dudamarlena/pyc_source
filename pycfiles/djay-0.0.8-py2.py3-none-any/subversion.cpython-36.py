...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_internal/vcs/subversion.py
# Compiled at: 2019-07-30 18:46:55
# Size of source mod 2**32: 11697 bytes
from __future__ import absolute_import
import logging, os, re, sys
from pip._internal.utils.logging import indent_log
from pip._internal.utils.misc import display_path, rmtree, split_auth_from_netloc
...
class Subversion(VersionControl):
            ...
            if cls.dirname not in dirs:
                ...
                if not os.path.exists(entries_fn):
                    ...
                    if base == location:
                        ...
                        if not dirurl or not dirurl.startswith(base):
                            dirs[:] = []
                            continue
    ...
    def export(self, location, url):
        ...
        with indent_log():
            if os.path.exists(location):
                rmtree(location)
            cmd_args = ['export'] + self.get_remote_call_options() + rev_options.to_args() + [url, location]
    ...
    def fetch_new(self, dest, url, rev_options):
        ...
        cmd_args = [
         'checkout', '-q'] + self.get_remote_call_options() + rev_options.to_args() + [url, dest]
        self.run_command(cmd_args)

    def switch(self, dest, url, rev_options):
        cmd_args = [
         'switch'] + self.get_remote_call_options() + rev_options.to_args() + [url, dest]
        self.run_command(cmd_args)

    def update(self, dest, url, rev_options):
        cmd_args = [
         'update'] + self.get_remote_call_options() + rev_options.to_args() + [dest]
        self.run_command(cmd_args)
