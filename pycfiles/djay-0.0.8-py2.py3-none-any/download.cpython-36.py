...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_internal/download.py
# Compiled at: 2019-07-30 18:46:55
# Size of source mod 2**32: 42649 bytes
from __future__ import absolute_import
import cgi, email.utils, json, logging, mimetypes, os, platform, re, shutil, sys
...
class MultiDomainBasicAuth(AuthBase):
    ...
    def _get_new_credentials(self, original_url, allow_netrc=True, allow_keyring=True):
        ...
        if username is not None and password is not None:
            ...
            if index_url:
                if index_url_user_password[0] is not None:
            ...
            if allow_netrc:
                ...
                if netrc_auth:
                    ...
                    return netrc_auth
            if allow_keyring:
                kr_auth = _get_keyring_auth(index_url, username) or _get_keyring_auth(netloc, username)
                if kr_auth:
                ...
                if username is not None:
                    if password is not None:
                        self.passwords[parsed.netloc] = (
                         username, password)
                        if save:
                            if self._should_save_password_to_keyring():
                                self._credentials_to_save = (
                                 parsed.netloc, username, password)
                resp.content
    ...
    def save_credentials(self, resp, **kwargs):
        ...
        if not keyring is not None:
            raise AssertionError('should never reach here without keyring')
        else:
            if not keyring:
                return
    ...
    def __init__(self, *args, **kwargs):
        ...
        retries = urllib3.Retry(total=retries,
         ...
         500, 503, 520, 527],
          backoff_factor=0.25)
        if cache:
            secure_adapter = CacheControlAdapter(cache=SafeFileCache(cache, use_dir_lock=True),
              max_retries=retries)
        else:
