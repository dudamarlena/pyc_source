...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/click/click/termui.py
# Compiled at: 2019-07-30 18:47:04
# Size of source mod 2**32: 23207 bytes
import os, sys, struct, inspect, itertools
from ._compat import raw_input, text_type, string_types, isatty, strip_ansi, get_winterm_size, DEFAULT_COLUMNS, WIN
from .utils import echo
from .exceptions import Abort, UsageError
from .types import convert_type, Choice, Path
from .globals import resolve_color_default
visible_prompt_func = raw_input
_ansi_colors = {'black':30, 
 'red':31, 
 'green':32, 
 'yellow':33, 
 'blue':34, 
...
def echo_via_pager(text_or_generator, color=None):
    ...
    if inspect.isgeneratorfunction(text_or_generator):
        ...
        if isinstance(text_or_generator, string_types):
            i = [
             text_or_generator]
        else:
...
def style(text, fg=None, bg=None, bold=None, dim=None, underline=None, blink=None, reverse=None, reset=True):
    ...
    * ``reset`` (reset the color code only)
    ...
    if fg:
        try:
            bits.append('\x1b[%dm' % _ansi_colors[fg])
        except KeyError:
            raise TypeError('Unknown color %r' % fg)

    if bg:
        try:
            bits.append('\x1b[%dm' % (_ansi_colors[bg] + 10))
        except KeyError:
            raise TypeError('Unknown color %r' % bg)

    if bold is not None:
        bits.append('\x1b[%dm' % (1 if bold else 22))
    if dim is not None:
        bits.append('\x1b[%dm' % (2 if dim else 22))
    if underline is not None:
        bits.append('\x1b[%dm' % (4 if underline else 24))
    if blink is not None:
        bits.append('\x1b[%dm' % (5 if blink else 25))
    if reverse is not None:
        bits.append('\x1b[%dm' % (7 if reverse else 27))
    bits.append(text)
    if reset:
