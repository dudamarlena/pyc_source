...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_vendor/msgpack/fallback.py
# Compiled at: 2019-07-30 18:46:56
# Size of source mod 2**32: 37491 bytes
...
from pip._vendor.msgpack.exceptions import BufferFull, OutOfData, ExtraData, FormatError, StackError
...
def _get_data_from_buffer(obj):
    try:
        ...
        if PY2:
            ...
            warnings.warn(('using old buffer interface to unpack %s; this leads to unpacking errors if slicing is used and will be removed in a future version' % type(obj)),
              RuntimeWarning,
              stacklevel=3)
        else:
            raise
...
def unpack(stream, **kwargs):
    warnings.warn("Direct calling implementation's unpack() is deprecated, Use msgpack.unpack() or unpackb() instead.",
      DeprecationWarning,
      stacklevel=2)
    data = stream.read()
...
class Unpacker(object):
    ...
    def __init__(self, file_like=None, read_size=0, use_list=True, raw=True, strict_map_key=False, object_hook=None, object_pairs_hook=None, list_hook=None, encoding=None, unicode_errors=None, max_buffer_size=0, ext_hook=ExtType, max_str_len=-1, max_bin_len=-1, max_array_len=-1, max_map_len=-1, max_ext_len=-1):
        if encoding is not None:
            warnings.warn('encoding is deprecated, Use raw=False instead.',
              DeprecationWarning,
              stacklevel=2)
        else:
            if unicode_errors is None:
                ...
                if max_str_len == -1:
                    max_str_len = max_buffer_size or 1048576
                if max_bin_len == -1:
                    max_bin_len = max_buffer_size or 1048576
                if max_array_len == -1:
                    max_array_len = max_buffer_size or 131072
                if max_map_len == -1:
                    max_map_len = max_buffer_size // 2 or 32768
                if max_ext_len == -1:
                    max_ext_len = max_buffer_size or 1048576
                ...
                if list_hook is not None:
                    if not callable(list_hook):
                        raise TypeError('`list_hook` is not callable')
                if object_hook is not None:
                    if not callable(object_hook):
                        raise TypeError('`object_hook` is not callable')
                if object_pairs_hook is not None:
                    if not callable(object_pairs_hook):
                        raise TypeError('`object_pairs_hook` is not callable')
                if object_hook is not None:
                    if object_pairs_hook is not None:
                        raise TypeError('object_pairs_hook and object_hook are mutually exclusive')
            raise callable(ext_hook) or TypeError('`ext_hook` is not callable')
...
class Packer(object):
    ...
    def __init__(self, default=None, encoding=None, unicode_errors=None, use_single_float=False, autoreset=True, use_bin_type=False, strict_types=False):
        if encoding is None:
            encoding = 'utf_8'
        else:
            warnings.warn('encoding is deprecated, Use raw=False instead.',
                    ...
                    if check(obj, bool):
                        ...
                        if check(obj, int_types):
                            if 0 <= obj < 128:
                                ...
                                elif check(obj, (bytes, bytearray)):
                                    n = len(obj)
                                    if n >= 4294967296:
