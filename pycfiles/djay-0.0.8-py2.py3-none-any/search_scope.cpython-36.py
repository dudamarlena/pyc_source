...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-sin1koo5/pip/pip/_internal/models/search_scope.py
    ...
    def get_formatted_locations(self):
        ...
        if self.index_urls:
            if self.index_urls != [PyPI.simple_url]:
                lines.append('Looking in indexes: {}'.format(', '.join(redact_password_from_url(url) for url in self.index_urls)))
        if self.find_links:
            lines.append('Looking in links: {}'.format(', '.join(redact_password_from_url(url) for url in self.find_links)))
        return '\n'.join(lines)
