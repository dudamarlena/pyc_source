...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/docutils/docutils/utils/math/math2html.py
...
class CommandLineParser(object):
    ...
    def readoption(self, args):
        ...
        if args[0].startswith('"'):
            ...
            del args[0]
            return (
             key, self.readquoted(args, initial))
        else:
...
class FormulaParser(Parser):
    ...
    def parseheader(self, reader):
        ...
        if not type:
            reader.nextline()
            type = self.parsetype(reader)
            type or Trace.error('Unknown formula type in ' + reader.currentline().strip())
            return [
             'unknown']
        else:
            return [
...
class NumberGenerator(object):
    ...
    def getdependentcounter(self, type, master):
        ...
        if type not in self.counters or not self.counters[type].master:
...
class LimitsProcessor(MathsProcessor):
    ...
    def process(self, contents, index):
        ...
        if Options.simplemath:
            ...
            if self.checkscript(contents, index):
...
class HybridFunction(ParameterFunction):
    ...
    def writepos--- This code section failed: ---

 L.4959         0  BUILD_LIST_0          0 
                2  STORE_FAST               'result'

 L.4960         4  SETUP_LOOP          174  'to 174'
                6  LOAD_FAST                'pos'
                8  LOAD_ATTR                finished
               10  CALL_FUNCTION_0       0  '0 positional arguments'
               12  POP_JUMP_IF_TRUE    172  'to 172'

 L.4961        14  LOAD_FAST                'pos'
               16  LOAD_ATTR                checkskip
               18  LOAD_STR                 '$'
               20  CALL_FUNCTION_1       1  '1 positional argument'
               22  POP_JUMP_IF_FALSE    50  'to 50'

 L.4962        24  LOAD_FAST                'self'
               26  LOAD_ATTR                writeparam
               28  LOAD_FAST                'pos'
               30  CALL_FUNCTION_1       1  '1 positional argument'
               32  STORE_FAST               'param'

 L.4963        34  LOAD_FAST                'param'
               36  POP_JUMP_IF_FALSE   170  'to 170'

 L.4964        38  LOAD_FAST                'result'
               40  LOAD_ATTR                append
               42  LOAD_FAST                'param'
 ...
 L.4965        50  LOAD_FAST                'pos'
               52  LOAD_ATTR                checkskip
               54  LOAD_STR                 'f'
               56  CALL_FUNCTION_1       1  '1 positional argument'
               58  POP_JUMP_IF_FALSE    92  'to 92'

 L.4966        60  LOAD_FAST                'self'
               62  LOAD_ATTR                writefunction
               64  LOAD_FAST                'pos'
               66  CALL_FUNCTION_1       1  '1 positional argument'
               68  STORE_FAST               'function'

 L.4967        70  LOAD_FAST                'function'
               72  POP_JUMP_IF_FALSE   170  'to 170'

 L.4968        74  LOAD_CONST               None
               76  LOAD_FAST                'function'
               78  STORE_ATTR               type

 L.4969        80  LOAD_FAST                'result'
               82  LOAD_ATTR                append
               84  LOAD_FAST                'function'
               86  CALL_FUNCTION_1       1  '1 positional argument'
               88  POP_TOP          
               90  JUMP_BACK             6  'to 6'
               92  ELSE                     '170'

 L.4970        92  LOAD_FAST                'pos'
               94  LOAD_ATTR                checkskip
               96  LOAD_STR                 '('
               98  CALL_FUNCTION_1       1  '1 positional argument'
              100  POP_JUMP_IF_FALSE   122  'to 122'

 L.4971       102  LOAD_FAST                'result'
              104  LOAD_ATTR                append
              106  LOAD_FAST                'self'
              108  LOAD_ATTR                writebracket
              110  LOAD_STR                 'left'
              112  LOAD_STR                 '('
 ...
 L.4972       122  LOAD_FAST                'pos'
              124  LOAD_ATTR                checkskip
              126  LOAD_STR                 ')'
              128  CALL_FUNCTION_1       1  '1 positional argument'
              130  POP_JUMP_IF_FALSE   152  'to 152'

 L.4973       132  LOAD_FAST                'result'
              134  LOAD_ATTR                append
              136  LOAD_FAST                'self'
              138  LOAD_ATTR                writebracket
              140  LOAD_STR                 'right'
              142  LOAD_STR                 ')'
              144  CALL_FUNCTION_2       2  '2 positional arguments'
              146  CALL_FUNCTION_1       1  '1 positional argument'
              148  POP_TOP          
              150  JUMP_BACK             6  'to 6'
              152  ELSE                     '170'

 L.4975       152  LOAD_FAST                'result'
              154  LOAD_ATTR                append
              156  LOAD_GLOBAL              FormulaConstant
              158  LOAD_FAST                'pos'
              160  LOAD_ATTR                skipcurrent
              ...
              168  POP_TOP          
            170_0  COME_FROM            72  '72'
            170_1  COME_FROM            36  '36'
              170  JUMP_BACK             6  'to 6'
              172  POP_BLOCK        
            174_0  COME_FROM_LOOP        4  '4'

 L.4976       174  LOAD_FAST                'result'
              176  RETURN_VALUE     
               -1  RETURN_LAST      
        ...
        if not pos.current().isdigit():
            ...
            for variable in self.params:
                if variable in tag:
                    ...
                    param.literal or Trace.error('Parameters in tag ' + tag + ' should be literal: {' + variable + '!}')
                    continue
