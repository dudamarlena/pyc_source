...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pkginfo/pkginfo/commandline.py
...
def _parse_options(args=None):
    ...
    parser.add_option('-m', '--metadata-version', default=None, help='Override metadata version')
    parser.add_option('-f', '--field', dest='fields', action='append', help='Specify an output field (repeatable)')
    parser.add_option('-d', '--download-url-prefix', dest='download_url_prefix',
      help='Download URL prefix')
    parser.add_option('--simple', dest='output', action='store_const', const='simple',
      default='simple',
      help='Output as simple key-value pairs')
    parser.add_option('-s', '--skip', dest='skip', action='store_true', default=True,
      help='Skip missing values in simple output')
    parser.add_option('-S', '--no-skip', dest='skip', action='store_false', help="Don't skip missing values in simple output")
    parser.add_option('--single', dest='output', action='store_const', const='single',
      help='Output delimited values')
    parser.add_option('--item-delim', dest='item_delim', action='store', default=';',
      help='Delimiter for fields in single-line output')
    parser.add_option('--sequence-delim', dest='sequence_delim', action='store',
      default=',',
      help='Delimiter for multi-valued fields')
    parser.add_option('--csv', dest='output', action='store_const', const='csv',
      help='Output as CSV')
    parser.add_option('--ini', dest='output', action='store_const', const='ini',
      help='Output as INI')
    parser.add_option('--json', dest='output', action='store_const', const='json',
      help='Output as JSON')
