...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/jinja2/jinja2/nativetypes.py
...
def native_concat(nodes):
    ...
    if not head:
        ...
        if len(head) == 1:
            ...
            out = ''.join([text_type(v) for v in chain(head, nodes)])
    try:
...
class NativeCodeGenerator(CodeGenerator):
    __doc__ = 'A code generator which avoids injecting ``to_string()`` calls around the\n    internal code Jinja uses to render templates.\n    '
...
class NativeEnvironment(Environment):
    __doc__ = 'An environment that renders templates to native Python types.'
