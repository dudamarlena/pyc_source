...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pytest-sugar/pytest_sugar.py
# Compiled at: 2019-07-30 18:47:09
# Size of source mod 2**32: 15909 bytes
"""
pytest_sugar
~~~~~~~~~~~~
...
def pytest_addoption(parser):
    ...
    group._addoption('--nosugar',
      action='store_false', dest='sugar', default=True, help='disable pytest-sugar')
...
def pytest_configure(config):
    if config.option.sugar:
        ...
        print(colored('WARNING: --nosugar is deprecated, please use -p no:sugar instead.', 'yellow'))
...
class SugarTerminalReporter(TerminalReporter):
    ...
    def pytest_sessionstart(self, session):
        ...
        self.write_line(('Test session starts (platform: %s, Python %s, pytest %s, pytest-sugar %s)' % (
    ...
    def insert_progress(self):
        ...
        def get_progress_bar():
            ...
            if progressbar == '100% ':
                if self.tests_taken < self.tests_count:
                    progressbar = '99% '
            else:
            ...
            for block, success in self.progress_blocks:
                ...
                if last < block:
                    progressbar += colored(bar[last:block], last_theme, 'on_' + THEME['progressbar_background'])
                progressbar += colored(bar[block], theme, 'on_' + THEME['progressbar_background'])
                last = block + 1
                last_theme = theme

            if last < len(bar):
                progressbar += colored(bar[last:len(bar)], last_theme, 'on_' + THEME['progressbar_background'])
            return progressbar
    ...
    def append_string(self, append_string=''):
        ...
        num_spaces = console_width - real_string_length(self.current_line) - real_string_length(append_string) - LEN_RIGHT_MARGIN
    ...
    def get_max_column_for_test_status(self):
        return self._tw.fullwidth - LEN_PROGRESS_PERCENTAGE - LEN_PROGRESS_BAR - LEN_RIGHT_MARGIN
    ...
    def begin_new_line(self, report, print_filename):
        ...
        if print_filename:
            self.current_line = ' ' + colored(test_location, THEME['path']) + colored(test_name, THEME['name']) + ' '
        else:
                ...
                if path != self.currentfspath2:
                    ...
                    if report.failed:
                        if not self.progress_blocks or self.progress_blocks[(-1)][0] != block:
                            ...
                            if self.progress_blocks and self.progress_blocks[(-1)][0] == block:
                                self.progress_blocks[(-1)][1] = False
                    elif not self.progress_blocks or self.progress_blocks[(-1)][0] != block:
