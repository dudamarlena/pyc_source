...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-m_4qh6p6/pkginfo/pkginfo/tests/test_distribution.py
# Compiled at: 2019-07-30 18:47:11
# Size of source mod 2**32: 16703 bytes
import unittest
...
class DistributionTests(unittest.TestCase):
    ...
    def test_parse_Metadata_Version_1_0(self):
        ...
        self.assertEqual(list(dist), [x[1] for x in HEADER_ATTRS_1_0])
    ...
    def test_parse_Metadata_Version_1_1(self):
        ...
        self.assertEqual(list(dist), [x[1] for x in HEADER_ATTRS_1_1])
    ...
    def test_parse_Metadata_Version_1_2(self):
        ...
        self.assertEqual(list(dist), [x[1] for x in HEADER_ATTRS_1_2])
    ...
    def test_parse_Metadata_Version_2_1(self):
        ...
        self.assertEqual(list(dist), [x[1] for x in HEADER_ATTRS_2_1])
    ...
    def test_parse_Description(self):
        dist = self._makeOne()
        dist.parse('Description: This package enables integration with foo servers.')
        self.assertEqual(dist.description, 'This package enables integration with foo servers.')

    def test_parse_Description_multiline(self):
        dist = self._makeOne()
        dist.parse('Description: This package enables integration with\n        foo servers.')
        self.assertEqual(dist.description, 'This package enables integration with\nfoo servers.')
    ...
    def test_parse_Download_URL(self):
        dist = self._makeOne('1.1')
        dist.parse('Download-URL: http://example.com/package/mypackage-0.1.zip')
        self.assertEqual(dist.download_url, 'http://example.com/package/mypackage-0.1.zip')
    ...
    def test_parse_Requires_Dist_single_w_env_marker(self):
        dist = self._makeOne('1.2')
        dist.parse("Requires-Dist: SpanishInquisition; python_version == '1.4'")
        self.assertEqual(list(dist.requires_dist), [
