...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-x0nyl_ya/baron/baron/inner_formatting_grouper.py
...
GROUP_THOSE = ('ENDL', 'COMMENT', 'SPACE')
ENTER_GROUPING_MODE = ('LEFT_PARENTHESIS', 'LEFT_BRACKET', 'LEFT_SQUARE_BRACKET')
QUIT_GROUPING_MODE = ('RIGHT_PARENTHESIS', 'RIGHT_BRACKET', 'RIGHT_SQUARE_BRACKET')
GROUP_ON = ('COMMA', 'COLON', 'STRING', 'RAW_STRING', 'INTERPOLATED_STRING', 'INTERPOLATED_RAW_STRING',
            'BINARY_STRING', 'BINARY_RAW_STRING', 'UNICODE_STRING', 'UNICODE_RAW_STRING',
            'AS', 'IMPORT', 'DOUBLE_STAR', 'DOT', 'LEFT_SQUARE_BRACKET', 'STAR',
            'SLASH', 'PERCENT', 'DOUBLE_SLASH', 'PLUS', 'MINUS', 'LEFT_SHIFT', 'RIGHT_SHIFT',
            'AMPER', 'CIRCUMFLEX', 'VBAR', 'LESS', 'GREATER', 'EQUAL_EQUAL', 'LESS_EQUAL',
            'GREATER_EQUAL', 'NOT_EQUAL', 'IN', 'IS', 'NOT', 'AND', 'OR', 'AT', 'IF',
            'ELSE', 'FROMEQUAL', 'PLUS_EQUAL', 'MINUS_EQUAL', 'AT_EQUAL', 'STAR_EQUAL',
            ...
            'ENDL', 'FOR', 'COLON', 'RAW_STRING', 'UNICODE_STRING', 'UNICODE_RAW_STRING') + ENTER_GROUPING_MODE + QUIT_GROUPING_MODE

def append_to_token_after(token, to_append_list):
    if len(token) == 2:
        return (
         token[0], token[1], [], to_append_list)
    else:
        if len(token) == 3:
            return (
             token[0], token[1], token[2], to_append_list)
        if len(token) == 4:
            return (
             token[0], token[1], token[2], token[3] + to_append_list)
...
def append_to_token_before(token, to_append_list):
    if len(token) == 2:
        ...
        if len(token) == 3:
            return (
             token[0], token[1], to_append_list + token[2], [])
        if len(token) == 4:
            return (
             token[0], token[1], to_append_list + token[2], token[3])
