...
# [GCC 8.4.0]
# Embedded file name: /home/ivan/release/sqlitedict/tests/test_core.py
# Compiled at: 2018-01-26 03:05:12
import json, unittest, tempfile, sys, os, sqlitedict
...
class NamedSqliteDictCreateOrReuseTest(TempSqliteDictTest):
    ...
    def test_irregular_tablenames(self):
        ...
        self.assertEqual(db['key'], 'value')
        db.close()
        with self.assertRaisesRegexp(ValueError, '^Invalid tablename '):
            SqliteDict(':memory:', '"')
...
class SqliteDictTerminateFailTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.chmod(self.fname, 0)
        os.chmod(os.path.dirname(self.fname), 0)

    def tearDown(self):
        os.chmod(os.path.dirname(self.fname), 448)
        os.chmod(self.fname, 384)
...
class SqliteDictJsonSerializationTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.db = SqliteDict(filename=self.fname, tablename='test', encode=json.dumps, decode=json.loads)
    ...
    def test_str(self):
        ...
        assert self.get_json('test') == '"Test \\u30c6\\u30b9\\u30c8"'
