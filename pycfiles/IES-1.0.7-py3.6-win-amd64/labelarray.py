...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\lib\labelarray.py
# Compiled at: 2018-04-03 04:51:51
# Size of source mod 2**32: 27103 bytes
"""
...
from strategycontainer.utils.functional import instance
from strategycontainer.utils.preprocess import preprocess
from strategycontainer.utils.sentinel import sentinel
from strategycontainer.utils.input_validation import coerce, expect_kinds, expect_types, optional
from strategycontainer.utils.numpy_utils import bool_dtype, unsigned_int_dtype_with_size_in_bytes, is_object, object_dtype
from strategycontainer.utils.pandas_utils import ignore_pandas_nan_categorical_warning
...
class LabelArray(ndarray):
    ...
    def __new__(cls, values, missing_value, categories=None, sort=True):
        if not is_object(values):
            ...
            if categories is None:
                codes, categories, reverse_categories = factorize_strings((values.ravel()),
                  missing_value=missing_value,
                  sort=sort)
            else:
                codes, categories, reverse_categories = factorize_strings_known_categories((values.ravel()),
                  categories=categories,
                  missing_value=missing_value,
                  sort=sort)
        categories.setflags(write=False)
        return cls.from_codes_and_metadata(codes=(codes.reshape(values.shape)),
          categories=categories,
    ...
    def as_categorical(self, name=None):
        ...
        with ignore_pandas_nan_categorical_warning():
            return pd.Categorical.from_codes((self.as_int_array()),
    ...
    def as_categorical_frame(self, index, columns, name=None):
        ...
        if expected_shape != self.shape:
            raise ValueError("Can't construct a DataFrame with provided indices:\n\nLabelArray shape is {actual}, but index and columns imply that shape should be {expected}.".format(actual=(self.shape),
              expected=expected_shape))
        return pd.Series(index=(pd.MultiIndex.from_product([index, columns])),
          data=self.ravel().as_categorical(name=name)).unstack()
    ...
    def _equality_check(op):
        ...
        def method(self, other):
            if isinstance(other, LabelArray):
                ...
                if isinstance(other, self.SUPPORTED_SCALAR_TYPES):
    ...
    def view(self, dtype=_NotPassed, type=_NotPassed):
        if type is _NotPassed:
            if dtype not in (_NotPassed, self.dtype):
    ...
    def astype(self, dtype, order='K', casting='unsafe', subok=True, copy=True):
        if dtype == self.dtype:
            ...
            if dtype.kind == 'S':
                return self.as_string_array().astype(dtype,
                  order=order,
                  casting=casting,
                  subok=subok,
                  copy=copy)
        raise TypeError('%s can only be converted into object, string, or void, got: %r' % (
         type(self).__name__,
         dtype))
    ...
    def empty_like(self, shape):
        ...
        """
        return type(self).from_codes_and_metadata(codes=np.full(shape,
    ...
    def map(self, f):
        ...
        new_categories, bloated_inverse_index = np.unique(new_categories_with_duplicates,
          return_inverse=True)
        ...
        return self.from_codes_and_metadata(new_codes,
          new_categories,
...
class _sortable_sentinel(object):
    __doc__ = 'Dummy object that sorts before any other python object.\n    '
