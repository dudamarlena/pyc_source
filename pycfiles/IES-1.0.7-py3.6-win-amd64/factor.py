...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\tsp\factors\factor.py
# Compiled at: 2018-10-19 02:44:00
# Size of source mod 2**32: 59249 bytes
"""
factor.py
"""
...
from strategycontainer.exception import BadPercentileBounds, UnknownRankMethod, UnsupportedDataType
from strategycontainer.lib.normalize import naive_grouped_rowwise_apply
from strategycontainer.lib.rank import masked_rankdata_2d, rankdata_1d_descending
from strategycontainer.tsp.api_utils import restrict_to_dtype
from strategycontainer.tsp.classifiers import Classifier, Everything, Quantiles
from strategycontainer.tsp.dtypes import CLASSIFIER_DTYPES, FACTOR_DTYPES, FILTER_DTYPES
from strategycontainer.tsp.expression import BadBinaryOperator, COMPARISONS, is_comparison, MATH_BINOPS, method_name_for_op, NumericalExpression, NUMEXPR_MATH_FUNCS, UNARY_OPS, unary_op_name
from strategycontainer.tsp.filters import Filter, NumExprFilter, PercentileFilter, MaximumFilter, NotNullFilter, NullFilter
from strategycontainer.tsp.mixins import AliasedMixin, CustomTermMixin, DownsampledMixin, LatestMixin, PositiveWindowLengthMixin, RestrictedDTypeMixin, SingleInputMixin
from strategycontainer.tsp.sentinels import NotSpecified, NotSpecifiedType
from strategycontainer.tsp.term import ComputableTerm, Term
from strategycontainer.utils.functional import with_doc, with_name
from strategycontainer.utils.input_validation import expect_types
from strategycontainer.utils.math_utils import nanmean, nanstd
from strategycontainer.utils.memoize import classlazyval
from strategycontainer.utils.numpy_utils import bool_dtype, coerce_to_dtype, float64_dtype
...
def binop_return_dtype(op, left, right):
    ...
    if is_comparison(op):
        if left != right:
            raise TypeError("Don't know how to compute {left} {op} {right}.\nComparisons are only supported between Factors of equal dtypes.".format(left=left,
              op=op,
              right=right))
        return bool_dtype
    else:
        if left != float64_dtype or right != float64_dtype:
            raise TypeError("Don't know how to compute {left} {op} {right}.\nArithmetic operators are only supported between Factors of dtype 'float64'.".format(left=(left.name),
              op=op,
              right=(right.name)))
...
def binary_operator(op):
    ...
    def binary_operator(self, other):
        ...
        if isinstance(self, NumExprFactor):
            ...
            return return_type('({left}) {op} ({right})'.format(left=self_expr,
              op=op,
              right=other_expr),
              new_inputs,
              dtype=(binop_return_dtype(op, self.dtype, other.dtype)))
        if isinstance(other, NumExprFactor):
            return commuted_method_getter(other)(self)
        if isinstance(other, Term):
            if self is other:
                return return_type('x_0 {op} x_0'.format(op=op),
                  (
                 self,),
                  dtype=(binop_return_dtype(op, self.dtype, other.dtype)))
            else:
                return return_type('x_0 {op} x_1'.format(op=op),
                  (
                 self, other),
                  dtype=(binop_return_dtype(op, self.dtype, other.dtype)))
        if isinstance(other, Number):
            return return_type('x_0 {op} ({constant})'.format(op=op, constant=other),
              binds=(
             self,),
              dtype=(binop_return_dtype(op, self.dtype, other.dtype)))
...
def reflected_binary_operator(op):
    ...
    def reflected_binary_operator(self, other):
        if isinstance(self, NumericalExpression):
            self_expr, other_expr, new_inputs = self.build_binary_op(op, other)
            return NumExprFactor('({left}) {op} ({right})'.format(left=other_expr,
              right=self_expr,
              op=op),
              new_inputs,
              dtype=(binop_return_dtype(op, other.dtype, self.dtype)))
        if isinstance(other, Number):
            return NumExprFactor('{constant} {op} x_0'.format(op=op, constant=other),
              binds=(
             self,),
              dtype=(binop_return_dtype(op, other.dtype, self.dtype)))
...
def unary_operator(op):
    ...
    def unary_operator(self):
        if self.dtype != float64_dtype:
            raise TypeError("Can't apply unary operator {op!r} to instance of {typename!r} with dtype {dtypename!r}.\n{op!r} is only supported for Factors of dtype 'float64'.".format(op=op,
              typename=(type(self).__name__),
              dtypename=(self.dtype.name)))
        if isinstance(self, NumericalExpression):
            return NumExprFactor('{op}({expr})'.format(op=op, expr=(self._expr)),
              (self.inputs),
              dtype=float64_dtype)
        else:
            return NumExprFactor('{op}x_0'.format(op=op),
              (
             self,),
              dtype=float64_dtype)
...
def function_application(func):
    ...
    def mathfunc(self):
        if isinstance(self, NumericalExpression):
            return NumExprFactor('{func}({expr})'.format(func=func, expr=(self._expr)),
              (self.inputs),
              dtype=float64_dtype)
        else:
            return NumExprFactor('{func}(x_0)'.format(func=func),
              (
             self,),
              dtype=float64_dtype)
...
if_not_float64_tell_caller_to_use_isnull = restrict_to_dtype(dtype=float64_dtype,
  message_template='{method_name}() was called on a factor of dtype {received_dtype}.\n{method_name}() is only defined for dtype {expected_dtype}.To filter missing data, use isnull() or notnull().')
float64_only = restrict_to_dtype(dtype=float64_dtype,
  message_template='{method_name}() is only defined on Factors of dtype {expected_dtype}, but it was called on a Factor of dtype {received_dtype}.')
...
class Factor(RestrictedDTypeMixin, ComputableTerm):
    ...
    @expect_types(mask=(
     Filter, NotSpecifiedType),
      groupby=(
     Classifier, NotSpecifiedType))
    @float64_only
    def demean(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        """
        return GroupedRowTransform(transform=demean,
          transform_args=(),
          factor=self,
          groupby=groupby,
          dtype=(self.dtype),
    ...
    def zscore(self, mask=NotSpecified, groupby=NotSpecified):
        ...
        :meth:`pandas.DataFrame.groupby`
        """
        return GroupedRowTransform(transform=zscore,
          transform_args=(),
          factor=self,
          groupby=groupby,
          dtype=(self.dtype),
    ...
    def rank(self, method='ordinal', ascending=True, mask=NotSpecified, groupby=NotSpecified):
        ...
        if groupby is NotSpecified:
            ...
            return GroupedRowTransform(transform=(rankdata if ascending else rankdata_1d_descending),
              transform_args=(
             method,),
              factor=self,
              groupby=groupby,
    ...
    def pearsonr(self, target, correlation_length, mask=NotSpecified):
        ...
        return RollingPearson(base_factor=self,
          target=target,
    ...
    def spearmanr(self, target, correlation_length, mask=NotSpecified):
        ...
        return RollingSpearman(base_factor=self,
          target=target,
          correlation_length=correlation_length,
          mask=mask)
    ...
    def linear_regression(self, target, regression_length, mask=NotSpecified):
        ...
        return RollingLinearRegression(dependent=self,
          independent=target,
          regression_length=regression_length,
          mask=mask)

    @expect_types(min_percentile=(
     int, float),
      max_percentile=(
     int, float),
      mask=(
    ...
    def winsorize(self, min_percentile, max_percentile, mask=NotSpecified, groupby=NotSpecified):
        ...
        if not 0.0 <= min_percentile < max_percentile <= 1.0:
            raise BadPercentileBounds(min_percentile=min_percentile,
              max_percentile=max_percentile,
              upper_bound=1.0)
        return GroupedRowTransform(transform=winsorize,
          transform_args=(
...
class GroupedRowTransform(Factor):
 ...
 GroupedRowTransform, transform=transform, 
 ...
 factor, groupby), 
    ...
    def _compute(self, arrays, dates, assets, mask):
        ...
        return where(group_labels != null_label, naive_grouped_rowwise_apply(data=data,
          group_labels=group_labels,
          func=(self._transform),
          func_args=(self._transform_args),
          out=empty_like(data, dtype=(self.dtype))), self.missing_value)
...
class Rank(SingleInputMixin, Factor):
    ...
    def _validate(self):
        if self._method not in _RANK_METHODS:
            raise UnknownRankMethod(method=(self._method),
    ...
    def __repr__(self):
        return "{type}({input_}, method='{method}', mask={mask})".format(type=(type(self).__name__),
...
class CustomFactor(PositiveWindowLengthMixin, CustomTermMixin, Factor):
    ...
    def _validate(self):
        try:
            ...
            if self.dtype in CLASSIFIER_DTYPES:
                ...
                if self.dtype in FILTER_DTYPES:
                    raise UnsupportedDataType(typename=(type(self).__name__),
                      dtype=(self.dtype),
    ...
    def __getattribute__(self, name):
        ...
        try:
            ...
            raise AttributeError('Instance of {factor} has no output named {attr!r}. Possible choices are: {choices}.'.format(factor=(type(self).__name__),
              attr=name,
...
class RecarrayField(SingleInputMixin, Factor):
    ...
    def __new__(cls, factor, attribute):
        return super(RecarrayField, cls).__new__(cls,
          attribute=attribute,
          inputs=[
         factor],
          window_length=0,
