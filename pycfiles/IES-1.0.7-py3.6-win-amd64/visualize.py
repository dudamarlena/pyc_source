...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\tsp\visualize.py
# Compiled at: 2018-01-16 21:29:26
...
from strategycontainer.tsp.data import BoundColumn
from strategycontainer.tsp import Filter, Factor, Classifier, Term
...
def _render(g, out, format_, include_asset_exists=False):
    ...
    with graph(f, 'G', **graph_attrs):
        ...
        for term in filter_nodes(include_asset_exists, topological_sort(g.graph)):
            if not term in in_nodes:
                if term in out_nodes:
                    continue
                add_term_node(f, term)

        for source, dest in g.graph.edges():
            if source is AssetExists():
                if not include_asset_exists:
                    continue
    ...
    try:
        ...
        if e.errno == errno.ENOENT:
            raise RuntimeError("Couldn't find `dot` graph layout program. Make sure Graphviz is installed and `dot` is on your path.")
        else:
            raise
