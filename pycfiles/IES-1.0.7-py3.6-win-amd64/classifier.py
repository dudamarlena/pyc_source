...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\tsp\classifiers\classifier.py
# Compiled at: 2018-01-16 04:23:15
# Size of source mod 2**32: 18193 bytes
"""
classifier.py
"""
...
import pandas as pd
from strategycontainer.exception import UnsupportedDataType
from strategycontainer.lib.labelarray import LabelArray
from strategycontainer.lib.quantiles import quantiles
...
from strategycontainer.tsp.term import ComputableTerm
from strategycontainer.utils.compat import unicode
from strategycontainer.utils.input_validation import expect_types, expect_dtypes
from strategycontainer.utils.memoize import classlazyval
from strategycontainer.utils.numpy_utils import categorical_dtype, int64_dtype, vectorized_is_element
from ..filters import ArrayPredicate, NotNullFilter, NullFilter, NumExprFilter
from ..mixins import AliasedMixin, CustomTermMixin, DownsampledMixin, LatestMixin, PositiveWindowLengthMixin, RestrictedDTypeMixin, SingleInputMixin, StandardOutputs
string_classifiers_only = restrict_to_dtype(dtype=categorical_dtype,
  message_template='{method_name}() is only defined on Classifiers producing strings but it was called on a Classifier of dtype {received_dtype}.')
...
class Classifier(RestrictedDTypeMixin, ComputableTerm):
    ...
    def __ne__(self, other):
        ...
        if isinstance(other, Number) != (self.dtype == int64_dtype):
            raise InvalidClassifierComparison(self, other)
        if isinstance(other, Number):
            return NumExprFilter.create('((x_0 != {other}) & (x_0 != {missing}))'.format(other=(int(other)),
    ...
    def element_of(self, choices):
        ...
        try:
            ...
            raise TypeError('Expected `choices` to be an iterable of hashable values, but got {} instead.\nThis caused the following error: {!r}.'.format(choices, e))

        if self.missing_value in choices:
            raise ValueError('Found self.missing_value ({mv!r}) in choices supplied to {typename}.{meth_name}().\nMissing values have NaN semantics, so the requested comparison would always produce False.\nUse the isnull() method to check for missing values.\nReceived choices were {choices}.'.format(mv=(self.missing_value),
              typename=(type(self).__name__),
        ...
        if self.dtype == int64_dtype:
            if only_contains(int, choices):
                return ArrayPredicate(term=self,
                  ...
                  opargs=(
                 choices,))
            raise TypeError('Found non-int in choices for {typename}.element_of.\nSupplied choices were {choices}.'.format(typename=(type(self).__name__),
              choices=choices))
        else:
            if self.dtype == categorical_dtype:
                if only_contains((bytes, unicode), choices):
                    return ArrayPredicate(term=self,
                      op=(LabelArray.element_of),
                      opargs=(
                     choices,))
                raise TypeError('Found non-string in choices for {typename}.element_of.\nSupplied choices were {choices}.'.format(typename=(type(self).__name__),
                  choices=choices))
    ...
    def postprocess(self, data):
        if self.dtype == int64_dtype:
            ...
            assert isinstance(data, LabelArray), 'Expected a LabelArray, got %s.' % type(data)
    ...
    def to_workspace_value(self, result, assets):
        if self.dtype == int64_dtype:
            ...
            assert isinstance(result.values, pd.Categorical), 'Expected a Categorical, got %r.' % type(result.values)
            with_missing = pd.Series(data=(pd.Categorical(result.values, result.values.categories.union([self.missing_value]))),
              index=(result.index))
            return LabelArray(super(Classifier, self).to_workspace_value(with_missing, assets), self.missing_value)
...
class Everything(Classifier):
    ...
    def _compute(self, arrays, dates, assets, mask):
        return where(mask, zeros(shape=(mask.shape), dtype=int64_dtype), self.missing_value)
...
class Relabel(SingleInputMixin, Classifier):
    ...
    def __new__(cls, term, relabeler):
        return super(Relabel, cls).__new__(cls,
          inputs=(
         term,),
          dtype=(term.dtype),
          mask=(term.mask),
    ...
    def _compute(self, arrays, dates, assets, mask):
        ...
        if isinstance(data, LabelArray):
            ...
            raise NotImplementedError('Relabeling is not currently supported for int-dtype classifiers.')
        return result
...
class InvalidClassifierComparison(TypeError):
    ...
    def __init__(self, classifier, compval):
        super(InvalidClassifierComparison, self).__init__("Can't compare classifier of dtype {dtype} to value {value} of type {type}.".format(dtype=(classifier.dtype),
          value=compval,
