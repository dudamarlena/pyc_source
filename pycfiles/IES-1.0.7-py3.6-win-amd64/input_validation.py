...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\utils\input_validation.py
# Compiled at: 2018-01-16 01:11:07
# Size of source mod 2**32: 25577 bytes
...
from .functional import getattrs
from .preprocess import call, preprocess
if PY3:
...
def verify_indices_all_unique(obj):
    ...
    axis_names = [
     ('index', ),
     ('index', 'columns'),
     ('items', 'major_axis', 'minor_axis')][(obj.ndim - 1)]
    for axis_name, index in zip(axis_names, obj.axes):
        if index.is_unique:
            pass
        else:
            raise ValueError('Duplicate entries in {type}.{axis}: {dupes}.'.format(type=(type(obj).__name__),
              axis=axis_name,
...
def optionally(preprocessor):
    ...
    def wrapper(func, argname, arg):
        if arg is None:
            ...
            return preprocessor(func, argname, arg)
...
def ensure_dtype(func, argname, arg):
    ...
    try:
        ...
        raise TypeError("{func}() couldn't convert argument {argname}={arg!r} to a numpy dtype.".format(func=(_qualified_name(func)),
          argname=argname,
          arg=arg))
...
def ensure_timezone(func, argname, arg):
    ...
    if isinstance(arg, string_types):
        return timezone(arg)
    raise TypeError("{func}() couldn't convert argument {argname}={arg!r} to a timezone.".format(func=(_qualified_name(func)),
      argname=argname,
      arg=arg))
...
def expect_dtypes(__funcname=_qualified_name, **named):
    ...
    for name, type_ in iteritems(named):
        if not isinstance(type_, (dtype, tuple)):
            raise TypeError('expect_dtypes() expected a numpy dtype or tuple of dtypes for argument {name!r}, but got {dtype} instead.'.format(name=name,
              dtype=dtype))
    ...
    def _expect_dtype(dtypes):
        ...
        def error_message(func, argname, value):
            ...
            return '{funcname}() expected a value with dtype {dtype_str} for argument {argname!r}, but got {value!r} instead.'.format(funcname=(get_funcname(func)),
...
def expect_kinds(**named):
    ...
    for name, kind in iteritems(named):
        if not isinstance(kind, (str, tuple)):
            raise TypeError('expect_dtype_kinds() expected a string or tuple of strings for argument {name!r}, but got {kind} instead.'.format(name=name,
              kind=dtype))
    ...
    def _expect_kind(kinds):
        ...
        def error_message(func, argname, value):
            ...
            return '{funcname}() expected a numpy object of kind {kinds} for argument {argname!r}, but got {value!r} instead.'.format(funcname=(_qualified_name(func)),
...
def expect_types(__funcname=_qualified_name, **named):
    ...
    for name, type_ in iteritems(named):
        if not isinstance(type_, (type, tuple)):
            raise TypeError("expect_types() expected a type or tuple of types for argument '{name}', but got {type_} instead.".format(name=name,
              type_=type_))
    ...
    def _expect_type(type_):
        ...
        return make_check(exc_type=TypeError,
          template=template,
...
def make_check(exc_type, template, pred, actual, funcname):
    ...
    def _check(func, argname, argvalue):
        if pred(argvalue):
            raise exc_type(template % {'funcname':get_funcname(func), 
             'argname':argname, 
...
def expect_bounded(__funcname=_qualified_name, **named):
    ...
    def _make_bounded_check(bounds):
        ...
        return make_check(exc_type=ValueError,
          template=template,
          pred=should_fail,
          actual=repr,
          funcname=__funcname)
...
def _expect_bounded(make_bounded_check, __funcname, **named):
    ...
    for name, bounds in iteritems(named):
        if not valid_bounds(bounds):
            raise TypeError("expect_bounded() expected a tuple of bounds for argument '{name}', but got {bounds} instead.".format(name=name,
              bounds=bounds))
    ...
    def _expect_dimension(expected_ndim):
        ...
        def _check(func, argname, argvalue):
            ...
            if actual_ndim != expected_ndim:
                ...
                raise ValueError('{func}() expected a {expected:d}-D array for argument {argname!r}, but got a {actual} instead.'.format(func=(get_funcname(func)),
