...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\lib\adjusted_array.py
# Compiled at: 2018-04-03 04:51:51
# Size of source mod 2**32: 9070 bytes
from textwrap import dedent
from numpy import bool_, dtype, float32, float64, int32, int64, int16, uint16, ndarray, uint32, uint8
from strategycontainer.exception import WindowLengthNotPositive, WindowLengthTooLong
from .labelarray import LabelArray
from strategycontainer.utils.numpy_utils import datetime64ns_dtype, float64_dtype, int64_dtype, uint8_dtype
...
BOOL_DTYPES = frozenset(map(dtype, [bool_, uint8]))
FLOAT_DTYPES = frozenset(map(dtype, [float32, float64]))
INT_DTYPES = frozenset(map(dtype, [int16, uint16, int32, int64, uint32]))
DATETIME_DTYPES = frozenset(map(dtype, ['datetime64[ns]', 'datetime64[D]']))
OBJECT_DTYPES = frozenset(map(dtype, ['O']))
STRING_KINDS = frozenset(['S', 'U'])
REPRESENTABLE_DTYPES = BOOL_DTYPES.union(FLOAT_DTYPES, INT_DTYPES, DATETIME_DTYPES, OBJECT_DTYPES)
...
CONCRETE_WINDOW_TYPES = {float64_dtype: Float64Window, 
...
def _normalize_array(data, missing_value):
    ...
    if isinstance(data, LabelArray):
        ...
        if data_dtype in BOOL_DTYPES:
            return (
             data.astype(uint8), {'dtype': dtype(bool_)})
        if data_dtype in FLOAT_DTYPES:
            return (
             data.astype(float64), {'dtype': dtype(float64)})
        if data_dtype in INT_DTYPES:
            return (
             data.astype(int64), {'dtype': dtype(int64)})
        if is_categorical(data_dtype):
            ...
            return (
             LabelArray(data, missing_value, sort=False), {})
        if data_dtype.kind == 'M':
            try:
...
class AdjustedArray(object):
    ...
    __slots__ = ('_data', '_view_kwargs', 'adjustments', 'missing_value', '__weakref__')
    ...
    def traverse(self, window_length, offset=0, perspective_offset=0):
        """
        ...
        return self._iterator_type(data,
          (self._view_kwargs),
          (self.adjustments),
          offset,
          window_length,
...
def _check_window_params(data, window_length):
    ...
    if window_length > data.shape[0]:
        raise WindowLengthTooLong(nrows=(data.shape[0]),
