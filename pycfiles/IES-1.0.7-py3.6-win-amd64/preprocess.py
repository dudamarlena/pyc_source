...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\utils\preprocess.py
# Compiled at: 2018-01-16 01:08:58
# Size of source mod 2**32: 7475 bytes
"""
...
_code_argorder = (('co_argcount', 'co_kwonlyargcount') if PY3 else ('co_argcount', )) + ('co_nlocals',
                                                                                         'co_stacksize',
                                                                                         'co_flags',
                                                                                         'co_code',
                                                                                         'co_consts',
    ...
    **processors : dict
    ...
    def _decorator(f):
        ...
        if defaults is None:
            defaults = ()
        no_defaults = (
         NO_DEFAULT,) * (len(args) - len(defaults))
        ...
        if not all(isinstance(arg, str) for arg in args):
            raise TypeError("Can't validate functions using tuple unpacking: %s" % (
             argspec,))
