...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\strategycontainer\utils\functional.py
...
def _gen_unzip(it, elem_len):
    ...
    if elem_len is not None:
        if elem_len != first_elem_len:
            raise ValueError('element at index 0 was length %d, expected %d' % (
             first_elem_len,
             elem_len))
    else:
    ...
    for n, elem in enumerate(it, 1):
        if len(elem) != elem_len:
            raise ValueError('element at index %d was length %d, expected %d' % (
             n,
             len(elem),
             elem_len))
        yield elem
