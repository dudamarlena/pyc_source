...
# [GCC 8.4.0]
# Embedded file name: /home/dethos/.virtualenvs/mdvis/lib/python3.5/site-packages/mdvis/server.py
# Compiled at: 2016-02-23 18:58:25
...
import webbrowser, os, logging, sys
ALLOWED_EXTENSIONS = [
 'gif', 'jpeg', 'jpg', 'bmp', 'png', 'mp4', 'webm']
...
def index():
    ...
    return render_template('index.html', indexpage=indexpage, menu=menu)
...
def show(file_path):
    ...
    if node:
        ...
        if node.get('extension', '') == 'md':
            ...
            return render_template('document.html', content=content, file_path=file_path)
        ifile = get_index_file(node)
        if ifile:
            pass
        return redirect(url_for('show', file_path='{}/{}'.format(file_path, ifile)))
