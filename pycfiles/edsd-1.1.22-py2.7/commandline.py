...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/commandline.py
# Compiled at: 2016-10-12 04:52:34
"""
...
def parse_cp(cp):
    ...
    if not m:
        ...
        if check_id is not None and 0 <= check_id < len(checks):
            point_id = int(point_id) - 1 if point_id and point_id.isdigit() else None
...
class CheckPointCmd(cmd.Cmd):
    ...
    def do_verbose(self, cp):
        ...
        try:
            ...
            print 'current verbose status is "%s"' % ('on' if DEBUG else 'off')
