...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/hsf/parser.py
# Compiled at: 2017-03-07 02:10:09
"""
edas-detector.parser : Parse hsf xml files to get consumer and provider
"""
import os, re, shutil, edsd.envcmd, edsd.common, edsd.logpeeker, edsd.err
...
class HSF(object):
    ...
    def parse_from_xml_with_beans(cls, bean_xml):
        ...
        if not props:
            ...
            return cls(bean_xml.get('id'), interface=props.get('interfaceName'), group=props.get('group'), version=props.get('version'))
...
class Consumer(HSF):
    ...
    def collect(self, path):
        ...
        if sp is None:
            ...
            srcs = [ os.path.join(sp, x, '%s.dat' % self.group) for x in os.listdir(sp) if self.interface in x
                   ]
            if len(srcs) == 0:
                return
    ...
    def parsed_from_xml(cls, xml):
        if xml is None:
            ...
            return Consumer(xml.get('id'), xml.get('interface'), xml.get('group'))

    def __eq__(self, other):
        if other is None or not isinstance(other, Provider):
            return False
        return self.interface == other.interface and self.group == other.group
...
class Provider(HSF):
    ...
    def parsed_from_xml(cls, xml):
        if xml is None:
            ...
            return Provider(xml.get('id'), xml.get('interface'), xml.get('group'), xml.get('version'))
    ...
    def parse_from_xml_with_beans(cls, bean_xml):
        ...
        if not props:
            ...
            props = dict([ (x.get('name'), value(x)) for x in props ])
            return cls(bean_xml.get('id'), interface=props.get('serviceInterface'), group=props.get('group'), version=props.get('serviceVersion'))

    def __eq__(self, other):
        if other is None or not isinstance(other, Consumer):
            return False
        return self.interface == other.interface and self.version == other.version and self.group == other.group
...
class HSFactory(object):
    ...
    def cc_logpath(self):
        if self._cc_logpath is not None:
            ...
            self._cc_logpath = edsd.logpeeker.move_patial_log(log_path, created)
            return self._cc_logpath
    ...
    def parse_beanns_services(cls, root):
        ...
        providers = filter(lambda b: 'com.taobao.hsf.app.spring.util.HSFSpringProviderBean' == b.get('class'), beans)
        consumers = filter(lambda b: 'com.taobao.hsf.app.spring.util.HSFSpringConsumerBean' == b.get('class'), beans)
        consumers = filter(bool, map(Consumer.parse_from_xml_with_beans, consumers))
        providers = filter(bool, map(Provider.parse_from_xml_with_beans, providers))
        cls._providers.update(providers)
        cls._consumers.update(consumers)