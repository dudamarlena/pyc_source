...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/cps.py
# Compiled at: 2017-03-07 02:10:09
"""
Check Points - a collection of defined check points.
"""
import math, os, re, sys, time, edsd.hsf
from datetime import datetime
from edsd.envcmd import syscmd, get_server_datetime, test_http, test_socket, tomcat_home, localhost_logpath, cai_server_host_port, resolve_name, config_server_host_port, diamond_server_host, edas_agent_home, notify_console_alive, dauth_diamond_host, get, hasconsumer, hasprovider, agent_pid, tomcat_pid, deploy_dir, services_inside_runtime_info, edas_agent_version, read_services_from_pandora
...
@check(name='System Status', strict=False, category='basic')
class CheckSystemStatus(object):
    ...
    def is_platform_supported(self):
        ...
        m = r.search(issue)
        if not m:
            report_fail('CentOS or AliyunOS is recommend, current: %s' % issue)
        report_info(issue)

    @point()
    def ulimit_info(self):
        u"""检测点:查看 ulimit -a 的信息"""
...
class NetworkStatus(object):
    ...
    def local_hosts_content(self):
        u"""执行并汇报 cat /etc/hosts 命令的信息"""
        report_info(syscmd('cat /etc/hosts'))

    @point()
    def resolved_hostname(self):
        u"""执行并汇报 hostname -i 命令的信息"""
    ...
    def iptables(self):
        ...
        result = {}
        for t in tables:
            r = syscmd('iptables -n -L -t %s | grep -v "^Chain\\|^target\\|^$"' % t)
            if not r.strip():
                continue
            result[t] = r.strip()
...
@check(name='Local Date Time', category='basic')
...
@check(name='Java Status', category='basic,java')
class CheckJavaHome(object):
    ...
    @point()
    def jvms(self):
        u"""读取 JVM 的信息,包括 Agent 与 TOMCAT"""
        aid = agent_pid()
        if not aid:
...
@check(name='Authentication', category='advanced,edas,dauth,hsf-precheck')
...
@check(name='Agent Status', category='advanced,edas,agent,hsf-precheck')
class CheckAgentStatus:
    ...
    检测点:
        ...
        if not os.path.isdir(edas_home) or not os.path.isfile('%s/bin/edas-agent' % edas_home):
    ...
    检测点:
        ...
        if not test_http(url):
            report_fail('EDAS Console is not able to connect, resolved: %s' % internal_host)
        internal_host = syscmd('getent hosts edas-internal.console.aliyun.com')
    ...
    检测点:
        ...
        """
        import math
        last_hb_dt = syscmd("tail -n 30 %s/logs/agent.log | grep 'ecu.heartbeat] INFO' | tail -n 1 | awk '{print $1}'" % edas_agent_home())
        if not last_hb_dt:
            report_fail('Heart beat fail')
        scds = [3600, 60, 1]
        seconds = sum([ a * b for a, b in zip(scds, map(int, last_hb_dt.split(':')))
                      ])
...
@check(name='EDAS Status', category='advanced,edas')
class CheckEdasContainer(object):
    ...
    检测点:
        ...
        if not self.within_startup_timespan(startuptime):
            report_fail('Catalina is not found started(startup duration too long)')
        fpath = self.log_path
...
@check(name='Name Server Status', category='advanced,cai,hsf-precheck')
class CheckNameServer(object):
    ...
    检测点:
        ...
        url = 'http://%s:8080/diamond-server/config.do?group=spas_info&dataId=spas_authentication_url' % h
...
@check(name='HSF Status', category='advanced,hsf,hsf-services')
class CheckHsfStatus(object):
    ...
    检测点:
        ...
        if not has:
            report_fail('没有发现有HSF的provider或consumer')
    ...
    def hsf_services(self):
        ...
        lsconsumers, lsproviders = filter(bool, lsconsumers), filter(bool, lsproviders)
        report_info('xml defined consumers count: %d, providers count: %d' % (
         len(lsconsumers),
         len(lsproviders)), data=dict(consumers={'xml': lsconsumers}, providers={'xml': lsproviders}))
