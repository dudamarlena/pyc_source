...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/hsfgrepper.py
...
import json, re, edsd.hsf
from . import __version__
from .envcmd import deploy_dir, services_inside_runtime_info, syscmd, test_socket, read_services_from_pandora, tomcat_params, edas_agent_home, tomcat_pid, read_setenv_info, parse_from_cmdline
...
def provider_ping(service_name):
    ...
    add_report('found %d configured hsf(s) has same interface.' % len(providers))
    strict_matcher = lambda x: x.interface == hsf.interface and x.group == hsf.group and x.version == hsf.version
    providers = filter(strict_matcher, providers)
    if len(providers) == 0:
        report_fail('provider not found after strict matching')
    add_report('found %d configured hsf(s) after strict matching.' % len(providers))
    ...
    add_report('found %d configured hsf(s) has same interface inside runtime log.' % len(rproviders))
    rconsumers = filter(strict_matcher, rproviders)
    if len(rconsumers) == 0:
        report_fail('providers not found inside runtime log after strict matching')
...
def consumer_ping(service_name):
    ...
    add_report('found %d configured hsf(s) has same interface.' % len(consumers))
    strict_matcher = lambda x: x.interface == hsf.interface and x.group == hsf.group and x.version == hsf.version
    consumers = filter(strict_matcher, consumers)
    if len(consumers) == 0:
        report_fail('cusumer not found after strict matching')
    add_report('found %d configured hsf(s) after strict matching.' % len(consumers))
    ...
    add_report('found %d configured hsf(s) has same interface inside runtime log.' % len(rconsumers))
    ...
    if not p:
        report_fail('Pandora is not started')
    c = syscmd('curl -v  http://localhost:12201/hsf/ls?k=%s:%s 2> /dev/null | grep %s' % (
     hsf.interface, hsf.version, hsf.group))
    if not c:
...
def call_service(service_name):
    ...
    syscmd('java -jar -Dtenant.id=%s -Dspas.identity=%s -Daddress.server.domain=%s -Daddress.server.port=%s -Dconfigserver.client.port=%s -Dpandora.location=%s %s/hsf-check.jar %s &>/dev/null' % (
     tenant, spas, as_domain, as_port, cs_port, pandora,
