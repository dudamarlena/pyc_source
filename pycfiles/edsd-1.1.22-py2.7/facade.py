...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/facade.py
# Compiled at: 2016-10-18 07:28:35
"""
...
def run():
    info_('正在为您开始EDAS运行环境的依赖检测,请稍候...', end=True)
    try:
        for f in checks:
            f()
...
def check(name=None, strict=True, category=None):
    ...
    def dec(clz):
        ...
        attributes = dict(_ch_name=ch_name, _check_id=func_id, _category=category, _points=pts, __doc__=clz.__doc__, _strict=strict)
...
def format_point(f):
    return '   ' + colored('*', 'green') + ('{0: >2}: ({1: >16})').format(f._point_id, f._point_name)
