...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/edsd/envcmd.py
...
"""
import email.utils, functools, httplib, os, re
...
def read_setenv_info():
    ...
    if th is None:
        ...
        if args is None:
            return
        return dict([ tuple(x[2:].split('=')) for x in args.split(' ') if x and x.startswith('-D')
                    ])
...
def tomcatxml_has(keyword, *_):
    ...
    if not dir:
        return False
    r = syscmd("find %s/deploy/ -name '*.xml' -exec grep '%s' {} \\;" % (
     dir, keyword))
    if r:
        return r.strip() != ''
    return False
...
hasconsumer = functools.partial(tomcatxml_has, 'consumer\\|HSFSpringConsumerBean')
hasprovider = functools.partial(tomcatxml_has, 'provider\\|HSFSpringProviderBean')

def findhsf_xmlfile(keyword, dir):
    r = syscmd('find %s/WEB-INF/ -name "*.xml" -exec grep "%s" -l {} \\;' % (
     dir, keyword))
    fnames = r.split('\n')
    return [ os.path.join(dir, x[2:]) for x in fnames if x.startswith('./') ] + [ x for x in fnames if os.path.isfile(x) ]


consumerxml_files = functools.partial(findhsf_xmlfile, 'consumer\\|HSFSpringConsumerBean')
providerxml_files = functools.partial(findhsf_xmlfile, 'provider\\|HSFSpringProviderBean')
...
def parse_from_cmdline_for_key(tpid, keyword):
    pinfo = syscmd('cat /proc/%s/cmdline | xargs -i -0 -exec echo {} | grep "%s"' % tpid, keyword).strip()
    try:
...
def spas_info():
    env_info = read_setenv_info() or {}
    spas_file = env_info.get('spas.identity', '/home/admin/.spas_key/default')
...
def load_properties(fname):
    with open(fname) as (f):
        kvs = [ l.strip().split('=', 1) for l in f.readlines() if l and l.strip() ]
        return dict(kvs)
    return {}
...
def guess_tomcat_home():
    l = syscmd("ls -l /home/admin | grep 'tomcat\\|edas-container' | awk '/^d/ {print $NF}'")
    l = l.split('\n')
    if not l:
        return
    else:
...
def times():
    import time
    return (
     get_server_datetime('http://www.taobao.com/'), time.time())
