...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/vod/pluzz/pluzz.py
# Compiled at: 2014-05-20 13:50:13
...
"""
import sys, json, time, textwrap, itertools
...
class PluzzShow(VodServiceShow):
    ...
    def get_summary(self):
        ...
        yield (
         _('Id'), str(self['id']), 'short')
        yield (
         _('Broadcast'), t_when, 'short')
        yield (
         _('Length'), str(self['duree']), 'short')
...
class PluzzService(VodService):
    ...
    def list(self, query=None, category=None, channel=None, limit=100, sort='', page=0):
        ...
        url = self.search_url.format(filter='/'.join(filter_), sort=self.sort.format(sort), limit=limit)
        ...
        return [self.Show({'id': i[0].attrib['href'].split(',')[(-1)].split('.')[0],  'titre': i[0].text.strip(),  'image': i[1].attrib['data-src']}) for i in itertools.zip_longest(*([iter(l)] * 2), fillvalue=None)]
    ...
    def get_show(self, uri):
        ...
        if 'code' in show.keys():
            if show['status'] == 'NOK':
                raise Exception(show['message'])
        return self.Show(show, self.out, AVConvDownloader)
