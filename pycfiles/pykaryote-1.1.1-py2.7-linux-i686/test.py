...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pykaryote/test/test.py
# Compiled at: 2013-08-30 10:10:09
import unittest, os, os.path, shutil, glob, tempfile, ConfigParser as configparser, matplotlib
matplotlib.use('Agg')
...
import pykaryote.sim.environment, pykaryote.sim.simulation, pykaryote.utils.comparison as comparison
...
class TestGlobals(unittest.TestCase):
    ...
    def test_init_globals(self):
        ...
        sim_config = os.path.join(SCRIPT_DIR, 'test_data', 'testInitGlobals-sim.cfg')
...
class TestEnvironment(unittest.TestCase):
    ...
    def test_make_gaussian_grid(self):
        ...
        self.assertTrue(self.env.grid[(9, 9, 0)] < 1e-07)
...
class TestReadComparison(unittest.TestCase):
    ...
    def test_get_config(self):
        ...
        """
        options, values = comparison.get_config(os.path.join(self._base_dir, 'get_config_cmp.cfg'))
        self.assertEqual(options, ('test_option', ))
        self.assertEqual(values, [(1, ), (2, ), (3, )])
        options, values = comparison.get_config(os.path.join(self._base_dir, 'get_config_2_cmp.cfg'))
...
class TestMakeConfig(unittest.TestCase):
    ...
    def setUp(self):
        self._base_dir = os.path.join(SCRIPT_DIR, 'test_data')
        self._out_filename = os.path.join(self._base_dir, 'temp_made_config.cfg')
    ...
    def test_make_config(self):
        ...
        comparison.make_cfg(['point_mutate_chance'], [3.14], base_config, self._out_filename)
...
class TestTimeSpentInGaussian(unittest.TestCase):
    ...
    def setUp(self):
        ...
        sim = pykaryote.sim.simulation.Simulation(os.path.join(self.test_data_dir, 'TestTimeSpentInGaussian-sim.cfg'), name=self.sim_name, data=self.data_dir)
        sim.run(log=False, verbose=False)
        self._analyzer_dir = os.path.join(self.data_dir, self.sim_name, 'analyzer')
    ...
    def test_flat_environment(self):
        ...
        sim = pykaryote.sim.simulation.Simulation(os.path.join(self.test_data_dir, 'TestTimeSpentInFlat-sim.cfg'), name=sim_name, data=self.data_dir)
        sim.run(log=False, verbose=False)
        time_spent_data = [ f for f in os.listdir(analyzer_dir) if f[:27] == 'time_spent_in_gaussian_gen_'
                          ]
        self.assertEqual(time_spent_data, [])
        ann = analysis.Analyzer([analyzer_dir])
        ann.plot_time_spent_in_gaussian()
        plt.savefig(os.path.join(self.data_dir, 'time_spent_in_gaussian_flat.png'))
        shutil.rmtree(os.path.join(self.data_dir, sim_name), ignore_errors=True)

    def tearDown(self):
        """Removes simulation data.
        """
        shutil.rmtree(os.path.join(self.data_dir, self.sim_name), ignore_errors=True)
...
class TestEnvironmentDraw(unittest.TestCase):
    ...
    def setUp(self):
        ...
        gv.init_globals(os.path.join(self.data_dir, 'testEnvironmentDraw-sim.cfg'))
    ...
    def test_draw_environment(self):
        ...
        env = pykaryote.sim.environment.Environment((gv.columns, gv.rows), log=False)
...
class TestAnalyzerQuickRead(unittest.TestCase):
    ...
    def setUp(self):
        ...
        aggregate = self.ann.quick_read('gens_until_complexity_avg', mode='threshold')
    ...
    def test_complexity(self):
        ...
        self.assertTrue(self.complexity[1] != 0.0010190660996112)
        self.assertEqual(self.complexity[999], 3.4628808880592397)
    ...
    def test_variable_length_data(self):
        ...
        data_folders = glob.glob(os.path.join(base_dir, 'analyzer_variable_length', '*', 'analyzer'))
