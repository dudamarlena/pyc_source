...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pykaryote/utils/comparison.py
# Compiled at: 2013-08-30 10:10:09
"""
...
"""
import numpy as np, shutil, os, os.path, glob, math, ConfigParser as configparser, warnings, pkg_resources, matplotlib.pyplot as plt
from pykaryote.sim.environment import Environment
import pykaryote.utils.environment_draw as environment_draw, pykaryote.utils.analysis as analysis
from pykaryote.sim.simulation import Simulation
from pykaryote.utils.globals import settings as gv
warnings.filterwarnings('ignore', '.*Empty input file.*')
metrics_one_col = [
 'complexity_avg', 'irreducible_complexity_avg',
...
metrics_to_plot_final_values = [
 'genome_length_avg', 'complexity_avg']
...
def make_cfg(options, replacement_values, base_config, out_filename):
    ...
    with pkg_resources.resource_stream('pykaryote', os.path.join('configs', 'master.cfg')) as (master_file):
...
class Comparer(object):
    ...
    *Args*:
    ...
    def __init__(self, sim_config, cmp_config, name='Experiment', data='.', clear=True, identical_genomes=True):
    ...
    def run(self, verbose=True):
        ...
        *Args*:
        ...
        for i in range(len(self.values)):
            ...
            sim = Simulation(os.path.join(self.data, self.name, 'sim-%s.cfg' % i), name=name, data=os.path.join(self.data, self.name), comparison=True)
        ...
        agg = AnalyzerAggregator(self.data, self.name, self.values, sim_config=self._sim_config)
        agg.compile_all()
...
class AnalyzerAggregator(object):
    ...
    def compile_all(self):
        ...
        try:
            os.makedirs(os.path.join(self.data, self.name, 'comparative-analyzer'))
        except:
            pass
    ...
    def _compile_time_spent_in_gaussian(self):
        ...
        """
        for i in range(len(self.values)):
            analyzer_dir = os.path.join(self.data, self.name, ('sim-{}').format(i), 'analyzer')
            out_dir = os.path.join(self.data, self.name, 'comparative-analyzer', ('sim-{}').format(i))
            try:
                os.makedirs(out_dir)
            except OSError:
                pass

            for data_file in glob.glob(os.path.join(analyzer_dir, 'time_spent_in_gaussian_gen_*')):
    ...
    def _compile_threshold(self, metric):
        ...
        """
        out_path = os.path.join(self.data, self.name, 'comparative-analyzer', metric)
        means = []
        stddevs = []
        blankss = []
        for i in range(len(self.values)):
    ...
    def compile_metric(self, metric, read_style):
        ...
        *Args*:
        ...
        if read_style == 'one_col':
            for i in range(len(self.values)):
                datas.append(np.loadtxt(os.path.join(self.data, self.name, 'sim-%s' % i, 'analyzer', metric), dtype=float))
                if datas[(-1)].shape == ():
                    datas[-1] = np.array([datas[(-1)]], dtype=float)

            num_rows = [ val.shape[0] for val in datas ]
            f = open(os.path.join(self.data, self.name, 'comparative-analyzer', metric), 'w')
            ...
            for i in range(len(self.values)):
                datas.append(np.loadtxt(os.path.join(self.data, self.name, 'sim-%s' % i, 'analyzer', metric), dtype=float, ndmin=2))

            num_cols = [ val.shape[1] for val in datas ]
            num_rows = [ val.shape[0] for val in datas ]
            for col in range(max(num_cols)):
                f = open(os.path.join(self.data, self.name, 'comparative-analyzer', '%s-%s' % (metric, 1 + col)), 'w')
                for row in range(max(num_rows)):
                    for val in range(len(datas)):
...
class Grapher(object):
    ...
    *Args*:
    ...
    def __init__(self, data_folder, sim_config, cmp_config_filename=None, verbose=True):
        ...
        try:
            num_runs = np.loadtxt(os.path.join(self.base, 'comparative-analyzer', 'num_runs'), dtype=float, ndmin=1)
            if min(num_runs) == max(num_runs):
                self.num_runs = ('{}').format(int(num_runs[0]))
            else:
                self.num_runs = ('{} to {}').format(int(min(num_runs)), int(max(num_runs)))
        except IOError:
            self.num_runs = '1'
    ...
    def save_all(self):
        ...
        for m in metrics_each_col:
            for f in get_metrics_files(os.path.join(self.base, 'comparative-analyzer'), m):
                plt.figure()
                self.plot(f)
        ...
        for m in metrics_to_plot_final_values:
            ...
            plt.savefig(os.path.join(self.base, 'graphs', '%s-final-values.png' % m))
    ...
    def plot(self, metric):
        ...
        *Args*:
        ...
        try:
            data = np.loadtxt(os.path.join(self.base, 'comparative-analyzer', metric), dtype=float, ndmin=2)
        except IOError:
            print 'no data found in', metric, 'a'

        plt.axes([0.1, 0.1, 0.65, 0.8])
        if logy and not any(row[0] > 0 for row in data):
        ...
        for c in range(data.shape[1]):
            ...
            f(data[:, c], linewidth=amount_done * 2 + 0.5, color=(
             0.3 - 0.3 * amount_done, 1 - amount_done, 0.3 * amount_done), label=str(self.values[c]), linestyle=[
             'solid', 'dashed', 'dashdot', 'dotted'][(c % 4)])

        plt.title('%s over %s runs' % (metric, self.num_runs))
        plt.xlabel('generation')
        plt.ylabel(metric)
        plt.legend(title=self.vary_attrs, bbox_to_anchor=(1.01, 1.0), loc=2, borderaxespad=0.0)
    ...
    def plot_final_values(self, metric):
        ...
        try:
            data = np.genfromtxt(os.path.join(self.base, 'comparative-analyzer', metric), dtype=float)
        except IOError:
        ...
        num_generations = data.shape[0]
        bar_x_cords = np.arange(num_bars)
        metrics_at_final_generation = data[(-1)]
        width = 0.2
        bar_labels = self.values
        p1 = plt.bar(bar_x_cords, metrics_at_final_generation, width, color='#175EC8')
        plt.ylabel(metric)
    ...
    def plot_threshold(self, metric, thresholds, save_dir=None):
        ...
        def end_plot(name, save_dir=save_dir):
            if save_dir is not None:
                plt.savefig(os.path.join(self.base, 'graphs', ('{}.png').format(name)))
            return
        ...
        for threshold_index, threshold in enumerate(thresholds):
            ...
            plt.ylim(0.0, 1.0)
            p1 = plt.bar(bar_x_cords, percent_blanks_this_threshold, width, color='#175EC8')
            plt.ylabel('percent runs')
            plt.xlabel(self.vary_attrs)
            plt.title(('percent runs which did not reach {} {} over {} runs').format(metric, threshold, self.num_runs))
            plt.xticks(bar_x_cords + width / 2.0, bar_labels)
            plt.subplots_adjust(bottom=0.1)
            end_plot(('percent_not_passed_{}_{}').format(metric, threshold))
            new_plot()
            failed_threshold = (1.0 - gv.mean_to_aggregate) / 2.0
            bad_mask = blanks[threshold_index] >= (num_runs * failed_threshold).astype(int)
            good_mask = ~bad_mask
            ...
            if len(good_bar_x_cords):
                good = plt.bar(good_bar_x_cords, good_gens_until_threshold, width, yerr=good_stddev, color='#175EC8', ecolor='#0A122A', label='threshold always reached')
            if len(bad_bar_x_cords):
                bad = plt.bar(bad_bar_x_cords, bad_gens_until_threshold, width, yerr=bad_stddev, color='#C60F0F', ecolor='#2A0A0A', label='contains blanks')
            plt.ylabel('generations')
            plt.xlabel(self.vary_attrs)
            plt.title(('generations until {} {} over {} runs').format(metric, threshold, self.num_runs))
            plt.xticks(bar_x_cords + width / 2.0, bar_labels)
            plt.axis('auto')
            plt.ylim(ymin=0.0)
            plt.subplots_adjust(bottom=0.1)
    ...
    def draw_environment(self, save_dir):
        ...
        environment_draw.draw_environment(env.grid, os.path.join(save_dir, 'environment.png'))
    ...
    def save_time_spent_in_gaussian(self):
        ...
        for sim_dir in glob.glob(os.path.join(self.base, 'comparative-analyzer', 'sim-*')):
            ann = analysis.Analyzer([sim_dir])
            ann.plot_time_spent_in_gaussian()
            sim_num = os.path.split(sim_dir)[1][4:]
            plt.savefig(os.path.join(save_dir, ('time_spent_in_gaussians_sim_{}').format(sim_num)))