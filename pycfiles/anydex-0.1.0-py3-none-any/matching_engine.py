...
# [GCC 8.4.0]
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/matching_engine.py
# Compiled at: 2019-05-25 07:31:56
import logging
...
class PriceTimeStrategy(MatchingStrategy):
    ...
    def match(self, order_id, price, quantity, is_ask):
        ...
        while cur_tick_entry and quantity_to_match > 0:
            if cur_tick_entry.is_blocked_for_matching(order_id) or order_id.trader_id == cur_tick_entry.order_id.trader_id:
            ...
            if not cur_tick_entry:
                try:
                    if is_ask:
                        next_price_level = self.order_book.bids.get_price_level_list(price.num_type, price.denom_type).prev_item(cur_price_level_price)
                    else:
                        next_price_level = self.order_book.asks.get_price_level_list(price.num_type, price.denom_type).succ_item(cur_price_level_price)
...
class MatchingEngine(object):
    ...
    def match(self, tick_entry):
        ...
        """
        now = time()
        matched_ticks = self.matching_strategy.match(tick_entry.order_id, tick_entry.price, tick_entry.available_for_matching, tick_entry.tick.is_ask())
        diff = time() - now
