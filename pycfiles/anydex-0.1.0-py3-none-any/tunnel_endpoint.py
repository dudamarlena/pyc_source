...
# [GCC 8.4.0]
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/REST/tunnel_endpoint.py
...
class TunnelCircuitsEndpoint(BaseEndpoint):
    ...
    def render_GET(self, request):
        ...
        return self.twisted_dumps({'circuits': [ {'circuit_id': circuit.circuit_id, 'goal_hops': circuit.goal_hops, 'actual_hops': len(circuit.hops), 'verified_hops': [ hexlify(hop.mid) for hop in circuit.hops ], 'unverified_hop': hexlify(circuit.unverified_hop.mid) if circuit.unverified_hop else '', 'type': circuit.ctype, 'state': circuit.state, 'bytes_up': circuit.bytes_up, 'bytes_down': circuit.bytes_down, 'creation_time': circuit.creation_time} for circuit in self.tunnels.circuits.values()
                     ]})
...
class TunnelRelaysEndpoint(BaseEndpoint):
    ...
    def render_GET(self, request):
        if not self.tunnels:
            request.setResponseCode(http.NOT_FOUND)
            return self.twisted_dumps({'error': 'tunnel community not found'})
        return self.twisted_dumps({'relays': [ {'circuit_from': circuit_from, 'circuit_to': relay.circuit_id, 'is_rendezvous': relay.rendezvous_relay, 'bytes_up': relay.bytes_up, 'bytes_down': relay.bytes_down, 'creation_time': relay.creation_time} for circuit_from, relay in self.tunnels.relay_from_to.items()
                   ]})
...
class TunnelExitsEndpoint(BaseEndpoint):
    ...
    def render_GET(self, request):
        if not self.tunnels:
            request.setResponseCode(http.NOT_FOUND)
            return self.twisted_dumps({'error': 'tunnel community not found'})
        return self.twisted_dumps({'exits': [ {'circuit_from': circuit_from, 'enabled': exit_socket.enabled, 'bytes_up': exit_socket.bytes_up, 'bytes_down': exit_socket.bytes_down, 'creation_time': exit_socket.creation_time} for circuit_from, exit_socket in self.tunnels.exit_sockets.items()
                  ]})
