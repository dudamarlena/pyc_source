...
# [GCC 8.4.0]
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/restapi/asks_bids_endpoint.py
...
class BaseAsksBidsEndpoint(BaseMarketEndpoint):
    ...
    def create_ask_bid_from_params(parameters):
        ...
        first_asset_type = get_param(parameters, 'first_asset_type').decode('utf-8')
        second_asset_type = get_param(parameters, 'second_asset_type').decode('utf-8')
        return (
         AssetPair(AssetAmount(first_asset_amount, first_asset_type), AssetAmount(second_asset_amount, second_asset_type)), timeout)
    ...
    def render_PUT(self, request):
        ...
        def on_ask_created(ask):
            if not request.finished:
                request.write(json.twisted_dumps({'assets': ask.assets.to_dictionary(), 
        ...
        self.get_market_community().create_ask(*BaseAsksBidsEndpoint.create_ask_bid_from_params(parameters)).addCallback(on_ask_created)
        return NOT_DONE_YET
...
class BidsEndpoint(BaseAsksBidsEndpoint):
    ...
    def render_PUT(self, request):
        ...
        def on_bid_created(bid):
            if not request.finished:
                request.write(json.twisted_dumps({'assets': bid.assets.to_dictionary(), 
        ...
        self.get_market_community().create_bid(*BaseAsksBidsEndpoint.create_ask_bid_from_params(parameters)).addCallback(on_bid_created)
        return NOT_DONE_YET