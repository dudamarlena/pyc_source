...
# [GCC 8.4.0]
# Embedded file name: /Users/martijndevos/Documents/anydex-core/anydex/core/clearing_policy.py
# Compiled at: 2019-06-08 16:30:14
...
class SingleTradeClearingPolicy(ClearingPolicy):
    ...
    def should_trade(self, trader_id):
            ...
            for block in blocks:
                if block.type == 'tx_init':
                    ...
                    if txid not in tx_status:
                        self.logger.warning('Found payment block without having tx_init block for transaction %s!', hexlify(txid))
                        continue
                    if block.link_sequence_number != 0:
                    ...
                    if txid not in tx_status:
                        self.logger.warning('Found tx_done block without having tx_init block for transaction %s!', hexlify(txid))
                        continue
                    tx_status[txid] = True
        ...
        if trader_id in self.currently_crawling:
            self.logger.info('Clearing policy not accepting trade with trader %s - we are already crawling this peer', trader_id.as_hex())
            returnValue(False)
