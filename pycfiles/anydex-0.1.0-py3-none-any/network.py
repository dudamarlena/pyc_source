...
# [GCC 8.4.0]
# Embedded file name: /Users/martijndevos/Documents/anydex-core/pyipv8/ipv8/peerdiscovery/network.py
...
class Network(object):
    ...
    def discover_address(self, peer, address, service=None):
        ...
        with self.graph_lock:
            if address not in self._all_addresses or self._all_addresses[address][0] not in [ p.mid for p in self.verified_peers ]:
        ...
        with self.graph_lock:
            ...
            for peer in self.verified_peers:
                if peer.address and peer.address != ('0.0.0.0', 0):
                    out += inet_aton(cast_to_chr(peer.address[0]) if isinstance(peer.address[0], bytes) else peer.address[0]) + pack('>H', peer.address[1])
