...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pyvb/vb.py
# Compiled at: 2008-04-30 13:42:21
"""
...
class VB:
    ...
    def listHdds(self, uuid=None, usage=True):
    ...
    def cloneVDI(self, hdd, path, vm=None):
        cmd = '%s %s "%s"' % (VB_COMMAND_CLONEVDI, hdd.getUUID(), path)
        command = VBCommand(command=cmd)
        command.run()
        if vm:
            cloned_hdd = self.registerVDI(path)
            self.attachVDI(vm, cloned_hdd)
            return cloned_hdd

    def registerVDI(self, path):
        cmd = '%s %s "%s"' % (VB_COMMAND_REGISTERIMAGE, 'disk', path)
        command = VBCommand(command=cmd)
        command.run()
        for i in self.listHdds(usage=False):
            if i.getPath() == path:
                return i