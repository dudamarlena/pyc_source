...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pyvb/hdd.py
# Compiled at: 2008-04-30 13:22:01
"""
...
class vbHddParser(vbParser):
    ...
    def __init__(self):
        ...
        self.attributes = {'uuid': VB_RE_UUID, 'storagetype': VB_RE_STORAGETYPE, 
           'path': VB_RE_PATH, 
    ...
    def parseSingle(self, file):
        """Intended usage of this method is to parse the output of a 
        single HDD.  We remove the "usage" attribute and parse the 
        output as usual.
        @param file: The file to parse.
        @type file: File/List
        @return: A list of L{pyvb.hdd.vbHdd} instances.
        ...
        hdds = []
        for result in self._parse(file):
            found_hdd = vbHdd(uuid=result['uuid'], storagetype=result['storagetype'], path=result['path'], accessible=result['accessible'])
            hdds.append(found_hdd)
