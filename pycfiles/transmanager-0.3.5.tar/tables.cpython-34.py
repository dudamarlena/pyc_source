...
# [GCC 8.4.0]
# Embedded file name: /var/www/transmanager/transmanager/tables.py
...
class TaskTable(tables.Table):
    ...
    class Meta:
        model = TransTask
        fields = ('user', 'language', 'object_name', 'object_pk', 'object_field_label',
                  'object_field_value', 'number_of_words', 'date_creation', 'date_modification',
                  'done', 'detail')
    ...
    def render_detail(self, record, value):
        ...
        if self.GET:
            return mark_safe('<a href="{0}?{1}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>'.format(url, self.GET.urlencode()))
        else:
            return mark_safe('<a href="{0}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> </a>'.format(url))