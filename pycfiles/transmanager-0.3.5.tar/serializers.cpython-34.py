...
# [GCC 8.4.0]
# Embedded file name: /var/www/transmanager/transmanager/serializers.py
...
class TaskBulksSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        ...
        try:
            ...
            for item in self.model_class.objects.language(manager.get_main_language()).filter(pk__in=self.ids).all():
                create_translations_for_item_and_its_children.delay(self.model_class, item.pk, self.languages, update_item_languages=True)
    ...
    def delete(self, **kwargs):
        ...
        try:
            manager = Manager()
            for item in self.model_class.objects.language(manager.get_main_language()).filter(pk__in=self.ids).all():
                delete_translations_for_item_and_its_children.delay(self.model_class, item.pk, self.languages, update_item_languages=True)
...
class TransTaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TransTask
        fields = ('user', 'language', 'object_class', 'object_pk', 'date_creation',
                  'date_modification', 'notified', 'date_notified', 'object_name',
