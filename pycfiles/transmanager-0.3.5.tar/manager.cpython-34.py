...
# [GCC 8.4.0]
# Embedded file name: /var/www/transmanager/transmanager/manager.py
# Compiled at: 2016-06-28 04:42:39
# Size of source mod 2**32: 27560 bytes
import logging
...
class Manager(object):
        ...
        for field in differences:
            ...
            if not value is None:
                if value == '':
                    continue
                try:
                    TransTask.objects.filter(language__code=lang, object_field=field, object_name=object_name, object_pk=object_pk).update(done=True, date_modification=datetime.now(), object_field_value_translation=value)
    ...
    def _get_translated_field_names(model_instance):
        ...
        hvad_internal_fields = [
         'id', 'language_code', 'master', 'master_id', 'master_id']
    ...
    def log(self, msg):
        ...
        if self.master_class and self.instance_class:
            logger.info('{0} - {1} - {2} - {3} - lang: {4} msg: {5}'.format(self.ct_master.app_label, self.ct_master.model, self.instance_class, self.instance.language_code, self.instance.pk, msg))
        else:
            ...
            for field in fields:
                ...
                if not object_field_value == '':
                    ...
                    for lang in languages:
                        ...
                        for user in users:
                            if self.bulk_mode:
                                if self.exists_destination_lang_value(item, field, lang):
                                    continue
                                ob_class_name = item.__class__.__name__
                                self.log('creating or updating object_class: {} | object_pk:{} | object_field: {}'.format(ob_class_name, item.pk, field))
                                app_label = item._meta.app_label
                                model = ob_class_name.lower()
                                ct = ContentType.objects.get_by_natural_key(app_label, model)
                                try:
                                    task, created = TransTask.objects.get_or_create(content_type=ct, object_class=ob_class_name, object_pk=item.pk, object_field=field, language=language, user=user, defaults={'object_name': '{} - {}'.format(app_label, item._meta.verbose_name), 
                                    ...
                                    tasks = TransTask.objects.filter(content_type=ct, object_class=ob_class_name, object_pk=item.pk, object_field=field, language=language, user=user)
    ...
    def update_model_languages(self, model_class, languages):
        ...
        if not new_langs:
            return
        mod_lan, created = TransModelLanguage.objects.get_or_create(model='{} - {}'.format(model_class._meta.app_label, model_class._meta.model.__name__.lower()))
    ...
    def delete_translations_for_item_and_its_children(self, item, languages=None):
        ...
        object_pk = item.pk
        filter_by = {'object_class': object_class, 
