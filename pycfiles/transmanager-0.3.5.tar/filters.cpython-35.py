...
# [GCC 8.4.0]
# Embedded file name: /var/www/transmanager/transmanager/filters/filters.py
# Compiled at: 2018-07-05 06:27:41
...
class TaskFilter(django_filters.FilterSet):
    ...
    usuario = django_filters.ModelChoiceFilter(queryset=TransUser.objects.all(), required=False, label=_('Usuario'), name='user')
    content_type = django_filters.ChoiceFilter(choices=(), required=False, label=_('Clase'))
    language = django_filters.ModelChoiceFilter(queryset=TransLanguage.objects.order_by('name'), required=False, label=_('Idioma'))
    record_status = django_filters.CharFilter(label=_('Estado'), widget=Select(choices=RECORD_STATUS), required=False, method='filter_record_status')
    record_types = django_filters.CharFilter(label=_('Tipo registro'), widget=Select(choices=RECORDS_TYPES), method='filter_record_types')
    ...
    class Meta:
        model = TransTask
        fields = ('usuario', 'language', 'content_type', 'record_types', 'record_status',
                  'object_pk', 'dates')
    ...
    def filter_record_status(queryset, name, value):
        if value == 'translated':
            return queryset.extra(where=[
             "object_field_value_translation IS NOT NULL AND object_field_value_translation !=''"])
        if value == 'not_translated':
            return queryset.extra(where=[
             "object_field_value_translation IS NULL OR object_field_value_translation =''"])
        return queryset
    ...
    def filter_record_types(queryset, name, value):
        if value == 'modified':
            return queryset.extra(where=[
             "to_char(date_creation, 'YY-MM-DD HH24:MI')!=to_char(date_modification, 'YY-MM-DD HH24:MI')"])
    ...
    def get_choices_for_models():
        ...
        qs = ContentType.objects.exclude(model__contains='translation').filter(app_label__in=apps).order_by('app_label', 'model')
        for ct in qs:
        ...
        choices = [
         (
          str(_('Todas las clases')), (('', _('Todas las clases')),))]
