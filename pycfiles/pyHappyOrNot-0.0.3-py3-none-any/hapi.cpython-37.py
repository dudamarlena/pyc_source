...
# Embedded file name: build\bdist.win-amd64\egg\pyhapi\hapi.py
# Compiled at: 2020-03-18 09:19:44
# Size of source mod 2**32: 71002 bytes
__doc__ = "Wrapper for HAPI's APIs.\nAuthor  : Maajor\nEmail   : info@ma-yidong.com\n"
from ctypes import cdll, POINTER, c_int, c_int32, c_int64, c_float, c_double, c_bool, byref, c_char_p, create_string_buffer
import asyncio
...
def cleanup(session):
    ...
    assert result == HDATA.Result.SUCCESS, 'Cleanup Failed with {0}'.format(HDATA.Result(result).name)
...
def close_session(session):
    ...
    assert result == HDATA.Result.SUCCESS, 'Close Session Failed with {0}'.format(HDATA.Result(result).name)
...
def start_thrift_named_pipe_server(server_options):
    ...
    """
    processid = c_int32()
    result = HAPI_LIB.HAPI_StartThriftNamedPipeServer(byref(server_options), c_char_p('hapi'.encode('utf-8')), byref(processid))
    assert result == HDATA.Result.SUCCESS, 'StartThriftNamedPipeServer Failed with {0}'.format(HDATA.Result(result).name)
...
def create_thrift_named_pipe_session(session):
    ...
    Args:
        session (int64): session id
    """
    result = HAPI_LIB.HAPI_CreateThriftNamedPipeSession(byref(session), c_char_p('hapi'.encode('utf-8')))
    assert result == HDATA.Result.SUCCESS, 'CreateThriftNamedPipeSession Failed with {0}'.format(HDATA.Result(result).name)


def initialize(session, cook_option, use_cooking_thread=True, cooking_thread_stack_size=-1, houdini_environment_files='', otl_search_path='', dso_search_path='', image_dso_search_path='', audio_dso_search_path=''):
    ...
    Args:
        ...
        use_cooking_thread (bool, optional): Use a separate thread for cooking of assets.             This allows for asynchronous cooking and larger stack size.. Defaults to True.
        cooking_thread_stack_size (int, optional): Set the stack size of the cooking thread.             Use -1 to set the stack size to the Houdini default. This value is in bytes.                  Defaults to -1.
        houdini_environment_files (str, optional): A list of paths,             separated by a ";" on Windows and a ":" on Linux and Mac,                  to .env files that follow the same syntax as the houdini.env file in Houdini's                       user prefs folder.These will be applied after the default houdini.env file                           and will overwrite the process' environment variable values. You an use                               this to enforce a stricter environment when running engine.                                   For more info, see:                                       http://www.sidefx.com/docs/houdini/basics/config_env.                                           Defaults to "".
        otl_search_path (str, optional): The directory where OTLs are searched for.             You can pass NULL here which will only use the default Houdini OTL search paths.                 You can also pass in multiple paths separated by a ";" on Windows and a ":"                     on Linux and Mac.                     If something other than NULL is passed the default Houdini search paths                         will be appended to the end of the path string.. Defaults to "".
        dso_search_path (str, optional): The directory where generic DSOs (custom plugins)             are searched for. You can pass NULL here which will only use the default Houdini                 DSO search paths.                 You can also pass in multiple paths separated by a ";" on Windows and a ":"                     on Linux and Mac.                     If something other than NULL is passed the default Houdini search paths                         will be appended to the end of the path string.. Defaults to "".
        image_dso_search_path (str, optional): The directory where image DSOs (custom plugins)             are searched for.             You can pass NULL here which will only use the default Houdini DSO search paths.                 You can also pass in multiple paths separated by a ";" on Windows and a ":"                     on Linux and Mac. If something other than NULL is passed the default Houdini                         search paths will be appended to the end of the path string..                             Defaults to "".
        audio_dso_search_path (str, optional): The directory where audio DSOs (custom plugins)            are searched for. You can pass NULL here which will only use the default Houdini                 DSO search paths. You can also pass in multiple paths separated by a ";"                     on Windows and a ":" on Linux and Mac. If something other than NULL                         is passed the default Houdini search paths will be appended to                             the end of the path string.. Defaults to "".
    """
    result = HAPI_LIB.HAPI_Initialize(byref(session), byref(cook_option), c_bool(use_cooking_thread), c_int32(cooking_thread_stack_size), c_char_p(houdini_environment_files.encode('utf-8')), c_char_p(otl_search_path.encode('utf-8')), c_char_p(dso_search_path.encode('utf-8')), c_char_p(image_dso_search_path.encode('utf-8')), c_char_p(audio_dso_search_path.encode('utf-8')))
    assert result in (HDATA.Result.SUCCESS,
     HDATA.Result.ALREADY_INITIALIZED), 'Initialize Failed with {0}'.format(HDATA.Result(result).name)
...
def load_asset_library_from_file(session, file_path, allow_overwrite=True):
    ...
    Args:
        ...
        allow_overwrite (bool, optional): With this true, if the library file being             loaded contains asset definitions that have already been loaded they will                 overwrite the existing definitions. . Defaults to True.

    Returns:
        int: Newly loaded otl id to be used
    """
    asset_lib_id = c_int32()
    result = HAPI_LIB.HAPI_LoadAssetLibraryFromFile(byref(session), c_char_p(file_path.encode('utf-8')), c_bool(allow_overwrite), byref(asset_lib_id))
    assert result == HDATA.Result.SUCCESS, 'LoadAssetLibraryFromFile Failed with {0}'.format(HDATA.Result(result).name)
    return asset_lib_id


def _get_available_asset_count(session, asset_lib_id):
    asset_count = c_int32()
    result = HAPI_LIB.HAPI_GetAvailableAssetCount(byref(session), asset_lib_id, byref(asset_count))
    assert result == HDATA.Result.SUCCESS, 'GetAvailableAssetCount Failed with {0}'.format(HDATA.Result(result).name)
    return asset_count
...
def get_available_assets(session, asset_lib_id):
    ...
    result = HAPI_LIB.HAPI_GetAvailableAssets(byref(session), asset_lib_id, byref(asset_string_buffer), asset_count)
    assert result == HDATA.Result.SUCCESS, 'GetAvailableAssets Failed with {0}'.format(HDATA.Result(result).name)
    asset_names = []
...
def create_input_node(session, node_label):
    ...
    Args:
        session (int64): The session of Houdini you are interacting with.
        node_label ( str): Give this input node a name for easy debugging.             The node's parent OBJ node and the Null SOP node will both get                 this given name with "input_" prepended. You can also pass                     NULL in which case the name will be "input#" where # is                     some number.

    Returns:
        int : Newly created node's id
    """
    node_id = c_int32()
    result = HAPI_LIB.HAPI_CreateInputNode(byref(session), byref(node_id), c_char_p(node_label.encode('utf-8')))
    assert result == HDATA.Result.SUCCESS, 'CreateInputNode Failed with {0}'.format(HDATA.Result(result).name)
...
def create_heightfield_input_node(session, node_label, xsize, ysize, voxel_size):
    ...
    Returns:
        int[4] : Newly created id for Heightfield node, for height volume            for mask volume and for merge node
    """
    heightfield_id = c_int32()
    height_id = c_int32()
    mask_id = c_int32()
    merge_id = c_int32()
    result = HAPI_LIB.HAPI_CreateHeightfieldInputNode(byref(session), -1, c_char_p(node_label.encode('utf-8')), xsize, ysize, voxel_size, byref(heightfield_id), byref(height_id), byref(mask_id), byref(merge_id))
    assert result == HDATA.Result.SUCCESS, 'CreateHeightfieldInputNode Failed with {0}'.format(HDATA.Result(result).name)
    return [
     heightfield_id.value, height_id.value, mask_id.value, merge_id.value]
...
def create_heightfield_volume_input_node(session, node_label, xsize, ysize, voxel_size):
    ...
    """
    node_id = c_int32()
    result = HAPI_LIB.HAPI_CreateHeightfieldInputVolumeNode(byref(session), -1, byref(node_id), c_char_p(node_label.encode('utf-8')), xsize, ysize, voxel_size)
    assert result == HDATA.Result.SUCCESS, 'CreateHeightfieldVolumeInputNode Failed with {0}'.format(HDATA.Result(result).name)
    return node_id.value
...
def create_node(session, operator_name, node_label='', parent_node_id=-1, cook_on_creation=False):
    ...
    """
    node_id = c_int32()
    result = HAPI_LIB.HAPI_CreateNode(byref(session), c_int(parent_node_id), c_char_p(operator_name.encode('utf-8')), c_char_p(node_label.encode('utf-8')), c_bool(cook_on_creation), byref(node_id))
    assert result == HDATA.Result.SUCCESS, 'CreateNode Failed with {0}'.format(HDATA.Result(result).name)
    return node_id.value
...
def delete_node(session, node_id):
    ...
    assert result == HDATA.Result.SUCCESS, 'DeleteNode Failed with {0}'.format(HDATA.Result(result).name)
...
def cook_node(session, cook_option, node_id):
    ...
    assert result == HDATA.Result.SUCCESS, 'CookNode Failed with {0}'.format(HDATA.Result(result).name)
...
async def cook_node_async(session, cook_option, node_id):
    ...
    """
    result = HAPI_LIB.HAPI_CookNode(byref(session), node_id, byref(cook_option))
    assert result == HDATA.Result.SUCCESS, 'CookNodeAsync Failed with {0}'.format(HDATA.Result(result).name)
...
def wait_cook(session, status_report_interval=1):
    ...
    Args:
        session (int64): The session of Houdini you are interacting with.
        status_report_interval (int, optional): time interval in seconds to query cook status.             Defaults to 1.
    """
    loop = asyncio.get_event_loop()
...
async def wait_cook_async(session, status_report_interval=1):
    ...
    while True:
        cook_result = HAPI_LIB.HAPI_GetStatus(byref(session), 2, byref(cook_status))
        continuestate = cook_status.value > HDATA.State.MAX_READY_STATE and cook_result == HDATA.Result.SUCCESS
        print('Cook Status at {0} : {1}'.format(datetime.now().strftime('%H:%M:%S'), _get_status_string(session, HDATA.StatusType.COOK_STATE, HDATA.StatusVerbosity.MESSAGES)))
    ...
    if not (cook_result == HDATA.Result.SUCCESS and cook_status.value == HDATA.State.READY):
        raise AssertionError('CookNode Failed with {0} and Cook Status is {1}'.format(HDATA.Result(cook_result).name, HDATA.State(cook_status.value).name))
...
def query_node_input(session, node_id, input_index=0):
    ...
    result = HAPI_LIB.HAPI_QueryNodeInput(byref(session), c_int(node_id), c_int(input_index), byref(connect_node_id))
    assert result == HDATA.Result.SUCCESS, 'QueryNodeInput Failed with {0}'.format(HDATA.Result(result).name)
...
def connect_node_input(session, node_id, node_id_to_connect, input_index=0, output_index=0):
    ...
    """
    result = HAPI_LIB.HAPI_ConnectNodeInput(byref(session), c_int(node_id), c_int(input_index), c_int(node_id_to_connect), c_int(output_index))
    assert result == HDATA.Result.SUCCESS, 'ConnectNodeInput Failed with {0}'.format(HDATA.Result(result).name)
...
def disconnect_node_input(session, node_id, input_index=0):
    ...
    assert result == HDATA.Result.SUCCESS, 'DisconnectNodeInput Failed with {0}'.format(HDATA.Result(result).name)
...
def get_composed_child_node_list(session, node_id, count):
    ...
    result = HAPI_LIB.HAPI_GetComposedChildNodeList(byref(session), node_id, byref(id_buffer), c_int(count))
    assert result == HDATA.Result.SUCCESS, 'GetComposedChildNodeList Failed with {0}'.format(HDATA.Result(result).name)
    return id_buffer


def compose_child_node_list(session, node_id, node_type=HDATA.NodeType.ANY, node_flag=HDATA.NodeFlags.ANY):
    ...
    Args:
        session (int): The session of Houdini you are interacting with.
        node_id (int): The node to get.
        node_type (NodeType, optional): The node type by which to filter the children.             Defaults to HDATA.NodeType.ANY.
        node_flag (NodeFlags, optional): The node flags by which to filter the children.             Defaults to HDATA.NodeFlags.ANY.
    """
    child_count = c_int32()
    result = HAPI_LIB.HAPI_ComposeChildNodeList(byref(session), node_id, c_int(node_type), c_int(node_flag), c_bool(False), byref(child_count))
    assert result == HDATA.Result.SUCCESS, 'ComposeChildNodeList Failed with {0}'.format(HDATA.Result(result).name)
...
def get_geo_info(session, node_id):
    ...
    """
    geo_info = HDATA.GeoInfo()
    result = HAPI_LIB.HAPI_GetGeoInfo(byref(session), node_id, byref(geo_info))
    assert result == HDATA.Result.SUCCESS, 'GetGeoInfo Failed with {0}'.format(HDATA.Result(result).name)
    return geo_info
...
def get_part_info(session, node_id, part_id=0):
    ...
    assert result == HDATA.Result.SUCCESS, 'GetPartInfo Failed with {0}'.format(HDATA.Result(result).name)
    return part_info
...
def set_volume_info(session, node_id, part_id, volume_info):
    ...
    """
    result = HAPI_LIB.HAPI_SetVolumeInfo(byref(session), node_id, part_id, byref(volume_info))
    assert result == HDATA.Result.SUCCESS, 'SetVolume Failed with {0}'.format(HDATA.Result(result).name)
...
def get_volume_info(session, node_id, part_id):
    ...
    result = HAPI_LIB.HAPI_GetVolumeInfo(byref(session), node_id, part_id, byref(volume_info))
    assert result == HDATA.Result.SUCCESS, 'GetVolumeInfo Failed with {0}'.format(HDATA.Result(result).name)
    return volume_info
...
def get_first_volume_tile(session, node_id, part_id=0):
    ...
    result = HAPI_LIB.HAPI_GetFirstVolumeTile(byref(session), node_id, part_id, byref(volume_tile_info))
    assert result == HDATA.Result.SUCCESS, 'GetFirstVolumeTile Failed with {0}'.format(HDATA.Result(result).name)
    return volume_tile_info


def get_next_volume_tile(session, node_id, part_id=0):
    """Wrapper for HAPI_GetNextVolumeTile
    terate through a volume based on 8x8x8 sections of the volume     Continue iterating through the value of the volume at part_id
    ...
    """
    volume_tile_info = HDATA.VolumeTileInfo()
    result = HAPI_LIB.HAPI_GetNextVolumeTile(byref(session), node_id, part_id, byref(volume_tile_info))
    assert result == HDATA.Result.SUCCESS, 'GetFirstVolumeTile Failed with {0}'.format(HDATA.Result(result).name)
    return volume_tile_info
...
def get_composed_object_list(session, node_id, count):
    ...
    result = HAPI_LIB.HAPI_GetComposedObjectList(byref(session), node_id, byref(object_info_buffer), c_int(0), c_int(count))
    assert result == HDATA.Result.SUCCESS, 'GetComposedObjectList Failed with {0}'.format(HDATA.Result(result).name)
...
def compose_object_list(session, node_id):
    ...
    """
    child_count = c_int32()
    result = HAPI_LIB.HAPI_ComposeObjectList(byref(session), node_id, None, byref(child_count))
    assert result == HDATA.Result.SUCCESS, 'ComposeObjectList Failed with {0}'.format(HDATA.Result(result).name)
    return child_count.value
...
def get_node_info(session, node_id):
    ...
    assert result == HDATA.Result.SUCCESS, 'GetNodeInfo Failed with {0}'.format(HDATA.Result(result).name)
    return node_info
...
def get_asset_info(session, node_id):
    ...
    assert result == HDATA.Result.SUCCESS, 'GetAssetInfo Failed with {0}'.format(HDATA.Result(result).name)
    return asset_info
...
def get_parameters(session, node_id, node_info):
    ...
    result = HAPI_LIB.HAPI_GetParameters(byref(session), node_id, byref(params), c_int32(0), c_int32(node_info.parmCount))
    assert result == HDATA.Result.SUCCESS, 'GetParameters Failed with {0}'.format(HDATA.Result(result).name)
    return params
...
def get_parm_int_value(session, node_id, parmname, tupleid=0):
    ...
    """
    val = c_int32()
    result = HAPI_LIB.HAPI_GetParmIntValue(byref(session), node_id, c_char_p(parmname.encode('utf-8')), tupleid, byref(val))
    assert result == HDATA.Result.SUCCESS, 'GetParmIntValue Failed with {0}'.format(HDATA.Result(result).name)
    return val.value
...
def get_parm_float_value(session, node_id, parmname, tupleid=0):
    ...
    """
    val = c_float()
    result = HAPI_LIB.HAPI_GetParmFloatValue(byref(session), node_id, c_char_p(parmname.encode('utf-8')), tupleid, byref(val))
    assert result == HDATA.Result.SUCCESS, 'GetParmFloatValue Failed with {0}'.format(HDATA.Result(result).name)
    return val.value
...
def get_parm_string_value(session, node_id, parmname, tupleid=0):
    ...
    """
    stringsh = c_int32()
    result = HAPI_LIB.HAPI_GetParmStringValue(byref(session), node_id, c_char_p(parmname.encode('utf-8')), tupleid, True, byref(stringsh))
    assert result == HDATA.Result.SUCCESS, 'GetParamStringValue Failed with {0}'.format(HDATA.Result(result).name)
...
def set_parm_int_value(session, node_id, parmname, value, tupleid=0):
    ...
    Args:
        ...
        tupleid (int, optional): Index within the parameter's values tuple.. Defaults to 0.
    """
    result = HAPI_LIB.HAPI_SetParmIntValue(byref(session), node_id, c_char_p(parmname.encode('utf-8')), tupleid, value)
    assert result == HDATA.Result.SUCCESS, 'SetParmIntValue Failed with {0}'.format(HDATA.Result(result).name)
...
def set_parm_float_value(session, node_id, parmname, value, tupleid=0):
    ...
    Args:
        ...
        tupleid (int, optional): Index within the parameter's values tuple.. Defaults to 0.
    """
    result = HAPI_LIB.HAPI_SetParmFloatValue(byref(session), node_id, c_char_p(parmname.encode('utf-8')), c_int(tupleid), c_float(value))
    assert result == HDATA.Result.SUCCESS, 'SetParmFloatValue Failed with {0}'.format(HDATA.Result(result).name)
...
def set_parm_string_value(session, node_id, parmid, value, tupleid=0):
    ...
    Args:
        ...
        tupleid (int, optional): Index within the parameter's values tuple.. Defaults to 0.
    """
    result = HAPI_LIB.HAPI_SetParmStringValue(byref(session), node_id, c_char_p(value.encode('utf-8')), parmid, tupleid)
    assert result == HDATA.Result.SUCCESS, 'SetParamStringValue Failed with {0}'.format(HDATA.Result(result).name)
...
def set_part_info(session, node_id, part_info):
    ...
    assert result == HDATA.Result.SUCCESS, 'SetPartInfo Failed with {0}'.format(HDATA.Result(result).name)
...
def set_curve_info(session, node_id, curve_info):
    ...
    assert result == HDATA.Result.SUCCESS, 'SetCurveInfo Failed with {0}'.format(HDATA.Result(result).name)
...
def set_curve_counts(session, node_id, part_id, curve_count):
    ...
    """
    intp = POINTER(c_int)
    result = HAPI_LIB.HAPI_SetCurveCounts(byref(session), node_id, part_id, curve_count.flatten().ctypes.data_as(intp), 0, curve_count.shape[0])
    assert result == HDATA.Result.SUCCESS, 'SetCurveCounts Failed with {0}'.format(HDATA.Result(result).name)
...
def set_curve_knots(session, node_id, part_id, curve_knots):
    ...
    if not isinstance(curve_knots, np.ndarray):
        return
    intp = POINTER(c_int)
    result = HAPI_LIB.HAPI_SetCurveKnots(byref(session), node_id, part_id, curve_knots.flatten().ctypes.data_as(intp), 0, curve_knots.shape[0])
    assert result == HDATA.Result.SUCCESS, 'SetCurveKnots Failed with {0}'.format(HDATA.Result(result).name)
...
def get_curve_info(session, node_id, part_id):
    ...
    result = HAPI_LIB.HAPI_GetCurveInfo(byref(session), node_id, part_id, byref(curve_info))
    assert result == HDATA.Result.SUCCESS, 'GetCurveInfo Failed with {0}'.format(HDATA.Result(result).name)
    return curve_info
...
def get_curve_counts(session, node_id, part_id, curve_count):
    ...
    result = HAPI_LIB.HAPI_GetCurveCounts(byref(session), node_id, part_id, byref(data_buffer), 0, curve_count)
    assert result == HDATA.Result.SUCCESS, 'GetCurveCounts Failed with {0}'.format(HDATA.Result(result).name)
...
def get_curve_knots(session, node_id, part_id, knot_length):
    ...
    result = HAPI_LIB.HAPI_GetCurveKnots(byref(session), node_id, part_id, byref(data_buffer), 0, knot_length)
    assert result == HDATA.Result.SUCCESS, 'GetCurveKnots Failed with {0}'.format(HDATA.Result(result).name)
...
def add_attribute(session, node_id, name, attrib_info):
    ...
    """
    result = HAPI_LIB.HAPI_AddAttribute(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info))
    assert result == HDATA.Result.SUCCESS, 'AddAttribute Failed with {0}'.format(HDATA.Result(result).name)
...
def set_attribute_float_data(session, node_id, name, attrib_info, data):
    ...
    result = HAPI_LIB.HAPI_SetAttributeFloatData(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info), data.flatten().ctypes.data_as(floatp), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'SetAttributeFloatData Failed with {0}'.format(HDATA.Result(result).name)
...
def set_attribute_float64_data(session, node_id, name, attrib_info, data):
    ...
    result = HAPI_LIB.HAPI_SetAttributeFloat64Data(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info), data.flatten().ctypes.data_as(floatp), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'SetAttributeFloatData Failed with {0}'.format(HDATA.Result(result).name)
...
def set_attribute_int64_data(session, node_id, name, attrib_info, data):
    ...
    result = HAPI_LIB.HAPI_SetAttributeInt64Data(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info), data.flatten().ctypes.data_as(intp), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'SetAttributeFloatData Failed with {0}'.format(HDATA.Result(result).name)
...
def set_attribute_int_data(session, node_id, name, attrib_info, data):
    ...
    """
    intp = POINTER(c_int)
    result = HAPI_LIB.HAPI_SetAttributeIntData(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info), data.flatten().ctypes.data_as(intp), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'SetAttributeFloatData Failed with {0}'.format(HDATA.Result(result).name)
...
def set_attribute_string_data(session, node_id, name, attrib_info, data):
    ...
    result = HAPI_LIB.HAPI_SetAttributeStringData(byref(session), node_id, 0, c_char_p(name.encode('utf-8')), byref(attrib_info), data.flatten().ctypes.data_as(charp), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'SetAttributeFloatData Failed with {0}'.format(HDATA.Result(result).name)


STORAGE_TYPE_TO_SET_ATTRIB = {HDATA.StorageType.INT: set_attribute_int_data, 
...
def get_attribute_names(session, node_id, part_info, attrib_type=HDATA.AttributeOwner.POINT):
    ...
    Args:
        ...
        attrib_type (AttributeOwner, optional): Type of attribute.             Defaults to HDATA.AttributeOwner.POINT.
    """
    attrib_names = []
    ...
    result = HAPI_LIB.HAPI_GetAttributeNames(byref(session), node_id, part_info.id, attrib_type, byref(string_handle_buffer), attrib_count)
    assert result == HDATA.Result.SUCCESS, 'GetAttributeNames Failed with {0}'.format(HDATA.Result(result).name)
    attrib_names = []
...
def get_attribute_info(session, node_id, part_id, name, attrib_type):
    ...
    result = HAPI_LIB.HAPI_GetAttributeInfo(byref(session), c_int(node_id), c_int(part_id), c_char_p(name.encode('utf-8')), attrib_type, byref(attrib_info))
...
def get_attribute_int_data(session, node_id, part_id, name, attrib_info):
    ...
    result = HAPI_LIB.HAPI_GetAttributeIntData(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), byref(attrib_info), -1, byref(data_buffer), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'GetAttributeIntData Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.int32)
...
def get_attribute_int64_data(session, node_id, part_id, name, attrib_info):
    ...
    result = HAPI_LIB.HAPI_GetAttributeInt64Data(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), byref(attrib_info), -1, byref(data_buffer), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'GetAttributeInt64Data Failed with {0}'.format(HDATA.Result(result).name)
...
def get_attribute_float_data(session, node_id, part_id, name, attrib_info):
    ...
    result = HAPI_LIB.HAPI_GetAttributeFloatData(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), byref(attrib_info), -1, byref(data_buffer), 0, attrib_info.count)
...
def get_attribute_float64_data(session, node_id, part_id, name, attrib_info):
    ...
    result = HAPI_LIB.HAPI_GetAttributeFloat64Data(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), byref(attrib_info), -1, byref(data_buffer), 0, attrib_info.count)
    assert result == HDATA.Result.SUCCESS, 'GetAttributeFloat64Data Failed with {0}'.format(HDATA.Result(result).name)
...
def get_attribute_string_data(session, node_id, part_id, name, attrib_info):
    ...
    result = HAPI_LIB.HAPI_GetAttributeStringData(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), byref(attrib_info), -1, byref(data_buffer), 0, attrib_info.count)
...
STORAGE_TYPE_TO_GET_ATTRIB = {HDATA.StorageType.INT: get_attribute_int_data, 
...
def set_vertex_list(session, node_id, vertex_list_array):
    ...
    """
    intp = POINTER(c_int)
    result = HAPI_LIB.HAPI_SetVertexList(byref(session), node_id, 0, vertex_list_array.flatten().ctypes.data_as(intp), 0, np.size(vertex_list_array))
    assert result == HDATA.Result.SUCCESS, 'SetVertexList Failed with {0}'.format(HDATA.Result(result).name)
...
def set_face_counts(session, node_id, face_counts_array):
    ...
    """
    intp = POINTER(c_int)
    result = HAPI_LIB.HAPI_SetFaceCounts(byref(session), node_id, 0, face_counts_array.ctypes.data_as(intp), 0, face_counts_array.shape[0])
    assert result == HDATA.Result.SUCCESS, 'SetFaceCounts Failed with {0}'.format(HDATA.Result(result).name)
...
def get_vertex_list(session, node_id, part_info):
    ...
    result = HAPI_LIB.HAPI_GetVertexList(byref(session), node_id, part_info.id, byref(data_buffer), 0, part_info.vertexCount)
    assert result == HDATA.Result.SUCCESS, 'GetVertexList Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.int32)
    return data_np
...
def get_face_counts(session, node_id, part_info):
    ...
    result = HAPI_LIB.HAPI_GetFaceCounts(byref(session), node_id, part_info.id, byref(data_buffer), 0, part_info.faceCount)
    assert result == HDATA.Result.SUCCESS, 'GetFaceCounts Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.int32)
    return data_np
...
def set_heightfield_data(session, node_id, part_id, name, data_array):
    ...
    Args:
        ...
        node_id (int): The node to get.
        part_id (int): The part id
        name (str): The name of the volume used for the heightfield.             If set to "height" the values will be used for height information,                 if not, the data will used as a mask.
        data_array (np.ndarray): volume info of this node

    """
    floatp = POINTER(c_float)
    result = HAPI_LIB.HAPI_SetHeightFieldData(byref(session), node_id, part_id, c_char_p(name.encode('utf-8')), data_array.flatten().ctypes.data_as(floatp), 0, np.size(data_array))
    assert result == HDATA.Result.SUCCESS, 'SetHeightfieldData Failed with {0}'.format(HDATA.Result(result).name)
...
def get_heightfield_data(session, node_id, part_id, volume_info):
    ...
    result = HAPI_LIB.HAPI_GetHeightFieldData(byref(session), node_id, part_id, byref(data_buffer), 0, data_size)
    assert result == HDATA.Result.SUCCESS, 'GetHeightfieldData Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.float32)
...
def get_volume_tile_float_data(session, node_id, part_id, volume_tile_info, tuple_size):
    ...
    """
    data_size = 512 * tuple_size
    data_buffer = (c_float * data_size)()
    result = HAPI_LIB.HAPI_GetVolumeTileFloatData(byref(session), node_id, part_id, c_float(-8.8), byref(volume_tile_info), byref(data_buffer), data_size)
    assert result == HDATA.Result.SUCCESS, 'GetVolumeTileFloatData Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.float32)
    return data_np
...
def get_volume_tile_int_data(session, node_id, part_id, volume_tile_info, tuple_size):
    ...
    result = HAPI_LIB.HAPI_GetVolumeTileIntData(byref(session), node_id, part_id, c_int(-8), byref(volume_tile_info), byref(data_buffer), data_size)
    assert result == HDATA.Result.SUCCESS, 'GetVolumeTileIntData Failed with {0}'.format(HDATA.Result(result).name)
    data_np = np.frombuffer(data_buffer, np.int32)
    return data_np
...
def set_volume_tile_float_data(session, node_id, part_id, volume_tile_info, data_array, tuple_size):
    ...
    floatp = POINTER(c_float)
    data_size = 512 * tuple_size
    result = HAPI_LIB.HAPI_SetVolumeTileFloatData(byref(session), node_id, part_id, byref(volume_tile_info), data_array.flatten().ctypes.data_as(floatp), data_size)
    assert result == HDATA.Result.SUCCESS, 'SetVolumeTileIntData Failed with {0}'.format(HDATA.Result(result).name)
...
def set_volume_tile_int_data(session, node_id, part_id, volume_tile_info, data_array, tuple_size):
    ...
    intp = POINTER(c_int)
    data_size = 512 * tuple_size
    result = HAPI_LIB.HAPI_SetVolumeTileIntData(byref(session), node_id, part_id, byref(volume_tile_info), data_array.flatten().ctypes.data_as(intp), data_size)
    assert result == HDATA.Result.SUCCESS, 'SetVolumeTileIntData Failed with {0}'.format(HDATA.Result(result).name)
...
def commit_geo(session, node_id):
    ...
    assert result == HDATA.Result.SUCCESS, 'CommitGeo Failed with {0}'.format(HDATA.Result(result).name)
...
def load_hip_file(session, hipname, cook_on_load):
    ...
    """
    result = HAPI_LIB.HAPI_LoadHIPFile(byref(session), c_char_p(hipname.encode('utf-8')), c_bool(cook_on_load))
    assert result == HDATA.Result.SUCCESS, 'LoadHipFile Failed with {0}'.format(HDATA.Result(result).name)
...
def _get_string_buf_length(session, string_handle):
    ...
    assert result == HDATA.Result.SUCCESS, 'GetStringBufLength Failed with {0}'.format(HDATA.Result(result).name)
...
def _get_string(session, string_handle, length):
    ...
    assert result == HDATA.Result.SUCCESS, 'GetString Failed with {0}'.format(HDATA.Result(result).name)
...
def _get_status_string(session, status=HDATA.StatusType.COOK_RESULT, verbosity=HDATA.StatusVerbosity.ERRORS):
    """Get current status string
    Wrapper for HAPI_GetStatusString and HAPI_GetStatusStringBufLength

    Args:
        session (int64): The session of Houdini you are interacting with.
        status (StatusType, optional): StatusType querying.             Defaults to HDATA.StatusType.COOK_RESULT.
        verbosity (StatusVerbosity, optional): Preferred verbosity level.             Defaults to HDATA.StatusVerbosity.ERRORS.

    Returns:
        str: status string
    """
    buffer_length = c_int32()
    result = HAPI_LIB.HAPI_GetStatusStringBufLength(byref(session), status, verbosity, byref(buffer_length))
    assert result == HDATA.Result.SUCCESS, 'GetStatusStringBufLength Failed with {0}'.format(HDATA.Result(result).name)
    buffers = create_string_buffer(buffer_length.value)
    result = HAPI_LIB.HAPI_GetStatusString(byref(session), status, buffers, buffer_length)
    assert result == HDATA.Result.SUCCESS, 'GetStatusString Failed with {0}'.format(HDATA.Result(result).name)
    return buffers.value.decode()