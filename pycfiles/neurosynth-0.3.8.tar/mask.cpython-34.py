...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/base/mask.py
# Compiled at: 2015-08-31 13:52:47
# Size of source mod 2**32: 9232 bytes
import numpy as np, nibabel as nb
from six import string_types

class Masker(object):
    __doc__ = ' Handles vectorization/masking/unmasking of images. '
    ...
    def add(self, layers, above=None, below=None):
        ...
        def add_named_layer(name, image):
            ...
            if above is not None:
                image[image < above] = 0.0
            if below is not None:
                image[image > below] = 0.0
    ...
    def get_image(self, image, output='vector'):
        ...
        if not type(image).__module__.startswith('numpy'):
            raise ValueError('Input image must be a string, a NiBabel image, or a numpy array.')
    ...
    def get_mask(self, layers=None, output='vector', in_global_mask=True):
        ...
        if layers is None:
            ...
            layers = [
             layers]
        layers = map(--- This code section failed: ---

 L. 213         0  LOAD_GLOBAL              isinstance
                3  LOAD_FAST                'x'
                6  LOAD_GLOBAL              string_types
