...
def run_lda(abstracts, n_topics=50, n_words=31, n_iters=1000, alpha=None, beta=0.001):
    ...
    import_str = '{mallet} import-dir --input {absdir} --output {outdir}/topic-input.mallet --keep-sequence --remove-stopwords'.format(mallet=mallet_bin,
      absdir=absdir,
      outdir=tempdir)
    train_str = '{mallet} train-topics --input {out}/topic-input.mallet --num-topics {n_topics} --num-top-words {n_words} --output-topic-keys {out}/topic_keys.txt --output-doc-topics {out}/doc_topics.txt --num-iterations {n_iters} --output-model {out}/saved_model.mallet --random-seed 1 --alpha {alpha} --beta {beta}'.format(mallet=mallet_bin,
      out=tempdir,
      n_topics=n_topics,
      n_words=n_words,
      n_iters=n_iters,
    ...
    n_cols = 2 * n_topics + 1
    dt_df = pd.read_csv((os.path.join(tempdir, 'doc_topics.txt')), delimiter='\t',
      skiprows=1,
      header=None,
      index_col=0)
    ...
    keys_df = pd.read_csv((os.path.join(tempdir, 'topic_keys.txt')), delimiter='\t',
      header=None,
      index_col=0)
    keys_df = keys_df[[2]]
