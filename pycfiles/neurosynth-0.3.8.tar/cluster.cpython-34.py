...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/analysis/cluster.py
# Compiled at: 2015-09-01 11:10:27
# Size of source mod 2**32: 10360 bytes
...
class Clusterable(object):
    ...
    def __init__(self, dataset, mask=None, features=None, feature_threshold=None, min_voxels=None, min_studies=None):
        self.dataset = dataset
        self.masker = deepcopy(dataset.masker)
        if features is not None:
            ids = dataset.get_studies(features=features, frequency_threshold=feature_threshold)
...
def magic(dataset, method='coactivation', roi_mask=None, coactivation_mask=None, features=None, feature_threshold=0.05, min_voxels_per_study=None, min_studies_per_voxel=None, reduce_reference='pca', n_components=100, distance_metric='correlation', clustering_algorithm='kmeans', n_clusters=5, clustering_kwargs={}, output_dir=None, filename=None, coactivation_images=False, coactivation_threshold=0.1):
    ...
    """
    roi = Clusterable(dataset, roi_mask, min_voxels=min_voxels_per_study, min_studies=min_studies_per_voxel, features=features, feature_threshold=feature_threshold)
    if method == 'coactivation':
        reference = Clusterable(dataset, coactivation_mask, min_voxels=min_voxels_per_study, min_studies=min_studies_per_voxel, features=features, feature_threshold=feature_threshold)
    else:
        if method == 'features':
            ...
            if reduce_reference is not None:
                if isinstance(reduce_reference, string_types):
                    reduce_reference = {'pca': sk_decomp.RandomizedPCA,  'ica': sk_decomp.FastICA}[reduce_reference](n_components)
                transpose = method == 'coactivation'
                reference = reference.transform(reduce_reference, transpose=transpose)
            if method == 'coactivation':
                distances = pairwise_distances(roi.data, reference.data, metric=distance_metric)
            else:
                distances = reference.data
        if isinstance(clustering_algorithm, string_types):
            clustering_algorithm = {'kmeans': sk_cluster.KMeans,  'minik': sk_cluster.MiniBatchKMeans}[clustering_algorithm](n_clusters, **clustering_kwargs)
        labels = clustering_algorithm.fit_predict(distances) + 1.0
        ...
        if output_dir is not None:
            ...
            if coactivation_images:
                for l in np.unique(voxel_labels):
                    ...
                    ids = dataset.get_studies(mask=roi_mask, activation_threshold=coactivation_threshold)
                    ma = meta.MetaAnalysis(dataset, ids)
                    ma.save_results(output_dir=join(output_dir, 'coactivation'), prefix='cluster_%d_coactivation' % l)
