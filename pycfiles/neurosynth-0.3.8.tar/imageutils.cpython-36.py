...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/base/imageutils.py
# Compiled at: 2018-05-04 12:26:17
# Size of source mod 2**32: 6040 bytes
""" Miscellaneous image-related utility functions. """
import logging, nibabel as nb
...
def get_sphere(coords, r=4, vox_dims=(2, 2, 2), dims=(91, 109, 91)):
    ...
    sphere = cube[:,
     np.sum(np.dot(np.diag(vox_dims), cube) ** 2, 0) ** 0.5 <= r]
    sphere = np.round(sphere.T + coords)
    return sphere[
     (np.min(sphere, 1) >= 0) & (np.max(np.subtract(sphere, dims), 1) <= -1), :].astype(int)
