...
# Compiled at: 2018-07-10 12:15:49
...
class Parser(object):
    ...
    def p_list_and(self, p):
        ...
        p[0] = pd.concat([
         p[1], p[3]],
          axis=1).dropna().apply((self.func), axis=1)

    def p_list_or(self, p):
        """list : list OR list"""
        p[0] = pd.concat([
         p[1], p[3]],
          axis=1).fillna(0.0).apply((self.func), axis=1)
    ...
    def p_list_feature(self, p):
        """list : feature
            | WORD """
        p[0] = self.dataset.get_studies(features=(p[1]),
          frequency_threshold=(self.threshold),
          func=(self.func),
          return_type='weights')
