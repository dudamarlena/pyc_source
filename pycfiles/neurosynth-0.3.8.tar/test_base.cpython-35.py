...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/tests/test_base.py
# Compiled at: 2015-12-24 09:36:13
# Size of source mod 2**32: 14560 bytes
import unittest, numpy as np, pandas as pd, tempfile, os, shutil
...
class TestBase(unittest.TestCase):
    ...
    def test_dataset_initializes(self):
        ...
        self.assertIsNotNone(self.dataset.activations['extra_field'].iloc[2], 'field')
    ...
    def test_feature_addition(self):
        ...
        new_data = pd.DataFrame(np.array([
         ...
         [
          0.02, 0.0008, 0.0003],
         [
          0.001, 0.002, 0.05]]), index=[
         'study1', 'study4', 'study6'], columns=['g1', 'g2', 'g3'])
    ...
    def test_selection_by_features(self):
        ...
        img_data = d.get_studies(features=[
         'f1', 'f3', 'g1'], activation_threshold=0.001, func=np.max, return_type='data')
    ...
    def test_selection_by_expression(self):
        ...
        if have_ply:
            ids = self.dataset.get_studies(expression='* &~ (g*)', func=np.sum, frequency_threshold=0.003)
            self.assertEqual(sorted(ids), ['study3', 'study5'])
            ids = self.dataset.get_studies(expression='f* > 0.005', func=np.mean, frequency_threshold=0.0)
            self.assertEqual(ids, ['study3'])
            ids = self.dataset.get_studies(expression='f* < 0.05', func=np.sum, frequency_threshold=0.0)
            self.assertEqual(sorted(ids), ['study1', 'study2', 'study3',
             'study4', 'study5'])
            ids = self.dataset.get_studies(expression='f* | g*', func=np.mean, frequency_threshold=0.003)
            self.assertEqual(sorted(ids), ['study1', 'study2', 'study3',
             'study4'])
            ids = self.dataset.get_studies(expression='(f* & g*)', func=np.sum, frequency_threshold=0.001)
            self.assertEqual(sorted(ids), ['study1', 'study4'])
            self.dataset.feature_table.data.columns = [
             'f1', 'f2', 'my ngram',
             'my ngram reprise', 'g1']
            ids = self.dataset.get_studies(expression='my ngram reprise')
            self.assertEqual(ids, ['study5'])
            ids = self.dataset.get_studies(expression='my ngram*', frequency_threshold=0.01)
    ...
    def test_selection_by_mask(self):
        ...
        ids = self.dataset.get_studies(mask=get_test_data_path() + 'sgacc_mask.nii.gz')
    ...
    def test_selection_by_multiple_criteria(self):
        ids = self.dataset.get_studies(peaks=[
         [
          3, 30, -9]], r=40, expression='f* < 0.013', func=np.sum, frequency_threshold=0.0)
    ...
    def test_unmask(self):
        ...
        ids = dataset.get_studies(mask=get_test_data_path() + 'sgacc_mask.nii.gz')
    ...
    def test_get_feature_counts(self):
        feature_counts = self.dataset.get_feature_counts(threshold=1.0)
        self.assertEqual(feature_counts, dict((f, 0) for f in self.dataset.get_feature_names()))
        ...
        feature_counts_ = dict([(feature, np.sum(self.dataset.feature_table.data.to_dense().ix[:, col] > 0.001)) for col, feature in enumerate(self.dataset.feature_table.feature_names)])
        self.assertEqual(feature_counts, feature_counts_)

    def test_get_feature_data(self):
        """ Test retrieval of Mappable x feature data. """
        feature_data = self.dataset.get_feature_data(features=[
         'f1', 'f4', 'g1'])
        self.assertEqual(feature_data.shape, (5, 3))
        feature_data = self.dataset.get_feature_data(ids=[
         'study3', 'study1'], dense=False)
    ...
    def test_get_image_data(self):
        ...
        image_data = self.dataset.get_image_data(ids=[
         'study1', 'study2', 'study5'], dense=True)
        self.assertEquals(image_data.shape, (228453, 3))
        image_data = self.dataset.image_table.get_image_data(ids=[
         'study1', 'study2', 'study5'], dense=False)
        self.assertEquals(image_data.shape, (228453, 3))
    ...
    def test_get_features_by_ids(self):
        ...
        features = self.dataset.feature_table.get_features_by_ids(['study2', 'study5'], func=np.sum, threshold=0.0, get_weights=True)
