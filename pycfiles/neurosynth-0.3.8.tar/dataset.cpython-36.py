...
# Size of source mod 2**32: 33537 bytes
...
class Dataset(object):
    ...
    def _load_activations(self, filename):
        ...
        if set(mc) - set(list(activations.columns)):
            ...
            ijk = pd.DataFrame((transformations.xyz_to_mat(xyz)),
              columns=['i', 'j', 'k'])
    ...
    def get_studies(self, features=None, expression=None, mask=None, peaks=None, frequency_threshold=0.001, activation_threshold=0.0, func=np.sum, return_type='ids', r=6):
        ...
        if features is not None:
            if return_type == 'weights':
                ...
                return self.feature_table.get_ids(features,
                  frequency_threshold, func, get_weights=True)
...
class ImageTable(object):
    ...
    def __init__(self, dataset, r=6, use_sparse=True):
        ...
        for i, (name, data) in enumerate(activations.groupby('id')):
            ...
            img = imageutils.map_peaks_to_image((data[['i', 'j', 'k']].values),
              r=r, header=(self.masker.get_header()))
...
class FeatureTable(object):
    __doc__ = ' A FeatureTable instance stores a matrix of studies x features,\n    along with associated manipulation methods. '
    ...
    def add_features(self, features, merge='outer', duplicates='ignore', min_studies=0, threshold=0.0001):
        ...
        data = old_data.merge(features,
          how=merge, left_index=True, right_index=True)
    ...
    def get_ids_by_expression(self, expression, threshold=0.001, func=np.sum):
        ...
        lexer = lp.Lexer()
        lexer.build()
        parser = lp.Parser(lexer,
          (self.dataset), threshold=threshold, func=func)
        parser.build()
