...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/base/dataset.py
# Compiled at: 2015-08-31 19:38:44
# Size of source mod 2**32: 39721 bytes
""" Base classes for representing and manipulating data. """
import logging, re, os, sys, numpy as np, pandas as pd
...
def download(path='.', url=None, unpack=False):
    ...
    while True:
        ...
        status = '{0}  [{1:.2%}]'.format(bytes_dl, p)
...
class Dataset(object):
    ...
    def __init__(self, filename, feature_filename=None, masker=None, r=6, transform=True, target='MNI', **kwargs):
        ...
        if masker is None:
            ...
            masker = os.path.join(resource_dir, 'MNI152_T1_2mm_brain.nii.gz')
    ...
    def _load_mappables_from_txt(self, filename):
        ...
        return list(contents.groupby('id', as_index=False).apply(lambda x: mappable.Article(x, self.transformer)))
    ...
    def get_studies(self, features=None, expression=None, mask=None, peaks=None, frequency_threshold=0.001, activation_threshold=0.0, func=np.sum, return_type='ids', r=6):
        ...
        if peaks is not None:
            ...
            m = self.masker
            img = imageutils.map_peaks_to_image(peaks, r, vox_dims=m.vox_dims, dims=m.dims, header=m.get_header())
            results.append(self.get_studies(mask=img, activation_threshold=activation_threshold))
    ...
    @deprecated('get_mappables() is deprecated and will be removed in 0.5. Please use get_studies().')
    ...
    @deprecated('get_ids_by_features() is deprecated and will be removed in 0.5. Please use get_studies(features=...).')
    def get_ids_by_features(self, features, threshold=0.001, func=np.sum, get_image_data=False, get_weights=False, dense=True):
        ...
        if get_image_data:
            return self.get_image_data(ids, dense=dense)
        return ids

    @deprecated('get_ids_by_expression() is deprecated and will be removed in 0.5. Please use get_studies(expression=...).')
    ...
    @deprecated('get_ids_by_mask() is deprecated and will be removed in 0.5. Please use get_studies(mask=...).')
    ...
    @deprecated('get_ids_by_peaks() is deprecated and will be removed in 0.5. Please use get_studies(peaks=...).')
    def get_ids_by_peaks(self, peaks, r=10, threshold=0.0, get_image_data=False):
        ...
        peaks = np.array(peaks)
        peaks = transformations.xyz_to_mat(peaks)
        img = imageutils.map_peaks_to_image(peaks, r, vox_dims=self.masker.vox_dims, dims=self.masker.dims, header=self.masker.get_header())
...
class ImageTable(object):
    ...
    def __init__(self, dataset=None, mappables=None, masker=None, r=6, use_sparse=True):
        ...
        self.r = r
        data_shape = (
         self.masker.n_vox_in_vol, len(mappables))
        if use_sparse:
            vals, rows, cols = [], [], []
        else:
        ...
        for i, s in enumerate(mappables):
            ...
            img = imageutils.map_peaks_to_image(s.peaks, r=r, header=self.masker.get_header())
    ...
    def trim(self, ids):
        ...
        self.data = self.get_image_data(ids, dense=False)
        idxs = np.where(np.in1d(np.array(self.ids), np.array(ids)))[0]
...
class FeatureTable(object):
    __doc__ = ' A FeatureTable instance stores a matrix of mappables x features,\n    along with associated manipulation methods. '
    ...
    def get_ids(self, features, threshold=0.0, func=np.sum, get_weights=False):
        ...
        if get_weights:
            return above_thresh
        return list(above_thresh.index)
    ...
    def _sdf_to_csr(self):
        ...
        self.data = {'columns': list(data.columns), 
         'index': list(data.index), 
         'values': sparse.csr_matrix(data.values)}

    def _csr_to_sdf(self):
        """ Inverse of _sdf_to_csr(). """
        self.data = pd.DataFrame(self.data['values'].todense(), index=self.data['index'], columns=self.data['columns']).to_sparse()