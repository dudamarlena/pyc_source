...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/analysis/decode.py
# Compiled at: 2015-09-27 23:51:36
...
class Decoder:
    ...
    def decode(self, images, save=None, round=4, names=None, **kwargs):
        ...
        methods = {'pearson': self._pearson_correlation, 
    ...
    def _load_features_from_dataset(self, features=None, image_type=None, threshold=0.001):
        ...
        self.feature_images = meta.analyze_features(self.dataset, self.feature_names, image_type=image_type, threshold=threshold)
    ...
    def plot_polar(self, data, n_top=3, overplot=False, labels=None, palette='husl', size=6, ax=None, features=None, color=None, ylim=None, yticks=None):
        ...
        if labels is None:
            if features is not None:
                ...
                for i in range(n_panels):
                    labels.extend(data.iloc[:, i].order(ascending=False).index[:n_top])
        ...
        if overplot:
            ...
            fig, axes = plt.subplots(1, n_panels, sharex=False, sharey=False, subplot_kw=dict(polar=True))
            fig.set_size_inches((size * n_panels, size))
        ...
        for i in range(n_panels):
            if overplot:
                ...
                ax = axes[i]
                alpha = 0.8
                ax.set_title('Cluster %d' % (i + 1), fontsize=22, y=1.12)
            ax.set_ylim(data.values.min(), data.values.max())
            d = data.iloc[:, i].values
            col = colors[i] if color is None else color
            ax.fill(theta, d, color=col, alpha=alpha, ec='k', linewidth=0)
            ax.fill(theta, d, alpha=1.0, ec=col, linewidth=2, fill=False)
            ax.set_xticks(theta)
