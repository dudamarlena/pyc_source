...
# [GCC 8.4.0]
# Embedded file name: /Users/tal/Dropbox/Code/neurosynth/neurosynth/analysis/classify.py
# Compiled at: 2016-03-08 21:28:11
# Size of source mod 2**32: 15079 bytes
...
def feature_selection(feat_select, X, y):
    ...
    if re.match('.*-best', feat_select) is not None:
        ...
        with warnings.catch_warnings():
            ...
            features_selected = np.where(selector.fit(X, y).get_support() == True)[0]
    else:
...
def classify_regions(dataset, masks, method='ERF', threshold=0.08, remove_overlap=True, regularization='scale', output='summary', studies=None, features=None, class_weight='auto', classifier=None, cross_val='4-Fold', param_grid=None, scoring='accuracy'):
...
class Classifier:
    ...
    def __init__(self, clf_method='ERF', classifier=None, param_grid=None):
        ...
        if classifier is not None:
            ...
            if clf_method == 'SVM':
                ...
                if clf_method == 'ERF':
                    ...
                    self.clf = ExtraTreesClassifier(n_estimators=100, max_depth=None, min_samples_split=1, random_state=0)
                else:
                    if clf_method == 'GBC':
                        from sklearn.ensemble import GradientBoostingClassifier
                        self.clf = GradientBoostingClassifier(n_estimators=100, max_depth=1)
                    else:
                        if clf_method == 'Dummy':
        ...
        if isinstance(param_grid, dict):
            ...
            self.clf = GridSearchCV(estimator=self.clf, param_grid=param_grid)
    ...
    def cross_val_fit(self, X, y, cross_val='4-Fold', scoring='accuracy', feat_select=None, class_weight='auto'):
        ...
        if isinstance(self.clf, GridSearchCV):
            import warnings
            if feat_select is not None:
                warnings.warn('Cross-validated feature selection not supported witih GridSearchCV')
    ...
    def feat_select_cvs(self, scoring=None, feat_select=None):
        ...
        for train, test in self.cver:
            ...
            if feat_select is not None:
                fs = feature_selection(feat_select, X_train, y_train)
                self.features_selected.append(fs)
