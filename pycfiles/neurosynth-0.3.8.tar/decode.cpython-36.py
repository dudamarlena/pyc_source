...
# Compiled at: 2018-07-10 15:24:52
# Size of source mod 2**32: 12074 bytes
""" Decoding tools"""
import numpy as np
...
class Decoder:
    ...
    def __init__(self, dataset=None, method='pearson', features=None, mask=None, image_type='association-test_z', threshold=0.001):
        ...
        if from_array:
            ...
            if path.exists(features[0]):
                ...
                self._load_features_from_dataset(features,
                  image_type=image_type, threshold=threshold)
    ...
    def _load_features_from_dataset(self, features=None, image_type=None, threshold=0.001):
        ...
        self.feature_images = meta.analyze_features((self.dataset),
          (self.feature_names), image_type=image_type, threshold=threshold)
    ...
    def plot_polar(self, data, n_top=3, overplot=False, labels=None, palette='husl'):
        ...
        for i in range(n_panels):
            ...
            ax.fill(theta, d, alpha=1.0, ec=(colors[i]), linewidth=2,
              fill=False)
            ax.set_xticks(theta)
