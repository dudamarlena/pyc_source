...
# [GCC 8.4.0]
# Embedded file name: /media/xacce/addit/Projects/inventory/venv/src/sensei2/sensei2/sensei/sensei_room.py
# Compiled at: 2015-11-10 12:22:31
from operator import attrgetter
import string, os, importlib
...
class Sensei(object):
        ...
        for rule in self.rules:
            ...
            for field in fields:
                if field.attname not in ignore_fields:
                    if 'plugins' in rule.keys() and field.attname in rule['plugins'] and rule['plugins'][field.attname] in self.plugins.keys():
                        ...
                        for handler in self.all_handlers:
                            ...
                            if not getattr(field, 'null'):
                                raise HandlerException('no handler for field %s in model %s' % (field.attname, rule['model']))
    ...
    def get_random_url(self):
        url = '%(protocol)s%(domain)s.%(zone)s' % {'protocol': choice(['https://', 'http://']), 'domain': self.get_random_string(10), 
