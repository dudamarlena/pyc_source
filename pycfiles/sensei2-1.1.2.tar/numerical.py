...
# [GCC 8.4.0]
# Embedded file name: /media/xacce/addit/Projects/inventory/venv/src/sensei2/sensei2/sensei/handlers/numerical.py
# Compiled at: 2015-10-21 13:50:56
from decimal import *
...
class FloatFieldHandler(BaseHandler):
    ...
    def prepare_value(self, obj, field, sensei):
        if not field.choices:
            return sensei.get_random_float()
        return choice([ x[0] for x in field.choices ])


class BooleanFieldHandler(BaseHandler):
...
class CommaSeparatedIntegerFieldHandler(BaseHandler):
    ...
    def prepare_value(self, obj, field, sensei):
        ...
        return '%(part1)s,%(part2)s' % {'part1': sensei.get_random_int_string(max_len=max_number_len), 'part2': sensei.get_random_int_string(max_len=max_number_len)}
...
class DecimalFieldHandler(BaseHandler):
    ...
    def prepare_value(self, obj, field, sensei):
        ...
        value = '%(part1)s.%(part2)s' % {'part1': sensei.get_random_int_string(max_len=max_digits - decimal_places), 'part2': sensei.get_random_int_string(max_len=decimal_places)}
