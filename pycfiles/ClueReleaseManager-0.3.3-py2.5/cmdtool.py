...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/relmgr/cmdtool.py
# Compiled at: 2009-09-18 12:06:12
import logging, optparse, os, subprocess, sys, tempfile, pkg_resources
...
class Runner(object):
    ...
    def main(self, args=None, extraargs=None):
        ...
        if cmd == 'updateuser':
            ...
            pypi.security_manager.update_user(params[0], params[1], params[2], roledict.get('', []))
            for (distro_id, roles) in roledict.items():
                pypi.security_manager.update_roles(distro_id=distro_id, username=params[0], roles=roles)

        elif cmd == 'updategroup':
            roledict = self.split_roles(*params[1:])
            pypi.security_manager.update_group(params[0], roledict.get('', []))
            for (distro_id, roles) in roledict.items():
                pypi.security_manager.update_roles(distro_id=distro_id, groupname=params[0], roles=roles)
            ...
            pypi.index_manager.add_index_item(distro_id, indexname, target_distro_id, target_version)
        elif cmd == 'delindexentry':
            distro_id = params[0]
            indexname = params[1]
            target_distro_id = params[2]
            pypi.index_manager.del_index_item(distro_id, indexname, target_distro_id)
        elif cmd == 'setupindex':
            parser = optparse.OptionParser()
            parser.add_option('-f', '--overwrite', dest='overwrite', action='store_true', help='Force overwrite', default=False)
            (options, args) = parser.parse_args(params)
            self.setupindex(pypi, args[0], args[1], args[2:], overwrite=options.overwrite)
        else:
    ...
    def setupindex(self, pypi, distro_id, indexname, reqs, overwrite=False):
        ...
        if not overwrite and pypi.index_manager.has_index(distro_id, indexname):
            raise ValueError('Index by the name of "%s" for distro "%s" already exists, use --overwrite to overwrite' % (
             indexname, distro_id))
        try:
        ...
        froze = subprocess.Popen([pip_path, 'freeze'], stdout=subprocess.PIPE).communicate()[0]
        print
        ...
        for x in index_reqs:
            ...
            pypi.index_manager.add_index_item(distro_id, indexname, req.project_name, req.specs[0][1])

        print 'Created index "%s" for distro "%s" with the following requirements' % (
         distro_id, indexname)
        for x in index_reqs:
            print '  ', x.strip()
