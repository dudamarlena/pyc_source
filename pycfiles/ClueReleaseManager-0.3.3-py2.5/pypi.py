...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/relmgr/pypi.py
...
import threading, pkg_resources
...
class PyPi(object):
    ...
    def perform_action(self, action='', **kwargs):
        if action not in self.actions:
            self.logger.error('Action "%s" is not handled by this server' % action)
            raise PyPiError('Action "%s" is not handled by this server' % action)
    ...
    def has_role(self, distro_id, *roles):
        if AUTHENTICATED_ROLE in roles and self.get_active_user() != ANONYMOUS:
            return True
        derived = self.security_manager.get_roles(self.get_active_user(), distro_id, True)
        for x in roles:
            if x in derived:
                return True
    ...
    def upload_files(self, name, content, **kwargs):
        ...
        if not self.has_role(distro_id, model.OWNER_ROLE, MANAGER_ROLE):
            raise SecurityError('"%s" is not the owner of "%s" distro' % (
             self.get_active_user(), distro_id))
        targetdir = os.path.join(self.basefiledir, distro_id[0], distro_id)
    ...
    def get_index(self, distro_id, indexname):
        if not self.has_role(distro_id, READER_ROLE, MANAGER_ROLE, model.OWNER_ROLE):
        ...
        for reqstr in index:
            ...
            targetdir = os.path.join(self.basefiledir, targetdistro.distro_id[0], targetdistro.distro_id)
            if not os.path.exists(targetdir):
                continue
            for fname in os.listdir(targetdir):
                for d in package_index.distros_for_filename(os.path.join(targetdir, fname)):
    ...
    def search(self, s, order_by='distro_id'):
        ...
        expr = '%%%s%%' % s
        query = query.filter(sa.or_(model.SQLDistro.name.like(expr), model.SQLDistro.description.like(expr), model.SQLDistro.summary.like(expr)))
        ...
        distros = [ x for x in query if self.has_role(x.distro_id, READER_ROLE, MANAGER_ROLE, model.OWNER_ROLE)
                  ]
        return distros
    ...
    def get_files(self, distro_id):
        ...
        res.sort(lambda x, y: cmp(version_info(x), version_info(y)), reverse=True)
        return res
    ...
    actions = {'submit': update_metadata, 
       'user': register_user, 
