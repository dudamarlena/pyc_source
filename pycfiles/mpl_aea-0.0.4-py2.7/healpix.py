...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mpl_aea/healpix.py
...
def ang2pix(nside, theta, phi):
    ...
    z = numpy.cos(theta)
    s = numpy.sin(theta)
    tt = phi / (0.5 * numpy.pi) % 4
    result = numpy.zeros(z.shape, dtype='i8')
    mask = (z < 2.0 / 3) & (z > -2.0 / 3)
...
def xy2ang(x, y):
    ...
    def polarcaps(x, y):
        ...
        z = (1 - 1.0 / 3 * (2 - 4 * ya / numpy.pi) ** 2) * y / ya
        return (numpy.arccos(z), phi)
...
def vertices(nside, pix, step=(0.5, 0.5, 0.5, 0.5)):
    """ Calculate the vertices for pixels 
            3
         0     2
            1
        Returns: theta, phi
    ...
    x[(Ellipsis, 0)] = xc - step[0] * xstep
    y[(Ellipsis, 0)] = yc
    x[(Ellipsis, 1)] = xc
    y[(Ellipsis, 1)] = yc + step[1] * ystep
    x[(Ellipsis, 2)] = xc + step[2] * xstep
    y[(Ellipsis, 2)] = yc
    x[(Ellipsis, 3)] = xc
    y[(Ellipsis, 3)] = yc - step[3] * ystep
