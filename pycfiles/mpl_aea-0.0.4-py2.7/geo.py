...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mpl_aea/geo.py
# Compiled at: 2017-04-14 21:04:55
from __future__ import absolute_import, division, print_function, unicode_literals
import math, numpy as np, numpy.ma as ma, matplotlib
rcParams = matplotlib.rcParams
from matplotlib.axes import Axes
from matplotlib import cbook
from matplotlib.patches import Circle
from matplotlib.path import Path
import matplotlib.spines as mspines, matplotlib.axis as maxis
...
class GeoAxes(SkymapperAxes):
    ...
    class ThetaFormatter(Formatter):
        ...
        def __call__(self, x, pos=None):
            ...
            if rcParams[b'text.usetex'] and not rcParams[b'text.latex.unicode']:
                return b'$%0.0f^\\circ$' % degrees
            else:
                return b'%0.0f°' % degrees
    ...
    def cla(self):
        ...
        self.grid(rcParams[b'axes.grid'])
    ...
    def _set_lim_and_transforms(self):
        ...
        yaxis_stretch = Affine2D().scale(360, 1.0)
    ...
    def get_xaxis_transform(self, which=b'grid'):
        if which not in ('tick1', 'tick2', 'grid'):
            msg = b"'which' must be on of [ 'tick1' | 'tick2' | 'grid' ]"
            raise ValueError(msg)
        return self._xaxis_transform

    def get_xaxis_text1_transform(self, pad):
        return (
         self._xaxis_text1_transform, b'bottom', b'center')

    def get_xaxis_text2_transform(self, pad):
        return (
         self._xaxis_text2_transform, b'top', b'center')

    def get_yaxis_transform(self, which=b'grid'):
        if which not in ('tick1', 'tick2', 'grid'):
            msg = b"'which' must be one of [ 'tick1' | 'tick2' | 'grid' ]"
            raise ValueError(msg)
        return self._yaxis_transform

    def get_yaxis_text1_transform(self, pad):
        return (
         self._yaxis_text1_transform, b'center', b'right')

    def get_yaxis_text2_transform(self, pad):
        return (
         self._yaxis_text2_transform, b'center', b'left')

    def _gen_axes_patch(self):
        return Circle((0.5, 0.5), 0.5)

    def _gen_axes_spines(self):
        return {b'geo': mspines.Spine.circular_spine(self, (0.5, 0.5), 0.5)}
    ...
    def set_xlim(self, *args, **kwargs):
        raise TypeError(b'It is not possible to change axes limits for geographic projections. Please consider using Basemap or Cartopy.')
    ...
    def set_longitude_grid(self, degrees):
        ...
        self.xaxis.set_major_locator(FixedLocator(np.linspace(0, 360, number, True)[1:-1]))
    ...
    def set_latitude_grid(self, degrees):
        ...
        self.yaxis.set_major_locator(FixedLocator(np.linspace(-90, 90, number, True)[1:-1]))
...
class AitoffAxes(GeoAxes):
    ...
    class InvertedAitoffTransform(Transform):
        ...
        def transform_non_affine(self, xy):
            raise RuntimeError(b'invertedaitoff is not implemented')
            return xy
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.set_aspect(0.5, adjustable=b'box', anchor=b'C')
        self.cla()
...
class MollweideAxes(GeoAxes):
    ...
    class MollweideTransform(SkymapperTransform):
        ...
        def transform_non_affine(self, ll):
            ...
            if ihigh.any():
                e = clat[ihigh]
                d = 0.5 * (3 * np.pi * e ** 2) ** 0.3333333333333333
...
class LambertAxes(GeoAxes):
    ...
    class InvertedLambertTransform(Transform):
        ...
        def transform_non_affine(self, xy):
            ...
            clat = self._center_latitude
            p = np.sqrt(x * x + y * y)
            p = np.where(p == 0.0, 1e-09, p)
            c = 2.0 * np.arcsin(0.5 * p)
            sin_c = np.sin(c)
            cos_c = np.cos(c)
            lat = np.arcsin(cos_c * np.sin(clat) + y * sin_c * np.cos(clat) / p)
            lon = clong + np.arctan(x * sin_c / (p * np.cos(clat) * cos_c - y * np.sin(clat) * sin_c))
            return np.degrees(np.concatenate((lon + np.pi, lat), 1))
    ...
    def __init__(self, *args, **kwargs):
        self._longitude_cap = np.pi / 2.0
        self._center_longitude = kwargs.pop(b'center_longitude', 0.0)
        self._center_latitude = kwargs.pop(b'center_latitude', 0.0)
        GeoAxes.__init__(self, *args, **kwargs)
        self.set_aspect(b'equal', adjustable=b'box', anchor=b'C')
        self.cla()
