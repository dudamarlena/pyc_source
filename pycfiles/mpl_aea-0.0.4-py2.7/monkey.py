...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mpl_aea/monkey.py
# Compiled at: 2017-04-14 21:04:55
import matplotlib.transforms as mtransforms, matplotlib.path as mpath, matplotlib.artist as martist
...
class TransformedPatchPath(mtransforms.TransformedPath):
    ...
    def _revalidate(self):
        ...
        if self._transformed_path is None or self._path != patch_path or self._invalid & self.INVALID_NON_AFFINE == self.INVALID_NON_AFFINE:
            ...
            self._transformed_points = mpath.Path._fast_from_codes_and_verts(self._transform.transform_non_affine(patch_path.vertices), None, {'interpolation_steps': patch_path._interpolation_steps, 'should_simplify': patch_path.should_simplify})
        self._invalid = 0
        return
