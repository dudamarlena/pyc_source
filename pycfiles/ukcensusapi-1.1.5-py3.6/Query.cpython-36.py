...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ukcensusapi/Query.py
...
class Query:
    ...
    """
        ...
        if add_geog:
            query_params['geography'] = self._Query__add_geog(meta)
    ...
    def __add_geog(self, metadata):
        ...
        if coverage == 'E':
            coverage_codes = [
             Api.Nomisweb.GeoCodeLookup['England']]
        else:
            if coverage == 'EW':
                coverage_codes = [
                 Api.Nomisweb.GeoCodeLookup['EnglandWales']]
            else:
                if coverage == 'GB':
                    coverage_codes = [
                     Api.Nomisweb.GeoCodeLookup['GB']]
                else:
                    if coverage == 'UK':
                        coverage_codes = [
                         Api.Nomisweb.GeoCodeLookup['UK']]
                    else:
    ...
    """
        ...
        with open(str(snippet_file), 'w') as (py_file):
            ...
            if 'geography' not in query_params:
