...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/Gates/backtrack.py
...
def backtrack(element, depth):
    ...
    if not (isinstance(element, GATES) or isinstance(element, Connector) or type(element) in [bool, int]):
        raise Exception('ERROR: Element must be either a Gate or Connector')
    ret_value = None
    if isinstance(element, GATES):
        if depth == 0:
            ret_value = (
             element, bool(element.output()))
        else:
            ret_value_list = []
