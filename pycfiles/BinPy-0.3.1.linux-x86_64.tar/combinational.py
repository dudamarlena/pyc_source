...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/Combinational/combinational.py
...
class FullSubtractor(GATES):
    ...
    def trigger(self):
        ...
        y = self.inputs[1]
        z = self.inputs[2]
        D = OR(AND(NOT(x), NOT(y), z), AND(NOT(x), y, NOT(z)), AND(x, NOT(y), NOT(z)), AND(x, y, z)).output()
...
class BinarySubtractor(GATES):
    ...
    def trigger(self):
        ...
        borrow = self.borrow
        for i in range(self.size - 1, -1, -1):
            D, borrow = FullSubtractor(self.inputs[0][i], self.inputs[1][i], borrow).output()
            result.append(D)
...
class Encoder(GATES):
    ...
    def __init__(self, *inputs):
        if not (len(inputs) > 1 and len(inputs) & len(inputs) - 1 == 0):
            raise Exception('ERROR: Number of inputs should be a power of 2')
        if not (inputs.count(1) == 1 or list(x.state for x in filter(lambda i: isinstance(i, Connector), inputs)).count(1) == 1):
    ...
    def setInput(self, index, value):
        ...
        if index >= len(temp):
            temp.append(value)
            if not (temp.count(1) == 1 or list(x.state for x in filter(lambda i: isinstance(i, Connector), temp)).count(1) == 1):
                raise Exception('ERROR: Invalid Input')
                self.inputs.append(value)
            for i in range(len(self.outputType), int(math.log(len(self.inputs), 2))):
                self.outputType.append(0)
