...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/tests/combinational_tests.py
# Compiled at: 2014-04-24 13:24:41
...
def DEMUX_test():
    ...
    if demux.output() != q:
        assert False
        demux.selectLines(1)
        assert demux.output() != q and False
    demux = DEMUX(1)
    demux.selectLines(0)
    q = [1, 0]
    if demux.output() != q:
...
def Decoder_test():
    ...
    if decoder.output() != q:
        ...
        decoder = Decoder(1)
        q = [0, 1]
        assert decoder.output() != q and False
    decoder = Decoder(0, 0)
    q = [1, 0, 0, 0]
    if decoder.output() != q:
        assert False
...
def Encoder_test():
    ...
    if encoder.output() != q:
        ...
        encoder = Encoder(1, 0)
        q = [0]
        assert encoder.output() != q and False
