...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/Sequential/sequential.py
...
class SRLatch(FlipFlop):
    ...
    def __init__(self, S, R, enable, clk, preset=Connector(1), clear=Connector(1), a=Connector(0), b=Connector(1)):
    ...
    def trigger(self):
        if self.clear.state == 1 and self.preset.state == 0:
            return self.setff()
        if self.preset.state == 1 and self.clear.state == 0:
            return self.resetff()
        if not (self.clear.state or self.preset.state):
            print('Error: Invalid State - Resetting the Latch')
            self.clear.state = 1
            self.preset.state = 1
        elif self.clkoldval == 1 and self.clk.state == 0:
...
class DFlipFlop(FlipFlop):
    ...
    def __init__(self, D, enable, clk, preset=Connector(1), clear=Connector(1), a=Connector(0), b=Connector(0)):
        FlipFlop.__init__(self, enable, clk, a, b)
        self.D = Connector(0)
...
class JKFlipFlop(FlipFlop):
    ...
    def __init__(self, J, K, enable, clk, preset=Connector(1), clear=Connector(1), a=Connector(0), b=Connector(1)):
        FlipFlop.__init__(self, enable, clk, a, b)
        self.J = Connector(0)
        self.K = Connector(0)
        self.preset = Connector(1)
...
class TFlipFlop(JKFlipFlop):
    ...
    def __init__(self, T, enable, clk, preset=Connector(1), clear=Connector(1), a=Connector(), b=Connector()):
