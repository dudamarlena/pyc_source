...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/Algorithms/ExpressionConvert.py
...
def mergeNot(case, expr):
    ...
    if expr[(-1)] == ')':
        ...
        if gate == 'AND' and case == 'general':
            return 'NAND' + expr[index:]
        if gate == 'NOT':
            return expr[index + 1:-1]
        if gate == 'XOR' and case == 'general':
            return 'XNOR' + expr[index:]
        if gate == 'XNOR' and case == 'general':
            return 'XOR' + expr[index:]
        if gate == 'NAND' and case == 'general':
            return 'AND' + expr[index:]
        if gate == 'NOR' and case == 'general':
...
def to_and_or_not(gate, op1, op2):
    ...
    if gate == 'XOR':
        return 'OR(AND(' + op1 + ', ' + mergeNot('general', op2) + '), AND(' + mergeNot('general', op1) + ', ' + op2 + '))'
    if gate == 'XNOR':
        return 'OR(AND(' + mergeNot('general', op1) + ', ' + mergeNot('general', op2) + '), AND(' + op1 + ', ' + op2 + '))'
...
def to_nand(gate, op1, op2):
    ...
    if gate == 'OR':
        return 'NAND(' + mergeNot('special', op1) + ', ' + mergeNot('special', op2) + ')'
    if gate == 'NAND':
        return gate + '(' + op1 + ', ' + op2 + ')'
    if gate == 'NOR':
...
def to_nor(gate, op1, op2):
    ...
    if gate == 'AND':
        return 'NOR(' + mergeNot('special', op1) + ', ' + mergeNot('special', op2) + ')'
    if gate == 'NOR':
        return gate + '(' + op1 + ', ' + op2 + ')'
    if gate == 'NAND':
...
def remove_not(gate, exp):
    ...
    while 'NOT' in exp:
        ...
        exp = exp[:index] + gate + '(' + exp[index + 4:index2] + ', ' + exp[index + 4:index2] + ')' + exp[index2 + 1:]

    return exp


def convertExpression(expr, two_input=0, only_nand=0, only_nor=0, only_and_or_not=0):
    ...
    while ')' in list1:
        ...
        if len(list1) > 1:
            ...
            if gate != 'NOT':
                ...
                elif only_nand == 0 and only_nor == 0 and only_and_or_not == 0:
                    new_element = gate + '(' + op1 + ', ' + op2 + ')'
