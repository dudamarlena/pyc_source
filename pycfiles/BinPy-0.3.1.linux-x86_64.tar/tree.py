...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/BinPy/Gates/tree.py
...
class Tree:
    ...
    def backtrack(self, hist=None):
        ...
        if hist is not None:
            ...
            self.hist = CycleHist()
        if self.depth < 0:
            raise Exception('ERROR: Depth of backtrack function must be bigger or                    equal to 0')
        if not (isinstance(self.element, GATES) or isinstance(self.element, Connector) or type(self.element) in [bool, int]):
    ...
    def printTuple(self, tree_node, space=0):
        ...
        stdout.write('|- ')
        if not self.cycles:
            if type(self.element) not in [int, bool] and self.hist.isRepeated(self.element):
