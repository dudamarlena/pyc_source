...
# Embedded file name: build\bdist.win32\egg\pylon\io\rst.py
# Compiled at: 2010-12-26 13:36:33
__doc__ = ' Defines a class for writing case data to a ReStructuredText file.\n'
...
class ReSTWriter(_CaseWriter):
    ...
    def write_generator_data(self, file):
        ...
        sep = ('=' * col1_width + ' ') * 2 + ('=' * col_width_bool + ' ') + ('=' * col_width + ' ') * 5 + ('=' * col_width_poly + ' ') * 3 + '\n'
        file.write(sep)
        file.write(('Name').center(col1_width) + ' ')
        file.write(('Bus').center(col1_width) + ' ')
    ...
    def write_how_many(self, file):
        ...
        report = CaseReport(self.case)
        components = [
         ('Bus', 'n_buses'), ('Generator', 'n_generators'),
