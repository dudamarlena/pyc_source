...
# Embedded file name: build\bdist.win32\egg\pylon\test\reader_test.py
# Compiled at: 2010-12-26 13:36:33
__doc__ = ' Data file parsing tests.\n'
import os.path
from time import time
from unittest import TestCase, main
from pylon.io import MATPOWERReader, PSSEReader, PickleReader
...
class ReaderTest(TestCase):
    ...
    def _validate_object_numbers(self, n_buses, n_branches, n_gen):
        ...
        """
        c = self.case
        self.assertEqual(len(c.buses), n_buses, '%d buses expected, %d found' % (n_buses, len(c.buses)))
        self.assertEqual(len(c.branches), n_branches, '%d branches expected, %d found' % (n_branches, len(c.branches)))
        self.assertEqual(len(c.generators), n_gen, '%d generators expected, %d found' % (n_gen, len(c.generators)))
    ...
    def _validate_branch_connections(self, from_idxs, to_idxs):
        ...
        for e in c.branches:
            ...
            self.assertEqual(from_idx, from_expected, 'From bus %d expected, %d found' % (
             from_expected, from_idx))
            to_idx = c.buses.index(e.to_bus)
            to_expected = to_idxs[c.branches.index(e)]
            self.assertEqual(to_idx, to_expected, 'To bus %d expected, %d found' % (
             to_expected, to_idx))
...
class MatpowerReaderTest(ReaderTest):
    ...
    def test_case30pwl(self):
        ...
        self._validate_branch_connections(from_idxs=[
         0, 0, 1, 2, 1, 1, 3, 4, 5, 5, 5, 5, 8, 8, 3, 11, 11,
         11, 11, 13, 15, 14, 17, 18, 9, 9, 9, 9, 20, 14, 21,
         22, 23, 24, 24, 27, 26, 26, 28, 7, 5], to_idxs=[
         1, 2, 3, 3, 4, 5, 5, 6, 6, 7, 8, 9, 10, 9, 11, 12, 13,
...
class PSSEReaderTest(TestCase):
    ...
    def test_sample(self):
        ...
        self.assertEqual(len(case.buses), 46)
        pl = 5
        ...
        load_buses = [ b for b in case.buses if b.p_demand > 0.0 or b.q_demand > 0.0
                     ]
        ...
        self.assertEqual(case.branches[0].r, 0.0026, pl)
        self.assertEqual(case.branches[0].x, 0.046, pl)
        self.assertEqual(case.branches[0].b, 3.5, pl)
        self.assertEqual(case.branches[0].rate_a, 1200.0, pl)
        self.assertEqual(case.branches[0].rate_b, 1100.0, pl)
        self.assertEqual(case.branches[0].rate_c, 1000.0, pl)
        ...
        self.assertEqual(case.branches[30].r, 9e-05, pl)
        ...
        self.assertEqual(case.branches[30].rate_a, 1200.0, pl)
        self.assertEqual(case.branches[30].rate_b, 1100.0, pl)
        self.assertEqual(case.branches[30].rate_c, 1000.0, pl)
        self.assertEqual(len(case.branches), 44 + 12)
