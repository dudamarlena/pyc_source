...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mygrate/binlog.py
# Compiled at: 2014-07-18 08:26:34
import os, sys, os.path, re, signal, subprocess, optparse, logging, glob, time
from datetime import datetime
from ast import literal_eval
from time import sleep
import MySQLdb, bitstring
...
class ValueParser(object):
    ...
    __slots__ = [
     'line', 'valid', 'value']
    _int_pattern = re.compile('^[-0-9]+ \\(([0-9]+)\\)$')
...
class QueryBase(object):
    ...
    def __repr__(self):
        return ('<Query {0} {1} WHERE={2!s} SET={3!s}>').format(self.type, self.table, self.values['WHERE'], self.values['SET'])


class InsertQuery(QueryBase):
    _initial_pattern = re.compile('^INSERT INTO (.*)$')
    type = 'INSERT'
...
class UpdateQuery(QueryBase):
    _initial_pattern = re.compile('^UPDATE (.*)$')
    type = 'UPDATE'
...
class DeleteQuery(QueryBase):
    _initial_pattern = re.compile('^DELETE FROM (.*)$')
    type = 'DELETE'
...
class QueryParser(object):
    ...
    def parse(self, line):
        ...
        if line.startswith('INSERT'):
            ...
            query = InsertQuery(line, self.callbacks, self.column_names, self.char_sets)
            self.current = query if query.table in registered_tables else None
        elif line.startswith('UPDATE'):
            self._handle_completion()
            query = UpdateQuery(line, self.callbacks, self.column_names, self.char_sets)
            self.current = query if query.table in registered_tables else None
        elif line.startswith('DELETE'):
            self._handle_completion()
            query = DeleteQuery(line, self.callbacks, self.column_names, self.char_sets)
            self.current = query if query.table in registered_tables else None
        elif self.current:
...
class BinlogParser(object):
    ...
    def __init__(self, index_file, pos_dir, callbacks, column_names=None, char_sets=None):
        self.done = False
    ...
    def process_binlog(self, binlog):
        ...
        args = [
         'mysqlbinlog', '-v', '--base64-output=DECODE-ROWS', binlog,
         '-j', last_position,
         '--set-charset=utf8']
    ...
    def set_binlogpos_at_end(self, binlog):
        ...
        self.log.info(('changing {0} from {1} to {2}').format(pos_file, old_pos, binlog_size))
...
def main():
    ...
    """
    description = 'This program follows changes in the MySQL binlog, producing job tasks for each\nchange. It is intended to be long-running, and will briefly pause after\ncatching up each binlog before checking for new changes.\n\nConfiguration for %prog is done with configuration files. This is either\n/etc/mygrate.ini, ~/.mygrate.ini, or an alternative specified by the\nMYGRATE_CONFIG environment variable.\n'
    op = optparse.OptionParser(description=description)
    op.add_option('-d', '--daemon', action='store_true', help='Daemonize the process before binlog tracking begins')
    op.add_option('-p', '--pid-file', metavar='FILE', help='Write the process ID to FILE')
