...
# [GCC 8.4.0]
# Embedded file name: /Users/thoas/Sites/Python/ulule/python-mangopay/mangopay/utils.py
...
if six.PY3:
    ...
    request.URLopener.open_https = orig
elif six.PY2:
    import urllib
    orig = urllib.URLopener.open_https
    urllib.URLopener.open_https = orig
import datetime, decimal, copy

@python_2_unicode_compatible
class Money(object):
    __hash__ = None

    def __init__(self, amount=b'0', currency=None):
        try:
            self.amount = decimal.Decimal(amount)
        except decimal.InvalidOperation:
            raise ValueError((b"amount value could not be converted to Decimal(): '{}'").format(amount))

        self.currency = currency

    def __repr__(self):
        return (b'{} {}').format(self.currency, self.amount)

    def __str__(self):
        return force_text((b'{} {:,.2f}').format(self.currency, self.amount))

    def __lt__(self, other):
        if isinstance(other, Money):
            if other.currency != self.currency:
                raise CurrencyMismatch(self.currency, other.currency, b'<')
            other = other.amount
    ...
    def __mul__(self, other):
        if isinstance(other, Money):
            raise TypeError(b'multiplication is unsupported between two money objects')
        amount = self.amount * other
        return self.__class__(amount, self.currency)
    ...
    def __mod__(self, other):
        if isinstance(other, Money):
            raise TypeError((b"modulo is unsupported between two '{}' objects").format(self.__class__.__name__))
        if other == 0:
            raise ZeroDivisionError()
        amount = self.amount % other
        return self.__class__(amount, self.currency)
    ...
    def __divmod__(self, other):
        if isinstance(other, Money):
            if other.currency != self.currency:
                raise CurrencyMismatch(self.currency, other.currency, b'divmod')
            if other.amount == 0:
                raise ZeroDivisionError()
        ...
        return (
         self.__class__(whole, self.currency),
         self.__class__(remainder, self.currency))

    def __pow__(self, other):
        if isinstance(other, Money):
            raise TypeError((b"power operator is unsupported between two '{}' objects").format(self.__class__.__name__))
        amount = self.amount ** other
        return self.__class__(amount, self.currency)
...
class Choices(object):
        ...
        for choice in choices:
            if isinstance(choice, (list, tuple)):
                if len(choice) == 3:
                    ...
                    raise ValueError(b"Choices can't take a list of length %s, only 2 or 3" % len(choice))
            else:
    ...
    def __repr__(self):
        return b'%s(%s)' % (
         ...
         (b', ').join(b'%s' % repr(i) for i in self._triples))
...
def force_text(s, encoding=b'utf-8', strings_only=False, errors=b'strict'):
    """
    ...
    try:
        ...
        s = (b' ').join([ force_text(arg, encoding, strings_only, errors) for arg in s
                        ])

    return s


def force_bytes(s, encoding=b'utf-8', strings_only=False, errors=b'strict'):
    """
    ...
    if isinstance(s, bytes):
        if encoding == b'utf-8':
            ...
            return s.decode(b'utf-8', errors).encode(encoding, errors)
