...
# [GCC 8.4.0]
# Embedded file name: /Users/thoas/Sites/Python/ulule/python-mangopay/mangopay/query.py
...
class SelectQuery(BaseQuery):
    ...
    def list(self, reference, resource_model, handler=None):
        handler = handler or self.handler
        result, data = handler.request(self.method, '/%s/%d/%s' % (resource_model._meta.verbose_name_plural, reference,
...
class InsertQuery(BaseQuery):
    ...
    def execute(self, handler=None):
        ...
        result, data = handler.request(self.method, url, data=data)
