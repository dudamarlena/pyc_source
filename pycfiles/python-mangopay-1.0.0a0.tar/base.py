...
# [GCC 8.4.0]
# Embedded file name: /Users/thoas/Sites/Python/ulule/python-mangopay/mangopay/base.py
...
class BaseApiModel(object):
    ...
    def __eq__(self, other):
        return other.__class__ == self.__class__ and self.get_pk() and other.get_pk() == self.get_pk()
    ...
    def save(self, handler=None, cls=None):
        ...
        if self.get_pk():
            update = self.update(self.get_pk(), **field_dict)
    ...
    def one(self, resource_model):
        return resource_model.select().get(self.get_pk(), resource_model=self.__class__, handler=self.handler)

    def list(self, resource_model):
        return resource_model.select().list(self.get_pk(), self.__class__, handler=self.handler)
