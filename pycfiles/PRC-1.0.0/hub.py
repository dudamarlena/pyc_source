...
# Embedded file name: build\bdist.win-amd64\egg\prboard\hub.py
# Compiled at: 2016-04-23 03:30:36
__doc__ = '\nModule to work with Github repo\n'
import logging
from functools import partial
import github, github.GithubObject, github.PaginatedList, github.Repository, settings
...
class Github(github.Github):
    ...
    def get_user_repos(self, user=NotSet, since=NotSet):
        ...
        return github.PaginatedList.PaginatedList(github.Repository.Repository, self.__requester, ('/users/{}/repos').format(user), url_parameters)
    ...
    def get_org_repos(self, org, since=NotSet):
        ...
        if since is not NotSet:
            url_parameters['since'] = since
        return github.PaginatedList.PaginatedList(github.Repository.Repository, self.__requester, ('orgs/{}/repositories').format(org), url_parameters)
...
class RepoDash(object):
    ...
    def __init__(self, repo_url=None, ghub=None, repo=None, user_or_token=None, password=None, pr_filter=None, cmd=True, state=NotSet):
        ...
        """
        if not (ghub is None and (repo is not None or user_or_token is not None)):
            raise AssertionError('User ID or Token must be provided.')
            assert repo or repo_url, 'Repo object or Repo url must be provided.'
            self.ghub = repo or ghub and ghub or github.Github(user_or_token, password, base_url=settings.PRBOARD_BASE_URL)
...
class DashBoard(object):
    ...
    def __init__(self, user_or_token=settings.PRBOARD_GITHUB_USERNAME, password=settings.PRBOARD_GITHUB_PASSWORD, repo_filter=None, pr_filter=None, state=NotSet, cmd=True, **kwargs):
        """
