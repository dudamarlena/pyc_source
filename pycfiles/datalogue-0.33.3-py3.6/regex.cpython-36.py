...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/regex.py
...
from datalogue.utils import SerializableStringEnum, exists, _parse_datetime, _parse_uuid, _parse_list
from datalogue.utils import map_option
...
class RegexTestSample:
    ...
    def __repr__(self):
        return f"\n  RegexTestSample(text: {self.text!r}, status: {self.status})"
...
class Regex:
    ...
    def __init__(self, name: str, pattern: str, description: str=None, test_data: Optional[Union[(List[str], List[RegexTestSample])]]=[], id: Optional[UUID]=None, owner: Optional[UUID]=None, editors: Optional[List[UUID]]=None, created_at: Optional[datetime]=None, updated_at: Optional[datetime]=None, updated_by: Optional[UUID]=None):
        self.id = id
        self.name = name
        self.pattern = pattern
        self.description = description
        if test_data is not None:
            if all(isinstance(test_datum_text, str) for test_datum_text in test_data):
                self.test_data = list(map(lambda test_datum: RegexTestSample(test_datum), test_data))
        if test_data is not None and all(isinstance(test_datum_text, RegexTestSample) for test_datum_text in test_data):
    ...
    def __eq__(self, other):
        return self.name == other.name and self.pattern == other.pattern and self.description == other.description and self.test_data == other.test_data and self.owner == other.owner and self.editors == other.editors and self.created_at == other.created_at and self.updated_at == other.updated_at and self.updated_by == other.updated_by

    def __repr__(self):
        return f"Regex(id: {self.id!r}, name: {self.name!r}, pattern: {self.pattern!r},  description: {self.description!r}, test_data: {self.test_data})"
