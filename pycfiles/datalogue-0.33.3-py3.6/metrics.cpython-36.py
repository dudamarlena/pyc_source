...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/metrics.py
# Compiled at: 2020-04-14 15:37:55
# Size of source mod 2**32: 6351 bytes
...
class ArgoMetrics:
    ...
    def __init__(self, argo_work_dir: str, experiment_name: str, class_map_path: str):
        if not os.path.isdir(argo_work_dir):
            raise DtlError(f"{argo_work_dir} is not a directory. Path to the template's work directory(i.e. nightshade/work/cbc")
    ...
    def _fetch_training_files(self) -> List[str]:
        tfs = sorted(glob(os.path.join(self.argo_work_dir, self.experiment_name, 'control', '*-train-completed', 'epoch-*-experiment-tracking.json')))
        if len(tfs) == 0:
            raise DtlError(f"No Training Files found in {self.argo_work_dir}, experiment: {self.experiment_name}")
        return tfs

    def _fetch_test_file(self) -> str:
        tfs = sorted(glob(os.path.join(self.argo_work_dir, self.experiment_name, 'control', '*-test-completed', 'test-results.json')))
        if len(tfs) == 0:
            raise DtlError(f"No Test Files found in {self.argo_work_dir}, experiment: {self.experiment_name}")
        return tfs[(-1)]

    @staticmethod
    def _load_class_map(class_map_path: str) -> dict:
        if not os.path.isfile(class_map_path):
            raise DtlError(f"{class_map_path} is not a file. Please provide a Json or Yaml file!")
        with open(class_map_path, 'r', encoding='utf-8') as (f):
            if class_map_path.endswith('json'):
                return json.load(f)
            if class_map_path.endswith('yaml'):
                return yaml.load(f)
            raise DtlError(f"{class_map_path} is not Json or Yaml. Unable to Load Class Map")
    ...
    def create_payload(self) -> List[dict]:
        ...
        for file in self.training_files:
            ...
            for phase in ('training', 'validation'):
                M['metrics'][phase] = {'loss':data['metrics'][phase]['loss'],  'acc':data['metrics'][phase]['acc'], 
                ...
                if 'validation' == phase:
                    conf_mat = np.array(data['metrics'][phase]['confusion_matrix']['matrix'])
                    evaluation = self.get_metrics(conf_mat,
                      phase_name=f"{self.experiment_name}_VAL_epoch_{data['epoch']}_", class_names=(data['metrics'][phase]['confusion_matrix']['index']))
        ...
        for phase in ('training', 'validation'):
            M['metrics'][phase] = {'loss':test_data['metrics']['test']['loss'],  'acc':test_data['metrics']['test']['acc'], 
            ...
            conf_mat = np.array(test_data['metrics']['test']['confusion_matrix']['matrix'])
            evaluation = self.get_metrics(conf_mat, phase_name=f"{self.experiment_name}_test_",
