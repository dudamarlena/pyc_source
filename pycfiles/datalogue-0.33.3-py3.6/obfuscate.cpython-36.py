...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/transformations/obfuscate.py
# Compiled at: 2020-04-14 15:37:55
# Size of source mod 2**32: 10494 bytes
import abc
...
from datalogue.utils import SerializableStringEnum, _parse_string_list, _parse_list
...
class ObfuscationType(abc.ABC):
    ...
    def __init__(self, obfuscation_type):
...
class Obfuscate(Transformation):
    ...
    def __init__(self, targets: List[Union[(ByClass, ByFieldName)]], obfuscation_type: ObfuscationType):
        """
    ...
    def __repr__(self):
        return f"Obfuscate(targets: {self.targets})obfuscation_type: {self.obfuscation_type})"
    ...
    def _as_payload(self) -> dict:
        ...
        for elem in self.targets:
            if isinstance(elem, ByClass):
                ...
                if isinstance(elem, ByFieldName):
...
class Encrypt(ObfuscationType):
    ...
    def __init__(self, key: str, encryption_algorithm: EncryptionAlgorithm=EncryptionAlgorithm.AES_256):
        """
    ...
    def __repr__(self):
        return f"Encrypt(encryption_algorithm: {self.encryption_algorithm}), key: {self.key}"
...
class Decrypt(Transformation):
    ...
    def __init__(self, key: str, targets: List[Union[(ByClass, ByFieldName)]], encryption_algorithm: EncryptionAlgorithm=EncryptionAlgorithm.AES_256):
        """
    ...
    def __repr__(self):
        return f"Decrypt(targets: {self.targets},encryption_algorithm: {self.encryption_algorithm})"
...
class Replace(ObfuscationType):
    ...
    def __init__(self, replacement: str, left_offset: Optional[int]=None, right_offset: Optional[int]=None):
        """
    ...
    def __repr__(self):
        return f"Replace(replacement: {self.replacement}, left_offset: {self.left_offset!r}, right_offset: {self.right_offset!r})"
...
class Hash(ObfuscationType):
    ...
    def __init__(self, hash_algorithm: HashAlgorithm=HashAlgorithm.SHA3_512, salt_hash: Optional[str]=None):
        """
    ...
    def __repr__(self):
        return f"Hash(hash_algorithm: {self.hash_algorithm}, salt_hash: {self.salt_hash})"
