...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/clients/automation.py
# Compiled at: 2020-04-14 15:37:55
# Size of source mod 2**32: 19904 bytes
...
class _RegexClient:
    ...
    def create(self, regex: Regex) -> Union[(DtlError, Regex)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/regexes', HttpMethod.POST, Regex._as_payload(regex))
        if isinstance(res, DtlError):
            return res
        else:
            return Regex._from_payload(res)

    def update(self, id: UUID, name: Optional[str]=None, description: Optional[str]=None, pattern: Optional[str]=None) -> Union[(Regex, DtlError)]:
        """
        ...
        res = self.http_client.make_authed_request(self.service_uri + '/regexes/' + str(id), HttpMethod.PUT, req_body)
        if isinstance(res, DtlError):
            return res
        else:
            return Regex._from_payload(res)
 ...
 SharePermission, DtlError)]:
    ...
    def search(self, name: Optional[str]=None, description: Optional[str]=None, test_data: Optional[str]=None, match: Optional[str]=None, owner: Optional[UUID]=None, page: int=1, size: int=25, order: SearchOrder=SearchOrder.created_at_desc) -> Union[(List[Regex], DtlError)]:
        """
        ...
        res = self.http_client.make_authed_request((self.service_uri + '/regexes'),
          (HttpMethod.GET),
          params=req_params)
        if isinstance(res, DtlError):
            return res
    ...
    def test(self, id: UUID, data: List[str]) -> Union[(List[RegexTestSample], DtlError)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/regexes/' + str(id) + '/match', HttpMethod.PUT, data)
        if isinstance(res, DtlError):
            return res
        else:
    ...
    def add_test_data(self, id: UUID, test_data: List[str]) -> Union[(Regex, DtlError)]:
        ...
        :return: Regex reference with more information id, owner, regex test sample statuses
        """
        res = self.http_client.make_authed_request(self.service_uri + '/regexes/' + str(id) + '/test-data', HttpMethod.PUT, test_data)
        if isinstance(res, DtlError):
            return res
        else:
            return Regex._from_payload(res)
    ...
    def remove_test_data(self, id: UUID, test_data: List[str]) -> Union[(Regex, DtlError)]:
        ...
        :return: Regex reference with more information id, owner, regex test sample statuses
        """
        res = self.http_client.make_authed_request(self.service_uri + '/regexes/' + str(id) + '/test-data', HttpMethod.DELETE, test_data)
        if isinstance(res, DtlError):
            return res
        else:
            return Regex._from_payload(res)
    ...
    def get(self, id: Union[(str, UUID)]) -> Union[(DtlError, Regex)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/regexes/' + str(id), HttpMethod.GET)
        if isinstance(res, DtlError):
            return res
        else:
            return Regex._from_payload(res)
    ...
    def delete(self, regex_id: Union[(str, UUID)]) -> Union[(DtlError, bool)]:
        ...
        """
        resp = self.http_client.execute_authed_request(self.service_uri + '/regexes/' + str(regex_id), HttpMethod.DELETE)
        if isinstance(resp, DtlError):
            return resp
        else:
            if resp.status_code == 200:
...
class _ClassifierClient:
    ...
    def _fill_domain(self, classifier: Classifier, class_ids: List[str]):
        ...
        for class_id in class_ids:
            ...
            if isinstance(res, DtlError):
                res = OntologyNode(name='Permission denied',
    ...
    def create(self, classifier: Classifier) -> Union[(DtlError, Classifier)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/classifiers', HttpMethod.POST, classifier._as_payload())
        if isinstance(res, DtlError):
            return res
        else:
    ...
    def update(self, id: UUID, name: Optional[str]=None, description: Optional[str]=None, classification_methods: Optional[List[ClassificationMethod]]=None, default_class_id: Optional[UUID]=None) -> Union[(
 DtlError, Classifier)]:
        """
        ...
        if default_class_id is not None:
            if default_class_id == '' or is_valid_uuid(default_class_id):
                ...
                raise DtlError('default_class_id can either be None, empty string, or in UUID format. Other formats are not allowed.')
            req_body['defaultClassId'] = str(default_class_id)
        res = self.http_client.make_authed_request(self.service_uri + '/classifiers/' + str(id), HttpMethod.PUT, req_body)
    ...
    def get(self, id: Union[(str, UUID)]) -> Union[(DtlError, Classifier)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/classifiers/' + str(id), HttpMethod.GET)
        if isinstance(res, DtlError):
            return res
        else:
            new_classifier = Classifier._from_payload(res)
    ...
    def delete(self, classifier_id: Union[(str, UUID)]) -> Union[(DtlError, bool)]:
        ...
        :return: True if successful, or DtlError if failed
        """
        res = self.http_client.make_authed_request(self.service_uri + '/classifiers/' + str(classifier_id), HttpMethod.DELETE)
        if isinstance(res, DtlError):
            return res
        else:
            return True
 ...
 SharePermission, DtlError)]:
    ...
    def list(self, page: int=1, item_per_page: int=25) -> Union[(DtlError, List[Regex])]:
        ...
        res = self.http_client.make_authed_request((self.service_uri + '/classifiers'),
          (HttpMethod.GET),
          params=req_params)
        if isinstance(res, DtlError):
            return res
...
class _PasswordClient:
    ...
    def generate_key(self, password: str, encryption_algorithm: EncryptionAlgorithm=EncryptionAlgorithm.AES_256) -> Union[(DtlError, str)]:
        """
    ...
    def generate_salt(self, password: str) -> Union[(DtlError, str)]:
        """
