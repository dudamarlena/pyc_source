...
class _JobsClient:
    ...
    def list(self, page: int=1, item_per_page: int=25) -> Union[(DtlError, List[Job])]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + '/stream-collections/schedules', HttpMethod.GET)
    ...
    def cancel(self, job_id: UUID) -> Union[(DtlError, Job)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + f"/v2/jobs/{str(job_id)}/cancel", HttpMethod.POST)
        if isinstance(res, DtlError):
            return res
        else:
    ...
    def get(self, job_id: UUID) -> Union[(DtlError, Job)]:
        ...
        """
        res = self.http_client.make_authed_request(self.service_uri + f"/v2/jobs/{str(job_id)}/status", HttpMethod.GET)
        if isinstance(res, DtlError):
            return res
        else:
            return _job_from_payload(res)