...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/models/training.py
# Compiled at: 2020-04-14 15:37:55
# Size of source mod 2**32: 52138 bytes
...
import itertools, pandas as pd, datetime
from collections import namedtuple
from datalogue.errors import _enum_parse_error, DtlError
from datalogue.utils import _parse_list, _parse_string_list, SerializableStringEnum, _parse_uuid, map_option
...
class MetricsDetails:
    ...
    def _from_payload(json: dict) -> Union[(DtlError, 'MetricsDetails')]:
        ...
        if not isinstance(f1_score, dict):
...
class TrainingState:
    ...
    def __repr__(self):
        return f"(id: {self.id}, ontology_id: {self.ontology_id}, start_time: {self.start_time}, status: {self.status}, requested_by: {self.requested_by}, user_id: {self.user_id} details: {self.details}, number_of_epochs: {self.number_of_epochs},model_type: {self.model_type}, epochs: {self.epochs}, test_metrics: {self.test_metrics})"
    ...
    def _from_payload(json: dict) -> Union[(DtlError, 'TrainingState')]:
        ...
        if test_metrics is not None:
            ...
            return isinstance(user_id, str) or DtlError("A 'userId' field is missing")
        else:
        ...
        for epoch_data in json_epochs:
            for epoch_data_metric in epoch_data.metrics:
                if epoch_data_metric.metrics_type == 'validation':
                    ...
                    df = pd.DataFrame((cm.matrix), columns=(cm.index), index=(cm.index))
    ...
    def _build_list_with_gral_metrics(res_object, metric):
        ...
        list_metrics.append(float(res_object['test']['metrics'].loc[(res_object['test']['metrics']['index'] == metric + '_test_set')].iloc[:,
         1:].mean(axis=1)))
        return list_metrics
    ...
    def _from_training_payload(training_state: 'TrainingState') -> tuple:
        ...
        if res_object['description'].loc[('end_reason', 'Description')] == 'Successful':
            ...
            res_object['validation']['metrics'], res_object['validation']['cm'] = TrainingState._create_validation_confusion_matrices_and_metrics(training_state.epochs)
            res_object['test']['cm'], res_object['test']['metrics'] = TrainingState._create_test_cm_and_metrics(training_state.test_metrics)
            ...
            if training_state.status == 'Ended' and res_object['description'].loc[('end_reason',
...
class Training:
    ...
    def _from_payload(payload: dict) -> Union[(DtlError, 'Training')]:
        ...
        if states is not None:
            ...
            states = list()
        return Training(id=id, states=states)
...
class DeploymentStatus:
    ...
    def _from_payload(json: dict) -> Union[(DtlError, 'DeploymentStatus')]:
        ...
        if status_type is None:
            return DtlError("'_type' field is missing")
        else:
...
class Deployment:
    ...
    def __init__(self, id: UUID, ontology_id: UUID, training_id: UUID, model_type: ModelType, status: DeploymentStatus, created_at: datetime):
        self.id = id
        self.ontology_id = ontology_id
    ...
    def __repr__(self):
        return f"Deployment(id: {self.id}, ontology_id: {self.ontology_id}, training_id: {self.training_id},model_type: {str(self.model_type)}, status: {str(self.status)}, created_at: {self.created_at})"

    @staticmethod
    def _from_payload(json: dict) -> Union[(DtlError, 'Deployment')]:
        id = map_option(json.get('id'), _parse_uuid)
        if id is None:
            return DtlError("'id' field is missing")
        if isinstance(id, DtlError):
            return id
        ontology_id = map_option(json.get('ontologyId'), _parse_uuid)
        if ontology_id is None:
            return DtlError("'ontologyId' field is missing")
        if isinstance(ontology_id, DtlError):
            return ontology_id
        training_id = map_option(json.get('trainingId'), _parse_uuid)
        if training_id is None:
            return DtlError("'trainingId' field is missing")
        if isinstance(training_id, DtlError):
            return training_id
        model_type = json.get('modelType')
        if model_type is None:
            return DtlError("'modelType' field is missing")
        model_type = model_type_from_str(model_type)
        if isinstance(model_type, DtlError):
            return model_type
        status = json.get('status')
        if status is None:
            return DtlError("'status' field is missing")
        status = DeploymentStatus._from_payload(status)
        if isinstance(status, DtlError):
            return status
        created_at = json.get('createdAt')
        if created_at is None:
            return DtlError("'createdAt' field is missing")
        try:
            created_at = parse(created_at)
        except ValueError:
            return DtlError("'createdAt' could not be parsed as a valid date")
        else:
            return Deployment(id, ontology_id, training_id, model_type, status, created_at)


class Annotation:

    def __init__(self, id: UUID, token: str, start_char: int, end_char: int, class_id: UUID, timestamp: datetime, created_by: UUID):
        self.id = id
        self.token = token
...
class Tag:
    ...
    @staticmethod
    def from_payload(json: dict) -> Union[(DtlError, 'Tag')]:
        ...
        if not isinstance(raw_tag, str):
            ...
            return Tag(start_token, end_token, tag)
...
class OriginalSection:
    ...
    def __init__(self, section_id: UUID, datastore_id: UUID, source_path: List[str], tokens: List[str], token_types: List[str], rejected: bool, tags: List[List[Tag]]):
    ...
    def __repr__(self):
        return f"Section(section_id: {self.section_id}, datastore_id: {self.datastore_id}, source_path: {self.source_path},tokens: {self.tokens},token_types: {self.token_types},rejected: {self.rejected},tags: {self.tags},)"
        ...
        if not isinstance(rejected, bool):
            ...
            for annotation in raw_tags:
                if not isinstance(annotation, List):
                    return DtlError("A 'tags' field is missing")
                annotation_tags = _parse_list(Tag.from_payload)(annotation)
    ...
    def replace_tag_by(self, test: Callable[([Tag], bool)], generate_tokens: Callable[([], List[Token])]) -> List['SynthesizedSection']:
        """
        Create new sections
    ...
    def replace_tag_by_many(self, test: Callable[([Tag], bool)], generate_tokens: Callable[([], List[List[Token]])]) -> List['SynthesizedSection']:
        """
        Create new sections
        replace tag when 'test' is true using 'generate_tokens' method to replace original tokens in the section
        Returned list of NewSections to be created as augmented data.
        ...
        def replace_tag(section, tag, all_tags):
            ...
            for token_definition in new_tokens_definition:
                ...
                def adjust_tags_positions(t):
                    return Tag(t.start_token + length_diff, t.end_token + length_diff, t.tag)

                after_tags = [adjust_tags_positions(t) for t in after_tags]
                tag_sections.append(SynthesizedSection(None, section.section_id, section.datastore_id, section.source_path, tokens, token_types, section.rejected, before_tags + [Tag(tag.start_token, tag.start_token + len(new_tokens), tag.tag)] + after_tags))
                return tag_sections
    ...
    def __eq__(self, other):
        if isinstance(other, OriginalSection):
            return self.section_id == other.section_id and self.datastore_id == other.datastore_id and self.source_path == other.source_path and self.tokens == other.tokens and self.token_types == other.token_types and self.tags == other.tags
        else:
            return False
...
class SynthesizedSection:
    ...
    def __init__(self, section_id: Optional[UUID], original_id: UUID, datastore_id: UUID, source_path: List[str], tokens: List[str], token_types: List[str], rejected: bool, tags: List[Tag]):
        self.section_id = section_id
    ...
    def __repr__(self):
        return f"Section(section_id: {self.section_id}, datastore_id: {self.datastore_id}, source_path: {self.source_path},tokens: {self.tokens},token_types: {self.token_types},rejected: {self.rejected},tags: {self.tags},original_id: {self.original_id})"
    ...
    def from_payload(json: dict) -> Union[(DtlError, 'SynthesizedSection')]:
        ...
        if not isinstance(rejected, bool):
            ...
            if not isinstance(raw_tags, List):
                return DtlError("A 'tags' field is missing")
            tags = _parse_list(Tag.from_payload)(raw_tags)
    ...
    def __eq__(self, other):
        if isinstance(other, SynthesizedSection):
            return self.section_id == other.section_id and self.original_id == other.original_id and self.datastore_id == other.datastore_id and self.source_path == other.source_path and self.tokens == other.tokens and self.token_types == other.token_types and self.tags == other.tags
        else:
            return False

    def _as_payload(self):
        return {'datastoreId':str(self.datastore_id), 
...
class SynthesisStats:
    ...
    def __init__(self, generated_sections: int, processed_sections: int, avg_number_generated_sections: float, std_number_generated_section: float):
        """
    ...
    def __eq__(self, other):
        if isinstance(other, SynthesisStats):
            return self.generated_sections == other.generated_sections and self.processed_sections == other.processed_sections and self.avg_number_generated_sections == other.avg_number_generated_sections and self.std_number_generated_section == other.std_number_generated_section