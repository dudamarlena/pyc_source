...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tropopause/cloudformation.py
...
def FilesDecorator(func):
    ...
    def wrapper(*args, **kwargs):
        kwargs['files'] = InitFiles({'/etc/cfn/cfn-hup.conf':InitFile(content=Join('', [
          '[main]\n',
          'stack=',
          Ref('AWS::StackId'),
          '\n',
          ...
          '\n']),
           ...
           owner='root',
           group='root'), 
         '/etc/cfn/hooks.d/cfn-auto-reloader.conf':InitFile(content=Join('', [
...
def ServicesDecorator(func):
    ...
    def wrapper(*args, **kwargs):
        kwargs['services'] = {'sysvinit': InitServices({'cfn-hup': InitService(ensureRunning='true',
                                   enabled='true',
                                   files=[
...
def CommandsDecoratorHTTP(func):
    ...
    def wrapper(*args, **kwargs):
        if 'url' in kwargs:
            if valid_url(kwargs['url']):
                kwargs['commands'] = {'01_bootstrap_from_http': {'command': '/usr/bin/curl -s ' + kwargs['url'] + ' | /bin/sh'}}
        return func(*args, **kwargs)
...
def CommandsDecoratorS3(func):
    ...
    def wrapper(*args, **kwargs):
        if 'url' in kwargs:
            kwargs['commands'] = {'01_bootstrap_from_s3': {'command': '/usr/bin/aws s3 cp ' + kwargs['url'] + ' - | /bin/sh'}}
        return func(*args, **kwargs)
...
class InitConfigFromHTTP(InitConfig):
    __doc__ = ' Bootstrap an instance from HTTP(S) '
...
class InitConfigFromS3(InitConfig):
    __doc__ = ' Bootstrap an instance from S3 '
