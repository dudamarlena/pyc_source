...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tropopause/ec2.py
# Compiled at: 2018-11-06 11:27:11
# Size of source mod 2**32: 7436 bytes
from tropopause import Tags
from troposphere import GetAtt, Ref, Template
from troposphere import Tags as upstreamTags
from troposphere.ec2 import EIP, Subnet, VPC, InternetGateway, NatGateway, Route, RouteTable, SubnetRouteTableAssociation, VPCPeeringConnection, VPCGatewayAttachment, SecurityGroup, SecurityGroupRule, SecurityGroupIngress
import yaml
...
def AddTagsFromVPC(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if isinstance(args[(-1)], Template):
            vpc = next(i for i in args[(-1)].resources.items() if isinstance(i[(-1)], VPC))[(-1)]
        if isinstance(vpc, VPC):
            tags = tags + vpc.properties['Tags']
        if 'Tags' in kwargs:
            if isinstance(kwargs['Tags'], upstreamTags):
...
class InternetGatewayVPC(VPC):
    __doc__ = ' Overrides the standard Troposphere VPC to deal with boilerplate '
...
class PublicSubnet(Subnet):
    __doc__ = ' Overrides Subnet, creates a NAT gateway '
...
class PrivateSubnet(Subnet):
    __doc__ = ' Overrides Subnet, routes traffic through an existing NAT gateway '
...
class SecureSubnet(Subnet):
    __doc__ = ' Overrides Subnet, no route to the Internet '
...
class SecurityGroupFromYaml(SecurityGroup):
    __doc__ = ' Allows for a config yaml to be passed in instead of Ingress/Egress '
                        ...
                        for cidr in doc[ruleset]['cidr']:
                            for protocol in doc[ruleset]['protocols']:
                                for port in doc[ruleset]['protocols'][protocol]:
                                    sgrules.append(SecurityGroupRule((''.join(ch for ch in cidr + protocol + port if ch.isalnum())),
                                      CidrIp=cidr,
                                      IpProtocol=protocol,
                                      FromPort=port,
                                      ToPort=port))
    ...
    def __init__(self, title, template, *args, **kwargs):
        if 'SecurityGroupIngress' in kwargs:
            if isinstance(kwargs['SecurityGroupIngress'], str):
                kwargs['SecurityGroupIngress'] = self._rulesFromYaml(kwargs['SecurityGroupIngress'])
        if 'SecurityGroupEgress' in kwargs:
            if isinstance(kwargs['SecurityGroupEgress'], str):
                kwargs['SecurityGroupEgress'] = self._rulesFromYaml(kwargs['SecurityGroupEgress'])
        (super().__init__)(title, template, *args, **kwargs)