...
# [GCC 8.4.0]
# Embedded file name: build-py2\bdist.egg\xl\_impl\collapsed_matrix.py
...
class CollapsedMatrix(object):
    ...
    def collapsed_data(self, val):
        ...
        for row in val:
            if not len(row) == self.num_columns:
                raise ValueError('All rows given must have a length equal to the number of present columns')
