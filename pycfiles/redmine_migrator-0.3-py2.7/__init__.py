...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/redmine_migrator/__init__.py
# Compiled at: 2013-05-15 08:47:59
import argparse
...
def main():
    ...
    pg_tables = pg.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name").fetchall()
    metadata = []
    for table in pg_tables:
        table_name = table.table_name
        columns = pg.execute('SELECT column_name FROM information_schema.columns WHERE table_name = :table_name', {'table_name': table_name}).fetchall()
...
def update_pk_sequence(pgconn, table_name):
    ...
    :rtype: bool
    """
    current_seq_value = pgconn.execute(('SELECT max(id) FROM {table_name}').format(table_name=table_name)).fetchone()[0]
...
def insert_statement(table_name, columns, data=None):
    ...
    return ('INSERT INTO {table_name} ({columns_list}) VALUES ({values_list})').format(table_name=table_name, columns_list=(', ').join(columns_list), values_list=(', ').join(values_list))
...
def update_statement(table_name, columns, data=None):
    ...
    return ('UPDATE {table_name} SET {columns_list} WHERE id = :id').format(table_name=table_name, columns_list=(', ').join(columns_list))
...
def update_pk_sequence_statement(table_name, currval=None):
    ...
    if currval is None:
        currval = 0
    return ('ALTER SEQUENCE {sequence_name} RESTART WITH {nextval}').format(sequence_name=sequence_name(table_name), nextval=currval + 1)
...
def handle_standard_table(pgconn, table_name, columns, record):
    ...
    if 'id' in columns:
        data_exists = pgconn.execute(('SELECT 1 FROM {table_name} WHERE id = :id').format(table_name=table_name), {'id': data['id']}).fetchone()
        if data_exists:
...
def handle_schema_migrations(pgconn, table_name, columns, record):
    ...
    data_exists = pgconn.execute('SELECT 1 FROM schema_migrations WHERE version = :version', {'version': record.version}).fetchone()
    if data_exists:
        return True
...
NON_STANDARD_TABLES = {'schema_migrations': handle_schema_migrations, 
