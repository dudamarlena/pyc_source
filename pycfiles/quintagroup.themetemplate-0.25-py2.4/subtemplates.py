...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quintagroup/themetemplate/localcommands/subtemplates.py
...
class SkinLayerSubTemplate(QThemeSubTemplate):
    ...
    vars = [
     var('layer_name', 'Skin Layer name (should not contain spaces)', default='skin_layer'), var('insert_type', 'Where insert the layer ("after" or "before")', default='after'),
     var('insert_control_layer', 'Layer after or before which your layer will be inserted, "*" accepted, which mean all', default='custom')]
...
class CSSSubTemplate(QThemeSubTemplate):
    ...
    vars = [
     var('css_resource_name', 'Name of CSS resource', default='main.css'), var('css_file_path', 'Path to CSS file. If no path-empty file will be created.', default=''), var('cssreg_media', 'Optional.Possible values:screen,print,projection,handheld', default='screen'), var('cssreg_rel', 'Optional', default='stylesheet'), var('cssreg_rendering', 'Optional.Possible values:import,link,inline', default='inline'), var('cssreg_cacheable', '', default='True'), var('cssreg_compression', 'Compression type', default='safe'), var('cssreg_cookable', 'Boolean, aka merging allowed', default='True'), var('cssreg_enables', 'Optional.Boolean', default='1'), var('cssreg_expression', 'Optional.A tal condition.', default='')]
...
class CSSSkinLayerSubTemplate(CSSSubTemplate):
    ...
    vars = [
     var('layer_name', 'Layer name for css resource add to', default='skin_layer')] + copy.deepcopy(CSSSubTemplate.vars)
...
class JSSubTemplate(QThemeSubTemplate):
    ...
    vars = [
     var('js_resource_name', 'Name of JS resource', default='foo.js'), var('js_file_path', 'Path to JS file. If no path - empty file will be created', default=''), var('jsreg_inline', 'Optional.Boolean', default='False'), var('jsreg_cacheable', '', default='True'), var('jsreg_compression', 'Compression type.Possible:none,safe,full,safe-encode,full-encode', default='safe'), var('jsreg_cookable', 'Boolean, aka merging allowed', default='True'), var('jsreg_enables', 'Optional.Boolean', default='1'), var('jsreg_expression', 'Optional.A tal condition.', default='')]
...
class ViewletOrderSubTemplate(QThemeSubTemplate):
    ...
    compo_template_markers = []
    shared_vars = [
     'viewlet_profile_marker']
    vars = [
     var('viewlet_name', 'Viewlet name', default='example'), var('viewlet_manager_interface', 'Viewlet manager interface', default='plone.app.layout.viewlets.interfaces.IPortalHeader'), var('viewlet_manager_name', 'Viewlet manager name', default='plone.portalheader'), var('viewlet_permission', 'Viewlet permission', default='zope2.View'), var('insert_type', 'Where insert the viewlet ("after" or "before")', default='after'), var('insert_control_viewlet', 'Viewlet after or before which your viewlet will be inserted, "*" accepted, which mean all', default='*'), var('layer_interface', 'Layer interface for registry this viewlet on', default='.interfaces.IThemeSpecific'), var('layer_name', 'Layer name for registry this viewlet on', default='')]
    ...
    def pre(self, command, output_dir, vars):
        ...
        viewlet_profile_marker = '[order_%s] viewlet stuff goes here' % ('.').join([vars['viewlet_manager_name'], vars['qplone3_theme_skinname'], vars['qplone3_theme_skinbase']])
        ...
        self.compo_template_markers.append(('viewlet_profiles', viewlet_profile_marker))
    ...
    def add_order_tag(self, output_dir, vars, pmarker):
        ...
        if os.path.exists(theme_vars_fp):
            ...
            used_subtemplates = filter(None, [ st.strip() for st in config.get(sec, opt).split(',') ])
            if self.name in used_subtemplates:
                sections = [
                 self.name]
                if config.has_section('multiple_templates') and config.has_option('multiple_templates', self.name):
                    ms_sections = config.get('multiple_templates', self.name)
                    sections = [ s.strip() for s in ms_sections.split(',') ]
                pmarkers = [ config.get(sec, 'viewlet_profile_marker') for sec in sections ]
...
class ViewletHiddenSubTemplate(QThemeSubTemplate):
    ...
    compo_template_markers = []
    vars = [
     var('viewlet_name', 'Viewlet name', default='plone.global_sections'), var('viewlet_manager_name', 'Viewlet manager name', default='plone.portalheader')]

    def pre(self, command, output_dir, vars):
        """ Set 'css_resource_content' value from css_file_path
        """
        super(ViewletHiddenSubTemplate, self).pre(command, output_dir, vars)
        viewlet_profile_marker = '[hidden_%s] viewlet stuff goes here' % ('.').join([vars['viewlet_manager_name'], vars['qplone3_theme_skinname']])
        vars['add_hidden_tag'] = self.add_hidden_tag(output_dir, vars, viewlet_profile_marker)
        vars['viewlet_profile_marker'] = viewlet_profile_marker
        self.compo_template_markers.append(('viewlet_hidden_profiles', viewlet_profile_marker))
