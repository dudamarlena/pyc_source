...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/postprocess/files_post_handler/postprocess.py
...
class FilesPostHandler(Component):
    ...
    def __init__(self, config_node, process_name):
        ...
        if not self.output_folder:
            raise ValueError('You must set an output folder to copy output files')
        if not os.path.exists(self.output_folder):
            raise ValueError('Output directory %s does not exist' % self.output_folder)
    ...
    def launch(self, values, out):
        for event in values:
            if event.output_files:
                if self.to_zip:
                    zip_target_filename = os.path.join(self.output_folder, self.__get_zip_target_filename(event))
                    zarchive = zipfile.ZipFile(zip_target_filename, mode='w', compression=zipfile.ZIP_DEFLATED)
                for output_file in event.output_files:
                    storage_filename = self.storage.get_filename(output_file.file_uuid)
                    if self.to_zip:
                        zarchive.write(storage_filename, output_file.filename)
                    else:
                        target_filename = os.path.join(self.output_folder, output_file.filename)
                        shutil.copy(storage_filename, target_filename)
                    yield True

                if self.to_zip:
                    zarchive.close()
                    self.message_callback('Files were added to the %s archive' % zip_target_filename, message_type='success')
                else:
                    self.message_callback('Files were copied to the %s folder' % self.output_folder, message_type='success')
            yield True