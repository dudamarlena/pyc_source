...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/filters.py
...
class geotime(Filter):
    ...
    param_schema = dict(Filter.param_schema, **{'min_lat': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'max_lat': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'min_lon': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'max_lon': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'min_alt': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'max_alt': {'type': 'float', 'min': -360.0, 'max': 360.0}, 'min_time': {'type': 'float', 'min': 0, 'max': 4294967295}, 'max_time': {'type': 'float', 'min': 0, 'max': 4294967295}})

    def __init__(self, geotime_feature_name=FC.GEOCOORDS_PREFIX + 'both_co_LOC_1'):
    ...
    def create_predicate(self):
        ...
        def pred((cid, fc)):
            ...
            return any(any(imap(in_bbox, coords)) for _, coords in feature.iteritems())
...
class nilsimsa_near_duplicates(Filter):
    ...
    def __init__(self, label_store, store, nilsimsa_feature_name='#nilsimsa_all', threshold=0.9):
        self.label_store = label_store
        self.store = store
        self.nilsimsa_feature_name = nilsimsa_feature_name
        self.threshold = threshold
        logger.info('nilsimsa_feature_name=%r and threshold=%r', nilsimsa_feature_name, threshold)
    ...
    def create_predicate(self):
        ...
        def accumulating_predicate((content_id, fc)):
            ...
            for hash1, hash2 in product(sim_feature, accumulator):
                score = nilsimsa.compare_digests(hash1, hash2, threshold=self.threshold)
                score /= 128.0
