...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/tests/test_filter_preds.py
...
def test_nilsimsa_near_duplicates_basic(label_store, store):
    fcs = [ (str(idx), make_fc(text)) for idx, text in enumerate(near_duplicate_texts)
          ]
    query_content_id, query_fc = fcs.pop(0)
    store.put([(query_content_id, query_fc)])
    accumulating_predicate = nilsimsa_near_duplicates(label_store, store, threshold=0).set_query_id(query_content_id).create_predicate()
    assert len(filter(accumulating_predicate, fcs)) == 0


def test_nilsimsa_near_duplicates_update_logic(label_store, store):
    fcs = [ (str(idx), make_fc(text)) for idx, text in enumerate(chain(*repeat(near_duplicate_texts, 1000)))
          ]
    query_content_id, query_fc = fcs.pop(0)
    store.put([(query_content_id, query_fc)])
    accumulating_predicate = nilsimsa_near_duplicates(label_store, store, threshold=120).set_query_id(query_content_id).create_predicate()
    start = time.time()
...
def test_nilsimsa_near_duplicates_speed_perf(label_store, store, num_texts=5, num_exact_dups_each=10, num_near_dups_each=10):
    ...
    query_content_id, query_fc = fcs.pop(0)
    store.put([(query_content_id, query_fc)])
    accumulating_predicate = nilsimsa_near_duplicates(label_store, store, threshold=0.85).set_query_id(query_content_id).create_predicate()
    start = time.time()
    results = filter(accumulating_predicate, fcs)
    elapsed = time.time() - start
    print '%d filtered to %d in %f seconds, %f per second' % (
     len(fcs), len(results), elapsed, len(fcs) / elapsed)
    assert len(results) == num_texts - 1
...
def test_geotime_filter():
    ...
    fc3 = FC()
    fc3[fname] = gc3
    pred = geotime().set_query_params({'min_lat': 0, 
       'max_lat': 20, 'min_lon': -20, 
       'max_lon': 0, 'min_time': 0}).create_predicate()
