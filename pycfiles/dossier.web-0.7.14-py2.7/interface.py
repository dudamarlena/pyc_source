...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/interface.py
    ...
    def apply_param_schema(self):
        ...
        for name, schema in getattr(self, 'param_schema', {}).iteritems():
            ...
            if schema['type'] == 'bool':
                ...
                v = param_num(name, int, default=default, minimum=schema.get('min', 0), maximum=schema.get('max', 1000000))
            elif schema['type'] == 'float':
                v = param_num(name, float, default=default, minimum=schema.get('min', 0), maximum=schema.get('max', 1000000))
...
class SearchEngine(Queryable):
    ...
    param_schema = {'limit': {'type': 'int', 'default': 30, 'min': 0, 'max': 1000000}, 'omit_fc': {'type': 'bool', 'default': 0}}
    ...
    def create_filter_predicate(self):
        ...
        """
        assert self.query_content_id is not None, 'must call SearchEngine.set_query_id first'
        ...
        for name, p in init_filters:
            preds.append(p.set_query_id(self.query_content_id).set_query_params(self.query_params).create_predicate())

        return lambda (cid, fc): fc is not None and all(p((cid, fc)) for p in preds)
