...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/builder.py
...
import inspect, json, logging, bottle
...
class WebBuilder(object):
    ...
    def __init__(self, add_default_routes=True):
        ...
        self.search_engines = {'random': builtin_engines.random, 
           'plain_index_scan': builtin_engines.plain_index_scan}
        self.filters = {'already_labeled': already_labeled}
    ...
    def enable_cors(self):
        ...
        def options_response(res):
            if bottle.request.method == 'OPTIONS':
                ...
                new_res.headers['Access-Control-Allow-Methods'] = bottle.request.headers.get('Access-Control-Request-Method', '')
                new_res.headers['Access-Control-Allow-Headers'] = bottle.request.headers.get('Access-Control-Request-Headers', '')
                return new_res
...
def add_cli_arguments(p):
    p.add_argument('--bottle-debug', action='store_true', help="Enable Bottle's debug mode.")
    p.add_argument('--reload', action='store_true', help="Enable Bottle's reloading functionality.")
    p.add_argument('--port', type=int, default=8080)
    p.add_argument('--host', default='localhost')
    p.add_argument('--server', default='wsgiref', help="The web server to use. You only need to change this if you're running a production server.")