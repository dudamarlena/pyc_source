...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/label_folders.py
...
class Folders(object):
    ...
    def folders(self, ann_id=None):
        ...
        return imap(lambda id: self.unwrap_folder_content_id(id)['folder_id'], self.store.scan_prefix_ids(prefix))
    ...
    def add_item(self, folder_id, subfolder_id, content_id, subtopic_id=None, ann_id=None):
        ...
        if self.store.get(folder_cid) is None:
            raise KeyError(folder_id)
        lab = Label(folder_cid, content_id, ann_id, CorefValue.Positive, subtopic_id1=subfolder_sid, subtopic_id2=subtopic_id)
    ...
    def unwrap_folder_content_id(self, cid):
        ...
        return {'annotator_id': urllib.unquote(annotator_id), 'folder_id': urllib.unquote(fid)}
