...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/folder.py
...
class Folders(object):
    ...
    def key(self, inode):
        return (
         self.namespace, self.owner, long(inode))
    ...
    def root(self):
        try:
            return self.from_inode(0, path='/')
        except KeyError:
            root = Folder(namespace=self.namespace, owner=self.owner, inode=0, path='/', meta_data={}, data='')
        ...
        for is_stem, prev, component in path_components(path):
            ...
            try:
                ...
                if is_stem:
                    ...
                    if child.is_item():
                        self.kvl.put(self.TABLE, Folders.to_kvlayer(child._replace(data=data)))
            except KeyError:
                factory = Item if is_stem and not is_folder else Folder
                child = factory(namespace=self.namespace, owner=self.owner, inode=self.fresh_inode(), path=None, meta_data={}, data=data)
    ...
    def from_kvlayer(key, data, path=None):
        ...
        if data.get('is_folder', False):
            factory = Folder
        return factory(namespace=uni(key[0]), owner=uni(key[1]), inode=key[2], path=path, meta_data=meta_data, data=user_data)

    @staticmethod
    def to_kvlayer(item):
        data = cbor.dumps({'is_folder': item.is_folder(), 
           'meta_data': item.meta_data, 
           'user_data': item.data})
        return (
         (
          utf8(item.namespace), utf8(item.owner), item.inode), data)
