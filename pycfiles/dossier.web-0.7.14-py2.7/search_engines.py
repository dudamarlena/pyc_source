...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/search_engines.py
...
class random(SearchEngine):
    ...
    def recommendations(self):
        if 'NAME' not in self.store.index_names():
            return {'results': []}
        else:
            ...
            cids = []
            for name in fc.get('NAME', {}):
                cids.extend(self.store.index_scan_ids('NAME', name))
...
class plain_index_scan(SearchEngine):
    ...
    def recommendations(self):
        ...
        results = ifilter(predicate, ((cid, self.store.get(cid)) for cid in cids))
        sample = streaming_sample(results, self.params['limit'], self.params['limit'] * 10)
        ...
        if query_fc is None:
            ...
            for idx_name in self.store.index_names():
                ...
                if isinstance(feat, unicode):
                    logger.info('[Unicode index: %s] scanning for "%s"', idx_name, feat)
