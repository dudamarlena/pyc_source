...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dossier/web/routes.py
...
import os, os.path as path, urllib, urlparse, bottle
...
def v1_search(request, response, visid_to_dbid, config, search_engines, filters, cid, engine_name):
    ...
    * **filter** sets the filtering function. The default
    ...
    search_engine = config.create(search_engine).set_query_id(db_cid).set_query_params(query)
...
def v1_random_fc_get(response, dbid_to_visid, store):
    ...
    return [
     dbid_to_visid(sample[0]), util.fc_to_json(store.get(sample[0]))]


@app.put('/dossier/v1/label/<cid1>/<cid2>/<annotator_id>')
def v1_label_put(request, response, visid_to_dbid, config, label_hooks, label_store, cid1, cid2, annotator_id):
    ...
    lab = Label(visid_to_dbid(cid1), visid_to_dbid(cid2), annotator_id, coref_value, subtopic_id1=request.query.get('subtopic_id1'), subtopic_id2=request.query.get('subtopic_id2'))
    label_store.put(lab)
    response.status = 201
...
def v1_label_connected(request, response, visid_to_dbid, dbid_to_visid, label_store, cid, subid=None):
...
def v1_label_expanded(request, response, label_store, visid_to_dbid, dbid_to_visid, cid, subid=None):
...
def v1_label_negative_inference(request, response, visid_to_dbid, dbid_to_visid, label_store, cid):
    ...
    """
    lab_to_json = partial(label_to_json, dbid_to_visid)
    labs = imap(lab_to_json, label_store.negative_inference(visid_to_dbid(cid)))
    return list(paginate(request, response, labs))
...
def v1_subfolder_list(request, response, kvlclient, fid):
    ...
    fid = urllib.unquote(fid)
    try:
        return sorted(imap(attrgetter('name'), ifilter(lambda it: it.is_folder(), new_folders(kvlclient, request).list(fid))))
    except KeyError:
        response.status = 404
        return []
...
def v1_subfolder_add(request, response, kvlclient, fid, sfid, cid, subid=None):
    ...
    ``annotator_id`` query parameter.)
    """
    assert subid is not None and '@' not in subid
    path = [
     urllib.unquote(fid),
     urllib.unquote(sfid),
...
def v1_folder_delete(request, response, kvlclient, fid, sfid=None, cid=None, subid=None):
...
def v1_folder_rename(request, response, kvlclient, fid_src, fid_dest, sfid_src=None, sfid_dest=None):
...
def assert_valid_folder_id(ident):
    if ' ' in ident or '/' in ident:
        bottle.abort(500, "Folder ids cannot contain spaces or '/' characters.")
