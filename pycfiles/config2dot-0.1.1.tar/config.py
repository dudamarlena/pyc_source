...
# Embedded file name: /Users/grimen/Dev/Private/python-config2/config2/config.py
# Compiled at: 2019-05-09 01:08:22
import rootpath
rootpath.append()
import sys, os, logging, inspect, mybad
...
DEFAULT_CONFIG_FORMATS = [
 'json',
 'yml']
DEFAULT_CONFIG_FORMAT_EXTENSIONS = {'json': 'json', 
   'yaml': 'yaml', 
   'yml': 'yaml'}
DEFAULT_CONFIG_SERIALIZERS = {'json': json, 
   'yaml': yaml}
...
class Config(AttributeDict):
    ...
    def __init__(self, env=None, path=None, config_path=None, config_directory_name=None, logger=False, detect=True, silent=True):
        ...
        if not path:
            try:
                ...
                if PY2:
                    frames = [ inspect.getframeinfo(frame[0]) for frame in inspect.stack() ]
                else:
                ...
                for frame in frames:
                    ...
                    is_not_importlib = 'frozen importlib' not in frame.filename
    ...
    def reload(self):
        try:
            ...
            env_variables_file = dict(enumerate(filter(lambda config_file: config_file.basename == DEFAULT_CONFIG_CUSTOM_ENV_FILE_BASENAME, detected_files))).get(0, None)
            default_config_file = dict(enumerate(filter(lambda config_file: config_file.basename == DEFAULT_CONFIG_DEFAULT_FILE_BASENAME, detected_files))).get(0, None)
            env_config_files = filter(lambda config_file: config_file.basename not in DEFAULT_CONFIG_RESERVED_FILE_BASENAMES, detected_files)
            env_config_files = list(env_config_files)
            env_config_file = dict(enumerate(filter(lambda config_file: config_file.basename == self.__env__, detected_files))).get(0, None)
            config_files = [
             default_config_file] + env_config_files
            config_files = filter(lambda config_file: config_file is not None, [
             default_config_file, env_config_file])
            config_files = list(config_files)
            files = config_files + [env_variables_file]
            files = filter(lambda config_file: config_file is not None, files)
            files = list(files)
            try:
            ...
            config_datas = map(lambda _config_file: _config_file and _config_file.data and _config_file.data.copy() or {}, config_files)
        ...
        try:
            for config_file_name in listdir(config_path):
                ...
                if is_valid_config_file_extension:
                    config_files.append(AttributeDict({'basename': config_file_basename, 
