...
# [GCC 8.4.0]
# Embedded file name: /home/kieffer/workspace/sift_pyocl/build/lib.linux-x86_64-2.7/sift_pyocl/match.py
...
import time, math, os, logging, sys, threading, gc, numpy
...
class MatchPlan(object):
    ...
    """
    kernels = {'matching_gpu': 64, 'matching_cpu': 16, 
       'memset': 128}
    ...
    def match(self, nkp1, nkp2, raw_results=False):
        ...
        with self._sem:
            ...
            if min(kpt1_gpu.size, kpt2_gpu.size) > self.buffers['match'].shape[0]:
                ...
                self.buffers['match'] = pyopencl.array.empty(self.queue, (self.kpsize, 2), dtype=numpy.int32)
            self._reset_output()
            evt = self.programs[self.matching_kernel].matching(self.queue, calc_size((nkp1.size,), (self.kernels[self.matching_kernel],)), (self.kernels[self.matching_kernel],), kpt1_gpu.data, kpt2_gpu.data, self.buffers['match'].data, self.buffers['cnt'].data, numpy.int32(self.kpsize), numpy.float32(par.MatchRatio * par.MatchRatio), numpy.int32(nkp1.size), numpy.int32(nkp2.size))
            if self.profile:
    ...
    def _reset_buffer1(self):
        ev1 = self.programs['memset'].memset_kp(self.queue, calc_size((self.buffers['Kp_1'].size,), (self.kernels['memset'],)), (self.kernels['memset'],), self.buffers['Kp_1'].data, numpy.float32(-1.0), numpy.uint8(0), numpy.int32(self.buffers['Kp_1'].size))
        if self.profile:
            self.events.append(('memset Kp1', ev1))

    def _reset_buffer2(self):
        ev2 = self.programs['memset'].memset_kp(self.queue, calc_size((self.buffers['Kp_2'].size,), (self.kernels['memset'],)), (self.kernels['memset'],), self.buffers['Kp_2'].data, numpy.float32(-1.0), numpy.uint8(0), numpy.int32(self.buffers['Kp_2'].size))
        if self.profile:
            self.events.append(('memset Kp2', ev2))

    def _reset_output(self):
        ev3 = self.programs['memset'].memset_int(self.queue, calc_size((self.buffers['match'].size,), (self.kernels['memset'],)), (self.kernels['memset'],), self.buffers['match'].data, numpy.int32(-1), numpy.int32(self.buffers['match'].size))
        ev4 = self.programs['memset'].memset_int(self.queue, (1, ), (1, ), self.buffers['cnt'].data, numpy.int32(0), numpy.int32(1))
        if self.profile:
