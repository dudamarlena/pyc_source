...
# [GCC 8.4.0]
# Embedded file name: /home/kieffer/workspace/sift_pyocl/build/lib.linux-x86_64-2.7/sift_pyocl/alignment.py
...
class LinearAlign(object):
    ...
    def __init__(self, image, devicetype='CPU', profile=False, device=None, max_workgroup_size=None, ROI=None, extra=0, context=None, init_sigma=None):
        """
        ...
        if self.RGB:
            if self.max_workgroup_size == 1:
                ...
                self.wg = (4, 8, 4)
        elif self.max_workgroup_size == 1:
            self.wg = (1, 1)
        else:
            self.wg = (8, 4)
        self.sift = SiftPlan(template=image, context=self.ctx, profile=self.profile, max_workgroup_size=self.max_workgroup_size, init_sigma=init_sigma)
    ...
    def align(self, img, shift_only=False, return_all=False, double_check=False, relative=False, orsa=False):
        ...
        with self.sem:
            ...
            if double_check and len_match >= 18:
