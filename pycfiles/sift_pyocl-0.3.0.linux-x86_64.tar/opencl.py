...
# [GCC 8.4.0]
# Embedded file name: /home/kieffer/workspace/sift_pyocl/build/lib.linux-x86_64-2.7/sift_pyocl/opencl.py
# Compiled at: 2014-10-24 09:59:44
"""
...
FLOP_PER_CORE = {'GPU': 64, 'CPU': 4, 
   'ACC': 8}
NVIDIA_FLOP_PER_CORE = {(1, 0): 24, (1, 1): 24, 
   (1, 2): 24, 
   (1, 3): 24, 
   (2, 0): 64, 
   (2, 1): 96, 
...
class Device(object):
    ...
    def __init__(self, name='None', dtype=None, version=None, driver_version=None, extensions='', memory=None, available=None, cores=None, frequency=None, flop_core=None, idx=0, workgroup=1):
        """
    ...
    def pretty_print(self):
        ...
        lst = [
         'Name\t\t:\t%s' % self.name,
         'Type\t\t:\t%s' % self.type,
         'Memory\t\t:\t%.3f MB' % (self.memory / 1048576.0),
        ...
        for idx, platform in enumerate(pyopencl.get_platforms()):
            ...
            for idd, device in enumerate(platform.get_devices()):
                ...
                if pypl.vendor == 'NVIDIA Corporation' and devtype == 'GPU' and 'compute_capability_major_nv' in dir(device):
                    comput_cap = (
                     device.compute_capability_major_nv, device.compute_capability_minor_nv)
                ...
                if devtype == 'CPU' and pypl.vendor == 'Apple':
                    ...
                    workgroup = 1
                pydev = Device(device.name, devtype, device.version, device.driver_version, extensions, device.global_mem_size, bool(device.available), device.max_compute_units, device.max_clock_frequency, flop_core, idd, workgroup)
                ...
                if dtype in ('ALL', 'DEF') or device.type == dtype:
                    if memory is None or memory <= device.memory:
                        ...
                        if found:
                            ...
                            if not best_found:
                                best_found = (
                                 platformid, deviceid, device.flops)
...
def allocate_cl_buffers(buffers, device, context):
    ...
    logger.info('%.3fMB are needed on device which has %.3fMB', ualloc / 1000000.0, memory / 1000000.0)
    if ualloc >= memory:
    ...
    try:
        for name, flag, dtype, size in buffers:
            mem[name] = pyopencl.Buffer(context, flag, numpy.dtype(dtype).itemsize * size)
