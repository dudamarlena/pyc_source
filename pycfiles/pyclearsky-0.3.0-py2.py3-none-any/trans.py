...
# Embedded file name: build/bdist.linux-i686/egg/pyclearsilver/trans/trans.py
# Compiled at: 2005-06-30 14:51:56
import sys, string, os, getopt, pwd, signal, time, re, fcntl, tstart, db_trans
from log import *
import neo_cgi, neo_util, odb
...
class Translator:
    ...
    def __init__(self):
        ...
        self.ignore_paths = [
         'tmpl/m']
        self.ignore_files = ['blah_ignore.cs']
    ...
    def parseHTMLTag(self, data):
        ...
        tag = data[:i].lower()
        results = []
        attrfind = re.compile('\\s*([a-zA-Z_][-.a-zA-Z_0-9]*)(\\s*=\\s*(\\\'[^\\\']*\\\'|"[^"]*"|[^ \\t\\n<>]*))?')
        k = i
        attrs = {}
        attrs_beg = {}
        while k < len(data):
    ...
    def stringsHDF(self, prefix, locations, lang='en', exist=0, tiered=0):
        ...
        if exist == 1:
            log('Missing %d strings for lang %s' % (count, lang))
        return hdf
