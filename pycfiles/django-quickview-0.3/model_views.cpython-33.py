...
# [GCC 8.4.0]
# Embedded file name: C:\Dev\python\tmp\qvtest\testsite\quickview\model_views.py
# Compiled at: 2013-02-27 12:51:46
...
class ModelQuickView(QuickView):
    ...
    def list(cls, request, *args, **kwargs):
        ...
        if request.is_ajax():
            params = {cls.object_list: cls.use_pagination and cls.dictify_queryset(cls.pagination(request)) or cls.dictify_queryset(cls.get_queryset(request))}
    ...
    def dynamic_ajax_page(cls, request):
        ...
        params = {'app_label': cls.model._meta.app_label.lower(), 
         'model_names': [
                         model_name], 
         'field_names': {model_name: get_field_names_from_model(cls.model)},  'ajax_views': ajax_views}
        return SimpleTemplateResponse(template='quickview/quickview_model_api.js', context=params, content_type='text/javascript')
    ...
    def debug_view(cls, request):
        ...
        context = {'generated_urls': [{'url': urlregex._regex,  'function': urlregex._callback.__name__,  'name': urlregex.name} for urlregex in views]}
