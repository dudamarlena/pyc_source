...
# [GCC 8.4.0]
# Embedded file name: C:\Dev\python\tmp\qvtest\testsite\quickview\management\commands\generate_templates.py
# Compiled at: 2013-02-27 08:00:59
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not models_to_process:
            ...
            sys.exit(1)
        apps = [app for app in settings.INSTALLED_APPS if 'django' not in app and 'quickview' not in app]
        if app_label not in apps:
            print("No such app '%s'." % app_label)
            sys.exit(1)
        models = get_models(get_app(app_label))
        for model in models_to_process:
            if model not in [f.__name__ for f in models]:
        ...
        if 'from quickview import ModelQuickView\n' not in lines:
        ...
        for model_name in models_to_process:
            ...
            if 'class %sView(ModelQuickView):\n' % model_name not in lines:
                lines.append('\n\n')
                for model_name in models_to_process:
        ...
        for model_name in models_to_process:
            ...
            if os.path.exists(tf):
                ...
                sys.exit(1)
            os.makedirs(tf)
            index_template, delete_template, add_template, update_template, detail_template, base_site_template = get_templates(app_label, model_name)
