...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.10-intel/egg/peeweemgr/database.py
# Compiled at: 2016-08-23 06:14:42
"""
...
class DBManager(object):
    ...
    def load_database(self, setting):
        ...
        wrap_retry_handler = lambda dbclass_name, dbclass: type('Retry%s' % dbclass_name, (
         RetryOperationalError, dbclass), {'__module__': dbclass.__module__})
        if pool:
            pool_class = load_class('playhouse.pool.%s' % pool) if pool else None
            try:
                dbpool = wrap_retry_handler(pool, pool_class)(database_name, max_connections=pool_args.get('max_connections', 20), stale_timeout=pool_args.get('stale_timeout', None), **setting)
                return dbpool
            except ImportError:
