...
# [GCC 8.4.0]
# Embedded file name: /home/chris/git/planner/planner/tests/domains/driving/map.py
# Compiled at: 2018-01-26 16:49:59
...
class Map(object):
    ...
    def load(cls, points_fn, connections_fn):
        ...
        if not os.path.isfile(points_fn):
            raise AssertionError('Points file does not exist: %s' % points_fn)
            _connections_fn = os.path.isfile(connections_fn) or os.path.join(os.path.dirname(os.path.realpath(__file__)), connections_fn)
...
class Test(unittest.TestCase):
    ...
    def test_map(self):
        ...
        self.assertEqual(sorted(m.iter_neighbors('d')), [
         ('e', 12), ('f', 5), ('g', 10)])
        g = nx.Graph()
