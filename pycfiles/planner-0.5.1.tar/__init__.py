...
# [GCC 8.4.0]
# Embedded file name: /home/chris/git/planner/planner/tests/domains/xo/__init__.py
# Compiled at: 2018-01-25 13:13:42
...
def label_state(game, turn, mycolor):
    ...
    """
    if turn == mycolor:
        return dict(expected_fitness_default=0, expected_fitness_aggregator=max)
    else:
        return dict(expected_fitness_default=+float('inf'), expected_fitness_aggregator=min)
...
def fitness(**kwargs):
    ...
    score = (winner == mycolor) * 0.9 + (1 - moves / 9.0) * 0.1
    return score