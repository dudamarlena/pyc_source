...
# [GCC 8.4.0]
# Embedded file name: C:\Users\R and L\Dropbox\Coding Notes and Programming\gcapi-python\gcapi\gcapi_client.py
# Compiled at: 2020-02-29 16:31:02
# Size of source mod 2**32: 7679 bytes
...
class GCapiClient:
    ...
    def get_prices(self, market_id=None, num_ticks=None, from_ts=None, to_ts=None):
        ...
        if market_id is None:
            ...
            if to_ts is not None:
                r = self.session.get(self.rest_url + f"/market/{market_id}/tickhistorybetween?fromTimeStampUTC={from_ts}&toTimestampUTC={to_ts}")
        else:
            if not num_ticks:
                num_ticks = 1
            if from_ts is not None:
                r = self.session.get(self.rest_url + f"/market/{market_id}/tickhistorybefore?maxResults={num_ticks}&toTimeStampUTC={to_ts}")
            else:
                if to_ts is not None:
                    r = self.session.get(self.rest_url + f"/market/{market_id}/tickhistoryafter?maxResults={num_ticks}&fromTimeStampUTC={from_ts}")
                else:
    ...
    def get_ohlc(self, market_id=None, num_ticks=None, interval='HOUR', span=1, from_ts=None, to_ts=None):
        ...
        if market_id is None:
            ...
            if from_ts is not None:
                if to_ts is not None:
                    r = self.session.get(self.rest_url + f"/market/{market_id}/barhistorybetween?interval={interval}&span={span}fromTimeStampUTC={from_ts}&toTimestampUTC={to_ts}")
            if not num_ticks:
                num_ticks = 1
            if from_ts is not None:
                r = self.session.get(self.rest_url + f"/market/{market_id}/barhistorybefore?interval={interval}&span={span}&maxResults={num_ticks}&toTimeStampUTC={to_ts}")
            else:
                if to_ts is not None:
                    r = self.session.get(self.rest_url + f"/market/{market_id}/tickhistoryafter?interval={interval}&span={span}&maxResults={num_ticks}&fromTimeStampUTC={from_ts}")
                else:
    ...
    def trade_order(self, quantity, offer_price, direction, trading_acc_id=None, market_id=None, market_name=None, stop_loss=None, take_profit=None, trigger_price=None):
        """
        ...
        if trading_acc_id is None:
            ...
            if market_id is None:
                ...
                if market_name is None:
                    ...
                    ifdone = {}
                    if stop_loss:
                        ifdone['Stop'] = {'TriggerPrice':stop_loss, 
                         'Direction':opp_direction,  'Quantity':quantity}
                if take_profit:
                    ifdone['Limit'] = {'TriggerPrice':take_profit, 
                     'Direction':opp_direction,  'Quantity':quantity}
            if len(ifdone):
                trade_details['IfDone'] = [
                 ifdone]
...
if __name__ == '__main__':
    api = GCapiClient(username='rickykim932@gmail.com', password='rickykim932', appkey='KM.Kim', proxies=None)
    api.get_account_info()
    print(api.list_open_positions())