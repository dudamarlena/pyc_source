...
# [GCC 8.4.0]
# Embedded file name: signaturesimulator/sense/surface/i2em.py
# Compiled at: 2018-04-20 05:38:32
"""
...
class I2EM(SurfaceScatter):
    ...
    def init_model(self):
        ...
        if self.auto:
            ...
            while error > 1e-08:
                nspec += 1
                error = (self._ks2 * (2.0 * self._cs) ** 2.0) ** nspec / math.factorial(nspec)

            self.n_spec = nspec
        else:
            self.n_spec = 15
        I = np.arange(self.n_spec)
        self._fac = map(math.factorial, I + 1)
    ...
    def _init_hlp(self):
        ...
        self._ks2 = self.ks ** 2.0
        self._cs = np.cos(self.theta)
        self._cs2 = self._cs ** 2.0
    ...
    def _i2em_bistatic(self):
        ...
        self.fac = map(math.factorial, idx)
        self.wn, self.rss = self.calc_roughness_spectrum(acf_type=self.acf_type)
        Ivv, Ihh = self._calc_Ipp()
        Ivv_abs = np.abs(Ivv)
        Ihh_abs = np.abs(Ihh)
        ShdwS = self._calc_shadowing()
        a0 = self.wn / self.fac * self.sig ** (2.0 * idx)
        hlp = ShdwS * 0.5 * self.k ** 2 * np.exp(-self.sig ** 2 * (self._kz ** 2.0 + self._ksz ** 2.0))
        sigvv = np.sum(a0 * Ivv_abs ** 2.0) * hlp
        sighh = np.sum(a0 * Ihh_abs ** 2.0) * hlp
        return (sigvv, sighh)
    ...
    def _integrate_xpol(self, rvh):
        ...
        """
        ans, err = dblquad(self._xpol_integralfunc, 0.1, 1.0, lambda x: 0.0, lambda x: 1.0, args=[[rvh, self.eps, self._ks2, self._cs2, self.rss, self._cs, self._fac, self._kl2, self._s, self._get_acf_id()]])
        return ans
    ...
    def _xpol_integralfunc(self, r, phi, *args):
        ...
        d = rm / qt
        B3 = rx * ry / cs
        fvh1 = (b - c) * (1.0 - 3.0 * rvh) - (b - c / eps) * rp
        fvh2 = (a - d) * (1.0 + 3.0 * rvh) - (a - d * eps) * rm
        Fvh = np.abs((fvh1 + fvh2) * B3) ** 2.0
        au = q / r / 1.414 / rss
        fsh = 0.2821 / au * np.exp(-au ** 2.0) - 0.5 * (1.0 - math.erf(au))
        sha = 1.0 / (1.0 + fsh)
        ...
        acc = np.exp(-2.0 * ks2 * cs2) / (16.0 * np.pi)
        VH = 4.0 * acc * Fvh * vhmnsum * r
        y = VH * sha
        return y
    ...
    def _calc_shadow_cross(self):
        ...
        farg = ct / np.sqrt(2.0) / self.rss
        gamma = 0.5 * (np.exp(-farg ** 2.0) / 1.772 / farg - math.erfc(farg))
    ...
    def _calc_shadowing(self):
        if self.mode == 'backscatter':
            ...
            rslp = self.rss
            ctorslp = ct / math.sqrt(2.0) / rslp
            ctsorslp = cts / np.sqrt(2.0) / rslp
            shadf = 0.5 * (np.exp(-ctorslp ** 2.0) / np.sqrt(np.pi) / ctorslp - math.erfc(ctorslp))
            shadfs = 0.5 * (np.exp(-ctsorslp ** 2.0) / np.sqrt(np.pi) / ctsorslp - math.erfc(ctsorslp))
    ...
    def _calc_Ipp(self):
        n = np.arange(self.niter) + 1.0
        qi = self.k * self._cs
        qs = self.k * self._css
        h1 = np.exp(-self.sig ** 2.0 * self._kz * self._ksz) * (self._kz + self._ksz) ** n
        ...
        Ivv = fvv * h1
        Ivv += 0.25 * (Fvvupi * (self._ksz - qi) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qi ** 2.0 - qi * (self._ksz - self._kz))))
        Ivv += 0.25 * (Fvvdni * (self._ksz + qi) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qi ** 2.0 + qi * (self._ksz - self._kz))))
        Ivv += 0.25 * (Fvvups * (self._kz + qs) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qs ** 2.0 - qs * (self._ksz - self._kz))))
        Ivv += 0.25 * (Fvvdns * (self._kz - qs) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qs ** 2.0 + qs * (self._ksz - self._kz))))
        Ihh = fhh * h1
        Ihh += 0.25 * (Fhhupi * (self._ksz - qi) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qi ** 2.0 - qi * (self._ksz - self._kz))))
        Ihh += 0.25 * (Fhhdni * (self._ksz + qi) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qi ** 2.0 + qi * (self._ksz - self._kz))))
        Ihh += 0.25 * (Fhhups * (self._kz + qs) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qs ** 2.0 - qs * (self._ksz - self._kz))))
        Ihh += 0.25 * (Fhhdns * (self._kz - qs) ** (n - 1) * np.exp(-self.sig ** 2.0 * (qs ** 2.0 + qs * (self._ksz - self._kz))))
        return (
         Ivv, Ihh)

    def calc_fpp(self, Rvi, Rhi):
        Rvt, Rht = self.calc_reflection_coefficients(Rvi, Rhi)
        fvv = 2.0 * Rvt * (self._s * self._ss - (1.0 + self._cs * self._css) * self._cfs) / (self._cs + self._css)
        fhh = -2.0 * Rht * (self._s * self._ss - (1.0 + self._cs * self._css) * self._cfs) / (self._cs + self._css)
        return (fvv, fhh)
    ...
    def Fppupdn(self, u_d, i_s, Rvi, Rhi):
        ...
        if i_s == 1:
            Gqi = u_d * self._kz
            Gqti = u_d * self.k * np.sqrt(self.eps - self._s ** 2.0)
            qi = u_d * self._kz
            c11 = self.k * self._cfs * (self._ksz - qi)
            c21 = self._cs * (self._cfs * (self.k ** 2 * self._s * self._cf * (self._ss * self._cfs - self._s * self._cf) + Gqi * (self.k * self._css - qi)) + self.k ** 2.0 * self._cf * self._s * self._ss * self._sfs ** 2.0)
            c31 = self.k * self._s * (self._s * self._cf * self._cfs * (self.k * self._css - qi) - Gqi * (self._cfs * (self._ss * self._cfs - self._s * self._cf) + self._ss * self._sfs ** 2.0))
            ...
            c22 = self._cs * (self._cfs * (self.k ** 2.0 * self._s * self._cf * (self._ss * self._cfs - self._s * self._cf) + Gqti * (self.k * self._css - qi)) + self.k ** 2.0 * self._cf * self._s * self._ss * self._sfs ** 2.0)
            c32 = self.k * self._s * (self._s * self._cf * self._cfs * (self.k * self._css - qi) - Gqti * (self._cfs * (self._ss * self._cfs - self._s * self._cf) - self._ss * self._sfs ** 2.0))
            ...
            Gqs = u_d * self._ksz
            Gqts = u_d * self.k * np.sqrt(self.eps - self._ss ** 2.0)
            qs = u_d * self._ksz
            c11 = self.k * self._cfs * (self._kz + qs)
            c21 = Gqs * (self._cfs * (self._cs * (self.k * self._cs + qs) - self.k * self._s * (self._ss * self._cfs - self._s * self._cf)) - self.k * self._s * self._ss * self._sfs ** 2.0)
            c31 = self.k * self._ss * (self.k * self._cs * (self._ss * self._cfs - self._s * self._cf) + self._s * (self._kz + qs))
            c41 = self.k * self._css * (self._cfs * (self._cs * (self._kz + qs) - self.k * self._s * (self._ss * self._cfs - self._s * self._cf)) - self.k * self._s * self._ss * self._sfs ** 2.0)
            c51 = -self._css * (self.k ** 2.0 * self._ss * (self._ss * self._cfs - self._s * self._cf) + Gqs * self._cfs * (self._kz + qs))
            c12 = self.k * self._cfs * (self._kz + qs)
            c22 = Gqts * (self._cfs * (self._cs * (self._kz + qs) - self.k * self._s * (self._ss * self._cfs - self._s * self._cf)) - self.k * self._s * self._ss * self._sfs ** 2.0)
            c32 = self.k * self._ss * (self.k * self._cs * (self._ss * self._cfs - self._s * self._cf) + self._s * (self._kz + qs))
            c42 = self.k * self._css * (self._cfs * (self._cs * (self._kz + qs) - self.k * self._s * (self._ss * self._cfs - self._s * self._cf)) - self.k * self._s * self._ss * self._sfs ** 2.0)
            c52 = -self._css * (self.k ** 2.0 * self._ss * (self._ss * self._cfs - self._s * self._cf) + Gqts * self._cfs * (self._kz + qs))
        q = self._kz
        qt = self.k * np.sqrt(self.eps - self._s ** 2.0)
        vv = (1.0 + Rvi) * (-(1 - Rvi) * c11 / q + (1.0 + Rvi) * c12 / qt)
        vv += (1.0 - Rvi) * ((1 - Rvi) * c21 / q - (1.0 + Rvi) * c22 / qt)
        vv += (1.0 + Rvi) * ((1 - Rvi) * c31 / q - (1.0 + Rvi) * c32 / self.eps / qt)
        vv += (1.0 - Rvi) * ((1 + Rvi) * c41 / q - self.eps * (1.0 - Rvi) * c42 / qt)
        vv += (1.0 + Rvi) * ((1 + Rvi) * c51 / q - (1.0 - Rvi) * c52 / qt)
        hh = (1.0 + Rhi) * ((1.0 - Rhi) * c11 / q - self.eps * (1.0 + Rhi) * c12 / qt)
        hh -= (1.0 - Rhi) * ((1.0 - Rhi) * c21 / q - (1.0 + Rhi) * c22 / qt)
        hh -= (1.0 + Rhi) * ((1.0 - Rhi) * c31 / q - (1.0 + Rhi) * c32 / qt)
        hh -= (1.0 - Rhi) * ((1.0 + Rhi) * c41 / q - (1.0 - Rhi) * c42 / qt)
        hh -= (1.0 + Rhi) * ((1.0 + Rhi) * c51 / q - (1.0 - Rhi) * c52 / qt)
        return (
         vv, hh)

    def _calc_r_transition(self):
        """ compute R transition """
        Rv0 = (np.sqrt(self.eps) - 1.0) / (np.sqrt(self.eps) + 1.0)
        Rh0 = -Rv0
        Ft = 8.0 * Rv0 ** 2.0 + self._ss * (self._cs + np.sqrt(self.eps - self._s2)) / (self._cs * np.sqrt(self.eps - self._s2))
        idx = np.arange(self.niter) + 1
        a0 = (self.ks * self._cs) ** (2.0 * idx) / self.fac
        a1 = np.sum(a0 * self.wn)
        b1 = np.sum(a0 * np.abs(Ft / 2.0 + 2.0 ** (idx + 1) * Rv0 / self._cs * np.exp(-(self.ks * self._cs) ** 2.0)) ** 2.0 * self.wn)
        St = 0.25 * np.abs(Ft) ** 2.0 * a1 / b1
        St0 = 1.0 / np.abs(1.0 + 8.0 * Rv0 / (self._cs * Ft)) ** 2.0
        Tf = 1.0 - St / St0
        return (
         Rv0, Rh0, Tf)
...
class Roughness(object):
    ...
    def _init(self):
        ...
        self._kl2 = self._kl ** 2.0
        self.wvnb = self.k * np.sqrt((ss * cfs - self._s * cf) ** 2.0 + (ss * sfs - self._s * sf) ** 2.0)
...
def _calc_wn_matrix_gauss(rx, ry, nspec, kl2, s):
    wn = np.zeros(nspec)
    for i in xrange(nspec):
        wn[i] = 0.5 * kl2 / (i + 1.0) * np.exp(-kl2 * ((rx - s) ** 2.0 + ry ** 2.0) / (4.0 * (i + 1)))
...
def _calc_wm_matrix_gauss(rx, ry, nspec, kl2, s):
    wm = np.zeros(nspec)
    for i in xrange(nspec):
        wm[i] = 0.5 * kl2 / (i + 1.0) * np.exp(-kl2 * ((rx + s) ** 2.0 + ry ** 2.0) / (4.0 * (i + 1)))
...
class GaussianSpectrum(Roughness):
    ...
    def wn(self):
        n = self.n
        wn = self.l ** 2.0 / (2.0 * n) * np.exp(-(self.wvnb * self.l) ** 2.0 / (4.0 * n))
        rss = np.sqrt(2.0) * self.sig / self.l
        return (wn, rss)
...
def _calc_wn_matrix_exp(rx, ry, nspec, kl2, s):
    wn = np.zeros(nspec)
    for i in xrange(nspec):
        wn[i] = (i + 1) * kl2 / ((i + 1) ** 2.0 + kl2 * ((rx - s) ** 2.0 + ry ** 2.0)) ** 1.5
...
def _calc_wm_matrix_exp(rx, ry, nspec, kl2, s):
    wm = np.zeros(nspec)
    for i in xrange(nspec):
        wm[i] = (i + 1) * kl2 / ((i + 1) ** 2.0 + kl2 * ((rx + s) ** 2.0 + ry ** 2.0)) ** 1.5
...
class ExponentialSpectrum(Roughness):
    ...
    def wn(self):
        n = self.n
        wn = self.l ** 2.0 / n ** 2.0 * (1.0 + (self.wvnb * self.l / n) ** 2.0) ** (-1.5)
        rss = self.sig / self.l
        return (wn, rss)
