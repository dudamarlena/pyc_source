...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/dicttokv/main.py
# Compiled at: 2014-01-01 08:28:28
...
class DictToKeyValue(object):
    ...
    @classmethod
    def keys_to_keystr(cls, keys, delimiter=b'.', ignore_number=False):
        if ignore_number:
    ...
    @classmethod
    def to_tsvlines(cls, target, delimiter=b'.', ignore_number=False):
        for keys, v in cls.from_object(target):
            yield (b'{}\t{}').format(cls.keys_to_keystr(keys, delimiter, ignore_number), v)
    ...
    def restore(cls, converted):
        ...
        def create_path_setter(target_root, keys):
            ...
            for key, next_key in izip(keys, next_keys):
                if next_key is None:
                    break
                if isinstance(s, dict) and key not in s or isinstance(s, list) and len(s) - 1 < key:
