...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nemweb/nemweb_current.py
...
class CurrentFileHandler:
    ...
    def update_data(self, dataset):
        ...
        page = requests.get('{0}/{1}/{2}/'.format(self.base_url, self.section, dataset.dataset_name))
        regex = re.compile('/{0}/{1}/{2}'.format(self.section, dataset.dataset_name, dataset.nemfile_pattern))
...
CurrentDataset = namedtuple('NemwebCurrentFile', [
 ...
 'datetime_format',
 'tables'])
DATASETS = {'dispatch_scada': CurrentDataset(dataset_name='Dispatch_SCADA', nemfile_pattern='PUBLIC_DISPATCHSCADA_([0-9]{12})_[0-9]{16}.zip', datetime_format='%Y%m%d%H%M', tables=[
                    'DISPATCH_UNIT_SCADA']), 
 
 'trading_is': CurrentDataset(dataset_name='TradingIS_Reports', nemfile_pattern='PUBLIC_TRADINGIS_([0-9]{12})_[0-9]{16}.zip', datetime_format='%Y%m%d%H%M', tables=[
                'TRADING_PRICE',
                'TRADING_REGIONSUM',
                'TRADING_INTERCONNECTORRES']), 
 
 'rooftopPV_actual': CurrentDataset(dataset_name='ROOFTOP_PV/ACTUAL', nemfile_pattern='PUBLIC_ROOFTOP_PV_ACTUAL_([0-9]{14})_[0-9]{16}.zip', datetime_format='%Y%m%d%H%M00', tables=[
                      'ROOFTOP_ACTUAL']), 
 
 'next_day_actual_gen': CurrentDataset(dataset_name='Next_Day_Actual_Gen', nemfile_pattern='PUBLIC_NEXT_DAY_ACTUAL_GEN_([0-9]{8})_[0-9]{16}.zip', datetime_format='%Y%m%d', tables=[
                         'METER_DATA_GEN_DUID']), 
 
 'dispatch_is': CurrentDataset(dataset_name='DispatchIS_Reports', nemfile_pattern='PUBLIC_DISPATCHIS_([0-9]{12})_[0-9]{16}.zip', datetime_format='%Y%m%d%H%M', tables=[
                 'DISPATCH_PRICE',
                 'DISPATCH_REGIONSUM',
                 'DISPATCH_INTERCONNECTORRES']), 
 
 'next_day_dispatch': CurrentDataset(dataset_name='Next_Day_Dispatch', nemfile_pattern='PUBLIC_NEXT_DAY_DISPATCH_([0-9]{8})_[0-9]{16}.zip', datetime_format='%Y%m%d', tables=[
