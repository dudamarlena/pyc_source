...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dronekit/__init__.py
...
----
"""
import collections, copy, logging, math, struct, time, monotonic
...
class Version(object):
    ...
    def __init__(self, raw_version, autopilot_type, vehicle_type):
        ...
        if raw_version is None:
            ...
            self.major = raw_version >> 24 & 255
            self.minor = raw_version >> 16 & 255
            self.patch = raw_version >> 8 & 255
            self.release = raw_version & 255
        return
...
class Locations(HasObservers):
    ...
    def __init__(self, vehicle):
        ...
        def listener(vehicle, name, m):
            self._lat, self._lon = m.lat / 10000000.0, m.lon / 10000000.0
...
class Vehicle(HasObservers):
    ...
    def __init__(self, handler):
        ...
        self._mavlink_statustext_severity = {0: logging.CRITICAL, 
           1: logging.CRITICAL, 
           2: logging.CRITICAL, 
           3: logging.ERROR, 
           4: logging.WARNING, 
        ...
        self._ready_attrs = {
         'commands'}
        self._default_ready_attrs = [
         'parameters', 'gps_0', 'armed', 'mode', 'attitude']
        ...
        def listener(self, name, msg):
            self._home_location = LocationGlobal(msg.latitude / 10000000.0, msg.longitude / 10000000.0, msg.altitude / 1000.0)
    ...
    def system_status(self):
        ...
        * ``POWEROFF``: System just initialized its power-down sequence, will shut down now.
        ...
        return {0: SystemStatus('UNINIT'), 
           ...
           7: SystemStatus('POWEROFF')}.get(self._system_status, None)
    ...
    def groundspeed(self, speed):
        speed_type = 1
        msg = self.message_factory.command_long_encode(0, 0, mavutil.mavlink.MAV_CMD_DO_CHANGE_SPEED, 0, speed_type, speed, -1, 0, 0, 0, 0)
    ...
    def mount_status(self):
        ...
        """
        return [
         self._mount_pitch, self._mount_yaw, self._mount_roll]
    ...
    def home_location(self, pos):
        ...
        self.send_mavlink(self.message_factory.command_long_encode(0, 0, mavutil.mavlink.MAV_CMD_DO_SET_HOME, 0, 0, 0, 0, 0, pos.lat, pos.lon, pos.alt))
    ...
    def arm(self, wait=True, timeout=None):
        ...
        self.armed = True
        if wait:
            self.wait_for(lambda : self.armed, timeout=timeout, errmsg='failed to arm vehicle')
    ...
    def wait_for_mode(self, mode, timeout=None):
        ...
        self.mode = mode
        self.wait_for(lambda : self.mode.name == mode.name, timeout=timeout, errmsg='failed to set flight mode')
    ...
    def simple_takeoff(self, alt=None):
        ...
        if alt is not None:
            ...
            self._master.mav.command_long_send(0, 0, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0, altitude)
        return
    ...
    def wait_ready(self, *types, **kwargs):
        ...
        while not await_attributes.issubset(self._ready_attrs):
            ...
            if now - start > timeout:
                if raise_exception:
                    raise TimeoutError('wait_ready experienced a timeout after %s seconds.' % timeout)
                else:
                    return False
            if still_waiting_callback and now - still_waiting_last_message_sent > still_waiting_message_interval:
    ...
    def reboot(self):
        ...
        reboot_msg = self.message_factory.command_long_encode(0, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_REBOOT_SHUTDOWN, 0, 1, 0, 0, 0, 0, 0, 0)
        self.send_mavlink(reboot_msg)

    def send_calibrate_gyro(self):
        """Request gyroscope calibration."""
        calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 1, 0, 0, 0, 0, 0, 0)
    ...
    def send_calibrate_magnetometer(self):
        ...
        if self._autopilot_type == mavutil.mavlink.MAV_AUTOPILOT_ARDUPILOTMEGA:
            calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_DO_START_MAG_CAL, 0, 0, 1, 1, 0, 0, 0, 0)
        else:
            calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 0, 1, 0, 0, 0, 0, 0)
        self.send_mavlink(calibration_command)
    ...
    def send_calibrate_accelerometer(self, simple=False):
        ...
        """
        calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 0, 0, 0, 0, 4 if simple else 1, 0, 0)
        self.send_mavlink(calibration_command)

    def send_calibrate_vehicle_level(self):
        """Request vehicle level (accelerometer trim) calibration."""
        calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 0, 0, 0, 0, 2, 0, 0)
        self.send_mavlink(calibration_command)

    def send_calibrate_barometer(self):
        """Request barometer calibration."""
        calibration_command = self.message_factory.command_long_encode(self._handler.target_system, 0, mavutil.mavlink.MAV_CMD_PREFLIGHT_CALIBRATION, 0, 0, 0, 1, 0, 0, 0, 0)
        self.send_mavlink(calibration_command)
...
class Gimbal(object):
    ...
    def rotate(self, pitch, roll, yaw):
        ...
        """
        msg = self._vehicle.message_factory.mount_configure_encode(0, 1, mavutil.mavlink.MAV_MOUNT_MODE_MAVLINK_TARGETING, 1, 1, 1)
        self._vehicle.send_mavlink(msg)
        msg = self._vehicle.message_factory.mount_control_encode(0, 1, pitch * 100, roll * 100, yaw * 100, 0)
        self._vehicle.send_mavlink(msg)
    ...
    def release(self):
        ...
        """
        msg = self._vehicle.message_factory.mount_configure_encode(0, 1, mavutil.mavlink.MAV_MOUNT_MODE_RC_TARGETING, 1, 1, 1)
        self._vehicle.send_mavlink(msg)
        ...
        if self._vehicle._wpts_dirty:
            ...
            if self._vehicle._wploader.count() > 0:
                self._vehicle._wp_uploaded = [
                 False] * self._vehicle._wploader.count()
