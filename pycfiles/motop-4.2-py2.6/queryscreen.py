...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/libmotop/queryscreen.py
# Compiled at: 2014-01-17 04:06:33
...
class StatusBlock(Block):
    columnHeaders = ('Server', 'QPS', 'Active', 'Queue', 'Flush', 'Connection', 'Network I/O',
                     'Memory', 'Page Faults')
    ...
    def reset(self):
        ...
        for server in self.__servers:
            ...
            if status:
                ...
                cells.append([ v * 1000000 for v in status.deepget('mem', ('resident',
                                                                           'mapped')) if v is not None ])
...
class ServerBasedBlock(Block):
    ...
    def connectedServers(self):
        return [ server for server in self.__servers if server.connected() if server not in self.__hiddenServers ]
...
class OperationBlock(Block):
    columnHeaders = ('Server', 'Opid', 'Client', 'Type', 'Sec', 'Locks', 'Namespace',
                     'Query')
