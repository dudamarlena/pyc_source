...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wafw00f/lib/evillib.py
...
class waftoolsengine:
    ...
    def __init__(self, target='https://example.com', debuglevel=0, path='/', proxies=None, redir=True, head=None):
        self.target = target
    ...
    def Request(self, headers=None, path=None, params={}, delay=0, timeout=7):
        try:
            ...
            if not headers:
                ...
                h = headers
            req = requests.get((self.target), proxies=(self.proxies), headers=h, timeout=timeout, allow_redirects=(self.allowredir),
              params=params,
              verify=False)
