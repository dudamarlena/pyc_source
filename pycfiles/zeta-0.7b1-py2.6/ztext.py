...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/zeta/lib/ztext.py
# Compiled at: 2010-07-11 02:15:54
...
class Structure(object):
    ...
    def identify(self):
        ...
        rc = any([ all([ mattr in attrs for mattr in mustattrs ]) for mustattrs in self.mustattrss
                 ])
...
class Swiki(Structure):
    type = 'staticwiki'
    attributes = {'path': [
              'path', 'path-url', 'path url', 'url', 'pathurl']}
    mustattrss = [
     [
      'path']]
    hbdelimiter = {'text': [
...
class Attachment(Structure):
    ...
    attributes = {'id': [
            ...
            'attachment-id'], 
       'projectname': [
                     'project', 'projectname', 'project-name', 'project_name',
                     'project name'], 
       'projectid': [
                   'pid', 'projectid', 'project-id', 'project_id',
                   'project id'], 
       'summary': [
                 'summary'], 
       'tags': [
    ...
    def commit(self, config, byuser, tz='UTC', attachments=[]):
        ...
        if aid:
            ...
            for (fname, file) in attachments:
                att = attcomp.create_attach(fname, FileC(file), uploader=byuser, summary=summary)
...
class Wiki(Structure):
    type = 'wiki'
    attributes = {'wikiid': [
                'wikiid', 'wiki-id', 'wiki_id', 'wiki id'], 
       'pagename': [
                  'page', 'pagename', 'page-name', 'page_name', 'page name'], 
       'wikiurl': [
                 'wikiurl', 'wiki-url', 'wiki url'], 
       'projectname': [
                     'project', 'projectname', 'project-name', 'project_name',
                     'project name'], 
       'projectid': [
                   ...
                   'project id'], 
       'type': [
              'type', 'wikitype', 'wiki-type', 'wiki_type', 'wiki type'], 
       'summary': [
                 'summary'], 
       'tags': [
              'tag', 'tags'], 
    ...
    def commit(self, config, byuser, tz='UTC', attachments=[]):
        ...
        if w:
            ...
            comment and wikicomp.create_wikicomment(w, (
             None, byuser, w.latest_version, comment), byuser=byuser)
            wtype = kwargs.pop('type', None)
            wtype = wtype and wikicomp.get_wikitype(wtype)
            summary = kwargs.pop('summary', None)
            if wtype or summary:
                wikicomp.config_wiki(w, type=wtype, summary=summary, byuser=byuser)
            ...
            w = wikicomp.create_wiki(unicode(wikiurl), type=wtype, summary=summary, creator=byuser)
            if w:
                fav = kwargs.pop('favorite', None)
                fav == True and wikicomp.addfavorites(w, [byuser], byuser=byuser)
                fav == False and wikicomp.delfavorites(w, [byuser], byuser=byuser)
        ...
        if w:
            for (fname, fcont) in attachments:
                att = attcomp.create_attach(fname, FileC(fcont), uploader=byuser)
...
class Ticket(Structure):
    type = 'ticket'
    attributes = {'ticket': [
                'ticket', 'tid', 'ticketid', 'ticket-id', 'ticket_id',
                'ticket id',
       ...
       'projectid': [
                   ...
                   'project id'], 
       'type': [
              'type', 'tickettype', 'ticket-type', 'ticket_type',
              'ticket type'], 
       'severity': [
                  'severity', 'ticketseverity', 'ticket-severity',
       ...
       'duedate': [
                 'duedate', 'due-date', 'due_date', 'due date'], 
       'promptuser': [
                    'promptuser', 'prompt-user', 'prompt_user',
                    'prompt user'], 
       'components': [
                    'comp', 'comps', 'component', 'components'], 
       'milestones': [
                    'mstn', 'mstns', 'milestone', 'milestones'], 
       'versions': [
    ...
    def validate(self):
        ...
        try:
            ...
            if 'components' in data:
                data['components'] = sanitizevalues(data.get('components', ''))
            if 'milestones' in data:
                data['milestones'] = sanitizevalues(data.get('milestones', ''))
            if 'versions' in data:
                data['versions'] = sanitizevalues(data.get('versions', ''))
            if 'blocking' in data:
    ...
    def commit(self, config, byuser, tz='UTC', attachments=[]):
        ...
        if t:
            ...
            comment and tckcomp.create_ticket_comment(t, (
             None, comment, byuser), byuser=byuser)
            summary = kwargs.pop('summary', None)
            descr = kwargs.pop('description', None)
            if summary != None or descr != None:
                tckdet = [
                 t.id, summary, descr, t.type, t.severity]
                tckcomp.create_ticket(None, tckdet, t.promptuser, update=True, byuser=byuser)
            status = kwargs.pop('status', None)
            duedate = kwargs.pop('duedate', None)
            duedate = duedate and h.duedate2dt(duedate, tz) or None
            if status == ts.status.tck_statusname and duedate != None or status == None and duedate != None:
                tstatdet = [ts.id, ts.status, duedate]
                tckcomp.create_ticket_status(t, tstatdet, byuser, update=True, byuser=byuser)
            elif status:
                tstatdet = [
                 None, status, duedate]
            ...
            if p and ttype and severity and summary:
                ...
                t = tckcomp.create_ticket(p, tckdet, promptuser=puser, owner=byuser, byuser=byuser)
            if t:
                fav = kwargs.pop('favorite', None)
                fav == True and tckcomp.addfavorites(t, [byuser], byuser=byuser)
                fav == False and tckcomp.delfavorites(t, [byuser], byuser=byuser)
...
class Review(Structure):
    type = 'review'
    attributes = {'reviewid': [
                  'reviewid', 'review-id', 'review_id', 'review id'], 
       'rcmtid': [
                'review-comment', 'review comment', 'review_comment'], 
       'projectname': [
                     'project', 'projectname', 'project-name', 'project_name',
                     'project name'], 
       'projectid': [
                   ...
                   'project id'], 
       'position': [
                  'position', 'lineno', 'line-no', 'line no'], 
       'nature': [
                'nature'], 
       'action': [
                'action'], 
    ...
    def commit(self, config, byuser, tz='UTC', attachments=[]):
        ...
        if r and comment and position:
            revcomp.create_reviewcomment(r, (
             None, position, comment, byuser, nature, None), byuser=byuser)
        elif rcmt:
            revcomp.process_reviewcomment(rcmt, reviewnature=nature, reviewaction=action, approve=approved, byuser=byuser)
        if r:
            fav = kwargs.pop('favorite', None)
