...
# [GCC 8.4.0]
# Embedded file name: /home/makism/Github/dyconnmap-public-master/tests/test_ts.py
# Compiled at: 2020-02-01 04:44:31
# Size of source mod 2**32: 10394 bytes
import numpy as np
from numpy import testing
from dyconnmap.ts import aaft, fdr, phase_rand, surrogate_analysis, entropy_reduction_rate, symoblic_transfer_entropy, embed_delay, sample_entropy, ordinal_pattern_similarity, permutation_entropy, entropy, rr_order_patterns, wald, markov_matrix, transition_rate, occupancy_time, teager_kaiser_energy, dcorr, fisher_score, fisher_z_plv, fisher_z, complexity_index, fnn, cv, icc_31
ts = None
...
def test_surrogate_analysis():
    ...
    p_vals, surr_vals, surrogates, r_value = surrogate_analysis(ts1,
      ts2, num_surr=1000, estimator_func=None, ts1_no_surr=False, rng=rng)
...
def test_surrogate_analysis_fdr():
    ...
    ts1 = data[0, 0:512].ravel()
    ts2 = data[1, 0:512].ravel()
    p_val, surr_vals, surrogates, r_value = surrogate_analysis(ts1,
      ts2, num_surr=1000, estimator_func=None, ts1_no_surr=False, rng=rng)
    num_ts = 2
...
def test_surrogate_analysis2():
    ...
    ts2 = data[1, 0:512].ravel()
    p_val, surr_vals, surrogates, r_value = surrogate_analysis(ts1,
      ts2, num_surr=1000, ts1_no_surr=True, rng=rng)
...
def test_cv():
    ...
    cv_val = cv(X)
    np.testing.assert_almost_equal(cv_val, 0.6483440397399414)