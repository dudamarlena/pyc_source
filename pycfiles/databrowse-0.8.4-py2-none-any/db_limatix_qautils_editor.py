...
# [GCC 8.4.0]
# Embedded file name: C:\Files\Research\databrowse\databrowse\plugins\db_limatix_qautils_editor\db_limatix_qautils_editor.py
# Compiled at: 2018-06-29 17:51:46
""" plugins/renderers/db_checklist_chx.py - Generic Checklist Files """
import os, os.path
from lxml import etree
from databrowse.support.renderer_support import renderer_class
import subprocess, shutil, sys, imp
    ...
    def getContent(self):
        if self._caller != 'databrowse':
            ...
            if 'ajax' in self._web_support.req.form and 'save' in self._web_support.req.form and self._style_mode == 'fill_out_checklist':
                ...
                if all(k in self._web_support.req.form for k in ('file', 'specimen',
                                                                 'perfby', 'date',
                                                                 'dest')):
                    filestring = self._web_support.req.form['file'].value
                ...
                if self._content_mode == 'full' and self._style_mode == 'fill_out_checklist':
                    ...
                    return xmloutput
                if self._content_mode == 'full' and self._style_mode == 'edit_checklist':
                    f = open(self._fullpath, 'r')
                    xml = etree.parse(f)
                    f.close()
                    xmlroot = xml.getroot()
