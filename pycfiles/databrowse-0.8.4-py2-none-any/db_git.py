...
# [GCC 8.4.0]
# Embedded file name: C:\Files\Research\databrowse\databrowse\plugins\db_git\db_git.py
# Compiled at: 2018-11-06 11:54:38
""" plugins/renderers/db_git.py - Basic Output for Any Folder """
import pdb, os, os.path, git, magic, datetime
    ...
    def recursive_search(self, xmlcontents, rootpath, root='', relroot=''):
        ...
        for item in dirlist:
            ...
            if os.path.isdir(itemfullpath):
                ...
                self.recursive_search(xmldir, itemfullpath, root=os.path.join(root, os.path.basename(itemfullpath)).replace('\\', '/'), relroot=os.path.join(relroot, os.path.basename(itemrelpath)).replace('\\', '/'))
            else:
                ...
                try:
                    ...
                    magicstore.load()
                    contenttype = magicstore.file(os.path.realpath(itemfullpath))
    ...
    def getContent(self):
        ...
        if self.isGit():
            ...
            if self._content_mode == 'full':
                ...
                xmlroot = etree.Element('{%s}git' % self._namespace_uri, nsmap=self.nsmap, name=os.path.basename(self._relpath), resurl=self._web_support.resurl, downlink=downlink)
