...
# Compiled at: 2020-02-17 23:22:54
# Size of source mod 2**32: 26929 bytes
""" support/renderer_support.py - Encapsulation Class for Renderer Plugins """
from lxml import etree
from errno import EEXIST
from stat import *
import sys, os, io, string, random, copy, fnmatch

class renderer_class(object):
    __doc__ = ' Renderer Plugin Support - Encapsulation Class for Renderer Plugins '
    _relpath = None
    _fullpath = None
    _web_support = None
    ...
    def getURL(self, relpath, **kwargs):
        ...
        if self._handlers[(-1)] != self.__class__.__name__ and 'handler' not in kwargs:
            ...
            if 'handler' in kwargs:
                if kwargs['handler'] is None:
                    del kwargs['handler']
                elif 'showhiddenfiles' in self._web_support.req.form and 'showhiddenfiles' not in kwargs:
                    kwargs['showhiddenfiles'] = ''
                else:
                    if 'showhiddenfiles' in kwargs:
                        if kwargs['showhiddenfiles'] is None:
    ...
    class CacheFileHandler(io.FileIO):
        __doc__ = ' Overrride File Close Class to Reassign Timestamp '
        timestamp = None
 ...
 L. 383         0  LOAD_GLOBAL              super
                2  LOAD_GLOBAL              renderer_class
                4  LOAD_ATTR                CacheFileHandler
                6  LOAD_FAST                'self'
 ...
 L. 386        48  LOAD_FAST                'st'
               50  LOAD_GLOBAL              ST_ATIME
               52  BINARY_SUBSCR    
               54  STORE_FAST               'atime'

 L. 387        56  LOAD_GLOBAL              os
               58  LOAD_METHOD              utime
               60  LOAD_FAST                'self'
               62  LOAD_ATTR                name
               64  LOAD_FAST                'atime'
               66  LOAD_FAST                'self'
               68  LOAD_ATTR                timestamp
               70  BUILD_TUPLE_2         2 
