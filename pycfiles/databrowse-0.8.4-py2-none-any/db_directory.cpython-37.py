...
# Compiled at: 2020-02-17 23:34:02
# Size of source mod 2**32: 26686 bytes
...
class db_directory(renderer_class):
    __doc__ = ' Default Folder Renderer - Basic Output for Any Folder '
    _xml = None
    ...
    def specimen_search(self, rootpath, fileexts):
        ...
        for item in dirlist:
            ...
            ext = os.path.isdir(itemfullpath) or os.path.splitext(itemfullpath)[1]
            if ext in fileexts:
    ...
    def recursiveloop(self, dirname, chxlist):
        chxpath = os.path.abspath(os.path.join(self._web_support.dataroot + '/' + self._web_support.checklistpath + '/' + dirname))
        if os.path.exists(chxpath):
            chxdirlist = self.getDirectoryList(chxpath)
            for item in chxdirlist:
                if item.endswith('.chx'):
                    itemurl = self.getURL((os.path.normpath(self._web_support.checklistpath + '/' + dirname + '/' + item).replace('\\', '/')),
                      handler=None)
                    etree.SubElement(chxlist, ('{%s}chxfile' % self._namespace_uri), nsmap=(self.nsmap), url=itemurl, name=item)
                if os.path.isdir(os.path.join(chxpath, item)) and len([x for x in self.getDirectoryList(os.path.abspath(self._web_support.dataroot + '/' + self._web_support.checklistpath + '/' + os.path.normpath(dirname + '/' + item))) if not x.endswith('.chx') if os.path.isdir(os.path.abspath(self._web_support.dataroot + '/' + self._web_support.checklistpath + '/' + os.path.normpath(dirname + '/' + item + '/' + x)))]) > 0:
                    subchxlist = etree.SubElement(chxlist, ('{%s}chxdir' % self._namespace_uri), nsmap=(self.nsmap), url=self.getURL((os.path.normpath(self._web_support.checklistpath + '/' + dirname + '/' + item)),
                      handler=None),
                      name=item)
    ...
    def __init__(self, relpath, fullpath, web_support, handler_support, caller, handlers, content_mode=_default_content_mode, style_mode=_default_style_mode, recursion_depth=_default_recursion_depth):
        ...
        if caller == 'databrowse':
            ...
            xmlroot = etree.Element(('{%s}dir' % self._namespace_uri), nsmap=(self.nsmap), path=(self._fullpath), relpath=(self._relpath),
              dataroot=(self._web_support.dataroot),
              uphref=uphref,
              resurl=(self._web_support.resurl),
              siteurl=(self._web_support.siteurl),
              root='True')
            ...
            xmlroot = etree.Element(('{%s}dir' % self._namespace_uri), nsmap=(self.nsmap), name=(os.path.basename(self._relpath)), path=(self._fullpath),
            ...
            if recursion_depth != 0:
                ...
                for item in dirlist:
                    ...
                    if handler in self._handler_support.directoryplugins:
                        ...
                        if handler in self._handler_support.directoryplugins:
                            renderer = self.__class__(itemrelpath, itemfullpath, (self._web_support), (self._handler_support), caller, handlers,
                              content_mode=content_mode, style_mode=style_mode, recursion_depth=(recursion_depth - 1))
                        else:
                        ...
                        if os.path.islink(itemfullpath):
                            ...
                            if not os.access(itemfullpath, os.W_OK):
                                overlay = 'readonly'
                            else:
                                if not os.access(itemfullpath, os.R_OK):
                                    overlay = 'unreadable'
                                else:
                                    overlay = 'none'
                    if content is not None and content.tag.startswith('{%s}' % self._namespace_uri):
                        ...
                        xmlchild = etree.SubElement(xmlroot, ('{%s}file' % self._namespace_uri), nsmap=(self.nsmap), fullpath=itemfullpath,
                          relpath=itemrelpath,
                          basename=(os.path.basename(itemfullpath)),
                          link=self.getURL(itemrelpath, handler=None),
                          icon=icon,
                          overlay=overlay)
        ...
        if len(filelist) > 0:
            ...
            xmlsearch = etree.Element(('{%s}search' % self._namespace_uri), resurl=(self._web_support.resurl),
