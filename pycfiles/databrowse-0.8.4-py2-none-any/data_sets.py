...
# [GCC 8.4.0]
# Embedded file name: C:\Files\Research\databrowse\databrowse\plugins\db_sdt_viewer\data_sets.py
...
class SDTDataSets:
    ...
    def check_preexisting(self):
        ...
        if 'frame' in self.parent._web_support.req.form:
            fprefix = 'Dataset_' + str(self.parent._web_support.req.form['dataset'].value) + '_Frame' + str(self.parent._web_support.req.form['frame'].value) + '_' + self.color
    ...
    def bscan_1(self):
        plt.imshow(self.ds['v'][:, 0, :], cmap=self.color, origin='lower', extent=[self.ds['x'][0],
         self.ds['x'][(-1)],
         self.ds['t'][0],
         self.ds['t'][(-1)]])
        axis = 'Axis 1'
    ...
    def bscan_2(self):
        plt.imshow(self.ds['v'][0, :, :], cmap=self.color, origin='lower', extent=[self.ds['y'][0],
         self.ds['y'][(-1)],
         self.ds['t'][0],
         self.ds['t'][(-1)]])
        axis = 'Axis 2'
    ...
    def cscan(self):
        ...
        plt.imshow(self.ds['v'][:, :, frame], cmap=self.color, origin='lower', extent=[self.ds['x'][0],
         self.ds['x'][(-1)],
         self.ds['y'][0],
         self.ds['y'][(-1)]])
        axis1 = 'Axis 1'
        ...
        self.fprefix = 'Dataset_' + str(self.parent._web_support.req.form['dataset'].value + '_Frame%d' % frame) + '_' + self.color
        self.ext = 'png'
        self.contenttype = 'image/png'
        self.save()
        self.size = os.path.getsize(self.parent.getCacheFileName(self.fprefix, self.ext))
