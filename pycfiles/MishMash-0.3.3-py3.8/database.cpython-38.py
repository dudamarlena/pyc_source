...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mishmash/database.py
# Compiled at: 2020-03-01 00:36:11
# Size of source mod 2**32: 3322 bytes
from pathlib import Path
from collections import namedtuple
import nicfit, alembic, alembic.config
...
def search(session, query):
    ...
    artists = session.query(Artist).filter(or_(Artist.name.ilike(f"%%{query}%%"), Artist.name.ilike(f"%%{flat_query}%%"))).all()
    albums = session.query(Album).filter(Album.title.ilike(f"%%{query}%%")).all()
    tracks = session.query(Track).filter(Track.title.ilike(f"%%{query}%%")).all()
    return dict(artists=artists, albums=albums,
      tracks=tracks)