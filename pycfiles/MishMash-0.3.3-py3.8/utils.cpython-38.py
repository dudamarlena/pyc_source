...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mishmash/commands/sync/utils.py
# Compiled at: 2019-12-28 16:02:25
    ...
    for album in session.query(Album).all():
        ...
        for artist in session.query(Artist).all():
            if artist.id == VARIOUS_ARTISTS_ID or artist.id in found_ids:
                ...
                any_track = session.query(Track).filter(Track.artist_id == artist.id).first()
                any_album = session.query(Album).filter(Album.artist_id == artist.id).first()
                if not any_track:
                    any_album and any_album.tracks or log.warn('Deleting artist: %s' % str(artist))
            ...
            session.flush()
            return (
             num_orphaned_tracks, num_orphaned_artists, num_orphaned_albums)
...
def syncImage(img, current, session):
    ...
    for db_img in current.images:
        img_info = (
         img.type, img.md5, img.size)
        db_img_info = (db_img.type, db_img.md5, db_img.size)
        if db_img_info == img_info:
            img = None
            break
    else:
        if db_img.type == img.type:
            if db_img.description == img.description:
