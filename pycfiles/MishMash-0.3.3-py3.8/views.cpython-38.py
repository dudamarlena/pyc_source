...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mishmash/web/views.py
...
from eyed3.core import LP_TYPE, EP_TYPE, COMP_TYPE, VARIOUS_TYPE, LIVE_TYPE, DEMO_TYPE, SINGLE_TYPE, ALBUM_TYPE_IDS
...
@view_config(route_name='home', renderer='templates/home.pt', layout='main-layout')
def home_view(request):
    return ResponseDict()


@view_config(route_name='all_artists', renderer='templates/artists.pt', layout='main-layout')
def allArtistsView(request):
    ...
    for artist in session.query(Artist).order_by(Artist.sort_name).all():
        types = set([alb.type for alb in artist.albums])

    for t in types:
        if t not in artist_types:
            artist_types[t] = {'active':t in active_types,  'display_name':TYPE_DISPLAY_NAMES[t]}
        ...
        for k in keys:
            ...
            return ResponseDict(artist_keys=keys, artist_dict=artist_dict, artist_types=artist_types, artist_count=count,
              key_columns=key_cols)


@view_config(route_name='artist', renderer='templates/artist.pt', layout='main-layout')
def artistView(request):
    ...
    if active_tab == SINGLE_TYPE:
        ...
        active_albums = []
    if active_albums:
        active_albums = util.sortByDate(active_albums, active_tab == LIVE_TYPE)
    else:
        for a in active_albums:
            covers = [img for img in a.images if img.type == Image.FRONT_COVER_TYPE]
            a.cover = random.choice(covers) if covers else None
        else:
            tabs = {}
            if artist.id != VARIOUS_ARTISTS_ID:
                for name in all_tabs:
                    t = dict(name=name, display_name=(TYPE_DISPLAY_NAMES[name]), active=(active_tab == name), has_items=(bool(len(_filterByType(name, artist, albums)))))
                    if name in tabs:
            ...
            return ResponseDict(artist=artist, active_tab=active_tab,
...
def covers(request):
    return _imageView(request, default_resp=Response(content_type='image/png', body=DEFAULT_COVER_DATA))
...
with open(os.path.join(os.path.dirname(__file__), 'static', 'record150.png'), 'rb') as (_cover_fp):
    DEFAULT_COVER_DATA = _cover_fp.read()
del _cover_fp

@view_config(route_name='new_music', renderer='templates/new_music.pt', layout='main-layout')
...
@view_config(route_name='album', renderer='templates/album.pt', layout='main-layout')
def albumView(request):
    ...
    if not album:
        raise HTTPNotFound()
    return ResponseDict(album=album, formatTime=formatTime)


@view_config(route_name='search', renderer='templates/search_results.pt', layout='main-layout')
...
@view_config(route_name='all_albums', renderer='templates/albums.pt', layout='main-layout')
def allAlbumsView(request):
    album_dict = {}
    album_types = {}
    if 'filter-form' in request.params and request.params.get('filter-form') == 'true':
        active_types = request.params.getall('type') or set()
    else:
        active_types = _getActiveAlbumTypes(request.params)
    session = request.DBSession
    for album in session.query(Album).order_by(Album.original_release_date).all():
        if album.type not in album_types:
            album_types[album.type] = {'active':album.type in active_types,  'display_name':TYPE_DISPLAY_NAMES[album.type]}
