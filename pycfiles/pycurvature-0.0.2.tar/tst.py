...
# Embedded file name: build\bdist.win32\egg\pycurry\tst.py
# Compiled at: 2009-03-01 05:27:14
...
import os, sys, inspect, threading, cProfile, pstats, logging, logging.config, unittest, collections, optparse, pycurry as pyc, pycurry.dbc as dbc, pycurry.err as err
...
class pylint(object):
    ...
    def __init__(self, data, config=cfg):
        ...
        try:
            ...
            print("install pylint from '%s'" % self.url, file=sys.stderr)
            raise
        except SystemExit:
            pass
...
class coverage(object):
    ...
    def __init__(self, modules, stream=sys.stdout):
        ...
        try:
            ...
            print("install coverage from '%s'" % self.url, file=sys.stderr)
            raise
...
class profiler(object):
    ...
    def init(cls, modules, info, sort, stream=sys.stdout):
        ...
        require: not [inf for inf in info                 if not inf in (cls.calls, cls.callers, cls.callees)]
        ...
        with cls.__sync:
            for mod in modules:
                if hasattr(mod, '__file__'):
                    if len(cls.__mods) != 0:
                        cls.__mods += '|'
                    cls.__mods += os.path.splitext(mod.__file__.replace('\\', '\\\\').replace('.', '\\.'))[0]
...
def main(suite, modules, package=None):
    ...
    """
    parser = optparse.OptionParser(description='Run your tests with optional static and dynamic analysis.')
    parser.add_option('-v', '--verbose', action='store_true', help='name the tests executing')
    parser.add_option('-d', '--debug', action='store_true', help='run the test in debug mode')
    parser.add_option('-l', '--logging', help='the logging configuration file to use')
    parser.add_option('--lint', action='store_true', help='perform static analysis on the code to test')
    parser.add_option('--lintrc', help='the lint configuration file to use')
    parser.add_option('-c', '--coverage', action='store_true', help='measure code coverage on the code to test')
    parser.add_option('-p', '--profiling', action='store_true', help='measure performance profiling on the code to test')
    parser.add_option('--statistics', action='append', choices=(
     profiler.calls, profiler.callers, profiler.callees), help='which profiling statistics to analyse')
    parser.add_option('--sort', choices=(
     profiler.calls, profiler.time), help='sort on number of calls or on cumulative time')
    parser.set_defaults(verbose=False, debug=False, logging='pycurry/log.ini', lint=False, lintrc=pylint.cfg, coverage=False, profiling=False, statistics=[
     profiler.calls], sort=profiler.calls)
    ...
    if loggingrc is None:
        parser.error("cannot find logging configuration file: '%s'" % options.logging)
    lintrc = pyc.resolve_filename(options.lintrc)
    if lintrc is None:
        parser.error("cannot find lint configuration file: '%s'" % options.lintrc)
    logging.config.fileConfig(loggingrc)
    err.unhandled_exception_handler.append_handler(err.print_exception_handler())
    if options.debug:
        err.unhandled_exception_handler.append_handler(err.pdb_exception_handler())
    else:
        err.unhandled_exception_handler.append_handler(err.log_exception_handler())
    if options.lint:
    ...
    if options.debug:
        ...
        err.unhandled_exception_handler.prepend_handler(err.unittest_exception_handler(runner.result))
        if options.coverage:
