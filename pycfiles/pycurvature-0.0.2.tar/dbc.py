...
class level(unittest.TestCase):
    ...
    def test_set(self):
        ...
        self.failUnlessEqual(sut.level.set(sut.level.invariant), sut.level.ensure)
    ...
    def test_lt(self):
        self.failUnless(sut.level(None) < sut.level(sut.level.require) < sut.level(sut.level.ensure) < sut.level(sut.level.invariant))
        self.failIf(sut.level(sut.level.invariant) < sut.level(sut.level.invariant))
...
class tls_condition(unittest.TestCase):
    ...
    def test_cur(self):
        with self.__sut as (test):
            self.failUnless(test)
            thd = threading.Thread(name=pyc.fully_qualified_name(self.test_cur), target=err.unhandled_exception_handler.handle, args=[
             self.test_n])
            thd.start()
            thd.join()
...
class scan(unittest.TestCase):
    ...
    def test_cls_defined_inv(self):
        ...
        self.failUnlessRaises(ValueError, sut.dbc_class.scan_inv, cls_defined)

    def test_cls_defined_pre(self):
        sut.dbc_class.scan_pre(cls_defined.defined, cls_defined)
        self.failUnlessRaises(ValueError, sut.dbc_class.scan_pre, cls_defined.defined, cls_defined)

    def test_cls_defined_post(self):
        sut.dbc_class.scan_post(cls_defined.defined, cls_defined)
        self.failUnlessRaises(ValueError, sut.dbc_class.scan_post, cls_defined.defined, cls_defined)

    def test_func_defined_pre(self):
        sut.dbc_class.scan_pre(func_defined)
        self.failUnlessRaises(ValueError, sut.dbc_class.scan_pre, func_defined)

    def test_func_defined_post(self):
        sut.dbc_class.scan_post(func_defined)
        self.failUnlessRaises(ValueError, sut.dbc_class.scan_post, func_defined)
    ...
    def test_cls_a_new_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_a.__new__, cls_a)

    def test_cls_a_del_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_a.__del__, cls_a)
    ...
    def test_cls_a_public_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_a.public, cls_a)
    ...
    def test_cls_a_but_override_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_a.but_override, cls_a)

    def test_cls_a_no_override_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_a.no_override, cls_a)

    def test_cls_b_init_pre(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, cls_b.__init__, cls_b)
    ...
    def test_func_override_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_pre, func_override)
    ...
    def test_cls_a_new_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, cls_a.__new__, cls_a)

    def test_cls_a_del_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, cls_a.__del__, cls_a)
    ...
    def test_cls_a_but_override_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, cls_a.but_override, cls_a)

    def test_cls_a_no_override_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, cls_a.no_override, cls_a)

    def test_cls_b_init_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, cls_b.__init__, cls_b)
    ...
    def test_func_override_post(self):
        self.failUnlessRaises(SyntaxError, sut.dbc_class.scan_post, func_override)
    ...
    def test_scan_func_locals(self):
        ...
        self.failUnlessEqual(sut.dbc_class.scan_locals(func_locals, spec, 1), {'a': 1, 'b': 0})
        self.failUnlessEqual(sut.dbc_class.scan_locals(func_locals, spec, 1, 2), {'a': 1, 'b': 2})
        self.failUnlessEqual(sut.dbc_class.scan_locals(func_locals, spec, a=1), {'a': 1, 'b': 0})
        self.failUnlessEqual(sut.dbc_class.scan_locals(func_locals, spec, a=1, b=2), {'a': 1, 'b': 2})
        self.failUnlessRaises(TypeError, sut.dbc_class.scan_locals, func_locals, spec)
        self.failUnlessRaises(TypeError, sut.dbc_class.scan_locals, func_locals, spec, 1, 2, 3)
        self.failUnlessRaises(TypeError, sut.dbc_class.scan_locals, func_locals, spec, c=3)
...
class parse(unittest.TestCase):
    ...
    def test_cls_d_inv(self):
        sut.dbc_class.parse_inv({'_cls_a__inv': True, '_cls_b__inv': True}, cls_d.override, cls_d)
        self.failUnlessRaises(sut.class_invariant_exception, sut.dbc_class.parse_inv, {'_cls_a__inv': True, '_cls_b__inv': False}, cls_d.override, cls_d)
        self.failUnlessRaises(sut.class_invariant_exception, sut.dbc_class.parse_inv, {'_cls_a__inv': False, '_cls_b__inv': True}, cls_d.override, cls_d)
        self.failUnlessRaises(sut.class_invariant_exception, sut.dbc_class.parse_inv, {'_cls_a__inv': False, '_cls_b__inv': False}, cls_d.override, cls_d)
    ...
    def test_cls_a_public_post(self):
        ...
        self.failUnlessRaises(sut.postcondition_exception, sut.dbc_class.parse_post, {}, False, cls_a.public, cls_a)

    def test_cls_d_override_post(self):
        sut.dbc_class.parse_post({'_cls_a__post': True, '_cls_b__post': True}, None, cls_d.override, cls_d)
        self.failUnlessRaises(sut.postcondition_exception, sut.dbc_class.parse_post, {'_cls_a__post': True, '_cls_b__post': False}, None, cls_d.override, cls_d)
        self.failUnlessRaises(sut.postcondition_exception, sut.dbc_class.parse_post, {'_cls_a__post': False, '_cls_b__post': True}, None, cls_d.override, cls_d)
        self.failUnlessRaises(sut.postcondition_exception, sut.dbc_class.parse_post, {'_cls_a__post': False, '_cls_b__post': False}, None, cls_d.override, cls_d)
        return
    ...
    def test_func_contract_post(self):
        ...
        self.failUnlessRaises(sut.postcondition_exception, sut.dbc_class.parse_post, {}, False, func_contract)
    ...
    def test_cls_d_override_pre(self):
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_b': True}, cls_d.override, cls_d)
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_b': False}, cls_d.override, cls_d)
        sut.dbc_class.parse_pre({'pre_a': False, 'pre_b': True}, cls_d.override, cls_d)
        sut.dbc_class.parse_pre({'pre_a': False, 'pre_b': False}, cls_d.override, cls_d)

    def test_cls_a_foo_pre(self):
        sut.dbc_class.parse_pre({'pre_a': True}, cls_a.foo, cls_a)
        self.failUnlessRaises(sut.precondition_exception, sut.dbc_class.parse_pre, {'pre_a': False}, cls_a.foo, cls_a)

    def test_cls_b_foo_pre(self):
        sut.dbc_class.parse_pre({'pre_a': True}, cls_b.foo, cls_b)
        self.failUnlessRaises(sut.precondition_exception, sut.dbc_class.parse_pre, {'pre_a': False}, cls_b.foo, cls_b)

    def test_cls_c_foo_pre(self):
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_c': True}, cls_c.foo, cls_c)
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_c': False}, cls_c.foo, cls_c)
        sut.dbc_class.parse_pre({'pre_a': False, 'pre_c': True}, cls_c.foo, cls_c)
        self.failUnlessRaises(sut.precondition_exception, sut.dbc_class.parse_pre, {'pre_a': False, 'pre_c': False}, cls_c.foo, cls_c)

    def test_cls_d_foo_pre(self):
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_c': True}, cls_d.foo, cls_d)
        sut.dbc_class.parse_pre({'pre_a': True, 'pre_c': False}, cls_d.foo, cls_d)
        sut.dbc_class.parse_pre({'pre_a': False, 'pre_c': True}, cls_d.foo, cls_d)
        self.failUnlessRaises(sut.precondition_exception, sut.dbc_class.parse_pre, {'pre_a': False, 'pre_c': False}, cls_d.foo, cls_d)
    ...
    def test_func_contract_pre(self):
        ...
        self.failUnlessRaises(sut.precondition_exception, sut.dbc_class.parse_pre, {'pre': False}, func_contract)
...
class dbc_class(unittest.TestCase):
    ...
    def test_require(self):
        import pycurry.test.xyz as xyz
        xyz.require().foo()
        self.failUnlessRaises(sut.precondition_exception, xyz.require().foo, False)

    def test_ensure(self):
        import pycurry.test.xyz as xyz
        xyz.ensure().foo()
        self.failUnlessRaises(sut.precondition_exception, xyz.ensure().foo, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, xyz.ensure().foo, post=False)

    def test_invariant(self):
        import pycurry.test.xyz as xyz
        xyz.invariant().foo()
        self.failUnlessRaises(sut.precondition_exception, xyz.invariant().foo, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, xyz.invariant().foo, post=False)
        self.failUnlessRaises(sut.class_invariant_exception, xyz.invariant, False)
...
class doc_test(unittest.TestCase):
    ...
    def test_require(self):
        ...
        except sut.precondition_exception, e:
            str(e)
    ...
    def test_ensure(self):
        ...
        except sut.postcondition_exception, e:
            str(e)
    ...
    def test_invariant(self):
        ...
        except sut.class_invariant_exception, e:
            str(e)
...
class access_test(unittest.TestCase):
    ...
    def test_public(self):
        self.failUnlessRaises(sut.precondition_exception, self.__impl.public, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, self.__impl.public, post=False)

    def test_protected(self):
        self.failUnlessRaises(sut.precondition_exception, self.__impl._protected, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, self.__impl._protected, post=False)

    def test_private(self):
        self.failUnlessRaises(sut.precondition_exception, self.__impl._access__private, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, self.__impl._access__private, post=False)

    def test_internal(self):
        self.failUnlessRaises(sut.precondition_exception, self.__impl.__internal__, pre=False)
        self.failUnlessRaises(sut.postcondition_exception, self.__impl.__internal__, post=False)
...
class interface(object):
    ...
    def foo(self, int_pre=True, int_post=True, abs_pre=True, abs_post=True, con_pre=True, con_post=True):
        """
        require: int_pre
        ensure: int_post
        """
...
class abstract_class(interface):
    ...
    def __init__(self, abs_pre=True, abs_post=True, abs_inv=True):
        """
        require: abs_pre
        ensure: abs_post
        """
...
class concrete_class(abstract_class):
    ...
    def __init__(self, abs_pre=True, abs_post=True, abs_inv=True, con_pre=True, con_post=True, con_inv=True, con_abs_inv=True):
        """
        require: con_pre
        ensure: con_post
        """
...
class inheritance_test(unittest.TestCase):
    ...
    def test_foo10(self):
        try:
            concrete_class().foo(int_pre=False, abs_pre=False, con_pre=False)
        except sut.precondition_exception, e:
    ...
    def test_foo14(self):
        try:
            concrete_class().foo(int_post=False, abs_post=False, con_post=False)
        except sut.postcondition_exception, e:
...
class levels_test(unittest.TestCase):
    ...
    def test_req1(self):
        self.failUnlessRaises(sut.precondition_exception, req_level, False)

    def test_req2(self):
        self.failUnlessRaises(sut.precondition_exception, req_level().foo, False)

    def test_ens0(self):
        ens_level().foo()

    def test_ens1(self):
        self.failUnlessRaises(sut.postcondition_exception, ens_level, False)

    def test_ens2(self):
        self.failUnlessRaises(sut.postcondition_exception, ens_level().foo, False)

    def test_inv0(self):
        inv_level().foo()

    def test_inv1(self):
        self.failUnlessRaises(sut.class_invariant_exception, inv_level, False)

    def test_inv2(self):
        self.failUnlessRaises(sut.class_invariant_exception, inv_level().foo, False)
...
class dbc_function(unittest.TestCase):
    ...
    def test_require(self):
        with sut.level(sut.level.require):
            ...
            self.failUnless(foo() is True)
            self.failUnlessRaises(sut.precondition_exception, foo, pre=False)
            self.failUnless(foo(result=False) is False)
    ...
    def test_ensure(self):
        with sut.level(sut.level.ensure):
            ...
            self.failUnless(foo() is True)
            self.failUnlessRaises(sut.precondition_exception, foo, pre=False)
            self.failUnlessRaises(sut.postcondition_exception, foo, result=False)
