...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/CIDAN/LSSC/functions/data_manipulation.py
...
def load_filter_tif_stack(*, path, filter: bool, median_filter: bool, median_filter_size: Tuple[(int, int, int)], z_score: bool, slice_stack: bool, slice_every, slice_start: int):
    """
    ...
    if type(path) == list or os.path.isdir(path):
        ...
        for num, x in enumerate(paths):
            ...
            if filter:
                image = filter_stack(stack=image, median_filter=median_filter, median_filter_size=median_filter_size,
                  z_score=z_score)
...
def save_image(volume: np.ndarray, name: str, directory: str, shape: tuple, number_save: int):
    ...
    for x in range(number_save):
        fig, ax = plt.subplots()
        imgplot = ax.imshow(np.reshape(volume, shape)[(shape[0] // number_save * x)])
        fig.savefig(os.path.join(directory, name + '_' + str(x)))


def filter_stack(*, stack: np.ndarray, median_filter: bool, median_filter_size: Tuple[(int, int, int)], z_score: bool):
    if z_score:
...
def pixel_num_to_2d_cord(pixel_list, volume_shape):
    ...
    def convert_one(num):
        return (
         num % volume_shape[2], num // volume_shape[2])
