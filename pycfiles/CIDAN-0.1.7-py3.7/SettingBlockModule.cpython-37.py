...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/CIDAN/GUI/SettingWidget/SettingBlockModule.py
# Compiled at: 2020-04-29 16:30:52
# Size of source mod 2**32: 14900 bytes
import CIDAN.GUI.Inputs.BoolInput as BoolInput
import CIDAN.GUI.Inputs.IntInput as IntInput
import CIDAN.GUI.Inputs.FloatInput as FloatInput
...
def filter_setting_block(main_widget):
    ...
    return SettingBlockModule('Filter Settings', [
     BoolInput(display_name='Median filter:', program_name='median_filter',
     ...
     IntInput(display_name='Median filter size:',
       program_name='median_filter_size',
       on_change_function=(lambda x, y: data_handler.change_filter_param(x, y)),
       default_val=(data_handler.filter_params['median_filter_size']),
       tool_tip='The size of the median filter for each timestep',
       min=1,
       max=50,
       step=1),
     BoolInput(display_name='Z-score:',
       program_name='z_score',
       on_change_function=(lambda x, y: data_handler.change_filter_param(x, y)),
       default_val=(data_handler.filter_params['z_score']),
...
def dataset_setting_block(main_widget):
    ...
    return SettingBlockModule('Dataset Settings', [
     BoolInput(display_name='Slice stack:', program_name='slice_stack',
       on_change_function=(lambda x, y: data_handler.change_dataset_param(x, y)),
       default_val=(data_handler.dataset_params['slice_stack']),
       tool_tip='Used to Select only certain timepoints from a dataset',
       display_tool_tip=True),
     IntInput(display_name='Slice every:', program_name='slice_every',
       on_change_function=(lambda x, y: data_handler.change_dataset_param(x, y)),
       ...
       max=100,
       step=1),
     IntInput(display_name='Slice start:', program_name='slice_start',
       on_change_function=(lambda x, y: data_handler.change_dataset_param(x, y)),
...
def roi_extraction_settings_block(main_widget):
    ...
    return SettingBlockModule('ROI Extraction Settings', [
     IntInput(display_name='Max Number of ROIs per spatial box:', program_name='num_rois',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['num_rois']),
       tool_tip='Max number of ROIs to select for each spatial box',
       min=1,
       max=1000,
       step=1),
     IntInput(display_name='ROI size minimum:',
       program_name='roi_size_min',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['roi_size_min']),
       tool_tip='Minimum size in pixels for a region of interest',
       min=1,
       max=1000,
       step=1),
     IntInput(display_name='ROI size maximum:',
       program_name='roi_size_max',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['roi_size_max']),
       tool_tip='Maximum size in pixels for a region of interest',
       min=1,
       max=100000,
       step=1),
     BoolInput(display_name='Run refinement step:',
       program_name='refinement',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['refinement']),
       tool_tip='Whether to run the refinement step, greatly improves results'),
     FloatInput(display_name='Merge temporal coefficient:',
       program_name='merge_temporal_coef',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['merge_temporal_coef']),
       tool_tip='The coefficient that determines if two overlapping regions are merged based on their temporal correlation',
       min=0,
       max=1,
       step=0.01),
     BoolInput(display_name='Fill holes:',
       program_name='fill_holes',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['fill_holes']),
...
def roi_advanced_settings_block(main_widget):
    ...
    return SettingBlockModule('Advanced Settings', [
     IntInput(display_name='Number of eigen vectors:', program_name='num_eig',
       on_change_function=(lambda x, y: data_handler.change_eigen_param(x, y)),
       default_val=(data_handler.eigen_params['num_eig']),
       tool_tip='Number of eigen vectors to generate',
       min=1,
       max=1000,
       step=1),
     FloatInput(display_name='Eigen threshold value:', program_name='eigen_threshold_value',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['eigen_threshold_value']),
       tool_tip='Number of eigen vectors to select at each point',
       min=0,
       max=1,
       step=0.01),
     FloatInput(display_name='Elbow threshold value:', program_name='elbow_threshold_value',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['elbow_threshold_value']),
       tool_tip='Number of eigen vectors to select at each point',
       min=0,
       max=1,
       step=0.01),
     BoolInput(display_name='Merge ROIs:',
       program_name='merge',
       on_change_function=(lambda x, y: data_handler.change_roi_extraction_param(x, y)),
       default_val=(data_handler.roi_extraction_params['merge']),
       tool_tip='Whether to merge rois with similar time traces'),
     IntInput(display_name='Number of timesteps:', program_name='total_num_time_steps',
       on_change_function=(lambda x, y: data_handler.change_box_param(x, y)),
