...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/CIDAN/LSSC/functions/roi_extraction.py
# Compiled at: 2020-04-29 02:03:11
# Size of source mod 2**32: 19748 bytes
import numpy as np
...
import scipy.sparse.csgraph as connected_components_graph
from scipy.sparse import csr_matrix
from dask import delayed

@delayed
def roi_extract_image(*, e_vectors: np.ndarray, original_shape: tuple, original_2d_vol: np.ndarray, merge: bool, num_rois: int, refinement: bool, num_eigen_vector_select: int, max_iter: int, roi_size_min: int, fill_holes: bool, elbow_threshold_method: bool, elbow_threshold_value: float, eigen_threshold_method: bool, eigen_threshold_value: float, merge_temporal_coef: float, roi_size_limit: int, box_num: int) -> List[np.ndarray]:
    """
    ...
    pixel_embedings = embed_eigen_norm(e_vectors)
    initial_pixel_list = np.flip(np.argsort(pixel_embedings))
    roi_list = []
    iter_counter = 0
    while len(roi_list) < num_rois:
        if len(initial_pixel_list) > 0 and iter_counter < max_iter:
            iter_counter += 1
            initial_pixel = initial_pixel_list[0]
            ...
            small_pixel_distance = pixel_distance(small_eigen_vectors, initial_pixel)
            pixels_in_roi = np.nonzero(small_pixel_distance <= small_pixel_embeding_norm)[0]
            pixels_in_roi_comp = connected_component(pixel_length, original_shape, pixels_in_roi, initial_pixel)
            pixels_in_roi_final = pixels_in_roi_comp
            if refinement:
                rf_eigen_vectors = select_eigen_vectors(e_vectors, pixels_in_roi_final,
                  num_eigen_vector_select,
                  threshold_method=eigen_threshold_method,
                  threshold=eigen_threshold_value)
                rf_pixel_embedding_norm = embed_eigen_norm(rf_eigen_vectors)
                rf_initial_point = rf_select_initial_point(rf_pixel_embedding_norm, pixels_in_roi_final)
                rf_pixel_distance = pixel_distance(rf_eigen_vectors, rf_initial_point)
                if elbow_threshold_method:
                    threshold = elbow_threshold_value * elbow_threshold(rf_pixel_distance, (np.argsort(rf_pixel_distance)),
                      half=True,
                      num=iter_counter)
                    rf_pixels_in_roi = np.nonzero(rf_pixel_distance < threshold)[0]
                else:
                    rf_pixels_in_roi = np.nonzero(rf_pixel_distance <= rf_pixel_embedding_norm)[0]
                rf_pixels_in_roi_comp = connected_component(pixel_length, original_shape, rf_pixels_in_roi, rf_initial_point)
                pixels_in_roi_final = rf_pixels_in_roi_comp
            if roi_size_min < len(pixels_in_roi_final) < roi_size_limit:
                roi_list.append(pixels_in_roi_final)
                initial_pixel_list = np.extract(np.in1d(initial_pixel_list, pixels_in_roi_final, assume_unique=True,
                  invert=True), initial_pixel_list)
                if initial_pixel not in pixels_in_roi_final:
                    initial_pixel_list = np.delete(initial_pixel_list, 0)
        else:
            initial_pixel_list = np.delete(np.append(initial_pixel_list, initial_pixel_list[0]), 0)

    if fill_holes:
        roi_list = fill_holes_func(roi_list, pixel_length, original_shape)
    if merge:
        roi_list = merge_rois(roi_list, temporal_coefficient=merge_temporal_coef,
...
def fill_holes_func(roi_list: List[np.ndarray], pixel_length: int, original_shape: Tuple[int]) -> List[np.ndarray]:
    """
    ...
    for num, roi in enumerate(roi_list):
        ...
        image_filled = binary_fill_holes(np.reshape(original_zeros, original_shape[1:]))
...
def pixel_distance(eigen_vectors: np.ndarray, pixel_num: int) -> np.ndarray:
    ...
    """
    return np.sum((np.power(eigen_vectors - eigen_vectors[pixel_num], 2)), axis=1)


def connected_component(pixel_length: int, original_shape: Tuple[int], pixels_in_roi: np.ndarray, initial_pixel_number: int) -> np.ndarray:
    """
    ...
    pixels_in_roi_new = np.nonzero(blobs_labels == correct_label)[0]
    return pixels_in_roi_new


def select_eigen_vectors(eigen_vectors: np.ndarray, pixels_in_roi: np.ndarray, num_eigen_vector_select: int, threshold_method: bool=False, threshold: float=0.1) -> np.ndarray:
    """
    ...
    if threshold_method:
        threshold_filter = pixel_eigen_vec_values > threshold * pixel_eigen_vec_values[pixel_eigen_vec_values_sort_indices[0]]
        small_eigen_vectors = eigen_vectors[:, np.nonzero(threshold_filter)[0]]
    else:
        pixel_eigen_vec_values_sort_indices = np.flip(np.argsort(pixel_eigen_vec_values))
        small_eigen_vectors = eigen_vectors[:,
         pixel_eigen_vec_values_sort_indices[:num_eigen_vector_select]]
    return small_eigen_vectors


def rf_select_initial_point(pixel_embedings: np.ndarray, pixels_in_roi: np.ndarray):
    """
    ...
    """
    indice_in_roi = np.flip(np.argsort(pixel_embedings[pixels_in_roi]))[0]
    return np.sort(pixels_in_roi)[indice_in_roi]


def elbow_threshold(pixel_vals: np.ndarray, pixel_val_sort_indices: np.ndarray, half: bool=True, num=0) -> float:
    """
    ...
    pixel_vals_sorted_zipped = np.array(list(zip(range(n_points), pixel_vals[pixel_val_sort_indices[:n_points]])))
    first_point = pixel_vals_sorted_zipped[0, :]
    last_point = pixel_vals_sorted_zipped[-1, :]
    line_vec = last_point - first_point
    line_vec_norm = line_vec / np.sum(np.power(line_vec, 2)) ** 0.5
    dist_from_first = pixel_vals_sorted_zipped - first_point
    proj_point_to_line = np.matmul(dist_from_first, line_vec_norm[:, None]) * line_vec_norm
    vec_to_line = dist_from_first - proj_point_to_line
    dist_to_line = np.power(np.sum((np.power(vec_to_line, 2)), axis=1), 0.5)
    dist_max_indice = np.argmax(dist_to_line)
    threshold = pixel_vals_sorted_zipped[dist_max_indice][1]
    return threshold


def merge_rois(roi_list: List, temporal_coefficient: float, original_2d_vol: np.ndarray):
    """
...
def compare_time_traces(trace_1, trace_2):
    ...
    bottom = (np.dot(trace_1_sub_mean, trace_1_sub_mean) * np.dot(trace_2_sub_mean, trace_2_sub_mean)) ** 0.5
    return top / bottom


def compare_roi(roi1: List[int], roi2: List[int], temporal_coefficient: int, original_2d_vol: np.ndarray) -> bool:
    """
    ...
    -------
    """
    if any([x in roi2 for x in roi1]):
        roi1_time_trace_avg = np.mean((original_2d_vol[roi1]), axis=0)
        roi2_time_trace_avg = np.mean((original_2d_vol[roi2]), axis=0)
        roi1_time_trace_avg_z_scored = (roi1_time_trace_avg - np.mean(roi1_time_trace_avg)) / np.std(roi1_time_trace_avg)
        roi2_time_trace_avg_z_scored = (roi2_time_trace_avg - np.mean(roi2_time_trace_avg)) / np.std(roi2_time_trace_avg)
        if np.linalg.norm(roi1_time_trace_avg_z_scored - roi2_time_trace_avg_z_scored) < temporal_coefficient * original_2d_vol.shape[0] ** 0.5:
            return True
    return False
