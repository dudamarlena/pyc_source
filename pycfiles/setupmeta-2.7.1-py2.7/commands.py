...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/setupmeta/commands.py
# Compiled at: 2020-04-30 15:23:01
"""
Commands contributed by setupmeta
"""
import collections, os, platform, shutil, sys
from distutils.command.check import check as check_cmd
from itertools import chain
import setuptools, setupmeta
        ...
        if reqs:
            with setupmeta.temp_resource():
                ...
                for req in reqs:
                    args = common_args[:]
                    args.append(req)
                    setupmeta.run_program(easy_install, env=env, *args)
...
class TwineCommand(setuptools.Command):
    ...
    def run_command(self, message, *args):
        ...
        print message
        setupmeta.run_program(fatal=True, *args)
