...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/cerberus/client.py
# Compiled at: 2020-02-19 14:41:54
...
import ast, json, logging, sys, warnings, os
logger = logging.getLogger(__name__)

class CerberusClient(object):
    __doc__ = ' Cerberus Python Client for interacting with\n        Cerberus APIs. Authentication is done\n        via the Auth Classes'
    HEADERS = {'Content-Type': 'application/json'}

    def __init__(self, cerberus_url, username=None, password=None, region='us-west-2', token=None, aws_session=None, verbose=None):
        """
    ...
    def get_categories(self):
        ...
        sdb_resp = get_with_retry((self.cerberus_url + '/v1/category'), headers=(self.HEADERS))
    ...
    def create_sdb(self, name, category_id, owner, description='', user_group_permissions=None, iam_principal_permissions=None):
        ...
        temp_data = {'name':name,  'description':description, 
    ...
    def delete_sdb(self, sdb_id):
        ...
        sdb_resp = delete_with_retry((self.cerberus_url + '/v2/safe-deposit-box/' + sdb_id), headers=(self.HEADERS))
        throw_if_bad_response(sdb_resp)
        return sdb_resp
    ...
    def get_sdb_keys(self, path):
        ...
        list_resp = get_with_retry((self.cerberus_url + '/v1/secret/' + path + '/?list=true'),
          headers=(self.HEADERS))
    ...
    def get_sdb_by_id(self, sdb_id):
        ...
        sdb_id -- this is the id of the safe deposit box, not the path.
        """
        sdb_resp = get_with_retry((self.cerberus_url + '/v2/safe-deposit-box/' + sdb_id), headers=(self.HEADERS))
        throw_if_bad_response(sdb_resp)
        return sdb_resp.json()
    ...
    def get_sdb_secret_version_paths(self, sdb_id):
        ...
        sdb_resp = get_with_retry((str.join('', [self.cerberus_url, '/v1/sdb-secret-version-paths/', sdb_id])), headers=(self.HEADERS))
        throw_if_bad_response(sdb_resp)
        return sdb_resp.json()
    ...
    def update_sdb(self, sdb_id, owner=None, description=None, user_group_permissions=None, iam_principal_permissions=None):
        ...
        if user_group_permissions is not None:
            if len(user_group_permissions) > 0:
                temp_data['user_group_permissions'] = user_group_permissions
        if iam_principal_permissions is not None:
            if len(iam_principal_permissions) > 0:
                temp_data['iam_principal_permissions'] = iam_principal_permissions
        data = json.encoder.JSONEncoder().encode(temp_data)
        sdb_resp = put_with_retry((self.cerberus_url + '/v2/safe-deposit-box/' + sdb_id), data=(str(data)), headers=(self.HEADERS))
        throw_if_bad_response(sdb_resp)
        return sdb_resp.json()

    def delete_file(self, secure_data_path):
        """Delete a file at the given secure data path"""
        secret_resp = delete_with_retry((self.cerberus_url + '/v1/secure-file/' + secure_data_path), headers=(self.HEADERS))
    ...
    def get_file_metadata(self, secure_data_path, version=None):
        ...
        secret_resp = head_with_retry((str.join('', [self.cerberus_url, '/v1/secure-file/', secure_data_path])), params=payload,
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
    ...
    def _get_file(self, secure_data_path, version=None):
        ...
        if not version:
            version = 'CURRENT'
        payload = {'versionId': str(version)}
        secret_resp = get_with_retry((str.join('', [self.cerberus_url, '/v1/secure-file/', secure_data_path])), params=payload,
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp
    ...
    def list_files(self, secure_data_path, limit=None, offset=None):
        ...
        secret_resp = get_with_retry((self.cerberus_url + '/v1/secure-files/' + secure_data_path), params=payload,
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
    ...
    def put_file(self, secure_data_path, filehandle, content_type=None):
        ...
        secret_resp = post_with_retry((self.cerberus_url + '/v1/secure-file/' + secure_data_path), files=data,
          headers=headers)
        throw_if_bad_response(secret_resp)
        return secret_resp
    ...
    def delete_secret(self, secure_data_path):
        ...
        secret_resp = delete_with_retry((self.cerberus_url + '/v1/secret/' + secure_data_path), headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp
    ...
    def get_secret(self, secure_data_path, key, version=None):
        ...
        """
        warnings.warn('get_secret is deprecated, use get_secrets_data instead', DeprecationWarning)
    ...
    def _get_secrets(self, secure_data_path, version=None):
        ...
        if not version:
            version = 'CURRENT'
        payload = {'versionId': str(version)}
        secret_resp = get_with_retry((str.join('', [self.cerberus_url, '/v1/secret/', secure_data_path])), params=payload,
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp.json()
    ...
    def get_secrets(self, secure_data_path, version=None):
        ...
        """
        warnings.warn('get_secrets is deprecated, use get_secrets_data instead', DeprecationWarning)
    ...
    def get_secret_versions(self, secure_data_path, limit=None, offset=None):
        ...
        if not offset or offset < 0:
            offset = 0
        payload = {'limit':str(limit),  'offset':str(offset)}
        secret_resp = get_with_retry((str.join('', [self.cerberus_url, '/v1/secret-versions/', secure_data_path])), params=payload,
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp.json()
    ...
    def list_secrets(self, secure_data_path):
        ...
        secure_data_path = self._add_slash(secure_data_path)
        secret_resp = get_with_retry((self.cerberus_url + '/v1/secret/' + secure_data_path + '?list=true'), headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp.json()
    ...
    def put_secret(self, secure_data_path, secret, merge=True):
        ...
        secret_resp = post_with_retry((self.cerberus_url + '/v1/secret/' + secure_data_path), data=(str(data)),
          headers=(self.HEADERS))
        throw_if_bad_response(secret_resp)
        return secret_resp
    ...
    def secret_merge(self, secure_data_path, key):
        ...
        temp_key = {}
        if get_resp.status_code == requests.codes.bad:
