...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/vb_ihmm/model/states.py
# Compiled at: 2014-03-13 01:38:48
"""
...
class Mixture(DiscreteStates):
    ...
    def __init__(self, sensors, N, K, hyperparams=None):
        ...
        if hyperparams is None:
            hyperparams = {'alpha_tau_pi0': ones(K), 'alpha_tau_pi1': ones(K), 'c': 1e-06, 
               'd': 0.1}
    ...
    def m(self):
        ...
        self._tau_c = self._hyperparams['c'] + 1.0
...
class HMM(DiscreteStates):
    ...
    def m(self, sensor_exp_z=None):
        ...
        self._tau_c = self._hyperparams['c'] + 1.0
        self._tau_d = self._hyperparams['d'] - self._acc(self._tau_a0, self._tau_a1)
        if sensor_exp_z is None:
            sensor_exp_z = exp_z
        [ s.m(exp_z) for s in self._sensors ]
        return
    ...
    def _do_e(self, ln_obs_lik):
        ...
        self._exp_z[self._freeNs, :] = self._eZ(ln_alpha_exp_z, ln_beta_exp_z, N)[self._freeNs, :]
        for vi, i in zip(self._fixedNs, itt.count()):
            self._exp_z[vi, :] = self._valExpZFixed[i, :]
