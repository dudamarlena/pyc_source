...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/obol/main.py
# Compiled at: 2016-07-19 18:08:33
import sys, logging
...
class ObolApp(App):
    ...
    def __init__(self):
        ...
        super(ObolApp, self).__init__(description='Obol: LDAP command line tool', version='2.0', command_manager=manager, deferred_help=True)
        config = ConfigParser({'user_tree': 'ou=People', 'group_tree': 'ou=Group'})
    ...
    def build_option_parser(self, description, version, argparse_kwargs=None):
        parser = super(ObolApp, self).build_option_parser(description, version, argparse_kwargs)
        parser.add_argument('-D', metavar='BIND DN', default=self.default('bindDN', 'cn=Manager,dc=local'))
        parser.add_argument('-w', metavar='BIND PASSWORD', default=self.default('password'))
        parser.add_argument('-H', metavar='HOST', default=self.default('host', 'ldap://localhost'))
        parser.add_argument('-b', metavar='BASE_DN', default=self.default('baseDN', 'dc=local'))
        loglevels = [ key for key in logging._levelNames if isinstance(key, str)
                    ]
        parser.add_argument('--logLevel', choices=loglevels, default=self.default('logLevel', 'INFO'))
        return parser
