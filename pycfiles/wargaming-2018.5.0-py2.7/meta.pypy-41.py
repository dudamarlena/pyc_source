...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wargaming/meta.py
# Compiled at: 2018-05-14 16:33:13
import os, json, requests, six
from retrying import retry
from datetime import datetime
from bs4 import BeautifulSoup
from wargaming.exceptions import RequestError, ValidationError
from wargaming.settings import ALLOWED_GAMES, ALLOWED_REGIONS, HTTP_USER_AGENT_HEADER, RETRY_COUNT, GAME_API_ENDPOINTS, REGION_TLD_MAP
...
class WGAPI(object):
    ...
    def __init__(self, url, http_method='GET', parser=None, stop_max_attempt_number=RETRY_COUNT, **kwargs):
        ...
        self._data = None
        self.error = None
        self._fetch_data = retry(stop_max_attempt_number=stop_max_attempt_number, retry_on_exception=lambda ex: isinstance(ex, RequestError) and ex.code == 504)(self._fetch_data)

    def _fetch_data(self):
        if self.http_method == 'GET':
            response = requests.get(self.url, params=self.params, headers={'User-Agent': HTTP_USER_AGENT_HEADER})
        else:
            if self.http_method == 'POST':
                response = requests.post(self.url, data=self.params, headers={'User-Agent': HTTP_USER_AGENT_HEADER})
            else:
...
class MetaAPI(type):
    ...
    def __new__(mcs, name, bases, attrs):
        ...
        def make_api_call(call_schema):
            ...
            parameters = {parameter['name']:parameter for parameter in call_schema['parameters']}
                ...
                for field, value in kwargs.items():
                    if field not in parameters:
                        ...
                        if field_id in parameters and isinstance(value, dict) and field_id in value:
                            ...
                            del kwargs[field]
                        elif field_id in parameters and isinstance(value, list) and isinstance(value[0], dict) and field_id in value[0]:
                            kwargs[field_id] = [ i[field_id] ]
                            del kwargs[field]
                        else:
