...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/bayes_on_redis/bayes_on_redis.py
# Compiled at: 2011-06-28 00:33:11
import operator, math, os.path, re
from redis import Redis

class BayesOnRedis:
    categories_key = 'BayesOnRedis:categories'
    one_or_two_words_re = re.compile('\\b[^\\s]{1,2}\\b', re.IGNORECASE)
    non_alphanumeric_and_non_dot_re = re.compile('[^\\w\\.]', re.IGNORECASE)
...
class Stopword:
    ...
    def to_re(self):
        if not self.stopwords_re:
            self.stopwords_re = re.compile('\\b(%s)\\b' % ('|').join(self.to_list()), re.IGNORECASE)
