...
# Embedded file name: build/bdist.linux-x86_64/egg/advancedcaching/hildongui.py
...
import geo, geocaching, gobject, gtk, hildon
...
class HildonGui(HildonToolsDialog, HildonSearchPlace, HildonFieldnotes, HildonSearchGeocaches, HildonAboutDialog, HildonDownloadMap, SimpleGui):
    ...
    def _on_show_preferences(self, caller, event):
        ...
        update = {'tts_interval': tts_get_result(), 
    ...
    def _on_show_settings_dialog(self, widget, data):
        ...
        if result != gtk.RESPONSE_ACCEPT:
            ...
            if self.settings['options_show_html_description'] != check_show_html_description.get_active():
                self.old_cache_window = None
            update = {'options_username': username.get_text(), 
    ...
    def _get_rotate_settings(self):
        ...
        return (
         rotate_screen, rotate_get_result)
    ...
    def _show_target_input_list(self, start, show_current=False):
        ...
        if result == gtk.RESPONSE_ACCEPT:
            ...
            if result == RESPONSE_CURRENT and current != None:
                return current
            return
    ...
    def show_cache(self, cache, select=True):
        if cache == None:
            ...
            if self.current_cache != None:
                if self.current_cache.name == cache.name and self.old_cache_window != None:
                ...
                if cache.was_downloaded():
                    ...
                    text_longdesc = re.sub('(?i)<img[^>]+?>', ' [to get all images, re-download description] ', re.sub('\\[\\[img:([^\\]]+)\\]\\]', lambda a: self._replace_image_callback(a, cache), cache.desc))
                    widget_description = self.settings['options_show_html_description'] or gtk.Label()
    ...
    def _on_edit_user_coordinate(self, caller, cache, source_id=None, coordinate=None):
        if source_id == None:
            ...
            if info['type'] == geocaching.GeocacheCoordinate.USER_TYPE_CALC_STRING:
                ...
                if info['type'] == geocaching.GeocacheCoordinate.USER_TYPE_COORDINATE:
            ...
            if res == gtk.RESPONSE_ACCEPT:
                try:
                    ...
                    if source_id == None:
                        ...
                        value = (
                         coordinate.signature, after_calc)
                        id = None
    ...
    def __show_edit_calculation(self, before_calc, before_name, button_text):
        ...
        dialog.hide()
        return (
         res, entry_name.get_text(), entry_calc.get_text())
    ...
    def _on_cache_changed(self, something, cache):
        if self.current_cache != None and cache.name == self.current_cache.name and self.current_cache_window_open:
