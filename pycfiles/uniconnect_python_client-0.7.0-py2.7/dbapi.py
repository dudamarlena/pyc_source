...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/uniconnect/dbapi.py
...
import datetime
from uniconnect import constants
import uniconnect.exceptions, uniconnect.client, uniconnect.logging, uniconnect.redirect
from uniconnect.transaction import Transaction, IsolationLevel, NO_TRANSACTION
__all__ = [
...
class Connection(object):
    """uniconnect supports transactions and the ability to either commit or rollback
    ...
    def __init__(self, host, port=constants.DEFAULT_PORT, user=None, source=constants.DEFAULT_SOURCE, catalog=constants.DEFAULT_CATALOG, schema=constants.DEFAULT_SCHEMA, session_properties=None, http_headers=None, http_scheme=constants.HTTP, auth=constants.DEFAULT_AUTH, redirect_handler=uniconnect.redirect.GatewayRedirectHandler(), max_attempts=constants.DEFAULT_MAX_ATTEMPTS, request_timeout=constants.DEFAULT_REQUEST_TIMEOUT, isolation_level=IsolationLevel.AUTOCOMMIT):
        self.host = host
        self.port = port
        self.user = user
        self.source = source
        ...
        self._http_session = uniconnect.client.uniconnectRequest.http.Session()
    ...
    def close(self):
        """uniconnect does not have anything to close"""
        pass
    ...
    def _create_request(self):
        return uniconnect.client.uniconnectRequest(self.host, self.port, self.user, self.source, self.catalog, self.schema, self.session_properties, self._http_session, self.http_headers, NO_TRANSACTION, self.http_scheme, self.auth, self.redirect_handler, self.max_attempts, self.request_timeout)
...
class Cursor(object):
    ...
    def description(self):
        if self._query.columns is None:
            ...
            return [ (col['name'], col['type'], None, None, None, None, None) for col in self._query.columns
                   ]
    ...
    def rowcount(self):
        ...
        uniconnect cannot reliablity determine the number of rows returned by an
    ...
    def setinputsizes(self, sizes):
        raise uniconnect.exceptions.NotSupportedError

    def setoutputsize(self, size, column):
        raise uniconnect.exceptions.NotSupportedError

    def execute(self, operation, params=None):
        self._query = uniconnect.client.uniconnectQuery(self._request, sql=operation)
    ...
    def fetchone(self):
        ...
        try:
            ...
            return
        except uniconnect.exceptions.HttpError as err:
            raise uniconnect.exceptions.OperationalError(str(err))
    ...
    def cancel(self):
        if self._query is None:
            raise uniconnect.exceptions.OperationalError('Cancel query failed; no running query')
        self._query.cancel()
        return
...
DATETIME = DBAPITypeObject('DATE', 'TIME', 'TIME WITH TIME ZONE', 'TIMESTAMP', 'TIMESTAMP WITH TIME ZONE', 'INTERVAL YEAR TO MONTH', 'INTERVAL DAY TO SECOND')
