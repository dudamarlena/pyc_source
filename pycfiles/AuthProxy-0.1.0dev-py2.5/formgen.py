...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/authproxy/lib/formgen.py
...
def setup_form(command, filename, section, vars):
    ...
    try:
        ...
        groups = {'group_uid': {'opts': list_groups, 'selected': [
                                    list_groups[0], list_groups[1]], 
                         'multiple': True, 
                         'size': 5}}
        radio = {'group_uid': list_groups}
        options = dict(password=[
         'password'], readonly_pk=False, readonly=[
         'uid'], dropdown=groups, alias={'group_uid': 'group_name'}, doc={'group_uid': 'nom du groupe'})
