...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/authproxy/lib/authkit_adapter/authenticate/open_id.py
# Compiled at: 2007-12-04 08:48:33
...
class AuthOpenIDHandler:
    ...
    def __init__(self, app, store_type, store_config, baseurl, path_signedin, template=None, session_secret=None, session_key='authkit_openid', session_middleware='beaker.session', path_verify='/verify', path_process='/process', urltouser=None, charset=None, sreg_required=None, sreg_optional=None, sreg_policyurl=None):
    ...
    def verify(self, environ, start_response):
        ...
        try:
            ...
            response = render(self.template, message='Error retrieving identity URL: %s' % cgi.escape(str(exc.why)), value=self._quoteattr(openid_url), css_class='error', action=baseurl + self.path_verify)
            start_response('200 OK', [
             (
              'Content-type', 'text/html' + self.charset),
             (
...
class OpenIDUserSetter(AuthKitUserSetter):
    ...
    def __init__(self, app, **options):
        app = AuthOpenIDHandler(app, store_type=options['store_type'], store_config=options['store_config'], baseurl=options.get('baseurl', ''), path_signedin=options['path_signedin'], path_process=options.get('path_process', '/process'), template=options['template'], urltouser=options['urltouser'], charset=options['charset'], sreg_required=options['sreg_required'], sreg_optional=options['sreg_optional'], sreg_policyurl=options['sreg_policyurl'])
        if options['session_middleware'] == 'beaker.session':
            app = SessionMiddleware(app, key=options['session_key'], secret=options['session_secret'])
        self.app = app
