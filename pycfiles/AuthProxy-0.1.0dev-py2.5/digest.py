...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/authproxy/lib/authkit_adapter/authenticate/digest.py
# Compiled at: 2005-08-14 21:53:19
...
from authproxy.lib.authkit_adapter.authenticate import AuthKitConfigError, get_template, valid_password, get_authenticate_function, strip_base, RequireEnvironKey, AuthKitUserSetter, AuthKitAuthHandler
import logging
...
def load_digest_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.digest'):
    auth_handler_params = {}
    user_setter_params = {}
    authenticate_conf = strip_base(auth_conf, 'authenticate.')
    (app, authfunc, users) = get_authenticate_function(app, authenticate_conf, prefix=prefix + 'authenticate.', format='digest')
...
def make_digest_auth_handler(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.digest'):
    (app, auth_handler_params, user_setter_params) = load_digest_config(app, auth_conf, app_conf=None, global_conf=None, prefix='authkit.digest')
    app = MultiHandler(app)
    app.add_method('digest', DigestAuthHandler, auth_handler_params['realm'], auth_handler_params['authfunc'])
    app.add_checker('digest', status_checker)
    app = DigestUserSetter(app, user_setter_params['realm'], user_setter_params['authfunc'], user_setter_params['users'])
    return app
