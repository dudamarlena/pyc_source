...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/authproxy/lib/sqlalchemy_04_driver.py
# Compiled at: 2007-11-14 12:31:44
...
def setup_model(model, metadata, **p):
    ...
    model.Role = Role
    model.groups_table = Table('groups', metadata, Column('uid', types.Integer, primary_key=True), Column('name', types.String(255), unique=True, nullable=False))
    model.roles_table = Table('roles', metadata, Column('uid', types.Integer, primary_key=True), Column('name', types.String(255), unique=True, nullable=False))
    model.users_table = Table('users', metadata, Column('uid', types.Integer, primary_key=True), Column('username', types.String(255), unique=True, nullable=False), Column('password', types.String(255), nullable=False), Column('group_uid', types.Integer, ForeignKey('groups.uid')))
    model.users_roles_table = Table('users_roles', metadata, Column('user_uid', types.Integer, ForeignKey('users.uid')), Column('role_uid', types.Integer, ForeignKey('roles.uid')))
    model.groups_mapper = mapper(Group, model.groups_table, properties={'users': relation(User)})
    model.users_mapper = mapper(User, model.users_table, properties={'roles': relation(Role, lazy=True, secondary=model.users_roles_table), 
       'group': relation(Group)})
    model.roles_mapper = mapper(Role, model.roles_table, properties={'users': relation(User, lazy=True, secondary=model.users_roles_table)})
...
class UsersFromDatabase(Users):
    ...
    def user_create(self, username, password, group=None):
        ...
        if group is None:
            ...
            new_user = self.model.User(username=username.lower(), password=self.encrypt(password), group_uid=self.session.query(self.model.Group).filter_by(name=group.lower()).one().uid)
