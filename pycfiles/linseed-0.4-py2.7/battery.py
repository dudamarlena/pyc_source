...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/linseed/battery.py
# Compiled at: 2012-02-12 04:54:41
...
class Info(object):
    ...
    def design_capacity(self):
        ...
        return Quantity(int(self.info['design capacity'][0]), self.info['design capacity'][1])

    @property
    def last_full_capacity(self):
        """The capacity of the battery the last time it was full."""
        return Quantity(int(self.info['last full capacity'][0]), self.info['last full capacity'][1])

    @staticmethod
    def _read(battid):
        filename = ('/proc/acpi/battery/BAT{0}/info').format(battid)
        try:
            return read._read(filename, parsing.paren_parse)
        except IOError:
            raise ValueError(('Unable to read info file for battery {0}: {1}').format(battid, filename))
...
class State(object):
    ...
    def remaining_capacity(self):
        ...
        return Quantity(int(self.state['remaining capacity'][0]), self.state['remaining capacity'][1])
    ...
    def _read(battid):
        ...
        try:
            ...
            raise ValueError(('Unable to read state file for battery {0}: {1}').format(battid, filename))
...
class Battery(object):
    ...
    def __str__(self):
        ...
        if self.state.charging_state == 'discharging':
            ...
            if self.state.charging_state == 'charging':
                return ('{0}%+').format(cap)
            return 'Full'
