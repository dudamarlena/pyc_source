...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/weargoggles/django-coreapi/django_coreapi/transports.py
...
def _coerce_to_error_content(node):
    ...
    if isinstance(node, (Document, Object)):
        return OrderedDict([(key, _coerce_to_error_content(value)) for key, value in node.data.items()])
    if isinstance(node, Array):
        return [_coerce_to_error_content(item) for item in node if not isinstance(item, Link)]
    return node
...
def _coerce_to_error(obj, default_title):
    ...
    if isinstance(obj, Document):
        return Error(title=obj.title or default_title, content=_coerce_to_error_content(obj))
