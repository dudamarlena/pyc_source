...
class TestAMPConnector(unittest.TestCase):
    ...
    def test_protocol(self):
        ...
        return ac.finished.addCallback(lambda _: self.assertEqual(sa.getvalue(), (b'').join(str(x).encode('ascii') for x in range(99))))
...
class TestProcessPool(unittest.TestCase):
    ...
    def test_changeChildrenReactor(self):
        ...
        def checkDefault():
            pp = pool.ProcessPool(starter=main.ProcessStarter(childReactor=FIRST,
              ...
              max=MAX)
            pp.start()
            return pp.doWork(Reactor).addCallback(self.assertEquals, {'classname': b'SelectReactor'}).addCallback(lambda _: pp.stop())
        ...
        def checkPool(_):
            pp = pool.ProcessPool(starter=main.ProcessStarter(childReactor=SECOND,
              ...
              max=MAX)
            pp.start()
            return pp.doWork(Reactor).addCallback(self.assertEquals, {'classname': b'PollReactor'}).addCallback(lambda _: pp.stop())
    ...
    def test_supplyChildArgs(self):
        ...
        def _check(result):
            return pp.doWork(Write).addCallback(self.assertEquals, {'response': b'body'})
    ...
    def test_processRestartAfterTimeout(self):
        ...
        def _work(_):
            ...
            d2.addCallback(lambda result: self.assertEqual(result, {'response': b'hello'}))
