...
# [GCC 8.4.0]
# Embedded file name: /Users/kyle/flask-cms/venv/lib/python2.7/site-packages/flask_xxl/mr_bob.py
...
def print_template_files(template=None):
    if template is None:
        ...
        if template not in get_templates():
            ...
            return
        print ('\n').join(map(str, [ remove_bob(remove_dir(MRBOB_TEMPLATE_DIR, x)) for x in get_all_files(os.path.join(MRBOB_TEMPLATE_DIR, template)) if x is not None and not x.endswith('.pyc')
                                   ]))
        return
