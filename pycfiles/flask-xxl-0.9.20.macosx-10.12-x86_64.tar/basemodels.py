...
# [GCC 8.4.0]
# Embedded file name: /Users/kyle/flask-cms/venv/lib/python2.7/site-packages/flask_xxl/basemodels.py
...
class BaseMixin(Base):
    ...
    def get_all_columns(cls, exclude=['id']):
        ...
        for col in cls.__table__.c._all_columns:
            if col.name not in exclude and not col.name.endswith('id'):
                rtn.append((col.name, _clean_name(col.name)))

        for attr in dir(cls):
            if attr not in exclude:
                if attr not in [ x[0] for x in rtn ]:
