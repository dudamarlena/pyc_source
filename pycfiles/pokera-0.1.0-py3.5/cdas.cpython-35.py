...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/cerebro/cdas.py
# Compiled at: 2018-05-23 17:03:27
# Size of source mod 2**32: 30673 bytes
from __future__ import absolute_import
import datetime, os, random
from decimal import Decimal
from cerebro._util import get_logger_and_init_null
from cerebro._thrift_api import TGetDatabasesParams, TGetTablesParams, TNetworkAddress, TPlanRequestParams, TRequestType, TExecDDLParams, TExecTaskParams, TFetchParams, TRecordFormat, TTypeId, CerebroRecordServicePlanner, RecordServiceWorker
from cerebro._thrift_util import create_socket, get_transport, TTransportException, TBinaryProtocol, PlannerClient, WorkerClient, KERBEROS_NOT_ENABLED_MSG, SOCKET_READ_ZERO
...
class CerebroContext:
    ...
    def __init__(self, application_name):
        ...
        self._CerebroContext__auth = None
        self._CerebroContext__service_name = None
        self._CerebroContext__token = None
        self._CerebroContext__host_override = None
        self._CerebroContext__name = application_name
        self._CerebroContext__configure()
    ...
    def enable_kerberos(self, service_name, host_override=None):
        ...
        self._CerebroContext__auth = 'GSSAPI'
        self._CerebroContext__service_name = service_name
        self._CerebroContext__host_override = host_override
    ...
    def enable_token_auth(self, token_str=None, token_file=None):
        ...
        if token_file:
            with open(os.path.expanduser(token_file), 'r') as (t):
                token_str = t.read()
        self._CerebroContext__auth = 'TOKEN'
        self._CerebroContext__token = token_str.strip()
        self._CerebroContext__service_name = 'cerebro'
        self._CerebroContext__host_override = None
    ...
    def connect(self, host='localhost', port=12050, timeout=None):
        ...
        """
        host, port = self._CerebroContext__random_host(host, port)
        auth_mechanism = self._CerebroContext__get_auth()
        ...
        transport = None
        try:
            transport = get_transport(sock, host, auth_mechanism, self._CerebroContext__service_name, None, None, self._CerebroContext__token, self._CerebroContext__host_override)
            transport.open()
            protocol = TBinaryProtocol(transport)
            service = _ThriftService(PlannerClient(CerebroRecordServicePlanner, protocol))
            planner = PlannerConnection(service, self)
            planner.set_application(self._CerebroContext__name)
            return planner
        except (TTransportException, IOError) as e:
            sock.close()
            if transport:
                transport.close()
            self._CerebroContext__handle_transport_exception(e)
            raise e
        except:
            sock.close()
            if transport:
    ...
    def connect_worker(self, host='localhost', port=13050, timeout=None):
        ...
        try:
            ...
            worker.set_application(self._CerebroContext__name)
            return worker
        except (TTransportException, IOError) as e:
            sock.close()
            if transport:
    ...
    def __configure(self):
        ...
        if os.path.exists(token_file):
            with open(token_file, 'r') as (t):
                self._CerebroContext__token = t.read().strip()
                self._CerebroContext__auth = 'TOKEN'
                self._CerebroContext__service_name = 'cerebro'
    ...
    def __get_auth(self):
        ...
        auth_mechanism = self._CerebroContext__auth
...
class PlannerConnection:
    __doc__ = 'A connection to a CDAS planner. '
    ...
    def _ensure_serialization_support(plan):
        if not plan.supported_result_formats or TRecordFormat.ColumnarNumPy not in plan.supported_result_formats:
            raise IOError('PyCerebro requires the server to support the ' + '`ColumnarNumPy` serialization format. Please upgrade the ' + 'server to at least 0.8.1.')
                ...
                try:
                    while 1:
                        ...
                        if fetch_result.num_records:
                            serialize_fn(schema, fetch_result.columnar_records, fetch_result.num_records, *serialize_fn_args)
...
def _deserialize_to_json(schema, columnar_records, num_records, result):
    col_names, data, _, is_nulls = _columnar_batch_to_python(schema, columnar_records, num_records)
...
def _deserialize_to_pandas(schema, columnar_records, num_records, data_frames):
    import numpy, pandas
    col_names, data, any_nulls, is_nulls = _columnar_batch_to_python(schema, columnar_records, num_records)
