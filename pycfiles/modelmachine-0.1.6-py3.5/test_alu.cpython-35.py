...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modelmachine/tests/test_alu.py
# Compiled at: 2016-01-12 04:11:02
...
class TestArithmeticLogicUnit:
    __doc__ = 'Test case for arithmetic logic unit.'
    registers = None
    alu = None
    ...
    def setup(self):
        ...
        register_names = {'R1': 'R1', 'R2': 'R2', 'FLAGS': 'FLAGS', 
         'S': 'S', 'RES': 'R1', 
         'ADDR': 'ADDR', 'PC': 'PC'}
        self.alu = ArithmeticLogicUnit(self.registers, register_names, BYTE_SIZE, WORD_SIZE)
                        ...
                        if 3 * self.min_int < i < 2 * self.min_int:
                            ...
                            if i == 2 * self.min_int:
                                ...
                                if 2 * self.min_int < i < self.min_int:
                                    assert self.registers.fetch('FLAGS', BYTE_SIZE) == OF | CF
                                    if self.min_int <= i < 0:
                                        if not self.registers.fetch('FLAGS', BYTE_SIZE) == SF | CF:
                        ...
                        if i == 2 * self.max_int:
                            ...
                            if 2 * self.max_int < i < 3 * self.max_int:
                                ...
                                if 3 * self.max_int <= i < 4 * self.max_int:
                                    if not self.registers.fetch('FLAGS', BYTE_SIZE) == OF | CF | SF:
                                        raise AssertionError
    ...
    def test_get_ops(self):
        ...
        for i in range(self.min_int, self.max_int):
            ...
            if not self.alu.get_unsigned_ops() == unsigned:
                raise AssertionError
    ...
    def test_add(self):
        ...
        for i in range(1, self.max_int):
            ...
            if i < self.max_int - 10:
                if not self.registers.fetch('FLAGS', BYTE_SIZE) == 0:
                    raise AssertionError
            else:
                assert self.registers.fetch('FLAGS', BYTE_SIZE) == OF | SF
                self.registers.put('R1', -i % 2 ** BYTE_SIZE, BYTE_SIZE)
                ...
                self.alu.add()
                if not self.registers.fetch('S', BYTE_SIZE) == (-i - 10) % 2 ** BYTE_SIZE:
                    raise AssertionError
            if -i >= self.min_int + 10:
                if not self.registers.fetch('FLAGS', BYTE_SIZE) == SF | CF:
                    raise AssertionError
            elif not self.registers.fetch('FLAGS', BYTE_SIZE) == OF | CF:
                raise AssertionError
    ...
    def test_sub(self):
        ...
        for i in range(1, self.max_int):
            ...
            if i < 10:
                if not self.registers.fetch('FLAGS', BYTE_SIZE) == CF | SF:
                    raise AssertionError
            else:
                if i == 10:
                    if not self.registers.fetch('FLAGS', BYTE_SIZE) == ZF:
                        raise AssertionError
                elif not self.registers.fetch('FLAGS', BYTE_SIZE) == 0:
                    raise AssertionError
                self.registers.put('R1', -i % 2 ** BYTE_SIZE, BYTE_SIZE)
                self.registers.put('R2', 10, BYTE_SIZE)
                self.alu.sub()
                if not self.registers.fetch('S', BYTE_SIZE) == (-i - 10) % 2 ** BYTE_SIZE:
                    raise AssertionError
            if -i >= self.min_int + 10:
                if not self.registers.fetch('FLAGS', BYTE_SIZE) == SF:
                    raise AssertionError
            elif not self.registers.fetch('FLAGS', BYTE_SIZE) == OF:
                raise AssertionError
    ...
    def test_mul(self):
        ...
        self.alu.umul()
        assert self.registers.fetch('S', BYTE_SIZE) == 375 % 2 ** BYTE_SIZE
        assert self.registers.fetch('FLAGS', BYTE_SIZE) == OF | CF
    ...
    def run_cond_jump(self, should_jump, first, second, *vargs, **kvargs):
        ...
        if should_jump:
            if not self.registers.fetch('PC', WORD_SIZE) == 2:
                raise AssertionError
        elif not self.registers.fetch('PC', WORD_SIZE) == 1:
            raise AssertionError
