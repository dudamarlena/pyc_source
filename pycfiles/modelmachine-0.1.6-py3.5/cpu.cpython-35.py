...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modelmachine/cpu.py
...
class AbstractCPU:
    __doc__ = 'CPU must have methods: load_program, print_result and run_fie.'
    ram = None
    registers = None
    register_names = None
    alu = None
    ...
    def load_program(self, program, input_function=input):
        ...
        def get_section_index(section):
            ...
            if section not in program:
                raise ValueError('Cannot find section {section}'.format(section=section))
        ...
        if not config_start < code_start < input_start:
            raise ValueError('Wrong section order, should be: config, code, input')
        ...
        for line in config_list:
            if line == '':
                ...
                if len(line) != 2:
                    raise ValueError('Wrong config format: `{line}`'.format(line='='.join(line)))
...
class CPUMM3(AbstractCPU):
    ...
    def __init__(self, protect_memory):
        ...
        self.ram = RandomAccessMemory(word_size=word_size, memory_size=memory_size, endianess='big', is_protected=protect_memory)
        self.registers = RegisterMemory()
        self.register_names = BCU3.register_names
        self.alu = ArithmeticLogicUnit(registers=self.registers, register_names=self.register_names, operand_size=word_size, address_size=address_size)
        self.control_unit = BCU3(ir_size=word_size, registers=self.registers, ram=self.ram, alu=self.alu, operand_size=word_size, address_size=address_size)
        self.io_unit = InputOutputUnit(ram=self.ram, start_address=0, word_size=word_size)


class CPUMM2(AbstractCPU):
    __doc__ = 'CPU model machine 2.'
    ...
    def __init__(self, protect_memory):
        ...
        self.alu = ArithmeticLogicUnit(registers=self.registers, register_names=self.register_names, operand_size=word_size, address_size=address_size)
        self.control_unit = BCU2(ir_size=word_size, registers=self.registers, ram=self.ram, alu=self.alu, operand_size=word_size, address_size=address_size)
        self.io_unit = InputOutputUnit(ram=self.ram, start_address=0, word_size=word_size)


class CPUMMV(AbstractCPU):
    __doc__ = 'CPU variable model machine.'
    ...
    def __init__(self, protect_memory):
        ...
        memory_size = 2 ** address_size
        self.ram = RandomAccessMemory(word_size=byte_size, memory_size=memory_size, endianess='big', is_protected=protect_memory)
        self.registers = RegisterMemory()
        self.register_names = BCUV.register_names
        self.alu = ArithmeticLogicUnit(registers=self.registers, register_names=self.register_names, operand_size=word_size, address_size=address_size)
        self.control_unit = BCUV(ir_size=word_size, registers=self.registers, ram=self.ram, alu=self.alu, operand_size=word_size, address_size=address_size)
        self.io_unit = InputOutputUnit(ram=self.ram, start_address=0, word_size=word_size)


class CPUMM1(AbstractCPU):
    __doc__ = 'CPU model machine 1.'
    ...
    def __init__(self, protect_memory):
        ...
        self.alu = ArithmeticLogicUnit(registers=self.registers, register_names=self.register_names, operand_size=word_size, address_size=address_size)
        self.control_unit = BCU1(ir_size=word_size, registers=self.registers, ram=self.ram, alu=self.alu, operand_size=word_size, address_size=address_size)
        self.io_unit = InputOutputUnit(ram=self.ram, start_address=0, word_size=word_size)


class CPUMMM(AbstractCPU):
    __doc__ = 'CPU address modification model machine.'
    ...
    def __init__(self, protect_memory):
        ...
        self.alu = ArithmeticLogicUnit(registers=self.registers, register_names=self.register_names, operand_size=operand_size, address_size=address_size)
        self.control_unit = BCUM(ir_size=ir_size, registers=self.registers, ram=self.ram, alu=self.alu, operand_size=operand_size, address_size=address_size)
        self.io_unit = InputOutputUnit(ram=self.ram, start_address=0, word_size=operand_size)
