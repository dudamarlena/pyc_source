...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modelmachine/io.py
...
class InputOutputUnit:
    __doc__ = 'Allow to input and output program and data.'
    ...
    def store_hex(self, start, size):
        ...
        if size % self.ram.word_size != 0:
            raise KeyError('Cannot save {size} bits, word size is {word_size}'.format(size=size, word_size=self.ram.word_size))
        result = []
