...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modelmachine/memory.py
# Compiled at: 2016-03-01 18:44:57
# Size of source mod 2**32: 8049 bytes
...
def little_endian_encode(value, word_size, bits):
    ...
    size = bits // word_size
    if value < 0:
        raise ValueError('Cannot encode negative value: {value}'.format(value=value))
    if value == 0:
        return [0] * size
    else:
        result = []
        ...
        if len(result) * word_size > bits:
            raise ValueError('Integer is too long: {value}, expected {bits} bits integer'.format(value=copy_value, bits=bits))
...
class AbstractMemory(dict):
    __doc__ = 'Class from which inherits concrete memory.'
    word_size = None
    ...
    def __init__(self, word_size, endianess='big', addresses=None):
        ...
        if endianess == 'big':
            ...
            if endianess == 'little':
                ...
                raise ValueError('Unexpected endianess: {endianess}'.format(endianess=endianess))

    def check_word_size(self, word):
        """Check that value can be represented by word with the size."""
        if not 0 <= word < 2 ** self.word_size:
            raise ValueError('Wrong word format: {word}, should be 0 <= word < {max_value}'.format(word=word, max_value=2 ** self.word_size))
    ...
    def check_bits_count(self, address, bits):
        ...
        if bits % self.word_size != 0:
            raise KeyError('Cannot operate with non-integer word counter: needs {bits} bits, but word size is {word_size}'.format(bits=bits, word_size=self.word_size))
    ...
    def fetch(self, address, bits):
        ...
        if size == 1:
            ...
            return self.decode([self[i] for i in range(address, address + size)], self.word_size)
...
class RandomAccessMemory(AbstractMemory):
    __doc__ = 'Random access memory.\n\n    Addresses is x: 0 <= x < memory_size.\n    If is_protected == True, you cannot read unassigned memory\n    (usefull for debug).\n    '

    def __init__(self, word_size, memory_size, endianess, is_protected=True, *vargs, **kvargs):
        """Read help(type(x))."""
    ...
    def check_address(self, address):
        ...
        if not isinstance(address, int):
            raise TypeError('Address should be int, not {type}'.format(type=str(type(address))))
        if not 0 <= address < len(self):
            raise KeyError('Invalid address {address}, should be 0 <= address < {memory_size}'.format(address=hex(address), memory_size=len(self)))

    def __missing__(self, address):
        """If addressed memory not defined."""
        self.check_address(address)
        if self.is_protected:
            raise KeyError('Cannot read memory by address: {address}, it is dirty memory, clean it first'.format(address=hex(address)))
        else:
            warnings.warn('Read memory by address: {address}, it is dirty memory, clean it first'.format(address=hex(address)), stacklevel=4)
            return 0
...
class RegisterMemory(AbstractMemory):
    ...
    def add_register(self, name, register_size):
        ...
        if name in self.register_sizes and self.register_sizes[name] != register_size:
            raise KeyError('Cannot add register with name `{name}` and size `{register_size}`, register with this name and `{exist_size}` size already exists'.format(name=name, register_size=register_size, exist_size=self.register_sizes[name]))
    ...
    def check_address(self, name):
        ...
        if name not in self.register_sizes:
            raise KeyError('Invalid register name: {name}'.format(name=name))

    def check_bits_count(self, name, size):
        """Bit count must be equal to word_size."""
        if size != self.register_sizes[name]:
            raise KeyError('Invalid register {name} size: {size}. Should be {ex_size}'.format(name=name, size=size, ex_size=self.register_sizes[name]))
        else:
            self.word_size = size