...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/modelmachine/tests/test_memory.py
...
def test_endianess():
    """Simple test."""
    assert big_endian_decode([1, 2, 3], 8) == 66051
    assert little_endian_decode([1, 2, 3], 8) == 197121
    assert big_endian_encode(66051, 8, 24) == [1, 2, 3]
    assert little_endian_encode(197121, 8, 24) == [1, 2, 3]
    assert big_endian_encode(0, 8, 24) == [0, 0, 0]
    assert little_endian_encode(0, 8, 24) == [0, 0, 0]


class TestAbstractMemory:
    __doc__ = 'Test case for abstract memory class.'
    memory = None
...
class TestRandomAccessMemory:
    ...
    def test_setitem(self):
        ...
        for i in range(len(self.ram)):
            ...
            if not i in self.ram:
                raise AssertionError

        for i in range(len(self.ram), 2 * len(self.ram)):
            with raises(KeyError):
                self.ram[i] = i
            if not i not in self.ram:
                raise AssertionError
    ...
    def test_fetch(self):
        ...
        for i in range(5, 9):
            self.ram[i] = i
            if not self.ram.fetch(i, WORD_SIZE) == i:
                raise AssertionError

        assert self.ram.fetch(5, 4 * WORD_SIZE) == 396140812682002152440041832456
        assert self.ram.fetch(5, 4 * WORD_SIZE) == big_endian_decode([5, 6, 7, 8], WORD_SIZE)
        self.ram[5] = 0
        assert self.ram.fetch(5, 4 * WORD_SIZE) == big_endian_decode([0, 6, 7, 8], WORD_SIZE)
        assert self.ram.fetch(5, 4 * WORD_SIZE) == 110680464472322080776
        with raises(KeyError):
        ...
        assert self.ram.fetch(5, 4 * WORD_SIZE) == 633825300243241909290088267781
        assert self.ram.fetch(5, 4 * WORD_SIZE) == little_endian_decode([5, 6, 7, 8], WORD_SIZE)
    ...
    def test_put(self):
        ...
        for i in range(5, 9):
            if not self.ram[i] == value_list[(i - 5)]:
                raise AssertionError
...
class TestRegisterMemory:
    __doc__ = 'Test case for RegisterMemory.'
    registers = None
