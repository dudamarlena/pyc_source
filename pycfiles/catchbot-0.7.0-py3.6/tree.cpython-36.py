...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/catchbot/hook/tree.py
# Compiled at: 2018-06-23 13:58:03
# Size of source mod 2**32: 2096 bytes
from pkg_resources import resource_stream
import yaml, catchbot
_HOOK = 'hook'
_TYPES = 'types'
    ...
    if isinstance(hook_tree, list):
        ...
        if isinstance(hook_tree, str):
            ...
            if all_dict_children:
                return
            all_list_or_str_child = all(isinstance(value, list) or isinstance(value, str) for value in hook_tree.values())
            msg = all_list_or_str_child or 'All children must be either of {} type or any of ({}, {}) types'.format(dict, str, list)
            raise BadHookTree(msg)
        keys = set(hook_tree.keys())
        is_types_subset = set(types) & keys == keys
        msg = is_types_subset or 'Types restriction is not satisfied, got: {}'.format(keys)
        raise BadHookTree(msg)
