...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimir/aggregator/writer.py
...
class AtomWriter(object):
    ...
    def _generate_contributor(self, data):
        return [ self.generate_person('contributor', item) for item in data
               ]
    ...
    def _generate_feedburner_origlink(self, data):
        element = domish.Element(('http://rssnamespace.org/feedburner/ext/1.0', 'origLink'))
        element.addContent(data)
        return element
