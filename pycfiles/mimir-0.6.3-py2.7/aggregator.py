...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimir/aggregator/aggregator.py
# Compiled at: 2012-05-10 09:31:40
"""
Feed Aggregator.
...
class FileFeedStorage(object):
    ...
    def _readFeedList(self):
        ...
        for handle, url in feedList:
            self.feeds[handle] = {'handle': handle, 'href': url}

    def _writeFeedList(self):
        """
        Write out list of feeds
        """
        feedList = [ '%s %s\n' % (f['handle'], f['href']) for f in self.feeds.itervalues()
                   ]
        feedList.sort()
...
class AggregatorService(service.Service):
    ...
    def aggregate(self, handle, useCache=1):
        ...
        def aggregateFeed(cachedFeed):
            ...
            d = defer.maybeDeferred(fetcher.getFeed, str(cachedFeed['href']), agent=self.agent, headers=headers, useCache=useCache)
    ...
    def findFreshEntries(self, result, handle, cachedFeed):
        ...
        if discoveredEntries:
            d = self.handler.entriesDiscovered(handle, result, discoveredEntries)
        else:
...
components.registerAdapter(XMPPControl, IAggregatorService, IXMPPHandler)
...
class AddFeedResource(resource.Resource):
    ...
    def http_POST(self, request):
        ...
        def trapInvalidHandle(failure):
            failure.trap(InvalidHandleError)
            return http.StatusResponse(responsecode.BAD_REQUEST, 'Invalid handle')
...
components.registerAdapter(AtomPublisher, pubsub.IPubSubClient, IFeedHandler)