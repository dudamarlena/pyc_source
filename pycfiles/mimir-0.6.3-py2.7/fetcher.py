...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mimir/aggregator/fetcher.py
...
feeds = [
 'http://test.ralphm.net/blog/atom']
...
class HTTPClientFeedFactory(client.HTTPClientFactory):
    ...
    def __init__(self, url, method='GET', postdata=None, headers=None, agent='Twisted PageGetter', timeout=0, cookies=None, followRedirect=1, useCache=1):
        ...
        client.HTTPClientFactory.__init__(self, url=url, method=method, postdata=postdata, headers=headers, agent=agent, timeout=timeout, cookies=cookies, followRedirect=followRedirect)
        return

    def page(self, page):
        if self.waiting:
            self.waiting = 0
            resource = FeedResource(page, self.url, self.real_status or self.status, self.response_headers)
