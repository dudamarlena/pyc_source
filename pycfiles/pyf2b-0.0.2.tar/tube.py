...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/controllers/tube.py
# Compiled at: 2011-02-17 10:57:48
__doc__ = 'Crud Tube Controllers'
...
from pyf.services.controllers.crud import SecureCrudRestController, DataGrid, render_boolean, render_link_field, has_model_permission, itemize
from pyf.services.controllers.crud import FancyTableFiller as TableFiller
from pyf.services.controllers.tubelayer import tubelayer_table, tubelayer_table_filler
...
import simplejson, transaction, time, tw.forms
...
class TubeController(SecureCrudRestController):
    ...
    class table_filler_type(TableFiller):
        ...
        def layers(self, object):
            ...
            for layer in object.layers:
                if layer.variant_name not in variant_names:
    ...
    @expose()
    @expose('json')
    @validate(validators={'tube_id': validators.Int(not_none=True), 
    ...
    def launch(self, tube_id, variant=None, options=None, packets=None, **kwargs):
        ...
        if variant == 'no_variant':
            variant = None
        event_track_id = launch_tube(tube_id, user_name=user_name, variant=variant, defered=True, options=options, source=packets)
        time.sleep(1)
    ...
    def graph(self, tube_id):
        tube = DBSession.query(Tube).get(tube_id)
        tube_dict = dict(name=tube.display_name, id='tube_%s' % tube.id, data=dict(type='tube'), children=list())
        variant_names = tube.variant_names
        tube_dict['children'].append(dict(name='default', id='var_default', data=dict(type='variant'), children=[
         tube.get_graph()]))
        for name in variant_names:
            tube_dict['children'].append(dict(name=name, id='var_%s' % name, data=dict(type='variant'), children=[
             tube.get_graph(variant=name)]))
    ...
    def get_graph(self, tube_id):
        try:
            tube = DBSession.query(Tube).get(tube_id)
            infos = tube.get_network_graph(full_tree=False, all_infos=True, compute_position=False)
            ...
            for node in nodes:
                ...
                value_info = dict(code=node.get('code'), content=node.get('content'), joiner=dict(joiner_type=node.get('joiner', 'linear'), content=node.get('joiner_content', '')))
                if node.get('source', 'plugin') == 'plugin':
                    ...
                    if getattr(module_class, 'configuration', None) is not None:
                        ...
                        for configuration_key in module_class.configuration:
                            if hasattr(configuration_key, 'gets_node_list') and configuration_key.gets_node_list:
                    ...
                    value_info['config_keys'] = [ [key, value] for (key, value) in node.get('config_keys', dict()).iteritems()
                                                ]
                value_info.pop('name', None)
                modules.append(dict(name=module_type, value=dict(name=node.get('name'), plugin=node.get('pluginname'), **value_info), config=dict(position=[
                 node.get('x', 0),
                 node.get('y', 0)])))
                name_to_module_id[node.get('name')] = item_id

            for (parent, child) in links:
                wires.append(dict(src=dict(moduleId=name_to_module_id[parent], terminal='out'), tgt=dict(moduleId=name_to_module_id[child], terminal='in')))

            return dict(status='success', modules=modules, wires=wires)
        except Exception, e:
            log.exception(e)
            return dict(status='error', reason=unicode(e))
        ...
        try:
            ...
            for module in payload['modules']:
                if module.get('name') == 'comment':
                    el = etree.SubElement(process, 'comment', position=(',').join(map(str, module.get('config').get('position'))))
                ...
                el = etree.SubElement(process, 'node', **{'from': module_source, 'type': module_type, 
                ...
                if module_values.get('joiner'):
                    ...
                    if joiner_info.get('content'):
                        try:
                            ...
                            return dict(status='error', reason='bad formed xml in joiner node of %s' % module_values.get('name'))
                ...
                if is_gui_plugin:
                    ...
                    if module_values.get('content'):
                        try:
                            ...
                            return dict(status='error', reason='bad formed xml in node %s' % module_values.get('name'))
            ...
            elif tube_id is None:
                return dict(status='error', reason='Trying to create a tube without specifying properties')
            DBSession.add(tube)
            DBSession.flush()
            get_repository().store_item(tube, user=request.identity['user'].user_name)
            return dict(status='success', tube_id=Tube.by_name(tube.name).id)
        except Exception, e:
            log.exception(e)
            return dict(status='error', reason=unicode(e))
    ...
    def language_definition(self, jsonp=''):
        ...
        terminals_definition = [
         {'name': 'out', 
            'direction': [
                        0, 1], 
            'offsetPosition': {'left': 86, 
                               'bottom': -15}, 
            'ddConfig': {'type': 'output', 
                         'allowedTypes': [
                                        'input']}, 
            'editable': True},
         {'name': 'in', 
        ...
        for (plugin_type, plugin_list) in plugins.iteritems():
            ...
            for (plugin_name, plugin) in plugin_list:
                if hasattr(plugin, 'configuration') and plugin.configuration is not None:
                    fields = []
                    if plugin_type != 'producers':
                        fields.insert(0, {'type': 'group', 
                           'inputParams': {'name': 'joiner', 
                                           'legend': 'Joiner Info', 
                                           'collapsible': True, 
                                           'collapsed': True, 
                                           'fields': [
                                                    {'type': 'select', 
                                                       'inputParams': {'label': 'Type', 
                                                                       'name': 'joiner_type', 
                                                                       'value': 'linear', 
                                                                       'selectValues': joiner_plugin_names}},
                                                    {'type': 'text', 
                                                       'inputParams': {'name': 'content', 
                                                                       'label': 'Content', 
                                                                       'value': ''}}]}})
                    module_definition = dict(name='%s.%s.%s' % (resource_manager.main_namespace, plugin_type, plugin_name), container=dict(terminals=terminals, xtype='WireIt.FormContainer', title='Adapter', fields=fields, collapsible=False, display_name=getattr(plugin, 'display_name', plugin.__name__)))
