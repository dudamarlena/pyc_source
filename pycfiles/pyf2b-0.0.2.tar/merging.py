...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/dataflow/merging.py
# Compiled at: 2011-02-23 05:58:06
...
def merge_iterators(iterators, key=None):
    ...
    merged = merge(*[ prepare_merging(iterator, i) for (i, iterator) in enumerate(iterators)
                    ])
    return imap(untangle, imap(itemgetter(1), groupby(merged, key=itemgetter(0))))