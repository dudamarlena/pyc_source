...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/transport/packets.py
# Compiled at: 2011-01-05 05:28:01
from simplejson.encoder import JSONEncoder
from simplejson import loads
import operator, datetime, decimal
from itertools import imap, izip
from pyf.splitter.inputsplitter import input_item_separator, tokenize, EndOfFileError
import re
...
class Packet(object):
    ...
    def prepare_data_dict(self, data):
        ...
        for (key, value) in data.iteritems():
            ...
            data_dict[key] = dict(value=value, metadata=metas)
    ...
    def __getattr__(self, attribute):
        if attribute not in self.__data:
    ...
    def unserialize_value(self, value, metadata):
        ...
        if metatype == 'int' or metatype == 'string':
            ...
            value = [ self.unserialize_value(val.get('value'), val.get('metadata')) for val in value
                    ]
        return value
...
def encode_packet_flow(object_flow, separator=input_item_separator, iterencode=False, compress=False):
    ...
    for obj in object_flow:
        if iterencode:
            if compress:
                raise NotImplementedError, "Can't use iterencode with compress in encode_packet_flow"
...
def decode_packet_flow(source, buffersize=16384, separator=input_item_separator, decompress=False, pure_flow=False):
