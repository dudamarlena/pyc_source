...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/controllers/dispatch.py
# Compiled at: 2010-12-14 10:51:23
__doc__ = 'Crud Dispatch Controllers'
...
from pyf.services.controllers.crud import SecureCrudRestController, DataGrid, render_boolean, render_link, render_link_field, has_model_permission
from pyf.services.controllers.crud import FancyTableFiller as TableFiller
from pyf.services.controllers.utils import tube_select_getter, descriptor_select_getter
...
import time, transaction, operator, simplejson
from pylons.controllers.util import abort

def get_variants(tube):
    variants = list()
    for layer in tube.layers:
        if layer.variant_name not in variants:
...
class DispatchController(SecureCrudRestController):
    ...
    class new_form_type(AddRecordForm):
        ...
        tube = SingleSelectField('tube', label_text='Tube', options=tube_select_getter(needs_source=True), not_empty=True)
        descriptor = SingleSelectField('descriptor', label_text='Descriptor', options=descriptor_select_getter, not_empty=True)
    ...
    class table_filler_type(TableFiller):
        ...
        descriptor = render_link_field('/descriptors/%s/edit', 'descriptor', 'display_name', is_list=False)
    ...
    def get_one(self, dispatch_id):
        ...
        return dict(dispatch=dispatch, variants=get_variants(dispatch.tube), launcher=DispatchLauncher(dispatch.id))
    ...
    @expose()
    @expose('json')
    @validate(validators={'doc': validators.FieldStorageUploadConverter(), 
    ...
    def upload(self, dispatch_id, doc, variant=None, encoding=None, options=None, **kwargs):
        ...
        event_track = create_event_track(dispatch.tube, storage_file_uuid=uuid, source_filename=doc.filename, encoding=used_encoding, dispatch=dispatch, variant_name=used_variant)
        ...
        logger.info('Process started using dispatch %s (variant %s) by user %s (%s)' % (
         ...
         request.environ['REMOTE_ADDR']), user=user, source=user.user_name)
        flash('Created event track %s' % event_track_id)
        transaction.commit()
        add_single_task(router.process_using_dispatch, args=[
         doc.file], kw=dict(variant=used_variant, encoding=used_encoding, options=options), initialdelay=0)
        time.sleep(1)
        if request.response_type == 'application/json':
