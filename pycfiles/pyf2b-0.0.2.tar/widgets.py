...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/controllers/widgets.py
...
from tw.forms import InputField, SingleSelectField, MultipleSelectField, TableForm, HiddenField, TableFieldSet, RadioButtonList, CalendarDatePicker, FileField, CheckBox
...
class Launcher(object):
    ...
    def get_widgets(self, module):
        ...
        fieldsets = list()
        result = list()
        for info in sorted(self.tube.get_parameters().values(), key=itemgetter('order')):
            if info.has_key('group'):
                ...
                if not fieldsets_registry.has_key(fieldset):
                    ...
                    if info.has_key('group_description'):
                        ...
                        fieldset_help = ''
                    fieldsets_registry[fieldset] = TableFieldSet(fieldset, label_text=fieldset_label, legend='', help_text=fieldset_help)
        ...
        if self.variant_names:
            result.insert(0, SingleSelectField('variant', options=[
             ('no_variant', '--')] + self.variant_names))
    ...
    def get_default_widget_info(self, widget_info):
        return dict(label_text=widget_info.get('name'), help_text=widget_info.get('description'), default=widget_info.get('default'))

    def get_input_widget(self, widget_info, widget=None):
        if widget is None:
            widget = InputField
        return widget(widget_info.get('key'), **self.get_default_widget_info(widget_info))

    def get_input_range_widget(self, widget_info, widget=None):
        if widget is None:
            widget = InputField
        from_widget = widget(widget_info.get('from_key'), label_text=widget_info.get('from_label', 'From'), default=widget_info.get('from_default'), attrs={'name': widget_info.get('from_key')})
        to_widget = widget(widget_info.get('to_key'), label_text=widget_info.get('to_label', 'To'), default=widget_info.get('to_default'), attrs={'name': widget_info.get('to_key')})
        return TableFieldSet(widget_info.get('key'), label_text=widget_info.get('name'), legend='', help_text=widget_info.get('description'), children=[
         from_widget, to_widget])

    def get_date_widget(self, widget_info):
        widget_callable = create_function(CalendarDatePicker, caller_args_count=1, kwargs=dict(date_format=widget_info.get('format')))
        return self.get_input_widget(widget_info, widget=widget_callable)

    def get_date_range_widget(self, widget_info):
        widget_callable = create_function(CalendarDatePicker, caller_args_count=1, kwargs=dict(date_format=widget_info.get('format')))
        return self.get_input_range_widget(widget_info, widget=widget_callable)
    ...
    def get_radio_widget(self, widget_info, widget=None):
        ...
        return widget(widget_info.get('key'), options=options, **self.get_default_widget_info(widget_info))
...
class DispatchLauncher(Launcher):
    ...
    def get_widgets(self, module):
        ...
        encoding_info = dict(key='encoding', name='Encoding:', description='The source file encoding', default=self.dispatch.descriptor.default_encoding)
        source_file_info = dict(key='doc', name='File:', description='')
        widgets.append(self.get_input_widget(encoding_info))
        widgets.append(self.get_input_widget(source_file_info, widget=FileField))
        return widgets

    def get_form(self):
        return TableForm('launch_dispatch_%s' % self.dispatch.name, action=tg.url('/dispatchs/upload'), children=self.get_widgets(self.module))
...
class TubeLauncher(Launcher):
    ...
    def get_form(self):
        return TableForm('launch_tube_%s' % self.tube.name, action=tg.url('/tubes/launch'), children=self.get_widgets(self.module))