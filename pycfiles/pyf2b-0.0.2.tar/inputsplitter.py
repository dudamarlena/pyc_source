...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/splitter/inputsplitter.py
# Compiled at: 2015-10-09 17:48:47
__doc__ = ' Low-level item flow splitting, used to save on hard disk data flows in\nseparate reusable bucket_filenames.\n'
...
if six.PY2:
    ...
    input_item_separator = '\x00\x017B\x00'
...
def tokenize(buf, separator):
    ...
    remainer = items[(-1)]
    return (
     return_items, remainer)


def get_input_item_str_flow(source_file, separator=input_item_separator, chunk_size=32768):
    ...
    if buf:
        raise EndOfFileError('Bad file content: EOF reached but buffer not empty')
...
def get_input_item_flow(source_file, separator=input_item_separator, chunk_size=32768):
    ...
    The open file must be closed by the caller.
    """
    for line in get_input_item_str_flow(source_file, separator=separator, chunk_size=chunk_size):
...
def get_splitter(input_flow=None, max_items=200, split_attribute=None, force_split=False):
    """Function to instanciate the adequate splitter, whether you want to:

    * just split the `input_flow` of items into bucket_filenames of a
     `max_items` maximum size with the `InputSplitterSingle`
    * use the `InputSplitterByAttribute` to guarantee that two items from the
      ...
      **will not** be separated into different bucket_filenames. You might
      optionally want to guarantee that two items with a different value for
       their `split_node` attribute **will** be separated.
    """
    if split_attribute:
        splitter = InputSplitterByAttribute(input_flow, max_items, split_attribute, force_split)
    else:
...
class InputSplitterSingle(object):
    ...
    def push(self, input_item):
        """ Permits to push a single item in the splitter,
        returning the corresponding bucket
        """
        self.current_bucket.write(pickle.dumps(input_item, protocol=protocol) + input_item_separator)
        ...
        if not self.max_items == 0 and self.current_bucket_size >= self.max_items:
            self.bucket_filenames.append(self.current_bucket.name)
            self.current_bucket.close()
            self.current_bucket = self.get_new_bucket()
        return bucket_filename

    def split(self):
        """Launch the splitting and finalize it,
        returning the bucket_filenames
    ...
    def finalize(self):
        """Finalizes the splitter, and returns the bucket_filenames
        this is called by the finalize method. You should not need to call
        this method yourself instead in corner cases and tests
        """
        self.current_bucket.close()
...
class InputSplitterByAttribute(object):
    ...
    :param split_attribute: The attribute name for which equal values
     will not be separated
    :type split_attribute: String

    :param force_split: Do not group items with different `split_attribute`
     values if `True`
    ...
    def __choose_bucket(self, line_number):
        ...
        if not self.max_items == 0 and ref_value_count > self.max_items:
            msg = 'Too many reference values (%s) found for %s,' % (
             ref_value_count, reference_value)
    ...
    def __store_input_item(self, input_item_str, line_number):
        ...
        """
        reference_value = self.line_reference[line_number]
        if reference_value in self.bucket_reference_value:
            bucket_filename = self.bucket_reference_value[reference_value]
        else:
            bucket_filename = self.__choose_bucket(line_number)
    ...
    def __get_reference_value(self, input_item):
        ...
        if not hasattr(input_item, self.split_attribute):
            raise InputSplitterError('Input item does not have a %s attribute' % self.split_attribute)
    ...
    def __save_reference(self, reference_value, linenumber):
        ...
        """
        if reference_value not in self.reference_size:
    ...
    def __prepare(self, input_flow):
        ...
        for line_number, input_item in enumerate(input_flow):
            ...
            self.store_input_item(file_data, pickle.dumps(input_item, protocol=protocol))
    ...
    @staticmethod
    def store_input_item(source_file, input_item_bin, separator=input_item_separator):
        source_file.write(input_item_bin + separator)

    def split(self):
        """ Launches the splitting,
         returning bucket filenames in an array.
        """
