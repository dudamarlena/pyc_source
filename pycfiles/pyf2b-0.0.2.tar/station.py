...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/core/station.py
# Compiled at: 2010-07-19 12:04:24
...
def dispatcher(flow, client=None):
    ...
    try:
        try:
            ...
            (timestamp, userid, tokens, user_data) = auth_tkt.parse_ticket(config.sa_auth.get('cookie_secret', 'secret'), header.authtkt, '0.0.0.0')
            user = get_user_from_id(userid)
            permissions = map(operator.attrgetter('permission_name'), user.permissions)

            def check_permission(perm):
                assert perm in permissions or 'manage' in permissions, 'Not authorized'
            ...
            if header.action == 'launch_tube':
                ...
                if 'object_id' in header:
                    ...
                    raise Exception, 'No info about which tube to launch. Please provide either object_id or object_name.'
                ...
                (event_track_id, router, kwargs) = launch_tube(tube.id, user.user_name, variant=header.get('variant'), options=options, source=flow, only_prepare=True)
                client.message(Packet(dict(type='appinfo', key='event_track_id', value=event_track_id)))
                router.add_listener('failure', client.error)
                router.add_listener('info', lambda msg, **kw: client.message(Packet(dict(type='info', message=msg, **kw))))
                router.add_listener('success', lambda *a, **kw: client.success('Event %s finished successfully' % event_track_id))
...
def init_station():
    ...
    scheduler.add_single_task(reactor.run, kw=dict(installSignalHandlers=0), initialdelay=0)