...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/controllers/root.py
...
from pyf.services.controllers.user import UserController, GroupController, PermissionController
...
class RootController(BaseController):
    ...
    def index(self):
        ...
        return dict(standalone_tubes=Tube.get_tubes(needs_source=False), sourced_tubes=Tube.get_tubes(needs_source=True))
    ...
    def plugins(self):
        ...
        modules = list()
        language = dict(languageName='ComPyf', modules=modules)
        for (plugin_type, plugin_list) in plugins.iteritems():
            for (plugin_name, plugin) in plugin_list:
                modules.append(dict(name=plugin_name, inputs=plugin_type != 'producers' and ['input'] or list(), outputs=plugin_type != 'consumers' and ['output'] or list()))
