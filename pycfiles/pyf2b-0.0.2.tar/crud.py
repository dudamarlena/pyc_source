...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/services/controllers/crud.py
# Compiled at: 2011-05-09 04:25:40
__doc__ = 'Crud Controllers'
from xml.sax.saxutils import escape
from tg import expose, url, redirect, request, response
from tg.decorators import paginate, with_trailing_slash, Decoration, without_trailing_slash
...
class SecureCrudRestController(RestController):
    ...
    def get_all(self, *args, **kw):
        ...
        if pylons.request.response_type == 'application/json':
            ...
            return dict(model=self.model.__name__, value_list=query, line_formater=self.table_filler.get_value_item, kw=kw)
    ...
    def docs(self):
        return dict(title=self.model.__name__, description=publish_parts(trim(self.model.__doc__), writer_name='html')['html_body'], contents=(key.startswith('_') or (key, publish_parts(trim(value.__doc__), writer_name='html')['html_body']) for (key, value) in self.model.__dict__.iteritems()))
...
def render_boolean(attr):
    ...
    def render_func(controller, obj):
        ...
        return '<img src="/images/silk/%(img)s" alt="%(value)s" />' % {'img': img, 
           'value': value}
...
def render_link_field(base_url, property, order_by, is_list=True, url_attr='id', display_name_property='display_name'):

    def render_func(controller, obj):
        get_link = render_link(base_url, display_name_property=display_name_property)
        if is_list:
...
def itemize(ename):
    ...
    return dict({'group': 'group', 
       ...
       'tube': 'cog', 
       'tubetask': 'date'}.items() + ITEM_TYPES.items()).get(ename, 'page_white')
