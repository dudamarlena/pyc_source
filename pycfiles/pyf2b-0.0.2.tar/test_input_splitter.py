...
# Embedded file name: build/bdist.linux-x86_64/egg/tests/test_input_splitter.py
# Compiled at: 2015-10-09 17:41:36
import os, datetime, unittest
from pyf.splitter import tokenize, get_splitter, chain_splitters, get_input_item_flow, get_input_item_str_flow, EndOfFileError, InputSplitterError, input_item_separator
...
class TestTokenize(unittest.TestCase):
    ...
    def test_tokenize(self):
        ...
        result = tokenize('', separator=input_item_separator)
        assert result == ([], ''), "Tokenize returned (%s, '%s') instead of ([], b'')" % (
         result[0], result[1])
        result = tokenize(input_item_separator, separator=input_item_separator)
        assert result == ([''], ''), "Tokenize returned (%s, '%s') instead of ([b''], b'')" % (
         result[0], result[1])
        result = tokenize('s' + input_item_separator, separator=input_item_separator)
        assert result == (['s'], ''), "Tokenize returned (%s, '%s') instead of ([b's'], b'')" % (
         result[0], result[1])
        result = tokenize('s', separator=input_item_separator)
        assert result == ([], 's'), "Tokenize returned (%s, '%s') instead of ([], b's')" % (
         result[0], result[1])
        result = tokenize(input_item_separator + 's', separator=input_item_separator)
        assert result == ([''], 's'), "Tokenize returned (%s, '%s') instead of ([b''], b's')" % (
         result[0], result[1])
...
def test_read_file_success():
    ...
    data = [
     's', '', '\x07ava', '\x11\x12\x12ava', 'agfdaaaaaaa']
    f = input_datafile(sep.join(data) + sep)
    count = 0
    for element in get_input_item_str_flow(f, chunk_size=1):
        assert element == data[count], 'The parser yielded %s instead of %s' % (element, data[count])
        count += 1
...
class TestSimplerSplitter(unittest.TestCase):
    ...
    def test_simple_splitter_empty_max(self):
        ...
        assert len(result) == 0, 'The splitter returned %s files instead of none' % len(result)
    ...
    def test_simple_splitter_one_nomax(self):
        ...
        self.splitter = get_splitter(input_items, 0)
        result = self.splitter.split()
        assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
        count = 0
        f = open(result[0], 'rb')
        for item in get_input_item_flow(f):
            assert item == input_items[count], '[File %s][Item %s] Got %s, %s instead of %s, %s' % (
             f.name, count,
             item, type(item),
    ...
    def test_simple_splitter_one_max1(self):
        ...
        f = open(result[0], 'rb')
        for item in get_input_item_flow(f):
            assert item == input_items[count], '[File %s][Item %s] Got %s instead of %s' % (
    ...
    def test_simple_splitter_eleven_maxmore(self):
        ...
        o = TestItem('ref0', 'value0')
        input_items.append(o)
        input_items.extend([ TestItem('ref1', 'value%s' % i) for i in range(1, 10) ])
        self.splitter = get_splitter(input_items, 0)
        result = self.splitter.split()
        assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
        count = 0
        ...
        assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
    ...
    def test_simple_splitter_eleven_max3(self):
        ...
        result = self.splitter.split()
        assert len(result) == 4, 'The splitter returned %s files instead of 4' % len(result)
        count = 0
        for filename in result:
            f = open(filename, 'rb')
            for item in get_input_item_flow(f):
    ...
    def test_simple_splitter_ten_max10(self):
        input_items = list()
        input_items.extend([ TestItem('ref1', 'value%s' % i) for i in range(0, 10) ])
        self.splitter = get_splitter(input_items, 10)
        result = self.splitter.split()
        assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
        count = 0
...
def test_splitter_attr():
    ...
    f = open(result[0], 'rb')
    for item in get_input_item_flow(f):
        assert item == input_items[count], 'Got %s instead of %s' % (input_items[count], item)
        count += 1
    ...
    for filename in result:
        ...
        for item in get_input_item_flow(f):
            ...
            assert ref_value not in already_seen, 'The value %s should have been in a previous file' % ref_value
            count += 1
    ...
    assert len(result) == 2, 'The splitter returned %s files instead of 2' % len(result)
    count = 0
    already_seen = list()
    for filename in result:
        ref_values = set()
...
def test_splitter_force_attr():
    ...
    split_attribute = 'field1'
    input_items = list()
    result = get_splitter(input_items, 0, split_attribute, force_split=True).split()
    assert len(result) == 0, 'The splitter returned %s files instead of none' % len(result)
    result = get_splitter(input_items, 1, split_attribute, force_split=True).split()
    assert len(result) == 0, 'The splitter returned %s files instead of none' % len(result)
    input_items.append(TestItem('ref0', 'value0'))
    result = get_splitter(input_items, 0, split_attribute, force_split=True).split()
    assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
    ...
    assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
     count, len(input_items))
    result = get_splitter(input_items, 10, split_attribute, force_split=True).split()
    assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
    count = 0
    f = open(result[0], 'rb')
    for item in get_input_item_flow(f):
    ...
    for filename in result:
        ...
        for item in get_input_item_flow(f):
            ...
            if not bucket_value:
                bucket_value = value
            assert value == bucket_value, 'We got an item with %s for its split_attribute in a file of items with %s' % (
             value, bucket_value)
            count += 1
    ...
    assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
     count, len(input_items))
    result = get_splitter(input_items, 3, split_attribute, force_split=True).split()
    assert len(result) == 2, 'The splitter returned %s files instead of 2' % len(result)
    count = 0
    for filename in result:
        bucket_value = None
    ...
    assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
     count, len(input_items))
    result = get_splitter(input_items, 5, split_attribute, force_split=True).split()
    assert len(result) == 2, 'The splitter returned %s files instead of 2' % len(result)
    count = 0
    for filename in result:
        bucket_value = None
...
def test_splitter_force_attr_fail_max():
    ...
    input_items.extend([ TestItem('ref0', 'value%s' % i) for i in range(10) ])
    input_items.extend([ TestItem('ref1', 'value%s' % i) for i in range(5) ])
    _ = get_splitter(input_items, 5, split_attribute, force_split=True).split()
...
def test_splitter_datetime():
    ...
    for fname in fnames:
        ...
        for index, item in enumerate(flow):
            assert item == elements[index], "Expected: '%s' got '%s'" % (item, elements[index])
...
def test_splitter_unicode():
    ...
    f = open(result[0], 'rb')
    for item in get_input_item_flow(f):
        assert item.field2 == unicode_field, "Simple splitter didn't preserve unicode value: got %s instead of %s" % (
    ...
    f = open(result[0], 'rb')
    for item in get_input_item_flow(f):
        assert item.field2 == unicode_field, "Splitter by attribute didn't preserve unicode value: got %s instead of %s" % (
         item.field2, unicode_field)

    f.close()
    result = get_splitter(input_items, 2, split_attribute, force_split=True).split()
    assert len(result) == 1, 'The splitter returned %s files instead of 1' % len(result)
    f = open(result[0], 'rb')
    for item in get_input_item_flow(f):
        assert item.field2 == unicode_field, "Splitter by attribute (forcing to split) didn't preserve unicode value: got %s instead of %s" % (
         item.field2, unicode_field)
...
def test_chaining_splitters_single():
    ...
    result = chain_splitters(input_items, params_splitters)
    assert len(result) == 3, 'The splitter returned %s files instead of 3' % len(result)
    count = 0
    for filename in result:
        f = open(filename, 'rb')
        for item in get_input_item_flow(f):
    ...
    assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
     count, len(input_items))
    params_splitters = [{'split_attribute': split_attribute, 'force_split': True}]
    result = chain_splitters(input_items, params_splitters)
    assert len(result) == 5, 'The splitter returned %s files instead of 5' % len(result)
    count = 0
    for filename in result:
        bucket_value = None
        f = open(filename, 'rb')
...
def test_chaining_splitters():
    ...
    params_splitters = [{'split_attribute': first_split_attribute}, {'split_attribute': second_split_attribute, 'force_split': True}]
    result = chain_splitters(input_items, params_splitters)
    assert len(result) == 3, 'The splitter returned %s files instead of 3' % len(result)
    count = 0
    for filename in result:
    ...
    assert count == len(input_items), 'We got %s items after splitting, instead of the %s we had before' % (
     count, len(input_items))
    params_splitters = [{'max_items': 8, 'split_attribute': first_split_attribute}, {'split_attribute': second_split_attribute, 'force_split': True}]
    result = chain_splitters(input_items, params_splitters)
    assert len(result) == 9, 'The splitter returned %s files instead of 9' % len(result)
    count = 0
    already_seen = list()
    for filename in result:
        bucket_value = None
        ...
        for item in get_input_item_flow(f):
            ...
            assert value == bucket_value, 'We got an item with %s for its split_attribute in a file of items with %s' % (
             value, bucket_value)
            ref_value = (
             getattr(item, first_split_attribute), value)
            ref_values.add(ref_value)
            assert ref_value not in already_seen, 'The value %s should have been in a previous file' % (
             ref_value,)
            count += 1
