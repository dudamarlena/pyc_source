...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/minio_storage/management/commands/minio.py
# Compiled at: 2019-10-22 14:18:07
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        group.add_argument('--class',
          type=str,
          default='minio_storage.storage.MinioMediaStorage',
          help='Storage class to modify (media/static are short names for default classes)')
        ...
        cmds._parser_class = argparse.ArgumentParser
        ...
        ls.add_argument('-r',
          '--recursive', action='store_true', help='find files recursive')
        ls.add_argument('-p', '--prefix', type=str, default='', help='path prefix')
        ls.add_argument('--buckets',
          action='store_true', help='list buckets instead of files')
        ls.add_argument('-f',
          '--format',
          type=str,
          default='$name',
    ...
    def handle(self, *args, **options):
        ...
        if command == self.LIST:
            ...
            if options['dirs'] or options['files']:
                ...
                summary = False
            return self.bucket_list(storage,
              bucket_name,
              ...
              summary=summary)
        if command == self.POLICY:
            if options['set'] is not None:
                return self.policy_set(storage,
                  bucket_name, policy=(Policy(options['set'])))
    ...
    def storage(self, options):
        class_name = {'media':'minio_storage.storage.MinioMediaStorage', 
         'static':'minio_storage.storage.MinioStaticStorage'}.get(options['class'], options['class'])
        try:
    ...
    def bucket_list(self, storage, bucket_name: str, *, prefix: str, list_dirs: bool, list_files: bool, recursive: bool, format: str=None, summary: bool=True):
        try:
            objs = storage.client.list_objects_v2(bucket_name,
            ...
            def fmt(o):
                ...
                return template.substitute(name=(o.object_name),
                  size=(o.size),
