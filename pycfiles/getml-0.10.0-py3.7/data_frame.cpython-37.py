...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/getml/engine/data_frame.py
# Compiled at: 2019-12-09 07:12:30
# Size of source mod 2**32: 46352 bytes
...
class DataFrame(object):
    ...
    def __init__(self, name, join_keys=None, time_stamps=None, categorical=None, discrete=None, numerical=None, targets=None, units=None):
        self.name = name
        ...
        self._DataFrame__categorical_columns = []
        for i, name in enumerate(categorical_names):
            self._DataFrame__categorical_columns.append(columns.CategoricalColumn(name=name,
              role='categorical',
              num=i,
              df_name=(self.name)))

        discrete_units = self._DataFrame__extract_units(discrete_names)
        self._DataFrame__discrete_columns = []
        for i, name in enumerate(discrete_names):
            self._DataFrame__discrete_columns.append(columns.Column(name=name,
              unit=(discrete_units[i]),
              role='discrete',
              num=i,
              df_name=(self.name)))

        self._DataFrame__join_key_columns = []
        for i, name in enumerate(join_key_names):
            self._DataFrame__join_key_columns.append(columns.CategoricalColumn(name=name,
              role='join_key',
              num=i,
              df_name=(self.name)))

        numerical_units = self._DataFrame__extract_units(numerical_names)
        self._DataFrame__numerical_columns = []
        for i, name in enumerate(numerical_names):
            self._DataFrame__numerical_columns.append(columns.Column(name=name,
              unit=(numerical_units[i]),
              role='numerical',
              num=i,
              df_name=(self.name)))

        self._DataFrame__target_columns = []
        for i, name in enumerate(target_names):
            self._DataFrame__target_columns.append(columns.Column(name=name,
              role='target',
              num=i,
              df_name=(self.name)))

        self._DataFrame__time_stamp_columns = []
        for i, name in enumerate(time_stamp_names):
            self._DataFrame__time_stamp_columns.append(columns.Column(name=name,
              role='time_stamp',
              num=i,
              df_name=(self.name)))
    ...
    def __extract_units(self, colnames):
        return [self.units[col] if col in self.units else '' for col in colnames]
    ...
    def __send_data(self, data_frame, s):
        for col in self._DataFrame__categorical_columns:
            col.send(data_frame[[col.name]].values.astype(np.str), s)

        for col in self._DataFrame__discrete_columns:
            if 'time stamp' in col.thisptr['unit_']:
                col.send(self._DataFrame__transform_timestamps(data_frame[[col.name]]), s)
            else:
                col.send(data_frame[[col.name]].apply((pd.to_numeric),
                  errors='coerce').values, s)

        for col in self._DataFrame__join_key_columns:
            col.send(data_frame[[col.name]].values.astype(np.str), s)

        for col in self._DataFrame__numerical_columns:
            if 'time stamp' in col.thisptr['unit_']:
                col.send(self._DataFrame__transform_timestamps(data_frame[[col.name]]), s)
            else:
                col.send(data_frame[[col.name]].apply((pd.to_numeric),
        ...
        for col in self._DataFrame__target_columns:
            col.send(data_frame[[col.name]].apply((pd.to_numeric),
              errors='raise').values, s)

        for col in self._DataFrame__time_stamp_columns:
            col.send(self._DataFrame__transform_timestamps(data_frame[[col.name]]), s)
    ...
    def __transform_timestamps(self, time_stamps):
        ...
        for colname in time_stamps.columns:
            if pd.api.types.is_numeric_dtype(time_stamps[colname]):
                ...
                transformed[colname] = time_stamps[[colname]].apply((pd.to_datetime),
                  errors='coerce').apply((pd.to_numeric),
                  errors='coerce').apply(lambda val: val / 86400000000000.0)[colname]
    ...
    def add_categorical(self, col, name, unit=''):
        ...
        """
        self._DataFrame__add_categorical_column(col, name, 'categorical', unit)
    ...
    def add_discrete(self, col, name, unit=''):
        ...
        Args:
            ...
            unit (otional): Unit of the column.
        """
        self._DataFrame__add_column(col, name, 'discrete', unit)
    ...
    def add_join_key(self, col, name):
        ...
        Args:
            ...
            name (str): Name of the new column in the DataFrame.
        """
        self._DataFrame__add_categorical_column(col, name, 'join_key', '')
    ...
    def add_numerical(self, col, name, unit=''):
        ...
        Args:
            ...
            unit (otional): Unit of the column.
        """
        self._DataFrame__add_column(col, name, 'numerical', unit)
    ...
    def add_target(self, col, name):
        ...
        Args:
            ...
            name (str): Name of the new column in the DataFrame.
        """
        self._DataFrame__add_column(col, name, 'target', '')
    ...
    def add_time_stamp(self, col, name):
        ...
        Args:
            ...
            name (str): Name of the new column in the DataFrame.
        """
        self._DataFrame__add_column(col, name, 'time_stamp', '')
    ...
    def append(self, data_frame, sock=None):
        ...
        """
        self._DataFrame__check_plausibility(data_frame)
        cmd = dict()
        ...
        if sock is None:
            ...
            s = sock
            comm.send_string(s, json.dumps(cmd))
        self._DataFrame__send_data(data_frame, s)
        self._DataFrame__close(s)
        if sock is None:
            s.close()
        return self
    ...
    def categorical(self, name):
        ...
        """
        return self._DataFrame__get_column(name, self._DataFrame__categorical_columns)
    ...
    def categorical_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__categorical_columns]
    ...
    def discrete(self, name):
        ...
        Args:
            name (str): Name of the column.
        """
        return self._DataFrame__get_column(name, self._DataFrame__discrete_columns)
    ...
    def discrete_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__discrete_columns]
    ...
    def join(self, name, other, join_key, other_join_key=None, cols=None, other_cols=None, how='inner', where=None):
        """
    ...
    def join_key(self, name):
        ...
        """
        return self._DataFrame__get_column(name, self._DataFrame__join_key_columns)
    ...
    def join_key_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__join_key_columns]
    ...
    def n_categorical(self):
        ...
        """
        return len(self._DataFrame__categorical_columns)
    ...
    def n_discrete(self):
        ...
        """
        return len(self._DataFrame__discrete_columns)
    ...
    def n_join_keys(self):
        ...
        Number of join keys.
        """
        return len(self._DataFrame__join_key_columns)
    ...
    def n_numerical(self):
        ...
        """
        return len(self._DataFrame__numerical_columns)
    ...
    def n_targets(self):
        ...
        """
        return len(self._DataFrame__target_columns)
    ...
    def n_time_stamps(self):
        ...
        """
        return len(self._DataFrame__time_stamp_columns)
    ...
    def numerical(self, name):
        ...
        Args:
            name (str): Name of the column.
        """
        return self._DataFrame__get_column(name, self._DataFrame__numerical_columns)
    ...
    def numerical_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__numerical_columns]
    ...
    def random(self, seed=5849):
        ...
        """
        col = columns._VirtualColumn(df_name=(self.name),
          operator='random',
          operand1=None,
          operand2=None)
    ...
    def read_csv(self, fnames, append=True, quotechar='"', sep=',', time_formats=[
 '%Y-%m-%dT%H:%M:%s%z', '%Y-%m-%d %H:%M:%S', '%Y-%m-%d']):
    ...
    def rowid(self):
        ...
        """
        return columns._VirtualColumn(df_name=(self.name),
          operator='rowid',
          operand1=None,
          operand2=None)
    ...
    def rm_categorical(self, name):
        ...
        """
        self._DataFrame__rm_col(name, 'categorical')
    ...
    def rm_discrete(self, name):
        ...
        Args:
            name (str): Name of the column to be removed.
        """
        self._DataFrame__rm_col(name, 'discrete')
    ...
    def rm_join_key(self, name):
        ...
        Args:
            name (str): Name of the column to be removed.
        """
        self._DataFrame__rm_col(name, 'join_key')
    ...
    def rm_numerical(self, name):
        ...
        Args:
            name (str): Name of the column to be removed.
        """
        self._DataFrame__rm_col(name, 'numerical')
    ...
    def rm_target(self, name):
        ...
        Args:
            name (str): Name of the column to be removed.
        """
        self._DataFrame__rm_col(name, 'target')
    ...
    def rm_time_stamp(self, name):
        ...
        Args:
            name (str): Name of the column to be removed.
        """
        self._DataFrame__rm_col(name, 'time_stamp')
    ...
    def target(self, name):
        ...
        Args:
            name (str): Name of the column.
        """
        return self._DataFrame__get_column(name, self._DataFrame__target_columns)
    ...
    def target_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__target_columns]
    ...
    def time_stamp(self, name):
        ...
        Args:
            name (str): Name of the column.
        """
        return self._DataFrame__get_column(name, self._DataFrame__time_stamp_columns)
    ...
    def time_stamp_names(self):
        ...
        """
        return [col.name for col in self._DataFrame__time_stamp_columns]
