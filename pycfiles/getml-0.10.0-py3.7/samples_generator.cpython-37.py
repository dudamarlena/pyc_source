...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/getml/datasets/samples_generator.py
# Compiled at: 2020-03-16 07:21:38
# Size of source mod 2**32: 43565 bytes
"""
...
def make_categorical(n_rows_population=500, n_rows_peripheral=125000, random_state=None, population_name='', peripheral_name='', aggregation=aggregations.Count):
    ...
    * `targets`: target variable. Defined as the number of matching entries in
    ...
    Args:
        ...
        population_name (string, optional):
            ...
            :class:`~getml.data.DataFrame` holding the population
            table. If set to a name already existing on the getML
            engine, the corresponding :class:`~getml.data.DataFrame`
        ...
        peripheral_name (string, optional):
            ...
            Name assigned to the create
            :class:`~getml.data.DataFrame` holding the peripheral
            table. If set to a name already existing on the getML
            engine, the corresponding :class:`~getml.data.DataFrame`
            will be overwritten. If set to an empty string, a unique
            name will be generated by concatenating
    ...
    Returns:
        tuple: tuple containing:
            ...
            * population (:class:`getml.data.DataFrame`): Population table

            * peripheral (:class:`getml.data.DataFrame`): Peripheral table
    ...
    if random_state is None:
        ...
        temp = peripheral_table.merge((population_table[['join_key', 'time_stamp_population']]),
          how='left',
          on='join_key')
        temp = temp[((temp['time_stamp_peripheral'] <= temp['time_stamp_population']) & (temp['column_01'] != '1') & (temp['column_01'] != '2') & (temp['column_01'] != '9'))]
        temp = _aggregate(temp, aggregation, 'column_01', 'join_key')
        temp = temp.rename(index=str, columns={'column_01': 'targets'})
        population_table = population_table.merge(temp,
          how='left',
          on='join_key')
        del temp
        population_table = population_table.rename(index=str,
          columns={'time_stamp_population': 'time_stamp'})
        peripheral_table = peripheral_table.rename(index=str,
          columns={'time_stamp_peripheral': 'time_stamp'})
        population_table.targets = np.where(np.isnan(population_table['targets']), 0, population_table['targets'])
        if not population_name:
            population_name = 'categorical_population_' + str(random_state)
        peripheral_name = peripheral_name or 'categorical_peripheral_' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'categorical':[
      'column_01'], 
     'time_stamp':[
      'time_stamp'], 
     'target':[
      'targets']}).read_pandas(population_table)
    peripheral_on_engine = data.DataFrame(name=peripheral_name,
      roles={'join_key':[
      'join_key'], 
     'categorical':[
      'column_01'], 
     'time_stamp':[
      'time_stamp']}).read_pandas(peripheral_table)
    return (
     population_on_engine, peripheral_on_engine)


def make_discrete(n_rows_population=500, n_rows_peripheral=125000, random_state=None, population_name='', peripheral_name='', aggregation=aggregations.Count):
    """Generate a random dataset with categorical variables
    ...
    * `targets`: target variable. Defined as the minimum value greater than 0
       in the peripheral table for which ``time_stamp_peripheral <
    ...
    if random_state is None:
        ...
        temp = peripheral_table.merge((population_table[['join_key', 'time_stamp_population']]),
          how='left',
          on='join_key')
        temp = temp[((temp['time_stamp_peripheral'] <= temp['time_stamp_population']) & (temp['column_01'] > 0.0))]
        temp = _aggregate(temp, aggregation, 'column_01', 'join_key')
        temp = temp.rename(index=str, columns={'column_01': 'targets'})
        population_table = population_table.merge(temp,
          how='left',
        ...
        peripheral_name = peripheral_name or 'discrete_peripheral_' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'numerical':[
...
def make_numerical(n_rows_population=500, n_rows_peripheral=125000, random_state=None, population_name='', peripheral_name='', aggregation=aggregations.Count):
    ...
    * `targets`: target variable. Defined as the number of matching entries in
    ...
    if random_state is None:
        ...
        temp = peripheral_table.merge((population_table[['join_key', 'time_stamp_population']]),
          how='left',
          on='join_key')
        temp = temp[((temp['time_stamp_peripheral'] <= temp['time_stamp_population']) & (temp['time_stamp_peripheral'] >= temp['time_stamp_population'] - 0.5))]
        temp = _aggregate(temp, aggregation, 'column_01', 'join_key')
        temp = temp.rename(index=str, columns={'column_01': 'targets'})
        population_table = population_table.merge(temp,
          how='left',
        ...
        peripheral_name = peripheral_name or 'numerical_peripheral_' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'numerical':[
...
def make_same_units_categorical(n_rows_population=500, n_rows_peripheral=125000, random_state=None, population_name='', peripheral_name='', aggregation=aggregations.Count):
    """Generate a random dataset with categorical variables
    ...
    * `targets`: target variable. Defined as the number of matching entries in
    ...
    random = np.random.RandomState(random_state)
    population_table = pd.DataFrame()
    population_table['column_01_population'] = (random.rand(n_rows_population) * 10.0).astype(np.int).astype(np.str)
    population_table['join_key'] = range(n_rows_population)
    population_table['time_stamp_population'] = random.rand(n_rows_population)
    peripheral_table = pd.DataFrame()
    peripheral_table['column_01_peripheral'] = (random.rand(n_rows_peripheral) * 10.0).astype(np.int).astype(np.str)
    peripheral_table['column_02'] = random.rand(n_rows_peripheral) * 2.0 - 1.0
    peripheral_table['join_key'] = [int(float(n_rows_population) * random.rand(1)[0]) for i in range(n_rows_peripheral)]
    peripheral_table['time_stamp_peripheral'] = random.rand(n_rows_peripheral)
    temp = peripheral_table.merge((population_table[['join_key', 'time_stamp_population',
     'column_01_population']]),
      how='left',
      on='join_key')
    temp = temp[((temp['time_stamp_peripheral'] <= temp['time_stamp_population']) & (temp['column_01_peripheral'] == temp['column_01_population']))]
    ...
    peripheral_table = peripheral_table.rename(index=str,
      columns={'time_stamp_peripheral': 'time_stamp'})
    population_table['targets'] = [0.0 if val != val else val for val in population_table['targets']]
    population_name = population_name or 'make_same_units_categorical_population__' + str(random_state)
    peripheral_name = peripheral_name or 'make_same_units_categorical_peripheral__' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'categorical':[
...
def make_same_units_numerical(n_rows_population=500, n_rows_peripheral=125000, random_state=None, population_name='', peripheral_name='', aggregation=aggregations.Count):
    """Generate a random dataset with continous numerical variables
    ...
    * `targets`: target variable. Defined as the number of matching entries in
    ...
    if random_state is None:
        ...
        random = np.random.RandomState(random_state)
        population_table = pd.DataFrame()
        population_table['column_01_population'] = random.rand(n_rows_population) * 2.0 - 1.0
        population_table['join_key'] = range(n_rows_population)
        population_table['time_stamp_population'] = random.rand(n_rows_population)
        peripheral_table = pd.DataFrame()
        peripheral_table['column_01_peripheral'] = random.rand(n_rows_peripheral) * 2.0 - 1.0
        peripheral_table['join_key'] = [int(float(n_rows_population) * random.rand(1)[0]) for i in range(n_rows_peripheral)]
        peripheral_table['time_stamp_peripheral'] = random.rand(n_rows_peripheral)
        temp = peripheral_table.merge((population_table[['join_key', 'time_stamp_population',
         'column_01_population']]),
          how='left',
          on='join_key')
        temp = temp[((temp['time_stamp_peripheral'] <= temp['time_stamp_population']) & (temp['column_01_peripheral'] > temp['column_01_population'] - 0.5))]
        ...
        peripheral_name = peripheral_name or 'same_unit_numerical_peripheral_' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'numerical':[
...
def make_snowflake(n_rows_population=500, n_rows_peripheral1=5000, n_rows_peripheral2=125000, random_state=None, population_name='', peripheral_name1='', peripheral_name2='', aggregation1=aggregations.Sum, aggregation2=aggregations.Count):
    """Generate a random dataset with continous numerical variables
    ...
    * `targets`: target variable as defined by the SQL block below:
    ...
    Args:
        ...
        peripheral_name1 (string, optional):
            ...
            Name assigned to the create
            :class:`~getml.data.DataFrame` holding the first
            peripheral table. If set to a name already existing on the
            getML engine, the corresponding
            :class:`~getml.data.DataFrame` will be overwritten. If
        ...
        peripheral_name2 (string, optional):
            ...
            Name assigned to the create
            :class:`~getml.data.DataFrame` holding the second
            peripheral table. If set to a name already existing on the
            getML engine, the corresponding
            :class:`~getml.data.DataFrame` will be overwritten. If
            set to an empty string, a unique name will be generated by
    ...
    Returns:
        tuple: tuple containing:
            ...
            * peripheral_2 (:class:`getml.data.DataFrame`): Peripheral table
    """
    if random_state is None:
        random_state = datetime.datetime.now().microsecond
    else:
        ...
        peripheral_table['join_key'] = [int(float(n_rows_population) * random.rand(1)[0]) for i in range(n_rows_peripheral1)]
        ...
        peripheral_table2['join_key2'] = [int(float(n_rows_peripheral1) * random.rand(1)[0]) for i in range(n_rows_peripheral2)]
        peripheral_table2['time_stamp_peripheral2'] = random.rand(n_rows_peripheral2)
        temp = peripheral_table2.merge((peripheral_table[['join_key2', 'time_stamp_peripheral']]),
          how='left',
          on='join_key2')
        temp = temp[((temp['time_stamp_peripheral2'] <= temp['time_stamp_peripheral']) & (temp['time_stamp_peripheral2'] >= temp['time_stamp_peripheral'] - 0.5))]
        temp = _aggregate(temp, aggregation2, 'column_01', 'join_key2')
        temp = temp.rename(index=str, columns={'column_01': 'temporary'})
        peripheral_table = peripheral_table.merge(temp,
          how='left',
          on='join_key2')
        del temp
        peripheral_table['temporary'] = [0.0 if val != val else val for val in peripheral_table['temporary']]
        temp2 = peripheral_table.merge((population_table[['join_key', 'time_stamp_population']]),
          how='left',
          on='join_key')
        ...
        population_table = population_table.merge(temp2,
          how='left',
          on='join_key')
        del temp2
        population_table['targets'] = [0.0 if val != val else val for val in population_table['targets']]
        ...
        peripheral_table = peripheral_table.rename(index=str,
          columns={'time_stamp_peripheral': 'time_stamp'})
        peripheral_table2 = peripheral_table2.rename(index=str,
          columns={'time_stamp_peripheral2': 'time_stamp'})
        if not population_name:
        ...
        peripheral_name2 = peripheral_name2 or 'snowflake_peripheral_2_' + str(random_state)
    population_on_engine = data.DataFrame(name=population_name,
      roles={'join_key':[
      'join_key'], 
     'numerical':[
     ...
     'target':[
      'targets']}).read_pandas(population_table)
    peripheral_on_engine = data.DataFrame(name=peripheral_name1,
      roles={'join_key':[
      'join_key', 'join_key2'], 
     'numerical':[
      'column_01'], 
    ...
    peripheral_on_engine2 = data.DataFrame(name=peripheral_name2,
     ...
     'numerical':[
      'column_01'], 
     'time_stamp':[
      'time_stamp']}).read_pandas(peripheral_table2)
    return (
     population_on_engine, peripheral_on_engine, peripheral_on_engine2)