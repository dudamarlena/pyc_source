...
class _VirtualBooleanColumn(object):
    ...
    def __init__(self, df_name, operator, operand1, operand2):
        ...
        self.thisptr['operand1_'] = self._VirtualBooleanColumn__parse_operand(operand1)
        if operand2 is not None:
            self.thisptr['operand2_'] = self._VirtualBooleanColumn__parse_operand(operand2)

    def __and__(self, other):
        return _VirtualBooleanColumn(df_name=(self.thisptr['df_name_']),
          operator='and',
          operand1=self,
          operand2=other)
    ...
    def __parse_operand(self, operand):
        if isinstance(operand, bool):
            ...
            if isinstance(operand, str):
                return {'type_':'CategoricalValue', 
                 'value_':operand}
            if isinstance(operand, numbers.Number):
                return {'type_':'Value', 
                 'value_':operand}
            if self.thisptr['operator_'] in ('and', 'or', 'not', 'xor') and operand.thisptr['type_'] != 'VirtualBooleanColumn':
...
class CategoricalColumn(container.Container):
    ...
    def __init__(self, name='', unit='', role='categorical', num=0, df_name=''):
        super(CategoricalColumn, self).__init__()
        CategoricalColumn.num_categorical_matrices += 1
        if name == '':
            self.name = 'CategoricalColumn ' + str(CategoricalColumn.num_categorical_matrices)
        else:
            self.name = name
        self.thisptr = dict()
        self.thisptr['df_name_'] = df_name
...
class _VirtualCategoricalColumn(object):
    ...
    def __init__(self, df_name, operator, operand1, operand2):
        ...
        if operand1 is not None:
            self.thisptr['operand1_'] = self._VirtualCategoricalColumn__parse_operand(operand1)
        if operand2 is not None:
            self.thisptr['operand2_'] = self._VirtualCategoricalColumn__parse_operand(operand2)

    def __parse_operand(self, operand):
        if isinstance(operand, str):
            return {'type_':'CategoricalValue', 
             'value_':operand}
            if self.thisptr['operator_'] != 'to_str':
                if operand.thisptr['type_'] != 'CategoricalColumn':
                    if operand.thisptr['type_'] != 'VirtualCategoricalColumn':
                        raise Exception('This operator can only be applied to a CategoricalColumn!')
        elif self.thisptr['operator_'] == 'to_str':
            if operand.thisptr['type_'] != 'Column':
                if operand.thisptr['type_'] != 'VirtualColumn' and operand.thisptr['type_'] != 'VirtualBooleanColumn':
...
class Column(container.Container):
    ...
    def __init__(self, name='', unit='', role='numerical', num=0, df_name=''):
        super(Column, self).__init__()
        Column.num_columns += 1
        if name == '':
            self.name = 'Column ' + str(Column.num_columns)
        else:
            self.name = name
        self.thisptr = dict()
        self.thisptr['df_name_'] = df_name
...
class _VirtualColumn(object):
    ...
    def __init__(self, df_name, operator, operand1, operand2):
        ...
        self.thisptr['operator_'] = operator
        if operand1 is not None:
            self.thisptr['operand1_'] = self._VirtualColumn__parse_operand(operand1)
        if operand2 is not None:
            self.thisptr['operand2_'] = self._VirtualColumn__parse_operand(operand2)
    ...
    def __parse_operand(self, operand):
        if isinstance(operand, numbers.Number):
            ...
            special_ops = [
             'to_num', 'to_ts']
            if self.thisptr['operator_'] not in special_ops:
                if operand.thisptr['type_'] != 'Column':
                    if operand.thisptr['type_'] != 'VirtualColumn':
                        raise Exception('This operator can only be applied to a Column!')
        elif self.thisptr['operator_'] in special_ops:
            if operand.thisptr['type_'] != 'CategoricalColumn' and operand.thisptr['type_'] != 'VirtualCategoricalColumn':
                raise Exception('This operator can only be applied to a CategoricalColumn!')
        return operand.thisptr
...
def __abs(self):
    ...
    return _VirtualColumn(df_name=(self.thisptr['df_name_']),
      operator='abs',
      operand1=self,
      operand2=None)
...
def __concat(self, other):
    return _VirtualCategoricalColumn(df_name=(self.thisptr['df_name_']),
      operator='concat',
      operand1=self,
      operand2=other)
...
def __substr(self, begin, length):
    ...
    """
    col = _VirtualCategoricalColumn(df_name=(self.thisptr['df_name_']),
      operator='substr',
      operand1=self,
      operand2=None)
...
def __to_ts(self, time_formats=[
 '%Y-%m-%dT%H:%M:%s%z', '%Y-%m-%d %H:%M:%S', '%Y-%m-%d']):
    ...
    """
    col = _VirtualColumn(df_name=(self.thisptr['df_name_']),
      operator='to_ts',
      operand1=self,
      operand2=None)
