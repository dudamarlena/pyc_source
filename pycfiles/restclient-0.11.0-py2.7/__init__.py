...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/restclient/__init__.py
# Compiled at: 2012-11-17 15:17:26
"""
...
"""
import httplib2, mimetypes, thread, types, urllib, urllib2
try:
    import json
except ImportError:
...
def post_multipart(host, selector, method, fields, files, headers=None, return_resp=False, scheme='http', credentials=None, httplib_params=None):
    """
...
def GET(url, params=None, files=None, accept=[], headers=None, async=False, resp=False, credentials=None, httplib_params=None):
    ...
    """
    return rest_invoke(url=url, method='GET', params=params, files=files, accept=accept, headers=headers, async=async, resp=resp, credentials=credentials, httplib_params=httplib_params)


def POST(url, params=None, files=None, accept=[], headers=None, async=True, resp=False, credentials=None, httplib_params=None):
    ...
    response object and the content instead of just the content.
    """
    return rest_invoke(url=url, method='POST', params=params, files=files, accept=accept, headers=headers, async=async, resp=resp, credentials=credentials, httplib_params=httplib_params)


def PUT(url, params=None, files=None, accept=[], headers=None, async=True, resp=False, credentials=None, httplib_params=None):
    ...
    response object and the content instead of just the content.
    """
    return rest_invoke(url=url, method='PUT', params=params, files=files, accept=accept, headers=headers, async=async, resp=resp, credentials=credentials, httplib_params=httplib_params)


def DELETE(url, params=None, files=None, accept=[], headers=None, async=True, resp=False, credentials=None, httplib_params=None):
    ...
    response object and the content instead of just the content.
    """
    return rest_invoke(url=url, method='DELETE', params=params, files=files, accept=accept, headers=headers, async=async, resp=resp, credentials=credentials, httplib_params=httplib_params)


def rest_invoke(url, method='GET', params=None, files=None, accept=[], headers=None, async=False, resp=False, httpcallback=None, credentials=None, httplib_params=None):
...
def _rest_invoke(url, method='GET', params=None, files=None, accept=None, headers=None, resp=False, httpcallback=None, credentials=None, httplib_params=None):
    if params is None:
        params = {}
    if files is None:
        files = {}
    ...
    if method in ('POST', 'PUT') and 'Content-Type' not in headers:
        ...
        params = urllib.urlencode(fix_params(params))
    if files:
        return post_multipart(extract_host(url), extract_path(url), method, unpack_params(params), unpack_files(fix_files(files)), fix_headers(headers), resp, scheme=extract_scheme(url), credentials=credentials, httplib_params=httplib_params)
    else:
        return non_multipart(params, extract_host(url), method, extract_path(url), fix_headers(headers), resp, scheme=extract_scheme(url), credentials=credentials, httplib_params=httplib_params)
        return


def non_multipart(params, host, method, path, headers, return_resp, scheme='http', credentials=None, httplib_params=None):
    if httplib_params is None:
        httplib_params = {}
    if method == 'GET':
...
if __name__ == '__main__':
    print rest_invoke('http://localhost:9090/', method='POST', params={'value': 'store this'}, accept=[
     'text/plain', 'text/html'], async=False)
    ...
    print rest_invoke('http://localhost:9090/foo/', params={'foo‒': '‒'}, headers={'foo‒': 'foo‒'})
    r = rest_invoke('http://localhost:9090/resize', method='POST', files={'image‒': {'file': image, 'filename': 'samp‒le.jpg'}}, async=False)