...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/restclient/test/test_everything.py
...
import threading, os, BaseHTTPServer, cgi
...
@servify
def test_post_image():
    result = POST(hostname + 'resize', files={'image': {'file': image, 'filename': 'sample.jpg'}}, async=False)
...
def test_get_unicode():
    expected = ('GET\nfoo‒: ‒\nDONE\n').encode('utf-8')
    r = GET(unicode(hostname + 'foo/'), params={'foo‒': '‒'}, headers={'foo‒': 'foo‒'})
...
def test_post_unicode():
    result = POST(unicode(hostname + 'foo/'), params={'foo‒': '‒'}, async=False)
    assert result.startswith('POST /foo/')
    expected = 'foo%E2%80%92=%E2%80%92'
