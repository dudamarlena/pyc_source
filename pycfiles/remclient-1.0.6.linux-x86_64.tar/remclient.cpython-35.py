...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/site-packages/remclient/remclient.py
# Compiled at: 2015-12-25 07:54:54
# Size of source mod 2**32: 41049 bytes
...
import six, logging, time, os, re, sre_parse, hashlib, getpass, types, socket, sys, itertools, datetime
...
class Queue(object):
    __doc__ = 'прокси объект для работы с очередями REM'
    ...
    def ListPackets(self, filter, name_regex=None, prefix=None):
        ...
        assert filter in ('errored', 'suspended', 'worked', 'waiting', 'pending', 'working',
                          'all')
    ...
    def ListUpdated(self, last_modified, filter=None):
        if filter:
            assert filter in ('errored', 'suspended', 'worked', 'waiting', 'pending',
                              'working', 'all')
...
class JobPacket(object):
    __doc__ = 'прокси объект для создания пакетов задач REM'
    DEFAULT_TRIES_COUNT = 5

    def __init__(self, connector, name, priority, notify_emails, wait_tags, set_tag, check_tag_uniqueness, resetable, kill_all_jobs_on_error=True, packet_name_policy=DEFAULT_DUPLICATE_NAMES_POLICY):
        self.conn = connector
        self.proxy = connector.proxy
    ...
    def AddJob(self, shell, parents=None, pipe_parents=None, set_tag=None, tries=DEFAULT_TRIES_COUNT, files=None, max_err_len=None, retry_delay=None, pipe_fail=False, description='', notify_timeout=NOTIFICATION_TIMEOUT, max_working_time=KILL_JOB_DEFAULT_TIMEOUT, output_to_status=False):
        ...
        if files is not None:
            self.AddFiles(files)
        return JobInfo(id=self.proxy.pck_add_job(self.id, shell, parents, pipe_parents, set_tag, tries, max_err_len, retry_delay, pipe_fail, description, notify_timeout, max_working_time, output_to_status))
    ...
    def AddJobsBulk(self, *jobs):
        ...
        for job in jobs:
            ...
            multicall.pck_add_job(self.id, job['shell'], parents, pipe_parents, job.get('set_tag', None), job.get('tries', self.DEFAULT_TRIES_COUNT), job.get('max_err_len', None), job.get('retry_delay', None), job.get('pipe_fail', None), job.get('description', ''), job.get('notify_timeout', NOTIFICATION_TIMEOUT), job.get('max_working_time', KILL_JOB_DEFAULT_TIMEOUT), job.get('output_to_status', False))
...
class JobInfo(object):
    __doc__ = 'объект, инкапсулирующий информацию о задаче REM'
...
class Tag(object):
    __doc__ = 'прокси объект для манипуляции тэгами'
...
class TagsBulk(object):
    __doc__ = 'Class for bulk operations on tags.'
    ...
    def __init__(self, conn, tags=None, name_regex=None, prefix=None):
        ...
        if tags is not None:
            ...
            if name_regex is not None or prefix is not None:
...
class Connector(object):
    __doc__ = 'объект коннектор, для работы с REM'
    ...
    def Packet(self, pckname, priority=MAX_PRIORITY, notify_emails=[], wait_tags=(), set_tag=None, check_tag_uniqueness=False, resetable=True, kill_all_jobs_on_error=True):
        ...
        try:
            ...
            return JobPacket(self, pckname, priority, notify_emails, wait_tags, set_tag, check_tag_uniqueness, resetable, kill_all_jobs_on_error=kill_all_jobs_on_error, packet_name_policy=self.packet_name_policy)
        except xmlrpc_client.Fault as e:
            if 'DuplicatePackageNameException' in e.faultString:
    ...
    def PacketInfo(self, packet):
        ...
        pck_id = packet.id if isinstance(packet, JobPacket) else packet if isinstance(packet, str) else None
        if pck_id is None:
...
class RetriableXMLRPCProxy(xmlrpc_client.ServerProxy):
    ...
    def __init__(self, uri, tries, **kws):
        self._RetriableXMLRPCProxy__maxTries = tries
        self._RetriableXMLRPCProxy__verbose = kws.pop('verbose')
        self._RetriableXMLRPCProxy__uri = uri
        kws['transport'] = AuthTransport()
        xmlrpc_client.ServerProxy.__init__(self, uri, **kws)

    def __getattr__(self, name):
        return _RetriableMethod(xmlrpc_client.ServerProxy.__getattr__(self, name), self._RetriableXMLRPCProxy__maxTries, self._RetriableXMLRPCProxy__verbose, socket.error)
