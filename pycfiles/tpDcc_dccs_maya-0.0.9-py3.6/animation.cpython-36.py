...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/animation.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 27009 bytes
"""
    ...
    def key_all_animation_curves_in_given_frame(self, frame, tangent_type='step'):
        ...
        for anim_curve in self._anim_curves:
            try:
                ...
                LOGGER.error('Error while keying animation curve "{}" on frame "{}" | {} | {}'.format(anim_curve, frame, exc, traceback.format_exc()))
                ...
                if all_anim_keyframes is None:
                    ...
                    for i, key in enumerate((self._start_frame, self._end_frame)):
                        ...
                        if infinity == 3:
                            ...
                            copy_option = 'keys'
                            first_frame_value = round(maya.cmds.keyframe(anim_curve,
                              query=True, time=(first_frame, first_frame), valueChange=True)[0], 3)
                            last_frame_value = round(maya.cmds.keyframe(anim_curve,
                              query=True, time=(last_frame, last_frame), valueChange=True)[0], 3)
                            if first_frame_value != last_frame_value:
                                ...
                                maya.cmds.copyKey(anim_curve, time=(copy_cycle_first_frame, copy_cycle_last_frame), float=(
                                 copy_cycle_first_frame, copy_cycle_last_frame),
                                  option=copy_option,
                                  hierarchy='none',
                                  controlPoints=False)
                                ...
                                if where == 'head':
                                    ...
                                    maya.cmds.pasteKey(anim_curve,
                                      time=(paste_frame, paste_frame), option='merge', copies=total_cycles_to_paste,
                                      connect=False,
                                    ...
                                    if where == 'head':
                                        if all_keys:
                                            if first_frame in all_keys:
                                                key_index = all_keys.index(first_frame)
                                                neight_inn_tangent = maya.mel.eval('keyTangent -q -inTangentType {};'.format(anim_curve))[key_index]
                                                neight_out_tangent = maya.mel.eval('keyTangent -q -outTangentType {};'.format(anim_curve))[key_index]
                        maya.cmds.setKeyframe(anim_curve, insert=True, time=key)
                        self._inserted_key_curves = self._add_to_delete_keys_dict(key, [anim_curve, [key, neight_inn_tangent, neight_out_tangent]], self._inserted_key_curves)
                        if infinity == 0:
                            maya.mel.eval('keyTangent -time {} -outTangentType "step" {} ;'.format(key, anim_curve))
                            if where == 'tail':
                                maya.mel.eval('keyTangent -time {} -outTangentType "step" {} ;'.format(last_frame, anim_curve))
                            if where == 'head':
                                maya.mel.eval('keyTangent -time {} -inTangentType "{}" -outTangentType "{}" {} ;'.format(first_frame, neight_inn_tangent, neight_out_tangent, anim_curve))
        ...
        for key in self._inserted_key_curves:
            for data in self._inserted_key_curves[key]:
                ...
                if neight_inn_tangent is not None:
                    maya.mel.eval('keyTangent -time {} -inTangentType "{}" -outTangentType "{}" {} ;'.format(preserve_tangent_key, neight_inn_tangent, neight_out_tangent, anim_curve))
...
def valid_anim_curve(anim_curve):
    ...
    if not maya.cmds.referenceQuery(anim_curve, isNodeReferenced=True):
...
def load_maya_animation_import_export_plugin():
    ...
    if not maya.cmds.pluginInfo(anim_plugin_name, query=True, loaded=True, n=True):
        try:
            ...
            LOGGER.error('Error importing animation plugin: "{}" | {} | {}'.format(anim_plugin_name, exc, traceback.format_exc()))
            return False
        ...
        for i, frame in enumerate(frames):
            ...
            if existing_frame is not None:
                if not all_keys or frame not in all_keys:
                    LOGGER.warning('Frame {} not found in {} keys for animation curve "{}"'.format(frame, len(all_keys), anim_curve))
                    continue
                key_index = all_keys.index(frame)
                current_tangent_out = maya.mel.eval('keyTangent -q -ott {}'.format(anim_curve))[key_index]
                if current_tangent_out != 'step':
                    if current_tangent_out != 'stepnext':
...
def check_anim_curves_has_fraction_keys(anim_curves, selected_range=None):
    ...
    if selected_range is not None:
        if type(selected_range) in [list, tuple]:
                    ...
                    if not frame.is_integer():
                        ...
                        if round_frame in keyframes or round_frame in keys_inserted:
                            ...
                            if round_frame > frame:
                                ...
                                if round_frame not in keyframes:
                                    if round_frame not in keys_inserted:
                                        try_again = False
                            if round_frame < frame:
                                if try_again:
                        ...
                        if frame_index != 0:
                            ...
                            pre_value = maya.cmds.keyframe(anim_curve,
                              time=(pre_frame, pre_frame), query=True, valueChange=True)
                            if pre_value is not None:
                                if current_value == pre_value[0]:
                                    is_hold = True
                        if frame_index != keyframes.index(keyframes[(-1)]):
                            post_frame = keyframes[(frame_index + 1)]
                            post_value = maya.cmds.keyframe(anim_curve,
                              time=(post_frame, post_frame), query=True, valueChange=True)
                            if post_value is not None:
                                if current_value == post_value[0]:
                                    is_hold = True
                        try:
                            if is_hold:
                                maya.cmds.keyframe(anim_curve,
                                  edit=True, absolute=True, timeChange=round_frame, time=(frame, frame))
                            else:
...
def get_active_frame_range():
    ...
    """
    return (
     maya.cmds.playbackOptions(query=True, minTime=True), maya.cmds.playbackOptions(query=True, maxTime=True))
...
def set_active_frame_range(start_frame, end_frame):
    ...
    :param end_frame: int
    """
    return maya.cmds.playbackOptions(animationStartTime=start_frame,
      minTime=start_frame,
...
def bake_animation(nodes, min_time=None, max_time=None):
    ...
    maya.cmds.bakeResults(nodes,
      simulation=True, t=(min_time, max_time), sampleBy=1, oversamplingRate=1, disableImplicitControl=True, preserveOutsideKeys=True,
...
def quick_driven_key(source, target, source_values, target_values, infinite=False, tangent_type='linear'):
    ...
    for i in range(len(source_values)):
        maya.cmds.setDrivenKeyframe(target,
          cd=source, driverValue=(source_values[i]), value=(target_values[i]),
          itt=(tangent_type[0]),
          ott=(tangent_type[1]))
