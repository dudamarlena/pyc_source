...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/camera.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 10113 bytes
"""
...
def get_all_cameras(exclude_standard_cameras=True, return_transforms=True, full_path=True):
    ...
    if exclude_standard_cameras:
        cameras = [c for c in maya.cmds.ls(type='camera',
          long=full_path) if not maya.cmds.camera(c, query=True, sc=True)]
    else:
        ...
        if maya.cmds.getPanel(typeOf=panel) == 'modelPanel':
            ...
            if cam:
                if maya.cmds.nodeType(cam) == 'transform':
                    return cam
                if maya.cmds.objectType(cam, isAType='shape'):
...
def set_current_camera(camera_name):
    ...
    if maya.cmds.nodeType(camera_name) == 'transform':
        ...
        if shapes:
            if maya.cmds.nodeType(shapes[0]) == 'camera':
...
def get_eye_point(camera_name):
    ...
    return [
     camera_pt.x, camera_pt.y, camera_pt.z]
