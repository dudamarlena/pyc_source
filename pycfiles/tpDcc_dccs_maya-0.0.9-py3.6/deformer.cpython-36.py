...
class MayaDeformerData(object):
    ...
    def build_data(self):
        ...
        if not self._deformer:
            ...
            for geo in self._data['affectedGeometry']:
                ...
                self._data[geo]['membership'] = deformer.get_deformer_set_member_indices(deformer=(self._deformer),
                  geometry=geo)
    ...
    def get_deformer_attr_values(self):
        ...
        for attr in self._data['attrValueList']:
            if not cmds.getAttr((deformer + '.' + attr), se=True) and cmds.listConnections((deformer + '.' + attr),
              s=True, d=False):
    ...
    def get_deformer_attr_connections(self):
        ...
        for attr in self._data['attrConnectionList']:
            attr_cnt = cmds.listConnections((deformer + '.' + attr),
              s=True, d=False, p=True, sh=True, skipConversionNodes=True)
            if attr_cnt:
    ...
    def custom_deformer_atributes(self, deformer_type):
        ...
        if deformer_type == 'curveTwist':
            ...
            if deformer_type == 'directionalSmooth':
