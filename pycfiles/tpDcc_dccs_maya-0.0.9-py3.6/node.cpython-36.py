...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/node.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 31120 bytes
"""
    ...
    if api_type == maya.OpenMaya.MFn.kTypedAttribute:
        ...
        if api_type == maya.OpenMaya.MFn.kNumericAttribute:
            ...
            if plug_type == maya.OpenMaya.MFnNumericData.kBoolean:
                return plug.asBool()
            if plug_type in [maya.OpenMaya.MFnNumericData.kShort, maya.OpenMaya.MFnNumericData.kInt,
...
def get_input_attributes(node):
    ...
    if not maya.cmds.objExists(node):
        ...
        inputs = maya.cmds.listConnections(node,
          connections=True, destination=False, source=True, plugs=True, skipConversionNodes=True)
        if inputs:
            inputs.reverse()
        else:
            inputs = list()
...
def get_output_attributes(node):
    ...
    if not maya.cmds.objExists(node):
        raise Exception('Object "{}" does not exists!'.format(node))
    outputs = maya.cmds.listConnections(node,
      connections=True, destination=True, source=False, plugs=True, skipConversionNodes=True)
    if not outputs:
        outputs = list()
    return outputs
