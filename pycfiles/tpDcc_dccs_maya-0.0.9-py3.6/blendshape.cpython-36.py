...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/blendshape.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 22158 bytes
"""
This module include data class for blendshapes
"""
from __future__ import print_function, division, absolute_import
import logging, tpDcc.dccs.maya as maya
...
def add_empty_target(blend_shape, target_alias='', input_target_items=None):
    ...
    for i in out_geo:
        (maya.cmds.setAttr)(
 '{}.inputTarget[{}].inputTargetGroup[{}].inputTargetItem[6000].inputPointsTarget'.format(blend_shape, i, next_index), *[0], **{'type': 'pointArray'})
        if input_target_items:
            for iti in input_target_items:
                (maya.cmds.setAttr)(
 '{}.inputTarget[{}].inputTargetGroup[{}].inputTargetItem[{}].inputPointsTarget'.format(blend_shape, i, next_index, iti), *[0], **{'type': 'pointArray'})
...
def add_target(blend_shape, target, base='', target_index=-1, target_alias='', target_weight=0.0, topology_check=True):
    ...
    if base:
        if not maya.cmds.objExists(base):
...
def connect_to_target(blend_shape, target_geo, target_name, base_geo, weight=1.0, force=False):
    ...
    elif not maya.cmds.objExists(target_geo):
        raise Exception('Target geometry "{}" does not exists!'.format(target_geo))
    else:
        target_index = get_target_index(blend_shape, target_name)
        if force:
            ...
            if geo_shape:
                ...
                geo_type = 'none'
            geo_dict = {'mesh':'.worldMesh[0]', 
             'nurbsSurface':'.worldSpace[0]',  'nurbsCurve':'.worldSpace[0]'}
            if geo_type in geo_dict:
                geo_attr = geo_dict[geo_type]
            else:
                geo_attr = ''
            weight_index = int(weight * 6000)
            maya.cmds.connectAttr((geo_shape + geo_attr),
              (blend_shape + '.inputTarget[' + str(geo_index) + '].inputTargetGroup[' + str(target_index) + '].inputTargetItem[' + str(weight_index) + '].inputGeomTarget'),
              f=True)
        elif has_target(blend_shape, target_name) and weight != 1.0:
