...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/dcc.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 150936 bytes
"""
...
class MayaDcc(abstract_dcc.AbstractDCC, object):
    ...
    def node_unreference(node):
        ...
        if ref_utils.is_referenced(node):
            ...
            if ref_utils.is_reference(node):
                ref_node = node
        if ref_node:
    ...
    def node_children(node, all_hierarchy=True, full_path=True):
        ...
        :return: list<str>
        """
        return maya.cmds.listRelatives(node,
          children=True, allDescendents=all_hierarchy, shapes=False, fullPath=full_path)
    ...
    def list_children(node, all_hierarchy=True, full_path=True, children_type=None):
        ...
        if children_type:
            return maya.cmds.listRelatives(node,
              children=True, allDescendents=all_hierarchy, fullPath=full_path, type=children_type)
        else:
            return maya.cmds.listRelatives(node, children=True, allDescendents=all_hierarchy, fullPath=full_path)

    @staticmethod
    def list_relatives(node, all_hierarchy=False, full_path=True, relative_type=None, shapes=False, intermediate_shapes=False):
        """
        ...
        if relative_type:
            return maya.cmds.listRelatives(node,
              allDescendents=all_hierarchy, fullPath=full_path, type=relative_type, shapes=shapes,
              noIntermediate=(not intermediate_shapes))
        else:
            return maya.cmds.listRelatives(node,
              allDescendents=all_hierarchy, fullPath=full_path, shapes=shapes, noIntermediate=(not intermediate_shapes))
    ...
    def list_shapes(node, full_path=True, intermediate_shapes=False):
        ...
        """
        return maya.cmds.listRelatives(node,
          shapes=True, fullPath=full_path, children=True, noIntermediate=(not intermediate_shapes))
    ...
    def list_shapes_of_type(node, shape_type=None, full_path=True, intermediate_shapes=False):
        ...
        :return: list<str>
        """
        return shape_utils.get_shapes_of_type(node_name=node,
          shape_type=shape_type,
          full_path=full_path,
          no_intermediate=(not intermediate_shapes))
    ...
    def list_children_shapes(node, all_hierarchy=True, full_path=True, intermediate_shapes=False):
        ...
        :return:
        """
        return shape_utils.get_shapes_in_hierarchy(transform_node=node,
          full_path=full_path,
    ...
    def apply_shader(material, node):
        ...
        if maya.cmds.nodeType(material) in ('surfaceShader', 'lambert'):
            ...
            if maya.cmds.nodeType(material) == 'shadingEngine':
 ...
 node, ln=attribute_name, at='double', dv=default_value, min=min_value, max=max_value, **kwargs)
    ...
    def set_float_vector3_attribute_value(node, attribute_name, attribute_value):
        ...
        :param attribute_value: str
        """
        return maya.cmds.setAttr(('{}.{}'.format(node, attribute_name)),
          (float(attribute_value[0])),
          (float(attribute_value[1])), (float(attribute_value[2])), type='double3')
    ...
    def connect_attribute(source_node, source_attribute, target_node, target_attribute, force=False):
        ...
        :param force: bool
        """
        return maya.cmds.connectAttr(('{}.{}'.format(source_node, source_attribute)),
    ...
    def connect_visibility(node, attr, target_node, default_value=True):
        ...
        """
        return attribute.connect_visibility(('{}.{}'.format(node, attr)),
          target_node, default_value=default_value)
    ...
    def store_world_matrix_to_attribute(node, attribute_name='worldMatrix', **kwargs):
        ...
        return attribute.store_world_matrix_to_attribute(transform=node,
    ...
    def import_file(file_path, force=True, **kwargs):
        ...
        if namespace:
            ...
            if unique_namespace:
                ...
                return maya.cmds.file(file_path,
                  i=True, f=force, returnNewNodes=True, mergeNamespacesOnClash=True, namespace=namespace)
        else:
    ...
    def reference_file(file_path, force=True, **kwargs):
        ...
        if namespace:
            ...
            if unique_namespace:
                ...
                return maya.cmds.file(file_path,
                  reference=True, f=force, returnNewNodes=True, mergeNamespacesOnClash=True,
                  namespace=namespace)
        else:
    ...
    def confirm_dialog(title, message, button=None, cancel_button=None, default_button=None, dismiss_string=None):
        ...
        if button:
            if cancel_button:
                if dismiss_string:
                    if default_button:
                        return maya.cmds.confirmDialog(title=title,
                          message=message,
                          button=button,
                          cancelButton=cancel_button,
                          defaultButton=default_button,
                          dismissString=dismiss_string)
        if button:
    ...
    def add_shelf_separator(shelf_name):
        ...
        """
        return maya.cmds.separator(parent=shelf_name,
          manage=True,
          visible=True,
          horizontalong=False,
          style='shelf',
    ...
    def offset_keyframes(node, attribute_name, start_time, end_time, duration):
        ...
        """
        return maya.cmds.keyframe(('{}.{}'.format(node, attribute_name)),
          relative=True, time=(start_time, end_time), timeChange=duration)
    ...
    @staticmethod
    def find_unique_name(obj_names=None, filter_type=None, include_last_number=True, do_rename=False, search_hierarchy=False, selection_only=True, **kwargs):
        """
       ...
       :param search_hierarchy: bool, Whether to search objects in hierarchies
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.find_unique_name_by_filter(filter_type=filter_type,
              include_last_number=include_last_number,
              do_rename=do_rename,
            ...
            return name.find_unique_name(obj_names=obj_names,
              include_last_number=include_last_number,
              do_rename=do_rename,
              rename_shape=rename_shape)
    ...
    def find_available_name(node_name, **kwargs):
        ...
        return name.find_available_name(name=node_name,
          suffix=suffix,
          index=index,
          padding=padding,
          letters=letters,
    ...
    def export_shot_animation_curves(anim_curves_to_export, export_file_path, start_frame, end_frame, *args, **kwargs):
        ...
        return sequencer.export_shot_animation_curves(anim_curves_to_export=anim_curves_to_export,
          export_file_path=export_file_path,
          start_frame=start_frame,
          end_frame=end_frame,
          sequencer_least_key=sequencer_least_key,
          sequencer_great_key=sequencer_great_key)
    ...
    def import_shot_animation_curves(anim_curves_to_import, import_file_path, start_frame, end_frame):
        ...
        :param kwargs:
        """
        return sequencer.import_shot_animation_curves(anim_curves_to_import=anim_curves_to_import,
    ...
    def remove_keys_from_animation_curves(range_to_delete, anim_curves=None):
        ...
        :param anim_curves: list(str)
        """
        return animation.delete_keys_from_animation_curves_in_range(range_to_delete=range_to_delete,
    ...
    def convert_fraction_keys_to_whole_keys(animation_curves=None, consider_selected_range=False):
        ...
        :return:
        """
        return animation.convert_fraction_keys_to_whole_keys(animation_curves=animation_curves,
          consider_selected_range=consider_selected_range)
 ...
 point_to, source, aim=aim_axis, upVector=up_axis, worldUpVector=world_up_axis, worldUpType=world_up_type, 
         weight=weight, mo=maintain_offset, **kwargs)
    ...
    def freeze_transforms(node, **kwargs):
        ...
        return transform.freeze_transforms(node=node,
          translate=translate,
          rotate=rotate,
          scale=scale,
          normal=normal,
    ...
    def filter_nodes_by_type(filter_type, search_hierarchy=False, selection_only=True, **kwargs):
        ...
        return filtertypes.filter_by_type(filter_type=filter_type,
          search_hierarchy=search_hierarchy,
          selection_only=selection_only,
          dag=dag,
    ...
    @staticmethod
    def add_name_prefix(prefix, obj_names=None, filter_type=None, add_underscore=False, search_hierarchy=False, selection_only=True, **kwargs):
        """
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.add_prefix_by_filter(prefix=prefix,
            ...
            return name.add_prefix(prefix=prefix,
              obj_names=obj_names,
              add_underscore=add_underscore,
              rename_shape=rename_shape)

    @staticmethod
    def add_name_suffix(suffix, obj_names=None, filter_type=None, add_underscore=False, search_hierarchy=False, selection_only=True, **kwargs):
        """
        Add prefix to node name
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.add_suffix_by_filter(suffix=suffix,
              filter_type=filter_type,
              add_underscore=add_underscore,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
            ...
            return name.add_suffix(suffix=suffix,
              obj_names=obj_names,
              add_underscore=add_underscore,
              rename_shape=rename_shape)

    @staticmethod
    def remove_name_prefix(obj_names=None, filter_type=None, separator='_', search_hierarchy=False, selection_only=True, **kwargs):
        """
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.edit_item_index_by_filter(index=0,
              filter_type=filter_type,
              text='',
              mode=(name.EditIndexModes.REMOVE),
              separator=separator,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
              selection_only=selection_only,
            ...
            return name.edit_item_index(obj_names=obj_names,
              index=0,
              mode=(name.EditIndexModes.REMOVE),
              separator=separator,
              rename_shape=rename_shape)
    ...
    @staticmethod
    def remove_name_suffix(obj_names=None, filter_type=None, separator='_', search_hierarchy=False, selection_only=True, **kwargs):
        """
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.edit_item_index_by_filter(index=(-1),
              filter_type=filter_type,
              text='',
              mode=(name.EditIndexModes.REMOVE),
              separator=separator,
    ...
    def auto_name_suffix(obj_names=None, filter_type=None, search_hierarchy=False, selection_only=True, **kwargs):
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.auto_suffix_object_by_type(filter_type=filter_type,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
              selection_only=selection_only,
              dag=False,
    ...
    @staticmethod
    def remove_name_numbers(obj_names=None, filter_type=None, search_hierarchy=False, selection_only=True, remove_underscores=True, trailing_only=False, **kwargs):
        """
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.remove_numbers_from_object_by_filter(filter_type=filter_type,
              rename_shape=rename_shape,
            ...
            return name.remove_numbers_from_object(obj_names=obj_names,
              trailing_only=trailing_only,
              rename_shape=rename_shape,
              remove_underscores=remove_underscores)

    @staticmethod
    def renumber_objects(obj_names=None, filter_type=None, remove_trailing_numbers=True, add_underscore=True, padding=2, search_hierarchy=False, selection_only=True, **kwargs):
        """
        Removes numbers from node names
        :param obj_names: str or list(str), name of list of node names to rename
        :param filter_type: str, name of object type to filter the objects to apply changes ('Group, 'Joint', etc)
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.renumber_objects_by_filter(filter_type=filter_type,
            ...
            return name.renumber_objects(obj_names=obj_names,
              remove_trailing_numbers=remove_trailing_numbers,
              add_underscore=add_underscore,
              padding=padding)

    @staticmethod
    def change_suffix_padding(obj_names=None, filter_type=None, add_underscore=True, padding=2, search_hierarchy=False, selection_only=True, **kwargs):
        """
        Removes numbers from node names
        :param obj_names: str or list(str), name of list of node names to rename
        :param filter_type: str, name of object type to filter the objects to apply changes ('Group, 'Joint', etc)
        ...
        rename_shape = kwargs.get('rename_shape', True)
        if filter_type:
            return name.change_suffix_padding_by_filter(filter_type=filter_type,
              add_underscore=add_underscore,
              padding=padding,
              rename_shape=rename_shape,
              search_hierarchy=search_hierarchy,
    ...
    def create_nurbs_cylinder(name='cylinder', radius=1.0, **kwargs):
        ...
        construction_history = kwargs.get('construction_history', True)
        return maya.cmds.cylinder(name=name,
          ax=axis,
          ssw=0,
          esw=360,
          r=radius,
    ...
    def rebuild_curve(curve, spans, **kwargs):
        ...
        return maya.cmds.rebuildCurve(curve,
          spans=spans, rpo=replace_original, rt=rebuild_type, end=end_knots, kr=keep_range, kcp=keep_control_points,
          kep=keep_end_points,
          kt=keep_tangents,
          d=degree,
    ...
    def convert_surface_to_bezier(surface, **kwargs):
        ...
        return maya.cmds.rebuildSurface(surface,
          ch=construction_history, rpo=replace_original, su=spans_u,
          sv=spans_v,
          rt=7,
          du=degree_u,
          dv=degree_v)
    ...
    def create_empty_mesh(mesh_name):
        ...
        :return: str
        """
        return maya.cmds.polyCreateFacet(name=mesh_name,
          ch=False,
          tx=1,
          s=1,
    ...
    def create_ik_handle(name, start_joint, end_joint, solver_type=None, curve=None, **kwargs):
        ...
        if curve:
            if maya.cmds.objExists(curve):
                handle.set_curve(curve)
        return handle.create()

    @staticmethod
    def create_spline_ik_stretch(curve, joints, node_for_attribute=None, create_stretch_on_off=False, stretch_axis='X', **kwargs):
        """
        ...
        return ik_utils.create_spline_ik_stretch(curve,
          joints, node_for_attribute=node_for_attribute, create_stretch_on_off=create_stretch_on_off, scale_axis=stretch_axis,
          create_bulge=create_bulge)

    @staticmethod
    def create_cluster_surface(surface, name, first_cluster_pivot_at_start=True, last_cluster_pivot_at_end=True, join_ends=False):
        """
        ...
        return (
         cluster_surface.get_cluster_handle_list(), cluster_surface.get_cluster_list())

    @staticmethod
    def create_cluster_curve(curve, name, first_cluster_pivot_at_start=True, last_cluster_pivot_at_end=True, join_ends=False):
        """
        ...
        return (
         cluster_curve.get_cluster_handle_list(), cluster_curve.get_cluster_list())
    ...
    def create_nurbs_surface_from_transforms(transforms, name, spans=-1, offset_axis='Y', offset_amount=1):
        ...
        """
        return geo_utils.transforms_to_nurbs_surface(transforms,
          name, spans=spans, offset_axis=offset_axis, offset_amount=offset_amount)
    ...
    def get_color_of_side(side='C', sub_color=False):
        ...
        if MayaDcc.name_is_center(side):
            ...
            if MayaDcc.name_is_left(side):
                side = 'L'
            else:
                if MayaDcc.name_is_right(side):
                    side = 'R'
                else:
                    side = 'C'
            if not sub_color:
...
class MayaProgessBar(progressbar.AbstractProgressBar, object):
    ...
    def __init__(self, title='', count=None, begin=True):
        ...
        maya.cmds.progressBar((self.progress_ui),
          edit=True, beginProgress=begin, isInterruptable=True, status=title, maxValue=count)
