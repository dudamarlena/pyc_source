...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/meta/metautils.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 112695 bytes
"""
Module that contains functions and classes related with meta system
"""
import copy, pprint, logging, euclid, tpDcc.dccs.maya as maya
...
class MetaAttributeUtils(object):
    ...
    attr_types = dict(message=('message', 'msg', 'm'), double=('float', 'fl', 'f',
    ...
    def validate_attr_type_match(cls, type_a, type_b):
        ...
        if type_a == type_b:
            ...
            for o in cls.attr_types.keys():
                if type_a in cls.attr_types.get(o):
                    if type_b in cls.attr_types.get(o):
                        return True
                ...
                if _data is not None:
                    ...
                    try:
                        if _attr_type == 'string':
                            ...
                            if _attr_type == 'double':
                                _data = float(_data)
                            else:
                                if _attr_type == 'long':
                                    _data = int(_data)
                    except Exception as e:
                        LOGGER.error('|Convert Attribute Type| >> Failed to convert data: {0} | type: {1} | err: {2}'.format(_data, _attr_type, e))
    ...
    def get(*args, **kwargs):
        ...
        try:
            ...
            LOGGER.debug('|Attribute Getter| >> {0} failed to return type. Exists: {1}'.format(combined, maya.cmds.objExists(combined)))
            return
    ...
    def get_driver(node, attr=None, get_node=False, skip_conversion_nodes=False, long_names=True):
        ...
        if not MetaAttributeUtils.is_connected(attr_dict):
            ...
            if get_node:
                connections = maya.cmds.listConnections(combined, skipConversionNodes=skip_conversion_nodes, destination=False,
                  source=True,
                  plugs=False)
                if not connections:
                    ...
                    if parent:
                        ...
                        return MetaAttributeUtils.get_driver((attr_dict['node']), parent, get_node=get_node, skip_conversion_nodes=skip_conversion_nodes,
                ...
                if maya.cmds.connectionInfo(combined, isDestination=True):
                    ...
                    if not connections:
                        connections = [
                         maya.cmds.connectionInfo(combined, sourceFromDestination=True)]
                    if connections:
                        if skip_conversion_nodes:
                            if MetaAttributeValidator.get_maya_type(node=connections) == 'unitConversion':
                                parent = MetaAttributeUtils.get_parent(attr_dict)
                                if parent:
                                    LOGGER.debug('|Driver Attribute Getter| >> Parent Attribute Check: {}'.format(parent))
                                    return MetaAttributeUtils.get_driver((attr_dict['node']), parent, get_node=get_node, skip_conversion_nodes=skip_conversion_nodes,
    ...
    def get_driven(node, attr=None, get_node=False, skip_conversion_nodes=False, long_names=True):
        ...
        combined = attr_dict['combined']
        if get_node:
            connections = maya.cmds.listConnections(combined, skipConversionNodes=skip_conversion_nodes, destination=True,
              source=False,
              plugs=False)
            if not connections:
                return False
    ...
    def add(obj, attr=None, attr_type=None, enum_options=['off', 'on'], *args, **kwargs):
        ...
        try:
            if enum_options is None:
                ...
                if _type == 'string':
                    (maya.cmds.addAttr)(node, *args, ln=attr_name, dt='string', **kwargs)
                else:
                    if _type == 'double':
                        (maya.cmds.addAttr)(node, *args, ln=attr_name, at='float', **kwargs)
                    else:
                        if _type == 'long':
                            (maya.cmds.addAttr)(node, *args, ln=attr_name, at='long', **kwargs)
                        else:
                            if _type == 'double3':
                                (maya.cmds.addAttr)(node, *args, ln=attr_name, at='double3', **kwargs)
                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'X', p=attr_name, at='double', **kwargs)
                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'Y', p=attr_name, at='double', **kwargs)
                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'Z', p=attr_name, at='double', **kwargs)
                            else:
                                if _type == 'enum':
                                    if type(enum_options) in [list, tuple]:
                                        enum_options = '%s' % ':'.join(enum_options)
                                    (maya.cmds.addAttr)(node, *args, ln=attr_name, at='enum', en=enum_options, **kwargs)
                                    maya.cmds.setAttr((node + '.' + attr_name), e=True, keyable=True)
                                else:
                                    if _type == 'bool':
                                        (maya.cmds.addAttr)(node, *args, ln=attr_name, at='bool', **kwargs)
                                        maya.cmds.setAttr((node + '.' + attr_name), edit=True, channelBox=True)
                                    else:
                                        if _type == 'message':
                                            (maya.cmds.addAttr)(node, *args, ln=attr_name, at='message', **kwargs)
                                        else:
                                            if _type == 'float3':
                                                (maya.cmds.addAttr)(node, *args, ln=attr_name, at='float3', **kwargs)
                                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'X', p=attr_name, at='float', **kwargs)
                                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'Y', p=attr_name, at='float', **kwargs)
                                                (maya.cmds.addAttr)(node, *args, ln=attr_name + 'Z', p=attr_name, at='float', **kwargs)
                                            else:
    ...
    def get_numeric_attribute_state(*args):
        ...
        if not numeric:
            if MetaAttributeUtils.get_children(attr_dict):
                return {}
        try:
    ...
    def get_attribute_state(*args):
        ...
        data_dict = {'type':maya.cmds.getAttr(combined, type=True),  'locked':maya.cmds.getAttr(combined, lock=True), 
    ...
    def copy_to(from_object, from_attr, to_object=None, to_attr=None, convert_to_match=True, values=True, in_connection=False, out_connections=False, keep_source_connections=True, copy_settings=True, driven=None):
        ...
        if combined == attr_dict_target['combined']:
            ...
            if values:
                if not MetaAttributeUtils.validate_attr_type_name(dict_source_flags['type']):
                ...
                elif MetaAttributeUtils.validate_attr_type_match(dict_source_flags['type'], dict_target_flags['type']) or dict_target_flags['dynamic'] and convert_to_match:
    ...
    def break_connection(*args):
        ...
        if MetaAttributeUtils.get_type(attr_dict) == 'message':
            ...
            dst = maya.cmds.listConnections(combined,
              skipConversionNodes=False, destination=True, source=False, plugs=True)
            if dst:
        ...
        elif maya.cmds.connectionInfo(combined, isDestination=True):
            source_connections = maya.cmds.listConnections(combined,
              skipConversionNodes=False, destination=False, source=True, plugs=True)
            family = source_connections or MetaAttributeUtils.get_family_dict(attr_dict)
    ...
    def get_name_long(*args):
        ...
        try:
            ...
            raise RuntimeError('|Long Attribute Name Getter| >> Attribute does nost exists: {}'.format(attr_dict['combined']))
    ...
    def set_hidden(node, attr=None, arg=None):
        ...
        if arg:
            if children:
                ...
                if not MetaAttributeUtils.is_hidden(attr_dict):
                    if MetaAttributeUtils.is_keyed(attr_dict):
    ...
    def set_keyable(node, attr=None, arg=None):
        ...
        if children:
            ...
            hidden = arg or MetaAttributeUtils.is_hidden(attr_dict)
    ...
    def is_numeric(*args):
        ...
        """
        attr_dict = (MetaAttributeUtils.validate_attribute)(*args)
        if maya.cmds.getAttr((attr_dict['combined']), type=True) in ('string', 'message',
                                                                     'enum', 'bool'):
            return False
        else:
            return True
    ...
    def get_default(*args):
        ...
        if not MetaAttributeUtils.is_dynamic(attr_dict):
            ...
            if long_attr in ('translateX', 'translateY', 'translateZ', 'translate',
                             'rotateX', 'rotateY', 'rotateZ', 'rotate', 'scaleX',
                             'scaleY', 'scaleZ', 'scale'):
    ...
    def get_message(message_holder, message_attr=None, data_attr=None, data_key=None, simple=False):
        ...
        if dict_type in ('string', ):
            ...
            if msg_buffer:
                if maya.cmds.objectType(msg_buffer[0]) == 'reference':
                    ...
                    if maya.cmds.objExists(msg_buffer[0]) and not maya.cmds.objectType(msg_buffer[0]) == 'reference':
                        ...
                        if meta_node.has_attr(attr_dict_data['attr']):
                            ...
                            if dict_buffer.get(data_key):
                                ...
                                return [
                                 msg_buffer[0] + '.' + dict_buffer.get(data_key)]
                        return msg_buffer
                    else:
    ...
    def set_message(message_holder, message_attr, message, data_attr=None, data_key=None, simple=False, connect_back=None):
        ...
        try:
            ...
            if issubclass(type(message), list) or multi:
                ...
                if len(message) > 1 or multi:
                    if maya.cmds.objExists(combined):
                        ...
                        maya.cmds.addAttr(combined, query=True, m=True) or LOGGER.warning('|Message Setter| >> Not a multi message attribute. Converting ...')
                        MetaAttributeUtils.delete(attr_dict)
                        MetaAttributeUtils.add(message_holder, message_attr, 'message', m=True, im=False)
                        store_message_multi(message, attr_dict)
                        return True
                        ...
                        if _buffer:
                            if [name_utils.get_long_name(m) for m in _buffer] == message_long:
            ...
            if '.' in message:
                ...
                if MetaAttributeValidator.is_shape(message):
                    mode = 'shape'
                ...
                if messaged_extra:
                    ...
                    if maya.cmds.objExists(combined):
                        ...
                        maya.cmds.addAttr(combined, query=True, m=True) or LOGGER.debug('|Message Setter| >> MessageSimple')
                        if _buffer:
                            if name_utils.get_long_name(_buffer[0]) == message_long:
                                LOGGER.debug('|Message Setter| >> Message match. Good to go')
                                return True
                        MetaAttributeUtils.break_connection(attr_dict)
                        store_message(messaged_node, messaged_extra, attr_dict, dict_data_attr, data_key)
    ...
    def data_list_exists(node=None, attr=None, mode=None, data_attr=None):
        ...
        for i, k in enumerate(attrs_dict.keys()):
            ...
            if mode == 'message':
                ...
                if MetaAttributeUtils.get(node, str_attr) is not None:
                    return True
...
class MetaMessageListUtils(object):
    ...
    def message_list_index(node=None, attr=None, data=None, data_attr=None):
        return MetaDataListUtils.data_list_index(node=node, attr=attr, data=(MetaAttributeValidator.meta_node_string(data)),
          mode='message',
          data_attr=data_attr)
    ...
    def message_list_remove(node=None, attr=None, data=None, data_attr=None):
        return MetaDataListUtils.data_list_remove(node=node, attr=attr, data=(MetaAttributeValidator.meta_node_string(data)),
          mode='message',
          data_attr=data_attr)
