...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/geometry.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 21949 bytes
"""
...
def transforms_to_nurbs_surface(transforms, name, spans=-1, offset_axis='Y', offset_amount=1):
    ...
    for xform in transforms:
        ...
        maya.cmds.move((vct[0] * offset_amount),
          (vct[1] * offset_amount), (vct[2] * offset_amount), xform_1, relative=True, os=True)
        maya.cmds.move((vct[0] * -offset_amount),
          (vct[1] * -offset_amount), (vct[2] * -offset_amount), xform_2, relative=True, os=True)
    ...
    if not spans == -1:
        for crv in crvs:
            maya.cmds.rebuildCurve(crv,
              ch=False, rpo=True, rt=0, end=1, kr=False, kcp=False, kep=True, kt=False, spans=spans,
              degree=3,
              tol=0.01)
