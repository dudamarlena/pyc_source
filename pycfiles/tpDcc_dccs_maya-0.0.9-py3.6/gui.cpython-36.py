...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/gui.py
...
import logging, functools, traceback, contextlib
from collections import OrderedDict
from Qt.QtWidgets import *
import tpDcc, tpDcc.dccs.maya as maya
...
class DockWrapper(object):
    ...
    def create(self):
        ...
        if self._exists():
            ...
            maya.cmds.dockControl((self._dock_name), aa=(self._allowed_areas), a=(self._dock_area), content=(self._name), label=(self._label),
              fl=floating,
              visible=True,
              fcc=(self._floating_changed))
...
def get_script_editor(source_type='python', command_completion=False, show_tooltip_help=False):
    ...
    executer = maya.cmds.cmdScrollFieldExecuter(sourceType=source_type,
      commandCompletion=command_completion,
      showTooltipHelp=show_tooltip_help)
...
def force_stack_trace_on():
    ...
    try:
        ...
        if what_is != 'Unknown':
            ...
            if last_focused_command_reporter:
                if last_focused_command_reporter != '':
...
def get_time_slider_range(highlighted=True, within_highlighted=True, highlighted_only=False):
    ...
    if not highlighted_only:
        return [maya.cmds.playbackOptions(query=True,
           minTime=True), maya.cmds.playbackOptions(query=True, maxTime=True)]
...
def create_independent_panel(width, height, off_screen=False):
    ...
    window = maya.cmds.window(width=width,
      height=height,
...
def add_attribute_widget(layout, layout_parent, lbl, attr=None, attr_type='cbx', size=[10, 60, 40, 80], attr_changed_fn=None):
    ...
    try:
        if attr_type == 'cbx':
            ...
            if attr_type == 'color':
                ui_item = maya.cmds.attrColorSliderGrp(label=lbl,
                  attribute=attr,
                  cl4=['left', 'left', 'left', 'left'],
                  cw4=[10, 15, 50, 80])
            else:
                if attr_type == 'floatSlider':
                    ui_item = maya.cmds.attrFieldSliderGrp(label=lbl,
                      attribute=attr,
                      cl4=['left', 'left', 'left', 'left'],
                      cw4=size,
                      pre=2)
...
def delete_workspace_control(control_name, reset_floating=True):
    ...
    if not tpDcc.Dcc.get_version() > 2017:
        ...
        if maya.cmds.workspaceControl(control_name, query=True, exists=True):
            ...
            floating = None
        if maya.cmds.workspaceControlState(control_name,
          query=True, exists=True):
            if not (floating or floating and reset_floating):
