...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/namespace.py
# Compiled at: 2020-05-02 23:38:23
# Size of source mod 2**32: 14673 bytes
"""
Module that contains functions and classes related with namespaces
"""
import types, tpDcc.dccs.maya as maya
...
def get_namespace(name, check_obj=True, top_only=True):
    ...
    if check_obj:
        ...
        namespace = ''
        return name.count(':') or namespace
    else:
        if top_only:
...
def assign_namespace_to_object_by_filter(namespace, filter_type, force_create=True, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return assign_namespace_to_object(filtered_obj_list,
          namespace=namespace, force_create=force_create, rename_shape=rename_shape)


def remove_namespace_from_object_by_filter(namespace, filter_type, rename_shape=True, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
    ...
    if not filtered_obj_list:
        ...
        return remove_namespace_from_object(filtered_obj_list,
          namespace=namespace, rename_shape=rename_shape)