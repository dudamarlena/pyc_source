...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/calibration_client/calibration_client.py
# Compiled at: 2019-08-05 18:01:11
# Size of source mod 2**32: 22155 bytes
__doc__ = 'CalibrationClient class'
import logging
...
class CalibrationClient(Calibration, CalibrationConstant, CalibrationConstantVersion, Condition, Parameter, DeviceType, PhysicalDevice):
    ...
    @staticmethod
    def set_calibration_constant_version(cal_client, calibration_constant_id, ccv_h):
        ...
        ccv_flg_deployed = Util.get_opt_hash_val(ccv_h, 'flg_deployed', def_val='true')
    ...
    def set_condition_from_dict(cal_client, detector_condition):
        cal_cond = CalibrationClient.get_condition_from_dict(cal_client, detector_condition)
    ...
    def search_condition_from_dict(cal_client, event_at, detector_condition):
        cal_cond = CalibrationClient.get_condition_from_dict(cal_client, detector_condition)
        cond = Condition(cal_client, cal_cond['name'], cal_cond['flg_available'], event_at, cal_cond['parameters_conditions_attributes'], cal_cond['description'])
    ...
    @staticmethod
    def search_possible_conditions_from_dict(cal_client, event_at, detector_condition):
        cal_cond = CalibrationClient.get_condition_from_dict(cal_client, detector_condition)
        cond = Condition(cal_client, cal_cond['name'], cal_cond['flg_available'], event_at, cal_cond['parameters_conditions_attributes'], cal_cond['description'])
        resp = cond.get_possible()
        return resp
    ...
    def get_condition_from_dict(cal_client, detector_condition):
        ...
        for param in detector_condition['parameters']:
            ...
            parameter_condition = {'parameter_id':str(param_id), 
        ...
        condition_name = Util.get_opt_hash_val(detector_condition, 'name', def_val=def_cond_name)
        condition_flg_avail = Util.get_opt_hash_val(detector_condition, 'flg_available',
          def_val='true')
        condition_desc = Util.get_opt_hash_val(detector_condition, 'description')
        cal_cond = {'name':condition_name, 
    ...
    def inject_new_calibration_constant_version(cal_client, inject_h):
        ...
        resp = CalibrationClient.set_condition_from_dict(cal_client, inj_detector_condition)
        if resp['success']:
            condition = resp['data']
            condition_id = condition['id']
            ...
            return resp
            resp = CalibrationClient.set_calibration_constant(cal_client, condition, inj_cal_const)
            if resp['success']:
                ...
                logging.error(error_msg)
                return resp
            resp = CalibrationClient.set_calibration_constant_version(cal_client, cc_id, inj_cal_const_version)
            if resp['success']:
    ...
    def retrieve_calibration_constant_version(cal_client, retrieve_h):
        ...
        resp = CalibrationClient.search_possible_conditions_from_dict(cal_client, event_at, ret_detector_condition)
        if resp['success']:
            ...
            if resp['success']:
                ...
                if resp['success']:
                    ...
                    logging.error(error_msg)
                    return resp
                resp = CalibrationConstant.get_all_by_conditions(cal_client, calibration_id, device_type_id, condition_ids)
                if resp['success']:
            ...
            resp = CalibrationConstantVersion.get_closest_by_time(cal_client, cc_ids, device_id, event_at, snapshot_at)
            if resp['success']:
    ...
    def retrieve_all_calibration_constant_versions(cal_client, retrieve_h):
        ...
        if resp['success']:
            ...
            logging.debug('* device_type_id == {0}'.format(device_type_id))
            logging.debug('* calibration_constant_ids == {0}'.format(cc_ids))
            resp = CalibrationConstantVersion.get_all_versions(cal_client, cc_ids, device_id, event_at, snapshot_at)
            if resp['success']:
    ...
    def update_calibration_constant_version(cal_client, update_h):
        ...
        if resp['success']:
            ...
            logging.error(error_msg)
            return resp
            ccv_pars = {k:v for k, v in ccv.items() if not isinstance(v, dict) if k != 'id' if k != 'id'}
            ccv_pars['calibration_constant_id'] = ccv['calibration_constant']['id']
            ccv_pars['physical_device_id'] = ccv['physical_device']['id']
            updatable = [
             'description', 'end_idx', 'start_idx', 'begin_at',
            ...
            new_ccv = CalibrationConstantVersion(calibration_client=cal_client, **ccv_pars)
            new_ccv.id = ccv_id
            resp = new_ccv.update()
            if resp['success']:
