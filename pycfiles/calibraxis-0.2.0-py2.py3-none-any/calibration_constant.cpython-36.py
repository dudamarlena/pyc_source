...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/calibration_client/modules/calibration_constant.py
# Compiled at: 2019-08-05 17:28:52
# Size of source mod 2**32: 5805 bytes
__doc__ = 'CalibrationConstant module class'
import logging
...
class CalibrationConstant(CalibrationConstantApi):

    def __init__(self, calibration_client, name, calibration_id, device_type_id, condition_id, flg_available, flg_auto_approve, description=''):
    ...
    def create(self):
        ...
        response = cal_client.create_calibration_constant_api(self._CalibrationConstant__get_resource())
    ...
    def update(self):
        cal_client = self.calibration_client
        response = cal_client.update_calibration_constant_api(self.id, self._CalibrationConstant__get_resource())
    ...
    def create_from_dict(cal_client, cc):
        new_cc = CalibrationConstant(calibration_client=cal_client, name=(cc['name']),
          calibration_id=(cc['calibration_id']),
          device_type_id=(cc['device_type_id']),
    ...
    def set_from_dict(cal_client, cc):
        resp = CalibrationConstant.get_by_uk(cal_client, cc['calibration_id'], cc['device_type_id'], cc['condition_id'])
        if resp['success']:
    ...
    def get_by_id(cal_client, calibration_constant_id):
        response = cal_client.get_calibration_constant_by_id_api(calibration_constant_id)
    ...
    @staticmethod
    def get_all_by_conditions(cal_client, calibration_id, device_type_id, condition_ids):
        res = cal_client.get_all_calibration_constants_by_conds_api(calibration_id, device_type_id, condition_ids)
        Base.cal_debug(MODULE_NAME, 'cc >> get_all_calibration_constants_by_conds_api', res)
        return Base.format_response(res, GET, OK, MODULE_NAME)

    @staticmethod
    def get_by_uk(cal_client, calibration_id, device_type_id, condition_id):
        res = cal_client.get_calibration_constant_by_uk_api(calibration_id, device_type_id, condition_id)
        Base.cal_debug(MODULE_NAME, 'cc >> get_by_uk', res)
        return Base.format_response(res, GET, OK, MODULE_NAME)

    def __get_resource(self):
        calibration_constant = {CALIBRATION_CONSTANT: {'name':self.name, 
