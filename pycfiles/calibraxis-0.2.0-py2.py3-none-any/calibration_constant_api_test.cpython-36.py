...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/calibration_client/tests/apis/calibration_constant_api_test.py
# Compiled at: 2019-08-05 17:28:52
# Size of source mod 2**32: 10497 bytes
__doc__ = 'CalibrationConstantApiTest class'
import unittest
...
class CalibrationConstantApiTest(ApiBase, unittest.TestCase):
    ...
    db_cal_const_1 = {'calibration_constant': {'name':'CALIBRATION_CONSTANT_TEST-1_DO_NOT_DELETE', 
                              'calibration_id':'-1', 
                              'device_type_id':'-1', 
                              'condition_id':'-1', 
                              'description':'Created automatically via seed:seed_tests'}}
    db_cal_const_2 = {'calibration_constant': {'name':'CALIBRATION_CONSTANT_TEST-2_DO_NOT_DELETE', 
                              'calibration_id':'-1', 
                              'device_type_id':'-1', 
                              'condition_id':'-2', 
                              'description':'Created automatically via seed:seed_tests'}}
    ...
    def test_create_calibration_constant_api(self):
        _CalibrationConstantApiTest__unique_name = Generators.generate_unique_name('CConstantApi')
        calibration_const = {'calibration_constant': {'name':_CalibrationConstantApiTest__unique_name, 
                                  'calibration_id':'-2', 
                                  'device_type_id':'-1', 
                                  'condition_id':'-1', 
        ...
        expect_db_all = [
         self.db_cal_const_1['calibration_constant'],
         self.db_cal_const_2['calibration_constant']]
        received = self._CalibrationConstantApiTest__create_entry_api(calibration_const, expect)
        cal_constant_id = received['id']
        cal_constant_name = received['name']
        self._CalibrationConstantApiTest__create_error_entry_uk_api(calibration_const)
        condition_ids = [
         self.db_cal_const_1['calibration_constant']['condition_id'],
         self.db_cal_const_2['calibration_constant']['condition_id']]
        self._CalibrationConstantApiTest__get_all_calibration_constants_by_conditions_api(self.db_cal_const_1['calibration_constant']['calibration_id'], self.db_cal_const_1['calibration_constant']['device_type_id'], condition_ids, expect_db_all)
        self._CalibrationConstantApiTest__get_entry_by_uk_api(calibration_const, expect)
        self._CalibrationConstantApiTest__get_entry_by_uk_api_not_found_api()
        self._CalibrationConstantApiTest__get_all_entries_by_name_api(cal_constant_name, expect)
        self._CalibrationConstantApiTest__get_entry_by_id_api(cal_constant_id, expect)
        self._CalibrationConstantApiTest__update_entry_api(cal_constant_id, expect)
        self._CalibrationConstantApiTest__delete_entry_by_id_api(cal_constant_id)
    ...
    def __create_entry_api(self, entry_info, expect):
        response = self.cal_client_api.create_calibration_constant_api(entry_info)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        calibration_const_upd = {'calibration_constant': {'name':unique_name, 
                                  'flg_auto_approve':'false', 
                                  'flg_available':'false', 
                                  'description':'desc 01 updated!'}}
        resp = self.cal_client_api.update_calibration_constant_api(entry_id, calibration_const_upd)
    ...
    def __get_all_entries_by_name_api(self, name, expect):
        resp = self.cal_client_api.get_all_calibration_constants_by_name_api(name)
    ...
    def __get_entry_by_uk_api(self, entry, expect):
        resp = self.cal_client_api.get_calibration_constant_by_uk_api(entry['calibration_constant']['calibration_id'], entry['calibration_constant']['device_type_id'], entry['calibration_constant']['condition_id'])
        resp_content = self.load_response_content(resp)
        receive = resp_content
        self.fields_validation(receive, expect)
        self.assert_eq_status_code(resp.status_code, OK)
    ...
    def __get_entry_by_uk_api_not_found_api(self):
        ...
        dev_type_id = -99
        cond_id = -99
        resp = self.cal_client_api.get_calibration_constant_by_uk_api(calibration_id, dev_type_id, cond_id)
        self.get_and_validate_resource_not_found(resp)

    def __get_all_calibration_constants_by_conditions_api(self, calibration_id, device_type_id, condition_ids, expect):
        resp = self.cal_client_api.get_all_calibration_constants_by_conds_api(calibration_id, device_type_id, condition_ids)
        resp_content = self.load_response_content(resp)
        receive = resp_content
        self.assertEqual(len(receive), len(receive), 'Array size should be 2')
        self.assertEqual(receive[0]['name'], self.db_cal_const_2['calibration_constant']['name'], 'Name of first calibration constant should match')
        self.assertEqual(receive[1]['name'], self.db_cal_const_1['calibration_constant']['name'], 'Name of first calibration constant should match')
        self.assert_eq_status_code(resp.status_code, OK)
