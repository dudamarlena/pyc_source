...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/calibration_client/tests/apis/calibration_constant_version_api_test.py
# Compiled at: 2019-08-05 17:28:52
# Size of source mod 2**32: 10590 bytes
__doc__ = 'CalibrationConstantVersionApiTest class'
import unittest
...
class CalibrationConstantVersionApiTest(ApiBase, unittest.TestCase):
    ...
    _CalibrationConstantVersionApiTest__unique_name = Generators.generate_unique_name('CCVersionApi')
    _CalibrationConstantVersionApiTest__unique_file_name = Generators.generate_unique_file_name()
    _CalibrationConstantVersionApiTest__begin_at = Generators.generate_timestamp_str(0)
    _CalibrationConstantVersionApiTest__calibration_constant_version_01 = {'calibration_constant_version': {'name':_CalibrationConstantVersionApiTest__unique_name, 
                                      'file_name':_CalibrationConstantVersionApiTest__unique_file_name, 
                                      ...
                                      'begin_at':_CalibrationConstantVersionApiTest__begin_at, 
                                      'start_idx':'1', 
                                      'end_idx':'2', 
    ...
    def test_create_calibration_constant_version_api(self):
        ccv = self._CalibrationConstantVersionApiTest__calibration_constant_version_01
        expect = ccv['calibration_constant_version']
        received = self._CalibrationConstantVersionApiTest__create_entry_api(ccv, expect)
        ccv_id = received['id']
        ccv_name = received['name']
        self._CalibrationConstantVersionApiTest__create_error_entry_uk_api(ccv)
        self._CalibrationConstantVersionApiTest__test_get_version_int((expect['begin_at']), expect_ccv_id=ccv_id)
        self._CalibrationConstantVersionApiTest__get_all_entries_by_name_api(ccv_name, expect)
        self._CalibrationConstantVersionApiTest__get_entry_by_id_api(ccv_id, expect)
        self._CalibrationConstantVersionApiTest__update_entry_api(ccv_id, expect)
    ...
    def __create_entry_api(self, entry_info, expect):
        response = self.cal_client_api.create_calibration_constant_version_api(entry_info)
    ...
    def __create_error_entry_uk_api(self, entry_info):
        ...
        expect = {'info': {'name':['has already been taken'],  'calibration_constant_id':[
    ...
    def __test_get_version_int(self, event_at, expect_ccv_id=None):
        ...
        snapshot_at = ''
        resp = self.cal_client_api.get_calibration_constant_version_by_uk_api(calibration_constant_id, physical_device_id, event_at, snapshot_at)
    ...
    def __get_all_entries_by_name_api(self, name, expect):
        resp = self.cal_client_api.get_all_calibration_constant_versions_by_name_api(name)
        receive = self.get_and_validate_all_entries_by_name(resp)
        self.fields_validation(receive, expect)

    def __get_entry_by_id_api(self, entry_id, expect):
        resp = self.cal_client_api.get_calibration_constant_version_by_id_api(entry_id)
        receive = self.get_and_validate_entry_by_id(resp)
        self.fields_validation(receive, expect)

    def __update_entry_api(self, entry_id, expect):
        _CalibrationConstantVersionApiTest__unique_name = Generators.generate_unique_name('CCVersionApiUpd')
        _CalibrationConstantVersionApiTest__unique_file_name = Generators.generate_unique_file_name()
        _CalibrationConstantVersionApiTest__begin_at = Generators.generate_timestamp_str(1)
        ccv_upd = {'calibration_constant_version': {'name':_CalibrationConstantVersionApiTest__unique_name, 
                                          'file_name':_CalibrationConstantVersionApiTest__unique_file_name, 
        ...
        response = self.cal_client_api.update_calibration_constant_version_api(entry_id, ccv_upd)
        resp_content = self.load_response_content(response)
        receive = resp_content
