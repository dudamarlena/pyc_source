...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bfnet/BFHandler.py
# Compiled at: 2015-12-07 14:44:43
# Size of source mod 2**32: 10046 bytes
import _ssl, asyncio, logging, multiprocessing, ssl, sys, types
...
class ButterflyHandler(object):
    ...
    def on_connection(self, butterfly: Butterfly):
        ...
        self.butterflies['{}:{}'.format(butterfly.ip, butterfly.client_port)] = (
         butterfly, handler)
    ...
    @classmethod
    def get_handler(cls, loop: asyncio.AbstractEventLoop, ssl_context: ssl.SSLContext=None, log_level: int=logging.INFO,
                    buffer_size: int=asyncio.streams._DEFAULT_LIMIT):
        """
    ...
    def create_server(self, bind_options: tuple, ssl_options: tuple) -> Net:
        ...
        self._server = yield from self._event_loop.create_server(self.butterfly_factory, host=host, port=port, ssl=self._ssl)
