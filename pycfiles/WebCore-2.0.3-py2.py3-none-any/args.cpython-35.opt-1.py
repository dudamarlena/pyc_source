...
class ValidateArgumentsExtension(object):
    ...
    def _mutate(self, context, endpoint, args, kw):
        try:
            ...
            log.error(str(e).replace(endpoint.__name__, safe_name(endpoint)), extra=dict(request=id(context), endpoint=safe_name(endpoint), endpoint_args=args, endpoint_kw=kw))
