...
class ACLExtension(object):
    ...
    def mutate(self, context, handler, args, kw):
        ...
        if grant.result is False:
            log.error('Endpoint authorization failed: ' + repr(grant), extra=dict(grant=False, predicate=repr(grant.predicate) if grant.predicate else None, path=str(grant.path) if grant.path else None, source=safe_name(grant.source) if grant.source else None))
            raise HTTPForbidden()
    ...
    def transform(self, context, handler, result):
        ...
        if valid.result is False:
            log.error('Response rejected due to return value authorization failure.', extra=dict(grant=False, predicate=repr(valid.predicate) if valid.predicate else None, path=str(valid.path) if valid.path else None, result=safe_name(type(result))))
