...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/bitme/bitme.py
# Compiled at: 2013-10-27 21:29:54
...
class BitmeAPI:
    ...
    def _query(self, url, params={}, auth=False, method='GET'):
        ...
        """
        nonce = bitme_internal.datetime_to_timestamp(datetime.now()) * 1000 + 1000000000
        params['nonce'] = nonce
        params = OrderedDict(params)
        p_encoded = urlencode(params)
        mac = base64.b64encode(hmac.new(self.apisecret, p_encoded, hashlib.sha512).digest())
        headers = {'Rest-Key': self.apikey, 'Rest-Sign': mac}
        if method == 'GET':
            r = requests.get(API_URLROOT + url, headers=headers, params=params, verify=True)
        elif method == 'POST':
            logging.debug('POSTDATA: ' + str(params))
            r = requests.post(API_URLROOT + url, headers=headers, data=params, verify=True)
        else:
    ...
    def create_order(self, order_type_cd, quantity, rate, currency_pair='LTCBTC'):
        """
        Create a new order
        ...
        """
        params = {'order_type_cd': order_type_cd, 'quantity': quantity, 
           'currency_pair': currency_pair, 
           'rate': rate}
        return self._query('/order/create', params=params, method='POST', auth=True)
    ...
    def cancel_order(self, uuid):
        ...
        """
        return self._query('/order/cancel', params={'uuid': uuid}, method='POST', auth=True)
    ...
    def get_transactions(self, currency_cd='BTC', limit=10, order_by='DESC', page='1'):
        """
        ...
        """
        return self._query('/transactions/%s' % (currency_cd,), params={'limit': limit, 'orderBy': order_by, 
           'page': page}, auth=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--apikeyfile', default='APIKEY', help='The file containing the API key (not the secret!)')
    parser.add_argument('--apisecretfile', help='The file containing the api secret.  if unspecified, access to privileged api calls will not be possible.')
    parser.add_argument('--loglevel', default='INFO', help='Set log level [INFO, DEBUG, etc.]')
