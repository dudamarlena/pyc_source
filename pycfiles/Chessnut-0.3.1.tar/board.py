...
# Embedded file name: build/bdist.linux-i686/egg/chessmind/core/board.py
# Compiled at: 2008-04-25 20:07:04
...
class ChessBoard(Board):
    ...
    def __getitem__(self, square):
        ...
        PositionException: ...
        """
        if type(square) in (type('A1'), type('A1')):
        ...
        try:
            ...
            raise position.PositionException, 'Position (%s, %s) out of range.' % (row, col)
    ...
    def move(self, start, target, unconditional=False, promotion=False):
        ...
        if state == CASTLING:
            for castle in CASTLE_PAIRS:
                if castle['colour'] == mover.colour and castle['king_end'] == target:
        ...
        for row in range(self.nrows):
            for col in range(self.ncols):
                if self[(row, col)].isEnemy(king):
                    ...
                    if square in moves:
                        return True
                    special = [ sq for (sq, ep) in self[attacker].attackMoves(attacker) ]
    ...
    def freePath(self, start, target):
        ...
        if startcol == targetcol:
            e = 'Start (%s) and target (%s) square are the same' % (start, target)
    ...
    def castlingMoves(self, start, unconditional=False):
        ...
        if not king.can_castle:
            return moves
        for castle in CASTLE_PAIRS:
            if castle['colour'] == king.colour and castle['king_start'] == start:
    ...
    def castlingAllowed(self, castle, unconditional=False):
        ...
        if not (rook.isRook() and rook.can_castle and self.freePath(king_start, rook_start)):
            return False
        if unconditional:
            return True
    ...
    def specialAttackMoves(self, start, unconditional=False):
        ...
        for (move, en_passant) in thisPiece.attackMoves(start):
            ...
            elif thisPiece.isEnemy(self[en_passant]) and self[en_passant].isEnPassantTarget():
    ...
    def validateMove(self, mover, start, target, unconditional=False):
        ...
        if target not in self.calcMovesForPiece(start, unconditional):
            for (end, ep) in self.specialAttackMoves(start, unconditional):
                ...
                raise position.ImpossibleMove, 'Position %s cannot be reached by %s at %s.' % (target, mover.name, start)
        return instructions
    ...
    def findPiece(self, symbol, colour):
        ...
        for row in range(self.nrows):
            for col in range(self.ncols):
                if self[(row, col)].symbol == symbol and self[(row, col)].colour == colour:
            ...
            for col in range(self.ncols):
                if self[(row, col)].colour == self.player:
                    ...
                    for move in moves:
                        ...
                        if depth == 0:
                            value = board.value()
                        options.append({'start': friend, 'target': move, 'value': value})
