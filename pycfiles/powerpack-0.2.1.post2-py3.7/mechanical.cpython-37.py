...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/powerpack/mechanical.py
# Compiled at: 2020-05-10 14:01:09
# Size of source mod 2**32: 124152 bytes
"""
...
import numpy as npy, networkx as nx, volmdlr as vm
...
class PackStructure(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'size', 'cases', 'long_rails', 'trans_rails', 'rail_specs',
      'th_casing', 'triangle', 'radius'], 
     'properties':{'size':{'type':'array', 
       'title':'Size', 
       'order':1, 
       'items':{'type':'number', 
        'editable':True, 
      ...
      'cases':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.mechanical.Case']}, 
       'editable':True, 
       'description':'List of cases'}, 
      'long_rails':{'type':'array', 
       'title':'Longitudinal Rails', 
       'order':3, 
       'items':{'type':'object', 
        'editable':True, 
        'classes':[
         'powerpack.mechanical.LongitudinalRail']}, 
       'editable':True, 
       'description':'List of longitudinal rails'}, 
      'trans_rails':{'type':'array', 
       'title':'Transversal Rails', 
       'order':4, 
       'items':{'type':'object', 
        'editable':True, 
        'classes':[
         'powerpack.mechanical.TransversalRail']}, 
       'editable':True, 
       'description':'List of transversal rails'}, 
      'rail_specs':{'type':'object', 
       'title':'Rails Specifications', 
       'editable':True, 
       'description':'Specifications for rails', 
       'order':5, 
       'properties':{'longitudinal':{'type':'number', 
         'editable':True, 
         'step':0.001, 
         'minimum':0, 
         'example':[
    ...
    def __init__(self, size, cases, long_rails, trans_rails, rail_specs, th_casing=0.005, triangle=(0, 0), radius=0.01, name=''):
        self.size = size
        self.cases = cases
    ...
    def __eq__(self, other_ps):
        if len(self.cases) == len(other_ps.cases):
            if len(self.long_rails) == len(other_ps.long_rails):
                if len(self.trans_rails) == len(other_ps.trans_rails):
                    equal = npy.allclose(self.size, other_ps.size) and all([elt == other_elt for elt, other_elt in zip(self.cases + self.long_rails + self.trans_rails, other_ps.cases + other_ps.long_rails + other_ps.trans_rails)])
                    return equal
        return False

    def __hash__(self):
        elements_hash = sum([hash(elt) for elt in self.cases + self.long_rails + self.trans_rails])
    ...
    def RailsFromCases(self, cases, long_rails, trans_rails, rail_specs):
        ...
        for case_index, case in enumerate(cases):
            ...
            if case is not None:
                if i:
                    if case.position[1] < long_rails[(i - 1)].position + lr_width / 2.0:
                        case.position[1] = long_rails[(i - 1)].position + lr_width / 2.0
                        ...
                        if sym_case_index is not None:
                            ...
                            if sym_i == 0:
                                ...
                                sym_case.position[1] = -(long_rails[(i - 1)].position + lr_width / 2.0 + sym_case.size[1])
                if j:
                    if case.position[0] < trans_rails[(j - 1)].position + tr_width / 2.0:
                        case.position[0] = trans_rails[(j - 1)].position + tr_width / 2.0
            ...
            if i < n_lr:
                if i not in fixed_lr_indices:
                    rail = LongitudinalRail(current_y + lr_width / 2.0, self.size[0], lr_width, self.size[2])
                    long_rails[i] = rail
                    fixed_lr_indices.append(i)
                    current_y += rail.width
                if i == n_lr and j < n_tr and j not in fixed_tr_indices:
                    rail = TransversalRail(max_x + tr_width / 2.0, self.size[1], tr_width, self.size[2])
                    trans_rails[j] = rail
    ...
    def CheckViability(self, longitudinal_tolerances, transversal_tolerances, max_pack_size):
        ...
        if j_ref and j_ref != j_last:
            ...
            x_ref = later_rail.position - later_rail.width / 2.0 - (prior_rail.position + prior_rail.width / 2.0)
        else:
            if j_ref == j_last:
                prior_rail = self.trans_rails[(j_ref - 1)]
                x_ref = max_pack_size[0] - (prior_rail.position + prior_rail.width / 2.0)
            else:
                later_rail = self.trans_rails[j_ref]
                x_ref = later_rail.position - later_rail.width / 2.0
        if i_ref and i_ref != i_last:
            prior_rail = self.long_rails[(i_ref - 1)]
            later_rail = self.long_rails[i_ref]
            y_ref = later_rail.position - later_rail.width / 2.0 - (prior_rail.position + prior_rail.width / 2.0)
        else:
            if i_ref == i_last:
                prior_rail = self.long_rails[(i_ref - 1)]
                y_ref = max_pack_size[1] - (prior_rail.position + prior_rail.width / 2.0)
            else:
                later_rail = self.long_rails[i_ref]
                y_ref = later_rail.position - later_rail.width / 2.0
        cases_ref = []
        ...
        for case_index, case in enumerate(self.cases):
            ...
            if case is not None:
                if case.position[0] + case.size[0] > max_pack_size[0] or case.position[1] + case.size[1] > max_pack_size[1] / 2.0 or case.position[2] + case.size[2] > max_pack_size[2]:
                    return False
                if complete_column_ref:
                    longitudinal_tolerance = longitudinal_tolerances[j]
                    transversal_tolerance = transversal_tolerances[i]
                    if not (case.size[0] >= x_ref * longitudinal_tolerance['minus'] and case.size[0] <= x_ref * longitudinal_tolerance['plus'] and case.size[1] <= y_ref * transversal_tolerance['plus']):
                        return False
    ...
    def LowerCasingVolume(self, fem=False):
        ...
        if fem:
            p1 = vm.Point2D((0, triangle_y - self.size[1] / 2.0))
            p2 = vm.Point2D((0, self.size[1] / 2.0 - triangle_y))
            p3 = vm.Point2D((triangle_x, self.size[1] / 2.0))
            p4 = vm.Point2D((self.size[0] - triangle_x, self.size[1] / 2.0))
            p5 = vm.Point2D((self.size[0], self.size[1] / 2.0 - triangle_y))
            p6 = vm.Point2D((self.size[0], triangle_y - self.size[1] / 2.0))
            p7 = vm.Point2D((self.size[0] - triangle_x, -self.size[1] / 2.0))
            p8 = vm.Point2D((triangle_x, -self.size[1] / 2.0))
        ...
        sides = primitives3D.ExtrudedProfile(vm.O3D, vm.X3D, vm.Y3D, outer_primitive, [inner_primitive], self.size[2] * vm.Z3D, 'sides')
        bottom = primitives3D.ExtrudedProfile(vm.O3D, vm.X3D, vm.Y3D, outer_primitive, [], -self.th_casing * vm.Z3D, 'bottom')
        screw_holes_contour = inner_primitive.Offset(-(self.th_casing + screw_holes_clearance + 0.5 * screw_holes_diameter))
        belt_outer_contour = inner_primitive.Offset(-(2 * screw_holes_clearance + screw_holes_diameter + self.th_casing))
        if fem:
            belt = primitives3D.ExtrudedProfile(vm.Z3D * self.size[2], vm.X3D, vm.Y3D, belt_outer_contour, [
             inner_contour], self.th_casing * vm.Z3D, 'belt')
        else:
            screw_holes = []
            ...
            belt = primitives3D.ExtrudedProfile(vm.Z3D * self.size[2], vm.X3D, vm.Y3D, belt_outer_contour, [
             inner_primitive] + screw_holes, self.th_casing * vm.Z3D, 'belt')
    ...
    def Dict(self):
        ...
        dict_.update({'cases':[case.Dict() for case in self.cases],  'long_rails':[rail.Dict() for rail in self.long_rails], 
    ...
    def DictToObject(cls, dict_):
        ...
        long_rails = [LongitudinalRail.DictToObject(rail_dict) for rail_dict in dict_['long_rails']]
        trans_rails = [TransversalRail.DictToObject(rail_dict) for rail_dict in dict_['trans_rails']]
        rail_specs = dict_['rail_specs']
        th_casing = dict_['th_casing']
        triangle = dict_['triangle']
        radius = dict_['radius']
        name = dict_['name']
        pack_structure = cls(size=size, cases=cases, long_rails=long_rails,
          trans_rails=trans_rails,
...
class JunctionBox(DessiaObject):
    ...
    def CADVolumes(self):
        ...
        volume = primitives3D.ExtrudedProfile(origin, xp, zp, contour, [], yp)
        return [volume]
    ...
    @classmethod
    def DictToObject(cls, dict_):
        return cls(position=(dict_['position']), length=(dict_['length']), width=(dict_['width']),
          height=(dict_['height']),
          name=(dict_['name']))
...
class MechBattery(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'mech_module', 'elec_battery', 'pack_structure', 'module_gap'], 
     'properties':{'mech_module':{'type':'object', 
       'title':'Mechanical Module', 
       'order':1, 
       'classes':[
        'powerpack.mechanical.MechModule'], 
       'editable':True, 
       'description':'Module'}, 
      'elec_battery':{'type':'object', 
      ...
      'pack_structure':{'type':'object', 
       'title':'Pack Structure', 
       'order':3, 
       'classes':[
        'powerpack.mechanical.PackStructure'], 
       'editable':True, 
       'description':'Pack structure'}, 
      'module_gap':{'type':'number', 
       'title':'Module Gap', 
       'order':4, 
       'step':0.001, 
       'minimum':0, 
    ...
    def __init__(self, mech_module, elec_battery, pack_structure, module_gap, hydraulic_nodes=[], hydraulic_connections=[], wiring=None, hydraulic_circuit=None, thermal_circuit=None, name=''):
    ...
    def __eq__(self, other_mb):
        equal = self.mech_module == other_mb.mech_module and self.pack_structure == other_mb.pack_structure and self.module_gap == other_mb.module_gap and self.elec_battery == other_mb.elec_battery
        return equal

    def __hash__(self):
        mb_hash = hash(self.mech_module) + hash(self.pack_structure) + int(1500 * self.module_gap) + hash(self.elec_battery)
        return mb_hash
    ...
    def CoolingPlatesSocketsPositions(self):
        ...
        cooling_plates_sockets_positions = {c:c.SocketsPositions(f) for c, f in cooling_plates_frames.items()}
    ...
    def WiringOptimizer(self, wire_diameter=0.008):
        ...
        y = 0
        w1, w2, w3, w4 = ({}, {}, {}, {})
        nlr = len(self.pack_structure.long_rails)
        ntr = len(self.pack_structure.trans_rails)
                    ...
                    for n_tl in range(3):
                        ...
                        for module_side in (False, True):
                            ...
                            for point_socket in (pp, pm):
                                if point_socket[2] != self.pack_structure.size[2] + 0.5 * wire_diameter:
    ...
    def ThermoHydraulicCircuits(self, hydraulic_network, diameter=0.008, dP=1):
        ...
        for hy_node in hydraulic_network.nodes:
            if hy_node.__class__.__name__ == 'CoolantSocket':
                ...
                piping_endpoints = [hy_node.position]
            else:
                piping_endpoints = []
            ...
            if nconnec == 2:
                ...
                rl = primitives3D.RoundedLineSegments3D(piping_endpoints, {i + 1:3 * diameter for i in range(2)},
                  adapt_radius=True)
                pipes.extend([hy.pipes.PipesFromVolmdlrPrimitives(p, diameter) for p in rl.basis_primitives])
                points.extend([p.points[0] for p in rl.basis_primitives] + [rl.basis_primitives[(-1)].points[(-1)]])
            elif nconnec == 1:
        ...
        for cooling_plate in hydraulic_network.cooling_plates:
            ...
            pipes.append(user_pipe)
            up2cp[cooling_plate] = user_pipe

        if sockets[0] not in points:
            points.append(sockets[0])
        if sockets[1] not in points:
    ...
    def plot_data(self, max_size=None):
        ...
        """
        plot_datas = []
        elements = self.pack_structure.trans_rails + self.pack_structure.long_rails + self.pack_structure.cases
        for element in elements:
            if element is not None:
                plot_datas.extend(element.plot_data())

        if max_size is not None:
            p1 = vm.Point2D((0, -max_size[1] / 2.0))
    ...
    def CADContour(self):
        ...
        p0 = vm.Point2D((-0.01, -0.01))
        p1 = vm.Point2D((self.pack_structure.size[0] + 0.02, -0.01))
        p2 = vm.Point2D((self.pack_structure.size[0] + 0.02, self.pack_structure.size[1] + 0.02))
        p3 = vm.Point2D((-0.01, self.pack_structure.size[1] + 0.02))
        l0 = vm.LineSegment2D(p0, p1)
        l1 = vm.LineSegment2D(p1, p2)
        l2 = vm.LineSegment2D(p2, p3)
        l3 = vm.LineSegment2D(p3, p0)
    ...
    def Dict(self):
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'mech_module':self.mech_module.Dict(),  'elec_battery':self.elec_battery.Dict(), 
    ...
    def DictToObject(cls, dict_):
        ...
        mech_battery = cls(mech_module=mech_module, elec_battery=elec_battery,
...
class Case(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'properties':{'position':{'type':'object', 
      ...
      'case_content':{'type':'object', 
       'title':'Case Content', 
       'order':3, 
       'classes':[
        'powerpack.mechanical.CaseContent'], 
       'editable':True, 
    ...
    def __eq__(self, other_case):
        equal = self.position == other_case.position and npy.allclose(self.size, other_case.size) and self.case_content == other_case.case_content
        return equal
    ...
    def Dict(self):
        ...
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'position':self.position.to_dict(),  'case_content':case_content_dict, 
         'size':float_size})
        return dict_
    ...
    def DictToObject(cls, dict_):
        ...
        if 'case_content' in dict_:
            ...
            case_content = None
        case = cls(position=position, size=(dict_['size']), case_content=case_content,
          name=(dict_['name']))
        return case
...
class CaseContent(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'grids'], 
     'properties':{'grids': {'type':'array', 
                'title':'Grids', 
                'order':1, 
                'items':{'type':'object', 
                 'classes':[
                  'powerpack.mechanical.Grid']}, 
                'editable':True, 
    ...
    def __eq__(self, other_content):
        if len(self.grids) == len(other_content.grids):
            equal = all([grid == other_grid for grid, other_grid in zip(self.grids, other_content.grids)])
            return equal
        return False
...
class Grid(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'grid', 'mech_module', 'basis', 'cp_normal_vector'], 
     'properties':{'grid':{'type':'array', 
      ...
      'basis':{'type':'object', 
       ...
       'order':3, 
       'classes':[
        'volmdlr.core.Basis3D'], 
       'editable':True, 
       'description':'Orientation basis of modules'}, 
      'cp_normal_vector':{'type':'object', 
       'title':'Cooling Plates Normal Vector', 
       'order':4, 
       'classes':[
        'volmdlr.core.Vector3D'], 
       'editable':True, 
       'description':'Cooling plates normal vector'}}}

    def __init__(self, grid, mech_module, basis, cooling_plates_normal_vector, piles=None, modules_side=None, cp_thickness=0.01, module_gap=0.005, name=''):
        self.grid = grid
        self.mech_module = mech_module
        self.basis = basis
        ...
        self.cp_direction_vector = cooling_plates_normal_vector.Rotation(vm.O3D, vm.Z3D, math.pi / 2)
        n_modules_norm = abs(npy.dot(self.cp_normal_vector.vector, grid))
        if n_modules_norm + 1 == len(piles):
            self.piles = piles
        else:
            msg = 'Number of modules in piles normal direction ({}) '.format(n_modules_norm) + 'and cooling plates piles ({}) is not consistent. '.format(len(piles)) + 'Add or remove piles to match number_of_modules == len(piles) - 1.'
    ...
    def __eq__(self, other_grid):
        equal = self.grid == other_grid.grid and self.mech_module == other_grid.mech_module and self.basis == other_grid.basis
        return equal
    ...
    def Frames(self, position):
        ...
        cp_frames = {}
        imodule = 0
        cp_basis = vm.Basis3D(self.cp_normal_vector, self.cp_direction_vector, vm.Z3D)
        if self.isempty:
            module_vect_size_ijk = vm.Point3D((0, 0, 0))
        else:
        ...
        for m in range(int(round(abs(npy.dot(self.grid, cp_basis.v.vector))))):
            ...
            module_pos_b += abs(npy.dot(module_vect_size_ijk.vector, cp_basis.v.vector))
        ...
        for i, cp_pile in enumerate(self.cp_piles):
            ...
            if cp_pile:
                ...
                for j, cp in enumerate(cp_pile):
                    ...
                    cp_pos_ijk = (abs(npy.dot(cp_pos, cp_basis.u.vector)),
                     ...
                     0)
                    cp_frame = vm.Frame3D(vm.Point3D(cp_pos_ijk), cp_basis.u, cp_basis.v, cp_basis.w)
            ...
            if i < int(abs(npy.dot(self.grid, cp_basis.u.vector))):
                if cp_pile:
                    ...
                    module_pos_a += self.module_gap
                module_pos_a += 0.5 * abs(npy.dot(module_vect_size_ijk.vector, cp_basis.u.vector))
                for m in range(int(round(abs(npy.dot(self.grid, cp_basis.v.vector))))):
                    module_pos_b = modules_b_positions[m] + 0.5 * abs(npy.dot(module_vect_size_ijk.vector, cp_basis.v.vector))
                    ...
                    module_frame = vm.Frame3D(vm.Point3D(module_origin_ijk), self.basis.u, self.basis.v, self.basis.w)
    ...
    def Dimensions(self):
        ...
        vect_size = (module_dim_a + cp_dim_a + 2 * self.module_gap) * self.cp_normal_vector + (module_dim_b + cp_dim_b) * self.cp_direction_vector + module_vect_size_ijk[2] * vm.Z3D
    ...
    def CADExport(self, name='An_unnamed_grid', python_path='python', freecad_path='/usr/lib/freecad/lib/', export_types=('stl', 'fcstd')):
        """
    ...
    def Copy(self, deep=False):
        ...
        grid = Grid(self.grid, module, self.basis, self.cp_normal_vector, self.piles, self.modules_side, self.cp_thickness, self.module_gap)
        return grid
    ...
    def DictToObject(cls, dict_):
        ...
        grid = cls(grid=(dict_['grid']), mech_module=mech_module, basis=basis, cooling_plates_normal_vector=cooling_plate_normal_vector,
          piles=(dict_['piles']),
          modules_side=(dict_['modules_side']),
...
class MechModule(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'cell', 'cell_basis', 'grid', 'side_plates_thickness',
     ...
     'properties':{'cell':{'type':'object', 
      ...
      'grid':{'type':'array', 
       ...
       'editable':True, 
       'description':'Grid'}, 
      'side_plates_thickness':{'type':'number', 
      ...
      'end_plates_thickness':{'type':'number', 
      ...
      'bottom_plate_thickness':{'type':'number', 
    ...
    def __init__(self, cell, cell_basis, grid, name=''):
        ...
        self.side_plates_thickness = 0.002
        self.end_plates_thickness = 0.006
        self.bottom_plate_thickness = 0.002
        self.terminals_layout = 0
        self._utd_size = False
        DessiaObject.__init__(self, name=name)

    def __eq__(self, other_module):
        equal = self.cell == other_module.cell and self.cell_basis == other_module.cell_basis and self.grid == other_module.grid and self.side_plates_thickness == other_module.side_plates_thickness and self.end_plates_thickness == other_module.end_plates_thickness and self.bottom_plate_thickness == other_module.bottom_plate_thickness
        return equal

    def __hash__(self):
        module_hash = hash(self.cell) + sum(self.grid) + int(1000 * self.side_plates_thickness) + int(4500 * self.end_plates_thickness) + int(2000 * self.bottom_plate_thickness)
        return module_hash
    ...
    def _get_size(self):
        ...
        if not self._utd_size:
            ...
            self._size = [
             self.grid[0] * abs(cell_size[0]) + abs(plate_size[0]),
            ...
            if self.terminals_layout == 0:
                ...
                if self.terminals_layout == 1:
                    ...
                    if self.terminals_layout == 2:
                        ...
                        raise ValueError
        return (
         frame.OldCoordinates(minus_position),
    ...
    def PlateDimensions(self):
        ...
        """
        plates_vect_size_uvw = vm.Point3D((
         2 * (self.end_plates_thickness + self.side_plates_thickness),
    ...
    def plot_data(self, frame=vm.OXYZ, detail=True):
        ...
        block = primitives3D.Block(vm.Frame3D(frame.origin, vect_size_ijk[0] * vm.X3D, vect_size_ijk[1] * vm.Y3D, vect_size_ijk[2] * vm.Z3D))
        ...
        if detail:
            ...
            plot_datas.append(vm.Circle2D(vm.Point2D((terminal_positions[0][0], terminal_positions[0][1])), 0.005).plot_data(color='black', fill='black'))
            plot_datas.append(vm.Circle2D(vm.Point2D((terminal_positions[1][0], terminal_positions[1][1])), 0.005).plot_data(color='red', fill='red'))
        cell_size = self.CellDimensions()
        plate_size = self.PlateDimensions()
        ...
        for i in range(self.grid[0]):
            for j in range(self.grid[1]):
                for k in range(self.grid[2]):
                    xc_xyz = m_module[0] + 0.5 * abs((2 * i + 1) * cell_size[0]) + 0.5 * abs(plate_size[0])
                    yc_xyz = m_module[1] + 0.5 * abs((2 * j + 1) * cell_size[1]) + 0.5 * abs(plate_size[1])
                    zc_xyz = m_module[2] + 0.5 * abs((2 * k + 1) * cell_size[2]) + 0.5 * abs(plate_size[2])
                    m_cell_xyz = vm.Point3D((xc_xyz, yc_xyz, zc_xyz))
                    m_cell_ijk = frame.OldCoordinates(m_cell_xyz)
                    cell_frame = vm.Frame3D(vm.Point3D(m_cell_ijk), frame.Basis().OldCoordinates(self.cell_basis.u), frame.Basis().OldCoordinates(self.cell_basis.v), frame.Basis().OldCoordinates(self.cell_basis.w))
    ...
    def StructureCADVolumes(self, frame):
        ...
        left_plate = primitives3D.ExtrudedProfile(origin, (frame.u),
          (frame.v), c1,
          [], (h * frame.w), color=[
         0.8, 0.8, 0.8],
          alpha=0.7,
        ...
        right_plate = primitives3D.ExtrudedProfile(origin, (frame.u),
          (frame.v), c2,
          [], (h * frame.w), color=[
         0.8, 0.8, 0.8],
          alpha=0.7,
        ...
        end_plate1 = primitives3D.ExtrudedProfile(origin, (frame.u),
          (frame.v), c1,
          [], (h * frame.w), color=[
         0.8, 0.8, 0.8],
          alpha=0.7,
          name='end plate 1')
        p1 = (-0.5 * w + self.side_plates_thickness) * vm.Y2D + (0.5 * l - self.end_plates_thickness - self.side_plates_thickness) * vm.X2D
        p2 = p1 + self.end_plates_thickness * vm.X2D
        p3 = p2 + (w - 2 * self.side_plates_thickness) * vm.Y2D
        p4 = p3 - self.end_plates_thickness * vm.X2D
        ls1 = vm.LineSegment2D(p1, p2)
        ...
        end_plate2 = primitives3D.ExtrudedProfile(origin, (frame.u),
          (frame.v), c2,
          [], (h * frame.w), color=[
         0.8, 0.8, 0.8],
          alpha=0.7,
          name='end plate 2')
        return [
         left_plate, right_plate, end_plate1, end_plate2]
        ...
        for i in range(self.grid[0]):
            for j in range(self.grid[1]):
                for k in range(self.grid[2]):
                    ...
                    m_cell_xyz = vm.Point3D((xc_xyz, yc_xyz, zc_xyz))
                    m_cell_ijk = frame.OldCoordinates(m_cell_xyz)
                    cell_frame = vm.Frame3D(vm.Point3D(m_cell_ijk), cell_u, cell_v, cell_w)
...
class CoolingPlate(DessiaObject):
    ...
    def CADVolume(self, frame=vm.O3D):
        ...
        """
        origin = frame.origin - frame.u * self.size[0] / 2.0 + frame.w * self.size[2] / 2.0
        volume = primitives3D.ExtrudedProfile(origin, (frame.v),
          (frame.w),
          (self.CADContour()),
          [], (frame.u * self.size[0]),
          name='Cooling_plate')
    ...
    def CADContour(self):
        ...
        """
        p0 = vm.Point2D((-self.size[1] / 2.0, -self.size[2] / 2.0))
        p1 = vm.Point2D((self.size[1] / 2.0, -self.size[2] / 2.0))
        p2 = vm.Point2D((self.size[1] / 2.0, self.size[2] / 2.0))
        p3 = vm.Point2D((-self.size[1] / 2.0, self.size[2] / 2.0))
        l0 = vm.LineSegment2D(p0, p1)
        l1 = vm.LineSegment2D(p1, p2)
        l2 = vm.LineSegment2D(p2, p3)
        l3 = vm.LineSegment2D(p3, p0)
...
class Rail(DessiaObject):
    ...
    def __eq__(self, other_rail):
        if self.__class__ == other_rail.__class__:
            equal = self.position == other_rail.position and self.length == other_rail.length and self.width == other_rail.width and self.height == other_rail.height and self.thickness == other_rail.thickness
            return equal
        return False

    def __hash__(self):
        rail_hash = 1000 * self.position + 750 * self.length + self.width + 4563 * self.height + 75112 * self.thickness
    ...
    def CADContour(self, radius=0):
        ...
        Computes rail section for CAD Export
        """
        p0 = vm.Point2D((-self.width / 2.0, -self.height / 2.0))
        p1 = vm.Point2D((self.width / 2.0, -self.height / 2.0))
        p2 = vm.Point2D((self.width / 2.0, -self.height / 2.0 + self.thickness))
        p3 = vm.Point2D((self.thickness / 2.0, -self.height / 2.0 + self.thickness))
        p4 = vm.Point2D((self.thickness / 2.0, self.height / 2.0 - self.thickness))
        p5 = vm.Point2D((self.width / 2.0, self.height / 2.0 - self.thickness))
        p6 = vm.Point2D((self.width / 2.0, self.height / 2.0))
        p7 = vm.Point2D((-self.width / 2.0, self.height / 2.0))
        p8 = vm.Point2D((-self.width / 2.0, self.height / 2.0 - self.thickness))
        p9 = vm.Point2D((-self.thickness / 2.0, self.height / 2.0 - self.thickness))
        p10 = vm.Point2D((-self.thickness / 2.0, -self.height / 2.0 + self.thickness))
        p11 = vm.Point2D((-self.width / 2.0, -self.height / 2.0 + self.thickness))
    ...
    def Dict(self):
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'position':float(self.position),  'length':float(self.length), 
...
class TransversalRail(Rail):
    ...
    def __init__(self, position, length, width, height, thickness=0, name=''):
        Rail.__init__(self, position=position, length=length, width=width, height=height,
          thickness=thickness,
          name=name)
    ...
    def CADVolume(self, radius=0):
        ...
        volume = primitives3D.ExtrudedProfile((origin + 0.5 * self.height * vm.Z3D), (vm.X3D),
          (vm.Z3D), contour,
          [], (self.length * vm.Y3D),
          name='Transversal rail')
        return volume

    def CADExport(self, name='An_unnamed_rail', python_path='python', freecad_path='/usr/lib/freecad/lib/', export_types=('stl', 'fcstd')):
        """
    ...
    def Copy(self):
        ...
        """
        return TransversalRail(position=(self.position), length=(self.length), width=(self.width),
          height=(self.height),
...
class LongitudinalRail(Rail):
    ...
    def CADVolume(self, radius=0):
        ...
        volume = primitives3D.ExtrudedProfile((origin + 0.5 * self.height * vm.Z3D), (vm.Y3D),
          (vm.Z3D), contour,
          [], (vm.X3D * self.length),
    ...
    def Copy(self):
        ...
        Returns a copy of the rail object
        """
        return LongitudinalRail(position=(self.position), length=(self.length), width=(self.width),
          height=(self.height),
          thickness=(self.thickness),
          name=(self.name))
...
class PowerPackMechanicalSimulator(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'powerpack_thermal_simulator', 'mech_battery'], 
     'properties':{'powerpack_thermal_simulator':{'type':'object', 
       'title':'Powerpack Thermal Simulators', 
       'classes':[
        'powerpack.thermal.PowerPackThermalSimulator'], 
       'description':'Powerpack thermal simulation', 
       'editable':True, 
       'order':1}, 
      'mech_battery':{'type':'object', 
       'title':'Mech Battery', 
       'classes':[
        'powerpack.mechanical.MechBattery'], 
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'powerpack_thermal_simulator':self.powerpack_thermal_simulator.Dict(),  'mech_battery':self.mech_battery.Dict()})
        return dict_
    ...
    def DictToObject(cls, dict_):
        ...
        mech_simulator = cls(powerpack_thermal_simulator=therm_simulator, mech_battery=mech_battery,
          name=(dict_['name']))
        return mech_simulator


BASES = [
 vm.Basis3D(vm.X3D, vm.Y3D, vm.Z3D),
...
def GenerateMechModules(elec_battery, two_cell_cooling=True):
    ...
    """
    cells = [
     elec_battery.mms.module.cms.cell]
    ...
    for cell in cells:
        ...
        for grid in ok_grids:
            for cell_basis in BASES:
                mech_module = MechModule(cell=cell, cell_basis=cell_basis, grid=grid, name='Automatically generated module')
