...
# [GCC 8.4.0]
# Embedded file name: C:\Users\gui_s\AppData\Local\Temp\pip-build-vljg4uzq\pyedflib\pyedflib\edfwriter.py
# Compiled at: 2018-06-15 05:54:11
# Size of source mod 2**32: 28026 bytes
...
class EdfWriter(object):
    ...
    def __init__(self, file_name, n_channels, file_type=FILETYPE_EDFPLUS):
        ...
        for i in np.arange(self.n_channels):
            if self.file_type == FILETYPE_BDFPLUS or self.file_type == FILETYPE_BDF:
                self.channels.append({'label':'test_label',  'dimension':'mV',  'sample_rate':100,  'physical_max':1.0, 
                 'physical_min':-1.0,  'digital_max':8388607, 
                 'digital_min':-8388608,  'prefilter':'pre1', 
                 'transducer':'trans1'})
            else:
                if self.file_type == FILETYPE_EDFPLUS or self.file_type == FILETYPE_EDF:
                    self.channels.append({'label':'test_label',  'dimension':'mV',  'sample_rate':100,  'physical_max':1.0, 
    ...
    def update_header(self):
        ...
        if isinstance(self.gender, int):
            ...
            set_startdatetime(self.handle, self.recording_start_time.year, self.recording_start_time.month, self.recording_start_time.day, self.recording_start_time.hour, self.recording_start_time.minute, self.recording_start_time.second)
