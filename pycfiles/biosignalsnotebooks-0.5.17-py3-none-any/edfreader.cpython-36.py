...
# [GCC 8.4.0]
# Embedded file name: C:\Users\gui_s\AppData\Local\Temp\pip-build-vljg4uzq\pyedflib\pyedflib\edfreader.py
...
__all__ = [
 'EdfReader', 'DO_NOT_READ_ANNOTATIONS',
...
class EdfReader(CyEdfReader):
    ...
    def getHeader(self):
        ...
        None
        """
        return {'technician':self.getTechnician(), 
         'recording_additional':self.getRecordingAdditional(),  'patientname':self.getPatientName(), 
         'patient_additional':self.getPatientAdditional(),  'patientcode':self.getPatientCode(), 
         'equipment':self.getEquipment(),  'admincode':self.getAdmincode(), 
         'gender':self.getGender(),  'startdate':self.getStartdatetime(),  'birthdate':self.getBirthdate()}
    ...
    def file_info_long(self):
        ...
        for ii in np.arange(self.signals_in_file):
            print('label:', self.getSignalLabels()[ii], 'fs:', self.getSampleFrequencies()[ii], 'nsamples', self.getNSamples()[ii])