...
# [GCC 8.4.0]
# Embedded file name: /Users/ramusus/workspace/manufacture/env/src/django-vkontakte-ads/vkontakte_ads/admin.py
...
YEAR_CHOICES = [
 (0, 'Любой')] + [ (year, 'c %d' % year) for year in range(2017, 1954, -1) ]
AGE_FROM_CHOICES = [(0, 'Любой')] + [ (age, 'c %d' % age) for age in range(12, 81) ]
AGE_TO_CHOICES = [(0, 'Любой')] + [ (age, 'до %d' % age) for age in range(23, 81) ]

def campaign_link(obj):
    return '<a href="%s">%s</a>' % (reverse('admin:vkontakte_ads_campaign_change', args=(obj.pk,)), obj)
...
def ad_link(obj):
    return '<a href="%s">%s</a>' % (reverse('admin:vkontakte_ads_ad_change', args=(obj.pk,)), obj)
...
def related_campaign_link(obj):
    return '<a href="%s">%s</a>' % (reverse('admin:vkontakte_ads_campaign_change', args=(obj.campaign.pk,)), obj.campaign)


related_campaign_link.short_description = 'Кампания'
related_campaign_link.allow_tags = True

def related_account_link(obj):
    return '<a href="%s">%s</a>' % (reverse('admin:vkontakte_ads_account_change', args=(obj.account.pk,)), obj.account)


related_account_link.short_description = 'Кабинет'
...
class StatisticInline(generic.GenericTabularInline):
    model = Statistic
    fields = ('day', 'month', 'overall', 'impressions', 'clicks', 'spent', 'video_views',
              'join_rate')
...
class ImageInline(admin.StackedInline):
    model = Image
    form = ImageForm
    readonly_fields = ('hash', 'photo', 'server', 'size', 'aid', 'width', 'height',
                       'post_url')
    fieldsets = (
     (
      None,
...
class AccountAdmin(CrudVkontakteAdmin, VkontakteAdsMixin):
    inlines = [
     CampaignsInline, StatisticInline]
...
class CampaignAdmin(CrudVkontakteAdmin, VkontakteAdsMixin):
    ...
    inlines = [AdsInline, StatisticInline]
...
class AdAdmin(CrudVkontakteAdmin, VkontakteAdsMixin):
    list_display = (
     'name', 'status', 'archived', related_campaign_link, 'cpc', 'cpm')
    ...
    def get_urls(self):
        ...
        urls_new = patterns('', url('^(.+)/targeting_stats/$', targeting_stats, name='vkontakte_ads_ad_targeting_stats'))
        return urls_new + urls


class ClientAdmin(CrudVkontakteAdmin, VkontakteAdsMixin):
    list_display = (
     'name', related_account_link, 'day_limit', 'all_limit')
    list_filter = ('account', )
    inlines = [CampaignsInline, StatisticInline]


class StatisticAdmin(VkontakteModelAdmin):
    list_display = ('day', 'month', 'overall', 'content_object', 'impressions', 'clicks',
                    'spent', 'video_views', 'join_rate', 'reach', 'ctr', 'cpc', 'cpm')
