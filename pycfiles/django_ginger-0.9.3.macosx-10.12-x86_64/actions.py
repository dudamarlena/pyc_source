...
class GingerFormMixinBase(object):
    ...
    def __init__(self, **kwargs):
        parent_cls = (isinstance(self, forms.ModelForm) or forms).Form if 1 else forms.ModelForm
...
class GingerModelFormSet(forms.BaseModelFormSet):
    ...
    def __init__(self, **kwargs):
        parent_cls = (isinstance(self, forms.BaseModelFormSet) or forms).BaseFormSet if 1 else forms.BaseModelFormSet
        constructor = parent_cls.__init__
        keywords = set(inspect.getargspec(constructor).args)
        context = {}
        for key in kwargs.copy():
