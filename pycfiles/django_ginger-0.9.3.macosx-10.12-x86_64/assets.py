...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/management/commands/assets.py
# Compiled at: 2015-04-29 01:50:53
import os, re, hashlib, subprocess, importlib, inspect, optparse
...
def project_path():
    attrs = [
     'BASE_DIR', 'PROJECT_DIR', 'PROJECT_ROOT']
    try:
...
def pack_js():
    ...
    if os.path.exists(outfile):
        ...
        return
    remove_files(find_asset('js'), '.{16}\\.min\\.js$')
    require_module = asset(requirejs_path)[:-3]
    options = {'baseUrl': asset('js'), 
       'name': 'main', 
       'out': outfile, 
...
def pack_css():
    ...
    change_version(template('requirecss.html'), lambda d: "<link href='{{STATIC_URL}}css/%s' rel='stylesheet' type='text/css'>" % hashcss)
    if os.path.exists(outfile):
        print 'No change detected'
        return
    remove_files(find_asset('css'), '.{16}\\.min.css$')
