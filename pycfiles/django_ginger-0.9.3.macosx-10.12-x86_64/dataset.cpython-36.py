...
# Compiled at: 2017-07-05 09:39:25
# Size of source mod 2**32: 11348 bytes
from __future__ import division
import copy, itertools, inspect, weakref
...
class BoundColumn(object):
    ...
    def sort(self, key=None, reverse=False):
        i = self.position
        self.schema.rows.sort(reverse=reverse, key=(lambda row: key(row[i]) if key is not None else row[i]))
...
class GingerDataSet(DataSetBase):
    ...
    def get_column_dict(cls):
        values = sorted((inspect.getmembers(cls, lambda value: isinstance(value, Column))),
