...
# Compiled at: 2017-06-27 12:17:08
# Size of source mod 2**32: 24491 bytes
...
class GingerTemplateView(GingerView, TemplateResponseMixin):
    ...
    def build_link(self):
        ...
        return Link(content=(self.get_page_label()), icon=(self.get_page_icon()),
          is_active=is_active,
          url=url)
...
class GingerFormView(GingerTemplateView):
    ...
    def post(self, request, *args, **kwargs):
        if self.can_submit():
            return self.process_submit(form_key=(self.get_form_key()),
              data=(request.POST),
              files=(request.FILES))
        else:
    ...
    def process_commit(self):
        ...
        for step_name in self.get_step_names():
            ...
            if not form_obj.is_valid():
                if not step.can_skip or form_obj.is_bound:
...
class GingerListView(MultipleObjectViewMixin, GingerTemplateView):
    ...
    def paginate_queryset(self, queryset):
        return paginate(queryset, (self.request), parameter_name=(self.page_parameter_name),
