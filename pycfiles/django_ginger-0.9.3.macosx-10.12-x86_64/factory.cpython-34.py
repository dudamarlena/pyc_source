...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/pretenses/factory.py
# Compiled at: 2016-06-22 09:29:58
# Size of source mod 2**32: 10220 bytes
from decimal import Decimal
from django.core.exceptions import ValidationError
from django.core.files import File
import contextlib, pytz.exceptions as pyex, random, importlib
...
class Factory(object):
    _Factory__cache = {}
    _Factory__modules = None
    highest_id = 0
    total = 0
    name = None

    def __new__(cls, model):
        if model not in cls._Factory__cache:
            obj = super(Factory, cls).__new__(cls)
            cls._Factory__cache[model] = obj
            obj.model = model
            obj.meta = model._meta
            obj.fields = obj.meta.fields
            Factory.load_pretenses()
        else:
            obj = cls._Factory__cache[model]
        return obj
    ...
    def load_pretenses(cls):
        if cls._Factory__modules is None:
            cls._Factory__modules = []
            for app in apps.get_app_configs():
                name = app.module.__name__
                module_name = '%s.pretenses' % name
                try:
                    mod = importlib.import_module(module_name)
                    cls._Factory__modules.append(mod)
...
class DefaultProcessor(object):
    ...
    def process_positive_small_integer_field(self, field):
        return streams.IntegerStream(end=4294967296).next(field)

    def process_positive_integer_field(self, field):
        return streams.IntegerStream(end=4294967296).next(field)

    def process_small_integer_field(self, field):
        return streams.IntegerStream(end=256).next(field)

    def process_big_integer_field(self, field):
        return streams.IntegerStream(end=64).next(field)

    def process_integer_field(self, field):
        return streams.IntegerStream(end=1024).next(field)
