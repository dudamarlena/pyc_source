...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/views/base.py
# Compiled at: 2017-08-01 20:29:54
# Size of source mod 2**32: 10462 bytes
...
__all__ = [
 'GingerView', 'GingerViewSetMixin', 'view', 'list_view', 'object_view']
P = pattern.Pattern
...
class GingerMetaView(type):
    _GingerMetaView__position = 0
    _GingerMetaView__actions = None

    def __init__(cls, name, bases, attrs):
        super(GingerMetaView, cls).__init__(name, bases, attrs)
        GingerMetaView._GingerMetaView__position += 1
        cls.position = GingerMetaView._GingerMetaView__position
        cls.__abstract__ = attrs.get('__abstract__', False)
        cls.base_name = '_'.join(camel_case_to_spaces(re.sub('(?i)view.*', '', cls.__name__)).split())
...
class GingerView(BasicView, GingerSessionDataMixin):
    ...
    def process_request(self, request):
        ...
        if hasattr(self, 'object'):
            if not self.is_authorized(self.user, self.object):
                raise PermissionDenied
...
class GingerViewSetMixin(object):
    ...
    def as_urls(cls, **kwargs):
        ...
        for subview in get_child_views(cls):
            ...
            view_class = url(url_regex, (cls.as_view)(action=subview.name, 
             url_name=url_name, 
