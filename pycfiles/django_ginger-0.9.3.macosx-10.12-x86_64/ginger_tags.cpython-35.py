...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/templatetags/ginger_tags.py
# Compiled at: 2015-04-17 07:06:56
# Size of source mod 2**32: 5179 bytes
...
def create_csrf_tag(context):
    ...
    if not csrf_token:
        raise Exception('No csrf token provided in the context')
...
def field_to_html(field, kwargs):
    ...
    process = lambda a: gutils.camel_to_hyphen(re.sub('widget|field', '', a.__class__.__name__)).lower()
    ...
    context = {'is_valid': is_valid, 
     ...
     'prefix': ''}
    template = [
     template_name, 'ginger/fields/%s.html' % layout]
...
def make_class_name(obj):
    return gutils.camel_to_hyphen(re.sub('widget|field|ginger', '', obj.__class__.__name__)).lower()
