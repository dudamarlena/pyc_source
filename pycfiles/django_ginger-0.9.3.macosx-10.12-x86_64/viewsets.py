...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/views/viewsets.py
# Compiled at: 2017-01-06 02:16:35
...
class ListViewSetMixin(object):
    ...
    def list(self, request):
        ...
        if object_list_formatter:
            object_list = object_list_formatter(object_list, sort_key=self.context_order_key, sort_field=filter_form.fields.get(self.context_order_key) if filter_form else None)
...
class GingerModelViewSet(GingerViewSetMixin, GingerFormView):
    ...
    def get_object_formatter(self):
        if self.object_formatter is None:
            ...
            self.object_formatter = object_formatter_factory(model_class, include=self.formatter_fields)
        return self.object_formatter

    def get_object_list_formatter(self):
        if self.object_list_formatter is None:
            model_class = self.get_queryset().model
            self.object_list_formatter = table_formatter_factory(model_class, include=self.formatter_fields, get_cell_url=lambda me, cell: self.reverse('detail', object_id=cell.source.id))
