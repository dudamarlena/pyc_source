...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/tests/views.py
# Compiled at: 2014-12-29 10:20:12
...
class TestGingerResponse(test.SimpleTestCase):
    ...
    def test_json(self):
        value = 'mickey mouse'
        template = templates.from_string('{{name}}')
        resp = GingerResponse(self.request, template, {'name': value}, content_type='application/json')
        content = resp.render().content
...
class TestGingerFormDone(test.SimpleTestCase):
    ...
    def test_valid_post(self):
        session = {}
        name = 'asdasda asdasd'
        request = self.request('/form/', method='post', data={'name': name, 'age': 32}, session=session)
        response = self.view(request)
        self.assertEqual(response.status_code, 302)
    ...
    def test_invalid_post(self):
        session = {}
        name = 'asdasda asdasd'
        request = self.request('/form/', method='post', data={'name': name, 'age': 320}, session=session)
        response = self.view(request)
        self.assertEqual(response.status_code, 200)
...
@override_settings(MEDIA_ROOT=MEDIA_ROOT, TEMPLATE_DIRS=(
 joinpath(dirname(realpath(__file__)), 'templates'),))
