...
# [GCC 8.4.0]
# Embedded file name: /Users/shodh/Projects/django_ginger/ginger/forms/actions.py
# Compiled at: 2016-08-18 12:58:46
...
class GingerFormMixinBase(object):
    ...
    def result(self):
        self.is_valid()
        return self._GingerFormMixinBase__result
    ...
    def full_clean(self):
        ...
        try:
            _ = self._GingerFormMixinBase__result
            ...
            try:
                ...
                self._GingerFormMixinBase__result = result
...
class GingerFormMixin(object):
    ...
    def __init__(self, **kwargs):
        ...
        parent_keywords = set(inspect.getargspec(parent_constructor).args) if func(parent_constructor) is not func(constructor) else set()
    ...
    def process_context(self):
        context = self.context.copy()
        if not isinstance(self, GingerSearchFormMixin):
            if hasattr(self, 'cleaned_data'):
                if hasattr(self, 'save'):
                    instance = self.save(commit=False)
                    context['instance'] = instance
        ...
        for name, field in six.iteritems(fields):
            ...
            if callable(data):
                ...
                if isinstance(data, (datetime.datetime, datetime.time)):
                    if not getattr(field.widget, 'supports_microseconds', True):
    ...
    def result(self):
        self.is_valid()
        return self._GingerFormMixin__result
    ...
    def full_clean(self):
        ...
        try:
            _ = self._GingerFormMixin__result
        except AttributeError:
            result = None
            try:
                try:
                ...
                self._GingerFormMixin__result = result
...
class GingerSearchFormMixin(GingerFormMixin):
    ...
    def process_queryset_filters(self, page=None, parameter_name='page', page_limit=10, per_page=20, **kwargs):
            ...
            if not name not in allowed:
                ...
                if hasattr(self, 'filter_%s' % name):
                    ...
                    if hasattr(self, 'handle_%s' % name):
                        ...
                        if hasattr(field, 'handle_queryset'):
        ...
        if page is not None:
            queryset = self.paginate(queryset, page, parameter_name=parameter_name, page_limit=page_limit, per_page=per_page)
        return queryset
    ...
    def to_json(self):
        ...
        if isinstance(result, Page):
            return {'data': result.object_list, 
             'page': result}
        else:
            return {'data': result}
...
class GingerDataFormMixin(GingerSearchFormMixin):
    ...
    def get_dataset_class(self):
        try:
            return next(f.dataset_class for f in six.itervalues(self.fields) if hasattr(f, 'dataset_class'))
        except StopIteration:
            return self.dataset_class

    def get_dataset_filter_names(self):
        names = set(name for name, f in six.iteritems(self.fields) if getattr(f, 'process_list', False))
...
class GingerModelFormSet(forms.BaseModelFormSet):
    ...
    def result(self):
        self.is_valid()
        return self._GingerModelFormSet__result
    ...
    def full_clean(self):
        ...
        try:
            _ = self._GingerModelFormSet__result
        except AttributeError:
            result = None
            try:
                try:
                    if self.is_bound:
                        if not any(self._errors) and not self._non_form_errors:
                            context = self.process_context()
                            result = self.execute(**context)
                except forms.ValidationError as ex:
                    self.add_error(None, ex)
                ...
                self._GingerModelFormSet__result = result
