...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DPAPI/Core/credhist.py
...
class RPC_SID(eater.DataStruct):
    ...
    def parse(self, data):
        ...
        n = data.eat('B')
        self.idAuth = struct.unpack('>Q', '\x00\x00' + data.eat('6s'))[0]
        self.subAuth = data.eat('%dL' % n)

    def __str__(self):
        s = [
         'S-%d-%d' % (self.version, self.idAuth)]
...
class CredhistEntry(eater.DataStruct):
    ...
    def decryptWithKey(self, enckey):
        ...
        cleartxt = crypto.dataDecrypt(self.cipherAlgo, self.hashAlgo, self.encrypted, enckey, self.iv, self.rounds)
