...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/DPAPI/Core/crypto.py
...
CryptoAlgo.add_algo(26115, name='DES3', keyLength=192, IVLength=64, blockLength=64, m2='des_ede3_cbc', keyFixup=des_set_odd_parity)
CryptoAlgo.add_algo(26121, name='DES2', keyLength=128, IVLength=64, blockLength=64, m2='des_ede_cbc', keyFixup=des_set_odd_parity)
CryptoAlgo.add_algo(26129, name='AES', keyLength=128, IVLength=128, blockLength=128, m2='aes_128_cbc')
CryptoAlgo.add_algo(26126, name='AES-128', keyLength=128, IVLength=128, blockLength=128, m2='aes_128_cbc')
CryptoAlgo.add_algo(26127, name='AES-192', keyLength=192, IVLength=128, blockLength=128, m2='aes_192_cbc')
CryptoAlgo.add_algo(26128, name='AES-256', keyLength=256, IVLength=128, blockLength=128, m2='aes_256_cbc')
CryptoAlgo.add_algo(26113, name='DES', keyLength=64, IVLength=64, blockLength=64, m2='des_cbc', keyFixup=des_set_odd_parity)
CryptoAlgo.add_algo(32777, name='HMAC', digestLength=160, blockLength=512)
CryptoAlgo.add_algo(32769, name='md2', digestLength=128, blockLength=128)
CryptoAlgo.add_algo(32770, name='md4', digestLength=128, blockLength=512)
CryptoAlgo.add_algo(32771, name='md5', digestLength=128, blockLength=512)
CryptoAlgo.add_algo(32772, name='sha1', digestLength=160, blockLength=512)
CryptoAlgo.add_algo(32780, name='sha256', digestLength=256, blockLength=512)
CryptoAlgo.add_algo(32781, name='sha384', digestLength=384, blockLength=1024)
CryptoAlgo.add_algo(32782, name='sha512', digestLength=512, blockLength=1024)
...
def CryptSessionKeyXP(masterkey, nonce, hashAlgo, entropy=None, strongPassword=None):
    ...
    ipad = ('').join(chr(ord(masterkey[i]) ^ 54) for i in range(hashAlgo.blockSize))
    opad = ('').join(chr(ord(masterkey[i]) ^ 92) for i in range(hashAlgo.blockSize))
...
def CryptDeriveKey(h, cipherAlgo, hashAlgo):
    ...
    ipad = ('').join(chr(ord(h[i]) ^ 54) for i in range(hashAlgo.blockSize))
    opad = ('').join(chr(ord(h[i]) ^ 92) for i in range(hashAlgo.blockSize))
...
def SystemFunction005(secret, key):
    ...
    decrypted_data = ''
    j = 0
    algo = CryptoAlgo(26115)
    for i in range(0, len(secret), 8):
        enc_block = secret[i:i + 8]
        block_key = key[j:j + 7]
        des_key = []
        des_key.append(ord(block_key[0]) >> 1)
        des_key.append((ord(block_key[0]) & 1) << 6 | ord(block_key[1]) >> 2)
        des_key.append((ord(block_key[1]) & 3) << 5 | ord(block_key[2]) >> 3)
        des_key.append((ord(block_key[2]) & 7) << 4 | ord(block_key[3]) >> 4)
        des_key.append((ord(block_key[3]) & 15) << 3 | ord(block_key[4]) >> 5)
        des_key.append((ord(block_key[4]) & 31) << 2 | ord(block_key[5]) >> 6)
        des_key.append((ord(block_key[5]) & 63) << 1 | ord(block_key[6]) >> 7)
        des_key.append(ord(block_key[6]) & 127)
...
def derivePwdHash(pwdhash, userSID, digest='sha1'):
    ...
    return M2Crypto.EVP.hmac(pwdhash, (userSID + '\x00').encode('UTF-16LE'), digest)
