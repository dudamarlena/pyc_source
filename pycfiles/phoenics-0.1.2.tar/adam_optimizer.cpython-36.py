...
# [GCC 8.4.0]
# Embedded file name: /home/flo/Phoenics/master/src/phoenics/Acquisition/NumpyOptimizers/adam_optimizer.py
...
class AdamOptimizer(AbstractOptimizer):
    ...
    def __init__(self, func=None, eta=0.1, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0, *args, **kwargs):
    ...
    def get_update(self, vector):
        ...
        if self.initial_decay > 0.0:
            eta *= 1.0 / (1.0 + self.decay * self.iterations)
        next_iter = self.iterations + 1
        eta_next = eta * (np.sqrt(1.0 - np.power(self.beta_2, next_iter)) / (1.0 - np.power(self.beta_1, next_iter)))
        if not hasattr(self, 'ms'):
            self.ms = [0.0 for element in vector]
            self.vs = [0.0 for element in vector]
        ...
        for index, element, grad, mass, vel in zip(range(len(vector)), vector, grads, self.ms, self.vs):
            m_next = self.beta_1 * mass + (1.0 - self.beta_1) * grad
            v_next = self.beta_2 * vel + (1.0 - self.beta_2) * np.square(grad)
