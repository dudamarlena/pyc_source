...
# [GCC 8.4.0]
# Embedded file name: /home/flo/Phoenics/master/src/phoenics/SampleSelector/sample_selector.py
...
class SampleSelector(Logger):
    ...
    def select(self, num_samples, proposals, kernel_contribution, sampling_param_values, obs_params):
        ...
        for batch_index in range(len(sampling_param_values)):
            ...
            ident_indices = np.where(min_distances < 1e-08)[0]
            exp_objs[(batch_index, ident_indices)] = 0.0
        ...
        for sample_index in range(num_samples):
            ...
            for batch_index in range(len(sampling_param_values)):
                ...
                for proposal_index, proposal in enumerate(batch_proposals[:exp_objs.shape[1]]):
                    ...
                    div_crits[proposal_index] = np.minimum(1.0, np.mean(np.exp(2.0 * (min_distance - char_dists) / feature_ranges)))
                ...
                exp_objs[(batch_index, largest_reward_index)] = 0.0
