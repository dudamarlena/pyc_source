...
# [GCC 8.4.0]
# Embedded file name: /home/flo/Phoenics/master/src/phoenics/BayesianNetwork/EdwardInterface/edward_interface.py
# Compiled at: 2019-11-24 12:43:13
# Size of source mod 2**32: 11898 bytes
"""
...
import pickle, numpy as np, tensorflow as tf, edward as ed
...
class EdwardNetwork(object):
    ...
    def declare_training_data(self, obs_params, obs_objs):
        ...
        for kernel_index, kernel_name in enumerate(self.config.kernel_names):
            ...
            lower_rescalings[kernel_index] = low
            upper_rescalings[kernel_index] = up
    ...
    def construct_model(self):
        with self.graph.as_default():
            ...
            self.prior_scale = ed.models.Deterministic(1.0 / tf.sqrt(self.prior_tau))
            self.post_layer_outputs = [
             self.x]
            self.posteriors = {}
            for layer_index in range(self._num_layers):
                weight_shape, bias_shape = weight_shapes[layer_index], bias_shapes[layer_index]
            ...
            self.post_scale = ed.models.Deterministic(1.0 / self.post_sqrt_tau)
            post_kernels = {}
            targets_dict = {}
            ...
            while kernel_element_index < len(self.config.kernel_names):
                ...
                post_kernels['param_%d' % target_element_index] = {'loc':ed.models.Deterministic(post_support), 
