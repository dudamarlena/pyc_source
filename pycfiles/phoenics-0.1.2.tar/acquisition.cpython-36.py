...
# [GCC 8.4.0]
# Embedded file name: /home/flo/Phoenics/master/src/phoenics/Acquisition/acquisition.py
...
class Acquisition(Logger):
    ...
    def propose(self, best_params, kernel_contribution, sampling_param_values, num_samples=50, parallel='True', dominant_samples=None, dominant_strategy=None):
        ...
        random_proposals = self._propose_randomly(best_params,
          num_samples, dominant_samples=dominant_samples)
        import time
        start = time.time()
        optimized_proposals = self._optimize_proposals(random_proposals,
          kernel_contribution, dominant_samples=dominant_samples)
        end = time.time()
