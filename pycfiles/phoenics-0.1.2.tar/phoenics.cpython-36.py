...
# [GCC 8.4.0]
# Embedded file name: /home/flo/Phoenics/master/src/phoenics/phoenics.py
# Compiled at: 2019-11-24 14:19:17
# Size of source mod 2**32: 6036 bytes
"""
    ...
    def create_folders(self):
        if not os.path.isdir(self.config.get('scratch_dir')):
            ...
            if self.config.get_db('has_db'):
                if not os.path.isdir(self.config.get_db('path')):
                    try:
    ...
    def recommend(self, observations=None, as_array=False):
        ...
        if observations is None:
            ...
            if len(observations) == 0:
                ...
                samples = self.sample_selector.select(self.config.get('batches'), proposed_samples, kernel_contribution, sampling_param_values, obs_params)
        ...
        if self.config.get_db('has_db'):
            db_entry = {'start_time':start_time, 
             'end_time':end_time,  'received_obs':observations, 
...
if __name__ == '__main__':
    observations = [
     {'param_0':[
       -1.0, -1.0], 
      'param_1':[1.0],  'obj_0':0.1,  'obj_1':0.2},
     {'param_0':[
       1.0, 1.0], 
      'param_1':[-1.0],  'obj_0':0.2,  'obj_1':0.1}]
    phoenics = Phoenics()
