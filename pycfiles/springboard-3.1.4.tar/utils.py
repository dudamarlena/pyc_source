...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/universalcore/springboard/springboard/utils.py
...
def is_excluded_path(path, excluded_paths):
    ...
    return path and any([ p for p in excl_paths if path.startswith(p) ])
...
class Paginator(object):
    ...
    def get_page(self):
        return self.results[self.page * self.results_per_page:(self.page + 1) * self.results_per_page]
    ...
    def total_pages(self):
        return int(math.ceil(float(self.total_count()) / float(self.results_per_page)))

    def page_numbers(self):
        if self.page - self.buffer_value < 0:
            return [ page_number for page_number in range(0, min([self.slider_value, self.total_pages()]))
                   ]
        else:
            if self.page + self.buffer_value >= self.total_pages():
                return [ page_number for page_number in range(max(self.total_pages() - self.slider_value, 0), self.total_pages())
                       ]
            return range(self.page - self.buffer_value, self.page + self.buffer_value + 1)
...
class CachingRepoHelper(RepoHelper):
    ...
    def active_branch_name(self):
        if not hasattr(self, '_active_branch_name'):
            self._active_branch_name = super(CachingRepoHelper, self).active_branch_name()
