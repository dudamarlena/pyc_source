...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/postprocess/email_sender/postprocess.py
...
Your friendly server
    ...
    def launch(self, values, out):
        for event in values:
            ...
            if not to:
                raise ValueError('No recipient specified for the postprocess email')
            tmpl_vars = dict(process_name=event.tube.display_name, variant_name=event.variant_name)
            subject_tmpl = TextTemplate(self.get_config_key('subject'))
            body_tmpl = TextTemplate(self.get_config_key('body'))
            message = Message(self.get_config_key('from'), to, subject_tmpl.generate(**tmpl_vars).render())
