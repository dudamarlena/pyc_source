...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/core/svg.py
...
def calculate_start_angle(x1, y1, rx, ry, coordAngle, largeArcFlag, sweep_flag, x2, y2):
    ...
    try:
        sweep_angle = math.degrees(angle([(rotated_x1 - ccx) / rx, (rotated_y1 - ccy) / ry], [
...
class SvgParser(object):
    ...
    def get_svg_element_font_size(svg_elem):
        ...
        if 'style' in svg_elem._attributes:
            match = re.search('(font\\-size\\:(\\d+\\.?\\d*)px\\;)', svg_elem._attributes['style'])
            if not match:
...
class SvgNormalizer(object):
    ...
    def __init__(self, svg_elem, new_resolution, svg_original_resolution):
        ...
        if svg_elem.__class__.__name__ == 'rect':
            ...
            if svg_elem.__class__.__name__ == 'tspan':
                self._normalize_tspan(svg_elem)
            else:
                if svg_elem.__class__.__name__ == 'text':
                    self._normalize_text(svg_elem)
                else:
                    if svg_elem.__class__.__name__ == 'line':
                        self._normalize_line(svg_elem)
                    else:
                        if svg_elem.__class__.__name__ == 'path':
        ...
        for i in range(len(res.keys())):
            if new_width > res.keys()[i]:
                if i == 0:
                    ...
                    size_per_pixel = (res[res.keys()[(i - 1)]] - res[res.keys()[i]]) / (res.keys()[(i - 1)] - res.keys()[i])
                    font_proportion = res[res.keys()[i]] + (new_width - res.keys()[i]) * size_per_pixel
            break
        else:
            font_proportion = new_width * (res[res.keys()[(len(res.keys()) - 1)]] / res.keys()[(len(res.keys()) - 1)])
