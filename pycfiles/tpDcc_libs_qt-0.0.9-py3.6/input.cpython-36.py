...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/core/input.py
# Compiled at: 2020-04-24 23:12:07
# Size of source mod 2**32: 4697 bytes
"""
...
class InputAction(object):
    ...
    def __init__(self, name='defaultName', action_type=InputActionType.Keyboard, group='default', mouse=Qt.NoButton, key=None, modifiers=Qt.NoModifier):
    ...
    def __str__(self):
        return '{} {} {}'.format(QKeySequence(self.get_modifiers()).toString(), self.get_mouse_button().name.encode('utf-8'), QKeySequence(self.get_key().toString()))
    ...
    def from_dict(self, dict_data):
        try:
            ...
            self._data['modifiers'] = self._list_of_modifiers_to_enum([Qt.KeyboardModifier(i) for i in dict_data['modifiers']])
            self._action_type = InputActionType(dict_data['actionType'])
            return self
        except Exception as exc:
            LOGGER.error('Impossible to load Input Action from given dictionary: {} | {}'.format(exc, traceback.format_exc()))
            return
