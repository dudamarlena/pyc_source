...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/dock.py
# Compiled at: 2020-04-24 23:12:07
# Size of source mod 2**32: 60955 bytes
"""
...
def splitter_contains_section_widget(splitter):
    for i in range(splitter.count()):
        w = splitter.widget(i)
        if splitter:
            if splitter_contains_section_widget(splitter):
                return True
            elif issubclass(w, DockSectionWidget):
                return True
    ...
    while do_again:
        ...
        for i in range(len(splitters)):
            ...
            if not sp.property('dock-splitter').toBool():
                ...
                if sp.countT() > 0:
                    if splitter_contains_section_widget(sp):
                        continue
                del splitters[i]
                do_again = True
                break
    ...
    if not layout or layout.count() <= 0:
        ...
        for i in range(layout.count()):
            ...
            if not layout_item.widget():
                pass
            elif isinstance(layout_item.widget(), QSplitter):
...
class DockSectionContent(object):
    ...
    def new_section_content(unique_name, container, title, content):
        ...
        if unique_name == '':
            ...
            if DockContainer.check_section_container_lookup_map_by_name(container, unique_name):
                tp.logger.error('Cannot create DockSectionContent with already used unique_name')
                return
            if not container or not title or not content:
...
class DockSectionTitleWidget(QFrame, object):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if drop_area != drop.DropArea.InvalidDropArea:
            ...
            if self._tab_moving:
                if find_parent_section_widget(self) is not None:
        ...
        if container_widget is None:
            ...
            if self._floating_widget and event.buttons() & Qt.LeftButton:
                ...
                if section:
                    ...
                    if container_widget.outer_top_drop_rect().contains(container_widget.mapFromGlobal(QCursor.pos())):
                        ...
                        if container_widget.outer_right_drop_rect().contains(container_widget.mapFromGlobal(QCursor.pos())):
                            ...
                            if container_widget.outer_bottom_drop_rect().contains(container_widget.mapFromGlobal(QCursor.pos())):
                                ...
                                if container_widget.outer_left_drop_rect().contains(container_widget.mapFromGlobal(QCursor.pos())):
                ...
                if not self._drag_start_pos.isNull():
                    if event.buttons() & Qt.LeftButton:
                        if section_widget is not None:
                            if not section_widget.title_area_geometry().contains(section_widget.mapFromGlobal(event.globalPos())):
                                event.accept()
                                ...
                                if data is None:
                                    ...
                                    self._floating_widget = DockFloatingWidget(container=container_widget, section_content=(data.content), section_title_widget=(data.title_widget),
                                      content_widget=(data.content_widget),
                                    ...
                                    return
        if self._tab_moving:
            if find_parent_section_widget(self):
                event.accept()
        ...
        if not self._drag_start_pos.isNull():
            if event.buttons() & Qt.LeftButton:
                if (event.pos() - self._drag_start_pos).manhattanLength() >= QApplication.startDragDistance():
                    if find_parent_section_widget(self) is not None:
                        if find_parent_section_widget(self).title_area_geometry().contains(find_parent_section_widget(self).mapFromGlobal(event.globalPos())):
                            event.accept()
    ...
    def index_of_content_by_title_pos(self, p, exclude):
        ...
        for i in range(len(self._section_titles)):
            if self._section_titles[i].geometry().contains(p):
                if exclude is None or self._section_titles[i] != exclude:
                    index = i
                    break
...
class DockFloatingWidget(QWidget, object):
    ...
    def clean(self):
        del self._container._floatings[:]
...
class DockContainer(QFrame, object):
    ...
    def drop_content_outer_helper(self, parent_layout, internal_data, orientation, append):
        if parent_layout is None:
            ...
            if not old_splitter:
                ...
                if old_splitter.orientation() == orientation or old_splitter.count() == 1:
    ...
    def drop_content(self, internal_data, target_section_widget, drop_area, auto_active):
        ...
        if self.is_empty():
            ...
            drop_area = drop.DropArea.CenterDropArea
        elif not target_section_widget:
            if drop_area == drop.DropArea.TopDropArea:
                ret = self.drop_content_outer_helper(parent_layout=(self.main_layout),
                  internal_data=internal_data,
                  orientation=(Qt.Vertical),
                  append=False)
            else:
                if drop_area == drop.DropArea.RightDropArea:
                    ret = self.drop_content_outer_helper(parent_layout=(self.main_layout),
                      internal_data=internal_data,
                      orientation=(Qt.Horizontal),
                      append=True)
                else:
                    if drop_area == drop.DropArea.CenterDropArea or drop_area == drop.DropArea.BottomDropArea:
                        ret = self.drop_content_outer_helper(parent_layout=(self.main_layout),
                          internal_data=internal_data,
                          orientation=(Qt.Vertical),
                          append=True)
                    else:
                        if drop_area == drop.DropArea.LeftDropArea:
                            self.drop_content_outer_helper(parent_layout=(self.main_layout),
                              internal_data=internal_data,
                              orientation=(Qt.Horizontal),
                              append=False)
                        else:
        ...
        if target_section_splitter is None:
            ...
            if drop_area == drop.DropArea.TopDropArea:
                ...
                if drop_area == drop.DropArea.RightDropArea:
                    ...
                    if drop_area == drop.DropArea.BottomDropArea:
                        section_widget = self.new_section_widget()
                        section_widget.add_data_content(internal_data, True)
                        if target_section_splitter.orientation() == Qt.Vertical:
                            index = target_section_splitter.indexOf(target_section_widget)
                        ...
                        if drop_area == drop.DropArea.LeftDropArea:
                            ...
                            if drop_area == drop.DropArea.CenterDropArea:
    ...
    def add_section_content(self, section_content, section_widget, drop_area):
        ...
        if section_content is None:
            ...
            return self.drop_content(internal_data=data,
              target_section_widget=section_widget,
              drop_area=drop_area,
              auto_active=False)
        ...
        if section_content is None:
            ...
            if section_content.uid() in self._hidden_section_contents:
                ...
                if hidden_section_item.preferred_section_id > 0:
                    ...
                    if len(self._sections) and self._sections[0] is not None:
