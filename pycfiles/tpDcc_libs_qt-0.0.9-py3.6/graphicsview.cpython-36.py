...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/qt/widgets/graphicsview.py
# Compiled at: 2020-04-24 23:12:07
# Size of source mod 2**32: 18154 bytes
"""
...
class BaseGraphicsView(QGraphicsView, object):
    ...
    def mousePressEvent(self, event):
        ...
        if self._pressed_item:
            if isinstance(self._pressed_item, QGraphicsItem):
    ...
    def remove_item_by_name(self, name):
        ...
        """
        if self.scene():
            [self.scene().removeItem(i) for i in self.scene().items() if hasattr(i, 'name') if i.name == name]
