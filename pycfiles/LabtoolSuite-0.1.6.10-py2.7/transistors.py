...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/Apps/transistors.py
# Compiled at: 2015-05-19 05:33:47
"""
...
class Handler(QFrame, ConvenienceClass, Ui_Form):
    ...
    def __init__(self, exp):
        ...
        self.I.set_pvs1(0)
        self.Exp.setRange(self.plot, -5, -0.02, 10, 0.04)
    ...
    def start(self, x=None):
        ...
        if not self.looptimer.isActive():
            self.looptimer = self.loopTask(self.timegap, self.acquire, 0)
    ...
    def updateBaseCurrent(self):
        self.base_current = 1000.0 * self.base_voltage / self.seriesR
    ...
    def delete_curve(self):
        ...
        if c > -1:
            ...
            self.plot.removeItem(self.curves[c])
            self.plot.removeItem(self.legends[c])
            self.curves.pop(c)
            self.legends.pop(c)
    ...
    def changeTransistorType(self, t):
        if t:
            ...
            self.MINV = 0.0
            self.MAXV = 2.0
        self.Exp.set2DRange(self.MINV - 1, -0.02, (self.MAXV - self.MINV + 2) * 1.2, 0.04)
    ...
    def acquire(self, chan):
        ...
        self.VceLabel.setText('V_ce = %0.2fV' % V)
        self.IceLabel.setText('I_ce = %0.2fmA' % (I * 1000.0))
        self.X.append(V)
        self.Y.append(I)
        ...
        if self.Vval > self.MAXV:
            ...
            if self.transistor_type == 'NPN':
                ...
                anchor = (1, 0)
            text = pg.TextItem(html=txt, anchor=anchor, border='w', fill=(0, 0, 255,
                                                                          100))
