...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/scope_interface.py
# Compiled at: 2015-04-28 02:45:51
import os
...
import packet_handler, I2C_class, SPI_class, NRF24L01_class
from achan import *
...
class Interface():
    ...
    def __fetch_channel__(self, channel_number):
        ...
        for i in range(int(samples / splitting)):
            ...
            data += self.H.fd.read(splitting * 2)
            self.H.__get_ack__()
        ...
        data += self.H.fd.read(2 * (samples % splitting))
        self.H.__get_ack__()
    ...
    def fetch_LA_channels(self, trigchan=1):
        ...
        if self.dchans[(trigchan - 1)].dlength > 1:
            if self.digital_channels_in_buffer == 1:
                offset = self.dchans[(trigchan - 1)].timestamps[0]
            else:
                offset = 0
        else:
            offset = 0
