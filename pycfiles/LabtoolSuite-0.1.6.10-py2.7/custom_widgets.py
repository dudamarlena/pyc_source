...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/custom_widgets.py
# Compiled at: 2015-06-14 14:05:55
"""
...
class CustomWidgets:
    ...
    class timingHandler(QFrame, Ui_ClickingOptions):
        ...
        def clicked(self):
            ...
            elif 'time' in self.cmdname:
                self.label.setText('%.2e S' % val)
            else:
                self.label.setText('%.1f Hz' % val)
    ...
    class sourceHandler(QFrame, Ui_Sliding):
        ...
        def setValue(self, val):
            if self.name == 'pvs1':
                retval = self.I.set_pvs1(val * 10.0 / 4095 - 5)
            elif self.name == 'pvs2':
