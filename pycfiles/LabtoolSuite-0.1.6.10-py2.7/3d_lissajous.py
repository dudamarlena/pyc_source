...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/Apps/3d_lissajous.py
# Compiled at: 2015-04-22 05:52:58
...
class Handler(QFrame, ConvenienceClass, Ui_Form):
    ...
    def acquire(self):
        ...
        self.launchdisplay = self.delayedTask(self.tg * self.samples * 1e-06 + 10, self.display)

    def display(self):
        X = self.I.fetch_trace(self.Xchan)[1]
        Y = self.I.fetch_trace(self.Ychan)[1]
        Z = self.I.fetch_trace(self.Zchan)[1]
        self.Exp.clearLinesOnPlane(self.plot)
        self.Exp.draw3dLine(self.plot, np.array(Z) * self.scaleZ + self.offZ, np.array(X) * self.scaleX + self.offX, np.array(Y) * self.scaleY + self.offY, (0,
                                                                                                                                                             100,
                                                                                                                                                             255))
        self.launchacquire = self.delayedTask(self.delay, self.acquire)
