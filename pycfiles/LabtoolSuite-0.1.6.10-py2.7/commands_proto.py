...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/commands_proto.py
# Compiled at: 2015-06-13 05:26:13
...
def InttoString(val):
    return chr(val & 255) + chr(val >> 8 & 255)
...
def PR2(T, multiplier, postscaler):
    table_size = 180
    return T * 8000000.0 * multiplier / (table_size * (1 + postscaler))


def getfreq(pr2, m, p):
    table_size = 180
    return 8000000.0 * m / (round(pr2) * table_size * (p + 1))
