...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Labtools/interface.py
# Compiled at: 2015-06-15 09:30:38
import os
...
import packet_handler, I2C_class, SPI_class, NRF24L01_class, MCP4728_class
from achan import *
...
class Interface(object):
    ...
    def capture_traces(self, num, samples, tg, channel_one_input='CH1', CH123SA=0, **kwargs):
        ...
        if num == 1:
            ...
            if self.timebase < 1.75:
                self.timebase = 1.75
            if samples > self.MAX_SAMPLES / 4:
                samples = self.MAX_SAMPLES / 4
            self.achans[0].set_params(channel=channel_one_input, length=samples, timebase=self.timebase, resolution=TEN_BIT)
            for a in range(1, 4):
                self.achans[a].set_params(channel=['NONE', 'CH2', 'CH3', 'CH4'][a], length=samples, timebase=self.timebase, resolution=TEN_BIT)

            self.H.__sendByte__(CAPTURE_FOUR)
            self.H.__sendByte__(CHOSA | CH123SA << 4 | triggerornot)
    ...
    def configure_trigger(self, chan, level, resolution=10):
        ...
        if resolution == 12:
            level = 2047 - 31 * level * self.gain_values[self.achans[chan].gain] * 4.0
        else:
    ...
    def get_capacitance(self, current_range, trim, Charge_Time):
        """
                ...
                """
        self.H.__sendByte__(COMMON)
        currents = [0.0005775, 5.3e-07, 5.775e-06, 5.775e-05]
        ...
        C = Charge_Current * Charge_Time * 1e-06 / V
    ...
    def set_sine_phase(self, phase):
        ...
        self.H.__sendInt__(int(4095 * phase / 360.0) & 16383)
        self.H.__get_ack__()
    ...
    def set_waveform_type(self, channel, waveform='sine'):
        ...
        self.H.__sendByte__(1 << wave.get(waveform, 0) | 16 << channel)
        self.H.__get_ack__()
    ...
    def select_freq(self, channel, register):
        ...
        self.H.__sendByte__(1 << register | 16 << channel)
        self.H.__get_ack__()
    ...
    def write_dac(self, channel, n):
        ...
        val = channel << 15 | 16384 | 8192 | 4096 | n
    ...
    def WS2182B(self, col):
        ...
        R = reverse_bits(col[0])
        G = reverse_bits(col[1])
        B = reverse_bits(col[2])
        self.H.__sendByte__(B)
        self.H.__sendByte__(R)
        self.H.__sendByte__(G)
        self.H.__get_ack__()
    ...
    def servo(self, chan, angle):
        """
                ...
                **Arguments** 
                ...
                """
        if chan == 1:
            self.set_sqr1(10000, 750 + int(angle * 1900 / 180), 2)
        elif chan == 2:
            self.set_sqr2(10000, 750 + int(angle * 1900 / 180), 2)
