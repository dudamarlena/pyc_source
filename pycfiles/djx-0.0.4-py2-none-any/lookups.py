...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/db/models/lookups.py
...
from django.db.models.fields import DateTimeField, DecimalField, Field, IntegerField
...
class Lookup(object):
    ...
    def rhs_is_direct_value(self):
        return not (hasattr(self.rhs, 'as_sql') or hasattr(self.rhs, 'get_compiler'))
...
class FieldGetDbPrepValueMixin(object):
    ...
    def get_db_prep_lookup(self, value, connection):
        ...
        return ('%s',
         [ get_db_prep_value(v, connection, prepared=True) for v in value ] if self.get_db_prep_lookup_value_is_iterable else [
...
class Search(BuiltinLookup):
    ...
    def as_sql(self, compiler, connection):
        warnings.warn('The `__search` lookup is deprecated. See the 1.10 release notes for how to replace it.', RemovedInDjango20Warning, stacklevel=2)
        lhs, lhs_params = self.process_lhs(compiler, connection)
