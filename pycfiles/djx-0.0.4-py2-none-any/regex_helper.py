...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/utils/regex_helper.py
    ...
    try:
        while True:
            if escaped:
                ...
                raise NotImplementedError(b'Awaiting Implementation')
            elif ch == b'^':
                pass
            elif ch == b'$':
                break
                ...
                if ch != b'?' or escaped:
                    ...
                    if ch in b'!=<':
                        ...
                        warnings.warn(b'Using (?%s) in url() patterns is deprecated.' % ch, RemovedInDjango21Warning)
                        walk_to_end(ch, pattern_iter)
                    elif ch == b':':
