...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/monkeypatch.py
...
def annotated_getattr(obj, name, ann):
    try:
        ...
        raise AttributeError('%r object at %s has no attribute %r' % (type(obj).__name__, ann, name))
...
class MonkeyPatch(object):
    ...
    def _warn_if_env_name_is_not_str(self, name):
        ...
        if six.PY2 and not isinstance(name, str):
            warnings.warn(pytest.PytestWarning(('Environment variable name {!r} should be str').format(name)))
    ...
    def setenv(self, name, value, prepend=None):
        ...
        if not isinstance(value, str):
            warnings.warn(pytest.PytestWarning(('Value of environment variable {name} type should be str, but got {value!r} (type: {type}); converted to str implicitly').format(name=name, value=value, type=type(value).__name__)), stacklevel=2)
            value = str(value)
