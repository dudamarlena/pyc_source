...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/urllib3/urllib3/connectionpool.py
...
class HTTPConnectionPool(ConnectionPool, RequestMethods):
    ...
    def urlopen(self, method, url, body=None, headers=None, retries=None, redirect=True, assert_same_host=True, timeout=_Default, pool_timeout=None, release_conn=None, chunked=False, body_pos=None, **response_kw):
        ...
        if not conn:
            ...
            return self.urlopen(method, url, body, headers, retries, redirect, assert_same_host, timeout=timeout, pool_timeout=pool_timeout, release_conn=release_conn, body_pos=body_pos, **response_kw)
        else:

            def drain_and_release_conn(response):
                try:
                    response.read()
                except (TimeoutError, HTTPException, SocketError, ProtocolError, BaseSSLError, SSLError) as e:
                    pass
            ...
            if redirect_location:
                ...
                return self.urlopen(method, redirect_location, body, headers, retries=retries, redirect=redirect, assert_same_host=assert_same_host, timeout=timeout, pool_timeout=pool_timeout, release_conn=release_conn, body_pos=body_pos, **response_kw)
            ...
            if retries.is_retry(method, response.status, has_retry_after):
                ...
                return self.urlopen(method, url, body, headers, retries=retries, redirect=redirect, assert_same_host=assert_same_host, timeout=timeout, pool_timeout=pool_timeout, release_conn=release_conn, body_pos=body_pos, **response_kw)
            return response
