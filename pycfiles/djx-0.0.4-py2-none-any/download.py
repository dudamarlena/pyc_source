...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-HZd96S/pip/pip/_internal/download.py
# Compiled at: 2019-02-14 00:35:06
from __future__ import absolute_import
import cgi, email.utils, getpass, json, logging, mimetypes, os, platform, re, shutil, sys
...
from pip._internal.utils.misc import ARCHIVE_EXTENSIONS, ask_path_exists, backup_dir, call_subprocess, consume, display_path, format_size, get_installed_version, rmtree, split_auth_from_netloc, splitext, unpack_file
...
def user_agent():
    ...
    """
    data = {'installer': {'name': 'pip', 'version': pip.__version__}, 'python': platform.python_version(), 
    ...
    return ('{data[installer][name]}/{data[installer][version]} {json}').format(data=data, json=json.dumps(data, separators=(',',
                                                                                                                             ':'), sort_keys=True))
...
class SafeFileCache(FileCache):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not check_path_owner(self.directory):
            logger.warning("The directory '%s' or its parent directory is not owned by the current user and the cache has been disabled. Please check the permissions and owner of that directory. If executing pip with sudo, you may want sudo's -H flag.", self.directory)
            self.directory = None
        return
...
class PipSession(requests.Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        retries = urllib3.Retry(total=retries, status_forcelist=[
         500, 503, 520, 527], backoff_factor=0.25)
        if cache:
            secure_adapter = CacheControlAdapter(cache=SafeFileCache(cache, use_dir_lock=True), max_retries=retries)
        else:
...
_scheme_re = re.compile('^(http|https|file):', re.I)
...
def unpack_url(link, location, download_dir=None, only_download=False, session=None, hashes=None, progress_bar='on'):
    """Unpack link.
    ...
    if is_vcs_url(link):
        ...
        if session is None:
            session = PipSession()
        unpack_http_url(link, location, download_dir, session, hashes=hashes, progress_bar=progress_bar)
    if only_download:
