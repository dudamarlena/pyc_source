...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-HZd96S/pip/pip/_vendor/chardet/cli/chardetect.py
...
def description_of(lines, name=b'stdin'):
    ...
    result = u.result
    if PY2:
        name = name.decode(sys.getfilesystemencoding(), b'ignore')
...
def main(argv=None):
    ...
    """
    parser = argparse.ArgumentParser(description=b'Takes one or more file paths and reports their detected                      encodings')
    parser.add_argument(b'input', help=b'File whose encoding we would like to determine.                               (default: stdin)', type=argparse.FileType(b'rb'), nargs=b'*', default=[
