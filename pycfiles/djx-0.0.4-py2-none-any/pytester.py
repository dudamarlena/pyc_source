...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/pytester.py
# Compiled at: 2019-02-14 00:35:47
"""(disabled by default) support for testing pytest and pytest plugins."""
from __future__ import absolute_import
from __future__ import division
from __future__ import print_function
import codecs, distutils.spawn, gc, os, platform, re, subprocess, sys, time, traceback
from fnmatch import fnmatch
from weakref import WeakKeyDictionary
import py, six, pytest
...
def pytest_addoption(parser):
    ...
    parser.addoption('--runpytest', default='inprocess', dest='runpytest', choices=('inprocess',
                                                                                    'subprocess'), help="run pytest sub runs in tests using an 'inprocess' or 'subprocess' (python -m main) method")
    ...
    def _exec_lsof(self):
        ...
        with open(os.devnull, 'wb') as (devnull):
            return subprocess.check_output((
             'lsof', '-Ffn0', '-p', str(pid)), stderr=devnull).decode()
...
winpymap = {'python2.7': 'C:\\Python27\\python.exe', 
   'python3.4': 'C:\\Python34\\python.exe', 
   'python3.5': 'C:\\Python35\\python.exe', 
    ...
    try:
        ...
        if executable:
            ...
            popen = subprocess.Popen([
             str(executable), '--version'], universal_newlines=True, stderr=subprocess.PIPE)
    ...
    def matchreport(self, inamepart='', names='pytest_runtest_logreport pytest_collectreport', when=None):
        ...
        if not values:
            raise ValueError('could not find test report matching %r: no test reports at all!' % (
             inamepart,))
        if len(values) > 1:
            raise ValueError('found 2 or more testreports matching %r: %s' % (inamepart, values))
        return values[0]
...
class RunResult(object):
    ...
    def assert_outcomes(self, passed=0, skipped=0, failed=0, error=0, xpassed=0, xfailed=0):
        ...
        obtained = {'passed': d.get('passed', 0), 
...
class SysPathsSnapshot(object):
    ...
    def __init__(self):
        self.__saved = (
         list(sys.path), list(sys.meta_path))
...
class Testdir(object):
    ...
    def copy_example(self, name=None):
        ...
        if name is None:
            ...
            if maybe_dir.isdir():
                ...
                raise LookupError(('{} cant be found as module or package in {}').format(func_name, example_dir.bestrelpath(self.request.confg.rootdir)))
        else:
        ...
        if example_path.isdir() and not example_path.join('__init__.py').isfile():
            ...
            if example_path.isfile():
                result = self.tmpdir.join(example_path.basename)
                example_path.copy(result)
                return result
            raise LookupError(('example "{}" is not found as a file or directory').format(example_path))
            return
    ...
    def run(self, *cmdargs, **kwargs):
        ...
        cmdargs = [ str(arg) if isinstance(arg, py.path.local) else arg for arg in cmdargs ]
    ...
    def runpytest_subprocess(self, *args, **kwargs):
        ...
        return self.run(timeout=kwargs.get('timeout'), *args)
