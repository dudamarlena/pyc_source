...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/template/loader_tags.py
# Compiled at: 2019-02-14 00:35:17
import logging, posixpath, warnings
    ...
    def find_template(self, template_name, context):
        ...
        for loader in context.template.engine.template_loaders:
            if not loader.supports_recursion:
                history = context.render_context.setdefault(self.context_key, [context.template.origin.template_name])
                if template_name in history:
                    raise ExtendsError('Cannot extend templates recursively when using non-recursive template loaders')
                template = context.template.engine.get_template(template_name)
                history.append(template_name)
                return template

        history = context.render_context.setdefault(self.context_key, [context.template.origin])
...
class IncludeNode(Node):
    ...
    def render(self, context):
        ...
        try:
            ...
            warnings.warn("Rendering {%% include '%s' %%} raised %s. In Django 2.1, this exception will be raised rather than silenced and rendered as an empty string." % (
             template_name, e.__class__.__name__), RemovedInDjango21Warning)
            logger.warning("Exception raised while rendering {%% include %%} for template '%s'. Empty string rendered instead.", template_name, exc_info=True)
            return ''
...
def construct_relative_path(current_template_name, relative_name):
    ...
    if new_name.startswith('../'):
        raise TemplateSyntaxError("The relative path '%s' points outside the file hierarchy that template '%s' is in." % (
         relative_name, current_template_name))
    if current_template_name.lstrip('/') == new_name:
        raise TemplateSyntaxError("The relative path '%s' was translated to template name '%s', the same template in which the tag appears." % (
         relative_name, current_template_name))
    return '"%s"' % new_name
