...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/attrs/attr/_make.py
# Compiled at: 2019-02-14 00:35:48
from __future__ import absolute_import, division, print_function
import copy, hashlib, linecache, sys, threading, warnings
...
_classvar_prefixes = (
 'typing.ClassVar', 't.ClassVar', 'ClassVar')
...
def attrib(default=NOTHING, validator=None, repr=True, cmp=True, hash=None, init=True, convert=None, metadata=None, type=None, converter=None, factory=None, kw_only=False):
    r"""
    ...
    if convert is not None:
        if converter is not None:
            raise RuntimeError("Can't pass both `convert` and `converter`.  Please use `converter` only.")
        warnings.warn('The `convert` argument is deprecated in favor of `converter`.  It will be removed after 2019/01.', DeprecationWarning, stacklevel=2)
        converter = convert
    ...
    if metadata is None:
        metadata = {}
    return _CountingAttr(default=default, validator=validator, repr=repr, cmp=cmp, hash=hash, init=init, converter=converter, metadata=metadata, type=type, kw_only=kw_only)
...
_Attributes = _make_attr_tuple_class('_Attributes', ['attrs',
 'base_attrs',
 'base_attrs_map'])
...
def _transform_attrs(cls, these, auto_attribs, kw_only):
    ...
    if these is not None:
        ...
        was_kw_only = False
        for a in attrs:
            if was_kw_only is False and had_default is True and a.default is NOTHING and a.init is True and a.kw_only is False:
                raise ValueError('No mandatory attributes allowed after an attribute with a default value or factory.  Attribute in question: %r' % (
                 a,))
            elif had_default is False and a.default is not NOTHING and a.init is not False and a.kw_only is False:
                had_default = True
            if was_kw_only is True and a.kw_only is False:
                raise ValueError(('Non keyword-only attributes are not allowed after a keyword-only attribute.  Attribute in question: {a!r}').format(a=a))
...
class _ClassBuilder(object):
    ...
    def __init__(self, cls, these, slots, frozen, weakref_slot, auto_attribs, kw_only, cache_hash):
    ...
    def _patch_original_class(self):
        ...
        if self._delete_attribs:
            for name in self._attr_names:
                if name not in base_names and getattr(cls, name, None) is not None:
                    try:
                        delattr(cls, name)
    ...
    def _create_slots_class(self):
        ...
        """
        base_names = self._base_names
        cd = {k:v for k, v in iteritems(self._cls_dict) if k not in tuple(self._attr_names) + ('__dict__',
                                                                                               '__weakref__')}
    ...
    def add_hash(self):
        self._cls_dict['__hash__'] = self._add_method_dunders(_make_hash(self._attrs, frozen=self._frozen, cache_hash=self._cache_hash))
        return self

    def add_init(self):
        self._cls_dict['__init__'] = self._add_method_dunders(_make_init(self._attrs, self._has_post_init, self._frozen, self._slots, self._cache_hash, self._base_attr_map))
        return self

    def add_cmp(self):
        cd = self._cls_dict
        cd['__eq__'], cd['__ne__'], cd['__lt__'], cd['__le__'], cd['__gt__'], cd['__ge__'] = (self._add_method_dunders(meth) for meth in _make_cmp(self._attrs))
        return self
...
def attrs(maybe_cls=None, these=None, repr_ns=None, repr=True, cmp=True, hash=None, init=True, slots=False, frozen=False, weakref_slot=True, str=False, auto_attribs=False, kw_only=False, cache_hash=False):
    r"""
    ...
    def wrap(cls):
        ...
        builder = _ClassBuilder(cls, these, slots, frozen, weakref_slot, auto_attribs, kw_only, cache_hash)
        if repr is True:
...
def _make_hash(attrs, frozen, cache_hash):
    attrs = tuple(a for a in attrs if a.hash is True or a.hash is None and a.cmp is True)
    tab = '        '
    sha1 = hashlib.sha1()
...
WARNING_CMP_ISINSTANCE = 'Comparision of subclasses using __%s__ is deprecated and will be removed in 2019.'
...
def _make_cmp(attrs):
    ...
    def __lt__(self, other):
        ...
        if isinstance(other, self.__class__):
            if other.__class__ is not self.__class__:
                warnings.warn(WARNING_CMP_ISINSTANCE % ('lt', ), DeprecationWarning)
    ...
    def __le__(self, other):
        ...
        if isinstance(other, self.__class__):
            if other.__class__ is not self.__class__:
                warnings.warn(WARNING_CMP_ISINSTANCE % ('le', ), DeprecationWarning)
            return attrs_to_tuple(self) <= attrs_to_tuple(other)
        else:
            return NotImplemented
    ...
    def __gt__(self, other):
        ...
        if isinstance(other, self.__class__):
            if other.__class__ is not self.__class__:
                warnings.warn(WARNING_CMP_ISINSTANCE % ('gt', ), DeprecationWarning)
            return attrs_to_tuple(self) > attrs_to_tuple(other)
        else:
            return NotImplemented
    ...
    def __ge__(self, other):
        ...
        if isinstance(other, self.__class__):
            if other.__class__ is not self.__class__:
                warnings.warn(WARNING_CMP_ISINSTANCE % ('ge', ), DeprecationWarning)
            return attrs_to_tuple(self) >= attrs_to_tuple(other)
        else:
            return NotImplemented
...
def _add_cmp(cls, attrs=None):
    ...
    cls.__eq__, cls.__ne__, cls.__lt__, cls.__le__, cls.__gt__, cls.__ge__ = _make_cmp(attrs)
    return cls
...
def _make_init(attrs, post_init, frozen, slots, cache_hash, base_attr_map):
    ...
    script, globs, annotations = _attrs_to_init_script(attrs, frozen, slots, post_init, cache_hash, base_attr_map)
    locs = {}
    bytecode = compile(script, unique_filename, 'exec')
...
def _add_init(cls, frozen):
    ...
    """
    cls.__init__ = _make_init(cls.__attrs_attrs__, getattr(cls, '__attrs_post_init__', False), frozen, _is_slot_cls(cls), cache_hash=False, base_attr_map={})
    return cls
...
def _attrs_to_init_script(attrs, frozen, slots, post_init, cache_hash, base_attr_map):
    """
    ...
    if frozen is True:
        if slots is True:
            ...
            def fmt_setter(attr_name, value_var):
                return "_setattr('%(attr_name)s', %(value_var)s)" % {'attr_name': attr_name, 
                   'value_var': value_var}

            def fmt_setter_with_converter(attr_name, value_var):
                conv_name = _init_converter_pat.format(attr_name)
                return "_setattr('%(attr_name)s', %(conv)s(%(value_var)s))" % {'attr_name': attr_name, 
                   'value_var': value_var, 
                   'conv': conv_name}
            ...
            def fmt_setter(attr_name, value_var):
                if _is_slot_attr(attr_name, base_attr_map):
                    res = "_setattr('%(attr_name)s', %(value_var)s)" % {'attr_name': attr_name, 
                       'value_var': value_var}
                else:
                    res = "_inst_dict['%(attr_name)s'] = %(value_var)s" % {'attr_name': attr_name, 
                       'value_var': value_var}
                return res
            ...
            def fmt_setter_with_converter(attr_name, value_var):
                ...
                return tmpl % {'attr_name': attr_name, 
                   'value_var': value_var, 
                   'c': conv_name}
        ...
        def fmt_setter(attr_name, value):
            return 'self.%(attr_name)s = %(value)s' % {'attr_name': attr_name, 
               'value': value}

        def fmt_setter_with_converter(attr_name, value_var):
            conv_name = _init_converter_pat.format(attr_name)
            return 'self.%(attr_name)s = %(conv)s(%(value_var)s)' % {'attr_name': attr_name, 
               'value_var': value_var, 
               'conv': conv_name}
    ...
    for a in attrs:
        ...
        if a.init is False:
            if has_factory:
                ...
                lines.append(fmt_setter(attr_name, ("attr_dict['{attr_name}'].default").format(attr_name=attr_name)))
    ...
    if kw_only_args:
        if PY2:
            raise PythonTooOldError('Keyword-only arguments only work on Python 3 and later.')
        args += ('{leading_comma}*, {kw_only_args}').format(leading_comma=', ' if args else '', kw_only_args=(', ').join(kw_only_args))
    return (
...
class Attribute(object):
    ...
    *Read-only* representation of an attribute.
    ...
    """
    __slots__ = ('name', 'default', 'validator', 'repr', 'cmp', 'hash', 'init', 'metadata',
                 'type', 'converter', 'kw_only')

    def __init__(self, name, default, validator, repr, cmp, hash, init, convert=None, metadata=None, type=None, converter=None, kw_only=False):
    ...
    @property
    def convert(self):
        warnings.warn('The `convert` attribute is deprecated in favor of `converter`.  It will be removed after 2019/01.', DeprecationWarning, stacklevel=2)
    ...
    def from_counting_attr(cls, name, ca, type=None):
        ...
        inst_dict = {k:getattr(ca, k) for k in Attribute.__slots__ if k not in ('name',
                                                                                'validator',
                                                                                'default',
                                                                                'type',
                                                                                'convert')}
        return cls(name=name, validator=ca._validator, default=ca._default, type=type, **inst_dict)
...
_a = [ Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=True, hash=name != 'metadata', init=True) for name in Attribute.__slots__ if name != 'convert'
     ]
Attribute = _add_hash(_add_cmp(_add_repr(Attribute, attrs=_a), attrs=_a), attrs=[ a for a in _a if a.hash ])
...
class _CountingAttr(object):
    ...
    *Internal* data structure of the attrs library.  Running into is most
    ...
    """
    __slots__ = ('counter', '_default', 'repr', 'cmp', 'hash', 'init', 'metadata',
                 '_validator', 'converter', 'type', 'kw_only')
    __attrs_attrs__ = tuple(Attribute(name=name, default=NOTHING, validator=None, repr=True, cmp=True, hash=True, init=True, kw_only=False) for name in ('counter',
                                                                                                                                                         '_default',
                                                                                                                                                         'repr',
                                                                                                                                                         'cmp',
                                                                                                                                                         'hash',
                                                                                                                                                         'init')) + (
     Attribute(name='metadata', default=None, validator=None, repr=True, cmp=True, hash=False, init=True, kw_only=False),)
    cls_counter = 0

    def __init__(self, default, validator, repr, cmp, hash, init, converter, metadata, type, kw_only):
