...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/db/models/functions/datetime.py
...
class TruncBase(TimezoneMixin, Transform):
    ...
    def as_sql(self, compiler, connection):
        ...
        return (
         sql, inner_params + params)

    def resolve_expression(self, query=None, allow_joins=True, reuse=None, summarize=False, for_save=False):
        copy = super(TruncBase, self).resolve_expression(query, allow_joins, reuse, summarize, for_save)
        field = copy.lhs.output_field
        if not isinstance(field, (DateField, TimeField)):
            raise AssertionError("%r isn't a DateField, TimeField, or DateTimeField." % field.name)
            raise (isinstance(copy.output_field, (DateField, DateTimeField, TimeField)) or ValueError)('output_field must be either DateField, TimeField, or DateTimeField')
        output_field = copy.output_field
        explicit_output_field = field.__class__ != copy.output_field.__class__
        if type(field) == DateField and (isinstance(output_field, DateTimeField) or copy.kind in ('hour',
                                                                                                  'minute',
                                                                                                  'second',
                                                                                                  'time')):
            raise ValueError("Cannot truncate DateField '%s' to %s. " % (
             field.name, output_field.__class__.__name__ if explicit_output_field else 'DateTimeField'))
        elif isinstance(field, TimeField) and (isinstance(output_field, DateTimeField) or copy.kind in ('year',
                                                                                                        'month',
                                                                                                        'day',
                                                                                                        'date')):
