...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-QtVhoA/setuptools/setuptools/dist.py
...
def get_metadata_version(self):
    ...
    if mv is None:
        ...
        elif self.maintainer is not None or self.maintainer_email is not None or getattr(self, 'python_requires', None) is not None:
...
def check_package_data(dist, attr, value):
    ...
    raise DistutilsSetupError(attr + ' must be a dictionary mapping package names to lists of wildcard patterns')
...
class Distribution(_Distribution):
    ...
    def _move_install_requirements_markers(self):
        ...
        self.extras_require = dict((k, [ str(r) for r in map(self._clean_req, v) ]) for k, v in self._tmp_extras_require.items())
        return
    ...
    def fetch_build_eggs(self, requires):
        ...
        resolved_dists = pkg_resources.working_set.resolve(pkg_resources.parse_requirements(requires), installer=self.fetch_build_egg, replace_conflicting=True)
