...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-QtVhoA/setuptools/setuptools/package_index.py
    ...
    if basename.endswith('.egg') and '-' in basename:
        ...
        if basename.endswith('.whl') and '-' in basename:
            ...
            if not wheel.is_compatible():
                return []
            return [
             Distribution(location=location, project_name=wheel.project_name, version=wheel.version, precedence=EGG_DIST + 1)]
...
class PackageIndex(Environment):
    ...
    def gen_setup(self, filename, fragment, tmpdir):
        match = EGG_FRAGMENT.match(fragment)
        dists = match and [ d for d in interpret_distro_name(filename, match.group(1), None) if d.version
                          ] or []
        if len(dists) == 1:
