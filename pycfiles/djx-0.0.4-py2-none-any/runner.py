...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/runner.py
...
class CallInfo(object):
    ...
    def __repr__(self):
        if self.excinfo is not None:
            ...
            status = 'result'
        return ('<CallInfo when={when!r} {status}: {value}>').format(when=self.when, value=value, status=status)
...
class SetupState(object):
    ...
    def _teardown_with_finalization(self, colitem):
        ...
        for colitem in self._finalizers:
            assert colitem is None or colitem in self.stack or isinstance(colitem, tuple)
