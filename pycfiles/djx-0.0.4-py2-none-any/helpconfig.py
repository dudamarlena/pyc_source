...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/helpconfig.py
...
class HelpAction(Action):
    ...
    def __init__(self, option_strings, dest=None, default=False, help=None):
        super(HelpAction, self).__init__(option_strings=option_strings, dest=dest, const=True, default=default, nargs=0, help=help)
...
def pytest_addoption(parser):
    ...
    group.addoption('--version', action='store_true', help='display pytest lib version and import information.')
    group._addoption('-h', '--help', action=HelpAction, dest='help', help='show help message and configuration info')
    group._addoption('-p', action='append', dest='plugins', default=[], metavar='name', help='early-load given plugin (multi-allowed). To avoid loading of plugins, use the `no:` prefix, e.g. `no:doctest`.')
    (
     group.addoption('--traceconfig', '--trace-config', action='store_true', default=False, help='trace considerations of conftest.py files.'),)
    group.addoption('--debug', action='store_true', dest='debug', default=False, help="store internal tracing debug information in 'pytestdebug.log'.")
...
def pytest_cmdline_parse():
    ...
    if config.option.debug:
        ...
        debugfile.write('versions pytest-%s, py-%s, python-%s\ncwd=%s\nargs=%s\n\n' % (
         pytest.__version__,
         py.__version__,
