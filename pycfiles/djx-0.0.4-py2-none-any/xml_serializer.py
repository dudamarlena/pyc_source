...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/core/serializers/xml_serializer.py
...
class Serializer(base.Serializer):
    ...
    def indent(self, level):
        if self.options.get(b'indent') is not None:
            self.xml.ignorableWhitespace(b'\n' + b' ' * self.options.get(b'indent') * level)
        return
    ...
    def start_object(self, obj):
        ...
        self.indent(1)
        attrs = OrderedDict([(b'model', force_text(obj._meta))])
        if not self.use_natural_primary_keys or not hasattr(obj, b'natural_key'):
            obj_pk = obj._get_pk_val()
            if obj_pk is not None:
                attrs[b'pk'] = force_text(obj_pk)
    ...
    def handle_field(self, obj, field):
        ...
        """
        self.indent(2)
        self.xml.startElement(b'field', OrderedDict([
         (
          b'name', field.name),
         (
        ...
        if getattr(obj, field.name) is not None:
            try:
                ...
                raise ValueError(b'%s.%s (pk:%s) contains unserializable characters' % (
    ...
    def handle_fk_field(self, obj, field):
        ...
        if related_att is not None:
            if self.use_natural_foreign_keys and hasattr(field.remote_field.model, b'natural_key'):
    ...
    def handle_m2m_field(self, obj, field):
        ...
        if field.remote_field.through._meta.auto_created:
            ...
            if self.use_natural_foreign_keys and hasattr(field.remote_field.model, b'natural_key'):
                ...
                def handle_m2m(value):
                    self.xml.addQuickElement(b'object', attrs={b'pk': force_text(value._get_pk_val())})
    ...
    def _start_relational_field(self, field):
        ...
        self.xml.startElement(b'field', OrderedDict([
         (
          b'name', field.name),
         (
          b'rel', field.remote_field.__class__.__name__),
         (
          b'to', force_text(field.remote_field.model._meta))]))
...
class Deserializer(base.Deserializer):
    ...
    def __init__(self, stream_or_string, **options):
        ...
        self.ignore = options.pop(b'ignorenonexistent', False)
    ...
    def _handle_object(self, node):
        ...
        if node.hasAttribute(b'pk'):
            data[Model._meta.pk.attname] = Model._meta.pk.to_python(node.getAttribute(b'pk'))
        m2m_data = {}
    ...
    def _handle_fk_field_node(self, node, field):
        ...
        if node.getElementsByTagName(b'None'):
            ...
            if hasattr(model._default_manager, b'get_by_natural_key'):
    ...
    def _handle_m2m_field_node(self, node, field):
        ...
        if hasattr(default_manager, b'get_by_natural_key'):
            ...
            def m2m_convert(n):
                ...
                if keys:
                    ...
                    obj_pk = model._meta.pk.to_python(n.getAttribute(b'pk'))
                return obj_pk

        else:

            def m2m_convert(n):
                return model._meta.pk.to_python(n.getAttribute(b'pk'))
