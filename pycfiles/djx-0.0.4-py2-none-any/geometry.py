...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/contrib/gis/geos/geometry.py
...
class GEOSGeometry(GEOSBase, ListMixin):
    ...
    def _post_init(self, srid):
        ...
        if type(self) == GEOSGeometry:
            if GEOSGeometry._GEOS_CLASSES is None:
                ...
                GEOSGeometry._GEOS_CLASSES = {0: Point, 
                   1: LineString, 
                   2: LinearRing, 
                   3: Polygon, 
                   4: MultiPoint, 
    ...
    def __eq__(self, other):
        ...
        if isinstance(other, six.string_types):
            if other.startswith(b'SRID=0;'):
                return self.ewkt == other[7:]
    ...
    def get_srid(self):
        warnings.warn(b'`get_srid()` is deprecated, use the `srid` property instead.', RemovedInDjango20Warning, 2)
        return self.srid

    def set_srid(self, srid):
        warnings.warn(b'`set_srid()` is deprecated, use the `srid` property instead.', RemovedInDjango20Warning, 2)
        self.srid = srid
    ...
    def ewkt(self):
        ...
        srid = self.srid
        if srid:
            return b'SRID=%s;%s' % (srid, self.wkt)
        return self.wkt
    ...
    def json(self):
        ...
        """
        return json.dumps({b'type': self.__class__.__name__, b'coordinates': self.coords})
