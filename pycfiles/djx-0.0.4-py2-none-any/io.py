...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/contrib/gis/geos/prototypes/io.py
# Compiled at: 2019-02-14 00:35:16
import threading
from ctypes import POINTER, Structure, byref, c_char, c_char_p, c_int, c_size_t
from django.contrib.gis.geos.base import GEOSBase
from django.contrib.gis.geos.libgeos import GEOM_PTR, GEOSFuncFactory, geos_version_info
...
wkt_reader_read = GEOSFuncFactory('GEOSWKTReader_read', argtypes=[WKT_READ_PTR, c_char_p], restype=GEOM_PTR, errcheck=check_geom)
...
wkt_writer_write = GEOSFuncFactory('GEOSWKTWriter_write', argtypes=[WKT_WRITE_PTR, GEOM_PTR], restype=geos_char_p, errcheck=check_string)
...
class IOBase(GEOSBase):
    ...
    def __init__(self):
        ...
        self.destructor.func = self.destructor.get_func(*self.destructor.args, **self.destructor.kwargs)
...
class WKTWriter(IOBase):
    ...
    def trim(self, flag):
        if bool(flag) != self._trim:
            ...
            wkt_writer_set_trim(self.ptr, '\x01' if flag else '\x00')
