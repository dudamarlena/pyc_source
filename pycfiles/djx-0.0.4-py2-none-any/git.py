...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-HZd96S/pip/pip/_internal/vcs/git.py
...
class Git(VersionControl):
    ...
    def get_revision_sha(self, dest, rev):
        ...
        """
        output = self.run_command(['show-ref', rev], cwd=dest, show_stdout=False, on_returncode='ignore')
        refs = {}
    ...
    def resolve_revision(self, dest, url, rev_options):
        ...
        if sha is not None:
            ...
            if not rev.startswith('refs/'):
                return rev_options
            self.run_command([
             'fetch', '-q', url] + rev_options.to_args(), cwd=dest)
    ...
    def fetch_new(self, dest, url, rev_options):
        ...
        logger.info('Cloning %s%s to %s', redact_password_from_url(url), rev_display, display_path(dest))
    ...
    def get_remote_url(cls, location):
        ...
        stdout = cls.run_command([
         'config', '--get-regexp', 'remote\\..*\\.url'], extra_ok_returncodes=(1, ), show_stdout=False, cwd=location)
    ...
    def _get_subdirectory(cls, location):
        ...
        git_dir = cls.run_command(['rev-parse', '--git-dir'], show_stdout=False, cwd=location).strip()
    ...
    def update_submodules(self, location):
        if not os.path.exists(os.path.join(location, '.gitmodules')):
            return
        self.run_command([
         'submodule', 'update', '--init', '--recursive', '-q'], cwd=location)
    ...
    def controls_location(cls, location):
        if super(Git, cls).controls_location(location):
            return True
        try:
            r = cls.run_command(['rev-parse'], cwd=location, show_stdout=False, on_returncode='ignore')
            return not r
        except BadCommand:
