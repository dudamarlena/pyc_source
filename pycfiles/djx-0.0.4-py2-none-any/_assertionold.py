...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/py/py/_code/_assertionold.py
...
class Compare(Interpretable):
    ...
    def eval(self, frame):
        ...
        for operation, expr2 in self.ops:
            ...
            try:
                self.result = frame.eval(source, __exprinfo_left=expr.result, __exprinfo_right=expr2.result)
            except passthroughex:
                raise
            except:
                raise Failure(self)
...
def check(s, frame=None):
    ...
    if not isinstance(expr, ast.Expression):
        ...
        frame.is_true(node.result) or sys.stderr.write('assertion failed: %s\n' % node.nice_explanation())
    return
