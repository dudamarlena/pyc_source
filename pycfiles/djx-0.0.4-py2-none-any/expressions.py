...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/db/models/expressions.py
...
class BaseExpression(object):
    ...
    def get_db_converters(self, connection):
        return [
         self.convert_value] + self.output_field.get_db_converters(connection)
    ...
    def _parse_expressions(self, *expressions):
        return [ arg if hasattr(arg, 'resolve_expression') else F(arg) if isinstance(arg, six.string_types) else Value(arg) for arg in expressions
               ]
    ...
    def resolve_expression(self, query=None, allow_joins=True, reuse=None, summarize=False, for_save=False):
        ...
        c.set_source_expressions([ expr.resolve_expression(query, allow_joins, reuse, summarize) for expr in c.get_source_expressions()
                                 ])
        return c
    ...
    def relabeled_clone(self, change_map):
        clone = self.copy()
        clone.set_source_expressions([ e.relabeled_clone(change_map) for e in self.get_source_expressions() ])
        return clone
        ...
        if not isinstance(side, DurationValue):
            try:
                ...
                if output.get_internal_type() == 'DurationField':
                    ...
                    return (
                     connection.ops.format_for_duration_arithmetic(sql), params)
...
class Func(Expression):
    ...
    def __init__(self, *expressions, **extra):
        if self.arity is not None and len(expressions) != self.arity:
            raise TypeError("'%s' takes exactly %s %s (%s given)" % (
...
class Ref(Expression):
    ...
    def as_sql(self, compiler, connection):
        return (
         '%s' % connection.ops.quote_name(self.refs), [])
...
class When(Expression):
    ...
    def get_source_fields(self):
        return [
         self.result._output_field_or_none]
...
class Subquery(Expression):
    ...
    def resolve_expression(self, query=None, allow_joins=True, reuse=None, summarize=False, for_save=False):
        ...
        def resolve(child):
            if hasattr(child, 'resolve_expression'):
                resolved = child.resolve_expression(query=query, allow_joins=allow_joins, reuse=reuse, summarize=summarize, for_save=for_save)
    ...
    def get_source_expressions(self):
        return [ x for x in [ getattr(expr, 'lhs', None) for expr in self.queryset.query.where.children
                            ] if x
               ]
    ...
    def relabeled_clone(self, change_map):
        ...
        clone.queryset.query.external_aliases.update(alias for alias in change_map.values() if alias not in clone.queryset.query.tables)
        return clone
...
class OrderBy(BaseExpression):
    ...
    def __repr__(self):
        return ('{}({}, descending={})').format(self.__class__.__name__, self.expression, self.descending)
