...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/gunicorn/gunicorn/glogging.py
# Compiled at: 2019-02-14 00:35:18
import base64, binascii, time, logging
...
SYSLOG_FACILITIES = {'auth': 4, 
   ...
   'local6': 22, 
   'local7': 23}
CONFIG_DEFAULTS = dict(version=1, disable_existing_loggers=False, loggers={'root': {'level': 'INFO', 'handlers': ['console']}, 'gunicorn.error': {'level': 'INFO', 
                      'handlers': [
                                 'error_console'], 
                      'propagate': True, 
   ...
   'gunicorn.access': {'level': 'INFO', 
                       'handlers': [
                                  'console'], 
                       'propagate': True, 
                       'qualname': 'gunicorn.access'}}, handlers={'console': {'class': 'logging.StreamHandler', 
               'formatter': 'generic', 
               'stream': 'ext://sys.stdout'}, 
   'error_console': {'class': 'logging.StreamHandler', 
                     'formatter': 'generic', 
                     'stream': 'ext://sys.stderr'}}, formatters={'generic': {'format': '%(asctime)s [%(process)d] [%(levelname)s] %(message)s', 
...
class Logger(object):
    ...
    def setup(self, cfg):
        ...
        if dictConfig is None and cfg.logconfig_dict:
            util.warn('Dictionary-based log configuration requires Python 2.7 or above.')
    ...
    def atoms(self, resp, req, environ, request_time):
        ...
        atoms = {'h': environ.get('REMOTE_ADDR', '-'), 'l': '-', 
    ...
    def access(self, resp, req, environ, request_time):
        ...
        for format details
        """
        if not (self.cfg.accesslog or self.cfg.logconfig or self.cfg.logconfig_dict or self.cfg.syslog and not self.cfg.disable_redirect_access_to_syslog):
            return
