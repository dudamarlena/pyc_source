...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/db/models/sql/compiler.py
...
class SQLCompiler(object):
    ...
    def collapse_group_by(self, expressions, having):
        if self.connection.features.allows_group_by_pk:
            pk = None
            for expr in expressions:
                if getattr(expr, 'target', None) == self.query.model._meta.pk and getattr(expr, 'alias', None) == self.query.tables[0]:
                    pk = expr
                    break
            ...
            pks = {expr for expr in expressions if hasattr(expr, 'target') and expr.target.primary_key if hasattr(expr, 'target') and expr.target.primary_key}
        ...
        if not restricted and self.query.max_depth and cur_depth > self.query.max_depth:
            ...
            for f in opts.fields:
                ...
                if restricted:
                    ...
                    if not f.is_relation:
                        if next or f.name in requested:
                            raise FieldError("Non-relational field given in select_related: '%s'. Choices are: %s" % (
                             f.name,
                ...
                if not select_related_descend(f, restricted, requested, only_load.get(field_model)):
                    continue
                klass_info = {'model': f.remote_field.model, 
                   'field': f, 
                   'reverse': False, 
                   'from_parent': False}
            ...
            if restricted:
                ...
                if fields_not_found:
                    ...
                    raise FieldError('Invalid field name(s) given in select_related: %s. Choices are: %s' % (
                     (', ').join(invalid_fields),
                     (', ').join(_get_field_choices()) or '(none)'))
            return related_klass_infos
...
class SQLInsertCompiler(SQLCompiler):
    ...
    def prepare_value(self, field, value):
        ...
        if hasattr(value, 'resolve_expression'):
            ...
            if value.contains_column_references:
                raise ValueError('Failed to insert expression "%s" on %s. F() expressions can only be used to update, not to insert.' % (
                 value, field))
    ...
    def execute_sql(self, return_id=False):
        assert not (return_id and len(self.query.objs) != 1 and not self.connection.features.can_return_ids_from_bulk_insert)
        ...
        with self.connection.cursor() as (cursor):
            ...
            return self.connection.ops.last_insert_id(cursor, self.query.get_meta().db_table, self.query.get_meta().pk.column)
            ...
            for field, model, val in self.query.values:
                if hasattr(val, 'resolve_expression'):
                    ...
                    if field.remote_field:
                        ...
                        raise TypeError('Tried to update field %s with a model instance, %r. Use a value compatible with %s.' % (
