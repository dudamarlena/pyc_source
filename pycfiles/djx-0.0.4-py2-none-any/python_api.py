...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/python_api.py
# Compiled at: 2019-02-14 00:35:47
from __future__ import absolute_import
import math, pprint, sys, warnings
    ...
    def _yield_comparisons(self, actual):
        ...
        if np.isscalar(actual):
            for i in np.ndindex(self.expected.shape):
                yield (
                 actual, self.expected[i].item())

        else:
            for i in np.ndindex(self.expected.shape):
                yield (
                 actual[i].item(), self.expected[i].item())
    ...
    def _check_type(self):
        ...
        for index, x in enumerate(self.expected):
            if isinstance(x, type(self.expected)):
                ...
                raise _non_numeric_type_error(self.expected, at=('index {}').format(index))
...
class ApproxScalar(ApproxBase):
    ...
    DEFAULT_RELATIVE_TOLERANCE = 1e-06
