...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pluggy/pluggy/manager.py
...
def _warn_for_function(warning, function):
    warnings.warn_explicit(warning, type(warning), lineno=function.__code__.co_firstlineno, filename=function.__code__.co_filename)
...
class PluginManager(object):
    ...
    def __init__(self, project_name, implprefix=None):
        ...
        if implprefix is not None:
            warnings.warn('Support for the `implprefix` arg is now deprecated and will be removed in an upcoming release. Please use HookimplMarker.', DeprecationWarning, stacklevel=2)
    ...
    def parse_hookimpl_opts(self, plugin, name):
        ...
        if not inspect.isroutine(method):
            ...
            if res is not None and not isinstance(res, dict):
                ...
                _warn_for_function(DeprecationWarning('The `implprefix` system is deprecated please decorate this function using an instance of HookimplMarker.'), method)
                res = {}
            return res
                ...
                if not hook.has_spec():
                    for hookimpl in hook.get_hookimpls():
                        if not hookimpl.optionalhook:
                            raise PluginValidationError(hookimpl.plugin, 'unknown hook %r in plugin %r' % (
    ...
    def subset_hook_caller(self, name, remove_plugins):
        ...
        if plugins_to_remove:
            hc = _HookCaller(orig.name, orig._hookexec, orig.spec.namespace, orig.spec.opts)
