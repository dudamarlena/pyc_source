...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/terminal.py
...
import argparse, collections, platform, sys, time, attr, pluggy, py, six
...
class MoreQuietAction(argparse.Action):
    ...
    def __init__(self, option_strings, dest, default=None, required=False, help=None):
        super(MoreQuietAction, self).__init__(option_strings=option_strings, dest=dest, nargs=0, default=default, required=required, help=help)
...
def pytest_addoption(parser):
    ...
    group._addoption('--verbosity', dest='verbose', type=int, default=0, help='set verbosity')
    group._addoption('-r', action='store', dest='reportchars', default='', metavar='chars', help='show extra test summary info as specified by chars (f)ailed, (E)error, (s)skipped, (x)failed, (X)passed, (p)passed, (P)passed with output, (a)all except pP. Warnings are displayed at all times except when --disable-warnings is set')
    ...
    group._addoption('--tb', metavar='style', action='store', dest='tbstyle', default='auto', choices=[
     'auto', 'long', 'short', 'no', 'line', 'native'], help='traceback print mode (auto/long/short/line/native/no).')
    group._addoption('--show-capture', action='store', dest='showcapture', choices=[
     'no', 'stdout', 'stderr', 'log', 'all'], default='all', help="Controls how captured stdout/stderr/log is shown on failed tests. Default is 'all'.")
    group._addoption('--fulltrace', '--full-trace', action='store_true', default=False, help="don't cut any tracebacks (default is to cut).")
    group._addoption('--color', metavar='color', action='store', dest='color', default='auto', choices=[
     'yes', 'no', 'auto'], help='color terminal output (yes/no/auto).')
...
def pytest_report_teststatus(report):
    if report.passed:
        ...
        if report.when != 'call':
            letter = 'f'
    return (
     report.outcome, letter, report.outcome.upper())
...
class TerminalReporter(object):
    ...
    def pytest_warning_captured(self, warning_message, item):
        ...
        warning_report = WarningReport(fslocation=fslocation, message=message, nodeid=nodeid)
    ...
    def report_collect(self, final=False):
        if self.config.option.verbose < 0:
            ...
            if not final:
                t = time.time()
                if self._collect_report_last_write is not None and self._collect_report_last_write > t - 0.5:
                    return
    ...
    def pytest_sessionstart(self, session):
        ...
        if not self.showheader:
            ...
            msg += ', pytest-%s, py-%s, pluggy-%s' % (
             ...
             py.__version__,
             pluggy.__version__)
            if self.verbosity > 0 or self.config.option.debug or getattr(self.config.option, 'pastebin', None):
    ...
    def pytest_report_header(self, config):
        ...
        lines = [
         'rootdir: %s, inifile:%s' % (config.rootdir, inifile)]
    ...
    def pytest_sessionfinish(self, exitstatus):
        ...
        if exitstatus in summary_exit_codes:
            self.config.hook.pytest_terminal_summary(terminalreporter=self, exitstatus=exitstatus, config=self.config)
    ...
    def summary_deprecated_python(self):
        if sys.version_info[:2] <= (3, 4) and self.verbosity >= 0:
            ...
            self.line(('You are using Python {}, which will no longer be supported in pytest 5.0').format(using_version), yellow=True, bold=False)
