...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/doctest.py
...
def _init_runner_class():
    ...
    class PytestDoctestRunner(doctest.DebugRunner):
        ...
        def __init__(self, checker=None, verbose=None, optionflags=0, continue_on_failure=True):
            doctest.DebugRunner.__init__(self, checker=checker, verbose=verbose, optionflags=optionflags)
...
class DoctestItem(pytest.Item):
    ...
    def reportinfo(self):
        return (
         self.fspath, self.dtest.lineno, '[doctest] %s' % self.name)
...
class DoctestTextfile(pytest.Module):
    ...
    def collect(self):
        ...
        runner = _get_runner(verbose=0, optionflags=optionflags, checker=_get_checker(), continue_on_failure=_get_continue_on_failure(self.config))
...
def _get_report_choice(key):
    ...
    """
    import doctest
    return {DOCTEST_REPORT_CHOICE_UDIFF: doctest.REPORT_UDIFF, 
