...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-QtVhoA/setuptools/setuptools/_vendor/packaging/version.py
...
def _parse_local_version(local):
    ...
    if local is not None:
        return tuple((part.lower() if 1 else int(part)) for part in _local_version_seperators.split(local) if not part.isdigit())
    else:
        return
