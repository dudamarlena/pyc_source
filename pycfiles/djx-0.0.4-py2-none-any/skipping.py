...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/skipping.py
...
def pytest_addoption(parser):
    ...
    group.addoption('--runxfail', action='store_true', dest='runxfail', default=False, help='run tests even if they are marked xfail')
...
def pytest_configure(config):
    ...
    config.addinivalue_line('markers', "xfail(condition, reason=None, run=True, raises=None, strict=False): mark the test function as an expected failure if eval(condition) has a True value. Optionally specify a reason for better reporting and run=False if you don't even want to execute the test function. If only specific exception(s) are expected, you can list them in raises, and if the test fails in other ways, it will be reported as a true failure. See https://docs.pytest.org/en/latest/skipping.html")
...
def _get_report_str(terminalreporter, report):
    _category, _short, verbose = terminalreporter.config.hook.pytest_report_teststatus(report=report, config=terminalreporter.config)
    return verbose


REPORTCHAR_ACTIONS = {'x': show_xfailed, 
   'X': show_xpassed, 
   'f': shower('failed'), 
   'F': shower('failed'), 
   's': show_skipped, 
