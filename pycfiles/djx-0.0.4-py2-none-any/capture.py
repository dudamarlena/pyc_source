...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/capture.py
...
def pytest_addoption(parser):
    ...
    group._addoption('--capture', action='store', default='fd' if hasattr(os, 'dup') else 'sys', metavar='method', choices=[
     'fd', 'sys', 'no'], help='per-test capturing method: one of fd|sys|no.')
...
def _ensure_only_one_capture_fixture(request, name):
    ...
    if fixtures:
        ...
        raise request.raiseerror(('cannot use {} and {} at the same time').format(fixtures, name))
...
class MultiCapture(object):
    ...
    def readouterr(self):
        ...
        return CaptureResult(self.out.snap() if self.out is not None else '', self.err.snap() if self.err is not None else '')
