...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-HZd96S/pip/pip/_internal/commands/search.py
...
def print_results(hits, name_column_width=None, terminal_width=None):
    if not hits:
        ...
        if name_column_width is None:
            name_column_width = max([ len(hit['name']) + len(highest_version(hit.get('versions', ['-']))) for hit in hits
                                    ]) + 4
