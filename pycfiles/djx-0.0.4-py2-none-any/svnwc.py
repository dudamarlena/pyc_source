...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/py/py/_path/svnwc.py
...
class SvnWCCommandPath(common.PathBase):
    ...
    def checkout(self, url=None, rev=None):
        ...
        if rev is None or rev == -1:
            if sys.platform != 'win32' and _getsvnversion() == '1.3':
                url += '@HEAD'
    ...
    def remove(self, rec=1, force=1):
        ...
        if not rec:
            ...
            self.check(versioned=True) or py.path.local(self).remove()
            return
        flags = []
        if force:
            flags.append('--force')
