...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/http/multipartparser.py
...
import base64, binascii, cgi, sys
...
__all__ = (
 b'MultiPartParser', b'MultiPartParserError', b'InputStreamExhausted')
...
class MultiPartParser(object):
    ...
    def __init__(self, META, input_data, upload_handlers, encoding=None):
        ...
        """
        content_type = META.get(b'CONTENT_TYPE', b'')
        ...
        try:
            content_length = int(META.get(b'CONTENT_LENGTH', 0))
                ...
                for item_type, meta_data, field_stream in Parser(stream, self._boundary):
                    ...
                    if item_type == FIELD:
                        ...
                        if settings.DATA_UPLOAD_MAX_NUMBER_FIELDS is not None and settings.DATA_UPLOAD_MAX_NUMBER_FIELDS < num_post_keys:
                            raise TooManyFieldsSent(b'The number of GET/POST parameters exceeded settings.DATA_UPLOAD_MAX_NUMBER_FIELDS.')
                        ...
                        if settings.DATA_UPLOAD_MAX_MEMORY_SIZE is not None and num_bytes_read > settings.DATA_UPLOAD_MAX_MEMORY_SIZE:
                            raise RequestDataTooBig(b'Request body exceeded settings.DATA_UPLOAD_MAX_MEMORY_SIZE.')
                        ...
                        if not file_name:
                            continue
                        content_type, content_type_extra = meta_data.get(b'content-type', (b'', {}))
                        content_type = content_type.strip()
                        charset = content_type_extra.get(b'charset')
                        try:
                            ...
                            for chunk in field_stream:
                                if transfer_encoding == b'base64':
                                    ...
                                    try:
                                        ...
                                        msg = b'Could not decode base64 data: %r' % e
...
class LazyStream(six.Iterator):
    ...
    def _update_unget_history(self, num_bytes):
        ...
        if number_equal > 40:
            raise SuspiciousMultipartForm(b"The multipart parser got stuck, which shouldn't happen with normal uploaded files. Check for malicious upload activity; if there is none, report this to the Django developers.")
