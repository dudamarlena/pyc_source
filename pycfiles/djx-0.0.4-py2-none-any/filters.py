...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/contrib/admin/filters.py
...
class SimpleListFilter(ListFilter):
    ...
    def choices(self, changelist):
        yield {'selected': self.value() is None, 
           'query_string': changelist.get_query_string({}, [self.parameter_name]), 
           'display': _('All')}
        for lookup, title in self.lookup_choices:
            yield {'selected': self.value() == force_text(lookup), 
...
class RelatedFieldListFilter(FieldListFilter):
    ...
    def choices(self, changelist):
        ...
        for pk_val, val in self.lookup_choices:
            yield {'selected': self.lookup_val == force_text(pk_val), 
               'query_string': changelist.get_query_string({self.lookup_kwarg: pk_val}, [
                              self.lookup_kwarg_isnull]), 
               'display': val}

        if self.include_empty_choice:
            yield {'selected': bool(self.lookup_val_isnull), 'query_string': changelist.get_query_string({self.lookup_kwarg_isnull: 'True'}, [
                              self.lookup_kwarg]), 
...
class BooleanFieldListFilter(FieldListFilter):
    ...
    def __init__(self, field, request, params, model, model_admin, field_path):
        ...
        if self.used_parameters and self.lookup_kwarg in self.used_parameters and self.used_parameters[self.lookup_kwarg] in ('1',
                                                                                                                              '0'):
    ...
    def choices(self, changelist):
        for lookup, title in (
         ...
         (
          '0', _('No'))):
            yield {'selected': self.lookup_val == lookup and not self.lookup_val2, 'query_string': changelist.get_query_string({self.lookup_kwarg: lookup}, [
                              self.lookup_kwarg2]), 
               'display': title}

        if isinstance(self.field, models.NullBooleanField):
            yield {'selected': self.lookup_val2 == 'True', 'query_string': changelist.get_query_string({self.lookup_kwarg2: 'True'}, [
                              self.lookup_kwarg]), 
               'display': _('Unknown')}
        return


FieldListFilter.register(lambda f: isinstance(f, (models.BooleanField, models.NullBooleanField)), BooleanFieldListFilter)
    ...
    def choices(self, changelist):
        ...
        for lookup, title in self.field.flatchoices:
            if lookup is None:
                none_title = title
                continue
            yield {'selected': force_text(lookup) == self.lookup_val, 'query_string': changelist.get_query_string({self.lookup_kwarg: lookup}, [self.lookup_kwarg_isnull]), 
               'display': title}
