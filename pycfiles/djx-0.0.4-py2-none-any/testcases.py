...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/test/testcases.py
# Compiled at: 2019-02-14 00:35:17
from __future__ import unicode_literals
import difflib, json, posixpath, sys, threading, unittest, warnings
...
from django.test.utils import CaptureQueriesContext, ContextList, compare_xml, modify_settings, override_settings
from django.utils import six
from django.utils.decorators import classproperty
from django.utils.deprecation import RemovedInDjango20Warning
from django.utils.encoding import force_text
from django.utils.six.moves.urllib.parse import unquote, urljoin, urlparse, urlsplit, urlunsplit
from django.utils.six.moves.urllib.request import url2pathname
from django.views.static import serve
__all__ = (
 b'TestCase', b'TransactionTestCase',
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if exc_type is not None:
            ...
            self.test_case.assertEqual(executed, self.num, b'%d queries executed, %d expected\nCaptured queries were:\n%s' % (
             executed, self.num,
             (b'\n').join(query[b'sql'] for query in self.captured_queries)))
            return
...
class _CursorFailure(object):
    ...
    def __call__(self):
        raise AssertionError(b"Database queries aren't allowed in SimpleTestCase. Either use TestCase or TransactionTestCase to ensure proper test isolation or set %s.allow_database_queries to True to silence this failure." % self.cls_name)
...
class SimpleTestCase(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        if hasattr(cls, b'_cls_modified_context'):
            cls._cls_modified_context.disable()
            delattr(cls, b'_cls_modified_context')
        if hasattr(cls, b'_cls_overridden_context'):
            cls._cls_overridden_context.disable()
            delattr(cls, b'_cls_overridden_context')
    ...
    def assertRedirects(self, response, expected_url, status_code=302, target_status_code=200, host=None, msg_prefix=b'', fetch_redirect_response=True):
        ...
        """
        if host is not None:
            warnings.warn(b'The host argument is deprecated and no longer used by assertRedirects', RemovedInDjango20Warning, stacklevel=2)
        if msg_prefix:
            msg_prefix += b': '
        if hasattr(response, b'redirect_chain'):
            ...
            if not path.startswith(b'/'):
                url = urljoin(response.request[b'PATH_INFO'], url)
                path = urljoin(response.request[b'PATH_INFO'], path)
            if fetch_redirect_response:
                domain, port = split_domain_port(netloc)
                if domain and not validate_host(domain, settings.ALLOWED_HOSTS):
                    raise ValueError(b"The test client is unable to fetch remote URLs (got %s). If the host is served by Django, add '%s' to ALLOWED_HOSTS. Otherwise, use assertRedirects(..., fetch_redirect_response=False)." % (
                     url, domain))
                redirect_response = response.client.get(path, QueryDict(query), secure=scheme == b'https')
        ...
        if url != expected_url:
            ...
            relative_url = urlunsplit((b'', b'', e_path, e_query, e_fragment))
            if url == relative_url:
                warnings.warn(b'assertRedirects had to strip the scheme and domain from the expected URL, as it was always added automatically to URLs before Django 1.9. Please update your expected URLs by removing the scheme and domain.', RemovedInDjango20Warning, stacklevel=2)
                expected_url = relative_url
        self.assertEqual(url, expected_url, msg_prefix + b"Response redirected to '%s', expected '%s'" % (url, expected_url))
        return
    ...
    def assertContains(self, response, text, count=None, status_code=200, msg_prefix=b'', html=False):
        ...
        if count is not None:
            self.assertEqual(real_count, count, msg_prefix + b'Found %d instances of %s in response (expected %d)' % (real_count, text_repr, count))
        else:
    ...
    def assertNotContains(self, response, text, status_code=200, msg_prefix=b'', html=False):
        ...
        """
        text_repr, real_count, msg_prefix = self._assert_contains(response, text, status_code, msg_prefix, html)
        self.assertEqual(real_count, 0, msg_prefix + b'Response should not contain %s' % text_repr)
    ...
    def assertFormsetError(self, response, formset, form_index, field, errors, msg_prefix=b''):
        """
            ...
            for err in errors:
                if field is not None:
                    if field in context[formset].forms[form_index].errors:
                        ...
                        self.assertTrue(err in field_errors, msg_prefix + b"The field '%s' on formset '%s', form %d in context %d does not contain the error '%s' (actual errors: %s)" % (
                         field, formset, form_index, i, err, repr(field_errors)))
                    elif field in context[formset].forms[form_index].fields:
                        self.fail(msg_prefix + b"The field '%s' on formset '%s', form %d in context %d contains no errors" % (
                         field, formset, form_index, i))
                    else:
                        self.fail(msg_prefix + b"The formset '%s', form %d in context %d does not contain the field '%s'" % (
                         formset, form_index, i, field))
                elif form_index is not None:
                    non_field_errors = context[formset].forms[form_index].non_field_errors()
                    self.assertFalse(len(non_field_errors) == 0, msg_prefix + b"The formset '%s', form %d in context %d does not contain any non-field errors." % (
                     formset, form_index, i))
                    self.assertTrue(err in non_field_errors, msg_prefix + b"The formset '%s', form %d in context %d does not contain the non-field error '%s' (actual errors: %s)" % (
                     formset, form_index, i, err, repr(non_field_errors)))
                else:
                    non_form_errors = context[formset].non_form_errors()
                    self.assertFalse(len(non_form_errors) == 0, msg_prefix + b"The formset '%s' in context %d does not contain any non-form errors." % (
                     formset, i))
                    self.assertTrue(err in non_form_errors, msg_prefix + b"The formset '%s' in context %d does not contain the non-form error '%s' (actual errors: %s)" % (
                     formset, i, err, repr(non_form_errors)))

        if not found_formset:
            self.fail(msg_prefix + b"The formset '%s' was not used to render the response" % formset)
        return
    ...
    def _assert_template_used(self, response, template_name, msg_prefix):
        ...
        if msg_prefix:
            msg_prefix += b': '
        if template_name is not None and response is not None and not hasattr(response, b'templates'):
            raise ValueError(b'assertTemplateUsed() and assertTemplateNotUsed() are only usable on responses fetched using the Django test Client.')
        if not hasattr(response, b'templates') or response is None and template_name:
            if response:
                template_name = response
                response = None
            return (
             template_name, None, msg_prefix)
        else:
            template_names = [ t.name for t in response.templates if t.name is not None ]
            return (
             None, template_names, msg_prefix)

    def assertTemplateUsed(self, response=None, template_name=None, msg_prefix=b'', count=None):
        """
        ...
        if context_mgr_template:
            ...
            if count is not None:
                self.assertEqual(template_names.count(template_name), count, msg_prefix + b"Template '%s' was expected to be rendered %d time(s) but was actually rendered %d time(s)." % (
    ...
    def assertRaisesMessage(self, expected_exception, expected_message, *args, **kwargs):
        ...
        Args:
            ...
            kwargs: Extra kwargs.
        """
        callable_obj = kwargs.pop(b'callable_obj', None)
        if callable_obj:
            warnings.warn(b'The callable_obj kwarg is deprecated. Pass the callable as a positional argument instead.', RemovedInDjango20Warning)
        elif len(args):
            callable_obj = args[0]
            args = args[1:]
    ...
    def assertInHTML(self, needle, haystack, count=None, msg_prefix=b''):
        ...
        if count is not None:
            self.assertEqual(real_count, count, msg_prefix + b"Found %d instances of '%s' in response (expected %d)" % (real_count, needle, count))
        else:
    ...
    def assertXMLEqual(self, xml1, xml2, msg=None):
        ...
        if not result:
            ...
            diff = b'\n' + (b'\n').join(difflib.ndiff(six.text_type(xml1).splitlines(), six.text_type(xml2).splitlines()))
            standardMsg = self._truncateMessage(standardMsg, diff)
            self.fail(self._formatMessage(msg, standardMsg))
...
class TransactionTestCase(SimpleTestCase):
    ...
    def _pre_setup(self):
        ...
        * If the class has a 'fixtures' attribute, installing these fixtures.
        ...
        if self.available_apps is not None:
            ...
            setting_changed.send(sender=settings._wrapped.__class__, setting=b'INSTALLED_APPS', value=self.available_apps, enter=True)
        ...
        try:
            ...
            if self.available_apps is not None:
                ...
                setting_changed.send(sender=settings._wrapped.__class__, setting=b'INSTALLED_APPS', value=settings.INSTALLED_APPS, enter=False)
            raise
    ...
    def _databases_names(cls, include_mirrors=True):
        if getattr(cls, b'multi_db', False):
            return [ alias for alias in connections if include_mirrors or not connections[alias].settings_dict[b'TEST'][b'MIRROR']
                   ]
        else:
            return [
             DEFAULT_DB_ALIAS]
    ...
    def _fixture_setup(self):
        for db_name in self._databases_names(include_mirrors=False):
            ...
            if self.serialized_rollback and hasattr(connections[db_name], b'_test_serialized_contents'):
                if self.available_apps is not None:
                    apps.unset_available_apps()
                ...
                if self.available_apps is not None:
                    apps.set_available_apps(self.available_apps)
            if self.fixtures:
                call_command(b'loaddata', *self.fixtures, **{b'verbosity': 0, b'database': db_name})
    ...
    def _fixture_teardown(self):
        for db_name in self._databases_names(include_mirrors=False):
            inhibit_post_migrate = self.available_apps is not None or self.serialized_rollback and hasattr(connections[db_name], b'_test_serialized_contents')
            call_command(b'flush', verbosity=0, interactive=False, database=db_name, reset_sequences=False, allow_cascade=self.available_apps is not None, inhibit_post_migrate=inhibit_post_migrate)
    ...
    def assertQuerysetEqual(self, qs, values, transform=repr, ordered=True, msg=None):
        ...
        values = list(values)
        if len(values) > 1 and hasattr(qs, b'ordered') and not qs.ordered:
            raise ValueError(b'Trying to compare non-ordered queryset against more than one ordered values')
...
class TestCase(TransactionTestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.fixtures:
            for db_name in cls._databases_names(include_mirrors=False):
                try:
                    call_command(b'loaddata', *cls.fixtures, **{b'verbosity': 0, 
                       b'commit': False, 
                       b'database': db_name})
                except Exception:
    ...
    def _should_check_constraints(self, connection):
        return connection.features.can_defer_constraint_checks and not connection.needs_rollback and connection.is_usable()
...
class CheckCondition(object):
    ...
    def __get__(self, instance, cls=None):
        if any(getattr(base, b'__unittest_skip__', False) for base in cls.__bases__):
            return True
...
def _deferredSkip(condition, reason):
    ...
    def decorator(test_func):
        if not (isinstance(test_func, type) and issubclass(test_func, unittest.TestCase)):
            ...
            test_item = test_func
            skip = test_func.__dict__.get(b'__unittest_skip__')
...
def skipIfDBFeature(*features):
    ...
    """
    return _deferredSkip(lambda : any(getattr(connection.features, feature, False) for feature in features), b'Database has feature(s) %s' % (b', ').join(features))
...
def skipUnlessDBFeature(*features):
    ...
    """
    return _deferredSkip(lambda : not all(getattr(connection.features, feature, False) for feature in features), b"Database doesn't support feature(s): %s" % (b', ').join(features))
...
def skipUnlessAnyDBFeature(*features):
    ...
    """
    return _deferredSkip(lambda : not any(getattr(connection.features, feature, False) for feature in features), b"Database doesn't support any of the feature(s): %s" % (b', ').join(features))
...
class FSFilesHandler(WSGIHandler):
    ...
    def serve(self, request):
        ...
        final_rel_path = os_rel_path.replace(b'\\', b'/').lstrip(b'/')
...
class LiveServerTestCase(TransactionTestCase):
    ...
    def live_server_url(cls):
        return b'http://%s:%s' % (cls.host, cls.server_thread.port)
    ...
    def setUpClass(cls):
        ...
        for conn in connections.all():
            if conn.vendor == b'sqlite' and conn.is_in_memory_db():
                conn.allow_thread_sharing = True
                connections_override[conn.alias] = conn

        cls._live_server_modified_settings = modify_settings(ALLOWED_HOSTS={b'append': cls.host})
...
class SerializeMixin(object):
    ...
    def setUpClass(cls):
        if cls.lockfile is None:
            raise ValueError((b"{}.lockfile isn't set. Set it to a unique value in the base class.").format(cls.__name__))
