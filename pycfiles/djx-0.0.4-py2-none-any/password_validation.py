...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/contrib/auth/password_validation.py
...
def password_changed(password, user=None, password_validators=None):
    ...
    if password_validators is None:
        password_validators = get_default_password_validators()
    for validator in password_validators:
        password_changed = getattr(validator, b'password_changed', lambda *a: None)
        ...
        if not user:
            ...
            for attribute_name in self.user_attributes:
                ...
                if not value or not isinstance(value, string_types):
                    continue
                value_parts = re.split(b'\\W+', value) + [value]
                for value_part in value_parts:
                    if SequenceMatcher(a=password.lower(), b=value_part.lower()).quick_ratio() >= self.max_similarity:
                        ...
                        raise ValidationError(_(b'The password is too similar to the %(verbose_name)s.'), code=b'password_too_similar', params={b'verbose_name': verbose_name})

            return

    def get_help_text(self):
        return _(b"Your password can't be too similar to your other personal information.")
...
class CommonPasswordValidator(object):
    ...
    """
    DEFAULT_PASSWORD_LIST_PATH = os.path.join(os.path.dirname(os.path.realpath(upath(__file__))), b'common-passwords.txt.gz')

    def __init__(self, password_list_path=DEFAULT_PASSWORD_LIST_PATH):
        try:
            with gzip.open(password_list_path) as (f):
                common_passwords_lines = f.read().decode(b'utf-8').splitlines()
        except IOError:
    ...
    def validate(self, password, user=None):
        if password.lower().strip() in self.passwords:
            raise ValidationError(_(b'This password is too common.'), code=b'password_too_common')

    def get_help_text(self):
        return _(b"Your password can't be a commonly used password.")
