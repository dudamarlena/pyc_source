...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-zr3xXj/pytest/_pytest/junitxml.py
...
import functools, os, re, sys, time, py, six, pytest
...
_legal_xml_re = [ '%s-%s' % (six.unichr(low), six.unichr(high)) for low, high in _legal_ranges if low < sys.maxunicode
                ]
...
def record_xml_attribute(request):
    ...
    if xml is not None and xml.family != 'xunit1':
        request.node.warn(PytestWarning('record_xml_attribute is incompatible with junit_family: %s (use: legacy|xunit1)' % xml.family))
...
def pytest_configure(config):
    ...
    if xmlpath and not hasattr(config, 'slaveinput'):
        config._xml = LogXML(xmlpath, config.option.junitprefix, config.getini('junit_suite_name'), config.getini('junit_logging'), config.getini('junit_duration_report'), config.getini('junit_family'))
...
class LogXML(object):
    ...
    def __init__(self, logfile, prefix, suite_name='pytest', logging='no', report_duration='total', family='xunit1'):
    ...
    def pytest_sessionfinish(self):
        ...
        logfile.write(Junit.testsuite(self._get_global_properties_node(), [ x.to_xml() for x in self.node_reporters_ordered ], name=self.suite_name, errors=self.stats['error'], failures=self.stats['failure'], skipped=self.stats['skipped'], tests=numtests, time='%.3f' % suite_time_delta).unicode(indent=0))
        logfile.close()
