...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/static3/static.py
# Compiled at: 2019-02-14 00:35:18
"""
Copyright (C) 2012 Roman Mohr <roman@fenkhuber.at>
"""
import mimetypes, email.utils as rfc822, time, string, sys
...
class StatusApp:
    ...
    def __call__(self, environ, start_response, headers=[]):
        ...
        if environ['REQUEST_METHOD'] == 'HEAD':
            ...
            return [
             _encode(self.message, self.encoding)]
...
class Cling(object):
    ...
    def __call__(self, environ, start_response):
        ...
        prezipped = 'gzip' in environ.get('HTTP_ACCEPT_ENCODING', []) and path.exists(full_path + '.gz')
        if prezipped:
            full_path += '.gz'
        ...
        try:
            ...
            if if_modified and rfc822.parsedate(if_modified) >= rfc822.parsedate(last_modified):
    ...
    def _is_under_root(self, full_path):
        ...
        if (path.abspath(full_path) + path.sep).startswith(path.abspath(self.root) + path.sep):
            return True
        else:
            return False
    ...
    def _add_headers(self, headers, path, content_type):
        ...
        for config in getattr(self, 'headers', []):
            if path.startswith(config.get('prefix', DEFAULT)) or content_type == config.get('type', DEFAULT) or path.endswith(config.get('ext', DEFAULT)):
...
class Shock(Cling):
    ...
    def _guess_type(self, full_path):
        ...
        if magic is not None:
            return mimetypes.guess_type(magic.old_path(full_path))[0] or 'text/plain'
        else:
            return mimetypes.guess_type(full_path)[0] or 'text/plain'
            return
    ...
    def _body(self, full_path, environ, file_like):
        ...
        magic = self._match_magic(full_path)
        if magic is not None:
            return [ _encode(s, self.encoding) for s in magic.body(environ, file_like)
                   ]
        else:
            way_to_send = environ.get('wsgi.file_wrapper', iter_and_close)
            return way_to_send(file_like, self.block_size)
...
class KidMagic(StringMagic):
    ...
    def body(self, environ, full_path):
        ...
        template = kid.Template(file=full_path, environ=environ, **self.variables)
...
class GenshiMagic(StringMagic):
    ...
    def body(self, environ, full_path):
        ...
        return [
         template.generate(**variables).render('html', doctype='html')]


def command():
    parser = OptionParser(usage='%prog DIR [HOST][:][PORT]', version='static 0.3.6')
