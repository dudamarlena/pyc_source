...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/db/backends/mysql/validation.py
...
class DatabaseValidation(BaseDatabaseValidation):
    ...
    def _check_sql_mode(self, **kwargs):
        ...
        if not modes & {'STRICT_TRANS_TABLES', 'STRICT_ALL_TABLES'}:
            return [
             checks.Warning("MySQL Strict Mode is not set for database connection '%s'" % self.connection.alias, hint="MySQL's Strict Mode fixes many data integrity problems in MySQL, such as data truncation upon insertion, by escalating warnings into errors. It is strongly recommended you activate it. See: https://docs.djangoproject.com/en/%s/ref/databases/#mysql-sql-mode" % (
              get_docs_version(),), id='mysql.W002')]
        return []
    ...
    def check_field(self, field, **kwargs):
        ...
        if getattr(field, 'remote_field', None):
            ...
            if field_type.startswith('varchar') and field.unique and (field.max_length is None or int(field.max_length) > 255):
                errors.append(checks.Error('MySQL does not allow unique CharFields to have a max_length > 255.', obj=field, id='mysql.E001'))
            return errors