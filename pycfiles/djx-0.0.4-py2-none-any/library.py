...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/Django/django/template/library.py
...
class Library(object):
    ...
    def tag(self, name=None, compile_function=None):
        if name is None and compile_function is None:
            ...
            if name is not None and compile_function is None:
                ...
                if name is not None and compile_function is not None:
                    self.tags[name] = compile_function
                    return compile_function
                raise ValueError('Unsupported arguments to Library.tag: (%r, %r)' % (
    ...
    def filter(self, name=None, filter_func=None, **flags):
        ...
        if name is None and filter_func is None:
            ...
            if name is not None and filter_func is None:
                if callable(name):
                ...
                if name is not None and filter_func is not None:
                    ...
                    return filter_func
                raise ValueError('Unsupported arguments to Library.filter: (%r, %r)' % (
                 name, filter_func))
            return
    ...
    def assignment_tag(self, func=None, takes_context=None, name=None):
        warnings.warn('assignment_tag() is deprecated. Use simple_tag() instead', RemovedInDjango20Warning, stacklevel=2)
...
def parse_bits(parser, bits, params, varargs, varkw, defaults, takes_context, name):
    ...
    for bit in bits:
        ...
        if kwarg:
            ...
            if param not in params and varkw is None:
                raise TemplateSyntaxError("'%s' received unexpected keyword argument '%s'" % (
                 name, param))
            elif param in kwargs:
                raise TemplateSyntaxError("'%s' received multiple values for keyword argument '%s'" % (
                 name, param))
            else:
    ...
    if unhandled_params:
        raise TemplateSyntaxError("'%s' did not receive value(s) for the argument(s): %s" % (
...
def import_library(name):
    ...
    try:
        ...
        raise InvalidTemplateLibrary("Invalid template library specified. ImportError raised when trying to load '%s': %s" % (
         name, e))
