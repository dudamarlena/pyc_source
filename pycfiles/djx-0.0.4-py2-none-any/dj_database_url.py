...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-install-n_sfyb/dj-database-url/dj_database_url.py
...
def parse(url, engine=None, conn_max_age=0, ssl_require=False):
    ...
    port = str(url.port) if url.port and engine == SCHEMES['oracle'] else url.port
    config.update({'NAME': urlparse.unquote(path or ''), 
    ...
    if 'currentSchema' in options and engine in ('django.contrib.gis.db.backends.postgis',
