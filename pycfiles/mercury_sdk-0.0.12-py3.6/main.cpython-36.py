...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mercury_sdk/mcli/main.py
# Compiled at: 2019-02-12 12:07:18
# Size of source mod 2**32: 11688 bytes
import argparse, colorama, json, logging, os, pkg_resources
...
def options():
    parser = argparse.ArgumentParser(description='The Mercury Command Line Interface',
    ...
    parser.add_argument('--program-directory', default=DEFAULT_PROGRAM_DIR, help='Alternative location for program data')
    parser.add_argument('--token-cache', default=TOKEN_CACHE, help='alternative location of the token cache')
    parser.add_argument('-m', '--mercury-url', default=(os.environ.get(MERCURY_API_URL_ENV)),
      help='The mercury url to use')
    parser.add_argument('-v', dest='verbosity', action='count',
      default=0,
      help='Verbosity level -v, somewhat verbose, -vvv really verbose')
    subparsers = parser.add_subparsers(dest='command', help='<command> --help')
    login_parser = subparsers.add_parser('login',
      help='Login to the authentication service and store the token in the local environment')
    login_parser.add_argument('--no-store', default=False, action='store_true', help='Do not store the token')
    login_parser.add_argument('-q', '--quiet', action='store_true', help='Do not print the token')
    ...
    set_token_parser.add_argument('--expire-at',
      default=8, help='The number of hours to consider the token valid')
    inv_parser = subparsers.add_parser('inventory', help='Inventory query operations')
    inv_parser.add_argument('-q', '--query', default='{}', help=query_help)
    inv_parser.add_argument('-p', '--projection', default='', help='Specify the key projection to produce the desired output')
    inv_parser.add_argument('-n', '--max-items', default=100)
    inv_parser.add_argument('-a', '--active', help='Only search for active devices',
      action='store_true')
    inv_parser.add_argument('mercury_id', default=None, nargs='?', help='Get a device record by mercury_id')
    rpc_parser = subparsers.add_parser('rpc',
      help='RPC commands')
    rpc_subparsers = rpc_parser.add_subparsers(dest='rpc_command')
    rpc_submit_parser = rpc_subparsers.add_parser('submit', help='Submit a job')
    rpc_submit_parser.add_argument('-q', '--query', help=query_help)
    rpc_submit_parser.add_argument('-t', '--target', help='A single mercury id to run the command on')
    ...
    rpc_submit_parser.add_argument('-w', '--wait', action='store_true', help='Wait for the job to complete and print the results')
    ...
    rpc_job_parser.add_argument('-s', '--status', action='store_true', help='Get status information for tasks')
    rpc_job_parser.add_argument('-t', '--tasks', action='store_true', help='Get task data')
    shell_parser = subparsers.add_parser('shell',
      help='Enter a shell for interactive inventory management')
    shell_parser.add_argument('-q', '--query', default=None, help='Set the initial target query for the shell')
    shell_parser.add_argument('-t', '--target', default=None, help='The mercury_id of a single target')
    shell_parser.add_argument('-r', '--run', default=None, help='Instead of entering the shell,run the command, print the result, and exit')
    shell_parser.add_argument('--quiet', default=False, action='store_true', help='Suppress command output')
    shell_parser.add_argument('--raw', default=False, action='store_true', help='Do not print agent info, only the raw command output')
    press_parser = subparsers.add_parser('press', help='Install an operating system to /mnt/press')
    press_parser.add_argument('-c', '--configuration', dest='press_configuration', help='The path to a press configuration in yaml format')
    press_parser.add_argument('-t', '--target', help='The mercury_id of a single target')
    press_parser.add_argument('-w', '--wait', action='store_true', help='Wait for the job to complete and print the results')
    deployment_parser = subparsers.add_parser('deploy',
      help='[EXPERIMENTAL] Deploy many servers at once using an asset file')
    deployment_parser.add_argument('-q',
      '--query', help=query_help)
    deployment_parser.add_argument('-a',
      '--asset-backend', help='The asset backend plugin to use')
...
def router(command, configuration):
    ...
    if command == 'rpc':
        ...
        if rpc_command == 'submit':
            ...
            print(operations.make_rpc(rpc_client, target_query,
    ...
    if command == 'shell':
        rpc_client, target_query = _prepare_rpc()
        mshell = shell.MercuryShell(rpc_client, initial_query=target_query, quiet=(configuration['quiet']),
...
def main():
    ...
    if not os.path.exists(namespace.program_directory):
        os.makedirs(namespace.program_directory, 448)
