...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pivpy/graphics.py
...
def quiver(data, arrScale=25.0, threshold=None, nthArr=1, aspectratio='equal', colbar=False, colbar_orient='vertical', units=None, streamlines=False):
    """
    ...
    if threshold is not None:
        ...
        if len(plt.get_fignums()) == 0:
            ...
            fig = plt.gcf()
        ax = plt.gca()
    ax.quiver(x, y, u, v, units='width', scale=(np.max(S * arrScale)),
      headwidth=2)
    if streamlines == True:
...
def contour_plot(data, threshold=None, contourLevels=None, colbar=None, logscale=False, aspectratio='equal', units=None):
    ...
    if 'w' not in data.var():
        ...
        if units is not None:
            ...
            lUnits = data.attrs['units'][0]
            propUnits = data.attrs['variables'][(-1)] + data.attrs['units'][(-1)]
        f, ax = plt.subplots()
        if threshold is not None:
            ...
            if contourLevels == None:
                levels = np.linspace(np.min(data['w'].values), np.max(data['w'].values), 10)
            else:
                levels = contourLevels
        if logscale:
            c = ax.contourf((data.x), (data.y), (np.abs(data['w'].T)), levels=levels, cmap=(plt.get_cmap('RdYlBu')),
              norm=(plt.colors.LogNorm()))
        else:
            c = ax.contourf((data.x), (data.y), (data['w'].T), levels=levels, cmap=(plt.get_cmap('RdYlBu')))
...
def animate(data, arrowscale=1, savepath=None):
    ...
    M = np.sqrt(U ** 2 + V ** 2)
    Q = ax.quiver((X[::3, ::3]), (Y[::3, ::3]), (U[::3, ::3]),
      (V[::3, ::3]), (M[::3, ::3]), units='inches',
      scale=arrowscale)
    cb = plt.colorbar(Q)
    units = data.attrs['units']
    cb.ax.set_ylabel('velocity (' + units[2] + ')')
    text = ax.text(0.2, 1.05, ('1/' + str(len(data.t))), ha='center', va='center', transform=(ax.transAxes))
    ...
    anim = FuncAnimation(fig, update_quiver, fargs=(Q, data, text), frames=(len(data.t)),
      blit=False)
