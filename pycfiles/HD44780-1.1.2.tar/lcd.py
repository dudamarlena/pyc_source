...
# [GCC 8.4.0]
# Embedded file name: /home/mezgrman/projects/HD44780/hd44780/lcd.py
# Compiled at: 2013-04-15 10:08:14
...
class Display:
    CONTROL_CHARACTERS = (13, 24, 27, 127)
    ...
    def write_string(self, string, parse_custom=True, update=False, align='left'):
        if parse_custom:
            string = re.sub('<(?P<num>[0-7])>', self._custom_sub, string)
    ...
    def clear(self):
        self.write_value(1, data=False)

    def home(self):
        self.write_value(2, data=False)
        self.cursor_pos = [0, 0]

    def set_entry_mode(self, rtl=False, scroll=True):
        _rtl = 2 if rtl else 0
        _scroll = 1 if scroll else 0
        self.write_value(4 + _rtl + _scroll, data=False)

    def set_display_enable(self, enable=True, cursor=False, cursor_blink=False):
        _enable = 4 if enable else 0
        _cursor = 2 if cursor else 0
        _cursor_blink = 1 if cursor_blink else 0
        self.write_value(8 + _enable + _cursor + _cursor_blink, data=False)

    def scroll(self, right=False):
        _right = 4 if right else 0
        self.write_value(24 + _right, data=False)

    def move_cursor(self, left=False):
        _left = 0 if left else 4
        self.write_value(16 + _left, data=False)
        if left:
            self.cursor_pos[0] -= 1
        else:
            self.cursor_pos[0] += 1
    ...
    def set_configuration(self, multiline=True, five_seven_font=True):
        _multiline = 8 if multiline else 0
        _five_seven_font = 4 if five_seven_font else 0
        self.write_value(32 + _multiline + _five_seven_font, data=False)
    ...
    def load_custom_character(self, pos, char):
        if type(char) is file:
            ...
            _char = char
        for i in range(8):
            self.write_value(64 + (pos << 3) + i, data=False)
