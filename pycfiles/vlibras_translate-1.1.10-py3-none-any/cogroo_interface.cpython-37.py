...
# [GCC 8.4.0]
# Embedded file name: /media/caiomoraes/Acer/Users/CaioMoraes/Documents/computacaoUFPB/TCC/pacote_vlibras/vlibras_translate/cogroo4py/cogroo_interface.py
# Compiled at: 2019-09-20 19:42:55
    ...
    def __init__(self, sentence, cogroo_chunk):
        ...
        if len(tokens) > 0:
            ...
            for token in sentence.tokens:
                if token.start >= self.start:
                    if token.end <= self.end:
...
class Cogroo:
    ...
    def lemmatize(self, text):
        if text is None or text == '':
            return ''
        elif type(text) is not Document:
