...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexcom_reader/database_records.py
# Compiled at: 2016-05-28 22:31:57
import crc16, constants, struct, util, binascii
...
class BaseDatabaseRecord(object):
    ...
    def _CheckFormat(cls):
        if cls.FORMAT is None or not cls.FORMAT:
            raise NotImplementedError('Subclasses of %s need to define FORMAT' % cls.__name__)
        return
...
class InsertionRecord(GenericTimestampedRecord):
    FIELDS = [
     'insertion_time', 'session_state']
    FORMAT = '<3IcH'

    @property
    def insertion_time(self):
        if self.data[2] == 4294967295:
...
class Calibration(GenericTimestampedRecord):
    FORMAT = '<2Iddd3cdb'
    FIELDS = [
     'slope', 'intercept', 'scale', 'decay', 'numsub', 'raw']
