...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dexcom_reader/readdata.py
# Compiled at: 2016-05-28 22:31:57
import crc16, constants, database_records, datetime, serial, sys, time, packetwriter, struct, re, util, xml.etree.ElementTree as ET, platform
...
class Dexcom(object):
    ...
    @staticmethod
    def FindDevice():
        return util.find_usbserial(constants.DEXCOM_G4_USB_VENDOR, constants.DEXCOM_G4_USB_PRODUCT)
    ...
    def LocateAndDownload(cls):
        ...
        if not device:
            ...
            print '- CGM commitable records: %d' % len([ not x.display_only for x in dex.ReadRecords('EGV_DATA') ])
    ...
    PARSER_MAP = {'USER_EVENT_DATA': database_records.EventRecord, 
    ...
    def ParsePage(self, header, data):
        ...
        if record_type in xml_parsed:
            return [database_records.GenericXMLRecord.Create(data, 0)]
        raise NotImplementedError('Parsing of %s has not yet been implemented' % record_type)
