...
# Compiled at: 2020-04-13 14:55:50
# Size of source mod 2**32: 114408 bytes
...
class DigitalCellSorter(VisualizationFunctions):
    ...
    def __init__(self, df_expr, dataName, geneNamesType, geneListFileName, mitochondrialGenes, sigmaOverMeanSigma, nClusters, nFineClusters, doFineClustering, minSizeForFineClustering, clusteringFunction, nComponentsPCA, nSamples_pDCS, nSamples_Hopfield, saveDir, makeMarkerSubplots, availableCPUsCount, zScoreCutoff, subclusteringName, doQualityControl, doBatchCorrection, makePlots, minimumNumberOfMarkersPerCelltype, nameForUnknown, nameForLowQC, countDepthCutoffQC, numberOfGenesCutoffQC, mitochondrialGenesCutoffQC, excludedFromQC, thresholdForUnknown_pDCS, thresholdForUnknown_ratio, thresholdForUnknown_Hopfield=None'dataName''alias'NoneNone0.01103True50AgglomerativeClustering2003000500os.path.join('')Falsemin(12, os.cpu_count())0.3NoneTrueTrueTrue10'Unassigned''Failed QC'0.50.51.5None0.00.00.0, thresholdForUnknown=0.2, layout='TSNE', HopfieldTemperature=0.1, annotationMethod='ratio-pDCS-Hopfield'):
    ...
    def makeProjectionPlotAnnotated(self):
        ...
        self.makeProjectionPlot((df_projection.values), (df_markers_expr.columns.get_level_values('label')), 'by_clusters_annotated', colormap=colormap,
          legend=False)
    ...
    def makeIndividualGeneExpressionPlot(self, gene, hideClusterLabels=False, outlineClusters=True):
        ...
        self.internalMakeMarkerSubplots(df_markers_expr, (df_projection.values),
          hugo_cd_dict,
...
pd.concat([df.iloc[np.where(df.index != tagForMissing)[0]], df.iloc[np.where(df.index == tagForMissing)[0]]], axis=0, sort=False) # Avoid dead code: df
    ...
    def annotateWith_Hopfield_Scheme(self, df_markers_expr, df_marker_cell_type):
        ...
        def propagateHopfield(sigma, xi, T=0.2, tmax=75, fractionToUpdate=0.5, underlyingNetwork=None, typesNames=None, clustersNames=None, recordTrajectories=False, id=None, printSwitchingFraction=False):
            ...
            try:
                ...
                return
                xi_inv = Q_inv.dot(xi.T)
                J = xi.dot(xi_inv)
                sigma_ = sigma.copy()
                sigma_[sigma_ < 0.0] = 0.0
                initial = np.argmax((xi_inv.dot(sigma_)), axis=0)
                initial[np.where(np.max((xi_inv.dot(sigma_)), axis=0) < 1e-12)] = -1
                ...
                final[np.where(np.max((xi_inv.dot(sigma_)), axis=0) < 1e-12)] = -1
        ...
        for i in range(self.nSamples_Hopfield):
            ...
            hop = propagateHopfield((df_sig.values * 2.0 - 1.0), (df_marker_cell_type.values), typesNames=(df_marker_cell_type.columns.values),
    ...
    def loadExpressionData(self):
        ...
        if self.KeyInFile('df_expr', self.fileHDFpath):
            ...
            self._df_expr = pd.read_hdf((self.fileHDFpath), key='df_expr', mode='r').unstack(level=(-1), fill_value=0.0).T
