...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_DCS\VS\DigitalCellSorter\DigitalCellSorter\ReadPrepareDataHCApreviewDataset.py
# Compiled at: 2020-03-06 12:24:01
# Size of source mod 2**32: 9381 bytes
import os, copy, h5py, numpy as np, pandas as pd
...
def getHCAdataByURL(url, extractPath):
    ...
    Usage:
        url = "https://data.humancellatlas.org/project-assets/project-matrices/8185730f-4113-40d3-9cc3-929271784c2b.homo_sapiens.mtx.zip"
    ...
    if not os.path.exists(extractPath):
        ...
        for tempName in ('cells', 'genes', 'barcodes'):
            write(pd.read_csv((os.path.join(extractPath, tempName + '.tsv')), delimiter='\t', index_col=0, header=0), os.path.join(extractPath, tempName))


def recordH5filesOfDonors_in_HCA_Datasets(extractPath, organName=None, donorIDcolumn='donor_organism.provenance.document_id', organColumn='derived_organ_parts_label', useHogoGeneNames=True):
    """Record *.h5 files of HCA Bone Marrow dataset
    ...
    Usage:
        recordH5filesOfDonors_HCA_Datasets(extractPath, organName='retina')
    """
    ...
    for donorID in donorIDs:
        ...
        if not os.path.isfile(os.path.join(extractPath, 'dfDonorID %s' % donorID + '.h5')):
            ...
            dfDonorID = pd.DataFrame(data=(dataSparseMatrix[:, idxDonorID].todense()), index=(genes.index if not useHogoGeneNames else genes.values),
            ...
            print(dfDonorID.shape)
            dfDonorID = dfDonorID.loc[(dfDonorID.sum(axis=1) > 0.0)]
...
def PrepareDataOnePatient_PREVIEW_DATASET(filename, patient, saveFolderName, useAllData=True, cellsLimitToUse=1000, randomlySample=True, randomSeed=0):
