...
# Compiled at: 2020-04-13 10:10:43
# Size of source mod 2**32: 73620 bytes
import os, copy, numpy as np, pandas as pd, scipy.stats, scipy.signal, scipy.cluster.hierarchy
...
class VisualizationFunctions:
    ...
    def internalMakeMarkerSubplots(self, df, X_projection, hugo_cd_dict, NoFrameOnFigures=False, HideClusterLabels=False, outlineClusters=True, analyzeBy='cluster'):
        ...
        for counter, marker in enumerate(df.index.values):
            MarkerSubplot(counter, marker, pd.DataFrame(data=(np.reshape(np.array(df.loc[marker]), (1, len(df.loc[marker])))), columns=(df.columns), index=[marker]), votingResults, X_projection, cellClusterIndexLabel, hugo_cd_dict, self.dataName, self.saveDir, NoFrameOnFigures, HideClusterLabels, XLIM, YLIM, directory, outlineClusters)
    ...
    def makeCellMarkersPiePlot(self, type1, type2, df_marker_cell_type='all', nameToAppend=None, listUnexpressedMarkers=True, orthogonalSectorsShift=0.1, rotationAngle=0, dpi=600, extension='png'):
