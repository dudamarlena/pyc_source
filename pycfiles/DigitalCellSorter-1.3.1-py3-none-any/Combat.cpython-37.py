...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_DCS\VS\DigitalCellSorter\DigitalCellSorter\Combat.py
# Compiled at: 2019-10-25 14:33:06
# Size of source mod 2**32: 7017 bytes
import pandas as pd, patsy, sys
...
def design_mat(mod, numerical_covariates, batch_levels):
    design = patsy.dmatrix(('~ 0 + C(batch, levels=%s)' % str(batch_levels)), mod,
...
def combat(data, batch, model=None, numerical_covariates=None):
    ...
    numerical_covariates = [list(model.columns).index(c) if isinstance(c, str) else c for c in numerical_covariates if c not in drop_cols]
    ...
    for i, batch_idxs in enumerate(batch_info):
        temp = it_sol(s_data[batch_idxs], gamma_hat[i], delta_hat[i], gamma_bar[i], t2[i], a_prior[i], b_prior[i])
