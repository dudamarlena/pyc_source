...
# [GCC 8.4.0]
# Embedded file name: D:\Projects\A_DCS\VS\DigitalCellSorter\DigitalCellSorter\DigitalCellSorter.py
# Compiled at: 2019-10-16 13:18:06
# Size of source mod 2**32: 69230 bytes
...
"""
import os, platform, copy, multiprocessing, numpy as np, pandas as pd, scipy.stats, scipy.signal, scipy.cluster.hierarchy, scipy.spatial.distance
...
class DigitalCellSorter(VisualizationFunctions):
    ...
    def __init__(self, dataName='dataName', geneListFileName=None, mitochondrialGenes=None, sigmaOverMeanSigma=0.3, nClusters=5, clusteringFunction=AgglomerativeClustering, nComponentsPCA=200, nSamplesDistribution=10000, saveDir=os.path.join(''), makeMarkerSubplots=False, availableCPUsCount=os.cpu_count(), zScoreCutoff=0.3, subclusteringName=None, doQualityControl=True, doBatchCorrection=True, makePlots=True, minimumNumberOfMarkersPerCelltype=10, minimumScoreForUnknown=0.3):
        """Initialization function. Automatically called when an instance on Digital Cell Sorter is created"""
        defaultGeneList = 'CIBERSORT'
    ...
    def normalize(self, df_expr, median=None):
        ...
        MIN = np.min(df_expr.values[(df_expr.values > 0.0)])
        if MIN <= 0.0:
            raise ValueError
        df_expr = df_expr.replace(0.0, MIN)
    ...
    def project(self, df_expr, PCAonly=False, do_fast_tsne=True):
        ...
        print('Explained variance:', np.round(np.sum(_PCA.explained_variance_ratio_) * 100.0, 2), '%')
        if not PCAonly:
    ...
    def dcsVotingScheme(self, df_markers_expr, df_marker_cell_type):
        ...
        df_null_distributions = random_df_V.apply((lambda s: scipy.stats.rv_histogram(np.histogram(s, bins=100, range=(0, 1)))._hpdf / 100.0), axis=1).apply(pd.Series).T
        df_null_distributions.columns.names = ['CellType', 'Cluster']
        print('Processing voting results')
        df_L = (df_V - pd.Series(data=np.mean((random_df_V.values), axis=1), index=(random_df_V.index)).unstack()) / pd.Series(data=np.std((random_df_V.values), axis=1), index=(random_df_V.index)).replace(0.0, np.inf).unstack()
        ...
        supporting_markers = [' // '.join(np.intersect1d(all_markers[df_V.loc['Predicted cell type'].loc[cluster].split(' #')[0]], df_V.loc['All markers'].loc[cluster].split(' // ')).tolist()) for cluster in df_V.columns]
    ...
    def extractNewMarkerGenes(self, cluster=None, top=100, zScoreCutoff=0.3, removeUnknown=False):
        ...
        if removeUnknown:
            if 'Unknown' in df_gene_cluster_centroids_merged.columns:
    ...
    def visualize(self):
        ...
        if self.toggleMakeTSNEplotAnnotatedClusters:
            ...
            self.makeTSNEplot((df_tsne.values), (df_markers_expr.columns.get_level_values('label')), (self.dataName), (self.saveDir), 'by_clusters_annotated', colormap=colormap,
              legend=False)
            timeMark()
    ...
    def process(self, df_expr, visualize=True):
        ...
        if self.saveDir != os.path.join(''):
            ...
            if self.toggleDoBatchCorrection:
                ...
                if len(np.unique(patients)) == 1:
                    ...
                    df_expr[where_zeros] = 0.0
                    print('Setting negative values to zeros')
                    df_expr[df_expr < 0.0] = 0.0
                    timeMark()
        ...
        cellClusterIndexLabel = self.cluster((df_xpca.values), df_expr=(df_expr if sendDfExpr else None),
...
pd.concat([df.iloc[np.where(df.index != tagForMissing)[0]], df.iloc[np.where(df.index == tagForMissing)[0]]], axis=0, sort=False) # Avoid dead code: df
    ...
    def getAnomalyScores(self, trainingSet, testingSet, printResults=False):
        ...
        """
        instanceIsolationForest = IsolationForest(behaviour='new', max_samples=(np.min([trainingSet.shape[1] - 1, 100])),
    ...
    def getIndividualGeneExpressionPlot(self, gene, hideClusterLabels=False, outlineClusters=True):
        ...
        df_tsne = pd.read_hdf((os.path.join(self.saveDir, self.dataName + '_processed.h5')), key='df_tsne', mode='r')
        df_markers_expr = df_markers_expr.reindex((df_tsne.columns), axis=1).fillna(0.0)
    ...
    def getIndexOfGoodQualityCells(self, saveDir, dataName, count_depth_cutoff=0.5, number_of_genes_cutoff=0.5, mitochondrial_genes_cutoff_upper_bound=3.0):
        ...
        df_QC['isGoodQuality'].loc[(df_QC['count_depth'] > cutoff_count_depth) & (df_QC['number_of_genes'] > cutoff_number_of_genes) & (df_QC['fraction_of_mitochondrialGenes'] < cutoff_fraction_of_mitochondrialGenes)] = True
