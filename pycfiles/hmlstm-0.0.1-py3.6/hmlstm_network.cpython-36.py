...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hmlstm/hmlstm_network.py
# Compiled at: 2017-07-07 16:07:57
...
class HMLSTMNetwork(object):
    ...
    def __init__(self, input_size=29, output_size=1, num_layers=3, hidden_state_sizes=50, out_hidden_size=100, embed_size=100, task='classification'):
        ...
        if type(hidden_state_sizes) is list:
            if len(hidden_state_sizes) != num_layers:
                raise ValueError('The number of hidden states provided must be the' + ' same as the nubmer of layers.')
        else:
            if type(hidden_state_sizes) == int:
                self._hidden_state_sizes = [
                 hidden_state_sizes] * self._num_layers
            else:
        ...
        self.batch_in = tf.placeholder((tf.float32),
          shape=batch_shape, name='batch_in')
        self.batch_out = tf.placeholder((tf.float32),
          shape=batch_shape, name='batch_out')
    ...
    def _initialize_gate_variables(self):
        with vs.variable_scope('gates_vars'):
            for l in range(self._num_layers):
                vs.get_variable(('gate_%s' % l),
                  [sum(self._hidden_state_sizes), 1], dtype=(tf.float32))
    ...
    def _initialize_output_variables(self):
        with vs.variable_scope('output_module_vars'):
            ...
            vs.get_variable('w1',
              [self._embed_size, self._out_hidden_size], dtype=(tf.float32))
            vs.get_variable('w2',
              [self._out_hidden_size, self._out_hidden_size], dtype=(tf.float32))
            vs.get_variable('w3',
              [self._out_hidden_size, self._output_size], dtype=(tf.float32))
    ...
    def gate_input(self, hidden_states):
        with vs.variable_scope('gates_vars', reuse=True):
            ...
            split = array_ops.split(value=hidden_states,
    ...
    def create_multicell(self, batch_size, reuse):
        ...
        def hmlstm_cell(layer):
            ...
            return HMLSTMCell(self._hidden_state_sizes[layer], batch_size, h_below_size, h_above_size, reuse)
    ...
    def split_out_cell_states(self, accum):
        ...
        split_states = array_ops.split(value=accum, num_or_size_splits=splits,
          axis=1)
        cell_states = []
        for l in range(self._num_layers):
            c = split_states[(l * 3)]
    ...
    def get_h_aboves(self, hidden_states, batch_size, hmlstm):
        ...
        h_above_for_last_layer = tf.zeros([
         batch_size, hmlstm._cells[(-1)]._h_above_size],
          dtype=(tf.float32))
        h_aboves = array_ops.concat([
         concated_hs, h_above_for_last_layer],
          axis=1)
        return h_aboves
    ...
    def network(self, batch_size, reuse):
        ...
        def scan_rnn(accum, elem):
            ...
            h_aboves = self.get_h_aboves([cs.h for cs in cell_states], batch_size, hmlstm)
        ...
        def map_output(elem):
            ...
            cell_states, outcome = array_ops.split(value=elem, num_or_size_splits=splits,
              axis=1)
        ...
        return (
         train, loss, indicators, predictions)

    def train(self, batches_in, batches_out, load_vars_from_disk=False, epochs=3):
    ...
    def predict(self, signal, variable_path='./hmlstm_ckpt'):
        ...
        batch_out_size = (
         signal.shape[0], signal.shape[1], self._output_size)
        _predictions = self._session.run(predictions, {self.batch_in: signal, 
    ...
    def predict_boundaries(self, signal, variable_path='./hmlstm_ckpt'):
        ...
        batch_out_size = (
         signal.shape[0], signal.shape[1], self._output_size)
        _indicators = self._session.run(indicators, {self.batch_in: signal, 
         self.batch_out: np.zeros(batch_out_size)})
