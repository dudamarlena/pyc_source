...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hmlstm/hmlstm_cell.py
# Compiled at: 2017-07-07 14:07:49
...
class HMLSTMCell(rnn_cell_impl.RNNCell):
    ...
    def state_size(self):
        return (
         self._num_units, self._num_units, 1)
    ...
    def call(self, inputs, state):
        ...
        hb, zb, ha = array_ops.split(value=inputs,
        ...
        bias_init = tf.constant_initializer((-100000.0), dtype=(tf.float32))
        concat = rnn_cell_impl._linear(states, length, bias=False, bias_initializer=bias_init)
    ...
    def calculate_new_cell_state(self, c, g, i, f, z, zb):
        ...
        for b in range(self._batch_size):
            ...
            new_c[b] = tf.case({tf.equal((tf.squeeze(z[b])),
  tf.constant(1.0, dtype=(tf.float32)), name='flush_c_xxx'): flush_c, 
             
             tf.logical_and((tf.equal(tf.squeeze((z[b]), name='squeeze_z_xxx'), tf.constant(0.0, dtype=(tf.float32)))),
  (tf.equal(tf.squeeze((zb[b]), name='squeeze_zb_xxx'), tf.constant(0.0, dtype=(tf.float32)))),
  name='copy_c_xxx'): copy_c, 
             
             tf.logical_and((tf.equal(tf.squeeze(z[b]), tf.constant(0.0, dtype=(tf.float32)))),
  (tf.equal(tf.squeeze(zb[b]), tf.constant(1.0, dtype=(tf.float32)))),
  name='update_c_xxx'): update_c},
              default=default_c,
              exclusive=True)
    ...
    def calculate_new_hidden_state(self, h, o, new_c, z, zb):
        ...
        for b in range(self._batch_size):
            ...
            new_h[b] = tf.cond(tf.logical_and(tf.equal(tf.squeeze(z[b]), tf.constant(0.0, dtype=(tf.float32))), tf.equal(tf.squeeze(zb[b]), tf.constant(0.0, dtype=(tf.float32)))), copy_h, update_h)
