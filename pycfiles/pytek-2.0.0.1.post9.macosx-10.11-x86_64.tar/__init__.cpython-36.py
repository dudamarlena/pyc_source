...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/site-packages/pytek/__init__.py
# Compiled at: 2018-02-24 21:26:55
# Size of source mod 2**32: 25777 bytes
"""
...
class TDS3k(Configurable):
    ...
    ID_REGEX = re.compile('^TEKTRONIX,TDS 3\\d{3},')
    ...
    def send_command(self, command, *args):
        ...
        args = [
         command] + list(args)
        self.port.write(('%s\r' % ' '.join(args)).encode('utf-8'))
    ...
    _TDS3k__TRIGGER_STATE_LIST = [
     ...
     [
      'trigger', 'trig']]
    _TDS3k__TRIGGER_STATES = {}
    for seq in _TDS3k__TRIGGER_STATE_LIST:
        val = seq[0]
        for k in seq:
            _TDS3k__TRIGGER_STATES[k] = val
    ...
    def trigger_state(self):
        ...
        * **trigger** - indicates that the oscilloscope has seen a trigger and is acquiring the posttrigger information.
        ...
        try:
            return self._TDS3k__TRIGGER_STATES[val.lower()]
        except KeyError:
            return val

    _TDS3k__WFM_PREAMBLE_FIELDS = (
     (
     ...
     (
      'y_unit', str))
    _TDS3k__WFM_PREAMBLE_FIELD_NAMES = tuple(f[0] for f in _TDS3k__WFM_PREAMBLE_FIELDS)
    _TDS3k__WFM_PREAMBLE_FIELD_CONVERTERS = tuple(f[1] for f in _TDS3k__WFM_PREAMBLE_FIELDS)
    ...
    def get_waveform_preamble(self):
        ...
        return dict(zip(self._TDS3k__WFM_PREAMBLE_FIELD_NAMES, [self._TDS3k__WFM_PREAMBLE_FIELD_CONVERTERS[i](wfm[i]) for i in range(len(wfm))]))
    ...
    def get_waveform(self, source='CH1', double=True, start=1, stop=10000, preamble=False, timing=False):
        ...
        points = curve[1]
        data = ((xzero + i * dx, (points[i] - yoff) * ym + yzero) for i in range(len(points)))
        if preamble or timing:
            if preamble:
                ret = [
                 curve[0], data]
