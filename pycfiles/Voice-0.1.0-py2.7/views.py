...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/voice/views.py
# Compiled at: 2011-09-23 21:21:27
...
def index(request):
    ...
    if len(group) > 0:
        grouped.append(group)
    context = RequestContext(request, {'grouped_features': grouped, 
       'form': form, 
...
def admin(request):
    sort, features = features_by_sort(request.GET.get('sort', None))
    return render_to_response('voice/admin/index.html', {'request': request, 
       'features': features, 
       'sort': sort})
...
def features_by_sort(sort):
    if sort == 'newest':
        ...
        sort = 'popular'
        features = Feature.objects.filter(state='V').annotate(Count('votes')).order_by('-votes__count')
    return (
     sort, features)
...
def static_media(request, path):
    ...
    import mimetypes, os.path, posixpath, stat, urllib
