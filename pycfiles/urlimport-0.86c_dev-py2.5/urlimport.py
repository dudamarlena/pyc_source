...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/urlimport/urlimport.py
# Compiled at: 2010-04-17 22:16:04
"""enables to import modules through the web, by adding urls to the python path.
"""
import stringprep, array, unicodedata, _strptime, sys, os, re, imp, marshal
...
from urllib2 import HTTPDefaultErrorHandler, HTTPSHandler, HTTPError, HTTPRedirectHandler, build_opener, urlopen, Request
DOT_PYO = '.pyo'
DOT_PYC = '.pyc'
DOT_PY = '.py'
PY_FILES = (DOT_PYO, DOT_PYC, DOT_PY)
DYN_SUFIX = [ ext for (ext, mode, type) in imp.get_suffixes() if type == 3 and ext.startswith('.')
            ][0]
settings = sys.__dict__.setdefault('urlimport_settings', dict(py_version_string='$PYTHON_VERSION', ssl_cert='', ssl_key='', debug=0))
...
class DefaultErrorHandler(HTTPDefaultErrorHandler, HTTPRedirectHandler):
    ...
    def http_error_301(self, req, fp, code, msg, headers):
        response = HTTPRedirectHandler.http_error_301(self, req, fp, code, msg, headers)
...
class UrlFinder():
    re_url_ok = re.compile('^http://|^ftp://|^https://')
    ...
    def find_module(self, fullname, mpath=None):
        ...
        file_types = [ (ext, None) for ext in PY_FILES ] + [(DYN_SUFIX, None)] + [ ('/__init__%s' % ext, head + '/') for ext in PY_FILES ]
        cache_tested = False
        for (tail, __path) in file_types:
            ...
            if not cache_tested:
                cache_tested = True
                try:
                    (cachedFile, fileData, url, __path, tail) = self.read_cache(fullname)
            ...
            try:
                if not use_cache:
                    ...
                    (fileData, cachedFile) = self.caching_get_file(url, fullname, tail, __path)
    ...
    def user_agent(self, url, host=None):
        ...
        user_agent = settings.get('user_agent.%s' % url, settings.get('user_agent.%s' % host, settings.get('user_agent', None)))
    ...
    def no_cache(self, url, host=None):
        if host is None:
            (proto, user, passwd, host, port, path, query, fragment) = self.split_url(url)
        return settings.get('no_cache.%s' % url, settings.get('no_cache.%s' % host, settings.get('no_cache', False)))

    def cached_file(self, module, tail):
        return os.path.join(self.cache_dir, module.replace('.', os.path.sep)) + tail
    ...
    def read_cache(self, module):
        ...
        if modincache is not None:
            ...
            try:
                (fileData, cachedFile) = self.caching_get_file(url, module, tail, __path, last_modified, etag, expires)
                indexConn.close()
    ...
    def caching_get_file(self, url, module, tail, _UrlFinder__path, cached_last_modified=None, cached_etag=None, expires=None):
        """get the file data using cache.
        """
        keep_cached = not_expired = expires is not None and expires > datetime(*gmtime()[:6])
        if not keep_cached:
            (fileData, expires, last_modified, keep_cached, etag, url) = self.get_file_data(url, cached_last_modified, cached_etag)
        cachedFile = self.cached_file(module, tail)
        if keep_cached:
        ...
        if not no_cache or keep_cached:
            ...
            if expires is None:
                cache_time = settings.get('cache_time.%s' % self.path, settings.get('cache_time.%s' % host, settings.get('cache_time', None)))
    ...
    def get_file_data(self, url, cached_last_modified, cached_etag):
        ...
        key = settings.get('ssl_key.%s' % self.path, settings.get('ssl_key.%s' % host, settings.get('ssl_key', '')))
        cert = settings.get('ssl_cert.%s' % self.path, settings.get('ssl_cert.%s' % host, settings.get('ssl_cert', '')))
        opener = settings.get('opener.%s' % self.path, settings.get('opener.%s' % host, settings.get('opener', None)))
        request = Request(url)
        ...
        if opener is None:
            ...
            fileData = None
            fileData = keep_cached or response.read()
    ...
    def split_url(self, url):
        ...
        return (
         scheme, user, passwd, netloc, port, path, query, fragment)
