...
# Embedded file name: C:\Projects\GitHub\AeroSandbox\aerosandbox\visualization.py
# Compiled at: 2020-04-21 22:27:17
# Size of source mod 2**32: 10972 bytes
import plotly.express as px
import plotly.graph_objects as go
import plotly.io as pio
import dash, numpy as np, casadi as cas
...
def reflect_over_XZ_plane(input_vector):
    ...
    elif len(shape) == 2 and shape[1] == 1 and shape[0] == 3:
        output_vector = output_vector * cas.vertcat(1, -1, 1)
...
class Figure3D:
    ...
    def add_line(self, points, mirror=False):
        """
        ...
        if mirror:
            ...
            self.add_line(points=reflected_points,
              mirror=False)

    def add_streamline(self, points, mirror=False):
        """
        Adds a line (or series of lines) to draw.
        :param points: an iterable with an arbitrary number of items. Each item is a 3D point, represented as an iterable of length 3.
        :param mirror: Should we also draw a version that's mirrored over the XZ plane? [boolean]
        ...
        if mirror:
            reflected_points = [reflect_over_XZ_plane(point) for point in points]
            self.add_streamline(points=reflected_points,
              mirror=False)

    def add_tri(self, points, intensity=0, outline=False, mirror=False):
        """
    ...
    def add_quad(self, points, intensity=0, outline=True, mirror=False):
        """
    ...
    def draw(self, show=True, title='', colorbar_title='', colorscale='viridis'):
        self.fig.add_trace(go.Mesh3d(x=(self.x_face),
          y=(self.y_face),
          z=(self.z_face),
          i=(self.i_face),
          j=(self.j_face),
        ...
        self.fig.add_trace(go.Scatter3d(x=(self.x_line),
          y=(self.y_line),
          z=(self.z_line),
          mode='lines',
          name='',
          ...
          showlegend=False))
        self.fig.add_trace(go.Scatter3d(x=(self.x_streamline),
          y=(self.y_streamline),
          z=(self.z_streamline),
          mode='lines',
          name='',
...
def spy(matrix, show=True):
    ...
    fig = go.Figure(data=go.Heatmap(y=i_index,
      x=j_index,
      z=val,
      colorscale='RdBu',
      showscale=False))
...
def contour(func, x_range, y_range, resolution=50, show=True):
    """
