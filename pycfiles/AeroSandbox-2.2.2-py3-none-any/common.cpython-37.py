...
# Embedded file name: c:\projects\github\aerosandbox\aerosandbox\geometry\common.py
# Compiled at: 2020-04-27 01:04:14
...
def kulfan_coordinates(lower_weights=-0.2 * np.ones(5), upper_weights=0.2 * np.ones(5), enforce_continuous_LE_radius=True, TE_thickness=0.005, n_points_per_side=100, N1=0.5, N2=1.0):
    """
    ...
    def shape(w, x):
        ...
        S_matrix = w * K * np.expand_dims(x, 1) ** np.arange(n + 1) * np.expand_dims(1 - x, 1) ** (n - np.arange(n + 1))
