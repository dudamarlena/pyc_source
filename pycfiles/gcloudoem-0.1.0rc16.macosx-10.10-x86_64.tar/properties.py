...
# [GCC 8.4.0]
# Embedded file name: /Users/rstuart/.pyenv/versions/gcloudoem/lib/python2.7/site-packages/gcloudoem/properties.py
# Compiled at: 2016-10-04 04:32:04
from __future__ import absolute_import, division
import calendar, datetime, json, pickle, re, pytz, six, zlib
...
class ReferenceProperty(BaseProperty):
    ...
    def to_protobuf(self, value):
        if isinstance(value, self.entity_cls):
            value = value.key
        return (
         'blob_value', pickle.dumps(value, protocol=2))

    def validate(self, value):
        if not isinstance(value, (self.entity_cls, Key)):
            self.error('A ReferenceProperty only accepts an %s instance or a Key as its value.' % self.entity_cls.__name__)
        if isinstance(value, self.entity_cls) and value.key.is_partial or isinstance(value, Key) and value.is_partial:
...
class EmailProperty(TextProperty):
    ...
    user_regex = re.compile('(^[-!#$%&\'*+/=?^_`{}|~0-9A-Z]+(\\.[-!#$%&\'*+/=?^_`{}|~0-9A-Z]+)*$|^"([\\001-\\010\\013\\014\\016-\\037!#-\\[\\]-\\177]|\\\\[\\001-\\011\\013\\014\\016-\\177])*"$)', re.IGNORECASE)
    domain_regex = re.compile('((?:[A-Z0-9](?:[A-Z0-9-]{0,61}[A-Z0-9])?\\.)+)(?:[A-Z0-9-]{2,63}(?<!-))$', re.IGNORECASE)
    literal_regex = re.compile('\\[([A-f0-9:\\.]+)\\]$', re.IGNORECASE)
    ipv4 = re.compile('^(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)(\\.(25[0-5]|2[0-4]\\d|[0-1]?\\d?\\d)){3}$')
...
class PickleProperty(BlobProperty):
    ...
    def to_protobuf(self, value):
        return super(PickleProperty, self).to_protobuf(pickle.dumps(value, protocol=2))
...
class DateTimeProperty(BaseProperty):
    ...
    def from_protobuf(self, pb_value):
        naive = datetime.datetime.utcfromtimestamp(0) + datetime.timedelta(seconds=pb_value.timestamp_value.seconds, microseconds=pb_value.timestamp_value.nanos / 1000.0)
    ...
    def to_protobuf(self, value):
        ...
        ms_value = int(calendar.timegm(value.timetuple()) * 1000000.0) + value.microsecond
        seconds, micros = divmod(ms_value, 1000000)
        nanos = micros * 1000
...
class TimeProperty(DateTimeProperty):
    ...
    def to_protobuf(self, value):
        ...
        if not value.tzinfo:
            value = value.replace(tzinfo=pytz.utc)
        value = value.astimezone(pytz.utc)
        value = int(calendar.timegm(value.timetuple()) * 1000000.0) + value.microsecond
        return (name, value)
...
class ListProperty(ContainerBaseProperty):
    ...
    def to_protobuf(self, value):
        return (
         'array_value', [ self.property.to_protobuf(v) for v in value ])
