...
# [GCC 8.4.0]
# Embedded file name: /Users/rstuart/.pyenv/versions/gcloudoem/lib/python2.7/site-packages/gcloudoem/datastore/transaction.py
# Compiled at: 2016-10-04 04:50:07
...
class Transaction(object):
    ...
    def _add_auto_id_entity(self, entity):
        ...
        if not entity.key.is_partial:
            raise ValueError(b'Entity has a completed key')
    ...
    def _assign_entity_to_mutation(self, entity, force_insert=False):
        ...
        """
        key = getattr(entity, b'key')
        key_pb = entity._properties[b'key'].to_protobuf(key)
        ...
        for name, property in entity._properties.items():
            if isinstance(property, KeyProperty):
                continue
            value_is_reference = isinstance(property, ReferenceProperty)
            if value_is_reference:
                value = entity._data[property.name]
            else:
    ...
    def put(self, entity):
        ...
        if entity.key is None:
            raise ValueError(b'Entity must have a key')
    ...
    def delete(self, entity):
        ...
        if entity.key.is_partial:
            raise ValueError(b'Entity myst have a complete key')
        key_pb = entity._properties[b'key'].to_protobuf(entity.key)
    ...
    def begin(self):
        ...
        if self._status != self._INITIAL:
            raise ValueError(b'Transaction already started previously.')
    ...
    def commit(self):
        ...
        try:
            ...
            completed_keys = [ mut_result.key for mut_result in mut_results if mut_result.HasField(b'key') ]
            for new_key_pb, entity in zip(completed_keys, self._auto_id_entities):
                entity._data[b'key']._id = new_key_pb.path[(-1)].id
