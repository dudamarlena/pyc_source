...
# [GCC 8.4.0]
# Embedded file name: /Users/rstuart/.pyenv/versions/gcloudoem/lib/python2.7/site-packages/gcloudoem/django/auth.py
# Compiled at: 2015-04-21 23:49:15
...
try:
    ...
    def get_hexdigest(algorithm, salt, raw_password):
        ...
        raise ValueError(b'Got unknown password algorithm type in password')


    def check_password(raw_password, password):
        algo, salt, hash = password.split(b'$')
    ...
    def make_password(raw_password):
        ...
        return b'%s$%s$%s' % (algo, salt, hash)


class ContentType(Entity):
    name = TextProperty(max_length=100)
    app_label = TextProperty(max_length=100)
    model = TextProperty(max_length=100, verbose_name=_(b'python model class name'))
    objects = ContentTypeManager()

    class Meta:
        verbose_name = _(b'content type')
        verbose_name_plural = _(b'content types')
        kind = b'django_content_type'
        ordering = ('name', )
...
class PermissionManager(models.Manager):
    ...
    def get_by_natural_key(self, codename, app_label, model):
        return self.get(codename=codename, content_type=ContentType.objects.get_by_natural_key(app_label, model))
...
class Permission(Entity):
    ...
    """
    name = TextProperty(max_length=50, verbose_name=_(b'username'))
    codename = TextProperty(max_length=100, verbose_name=_(b'codename'))
    objects = PermissionManager()

    class Meta:
        verbose_name = _(b'permission')
        verbose_name_plural = _(b'permissions')

    def __unicode__(self):
        return b'%s | %s | %s' % (str(self.content_type.app_label), str(self.content_type), str(self.name))

    def natural_key(self):
        return (
         self.codename,) + self.content_type.natural_key()

    natural_key.dependencies = [b'contenttypes.contenttype']
...
class Group(Entity):
    ...
    """
    name = TextProperty(max_length=80, verbose_name=_(b'name'))
    permissions = ListProperty(ReferenceProperty(Permission, verbose_name=_(b'permissions'), required=False))

    class Meta:
        verbose_name = _(b'group')
        verbose_name_plural = _(b'groups')
...
class UserManager(models.Manager):
    ...
    def create_user(self, username, email, password=None):
        ...
        now = datetime_now()
        try:
            email_name, domain_part = email.strip().split(b'@', 1)
        except ValueError:
            pass
        else:
            email = (b'@').join([email_name, domain_part.lower()])
    ...
    def make_random_password(self, length=10, allowed_chars=b'abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789'):
        """Generates a random password with the given length and given allowed_chars."""
        from random import choice
        return (b'').join([ choice(allowed_chars) for _ in range(length) ])
...
class User(Entity):
    ...
    """
    username = TextProperty(max_length=30, required=True, verbose_name=_(b'username'), help_text=_(b'Required. 30 characters or fewer. Letters, numbers and @/./+/-/_ characters'))
    first_name = TextProperty(max_length=30, verbose_name=_(b'first name'))
    last_name = TextProperty(max_length=30, verbose_name=_(b'last name'))
    email = EmailProperty(verbose_name=_(b'e-mail address'))
    password = TextProperty(max_length=128, verbose_name=_(b'password'), help_text=_(b'Use \'[algo]$[iterations]$[salt]$[hexdigest]\' or use the <a href="password/">change password form</a>.'))
    is_staff = BooleanProperty(default=False, verbose_name=_(b'staff status'), help_text=_(b'Designates whether the user can log into this admin site.'))
    is_active = BooleanProperty(default=True, verbose_name=_(b'active'), help_text=_(b'Designates whether this user should be treated as active. Unselect this instead of deleting accounts.'))
    is_superuser = BooleanProperty(default=False, verbose_name=_(b'superuser status'), help_text=_(b'Designates that this user has all permissions without explicitly assigning them.'))
    last_login = DateTimeProperty(default=datetime_now, verbose_name=_(b'last login'))
    date_joined = DateTimeProperty(default=datetime_now, verbose_name=_(b'date joined'))
    user_permissions = ListProperty(ReferenceProperty(Permission), verbose_name=_(b'user permissions'), help_text=_(b'Permissions for the user.'))
    USERNAME_FIELD = b'username'
    REQUIRED_FIELDS = [b'email']
    ...
    def get_full_name(self):
        ...
        """
        full_name = b'%s %s' % (self.first_name or b'', self.last_name or b'')
    ...
    def get_group_permissions(self, obj=None):
        ...
        for backend in auth.get_backends():
            if hasattr(backend, b'get_group_permissions'):
    ...
    def get_profile(self):
        ...
        profiles.
        """
        if not hasattr(self, b'_profile_cache'):
            from django.conf import settings
            if not getattr(settings, b'AUTH_PROFILE_MODULE', False):
                raise SiteProfileNotAvailable(b'You need to set AUTH_PROFILE_MODULE in your project settings')
            try:
                app_label, model_name = settings.AUTH_PROFILE_MODULE.split(b'.')
            except ValueError:
                raise SiteProfileNotAvailable(b'app_label and model_name should be separated by a dot in the AUTH_PROFILE_MODULE setting')

            try:
                model = models.get_model(app_label, model_name)
                if model is None:
                    raise SiteProfileNotAvailable(b'Unable to load the profile model, check AUTH_PROFILE_MODULE in your project settings')
    ...
    def authenticate(self, username=None, password=None):
        ...
        if user:
            if password and user.check_password(password):
                ...
                user.backend = b'%s.%s' % (backend.__module__, backend.__class__.__name__)
                return user
        return
