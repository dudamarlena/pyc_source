...
# [GCC 8.4.0]
# Embedded file name: /Users/rstuart/.pyenv/versions/gcloudoem/lib/python2.7/site-packages/gcloudoem/entity.py
# Compiled at: 2016-10-04 01:35:05
"""
...
class Entity(with_metaclass(EntityMeta, BaseEntity)):
    ...
    * A ``key`` attribute of type :class:`~gcloudoem.properties.KeyProperty` will be injected onto the instance.
    ...
    def from_protobuf(cls, pb):
        key = None
        if pb.HasField(b'key'):
            key = cls._properties[b'key'].from_protobuf(pb.key)
        entity_props = {b'key': key}
        for name, property_pb in six.iteritems(pb.properties):
            if not hasattr(cls, name):
                raise ValueError(b"Entity %s doesn't have a property by the name %s" % (
                 cls._meta.kind, name))
