...
# [GCC 8.4.0]
# Embedded file name: /Users/rstuart/.pyenv/versions/gcloudoem/lib/python2.7/site-packages/gcloudoem/datastore/query.py
# Compiled at: 2016-10-04 01:54:38
...
class Query(object):
    ...
    """
    OPERATORS = {'<=': query_pb.PropertyFilter.LESS_THAN_OR_EQUAL, 
    ...
    def __init__(self, entity, ancestor=None, filters=(), projection=(), order=(), group_by=(), limit=None, offset=0):
        """
        ...
        if self.OPERATORS[operator] in (
         query_pb.PropertyFilter.LESS_THAN_OR_EQUAL, query_pb.PropertyFilter.GREATER_THAN_OR_EQUAL):
            if not self._has_inequality_filter:
                ...
                raise InvalidQueryError('Datastore only supports inequality operators on a single property within a query.')
    ...
    def clone(self):
        return self.__class__(self._entity, self._ancestor, self.filters, self.projection, self.order, self.group_by, self._limit, self._offset)
...
class Cursor(object):
    ...
    def next_page(self):
        ...
        query_results = self._connection.run_query(query_pb=pb, namespace=self._connection.namespace, transaction_id=transaction and transaction.id)
