...
# [GCC 8.4.0]
# Embedded file name: /Users/bugnofree/DevSpace/sshmgr/src/__main__.py
# Compiled at: 2019-07-24 01:49:49
...
def parse_targets(input_hosts):
    ...
    if input_hosts.startswith(flag_file):
        ...
        if input_hosts.startswith(flag_list):
    ...
    for host in hosts:
        ...
        if conn is None:
            ...
            if args.net:
                infokind.append('net')
            if args.factory:
                infokind.append('factory')
            if args.cpu:
                infokind.append('cpu')
            if args.mem:
                infokind.append('mem')
            if args.disk:
                infokind.append('disk')
...
def parse_docker_cmd(args):
    if not args.hosts:
        ...
        if args.hosts.startswith(flag_file):
            ...
            if args.hosts.startswith(flag_list):
    ...
    for curhost in hosts:
        ...
        if args.new:
            ...
            dckrinfo, ok = ssh.newdckr((args.new), apt=(args.apt), dockerfile=(args.fdocker),
              himsg=(args.himsg),
...
def main():
    ...
    parser.add_argument('-v', '--version', action='version', version=(util.get_version()), help='Show the version of sshmgr')
    parser.add_argument('--hosts', nargs='?', metavar='hosts', type=str, help='The host(s) to be operated on')
    parser.add_argument('--sshkey', type=str, metavar='path_to_the_new_ssh_key', help='Update ssh key for administrator')
    ...
    docker_cmd_parser.add_argument('--require-ports', metavar='the_required_ports',
      nargs='+',
      type=int,
      default=[],
      help='The required ports exposed on host for your user.')
    docker_cmd_parser.add_argument('--portnum', metavar='the_number_of_port',
      type=int,
      default=3,
      help='The required ports exposed on host for your user.')
    docker_cmd_parser.add_argument('--apt', choices=[
     'ubuntu.16.04.offical', 'ubuntu.16.04.tsinghua'],
      default='ubuntu.16.04.offical',
      help='Select apt source.list, the default is ubuntu.16.04.offical')
    docker_cmd_parser.add_argument('--himsg', metavar='hello_message', default='Your identifier is:',
      type=str,
