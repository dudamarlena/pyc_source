...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-jsonattrs/jsonattrs/models.py
# Compiled at: 2018-06-01 08:19:36
# Size of source mod 2**32: 14152 bytes
...
def schema_cache_key(content_type, selectors):
    return 'jsonattrs:schema:' + content_type.app_label + ',' + content_type.model + ':' + ','.join([str(s) for s in selectors])
    ...
    def _get_selectors(self, instance, content_type=None):
        ...
        if len(self.content_type_to_selectors) == 0:
            for k, v in settings.JSONATTRS_SCHEMA_SELECTORS.items():
                a, m = k.split('.')
                self.content_type_to_selectors[ContentType.objects.get(app_label=a, model=m)] = v
...
class Schema(models.Model):
    ...
    selectors = ArrayField(models.CharField(max_length=256, blank=True), default=list, blank=True)
    ...
    def __str__(self):
        return '<Schema #{}: {}+{}>'.format(self.id, self.content_type if self.content_type_id is not None else 'None', self.selectors)
...
def compose_schemas(*schemas):
    ...
    if cached:
        ...
        return (
         attrs, required_attrs, default_attrs)
    schema_attrs = [s.attributes.select_related('attr_type').all() for s in schemas]
    attrs = OrderedDict()
    required_attrs = set()
    default_attrs = set()
    ...
    required_attrs = {n for n, a in attrs.items() if a.required if a.required}
    default_attrs = {n for n, a in attrs.items() if a.default is not None and a.default != '' if a.default is not None and a.default != ''}
...
def create_attribute_type(name, label, form_field, widget=None, validator_re=None, validator_type=None):
    if not AttributeType.objects.filter(name=name).exists():
        AttributeType.objects.create(name=name, label=label, form_field=form_field, widget=widget, validator_re=validator_re, validator_type=validator_type)
...
def create_attribute_types():
    create_attribute_type('boolean', 'Boolean', 'BooleanField', validator_type='bool', validator_re='true|false|True|False')
    create_attribute_type('text', 'Text', 'CharField', validator_type='str')
    create_attribute_type('text_multiline', 'Multiline text', 'CharField', validator_type='str', widget='Textarea')
    ...
    create_attribute_type('integer', 'Integer', 'IntegerField', validator_re='[-+]?\\d+')
    create_attribute_type('decimal', 'Decimal number', 'DecimalField', validator_re='[-+]?\\d+(\\.\\d+)?')
    ...
    create_attribute_type('select_multiple', 'Select multiple:', 'MultipleChoiceField')
        ...
        if choices is not None and choices != []:
            if choice_labels is not None:
                if len(choices) != len(choice_labels):
                    raise ValueError("lengths of choices and choice_labels don't match for Attribute")
                if not all([isinstance(l, str) or isinstance(l, dict) for l in choice_labels]):
                    raise ValueError('non-string choice label in Attribute')
            else:
                allstr = all([isinstance(c, str) for c in choices])
                alltuple2 = all([isinstance(c, tuple) and len(c) == 2 and isinstance(c[1], str) for c in choices])
...
class Attribute(models.Model):
    schema = models.ForeignKey(Schema, related_name='attributes', on_delete=models.CASCADE)
    name = models.CharField(max_length=256)
    ...
    def long_name(self):
        if self.long_name_xlat is None or isinstance(self.long_name_xlat, str):
            ...
            return self.long_name_xlat.get(get_language(), self.long_name_xlat[self.schema.default_language])
    ...
    def choice_labels(self):
        if self.choice_labels_xlat is None or isinstance(self.choice_labels_xlat, (list, tuple)) and (len(self.choice_labels_xlat) == 0 or isinstance(self.choice_labels_xlat[0], str)):
            return self.choice_labels_xlat
        else:
            return [cl.get(get_language(), cl[self.schema.default_language]) for cl in self.choice_labels_xlat]
    ...
    def validate(self, value):
        empty_vals = (
         '', [''])
        if self.required and self.default == '' and (value is None or value in empty_vals):
            raise ValidationError(_('Missing required field %(field)s'), params={'field': self.name})
        atype = self.attr_type
        if value in empty_vals and atype.name in ('integer', 'decimal', 'select_one',
                                                  'select_multiple'):
            value = None
            ...
            if type(value) == list:
                for v in value:
                    if v not in self.choices:
                        raise ValidationError(_('Invalid choice for %(field)s: "%(value)s"'), params={'field': self.name, 'value': v})

        elif value not in self.choices:
            raise ValidationError(_('Invalid choice for %(field)s: "%(value)s"'), params={'field': self.name, 'value': value})
        if isinstance(value, str):
            if atype.validator_re is not None and re.match(atype.validator_re, value) is None:
                raise ValidationError(_('Validation failed for %(field)s: "%(value)s"'), params={'field': self.name, 'value': value})
    ...
    def to_dict(self):
        return {k:v for k, v in self.__dict__.items() if k in [f.attname for f in self._meta.fields]}