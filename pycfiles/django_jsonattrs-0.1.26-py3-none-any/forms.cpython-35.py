...
# [GCC 8.4.0]
# Embedded file name: /Volumes/Work/dev/cadasta/django-jsonattrs/jsonattrs/forms.py
...
FORM_FIELDS = {'BooleanField': forms.BooleanField, 
...
def form_field_from_name(name):
    ...
    raise ValueError("Invalid form field name for attribute: '{}'".format(name))
    ...
    def set_initial(self, args, name, attr, attrvals):
        if name in attrvals:
            if attr.attr_type.form_field == 'BooleanField':
                args['initial'] = attrvals[name] if isinstance(attrvals[name], bool) else attrvals[name] != 'False'
        else:
