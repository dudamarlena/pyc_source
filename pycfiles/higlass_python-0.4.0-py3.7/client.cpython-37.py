...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/higlass/client.py
# Compiled at: 2020-01-26 00:21:31
# Size of source mod 2**32: 24066 bytes
from copy import deepcopy
import json, slugid, logging, os
...
__all__ = [
 'Track', 'CombinedTrack', 'View', 'ViewConf']
_track_default_position = {'2d-rectangle-domains':'center', 
 'bedlike':'top', 
 'horizontal-bar':'top', 
...
_datatype_default_track = {'2d-rectangle-domains':'2d-rectangle-domains', 
 'bedlike':'bedlike', 
...
class Track(Component):
    ...
    def __init__(self, track_type=None, position=None, tileset=None, file_url=None, filetype=None, options=None, **kwargs):
...
class DividedTrack(Track):
    __doc__ = 'A track representing one tileset divided by another.\n\n    Only works with some tileset types.\n    '

    def __init__(self, numerator, denominator, *args, **kwargs):
        if numerator.conf['type'] != denominator.conf['type']:
            raise ValueError(f"Different track types: {numerator.conf['type']}, {denominator.conf['type']}")
        ...
        data_config = {'type':'divided', 
         'children':[
          {'server':numerator_server, 
           'tilesetUid':numerator_uuid},
          {'server':denominator_server, 
           'tilesetUid':denominator_uuid}]}
        (super().__init__)(args, data=data_config, type=track_type, position=position, options=options, height=height, **kwargs)
...
class View(Component):
    ...
    def __init__(self, tracks=[], x=0, y=0, width=12, height=6, initialXDomain=None, initialYDomain=None, uid=None, overlays=[], chrominfo=None, geneinfo=None):
        if uid is None:
            uid = slugid.nice()
        else:
            self.uid = uid
            self.conf = {'uid':uid, 
             'tracks':{'top':[],  'center':[],  'left':[],  'right':[],  'bottom':[]},  'layout':{'w':width, 
              'h':height,  'x':x,  'y':y}}
    ...
    def add_track(self, track, position=None):
        ...
        if position is None:
            if track.position is not None:
                ...
                if track.type in _track_default_position:
    ...
    def from_dict(cls, conf):
        ...
        if 'genomePositionSearchBox' in conf:
            self.conf['genomePositionSearchBox'] = json.loads(json.dumps(conf['genomePositionSearchBox']))
    ...
    def add_overlay(self, overlay):
        ...
        try:
            ...
            overlay_conf = {'uid':overlay.get('uid', 'overlay'), 
...
class ViewConf(Component):
    __doc__ = 'Configure a dashboard'

    def __init__(self, views=[], location_syncs=[], value_scale_syncs=[], zoom_syncs=[]):
        self.conf = {'editable':True, 
         'views':[],  'trackSourceServers':[
          'http://higlass.io/api/v1'], 
         'locationLocks':{'locksByViewUid':{},  'locksDict':{}},  'valueScaleLocks':{'locksByViewUid':{},  'locksDict':{}},  'zoomLocks':{'locksByViewUid':{},  'locksDict':{}},  'exportViewUrl':'http://higlass.io/api/v1/viewconfs'}
        self._views_by_id = {}
        for view in views:
            self.add_view(view)
    ...
    def _extract_view_track_uids(self, definition):
        if isinstance(definition, tuple):
            ...
            if isinstance(definition, Track):
        ...
        for definition in tracks_to_sync:
            ...
            if not v_uid is None:
                ...
                if lock_id not in locks_dict:
                    locks_dict[lock_id] = {}
                locks_dict[lock_id][vt_uid] = {'view':v_uid, 
                 'track':t_uid}
    ...
    def from_link(cls, url):
        ...
        if parts.path.strip('/') == 'app':
            ...
            if parts.path.strip('/') in ('api/v1/viewconfs', 'l', 'link'):
                if 'd' not in query:
                    raise ValueError('Viewconf ID not found in query')
                conf_id = query['d'][0]
            else:
