...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/higlass/server.py
# Compiled at: 2020-01-26 00:21:31
# Size of source mod 2**32: 14386 bytes
from functools import partial
from io import StringIO
import multiprocess as mp, cytoolz as toolz
import os.path as op
import platform, logging, logging.handlers, socket, tempfile, json, time, os
from flask import Flask
from flask import request, jsonify
from flask_cors import CORS
from fuse import FUSE
import requests, slugid, sh
__all__ = [
 'Server']
...
def create_app(name, tilesets, fuse=None):
    ...
    def register_url():
        ...
        if js['filetype'] not in by_filetype:
            return (jsonify({'error': 'Unknown filetype: {}'.format(js['filetype'])}),
             400)
        if fuse is None:
    ...
    def chrom_sizes():
        ...
        if ts is None:
            ...
            return hasattr(ts, 'chromsizes') or jsonify({'error': 'Tileset does not have chrom sizes.'})
        data = ts.chromsizes
        if incl_cum:
            ...
            if incl_cum:
                j = {ts.uuid: {chrom:{'size':size,  'offset':offset} for chrom, size, offset in data}}
            else:
                j = {ts.uuid: {chrom:{'size': size} for chrom, size in data}}
            return jsonify(j)
            if res_type == 'tsv':
                if incl_cum:
                    return '\n'.join(('{}\t{}\t{}'.format(chrom, size, offset) for chrom, size, offset in data))
                return '\n'.join(('{}\t{}'.format(chrom, size) for chrom, size in data))
        else:
            return (
             jsonify({'error': 'Unknown response type'}), 500)

    @app.route('/api/v1/uids_by_filename/', methods=['GET'])
    def uids_by_filename():
        return jsonify({'count':len(tilesets), 
    ...
    def list_tilesets():
        ...
        return jsonify({'count':len(tsets), 
         'next':None, 
         'previous':None, 
    ...
    def tiles():
        ...
        if not tids_requested:
            return (
             jsonify({'error': 'No tiles requested'}), 400)
...
class FuseProcess:
    ...
    def setup(self):
        ...
        self.teardown()
        disk_cache_size = 33554432
        lru_capacity = 400

        def start_fuse(directory, protocol):
            try:
                fuse = FUSE(HttpFs(protocol,
...
class Server:
    ...
    def __init__(self, tilesets, host='localhost', port=None, name=None, fuse=True, tmp_dir=OS_TEMPDIR, log_level=logging.INFO, log_file=None):
 ...
 self.app.run, debug=debug, 
    ...
    def tileset_info(self, uid):
        ...
        """
        url = 'http://{host}:{port}/api/v1/tileset_info/?d={uid}'.format(host=(self.host),
          port=(self.port),
          uid=uid)
    ...
    def tiles(self, uid, z, x, y=None):
        ...
        url = 'http://{host}:{port}/api/v1/tiles/?d={tile_id}'.format(host=(self.host),
          port=(self.port),
          tile_id=tile_id)
        req = requests.get(url)
        if req.status_code != 200:
    ...
    def chromsizes(self, uid):
        ...
        """
        url = 'http://{host}:{port}/api/v1/chrom-sizes/?id={uid}'.format(host=(self.host),
          port=(self.port),
          uid=uid)
        req = requests.get(url)
        if req.status_code != 200:
