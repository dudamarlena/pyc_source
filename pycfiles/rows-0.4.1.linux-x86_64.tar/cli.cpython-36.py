...
# [GCC 8.4.0]
# Embedded file name: /home/turicas/software/pyenv/versions/rows/lib/python3.6/site-packages/rows/cli.py
# Compiled at: 2019-02-13 03:47:25
# Size of source mod 2**32: 27235 bytes
import os, pathlib, sqlite3, sys
from io import BytesIO
import click, requests.exceptions, requests_cache, six
from tqdm import tqdm
import rows
from rows.fields import make_header
from rows.utils import ProgressBar, csv_to_sqlite, detect_source, download_file, export_to_uri, generate_schema, import_from_source, import_from_uri, load_schema, pgexport, pgimport, sqlite_to_csv, uncompressed_size
...
def _import_table(source, encoding, verify_ssl=True, *args, **kwargs):
 ...
 source, *args, default_encoding=DEFAULT_INPUT_ENCODING, verify_ssl=verify_ssl, encoding=encoding, progress=True, **kwargs)
    except requests.exceptions.SSLError:
        click.echo('ERROR: SSL verification failed! Use `--verify-ssl=no` if you want to ignore.',
          err=True)
        sys.exit(2)
    else:
        return table
...
class AliasedGroup(click.Group):
    ...
    def get_command(self, ctx, cmd_name):
        return click.Group.get_command(self, ctx, cmd_name) or click.Group.get_command(self, ctx, cmd_name.replace('2', '-to-'))
...
def convert(input_encoding, output_encoding, input_locale, output_locale, verify_ssl, order_by, fields, fields_exclude, source, destination):
    ...
    if order_by is not None:
        ...
        if output_locale is not None:
            ...
            export_to_uri(table,
              destination, encoding=output_encoding, export_fields=export_fields)
...
@click.argument('destination')
def join(input_encoding, output_encoding, input_locale, output_locale, verify_ssl, order_by, fields, fields_exclude, keys, sources, destination):
...
@click.argument('sources', nargs=(-1), required=True)
@click.argument('destination')
def sum_(input_encoding, output_encoding, input_locale, output_locale, verify_ssl, order_by, fields, fields_exclude, sources, destination):
    import_fields = _get_import_fields(fields, fields_exclude)
    if input_locale is not None:
        with rows.locale_context(input_locale):
            tables = [_import_table(source, encoding=input_encoding, verify_ssl=verify_ssl, import_fields=import_fields) for source in sources]
    else:
        tables = [_import_table(source, encoding=input_encoding, verify_ssl=verify_ssl, import_fields=import_fields) for source in sources]
    result = sum(tables)
    if order_by is not None:
...
@click.option('--input-locale')
@click.option('--output-locale')
@click.option('--frame-style',
  default='ascii', help='Options: ascii, single, double, none')
...
def print_(input_encoding, output_encoding, input_locale, output_locale, frame_style, table_index, verify_ssl, fields, fields_exclude, order_by, source):
    import_fields = _get_import_fields(fields, fields_exclude)
    if input_locale is not None:
        with rows.locale_context(input_locale):
            table = _import_table(source,
...
@click.argument('sources', nargs=(-1), required=True)
def query(input_encoding, output_encoding, input_locale, output_locale, verify_ssl, samples, output, frame_style, query, sources):
    ...
    if not query.lower().startswith('select'):
        ...
        if len(sources) == 1:
            ...
            if source.plugin_name in ('sqlite', 'postgresql'):
                result = import_from_source(source,
                  DEFAULT_INPUT_ENCODING, query=query, samples=samples)
            else:
                if input_locale is not None:
                    with rows.locale_context(input_locale):
                        table = import_from_source(source,
                          DEFAULT_INPUT_ENCODING, samples=samples)
                else:
                    table = import_from_source(source,
                      DEFAULT_INPUT_ENCODING, samples=samples)
            ...
            if input_locale is not None:
                with rows.locale_context(input_locale):
                    tables = [_import_table(source, encoding=input_encoding, verify_ssl=verify_ssl, samples=samples) for source in sources]
            else:
                tables = [_import_table(source, encoding=input_encoding, verify_ssl=verify_ssl, samples=samples) for source in sources]
            sqlite_connection = sqlite3.Connection(':memory:')
            for index, table in enumerate(tables, start=1):
                rows.export_to_sqlite(table,
                  sqlite_connection, table_name=('table{}'.format(index)))
        ...
        if output is None:
            ...
            if output_locale is not None:
                with rows.locale_context(output_locale):
                    rows.export_to_txt(result,
                      fobj, encoding=output_encoding, frame_style=frame_style)
            else:
                rows.export_to_txt(result,
                  fobj, encoding=output_encoding, frame_style=frame_style)
            fobj.seek(0)
            click.echo(fobj.read())
        elif output_locale is not None:
            with rows.locale_context(output_locale):
...
def schema(input_encoding, input_locale, verify_ssl, output_format, fields, fields_exclude, samples, source, output):
    samples = samples if samples > 0 else None
    import_fields = _get_import_fields(fields, fields_exclude)
    ...
    if input_locale is not None:
        ...
        table = import_from_source(source,
          DEFAULT_INPUT_ENCODING, samples=samples, import_fields=import_fields)
    export_fields = _get_export_fields(table.field_names, fields_exclude)
    if export_fields is None:
...
def command_sqlite_to_csv(batch_size, dialect, source, table_name, output):
    ...
    prefix = '[{db_filename}#{tablename} -> {filename}]'.format(db_filename=(input_filename.name),
      tablename=table_name,
      filename=(output_filename.name))
    progress = ProgressBar(prefix=prefix, pre_prefix='')
    sqlite_to_csv(input_filename=(six.text_type(input_filename)),
      table_name=table_name,
      dialect=dialect,
...
def command_pgimport(input_encoding, no_create_table, dialect, schema, source, database_uri, table_name):
    progress = ProgressBar(prefix='Importing data',
    ...
    import_meta = pgimport(filename=source,
      encoding=input_encoding,
      dialect=dialect,
...
def command_pgexport(output_encoding, dialect, database_uri, table_name, destination):
    ...
    pgexport(database_uri=database_uri,
      table_name=table_name,
