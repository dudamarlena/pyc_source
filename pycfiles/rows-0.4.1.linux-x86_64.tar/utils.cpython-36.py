...
# [GCC 8.4.0]
# Embedded file name: /home/turicas/software/pyenv/versions/rows/lib/python3.6/site-packages/rows/utils.py
# Compiled at: 2019-02-14 16:10:52
# Size of source mod 2**32: 31943 bytes
from __future__ import unicode_literals
import cgi, csv, gzip, io, itertools, mimetypes, os, re, shlex, sqlite3, subprocess, tempfile
...
try:
    ...
    TEXT_PLAIN = {'txt':'text/txt', 
     'text':'text/txt', 
     'csv':'text/csv', 
     'json':'application/json'}
    OCTET_STREAM = {'microsoft ooxml':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 
    ...
    FILE_EXTENSIONS = {'csv':'text/csv', 
     ...
     'pdf':'application/pdf'}
    MIME_TYPE_TO_PLUGIN_NAME = {'application/json':'json', 
    ...
    POSTGRESQL_TYPES = {rows.fields.BinaryField: 'BYTEA', 
     ...
     rows.fields.IntegerField: 'BIGINT', 
    ...
    class ProgressBar:
        ...
        def __init__(self, prefix, pre_prefix='', total=None, unit=' rows'):
            self.prefix = prefix
            self.progress = tqdm(desc=pre_prefix,
              total=total,
              unit=unit,
              unit_scale=True,
              dynamic_ncols=True)
    ...
    class Source(object):
        __doc__ = 'Define a source to import a `rows.Table`'
        __slots__ = [
         'plugin_name', 'uri', 'encoding', 'delete']
        ...
        def __repr__(self):
            return 'Source(plugin_name={}, uri={}, encoding={}, delete={})'.format(self.plugin_name, self.uri, self.encoding, self.delete)
    ...
    def normalize_mime_type(mime_type, mime_name, file_extension):
        ...
        if mime_type == 'text/plain':
            if file_extension in TEXT_PLAIN:
                return TEXT_PLAIN[file_extension]
        if mime_type == 'application/octet-stream':
            if mime_name in OCTET_STREAM:
    ...
    def download_file(uri, filename=None, verify_ssl=True, timeout=5, progress=False, detect=False, chunk_size=8192, sample_size=1048576):
    ...
    """
        if uri.lower().startswith('http://') or uri.lower().startswith('https://'):
            return download_file(uri,
              verify_ssl=verify_ssl, timeout=timeout, progress=progress, detect=True)
        else:
    ...
    def csv_to_sqlite(input_filename, output_filename, samples=None, dialect=None, batch_size=10000, encoding='utf-8', callback=None, force_types=None, chunk_size=8388608, table_name='table1', schema=None):
        ...
        def fobj(self):
            if self._fobj is None:
                if getattr(self.filename_or_fobj, 'read', None) is not None:
                    ...
                    self._fobj = open_compressed((self.filename_or_fobj),
                      mode='w', encoding=(self.encoding))
            return self._fobj
    ...
    def get_psql_command(command, user=None, password=None, host=None, port=None, database_name=None, database_uri=None):
        if database_uri is None:
            ...
            database_uri = 'postgres://{user}:{password}@{host}:{port}/{name}'.format(user=user,
              password=password,
              host=host,
              port=port,
              name=database_name)
    ...
    def get_psql_copy_command(table_name, header, encoding='utf-8', user=None, password=None, host=None, port=None, database_name=None, database_uri=None, dialect=csv.excel, direction='FROM'):
        ...
        copy = "\\copy {table_name} {header}{direction} STDIN DELIMITER '{delimiter}' QUOTE '{quote}' ENCODING '{encoding}' CSV HEADER;".format(table_name=table_name,
          header=header,
          direction=direction,
    ...
    def pgimport(filename, database_uri, table_name, encoding='utf-8', dialect=None, create_table=True, schema=None, callback=None, timeout=0.1, chunk_size=8388608, max_samples=10000):
    ...
    """
        ...
        sample = fobj.read(chunk_size)
        if dialect is None:
            dialect = rows.plugins.csv.discover_dialect((sample.encode(encoding)),
              encoding=encoding)
        else:
            if isinstance(dialect, six.text_type):
                dialect = csv.get_dialect(dialect)
        ...
        if create_table:
            if schema is None:
                data = [dict(zip(field_names, row)) for row in itertools.islice(reader, max_samples)]
            ...
            columns = ['{} {}'.format(name, POSTGRESQL_TYPES.get(type_, DEFAULT_POSTGRESQL_TYPE)) for name, type_ in zip(field_names, field_types)]
            create_table = SQL_CREATE_TABLE.format(table_name=table_name,
              field_types=(', '.join(columns)))
            execute_command(get_psql_command(create_table, database_uri=database_uri))
        command = get_psql_copy_command(database_uri=database_uri,
          dialect=dialect,
          direction='FROM',
          encoding=encoding,
          header=field_names,
    ...
    """
        if output_format == 'txt':
            ...
            if output_format == 'sql':
                sql_fields = {rows.fields.BinaryField: 'BLOB', 
                ...
                fields = ['    {} {}'.format(field_name, sql_fields[field_type]) for field_name, field_type in table.fields.items() if field_name in export_fields]
                sql = dedent('\n                CREATE TABLE IF NOT EXISTS {name} (\n                {fields}\n                );\n                ').strip().format(name=(table.name),
                  fields=(',\n'.join(fields))) + '\n'
                output_fobj.write(sql)
            elif output_format == 'django':
                django_fields = {rows.fields.BinaryField: 'BinaryField', 
                ...
                if rows.fields.JSONField in [table.fields[field_name] for field_name in export_fields]:
    ...
    """
        ...
        elif not 'field_type' in field_names:
            raise AssertionError
        types = {key:getattr(rows.fields, key) for key in dir(rows.fields) if 'Field' in key if key != 'Field' if key != 'Field'}
        return OrderedDict([(row.field_name, types[(row.field_type.capitalize() + 'Field')]) for row in table])
