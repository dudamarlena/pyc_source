...
# [GCC 8.4.0]
# Embedded file name: /home/e210990/bin/python26/lib/python2.6/site-packages/stashy/pullrequests.py
...
class PullRequestRef(object):
    ...
    def to_dict(self):
        return dict(id=self.id, repository=dict(slug=self.repo_slug, project=dict(key=self.project_key), name=self.repo_slug))
...
class PullRequests(ResourceBase, IterableResource):
    ...
    def _make_ref(self, ref, refName='the ref'):
        ...
        if isinstance(ref, PullRequestRef):
            return ref.to_dict()
    ...
    def create(self, title, fromRef, toRef, description='', state='OPEN', reviewers=None):
        ...
        """
        data = dict(title=title, description=description, fromRef=self._make_ref(fromRef, 'fromRef'), toRef=self._make_ref(toRef, 'toRef'), state=state)
        if reviewers is not None:
            data['reviewers'] = []
