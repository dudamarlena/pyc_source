...
# [GCC 8.4.0]
# Embedded file name: /home/hm/GIT/yahoostats/yahoostats/chromium_downloader.py
# Compiled at: 2020-05-05 15:01:45
...
from yahoostats import __chromium_revision__, __home_path__
logger = logging.getLogger(__name__)
DOWNLOADS_FOLDER = Path(__home_path__) / 'local-chromium'
...
downloadURLs = {'linux':f"{BASE_URL}/Linux_x64/{REVISION}/chrome-linux.zip", 
 'mac':f"{BASE_URL}/Mac/{REVISION}/chrome-mac.zip", 
 'win32':f"{BASE_URL}/Win/{REVISION}/chrome-win32.zip", 
 'win64':f"{BASE_URL}/Win_x64/{REVISION}/chrome-win32.zip"}
chromiumExecutable = {'linux':DOWNLOADS_FOLDER / REVISION / 'chrome-linux' / 'chrome', 
...
def current_platform() -> str:
    ...
    if sys.platform.startswith('darwin'):
        return 'mac'
    if sys.platform.startswith('win') or sys.platform.startswith('msys') or sys.platform.startswith('cyg'):
...
def extract_zip(data: BytesIO, path: Path) -> None:
    ...
    if current_platform() == 'mac':
        ...
        if not shutil.which('unzip'):
            raise OSError(f"Failed to automatically extract chromium.Please unzip {zip_path} manually.")
        proc = subprocess.run([
         'unzip', str(zip_path)],
          cwd=(str(path)),
