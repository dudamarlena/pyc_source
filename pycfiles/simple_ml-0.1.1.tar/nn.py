...
# [GCC 8.4.0]
# Embedded file name: simple_ml/nn/nn.py
# Compiled at: 2016-09-08 22:19:57
from pprint import pprint
import numpy as np, random, itertools, pdb, scipy, copy, sklearn
...
class NeurualNetworkClassifier(object):
    ...
    def __init__(self, hidden_layer_size, activation='relu', cost_function='cross_entrophy', learning_rate=0.1, weight_std=1.0, alpha=1.0, sgd_batch_size=64, sgd_epoch=30, print_every=10):
    ...
    def forward(self, X_train):
        mid1 = np.matmul(X_train, self.W1) + self.b1
        t1 = np.where(mid1 >= 0, mid1, 0)
        mid2 = np.matmul(t1, self.W2) + self.b2
        t2 = np.where(mid2 >= 0, mid2, 0)
        return (
         t2,
         {'input': X_train, 
            't1': t1, 
    ...
    def backward(self, grad, cache):
        ...
        mid2 = cache['mid2']
        X = cache['input']
        d_mid2 = np.where(mid2 >= 0, grad, 0)
        d_t1 = np.matmul(d_mid2, self.W2.T)
        d_b2 = np.sum(d_mid2, axis=0)
        d_W2 = np.matmul(t1.T, d_mid2)
        d_mid1 = np.where(mid1 >= 0, d_t1, 0)
...
if __name__ == '__main__':
    nn = NeurualNetworkClassifier(hidden_layer_size=20, learning_rate=1, alpha=0.0, weight_std=0.1, print_every=1, sgd_epoch=500)
