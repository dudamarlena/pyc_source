...
# [GCC 8.4.0]
# Embedded file name: neural_net.py
# Compiled at: 2016-03-23 20:46:08
from pprint import pprint
import numpy as np, itertools, pdb, scipy, copy, sklearn
from sklearn.metrics import accuracy_score

class NeurualNetworkClassifier(object):

    def __init__(self, hidden_layer_sizes, activation='relu', cost_function='svm', learning_rate=0.1, alpha=1.0, sgd_batch_size=10, sgd_epoch=30):
    ...
    def fit(self, X, y):
        ...
        if self.should_init:
            self.initialize()
        assert self.cost_function == 'mse' and self.hidden_layer_sizes[(-1)] == len(self.labels)
        n = len(y)
        prev_cost = None
        epoch = 0
        while True:
    ...
    def gradient_check(self, X, y):
        ...
        print 'dw: \n', delta_w, '\n=========\n', num_dw
        print 'db: \n', delta_b, '\n=========\n', num_db
        dw_error = [ np.abs(dw - ndw) / np.maximum(dw, ndw) for dw, ndw in itertools.izip(delta_w, num_dw)
                   ]
        db_error = [ np.abs(db - ndb) / np.maximum(db, ndb) for db, ndb in itertools.izip(delta_b, num_db)
                   ]
        print '==========================='
        pprint(dw_error)
        pprint(db_error)
