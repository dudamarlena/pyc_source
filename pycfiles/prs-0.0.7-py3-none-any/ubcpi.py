...
# Compiled at: 2016-11-21 18:45:09
__doc__ = 'A Peer Instruction tool for edX by the University of British Columbia.'
import os, random
...
def validate_options(options):
    ...
    if not any(error in ('Minimum Characters', 'Maximum Characters') for error in errors) and int(options['rationale_size']['max']) <= int(options['rationale_size']['min']):
...
class PeerInstructionXBlock(XBlock, MissingDataFetcherMixin, PublishEventMixin):
    ...
    options = List(default=[{'text': _('Air'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}, {'text': _('Soil'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}, {'text': _('Water'), 'image_url': '', 'image_position': 'below', 'image_show_fields': 0, 'image_alt': ''}], scope=Scope.content, help='The possible options from which the student may select')
    ...
    def student_view(self, context=None):
        ...
        if answers.has_revision(0) and not answers.has_revision(1):
            js_vals['other_answers'] = get_other_answers(self.sys_selected_answers, self.seeds, self.get_student_item_dict, self.algo, self.options)
    ...
    def get_persisted_data(self):
        ...
        ret = {'answer_original': answers.get_vote(0), 
           ...
           'rationale_revised': answers.get_rationale(1)}
        if answers.has_revision(0) and not answers.has_revision(1):
            ret['other_answers'] = get_other_answers(self.sys_selected_answers, self.seeds, self.get_student_item_dict, self.algo, self.options)
        if answers.has_revision(1):
