...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ubcpi/test/test_answer_pool.py
# Compiled at: 2015-08-31 13:52:46
import unittest
from ddt import file_data, ddt
from mock import patch, call, MagicMock
from ubcpi.answer_pool import offer_answer, validate_seeded_answers_simple, UnknownChooseAnswerAlgorithm, validate_seeded_answers_random, validate_seeded_answers, get_other_answers, get_other_answers_simple, get_other_answers_random
...
class TestAnswerPool(unittest.TestCase):
    ...
    def test_simple_algo_insert_max(self):
        ...
        for i in range(50):
            for cur_expected_answer in options:
                ...
                offer_answer(pool, cur_expected_answer, cur_expected_rationale, cur_expected_student_id, {'name': 'simple'})
    ...
    def test_validate_seeded_answers_simple(self):
        ...
        answers = [{'answer': 0, 'desc': 'rationale A'}, {'answer': 1, 'desc': 'rationale B'}, {'answer': 2, 'desc': 'rationale C'}]
        self.assertIsNone(validate_seeded_answers_simple(answers, options, {'algo': 'simple'}))
        options = [{'text': 'option1'}, {'text': 'optionA'}, {'text': 'optionB'}, {'text': 'optionC'}, {'text': 'optionD'}]
        self.assertEqual(validate_seeded_answers_simple(answers, options, {'algo': 'simple'}), {'seed_error': 'Missing option seed(s): Option 4, Option 5'})
    ...
    def test_get_other_answers_simple(self, data, mock):
        ...
        with patch('random.choice', side_effect=data['choice_result']):
            result = get_other_answers_simple(data['pool'], data['seeds'], student_item_dict_func, data['num_responses'])
    ...
    def test_get_other_answers_random(self, data, mock):
        ...
        with patch('random.shuffle', side_effect=side_effect):
            result = get_other_answers_random(data['pool'], data['seeds'], student_item_dict_func, data['num_responses'])
        self.assertEqual(result, data['expect'])

    def test_validate_seeded_answers_random(self):
        self.assertIsNone(validate_seeded_answers_random([{'answer': 1}]))
        self.assertEqual(validate_seeded_answers_random([]), {'seed_error': 'Missing 1 option seed'})
