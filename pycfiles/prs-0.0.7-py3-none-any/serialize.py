...
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/ubcpi/serialize.py
# Compiled at: 2015-12-16 15:01:15
    ...
    if image_el is None:
        ...
        for attr, key in IMAGE_ATTRIBUTES.iteritems():
            if attr in image_el.attrib:
                image_dict[key] = int(image_el.attrib[attr]) if unicode(image_el.attrib[attr]).isnumeric() else unicode(image_el.attrib[attr])
...
def parse_options_xml(root):
    ...
    return (
     options, correct_option, rationale)
...
def parse_from_xml(root):
    ...
    return {'display_name': display_name, 
       ...
       'options': options, 
       'rationale_size': {'min': rationale_size_min, 'max': rationale_size_max}, 'correct_answer': correct_answer, 
