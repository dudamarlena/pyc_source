...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/patches.py
...
def _cachekey_lookupuserbyattr(meth, self, *args, **kwargs):
    sig = (
     meth.__name__, self.__name__, args, kwargs.items())
...
def _lookupuserbyattr(self, *args, **kwargs):
    try:
        return _lookupuserbyattr_cache_wrapper(self._old__lookupuserbyattr, *args, **kwargs)
...
def LDAPDelegate_search(self, *args, **kwargs):
    try:
        return LDAPDelegate_search_cache_wrapper(self._old_search, *args, **kwargs)
    except ValueError as exc:
        return exc.message