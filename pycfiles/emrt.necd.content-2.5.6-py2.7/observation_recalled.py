...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/notifications/observation_recalled.py
...
def factory(role, suffix, for_states):
    ...
    def handler(observation, event):
        if event.action != SKIP_UNLESS_ACTION:
            return
        _wf_state = observation.workflow_history.get('esd-review-workflow')[(-2)]['review_state']
        _obs_state = OBS_STATES[_wf_state]
        if _obs_state not in for_states:
            return
        _mail_subject = MAIL_SUBJECT.format(state=_obs_state)
        _mail_template = PageTemplateFile(MAIL_TEMPLATE.format(state=_obs_state, suffix=suffix))
        notify(observation, _mail_template, _mail_subject, role, NOTIFICATION_NAME)
