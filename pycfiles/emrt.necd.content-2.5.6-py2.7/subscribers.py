...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/subscribers.py
        ...
        with api.env.adopt_roles(roles=['Manager']):
            ...
            if questions:
                ...
                if api.content.get_state(question) == 'draft':
                    api.content.transition(obj=question, transition='close')
                elif api.content.get_state(question) in ('drafted', 'recalled-lr'):
                    api.content.transition(obj=question, transition='close-lr')
    elif event.action == 'recall-from':
        with api.env.adopt_roles(roles=['Manager']):
            questions = [ c for c in observation.values() if c.portal_type == 'Question' ]
            if questions:
                question = questions[0]
                api.content.transition(obj=question, transition='recall')
