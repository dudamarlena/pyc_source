...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/emrt/necd/content/upgrades/portal_workflow.py
# Compiled at: 2019-06-12 10:27:28
import gc
...
def get_workflow_type_mapping(wft):
    ...
    return {k:(v[0] if v else default_wf) for k, v in wft._chains_by_type.items()}
...
def upgrade(wft, catalog, type_mapping, queries):
    ...
    for _query in queries:
        query = {k:v for k, v in _query.items() if k != 'reindex_self_only' if k != 'reindex_self_only'}
