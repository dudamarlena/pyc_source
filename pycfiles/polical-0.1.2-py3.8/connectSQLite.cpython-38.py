...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/polical/connectSQLite.py
# Compiled at: 2020-05-13 11:24:47
...
def addTarTID(TarUID, TarTID, username):
    ...
    idUsuarios = getUserID(username)
    cur.execute('UPDATE TareasUsuarios SET TarUsrTID = ?, TarUsrEstado = ? WHERE idUsuarios = ? AND idTareas = ?;', (TarTID, 'E', idUsuarios, idTareas))
    cur.connection.commit()
    return cur
