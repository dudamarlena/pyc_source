...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/importer.py
...
import os, sys, types, pkgutil, inspect, traceback, importlib
...
class Importer(object):
    ...
    def create_logger(self, debug=False):
        ...
        if not os.path.exists(log_path):
            ...
            if debug_env_var in os.environ and os.environ.get(debug_env_var) in ('True',
                                                                                 'true'):
            ...
            if name not in self.loaded_modules.keys():
                if name in skip_modules:
                    ...
                    if mod and isinstance(mod, types.ModuleType):
                        self.loaded_modules[mod.__name__] = [
                         os.path.dirname(mod.__file__), mod]
...
class SimpleImporter(object):
    ...
    def create_logger(self):
        ...
        if not os.path.exists(log_path):
            ...
            log = log_utils.create_logger(logger_name=(self._module_name), logger_path=log_path)
            logger = log.logger
            if '{}_DEV'.format(self._module_name.upper()) in os.environ and os.environ.get('{}_DEV'.format(self._module_name.upper())) in ('True',
                                                                                                                                           'true'):
                logger.setLevel(log_utils.LoggerLevel.DEBUG)
            else:
                logger.setLevel(log_utils.LoggerLevel.WARNING)
