...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/fileio.py
# Compiled at: 2020-05-02 23:38:35
# Size of source mod 2**32: 29039 bytes
"""
Utility methods related to write/read (IO) text files
"""
from __future__ import print_function, division, absolute_import
import os, sys, stat, json, string, shutil, getpass, datetime, subprocess
...
def remove_extension(file_path):
    ...
    new_name = file_path
    if len(split_path) > 1:
        new_name = string.join(split_path[:-1], '.')
    return new_name
...
def get_file_size(file_path, round_value=2):
    ...
    size_format = round(size * 1e-06, round_value)
    return size_format
...
def get_last_modified_date(file_path):
    ...
    :param file_path: str
    :return: str, formated date and time
    """
...
def write_lines(file_path, lines, append=False):
    ...
    """
    write_file = FileWriter(file_path=file_path)
...
def is_same_text_content(file1, file2):
    ...
    :return: bool
    """
    text1 = get_file_text(file_path=file1)
    text2 = get_file_text(file_path=file2)
    if text1 == text2:
        return True
    else:
        return False
