...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/libs/python/config.py
...
import os, logging, platform, subprocess, configparser
...
class Config(configparser.RawConfigParser, object):
    ...
    def _create(self):
        ...
        """
        LOGGER.info('Initializing {0} Settings, creating configuration file: {1}\n'.format(self._app_name, self.config_file))
        ...
        f.close()
        self.update()
        LOGGER.info('{0} has successfully created a new configuration file at: {1}\n'.format(self._app_name, str(self.config_file)))
