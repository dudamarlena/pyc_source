...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/tukey/tukey-middleware/auth_proxy/auth_system.py
# Compiled at: 2012-11-05 11:57:34
import auth_db, datetime, httplib, json, logging, time
...
class FakeId(object):
    ...
    def _format_tenant(self, tenant_name, id):
        ...
        """
        return {'tenants_links': [], 'tenants': [
                     {'enabled': True, 
                        'description': None, 
                        'name': tenant_name, 
                        'id': id}]}

    def _format_endpoint(self, token_id, tenant_id, user_id, tenant_name, username, expires, url):
        """The Keystone API endpoints format.
        """
        return {'access': {'token': {'expires': self._expiration(), 
                                'id': token_id, 
...
class OpenStackAuth(AuthSystem):
    ...
    def authenticate(self, method, identifier, cloud_name):
        ...
        creds = {'username': username, 
           'password': password}
        wrapped_creds = {'auth': {'passwordCredentials': creds}}
...
class EucalyptusAuth(AuthSystem, FakeId):
    ...
    def fake_token(self):
        return self._format_token(self.username, self.user_id, self.token_id, self.expires)

    def fake_tenant(self):
        return self._format_tenant(self.tenant_name, self.tenant_id)

    def fake_endpoint(self):
        return self._format_endpoint(self.token_id, self.tenant_id, self.user_id, self.tenant_name, self.username, self.expires, self.url)