...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sskk/canossa/canossa/attribute.py
...
_ATTR_BG = 18
_ATTR_NRC = 27
_ATTR_DEFAULT = 256 << _ATTR_FG | 256 << _ATTR_BG
_NRC_REVERSE_MAP = [
 'B', '0', 'A', '4',
 'C', 'R', 'Q', 'K',
 'Y', 'E', '6', 'Z',
...
class Attribute:
    ...
    def draw(self, s, attr=None):
        ...
        charset = value & 7680
        if charset != value_current & 15 << _ATTR_NRC:
            if len(_NRC_REVERSE_MAP) > charset:
                s.write('\x1b(%c' % _NRC_REVERSE_MAP[charset])
        s.write('\x1b[%sm' % (';').join([ str(p) for p in params ]))
    ...
    def set_charset(self, charset):
        ...
        try:
            ...
            self._attrvalue = value & ~(15 << _ATTR_NRC) | code << _ATTR_NRC
        except ValueError:
            return False
