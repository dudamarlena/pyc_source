...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sskk/dictionary.py
# Compiled at: 2014-03-11 11:41:30
import os, inspect, re, settings, logging, time, thread
...
class SkkLineLoader:
    ...
    def _decode_line(self, line):
        try:
            ...
            self._encoding = 1 - self._encoding
...
class Completer:
    ...
    def suggest(self, key, finals):
        ...
        candidate = list()
        if not finals:
            expander.expand('', _current, candidate)
        else:
            for _key in finals:
                key_len = len(_key)
                if key_len == 1:
...
completer = Completer()
_control_chars = re.compile(b'[\x00-\x1f\x7f\x80-\x9f\xff]')
