...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/sskk/canossa/canossa/iframe.py
...
class IMouseListenerImpl(IMouseListener):
    ...
    def onmousedown(self, context, x, y):
        ...
        if hittest == _HITTEST_CLIENTAREA:
            ...
            if screen.mouse_protocol == MOUSE_PROTOCOL_ANY_EVENT or screen.mouse_protocol == MOUSE_PROTOCOL_BUTTON_EVENT or screen.mouse_protocol == MOUSE_PROTOCOL_NORMAL or screen.mouse_protocol == MOUSE_PROTOCOL_X10:
    ...
    def onscrolldown(self, context, x, y):
        ...
        if hittest == _HITTEST_CLIENTAREA:
            screen = self.innerscreen
            if screen.mouse_protocol == MOUSE_PROTOCOL_ANY_EVENT or screen.mouse_protocol == MOUSE_PROTOCOL_BUTTON_EVENT or screen.mouse_protocol == MOUSE_PROTOCOL_NORMAL:
    ...
    def _emit_mouseevent(self, context, encoding, b, x, y):
        if encoding == MOUSE_ENCODING_NORMAL:
            ...
            y += 32
            b += 32
            context.puts('\x1b[M%c%c%c' % (unichr(b), unichr(x + 1), unichr(y + 1)))
...
class InnerFrame(tff.DefaultHandler, IInnerFrame, IMouseListenerImpl, IFocusListenerImpl):

    def __init__(self, session, listener, outerscreen, top, left, row, col, command, termenc, termprop):
        ...
        window.alloc(left - self._padding_left, top - self._padding_top, col + self._padding_left + self._padding_right, row + self._padding_top + self._padding_bottom)
        self._window = window
        ...
        if termprop.wcwidth(9472) == 1:
            self._horizontal_bar = '─'
            self._horizontal_bar2 = '═'
        else:
            self._horizontal_bar = '-'
            self._horizontal_bar2 = '-'
        if termprop.wcwidth(9474) == 1:
            self._vertical_bar = '│'
            self._vertical_bar2 = '║'
        else:
            self._vertical_bar = '|'
            self._vertical_bar2 = '|'
        if termprop.wcwidth(9492) == 1:
            self._bottom_left_corner = '└'
            self._bottom_left_corner2 = '╚'
            self._bottom_left_corner3 = '╘'
            self._bottom_left_corner4 = '╙'
        else:
            self._bottom_left_corner = '+'
            self._bottom_left_corner2 = '+'
            self._bottom_left_corner3 = '+'
            self._bottom_left_corner4 = '+'
        if termprop.wcwidth(9496) == 1:
            self._bottom_right_corner = '┘'
            self._bottom_right_corner2 = '╝'
            self._bottom_right_corner3 = '╛'
            self._bottom_right_corner4 = '╜'
        else:
            self._bottom_right_corner = '+'
            self._bottom_right_corner2 = '+'
            self._bottom_right_corner3 = '+'
            self._bottom_right_corner4 = '+'
        self._tty = session.add_subtty('xterm', 'ja_JP.UTF-8', command, row, col, termenc, self, canossa)
        self._title = command
    ...
    def _drawtitle(self, dirtyregion):
        ...
        if title_length < width - 11:
            ...
            title = '  ' + self._title[0:width - 2 - 9] + '...   [x]'
        else:
            title = ' ' * (width - 3) + '[x]'
            ...
            if top + index < outerscreen.height:
                if top + index >= 0:
                    ...
                    if dirtyrange:
                        ...
                        if dirty_width < 0:
                            continue
                        innerscreen.copyrect(window, dirty_left - left, index, dirty_width, 1, dirty_left, top + index, lazy=True)
    ...
    def checkdirty(self, region):
        ...
        if self._dirty:
            ...
            dirtyregion = region.sub(left - self._padding_left, top - self._padding_top, width + self._padding_left + self._padding_right, height + self._padding_top + self._padding_bottom)
            self._dirty = False
    ...
    def draw(self, region):
        if self.enabled:
            ...
            width = innerscreen.width
            height = innerscreen.height
            dirtyregion = region.add(left - self._padding_left, top - self._padding_top, width + self._padding_left + self._padding_right, height + self._padding_top + self._padding_bottom)
