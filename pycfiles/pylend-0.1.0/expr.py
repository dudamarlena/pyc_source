...
# Embedded file name: build/bdist.linux-x86_64/egg/lems/parser/expr.py
# Compiled at: 2016-03-18 06:01:10
__doc__ = '\nExpression parser\n\n@author: Gautham Ganapathy\n@organization: LEMS (http://neuroml.org/lems/, https://github.com/organizations/LEMS)\n@contact: gautham@lisphacker.org\n'
from lems.base.base import LEMSBase
from lems.base.stack import Stack
known_functions = [
 'exp', 'log', 'sqrt', 'sin', 'cos', 'tan', 'sinh', 'cosh', 'tanh', 'abs', 'ceil', 'factorial', 'random', 'H']
...
class OpNode(ExprNode):
    ...
    def to_python_expr(self):
        return ('({0} {1} {2})').format(self.left.to_python_expr(), self.op, self.right.to_python_expr())
...
class ExprParser(LEMSBase):
    ...
    def tokenize(self):
        ...
        while i < len(ps):
            ...
            if ps[i].isalpha():
                ...
                if ps[i].isdigit():
                    while i < len(ps) and (ps[i].isdigit() or ps[i] == '.' or ps[i] == 'e' or ps[i] == 'E' or ps[i] == '+' and (ps[(i - 1)] == 'e' or ps[(i - 1)] == 'E') or ps[i] == '-' and (ps[(i - 1)] == 'e' or ps[(i - 1)] == 'E')):
                        token += ps[i]
                        i += 1
    ...
    def cleanup_stacks(self):
        ...
        while self.op_stack.top() != '$':
            if self.debug:
                print '5> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
            op = self.op_stack.pop()
            right = self.make_op_node(op, right)
            if self.debug:
                print '6> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
            if self.debug:
                print '7> %s' % right
    ...
    def parse_token_list_rec(self, min_precedence):
        ...
        while self.token_list:
            ...
            if self.debug:
                print '1> Token: %s, next: %s, op stack: %s, val stack: %s, node stack: %s' % (token, la, self.op_stack, self.val_stack, self.node_stack)
            ...
            if token == '(':
                ...
                if self.debug:
                    print 'OP Token: %s (prior: %i), top: %s (prior: %i)' % (token, self.priority(token), stack_top, self.priority(stack_top))
                if self.priority(token) < self.priority(stack_top):
                    ...
                    print '  Priority of %s is greater than %s' % (token, stack_top)
                self.op_stack.push(token)
            else:
                if self.debug:
                ...
                if stack_top == '$':
                    ...
                    self.val_stack.push('$')
                elif self.is_op(la) and self.priority(stack_top) < self.priority(la):
                    if self.debug:
                        print 'option b'
                    self.node_stack.push(ValueNode(token))
                    self.val_stack.push('$')
            ...
            if close_bracket:
                stack_top = self.op_stack.top()
                if self.debug:
                    print '+ Closing bracket, op stack: %s, node stack: %s la: %s' % (self.op_stack, self.node_stack, la)
                if self.debug:
                    print '>>> Tokens left: %s' % self.token_list
                if stack_top == '$':
                    if self.debug:
                    ...
                    if self.is_op(la) and self.priority(stack_top) < self.priority(la):
                        ...
                        if self.debug:
                            print '+ option c, nodes: %s' % self.node_stack
                        if self.debug:
                            print '35> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
                        right = self.node_stack.pop()
                        op = self.op_stack.pop()
                        ...
                        self.val_stack.push('$')
                        if self.debug:
                            print '36> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
            if self.debug:
                print '2> Token: %s, next: %s, op stack: %s, val stack: %s, node stack: %s' % (token, la, self.op_stack, self.val_stack, self.node_stack)
            if self.debug:
                print ''

        if self.debug:
            print '3> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
        ret = self.cleanup_stacks()
        if self.debug:
            print '4> op stack: %s, val stack: %s, node stack: %s' % (self.op_stack, self.val_stack, self.node_stack)
        if self.debug:
