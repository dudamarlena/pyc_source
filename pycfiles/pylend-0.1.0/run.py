...
# Embedded file name: build/bdist.linux-x86_64/egg/lems/run.py
# Compiled at: 2016-11-17 11:20:38
...
def process_args():
    ...
    parser.add_argument('-I', type=str, metavar='<Include directory>', action='append', help='Directory to be searched for included files')
    parser.add_argument('lems_file', type=str, metavar='<LEMS file>', help='LEMS file to be simulated')
    parser.add_argument('-nogui', action='store_true', help="If this is specified, just parse & simulate the model, but don't show any plots")
    parser.add_argument('-dlems', action='store_true', help='If this is specified, export the LEMS file as ' + dlems_info)
...
def plot_recording(recording):
    global fig_count
    import matplotlib.pyplot as pylab, numpy
    data_output = recording.data_output
    ...
    plot, = pylab.plot(x, y, color=recorder.color, label=recorder.quantity)
