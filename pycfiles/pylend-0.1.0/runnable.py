...
# Embedded file name: build/bdist.linux-x86_64/egg/lems/sim/runnable.py
...
class Reflective(LEMSBase):
    ...
    def add_method(self, method_name, parameter_list, statements):
        ...
        if self.debug and statements != []:
            ...
            print code_string
        exec compile(ast.parse(code_string), '<unknown>', 'exec') in g, l
    ...
    def add_instance_variable(self, variable, initial_value):
        ...
        code_string = ('self.{0} = {1}\nself.{0}_shadow = {1}').format(variable, initial_value)
...
class Runnable(Reflective):
        ...
        for ctype in runnable.component.types:
            if ctype in self.attachments:
                ...
                if name not in self.__dict__:
                    raise SimBuildError(('Cannot attach {0} to {1} in {2}').format(runnable.id, name, self.id))
    ...
    def register_event_out_callback(self, port, callback):
        if self.debug:
            print 'register_event_out_callback on %s, port: %s, callback: %s' % (self.id, port, callback)
        if port in self.event_out_callbacks:
            self.event_out_callbacks[port].append(callback)
        else:
            raise SimBuildError(("No event out port '{0}' in component '{1}'").format(port, self.id))
        if self.debug:
    ...
    def resolve_path(self, path):
        ...
        if path == '':
            ...
            if child in self.children:
                ...
                if child in self.component.parameters:
                    ctx = self.component
                    p = ctx.parameters[child]
                    return self.resolve_path('../' + p.value)
                if child in self.__dict__.keys():
                ...
                raise SimBuildError(("Unable to find child '{0}' in '{1}'").format(child, self.id))
            if new_path == '':
                return childobj
    ...
    def add_variable_recorder2(self, data_output, recorder, path, full_path):
        if path[0] == '/':
            ...
            if child in self.children:
                childobj = self.children[child]
                if idx == -1:
                    childobj.add_variable_recorder2(data_output, recorder, new_path, full_path)
                else:
                    childobj.array[idx].add_variable_recorder2(data_output, recorder, new_path, full_path)
                ...
                if childobj:
                    ...
                    raise SimBuildError(("Unable to find the child '{0}' in '{1}'").format(child, self.id))
            else:
                raise SimBuildError(("Unable to find a child '{0}' in '{1}'").format(child, self.id))
        else:
    ...
    def single_step(self, dt):
        try:
            ...
            if isinstance(e, ArithmeticError):
                print 'This is an arithmetic error. Consider reducing the integration time step.'
            sys.exit(0)
    ...
    def single_step2(self, dt):
        ...
        if False:
            print (
             '2', self.parent.parent.parent.parent.uid, self.parent.parent.parent.parent.v)
