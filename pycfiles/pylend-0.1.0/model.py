...
# Embedded file name: build/bdist.linux-x86_64/egg/lems/model/model.py
# Compiled at: 2017-12-08 05:06:50
...
import xml.dom.minidom as minidom, logging
...
class Model(LEMSBase):
    ...
    def export_to_dom(self):
        ...
        """
        namespaces = 'xmlns="http://www.neuroml.org/lems/%s" ' + 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ' + 'xsi:schemaLocation="http://www.neuroml.org/lems/%s %s"'
    ...
    def resolve_component_type(self, component_type):
        ...
        if component_type.extends:
            try:
                ...
                raise ModelError("Component type '{0}' trying to extend unknown component type '{1}'", component_type.name, component_type.extends)
    ...
    def fatten_component(self, c):
        ...
        try:
            ...
            raise ModelError("Unable to resolve type '{0}' for component '{1}'; existing: {2}", c.type, c.id, self.component_types.keys())
        ...
        for parameter in ct.parameters:
            ...
            if parameter.name in c.parameters:
                ...
                raise ModelError("Parameter '{0}' not initialized for component '{1}'", parameter.name, c.id)
        ...
        for link in ct.links:
            if link.name in c.parameters:
                ...
                raise ModelError("Link parameter '{0}' not initialized for component '{1}'", link.name, c.id)
        ...
        for path in ct.paths:
            if path.name in c.parameters:
                ...
                raise ModelError("Path parameter '{0}' not initialized for component '{1}'", path.name, c.id)
        ...
        for cref in ct.component_references:
            ...
            if cref.name in c.parameters:
                ...
                raise ModelError("Component reference '{0}' not initialized for component '{1}'", cref.name, c.id)
                ...
                if parent_comp == None:
                    if child.id == fc.id and comp.id == fc.parent_id:
                        ...
                        for child2 in child.children:
                            if self.debug:
                                print '    - Checking child: %s, %s' % (child.id, child2.id)
    ...
    def resolve_structure(self, fc, ct):
        ...
        for w in ct.structure.withs:
            try:
                ...
                raise ModelError("Unable to resolve With parameters for '{0}' in component '{1}'", w.as_, fc.id)
        ...
        for ev in ct.structure.event_connections:
            try:
                ...
                if self.debug:
                    print 'EC..: ' + from_inst + ' to ' + to_inst + ' in ' + str(fc.paths)
                if len(fc.texts) > 0 or len(fc.paths) > 0:
                    ...
                    if '../' in ev.receiver:
                        ...
                        if self.debug:
                            print 'Finding %s in the parent of: %s (%i)' % (parent_attr, fc, id(fc))
                        for comp in self.components.values():
                            if self.debug:
                                print ' - Checking %s (%i)' % (comp.id, id(comp))
                            for child in comp.children:
                                if self.debug:
                                    print '    - Checking %s (%i)' % (child.id, id(child))
                                for child2 in child.children:
                                    if child2.id == fc.id and child2.type == fc.type and child.id == fc.parent_id:
                                        if self.debug:
                                            print '    - Got it?: %s (%i), child: %s' % (child.id, id(child), child2)
                ...
                ev2 = EventConnection(from_inst, to_inst, source_port, target_port, receiver, receiver_container)
                if self.debug:
        ...
        for ch in ct.structure.child_instances:
            try:
                ...
                if self.debug:
                    print e
                raise ModelError("Unable to resolve child instance parameters for '{0}' in component '{1}'", ch.component, fc.id)
        ...
        for mi in ct.structure.multi_instantiates:
            try:
                if mi.component:
                    mi2 = MultiInstantiate(component=fc.component_references[mi.component].referenced_component, number=int(fc.parameters[mi.number].numeric_value))
                else:
                    mi2 = MultiInstantiate(component_type=fc.component_references[mi.component_type].referenced_component, number=int(fc.parameters[mi.number].numeric_value))
            except:
                raise ModelError("Unable to resolve multi-instantiate parameters for '{0}' in component '{1}'", mi.component, fc)
    ...
    def resolve_simulation(self, fc, ct):
        ...
        for run in ct.simulation.runs:
            try:
                run2 = Run(fc.component_references[run.component].referenced_component, run.variable, fc.parameters[run.increment].numeric_value, fc.parameters[run.total].numeric_value)
            except:
                raise ModelError("Unable to resolve simulation run parameters in component '{0}'", fc.id)

            fc.simulation.add(run2)

        for record in ct.simulation.records:
            try:
                record2 = Record(fc.paths[record.quantity].value, fc.parameters[record.scale].numeric_value if record.scale else 1, fc.texts[record.color].value if record.color else '#000000')
            except:
                raise ModelError("Unable to resolve simulation record parameters in component '{0}'", fc.id)

            fc.simulation.add(record2)

        for event_record in ct.simulation.event_records:
            try:
                event_record2 = EventRecord(fc.paths[event_record.quantity].value, fc.texts[event_record.eventPort].value)
            except:
                raise ModelError("Unable to resolve simulation event_record parameters in component '{0}'", fc.id)
        ...
        for dd in ct.simulation.data_displays:
            try:
                ...
                raise ModelError("Unable to resolve simulation display parameters in component '{0}'", fc.id)
        ...
        for dw in ct.simulation.data_writers:
            try:
                ...
                raise ModelError("Unable to resolve simulation writer parameters in component '{0}'", fc.id)
        ...
        for ew in ct.simulation.event_writers:
            try:
                ...
                ew2 = EventWriter(path, fc.texts[ew.file_name].value, fc.texts[ew.format].value)
            except:
                raise ModelError("Unable to resolve simulation writer parameters in component '{0}'", fc.id)
    ...
    def get_numeric_value(self, value_str, dimension=None):
        ...
        elif sym in self.units:
            unit = self.units[sym]
            if dimension:
                if dimension != unit.dimension and dimension != '*':
                    raise SimBuildError("Unit symbol '{0}' cannot be used for dimension '{1}'", sym, dimension)
            else:
                dimension = unit.dimension
            numeric_value = number * 10 ** unit.power * unit.scale + unit.offset
        else:
            raise SimBuildError("Unknown unit symbol '{0}'. Known: {1}", sym, self.units)
