...
# [GCC 8.4.0]
# Embedded file name: ../orthnet/utils/poly.py
# Compiled at: 2018-03-11 14:55:07
...
class Poly1d:
    ...
    def __init__(self, module, degree, x, initial, recurrence):
        ...
        if not self.module in ('tensorflow', 'pytorch'):
            raise AssertionError("Module should be either 'tensorflow' or 'pytorch'.")
        else:
        ...
        if self.module == 'tensorflow':
            if not isinstance(x, tf.Variable):
                assert isinstance(x, tf.Tensor), 'x should be an isinstance of tensorflow.Variable or tensorflow.Tensor.'
        else:
            if not isinstance(x, torch.autograd.Variable):
                assert isinstance(x, torch.Tensor), 'x should be an isinstance of torch.autograd.Variable or torch.Tensor.'
        self.degree = degree
        self.x = x
...
class Poly:
    ...
    def __init__(self, module, degree, x, initial, recurrence):
        ...
        if not self.module in ('tensorflow', 'pytorch'):
            raise AssertionError("Module should be either 'tensorflow' or 'pytorch'.")
        elif not (degree >= 0 and isinstance(degree, int)):
            raise AssertionError('Degree should be a non-negative integer.')
        if not len(initial) == 2:
            raise AssertionError('Need two initial functions.')
        else:
            if self.module == 'tensorflow':
                if not isinstance(x, tf.Variable):
                    assert isinstance(x, tf.Tensor), 'x should be an isinstance of tensorflow.Variable or tensorflow.Tensor.'
