...
# [GCC 8.4.0]
# Embedded file name: /home/cabox/envs/tailchaser/lib/python2.7/site-packages/tailchaser/pipes.py
# Compiled at: 2016-08-22 13:35:29
import argparse, getpass, logging, os, platform, sys
try:
    import regex
except ImportError:
    import re as regex
...
class System(object):
    ...
    def args(self):
        return (
         Args('--config-endpoint', default=self.CONFIG_ENDPOINT, help='overrides default hostname for central, %s. Add port if needed like this: some_host:8000' % self.CONFIG_ENDPOINT),
         Args('--logging', choices=['DEBUG', 'INFO', 'WARN', 'ERROR', 'CRITICAL'], default='ERROR', help='logging level, default: ERROR'),
    ...
    def configure(self, *nodes, **kwargs):
        ...
        parser = argparse.ArgumentParser(description=doc % __version__, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    def config_params(self, args):
        return dict(pid=self.pid, where=self.host, who=self.user)
...
class CollectRecords(Node):
    ...
    def configure(self, config):
        config['start_of_record_re'] = regex.compile(config['record_seperator_regex']) if config['record_seperator_regex'] else None
        return config

    @classmethod
    def args(cls):
        return (
         Args('--start-of-record-re', default=None, help='use this regex expresion to define the start of a record, default: None'),)
