...
# [GCC 8.4.0]
# Embedded file name: /home/cabox/envs/tailchaser/lib/python2.7/site-packages/tailchaser/tailer.py
# Compiled at: 2016-08-22 11:53:48
"""Module that contains the the chaser (producer) classes.

.. moduleauthor:: Thanos Vassilakis <thanosv@gmail.com>

"""
import argparse, binascii, glob, gzip
try:
    import bz2
except ImportError:
    bz2 = None

import logging.handlers, os, platform, pickle, regex, shutil, sys, tempfile, time, collections, six
...
class Tailer(object):
    ...
    RUNNING = 'RUNNING'
    WAITING = 'WAITING'
    ARGS = ('only_backfill', 'dont_backfill', 'read_period', 'clear_checkpoint', 'read_pause',
            'temp_dir', 'start_of_record_re', 'filter_re')

    def __init__(self, only_backfill=ONLY_BACKFILL, dont_backfill=DONT_BACKFILL, read_period=READ_PERIOD, clear_checkpoint=CLEAR_CHECKPOINT, read_pause=READ_PAUSE, temp_dir=TMP_DIR, start_of_record_re=None, filter_re=None, windows=None):
            ...
            if stat.st_mtime > checkpoint[1]:
                ...
                if sig != checkpoint[0]:
                    ...
                    if checkpoint[2] == stat.st_size:
                        continue
                    checkpoint = (
                     sig, stat.st_mtime, checkpoint[2])
                self.state = 'RUNNING'
                return (
                 pos + 1 != len(with_stats), (file_name, checkpoint))
            if stat.st_mtime == checkpoint[1]:
                ...
                if sig == checkpoint[0]:
                    if stat.st_size == checkpoint[2]:
                        if self.state == self.RUNNING:
                            ...
                            return
                checkpoint = (
                 checkpoint[0], stat.st_mtime, checkpoint[2])
                return (
                 pos + 1 != len(with_stats), (file_name, checkpoint))
    ...
    def backfill(self, file_info):
        ...
        for offset, record in self.process(copied_file_path, sig, st_mtime, offset):
            if not record:
                break
            yield (
             copied_file_path, (sig, st_mtime, offset), record)
    ...
    def tail(self, file_info):
        ...
        while True:
            ...
            if record:
                yield (
                 self.make_sig(file_to_process), (sig, st_mtime, offset), record)
    ...
    def sig(cls, file_to_check_fh, stat=None):
        return str(binascii.crc32(file_to_check_fh.read(SIG_SZ)) & 4294967295)
    ...
    def build_arg_parser(cls, parser=None):
        ...
        parser.add_argument('--windows', action='store_true', default=Tailer.is_windows(), help='run as if the platform is windows, default: %s' % Tailer.is_windows())
        parser.add_argument('file-pattern', help='file pattern to tail, such as /var/log/access.*')
        parser.add_argument('--only-backfill', action='store_true', default=Tailer.ONLY_BACKFILL, help="don't tail, default: %s" % Tailer.ONLY_BACKFILL)
        parser.add_argument('--dont-backfill', action='store_true', default=Tailer.DONT_BACKFILL, help='basically only tail, default: %s' % Tailer.DONT_BACKFILL)
        parser.add_argument('--clear-checkpoint', action='store_true', default=Tailer.CLEAR_CHECKPOINT, help='start form the beginning, default: %s' % Tailer.CLEAR_CHECKPOINT)
        parser.add_argument('--read-period', type=int, default=Tailer.READ_PERIOD, help='how long you read before you pause.' + "If zero you don't pause, default: %s" % Tailer.READ_PERIOD)
        parser.add_argument('--read-pause', type=int, default=Tailer.READ_PAUSE, help='how long you pause between reads, default: %s' % Tailer.READ_PAUSE)
        parser.add_argument('--reading-from', choices=['unix', 'win'], default='win', help='sets how long you rad and then pause, default: win')
        parser.add_argument('--temp-dir', default=Tailer.TMP_DIR, help='on backfil files are copied to a temp directory.' + 'Use this to set this directory, default: %s' % Tailer.TMP_DIR)
        parser.add_argument('--logging', choices=['DEBUG', 'INFO', 'WARN', 'ERROR', 'CRITICAL'], default='ERROR', help='logging level, default: ERROR')
        parser.add_argument('--start-of-record-re', default=None, help='use this regex expresion to define the start of a record, default: None')
        parser.add_argument('--filter-re', default=None, help='use this regex as a positive filter of records, default: None')
        parser.add_argument('--show-config', action='store_true', default=False, help='dump-configuration')
        return parser
