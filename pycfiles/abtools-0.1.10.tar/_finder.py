...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanbriney/git/abtools/abtools/_finder.py
# Compiled at: 2016-07-28 04:03:25
from __future__ import print_function
import multiprocessing as mp, platform, os, subprocess as sp, sys, tempfile
...
def parse_args():
    ...
    parser.add_argument('--collection-prefix', dest='collection_prefix', default=None, help='If supplied, will iteratively process only collections beginning with <collection_prefix>.')
    parser.add_argument('-o', '--output', dest='output_dir', default=None, help='Output directory figure files. If not provided, figures will not be generated.                         Directory will be created if it does not already exist.')
    parser.add_argument('-t', '--temp', dest='temp_dir', required=True, help='Directory for temporary storage.                         Will be created if it does not already exist. Required.')
    parser.add_argument('-l', '--log', dest='log', default=None, help='The log file, to which the blast_parse log info will be written.                         Default is <output>/abfinder.log.')
    parser.add_argument('-C', '--cluster', dest='cluster', default=False, action='store_true', help='Use if performing computation on a Celery cluster.                         If set, input files will be split into many subfiles and passed to a Celery queue.                         If not set, input files will still be split,                         but will be distributed to local processors using multiprocessing.')
    parser.add_argument('-i', '--ip', dest='ip', default='localhost', help="The IP address for the MongoDB server.                          Defaults to 'localhost'.")
    parser.add_argument('--port', dest='port', default=27017, help="The port for the MongoDB server. Defaults to '27017'.")
    ...
    parser.add_argument('-q', '--chain', dest='chain', default='heavy', choices=[
     'heavy', 'kappa', 'lambda', 'light'], help="The chain type of the subject sequence.                         Options are 'heavy', 'kappa', 'lambda' and 'light'.                         Default is 'heavy'.")
    parser.add_argument('-n', '--no_update', dest='update', action='store_false', default=True, help="Does not update the MongoDB with AbFinder info.                         Can save some time if the identity calculations aren't needed again.")
    parser.add_argument('--no_figure', dest='make_figure', action='store_false', default=True, help="Does not make the identity/divergence figure.                         Useful if you don't want the figure, just the identity info written to the database.")
    parser.add_argument('--single-process-update', dest='single_process_update', action='store_true', default=False, help='Perform the MongoDB update using a single process (without multiprocessing).')
    parser.add_argument('--update-threads', dest='update_threads', type=int, default=25, help='Number of threads to use when update the MongoDB database. Default is 25.')
    parser.add_argument('-N', '--nucleotide', dest='is_aa', action='store_false', default=True, help='Use nucleotide sequences for alignment. Default is amino acid sequences.                         Ensure standard format matches.')
    parser.add_argument('-x', '--xmin', dest='x_min', type=int, default=-1, help='Minimum X-axis (germline divergence) value for the AbCompare plot. Default is -1.')
    parser.add_argument('-X', '--xmax', dest='x_max', type=int, default=35, help='Maximum X-axis (germline divergence) value for the AbCompare plot. Default is 35.')
    parser.add_argument('-y', '--ymin', dest='y_min', type=int, default=65, help='Minimum Y-axis (mAb identity) value for the AbCompare plot. Default is 65.')
    parser.add_argument('-Y', '--ymax', dest='y_max', type=int, default=101, help='Maximum Y-axis (mAb identity) value for the AbCompare plot. Default is 101.')
    parser.add_argument('-g', '--gridsize', dest='gridsize', type=int, default=0, help='Gridsize for the AbFinder hexbin plot.                         Default is 36 for amino acid sequences and 50 for nucleotide sequences.')
    parser.add_argument('--colormap', dest='colormap', default='Blues', help="Colormap to be used in the AbFinder hexbin plots.                         Can accept a matplotlib cmap or the name of one of matplotlib's builtin cmaps.                         Default is 'Blues'.")
    parser.add_argument('--mincount', dest='mincount', default=3, type=int, help='Minimum number of sequences in a hexbin for that hexbin to be colored.                         Default is 3.')
    parser.add_argument('--skip-padding', dest='remove_padding', default=True, action='store_false', help='If set, will not remove padding field from MongoDB.')
    parser.add_argument('-D', '--debug', dest='debug', action='store_true', default=False, help='If set, will write all failed/exception sequences to file                         and should give more informative errors.')
    return parser


class Args(object):

    def __init__(self, db=None, collection=None, output=None, temp=None, log=None, cluster=False, ip='localhost', port=27017, user=None, password=None, update=True, standard=None, chain='heavy', is_aa=True, x_min=-1, x_max=35, y_min=65, y_max=101, gridsize=0, mincount=3, colormap='Blues', debug=False):
        super(Args, self).__init__()
        if not all([db, output, temp, standard]):
            err = 'You must provide a MongoDB database name, output and temp directories,                 and a file containing one or more comparison (standard) sequences in FASTA format.'
        ...
        if chain not in ('heavy', 'kappa', 'lambda', 'light'):
            err = 'Please select an appropriate chain.                 Valid choices are: heavy, light, kappa and lambda.'
            raise RuntimeError(err)
        self.chain = chain
...
def update_db(db, standard, scores, collection, args):
    ...
    print_update_info()
    start = time.time()
    conn = mongodb.get_connection(args.ip, args.port, args.user, args.password)
    ...
    logger.info(('Updating took {} seconds. ({} sequences per second)').format(round(run_time, 2), round(len(scores) / run_time, 1)))
...
def update(db, collection, data, standard, version, args):
    ...
    if int(version.split('.')[0]) < 3:
        result = coll.update({'seq_id': {'$in': ids}}, {'$set': {('{}.{}').format(mab_id_field, standard.lower()): float(score)}}, multi=True)
    else:
        result = coll.update_many({'seq_id': {'$in': ids}}, {'$set': {('{}.{}').format(mab_id_field, standard.lower()): float(score)}})
        if args.debug:
...
def _run_jobs_via_multiprocessing(files, standard, args):
    ...
    divergences = pd.Series([ 100.0 - r[2] for r in results ], index=ids)
