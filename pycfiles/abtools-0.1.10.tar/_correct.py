...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanbriney/git/abtools/abtools/_correct.py
# Compiled at: 2017-07-12 21:06:01
from __future__ import print_function
from collections import Counter
import json, math, multiprocessing as mp, os, shelve, sqlite3
from StringIO import StringIO
import subprocess as sp, sys, tempfile, time, urllib, uuid, numpy as np
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser('Clusters sequences using either an identity threshold or unique                                       antibody identifiers (UAIDs).                                       Calculates either the centroid or consensus sequence for each                                       identity/UAID cluster passing a cluster size threshold.')
    parser.add_argument('-d', '--database', dest='db', default=None, help='Name of the MongoDB database to query. Required')
    ...
    parser.add_argument('-j', '--json', dest='json', default=None, help='Input JSON file or directory of JSON files.')
    parser.add_argument('-M', '--minimal-input', dest='minimal_input', default=None, help='Input MINIMAL file or directory of MINIMAL files.')
    parser.add_argument('-o', '--output', dest='output', required=True, help='Output directory for the FASTA files. Required')
    parser.add_argument('-l', '--log', dest='log', help='Location for the log file.                         If not provided, log will be written to <output>/abcorrect.log')
    parser.add_argument('-t', '--temp', dest='temp_dir', required=True, help='Directory for temporary files, including the SQLite database. Required')
    parser.add_argument('-i', '--ip', dest='ip', default='localhost', help="IP address for the MongoDB server.  Defaults to 'localhost'.")
    parser.add_argument('--port', dest='port', default=27017, type=int, help="IP address for the MongoDB server.  Defaults to 'localhost'.")
    parser.add_argument('-u', '--user', dest='user', default=None, help='Username for the MongoDB server. Not used if not provided.')
    parser.add_argument('-p', '--password', dest='password', default=None, help='Password for the MongoDB server. Not used if not provided.')
    parser.add_argument('-m', '--min', dest='min_seqs', default=1, type=int, help='Minimum number of sequences for finding centroids from a UAID group.                         Defaults to 1.')
    parser.add_argument('-U', '--no-uaid', dest='uaid', action='store_false', default=True, help='Clusters sequences by identity rather than using universal antibody IDs (UAIDs).')
    parser.add_argument('-P', '--parse-uids', dest='parse_uaids', action='append', default=[], help="Parameters for UID parsing.                         If UIDs are present in multiple locations (for example, short UIDs at both the start                         and end of a sequencing read), multiple parameters may be supplied by providing --parse-uids                         multiple times at runtime.                         Supplying a single positive integer will result in that number of nucleotides being parsed                         from the start (5' end) of the raw sequence. A single negative integer will result in that number of nucleotides                         being parsed from the end (3' end) of the raw sequence.                         for locations other than the start/end of the sequence, two positions may be supplied (0-based indexing)                         separated by a comma (no whitespace) that indicate the start and end of the index position.                         If the '--no-uaid' flag is also used, this option will be ignored.                         For a UAD of length 20 at the start of the sequence, option should be passed as '--parse-uids 20'.                         For a UID of length 20 at the end of the sequence, option should be passed as '--parse-uids -20'.                         For a UID of length 20 that starts at the 5th position in the sequence, option should be passed as                         '--parse-uids 4,24'.                         For two UIDs, both length 20, one that starts at the 5th position and one at the 3' end of the sequence,                         options should be passed as '--parse-uids 4,24 --parse-uids -20'.                         Default is to not parse UAIDs.")
    parser.add_argument('-C', '--centroid', dest='consensus', action='store_false', default=True, help='Generates consensus sequences for each UAID or homology cluster.                         Default is to identify cluster centroids.')
    parser.add_argument('-I', '--identity-threshold', dest='identity_threshold', default=0.975, type=float, help='If performing identity-based clustering, this threshold is used for the clustering.                         If performing UID-based correction, this threshold is used when clustering sequences assigned                         to the same UID bin. Default is 0.975.')
    parser.add_argument('--only-largest-cluster', default=False, action='store_true', help='If set while calculating centroids using UAIDs,                         only the largest centroid for each UAID cluster is retained.')
    parser.add_argument('--non-redundant', dest='non_redundant', action='store_true', default=False, help='If set, will make a non-redundant FASTA file using Unix sort.                         This is much faster than CD-HIT, but can only remove exact duplicates.                         Note that this is not exactly equivalent to clustering with a threshold of 1.0,                         because sequences of different length that align perfectly for the entirety                         of the shorter sequence will be collapsed when using CD-HIT but will not be                         collapsed by Unix sort.')
    parser.add_argument('-g', '--germs', dest='germs', default=None, help='Path to a FASTA-formatted file of germline V gene sequences.                         Required if building consensus sequences, not required for centroids.')
    parser.add_argument('--aa', dest='aa', action='store_true', default=False, help='If set, use amino acid sequences for identity-based clustering.')
    parser.add_argument('--clustering_field', dest='clustering_field', default='vdj_nt', help="The MongoDB field to be used for clustering.                         Default is 'vdj_nt'.")
    parser.add_argument('--output_field', dest='output_field', default='oriented_input', help="The MongoDB field to be used for creating consensus/centroid sequences.                         Default is 'oriented_input'.")
    parser.add_argument('--cdhit-cluster-uids', dest='unix_sort_uids', action='store_false', default=True, help='If set, uses Unix sort rather than CD-HIT to cluster sequences by UID.')
    parser.add_argument('--no-cluster-sizes', dest='include_cluster_size', action='store_false', default=True, help='If set, the sequence name of the consensus/centroid will not include the cluster size.                         Default is to include the cluster size in the consensus/centroid name.')
    parser.add_argument('-k', '--chunksize', dest='chunksize', default=50, type=int, help='Approximate number of sequences in each consensus/centroid calculation job.                         Defaults to 50.')
    parser.add_argument('--cluster', dest='cluster', default=False, action='store_true', help='Use if performing computation on a Celery cluster.                         If set, input files will be split into many subfiles and passed                         to a Celery queue. If not set, input files will still be split, but                         will be distributed to local processors using multiprocessing.')
    parser.add_argument('-D', '--debug', dest='debug', action='store_true', default=False, help='If set, will run in debug mode.')
    parser.add_argument('-s', '--sleep', dest='sleep', type=int, default=0, help='Delay, in minutes, until the script starts executing. Default is 0.')
    return parser


class Args(object):

    def __init__(self, db=None, collection=None, json=None, minimal_input=None, output=None, log=None, temp_dir=None, include_cluster_size=True, ip='localhost', port=27017, user=None, password=None, min_seqs=1, identity_threshold=0.975, chunksize=50, cluster=False, uaid=True, parse_uaids=None, non_redundant=False, unix_sort_uids=False, consensus=True, only_largest_cluster=False, germs=None, aa=False, clustering_field='vdj_nt', output_field='oriented_input', debug=False, sleep=0):
...
def query(db, collection, args):
    ...
    if args.minimal_input:
                ...
                if i == 0:
                    ...
                    try:
                        ...
                        if args.parse_uaids:
                            ...
                            uid = l[uid_index]
                        d = {'seq_id': l[seq_id_index], 'uaid': uid, args.clustering_field: l[clustering_index], 
                           args.output_field: l[output_index], 
                           'v_gene': {'full': l[v_gene_index]}, 'raw_query': l[raw_index]}
                        results.append(d)
                    except IndexError:
                        continue
        ...
        for r in _results:
            ...
            try:
                d = {'seq_id': r['seq_id'], args.clustering_field: r[args.clustering_field], args.output_field: r[args.output_field], 
...
def make_unix_uid_sort_input(seq_db, args):
    ...
    with open(sort_input.name, 'w') as (f):
        f.write(('\n').join(seq_strings))
    return (
     sort_input.name, len(seq_strings))
...
def process_unix_sorted_uid_file(sorted_file, seq_db, args):
    ...
    logger.info(('The average cluster contains {} sequences; the largest contains {} sequences.').format(round(1.0 * sum(sizes) / len(sizes), 2), max(sizes)))
...
def parse_clusters(cluster_handle, seq_db, args):
    ...
    logger.info(('The average cluster contains {} sequences; the largest contains {} sequences.').format(round(1.0 * sum(lengths) / len(lengths), 2), max(lengths)))
    logger.info(('Cluster parsing and sequence retrieval took {} seconds\n').format(round(time.time() - start, 2)))
...
def write_output(collection, fastas, sizes, collection_start_time, args):
    ...
    logger.info(('{} was processed in {} seconds.').format(collection, round(time.time() - collection_start_time, 2)))
    logger.info('')
    return
...
def countdown(args):
    ...
    while not done:
        ...
        if elapsed < args.sleep * 60:
            h = int((args.sleep - elapsed / 60.0) / 60)
            m = int((args.sleep - elapsed / 60.0) % 60)
