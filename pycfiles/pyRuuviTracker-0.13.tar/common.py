...
# Embedded file name: build/bdist.linux-x86_64/egg/pyrutracker/common.py
# Compiled at: 2013-11-26 11:34:30
from collections import namedtuple, OrderedDict
import urllib, urlparse, re, string
from datetime import datetime
import bs4
RUTRACKER_URL = 'http://rutracker.org/forum/index.php'
PAGE_SIZE = 50
PATTERN = '^([^:?!()]+)[\\s]*:([^\\n]+)'
MONTHES = {'Янв': '01', 'Фев': '02', 'Мар': '03', 'Апр': '04', 'Май': '05', 
...
def pages(doc):
    ...
    link = urlparse.urljoin(RUTRACKER_URL, re.sub('&start=\\d+', '', url))
...
def subforums(doc):
    ...
    for el in doc.select('h4.forumlink'):
        ...
        try:
            date_descr = clear_text(el.find_parent('tr').find_all('td')[(-1)].p.text)
            date = date_descr[:15]
        ...
        yield Subforum(title=clear_text(el.text), date=date, url=link, headers=headers)
...
def topics(doc):
    ...
    for el in doc.select('table tr.hl-tr a.topictitle'):
        ...
        yield Topic(title=clear_text(el.text), size=size, date=date, get_description=fn, url=link, headers=headers)
...
def _topic_text(doc):
    ...
    for el in topic.select('.post-br') + topic.select('.post-hr') + topic.find_all('br'):
        el.string = '\n\n'
