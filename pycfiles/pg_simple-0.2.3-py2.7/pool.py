...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/pg_simple/pool.py
# Compiled at: 2014-08-20 02:09:23
__author__ = 'Erick Almeida and Masroor Ehsan'
import urlparse, os, gc, threading, time, psycopg2, psycopg2.extensions as _ext
...
def config_pool(max_conn=5, expiration=60, disable_pooling=False, pool_manager=SimpleConnectionPool, **kwargs):
    ...
    if not dsn:
        if db_url:
            ...
            config = {'database': params.path[1:], 'user': params.username, 
    ...
    if dsn:
        __pool_manager__ = pool_manager(expiration=expiration, max_conn=max_conn, disable_pooling=disable_pooling, dsn=dsn, debug=debug)
    else:
        __pool_manager__ = pool_manager(expiration=expiration, max_conn=max_conn, disable_pooling=disable_pooling, database=config['database'], host=config.get('host'), port=config.get('port'), user=config.get('user'), password=config.get('password'), debug=debug)
    return
