...
class Crypt(object):
    ...
    def jwt_encode(self, data, expires_in=1, **kw):
        ...
        """
        expires_in *= 60
        s = (itsdangerous.TimedJSONWebSignatureSerializer)(secret_key=self.secret_key, expires_in=expires_in, 
         salt=self.salt, **kw)
        return s.dumps(data)
    ...
    def url_safe_encode(self, data, expires_in=None, **kw):
        ...
        if expires_in:
            expires_in *= 60
            s = URLSafeTimedSerializer2(secret_key=self.secret_key, expires_in=expires_in, 
             salt=self.salt, **kw)
        else:
