...
# Compiled at: 2019-09-08 18:53:42
"""
core.py
...
def get_interesting_members_http_error(base_class, cls):
    ...
    predicate = inspect.ismethod if six.PY2 else inspect.isfunction
    all_members = inspect.getmembers(cls, predicate=predicate)
    return (member for member in all_members if member[0] not in base_members and (hasattr(member[1], '__self__') and member[1].__self__ not in inspect.getmro(cls) if six.PY2 else True) and member[0].startswith('_') and match_http_error(member[0]) and not member[0].startswith('before_') and not member[0].startswith('after_'))
