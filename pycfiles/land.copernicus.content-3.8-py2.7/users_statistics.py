...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/land/copernicus/content/browser/users_statistics.py
# Compiled at: 2018-02-01 12:07:45
...
import plone.api as api, transaction, xlwt
USERS_STATISTICS_KEY = 'land.copernicus.content.users_statistics'
SHEET_TITLE = 'Users data'
WIDTH_UNIT = 340
SHEET_COLUMNS = {'user_id': [
             0, 'Username', 15 * WIDTH_UNIT], 
   'user_full_name': [
                    1, 'Fullname', 25 * WIDTH_UNIT], 
   'user_email': [
                2, 'Contact email', 35 * WIDTH_UNIT], 
   'user_memberships': [
                      3, 'Group memberships', 50 * WIDTH_UNIT], 
   'institutional_domain': [
                          4, 'Institutional Domain', 100 * WIDTH_UNIT], 
...
def get_user_data_list(context):
    ...
    for member in membership_tool.listMembers():
        ...
        institutional_domain = member.getProperty('institutional_domain')
        professional_thematic_domain = member.getProperty('thematic_domain')
        groups = api.group.get_groups(user=member)
        user_memberships = ('; ').join([ group.id for group in groups ])
        yield dict(user_id=user_id, user_full_name=user_full_name, user_email=user_email, user_memberships=user_memberships, institutional_domain=institutional_domain, professional_thematic_domain=professional_thematic_domain)
        ...
        for row_index, data_dict in enumerate(user_data_records):
            for data_key in data_dict:
                ...
                if isinstance(value, DateTime):
                    ...
                    xls_sheet.write(row_index + 1, column_index, value.decode('utf-8'))
                else:
...
def period_title(period):
    ...
    """
    return ('{0}-{1}').format(period[0].strftime('%Y/%m/%d'), period[1].strftime('%Y/%m/%d'))
...
def save_users_statistics_reports(site, time_periods, reports):
    ...
    """
    stats_annot = IAnnotations(site).setdefault(USERS_STATISTICS_KEY, PersistentDict({}))
            ...
            if user_member_data is not None:
                ...
                if active_last is not None and active_from is not None:
                    ...
                    for j in range(0, len(time_periods)):
                        ...
                        if active_last >= start_date and active_from <= end_date and was_active:
                            res[j]['active'] += 1
...
def solve_pending_reports(site):
    ...
    reports = generate_users_statistics(site, time_periods=pending_time_periods)
    return save_users_statistics_reports(site, time_periods=pending_time_periods, reports=reports)
