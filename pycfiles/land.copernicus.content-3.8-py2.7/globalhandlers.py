...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/land/copernicus/content/globalhandlers.py
...
def userBeforeDeleted(user_id, event):
    ...
    subject = 'Copernicus Land Monitoring Service - deleted account'
    mail_text = ('\nHi!\nYour land.copernicus.eu account ({0}) was deleted.\n\nBest regards,\nCopernicus Team at the European Environment Agency').format(user_id)
    try:
        mail_host = api.portal.get_tool(name='MailHost')
        return mail_host.simple_send(mto=email, mfrom=mfrom, subject=subject, body=mail_text, immediate=True)
...
def forceUpdateProfile(principal, event):
    ...
    if site is not None:
        ...
        if t_d == '' or i_d == '':
            ...
            messages.add('Please complete your profile adding your Professional ' + 'thematic domain and Institutional domain.', type='info')
...
def saveAutoExtractedFileSize(landfile, event):
    ...
    try:
        ...
        if '302 Moved' in stdout:
            ...
            file_size = int([ token for token in part.split() if token.isdigit() ][0])
