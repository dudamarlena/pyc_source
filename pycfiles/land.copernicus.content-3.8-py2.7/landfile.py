...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/land/copernicus/content/content/landfile.py
...
MOD_64 = 18446744073709551616
SHIFT_63 = 9223372036854775808
...
class SchemaExtender(object):
    ...
    def add_file_category(self, category_name, category_value=''):
        """ Save new category in file categories tuple
        """
        old_file_categories = self.context.fileCategories
        new_file_category = {'name': category_name, 
    ...
    def getFields(self):
        ...
        default_values = [ {'name': col, 'value': ''} for col in columns ]
        field = ExtendedDataGridField('fileCategories', searchable=True, columns=('name',
                                                                                  'value'), default=default_values, allow_empty_rows=True, allow_delete=False, allow_insert=False, allow_reorder=False, widget=DataGridWidget(label='Categorization of this file', description='Enter, for each category, its value'))
        return [
         field]