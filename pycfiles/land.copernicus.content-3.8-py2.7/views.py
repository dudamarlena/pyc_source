...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/land/copernicus/content/browser/views.py
...
def is_EIONET_member(member):
    ...
    site = api.portal.get()
    try:
        return 'EIONET' in site.acl_users.get('ldap-plugin').acl_users.searchUsers(uid=member.getId())[0].get('dn', '')
    except Exception:
        return False
...
class RedirectDownloadUrl(BrowserView):
    ...
    def url_login_with_params(self):
        ...
        login_url = root_url + '/login' + '?came_from=' + land_item_url + '?fieldsetlegend-download=True'
        if auto_selected:
    ...
    def url_open_download_tab(self):
        ...
        url_tab = self.context.aq_parent.absolute_url() + '?fieldsetlegend-download=true'
        return url_tab
    ...
    def url_download(self):
        ...
        """
        land_item_url = self.context.aq_parent.absolute_url()
        url_download = land_item_url + '/@@download-land-file?remoteUrl=' + self.context.remoteUrl
        return url_download

    def __call__(self):
        is_anonymous = bool(getToolByName(getSite(), 'portal_membership').isAnonymousUser())
        if not is_anonymous:
        ...
        if remoteUrl_exists(remoteUrl):
            if profile_is_complete:
                ...
                return self.request.response.redirect(self.url_profile_error())
...
class AdminLandFilesView(BrowserView):
    ...
    def show_error(self, item, action, details=''):
        ...
        messages.add(('Error on {action} {item} {details}').format(action=action, item=item, details=details), type=ACTION_ERROR)

    def show_info(self, item, action, details=''):
        """ Show an info message related to an action for a land file
        """
        messages = IStatusMessage(self.request)
        messages.add(('Success on {action} {item} {details}').format(action=action, item=item, details=details), type=ACTION_INFO)

    def _do_get(self, landfile):
        return dict(status=ACTION_SUCCESS, title=landfile.title, id=landfile.shortname, description=landfile.description, download_url=landfile.remoteUrl, categorization_tags=landfile.fileCategories, size=landfile.fileSize)
    ...
    def do_get(self, title, logs=True):
        ...
        if result['status'] == ACTION_ERROR and logs is True:
            self.show_error(title, ACTION_GET, '- No item with this title found')
        return result
    ...
    def do_post(self, title, description, download_url, categorization_tags, logs=True):
        ...
        valid_tags = tuple((name, value) for name, value in parse_tags(categorization_tags) if name in fileCategories)
        lfa = LandFileApi(self.context.landfiles)
        try:
            lfa.add_with_filesize(title=title, description=description, remoteUrl=download_url, fileCategories=valid_tags)
            if logs is True:
    ...
    def do_delete(self, title, logs=True):
        ...
        try:
            ...
            result['status'] = ACTION_ERROR
            if logs is True:
                self.show_error(title, ACTION_DELETE, '- No items with this title found')
    ...
    def do_delete_all(self, logs=True):
        ...
        return [ dict(title=title, status=ACTION_SUCCESS) for title in landfiles_titles ]

    def do_put(self, title, description, download_url, categorization_tags, logs=True):
        """ Replace a landfile
        """
        result = {'title': title, 
           'status': ACTION_SUCCESS}
        ...
        lfa = LandFileApi(self.context.landfiles)
        try:
            lfa.edit_with_filesize(title, title=title, description=description, remoteUrl=download_url, fileCategories=valid_tags)
            if logs is True:
    ...
    def do_operations(self):
        ...
        if txt_file.filename is not '':
            ...
            if action == ACTION_GET:
                ...
                result = json.dumps(output_json, ensure_ascii=False)
                ...
                for landfile in landfiles:
                    a_result = self.do_post(title=landfile[0], description=landfile[1], download_url=landfile[2], categorization_tags=landfile[3])
                ...
                result = []
                for landfile in landfiles:
                    a_result = self.do_put(title=landfile[0], description=landfile[1], download_url=landfile[2], categorization_tags=landfile[3])
                    result.append(a_result)

                result = json.dumps(result, ensure_ascii=False)
            else:
                result = {}
                self.show_error(action, ACTION_EVALUATE, ' - missing action. Use radio buttons to select one.')
        else:
            result = {}
            self.show_error(txt_file.filename, ACTION_EVALUATE, ' - missing text file. Use file input to upload it.')
        return result
