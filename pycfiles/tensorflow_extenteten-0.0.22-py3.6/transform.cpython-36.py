...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/extenteten/transform.py
...
def batch(*tensor_lists_or_dicts):
    ...
    if not type_ in [list, tuple, dict]:
        raise AssertionError
    elif not all(type_ == type(obj) for obj in tensor_lists_or_dicts):
        raise AssertionError
    if type_ == tuple:
        type_ = list
        tensor_lists_or_dicts = [list(tuple_) for tuple_ in tensor_lists_or_dicts]
    if not type_ != dict:
        if not all(tensor_lists_or_dicts[0].keys() == dict_.keys() for dict_ in tensor_lists_or_dicts):
            raise AssertionError
    tensor_lists = tensor_lists_or_dicts if type_ == list else [dict_.values() for dict_ in tensor_lists_or_dicts]
    tensor_list = [dynamic_pack(*tensor_list) for tensor_list in zip(*tensor_lists)]
    if type_ == list:
        return tensor_list
    else:
        return {key:tensor for key, tensor in zip(tensor_lists_or_dicts[0].keys(), tensor_list)}
...
def _pad_to_shape(x, shape):
    ...
    if not static_rank(shape) == 1:
        raise AssertionError
    elif not static_rank(x) == static_shape(shape)[0]:
        raise AssertionError
    paddings = tf.concat([tf.expand_dims(paddings, 1) for paddings in [
     tf.zeros_like(shape), shape - tf.shape(x)]], 1)
