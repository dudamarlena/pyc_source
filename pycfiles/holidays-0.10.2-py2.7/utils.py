...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/holidays/utils.py
# Compiled at: 2020-01-30 17:23:45
import inspect, holidays
from datetime import date

def list_supported_countries():
    """List all supported countries incl. their abbreviation."""
    return [ name for name, obj in inspect.getmembers(holidays.countries, inspect.isclass)
           ]
...
def CountryHoliday(country, years=[], prov=None, state=None, expand=True, observed=True):
    try:
        ...
        country_holiday = country(years=years, prov=prov, state=state, expand=expand, observed=observed)
