...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/holidays/countries/spain.py
# Compiled at: 2020-03-22 12:40:30
...
class Spain(HolidayBase):
    ...
    def _populate(self, year):
        ...
        if year < 2015 and self.prov and self.prov in ('AR', 'CL', 'CM', 'EX', 'GA',
                                                       'MD', 'ML', 'MC', 'NC', 'PV',
                                                       'VC'):
            self[date(year, MAR, 19)] = 'San José'
        elif year == 2015 and self.prov and self.prov in ('CM', 'MD', 'ML', 'MC', 'NC',
                                                          'PV', 'VC'):
            self[date(year, MAR, 19)] = 'San José'
        elif year == 2016 and self.prov and self.prov in ('ML', 'MC', 'PV', 'VC'):
            self[date(year, MAR, 19)] = 'San José'
        elif year == 2017 and self.prov and self.prov in ('PV', ):
            self[date(year, MAR, 19)] = 'San José'
        elif 2018 <= year <= 2019 and self.prov and self.prov in ('GA', 'MC', 'NC',
                                                                  'PV', 'VC'):
            self[date(year, MAR, 19)] = 'San José'
        elif 2020 <= year <= 2025 and self.prov and self.prov in ('CM', 'GA', 'MC',
                                                                  'NC', 'PV', 'VC'):
            self[date(year, MAR, 19)] = 'San José'
