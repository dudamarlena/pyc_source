...
# [GCC 8.4.0]
# Embedded file name: /Users/raymond/Projects/TutorGen/python-client/env/lib/python3.4/site-packages/hpitclient/message_sender_mixin.py
...
class MessageSenderMixin(RequestsMixin):
    ...
    def send(self, message_name, payload, callback=None):
        ...
        response = self._post_data('message', {'name': message_name,  'payload': payload}).json()
        if callback:
    ...
    def send_transaction(self, payload, callback=None):
        ...
        """
        response = self._post_data('message', {'name': 'transaction',  'payload': payload}).json()
        if callback:
            self.response_callbacks[response['message_id']] = callback
        return response
    ...
    def share_resource(self, resource_token, other_entity_ids):
        ...
        response = self._post_data('share-message', {'resource_id': resource_token, 
