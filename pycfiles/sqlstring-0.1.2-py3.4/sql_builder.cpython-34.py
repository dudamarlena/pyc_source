...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlstring/sql_builder.py
# Compiled at: 2016-06-15 12:04:46
# Size of source mod 2**32: 12793 bytes
import six, sqlize, sqlparse
...
class StringBuilder(object):
    ...
    def _get_result_value(self, value_set_list, default_value=None, sort_list=True):
        ...
        if True == sort_list:
            value_list.sort()
        result_value = (
         default_value, value_list)[(len(value_list) > 0)]
        return result_value
...
class SelectBuilder(StringBuilder):
    __doc__ = ' Creates SQL select query string '
    ...
    def __init__(self, limit=None, offset=None, distinct=False):
        ...
        item_list = [
         'table', 'column', 'where', 'where_and', 'where_or', 'group_by', 'order_by']
...
class UpdateBuilder(StringBuilder):
    __doc__ = ' Creates SQL `UPDATE` query string. '
...
class DeleteBuilder(StringBuilder):
    __doc__ = ' Creates SQL `DELETE` query string. '
