...
# Embedded file name: /home/travis/build/beylsp/proxy-toggle/tests/test_px.py
# Compiled at: 2016-08-29 11:19:46
import base64, errno, getpass, mock, os, string, shutil, subprocess, sys, tempfile, unisquid, unittest
...
class TestProxyStore(unittest.TestCase):
    ...
    def test_ask_question_with_my_validator_success(self):
        q = 'Do you feel lucky?'
        a = 'Yes'
        answer = self.store._ask(q, input_function=lambda x: a, validator=lambda x: True)
        self.assertEquals(answer, a)
    ...
    def test_ask_question_with_my_validator_failed(self):
        ...
        self.count = 0
        q = 'Do you feel lucky?'
        answer = self.store._ask(q, input_function=lambda x: 'Yes', validator=validate)
        self.assertEquals(self.count, 2)
    ...
    def test_get_user_input_calls_for_correct_input(self):
        ...
        expected_questions = [mock.call('%s host: ' % prefix),
         ...
         mock.call('%s password: ' % prefix, input_function=getpass.getpass)]
    ...
    def test_write_config_creates_file_with_correct_permission(self):
        ...
        self.store._write_config('doe', 'http://corporate.proxy.com', 'john')
        os.open.assert_called_once_with(cfg_file, os.O_WRONLY | os.O_CREAT, 384)
    ...
    def test_write_pass_creates_file_with_correct_permission(self):
        ...
        os.fdopen = mock.MagicMock()
        self.store._write_pass('mysecretpassword')
        os.open.assert_called_once_with(pass_file, os.O_WRONLY | os.O_CREAT, 384)
...
class TestProxyExec(unittest.TestCase):
    ...
    def test_env_returns_correct_environment(self):
        ...
        settings = (
         user, password, urlo)
        self.executor.get_proxy_settings = mock.MagicMock(return_value=(
         user, password, urlo))
    ...
    def test_call_executes_child_process(self):
        cmd = [
         'arg1', 'arg2', 'arg3']
        expected_args = [
         mock.call((' ').join(cmd), shell=True, env={})]
    ...
    def test_call_with_nouser_flag_executes_child_process(self):
        cmd = [
         'arg1', 'arg2', 'arg3']
        expected_args = [
         mock.call((' ').join(cmd[1:]), shell=True, env={})]
        subprocess.Popen = mock.MagicMock()
        with mock.patch('proxytoggle.px.ProxyExec.env', return_value={}):
