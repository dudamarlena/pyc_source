...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/pysam/__init__.py
# Compiled at: 2015-04-29 05:25:18
...
import pysam.csamtools as csamtools, pysam.Pileup as Pileup, os
...
class SamtoolsDispatcher(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        """
        retval, stderr, stdout = csamtools._samtools_dispatch(self.dispatch, args, catch_stdout=kwargs.get('catch_stdout', True))
        if retval:
            raise SamtoolsError('csamtools returned with error %i: %s' % (
             retval, ('\n').join(stderr)))
        self.stderr = stderr
        stderr = [ x for x in stderr if not (x.startswith('[sam_header_read2]') or x.startswith('[bam_index_load]') or x.startswith('[bam_sort_core]') or x.startswith('[samopen] SAM header is present'))
                 ]
        if stderr:
            raise SamtoolsError(('\n').join(stderr))
...
__all__ = libchtslib.__all__ + ctabix.__all__ + cvcf.__all__ + cbcf.__all__ + cfaidx.__all__ + calignmentfile.__all__ + csamfile.__all__ + ['SamtoolsError', 'SamtoolsDispatcher'] + list(SAMTOOLS_DISPATCH) + [
 'Pileup']
...
def get_libraries():
    ...
    dirname = os.path.abspath(os.path.join(os.path.dirname(__file__)))
    return [ os.path.join(dirname, x) for x in ('libchtslib.so', 'TabProxies.so', 'cfaidx.so',
                                                'csamfile.so', 'cvcf.so', 'cbcf.so',
                                                'ctabix.so')
           ]