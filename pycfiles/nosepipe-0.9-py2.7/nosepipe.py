...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nosepipe.py
# Compiled at: 2019-05-30 15:27:23
...
"""
import logging, os, pickle, struct, subprocess, sys, nose.plugins
__version__ = '0.9'
...
class ProcessIsolationReporterPlugin(nose.plugins.Plugin):
    ...
    def _send_test_event(self, method_name, test, err=None):
        if err is not None:
            exc_pickle = pickle.dumps(self._fake_exc_info(err)).decode('latin1')
...
class SubprocessTestProxy(object):
    ...
    def __call__(self, result):
        ...
        try:
            popen = subprocess.Popen(argv, cwd=self._cwd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=useshell)
        except OSError as e:
            ...
            while True:
                ...
                if len(data) < request_len:
                    raise Exception('short message body (want %d, got %d)\n' % (
                     request_len, len(data)) + 'Something went wrong\nMessage: %s' % (header + data).decode('latin1'))
...
class ProcessIsolationPlugin(nose.plugins.Plugin):
    ...
    def __init__(self):
        ...
        self._argv += ProcessIsolationPlugin._get_nose_whitelisted_argv(offset=nosetests_index + 1)
        self._cwd = os.getcwd()
        return

    @staticmethod
    def _get_nose_whitelisted_argv(offset=1):
        whitelist = {'--debug-log': True, 
           '--logging-config': True, 
    ...
    def options(self, parser, env):
        ...
        parser.add_option('--with-process-isolation-individual', action='store_true', help='Run only selected tests in separate ' + 'processes [%s].' % env_opt, default=env.get(env_opt))
    ...
    def do_isolate(self, test):
        ...
        if hasattr(test.test, 'nosepipe_isolate'):
            return True
        if hasattr(test.test, 'test') and hasattr(test.test.test, 'nosepipe_isolate'):
            return True
