...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3o/renderers/juno/main.py
# Compiled at: 2018-07-03 11:49:12
import jpype
from jpype import startJVM
from jpype import JPackage
import os, json, platform, logging, pkg_resources
...
def get_oo_context():
    ...
    if os.name in ('nt', 'os2', 'ce'):
        ...
        context['oooclasses_subpath'] = os.path.join('Basis', 'program', 'classes')
    else:
        context['java_classpath_sep'] = ':'
...
def start_jvm(jvm, oobase, urebase, max_mem):
    ...
    jar = pkg_resources.resource_filename('py3o.renderers.juno', 'py3oconverter.jar')
    oojars = list()
    ...
    def convert(self, infilename, outfilename, filtername, pdf_options):
        ...
        for option_name, option_val in pdf_options.items():
            if not isinstance(option_name, (str, unicode)):
                log.error('The keys of the pdf_options dict must be strings.(wrong key: %s type %s', option_name, type(option_name))
                continue
            if isinstance(option_val, bool):
                ...
                log.error('Unsupported option type: %s type %s', option_name, type(option_val))
        ...
        self.jconvertor.convert(infilename, outfilename, filtername, pdf_options_json, pdf_options_types_json)