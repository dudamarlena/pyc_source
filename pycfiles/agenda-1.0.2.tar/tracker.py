...
# Embedded file name: build/bdist.linux-x86_64/egg/ageliaco/tracker/content/tracker.py
# Compiled at: 2011-09-20 15:56:48
...
grok.global_utility(KeywordsVocabulary, name='ageliaco.rd.tracker.Subjects')

class ITracker(form.Schema):
    """
    ITracker
    """
    content = RichText(title=_('Content'), required=True)
...
class View(grok.View):
    ...
    def subtrackers(self):
        ...
        log('all subtrackers')
        return catalog(object_provides=[ITracker.__identifier__], path={'query': ('/').join(context.getPhysicalPath()), 'depth': 1}, sort_on='sortable_title')
    ...
    def subTrackerIssues(self, object, wf_state='all'):
        ...
        if wf_state == 'all':
            log('all issues')
            return catalog(object_provides=[IIssue.__identifier__, ITracker.__identifier__], path={'query': object.getPath(), 'depth': 1}, sort_on='modified', sort_order='reverse')
        return catalog(object_provides=[IIssue.__identifier__, ITracker.__identifier__], review_state=wf_state, path={'query': object.getPath(), 'depth': 1}, sort_on='sortable_title')
    ...
    def issues(self, wf_state='all'):
        ...
        if wf_state == 'all':
            log('all issues')
            return catalog(object_provides=[IIssue.__identifier__], path={'query': ('/').join(context.getPhysicalPath()), 'depth': 1}, sort_on='modified', sort_order='reverse')
        aq = catalog(object_provides=[IIssue.__identifier__], review_state=wf_state, path={'query': ('/').join(context.getPhysicalPath()), 'depth': 1}, sort_on='deadline')
        return aq