...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3o/types/config.py
# Compiled at: 2016-05-25 09:30:55
...
class Py3oTypeConfig(object):
    ...
    def __init__(self, *args, **kwargs):
        self.config = kwargs
        self.types = {key:typ.get_type_for_config(kwargs) for key, typ in type(self).types.items()}

    @classmethod
    def get_meta_var_key_attribute(cls, nsmap):
        """Helper method to get the full name for meta:name attribute."""
        return '{{{}}}{}'.format(nsmap[cls.META_VAR_TAG_NS], cls.META_VAR_KEY_ATTRIBUTE)

    @classmethod
    def get_meta_tag(cls, nsmap):
        """Helper method to get the full name for <meta:user-defined> tags."""
        return '{{{}}}{}'.format(nsmap[cls.META_VAR_TAG_NS], cls.META_VAR_TAG_NAME)
    ...
    def inspect_meta(cls, odf_file):
        ...
        try:
            ...
            raise InvalidODFFile('Cannot parse XML in {}: {}'.format(cls.META_ARCNAME, six.text_type(e)))
        ...
        if meta_root is None:
            raise InvalidODFFile('No {} element in {}'.format(root_tag, cls.META_ARCNAME))
        odf_archive.close()
    ...
    def from_odf_file(cls, odf_file, defaults=None):
        ...
        config = {var.get(meta_var_key):var.text for var in meta_vars}
        if defaults:
    ...
    def apply_to_odf_file(self, in_file, out_file=None):
        ...
        for key, val in self.config.items():
            new_var = etree.SubElement(meta_root, meta_tag, **{meta_var_key: key})
