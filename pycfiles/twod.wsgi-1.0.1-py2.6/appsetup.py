...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/twod/wsgi/appsetup.py
# Compiled at: 2011-06-29 05:28:07
"""
...
def _set_up_settings(global_conf, local_conf):
    ...
    if hasattr(settings_module, 'DEBUG'):
        raise ValueError('Module %s must not define "DEBUG". It must be set in the PasteDesploy configuration file as "debug".' % django_settings_module)
    ...
    for (setting_name, setting_value) in options.items():
        if not hasattr(settings_module, setting_name):
            ...
            _LOGGER.warn('"%s" will not be overridden in %s', setting_name, django_settings_module)
...
def _convert_options(global_conf, local_conf):
    ...
    if 'DEBUG' in global_conf or 'DEBUG' in local_conf:
        raise ValueError("Do not set Django's DEBUG in the configuration file; use Paste's 'debug' instead")
    if 'debug' not in global_conf:
        raise ValueError("Paste's 'debug' option must be set in the configuration file")
    ...
    custom_none_if_empty_settings = aslist(global_conf.get('twod.none_if_empty_settings', ''))
    ...
    none_if_empty_settings = _DJANGO_NONE_IF_EMPTY_SETTINGS | frozenset(custom_none_if_empty_settings)
    options = {}
    for (option_name, option_value) in local_conf.items():
        if option_name in booleans:
            ...
            nested_tuple = tuple(tuple(val.strip() for val in tuple_.strip().split(';') if val) for tuple_ in option_value.strip().splitlines())
            ...
            dictionary = dict((key.strip(), value.strip()) for (key, value) in items)
            ...
            raise ValueError('Django setting %s is not (yet) supported; you have to define it in your options module.' % option_name)
        else:
