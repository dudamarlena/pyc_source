...
# [GCC 8.4.0]
# Embedded file name: /Users/anyi/workspace/django-vali/vali/templatetags/vali_tags.py
    ...
    if not isinstance(request, HttpRequest):
        ...
        try:
            ...
            if 'group' in _config and _config['group']:
                if 'group_marker' in _config:
                    if _config['group_marker']:
...
def vali_fieldset(fieldset):
    for field in fieldset.form.fields:
        if isinstance(fieldset.form.fields[field], ModelMultipleChoiceField):
            ...
            fieldset.form.fields[field].widget = ValiRelatedFieldWidgetWrapper(relw.widget, relw.rel, relw.admin_site, relw.can_add_related, relw.can_change_related, relw.can_delete_related)
        else:
            attrs = fieldset.form.fields[field].widget.attrs
            oldclass = attrs['class'] if 'class' in attrs else ''
            if fieldset.form.errors:
                if field in fieldset.form._errors:
