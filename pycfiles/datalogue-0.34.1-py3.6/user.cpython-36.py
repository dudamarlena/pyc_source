...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/datalogue/clients/user.py
...
class _UserClient:
    ...
    def get(self, user_id: UUID) -> Union[(DtlError, User)]:
        ...
        res = self.http_client.make_authed_request('/user/public', (HttpMethod.GET), params=params)
        if isinstance(res, DtlError):
    ...
    def search_by_email(self, text_in_email: str, page: int=1, item_per_page: int=25) -> Union[(DtlError, User)]:
        ...
        """
        res = self.http_client.make_authed_request(f"/organization/users/search?email={text_in_email}&page={page}&size={item_per_page}", HttpMethod.GET)
        if isinstance(res, DtlError):
