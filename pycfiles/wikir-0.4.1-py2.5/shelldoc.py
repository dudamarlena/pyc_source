...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/wikir/tests/shelldoc.py
...
shell_sess_re = re.compile('(?P<indent>[\\s]+)(?P<token>(\\$|\\|)\\s?)(?P<line>.*)')
...
class ShellSession(object):
    ...
    def __unicode__(self):
        ...
        desc = '<%s($ %s ... and %s more)>' % (self.__class__.__name__, cmd, len(self))
        return desc
...
def validate_command(cmd, expected_output=''):
    ...
    context = textwrap.dedent('        COMMAND: %s\n        WORKING DIR: %s\n        ' % (
