...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/wikir/wikir.py
# Compiled at: 2009-05-15 11:59:08
"""core"""
import re, pydoc, optparse, inspect, tokenize, docutils.core
...
wiki_word_re = re.compile('^[A-Z][a-z]+(?:[A-Z][a-z]+)+')
auto_url_re = re.compile('^(http|https|ftp)\\://')
...
class WikiVisitor(SparseNodeVisitor):
    ...
    def _find_header_level(self, node):
        if isinstance(node.parent, nodes.topic):
            ...
            assert isinstance(node.parent, nodes.section), 'unexpected parent: %s' % node.parent.__class__
