...
# [GCC 8.4.0]
# Embedded file name: /data/devel/django_backstage/backstage/venue/venue.py
# Compiled at: 2014-07-07 19:35:33
...
class Venue:
    ...
    def __init__(self, venue_home):
        ...
        if not os.path.exists(venue_secret):
            ...
            self.venue_home = venue_home.rstrip('/')
            ...
            if self.venue_home not in sys.path:
                sys.path.insert(0, self.venue_home)
            if self.venue_root not in sys.path:
                sys.path.insert(0, self.venue_root)
            if self.acts_root not in sys.path:
