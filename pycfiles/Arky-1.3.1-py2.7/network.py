...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arky\cli\network.py
# Compiled at: 2017-12-21 16:22:13
"""
Usage: 
...
from .. import util
from . import DATA
import arky, sys, imp, webbrowser
...
def use(param):
    if not param['<name>']:
        ...
        if choices:
            ...
            return False
    DATA.initialize()
    rest.use(param.get('<name>'), broadcast=int(param.get('--broadcast', 10)), timeout=float(param.get('--latency', 5000)) / 1000)
