...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arky\ark\aip11.py
# Compiled at: 2017-11-08 00:53:31
from .. import slots
from .. import rest
from .. import cfg
from . import __PY3__
...
class Payload:
    ...
    def type0(**kw):
        ...
        return struct.pack('<QI21s' if __PY3__ else '<QI' + 21 * 'c', kw.get('amount', 0), kw.get('expiration', 0), recipientId)
    ...
    def type1(**kw):
        ...
        return struct.pack(33 * 'c', secondPublicKey)
    ...
    def type2(**kw):
        ...
        if username:
            ...
            if 3 <= length <= 255:
                ...
                return struct.pack('<B' + length * 'c', length, username)
...
def getHeaders(**kw):
    ...
    header = struct.pack('<BBBBI', kw.get('head', 255), kw.get('version', 2), kw.get('network', int(cfg.marker, base=16)), kw.get('type', 0), int(slots.getTime()))
    header += struct.pack('<33s', crypto.unhexlify(publicKey)) if __PY3__ else struct.pack(33 * 'c', publicKey)
    header += struct.pack('<Q', kw.get('fees', 0))
    vendorField = kw.get('vendorField', '')
    n = min(255, len(vendorField))
    header += struct.pack('<B', n)
    if n > 0:
        header += struct.pack('<%ss' % n, crypto.unhexlify(publicKey[:n])) if __PY3__ else struct.pack(n * 'c', publicKey[:n])
...
def sendTransaction(**kw):
    ...
    result['broadcast'] = '%.1f%%' % (100.0 * success / len(cfg.peers))
    return result