...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arky\util.py
# Compiled at: 2018-01-13 11:56:58
from . import __PY3__
from . import HOME
from . import ROOT
from . import slots
from . import rest
from . import cfg
import io, os, sys, json, struct, hashlib, logging, getpass, binascii, requests, threading
basint = (lambda e: e) if __PY3__ else (lambda e: ord(e))
...
pack_bytes = (lambda f, v: pack('!' + '%ss' % len(v), f, (v,))) if __PY3__ else (lambda f, v: pack('!' + 'c' * len(v), f, v))
...
def getVoteForce(address, **kw):
    balance = kw.pop('balance', 0) / 100000000.0
    if not balance:
        balance = float(rest.GET.api.accounts.getBalance(address=address, returnKey='balance')) / 100000000.0
    ...
    for tx in history:
        ...
        balance += (tx['fee'] + tx['amount'] if tx['senderId'] == address else -tx['amount']) / 100000000.0
        if tx['type'] == 3:
            brk = True
            break
        end = tx['timestamp']
...
def createBase(secret):
    ...
    base = ''
    for c in hexlify(hashlib.md5((isinstance(secret, bytes) or secret.encode)() if 1 else secret).digest()):
        try:
