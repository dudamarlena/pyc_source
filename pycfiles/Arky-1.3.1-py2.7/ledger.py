...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arky\cli\ledger.py
# Compiled at: 2018-01-12 17:11:03
"""
Usage: 
...
def send(param):
    if DATA.ledger:
        ...
        if amount:
            ...
            tx = dict(type=0, timestamp=int(slots.getTime()), fee=cfg.fees['send'], amount=amount * 100000000, recipientId=param['<address>'], vendorField=param['<message>'])
...
def vote(param):
    ...
    if len(lst):
        ...
        tx = dict(type=3, timestamp=int(slots.getTime()), fee=cfg.fees['vote'], amount=0, recipientId=DATA.ledger['address'], asset={'votes': lst})
        if _sign(tx, DATA.ledger['path']):
            _send(tx)