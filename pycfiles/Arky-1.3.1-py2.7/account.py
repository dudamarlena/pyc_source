...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\arky\cli\account.py
# Compiled at: 2018-01-13 11:24:10
"""
Usage:
...
def _whereami():
    if DATA.account:
        return 'account[%s]' % util.shortAddress(DATA.getCurrent1stPKey() if DATA.escrowed else DATA.getCurrentAddress())
    else:
        return 'account'
...
def save(param):
    if DATA.account:
        util.dumpAccount(util.createBase(util.hidenInput('Enter pin code: ')), DATA.account['address'], DATA.firstkeys['privateKey'], DATA.secondkeys.get('privateKey', None), param['<name>'])
    return
...
def register(param):
    if DATA.account:
        if param['2ndSecret']:
            ...
            if askYesOrNo('Register second public key %s ?' % secondPublicKey) and checkSecondKeys():
                sys.stdout.write('    Broadcasting second secret registration...\n')
                _send(arky.core.crypto.bakeTransaction(type=1, publicKey=DATA.firstkeys['publicKey'], privateKey=DATA.firstkeys['privateKey'], secondPrivateKey=DATA.secondkeys.get('privateKey', None), asset={'signature': {'publicKey': secondPublicKey}}))
        elif param['escrow']:
            ...
            if askYesOrNo('Register thirdparty public key %s ?' % secondPublicKey) and checkSecondKeys():
                sys.stdout.write('    Broadcasting thirdparty registration...\n')
                _send(arky.core.crypto.bakeTransaction(type=1, publicKey=DATA.firstkeys['publicKey'], privateKey=DATA.firstkeys['privateKey'], secondPrivateKey=DATA.secondkeys.get('privateKey', None), asset={'signature': {'publicKey': secondPublicKey}}))
        else:
            username = param['<username>']
            if askYesOrNo('Register %s account as delegate %s ?' % (DATA.account['address'], username)) and checkSecondKeys():
                sys.stdout.write('    Broadcasting delegate registration...\n')
                _send(arky.core.crypto.bakeTransaction(type=2, publicKey=DATA.firstkeys['publicKey'], privateKey=DATA.firstkeys['privateKey'], secondPrivateKey=DATA.secondkeys.get('privateKey', None), asset={'delegate': {'username': username, 'publicKey': DATA.firstkeys['publicKey']}}))
    return
...
def vote(param):
    ...
    if len(lst) and askYesOrNo('%s %s ?' % (verb, (', ').join(to_vote))) and checkSecondKeys():
        _send(arky.core.crypto.bakeTransaction(type=3, recipientId=DATA.account['address'], publicKey=DATA.firstkeys['publicKey'], privateKey=DATA.firstkeys['privateKey'], secondPrivateKey=DATA.secondkeys.get('privateKey', None), asset={'votes': lst}))
    return
...
def send(param):
    if DATA.account:
        ...
        if amount and askYesOrNo('Send %(amount).8f %(token)s to %(recipientId)s ?' % {'token': cfg.token, 'amount': amount, 'recipientId': param['<address>']}) and checkSecondKeys():
            _send(arky.core.crypto.bakeTransaction(amount=amount * 100000000, recipientId=param['<address>'], vendorField=param['<message>'], publicKey=DATA.firstkeys['publicKey'], privateKey=DATA.firstkeys['privateKey'], secondPrivateKey=DATA.secondkeys.get('privateKey', None)))
    return