...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\foxInstaller\__init__.py
# Compiled at: 2013-10-15 10:12:56
...
config = {'PROJECT_NAME': None, 
   ...
   'PACKAGES': [
              ...
              'git']}
parser = OptionParser()
parser.add_option('-y', '--yes', action='store_true', dest='ignore_prompts', default=False, help='Dont ask for confirmations from user.')
parser.add_option('-q', '--no_config', action='store_true', dest='no_config', default=False, help='Dont ask for config from stdin.')
parser.add_option('-v', '--verbose', action='store_true', dest='verbose', default=False, help='Verbose logging.')
try:
    from cfg import basic_config
    config = dict(config.items() + basic_config.items())
    if not config.get('INNER_PROJECT_NAME', False):
        config['INNER_PROJECT_NAME'] = config['PROJECT_NAME']
...
class FoxInstaller(object):
    ...
    def create_venv(self):
        ...
        if not os.path.exists(os.path.join(self.install_dir, self.config['VENV_DIR'], venv_bin, 'activate')):
            with self.log_helper('Creating venv'):
                check_call(['virtualenv', '--system-site-packages', os.path.join(self.install_dir, self.config['VENV_DIR'])], stdout=open(os.devnull, 'wb'), stderr=STDOUT)
        else:
            print '* Found venv.'
    ...
    def create_files(self):
        ...
        with self.log_helper('Creating upstart job'):
            self.file_from_template('gunicorn-%s.conf' % self.config['PROJECT_NAME'], 'upstart_job', '/etc/init/', True)
    ...
    def chown_and_chmod(self):
        if self.is_unix:
            with self.log_helper('Changing user permissions'):
                ...
                OSHelpers.chmod_r(self.install_dir, 448)
    ...
    def dependencies(self):
        with self.log_helper('Installing dependencies'):
            ...
            with open(os.devnull, 'w') as (fnull):
                call(cmd, stdout=fnull, stderr=fnull, shell=True)

    def syncdb(self):
        with self.log_helper('Running syncdb'):
            if self.is_unix:
                cmd = 'source %(INSTALL_DIR)s/%(VENV_DIR)s/bin/activate && cd %(INNER_PROJECT_NAME)s && python manage.py syncdb'
            else:
                cmd = 'cmd.exe /K "cd %(VENV_DIR)s/Scripts & call activate.bat & cd ..\\..\\%(INNER_PROJECT_NAME)s & python manage.py syncdb'
            cmd = cmd % dict(self.config.items() + {'INSTALL_DIR': self.install_dir}.items())
            os.system(cmd)
