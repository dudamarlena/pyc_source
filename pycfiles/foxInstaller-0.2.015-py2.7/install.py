...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\foxInstaller\install.py
# Compiled at: 2013-09-04 09:25:20
...
class FoxInstaller(object):
    ...
    def pull(self):
        with self.log_helper('Pulling from server'):
            check_call(['git pull'], stdout=open(os.devnull, 'wb'), stderr=STDOUT)
    ...
    def dependencies(self):
        with self.log_helper('Installing dependencies'):
            if self.is_unix:
                ...
                cmd = 'cmd.exe /K "cd %(VENV_DIR)s/Scripts & call activate.bat & cd ..\\.. & pip install -r requirements.txt'
