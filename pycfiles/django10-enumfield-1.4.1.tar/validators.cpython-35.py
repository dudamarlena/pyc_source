...
# [GCC 8.4.0]
# Embedded file name: /home/jb/projects/i2biz/misc/django-enumfield/django_enumfield/validators.py
...
def validate_valid_transition(enum, from_value, to_value):
    ...
    if hasattr(enum, '_transitions') and not enum.is_valid_transition(from_value, to_value):
        ...
        raise InvalidStatusOperationError(message.format(enum=enum.__name__, from_value=enum.name(from_value), to_value=enum.name(to_value) or to_value))
