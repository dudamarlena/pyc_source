...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/utils.py
...
"""
import inspect, re, os, sys, pkgutil
...
from werkzeug._compat import unichr, text_type, string_types, iteritems, reraise, PY2
from werkzeug._internal import _DictAccessorProperty, _parse_signature, _missing
_format_re = re.compile('\\$(?:(%s)|\\{(%s)\\})' % ('[a-zA-Z_][a-zA-Z0-9_]*', '[a-zA-Z_][a-zA-Z0-9_]*'))
_entity_re = re.compile('&([^;]+);')
_filename_ascii_strip_re = re.compile('[^A-Za-z0-9_.-]')
_windows_device_files = ('CON', 'AUX', 'COM1', 'COM2', 'COM3', 'COM4', 'LPT1', 'LPT2',
                         'LPT3', 'PRN', 'NUL')
        ...
        def proxy(*children, **arguments):
            ...
            if not children and tag in self._empty_elements:
                ...
                if children_as_string:
                    ...
                    elif tag in self._c_like_cdata:
                        if self._dialect == 'xhtml':
...
def validate_arguments(func, args, kwargs, drop_extra=True):
    ...
    elif extra or extra_positional:
        if not drop_extra:
...
def bind_arguments(func, args, kwargs):
    ...
    if vararg_var is not None:
        ...
        if kwarg_var is not None:
            ...
            if multikw:
                raise TypeError('got multiple values for keyword argument ' + repr(next(iter(multikw))))
...
class ArgumentValidationError(ValueError):
    __doc__ = 'Raised if :func:`validate_arguments` fails to validate'

    def __init__(self, missing=None, extra=None, extra_positional=None):
        self.missing = set(missing or ())
        self.extra = extra or {}
        self.extra_positional = extra_positional or []
        ValueError.__init__(self, 'function arguments invalid.  (%d missing, %d additional)' % (
         len(self.missing),
         len(self.extra) + len(self.extra_positional)))


class ImportStringError(ImportError):
    __doc__ = 'Provides information about a failed :func:`import_string` attempt.'
    import_name = None
    exception = None
...
from werkzeug.http import quote_header_value, unquote_header_value, cookie_date
