...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/wsgi.py
...
"""
import re, os, sys, posixpath, mimetypes
from itertools import chain
from zlib import adler32
from time import time, mktime
from datetime import datetime
from functools import partial, update_wrapper
from werkzeug._compat import iteritems, text_type, string_types, implements_iterator, make_literal_wrapper, to_unicode, to_bytes, wsgi_get_bytes, try_coerce_native, PY2
...
def get_current_url(environ, root_only=False, strip_querystring=False, host_only=False, trusted_hosts=None):
    ...
    """
    tmp = [
     environ['wsgi.url_scheme'], '://', get_host(environ, trusted_hosts)]
    cat = tmp.append
    if host_only:
...
def get_host(environ, trusted_hosts=None):
    ...
    if (
     environ['wsgi.url_scheme'], environ['SERVER_PORT']) not in (('https', '443'),
                                                                 ('http', '80')):
...
def extract_path_info(environ_or_baseurl, path_or_url, charset='utf-8', errors='replace', collapse_http_schemes=True):
    ...
    elif not (base_scheme in ('http', 'https') and base_scheme == cur_scheme):
        return
...
class SharedDataMiddleware(object):
    ...
    def get_file_loader(self, filename):
        return lambda x: (
         os.path.basename(filename), self._opener(filename))
    ...
    def generate_etag(self, mtime, file_size, real_filename):
        ...
        return 'wzsdm-%d-%s-%s' % (
         ...
         file_size,
         adler32(real_filename) & 4294967295)
...
class DispatcherMiddleware(object):
    __doc__ = "Allows one to mount middlewares or applications in a WSGI application.\n    This is useful if you want to combine multiple WSGI applications::\n\n        app = DispatcherMiddleware(app, {\n            '/app2':        app2,\n            '/app3':        app3\n        })\n    "
...
class FileWrapper(object):
    __doc__ = "This class can be used to convert a :class:`file`-like object into\n    an iterable.  It yields `buffer_size` blocks until the file is fully\n    read.\n\n    You should not use this class directly but rather use the\n    :func:`wrap_file` function that uses the WSGI server's file wrapper\n    support if it's available.\n\n    .. versionadded:: 0.5\n\n    If you're using this object together with a :class:`BaseResponse` you have\n    to use the `direct_passthrough` mode.\n\n    :param file: a :class:`file`-like object with a :meth:`~file.read` method.\n    :param buffer_size: number of bytes for one iteration.\n    "
...
def make_chunk_iter(stream, separator, limit=None, buffer_size=10240):
...
class LimitedStream(object):
    ...
    def exhaust(self, chunk_size=65536):
