...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/flask/testsuite/basic.py
# Compiled at: 2014-02-15 13:00:30
# Size of source mod 2**32: 47207 bytes
"""
...
"""
import re, uuid, time, flask, pickle, unittest
...
class BasicFunctionalityTestCase(FlaskTestCase):
    ...
    def test_session_using_server_name(self):
        app = flask.Flask(__name__)
        app.config.update(SECRET_KEY='foo', SERVER_NAME='example.com')
    ...
    def test_session_using_server_name_and_port(self):
        app = flask.Flask(__name__)
        app.config.update(SECRET_KEY='foo', SERVER_NAME='example.com:8080')
    ...
    def test_session_using_server_name_port_and_path(self):
        app = flask.Flask(__name__)
        app.config.update(SECRET_KEY='foo', SERVER_NAME='example.com:8080', APPLICATION_ROOT='/foo')
    ...
    def test_session_using_application_root(self):
        ...
        app.config.update(SECRET_KEY='foo', APPLICATION_ROOT='/bar')
    ...
    def test_session_using_session_settings(self):
        app = flask.Flask(__name__)
        app.config.update(SECRET_KEY='foo', SERVER_NAME='www.example.com:8080', APPLICATION_ROOT='/test', SESSION_COOKIE_DOMAIN='.example.com', SESSION_COOKIE_HTTPONLY=False, SESSION_COOKIE_SECURE=True, SESSION_COOKIE_PATH='/')
    ...
    def test_session_expiration(self):
        ...
        match = re.search('\\bexpires=([^;]+)(?i)', rv.headers['set-cookie'])
        ...
        self.assert_in('set-cookie', rv.headers)
        match = re.search('\\bexpires=([^;]+)', rv.headers['set-cookie'])
    ...
    def test_make_response_with_response_instance(self):
        app = flask.Flask(__name__)
        with app.test_request_context():
            rv = yield from flask.make_response(flask.jsonify({'msg': 'W00t'}), 400)
            ...
            rv = flask.make_response(flask.Response('', headers={'Content-Type': 'text/html'}), 400, [('X-Foo', 'bar')])
            self.assertEqual(rv.status_code, 400)
    ...
    def test_url_generation(self):
        ...
        with app.test_request_context():
            ...
            self.assert_equal(flask.url_for('hello', name='test x', _external=True), 'http://localhost/hello/test%20x')
    ...
    def test_static_files(self):
        ...
        with app.test_request_context():
            self.assert_equal(flask.url_for('static', filename='index.html'), '/static/index.html')
        rv.close()
    ...
    def test_test_app_proper_environ(self):
        ...
        try:
            ...
            self.assert_equal(str(e), 'the server name provided ' + "('localhost.localdomain:443') does not match the " + "server name from the WSGI environment ('localhost.localdomain')")
        ...
        try:
            ...
            self.assert_equal(str(e), 'the server name provided ' + "('localhost.localdomain') does not match the " + "server name from the WSGI environment ('foo.localhost')")
    ...
    def test_url_processors(self):
        ...
        def add_language_code(endpoint, values):
            if flask.g.lang_code is not None:
    ...
    def test_routing_redirect_debugging(self):
        ...
        with app.test_client() as (c):
            try:
                ...
                self.assert_in('Make sure to directly send your POST-request to this URL', str(e))
            else:
                self.fail('Expected exception')
