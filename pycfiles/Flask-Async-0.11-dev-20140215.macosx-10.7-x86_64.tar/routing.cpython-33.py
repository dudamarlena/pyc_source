...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/routing.py
# Compiled at: 2014-01-20 15:46:16
# Size of source mod 2**32: 62520 bytes
"""
    werkzeug.routing
    ~~~~~~~~~~~~~~~~
...
class RoutingException(Exception):
    __doc__ = 'Special exceptions that require the application to redirect, notifying\n    about missing urls, etc.\n\n    :internal:\n    '


class RequestRedirect(HTTPException, RoutingException):
    __doc__ = 'Raise if the map requests a redirect. This is for example the case if\n    `strict_slashes` are activated and an url that requires a trailing slash.\n\n    The attribute `new_url` contains the absolute destination url.\n    '
    code = 301
...
class RequestSlash(RoutingException):
    __doc__ = 'Internal exception.'


class RequestAliasRedirect(RoutingException):
    __doc__ = 'This rule is an alias and wants to redirect to the canonical URL.'
...
class BuildError(RoutingException, LookupError):
    __doc__ = 'Raised if the build system cannot find a URL for an endpoint with the\n    values provided.\n    '
...
class ValidationError(ValueError):
    __doc__ = 'Validation error.  If a rule converter raises this exception the rule\n    does not match the current URL and the next URL is tried.\n    '


class RuleFactory(object):
    __doc__ = "As soon as you have more complex URL setups it's a good idea to use rule\n    factories to avoid repetitive tasks.  Some of them are builtin, others can\n    be added by subclassing `RuleFactory` and overriding `get_rules`.\n    "
...
class Submount(RuleFactory):
    __doc__ = "Like `Subdomain` but prefixes the URL rule with a given string::\n\n        url_map = Map([\n            Rule('/', endpoint='index'),\n            Submount('/blog', [\n                Rule('/', endpoint='blog/index'),\n                Rule('/entry/<entry_slug>', endpoint='blog/show')\n            ])\n        ])\n\n    Now the rule ``'blog/show'`` matches ``/blog/entry/<entry_slug>``.\n    "
...
class EndpointPrefix(RuleFactory):
    __doc__ = "Prefixes all endpoints (which must be strings for this factory) with\n    another string. This can be useful for sub applications::\n\n        url_map = Map([\n            Rule('/', endpoint='index'),\n            EndpointPrefix('blog/', [Submount('/blog', [\n                Rule('/', endpoint='index'),\n                Rule('/entry/<entry_slug>', endpoint='show')\n            ])])\n        ])\n    "
...
class RuleTemplateFactory(RuleFactory):
    __doc__ = 'A factory that fills in template variables into rules.  Used by\n    `RuleTemplate` internally.\n\n    :internal:\n    '
...
class Rule(RuleFactory):
    ...
    def empty(self):
        ...
        return Rule(self.rule, defaults, self.subdomain, self.methods, self.build_only, self.endpoint, self.strict_slashes, self.redirect_to, self.alias, self.host)
    ...
    def compile(self):
        ...
        if self.build_only:
            ...
            regex = '^%s%s$' % (
             ''.join(regex_parts),
             (not self.is_leaf or not self.strict_slashes) and '(?<!/)(?P<__suffix__>/?)' or '')
    ...
    def build(self, values, append_unknown=True):
        ...
        if append_unknown:
            ...
            if query_vars:
                url += '?' + url_encode(query_vars, charset=self.map.charset, sort=self.map.sort_parameters, key=self.map.sort_key)
        return (
         domain_part, url)
...
class BaseConverter(object):
    __doc__ = 'Base class for all converters.'
    regex = '[^/]+'
    weight = 100
...
class UnicodeConverter(BaseConverter):
    __doc__ = "This converter is the default converter and accepts any string but\n    only one path segment.  Thus the string can not include a slash.\n\n    This is the default validator.\n\n    Example::\n\n        Rule('/pages/<page>'),\n        Rule('/<string(length=2):lang_code>')\n\n    :param map: the :class:`Map`.\n    :param minlength: the minimum length of the string.  Must be greater\n                      or equal 1.\n    :param maxlength: the maximum length of the string.\n    :param length: the exact length of the string.\n    "
...
class AnyConverter(BaseConverter):
    __doc__ = 'Matches one of the items provided.  Items can either be Python\n    identifiers or strings::\n\n        Rule(\'/<any(about, help, imprint, class, "foo,bar"):page_name>\')\n\n    :param map: the :class:`Map`.\n    :param items: this function accepts the possible items as positional\n                  arguments.\n    '
...
class PathConverter(BaseConverter):
    __doc__ = "Like the default :class:`UnicodeConverter`, but it also matches\n    slashes.  This is useful for wikis and similar applications::\n\n        Rule('/<path:wikipage>')\n        Rule('/<path:wikipage>/edit')\n\n    :param map: the :class:`Map`.\n    "
    regex = '[^/].*?'
    weight = 200


class NumberConverter(BaseConverter):
    __doc__ = 'Baseclass for `IntegerConverter` and `FloatConverter`.\n\n    :internal:\n    '
    weight = 50
    ...
    def bind_to_environ(self, environ, server_name=None, subdomain=None):
        ...
        if server_name is None:
            if 'HTTP_HOST' in environ:
                ...
                if (environ['wsgi.url_scheme'], environ['SERVER_PORT']) not in (('https', '443'),
                                                                                ('http', '80')):
        ...
        return Map.bind(self, server_name, script_name, subdomain, environ['wsgi.url_scheme'], environ['REQUEST_METHOD'], path_info, query_args=query_args)
...
class MapAdapter(object):
    __doc__ = 'Returned by :meth:`Map.bind` or :meth:`Map.bind_to_environ` and does\n    the URL matching and building based on runtime information.\n    '
    ...
    def match(self, path_info=None, method=None, return_rule=False, query_args=None):
        ...
        path = '%s|/%s' % (
         self.map.host_matching and self.server_name or self.subdomain, path_info.lstrip('/'))
        have_match_for = set()
        for rule in self.map._rules:
            try:
                rv = rule.match(path)
            except RequestSlash:
                raise RequestRedirect(self.make_redirect_url(path_info + '/', query_args))
            except RequestAliasRedirect as e:
                raise RequestRedirect(self.make_alias_redirect_url(path, rule.endpoint, e.matched_values, method, query_args))
    ...
    def make_redirect_url(self, path_info, query_args=None, domain_part=None):
        ...
        return str('%s://%s/%s%s' % (
         ...
         posixpath.join(self.script_name[:-1].lstrip('/'), url_quote(path_info.lstrip('/'), self.map.charset, safe='/:|+')),
         suffix))
