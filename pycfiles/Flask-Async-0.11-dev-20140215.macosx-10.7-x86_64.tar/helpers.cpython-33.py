...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/flask/helpers.py
# Compiled at: 2014-02-15 13:00:30
# Size of source mod 2**32: 34349 bytes
"""
    flask.helpers
    ~~~~~~~~~~~~~
...
"""
import os, sys, pkgutil, posixpath, mimetypes
...
def send_file(filename_or_fp, mimetype=None, as_attachment=False, attachment_filename=None, add_etags=True, cache_timeout=None, conditional=False):
    ...
    if not attachment_filename:
        if not mimetype and isinstance(filename, string_types):
            warn(DeprecationWarning('The filename support for file objects passed to send_file is now deprecated.  Pass an attach_filename if you want mimetypes to be guessed.'), stacklevel=2)
    if add_etags:
        warn(DeprecationWarning('In future flask releases etags will no longer be generated for file objects passed to the send_file function because this behavior was unreliable.  Pass filenames instead if possible, otherwise attach an etag yourself based on another value'), stacklevel=2)
    if filename is not None:
        if not os.path.isabs(filename):
            filename = os.path.join(current_app.root_path, filename)
    if mimetype is None:
        if filename or attachment_filename:
        ...
        if filename is not None:
            try:
                rv.set_etag('flask-%s-%s-%s' % (
                 ...
                 adler32(filename.encode('utf-8') if isinstance(filename, text_type) else filename) & 4294967295))
            except OSError:
                warn('Access %s failed, maybe it does not exist, so ignore etags in headers' % filename, stacklevel=2)
    ...
    if loader is None or import_name == '__main__':
        ...
        if hasattr(loader, 'get_filename'):
            ...
            if hasattr(loader, 'is_package'):
                ...
                raise AttributeError('%s.is_package() method is missing but is required by Flask of PEP 302 import hooks' % loader.__class__.__name__)
...
class locked_cached_property(object):
    __doc__ = 'A decorator that converts a function into a lazy property.  The\n    function wrapped is called the first time to retrieve the result\n    and then that calculated result is used the next time you access\n    the value.  Works like the one in Werkzeug but has a lock for\n    thread safety.\n    '
...
class _PackageBoundObject(object):
    ...
    del _get_static_folder
    del _set_static_folder
    ...
    del _get_static_url_path
    del _set_static_url_path
