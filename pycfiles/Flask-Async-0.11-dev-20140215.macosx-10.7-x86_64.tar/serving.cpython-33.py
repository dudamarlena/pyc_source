...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/serving.py
# Compiled at: 2014-01-20 16:48:35
# Size of source mod 2**32: 27481 bytes
"""
    werkzeug.serving
    ~~~~~~~~~~~~~~~~
...
import os, socket, sys, time, signal, subprocess
...
from werkzeug._compat import iteritems, PY2, reraise, text_type, wsgi_encoding_dance
from werkzeug.urls import url_parse, url_unquote
from werkzeug.exceptions import InternalServerError, BadRequest

class WSGIRequestHandler(BaseHTTPRequestHandler, object):
    __doc__ = 'A request handler that implements WSGI dispatching.'
...
class _SSLConnectionFix(object):
    __doc__ = 'Wrapper around SSL connection to provide a working makefile().'
...
class BaseWSGIServer(HTTPServer, object):
    __doc__ = 'Simple single-threaded, single-process WSGI server.'
    multithread = False
    multiprocess = False
...
class ThreadedWSGIServer(ThreadingMixIn, BaseWSGIServer):
    __doc__ = 'A WSGI server that does threading.'
    multithread = True


class ForkingWSGIServer(ForkingMixIn, BaseWSGIServer):
    __doc__ = 'A WSGI server that does forking.'
    multiprocess = True
...
def make_server(host, port, app=None, threaded=False, processes=1, request_handler=None, passthrough_errors=False, ssl_context=None, loop=None):
...
def run_simple(hostname, port, application, use_reloader=False, use_debugger=False, use_evalex=True, extra_files=None, reloader_interval=1, threaded=False, processes=1, request_handler=None, static_files=None, passthrough_errors=False, ssl_context=None):
    ...
    def inner(loop, _):
        make_server(hostname, port, application, threaded, processes, request_handler, passthrough_errors, ssl_context, loop)
