...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/_compat.py
# Compiled at: 2014-01-20 15:46:16
# Size of source mod 2**32: 6184 bytes
...
if PY2:
    ...
    int_to_byte = chr
    iterkeys = lambda *args, d, **args: d.iterkeys(*args, **kwargs)
    itervalues = lambda *args, d, **args: d.itervalues(*args, **kwargs)
    iteritems = lambda *args, d, **args: d.iteritems(*args, **kwargs)
    iterlists = lambda *args, d, **args: d.iterlists(*args, **kwargs)
    iterlistvalues = lambda *args, d, **args: d.iterlistvalues(*args, **kwargs)
    ...
    def fix_tuple_repr(obj):
        ...
        def __repr__(self):
            ...
            return '%s(%s)' % (cls.__name__,
             ', '.join('%s=%r' % (field, self[index]) for index, field in enumerate(cls._fields)))
    ...
    string_types = (str,)
    integer_types = (int,)
    iterkeys = lambda *args, d, **args: iter(d.keys(*args, **kwargs))
    itervalues = lambda *args, d, **args: iter(d.values(*args, **kwargs))
    iteritems = lambda *args, d, **args: iter(d.items(*args, **kwargs))
    iterlists = lambda *args, d, **args: iter(d.lists(*args, **kwargs))
    iterlistvalues = lambda *args, d, **args: iter(d.listvalues(*args, **kwargs))
