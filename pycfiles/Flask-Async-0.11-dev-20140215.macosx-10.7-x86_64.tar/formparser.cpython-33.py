...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/formparser.py
# Compiled at: 2014-01-20 15:46:16
# Size of source mod 2**32: 21127 bytes
"""
    werkzeug.formparser
    ~~~~~~~~~~~~~~~~~~~
...
class FormDataParser(object):
    __doc__ = 'This class implements parsing of form data for Werkzeug.  By itself\n    it can parse multipart and url encoded form data.  It can be subclassed\n    and extended but for most mimetypes it is a better idea to use the\n    untouched stream and expose it as separate attributes on a request\n    object.\n\n    .. versionadded:: 0.8\n\n    :param stream_factory: An optional callable that returns a new read and\n                           writeable file descriptor.  This callable works\n                           the same as :meth:`~BaseResponse._get_file_stream`.\n    :param charset: The character set for URL and url encoded form data.\n    :param errors: The encoding error behavior.\n    :param max_form_memory_size: the maximum number of bytes to be accepted for\n                           in-memory stored form data.  If the data\n                           exceeds the value specified an\n                           :exc:`~exceptions.RequestEntityTooLarge`\n                           exception is raised.\n    :param max_content_length: If this is provided and the transmitted data\n                               is longer than this value an\n                               :exc:`~exceptions.RequestEntityTooLarge`\n                               exception is raised.\n    :param cls: an optional dict class to use.  If this is not specified\n                       or `None` the default :class:`MultiDict` is used.\n    :param silent: If set to False parsing errors will not be caught.\n    '
    ...
    def _parse_urlencoded(self, stream, mimetype, content_length, options):
        if self.max_form_memory_size is not None:
            if content_length is not None and content_length > self.max_form_memory_size:
                raise exceptions.RequestEntityTooLarge()
        form = url_decode_stream(stream, self.charset, errors=self.errors, cls=self.cls)
        return (stream, form, self.cls())

    parse_functions = {'multipart/form-data': _parse_multipart, 
...
class MultiPartParser(object):
    ...
    def __init__(self, stream_factory=None, charset='utf-8', errors='replace', max_form_memory_size=None, cls=None, buffer_size=65536):
        self.stream_factory = stream_factory
        self.charset = charset
        self.errors = errors
        self.max_form_memory_size = max_form_memory_size
    ...
    def start_file_streaming(self, filename, headers, total_content_length):
        ...
        container = self.stream_factory(total_content_length, content_type, filename, content_length)
    ...
    def parse_lines(self, file, boundary, content_length):
        ...
        if terminator == last_part:
            ...
            while terminator != last_part:
                ...
                if filename is None:
                    yield (
                     _begin_form, (headers, name))
                else:
                    yield (
                     _begin_file, (headers, name, filename))
                buf = b''
                for line in iterator:
                    if not line:
            ...
            if ellt == _begin_file:
                ...
                if is_file:
                    ...
                    yield ('file',
                     (
                      name,
                      FileStorage(container, filename, name, headers=headers)))
                else:
    ...
    def parse(self, file, boundary, content_length):
        formstream, filestream = tee(self.parse_parts(file, boundary, content_length), 2)
