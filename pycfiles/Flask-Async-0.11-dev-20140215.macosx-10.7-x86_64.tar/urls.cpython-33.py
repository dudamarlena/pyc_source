...
# [GCC 8.4.0]
# Embedded file name: /Users/mrdon/dev/flask-async/venv/lib/python3.3/site-packages/werkzeug/urls.py
# Compiled at: 2014-01-20 15:46:16
# Size of source mod 2**32: 31741 bytes
"""
    werkzeug.urls
    ~~~~~~~~~~~~~
...
"""
import re
from werkzeug._compat import text_type, PY2, to_unicode, to_native, implements_to_string, try_coerce_native, normalize_string_tuple, make_literal_wrapper, fix_tuple_repr
from werkzeug._internal import _encode_idna, _decode_idna
from werkzeug.datastructures import MultiDict, iter_multi_items
from collections import namedtuple
_scheme_re = re.compile('^[a-zA-Z0-9+-.]+$')
_always_safe = b'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_.-+'
_hexdigits = '0123456789ABCDEFabcdef'
_hextobyte = dict(((a + b).encode(), int(a + b, 16)) for a in _hexdigits for b in _hexdigits)
...
class URL(_URLTuple, _URLMixin):
    __doc__ = 'Represents a parsed URL.  This behaves like a regular tuple but\n    also has some extra attributes that give further insight into the\n    URL.\n    '
    __slots__ = ()
    _at = '@'
    _colon = ':'
    _lbracket = '['
    ...
    def encode(self, charset='utf-8', errors='replace'):
        ...
        """
        return BytesURL(self.scheme.encode('ascii'), self.encode_netloc(), self.path.encode(charset, errors), self.query.encode(charset, errors), self.fragment.encode(charset, errors))


class BytesURL(_URLTuple, _URLMixin):
    __doc__ = 'Represents a parsed URL in bytes.'
    __slots__ = ()
    _at = b'@'
    _colon = b':'
    _lbracket = b'['
...
def url_parse(url, scheme=None, allow_fragments=True):
    ...
    i = url.find(s(':'))
    if i > 0:
        if _scheme_re.match(to_native(url[:i], errors='replace')):
            rest = url[i + 1:]
    ...
    if url[:2] == s('//'):
        ...
        if s('[') in netloc and s(']') not in netloc or s(']') in netloc and s('[') not in netloc:
...
def _url_decode_impl(pair_iter, charset, decode_keys, include_empty, errors):
    for pair in pair_iter:
        ...
        yield (
         key, url_unquote_plus(value, charset, errors))
