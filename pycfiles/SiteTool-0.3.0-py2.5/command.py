...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sitetool/command.py
# Compiled at: 2009-07-30 07:51:55
...
"""
import os.path, logging, sys, getopt, subprocess, math
...
def handle_command_create(option_sets, command_options, args):
    r"""    usage: ``%(program)s create [OPTION]... TEMPLATE PAGE``
...
def handle_command_list(option_sets, command_options, args):
    r"""    usage: ``%(program)s list [OPTIONS]... [PAGE|TEMPLATE]``
...
def handle_command_get(option_sets, command_options, args):
    r"""    usage: ``%(program)s get [OPTIONS]... [PAGE|TEMPLATE]``
    ...
    if not command_options.has_key('area'):
        raise getopt.GetoptError('No AREA to get specified. Use the -a option.')
    else:
...
def handle_command_set(option_sets, command_options, args):
    r"""    usage: ``%(program)s set [OPTIONS]... [PAGE|TEMPLATE]``
    ...
    if not command_options.has_key('area'):
        ...
        if len(area_options) > 1:
            raise getopt.GetoptError('Only one --area or -a option is allowed')
        area = area_options[0]['value']
    if command_options.has_key('content') and command_options.has_key('interactive'):
        raise getopt.GetoptError('The %s and %s options cannot be used together' % (
         command_options['content'][0]['name'],
         command_options['interactive'][0]['name']))
    if not command_options.has_key('content') and not command_options.has_key('interactive'):
        log.info('Reading from STDIN (press Ctrl+C to abort, Press Ctrl+D twice to save)')
        try:
        ...
        if command_options.has_key('content'):
            if len(command_options['content']) > 1:
                raise getopt.GetoptError('Only one --content or -e option is allowed')
...
def handle_command_convert(option_sets, command_options, args):
    r"""    usage: ``%(program)s convert [OPTIONS]... [PLUGIN_OPTIONS]...``
    ...
    if command_options.has_key('config'):
        internal_vars['config'] = parse_html_config(command_options['config'][0]['value'])
    ...
    if command_options.has_key('template'):
        ...
        internal_vars['config']['DEFAULT_TEMPLATE'] = uniform_path(command_options['template'][0]['value'])
    ...
    if command_options.has_key('site_root'):
        ...
        site_root = path_from_config(command_options['config'][0]['value'], internal_vars['config']['SITE_ROOT'])
    else:
        raise getopt.GetoptError('No SITE_ROOT specified')
    if command_options.has_key('start_path'):
        internal_vars['start_path'] = uniform_path(command_options['start_path'][0]['value'])
    ...
    import sitetool.convert.rst, sitetool.convert.blog, gallery.plugin, sitetool.convert.suunto
    plugin_classes = [
     (
...
def handle_command_update(option_sets, command_options, args):
    r"""    usage: ``%(program)s update [OPTIONS]...``
    ...
    if command_options.has_key('site_root'):
        internal_vars['site_root'] = uniform_path(command_options['site_root'][0]['value'])
    elif config.has_key('SITE_ROOT'):
        internal_vars['site_root'] = path_from_config(command_options['config'][0]['value'], config['SITE_ROOT'])
    if command_options.has_key('start_path'):
        internal_vars['start_path'] = uniform_path(command_options['start_path'][0]['value'])
    if not internal_vars.has_key('start_path') and not internal_vars.has_key('site_root'):
    ...
    if not internal_vars.has_key('update_templates') and not internal_vars.has_key('update_context_items') and not internal_vars.has_key('update_dynamic_items') and not internal_vars.has_key('update_static_items'):
...
def handle_command_move(option_sets, command_options, args):
    r"""    usage: ``%(program)s move [OPTIONS]... SOURCE DEST``
...
def handle_command_deploy(option_sets, command_options, args):
    r"""    usage: ``%(program)s deploy [OPTIONS]...``
    ...
    """
    set_error_on(command_options, allowed=[
     'config', 'exclude_from', 'site_root', 'remote_scp', 'delete', 'dry_run'])
    if args:
        raise getopt.GetoptError('Unexpected argument %r' % args[0])
    internal_vars = {}
    config = {}
...
def handle_command_sitemap(option_sets, command_options, args):
    r"""    usage: ``%(program)s sitemap [OPTIONS]... SITEMAP``
    ...
    """
    set_error_on(command_options, allowed=[
     'config', 'site_root', 'format', 'template'])
    if len(args) < 1:
    ...
    if command_options.has_key('site_root'):
        ...
        raise getopt.GetoptError('No SITE_ROOT specified')
    if command_options.has_key('template'):
        internal_vars['template'] = uniform_path(command_options['template'][0]['value'])
    elif config.has_key('DEFAULT_TEMPLATE'):
        internal_vars['template'] = path_from_config(command_options['config'][0]['value'], config['DEFAULT_TEMPLATE'])
    else:
        raise getopt.GetoptError('No TEMPLATE specified')
...
def handle_command_broken(option_sets, command_options, args):
    r"""    usage: ``%(program)s broken [OPTIONS]...`` 
...
def handle_command_orphan(option_sets, command_options, args):
    r"""    usage: ``%(program)s orphan [OPTIONS]...``
    ...
    """
    set_error_on(command_options, allowed=[
     'config', 'relative', 'site_root', 'start_path'])
    if len(args):
        raise getopt.GetoptError('Unexpected argument %r' % args[0])
    internal_vars = {}
    config = {}
...
def handle_command_export(option_sets, command_options, args):
    r"""    usage: ``%(program)s export [OPTIONS]... [MYSQL_OPTIONS] DEST``
    ...
    if command_options.has_key('mode'):
        ...
        if not internal_vars['mode'] == 'wordpress-rst':
            raise getopt.GetoptError('Unsupported mode %r' % internal_vars['mode'])
    if command_options.has_key('site_root'):
        internal_vars['site_root'] = uniform_path(command_options['site_root'][0]['value'])
    elif config.has_key('SITE_ROOT'):
        internal_vars['site_root'] = path_from_config(command_options['config'][0]['value'], config['SITE_ROOT'])
        ...
        raise getopt.GetoptError('No SITE_ROOT specified')
    connect = {}
    if command_options.has_key('password') and command_options.has_key('interactive'):
        raise getopt.GetoptError('You cannot specify both %s and %s' % (
...
def handle_command_redirect(option_sets, command_options, args):
    r"""    usage: ``%(program)s redirect [OPTIONS]...``
...
def handle_command_resize(option_sets, command_options, args):
    r"""    usage: ``%(program)s resize [OPTIONS]... -z 800x600 -F png -o test.rst FILES...``
    ...
    """
    internal_vars = set_error_on(command_options, allowed=[
     'output',
     'format',
     'size'])
    if not args:
...
command_handler_factories = {'create': makeHandler(handle_command_create), 
...
option_sets = {'relative': [
              dict(type='command', long=[
               '--relative'], short=[])], 
   'exclude_from': [
                  dict(type='command', long=[
                   '--exclude-from'], short=[], metavar='EXCLUDE_FROM')], 
   'video_overlay_file': [
                        dict(type='command', long=[
                         '--video-overlay-file'], short=[
                         '-y'], metavar='OVERLAY_FILE')], 
   'gallery_thumb': [
                   dict(type='command', long=[
                    '--gallery-thumb'], short=[])], 
   'gallery_reduced': [
                     dict(type='command', long=[
                      '--gallery-reduced'], short=[])], 
   'gallery_exif': [
                  dict(type='command', long=[
                   '--gallery-exif'], short=[])], 
   'gallery_h264': [
   ...
   'config': [
            dict(type='command', long=[
             ...
             '-c'], metavar='CONFIG')], 
   'help': [
          dict(type='shared', long=[
           '--help'], short=[])], 
   'version': [
             dict(type='program', long=[
              '--version'], short=[])], 
   'verbose': [
             dict(type='program', long=[
              '--verbose'], short=[
