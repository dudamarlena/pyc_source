...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sitetool/convert/suunto.py
# Compiled at: 2009-05-11 20:06:01
"""
...
import logging, urllib2, zipfile, logging, re, os, sys
    ...
    def on_file(self, path):
        ...
        for file in sde.infolist():
            if file.orig_filename.endswith('.xml'):
                ...
                dive = Dive(dom)
                log.debug('Extracted dive %s, %d properties, %d data points', dive.properties['DIVENUMBER'], len(dive.properties), len(dive.samples))
                dest_page = os.path.join(os.path.split(path)[0], 'log_%s.suunto.html' % dive.properties['DIVENUMBER'])
                if os.path.exists(dest_page) and not os.stat(path).st_mtime > os.stat(dest_page).st_mtime and not self.force:
                ...
                if os.path.exists(dest_chart) and not os.stat(path).st_mtime > os.stat(dest_chart).st_mtime and not self.force:
