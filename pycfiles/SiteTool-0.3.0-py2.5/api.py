...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sitetool/api.py
# Compiled at: 2009-07-29 06:54:24
...
def convert(site_root, plugin_classes, config=None, recursive=False, force=False, user_files=False, generated_files=False, plugins=None, start_path=None, update_context_items=False):
    ...
    if not start_path.startswith(site_root):
        raise TemplateError('Path %r is not under site root %r' % (start_path, site_root))
    source = start_path
    if plugins is None:
        plugins = []
        for (name, plugin_class) in plugin_classes:
            plugin = plugin_class(config=config, site_root=site_root, user_files=user_files, generated_files=generated_files, force=force)
    ...
    if os.path.isdir(source):
        if recursive:
            ...
            for directory in directories:
                convert(site_root, plugin_classes, start_path=directory, config=config, recursive=False, force=force, user_files=user_files, generated_files=generated_files, plugins=plugins, update_context_items=False)
    ...
    if params:
        update(start_path, site_root, recursive=True, **params)
    return
...
def update(start_path=None, site_root=None, recursive=False, update_templates=False, update_context_items=False, update_dynamic_items=False, update_static_items=False):
    ...
    """
    if not site_root:
        raise TemplateError('No site root specified')
    if start_path is None:
        start_path = site_root
    if not os.path.exists(os.path.join(site_root, 'Templates')):
        raise TemplateError("Path %r does not appear to be a vaild site root, it has no 'Templates' directory" % site_root)
    if not start_path.startswith(site_root):
        raise TemplateError('Path %r is not under site root %r' % (start_path, site_root))
    if update_templates is False and update_context_items is False and update_dynamic_items is False and update_static_items is False:
        raise TemplateError('Nothing to update. Expected one of update_templates, update_context_items, update_dynamic_items or update_static_items to be True.')
    ...
    if os.path.isdir(start_path):
                    ...
                    if filename.endswith('.html') or filename.endswith('.dwt'):
                        ...
                        try:
                            if filename.endswith('.dwt'):
                                update(start_path=path, site_root=site_root, recursive=recursive, update_templates=False, update_context_items=update_context_items, update_dynamic_items=update_dynamic_items, update_static_items=update_static_items)
                            else:
                                update(start_path=path, site_root=site_root, recursive=recursive, update_templates=update_templates, update_context_items=update_context_items, update_dynamic_items=update_dynamic_items, update_static_items=update_static_items)
                            passed += 1
        ...
        if update_templates:
            ...
            reapply_template_to_page(start_path, delete_missing_sections=[])
        if update_context_items or update_dynamic_items or update_static_items:
            log.debug('Re-applying library items to %r...', start_path)
            reapply_library_items_to_page(start_path, site_root, update_context_items=update_context_items, update_dynamic_items=update_dynamic_items, update_static_items=update_static_items)
    return
...
def move(src, dst, site_root, update_other_links=False, update_context_items=False):
    ...
    if os.path.isdir(src):
        ...
        for (dirpath, dirnames, filenames) in os.walk(src):
            ...
            for filename in filenames:
                for cf in move_file(os.path.join(dirpath, filename), os.path.join(new_dirpath, filename), site_root, update_other_links):
...
def sitemap(sitemap_path, site_root, template):
    ...
    if not sitemap_path.startswith(site_root):
        raise TemplateError('Sitemap %r is not under site root %r' % (sitemap_path, site_root))
    ...
    for (dirpath, dirnames, filenames) in os.walk(site_root):
        ...
        if 'index.html' in filenames:
            ...
            current_path = handle_sitemap_entry(site_root, content, dirpath, 'index.html', current_path)
            filenames.pop(filenames.index('index.html'))
            content.append('</div>')
        for filename in filenames:
            if filename.endswith('.html'):
                current_path = handle_sitemap_entry(site_root, content, dirpath, filename, current_path)
    ...
    reapply_library_items_to_page(sitemap_path, site_root=site_root, update_context_items=True)
    return
...
def export():
    ...
    connect['db'] = args[1]
    (posts, tags) = export(connect, options.verbose)
