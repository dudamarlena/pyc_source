...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sitetool/template/dreamweaver.py
# Compiled at: 2009-07-30 16:56:45
...
"""
import os.path, logging, os, re, shutil, posixpath
...
class DreamweaverTemplateInstance(object):
    ...
    def __init__(self, filename=None, template=None, version=4):
        ...
        if filename and template:
            raise TemplateError('You cannot specify both a file and template text to be used.')
        self.page_regions = {}
        ...
        (self.instance_type, template_path) = determine_instance_type(self.template, self.version)
        if template_path is not None:
            self.template_path = os.path.abspath(os.path.join(os.path.dirname(filename), template_path))
        elif self.filename:
            self.template_path = os.path.abspath(self.filename)
        else:
            self.template_path = None
        if self.version == 4:
            name_regex = re.compile('<!--\\s*?' + self.instance_type.capitalize() + 'BeginEditable\\s*?name=\\"(\\W*\\s*.*\\w*)?\\"\\s*?-->')
            content_regex = re.compile('<!--\\s*?' + self.instance_type.capitalize() + 'EndEditable\\s*?-->')
        else:
            name_regex = re.compile('<!--(\\s*?)#BeginEditable(\\s*?)"(\\W*\\s*.*\\w*)?\\"(\\s*?)-->')
            content_regex = re.compile('<!--\\s*?' + '#EndEditable\\s*?-->')
        pos = 0
        prev_match = ''
        while 1:
            ...
            content_match = content_regex.search(self.template, begin)
    ...
    def get(self, key):
        ...
        if self.page_regions.has_key(key):
            ...
            raise TemplateError('Error, %r is not an editable region the template.' % key)
    ...
    def save_as_page(self, filename=None, template_path=None, old_path=None, new_path=None, tidy=False):
        ...
        if self.instance_type == 'instance':
            if not old_path:
                ...
                if not old_path:
                    raise TemplateError("Please specify 'old_path', the full path and filename the page was loaded from.")
            if not new_path:
                new_path = filename or self.filename
                if not new_path:
                    raise TemplateError("Please specify 'new_path', the full path and filename the page should be saved to.")
            if not template_path:
                template_path = self.template_path
                if not template_path:
                    raise TemplateError("Please specify 'template_path', the full path and filename of the template the page uses.")
        else:
            if old_path:
                raise TemplateError("You shouldn't specify 'old_path' when the template instance was derived from a template because the 'template_path' is used instead")
            if not template_path:
                template_path = self.template_path
                if not template_path:
                    raise TemplateError("Please specify 'template_path', the full path and filename of the original template")
            old_path = template_path
            if not new_path:
                new_path = filename
                if not filename:
                    raise TemplateError("Please specify 'new_path', the full path and filename of the new page to be created")
        output = []
        first = True
        site_root = os.path.sep.join(os.path.split(os.path.dirname(template_path))[:-1])
        for (k, end, template_region, start) in self.template_regions:
            template_region = update_links(site_root=site_root, old_path=old_path, new_path=new_path, content=template_region)
            if self.instance_type == 'template':
                if first:
                    first = False
                    pos = end_of_start_html_tag(template_region)
                    rel = relpath(self.template_path, posixpath.dirname(new_path))
                    template_region = '%s%s%s' % (
                     template_region[:pos],
                     '<!-- InstanceBegin template="' + rel + '" codeOutsideHTMLIsLocked="false" -->',
                     template_region[pos:])
                    output.append(template_region)
                    output.append(start.replace('TemplateBeginEditable', 'InstanceBeginEditable'))
                    if k in self.updated_regions:
                        output.append(self.page_regions[k])
                    else:
                        output.append(update_links(site_root=site_root, old_path=old_path, new_path=new_path, content=self.page_regions[k]))
                elif k is not None:
                    output.append(end.replace('TemplateEndEditable', 'InstanceEndEditable'))
                    output.append(template_region)
                    output.append(start.replace('TemplateBeginEditable', 'InstanceBeginEditable'))
                    if k in self.updated_regions:
                        output.append(self.page_regions[k])
...
def determine_instance_type(template, version):
    ...
    """
    if version == 4:
        regex = re.compile('<!--\\s*?InstanceBegin\\s*?template\\s*?=\\s*?("|\')(.*?)("|\').*?-->')
    else:
        regex = re.compile('<!--\\s*?#BeginTemplate\\s*?("|\')(.*?)("|\').*?-->')
...
def end_of_start_html_tag(template):
    start_html_regex = re.compile('<\\s*?html [^<]*?>', re.IGNORECASE)
    start_html_match = start_html_regex.search(template)
    if not start_html_match:
        raise TemplateError('No <html> tag could be found in the template.')
    return start_html_match.end(0)


def start_of_end_html_tag(template):
    end_html_regex = re.compile('</\\s*?html\\s*?>', re.IGNORECASE)
    end_html_match = end_html_regex.search(template)
...
def tidy_output(output):
    if not tidy_available:
        raise TemplateError("The 'tidy' module from uTidyLib is not available")
    options = dict(output_xhtml=1, add_xml_decl=1, indent=1, tidy_mark=0, wrap=78)
    output = tidylib.parseString(output.encode('utf-8'), **options)
    return output


href_and_src_pattern = '<([^>]*?)(href|src)(\\s*?)=(\\s*?)("|\')(.*?)("|\')(\\s*?.*?)>'
...
def update_links(site_root, old_path, new_path, content):
    ...
    if not old_path.startswith(site_root):
        raise TemplateError('The old_path (%r) is not under site_root (%r).' % (
         old_path,
         site_root))
    elif not new_path.startswith(site_root):
        raise TemplateError('The new_path (%r) is not under site_root (%r).' % (
         new_path,
         site_root))

    def get_new_link(old_path, old_link, new_path):
        link_path = posixpath.abspath(posixpath.join(posixpath.dirname(old_path), old_link))
        new_link = relpath(link_path, posixpath.dirname(new_path))
        if (old_link == '.' or old_link.endswith('/')) and not new_link.endswith('/'):
            new_link += '/'
        return new_link
    ...
    (content, replaced_links) = re.subn(href_and_src_pattern, href_and_src_replace, content)
    library_item_pattern = '<!--(\\s*?)#Begin(ContextItem|LibraryItem|DynamicItem)(\\s*?)"(\\W*\\s*.*\\w*)?\\"(\\s*?)-->'
    ...
    (content, replaced_links) = re.subn(library_item_pattern, library_item_replace, content)
    return content
...
def reapply_template_to_pages(template_path, pages, delete_missing_sections=[]):
    ...
    for page_filename in pages:
        ...
        if not page.template_path == template.template_path:
            raise TemplateError('The page %r uses the template %r not %r' % (
             page_filename,
             page.template_path,
...
def update_library_item_links_for_path(library_item_filename, destination_filename):
    ...
    result = update_links(site_root=site_root, old_path=old_path, new_path=new_path, content=content)
    return result
...
def extract_title(filename, full=False):
    ...
    fp.close()
    section_property_regex = re.compile('<!--\\s*?InstanceBeginEditable\\s*?name="section_property"\\s*?--><!--\\s*?(.*?)\\s*?--><!--\\s*?InstanceEndEditable\\s*?-->')
    ...
    if section_property_match:
        ...
        if not title_match:
            title = ('.').join(filename.split(os.sep)[(-1)].split('.')[:-1]).replace('-', ' ').capitalize()
        else:
...
def update_context_item_links_for_path(context_item_filename, destination_filename, site_root=None):
    ...
    if type == 'section_navigation.cti':
        ...
        for file_ in files:
            ...
            elif os.path.isdir(os.path.join(dir, file_)):
        ...
        site_index = os.path.join(site_root, 'index.html')
...
def reapply_library_items_to_page(page, site_root=None, update_context_items=False, update_dynamic_items=False, update_static_items=False):
    ...
    if not library_types:
        raise TemplateError('You have not chosen which types of library item in %r to update' % page)
    ...
    start_regex = re.compile('<!--\\s*?#Begin(%s)\\s*?"(\\W*\\s*.*\\w*)?\\"\\s*?-->' % regex_string)
    end_regex = re.compile('<!--\\s*?#End(%s)\\s*?-->' % regex_string)
    pos = 0
    while 1:
        ...
        end_match = end_regex.search(data, end_of_start)
        ...
        if not updated_library_items.has_key(path):
            realpath = os.path.normpath(os.path.join(os.path.dirname(filename), path))
            if library_item_type in ('DynamicItem', 'LibraryItem'):
                updated_library_items[path] = update_library_item_links_for_path(library_item_filename=realpath, destination_filename=filename)
            elif library_item_type in ('ContextItem', ):
                updated_library_items[path] = update_context_item_links_for_path(context_item_filename=realpath, destination_filename=filename, site_root=site_root)
            else:
...
def replace_link(filename, src, dst):
    ...
    data = fp.read().decode('utf-8')
    fp.close()
    href_and_src_pattern = '<([^>]*?)(href|src)(\\s*)=(\\s*)("|\')%s("|\')(\\s*.*?)>' % re.escape(cur_link)
    ...
    (data, replaced_links) = re.subn(href_and_src_pattern, href_and_src_replace, data)
    if replaced_links:
        fp = open(filename, 'w')
...
def reapply_template_to_page(page, delete_missing_sections=[]):
    ...
    data = fp.read().decode('utf-8')
    fp.close()
    template_regex = re.compile('<!--\\s*?InstanceBegin\\s*?template\\s*?=\\s*?"(.*?)\\"[^>]*?-->')
...
def move_file(src, dst, site_root, update_other_links=False):
    ...
    if src.endswith('.html'):
        data = update_links(site_root=site_root, old_path=src, new_path=dst, content=data)
            ...
            for (dirpath, dirnames, filenames) in os.walk(site_root):
                for filename in filenames:
                    if filename.endswith('.html'):
                        ...
                        replace_link(filename=path, src=src, dst=dst)
