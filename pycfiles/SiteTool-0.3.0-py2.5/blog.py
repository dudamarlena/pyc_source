...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sitetool/convert/blog.py
# Compiled at: 2009-07-30 16:02:11
"""Manage a blog based on reStructuredText files"""
import datetime, logging, os.path, re
...
class BlogPlugin(Plugin):
    ...
    def on_file(self, path, page=None, template=None):
        ...
        if os.path.exists(html_version):
            if not os.stat(path).st_mtime > os.stat(html_version).st_mtime:
                if not self.force:
            ...
            tag_r or log.warning('No tags found for %s' % path)
        else:
...
tags_regex = re.compile('<th class="docinfo-name">Tags:</th><td class="field-body">(.*?)</td>')
...
def extract_tags(content):
    ...
    if tags_match:
        ...
        return (
         result, tags_match.start(1), tags_match.end(1))
    else:
        return ([], None, None)
    return
...
def build_index(base):
    ...
    tags = {}
    tag_paths = {}
    posted_regex = re.compile('<th class="docinfo-name">Posted:</th><td class="field-body">([0-9: \\-]*?)</td>')
...
def build_index_pages(years, tags, tag_paths, base, template, site_root, config):
    ...
    for (tag, v) in tags.items():
        ...
        v.reverse()
        for (date, item) in v:
            content.append('<li><a href="../%s/%s">%s</a> (posted %s)</li>' % (
             date.year,
    ...
    for (year, v) in years.items():
        ...
        v.reverse()
        for (date, item) in v:
            content.append('<li><a href="%s">%s</a> (posted %s)</li>' % (
             item['filename'][:-5] + '.html',
             item['title'],
             date.strftime('%Y-%m-%d %H:%M')))
