...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/samples/amazon_kclpy_helper.py
...
def get_kcl_app_command(java, multi_lang_daemon_class, properties, paths=[]):
    ...
    """
    return ('{java} -cp {cp} {daemon} {props}').format(java=java, cp=get_kcl_classpath(properties, paths), daemon=multi_lang_daemon_class, props=os.path.basename(properties))


if __name__ == '__main__':
    parser = argparse.ArgumentParser('A script for generating a command to run an Amazon KCLpy app')
    parser.add_argument('--print_classpath', dest='print_classpath', action='store_true', default=False, help='Print a java class path.\noptional arguments: --path')
    parser.add_argument('--print_command', dest='print_command', action='store_true', default=False, help='Print a command for running an Amazon KCLpy app.\nrequired ' + 'args: --java --properties\noptional args: --classpath')
    parser.add_argument('-j', '--java', dest='java', help='The path to the java executable e.g. <some root>/jdk/bin/java', metavar='PATH_TO_JAVA')
    parser.add_argument('-p', '--properties', '--props', '--prop', dest='properties', help='The path to a properties file (relative to where you are running this script)', metavar='PATH_TO_PROPERTIES')
    parser.add_argument('--sample', '--sample-props', '--use-sample-properties', dest='use_sample_props', help='This will use the sample.properties file included in this package as the properties file.', action='store_true', default=False)
    parser.add_argument('-c', '--classpath', '--path', dest='paths', action='append', default=[], help='Additional path to add to java class path. May be specified any number of times', metavar='PATH')
