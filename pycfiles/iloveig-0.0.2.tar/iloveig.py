...
# [GCC 8.4.0]
# Embedded file name: /Users/xwaynec/Projects/website/python/iloveig/iloveig/iloveig.py
# Compiled at: 2017-08-15 05:14:14
import os, re, json, sys, time, platform, requests
...
def find_query_id(soup):
    ...
    for qid in query_ids:
        try:
            response = requests.get(url='https://www.instagram.com/graphql/query/', params={'first': '12', 
               'after': end_cursor, 
               'id': user_id, 
               'query_id': qid}, headers={'cookie': 'csrftoken=' + csrf_token, 
...
def iloveig(url):
    ...
    r = requests.get(url=url, headers={'Accept-Language': 'zh-TW,zh;q=0.8,en-US;q=0.6,en;q=0.4'})
    ...
    while has_next_page:
        try:
            ...
            response = requests.get(url='https://www.instagram.com/graphql/query/', params={'first': '12', 
               'after': end_cursor, 
               'id': user_id, 
               'query_id': query_id}, headers={'cookie': 'csrftoken=' + csrf_token, 
               'x-requested-with': 'XMLHttpRequest', 
               'Accept-Language': 'zh-TW,zh;q=0.8,en-US;q=0.6,en;q=0.4', 
               'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1'})
            obj = json.loads(response.text)
