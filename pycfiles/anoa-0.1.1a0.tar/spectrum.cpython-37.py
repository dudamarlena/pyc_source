...
# Embedded file name: build/bdist.linux-x86_64/egg/ann_solo/spectrum.py
# Compiled at: 2019-08-06 14:15:07
# Size of source mod 2**32: 8264 bytes
import functools, math, mmh3, numba as nb, numpy as np
...
def process_spectrum(spectrum: MsmsSpectrum, is_library: bool) -> MsmsSpectrum:
    ...
    if spectrum.is_processed:
        ...
        if config.remove_precursor:
            ...
            if not _check_spectrum_valid(spectrum.mz, min_peaks, min_mz_range):
                ...
                spectrum.is_processed = True
                return spectrum
        spectrum = spectrum.filter_intensity(config.min_intensity, config.max_peaks_used_library if is_library else config.max_peaks_used)
        spectrum.is_valid = _check_spectrum_valid(spectrum.mz, min_peaks, min_mz_range) or False
        spectrum.is_processed = True
        return spectrum
    ...
    if scaling is not None:
        spectrum = spectrum.scale_intensity(scaling,
          max_rank=(config.max_peaks_used_library if is_library else config.max_peaks_used))
...
def get_dim(min_mz, max_mz, bin_size):
    ...
    return (
     round((end_dim - start_dim) / bin_size), start_dim, end_dim)
...
def spectrum_to_vector(spectrum: MsmsSpectrum, min_mz: float, max_mz: float, bin_size: float, hash_len: int, norm: bool=True, vector: np.ndarray=None) -> np.ndarray:
    """
...
class SpectrumSpectrumMatch:
    ...
    def __init__(self, query_spectrum: MsmsSpectrum, library_spectrum: MsmsSpectrum=None, search_engine_score: float=math.nan, q: float=math.nan, num_candidates: int=0):
