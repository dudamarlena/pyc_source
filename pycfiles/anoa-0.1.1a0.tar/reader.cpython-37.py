...
# Embedded file name: build/bdist.linux-x86_64/egg/ann_solo/reader.py
# Compiled at: 2019-08-06 14:15:07
# Size of source mod 2**32: 11493 bytes
import collections, logging, os, pickle
from functools import lru_cache
from typing import Iterator
from typing import List
from typing import Tuple
import joblib, numpy as np, pandas as pd, tqdm
...
class SpectralLibraryReader:
    ...
    def __init__(self, filename: str, config_hash: str=None) -> None:
        ...
        if not os.path.isfile(config_filename):
            ...
            config_lib_filename, self.spec_info, load_hash = joblib.load(config_filename)
            if config_lib_filename != os.path.basename(self._filename):
                do_create = True
                logging.warning('The configuration corresponds to a different file format of this spectral library')
    ...
    def _get_config_filename(self) -> str:
        ...
        if self._config_hash is not None:
            return f"{os.path.splitext(self._filename)[0]}_{self._config_hash[:7]}.spcfg"
    ...
    def _create_config(self) -> None:
        ...
        """
        logging.info('Create the spectral library configuration for file %s', self._filename)
        self.is_recreated = True
        temp_info = collections.defaultdict(lambda : {'id':[],  'precursor_mz':[]})
        offsets = {}
        with self as (lib_reader):
            for spectrum, offset in tqdm.tqdm((lib_reader.get_all_spectra()), desc='Library spectra read', unit='spectra'):
        ...
        self.spec_info = {'charge':{charge:{'id':np.asarray(charge_info['id'], np.uint32),  'precursor_mz':np.asarray(charge_info['precursor_mz'], np.float32)} for charge, charge_info in temp_info.items()}, 
         'offset':offsets}
        config_filename = self._get_config_filename()
    ...
    def get_spectrum(self, spec_id: int, process_peaks: bool=False) -> MsmsSpectrum:
        """
        ...
        Spectrum
            ...
            identifier.
        """
        spectrum = self._parser.read_spectrum(self.spec_info['offset'][spec_id])[0]
...
def verify_extension(supported_extensions: List[str], filename: str) -> None:
    ...
    if ext.lower() not in supported_extensions:
        ...
        raise FileNotFoundError(f"Unrecognized file format (supported file formats: {', '.join(supported_extensions)})")
...
def read_mgf(filename: str) -> Iterator[MsmsSpectrum]:
    ...
    for i, mgf_spectrum in enumerate(mgf.read(filename)):
        ...
        spectrum = MsmsSpectrum(identifier, precursor_mz, precursor_charge, (mgf_spectrum['m/z array']),
...
def read_mztab_ssms(filename: str) -> pd.DataFrame:
    ...
    ssms = pd.read_csv(filename, sep='\t', header=skiplines, index_col='PSM_ID')
    ssms.drop('PSH', 1, inplace=True)
    ssms['opt_ms_run[1]_cv_MS:1002217_decoy_peptide'] = ssms['opt_ms_run[1]_cv_MS:1002217_decoy_peptide'].astype(bool)
