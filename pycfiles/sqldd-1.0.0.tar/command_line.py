...
# [GCC 8.4.0]
# Embedded file name: /Users/rkeilty/git/sql-data-dependency/sql_data_dependency/command_line.py
# Compiled at: 2017-02-06 22:27:10
...
class SqlDataDependencyCommand(object):
    ...
    @classmethod
    def parse_args(cls):
        parser = argparse.ArgumentParser(description='SQL Data Dependency Tool', epilog='One of either ([table][primary_key]) or [--json] is required.')
        ...
        parser.add_argument('--sqldump_table_defs', action='store_true', default=False, help='Dump _all_ table defs, even those without dependencies.  Useful for constructing skeleton DBs.')
    ...
    def process(cls):
        args = cls.parse_args()
        sql_data_dependency_manager = SqlDatabaseDependencyManager(server=args.server, database=args.database, username=args.username, password=args.password, port=args.port)
        ...
        if args.sqldump:
            dump = sql_data_dependency_manager.create_mysqldump(dependencies, dump_all_table_definitions=args.sqldump_table_defs)
