...
# Embedded file name: build/bdist.linux-x86_64/egg/pyogp/lib/client/region.py
...
class Region(object):
    ...
    def __init__(self, global_x=0, global_y=0, seed_capability_url=None, udp_blacklist=None, sim_ip=None, sim_port=None, circuit_code=None, agent=None, settings=None, message_handler=None, handle=None, events_handler=None):
        ...
        self.region_caps_list = [
         'ChatSessionRequest', 'CopyInventoryFromNotecard', 'DispatchRegionInfo', 'EstateChangeInfo', 'EventQueueGet', 'FetchInventory', 'WebFetchInventoryDescendents', 'FetchLib', 'FetchLibDescendents', 'GroupProposalBallot', 'HomeLocation', 'MapLayer', 'MapLayerGod', 'NewFileAgentInventory', 'ParcelPropertiesUpdate', 'ParcelVoiceInfoRequest', 'ProvisionVoiceAccountRequest', 'RemoteParcelRequest', 'RequestTextureDownload', 'SearchStatRequest', 'SearchStatTracking', 'SendPostcard', 'SendUserReport', 'SendUserReportWithScreenshot', 'ServerReleaseNotes', 'StartGroupProposal', 'UpdateAgentLanguage', 'UpdateGestureAgentInventory', 'UpdateNotecardAgentInventory', 'UpdateScriptAgent', 'UpdateGestureTaskInventory', 'UpdateNotecardTaskInventory', 'UpdateScriptTask', 'ViewerStartAuction', 'UntrustedSimulatorMessage', 'ViewerStats']
    ...
    def connect(self):
        ...
        self.message_manager = MessageManager(Host((self.sim_ip, self.sim_port)), self.message_handler, self.capabilities, self.settings)
    ...
    def send_LogoutRequest(self, agent_id, session_id):
        ...
        packet = Message('LogoutRequest', Block('AgentData', AgentID=agent_id, SessionID=session_id))
    ...
    def sendUseCircuitCode(self, circuit_code, session_id, agent_id):
        ...
        packet = Message('UseCircuitCode', Block('CircuitCode', Code=circuit_code, SessionID=session_id, ID=agent_id))
    ...
    def sendCompleteAgentMovement(self, agent_id, session_id, circuit_code):
        ...
        packet = Message('CompleteAgentMovement', Block('AgentData', AgentID=agent_id, SessionID=session_id, CircuitCode=circuit_code))
        self.message_manager.send_udp_message(packet, reliable=True)

    def sendUUIDNameRequest(self, agent_ids=[]):
        """ sends a packet requesting the name corresponding to a UUID """
        packet = Message('UUIDNameRequest', *[ Block('UUIDNameBlock', ID=agent_id) for agent_id in agent_ids ])
        self.message_manager.send_udp_message(packet)

    def sendAgentUpdate(self, AgentID, SessionID, BodyRotation=(0.0, 0.0, 0.0, 1.0), HeadRotation=(0.0, 0.0, 0.0, 1.0), State=0, CameraCenter=(0.0, 0.0, 0.0), CameraAtAxis=(0.0, 0.0, 0.0), CameraLeftAxis=(0.0, 0.0, 0.0), CameraUpAxis=(0.0, 0.0, 0.0), Far=0, ControlFlags=0, Flags=0):
        """ sends an AgentUpdate packet to *this* simulator"""
        packet = Message('AgentUpdate', Block('AgentData', AgentID=AgentID, SessionID=SessionID, BodyRotation=BodyRotation, HeadRotation=HeadRotation, State=State, CameraCenter=CameraCenter, CameraAtAxis=CameraAtAxis, CameraLeftAxis=CameraLeftAxis, CameraUpAxis=CameraUpAxis, Far=Far, ControlFlags=ControlFlags, Flags=Flags))
        self.enqueue_message(packet)

    def sendRegionHandshakeReply(self, AgentID, SessionID, Flags=0):
        """ sends a RegionHandshake packet """
        packet = Message('RegionHandshakeReply', Block('AgentData', AgentID=AgentID, SessionID=SessionID), Block('RegionInfo', Flags=Flags))
        self.message_manager.send_udp_message(packet, reliable=True)

    def sendCompletePingCheck(self, PingID):
        """ sends a CompletePingCheck packet """
        packet = Message('CompletePingCheck', Block('PingID', PingID=PingID))
        self.message_manager.send_udp_message(packet)
        self.last_ping += 1
    ...
    def handle_to_xy(handle):
        ...
        y = int((handle & 4294967295) / Region.WIDTH)
        return (
         x, y)
...
class RegionSeedCapability(Capability):
    ...
    def get(self, names=[], settings=None):
        ...
        if settings != None:
            ...
            self.settings = Settings()
        payload = names
        parsed_result = self.POST(payload)
        if self.settings.ENABLE_CAPS_LOGGING:
        ...
        for name in names:
            ...
            elif self.settings.ENABLE_CAPS_LOGGING:
