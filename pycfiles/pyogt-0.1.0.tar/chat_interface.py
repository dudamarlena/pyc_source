...
# Embedded file name: build/bdist.macosx-10.6-universal/egg/pyogp/apps/examples/chat_interface.py
...
class Login(object):
    ...
    def login(self, event=None):
        ...
        if start_location == '':
            start_location = None
        api.spawn(self.parent.login_agent, self.first_name.get(), self.last_name.get(), self.password.get(), start_location)
...
class Chat_window(object):
    ...
    def login_agent(self, firstname, lastname, password, start_location):
        self.agent.login('https://login.agni.lindenlab.com/cgi-bin/login.cgi', firstname=firstname, lastname=lastname, password=password, start_location=start_location, connect_region=True)
