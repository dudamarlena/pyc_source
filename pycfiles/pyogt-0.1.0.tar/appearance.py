...
# Embedded file name: build/bdist.linux-x86_64/egg/pyogp/lib/client/appearance.py
...
from pyogp.lib.client.enums import BakedIndex, TextureIndex, WearableMap, AssetType, WearablesIndex
...
class AppearanceManager(DataManager):
    ...
    def wear_item(self, item, wearable_type):
        ...
        self.send_AgentSetAppearance(self.agent.agent_id, self.agent.session_id, self.get_size(), self.bakedTextures, self.TextureEntry, self.visualParams)
        self.send_AgentIsNowWearing(self.agent.agent_id, self.agent.session_id, self.wearables)
    ...
    def request_agent_wearables(self):
        ...
        """
        if self.agent.agent_id == None or self.agent.session_id == None or str(self.agent.agent_id) == str(UUID()) or str(self.agent.session_id) == str(UUID()):
            logger.warning('Agent has either no agent_id or session_id, message not sent')
            return
        self.send_AgentWearablesRequest(self.agent.agent_id, self.agent.session_id)
        return
    ...
    def get_size(self):
        ...
        """
        height = 1.706 + self.visualParams[692].value * 0.1918 + self.visualParams[842].value * 0.0375 + self.visualParams[33].value * 0.12022 + self.visualParams[682].value * 0.01117 + self.visualParams[756].value * 0.038 + self.visualParams[198].value * 0.08 + self.visualParams[503].value * 0.07
        return Vector3(X=0.45, Y=0.6, Z=height)
    ...
    def wearableArrived(self, assetID, isSuccess):
        ...
        if isSuccess:
            ...
            self.send_AgentCachedTexture(self.agent.agent_id, self.agent.session_id, self.bakedTextures, self.wearables)
    ...
    def onAgentWearablesUpdate(self, packet):
        ...
        for wearableData in packet['WearableData']:
            ...
            if str(assetID) != '00000000-0000-0000-0000-000000000000':
                self.agent.asset_manager.request_asset(assetID, wearable.getAssetType(), True, self.wearableArrived)
    ...
    def send_AgentWearablesRequest(self, AgentID, SessionID):
        ...
        """
        packet = Message('AgentWearablesRequest', Block('AgentData', AgentID=AgentID, SessionID=SessionID))
    ...
    def send_AgentIsNowWearing(self, AgentID, SessionID, wearables):
        ...
        args += [ Block('WearableData', ItemID=wearables[key].ItemID, WearableType=wearables[key].WearableType) for key in wearables.keys() ]
        packet = Message('AgentIsNowWearing', *args)
        self.agent.region.enqueue_message(packet, True)

    def send_AgentCachedTexture(self, AgentID, SessionID, bakedTextures, wearables):
        """
        Ask the simulator what baked textures it has cached.
        TODO Create a one-shot callback?
        """
        args = [
         Block('AgentData', AgentID=AgentID, SessionID=SessionID, SerialNum=self.AgentCachedSerialNum)]
        args += [ Block('WearableData', ID=bakedTextures[i].get_hash(wearables), TextureIndex=i) for i in range(BakedIndex.BAKED_COUNT) ]
        packet = Message('AgentCachedTexture', *args)
        self.AgentCachedSerialNum += 1
        self.agent.region.enqueue_message(packet, True)

    def send_AgentSetAppearance(self, AgentID, SessionID, Size, bakedTextures, TextureEntry, visualParams):
        """
        Informs simulator how avatar looks
        """
        args = [
         Block('AgentData', AgentID=AgentID, SessionID=SessionID, SerialNum=self.AgentSetSerialNum, Size=Size)]
        args += [ Block('WearableData', CacheID=bakedTextures[i].TextureID, TextureIndex=bakedTextures[i].bakedIndex) for i in range(BakedIndex.BAKED_COUNT) ]
        args += [Block('ObjectData', TextureEntry=TextureEntry)]
        paramkeys = visualParams.keys()
        paramkeys.sort()
        args += [ Block('VisualParam', ParamValue=visualParams[key].floatToByte()) for key in paramkeys if visualParams[key].group is 0 ]
