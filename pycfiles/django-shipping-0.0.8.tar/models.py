...
# [GCC 8.4.0]
# Embedded file name: /Users/marcel/projects/quatix/django-shipping/shipping/models.py
# Compiled at: 2013-03-16 15:02:27
...
class Bin(models.Model):
    ...
    def __unicode__(self):
        if self.width and self.height and self.length:
            return ('{name} ({w}x{h}x{l})').format(name=self.name, w=self.width, h=self.height, l=self.length)
        return self.name
...
class Carrier(models.Model):
    ...
    def estimate_shipping(self, dimensions, country, zipcode=None, state=None, city=None):
        ...
        total_cost, currency = self.interface.get_shipping_cost(bin=best_bin, packages=best_packing, country=country, zipcode=zipcode, state=state, city=city)
        return (
         total_cost, currency)
    ...
    def get_best_bin_for_packages(self, packages):
        ...
        for package in my_packages:
            if package.heigth > greater_package.heigth and package.width > greater_package.width and package.length > greater_package.length:
                return package.bin
...
class CorreiosCarrier(Carrier):
    ...
    @property
    def interface(self):
        return CorreiosInterface(zip_from=self.zip_code, company=self.correios_company, password=self.correios_password, service=CorreiosService.SEDEX)