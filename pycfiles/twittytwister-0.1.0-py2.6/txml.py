...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twittytwister/txml.py
# Compiled at: 2010-04-14 12:36:38
...
class SimpleListHandler(BaseXMLHandler):
    ...
    def __init__(self, n):
        type = self.ITEM_TYPE
        tag = self.item_tag()
        super(SimpleListHandler, self).__init__(n, handler_dict={tag: type}, enter_unknown=True)
...
def topLevelXMLHandler(toplevel_type):
    ...
    return BaseXMLHandler(None, handler_dict={toplevel_type.MY_TAG: toplevel_type}, enter_unknown=True)
