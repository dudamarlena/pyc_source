...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/twittytwister/twitter.py
# Compiled at: 2010-05-16 00:30:32
"""
...
"""
import base64, urllib, mimetypes, mimetools, logging
...
def __downloadPage(factory, *args, **kwargs):
    ...
    if downloader.scheme == 'https':
        ...
        reactor.connectSSL(downloader.host, downloader.port, downloader, contextFactory)
    else:
        reactor.connectTCP(downloader.host, downloader.port, downloader)
    return downloader
...
class Twitter(object):
    ...
    def __init__(self, user=None, passwd=None, base_url=BASE_URL, search_url=SEARCH_URL, consumer=None, token=None, signature_method=SIGNATURE_METHOD, client_info=None, timeout=0):
    ...
    def __postMultipart(self, path, fields=(), files=()):
        ...
        c = getPage(url, method='POST', agent=self.agent, postdata=body, headers=headers, timeout=self.timeout)
    ...
    def __post(self, path, args={}):
        ...
        c = getPage(url, method='POST', agent=self.agent, postdata=self._urlencode(args), headers=headers, timeout=self.timeout)
        return self.__clientDefer(c)
    ...
    def __postPage(self, path, parser, args={}):
        ...
        if self.client_info != None:
            headers.update(self.client_info.get_headers())
            args['source'] = self.client_info.get_source()
        return self.__doDownloadPage(url, parser, method='POST', agent=self.agent, postdata=self._urlencode(args), headers=headers, timeout=self.timeout)
    ...
    def __downloadPage(self, path, parser, params=None):
        ...
        return self.__doDownloadPage(url, parser, agent=self.agent, headers=headers, timeout=self.timeout)
    ...
    def friends(self, delegate, params={}, extra_args=None):
        ...
        return self.__get('/statuses/friends_timeline.xml', delegate, params, txml.Statuses, extra_args=extra_args)

    def home_timeline(self, delegate, params={}, extra_args=None):
        """Get updates from friends.

        Calls the delgate once for each status object received."""
        return self.__get('/statuses/home_timeline.xml', delegate, params, txml.Statuses, extra_args=extra_args)

    def mentions(self, delegate, params={}, extra_args=None):
        return self.__get('/statuses/mentions.xml', delegate, params, txml.Statuses, extra_args=extra_args)
    ...
    def user_timeline(self, delegate, user=None, params={}, extra_args=None):
        ...
        if user:
            params['id'] = user
        return self.__get('/statuses/user_timeline.xml', delegate, params, txml.Statuses, extra_args=extra_args)

    def list_timeline(self, delegate, user, list_name, params={}, extra_args=None):
        return self.__get('/%s/lists/%s/statuses.xml' % (user, list_name), delegate, params, txml.Statuses, extra_args=extra_args)
    ...
    def show_user(self, user):
        ...
        d = defer.Deferred()
        self.__downloadPage(url, txml.Users(lambda u: d.callback(u))).addErrback(lambda e: d.errback(e))
        return d
    ...
    def update_profile_image(self, filename, image):
        ...
        Returns no useful data."""
        return self.__postMultipart('/account/update_profile_image.xml', files=(
         (
...
class TwitterFeed(Twitter):
    ...
    def _rtfeed(self, url, delegate, args):
        ...
        print 'Fetching', url
        return downloadPage(url, txml.HoseFeed(delegate), agent=self.agent, headers=headers).deferred
    ...
    def sample(self, delegate, args=None):
        ...
        """
        return self._rtfeed('http://stream.twitter.com/1/statuses/sample.xml', delegate, args)
    ...
    def firehose(self, delegate, args=None):
        ...
        """
        return self._rtfeed('http://stream.twitter.com/1/statuses/firehose.xml', delegate, args)

    def filter(self, delegate, args=None):
        """
        Returns public statuses that match one or more filter predicates.
        """
        return self._rtfeed('http://stream.twitter.com/1/statuses/filter.xml', delegate, args)
