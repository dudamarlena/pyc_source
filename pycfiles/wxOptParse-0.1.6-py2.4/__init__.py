...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wxoptparse/__init__.py
# Compiled at: 2006-12-14 13:27:05
import optparse
from optparse import OptionParser
from elementtree import ElementTree
import wx, wx.lib.intctrl, textwrap, os, sys, re
...
class SecondWindow(wx.Frame):
    ...
    def __init__(self):
        ...
        title = 'wxOptParser'
        wx.Frame.__init__(self, None, wx.ID_ANY, title, size=(900, 600), style=wx.DEFAULT_FRAME_STYLE)
...
class MainWindow(wx.Frame):
    ...
    def _addCtrl(self, aVBox, ctrl, option, eventId, function, text=None):
        if text:
            aVBox.Add(wx.StaticText(self.panel, -1, textwrap.fill(text), size=(600,
                                                                               -1)), 0, flag=wx.LEFT | wx.TOP, border=5)
    ...
    def OnClickFile(self, event):
        ...
        dlg = wx.FileDialog(self, message='Choose a file', defaultDir=os.getcwd(), defaultFile='', wildcard='*.*', style=wx.OPEN | wx.CHANGE_DIR)
    ...
    def _buildParams(self, useQuotes=False):
        ...
        if extraArgsValue:
            extraArgs = [ x.strip() for x in extraArgsValue.split(' ') if len(x.strip()) > 0 ]
            if len(extraArgs) > 0:
...
class IterOptions:
    ...
    def next(self):
        if self.nIndex < len(self.list):
            self.nIndex += 1
            return MyOption(self.list[(self.nIndex - 1)], self.parent.et, self.parent.parsed_options)
        raise StopIteration
...
def _WxOptParseGetXmlFromFilename(strFilename):
    strFilename = re.sub('\\..{1,4}$', '.args', strFilename)
...
def handleCommandLine():
    ...
    sys.argv[0] = os.path.basename(strFilename)
