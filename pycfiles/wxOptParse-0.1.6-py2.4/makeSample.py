...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/wxoptparse/makeSample.py
# Compiled at: 2006-12-04 06:40:55
...
class MakeSample:
    ...
    def doMatches(self, line):
        re_four = re.compile('\\s+\\-(.),\\s+\\-\\-([^ ]+),\\s+\\-\\-([^ ]+)\\s+(.*)')
        re_three = re.compile('\\s+\\-(.),\\s+\\-\\-([^ ]+)\\s+(.*)')
        re_two = re.compile('\\s+\\-\\-([^ ]+)\\s+(.*)')
        match = re_four.match(line)
        if match:
            self.params.append({'short': match.group(1), 'long': [match.group(2), match.group(3)], 'desc': match.group(4)})
            return
        ...
        if match:
            ...
            if short == 'h' and match.group(2) == 'help':
                return
            if short == 'h':
                self.params.append({'long': [match.group(2)], 'desc': match.group(3)})
            else:
                self.params.append({'short': short, 'long': [match.group(2)], 'desc': match.group(3)})
            return
        match = re_two.match(line)
        if match:
            if match.group(1) == 'help':
                return
            self.params.append({'long': [match.group(1)], 'desc': match.group(2)})
            return
