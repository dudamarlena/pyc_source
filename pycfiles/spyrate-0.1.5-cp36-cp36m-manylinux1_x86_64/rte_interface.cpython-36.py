...
# [GCC 8.4.0]
# Embedded file name: /tmp/pip-req-build-aao45hdu/spyrate/rte/rte_interface.py
# Compiled at: 2019-07-30 06:23:08
# Size of source mod 2**32: 31020 bytes
"""
This module defines the interfaces to RTE implementations
"""
import os, importlib, abc, numpy as np
...
class RTEInterface(object):
    ...
    def __init__(self, precision='double', direction='bottom_to_top'):
        """
    ...
    @abc.abstractmethod
    def apply_gpoint_bc(self, downward_flux, incident_flux):
        """
    ...
    @abc.abstractmethod
    def apply_scaled_gpoint_bc(self, downward_flux, incident_flux, scale_factor):
        """
    ...
    @abc.abstractmethod
    def longwave_no_scattering(self, secant_propagation_angle, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, quadrature_weight, optical_tau):
        """
    ...
    @abc.abstractmethod
    def gaussian_longwave_no_scattering(self, secant_propagation_angle, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, quadrature_weight, num_angles, optical_tau):
        """
        has to be implemented by an inheriting class
        """
        pass
    ...
    @abc.abstractmethod
    def two_stream_longwave(self, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, single_scattering_albedo, asymmetry_parameter, optical_tau):
        """
        has to be implemented by an inheriting class
        """
        pass
    ...
    @abc.abstractmethod
    def shortwave_no_scattering(self, cos_zenith_angle, direct_beam_flux, optical_tau):
        """
        has to be implemented by an inheriting class
        """
        pass
    ...
    @abc.abstractmethod
    def two_stream_shortwave(self, cos_zenith_angle, direct_beam_flux, single_scattering_albedo, asymmetry_parameter, surface_albedo_direct, surface_albedo_diffuse, downward_flux, optical_tau):
        """
        has to be implemented by an inheriting class
        """
        pass
...
class CompiledRTEInterface(RTEInterface):
    ...
    def __init__(self, rte_extension=None, **kwargs):
        ...
        if self.precision == 'single':
            ...
            if self.precision == 'double':
                ...
                raise ValueError('Uknown precision type {}'.format(self.precision))
    ...
    def apply_zero_bc(self, downward_flux):
        ...
        num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi = self.get_cffi_dims_and_dir(downward_flux)
        downward_flux_ffi = self.convert_to_from_cffi(downward_flux)
        self._rte_extension.lib.apply_BC_0(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
    ...
    def apply_gpoint_bc(self, downward_flux, incident_flux):
        ...
        """
        validate_bc_inputs(downward_flux, incident_flux=incident_flux)
        num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi = self.get_cffi_dims_and_dir(downward_flux)
        downward_flux_ffi = self.convert_to_from_cffi(downward_flux)
        inc_flux_ffi = self.convert_to_from_cffi(incident_flux)
        self._rte_extension.lib.apply_BC_gpt(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, inc_flux_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        return down_flux_out.reshape(downward_flux.shape)
    ...
    def apply_scaled_gpoint_bc(self, downward_flux, incident_flux, scale_factor):
        ...
        self._rte_extension.lib.apply_BC_factor(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, inc_flux_ffi, scale_factor_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        return down_flux_out.reshape(downward_flux.shape)
    ...
    def longwave_no_scattering(self, secant_propagation_angle, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, quadrature_weight, optical_tau):
        ...
        Implementation of abstract method
        """
        validate_lw_solver_inputs(planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, optical_tau, secant_propagation_angle, quadrature_weight)
        ...
        self._rte_extension.lib.lw_solver_noscat(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, sec_angle_ffi, weight_ffi, tau_ffi, planck_mid_ffi, planck_inc_ffi, planck_dec_ffi, sfc_emiss_ffi, sfc_src_ffi, upward_flux_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        up_flux_out = self.convert_to_from_cffi(upward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        return (
         up_flux_out.reshape(downward_flux.shape), down_flux_out.reshape(downward_flux.shape))

    def gaussian_longwave_no_scattering(self, secant_propagation_angle, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, quadrature_weight, num_angles, optical_tau):
        """
        Implementation of abstract method
        """
        validate_lw_solver_inputs(planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, optical_tau, secant_propagation_angle, quadrature_weight, num_angles)
        upward_flux = np.empty_like(downward_flux)
        num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi = self.get_cffi_dims_and_dir(downward_flux)
        downward_flux_ffi = self.convert_to_from_cffi(downward_flux)
        upward_flux_ffi = self.convert_to_from_cffi(upward_flux)
        ...
        self._rte_extension.lib.lw_solver_noscat_GaussQuad(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, num_angles_ffi, sec_angle_ffi, weight_ffi, tau_ffi, planck_mid_ffi, planck_inc_ffi, planck_dec_ffi, sfc_emiss_ffi, sfc_src_ffi, upward_flux_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        up_flux_out = self.convert_to_from_cffi(upward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
    ...
    def two_stream_longwave(self, planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, single_scattering_albedo, asymmetry_parameter, optical_tau):
        ...
        """
        validate_lw_solver_inputs(planck_src_mid,
        ...
        self._rte_extension.lib.lw_solver_2stream(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, tau_ffi, ssa_ffi, asym_ffi, planck_mid_ffi, planck_inc_ffi, planck_dec_ffi, sfc_emiss_ffi, sfc_src_ffi, upward_flux_ffi, downward_flux_ffi)
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
        up_flux_out = self.convert_to_from_cffi(upward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
    ...
    def shortwave_no_scattering(self, cos_zenith_angle, direct_beam_flux, optical_tau):
        ...
        Implementation of abstract method
        """
        validate_sw_solver_inputs(cos_zenith_angle, direct_beam_flux, optical_tau)
        num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi = self.get_cffi_dims_and_dir(direct_beam_flux)
        direct_flux_ffi = self.convert_to_from_cffi(direct_beam_flux)
        zen_angle_ffi = self.convert_to_from_cffi(cos_zenith_angle)
        tau_ffi = self.convert_to_from_cffi(optical_tau)
        self._rte_extension.lib.sw_solver_noscat(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, tau_ffi, zen_angle_ffi, direct_flux_ffi)
        direct_flux_out = self.convert_to_from_cffi(direct_flux_ffi,
          num_bytes=(direct_beam_flux.nbytes))
    ...
    def two_stream_shortwave(self, cos_zenith_angle, direct_beam_flux, single_scattering_albedo, asymmetry_parameter, surface_albedo_direct, surface_albedo_diffuse, downward_flux, optical_tau):
        ...
        Implementation of abstract method
        """
        validate_sw_solver_inputs(cos_zenith_angle, direct_beam_flux, optical_tau, single_scattering_albedo, asymmetry_parameter, surface_albedo_direct, surface_albedo_diffuse, downward_flux)
        upward_flux = np.empty_like(downward_flux)
        num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi = self.get_cffi_dims_and_dir(direct_beam_flux)
        downward_flux_ffi = self.convert_to_from_cffi(downward_flux)
        upward_flux_ffi = self.convert_to_from_cffi(upward_flux)
        ...
        self._rte_extension.lib.sw_solver_2stream(num_cols_ffi, num_layers_ffi, num_gpts_ffi, top_at_1_ffi, tau_ffi, ssa_ffi, asym_ffi, zen_angle_ffi, sfc_alb_dir_ffi, sfc_alb_diff_ffi, upward_flux_ffi, downward_flux_ffi, direct_flux_ffi)
        direct_flux_out = self.convert_to_from_cffi(direct_flux_ffi,
          num_bytes=(direct_beam_flux.nbytes))
        down_flux_out = self.convert_to_from_cffi(downward_flux_ffi,
          num_bytes=(downward_flux.nbytes))
    ...
    def get_cffi_dims_and_dir(self, downward_flux):
        ...
        if self.direction == 'bottom_to_top':
            ...
            if self.direction == 'top_to_bottom':
                top_at_1 = True
        num_gpts_ffi = self._ffi.new('int *', downward_flux.shape[0])
        num_cols_ffi = self._ffi.new('int *', downward_flux.shape[2])
        num_layers_ffi = self._ffi.new('int *', downward_flux.shape[1] - 1)
        top_at_1_ffi = self._ffi.new('bool *', top_at_1)
        return (
         num_gpts_ffi, num_cols_ffi, num_layers_ffi, top_at_1_ffi)
    ...
    def convert_to_from_cffi(self, c_or_numpy_array, num_bytes=None):
        ...
        if isinstance(c_or_numpy_array, np.ndarray):
            out_buffer = self._ffi.cast('{} *'.format(self._ffi_type), c_or_numpy_array.ctypes.data)
        else:
            if isinstance(c_or_numpy_array, self._ffi.CData):
                out_buffer = np.frombuffer((self._ffi.buffer(c_or_numpy_array, num_bytes)),
                  dtype=(self._numpy_type))
            else:
                raise ValueError('Buffer must be either numpy.ndarray or ffi CData,                not {}'.format(type(c_or_numpy_array)))
        return out_buffer


def validate_bc_inputs(downward_flux, incident_flux=None, scale_factor=None):
    """
    ...
    try:
        ...
        raise ValueError('Incorrect shape for downward flux {}'.format(downward_flux.shape))

    if incident_flux is not None:
        try:
            assert len(incident_flux.shape) == 2
        except AssertionError:
            raise ValueError('Incorrect shape for incident flux {}'.format(incident_flux.shape))

        try:
            assert incident_flux.shape[0] == downward_flux.shape[0]
        except AssertionError:
            raise ValueError('Incompatible gpts dimension for downward ({}) and incident flux ({})'.format(downward_flux.shape[0], incident_flux.shape[0]))

        try:
            assert incident_flux.shape[1] == downward_flux.shape[2]
        except AssertionError:
            raise ValueError('Incompatible columns dimension for downward ({}) and incident flux ({})'.format(downward_flux.shape[2], incident_flux.shape[1]))

    if scale_factor is not None:
        try:
            assert len(scale_factor.shape) == 1
        except AssertionError:
            raise ValueError('Incorrect shape for scale factor {}'.format(scale_factor.shape))

        try:
            assert scale_factor.shape[0] == downward_flux.shape[2]
        except AssertionError:
            raise ValueError('Incompatible columns dimension for down flux ({}) and scale factor ({})'.format(downward_flux.shape[2], scale_factor.shape[0]))


def validate_lw_solver_inputs(planck_src_mid, planck_src_iface_decreasing, planck_src_iface_increasing, surface_emissivity, surface_source, downward_flux, optical_tau, secant_propagation_angle=None, quadrature_weight=None, num_angles=None, ssa=None, asym_param=None):
    """
...
def validate_sw_solver_inputs(cos_zenith_angle, direct_beam_flux, optical_tau, single_scattering_albedo=None, asymmetry_parameter=None, surface_albedo_direct=None, surface_albedo_diffuse=None, downward_flux=None):
    """
    Validate longwave solver inputs.
    """
    if not len(direct_beam_flux.shape) == 3:
...
class UserCompiledRTEInterface(CompiledRTEInterface):
    ...
    def __init__(self, compiler='gcc', python_module_name='_rte_wrapper', compiled_library_path=None, **kwargs):
        """
        ...
        if not os.path.exists(absolute_library_path):
            raise OSError('{} does not exist!'.format(absolute_library_path))
