...
# [GCC 8.4.0]
# Embedded file name: /home/lance/.virtualenvs/wolverine/lib/python3.5/site-packages/wolverine/module/controller/zmq.py
...
import aiozmq, msgpack, types, zmq
...
class ZMQMicroController(MicroController):
    bind_types = {'observer': zmq.ROUTER, 
     'provider': zmq.DEALER}
    ...
    def connect_client(self, name, func, **options):
        ...
        try:
            client = yield from aiozmq.create_zmq_stream(zmq.DEALER)
            yield from client.transport.enable_monitor()
            self.app.loop.create_task(self.monitor_stream(name, client))
            yield from client.transport.bind(uri)
        except Exception as ex:
            logger.error('failed to bind zqm socket for dealer ' + service_name, exc_info=True)
            return

        service_opts = {'service_id': service_name, 
         'address': address, 
         'port': int(port), 
         'tags': tags, 
         'check_ttl': str(check_ttl) + 's', 
         'ttl_ping': int(ttl_ping)}
        up = yield from self.app.router.add_client(client, name, **service_opts)
        if up:
            logger.info('service ' + name + ' registered with consul with id ' + service_name)
        else:
            logger.error('failed to register client ' + service_name + ' with consul... ')
    ...
    def connect_service(self, name, service):
        ...
        @self.app.registry.listen(name, listen_type=listen_type, singleton=True, **options)
        def discover_service(data):
            try:
                data = self.app.registry.unwrap(data, listen_type)
                if data and 'passing' in data:
                    new = list(set(data['passing'].keys()) - set(self.app.router.servers.keys()))
                    removed = list(set(self.app.router.servers.keys()) - set(data['passing'].keys()))
                    ...
                    for key in new:
                        ...
                        port = s.pop('Port', '')
                        uri = address + ':' + str(port)
                        service = yield from self.bind_service(key, uri, bind_type)
        ...
        def run():
            ...
            while alive:
                try:
                    ...
                    self.app.loop.create_task(self.handle_service_data(work, service_name))
                except aiozmq.ZmqStreamClosed:
    ...
    def handle_service_data(self, d, service_name):
        ...
        try:
            try:
                ...
                logger.error('failure while handling data', extra={'service_name': service_name, 
                 'data': d}, exc_info=True)

        finally:
            if state != 2:
                logger.error('service handler reached invalid state', extra={'state': 2})