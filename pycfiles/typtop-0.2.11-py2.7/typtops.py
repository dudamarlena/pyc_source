...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/typtop/typtops.py
# Compiled at: 2017-03-23 16:23:54
from __future__ import print_function
import os, sys, argparse, requests, random, json
from typtop.dbaccess import UserTypoDB, call_check, is_user, get_time, get_machine_id
from typtop.config import NUMBER_OF_ENTRIES_TO_ALLOW_TYPO_LOGIN, WARM_UP_CACHE, VERSION, DISTRO, BINDIR, first_msg, LOG_DIR, DB_NAME, TEST, SEC_DB_PATH
...
def _get_typoDB():
    ...
    try:
        ...
        print(('It seems you have not initialized the db. Try running "sudo {} --init" to initialize the db.\nThe error I ran into is the following:\n{}').format(sys.argv[0], e))
        return
...
def call_send_logs(args):
    ...
    users = [user]
    force = True if len(args.send_log) > 1 and args.send_log[1] == 'force' else False
    if user == 'all':
        users = [ d for d in os.listdir(SEC_DB_PATH) if os.path.isdir(os.path.join(SEC_DB_PATH, d)) and is_user(d)
                ]
    for user in users:
...
def send_logs(typo_db, force=False):
    ...
    try:
        r = requests.post(url, data=dict(uid=install_id.strip() + '#' + str(VERSION), data=dbdata, test=int(TEST)), allow_redirects=True, verify=CERT_FILE)
...
def initiate_typodb():
    ...
    if False:
        ...
        download_url = ('https://github.com/rchatterjee/pam-typopw/archive/{0}.zip').format(VERSION)
        cmd = ('\n        cd /tmp/ && {download_bin} {download_url} && unzip -qq -o {version}.zip         && cd pam-typopw-{version}/{subdir} && {makecmd};\n        cd /tmp && rm -rf {version}.zip pam-typopw*\n        ').format(branch=branch, subdir=subdir, download_url=download_url, download_bin=download_bin, makecmd=makecmd, version=VERSION)
        os.system(cmd)


common_auth = {'debian': '/etc/pam.d/common-auth', 
   'fedora': '/etc/pam.d/system-auth', 
   'darwin': '', 
   'arch': '/etc/pam.d/system-auth'}[DISTRO]
...
parser.add_argument('--init', action='store_true', help='To initialize the DB. You have to run this once you install pam_typtop')
parser.add_argument('--allowtypo', type=str.lower, choices=['yes', 'no'], help='Allow login with typos of the password')
parser.add_argument('--allowupload', type=str.lower, choices=['yes', 'no'], help='Allow uploading the non-sensitive anonymous data into the server for research purposes.')
parser.add_argument('--id', action='store_true', help='Get Installation id')
parser.add_argument('--send-log', nargs='*', type=str, action='store', metavar=('user',
                                                                                'force'), help='Send the logs to the server')
parser.add_argument('--status', action='store', nargs='+', metavar='user', help='Prints current states of the typo-tolerance.Needs a username as argument.')
parser.add_argument('--uninstall', action='store_true', help='Uninstall TypToP from your machine. Will delete all the data related to TypTop too.')
parser.add_argument('--update', action='store_true', help='Updates TypTop to the latest released version')
parser.add_argument('--check', action='store', nargs=3, help="(INTERNAL FUNCTION. PLEASE DON'T CALL THIS.)")
parser.add_argument('--debug', action='store_true', help='Prepare report for debugging')
parser.add_argument('--version', action='store_true', help='What is the version of TypTop')
    ...
    try:
        ...
        if args.status:
            ...
            for user in users:
                ...
                print(('\tIs enough logins to allow typos: {}').format(typoDB.check_login_count(update=False)))
        ...
        if args.uninstall:
            r = _raw_input('Uninstalling pam_typtop. Will delete all the databases.\nPlease confirm. (yN)')
            if r and r.lower() == 'y':
        ...
        if args.debug:
            p = subprocess.Popen('pip show numpy', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
            numpypath = ''
            for l in p.stdout:
                l = l.strip().split(': ', 1)
                if l[0] == 'Location':
