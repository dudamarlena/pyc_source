...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/typtop/pycryptodome_pwpkcrypto.py
# Compiled at: 2016-12-15 18:38:58
from Crypto.Protocol.KDF import PBKDF2
from Crypto.PublicKey import ECC
from Crypto.Cipher import AES
from Crypto.Hash import SHA1, SHA256, SHA224, SHA256, SHA384, SHA512, HMAC
import os, os, random
from base64 import urlsafe_b64encode, urlsafe_b64decode
import struct
HASH_ALGOS = {'sha1': SHA1, 
   'sha224': SHA224, 
   'sha256': SHA256, 
   'sha384': SHA384, 
   'sha512': SHA512}
...
def _slow_hash(pw, sa):
    return PBKDF2(pw, sa, dkLen=16, count=HASH_CNT, prf=hmac256)
...
def pwencrypt(pw, m):
    ...
    sa = os.urandom(SALT_LENGTH)
    key = PBKDF2(pw, sa, dkLen=16, count=itercnt, prf=hmac256)
...
def pwdecrypt(pw, full_ctx_b64):
    ...
    key = PBKDF2(pw, sa, dkLen=16, count=int(itercnt), prf=hmac_tmp)
    try:
