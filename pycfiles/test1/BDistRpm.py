...
# [GCC 8.4.0]
# Embedded file name: \Ft\Lib\DistExt\BDistRpm.py
# Compiled at: 2006-08-17 10:20:34
import os, glob
...
class BDistRpm(bdist_rpm.bdist_rpm):
    ...
    user_options.insert(index, ('patch-files=', None, 'list of patches (space or comma-separated)'))
    del index
    ...
    def run(self):
        ...
        self.execute(write_file, (
         spec_path, self._make_spec_file()), "writing '%s'" % spec_path)
        if self.spec_only:
            return
        ...
        if self.source_only:
            ...
            rpm_cmd.append('-ba')
        if self.rpm3_mode:
            rpm_cmd.extend(['--define', '_topdir %s/%s' % (os.getcwd(), self.rpm_base)])
    ...
    def _make_spec_file(self):
        ...
        spec_file = ['%{expand: %%define pyver %(' + pyver + ')}', '%define pylib %{_libdir}/python%{pyver}/site-packages', '', '%define name ' + self.distribution.get_name(), '%define version ' + self.distribution.get_version(), '%define release ' + self.release, '', 'Summary: ' + self.distribution.get_description()]
        ...
        for pkg in self.distribution.sub_packages:
            ...
            spec_file.extend(['', '%package ' + dist.get_name(), 'Summary: ' + dist.get_description(), 'Group: ' + self.group])
            for dep in dist.dependencies:
                spec_file.append('Requires: %%{name}-%s = %%{version}-%%{release}' % dep)

            spec_file.extend(['', '%description ' + dist.get_name(), dist.get_long_description()])
            distributions.append(dist)

        if self.distribution.has_docs():
            spec_file.extend(['', '%package Docs', 'Summary: Documentation for %{name}', 'Group: ' + self.group, '', '%description Docs', 'The %{name}-Docs package contains documentation on the %{name} libraries.  The documentation is provided in ASCII text files and in HTML files.'])
        if self.distribution.has_devel():
            spec_file.extend(['', '%package Tests', 'Summary: The test suite for %{name}', 'Group: ' + self.group])
            for pkg in self.distribution.sub_packages:
                spec_file.append('Requires: %%{name}-%s = %%{version}-%%{release}' % pkg)

            spec_file.extend(['', '%description Tests', 'This package includes various tests for %{name}, including regression tests and benchmarks.'])
        spec_file.extend(['', '%prep', '%setup'])
        if self.use_rpm_opt_flags:
            python = 'env CFLAGS="$RPM_OPT_FLAGS" ' + self.python
        else:
            python = self.python
        spec_file.extend(['', '%build', python + ' setup.py config \\\n\t--prefix=%{_prefix} \\\n\t--exec-prefix=%{_exec_prefix} \\\n\t--pythonlibdir=%{pylib} \\\n\t--bindir=%{_bindir} \\\n\t--datadir=%{_datadir}/%{name} \\\n\t--sysconfdir=%{_sysconfdir} \\\n\t--localstatedir=%{_localstatedir}/%{name} \\\n\t--libdir=%{_libdir}/%{name} \\\n\t--docdir=%{_docdir}', '%s setup.py build' % python])
        spec_file.extend(['', '%install', '%s setup.py install --root=$RPM_BUILD_ROOT' % self.python])
        spec_file.extend(['', '%clean', 'rm -rf $RPM_BUILD_ROOT'])
        ...
        for dist in distributions:
            spec_file.extend(['', '%%files %s' % dist.get_name(), '%defattr(-,root,root)'])
            ...
            for command in ('install_lib', 'install_scripts', 'install_data', 'install_localstate'):
        ...
        if self.distribution.has_docs():
            spec_file.extend(['', '%files Docs', '%defattr(-,root,root)'])
            install_docs = self.get_finalized_command('install_docs')
            for fname in install_docs.get_outputs():
                spec_file.append(fname)

        if self.distribution.has_devel():
            spec_file.extend(['', '%files Tests', '%defattr(-,root,root)'])
