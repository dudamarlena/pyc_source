...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\XsltFunctions.py
# Compiled at: 2005-08-02 17:43:01
"""
...
def Document(context, object, nodeSet=None):
    ...
    if nodeSet:
        ...
        if not nodeSet:
            raise XsltRuntimeException(Error.DOC_FUNC_EMPTY_NODESET, context.currentInstruction)
        try:
            base = getattr(nodeSet[0], 'baseURI')
        except AttributeError:
            raise XsltRuntimeException(Error.UNKNOWN_NODE_BASE_URI, context.currentInstruction, 'first node in second argument to document()')
    ...
    if not isinstance(object, NodesetType):
        ...
        if IsAbsolute(ref):
            ...
            base = context.currentInstruction.baseUri
            if not base:
                raise XsltRuntimeException(Error.UNKNOWN_NODE_BASE_URI, context.currentInstruction, 'node containing document() call')
    ...
    for node in object:
        ...
        if IsAbsolute(ref):
            ...
            if not base:
                raise XsltRuntimeException(Error.UNKNOWN_NODE_BASE_URI, context.currentInstruction, 'first node in first argument to document()')
        uris[context.processor.inputSourceFactory.resolver.normalize(ref, base)] = 1
        ...
        if uri in context.documents:
            ...
            try:
                if uri in sheet.root.sources:
                    ...
                    isrc = context.processor._documentInputSource.clone(stream, uri, 'XSLT DOCUMENT FUNCTION')
                else:
                    isrc = context.processor._documentInputSource.resolve(uri, hint='XSLT DOCUMENT FUNCTION')
...
def Key(context, qname, keyList):
    ...
    if not qname:
        raise XsltRuntimeException(Error.INVALID_QNAME_ARGUMENT, context.currentInstruction, qname)
...
def GenerateId(context, nodeSet=None):
    ...
    if nodeSet is not None and type(nodeSet) != type([]):
        raise XsltRuntimeException(Error.WRONG_ARGUMENT_TYPE, context.currentInstruction)
    if nodeSet is None:
        return 'id%r' % id(context.node)
    elif nodeSet:
        nodeSet.sort()
        return 'id%r' % id(nodeSet[0])
    else:
        return ''
    return
...
def FormatNumber(context, number, formatString, decimalFormatName=None):
    ...
    try:
        ...
        raise XsltRuntimeException(Error.UNDEFINED_DECIMAL_FORMAT, decimalFormatName)

    return routines.FormatNumber(num, format_string, decimal_format)
    return


CoreFunctions = {(EMPTY_NAMESPACE, 'document'): Document, (EMPTY_NAMESPACE, 'key'): Key, (EMPTY_NAMESPACE, 'current'): Current, (EMPTY_NAMESPACE, 'generate-id'): GenerateId, (EMPTY_NAMESPACE, 'system-property'): SystemProperty, (EMPTY_NAMESPACE, 'function-available'): FunctionAvailable, (EMPTY_NAMESPACE, 'element-available'): ElementAvailable, (EMPTY_NAMESPACE, 'format-number'): FormatNumber, (EMPTY_NAMESPACE, 'unparsed-entity-uri'): UnparsedEntityUri}