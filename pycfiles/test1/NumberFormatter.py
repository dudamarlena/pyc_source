...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\NumberFormatter.py
# Compiled at: 2005-03-13 16:57:32
import re
...
class Formatter:
    ...
    def __init__(self, format):
        ...
        if not groups:
            raise XsltException(Error.ILLEGAL_NUMBER_FORMAT_VALUE, self._format)
...
class DefaultFormatter(Formatter):
    ...
    def format(self, numbers, groupSize, groupSeparator):
        ...
        for (number, token, separator) in zipped[:len(numbers)]:
            ...
            if token[(-1)] == '1':
                result.append(self._numeric(number, len(token), groupSize, groupSeparator))
