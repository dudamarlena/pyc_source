...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\_4xml.py
...
class XmlCommandLineApp(CommandLineApp.CommandLineApp):
    ...
    options = [
     Options.Option('v', 'validate', 'DTD validate the input file as it is being parsed'),
     Options.Option('e', 'encoding=ENC', 'The encoding to use for output'),
     Options.Option(None, 'input-encoding=ENC', 'The encoding to assume for input'),
     Options.Option('o', 'outfile=FILE', 'Direct output to FILE (file will be overwritten if it exists)'),
     Options.Option('p', 'pretty', 'Pretty-print the result'),
     Options.Option('n', 'noserialize', 'Do not serialize; just parse'),
     Options.Option(None, 'html', 'Use HTML mode when pretty-printing (emit XHTML as HTML)'),
     Options.Option(None, 'noxinclude', 'Do not expand XIncludes'),
     Options.Option(None, 'rng=FILE', 'Apply RELAX NG from the given file (technically RNG with XVIF features)')]
    arguments = [
     Arguments.RequiredArgument('source-uri', 'The URI of the XML document to parse, or "-" to indicate standard input.')]

    def validate_arguments(self, args):
        if not args:
            raise SystemExit('A source URI argument is required. See "%s -h" for usage info.' % sys.argv[0])
...
def Run(options, args):
    ...
    try:
        source_isrc = SourceArgToInputSource(sourceUri, InputSource.DefaultFactory, processIncludes=xinclude, encoding=input_encoding)
        rng_isrc = None
        if rng_sourceUri:
            rng_isrc = SourceArgToInputSource(rng_sourceUri, InputSource.DefaultFactory, processIncludes=xinclude, encoding=input_encoding)
    except Exception, e:
    ...
    if rng_isrc is not None:
        try:
            ...
            raise SystemExit('Missing RELAX-NG support library.  It is available in the Ft.Xml.ThirdParty package.')
