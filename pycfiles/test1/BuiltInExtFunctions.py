...
# [GCC 8.4.0]
# Embedded file name: \Ft\Xml\Xslt\BuiltInExtFunctions.py
# Compiled at: 2005-03-18 18:47:19
"""
...
__all__ = [
 'ExtNamespaces', 'ExtFunctions', 'FtKey', 'Lookup', 'Map', 'DocAsString', 'SearchRe', 'SerializeXml', 'Split']
...
def Map(context, funcname, *nodesets):
    ...
    if prefix and not uri:
        raise XsltRuntimeException(Error.UNDEFINED_PREFIX, context.currentInstruction, prefix)
    expanded = (
     prefix and uri or '', local)
...
def DocAsString(context, obj, encoding='UTF-8'):
    ...
    if isinstance(obj, list):
        result = ''
        for node in obj:
            result += DocAsString(context, Conversions.StringValue(node), encoding)

    else:
        if hasattr(context, 'processor'):
            isrc = context.processor.inputSourceFactory.fromUri(baseUri)
            isrc = isrc.resolve(Conversions.StringValue(obj), hint='XSLT DOCUMENT FUNCTION')
            data = isrc.read()
            isrc.close()
            return data
        else:
            uri = Uri.BASIC_RESOLVER.normalize(Conversions.StringValue(obj), baseUri)
        if obj == '' and baseUri in sheet.root.sources:
...
def SerializeXml(context, nodeset, method=None, omitxmldecl=1):
    """
...
ExtFunctions = {(FT_EXT_NAMESPACE, 'key'): FtKey, (FT_EXT_NAMESPACE, 'lookup'): Lookup, (FT_EXT_NAMESPACE, 'search-re'): SearchRe, (FT_EXT_NAMESPACE, 'map'): Map, (FT_EXT_NAMESPACE, 'doc-as-string'): DocAsString, (FT_EXT_NAMESPACE, 'serialize-xml'): SerializeXml, (FT_EXT_NAMESPACE, 'split'): Split}