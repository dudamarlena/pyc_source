...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyfva/clientes/validador.py
# Compiled at: 2020-01-01 13:57:11
# Size of source mod 2**32: 14027 bytes
"""
Created on 19 jul. 2017

@author: luis
"""
from pyfva.soap.validador_certificado import ValideElServicio as ValideServicioCertificado, ValidadorDeCertificadoSoapServiceStub, SoliciteLaValidacionDelCertificadoDeAutenticacion
from pyfva.soap.validador_documento import ValideElServicio as ValideServicioDocumento, ValidadorDeDocumentoSoapServiceStub, ValideElDocumentoXmlEnvelopedCoFirma, ValideElDocumentoXmlEnvelopedContraFirma, ValideElDocumentoMSOffice, ValideElDocumentoOdf, ValideElDocumentoPdf
from pyfva.soap import settings
from pyfva.constants import get_text_representation, ERRORES_VALIDA_CERTIFICADO, ERRORES_VALIDAR_ODF, ERRORES_VALIDAR_MSOFFICE, ERRORES_VALIDAR_XMLCONTRAFIRMA, ERRORES_VALIDAR_XMLCOFIRMA, ERRORES_VALIDAR_PDF
import traceback
from pyfva import logger

class ClienteValidador(object):
    __doc__ = 'Permite validar una firma o un documento utilizando los servicios del BCCR\n\n    Los documentos que se pueden validar son:\n\n    * Certificados digitales (CA nacional)\n    * XML: con cofirma y contrafirma\n    * MSOffice: .docx, .xlsx y .pptx\n    * ODF: .odt, .ods y .odp \n    * PDF: .pdf\n\n    .. note:: \n        Los parámetros negocio y entidad de momento no son requeridos, pero puede que en un futuro cercano\n        lo sean, por lo que se recomienda suministrarlos.\n\n    :param negocio: número de identificación del negocio (provisto por el BCCR)\n    :param entidad: número de identificación de la entidad (provisto por el BCCR)\n    '
    DEFAULT_CERTIFICATE_ERROR = {'codigo_error':1, 
     'texto_codigo_error':get_text_representation(ERRORES_VALIDA_CERTIFICADO, 1), 
     'exitosa':False, 
     'certificado':None}
    ...
    def validar_documento(self, documento, formato):
        ...
        **DEFAULT_DOCUMENT_ERROR**.
        ...
        """
        logger.debug({'message':'Validador: validar_documento',  'data':{'format':formato, 
          'data':repr(locals())}, 
         'location':__file__})
        try:
            if formato == 'cofirma':
                    ...
                    if formato == 'msoffice':
                        ...
                        if formato == 'odf':
                            ...
                            if formato == 'pdf':
                                ...
                                logger.error({'message':'Validador: validando documento',  'data':{'format':formato, 
        ...
        logger.info({'message':'Validador: validar_documento',  'data':{'format':formato, 
          'result':dev}, 
         'location':__file__})
        return dev
    ...
    def validar_certificado_autenticacion(self, certificado):
        ...
        **DEFAULT_CERTIFICATE_ERROR**.
        ...
        """
        logger.debug({'message':'Validador: validar_certificado_autenticacion', 
         'data':repr(locals()),  'location':__file__})
        try:
            dev = self._validar_certificado_autenticacion(certificado)
        except Exception as e:
            logger.error({'message':'Validador: validando certificado',  'data':e, 
             'location':__file__})
            dev = self.DEFAULT_CERTIFICATE_ERROR

        logger.info({'message':'Validador: validar_certificado_autenticacion result ', 
    ...
    def validar_servicio(self, servicio):
        ...
        logger.info({'message':'Validador: Validar servicio',  'data':{'servicio':servicio, 
          'result':dev}, 
         'location':__file__})
        return dev
    ...
    def _validar_documento_cofirma(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoXmlEnvelopedCoFirmaResult, ERRORES_VALIDAR_XMLCOFIRMA)
        except Exception as e:
    ...
    def _validar_documento_contrafirma(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoXmlEnvelopedContraFirmaResult, ERRORES_VALIDAR_XMLCONTRAFIRMA)
        except Exception as e:
            traceback.print_exc()
    ...
    def _validar_documento_msoffice(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoMSOfficeResult, ERRORES_VALIDAR_MSOFFICE)
        except Exception as e:
            logger.error({'message':'Validador: validando  MSOffice',  'data':e, 
             'location':__file__})
    ...
    def _validar_documento_odf(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoOdfResult, ERRORES_VALIDAR_ODF)
        except Exception as e:
    ...
    def _validar_documento_pdf(self, documento):
        ...
        try:
            ...
            dev = self._extract_documento_xml(status.soap_body.ValideElDocumentoPdfResult, ERRORES_VALIDAR_PDF)
        except Exception as e:
    ...
    def _extract_documento_xml(self, result, ERRORES_VALIDACION):
        ...
        dev['texto_codigo_error'] = (
         get_text_representation(ERRORES_VALIDACION, dev['codigo_error']),)
        ...
        if result.ErroresEncontrados:
            dev['errores_encontrados'] = [(error.Codigo, error.Detalle) for error in result.ErroresEncontrados.ErrorDeDocumento]
        if result.Firmantes:
            dev['firmantes'] = [{'identificacion':x.Cedula,  'fecha_firma':x.FechaDeFirma,  'nombre':x.NombreCompleto} for x in result.Firmantes.Firmante]
        return dev
    ...
    def _extract_certificado_autenticacion(self, result):
        ...
        if result.FueExitosa:
            ...
            dev['certificado'] = {'tipo_identificacion':cert.TipoDeIdentificacion, 
    ...
    def _validar_servicio_certificado(self):
        ...
        try:
            ...
            logger.error({'message':'Validador: Servicio de validado de certificado fallando',  'data':e, 
             'location':__file__})
            dev = False
    ...
    def _validar_servicio_documento(self):
        ...
        try:
            ...
            logger.error({'message':'Validador: Servicio de validado de documentos fallando', 
