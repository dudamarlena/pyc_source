...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/atlas/parse_countries.py
...
for country in countries:
    ...
    if fields['border'] is not None and 'MULTIPOLYGON' not in fields['border']:
        ...
        sql_country += "(%d,'%s','%s',GeometryFromText('%s', 4326),%s)," % (
         ...
         "GeometryFromText('%s', 4326)" % code_geom[code]['centroid'] if code in code_geom and 'centroid' in code_geom[code] else 'null')
    else:
