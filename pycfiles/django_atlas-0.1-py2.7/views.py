...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/atlas/views.py
...
def set_location(request):
    ...
    if 'REMOTE_ADDR' in request.META:
        ...
        return HttpResponseBadRequest('Client cannot be located using IP %s' % request.META['REMOTE_ADDR'])
    if 'atlas_id' in request.COOKIES:
        return HttpResponseBadRequest('Client cannot be located using location %s' % request.COOKIES['atlas_id'])
