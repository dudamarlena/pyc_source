...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/spotifylib/spotifylib.py
...
class SpotifyAuthenticator(object):
    ...
    def __init__(self, client_id, client_secret, username, password, callback, scope):
        """
        ...
        :param scope: string
        """
        self._logger = logging.getLogger(('{base}.{suffix}').format(base=LOGGER_BASENAME, suffix=self.__class__.__name__))
    ...
    def _get_referer(self):
        ...
        """
        params = (
         ('{auth}?scope={scope}&').format(auth=AUTH_WEB_URL, scope=quote(self._scope)),
         ('redirect_uri={call}?').format(call=quote(self._callback, safe=':')),
         'response_type=code?',
         ('client_id={client_id}').format(client_id=self.user.client_id))
        referer = (('{login_url}?').format(login_url=LOGIN_WEB_URL),
         ('continue={params}').format(params=quote(('').join(params), safe=':')))
        return ('').join(referer)
    ...
    def _get_authorization(self):
        ...
        :return: boolean
        """
        params = {'scope': self._scope, 'redirect_uri': self._callback, 
           'response_type': 'code', 
           'client_id': self.user.client_id}
        response = self.session.get(AUTH_WEB_URL, headers=HEADERS, params=params)
        if not response.ok:
            self._logger.exception(response.content)
            raise SpotifyError(('Failed to get authorization page. Message: {}').format(response.content))
        __bon_cookie = {'name': '__bon', 'value': self.__get_bon(response)}
    ...
    def _login_to_account(self):
        ...
        :return: boolean
        """
        payload = {'remember': 'true', 'username': self.user.username, 
           'password': self.user.password, 
           'csrf_token': self.session.cookies.get('csrf_token')}
        response = self.session.post(API_LOGIN_URL, data=payload, headers=HEADERS)
        if response.status_code == 400:
            self._logger.exception(response.content)
            raise SpotifyError(('Failed to login to API. Message: {}').format(response.content))
        return True
    ...
    def _accept_app_to_account(self):
        ...
        """
        payload = {'scope': self._scope, 'redirect_uri': self._callback, 
           'response_type': 'code', 
           'client_id': self.user.client_id, 
           'csrf_token': self.session.cookies.get('csrf_token')}
        response = self.session.post(ACCEPT_URL, data=payload, headers=HEADERS)
        if response.status_code == 400:
            self._logger.exception(response.content)
    ...
    def _get_token(self, response):
        ...
        try:
            ...
            self._logger.exception(response.content)
            raise SpotifyError(('Error while getting the token. Got: {}').format(response.content))

        payload = {'grant_type': 'authorization_code', 'code': code, 'redirect_uri': self._callback}
    ...
    def _retrieve_token(session, user, payload):
        ...
        :return: Token namedtuple
        """
        base64encoded = b64encode(('{user_id}:{secret}').format(user_id=user.client_id, secret=user.client_secret))
        headers = {'Authorization': ('Basic {}').format(base64encoded)}
        response = session.post(TOKEN_URL, data=payload, headers=headers)
        if response.status_code == 400:
            LOGGER.exception(response.content)
            raise SpotifyError(("Couldn't get new token from refresh token. Got: {}").format(response.content))
        ...
        if not all(token_values):
            LOGGER.exception(response.content)
            raise ValueError(('Incomplete token response received. Got: {}').format(response.json()))
    ...
    def _patched_request(self, method, url, **kwargs):
        ...
        :return: Response instance
        """
        self._logger.debug(('Using patched request for method {method}, url {url}').format(method=method, url=url))
        ...
        if response.status_code == 401 and response.json() == INVALID_TOKEN_MSG:
            ...
            self.session.token = self.session.renew_token(self.session, self.user, self.token)
...
class Spotify(object):
    ...
    def __new__(cls, client_id, client_secret, username, password, callback, scope):
        """
        ...
        """
        authenticated = SpotifyAuthenticator(client_id, client_secret, username, password, callback, scope)
        spotify = OriginalSpotify(auth=authenticated.token.access_token, requests_session=authenticated.session)
