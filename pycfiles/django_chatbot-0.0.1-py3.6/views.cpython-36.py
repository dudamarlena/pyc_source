...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/django/chatbot/views.py
...
def web_hook(request):
    ...
    if sender_id not in chat.conversation:
        ...
        chat.conversation[sender_id].append_bot(getattr(settings, 'START_MESSAGE', 'Welcome to ChatBotAI'))
    ...
    return JsonResponse({'status':'Success', 
     'messages':[{'id':msg.id,  'text':msg.message,  'created':msg.created.strftime('%Y-%m-%d %H:%M:%S'),  'by':'bot' if msg.bot else 'user'} for msg in msgs]})