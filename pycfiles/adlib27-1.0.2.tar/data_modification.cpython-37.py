...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/adversaries/datamodification/data_modification.py
# Compiled at: 2018-07-17 12:56:28
# Size of source mod 2**32: 14148 bytes
...
class DataModification(Adversary):
    ...
    def __init__(self, learner, target_theta, lda=0.001, alpha=0.0001, beta=0.05, decay=-1, eta=0.9, max_iter=250, verbose=False):
        """
    ...
    def attack(self, instances) -> List[Instance]:
        ...
        while not iteration == 0:
            if not fv_dist > self.alpha or iteration < self.max_iter:
                print('Iteration: ', iteration, ' - FV distance: ', fv_dist, ' - theta distance: ',
                  theta_dist, ' - beta: ', (self.beta), sep='')
                begin = time.time()
                self._write_to_file()
        ...
        print('Iteration: FINAL - FV distance: ', fv_dist, ' - theta distance: ',
          theta_dist, ' - alpha: ', (self.alpha), ' - beta: ',
          (self.beta), '\n', sep='')
        if self.verbose:
            print('\n\nTarget Theta:\n\n', self.target_theta, '\n\nTheta:\n\n', self.theta, '\n')
        ...
        for i, fv in enumerate(self.fvs):
            ...
            self.return_instances[i].feature_vector = RealFeatureVector(feature_count, indices, data)
    ...
    def _calculate_constants(self):
        ...
        self._calc_theta()
        self.logistic_vals = [logistic_function(self.labels[i] * self.g_arr[i]) for i in range(len(self.instances))]
    ...
    def _calc_theta(self):
        ...
        """
        self.learner.fit(self.fvs, self.labels)
        self.theta = self.learner.decision_function(np.eye(self.instances[0].get_feature_count()))
    ...
    def _calc_gradient(self):
        ...
        command_fail = True
        if self.system == 'Darwin' or self.system == 'Linux' or self.system == 'Windows':
            ...
            command += ' ' + str(self.lda) + ' ' + str(self.fvs.shape[0]) + ' ' + str(self.fvs.shape[1])
            if os.system(command) == 0:
                with open('./partial_f_partial_capital_d.txt') as (file):
                    tmp = []
                    for line in file:
                        matrix = list(map(lambda x: float(x), list(filter(lambda x: x != '', line[:-1].split(' ')))))
                ...
                with open('./partial_f_partial_theta.txt') as (file):
                    tmp = []
                    for line in file:
                        tmp.append(list(map(lambda x: float(x), list(filter(lambda x: x != '', line[:-1].split(' '))))))
        ...
        if command_fail:
            ...
            matrices_1 = pool.map(self._calc_partial_f_partial_capital_d, range(len(self.instances)))
            pool.close()
            pool.join()
    ...
    def _calc_partial_f_partial_theta(self):
        ...
        """
        pool = mp.Pool(mp.cpu_count())
        matrix = pool.map(lambda j: list(map(lambda k: self._calc_partial_f_j_partial_theta_k(j, k), range(len(self.theta)))), range(len(self.theta)))
        pool.close()
        pool.join()
    ...
    def _calc_partial_f_partial_capital_d(self, i):
        ...
        :return: the partial derivative
        """
        matrix = list(map(lambda j: list(map(lambda k: self._calc_partial_f_j_partial_x_k(i, j, k), range(len(self.theta)))), range(len(self.theta))))
        return np.array(matrix)
    ...
    def _calc_partial_f_j_partial_x_k(self, i, j, k):
        ...
        """
        val = self.logistic_vals[i]
        return (1 - val) * (val * self.theta[k] * self.fvs[i][j] - self.labels[i] if j == k else 0)
