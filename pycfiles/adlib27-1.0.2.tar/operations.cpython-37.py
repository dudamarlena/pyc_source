...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/data_reader/operations.py
...
def sparsify(instances: List[Instance]):
    ...
    return (
     labels, csr_matrix((data, indices, indptr), shape=(len(instances), num_features)))
...
def load_dataset(emailData: EmailDataset) -> List[Instance]:
    ...
    for i in range(0, emailData.num_instances):
        if emailData.binary:
            ...
            tmp_vector = RealFeatureVector(num_features, indices[indptr[i]:indptr[(i + 1)]].tolist(), instance_data)
