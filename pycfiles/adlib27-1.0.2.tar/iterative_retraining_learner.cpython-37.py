...
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/adlib/learners/iterative_retraining_learner.py
# Compiled at: 2018-07-25 18:15:23
...
class IterativeRetrainingLearner(Learner):
    ...
    def __init__(self, training_instances: List[Instance], verbose=False):
        ...
        self.lnr = TRIMLearner((self.training_instances), (int(0.75 * len(self.training_instances))),
    ...
    def train(self):
        ...
        self.loss = logistic_loss(self.training_instances, self.lnr) / sum(self.irl_selection)
        sorted_loss = self.loss[:]
        sorted_loss.sort()
        step_size = np.mean(np.array(list(filter(lambda x: x > 0, sorted_loss[1:] - sorted_loss[:-1]))))
        ...
        while self.loss_threshold < max_loss_threshold:
            ...
            if self.verbose:
                ...
                if loss_list[(-2)] == loss_list[(-1)] == loss:
    ...
    def _train_helper(self):
        ...
        while np.linalg.norm(self.irl_selection - old_irl_selection) != 0:
            ...
            if sum(self.irl_selection) < 0.5 * len(self.training_instances):
                raise ValueError()
            if self.verbose:
                print('IRL Iteration:', iteration, '- number of instances:', sum(self.irl_selection))
            self.lnr.irl_selection = self.irl_selection
            self.lnr.train()
            self.loss = logistic_loss(self.training_instances, self.lnr) / sum(self.irl_selection)
            iteration += 1
