...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/validino/ccvalidate.py
...
"""
import re, operator
_numonlyRE = re.compile('[- ]')
cards = (MASTERCARD, VISA, AMEX, DINERS_CLUB, CARTE_BLANCHE, DISCOVER, EN_ROUTE, JCB) = ('MasterCard',
                                                                                         'Visa',
                                                                                         'American Express',
                                                                                         'Diners Club',
                                                                                         'Carte Blanche',
...
def _gen_fake(cctype, start=None, num=1, prefix=None, length=None):
    """
    ...
    if start is None:
        ...
        raise ValueError, 'starting value %s inconsistent with prefix %s and length %s' % (
         start, prefix, length)
    res = []
    while 1:
