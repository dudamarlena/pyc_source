...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/validino/base.py
...
class Schema(object):
    ...
    def __init__(self, subvalidators, msg=None, allow_missing=True, allow_extra=True):
    ...
    def __call__(self, data):
        ...
        if not (self.allow_extra and self.allow_missing):
            ...
            if not self.allow_extra:
                if inputkeys.difference(schemakeys):
                    raise Invalid(_msg(self.msg, 'schema.extra', 'extra keys in input'))
            if not self.allow_missing:
                if schemakeys.difference(inputkeys):
                    raise Invalid(_msg(self.msg, 'schema.missing', 'missing keys in input'))
        ...
        if exceptions:
            raise Invalid(_msg(self.msg, 'schema.error', 'Problems were found in the submitted data.'), exceptions)
        return res
...
def confirm_type(typespec, msg=None):
    ...
    def f(value):
        if isinstance(value, f.typespec):
            return value
        raise Invalid(_msg(f.msg, 'confirm_type', 'unexpected type'))
...
def translate(mapping, msg=None):
    ...
    def f(value):
        try:
            ...
            raise Invalid(_msg(f.msg, 'belongs', 'invalid choice'))
    ...
    def f(value):
        if isinstance(value, unicode):
            ...
            try:
                ...
                raise Invalid(_msg(f.msg, 'to_unicode', 'decoding error'))
...
def is_scalar(msg=None, listtypes=(list,)):
    ...
    def f(value):
        if isinstance(value, f.listtypes):
            raise Invalid(_msg(f.msg, 'is_scalar', 'expected scalar value'))
        return value
...
def is_list(msg=None, listtypes=(list,)):
    ...
    def f(value):
        if not isinstance(value, f.listtypes):
            raise Invalid(_msg(f.msg, 'is_list', 'expected list value'))
        return value
...
def empty(msg=None):
    ...
    def f(value):
        if value == '' or value is None:
            return value
        raise Invalid(_msg(f.msg, 'empty', 'No value was expected'))
        return
...
def not_empty(msg=None):
    ...
    def f(value):
        if value != '' and value != None:
            return value
        raise Invalid(_msg(f.msg, 'notempty', 'A non-empty value was expected'))
        return
...
def clamp(min=None, max=None, msg=None):
    ...
    def f(value):
        if f.min is not None and value < f.min:
            raise Invalid(_msg(f.msg, 'min', 'value below minimum'))
        if f.max is not None and value > f.max:
            raise Invalid(_msg(f.msg, 'max', 'value above maximum'))
        return value
...
def clamp_length(min=None, max=None, msg=None):
    ...
    def f(value):
        ...
        if f.min is not None and vlen < f.min:
            raise Invalid(_msg(f.msg, 'minlen', 'too short'))
        if f.max is not None and vlen > f.max:
            raise Invalid(_msg(f.msg, 'maxlen', 'too long'))
        return value
...
def not_belongs(domain, msg=None):
    ...
    def f(value):
        if value not in f.domain:
            return value
        raise Invalid(_msg(f.msg, 'not_belongs', 'invalid choice'))
...
def parse_time(format, msg=None):
    ...
    def f(value):
        try:
            ...
            raise Invalid(_msg(f.msg, 'parse_time', 'invalid time'))
...
def integer(msg=None):
    ...
    def f(value):
        try:
            ...
            raise Invalid(_msg(f.msg, 'integer', 'not an integer'))
...
def regex(pat, msg=None):
    ...
    def f(value):
        ...
        if not m:
            raise Invalid(_msg(f.msg, 'regex', 'does not match pattern'))
        return value
...
def fields_equal(msg=None, field=None):
    ...
    def f(values):
        if len(set(values)) != 1:
            m = _msg(f.msg, 'fields_equal', 'fields not equal')
            if f.field is None:
                raise Invalid(m)
            else:
...
def fields_match(name1, name2, msg=None, field=None):
    ...
    def f(value):
        if value[f.name1] != value[f.name2]:
            m = _msg(f.msg, 'fields_match', 'fields do not match')
