...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/validino/tests/test_extra.py
...
def test_credit_card_2():
    ...
    validators = dict(cc_card=(V.strip, V.not_empty('Please enter a credit card number')), cc_type=V.belongs(('Visa',
                                                                                                              'Discover'), msg='belongs'))
    v = V.credit_card(require_type=True, cc_field='cc_card', cc_type_field='cc_type')
    validators[('cc_card', 'cc_type')] = v
    s = V.Schema(validators)
    data = dict(cc_card=cc, cc_type='Visa')
    assert s(data) == data
    ...
    data = dict(cc_card=cc, cc_type='Frog')
    try:
        s(data)
    except V.Invalid, e:
        errors = e.unpack_errors()
