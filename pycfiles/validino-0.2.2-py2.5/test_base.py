...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-ppc/egg/validino/tests/test_base.py
...
def test_compose():
    messages = dict(integer='please enter an integer', belongs='invalid choice', min='too small', max='too big')
    v = V.compose(V.default(40), V.strip, V.integer(msg=messages), V.belongs(range(4, 100, 4), messages), V.clamp(min=20, max=50, msg=messages))
    assert v(None) == 40
    assert v('40') == 40
    assert v('44  ') == 44
...
def test_dict_nest():
    d = {'robots.bob.size': 34, 'robots.bob.color': 'blue', 
...
def test_schema_1():
    s = V.Schema(dict(username=(V.strip,
     V.regex('[a-z][a-z0-9]+', 'invalid username'),
     V.clamp_length(max=16, msg='username is too long')), user_id=V.either(V.empty(), V.compose(V.integer('not an integer'), V.clamp(min=1, max=9999, msg='out of range'))), department=(
     V.strip,
     V.belongs(['interactive', 'programming'], 'department not recognized'))), 'there were errors with your submission')
    data = dict(username='jsmullyan', user_id='1', department='interactive')
    newdata = s(data)
...
def test_schema_2():
    s = V.Schema(dict(x=(V.integer('intx'), V.clamp(min=5, max=100, msg='clampx')), y=(
     V.integer('inty'), V.clamp(min=5, max=100, msg='clampy')), text=V.strip), 'schema')
...
def test_schema_3():
    v = V.Schema(dict(x=(V.integer('intx'), V.clamp(min=5, max=100, msg='clampx')), y=(
     V.integer('inty'), V.clamp(min=5, max=100, msg='clampy')), text=V.strip), {'schema.error': 'schema', 'schema.extra': 'extra', 
       'schema.missing': 'missing'}, False, False)
    d1 = dict(x=40, y=20, text='hi there')
    assert v(d1) == d1
    d2 = dict(x=1, y=20, text='hi there')
    assert_invalid(lambda : v(d2), 'schema')
...
def test_excursion():
    ...
    v = V.excursion(lambda x: x.split('@')[0], V.belongs(['gadzooks', 'willy'], msg='pancreatic'))
    assert x == v(x)
...
def test_unpack_1():
    e = V.Invalid({'ding': [V.Invalid('pod')], 'dong': [
              V.Invalid('piddle')]})
    res = e.unpack_errors()
    print res
...
def test_unpack_3():
    errors = dict(frog='My peachy frog hurts', dog='My dog has warts up and down his spine', insect='I would characterize this insect as flawed')
    e = V.Invalid(**errors)
    u = e.unpack_errors()
    print u
