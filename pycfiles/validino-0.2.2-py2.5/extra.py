    ...
    def f(value):
        ...
        if f.check_dns:
            try:
                ...
                raise Invalid(_msg(f.msg, 'email.socket_error', 'socket error'))
            else:
                if not dnsdomains:
                    raise Invalid(_msg(f.msg, 'email.domain_error', 'no such domain'))
        return value
...
def credit_card(types=None, require_type=False, msg=None, cc_field='cc_number', cc_type_field='cc_type'):
    if types is None:
        types = _cc.cards
    ...
    def f(values):
        ...
        if f.require_type and cc_type is None:
            m = _msg(f.msg, 'credit_card.require_type', 'no credit card type specified')
            exc.add_error_message(f.cc_type_field, m)
        elif cc_type is not None and cc_type not in f.types:
            m = _msg(f.msg, 'credit_card.type_check', 'unrecognized credit card type')
            exc.add_error_message(f.cc_type_field, m)
        else:
            type_ok = True
        try:
            ...
            m = _msg(f.msg, 'credit_card.invalid', 'invalid credit card number')
...
_ip_pat = '^%s$' % ('\\.').join([('|').join([ str(x) for x in range(256) ] * 4)])
ip = partial(regex, _ip_pat)
ip.__doc__ = '\n\nReturns a validator that tests whether an ip address is properly formed.\n\n'

def url(check_exists=False, schemas=('http', 'https'), default_schema='http', default_host='', msg=None):
