...
# Embedded file name: build/bdist.linux-x86_64/egg/any2/transformers.py
# Compiled at: 2015-11-04 08:39:23
...
class IndexTransformer(BaseTransformer):
    ...
    def apply(self, value, index=None):
        ...
        """
        if index is None:
            raise TransformationError(("index cannot be None for value '{}'").format(value))
...
class NameTransformer(IndexTransformer):
    ...
    def register_func(self, criterion, func):
        if criterion not in self.colnames:
            raise ColumnMappingError(('Name {} should be in colnames, cannot register function {}').format(criterion, func))
        else:
