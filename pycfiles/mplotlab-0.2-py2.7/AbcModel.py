...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mplotlab\models\AbcModel.py
# Compiled at: 2016-02-07 10:52:12
from mplotlab.utils import checkTypeReturned, checkTypeParams
from copy import copy
from AbcType import AType, STRING
...
class AbcModel(AType):
    ...
    def __init__(self, **k):
        ...
        for name, infos in self.attributeInfos:
            ...
            if not isinstance(value, vclass):
                raise Exception("Attribute default value of '%s'" + "should be an instance of '%s' " % (name, vclass))
    ...
    def getSubEt(clsname, parameter, model, et, **attr):
        return AType.getSubEt(clsname, parameter, model, et, id=str(model.get_id()), name=model.get_name(), **attr)
    ...
    def __str__(self, it=''):
        ...
        for name, (_, vType, _, _) in self.attributeInfos:
            ...
            if issubclass(vType, AbcModel):
                ...
                sep = '\n' + it + '\t' + '-' * 25
                value = sep.join(map(lambda x: x.__str__(it + '\t'), value) + [
                 '\n'])
