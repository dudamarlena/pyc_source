...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\dynn\layers\recurrent_layers.py
# Compiled at: 2018-09-14 09:17:02
...
class ElmanRNN(ParametrizedLayer, RecurrentCell):
    ...
    def init(self, test=False, update=True):
        ...
        if not test:
            if self.dropout > 0:
                self.dropout_mask_x = dy.dropout(dy.ones(self.input_dim), self.dropout)
                self.dropout_mask_h = dy.dropout(dy.ones(self.hidden_dim), self.dropout)

    def __call__(self, x, h):
        r"""Perform the recurrent update.
...
class LSTM(ParametrizedLayer, RecurrentCell):
    ...
    def init(self, test=False, update=True):
        ...
        self.test = test
        if not test:
            if self.dropout_x > 0 or self.dropout_h > 0:
                self.dropout_mask_x = dy.dropout(dy.ones(self.input_dim), self.dropout_x)
                self.dropout_mask_h = dy.dropout(dy.ones(self.hidden_dim), self.dropout_h)
    ...
    def __call__(self, x, h, c):
        ...
        if not self.test:
            if self.dropout_x > 0 or self.dropout_h > 0:
                gates = dy.vanilla_lstm_gates_dropout(x, h, self.Whx, self.Whh, self.b, self.dropout_mask_x, self.dropout_mask_h)
        else:
