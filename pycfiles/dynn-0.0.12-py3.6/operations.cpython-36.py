...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\dynn\operations.py
# Compiled at: 2018-09-19 16:39:00
...
def squeeze(x, d=0):
    ...
    if d < 0:
        d += len(dim)
    if d < 0 or d >= len(dim):
        raise ValueError(f"Dimension {d} out of bounds for {len(dim)}-dimensional expression")
    if dim[d] != 1:
        raise ValueError(f"Trying to squeeze dimension {d} of size {dim[d]}!=1")
...
def unsqueeze(x, d=0):
    ...
    if d < 0:
        d += len(dim) + 1
    if d < 0 or d > len(dim):
        raise ValueError(f"Cannot insert dimension at position {d} out of bounds for {len(dim)}-dimensional expression")
    new_dim = list(dim)
    new_dim.insert(d, 1)
