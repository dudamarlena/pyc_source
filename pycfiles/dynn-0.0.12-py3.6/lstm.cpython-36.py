...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\dynn\layers\lstm.py
# Compiled at: 2018-09-13 14:35:40
# Size of source mod 2**32: 9477 bytes
"""FIXME"""
...
class NaryTreeLSTMCell(BaseLayer):
    ...
    def __init__(self, arity, input_dim, hidden_dim, pc, dropout=0.0, diagonal=False):
        ...
        self.Wg_p = self.pc.add_parameters((
         self.dh, self.incoming_dim),
          name='Wg')
        self.Wf_p = [self.pc.add_parameters((self.dh, self.diagonal_dim), name=f"Wf{i}") for i in range(self.arity)]
        self.bi_p = self.pc.add_parameters((self.dh),
          name='bi', init=(ZeroInit()))
        self.bo_p = self.pc.add_parameters((self.dh),
          name='bo', init=(ZeroInit()))
        self.bg_p = self.pc.add_parameters((self.dh),
          name='bg', init=(ZeroInit()))
        self.bf_p = [self.pc.add_parameters((self.dh), name=f"bf{i}", init=(OneInit())) for i in range(self.arity)]
    ...
    def _check_args_arity(self, args):
        ...
        if not (good_with_input or good_without_input):
            raise ValueError(f"{nargs} arguments for {self.arity}-ary TreeLSTM (should be 2 * arity + 1)")
    ...
    def __call__(self, *args):
        ...
        if not self.test:
            ...
            if self.diagonal:
                self.f = [dy.logistic(dy.affine_transform([self.bf[i], self.Wf[i], h_m1[i]])) for i in range(self.arity)]
            else:
                self.f = [dy.logistic(dy.affine_transform([self.bf[i], self.Wf[i], h_m1])) for i in range(self.arity)]
        self.c = dy.cmult(self.i, self.g)
        if self.arity > 0:
            self.c += dy.esum([dy.cmult(self.f[i], c_m1[i]) for i in range(self.arity)])
...
class StackedLSTM(BaseLayer):
    __doc__ = 'Wrapper for stacked LSTM layers\n\n    Takes in a sequence of LSTM cells\n    '
...
def transduce_bilstm(lstm_forward, lstm_backward, xs, h0_fwd, c0_fwd, h0_bwd, c0_bwd, lengths=None):
    ...
    Returns:
        tuple: hs_fwd, hs_bwd: list of output states of the forward
            and backward pass
    """
    hs_fwd = transduce_lstm(lstm_forward,
      xs, h0_fwd, c0_fwd, lengths=lengths, backward=False)
    hs_bwd = transduce_lstm(lstm_backward,
      xs, h0_bwd, c0_bwd, lengths=lengths, backward=True)
    return (
     hs_fwd, hs_bwd)