...
# [GCC 8.4.0]
# Embedded file name: /home/cufrancis/Develop/pelicantool/.env/lib/python3.6/site-packages/pelicantool/article.py
# Compiled at: 2018-01-13 04:30:15
...
class Article(ActionInterface):
    ...
    def get_article_attrs(self):
        ...
        data['slug'] = ask('Slug:', answer=str_compat, default=(translation(data['title'])))
        data['author'] = ask('Author:', answer=str_compat, default=(self.args['Article']['author']))
        date_format = self.args.get('date_format', '%Y-%m-%d %H:%M:%S')
        data['date'] = ask('Date:', answer=str_compat, default=(datetime.datetime.now().strftime(date_format)))
        data['modified'] = ask('Modified Date:', answer=str_compat, default=(datetime.datetime.now().strftime(date_format)))
        default_file_name = (datetime.datetime.now().strftime(self.args['filename_format']).format)(**data)
        data['file_name'] = ask('Default file name:', answer=str_compat, default=default_file_name)
