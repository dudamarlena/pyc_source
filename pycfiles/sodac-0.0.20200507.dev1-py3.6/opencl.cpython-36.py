...
def add_arguments(parser):
    ...
    parser.add_argument('--xocl-part-num',
      type=str, dest='xocl_part_num', metavar='string', help='part number used for Vivado HLS')
    parser.add_argument('--xocl-clock-period',
      type=str, dest='xocl_clock_period', metavar='ns', help='target clock period in nanoseconds, used for Vivado HLS')
...
def print_code(stencil: core.Stencil, args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    if args.kernel_file is not None:
        ...
        if args.xo_file is not None:
            with tempfile.TemporaryFile(mode='w+b') as (tmp_obj):
                rtl_kernel.print_code(stencil,
                  tmp_obj,
                  device_info=backend.parse_device_info(parser,
                  args,
                ...
                tmp_obj.seek(0)
                if args.xo_file == '-':
                    shutil.copyfileobj(tmp_obj, sys.stdout)
                else:
