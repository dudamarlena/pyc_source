...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/codegen/xilinx/host.py
# Compiled at: 2020-05-07 15:50:46
# Size of source mod 2**32: 55366 bytes
...
def print_header(printer):
    for header in ('assert', 'float', 'math', 'stdbool', 'stddef', 'stdint', 'stdio',
                   'stdlib', 'string'):
    ...
    printer.println()
    for header in ('ap_int', 'fcntl', 'time', 'unistd', 'sys/types', 'sys/stat', 'CL/opencl'):
        printer.println('#include <%s.h>' % header)

    printer.println()
    for name in ('std::array', 'std::count', 'std::fill', 'std::string', 'std::unordered_map'):
...
def print_load_xclbin2(printer):
    ...
    println()
    println('int64_t load_xclbin2_to_memory(const char *filename, char **kernel_binary, char** device)')
    do_scope()
    ...
    println('uint64_t size_left = size - sizeof(magic) - sizeof(cipher) - sizeof(key_block) - sizeof(unique_id) - sizeof(size);')
...
def print_halide_rewrite_buffer(printer):
    ...
    for dim in range(4):
        println('                  int32_t min{0}, int32_t extent{0}, int32_t stride{0}{1}'.format(dim, ',,,)'[dim]))
...
def print_halide_error_codes(printer):
    i = 0
    for item in ('success', 'generic_error', 'explicit_bounds_too_small', 'bad_elem_size',
                 'access_out_of_bounds', 'buffer_allocation_too_large', 'buffer_extents_too_large',
                 'constraints_make_required_region_smaller', 'constraint_violated',
                 'param_too_small', 'param_too_large', 'out_of_memory', 'buffer_argument_is_null',
                 'debug_to_file_failed', 'copy_to_host_failed', 'copy_to_device_failed',
                 'device_malloc_failed', 'device_sync_failed', 'device_free_failed',
                 'no_device_interface', 'matlab_init_failed', 'matlab_bad_param_type',
                 'internal_error', 'device_run_failed', 'unaligned_host_ptr', 'bad_fold',
...
def print_halide_error_report(printer):
    println = printer.println
    println('int halide_error_bad_elem_size(void *user_context, const char *func_name,')
    println('                 const char *type_name, int elem_size_given, int correct_elem_size) {')
    println('  fprintf(*error_report, "%s has type %s but elem_size of the buffer passed in is %d instead of %d",')
    println('      func_name, type_name, elem_size_given, correct_elem_size);')
    println('  return halide_error_code_bad_elem_size;')
    println('}')
    println('int halide_error_constraint_violated(void *user_context, const char *var, int val,')
    println('                   const char *constrained_var, int constrained_val) {')
    ...
    println('}')
    println('int halide_error_buffer_allocation_too_large(void *user_context, const char *buffer_name, uint64_t allocation_size, uint64_t max_size) {')
    println('  fprintf(*error_report, "Total allocation for buffer %s is %lu, which exceeds the maximum size of %lu",')
    println('      buffer_name, allocation_size, max_size);')
    println('  return halide_error_code_buffer_allocation_too_large;')
    println('}')
    println('int halide_error_buffer_extents_too_large(void *user_context, const char *buffer_name, int64_t actual_size, int64_t max_size) {')
    println('  fprintf(*error_report, "Product of extents for buffer %s is %ld, which exceeds the maximum size of %ld",')
    println('      buffer_name, actual_size, max_size);')
    println('  return halide_error_code_buffer_extents_too_large;')
    println('}')
    println('int halide_error_access_out_of_bounds(void *user_context, const char *func_name, int dimension, int min_touched, int max_touched, int min_valid, int max_valid) {')
    println('  if(min_touched < min_valid) {')
    println('    fprintf(*error_report, "%s is accessed at %d, which is before the min (%d) in dimension %d", func_name, min_touched, min_valid, dimension);')
    println('  } else if(max_touched > max_valid) {')
    println('    fprintf(*error_report, "%s is acccessed at %d, which is beyond the max (%d) in dimension %d", func_name, max_touched, max_valid, dimension);')
    println('  }')
...
def print_wrapped(printer, stencil):
    ...
    tensors = [(_.name, _.c_type) for _ in stencil.input_stmts] + [(_.name, _.c_type) for _ in stencil.output_stmts] + [(_.name, _.c_type) for _ in stencil.param_stmts]
    ...
    for output_name in stencil.output_names:
        ...
        for dim in range(stencil.dim):
            stride = '1'
            if dim > 0:
                stride = '*'.join('%s_size_dim_%d' % (output_name, d) for d in range(dim))
            output_str[dim] = ', var_{0}_min_{1}, {0}_size_dim_{1}, {2}'.format(output_name, dim, stride)
    ...
    for input_name in stencil.input_names:
        ...
        for dim in range(stencil.dim):
            println('int32_t %s = %s_size_dim_%d + %d;' % (
             ...
             core.get_stencil_dim(core.get_overall_stencil_window(stencil.tensors[stencil.input_names[0]], stencil.tensors[stencil.output_names[0]]))[dim] - 1))
        ...
        for dim in range(stencil.dim):
            ...
            input_str[dim] = ', var_%s_min_%d, %s, %s' % (
    ...
    println('bool %s = %s;' % (new_var(),
     ' || '.join('var_%s_host_and_dev_are_null' % _ for _ in stencil.output_names + stencil.input_names)))
    ...
    for d in range(stencil.dim - 1):
        println('int32_t tile_num_dim_{d} = ({}_size_dim_{d}-STENCIL_DIM_{d}+1+TILE_SIZE_DIM_{d}-STENCIL_DIM_{d})/(TILE_SIZE_DIM_{d}-STENCIL_DIM_{d}+1);'.format((stencil.input_names[0]),
          d=d))

    println()

    def for_banks():
        return printer.for_('auto bank', '{{{}}}'.format(', '.join(map(str, range(soda_util.MAX_DRAM_BANK)))))

    println('// change #bank if there is a env var defined')
    println('unordered_map<string, array<bool, %d>> use_bank;' % soda_util.MAX_DRAM_BANK)
    println('unordered_map<string, int> num_bank;')
    println('unordered_map<string, array<int, %d>> bank_vec;' % soda_util.MAX_DRAM_BANK)
    for stmt in stencil.output_stmts + stencil.input_stmts + stencil.param_stmts:
        name = stmt.name
        println('use_bank["{}"] = {{{}}};'.format(name, ', '.join(('true' if _ in stmt.dram else 'false') for _ in range(soda_util.MAX_DRAM_BANK))))
        println('num_bank["{}"] = {{{}}};'.format(name, len(stmt.dram)))
        println('bank_vec["{}"] = {{{}}};'.format(name, ', '.join(map(str, stmt.dram))))
            ...
            with printer.if_("env_var.find(':') != string::npos"):
                ...
                with printer.do_while('next_pos != string::npos'):
                    ...
                    with printer.if_('use_bank.count(name)'):
                        ...
                        with printer.do_while('dot_pos != string::npos && dot_pos < next_pos'):
                        ...
                        with printer.else_():
                            println('fprintf(*error_report, "WARNING: unknown name %s\\n", name.c_str());')
                ...
                with printer.else_():
                    ...
                    with printer.for_('const string name', '{%s}' % ', '.join(map('"{}"'.format, var_names))):
                        println('fill(use_bank[name].begin(), use_bank[name].end(), false);')
                        println('num_bank[name] = 0;')
                        println('int idx = 0;')
                        println('size_t pos = 0;')
    ...
    println('int64_t tile_burst_num = (tile_pixel_num-1)/(BURST_WIDTH/%d*num_bank["%s"])+1;' % (
     stencil.input_stmts[0].haoda_type.width_in_bits,
     stencil.input_names[0]))
    println('int64_t tile_size_linearized_i = tile_burst_num*(BURST_WIDTH/{stmt.width_in_bits}*num_bank["{stmt.name}"]);'.format(stmt=(stencil.input_stmts[0])))
    println('int64_t tile_size_linearized_o = tile_burst_num*(BURST_WIDTH/{stmt.width_in_bits}*num_bank["{stmt.name}"]);'.format(stmt=(stencil.output_stmts[0])))
    println()
    ...
    for name in stencil.input_names + stencil.output_names:
        println('cl_mem var_%s_cl_bank[%d] = {};' % (name, soda_util.MAX_DRAM_BANK))
    ...
    for stmt in stencil.input_stmts:
        println('uint64_t var_{stmt.name}_buf_size = sizeof({stmt.c_type})*({}*tile_size_linearized_i/num_bank["{stmt.name}"]+((STENCIL_DISTANCE-1)/(BURST_WIDTH/{stmt.width_in_bits}*num_bank["{stmt.name}"])+1)*(BURST_WIDTH/{stmt.width_in_bits}));'.format(('*'.join('tile_num_dim_%d' % _ for _ in range(stencil.dim - 1))),
          stmt=stmt))

    for stmt in stencil.output_stmts:
        println('uint64_t var_{stmt.name}_buf_size = sizeof({stmt.c_type})*({}*tile_size_linearized_o/num_bank["{stmt.name}"]+((STENCIL_DISTANCE-1)/(BURST_WIDTH/{stmt.width_in_bits}*num_bank["{stmt.name}"])+1)*(BURST_WIDTH/{stmt.width_in_bits}));'.format(('*'.join('tile_num_dim_%d' % x for x in range(stencil.dim - 1))),
          stmt=stmt))
    ...
    println()
    println('int status;')
    println('size_t kernel_binary_sizes[1] = {static_cast<size_t>(kernel_binary_size)};')
    ...
    println('unordered_map<string, array<cl_mem_ext_ptr_t, %d>> mem_ext_bank;' % soda_util.MAX_DRAM_BANK)
    with printer.for_('const auto& use_bank_pair', 'use_bank'):
        with for_banks():
            with printer.if_('use_bank_pair.second[bank]'):
                println('mem_ext_bank[use_bank_pair.first][bank].flags = XCL_MEM_DDR_BANK[bank];')
    with for_banks():
        for name in stencil.input_names:
            with printer.if_('use_bank["{name}"][bank]'.format(name=name)):
                println('var_{name}_cl_bank[bank] = clCreateBuffer(context, CL_MEM_READ_ONLY|CL_MEM_EXT_PTR_XILINX, var_{name}_buf_size, &mem_ext_bank["{name}"][bank], nullptr);'.format(name=name))
                with printer.if_('var_{name}_cl_bank[bank] == nullptr'.format(name=name)):
                    println('fprintf(*error_report, "ERROR: Failed to allocate device memory\\n");')
                    println('exit(EXIT_FAILURE);')

        for name in stencil.output_names:
            with printer.if_('use_bank["{name}"][bank]'.format(name=name)):
                println('var_{name}_cl_bank[bank] = clCreateBuffer(context, CL_MEM_WRITE_ONLY|CL_MEM_EXT_PTR_XILINX, var_{name}_buf_size, &mem_ext_bank["{name}"][bank], nullptr);'.format(name=name))
                with printer.if_('var_{name}_cl_bank[bank] == nullptr'.format(name=name)):
                    println('fprintf(*error_report, "ERROR: Failed to allocate device memory\\n");')
                    println('exit(EXIT_FAILURE);')

    for param in stencil.param_stmts:
        println('var_%s_cl = clCreateBuffer(context, CL_MEM_READ_ONLY, %s*sizeof(%s), nullptr, nullptr);' % (
    ...
    println()
    println('cl_event write_events[%d];' % (soda_util.MAX_DRAM_BANK * len(stencil.input_stmts) + len(stencil.param_stmts)))
    println('cl_event* write_event_ptr = write_events;')
    for param in stencil.param_stmts:
        println('%s* var_%s_buf = (%s*)clEnqueueMapBuffer(commands, var_%s_cl, CL_FALSE, CL_MAP_WRITE, 0, %s*sizeof(%s), 0, nullptr, write_event_ptr++, &err);' % (
         param.c_type, param.name, param.c_type, param.name,
         '*'.join(map(str, param.size)), param.c_type))

    for stmt in stencil.input_stmts:
        println('{stmt.c_type}* var_{stmt.name}_buf_bank[{}] = {{}};'.format((soda_util.MAX_DRAM_BANK),
          stmt=stmt))

    for stmt in stencil.input_stmts:
        with for_banks():
            with printer.if_('use_bank["{name}"][bank]'.format(name=(stmt.name))):
                println('var_{stmt.name}_buf_bank[bank] = reinterpret_cast<{stmt.c_type}*>(clEnqueueMapBuffer(commands, var_{stmt.name}_cl_bank[bank], CL_FALSE, CL_MAP_WRITE, 0, var_{stmt.name}_buf_size, 0, nullptr, write_event_ptr++, &err));'.format(stmt=stmt))

    println('clWaitForEvents(write_event_ptr - write_events, write_events);')
    println()
    println('// tiling')
    for dim in range(stencil.dim - 2, -1, -1):
        println('for(int32_t tile_index_dim_{0} = 0; tile_index_dim_{0} < tile_num_dim_{0}; ++tile_index_dim_{0})'.format(dim))
        do_scope()
        println('int32_t actual_tile_size_dim_{0} = (tile_index_dim_{0}==tile_num_dim_{0}-1) ? {1}_size_dim_{0}-(TILE_SIZE_DIM_{0}-STENCIL_DIM_{0}+1)*tile_index_dim_{0} : TILE_SIZE_DIM_{0};'.format(dim, stencil.input_names[0]))

    println('#pragma omp parallel for', 0)
    println('for(int32_t {0} = 0; {0} < {1}_size_dim_{2}; ++{0})'.format(soda_util.COORDS_IN_TILE[(stencil.dim - 1)], stencil.input_names[0], stencil.dim - 1))
    do_scope()
    for dim in range(stencil.dim - 2, -1, -1):
        println('for(int32_t {0} = 0; {0} < actual_tile_size_dim_{1}; ++{0})'.format(soda_util.COORDS_IN_TILE[dim], dim))
        do_scope()

    println('// (%s) is coordinates in tiled image' % ', '.join(soda_util.COORDS_TILED))
    println('// (%s) is coordinates in original image' % ', '.join(soda_util.COORDS_IN_ORIG))
    println('// (%s) is coordinates in a tile' % ', '.join(soda_util.COORDS_IN_TILE))
    offset_in_tile = '+'.join('%c%s' % (soda_util.COORDS_IN_TILE[x],
     ''.join('*TILE_SIZE_DIM_%d' % xx for xx in range(x))) for x in range(stencil.dim))
    println('int32_t burst_index = ({}) / (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]);'.format(offset_in_tile,
      stmt=(stencil.input_stmts[0])))
    println('int32_t burst_residue = ({}) % (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]);'.format(offset_in_tile,
      stmt=(stencil.input_stmts[0])))
    for dim in range(stencil.dim - 1):
        println('int32_t {0} = tile_index_dim_{1}*(TILE_SIZE_DIM_{1}-STENCIL_DIM_{1}+1)+{2};'.format(soda_util.COORDS_IN_ORIG[dim], dim, soda_util.COORDS_IN_TILE[dim]))

    println('int32_t %c = %c;' % (soda_util.COORDS_IN_ORIG[(stencil.dim - 1)],
     soda_util.COORDS_IN_TILE[(stencil.dim - 1)]))
    println('int64_t tiled_offset = ({}) * tile_size_linearized_i + burst_index * (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]) + burst_residue;'.format(('+'.join('%stile_index_dim_%d' % (
     ''.join('tile_num_dim_%d*' % xx for xx in range(x)), x) for x in range(stencil.dim - 1))),
      stmt=(stencil.input_stmts[0])))
    println('int64_t original_offset = %s;' % '+'.join('%c*var_%s_stride_%d' % (soda_util.COORDS_IN_ORIG[x], stencil.input_names[0], x) for x in range(stencil.dim)))
    for name in stencil.input_names:
        println('var_{name}_buf_bank[bank_vec["{name}"][tiled_offset % num_bank["{name}"]]][tiled_offset / num_bank["{name}"]] = var_{name}[original_offset];'.format(name=name))
    ...
    println()
    for param in stencil.param_stmts:
        println('memcpy(var_{0}_buf, var_{0}, {1}*sizeof({2}));'.format(param.name, '*'.join(map(str, param.size)), param.c_type))
    ...
    if stencil.param_stmts:
        ...
        for idx, param in enumerate(stencil.param_stmts):
            println('err |= clEnqueueUnmapMemObject(commands, var_{0}_cl, var_{0}_buf, 0, nullptr, write_events+{1});'.format(param.name, idx))

        for name in stencil.input_names:
            println('write_event_ptr = write_events;')
            with for_banks():
                with printer.if_('use_bank["{}"][bank]'.format(name)):
                    println('err |= clEnqueueUnmapMemObject(commands, var_{name}_cl_bank[bank], var_{name}_buf_bank[bank], 0, nullptr, write_event_ptr++);'.format(name=name))
        ...
        for d in range(stencil.dim - 1):
            println('fprintf(*error_report, "INFO: tile_num_dim_{0} = %d, TILE_SIZE_DIM_{0} = %d\\n", tile_num_dim_{0}, TILE_SIZE_DIM_{0});'.format(d))
        ...
        println('int64_t tile_data_num = ((int64_t({stmt.name}_size_dim_{dim}){} - 1) / (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]) + 1) * BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"] / UNROLL_FACTOR;'.format((''.join(' * TILE_SIZE_DIM_%d' % _ for _ in range(stencil.dim - 1))),
          stmt=(stencil.input_stmts[0]),
          dim=(stencil.dim - 1)))
        println('int64_t coalesced_data_num = ((int64_t({stmt.name}_size_dim_{dim}){} * {} + STENCIL_DISTANCE - 1) / (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]) + 1);'.format((''.join('*TILE_SIZE_DIM_%d' % _ for _ in range(stencil.dim - 1))),
          ('*'.join('tile_num_dim_%d' % _ for _ in range(stencil.dim - 1))),
          stmt=(stencil.input_stmts[0]),
          dim=(stencil.dim - 1)))
        println('fprintf(*error_report, "INFO: tile_data_num = %ld, coalesced_data_num = %ld\\n", tile_data_num, coalesced_data_num);')
        println()
        for name in stencil.output_names + stencil.input_names:
            with for_banks():
                with printer.if_('use_bank["{name}"][bank]'.format(name=name)):
                    println('err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_%s_cl_bank[bank]);' % name)

        for name in stencil.param_names:
            println('err |= clSetKernelArg(kernel, kernel_arg++, sizeof(cl_mem), &var_%s_cl);' % name)

        println('err |= clSetKernelArg(kernel, kernel_arg++, sizeof(coalesced_data_num), &coalesced_data_num);')
        println('if(err != CL_SUCCESS)')
        do_scope()
        ...
        println('err = clEnqueueTask(commands, kernel, write_event_ptr - write_events, write_events, &execute_event);')
        with printer.if_('getenv("XCL_EMULATION_MODE") == nullptr'):
            ...
            println('elapsed_time = (double(execute_end.tv_sec-execute_begin.tv_sec)+(execute_end.tv_nsec-execute_begin.tv_nsec)/1e9)*1e6;')
            println('printf("Kernel execution time: %lf us\\n", elapsed_time);')
            println('printf("Kernel throughput:   %%lf pixel/ns\\n", %s / elapsed_time / 1e3);' % '*'.join('%s_size_dim_%d' % (stencil.input_names[0], _) for _ in range(stencil.dim)))
            with printer.else_():
                println('clWaitForEvents(1, &execute_event);')
                println('fprintf(*error_report, "INFO: Emulation mode\\n");')
        println()
        println('cl_event read_events[%d];' % (soda_util.MAX_DRAM_BANK * len(stencil.output_stmts)))
        ...
        for stmt in stencil.output_stmts:
            with for_banks():
                with printer.if_('use_bank["{name}"][bank]'.format(name=(stmt.name))):
                    println('var_{stmt.name}_buf_bank[bank] = reinterpret_cast<{stmt.c_type}*>(clEnqueueMapBuffer(commands, var_{stmt.name}_cl_bank[bank], CL_FALSE, CL_MAP_READ, 0, var_{stmt.name}_buf_size, 0, nullptr, read_event_ptr++, &err));'.format(stmt=stmt))
        ...
        if stencil.preserve_border:
            overall_stencil_window = util.get_overall_stencil_window(stencil.output.parent.preserve_border_from(), stencil.output)
            overall_stencil_offset = util.get_stencil_window_offset(overall_stencil_window)
            overall_stencil_dim = util.get_stencil_dim(overall_stencil_window)
            println('for(int32_t {var} = 0; {var} < {0}_size_dim_{1}; ++{var})'.format((stencil.input.name),
              (stencil.dim - 1),
              var=(soda_util.COORDS_IN_TILE[(stencil.dim - 1)])))
            do_scope()
            for dim in range(stencil.dim - 2, -1, -1):
                println('for(int32_t {var} = tile_index_dim_{dim}==0 ? 0 : {0}; {var} < actual_tile_size_dim_{dim}-(tile_index_dim_{dim}==tile_num_dim_{dim}-1 ? 0 : {1}); ++{var})'.format((overall_stencil_offset[dim]),
            ...
            overall_stencil_window = core.get_overall_stencil_window(stencil.tensors[stencil.input_names[0]], stencil.tensors[stencil.output_names[0]])
            ...
            println('#pragma omp parallel for', 0)
            println('for(int32_t {var} = {}; {var} < {}_size_dim_{}-{}; ++{var})'.format((overall_stencil_offset[(stencil.dim - 1)]),
              (stencil.output_names[0]),
              (stencil.dim - 1),
              (overall_stencil_dim[(stencil.dim - 1)] - 1 - overall_stencil_offset[(stencil.dim - 1)]),
              var=(soda_util.COORDS_IN_TILE[(stencil.dim - 1)])))
            do_scope()
            for dim in range(stencil.dim - 2, -1, -1):
                println('for(int32_t {var} = {}; {var} < actual_tile_size_dim_{}-{}; ++{var})'.format((overall_stencil_offset[dim]),
                  dim,
                  (overall_stencil_dim[dim] - 1 - overall_stencil_offset[dim]),
                  var=(soda_util.COORDS_IN_TILE[dim])))
                do_scope()
    ...
    println('int32_t %c = %c;' % (soda_util.COORDS_IN_ORIG[(stencil.dim - 1)],
     soda_util.COORDS_IN_TILE[(stencil.dim - 1)]))
    println('int64_t original_offset = %s;' % '+'.join('%c*var_%s_stride_%d' % (soda_util.COORDS_IN_ORIG[x], stencil.output_names[0], x) for x in range(stencil.dim)))
    for stmt in stencil.output_stmts:
        overall_stencil_window = core.get_overall_stencil_window(map(stencil.tensors.get, stencil.input_names), stencil.tensors[stmt.name])
        overall_stencil_distance = core.get_stencil_distance(overall_stencil_window, stencil.tile_size)
        stencil_offset = overall_stencil_distance - soda_util.serialize(core.get_stencil_window_offset(overall_stencil_window), stencil.tile_size)
        println('int32_t burst_index_{stmt.name} = ({} + {}) / (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]);'.format(offset_in_tile,
          stencil_offset,
          stmt=stmt))
        println('int32_t burst_residue_{stmt.name} = ({} + {}) % (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]);'.format(offset_in_tile,
          stencil_offset, stmt=stmt))
        println('int64_t tiled_offset_{stmt.name} = ({}) * tile_size_linearized_o + burst_index_{stmt.name} * (BURST_WIDTH / {stmt.width_in_bits} * num_bank["{stmt.name}"]) + burst_residue_{stmt.name};'.format(('+'.join('%stile_index_dim_%d' % (
         ''.join('tile_num_dim_%d*' % xx for xx in range(x)), x) for x in range(stencil.dim - 1))),
          stmt=stmt))
        println('var_{name}[original_offset] = var_{name}_buf_bank[bank_vec["{name}"][tiled_offset_{name} % num_bank["{name}"]]][tiled_offset_{name} / num_bank["{name}"]];'.format(name=(stmt.name)))
    ...
    for name in stencil.output_names:
        ...
        with for_banks():
            with printer.if_('use_bank["{name}"][bank]'.format(name=name)):
                println('clEnqueueUnmapMemObject(commands, var_{name}_cl_bank[bank], var_{name}_buf_bank[bank], 0, nullptr, read_event_ptr++);'.format(name=name))
        println('clWaitForEvents(read_event_ptr - read_events, read_events);')
...
def print_entrance(printer, stencil):
    println = printer.println
    tensors = [(stmt.name, stmt.c_type) for stmt in stencil.input_stmts + stencil.output_stmts + stencil.param_stmts]
    println('int %s(%sconst char* xclbin) HALIDE_FUNCTION_ATTRS' % (
     stencil.app_name,
     ''.join('buffer_t *var_%s_buffer, ' % x[0] for x in tensors)))
    printer.do_scope()
    for tensor in tensors:
        print_unload_buffer(printer, *tensor)

    println('return %s_wrapped(%sxclbin);' % (stencil.app_name,
     ''.join('var_%s_buffer, ' % x[0] for x in tensors)))
    printer.un_scope()
    println()
...
def print_unload_buffer(printer, buffer_name, buffer_type):
    ...
    println('const bool var_{0}_host_and_dev_are_null = (var_{0}_buffer->host == nullptr) && (var_{0}_buffer->dev == 0);'.format(buffer_name))
    ...
    println('int32_t var_{0}_elem_size = var_{0}_buffer->elem_size;'.format(buffer_name))
...
def print_check_elem_size(printer, buffer_name, buffer_type):
    ...
    println('if(!%s)' % last_var())
    printer.do_scope()
    println('int32_t %s = halide_error_bad_elem_size(nullptr, "Buffer %s", "%s", var_%s_elem_size, %d);' % (
...
def print_test(printer, stencil):
    ...
    println('int %s_test(const char* xclbin, const int dims[4])' % stencil.app_name)
    do_scope()
    ...
    for name in stencil.input_names:
        ...
        for d in range(0, stencil.dim):
            dim = stencil.dim - d - 1
            println('for(int32_t {var} = 0; {var}<dims[{}]; ++{var})'.format(dim,
        ...
        println('%s_img[%s] = %s;' % (
         name,
         '+'.join('%c*%s.stride[%d]' % (soda_util.COORDS_IN_ORIG[d], name, d) for d in range(input_dim)), init_val))
    ...
    for param in stencil.param_stmts:
        ...
        for dim, size in enumerate(param.size):
            println('for(int32_t {var} = 0; {var}<{}; ++{var})'.format(size,
              var=(soda_util.COORDS_IN_ORIG[dim])))
            do_scope()

        println('%s_img%s = %s;' % (
         param.name,
         sum(('[%c]' % soda_util.COORDS_IN_ORIG[d] for d in range(len(param.size))), ''),
    ...
    println('{}({}xclbin);'.format(stencil.app_name, ''.join(map('&{}, '.format, stencil.input_names + stencil.output_names + stencil.param_names))))
    println()
    ...
    for tensor in stencil.chronological_tensors:
        if tensor.is_input():
            ...
            println('#pragma omp parallel for', 0)
            stencil_window = core.get_overall_stencil_window(tuple(map(stencil.tensors.get, stencil.input_names)), tensor)
            stencil_dim = core.get_stencil_dim(stencil_window)
            output_idx = core.get_stencil_window_offset(stencil_window)
            for d in range(0, stencil.dim):
                dim = stencil.dim - d - 1
                println('for(int32_t {var} = {}; {var}<dims[{}]-{}; ++{var})'.format((output_idx[dim]),
                  dim,
            ...
            def mutate_load_for_host(obj, args):
                if isinstance(obj, ir.Ref):
                    if obj.name in stencil.param_names:
                        return ir.make_var(('%s_img%s' % (obj.name, ''.join('[%d]' % _ for _ in obj.idx))),
                          haoda_type=(obj.haoda_type))
                    return ir.make_var(('%s_img[%s]' % (
                     obj.name,
                     '+'.join('(%c%+d)*%s.stride[%d]' % (soda_util.COORDS_IN_ORIG[d], obj.idx[d] - tensor.st_ref.idx[d], obj.name, d) for d in range(stencil.dim)))),
                      haoda_type=(obj.haoda_type))
                else:
                    return obj
            ...
            def mutate_store_for_host(obj, args):
                if isinstance(obj, ir.Ref):
                    ...
                    return ir.make_var(('%s_img[%s]' % (
                     obj.name,
                     '+'.join('%c*%s.stride[%d]' % (soda_util.COORDS_IN_ORIG[d], obj.name, d) for d in range(stencil.dim)))),
                      haoda_type=(obj.haoda_type))
                else:
                    return obj
            ...
            for let in tensor.lets:
                ...
                println('const {} {} = {};'.format(let.c_type, let.name, let.expr.visit(mutate_load_for_host).c_expr))

            println('// {} = {}'.format(tensor.st_ref, tensor.expr))
            println('{} = {};'.format(tensor.st_ref.visit(mutate_store_for_host), tensor.expr.visit(mutate_load_for_host).c_expr))
            if tensor.is_output():
                run_result = '%s_img[%s]' % (tensor.name,
                 '+'.join('%c*%s.stride[%d]' % (soda_util.COORDS_IN_ORIG[d], tensor.name, d) for d in range(stencil.dim)))
                ...
                if tensor.haoda_type.is_float:
                    ...
                    println('if(double(val_fpga-val_cpu)*double(val_fpga-val_cpu) > threshold && double(val_fpga-val_cpu)*double(val_fpga-val_cpu)/(double(val_cpu)*double(val_cpu)) > threshold)')
                    do_scope()
    ...
    un_scope()
    println()
    for var in stencil.input_names + stencil.local_names + stencil.output_names + stencil.param_names:
...
def print_code(stencil, host_file):
    ...
    if stencil.preserve_border:
        ...
        overall_stencil_window = core.get_overall_stencil_window(map(stencil.tensors.get, stencil.input_names), stencil.tensors[stencil.output_names[0]])
    overall_stencil_distance = core.get_stencil_distance(overall_stencil_window, stencil.tile_size)
