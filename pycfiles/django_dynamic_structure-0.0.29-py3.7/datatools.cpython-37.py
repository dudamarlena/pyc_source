...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dyn_struct/datatools.py
...
def check_class_arguments(cls, kwargs):
    ...
    available_arg_names = set([param for param in classes_params if param not in ('args',
                                                                                  'kwargs',
                                                                                  'self')])
    kwargs_names = set(kwargs.keys())
    error_keys = kwargs_names - available_arg_names
    if error_keys:
        raise CheckClassArgumentsException('{0} - неизвестные ключи для {2}. Выбирайте необходимые из {1}'.format(', '.join(error_keys), ', '.join(available_arg_names), cls))


def structure_to_dict(struct, is_compact=True):
    struct_info = {'name':struct.name, 
...
def structure_from_dict(struct_info, use_local_version=False, struct_name=None):
    ...
    if use_local_version:
        ...
        created or struct.clone()
        struct = models.DynamicStructure.objects.get(name=name)
    else:
        version = struct_info.get('version', 1)
        struct, _ = models.DynamicStructure.objects.get_or_create(name=name,
          version=version,
          is_deprecated=(struct_info.get('is_deprecated', False)))
    for field_info in struct_info['fields']:
        (models.DynamicStructureField.objects.get_or_create)(structure=struct, **field_info)
...
def get_structure_fields_data(struct):
    ...
    for field in struct.fields.all():
        fields_data.append({'header':field.header, 
         'name':field.name, 
