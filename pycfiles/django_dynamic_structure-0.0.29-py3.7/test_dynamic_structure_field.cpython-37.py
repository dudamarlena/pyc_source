...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/dyn_struct/tests/db/models/test_dynamic_structure_field.py
...
class DynamicStructureFieldTestCase(BaseModels):
    ...
    def test_clean_is_header_true_with_name(self):
        ...
        self.assertIn('Если указывается заголовок, то поля "Название", "Поле" и "Виджет" указывать не нужно', ex.exception)
    ...
    def test_clean_with_check_widget_arguments_exception(self):
        ...
        with self.assertRaises(ValidationError) as (ex):
            self.field_struct.clean()
        self.assertIn("Не удалось создать поле формы (module 'django.forms.widgets' has no attribute '{}')".format(self.field_struct.widget), ex.exception)
        self.assertEqual('invalid', ex.exception.code)
