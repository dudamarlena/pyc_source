...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_jira/backend.py
...
class JiraBaseBackend(ServiceBackend):
    ...
    def __init__(self, settings, project=None, impact_field=None, reporter_field=None, default_issue_type='Task', verify=False):
    ...
    def get_resources_for_import(self):
        return [ {'name': proj.name, 'backend_id': proj.key} for proj in self.manager.projects()
               ]
...
class JiraBackend(JiraBaseBackend):
    ...
    def manager(self):
        try:
            ...
            try:
                self._manager = JIRA(server=self.settings.backend_url, options={'verify': self.verify}, basic_auth=(
                 self.settings.username, self.settings.password), validate=False)
            except JIRAError as e:
                six.reraise(JiraBackendError, e)
    ...
    def create_issue(self, issue):
        args = dict(project=issue.project.backend_id, summary=issue.summary, description=issue.description, issuetype={'name': self.default_issue_type})
    ...
    def import_project_issues(self, project):
        ...
        for backend_issue in self.manager.search_issues('project=%s' % project.backend_id):
            backend_issue._parse_raw(backend_issue.raw)
            fields = backend_issue.fields
            impact = getattr(fields, impact_field, None)
            priority = fields.priority.name
            issue = project.issues.create(impact=self.convert_field(impact, project.issues.model.Impact.CHOICES), type=fields.issuetype.name, status=fields.status.name, summary=fields.summary, priority=self.convert_field(priority, project.issues.model.Priority.CHOICES, mapping_setting='JIRA_PRIORITY_MAPPING'), description=fields.description or '', resolution=fields.resolution or '', updated_username=fields.creator.displayName, backend_id=backend_issue.key, created=parse_datetime(fields.created), updated=parse_datetime(fields.updated), state=project.issues.model.States.OK)
            for backend_comment in self.manager.comments(backend_issue):
                tmp = issue.comments.model()
                tmp.clean_message(backend_comment.body)
                issue.comments.create(user=tmp.user, message=tmp.message, created=parse_datetime(backend_comment.created), backend_id=backend_comment.id, state=issue.comments.model.States.OK)
