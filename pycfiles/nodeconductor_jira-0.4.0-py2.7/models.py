...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_jira/models.py
...
class JiraService(structure_models.Service):
    projects = models.ManyToManyField(structure_models.Project, related_name='jira_services', through='JiraServiceProjectLink')
...
class Project(core_models.StateMixin, structure_models.ResourceMixin):
    ...
    service_project_link = models.ForeignKey(JiraServiceProjectLink, related_name='projects', on_delete=models.PROTECT)
    ...
    def get_backend(self):
        return super(Project, self).get_backend(project=self.backend_id, impact_field=self.impact_field, reporter_field=self.reporter_field, default_issue_type=self.default_issue_type)
...
class Comment(structure_models.StructureLoggableMixin, JiraSubPropertyIssue):
    ...
    def clean_message(self, message):
        ...
        if not template:
            ...
            User = get_user_model()
            template = re.sub('([\\^~*?:\\(\\)\\[\\]|+])', '\\\\\\1', template)
            pattern = template.format(body='', user=User(full_name='(.+?)', username='([\\w.@+-]+)'))
