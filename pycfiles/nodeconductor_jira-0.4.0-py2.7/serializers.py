...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nodeconductor_jira/serializers.py
...
class ServiceSerializer(structure_serializers.BaseServiceSerializer):
    SERVICE_ACCOUNT_FIELDS = {'backend_url': 'JIRA host (e.g. https://jira.example.com/)', 
...
class ServiceProjectLinkSerializer(structure_serializers.BaseServiceProjectLinkSerializer):
    ...
    class Meta(structure_serializers.BaseServiceProjectLinkSerializer.Meta):
        ...
        extra_kwargs = {'service': {'lookup_field': 'uuid', 'view_name': 'jira-detail'}}


class ProjectSerializer(structure_serializers.BaseResourceSerializer):
    key = serializers.CharField(write_only=True)
    service = serializers.HyperlinkedRelatedField(source='service_project_link.service', view_name='jira-detail', read_only=True, lookup_field='uuid')
    service_project_link = serializers.HyperlinkedRelatedField(view_name='jira-spl-detail', queryset=models.JiraServiceProjectLink.objects.all())

    class Meta(structure_serializers.BaseResourceSerializer.Meta):
        model = models.Project
        view_name = 'jira-projects-detail'
        protected_fields = structure_serializers.BaseResourceSerializer.Meta.protected_fields + ('key', )
        fields = structure_serializers.BaseResourceSerializer.Meta.fields + ('key',
                                                                             'impact_field',
                                                                             'reporter_field',
                                                                             'default_issue_type',
                                                                             'available_for_all')
...
class ProjectImportSerializer(structure_serializers.BaseResourceImportSerializer):
    ...
    class Meta(structure_serializers.BaseResourceImportSerializer.Meta):
        model = models.Project
        view_name = 'jira-projects-detail'
        fields = structure_serializers.BaseResourceImportSerializer.Meta.fields + ('impact_field',
                                                                                   'reporter_field',
                                                                                   'default_issue_type',
                                                                                   'available_for_all')
    ...
    def create(self, validated_data):
        ...
        try:
            ...
            raise serializers.ValidationError({'backend_id': "Can't find project with ID %s" % validated_data['backend_id']})
...
class JiraPropertySerializer(AugmentedSerializerMixin, serializers.HyperlinkedModelSerializer):
    ...
    class Meta(object):
        ...
        fields = ('url', 'uuid', 'user', 'user_uuid', 'user_name', 'user_email', 'state',
                  'error_message')
        read_only_fields = ('uuid', 'user', 'error_message')
        extra_kwargs = {'user': {'lookup_field': 'uuid', 'view_name': 'user-detail'}}
        related_paths = {'user': ('uuid', 'name', 'email')}
...
class CommentSerializer(JiraPropertySerializer):
    ...
    class Meta(JiraPropertySerializer.Meta):
        ...
        fields = JiraPropertySerializer.Meta.fields + ('issue', 'issue_uuid', 'issue_key',
                                                       'message', 'created')
        protected_fields = ('issue', )
        extra_kwargs = dict(url={'lookup_field': 'uuid', 'view_name': 'jira-comments-detail'}, issue={'lookup_field': 'uuid', 'view_name': 'jira-issues-detail'}, **JiraPropertySerializer.Meta.extra_kwargs)
        related_paths = dict(issue=('uuid', 'key'), **JiraPropertySerializer.Meta.related_paths)
...
class AttachmentSerializer(JiraPropertySerializer):
    ...
    class Meta(JiraPropertySerializer.Meta):
        ...
        fields = JiraPropertySerializer.Meta.fields + ('issue', 'issue_uuid', 'issue_key',
                                                       'file')
        protected_fields = ('issue', )
        extra_kwargs = dict(url={'lookup_field': 'uuid', 'view_name': 'jira-attachments-detail'}, issue={'lookup_field': 'uuid', 'view_name': 'jira-issues-detail'}, **JiraPropertySerializer.Meta.extra_kwargs)
        related_paths = dict(issue=('uuid', 'key'), **JiraPropertySerializer.Meta.related_paths)
...
class IssueSerializer(JiraPropertySerializer):
    ...
    class Meta(JiraPropertySerializer.Meta):
        model = models.Issue
        fields = JiraPropertySerializer.Meta.fields + ('project', 'project_uuid', 'project_name',
                                                       'key', 'summary', 'description',
                                                       'resolution', 'status', 'type',
                                                       'priority', 'impact', 'created',
                                                       'updated', 'updated_username',
                                                       'access_url', 'comments')
        read_only_fields = ('type', 'status', 'resolution', 'updated_username', 'error_message')
        protected_fields = ('project', 'key')
        extra_kwargs = dict(url={'lookup_field': 'uuid', 'view_name': 'jira-issues-detail'}, project={'lookup_field': 'uuid', 'view_name': 'jira-projects-detail'}, **JiraPropertySerializer.Meta.extra_kwargs)
        related_paths = dict(project=('uuid', 'name'), **JiraPropertySerializer.Meta.related_paths)
...
class WebHookReceiverSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        priority = backend.convert_field(fields['priority']['name'], models.Issue.Priority.CHOICES, mapping_setting='JIRA_PRIORITY_MAPPING')
        ...
        try:
            ...
            if event_type == self.Event.CREATE:
                project.issues.create(type=fields['issuetype']['name'], status=fields['status']['name'], summary=fields['summary'], impact=impact, priority=priority, description=fields['description'] or '', resolution=fields['resolution'] or '', updated_username=validated_data['user']['displayName'], backend_id=validated_data['issue']['key'], state=models.Issue.States.OK)
            else:
        ...
        if event_type == self.Event.UPDATE:
            if 'comment' in validated_data:
                ...
                issue.comments.update_or_create(backend_id=validated_data['comment']['id'], defaults={'message': message, 
