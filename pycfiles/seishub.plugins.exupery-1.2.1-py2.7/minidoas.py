...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\plugins\exupery\minidoas.py
...
from seishub.core.packages.installer import registerIndex, registerStylesheet, registerSchema
...
class MiniDOASStationResourceType(Component):
    ...
    registerStylesheet('xslt' + os.sep + 'minidoas-station_metadata.xslt', 'metadata')
    registerIndex('project_id', '/MiniDOAS_Station/@project_id', 'text')
    registerIndex('volcano_id', '/MiniDOAS_Station/@volcano_id', 'text')
    registerIndex('start_datetime', '/MiniDOAS_Station/start_datetime/value', 'datetime')
    registerIndex('end_datetime', '/MiniDOAS_Station/end_datetime/value', 'datetime')
...
class MiniDOASDataResourceType(Component):
    ...
    registerIndex('start_datetime', '/MiniDOAS_data/start_datetime/value', 'datetime')
    registerIndex('end_datetime', '/MiniDOAS_data/end_datetime/value', 'datetime')
...
class MiniDOASStationLayer(Component):
    ...
    def createView(self):
        ...
        xmlindex_list = catalog.getIndexes(package_id='exupery', resourcetype_id='minidoas-station')
        filter = [
         'project_id', 'volcano_id', 'station_id', 'latitude',
