...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\plugins\exupery\so2.py
...
from seishub.core.packages.installer import registerSchema, registerIndex, registerStylesheet
...
class SO2GOME2ResourceType(Component):
    ...
    registerIndex('upperleft_latitude', '/SO2/range_upperleft/latitude/value', 'float')
    registerIndex('upperleft_longitude', '/SO2/range_upperleft/longitude/value', 'float')
    registerIndex('lowerright_latitude', '/SO2/range_lowerright/latitude/value', 'float')
    registerIndex('lowerright_longitude', '/SO2/range_lowerright/longitude/value', 'float')
    registerIndex('max_SO2_value', '/SO2/max_SO2_value', 'float')
    registerIndex('SO2_range1', '/SO2/measurements_with_SO2_between_0.5DU_and_1DU', 'numeric')
    registerIndex('SO2_range2', '/SO2/measurements_with_SO2_between_1DU_and_1.5DU', 'numeric')
    registerIndex('SO2_range3', '/SO2/measurements_with_SO2_above_1.5DU', 'numeric')
    registerIndex('alert_level', '/SO2/SO2_alert_level', 'numeric')
    registerIndex('local_path_image', '/SO2/files/file/local_path[../@id="GOME2_SO2 image"]', 'text')
    registerIndex('local_path_grid', '/SO2/files/file/local_path[../@id="GOME2_SO2 data"]', 'text')
...
class SO2TrajectoriesResourceType(Component):
    ...
    registerStylesheet('xslt' + os.sep + 'so2-traj-disp_metadata.xslt', 'metadata')
    registerIndex('project_id', '/Trajectories/@project_id', 'text')
    registerIndex('volcano_id', '/Trajectories/@volcano_id', 'text')
    registerIndex('start_datetime', '/Trajectories/start_datetime/value', 'datetime')
    registerIndex('end_datetime', '/Trajectories/end_datetime/value', 'datetime')
    registerIndex('upperleft_latitude', '/Trajectories/range_upperleft/latitude/value', 'float')
    registerIndex('upperleft_longitude', '/Trajectories/range_upperleft/longitude/value', 'float')
    registerIndex('lowerright_latitude', '/Trajectories/range_lowerright/latitude/value', 'float')
    registerIndex('lowerright_longitude', '/Trajectories/range_lowerright/longitude/value', 'float')
    registerIndex('latitude', '/Trajectories/latitude/value', 'float')
    registerIndex('longitude', '/Trajectories/longitude/value', 'float')
    registerIndex('local_path_image', '/Trajectories/files/file/local_path[../@format="GeoTIFF"]', 'text')
...
class SO2TrajectoriesKMLMapper(Component):
    ...
    def process_GET(self, request):
        ...
        for i in result:
            ...
            Sub(s, 'start_datetime').text = i.start_datetime.isoformat()
            Sub(s, 'end_datetime').text = i.end_datetime.isoformat()
            Sub(s, 'url').text = 'seishub://exupery/gis/kml?document_id=%s' % str(i.document_id)
