...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\seishub\plugins\exupery\infrared.py
...
class InfraredResourceType(Component):
    ...
    registerStylesheet('xslt' + os.sep + 'infrared_hotspots_metadata.xslt', 'metadata')
    registerIndex('project_id', '/infrared/@project_id', 'text')
    registerIndex('volcano_id', '/infrared/@volcano_id', 'text')
    registerIndex('start_datetime', '/infrared/start_datetime/value', 'datetime')
    registerIndex('end_datetime', '/infrared/end_datetime/value', 'datetime')
    registerIndex('upperleft_latitude', '/infrared/range_upperleft/latitude/value', 'float')
    registerIndex('upperleft_longitude', '/infrared/range_upperleft/longitude/value', 'float')
    registerIndex('lowerright_latitude', '/infrared/range_lowerright/latitude/value', 'float')
    registerIndex('lowerright_longitude', '/infrared/range_lowerright/longitude/value', 'float')
    registerIndex('satellite', '/infrared/processing/satellite', 'text')
    registerIndex('satellite_zenith_deg', '/infrared/processing/satellite_zenith_deg', 'float')
    registerIndex('alert_level', '/infrared/hotspots_overview/thermal_anomaly_alert_level', 'numeric')
    registerIndex('radiant_flux_overview', '/infrared/hotspots_overview/radiant_flux_MW', 'float')
    registerIndex('latitude_hotspot', '/infrared/hotspots/hotspot#latitude/value', 'float')
    registerIndex('longitude_hotspot', '/infrared/hotspots/hotspot#longitude/value', 'float')
    registerIndex('radiant_flux_hotspot', '/infrared/hotspots/hotspot#radiant_flux_MW', 'float')
    registerIndex('temperature_hotspot', '/infrared/hotspots/hotspot#temperature_of_thermal_anomaly_K', 'float')
    registerIndex('area_hotspot', '/infrared/hotspots/hotspot#area_of_thermal_anomaly_ha', 'float')
    registerIndex('percentage_area_hotspot', '/infrared/hotspots/hotspot#percentage_of_area_of_thermal_anomaly', 'float')
    registerIndex('effusion_hotspot', '/infrared/hotspots/hotspot#effusion_rate_m3_s', 'float')
    registerIndex('local_path_image', '/infrared/files/file/local_path[../@id="IRimage_8bit"]', 'text')
    registerIndex('local_path_image_16bit', '/infrared/files/file/local_path[../@id="IRimage_16bit"]', 'text')
...
class InfraredHotspotSQLView(Component):
    ...
    def createView(self):
        ...
        xmlindex_list = catalog.getIndexes(package_id='exupery', resourcetype_id='infrared')
        filter = [
         'project_id', 'volcano_id', 'latitude_hotspot',
        ...
        if not xmlindex_list:
            ...
            options = [
             sql.func.GeomFromText(sql.text("'POINT(' || longitude_hotspot.keyval || ' ' || " + "latitude_hotspot.keyval || ')', 4326")).label('geom')]
...
class InfraredGeoTIFFMapper(Component):
    ...
    def process_GET(self, request):
        tab = Table('/exupery/infrared', request.env.db.metadata, autoload=True)
        try:
