...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/kershaw/io.py
# Compiled at: 2015-05-04 11:16:57
...
def file_lines(file_path, start=1, end=None, length=None, include_empty=True):
    ...
    if not end is None:
        assert end > start, 'end_line must be greater than start_line (value is one-indexed)'
...
def head(file_path, length=1, short_circuit_singles=True, include_empty=True):
    ...
    if short_circuit_singles and length == 1:
        return lines[0]
    return lines
