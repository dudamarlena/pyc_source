...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/rpiTempSrc/rpi_tempmon.py
# Compiled at: 2018-04-22 15:57:31
# Size of source mod 2**32: 10200 bytes
"""
# ========================HEADER=======================================
...
"""
import sys, os, datetime, argparse, configparser, time, socket
...
def stresstest_func(testrun_count):
    ...
    print(DESTLOG)
    yaxislist, cpulist, cpu_uselist, plotlabel1, plotlabel2, ylabel, title = Work.stresstest(DESTLOG, testrun_count)
    mychoice = input('\nDo you want to view graph of stress test? [y/N]')
    if mychoice == 'y' or mychoice == 'Y':
        mygraph = RpiTempmonGraph.MatplotGraph('RPi Tempmon :')
        mygraph.draw_graph(yaxislist, cpulist, cpu_uselist, plotlabel1, plotlabel2, ylabel, title)
...
def process_cmd_arguments():
    ...
    parser.add_argument('-v', help='Print rpi_tempmon version and quit', default=False, dest='version', action='store_true')
    parser.add_argument('-l', help='Log file mode', default=False, dest='logfile', action='store_true')
    parser.add_argument('-s', help=' convert Log file to csv', default=False, dest='csv_convert', action='store_true')
    parser.add_argument('-L', help='Log folder mode', default=False, dest='logfolder', action='store_true')
    parser.add_argument('-m', help='Send email mode with ssmtp', default=False, dest='mail', action='store_true')
    parser.add_argument('-g', help='Draw a graph from log file data', default=False, dest='graphlog', action='store_true')
    parser.add_argument('-c', help='Continuous mode, + integer arg in secs for delay time', type=int, dest='cont')
    parser.add_argument('-a', help='Analysis the log file and report', default=False, dest='data', action='store_true')
    parser.add_argument('-n', help='notify mode +int arg, 2=notify always ,    3=notify only on Cpu limit exceed', type=int, dest='notify')
    parser.add_argument('-ST', help='Stress test mode, + integer arg for number of runs', type=int, dest='stresstest')
...
def read_configfile_func():
    ...
    try:
        ...
        if int(cpu_limit) < 1 or int(cpu_limit) > 99:
            ...
            quit()
        config_file_data = (
         mailuser, mail_alert, alarm_mode, cpu_limit, led_mode, led_num)
