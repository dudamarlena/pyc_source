...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/rpiTempMod/RpiTempmonGraph.py
# Compiled at: 2018-04-22 19:02:44
# Size of source mod 2**32: 9277 bytes
"""
Module imported into rpi_tempmon contains
containing a class with methods to display graphs from matlib
Class : MatplotGraph
"""
import os, re, dateutil, matplotlib as mpl
mpl.use('tkagg')
...
class MatplotGraph(object):
    ...
    def graph_log_data(self, destlog):
        ...
        self.display_menu()
        timelist, unixlist, cpulist, gpulist, cpu_uselist, ramlist, swaplist = Work.data_func(destlog, True)
        if re.match('[1-4]', self.selection):
            ...
            if self.selection == '1' or self.selection == '5':
                ...
                self.draw_graph(yaxislist, cpulist, gpulist, plotlabel1, plotlabel2, ylabel, title)
            else:
                if self.selection == '2' or self.selection == '6':
                    plotlabel1 = 'CPU temp'
                    plotlabel2 = 'CPU usage'
                    ylabel = 'CPU usage(%) / Temperature (degrees)'
                    title = 'CPU temperature and usage RPi'
                    self.draw_graph(yaxislist, cpulist, cpu_uselist, plotlabel1, plotlabel2, ylabel, title)
                else:
                    if self.selection == '3' or self.selection == '7':
                        plotlabel1 = 'RAM'
                        plotlabel2 = 'SWAP'
                        ylabel = 'Memory (% used)'
                        title = 'RAM & Swap memory usage of RPi'
                        self.draw_graph(yaxislist, ramlist, swaplist, plotlabel1, plotlabel2, ylabel, title)
                    else:
                        if self.selection == '4' or self.selection == '8':
                            plotlabel1 = 'CPU'
                            ylabel = 'CPU (% used)'
                            title = 'CPU usage of RPi'
                            self.draw_graph(yaxislist, cpu_uselist, False, plotlabel1, False, ylabel, title)
                        else:
    ...
    def draw_graph(self, timelist, yaxis_list1, yaxis_list2, plot_label1, plot_label2, yaxis_label, graph_title):
        ...
        plt.plot(mydates, yaxis_list1, label=plot_label1, color='green', marker='x')
        if plot_label2:
            plt.plot(mydates, yaxis_list2, label=plot_label2, marker='*')
