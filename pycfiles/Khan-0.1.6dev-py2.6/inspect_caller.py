...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/controller/inspect_caller.py
...
class InspectCaller(ActionDecorator):
    ...
    def __call__(self, func):
        ...
        def wrapper(controller, *args, **kargs):
            ...
            args_len = len(args)
            if args_len > 0:
                func_defaults_len = len(func_defaults) if func_defaults is not None else 0
                for i in range(1, len(func_args)):
                    func_arg_name = func_args[i]
                    if args_len > i - 1:
                        data[func_arg_name] = args[(i - 1)]
                    elif func_defaults_len > 0:
...
class ParamProvider(object):
    ...
    def __and__(self, other):
        assert isinstance(other, ParamProvider), TypeError('`other` should be a `ParamProvider` subclass')
