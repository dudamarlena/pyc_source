...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/scripts/serve.py
...
import re, os, errno, sys, time, threading, atexit, logging
from paste.reloader import watch_file
from khan.deploy import run_server_from_zcml
from khan.command import Command, BadCommand
__all__ = [
 'ServeCommand', 'DaemonizeException']
MAXFD = 1024
...
class ServeCommand(Command):
    ...
    You can also include variable assignments like 'http_port=7010'
    ...
    parser.add_option('--server-name', dest='server_name', metavar='SECTION_NAME', help='Use the named server as defined in the configuration file (default: main)')
    parser.add_option('-p', '--package', dest='package', default=None, metavar='PYTHON_PACKGE', help='Project package.')
    parser.add_option('-z', '--zcml', dest='zcml', default='configure.zcml', metavar='ZCML', help='Project zcml file.')
    if hasattr(os, 'fork'):
        parser.add_option('--daemon', dest='daemon', action='store_true', help='Run in daemon (background) mode')
    parser.add_option('--pid-file', dest='pid_file', metavar='FILENAME', help='Save PID to file (default to paster.pid if running in daemon mode)')
    ...
    parser.add_option('--reload-interval', dest='reload_interval', default=1, help='Seconds between checking files (low number can cause significant CPU usage)')
    ...
    parser.add_option('--stop-daemon', dest='stop_daemon', action='store_true', help='Stop a daemonized server (given a PID file, or default paster.pid file)')
    _scheme_re = re.compile('^[a-z][a-z]+:', re.I)
    default_verbosity = 1
    ...
    def command(self):
        ...
        if self.options.stop_daemon:
            ...
            if self.requires_config_file:
                if not self.args:
                    raise BadCommand('You must give a ZCML config file')
            ...
            if self.options.reload:
                if os.environ.get(self._reloader_environ_key):
                    ...
                    if package:
                        ...
                        watch_file(os.path.join(os.path.dirname(pkg.__file__), zcml))
                    elif zcml:
                        watch_file(os.path.abspath(zcml))
                else:
            ...
            try:
                run_server_from_zcml(package, zcml=zcml, server_name=server_name or 'main')
    ...
    def daemonize(self):
        ...
        if pid:
            raise DaemonizeException('Daemon is already running (PID: %s from PID file %s)' % (
    ...
    def restart_with_monitor(self, reloader=False):
        ...
        while 1:
            args = [
             self.quote_first_command_arg(sys.executable)] + sys.argv
