...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/security/auth.py
...
def new_request_classifier(environ):
    ...
    if req_type == requestTypes.BROWSER:
        return 'browser'
    if req_type == requestTypes.DAV:
        return 'dav'
    if req_type in [requestTypes.XMLPOST, requestTypes.JSONPOST]:
        return 'rpc'
    return 'browser'
...
def make_auth_middleware(global_conf, identifiers, authenticators, challengers, mdproviders=None, group_adapters=None, permission_adapters=None, conf=None, request_classifier=None, challenge_decider=None, skip_authentication=False):
    u"""
    EGG Entry Point::
    ...
    def middleware(app):
        return setup_auth(app, group_adapters=group_adapter_plugins, permission_adapters=permission_adapter_plugins, identifiers=identifier_plugins, authenticators=authenticator_plugins, challengers=challenger_plugins, mdproviders=mdprovider_plugins, classifier=request_classifier and eval_import(request_classifier) or new_request_classifier, challenge_decider=challenge_decider and eval_import(challenge_decider) or default_challenge_decider, skip_authentication=asbool(skip_authentication), log_stream=log, log_level=asbool(global_conf.get('debug', False)) and logging.DEBUG or logging.INFO)
...
class AuthTKT(auth_tkt.AuthTktCookiePlugin):
    ...
    def __init__(self, secret, cookie_name=TICKET_NAME, secure=False, include_ip=False, timeout=None, reissue_time=None, userid_checker=None):
...
def make_authtkt_object(global_conf, secret=None, secretfile=None, cookie_name=AuthTKT.TICKET_NAME, secure=False, include_ip=False, timeout=None, reissue_time=None, userid_checker=None):
    u"""
    ...
    plugin = AuthTKT(secret, cookie_name, asbool(secure), asbool(include_ip), timeout, reissue_time, userid_checker)
    return plugin
