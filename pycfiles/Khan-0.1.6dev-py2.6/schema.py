...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/schema.py
...
__all__ = [
 'String', 'Unicode', 'Bool', 'Boolean', 'SBool', 'SBoolean', 'Int', 'Integer', 'Float',
    ...
    def _to_python(self, value):
        ...
        if self.iencoding:
            try:
                ...
                raise ValidationError(_('The `value` must be a string (not a %(type)s: %(value)r)' % dict(type=type(value), value=value)))
...
class Email(BaseString):
    ...
    def _to_python(self, value):
        ...
        if not self.usernameRE.search(username):
            raise ValidationError(_('The username portion of the email address is invalid (the portion before the @: %(username)s)') % dict(username=username))
        if not self.domainRE.search(domain):
            raise ValidationError(_('The domain portion of the email address is invalid (the portion after the @: %(domain)s)') % dict(domain=domain))
        return value.strip()
...
class URL(BaseString):
    ...
    def _to_python(self, value):
        ...
        if self.require_tld and not match.group('domain'):
            raise ValidationError(_('You must provide a full domain name (like %(domain)s.com)') % dict(domain=match.group('tld')))
        return value
    ...
    def _to_python(self, value):
        if isinstance(value, (str, unicode)):
            ...
            if not value or value in self.false_values:
                return False
            raise ValidationError(_('Value should be %(true)r or %(false)r' % dict(true=self.true_values[0], false=self.false_values[0])))
        return bool(value)
...
class File(SchemaType):
    ...
    def __init__(self, types_supported=None):
        if types_supported is not None:
            self._types_supported = '\\.(%s)$' % ('|').join([ str(t).strip() for t in types_supported.split(',') ])
        return

    def _is_supported(self, path):
        return self._types_supported is not None and re.search(self._types_supported, path) is not None
...
class Schema(SchemaType, DictMixin):
    ...
    def __setitem__(self, name, value):
        assert isinstance(name, basestring) and not name.startswith('_'), ValueError("`name` must not starts with '_'")
    ...
    def keys(self):
        return dict(filter(lambda x: not x[0].startswith('_') and isinstance(x[1], SchemaType), inspect.getmembers(self))).keys()
