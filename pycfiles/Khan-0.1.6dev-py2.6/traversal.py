...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/traversal.py
...
__all__ = [
 'traverse', 'TravelableModel', 'DictModel', 'ObjectModel', 'Traverser', 'TraversalController']
    ...
    for segment in path.split('/'):
        ...
        if not segment or segment == '.':
            ...
            try:
                ...
                raise TypeError('Could not decode path segment %r using the UTF-8 decoding scheme' % segment)
...
def quote_path_segment(segment):
    ...
    try:
        ...
        if segment.__class__ is unicode:
...
class GraphTraverser(object):
    ...
    def __call__(self, request):
        ...
        for segment in vpath_tuple:
            if segment[:2] == '@@':
                return {'context': ob, 'view_name': segment[2:], 
            ...
            try:
                ...
                return {'context': ob, 'view_name': segment, 
                   'subpath': vpath_tuple[i + 1:], 
                   'traversed': vpath_tuple[:vroot_idx + i + 1], 
                   'virtual_root': vroot, 
                   'virtual_root_path': vroot_tuple, 
        ...
        return {'context': ob, 'view_name': '', 'subpath': subpath, 'traversed': vpath_tuple, 
           'virtual_root': vroot, 'virtual_root_path': vroot_tuple, 
           'root': root}
