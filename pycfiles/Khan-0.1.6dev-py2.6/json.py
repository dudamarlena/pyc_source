...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/json.py
# Compiled at: 2010-07-26 21:31:41
"""
json 工具
=================================
...
class JSONRPCBuilder(object):
    ...
    @staticmethod
    def request(method, params=None, id=None, version='1.0'):
        u"""
        构建 jsonrpc 请求
        ...
        :param id: id, 
        :param version: jsonrpc protocol version, defualts is 1.0
    ...
    def response(result=None, error=None, id=None, version='1.0'):
        ...
        if error is not None:
            json['error'] = {'code': error.code, 'message': error.message, 'data': error.data}
        else:
            assert id is not None, ValueError('This `id` MUST be an integer')
            id = int(id)
            json['result'] = result
        json['id'] = id
        return dumps(json)
...
def jsonrpc_loads(data, version=None):
    ...
    if 'method' in jsonrpc_data:
        if isinstance(jsonrpc_data['method'], basestring):
            jsonrpc_data['method'] = jsonrpc_data['method'].strip()
            assert jsonrpc_data['method'], JSONRPCParseError('`method` field invalid.')
            assert 'params' in jsonrpc_data and type(jsonrpc_data['params']) in [tuple, set, list, dict], JSONRPCParseError('`params` field invalid.')
    elif 'result' in jsonrpc_data:
        assert 'error' not in jsonrpc_data, JSONRPCParseError('`error` field must be `null`')
        assert 'id' in jsonrpc_data and jsonrpc_data['id'] != None, JSONRPCParseError('`id` field missing or invalid.')
    elif 'error' in jsonrpc_data:
        assert 'code' in jsonrpc_data['error'] and type(jsonrpc_data['error']['code']) in [int] and 'message' in jsonrpc_data['error'], JSONRPCParseError('`code` or `messgae` fields invalid.')
    else:
        raise JSONRPCParseError('`result` or `error` field missing')
    return jsonrpc_data
...
class JSONRPCService(DictMixin, object):
    ...
    def _wapper_call(self, environ, start_response):
        ...
        if not req.method == 'POST':
            ...
            resp = Response(JSONRPCBuilder.response(error=JSONRPCInvalidRequestError()), content_type=JSON_MIME_TYPE, charset='utf-8')
            ...
            try:
                ...
                resp = Response(JSONRPCBuilder.response(error=JSONRPCParseError()), content_type=JSON_MIME_TYPE, charset='utf-8')
                return resp(environ, start_response)
            else:
                if 'version' in jsonrpc_data:
                    ...
                    version = '1.0'
                if 'id' in jsonrpc_data:
                    id = jsonrpc_data['id']
                    is_notification = False
                else:
                    id = None
                    is_notification = True
                    self._logger.debug('jsonrpc request is a notification.')
                if 'method' not in jsonrpc_data:
                    self._logger.debug('`method` field not found.')
                    if is_notification:
                        resp = Response()
                    else:
                        resp = Response(JSONRPCBuilder.response(error=JSONRPCParseError(), version=version, id=id), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
                args = []
                kargs = {}
                ...
                if provider is None:
                    ...
                    if is_notification:
                        ...
                        resp = Response(JSONRPCBuilder.response(error=JSONRPCMethodNotFoundError(), version=version, id=id), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
                ...
                try:
                    ...
                    if is_notification:
                        ...
                        resp = Response(JSONRPCBuilder.response(result=result, id=id, version=version), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
                except HTTPException, httpe:
                    self._logger.debug('`%r` raised HTTPException: %s (code: %s)', provider, httpe.__class__.__name__, httpe.wsgi_response.code, exc_info=True)
                    ...
                    if is_notification:
                        ...
                        resp = Response(JSONRPCBuilder.response(error=JSONRPCInvalidParamsError(), id=id, version=version), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
                    ...
                    if is_notification:
                        ...
                        resp = Response(JSONRPCBuilder.response(error=e, id=id, version=version), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
                except:
                    ...
                    if is_notification:
                        ...
                        resp = Response(JSONRPCBuilder.response(error=JSONRPCInternalError(), id=id, version=version), content_type=JSON_MIME_TYPE, charset='utf-8')
                    return resp(environ, start_response)
...
class JSONRPCClient(object):
    ...
    """
    default_headers = {'Content-type': JSON_MIME_TYPE, 
       'Accept': 'text/plain'}
    ...
    def __call__(self, *args, **kargs):
        ...
        if 'error' in jsonrpc_data and jsonrpc_data['error'] != None:
            if jsonrpc_data['error']['code'] in jsonrpc_error_map:
                err_cls = jsonrpc_error_map[jsonrpc_data['error']['code']](jsonrpc_data['error']['message'], data=jsonrpc_data['error'].get('data', None))
            else:
                err_cls = JSONRPCError(jsonrpc_data['error']['message'], jsonrpc_data['error']['code'], data=jsonrpc_data['error'].get('data', None))
            raise err_cls
        else:
            if 'result' in jsonrpc_data:
                return jsonrpc_data['result']
            raise JSONRPCServerError('response invalid')
        return
...
class JSONRPCClientCommand(Command):
    ...
    parser.add_option('-s', '--service', dest='service', default='http://localhost', help='the json-rpc service url [default: %default]')
    parser.add_option('-u', '--user', dest='user', default=None, help='Specifie user name')
    parser.add_option('-p', '--password', dest='password', default=None, metavar='PWD', help='Specifie password of the user [default: %default]')
    parser.add_option('-d', '--debug', dest='debug', action='store_true', help='debug mode [default: disabled]')
