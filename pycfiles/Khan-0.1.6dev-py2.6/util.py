...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/khan/utils/util.py
# Compiled at: 2010-07-21 02:06:51
"""
常用工具
=================================
...
.. autoclass:: Singleton
"""
import os, logging, mimetypes, urllib, threading, hashlib, pkg_resources, time, random, uuid
...
__all__ = [
 ...
 'ashandler',
 'Singleton']
DAV_METHODS = ('OPTIONS', 'PROPFIND', 'PROPPATCH', 'MKCOL', 'LOCK', 'UNLOCK', 'TRACE',
               'DELETE', 'COPY', 'MOVE')
DAV_USERAGENTS = ('Microsoft Data Access Internet Publishing Provider', 'WebDrive',
                  'Zope External Editor', 'WebDAVFS', 'Goliath', 'neon', 'davlib',
                  'wsAPI', 'Microsoft-WebDAV')
...
def request_classifier(environ):
    ...
    if request_method.lower() == 'post':
        ...
        if content_type in [JSON_MIME_TYPE, 'text/javascript']:
...
def abort(status_code=None, detail='', headers=None, comment=None, **kargs):
    ...
    logger.debug('Aborting request, status: %s, detail: %r, headers: %r, comment: %r', status_code, detail, headers, comment)
    raise exc.exception
...
def unique_id(url=None):
    ...
    result = ('').join([ '%02d' % x for x in time.localtime(time.time())[:6] ]) + '-' + unique_id
    return result
