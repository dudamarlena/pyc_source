...
# [GCC 8.4.0]
# Embedded file name: /Users/wms/cloud-volume/cloudvolume/microviewer/__init__.py
# Compiled at: 2020-05-01 13:49:43
import os
try:
...
def to_volumecutout(img, image_type, resolution=None, offset=None, hostname='localhost'):
    ...
    return VolumeCutout(buf=img, path=ExtractedPath('mem', hostname, '/', '', '', '', ''), cloudpath='IN MEMORY', resolution=resolution, mip=-1, layer_type=image_type, bounds=Bbox(offset, offset + Vec(*img.shape[:3])), handle=None)
...
def hyperview(img, segmentation, resolution=None, offset=None, hostname='localhost', port=DEFAULT_PORT):
    img = to3d(img)
    ...
    segmentation = to_volumecutout(segmentation, 'segmentation', resolution, offset, hostname)
...
def view(img, segmentation=False, resolution=None, offset=None, hostname='localhost', port=DEFAULT_PORT):
    from cloudvolume.volumecutout import VolumeCutout
    img = to3d(img)
    resolution = getresolution(img, resolution)
    offset = getoffset(img, offset)
    img = emulate_eight_uint64(img)
    cutout = VolumeCutout(buf=img, path=ExtractedPath('mem', hostname, '/', '', '', '', ''), cloudpath='IN MEMORY', resolution=resolution, mip=-1, layer_type='segmentation' if segmentation else 'image', bounds=Bbox(offset, offset + Vec(*img.shape[:3])), handle=None)
...
class ViewerServerHandler(BaseHTTPRequestHandler):
    ...
    def serve_parameters(self):
        ...
        if len(self.cutouts) == 1:
            ...
            msg = json.dumps({'viewtype': 'single', 
               ...
               'cloudpath': [
                           cutout.cloudpath], 
               'mip': cutout.mip, 
               'bounds': [ int(_) for _ in cutout.bounds.to_list() ], 'resolution': cutout.resolution.tolist(), 
               'data_types': [
                            str(cutout.dtype)], 
               'data_bytes': np.dtype(cutout.dtype).itemsize})
        else:
            img, seg = self.cutouts
            msg = json.dumps({'viewtype': 'hyper', 
               'dataset': img.dataset_name, 
               'layers': [
                        img.layer, seg.layer], 
               'protocol': img.path.protocol, 
               'cloudpath': [
                           img.cloudpath, seg.cloudpath], 
               'mip': img.mip, 
               'bounds': [ int(_) for _ in img.bounds.to_list() ], 'resolution': img.resolution.tolist(), 
               'data_types': [
                            str(img.dtype), str(seg.dtype)], 
               'data_bytes': [
