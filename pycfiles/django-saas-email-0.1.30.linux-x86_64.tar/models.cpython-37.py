...
# [GCC 8.4.0]
# Embedded file name: /home/leotrubach/development/django-saas-email/venv/lib/python3.7/site-packages/django_saas_email/models.py
# Compiled at: 2019-04-11 15:30:09
# Size of source mod 2**32: 16656 bytes
import base64, json, uuid, os, html2text, html2text.config, sendgrid
from python_http_client import HTTPError
from sendgrid.helpers.mail import Email, Content, Mail as HelperMail, Attachment as HelperAttachment
from django.conf import settings
from tinymce import models as tinymce_models
from django_saas_email.utils import _noautoescape
...
class AbstractMailTemplate(models.Model):
    ...
    name = models.CharField((_('Template name')),
      help_text=(_('Template name; a short all-lowercase string')), max_length=100, unique=True)
    subject = models.CharField((_('Email subject line template')),
      help_text=(_('A format string like "Hello {}"; required')), max_length=200)
    html_template = tinymce_models.HTMLField((_('HTML template (required)')),
      help_text=(_("The HTML template, written with Django's template syntax; required")))
    text_template = models.TextField((_('Text template (optional)')),
    ...
    def render_subject(self, context):
        ...
        return Template((_noautoescape(self.subject)), engine=(self.backend.engine)).render(context)
    ...
    def render_with_context(self, context):
        ...
        html_context = {'EMAIL_CONTENT':email_content_html, 
        ...
        if self.text_template:
            text_output = Template((_noautoescape(self.text_template)),
              engine=(self.backend.engine)).render(context)
        else:
...
class MailManager(models.Manager):
    ...
    def create_mail(self, template_name=None, context=None, to_address=None, to_name=None, from_address=None, from_name=None, subject=None, selected_attachments=None, text=None, cc_address=None, bcc_address=None):
        ...
        mail = self.create(template=template,
...
class AbstractMail(models.Model):
    ...
    from_address = models.EmailField((_('Sender email address')),
      help_text=(_("The 'from' field of the email")), null=False, blank=False)
    from_name = models.CharField(max_length=100,
      verbose_name='Sender name',
      help_text=(_('Email sender name')),
      null=True,
      blank=True)
    to_address = models.EmailField((_('Recipient email address')),
      help_text=(_("The 'to' field of the email")), null=False, blank=False)
    to_name = models.CharField(max_length=100,
      verbose_name=(_('Recipient name')),
      help_text=(_('Email recipient name')),
      null=True,
      blank=True)
    cc_address = models.EmailField((_('CC email address')),
      help_text=(_("The 'cc' field of the email")), null=True, blank=True)
    bcc_address = models.EmailField((_('BCC email address')),
      help_text=(_("The 'bcc' field of the email")), null=True, blank=True)
    delivery_mail_id = models.IntegerField((_('Unique mail sender ID')),
      help_text=(_('The ID is saved after correct sending')),
      null=True,
      blank=True,
      editable=False)
    delivery_status = models.IntegerField((_('Status of Mail sender')),
      help_text=(_('The Mail sender status')), null=True, blank=True, editable=False)
    ...
    subject = models.CharField((_('Email Subject line')),
      help_text=(_('Subject line for a mail')), max_length=500, null=True, blank=True)
    context = JSONField((_('Data of email context')),
      help_text=(_('JSON dump of context dictionary used to fill in templates')),
      null=True,
      blank=True)
    time_created = models.DateTimeField((_('Creation time')),
      help_text=(_('When was the mail created?')), default=(timezone.now), editable=False)
    time_sent = models.DateTimeField((_('Sent time')),
      help_text=(_('When was the mail send via the backend?')), null=True, blank=True, editable=False)
    time_delivered = models.DateTimeField((_('Delivery time')),
      help_text=(_('Actual delivery time by the email backend')),
      null=True,
      blank=True,
      editable=False)
    used_backend = models.CharField((_('E-Mail Backend')),
    ...
    def render_mail(self):
        ...
        if self.template:
            ...
            if self.subject:
                rendered_subject = Template(_noautoescape(self.subject)).render(context)
            else:
    ...
    def send(self, sendgrid_api=False):
        ...
        if sendgrid_api:
            if not settings.SENDGRID_API_KEY:
                ...
                if self.cc_address:
                    if self.to_address != self.cc_address:
                        to_emails.append(sendgrid.Cc(self.cc_address))
                if self.bcc_address:
                    if self.to_address != self.bcc_address:
                        if not self.cc_address or self.cc_address != self.bcc_address:
                ...
                for attachment in self.selected_attachments.all():
                    ha = HelperAttachment(file_content=(base64.b64encode(attachment.attached_file.read()).decode('ascii')),
