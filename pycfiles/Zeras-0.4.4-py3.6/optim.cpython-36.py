...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Zeras/optim.py
# Compiled at: 2020-03-15 07:46:43
# Size of source mod 2**32: 8605 bytes
...
def linear_warmup_and_exp_decayed_lr(settings, global_step):
    """ settings.learning_rate_base
        ...
        settings.staircase    
    """
    learning_rate = tf.constant(value=(settings.learning_rate_base), shape=[], dtype=(tf.float32))
    if settings.warmup_steps:
        ...
        learning_rate = tf.train.exponential_decay(learning_rate, step_surplus, settings.decay_steps, settings.decay_rate, settings.staircase)
        warmup_percent_done = global_steps_float / warmup_steps_float
        warmup_learning_rate = settings.learning_rate_base * warmup_percent_done
        learning_rate = tf.cond(global_steps_int < warmup_steps_int, lambda : warmup_learning_rate, lambda : learning_rate)
    else:
        learning_rate = tf.train.exponential_decay(learning_rate, global_step, settings.decay_steps, settings.decay_rate, settings.staircase)
...
def linear_warmup_and_polynomial_decayed_lr(settings, global_step):
    ...
    if settings.warmup_steps:
        ...
        warmup_steps_float = tf.cast(warmup_steps_int, tf.float32)
        step_surplus = global_steps_int - warmup_steps_int
        learning_rate = tf.train.polynomial_decay(learning_rate, step_surplus,
          (settings.decay_steps),
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
    __doc__ = ' A basic Adam optimizer that includes "correct" L2 weight decay.\n    '

    def __init__(self, learning_rate, weight_decay_rate=0.0, beta_1=0.9, beta_2=0.999, epsilon=1e-06, exclusions_from_weight_decay=None, name='AdamWeightDecayOptimizer'):
        """
        """
...
def adam_wd_optimizer(settings, learning_rate_tensor_or_value):
    """
    """
    opt = AdamWeightDecayOptimizer(learning_rate_tensor_or_value, weight_decay_rate=(settings.reg_lambda),
