...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Zeras/vocab.py
# Compiled at: 2020-03-15 07:46:43
# Size of source mod 2**32: 17202 bytes
...
class Vocab(object):
    ...
    def filter_tokens_by_cnt(self, min_cnt):
        filtered_tokens = [self.dict_id2token[idd] for idd in range(self.size()) if self.dict_token_cnt[self.dict_id2token[idd]] >= min_cnt]
        self.dict_token2id = {}
        self.dict_id2token = {}
        self._add_predefined_and_initial_tokens()
    ...
    def remove_specified_tokens(self, list_tokens_to_remove, regex_to_remove=[]):
        ...
        for idd in range(self.size()):
            ...
            if token in list_tokens_to_remove:
                pass
            else:
    ...
    def _save_embeddings_to_file_bin(self, emb_path):
        ...
        with open(emb_path, 'wb') as (fp):
            fp.write(num_words_str.encode('utf-8'))
