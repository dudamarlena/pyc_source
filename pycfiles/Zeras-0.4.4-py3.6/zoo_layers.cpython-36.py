...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Zeras/zoo_layers.py
# Compiled at: 2020-03-15 07:46:43
# Size of source mod 2**32: 15010 bytes
"""
...
def get_position_emb_mat(max_seq_len, posi_emb_dim, posi_emb_model, trainable=False, name='position_embeddings'):
    """
    """
    ...
    for idx in range(max_seq_len):
        for idm in range(posi_emb_dim):
            arg_mat[(idx, idm)] = idx * 0.0001 ** (d_model_recip_2 * idm)

    pe_sin = np.sin(arg_mat)
    pe_cos = np.cos(arg_mat)
    pe_sin = np.expand_dims(pe_sin, -1)
    pe_cos = np.expand_dims(pe_cos, -1)
    pe_all = np.concatenate([pe_sin, pe_cos], -1)
    pe_all = np.reshape(pe_all, [max_seq_len, -1])
    pe_all = pe_all[:, 0:posi_emb_dim]
    pe_mat = tf.get_variable(name, shape=(max_seq_len, posi_emb_dim), initializer=(tf.constant_initializer(pe_all)),
      trainable=trainable)
    return pe_mat


def create_dense_vars(input_size, output_size, weight_mat=None, use_bias=True, bias_init_value=0.0, scope='dense'):
    """
    """
    with tf.variable_scope(scope):
        if weight_mat is None:
            W = tf.get_variable('kernel', [input_size, output_size], initializer=(tf.variance_scaling_initializer()),
              dtype=(tf.float32))
        else:
            W = weight_mat
        if use_bias:
            b = tf.get_variable('bias', [output_size], initializer=(tf.constant_initializer(bias_init_value)),
              dtype=(tf.float32))
        else:
            b = None
    return (
...
def dense(x, output_size, weight_mat=None, transpose_b=False, use_bias=True, bias_init_value=0.0, scope='dense'):
    """
    """
    input_size = x.get_shape().as_list()[(-1)]
    wb = create_dense_vars(input_size, output_size, weight_mat=weight_mat,
      use_bias=use_bias,
...
def layer_norm(x, scope='layer_norm'):
    ...
    with tf.variable_scope(scope):
        beta = tf.get_variable('layer_norm_beta', [num_units], initializer=(tf.ones_initializer()),
          trainable=True,
          dtype=(tf.float32))
        gamma = tf.get_variable('layer_norm_gamma', [num_units], initializer=(tf.zeros_initializer()),
          trainable=True,
          dtype=(tf.float32))
        mean, std = tf.nn.moments(x, [-1], keep_dims=True, name='moments')
        return beta * (x - mean) / (std + 1e-06) + gamma


def layer_norm_api(x, scope=None):
    """
    """
    out = tf.contrib.layers.layer_norm(inputs=x, begin_norm_axis=(-1),
      begin_params_axis=(-1),
      scope=scope)
    return out


def multihead_attention_layer(num_heads, num_units, query, key, value, mask_mat=None, keep_prob=1.0, scope='mha'):
    """
    """
...
def att_qkv_layer(inputs, memory, values, mask_m, att_dim, keep_prob=1.0, scope='qkv'):
    ...
    """
    with tf.variable_scope(scope):
        d_inputs = tf.nn.dropout(inputs, keep_prob=keep_prob)
        ...
        mask_3d = tf.cast(tf.expand_dims(mask_m, axis=1), tf.float32)
        att_masked = tf.add(att_mat, 1e+30 * (mask_3d - 1))
        logits = tf.nn.softmax(att_masked)
        d_values = tf.nn.dropout(values, keep_prob=keep_prob)
        values_d = dense(d_values, att_dim, use_bias=False, scope='values')
        outputs = tf.matmul(logits, values_d)
    return outputs
...
def qk_value_pool_layer(qk_mat, values, mask_k, hidden, keep_prob=1.0, scope='qk_pool'):
    with tf.variable_scope(scope):
        mask_3d = tf.cast(tf.expand_dims(mask_k, axis=1), tf.float32)
        att_masked = tf.add(qk_mat, 1e+30 * (mask_3d - 1))
        logits = tf.nn.softmax(att_masked)
        d_values = tf.nn.dropout(values, keep_prob=keep_prob)
...
def att_pool_layer(query, seq, seq_mask, att_dim, keep_prob=1.0, scope='att_pooling'):
    ...
    with tf.variable_scope(scope):
        ...
        d_inputs = tf.nn.dropout(query, keep_prob=keep_prob)
        d_memory = tf.nn.dropout(seq, keep_prob=keep_prob)
        inputs_d = dense(d_inputs, att_dim, use_bias=False, scope='inputs')
        memory_d = dense(d_memory, att_dim, use_bias=False, scope='memory')
        att_mat = tf.matmul(inputs_d, tf.transpose(memory_d, [0, 2, 1])) / att_dim ** 0.5
        mask_3d = tf.cast(tf.expand_dims(seq_mask, axis=1), tf.float32)
        att_masked = tf.add(att_mat, 1e+30 * (mask_3d - 1))
        logits = tf.nn.softmax(att_masked)
        d_values = tf.nn.dropout(seq, keep_prob=keep_prob)
        values_d = dense(d_values, att_dim, use_bias=False, scope='values')
        outputs = tf.matmul(logits, values_d)
...
def rnn_layer(input_sequence, sequence_length, rnn_size, keep_prob=1.0, activation=None, concat=True, scope='bi-lstm'):
    ...
    cell_fw = tf.contrib.rnn.LSTMCell(rnn_size, activation=act, initializer=weight_initializer)
    cell_bw = tf.contrib.rnn.LSTMCell(rnn_size, activation=act, initializer=weight_initializer)
    rnn_output, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, input_sequence, sequence_length=sequence_length,
      time_major=False,
      dtype=(tf.float32),
      scope=scope)
    if concat:
...
def gru_layer(input_sequence, sequence_length, rnn_size, keep_prob=1.0, activation=None, concat=True, scope='bi-gru'):
...
def gather_and_pad_layer(x, num_items):
    ...
    def body(time, posi_s, bsd_s, mask_s):
        ...
        d = T - num_items[time]
        chunk, mask_c = tf.cond(d > 0, lambda : (
         tf.concat([chunk, tf.tile(pad_item, [d, 1])], 0),
         tf.concat([mask_c, tf.tile(zero_int32, [d])], 0)), lambda : (
         chunk, mask_c))
        bsd_s = bsd_s.write(time, chunk)
        mask_s = mask_s.write(time, mask_c)
        return (time + 1, posi_e, bsd_s, mask_s)

    t, p, bsd_w, mask_w = tf.while_loop(cond=condition, body=body, loop_vars=(
     time, posi, bsd_ta, mask_ta))
    bsd = bsd_w.stack()
    mask = mask_w.stack()
    return (
     bsd, mask)