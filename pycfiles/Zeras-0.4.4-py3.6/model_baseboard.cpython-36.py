...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/Zeras/model_baseboard.py
# Compiled at: 2020-03-15 07:51:07
# Size of source mod 2**32: 33304 bytes
"""
Created on Sat Sep  7 23:19:42 2019
@author: li-ming-fan
"""
import os, numpy as np, re, collections, time, logging, json, tensorflow as tf
...
class ModelBaseboard(metaclass=ABCMeta):
    ...
    def __init__(self, settings, learning_rate_schedule=linear_warmup_and_polynomial_decayed_lr, customized_optimizer=adam_optimizer):
    ...
    def set_model_settings(self, settings):
            ...
            if 'soft_placement' not in settings.__dict__.keys():
                ...
                if 'gpu_mem_growth' not in settings.__dict__.keys():
                    ...
                    if 'reg_exclusions' not in settings.__dict__.keys():
                        settings.__dict__['reg_exclusions'] = [
                         'embedding', 'bias', 'layer_norm', 'LayerNorm']
        ...
        self.sess_config = tf.ConfigProto(log_device_placement=(settings.log_device), allow_soft_placement=(settings.soft_placement))
    ...
    def prepare_for_train_and_valid_single_gpu(self, dir_ckpt=None):
        ...
        with self._graph.as_default():
            self.global_step = tf.get_variable('global_step', shape=[], dtype=(tf.int32), initializer=(tf.constant_initializer(0)),
              trainable=False)
            ...
            if self.settings.reg_lambda > 0.0:
                if self.settings.optimizer_type != 'adam_wd':
                    loss_reg = tf.add_n([tf.nn.l2_loss(v) for v in self.trainable_vars if not is_excluded(v)])
            ...
            self._train_op = self._opt.apply_gradients(grad_and_vars, global_step=(self.global_step))
        ...
        if dir_ckpt is not None:
            self.load_vars_from_ckpt(dir_ckpt)
    ...
    def prepare_for_train_and_valid_multi_gpu(self, dir_ckpt=None):
        ...
        with self._graph.as_default():
            ...
            with tf.variable_scope(vs_str):
                for gid in range(self.num_gpu):
                    with tf.device('/gpu:%d' % gid):
                        with tf.name_scope('bundle_%d' % gid):
                            output_tensors = self.build_inference(inputs[gid])
                            loss_metric_tensors = self.build_loss_and_metric(output_tensors, labels[gid])
            ...
            self._train_op = self._opt.apply_gradients(grads_summed, global_step=(self.global_step))
            self._saver = tf.train.Saver()
            self._saver_best = tf.train.Saver()
            self._sess = tf.Session(graph=(self._graph), config=(self.sess_config))
            self._sess.run(tf.global_variables_initializer())
            ...
            print()
            loss_w = [loss_list[gid] * self.settings.gpu_batch_split[gid] for gid in range(self.num_gpu)]
            self._loss_tensor = tf.add_n(loss_w) / self.settings.batch_size
            if self.settings.use_metric_in_graph:
                metric_w = [metric_list[gid] * self.settings.gpu_batch_split[gid] for gid in range(self.num_gpu)]
    ...
    def assign_learning_rate(self, lr_value):
        with self._graph.as_default():
            self._sess.run(tf.assign(self.learning_rate_tensor, tf.constant(lr_value, dtype=(tf.float32))))

    def save_ckpt_best(self, model_dir, model_name, step):
        self._saver_best.save((self._sess), (os.path.join(model_dir, model_name)), global_step=step)

    def save_ckpt(self, model_dir, model_name, step):
        self._saver.save((self._sess), (os.path.join(model_dir, model_name)), global_step=step)
    ...
    def load_ckpt_and_save_pb_file(model, dir_ckpt):
        ...
        constant_graph = graph_util.convert_variables_to_constants((model._sess),
          (model._sess.graph_def), output_node_names=(model.pb_save_names))
    ...
    def prepare_for_prediction_with_pb(self, pb_file_path=None):
        ...
        """
        if pb_file_path is None:
            pb_file_path = self.settings.pb_file
        if not os.path.exists(pb_file_path):
            if not False:
                raise AssertionError('ERROR: %s NOT exists, when prepare_for_prediction()' % pb_file_path)
        self._graph = tf.Graph()
        with self._graph.as_default():
...
def get_assignment_map_replaced(init_ckpt, name_replacement_dict={}, list_vars=None):
...
def initialize_from_ckpt(init_ckpt, name_replacement_dict={}, non_trainable_names=[], list_vars=None, assignment_map=None):
    """ name_replacement_dict = { old_name_str_chunk: new_name_str_chunk }
        non_trainable_names = ["bert", "word_embeddings"]  # for example
    """
    if assignment_map is None:
        assignment_map = get_assignment_map_replaced(init_ckpt, name_replacement_dict, list_vars)
