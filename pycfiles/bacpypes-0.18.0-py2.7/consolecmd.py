...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/consolecmd.py
# Compiled at: 2016-12-14 13:49:55
"""
Console Command
"""
import sys, types, os, gc, signal, cmd, logging
...
def console_interrupt(*args):
    if _debug:
        console_interrupt._debug('console_interrupt %r', args)
...
class ConsoleCmd(cmd.Cmd, Thread, Logging):
    ...
    def run(self):
        ...
        self.cmdloop()
        if _debug:
            ConsoleCmd._debug('    - done cmdloop')
    ...
    def do_bugout(self, args):
        ...
        if not logger:
            self.stdout.write('not a valid logger name\n')
        elif loggerName not in self.handlers:
    ...
    def do_buggers(self, args):
        ...
        for loggerName in sorted(loggers):
            if args and args[0] not in loggerName:
                continue
    ...
    def do_help(self, args):
        ...
        """
        if _debug:
            ConsoleCmd._debug('do_help %r', args)
