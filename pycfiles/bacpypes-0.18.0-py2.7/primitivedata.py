...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/primitivedata.py
# Compiled at: 2020-01-29 15:49:52
"""
Primitive Data
"""
import sys, struct, time, re, unicodedata
...
class Tag(object):
    ...
    def __eq__(self, tag):
        return self.tagClass == tag.tagClass and self.tagNumber == tag.tagNumber and self.tagLVT == tag.tagLVT and self.tagData == tag.tagData
...
class Integer(Atomic):
    ...
    def encode(self, tag):
        data = bytearray(struct.pack('>I', self.value & 4294967295))
        if self.value < 0:
            while len(data) > 1:
                if data[0] != 255:
                    break
...
class Enumerated(Atomic):
    ...
    def is_valid(cls, arg):
        ...
        """
        return isinstance(arg, (int, long)) and arg >= 0 or isinstance(arg, str)
...
_mm = '(?P<month>0?[1-9]|1[0-4]|odd|even|255|[*])'
_dd = '(?P<day>[0-3]?\\d|last|odd|even|255|[*])'
_yy = '(?P<year>\\d{2}|255|[*])'
_yyyy = '(?P<year>\\d{4}|255|[*])'
_dow = '(?P<dow>[1-7]|mon|tue|wed|thu|fri|sat|sun|255|[*])'
...
def _merge(*args):
    ...
    return re.compile('^' + ('[/-]').join(args) + '(?:\\s+' + _dow + ')?$')
...
class Date(Atomic):
    ...
    def now(self, when=None):
        ...
        self.value = (
         tup[0] - 1900, tup[1], tup[2], tup[6] + 1)
        return self
...
class Time(Atomic):
    ...
    def now(self, when=None):
        ...
        tup = time.localtime(when)
        self.value = (
         tup[3], tup[4], tup[5], int((when - int(when)) * 100))
        return self
...
class ObjectType(Enumerated):
    ...
    enumerations = {'accessDoor': 30, 'accessPoint': 33, 
       'accessRights': 34, 
       'accessUser': 35, 
       'accessZone': 36, 
       'accumulator': 23, 
...
class ObjectIdentifier(Atomic):
    ...
    maximum_instance_number = 4194303
