...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/vlan.py
...
class Network:
    ...
    def __init__(self, name='', broadcast_address=None, drop_percent=0.0):
        if _debug:
            Network._debug('__init__ name=%r broadcast_address=%r drop_percent=%r', name, broadcast_address, drop_percent)
        self.name = name
        self.nodes = []
    ...
    def process_pdu(self, pdu):
        ...
        """
        if _debug:
            Network._debug('process_pdu(%s) %r', self.name, pdu)
        if self.traffic_log:
...
class Node(Server):
    ...
    def __init__(self, addr, lan=None, name='', promiscuous=False, spoofing=False, sid=None):
        if _debug:
            Node._debug('__init__ %r lan=%r name=%r, promiscuous=%r spoofing=%r sid=%r', addr, lan, name, promiscuous, spoofing, sid)
...
class IPNetwork(Network):
    ...
    def __init__(self, name=''):
        if _debug:
            IPNetwork._debug('__init__')
        Network.__init__(self, name=name)

    def add_node(self, node):
        if _debug:
            IPNetwork._debug('add_node %r', node)
        if not self.nodes:
...
class IPNode(Node):
    ...
    def __init__(self, addr, lan=None, promiscuous=False, spoofing=False, sid=None):
        if _debug:
            IPNode._debug('__init__ %r lan=%r', addr, lan)
        if not isinstance(addr, Address) or not hasattr(addr, 'addrTuple') or not hasattr(addr, 'addrBroadcastTuple'):
...
class IPRouterNode(Client):
    ...
    def __init__(self, router, addr, lan):
        if _debug:
            IPRouterNode._debug('__init__ %r %r lan=%r', router, addr, lan)
        self.router = router
        self.lan = lan
    ...
    def confirmation(self, pdu):
        if _debug:
            IPRouterNode._debug('confirmation %r', pdu)
        self.router.process_pdu(self, pdu)

    def process_pdu(self, pdu):
        if _debug:
            IPRouterNode._debug('process_pdu %r', pdu)
        self.request(pdu)
