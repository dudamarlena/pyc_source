...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/AryaLogger/convert_apic_dn_to_cobra.py
# Compiled at: 2015-08-10 13:57:07
"""
...
def convert_dn_to_cobra(dn_str):
    ...
    for arn in dn_dict.items():
        if len(list(arn[1]['namingVals'])) > 0:
            ...
            nvals_str = ''
        print ('{0} = {1}({2}{3})').format(arn[1]['moClassName'], arn[1]['className'], arn[1]['parentMoOrDn'], nvals_str)
...
if __name__ == '__main__':
    ...
    print ''
    URL = 'https://10.122.254.211/api/node/mo/topology/HDfabricOverallHealt' + 'h5min-0.json'
    convert_dn_to_cobra(apic_rest_urlparse(URL).dn_or_class)
    print ''
    URL = 'https://10.122.254.211/api/node/mo/uni/tn-mgmt/mgmtp-default/oob' + '-default.json'
    convert_dn_to_cobra(apic_rest_urlparse(URL).dn_or_class)
    print