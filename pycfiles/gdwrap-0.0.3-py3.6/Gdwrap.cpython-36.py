...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/gdwrap/Gdwrap.py
# Compiled at: 2018-06-20 05:24:06
...
class Gdwrap:
    ...
    def file_list(self, page_size: int, fields: str, order_by: str, query: str) -> dict:
        ...
        :return:
        """
        service = self._Gdwrap__get_service()
        results = service.files().list(pageSize=page_size,
          fields=fields,
          orderBy=order_by,
          q=query).execute()
    ...
    def file_download(self, item_id: str, item_name: str, dir_name: str) -> bool:
        ...
        self._Gdwrap__create_download_dir(dir_name)
    ...
    def file_upload(self, folder_id: str, file_name: str, mine_type: str) -> bool:
        ...
        body = {'name':os.path.split(file_name)[-1], 
         'mimeType':'text/csv', 
         'parents':[
          folder_id]}
    ...
    def __create_download_dir(dir_name) -> None:
        return os.makedirs(dir_name, 511, True)