...
# Compiled at: 2020-03-24 05:35:19
...
def print_2d(exp_name, num_winners=0, mask=False, angle=0, reverse=False, update=False, values='default', coloring='purple', num_elections=800, winners_order='approx_cc', main_order=''):
    """ Print the two-dimensional embedding of multi-dimensional map of the elections """
    model = obj.Model_2d(exp_name, num_winners=num_winners, num_elections=num_elections, winners_order=winners_order, main_order=main_order)
...
def print_matrix(exp_name, scale=1.0):
...
def prepare_approx_cc_order(exp_name):
    ...
    for i in range(num_elections):
        ...
        dst = os.path.join(dst, 'experiments', str(exp_name), 'elections', 'soc_approx_cc', 'tmp_' + str(i) + '.soc')
        copyfile(src, dst)
