...
# Compiled at: 2020-03-12 06:33:00
    ...
    if reverse:
        ...
        if update:
            ...
            if shades:
                ...
                for k in range(model.num_families):
                    for _ in range(model.family_size[k]):
                        if ctr < 200:
                            ...
                            shade = 1.0
                        ax.scatter((model.points[ctr][0]), (model.points[ctr][1]), color=(model.colors[k]),
                          alpha=(model.alphas[k] * shade),
                          s=9)
                        ctr += 1
                ...
                for k in range(model.num_families):
                    if k < 30:
                        ax.scatter((model.points_by_families[k][0]), (model.points_by_families[k][1]), color=(model.colors[k]),
                          label=(model.labels[k]),
                          alpha=(model.alphas[k]),
                          s=9)
                    else:
                        ax.scatter((model.points_by_families[k][0]), (model.points_by_families[k][1]), color=(model.colors[k]),
                          label=(model.labels[k]),
...
def generate_matrix(experiment_id, scale=1.0):
    file_name = os.path.join(os.path.abspath(os.path.dirname(__file__)), '..')
    ...
    matrix = [[0.0 for _ in range(second_line)] for _ in range(second_line)]
    ...
    fig, ax = plt.subplots()
    matrix_order = [[0.0 for _ in range(num_families_order)] for _ in range(num_families_order)]
    for i in range(num_families_order):
