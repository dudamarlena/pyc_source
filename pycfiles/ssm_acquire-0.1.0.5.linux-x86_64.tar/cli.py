...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/ssm_acquire/cli.py
# Compiled at: 2019-08-06 11:11:16
"""Console script for ssm_acquire."""
import boto3, sys, click, itertools, time
...
def main(instance_id, region, build, acquire, interrogate, analyze, deploy):
    ...
    if acquire is True or interrogate is True or build is True or analyze is True:
        ...
        ssm_client = boto3.client('ssm', aws_access_key_id=credentials['Credentials']['AccessKeyId'], aws_secret_access_key=credentials['Credentials']['SecretAccessKey'], aws_session_token=credentials['Credentials']['SessionToken'])
    spinner = itertools.cycle(['-', '/', '|', '\\'])
    if analyze is True:
        logger.info('Analysis mode active.')
        analyzer = da.RekallManager(instance_id, credentials)
    ...
    if build is True:
        ...
        if status == 'Success':
            logger.info(('Rekall profile build complete. A .zip has been added to the asset store for instance: {}').format(instance_id))
        else:
    ...
    if interrogate is True:
        ...
        logger.info(('Attemping to interrogate the instance using the OSQuery binary for instance_id: {}').format(instance_id))
        ...
        if status == 'Success':
            logger.info(('Interrogation of system complete.  The result of this has been added to asset store for: {}').format(instance_id))
        else:
