...
class S3Manager(object):
    ...
    def _connect(self):
        if self.s3_client is None:
            ...
            self.s3_client = boto3.client('s3', aws_access_key_id=self.credentials['Credentials']['AccessKeyId'], aws_secret_access_key=self.credentials['Credentials']['SecretAccessKey'], aws_session_token=self.credentials['Credentials']['SessionToken'])
        return

    def list_objects_for_key(self, object_key):
        self._connect()
        response = self.s3_client.list_objects(Bucket=self.bucket_name, Prefix=object_key)
    ...
    def get_files(self, object_keys):
        ...
        for object_key in object_keys:
            ...
            response = self.s3_client.get_object(Bucket=self.bucket_name, Key=object_key.get('Key'))
...
class RekallManager(object):
    ...
    def _run_a_container(self, command, volumes):
        return self.client.containers.run(image=self.docker_image, command=command, detach=True, volumes=volumes)
    ...
    def run_yara_scan(self):
        ...
        if os.path.isdir(yara_file_dir) and len(os.listdir(yara_file_dir)) != 0:
            for yara_file in os.listdir(yara_file_dir):
                ...
                command = ('rekall -f /files/capture.aff4 --profile /files/{}json {} {}                         --format=json --output=/files/{}-{}-output.json').format(rekall_profile_name.split('zip')[0], plugin, additional_arg, ('yara-scan-{}').format(yara_file), self.instance_id)

            container = self._run_a_container(command, {('/tmp/{}').format(self.instance_id): {'bind': ('/files').format(self.instance_id), 'mode': 'rw'}, ('{}').format(yara_file_dir): {'bind': '/opt/yarascan', 'mode': 'rw'}})
    ...
    def run_rekall_plugins(self):
        ...
        command = ('rekall convert_profile {} {}json').format(rekall_profile_name, rekall_profile_name.split('zip')[0])
        volumes = {('/tmp/{}').format(self.instance_id): {'bind': '/files', 'mode': 'rw'}}
        ...
        for plugin in self.rekall_plugins:
            ...
            command = ('rekall -f /files/capture.aff4 --profile /files/{}json {}                     --format=json --output=/files/{}-{}-output.json').format(rekall_profile_name.split('zip')[0], plugin, plugin, self.instance_id)
            volumes = {('/tmp/{}').format(self.instance_id): {'bind': '/files', 'mode': 'rw'}}
            container = self._run_a_container(command, volumes)
            plugin_containers.append({'plugin': plugin, 
               'container': container})
        ...
        for plugin in self.rekall_plugins:
            ...
            s3_manager.put_file(('/tmp/{}/{}-{}-output.json').format(self.instance_id, plugin, self.instance_id), self.instance_id)
