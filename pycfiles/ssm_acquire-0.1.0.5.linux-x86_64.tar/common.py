...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/ssm_acquire/common.py
...
def load_transfer(credentials, instance_id):
    ...
    transfer_plan = jinja_template.render(ssm_acquire_access_key=credentials['Credentials']['AccessKeyId'], ssm_acquire_secret_key=credentials['Credentials']['SecretAccessKey'], ssm_acquire_session_token=credentials['Credentials']['SessionToken'], ssm_acquire_s3_bucket=s3_bucket, ssm_acquire_instance_id=instance_id)
...
def load_build(credentials, instance_id):
    ...
    s3_bucket = config('asset_bucket', namespace='ssm_acquire')
    jinja_template = Template(template_contents)
    build_plan = jinja_template.render(ssm_acquire_access_key=credentials['Credentials']['AccessKeyId'], ssm_acquire_secret_key=credentials['Credentials']['SecretAccessKey'], ssm_acquire_session_token=credentials['Credentials']['SessionToken'], ssm_acquire_s3_bucket=s3_bucket, ssm_acquire_instance_id=instance_id)
...
def load_interrogate(credentials, instance_id):
    ...
    s3_bucket = config('asset_bucket', namespace='ssm_acquire')
    jinja_template = Template(template_contents)
    interrogate_plan = jinja_template.render(ssm_acquire_access_key=credentials['Credentials']['AccessKeyId'], ssm_acquire_secret_key=credentials['Credentials']['SecretAccessKey'], ssm_acquire_session_token=credentials['Credentials']['SessionToken'], ssm_acquire_s3_bucket=s3_bucket, ssm_acquire_instance_id=instance_id)
...
def run_command(client, commands, instance_id):
    ...
    response = client.send_command(InstanceIds=[
     instance_id], DocumentName='AWS-RunShellScript', Comment=('Incident response step execution for: {}').format(instance_id), Parameters={'commands': commands})
    return response
...
def check_status(client, response, instance_id):
    ...
    response = client.get_command_invocation(CommandId=response['Command']['CommandId'], InstanceId=instance_id)
