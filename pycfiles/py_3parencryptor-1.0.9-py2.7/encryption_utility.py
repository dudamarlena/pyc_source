...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hpe3parencryptor/encryption_utility.py
# Compiled at: 2018-10-26 08:18:02
import etcd, json, argparse, string
from configparser import SafeConfigParser
from Crypto.Cipher import AES
import base64, sys, subprocess, os, time
LEN16 = 16
LEN24 = 24
LEN32 = 32
BACKENDROOT = '/backend'
...
parser = argparse.ArgumentParser(description='Encryption Tool', usage='hpe3parencryptor [OPTIONS]')
parser.add_argument('-a', nargs=2, help='key addition, need key and secret', metavar=('key',
                                                                                      'secret'))
parser.add_argument('-d', action='store_true', help='This will delete the key stored')
parser.add_argument('--backend', dest='backend', help='backend name, default is %s' % BACKEND, default=BACKEND)
...
class EtcdUtil(object):
    ...
    def __init__(self, host, port, client_cert, client_key):
        ...
        if client_cert is not None and client_key is not None:
            if len(host_tuple) > 0:
                self.client = etcd.Client(host=host_tuple, port=port, protocol='https', cert=(
                 client_cert, client_key), allow_reconnect=True)
            else:
                self.client = etcd.Client(host=host, port=port, protocol='https', cert=(
                 client_cert, client_key))
        else:
            if len(host_tuple) > 0:
                self.client = etcd.Client(host=host_tuple, port=port, protocol='http', allow_reconnect=True)
            else:
...
cl = EtcdUtil(host_etcd_ip_address, host_etcd_port_number, host_etcd_client_cert, host_etcd_client_key)
