...
# [GCC 8.4.0]
# Embedded file name: /Users/leon/.local/lib/python2.7/site-packages/VideoAnalysis/__init__.py
# Compiled at: 2014-12-10 11:38:17
...
class Rectangle(object):
    ...
    def __repr__(self):
        name = self.__class__.__name__
        f = '{name}(({x0}, {x1}), ({y0}, {y1}))'
        return f.format(name=name, x0=self.xi.lb, x1=self.xi.ub, y0=self.yi.lb, y1=self.yi.ub)
...
class HTMLImageWidget(HTMLWidget):
    ...
    def __init__(self, image, description='', maxHeight=512, maxWidth=512, maxScale=1.0, scratchDir=None, imageURLBase='/static/scratch/', autoEnhance=True, **kwargs):
        super(HTMLImageWidget, self).__init__(description=description, **kwargs)
        self.setImage(image=image, description=description, maxHeight=maxHeight, maxWidth=maxWidth, maxScale=maxScale, scratchDir=scratchDir, imageURLBase=imageURLBase, autoEnhance=autoEnhance)

    __init__.argNames = lambda : argNames(HTMLWidget)

    def setImage(self, image, description='', maxHeight=512, maxWidth=512, maxScale=1.0, scratchDir=None, imageURLBase='/static/scratch/', autoEnhance=True):
        if scratchDir:
            ...
            try:
                ...
                self.scratchDir = os.environ['HOME'] + '/.ipython/profile_default' + imageURLBase
        ...
        self.scale = min(float(self.maxWidth) / self.imageWidth, float(self.maxHeight) / self.imageHeight, self.maxScale)
