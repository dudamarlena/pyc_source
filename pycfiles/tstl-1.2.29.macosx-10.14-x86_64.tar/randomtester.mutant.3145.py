...
# [GCC 8.4.0]
# Embedded file name: /opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tstl/randomtester.mutant.3145.py
# Compiled at: 2019-02-26 15:24:16
    ...
    if config.swarmProbs is not None:
        ...
        while config.maxTests == -1 or ntests < config.maxTests:
            ...
            if config.replayable:
                currtest = open('currtest.test', 'w')
            testFailed = False
            if not (config.exploit is not None and time.time() - start >= config.startExploit and testsWithNoNewCoverage >= config.startExploitStall):
                if neverExploited:
