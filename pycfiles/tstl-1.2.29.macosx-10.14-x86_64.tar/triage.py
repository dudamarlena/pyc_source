...
# [GCC 8.4.0]
# Embedded file name: /opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tstl/triage.py
...
def parse_args():
    ...
    parser.add_argument('testglob', metavar='filename', type=str, default=None, help='Glob for tests to run.')
    parser.add_argument('--noCheck', action='store_true', help='Do not check properties.')
    parser.add_argument('--showTests', action='store_true', help='Show the tests.')
    parser.add_argument('--abstractStrings', action='store_true', help='Abstract away strings in exceptions.')
    parser.add_argument('--ignoreContaining', type=str, default=None, help='Ignore tests with provided string(s) in an action (separate strings with ";;").')
    parser.add_argument('--actionClasses', action='store_true', help='Use set of action classes in test as part of signature.')
...
def main():
    ...
    for fn in glob.glob(config.testglob):
        ...
        if fails:
            ...
            sig = (
             noDigits(e), noDigits(line))
            if config.abstractStrings:
                sig = (
                 noStrings(sig[0]), sig[1])
            ...
            if sig not in signatures:
                ...
                signatures[sig] = (
                 fn, t, sut.failure(),
                 sut.prettyName(t[(-1)][0]), signatures[sig][4] + 1)
            else:
    ...
    for sig in sorted(signatures.keys(), key=lambda x: signatures[x][4], reverse=True):
        ...
        print('COUNT:', signatures[sig][4], '(' + str(round(signatures[sig][4] / numTests, 2) * 100) + '%)')
        if config.showTests:
