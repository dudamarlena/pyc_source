...
# [GCC 8.4.0]
# Embedded file name: /opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tstl/harnessmaker.py
# Compiled at: 2019-11-22 11:03:44
...
def parse_args():
    ...
    parser.add_argument('tstl', metavar='filename', type=str, default=None, help='Path to the .tstl file.')
    parser.add_argument('-v', '--version', action='store_true', help='Display the tstl version.')
    parser.add_argument('-o', '--output', type=str, default='sut.py', help='Name of the file containing the generated harness code (default = sut.py)')
    parser.add_argument('-c', '--classname', type=str, default='sut', help='Name of the class representing the SUT (default = sut)')
    parser.add_argument('-n', '--noCover', action='store_true', help='Disable generating coverage collection support.  Faster testing, but no coverage info!')
    parser.add_argument('-r', '--coverReload', action='store_true', help='Generate coverage for module reload behavior.')
    parser.add_argument('-i', '--coverInit', action='store_true', help='Generate coverage for SUT initialization behavior.')
    parser.add_argument('-e', '--enumerateEnabled', action='store_true', help='Enumerate enabled actions instead of trying to find one, useful when |actions| typically >> |enabled|.')
    parser.add_argument('-R', '--defaultReplay', action='store_true', help='Backtracking defaults to replay method.')
    parser.add_argument('--forceRefExceptionMatch', action='store_true', help='Exceptions must match (basic type, not message) with reference.')
    parser.add_argument('--forceStrongRefExceptionMatch', action='store_true', help='Exceptions must match message with reference.')
    parser.add_argument('--checkFailureDeterminism', action='store_true', help='Check that failures are deterministic.')
    parser.add_argument('-a', '--ignoreAngles', action='store_true', help='Do not use angle brackets as TSTL markers, for use with some languages.')
    parser.add_argument('-s', '--stats', action='store_true', help='Enable statistics on each action.  Slows exection considerably.')
    parser.add_argument('-p', '--pylib', action='store_true', help='Add if you need to test something in the standard Python library.  Will blow up coverage!')
    parser.add_argument('--noReload', action='store_true', help='Do not reload module on each restart.  For testing modules with broken reload (e.g. TensorFlow)')
...
def main():
            ...
            if inside_literal_block or line[0] == '@':
                ...
                if re.match('def guarded', line):
                    ...
                    if line.find('%COMMIT%') != -1:
                        outf.write(line.replace('%COMMIT%', 'if SPECULATIVE_CALL: return True'))
                    m = re.match('.*PRE%\\((.+)\\)%.*', line)
                    while m:
                        anyPRE = True
                        pre_expr = m.groups()[0]
                        spre_expr = "'''" + pre_expr + "'''"
                        line = line.replace('PRE%(' + pre_expr + ')%', '__pre[' + spre_expr + ']', 1)
                        function_code = [
    ...
    if not len(code) > 0:
        raise AssertionError('No non-comment lines found in .tstl file')
        poolPrefix = 'self.p_'
        sourceSet = []
        initSet = []
        firstInit = True
        ...
        for c in code:
            ...
            if cs[0][0] == '#':
                ...
                refLeft = rs[0]
                refRight = rs[1][:-1]
                leftMeth = re.match('METHOD\\((\\S+)\\)', refLeft)
                if leftMeth:
                    method = leftMeth.groups()[0]
                    refLeft = '(\\S+)\\.' + method + '\\(\\)'
                leftCall = re.match('CALL\\((\\S+)\\)', refLeft)
                if leftCall:
                    function = leftCall.groups()[0]
                    refLeft = function + '\\((\\S+)\\)'
                rightCall = re.match('CALL\\((\\S+)\\)', refRight)
                if rightCall:
                    function = rightCall.groups()[0]
                    refRight = function + '(\\1)'
                rightMeth = re.match('METHOD\\((\\S+)\\)', refRight)
                if rightMeth:
        ...
        for line in logSet:
            ...
            if refl != line:
                for base in referenceMap:
                    if re.match('^[a-zA-Z_0-9]+$', refl):
                        refl = re.sub('\\b' + base + '\\b', referenceMap[base], refl)
                    else:
        ...
        for corig in code:
            ...
            if assumeSet == []:
                ...
                guardConds = [
                 '(self.__assumptionViolated is None)']
            changes = []
            okExcepts = ''
            ...
            if corig[0] == '{':
                ...
                for p in poolSet:
                    ...
                    for assign in plhs:
                        ...
                        changes.append('if self.__useCould and ' + g + " is not None: self.noLongerNone('''" + g + "''')")
                        changes.append('if self.__useCould and ' + g + " is not None: self.noLongerUsed('''" + g + "''')")
                        g = g.replace('[', '_used[')
                ...
                for base in referenceMap:
                    if re.match('^[a-zA-Z_0-9]+$', refC):
                        refC = re.sub('\\b' + base + '\\b', referenceMap[base], refC)
                    else:
                ...
                if refC != newC:
                    ...
                    if config.forceStrongRefExceptionMatch:
                        ...
                        genCode.append(baseIndent + 'assert type(raised) == type(refRaised), ' + '" (%s) == (%s) " % (type(raised), type(refRaised))\n')
                    if comparing:
        ...
        if not config.noCover:
            ...
            if len(sourceSet) > 0:
                covc = covc[:-1] + '] + self.moduleLocations(),'
            else:
                covc += 'self.moduleLocations(),'
            genCode.append(covc)
            if config.pylib:
        ...
        if finallySet != []:
            for f in finallySet:
                ...
                for base in referenceMap:
                    if re.match('^[a-zA-Z_0-9]+$', refF):
                        refF = re.sub('\\b' + base + '\\b', referenceMap[base], refF)
                    else:
        ...
        boilerplate_path = config.noCover or os.path.join(base_path, 'static', 'boilerplate_cov')
