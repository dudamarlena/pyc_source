...
# [GCC 8.4.0]
# Embedded file name: /opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tstl/randomtester.py
# Compiled at: 2019-11-04 15:45:19
from __future__ import print_function
import os, sys, math, random, time, traceback, argparse, glob, datetime, inspect
...
def parse_args():
    ...
    parser.add_argument('-t', '--timeout', type=int, default=3600, help='Timeout in seconds (3600 default).')
    ...
    parser.add_argument('-w', '--swarm', action='store_true', help='Turn on standard swarm testing.')
    parser.add_argument('-r', '--running', action='store_true', help='Produce running branch coverage report.')
    parser.add_argument('--normalize', action='store_true', help='Normalize/simplify after reduction.')
    parser.add_argument('--generalize', action='store_true', help='Generalize tests.')
    parser.add_argument('-n', '--noCover', action='store_true', help="Don't check code coverage.")
    parser.add_argument('--postCover', action='store_true', help='No coverage during test generation/execution; compute coverage at end. ' + 'Adds runtime, for experiments on lightweight methods.')
    parser.add_argument('--html', type=str, default=None, help='Write HTML report (directory to write to, None/no html report by default).')
    parser.add_argument('-m', '--maxTests', type=int, default=-1, help='Maximum #tests to run (-1 = infinite default).')
    parser.add_argument('-o', '--output', type=str, default='failure.' + str(os.getpid()) + '.test', help='Filename to save failing test(s).')
    parser.add_argument('-M', '--multiple', action='store_true', help='Allow multiple failures.')
    parser.add_argument('--replayable', action='store_true', help='Keep replayable file of current test, in case of crash.')
    parser.add_argument('--total', action='store_true', help='Keep a file with ALL TESTING ACTIONS in case of crash.')
    parser.add_argument('--noCheck', action='store_true', help='Do not check properties.')
    parser.add_argument('--uncaught', action='store_true', help='Allow uncaught exceptions in actions.')
    parser.add_argument('--checkDeterminism', action='store_true', help='Check determinism of pool objects.')
    parser.add_argument('--determinismTries', type=int, default=1, help='Number of tries to catch nondeterminism (default 1).')
    parser.add_argument('--determinismDelay', type=float, default=0, help='Delay when checking for nondeterminism (default 0).')
    parser.add_argument('--checkProcessDeterminism', action='store_true', help='Check that tests are process deterministic.')
    parser.add_argument('--processDetTries', type=int, default=1, help='Number of tries to catch process nondeterminism (default 1).')
    parser.add_argument('--processDetDelay', type=float, default=0, help='Delay when checking process nondeterminism (default 0).')
    parser.add_argument('-q', '--quickTests', action='store_true', help='Produce quick tests for coverage (save a test all coverage targets).')
    parser.add_argument('--quickPrefix', type=str, default='quick', help="Prefix to use for quick tests (default 'quick'.")
    parser.add_argument('--readQuick', action='store_true', help='Read existing quick tests (and add to them if producing quick tests).')
    parser.add_argument('--localize', action='store_true', help='Produce fault localization (Ochai formula) if there are any failing tests.')
    parser.add_argument('--localizeTop', type=int, default=20, help='Show top n localization results (default=20).')
    parser.add_argument('--full', action='store_true', help="Don't reduce -- report full failing test.")
    parser.add_argument('-l', '--logging', type=int, default=None, help='Set logging level')
    parser.add_argument('--failedLogging', type=int, default=None, help='Set failed test case logging level')
    parser.add_argument('--progress', action='store_true', help='Turn on progress report.')
    parser.add_argument('--timedProgress', type=int, default=30, help='Turn on progress reports at x second intervals (default = 30 seconds).')
    parser.add_argument('--ddmin', action='store_true', help='Use standard binary-search-like ddmin instead of greedy single-step method.')
    parser.add_argument('--useDependencies', action='store_true', help='Use dependencies to prune list of enabled actions.')
    parser.add_argument('--enumerateEnabled', action='store_true', help='Instead of guessing enabled actions, enumerate them;' + 'can speed testing greatly for cases where almost all actions are usually disabled.')
    parser.add_argument('--noEnumerateEnabled', action='store_true', help='Turn off enumeration of enabled actions in SUTs compiled with that default.')
    parser.add_argument('-k', '--keepLast', action='store_true', help='Keep last action the same when reducing: slippage avoidance heuristic.')
    parser.add_argument('--noPruneGuards', action='store_true', help='Do not prune based on guards (useful in determinism checks)')
    parser.add_argument('--swarmP', type=float, default=0.5, help='Swarm inclusion probability.')
    parser.add_argument('--computeFeatureStats', action='store_true', help='When using swarm testing, compute coverage triggers and suppressors')
    parser.add_argument('--saveSwarmCoverage', type=str, default=None, help='Save data on swarm configs and coverage to file.')
    parser.add_argument('--highLowSwarm', type=float, default=None, help='Apply high/low probability swarm testing with high portion of action being P.')
    parser.add_argument('--swarmProbs', type=str, default=None, help='File with probabilities for any kind of swarm.')
    parser.add_argument('--swarmFromTest', type=str, default=None, help='File with test on which to base 0/1 swarm probabilities.')
    parser.add_argument('--swarmSwitch', type=int, default=None, help='How many times to switch swarm config during each test.')
    parser.add_argument('--swarmLength', type=int, default=None, help='How long a swarm config persists (only relevant with --swarmSwitch).')
    parser.add_argument('--probs', type=str, default=None, help='Guide testing by an action class probability file.')
    parser.add_argument('--equalProbs', action='store_true', help='Force all action classes to have equal probabilities.')
    parser.add_argument('--generateLOC', type=str, default=None, help='Generate LOC data file to bias testing by LOC estimates for actions.' + 'Parameter is the filename.')
    parser.add_argument('--biasLOC', type=str, default=None, help='Read LOC data file to bias testing by LOC estimates for actions. ' + 'Uses files produced by --generateLOC. ' + "Any file of the same structure (action class / LOC count pairs, with ' %%%% '" + 'separating) will work, however.')
    parser.add_argument('--LOCBaseline', type=float, default=0.2, help='Baseline probability for actions that do not call any SUT code (default 0.2).')
    parser.add_argument('--LOCProbs', action='store_true', help='Guide testing by approximate relative lines of top-level code in actions. ' + 'Note: much less effective than generateLOC/biasLOC in most cases')
    parser.add_argument('--markov', type=str, default=None, help='Guide testing by a Markov model file.')
    parser.add_argument('--markovP', type=float, default=1.0, help='Probability to guide action choice by Markov model.')
    parser.add_argument('--sequencesFromTests', type=str, default=None, help='Construct tests from subsequences of a collection of existing tests' + '(glob pattern for tests, default None).')
    parser.add_argument('--sequenceP', type=float, default=1.0, help='Probability to guide action choice by sequence.')
    parser.add_argument('--sequenceSize', type=int, default=3, help='Minimum size of a sequence (that is not an entire test) from the tests in the' + 'directory for --sequencesFromTests (default 3).')
    parser.add_argument('--useQuickSequences', action='store_true', help='New quick tests add sequences.')
    parser.add_argument('-x', '--exploit', type=float, default=None, help='Probability to exploit stored coverage tests.')
    parser.add_argument('--savePool', type=str, default=None, help='Save pool generated during exploitation.')
    parser.add_argument('--readPool', type=str, default=None, help='Read pool files generated during exploitation.')
    parser.add_argument('--startExploit', type=float, default=0.0, help='Time at which exploitation starts (default 0.0, LOWER BOUND: this plus' + 'startExploitStall must hold).')
    parser.add_argument('--startExploitStall', type=int, default=0, help='Number of no-new-coverage tests at which exploitation starts (default 0,' + 'LOWER BOUND: this plus startExploit must hold).')
    parser.add_argument('--verboseExploit', action='store_true', help='Exploitation is verbose (info on pool, etc.).')
    parser.add_argument('--reducePool', action='store_true', help='Reduce tests by their new coverage before adding to exploitation pool.')
    parser.add_argument('--exploitCeiling', type=float, default=0.5, help='Max ratio to mean coverage count for exploitation.')
    parser.add_argument('--Pmutate', type=float, default=0.0, help='Probability to mutate exploited tests (default = 0.0 -- no mutation).')
    parser.add_argument('--Pcrossover', type=float, default=0.2, help='Probability to try crossover when mutating exploited tests (default = 0.2).')
    parser.add_argument('--useHints', action='store_true', help='Use hint values (utility to maximize) as another cause for exploitation.')
    parser.add_argument('--noCoverageExploit', action='store_true', help='Do not use coverage in exploitation (only useful with heuristic hints).')
    parser.add_argument('--exploitBestHint', type=int, default=1, help='How many of the best heuristic values to put in active pool for exploitation.')
    parser.add_argument('--internal', action='store_true', help='Produce internal coverage report at the end.')
    parser.add_argument('--fullCoverage', type=str, default=None, help='File to save a complete coverage report to.')
    parser.add_argument('--coverFile', type=str, default='coverage.out', help="File to write coverage report to ('coverage.out' default).")
    parser.add_argument('--noExceptionMatch', action='store_true', help='Do not force exceptions in reduced / normalized failures to match.')
    parser.add_argument('-v', '--verbose', action='store_true', help='Run in verbose mode.')
    parser.add_argument('--silentFail', action='store_true', help="Don't make failure replays verbose.")
    parser.add_argument('--silentSUT', action='store_true', help='Silence SUT actions (no output).')
    parser.add_argument('--throughput', action='store_true', help='Measure action throughput.')
    parser.add_argument('--profile', action='store_true', help='Profile actions.')
    parser.add_argument('--profileProbs', action='store_true', help='Use action profile to prefer less-taken actions.')
    parser.add_argument('--stopSaturated', action='store_true', help="Use multiple 'sessions' to estimate saturation and stop when saturated.")
    parser.add_argument('--sessions', type=int, default=4, help='Number of sessions to use for estimating coverage saturation (default 4).  ' + 'More sessions means less chance of missing coverage/faults.')
    parser.add_argument('--stopWhenBranches', type=int, default=None, help='Stop when branch coverage exceeds a given target value (default None).')
    parser.add_argument('--stopWhenStatements', type=int, default=None, help='Stop when statement coverage exceeds a given target value (default None).')
    parser.add_argument('--stopWhenNoCoverage', type=int, default=None, help='Stop when no additional coverage for this many tests (default None).')
    parser.add_argument('--stopTestWhenNoCoverage', type=int, default=None, help='Stop test when no additional coverage for this many steps (default None).')
    parser.add_argument('--stopTestWhenThroughputBelow', type=float, default=None, help='Stop test when throughput (actions/sec) falls below threshold (default None).')
    parser.add_argument('--stopWhenHitBranch', type=str, default=None, help='Stop testing when given branch is hit (default None).')
    parser.add_argument('--stopWhenHitStatement', type=str, default=None, help='Stop testing when given statement is hit (default None).')
    parser.add_argument('--trackMaxCoverage', type=str, default=None, help='Track test with highest branch/statement (tiebreaker) coverage and store' + 'in this file (default None).')
    parser.add_argument('--maxMustHitBranch', type=str, default=None, help='Best coverage test must hit this branch (default None).')
    parser.add_argument('--maxMustHitStatement', type=str, default=None, help='Best coverage test must hit this statement (default None).')
    parser.add_argument('--verboseActions', action='store_true', help='Make test actions verbose.')
    parser.add_argument('--hideOpaque', action='store_true', help='Do not print values of opaque pools.')
    parser.add_argument('--showActions', action='store_true', help='Show actions as they run')
    parser.add_argument('--noAlphaConvert', action='store_true', help='Do not alpha convert failing tests.')
    parser.add_argument('--compareFails', action='store_true', help='Compare all failing tests.')
    parser.add_argument('--noSwarmDependencies', action='store_true', help='[EXPERIMENTAL] This forces swarm to not use dependencies. ' + 'Not so much experimental as just a bad idea.')
    parser.add_argument('--noSwarmForceParent', action='store_true', help='Removes condition forcing swarm generation to include at least one' + ' parent (dependent-on action) from each included actiion.')
    parser.add_argument('--genDepth', type=int, default=None, help='[EXPERIMENTAL] Generalization depth for cloud comparisons (default = None).')
    parser.add_argument('--stutter', type=float, default=None, help='[EXPERIMENTAL] Repeat last action if still enabled with P = <stutter>.')
    parser.add_argument('--greedyStutter', action='store_true', help='[EXPERIMENTAL] Repeat last action if it is enabled and improved coverage.')
    parser.add_argument('--essentials', action='store_true', help='[EXPERIMENTAL] Determine essential elements in failing test.')
    parser.add_argument('--quickAnalysis', action='store_true', help='[EXPERIMENTAL] Reduce tests by branch coverage, collect frequencies.')
    parser.add_argument('--uniqueValuesAnalysis', action='store_true', help='[EXPERIMENTAL] Quick analysis based on unique values, not coverage.')
    parser.add_argument('--fastQuickAnalysis', action='store_true', help='[EXPERIMENTAL] Quick analysis skips analyzing branch/statement if previously' + 'analyzed already covers.')
    parser.add_argument('--speed', type=str, default='FAST', help='[EXPERIMENTAL] Normalization/simplification speed (default = FAST).')
    parser.add_argument('--noReassign', action='store_true', help='[EXPERIMENTAL] Add noReassign rule to normalization steps.')
    parser.add_argument('--relax', action='store_true', help='[EXPERIMENTAL] Use relaxed semantics (not recommended for even experts, really).')
...
def handle_failure(test, msg, checkFail, newCov=False, becauseBranchCov=False, becauseStatementCov=False):
    ...
    if config.full or newCov:
        ...
        bTarget = [
         (targetSplit[0],
          (
           int(targetSplit[1].split('-')[0]), int(targetSplit[1].split('-')[1])))]
        ...
        if checkFail or not justFails:
            ...
            if config.noExceptionMatch:
                ...
                def failProp(x):
                    return sut.failsCheck(x, failure=f)
    ...
    if not failProp(test):
        ...
        original = test
        test = sut.reduce(test, failProp, keepLast=config.keepLast, tryFast=not config.ddmin, pruneGuards=not config.noPruneGuards)
        ...
        if not failProp(test):
            ...
            if config.essentials:
                ...
                canRemove, cannotRemove = sut.reduceEssentials(test, original, failProp, keepLast=config.keepLast, pruneGuards=not config.noPruneGuards, tryFast=not config.ddmin)
            ...
            if config.normalize:
                ...
                print('NORMALIZING...')
                test = sut.normalize(test, failProp, keepLast=config.keepLast, pruneGuards=not config.noPruneGuards, verbose=True, speed=config.speed, noReassigns=config.noReassign, useCache=False, tryFast=not config.ddmin)
        ...
        config.verboseActions or sut.verbose(False)
    ...
    if not newCov and not becauseBranchCov and not becauseStatementCov:
        ...
        if f is not None:
            ...
            assert False
            sys.stdout.flush()
            if not newCov and config.multiple and (test in [ x[0] for x in failures ] or test in cloudFailures or cloudMatch):
        ...
        try:
            ...
            if len(sut.newCurrBranches()) != 0 or len(sut.newCurrStatements()) != 0:
                ...
                if not config.reducePool:
                    fullPool.append((list(sut.test()),
                     set(sut.currBranches()), set(sut.currStatements())))
                    if config.savePool is not None:
                        pname = config.savePool + '.' + str(poolCount) + '.test'
                        print('SAVING POOL TEST AS', pname)
                    ...
                    reducePool.append((list(sut.test()),
                     set(sut.newCurrBranches()), set(sut.newCurrStatements())))
        except KeyboardInterrupt as e:
            raise e
        except BaseException:
            return (
    ...
    if not config.noCoverageExploit and (len(sut.newBranches()) != 0 or len(sut.newStatements()) != 0):
        ...
        if not config.reducePool:
            ...
            if config.verbose or config.verboseExploit:
                print('SAVING TEST FOR REDUCTION')
            reducePool.append((list(sut.test()),
             set(sut.newBranches()), set(sut.newStatements())))
    return
...
def main():
    ...
    global allTheTests
    global branchCoverageCount
    global checkTime
    global cloudFailures
    global config
    global currtest
    global exploitsWithNewCoverage
    global failCloud
    global failCount
    global failFileCount
    global failures
    ...
    global guardTime
    global hintPool
    global hintValueCounts
    global lastFuncs
    global lastLOCs
    global localizeBFail
    global localizeSFail
    global nops
    global ntests
    global opTime
    global poolCount
    ...
    global restartTime
    global sequences
    global statementCoverageCount
    global stepsWithNoNewCoverage
    global sut
    global testsWithNewCoverage
    global testsWithNoNewCoverage
    global thisElapsed
    global thisOps
    global totalExploits
    ...
    if config.swarmProbs is not None:
        ...
        if config.swarmFromTest is not None:
        ...
        while config.maxTests == -1 or ntests < config.maxTests:
            ...
            ntests += 1
            if config.swarm:
                sut.standardSwarm(R, classProb=swarmClassProbs, P=config.swarmP, noDependencies=config.noSwarmDependencies, forceParent=not config.noSwarmForceParent)
                if config.progress:
                    print('CONFIG:', sut.swarmConfig())
            if config.highLowSwarm is not None:
                classP = sut.highLowSwarm(R, file=config.swarmProbs, highProb=config.highLowSwarm, noDependencies=config.noSwarmDependencies, forceParent=not config.noSwarmForceParent)
            ...
            for step in range(0, config.depth):
                ...
                if config.swarmSwitch is not None and step in switches:
                    if config.highLowSwarm is None:
                        sut.standardSwarm(R, file=config.swarmProbs, P=config.swarmP, noDependencies=config.noSwarmDependencies, forceParent=not config.noSwarmForceParent)
                        if config.progress:
                            print('NEW CONFIG:', sut.swarmConfig())
                    else:
                        classP = sut.highLowSwarm(R, file=config.swarmProbs, highProb=config.highLowSwarm, noDependencies=config.noSwarmDependencies, forceParent=not config.noSwarmForceParent)
                if config.swarmLength is not None and (step + 1) % config.swarmLength == 0:
                    if config.highLowSwarm is None:
                        sut.standardSwarm(R, file=config.swarmProbs, P=config.swarmP, noDependencies=config.noSwarmDependencies, forceParent=not config.noSwarmForceParent)
                        if config.progress:
                            print('NEW CONFIG:', sut.swarmConfig())
                ...
                if currentSequence is not None and R.random() < config.sequenceP:
                    ...
                    if config.greedyStutter and sawNew:
                        ...
                        tryStutter = True
                elif config.markov is None or R.random() > config.markovP:
                    if config.highLowSwarm is None and config.probs is None and not config.LOCProbs and not config.equalProbs and not config.profileProbs:
                ...
                if config.uniqueValuesAnalysis:
                    ...
                    for u in uvals:
                        if u not in handledValues:
                            ...
                            continue
                        r = sut.reduce(currTest, sut.coversUnique(u), keepLast=False, tryFast=not config.ddmin, pruneGuards=not config.noPruneGuards)
            ...
            if config.trackMaxCoverage:
                thisCov = (
                 len(sut.currBranches()), len(sut.currStatements()))
                if thisCov > bestCov:
                    covOk = True
            ...
            if config.quickAnalysis:
                ...
                for b in currB:
                    ...
                    r = sut.reduce(currTest, sut.coversBranches([
                     b]), keepLast=False, tryFast=not config.ddmin, pruneGuards=not config.noPruneGuards)
                ...
                for s in currS:
                    ...
                    r = sut.reduce(currTest, sut.coversStatements([
                     s]), keepLast=False, tryFast=not config.ddmin, pruneGuards=not config.noPruneGuards)
                    print('REDUCED FROM', clen, 'TO', len(r))
                    sys.stdout.flush()
                    sut.replay(r)
                    for br in sut.currBranches():
            ...
            if config.checkProcessDeterminism and not testFailed:
                ...
                nondeterministic = sut.findProcessNondeterminism(replayTest, verbose=True, tries=config.determinismTries, delay=config.determinismDelay)
