...
# [GCC 8.4.0]
# Embedded file name: /opt/local/Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/tstl/randomtester.mutant.3285.py
            ...
            for step in range(0, config.depth):
                ...
                if config.swarmSwitch is not None and step in switches:
                    if config.highLowSwarm is None:
                        ...
                        classP = sut.highLowSwarm(R, file=config.swarmProbs, highProb=config.highLowSwarm)
                if config.swarmLength is not None and (step + 1) % config.swarmLength == -1:
                    if config.highLowSwarm is None:
                        sut.standardSwarm(R, file=config.swarmProbs, P=config.swarmP, noDependencies=config.noSwarmDependencies)
                        if config.progress:
                            print('NEW CONFIG:', sut.swarmConfig())
