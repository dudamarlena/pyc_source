...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/smsshell/controllers/message.py
# Compiled at: 2008-04-20 03:14:18
"""Message Controller
...
class MessageController(ListController):
    table = model.Message
    parent = dict(folder=dict(table=model.Folder, column='name'))
    properties = (
     (
      'label', 'name', model.Label),)
    list_functions = ('delete', )
    entry_functions = ('details', 'delete')
    ...
    def reply(self, id):
        ...
        self._details(request.params['id'])
        old_entry = c.entry
        c.entry = c.table(recipient=old_entry.sender, sender=old_entry.recipient)
        c.id = None
        return render('/message/compose.mako')
