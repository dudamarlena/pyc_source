...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\director.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 28342 bytes
"""
...
import warnings, pyglet
from pyglet import window, event
from pyglet import gl
import pyglet.clock, cocos, cocos.audio, cocos.custom_clocks, cocos.fps
if hasattr(sys, 'is_cocos_sphinx') and sys.is_cocos_sphinx:
    __all__ = [
     'director', 'Director', 'DefaultHandler']
else:
    __all__ = [
     'director', 'DefaultHandler']
...
class Director(event.EventDispatcher):
    ...
    def init(self, *args, **kwargs):
        ...
        if 'do_not_scale' in kwargs:
            ...
            if 'autoscale' in kwargs:
                warnings.warn("Conflict between deprecated 'do_not_scale' and 'autoscale', 'autoscale' wins")
        ...
        self._window_virtual_aspect = self._window_virtual_width / float(self._window_virtual_height)
        self._offset_x = 0
        self._offset_y = 0
        if self.autoscale:
        ...
        if getenv('COCOS2D_NOSOUND', None) == '1' or audio_backend == 'pyglet':
            audio_settings = None
        if not cocos.audio._working:
            if audio_settings is not None:
                from cocos.audio.exceptions import NoAudioError
                msg = "cocos.audio isn't able to work without needed dependencies. Try installing pygame for fixing it, or forcing no audio mode by calling director.init with audio=None, or setting the COCOS2D_NOSOUND=1 variable in your env."
    ...
    def get_window_size(self):
        ...
        """
        return (
         self._window_virtual_width, self._window_virtual_height)
    ...
    def set_projection3D(self):
        ...
        gl.glLoadIdentity()
        gl.gluLookAt(vw / 2.0, vh / 2.0, vh / 1.1566, vw / 2.0, vh / 2.0, 0, 0.0, 1.0, 0.0)
