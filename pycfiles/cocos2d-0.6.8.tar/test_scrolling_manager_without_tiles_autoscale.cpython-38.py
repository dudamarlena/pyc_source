...
# [GCC 8.4.0]
# Embedded file name: D:\dev\cocos2020\test\test_scrolling_manager_without_tiles_autoscale.py
...
class SquareLand(cocos.layer.ScrollableLayer):
    ...
    def clamp(self, actor, new_pos):
        ...
        if x - actor.model_width // 2 < self.margin:
            ...
            if x + actor.model_width // 2 > self.world_width - self.margin:
                x = self.world_width - self.margin - actor.model_width // 2
            elif y - actor.model_height // 2 < self.margin:
                y = self.margin + actor.model_height // 2
            else:
                if y + actor.model_height // 2 > self.world_height - self.margin:
