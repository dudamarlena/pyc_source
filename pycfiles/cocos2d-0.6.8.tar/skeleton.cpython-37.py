...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\skeleton.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 15466 bytes
...
class ColorSkin(Skin):
    ...
    def draw(self):
        ...
        bones = self.skeleton.visit_children(lambda bone: (
         bone.label, bone.parent_matrix * bone.matrix))
        bones = dict(bones)
        gl.glPopMatrix()
...
class BitmapSkin(Skin):
    ...
    def regenerate(self):
        self.parts = [(name, position, scale, pyglet.resource.image(image, flip_y=flip_y, flip_x=flip_x)) for name, position, image, flip_x, flip_y, scale in self.skin_parts]
...
class Bone(object):
    ...
    def __init__(self, label, size, rotation, translation):
        ...
        self.label = label
        self.children = []
        self.matrix = (euclid.Matrix3.new_translate)(*translation) * euclid.Matrix3.new_rotate(math.radians(rotation))
    ...
    def move(self, dx, dy):
        self.translation.x += dx
        self.translation.y += dy
        self.matrix = (euclid.Matrix3.new_translate)(*self.translation) * euclid.Matrix3.new_rotate(self.rotation)
    ...
    def get_keyframe(self, offset=0):
        if offset == 0:
            if self.position in self.frames:
                return (
                 self.position, self.frames[self.position])
            return (None, None)
        else:
            if offset < 0:
                ...
                if abs(offset) <= len(prevs):
                    return (
                     prevs[offset], self.frames[prevs[offset]])
                return (None, None)
            else:
                if offset > 0:
                    ...
                    if abs(offset) <= len(next):
                        return (
                         next[(offset - 1)], self.frames[next[(offset - 1)]])
                    return (None, None)
