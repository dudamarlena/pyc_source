...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\euclid.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 74171 bytes
...
class Vector3(Slotted):
    ...
    def __eq__(self, other):
        ...
        if not (hasattr(other, '__len__') and len(other) == 3):
            raise AssertionError
    ...
    def __floordiv__(self, other):
        assert type(other) in scalar_types
        return Vector3(operator.floordiv(self.x, other), operator.floordiv(self.y, other), operator.floordiv(self.z, other))

    def __rfloordiv__(self, other):
        assert type(other) in scalar_types
        return Vector3(operator.floordiv(other, self.x), operator.floordiv(other, self.y), operator.floordiv(other, self.z))
...
class Matrix3(Slotted):
    ...
    def __setitem__(self, key, value):
        L = self[:]
        L[key] = value
        self.a, self.e, self.i, self.b, self.f, self.j, self.c, self.g, self.k = L
...
class Matrix4(Slotted):
    ...
    def __setitem__(self, key, value):
        L = self[:]
        L[key] = value
        self.a, self.e, self.i, self.m, self.b, self.f, self.j, self.n, self.c, self.g, self.k, self.o, self.d, self.h, self.l, self.p = L
    ...
    def transpose(self):
        self.a, self.e, self.i, self.m, self.b, self.f, self.j, self.n, self.c, self.g, self.k, self.o, self.d, self.h, self.l, self.p = (
         self.a, self.b, self.c, self.d,
         self.e, self.f, self.g, self.h,
         self.i, self.j, self.k, self.l,
         self.m, self.n, self.o, self.p)
    ...
    def determinant(self):
        return (self.a * self.f - self.e * self.b) * (self.k * self.p - self.o * self.l) - (self.a * self.j - self.i * self.b) * (self.g * self.p - self.o * self.h) + (self.a * self.n - self.m * self.b) * (self.g * self.l - self.k * self.h) + (self.e * self.j - self.i * self.f) * (self.c * self.p - self.o * self.d) - (self.e * self.n - self.m * self.f) * (self.c * self.l - self.k * self.d) + (self.i * self.n - self.m * self.j) * (self.c * self.h - self.g * self.d)
...
class Quaternion(Slotted):
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Vector3):
            ...
            yz2 = 2 * y * z
            zz = z * z
            return other.__class__(ww * Vx + wy2 * Vz - wz2 * Vy + xx * Vx + xy2 * Vy + xz2 * Vz - zz * Vx - yy * Vx, xy2 * Vx + yy * Vy + yz2 * Vz + wz2 * Vx - zz * Vy + ww * Vy - wx2 * Vz - xx * Vy, xz2 * Vx + yz2 * Vy + zz * Vz - wy2 * Vx - yy * Vz + wx2 * Vy - xx * Vz + ww * Vz)
        other = other.copy()
        other._apply_transform(self)
        return other
    ...
    def new_rotate_matrix(cls, m):
        if m[0] + m[5] + m[10] > 1e-08:
            t = m[0] + m[5] + m[10] + 1.0
            s = 0.5 / math.sqrt(t)
            return cls(s * t, (m[6] - m[9]) * s, (m[8] - m[2]) * s, (m[1] - m[4]) * s)
        if m[0] > m[5]:
            if m[0] > m[10]:
                t = m[0] - m[5] - m[10] + 1.0
                s = 0.5 / math.sqrt(t)
                return cls((m[6] - m[9]) * s, s * t, (m[1] + m[4]) * s, (m[8] + m[2]) * s)
        if m[5] > m[10]:
            t = -m[0] + m[5] - m[10] + 1.0
            s = 0.5 / math.sqrt(t)
            return cls((m[8] - m[2]) * s, (m[1] + m[4]) * s, s * t, (m[6] + m[9]) * s)
        t = -m[0] - m[5] + m[10] + 1.0
        s = 0.5 / math.sqrt(t)
        return cls((m[1] - m[4]) * s, (m[8] + m[2]) * s, (m[6] + m[9]) * s, s * t)

    new_rotate_matrix = classmethod(new_rotate_matrix)

    def new_interpolate(cls, q1, q2, t):
        if isinstance(q1, Quaternion):
            raise isinstance(q2, Quaternion) or AssertionError
        else:
            Q = cls()
        ...
        if B.r > A.r and d < B.r:
            ...
            if d >= A.r:
                if d >= B.r:
                    s1, s2 = (1, -1)
    v.normalize()
    return LineSegment2(Point2(A.c.x + s1 * v.x * A.r, A.c.y + s1 * v.y * A.r), Point2(B.c.x + s2 * v.x * B.r, B.c.y + s2 * v.y * B.r))
...
class Line2(Geometry, Slotted):
    ...
    def __init__(self, *args):
        if len(args) == 3:
            if not (isinstance(args[0], Point2) and isinstance(args[1], Vector2) and type(args[2]) == float):
                raise AssertionError
            self.p = args[0].copy()
            ...
            if len(args) == 2:
                if isinstance(args[0], Point2) and isinstance(args[1], Point2):
                    ...
                    if isinstance(args[0], Point2) and isinstance(args[1], Vector2):
                        self.p = args[0].copy()
                        self.v = args[1].copy()
                    else:
...
class Circle(Geometry, Slotted):
    ...
    def __init__(self, center, radius):
        if not (isinstance(center, Vector2) and type(radius) == float):
            raise AssertionError
        self.c = center.copy()
        self.r = radius
...
def _connect_line3_line3(A, B):
    if A.v:
        ...
        ub = B._u_in(ub) or max(min(ub, 1.0), 0.0)
    return LineSegment3(Point3(A.p.x + ua * A.v.x, A.p.y + ua * A.v.y, A.p.z + ua * A.v.z), Point3(B.p.x + ub * B.v.x, B.p.y + ub * B.v.y, B.p.z + ub * B.v.z))
        ...
        if B.r > A.r and d < B.r:
            ...
            if d >= A.r:
                if d >= B.r:
                    s1, s2 = (1, -1)
    v.normalize()
    return LineSegment3(Point3(A.c.x + s1 * v.x * A.r, A.c.y + s1 * v.y * A.r, A.c.z + s1 * v.z * A.r), Point3(B.c.x + s2 * v.x * B.r, B.c.y + s2 * v.y * B.r, B.c.z + s2 * v.z * B.r))
...
def _intersect_line3_sphere(L, S):
    ...
    if not L._u_in(u2):
        u2 = max(min(u2, 1.0), 0.0)
    return LineSegment3(Point3(L.p.x + u1 * L.v.x, L.p.y + u1 * L.v.y, L.p.z + u1 * L.v.z), Point3(L.p.x + u2 * L.v.x, L.p.y + u2 * L.v.y, L.p.z + u2 * L.v.z))
...
def _intersect_line3_plane(L, P):
    ...
    if not d:
        ...
        u = (P.k - P.n.dot(L.p)) / d
        return L._u_in(u) or None
    return Point3(L.p.x + u * L.v.x, L.p.y + u * L.v.y, L.p.z + u * L.v.z)
...
class Line3(Slotted):
    ...
    def __init__(self, *args):
        if len(args) == 3:
            if not (isinstance(args[0], Point3) and isinstance(args[1], Vector3) and type(args[2]) == float):
                raise AssertionError
            self.p = args[0].copy()
            self.v = args[1] * args[2] / abs(args[1])
        else:
...
class Sphere(Slotted):
    ...
    def __init__(self, center, radius):
        if not (isinstance(center, Vector3) and type(radius) == float):
            raise AssertionError
        self.c = center.copy()
        self.r = radius
...
class Plane(Slotted):
    ...
    def __init__(self, *args):
        if len(args) == 3:
            if not (isinstance(args[0], Point3) and isinstance(args[1], Point3) and isinstance(args[2], Point3)):
                raise AssertionError
            ...
            if len(args) == 2:
                if isinstance(args[0], Point3) and isinstance(args[1], Vector3):
                    ...
                    if isinstance(args[0], Vector3) and type(args[1]) == float:
    ...
    def _get_point(self):
        if self.n.z:
            return Point3(0.0, 0.0, self.k / self.n.z)
        if self.n.y:
            return Point3(0.0, self.k / self.n.y, 0.0)
        return Point3(self.k / self.n.x, 0.0, 0.0)
