...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\draw.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 13270 bytes
...
class Segment:
    ...
    @property
    def line_width(self):
        return Matrix3.new_rotate(math.radians(90)) * self.direction * (self.width / 2.0)
...
class Canvas(CocosNode):
    ...
    def endcap(self, line, cap_type):
        ...
        if cap_type == ROUND_CAP:
            s = Segment(line.start, line.start + -line.direction * line.width / 2, line.width)
            strip.extend([int(x) for x in flatten(s.bl, s.br, s.end, s.br, s.tr, s.end, s.bl, s.tl, s.end)])
            texcoord.extend([0.1, 0.9, 0.1, 0.5, 0.5, 0.9,
             0, 0, 0.5, 0, 1, 1,
             0, 0, 0.5, 0, 1, 1])
        else:
            if cap_type == SQUARE_CAP:
                segment = Segment(line.start, line.start + -line.direction * line.width / 2, line.width)
                    ...
                    if prev:
                        if inter:
                            if ctx.join == BEVEL_JOIN:
                                strip.extend([int(x) for x in list(inter) + list(bottom) + list(top)])
                                texcoord += [0.1, 0.9, 0.1, 0.5, 0.5, 0.9]
                            else:
                                if ctx.join in (MITER_JOIN, ROUND_JOIN):
                                    if bottom == top:
                                        far = Point2(*bottom)
                                    else:
                                        far = Ray2(Point2(*bottom), prev.direction).intersect(Ray2(Point2(*top), -current.direction))
                                    strip.extend([int(x) for x in list(inter) + list(bottom) + list(top) + list(bottom) + list(top) + list(far)])
                                    if ctx.join == ROUND_JOIN:
                                        texcoord += [0.1, 0.9, 0.1, 0.5, 0.5, 0.9, 0, 0, 1, 1, 0.5, 0]
                                    else:
                                        if ctx.join == MITER_JOIN:
                                            texcoord += [0.1, 0.9, 0.1, 0.5, 0.5, 0.9, 0.1, 0.9, 0.1, 0.5, 0.5, 0.9]
                    prev = current

                for s in segments:
                    strip.extend([int(x) for x in s.points])
                    texcoord += flatten(*[(0.1, 0.9, 0.1, 0.5, 0.5, 0.9) for x in range(len(s.points) // 6)])
