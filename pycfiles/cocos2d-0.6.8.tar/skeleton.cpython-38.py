...
# [GCC 8.4.0]
# Embedded file name: ../..\cocos\skeleton.py
...
try:
    ...
    class BitmapSkin(Skin):
        ...
        def get_control_points(self):
            return [(
             i, p[0]) for i, p in enumerate(self.skin_parts)]

        def regenerate(self):
            self.parts = [(
             name, position, scale,
             pyglet.resource.image(image, flip_y=flip_y, flip_x=flip_x)) for name, position, image, flip_x, flip_y, scale in self.skin_parts]
