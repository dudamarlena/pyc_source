...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\menu.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 27057 bytes
...
def verticalMenuLayout(menu):
    ...
    if menu.menu_halign == CENTER:
        ...
        if menu.menu_halign == RIGHT:
            pos_x = width - menu.menu_hmargin
        else:
            if menu.menu_halign == LEFT:
...
class Menu(Layer):
    ...
    def create_menu(self, items, selected_effect=None, unselected_effect=None, activated_effect=None, layout_strategy=verticalMenuLayout):
        """Creates the menu
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if new_idx < 0:
            ...
            if new_idx > len(self.children) - 1:
                new_idx = 0
            else:
...
class BaseMenuItem(CocosNode):
    __doc__ = 'An abstract menu item. It triggers a function when it is activated'
    ...
    def get_box(self):
        ...
        if self.item_halign == CENTER:
            ...
            if self.item_halign == RIGHT:
                x_diff = -width
            else:
                if self.item_halign == LEFT:
                    x_diff = 0
                else:
...
class MenuItem(BaseMenuItem):
    __doc__ = 'A menu item that shows a label. '
...
class ImageMenuItem(BaseMenuItem):
    __doc__ = ' A menu item that shows a selectable Image '
    ...
    def generateWidgets(self, pos_x, pos_y, font_item, font_item_selected):
        anchors = {'left':0, 
         'center':0.5,  'right':1,  'top':1,  'bottom':0}
        anchor = (anchors[font_item['anchor_x']] * self.image.width,
         anchors[font_item['anchor_y']] * self.image.height)
        self.item = Sprite((self.image), anchor=anchor, opacity=255, color=(font_item['color'][:3]))
        self.item.scale = font_item['font_size'] / float(self.item.height)
        self.item.position = (int(pos_x), int(pos_y))
        self.selected_item = Sprite((self.image), anchor=anchor, color=(font_item_selected['color'][:3]))
        self.selected_item.scale = font_item_selected['font_size'] / float(self.selected_item.height)
...
class MultipleMenuItem(MenuItem):
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol == key.LEFT:
            ...
            if symbol in (key.RIGHT, key.ENTER):
...
class EntryMenuItem(MenuItem):
    __doc__ = 'A menu item for entering a value.\n\n    When selected, ``self.value`` is toggled, the callback function is\n    called with ``self.value`` as argument.'
    value = property(lambda self: ''.join(self._value), lambda self, v: setattr(self, '_value', list(v)))
