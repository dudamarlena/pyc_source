...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\audio\pygame\base.py
...
def init():
    ...
    success = 0
    fail = 0
    SDL.SDL_Init(SDL.SDL_INIT_EVENTTHREAD | SDL.SDL_INIT_TIMER | SDL.SDL_INIT_NOPARACHUTE)
    if _video_autoinit():
        success += 1
    else:
        fail += 1
    for mod in list(sys.modules.values()):
        if hasattr(mod, '__PYGAMEinit__') and isinstance(mod.__PYGAMEinit__, collections.Callable):
            try:
                mod.__PYGAMEinit__()
                success += 1
            except:
...
def _rgba_from_obj(obj):
    ...
    if len(obj) == 3:
        return (
         int(obj[0]), int(obj[1]), int(obj[2]), 255)
    if len(obj) == 4:
        return obj
    return
