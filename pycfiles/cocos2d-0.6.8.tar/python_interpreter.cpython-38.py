...
# [GCC 8.4.0]
# Embedded file name: ../..\cocos\layer\python_interpreter.py
...
class PythonInterpreterLayer(ColorLayer):
    __doc__ = 'Runs an interactive Python interpreter as a child `Layer` of the current `Scene`.\n    '
        ...
        if motion == pyglet.window.key.MOTION_UP:
            ...
            if motion == pyglet.window.key.MOTION_DOWN:
                ...
                if motion == pyglet.window.key.MOTION_BACKSPACE:
                    return at_sol or self.caret.on_text_motion(motion)
                else:
                    if motion == pyglet.window.key.MOTION_LEFT:
                        return at_sol or self.caret.on_text_motion(motion)
                    else:
                        if motion == pyglet.window.key.MOTION_PREVIOUS_WORD:
                            return at_sol or self.caret.on_text_motion(motion)
                        else:
