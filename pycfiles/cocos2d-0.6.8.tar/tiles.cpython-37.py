...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\tiles.py
...
class Resource(object):
    __doc__ = 'Load some tile mapping resources from an XML file.\n    '
    cache = {}
    ...
    for tag in map.findall('tileset'):
        ...
        for c in tag.getchildren():
            if c.tag == 'image':
                ...
                tileset = TileSet.from_atlas(name, firstgid, path, tileset_tile_width, tileset_tile_height,
            ...
            if encoding is None:
                ...
                if encoding == 'csv':
                    ...
                    if encoding == 'base64':
                        ...
                        raise TmxUnsupportedVariant("Unsupported tiles layer format use 'csv', 'xml' or one of the 'base64'")
...
def imageatlas_factory(resource, tag):
    ...
    for child in tag:
        ...
        gl.glTexParameteri(tx.target, gl.GL_TEXTURE_WRAP_S, gl.GL_CLAMP_TO_EDGE)
        gl.glTexParameteri(tx.target, gl.GL_TEXTURE_WRAP_T, gl.GL_CLAMP_TO_EDGE)
        id = child.get('id')
...
class Tile(object):
    __doc__ = 'Tiles hold an image and some optional properties.\n    '
...
class TileSet(dict):
    __doc__ = 'Contains a set of Tile objects referenced by some id.\n    '
...
class RegularTesselationMap(object):
    __doc__ = 'A regularly tesselated map that allows access to its cells by index\n    (i, j).\n    '
...
class RectMapLayer(RectMap, MapLayer):
    __doc__ = 'A renderable, scrollable rect map.\n    '
...
class HexMap(RegularTesselationMap):
    ...
    def get_neighbor(self, cell, direction):
        ...
        if direction is self.UP:
            ...
            if direction is self.DOWN_LEFT:
                if cell.i % 2:
                    return self.get_cell(cell.i - 1, cell.j)
                return self.get_cell(cell.i - 1, cell.j - 1)
            else:
                if direction is self.DOWN_RIGHT:
                    if cell.i % 2:
                        return self.get_cell(cell.i + 1, cell.j)
                    return self.get_cell(cell.i + 1, cell.j - 1)
                else:
...
class RectMapCollider(object):
    __doc__ = 'moved to cocos.mapcolliders'
