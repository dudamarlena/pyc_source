...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\audio\SDL\audio.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 16772 bytes
...
_SDL_AudioDriverName = dll.private_function('SDL_AudioDriverName', arg_types=[
 c_char_p, c_int],
...
def _ctype_audio_format(fmt):
    if fmt == constants.AUDIO_U8:
        return c_ubyte
    if fmt == constants.AUDIO_S8:
        return c_char
    if fmt in (constants.AUDIO_U16LSB, constants.AUDIO_U16MSB):
        return c_ushort
    if fmt in (constants.AUDIO_S16LSB, constants.AUDIO_S16MSB):
        return c_short
    raise TypeError('Unsupported format %r' % fmt)


_SDL_OpenAudio = dll.private_function('SDL_OpenAudio', arg_types=[
 POINTER(SDL_AudioSpec), POINTER(SDL_AudioSpec)],
  return_type=c_int,
  error_return=(-1))
...
def SDL_OpenAudio(desired, obtained):
    ...
    ctype = [_ctype_audio_format(desired.format)]
...
SDL_GetAudioStatus = dll.function('SDL_GetAudioStatus', 'Get the current audio state.\n\n                                  :rtype: int\n                                  :return: one of SDL_AUDIO_STOPPED, SDL_AUDIO_PLAYING, SDL_AUDIO_PAUSED\n                                  ',
  args=[], arg_types=[], return_type=c_int)
SDL_PauseAudio = dll.function('SDL_PauseAudio', 'Pause and unpause the audio callback processing.\n\n                              It should be called with a parameter of 0 after opening the audio\n                              device to start playing sound.  This is so you can safely initalize\n                              data for your callback function after opening the audio device.\n                              Silence will be written to the audio device during the pause.\n\n                              :Parameters:\n                               - `pause_on`: int\n\n                              ',
  args=[
 'pause_on'],
  arg_types=[
 c_int],
  return_type=None)
_SDL_LoadWAV_RW = dll.private_function('SDL_LoadWAV_RW', arg_types=[
 POINTER(rwops.SDL_RWops),
 c_int,
...
_SDL_FreeWAV = dll.private_function('SDL_FreeWAV', arg_types=[
 POINTER(c_ubyte)],
  return_type=None)
...
_SDL_BuildAudioCVT = dll.private_function('SDL_BuildAudioCVT', arg_types=[
 POINTER(SDL_AudioCVT), c_ushort, c_ubyte, c_uint,
 c_ushort, c_ubyte, c_uint],
  return_type=c_int,
  error_return=(-1))

def SDL_BuildAudioCVT(src_format, src_channels, src_rate, dst_format, dst_channels, dst_rate):
...
SDL_LockAudio = dll.function('SDL_LockAudio', 'Guarantee the callback function is not running.\n\n                             The lock manipulated by these functions protects the callback function.\n                             During a LockAudio/UnlockAudio pair, you can be guaranteed that the\n                             callback function is not running.  Do not call these from the callback\n                             function or you will cause deadlock.\n                             ',
  args=[], arg_types=[], return_type=None)
SDL_UnlockAudio = dll.function('SDL_UnlockAudio', 'Release the audio callback lock.\n\n                               :see: `SDL_LockAudio`\n                               ',
  args=[], arg_types=[], return_type=None)
SDL_CloseAudio = dll.function('SDL_CloseAudio', 'Shut down audio processing and close the audio device.\n                              ',
  args=[], arg_types=[], return_type=None)