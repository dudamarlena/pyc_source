...
# [GCC 8.4.0]
# Embedded file name: d:\dev\cocos2020\cocos\audio\SDL\dll.py
# Compiled at: 2020-01-10 23:58:31
# Size of source mod 2**32: 10622 bytes
"""
"""
import six
...
class SDL_DLL:
    ...
    def __init__(self, library_name, version_function_name, version=None):
        ...
        if sys.platform == 'win32':
            try:
                ...
                raise ImportError('Dynamic library "%s" was not found' % library_name)
    ...
    def assert_version_compatible(self, name, since):
        ...
        if not version_compatible(since):
            ...
            raise cocos.audio.SDL.error.SDL_NotImplementedError('%s requires SDL version %s; currently using version %s' % (
    ...
    def function(self, name, doc, args=[], arg_types=[], return_type=None, dereference_return=False, require_return=False, success_return=None, error_return=None, since=None):
        ...
        """
        if since:
            if not self.version_compatible(since):

                def _f(*args, **kwargs):
                    import cocos.audio.SDL.error
                    raise cocos.audio.SDL.error.SDL_NotImplementedError('%s requires %s %s; currently using version %s' % (
