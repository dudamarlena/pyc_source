...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ticketpy\client.py
...
class ApiClient:
    ...
    def search(self, method, **kwargs):
        ...
        for k, v in kwargs.items():
            if k in ('includeTBA', 'includeTBD', 'includeTest'):
                updates[k] = self._ApiClient__yes_no_only(v)
            else:
                if k in ('size', 'radius', 'marketId'):
                    updates[k] = str(v)

        kwargs.update(updates)
        log.debug(kwargs)
        urls = {'events':self._ApiClient__method_url('events'), 
         'venues':self._ApiClient__method_url('venues'), 
         'attractions':self._ApiClient__method_url('attractions'), 
         'classifications':self._ApiClient__method_url('classifications')}
    ...
    def _handle_response(self, response):
        ...
        if response.status_code == 200:
            return self._ApiClient__success(response)
        else:
            if response.status_code == 401:
                self._ApiClient__fault(response)
            else:
                if response.status_code == 400:
                    self._ApiClient__error(response)
                else:
                    self._ApiClient__unknown_error(response)
    ...
    def __error(response):
        ...
        errors = [error(err['code'], err['detail'], err['_links']['about']['href']) for err in rj['errors']]
    ...
    @property
    def api_key(self):
        return self._ApiClient__api_key

    @api_key.setter
    def api_key(self, api_key):
        self._ApiClient__api_key = {'apikey': api_key}
...
class ApiException(Exception):
    __doc__ = 'Exception thrown for API-related error messages'
...
class PagedResponse:
    __doc__ = 'Iterates through API response pages'
