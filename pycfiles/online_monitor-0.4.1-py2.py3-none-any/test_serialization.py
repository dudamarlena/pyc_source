...
# [GCC 8.4.0]
# Embedded file name: /home/davidlp/git/online_monitor/online_monitor/testing/test_serialization.py
# Compiled at: 2019-06-26 09:47:55
""" Check the serialization utils for complex data structures
"""
import unittest, time, numpy as np, zmq
...
def send_normal(socket, data, scan_par_id, name='ReadoutData'):
    data_meta_data = dict(name=name, dtype=str(data[0].dtype), shape=data[0].shape, timestamp_start=data[1], timestamp_stop=data[2], error=data[3], scan_par_id=scan_par_id)
...
def send_std(socket, data, scan_par_id, name='ReadoutData'):
    data_with_meta_data = dict(data=data[0], name=name, timestamp_start=data[1], timestamp_stop=data[2], error=data[3], scan_par_id=scan_par_id)
