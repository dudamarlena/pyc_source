...
# Embedded file name: /home/alanr/monitor/ctypesgen-davidjamesca/ctypesgen/ctypesgen/printer_python/printer.py
...
class WrapperPrinter:
    ...
    def __init__(self, outpath, options, data):
        ...
        method_table = {'function': self.print_function, 
    ...
    def template_subs(self):
        template_subs = {'date': time.ctime(), 'argv': (' ').join([ x for x in sys.argv if not x.startswith('--strip-build-path') ]), 
    ...
    def print_header(self):
        ...
        if self.options.header_template:
            ...
            try:
                ...
                error_message('Cannot load header template from file "%s"  - using default template.' % path, cls='missing-file')
    ...
    def print_loader(self):
        ...
        loader_file.close()
        self.file.write('\n# End loader\n\n')
        self.file.write('add_library_search_dirs([%s])' % (', ').join([ repr(d) for d in self.options.runtime_libdirs ]))
        self.file.write('\n')
    ...
    def print_fixed_function(self, function):
        ...
        if function.source_library:
            self.file.write(("if hasattr(_libs['{L}'], '{CN}'):\n    {PN} = _libs['{L}'].{CN}\n").format(L=function.source_library, CN=function.c_name(), PN=function.py_name()))
        else:
            self.file.write(("for _lib in _libs.values():\n    if not hasattr(_lib, '{CN}'):\n        continue\n    {PN} = _lib.{CN}\n").format(CN=function.c_name(), PN=function.py_name()))
    ...
    def print_variadic_function(self, function):
        self.srcinfo(function.src)
        if function.source_library:
            self.file.write(("if hasattr(_libs['{L}'], '{CN}'):\n    _func = _libs['{L}'].{CN}\n    _restype = {RT}\n    _errcheck = {E}\n    _argtypes = [{t0}]\n    {PN} = _variadic_function(_func,_restype,_argtypes,_errcheck)\n").format(L=function.source_library, CN=function.c_name(), RT=function.restype.py_string(), E=function.errcheck.py_string(), t0=(', ').join([ a.py_string() for a in function.argtypes ]), PN=function.py_name()))
        else:
            self.file.write(("for _lib in _libs.values():\n    if hasattr(_lib, '{CN}'):\n        _func = _lib.{CN}\n        _restype = {RT}\n        _errcheck = {E}\n        _argtypes = [{t0}]\n        {PN} = _variadic_function(_func,_restype,_argtypes,_errcheck)\n").format(CN=function.c_name(), RT=function.restype.py_string(), E=function.errcheck.py_string(), t0=(', ').join([ a.py_string() for a in function.argtypes ]), PN=function.py_name()))

    def print_variable(self, variable):
        self.srcinfo(variable.src)
        if variable.source_library:
            self.file.write(('try:\n    {PN} = ({PS}).in_dll(_libs["{L}"], "{CN}")\nexcept:\n    pass\n').format(PN=variable.py_name(), PS=variable.ctype.py_string(), L=variable.source_library, CN=variable.c_name()))
        else:
            self.file.write(('for _lib in _libs.values():\n    try:\n        {PN} = ({PS}).in_dll(_lib, "{CN}")\n        break\n    except:\n        pass\n').format(PN=variable.py_name(), PS=variable.ctype.py_string(), CN=variable.c_name()))
    ...
    def print_func_macro(self, macro):
        self.srcinfo(macro.src)
        self.file.write(('def {MN}({MP}):\n    return {ME}\n').format(MN=macro.name, MP=(', ').join(macro.params), ME=macro.expr.py_string(True)))
