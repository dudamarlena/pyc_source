...
# Embedded file name: /home/alanr/monitor/ctypesgen-davidjamesca/ctypesgen/ctypesgen/test/../../ctypesgen/main.py
# Compiled at: 2019-10-20 08:23:36
...
def main(givenargs=None):
    ...
    op.add_option('',
      ...
      action='store_true',
      default=False,
      help='Run the lexer in optimized mode.  This mode requires write access to lextab.py file stored within the ctypesgen package.')
    op.add_option('-a',
      '--all-headers',
      action='store_true',
      dest='all_headers',
    ...
    op.add_option('',
      ...
      help="Choose output language (`py'[default], `py32', `py27', `py25', or `json').  The implementation for py32 does appear to be compatible down to at least Python2.7.15.  py25 and py27 are in any case _not_ compatible with >= Python3.  The default choice (py) attempts to select `py32', `py27', or `py25' based on the version of Python that runs this script.")
    op.add_option('',
      '--all-errors',
      action='store_true',
      default=False,
    ...
    if descriptions.all == []:
        if not options.all_headers:
            msgs.warning_message("There wasn't anything of use in the specified header file(s). Perhaps you meant to run with --all-headers to include objects from included sub-headers? ",
              cls='usage')