...
# Size of source mod 2**32: 43162 bytes
...
tokens = ('PP_DEFINE', 'PP_DEFINE_NAME', 'PP_DEFINE_MACRO_NAME', 'PP_UNDEFINE', 'PP_MACRO_PARAM',
          'PP_STRINGIFY', 'PP_IDENTIFIER_PASTE', 'PP_END_DEFINE', 'PRAGMA', 'PRAGMA_PACK',
          'PRAGMA_END', 'IDENTIFIER', 'CONSTANT', 'CHARACTER_CONSTANT', 'STRING_LITERAL',
          'SIZEOF', 'PTR_OP', 'INC_OP', 'DEC_OP', 'LEFT_OP', 'RIGHT_OP', 'LE_OP',
          'GE_OP', 'EQ_OP', 'NE_OP', 'AND_OP', 'OR_OP', 'MUL_ASSIGN', 'DIV_ASSIGN',
          'MOD_ASSIGN', 'ADD_ASSIGN', 'SUB_ASSIGN', 'LEFT_ASSIGN', 'RIGHT_ASSIGN',
          'AND_ASSIGN', 'XOR_ASSIGN', 'OR_ASSIGN', 'PERIOD', 'TYPE_NAME', 'TYPEDEF',
          'EXTERN', 'STATIC', 'AUTO', 'REGISTER', '_BOOL', 'CHAR', 'SHORT', 'INT',
...
def p_multi_string_literal(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('string concatenation', lambda x, y: x + y, '(%s + %s)', (False,
                                                                                                          False), p[1], p[2])
...
def p_postfix_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('array access', lambda a, b: a[b], '(%s [%s])', (True,
                                                                                                 False), p[1], p[3])
    elif p[2] == '(':
        if p[3] == ')':
...
def p_and_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise and', lambda x, y: x & y, '(%s & %s)', (False,
                                                                                                 False), p[1], p[3])
...
def p_exclusive_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise xor', lambda x, y: x ^ y, '(%s ^ %s)', (False,
                                                                                                 False), p[1], p[3])
...
def p_inclusive_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise or', lambda x, y: x | y, '(%s | %s)', (False,
                                                                                                False), p[1], p[3])
...
def p_logical_and_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('logical and', lambda x, y: x and y, '(%s and %s)', (True,
                                                                                                     True), p[1], p[3])
...
def p_logical_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('logical and', lambda x, y: x or , '(%s or %s)', (True,
                                                                                                  True), p[1], p[3])
...
def p_direct_declarator(p):
    ...
    if isinstance(p[1], cdeclarations.Declarator):
        ...
        p[0].identifier = p[1]
    if p[0].parameters:
        if len(p[0].parameters) == 1:
            param = p[0].parameters[0]
            if param.type.specifiers == ['void']:
                if not param.declarator:
                    p[0].parameters = ()
...
def p_direct_abstract_declarator(p):
    ...
    """
    if p[1] == '(':
        if isinstance(p[3], cdeclarations.Declarator):
            p[0] = p[3]
            p[3].attrib.update(p[2])
...
def p_pragma_pack(p):
    ...
    if len(p) == 6:
        ...
        if op == 'push':
            ...
            err = 'Syntax error for #pragma pack at {}:{}'.format(p.slice[1].filename, p.slice[1].lineno)
    else:
