...
if __name__ == '__main__':
    ...
    main()
    sys.exit()
import operator, os.path, re, sys, time, warnings
from . import yacc
from .. import expressions
from . import cdeclarations
tokens = ('PP_DEFINE', 'PP_DEFINE_NAME', 'PP_DEFINE_MACRO_NAME', 'PP_MACRO_PARAM',
          'PP_STRINGIFY', 'PP_IDENTIFIER_PASTE', 'PP_END_DEFINE', 'IDENTIFIER', 'CONSTANT',
          'CHARACTER_CONSTANT', 'STRING_LITERAL', 'SIZEOF', 'PTR_OP', 'INC_OP', 'DEC_OP',
          'LEFT_OP', 'RIGHT_OP', 'LE_OP', 'GE_OP', 'EQ_OP', 'NE_OP', 'AND_OP', 'OR_OP',
          'MUL_ASSIGN', 'DIV_ASSIGN', 'MOD_ASSIGN', 'ADD_ASSIGN', 'SUB_ASSIGN', 'LEFT_ASSIGN',
          'RIGHT_ASSIGN', 'AND_ASSIGN', 'XOR_ASSIGN', 'OR_ASSIGN', 'PERIOD', 'TYPE_NAME',
          'TYPEDEF', 'EXTERN', 'STATIC', 'AUTO', 'REGISTER', '_BOOL', 'CHAR', 'SHORT',
          'INT', 'LONG', 'SIGNED', 'UNSIGNED', 'FLOAT', 'DOUBLE', 'CONST', 'VOLATILE',
          'VOID', 'STRUCT', 'UNION', 'ENUM', 'ELLIPSIS', 'CASE', 'DEFAULT', 'IF',
          'ELSE', 'SWITCH', 'WHILE', 'DO', 'FOR', 'GOTO', 'CONTINUE', 'BREAK', 'RETURN',
          '__ASM__', '__ATTRIBUTE__', 'PACKED', 'ALIGNED', 'TRANSPARENT_UNION')
keywords = [
 'auto',
 '_Bool',
 'break',
...
def p_identifier(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.UnsupportedExpressionNode('Identifier pasting is not supported by ctypesgen.')
...
def p_multi_string_literal(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('string concatenation', lambda x, y: x + y, '(%s + %s)', (False, False), p[1], p[2])
...
def p_postfix_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('array access', lambda a, b: a[b], '(%s [%s])', (True, False), p[1], p[3])
    elif p[2] == '(':
        if p[3] == ')':
        ...
        p[0] = expressions.AttributeExpressionNode(lambda x, a: getattr(x, a), '(%s.%s)', p[1], p[3])
    elif p[2] == '->':
        p[0] = expressions.AttributeExpressionNode(lambda x, a: getattr(x.contents, a), '(%s.contents.%s)', p[1], p[3])
    elif p[2] == '++':
        p[0] = expressions.UnaryExpressionNode('increment', lambda x: x + 1, '(%s + 1)', False, p[1])
    elif p[2] == '--':
        p[0] = expressions.UnaryExpressionNode('decrement', lambda x: x - 1, '(%s - 1)', False, p[1])
...
def p_and_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise and', lambda x, y: x & y, '(%s & %s)', (False, False), p[1], p[3])
...
def p_exclusive_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise xor', lambda x, y: x ^ y, '(%s ^ %s)', (False, False), p[1], p[3])
...
def p_inclusive_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('bitwise or', lambda x, y: x | y, '(%s | %s)', (False, False), p[1], p[3])
...
def p_logical_and_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('logical and', lambda x, y: x and y, '(%s and %s)', (True, True), p[1], p[3])
...
def p_logical_or_expression(p):
    ...
    if len(p) == 2:
        ...
        p[0] = expressions.BinaryExpressionNode('logical and', lambda x, y: x or y, '(%s or %s)', (True, True), p[1], p[3])
...
class Attribs(dict):
    ...
    def __init__(self, packed=False, aligned=False, transparent_union=False):
        super(Attribs, self).__init__(packed=packed, aligned=aligned, transparent_union=transparent_union)
        self.__dict__ = self
...
def p_direct_abstract_declarator(p):
    ...
    if p[1] == '(' and isinstance(p[2], cdeclarations.Declarator):
        p[0] = p[2]
    elif isinstance(p[1], cdeclarations.Declarator):
        p[0] = p[1]
        if p[2] == '[':
            a = cdeclarations.Array()
            a.array = p[0].array
