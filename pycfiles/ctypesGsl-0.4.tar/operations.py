...
# Embedded file name: /home/alanr/monitor/ctypesgen-davidjamesca/ctypesgen/ctypesgen/processor/operations.py
...
def fix_conflicting_names(data, opts):
    ...
    the name conflict."""
    descriptions = data.functions + data.variables + data.structs + data.typedefs + data.enums + data.constants + data.macros
    important_names = {}
    preamble_names = set()
    ...
    for description in descriptions:
        if description.py_name() in important_names:
            ...
            if not description.dependents:
                description.warning('%s has been renamed to %s due to a name conflict with %s.' % (
                 original_name, description.casual_name(), conflict_name), cls='rename')
            else:
                description.warning('%s has been renamed to %s due to a name conflict with %s. Other objects depend on %s - those objects will be skipped.' % (
                 original_name, description.casual_name(), conflict_name, original_name), cls='rename')
        ...
        if not struct.opaque:
            for i, (name, type) in enumerate(struct.members):
                if name in keyword.kwlist:
                    struct.members[i] = (
                     '_' + name, type)
                    struct.warning('Member "%s" of %s has been renamed to "%s" because it has the same name as a Python keyword.' % (
                     name, struct.casual_name(), '_' + name), cls='rename')

    for macro in data.macros:
        if macro.params:
            for param in macro.params:
                if param in keyword.kwlist:
                    macro.error('One of the parameters to %s, "%s" has the same name as a Python keyword. %s will be skipped.' % (
                     macro.casual_name(), param, macro.casual_name()), cls='name-conflict')
...
def find_source_libraries(data, opts):
    ...
    for library_name in opts.libraries:
        try:
            ...
            warning_message('Could not load library "%s". Okay, I\'ll try to load it at runtime instead. ' % library_name, cls='missing-library')
            continue
