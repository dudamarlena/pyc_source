...
def make_enum_from_specifier(specifier):
    ...
    for e in specifier.enumerators:
        if e.expression:
            ...
            value = BinaryExpressionNode('addition', lambda x, y: x + y, '(%s + %s)', (
             False, False), IdentifierExpressionNode(last_name), ConstantExpressionNode(1))
        else:
...
class CtypesParser(CParser):
    ...
    def make_struct_from_specifier(self, specifier):
        ...
        if specifier.declarations:
            ...
            for declaration in specifier.declarations:
                t = self.get_ctypes_type(declaration.type, declaration.declarator, check_qualifiers=True)
            ...
            members = None
        return CtypesStruct(tag, specifier.is_packed, variety, members, src=(specifier.filename, specifier.lineno))
    ...
    def get_ctypes_type(self, typ, declarator, check_qualifiers=False):
        ...
        if isinstance(t, CtypesPointer) and isinstance(t.destination, CtypesSimple) and t.destination.name == 'char' and t.destination.signed:
    ...
    def handle_declaration(self, declaration, filename, lineno):
        ...
        if declarator is None:
            ...
            if declaration.storage == 'typedef':
                ...
                self.handle_ctypes_function(name, t.restype, t.argtypes, t.errcheck, t.variadic, filename, lineno)
