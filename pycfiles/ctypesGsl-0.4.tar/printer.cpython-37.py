...
class WrapperPrinter:
    ...
    def __init__(self, outpath, options, data):
        status_message('Writing to %s.' % (outpath or ))
        self.file = open(outpath, 'w') if outpath else sys.stdout
        self.options = options
        if self.options.strip_build_path:
            if self.options.strip_build_path[(-1)] != os.path.sep:
    ...
    def srcinfo(self, src):
        if src == None:
            ...
            if filename in ('<built-in>', '<command line>'):
                ...
                if self.options.strip_build_path:
                    if filename.startswith(self.options.strip_build_path):
    ...
    def print_header(self):
        ...
        if self.options.header_template:
            ...
            try:
                ...
                error_message(('Cannot load header template from file "%s"  - using default template.' % path),
                  cls='missing-file')
