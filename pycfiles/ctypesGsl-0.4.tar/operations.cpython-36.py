            ...
            for i, (name, type) in enumerate(struct.members):
                if name in keyword.kwlist:
                    ...
                    struct.warning(('Member "%s" of %s has been renamed to "%s" because it has the same name as a Python keyword.' % (
                     name, struct.casual_name(), '_' + name)),
                      cls='rename')
...
def find_source_libraries(data, opts):
    ...
    for library_name in opts.libraries:
        try:
            ...
            warning_message(('Could not load library "%s". Okay, I\'ll try to load it at runtime instead. ' % library_name),
              cls='missing-library')
            continue

        for symbol in all_symbols:
            if symbol.source_library == None and hasattr(library, symbol.c_name()):
