...
class DataCollectingParser(ctypesparser.CtypesParser, ctypesparser.CtypesTypeVisitor):
    ...
    def handle_ctypes_function(self, name, restype, argtypes, errcheck, variadic, filename, lineno):
        restype.visit(self)
        for argtype in argtypes:
            argtype.visit(self)

        function = FunctionDescription(name, restype, argtypes, errcheck, variadic=variadic, src=(filename, repr(lineno)))
        ...
        if name in self.already_seen_structs:
            ...
            if ctypestruct.opaque:
                if name not in self.already_seen_opaque_structs:
                    struct = StructDescription(ctypestruct.tag, ctypestruct.packed, ctypestruct.variety, None, True, ctypestruct, src=(
                     filename, str(lineno)))
                ...
                if name in self.already_seen_opaque_structs:
                    ...
                    struct = StructDescription(ctypestruct.tag, ctypestruct.packed, ctypestruct.variety, ctypestruct.members, False, src=(
                     filename, str(lineno)), ctype=ctypestruct)
                    self.structs.append(struct)
                    self.all.append(struct)
                    self.output_order.append(('struct', struct))
                    self.output_order.append(('struct-body', struct))
        ...
        if tag in self.already_seen_enums:
            ...
            if ctypeenum.opaque:
                ...
                if tag in self.already_seen_opaque_enums:
                    ...
                    enum = EnumDescription(ctypeenum.tag, ctypeenum.enumerators, src=(
                     filename, str(lineno)), ctype=ctypeenum)
                    enum.opaque = False
                    self.enums.append(enum)
                    self.all.append(enum)
                    self.output_order.append(('enum', enum))
        ...
        if expr == None:
            ...
            if isinstance(expr, CtypesType):
                if params:
                    ...
                    macro.error('%s has parameters but evaluates to a type. Ctypesgen does not support it.' % macro.casual_name(), cls='macro')
                    self.macros.append(macro)
                    self.all.append(macro)
                    self.output_order.append(('macro', macro))
                else:
