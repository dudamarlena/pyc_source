...
class EvaluationContext(object):
    __doc__ = 'Interface for evaluating expression nodes.\n    '
...
class TypeCastExpressionNode(ExpressionNode):
    ...
    def py_string(self, can_be_ctype):
        if isinstance(self.ctype, CtypesPointer):
            ...
            if isinstance(self.ctype, CtypesStruct):
                raise TypeError('conversion to non-scalar type ({}) requested from {}'.format(self.ctype, self.base.py_string(False)))
            elif isinstance(self.ctype, CtypesSimple):
                if (self.ctype.name, self.ctype.signed) == ('char', True):
                    ord_if_char = ''
            else:
                ord_if_char = 'ord_if_char'
            return '({to} ({ord_if_char}({frm}))).value'.format(to=(self.ctype.py_string()),
              ord_if_char=ord_if_char,
