...
# [GCC 8.4.0]
# Embedded file name: /home/alanr/monitor/ctypesgen-davidjamesca/ctypesgen/ctypesgen/parser/cparser.py
                ...
                if t.type == 'PP_END_DEFINE':
                    ...
                    if t.type == 'LPAREN':
                        ...
                        if t.type == 'PP_NUMBER':
                            ...
                            if t.type == 'IDENTIFIER' and t.value in cgrammar.keywords:
                                t.type = t.value.upper()
                            else:
                                if t.type == 'IDENTIFIER':
                                    if t.value in self.type_names:
                                        if self.pos < 2 or self.tokens[(self.pos - 2)].type not in ('VOID',
                                                                                                    '_BOOL',
                                                                                                    'CHAR',
                                                                                                    'SHORT',
                                                                                                    'INT',
...
class DebugCParser(CParser):
    __doc__ = 'A convenience class that prints each invocation of a handle_* method to\n    stdout.\n    '
