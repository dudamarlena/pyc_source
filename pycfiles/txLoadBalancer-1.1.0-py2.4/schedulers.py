...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/txlb/schedulers.py
...
class LeastConnsScheduler(BaseScheduler):
    ...
    def nextHost(self, clientAddr):
        if not self.hasHost():
            return
        hosts = [ (x[1], self.tracker.lastclose.get(x[0], 0), x[0]) for x in self.tracker.available.items() ]
        hosts.sort()
        return hosts[0][2]
...
class RandomWeightedScheduler(BaseScheduler):
    ...
    def nextHost(self, clientAddr):
        ...
        populationSize = sum([ weight for (hostPort, weight) in group.getWeights().items() if hostPort in hosts ])
