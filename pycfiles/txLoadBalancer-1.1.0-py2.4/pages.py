...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/txlb/admin/pages.py
...
class UnauthorizedResource(resource.Resource):
    ...
    def render(self, request):
        ...
        request.setHeader('WWW-authenticate', 'basic realm="PyDirector"')
...
class BasePage(resource.Resource):
    ...
    def getHeader(self, refreshURL='', msg=''):
        ...
        refresh = ''
        if refreshURL:
            refresh = template.refresh % (self.parent.conf.admin.refresh, refreshURL)
        if msg:
            msg = template.message % msg
        return template.header % (txlb.name, refresh, txlb.name, self.parent.serverVersion, socket.gethostname()) + msg
...
class RunningPage(BasePage):
    ...
    def getPage(self, request):
        ...
        content += template.refreshButtons % (time.ctime(time.time()), time.time(), stopStart)
        for service in self.parent.conf.getServices():
            ...
            for group in groups:
                ...
                if group is eg:
                    ...
                    klass = 'inactive'
                    desc = template.groupDescDisabled % (service.name, group.name)
                content += template.groupName % (klass, group.name)
                content += desc
                content += template.groupHeaderForm % (service.name, group.name, klass)
                ...
                for h in k:
                    ...
                    if totals.has_key(h):
                        ...
                        tc = '--'
                    content += template.hostInfo % (klass, hdict[h], h, oc, tc, f, urllib.quote(service.name), urllib.quote(group.name), urllib.quote(h))
                ...
                for k in bad.keys():
                    host = '%s:%s' % k
                    (when, what) = bad[k]
                    content += template.badHostInfo % (klass, hdict[host], host, what.getErrorMessage())
...
class DeleteHost(BasePage):
    ...
    def getPage(self, request):
        ...
        tracker = self.parent.director.getTracker(serviceName=service, groupName=group)
...
class AddHost(BasePage):
    ...
    def getPage(self, request):
        ...
        msg = 'Host %s(%s) added to %s / %s' % (name, ip, groupName, serviceName)
        request.redirect('/all?resultMessage=%s' % urllib.quote(msg))
        return 'OK'
