...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/txlb/model.py
...
class HostMapper(object):
    ...
    def __init__(self, proxy='', addresses=[], group='', lbType='', host='', address='', enabled=None, weight=1):
...
def convertMapperToModel(listOfMappers):
    ...
    for mapper in listOfMappers:
        ...
        group = ProxyGroup(mapper.groupName, mapper.lbType, mapper.groupEnabled)
        group = groupHolder.setdefault(mapper.groupName, group)
        host = ProxyHost(mapper.hostName, mapper.hostAddress[0], mapper.hostAddress[1], mapper.hostWeight)
        group.addHost(host)
