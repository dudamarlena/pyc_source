...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/txlb/manager.py
...
def checkConfigChanges(configFile, configuration, director):
    ...
    if current != disk:
        ...
        backupFile = '%s-%s' % (configFile, datetime.now().strftime('%Y%m%d%H%M%S'))
...
class GetClientAddress(amp.Command):
    ...
    arguments = [
     (
      'host', amp.String()), ('port', amp.Integer())]
    response = [
     (
      'host', amp.String()), ('port', amp.Integer())]
