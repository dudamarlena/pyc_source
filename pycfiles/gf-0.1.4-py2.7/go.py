...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/gf/go.py
# Compiled at: 2013-12-18 06:20:09
...
def _get_operations():
    ...
    for name, value in operator.__dict__.iteritems():
        if name.startswith('__') and name.endswith('__') and isinstance(value, (FunctionType, BuiltinFunctionType)):
...
operations = {}
__all__ = []
_reverse_operation_names = ('add', 'sub', 'mul', 'div', 'truediv', 'floordiv', 'mod',
...
def _install_operations():
    ...
    for name, value in _get_operations():
        doc = value.__doc__
        doc += '\n\n'
        doc += 'Called by the :meth:`AbstractObject.%s` special method.' % name
        basic_name = name[2:-2]
        if basic_name in _reverse_operation_names:
            doc += '\n'
            doc += 'Also called by :meth:`AbstractObject.__r%s__` with arguments reversed.' % basic_name
        operations[name] = globals()[name] = generic(name, doc)
        __all__.append(name)


_install_operations()
__init__ = generic('__init__', ":func:`__init__` initializes instantiates instances of :class:`AbstractObject` and it's subclasses.\n\nIt has a multi method for :class:`Object`. This multi-method does not accept any\nadditional parameters and has no effect.\nThere is no method for :class:`AbstractObject`, therefore\nthis class can not be instantiated.")
__call__ = generic('__call__', ':func:`__call__` is called when instances of :class:`AbstractObject` are called.')
__del__ = generic('__del__', ':func:`__del__` is called when instances of :class:`FinalizingMixin` are about to be destroyed.')
...
def _install_operations():
    ...
    def generate_operation(value):
        ...
        doc = get_doc(value)
        doc += '\n\n'
        doc += 'Calls the :func:`%s`-generic with its arguments.' % value.__name__
    ...
    def generate_reverse_operation(value):
        ...
        doc = get_doc(value)
        doc += '\n\n'
        doc += 'Calls the :func:`%s`-generic with its arguments reversed.' % value.__name__
        operation.__doc__ = doc
        return operation
...
__out__ = generic('__out__', 'Create a print string of an object using a :class:`Writer`.')
...
__spy__ = generic('__spy__', "Create a print string of an object using a `Writer`.\n\n.. note:: The function's name was taken from Prolog's `spy` debugging aid.")
...
__all__.extend(('__spy__', '__out__', '__init__', '__call__', '__del__', '__float__',
                '__int__', 'Writer', 'AbstractObject', 'Object', 'FinalizingMixin',
                'spy', 'as_string'))