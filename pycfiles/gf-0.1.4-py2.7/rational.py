...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/gf/examples/rational.py
# Compiled at: 2013-05-11 07:42:48
...
while 1:
    try:
        from gf import method, Object, __init__, __float__, __add__, __sub__, __mul__, __div__, __neg__, __eq__, __ne__, __lt__, __gt__, __le__, __ge__, __out__, __spy__, Writer
    except ImportError:
        if _python_path_tweaked:
            raise
        else:
            import sys, os
            sys.path.insert(0, os.path.abspath(os.path.normpath(os.path.join(os.path.dirname(__file__), os.pardir))))
...
def __init__(rational, numerator, denominator, cancel):
    ...
    """
    __init__.super(Rational, int, int, bool)(rational, numerator, denominator, cancel)
...
def __init__(rational, decimal):
    ...
    if exponent < 0:
        ...
        __init__(rational, int(decimal * denominator), int(denominator))
    else:
