...
# [GCC 8.4.0]
# Embedded file name: /usr/really-local/MIDIUtil/src/midiutil/MidiFile.py
# Compiled at: 2018-03-04 08:58:52
# Size of source mod 2**32: 73304 bytes
from __future__ import division, print_function
import math, struct, warnings
__version__ = 'HEAD'
TICKSPERQUARTERNOTE = 960
controllerEventTypes = {'pan': 10}
MAJOR = 0
MINOR = 1
SHARPS = 1
FLATS = -1
...
class NoteOn(GenericEvent):
    ...
    def __init__(self, channel, pitch, tick, duration, volume, annotation=None, insertion_order=0):
        self.pitch = pitch
    ...
    def __eq__(self, other):
        return self.evtname == other.evtname and self.tick == other.tick and self.pitch == other.pitch and self.channel == other.channel
...
class NoteOff(GenericEvent):
    ...
    def __init__(self, channel, pitch, tick, volume, annotation=None, insertion_order=0):
        self.pitch = pitch
        self.volume = volume
        self.channel = channel
        self.annotation = annotation
...
class Tempo(GenericEvent):
    ...
    def __eq__(self, other):
        return self.evtname == other.evtname and self.tick == other.tick and self.tempo == other.tempo
...
class KeySignature(GenericEvent):
    ...
    def __init__(self, tick, accidentals, accidental_type, mode, insertion_order=0):
...
class ProgramChange(GenericEvent):
    ...
    def __eq__(self, other):
        return self.evtname == other.evtname and self.tick == other.tick and self.programNumber == other.programNumber and self.channel == other.channel
...
class UniversalSysExEvent(GenericEvent):
    ...
    def serialize(self, previous_event_tick):
        ...
        if self.realTime:
            midibytes += struct.pack('>B', 127)
        else:
            midibytes += struct.pack('>B', 126)
...
class ControllerEvent(GenericEvent):
    ...
    def __init__(self, channel, tick, controller_number, parameter, insertion_order=0):
...
class ChannelPressureEvent(GenericEvent):
    ...
    def __eq__(self, other):
        return self.__class__.__name__ == other.__class__.__name__ and self.tick == other.tick and self.pressure_value == other.pressure_value and self.channel == other.channel
...
class PitchWheelEvent(GenericEvent):
    ...
    def serialize(self, previous_event_tick):
        ...
        LSB = self.pitch_wheel_value + 8192 & 127
...
class TrackName(GenericEvent):
    ...
    def __eq__(self, other):
        return self.evtname == other.evtname and self.tick == other.tick and self.trackName == other.trackName
...
class TimeSignature(GenericEvent):
    ...
    def __init__(self, tick, numerator, denominator, clocks_per_tick, notes_per_quarter, insertion_order=0):
...
class MIDITrack(object):
    ...
    def addNoteByNumber(self, channel, pitch, tick, duration, volume, annotation=None, insertion_order=0):
        ...
        """
        self.eventList.append(NoteOn(channel, pitch, tick, duration, volume, annotation=annotation,
          insertion_order=insertion_order))
        self.eventList.append(NoteOff(channel, pitch, (tick + duration), volume, annotation=annotation,
          insertion_order=insertion_order))
    ...
    def addControllerEvent(self, channel, tick, controller_number, parameter, insertion_order=0):
        ...
        """
        self.eventList.append(ControllerEvent(channel, tick, controller_number, parameter,
          insertion_order=insertion_order))
    ...
    def addUniversalSysEx(self, tick, code, subcode, payload, sysExChannel=127, realTime=False, insertion_order=0):
        """
        Add a Universal SysEx event.
        """
        self.eventList.append(UniversalSysExEvent(tick, realTime, sysExChannel, code,
          subcode, payload, insertion_order=insertion_order))
    ...
    def addChannelPressure(self, channel, tick, pressure_value, insertion_order=0):
        ...
        """
        self.eventList.append(ChannelPressureEvent(channel, tick, pressure_value, insertion_order=insertion_order))
    ...
    def addTimeSignature(self, tick, numerator, denominator, clocks_per_tick, notes_per_quarter, insertion_order=0):
        ...
        """
        self.eventList.append(TimeSignature(tick, numerator, denominator, clocks_per_tick,
          notes_per_quarter, insertion_order=insertion_order))
    ...
    def addKeySignature(self, tick, accidentals, accidental_type, mode, insertion_order=0):
        ...
        Add a copyright notice
        """
        self.eventList.append(KeySignature(tick, accidentals, accidental_type, mode,
          insertion_order=insertion_order))
    ...
    def changeNoteTuning(self, tunings, sysExChannel=127, realTime=True, tuningProgam=0, insertion_order=0):
        """
        ...
        self.eventList.append(UniversalSysExEvent(0, realTime, sysExChannel, 8,
          2, payload, insertion_order=insertion_order))
    ...
    def writeMIDIStream(self):
        ...
        self.MIDIdata += struct.pack('BBBB', 0, 255, 47, 0)
...
class MIDIFile(object):
    ...
    def __init__(self, numTracks=1, removeDuplicates=True, deinterleave=True, adjust_origin=False, file_format=1, ticks_per_quarternote=TICKSPERQUARTERNOTE, eventtime_is_ticks=False):
    ...
    def addNote(self, track, channel, pitch, time, duration, volume, annotation=None):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addNoteByNumber(channel, pitch, (self.time_to_ticks(time)),
          (self.time_to_ticks(duration)), volume,
    ...
    def addTrackName(self, track, time, trackName):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addTrackName((self.time_to_ticks(time)), trackName, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addTimeSignature(self, track, time, numerator, denominator, clocks_per_tick, notes_per_quarter=8):
        ...
        if self.header.numeric_format == 1:
            track = 0
        self.tracks[track].addTimeSignature((self.time_to_ticks(time)), numerator, denominator, clocks_per_tick,
          notes_per_quarter, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addTempo(self, track, time, tempo):
        ...
        if self.header.numeric_format == 1:
            track = 0
        self.tracks[track].addTempo((self.time_to_ticks(time)), tempo, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addCopyright(self, track, time, notice):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addCopyright((self.time_to_ticks(time)), notice, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addKeySignature(self, track, time, accidentals, accidental_type, mode, insertion_order=0):
        ...
        * ``MINOR``
        ...
        if self.header.numeric_format == 1:
            track = 0
        self.tracks[track].addKeySignature((self.time_to_ticks(time)), accidentals, accidental_type, mode,
    ...
    def addText(self, track, time, text):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addText((self.time_to_ticks(time)), text, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addProgramChange(self, tracknum, channel, time, program):
        ...
        if self.header.numeric_format == 1:
            tracknum += 1
        self.tracks[tracknum].addProgramChange(channel, (self.time_to_ticks(time)), program, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addChannelPressure(self, tracknum, channel, time, pressure_value):
        ...
        track.addChannelPressure(channel, (self.time_to_ticks(time)), pressure_value, insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addControllerEvent(self, track, channel, time, controller_number, parameter):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addControllerEvent(channel, (self.time_to_ticks(time)), controller_number, parameter,
          insertion_order=(self.event_counter))
        self.event_counter += 1
    ...
    def addPitchWheelEvent(self, track, channel, time, pitchWheelValue):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addPitchWheelEvent(channel, (self.time_to_ticks(time)), pitchWheelValue, insertion_order=(self.event_counter))
        self.event_counter += 1

    def makeRPNCall(self, track, channel, time, controller_msb, controller_lsb, data_msb, data_lsb, time_order=False):
        """
        ...
        track.addControllerEvent(channel, tick, 101, controller_msb,
          insertion_order=(self.event_counter))
        self.event_counter += 1
        tick += tick_incr
        track.addControllerEvent(channel, tick, 100, controller_lsb,
          insertion_order=(self.event_counter))
        self.event_counter += 1
        tick += tick_incr
        track.addControllerEvent(channel, tick, 6, data_msb,
          insertion_order=(self.event_counter))
        self.event_counter += 1
        tick += tick_incr
        if data_lsb is not None:
            track.addControllerEvent(channel, tick, 38, data_lsb,
              insertion_order=(self.event_counter))
            self.event_counter += 1

    def makeNRPNCall(self, track, channel, time, controller_msb, controller_lsb, data_msb, data_lsb, time_order=False):
        """
        ...
        track = self.tracks[track]
        tick_incr = 1 if time_order else 0
        track.addControllerEvent(channel, tick, 99, controller_msb,
          insertion_order=(self.event_counter))
        self.event_counter += 1
        tick += tick_incr
        track.addControllerEvent(channel, tick, 98, controller_lsb,
          insertion_order=(self.event_counter))
        self.event_counter += 1
        tick += tick_incr
        track.addControllerEvent(channel, tick, 6, data_msb,
    ...
    def changeNoteTuning(self, track, tunings, sysExChannel=127, realTime=True, tuningProgam=0):
        """
    ...
    def addSysEx(self, track, time, manID, payload):
        ...
        **Note**: This is a low-level MIDI function, so care must be used in
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addSysEx(tick, manID, payload, insertion_order=(self.event_counter))
        self.event_counter += 1

    def addUniversalSysEx(self, track, time, code, subcode, payload, sysExChannel=127, realTime=False):
        """
        ...
        **Note**: This is a low-level MIDI function, so care must be used in
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addUniversalSysEx(tick, code, subcode, payload, sysExChannel,
    ...
    def findOrigin(self):
        ...
        origin = 100000000
        for track in self.tracks:
            if len(track.MIDIEventList) > 0 and track.MIDIEventList[0].tick < origin:
...
def writeVarLength(i):
    ...
    if i == 0:
        ...
        hibit = 0
        while i > 0:
            vlbytes.append((i & 127 | hibit) & 255)
            i >>= 7
            hibit = 128
...
def frequencyTransform(freq):
    ...
    secondByte = min([int(cents) >> 7, 127])
    thirdByte = cents - (secondByte << 7)
    thirdByte = min([thirdByte, 127])
    if thirdByte == 127:
        if secondByte == 127:
            if firstByte == 127:
                thirdByte = 126
...
def sort_events(event):
    ...
    """
    return (
     event.tick, event.sec_sort_order, event.insertion_order)