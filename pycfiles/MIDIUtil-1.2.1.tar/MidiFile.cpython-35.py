...
# Compiled at: 2017-04-03 11:13:56
# Size of source mod 2**32: 65717 bytes
...
class GenericEvent(object):
    ...
    def __eq__(self, other):
        ...
        if self.time != other.time or self.type != other.type:
            return False
        if self.type == 'note' and (self.pitch != other.pitch or self.channel != other.channel):
            return False
        if self.type == 'tempo' and self.tempo != other.tempo:
            return False
        if self.type == 'programChange' and (self.programNumber != other.programNumber or self.channel != other.channel):
            return False
        if self.type == 'trackName' and self.trackName != other.trackName:
            return False
...
class UniversalSysExEvent(GenericEvent):
    ...
    def __init__(self, time, realTime, sysExChannel, code, subcode, payload, ordinal=1, insertion_order=0):
...
class MIDITrack(object):
    ...
    def addNoteByNumber(self, channel, pitch, time, duration, volume, annotation=None, insertion_order=0):
        """
        Add a note by chromatic MIDI number
        """
        self.eventList.append(Note(channel, pitch, time, duration, volume, annotation=annotation, insertion_order=insertion_order))

    def addControllerEvent(self, channel, time, controller_number, parameter, insertion_order=0):
        """
        Add a controller event.
        """
        self.eventList.append(ControllerEvent(channel, time, controller_number, parameter, insertion_order=insertion_order))
    ...
    def addSysEx(self, time, manID, payload, insertion_order=0):
        ...
        Add a SysEx event.
        """
        self.eventList.append(SysExEvent(time, manID, payload, insertion_order=insertion_order))

    def addUniversalSysEx(self, time, code, subcode, payload, sysExChannel=127, realTime=False, insertion_order=0):
        """
        Add a Universal SysEx event.
        """
        self.eventList.append(UniversalSysExEvent(time, realTime, sysExChannel, code, subcode, payload, insertion_order=insertion_order))
    ...
    def addTimeSignature(self, time, numerator, denominator, clocks_per_tick, notes_per_quarter, insertion_order=0):
        ...
        """
        self.eventList.append(TimeSignature(time, numerator, denominator, clocks_per_tick, notes_per_quarter, insertion_order=insertion_order))
    ...
    def addKeySignature(self, time, accidentals, accidental_type, mode, insertion_order=0):
        ...
        Add a copyright notice
        """
        self.eventList.append(KeySignature(time, accidentals, accidental_type, mode, insertion_order=insertion_order))
    ...
    def changeNoteTuning(self, tunings, sysExChannel=127, realTime=True, tuningProgam=0, insertion_order=0):
        ...
        self.eventList.append(UniversalSysExEvent(0, realTime, sysExChannel, 8, 2, payload, insertion_order=insertion_order))
    ...
    def processEventList(self):
        ...
        for thing in self.eventList:
            if thing.type == 'note':
                ...
                event = MIDIEvent('NoteOff', (thing.time + thing.duration) * TICKSPERBEAT, thing.ord - 0.1, thing.insertion_order)
                event.pitch = thing.pitch
                event.volume = thing.volume
                event.channel = thing.channel
                self.MIDIEventList.append(event)
                ...
                event = MIDIEvent('ProgramChange', thing.time * TICKSPERBEAT, thing.ord, thing.insertion_order)
    ...
    def writeEventsToStream(self):
        ...
        for event in self.MIDIEventList:
            if event.type == 'NoteOn':
                ...
                self.MIDIdata += struct.pack('>b', event.accidentals * event.accidental_type)
                self.MIDIdata += struct.pack('>B', event.mode)
            elif event.type == 'ProgramChange':
                code = 192 | event.channel
                varTime = writeVarLength(event.time)
                for timeByte in varTime:
                    self.MIDIdata += struct.pack('>B', timeByte)
                ...
                self.MIDIdata += struct.pack('B', 255)
                self.MIDIdata += struct.pack('B', 3)
                ...
                if event.type == 'PitchWheelEvent':
                    code = 224 | event.channel
                    varTime = writeVarLength(event.time)
                    for timeByte in varTime:
                        self.MIDIdata = self.MIDIdata + struct.pack('>B', timeByte)

                    MSB = event.pitch_wheel_value + 8192 >> 7
                    LSB = event.pitch_wheel_value + 8192 & 127
                    ...
                    if event.type == 'SysEx':
                        ...
                        self.MIDIdata += struct.pack('>B', 247)
                        ...
                        if event.realTime:
                            self.MIDIdata += struct.pack('>B', 127)
                        else:
                            self.MIDIdata += struct.pack('>B', 126)
...
class MIDIFile(object):
    ...
    def __init__(self, numTracks=1, removeDuplicates=True, deinterleave=True, adjust_origin=None, file_format=1):
        ...
        if adjust_origin is None:
            ...
            warnings.warn('Please explicitly set adjust_origin. Default behaviour will change in a future version.', FutureWarning)
        else:
    ...
    def addNote(self, track, channel, pitch, time, duration, volume, annotation=None):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addNoteByNumber(channel, pitch, time, duration, volume, annotation=annotation, insertion_order=self.event_counter)
    ...
    def addTimeSignature(self, track, time, numerator, denominator, clocks_per_tick, notes_per_quarter=8):
        ...
        if self.header.numeric_format == 1:
            track = 0
        self.tracks[track].addTimeSignature(time, numerator, denominator, clocks_per_tick, notes_per_quarter, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def addKeySignature(self, track, time, accidentals, accidental_type, mode, insertion_order=0):
        ...
        * ``MINOR``
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addKeySignature(time, accidentals, accidental_type, mode, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def addProgramChange(self, track, channel, time, program):
        ...
        """
        self.tracks[track].addProgramChange(channel, time, program, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def addControllerEvent(self, track, channel, time, controller_number, parameter):
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addControllerEvent(channel, time, controller_number, parameter, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def makeRPNCall(self, track, channel, time, controller_msb, controller_lsb, data_msb, data_lsb, time_order=False):
        ...
        self.event_counter += 1
        self.tracks[track].addControllerEvent(channel, time + delta, 100, controller_lsb, insertion_order=self.event_counter)
        self.event_counter += 1
        self.tracks[track].addControllerEvent(channel, time + 2.0 * delta, 6, data_msb, insertion_order=self.event_counter)
        self.event_counter += 1
        if data_lsb is not None:
            self.tracks[track].addControllerEvent(channel, time + 3.0 * delta, 38, data_lsb, insertion_order=self.event_counter)
            self.event_counter += 1
    ...
    def makeNRPNCall(self, track, channel, time, controller_msb, controller_lsb, data_msb, data_lsb, time_order=False):
        ...
        self.event_counter += 1
        self.tracks[track].addControllerEvent(channel, time + delta, 98, controller_lsb, insertion_order=self.event_counter)
        self.event_counter += 1
        self.tracks[track].addControllerEvent(channel, time + 2 * delta, 6, data_msb, insertion_order=self.event_counter)
        self.event_counter += 1
        if data_lsb is not None:
            self.tracks[track].addControllerEvent(channel, time + 3 * delta, 38, data_lsb, insertion_order=self.event_counter)
            self.event_counter += 1
    ...
    def changeNoteTuning(self, track, tunings, sysExChannel=127, realTime=True, tuningProgam=0):
        ...
        *frequency*) format.  As an example, if one wanted to change the
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].changeNoteTuning(tunings, sysExChannel, realTime, tuningProgam, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def addUniversalSysEx(self, track, time, code, subcode, payload, sysExChannel=127, realTime=False):
        ...
        **Note**: This is a low-level MIDI function, so care must be used in
        ...
        if self.header.numeric_format == 1:
            track += 1
        self.tracks[track].addUniversalSysEx(time, code, subcode, payload, sysExChannel, realTime, insertion_order=self.event_counter)
        self.event_counter += 1
    ...
    def findOrigin(self):
        ...
        origin = 1000000
        for track in self.tracks:
            if len(track.MIDIEventList) > 0 and track.MIDIEventList[0].time < origin:
...
def frequencyTransform(freq):
    ...
    if thirdByte == 127 and secondByte == 127 and firstByte == 127:
        thirdByte = 126
...
def sort_events(event):
    ...
    """
    return (
     event.time, event.ord, event.insertion_order)