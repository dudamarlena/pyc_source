...
# [GCC 8.4.0]
# Embedded file name: paxmusica/controller.py
# Compiled at: 2013-03-05 17:11:41
import os
...
@app.route('/<artist>/')
def albums(artist):
    return render_template('albums.html', albums=model.get_albums_by_artist(artist), artist=artist)


@app.route('/<artist>/<album>/')
def songs(artist, album):
    return render_template('songs.html', songs=model.get_songs_by_artist_and_album(artist, album), artist=artist, album=album)


@app.route('/playlist/')
def playlist():
    return render_template('/playlist.html', first_entry=model.get_first_playlist_entry(), remaining_entries=model.get_remaining_playlist_entries())
