...
# Compiled at: 2018-06-15 10:43:10
# Size of source mod 2**32: 81637 bytes
    ...
    def get_bar_indices(self, data_frame, style, chart_type, bar_ind):
        ...
        if style.chart_type is not None:
            if isinstance(style.chart_type, list):
                ...
                if 'bar' == style.chart_type:
                    xd = bar_ind
                    has_bar = 'barv'
                else:
                    if 'barh' == style.chart_type:
                        xd = bar_ind
                        has_bar = 'barh'
        ...
        if has_matrix == '2d-matrix' or has_matrix == '3d-matrix':
            ...
            for x in range(len(data_frame.index)):
                for y in range(len(data_frame.columns)):
                    plt.text(x + offset, y + offset, '%.0f' % data_frame.ix[(x, y)], horizontalalignment='center', verticalalignment='center')
...
cf = None
try:
    import plotly, cufflinks as cf, plotly, plotly.graph_objs as go
        ...
        for i in range(0, len(data_frame_list)):
            ...
            if chart_type_ord == 'line':
                ...
                if style.thin_margin:
                    fig.update(dict(layout=dict(margin=go.Margin(l=20, r=20, b=40, t=40, pad=0))))
                ...
                if chart_type[j] == 'line':
                    ...
                    if chart_type[j] == 'scatter':
                        ...
                        if chart_type[j] in ('dash', 'dashdot', 'dot'):
                            ...
                            if chart_type[j] in ('hv', 'vh', 'vhv', 'spline', 'linear'):
    ...
    def publish_plot(self, fig, style):
        ...
        if style.plotly_plot_mode == 'dash':
            ...
            if style.plotly_plot_mode == 'online':
                plotly.tools.set_credentials_file(username=style.plotly_username, api_key=style.plotly_api_key)
                plotly.plotly.plot(fig, filename=style.plotly_url, world_readable=style.plotly_world_readable, auto_open=not style.silent_display, asImage=style.plotly_as_image)
            else:
            ...
            if style.plotly_plot_mode != 'dash':
                try:
                    plotly.plotly.image.save_as(fig, filename=style.file_output, format='png', width=style.width * abs(style.scale_factor), height=style.height * abs(style.scale_factor))
                except:
                    pass
