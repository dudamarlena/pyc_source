...
# Embedded file name: E:\cuemacro\chartpy\chartpy\engine.py
# Compiled at: 2020-05-05 13:15:36
# Size of source mod 2**32: 85291 bytes
...
class EngineTemplate(ABC):
    ...
    def get_bar_indices--- This code section failed: ---

 L.  50         0  LOAD_STR                 'no-bar'
                2  STORE_FAST               'has_bar'

 L.  51         4  LOAD_FAST                'data_frame'
                6  LOAD_ATTR                index
                8  STORE_FAST               'xd'

 L.  52        10  LOAD_GLOBAL              len
               12  LOAD_FAST                'data_frame'
               14  LOAD_ATTR                columns
               16  CALL_FUNCTION_1       1  ''
               18  STORE_FAST               'no_of_bars'

 L.  54        20  LOAD_FAST                'style'
               22  LOAD_ATTR                chart_type
               24  LOAD_CONST               None
               26  COMPARE_OP               is-not
               28  POP_JUMP_IF_FALSE   158  'to 158'

 L.  55        30  LOAD_GLOBAL              isinstance
               32  LOAD_FAST                'style'
               34  LOAD_ATTR                chart_type
               36  LOAD_GLOBAL              list
               38  CALL_FUNCTION_2       2  ''
               40  POP_JUMP_IF_FALSE    98  'to 98'

 L.  56        42  LOAD_STR                 'bar'
               44  LOAD_FAST                'style'
               46  LOAD_ATTR                chart_type
               48  COMPARE_OP               in
               50  POP_JUMP_IF_FALSE    74  'to 74'

 L.  57        52  LOAD_FAST                'bar_ind'
               54  STORE_FAST               'xd'

 L.  58        56  LOAD_FAST                'style'
               58  LOAD_ATTR                chart_type
               60  LOAD_ATTR                count
               62  LOAD_STR                 'bar'
               64  CALL_FUNCTION_1       1  ''
               66  STORE_FAST               'no_of_bars'

 L.  59        68  LOAD_STR                 'barv'
               70  STORE_FAST               'has_bar'
               72  JUMP_ABSOLUTE       156  'to 156'
               74  ELSE                     '96'

 L.  60        74  LOAD_STR                 'stacked'
               76  LOAD_FAST                'style'
               78  LOAD_ATTR                chart_type
               80  COMPARE_OP               in
               82  POP_JUMP_IF_FALSE   156  'to 156'

 L.  61        84  LOAD_FAST                'bar_ind'
               86  STORE_FAST               'xd'

 L.  62        88  LOAD_CONST               1
               90  STORE_FAST               'no_of_bars'

 L.  63        92  LOAD_STR                 'barv'
               94  STORE_FAST               'has_bar'
               96  JUMP_ABSOLUTE       182  'to 182'
               98  ELSE                     '156'

 L.  64        98  LOAD_STR                 'bar'
              100  LOAD_FAST                'style'
              102  LOAD_ATTR                chart_type
              104  COMPARE_OP               ==
              106  POP_JUMP_IF_FALSE   118  'to 118'

 L.  65       108  LOAD_FAST                'bar_ind'
              110  STORE_FAST               'xd'

 L.  66       112  LOAD_STR                 'barv'
              114  STORE_FAST               'has_bar'
              116  JUMP_ABSOLUTE       182  'to 182'
              118  ELSE                     '156'

 L.  67       118  LOAD_STR                 'barh'
              120  LOAD_FAST                'style'
              122  LOAD_ATTR                chart_type
              124  COMPARE_OP               ==
              126  POP_JUMP_IF_FALSE   138  'to 138'

 L.  68       128  LOAD_FAST                'bar_ind'
              130  STORE_FAST               'xd'

 L.  69       132  LOAD_STR                 'barh'
              134  STORE_FAST               'has_bar'
              136  JUMP_ABSOLUTE       182  'to 182'
              138  ELSE                     '156'

 L.  70       138  LOAD_STR                 'stacked'
              140  LOAD_FAST                'style'
              142  LOAD_ATTR                chart_type
              144  COMPARE_OP               ==
              146  POP_JUMP_IF_FALSE   182  'to 182'

 L.  71       148  LOAD_FAST                'bar_ind'
              150  STORE_FAST               'xd'

 L.  72       152  LOAD_STR                 'barh'
              154  STORE_FAST               'has_bar'
            156_0  COME_FROM            82  '82'
              156  JUMP_FORWARD        182  'to 182'
              158  ELSE                     '182'

 L.  74       158  LOAD_FAST                'chart_type'
              160  LOAD_STR                 'bar'
              162  COMPARE_OP               ==
              164  POP_JUMP_IF_TRUE    174  'to 174'
              166  LOAD_FAST                'chart_type'
              168  LOAD_STR                 'stacked'
              170  COMPARE_OP               ==
            172_0  COME_FROM           164  '164'
              172  POP_JUMP_IF_FALSE   182  'to 182'

 L.  75       174  LOAD_FAST                'bar_ind'
              176  STORE_FAST               'xd'

 L.  76       178  LOAD_STR                 'barv'
              180  STORE_FAST               'has_bar'
            182_0  COME_FROM           172  '172'
            182_1  COME_FROM           156  '156'
            182_2  COME_FROM           146  '146'

 L.  78       182  LOAD_FAST                'xd'
              184  LOAD_FAST                'bar_ind'
              186  LOAD_FAST                'has_bar'
              188  LOAD_FAST                'no_of_bars'
              190  BUILD_TUPLE_4         4 
              192  RETURN_VALUE     
               -1  RETURN_LAST      
...
Parse error at or near `RETURN_VALUE' instruction at offset 192
    ...
    def generate_file_names(self, style, engine):
        if style.html_file_output is not None:
            if not style.auto_generate_html_filename:
                pass
            else:
                import time
                style.html_file_output = self.get_time_stamp() + '-' + engine + '.html'
                style.auto_generate_html_filename = True
        elif style.file_output is not None and not style.auto_generate_filename:
            pass
        else:
            import time
    ...
    def plot_chart(self, data_frame, style, chart_type):
        ...
        for data_frame in data_frame_list:
            ...
            if chart_type == 'heatmap':
                p1 = HeatMap(data_frame, title='Random',
                ...
                if has_bar == 'barv':
                    ...
                    if type(data_frame.index) == pandas.Timestamp or type(xd[0]) == pandas.Timestamp and type(xd[(-1)]) == pandas.Timestamp or type(data_frame.index) == pandas.DatetimeIndex:
...
class EngineMatplotlib(EngineTemplate):
    ...
    def plot_chart(self, data_frame, style, chart_type):
                ...
                if not isinstance(chart_type, list):
                    ...
                    if style.color == []:
                        ...
                        if chart_type == 'heatmap':
                            ...
                            Z = data_frame.values
                            if style.normalize_colormap:
                                movie_frame.append(ax_temp.plot_surface(X, Y, Z, cmap=color, rstride=1, cstride=1, vmax=maxz,
                                  vmin=minz))
                            else:
        ...
        if style.animate_figure:
            ...
            try:
                anim = animation.FuncAnimation((plt.gcf()), update, interval=(style.animate_frame_ms), blit=True, frames=(len(data_frame_list)),
                  init_func=init,
                  repeat=True)
            except Exception as e:
                print(str(e))
        ...
        if has_matrix == '2d-matrix' or has_matrix == '3d-matrix':
            ...
            for x in range(len(data_frame.index)):
                for y in range(len(data_frame.columns)):
                    plt.text((x + offset), (y + offset), ('%.0f' % data_frame.iloc[(x, y)]), horizontalalignment='center',
            ...
            if hasattr(data_frame.index[0], 'hour') and not hasattr(data_frame.index[0], 'month'):
                ax.xaxis.set_major_locator(MultipleLocator(28800.0))
                ax.xaxis.set_minor_locator(MultipleLocator(3600.0))
    ...
    def _create_brand_label(self, ax, anno, scale_factor):
        ax.annotate(anno, xy=(1, 1), xycoords='axes fraction', fontsize=(10 * abs(scale_factor)),
          color='white',
          xytext=(
         0 * abs(scale_factor), 15 * abs(scale_factor)),
          textcoords='offset points',
          va='center',
          ha='center',
        ...
        if style.display_source_label == True:
            if style.source is not None:
                ax.annotate(('Source: ' + style.source), xy=(1, 0), xycoords='axes fraction', fontsize=(7 * abs(style.scale_factor)),
                  xytext=(
                 -5 * abs(style.scale_factor), 10 * abs(style.scale_factor)),
                  textcoords='offset points',
                  ha='right',
                  va='top',
...
try:
    ...
    plotly.tools.set_config_file(plotly_domain='https://type-here.com', world_readable=(cc.plotly_world_readable),
        ...
        if isinstance(data_frame, go.Figure):
            ...
            try:
                if style.plotly_plot_mode == 'offline_html':
                    if style.scale_factor > 0:
                        scale = float(0.6666666666666666)
            except:
                pass
            ...
            for i in range(0, len(data_frame_list)):
                ...
                if chart_type_ord == 'choropleth':
                    ...
                    layout = dict(title=(style.title),
                      geo=dict(scope=(style.plotly_scope),
                      projection=dict(type=(style.plotly_projection)),
                      showlakes=True,
                      lakecolor='rgb(255, 255, 255)'))
                    fig = dict(data=data, layout=layout)
                else:
                    if style.plotly_helper == 'cufflinks':
                        if chart_type_ord == 'surface':
                            fig = data_frame.iplot(kind=chart_type, title=(style.title),
                            ...
                            if chart_type_ord == 'heatmap':
                                ...
                                if chart_type_ord == 'line':
                                    ...
                                    if chart_type_ord in ('dash', 'dashdot', 'dot'):
                                        ...
                                        if chart_type_ord == 'line+markers':
                                            full_line = True
                                            chart_type_ord = 'line'
                                            mode = 'lines+markers'
                                            marker_size = 5
                                ...
                                while m < 10:
                                    try:
                                        fig = data_frame.iplot(kind=chart_type_ord, title=(style.title),
                                          xTitle=(style.x_title),
                                          yTitle=(style.y_title),
                                          x=x,
                                          y=y,
                        ...
                        if style.plotly_helper == 'plotly_express':
                            pass
                ...
                fig.update(dict(layout=dict(legend=dict(x=0.05,
                  y=1))))
                if style.thin_margin:
                    fig.update(dict(layout=dict(margin=go.layout.Margin(l=20,
                      r=20,
                      b=40,
                      t=40,
                      pad=0))))
                ...
                if isinstance(chart_type, list):
                    for j in range(0, len(fig['data'])):
                        ...
                        if chart_type[j] == 'line':
                            ...
                            if chart_type[j] == 'line+markers':
                                mode = 'lines+markers'
                            else:
                                if chart_type[j] == 'scatter':
                                    mode = 'markers'
                                else:
                                    if chart_type[j] in ('dash', 'dashdot', 'dot'):
                                        dash = chart_type[j]
                                        mode = 'lines'
                                    else:
                                        if chart_type[j] in ('hv', 'vh', 'vhv', 'spline',
                                                             'linear'):
                                            line_shape = chart_type[j]
                                            mode = 'lines'
                                        else:
                                            if chart_type[j] == 'bubble':
                                                mode = 'markers'
            ...
            if style.candlestick_series is not None:
                if not style.plotly_webgl:
                    if isinstance(style.candlestick_series, Figure):
                        fig_candle = style.candlestick_series
                    else:
                        fig_candle = create_candlestick((style.candlestick_series['open']), (style.candlestick_series['high']),
    ...
    def publish_plot(self, fig, style):
        ...
        if style.plotly_plot_mode == 'dash':
            ...
            if style.plotly_plot_mode == 'online':
                plotly.tools.set_credentials_file(username=(style.plotly_username), api_key=(style.plotly_api_key))
                py_online.plot(fig, filename=(style.plotly_url), world_readable=(style.plotly_world_readable),
                ...
                if style.plotly_plot_mode == 'offline_html':
                    ...
                    if style.plotly_plot_mode == 'offline_png':
            ...
            if style.plotly_plot_mode == 'offline_image_png_in_html':
                ...
                if style.plotly_plot_mode == 'offline_image_svg_in_html':
                    return '<img src="data:image/svg;base64,' + base64.b64encode(plotly.io.to_image(fig, format='svg')).decode('utf8') + '">'
                if style.plotly_plot_mode == 'offline_jupyter':
                    py_offline.init_notebook_mode()
                    py_offline.iplot(fig)
                else:
                    if style.plotly_plot_mode == 'offline_jupyter_connected':
                        py_offline.init_notebook_mode(connected=True)
                        py_offline.iplot(fig)
                    else:
                        if style.plotly_plot_mode != 'dash':
                            try:
                                py_online.image.save_as(fig, filename=(style.file_output), format='png', width=(style.width * abs(style.scale_factor)),
...
def make_increasing_candle(open, high, low, close, dates, **kwargs):
    ...
    candle_incr_data = dict(type='box', x=increase_x, 
     y=increase_y, 
     whiskerwidth=0, 
     boxpoints=False, **kwargs)
    return [
     candle_incr_data]
...
def make_decreasing_candle(open, high, low, close, dates, **kwargs):
    ...
    candle_decr_data = dict(type='box', x=decrease_x, 
     y=decrease_y, 
     whiskerwidth=0, 
     boxpoints=False, **kwargs)
    return [
