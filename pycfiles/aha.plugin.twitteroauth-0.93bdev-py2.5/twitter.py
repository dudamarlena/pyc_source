...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/twitteroauth/twitter.py
# Compiled at: 2011-04-01 06:35:43
"""
...
import functools, logging, urllib
...
class TwitterMixin(OAuthMixin):
    ...
    def _oauth_consumer_token(self):
        return dict(key=self._twitter_consumer_key(), secret=self._twitter_consumer_secret())
    ...
    def authenticate_redirect(self):
        ...
        return self._on_request_token(self._OAUTH_AUTHENTICATE_URL, None, response)
    ...
    def twitter_request(self, path, callback, access_token=None, post_args=None, **args):
        ...
        try:
            if post_args is not None:
                response = urlfetch.fetch(url, method='POST', payload=urllib.urlencode(post_args), deadline=10)
            else:
                response = urlfetch.fetch(url, deadline=10)
        except urlfetch.DownloadError, e:
            logging.exception(e)
    ...
    def _on_twitter_request(self, callback, response):
        if not response:
            ...
            logging.warning('Invalid Twitter response (%d): %s', response.status_code, response.content)
            return callback(None)
        return callback(simplejson.loads(response.content))

    def _oauth_get_user(self, access_token, callback):
        callback = functools.partial(self._parse_user_response, callback)
        return self.twitter_request('/users/show/' + access_token['screen_name'], access_token=access_token, callback=callback)
