...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/okera/odas.py
# Compiled at: 2020-02-27 14:06:57
# Size of source mod 2**32: 91572 bytes
from __future__ import absolute_import
import okera, datetime
from functools import wraps
import os, random, pickle, pytz, time, certifi, jwt, prestodb, urllib3, xml.dom.minidom
...
from okera._thrift_api import TAssignAttributesParams, TAttribute, TAttributeValue, TCreateAttributesParams, TDeleteAttributesParams, TGetAttributesParams, TGetDatabasesParams, TGetDatasetsParams, TGetAttributeNamespacesParams, TGetPartitionsParams, TGetRegisteredObjectsParams, TGetTablesParams, TNetworkAddress, TPlanRequestParams, TRequestType, TExecDDLParams, TExecTaskParams, TFetchParams, TListFilesOp, TListFilesParams, TRecordServiceException, TRecordFormat, TTypeId, TUnassignAttributesParams, OkeraRecordServicePlanner, RecordServiceWorker
from okera._thrift_util import create_socket, get_transport, TTransportException, TBinaryProtocol, PlannerClient, WorkerClient, KERBEROS_NOT_ENABLED_MSG, SOCKET_READ_ZERO
from .concurrency import BaseBackgroundTask, ConcurrencyController, default_max_client_process_count
if HAS_NUMPY:
    import numpy
if HAS_PANDAS:
    import pandas
...
def _convert_presto_type(typeinfo, field):
    if field is None:
        return field
    else:
        if typeinfo['rawType'] == 'row':
            ...
            for idx in range(len(typeinfo['literalArguments'])):
                ...
                subfield = field[idx]
                new_field[literal] = _convert_presto_type(subtype, subfield)

            return new_field
        if typeinfo['rawType'] == 'array':
            subtype = typeinfo['typeArguments'][0]
            return list(map(lambda subfield: _convert_presto_type(subtype, subfield), field))
        if typeinfo['rawType'] == 'map':
            value_subtype = typeinfo['typeArguments'][1]
            new_field = {}
            for subkey in field:
                new_field[subkey] = _convert_presto_type(value_subtype, field[subkey])
        ...
        while True:
            try:
                ...
                if attempt == retries:
                    ...
                    delay_ms = random.randint(1, 2 ** (attempt - 1) * initial_retry_delay_ms)
                    _log.warning('Attempt failed. Sleeping %dms and then trying again.' % delay_ms)
...
class OkeraContext:
    ...
    def __init__(self, application_name, namespace, tz=pytz.utc, retries=3, initial_retry_delay_ms=2000, dialect=None):
        _log.debug('Creating Okera context')
        self._OkeraContext__auth = None
        self._OkeraContext__service_name = None
        self._OkeraContext__token = None
        self._OkeraContext__token_func = None
        self._OkeraContext__host_override = None
        self._OkeraContext__user = None
        self._OkeraContext__name = application_name
        self._OkeraContext__namespace = namespace
        self._OkeraContext__dialect = dialect
        self._OkeraContext__configure()
        self._OkeraContext__tz = tz
        self._OkeraContext__retries = retries
        self._OkeraContext__initial_retry_delay_ms = initial_retry_delay_ms
    ...
    def enable_kerberos(self, service_name, host_override=None):
        ...
        self._OkeraContext__auth = 'GSSAPI'
        self._OkeraContext__service_name = service_name
        self._OkeraContext__host_override = host_override
        self._OkeraContext__user = None
        _log.debug('Enabled kerberos')
        return self

    def enable_token_auth(self, token_str=None, token_file=None, token_func=None, user=None):
        ...
        self._OkeraContext__token_func = token_func
        self._OkeraContext__configure_token(token, user)
    ...
    def get_auth(self):
        ...
        return self._OkeraContext__auth

    def get_token(self):
        """ Returns the token string. Note that logging this should be done with care."""
        return self._OkeraContext__token

    def get_name(self):
        """ Returns name of this application. This is recorded for diagnostics on
            the server.
        """
        return self._OkeraContext__name

    def _get_user(self):
        """ Returns the user name. This is ignored if authentication is enabled. """
        return self._OkeraContext__user

    def get_namespace(self):
        return self._OkeraContext__namespace

    def get_dialect(self):
        return self._OkeraContext__dialect

    def get_timezone(self):
        return self._OkeraContext__tz

    def get_retries(self):
        return self._OkeraContext__retries

    def get_initial_retry_delay_ms(self):
        return self._OkeraContext__initial_retry_delay_ms

    @_rpc
    def connect(self, host='localhost', port=12050, timeout=10, presto_host=None, presto_port=None, namespace=None):
        ...
        namespace : str, optional
            The Presto namespace to use for this connection. If set, this will override the value
        ...
        planner = PlannerConnection(service, self, host, port, timeout, presto_host, presto_port, namespace)
        planner._set_application_internal(self._OkeraContext__name)
        return planner
    ...
    def _connect_planner_internal(self, host, port, timeout):
        ...
        """
        host, port = self._OkeraContext__pick_host(host, port)
        auth_mechanism = self._OkeraContext__get_auth()
        _log.debug('Connecting to planner %s:%s with %s authentication mechanism', host, port, auth_mechanism)
        sock = create_socket(host, port, timeout, False, None)
        transport = None
        try:
            transport = get_transport(sock, host, auth_mechanism, self._OkeraContext__service_name, None, None, self._OkeraContext__token, self._OkeraContext__host_override)
            transport.open()
            ...
            if transport:
                transport.close()
            self._OkeraContext__handle_transport_exception(e)
            raise e
        except:
            sock.close()
            if transport:
    ...
    def _connect_worker(self, host, port, timeout=10, options=None):
        ...
        worker._set_application_internal(self._OkeraContext__name)
        return worker
    ...
    def _connect_worker_internal(self, host, port, timeout, options):
        ...
        """
        host, port = self._OkeraContext__pick_host(host, port, options)
        auth_mechanism = self._OkeraContext__get_auth()
        _log.debug('Connecting to worker %s:%s with %s authentication mechanism', host, port, auth_mechanism)
        sock = create_socket(host, port, timeout, False, None)
        transport = None
        try:
            transport = get_transport(sock, host, auth_mechanism, self._OkeraContext__service_name, None, None, self._OkeraContext__token, self._OkeraContext__host_override)
    ...
    def __configure(self):
        ...
        if os.path.exists(token_file):
            with open(token_file, 'r') as (t):
                self._OkeraContext__configure_token(t.read().strip())
    ...
    def _has_token_func(self):
        ...
        return self._OkeraContext__token_func is not None

    def _generate_token(self):
        """ Generates a new token by calling the token_func """
        new_token = self._OkeraContext__token_func()
        new_user = _get_user_from_token(new_token)
        self._OkeraContext__configure_token(new_token, new_user)

    def __configure_token(self, token, user=None):
        if '.' in token:
            self._OkeraContext__token = token
            self._OkeraContext__auth = 'TOKEN'
            self._OkeraContext__service_name = 'cerebro'
            self._OkeraContext__user = user
        else:
            self._OkeraContext__token = None
            self._OkeraContext__auth = None
            self._OkeraContext__user = token
        self._OkeraContext__host_override = None

    def __handle_transport_exception(self, e):
        """ Maps transport layer exceptions to better user facing ones. """
        if self._OkeraContext__auth and e.message == SOCKET_READ_ZERO:
            e.message = 'Server did not respond to authentication handshake. ' + 'Ensure server has authentication enabled.'
        else:
            if not self._OkeraContext__auth and e.message == SOCKET_READ_ZERO:
                e.message = 'Client does not have authentication enabled but it appears ' + 'the server does. Enable client authentication.'
            elif self._OkeraContext__auth == 'GSSAPI' and KERBEROS_NOT_ENABLED_MSG in e.message:
                e.message = 'Client is authenticating with kerberos but kerberos is not ' + 'enabled on the server.'
        raise e

    def __get_auth(self):
        """ Canonicalizes user facing auth names to transport layer ones """
        auth_mechanism = self._OkeraContext__auth
...
class _ThriftService:
    __doc__ = ' Wrapper around a thrift service client object '
...
class PlannerConnection:
    __doc__ = 'A connection to an ODAS planner. '

    def __init__(self, thrift_service, ctx, host, port, timeout, presto_host, presto_port, namespace):
    ...
    def _reconnect(self):
        ...
        self.close()
        self.service = self.ctx._connect_planner_internal(self.host, self.port, self.timeout)
    ...
    def __choose_dialect(self, dialect):
        ...
        if dialect == DIALECT_PRESTO and is_presto_disabled:
            raise ValueError("Dialect '%s' was selected, but no Presto host/port were specified" % DIALECT_PRESTO)
        return dialect
    ...
    def open(self, path, preload_content=True, version=None):
        ...
        try:
            ...
            if urls and len(urls) != 1:
                raise ValueError('Unexpected result from server. Expecting at most one url.')
            return self.http_pool.request('GET', urls[0], preload_content=preload_content)
    ...
    def cat(self, path, as_utf8=True):
        ...
        if result.status != 200:
            if 'Content-Type' in result.headers and result.headers['Content-Type'] == 'application/xml':
    ...
    @_rpc
    def create_attribute(self, namespace, key, if_not_exists=False):
        ...
        if_not_exists : bool
            If true, don't fail the request if the attribute already exists.
        ...
        try:
            ...
            if ex.detail and 'duplicate key value' in ex.detail:
                if if_not_exists:
                    return
                raise RuntimeError('Attribute %s.%s already exists.' % (namespace, key))
            raise
    ...
    def delete_attribute(self, namespace, key):
        ...
        if result.delete_count > 1:
            _log.warning(('delete_attribute() deleted multiple (%d) attributes. ' + 'This is unexpected.') % result.delete_count)
        return result.delete_count >= 1

    @_rpc
    def list_attribute_namespaces(self):
    ...
    @_rpc
    def list_attributes(self, namespace=None):
    ...
    def __set_attribute(self, is_assign, namespace, key, db, dataset=None, column=None, if_existence=False):
        attr = TAttribute()
    ...
    @_rpc
    def assign_attribute(self, namespace, key, db, dataset=None, column=None, if_not_exists=False):
        ...
        if_not_exists : bool
            If true, don't fail the request if the attribute assignment already exists.

        Returns
        -------
        None
        """
        self._PlannerConnection__set_attribute(True, namespace, key, db, dataset, column, if_not_exists)

    @_rpc
    def unassign_attribute(self, namespace, key, db, dataset=None, column=None, if_not_exists=False):
        ...
        column : str, optional
            ...
            dataset.
        if_exists : bool
            If true, don't fail the request if the attribute assignment does not exist.

        Returns
        -------
        None
        """
        self._PlannerConnection__set_attribute(False, namespace, key, db, dataset, column, if_not_exists)
    ...
    @_rpc
    def list_datasets(self, db, filter=None, name=None):
    ...
    def scan_as_pandas(self, request, dialect=None, max_records=None, max_client_process_count=default_max_client_process_count(), max_task_count=None, requesting_user=None, options=None, ignore_errors=False, warnings=None, strings_as_utf8=False):
        ...
        chosen_dialect = self._PlannerConnection__choose_dialect(dialect)
        res = []
        if chosen_dialect == DIALECT_OKERA:
            plan = self.plan(request, max_task_count=max_task_count, requesting_user=requesting_user)
            ...
            concurrency_ctl = self._get_concurrency_controller_for_plan(plan, max_client_process_count)
            for task in plan.tasks:
                _log.debug('Executing task %s', str(task.task_id))
                concurrency_ctl.enqueueTask(PandasScanTask(self.ctx, plan.hosts, task, max_records, options, strings_as_utf8, output_format=output_format))

            result_list = self._start_and_wait_for_results(concurrency_ctl, len(plan.tasks), limit=max_records, is_pandas=True, ignore_errors=ignore_errors)
            if not result_list:
                col_names = []
            ...
            results, colnames = self._PlannerConnection__execute_presto_query(request, max_records)
            df = pandas.DataFrame(results, columns=colnames)
            return df

    def scan_as_json(self, request, dialect=None, max_records=None, warnings=None, max_client_process_count=default_max_client_process_count(), max_task_count=None, requesting_user=None, ignore_errors=False, strings_as_utf8=False):
        ...
        if chosen_dialect == DIALECT_OKERA:
            ...
            for task in plan.tasks:
                _log.debug('Executing task %s', str(task.task_id))
                concurrency_ctl.enqueueTask(JsonScanTask(self.ctx, plan.hosts, task, max_records, output_format=output_format, strings_as_utf8=strings_as_utf8))

            res = self._start_and_wait_for_results(concurrency_ctl, len(plan.tasks), limit=max_records, ignore_errors=ignore_errors)
        elif chosen_dialect == DIALECT_PRESTO:
            results, colnames = self._PlannerConnection__execute_presto_query(request, max_records, to_dict=True)
            res = results
        if max_records is not None:
            return res[:max_records]
        return res

    def __execute_presto_query(self, query, max_records, to_dict=False):
        username = self.ctx._get_user()
        token = self.ctx.get_token() or self.ctx._get_user()
        ...
        typeinfo = []
        try:
            conn = prestodb.dbapi.connect(host=self.presto_host, port=self.presto_port, user=username, catalog=self.namespace, http_scheme='https', auth=prestodb.auth.BasicAuthentication(username, token), isolation_level=prestodb.transaction.IsolationLevel.READ_UNCOMMITTED)
        ...
        for idx in range(len(results)):
            ...
            for typeidx in range(len(types)):
                ...
                if to_dict:
                    dict_result[name] = _convert_presto_type(typeinfo, result[typeidx])
                else:
                    result[typeidx] = _convert_presto_type(typeinfo, result[typeidx])
    ...
    @staticmethod
    def _start_and_wait_for_results(concurrency_ctl, task_count, limit=None, is_pandas=False, ignore_errors=False):
        results = []
        if not task_count:
            return results
        try:
    ...
    def _ensure_serialization_support(plan):
        assert_numpy_installed()
        if not plan.supported_result_formats or TRecordFormat.ColumnarNumPy not in plan.supported_result_formats:
            raise IOError('PyOkera requires the server to support the ' + '`ColumnarNumPy` serialization format. Please upgrade the ' + 'server to at least 0.8.1.')


class WorkerConnection:
    __doc__ = 'A connection to a CDAS worker. '
    ...
    def _reconnect(self):
        ...
        self.close()
        self.service = self.ctx._connect_worker_internal(self.host, self.port, self.timeout, self.options)
    ...
    def exec_task(self, task, max_records=None, output_format=TRecordFormat.ColumnarNumPy):
...
if HAS_NUMPY:
    _numpy_ttype_to_info = {TTypeId.BOOLEAN: (numpy.bool, 1, 'BOOLEAN'), 
...
def _convert_col_native(schema, cols, col_idx, data_col_idx, num_records, as_utf8, buf, is_null, ctx_tz, prefix=''):
    assert_numpy_installed()
    any_nulls = numpy.any(is_null)
    if schema[col_idx].type.type_id == TTypeId.STRING or schema[col_idx].type.type_id == TTypeId.VARCHAR:
        column = [
         None] * num_records
        offset = 0
        for idx in range(len(is_null)):
            if is_null[idx]:
                ...
                length = numpy.frombuffer(buf[offset:offset + 4], dtype=numpy.int32).item()
                offset += 4
        ...
        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.CHAR:
        offset = 0
        ...
        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.BOOLEAN or schema[col_idx].type.type_id == TTypeId.TINYINT or schema[col_idx].type.type_id == TTypeId.SMALLINT or schema[col_idx].type.type_id == TTypeId.INT or schema[col_idx].type.type_id == TTypeId.BIGINT or schema[col_idx].type.type_id == TTypeId.FLOAT or schema[col_idx].type.type_id == TTypeId.DOUBLE:
        offset = 0
        column = [None] * num_records
        ...
        for idx in range(len(is_null)):
            if is_null[idx]:
                ...
                column[idx] = numpy.frombuffer(buf[offset:offset + length], dtype=dtype).item()
                offset += length

        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.DATE:
        column = [
         None] * num_records
        ...
        for idx in range(len(is_null)):
            if is_null[idx]:
                ...
                days_offset = numpy.frombuffer(buf[offset:offset + length], dtype=numpy.int32).item()
                column[idx] = (epoch + datetime.timedelta(days_offset)).date()
                offset += length

        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.TIMESTAMP_NANOS:
        column = [
         None] * num_records
        epoch = datetime.datetime(1970, 1, 1, 0, 0)
        ms_length = 8
        ...
        for idx in range(len(is_null)):
            if is_null[idx]:
                ...
                ms_offset = numpy.frombuffer(buf[offset:offset + ms_length], dtype=numpy.int64).item()
                ns_offset = numpy.frombuffer(buf[offset + ms_length:offset + ms_length + ns_length], dtype=numpy.int32).item()
                column[idx] = datetime.datetime.fromtimestamp(ms_offset / 1000.0, ctx_tz)
                offset += ms_length + ns_length

        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.DECIMAL:
        ...
        return (
         column, 1, any_nulls, 0, 1)
    if schema[col_idx].type.type_id == TTypeId.ARRAY:
        column_lens = [
         0] * num_records
        column = [None] * num_records
        offset = 0
        ...
        for idx in range(len(is_null)):
            if is_null[idx]:
                ...
                column_lens[idx] = numpy.frombuffer(buf[offset:offset + length], dtype=numpy.int32).item()
                offset += length

        item_col = cols[(data_col_idx + 1)]
        item_buf, item_is_nulls = _prepare_column_native(schema[(col_idx + 1)], item_col)
        values, _, _, skip_cols, skip_data_cols = _convert_col_native(schema, cols, col_idx + 1, data_col_idx + 1, sum(column_lens), as_utf8, item_buf, item_is_nulls, ctx_tz, prefix=prefix + '    ')
        values_offset = 0
        ...
        return (
         column, 1, any_nulls, skip_cols + 1, skip_data_cols + 1)
    if schema[col_idx].type.type_id == TTypeId.RECORD:
        any_nulls = False
        column = [None] * num_records
        ...
        while child_idx < num_children:
            ...
            child_buf, child_is_nulls = _prepare_column_native(schema[(col_idx + child_col_idx_offset + 1)], child_col)
            values, _, _, skip_cols, skip_data_cols = _convert_col_native(schema, cols, col_idx + child_col_idx_offset + 1, data_col_idx + child_data_col_idx_offset, num_records, as_utf8, child_buf, child_is_nulls, ctx_tz, prefix=prefix + '    ')
            child_values[child_idx] = (
             child_col_name, values)
    ...
    if schema[col_idx].type.type_id == TTypeId.MAP:
        column_lens = [None] * num_records
        offset = 0
        length = 4
        for idx in range(len(is_null)):
            if is_null[idx]:
        ...
        key_values, _, _, _, _ = _convert_col_native(schema, cols, col_idx + 1, data_col_idx + 1, sum(column_lens), as_utf8, key_buf, key_is_nulls, ctx_tz, prefix=prefix + '  ')
        value_col = cols[(data_col_idx + 2)]
        value_buf, value_is_nulls = _prepare_column_native(schema[(col_idx + 2)], value_col)
        value_values, _, _, skip_cols, skip_data_cols = _convert_col_native(schema, cols, col_idx + 2, data_col_idx + 2, sum(column_lens), as_utf8, value_buf, value_is_nulls, ctx_tz, prefix=prefix + '  ')
        column = [
         None] * num_records
        map_offset = 0
        for idx in range(len(column_lens)):
        ...
        return (
         column, 1, any_nulls, skip_cols + 2, skip_data_cols + 2)
    raise RuntimeError('Unsupported type: %s' % TTypeId._VALUES_TO_NAMES[schema[col_idx].type.type_id])
...
def _convert_col(schema, cols, col_idx, num_records, as_utf8, buf, is_null, ctx_tz):
    ...
    if schema[col_idx].type.type_id == TTypeId.STRING or schema[col_idx].type.type_id == TTypeId.VARCHAR:
        ...
        if as_utf8:
            ...
            return (
             numpy.array(column, dtype=object), 1, any_nulls)
    else:
        if schema[col_idx].type.type_id == TTypeId.CHAR:
            off = 0
            column = [numpy.nan] * num_records
            ...
            if as_utf8:
                return (
                 [c.decode('utf-8') if isinstance(c, bytes) else c for c in column], 1, any_nulls)
            else:
                return (
                 numpy.array(column, dtype=object), 1, any_nulls)
        else:
            if schema[col_idx].type.type_id == TTypeId.BOOLEAN:
                return (numpy.frombuffer(buf, dtype=numpy.bool), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.TINYINT:
                return (numpy.frombuffer(buf, dtype=numpy.int8), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.SMALLINT:
                return (numpy.frombuffer(buf, dtype=numpy.int16), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.INT:
                return (numpy.frombuffer(buf, dtype=numpy.int32), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.BIGINT:
                return (numpy.frombuffer(buf, dtype=numpy.int64), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.FLOAT:
                return (numpy.frombuffer(buf, dtype=numpy.float32), 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.DOUBLE:
            ...
            if schema[col_idx].type.type_id == TTypeId.DECIMAL:
                ...
                return (
                 column, 1, any_nulls)
            if schema[col_idx].type.type_id == TTypeId.ARRAY:
                raise RuntimeError('Unsupported type: ' + TTypeId._VALUES_TO_NAMES[schema[col_idx].type.type_id])
            else:
                raise RuntimeError('Unsupported type: ' + TTypeId._VALUES_TO_NAMES[schema[col_idx].type.type_id])


def _columnar_batch_to_python(schema, columnar_records, num_records, ctx_tz=pytz.utc, strings_as_utf8=False, output_format=TRecordFormat.ColumnarNumPy):
    assert_numpy_installed()
    ...
    while col_idx < len(schema_cols):
        ...
        if output_format == TRecordFormat.ColumnarNumPy:
            ...
            val, idx, any_null = _convert_col(schema_cols, cols, col_idx, num_records, strings_as_utf8, buf, is_null, ctx_tz)
        else:
            buf, is_null = _prepare_column_native(schema_cols[col_idx], cols[data_col_idx])
            val, idx, any_null, skip_cols, skip_data_cols = _convert_col_native(schema_cols, cols, col_idx, data_col_idx, num_records, strings_as_utf8, buf, is_null, ctx_tz)
            col_idx += skip_cols
    ...
    return (
     col_names, data, any_nulls, is_nulls)


def context(application_name=None, namespace='okera', dialect=None):
    ...
    return OkeraContext(application_name, namespace, dialect=dialect)
...
class ScanTask(BaseBackgroundTask):
    ...
    def __call__(self):
        ...
        with self.ctx._connect_worker(self.plan_hosts, None, options=self.options) as (worker):
            try:
                try:
                    handle, schema = worker.exec_task(self.task, self.max_records, self.output_format)
                    while 1:
                        fetch_result = worker.fetch(handle)
                        assert fetch_result.record_format == self.output_format
                        if fetch_result.num_records:
                            t_results = self.deserialize(schema, fetch_result.columnar_records, fetch_result.num_records, self.output_format)
                            if t_results:
    ...
    def deserialize(self, schema, columnar_records, num_records, output_format):
        ...
        raise Exception('Invalid invocation of an abstract function: ' + 'BaseBackgroundTask::deserialize')


class JsonScanTask(ScanTask):

    def __init__(self, ctx, plan_hosts, task, max_records, strings_as_utf8, output_format):
        ScanTask.__init__(self, 'JsonScanTask', ctx, plan_hosts, task, max_records, None, output_format)
        self._JsonScanTask__strings_as_utf8 = strings_as_utf8

    @staticmethod
    def __fixup_datum(datum):
        if isinstance(datum, datetime.datetime):
            ...
            if isinstance(datum, dict):
                for key, value in datum.items():
                    datum[key] = JsonScanTask._JsonScanTask__fixup_datum(value)

                return datum
            if isinstance(datum, list):
                return [JsonScanTask._JsonScanTask__fixup_datum(child) for child in datum]
    ...
    def deserialize(self, schema, columnar_records, num_records, output_format):
        assert_numpy_installed()
        col_names, data, _, is_nulls = _columnar_batch_to_python(schema, columnar_records, num_records, self.ctx.get_timezone(), self._JsonScanTask__strings_as_utf8, output_format)
        ...
        for r in range(0, num_records):
            ...
            for c in range(0, num_cols):
                if is_nulls[c][r]:
                    ...
                    datum = data[c][r]
                    row[c] = JsonScanTask._JsonScanTask__fixup_datum(datum)
...
class PandasScanTask(ScanTask):
    ...
    def __init__(self, ctx, plan_hosts, task, max_records, options, strings_as_utf8, output_format):
        ScanTask.__init__(self, 'PandasScanTask', ctx, plan_hosts, task, max_records, options, output_format)
        self._PandasScanTask__strings_as_utf8 = strings_as_utf8

    def deserialize(self, schema, columnar_records, num_records, output_format):
        assert_numpy_installed()
        assert_pandas_installed()
        result = []
        col_names, data, any_nulls, is_nulls = _columnar_batch_to_python(schema, columnar_records, num_records, self.ctx.get_timezone(), self._PandasScanTask__strings_as_utf8, output_format)
        df = pandas.DataFrame(OrderedDict(zip(col_names, data)))
        if len(df):
            for c in range(0, len(col_names)):
                if not not any_nulls[c]:
                    if df[col_names[c]].dtype == 'object':
                        pass
                    else:
...
class OkeraFsStream:
    ...
    def __init__(self, planner, tbl, delimiter=',', quote_strings=True):
        ...
        self.status = 200
        self.headers = {}
        self.data = planner.scan_as_pandas(tbl, max_task_count=1, strings_as_utf8=True).to_csv(None, header=False, index=False)
