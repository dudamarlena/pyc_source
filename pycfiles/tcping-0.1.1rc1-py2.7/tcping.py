...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tcping.py
# Compiled at: 2017-04-12 06:16:19
"""
...
class Print(object):
    ...
    def raw(self):
        ...
        for row in self.rows:
            ...
            statistics_header = ('\n--- {}[:{}] tcping statistics ---').format(row.host, row.port)
            statistics_body = ('\n{} connections, {} successed, {} failed, {} success rate').format(total, row.successed, row.failed, row.success_rate)
            statistics_footer = ('\nminimum = {}, maximum = {}, average = {}').format(row.minimum, row.maximum, row.average)
...
class Ping(object):
    ...
    def statistics(self, n):
        ...
        self.print_.add_statistics(Statistics(self._host, self._port, self._successed, self._failed, success_rate, minimum, maximum, average))
