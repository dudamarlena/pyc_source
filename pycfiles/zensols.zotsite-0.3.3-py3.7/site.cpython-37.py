...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/zotsite/site.py
...
from zensols.zotsite import AppConfig, DatabaseReader, NavCreateVisitor, RegexItemMapper, IdItemMapper, FileSystemCopyVisitor, PruneVisitor, Library, PrintVisitor, Walker, BetterBibtexVisitor, UnsortedWalker, SortedWalker
logger = logging.getLogger(__name__)

class SiteCreator(object):
    __doc__ = 'Create the Zotero content web site.\n\n    '
    ...
    def walker(self) -> Walker:
        walker = {'none':'UnsortedWalker',  'case':'SortedWalker'}.get(self.sort)
        if walker is None:
    ...
    def library(self) -> Library:
        ...
        if name_pat is not None:
            ...
            match_children = self.config.get_option_boolean('match_children',
              expect=False)
    ...
    def _write_meta(self, fname):
        ...
        if hasattr(self.config, 'pkg'):
            pkg = self.config.pkg
            meta = {'version':pkg.version,  'project_name':pkg.project_name}
        else:
            meta = {'version':'<none>', 
             'project_name':'<none>'}
