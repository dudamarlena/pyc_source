...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/zotsite/navvisitor.py
# Compiled at: 2020-04-25 23:55:42
# Size of source mod 2**32: 5167 bytes
import logging, re
from zensols.zotsite import Visitor, ItemMapper, Item, Note, ZoteroObject
logger = logging.getLogger(__name__)

class NavCreateVisitor(Visitor):
    __doc__ = 'This class creates the data structure used by the Javascript navigation\n    widget in the created website.\n\n    '
    ITEM_ICONS = {'computerProgram':'floppy-disk',  'conferencePaper':'pencil', 
     'journalArticle':'file', 
    ...
    def _find_child_resource(self, item: Item, pat: re.Pattern):
        res = tuple(filter(lambda p: p is not None and pat.match(p), map(lambda c: self.itemmapper.get_resource_name(c), item.children)))
        if len(res) == 1:
            return res[0]

    def _find_child_name(self, item: Item, pat: re.Pattern):
        res = tuple(filter(lambda p: p is not None and pat.match(p), map(lambda c: c.name, item.children)))
        if len(res) > 0:
