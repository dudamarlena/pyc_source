...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/zensols/zotsite/domain.py
# Compiled at: 2020-04-25 23:55:42
# Size of source mod 2**32: 7504 bytes
import logging
...
class ZoteroObject(ABC):
    __doc__ = 'Represents any collection, item etc. Zotero data object.\n\n    '
...
class Note(ZoteroObject):
    __doc__ = 'Represents a note Zotero data object.\n\n    '
...
class Item(ZoteroObject):
    __doc__ = 'Represents an attachement object, like PDFs, site links etc.\n\n    '
    ...
    def path(self):
        ...
        if path:
            ...
            if m is None:
                abs_path = Path(path)
                assert abs_path.exists(), f"unknown storage and not a file: {path}"
            else:
                pdir = self.sel['key']
                fpart = m.group(1)
...
class Container(ZoteroObject):
    __doc__ = 'Container class holds items and sub-collections.\n\n    '
...
class Collection(Container):
    __doc__ = 'Represents a (sub)collection, which is a container for other collections and\n    items.\n\n    '
...
class Library(Container):
    __doc__ = 'Represents the top level object that contains the root level collections.\n\n    '
...
class PrintVisitor(Visitor):
    ...
    def enter_parent(self, parent: ZoteroObject):
        print(f"{' ' * (self.depth * 4)}{str(parent)} " + f"({parent.__class__.__name__})")
        self.depth += 1
