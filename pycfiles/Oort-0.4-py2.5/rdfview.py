...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/oort/rdfview.py
# Compiled at: 2007-10-06 15:13:43
...
class Selector(object):
    __slots__ = ('predicate', '_namespace', 'filters', '_subQueryMarker', '_finalSubQuery',
                 '_name', '_queryClass')
    ...
    def retreive_result(self, rdfQueryInstance, selectArgs):
        result = self._process_for_subqueries(rdfQueryInstance, self.select(*selectArgs), *selectArgs[:2])
        return result

    def _process_for_subqueries(self, rdfQueryInstance, rawresults, graph, lang):
    ...
    def __set__(self, rdfQueryInstance, value):
        ...
        if sub:
            if isinstance(value, list):
                prep.result = [ sub.from_dict(val, lang, BNode()) for val in value ]
            else:
...
class RdfQuery(object):
    ...
    def to_graph(self, lgraph=None):
        subject = self._subject or BNode()
        if not subject:
            return
    ...
    def to_dict(self, keepSubject=False):
        ...
        for selector in self._selectors.values():
            ...
            if isinstance(value, dict):
                d[name] = dict([ (key, self.__dict_convert(val, keepSubject)) for (key, val) in value.items()
                               ])
            elif hasattr(value, '__iter__'):
                d[name] = [ self.__dict_convert(val, keepSubject) for val in value ]
            else:
...
class collection(Selector):
    ...
    def select(self, graph, lang, subject):
        if self.multiple:
            allItems = [ graph.items(res) for res in graph.objects(subject, self.predicate) ]
            return list(chain(*allItems))
        else:
            return list(graph.items(graph.value(subject, self.predicate, None, any=True)))
        return
...
try:
    ...
    class localized_xml(UnarySelector):
        ...
        def select(self, graph, lang, subject):
            return language_filtered_xml(graph.objects(subject, self.predicate), lang)
...
class each_localized(TypeLocalized, EachSelector):
    ...
    def select(self, graph, lang, subject):
        return [ value for value in graph.objects(subject, self.predicate) if value.language == lang
               ]
...
class QueryContext(object):
    ...
    class PreparedQuery(object):
        ...
        def find_by(self, **kwargs):
            cx = self.context
            return self.query.find_by(cx._graph, cx._get_lang(), execCache=cx._execCache, **kwargs)