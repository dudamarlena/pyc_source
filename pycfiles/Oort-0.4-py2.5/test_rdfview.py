...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/oort/test/test_rdfview.py
# Compiled at: 2007-10-07 09:27:30
from nose.tools import assert_equals
from rdflib import ConjunctiveGraph as Graph, URIRef, Literal, BNode, Namespace, RDF
from oort.rdfview import RdfQuery, one, each, one_where_self_is, each_where_self_is, collection, localized, i18n_dict, each_localized, localized_xml, Sorter, Filter, run_queries, THIS_QUERY, selector
...
class TestItem:
    ...
    def test_one(self):
        assert self.item.name == 'Item X'

    def test_one_relation(self):
        assert self.item.unaryRelation.name == 'One Relation'
    ...
    def test_i18n_dict(self):
        ...
        assert_all_equals(titleLangs.values(), ['Example Item', 'Exempelsak'])


def test_localized():
    for (l, v) in [(en, 'Example Item'), (sv, 'Exempelsak')]:
        yield (
         assert_equals, Item(testgraph, l, itemX).title, Literal(v, l))


def test_each_localized():
    assert_all_equals(Item(testgraph, en, itemX).labels, ['En', 'eN'])
    assert_all_equals(Item(testgraph, sv, itemX).labels, ['Sv', 'sV'])
...
def test_localized_xml():
    for (l, v) in [(en, 'XML Data'), (sv, 'XML-data')]:
...
class CustomItem(Item):
    ...
    def randomRelation(self, testgraph, lang, subject):
        ...
        itemUris = [ uri for uri in testgraph.subjects(RDF.type, T.Item) if uri != subject
                   ]
...
class FilteredItem(RdfQuery):
    ...
    sortedRelations_attr = each(T.relation) >> Item | Sorter('name', reverse=True)
    sortedRelations_func = each(T.relation) >> Item | Sorter(lambda r: getattr(r, 'name'), reverse=True)
...
def test_filter():
    ...
    def assert_sortedrels(rels):
        assert [
         'Related 2', 'Related 1'] == [ it.name for it in rels ]
...
def test_from_dict():
    data = dict(name='Item X', title='Example Item', titleLang={'en': 'Example Item', 'sv': 'Exempelsak'}, labels=[
     'En', 'eN'], relations=[{'name': 'Related 1'}, {'name': 'Related 2'}], xmlData='<div/>')
...
def test_bound_with():
    ...
    yield (assert_equals, bound(testgraph, en, BNode()).name, 'Item X')
    bound = Item.bound_with(itemX, sv)
    yield (assert_equals, bound(testgraph, None, BNode()).title, Literal('Exempelsak', sv))
    return


def test_find_by():
    found = list(Item.find_by(testgraph, en, name=Literal('Item X')))
