...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/resonate/visual_timeline.py
# Compiled at: 2018-12-20 13:45:50
# Size of source mod 2**32: 13766 bytes
from datetime import datetime
import numpy as np, pandas as pd, plotly.offline as py
...
def consolidate_data(detections):
    ...
    detections = detections.groupby([
     'catalognumber', 'date', 'station'],
      as_index=False).agg({'det_counts':'count', 
     'latitude':'mean', 
     'longitude':'mean'})[[
     'catalognumber',
     'date',
    ...
    catalognumbers = pd.DataFrame((detections.catalognumber.unique()),
...
def create_trace(detections, total_grid, is_mapbox=False, colorscale='Rainbow'):
    """
    ...
    """
    trace = dict(lon=(total_grid[('x-' + str(detections.date.min().date()))].dropna()),
      lat=(total_grid[('y-' + str(detections.date.min().date()))].dropna()),
      text=(total_grid[('catalognumber-' + str(detections.date.min().date()))].dropna()),
      mode='markers',
      hoverinfo='lon+lat+text',
      marker=dict(size=(total_grid[('size-' + str(detections.date.min().date()))].dropna()),
      color=(total_grid[('color-' + str(detections.date.min().date()))].dropna()),
...
def create_frames(detections, total_grid, is_mapbox=False):
    ...
    for date in pd.date_range(detections.date.min(), detections.date.max()):
        ...
        if 'x-' + str(date) in total_grid.columns:
            frame_trace = dict(lon=(total_grid[('x-' + str(date))].dropna()),
              lat=(total_grid[('y-' + str(date))].dropna()),
              text=(total_grid[('catalognumber-' + str(date))].dropna()),
              mode='markers',
              hoverinfo='lon+lat+text',
              marker=dict(size=(total_grid[('size-' + str(date))].dropna()),
              color=(total_grid[('color-' + str(date))].dropna())))
        else:
            frame_trace = dict(lon=[
             0],
              lat=[
             0],
              text=[
...
def define_updatemenus(animation_time=1000, transition_time=300):
    ...
    updatemenus = dict(type='buttons',
     ...
     dict(method='animate',
       ...
       args=[
      None,
      dict(frame=dict(duration=animation_time, redraw=False),
        fromcurrent=True,
...
def define_layout(detections, title, plotly_geo=None, mapbox_token=None, style='light'):
    """
    ...
    if mapbox_token is None:
        ...
        plotly_geo.update(center=dict(lon=(detections.longitude.mean()),
          lat=(detections.latitude.mean())),
          lonaxis=dict(range=[
         detections.longitude.min(), detections.longitude.max()]),
          lataxis=dict(range=[
         detections.latitude.min(), detections.latitude.max()]))
        layout = dict(geo=plotly_geo,
          title=title,
          hovermode='closest',
          showlegend=False)
    else:
        mapbox = dict(accesstoken=mapbox_token,
          center=dict(lon=(detections.longitude.mean()),
          lat=(detections.latitude.mean())),
          zoom=5,
          style=style)
...
def timeline(detections, title='Timeline', height=700, width=1000, ipython_display=True, mapbox_token=None, plotly_geo=None, animation_time=1000, transition_time=300, slider_transition_time=300, colorscale='Rainbow', style='light'):
    """
    ...
    if mapbox_token is None:
        ...
        trace = create_trace(detections, total_grid, is_mapbox=True,
          colorscale=colorscale)
        frames = create_frames(detections, total_grid, is_mapbox=True)
    updatemenus = define_updatemenus(animation_time, transition_time)
    sliders = define_sliders(detections, animation_time, slider_transition_time)
    layout = define_layout(detections, title, mapbox_token=mapbox_token, style=style)
    layout.update(dict(updatemenus=[
     updatemenus],
      sliders=[
     sliders]))
    ...
    if ipython_display:
        ...
        fig = {'data':[
          trace], 
         'layout':layout,  'frames':frames}
        return py.plot(fig, filename=('{}.html'.format(title.lower().replace(' ', '_'))))