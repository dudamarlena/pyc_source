...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/resonate/residence_index.py
# Compiled at: 2018-12-20 13:45:50
# Size of source mod 2**32: 13439 bytes
import math
from datetime import datetime
import pandas as pd, plotly.graph_objs as go, plotly.offline as py, resonate.compress as cp
...
def total_days_count(detections):
    ...
    *** NOTE ****
    ...
    """
    detections['startdate'] = detections['startdate'].apply((datetime.strptime),
      args=('%Y-%m-%d %H:%M:%S', )).apply(datetime.date)
    detections['enddate'] = detections['enddate'].apply((datetime.strptime),
      args=('%Y-%m-%d %H:%M:%S', )).apply(datetime.date)
...
def aggregate_total_no_overlap(detections):
    ...
    """
    total = pd.Timedelta(0)
    detections = detections.sort_values(by='startdate',
...
def get_days(dets, calculation_method='kessel'):
    ...
    if calculation_method == 'aggregate_with_overlap':
        ...
        if calculation_method == 'aggregate_no_overlap':
            days = aggregate_total_no_overlap(dets)
        else:
            if calculation_method == 'timedelta':
...
def plot_ri(ri_data, ipython_display=True, title='Bubble Plot', height=700, width=1000, plotly_geo=None, filename=None, marker_size=6, scale_markers=False, colorscale='Viridis', mapbox_token=None):
    """
    plot_ri
    ...
    if mapbox_token is not None:
        ...
        mapbox = dict(accesstoken=mapbox_token,
          center=dict(lon=(ri_data.longitude.mean()),
        ...
        data = [
         {'lon':ri_data.longitude.tolist(), 
          ...
          'mode':'markers', 
          'marker':{'color':ri_data.residency_index.tolist(), 
           'size':marker_size, 
           'showscale':True, 
           'colorscale':colorscale, 
           'colorbar':{'title': 'Detection Count'}}, 
          'type':map_type}]
        ...
        plotly_geo.update(center=dict(lon=(ri_data.longitude.mean()),
          lat=(ri_data.latitude.mean())),
          lonaxis=dict(range=[
         ri_data.longitude.min(), ri_data.longitude.max()]),
          lataxis=dict(range=[
         ri_data.latitude.min(), ri_data.latitude.max()]))
...
def residency_index(detections, calculation_method='kessel'):
    ...
    for station in dets['station'].unique():
        ...
        station_dict = {'station':station, 
         'days_detected':total,  'residency_index':total / float(total_days),  'longitude':location['longitude'].values[0], 
         'latitude':location['latitude'].values[0]}
        station_list.append(station_dict)

    all_stations = pd.DataFrame(station_list)
    all_stations = all_stations.sort_values(by='days_detected',
      ascending=False).reset_index(drop=True)
    print('OK!')
    return all_stations