...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/resonate/filters.py
# Compiled at: 2019-06-05 10:11:35
# Size of source mod 2**32: 10452 bytes
...
def get_distance_matrix(detections):
    ...
    dist_mtx = pd.DataFrame((np.zeros(len(stn_locs) ** 2).reshape(len(stn_locs), len(stn_locs))),
      index=(stn_locs.index),
      columns=(stn_locs.index))
    for cstation in dist_mtx.columns:
        for rstation in dist_mtx.index:
            cpoint = (
             stn_locs.loc[(cstation, 'latitude')],
...
def filter_detections(detections, suspect_file=None, min_time_buffer=3600, distance_matrix=False):
    """
    ...
    if suspect_file:
        ...
        if mandatory_columns.issubset(detections.columns):
            ...
            detections.loc[:, 'datecollected'] = pd.to_datetime(detections['datecollected'])
            ...
            for anm in ind:
                anm_dets = grouped.get_group(anm).sort_values('datecollected',
                  ascending=True)
            ...
            susp_dets = detections[(~detections['unqdetecid'].isin(good_dets['unqdetecid']))]
        else:
    ...
    output_dict = {'filtered':good_dets, 
     'suspect':susp_dets}
    if distance_matrix:
        ...
        if dm_mandatory_columns.issubset(detections.columns):
            ...
            raise GenericException('Missing required input columns for                 distance matrix calc: {}'.format(dm_mandatory_columns - set(detections.columns)))
    return output_dict
...
def distance_filter(detections, maximum_distance=100000):
    ...
    if mandatory_columns.issubset(detections.columns):
        ...
        filtered_detections['filtered'] = distance_df[((distance_df.lag_distance_m <= maximum_distance) & (distance_df.lead_distance_m <= maximum_distance))].reset_index(drop=True)
        filtered_detections['suspect'] = distance_df[((distance_df.lag_distance_m > maximum_distance) | (distance_df.lead_distance_m > maximum_distance))].reset_index(drop=True)
...
def velocity_filter(detections, maximum_velocity=10):
    ...
    if mandatory_columns.issubset(detections.columns):
        ...
        filtered_detections = dict()
        filtered_detections['filtered'] = vel_df[((vel_df.lag_velocity <= maximum_velocity) & (vel_df.lead_velocity <= maximum_velocity))].reset_index(drop=True)
        filtered_detections['suspect'] = vel_df[((vel_df.lag_velocity > maximum_velocity) | (vel_df.lead_velocity > maximum_velocity))].reset_index(drop=True)
        return filtered_detections
    raise GenericException('Missing required input columns: {}'.format(mandatory_columns - set(detections.columns)))