...
# [GCC 8.4.0]
# Embedded file name: /home/nwinter/PycharmProjects/projects_nils/photon_core/photonai/helper/helper.py
# Compiled at: 2019-11-21 09:20:09
# Size of source mod 2**32: 8800 bytes
...
class PhotonPrintHelper:
    ...
    def _optimize_printing(pipe, config: dict):
        ...
        for el_key, el_value in config.items():
            ...
            if name in pipe.named_steps:
                new_pretty_key = '    ' + name + '->'
                prettified_config.append(new_pretty_key + pipe.named_steps[name].prettify_config_output(rest, el_value) + '\n')
            else:
    ...
    def config_to_human_readable_dict(pipe, specific_config):
        ...
        for el_key, el_value in specific_config.items():
            ...
            rest = '__'.join(items[1:])
            if name in pipe.named_steps:
                if name not in prettified_config:
...
class PhotonDataHelper:
    ...
    def split_data(X, y, kwargs, start=0, stop=1, indices=None):
        ...
        return (
         X_batched, y_batched, kwargs_dict_batched)
    ...
    def stack_data_horizontally(existing_array, new_array):
        ...
        * `b` [ndarray]:
        ...
        """
        if not existing_array is None:
            if isinstance(existing_array, np.ndarray):
