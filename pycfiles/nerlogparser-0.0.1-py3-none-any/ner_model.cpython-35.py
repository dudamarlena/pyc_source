...
# [GCC 8.4.0]
# Embedded file name: /home/hudan/Git/nerlogparser/nerlogparser/model/ner_model.py
# Compiled at: 2019-01-06 22:50:54
...
class NERModel(BaseModel):
    __doc__ = 'Specialized class of Model for NER'
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope('words'):
            if self.config.embeddings is None:
                ...
                _word_embeddings = tf.get_variable(name='_word_embeddings', dtype=tf.float32, shape=[
                 self.config.nwords, self.config.dim_word])
            else:
        ...
        with tf.variable_scope('chars'):
            if self.config.use_chars:
                _char_embeddings = tf.get_variable(name='_char_embeddings', dtype=tf.float32, shape=[
                 self.config.nchars, self.config.dim_char])
                char_embeddings = tf.nn.embedding_lookup(_char_embeddings, self.char_ids, name='char_embeddings')
                s = tf.shape(char_embeddings)
                char_embeddings = tf.reshape(char_embeddings, shape=[
                 s[0] * s[1], s[(-2)], self.config.dim_char])
                ...
                output = tf.reshape(output, shape=[
                 s[0], s[1], 2 * self.config.hidden_size_char])
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope('proj'):
            W = tf.get_variable('W', dtype=tf.float32, shape=[
             2 * self.config.hidden_size_lstm, self.config.ntags])
    ...
    def predict_batch(self, words):
        ...
        if self.config.use_crf:
            ...
            return (
             viterbi_sequences, sequence_lengths)
        else:
