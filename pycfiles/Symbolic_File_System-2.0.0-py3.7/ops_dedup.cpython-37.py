...
constants = {'DEDUP_FILE_EXTENSION': '.dedup'}
commands = {'FIND_DUPS':'find-dups', 
 'DEDUP':'dedup'}
messages = {'FIND_DUPS':{'HELP':'Find duplicates and save them as a JSON file in the target directory', 
  'HELP_OPT':{'PATH':'Path of the the target directory in the SFS to be de-duplicated. Defaults to the current working directory', 
  ...
  'OUTPUT':{'NO_DUPLICATES':'No duplicates found', 
   'DUPLICATE_COUNT':'Duplicates found: ', 
   'JSON_PATH':'Duplicates have been saved to JSON file: '}}, 
 'DEDUP':{'HELP':'Delete duplicates using the JSON file generated with command "{}"'.format(commands['FIND_DUPS']), 
  'HELP_OPT':{'PATH':'Path of the the target directory in the SFS to be de-duplicated. Defaults to the current working directory', 
...
def _dedup_ops_cli(parser, parents=()):
    dedup_json = parser.add_parser((commands['FIND_DUPS']),
      parents=parents,
      help=(messages['FIND_DUPS']['HELP']))
    dedup_json.add_argument('path', nargs='?', help=(messages['FIND_DUPS']['HELP_OPT']['PATH']))
    dedup_json.add_argument('-o',
      '--override', action='store_true', help=(messages['FIND_DUPS']['HELP_OPT']['OVERRIDE']))
    dedup_json.add_argument('-d',
      '--del-duplicates', action='store_true', help=(messages['FIND_DUPS']['HELP_OPT']['OVERRIDE']))
    dedup = parser.add_parser((commands['DEDUP']),
      parents=parents,
...
def find_dups(sfs, target_dir, keep='all'):
    ...
    return [[DuplicateLink((os.path.relpath(lnk.path, target_dir)), source_path=source_path, size=(stats.size), ctime=(stats.ctime), keep=0 if (keep == 'first' and i > 0) else 1) for i, (lnk, source_path, stats) in enumerate(node_dict[name][size])] for name in sorted(dups) for size in sorted(dups[name])]
...
def get_json_path(target_dir):
    ...
    return os.path.join(target_dir, os.path.basename(target_dir) + constants['DEDUP_FILE_EXTENSION'] + core.constants['SFS_FILE_EXTENSION'])