...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sfs/ops/ops_main.py
# Compiled at: 2018-12-31 02:53:27
...
messages = {'INIT':{'HELP':'Initialize a new SFS in the current directory. The current directory must be empty', 
  'ERROR':{'NON_EMPTY_DIR':'SFS can only be initialized in an empty directory', 
   'NESTED_SFS':'Current directory is inside another SFS'}}, 
 'IS_SFS':{'HELP':'Check whether a path is inside an SFS', 
  'HELP_OPT':{'PATH': 'Path to check (defaults to current working directory)'}, 
  'OUTPUT':{'YES':'Yes. SFS Root: ', 
   'NO':'No'}}}
commands = {'SFS_INIT':'init', 
 'IS_SFS':'is-sfs'}

@events.subscriber(events.events['CLI_REGISTRY'])
def _sfs_ops_cli(parser, parents=()):
    parser.add_parser((commands['SFS_INIT']),
      parents=parents,
      help=(messages['INIT']['HELP']))
    is_sfs = parser.add_parser((commands['IS_SFS']),
      parents=parents,
      help=(messages['IS_SFS']['HELP']))
    is_sfs.add_argument('path',
      nargs='?', help=(messages['IS_SFS']['HELP_OPT']['PATH']))
...
def _is_sfs(args):
    ...
    log.cli_output('{}{}'.format(messages['IS_SFS']['OUTPUT']['YES'], sfs.root) if sfs is not None else messages['IS_SFS']['OUTPUT']['NO'])