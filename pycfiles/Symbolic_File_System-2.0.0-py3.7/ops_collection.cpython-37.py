...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sfs/ops/ops_collection.py
# Compiled at: 2018-12-23 14:44:42
...
messages = {'ADD_COL':{'HELP':'Add a directory (aka collection) to the current SFS', 
  'HELP_OPT':{'PATH':'Path of the collection to be added to the SFS', 
 ...
 'IS_COL':{'HELP':'Check whether a path is inside an collection', 
  'HELP_OPT':{'PATH': 'Path to or in a collection'}, 
  'OUTPUT':{'YES':'Yes. Collection Root: ', 
   'NO':'No'}, 
  'ERROR':{'NOT_IN_SFS': 'Path is not in an SFS'}}, 
 'LIST_COLS':{'HELP':'List all collections in the current SFS', 
  'OUTPUT':{'COUNT':'No of collections available: ', 
   'COL_NAME':'Collection: ', 
   'COL_ROOT':'Base: ', 
   'NOT_AVAILABLE':'No collections available'}, 
  'ERROR':{'NOT_IN_SFS': 'Path is not in an SFS'}}, 
 'SYNC_COL':{'HELP':'Synchronize latest changes in a collection with the current SFS', 
  'HELP_OPT':{'NAME': 'Collection name'}, 
  'ERROR':{'NOT_IN_SFS':'Path is not in an SFS', 
   'NOT_A_COL_NAME':'Collection not available with the specified name'}, 
  'OUTPUT':{'ADDED':'Number of links added: ', 
   'UPDATED':'Number of links updated: ', 
   'DELETED':'Number of links deleted: '}}, 
 'DEL_COL':{'HELP':'Delete a collection and all associated files from the current SFS', 
  'HELP_OPT':{'NAME': 'Collection name'}, 
  'ERROR':{'NOT_IN_SFS':'Path is not in an SFS', 
   'NOT_A_COL_NAME':'Collection not available with the specified name'}, 
  'OUTPUT':'Number of links deleted: '}, 
 'DEL_ORPHANS':{'HELP':'Delete any orphaned links in the current SFS. An orphaned link is one that is not part of any registered collection', 
  'HELP_OPT':{'PATH': ''}, 
  'ERROR':{'NOT_IN_SFS': 'Path is not in an SFS'}, 
  'OUTPUT':'Number of links deleted: '}}
commands = {'ADD_COL':'add-col', 
 'IS_COL':'is-col', 
 'LIST_COLS':'list-cols', 
 'SYNC_COL':'sync-col', 
 'DEL_COL':'del-col', 
...
def _collection_ops_cli(parser, parents=()):
    add_col = parser.add_parser((commands['ADD_COL']),
      parents=parents,
    ...
    is_col = parser.add_parser((commands['IS_COL']),
      parents=parents,
      help=(messages['IS_COL']['HELP']))
    is_col.add_argument('path',
      help=(messages['IS_COL']['HELP_OPT']['PATH']))
    list_cols = parser.add_parser((commands['LIST_COLS']),
      parents=parents,
      help=(messages['LIST_COLS']['HELP']))
    sync_col = parser.add_parser((commands['SYNC_COL']),
      parents=parents,
      help=(messages['SYNC_COL']['HELP']))
    sync_col.add_argument('name',
      help=(messages['SYNC_COL']['HELP_OPT']['NAME']))
    del_col = parser.add_parser((commands['DEL_COL']),
      parents=parents,
      help=(messages['DEL_COL']['HELP']))
    del_col.add_argument('name',
      help=(messages['DEL_COL']['HELP_OPT']['NAME']))
    del_orphans = parser.add_parser((commands['DEL_ORPHANS']),
      parents=parents,
...
def _is_col(args):
    ...
    log.cli_output('{} {}'.format(messages['IS_COL']['OUTPUT']['YES'], col.base) if col is not None else messages['IS_COL']['OUTPUT']['NO'])
    ...
    if sfs is None:
        ...
        if len(cols) <= 0:
            ...
            for col in sorted((cols.values()), key=(lambda c: c.name)):
                log.cli_output('{}"{}"\t{}"{}"'.format(messages['LIST_COLS']['OUTPUT']['COL_NAME'], col.name, messages['LIST_COLS']['OUTPUT']['COL_ROOT'], col.base))
