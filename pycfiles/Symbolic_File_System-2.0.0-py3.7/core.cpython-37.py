...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sfs/core.py
# Compiled at: 2018-12-24 22:16:23
# Size of source mod 2**32: 12725 bytes
import os, collections, itertools, shutil
import sfs.file_system as fs
import sfs.log_utils as log
constants = {'SFS_DIR':'sfs', 
 'SFS_META_FILE':'meta', 
 'COLLECTION_DIR':'cols', 
...
class SFS:
    ...
    def _save(self):
        ...
        save_dict = {'collections': self.collections}
    ...
    def get_collection_by_name(self, name):
        ...
        if name in self.collections:
            return Collection.form_save_dict(self.collections[name], self.root, os.path.join(SFS.get_collections_dir(self.root), name))
...
class Collection:
    ...
    def update(self):
        ...
        """
        curr_stats = set([os.path.join(self.base, os.path.relpath(f.path, self.stats_base)) for root, files, dirs, links in fs.walk_bfs(self.stats_base) for f in files])
    ...
    def get_stats(self, col_path):
        ...
        if os.path.isfile(meta_path):
            return fs.load_unpickled(meta_path)