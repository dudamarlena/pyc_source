...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sfs/ops/ops_query.py
# Compiled at: 2018-12-31 02:58:59
...
messages = {'QUERY': {'HELP':'Get information about a link or directory in an SFS', 
           'HELP_OPT':{'PATH': 'Path of a link or directory (Defaults to current working directory)'}, 
           ...
           'OUTPUT':{'LINK':{'COL_NAME':'Collection: ', 
             'COL_PATH':'Path: ', 
             'CTIME':'Last changed: ', 
             'SIZE':'File Size: '}, 
...
def _query_ops_cli(parser, parents=()):
    query = parser.add_parser((commands['QUERY']),
      parents=parents,
    ...
    if sfs is None:
        ...
        if os.path.islink(path):
            ...
            if os.path.isdir(path):
                ...
                log.cli_output('{}{}'.format(messages['QUERY']['OUTPUT']['DIR']['SUB_DIRECTORIES'], dir_stats.sub_directories))
            else:
