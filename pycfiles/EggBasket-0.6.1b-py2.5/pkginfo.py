...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/eggbasket/pkginfo.py
# Compiled at: 2008-07-13 16:55:56
"""Helper functions for handling package meta data."""
__all__ = [
 'parse_pkg_info',
 'read_pkg_inforead_pkg_info_from_egg',
 'read_pkg_info_from_tar']
import logging, re, sys, tarfile, zipfile
from os.path import isdir, join
import turbogears as tg
from eggbasket.util import has_extension, get_base_url
_line_rx = re.compile('^([\\w-]+?):\\s?(.*)')
...
def read_pkg_info_from_tar(filename):
    ...
    try:
        pkg_info_filename = (i.name for i in tar if i.name.endswith('egg-info/PKG-INFO')).next()
...
def read_pkg_info_from_egg(filename):
    ...
    if isdir(filename):
        ...
        try:
            ...
            try:
                pkg_info_filename = (name for name in pkg.namelist() if name.endswith('/PKG-INFO')).next()
...
def read_pkg_info(filename):
    ...
    try:
        ...
        elif has_extension(filename, ('.tar', 'tar.bz2', 'tar.gz', 'tar.Z', '.tgz')):
