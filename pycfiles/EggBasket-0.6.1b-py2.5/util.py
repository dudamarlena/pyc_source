...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/eggbasket/util.py
...
__all__ = [
 ...
 'munge_pkg_info',
 'txt2html']
import os, re, string, cherrypy as cp, turbogears as tg
from eggbasket.odict import OrderedDict
try:
    from eggbasket.rest import HTML
    tg.config.update({'has_docutils': True})
except ImportError:
    tg.config.update({'has_docutils': False, 'eggbasket.pkg_desc_format': 'plain'})

_email_rx = re.compile('([-\\w.+]+@[\\w.]+?\\.\\w+)')

def _make_link(s):
    """Turn HTTP URLs and emails adresses into HTML link elements."""
    if _email_rx.search(s):
        return dict(value=s, html=_email_rx.sub('<a href="mailto:\\1">\\1</a>', s))
...
def _rewrite_url(urlfield, info):
    ...
    try:
        ...
        info[urlfield] = _make_link(get_base_url() + tg.url('/package/%s' % package))


def _xform_fieldname(s):
    """Return s with dashes replaced with spaces and every word capitalized."""
    s = s.replace('-', ' ')
...
def get_base_url():
    ...
    if not base_url:
        host = cp.request.headers.get('Host')
        if host:
...
def munge_pkg_info(pkg_info):
    ...
    Turns URLs into links and optionally rewrites homepage/download URLs.
