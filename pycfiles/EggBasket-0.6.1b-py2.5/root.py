...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/eggbasket/controllers/root.py
...
def add_sitevars(vars):
    ...
    vars['sitename'] = tg.config.get('eggbasket.sitename', 'EggBasket Package Index')
    vars['server_version'] = __version__
    vars['has_permission'] = has_permission
...
def get_static_content(name):
    ...
    try:
        ...
        text = _("Could not read static content resource '%s'.") % name
    else:
...
class Root(ErrorCatcher):
    ...
    def upload(self, name, content, *args, **kw):
        """Delegate package uploads to PackageController."""
        return self.package.upload(name, content, *args, **kw)
    ...
    def about(self, package=None):
        ...
        return dict(title=_('About'), heading=_('About the EggBasket Software'), content=content)

    @tg.expose(template='eggbasket.templates.generic')
    def help(self, package=None):
        """Display usage information for the software."""
        content = get_static_content('help.rst')
        return dict(title=_('Help'), heading=_('How to use EggBasket'), content=content)

    @tg.expose(template='eggbasket.templates.login')
    def login(self, forward_url=None, previous_url=None, *args, **kw):
        """Display the login form."""
        if not tg.identity.current.anonymous and tg.identity.was_login_attempted() and not tg.identity.get_identity_errors():
        ...
        return dict(message=msg, previous_url=previous_url, logging_in=True, original_parameters=cp.request.params, forward_url=forward_url)
