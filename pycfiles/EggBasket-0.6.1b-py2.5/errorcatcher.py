...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/eggbasket/controllers/errorcatcher.py
...
import datetime, logging, socket, StringIO, traceback, cherrypy
...
class ErrorCatcher(controllers.RootController):
    ...
    """
    _error_codes = {None: 'Unknown Error', 
       400: '400 - Bad Request', 
    ...
    _error_mail_templates = {None: 'cheetah:.templates.email_unhandled_exception'}
    ...
    def __init__(self, *args, **kw):
        ...
        self.no_email_on = config.get('error_catcher.no_email_on', (401, 403, 404))
    ...
    def cp_on_http_error(self, status, message):
        ...
        try:
            ...
            log.exception("CherryPy %s error (%s) for request '%s'", status, error_msg, url)
            buf = StringIO.StringIO()
            traceback.print_exc(file=buf)
            exc_traceback = buf.getvalue()
            buf.close()
            req = cherrypy.request
            data = dict(error_msg=error_msg, header_info=format_request_headers(req), is_admin=identity.in_group(self.admin_group_name), message=message, request_info=format_request_info(req), server=req.headers.get('host', socket.getfqdn()), status=status, url=url, timestamp=datetime.datetime.now(), traceback=exc_traceback, user_info=format_user_info(identity.current.user))
    ...
    def send_error_page(self, **data):
        ...
        """
        template = self._error_page_templates.get(data['status'], self._error_page_templates.get(None))
    ...
    def send_exception_email(self, **data):
        ...
        if not self.sender_email or not self.admin_email:
            msg = 'Configuration error: could not send error notification because sender and/or admin email address is not set.'
            log.exception(msg)
            raise RuntimeError(msg)
        template = self._error_mail_templates.get(data['status'], self._error_mail_templates.get(None))
        subject = '%(status)s ERROR on server %(server)s' % data
        body = self._render_error_template(template, 'plain', 'text/plain', data)
        if has_turbomail:
            ...
            self._send_email_by_smtp(self.sender_email, self.admin_email, msg.as_string())
        return
    ...
    def _render_error_template(self, template, format='html', content_type='text/html', data={}):
        if ':' in template:
        ...
        return controllers._process_output(data, template, format, content_type, None)