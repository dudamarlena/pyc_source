...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/pydo/drivers/sqliteconn.py
...
_converters = {datetime.datetime: lambda x: mx.DateTime.DateTimeFromTicks(time.mktime(x.timetuple())), 
   ...
   DATE: convert_DATE, TIMESTAMP: convert_TIMESTAMP, BINARY: lambda x: sqlite.Binary(x.value), 
...
class SqliteDBI(DBIBase):
    ...
    def __init__(self, connectArgs, pool=None, verbose=False, initFunc=None):
        if pool and not hasattr(pool, 'connect'):
            pool = ConnectionPool()
        super(SqliteDBI, self).__init__(connectArgs, sqlite.connect, sqlite, pool, verbose, initFunc)
