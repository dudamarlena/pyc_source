...
# [GCC 8.4.0]
# Embedded file name: /home/blooser/anaconda3/lib/python3.7/site-packages/youtubedownloader/download.py
# Compiled at: 2020-05-13 11:06:52
# Size of source mod 2**32: 27985 bytes
...
import os.path, pathlib, pickle, youtube_dl, atexit
...
def human_time(time):
    if not time:
        return '00:00'
    time = int(time)
    format = 'hh:mm:ss' if time >= 3600 else 'mm:ss'
...
class DownloadData(QObject):
    ...
    def pack(download_data):
        return {'title':download_data._title, 
...
class DownloadProgress(QObject):
    ...
    def pack(download_progress):
        return {'status':download_progress.status, 
...
class DownloadOptions(QObject):
    ...
    TEMPLATES = {'mp4':{'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4'}, 
     'webm':{'format': 'bestvideo[ext=webm]+bestaudio[ext=webm]/webm'}, 
     'mkv':{'format': 'bestvideo[ext=webm]+bestaudio[ext=m4a]/mkv'}, 
     'm4a':{'format': 'bestaudio[ext=m4a]/m4a'}, 
     'flac':{'format':'bestaudio/best', 
      'postprocessors':[
    ...
    def pack(download_options):
        return {'file_format':download_options.file_format, 
...
class DownloadTask(QThread):
    ...
    def __init__(self, url, options):
        ...
        self.download_post_process.started.connect(lambda : self.progress.emit({'status':'converting to {0}'.format(self.options.file_format),  'total_bytes':self.options.post_process_file_size}), Qt.QueuedConnection)
...
class Download(QObject):
    ...
    def check_if_redownload_needed(self):
        if not os.path.isfile(self.destination_file):
            if self.progress.status == 'finished':
