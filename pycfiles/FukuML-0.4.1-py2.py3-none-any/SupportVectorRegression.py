...
# [GCC 8.4.0]
# Embedded file name: /Users/fukuball/Projects/fuku-ml/FukuML/SupportVectorRegression.py
# Compiled at: 2016-12-10 12:07:28
import os, numpy as np, cvxopt, cvxopt.solvers, FukuML.Utility as utility, FukuML.MLBase as ml
...
class SupportVectorRegression(ml.Learner):
    ...
    def load_test_data(self, input_data_file=''):
        ...
        if self.feature_transform_mode == 'polynomial' or self.feature_transform_mode == 'legendre':
            ...
            self.test_X = utility.DatasetLoader.feature_transform(self.test_X, self.feature_transform_mode, self.feature_transform_degree)
        return (
         self.test_X, self.test_Y)
    ...
    def train(self):
        ...
        q = cvxopt.matrix(np.bmat([self.epsilon - self.train_Y, self.epsilon + self.train_Y]).reshape((-1,
                                                                                                       1)))
        ...
        free_sv_lower = np.logical_and(self.alpha_lower > 1e-05, self.alpha_lower < self.C)
