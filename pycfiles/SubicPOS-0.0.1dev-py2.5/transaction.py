...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/subicpos/controllers/transaction.py
...
class TransactionController(ListController):
    ...
    parent = dict(branch=dict(table=model.Branch, column='name'), type=dict(table=model.TransType, column='name'), pay_type=dict(table=model.PayType, column='name'))
    children = dict(trans_item=dict(table=model.TransItem, columns=('item', 'qty',
                                                                    'price'), parent=dict(item=dict(table=model.Item, column='code'))))
    prices = dict()
    total = 0
