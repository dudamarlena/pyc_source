...
# [GCC 8.4.0]
# Embedded file name: /home/roel.vandenberg/dev/lizard-connector/lizard_connector/queries.py
    ...
    def update(self, E=None, *queries, **f):
        ...
        for d in queries:
            try:
                ...
                raise LizardApiImproperQueryError('Query {} is not a dictionary or a string'.format(E))
...
def organisation(organisation_id=None, endpoint=None):
    query_key = {None: '', 
     'organisation': '', 
     'timeseries': 'location__organisation__'}.get(endpoint, 'organisation__') + 'unique_id'
...
def statistics(*statistics):
    statistics = commaify(*[x if x != 'mean' else 'count,sum' for x in statistics])
    return QueryDictionary(min_points=1, fields=statistics)


def feature_info(lat, lng, layername):
    return QueryDictionary(agg='curve', geom='POINT({lng}+{lat})'.format(lat=lat, lng=lng), srs='EPSG:4326', raster_names=layername, count=False)


def limits(layername, south_west, north_east):
    return QueryDictionary(request='getlimits', layers=layername, bbox=bbox(south_west, north_east), width=16, height=16, srs='epsg:4326')
...
def asset(asset, asset_id, endpoint=None):
    assert asset.lower() in ASSETS, 'asset not in asset list: %s' % ', '.join(ASSETS)
