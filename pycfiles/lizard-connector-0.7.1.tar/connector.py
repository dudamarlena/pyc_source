...
# [GCC 8.4.0]
# Embedded file name: /home/roel.vandenberg/dev/lizard-connector/lizard_connector/connector.py
# Compiled at: 2018-04-17 02:26:26
"""
...
import copy, json, getpass, time, sys
...
DEFAULT_API_VERSION = b'3'
ASYNC_POLL_TIME = 1
ASYNC_POLL_TIME_INCREASE = 1.5
ADDITIONAL_ENDPOINTS_V3 = ('raster_aggregates', )
DATA_DETAIL_ENDPOINTS = ('rasters', 'timeseries', 'opticalfibers')
DEFAULT_PARSER = parsers.scientific if parsers.SCIENTIFIC_AVAILABLE else parsers.json

class Connector(object):

    def __init__(self, username=None, password=None, parser=parsers.json, parser_kwargs=None):
        """
        Args:
    ...
    def get(self, url):
        ...
        try:
            json_ = json_.get(b'results', json_)
        finally:
            return json_
    ...
    def perform_request(self, url, data=None):
        ...
        if data:
            ...
            request_obj = urllib_request.Request(url, headers=headers, data=json.dumps(data).encode(b'utf-8'))
        else:
            request_obj = urllib_request.Request(url, headers=self.__header)
        resp = urlopen(request_obj)
        content_type = resp.headers[b'Content-Type']
        if content_type == b'application/json':
            return json.loads(resp.read().decode(b'UTF-8'))
        if b'text' in content_type:
            return resp.read().decode(b'UTF-8')
        return resp.read()
    ...
    def __header(self):
        ...
        """
        if self.use_header:
            return {b'username': self.__username, 
               b'password': self.__password}
        return {}
...
class PaginatedRequest(object):
    ...
    def _next_page(self):
        ...
        self._count = result.get(b'count')
        self.next_url = result.get(b'next')
        result = result.get(b'results', result)
...
class Endpoint(Connector):
    ...
    def __init__(self, endpoint, base=b'https://demo.lizard.net', version=DEFAULT_API_VERSION, data_detail=False, **kwargs):
        """
        Args:
        ...
        base = base.strip(b'/')
        if not base.startswith(b'https') and b'localhost' not in base:
            raise InvalidUrlError(b'base should start with https')
        base_url = urljoin(base, b'api/v') + str(version) + b'/'
        self.base_url = urljoin(base_url, self.endpoint)
        if not self.base_url.endswith(b'/'):
            self.base_url += b'/'
    ...
    def _build_url(self, page_size=1000, *querydicts, **queries):
        q = lizard_connector.queries.QueryDictionary(page_size=page_size, format=b'json')
        ...
        if self.data_detail:
            try:
                ...
                if self._detail_pk:
                    ...
                    raise LizardApiImproperQueryError(b'Missing `uuid` in query parameters.')

            base = urljoin(base, (b'{}/data/').format(uuid))
        elif self._detail_pk:
            base = urljoin(base, (b'{}/').format(self._detail_pk))
        query = b'?' + urlencode(q)
    ...
    def get(self, page_size=1000, parse=True, *querydicts, **queries):
        ...
        if isinstance(result, dict) and bool(result.get(b'next', False)):
            raise LizardApiTooManyResults(b'The Lizard API returns more than one result page. Please \nuse `get_paginated` or `get_async` methods instead for\nlarge api responses. Or increase the  page_size in the\nrequest parameters.')
        if parse:
    ...
    def get_async(self, call_back=None, lock=None, *querydicts, **queries):
        """
        ...
        if lock is None:
            lock = RLock()
        args = (
         call_back, lock) + querydicts
        thread = Thread(target=self._async_worker, args=args, kwargs=queries)
        thread.start()
        return

    def _poll_task(self, task_url, sleep_time=ASYNC_POLL_TIME):
        poll_result = super(Endpoint, self).get(task_url)
        task_status = poll_result.get(b'task_status')
        if task_status == b'PENDING':
            time.sleep(sleep_time)
            return (
             None, True)
        else:
            if task_status == b'SUCCESS':
                url = poll_result.get(b'result_url')
                return (
                 super(Endpoint, self).get(url), False)
    ...
    def _synchronous_get_async(self, *querydicts, **queries):
        queries.update({b'async': b'true'})
        page_size = queries.pop(b'page_size', 0)
        url = self._build_url(page_size=page_size, *querydicts, **queries)
        task_url = super(Endpoint, self).get(url).get(b'url')
        keep_polling = True
        result = None
    ...
    def create(self, uuid=None, sub_endpoint=b'data', **data):
        """
        ...
        """
        if uuid:
            post_url = urljoin(self.base_url, (b'{}/{}/').format(uuid, sub_endpoint))
        else:
...
class Client(Connector):
    ...
    def __init__(self, base=b'https://demo.lizard.net', username=None, password=None, parser=DEFAULT_PARSER, version=DEFAULT_API_VERSION, parser_kwargs=None, **kwargs):
        """
        Args:
            base (str): lizard-nxt url.
            username (str): lizard-api user name to log in. Without one no
        ...
        if username is not None:
            kwargs.update({b'username': username, 
               b'password': password or getpass.getpass()})
        self.base = base
        self.__endpoints = ()
        for endpoint_name in self.endpoints:
            endpoint_params = dict(endpoint=endpoint_name.replace(b'_', b'-'), base=self.base, version=self.api_version, parser=parser, parser_kwargs=parser_kwargs)
            endpoint_params.update(kwargs)
            endpoint = Endpoint(**endpoint_params)
            if endpoint_name in DATA_DETAIL_ENDPOINTS:
                endpoint_params[b'data_detail'] = True
                endpoint.data = Endpoint(**endpoint_params)
            setattr(self, endpoint_name, endpoint)

        super(Client, self).__init__(parser=parser, parser_kwargs=parser_kwargs, **kwargs)
        return

    @property
    def endpoints(self):
        root = Endpoint(base=self.base, endpoint=b'')
        result = root.get(page_size=0, format=b'json')
        endpoints = tuple(sorted(k.replace(b'-', b'_') for k in result.keys()))
        if self.api_version == b'3':
            endpoints = tuple(sorted(endpoints + ADDITIONAL_ENDPOINTS_V3))
        self.__dict__[b'endpoints'] = endpoints
        return endpoints