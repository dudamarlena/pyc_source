...
# [GCC 8.4.0]
# Embedded file name: lizard_connector/queries.py
# Compiled at: 2018-03-13 12:27:43
...
if sys.version_info.major < 3:
    ...
    unicode = str
ASSETS = ('bridge', 'culvert', 'fixeddrainagelevelarea', 'groundwaterstation', 'filter',
          'leveecrossection', 'levee', 'leveesection', 'leveezone', 'manhole', 'measuringstation',
          'monitoringwell', 'opticalfiber', 'orifice', 'outlet', 'overflow', 'parcel',
          'pipe', 'polder', 'pressurepipe', 'pumpeddrainagearea', 'pump', 'pumpstation',
          'sluice', 'wastewatertreatmentplant', 'weirs')
...
class QueryDictionary(dict):
    ...
    def update(self, E=None, *queries, **f):
        ...
        if E and (isinstance(E, str) or isinstance(E, unicode)):
            for k, v in urlparse.parse_qsl(E.strip(b'?')):
                self[k] = v
            ...
            for d in queries:
                try:
                    ...
                    raise LizardApiImproperQueryError((b'Query {} is not a dictionary or a string').format(E))
...
def commaify(*args):
    ...
    """
    return (b',').join(str(x) for x in args)


def wkt_point(lon, lat):
    return (b'').join((b'POINT (', str(lon), b' ', str(lat), b')'))


def wkt_polygon(polygon_coordinates, srid=None):
    points = [ (b' ').join([str(x), str(y)]) for x, y in polygon_coordinates ]
    srid_str = srid + b';' if srid else b''
    return srid_str + b'POLYGON ((' + (b', ').join(points) + b'))'
...
def in_bbox(south_west, north_east, endpoint=None, srid=None):
    ...
    Returns:
        a query dictionary.
    """
    query = b'geom_within' if endpoint == b'timeseries' else b'in_bbox'
...
def datetime_limits(start=None, end=None):
    ...
    start = jsdatetime.datetime_to_js(start) if isinstance(start, datetime.datetime) else start
    end = jsdatetime.datetime_to_js(end) if isinstance(end, datetime.datetime) else end
...
def organisation(organisation_id=None, endpoint=None):
    query_key = {None: b'', 
       b'organisation': b'', 
       b'timeseries': b'location__organisation__'}.get(endpoint, b'organisation__') + b'unique_id'
    org_query = QueryDictionary()
    if isinstance(organisation_id, str):
        org_query.update({query_key: organisation_id})
    elif organisation_id:
        org_query.update({query_key: (b',').join(org for org in organisation_id)})
    return org_query


def statistics(*statistics):
    statistics = commaify(*[ x if x != b'mean' else b'count,sum' for x in statistics
                           ])
    return QueryDictionary(min_points=1, fields=statistics)


def feature_info(lat, lng, layername):
    return QueryDictionary(agg=b'curve', geom=(b'POINT({lng}+{lat})').format(lat=lat, lng=lng), srs=b'EPSG:4326', raster_names=layername, count=False)


def limits(layername, south_west, north_east):
    return QueryDictionary(request=b'getlimits', layers=layername, bbox=bbox(south_west, north_east), width=16, height=16, srs=b'epsg:4326')
...
def asset(asset, asset_id, endpoint=None):
    assert asset.lower() in ASSETS, b'asset not in asset list: %s' % (b', ').join(ASSETS)
    if endpoint == b'timeseries':
        return QueryDictionary(object=b'%s$%d' % (asset.lower(), asset_id))
    if endpoint == b'locations':
        return QueryDictionary(object_type__model=asset.lower(), object_id=asset_id)
