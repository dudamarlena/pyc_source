...
# Compiled at: 2018-04-12 16:32:09
# Size of source mod 2**32: 14811 bytes
"""
...
class Endpoint(Connector):
    ...
    def __init__(self, endpoint, base='https://demo.lizard.net', version=DEFAULT_API_VERSION, **kwargs):
        """
        Args:
    ...
    def _synchronous_get_async(self, *querydicts, **queries):
        ...
        task_url = self.get(*querydicts, **queries).get('url')
        keep_polling = True
        result = None
        while keep_polling:
...
class Client(Connector):
    ...
    def __init__(self, base='https://demo.lizard.net', username=None, password=None, parser=DEFAULT_PARSER, version=DEFAULT_API_VERSION, parser_kwargs=None, **kwargs):
        ...
        for endpoint in self.endpoints:
            setattr(self, endpoint, Endpoint(endpoint=endpoint.replace('_', '-'), base=self.base, version=self.api_version, parser=parser, parser_kwargs=parser_kwargs, **kwargs))

        super(Client, self).__init__(parser=parser, parser_kwargs=parser_kwargs, **kwargs)

    @property
    def endpoints(self):
        if not self._Client__endpoints:
            root = Endpoint(base=self.base, endpoint='')
            result = root.get(page_size=0, format='json')
            self._Client__endpoints = tuple(sorted(k.replace('-', '_') for k in result.keys()))
            if self.api_version == '3':
                self._Client__endpoints = tuple(sorted(self._Client__endpoints + ADDITIONAL_ENDPOINTS_V3))
            return self._Client__endpoints