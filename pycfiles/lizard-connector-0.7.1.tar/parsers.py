...
# [GCC 8.4.0]
# Embedded file name: /home/roel.vandenberg/dev/lizard-connector/lizard_connector/parsers.py
# Compiled at: 2018-04-16 14:55:16
import collections
...
DATA_TYPE_FIELDS = ('events', 'data', 'percentiles')
ScientificResponse = collections.namedtuple('ScientificResponse', ('metadata', 'data'))
...
def __flatten_result(results, parent_key='', sep='__'):
    ...
    return (
     __flatten_dict(results, parent_key=parent_key, sep=sep), events)


def __to_timestamps(dataframe):
    if dataframe is None or dataframe.empty:
        return
    for time_column in (c for c in dataframe.columns if c.endswith('timestamp') or c in ('start',
                                                                                         'end')):
        dataframe[time_column] = pd.to_datetime(dataframe[time_column], unit='ms')
...
def __as_dataframes(results, sep='__', convert_timestamps=True):
    ...
    try:
        ...
        raise ImportError('Trying to convert to pandas Dataframe without pandas. Please install Pandas.')
    ...
    try:
        ...
        return (
         metadata_dataframe, [ f[1] for f in flattened ])

    if not is_event_dict:
        try:
            return (
             metadata_dataframe, [ np.array(flat_data) for _, flat_data in flattened ])
        except NameError:
            raise ImportError('Trying to convert to numpy array without numpy. Please install Numpy.')
    ...
    return (
     metadata_dataframe, event_dataframes)
...
def scientific(results, sep='__', convert_timestamps=True, detail=False):
    ...
    try:
        ...
        return ScientificResponse(*__as_dataframes(results, sep, convert_timestamps))
    except IndexError:
