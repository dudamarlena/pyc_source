...
class Connector(object):
    ...
    def __init__(self, username=None, password=None, parser=parsers.json, parser_kwargs=None):
        ...
        """
        self._Connector__username = username
        self._Connector__password = password
    ...
    def perform_request(self, url, data=None):
        ...
        """
        if data:
            headers = self._Connector__header
            headers['Content-Type'] = 'application/json'
            request_obj = urllib_request.Request(url, headers=headers, data=json.dumps(data).encode('utf-8'))
        else:
            request_obj = urllib_request.Request(url, headers=self._Connector__header)
    ...
    def use_header(self):
        ...
        """
        if self._Connector__username is None or self._Connector__password is None:
            return False
        return True
    ...
    def __header(self):
        ...
        """
        if self.use_header:
            return {'username': self._Connector__username, 
             'password': self._Connector__password}
        return {}
...
class Endpoint(Connector):
    ...
    def __init__(self, endpoint, base='https://demo.lizard.net', version=DEFAULT_API_VERSION, data_detail=False, **kwargs):
        """
        Args:
    ...
    def _build_url(self, page_size=1000, *querydicts, **queries):
        q = lizard_connector.queries.QueryDictionary(page_size=page_size, format='json')
        ...
        if self.data_detail:
            try:
                ...
                if self._detail_pk:
                    ...
                    raise LizardApiImproperQueryError('Missing `uuid` in query parameters.')
    ...
    def get(self, page_size=1000, parse=True, *querydicts, **queries):
        ...
        if isinstance(result, dict):
            if bool(result.get('next', False)):
                raise LizardApiTooManyResults('The Lizard API returns more than one result page. Please \nuse `get_paginated` or `get_async` methods instead for\nlarge api responses. Or increase the  page_size in the\nrequest parameters.')
        if parse:
    ...
    def create(self, uuid=None, sub_endpoint='data', **data):
        ...
        """
        if uuid:
            post_url = urljoin(self.base_url, '{}/{}/'.format(uuid, sub_endpoint))
        else:
...
class Client(Connector):
    ...
    def __init__(self, base='https://demo.lizard.net', username=None, password=None, parser=DEFAULT_PARSER, version=DEFAULT_API_VERSION, parser_kwargs=None, **kwargs):
        """
        Args:
            base (str): lizard-nxt url.
            username (str): lizard-api user name to log in. Without one no
        ...
        self.base = base
        self._Client__endpoints = ()
        for endpoint_name in self.endpoints:
            endpoint_params = dict(endpoint=endpoint_name.replace('_', '-'), base=self.base, version=self.api_version, parser=parser, parser_kwargs=parser_kwargs)
    ...
    def endpoints(self):
        ...
        endpoints = tuple(sorted(k.replace('-', '_') for k in result.keys()))
