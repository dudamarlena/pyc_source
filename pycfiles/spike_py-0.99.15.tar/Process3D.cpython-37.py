...
# [GCC 8.4.0]
# Embedded file name: /home/mad/Documents/spike/spike/v2/Process3D.py
...
def FT3D(audit, p_in_arg, f_in, f_out, inputfilename, outputfilename):
    ...
    audittrail(audit, 'text', 'details', 'from', filein, 'to', fileout, 'number of planes processed', n)
...
def process3DF2F3(audit, filein, fileout, p_in_arg3D, f_in_arg, f_out):
    ...
    L_f_in = change_key_dict('f2', 'f3', change_key_dict('f1', 'f2', f_in_arg))
    L_f_out = {}
    join(filein)
    dim(2)
    ...
    command = 'Process2D.FT2D("mute", L_p_in, L_f_in, L_f_out, "memory", "memory")'
    dict_ = globals()
...
def process3DF1F2(audit, filein, fileout, p_in_arg, f_in_arg, f_out):
    ...
    command = 'FT2D("mute", p_in, f_in, f_out, "memory", "memory")'
    dict_ = globals()
    for i in locals().keys():
        dict_[i] = locals()[i]
...
def process3DF1(audit, filein, fileout, p_in_arg, f_in_arg, f_out):
    ...
    command = 'FT_F1_2D("mute",p_in,f_in,f_out,"memory","memory")'
    dict_ = globals()
    for i in locals().keys():
        dict_[i] = locals()[i]
...
def post_3d(audit, file, p_in_arg, f_in, f_out, location):
    ...
    if key_is_true(p_in, '3D_projections'):
        ...
        audittrail(audit, 'text', 'Projections stored', 'F1 projection files', f_out['F1_projections'], 'F2 projection files', f_out['F2_projections'], 'F3 projection files', f_out['F3_projections'])
        disjoin()