...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tracdeveloper/debugger.py
...
from trac.web import HTTPBadRequest, HTTPNotFound, IRequestFilter, IRequestHandler
...
class TemplateDebugger(Component):
    ...
    def post_process_request(self, req, template, data, content_type):
        if 'debug' not in req.args:
            ...
            for time, key in [ (time, key) for time, key in self._cache_bytime.items() if time < datetime.now() - timedelta(hours=1)
                             ]:
                del self._cache[key]
    ...
    def match_request(self, req):
        match = re.match('/developer/debug(?:/(.*))?$', req.path_info)
        if match:
    ...
    def process_request(self, req):
        ...
        if not header or header.lower() != 'xmlhttprequest':
            add_stylesheet(req, 'common/css/code.css')
            return (
             'developer/debug_help.html', {}, None)
        else:
            path = req.args['path']
            token = req.args['token']
            if not (path is not None and token is not None):
                raise AssertionError
                key = token + ':' + path.split(':', 1)[0]
                data = self._cache.get(key)
                raise (data or HTTPNotFound)()
            node = data.lookup(path)
            data = {'node': node, 'drillable': self._is_drillable(req)}
            output = Chrome(self.env).render_template(req, 'developer/debug_node.html', data, fragment=True)
        ...
        if node.is_collection:
            if isinstance(node.value, (dict, DictMixin)):
                for name, value in sorted(node.value.items()):
                    yield self._add("['%s']" % name, value, '%s.%s' % (node.path, name))

            elif isinstance(node.value, (list, tuple)):
                for idx, value in enumerate(node.value):
                    yield self._add('[%d]' % idx, value, '%s.%d' % (node.path, idx))
...
class ObjectNode(object):
    ...
    COLLECTION_TYPES = (
     dict, DictMixin, list, tuple, set, frozenset)
