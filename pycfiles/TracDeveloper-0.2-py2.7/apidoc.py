...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/tracdeveloper/apidoc.py
...
class APIDocumentation(Component):
    ...
    mimetype_map = {'tracwiki': 'text/x-trac-wiki', 
       'restructuredtext': 'text/x-rst'}

    def match_request(self, req):
        match = re.match('/developer/doc(?:/(.*))?$', req.path_info)
        if match:
    ...
    def process_request(self, req):
        ...
        output = Chrome(self.env).render_template(req, 'developer/apidoc.html', data, fragment=True)
    ...
    def _get_methods(self, req, formatter, cls, exclude_methods=None):
        methods = [ getattr(cls, m) for m in dir(cls) if not m.startswith('_') and m not in (exclude_methods or [])
                  ]
        return [ {'name': m.__name__, 'args': inspect.formatargspec(*inspect.getargspec(m)), 'doc': formatter(req, inspect.getdoc(m))} for m in methods if inspect.ismethod(m)
               ]