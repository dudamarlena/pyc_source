...
# Embedded file name: build/bdist.linux-x86_64/egg/cloudplaya/client.py
# Compiled at: 2012-11-20 00:45:03
import cookielib, json, logging, os, math, random, re, mechanize, requests
...
class Client(object):
    ...
    ALBUM_SEARCH = [
     ('status', 'EQUALS', 'AVAILABLE')]
    PAGINATE_BY = 50
    USER_AGENT = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.142 Safari/535.19'
    ...
    def __init__(self, session_file=None):
        ...
        if not self.session_file:
            if 'APPDATA' in os.environ:
                ...
                logging.warning('Unable to find home directory for .cloudplayarc\n')
                homepath = os.getcwd()
            ...
            if line.startswith('amznMusic.appConfig ='):
                ...
                try:
                    ...
                    try:
                        ...
                        logging.error('Unable to locate key %s in amznMusic.appConfig' % e)
                        return False
                    else:
                        break

        auth_vars['cookies'] = ('; ').join([ '%s=%s' % (cookie.name, cookie.value) for cookie in cookiejar
                                           ])
    ...
    def get_track_list(self, album):
        data = {'sortCriteriaList': '', 
           'maxResults': album.num_tracks, 
           'nextResultsToken': 0, 
           'distinctOnly': 'false', 
           'countOnly': 'false'}
        data.update(self._build_search_criteria(key_prefix='selectCriteriaList', search=[
         ('status', 'EQUALS', 'AVAILABLE'),
         ('trackStatus', 'IS_NULL'),
         (
    ...
    def get_songs(self, search=[], sort=DEFAULT_SONG_SORT, *args, **kwargs):
        for song in self._search_library(return_type='TRACKS', result_cls=Song, result_key='songs', search=(self.SONG_SEARCH + search), columns=Song.COLUMNS, sort=sort, *args, **kwargs):
            yield song

    def get_albums(self, search=[], sort=DEFAULT_ALBUM_SORT, *args, **kwargs):
        for album in self._search_library(return_type='ALBUMS', result_cls=Album, result_key='albums', search=(self.ALBUM_SEARCH + search), columns=Album.COLUMNS, sort=sort, *args, **kwargs):
            yield album
    ...
    def get_album(self, artist_name, album_name):
        ...
        if len(results) > 1:
            logging.error("get_album returned too many results for artist '%s', album '%s'. Returning the first.", artist_name, album_name)
        if results:
            return results[0]
        else:
            return
    ...
    def get_artists(self, search=[], sort=DEFAULT_ARTIST_SORT, *args, **kwargs):
        for artist in self._search_library(return_type='ARTISTS', result_cls=Artist, result_key='artists', search=(self.ARTIST_SEARCH + search), columns=Artist.COLUMNS, sort=sort, *args, **kwargs):
            yield artist

    def _search_library(self, return_type, result_key, result_cls, search=[], columns=[], sort=[]):
        next_results_token = ''
        i = 0
        while 1:
            data = {'searchReturnType': return_type, 
    ...
    def _get(self, operation, data):
        headers = {'x-amzn-RequestId': self._make_request_id(), 'x-cp-token': self.cp_token, 
        ...
        data.update({'Operation': operation, 
           'ContentType': 'JSON', 
        ...
        if r.status_code != 200:
            error = result['Error']
            raise RequestError(error['Message'], error['Code'])
        return result
