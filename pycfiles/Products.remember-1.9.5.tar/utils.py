...
# Embedded file name: build/bdist.macosx-10.3-fat/egg/Products/Relations/utils.py
...
class AllowedTypesByIface:
    ...
    def invokeFactory(self, type_name, id, RESPONSE=None, *args, **kwargs):
        ...
        if not goodType(type_name, self.allowed_interfaces):
            raise ValueError, 'Type %s does not implement any of %s.' % (type_name, self.allowed_interfaces)
...
def getPortalTypesByInterfaces(context, allowedIfaces):
    ...
    for data in tool.listRegisteredTypes():
        ...
        kifaces_z2 = [ str(iface.__name__) for iface in Implements.flattenInterfaces((kifaces,)) ]
        kifaces_z3 = [ str(iface.__name__) for iface in interface.implementedBy(klass).flattened() ]
...
def adddeleteVocab(context, test_only=0, ruleset_ids=None):
    ...
    if not membership.checkPermission(permissions.ModifyPortalContent, context):
        return []
    ...
    for ruleset in rulesets:
        if not mtool.checkPermission('View', ruleset):
            continue
...
def getRelations(context, rulesetnames=[]):
    ...
    for ref in context.getReferenceImpl():
        if ref.relationship in ruleset_ids:
            ...
            result.append({'ruleset': ruleset, 'reference': ref, 'targetobj': ref.getTargetObject(), 'contentobj': ref.getContentObject(), 'actions': ruleset.listActionsFor(ref)})
