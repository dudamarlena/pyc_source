...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bxa/xspec/solver.py
# Compiled at: 2020-01-28 12:31:59
# Size of source mod 2**32: 15995 bytes
"""
...
import xspec, matplotlib.pyplot as plt
...
def store_chain(chainfilename, transformations, posterior):
    ...
    columns = [pyfits.Column(name=name, format='D', array=(t['aftertransform'](posterior[:, i]))) for i, name in enumerate(names)]
    columns.append(pyfits.Column(name='FIT_STATISTIC', format='D',
      array=(posterior[:, -1])))
...
class BXASolver(object):
    ...
    def __init__(self, transformations, prior_function=None, outputfiles_basename='chains/'):
    ...
    def run(self, evidence_tolerance=0.5, n_live_points=400, wrapped_params=None, **kwargs):
        """
                The remainder are multinest arguments (see PyMultiNest and MultiNest documentation!)
                n_live_points: 400 are often enough
        ...
        with XSilence():
            self.results = solve(log_likelihood, self.prior_function, n_dims, paramnames=self.paramnames, 
             outputfiles_basename=self.outputfiles_basename, 
             resume=resume, 
             Lepsilon=Lepsilon, n_live_points=n_live_points, 
             evidence_tolerance=evidence_tolerance, seed=-1, 
             max_iter=0, wrapped_params=wrapped_params, **kwargs)
    ...
    def posterior_predictions_convolved(self, component_names=None, plot_args=None, nsamples=400):
        """
        ...
        if component_names is None:
            ...
            def plot_convolved_components(content):
                ...
                for component in range(ncomponents):
                    ...
                    if component >= len(bands):
                        bands.append(PredictionBand(xmid, shadeargs=dict(color=(kwargs['color'])),
            ...
            self.posterior_predictions_plot(plottype='counts', callback=plot_convolved_components,
              nsamples=nsamples)
    ...
    def posterior_predictions_unconvolved(self, component_names=None, plot_args=None, nsamples=400):
        """
        ...
        self.posterior_predictions_plot(plottype='model', callback=plot_unconvolved_components,
          nsamples=nsamples)
        for band, label in zip(bands, component_names):
            band.shade(alpha=0.5, label=label)
            band.shade(q=0.495, alpha=0.1)
    ...
    def posterior_predictions_plot(self, plottype, callback, nsamples=None):
        ...
        if nsamples is not None:
            if len(posterior) > nsamples:
                if hasattr(numpy.random, 'choice'):
                    chosen = numpy.random.choice((numpy.arange(len(posterior))),
                      replace=False,
                      size=nsamples)
                else:
                    chosen = list(set(numpy.random.randint(0, (len(posterior)), size=(10 * nsamples))))[:nsamples]
...
def standard_analysis(transformations, outputfiles_basename, skipsteps=[], **kwargs):
    """
    ...
    analyzer = BXASolver(transformations=transformations, outputfiles_basename=outputfiles_basename)
    (analyzer.run)(**kwargs)
    ...
    if 'convolved' not in skipsteps:
        ...
        binned = binning(outputfiles_basename=(analyzer.outputfiles_basename), bins=(data['bins']),
          widths=(data['width']),
          data=(data['data']),
          models=(data['models']))
