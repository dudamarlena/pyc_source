...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bxa/sherpa/background/fitters.py
# Compiled at: 2020-01-28 12:31:59
# Size of source mod 2**32: 16220 bytes
...
def robust_opt(i, params):
    ...
    for p in params:
        ...
        go_up = True
        go_down = True
        for n in list(3.0 ** numpy.arange(1, 20)) + [None] + list(1.1 ** numpy.arange(1, 13)):
            if n is None:
                startval = bestval
...
class BackgroundStorage(object):
    ...
    def load_rsp(self):
        i = self.i
        copy_data(i, 1002)
        unit_arf = get_bkg_arf(1002)
        unit_arf.specresp = 0.0 * unit_arf.specresp + 1.0
        self.bunitrsp = get_response(1002, bkg_id=1)
        delete_data(1002)
    ...
    def store_bkg_model(self, stats, **kwargs):
        ...
        if stats > oldstats + 0.02:
            ...
            with open(self.backgroundparamsfile, 'w') as (f):
                json.dump(dict(params=params, stats=stats, plot=dict(x=(p.dataplot.x.tolist()), ydata=(p.dataplot.y.tolist()), ymodel=(p.modelplot.y.tolist())), **kwargs),
                  f,
                  indent=4)
...
class MultiFitter(object):
    ...
    def fit(self, **kwargs):
        ...
        for part in range(int(numpy.ceil(len(self.ids[1:]) * 1.0 / batchsize))):
    ...
    def fit_jointly_stage(self, stage, ids, plot=False):
        ...
        for i in ids:
            ...
            set_stat('chi2gehrels')
            doplot(i)
            params += [p for p in self.bm.stagepars if not p.link if not p.frozen]
...
__dir__ = [
 SingleFitter, MultiFitter, BackgroundStorage, robust_opt, my_bkg_stat]