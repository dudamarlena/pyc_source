...
# [GCC 8.4.0]
# Embedded file name: build/bdist.darwin-9.6.0-i386/egg/astrogrid/configobj.py
...
__version__ = '4.4.0'
__revision__ = '$Id: configobj.py 97 2007-05-29 15:51:00Z eddie $'
__docformat__ = 'restructuredtext en'
__all__ = (
 '__version__', 'DEFAULT_INDENT_TYPE', 'DEFAULT_INTERPOLATION', 'ConfigObjError', 'NestingError', 'ParseError', 'DuplicateError', 'ConfigspecError', 'ConfigObj', 'SimpleVal', 'InterpolationError', 'InterpolationLoopError', 'MissingInterpolationOption', 'RepeatSectionError', 'UnreprError', 'UnknownType', '__docformat__', 'flatten_errors')
...
class ConfigObj(Section):
    ...
    def __repr__(self):
        return 'ConfigObj({%s})' % (', ').join([ '%s: %s' % (repr(key), repr(self[key])) for key in self.scalars + self.sections ])
...
`` (is multiline).
        ...
        if multiline:
            ...
            if not self.list_values and '\n' not in value or not (multiline and ("'" in value and '"' in value or '\n' in value)):
                quot = self.list_values or noquot
            elif '\n' in value:
    ...
    def _handle_repeat(self, section, configspec):
        ...
        try:
            ...
            section_keys = [ entry for entry in configspec if isinstance(configspec[entry], dict) ]
            scalar_keys = [ entry for entry in configspec if not isinstance(configspec[entry], dict) ]
    ...
    def validate(self, validator, preserve_errors=False, copy=False, section=None):
        ...
        for entry in order:
            ...
            if entry not in section.scalars or entry in section.defaults:
                ...
                if copy and entry not in section.scalars:
                    section.comments[entry] = section._configspec_comments.get(entry, [])
                    section.inline_comments[entry] = section._configspec_inline_comments.get(entry, '')
            else:
                missing = False
                val = section[entry]
            try:
                    ...
                    if not self.stringify:
                        if isinstance(check, (list, tuple)):
                            ...
                            if check != val or missing:
                                section[entry] = check
                elif not copy and missing and entry not in section.defaults:
        ...
        for entry in section.sections:
            ...
            if copy:
                ...
                section.inline_comments[entry] = section._cs_section_inline_comments[entry]
