...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/elixir/relationships.py
# Compiled at: 2009-11-13 15:04:26
"""
...
class Relationship(Property):
    ...
    def create_properties(self):
        ...
        for arg in ('primaryjoin', 'secondaryjoin', 'remote_side', 'foreign_keys'):
    ...
    def inverse(self):
        if not hasattr(self, '_inverse'):
            if self.inverse_name:
                ...
                assert self.match_type_of(inverse), "Relationships '%s' in entity '%s' and '%s' in entity '%s' cannot be inverse of each other because their types do not form a valid combination." % (
...
class ManyToOne(Relationship):
    ...
    def __init__(self, of_kind, column_kwargs=None, colname=None, required=None, primary_key=None, field=None, constraint_kwargs=None, use_alter=None, ondelete=None, onupdate=None, target_column=None, *args, **kwargs):
        assert not (field and (column_kwargs or colname)), "ManyToOne can accept the 'field' argument or column arguments ('colname' or 'column_kwargs') but not both!"
        ...
        if source_desc.autoload:
            if self.colname:
                if 'primaryjoin' not in self.kwargs:
                    self.primaryjoin_clauses = _get_join_clauses(self.entity.table, self.colname, None, target_table)[0]
                    if not self.primaryjoin_clauses:
                        colnames = (', ').join(self.colname)
                        raise Exception("Couldn't find a foreign key constraint in table '%s' using the following columns: %s." % (
                         self.entity.table.name, colnames))
            if self.field:
                raise NotImplementedError("'field' argument not allowed on autoloaded table relationships.")
        else:
            fk_refcols = []
            fk_colnames = []
            ...
            for (key_num, target_col) in enumerate(target_columns):
                if self.field:
                    ...
                    col = Column(colname, target_col.type, **self.column_kwargs)
            ...
            source_desc.add_constraint(ForeignKeyConstraint(fk_colnames, fk_refcols, **self.constraint_kwargs))
        return
...
class ManyToMany(Relationship):
    ...
    def __init__(self, of_kind, tablename=None, local_colname=None, remote_colname=None, ondelete=None, onupdate=None, table=None, schema=None, column_format=None, filter=None, table_kwargs=None, *args, **kwargs):
        ...
        self.schema = schema
        if column_format:
            warnings.warn("The 'column_format' argument on ManyToMany relationships is deprecated. Please use the 'local_colname' and/or 'remote_colname' arguments if you want custom column names for this table only, or modify options.M2MCOL_NAMEFORMAT if you want a custom format for all ManyToMany tables", DeprecationWarning, stacklevel=3)
        ...
        if options.MIGRATION_TO_07_AID:
            self.column_format = migration_aid_m2m_column_formatter(lambda data: options.OLD_M2MCOL_NAMEFORMAT % data, self.column_format)
        self.filter = filter
        if filter is not None:
            if 'viewonly' not in kwargs:
                kwargs['viewonly'] = True
    ...
    def get_table(self):
        warnings.warn('The secondary_table attribute on ManyToMany objects is deprecated. You should rather use the table attribute.', DeprecationWarning, stacklevel=2)
        return self.table
    ...
    def create_tables(self):
        ...
        assert e1_schema == e2_schema or self.schema, 'Schema %r for entity %s differs from schema %r of entity %s. Consider using the schema-parameter. ' % (
        ...
        if self.user_tablename:
            ...
            if options.MIGRATION_TO_07_AID:
                oldname = self.inverse and e1_desc.tablename < e2_desc.tablename and '%s__%s' % (target_part, source_part) or '%s__%s' % (source_part, target_part)
                if oldname != tablename:
                    warnings.warn("The generated table name for the '%s' relationship on the '%s' entity changed from '%s' (the name generated by Elixir 0.6.1 and earlier) to '%s'. You should either rename the table in the database to the new name or use the tablename argument on the relationship to force the old name: tablename='%s'!" % (
                     self.name, self.entity.__name__, oldname,
                     tablename, oldname))
        if e1_desc.autoload:
            if not e2_desc.autoload:
                raise Exception("Entity '%s' is autoloaded and its '%s' ManyToMany relationship points to the '%s' entity which is not autoloaded" % (
                 self.entity.__name__, self.name,
                 self.target.__name__))
            self.table = Table(tablename, e1_desc.metadata, autoload=True, **complete_kwargs)
            if 'primaryjoin' not in self.kwargs or 'secondaryjoin' not in self.kwargs:
                self._build_join_clauses()
        else:
            ...
            for (num, desc, fk_name, rel, inverse, colnames, join_clauses) in (
             ...
             (
              ...
              self.remote_colname, self.secondaryjoin_clauses)):
                ...
                constraints.append(ForeignKeyConstraint(fk_colnames, fk_refcols, name=fk_name, onupdate=onupdate, ondelete=ondelete))

            args = columns + constraints
            self.table = Table(tablename, e1_desc.metadata, schema=schema, *args, **complete_kwargs)
            if DEBUG:
...
def migration_aid_m2m_column_formatter(oldformatter, newformatter):
    ...
    def debug_formatter(data):
        ...
        if new_name != old_name:
            ...
            complete_data.update(old_name=old_name, new_name=new_name, targetname=data['target'].__name__)
            warnings.warn("The '%(old_name)s' column in the '%(current_table)s' table, used as the intermediate table for the '%(relname)s' relationship on the '%(targetname)s' entity was renamed to '%(new_name)s'." % complete_data)
        return new_name
...
def rel_mutator_handler(target):
    ...
    def handler(entity, name, of_kind=None, through=None, via=None, *args, **kwargs):
        if through and via:
            setattr(entity, name, association_proxy(through, via, **kwargs))
            return
        elif through or via:
            raise Exception("'through' and 'via' relationship keyword arguments should be used in combination.")
