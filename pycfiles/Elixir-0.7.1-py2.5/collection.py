...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/elixir/collection.py
# Compiled at: 2009-11-13 14:49:09
"""
        ...
        if '.' in key:
            ...
            if res is None:
                if entity:
                    ...
                    raise Exception("This collection does not contain any entity corresponding to the key '%s'!" % key)
            elif len(res) > 1:
                raise Exception("'%s' resolves to several entities, you should use the full path (including the full module name) to that entity." % key)
            else:
                return res[0]
        return
...
class RelativeEntityCollection(BaseCollection):
    ...
    def resolve(self, key, entity):
        ...
        if '.' not in key or key.startswith('.'):
            ...
            if chunkstokeep < 0:
                raise Exception("Couldn't resolve relative target '%s' relative to '%s'" % (
