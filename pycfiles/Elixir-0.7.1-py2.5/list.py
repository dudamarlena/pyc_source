...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/elixir/ext/list.py
...
class ListEntityBuilder(object):
    ...
    def __init__(self, entity, qualifier=None, column_name='position'):
        warnings.warn("The act_as_list extension is deprecated. Please use SQLAlchemy's orderinglist extension instead", DeprecationWarning, stacklevel=6)
        self.entity = entity
    ...
    def create_non_pk_cols(self):
        if self.entity._descriptor.autoload:
            ...
            if not hasattr(self, 'position_column'):
                raise Exception("Could not find column '%s' in autoloaded table '%s', needed by entity '%s'." % (
                 self.column_name,
    ...
    def after_table(self):
        ...
        def _init_position(self):
            s = select([
             (func.max(position_column) + 1).label('value')], qualifier_method(self)).union(select([literal(1).label('value')]))
            a = s.alias()
        ...
        def _shift_items(self):
            self.table.update(and_(position_column > getattr(self, position_column_name), qualifier_method(self)), values={position_column: position_column - 1}).execute()

        _shift_items = before_delete(_shift_items)

        def move_to_bottom(self):
            self.table.update(and_(position_column >= getattr(self, position_column_name), qualifier_method(self)), values={position_column: position_column - 1}).execute()
            self.table.update(get_entity_where(self), values={position_column: select([
                               func.max(position_column) + 1], qualifier_method(self)).scalar()}).execute()
        ...
        def move_to(self, position):
            ...
            if position < current_position:
                where = and_(position <= position_column, position_column < current_position, qualifier_method(self))
                modifier = 1
            elif position > current_position:
                where = and_(current_position < position_column, position_column <= position, qualifier_method(self))
                modifier = -1
            self.table.update(where, values={position_column: position_column + modifier}).execute()
