...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/elixir/ext/associable.py
...
def associable(assoc_entity, plural_name=None, lazy=True):
    ...
    class Associable(object):
        ...
        def after_table(self):
            col = sa.Column('%s_assoc_id' % interface_name, sa.Integer, sa.ForeignKey('%s.id' % able_name))
            self.entity._descriptor.add_column(col)
            if not hasattr(assoc_entity, '_assoc_table'):
                metadata = assoc_entity._descriptor.metadata
                association_table = sa.Table('%s' % able_name, metadata, sa.Column('id', sa.Integer, primary_key=True), sa.Column('type', sa.String(40), nullable=False))
                tablename = '%s_to_%s' % (able_name, interface_name)
                association_to_table = sa.Table(tablename, metadata, sa.Column('assoc_id', sa.Integer, sa.ForeignKey(association_table.c.id, ondelete='CASCADE'), primary_key=True), sa.Column('%s_id' % interface_name, sa.Integer, sa.ForeignKey(assoc_entity.table.c.id, ondelete='RESTRICT'), primary_key=True))
        ...
        def after_mapper(self):
            if not hasattr(assoc_entity, '_assoc_mapper'):
                assoc_entity._assoc_mapper = sa.orm.mapper(GenericAssoc, assoc_entity._assoc_table, properties={'targets': sa.orm.relation(assoc_entity, secondary=assoc_entity._assoc_to_table, lazy=lazy, backref='associations', order_by=assoc_entity.mapper.order_by)})
            entity = self.entity
            entity.mapper.add_property(attr_name, sa.orm.relation(GenericAssoc, lazy=self.lazy, backref='_backref_%s' % entity.table.name))
            if self.uselist:

                def get(self):
                    if getattr(self, attr_name) is None:
                        setattr(self, attr_name, GenericAssoc(entity.table.name))
                ...
                def set(self, value):
                    if getattr(self, attr_name) is None:
                        setattr(self, attr_name, GenericAssoc(entity.table.name))
                    getattr(self, attr_name).targets = [
                     value]
                    return
