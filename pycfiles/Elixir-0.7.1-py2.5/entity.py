...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/elixir/entity.py
# Compiled at: 2009-11-13 14:50:38
"""
...
import sqlalchemy
from sqlalchemy import Table, Column, Integer, desc, ForeignKey, and_, ForeignKeyConstraint
from sqlalchemy.orm import MapperExtension, mapper, object_session, EXT_CONTINUE, polymorphic_union, ScopedSession, ColumnProperty
...
class EntityDescriptor(object):
    ...
    def __init__(self, entity):
        ...
        for base in entity.__bases__:
            if isinstance(base, EntityMeta):
                if is_entity(base) and not is_abstract_entity(base):
                    if self.parent:
                        raise Exception('%s entity inherits from several entities, and this is not supported.' % self.entity.__name__)
                    else:
                        self.parent = base
        ...
        complete_defaults.update({'metadata': elixir.metadata, 
    ...
    def setup_options(self):
        ...
        if not self.identity:
            ...
            raise Exception('You cannot use the "identity" option and the polymorphic_identity mapper option at the same time.')
            ...
            if self.parent:
                if self.inheritance == 'multi':
                    ...
                    for pk_col in parent_desc.primary_keys:
                        colname = options.MULTIINHERITANCECOL_NAMEFORMAT % {'entity': self.parent.__name__.lower(), 'key': pk_col.key}
                        pk_col_name = '%s.%s' % (tablename, pk_col.key)
                        fk = ForeignKey(pk_col_name, ondelete='cascade')
                        col = Column(colname, pk_col.type, fk, primary_key=True)
                        self.add_column(col)
                ...
                self.add_column(Column(colname, options.DEFAULT_AUTO_PRIMARYKEY_TYPE, primary_key=True))
    ...
    def setup_table(self, only_autoloaded=False):
            ...
            if self.parent:
                if self.inheritance == 'single':
                    ...
                    for con in self.parent._descriptor.constraints:
                        self.add_constraint(ForeignKeyConstraint([ e.parent.key for e in con.elements ], [ e._get_colspec() for e in con.elements ], name=con.name, onupdate=con.onupdate, ondelete=con.ondelete, use_alter=con.use_alter))

            if self.polymorphic and self.inheritance in ('single', 'multi') and self.children and not self.parent:
                self.add_column(Column(self.polymorphic, options.POLYMORPHIC_COL_TYPE))
        ...
        self.entity.table = Table(self.tablename, self.metadata, *args, **kwargs)
        if DEBUG:
    ...
    def setup_events(self):
        ...
        methods = {}
        all_methods = getmembers(self.entity, lambda a: isinstance(a, types.MethodType))
    ...
    def setup_mapper(self):
        ...
        if self.inheritance in ('single', 'concrete', 'multi'):
            if self.parent and (self.inheritance != 'concrete' or self.polymorphic):
            ...
            if self.polymorphic:
                if self.children:
                    if self.inheritance == 'concrete':
                        keys = [
                         (
                          self.identity, self.entity.table)]
                        keys.extend([ (child._descriptor.identity, child.table) for child in self._get_children()
                                    ])
                        pjoin = polymorphic_union(dict(keys), self.polymorphic, 'pjoin_%s' % self.identity)
                        kwargs['with_polymorphic'] = (
                         '*', pjoin)
                        kwargs['polymorphic_on'] = getattr(pjoin.c, self.polymorphic)
                    elif not self.parent:
                        kwargs['polymorphic_on'] = self.get_column(self.polymorphic)
        ...
        if 'primary_key' in kwargs:
            ...
            kwargs['primary_key'] = [ getattr(cols, colname) for colname in kwargs['primary_key']
                                    ]
        if self.session is None:
            ...
            raise Exception("Failed to map entity '%s' with its table or selectable. You can only bind an Entity to a ScopedSession object or None for manual session management." % self.entity.__name__)
        return
    ...
    def get_column(self, key, check_missing=True):
        try:
            ...
            if check_missing:
                raise Exception("No column named '%s' found in the table of the '%s' entity!" % (
        ...
        for other_rel in self.relationships:
            if rel.is_inverse(other_rel):
                if matching_rel is None:
                    ...
                    raise Exception("Several relations match as inverse of the '%s' relation in entity '%s'. You should specify inverse relations manually by using the inverse keyword." % (
    ...
    def primary_keys(self):
        ...
        elif self.parent and self.inheritance == 'single':
...
def instrument_class(cls):
    ...
    base_props = []
    for base in cls.__bases__:
        if isinstance(base, EntityMeta) and (not is_entity(base) or is_abstract_entity(base)):
            base_props += [ (name, deepcopy(attr)) for (name, attr) in getmembers(base, lambda a: isinstance(a, Property))
                          ]

    properties = [ (name, attr) for (name, attr) in cls.__dict__.iteritems() if isinstance(attr, Property)
                 ]
    sorted_props = sorted(base_props + properties, key=lambda i: i[1]._counter)
    ...
    def __setattr__(cls, key, value):
        if isinstance(value, Property):
            if hasattr(cls, '_setup_done'):
                raise Exception('Cannot set attribute on a class after setup_all')
            else:
...
def _install_autosetup_triggers(cls, entity_name=None):
    warnings.warn("The 'autosetup' option on entities is deprecated. Please call setup_all() manually after all your entities have been declared.", DeprecationWarning, stacklevel=4)
    ...
    if SA05orlater:
        warnings.warn("The automatic setup via metadata.create_all() through the autosetup option doesn't work with SQLAlchemy 0.5 and later!")
    else:
        original_table_iterator = md.table_iterator
        if not hasattr(original_table_iterator, '_non_elixir_patched_iterator'):
            ...
            table_iterator._non_elixir_patched_iterator = original_table_iterator
...
def _cleanup_autosetup_triggers(cls):
    ...
    if not SA05orlater:
        if hasattr(md.table_iterator, '_non_elixir_patched_iterator'):
            md.table_iterator = md.table_iterator._non_elixir_patched_iterator
...
def setup_entities(entities):
    ...
    for method_name in ('setup_autoload_table', 'create_pk_cols', 'setup_relkeys',
                        'before_table', 'setup_table', 'setup_reltables', 'after_table',
                        'setup_events', 'before_mapper', 'setup_mapper', 'after_mapper',
...
class EntityBase(object):
            ...
            if isinstance(value, dict):
                ...
                for row in value:
                    if not isinstance(row, dict):
                        raise Exception('Cannot send mixed (dict/non dict) data to list relationships in from_dict data.')
    ...
    def to_dict(self, deep={}, exclude=[]):
        ...
        col_prop_names = [ p.key for p in self.mapper.iterate_properties if isinstance(p, ColumnProperty) ]
