...
# [GCC 8.4.0]
# Embedded file name: /home/kevin/Documents/LibraryApp/LibAppDjango/catalog/tests/test_models.py
# Compiled at: 2019-08-12 04:23:44
# Size of source mod 2**32: 6494 bytes
import builtins as @py_builtins, _pytest.assertion.rewrite as @pytest_ar, datetime
...
class BookModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        bk = Book.objects.create(title='TestyBook', author=author, summary='testysummary',
          isbn='872727727',
          language=testLang)
        bk.genre.set(genres)
        title = Book.objects.get(id=1)
        BookInstance.objects.create(book=title, imprint='testimprint', id=1)
...
class test_book_instance(TestCase):
    ...
    def setUpTestData(cls):
        ...
        testLang = Language.objects.get(id=1)
        Book.objects.create(title='TestyBook', author=author, summary='testysummary',
          isbn='872727727',
          language=testLang,
          id=1)
        title = Book.objects.get(id=1)
        ...
        BookInstance.objects.create(book=title, imprint='testimprint', due_back=time,
          id=1)
        BookInstance.objects.create(book=title, imprint='testimprint', due_back=time_2,
          id=2)

    def test_book_isnt_overdue(self):
        book_instance = BookInstance.objects.get(id=1)
        isnt_due = book_instance.due_back and date.today() > book_instance.due_back
        self.assertIs(isnt_due, False)

    def test_book_is_overdue(self):
        book_instance = BookInstance.objects.get(id=2)
        is_due = book_instance.due_back and date.today() > book_instance.due_back
        self.assertIs(is_due, True)

    def test_str_is_equal_to_name(self):
        example = BookInstance.objects.get(id=1)
        expected_object_name = '{0} ({1})'.format(example.id, example.book.title)
