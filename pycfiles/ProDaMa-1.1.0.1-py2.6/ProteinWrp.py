...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ProDaMa/ProteinWrp.py
# Compiled at: 2009-10-11 05:46:30
"""
...
class ProteinWrp(object):
    ...
    def getProtein(self, str_id):
        ...
        if pdb_finder_data:
            for (field, value) in pdb_finder_data.items():
                setattr(protein, field, value or None)
    ...
    def removeObsoletes(self):
        ...
        """
        pdbWrp = PDBWrp()
        for str_id in pdbWrp.getObsoleteIds():
            self.remove(str_id)
