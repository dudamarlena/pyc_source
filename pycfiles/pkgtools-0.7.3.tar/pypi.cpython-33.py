...
# [GCC 8.4.0]
# Embedded file name: ./pkgtools/pypi.py
# Compiled at: 2013-10-09 08:55:47
# Size of source mod 2**32: 10299 bytes
import sys
from .utils import ext
if sys.version_info >= (3, ):
    import xmlrpc.client as xmlrpclib, urllib.request as urllib2
else:
...
class PyPIJson(object):
    ...
    def retrieve(self, version=None, req_func=None, timeout=None):
        ...
        if req_func is None:
            req_func = _request
        url = self.URL.format(self.package_name + ('/{0}'.format(version) if version else ''))
    ...
    def find(self, version=None):
        ...
        for release in data['urls']:
            yield (
             version, release['filename'], release['md5_digest'],
