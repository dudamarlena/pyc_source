...
# [GCC 8.4.0]
# Embedded file name: /Users/ganesh/work/silota-python/silota/models.py
# Compiled at: 2013-12-17 18:55:34
try:
...
class BaseResource(object):
    ...
    def new_from_dict(cls, d, h=None, **kwargs):
        d = to_python(obj=cls(), in_dict=d, str_keys=cls._strs, int_keys=cls._ints, date_keys=cls._dates, bool_keys=cls._bools, dict_keys=cls._dicts, object_map=cls._map, _h=h)
...
class Rule(BaseResource):
    ...
    def new(self, id, does, match, arg):
        ...
        if id == None:
            rules.append({'does': does, 'match': match, 
               'arg': arg})
            payload = {'rules': rules}
            r = self._h._http_resource(method='PUT', resource=(
             'feed', self.feed.id, 'rules'), data=payload)
    ...
    def _new(self, has, match, arg):
        ...
        existing_ids = [ r.id for r in self.feed.rules ]
        rules.append({'has': has, 'match': match, 
           'arg': arg})
        payload = {'rules': rules}
        r = self._h._http_resource(method='PUT', resource=(
         'feed', self.feed.id, 'rules'), data=payload)
...
class Feed(BaseResource):
    ...
    def new(self, url, sitemap_url, name=None):
        ...
        payload['name'] = name
        r = self._h._http_resource(method='POST', resource=(
         'topic', self.topic.id, 'feeds'), data=payload)
        id = json_decode(r.content).get('id')
        return self.topic.feeds.get(id)

    def delete(self):
        r = self._h._http_resource(method='DELETE', resource=(
         'feed', self.id))
        return r.ok

    @property
    def rules(self):
        return self._h._get_resources(resource=(
         'feed', self.id, 'rules'), obj=Rule, feed=self)
...
class Source(BaseResource):
    ...
    def new(self, url, label, type, selector_dict):
        ...
        r = self._h._http_resource(method='POST', resource=(
         'topic', self.topic.id, 'sources'), data=payload)
        id = json_decode(r.content).get('id')
...
class Document(BaseResource):
    ...
    def new(self, **kwargs):
        ...
        r = self._h._http_resource(method='POST', resource=(
         'topic', self.topic.id, 'documents'), data=payload)
        return json_decode(r.content)

    @classmethod
    def new_from_dict(cls, d, h=None, **kwargs):
        obj = to_python(obj=cls(), in_dict=d, str_keys=cls._strs, int_keys=cls._ints, date_keys=cls._dates, bool_keys=cls._bools, dict_keys=cls._dicts, object_map=cls._map, _h=h)
        c = cls()
        c._h = h
...
class Templates(BaseResource):
    ...
    def update(self, serp=None, suggest=None):
        ...
        r = self._h._http_resource(method='PUT', resource=(
         'topic', self.topic.id, 'templates'), data=payload)
...
class Schema(object):
    ...
    def update(self, schema):
        payload = schema
        r = self._h._http_resource(method='PUT', resource=(
         'topic', self.topic.id, 'schema'), data=payload)
        return r.content
...
class Topic(BaseResource):
    ...
    def new(self, name=None):
        ...
        payload['name'] = name
        r = self._h._http_resource(method='POST', resource=(
         'engine', self.engine.id, 'topics'), data=payload)
        id = json_decode(r.content).get('id')
    ...
    def feeds(self):
        return self._h._get_resources(resource=(
         'topic', self.id, 'feeds'), obj=Feed, topic=self)

    def get_documents(self, page=1):
        return self._h._get_resources(resource=(
         'topic', self.id, 'documents'), obj=Document, map=PagedKeyedListResource, topic=self, params={})

    documents = property(get_documents)

    def delete_documents(self):
        r = self._h._http_resource(method='DELETE', resource=(
         'topic', self.id, 'delete_documents'))
        return r.ok

    @property
    def schema(self):
        return self._h._get_resource(resource=(
         'topic', self.id, 'schema'), obj=Schema, topic=self)

    @property
    def templates(self):
        return self._h._get_resource(resource=(
         'topic', self.id, 'templates'), obj=Templates, topic=self)
    ...
    def crawl(self, force=False):
        if not force:
            ...
            r = self._h._http_resource(method='POST', resource=(
             'topic', self.id, 'crawl', 'force'))
        return r.json()

    def suggest(self, q):
        r = self._h._http_resource(method='GET', resource=(
         'topic', self.id, 'suggest'), params={'q': q})
        return r.json()
...
class Engine(BaseResource):
    ...
    def topics(self):
        return self._h._get_resources(resource=(
         'engine', self.id, 'topics'), obj=Topic, engine=self)
