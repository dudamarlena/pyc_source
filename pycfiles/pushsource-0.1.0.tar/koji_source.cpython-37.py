...
# [GCC 8.4.0]
# Embedded file name: /home/rmcgover/src/pushsource/src/pushsource/_impl/backend/koji_source.py
# Compiled at: 2020-02-03 20:08:31
...
class KojiSource(Source):
    __doc__ = 'Uses koji artifacts as the source of push items.'

    def __init__(self, url, dest=None, rpm=None, signing_key=None, basedir=None, threads=4, timeout=120, cache=None, executor=None):
        """Create a new source.
        ...
        self._executor = executor or Executors.thread_pool(max_workers=threads).with_retry()
    ...
    def _push_items_from_rpm_meta(self, rpm, meta):
        ...
        notfound = [
         RpmPushItem(name=rpm, dest=(self._dest), state='NOTFOUND')]
        if not meta:
            ...
            if self._signing_key:
                if not rpm_path:
                    LOG.error('RPM not found in koji with signing key(s) %s: %s', ', '.join(self._signing_key), rpm)
                    return notfound
        else:
            rpm_path = unsigned_path
        return [
         RpmPushItem(name=(os.path.basename(rpm_path)),
           src=rpm_path,
           dest=(self._dest),
    ...
    def __iter__(self):
        ...
        rpm_push_items_fs = [f_map(f, lambda meta: self._push_items_from_rpm_meta(rpm, meta)) for f, rpm in rpm_fs]
