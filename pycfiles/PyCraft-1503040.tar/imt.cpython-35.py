...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/antenna/imt.py
...
def _A_EH(phi, A_m, phi_3db, k=12.0):
    """
...
def _A_EV(theta, SLA_nu, theta_3db, k=12.0):
    """
    ...
    """
    return -np.minimum(k * ((theta - 90.0) / theta_3db) ** 2, SLA_nu)


@utils.ranged_quantity_input(azim=(
 -180, 180, apu.deg), elev=(
 -90, 90, apu.deg), G_Emax=(
 None, None, cnv.dB), A_m=(
 0, None, cnv.dB), SLA_nu=(
 0, None, cnv.dB), phi_3db=(
 0, None, apu.deg), theta_3db=(
 0, None, apu.deg), strip_input_units=True, output_unit=cnv.dBi)
def imt2020_single_element_pattern(azim, elev, G_Emax, A_m, SLA_nu, phi_3db, theta_3db, k=12.0):
    """
    ...
    """
    return imt2020_single_element_pattern_cython(azim, elev, G_Emax, A_m, SLA_nu, phi_3db, theta_3db, k=k)


@utils.ranged_quantity_input(azim=(
 -180, 180, apu.deg), elev=(
 -90, 90, apu.deg), azim_i=(
 -180, 180, apu.deg), elev_i=(
 -90, 90, apu.deg), G_Emax=(
 None, None, cnv.dB), A_m=(
 0, None, cnv.dB), SLA_nu=(
 0, None, cnv.dB), phi_3db=(
 0, None, apu.deg), theta_3db=(
 0, None, apu.deg), d_H=(
 0, None, cnv.dimless), d_V=(
 0, None, cnv.dimless), rho=(
 0, 1, cnv.dimless), strip_input_units=True, output_unit=cnv.dB)
def imt2020_composite_pattern(azim, elev, azim_i, elev_i, G_Emax, A_m, SLA_nu, phi_3db, theta_3db, d_H, d_V, N_H, N_V, rho=1 * cnv.dimless, k=12.0):
    """
    ...
    """
    return imt2020_composite_pattern_cython(azim, elev, azim_i, elev_i, G_Emax, A_m, SLA_nu, phi_3db, theta_3db, d_H, d_V, N_H, N_V, rho, k=k)
...
def _G_vr(x_v, k_v, k_p, theta_3db, G180):
    ...
    C = 10 * np.log10((180.0 / theta_3db) ** 1.5 * 0.125 / (1 + 8 * k_p)) / np.log10(22.5 / theta_3db)
    lambda_kv = 12 - C * np.log10(4) - 10 * np.log10(0.125 + k_v)
    G = np.empty_like(x_v)
    mask = x_v < x_k
...
@utils.ranged_quantity_input(azim=(
 -180, 180, apu.deg), elev=(
 -90, 90, apu.deg), G0=(
 None, None, cnv.dB), phi_3db=(
 0, None, apu.deg), theta_3db=(
 0, None, apu.deg), k_p=(
 0, None, cnv.dimless), k_h=(
 0, None, cnv.dimless), k_v=(
 0, None, cnv.dimless), tilt_m=(
 -90, 90, apu.deg), tilt_e=(
 -90, 90, apu.deg), strip_input_units=True, output_unit=cnv.dB)
def imt_advanced_sectoral_peak_sidelobe_pattern_400_to_6000_mhz(azim, elev, G0, phi_3db, theta_3db, k_p, k_h, k_v, tilt_m=0.0, tilt_e=0.0):
    r"""
    ...
    """
    return imt_advanced_sectoral_peak_sidelobe_pattern_cython(azim, elev, G0, phi_3db, theta_3db, k_p, k_h, k_v, tilt_m, tilt_e)
