...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/antenna/fixedlink.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 8187 bytes
...
def _fl_pattern_2_1(phi, diameter_over_wavelength, G_max):
    ...
    g1 = 2.0 + 15.0 * np.log10(d_wlen)
    phi_m = 20.0 / d_wlen * np.sqrt(G_max - g1)
    phi_r = 15.85 * d_wlen ** (-0.6)
    phi, d_wlen, G_max, g1, phi_m, phi_r = np.broadcast_arrays(phi, d_wlen, G_max, g1, phi_m, phi_r)
    gain = np.zeros(phi.shape, np.float64)
    mask = (0 <= phi) & (phi < phi_m)
    gain[mask] = G_max[mask] - 0.0025 * (d_wlen[mask] * phi[mask]) ** 2
    mask = (phi_m <= phi) & (phi < phi_r)
    gain[mask] = g1[mask]
    mask = (phi_r <= phi) & (phi < 48.0)
    gain[mask] = 32.0 - 25.0 * np.log10(phi[mask])
    mask = (48.0 <= phi) & (phi <= 180.0)
    gain[mask] = -10.0
    return gain
...
def _fl_pattern_2_2(phi, diameter_over_wavelength, G_max):
    ...
    mask = (48.0 <= phi) & (phi <= 180.0)
    gain[mask] = -10.0 - 10 * np.log10(d_wlen[mask])
    return gain
...
def _fl_pattern_2_3(phi, diameter_over_wavelength, G_max):
    ...
    phi, d_wlen, G_max, g1, phi_m, phi_t, phi_s = np.broadcast_arrays(phi, d_wlen, G_max, g1, phi_m, phi_t, phi_s)
    ...
    gain[mask] = 52 - 10 * np.log10(d_wlen[mask]) - 25 * np.log10(phi[mask])
    mask = (phi_s <= phi) & (phi <= 180.0)
    gain[mask] = -2.0 - 5 * np.log10(d_wlen[mask])
    return gain
...
@utils.ranged_quantity_input(phi=(
 ...
 0.001, 2, apu.m), G_max=(
 None, None, cnv.dBi), strip_input_units=True, output_unit=cnv.dBi)
def fl_pattern(phi, diameter, wavelength, G_max):
    """
    Antenna gain as a function of angular distance after `ITU-R Rec F.699
...
def _fl_hpbw_from_size(diameter, wavelength):
    return 70.0 * wavelength / diameter


@utils.ranged_quantity_input(diameter=(
 0.1, 1000.0, apu.m), wavelength=(
 0.001, 2, apu.m), strip_input_units=True, output_unit=apu.deg)
...
def _fl_G_max_from_size(diameter, wavelength):
    return 20.0 * np.log10(diameter / wavelength) + 7.7


@utils.ranged_quantity_input(diameter=(
 0.1, 1000.0, apu.m), wavelength=(
 0.001, 2, apu.m), strip_input_units=True, output_unit=cnv.dBi)
...
@utils.ranged_quantity_input(hpbw=(
 0.001, 90.0, apu.deg), strip_input_units=True, output_unit=cnv.dBi)
