...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/pathprof/heightprofile.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 8185 bytes
...
def _srtm_height_profile(lon_t, lat_t, lon_r, lat_r, step):
    ...
    distance, bearing_1_rad, bearing_2_rad = cygeodesics.inverse_cython(lon_t_rad, lat_t_rad, lon_r_rad, lat_r_rad)
    bearing_1 = np.degrees(bearing_1_rad)
    bearing_2 = np.degrees(bearing_2_rad)
    back_bearing = bearing_2 % 360 - 180
    distances = np.arange(0.0, distance + step, step)
    lons_rad, lats_rad, bearing_2s_rad = cygeodesics.direct_cython(lon_t_rad, lat_t_rad, bearing_1_rad, distances)
    ...
    if step > hgt_res / 1.5:
        hdistances = np.arange(0.0, distance + hgt_res / 3.0, hgt_res / 3.0)
        hlons, hlats, _ = cygeodesics.direct_cython(lon_t_rad, lat_t_rad, bearing_1_rad, hdistances)
        ...
        cygeodesics.regrid1d_with_x(hdistances, hheights, distances, heights, step / 2.35, regular=True)
    else:
        heights = srtm._srtm_height_data(lons, lats).astype(np.float64)
    return (
     lons, lats,
     distance * 0.001,
     distances * 0.001, heights,
...
@utils.ranged_quantity_input(lon_t=(
 ...
 -90, 90, apu.deg), step=(
 1.0, 100000.0, apu.m), strip_input_units=True, output_unit=(
...
@utils.ranged_quantity_input(lon_c=(
 -180, 180, apu.deg), lat_c=(
 -90, 90, apu.deg), map_size_lon=(
 0.002, 90, apu.deg), map_size_lat=(
 0.002, 90, apu.deg), map_resolution=(
 0.0001, 0.1, apu.deg), hprof_step=(
 0.01, 30, apu.km), strip_input_units=True, allow_none=True, output_unit=(
 apu.deg, apu.deg, apu.m))
def srtm_height_map(lon_c, lat_c, map_size_lon, map_size_lat, map_resolution=3.0 * apu.arcsec, hprof_step=None, do_cos_delta=True, do_coords_2d=False):
    """
    ...
    if hprof_step is None:
        hprof_step = map_resolution * 3600.0 / 1.0 * 30.0
    cosdelta = 1.0 / np.cos(np.radians(lat_c)) if do_cos_delta else 1.0
    xcoords = np.arange(lon_c - cosdelta * map_size_lon / 2, lon_c + cosdelta * map_size_lon / 2 + 1e-06, cosdelta * map_resolution)
    ycoords = np.arange(lat_c - map_size_lat / 2, lat_c + map_size_lat / 2 + 1e-06, map_resolution)
    xcoords2d, ycoords2d = np.meshgrid(xcoords, ycoords)
    heightmap = srtm._srtm_height_data(xcoords2d.flatten(), ycoords2d.flatten()).reshape(xcoords2d.shape)
    if do_coords_2d:
        xcoords, ycoords = xcoords2d, ycoords2d
    return (
     xcoords, ycoords, heightmap)
