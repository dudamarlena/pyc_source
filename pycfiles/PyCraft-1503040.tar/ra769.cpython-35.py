...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/protection/ra769.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 9606 bytes
...
from .. import utils
__all__ = [
 'ra769_calculate_entry', 'ra769_limits']

@utils.ranged_quantity_input(frequency=(
 1e-09, 1000000000000.0, apu.Hz), bandwidth=(
 1e-09, 100000000000.0, apu.Hz), T_A=(
 1e-09, 100000, apu.K), T_rx=(
 1e-09, 100000, apu.K), integ_time=(
 1e-09, None, apu.s))
def ra769_calculate_entry(frequency, bandwidth, T_A, T_rx, mode='continuum', scale='dB', integ_time=2000.0 * apu.s):
    """
    ...
    """
    modes = [
     'continuum', 'spectroscopy', 'vlbi']
    scales = ['linear', 'dB']
    assert mode in modes, 'mode must be either "continuum", "spectroscopy", or "vlbi"'
    assert scale in scales, 'scale must be either "linear" or "dB"'
    if mode != 'vlbi':
        T_rms = ((T_A + T_rx) / np.sqrt(integ_time * bandwidth)).to(apu.mK)
        _P_rms_nu = con.k_B * T_rms
        _Plim = 10 * apu.percent * _P_rms_nu * bandwidth
        _Slim = 4.0 * np.pi / con.c ** 2 * frequency ** 2 * _Plim
        P_rms_nu = _P_rms_nu.to(apu.Watt / apu.Hz if scale == 'linear' else cnv.dB_W_Hz)
        Plim = _Plim.to(apu.Watt if scale == 'linear' else cnv.dB_W)
        Plim_nu = (_Plim / bandwidth).to(apu.W / apu.Hz if scale == 'linear' else cnv.dB_W_Hz)
        Slim = _Slim.to(apu.Watt / apu.m ** 2 if scale == 'linear' else cnv.dB_W_m2)
        Slim_nu = (_Slim / bandwidth).to(apu.Jy if scale == 'linear' else cnv.dB_W_m2_Hz)
        Efield = np.sqrt(cnv.R0 * _Slim).to(apu.microvolt / apu.m if scale == 'linear' else cnv.dB_uV_m)
        Efield_norm = np.sqrt(cnv.R0 * _Slim * apu.MHz / bandwidth.to(apu.MHz)).to(apu.microvolt / apu.m if scale == 'linear' else cnv.dB_uV_m)
        return (
         T_rms, P_rms_nu, Plim, Plim_nu, Slim, Slim_nu, Efield, Efield_norm)
    else:
        _Plim_nu = 1 * apu.percent * con.k_B * (T_rx + T_A)
        _Slim_nu = 4.0 * np.pi / con.c ** 2 * frequency ** 2 * _Plim_nu
        Slim_nu = _Slim_nu.to(apu.Jy if scale == 'linear' else cnv.dB_W_m2_Hz)
        return Slim_nu


@utils.ranged_quantity_input(integ_time=(1e-09, None, apu.s))
def ra769_limits(mode='continuum', scale='dB', integ_time=2000.0 * apu.s):
    """
    ...
    csv_tab = np.genfromtxt(csv_path, delimiter=',', skip_header=1, names=True, dtype=np.float64)
    qtab = QTable(meta={'name': 'RA.769 {} limits'.format(mode)})
    freq = csv_tab['freq0'] * apu.MHz
    bandwidth = csv_tab['Delta_f'] * (apu.kHz if mode == 'spectroscopy' else apu.MHz)
    qtab['frequency'] = freq
    if mode != 'vlbi':
    ...
    if mode != 'vlbi':
        qtab['T_rms'], qtab['P_rms_nu'], qtab['Plim'], qtab['Plim_nu'], qtab['Slim'], qtab['Slim_nu'], qtab['Efield'], qtab['Efield_norm'] = ra769_calculate_entry(freq, bandwidth, qtab['T_A'], qtab['T_rx'], mode=mode, scale=scale, integ_time=integ_time)
    else:
        qtab['Slim_nu'] = ra769_calculate_entry(freq, 1 * apu.Hz, qtab['T_A'], qtab['T_rx'], mode=mode, scale=scale, integ_time=integ_time)
    tab = Table(qtab)
