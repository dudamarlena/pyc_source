...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/pathprof/propagation.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 56066 bytes
...
class PathProp(cyprop._PathProp):
    ...
    @utils.ranged_quantity_input(freq=(
     0.1, 100, apu.GHz), temperature=(
     None, None, apu.K), pressure=(
     None, None, apu.hPa), lon_t=(
     -180, 180, apu.deg), lat_t=(
     -90, 90, apu.deg), lon_r=(
     -180, 180, apu.deg), lat_r=(
     -90, 90, apu.deg), h_tg=(
     None, None, apu.m), h_rg=(
     None, None, apu.m), hprof_step=(
     None, None, apu.m), timepercent=(
     0, 50, apu.percent), omega=(
     0, 100, apu.percent), d_tm=(
     None, None, apu.m), d_lm=(
     None, None, apu.m), d_ct=(
     None, None, apu.m), d_cr=(
     None, None, apu.m), delta_N=(
     None, None, cnv.dimless / apu.km), N0=(
     None, None, cnv.dimless), hprof_dists=(
     None, None, apu.km), hprof_heights=(
     None, None, apu.m), hprof_bearing=(
     None, None, apu.deg), hprof_backbearing=(
     None, None, apu.deg), strip_input_units=True, allow_none=True, output_unit=None)
    def __init__(self, freq, temperature, pressure, lon_t, lat_t, lon_r, lat_r, h_tg, h_rg, hprof_step, timepercent, omega=0 * apu.percent, d_tm=None, d_lm=None, d_ct=None, d_cr=None, zone_t=cyprop.CLUTTER.UNKNOWN, zone_r=cyprop.CLUTTER.UNKNOWN, polarization=0, version=16, delta_N=None, N0=None, hprof_dists=None, hprof_heights=None, hprof_bearing=None, hprof_backbearing=None):
        super().__init__(freq, temperature, pressure, lon_t, lat_t, lon_r, lat_r, h_tg, h_rg, hprof_step, timepercent, omega=omega, d_tm=d_tm, d_lm=d_lm, d_ct=d_ct, d_cr=d_cr, zone_t=zone_t, zone_r=zone_r, polarization=polarization, version=version, delta_N=delta_N, N0=N0, hprof_dists=hprof_dists, hprof_heights=hprof_heights, hprof_bearing=hprof_bearing, hprof_backbearing=hprof_backbearing)
        self._PathProp__params = list(cyprop.PARAMETERS_BASIC)
        if self._pp['version'] == 14:
            self._PathProp__params += cyprop.PARAMETERS_V14
        elif self._pp['version'] == 16:
            self._PathProp__params += cyprop.PARAMETERS_V16
        for p in self._PathProp__params:
    ...
    def __str__(self):
        return '\n'.join('{}: {{:{}}} {}'.format('{:15s}', p[1], '{:10s}').format(p[0], self._pp[p[0]], p[2]) for p in self._PathProp__params)


@utils.ranged_quantity_input(output_unit=(
 cnv.dB, cnv.dB, cnv.dB))
...
@utils.ranged_quantity_input(G_t=(
 None, None, cnv.dBi), G_r=(
 None, None, cnv.dBi), strip_input_units=True, output_unit=cnv.dB)
def loss_troposcatter(pathprop, G_t=0.0 * cnv.dBi, G_r=0.0 * cnv.dBi):
    """
...
@utils.ranged_quantity_input(output_unit=(
 cnv.dB, cnv.dB, cnv.dB, cnv.dB, cnv.dB))
...
@utils.ranged_quantity_input(G_t=(
 None, None, cnv.dBi), G_r=(
 None, None, cnv.dBi), strip_input_units=True, output_unit=(
 cnv.dB, cnv.dB, cnv.dB, cnv.dB, cnv.dB, cnv.dB, cnv.dB))
def loss_complete(pathprop, G_t=0.0 * cnv.dBi, G_r=0.0 * cnv.dBi):
    """
...
@utils.ranged_quantity_input(h_g=(
 None, None, apu.m), freq=(
 None, None, apu.GHz), strip_input_units=True, output_unit=cnv.dB)
def clutter_correction(h_g, zone, freq):
    """
...
def _clutter_imt(freq, dist, location_percent, num_end_points=1):
    ...
    def Qinv(x):
        ...
        Z = ((0.010328 * T + 0.802853) * T + 2.515516698) / (((0.001308 * T + 0.189269) * T + 1.432788) * T + 1.0)
        Q = T - Z
        Q[mask] *= -1
        return Q
    ...
    L_clutter = -5 * np.log10(np.power(10, -0.2 * L_l) + np.power(10, -0.2 * L_s)) - 6 * Qinv(location_percent / 100.0)
    if num_end_points == 2:
        L_clutter *= 2
    return L_clutter
...
@utils.ranged_quantity_input(freq=(
 2, 67, apu.GHz), dist=(
 0.25, None, apu.km), location_percent=(
 0, 100, apu.percent), strip_input_units=True, output_unit=cnv.dB)
def clutter_imt(freq, dist, location_percent, num_end_points=1):
    """
    ...
    """
    return _clutter_imt(freq, dist, location_percent, num_end_points=num_end_points)


@utils.ranged_quantity_input(lon_t=(
 -180, 180, apu.deg), lat_t=(
 -90, 90, apu.deg), map_size_lon=(
 0.002, 90, apu.deg), map_size_lat=(
 0.002, 90, apu.deg), map_resolution=(
 0.0001, 0.1, apu.deg), d_tm=(
 None, None, apu.m), d_lm=(
 None, None, apu.m), d_ct=(
 None, None, apu.m), d_cr=(
 None, None, apu.m), omega_percent=(
 0, 100, apu.percent), strip_input_units=True, allow_none=True, output_unit=None)
def height_map_data(lon_t, lat_t, map_size_lon, map_size_lat, map_resolution=3.0 * apu.arcsec, do_cos_delta=True, zone_t=cyprop.CLUTTER.UNKNOWN, zone_r=cyprop.CLUTTER.UNKNOWN, d_tm=None, d_lm=None, d_ct=None, d_cr=None, omega_percent=0 * apu.percent):
    """
    ...
    """
    return cyprop.height_map_data_cython(lon_t, lat_t, map_size_lon, map_size_lat, map_resolution=map_resolution, do_cos_delta=1 if do_cos_delta else 0, zone_t=zone_t, zone_r=zone_r, d_tm=d_tm, d_lm=d_lm, d_ct=d_ct, d_cr=d_cr, omega=omega_percent)


@utils.ranged_quantity_input(freq=(
 0.1, 100, apu.GHz), temperature=(
 None, None, apu.K), pressure=(
 None, None, apu.hPa), h_tg=(
 None, None, apu.m), h_rg=(
 None, None, apu.m), timepercent=(
 0, 50, apu.percent), strip_input_units=True)
def atten_map_fast(freq, temperature, pressure, h_tg, h_rg, timepercent, hprof_data, polarization=0, version=16):
    """
    ...
    - In future versions, more entries may be added to the results
      dictionary.
    """
    float_res, int_res = cyprop.atten_map_fast_cython(freq, temperature, pressure, h_tg, h_rg, timepercent, hprof_data, polarization=polarization, version=version)
    return {'L_b0p': float_res[0] * cnv.dB, 
...
@utils.ranged_quantity_input(lon_t=(
 ...
 -180, 180, apu.deg), lat_r=(
 -90, 90, apu.deg), step=(
 1.0, 100000.0, apu.m), strip_input_units=True)
def height_path_data(lon_t, lat_t, lon_r, lat_r, step, zone_t=cyprop.CLUTTER.UNKNOWN, zone_r=cyprop.CLUTTER.UNKNOWN):
    """
    ...
    `~pycraf.pathprof.atten_path_fast`.
    """
    lons, lats, distance, distances, heights, bearing, back_bearing, backbearings = heightprofile._srtm_height_profile(lon_t, lat_t, lon_r, lat_r, step)
    ...
    delta_N, beta0, N0 = helper._radiomet_data_for_pathcenter(lon_mids, lat_mids, d_tm, d_lm)
    hprof_data = {}
    hprof_data['lons'] = lons
    hprof_data['lats'] = lats
    hprof_data['lon_mids'] = lon_mids
    hprof_data['lat_mids'] = lat_mids
    hprof_data['distances'] = distances
    hprof_data['heights'] = heights
    hprof_data['bearing'] = bearing
...
@utils.ranged_quantity_input(distance=(
 0, None, apu.km), step=(
 1.0, 100000.0, apu.m), lon_mid=(
 -180, 360, apu.deg), lat_mid=(
 -90, 90, apu.deg), strip_input_units=True)
def height_path_data_generic(distance, step, lon_mid, lat_mid, zone_t=cyprop.CLUTTER.UNKNOWN, zone_r=cyprop.CLUTTER.UNKNOWN):
    """
    Calculate height profile auxillary data needed for
    `~pycraf.pathprof.atten_path_fast`.
...
@utils.ranged_quantity_input(freq=(
 ...
 None, None, apu.m), timepercent=(
 0, 50, apu.percent), strip_input_units=True)
def atten_path_fast(freq, temperature, pressure, h_tg, h_rg, timepercent, hprof_data, polarization=0, version=16):
    """
    ...
    A typical usage would be::
        ...
        plt.show()
    """
    float_res, int_res = cyprop.atten_path_fast_cython(freq, temperature, pressure, h_tg, h_rg, timepercent, hprof_data, polarization=polarization, version=version)
    return {'L_b0p': float_res[0] * cnv.dB, 
     'L_bd': float_res[1] * cnv.dB, 
     'L_bs': float_res[2] * cnv.dB, 
     'L_ba': float_res[3] * cnv.dB, 
...
@utils.ranged_quantity_input(freq=(
 ...
 None, None, apu.m), hprof_step=(
 None, None, apu.m), timepercent=(
 0, 50, apu.percent), G_t=(
 None, None, cnv.dBi), G_r=(
 None, None, cnv.dBi), omega=(
 0, 100, apu.percent), d_tm=(
 None, None, apu.m), d_lm=(
 None, None, apu.m), d_ct=(
 None, None, apu.m), d_cr=(
 ...
 None, None, apu.deg), hprof_backbearing=(
 None, None, apu.deg), strip_input_units=True, allow_none=True, output_unit=None)
def losses_complete(freq, temperature, pressure, lon_t, lat_t, lon_r, lat_r, h_tg, h_rg, hprof_step, timepercent, G_t=0.0 * cnv.dBi, G_r=0.0 * cnv.dBi, omega=0 * apu.percent, d_tm=None, d_lm=None, d_ct=None, d_cr=None, zone_t=cyprop.CLUTTER.UNKNOWN, zone_r=cyprop.CLUTTER.UNKNOWN, polarization=0, version=16, delta_N=None, N0=None, hprof_dists=None, hprof_heights=None, hprof_bearing=None, hprof_backbearing=None):
    """
    ...
    """
    res = cyprop.losses_complete_cython(freq, temperature, pressure, lon_t, lat_t, lon_r, lat_r, h_tg, h_rg, hprof_step, timepercent, G_t=G_t, G_r=G_r, omega=omega, d_tm=d_tm, d_lm=d_lm, d_ct=d_ct, d_cr=d_cr, zone_t=zone_t, zone_r=zone_r, polarization=polarization, version=version, delta_N=delta_N, N0=N0, hprof_dists=hprof_dists, hprof_heights=hprof_heights, hprof_bearing=hprof_bearing, hprof_backbearing=hprof_backbearing)
    return {'L_b0p': res[0] * cnv.dB, 
     'L_bd': res[1] * cnv.dB, 
     'L_bs': res[2] * cnv.dB, 
     'L_ba': res[3] * cnv.dB, 
     'L_b': res[4] * cnv.dB, 
