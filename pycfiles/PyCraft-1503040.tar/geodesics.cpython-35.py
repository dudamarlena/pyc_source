...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/pathprof/geodesics.py
...
__all__ = [
 'geoid_inverse', 'geoid_direct', 'geoid_area']

@utils.ranged_quantity_input(lon1=(
 -np.pi, np.pi, apu.rad), lat1=(
 -np.pi / 2, np.pi / 2, apu.rad), lon2=(
 -np.pi, np.pi, apu.rad), lat2=(
 -np.pi / 2, np.pi / 2, apu.rad), strip_input_units=True, output_unit=(
 apu.m, apu.rad, apu.rad))
def geoid_inverse(lon1, lat1, lon2, lat2, eps=1e-12, maxiter=50):
    """
...
@utils.ranged_quantity_input(lon1=(
 -np.pi, np.pi, apu.rad), lat1=(
 -np.pi / 2, np.pi / 2, apu.rad), bearing1=(
 -np.pi, np.pi, apu.rad), dist=(
 0.1, None, apu.m), strip_input_units=True, output_unit=(
 apu.rad, apu.rad, apu.rad))
def geoid_direct(lon1, lat1, bearing1, dist, eps=1e-12, maxiter=50):
    """
    ...
    or the number of iterations exceeds `maxiter`.
    """
    return direct_cython(lon1, lat1, bearing1, dist, eps=eps, maxiter=maxiter, wrap=True)


@utils.ranged_quantity_input(lon1=(
 -np.pi, np.pi, apu.rad), lon2=(
 -np.pi, np.pi, apu.rad), lat1=(
 -np.pi / 2, np.pi / 2, apu.rad), lat2=(
 -np.pi / 2, np.pi / 2, apu.rad), strip_input_units=True, output_unit=apu.m ** 2)
