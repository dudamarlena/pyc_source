...
# Embedded file name: /Users/runner/runners/2.166.2/work/1/s/build/lib.macosx-10.9-x86_64-3.5/pycraf/atm/atm.py
# Compiled at: 2020-04-16 04:29:51
# Size of source mod 2**32: 85947 bytes
from __future__ import absolute_import, unicode_literals, division, print_function
import os
from functools import partial, lru_cache
import numbers, collections, numpy as np
...
EARTH_RADIUS = 6371.0
fname_oxygen = get_pkg_data_filename('../itudata/p.676-10/R-REC-P.676-10-201309_table1.csv')
fname_water = get_pkg_data_filename('../itudata/p.676-10/R-REC-P.676-10-201309_table2.csv')
oxygen_dtype = np.dtype([(str(s), np.float64) for s in ['f0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6']])
water_dtype = np.dtype([(str(s), np.float64) for s in ['f0', 'b1', 'b2', 'b3', 'b4', 'b5', 'b6']])
resonances_oxygen = np.genfromtxt(fname_oxygen, dtype=oxygen_dtype, delimiter=';')
resonances_water = np.genfromtxt(fname_water, dtype=water_dtype, delimiter=';')
AtmHeightProfile = collections.namedtuple('AtmHeightProfile', [
...
PathEndpoint = collections.namedtuple('PathEndpoint', [
...
def _airmass_from_elevation(elev):
    ...
    airmass[mask] = -0.02344 + 1.014 / np.sin(np.radians(elev[mask] + 5.18 / (elev[mask] + 3.35)))
    return airmass.reshape(elev_shape)


@utils.ranged_quantity_input(elev=(
 0, 90, apu.deg), strip_input_units=True, output_unit=cnv.dimless)
...
def _elevation_from_airmass(airmass):
    ...
    mask = airmass <= 1.887079914799858
    B = np.degrees(np.arcsin(1.014 / (airmass[(~mask)] + 0.02344)))
    elev = np.empty_like(airmass)
    elev[~mask] = 0.5 * B + 0.025 * np.sqrt(400.0 * B ** 2 + 2680.0 * B - 3799.0) - 1.675
    elev[mask] = np.degrees(np.arcsin(1 / airmass[mask]))
    return elev.reshape(airmass_shape)


@utils.ranged_quantity_input(airmass=(
 1, None, cnv.dimless), strip_input_units=True, output_unit=apu.deg)
...
@utils.ranged_quantity_input(atten=(
 1.000000000001, None, cnv.dimless), elev=(0, 90, apu.deg), strip_input_units=True, allow_none=True, output_unit=cnv.dimless)
...
@utils.ranged_quantity_input(tau=(
 1e-12, None, cnv.dimless), elev=(0, 90, apu.deg), strip_input_units=True, allow_none=True, output_unit=cnv.dB)
...
def _refractive_index(temp, press, press_w):
    return 1 + 1e-06 / temp * (77.6 * press - 5.6 * press_w + 375000.0 * press_w / temp)


@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), press=(
 1e-30, None, apu.hPa), press_w=(
 1e-30, None, apu.hPa), strip_input_units=True, output_unit=cnv.dimless)
...
def _saturation_water_pressure(temp, press, wet_type):
    ...
    EF = 1.0 + 0.0001 * (7.2 + press * (0.032 + 5.9e-06 * temp_C ** 2)) if wet_type == 'water' else 1.0 + 0.0001 * (2.2 + press * (0.0382 + 6.4e-06 * temp_C ** 2))
    a, b, c, d = (6.1121, 18.678, 257.14, 234.5) if wet_type == 'water' else (6.1115,
                                                                              23.036,
                                                                              279.82,
                                                                              333.7)
...
@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), press=(
 1e-30, None, apu.hPa), strip_input_units=True, output_unit=apu.hPa)
def saturation_water_pressure(temp, press, wet_type='water'):
    ...
    """
    return _saturation_water_pressure(temp, press, wet_type=wet_type)


def _pressure_water_from_humidity(temp, press, humidity, wet_type='water'):
    e_s = _saturation_water_pressure(temp, press, wet_type=wet_type)
    press_w = humidity / 100.0 * e_s
    return press_w


@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), press=(
 1e-30, None, apu.hPa), humidity=(
 0, 100, apu.percent), strip_input_units=True, output_unit=apu.hPa)
def pressure_water_from_humidity(temp, press, humidity, wet_type='water'):
    """
    ...
    press_w : `~astropy.units.Quantity`
        Water vapor partial pressure [hPa]
    """
    return _pressure_water_from_humidity(temp, press, humidity, wet_type=wet_type)


def _humidity_from_pressure_water(temp, press, press_w, wet_type='water'):
    e_s = _saturation_water_pressure(temp, press, wet_type=wet_type)
    humidity = 100.0 * press_w / e_s
    return humidity
...
@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), press=(
 1e-30, None, apu.hPa), press_w=(
 1e-30, None, apu.hPa), strip_input_units=True, output_unit=apu.percent)
def humidity_from_pressure_water(temp, press, press_w, wet_type='water'):
    """
    ...
    humidity : `~astropy.units.Quantity`
        Relative humidity [%]
    """
    return _humidity_from_pressure_water(temp, press, press_w, wet_type=wet_type)
...
@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), rho_w=(
 1e-30, None, apu.g / apu.m ** 3), strip_input_units=True, output_unit=apu.hPa)
...
@utils.ranged_quantity_input(temp=(
 1e-30, None, apu.K), press_w=(
 1e-30, None, apu.hPa), strip_input_units=True, output_unit=apu.g / apu.m ** 3)
...
def _profile_standard(height):
    ...
    layer_heights = np.array([0.0, 11.0, 20.0, 32.0, 47.0, 51.0, 71.0, 85.0])
    ...
    rho0 = 7.5
    h0 = 2.0
    layer_temp_gradients = np.array([-6.5, 0.0, 1.0, 2.8, 0.0, -2.8, -2.0])
    ...
    for i in range(1, len(layer_heights) - 1):
        ...
        layer_start_pressures.append(Pi * ((Ti / (Ti + Li * dh)) ** (34.163 / Li) if i - 1 not in (1,
                                                                                                   4) else np.exp(-34.163 * dh / Ti)))

    layer_start_temperatures = np.array(layer_start_temperatures)
    layer_start_pressures = np.array(layer_start_pressures)
    temperatures = layer_start_temperatures[indices] + (_height - layer_heights[indices]) * layer_temp_gradients[indices]
    pressures = np.empty_like(temperatures)
    mask = np.in1d(indices, [1, 4])
    indm = indices[mask]
    dhm = _height[mask] - layer_heights[indm]
    pressures[mask] = layer_start_pressures[indm] * np.exp(-34.163 * dhm / layer_start_temperatures[indm])
    mask = np.logical_not(mask)
    indm = indices[mask]
    dhm = _height[mask] - layer_heights[indm]
    Lim = layer_temp_gradients[indm]
    Tim = layer_start_temperatures[indm]
    pressures[mask] = layer_start_pressures[indm] * (Tim / (Tim + Lim * dhm)) ** (34.163 / Lim)
    rho_water = rho0 * np.exp(-_height / h0)
    pressures_water = rho_water * temperatures / 216.7
    mask = pressures_water / pressures < 2e-06
    pressures_water[mask] = pressures[mask] * 2e-06
    ...
    humidities_water = _humidity_from_pressure_water(temperatures, pressures, pressures_water, wet_type='water')
    humidities_ice = _humidity_from_pressure_water(temperatures, pressures, pressures_water, wet_type='ice')
    result = AtmHeightProfile(temperatures.reshape(height.shape).squeeze(), pressures.reshape(height.shape).squeeze(), rho_water.reshape(height.shape).squeeze(), pressures_water.reshape(height.shape).squeeze(), ref_indices.reshape(height.shape).squeeze(), humidities_water.reshape(height.shape).squeeze(), humidities_ice.reshape(height.shape).squeeze())
    return result


@utils.ranged_quantity_input(height=(
 0, 84.99999999, apu.km), strip_input_units=True, output_unit=None)
...
def _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs):
    """
    ...
    mask = pressure_water / pressure < 2e-06
    pressure_water[mask] = pressure[mask] * 2e-06
    rho_water[mask] = pressure_water[mask] / temperature[mask] * 216.7
    ref_index = _refractive_index(temperature, pressure, pressure_water)
    humidity_water = _humidity_from_pressure_water(temperature, pressure, pressure_water, wet_type='water')
    humidity_ice = _humidity_from_pressure_water(temperature, pressure, pressure_water, wet_type='ice')
    return AtmHeightProfile(temperature.reshape(height.shape).squeeze(), pressure.reshape(height.shape).squeeze(), rho_water.reshape(height.shape).squeeze(), pressure_water.reshape(height.shape).squeeze(), ref_index.reshape(height.shape).squeeze(), humidity_water.reshape(height.shape).squeeze(), humidity_ice.reshape(height.shape).squeeze())


@utils.ranged_quantity_input(height=(
 0, 99.99999999, apu.km), strip_input_units=True, output_unit=None)
def profile_lowlat(height):
    ...
    """
    temp_heights = [
     0.0, 17.0, 47.0, 52.0, 80.0, 100.0]
    temp_funcs = [
     lambda h: 300.4222 - 6.3533 * h + 0.005886 * h ** 2,
     lambda h: 194 + (h - 17.0) * 2.533,
     lambda h: 270.0,
     lambda h: 270.0 - (h - 52.0) * 3.0714,
     lambda h: 184.0]
    press_heights = [
     0.0, 10.0, 72.0, 100.0]
    press_funcs = [
    ...
    rho_heights = [
     0.0, 15.0, 100.0]
    rho_funcs = [
     lambda h: 19.6542 * np.exp(-0.2313 * h - 0.1122 * h ** 2 + 0.01351 * h ** 3 - 0.0005923 * h ** 4),
     lambda h: 0.0]
    ret = _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs)
    qret = tuple(q * u for q, u in zip(ret, atm_height_profile_units))
    return AtmHeightProfile(*qret)
...
def profile_midlat_summer(height):
    ...
    """
    temp_heights = [
     0.0, 13.0, 17.0, 47.0, 53.0, 80.0, 100.0]
    temp_funcs = [
     lambda h: 294.9838 - 5.2159 * h - 0.07109 * h ** 2,
     lambda h: 215.15,
     lambda h: 215.15 * np.exp(0.008128 * (h - 17.0)),
     lambda h: 275.0,
     lambda h: 275.0 + 20.0 * (1.0 - np.exp(0.06 * (h - 53.0))),
     lambda h: 175.0]
    press_heights = [
     0.0, 10.0, 72.0, 100.0]
    press_funcs = [
    ...
    rho_heights = [
     0.0, 15.0, 100.0]
    rho_funcs = [
     lambda h: 14.3542 * np.exp(-0.4174 * h - 0.0229 * h ** 2 + 0.001007 * h ** 3),
     lambda h: 0.0]
    ret = _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs)
    qret = tuple(q * u for q, u in zip(ret, atm_height_profile_units))
    return AtmHeightProfile(*qret)
...
def profile_midlat_winter(height):
    ...
    """
    temp_heights = [
     0.0, 10.0, 33.0, 47.0, 53.0, 80.0, 100.0]
    temp_funcs = [
     lambda h: 272.7241 - 3.6517 * h - 0.1759 * h ** 2,
     lambda h: 218.0,
     lambda h: 218.0 + 3.3571 * (h - 33.0),
     lambda h: 265.0,
     lambda h: 265.0 - 2.037 * (h - 53.0),
     lambda h: 210.0]
    press_heights = [
     0.0, 10.0, 72.0, 100.0]
    press_funcs = [
    ...
    rho_heights = [
     0.0, 10.0, 100.0]
    rho_funcs = [
     lambda h: 3.4742 * np.exp(-0.2697 * h - 0.03604 * h ** 2 + 0.0004489 * h ** 3),
     lambda h: 0.0]
    ret = _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs)
    qret = tuple(q * u for q, u in zip(ret, atm_height_profile_units))
    return AtmHeightProfile(*qret)
...
def profile_highlat_summer(height):
    ...
    """
    temp_heights = [
     0.0, 10.0, 23.0, 48.0, 53.0, 79.0, 100.0]
    temp_funcs = [
     lambda h: 286.8374 - 4.7805 * h - 0.1402 * h ** 2,
     lambda h: 225.0,
     lambda h: 225.0 * np.exp(0.008317 * (h - 23.0)),
     lambda h: 277.0,
     lambda h: 277.0 - 4.0769 * (h - 53.0),
     lambda h: 171.0]
    press_heights = [
     0.0, 10.0, 72.0, 100.0]
    press_funcs = [
    ...
    rho_heights = [
     0.0, 15.0, 100.0]
    rho_funcs = [
     lambda h: 8.988 * np.exp(-0.3614 * h - 0.005402 * h ** 2 - 0.001955 * h ** 3),
     lambda h: 0.0]
    ret = _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs)
    qret = tuple(q * u for q, u in zip(ret, atm_height_profile_units))
    return AtmHeightProfile(*qret)
...
def profile_highlat_winter(height):
    ...
    """
    temp_heights = [
     0.0, 8.5, 30.0, 50.0, 54.0, 100.0]
    temp_funcs = [
     lambda h: 257.4345 + 2.3474 * h - 1.5479 * h ** 2 + 0.08473 * h ** 3,
     lambda h: 217.5,
     lambda h: 217.5 + 2.125 * (h - 30.0),
     lambda h: 260.0,
     lambda h: 260.0 - 1.667 * (h - 54.0)]
    press_heights = [
     0.0, 10.0, 72.0, 100.0]
    press_funcs = [
     lambda Pstart, h: 1010.8828 - 122.2411 * h + 4.554 * h ** 2,
     lambda Pstart, h: Pstart * np.exp(-0.147 * (h - 10)),
     lambda Pstart, h: Pstart * np.exp(-0.15 * (h - 72))]
    rho_heights = [
     0.0, 10.0, 100.0]
    rho_funcs = [
     lambda h: 1.2319 * np.exp(0.07481 * h - 0.0981 * h ** 2 + 0.00281 * h ** 3),
     lambda h: 0.0]
    ret = _profile_helper(height, temp_heights, temp_funcs, press_heights, press_funcs, rho_heights, rho_funcs)
    qret = tuple(q * u for q, u in zip(ret, atm_height_profile_units))
    return AtmHeightProfile(*qret)
...
def _S_oxygen(press_dry, temp):
    ...
    """
    theta = 300.0 / temp
    factor = 1e-07 * press_dry * theta ** 3
    return resonances_oxygen['a1'] * factor * np.exp(resonances_oxygen['a2'] * (1.0 - theta))
...
def _S_water(press_w, temp):
    ...
    """
    theta = 300.0 / temp
    factor = 0.1 * press_w * theta ** 3.5
    return resonances_water['b1'] * factor * np.exp(resonances_water['b2'] * (1.0 - theta))
...
def _Delta_f_oxygen(press_dry, press_w, temp):
    ...
    """
    theta = 300.0 / temp
    df = resonances_oxygen['a3'] * 0.0001 * (press_dry * theta ** (0.8 - resonances_oxygen['a4']) + 1.1 * press_w * theta)
    return np.sqrt(df ** 2 + 2.25e-06)
...
def _Delta_f_water(press_dry, press_w, temp):
    ...
    df = b3 * 0.0001 * (press_dry * theta ** b4 + b5 * press_w * theta ** b6)
    return 0.535 * df + np.sqrt(0.217 * df ** 2 + 2.1316e-12 * f0 ** 2 / theta)
...
def _delta_oxygen(press_dry, press_w, temp):
    ...
    """
    theta = 300.0 / temp
    return (resonances_oxygen['a5'] + resonances_oxygen['a6'] * theta) * 0.0001 * (press_dry + press_w) * theta ** 0.8
...
def _N_D_prime2(freq_grid, press_dry, press_w, temp):
    ...
    """
    theta = 300.0 / temp
    d = 0.00056 * (press_dry + press_w) * theta ** 0.8
    sum_1 = 6.14e-05 / d / (1 + (freq_grid / d) ** 2)
    sum_2 = 1.4e-12 * press_dry * theta ** 1.5 / (1 + 1.9e-05 * freq_grid ** 1.5)
    return freq_grid * press_dry * theta ** 2 * (sum_1 + sum_2)


def _atten_specific_annex1(freq_grid, press_dry, press_w, temp):
    ...
    atten_o2 += _N_D_prime2(freq_grid, press_dry, press_w, temp)
    ...
    return (
     atten_o2 * 0.182 * freq_grid, atten_h2o * 0.182 * freq_grid)


@utils.ranged_quantity_input(freq_grid=(
 1e-30, 1000, apu.GHz), press_dry=(
 1e-30, None, apu.hPa), press_w=(
 1e-30, None, apu.hPa), temp=(
 1e-30, None, apu.K), strip_input_units=True, output_unit=(cnv.dB / apu.km, cnv.dB / apu.km))
def atten_specific_annex1(freq_grid, press_dry, press_w, temp):
    """
    ...
    sufficiently fine.
    """
    return _atten_specific_annex1(freq_grid, press_dry, press_w, temp)


@utils.ranged_quantity_input(specific_atten=(
 1e-30, None, cnv.dB / apu.km), path_length=(
 1e-30, None, apu.km), strip_input_units=True, output_unit=cnv.dB)
...
@utils.ranged_quantity_input(freq_grid=(
 1e-30, 1000, apu.GHz), heights=(
 0, 80, apu.km), strip_input_units=True, allow_none=True, output_unit=None)
def atm_layers(freq_grid, profile_func, heights=None):
    ...
    if heights is None:
        deltas = 0.0001 * np.exp(np.arange(900) / 100.0)
        heights = np.hstack([0.0, np.cumsum(deltas)])
    else:
    ...
    heights = np.hstack([heights, [2000.0, 400000.0, 6000000000.0, 3000000000000000.0, 3e+19, 3e+22, 3e+25]])
    ref_index = np.hstack([ref_index, [1, 1, 1, 1, 1, 1, 1, 1]])
    adict = {}
    adict['space_i'] = space_i
    adict['max_i'] = len(heights) - 1
    adict['freq_grid'] = freq_grid
    adict['heights'] = heights
    adict['radii'] = EARTH_RADIUS + heights
    ...
    for idx in range(1, space_i):
        atten_dry_db[idx], atten_wet_db[idx] = _atten_specific_annex1(freq_grid, press[idx], press_w[idx], temp[idx])
...
def _raytrace_path(elev, obs_alt, atm_layers_cache, max_arc_length=180.0, max_path_length=1000.0):
    ...
    obs_alt = max([1e-09, obs_alt])
    start_i = np.searchsorted(heights, obs_alt)
    if heights[start_i] == obs_alt:
        start_i += 1
        obs_alt += 1e-09
    path_params, refraction, is_space_path = path_helper_cython(start_i, space_i, max_i, elev, obs_alt, max_path_length, max_arc_length, radii, ref_index)
    return (
     path_params, refraction, is_space_path)


@utils.ranged_quantity_input(elevation=(
 -90, 90, apu.deg), obs_alt=(
 0, None, apu.km), max_arc_length=(
 1e-30, 180.0, apu.deg), max_path_length=(
 1e-30, None, apu.km), strip_input_units=True, output_unit=(None, apu.deg, None))
def raytrace_path(elevation, obs_alt, atm_layers_cache, max_arc_length=180.0 * apu.deg, max_path_length=1000.0 * apu.km):
    """
    ...
    """
    return _raytrace_path(elevation, obs_alt, atm_layers_cache, max_arc_length=max_arc_length, max_path_length=max_path_length)


def _path_endpoint(elev, obs_alt, atm_layers_cache, max_arc_length=180.0, max_path_length=1000.0):
    radii = atm_layers_cache['radii']
    heights = atm_layers_cache['heights']
    ref_index = atm_layers_cache['ref_index']
    space_i = atm_layers_cache['space_i']
    ...
    if heights[start_i] == obs_alt:
        start_i += 1
        obs_alt += 1e-09
    ret = PathEndpoint(*path_endpoint_cython(start_i, space_i, max_i, elev, obs_alt, max_path_length, max_arc_length, radii, ref_index))
    return ret
...
@utils.ranged_quantity_input(elevation=(
 ...
 0, None, apu.km), max_arc_length=(
 1e-30, 180.0, apu.deg), max_path_length=(
 1e-30, None, apu.km), strip_input_units=True, output_unit=None)
def path_endpoint(elevation, obs_alt, atm_layers_cache, max_arc_length=180.0 * apu.deg, max_path_length=1000.0 * apu.km):
    """
    ...
    """
    ret = _path_endpoint(elevation, obs_alt, atm_layers_cache, max_arc_length=max_arc_length, max_path_length=max_path_length)
    qtup = tuple((q * u if u else q) for q, u in zip(ret, path_endpoint_units))
    return PathEndpoint(*qtup)


def _find_elevation(obs_alt, target_alt, arc_length, atm_layers_cache, niter=50, interval=10, stepsize=0.05, seed=None):
    radii = atm_layers_cache['radii']
    heights = atm_layers_cache['heights']
    ref_index = atm_layers_cache['ref_index']
    space_i = atm_layers_cache['space_i']
    ...
    if arc_length < 1e-06:
        ...
        x1, y1 = 0.0, a_e + obs_alt
        x2, y2 = np.sin(arc_rad) * (a_e + target_alt), np.cos(arc_rad) * (a_e + target_alt)
    ...
    def func(x):
        elev = x[0]
        ret = path_endpoint_cython(start_i, space_i, max_i, elev, obs_alt, max_path_length, arc_length, radii, ref_index)
        h_n = ret[2]
        arc_len = ret[6]
        a_tot = ret[8]
        return (
    ...
    def opt_func(x):
        ...
        mmin = abs(h_n - target_alt) + abs(np.degrees(arc_len) - arc_length) + (0 if elev < 90 else elev - 90) + (0 if elev > -90 else -90 - elev)
        return mmin

    x0 = np.array([elev_init])
    minimizer_kwargs = {'method': 'BFGS'}
    res = basinhopping(opt_func, x0, T=0.05, minimizer_kwargs=minimizer_kwargs, niter=niter, interval=interval, stepsize=stepsize, seed=seed)
    elev_final = res['x'][0]
...
@utils.ranged_quantity_input(obs_alt=(
 0, None, apu.km), target_alt=(
 0, None, apu.km), arc_length=(
 1e-30, 180.0, apu.deg), strip_input_units=True, output_unit=(apu.deg, apu.km))
def find_elevation(obs_alt, target_alt, arc_length, atm_layers_cache, niter=50, interval=10, stepsize=0.05, seed=None):
    """
    ...
    """
    return _find_elevation(obs_alt, target_alt, arc_length, atm_layers_cache, niter=niter, interval=interval, stepsize=stepsize, seed=seed)


@utils.ranged_quantity_input(elevation=(
 -90, 90, apu.deg), obs_alt=(
 0, None, apu.km), t_bg=(
 1e-30, None, apu.K), max_arc_length=(
 1e-30, 180.0, apu.deg), max_path_length=(
 1e-30, None, apu.km), strip_input_units=True, output_unit=(cnv.dB, apu.deg, apu.K))
def atten_slant_annex1(elevation, obs_alt, atm_layers_dict, do_tebb=True, t_bg=2.73 * apu.K, max_arc_length=180.0 * apu.deg, max_path_length=1000.0 * apu.km):
    """
    ...
    path_params, refraction, is_space_path = _raytrace_path(elevation, obs_alt, adict, max_path_length=max_path_length, max_arc_length=max_arc_length)
    atten_db = adict['atten_db']
    temp = adict['temp']
    total_atten_db = np.sum(atten_db[path_params.layer_idx[1:]] * path_params.a_n[1:, np.newaxis], axis=0)
    if is_space_path and do_tebb:
        for idx, lidx in list(enumerate(path_params.layer_idx))[:0:-1]:
            if lidx >= adict['space_i']:
                pass
            else:
                atten_tot_lin = 10 ** (-atten_db[lidx] * path_params.a_n[idx] / 10.0)
                tebb *= atten_tot_lin
                tebb += (1.0 - atten_tot_lin) * temp[lidx]

    else:
        tebb[...] = np.nan
    return (
     total_atten_db, refraction, tebb)
...
def _phi_helper(r_p, r_t, args):
    ...
    """
    phi0, a, b, c, d = args
    return phi0 * r_p ** a * r_t ** b * np.exp(c * (1.0 - r_p) + d * (1.0 - r_t))


_HELPER_PARAMS = {'xi1': (1.0, 0.0717, -1.8132, 0.0156, -1.6515), 
 'xi2': (1.0, 0.5146, -4.6368, -0.1921, -5.7416), 
 'xi3': (1.0, 0.3414, -6.5851, 0.213, -8.5854), 
 'xi4': (1.0, -0.0112, 0.0092, -0.1033, -0.0009), 
 'xi5': (1.0, 0.2705, -2.7192, -0.3016, -4.1033), 
 'xi6': (1.0, 0.2445, -5.9191, 0.0422, -8.0719), 
 'xi7': (1.0, -0.1833, 6.5589, -0.2402, 6.131), 
 'gamma54': (2.192, 1.8286, -1.9487, 0.4051, -2.8509), 
 'gamma58': (12.59, 1.0045, 3.561, 0.1588, 1.2834), 
 'gamma60': (15.0, 0.9003, 4.1335, 0.0427, 1.6088), 
 'gamma62': (14.28, 0.9886, 3.4176, 0.1827, 1.3429), 
 'gamma64': (6.819, 1.432, 0.6258, 0.3177, -0.5914), 
 'gamma66': (1.908, 2.0717, -4.1404, 0.491, -4.8718), 
 'delta': (-0.00306, 3.211, -14.94, 1.583, -16.37)}
_helper_funcs = dict((k, partial(_phi_helper, args=v)) for k, v in _HELPER_PARAMS.items())
...
def _atten_specific_annex2(freq_grid, press, rho_w, temp):
    ...
    _temp = temp
    r_p = _press / 1013.0
    r_t = 288.0 / (_temp - 0.15)
    atten_dry = np.empty_like(_freq)
    atten_wet = np.zeros_like(_freq)
    h = dict((k, func(r_p, r_t)) for k, func in _helper_funcs.items())
    mask = _freq <= 54
    f = _freq[mask]
    atten_dry[mask] = f ** 2 * r_p ** 2 * 0.001 * (7.2 * r_t ** 2.8 / (f ** 2 + 0.34 * r_p ** 2 * r_t ** 1.6) + 0.62 * h['xi3'] / ((54.0 - f) ** (1.16 * h['xi1']) + 0.83 * h['xi2']))
    mask = (_freq > 54) & (_freq <= 60)
    f = _freq[mask]
    atten_dry[mask] = np.exp(np.log(h['gamma54']) / 24.0 * (f - 58.0) * (f - 60.0) - np.log(h['gamma58']) / 8.0 * (f - 54.0) * (f - 60.0) + np.log(h['gamma60']) / 12.0 * (f - 54.0) * (f - 58.0))
    mask = (_freq > 60) & (_freq <= 62)
    f = _freq[mask]
    atten_dry[mask] = h['gamma60'] + (h['gamma62'] - h['gamma60']) * (f - 60.0) / 2.0
    mask = (_freq > 62) & (_freq <= 66)
    f = _freq[mask]
    atten_dry[mask] = np.exp(np.log(h['gamma62']) / 8.0 * (f - 64.0) * (f - 66.0) - np.log(h['gamma64']) / 4.0 * (f - 62.0) * (f - 66.0) + np.log(h['gamma66']) / 8.0 * (f - 62.0) * (f - 64.0))
    mask = (_freq > 66) & (_freq <= 120)
    f = _freq[mask]
    atten_dry[mask] = f ** 2 * r_p ** 2 * 0.001 * (0.000302 * r_t ** 3.5 + 0.283 * r_t ** 3.8 / ((f - 118.75) ** 2 + 2.91 * r_p ** 2 * r_t ** 1.6) + 0.502 * h['xi6'] * (1.0 - 0.0163 * h['xi7'] * (f - 66.0)) / ((f - 66.0) ** (1.4346 * h['xi4']) + 1.15 * h['xi5']))
    mask = (_freq > 120) & (_freq <= 350)
    f = _freq[mask]
    atten_dry[mask] = h['delta'] + f ** 2 * r_p ** 3.5 * 0.001 * (0.000302 / (1.0 + 1.9e-05 * f ** 1.5) + 0.283 * r_t ** 0.3 / ((f - 118.75) ** 2 + 2.91 * r_p ** 2 * r_t ** 1.6))
    ...
    def _helper(a, eta, b, c, d, do_g):
        return a * eta * np.exp(b * (1 - r_t)) / ((f - c) ** 2 + d * eta ** 2) * (g(f, int(c + 0.5)) if do_g else 1.0)
    ...
    for args in [
     ...
     (
      290.0, eta_1, 0.41, 752, 0, True),
     (
      83328.0, eta_2, 0.99, 1780, 0, True)]:
        atten_wet += _helper(*args)

    atten_wet *= f ** 2 * r_t ** 2.5 * _rho_w * 0.0001
    return (
     atten_dry, atten_wet)


@utils.ranged_quantity_input(freq_grid=(
 1e-30, 350.0, apu.GHz), press=(
 1e-30, None, apu.hPa), rho_w=(
 1e-30, None, apu.g / apu.m ** 3), temp=(
 1e-30, None, apu.K), strip_input_units=True, output_unit=(cnv.dB / apu.km, cnv.dB / apu.km))
def atten_specific_annex2(freq_grid, press, rho_w, temp):
    ...
    """
    return _atten_specific_annex2(freq_grid, press, rho_w, temp)


@utils.ranged_quantity_input(freq_grid=(
 1e-30, 350.0, apu.GHz), press=(
 1e-30, None, apu.hPa), strip_input_units=True, output_unit=apu.km)
def equivalent_height_dry(freq_grid, press):
    ...
    t_1 = 4.64 / (1.0 + 0.066 * r_p ** (-2.3)) * np.exp(-((f - 59.7) / (2.87 + 12.4 * np.exp(-7.9 * r_p))) ** 2)
    t_2 = 0.14 * np.exp(2.12 * r_p) / ((f - 118.75) ** 2 + 0.031 * np.exp(2.2 * r_p))
    t_3 = 0.0114 * f / (1.0 + 0.14 * r_p ** (-2.6)) * ((-0.0247 + 0.0001 * f + 1.61e-06 * f ** 2) / (1.0 - 0.0169 * f + 4.1e-05 * f ** 2 + 3.2e-07 * f ** 3))
    h_0 = 6.1 * (1.0 + t_1 + t_2 + t_3) / (1.0 + 0.17 * r_p ** (-1.1))
    h_0[(h_0 > 10.8 * r_p ** 0.3) & (f < 70.0)] = 10.8 * r_p ** 0.3
    return h_0.squeeze()
...
def equivalent_height_wet(freq_grid, press):
    ...
    r_p = press / 1013.0
    f = np.atleast_1d(freq_grid).astype(dtype=np.float64, copy=False)
    s_w = 1.013 / (1.0 + np.exp(-8.6 * (r_p - 0.57)))

    def _helper(a, b, c):
        return a * s_w / ((f - b) ** 2 + c * s_w)

    h_w = 1.66 * (1.0 + _helper(1.39, 22.235, 2.56) + _helper(3.37, 183.31, 4.69) + _helper(1.58, 325.1, 2.89))
    return h_w.squeeze()


@utils.ranged_quantity_input(atten_dry=(
 1e-30, None, cnv.dB / apu.km), atten_wet=(
 1e-30, None, cnv.dB / apu.km), h_dry=(
 1e-30, None, apu.km), h_wet=(
 1e-30, None, apu.km), elev=(
 5, 90, apu.deg), strip_input_units=True, output_unit=cnv.dB)
