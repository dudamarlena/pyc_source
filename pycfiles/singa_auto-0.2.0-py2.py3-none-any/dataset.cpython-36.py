...
# [GCC 8.4.0]
# Embedded file name: /Users/nailixing/PyProjects/nusdb_rafiki/singa_auto/model/dataset.py
# Compiled at: 2020-04-24 21:27:49
# Size of source mod 2**32: 20347 bytes
from PIL import Image
import numpy as np, random, logging
from typing import List
import os, traceback, zipfile, io, abc, csv, pandas, tempfile, pandas as pd
...
class DatasetUtils:
    ...
    def load_dataset_of_image_files(self, dataset_path, min_image_size=None, max_image_size=None, mode='RGB', if_shuffle=False, lazy_load=False):
        """
        ...
        """
        if lazy_load:
            return ImageFilesDatasetLazy(dataset_path, min_image_size=min_image_size, max_image_size=max_image_size, mode=mode,
              if_shuffle=if_shuffle)
        else:
            return ImageFilesDataset(dataset_path, min_image_size=min_image_size, max_image_size=max_image_size, mode=mode,
              if_shuffle=if_shuffle)
    ...
    def normalize_images(self, images, mean=None, std=None):
        ...
        if len(images) == 0:
            ...
            if mean is None:
                mean = np.mean(images, axis=(0, 1, 2)).tolist()
            if std is None:
                std = np.std(images, axis=(0, 1, 2)).tolist()
...
class CorpusDataset(ModelDataset):
    ...
    def __init__(self, dataset_path, tags, split_by):
        ...
        self.tags = tags
        self.size, self.tag_num_classes, self.max_token_len, self.max_sent_len, self._sents = self._load(self.path, self.tags, split_by)
    ...
    def _load(self, dataset_path, tags, split_by):
        ...
        size = len(sents)
        return (
         size, tag_num_classes, max_token_len, max_sent_len, sents)
...
class ImageFilesDatasetLazy(ModelDataset):
    ...
    def _extract_zip(self, dataset_path):
        ...
        return (
         image_paths, image_classes, num_labeled_samples, num_classes)
