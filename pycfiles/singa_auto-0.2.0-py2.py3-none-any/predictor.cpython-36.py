...
# [GCC 8.4.0]
# Embedded file name: /root/singa_auto/predictor/predictor.py
...
class Predictor:
    ...
    def _pull_job_info(self):
        ...
        with self._meta_store:
            ...
            if inference_job.train_job_id is None:
                if inference_job.model_id is None:
                    raise InvalidInferenceJobError('No train job or checkpoint found with inference ID "{}"'.format(inference_job.id))
    ...
    def _get_predictions_from_workers(self, queries: List[Any]) -> List[List[Prediction]]:
        ...
        query_id_to_predictions = defaultdict(list)
