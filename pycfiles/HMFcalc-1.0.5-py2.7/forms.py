...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.5-x86_64/egg/HMFcalc/forms.py
# Compiled at: 2013-06-13 06:35:57
"""
...
import numpy as np, floppyforms as forms
...
class HMFInput(forms.Form):
    ...
    def __init__(self, add, minm=None, maxm=None, *args, **kwargs):
        ...
        if add == 'create':
            self.fields['min_M'] = forms.FloatField(label='Minimum Mass', initial=8.0, help_text=mark_safe('Units of log<sub>10</sub>(M<sub>&#9737</sub>)'), min_value=3.0, max_value=18.0)
            self.fields['max_M'] = forms.FloatField(label='Maximum Mass', initial=15.0, help_text=mark_safe('Units of log<sub>10</sub>(M<sub>&#9737</sub>)'), min_value=3.0, max_value=18.0)
            self.fields['M_step'] = forms.FloatField(label='Mass Bin Width', initial=0.05, help_text='Logarithmic Bins', min_value=1e-05, max_value=15.0)
        ...
        if add == 'create':
            self.helper.layout = Layout(TabHolder(Tab('Run Parameters', Div(Div('z', 'overdensity', 'WDM', 'approach', 'alternate_model', css_class='span4'), Div('extrapolate', 'k_begins_at', 'k_ends_at', 'min_M', 'max_M', 'M_step', css_class='span4')), Div(Fieldset('Optional Extra Plots', InlineCheckboxes('extra_plots')), css_class='span12')), Tab('Cosmological Parameters', Div(Div('co_transfer_file', 'co_transfer_file_upload', 'cp_label', 'cp_delta_c', 'cp_n', css_class='span4'), Div('cp_sigma_8', 'cp_H0', 'cp_omegab', 'cp_omegac', 'cp_omegav', css_class='span4')))), FormActions(Submit('submit', 'Calculate!', css_class='btn btn-primary btn-large')))
        else:
            self.helper.layout = Layout(TabHolder(Tab('Run Parameters', Div(Div('z', 'overdensity', 'WDM', 'approach', css_class='span4'), Div('extrapolate', 'k_begins_at', 'k_ends_at', 'alternate_model', css_class='span4')), Div(Fieldset('Optional Extra Plots', InlineCheckboxes('extra_plots')), css_class='span12')), Tab('Cosmological Parameters', Div(Div('co_transfer_file', 'co_transfer_file_upload', 'cp_label', 'cp_delta_c', 'cp_n', css_class='span4'), Div('cp_sigma_8', 'cp_H0', 'cp_omegab', 'cp_omegac', 'cp_omegav', css_class='span4')))), FormActions(Submit('submit', 'Calculate!', css_class='btn btn-primary btn-large')))

    z = FloatListField(label='Redshifts', initial='0', help_text='Comma-separated list', max_length=50, min_val=0, max_val=1100)
    overdensity = FloatListField(label='Virial Overdensity', help_text='Comma-separated list', max_length=50, initial=178, min_val=10)
    WDM = FloatListField(label='WDM Masses', required=False, help_text='Comma-separated list. In keV (eg. 0.05)', max_length=50, min_val=0.0001)
    approach_choices = [
    ...
    approach = forms.MultipleChoiceField(label='Fitting Function', choices=approach_choices, initial=[
     'ST'], required=False)
    alternate_model = forms.CharField(label=mark_safe('Custom Fitting Function'), help_text=mark_safe('Type a fitting function form (<a href="http://docs.scipy.org/doc/numpy/reference/routines.math.html">Python syntax</a>) in terms of mass variance (denoted by x). Eg. for Jenkins: 0.315*exp(-abs(log(1.0/x)+0.61)**3.8)'), required=False, widget=forms.Textarea(attrs={'cols': '40', 'rows': '3'}))
    transfer_choices = [
     ('transfers/PLANCK_transfer.dat', 'PLANCK'),
     ...
     ('custom', 'Custom')]
    co_transfer_file = forms.ChoiceField(label='Transfer Function', choices=transfer_choices, initial='transfers/WMAP7_transfer.dat')
    co_transfer_file_upload = forms.FileField(label='Upload Transfer Function', required=False, help_text="Custom file only used if Transfer Functions is 'Custom'")
    ...
    k_ends_at = FloatListField(label='Maximum k', initial=2000.0, help_text='Highest Wavenumber Used, Comma-Separated Decimals', min_val=0.1)
    k_begins_at = FloatListField(label='Minimum k', initial=1e-08, help_text='Lowest Wavenumber Used', max_val=10, min_val=0.0)
    optional_plots = [
     ('get_ngtm', 'N(>M)'),
     ('get_mgtm', 'M(>M)'),
     ('get_nltm', 'N(<M)'),
    ...
    extra_plots = forms.MultipleChoiceField(label='Optional Extra Plots', choices=optional_plots, initial=[
     'get_ngtm'], required=False)
    cp_label = forms.CharField(label='Unique Labels', initial='WMAP7', help_text='One unique identifier for each cosmology, separated by commas')
    ...
    cp_delta_c = FloatListField(label=mark_safe('&#948<sub>c</sub>'), initial='1.686', min_val=1, max_val=3)
    cp_n = FloatListField(label=mark_safe('n<sub>s</sub> '), initial='0.967', min_val=-4, max_val=3)
    cp_sigma_8 = FloatListField(label=mark_safe('&#963<sub>8</sub>'), initial='0.81', min_val=0.1)
    cp_H0 = FloatListField(label=mark_safe('H<sub>0</sub>'), initial='70.4', min_val=10, max_val=500.0)
    cp_omegab = FloatListField(label=mark_safe('&#937<sub>b</sub>'), initial='0.0455', min_val=0.005, max_val=0.65)
    cp_omegac = FloatListField(label=mark_safe('&#937<sub>c</sub>'), initial='0.226', min_val=0.02, max_val=2.0)
    cp_omegav = FloatListField(label=mark_safe('&#937<sub>&#923</sub>'), initial='0.728', min_val=0, max_val=1.6)
    ...
    def clean(self):
        ...
        set_kbounds_time = max((len(cleaned_data.get('k_ends_at')), len(cleaned_data.get('k_begins_at')))) * len(cleaned_data.get('cp_n')) * len(cleaned_data.get('cp_sigma_8')) * 0.072
        set_WDM_time = len(cleaned_data.get('WDM')) * 0.072
        set_z_time = len(cleaned_data.get('z')) * 0.003
        get_mf_time = len(cleaned_data.get('approach')) * len(cleaned_data.get('overdensity')) * len(cleaned_data.get('cp_delta_c')) * 6.64 * 1e-05
...
class PlotChoice(forms.Form):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        self.fields['plot_choice'] = forms.ChoiceField(label='View plot of', choices=plot_choices, initial='hmf')
        self.helper = FormHelper()
    ...
    download_choice = forms.ChoiceField(label=mark_safe('<a href="../hmf.pdf" id="plot_download">Download </a>'), choices=download_choices, initial='pdf-current')
