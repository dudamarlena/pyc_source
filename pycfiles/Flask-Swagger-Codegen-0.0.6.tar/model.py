...
# [GCC 8.4.0]
# Embedded file name: /Users/rejown/Development/Project/flask_restful_swagger_codegen/flask_swagger_codegen/model.py
# Compiled at: 2015-04-17 02:27:39
import re
from collections import OrderedDict, namedtuple

class Field(object):
    default_values = {'String': 'poooo', 
       'Integer': 9263, 
       'Float': 83.75, 
       'Decimal': 83.7578, 
    ...
    def field_name(cls, name):
        if name != cls.fields_module:
            return name
        return '_%s' % name

    def __repr__(self):
        args = self.args + [ '%s=%r' % (k, v) for k, v in self.kwargs.iteritems()
                           ]
...
class Resource(object):
    ...
    @property
    def path_params(self):
        return map(lambda x: x[1], re.findall('<(.*:)?(.*?)>', self.url))
...
class SwaggerFlaskModel(object):
    ...
    def routes(self):
        ...
        for _, res in self.resources.iteritems():
            routes.append(dict(resource=res, urls=res.urls, endpoint=res.endpoint))
        ...
        for res in self.resources.values():
            for ins in res.methods.values():
                if ins.response_filter:
                    filters[(ins.parent.endpoint, ins.name)] = ins.response_filter
