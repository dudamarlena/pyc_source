...
# [GCC 8.4.0]
# Embedded file name: /Users/rejown/Development/Project/flask_restful_swagger_codegen/flask_swagger_codegen/parser.py
# Compiled at: 2015-04-17 02:28:28
import yaml
        ...
        if isinstance(node, dict):
            if '$ref' in node:
                if paths[(-1)] in ('schema', 'items'):
                    self.swagger.set_by_keys(paths, node['$ref'].split('/')[(-1)])
                else:
                    self.swagger.set_by_keys(paths, self.swagger.get_by_ref(node['$ref']))
                return
