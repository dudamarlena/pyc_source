...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/rtorrent/torrent.py
# Compiled at: 2012-04-10 19:51:02
import rtorrent.rpc, rtorrent.peer, rtorrent.tracker, rtorrent.file
...
class Torrent():
    ...
    def get_peers(self):
        ...
        """
        self.peers = []
        retriever_methods = [ m for m in rtorrent.peer.methods if m.is_retriever() if m.is_available(self._rt_obj)
                            ]
    ...
    def get_trackers(self):
        ...
        """
        self.trackers = []
        retriever_methods = [ m for m in rtorrent.tracker.methods if m.is_retriever() if m.is_available(self._rt_obj)
                            ]
        m = rtorrent.rpc.Multicall(self)
    ...
    def get_files(self):
        ...
        """
        self.files = []
        retriever_methods = [ m for m in rtorrent.file.methods if m.is_retriever() if m.is_available(self._rt_obj)
                            ]
        m = rtorrent.rpc.Multicall(self)
        m.add('f.multicall', self.info_hash, '', *[ method.rpc_call + '=' for method in retriever_methods ])
        results = m.call()[0]
        offset_method_index = retriever_methods.index(rtorrent.rpc.find_method('f.get_offset'))
    ...
    def _is_hash_checking_queued(self):
        ...
        self.hash_checking_queued = self.hashing == 3 and self.hash_checking == False
...
methods = [
 ...
 Method(Torrent, 'get_ratio', 'd.get_ratio', post_process_func=lambda x: x / 1000.0),
 ...
 Method(Torrent, 'get_num_chunks_hashed', 'd.get_chunks_hashed', aliases=(
  'get_chunks_hashed',)),
 ...
 Method(Torrent, 'get_chunks_seen', 'd.chunks_seen', min_version=(
  0, 9, 1)),
