...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/rtorrent/__init__.py
# Compiled at: 2012-04-10 19:50:42
from rtorrent.common import _py3, cmd_exists, find_torrent, is_valid_port, bool_to_int, convert_version_tuple_to_str
from rtorrent.lib.torrentparser import TorrentParser
from rtorrent.rpc import Method
from rtorrent.torrent import Torrent
import os.path, rtorrent.rpc, sys, time
if _py3:
...
class RTorrent():
    ...
    def __init__(self, url, _verbose=False):
        ...
        self._rpc_methods = []
        self._torrent_cache = []
        assert self._verify_conn(self._get_xmlrpc_conn()) is True, 'rTorrent connection failed'
        self.client_version_tuple = tuple([ int(i) for i in self._get_xmlrpc_conn().system.client_version().split('.')
                                          ])
        assert self._meets_version_requirement() is True, ('Error: Minimum rTorrent version required is {0}').format(MIN_RTORRENT_VERSION_STR)
        self.update()
        self.get_torrents()
    ...
    def _verify_conn(self, conn):
        ...
        if 'system.client_version' not in self._rpc_methods or 'system.library_version' not in self._rpc_methods:
            return False
        else:
            return True
    ...
    def get_torrents(self, view='main'):
        ...
        retriever_methods = [ m for m in methods if m.is_retriever() if m.is_available(self)
                            ]
        ...
        for result in results:
            ...
            self.torrents.append(Torrent(self, info_hash=result[0], **results_dict))
    ...
    def load_torrent_simple(self, torrent, file_type, start=False, verbose=False):
        ...
        if file_type == 'file':
            assert os.path.isfile(torrent), ('Invalid path: "{0}"').format(torrent)
...
def _build_class_methods(class_obj):
    caller = lambda self, multicall, method, *args: multicall.add(method, self.rpc_id, *args)
...
def __check_supported_methods(rt):
    from pprint import pprint
    supported_methods = set([ m.rpc_call for m in methods + rtorrent.file.methods + rtorrent.torrent.methods + rtorrent.tracker.methods + rtorrent.peer.methods
                            ])
...
methods = [
 ...
 Method(RTorrent, 'get_api_version', 'system.api_version', min_version=(
  0, 9, 1)),
 Method(RTorrent, 'get_system_time', 'system.time', docstring='Get the current time of the system rTorrent is running on\n           \n           @return: time (posix)\n           @rtype: int'),
 ...
 Method(RTorrent, 'set_bind', 'set_bind', docstring='Set address bind\n           \n           @param arg: ip address\n           @type arg: str\n           '),
 Method(RTorrent, 'set_up_limit', 'set_upload_rate', docstring='Set global upload limit (in bytes)\n           \n           @param arg: speed limit\n           @type arg: int\n           '),
 ...
 Method(RTorrent, 'set_down_limit', 'set_download_rate', docstring='Set global download limit (in bytes)\n           \n           @param arg: speed limit\n           @type arg: int\n           '),
 ...
 Method(RTorrent, 'set_ip', 'set_ip', docstring='Set IP\n           \n           @param arg: ip address\n           @type arg: str\n           '),
 Method(RTorrent, 'set_timeout_safe_sync', 'set_timeout_safe_sync'),
 Method(RTorrent, 'set_preload_type', 'set_preload_type'),
 Method(RTorrent, 'set_check_hash', 'set_check_hash', docstring='Enable/Disable hash checking on finished torrents\n        \n            @param arg: True to enable, False to disable\n            @type arg: bool\n            ', boolean=True)]
_all_methods_list = [
 methods,
...
class_methods_pair = {RTorrent: methods, 
