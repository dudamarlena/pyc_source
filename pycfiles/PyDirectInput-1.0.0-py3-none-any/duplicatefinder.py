...
# Embedded file name: build/bdist.linux-i686/egg/pydirduplicatefinder/duplicatefinder.py
# Compiled at: 2009-08-15 11:15:59
import os, os.path, sys, optparse, shutil, filecmp
...
version = '0.3.0'
description = 'Analyse all files in one or more directories and manage duplicate files (the same file present with different names)'
...
def main(args=[]):
    ...
    group = optparse.OptionGroup(p, 'Filters', 'Use those options to limit and filter directories and files to check.\nOptions belowe that rely on file or directory name support usage of jolly characters and can also be used multiple times')
