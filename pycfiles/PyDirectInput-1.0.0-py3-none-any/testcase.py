...
# Embedded file name: build/bdist.linux-i686/egg/pydirduplicatefinder/tests/testcase.py
# Compiled at: 2009-08-15 06:55:53
__author__ = 'Keul - lucafbb AT gmail.com'
import os, os.path, sys, optparse, shutil, unittest, tempfile
    ...
    def addFile(self, name, directory_path, size=None, like=None):
       ...
       """
        ...
        if size and files.has_key(name):
            raise KeyError("A file with name %s already exists (size: %s bytes). Use a different name or don't use the size attribute to use the same file." % (
             name, files[name]['size']))
