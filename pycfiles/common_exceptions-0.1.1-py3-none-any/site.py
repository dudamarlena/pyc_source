# uncompyle6 version 3.6.7
# Python bytecode 2.7 (62211)
# Decompiled from: Python 3.8.2 (tags/v3.8.2:7b3ab59, Feb 25 2020, 23:03:10) [MSC v.1916 64 bit (AMD64)]
# Embedded file name: build/bdist.macosx-10.11-x86_64/egg/common_dibbs/clients/ar_client/models/site.py
# Compiled at: 2016-12-14 11:02:40
__doc__ = '\n    Appliance Registry API\n\n    Store appliances with the Appliance Registry API.\n\n    OpenAPI spec version: 0.1.13\n    \n    Generated by: https://github.com/swagger-api/swagger-codegen.git\n\n    Licensed under the Apache License, Version 2.0 (the "License");\n    you may not use this file except in compliance with the License.\n    You may obtain a copy of the License at\n\n        http://www.apache.org/licenses/LICENSE-2.0\n\n    Unless required by applicable law or agreed to in writing, software\n    distributed under the License is distributed on an "AS IS" BASIS,\n    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.\n    See the License for the specific language governing permissions and\n    limitations under the License.\n'
from pprint import pformat
from six import iteritems
import re

class Site(object):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """

    def __init__(self, name=None, type=None, contact_url=None, appliances=None):
        """
        Site - a model defined in Swagger

        :param dict swaggerTypes: The key is attribute name
                                  and the value is attribute type.
        :param dict attributeMap: The key is attribute name
                                  and the value is json key in definition.
        """
        self.swagger_types = {'name': 'str', 
           'type': 'str', 
           'contact_url': 'str', 
           'appliances': 'list[str]'}
        self.attribute_map = {'name': 'name', 
           'type': 'type', 
           'contact_url': 'contact_url', 
           'appliances': 'appliances'}
        self._name = name
        self._type = type
        self._contact_url = contact_url
        self._appliances = appliances

    @property
    def name(self):
        """
        Gets the name of this Site.
        Name of the site

        :return: The name of this Site.
        :rtype: str
        """
        return self._name

    @name.setter
    def name(self, name):
        """
        Sets the name of this Site.
        Name of the site

        :param name: The name of this Site.
        :type: str
        """
        self._name = name

    @property
    def type(self):
        """
        Gets the type of this Site.
        Type of the site (e.g. openstack)

        :return: The type of this Site.
        :rtype: str
        """
        return self._type

    @type.setter
    def type(self, type):
        """
        Sets the type of this Site.
        Type of the site (e.g. openstack)

        :param type: The type of this Site.
        :type: str
        """
        self._type = type

    @property
    def contact_url(self):
        """
        Gets the contact_url of this Site.
        Contact URL of the site

        :return: The contact_url of this Site.
        :rtype: str
        """
        return self._contact_url

    @contact_url.setter
    def contact_url(self, contact_url):
        """
        Sets the contact_url of this Site.
        Contact URL of the site

        :param contact_url: The contact_url of this Site.
        :type: str
        """
        self._contact_url = contact_url

    @property
    def appliances(self):
        """
        Gets the appliances of this Site.

        :return: The appliances of this Site.
        :rtype: list[str]
        """
        return self._appliances

    @appliances.setter
    def appliances(self, appliances):
        """
        Sets the appliances of this Site.

        :param appliances: The appliances of this Site.
        :type: list[str]
        """
        self._appliances = appliances

    def to_dict(self):
        """
        Returns the model properties as a dict
        """
        result = {}
        for attr, _ in iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(lambda x: x.to_dict() if hasattr(x, 'to_dict') else x, value))
            elif hasattr(value, 'to_dict'):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(lambda item: (item[0], item[1].to_dict()) if hasattr(item[1], 'to_dict') else item, value.items()))
            else:
                result[attr] = value

        return result

    def to_str(self):
        """
        Returns the string representation of the model
        """
        return pformat(self.to_dict())

    def __repr__(self):
        """
        For `print` and `pprint`
        """
        return self.to_str()

    def __eq__(self, other):
        """
        Returns true if both objects are equal
        """
        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """
        Returns true if both objects are not equal
        """
        return not self == other