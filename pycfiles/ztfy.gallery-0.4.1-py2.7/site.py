...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/gallery/skin/site.py
# Compiled at: 2013-09-21 08:54:45
...
class SiteManagerIndexView(BaseSiteManagerIndexView):
    ...
    def categories(self):
        ...
        if catalog is not None:
            ...
            if index is not None:
                intids = getUtility(IIntIds)
                return sorted(((id, intids.queryObject(id)) for id in index.values()), key=lambda x: II18n(x[1]).queryAttribute('shortname', request=self.request))
        return []
    ...
    def getCategory(self, topic):
        ...
        if not categories:
            return ''
        return (' - ').join('<a href="%s">%s</a>' % (absoluteURL(category, self.request), II18n(category).queryAttribute('shortname', request=self.request)) for category in categories)
    ...
    def __call__(self):
        ...
        if image is not None:
            ...
            if display is not None:
                return writer.write({'url': absoluteURL(display, self.request), 'title': II18n(image).queryAttribute('title', request=self.request)})
