...
# Embedded file name: build/bdist.linux-x86_64/egg/buildoutstarter/__init__.py
# Compiled at: 2013-03-14 01:46:30
...
def main():
    parser = argparse.ArgumentParser(description='Creates a zc.buildout Python project ' + 'with a given name.')
    parser.add_argument('name', help='The name of the project to create.')
    parser.add_argument('parent_dir', help='The optional parent directory to create the ' + 'project in. Defaults to the current directory.', nargs='?', default=os.getcwd())
    parser.add_argument('-p', '--package-name', help='The package name to use when creating ' + 'the project. By default, the [name] is sanitized and used, but there are times ' + 'when this yields undesirable results. Pass a custom name here to force a specific ' + 'package name.', default=None, required=False)
    parser.add_argument('--django', help='Create a Django project rather than a ' + 'simple Python project.', action='store_true')
    parser.add_argument('--ipython', help='Generate an IPython interpreter in addition ' + 'to the regular Python interpreter in the bin directory.', action='store_true')
    parser.add_argument('--pydev', help='Generate PyDev Eclipse project files.', action='store_true')
    parser.add_argument('--git', help='Generate a Git repository in the newly ' + 'created project. Includes .gitignore definitions.', action='store_true')
    args = parser.parse_args()
    parameters = GeneratorParameters(directory=os.path.join(args.parent_dir, args.name), project_name=args.name, sanitized_project_name=args.package_name if args.package_name else sanitize_name(args.name), is_django=args.django, is_ipython=args.ipython, is_pydev=args.pydev, is_git=args.git)
...
def generate_project_contents(parameters):
    ...
    if not parameters.is_django:
        os.makedirs(os.path.join(parameters.directory, 'src', parameters.sanitized_project_name))
        with open(os.path.join(parameters.directory, 'src', parameters.sanitized_project_name, '__init__.py'), 'w'):
            pass
    else:
...
def generate_django_project(parameters):
    ...
    subprocess.call([os.path.join('bin', 'python'), '-c',
     'from django.core.management import call_command ; ' + ("call_command('startproject', '{}', 'src')").format(parameters.sanitized_project_name)], cwd=parameters.directory)
...
def sanitize_name(project_name):
    ...
    project_name = re.sub('[^_A-Za-z0-9]', '', project_name)
    return project_name
...
class GeneratorParameters:
    ...
    def __init__(self, directory=None, project_name=None, sanitized_project_name=None, is_django=False, is_ipython=False, is_pydev=False, is_git=False):
    ...
    def to_dict(self):
        return {'directory': self.directory, 'project_name': self.project_name, 'sanitized_project_name': self.sanitized_project_name, 
           'is_django': self.is_django, 
           'is_ipython': self.is_ipython, 'is_pydev': self.is_pydev, 
           'is_git': self.is_git}
