...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\moda\evaluators\eval_all_models.py
# Compiled at: 2018-10-31 09:33:17
# Size of source mod 2**32: 12845 bytes
import json, os, numpy as np
from moda.dataprep import read_data
from moda.evaluators import eval_models, get_metrics_for_all_categories, summarize_metrics, get_final_metrics, eval_models_CV
...
def evaluate_all_models(datapath='SF3H_labeled.csv', min_date='01-01-2018', freq='3H', use_comet=False, models_to_run=[], window_size_for_metrics=1):
    try:
        ...
        if 'lstm' in models_to_run:
            ...
            result = eval_models(X, y, [model], label_col_name='label', train_percent=20, window_size_for_metrics=window_size_for_metrics)
        ...
        if 'ma_seasonal' in models_to_run or len(models_to_run) == 0:
            ...
            for num_std in (2.5, 3, 3.5, 4):
                for anomaly_type in anomaly_types:
                    model = MovingAverageSeasonalTrendinessDetector(is_multicategory=True, freq=freq, min_value=min_value, anomaly_type=anomaly_type,
                      num_of_std=num_std)
                    result = eval_models_CV(X, y, [model], label_col_name='label', n_splits=5, window_size_for_metrics=window_size_for_metrics)
            ...
            for num_std in (2.5, 3, 3.5, 4):
                for anomaly_type in anomaly_types:
                    for lo_frac in (0.1, 0.5, 1, 1.5):
                        model = STLTrendinessDetector(is_multicategory=True, freq=freq, min_value=min_value, anomaly_type=anomaly_type,
                          num_of_std=num_std,
                          lo_frac=lo_frac)
                        result = eval_models(X, y, [model], label_col_name='label', train_percent=20, window_size_for_metrics=window_size_for_metrics)
            ...
            for max_anoms in max_anoms_list:
                for threshold in (None, 'med_max', 'p95', 'p99'):
                    for alpha in (0.05, 0.1, 0.15):
                        model = TwitterAnomalyTrendinessDetector(is_multicategory=True, freq=freq, min_value=min_value, threshold=threshold,
                          max_anoms=max_anoms,
                          longterm=False,
                          alpha=alpha,
                          seasonality_freq=7)
        ...
        if 'azure' in models_to_run or len(models_to_run) == 0:
            ...
            sensitivity = [
             None, 0.3, 0.5, 1, 1.5, 2]
            for sens in sensitivity:
                print('sensitivity = {}'.format(sens))
                prediction = model.tune_prediction(prediction, sens)
                raw_metrics = get_metrics_for_all_categories((X[['value']]), (prediction[['prediction']]), (y[['label']]), window_size_for_metrics=5)
...
def print_azure_model(datapath, min_value, model, result, sens):
    ...
    print('sens = ' + str(sens) + ' min_value = ' + str(min_value) + ', dataset = ' + datapath)
    print('F1 score = ' + str(result[model.__name__]['f1']) + ', precision = ' + str(result[model.__name__]['precision']) + ', recall = ' + str(result[model.__name__]['recall']))
...
def print_twitter_result(alpha, datapath, min_value, model, result, threshold):
    ...
    if threshold is None:
        print('Threshold = None, Alpha = ' + str(alpha) + ', max_anoms = None, min_value = ' + str(min_value) + ', dataset = ' + datapath)
    else:
        print('Threshold = ' + threshold + ', Alpha = ' + str(alpha) + ', max_anoms = None, min_value = ' + str(min_value) + ', dataset = ' + datapath)
    print('F1 score = ' + str(result[model.__name__]['f1']) + ', precision = ' + str(result[model.__name__]['precision']) + ', recall = ' + str(result[model.__name__]['recall']))


def print_stl_result(anomaly_type, datapath, min_value, model, num_std, result):
    print('Evaluating STL')
    print('num_std = ' + str(num_std) + ', anomaly_type = ' + str(anomaly_type) + ', min_value = ' + str(min_value) + ', dataset = ' + datapath)
    print('F1 score = ' + str(result[model.__name__]['f1']) + ', precision = ' + str(result[model.__name__]['precision']) + ', recall = ' + str(result[model.__name__]['recall']))
...
if __name__ == '__main__':
    ...
    while model not in ('s', 'm', 't', 'a', 'l', 'all'):
        model = input('Select model to run: stl (s), ma_seasonal (m), twitter (t), azure anomaly finder (a), lstm (l) or all (all): ')
