...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/cli/pdb/cmd_prep.py
# Compiled at: 2019-11-20 15:49:29
# Size of source mod 2**32: 7955 bytes
...
DISU_THRESH_MIN = 1.6621792464987384
DISU_THRESH_MAX = 7.958857834400516
...
def cli(pdb_file, chains, smod, clean_pdb, minimize, prm, rtf, xplor_psf, out_prefix, psfgen, nmin, nsteps, auto_disu, delete_tmp):
    ...
    try:
        ...
        if clean_pdb:
            records = list(fix_atom_records(parse_pdb_stream(pdb_file, only_atoms=True)))
        else:
        ...
        segments = OrderedDict(item for item in all_segments.items() if item[0][1] in chains or is_wildcard and item[0][1][0] != 'h')
        records = list(r for segment in segments.values() for r in parse_pdb_stream(open(segment)))
        p = subprocess.Popen([psfgen_bin], stdin=(subprocess.PIPE),
        ...
        for (segid, chain), segment in segments.items():
            ...
            seg_cmd = 'segment {0} {{ first NONE; last NONE; pdb {1} }}'.format(segid, segment)
        ...
        if auto_disu:
            for ri, rj in combinations(filter(is_cys_sulfur, records), 2):
                if ri.resnum == rj.resnum:
                    if ri.segment == rj.segment:
                        continue
        ...
        if minimize:
            if nsteps > 0:
                ...
                with open(input_file, 'r') as (inp):
                    with open(fixed_atoms_file, 'w') as (out):
                        for line in inp:
                            if line.startswith('ATOM') and line[55:60] != ' 0.00':
                                out.write(line)

                cmd = [
                 nmin, '{0}.psf'.format(out_prefix), prm, rtf,
        ...
        logger.exception('Unexpected error: Working files location in {}'.format(workdir))
        raise
    else:
        if delete_tmp:
            import shutil
