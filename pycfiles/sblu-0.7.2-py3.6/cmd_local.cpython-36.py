...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/cli/cluspro/cmd_local.py
...
def run_enzyme(rec, lig, charmm_rtf, charmm_prm, rotations, output_dir):
...
@click.option('-m', '--mode', default='enzyme', type=(click.Choice(['enzyme', 'other', 'antibody'])))
def cli(output_dir, mode, rec, lig):
    if output_dir is None:
        ...
        output_dir.mkdir_p()
        if mode == 'enzyme':
            run_enzyme(rec, lig, PRMS_DIR / 'charmm' / 'charmm_param.rtf', PRMS_DIR / 'charmm' / 'charmm_param.prm', PRMS_DIR / 'rotsets' / 'rot70k.mol2.prm', output_dir)
        else:
            if mode == 'other':
                run_other()
            else:
