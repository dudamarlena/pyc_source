...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/cli/pdb/cmd_pdb_psf_concat.py
# Compiled at: 2018-10-15 19:16:31
# Size of source mod 2**32: 2702 bytes
import sys, logging, subprocess, click
...
def cli(files, psfgen, prefix, rtf):
    ...
    def psf_concat(psf_files, rtf_file, prefix, psfgen='psfgen'):
        p = subprocess.Popen([psfgen], stdin=(subprocess.PIPE),
                        ...
                        if line.startswith('ATOM  '):
                            ...
                            if atomi >= 99900:
                                if last_res_id != res_id:
                                    atomi = 1
                                last_res_id = res_id
                                line = '{0}{1:5d}{2}'.format(line[0:6], atomi, line[11:])
                            else:
