...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/rmsd.py
...
def interface_pwrmsd(rec, lig, interface_d=10.0, condensed=False):
    ...
    N = lig.numCoordsets()
    interfaces = [interface(rec.getCoords(), coords, interface_d) for coords in transformed]
    ...
    for i in range(N):
        for j in range(i, N):
            ...
            if len(combined_interface) == 0:
                ...
                d = rmsd(transformed[i][combined_interface], transformed[j][combined_interface])
...
def srmsd(crys, pdbs, only_ca=False, only_backbone=False, only_interface=False, interface_radius=10.0, rec=None):
