...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/cli/measure/cmd_ftrmsd.py
...
@click.option('-n', '--nftresults', type=(click.INT),
  default=None)
...
@click.option('--only-interface', is_flag=True, default=False, help='Only use inteface atoms. Requires --rec.')
@click.option('--interface-radius', type=(click.FLOAT), default=10.0, help='Radius around receptor to consider.')
@click.option('--rec', type=click.Path(exists=True), default=None, help='Receptor file if using interface mode.')
@click.option('-o', '--output', type=click.File(mode='w'), default=(click.open_file('-', 'w')),
  help='Write output to file (default: stdout)')
def cli(lig_file, lig_crys, ftfile, rotprm, sort_ftresults, nftresults, only_ca, only_backbone, only_interface, interface_radius, rec, output):
