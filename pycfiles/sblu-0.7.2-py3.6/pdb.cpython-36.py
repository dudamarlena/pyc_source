...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/pdb.py
# Compiled at: 2019-11-15 16:48:30
# Size of source mod 2**32: 9765 bytes
...
RECORD_TYPES = ('HEADER', 'SOURCE', 'AUTHOR', 'OBSLTE', 'KEYWDS', 'REVDAT', 'TITLE ',
                'EXPDTA', 'SPRSDE', 'SPLT  ', 'NUMMDL', 'JRNL  ', 'CAVEAT', 'MDLTYP',
                'REMARKS', 'COMPND', 'DBREF ', 'SEQADV', 'MODRES', 'DBREF1', 'DBREF2',
                'SEQRES', 'HET   ', 'HETNAM', 'HETSYN', 'FORMUL', 'HELIX ', 'SHEET ',
                'SSBOND', 'LINK  ', 'CISPEP', 'SITE  ', 'CRYST1', 'ORIGXn', 'SCALEn',
                'MTRIXn', 'MODEL ', 'ANISOU', 'HETATM', 'ATOM  ', 'TER   ', 'ENDMDL',
                'CONECT', 'MASTER', 'END   ')
SKIP_RESIDUES = ('HOH', 'WAT', 'ACE', 'NME')
RECORD_FIXES = ('MSE', 'PTR', 'HRG', 'DPP', 'HSD')
RESNAME_FIXES = {'ASH':'ASPH', 
 'GLH':'GLUH', 
 'CYX':'CYS', 
 'HIP':'HSC', 
 'HID':'HIS', 
...
def _field_parser(_slice, _type=str):
    ...
    def _parser(line):
        if len(line) > _slice.start:
            return _type(line[_slice])
        else:
            return _type()
...
ATOM_RECORD_FORMAT = '{record:<6s}{serial:5d} {name:4s}{altloc:1s}{resname:3s} {chain:1s}{resnum:4d}{icode:1s}   {x:8.3f}{y:8.3f}{z:8.3f}{occupancy:6.2f}{beta:6.2f}      {segment:<4s}{element:>2s}{charge:2s}\n'
ATOM_RECORD_SHORT_FORMAT = '{record:<6s}{serial:5d} {name:4s}{altloc:1s}{resname:3s} {chain:1s}{resnum:4d}{icode:1s}'
...
class AtomRecord(PDBRecord):
    ...
    def __repr__(self):
        return ATOM_RECORD_SHORT_FORMAT.format(record=(self.record_type),
          serial=(self.serial),
          name=(self.name),
          altloc=(self.altloc),
    ...
    def __str__(self):
        return ATOM_RECORD_FORMAT.format(record=(self.record_type),
          serial=(self.serial),
          name=(self.name),
          altloc=(self.altloc),
          resname=(self.resname),
...
def fix_atom_records(records, no_skips=False, all_het_to_atom=False, no_record_fixes=False, no_resname_fixes=False):
    ...
    for record in records:
        if record.record_type not in ('ATOM  ', 'HETATM'):
            ...
            if not no_skips:
                if record.resname in SKIP_RESIDUES:
                    continue
                fix_record = all_het_to_atom or not no_record_fixes and record.resname in RECORD_FIXES
                if fix_record:
                    record.record_type = 'ATOM  '
            else:
                if not no_resname_fixes:
                    if record.resname in RESNAME_FIXES:
...
def split_segments(records, add_segid=False, only_atoms=True):
    ...
    def end_segment(record):
        if last_atom_record is None:
            ...
            return resnum_delta not in (0, 1) or resnum_delta == 0 and last_atom_record.icode != record.icode or record.record_type == 'HETATM' and last_atom_record.resname != record.resname or last_atom_record.record_type == 'HETATM' and last_atom_record.resname != record.resname or record.chain != last_atom_record.chain
