...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/cli/cluspro/cmd_minimize.py
...
def cli(rec_pdb, rec_psf, lig_pdb, lig_psf, prm, rtf, subunits, output_prefix, clusters, ftfile, rotations):
    ...
    with open(ftfile, 'r') as (ifp):
        with os.fdopen(tmp_fd, 'w') as (ofp):
    ...
    try:
        ...
        logger.error('error running complex_refine: temporary ftfile located at {}'.format(tmp_filepath))
        raise
    else:
