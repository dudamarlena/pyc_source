...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sblu/ft.py
...
def read_rotations_stream(stream, limit=None):
    ...
    if limit is provided.
    """
    rotations = np.loadtxt(islice(iter(stream), 0, limit))
...
def read_ftresults_stream(stream, limit=None):
    ...
    """
    stream = iter(stream)
    return np.loadtxt((islice(stream, 0, limit)),
...
def apply_ftresults_atom_group(atom_group, ftresults, rotations, center=None, out=None):
    ...
    if out is None:
        out = atom_group.copy()
    new_coords = np.dot(rotations[ftresults['roti']], orig_coords.T).transpose(0, 2, 1)
