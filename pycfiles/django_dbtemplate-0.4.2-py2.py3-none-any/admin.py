...
# [GCC 8.4.0]
# Embedded file name: /home/kubus/workspace/django-dbtemplate/dbtemplate/admin.py
# Compiled at: 2015-06-11 11:02:26
import sys, yaml, cgitb, mimetypes
...
class TemplateAdminForm(forms.ModelForm):
    ...
    def clean_specs(self):
        try:
            ...
            raise forms.ValidationError(_('YAML parse error: %s') % e)

        if not isinstance(data, (dict,)):
            raise forms.ValidationError(_('Dictionary required.'))
        if not data.get('description'):
            raise forms.ValidationError(_("Key 'description' required."))
        if not data.get('context'):
            raise forms.ValidationError(_("Key 'context' required."))
...
class TemplateAdmin(admin.ModelAdmin):
    ...
    def render_change_form(self, request, context, obj, *args, **kwargs):
        ...
        return super(TemplateAdmin, self).render_change_form(request, context, *args, **kwargs)
    ...
    specs_description.short_description = _('description')

    def get_urls(self):
        urls = patterns('', url('(\\d+)/try/', self.admin_site.admin_view(self.try_render), name='database_template_try_render'))
    ...
    def try_render(self, request, pk):
        ...
        return HttpResponse(result, content_type=content_type, status=status_code)
