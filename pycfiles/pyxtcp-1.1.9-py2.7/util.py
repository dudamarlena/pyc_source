...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyxtcp/util.py
# Compiled at: 2015-10-06 09:11:50
...
CONNECTION_PREFIX = {CONNECTION_TYPE_IN_REQUEST: '-', 
...
class MessageUtils(object):
    ...
    def __init__(self):
        self.header_delimiter = '"r"n'
        self.header_delimiter_len = len(self.header_delimiter)
        self.header_item_delimiter = '"t'
        self.body_suffix = '"r"n'
        self.body_suffix_len = len(self.body_suffix)

    def encrypt(self, tube):
        connection_prefix = CONNECTION_PREFIX[tube.type_]
        return ('{type_}{topic_len}{item_delimiter}{topic}{item_delimiter}{body_len}{header_delimiter}{body}{body_suffix}').format(type_=connection_prefix, topic_len=len(tube.topic), item_delimiter=self.header_item_delimiter, topic=tube.topic, body_len=len(tube.body), header_delimiter=self.header_delimiter, body=tube.body, body_suffix=self.body_suffix)
    ...
    def parse_header(self, connection_type, message):
        ...
        if message[0] != connection_prefix:
            raise RPCInputError(('Malformed jx message from {}').format(message))
        ...
        if len(topic_msg) != topic_len:
            raise RPCInputError(('Multiple unequal topic length: {}, {}').format(topic_msg, topic_len))
        if topic_len <= 0:
            raise RPCInputError('Malformed jx message. message is empty')
        return Storage({'topic': topic_msg, 
           'body_len': body_len})

    def parse_body(self, message):
        if not message or len(message) < self.body_suffix_len or message[-self.body_suffix_len:] != self.body_suffix:
            raise RPCInputError(('Malformed js body message from {}').format(message))
