...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyxtcp/tcp/tornado/multi_client.py
# Compiled at: 2015-10-02 21:55:44
import Queue, functools, threading, traceback
...
class RPCClient(object):
    ...
    def _create_new_connection(self):
        self._connection = _ClientConnection(client=self, io_loop=self._io_loop)
...
class _ClientConnection(BasicConnection):
    ...
    def connect(self):
        ...
        def _connect():
            return self.client.tcp_client.connect(host=self.client_config.host, port=self.client_config.port, max_buffer_size=self.client_config.max_buffer_size, callback=self._on_connection_success)
...
class _ClientConnectionTubes(object):
    ...
    def append(self, connection_item):
        ...
        if not len(self.active) < self.client_config.max_clients:
            waiting_timeout_handle = self._io_loop.add_timeout(deadline=self._io_loop.time() + self.connection_item.waiting_timeout, callback=functools.partial(self._on_waiting_timeout, key))
        else:
    ...
    def _read_message(self, connection_item):
        try:
            ...
            header_data_future = self.connection.stream.read_until_regex(regex=message_utils.header_delimiter, max_bytes=connection_item.header_max_bytes)
            if connection_item.header_timeout is None:
                ...
                try:
                    header_tube = message_utils.parse_header(CONNECTION_TYPE_IN_REQUEST, self._header_data)
                except RPCInputError as e:
                    log.warn(e.error)
                    raise gen.Return(False)

                self._message.topic = header_tube.topic
                body_data_future = self.connection.stream.read_bytes(header_tube.body_len + len(message_utils.body_suffix))
