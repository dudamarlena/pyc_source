...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyxtcp/tcp/tornado/simple_client.py
# Compiled at: 2015-10-10 21:35:02
import json, functools, traceback
...
class _RPCClientConfig(object):
    ...
    def __init__(self, host, port, max_clients=5, max_buffer_size=None, max_response_size=None, connect_timeout=0.2):
        self.host = host
        self.port = port
...
class SimpleRPCClient(object):
    ...
    def __init__(self, host, port, max_clients=5, max_buffer_size=None, max_response_size=None, connect_timeout=0.2):
        ...
        self.client_config = _RPCClientConfig(host=host, port=port, max_clients=max_clients, max_buffer_size=max_buffer_size, max_response_size=max_response_size, connect_timeout=connect_timeout)
        self.tcp_client = TCPClient(io_loop=self._io_loop)

    def fetch(self, item):
        connection = _ClientConnection(client=self, io_loop=self._io_loop)
...
class _ClientConnection(BasicConnection):
    ...
    def connect(self, connection_item):
        self.set_connection_timeout_handler(self._io_loop.add_timeout(deadline=self._io_loop.time() + self.client_config.connect_timeout, callback=self._on_connection_timeout))
        return self.client.tcp_client.connect(host=self.client_config.host, port=self.client_config.port, max_buffer_size=self.client_config.max_buffer_size, callback=functools.partial(self._on_connection_success_item, connection_item))
    ...
    def _on_connection_success(self, connection_item):
        stream = yield self.client.tcp_client.connect(host=self.client_config.host, port=self.client_config.port, max_buffer_size=self.client_config.max_buffer_size)
    ...
    def _on_connection_success_item(self, connection_item, stream):
        ...
        log.debug(('Connection Success {}').format(self.client_config.address_str))
        try:
            try:
                self.stream = stream
                ...
                if read_status:
                    connection_item.callback(RPCMessage(CONNECTION_TYPE_IN_RESPONSE, self._message.topic, self._message.body))
                else:
    ...
    def _read_message(self, connection_item):
        try:
            ...
            header_data_future = self.stream.read_until_regex(regex=message_utils.header_delimiter, max_bytes=connection_item.header_max_bytes)
            if connection_item.header_timeout is None:
                self._header_data = yield header_data_future
            else:
                try:
                    self._header_data = yield gen.with_timeout(timeout=self._io_loop.time() + connection_item.header_timeout, future=header_data_future, io_loop=self._io_loop)
                except gen.TimeoutError:
                    log.error(('Timeout reading header from {}').format(self.client_config.address_str))
                    raise gen.Return(False)

                try:
                    header_tube = message_utils.parse_header(CONNECTION_TYPE_IN_RESPONSE, self._header_data)
                except RPCInputError as e:
                    log.error(e.error)
                    raise gen.Return(False)

                self._message.topic = header_tube.topic
                body_data_future = self.stream.read_bytes(header_tube.body_len + len(message_utils.body_suffix))
                if connection_item.body_timeout is None:
                    self._body_data = yield body_data_future
                else:
                    try:
                        self._body_data = yield gen.with_timeout(timeout=self._io_loop.time() + connection_item.body_timeout, future=body_data_future, io_loop=self._io_loop)
                    except gen.TimeoutError:
...
class RPCClientItem(object):
    ...
    def __init__(self, item, callback, header_max_bytes=None, header_timeout=None, body_max_bytes=None, body_timeout=None, waiting_timeout=0.2):
        assert callback is not None
        self.item = item
        self.callback = callback
        self.header_max_bytes = header_max_bytes or 1024
        self.header_timeout = header_timeout or 10
        self.body_max_bytes = body_max_bytes or 10485760
