...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyxtcp/tcp/tornado/server.py
# Compiled at: 2015-10-06 08:39:32
import traceback
...
class _ServerConfig(object):
    ...
    def __init__(self, header_max_bytes=None, header_timeout=None, body_max_bytes=None, body_timeout=None):
...
class RPCServer(TCPServer):
    ...
    def __init__(self, server_callback, io_loop=None, max_buffer_size=None, read_chunk_size=None, read_header_max_bytes=None, read_header_timeout=None, read_body_max_bytes=None, read_body_timeout=None):
        ...
        TCPServer.__init__(self, io_loop=self._io_loop, max_buffer_size=max_buffer_size, read_chunk_size=read_chunk_size)
        self.server_callback = server_callback
        self.server_config = _ServerConfig(header_max_bytes=read_header_max_bytes, header_timeout=read_header_timeout, body_max_bytes=read_body_max_bytes, body_timeout=read_body_timeout)
        ...
        try:
            try:
                ...
                if read_status:
                    ...
                    response_message = self._handle_server_callback(RPCMessage(CONNECTION_TYPE_IN_REQUEST, request_message['topic'], request_message['body']))
...
class _ConnectionUtils(object):
    ...
    def _read_message(self):
        try:
            header_data_future = self.stream.read_until_regex(regex=message_utils.header_delimiter, max_bytes=self.server_config.header_max_bytes)
            if self.server_config.header_timeout is None:
                self._header_data = yield header_data_future
            else:
                try:
                    self._header_data = yield gen.with_timeout(timeout=self._io_loop.time() + self.server_config.header_timeout, future=header_data_future, io_loop=self._io_loop)
                ...
                if self.server_config.body_timeout is None:
                    ...
                    try:
                        self._body_data = yield gen.with_timeout(timeout=self._io_loop.time() + self.server_config.body_timeout, future=body_data_future, io_loop=self._io_loop)
                    except gen.TimeoutError:
