...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteDeploy-1.5.0-py2.6.egg/paste/deploy/loadwsgi.py
...
class _App(_ObjectType):
    ...
    def invoke(self, context):
        if context.protocol in ('paste.composit_factory', 'paste.composite_factory'):
            ...
            if context.protocol == 'paste.app_factory':
                return fix_call(context.object, context.global_conf, **context.local_conf)
            raise 0 or AssertionError, 'Protocol %r unknown' % context.protocol
...
class _Filter(_ObjectType):
    ...
    def invoke(self, context):
        if context.protocol == 'paste.filter_factory':
            ...
            return fix_call(context.object, context.global_conf, **context.local_conf)
            if context.protocol == 'paste.filter_app_factory':
...
class _Server(_ObjectType):
    ...
    def invoke(self, context):
        if context.protocol == 'paste.server_factory':
            ...
            return fix_call(context.object, context.global_conf, **context.local_conf)
            if context.protocol == 'paste.server_runner':
...
class ConfigLoader(_Loader):
    ...
    def get_context(self, object_type, name=None, global_conf=None):
        if self.absolute_name(name):
            ...
            if section.startswith('filter-app:'):
                context = self._filter_app_context(object_type, section, name=name, global_conf=global_conf, local_conf=local_conf, global_additions=global_additions)
            elif section.startswith('pipeline:'):
                context = self._pipeline_app_context(object_type, section, name=name, global_conf=global_conf, local_conf=local_conf, global_additions=global_additions)
            ...
            if filter_with is not None:
                filter_with_context = LoaderContext(obj=None, object_type=FILTER_WITH, protocol=None, global_conf=global_conf, local_conf=local_conf, loader=self)
    ...
    def _filter_app_context(self, object_type, section, name, global_conf, local_conf, global_additions):
        if 'next' not in local_conf:
            raise LookupError("The [%s] section in %s is missing a 'next' setting" % (
    ...
    def _pipeline_app_context(self, object_type, section, name, global_conf, local_conf, global_additions):
        if 'pipeline' not in local_conf:
            raise LookupError("The [%s] section in %s is missing a 'pipeline' setting" % (
             section, self.filename))
    ...
    def find_config_section(self, object_type, name=None):
        ...
        if not possible:
            raise LookupError('No section %r (prefixed by %s) found in config %s' % (
             ...
             self.filename))
        if len(possible) > 1:
            raise LookupError('Ambiguous section names %r for section %r (prefixed by %s) found in config %s' % (
             possible, name,
             (' or ').join(map(repr, _flatten(object_type.config_prefixes))),
             self.filename))
        return possible[0]
...
class EggLoader(_Loader):
    ...
    def get_context(self, object_type, name=None, global_conf=None):
        ...
        return LoaderContext(entry_point, object_type, protocol, global_conf or {}, {}, self, distribution=pkg_resources.get_distribution(self.spec), entry_point_name=ep_name)
    ...
    def find_egg_entry_point(self, object_type, name=None):
        ...
        if not possible:
            ...
            raise LookupError('Entry point %r not found in egg %r (dir: %s; protocols: %s; entry_points: %s)' % (
             name, self.spec,
             dist.location,
             (', ').join(_flatten(object_type.egg_protocols)),
             (', ').join(_flatten([ (pkg_resources.get_entry_info(self.spec, prot, name) or {}).keys() for prot in protocol_options
                     ] or '(no entry points)'))))
        if len(possible) > 1:
            raise LookupError('Ambiguous entry points for %r in egg %r (protocols: %s)' % (
