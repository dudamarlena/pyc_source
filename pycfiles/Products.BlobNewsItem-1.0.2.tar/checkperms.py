...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/checkperms.py
...
modes = [
 ...
 (1024, 'setgid bit', 'setgid bit: make contents inherit permissions from directory'),
...
class _SymLink(_Rule):
    ...
    def check(self, path):
        assert path == self.path, '_Symlink should only be passed specific path %s (not %s)' % (
         self.path, path)
        try:
            ...
            if e.errno != 22:
                raise
            return [
             'Path %s is not a symlink (should point to %s)' % (
              path, self.dest)]

        if link != self.dest:
            return [
             'Path %s should symlink to %s, not %s' % (
              path, self.dest, link)]
        return []
