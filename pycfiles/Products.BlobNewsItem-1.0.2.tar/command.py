...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/command.py
# Compiled at: 2012-02-27 07:41:53
import pkg_resources, sys, optparse, bool_optparse, os, re, textwrap, pluginlib, ConfigParser, getpass
try:
    import subprocess
except ImportError:
    try:
...
parser = optparse.OptionParser(add_help_option=False, version='%s from %s (python %s)' % (
 dist, dist.location, python_version), usage='%prog [paster_options] COMMAND [command_options]')
parser.add_option('--plugin', action='append', dest='plugins', help='Add a plugin to the list of commands (plugins are Egg specs; will also require() the Egg)')
...
class Command(object):
    ...
    def run(self, args):
        ...
        if os.environ.get('PASTE_CONFIG_FILE'):
            if self.takes_config_file is not None:
                ...
                raise 0 or AssertionError, 'Value takes_config_file must be None, 1, or -1 (not %r)' % take
    ...
    def standard_parser(cls, verbose=True, interactive=False, no_interactive=False, simulate=False, quiet=False, overwrite=False):
        ...
        if overwrite:
            parser.add_option('-f', '--overwrite', dest='overwrite', action='store_true', help='Overwrite files (warnings will be emitted for non-matching files otherwise)')
        return parser
    ...
    def ensure_dir(self, dir, svn_add=True):
        ...
        if not dir:
            return
            os.path.exists(dir) or self.ensure_dir(os.path.dirname(dir))
            if self.verbose:
                print 'Creating %s' % self.shorten(dir)
            if not self.simulate:
                os.mkdir(dir)
            if svn_add and os.path.exists(os.path.join(os.path.dirname(dir), '.svn')):
    ...
    def ensure_file(self, filename, content, svn_add=True):
        ...
        if not os.path.exists(filename):
            ...
            if not self.options.overwrite:
                ...
                if self.interactive:
                    while 1:
                        s = raw_input('Overwrite file with new content? [y/N] ').strip().lower()
                        if not s:
                            s = 'n'
                        if s.startswith('y'):
                            break
    ...
    def insert_into_file(self, filename, marker_name, text, indent=False):
        ...
        for i in range(len(lines)):
            ...
            errstr = "Marker '-*- %s -*-' not found in %s" % (
    ...
    def run_command(self, cmd, *args, **kw):
        ...
        if subprocess is None:
            raise RuntimeError('Environment does not support subprocess module, cannot run command.')
        ...
        try:
            proc = subprocess.Popen([cmd] + list(args), cwd=cwd, stderr=stderr_pipe, stdout=subprocess.PIPE)
        except OSError, e:
            if e.errno != 2:
                raise
            raise OSError('The expected executable %s was not found (%s)' % (
             cmd, e))
    ...
    def write_file(self, filename, content, source=None, binary=True, svn_add=True):
        """
        Like ``ensure_file``, but without the interactivity.  Mostly
        deprecated.  (I think I forgot it existed)
        """
        import warnings
        warnings.warn('command.write_file has been replaced with command.ensure_file', DeprecationWarning, 2)
        if os.path.exists(filename):
            ...
            if content == old_content:
                if self.verbose:
                    print 'File %s exists with same content' % self.shorten(filename)
                return
        ...
        if not self.simulate:
            ...
            f.write(content)
            f.close()
            if not already_existed and svn_add and os.path.exists(os.path.join(os.path.dirname(filename), '.svn')):
    ...
    def write_vars(self, config, vars, section='pastescript'):
        ...
        for (key, value) in vars.items():
            if key not in existing_options:
                if '%s__eval__' % key not in existing_options:
                    isinstance(value, str) or p.set(section, '%s__eval__' % key, repr(value))
                else:
