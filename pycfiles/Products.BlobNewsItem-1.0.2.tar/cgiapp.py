...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/cgiapp.py
...
class CGIApplication(object):
    ...
    def __init__(self, global_conf, script, path=None, include_os_environ=True, query_string=None):
        ...
        self.path = path
        if '?' in script:
            assert query_string is None, "You cannot have '?' in your script name (%r) and also give a query_string (%r)" % (
             script, query_string)
        ...
        if os.path.abspath(script) != script:
            for path_dir in self.path:
                ...
                raise CGIError('Script %r not found in path %r' % (
                 script, self.path))
        else:
            self.script = script
    ...
    def __call__(self, environ, start_response):
        if 'REQUEST_URI' not in environ:
            environ['REQUEST_URI'] = urllib.quote(environ.get('SCRIPT_NAME', '')) + urllib.quote(environ.get('PATH_INFO', ''))
        ...
        proc = subprocess.Popen([
         self.script], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=cgi_environ, cwd=os.path.dirname(self.script))
...
def make_cgi_application(global_conf, script, path=None, include_os_environ=None, query_string=None):
    ...
    return CGIApplication(script, path=path, include_os_environ=include_os_environ, query_string=query_string)