...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/serve.py
# Compiled at: 2012-02-27 07:41:53
import re, os, errno, sys, time
try:
    import subprocess
except ImportError:
...
import threading, atexit, logging, ConfigParser
MAXFD = 1024
...
class ServeCommand(Command):
    ...
    if jython:
        parser.add_option('--disable-jython-reloader', action='store_true', dest='disable_jython_reloader', help='Disable the Jython reloader')
    ...
    def command(self):
        if self.options.stop_daemon:
            ...
            if self.requires_config_file:
                ...
                if len(self.args) > 1:
                    if self.args[1] in self.possible_subcommands:
                        cmd = self.args[1]
                ...
                if self.options.reload:
                    ...
                    if jython_monitor or os.environ.get(self._reloader_environ_key):
            ...
            server = self.loadserver(server_spec, name=server_name, relative_to=base, global_conf=vars)
            app = self.loadapp(app_spec, name=app_name, relative_to=base, global_conf=vars)
            if self.verbose > 0:
