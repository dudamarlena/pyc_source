...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/httpheaders.py
...
class HTTPHeader(object):
    ...
    def __new__(cls, name, category=None, reference=None, version=None):
        ...
        if self:
            assert self.name == name, 'duplicate registration with different capitalization'
            assert self.category == category, 'duplicate registration with different category'
            assert cls == self.__class__, 'duplicate registration with different class'
            return self
        ...
        self.sort_order = {'general': 1, 'request': 2, 'response': 3, 
    ...
    def values(self, *args, **kwargs):
        ...
        if not args:
            ...
            if dict == type(args[0]):
                ...
                raise 1 == len(args) and 'wsgi.version' in args[0] or AssertionError
...
for (name, category, version, style, comment) in (
 ...
 (
  'WWW-Authenticate', 'response', '1.0', 'multi-entry', 'RFC 2616, 14.47')):
    klass = {'multi-value': _MultiValueHeader, 'multi-entry': _MultiEntryHeader, 'date-header': _DateHeader, 
