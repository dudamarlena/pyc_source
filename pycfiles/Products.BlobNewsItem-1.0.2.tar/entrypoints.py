...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/PasteScript-1.7.5-py2.6.egg/paste/script/entrypoints.py
# Compiled at: 2012-02-27 07:41:53
import textwrap, os, pkg_resources
from command import Command, BadCommand
import fnmatch, re, traceback
...
class EntryPointCommand(Command):
    ...
    description = '    Shows information about one or many entry points (you can use\n    wildcards for entry point names).  Entry points are used for Egg\n    plugins, and are named resources -- like an application, template\n    plugin, or other resource.  Entry points have a [group] which\n    defines what kind of object they describe, and inside groups each\n    entry point is named.\n    '
    max_args = 2
    parser = Command.standard_parser(verbose=False)
    parser.add_option('--list', '-l', dest='list_entry_points', action='store_true', help='List all the kinds of entry points on the system')
    parser.add_option('--egg', '-e', dest='show_egg', help='Show all the entry points for the given Egg')
    parser.add_option('--regex', dest='use_regex', action='store_true', help='Make pattern match as regular expression, not just a wildcard pattern')
    ...
    def _safe_get_entry_point_description(self, ep, group):
        ...
        generic = meta or list(pkg_resources.iter_entry_points(meta_group, 'generic'))
        if not generic:
...
class SuperGeneric(object):
    ...
    def __init__(self, doc_object):
        ...
        try:
            ...
            elif hasattr(self.doc_object, '__call__') and not isinstance(self.doc_object, types.FunctionType):
