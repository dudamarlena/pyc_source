...
# [GCC 8.4.0]
# Embedded file name: /usr/local/plone-production-nbf-1/zeocluster/src/Products.BlobNewsItem/Paste-1.7.5.1-py2.6.egg/paste/httpserver.py
...
"""
import atexit, traceback, socket, sys, threading, urlparse, Queue, urllib, posixpath, time, thread, os
...
try:
    ...
    SocketErrors = (
     socket.error, SSL.ZeroReturnError, SSL.SysCallError)
except ImportError:
    SSL = None
...
class ThreadPool(object):
    ...
    def __init__(self, nworkers, name='ThreadPool', daemon=False, max_requests=100, hung_thread_limit=30, kill_thread_limit=1800, dying_limit=300, spawn_if_under=5, max_zombie_threads_before_die=0, hung_check_period=100, logger=None, error_email=None):
        ...
        assert not kill_thread_limit or kill_thread_limit >= hung_thread_limit, 'kill_thread_limit (%s) should be higher than hung_thread_limit (%s)' % (
        ...
        assert spawn_if_under <= nworkers, 'spawn_if_under (%s) should be less than nworkers (%s)' % (
        ...
        if not self.idle_workers and self.spawn_if_under:
            ...
            for worker in self.workers:
                if not hasattr(worker, 'thread_id'):
                    continue
                (time_started, info) = self.worker_tracker.get(worker.thread_id, (None,
                                                                                  None))
    ...
    def kill_worker(self, thread_id):
        ...
        self.dying_threads[thread_id] = (
         time.time(), thread_obj)
    ...
    def kill_hung_threads(self):
        ...
        if not self.kill_thread_limit:
            ...
            if working_workers:
                ...
                ave_time = 'N/A'
            self.logger.info('kill_hung_threads status: %s threads (%s working, %s idle, %s starting) ave time %s, max time %.2fsec, killed %s workers' % (
    ...
    def notify_problem(self, msg, subject=None, spawn_thread=True):
        ...
        if not self.error_email:
            return
        if spawn_thread:
            t = threading.Thread(target=self.notify_problem, args=(
             msg, subject, False))
            t.start()
            return
...
class ThreadPoolMixIn(object):
    ...
    def __init__(self, nworkers, daemon=False, **threadpool_options):
        ...
        self.thread_pool = ThreadPool(nworkers, ('ThreadPoolMixIn HTTP server on %s:%d' % (
         self.server_name, self.server_port)), daemon, **threadpool_options)
...
def serve(application, host=None, port=None, handler=None, ssl_pem=None, ssl_context=None, server_version=None, protocol_version=None, start_loop=True, daemon_threads=None, socket_timeout=None, use_threadpool=None, threadpool_workers=10, threadpool_options=None, request_queue_size=5):
    ...
    if converters.asbool(use_threadpool):
        server = WSGIThreadPoolServer(application, server_address, handler, ssl_context, int(threadpool_workers), daemon_threads, threadpool_options=threadpool_options, request_queue_size=request_queue_size)
    else:
        server = WSGIServer(application, server_address, handler, ssl_context, request_queue_size=request_queue_size)
        if daemon_threads:
