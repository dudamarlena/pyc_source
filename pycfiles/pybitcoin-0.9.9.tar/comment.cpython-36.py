...
# Embedded file name: build/bdist.linux-x86_64/egg/pybitbucket/comment.py
...
class Comment(BitbucketBase):
    ...
    def create_comment(content, snippet_id, username=None, client=Client()):
        ...
        template = '{+bitbucket_url}' + '/2.0/snippets/{username}/{snippet_id}' + '/comments'
        api_url = expand(template, {'bitbucket_url':client.get_bitbucket_url(), 
         'username':username, 
    ...
    @staticmethod
    def find_comment_for_snippet_by_id(snippet_id, comment_id, username=None, client=Client()):
        """
        ...
        if username is None:
            username = client.get_username()
        return next(Bitbucket(client=client).snippetCommentByCommentId(username=username,
          snippet_id=snippet_id,
          comment_id=comment_id))

    @staticmethod
    def find_comment_for_repository_commit_by_id(owner, repository_name, revision, comment_id, client=Client()):
        """
        ...
        generator.
        """
        return next(Bitbucket(client=client).repositoryCommitCommentByCommentId(owner=owner,
          repository_name=repository_name,
          revision=revision,
          comment_id=comment_id))

    @staticmethod
    def find_comment_for_repository_pullrequest_by_id(owner, repository_name, pullrequest_id, comment_id, client=Client()):
        """
        ...
        generator.
        """
        return next(Bitbucket(client=client).repositoryPullRequestCommentsByCommentId(owner=owner,
          repository_name=repository_name,
