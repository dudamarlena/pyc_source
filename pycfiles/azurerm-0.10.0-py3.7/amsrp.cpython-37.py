...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\azurerm\amsrp.py
# Compiled at: 2018-11-06 13:51:27
# Size of source mod 2**32: 33512 bytes
"""amsrp.py - azurerm functions for the Microsoft.Media resource provider and AMS Rest Interface."""
import json, urllib
from .restfns import do_get, do_put, do_post, do_delete, do_ams_auth, do_ams_get, do_ams_post, do_ams_put, do_ams_delete, do_ams_patch, do_ams_sto_put
...
def create_media_service_rg(access_token, subscription_id, rgname, location, stoname, msname):
    ...
    sub_id_str = '/subscriptions/' + subscription_id + '/resourceGroups/' + rgname + '/providers/Microsoft.Storage/storageAccounts/' + stoname
...
def create_contentkey_authorization_policy_options(access_token, key_delivery_type='2', name='HLS Open Authorization Policy', key_restriction_type='0'):
    ...
    path = '/ContentKeyAuthorizationPolicyOptions'
    endpoint = ''.join([ams_rest_endpoint, path])
    body = '{ \t\t"Name":"policy",\t\t"KeyDeliveryType":"' + key_delivery_type + '", \t\t"KeyDeliveryConfiguration":"", \t\t\t"Restrictions":[{ \t\t\t"Name":"' + name + '", \t\t\t"KeyRestrictionType":"' + key_restriction_type + '", \t\t\t"Requirements":null \t\t}] \t}'
