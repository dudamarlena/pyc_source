...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\azurerm\keyvault.py
# Compiled at: 2018-12-28 09:26:23
...
def create_keyvault(access_token, subscription_id, rgname, vault_name, location, template_deployment=True, tenant_id=None, object_id=None):
    ...
    access_policies = [
     {'tenantId':tenant_id,  'objectId':object_id,  'permissions':{'keys':[
       ...
       'certificates':[
        ...
        'recover'], 
       'storage':[
        'get', 'list', 'delete', 'set', 'update', 'regeneratekey',
        'setsas', 'listsas', 'getsas', 'deletesas']}}]
    vault_properties = {'tenantId':tenant_id, 
     'sku':{'family':'A',  'name':'standard'},  'enabledForTemplateDeployment':template_deployment, 
...
def set_keyvault_secret(access_token, vault_uri, secret_name, secret_value):
    ...
    attributes = {'created':current_time,  'enabled':True, 
     'exp':None, 
     'nbf':None, 
     'recoveryLevel':'Purgeable', 
     'updated':current_time}
    secret_body = {'attributes':attributes,  'contentType':None, 
     'kid':None, 
     'managed':None, 
