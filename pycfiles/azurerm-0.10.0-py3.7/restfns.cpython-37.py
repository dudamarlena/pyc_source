...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\azurerm\restfns.py
# Compiled at: 2018-11-06 13:51:27
# Size of source mod 2**32: 13445 bytes
"""azurerm restfns - REST functions for azurerm"""
import platform, pkg_resources, requests
from .settings import json_acceptformat, json_only_acceptformat, xml_acceptformat, charset, dsversion_min, dsversion_max, xmsversion, ams_rest_endpoint

def get_user_agent():
    """User-Agent Header. Sends library identification to Azure endpoint.
    """
    version = pkg_resources.require('azurerm')[0].version
    user_agent = 'python/{} ({}) requests/{} azurerm/{}'.format(platform.python_version(), platform.platform(), requests.__version__, version)
    return user_agent
...
def do_get_next(endpoint, access_token):
    ...
    while looping:
        ...
        if 'value' not in get_return:
            return get_return
        elif 'nextLink' not in get_return:
            looping = False
        else:
...
def do_ams_put(endpoint, path, body, access_token, rformat='json', ds_min_version='3.0;NetFx'):
    ...
    if rformat == 'json_only':
        ...
        headers = {'Content-Type':content_acceptformat,  'DataServiceVersion':min_ds, 
         'MaxDataServiceVersion':dsversion_max, 
         'Accept':json_acceptformat, 
         'Accept-Charset':charset, 
         'Authorization':'Bearer ' + access_token, 
