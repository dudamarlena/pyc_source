...
# [GCC 8.4.0]
# Embedded file name: /Users/Mardix/Dropbox/Projects/Python/web-portfolio/webmaster/decorators.py
# Compiled at: 2016-01-04 02:26:52
...
class Menu(object):
        ...
        def wrap(f):
            if name:
                ...
                if extends:
                    ...
                    elif 'endpoint' not in kwargs:
                        endpoint = '%s:%s' % (class_, f.__name__)
                        kwargs['endpoint'] = endpoint
                    module_ = extends.__module__
                    class_ = extends.__name__
                self._push(module_=module_, class_=class_, method_=method_, name=name, is_class_=is_class_, **kwargs)
            return f
    ...
    def _push(self, **kwargs):
        ...
        if path not in self.MENU:
            self.MENU[path] = {'name': None, 'endpoint': None, 
    ...
    def render(self):
        menu_list = []
        menu_index = 0
        for _, menu in self.MENU.items():
            menu_index += 1
            sub_menu = []
            menu['kwargs']['index'] = menu_index
            menu['kwargs']['active'] = False
            if 'visible' not in menu['kwargs']:
                print menu
                print ''
