...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/util.py
# Compiled at: 2018-01-24 00:52:58
import codecs, csv, datetime, math, optparse, random, re, socket, sys, time, urllib2, problems as problems_module
  ...
  """
    if not latest_version:
        ...
        try:
            ...
            description = 'During the new-version check, we failed to reach transitfeed server: Reason: %s [%s].' % (
             e.reason, e.code)
            problems.OtherProblem(description=description, type=problems_module.TYPE_NOTICE)
            return
...
def _MaxVersion(versions):
    ...
    versions.sort(lambda x, y: -cmp([ int(item) for item in x.split('.') ], [ int(item) for item in y.split('.') ]))
    if len(versions) > 0:
        return versions[0]
    else:
        return
...
def ValidateTimezone(timezone, column_name=None, problems=None):
  ...
  """
    ...
    if problems:
        import pytz
        problems.InvalidValue(column_name, timezone, '"%s" is not a common timezone name according to pytz version %s' % (
...
def ValidateRequiredFieldsAreNotEmpty(gtfs_object, required_field_names, problems=None):
    """
...
def ValidateAndReturnIntValue(value, allowed_values, default, allow_empty, column_name=None, problems=None):
    """
  ...
  """
    if allow_empty and IsEmpty(value):
        ...
        if int_value not in allowed_values:
            if problems and column_name:
                problems.InvalidValue(column_name, value, type=problems_module.TYPE_WARNING)
        return int_value
...
def FloatStringToFloat(float_string, problems=None):
    ...
    match = re.match('^[+-]?\\d+(\\.\\d+)?$', float_string)
...
def NonNegIntStringToInt(int_string, problems=None):
    ...
    match = re.match('^(?:0|[1-9]\\d*)$', int_string)
...
def ApproximateDistance(degree_lat1, degree_lng1, degree_lat2, degree_lng2):
  ...
  Earth is a sphere."""
    ...
    return EARTH_RADIUS * (2 * math.atan2(math.sqrt(x), math.sqrt(max(0.0, 1.0 - x))))


def ApproximateDistanceBetweenStops(stop1, stop2):
    """Compute approximate distance between two stops in meters. Assumes the
  Earth is a sphere."""
    if stop1.stop_lat is None or stop1.stop_lon is None or stop2.stop_lat is None or stop2.stop_lon is None:
        return
...
INVALID_LINE_SEPARATOR_UTF8 = {'\x0c': 'ASCII Form Feed 0x0C', 
   '\r': 'ASCII Carriage Return 0x0D, \\r', 
   '\u2028': 'Unicode LINE SEPARATOR U+2028', 
   '\u2029': 'Unicode PARAGRAPH SEPARATOR U+2029', 
   '\x85': 'Unicode NEXT LINE SEPARATOR U+0085'}
...
class EndOfLineChecker:
  ...
  """
    ...
    def next(self):
        ...
        self._line_number += 1
        m_eol = re.search('[\\x0a\\x0d]*$', next_line)
        if m_eol.group() == '\r\n':
            self._crlf += 1
            if self._crlf <= 5:
            ...
            if m_eol.group() == '\n':
                ...
                self._problems.InvalidLineEnd(codecs.getencoder('string_escape')(m_eol.group())[0], (
            ...
            for seq, name in INVALID_LINE_SEPARATOR_UTF8.items():
                if next_line_contents.find(seq) != -1:
                    self._problems.OtherProblem('Line contains %s' % name, context=(
                     self._name, self._line_number))
    ...
    def _FinalCheck(self):
        if self._crlf > 0 and self._lf > 0:
            ...
            if self._lf > len(self._lf_examples):
                lf_lines += ', ...'
            self._problems.OtherProblem('Found %d CR LF "\\r\\n" line end%s (line%s %s) and %d LF "\\n" line end%s (line%s %s). A file must use a consistent line end.' % (
             self._crlf, crlf_plural, crlf_plural,
...
class ISO4217(object):
    ...
    codes = {'AED': 784, 
       'AFN': 971, 'ALL': 8, 'AMD': 51, 'ANG': 532, 'AOA': 973, 'ARS': 32, 
       'AUD': 36, 'AWG': 533, 'AZN': 944, 'BAM': 977, 'BBD': 52, 'BDT': 50, 
       'BGN': 975, 'BHD': 48, 'BIF': 108, 'BMD': 60, 'BND': 96, 'BOB': 68, 
       'BOV': 984, 'BRL': 986, 'BSD': 44, 'BTN': 64, 'BWP': 72, 'BYR': 974, 
       'BZD': 84, 'CAD': 124, 'CDF': 976, 'CHE': 947, 'CHF': 756, 'CHW': 948, 
       'CLF': 990, 'CLP': 152, 'CNY': 156, 'COP': 170, 'COU': 970, 'CRC': 188, 
       'CUP': 192, 'CVE': 132, 'CYP': 196, 'CZK': 203, 'DJF': 262, 'DKK': 208, 
       'DOP': 214, 'DZD': 12, 'EEK': 233, 'EGP': 818, 'ERN': 232, 'ETB': 230, 
       'EUR': 978, 'FJD': 242, 'FKP': 238, 'GBP': 826, 'GEL': 981, 'GHC': 288, 
       ...
       'MXN': 484, 'MXV': 979, 'MYR': 458, 'MZN': 943, 'NAD': 516, 'NGN': 566, 
       'NIO': 558, 'NOK': 578, 'NPR': 524, 'NZD': 554, 'OMR': 512, 'PAB': 590, 
       'PEN': 604, 'PGK': 598, 'PHP': 608, 'PKR': 586, 'PLN': 985, 'PYG': 600, 
       'QAR': 634, 'ROL': 642, 'RON': 946, 'RSD': 941, 'RUB': 643, 'RWF': 646, 
       'SAR': 682, 'SBD': 90, 'SCR': 690, 'SDD': 736, 'SDG': 938, 'SEK': 752, 
       'SGD': 702, 'SHP': 654, 'SKK': 703, 'SLL': 694, 'SOS': 706, 'SRD': 968, 
       'STD': 678, 'SYP': 760, 'SZL': 748, 'THB': 764, 'TJS': 972, 'TMM': 795, 
       'TND': 788, 'TOP': 776, 'TRY': 949, 'TTD': 780, 'TWD': 901, 'TZS': 834, 
       'UAH': 980, 'UGX': 800, 'USD': 840, 'USN': 997, 'USS': 998, 'UYU': 858, 
       'UZS': 860, 'VEB': 862, 'VND': 704, 'VUV': 548, 'WST': 882, 'XAF': 950, 
       'XAG': 961, 'XAU': 959, 'XBA': 955, 'XBB': 956, 'XBC': 957, 'XBD': 958, 
       'XCD': 951, 'XDR': 960, 'XFO': None, 'XFU': None, 'XOF': 952, 'XPD': 964, 
       'XPF': 953, 'XPT': 962, 'XTS': 963, 'XXX': 999, 'YER': 886, 'ZAR': 710, 
       'ZMK': 894, 'ZWD': 716}
