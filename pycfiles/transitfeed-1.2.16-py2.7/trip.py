...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/trip.py
...
class Trip(GtfsObjectBase):
    ...
    def __init__(self, headsign=None, service_period=None, route=None, trip_id=None, field_dict=None):
        self._schedule = None
        self._headways = []
        if not field_dict:
            field_dict = {}
    ...
    """
        ...
        cursor = schedule._connection.cursor()
        cursor.execute('DELETE FROM stop_times WHERE trip_id=? and stop_sequence=? and stop_id=?', (
    ...
    """
        ...
        new_secs = stoptime.GetTimeSecs()
        cursor = schedule._connection.cursor()
        cursor.execute('SELECT max(stop_sequence), max(arrival_secs), max(departure_secs) FROM stop_times WHERE trip_id=?', (
         self.trip_id,))
        row = cursor.fetchone()
        if row[0] is None:
    ...
    def GetCountStopTimes(self):
        ...
        cursor.execute('SELECT count(*) FROM stop_times WHERE trip_id=?', (self.trip_id,))
    ...
    """
        ...
        if not stoptimes:
            ...
            if stoptimes[0].GetTimeSecs() is None or stoptimes[(-1)].GetTimeSecs() is None:
    ...
    def GetStopTimes(self, problems=None):
        ...
        cursor = self._schedule._connection.cursor()
        cursor.execute('SELECT arrival_secs,departure_secs,stop_headsign,pickup_type,drop_off_type,shape_dist_traveled,stop_id,stop_sequence,timepoint FROM stop_times WHERE trip_id=? ORDER BY stop_sequence', (
         self.trip_id,))
        stop_times = []
        ...
        for row in cursor.fetchall():
            ...
            stop_times.append(stoptime_class(problems=problems, stop=stop, arrival_secs=row[0], departure_secs=row[1], stop_headsign=row[2], pickup_type=row[3], drop_off_type=row[4], shape_dist_traveled=row[5], stop_sequence=row[7], timepoint=row[8]))
    ...
    return the first time of the first trip."""
        cursor = self._schedule._connection.cursor()
        cursor.execute('SELECT arrival_secs,departure_secs FROM stop_times WHERE trip_id=? ORDER BY stop_sequence LIMIT 1', (
         self.trip_id,))
        ...
        if arrival_secs != None:
            ...
            problems.InvalidValue('departure_time', '', 'The first stop_time in trip %s is missing times.' % self.trip_id)
            return
    ...
    return the last time of the last trip."""
        cursor = self._schedule._connection.cursor()
        cursor.execute('SELECT arrival_secs,departure_secs FROM stop_times WHERE trip_id=? ORDER BY stop_sequence DESC LIMIT 1', (
         self.trip_id,))
        arrival_secs, departure_secs = cursor.fetchone()
        if departure_secs != None:
            return departure_secs
            ...
            if arrival_secs != None:
                return arrival_secs
            problems.InvalidValue('arrival_time', '', 'The last stop_time in trip %s is missing times.' % self.trip_id)
            return
    ...
    def AddFrequency(self, start_time, end_time, headway_secs, exact_times=0, problem_reporter=problems_module.default_problem_reporter):
    ...
    """
        if start_time == None or start_time == '':
            ...
            if end_time <= start_time:
                problem_reporter.InvalidValue('end_time', end_time, 'should be greater than start_time')
            if not exact_times:
                exact_times = 0
            if exact_times not in (0, 1):
                problem_reporter.InvalidValue('exact_times', exact_times, 'Should be 0 (no fixed schedule) or 1 (fixed and regular schedule)')
    ...
    def ValidateDirectionId(self, problems):
        if hasattr(self, 'direction_id') and not util.IsEmpty(self.direction_id) and self.direction_id != '0' and self.direction_id != '1':
            problems.InvalidValue('direction_id', self.direction_id, 'direction_id must be "0" or "1"')
    ...
    def ValidateServiceIdExistsInServiceList(self, problems):
        if self._schedule and self.service_id:
            if self.service_id not in self._schedule.service_periods:
    ...
    def ValidateWheelchairAccessible(self, problems):
        if self.wheelchair_accessible:
            util.ValidateYesNoUnknown(self.wheelchair_accessible, 'wheelchair_accessible', problems)
    ...
    def ValidateNoDuplicateStopSequences(self, problems):
        cursor = self._schedule._connection.cursor()
        cursor.execute('SELECT COUNT(stop_sequence) AS a, stop_sequence FROM stop_times WHERE trip_id=? GROUP BY stop_sequence HAVING a > 1', (
         self.trip_id,))
        for row in cursor:
            problems.InvalidValue('stop_sequence', row[1], 'Duplicate stop_sequence in trip_id %s' % self.trip_id)
    ...
    def ValidateTripStartAndEndTimes(self, problems, stoptimes):
        if stoptimes:
            if stoptimes[0].arrival_time is None and stoptimes[0].departure_time is None:
                problems.OtherProblem('No time for start of trip_id "%s""' % self.trip_id)
            if stoptimes[(-1)].arrival_time is None and stoptimes[(-1)].departure_time is None:
                problems.OtherProblem('No time for end of trip_id "%s""' % self.trip_id)
        return

    def ValidateStopTimesSequenceHasIncreasingTimeAndDistance(self, problems, stoptimes):
        if stoptimes:
            ...
            for timepoint in stoptimes:
                ...
                if distance is not None:
                    if distance > prev_distance and distance >= 0:
                        ...
                        problems.InvalidValue('stoptimes.shape_dist_traveled', distance, 'For the trip %s the stop %s has shape_dist_traveled=%s, which should be larger than the previous ones. In this case, the previous distance was %s.' % (
                         self.trip_id, timepoint.stop_id, distance, prev_distance), type=type)
                if timepoint.arrival_secs is not None:
                    self._CheckSpeed(prev_stop, timepoint.stop, prev_departure, timepoint.arrival_secs, max_speed, problems)
                    if timepoint.arrival_secs >= prev_departure:
                        prev_departure = timepoint.departure_secs
                        prev_stop = timepoint.stop
                    else:
                        problems.OtherProblem('Timetravel detected! Arrival time is before previous departure at sequence number %s in trip %s' % (
                         timepoint.stop_sequence, self.trip_id))

        return

    def ValidateShapeDistTraveledSmallerThanMaxShapeDistance(self, problems, stoptimes):
        if stoptimes:
            if self.shape_id and self.shape_id in self._schedule._shapes:
                ...
                st = stoptimes[(-1)]
                if st.shape_dist_traveled and st.shape_dist_traveled > max_shape_dist:
                    problems.OtherProblem('In stop_times.txt, the stop with trip_id=%s and stop_sequence=%d has shape_dist_traveled=%f, which is larger than the max shape_dist_traveled=%f of the corresponding shape (shape_id=%s)' % (
                     self.trip_id, st.stop_sequence, st.shape_dist_traveled,
                     max_shape_dist, self.shape_id), type=problems_module.TYPE_WARNING)
                    ...
                    for st in stoptimes:
                        ...
                        if pt:
                            ...
                            if stop.stop_lat and stop.stop_lon:
                                distance = util.ApproximateDistance(stop.stop_lat, stop.stop_lon, pt[0], pt[1])
                                if distance > problems_module.MAX_DISTANCE_FROM_STOP_TO_SHAPE:
                                    problems.StopTooFarFromShapeWithDistTraveled(self.trip_id, stop.stop_name, stop.stop_id, pt[2], self.shape_id, distance, problems_module.MAX_DISTANCE_FROM_STOP_TO_SHAPE)
        ...
        for headway_index, headway in enumerate(self._headways[0:-1]):
            for other in self._headways[headway_index + 1:]:
                if other[0] < headway[1] and other[1] > headway[0]:
                    problems.OtherProblem('Trip contains overlapping headway periods %s and %s' % (
    ...
    def ValidateChildren(self, problems):
        ...
        self.ValidateStopTimesSequenceHasIncreasingTimeAndDistance(problems, stoptimes)
        self.ValidateShapeDistTraveledSmallerThanMaxShapeDistance(problems, stoptimes)
    ...
    def _CheckSpeed(self, prev_stop, next_stop, depart_time, arrive_time, max_speed, problems):
        if prev_stop != None:
            try:
            ...
            dist_between_stops = util.ApproximateDistanceBetweenStops(next_stop, prev_stop)
            if dist_between_stops is None:
                return
            if time_between_stops == 0:
                if depart_time % 60 != 0 or dist_between_stops / 1000 * 60 > max_speed:
                    problems.TooFastTravel(self.trip_id, prev_stop.stop_name, next_stop.stop_name, dist_between_stops, time_between_stops, speed=None, type=problems_module.TYPE_WARNING)
                return
            speed_between_stops = float(dist_between_stops) / 1000 / (float(time_between_stops) / 3600)
            if speed_between_stops > max_speed:
                problems.TooFastTravel(self.trip_id, prev_stop.stop_name, next_stop.stop_name, dist_between_stops, time_between_stops, speed_between_stops, type=problems_module.TYPE_WARNING)
        return
