...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/shape.py
...
class Shape(GtfsFactoryUser):
  ...
  taken by one or more Trips."""
    ...
    def AddPoint(self, lat, lon, distance=None, problems=problems_module.default_problem_reporter):
        shapepoint_class = self.GetGtfsFactory().ShapePoint
        shapepoint = shapepoint_class(self.shape_id, lat, lon, len(self.sequence), distance)
    ...
    def AddShapePointObjectUnsorted(self, shapepoint, problems):
        ...
        if len(self.sequence) == 0 or shapepoint.shape_pt_sequence >= self.sequence[(-1)]:
        ...
        if shapepoint.shape_pt_sequence in self.sequence:
            problems.InvalidValue('shape_pt_sequence', shapepoint.shape_pt_sequence, 'The sequence number %d occurs more than once in shape %s.' % (
    ...
    def ValidateShapePoints(self, problems):
        if not self.points:
            problems.OtherProblem('The shape with shape_id "%s" contains no points.' % self.shape_id, type=problems_module.TYPE_WARNING)
