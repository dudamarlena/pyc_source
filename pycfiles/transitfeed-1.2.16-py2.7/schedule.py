...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/schedule.py
# Compiled at: 2018-01-24 00:52:58
import bisect, cStringIO as StringIO, datetime, itertools, os
try:
...
import tempfile, time, warnings, weakref, zipfile, gtfsfactory, problems as problems_module
...
class Schedule(object):
  ...
  an agency.  This is the main class for this module."""
    ...
    def __init__(self, problem_reporter=None, memory_db=True, check_duplicate_trips=False, gtfs_factory=None):
    ...
    def ConnectDb(self, memory_db):
        ...
        def connector(db_file):
            if native_sqlite:
                ...
                return sqlite.connect('jdbc:sqlite:%s' % db_file, '', '', 'org.sqlite.JDBC')
    ...
    def AddAgencyObject(self, agency, problem_reporter=None, validate=False):
        if not agency._schedule is None:
            raise AssertionError
            problem_reporter = problem_reporter or self.problem_reporter
    ...
    def AddServicePeriodObject(self, service_period, problem_reporter=None, validate=True):
    ...
    def AddStopObject(self, stop, problem_reporter=None):
        ...
        if not stop._schedule is None:
            raise AssertionError
            if not problem_reporter:
                problem_reporter = self.problem_reporter
            return stop.stop_id or None
        else:
    ...
    def AddRouteObject(self, route, problem_reporter=None):
        ...
        if route.agency_id not in self._agencies:
            if not route.agency_id and len(self._agencies) == 1:
                ...
                problem_reporter.InvalidValue('agency_id', route.agency_id, 'Route uses an unknown agency_id.')
                return
    ...
    def AddFareObject(self, fare, problem_reporter=None):
        ...
        warnings.warn('No longer supported. The Fare class was renamed to FareAttribute, and all related functions were renamed accordingly.', DeprecationWarning)
    ...
    def AddFareRuleObject(self, rule, problem_reporter=None):
        ...
        if rule.fare_id in self.fares:
            ...
            problem_reporter.InvalidValue('fare_id', rule.fare_id, "(This fare_id doesn't correspond to any of the IDs defined in the fare attributes.)")

    def AddFeedInfoObject(self, feed_info, problem_reporter=None, validate=False):
        if not feed_info._schedule is None:
            raise AssertionError
            problem_reporter = problem_reporter or self.problem_reporter
    ...
    def AddTransferObject(self, transfer, problem_reporter=None):
        if not transfer._schedule is None:
            raise AssertionError('only add Transfer to a schedule once')
            problem_reporter = problem_reporter or self.problem_reporter
        transfer_id = transfer._ID()
        if transfer_id in self._transfers:
            self.problem_reporter.DuplicateID(self._gtfs_factory.Transfer._ID_COLUMNS, transfer_id, type=problems_module.TYPE_WARNING)
    ...
    def Load(self, feed_path, extra_validation=False):
        loader = self._gtfs_factory.Loader(feed_path, self, problems=self.problem_reporter, extra_validation=extra_validation)
        loader.Load()

    def _WriteArchiveString(self, archive, filename, stringio):
        zi = zipfile.ZipInfo(filename)
        zi.external_attr = 28704768
    ...
    """
        ...
        writer.writerow(self._gtfs_factory.ServicePeriod._FIELD_NAMES_CALENDAR_DATES)
        has_data = False
        ...
        if has_data:
            ...
            self._WriteArchiveString(archive, 'calendar_dates.txt', calendar_dates_string)
        ...
        if self.GetFareAttributeList():
            ...
            writer.writerows(f.GetFieldValuesTuple() for f in self.GetFareAttributeList())
    ...
    """
        ...
        for trip in self.GetTripList():
            ...
            service_id_to_departures[trip.service_id] += (trip.GetCountStopTimes() - 1) * trip_runs
        ...
        for date, services in date_services:
            ...
            day_departures = sum(service_id_to_departures[s.service_id] for s in services)
            date_trips.append((date, day_trips, day_departures))

        return date_trips

    def ValidateAgenciesHaveSameAgencyTimezone(self, problems):
        timezones_set = set(map(lambda agency: agency.agency_timezone, self.GetAgencyList()))
        if len(timezones_set) > 1:
            timezones_str = '"%s"' % ('", "').join(timezones_set)
            problems.InvalidValue('agency_timezone', timezones_str, 'All agencies should have the same time zone. Please review agency.txt.')
    ...
    def ValidateFeedInfoLangMatchesAgencyLang(self, problems):
        if self.feed_info is None:
            ...
            for agency in agencies:
                if not util.IsEmpty(agency.agency_lang) and not self.feed_info.feed_lang == agency.agency_lang:
                    problems.InvalidValue('feed_lang', 'The languages specified in feedinfo.txt and in agency.txt for agency with ID %s differ.' % agency.agency_id)
    ...
    """
        ...
        if first_date > today:
            problems.FutureService(time.mktime(first_date.timetuple()), first_date_origin)
    ...
    def ValidateServiceExceptions(self, problems, first_service_day, last_service_day):
        ...
        problems.NoServiceExceptions(start=first_service_day, end=last_service_day)

    def ValidateServiceRangeAndExceptions(self, problems, today, service_gap_interval):
        if today is None:
            today = datetime.date.today()
        start_date, end_date, start_date_origin, end_date_origin = self.GetDateRangeWithOrigins()
        if not end_date or not start_date:
            problems.OtherProblem('This feed has no effective service dates!', type=problems_module.TYPE_WARNING)
        else:
            try:
                last_service_day = datetime.datetime(*time.strptime(end_date, '%Y%m%d')[0:6]).date()
                first_service_day = datetime.datetime(*time.strptime(start_date, '%Y%m%d')[0:6]).date()
            except ValueError:
                pass

            self.ValidateServiceExceptions(problems, first_service_day, last_service_day)
            self.ValidateFeedStartAndExpirationDates(problems, first_service_day, last_service_day, start_date_origin, end_date_origin, today)
            if service_gap_interval is not None:
                service_gap_timedelta = datetime.timedelta(days=service_gap_interval - 1)
                one_year = datetime.timedelta(days=365)
                self.ValidateServiceGaps(problems, max(first_service_day, today - service_gap_timedelta), min(last_service_day, today + one_year), service_gap_interval)
        return
        ...
        for stop in self.stops.values():
            ...
            if stop.location_type != 1 and stop.parent_station:
                if stop.parent_station not in self.stops:
                    problems.InvalidValue('parent_station', util.EncodeUnicode(stop.parent_station), "parent_station '%s' not found for stop_id '%s' in stops.txt" % (
                     util.EncodeUnicode(stop.parent_station),
                     util.EncodeUnicode(stop.stop_id)))
                elif self.stops[stop.parent_station].location_type != 1:
                    problems.InvalidValue('parent_station', util.EncodeUnicode(stop.parent_station), "parent_station '%s' of stop_id '%s' must have location_type=1 in stops.txt" % (
                     util.EncodeUnicode(stop.parent_station),
                     util.EncodeUnicode(stop.stop_id)))
                else:
                    ...
                    if distance is not None:
                        if distance > problems_module.MAX_DISTANCE_BETWEEN_STOP_AND_PARENT_STATION_ERROR:
                            problems.StopTooFarFromParentStation(stop.stop_id, stop.stop_name, parent_station.stop_id, parent_station.stop_name, distance, problems_module.TYPE_ERROR)
                        elif distance > problems_module.MAX_DISTANCE_BETWEEN_STOP_AND_PARENT_STATION_WARNING:
                            problems.StopTooFarFromParentStation(stop.stop_id, stop.stop_name, parent_station.stop_id, parent_station.stop_name, distance, problems_module.TYPE_WARNING)

        return

    def ValidateNearbyStops(self, problems):
        sorted_stops = filter(lambda s: s.stop_lat and s.stop_lon, self.GetStopList())
        sorted_stops.sort(key=lambda x: [
         x.stop_lat, x.stop_lon, getattr(x, 'stop_id', None)])
        TWO_METERS_LAT = 1.8e-05
        for index, stop in enumerate(sorted_stops[:-1]):
            index += 1
            while index < len(sorted_stops) and sorted_stops[index].stop_lat - stop.stop_lat < TWO_METERS_LAT:
                distance = util.ApproximateDistanceBetweenStops(stop, sorted_stops[index])
                if distance is not None and distance < 2:
                    other_stop = sorted_stops[index]
                    if stop.location_type == 0 and other_stop.location_type == 0:
                        problems.StopsTooClose(util.EncodeUnicode(stop.stop_name), util.EncodeUnicode(stop.stop_id), util.EncodeUnicode(other_stop.stop_name), util.EncodeUnicode(other_stop.stop_id), distance)
                    elif stop.location_type == 1 and other_stop.location_type == 1:
                        problems.StationsTooClose(util.EncodeUnicode(stop.stop_name), util.EncodeUnicode(stop.stop_id), util.EncodeUnicode(other_stop.stop_name), util.EncodeUnicode(other_stop.stop_id), distance)
                    elif stop.location_type in (0, 1) and other_stop.location_type in (0,
                                                                                       1):
                        ...
                        if this_stop.parent_station != this_station.stop_id:
                            problems.DifferentStationTooClose(util.EncodeUnicode(this_stop.stop_name), util.EncodeUnicode(this_stop.stop_id), util.EncodeUnicode(this_station.stop_name), util.EncodeUnicode(this_station.stop_id), distance)
                index += 1
    ...
    def ValidateRouteNames(self, problems, validate_children):
        ...
        for route in self.routes.values():
            ...
            if name in route_names:
                problems.InvalidValue('route_long_name', long_name, 'The same combination of route_short_name and route_long_name shouldn\'t be used for more than one route, as it is for the for the two routes with IDs "%s" and "%s".' % (
                 route.route_id, route_names[name].route_id), type=problems_module.TYPE_WARNING)
            else:
    ...
    def ValidateTrips(self, problems):
        ...
        for trip in sorted(self.trips.values()):
            ...
            for index, st in enumerate(stop_times):
                ...
                if route_type == self._gtfs_factory.Route._ROUTE_TYPE_NAMES['Subway'] or route_type == self._gtfs_factory.Route._ROUTE_TYPE_NAMES['Bus']:
                    if stop_id not in stop_types:
                        stop_types[stop_id] = [
                         trip.route_id, route_type, 0]
                    elif stop_types[stop_id][1] != route_type and stop_types[stop_id][2] == 0:
                        stop_types[stop_id][2] = 1
                        if stop_types[stop_id][1] == self._gtfs_factory.Route._ROUTE_TYPE_NAMES['Subway']:
                        ...
                        problems.StopWithMultipleRouteTypes(st.stop.stop_name, stop_id, subway_route_id, bus_route_id)
            ...
            if self._check_duplicate_trips:
                if not stop_ids or not stop_times:
                    continue
                key = (
                 trip.service_id, stop_times[0].arrival_time, str(stop_ids))
                if key not in trips:
                    trips[key] = (
                     trip.route_id, trip.trip_id)
                else:
                    problems.DuplicateTrip(trips[key][1], trips[key][0], trip.trip_id, trip.route_id)
    ...
    same departure/arrival time."""
        ...
        def CheckSameTimeCount():
            if prev_departure_secs != -1 and consecutive_stop_times_with_fully_specified_same_time > 5:
                problems.TooManyConsecutiveStopTimesWithSameTime(trip.trip_id, consecutive_stop_times_with_fully_specified_same_time, prev_departure_secs)

        for index, st in enumerate(stop_times):
            if st.arrival_secs is None or st.departure_secs is None:
                consecutive_stop_times_with_potentially_same_time += 1
                continue
            if prev_departure_secs == st.arrival_secs and st.arrival_secs == st.departure_secs:
                consecutive_stop_times_with_potentially_same_time += 1
                consecutive_stop_times_with_fully_specified_same_time = consecutive_stop_times_with_potentially_same_time
            else:
                CheckSameTimeCount()
                consecutive_stop_times_with_potentially_same_time = 1
            ...
            for xi in range(len(trip_intervals)):
                ...
                for xj in range(xi + 1, len(trip_intervals)):
                    ...
                    if trip_a.service_id == trip_b.service_id:
                        problems.OverlappingTripsInSameBlock(trip_a.trip_id, trip_b.trip_id, block_id)
                    else:
    ...
    def ValidateRouteAgencyId(self, problems):
        for route in self.routes.values():
            if not util.IsEmpty(route.agency_id) and route.agency_id not in self._agencies:
                problems.InvalidAgencyID('agency_id', route.agency_id, 'route', route.route_id)
    ...
    def ValidateTripStopTimes(self, problems):
        for trip in self.trips.values():
            ...
            if not count_stop_times:
                problems.OtherProblem('The trip with the trip_id "%s" doesn\'t have any stop times defined.' % trip.trip_id, type=problems_module.TYPE_WARNING)
                if len(trip._headways) > 0:
                    problems.OtherProblem('Frequencies defined, but no stop times given in trip %s' % trip.trip_id, type=problems_module.TYPE_ERROR)
            elif count_stop_times == 1:
                problems.OtherProblem('The trip with the trip_id "%s" only has one stop on it; it should have at least one more stop so that the riders can leave!' % trip.trip_id, type=problems_module.TYPE_WARNING)
            else:
    ...
    def ValidateUnusedShapes(self, problems):
        ...
        if unused_shape_ids:
            problems.OtherProblem("The shapes with the following shape_ids aren't used by any trips: %s" % (', ').join(unused_shape_ids), type=problems_module.TYPE_WARNING)

    def Validate(self, problems=None, validate_children=True, today=None, service_gap_interval=None):
       ...
       (mostly interrelationships between the various data sets)."""
        ...
        self.ValidateServiceRangeAndExceptions(problems, today, service_gap_interval)
