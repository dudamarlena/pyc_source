...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/fareattribute.py
...
class FareAttribute(GtfsObjectBase):
    ...
    def __init__(self, fare_id=None, price=None, currency_type=None, payment_method=None, transfers=None, transfer_duration=None, field_dict=None):
        self._schedule = None
    ...
    def ValidateTransfers(self, problems):
        if not (self.transfers == None or isinstance(self.transfers, int) and self.transfers in range(0, 3)):
            problems.InvalidValue('transfers', self.transfers)
        return

    def ValidateTransferDuration(self, problems):
        if self.transfer_duration != None and not isinstance(self.transfer_duration, int):
