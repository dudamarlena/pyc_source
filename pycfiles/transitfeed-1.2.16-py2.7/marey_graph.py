...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/gtfsscheduleviewer/marey_graph.py
...
class MareyGraph:
    ...
    """
        ...
        if tot_dist > 0:
            ...
            pixel_grid = [
             0] + [ int(pd + sum(pixel_dist[0:i])) for i, pd in enumerate(pixel_dist)
                  ]
        else:
            pixel_grid = []
        return pixel_grid
    ...
    """
        ...
        tmpstrs = []
        for y in stations:
            tmpstrs.append('  <polyline class="Station" stroke="%s"       points="%s,%s, %s,%s" />' % (color, 20, 20 + y + 0.5, self._gwidth + 20, 20 + y + 0.5))
