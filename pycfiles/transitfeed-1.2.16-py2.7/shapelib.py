...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-intel/egg/transitfeed/shapelib.py
...
import copy, decimal, heapq, math
  ...
  """
    ...
    def __cmp__(self, other):
        if not isinstance(other, Point):
            raise TypeError('Point.__cmp__(x,y) requires y to be a "Point", not a "%s"' % type(other).__name__)
    ...
    """
        return self._approxEq(self.x, other.x) and self._approxEq(self.y, other.y) and self._approxEq(self.z, other.z)
    ...
    """
        ...
        if polys:
            ...
            for poly in polys[1:]:
                ...
                if last_point and first_point and last_point.GetDistanceMeters(first_point) <= merge_point_threshold:
  ...
  """
    ...
    """
        ...
        for shape in self._name_to_shape.itervalues():
            if start_point.GetDistanceMeters(shape.GetPoint(0)) < max_radius and end_point.GetDistanceMeters(shape.GetPoint(-1)) < max_radius:
...
class PolyGraph(PolyCollection):
  ...
  """
    ...
    def FindShortestMultiPointPath(self, points, max_radius=150, keep_best_n=10, verbosity=0):
        """
    ...
    """
        ...
        for i, point in enumerate(points):
            nearby = [ p for p in self._nodes.iterkeys() if p.GetDistanceMeters(point) < max_radius
                     ]
            if verbosity >= 2:
        ...
        for possible_points in nearby_points[2:]:
            ...
            for score, path in paths_found:
                ...
                for end in possible_points:
                    ...
                    if new_segment:
                        new_path = Poly.MergePolys([path, new_segment], merge_point_threshold=0)
