...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/marrow/templating/serialize/bencode.py
# Compiled at: 2012-05-23 13:18:32
from __future__ import unicode_literals, absolute_import
__all__ = [
 b'render', b'CodecError', b'EncodeError', b'DecodeError', b'Encoding', b'ChunkedEncoder', b'Bencode', b'EnhancedBencode']

def render(data, template=None, kind=b'enhanced', i18n=None, **kw):
    ...
    return (
     b'application/x-bencode', codecs[kind]().encode(data))
...
class ChunkedEncoder(Encoding):
    ...
    def encode(self, data):
        try:
            return getattr(self, b'encode_' + type(data).__name__)(data)
        except AttributeError:
            raise EncodeError(b"Unable to encode a chunk of type '%s'." % (type(data),))
...
class Bencode(ChunkedEncoder):
    ...
    def decode(self, data):
        length = len(data)
        if length == 0:
            raise DecodeError(b'Can not decode an empty string.')
        (data, processed) = self._decode(data)
        if processed != length:
            raise DecodeError(b'Did not fully decode input. %d of %d processed, %d bytes remaining.' % (processed, length, length - processed))
        return data

    def _decode(self, data, offset=0):
        signature = data[offset:offset + 1]
        if hasattr(self, b'decode_' + signature):
            return getattr(self, b'decode_' + signature)(data, offset + 1)
        if signature.isdigit():
            return self.decode_str(data, offset)
        raise DecodeError(b"Unable to decode unknown signature '%s'." % (signature,))
    ...
    def encode_list(self, data):
        return b'l%se' % ((b'').join([ self.encode(item) for item in data ]),)
    ...
    def encode_dict(self, data):
        return b'd%se' % ((b'').join([ self.encode(key) + self.encode(data[key]) for key in sorted(data.keys()) ]),)
...
class EnhancedBencode(Bencode):
    ...
    def encode_tuple(self, data):
        return b't%se' % ((b'').join([ self.encode(item) for item in data ]),)
    ...
    def encode_set(self, data):
        return b's%se' % ((b'').join([ self.encode(item) for item in data ]),)
    ...
    def encode_unicode(self, data):
        encoded = data.encode(b'utf-8')
        return b'u%d:%s' % (len(encoded), encoded)

    def decode_u(self, data, o):
        index = data.index(b':', o)
        length = int(data[o:index])
        offset = index + 1
        return (data[offset:offset + length].decode(b'utf-8'), offset + length)