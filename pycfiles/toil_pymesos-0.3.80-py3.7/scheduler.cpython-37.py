...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymesos/scheduler.py
# Compiled at: 2020-04-01 21:31:02
# Size of source mod 2**32: 19094 bytes
...
class MesosSchedulerDriver(Process, SchedulerDriver):
    ...
    def __init__(self, sched, framework, master_uri, use_addict=False, implicit_acknowledgements=True, principal=None, secret=None, failover=False, timeout=DAY):
        ...
        if principal is not None and secret is not None:
            self._basic_credential = 'Basic %s' % b2a_base64(('%s:%s' % (principal, secret)).encode('ascii')).decode('ascii').strip()
        else:
    ...
    def framework(self):
        ...
        version = self.version and tuple((int(n) for n in self.version.split('.')))
        capabilities = [c for c in framework.get('capabilities', []) if c['type'] != 'GPU_RESOURCES']
        if version:
            if version >= (1, 0, 0):
    ...
    def acceptOffers(self, offer_ids, operations, filters=None):
        ...
        framework_id = self.framework_id
        assert framework_id
        accept = dict(offer_ids=([offer_ids] if isinstance(offer_ids, dict) else offer_ids),
          operations=operations)
        if filters is not None:
            accept['filters'] = filters
        body = dict(type='ACCEPT',
          framework_id=dict(value=framework_id),
          accept=accept)
        self._send(body)
    ...
    def acceptInverseOffers(self, offer_ids, filters=None):
        framework_id = self.framework_id
        assert framework_id
        accept_inverse_offers = dict(inverse_offer_ids=([offer_ids] if isinstance(offer_ids, dict) else offer_ids))
        if filters is not None:
            accept_inverse_offers['filters'] = filters
        body = dict(type='ACCEPT_INVERSE_OFFERS',
          framework_id=dict(value=framework_id),
    ...
    def declineOffer(self, offer_ids, filters=None):
        ...
        framework_id = self.framework_id
        assert framework_id
        decline = dict(offer_ids=([offer_ids] if isinstance(offer_ids, dict) else offer_ids))
        if filters is not None:
            decline['filters'] = filters
        body = dict(type='DECLINE',
          framework_id=dict(value=framework_id),
          decline=decline)
        self._send(body)
    ...
    def declineInverseOffer(self, offer_ids, filters=None):
        ...
        framework_id = self.framework_id
        assert framework_id
        decline_inverse_offers = dict(inverse_offer_ids=([offer_ids] if isinstance(offer_ids, dict) else offer_ids))
        if filters is not None:
            decline_inverse_offers['filters'] = filters
        body = dict(type='DECLINE_INVERSE_OFFERS',
          framework_id=dict(value=framework_id),
    ...
    def acknowledgeOperationStatusUpdate(self, status):
        if self.connected:
            if 'uuid' in status:
                if 'operation_id' in status:
                    framework_id = self.framework_id
                    assert framework_id
                    body = dict(type='ACKNOWLEDGE_OPERATION_STATUS',
                      framework_id=dict(value=framework_id),
    ...
    def reconcileTasks(self, tasks):
        ...
        framework_id = self.framework_id
        assert framework_id
        body = dict(type='RECONCILE',
          framework_id=dict(value=framework_id),
          reconcile=dict(tasks=[dict(task_id=(task['task_id'])) for task in tasks]))
        self._send(body)
    ...
    def reconcileOperations(self, operations_):
        ...
        body = dict(type='RECONCILE_OPERATIONS',
          framework_id=dict(value=framework_id),
    ...
    def sendFrameworkMessage(self, executor_id, agent_id, data):
        ...
        framework_id = self.framework_id
        assert framework_id
        message = dict(agent_id=agent_id,
          executor_id=executor_id,
          data=data)
        body = dict(type='MESSAGE',
          framework_id=dict(value=framework_id),
          message=message)
        self._send(body)
    ...
    def onNewMasterDetectedMessage(self, data):
        ...
        try:
            ...
            logger.exception('No JSON content, probably connecting to older Mesos version.')
    ...
    def on_subscribed(self, info):
        ...
        port = int(port)
        master_info = dict(hostname=hostname,
          port=port)
        if self.version:
            master_info['version'] = self.version
        else:
            if 'master_info' in info:
                if 'version' in info['master_info']:
                ...
                self.sched.registered(self, self._dict_cls(framework_id), self._dict_cls(master_info))

    def on_offers(self, event):
        offers = event.get('offers', [])
        if offers:
            self.sched.resourceOffers(self, [self._dict_cls(offer) for offer in offers])
        version = self.version and tuple((int(n) for n in self.version.split('.')))
    ...
    def on_inverse_offers(self, event):
        ...
        if inverse_offers:
            self.sched.inverseOffers(self, [self._dict_cls(offer) for offer in inverse_offers])
    ...
    def on_rescind_inverse_offer(self, event):
        ...
        self.sched.inverseOfferRescinded(self, self._dict_cls(inverse_offer_id))
    ...
    def on_message(self, message):
        ...
        data = message['data']
        self.sched.frameworkMessage(self, self._dict_cls(executor_id), self._dict_cls(agent_id), data)
    ...
    def on_failure(self, failure):
        ...
        if 'executor_id' not in failure:
            ...
            self.sched.executorLost(self, self._dict_cls(failure['executor_id']), self._dict_cls(agent_id), failure['status'])
