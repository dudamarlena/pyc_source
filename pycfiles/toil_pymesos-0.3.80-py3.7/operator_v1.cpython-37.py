...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymesos/operator_v1.py
# Compiled at: 2020-04-01 21:31:02
# Size of source mod 2**32: 17734 bytes
import json, logging
...
class MesosOperatorDaemonDriver(OperatorDaemonDriver):
    ...
    def getHealth(self):
        body = dict(type='GET_HEALTH')
        return self._send(body)

    def getFlags(self):
        body = dict(type='GET_FLAGS')
        return self._send(body)

    def getVersion(self):
        body = dict(type='GET_VERSION')
        return self._send(body)

    def getMetrics(self, timeout):
        body = dict(type='GET_METRICS',
          get_metrics=dict(timeout=dict(nanoseconds=timeout)))
        return self._send(body)

    def getLoggingLevel(self):
        body = dict(type='GET_LOGGING_LEVEL')
        return self._send(body)

    def setLoggingLevel(self, level, duration):
        body = dict(type='SET_LOGGING_LEVEL',
          set_logging_level=dict(duration=dict(nanoseconds=duration),
          level=level))
        return self._send(body)

    def listFiles(self, path):
        body = dict(type='LIST_FILES',
          list_files=dict(path=path))
        return self._send(body)

    def readFile(self, path, length, offset):
        body = dict(type='READ_FILE',
          read_file=dict(length=length,
          offset=offset,
          path=path))
        return self._send(body)

    def getState(self):
        body = dict(type='GET_STATE')
        return self._send(body)

    def getFrameworks(self):
        body = dict(type='GET_FRAMEWORKS')
        return self._send(body)

    def getExecutors(self):
        body = dict(type='GET_EXECUTORS')
        return self._send(body)

    def getTasks(self):
        body = dict(type='GET_TASKS')
        return self._send(body)


class MesosOperatorMasterDriver(Process, MesosOperatorDaemonDriver):

    def __init__(self, master_uri, operator=None, use_addict=False, timeout=DAY):
        """
        ...
        """
        super(MesosOperatorMasterDriver, self).__init__(master=master_uri, timeout=timeout)
    ...
    def gen_request(self):
        body = json.dumps(dict(type='SUBSCRIBE'))
        ...
        if 'type' in event:
            ...
            if _type == 'heartbeat':
                ...
                if _type not in event:
                    logger.error('Missing `%s` in event %s' % (
                     _type, event))
                    return
                event = event[_type]
    ...
    def getAgents(self):
        body = dict(type='GET_AGENTS')
        return self._send(body)

    def getRoles(self):
        body = dict(type='GET_ROLES')
        return self._send(body)

    def getWeights(self):
        body = dict(type='GET_WEIGHTS')
        return self._send(body)

    def updateWeights(self, weight_infos):
        body = dict(type='UPDATE_WEIGHTS',
          update_weights=dict(weight_infos=[dict(role=(weight_info['role']), weight=(weight_info['weight'])) for weight_info in weight_infos]))
        self._send(body)

    def getMaster(self):
        body = dict(type='GET_MASTER')
        return self._send(body)

    def reserveResources(self, agent_id, resources):
        body = dict(type='RESERVE_RESOURCES',
          reserve_resources=dict(agent_id=dict(value=agent_id),
          resources=resources))
        self._send(body)

    def unreserveResources(self, agent_id, resources):
        body = dict(type='UNRESERVE_RESOURCES',
          unreserve_resources=dict(agent_id=dict(value=agent_id),
          resources=resources))
        self._send(body)

    def createVolumes(self, agent_id, volumes):
        body = dict(type='CREATE_VOLUMES',
          create_volumes=dict(agent_id=dict(value=agent_id),
          volumes=volumes))
        self._send(body)

    def destroyVolumes(self, agent_id, volumes):
        body = dict(type='DESTROY_VOLUMES',
          destroy_volumes=dict(agent_id=dict(value=agent_id),
          volumes=volumes))
        self._send(body)

    def getMaintenanceStatus(self):
        body = dict(type='GET_MAINTENANCE_STATUS')
        return self._send(body)

    def getMaintenanceSchedule(self):
        body = dict(type='GET_MAINTENANCE_SCHEDULE')
        return self._send(body)

    def updateMaintenanceSchedule(self, windows):
        body = dict(type='UPDATE_MAINTENANCE_SCHEDULE',
          update_maintenance_schedule=dict(schedule=dict(windows=windows)))
        self._send(body)

    def startMaintenance(self, machines):
        body = dict(type='START_MAINTENANCE',
          start_maintenance=dict(machines=[dict(hostname=(machine['hostname']), ip=(machine['ip'])) for machine in machines]))
        self._send(body)

    def stopMaintenance(self, machines):
        body = dict(type='STOP_MAINTENANCE',
          stop_maintenance=dict(machines=[dict(hostname=(machine['hostname']), ip=(machine['ip'])) for machine in machines]))
        self._send(body)

    def getQuota(self):
        body = dict(type='GET_QUOTA')
        return self._send(body)

    def setQuota(self, quota_request):
        body = dict(type='SET_QUOTA',
          set_quota=dict(quota_request=quota_request))
        self._send(body)

    def removeQuota(self, role):
        body = dict(type='REMOVE_QUOTA',
          remove_quota=dict(role=role))
        self._send(body)

    def markAgentGone(self, agent_id):
        body = dict(type='MARK_AGENT_GONE',
          mark_agent_gone=dict(agent_id=dict(value=agent_id)))
        self._send(body)
...
class MesosOperatorAgentDriver(MesosOperatorDaemonDriver):
    ...
    def getContainers(self, show_nested=False, show_standalone=False):
        body = dict(type='GET_CONTAINERS',
          get_containers=dict(show_nested=show_nested,
          show_standalone=show_standalone))
        return self._send(body)

    def launchNestedContainer(self, launch_nested_container):
        body = dict(type='LAUNCH_NESTED_CONTAINER',
          launch_nested_container=launch_nested_container)
        self._send(body)

    def waitNestedContainer(self, container_id, parent_id=None):
        body = dict(type='WAIT_NESTED_CONTAINER',
          wait_nested_container=dict(container_id=dict(value=container_id)))
    ...
    def killNestedContainer(self, container_id, parent_id=None):
        body = dict(type='KILL_NESTED_CONTAINER',
          kill_nested_container=dict(container_id=dict(value=container_id)))
        if parent_id is not None:
    ...
    def launchNestedContainerSession(self, launch_nested_container_session):
        body = dict(type='LAUNCH_NESTED_CONTAINER_SESSION',
    ...
    def attachContainerInput(self, container_id, process_ios):
        msg = dict(type='ATTACH_CONTAINER_INPUT',
          attach_container_input=dict(type='CONTAINER_ID',
          container_id=dict(value=container_id)))
        sort_keys = True
        msg_str = json.dumps(msg, sort_keys=sort_keys)
        record_io = '{}\n{}'.format(len(msg_str), msg_str)
        body = ''
        body += record_io
        process_io = dict(type='ATTACH_CONTAINER_INPUT',
          attach_container_input=dict(type='PROCESS_IO',
          process_io='DUMMY'))
    ...
    def attachContainerOutput(self, container_id):
        body = dict(type='ATTACH_CONTAINER_OUTPUT',
          attach_container_output=dict(container_id=dict(value=container_id)))
        headers = {'Accept':'application/recordio', 
         'Message-Accept':'application/json'}
        return self._send(body, headers=headers)

    def removeNestedContainer(self, container_id, parent_id=None):
        body = dict(type='REMOVE_NESTED_CONTAINER',
          remove_nested_container=dict(container_id=dict(value=container_id)))
        if parent_id is not None:
            body['remove_nested_container']['container_id']['parent'] = dict(value=parent_id)
        self._send(body)

    def addResourceProviderConfig(self, info):
        body = dict(type='ADD_RESOURCE_PROVIDER_CONFIG',
          add_resource_provider_config=dict(info=info))
        self._send(body)

    def updateResourceProviderConfig(self, info):
        body = dict(type='UPDATE_RESOURCE_PROVIDER_CONFIG',
          update_resource_provider_config=dict(info=info))
        self._send(body)

    def removeResourceProviderConfig(self, type, name):
        body = dict(type='REMOVE_RESOURCE_PROVIDER_CONFIG',
          remove_resource_provider_config=dict(type=type,
          name=name))
        self._send(body)

    def pruneImages(self, excluded_images=None):
        body = dict(type='PRUNE_IMAGES')
        if excluded_images is not None:
            body['prune_images'] = dict(excluded_images=excluded_images)
        self._send(body)