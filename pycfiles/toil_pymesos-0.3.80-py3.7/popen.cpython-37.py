...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymesos/subprocess/popen.py
# Compiled at: 2020-04-01 21:31:02
# Size of source mod 2**32: 13879 bytes
import os, sys, time, atexit, select, socket, signal, logging
...
class Redirector(object):
    ...
    def __init__(self):
        ...
        self._wakeup_fd = wfd
        self._aborted = False
        self._io_thread = Thread(target=(Redirector._loop),
          args=(self, rfd),
          name='Redirector')
    ...
    def _clear(self, fd):
        r = self._listeners.pop(fd, None) or self._readers.pop(fd, None) or self._writers.pop(fd, None)
        if not r:
            fd.close()
            return
        f, pid = r[:2]
            ...
            with self._lock:
                for fd in readable:
                    if fd in self._listeners:
                        ...
                        if buf:
                            f.write(buf)
                            more_to_read = bool(select.select([fd], [], [], 0)[0])
                        else:
                ...
                for fd in writeable:
                    if fd in self._readers:
                        ...
                        if buf:
                            try:
                                fd.sendall(buf)
                                more_to_read = bool(select.select([f], [], [])[0])
                            except socket.error:
                                logger.info('Closing reader %s', fd)
                                self._clear(fd)
...
class Popen(object):
    ...
    def __init__(self, args, bufsize=0, executable=None, stdin=None, stdout=None, stderr=None, preexec_fn=None, close_fds=None, shell=False, cwd=None, env=None, universal_newlines=False, startupinfo=None, creationflags=0, cpus=None, mem=None, gpus=None):
        ...
        if close_fds is False:
            logger.warning('Can not support `close_fds=False`, no fds will be inherrited.')
            ...
            if stderr == PIPE:
                ...
                if stderr is None:
                    ...
                    if stderr == STDOUT:
                        ...
                        _err = _dup_file(sys.stderr, 'wb', 0)
        self._handlers = cls._redirector.register(self.id, _in, _out, _err, self._io_complete)
                    ...
                    if self.stdin:
                        ...
                        if self.stdout:
                            if not self.stdout.closed:
                                ...
                                self.stdout = None
                        if self.stderr:
                            self.stderr.closed or self.stderr.close()
                            self.stderr = None
