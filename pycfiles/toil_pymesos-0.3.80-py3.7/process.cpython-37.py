...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymesos/process.py
# Compiled at: 2020-04-01 21:31:02
# Size of source mod 2**32: 13536 bytes
import os, re, sys, json, time, math, errno, random, select, signal, socket, logging
            ...
            if n_parsed != n_recv:
                if hasattr(self._parser, 'errno'):
                    ...
                    if self._parser.errno:
                        raise RuntimeError('Failed to parse, code:%s %s' % (
                         self._parser.errno,
                         self._parser.errstr))
                else:
                    raise RuntimeError('Failed to parse, code:%s' % (
                     self._parser.get_errno(),))
            if self._stream_id is None:
                if self._parser.is_headers_complete():
                    ...
                    if code == TEMPORARY_REDIRECT:
                        ...
                        logger.warning('Try to redirect to new master: %s', new_master)
...
class Process(object):
    ...
    def _notify(self):
        with self._lock:
            if self._wakeup_fds:
                os.write(self._wakeup_fds[1], b'\x00')
                ...
                while 1:
                    if not conn:
                        if self._master:
                            ...
                            readable, writeable, _ = select.select(to_read, to_write, [], SELECT_TIMEOUT)
                            ...
                            if not conn.connected:
                                ...
                                if err:
                                    ...
                                    conn.close()
                                    conn = None
                                    deadline = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(connect_deadline))
                                    logger.warning('connect to %s error: %s, sleep %ds and try again, deadline: %s', self._master, err, sleep_timeout, deadline)
                                    time.sleep(sleep_timeout)
                                    continue
                                else:
                                    num_conn_retry = 0
                            if writeable:
                                conn.write() or conn.close()
                                conn = None
                                self.stream_id = None
                                continue
                        if readable:
                            conn.read() or conn.close()
                            conn = None
                            self.stream_id = None
    ...
    def _backoff(self, num_conn_retry):
        ...
        new_timeout = SLEEP_TIMEOUT_INIT * math.pow(SLEEP_TIMEOUT_SCALE, num_conn_retry)
        old_timeout = new_timeout / SLEEP_TIMEOUT_SCALE
        return min(SLEEP_TIMEOUT_MAX, new_timeout + _random_time(new_timeout, old_timeout))
