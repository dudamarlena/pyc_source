...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymesos/executor.py
# Compiled at: 2020-04-01 21:31:02
# Size of source mod 2**32: 8321 bytes
import os, json, time, uuid, signal, logging
...
class MesosExecutorDriver(Process, ExecutorDriver):
    ...
    def __init__(self, executor, use_addict=False, timeout=DAY):
        ...
        super(MesosExecutorDriver, self).__init__(master=agent_endpoint, timeout=timeout)
    ...
    def gen_request(self):
        body = json.dumps(dict(type='SUBSCRIBE',
          ...
          subscribe=dict(unacknowledged_tasks=(list(self.tasks.values())),
    ...
    def on_subscribed(self, info):
        ...
        if not framework_info['id'] == self.framework_id:
            raise AssertionError
        elif self.executor_info is None or self.framework_info is None:
            self.executor_info = executor_info
            self.framework_info = framework_info
            self.executor.registered(self, self._dict_cls(executor_info), self._dict_cls(framework_info), self._dict_cls(agent_info))
        else:
