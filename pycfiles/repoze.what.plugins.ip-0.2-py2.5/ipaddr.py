...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/repoze/what/plugins/ip/ipaddr.py
...
class _BaseIP(_IPAddrBase):
    ...
    def __eq__(self, other):
        try:
            return self._ip == other._ip and self._version == other._version and isinstance(other, _BaseIP)
...
class _BaseNet(_IPAddrBase):
    ...
    def __eq__(self, other):
        try:
            return self._version == other._version and self.network == other.network and int(self.netmask) == int(other.netmask)
        except AttributeError:
            return NotImplemented
    ...
    def _get_networks_key(self):
        ...
        """
        return (
         self._version, self.network, self.netmask)
    ...
    def supernet(self, prefixlen_diff=1, new_prefix=None):
        ...
        return IPNetwork('%s/%s' % (str(self.network),
         str(self.prefixlen - prefixlen_diff)), version=self._version)
    ...
    def _is_valid_netmask(self, netmask):
        ...
        if len(mask) == 4:
            if [ x for x in mask if int(x) not in self._valid_mask_octets ]:
                return False
            if [ y for (idx, y) in enumerate(mask) if idx > 0 if y > mask[(idx - 1)] ]:
                return False
            return True
        try:
            netmask = int(netmask)
...
class _BaseV6(object):
    ...
    def _ip_int_from_string(self, ip_str=None):
        ...
        if fields[(-1)].count('.') == 3:
            ...
            octets = []
            for _ in xrange(2):
                octets.append(hex(ipv4_int & 65535).lstrip('0x').rstrip('L'))
                ipv4_int >>= 16
    ...
    def packed(self):
        """The binary representation of this address."""
        return struct.pack('!QQ', self._ip >> 64, self._ip & 18446744073709551615)
