...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/quintagroup/pingtool/PingTool.py
...
class PingTool(ATFolder, PloneFolder):
    ...
    manage_options = ({'label': 'Overview', 'action': 'manage_overview'},) + ATFolder.manage_options
    ...
    def om_icons(self):
        ...
        icons = ({'path': 'misc_/%s/tool.gif' % PROJECTNAME, 'alt': self.meta_type, 'title': self.meta_type},)
        if not getAdapter(self, ICanonicalURL, 'canonical_url_adapter').getCanonicalURL():
            icons = icons + ({'path': 'misc_/PageTemplates/exclamation.gif', 'alt': 'Error', 'title': 'PingTool needs setting canonical_url'},)
        return icons
    ...
    def pingFeedReader(self, context):
        ...
        if not pingProp['enable_ping']:
            message['portal_message'] = _('Ping is dissabled.')
            return (status, message)
        canonical_url = getAdapter(self, ICanonicalURL, 'canonical_url_adapter').getCanonicalURL()
        if not canonical_url:
            message['portal_message'] = _('Ping is impossible.Setup canonical_url.')
            return (status, message)
        sites = pingProp['ping_sites']
        message['portal_message'] = _('Select servers.')
        for site in sites:
            status = 'success'
            message['portal_message'] = _('The servers are pinged.')
