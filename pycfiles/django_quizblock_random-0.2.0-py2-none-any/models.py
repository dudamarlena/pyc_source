...
# [GCC 8.4.0]
# Embedded file name: /home/anders/work/python/django-quizblock-random/quizblock_random/models.py
...
class QuizRandom(Quiz):
    ...
    def get_random_question(self, section, quiz, user):
        ...
        if set_len > 0:
            ...
            for question in question_set:
                try:
                    current_question = QuestionUserLock.objects.filter(quiz_id=question.quiz.id).get(question_current=True, user_id=user.id)
    ...
    def edit_form(self):
        ...
        class EditForm(forms.Form):
            quiz_type = forms.CharField(widget=forms.widgets.Textarea(), initial=self.quiz_type)
            description = forms.CharField(widget=forms.widgets.Textarea(), initial=self.description)
            rhetorical = forms.BooleanField(initial=self.rhetorical)
            allow_redo = forms.BooleanField(initial=self.allow_redo)
            show_submit_state = forms.BooleanField(initial=self.show_submit_state)
            alt_text = '<a href="' + reverse('edit-quiz-random', args=[self.id]) + '">manage questions/answers</a>'
    ...
    def create(self, request):
        return QuizRandom.objects.create(quiz_type=request.POST.get('quiz_type', ''), description=request.POST.get('description', ''), rhetorical=request.POST.get('rhetorical', ''), allow_redo=request.POST.get('allow_redo', ''), show_submit_state=request.POST.get('show_submit_state', False))
...
class QuestionUserLock(models.Model):
    ...
    def create(self, section, question, user):
        return QuestionUserLock.objects.create(section_id=section.id, quiz_id=question.quiz.id, question_id=question.id, user_id=user.id)