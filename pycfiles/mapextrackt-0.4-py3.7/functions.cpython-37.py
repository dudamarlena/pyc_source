...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/MapExtrackt/functions.py
# Compiled at: 2020-05-09 01:00:47
# Size of source mod 2**32: 6502 bytes
import os, cv2, numpy as np, scipy
...
def get_rows_cols_no_size(no, width, height):
    ...
    for x in range(100):
        for y in range(100):
            if x * y == no:
                answers[abs(1 - abs(x * height / (y * width)))] = [
                 x, y]

    return (
     answers[sorted(answers.keys())[0]][1], answers[sorted(answers.keys())[0]][0])
...
def draw_text(img, text, subtext=None):
    ...
    smaller_size = int(size * 0.8)
    if os.name == 'nt':
        font_loc = 'C:\\Windows\\Fonts\x07rial.ttf'
    else:
        if os.name == 'posix':
            if os.path.isfile('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'):
                font_loc = '/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf'
            else:
                if os.path.isfile('/usr/share/fonts/truetype/noto/NotoSans-Regular.ttf'):
    ...
    if subtext is not None:
        ...
        sub_h = init_h + size_of_sub + img.size[0] * 0.005
        draw.text((init_w, sub_h), subtext,
          text_col,
          font=smaller_font,
          stroke_width=2,
