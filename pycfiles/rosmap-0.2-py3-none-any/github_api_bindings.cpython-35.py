...
# [GCC 8.4.0]
# Embedded file name: /home/robotics/rosmap/rosmap/api_bindings/github_api_bindings.py
# Compiled at: 2019-03-22 08:05:15
# Size of source mod 2**32: 7387 bytes
import json, logging, time, certifi, urllib3
PAGE_SIZE = 100
...
class GithubApiBindings:
    ...
    def __init__(self, username: str, password: str, rate_limit: float):
        ...
        """
        self._GithubApiBindings__username = username
        self._GithubApiBindings__password = password
        self._GithubApiBindings__rate_limit = rate_limit

    def _GithubApiBindings__form_github_request(self, url: str) -> urllib3.response:
        """
        Forms a request for the GitHub API.
        :param url: Request URL
        :return: Response
        """
        time.sleep(3600 / self._GithubApiBindings__rate_limit)
        http = urllib3.PoolManager(cert_reqs='CERT_REQUIRED', ca_certs=certifi.where())
        return http.request('GET', url, headers=urllib3.util.make_headers(basic_auth=self._GithubApiBindings__username + ':' + self._GithubApiBindings__password, user_agent=self._GithubApiBindings__username))

    def _GithubApiBindings__get_page(self, topic: str, pagesize: int, page: int, ascending: bool) -> dict:
        """
        ...
        if ascending:
            response = self._GithubApiBindings__form_github_request(url + '&sort=stars&order=asc')
        else:
            response = self._GithubApiBindings__form_github_request(url + '&sort=stars&order=dsc')
    ...
    def get_urls_of_topic(self, topic: str) -> set:
        ...
        data_asc = self._GithubApiBindings__get_page(topic, PAGE_SIZE, page, True)
        ...
        if repository_count <= 1000:
            ...
            logging.warning('[Github API Connector]: ' + 'GitHub API only allows <=1k results. ' + 'Will try to get all items by iterating multiple times.')
        repositories = set()
        ...
        while len(repositories) > old_repository_count:
            ...
            for repository in self._GithubApiBindings__get_page(topic, PAGE_SIZE, page, True)['items']:
                repositories.add(repository['clone_url'])

            if repository_count > 1000:
                for repository in self._GithubApiBindings__get_page(topic, PAGE_SIZE, page, False)['items']:
                    repositories.add(repository['clone_url'])

            page += 1

        logging.info('[Github API Connector]: Progress... [' + str(len(repositories)) + '/' + str(repository_count) + ']')
        return repositories

    def _GithubApiBindings__extract_next_url_from_header(self, header: dict) -> str:
        """
    ...
    def _GithubApiBindings__get_repo_substring(self, url, provider) -> str:
        """
    ...
    def get_issues(self, url: str, issue_state: str) -> iter:
        ...
        """
        project_string = self._GithubApiBindings__get_repo_substring(url, 'https://github.com/')
        next_uri = 'https://api.github.com/repos/' + project_string + '/issues?state=' + issue_state
        while next_uri != '':
            response = self._GithubApiBindings__form_github_request(next_uri)
            if response.status != 200:
                next_uri = ''
                ...
                next_uri = self._GithubApiBindings__extract_next_url_from_header(response.headers)
    ...
    def get_stargazer_count(self, url: str) -> int:
        ...
        """
        project_string = self._GithubApiBindings__get_repo_substring(url, 'https://github.com/')
        response = self._GithubApiBindings__form_github_request('https://api.github.com/repos/' + project_string)
        if response.status == 200:
            data = response.data
