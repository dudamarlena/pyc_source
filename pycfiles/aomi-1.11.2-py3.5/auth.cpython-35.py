...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/model/auth.py
...
import yaml, hvac, aomi.exceptions
...
class AppRoleSecret(Resource):
    ...
    def obj(self):
        ...
        s_obj = {'role_name': self.role_name, 
    ...
    def write(self, client):
        ...
        del s_obj['secret_id']
        client.create_role_custom_secret_id(self.role_name, secret_id, s_obj)

    @wrap_vault('reading')
    def read(self, client):
        try:
            return client.get_role_secret_id(self.role_name, self.obj()['secret_id'])
    ...
    def delete(self, client):
        client.delete_role_secret_id(self.role_name, self.obj()['secret_id'])
...
class LDAPGroup(Resource):
    __doc__ = 'LDAP Group Policy Mapping'
    ...
    def obj(self):
        return {'policies': sorted(self._obj.get('policies', []))}
...
class LDAPUser(Resource):
    ...
    def obj(self):
        return {'groups': ','.join(sorted(self._obj.get('groups', []))), 
         'policies': ','.join(sorted(self._obj.get('policies', [])))}


class UserPass(Auth):
    __doc__ = 'UserPass Authentication Backend'
    config_key = 'userpass'
    no_resource = True
...
class Policy(Resource):
    ...
    def obj(self):
        return render(hard_path(self.filename, self.opt.policies), self._obj).lstrip().strip().replace('\n\n', '\n')
    ...
    def read(self, client):
        ...
        if a_policy:
            return a_policy.lstrip().strip().replace('\n\n', '\n')
