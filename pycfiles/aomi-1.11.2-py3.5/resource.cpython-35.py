...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/model/resource.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 12174 bytes
"""Base Vault Resources"""
import os, shutil, logging, yaml, hvac.exceptions
from aomi.util import vault_time_to_s
from aomi.vault import wrap_hvac as wrap_vault
from aomi.helpers import is_tagged, hard_path, diff_dict, map_val, open_maybe_binary
from aomi.model.backend import MOUNT_TUNABLES, NOOP, CHANGED, ADD, DEL, OVERWRITE
import aomi.exceptions as aomi_excep
from aomi.validation import check_obj, specific_path_check, is_unicode, is_vault_time, secret_file
LOG = logging.getLogger(__name__)

class Resource(object):
    __doc__ = 'Vault Resource\n    All aomi derived Vault resources should extend this\n    class. It provides functionality for validation and\n    API CRUD operations.'
    required_fields = []
    config_key = None
    resource_key = None
    child = False
    ...
    def thaw(self, tmp_dir):
        ...
        for sfile in self.secrets():
            ...
            if not os.path.exists(src_file):
                if hasattr(self.opt, 'ignore_missing') and self.opt.ignore_missing:
                    LOG.warning(err_msg)
                    continue
                else:
    ...
    def tunable(self, obj):
        ...
        if 'tune' in obj:
            for tunable in MOUNT_TUNABLES:
                ...
                if tunable_key in self.tune and is_vault_time(self.tune[tunable_key]):
        ...
        if self.present and self.existing:
            if isinstance(self.existing, dict):
                ...
                if diff_dict(current, obj):
                    is_diff = CHANGED
            elif is_unicode(self.existing) and self.existing != obj:
                is_diff = CHANGED
    ...
    def sync(self, vault_client):
        ...
        if self.present and not self.existing:
            LOG.info('Writing new %s to %s', self.secret_format, self)
            self.write(vault_client)
        else:
            if self.present and self.existing:
                if self.diff() == CHANGED or self.diff() == OVERWRITE:
                    LOG.info('Updating %s in %s', self.secret_format, self)
                    self.write(vault_client)
            else:
                if not self.present and not self.existing:
                    LOG.info('No %s to remove from %s', self.secret_format, self)
                elif not self.present and self.existing:
                    LOG.info('Removing %s from %s', self.secret_format, self)
    ...
    def filtered(self):
        ...
        if not is_tagged(self.tags, self.opt.tags):
            LOG.info('Skipping %s as it does not have requested tags', self.path)
            return False
        if not specific_path_check(self.path, self.opt):
            LOG.info('Skipping %s as it does not match specified paths', self.path)
            return False
        return True
    ...
    def diff_write_only(resource):
        ...
        if resource.present and not resource.existing:
            return ADD
        if not resource.present and resource.existing:
            return DEL
        if resource.present and resource.existing:
            return OVERWRITE
        return NOOP
    ...
    def delete(self, client):
        ...
        except (hvac.exceptions.InvalidPath, hvac.exceptions.InvalidRequest) as vault_exception:
            if str(vault_exception).startswith('no handler for route'):
                return


class Secret(Resource):
    __doc__ = 'Vault Secrets\n    These Vault resources will have some kind of secret backend\n    underneath them. Seems to work with generic and AWS'
    config_key = 'secrets'


class Auth(Resource):
    __doc__ = 'Auth Backend'
...
class AuditLog(Resource):
    ...
    def __init__(self, log_obj, opt):
        ...
        obj = {'name': log_obj.get('name', self.backend)}
        obj_opt = dict()
