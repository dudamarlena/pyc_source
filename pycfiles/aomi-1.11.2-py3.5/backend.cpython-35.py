...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/model/backend.py
# Compiled at: 2017-11-17 12:53:02
# Size of source mod 2**32: 8315 bytes
"""Vault Secret Backends"""
import re, logging, hvac.exceptions
...
class VaultBackend(object):
    __doc__ = 'The abstract concept of a Vault backend'
    backend = None
    list_fun = None
    mount_fun = None
    unmount_fun = None
    ...
    def __init__(self, resource, opt, managed=True):
        ...
        if hasattr(resource, 'tune') and isinstance(resource.tune, dict):
            for tunable in MOUNT_TUNABLES:
                ...
                if tunable_key in resource.tune and not isinstance(resource.tune[tunable_key], tunable_type):
    ...
    def fetch(self, vault_client, backends):
        ...
        if not is_mounted(self.backend, self.path, backends) or self.tune_prefix is None:
            return
    ...
    def actually_mount(self, client):
        ...
        try:
            ...
            if self.description and a_obj:
                m_fun(self.backend, mount_point=self.path, description=self.description, config=a_obj)
            else:
                if self.description:
                    m_fun(self.backend, mount_point=self.path, description=self.description)
                else:
                    if a_obj:
                        m_fun(self.backend, mount_point=self.path, config=a_obj)
                    else:
                        m_fun(self.backend, mount_point=self.path)
...
class SecretBackend(VaultBackend):
    __doc__ = 'Secret Backends for actual Vault resources'
...
class AuthBackend(VaultBackend):
    __doc__ = 'Authentication backends for Vault access'
    ...
    def actually_mount(self, client):
        ...
        if self.description and self.config and 'description' in self.config:
            m_fun(self.backend, mount_point=self.path, description=self.config['description'])
        else:
            if self.description:
                m_fun(self.backend, mount_point=self.path, description=self.config['description'])
            else:
