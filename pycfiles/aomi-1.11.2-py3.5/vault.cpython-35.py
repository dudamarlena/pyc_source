...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/aomi/vault.py
...
import aomi.error, aomi.exceptions
...
def renew_secret(client, creds, opt):
    ...
    if client.version:
        ...
        if int(v_bits[0]) == 0 and int(v_bits[1]) <= 8 and int(v_bits[2]) <= 0:
            r_obj = {'increment': seconds}
...
class Client(hvac.Client):
    __doc__ = 'Our Vault Client Wrapper\n    This class will pass the existing hvac bits through. When interacting\n    with cubbyhole paths, it will use the non operational token in order\n    to preserve access.'

    def __init__(self, _url=None, token=None, _cert=None, _verify=True, _timeout=30, _proxies=None, _allow_redirects=True, _session=None):
        self.version = None
        ...
        ssl_verify = True
        if 'VAULT_SKIP_VERIFY' in os.environ and os.environ['VAULT_SKIP_VERIFY'] == '1':
            import urllib3
        ...
        super(Client, self).__init__(url=self.vault_addr, verify=ssl_verify, session=session)
    ...
    def connect(self, opt):
        ...
        if self.version:
            ...
            LOG.warning('Unable to deterine Vault version. Not all functionality is supported')
        LOG.info('Connected to %s as %s%s', self._url, display_name, vsn_string)
        if opt.reuse_token:
    ...
    def init_token(self):
        ...
        if 'VAULT_ROLE_ID' in os.environ and 'VAULT_SECRET_ID' in os.environ and os.environ['VAULT_ROLE_ID'] and os.environ['VAULT_SECRET_ID']:
            token = approle_token(self, os.environ['VAULT_ROLE_ID'], os.environ['VAULT_SECRET_ID'])
        ...
        if 'VAULT_USER_ID' in os.environ and 'VAULT_APP_ID' in os.environ and os.environ['VAULT_USER_ID'] and os.environ['VAULT_APP_ID']:
            token = app_token(self, os.environ['VAULT_APP_ID'].strip(), os.environ['VAULT_USER_ID'].strip())
        ...
        if approle_filename:
            ...
            if 'role_id' in creds and 'secret_id' in creds:
                token = approle_token(self, creds['role_id'], creds['secret_id'])
