...
# [GCC 8.4.0]
# Embedded file name: /Users/takada-at/python2/lib/python2.7/site-packages/tests/test_service.py
...
def test_save(config, mock_groups):
    with mock.patch('sg.service._confirm') as (dummy):
        ...
        SgService.save_groups(config, client, config.base_path / 'security_groups')
...
def test_diff(config, mock_groups):
    ...
    with mock.patch('sg.service._confirm') as (dummy):
        ...
        group.revoke(ip_protocol='tcp', from_port=22, to_port=22, cidr_ip='192.168.1.0/32')
        group.authorize(ip_protocol='tcp', from_port=22, to_port=22, cidr_ip='192.168.1.10/32')
        group.authorize(ip_protocol='tcp', from_port=22, to_port=22, cidr_ip='192.168.1.20/32')
...
def test_diff_list(config, mock_groups):
    tempdir = config.base_path
    client = AwsClient(config)
    SgService.save_groups(config, client, tempdir / 'security_groups', noconfirm=True)
    file_path = tempdir / 'security_groups/mock-group.csv'
    assert file_path.exists()
    group = client.get('mock-group')
    group.revoke(ip_protocol='tcp', from_port=22, to_port=22, cidr_ip='192.168.1.0/32')
    ...
    group2.authorize(ip_protocol='tcp', from_port=22, to_port=22, cidr_ip='192.168.1.10/32')
...
def test_commit(config, mock_groups):
    ...
    with mock.patch('sg.service._confirm') as (dummy):
        ...
        assert Rule(ip_protocol='tcp', from_port='22', to_port='22', cidr_ip='192.168.1.0/32', group=None) in rules
        assert Rule(ip_protocol='tcp', from_port='22', to_port='22', cidr_ip='192.168.1.10/32', group=None) not in rules
    return


def test_list_files_local(config, files):
    u"""SgService.list_files_localのテスト.
...
def test_commit_list(config, files):
    ...
    gr0 = Grant(ip_protocol='tcp', from_port='22', to_port='22', cidr_ip='192.168.10.0/32')
    gr1 = Grant(ip_protocol='tcp', from_port='33', to_port='44', group='mock-group2')
    gr2 = Grant(ip_protocol='tcp', from_port='22', to_port='22', cidr_ip='192.168.20.0/32')
    ...
    SgService.commit_list(config=config, client=AwsClient(config), file_path_list=[], noconfirm=True)
    diff_list = dict(SgService.diff_list(config, AwsClient(config), []))
