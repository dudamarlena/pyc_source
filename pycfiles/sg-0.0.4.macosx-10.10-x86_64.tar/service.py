...
# [GCC 8.4.0]
# Embedded file name: /Users/takada-at/python2/lib/python2.7/site-packages/sg/service.py
# Compiled at: 2015-12-16 03:37:52
...
def to_str(obj):
    if isinstance(obj, dict):
        return {to_str(key):to_str(val) for key, val in obj.items()}
    else:
        if obj is None:
            return obj
        else:
...
class SgService(object):
    ...
    @staticmethod
    def save_groups(config, client, path, noconfirm=False):
        u"""グループ設定をまとめて保存.
        ...
        for gr in client.groups:
            savepath = SgService.save(client=client, base_path=path, group=gr, noconfirm=noconfirm)
            if not savepath:
                continue
            save_path_list.append(savepath)
            group_settings.append(OrderedDict([('path', str(savepath.name)),
             (
              'group', gr.name),
             (
              'id', gr.id)]))
    ...
    @staticmethod
    def save(client, base_path, group, noconfirm=False):
        u"""セキュリティグループ設定をcsvに保存.
        ...
        diff = SgService.diff(client=client, group=group.name, target_file=file_path)
        ...
        if not file_path.exists() or noconfirm:
            ...
            allow = _confirm('save this setting?', more=('\n').join(more))
        if not allow:
            return file_path
        grants = []
        if file_path.exists():
    ...
    def diff(client, group, target_file):
        ...
        return Diff(local_only=file_contents - current_rules, remote_only=current_rules - file_contents)
    ...
    def diff_list(config, client, file_path_list):
        ...
        for file_path in file_path_list:
            ...
            diff = SgService.diff(client=client, group=target_group, target_file=file_path)
    ...
    def list_files_remote(config, client):
        u"""csvをリストアップ. 存在するセキュリティグループを基準にする.
    ...
    def list_files_local(config):
        u"""csvをリストアップ. ローカルにあるファイルを基準とする.
    ...
    @staticmethod
    def commit(client, diff, target_group, noconfirm=False):
        u"""リモートに保存.
        ...
        if noconfirm:
            ...
            allow = _confirm('post this setting?', more=('\n').join(more))
        if allow:
            for rule in diff.local_only:
                client.authorize(target_group=target_group, rule=rule)
                print('Add %s' % rule.as_line())

            for rule in diff.remote_only:
                client.remove_rule(target_group=target_group, rule=rule)
                print('Delete %s' % rule.as_line())

    @staticmethod
    def commit_list(config, client, file_path_list, noconfirm=False):
        u"""まとめてリモート保存.
        ...
        for file_path in file_path_list:
            ...
            if not group:
                continue
            diff = SgService.diff(client=client, group=group, target_file=file_path)
            SgService.commit(client=client, diff=diff, target_group=group, noconfirm=noconfirm)