...
# [GCC 8.4.0]
# Embedded file name: /Users/takada-at/python2/lib/python2.7/site-packages/sg/commands.py
...
def fetch(ctx, no_confirm=False):
    ...
    SgService.save_groups(config=ctx.obj['config'], client=sg, path=pathobj, noconfirm=no_confirm)


@group.command('diff')
@click.argument('file_path_list', type=click.Path(exists=True), nargs=-1)
@click.pass_context
def diff_(ctx, file_path_list):
    ...
    diffs = SgService.diff_list(config=config, client=client, file_path_list=file_path_list)
...
def commit(ctx, file_path_list, no_confirm=False):
    ...
    client = AwsClient(ctx.obj['config'])
    config = ctx.obj['config']
    SgService.commit_list(config=config, client=client, file_path_list=file_path_list, noconfirm=no_confirm)