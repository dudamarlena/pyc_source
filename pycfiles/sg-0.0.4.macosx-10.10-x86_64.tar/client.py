...
# [GCC 8.4.0]
# Embedded file name: /Users/takada-at/python2/lib/python2.7/site-packages/sg/client.py
# Compiled at: 2015-12-15 21:36:16
from __future__ import absolute_import, division, print_function
import boto, boto.ec2, boto.vpc, io
...
class Config(object):
    ...
    def __init__(self, config_path=None, region=None, base_path='.'):
    ...
    def get_connection(self):
        if self.key_file:
            with Path(self.key_file).open() as (fp):
                ...
                args = dict(aws_access_key_id=key, aws_secret_access_key=secretkey)
        else:
            args = dict()
        if self.region:
...
class AwsClient(object):
    ...
    def authorize(self, target_group, rule):
        ...
        if rule.cidr_ip:
            group_data.authorize(ip_protocol=rule.ip_protocol, from_port=rule.from_port, to_port=rule.to_port, cidr_ip=rule.cidr_ip, src_group=None)
        else:
            src_group = self.get(rule.group)
            group_data.authorize(ip_protocol=rule.ip_protocol, from_port=rule.from_port, to_port=rule.to_port, src_group=src_group)
        return

    def remove_rule(self, target_group, rule):
        group_data = self.get(group=target_group)
        if rule.cidr_ip:
            group_data.revoke(ip_protocol=rule.ip_protocol, from_port=rule.from_port, to_port=rule.to_port, cidr_ip=rule.cidr_ip, src_group=None)
        else:
            src_group = self.get(rule.group)
            group_data.revoke(ip_protocol=rule.ip_protocol, from_port=rule.from_port, to_port=rule.to_port, src_group=src_group)
        return
