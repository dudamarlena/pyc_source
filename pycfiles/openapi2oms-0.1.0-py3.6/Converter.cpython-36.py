...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/openapi2oms/Converter.py
...
class Converter:
    ...
    phases = None
    phases: typing.List[MappingPhase]
    oms: dict = {'oms':1,  'source':'openapi',  'hostedExternally':True}
    order_consume_http_successful_responses = [
     '200', '201', '202', '2XX',
     '204', 'default']
    http_base_url: str = ''
    ...
    def __init__(self, contents: dict, properties: dict):
        ...
        self.resolved_spec = ResolvingParser(spec_string=(json.dumps(self.contents)))
        if not properties:
            properties = {}
    ...
    def consume_info(self):
        ...
        self.oms['info'] = {'version':op_info['version'], 
         'title':op_info['title'], 
         'description':op_info.get('description', ''), 
         'license':{'name':op_info.get('license', {}).get('name', ''), 
          'url':op_info.get('license', {}).get('url', '')}}
        self.oms['contact'] = {'name':self.op_spec.get('contact', {}).get('name', ''), 
    ...
    def _get_argument_type(self, param_spec: dict) -> str:
        ...
        raise ConverterError('Using the "content" key to specify parameter serialising strategy is not supported at this time.')
    ...
    def _to_action(self, path: str, method: str, path_spec: dict) -> dict:
        ...
        if len(servers_for_this_path) > 0:
            if len(servers_for_this_path) > 1:
                raise ConverterError(f'Multiple server endpoints were found for the path "{path}". Only zero or one entries are supported.')
            base_url = servers_for_this_path[0]['url']
        url = f"{base_url}{path}"
        parsed_url = urlparse(url)
        if parsed_url.scheme != 'http':
            if parsed_url.scheme != 'https':
                raise ConverterError(f'The URL for the path "{path}" is invalid. Derived value is "{url}" (must have http(s) as the scheme)')
        arguments = {}
        action_def = {'help':path_spec.get('summary'), 
         'http':{'method':method, 
          'url':url}, 
         'arguments':arguments}
    ...
    def _insert_output(self, action_def: dict, path_spec: dict):
        ...
        content = chosen_response.get('content', {})
        if not content or len(content) == 0:
            action_def['output'] = {'type':None, 
             'help':description}
            return
        ...
        if content_type == 'text/plain':
            output_type = 'string'
        props = {}
        action_def['output'] = {'type':output_type, 
        ...
        if len(schema.get('properties', {})) > 0:
            self._insert_arguments(props, schema, include_location=False)

    def _insert_arguments(self, arguments, content_spec, include_location=True):
        for prop_name, prop_spec in content_spec['properties'].items():
            arg_type = OpenAPIActionUtil.to_oms_type(prop_spec['type'])
            arg = {'required':prop_spec.get('required', False), 
             'type':arg_type, 
             'in':'requestBody'}
            ...
            if arg_type == 'object':
                sub_props = {}
                self._insert_arguments(sub_props, prop_spec, include_location=False)
    ...
    def _get_preferred_content_type(self, possible_content_types) -> str:
        ...
        if len(possible_content_types) > 1:
            if 'application/json' in possible_content_types:
    ...
    def _insert_request_body(self, action, arguments, path_spec):
        ...
        if not request_body:
            ...
            if content_spec['type'] == 'object':
                ...
                arg = {'help':content_spec.get('description'), 
    ...
    def _insert_parameters(self, arguments, path_spec):
        for param in path_spec.get('parameters', []):
            arg = {'help':param.get('description'),  'required':param['required'], 
        ...
        for path in self.op_spec['paths']:
            for method, content in self.op_spec['paths'][path].items():
                if method not in ('get', 'head', 'post', 'put', 'delete', 'patch'):
                    ...
                    action_name = OpenAPIActionUtil.generate_action_name(path, method, content)
                    actions[action_name] = self._to_action(path, method, content)

    def consume_servers(self):
        servers = self.op_spec.get('servers', {})
        if len(servers) > 1:
            if self.properties.get(Properties.SERVER_INDEX, -1) == -1:
                raise ConverterError(f"The property {Properties.SERVER_INDEX} must be set when the OpenAPI spec contains more than 1 server. Found {len(servers)} entries.")
        server_index = self.properties.get(Properties.SERVER_INDEX, 0)
        if server_index > len(servers) - 1:
            raise ConverterError(f"Invalid value set for property {Properties.SERVER_INDEX}. Min: 0, max: {len(servers) - 1}, got: {server_index}")
        if len(servers) > 0:
            selected_server = servers[server_index]
            self.http_base_url = selected_server['url']
            if len(selected_server.get('variables', {})) > 0:
                raise ConverterError('Variables in the server object are not supported at this time.')
    ...
    def convert(self) -> dict:
        for phase in self.phases:
            ...
            if phase.required:
                if part is None:
                    raise ConverterError(f"OpenAPI field {phase.key} not found in the root")
                if part is not None:
                    phase.function()
