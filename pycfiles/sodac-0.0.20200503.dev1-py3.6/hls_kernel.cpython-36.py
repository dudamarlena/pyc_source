...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/codegen/xilinx/hls_kernel.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 29310 bytes
import collections, functools, logging, operator
...
def _print_interface(printer, kernel_name, inputs, outputs, super_source):
  ...
  """
    ...
    for name, c_type, bank, depth in outputs + inputs:
        println('#pragma HLS interface m_axi port={} offset=slave depth={} bundle={}'.format(get_port_name(name, bank), depth, get_bundle_name(name, bank)), 0)

    println()
    for name, _, bank, _ in outputs + inputs:
        println('#pragma HLS interface s_axilite port={} bundle=control'.format(get_port_name(name, bank)), 0)

    println('#pragma HLS interface s_axilite port=coalesced_data_num bundle=control', 0)
    println('#pragma HLS interface s_axilite port=return bundle=control', 0)
    println()
    for name, c_type, bank, _ in inputs + outputs:
        println('hls::stream<Data<{c_type}>> {name}("{name}");'.format(name=(get_port_buf_name(name, bank)),
          c_type=c_type))
        println('#pragma HLS stream variable={} depth=32'.format(get_port_buf_name(name, bank)), 0)
        println(('#pragma HLS data_pack variable={}'.format(get_port_buf_name(name, bank))),
          indent=0)
    ...
    for node in super_source.tpo_valid_node_gen():
        for fifo in node.fifos:
            ...
            println('#pragma HLS stream variable={} depth={}'.format(fifo.c_expr, fifo.depth + 2), 0)
            println(('#pragma HLS data_pack variable={}'.format(fifo.c_expr)), indent=0)

    println()
    println('#pragma HLS dataflow', 0)
    for name, _, bank, _ in inputs:
        println('BurstRead(&{}, {}, coalesced_data_num);'.format(get_port_buf_name(name, bank), get_port_name(name, bank)))
    ...
    for name, _, bank, _ in outputs:
        println('BurstWrite({}, &{}, coalesced_data_num);'.format(get_port_name(name, bank), get_port_buf_name(name, bank)))
...
def print_header(printer):
    println = printer.println
    for header in ('float', 'math', 'stdbool', 'stddef', 'stdint', 'stdio', 'string',
                   'ap_int', 'hls_stream'):
...
def print_code(stencil, output_file):
    ...
    for module_trait_id, module_trait in enumerate(stencil.module_traits):
        print_module_definition(printer, module_trait, module_trait_id, burst_width=(stencil.burst_width))
    ...
    _print_interface(printer, stencil.app_name + '_kernel', inputs, outputs, stencil.dataflow_super_source)
...
def _print_module_func_call(printer, node, module_trait_id, **kwargs):
    ...
    dram_reads = tuple('/* input*/ &' + util.get_port_buf_name(dram_ref.var, bank) for dram_ref, bank in node.dram_reads)
    dram_writes = tuple('/*output*/ &' + util.get_port_buf_name(dram_ref.var, bank) for dram_ref, bank in node.dram_writes)
...
def print_module_definition(printer, module_trait, module_trait_id, **kwargs):
    ...
    fifo_loads = tuple('/* input*/ hls::stream<Data<{}>>* {}'.format(_.c_type, _.ld_name) for _ in module_trait.loads)
    fifo_stores = tuple('/*output*/ hls::stream<Data<{}>>* {}{}'.format(expr.c_type, ir.FIFORef.ST_PREFIX, idx) for idx, expr in enumerate(module_trait.exprs))
    reads_in_lets = tuple(_.expr for _ in module_trait.lets)
    writes_in_lets = tuple(_.name for _ in module_trait.lets if not isinstance(_.name, str))
    ...
    if dram_reads:
        ...
        for dram_read in dram_reads:
            dram_read_map.setdefault(dram_read.var, collections.OrderedDict()).setdefault(dram_read.dram, []).append(dram_read)
        ...
        for var in dram_read_map:
            for dram in dram_read_map[var]:
                ...
                dram_read_map[var][dram] = collections.OrderedDict((_.offset, _) for _ in dram_read_map[var][dram])
                dram_reads = dram_read_map[var][dram]
                num_banks = len(next(iter(dram_reads.values())).dram)
                if var in num_bank_map:
                    if not num_bank_map[var] == num_banks:
                        raise AssertionError('inconsistent num banks')
                    else:
                        num_bank_map[var] = num_banks
                    _logger.debug('dram reads: %s', dram_reads)
                    assert tuple(sorted(dram_reads.keys())) == tuple(range(batch_size)), 'unexpected DRAM accesses pattern %s' % dram_reads
                    batch_width = sum(_.haoda_type.width_in_bits for _ in dram_reads.values())
                    del dram_reads
                    assert burst_width * num_banks >= batch_width and burst_width * num_banks % batch_width == 0, 'cannot process such a burst'
                    coalescing_factor = burst_width * num_banks // batch_width
                    ii = coalescing_factor
                else:
                    assert batch_width * num_banks % burst_width == 0, 'cannot process such a burst'
                    raise util.InternalError('cannot process such a burst yet')

            dram_reads = tuple(next(iter(_.values())) for _ in dram_read_map[var].values())
            all_dram_reads += dram_reads
            fifo_loads += tuple('/* input*/ hls::stream<Data<ap_uint<{burst_width}>>>* {bank_name}'.format(burst_width=burst_width, bank_name=(_.dram_fifo_name(bank))) for _ in dram_reads for bank in _.dram)
        ...
        if dram_writes:
            ...
            for var in dram_write_map:
                for dram in dram_write_map[var]:
                    ...
                    dram_write_map[var][dram] = collections.OrderedDict((_.offset, _) for _ in dram_write_map[var][dram])
                    ...
                    if var in num_bank_map:
                        ...
                        assert tuple(sorted(dram_writes.keys())) == tuple(range(batch_size)), 'unexpected DRAM accesses pattern %s' % dram_writes
                        batch_width = sum(_.haoda_type.width_in_bits for _ in dram_writes.values())
                        del dram_writes
                        assert burst_width * num_banks >= batch_width and burst_width * num_banks % batch_width == 0, 'cannot process such a burst'
                        coalescing_factor = burst_width * num_banks // batch_width
                        ii = coalescing_factor
                ...
                dram_writes = tuple(next(iter(_.values())) for _ in dram_write_map[var].values())
                fifo_stores += tuple('/*output*/ hls::stream<Data<ap_uint<{burst_width}>>>* {bank_name}'.format(burst_width=burst_width, bank_name=(_.dram_fifo_name(bank))) for _ in dram_writes for bank in _.dram)

    printer.print_func(('void {func_name}'.format)(**locals()), (fifo_stores + fifo_loads),
      align=0)
    do_scope(func_name)
    for dram_ref, bank in module_trait.dram_writes:
        println('#pragma HLS data_pack variable = {}'.format(dram_ref.dram_fifo_name(bank)), 0)
    ...
    for delay in delays:
        println('#pragma HLS dependence variable=%s inter false' % delay.buf_name, 0)

    println('if (%s)' % ' && '.join('!{fifo}->empty()'.format(fifo=fifo) for fifo in tuple(_.ld_name for _ in module_trait.loads) + tuple(_.dram_fifo_name(bank) for _ in all_dram_reads for bank in _.dram)))
    ...
    if not dram_write_map:
        for fifo_in in module_trait.loads:
            println(('const bool {fifo_in.ref_name}_enable = ReadData(&{fifo_in.ref_name}, {fifo_in.ld_name});'.format)(**locals()))

    else:
        for dram in all_dram_reads:
            for bank in dram.dram:
                println('const bool {dram_buf_name}_enable = ReadData(&{dram_buf_name}, {dram_fifo_name});'.format(dram_buf_name=(dram.dram_buf_name(bank)),
                  dram_fifo_name=(dram.dram_fifo_name(bank))))

        if not dram_write_map:
            println('const bool enabled = %s;' % ' && '.join(tuple('{_.ref_name}_enable'.format(_=_) for _ in module_trait.loads) + tuple('{}_enable'.format(_.dram_buf_name(bank)) for _ in all_dram_reads for bank in _.dram)))
        ...
        def mutate_dram_ref_for_writes(obj, kwargs):
            if isinstance(obj, ir.DRAMRef):
                ...
                return ir.Var(name='{}({msb}, {lsb})'.format((obj.dram_buf_name(bank)),
                  msb=msb, lsb=lsb),
                  idx=())
            else:
                return obj
            ...
            for coalescing_idx in range(coalescing_factor):
                for fifo_in in module_trait.loads:
                    if coalescing_idx == coalescing_factor - 1:
                        prefix = 'const bool {fifo_in.ref_name}_enable = '.format(fifo_in=fifo_in)
                    else:
                        prefix = ''
                    println('{prefix}ReadData(&{fifo_in.ref_name}, {fifo_in.ld_name});'.format(fifo_in=fifo_in,
                      prefix=prefix))

                if coalescing_idx == coalescing_factor - 1:
                    println('const bool enabled = %s;' % ' && '.join(tuple('{_.ref_name}_enable'.format(_=_) for _ in module_trait.loads) + tuple('{}_enable'.format(_.dram_buf_name(bank)) for _ in dram_reads for bank in _.dram)))
                    println('enable = enabled;')
                for idx, let in enumerate(module_trait.lets):
                    let = let.visit(mutate_dram_ref_for_writes, {'coalescing_idx':coalescing_idx, 
                     'unroll_factor':len(dram_write_map[let.name.var][let.name.dram])})
                    println('{} = Reinterpret<ap_uint<{width}>>({});'.format((let.name),
                      (let.expr.c_expr), width=(let.expr.haoda_type.width_in_bits)))

            for var in dram_write_map:
                for dram in (next(iter(_.values())) for _ in dram_write_map[var].values()):
                    for bank in dram.dram:
                        println('WriteData({}, {}, enabled);'.format(dram.dram_fifo_name(bank), dram.dram_buf_name(bank)))
        ...
        def mutate_dram_ref_for_reads(obj, kwargs):
            if isinstance(obj, ir.DRAMRef):
                ...
                lsb = elem_idx // num_banks * type_width
                msb = lsb + type_width - 1
                return ir.Var(name='Reinterpret<{c_type}>(static_cast<ap_uint<{width}>>({dram_buf_name}({msb}, {lsb})))'.format(c_type=(obj.c_type),
        ...
        if dram_read_map:
            for coalescing_idx in range(coalescing_factor):
                for idx, expr in enumerate(module_trait.exprs):
                    println('WriteData({}{}, {}, {});'.format(ir.FIFORef.ST_PREFIX, idx, expr.visit(mutate_dram_ref_for_reads, {'coalescing_idx':coalescing_idx, 
                     'unroll_factor':len(dram_read_map[expr.var][expr.dram])}).c_expr, 'true' if coalescing_idx < coalescing_factor - 1 else 'enabled'))

        else:
            for idx, expr in enumerate(module_trait.exprs):
                println('WriteData({}{}, {}({}), enabled);'.format(ir.FIFORef.ST_PREFIX, idx, expr.c_type, expr.c_expr))
...
def _print_read_data(printer):
    println = printer.println
    println('template<typename T> inline bool ReadData(T* data, hls::stream<Data<T>>* from)')
    printer.do_scope()
    println('#pragma HLS inline', indent=0)
...
def _print_write_data(printer):
    println = printer.println
    println('template<typename T> inline void WriteData(hls::stream<Data<T>>* to, const T& data, bool ctrl)')
    printer.do_scope()
    println('#pragma HLS inline', indent=0)
    println('Data<T> tmp;')
