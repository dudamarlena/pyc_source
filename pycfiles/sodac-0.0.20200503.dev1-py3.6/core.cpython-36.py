...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/core.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 32482 bytes
import collections, itertools, logging, operator
from typing import Dict, List, Tuple, Union
import cached_property, pulp, toposort
from haoda import ir, util
from haoda.ir import arithmetic
import soda.tensor, soda.util
...
class Stencil:
    ...
    def __init__(self, **kwargs):
            ...
            if dram_in is not None:
                if ':' in dram_in:
                    ...
                    for dram_map in dram_in.split('^'):
                        ...
                        input_stmt_map[var_name].dram = tuple(map(int, bank_list.split('.')))
            ...
            if dram_out is not None:
                if ':' in dram_out:
                    ...
                    for dram_map in dram_out.split(','):
                        ...
                        output_stmt_map[var_name].dram = tuple(map(int, bank_list.split('.')))
        ...
        if self.iterate > 1:
            if len(self.input_stmts) != len(self.output_stmts):
                raise util.SemanticError('number of input tensors must be the same as output if iterate > 1 times, currently there are %d input(s) but %d output(s)' % (
                 len(self.input_stmts), len(self.output_stmts)))
            if self.input_types != self.output_types:
                raise util.SemanticError('input must have the same type(s) as output if iterate > 1 times, current input has type %s but output has type %s' % (
                 util.lst2str(self.input_types), util.lst2str(self.output_types)))
            _logger.debug('pipeline %d iterations of [%s] -> [%s]' % (
             self.iterate,
             ', '.join('%s: %s' % (stmt.haoda_type, stmt.name) for stmt in self.input_stmts),
             ', '.join('%s: %s' % (stmt.haoda_type, stmt.name) for stmt in self.output_stmts)))
        ...
        _logger.debug('producer tensors: [%s]', ', '.join(tensor.name for tensor in self.producer_tensors))
        _logger.debug('consumer tensors: [%s]', ', '.join(tensor.name for tensor in self.consumer_tensors))
    ...
    def __str__(self) -> str:
        stmts = self.input_stmts + self.param_stmts + self.local_stmts + self.output_stmts
    ...
    def new_cr_var(self) -> str:
        while 1:
            ...
            self._cr_counter += 1
            if var not in {stmt.name for stmt in self.input_stmts + self.local_stmts + self.output_stmts}:
                return var

    @cached_property.cached_property
    def stmt_table(self) -> Dict[(str,
 Union[(soda.grammar.InputStmt, soda.grammar.LocalStmt,
  soda.grammar.OutputStmt, soda.grammar.ParamStmt)])]:
        return {stmt.name:stmt for stmt in self.input_stmts + self.local_stmts + self.output_stmts + self.param_stmts}
    ...
    """
        symbol_table = {}
        for name, haoda_type in zip(itertools.chain(self.input_names, self.local_names, self.output_names), itertools.chain(self.input_types, self.local_types, self.output_types)):
                ...
                def all_indices():
                    for load in loads:
                        if load.name in self.input_names:
                            ...
                            yield tuple(x + y for x, y in zip(load.idx, get_norm_idx(self.stmt_table[load.name])))

                norm_idx = tuple(x - y for x, y in zip(min((all_indices()), key=(lambda idx: tuple(reversed(tuple(idx))))), stmt.ref.idx))
    ...
    """
            ...
            if name in self.input_names:
                ...
                if name in self.output_names:
                    if iteration < self.iterate - 1:
                        return self.input_names[self.output_names.index(name)] + '_iter%d' % (iteration + 1)
                    else:
                        return name
        ...
        for iteration in range(self.iterate):
            ...
            tensors = []
            for stmt in itertools.chain(self.local_stmts, self.output_stmts):
                tensor = soda.tensor.Tensor(stmt.visit(mutate_name_callback), self.tile_size)
        ...
        lp_vars = {self.input_names[0]: 0}
        lp_helper_vars = {}
        objectives = []
        constraints = []
        for tensor in tensor_map.values():
            lp_var = pulp.LpVariable(('produced_offset_' + tensor.name), cat='Integer')
            lp_helper_var = pulp.LpVariable(('consumed_offset_' + tensor.name), cat='Integer')
        ...
        for st_tensor in tensor_map.values():
            for ld_tensor_name, offsets in st_tensor.ld_offsets.items():
                ...
                _logger.debug('%s @ %s accesses %s @ [%s, %s]', st_tensor.name, st_tensor.st_offset, ld_tensor_name, oldest_access, newest_access)
                lp_problem += lp_vars[ld_tensor_name] <= lp_vars[st_tensor.name] + (st_tensor.st_offset - newest_access)
                lp_problem += lp_helper_vars[ld_tensor_name] >= lp_vars[st_tensor.name] + (st_tensor.st_offset - oldest_access)
        ...
        for tensor in tensor_map.values():
            ...
            tensor.max_access = 0
            _logger.debug('%s should be produced @ %d and kept until %d', tensor.name, produce_offset, consume_offset)

        for ld_tensor in tensor_map.values():
            for st_tensor in ld_tensor.children.values():
                oldest_access = st_tensor.st_offset - min(st_tensor.ld_offsets[ld_tensor.name]) + st_tensor.produce_offset - ld_tensor.produce_offset
                newest_access = st_tensor.st_offset - max(st_tensor.ld_offsets[ld_tensor.name]) + st_tensor.produce_offset - ld_tensor.produce_offset
                _logger.debug('  producing %s @ %s accesses [%s, %s] pixels before %s produced @ %s', st_tensor.name, st_tensor.produce_offset, newest_access, oldest_access, ld_tensor.name, ld_tensor.produce_offset)
                ld_tensor.max_access = max(ld_tensor.max_access, oldest_access)

        for tensor in tensor_map.values():
            _logger.debug('%s should be kept for %s pixels', tensor.name, tensor.max_access)

        for tensor in tensor_map.values():
            if tensor.name in self.input_names:
                _logger.debug('<input tensor>: %s', tensor)
            else:
                if tensor.name in self.output_names:
    ...
    """
        return list(map(self.tensors.get, toposort.toposort_flatten({tensor.name:set(tensor.parents) for tensor in self.tensors.values()})))
    ...
    def all_points(self):
        all_points = {}
        for tensor in self.producer_tensors:
            all_points[tensor.name] = _get_points(self.tile_size, tensor, self.unroll_factor)
    ...
    def get_replicated_reuse_buffers(self):
        if not hasattr(self, 'replicated_reuse_buffers'):
            ...
            for tensor in self.producer_tensors:
                reuse_buffer = _get_replicated_reuse_buffer(self.tile_size, tensor, replication_factor)
                ...
                for start, end in reuse_buffer[1:]:
                    if first[(start % replication_factor)]:
                        ...
                        if start >= replication_factor:
                            self.replicated_reuse_buffer_lengths[tensor.name][end] = end // replication_factor
                            continue
                    self.replicated_reuse_buffer_lengths[tensor.name][end] = (end - start) // replication_factor

            _logger.debug('replicated_reuse_buffers: %s' % self.replicated_reuse_buffers)
            _logger.debug('replicated_reuse_buffer_lengths: %s' % self.replicated_reuse_buffer_lengths)
    ...
    def get_replicated_all_points(self):
        if not hasattr(self, 'replicated_all_points'):
            ...
            for tensor in self.producer_tensors:
                self.replicated_all_points[tensor.name] = _get_replicated_points(self.tile_size, tensor)
...
def _get_reuse_chains(tile_size, tensor, unroll_factor):
  ...
  """
    ...
    def unroll_offsets(child):
        ...
        for unroll_idx in range(unroll_factor):
            for offset in child.ld_offsets[tensor.name]:
                unrolled_offsets.add(unroll_idx + child.st_offset - offset + child.produce_offset - tensor.produce_offset)
    ...
    _logger.debug('Aâ€\xa0 of tensor %s: %s', tensor.name, A_dag)
    chains = []
    for chain_idx in reversed(range(unroll_factor)):
        chains.append(tuple(sorted(offset for offset in A_dag if offset % unroll_factor == chain_idx)))
    ...
    for child in tensor.children.values():
        ...
        for unroll_idx in range(unroll_factor):
            for idx, offset in enumerate(offsets):
                all_points[child.name].setdefault(unroll_idx + child.st_offset - offset + child.produce_offset - tensor.produce_offset, {})[unroll_factor - 1 - unroll_idx] = idx

    for child in tensor.children.values():
        for offset, points in all_points[child.name].items():
            for unroll_idx, point in points.items():
                _logger.debug('%s <- %s @ offset=%d <=> %s @ unroll_idx=%d', child.name, tensor.name, offset, util.idx2str(list(child.ld_indices[tensor.name].values())[point].idx), unroll_idx)
...
def _get_reuse_buffer(tile_size, tensor, unroll_factor):
  ...
  """
    reuse_buffer = [
     None]
    offsets = []
    for chain_id, chain in enumerate(_get_reuse_chains(tile_size, tensor, unroll_factor)):
...
def _get_replicated_reuse_chains(tile_size, tensor, replication_factor):
    _logger.debug('\x1b[1mget replicated reuse chains of tensor %s\x1b[0m' % tensor.name)
    A_dag = set()
    for stage in tensor.children:
        offsets = soda.util.serialize_iter(stage.window[tensor.name], tile_size)
        A_dag |= {max(offsets) - offset + stage.delay[tensor.name] for offset in offsets}

    _logger.debug('Aâ€\xa0 of tensor %s: %s' % (tensor.name, A_dag))
    chains = sum(reversed([tuple([
     tuple(sorted(x for x in A_dag if x % replication_factor == i))]) for i in range(replication_factor)]), ())
    for idx, chain in enumerate(chains):
        _logger.debug('reuse chain %d of tensor %s: %s' % (idx, tensor.name, chain))
...
def _get_replicated_points(tile_size, tensor):
    ...
    for stage in tensor.children:
        ...
        for idx, offset in enumerate(offsets):
            all_points[stage.name][max_offset - offset + stage.delay[tensor.name]] = idx
...
def _get_replicated_reuse_buffer(tile_size, tensor, replication_factor):
    reuse_buffer = [
     None]
    offsets = []
    for chain in _get_replicated_reuse_chains(tile_size, tensor, replication_factor):
        if chain:
...
def get_stencil_distance(stencil_window, tile_size):
    return max(soda.util.serialize_iter(stencil_window, tile_size)) + soda.util.serialize(get_stencil_window_offset(stencil_window), tile_size)


def get_stencil_dim(points):
    dimension = len(next(iter(points)))
    return [max_index - min_index + 1 for max_index, min_index in zip([max([point[dim] for point in points]) for dim in range(dimension)], [min([point[dim] for point in points]) for dim in range(dimension)])]
...
def get_overall_stencil_window(input_tensor, output_tensor):
    if isinstance(input_tensor, collections.Iterable):
        all_points = tuple(sorted((set.union)(*(set(get_overall_stencil_window(_, output_tensor)) for _ in input_tensor))))
        _logger.debug('overall stencil window of %s (%s) <- {%s} is %s (%d points)', output_tensor.name, ', '.join(['0'] * len(output_tensor.st_idx)), ', '.join(_.name for _ in input_tensor), all_points, len(all_points))
        return all_points
    else:
        idx = (
         id(input_tensor), id(output_tensor))
        ...
        for name, points in output_tensor.ld_indices.items():
            _logger.debug('%s@%s <- %s', output_tensor.name, util.idx2str(output_tensor.st_idx), util.idx2str(points.values()))
            if name != input_tensor.name:
                recursive_points = get_overall_stencil_window(input_tensor, output_tensor.parents[name])
                _logger.debug('recursive points: %s', util.idx2str(recursive_points))
                all_points |= (set.union)(*[{tuple(map(lambda a, b, c: a + b - c, _, point, output_tensor.st_idx)) for _ in recursive_points} for point in points])
            else:
                all_points |= {tuple(map(operator.sub, point, output_tensor.st_idx)) for point in points}

        all_points = tuple(sorted(all_points))
        _logger.debug('overall stencil window of %s (%s) <- %s is %s (%d points)', output_tensor.name, ', '.join(['0'] * len(output_tensor.st_idx)), input_tensor.name, all_points, len(all_points))
