    ...
    def verify_mode_depths(self) -> None:
        ...
        for node in self.tpo_valid_node_gen():
            if self in node.parents:
                ...
                latency_table[node] = max(parent.get_latency(node) + latency_table[parent] for parent in node.parents)
                min_capacity_table[node] = min(parent.get_latency(node) + latency_table[parent] + parent.fifo(node).depth for parent in node.parents)

        for node in self.tpo_valid_node_gen():
            debug_enabled = _logger.isEnabledFor(logging.DEBUG)
            check_failed = latency_table[node] > min_capacity_table[node]
            if debug_enabled or check_failed:
                _logger.debug if debug_enabled else _logger.warn('II=1 check %s: %s: latency %d %s min capacity %d', '✖' if check_failed else '✔', repr(node), latency_table[node], '>' if check_failed else '<=', min_capacity_table[node])
