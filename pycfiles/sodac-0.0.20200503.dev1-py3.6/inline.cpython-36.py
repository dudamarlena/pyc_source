...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/optimization/inline.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 10172 bytes
    ...
    if not stencil.local_stmts:
        ...
        for stmt in itertools.chain(stencil.local_stmts, stencil.output_stmts):
            for var_name, ref_list in visitor.get_load_dict(stmt).items():
                if not var_name in stencil.input_names:
                    if var_name == stmt.name:
                        pass
                    else:
                        refs.setdefault(var_name, set()).update(zip(ref_list, itertools.repeat(stmt)))

        refs = {name:next(iter(ref_set)) for name, ref_set in refs.items() if len(ref_set) == 1 if len(ref_set) == 1}
        if not refs:
            return stencil
        local_stmt_table = {stmt.name:idx for idx, stmt in enumerate(stencil.local_stmts)}
        ...
        for var_name, (ref, load_stmt) in sorted_refs:
            idx, store_stmt = {stmt.name:(idx, stmt) for idx, stmt in enumerate(stencil.local_stmts)}[var_name]
            ...
            load_stmt.let = lets + tuple(let.visit(replace_load, (ref, expr)) for let in load_stmt.let)
    ...
    if not stencil.local_stmts:
            ...
            for var_name, ref_list in visitor.get_load_dict(stmt).items():
                if not var_name in stencil.input_names:
                    if var_name == stmt.name:
                        ...
                        refs.setdefault(var_name, collections.OrderedDict()).setdefault(stmt, []).extend(ref_list)

        refs = {name:next(iter(ref_dict.items())) for name, ref_dict in refs.items() if len(visitor.get_load_set({stmt.name:stmt.expr for stmt in stencil.local_stmts}[name])) == 1}
        for name, (stmt, ref_list) in refs.items():
            _logger.info('name: %s stmt: %s ref_list: %s', name, stmt.name, util.lst2str(visitor.get_load_set({stmt.name:stmt.expr for stmt in stencil.local_stmts}[name])))
        ...
        for var_name, (load_stmt, ref_list) in sorted_refs:
            ...
            for ref in ref_list:
                ...
                lets = tuple(mutator.shift(let, offset) for let in store_stmt.let)
                expr = mutator.shift(store_stmt.expr, offset)
                _logger.info('`%s` is referenced only once by stmt %s, replace with `%s`', ref, load_stmt.name, expr)
                ref_table[ref] = expr

            replace_load = lambda obj, args: args.get(obj, obj)
            load_stmt.let = lets + tuple(let.visit(replace_load, ref_table) for let in load_stmt.let)
        ...
        for stmt in itertools.chain(stencil.local_stmts, stencil.output_stmts):
            _logger.debug('simplify  : %s', stmt)
            stmt.expr = arithmetic.simplify(arithmetic.base.reverse_distribute(stmt.expr))
            stmt.let = arithmetic.simplify(tuple(map(arithmetic.base.reverse_distribute, stmt.let)))
            _logger.debug('simplified:  %s', stmt)

        return inline2(stencil)


REBALANCE_THRESHOLDS = {ir.Type('float'): 32}
    ...
    for stmt in itertools.chain(stencil.local_stmts, stencil.output_stmts):
        if stmt.haoda_type not in REBALANCE_THRESHOLDS:
            ...
            if isinstance(stmt.expr, ir.AddSub) and set(stmt.expr.operator) == {'+'}:
                reduction = []
                for operator, operand in zip(('+', ) + getattr(stmt.expr, 'operator'), getattr(stmt.expr, 'operand')):
                    if isinstance(operand, ir.MulDiv) and operand.operator == ('*', ):
                        ...
                        if isinstance(opds[0], ir.AddSub):
                            ...
                            if isinstance(opds[1], ir.AddSub):
                ...
                if len(reductions) == 1:
                    ...
                    _logger.info("stmt %s has too many operations, breaking'em into %d ", stmt.name, len(reductions))
                    new_stmts = []
                    new_exprs = []
                    for reduction in reductions:
                        ...
                        for coeff, opds in reduction:
                            ...
                            if coeff is None:
                                ...
                                new_operands.append(ir.MulDiv(operator=('*', ), operand=(opds, coeff)))

                        new_exprs.append(stencil.propagate_type(ir.AddSub(operator=(tuple(new_operators[1:])), operand=(tuple(new_operands)))))

                    for new_expr in new_exprs[:-1]:
                        new_stmt_name = stencil.new_cr_var()
                        new_stmts.append(grammar.LocalStmt(ref=ir.Ref(name=new_stmt_name, lat=None,
                    ...
                    stmt.expr = ir.AddSub(operator=(new_exprs[(-1)].operator + ('+', ) * len(new_stmts)),
