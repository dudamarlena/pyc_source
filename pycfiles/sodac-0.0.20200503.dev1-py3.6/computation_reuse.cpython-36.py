...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/optimization/computation_reuse.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 71538 bytes
import collections, heapq, itertools, json, logging, operator, os, random, shutil, subprocess
...
def computation_reuse(stencil: 'soda.core.Stencil') -> 'soda.core.Stencil':
...
class CommSchedule(ScheduleBase):
      ...
      """
        ...
        while vars_to_process:
            ...
            for offset, src_vid in get_attrs(schedule, cr_vars):
                ...
                if src_vid not in vars_processed and cr_vars_table[src_vid] not in vars_to_process:
    ...
    def get_ir_node_with_rcr(self, stencil, rcrs: MutableMapping[(ir.BinaryOp, ir.Ref)], write_idx_table=None) -> ir.Node:
         ...
         (self.rattrs[0] + self.distance,
          self.right)):
            if isinstance(aattr, CommSchedule):
                ...
                node_with_cse = aattr.get_ir_node_with_rcr(stencil, rcrs, write_idx_table)
                node_with_cse_norm = mutator.normalize(node_with_cse, {ref.name:ref.idx for ref in rcrs.values()})
 ...
 ir.Ref)]) -> ir.Node:
        ...
        add_to_count(ir_node_with_rcr, norm_idx)
        for tcs in rcrs:
            add_to_count(tcs)
        ...
        do_acr = lambda op: mutator.replace_expressions(op,
          acrs, references=norm_refs)
        ...
        return arithmetic.simplify(ir.from_reduction(reduction[0], tuple(map(do_acr, reduction[1]))))
