...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/codegen/xilinx/rtl_kernel.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 24812 bytes
import collections, concurrent.futures, io, json, logging, os, shutil, sys, tarfile, tempfile
from typing import BinaryIO, Dict, Iterable, List, Optional, Sequence, Set, TextIO, Tuple
...
def print_code(stencil: core.Stencil, xo_file: BinaryIO, platform: Optional[str]=None, jobs: Optional[int]=os.cpu_count(), rpt_file: Optional[str]=None, interface: str='m_axi') -> None:
  ...
  """
    ...
    with tempfile.TemporaryDirectory(prefix='sodac-xrtl-') as (tmpdir):
        ...
        for module_trait_id, module_trait in enumerate(stencil.module_traits):
            sio = io.StringIO()
            hls_kernel.print_module_definition((util.CppPrinter(sio)), module_trait,
              module_trait_id,
        ...
        if interface == 'm_axi':
            sio = io.StringIO()
            print_dataflow_hls_interface(util.CppPrinter(sio), top_name, inputs, outputs)
        ...
        if interface == 'm_axi':
            hls_resources = hls_report.resources(os.path.join(tmpdir, 'report', top_name + '_csynth.xml'))
            hls_resources -= hls_report.resources(os.path.join(tmpdir, 'report', 'Dataflow_csynth.xml'))
        ...
        for module_id, nodes in enumerate(super_source.module_trait_table.values()):
            ...
            try:
                ...
                _logger.info('%s, usage: %5d times, II: %3d, Depth: %3d', hls_resource, use_count, perf.ii, perf.depth)
        ...
        _logger.info(hls_resources)
        if rpt_file:
            rpt_json = collections.OrderedDict([('name', top_name)] + list(hls_resources))
        ...
        with open((os.path.join(hdl_dir, f"{module_name}.v")), mode='w') as (fileobj):
            print_top_module(backend.VerilogPrinter(fileobj), stencil.dataflow_super_source, inputs, outputs, module_name, interface)
        util.pause_for_debugging()
...
def synthesis_module(tmpdir, kernel_files, module_name, device_info):
  ...
  """
    ...
    with tempfile.TemporaryFile(mode='w+b') as (tarfileobj):
        with backend.RunHls(tarfileobj, kernel_files,
          module_name,
        ...
        if proc.returncode == 0:
            ...
            with tarfile.open(mode='r', fileobj=tarfileobj) as (tar):
                tar.extractall(tmpdir, (f for f in tar.getmembers() if f.name.startswith('hdl') or f.name.startswith('report')))
    util.release_job_slot(job_server)
    return (proc.returncode, stdout, stderr)


FIFO_PORT_SUFFIXES = dict(data_in='_din', not_full='_full_n',
  write_enable='_write',
  data_out='_dout',
  not_empty='_empty_n',
  read_enable='_read',
  not_block='_blk_n')
AXIS_PORT_SUFFIXES = collections.OrderedDict(data='_TDATA',
  keep='_TKEEP',
  strb='_TSTRB',
  last='_TLAST',
  valid='_TVALID',
...
def print_kernel_xml(name: str, inputs: Iterable[Tuple[(str, str, int, str)]], outputs: Iterable[Tuple[(str, str, int, str)]], kernel_xml: TextIO, interface: str='m_axi') -> None:
    ...
    if interface == 'm_axi':
        for ports in (outputs, inputs):
            for port_name, bundle_name, width, _ in ports:
                args.append(backend.Arg(cat=(backend.Cat.MMAP),
                  name=port_name,
                  port=bundle_name,
                  ctype=f"ap_uint<{width}>*",
                  width=width))

        args.append(backend.Arg(cat=(backend.Cat.SCALAR),
        ...
        if interface == 'axis':
            for cat, ports in ((backend.Cat.ISTREAM, inputs), (backend.Cat.OSTREAM, outputs)):
...
def print_top_module(printer: backend.VerilogPrinter, super_source: dataflow.SuperSourceNode, inputs: Sequence[Tuple[(str, str, int, str)]], outputs: Sequence[Tuple[(str, str, int, str)]], module_name: str='Dataflow', interface: str='m_axi') -> None:
    """Generate kernel.xml file.
  ...
  """
    ...
    printer.println()
    (printer.printlns)(*(f"input  wire {arg};" for arg in input_args), *(f"output wire {arg};" for arg in output_args))
    for port_name, _, width, _ in outputs:
        if interface == 'm_axi':
            printer.printlns(f"output wire [{width - 1}:0] {port_name}_V_V{data_in};", f"input  wire {port_name}_V_V{not_full};", f"output wire {port_name}_V_V{write_enable};")
        elif interface == 'axis':
            printer.printlns(f"output wire [{width - 1}:0] {port_name}{data};", f"output wire [{width // 8 - 1}:0] {port_name}{keep};", f"output wire [{width // 8 - 1}:0] {port_name}{strb};", f"output wire {port_name}{last};", f"output wire {port_name}{valid};", f"input  wire {port_name}{ready};", f"wire [{width - 1}:0] {port_name}_V_V{data_in};", f"wire {port_name}_V_V{not_full};", f"wire {port_name}_V_V{write_enable};")

    for port_name, _, width, _ in inputs:
        if interface == 'm_axi':
            printer.printlns(f"input  wire [{width - 1}:0] {port_name}_V_V{data_out};", f"input  wire {port_name}_V_V{not_empty};", f"output wire {port_name}_V_V{read_enable};")
        elif interface == 'axis':
            printer.printlns(f"input  wire [{width - 1}:0] {port_name}{data};", f"input  wire [{width // 8 - 1}:0] {port_name}{keep};", f"input  wire [{width // 8 - 1}:0] {port_name}{strb};", f"input  wire {port_name}{last};", f"input  wire {port_name}{valid};", f"output wire {port_name}{ready};", f"wire [{width - 1}:0] {port_name}_V_V{data_out};", f"wire {port_name}_V_V{not_empty};", f"wire {port_name}_V_V{read_enable};")

    printer.println()
    printer.printlns("reg ap_done = 1'b0;", "reg ap_idle = 1'b1;", "reg ap_ready = 1'b0;")
    if interface == 'axis':
        printer.println("wire ap_start = 1'b1;")
    if interface == 'm_axi':
        for port_name, _, width, _ in outputs:
            printer.printlns(f"reg [{width - 1}:0] {port_name}{data_in};", f"wire {port_name}_V_V{write_enable};")
    ...
    if interface == 'axis':
        rst = '~ap_rst_n'
    (printer.printlns)(f"wire ap_rst_reg_0 = {rst};", *(f'(* shreg_extract = "no", max_fanout = {8 ** i} *) reg ap_rst_reg_{i};' for i in range(1, ap_rst_reg_level)), *(
    ...
    if ap_rst_reg_level > 0:
        with printer.always('posedge ap_clk'):
            printer.printlns(f"ap_rst_reg_{i + 1} <= ap_rst_reg_{i};" for i in range(ap_rst_reg_level))
    with printer.always('posedge ap_clk'):
        with printer.if_('ap_rst_reg'):
            printer.printlns("ap_done <= 1'b0;", "ap_idle <= 1'b1;", "ap_ready <= 1'b0;")
            printer.else_()
            printer.println('ap_idle <= ~ap_start;')
    printer.println()
    if interface == 'm_axi':
        printer.printlns(f"reg {port_name}_V_V{not_block};" for port_name, _, _, _ in ports)
        with printer.always('*'):
            (printer.printlns)(*(f"{port_name}_V_V{not_block} = {port_name}_V_V{not_full};" for port_name, _, _, _ in outputs), *(f"{port_name}_V_V{not_block} = {port_name}_V_V{not_empty};" for port_name, _, _, _ in inputs))
    printer.println()
    fifos = set()
    if interface == 'axis':
        for port_name, _, width, _ in outputs:
            width = width // 8
            ones = '1' * width
            printer.printlns(f"assign {port_name}{keep} = {width}'b{ones};", f"assign {port_name}{strb} = {width}'b{ones};", f"assign {port_name}{last} = 1'b0;")

        for port_name, _, width, _ in inputs:
            printer.module_instance('fifo_w{width}_d{depth}_A'.format(width=width, depth=2),
              (port_name + '_fifo'),
              args={'clk': 'ap_clk', 
             'reset': 'ap_rst_reg', 
             'if_read_ce': "1'b1", 
        ...
        for port_name, _, width, _ in outputs:
            printer.module_instance(f"fifo_w{width}_d2_A",
              (port_name + '_fifo'),
              args={'clk': 'ap_clk', 
             'reset': 'ap_rst_reg', 
             'if_read_ce': "1'b1", 
    ...
    for module in super_source.tpo_valid_node_gen():
        for fifo in module.fifos:
            ...
            printer.printlns(f"wire [{msb}:0] {name}{data_in};", f"wire {name}{not_full};", f"wire {name}{write_enable};", f"wire [{msb}:0] {name}{data_out};", f"wire {name}{not_empty};", f"wire {name}{read_enable};", '')
            printer.module_instance(f"fifo_w{fifo.width_in_bits}_d{fifo.depth + 2}_A",
              name,
              args={'clk': 'ap_clk', 
             'reset': 'ap_rst_reg', 
             'if_read_ce': "1'b1", 
    ...
    for module in super_source.tpo_valid_node_gen():
        ...
        for port, fifo in zip(module_trait.output_fifos, module.output_fifos):
            arg_dict.update({f"{port}_V{data_in}": f"{fifo}{data_in}", 
        ...
        for port, fifo in zip(module_trait.input_fifos, module.input_fifos):
            arg_dict.update({f"{port}_V{data_out}": f"{{1'b1, {fifo}{data_out}}}", 
        ...
        for dram_ref, bank in module.dram_reads:
            port = dram_ref.dram_fifo_name(bank)
            fifo = util.get_port_name(dram_ref.var, bank)
            arg_dict.update({f"{port}_V{data_out}": f"{{1'b1, {fifo}_V_V{data_out}}}", 
             f"{port}_V{not_empty}": f"{fifo}_V_V{not_empty}", 
             f"{port}_V{read_enable}": f"{fifo}_V_V{read_enable}"})

        printer.module_instance(util.get_func_name(module_trait_id), module.name, arg_dict)
        printer.println()
...
def print_dataflow_hls_interface(printer: backend.util.CppPrinter, top_name: str, inputs: Sequence[Tuple[(str, str, int, str)]], outputs: Sequence[Tuple[(str, str, int, str)]]) -> None:
    ports = (*outputs, *inputs)
    printer.printlns('#include <cstddef>', '#include <cstdint>', '#include <ap_int.h>', '#include <hls_stream.h>')
    ...
    printer.un_scope()
    params = [f"hls::stream<ap_uint<{width}>>& {name}" for name, _, width, _ in ports]
    printer.print_func('void Dataflow', params, align=0)
    printer.do_scope()
    (printer.printlns)(*(f"volatile ap_uint<{width}> {name}_read = {name}.read();" for name, _, width, _ in inputs), *(f"{name}.write(ap_uint<{width}>());" for name, _, width, _ in outputs))
    printer.un_scope()
    ...
    for port_name, bundle_name, _, _ in ports:
        printer.println(f"#pragma HLS interface m_axi port={port_name} offset=slave bundle={bundle_name}", 0)

    for port_name, _, _, _ in ports:
        printer.println(f"#pragma HLS interface s_axilite port={port_name} bundle=control", 0)

    printer.println('#pragma HLS interface s_axilite port=coalesced_data_num bundle=control', 0)
    printer.println('#pragma HLS interface s_axilite port=return bundle=control', 0)
    printer.println()
    ...
    for port_name, _, _, buf_name in inputs:
        printer.print_func('BurstRead',
          [
         buf_name, port_name, 'coalesced_data_num'],
          suffix=';',
          align=0)

    printer.print_func('Dataflow',
      [name for _, _, _, name in ports], suffix=';',
      align=0)
    for port_name, _, _, buf_name in outputs:
        printer.print_func('BurstWrite',
          [
