...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/soda/model/resource.py
# Compiled at: 2020-05-04 01:27:40
# Size of source mod 2**32: 9463 bytes
...
class XilinxHLSReport:
    ...
    def __init__(self, rpt_file):
        ...
        for line in rpt_file:
            if line.startswith('* Project:'):
                ...
                if line == '== Utilization Estimates\n':
                    ...
                    if hasattr(self, 'resources_used'):
                        if None in self.resources_used:
                            if '|Total' in line:
                                for resource, used in zip(self.resources_used, line.split('|')[2:-1]):
                                    self.resources_used[resource] = int(used)

                                del self.resources_used[None]
                    if hasattr(self, 'resources_available'):
                        if None in self.resources_available:
                            if '|Available' in line:
                                for resource, avail in zip(self.resources_available, line.split('|')[2:-1]):
                                    self.resources_available[resource] = int(avail)

                                del self.resources_available[None]
                    if hasattr(self, 'resources_instances'):
                        if None in self.resources_instances:
                            if '|Instance' in line:
                                for resource, avail in zip(self.resources_instances, line.split('|')[2:-1]):
                        ...
                        if hasattr(self, 'instances'):
                            if None in self.instances:
                                if '|compute_' in line:
                                    ...
                                    compute = self.instances.setdefault('compute', collections.OrderedDict())
                                    compute = compute.setdefault(func_name, collections.OrderedDict())
                                    compute[pe_id] = get_resource_dict(self.resources_used, line.split('|')[3:-1])
            if hasattr(self, 'instances') and None in self.instances and '|forward_' in line:
                ...
                forward = self.instances.setdefault('forward', collections.OrderedDict())
                forward = forward.setdefault(int(line.split('|')[2].split('_')[1]), collections.OrderedDict())
                forward = forward.setdefault(data_type, collections.OrderedDict())
                forward = forward.setdefault(fifo_depth, [])
                forward.append(get_resource_dict(self.resources_used, line.split('|')[3:-1]))
            elif hasattr(self, 'instances') and None in self.instances and '|' + self.app + '_kernel' in line:
                if line.split('|')[2].strip().endswith('_m_axi'):
                    m_axi = self.instances.setdefault('m_axi', collections.OrderedDict())
                    m_axi[line.split('|')[2].split('_')[2]] = get_resource_dict(self.resources_used, line.split('|')[3:-1])
                else:
                    if line.split('|')[2].strip().endswith('_control_s_axi'):
                        self.instances['control_s_axi'] = get_resource_dict(self.resources_used, line.split('|')[3:-1])
                    elif self.app + '_kernel_entry' in line.split('|')[2]:
                        self.instances['entry'] = get_resource_dict(self.resources_used, line.split('|')[3:-1])
            elif hasattr(self, 'instances') and None in self.instances and '|Block_proc' in line:
                self.instances['Block_proc'] = get_resource_dict(self.resources_used, line.split('|')[3:-1])
            elif hasattr(self, 'instances') and None in self.instances and '|load' in line:
                self.instances.setdefault('load', []).append(get_resource_dict(self.resources_used, line.split('|')[3:-1]))
            elif hasattr(self, 'instances') and None in self.instances and '|store' in line:
                self.instances.setdefault('store', []).append(get_resource_dict(self.resources_used, line.split('|')[3:-1]))
            elif hasattr(self, 'instances') and None in self.instances and '|pack_' in line:
                ...
                pack.append(get_resource_dict(self.resources_used, line.split('|')[3:-1]))
            elif hasattr(self, 'instances') and None in self.instances and '|unpack_' in line:
                ...
                unpack.append(get_resource_dict(self.resources_used, line.split('|')[3:-1]))
            else:
                if hasattr(self, 'instances'):
                    if None in self.instances:
                        if '|Total' in line:
        ...
        dram_bank_i = sum(1 for bus in self.instances['m_axi'] if bus.endswith('i'))
        dram_bank_o = sum(1 for bus in self.instances['m_axi'] if bus.endswith('o'))
    ...
    def check(self):
        ...
        for module in ('Block_proc', 'entry', 'control_s_axi'):
            if module in self.instances:
                accum_res(resources_used, self.instances[module])

        for module in ('m_axi', ):
            accum_res_iter(resources_used, self.instances[module].values())

        for module in ('load', 'store'):
            accum_res_iter(resources_used, self.instances[module])
            ...
            for degree in self.instances[module].values():
                for data_type in degree.values():
                    for fifo_depth in data_type.values():
                        accum_res_iter(resources_used, fifo_depth)
    ...
    def __init__(self, rpt_file):
        for line in rpt_file:
            ...
            if len(line_splited) > 1 and 'Name' in line_splited[1]:
                self.resources_used = collections.OrderedDict((resource.strip(), None) for resource in line_splited[2:-1])
            else:
                if hasattr(self, 'resources_used') and len(line_splited) == len(self.resources_used) + 3 and 'Used Resources' in line:
    ...
    for file_name in sys.argv[1:]:
        with open(file_name) as (f):
            if file_name.endswith('kernel_util_routed.rpt'):
                ...
                if file_name.endswith('_csynth.rpt'):
