...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/silme/io/clients.py
...
class IOClient(object):
    bomdict = {'utf_8_sig': codecs.BOM_UTF8, 'utf_16_be': codecs.BOM_UTF16_BE, 
    ...
    @classmethod
    def get_package(cls, path, code='default', object_type='l10nobject', source=None, ignore=[
 'CVS', '.svn', '.DS_Store', '.hg']):
        raise NotImplementedError()
...
class FileFormatClient(IOClient):
    ...
    def get_entitylist(cls, path, uri=None, source=False, code='default', parser=None):
        ...
        src = cls.get_source(path, encoding=parser.encoding, fallback=parser.fallback)
 ...
 'CVS', '.svn', '.DS_Store', '.hg']):
    ...
    def get_source(cls, path, encoding=None, fallback=None):
        ...
        if encoding is not None:
            ...
            if output[1] != encoding and (fallback is None or output[1] not in fallback):
                raise Exception('The encoding for path ' + path + ' is not ' + encoding + ' like expected, but ' + output[1] + '. ' + output[1] + ' is not in the fallback list' + ', break.')
            return output
        else:
            ...
            if char_detector:
                try:
                    coding = chardet.detect(cls.read_without_encoding(path))['encoding'].lower()
                except Exception, e:
                    pass
                else:
                    try:
            ...
            try:
                ...
                return (
                 cls.get_source_without_encoding(path), None)
