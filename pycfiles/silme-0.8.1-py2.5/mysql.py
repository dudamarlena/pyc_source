...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/silme/io/mysql.py
 ...
 'CVS', '.svn', '.DS_Store', '.hg']):
    ...
    def _connect(cls, path):
        if not cls._connected():
            (host, port, db, user, password, table) = cls._explode_path(path)
            cls.connection = MySQLdb.connect(host=host, port=port, user=user, passwd=password, db=db, use_unicode=True)
