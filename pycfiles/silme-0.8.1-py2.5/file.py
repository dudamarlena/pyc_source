...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-i386/egg/silme/io/file.py
...
class FileClient(FileFormatClient):
    ...
    def matches_path(cls, path):
        ...
        3) "file://foo"
        """
        return path.startswith('/') or path.startswith('./') or path.startswith('../') or path.startswith('file://')
 ...
 'CVS', '.svn', '.DS_Store', '.hg']):
    ...
    def write_blob(cls, blob, path):
        ...
        cls.write_source(blob.source, path, encoding=None)
        return
    ...
    def write_entitylist(cls, elist, path, encoding=None):
        ...
        cls.write_source(string, path, encoding)
        return True
    ...
    def write_structure(cls, object, path, encoding=None):
        ...
        cls.write_source(string, path, format_parser.encoding)
        return True
