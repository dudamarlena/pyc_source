...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/emaillib/emaillib.py
# Compiled at: 2017-09-18 09:38:01
"""Main module file"""
import logging, os, smtplib, re
...
class SmtpServer(object):
    ...
    def __init__(self, smtp_address, username=None, password=None, tls=True, ssl=False, port=None):
        self._smtp = None
    ...
    def send(self, sender, recipients, cc=None, bcc=None, subject='', body='', attachments=None, content='text'):
        ...
        try:
            ...
            self._smtp.sendmail(message.sender, message.recipients, message.as_string)
            result = True
            self._connected = False
...
class EasySender(object):
    ...
    def __init__(self, smtp_address, username=None, password=None, tls=False, ssl=True, port=None):
        self._server = SmtpServer(smtp_address, username, password, tls, ssl, port)
    ...
    def send(self, sender, recipients, cc=None, bcc=None, subject='', body='', attachments=None, content='text'):
        ...
        self._server.send(sender, recipients, cc, bcc, subject, body, attachments, content)
...
class Message(object):
    ...
    def __init__(self, sender, recipients, cc=None, bcc=None, subject='', body='', attachments=None, content='text'):
        self._sender = None
    ...
    def content(self, value):
        if value.lower() not in ('text', 'html'):
            raise ValueError(('Invalid content type :{content}.Allowed ["text", "html"]').format(content=value))
    ...
    def attachments(self, entries):
        ...
        for attachment in attachments:
            part = MIMEApplication(open(os.path.expanduser(attachment), 'rb').read())
