...
# [GCC 8.4.0]
# Embedded file name: build/lib.macosx-10.13-x86_64-2.7/Cheetah/Compiler.py
...
import re, time, random, warnings, copy, codecs
from Cheetah.Version import Version, VersionTuple
from Cheetah.SettingsManager import SettingsManager
from Cheetah.Utils.Indenter import indentize
from Cheetah import NameMapper
from Cheetah.Parser import Parser, ParseError, specialVarRE, STATIC_CACHE, REFRESH_CACHE, SET_GLOBAL, SET_MODULE, unicodeDirectiveRE, encodingDirectiveRE, escapedNewlineRE
...
_DEFAULT_COMPILER_SETTINGS = [
 ...
 (
  'useSearchList', True,
  "Enable the searchList, requires useNameMapper=True, if disabled, first portion of the $variable is a global, builtin, or local variable that doesn't need looking up in the searchList"),
 (
  'allowSearchListAsMethArg', True, ''),
 (
  'useAutocalling', True,
  'Detect and call callable objects in searchList, requires useNameMapper=True'),
 (
  'useStackFrames', True,
  'Used for NameMapper.valueFromFrameOrSearchList rather than NameMapper.valueFromSearchList'),
 (
  'useErrorCatcher', False,
  'Turn on the #errorCatcher directive for catching NameMapper errors, etc'),
 (
 ...
 (
  ...
  'All #import statements are relocated to the top of the generated Python module'),
 (
  'prioritizeSearchListOverSelf', False,
  'When iterating the searchList, look into the searchList passed into the initializer instead of Template members first'),
 (
 ...
 (
  ...
  'callable(parser, expr, exprType, rawExpr=None, startPos=None), exprType is the name of the directive, "psp" or "placeholder" The filters *must* return the expr or raise an expression, they can modify the expr if needed'),
 (
    ...
    def genNameMapperVar(self, nameChunks):
        ...
        if not useSearchList:
            ...
            if firstDotIdx != -1 and firstDotIdx < len(name):
                ...
                pythonCode = 'VFN(' + beforeFirstDot + ',"' + afterDot + '",' + repr(defaultUseAC and useAC) + ')' + remainder
            else:
                pythonCode = name + remainder
        else:
            if self.setting('useStackFrames'):
                pythonCode = 'VFFSL(SL,"' + name + '",' + repr(defaultUseAC and useAC) + ')' + remainder
            else:
                pythonCode = 'VFSL([locals()]+SL+[globals(), builtin],"' + name + '",' + repr(defaultUseAC and useAC) + ')' + remainder
            while nameChunks:
                name, useAC, remainder = nameChunks.pop()
                pythonCode = 'VFN(' + pythonCode + ',"' + name + '",' + repr(defaultUseAC and useAC) + ')' + remainder
...
class MethodCompiler(GenUtils):
    ...
    def __init__(self, methodName, classCompiler, initialMethodComment=None, decorators=None):
    ...
    def addPlaceholder(self, expr, filterArgs, rawPlaceholder, cacheTokenParts, lineCol, silentMode=False):
        ...
        if self.isErrorCatcherOn():
            methodName = self._classCompiler.addErrorCatcherCall(expr, rawCode=rawPlaceholder, lineCol=lineCol)
    ...
    def addSet(self, expr, exprComponents, setStyle):
        if setStyle is SET_GLOBAL:
            LVALUE, OP, RVALUE = exprComponents.LVALUE, exprComponents.OP, exprComponents.RVALUE
    ...
    def addInclude(self, sourceExpr, includeFrom, isRaw):
        self.addChunk('self._handleCheetahInclude(' + sourceExpr + ', trans=trans, ' + 'includeFrom="' + includeFrom + '", raw=' + repr(isRaw) + ')')
    ...
    def addRepeat(self, expr, lineCol=None):
        ...
        self.addFor('for __i%s in range(%s)' % (
         self._repeatCount, expr), lineCol=lineCol)
    ...
    def addElse(self, expr, dedent=True, lineCol=None):
        expr = re.sub('else[ \\f\\t]+if', 'elif', expr)
    ...
    def addYield(self, expr):
        ...
        if expr.replace('yield', '').strip():
            ...
            self.addChunk('else:')
            self.indent()
            self.addChunk('raise TypeError("This method cannot be called with a trans arg")')
            self.dedent()
    ...
    def nextCacheID(self):
        return '_' + str(random.randrange(100, 999)) + str(random.randrange(10000, 99999))
    ...
    def startCacheRegion(self, cacheInfo, lineCol, rawPlaceholder=None):
        ...
        self._cacheRegionsStack.append(ID)
        self.addChunk('')
        self.addChunk('## START CACHE REGION: ID=' + ID + '. line %s, col %s' % lineCol + ' in the source.')
        self.addChunk('_RECACHE_%(ID)s = False' % locals())
        self.addChunk('_cacheRegion_%(ID)s = self.getCacheRegion(regionID=' % locals() + repr(ID) + ', cacheInfo=%r' % cacheInfo + ')')
        self.addChunk('if _cacheRegion_%(ID)s.isNew():' % locals())
        self.indent()
        self.addChunk('_RECACHE_%(ID)s = True' % locals())
        self.dedent()
        self.addChunk('_cacheItem_%(ID)s = _cacheRegion_%(ID)s.getCacheItem(' % locals() + varyBy + ')')
        ...
        if interval:
            self.addChunk('_cacheItem_%(ID)s.setExpiryTime(currentTime() +' % locals() + str(interval) + ')')
        return
    ...
    def startCallRegion(self, functionName, args, lineCol, regionTitle='CALL'):
        ...
        self.addChunk('## START %(regionTitle)s REGION: ' % locals() + ID + ' of ' + functionName + ' at line %s, col %s' % lineCol + ' in the source.')
        self.addChunk('_orig_trans%(ID)s = trans' % locals())
    ...
    def _endCallArg(self):
        ...
        self.addChunk('_callKws%(ID)s[%(currCallArg)r] = _callCollector%(ID)s.response().getvalue()' % locals())
    ...
    def endCallRegion(self, regionTitle='CALL'):
        ID, callDetails = self._callRegionsStack[(-1)]
        functionName, initialKwArgs, lineCol = callDetails.functionName, callDetails.args, callDetails.lineCol
        ...
        self.addChunk('## END %(regionTitle)s REGION: ' % locals() + ID + ' of ' + functionName + ' at line %s, col %s' % lineCol + ' in the source.')
        self.addChunk('')
        self._callRegionsStack.pop()
    ...
    def startCaptureRegion(self, assignTo, lineCol):
        ...
        self.addChunk('## START CAPTURE REGION: ' + ID + ' ' + assignTo + ' at line %s, col %s' % lineCol + ' in the source.')
        self.addChunk('_orig_trans%(ID)s = trans' % locals())
        self.addChunk('_wasBuffering%(ID)s = self._CHEETAH__isBuffering' % locals())
        self.addChunk('self._CHEETAH__isBuffering = True')
    ...
    def setErrorCatcher(self, errorCatcherName):
        ...
        self.indent()
        self.addChunk('self._CHEETAH__errorCatcher = self._CHEETAH__errorCatchers["' + errorCatcherName + '"]')
        self.dedent()
        self.addChunk('else:')
        self.indent()
        self.addChunk('self._CHEETAH__errorCatcher = self._CHEETAH__errorCatchers["' + errorCatcherName + '"] = ErrorCatchers.' + errorCatcherName + '(self)')
        self.dedent()
    ...
    def setFilter(self, theFilter, isKlass):
        ...
        if isKlass:
            self.addChunk('_filter = self._CHEETAH__currentFilter = ' + theFilter.strip() + '(self).filter')
        elif theFilter.lower() == 'none':
...
class AutoMethodCompiler(MethodCompiler):
    ...
    def _useKWsDictArgForPassingTrans(self):
        alreadyHasTransArg = [ argname for argname, defval in self._argStringList if argname == 'trans'
                             ]
        return self.methodName() != 'respond' and not alreadyHasTransArg and self.setting('useKWsDictArgForPassingTrans')
    ...
    def _addAutoSetupCode(self):
        ...
        if self._streamingEnabled and not self.isClassMethod() and not self.isStaticMethod():
            ...
            self.addChunk('if (not trans and not self._CHEETAH__isBuffering and not callable(self.transaction)):')
            self.indent()
            self.addChunk('trans = self.transaction # is None unless self.awake() was called')
            self.dedent()
...
_initMethod_initCheetah = ("if not self._CHEETAH__instanceInitialized:\n    cheetahKWArgs = {}\n    allowedKWs = 'searchList namespaces filter filtersLib errorCatcher'.split()\n    for k,v in KWs.items():\n        if k in allowedKWs: cheetahKWArgs[k] = v\n    self._initCheetahInstance(**cheetahKWArgs)\n").replace('\n', '\n' + ' ' * 8)

class ClassCompiler(GenUtils):
    methodCompilerClass = AutoMethodCompiler
    methodCompilerClassForInit = MethodCompiler

    def __init__(self, className, mainMethodName='respond', moduleCompiler=None, fileName=None, settingsManager=None):
        ...
        self._setupState()
        methodCompiler = self._spawnMethodCompiler(mainMethodName, initialMethodComment='## CHEETAH: main method generated for this template')
    ...
    def _setupState(self):
        ...
        self._generatedAttribs.append('_CHEETAH_versionTuple = __CHEETAH_versionTuple__')
    ...
    def cleanupState(self):
        ...
        self.addAttribute('_mainCheetahMethod_for_' + self._className + ' = ' + repr(self._mainMethodName))

    def _setupInitMethod(self):
        __init__ = self._spawnMethodCompiler('__init__', klass=self.methodCompilerClassForInit)
    ...
    def _addSourceFileMonitoring(self, fileName):
        ...
        self.addChunk('if exists(self._filePath) and ' + 'getmtime(self._filePath) > self._fileMtime:')
        self.indent()
        self.addChunk('self._compile(file=self._filePath, moduleName=' + self._className + ')')
        self.addChunk('write(getattr(self, self._mainCheetahMethod_for_' + self._className + ')(trans=trans))')
        self.addStop()
        self.dedent()
    ...
    def _spawnMethodCompiler(self, methodName, klass=None, initialMethodComment=None):
        if klass is None:
            klass = self.methodCompilerClass
        decorators = self._decoratorsForNextMethod or []
        self._decoratorsForNextMethod = []
        methodCompiler = klass(methodName, classCompiler=self, decorators=decorators, initialMethodComment=initialMethodComment)
    ...
    def startMethodDef(self, methodName, argsList, parserComment):
        methodCompiler = self._spawnMethodCompiler(methodName, initialMethodComment=parserComment)
        self._setActiveMethodCompiler(methodCompiler)
        for argName, defVal in argsList:
    ...
    def addAttribute(self, attribExpr):
        if attribExpr.find('VFN(') != -1 or attribExpr.find('VFFSL(') != -1:
            raise ParseError(self, 'Invalid #attr directive. It should only contain ' + 'simple Python literals.')
    ...
    def addSuper(self, argsList, parserComment=None):
        ...
        argString = (',').join(argStringChunks)
        self.addFilteredChunk('super(%(className)s, self).%(methodName)s(%(argString)s)' % locals())
        return
    ...
    def addErrorCatcherCall(self, codeChunk, rawCode='', lineCol=''):
        if rawCode in self._placeholderToErrorCatcherMap:
            ...
            if not self.setting('outputRowColComments'):
                self._methodsIndex[methodName].addMethDocString('plus at line %s, col %s' % lineCol)
            return methodName
        ...
        catcherMeth = self._spawnMethodCompiler(methodName, klass=MethodCompiler, initialMethodComment='## CHEETAH: Generated from ' + rawCode + ' at line %s, col %s' % lineCol + '.')
        catcherMeth.setMethodSignature('def ' + methodName + '(self, localsDict={})')
        ...
        catcherMeth.indent()
        catcherMeth.addChunk('return self._CHEETAH__errorCatcher.warn(exc_val=e, code= ' + repr(codeChunk) + ' , rawCode= ' + repr(rawCode) + ' , lineCol=' + str(lineCol) + ')')
    ...
    def attributes(self):
        try:
            attribs = [ self.setting('indentationStep') + str(attrib) for attrib in self._generatedAttribs
                      ]
        except UnicodeEncodeError:
            attribs = [ self.setting('indentationStep') + unicode(attrib) for attrib in self._generatedAttribs
                      ]
...
class ModuleCompiler(SettingsManager, GenUtils):
    ...
    def __init__(self, source=None, file=None, moduleName='DynamicallyCompiledCheetahTemplate', mainClassName=None, mainMethodName=None, baseclassName=None, extraImportStatements=None, settings=None):
        ...
        elif isinstance(file, string_type):
        ...
        if source == '':
            ...
            if unicodeMatch:
                if encodingMatch:
                    raise ParseError(self, '#encoding and #unicode are mutually exclusive! Use one or the other.')
    ...
    def _spawnClassCompiler(self, className, klass=None):
        ...
        classCompiler = klass(className, moduleCompiler=self, mainMethodName=self.setting('mainMethodName'), fileName=self._filePath, settingsManager=self)
    ...
    def setBaseClass(self, baseClassName):
        ...
        elif not self.setting('autoImportForExtendsDirective') or baseClassName == 'object' or baseClassName in self.importedVarNames():
            self._getActiveClassCompiler().setBaseClass(baseClassName)
        else:
            baseclasses = []
