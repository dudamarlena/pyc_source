...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/recordconvert/__init__.py
# Compiled at: 2009-10-03 15:18:49
...
class ListOfRecords(list):
    ...
    def __getattr__(self, name):
        if len(self) and isinstance(self[0], AttributeDict) and hasattr(self[0], name):
...
class Record(AttributeDict):
    ...
    def __setitem__(self, name, value):
        ...
        if first_char_ord == 95 or first_char_ord >= 48 and first_char_ord <= 57:
            raise RecordConvertError('Key names cannot start with an underscore (_) character or a digit')
        for char in name:
            if not (ord(char) >= 97 and ord(char) <= 122 or ord(char) >= 65 and ord(char) <= 90 or ord(char) >= 48 and ord(char) <= 57 or ord(char) == 95):
                raise RecordConvertError('Record keys cannot contain the character %r' % char)
...
def toListOfRecords(converter, *k, **p):
    if isinstance(converter, Record):
        raise RecordConvertError('toListOfRecords() takes a converter such as toRecord() as its argument, not a Record instance')
...
def toRecord(converters, missing_defaults=None, empty_defaults=None, missing_errors=None, empty_errors=None, missing_or_empty_errors=None, missing_or_empty_defaults=None, filter_extra_fields=True, allow_extra_fields=True, raise_on_extra_fields=True, use_many_message_after_failures=3, msg_many_invalid_fields=MSG_DICTIONARY['msg_many_invalid_fields'], msg_some_invalid_fields=MSG_DICTIONARY['msg_some_invalid_fields'], msg_single_invalid_field=MSG_DICTIONARY['msg_single_invalid_field'], msg_field_not_allowed=MSG_DICTIONARY['msg_field_not_allowed'], msg_fields_not_allowed=MSG_DICTIONARY['msg_fields_not_allowed']):
