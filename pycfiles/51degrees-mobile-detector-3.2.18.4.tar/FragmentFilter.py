...
# Embedded file name: \Ft\Xml\ThirdParty\Xvif\FragmentFilter.py
...
class ApplyTo(_Callback):
    ...
    def match(self, uri, name, ruleClass):
        return ruleClass == Element and self.elements.has_key((uri, name)) or ruleClass == Attribute and self.attributes.has_key((uri, name))
...
class Attribute(_Callback):
    ...
    def endElementNS(self, rules, (nsURI, localName), qName):
        if self.prefix == None and self.nsURI != None or self.nsURI == None and self.prefix != None:
...
class RulesLoader(ContentHandler, _Callback):
    __module__ = __name__
    callbacks = {'http://simonstl.com/ns/fragments/': {'fragmentRules': FragmentRules, 'fragmentRule': FragmentRule, 'applyTo': ApplyTo, 'produce': Produce, 'element': Element, 'attribute': Attribute, 'skip': Skip, 'chars': Chars}}
