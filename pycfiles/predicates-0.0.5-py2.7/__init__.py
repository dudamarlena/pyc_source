...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/predicates/__init__.py
# Compiled at: 2013-08-29 09:42:53
from operator import lt, le, eq, ne, ge, gt, not_, truth, is_, is_not
from collections import Callable, Container, Hashable, Iterable, Iterator, Mapping, MutableMapping, MappingView, ItemsView, KeysView, ValuesView, Sequence, MutableSequence, Set, MutableSet, Sized
...
def _zip(*predicates):
    ...
    **TODO:** What's the correct mathematical term for this? The
    ...
    def _zip(*args, **kwargs):
        return all(predicate(arg) for predicate, arg in zip(predicates, args))
    ...
    * Mixed :meth:`~object.__getitem__` and :meth:`~object.__call__`
    ...
    def __getitem__(self, key):
        ...
        **NOTE:** keys must (currently) be integers or :func:`slice`
        objects.
        """
        key = key if isslice(key) else slice(key, key + 1) if key >= 0 else slice(key, None)
        ...
        def _args_factory(pos_predicate=None, **kw_predicates):
            ...
            if not (pos_predicate or kw_predicates):
                raise ValueError('must specify a predicate for positional args, ' + 'a set of predicates for keyword args, or both.')
            if not pos_predicate:

                def _args(*args, **kwargs):
                    return all(predicate(kwargs.get(kw, None)) for kw, predicate in kw_predicates.items())
            ...
            def _args(*args, **kwargs):
                return pos_predicate(*args[key]) and all(predicate(kwargs.get(kw, None)) for kw, predicate in kw_predicates.items())
...
def _inkw(atleast=False, atmost=False, exactly=False):
    ...
    **TODO:** add a new predicate, or extend this one, to validate
    ...
    keyword args.)
    """
    if exactly is not False:
        if not (atleast is False and atmost is False):
            raise ValueError("cannot mix 'exactly' and 'atleast' or 'atmost'")

        def _inkw(*args, **kwargs):
            return len(kwargs) == len(exactly) and all(kw in kwargs for kw in exactly)

        return _inkw
    if atleast is False and atmost is False:
        raise ValueError("must specify 'exactly' or one or both of 'atleast' and 'atmost'")
    if atleast is False:
        atleast = ()
...
def _isa(classinfo, docstring=None):
    ...
    if docstring is None:
        ...
        if name:
            name = ':class:`%s`' % name
        elif isiterable(classinfo):
            name = '(' + (', ').join(getattr(t, '__name__', str(t)) for t in classinfo) + ')'
        else:
            name = str(classinfo)
...
def _is(it, docstring=None):
    ...
    _is.__doc__ = docstring or '`True` if `obj` is %s' % getattr(it, '__name__', it)
    return _is
