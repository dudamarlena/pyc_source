...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/tests/sqla/test_inlineform.py
...
def test_inline_form():
    ...
    rv = client.post('/admin/user/new/', data={'name': 'fbar', 'info-0-key': 'foo', 
       'info-0-val': 'bar'})
    eq_(rv.status_code, 302)
    ...
    eq_(rv.status_code, 200)
    rv = client.post('/admin/user/edit/?id=2', data={'name': 'barfoo', 'info-0-id': 1, 
       'info-0-key': 'xxx', 'info-0-val': 'yyy'})
    eq_(UserInfo.query.count(), 1)
    eq_(UserInfo.query.one().key, 'xxx')
    rv = client.post('/admin/user/edit/?id=2', data={'name': 'barf', 'del-info-0': 'on', 
       'info-0-id': '1', 'info-0-key': 'yyy', 'info-0-val': 'xxx', 'info-1-id': None, 
       'info-1-key': 'bar', 'info-1-val': 'foo'})
    eq_(rv.status_code, 302)
    eq_(User.query.count(), 2)
    eq_(User.query.get(2).name, 'barf')
    eq_(UserInfo.query.count(), 1)
    eq_(UserInfo.query.one().key, 'bar')
...
def test_inline_form_base_class():
    ...
    class UserModelView(ModelView):
        inline_models = (
         (
          UserEmail, {'form_base_class': StubBaseForm}),)
        form_args = {'emails': {'validators': [ItemsRequired()]}}

    view = UserModelView(User, db.session)
    admin.add_view(view)
    data = {'name': 'emptyEmail', 
       'emails-0-email': ''}
    rv = client.post('/admin/user/new/', data=data)
    eq_(rv.status_code, 200)
    eq_(User.query.count(), 0)
    ok_('success!' in rv.data, rv.data)