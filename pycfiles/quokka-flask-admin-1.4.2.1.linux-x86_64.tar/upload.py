...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/form/upload.py
...
class FileUploadInput(object):
    ...
    def __call__(self, field, **kwargs):
        ...
        if field.data and isinstance(field.data, FileStorage):
            ...
            value = field.data or ''
        return HTMLString(template % {'text': html_params(type='text', readonly='readonly', value=value, name=field.name), 
...
class ImageUploadInput(object):
    ...
    """
    empty_template = '<input %(file)s>'
    data_template = '<div class="image-thumbnail"> <img %(image)s> <input type="checkbox" name="%(marker)s">Delete</input></div><input %(file)s>'
...
class FileUploadField(fields.StringField):
    ...
    def __init__(self, label=None, validators=None, base_path=None, relative_path=None, namegen=None, allowed_extensions=None, permission=438, allow_overwrite=True, **kwargs):
        """
            Constructor.
    ...
    def _save_file(self, data, filename):
        ...
        if not op.exists(op.dirname(path)):
            os.makedirs(os.path.dirname(path), self.permission | 73)
...
class ImageUploadField(FileUploadField):
    ...
    def __init__(self, label=None, validators=None, base_path=None, relative_path=None, namegen=None, allowed_extensions=None, max_size=None, thumbgen=None, thumbnail_size=None, permission=438, url_relative_path=None, endpoint='static', **kwargs):
        """
            Constructor.
        ...
        super(ImageUploadField, self).__init__(label, validators, base_path=base_path, relative_path=relative_path, namegen=namegen, allowed_extensions=allowed_extensions, permission=permission, **kwargs)
        return
    ...
    def _save_thumbnail(self, data, filename, format):
        if self.image and self.thumbnail_size:
            path = self._get_path(self.thumbnail_fn(filename))
            self._save_image(self._resize(self.image, self.thumbnail_size), path, format)
