...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/contrib/sqla/view.py
...
class ModelView(BaseModelView):
    ...
    """
    column_auto_select_related = ObsoleteAttr('column_auto_select_related', 'auto_select_related', True)
    column_select_related_list = ObsoleteAttr('column_select_related', 'list_select_related', None)
    column_display_all_relations = ObsoleteAttr('column_display_all_relations', 'list_display_all_relations', False)
        ...
        if self.column_sortable_list is None:
            ...
            for c in self.column_sortable_list:
                ...
                if path and hasattr(path[0], 'property'):
                    ...
                    raise Exception(("For sorting columns in a related table, column_sortable_list requires a string like '<relation name>.<column name>'. Failed on: {0}").format(c))
                elif getattr(column, 'key', None) is not None:
    ...
    def handle_filter(self, filter):
        if isinstance(filter, sqla_filters.BaseSQLAFilter):
            ...
            if isinstance(column, InstrumentedAttribute) and tools.need_join(self.model, column.table):
                self._filter_joins[column] = [
                 column.table]
        return filter
    ...
    def scaffold_form(self):
        ...
        form_class = form.get_form(self.model, converter, base_class=self.form_base_class, only=self.form_columns, exclude=self.form_excluded_columns, field_args=self.form_args, extra_fields=self.form_extra_fields)
    ...
    def scaffold_list_form(self, widget=None, validators=None):
        ...
        """
        converter = self.model_form_converter(self.session, self)
        form_class = form.get_form(self.model, converter, base_class=self.form_base_class, only=self.column_editable_list, field_args=validators)
    ...
    def scaffold_inline_form_models(self, form_class):
        """
            ...
            :param form_class:
                Form class
        """
        inline_converter = self.inline_model_form_converter(self.session, self, self.model_form_converter)
    ...
    def _apply_search(self, query, count_query, joins, count_joins, search):
        ...
        return (
         query, count_query, joins, count_joins)
    ...
    def get_list(self, page, sort_column, sort_desc, search, filters, execute=True, page_size=None):
        ...
        count_query = (self.simple_list_pager or self.get_count_query)() if 1 else None
        ...
        if filters and self._filters:
            query, count_query, joins, count_joins = self._apply_filters(query, count_query, joins, count_joins, filters)
