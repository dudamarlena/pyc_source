...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/tests/test_form_upload.py
...
def test_upload_field():
    ...
    _remove_testfiles()
    dummy = Dummy()
    with app.test_request_context(method='POST', data={'upload': (BytesIO('Hello World 1'), 'test1.txt')}):
    ...
    with app.test_request_context(method='POST', data={'upload': (BytesIO('Hello World 2'), 'test2.txt')}):
        my_form = TestForm(helpers.get_form_data())
        ok_(my_form.validate())
        my_form.populate_obj(dummy)
    ...
    with app.test_request_context(method='POST', data={'upload': (BytesIO('Hullo'), 'test1.txt')}):
...
def test_image_upload_field():
    ...
    class TestForm(form.BaseForm):
        upload = form.ImageUploadField('Upload', base_path=path, thumbnail_size=(
         100, 100, True))
    ...
    class TestAutoResizeForm(form.BaseForm):
        upload = form.ImageUploadField('Upload', base_path=path, max_size=(
         64, 64, True))
