...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/tests/sqla/test_form_rules.py
...
def test_form_rules():
    ...
    db.create_all()
    view = CustomModelView(Model1, db.session, form_rules=(
     'test2', 'test1', rules.Field('test4')))
    admin.add_view(view)
...
def test_rule_macro():
    ...
    Model1, _ = create_models(db)
    db.create_all()
    view = CustomModelView(Model1, db.session, create_template='macro.html', form_create_rules=(
...
def test_rule_header():
    ...
    Model1, _ = create_models(db)
    db.create_all()
    view = CustomModelView(Model1, db.session, form_create_rules=(
     rules.Header('hello'),))
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
...
def test_rule_field_set():
    ...
    db.create_all()
    view = CustomModelView(Model1, db.session, form_create_rules=(
     rules.FieldSet(['test2', 'test1', 'test4'], 'header'),))
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
    eq_(rv.status_code, 200)
...
def test_rule_inlinefieldlist():
    ...
    db.create_all()
    view = CustomModelView(Model1, db.session, inline_models=(
     Model2,), form_create_rules=('test1', 'model2'))
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
    eq_(rv.status_code, 200)
...
def test_inline_model_rules():
    ...
    view = CustomModelView(Model1, db.session, inline_models=[
     (
      Model2, dict(form_rules=('string_field', 'bool_field')))])
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
    eq_(rv.status_code, 200)
