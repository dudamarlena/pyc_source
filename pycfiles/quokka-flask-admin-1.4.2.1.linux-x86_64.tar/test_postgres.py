...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/tests/sqla/test_postgres.py
...
def test_hstore():
    ...
    rv = client.post('/admin/model/new/', data={'hstore_test-0-key': 'test_val1', 
...
def test_json():
    ...
    eq_(rv.status_code, 200)
    rv = client.post('/admin/jsonmodel/new/', data={'json_test': '{"test_key1": "test_value1"}'})
    eq_(rv.status_code, 302)
    rv = client.get('/admin/jsonmodel/')
    eq_(rv.status_code, 200)
    data = rv.data.decode('utf-8')
