...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/tests/sqla/test_basic.py
...
class CustomModelView(ModelView):
    ...
    def __init__(self, model, session, name=None, category=None, endpoint=None, url=None, **kwargs):
        ...
        super(CustomModelView, self).__init__(model, session, name, category, endpoint, url)
...
def create_models(db):
    ...
    class Model1(db.Model):
        ...
        def __init__(self, test1=None, test2=None, test3=None, test4=None, bool_field=False, date_field=None, time_field=None, datetime_field=None, enum_field=None):
            self.test1 = test1
            self.test2 = test2
            self.test3 = test3
            self.test4 = test4
    ...
    class Model2(db.Model):
        ...
        def __init__(self, string_field=None, int_field=None, bool_field=None, model1=None, float_field=None, string_field_default=None, string_field_empty_default=None):
...
def test_model():
    ...
    eq_(rv.status_code, 200)
    rv = client.post('/admin/model1/new/', data=dict(test1='test1large', test2='test2', time_field=time(0, 0, 0)))
    ...
    rv = client.get(url)
    eq_(rv.status_code, 200)
    ok_('00:00:00' in rv.data.decode('utf-8'))
...
def test_list_columns():
    ...
    view = CustomModelView(Model1, db.session, column_list=[
     'test1', 'test3'], column_labels=dict(test1='Column1'))
    admin.add_view(view)
    ...
    view2 = CustomModelView(Model1, db.session, endpoint='model1_2', column_list=[
     Model1.test1, Model1.test3], column_labels=dict(test1='Column1'))
...
def test_complex_list_columns():
    ...
    db.session.commit()
    view = CustomModelView(M2, db.session, column_list=[
     'model1.test1'])
    admin.add_view(view)
    client = app.test_client()
...
def test_exclude_columns():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model1, db.session, column_exclude_list=[
     'test2', 'test4', 'enum_field', 'date_field', 'time_field', 'datetime_field'])
    admin.add_view(view)
    eq_(view._list_columns, [
...
def test_column_searchable_list():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model2, db.session, column_searchable_list=[
...
def test_complex_searchable_list():
    ...
    view2 = CustomModelView(Model1, db.session, column_searchable_list=[
     Model2.string_field])
    admin.add_view(view2)
...
def test_complex_searchable_list_missing_children():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model1, db.session, column_searchable_list=[
...
def test_column_editable_list():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model1, db.session, column_editable_list=[
     'test1', 'enum_field'])
    admin.add_view(view)
    ...
    rv = client.post('/admin/model1/ajax/update/', data={'list_form_pk': '1', 
    ...
    rv = client.post('/admin/model1/ajax/update/', data={'list_form_pk': '1000', 
    ...
    admin.add_view(view)
    rv = client.post('/admin/model2/ajax/update/', data={'list_form_pk': '1', 
       'model1': '3'})
    data = rv.data.decode('utf-8')
    ok_('Record was successfully saved.' == data)
    rv = client.get('/admin/model2/')
...
def test_details_view():
    ...
    string_field_view = CustomModelView(Model2, db.session, can_view_details=True, column_details_list=[
     'string_field'], endpoint='sf_view')
...
def test_editable_list_special_pks():
    ...
    db.session.commit()
    client = app.test_client()
    rv = client.post('/admin/model1/ajax/update/', data={'list_form_pk': '1-1', 
...
def test_column_filters():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model1, db.session, column_filters=[
     'test1'])
    admin.add_view(view)
    client = app.test_client()
    eq_(len(view._filters), 7)
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Test1'] ], [
     (0, 'contains'),
     (1, 'not contains'),
     (2, 'equals'),
     (3, 'not equal'),
     ...
     (5, 'in list'),
     (6, 'not in list')])
    view = CustomModelView(Model2, db.session, column_filters=[
     'model1'])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Test1'] ], [
     (0, 'contains'),
     (1, 'not contains'),
     (2, 'equals'),
     (3, 'not equal'),
     ...
     (5, 'in list'),
     (6, 'not in list')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Test2'] ], [
     (7, 'contains'),
     (8, 'not contains'),
     (9, 'equals'),
     (10, 'not equal'),
     ...
     (12, 'in list'),
     (13, 'not in list')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Test3'] ], [
     (14, 'contains'),
     (15, 'not contains'),
     (16, 'equals'),
     (17, 'not equal'),
     ...
     (19, 'in list'),
     (20, 'not in list')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Test4'] ], [
     (21, 'contains'),
     (22, 'not contains'),
     (23, 'equals'),
     (24, 'not equal'),
     ...
     (26, 'in list'),
     (27, 'not in list')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Bool Field'] ], [
     (28, 'equals'),
     (29, 'not equal')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Enum Field'] ], [
     (30, 'equals'),
     (31, 'not equal'),
     (32, 'empty'),
     (33, 'in list'),
    ...
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Model1 / Bool Field'] ], [
     ...
     (1, 'not equal')])
    view = CustomModelView(Model2, db.session, column_filters=[
     'model1.bool_field', 'string_field'], column_labels={'model1.bool_field': 'Test Filter #1', 
       'string_field': 'Test Filter #2'})
    eq_(list(view._filter_groups.keys()), ['Test Filter #1', 'Test Filter #2'])
    fill_db(db, Model1, Model2)
    ...
    view = CustomModelView(Model2, db.session, column_filters=[
     'int_field'])
    admin.add_view(view)
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Int Field'] ], [
     (0, 'equals'),
     (1, 'not equal'),
     (2, 'greater than'),
     (3, 'smaller than'),
    ...
    ok_('test2_val_3' not in data)
    ok_('test2_val_4' not in data)
    view = CustomModelView(Model1, db.session, column_filters=['bool_field'], endpoint='_bools')
    admin.add_view(view)
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Bool Field'] ], [
     (0, 'equals'),
     (1, 'not equal')])
    ...
    ok_('test2_val_2' not in data)
    ok_('test2_val_3' not in data)
    view = CustomModelView(Model2, db.session, column_filters=['float_field'], endpoint='_float')
    admin.add_view(view)
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Float Field'] ], [
     (0, 'equals'),
     (1, 'not equal'),
     (2, 'greater than'),
     (3, 'smaller than'),
    ...
    ok_('test2_val_3' not in data)
    ok_('test2_val_4' not in data)
    view = CustomModelView(Model2, db.session, endpoint='_model2', column_filters=[
     'model1.bool_field'], column_list=[
     'string_field',
     'model1.id',
     'model1.bool_field'])
    admin.add_view(view)
    ...
    ok_('test2_val_4' not in data)
    view = CustomModelView(Model1, db.session, column_filters=[
     'test1'], endpoint='_model3', named_filter_urls=True)
    admin.add_view(view)
    ...
    ok_('test1_val_2' not in data)
    view = CustomModelView(Model1, db.session, column_filters=[
     'date_field', 'datetime_field', 'time_field'], endpoint='_datetime')
    admin.add_view(view)
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Date Field'] ], [
     (0, 'equals'),
     (1, 'not equal'),
     (2, 'greater than'),
     (3, 'smaller than'),
     ...
     (5, 'not between'),
     (6, 'empty')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Datetime Field'] ], [
     (7, 'equals'),
     (8, 'not equal'),
     (9, 'greater than'),
     (10, 'smaller than'),
     ...
     (12, 'not between'),
     (13, 'empty')])
    eq_([ (f['index'], f['operation']) for f in view._filter_groups['Time Field'] ], [
     (14, 'equals'),
     (15, 'not equal'),
     (16, 'greater than'),
     (17, 'smaller than'),
    ...
    ok_('timeonly_obj2' in data)
    view = CustomModelView(Model1, db.session, column_filters=[
     'enum_field'], endpoint='_enumfield')
    admin.add_view(view)
    ...
    ok_('enum_obj2' not in data)
    view = CustomModelView(Model2, db.session, column_filters=[
     filters.FilterEqual(Model1.test1, 'Test1')], endpoint='_relation_test')
    admin.add_view(view)
...
def test_hybrid_property():
    ...
    db.session.commit()
    client = app.test_client()
    view = CustomModelView(Model1, db.session, column_default_sort='number_of_pixels', column_filters=[
...
def test_url_args():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model1, db.session, page_size=2, column_searchable_list=[
     'test1'], column_filters=[
     'test1'])
    admin.add_view(view)
...
def test_form_columns():
    ...
    db.create_all()
    view1 = CustomModelView(Model, db.session, endpoint='view1', form_columns=('int_field',
                                                                               'text_field'))
    view2 = CustomModelView(Model, db.session, endpoint='view2', form_excluded_columns=('excluded_column', ))
    ...
    view4 = CustomModelView(Model, db.session, endpoint='view1', form_columns=[
     Model.int_field])
...
def test_default_sort():
    ...
    eq_(data[1].test1, 'b')
    eq_(data[2].test1, 'c')
    view2 = CustomModelView(M1, db.session, column_default_sort='test1', column_labels={'test1': 'blah'}, endpoint='m1_2')
    admin.add_view(view2)
    ...
    eq_(data[1].test1, 'b')
    eq_(data[2].test1, 'c')
    view3 = CustomModelView(M1, db.session, column_default_sort='test1', column_labels={'test1': 'blah'}, endpoint='m1_3', column_list=[
     'test1'])
...
def test_complex_sort():
    ...
    db.session.commit()
    view = CustomModelView(M2, db.session, column_list=[
     'string_field', 'model1.test1'], column_sortable_list=[
     'model1.test1'])
    admin.add_view(view)
    client = app.test_client()
...
def test_complex_sort_exception():
    app, db, admin = setup()
    M1, M2 = create_models(db)
    view = CustomModelView(M2, db.session, endpoint='model2_3', column_sortable_list=[
     M1.test1])
    admin.add_view(view)
...
def test_default_complex_sort():
    ...
    eq_(data[0].model1.test1, 'a')
    eq_(data[1].model1.test1, 'b')
    view2 = CustomModelView(M2, db.session, endpoint='model2_2', column_default_sort=(
     M1.test1, False))
    admin.add_view(view2)
    _, data = view2.get_list(0, None, None, None, None)
    eq_(len(data), 2)
...
def test_extra_fields():
    app, db, admin = setup()
    Model1, _ = create_models(db)
    view = CustomModelView(Model1, db.session, form_extra_fields={'extra_field': fields.StringField('Extra Field')})
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
    eq_(rv.status_code, 200)
...
def test_extra_field_order():
    app, db, admin = setup()
    Model1, _ = create_models(db)
    view = CustomModelView(Model1, db.session, form_columns=('extra_field', 'test1'), form_extra_fields={'extra_field': fields.StringField('Extra Field')})
    admin.add_view(view)
    client = app.test_client()
    rv = client.get('/admin/model1/new/')
    eq_(rv.status_code, 200)
...
def test_modelview_localization():
    ...
    def test_locale(locale):
        try:
            ...
            Model1, _ = create_models(db)
            view = CustomModelView(Model1, db.session, column_filters=[
             'test1', 'bool_field', 'date_field', 'datetime_field', 'time_field'])
            admin.add_view(view)
            client = app.test_client()
            rv = client.get('/admin/model1/')
            eq_(rv.status_code, 200)
            ...
            print (
             'Error on the following locale:', locale)
            raise
...
def test_ajax_fk():
    app, db, admin = setup()
    Model1, Model2 = create_models(db)
    view = CustomModelView(Model2, db.session, url='view', form_ajax_refs={'model1': {'fields': ('test1', 'test2')}})
    admin.add_view(view)
    ok_('model1' in view._form_ajax_refs)
    model = Model1('first')
    ...
    eq_(len(items), 1)
    eq_(items[0].test1, 'foo')
    form = view.create_form()
    eq_(form.model1.__class__.__name__, 'AjaxSelectField')
    with app.test_request_context('/admin/view/'):
        ok_('value=""' not in form.model1())
        form.model1.data = model
        ok_('data-json="[%s, &quot;first&quot;]"' % model.id in form.model1())
        ok_('value="1"' in form.model1())
    client = app.test_client()
    req = client.get('/admin/view/ajax/lookup/?name=model1&query=foo')
    eq_(req.data.decode('utf-8'), '[[%s, "foo"]]' % model2.id)
...
def test_ajax_fk_multi():
    ...
    db.create_all()
    view = CustomModelView(Model2, db.session, url='view', form_ajax_refs={'model1': {'fields': [
                           'name']}})
    admin.add_view(view)
    ok_('model1' in view._form_ajax_refs)
    ...
    db.session.commit()
    form = view.create_form()
    eq_(form.model1.__class__.__name__, 'AjaxSelectMultipleField')
    with app.test_request_context('/admin/view/'):
        ok_('data-json="[]"' in form.model1())
        form.model1.data = [
         model]
        ok_('data-json="[[1, &quot;first&quot;]]"' in form.model1())
    client = app.test_client()
...
def test_safe_redirect():
    ...
    admin.add_view(view)
    client = app.test_client()
    rv = client.post('/admin/model1/new/?url=http://localhost/admin/model2view/', data=dict(test1='test1large', test2='test2', _continue_editing='Save and Continue Editing'))
    eq_(rv.status_code, 302)
    ...
    rv = client.post('/admin/model1/new/?url=http://google.com/evil/', data=dict(test1='test1large', test2='test2', _continue_editing='Save and Continue Editing'))
    eq_(rv.status_code, 302)
    assert_true(rv.location.startswith('http://localhost/admin/model1/edit/'))
...
def test_unlimited_page_size():
    ...
    eq_(len(data), 21)
    _, data = view.get_list(0, None, None, None, None, execute=True, page_size=False)
    eq_(len(data), 21)
    return
...
def test_model_default():
    ...
    assert_true('This field is required' not in rv.data)
...
def test_export_csv():
    ...
    view = CustomModelView(Model1, db.session, can_export=True, column_list=[
     'test1', 'test2'], export_max_rows=2, endpoint='row_limit_2')
    admin.add_view(view)
    client = app.test_client()
    ...
    view = CustomModelView(Model1, db.session, can_export=True, column_list=[
     'test1', 'test2'], endpoint='no_row_limit')
    admin.add_view(view)
