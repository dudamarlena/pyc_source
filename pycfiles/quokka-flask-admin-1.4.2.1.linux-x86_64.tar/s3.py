...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.7/site-packages/flask_admin/contrib/fileadmin/s3.py
...
class S3Storage(object):
    ...
    def __init__(self, bucket_name, region, aws_access_key_id, aws_secret_access_key):
        ...
        """
        if not s3:
            raise ValueError('Could not import boto. You can install boto by using pip install boto')
        connection = s3.connect_to_region(region, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
...
class S3FileAdmin(BaseFileAdmin):
    ...
    def __init__(self, bucket_name, region, aws_access_key_id, aws_secret_access_key, *args, **kwargs):
        ...
        super(S3FileAdmin, self).__init__(storage=storage, *args, **kwargs)