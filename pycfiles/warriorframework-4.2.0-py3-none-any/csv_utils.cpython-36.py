...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/csv_utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 10471 bytes
"""
...
try:
    ...
    print_info('Install xlrd module in order to perform Excel to CSV or Excel to XML file conversion')
    print_info('Please follow the instructions in  http://xlrd.readthedocs.io/en/latest/installation.html to install the xlrd')
...
def convert_excel_to_csv(input_excel_file, output_csv_file_path=None, return_csv_file=False):
    """
        ...
        for rownum in range(sh.nrows):
            ...
            for index, value in enumerate(row_val):
                if sh.cell(rownum, index).ctype == 3:
                    year, month, day, hour, minute, sec = xlrd.xldate_as_tuple(value, wb.datemode)
...
def convert_csv_or_excel_to_xml(input_file, mapping_file=None, output_xml_file_path=None, overwrite='yes'):
    """
    ...
    try:
        if '.xls' in input_file:
            ...
            json_response = json.dumps(dict_response, sort_keys=False, indent=4, separators=(',',
                                                                                             ': '),
              encoding='utf-8')
            if mapping_file:
                mapping_dict = data_Utils.get_credentials(mapping_file, 'mapping_scheme')
            ...
            for i in range(len(dict_response)):
                ...
                for key, value in dict_response[i].items():
                    if mapping_dictionary:
                        if key in mapping_dictionary:
                            if mapping_dictionary[key]:
                                result.append('    <{0}>{1}</{0}>\n'.format(mapping_dictionary[key], value))
                        else:
                            result.append('    <{0}>{1}</{0}>\n'.format(key, value))
                    else:
                        result.append('    <{0}>{1}</{0}>\n'.format('_'.join(key.split()), value))
            ...
            output_dict['{0}_json_response'.format(input_file.replace('.csv', ''))] = json_response
            output_dict['{0}_xml_response'.format(input_file.replace('.csv', ''))] = xml_res
...
def convert_xml_to_csv(input_file, mapping_file=None, output_csv_file_path=None, overwrite='yes'):
    """
