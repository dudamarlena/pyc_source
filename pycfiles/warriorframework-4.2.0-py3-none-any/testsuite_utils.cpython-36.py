...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/testsuite_utils.py
...
def report_testsuite_result(suite_repository, suite_status):
    ...
    """
    print_info('\n ****** TestSuite Result ******')
    suite_status = {'TRUE':'PASS',  'FALSE':'FAIL',  'EXCEPTION':'ERROR',  'ERROR':'ERROR',  'RAN':'RAN'}.get(str(suite_status).upper())
...
def get_data_file_at_suite_step(element, suite_repository):
    """Gets the testcase/testsuite path  from the testsuite.xml/project.xml file """
    if suite_repository['suite_exectype'].upper() == 'ITERATIVE_SEQUENTIAL' or suite_repository['suite_exectype'].upper() == 'ITERATIVE_PARALLEL':
...
def get_jiraids_from_xmlfile(element):
    ...
    if jiraids is not None:
        if jiraids is not False:
