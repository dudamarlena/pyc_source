...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/cli_Utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 22933 bytes
"""
...
try:
    ...
    print_info('All default methods/functions that use cli will failwithout pexpect module. Users can however createtheir own custom libraries for cli interaction \n')
...
def pexpect_spawn_with_env(pexpect_obj, command, timeout, escape=False, env=None):
    ...
    child = wc_obj.pexpect_spawn_with_env(pexpect_obj, command, timeout, escape, env)
    return child


@mocked
def connect_ssh(ip, port='22', username='', password='', logfile=None, timeout=60, prompt='.*(%|#|\\$)', conn_options='', custom_keystroke='', escape='', **kwargs):
    """
    ...
    """
    print_warning("This method is obsolete and will be deprecated soon. Please use 'connect_ssh' method of 'PexpectConnect' class in 'warrior/Framework/ClassUtils/WNetwork/warrior_cli_class.py'")
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    session_object, conn_string = wc_obj.connect_ssh(ip=ip,
      port=port,
      username=username,
      password=password,
      logfile=logfile,
      ...
      escape=escape)
    return (
     session_object, conn_string)


@mocked
def connect_telnet(ip, port='23', username='', password='', logfile=None, timeout=60, prompt='.*(%|#|\\$)', conn_options='', custom_keystroke='', escape='', **kwargs):
    """
    ...
    :Returns:
        ...
        2. conn_string(pre and post login message)
    """
    print_warning("This method is obsolete and will be deprecated soon. Please use 'connect_telnet' method of 'PexpectConnect' class in 'warrior/Framework/ClassUtils/WNetwork/warrior_cli_class.py'")
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    session_object, conn_string = wc_obj.connect_telnet(ip=ip,
      port=port,
      username=username,
      password=password,
      logfile=logfile,
...
def disconnect_telnet(child):
    ...
    print_warning("This method is obsolete and will be deprecated soon. Please use 'disconnect_telnet' method of 'PexpectConnect' class in 'warrior/Framework/ClassUtils/WNetwork/warrior_cli_class.py'")
    if isinstance(child, WNetwork.warrior_cli_class.WarriorCli):
        child = child.disconnect_telnet()
    else:
        if isinstance(child, pexpect.spawn):
            wc_obj = WNetwork.warrior_cli_class.WarriorCli()
...
def disconnect(child):
    ...
    """
    print_warning("This method is obsolete and will be deprecated soon. Please use 'disconnect' method of 'PexpectConnect' class in 'warrior/Framework/ClassUtils/WNetwork/warrior_cli_class.py'")
    if isinstance(child, WNetwork.warrior_cli_class.WarriorCli):
...
def send_command_and_get_response(sessionobj, prompt1, prompt2, command):
    ...
    if isinstance(sessionobj, WNetwork.warrior_cli_class.WarriorCli):
        ...
        if isinstance(sessionobj, pexpect.spawn):
            wc_obj = WNetwork.warrior_cli_class.WarriorCli()
            wc_obj.conn_obj = WNetwork.warrior_cli_class.PexpectConnect()
            ...
            response = ''
            print_warning('Unable to send the command since the sessionobj is not supported by warrior_cli_class, status will be marked as ERROR.Please use warrior_cli_class for session establishment.')
    return response
...
def smart_action(datafile, call_system_name, raw_prompt, session_object, tag_value, connect_testdata=None):
    ...
    """
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    smart_testdatafile = wc_obj.smart_action(datafile, call_system_name, raw_prompt, session_object, tag_value, connect_testdata)
...
@mocked
def send_command(session_object, start_prompt, end_prompt, command, timeout=60):
    """
    ...
    if isinstance(session_object, WNetwork.warrior_cli_class.WarriorCli):
        status, response = session_object.send_command(start_prompt, end_prompt, command, timeout)
    else:
        if isinstance(session_object, pexpect.spawn):
            ...
            print_warning('Unable to send the command since the session_object is not an instance of warrior_cli_class, status will be marked as ERROR. Please use warrior_cli_class for session establishment.')
    return (
     status, response)
...
def _send_cmd(obj_session, **kwargs):
    ...
    if isinstance(obj_session, WNetwork.warrior_cli_class.WarriorCli):
        ...
        if isinstance(obj_session, pexpect.spawn):
            ...
            if isinstance(obj_session, ssh_utils_class.SSHComm):
...
def _get_response_dict(details_dict, index, response, response_dict):
    ...
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    status, resp_key_list = wc_obj._get_response_dict(details_dict, index, response, resp_key_list=[])
...
def start_threads(started_thread_for_system, thread_instance_list, same_system, unique_log_verify_list, system_name):
    ...
    """
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    started_thread_for_system, thread_instance_list, same_system = wc_obj.start_threads(started_thread_for_system, thread_instance_list, same_system, unique_log_verify_list, system_name)
    return (
...
def get_response_dict(started_thread_for_system, thread_instance_list, same_system, response):
    ...
    """
    wc_obj = WNetwork.warrior_cli_class.WarriorCli()
    remote_resp_dict = wc_obj.get_response_dict(started_thread_for_system, thread_instance_list, same_system, response)
...
def _send_cmd_get_status(obj_session, details_dict, index, system_name=None):
    ...
    if isinstance(obj_session, WNetwork.warrior_cli_class.WarriorCli):
        result, response = obj_session._send_cmd_get_status(details_dict, index, system_name)
    else:
        wc_obj = WNetwork.warrior_cli_class.WarriorCli()
        wc_obj.conn_obj = WNetwork.warrior_cli_class.PexpectConnect()
        wc_obj.conn_obj.target_host = obj_session
        result, response = wc_obj._send_cmd_get_status(details_dict, index, system_name)
...
def _get_obj_session(details_dict, obj_session, kw_system_name, index):
    ...
    if isinstance(obj_session, WNetwork.warrior_cli_class.WarriorCli):
        value, kw_system_name, details_dict = obj_session._get_obj_session(details_dict, kw_system_name, index)
    else:
        wc_obj = WNetwork.warrior_cli_class.WarriorCli()
        wc_obj.conn_obj = WNetwork.warrior_cli_class.PexpectConnect()
        wc_obj.conn_obj.target_host = obj_session
        value, kw_system_name, details_dict = wc_obj._get_obj_session(details_dict, kw_system_name, index)
