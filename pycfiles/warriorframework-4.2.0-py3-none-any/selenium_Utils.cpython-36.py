...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/selenium_Utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 20173 bytes
"""
        ...
        for key in path_list:
            if not key.endswith(']'):
                try:
                    ...
                    if not default:
                        print_error("Key '{0}' not found in json of the {1}. {2} seems to be an invalid path".format(key, file, path))
                    data_dict = None
                ...
                try:
                    ...
                    if not default:
                        print_error("Key '{0}' not found in json of the {1}. {2} seems to be an invalid path".format(key_list[0], file, path))
                    data_dict = None
                ...
                try:
                    ...
                    if not default:
                        print_error("Element '{0}' of key {1} not found in json of the {2}. {3} seems to be an invalid path".format(key_list[1], key_list[0], file, path))
                    data_dict = None
...
def split_kwargs_on_tag_equalto(kwargs, datafile, browser):
    ...
    for element in kwargs:
        if isinstance(kwargs[element], str) and kwargs[element] is not None and kwargs[element] is not False:
            if kwargs[element].startswith('tag') and '=' in kwargs[element] and datafile is not None:
    ...
    for element in final_dict:
        if final_dict[element] is not None and final_dict[element] is not False and '=' in final_dict[element]:
            ...
            if temp_list_1[0] in final_dict:
                ...
                mapper[element] = [
                 def_name_tuple[0], def_name_tuple[1]]
        else:
            mapper[element] = [
             def_name_tuple[0], def_name_tuple[1]]
...
def get_final_json_values(element, final_dict, mapper, def_name_tuple):
    ...
    reference = {'xpath':'xpath',  'id':'id',  'css_selector':'css',  'link_text':'link', 
     'partial_link_text':'partial_link',  'tag_name':'tag', 
     'class_name':'class',  'name_of_element':'name'}
    flag = False
    ...
    if element.lower() in reference:
        if mapper[element][0] == def_name_tuple[0]:
            if mapper[element][1] == def_name_tuple[1]:
                value = get_element_from_config_file((final_dict[mapper[element][0]]), (final_dict[mapper[element][1]]),
                  (reference[element.lower()]),
                  default=True)
        else:
            if mapper[element][1] is not None:
                value = get_element_from_config_file(final_dict[mapper[element][0]], final_dict[mapper[element][1]], reference[element.lower()])
            else:
                value = get_element_from_config_file(final_dict[mapper[element][0]], None, reference[element.lower()])
        final_dict[element] = value
    else:
        if element.lower() in locator_types:
            if mapper[element][0] == def_name_tuple[0]:
                if mapper[element][1] == def_name_tuple[1]:
                    value = get_element_from_config_file((final_dict[mapper[element][0]]), (final_dict[mapper[element][1]]),
                      (final_dict[element]),
                ...
                if mapper[element][1] is not None:
                    ...
                    value = get_element_from_config_file((final_dict[mapper[element][0]]), None,
                      (final_dict[element]), is_locator_type='yes')
            ...
            if flag:
                ...
                if mapper[element][0] == def_name_tuple[0]:
                    ...
                    if mapper[element][1] is not None:
                        value = get_element_from_config_file(final_dict[mapper[element][0]], final_dict[mapper[element][1]], element)
                    else:
                        value = get_element_from_config_file(final_dict[mapper[element][0]], None, element)
                final_dict[element] = value
    return final_dict


def get_browser_details(browser, datafile=None, br_name='browser_name', def_name_tuple=('DEF_ecf', 'DEF_et'), bw_comp=('element_config_file', 'element_tag'), **kwargs):
    ...
    for element in final_dict:
        if final_dict[element] is None and element in kwargs:
            final_dict[element] = kwargs[element]

    if kwargs[br_name] != 'all':
        if final_dict[br_name] != kwargs[br_name]:
            return
        ...
        for element in locator:
            for key in element:
                if key.strip().lower() == 'default':
                    if element[key].strip().lower() == 'yes':
...
def get_element_from_config_file(config_file, element_tag, child_tag, is_locator_type='no', default=False):
    """
    Gets default locators from json file
    """
    locator_types = ('xpath', 'id', 'css', 'link', 'partial_link', 'tag', 'class',
                     'name')
    ...
    if wt_datafile:
        ...
        if config_file is not False:
            ...
            elif is_locator_type == 'yes':
                final_value = [
                 child_tag, final_value]
            else:
...
def create_display():
    ...
    if data_Utils.get_object_from_datarepository('headless_display'):
        ...
        try:
            ...
            print_error('pyvirtualdisplay is not installed in order to launch the browser in headless mode')
            status = False
        except Exception as err:
            print_error('Encountered Exception: {0}, while trying to launch the browser in headless mode'.format(err))
            status = False
