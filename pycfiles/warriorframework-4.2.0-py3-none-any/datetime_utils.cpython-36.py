...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/datetime_utils.py
...
def wait_for_timeout(wait_time, unit='SECONDS', notify_count=4):
    ...
    try:
        ...
        if unit.upper() in ('SECOND', 'SECONDS', 'SEC', 'SECS'):
            ...
            if unit.upper() in ('MINUTE', 'MINUTES', 'MIN', 'MINS'):
                seconds = 60 * wait_time
            else:
                if unit.upper() in ('HOUR', 'HOURS'):
                    seconds = 3600 * wait_time
                else:
                    if unit.upper() in ('DAY', 'DAYS'):
                        seconds = 86400 * wait_time
                    else:
                        if unit.upper() in ('MONTH', 'MONTHS'):
                            seconds = 2592000 * wait_time
                        else:
                            if unit.upper() in ('YEAR', 'YEARS'):
                                seconds = 31536000 * wait_time
                            else:
                                print_warning('The supported unit of seconds is Seconds/Minutes/Hours/Months/YearsThe default unit of Seconds would be used')
        notify_count = int(notify_count)
        notify_sec = seconds / notify_count
        print_without_logging('Wait time of {0}s will be notified every {1}s'.format(seconds, notify_sec))
        for count in range(notify_count):
            print_without_logging('Remaining wait time: {:.1f}s'.format(seconds - count * notify_sec))
