...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/parallel_testsuite_driver.py
...
from warrior.WarriorCore.multiprocessing_utils import create_and_start_process_with_queue, get_results_from_queue, update_pj_junit_resultfile
from warrior.WarriorCore import testsuite_utils

def execute_parallel_testsuites(testsuite_list, project_repository, data_repository, auto_defects=False, ts_parallel=True):
    ...
    for i in range(len(ts_junit_list)):
        ts_junit_list[i].update_attr('impact', impact_dict.get(ts_impact_list[i].upper()), 'ts', ts_timestamp_list[i])
        ts_junit_list[i].update_attr('onerror', 'N/A', 'ts', ts_timestamp_list[i])

    data_repository['wt_junit_object'] = update_pj_junit_resultfile(data_repository['wt_junit_object'], ts_junit_list)
    project_status = Utils.testcase_Utils.compute_status_using_impact(ts_status_list, ts_impact_list)
    return project_status


def main(testsuite_list, project_repository, data_repository, auto_defects=False, ts_parallel=True):
    """Executes the list of testcases in parallel
    Computes and returns the testsuite status"""
    try:
        project_status = execute_parallel_testsuites(testsuite_list, project_repository, data_repository, auto_defects, ts_parallel)
    except Exception:
