...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/junit_class.py
...
class Junit(object):
    ...
    def init_arg(self, **kwargs):
        ...
        """
        default_keys = [
         'errors', 'failures', 'skipped', 'time', 'passes']
        result = {}
    ...
    def create_testcase(self, location, timestamp, ts_timestamp, name, classname='customTestsuite_independant_testcase_execution', tag='testcase', **kwargs):
        """
            Create a testcase element
        """
        if self.root.find('testsuite') is None:
            self.update_attr('timestamp', timestamp, 'pj', '0')
            (self.create_testsuite)(location=location, name=classname, timestamp=timestamp, display='False', **(self.init_arg)(**kwargs))
        for ts in self.root.findall('testsuite'):
            if ts.get('timestamp') == ts_timestamp:
                tc = (self.create_element)(tag, classname=classname, timestamp=timestamp, exceptions='0', 
                 keywords='0', name=name, display='True', **(self.init_arg)(**kwargs))
                ts.append(tc)
                properties = self.create_element('properties')
    ...
    def add_keyword_result(self, tc_timestamp, step_num, kw_name, status, kw_timestamp, duration, resultfile, impact, onerror, desc='', info='', tc_name='', tc_resultsdir=''):
        ...
        if str(status).lower() == 'true':
            ...
            if str(status).lower() == 'false':
                status = 'FAIL'
        keyword_items = {'type':'keyword',  'display':'True',  'step':step_num,  'name':kw_name, 
         'status':status,  'timestamp':kw_timestamp,  'time':duration, 
         'resultfile':resultfile,  'impact':impact, 
         'onerror':onerror,  'description':desc,  'info':info}
        failing_status = [
        ...
        self.add_property(name=kw_name, value='KEYWORD_DISCARD', elem_type='kw', timestamp=tc_timestamp,
    ...
    def add_requirement(self, requirement, timestamp):
        ...
        self.get_ts_with_timestamp(timestamp).find('properties').append(self.create_element('property', {'name':'requirement',  'value':requirement}))
    ...
    def add_project_location(self, location):
        """add a new requirement when called"""
        self.root.find('properties').append(self.create_element('property', {'name':'location',  'value':location}))
        self.root.append(self.create_element('property', {'name':'location',  'value':location}))
    ...
    def update_count(self, attr, value, elem_type, timestamp='0'):
        ...
        if elem_type == 'pj':
            ...
            if elem_type == 'ts':
                ...
                elem = self.get_tc_with_timestamp(timestamp)
            attr = str(attr).lower()
            statuses = {'true':'passes', 
             'false':'failures',  'exception':'exceptions',  'error':'errors', 
             'skip':'skipped'}
            if attr in statuses:
                attr = statuses[attr]
        ...
        if attr == 'status':
            if elem.tag == 'testcase':
                if attr == 'false':
                    ...
                    elem.append(self.create_element('failure', {'message': 'errors/exceptions encountered during testcase execution'}))
