...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/kw_driver_class.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 17417 bytes
"""
...
class KeywordOperations(object):
    ...
    def get_values_for_mandatory_args(self):
        ...
        def get_value(arg):
            ...
            print_error("value for mandatory argument '{0}' not available in data_repository/args_repository".format(args))
    ...
    def get_values_for_optional_args(self, arg_kv):
        ...
        for args in self.optional_args_list:
            if args in self.args_repository:
                ...
                print_info("executing with default value '{0}' for optional argument '{1}'".format(arg_kv[args], args))
    ...
    def execute_method_for_keyword(self):
        ...
        if kw_status:
            ...
            self.data_repository = self.update_data_repository(self.keyword, keyword_result, self.data_repository)
        ...
        if keyword_result is None:
            ...
            if isinstance(keyword_result, str):
                if keyword_result.upper() in ('ERROR', 'EXCEPTION', 'RAN'):
                    pNote_level("Keyword '{0}' returned an {1}".format(keyword, keyword_result), 'debug', 'kw')
                    data_repository['step-%s_status' % step_num] = keyword_result.upper()
            else:
                if isinstance(keyword_result, bool):
                    ...
                    if isinstance(keyword_result, dict):
                        pNote_level("Keyword '{0}' returned only a dictionary .. updating data_repository".format(keyword), 'debug', 'kw')
                        ...
                        if isinstance(keyword_result, tuple):
                            if isinstance(keyword_result[0], str):
                                if keyword_result[0] == 'EXCEPTION':
                                    pNote_level("Keyword  '{0}' execution raised anexception".format(keyword), 'debug', 'kw')
                                    data_repository['step-%s_status' % step_num] = keyword_result[0]
                                    data_repository['step-%s_exception' % step_num] = keyword_result[1]
                            elif isinstance(keyword_result[0], str):
                                if keyword_result[0].upper() == 'RAN':
                                    pNote_level("Keyword '{0}' returned a status..".format(keyword), 'debug', 'kw')
                                    data_repository['step-%s_status' % step_num] = 'RAN'
                            else:
                                pNote_level("Keyword '{0}' returned multiplevalues ".format(keyword), 'debug', 'kw')
                                data_repository['step-%s_status' % step_num] = 'Error'
                                for element in keyword_result:
                                    if isinstance(element, bool):
                                        pNote_level("Keyword '{0}' returneda status..".format(keyword), 'debug', 'kw')
                                        data_repository['step-%s_status' % step_num] = element
                                    else:
                                        if isinstance(element, dict):
                                            pNote_level("Keyword '{0}' returned a dictionary.. will update data_repository".format(keyword), 'debug', 'kw')
                                            data_repository.update(element)
                                        else:
                                            pNote_level("unexpected return type form keyword '{0}'... expecting bool or dict ".format(keyword), 'debug', 'kw')

                        else:
                            pNote_level("unexpected return type form keyword '{0}'... expecting bool/dict/error/exception".format(keyword), 'debug', 'kw')
