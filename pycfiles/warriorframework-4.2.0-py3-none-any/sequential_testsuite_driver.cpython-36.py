...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/sequential_testsuite_driver.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 14455 bytes
"""
...
import warrior.WarriorCore.testsuite_driver as testsuite_driver, warrior.WarriorCore.onerror_driver as onerror_driver
from warrior.Framework.Utils.testcase_Utils import pNote

def execute_sequential_testsuites(testsuite_list, project_repository, data_repository, auto_defects):
    ...
    while suite_cntr < len(testsuite_list):
        ...
        if testsuite.find('runmode') is not None:
            if testsuite.find('runmode').get('attempt') is not None:
            ...
            if Utils.file_Utils.fileExists(testsuite_path) or action is False:
                if not goto_testsuite:
                    if action is True:
                        testsuite_result = testsuite_driver.main(testsuite_path, data_repository=data_repository,
                          from_project=True,
                ...
                time.sleep(2)
                (pj_junit_object.create_testsuite)(location=os.path.dirname(testsuite_path), 
                 name=testsuite_nameonly, 
                 timestamp=tmp_timestamp, **pj_junit_object.init_arg())
                ...
                pj_junit_object.update_attr('impact', impact_dict.get(testsuite_impact.upper()), 'ts', tmp_timestamp)
            ...
            msg = print_error('Test suite does not exist in provided path: {0}'.format(testsuite_path))
        testsuite_status = 'ERROR'
        if goto_testsuite:
            if goto_testsuite == str(suite_cntr):
                goto_testsuite = False
        if goto_testsuite:
            if goto_testsuite != str(suite_cntr):
            ...
            if goto_testsuite_num is False:
                ...
                if goto_testsuite_num == 'ABORT':
                    onerror = 'Abort'
                else:
                    onerror = 'Goto:' + str(goto_testsuite_num)
                pj_junit_object.update_attr('impact', impact_dict.get(testsuite_impact.upper()), 'ts', data_repository['wt_ts_timestamp'])
                pj_junit_object.update_attr('onerror', onerror, 'ts', data_repository['wt_ts_timestamp'])
                string_status = {'TRUE':'PASS', 
                 'FALSE':'FAIL',  'ERROR':'ERROR',  'SKIP':'SKIP', 
                 'RAN':'RAN'}
                ...
                ts_status_list, ts_impact_list = common_execution_utils.compute_status(testsuite, ts_status_list, ts_impact_list, testsuite_status, testsuite_impact)
            ...
            if runmode is not None:
                if not project_error_value:
                    if runmode.upper() == 'RUF':
                        if testsuite_status is False:
                            goto_testsuite = str(value)
                if runmode.upper() == 'RUP' and testsuite_status is True:
                    goto_testsuite = str(value)
            elif retry_type is not None:
                if testsuite.find('retry') is not None:
                    if testsuite.find('retry').get('attempt') is not None:
                ...
                if retry_type.upper() == 'IF':
                    try:
                        if data_repository[retry_cond] == retry_cond_value:
                            ...
                            pNote("The given condition '{0}' matches the expectedvalue '{1}'".format(data_repository[retry_cond], retry_cond_value))
            ...
            if testsuite_status is False or testsuite_status == 'ERROR' or testsuite_status == 'EXCEPTION':
                goto_testsuite = onerror_driver.main(testsuite, project_error_action, project_error_value)
            if goto_testsuite in ('ABORT', 'ABORT_AS_ERROR'):
                break
            elif goto_testsuite:
                if int(goto_testsuite) < suite_cntr:
                    suite_cntr = int(goto_testsuite) - 1
                    goto_testsuite = False

    if testsuite.find('runmode') is not None:
        if testsuite.find('runmode').get('attempt') is not None:
            if testsuite.find('runmode').get('attempt') == testsuite.find('runmode').get('runmode_val'):
                print_info('\n----------------- End of Testsuite Runmode Execution -----------------\n')
    project_status = Utils.testcase_Utils.compute_status_using_impact(ts_status_list, ts_impact_list)
...
def main(testsuite_list, project_repository, data_repository={}, auto_defects=False):
    """ Executes suites in a project sequentially """
    try:
        project_status = execute_sequential_testsuites(testsuite_list, project_repository, data_repository, auto_defects)
    except Exception:
