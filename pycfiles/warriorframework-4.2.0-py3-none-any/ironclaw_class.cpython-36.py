...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/ironclaw_class.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 19174 bytes
"""
...
import warrior.WarriorCore.Classes.kw_driver_class as kw_driver_class, warrior.WarriorCore.Classes.execution_files_class as execution_files_class
from warrior.Framework.Utils.print_Utils import print_info, print_debug, print_error, print_exception, print_warning
...
class IronClaw(object):
    ...
    def testsuite_prerun(self, testsuite_filepath, root, check_files_dict=None):
        ...
        if testsuite_status:
            data_file_valid, check_files_dict = self.check_testsuite_input_datafile(testsuite_filepath, check_files_dict)
            testsuite_status &= data_file_valid
            for testcase in root.iter('Testcase'):
                tc_path_rel = testsuite_utils.get_path_from_xmlfile(testcase)
                tc_path = file_Utils.getAbsPath(tc_path_rel, os.path.dirname(testsuite_filepath))
                time.sleep(5)
    ...
    def project_prerun(self, project_filepath, root):
        ...
        if project_status:
            ...
            for testsuite in root.iter('Testsuite'):
                ...
                testsuite_path = file_Utils.getAbsPath(testsuite_path_rel, os.path.dirname(project_filepath))
                if os.path.isfile(testsuite_path):
                    print_info('\n')
                    print_info('Testsuite_path: {0}, Testsuitefile exists...'.format(testsuite_path))
                    ts_root = xml_Utils.getRoot(testsuite_path)
                    tsuite_status = self.testsuite_prerun(testsuite_path, ts_root, check_files_dict)
                else:
                    print_info('\n')
                    tsuite_status = False
                    print_error('testsuite_path: {0},                                Testsuite file does not exist'.format(testsuite_path))
        ...
        for step in steps_list:
            ...
            try:
                ...
                if len(module_list) == 0:
                    print_error('No Actions package/Module existfor the driver {0} '.format(driver_path))
                    continue
                package_list = self.import_modules(module_list)
                if len(package_list) > 0:
                    result = self.search_for_match(package_list, step.attrib['Keyword'], step.attrib['Driver'])
        ...
        try:
            for module_string in module_list:
                if module_string is not '':
                    package_list.append(__import__(('%s' % module_string),
                      fromlist=['Actions']))
    ...
    def search_for_match(cls, package_list, keyword, driver_name):
        ...
        if len(search_result_list) == 1:
            print_info("Found one matching method/function for keyword '{0}'".format(keyword))
        else:
            if len(search_result_list) == 0:
                print_info("There is no matching keyword: '{0}' for the Driver: '{1}'".format(keyword, driver_name))
                search_result_list = None
            else:
                if len(search_result_list) > 1:
                    print_info("More than one method/function of same name: '{0}' exists for the Driver: '{1}'".format(keyword, driver_name))
                    search_result_list = None
    ...
    def check_tc_input_datafile(cls, testcase_filepath, check_files_dict):
        ...
        if check_files_dict is None:
            ...
            if check_files_dict['check_datafile'] is not True:
                result, check_files_dict = cls._IronClaw__check_input_datafile(testcase_filepath, 'Testcase', check_files_dict)
            if check_files_dict['check_resultsdir'] is not True:
                cls._IronClaw__check_dir_exists(testcase_filepath, 'Resultsdir')
            if check_files_dict['check_logsdir'] is not True:
                cls._IronClaw__check_dir_exists(testcase_filepath, 'Logsdir')
        if False in result:
            return False
        else:
            return True
    ...
    def check_testsuite_input_datafile(cls, testsuite_filepath, check_files_dict):
        ...
        if check_files_dict is None:
            ...
            result, check_files_dict = cls._IronClaw__check_input_datafile(testsuite_filepath, 'Testsuite', check_files_dict)
            if check_files_dict['check_resultsdir'] is not True:
                cls._IronClaw__check_dir_exists(testsuite_filepath, 'Resultsdir')
            if check_files_dict['check_logsdir'] is not True:
                cls._IronClaw__check_dir_exists(testsuite_filepath, 'Logsdir')
    ...
    def check_proj_results_logsdir(cls, project_filepath):
        ...
        """
        check_files_dict = {}
        cls._IronClaw__check_dir_exists(project_filepath, 'Resultsdir')
        cls._IronClaw__check_dir_exists(project_filepath, 'Logsdir')
        check_files_dict['check_resultsdir'] = True
        check_files_dict['check_logsdir'] = True
    ...
    def __check_dir_exists(cls, filepath, dirtag):
        ...
        if directory is not False and directory is not None:
            ...
            if not os.path.exists(directory):
                print_warning('Directory does not exist in location {0}.\nWarrior framework will try to create the directory, if creation fails then default warriorspace will be used to collect logs/results'.format(directory))
        else:
    ...
    def __check_input_datafile(cls, filepath, testname, check_files_dict):
        ...
        """
        result = []
        input_data_file = xml_Utils.getChildTextbyParentTag(filepath, 'Details', 'InputDataFile')
        if input_data_file is not False:
            if input_data_file is not None:
                if testname is 'Testsuite':
                    ...
                    if str(input_data_file).upper() == 'NO_DATA':
                        ...
                        data_file_path = file_Utils.getAbsPath(input_data_file, os.path.dirname(filepath))
                        print_info('{0} input data_file_path: {1}'.format(testname, data_file_path))
                        if os.path.exists(data_file_path):
                            print_info('{0} Input datafile is present in location {1}'.format(testname, data_file_path))
                            result.append(True)
                        else:
                            print_error('{0} Input datafile is NOT present in location {1}'.format(testname, data_file_path))
                            result.append(False)
        if input_data_file is None or input_data_file is False:
            if testname is 'Testcase':
                print_info('InputDataFile is not provided,checking if default InputDataFile exists....')
                default_datafilepath = execution_files_class.get_default_xml_datafile(filepath)
...
def get_action_dirlist(driverpath):
    ...
    try:
        if os.path.isfile(driverpath):
            ...
            search_string = re.compile('package_list.*=.*\\]', re.DOTALL | re.MULTILINE)
