...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Actions/CommonActions/common_actions.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 22259 bytes
"""
...
class CommonActions(object):
    __doc__ = 'class CommonActions having methods (keywords) that are common for all the products'
        ...
        if result is not None:
            if result is not False:
                if resp_pat == result[resp_ref]:
                    pNote('Found resp_pat={0} for resp_ref={1} in the data repository'.format(resp_pat, resp_ref))
                else:
                    status = False
                    pNote('NOT found resp_pat={0} for resp_ref={1} in the data repository!!'.format(resp_pat, resp_ref), 'warning')
        Utils.testcase_Utils.report_substep_status(status)
        return status

    def store_in_repo(self, datavar=None, datavalue=None, type='str', filepath=None, jsonkey='repo_variables', bool_store_all=False):
        ...
        if filepath is not None:
            ...
            try:
                ...
                if isinstance(bool_store_all, bool):
                    if bool_store_all is True:
                        print_info('bool_store_all is set to True, all content of given json file will be stored in data repository')
                        update_datarepository(json_doc)
                        print_info('{0} dictionary stored in Warrior data_repository'.format(json_doc))
                        status = True
                    else:
                        isinstance(bool_store_all, bool) or print_error('invalid value : {0} given for bool_store_all,valid value: boolean True or False'.format(bool_store_all))
                        status = False
                else:
                    if jsonkey in json_doc:
                        ...
                        for var_key, var_value in list(repo_dict.items()):
                            ...
                            Utils.dict_Utils.verify_key_already_exists_and_update(orig_dict=dict_to_update,
                              new_dict=build_dict)

                        update_datarepository(dict_to_update)
                        print_info('{0} dictionary stored in Warrior data_repository'.format(dict_to_update))
                        status = True
                    else:
                        print_error('The {0} file is missing the key "repo_variables", please refer to the Samples in Config_files'.format(filepath))
                        status = True
            except ValueError:
        ...
        if type is None or datavalue is None:
            if filepath is None:
                print_error('Either Provide values to arguments "datavar" & "datavalue" or to argument "filepath"')
        return status
    ...
    def verify_data(self, expected, object_key, type='str', comparison='eq'):
        ...
        if result not in ('FALSE', 'TRUE'):
            return result
        if result == 'FALSE':
            print_error('Expected: {0} {1} {2} but found {0}={3}'.format(object_key, comparison, expected, value))
            return False
        if result == 'TRUE':
            print_info('Expected: {0} {1} {2} found the same'.format(object_key, comparison, expected, value))
            return True

    def set_env_var(self, var_key=None, var_value=None, filepath=None, jsonkey='environmental_variables', overwrite='yes'):
        ...
        if not any([var_key, var_value, filepath]):
            ...
            if overwrite == 'NO':
                if os.getenv(var_key):
                    print_info("Using ENV variable {0} set earlier with value '{1}'".format(var_key, os.getenv(var_key)))
            if var_key is not None and var_value is not None and overwrite in ('YES',
                                                                               'NO'):
                        ...
                        for var_key, var_value in list(env_dict.items()):
                            if overwrite == 'NO':
                                ...
                                if overwrite in ('YES', 'NO'):
                                    ...
                                    if os.environ[var_key] == var_value:
                                        print_info('Setting ENV variable {0} with value {1}'.format(var_key, var_value))
                                        status = True
                                else:
                                    print_error('The attribute overwrite can only accept values either yes or no')

                    else:
                        print_error('The {0} file is missing the key "environmental_variables", please refer to the Samples in Config_files'.format(filepath))
                        status = False
            except ValueError:
                print_error('The file {0} is not a valid json file'.format(filepath))
                status = False
    ...
    def verify_arith_exp(self, expression, expected, comparison='eq', repo_key='exp_op'):
        ...
        Utils.testcase_Utils.pNote(wdesc)
        status = Utils.data_Utils.verify_arith_exp(expression, expected, comparison, repo_key)
        return status
    ...
    def get_time_delta(self, start_time, end_time=None, time_diff='time_diff', max_time_diff=None):
        ...
        if max_time_diff and time_delta > int(max_time_diff):
