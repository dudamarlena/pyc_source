...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/html_results_class.py
...
"""
import os, json, getpass, multiprocessing
...
class LineResult:
    __doc__ = 'Class that generates html result line items'
    data = {}
    html = ''
    ...
    def __init__(self):
        ...
        self.keys = [
         'type', 'name', 'info', 'description', 'timestamp', 'duration', 'status', 'impact', 'onerror', 'msc', 'static',
         'dynamic']
    ...
    def set_dynamic_content(self, line):
        ...
        self.data['dynamic'] = [
         line.get('keywords'), line.get('passes'), line.get('failures'),
    ...
    def set_attributes(self, line, variant, stepcount):
        ...
        if 'Keyword' not in variant:
            ...
            logs_span = "<span style='padding-left:10px; padding-right: 10px;'><a  name='results-link' href='{0}' target='_blank' ><i name='logs-icon' class='fa fa-book'  data-logPath='{0}' katana-click='execution.resultsViewer.openConsoleLogFile' > </i></a></span>".format(line.get('console_logfile')) if line.get('console_logfile') else ''
            defects_span = "<span style='padding-left:10px; padding-right: 10px;'><a name='bug-link' href='{0}' target='_blank' ><i name='bug-icon' class='fa fa-bug'  data-logPath='{0}' katana-click='execution.resultsViewer.openDefectsJson'> </i></a></span>".format(line.get('defects')) if line.get('defects') else ''
            span_html = ''
            if variant == 'Testcase':
                span_html = logs_span
        ...
        self.data = {'nameAttr':variant + 'Record', 
         ...
         'msc':span_html, 
         'static':[
          'Count', 'Passed', 'Failed', 'Errors', 'Exceptions', 'Skipped'], 
         'locn':locn}
                ...
                for elem in self.keys:
                    if elem == 'dynamic':
                        ...
                        if elem == 'name':
                            ...
                            top_level += '<td rowspan="2">' + div_html + (self.data[elem] if self.data[elem] else '') + '</div></td>'
                        else:
                            if elem == 'type':
...
class WarriorHtmlResults:
    __doc__ = 'Class that generates html results using hte junit result file '
    lineObjs = []
    lineCount = 0
    recount = 0
    steps = 0
    ...
    def __init__(self, junit_file=None):
        ...
        self.html_template = '{0}{1}reporting{1}html_results_template.html'.format(Tools.__path__[0], os.sep)
    ...
    def write_live_results(self, junitObj, givenPath, is_final):
        ...
        if junitObj:
            ...
            if is_final is True:
                ...
                print_info('Open the Results summary file given below in a browser to view results summary for this execution')
