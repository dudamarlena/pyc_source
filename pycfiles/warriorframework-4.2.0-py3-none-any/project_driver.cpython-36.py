...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/project_driver.py
...
"""
import sys, os, shutil, copy, traceback, glob
...
from warrior.WarriorCore import common_execution_utils, sequential_testsuite_driver, parallel_testsuite_driver
...
def get_project_details(project_filepath, res_startdir, logs_startdir, data_repository):
    ...
    def_on_error_value = Utils.xml_Utils.getChildAttributebyParentTag(project_filepath, 'Details', 'default_onError', 'value')
    ...
    efile_obj = execution_files_class.ExecFilesClass(project_filepath, 'proj', res_startdir, logs_startdir)
...
def execute_project(project_filepath, auto_defects, jiraproj, res_startdir, logs_startdir, data_repository):
    ...
    if data_repository['war_file_type'] == 'Project':
        filename = os.path.basename(project_filepath)
        html_filepath = os.path.join(project_repository['project_execution_dir'], Utils.file_Utils.getNameOnly(filename)) + '.html'
    ...
    pj_junit_object = junit_class.Junit(filename=project_name, timestamp=project_start_time, name=project_name,
      display='True')
    pj_junit_object.update_attr('resultsdir', project_repository['project_execution_dir'], 'pj', project_start_time)
    pj_junit_object.update_attr('title', project_repository['project_title'], 'pj', project_start_time)
    pj_junit_object.add_property('resultsdir', project_repository['project_execution_dir'], 'pj', project_start_time)
    ...
    if 'jobid' in data_repository:
        ...
        execution_type = Utils.xml_Utils.getChildAttributebyParentTag(project_filepath, 'Details', 'type', 'exectype')
        ...
        if execution_type.upper() == 'PARALLEL_SUITES':
            ...
            project_status = parallel_testsuite_driver.main(testsuite_list, project_repository, data_repository,
              auto_defects, ts_parallel=True)
        else:
            if execution_type.upper() == 'SEQUENTIAL_SUITES':
                print_info('Executing suites sequentially')
                project_status = sequential_testsuite_driver.main(testsuite_list, project_repository, data_repository, auto_defects)
            else:
...
def compute_project_status(project_status, testsuite_status, testsuite_impact):
    ...
    if testsuite_impact.upper() == 'IMPACT':
        ...
        if testsuite_impact.upper() == 'NOIMPACT':
...
def report_project_result(project_status, project_repository):
    ...
    """
    project_status = {'TRUE':'PASS', 
     'FALSE':'FAIL',  'ERROR':'ERROR', 
     'EXCEPTION':'ERROR',  'RAN':'RAN'}.get(str(project_status).upper())
    print_info('Project:{0}  STATUS:{1}'.format(project_repository['project_name'], project_status))
    return project_status


def main(project_filepath, data_repository={}, auto_defects=False, jiraproj=None, res_startdir=None, logs_startdir=None):
    """ Project driver"""
    try:
        project_status, project_repository = execute_project(project_filepath, auto_defects, jiraproj, res_startdir, logs_startdir, data_repository)
    except Exception:
        project_status, project_repository = (False, None)
        print_error('unexpected error {0}'.format(traceback.format_exc()))

    return (
     project_status, project_repository)