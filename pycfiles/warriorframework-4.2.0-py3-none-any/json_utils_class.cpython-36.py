...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/json_utils_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 13356 bytes
"""
...
class JsonUtils(object):
    ...
    def compare_json_objects(self, json_object1, json_object2, case_conversion=False, write_diff_to_console=True, check_for_subset=False):
    ...
    def write_json_diff_to_file(self, json_object1, json_object2, output_file):
        ...
        """
        result = self.compare_json_objects(json_object1, json_object2, write_diff_to_console=False)
        status = True
        if not result:
            status = False
    ...
    def compare_json_using_jsonpath(self, response, list_of_jsonpath, list_of_expected_api_responses):
        ...
        for index, jsonpath in enumerate(list_of_jsonpath):
            ...
            if match is False:
                ...
                if not match:
                    status = False
                    print_error("For the given '{0}' the expected response value is '{1}'. It doesn't match or available in the actual response value '{2}'".format(jsonpath, list_of_expected_api_responses[index], value))
