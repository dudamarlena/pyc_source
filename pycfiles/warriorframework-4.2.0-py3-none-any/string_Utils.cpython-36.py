...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/string_Utils.py
...
def return_quote(a, start_pat='${', end_pat='}'):
    ...
    for index in range(len(a)):
        if mode == 0 and a[index:index + len(start_pat)] == start_pat:
            ...
            if mode == 1 and a[index:index + len(end_pat)] == end_pat:
                count -= 1
                if count == 0:
...
def sub_from_varconfig(varconfigfile, string_list, var_sub=None, start_pat='${', end_pat='}'):
    ...
    for i, string in enumerate(string_list):
        ...
        elif string and vc_file is not None:
            newstring = sub_from_varconfigfile(string, vc_file, start_pat, end_pat)
            newstring_list.append(newstring)
        else:
            newstring_list.append(string)
...
def sub_from_wdf(datafile, string_list, td_sys_list=None, kw_system_name=None):
    ...
    for i in range(0, len(string_list)):
        ...
        while found:
            if string:
                match = re.search('(\\$wdf\\{)([^\\}]*)(\\})', string, re.IGNORECASE)
                if match is not None:
                    ...
                    try:
                        ...
                        if system_or_subsystem == 'kw_system' or system_or_subsystem == 'current_system':
                            ...
                            if system_or_subsystem == 'target_system':
                        ...
                        if not value:
                            print_error("Value for '{0}' not provided in the datafile={1}under the system_or_subsystem = {2}".format(tag_or_attr, datafile, system_or_subsystem))
                    except Exception as err:
                        print_error('Incorrect format provided for substituting valuefrom warrior datafile in the test data file')
...
def sub_from_varsub(string, var_sub):
    ...
    :Returns:
        1. updated string
    """
    match = re.search('.*(\\[(var_sub)\\]).*', string, re.IGNORECASE)
    if match is not None:
