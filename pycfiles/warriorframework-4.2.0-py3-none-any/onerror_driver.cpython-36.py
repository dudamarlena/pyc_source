...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/onerror_driver.py
...
def main(node, def_on_error_action, def_on_error_value, exec_type=False, skip_invoked=True, current_step_number=None):
    ...
    if WarriorCliClass.mock:
        ...
        error_handle = {}
        action, value = getErrorHandlingParameters(node, def_on_error_action, def_on_error_value, exec_type, current_step_number)
        call_function = {'NEXT':next, 
         'GOTO':goto,  'ABORT':abort,  'ABORT_AS_ERROR':abortAsError,  'EXECUTE_AND_RESUME':execute_and_resume}.get(action.upper())
        if skip_invoked:
...
def get_failure_results(error_repository):
    ...
    if error_repository['action'] == 'NEXT':
        ...
        if error_repository['action'] in ('GOTO', 'EXECUTE_AND_RESUME'):
            return error_repository['value']
        if error_repository['action'] in ('ABORT', 'ABORT_AS_ERROR'):
...
def getErrorHandlingParameters(node, def_on_error_action, def_on_error_value, exec_type, current_step_number=None):
    ...
    if (runmode == 'RUP' or runmode == 'RUF') and current_step_number == runmode_value - 1 or runmode == 'RUF':
        action = action
        ...
        if action is None or action is False or action == '':
            ...
            if action is not None:
                if action is not False:
                    supported_values = [
                     'next', 'goto', 'abort', 'abort_as_error', 'execute_and_resume']
