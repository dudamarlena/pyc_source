...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/testcase_utils_class.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 29202 bytes
"""
...
"""
import xml.etree.ElementTree as ET, inspect, re, traceback
from warrior.Framework.Utils.print_Utils import print_info, print_debug, print_warning, print_error, print_exception, print_sub, print_notype, print_without_logging
from warrior.Framework.Utils import config_Utils

class TestcaseUtils(object):
    __doc__ = 'testcase utils class'
    ...
    def p_subkeyword(self, keyword_txt):
        ...
        print_info('\n***************Sub-Keyword: %s ***************\n' % keyword_txt)
        self.gsubkey[self.gsubkeyloop] = ET.SubElement(self.gstep[self.gsteploop], 'Keyword')
    ...
    def p_substep(self, substep_txt=''):
        ...
        self.gsubstep[self.gsubsteploop] = ET.SubElement(self.gstep[self.gsteploop], 'SubStep')
    ...
    def p_note_level(self, txt, print_type='info', level=None, ptc=True):
        ...
        print_util_types = ['-D-', '', '-I-', '-E-', '-W-', '-N-',
         '\x1b[1;31m-E-\x1b[0m', '\x1b[1;33m-W-\x1b[0m']
        p_type = {'INFO':print_info,  'DEBUG':print_debug, 
         'WARN':print_warning, 
         ...
         'SUB':print_sub, 
         'NOTYPE':print_notype, 
         'NOLOG':print_without_logging}.get(str(print_type).upper(), print_info)
        txt = self.rem_nonprintable_ctrl_chars(str(txt))
        if write_locn is None:
            write_locn = self.current_pointer
        if ptc:
            if print_type not in print_util_types:
                p_type(txt)
    ...
    def rem_nonprintable_ctrl_chars(self, txt):
        ...
        try:
            txt = re.sub('[^\\x20-\\x7E|\\x09-\\x0A]', '', txt)
    ...
    def p_status(self, status, level):
        ...
        if level.upper() == 'SUBSTEP':
            ...
            if level.upper() == 'STEP':
                ...
                if level.upper() == 'KEYWORD':
                    levelobj = self.gkeyword[self.gkeywordloop]
                else:
                    if level.upper() == 'SUBKEYWORD':
    ...
    def p_report_requirements(self, requirement_id):
        ...
        self.grequirement[self.grequirementloop] = ET.SubElement(self.current_pointer, 'Requirement')
    ...
    def convert_logic(text):
        ...
        result = {True:'PASS', 
         ...
         'EXCEPTION':'ERROR',  'RAN':'RAN',  'WARN':'WARN'}.get(text)
        if result is None:
    ...
    def report_status(self, status, text='', level='Keyword'):
        ...
        :Returns:
            None
        """
        status = {'TRUE':self.p_pass, 
         'FALSE':self.p_fail,  'SKIP':self.p_skip, 
         'EXCEPTION':self.p_exception,  'ERROR':self.p_error, 
         'RAN':self.p_ran}.get(str(status).upper())
        if status is None:
    ...
    def report_warning(self, status, text='', level='subStep'):
        ...
        if WarriorCliClass.mock:
            ...
            status = {'TRUE':self.p_pass, 
             'FALSE':self.p_warn,  'SKIP':self.p_skip, 
             'EXCEPTION':self.p_exception,  'ERROR':self.p_error, 
             'RAN':self.p_ran}.get(str(status).upper())
            if status is None:
                print_error('unexpected or no value received, expecting TRUE/FALSE/SKIP')
    ...
    def compute_system_resultfile(self, kw_resultfile_list, resultsdir, system_name):
        ...
        system_resultfile = self.file_utils().getCustomLogFile('system', system_results_dir, system_name, '.xml')
            ...
            if impact is not None:
                if impact is not False:
                    ...
                    if impact.lower() not in supported_values:
                        print_warning("unsupported value '{0}' provided for impact,supported values are '{1}',case-insensitive".format(impact, supported_values))
            ...
            if context is not None:
                if context is not False:
                    ...
                    if context.lower() not in supported_values:
                        print_warning("unsupported value '{0}' provided for context,supported values are '{1}',case-insensitive".format(context, supported_values))
    ...
    def get_defonerror_fromxml_file(self, filepath):
        ...
        from the testcase.xml/testsuite.xml/project.xml file """
        def_on_error_action = self.xml_utils().getChildAttributebyParentTag(filepath, 'Details', 'default_onError', 'action')
        if def_on_error_action is None or def_on_error_action is False:
            def_on_error_action = 'NEXT'
        else:
            if def_on_error_action is not None:
                if def_on_error_action is not False:
                    supported_values = [
                     'next', 'goto', 'abort', 'abort_as_error']
                    if str(def_on_error_action).lower() not in supported_values:
                        print_warning("unsupported option '{0}' provided for default_onErroraction, supported values are {1}".format(def_on_error_action, supported_values))
    ...
    def get_setup_on_error(self, filepath):
        ...
        setup_on_error_action = self.xml_utils().getChildAttributebyParentTag(filepath, 'Details', 'setup_onError', 'action')
        if setup_on_error_action is None or setup_on_error_action is False:
            setup_on_error_action = 'abort'
        else:
            if setup_on_error_action is not None:
                if setup_on_error_action is not False:
                    supported_values = [
                     'next', 'abort']
                    if str(setup_on_error_action).lower() not in supported_values:
                        print_warning("unsupported option '{0}' provided for setup_onErroraction, supported values are {1}".format(setup_on_error_action, supported_values))
