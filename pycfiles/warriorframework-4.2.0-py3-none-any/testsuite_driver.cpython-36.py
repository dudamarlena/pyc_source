...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/testsuite_driver.py
...
def report_testsuite_result(suite_repository, suite_status):
    ...
    print_info('\n ****** TestSuite Result ******')
    suite_status = {'TRUE':'PASS',  'FALSE':'FAIL',  'EXCEPTION':'FAIL',  'ERROR':'FAIL'}.get(str(suite_status).upper())
...
def get_testwrapper_file_details(testsuite_filepath, data_repository):
    ...
    if 'ow_testwrapperfile' in 'data_repository':
        ...
        if Utils.xml_Utils.nodeExists(testsuite_filepath, 'TestWrapperFile'):
            testwrapperfile = Utils.xml_Utils.getChildTextbyParentTag(testsuite_filepath, 'Details', 'TestWrapperFile')
        else:
            return [
...
def execute_testsuite(testsuite_filepath, data_repository, from_project, auto_defects, jiraproj, res_startdir, logs_startdir, ts_onError_action, queue, ts_parallel):
    ...
    suite_repository = get_suite_details(testsuite_filepath, data_repository, from_project, res_startdir, logs_startdir)
    if data_repository.get('random_tc_execution', False) or suite_repository['suite_random_exec']:
        ...
        randomize = False
    testcase_list = common_execution_utils.get_step_list(testsuite_filepath, 'Testcases',
      'Testcase', randomize=randomize)
    for testcase in testcase_list:
        ...
        if not jiraids:
            ...
            for jiraid in jiraids:
                ...
                if status is False:
                    print_warning('Cannot get status of jira issue {}, This issue is not considered for testcase skip validation'.format(jiraid))
                    continue
        ...
        if skip:
            ...
            print_info('Associated jira ids : {} are not Resolved or Closed, Skipping testcase >>>> {}'.format(jiraids, tc_path))
    ...
    data_repository['suite_exectype'] = 'iterative' if execution_type == 'ITERATIVE_SEQUENTIAL' or execution_type == 'ITERATIVE_PARALLEL' else execution_type
    ...
    if 'wt_junit_object' in data_repository:
        ...
        ts_junit_object = junit_class.Junit(filename=suite_name, timestamp=suite_timestamp, name='customProject_independant_testcase_execution',
    ...
    (ts_junit_object.create_testsuite)(location=os.path.dirname(testsuite_filepath), name=suite_name, 
     timestamp=suite_timestamp, suite_location=suite_repository['testsuite_filepath'], 
     title=suite_repository['suite_title'], 
     display=ts_junit_display, **ts_junit_object.init_arg())
    ts_junit_object.update_attr('resultsdir', suite_repository['suite_execution_dir'], 'ts', suite_timestamp)
    ts_junit_object.add_property('resultsdir', suite_repository['suite_execution_dir'], 'ts', suite_timestamp)
    ...
    testsuite_utils.pSuite_testsuite(junit_resultfile, suite_name, errors='0',
      skipped='0',
      tests=no_of_tests,
      failures='0',
      time='0',
    ...
    if 'jobid' in data_repository:
        testsuite_utils.pSuite_property(junit_resultfile, 'resultlocation', data_repository['jobid'])
    ...
    if data_repository['war_file_type'] == 'Suite':
        filename = os.path.basename(testsuite_filepath)
        html_filepath = os.path.join(suite_repository['suite_execution_dir'], Utils.file_Utils.getNameOnly(filename)) + '.html'
    ...
    if not from_project:
        ...
        if testwrapperfile:
            ...
            setup_tc_status, data_repository = testcase_driver.execute_testcase(testwrapperfile, data_repository,
              tc_context='POSITIVE', runtype=j_runtype,
              tc_parallel=None,
              queue=None,
              auto_defects=auto_defects,
              suite=None,
              jiraproj=None,
              tc_onError_action='ABORT_AS_ERROR',
              iter_ts_sys=None,
              steps_tag='Setup')
            print_info('*****************TESTWRAPPER SETUP EXECUTION END**********************')
        if setup_on_error_action == 'next' or setup_on_error_action == 'abort' and setup_tc_status == True:
            if execution_type.upper() == 'PARALLEL_TESTCASES':
                ...
                test_suite_status = parallel_testcase_driver.main(testcase_list, suite_repository, data_repository,
                  from_project, tc_parallel=True,
                  auto_defects=auto_defects)
            else:
                if execution_type.upper() == 'SEQUENTIAL_TESTCASES':
                    if runmode is None:
                        print_info('Executing testcases sequentially')
                        test_suite_status = sequential_testcase_driver.main(testcase_list, suite_repository, data_repository,
                          from_project, auto_defects=auto_defects)
                    else:
                        if runmode.upper() == 'RUF':
                            ...
                            while i < int(value):
                                ...
                                if str(test_suite_status).upper() == 'FALSE' or str(test_suite_status).upper() == 'ERROR':
                                    break
                    ...
                    execution_value = Utils.xml_Utils.getChildAttributebyParentTag(testsuite_filepath, 'Details', 'type', 'Max_Attempts')
            ...
            if runmode is not None:
                test_suite_status = common_execution_utils.compute_runmode_status(testsuite_status_list, runmode, suite_global_xml)
        else:
            print_error('Test cases in suite are not executed as setup failed to execute,setup status : {0}'.format(setup_tc_status))
            print_error('Steps in cleanup will be executed on besteffort')
            test_suite_status = 'ERROR'
    if not isinstance(test_suite_status, bool) or isinstance(test_suite_status, bool) and test_suite_status is False:
        if testwrapperfile:
            if Utils.xml_Utils.nodeExists(testwrapperfile, 'Debug'):
                print_info('*****************SUITE TESTWRAPPER DEBUG EXECUTION START*********************')
                data_repository['wt_data_type'] = j_data_type
                debug_tc_status, data_repository = testcase_driver.execute_testcase(testwrapperfile, data_repository,
                  tc_context='POSITIVE', runtype=j_runtype,
                  tc_parallel=None,
                  queue=None,
                  auto_defects=auto_defects,
    ...
    if testwrapperfile:
        ...
        data_repository['wt_data_type'] = j_data_type
        cleanup_tc_status, data_repository = testcase_driver.execute_testcase(testwrapperfile, data_repository,
          tc_context='POSITIVE', runtype=j_runtype,
          tc_parallel=None,
          queue=None,
          auto_defects=auto_defects,
    ...
    if test_suite_status == False:
        if ts_onError_action:
            if ts_onError_action.upper() == 'ABORT_AS_ERROR':
                print_info("Testsuite status will be marked as ERROR as onError action is setto 'abort_as_error'")
                test_suite_status = 'ERROR'
...
def main(testsuite_filepath, data_repository={}, from_project=False, auto_defects=False, jiraproj=None, res_startdir=None, logs_startdir=None, ts_onError_action=None, queue=None, ts_parallel=False):
    """Executes a test suite """
    try:
        test_suite_status, suite_repository = execute_testsuite(testsuite_filepath, data_repository, from_project, auto_defects, jiraproj, res_startdir, logs_startdir, ts_onError_action, queue, ts_parallel)
    except Exception:
        print_error('unexpected error {0}'.format(traceback.format_exc()))
        test_suite_status, suite_repository = (False, None)

    return (
     test_suite_status, suite_repository)