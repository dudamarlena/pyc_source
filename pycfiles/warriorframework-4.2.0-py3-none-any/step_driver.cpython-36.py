...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/step_driver.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 15475 bytes
"""
...
def send_keyword_to_productdriver(driver_name, plugin_name, keyword, data_repository, args_repository, repo_name):
...
def get_keyword_resultfile(data_repository, system_name, step_num, keyword):
    ...
    keyword_resultfile = Utils.file_Utils.getCustomLogFile(prefix, kw_results_dir, keyword, '.xml')
...
def get_step_console_log(filename, logsdir, console_name):
    ...
    print_info('\n... console logs for {0} will be logged in {1} '.format(console_name, console_logfile))
    Utils.config_Utils.debug_file(console_logfile)
    return console_logfile


def execute_step(step, step_num, data_repository, system_name, kw_parallel, queue, skip_invoked=True):
    ...
    if parallel is True:
        step_console_log = get_step_console_log(data_repository['wt_filename'], data_repository['wt_logsdir'], 'step-{0}_{1}_consoleLogs'.format(step_num, keyword))
    ...
    kw_resultfile = get_keyword_resultfile(data_repository, system_name, step_num, keyword)
    ...
    if step.find('runmode') is not None and step.find('runmode').get('attempt') is not None:
        ...
        print_info('KEYWORD ATTEMPT: {0}'.format(step.find('runmode').get('attempt')))
    else:
        ...
        if step.find('retry') is not None:
            if step.find('retry').get('attempt') is not None:
                print_info('KEYWORD ATTEMPT: {0}'.format(step.find('retry').get('attempt')))
        ...
        send_keyword_to_productdriver(driver, plugin, keyword, data_repository, args_repository, repo_name)
        keyword_status = data_repository[('step-%s_status' % step_num)]
        Utils.testcase_Utils.update_step_num(str(step_num))
        if context.upper() == 'NEGATIVE':
            if type(keyword_status) == bool:
                print_debug('Keyword status = {0}, Flip status as context is Negative'.format(keyword_status))
                keyword_status = not keyword_status
        if WarriorCliClass.mock:
            if keyword_status is True or keyword_status is False:
                keyword_status = 'RAN'
        print_info('')
        print_info('*** Keyword status ***')
        step_goto_value = False
        step_onError_action = Utils.xml_Utils.get_attributevalue_from_directchildnode(step, 'onError', 'action')
        if step_onError_action is not False:
            if step_onError_action.upper() == 'GOTO':
                step_goto_value = Utils.xml_Utils.get_attributevalue_from_directchildnode(step, 'onError', 'value')
        ...
        if step_onError_action.upper() == 'GOTO':
            if step_goto_value is False:
                step_goto_value = data_repository['wt_def_on_error_value']
        onerror = step_onError_action.upper()
        if step_goto_value is not False:
            if step_goto_value is not None:
                onerror = onerror + ' step ' + step_goto_value
        if keyword_status is False:
            if step_onError_action:
                if step_onError_action.upper() == 'ABORT_AS_ERROR':
                    if skip_invoked:
                        print_info("Keyword status will be marked as ERROR as onError action is set to'abort_as_error'")
                        keyword_status = 'ERROR'
        Utils.testcase_Utils.reportKeywordStatus(keyword_status, keyword)
        print_info('step number: {0}'.format(step_num))
        string_status = {'TRUE':'PASS', 
         'FALSE':'FAIL',  'ERROR':'ERROR', 
         'EXCEPTION':'EXCEPTION',  'SKIP':'SKIP',  'RAN':'RAN'}
        if str(keyword_status).upper() in list(string_status.keys()):
            data_repository['step_%s_result' % step_num] = string_status[str(keyword_status).upper()]
        else:
        ...
        if 'step-%s_exception' % step_num in data_repository:
            msg = 'Exception message: ' + data_repository[('step-%s_exception' % step_num)]
        ...
        if step.find('runmode') is not None:
            if step.find('runmode').get('attempt') is not None:
                if step.find('runmode').get('attempt') == step.find('runmode').get('runmode_val'):
        ...
        if step.get('loop_id'):
            ...
            loop_id = ''
    add_keyword_result(tc_junit_object, tc_timestamp, step_num, keyword, keyword_status,
      kw_start_time, kw_duration, kw_resultfile,
      impact, onerror, step_description, info=(str(loop_id) + str(args_repository)),
      tc_name=tc_name,
    ...
    if parallel is True:
        ...
        if not data_repository['war_parallel']:
            ...
            if war_file_type == 'Case':
                tc_junit_object.output_junit((data_repository['wt_resultsdir']), print_summary=False)
            else:
                if war_file_type == 'Suite':
    ...
    return (
     keyword_status, kw_resultfile, step_impact)


def add_keyword_result(tc_junit_object, tc_timestamp, step_num, keyword, keyword_status, kw_start_time, kw_duration, kw_resultfile, impact, onerror, step_description, info='', tc_name='', tc_resultsdir=''):
    """ Add keyword results into junit object """
    tc_junit_object.add_keyword_result(tc_timestamp, step_num, keyword, (str(keyword_status)),
      kw_start_time, kw_duration,
      kw_resultfile, impact,
      onerror, step_description, info=info, tc_name=tc_name,
      tc_resultsdir=tc_resultsdir)
    tc_junit_object.update_count(str(keyword_status), '1', 'tc', tc_timestamp)
    tc_junit_object.update_count('keywords', '1', 'tc', tc_timestamp)


def main(step, step_num, data_repository, system_name, kw_parallel=False, queue=None, skip_invoked=True):
    """Get a step, executes it and returns the result """
    try:
        step_status = execute_step(step, step_num, data_repository, system_name, kw_parallel, queue,
          skip_invoked=skip_invoked)
    except Exception:
        step_status = (
         False, [], data_repository['wt_step_impact'], False)
