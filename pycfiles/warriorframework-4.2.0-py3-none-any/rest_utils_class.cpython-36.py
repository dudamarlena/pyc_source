...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/rest_utils_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 28056 bytes
"""
...
"""
import re, time, os, os.path, json as JSON
...
class WRest(object):
    __doc__ = 'WRest class has methods required to interact\n    with REST interfaces'
    ...
    def import_requests(self):
        ...
        try:
            ...
            pNote('Requests module is not installedPlease install requests module toperform any activities related to REST interfaces', 'error')
        else:
            self.req = requests
    ...
    def cmp_response(self, response, expected_api_response, expected_response_type, output_file, generate_output_diff_file=True):
        """
            ...
            if expected_response_type in response.headers['Content-Type']:
                ...
                if 'xml' in response.headers['Content-Type']:
                    try:
                        ...
                        if '.xml' == extension:
                            pNote('File does not exist in the provided file path', 'error')
                            return False

                    status, sorted_file1, sorted_file2, output_file = Utils.xml_Utils.compare_xml(extracted_response, expected_api_response, output_file,
                      sorted_json=False)
                else:
                    if 'json' in response.headers['Content-Type']:
                        ...
                        status = self.json_utils.write_json_diff_to_file(extracted_response, expected_api_response, output_file)
                    else:
                        if 'text' in response.headers['Content-Type']:
                            ...
                            status = Utils.string_Utils.text_compare(extracted_response, expected_api_response, output_file)
                if not status:
                ...
                type_of_response = Utils.rest_Utils.get_type_of_api_response(response)
                pNote('Expected response type is {0}'.format(expected_response_type), 'info')
                pNote('API response type is {0}'.format(type_of_response), 'info')
                pNote('api_response and expected_api_response types do not match', 'error')
                return False
        else:
            return False

    def cmp_content_response(self, datafile, system_name, response, expected_api_response, expected_response_type, comparison_mode):
        """
            Performs the comparison between api response
            and expected_api_response
        ...
        if expected_response_type in response.headers['Content-Type']:
            ...
            if comparison_mode:
                ...
                path_list, responses_list = Utils.xml_Utils.list_path_responses_datafile(datafile, system_name)
            if path_list:
                if 'xml' in response.headers['Content-Type']:
    ...
    def report_response_status(cls, status, expected_response, action):
        ...
        if expected_response is None or expected_response is False or expected_response == [] or expected_response == '':
            ...
            if isinstance(expected_response, list):
                ...
                if str(status) == expected_response:
                    pNote('http {0} successful'.format(action), 'info')
                    result = True
        if not result:
    ...
    def check_connection(self, url, auth=None, **kwargs):
        ...
        try:
            ...
            if not str(api_response).startswith('2') or str(api_response).startswith('1'):
                pNote('Connection was successful, but there wasproblem accessing the resource: {0}'.format(url), 'info')
                status = False
        ...
        if api_response is not None:
            try:
                ...
                try:
                    ...
                    pNote('api_response (JSON format): \n {0}'.format(JSON.dumps(output_response, indent=4)))
            else:
