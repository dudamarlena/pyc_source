...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/testcase_steps_execution.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 21288 bytes
"""
...
"""
import traceback, warrior.WarriorCore.step_driver as step_driver, warrior.WarriorCore.onerror_driver as onerror_driver, warrior.WarriorCore.exec_type_driver as exec_type_driver
...
class TestCaseStepsExecutionClass:
    ...
    def execute_step(self, current_step_number, go_to_step_number):
        ...
        if not self.go_to_step_number or self.go_to_step_number == str(self.current_step_number):
            self.run_current_step, self.current_triggered_action = exec_type_driver.main((self.current_step), skip_invoked=(self.skip_invoked))
        ...
        runmode, value, runmode_timer = common_execution_utils.get_runmode_from_xmlfile(self.current_step)
        retry_type, retry_cond, retry_cond_value, retry_value, retry_interval = common_execution_utils.get_retry_from_xmlfile(self.current_step)
        if runmode is not None:
            return self._execute_runmode_step(runmode_timer, runmode, self.step_status, value)
        else:
            if retry_type is not None:
                return self._execute_retry_type_step(retry_type, retry_cond, retry_cond_value, retry_interval, retry_value)
    ...
    def _report_step_as_not_run(self):
        ...
        kw_resultfile = step_driver.get_keyword_resultfile(self.data_repository, self.system_name, self.current_step_number, keyword)
        ...
        self.data_repository['wt_junit_object'].add_keyword_result(self.data_repository['wt_tc_timestamp'], self.current_step_number, keyword, 'SKIPPED', kw_start_time, '0', 'skipped', impact_dict.get(step_impact.upper()), 'N/A')
        ...
        if self.current_triggered_action.upper() in ('ABORT', 'ABORT_AS_ERROR'):
            return (
             self.current_step_number, self.go_to_step_number, 'break')
        if self.current_triggered_action.upper() in ('SKIP', 'NEXT'):
            return (self.current_step_number, self.go_to_step_number, 'continue')
        else:
            if self.current_triggered_action == 'SKIP_INVOKED':
                if self.skip_invoked:
                    print_info('Skipping this step as it is an invoked step.')
                    return (self.current_step_number, self.go_to_step_number, 'continue')
            else:
                if self.current_triggered_action:
                    if int(self.current_triggered_action) < self.current_step_number:
                        self.current_step_number = int(self.current_triggered_action) - 1
            return (
             self.current_step_number, self.go_to_step_number, 'continue')
    ...
    def _execute_current_step(self):
        ...
        """
        try:
            result = step_driver.main((self.current_step), (self.current_step_number), (self.data_repository),
              (self.system_name), skip_invoked=(self.skip_invoked))
        ...
        self.go_to_step_number = False
        self.step_status_list, self.step_impact_list = common_execution_utils.compute_status(self.current_step, self.step_status_list, self.step_impact_list, step_status, step_impact)
        self.kw_resultfile_list.append(kw_resultfile)
        return step_status
    ...
    def _skip_because_of_goto(self):
        ...
        kw_start_time = Utils.datetime_utils.get_current_timestamp()
        step_impact = Utils.testcase_Utils.get_impact_from_xmlfile(self.current_step)
        impact_dict = {'IMPACT':'Impact', 
         'NOIMPACT':'No Impact'}
        self.data_repository['wt_junit_object'].add_keyword_result(self.data_repository['wt_tc_timestamp'], self.current_step_number, keyword, 'SKIPPED', kw_start_time, '0', 'skipped', impact_dict.get(step_impact.upper()), 'N/A', step_description)
        self.data_repository['step_{}_result'.format(self.current_step_number)] = 'SKIPPED'
        if self.current_step.find('runmode') is not None:
            if self.current_step.find('runmode').get('attempt') is not None:
                if self.current_step.find('runmode').get('attempt') == self.current_step.find('runmode').get('runmode_val'):
    ...
    def _execute_runmode_step(self, runmode_timer, runmode, step_status, value):
        ...
        if runmode_timer is not None:
            if runmode_evaluation:
        ...
        if runmode.upper() == 'RUF':
            if step_status is True or step_status is False:
                ...
                if step_status:
                    if self.current_step_number == runmode_value - 1:
                        self.go_to_step_number = onerror_driver.main((self.current_step),
                          (self.default_error_action), (self.default_error_value), skip_invoked=(self.skip_invoked),
            ...
            if step_status is False or str(step_status).upper() in ('ERROR', 'EXCEPTION'):
                ...
                if self.go_to_step_number in ('ABORT', 'ABORT_AS_ERROR'):
                    return (
                     self.current_step_number, self.go_to_step_number, 'break')
                if type(self.go_to_step_number) is list:
                    self._TestCaseStepsExecutionClass__run_execute_and_resume_mode()
        return (
         self.current_step_number, self.go_to_step_number, 'continue')
        ...
        if retry_type.upper() == 'IF':
            try:
                if self.data_repository[retry_cond] == retry_cond_value:
                    ...
                    pNote("The given condition '{0}' matches the expected value '{1}'".format(self.data_repository[retry_cond], retry_cond_value))
                    wait_for_timeout(retry_interval)
                else:
                    condition_met = False
                    print_warning("The condition value '{0}' does not match with the expected value '{1}'".format(self.data_repository[retry_cond], retry_cond_value))
            except KeyError:
            ...
            if retry_type.upper() == 'IF NOT':
                try:
                    if self.data_repository[retry_cond] != retry_cond_value:
                        condition_met = True
                        pNote('Wait for {0}sec before retrying'.format(retry_interval))
                        pNote("The condition value '{0}' does not match with the expected value '{1}'".format(self.data_repository[retry_cond], retry_cond_value))
                        wait_for_timeout(retry_interval)
                    else:
                        condition_met = False
                except KeyError:
                ...
                if not condition_met:
                    pNote("The given condition '{0}' matched with the value '{1}'".format(self.data_repository[retry_cond], retry_cond_value))
                    self.go_to_step_number = str(retry_value)
            return (
             self.current_step_number, self.go_to_step_number, 'continue')
    ...
    def _execute_step_otherwise(self, step_status):
        ...
        if step_status is False or str(step_status).upper() in ('ERROR', 'EXCEPTION'):
            ...
            if type(self.go_to_step_number) is list:
                self._TestCaseStepsExecutionClass__run_execute_and_resume_mode()
            elif self.go_to_step_number:
                if int(self.go_to_step_number) < self.current_step_number:
    ...
    def __run_execute_and_resume_mode(self):
        ...
        result = execute_steps(temp_step_list, (self.data_repository), (self.system_name), (self.parallel),
          (self.queue), skip_invoked=False, step_num=(self.go_to_step_number))
...
def execute_steps(step_list, data_repository, system_name, parallel, queue, skip_invoked=True, step_num=None):
    ...
    """
    if parallel is True:
        system_console_log = get_system_console_log(data_repository['wt_filename'], data_repository['wt_logsdir'], '{0}_consoleLogs'.format(system_name))
    else:
        goto_stepnum = False
        tc_step_exec_obj = TestCaseStepsExecutionClass(step_list, data_repository, goto_stepnum, system_name,
        ...
        if parallel is True:
            ...
            if skip_invoked:
                ...
                return (
                 tc_step_exec_obj.step_status_list, tc_step_exec_obj.kw_resultfile_list,
