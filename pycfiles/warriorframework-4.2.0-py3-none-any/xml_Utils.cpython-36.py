...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/xml_Utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 36055 bytes
"""
    ...
    try:
        ...
        for node in root.findall(pnode):
            ...
            if dnode == None:
                status = True
                return status
            if node.find(dnode) is not None:
...
def compare_xml(xml1, xml2, output_file=False, sorted_json=True, remove_namespaces=False, tag_list=[], attrib_list=[]):
    """
    ...
    try:
        ...
        output1 = json.loads(json.dumps(xmltodict.parse(xml1, xml_attribs=True)))
        output2 = json.loads(json.dumps(xmltodict.parse(xml2, xml_attribs=True)))
        ...
        if output1 == output2:
            return (
             True, sorted_file1, sorted_file2, None)
        diff = '\n'.join(difflib.ndiff(json_obj1.splitlines(), json_obj2.splitlines()))
        if output_file:
...
def compare_xml_using_xpath(response, list_of_xpath, list_of_expected_api_responses):
    ...
    if len(list_of_xpath) != len(list_of_expected_api_responses):
        print_error('The number of xpath given is differentfrom the number of expected responsePlease check the value\nlist_of_xpath: {}\nlist_of_expected_response: {}'.format(list_of_xpath, list_of_expected_api_responses))
        status = False
    if status:
        for index, xpath_pattern in enumerate(list_of_xpath):
            ...
            if match is False:
                try:
                    ...
                    print_warning("Python regex search failed, invalid expected_response_pattern '{}' is provided".format(list_of_expected_api_responses[index]))

                if not match:
                    status = False
                    print_error("For the given '{0}' the expected response value is '{1}'. It doesn't match or available in the actual response value '{2}'".format(xpath_pattern, list_of_expected_api_responses[index], value))
...
def list_path_responses_datafile(datafile, system_name):
    ...
    """
    path_element_list = getElementListWithSpecificXpath(datafile, "./*[@name='" + system_name + "']/comparison_mode/*")
    path_list = [x.text for x in path_element_list]
    resp_element_list = getElementListWithSpecificXpath(datafile, "./*[@name='" + system_name + "']/expected_api_response/*")
