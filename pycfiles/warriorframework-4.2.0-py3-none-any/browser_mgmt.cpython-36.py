...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/WSelenium/browser_mgmt.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 29637 bytes
"""
...
import platform, urllib.request, urllib.error, urllib.parse
from subprocess import check_output, CalledProcessError
from distutils.version import LooseVersion
from warrior.Framework.Utils.datetime_utils import get_current_timestamp
from warrior.Framework.Utils.testcase_Utils import pNote
from warrior.Framework.Utils.print_Utils import print_error, print_info, print_debug, print_exception, print_warning
...
try:
    ...
    KEYS = {1:Keys.NUMPAD1, 
     2:Keys.NUMPAD2,  3:Keys.NUMPAD3,  4:Keys.NUMPAD4, 
     5:Keys.NUMPAD5,  6:Keys.NUMPAD6,  7:Keys.NUMPAD7, 
     8:Keys.NUMPAD8,  9:Keys.NUMPAD9}
except ImportError as exception:
    print_exception(exception)

class BrowserManagement(object):
    __doc__ = 'Browser management class'
    ...
    def open_browser(self, browser_name='firefox', webdriver_remote_url=False, desired_capabilities=None, **kwargs):
        ...
        if 'profile_dir' in kwargs:
            ...
            if webdriver_remote_url:
                print_debug("Opening browser '{0}' through remote server at '{1}'".format(browser_name, webdriver_remote_url))
            else:
    ...
    def save_screenshot(self, browser_instance=None, filename=None, directory=None):
        """
    ...
    def go_to(self, url, browser_instance=None):
        ...
        try:
            ...
            if 'Reached error page' in str(err):
                print_error('Unable to Navigate to URL:{}possibly because of the url is not valid'.format(url))
            else:
                status = False
        except Exception as err:
            print_error(err)
    ...
    def open_tab(self, browser_instance=None, url=None, browser_type='firefox'):
        ...
        if browser_instance is None:
            ...
            if LooseVersion(self.get_browser_version(browser_instance)) < LooseVersion('47.0.0'):
                element = browser_instance.find_element_by_tag_name('body')
                element.send_keys(Keys.LEFT_CONTROL, 'n')
        else:
            if browser_type == 'firefox' or browser_type == 'chrome' and LooseVersion(self.get_browser_version(browser_instance)) > LooseVersion('60.0.0'):
                print_error("Firefox (47 or above) and Chrome with chromedriver (2.32 or above)doesn't support opening new tab. Open tab may not function correctly")
                status = False
            else:
                element = browser_instance.find_element_by_tag_name('body')
    ...
    def get_firefox_version(self, binary):
        ...
        try:
            ...
            match = re.search('\\d+\\.\\d+', raw_version)
    ...
    def _make_browser(self, browser_name, desired_capabilities=None, profile_dir=None, webdriver_remote_url=None, **kwargs):
        """method to open a browser, calls other sepcific/generic
        make browser methods to open a browser """
        browser_methods = {'ff':self._make_ff, 
         'firefox':self._make_ff,  'chrome':self._make_chrome}
        creation_method = browser_methods.get(browser_name, None)
        if creation_method is None:
            print_error('{} is not a supported browser. Please use firefox or chrome'.format(browser_name))
            browser = None
        else:
    ...
    def _make_ff(self, webdriver_remote_url, desired_capabilites, profile_dir, **kwargs):
        ...
        log_dir = get_object_from_datarepository('wt_logsdir') if gecko_log in (None,
                                                                                False) else gecko_log
        log_dir = os.path.join(log_dir, 'gecko_' + kwargs.get('browser_name', 'default') + '.log')
        browser = None
        try:
            if webdriver_remote_url:
                browser = self._create_remote_web_driver(webdriver.DesiredCapabilities.FIREFOX, webdriver_remote_url, desired_capabilites, ff_profile)
            else:
                optional_args = {}
                ...
                if binary not in (False, None):
                    if not fileExists(binary):
                        print_warning("Given firefox binary '{}' does not exist, default firefox will be used for execution.".format(binary))
                        binary = None
                else:
                    print_info('No value given for firefox binary, default firefox will be used for execution.')
                ...
                browser = (webdriver.Firefox)(firefox_binary=ffbinary, capabilities=ff_capabilities, 
            ...
            if 'executable needs to be in PATH' in str(err):
                ...
                if 'Expected browser binary location' in str(err):
        ...
        if browser is None:
            if any((LooseVersion(webdriver.__version__) < LooseVersion('3.5.0'), gecko_path is None)):
                print_info('Unable to create Firefox browser, one possible reason is becauseFirefox version >= 47.0.1 and Selenium version < 3.5In order to launch Firefox ver 47 and up, Selenium needs to be updated to >= 3.5and needs geckodriver > 0.16')
        return browser
    ...
    def _generic_make_browser(self, webdriver_type, desired_cap_type, webdriver_remote_url, desired_caps, **kwargs):
        ...
        if not webdriver_remote_url:
            ...
            browser = self._create_remote_web_driver(desired_cap_type, webdriver_remote_url, desired_caps)
        return browser

    def _create_remote_web_driver(self, capabilities_type, webdriver_remote_url, desired_capabilities=None, profile=None):
        ...
        return webdriver.Remote(desired_capabilities=desired_capabilities_object, command_executor=(str(webdriver_remote_url)),
