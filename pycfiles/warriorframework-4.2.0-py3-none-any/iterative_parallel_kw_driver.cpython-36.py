...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/iterative_parallel_kw_driver.py
...
from warrior.WarriorCore.multiprocessing_utils import create_and_start_process_with_queue, get_results_from_queue, update_tc_junit_resultfile
...
def execute_iterative_parallel(step_list, data_repository, tc_status, system_list):
    ...
    for system_name in system_list:
        ...
        args_dict = OrderedDict([('step_list', step_list),
         ...
         (
          'output_q', output_q)])
        process, jobs_list, output_q = create_and_start_process_with_queue(target_module, args_dict, jobs_list, output_q)
    ...
    for result in result_list:
        ...
        system_status = testcase_Utils.compute_status_using_impact(step_status_list, step_impact_list)
        system_resultfile = testcase_Utils.compute_system_resultfile(kw_resultfile_list, data_repository['wt_resultsdir'], system_name)
...
def main(step_list, data_repository, tc_status, system_list):
    ...
    try:
        testcase_status = execute_iterative_parallel(step_list, data_repository, tc_status, system_list)
    except Exception:
