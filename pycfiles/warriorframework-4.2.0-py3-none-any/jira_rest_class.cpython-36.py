...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/jira_rest_class.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 19516 bytes
"""
...
class Jira(object):
    ...
    def check_jira_issue(self, issue_summary):
        ...
        postdata_url = self.server + '/rest/api/2/search/?jql=summary~' + '"' + parsed_summary + '"'
        ...
        if response:
            ...
            print_error('JIRA Error code: ({0}), Error message: ({1})'.format(response.status_code, response.reason))
        return issue_id
    ...
    def update_jira_issue(self, jiraid, status):
        ...
        if not issue_type:
            ...
            if self.jiraproj is not None:
                ...
                sys_elem = xml_Utils.getElementWithTagAttribValueMatch(self.jira_template_xml, 'system', 'name', jiraproj)
            else:
                jiraproj = 'default'
                sys_elem = xml_Utils.getElementWithTagAttribValueMatch(self.jira_template_xml, 'system', jiraproj, 'true')
            if sys_elem is not None and sys_elem is not False:
                ...
                elif type_matched is not None and status in list(type_matched.keys()):
                    if type_matched[status][0]:
                        to_status = type_matched[status][0].lower()
                        oper_status = self.set_jira_issue_status(jiraid, to_status)
                    else:
                        print_error("No value provided for the tag '{0}' under issue_type '{1}' of project '{2}' in jira_config file '{3}'.".format(status, issue_type, jiraproj, 'Tools/jira/jira_config.xml'))
                else:
                    print_error('Cannot find the correct issue type in jira_config file, unable to update jira status')
            else:
                print_error("There is no project with name: '{0}' in the jira config file: '{1}'".format(self.jiraproj, 'Tools/jira/jira_config.xml'))
            return oper_status
    ...
    def create_jira_issue(self, issue_summary, issue_description, issue_type='Bug'):
        ...
        if not existed:
            response = requests.post(postdata_url, auth=(self.auth), headers=headers,
              data=postdata)
            if response:
                resp_dict = response.json()
            ...
            if self.append:
                print_info("Issue '{0}' already exists and the execution logs will be uploaded since the 'append_log' option is set to True in jira config file".format(str(existed)))
                issue_id = existed
            else:
                print_info("Issue '{0}' already exists and the execution logs will not be uploaded since the 'append_log' option is not set to True in jira config file".format(str(existed)))
        return issue_id
    ...
    def upload_logfile_to_jira_issue(self, issue_id, logfile, attachment_name='Log file'):
        ...
        response = requests.post(postdata_url, auth=(self.auth), files=files,
          headers=headers)
        if response:
            status = True
            print_info("{0} - '{1}' uploaded to Jira issue '{2}'".format(attachment_name, logfile_name, issue_id))
        else:
            print_error("Problem attaching logs to Jira issue '{0}'".format(issue_id))
            print_error('JIRA Error code: ({0}), Error message: ({1})'.format(response.status_code, response.reason))
        return status

    def create_issues_from_jsonlist(self, json_file_list, result_xml_file, issue_type='Bug'):
        ...
        for json_file in json_file_list:
            ...
            if issue_summary is not None:
                issue_id = self.create_jira_issue(issue_summary, issue_description, issue_type)
    ...
    def get_issue_description(cls, json_file):
        ...
        tc_name, keyword, step_num, issue_summary = (None, None, None, None)
        step = keyword
        desc = '------------------' + ' Description ' + '------------------' + '\\n'
        p_header = '------------------' + 'Problem Details' + '------------------' + '\\n'
        ...
        if not all([tc_name, keyword, step_num]):
            print_error('all/one of tc_name, keyword, step_num is missing..could not create jira ticket without these details')
        else:
            issue_summary = 'TC-' + str(tc_name).strip() + ':' + 'Keyword-' + str(keyword).strip() + ':' + 'Step{0}.'.format(str(step_num)) + str(step).strip() + '[FAILED]' + '\\n'
            desc = desc + '\\n' + issue_summary + '\\n' + '\\n' + p_header + '\\n'
            for attr in json_data:
        ...
        return (
         issue_summary, desc, str(step_num))
    ...
    def get_jira_system_creds(datafile, system_name, list_my_info):
        ...
        if system_name is not None:
            ...
            element = xml_Utils.getElementWithTagAttribValueMatch(datafile, 'system', 'default', 'true')
        if element is None:
            node_value = xml_Utils.getNodeValuebyAttribute(datafile, 'system', 'name')
            element = xml_Utils.getElementWithTagAttribValueMatch(datafile, 'system', 'name', node_value)
        if element is not None and element is not False:
            ...
            msg = "There is no project with name: '{0}' in the jira config file: '{1}'".format(system_name, 'Tools/jira/jira_config.xml')
            print_warning(msg)
            return False
    ...
    def set_jira_issue_status(self, jiraid, status):
        ...
        if postdata is None:
            ...
            if oper_status is True:
                resp_change_trans = requests.post(issue_trans_url, auth=(self.auth), headers=headers,
                  data=postdata)
                if resp_change_trans and resp_change_trans.status_code == 204:
                    print_info("Successfully changed the Jira issue status to '{0}'".format(trans['to']['name']))
                else:
                    print_error('Error while changing Jira issue status')
                    print_error('JIRA Error code: ({0}), Error message: ({1})'.format(resp_change_trans.status_code, resp_change_trans.reason))
                oper_status = False
        return oper_status