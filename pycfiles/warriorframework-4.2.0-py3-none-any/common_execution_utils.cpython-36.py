...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/common_execution_utils.py
...
"""
import copy, glob, os, random, json
...
def get_step_list(filepath, step_tag, sub_step_tag, randomize=False, loop_tag='Loop'):
    ...
    for child_node in steps:
        if child_node.tag == sub_step_tag:
            ...
            if child_node.tag == loop_tag:
                loop_count = child_node.get('id')
                if loop_count is None:
                    print_error('`id` attribute is mandatory in Loop tag. example : <Loop id="1" file="filename">')
                    return False
                json_file = child_node.get('file')
                if json_file is None:
                    print_error('`file` attribute is mandatory in Loop tag. example : <Loop id="1" file="filename">')
                    return False
                ...
                try:
                    ...
                    with open(filepath, 'r') as (json_handle):
                        ...
                        if not isinstance(json_doc, list):
                            valid_json = False
                            print_error('invalid json format specified,valid format : [{"arg1":"value"}, {"arg2":"value"}]')
                        else:
                            for blob in json_doc:
                                if not isinstance(blob, dict):
                                    valid_json = False
                                    print_error('element is {}. should be dict'.format(type(blob)))
                                    print_error('invalid json format specified,blob should be dict, valid format : [{"arg1":"value"}, {"arg2":"value"}]')

                except ValueError:
                    valid_json = False
                    print_error('The file {0} is not a valid json file'.format(filepath))
                except IOError:
                    valid_json = False
                ...
                for iter_number, _ in enumerate(json_doc):
                    for step_number, loop_step in enumerate(loop_steps):
                        ...
                        copy_step.set('loop_id', 'Loop:{}-Step:{}-Iter:{}'.format(loop_count, step_number + 1, iter_number + 1))
                        ...
                        if arguments is not None:
                            if arguments is not False:
                                for argument in arguments.findall('argument'):
                                    ...
                                    arg_value = Utils.data_Utils.sub_from_loop_json(arg_value, iter_number)
    ...
    if root.tag == 'Project' or root.tag == 'TestSuite':
        ...
        for orig_step in orig_step_list:
            ...
            if '*' not in orig_step_path:
                ...
                orig_step_abspath = Utils.file_Utils.getAbsPath(orig_step_path, os.path.dirname(filepath))
                print_info("Provided {0} path: '{1}' has asterisk(*) in it. All the Warrior XML files matching the given pattern will be executed.".format(sub_step_tag, orig_step_abspath))
                ...
                if step_files:
                    for step_file in step_files:
                        ...
                        print_info("{0}: '{1}' added to the execution list ".format(sub_step_tag, step_file))

                else:
                    print_warning("Asterisk(*) pattern match failed for '{}' due to at least one of the following reasons:\n1. No files matched the given pattern\n2. Invalid testcase path is given\n3. No testcase XMLs are available\nGiven path will be used for the Warrior execution.".format(orig_step_abspath))
                    step_list.append(orig_step)
    ...
    for _, step in enumerate(step_list):
        ...
        if retry_type is not None:
            if retry_value > 0:
                ...
                step_list_with_rmt_retry = append_step_list(step_list_with_rmt_retry, step, retry_value,
                  go_next, mode='retry', tag='count')
        ...
        if rt_value is not None and rt_type is not None:
            if runmode_timer is None or runmode_timer == '':
                ...
                try:
                    ...
                    print_warning('The value for Runmode interval is {0}, please provide seconds to wait in numerals'.format(runmode_timer))
                    runmode_timer = None

            if rt_type not in ('RUF', 'RUP', 'RMT'):
                print_warning("Unsupported value '{0}' provided for 'runmode:type' tag. Supported values : 'ruf, rup & rmt' and these values can not be combined with other values".format(rt_type))
                return (None, 1, None)
            try:
                rt_value = int(rt_value)
                if rt_value < 1:
                    rt_value = 1
                    print_warning("Value provided for 'runmode:value' tag '{0}' is less than '1', using default value 1 for execution".format(rt_value))
            except ValueError:
                print_warning("Unsupported value '{0}' provided for 'runmode:value' tag, please provide an integer, using default value '1' for execution".format(rt_value))
                rt_value = 1

    return (
     rt_type, rt_value, runmode_timer)
...
def get_retry_from_xmlfile(element):
    ...
    if retry_tag is not None and retry_tag.get('type') is not None:
        ...
        if retry_type not in ('if', 'if not'):
            print_warning("Unsupported value '{0}' provided for 'retry:type' tag. Supported values : 'if, if not' ".format(retry_type))
        ...
        if retry_interval.isdigit() is False:
            retry_interval = 5
            print_warning('The value provided for retry:retry_interval is not valid, using default value 5 for execution')
        retry_interval = int(retry_interval)
        if retry_value.isdigit() is False:
            retry_value = 5
            print_warning('The value provided for retry:retry_value is not valid, using default value 5 for execution')
...
def compute_runmode_status(global_status_list, runmode, global_xml):
    ...
    """
    if global_xml.find('runmode').get('status') not in (None, '', 'last_instance',
                                                        'expected'):
        print_warning('Unsupported value for status. Please provide a valid value. Using the Default value for execution')
        global_xml.find('runmode').set('status', '')
    if global_xml.find('runmode').get('status') is None or global_xml.find('runmode').get('status') == '' or runmode.upper() == 'RMT':
        if 'FALSE' in global_status_list or False in global_status_list:
            status_value = False
        else:
            if 'RAN' in global_status_list:
                status_value = 'RAN'
            else:
                if 'ERROR' in global_status_list:
                    status_value = 'ERROR'
                else:
                    status_value = True
        ...
        if global_xml.find('runmode').get('status') == 'last_instance':
            ...
            if global_xml.find('runmode').get('status') == 'expected' and (global_status_list[(-1)] is True or global_status_list[(-1)] == 'TRUE'):
                status_value = True
            else:
                status_value = global_status_list.pop()
    else:
        if runmode.upper() == 'RUF':
            if global_xml.find('runmode').get('status') == 'last_instance':
                status_value = global_status_list.pop()
            elif global_xml.find('runmode').get('status') == 'expected':
                if global_status_list[(-1)] is False or global_status_list[(-1)] == 'FALSE':
                    status_value = True
            else:
                status_value = global_status_list.pop()
        return status_value
...
def compute_status(element, status_list, impact_list, status, impact):
    ...
    if runmode is None:
        ...
        if element.find('runmode').get('status') not in (None, '', 'last_instance',
                                                         'expected'):
            print_warning('Unsupported value for status. Please provide a valid value. Using the Default value for execution')
        ...
        if runmode.upper() == 'RMT':
            ...
            if runmode.upper() == 'RUP':
                if element.find('runmode').get('status') is None or element.find('runmode').get('status') == '':
                    status_list.append(status)
                    impact_list.append(impact)
                else:
                    if (element.find('runmode').get('status') == 'last_instance' or element.find('runmode').get('status') == 'expected') and (status is True or element.find('runmode').get('attempt') == element.find('runmode').get('runmode_val')):
                        status_list.append(status)
                        impact_list.append(impact)
            else:
                if runmode.upper() == 'RUF':
                    if element.find('runmode').get('status') is None or element.find('runmode').get('status') == '':
                        ...
                        if element.find('runmode').get('status') == 'last_instance':
                            if status is False or element.find('runmode').get('attempt') == element.find('runmode').get('runmode_val'):
                                status_list.append(status)
                                impact_list.append(impact)
