...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/database_utils_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 16971 bytes
"""
...
def create_database_connection(server_type='resultservers', dbsystem=None):
    ...
    db_config_xml = os.path.join(Tools.__path__[0], db_config_fpath)
    db_type_list = get_database_details(db_config_xml, server_type, dbsystem, ['dbtype'])
    ...
    db_conn = False
    if db_type == 'mongodb':
        details_dict = get_database_details(db_config_xml, server_type, dbsystem, [
         'host', 'port', 'uri', 'dbname'])
        if details_dict is not False:
            details_dict.update({'server_type':server_type,  'dbsystem':dbsystem})
...
def get_database_details(config_file, server_type, system_name, info_list):
    ...
    if system_name is not None:
        element = xml_Utils.getElementWithTagAttribValueMatch(config_file, system, 'name', system_name)
    else:
        element = xml_Utils.getElementWithTagAttribValueMatch(config_file, system, 'default', 'true')
    if element is None:
        node_value = xml_Utils.getNodeValuebyAttribute(config_file, system, 'name')
        element = xml_Utils.getElementWithTagAttribValueMatch(config_file, system, 'name', node_value)
    if element is not None and element is not False:
        ...
        if system_name is not None:
            msg = "There is no system with name: '{0}' under '{1}' in the database config file: '{2}'".format(system_name, server_type, config_file)
            print_warning(msg)
        else:
            if server_type == 'dataservers':
                msg = 'Value for td_system/var_system tag is not provided in the datafile and there is no system listed in database config file under dataservers block to be used as default'
                print_warning(msg)
        return False


class WMongodb(object):
    __doc__ = ' Class to handle mongodb operations to add html results '
                ...
                if self.conn is not False:
                    if details_dict['server_type'] == 'resultservers':
                        ...
                        if self.dbname in self.conn.database_names():
                            ...
                            print_warning("database - '{}' is not in MongoDB server".format(self.dbname))
                    self.db = self.conn[self.dbname]
            except ImportError:
                print_warning('pymongo module is not installed and Warrior Framework uses it for establishing connection with MongoDB server')
        ...
        try:
            if details_dict:
                if details_dict['uri'] is not False:
                    ...
                    ip = details_dict['host'] if details_dict['host'] is not False else 'localhost'
                    port = details_dict['port'] if details_dict['port'] is not False else '27017'
                    conn = self.pymongo.MongoClient(ip, port)
                conn.server_info()
        except Exception as e:
            conn = False
            print_warning("Unable to establish connection with MongoDB server - '{0}' : {1}".format(details_dict['dbsystem'], e))
    ...
    def get_file_type(self, root):
        ...
        if proj_attrib['name'] != 'customProject_independant_testcase_execution':
            file_type = 'project'
        else:
            suite_elem = root.findall('testsuite')
            if suite_elem[0].attrib['name'] != 'customTestsuite_independant_testcase_execution':
                file_type = 'suite'
            else:
                file_type = 'case'
        return file_type
    ...
    def get_html_results(self, file_type, root):
        ...
        if file_type == 'case':
            case_elem = root.findall('testsuite')[0].findall('testcase')[0]
    ...
    def get_details_from_suite_element(self, suite_elem):
        ...
        for num, case_elem in enumerate(case_list, 1):
            suite_dict['case_results'][str(num)] = self.get_details_from_case_element(case_elem)
    ...
    def get_details_from_project_element(self, proj_elem):
        ...
        for num, suite_elem in enumerate(suite_list, 1):
            proj_dict['suite_results'][str(num)] = self.get_details_from_suite_element(suite_elem)
    ...
    def add_html_result_to_mongodb(self, input_xml):
        ...
        collection.update({'_id': results_dict['name']}, {'$push': {'results': results_dict}})
    ...
    def get_details_from_case_xml_element(self, case_xml_elem):
        ...
        child_list = xml_Utils.getChildElementsListWithSpecificXpath(case_xml_elem, '*')
        child_dict = {}
        for child in child_list:
            value = self.get_details_from_case_xml_element(child)
            if child.tag not in child_dict:
                if child.tag not in ('Note', 'SubStep', 'Requirement', 'argument'):
                    child_dict[child.tag] = [value] if value != {} else child.text
                else:
                    if child.tag == 'argument':
                        child_dict[child.tag] = [value] if value != {} else {child.attrib['name']: child.attrib['value']}
                    else:
                        child_dict[child.tag] = [value] if value != {} else [
                         child.text]
            else:
                if child.tag != 'argument':
                    child_dict[child.tag].append(value if value != {} else child.text)
                else:
                    child_dict[child.tag].update({child.attrib['name']: child.attrib['value']})
    ...
    def add_xml_result_to_mongodb(self, input_xml):
        ...
        collection.update({'_id': case_name}, {'$push': {'results': results_dict}})
    ...
    def get_doc_from_db(self, collection_name, document_name):
        ...
        if collection_name in self.db.collection_names():
            ...
            if testdata.count() == 1:
                ...
                print_warning("Document - '{}' is not found in the MongoDB server".format(document_name))
        else:
            print_warning("collection - '{}' is not found in the MongoDB server".format(collection_name))
        return tddoc
    ...
    def convert_tddict_to_xmlobj(self, root, tddict):
        ...
        for element in tddict:
            if isinstance(tddict[element], dict):
                self.convert_tddict_to_xmlobj(ET.SubElement(root, element), tddict[element])
            else:
                if isinstance(tddict[element], list):
                    for list_elem in tddict[element]:
                        if isinstance(list_elem, dict):
                            self.convert_tddict_to_xmlobj(ET.SubElement(root, element), list_elem)
    ...
    def get_tdblock_as_xmlobj(self, db_details):
        ...
        if td_collection is not None:
            if td_document is not None:
    ...
    def get_globalblock_as_xmlobj(self, db_details):
        ...
        if global_collection is not None:
            if global_document is not None:
                globaldoc = self.get_doc_from_db(global_collection, global_document)
        else:
            globaldoc = False
