...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Actions/CiRegressionActions/ci_regression_actions.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 27727 bytes
"""
...
class CIregressionActions(object):
    ...
    def create_tmp_dir(self):
        ...
        return (True,
         {'parallel_exec_tmp_dir': os.path.join(file_Utils.getDirName(self.logsdir), 'tmp')} if path else False)
    ...
    def check_kw_arg_type_prefix(self, str_value, int_value, float_value, bool_value, list_value, tuple_value, dict_value, file_value):
        ...
        if actual_contents != file_contents:
            pNote('contents of the file {} is <<{}>> which does not match expected <<{}>>'.format(file_value, actual_contents, file_contents), 'error')
            status = False
        return status

    def check_values_from_datafile(self, system_name, strvar, langs, states, currencys, ramspace, configfile, intvar, anotherfile):
        ...
        def check_type(var, varname, datatype):
            ...
            if vartype is not datatype:
                pNote('{} is expected to be {} type, but found to be of {} type'.format(varname, datatype, vartype), 'error')
                status = False
            return status

        status = True
        datafile = Utils.config_Utils.datafile
        tc_filepath = os.path.dirname(data_Utils.get_object_from_datarepository('wt_testcase_filepath'))
        ...
        try:
            if anotherfile.startswith('tag'):
                anotherfile = data_Utils.resolve_argument_value_to_get_tag_value(datafile, system_name, anotherfile)
            else:
        ...
        if type(intvar) is str:
            if intvar.startswith('tag'):
                intvar = data_Utils.resolve_argument_value_to_get_tag_value(datafile, system_name, intvar)
        else:
            status = check_type(intvar, 'intvar', int) and status
        return status

    def check_opt_values_from_datafile(self, langs=[
 'Sanskrit', 'Tamil'], strvar='I am a default variable', states='wtag=states', system_name='sys_wtag', currencys={'USA': 'USD'}, ramspace=False, configfile='../../config_files/check_file_type', intvar=496):
        """Verify the datatype of the value read from the datafile using either
        the tag or wtag feature
        :Argument:
            1. system_name = system name in the datafile
    ...
    def verify_delta(self, delta_key, int_num, float_min_val):
        ...
        if stored_delta:
            if len(stored_delta) != int_num:
                ...
                if not status:
                    pNote('Delta: {} not meet minimum value {}'.format(str(stored_delta), float_min_val))
        return status
