...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/configuration_element_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 15306 bytes
"""
...
"""
import xml.etree.ElementTree as ElementTree, re, decimal
...
class ConfigurationElement(object):
    ...
    def __find_match(self, string):
        ...
        :return:
        """
        text_between_pattern = '(.*?)'
        return re.search('.*(' + re.escape(self.start_pat) + text_between_pattern + re.escape(self.end_pat) + ').*', string)
    ...
    def expand_variables(self, string):
        ...
        :return:
        """
        new_string = self._ConfigurationElement__expand_variables(string)
        return new_string
    ...
    def __expand_variables(self, string):
        ...
        """
        return_value = string
        match = self._ConfigurationElement__find_match(return_value)
    ...
    def get_list_direct(self, string):
        ...
        result = {}
        check = self.end_pat
        match = self._ConfigurationElement__find_match(return_value[:return_value.find(check) + len(check)])
        while match is not None:
            try:
                parsed_value = self._ConfigurationElement__parse_list('{' + match.group(2) + '}')
    ...
    def get_list(self, string):
        ...
        :return:
            ...
            if no list/range found
                return a dict {'False':False}
        """
        new_string = self._ConfigurationElement__get_list(string)
        return new_string
    ...
    def __get_list(self, string):
        ...
        while match is not None:
            try:
                parsed_value = self._ConfigurationElement__parse_list(self.get_value(match.group(2)))
            except (ValueError, TypeError, AttributeError):
                print_error('Invalid list range found')
                return {'Error': False}
            else:
        ...
        if string.startswith('{') and string.endswith('}'):
            ...
            for var in parsed:
                if '..' in var and ':' in var:
                    ...
                    x = self._ConfigurationElement__frange(indexes[0], indexes[1], indexes[2])
                    for index in x:
                        result.append(str(index))

                elif '..' in var and ':' not in var:
                    indexes = re.split('\\.\\.', var)
                    x = self._ConfigurationElement__frange(indexes[0], indexes[1])
                    for index in x:
                        result.append(str(index))
    ...
    def get_value(self, value):
        ...
        try:
            ...
            print_error('Key error node ' + self.name + ' does not have sub node ' + value.split('.')[0] + '. Cannot complete remainder of search for ' + value)
            return
    ...
    def set_value(self, key, value):
        ...
        try:
            ...
            print_error('Key error node ' + self.name + ' does not have sub node ' + key.split('.')[0] + '. Cannot complete remainder of search for ' + key)
    ...
    def parse_data(self, *arg, **kwargs):
        ...
        for xml in arg:
            if 'elem_type' in kwargs:
                if kwargs['elem_type'] == 'xml_object':
                    config_tree = xml
            elif 'root' in kwargs:
                if not kwargs['root']:
                    config_tree = xml
    ...
    def parse_tree(self, node):
        ...
        for child in node:
            try:
                if child.attrib['name'] not in self.children:
                ...
                print_error('No name attribute for node ' + child.tag + '. Tree with root at node ' + child.tag + ' not parsed.')
    ...
    def print_me(self, depth=0):
        ...
        for child in self.children:
            data += '\n' + self._ConfigurationElement__tabs(depth + 1) + child + ': '
