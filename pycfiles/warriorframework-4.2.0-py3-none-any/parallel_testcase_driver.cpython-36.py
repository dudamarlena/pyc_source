...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/parallel_testcase_driver.py
...
def execute_parallel_testcases(testcase_list, suite_repository, data_repository, from_project, tc_parallel=True, auto_defects=False, iter_ts_sys=None):
    ...
    data_repository['wt_junit_object'] = update_ts_junit_resultfile(data_repository['wt_junit_object'], tc_junit_list, data_repository['wt_ts_timestamp'])
    testsuite_status = Utils.testcase_Utils.compute_status_using_impact(tc_status_list, tc_impact_list)
    return testsuite_status


def main(testcase_list, suite_repository, data_repository, from_project, tc_parallel=True, auto_defects=False, iter_ts_sys=None):
    """Executes the list of testcases in parallel
    Computes and returns the testsuite status"""
    try:
        testsuite_status = execute_parallel_testcases(testcase_list, suite_repository, data_repository, from_project, tc_parallel, auto_defects, iter_ts_sys)
    except Exception:
