...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/data_Utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 95735 bytes
"""
...
"""
import os, re, ast, copy, operator as op
...
def get_nc_request_rpc_string(config_datafile, xmlns, request_type, xmlns_tag):
    ...
    try:
        ...
        if data:
            ...
            testcase_Utils.pNote('XMLNS={0} is not found in config file ={1}'.format(xmlns, config_datafile), 'error')
            status = 'error'
...
def get_cred_value_from_elem(element, tag, startdir=''):
    ...
    if chelem is None:
        ...
        if 'wtype' in chelem.attrib:
            value = get_actual_cred_value(chelem.tag, chelem.text, chelem.attrib['wtype'], startdir)
        else:
            value = chelem.text
        return value
...
def get_credentials(datafile, system_name, myInfo=[], tag_name='system', attr_name='name', **kwargs):
    """
    ...
    """
    startdir = os.path.dirname(datafile)
    element = _get_system_or_subsystem(datafile, system_name, tag=tag_name, attr=attr_name)
    value = False
        ...
        if len(myInfo) == 0:
            for child in element:
                ...
                if 'wtype' in child.attrib:
                    val = get_actual_cred_value(child.tag, child.text, child.attrib['wtype'], startdir)
                ...
                if cred_value is None:
                    ...
                    if child_list:
                        ...
                        for child in child_list:
                            ...
                            if 'wtype' in child.attrib:
                                cred_value[child.tag] = get_actual_cred_value(child.tag, child.text, child.attrib['wtype'], startdir)
...
def _get_system_or_subsystem(datafile, system_name, tag='system', attr='name'):
    ...
    system_node = xml_Utils.getElementWithTagAttribValueMatch(datafile, tag, attr, system_name)
    msg1 = 'system={0} is not found in datafile={1}'.format(system_name, datafile)
    msg2 = 'The subsystem={0} is not found under the system={1} in datafile={2}'.format(subsystem_name, system_name, datafile)
    if system_node is not False:
        if system_node is not None:
            if subsystem_name is None:
                element = system_node
            else:
                subsystem = xml_Utils.getElementWithTagAttribValueMatch(system_node, 'subsystem', attr, subsystem_name)
                if subsystem is not False:
...
def get_var_configfile(datafile, system_name):
    ...
    status = _check_tag_or_attr_exists(datafile, system_name, 'var_config_file', 'system')
...
def get_child_tag_value_list(datafile, system_name, child_list=[], *args):
    ...
    element = xml_Utils.getElementWithTagAttribValueMatch(datafile, 'system', 'name', system_name)
    if element is not None and element is not False:
        tag_list = []
        value_list = []
        for child in child_list:
...
def get_command_details_from_testdata(testdatafile, varconfigfile=None, **attr):
    ...
    if isinstance(testdatafile, dict):
        print_info("Resolving testdata details from DB system - '{}'".format(testdatafile.get('td_system')))
        db_td_obj = database_utils_class.create_database_connection('dataservers', testdatafile.get('td_system'))
        root = db_td_obj.get_tdblock_as_xmlobj(testdatafile)
        if testdatafile.get('global_system') is not None:
            print_info("Resolving testdata-global block from DB system - '{}'".format(testdatafile.get('global_system')))
            db_tdglobal_obj = database_utils_class.create_database_connection('dataservers', testdatafile.get('global_system'))
    ...
    if isinstance(varconfigfile, dict):
        print_info("Resolving varconfig details from DB system - '{}'".format(varconfigfile.get('var_system')))
        db_var_obj = database_utils_class.create_database_connection('dataservers', varconfigfile.get('var_system'))
    ...
    for testdata in root.findall('testdata'):
        ...
        if execute_req:
            if exec_flag:
                ...
                td_obj = TestData()
                details_dict = td_obj.varsub_varconfig_substitutions(details_dict,
                  vc_file=None, var_sub=var_sub, start_pat=start_pat, end_pat=end_pat)
                ...
                if iter_type == 'per_td_block':
                    details_dict, cmd_loc_list = td_iter_obj.repeat_per_td_block(details_dict, cmd_loc_list)
                    details_dict = td_iter_obj.arrange_per_td_block(details_dict, cmd_loc_list)
                details_dict = td_obj.varsub_varconfig_substitutions(details_dict,
                  vc_file=varconfigfile, var_sub=None, start_pat=start_pat,
                  end_pat=end_pat)
    ...
    if not_found == len(root.findall('testdata')):
        print_warning('There are no rows with execute=yes and title={0}, row={1} in testdata {2} '.format(title, row, testdatafile))
...
def _get_mapping_details(global_obj, vfylist):
    ...
    """
    map_list = []
    g_verify = global_obj.find('verifications') if global_obj is not None else None
    ...
    if keys is None:
        ...
        for key in keylist:
            if testdata.find(key) is not None:
                ...
                if not key_found:
                    print_error("There is no pattern element for key '{}', please check".format(key))
...
@mocked
def _get_cmd_details(testdata, global_obj, system_name, varconfigfile, var_sub=None):
    """Get the command details from testdata file
    as a details dictionary"""
    details_dict = {}
    verifyparams = [
     'verify_text_list', 'verify_context_list', 'operator_list',
    ...
    for param, attrib in list(cmd_params.items()):
        ...
        if param == 'verify_list':
            ...
            if param in verifyparams:
                vfylist = details_dict['verify_list']
                resultant_list = _get_verification_details(testdata, global_obj, vfylist, attrib)
            else:
                if param == 'verify_on_list':
                    sys_list = details_dict['sys_list']
                    vfylist = details_dict['verify_list']
                    resultant_list = _get_verification_details(testdata, global_obj, vfylist, attrib, system_name, sys_list)
                else:
                    if param == 'verify_map_list':
                        vfylist = details_dict['verify_list']
                        vfylist, maplist = _get_mapping_details(global_obj, vfylist)
                        resultant_list = maplist
                    else:
                        if param == 'resp_key_list':
        ...
        if param == 'sys_list':
            resultant_list = string_Utils.sub_from_varconfig(varconfigfile, resultant_list)
...
def _get_cmdparams_list(testdata, global_obj, cmd_attrib):
    ...
    global_cmd_params = global_obj.find('command_params') if global_obj is not None else None
    global_exempt_list = ['send', 'monitor']
    resultant_list = []
    value_list = xml_Utils.getNodeListbyAttribute(testdata, 'command', cmd_attrib)
    for value in value_list:
        default_value = global_cmd_params.get(cmd_attrib) if global_cmd_params is not None else None
        if cmd_attrib not in global_exempt_list:
            value = default_value if value is None or value == '' else value
        else:
            if cmd_attrib in testdata.attrib:
                if cmd_attrib == 'monitor':
...
def _get_verification_details(testdata, global_obj, verify_list, cmd_attrib, system_name=None, sys_list=None):
        ...
        if verify is None or verify == '':
            ...
            if verify is not None:
                ...
                for element in new_verify_sublist:
                    ...
                    value = xml_Utils.get_attributevalue_from_directchildnode(testdata, element, cmd_attrib)
                    default_value = av_fromdc(g_verify, element, cmd_attrib) if g_verify is not None else False
                    if value is False:
                        value = default_value
                        if value is False:
                            print_warning('could not find specific or global value  for verification node={0}'.format(element))
                    if value is None or value == '':
                        value = 'yes' if cmd_attrib == 'found' else value
                        if cmd_attrib == 'verify_on':
    ...
    for system in sys_list:
        if system and system != system_name:
            ...
            if isinstance(vc_file, dict):
                print_info("Resolving varconfig details from DB system - '{}'".format(vc_file.get('var_system')))
                db_var_obj = database_utils_class.create_database_connection('dataservers', vc_file.get('var_system'))
...
def _get_pattern_list(testdata, global_obj, pattern='start'):
    ...
    """
    global_var_pattern = global_obj.find('variable_pattern') if global_obj is not None else None
    if pattern == 'start':
        resultant = '${'
    else:
        resultant = '}'
    if testdata.get(pattern + '_pattern') is not None:
        ...
        if global_var_pattern is not None:
            if global_var_pattern.get(pattern + '_pattern') is not None:
...
def get_exec_flag(testdata, title, row):
    ...
    if title and row:
        if testdata.get('title', None) == title:
            if process_row == row:
                exec_flag = True
    else:
        if title:
            ...
            if row:
                if process_row == row:
                    if testdata.get('title', None) is None:
                        exec_flag = True
                if process_row == row:
                    if testdata.get('title', None) == 'none':
                        exec_flag = True
                if process_row == row:
                    if testdata.get('title', None) == '':
                        exec_flag = True
            else:
                exec_flag = True
    return exec_flag
    ...
    for verify in verify_list:
        if verify is None:
            ...
            if verify is not None:
                ...
                verify_sublist = verify.split(',')
                for element in verify_sublist:
                    verify_text = xml_Utils.get_attributevalue_from_directchildnode(testdata, element, 'search')
                    verify_context = xml_Utils.get_attributevalue_from_directchildnode(testdata, element, 'found')
                    if verify_text is False:
                        print_error('could not find childnode:{0} under testdata:{1}'.format(element, testdata))
...
def verify_resp_across_sys(match_list, context_list, command, response, varconfigfile=None, verify_on_list=None, verify_list=None, remote_resp_dict=None, endprompt='', verify_group=None, log='true'):
    ...
    for i in range(0, len(verify_on_list)):
        for j in range(0, len(verify_on_list[i])):
            ...
            try:
                ...
                tmp_status = verify_cmd_response([match_list[i]], [context_list[i]], command, data, verify_on_list[i][j], varconfigfile, endprompt, verify_group, log)
...
def get_no_impact_logic(context_str):
    ...
    value = {'YES:NOIMPACT':(True, 'YES'), 
     ...
     'NO':(False, 'No'), 
     'N:NOIMPACT':(True, 'No'), 
     'N':(False, 'No')}.get(context_str.upper(), False)
    return value
...
def verify_cmd_response(match_list, context_list, command, response, verify_on_system, varconfigfile=None, endprompt='', verify_group=None, log='true'):
    ...
    """
    err_msg = 'Incorrect or no value provided for verification search/found, check the verification data provided for the command. Command result will be marked as ERROR'
    if varconfigfile:
    ...
    for i in range(0, len(match_list)):
        ...
        if context_list[i] and match_list[i]:
            ...
            if match_object:
                ...
                msg = "Found '{0}' in response to '{1}' on {2} & 'Found' tag is set to '{3}', so the {4}"
                pattern_match = True
                ...
                if cond_value:
                    ...
                    if len(grps) == 1:
                        ...
                        status = verify_relation(actual_value, cond_value, operator, cond_type)
                        msg += '\n{0} comparison: {1} {2} {3} '.format(cond_type or 'str', actual_value, operator, cond_value)
                        msg += 'succeeded' if status else 'failed'
                    else:
                        msg += 'pattern to compare should have exactly one group to match with the condition value: pattern failed'
                        nogroup = True
                        status = False
                else:
                    status = True
                ...
                match = match_list[i]
                msg = "Did not find '{0}' in response to '{1}' on {2} & 'Found' tag is set to '{3}' so the {4}"
                status = False
            if found is status:
            ...
            if log != 'false':
                if pattern_match is True and found is True:
                    print_info(msg.format(match_list[i], command, verify_on_system, 'Yes', 'verification Passed'))
                elif pattern_match is True and found is False:
                    print_debug(msg.format(match_list[i], command, verify_on_system, 'No', 'verification Failed'))
                elif pattern_match is False and found is True:
                    print_debug(msg.format(match_list[i], command, verify_on_system, 'Yes', 'verification Failed'))
                else:
                    if pattern_match is False and found is False:
                        print_info(msg.format(match_list[i], command, verify_on_system, 'No', 'verification Passed'))
        elif context_list[i]:
            if match_list[i] == '':
                noiimpact, found = get_no_impact_logic(context_list[i])
                found = string_Utils.conv_str_to_bool(found)
                ...
                msg += "found from command '{0}' on {2} :[{1}]:".format(command, verification_text, verify_on_system)
        ...
        if value:
            try:
                if result == 'ERROR' or result == 'EXCEPTION':
                    ...
                    if not comp_funcs[comparison](value, exp):
                        result = 'FALSE'
                        if type(value) != type(exp):
                            print_warning("The expected value '{0}' is of {1} and data_repository value '{2}' is of {3}".format(exp, type(exp), value, type(value)))
                        else:
                            print_warning("The key, value pair '{0}:{1}' present in the data_repository doesn't satisfy the expected value: '{2}' & condition: '{3}'".format(key, value, expected, comparison))
                    else:
                        print_info("The key, value pair '{0}:{1}' present in the  data_repository satisfies the expected type & condition '{2}:{3}'".format(key, value, data_type, comparison))
            except Exception as e:
...
def verify_arith_exp(expression, expected, comparison='eq', repo_key='exp_op'):
    ...
    def power(a, b):
        ...
        if any(abs(n) > 1000 for n in [a, b]):
            raise Exception("ValueError: Power operation is not supported on values higher than 1000: '{0}, {1}'".format(a, b))
        return op.pow(a, b)
    ...
    def eval_exp(parsed_exp):
        if isinstance(parsed_exp, ast.Num):
            ...
            if isinstance(parsed_exp, ast.BinOp):
                return operators[type(parsed_exp.op)](eval_exp(parsed_exp.left), eval_exp(parsed_exp.right))
            if isinstance(parsed_exp, ast.UnaryOp):
    ...
    comp_funcs = {'eq':lambda x, y: x == y, 
     ...
     'le':lambda x, y: x <= y}
    if comparison not in comp_funcs:
        print_error('Valid comparisons are {}'.format('/'.join(list(comp_funcs.keys()))))
        status = 'ERROR'
    if status is True:
        comp_result = comp_funcs[comparison](expression_ouput, expected)
        if comp_result is True:
            print_info("Expression output satisfies the given condition: '{0} {1} {2}'".format(expression_ouput, comparison, expected))
        else:
            status = False
            print_info("Expression output does not satisfy the given condition: '{0} {1} {2}'".format(expression_ouput, comparison, expected))
    output_dict = get_object_from_datarepository('verify_arith_exp') if get_object_from_datarepository('verify_arith_exp') else {}
    output_dict[repo_key] = expression_ouput
    print_info('Expression output: {0} is stored in a Key: {1} of Warrior data_repository'.format(expression_ouput, 'verify_arith_exp.' + repo_key))
    update_datarepository({'verify_arith_exp': output_dict})
    return status


def verify_resp_inorder(match_list, context_list, command, response, varconfigfile=None, verify_on_list=None, verify_list=None, remote_resp_dict=None, verify_group=None):
    ...
    """
    msg = "In-order verification requested for the command: '{0}' ".format(command)
    testcase_Utils.pNote(msg, 'debug')
    if varconfigfile:
        if varconfigfile is not None:
            match_list = string_Utils.sub_from_varconfig(varconfigfile, match_list)
    ...
    resp_details_dict = _get_resp_details(match_list, context_list, verify_on_list, verify_list, remote_resp_dict)
    for system in resp_details_dict:
        if resp_details_dict[system]:
            sys_status = verify_inorder_cmd_response(match_list, verify_list, system, command, resp_details_dict[system], verify_group)
        else:
            pNote('Verification can not be done for the system : {0}'.format(system), 'error')
            sys_status = 'ERROR'
...
def _get_resp_details(match_list, context_list, verify_on_list, verify_list, remote_resp_dict):
    ...
    resp_details_dict = _update_search_index(match_list, verify_list, remote_resp_dict, resp_details_dict)
    resp_details_dict = _get_resp_order(context_list, verify_list, resp_details_dict)
    return resp_details_dict


def _update_search_index(match_list, verify_list, remote_resp_dict, resp_details_dict):
    ...
    for system in resp_details_dict:
        ...
        for i, tag in enumerate(verify_list):
            if i in resp_details_dict[system] and resp_details_dict[system][i].get('verify') == tag and resp_details_dict[system][i].get('match') is True:
        ...
        for i in range(len(verify_list)):
            ...
            if resp_details_dict[system][i]['found'] is True:
                if resp_details_dict[system][i]['match'] is True:
                    index_status = _validate_index_value(i, index_list, context_list)
                    if index_status is True:
                        verify_order = True
            else:
                if resp_details_dict[system][i]['found'] is False:
...
@mocked
def verify_inorder_cmd_response(match_list, verify_list, system, command, verify_dict, verify_group=None):
        ...
        if i in verify_dict and verify_dict[i].get('verify') == tag:
            ...
            if match is True:
                ...
                if verify_order is True:
                    verify_status = True
                    if found is True:
                        msg = "Found '{0}' in response to '{1}' on {2} and '{0}' is in the correct order".format(match_list[i], command, system)
                        if verify_group:
                            cond_value = verify_group[1][i]
                            operator = verify_group[0][i]
                            cond_type = verify_group[2][i]
                            if cond_value and actual_value:
                                verify_status = verify_relation(actual_value, cond_value, operator, cond_type)
                                msg += '\n{0} comparison: {1} {2} {3} '.format(cond_type or 'str', actual_value, operator, cond_value)
                                if verify_status:
                                    msg += 'succeeded'
                                else:
                                ...
                                if cond_value:
                                    verify_status = False
                                    msg += '\npattern to compare should have exactly one group to match with the condition value: pattern failed'
                    else:
                        if verify_group[1][i] and not actual_value:
                            verify_status = False
                            msg += '\npattern to compare should have exactly one group to match with the condition value: pattern failed'
                else:
                    msg = "Found '{0}' in response to '{1}' on {2} but '{0}' is not in the specified location as expected".format(match_list[i], command, system)
            else:
                verify_status = False
                verify_order_list.append(tag)
                if found is True:
                    msg = "Found '{0}' in response to '{1}' on {2} but '{0}' not in the correct order".format(match_list[i], command, system)
                elif verify_group[1][i]:
                    if not actual_value:
                        msg = "Found '{0}' in response to '{1}' on {2} in the correct order, but \npattern to compare should have exactly one group to match with the condition value: pattern failed".format(match_list[i], command, system)
                    else:
                        msg = "Found '{0}' in response to '{1}' on {2}, but '{0}' is not expected to be found in the location where it is found now".format(match_list[i], command, system)
                else:
                    msg = "Did not find '{0}' in response to '{1}' on {2}".format(match_list[i], command, system)
                    verify_status = True if verify_order is True else False
                if noimpact is True:
                    verify_status = True
                    pNote('Noimpact was requested on the below verification, hence the failure would not impact command status')
                pNote(msg, 'debug')
        else:
            verify_status = 'ERROR'
            pNote("Verification of '{0}' in response to '{1}' on {2} can not be done".format(match_list[i], command, system), 'error')
    ...
    pNote("Search string(s) is/are found in the following order for the command '{0}': '{1}' on {2}".format(command, rcv_all_resp_string, system), 'debug')
    if verify_order_list:
        verify_order_str = ','.join(verify_order_list)
        pNote('Following verification string(s) - {0} not in the expected order on {1}'.format(verify_order_str, system), 'debug')
    else:
        pNote('Search string(s) is/are found in the expected order on {0}'.format(system), 'debug')
    return status
...
def resolve_argument_value_to_get_tag_value(datafile, system_name, element_value_in_argument):
    ...
    if element_value_in_argument.startswith('tag='):
        ...
        if tag_name:
            ...
            if system_name_list == [] or system_name_list is None or system_name_list is False:
                ...
                for system in system_name_list:
                    if system.attrib['name'] == system_name:
                        ...
                        if node_list == [] or node_list is None or node_list is False:
                            print_error('The tag value: {0} is not defined in the datafile:{1}'.format(tag_name, datafile))
                            return False
                        else:
                ...
                return element_value_in_argument
        print_error('The value for arg {0} is not defined in the case'.format(element_value_in_argument))
        return element_value_in_argument
    else:
        return element_value_in_argument
...
def get_netconf_data(datafile, config_name, **kwargs):
    ...
    config_data = get_credentials(datafile, config_name, attr_name='name', tag_name='config')
    for element in kwargs:
        if kwargs[element] is not None:
        ...
        if isinstance(credentials[tag], str) or credentials[tag] is False:
            ...
            if abspath:
                if os.path.isfile(abspath):
                    ...
                    print_warning("File '{0}' provided for tag '{1}' does not exist".format(abspath, tag))
...
def subst_var_patterns_by_prefix(raw_value, start_pattern='${', end_pattern='}', prefix='ENV', iter_number=None):
    ...
    """
    error_msg1 = 'Could not find any %s variable {0!r} corresponding to {1!r} provided in input data/testdata/loopjson file.\nWill default to None' % prefix
    error_msg2 = 'Unable to substitute %s variable {0!r} corresponding to {1!r} provided in input data/testdata file.\nThe value processed till now is {2!r} whose evaluation resulted in {3!r} exception' % prefix
    if type(raw_value) == dict:
        for k in raw_value:
            value = raw_value[k]
            extracted_var = string_Utils.return_quote(str(value), start_pattern, end_pattern)
            extracted_var = [string for string in extracted_var if prefix in string]
            if len(extracted_var) > 0:
                for string in extracted_var:
                    try:
                        if isinstance(raw_value[k], str):
                            raw_value[k] = raw_value[k].replace(start_pattern + string + end_pattern, get_var_by_string_prefix(string, iter_number))
                        else:
                            if isinstance(raw_value[k], (list, dict)):
                                raw_value[k] = str(raw_value[k]).replace(start_pattern + string + end_pattern, get_var_by_string_prefix(string, iter_number))
                                raw_value[k] = ast.literal_eval(raw_value[k])
                            else:
                                print_error('Unsupported format - ' + error_msg2.format(string, value))
                        ...
                        if isinstance(raw_value[k], str):
                            ...
                            if isinstance(raw_value[k], (list, dict)):
                                ...
                                if search_obj:
                                    raw_value[k] = ast.literal_eval(str(raw_value[k]).replace(search_obj.group(), 'None'))
                    except SyntaxError:
                        tuc_obj = TestcaseUtils()
                        print_info('Cannot convert below value into correct format, removing non-printable characters, will attempt conversion again')
                        print_info('<<{}>>'.format(raw_value[k]))
                        try:
                            raw_value[k] = tuc_obj.rem_nonprintable_ctrl_chars(raw_value[k])
                            raw_value[k] = ast.literal_eval(raw_value[k])
                        except Exception as exc:
                            print_error('Error - ' + error_msg2.format(string, value, raw_value[k], exc))
        ...
        if isinstance(raw_value, str):
            ...
            if extracted_var != []:
                for string in extracted_var:
                    try:
                        raw_value = raw_value.replace(start_pattern + string + end_pattern, get_var_by_string_prefix(string, iter_number))
                    except KeyError:
...
def sub_from_env_var(raw_value, start_pattern='${', end_pattern='}'):
    ...
    return subst_var_patterns_by_prefix(raw_value, start_pattern, end_pattern, 'ENV')


def sub_from_data_repo(raw_value, start_pattern='${', end_pattern='}'):
    """wrapper function for subst_var_patterns_by_prefix"""
    return subst_var_patterns_by_prefix(raw_value, start_pattern, end_pattern, 'REPO')


def sub_from_loop_json(raw_value, iter_number, start_pattern='${', end_pattern='}'):
    """wrapper function for subst_var_patterns_by_prefix"""
    return subst_var_patterns_by_prefix(raw_value, start_pattern, end_pattern, prefix='LOOP',
...
def substitute_var_patterns(raw_value, start_pattern='${', end_pattern='}'):
    ...
    """
    prefixes = {'ENV':(
      'environment', lambda var: os.environ[var]), 
     'REPO':(
      'data repository', get_object_from_datarepository)}
    error_msg = 'Could not find any {0} variable {1!r} corresponding to {2!r} provided in input data/testdata file.\nWill default to None'
    if raw_value is None:
        return raw_value
    if isinstance(raw_value, str):
        extracted_var = string_Utils.return_quote(raw_value, start_pattern, end_pattern)
        for string in extracted_var:
            ...
            if prefix in prefixes:
                try:
                    ...
                    print_error(error_msg.format(prefixes[prefix][0], string, raw_value))

                if val:
                    raw_value = raw_value.replace(start_pattern + string + end_pattern, val)
    ...
    if isinstance(raw_value, list):
        ...
        if isinstance(raw_value, dict):
            for key in raw_value:
                raw_value[key] = substitute_var_patterns(raw_value[key], start_pattern, end_pattern)

            return raw_value
        print_error('Unsupported format - raw_value should either be a string, list or dictionary')
        print_error('raw_value: #{}# and its type is {}'.format(raw_value, type(raw_value)))
        return raw_value


def process_subsystem_list--- This code section failed: ---

 L.1802         0  LOAD_GLOBAL              xml_Utils
                2  LOAD_ATTR                getElementWithTagAttribValueMatch
                4  LOAD_FAST                'datafile'
                6  LOAD_STR                 'system'

 L.1803         8  LOAD_STR                 'name'
               10  LOAD_FAST                'system_name'
               12  CALL_FUNCTION_4       4  '4 positional arguments'
               14  STORE_FAST               'system'

 L.1804        16  LOAD_FAST                'system'
               18  LOAD_CONST               None
               ...
               30  POP_JUMP_IF_FALSE   200  'to 200'

 L.1805        32  LOAD_FAST                'subsystem'
               34  LOAD_CONST               None
               36  COMPARE_OP               is-not
               38  POP_JUMP_IF_FALSE   194  'to 194'
               40  LOAD_FAST                'subsystem'
               42  LOAD_CONST               False
               44  COMPARE_OP               is-not
               46  POP_JUMP_IF_FALSE   194  'to 194'

 L.1806        48  LOAD_FAST                'subsystem'
               50  LOAD_ATTR                split
               52  LOAD_STR                 ','
               54  CALL_FUNCTION_1       1  '1 positional argument'
               56  STORE_FAST               'subsystem'

 L.1808        58  BUILD_LIST_0          0 
               60  STORE_FAST               'strip_subsystem_list'

 L.1809        62  SETUP_LOOP           94  'to 94'
               64  LOAD_FAST                'subsystem'
               66  GET_ITER         
               68  FOR_ITER             92  'to 92'
               70  STORE_FAST               'name'

 L.1810        72  LOAD_FAST                'name'
               74  LOAD_ATTR                strip
               76  CALL_FUNCTION_0       0  '0 positional arguments'
               78  STORE_FAST               'name'

 L.1811        80  LOAD_FAST                'strip_subsystem_list'
               82  LOAD_ATTR                append
               84  LOAD_FAST                'name'
               86  CALL_FUNCTION_1       1  '1 positional argument'
               88  POP_TOP          
               90  JUMP_BACK            68  'to 68'
               92  POP_BLOCK        
             94_0  COME_FROM_LOOP       62  '62'

 L.1813        94  BUILD_LIST_0          0 
               96  STORE_DEREF              'subsystem_list'

 L.1815        98  LOAD_CLOSURE             'subsystem_list'
              100  BUILD_TUPLE_1         1 
              102  LOAD_LISTCOMP            '<code_object <listcomp>>'
              104  LOAD_STR                 'process_subsystem_list.<locals>.<listcomp>'
              106  MAKE_FUNCTION_8          'closure'
              108  LOAD_FAST                'strip_subsystem_list'
              110  GET_ITER         
              112  CALL_FUNCTION_1       1  '1 positional argument'
              114  POP_TOP          

 L.1818       116  LOAD_CONST               False
              118  STORE_FAST               'all_subsys_flag'

 L.1819       120  SETUP_LOOP          156  'to 156'
              122  LOAD_DEREF               'subsystem_list'
              124  GET_ITER         
              126  FOR_ITER            154  'to 154'
              128  STORE_FAST               'name'

 L.1820       130  LOAD_GLOBAL              re
              132  LOAD_ATTR                search
              134  LOAD_FAST                'name'
              136  LOAD_ATTR                lower
              138  CALL_FUNCTION_0       0  '0 positional arguments'
              140  LOAD_STR                 'all'
              142  CALL_FUNCTION_2       2  '2 positional arguments'
              144  POP_JUMP_IF_FALSE   126  'to 126'

 L.1821       146  LOAD_CONST               True
              148  STORE_FAST               'all_subsys_flag'

 L.1822       150  BREAK_LOOP       
            152_0  COME_FROM           144  '144'
              152  JUMP_BACK           126  'to 126'
              154  POP_BLOCK        
            156_0  COME_FROM_LOOP      120  '120'

 L.1823       156  LOAD_FAST                'all_subsys_flag'
              158  LOAD_CONST               True
              160  COMPARE_OP               is
              162  POP_JUMP_IF_FALSE   180  'to 180'

 L.1825       164  LOAD_GLOBAL              xml_Utils
              166  LOAD_ATTR                getNodeListbyAttribute
              168  LOAD_FAST                'system'

 L.1826       170  LOAD_STR                 'subsystem'

 L.1827       172  LOAD_STR                 'name'
              174  CALL_FUNCTION_3       3  '3 positional arguments'
              176  STORE_DEREF              'subsystem_list'
              178  JUMP_ABSOLUTE       198  'to 198'
              180  ELSE                     '192'

 L.1829       180  LOAD_DEREF               'subsystem_list'
              182  BUILD_LIST_0          0 
              184  COMPARE_OP               ==
              186  POP_JUMP_IF_FALSE   198  'to 198'

 L.1830       188  LOAD_CONST               None
              190  STORE_DEREF              'subsystem_list'
              192  JUMP_ABSOLUTE       204  'to 204'
            194_0  COME_FROM            38  '38'

 L.1832       194  LOAD_CONST               None
              196  STORE_DEREF              'subsystem_list'
            198_0  COME_FROM           186  '186'
              198  JUMP_FORWARD        204  'to 204'
            200_0  COME_FROM            22  '22'

 L.1834       200  LOAD_CONST               None
              202  STORE_DEREF              'subsystem_list'
            204_0  COME_FROM           198  '198'

 L.1835       204  LOAD_DEREF               'subsystem_list'
              206  RETURN_VALUE     
               -1  RETURN_LAST      
...
def split_system_subsystem(system_name):
    ...
    subsystem_name = None if len(new_system_name) == 1 else new_system_name[1].split(']')[0]
    return (
...
def get_td_vc(datafile, system_name, td_tag, vc_tag):
    ...
    abspaths = get_filepath_from_system(datafile, system_name, testdata_tag, varconfig_tag)
    testdata = abspaths[0]
    varconfigfile = abspaths[1]
    return (
     testdata, varconfigfile)
...
def get_nc_config_string(config_datafile, config_name, var_configfile=None):
    ...
    status = True
    try:
        data = xml_Utils.get_element_by_attribute(config_datafile, 'config_data', 'name', config_name)
        if data:
            ...
            if not filepath_list:
                testcase_Utils.pNote('neither <config> nor a file containing <config> provided for the config_data = {0} in config file = {1}'.format(config_name, config_datafile), 'error')
                status = 'error'
        else:
            testcase_Utils.pNote('config_data={0} is not found in config file ={1}'.format(config_name, config_datafile), 'error')
            status = 'error'
    except IOError as err:
        testcase_Utils.pNote('File does not exist: {0}'.format(err), 'error')
        status = 'error'
...
def _check_tag_or_attr_exists(datafile, system_name, cnode, system='system'):
    ...
    if element is not None:
        if element is not False:
            if element.get(cnode, None) is not None or element.find(cnode) is not None:
                status = True
    return status


def get_default_ecf_and_et(arguments_dict, current_datafile, current_browser, name_tuple=('element_config_file', 'element_tag'), def_name_tuple=('DEF_ecf', 'DEF_et')):
    """
        ...
        if name_tuple[j] in arguments_dict:
            if arguments_dict[name_tuple[j]] is None:
                ...
                if arguments_dict[name_tuple[j]].startswith('tag') and '=' in arguments_dict[name_tuple[j]]:
                    ...
                    if '=' in arguments_dict[name_tuple[j]]:
                        temp_list = arguments_dict[name_tuple[j]].split('=')
...
def get_all_system_or_subsystem(datafile, system_name=None):
    ...
    if system_name is None:
        ...
        return xml_Utils.getElementListWithSpecificXpath(datafile, "./system[@name='%s']/*" % system_name)
...
def get_iteration_syslist(system_node_list, system_name_list):
    ...
    return (
     iteration_sysnamelist, iteration_sysnodelist)
