...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/defects_driver.py
...
class DefectsDriver(object):
    ...
    def create_failing_kw_json(self):
        ...
        if len(keyword_list) == 0:
            ...
            for keyword in tree.findall('Keyword'):
                fail_list = []
                if keyword.find('KeywordStatus').text.upper() in ('FAIL', 'EXCEPTION',
                                                                  'ERROR'):
                ...
                if len(fail_list) > 0:
                    json_file_name = '{0}{1}{2}_{3}.json'.format(self.defectsdir, os.sep, tc_name, kw_resultfile_nameonly)
    ...
    def create_jira_issues(self, defects_json_list):
        ...
        if self.w_jira_object is not None:
            if self.w_jira_object.status is not False:
                issue_list = self.w_jira_object.create_issues_from_jsonlist(defects_json_list, (self.xmlfile),
                  issue_type='Bug')
                ...
                print_error('could not obtain jira credentials!! check jira config file ')
        return issue_list
    ...
    def attach_logs_to_jira_issues(self, issue):
        ...
        time.sleep(3)
        self.w_jira_object.upload_logfile_to_jira_issue(issue, self.resultfile, 'full_resultfile.xml')
        time.sleep(3)
        self.w_jira_object.upload_logfile_to_jira_issue(issue, self.testcase_filepath, 'tescase.xml')
        time.sleep(3)