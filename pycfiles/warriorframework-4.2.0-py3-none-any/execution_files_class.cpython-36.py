...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/WarriorCore/Classes/execution_files_class.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 15921 bytes
"""
...
class ExecFilesClass(object):
    ...
    def create_ws_execution(self):
        ...
        try:
            ...
            print_error("Unable to create directory/file(s) under Warriorspace '{0}', hence aborting".format(ws_execution))
    ...
    def get_result_files(self):
        ...
        if self.res_startdir is not None:
            ...
            if self.res_startdir is None:
                results_location = xml_Utils.getChildTextbyParentTag(self.filepath, 'Details', 'Resultsdir')
                ...
                if results_location is None or results_location is False or str(results_location).strip() == '':
                    results_execdir = self.create_def_exec_dir()
                    rfile = self.get_exec_file_by_type('Results', results_execdir)
                elif results_location is not None and results_location is not False:
                    results_location_rel = str(results_location).strip()
                    results_location = file_Utils.getAbsPath(results_location_rel, os.path.dirname(self.filepath))
    ...
    def get_log_files(self):
        ...
        if self.logs_startdir is not None:
            ...
            if self.logs_startdir is None:
                ...
                if logs_location is None or logs_location is False or str(logs_location).strip() == '':
                    logs_execdir = self.create_def_exec_dir()
                    logfile = self.get_exec_file_by_type('Logs', logs_execdir)
                elif logs_location is not None:
                    if logs_location is not False:
                        logs_location_rel = str(logs_location).strip()
                        logs_location = file_Utils.getAbsPath(logs_location_rel, os.path.dirname(self.filepath))
                        results_location_rel = str(results_location).strip()
                        results_location = file_Utils.getAbsPath(results_location_rel, os.path.dirname(self.filepath))
    ...
    def checkdir_create_file(self, inpdir, dirname, colocate=False):
        ...
        dir_status = file_Utils.check_and_create_dir(inpdir)
        if dir_status:
            try:
                if colocate:
        ...
        if dir_status is False:
            print_warning("Creating directory/file(s) in provided path {0} failed. \n Hence Warrior Framework's default directory structure will be used for this execution.".format(inpdir))
            execdir = self.create_def_exec_dir()
            rfile = self.get_exec_file_by_type(dirname, execdir)
        return (
         rfile, execdir)
    ...
    def get_exec_file_by_type(self, dirname, exec_dir):
        ...
        if self.filetype == 'tc':
            ...
            if self.filetype == 'ts' or self.filetype == 'proj':
    ...
    def check_get_datatype(self, datafile):
        ...
        if str(datafile).upper().strip() == 'NO_DATA':
            ...
            if data_type is None or data_type is False or str(data_type).strip() == '':
                data_type = 'CUSTOM'
            else:
                if data_type is not None:
                    if data_type is not False:
                        ...
                        if data_type.lower() not in supported_values:
                            print_warning("unsupported value '{0}' provided for data_type, supported values are '{1}' and case-insensitive".format(data_type, supported_values))
            ...
            if run_type is not None:
                if run_type is not False:
                    ...
                    if run_type.lower() not in supported_values:
                        print_warning("unsupported value '{0}' provided for run_type,supported values are '{1}' and case-insensitive".format(run_type, supported_values))
