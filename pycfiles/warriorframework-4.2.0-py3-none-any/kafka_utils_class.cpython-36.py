...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/kafka_utils_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 11186 bytes
"""
...
class WarriorKafkaConsumer:
    ...
    def subscribe_to_topics(self, topics, **kwargs):
        ...
        try:
            self.kafka_consumer.subscribe(topics=topics, pattern=pattern,
              listener=listener)
            result = True
        except KafkaError as exc:
...
class WarriorKafkaClient:
    ...
    def create_topics(self, topic_sets, **kwargs):
        ...
        new_topics = [NewTopic(name=(tup[0]), num_partitions=(tup[1]), replication_factor=(tup[2])) for tup in topic_sets]
        print_info('creating topics')
        try:
            self.kafka_client.create_topics(new_topics=new_topics, timeout_ms=timeout,
    ...
    def delete_topics(self, topics, timeout=None):
        ...
        try:
            self.kafka_client.delete_topics(topics=topics, timeout_ms=timeout)
            result = True
        except KafkaError as exc:
    ...
    def create_partitions_in_topic(self, partitions, **kwargs):
        ...
        try:
            self.kafka_client.create_partitions(topic_partitions=topic_partitions, timeout_ms=timeout,
              validate_only=validate)
            result = True
        except KafkaError as exc:
