...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/email_utils.py
# Compiled at: 2020-02-05 00:22:48
# Size of source mod 2**32: 10173 bytes
"""
...
def set_params_send_email(addsubject, data_repository, files, mail_on):
    ...
    if compress.upper().startswith('Y'):
        print_info('compress attribute in w_settings.xml is set to Yes. So, all the email attachments will be compressed.')
        zip_files = []
...
def get_email_params(mail_on='per_execution'):
    ...
    if setting_elem is not None:
        ...
        if mail_on in ('first_failure', 'every_failure') or mail_on == 'per_execution' and mail_on_list != []:
        ...
        if 'compress' in setting_elem.keys():
            ...
            compress = 'No'
    return (
     smtp_host, sender, receivers, subject, compress)
...
def construct_mail_body(exec_type, abs_filepath, logs_dir, results_dir):
    ...
    """
    junit_result_file = os.path.join(results_dir, file_Utils.getNameOnly(file_Utils.getFileName(abs_filepath))) + '_junit.xml'
    ...
    suite_tc, body = ('', '')
    body_arg = '<html><body><p><b>{0}</b>{1}</p><p><b>Logs directory:</b>{2}</p><p><b>Results directory:</b>{3}</p><p><b>Execution Summary:</b></p><table cellspacing="10" cellpadding="0"><tr><td><b>Type</b></td><td><b>Name</b></td><td><b>Status</b></td><td><b>Path</b></td></tr>'.format(exec_type, abs_filepath, logs_dir, results_dir)
    if exec_type == 'Project: ':
        ...
        if exec_type == 'Test Suite: ' or exec_type == 'Test Case: ':
            for value in suite_tc_sum:
                suite_tc = suite_tc + '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td></tr>\n'.format(value[0], value[1], value[2], value[3])
...
def compose_send_email(exec_type, abs_filepath, logs_dir, results_dir, result, mail_on='per_execution'):
    ...
    :Arguments:
        ...
        6. mail_on(optional) - it is to specify when to send an email
           Supported options below:
                ...
                (3) every_failure
    """
    resultconverted = {'True':'Pass', 
     'False':'Fail',  'ERROR':'Error',  'EXCEPTION':'Exception', 
     'RAN':'Ran'}.get(str(result))
    ...
    report_attachment = results_dir + os.sep + file_Utils.getNameOnly(file_Utils.getFileName(abs_filepath)) + '.html'
    ...
    if not smtp_host:
        ...
        for attach_file in files or []:
            with open(attach_file, 'rb') as (fil):
                ...
                part.add_header('Content-Disposition', 'attachment;filename= %s' % basename(attach_file))
                message.attach(part)
        ...
        try:
            ...
            pNote('Error occurred while sending email, check w_settings.xmlconfiguration for correct smtp host, receiver email address etc.')