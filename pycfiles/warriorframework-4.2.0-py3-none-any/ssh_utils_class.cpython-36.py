...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/ClassUtils/ssh_utils_class.py
# Compiled at: 2020-02-05 00:22:47
# Size of source mod 2**32: 15056 bytes
"""
...
class SSHComm(object):
    ...
    def import_param(self):
        ...
        try:
            ...
            print_error('Paramiko module is not installedPlease install requests module toperform any activities related to SSH Communication')
        else:
            self.param = paramiko

    def ssh_con(self, retries=1, interval=1, timeout=60, verify_keys=False, invoke_shell=False):
        ...
        for attempt in range(retries):
            print_debug('Attempt{} connecting to {}'.format(attempt + 1, self.target))
            try:
                if not verify_keys:
                    self.sshobj.set_missing_host_key_policy(self.param.AutoAddPolicy())
                else:
                    self.sshobj.connect((self.target), (self.port), (self.uid), (self.pid),
                      timeout=timeout, look_for_keys=verify_keys)
                    if invoke_shell:
    ...
    def ssh_exec(self, command, timeout=None, get_pty=True):
        ...
        out = ''
        stdin, stdout, stderr = self.sshobj.exec_command(command, timeout=timeout,
          get_pty=get_pty)
        stdin.close()
        if self.logfile is not None:
            self.log.write('********************')
    ...
    def verify_response(self, command, exp_string, fail_resp, timeout=None):
        ...
        if out and exp_string in out and fail_resp not in out:
            ...
            if out:
                if exp_string not in out or fail_resp in out:
    ...
    def copy_file(self, remotepath, localpath, filename):
        ...
        print_info('Copying {} from {} to {}'.format(filename, remotepath, localpath))
        if filename:
            ...
            if result:
                if result is not 'ERROR':
                    if verify_text_list is not None:
                        result = data_Utils.verify_cmd_response(verify_text_list, verify_context_list, command, response, varconfigfile)
            command_status = {True:'Pass', 
             False:'Fail',  'ERROR':'ERROR'}.get(result)
    ...
    def download_remote_file_sftp(self, remotepath, localpath, filename, remotehost, username, password, port=22):
    ...
    def connect_target_via_host(self, target, user, auth, invoke_shell=False, log=None):
        ...
        try:
            ...
            channel = ssh_transport.open_channel('direct-tcpip', (target, 22), ('localhost',
                                                                                0))
            print_info('Connecting to target: {}'.format(target))
            target_session.sshobj.set_missing_host_key_policy(self.param.AutoAddPolicy())
            target_session.sshobj.connect(target, port=22, username=user, password=auth,
              sock=channel)
