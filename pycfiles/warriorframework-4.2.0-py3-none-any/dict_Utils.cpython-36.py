...
# [GCC 8.4.0]
# Embedded file name: /data/users/snayak/WARRIOR-4.2.0/warriorframework_py3/warrior/Framework/Utils/dict_Utils.py
    ...
    if isinstance(element, dict):
        ...
        if element is not None and element is not False and element != '':
            ...
            for i in range(0, len(element)):
                ...
                if element[i] is not None and element[i] is not False and element[i] != '':
                    ...
                    if len(element[i]) < 2:
                        if element[i][0] != '':
                            if element[i][0] is not None:
                                if element[i][0] is not False:
                                    pNote('{0} does not have a corresponding value. Hence, discarded.'.format(element[i][0]), 'error')
                        else:
                            pNote('Key Value undefined for data number {0}!'.format(i + 1), 'error')
                    elif element[i][0] != '':
                        if element[i][0] is not None:
                            if element[i][0] is not False:
                                if element[i][1] != '':
                                    if element[i][1] is not None:
                                        if element[i][1] is not False:
                        ...
                        if element[i][1] is not None:
                            if element[i][1] is not False:
                                pNote('{0} does not have a corresponding key. Hence, discarded.'.format(element[i][1]), 'error')
                    else:
                        pNote('Key Value undefined for data number {0}!'.format(i + 1), 'error')
                else:
                    pNote('Key Value undefined for data number {0}!'.format(i + 1), 'error')
