...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cdt/causality/pairwise/Jarfo_model/estimator.py
...
class CauseEffectEstimatorSymmetric(BaseEstimator):
    ...
    def transform(self, X):
        return (
         self.classifier_left.transform(X), self.classifier_right.transform(X))
...
class CauseEffectEstimatorID(BaseEstimator):
    ...
    def predict(self, X):
        ...
        elif not predictions_independence.min() >= 0:
...
class CauseEffectSystemCombination(BaseEstimator):
    ...
    def __init__(self, extractor=extract_features, weights=None, symmetrize=True, njobs=-1):
        ...
        self.systems = [
         CauseEffectEstimatorID(features_direction=(self.features),
           features_independence=(self.features),
           regressor=GradientBoostingClassifier,
           params=gbc_params,
           symmetrize=symmetrize),
         CauseEffectEstimatorSymmetric(features=(self.features),
           regressor=GradientBoostingClassifier,
           params=gbc_params,
           symmetrize=symmetrize),
         CauseEffectEstimatorOneStep(features=(self.features),
           regressor=GradientBoostingClassifier,
           params=gbc_params,
           symmetrize=symmetrize)]
