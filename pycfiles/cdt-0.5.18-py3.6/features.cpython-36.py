...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cdt/causality/pairwise/Jarfo_model/features.py
# Compiled at: 2020-02-05 06:32:12
# Size of source mod 2**32: 32771 bytes
"""
Feature extraction
...
def discrete_probability(x, tx, ffactor, maxdev):
    ...
    try:
        ...
        if isinstance(x, list):
            return Counter(np.array(x).flat)
        return Counter(x.flat)
...
def discretized_sequences(x, tx, y, ty, ffactor=3, maxdev=3):
    return (
     discretized_sequence(x, tx, ffactor, maxdev), discretized_sequence(y, ty, ffactor, maxdev))
    ...
    for a in cx:
        if cx[a] > minc:
            ...
            if not numerical(ty):
                ...
                if count_unique(y) > len_discretized_values(y, ty, ffactor, maxdev):
...
def determine_type(dffeature, categorical_threshold=70):
    ...
    def type_row(feature, categorical_threshold):
        ...
        if nunique_values < 3:
            ...
            if nunique_values < categorical_threshold:
                return CATEGORICAL
            return NUMERICAL
...
def extract_features(X, features=all_features, y=None, njobs=-1):
    ...
    while True:
        ...
        new_features_list = [(complete_feature_name(feature_name, column_names), column_names, extractor) for feature_name, column_names, extractor in features if can_be_extracted(feature_name, column_names)]
