...
# [GCC 8.4.0]
# Embedded file name: /home/paul/src/scspell-0.1/scspell_lib/_corpus.py
...
from _util import *
DICT_TYPE_NATURAL = 'NATURAL'
DICT_TYPE_FILETYPE = 'FILETYPE'
DICT_TYPE_FILEID = 'FILEID'
file_id_regex = re.compile('[a-zA-Z0-9_\\-]+')
...
class CorporaFile(object):
    ...
    def _parse_corpus(self, lines, offset):
        ...
        if dict_type == DICT_TYPE_NATURAL:
            ...
            mutter(VERBOSITY_DEBUG, '(Loaded natural language dictionary with %u tokens.)' % len(tokens))
            return offset
    ...
    def _parse_header_line(self, line, line_num):
        ...
        if dict_type == DICT_TYPE_FILETYPE:
            try:
                ...
                return (
                 dict_type, (descr, extensions))
        if dict_type == DICT_TYPE_FILEID:
            if file_id_regex.match(metadata) is None:
                raise ParsingError('%s metadata string "%s" on line %u is not a valid file ID.' % DICT_TYPE_FILEID, metadata, line_num)
