...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\middleware.py
# Compiled at: 2011-07-14 11:58:55
...
__all__ = [
 'ToscaWidgetsMiddleware', 'make_middleware']
...
class ToscaWidgetsMiddleware(object):
    ...
    def __init__(self, application, host_framework, prefix='/toscawidgets', inject_resources=True, serve_resources=True, require_once=False, render_filter=None, callback_security_default=always_deny):
...
def _extract_args(args, prefix, adapters={}):
    l = len(prefix)
    nop = lambda v: v
    return dict(((k[l:], adapters.get(k[l:], nop)(v)) for (k, v) in args.iteritems() if k.startswith(prefix)))
...
def make_middleware(app, config=None, **kw):
    ...
    **Available options:**
    ...
    toscawidgets.middleware.*
        ...
        when instantiating it. See its docstrings for details.
    ...
    middleware_args = _extract_args(config, 'toscawidgets.middleware.', {'inject_resources': asbool, 'serve_resources': asbool, 'require_once': asbool})
    hf_args = _extract_args(config, 'toscawidgets.framework.', {'enable_runtime_checks': asbool})
    app = ToscaWidgetsMiddleware(app, host_framework=host_framework(**hf_args), **middleware_args)
