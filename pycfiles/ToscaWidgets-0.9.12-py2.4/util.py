...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\util.py
# Compiled at: 2010-07-25 06:35:38
import inspect, re, warnings, time, weakref
from itertools import izip
__all__ = [
 'OrderedSet', 'RequestLocalDescriptor', 'lazystring', 'asbool', 'LRUCache', 'disable_runtime_checks']
...
def callable_wo_args(obj):
    ...
    if callable(obj) and (callable_cache.is_function(obj) or callable_cache.is_method(obj)):
        ...
        return args is not None and len(args) == arg_length or defaults is not None and len(args) == len(defaults)
    return False
...
def install_framework(force=False):
    ...
    if not hasattr(tw, 'framework') or force:
        ...
        dummy_framework.start_request({'SCRIPT_NAME': '', 'toscawidgets.prefix': '/toscawidgets', 'paste.registry': dummy_registry})
        tw.framework = StackedObjectProxy(dummy_framework, name='ToscaWidgetsFramework')
...
class MultipleReplacer(object):
    ...
    def __init__(self, dct, options=0):
        self._raw_regexp = ('|').join(('(%s)' % k for k in dct.keys()))
