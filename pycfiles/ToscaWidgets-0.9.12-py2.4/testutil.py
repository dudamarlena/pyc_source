...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\testutil.py
# Compiled at: 2011-07-14 12:03:55
import sys, doctest
...
__all__ = [
 'RequireMixin', 'WidgetMixin', 'WidgetTestCase', 'WidgetRequireTestCase', 'get_doctest_suite']
        ...
        try:
            ...
            def dummy_run(self, result=None):
                if not self._message_displayed:
                    print >> sys.stderr, 'Skipping all tests in %s due to missing requirements: %r' % (name, reqs)
    ...
    def assertNotInOutput(self, strings, *args, **kw):
        ...
        if isinstance(strings, basestring):
            strings = [
             strings]
        for s in strings:
            self.failUnless(s not in output, '%s\n\n%r in output' % (output, s))
...
def get_doctest_suite(doctest_files, doctest_modules):
    ...
    suite = doctest.DocFileSuite(*tuple(doctest_files), **{'optionflags': doctest.ELLIPSIS | doctest.NORMALIZE_WHITESPACE, 'module_relative': False})
    for mod in doctest_modules:
        try:
            suite.addTests(doctest.DocTestSuite(mod, **{'optionflags': doctest.ELLIPSIS | doctest.NORMALIZE_WHITESPACE}))
        except ValueError:
            pass
