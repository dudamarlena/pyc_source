...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\mods\base.py
# Compiled at: 2011-07-14 12:06:10
...
class HostFramework(object):
    ...
    **Constructor's arguments:**
    ...
    default_view
       ...
       templates. It's used to determine what conversion is necessary when
    ...
    enable_runtime_checks
       ...
       Disabling this option can significantly reduce Widget initialization
       time.
    ...
    def __init__(self, engines=None, default_view='toscawidgets', translator=lambda s: s, enable_runtime_checks=True, default_engine=None, aggregation_config=None):
        if engines is None:
    ...
    def pop_resources(self):
        ...
        if resources and 'head' in resources:
            ...
            if self.aggregated_js_mapping:
                self._replace_resources_with_aggregates(resources, self.aggregated_js_mapping, JSLink)
            if self.aggregated_css_mapping:
                self._replace_resources_with_aggregates(resources, self.aggregated_css_mapping, CSSLink)
        return resources
    ...
    def _compute_mapping(self, configs, mapping, kind):
        ...
        for config in configs:
            ...
            if (
             modname, filename) in aggregate_mappings:
    ...
    def _replace_resources_with_aggregates(self, resources, mapping, kind):
        ...
        for resource in resources['head']:
            if not isinstance(resource, kind):
                ...
                continue
            desc = (
             resource.modname, resource.active_filename())
            if desc in mapping:
