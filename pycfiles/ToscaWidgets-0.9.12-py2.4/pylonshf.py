...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\mods\pylonshf.py
# Compiled at: 2011-07-14 12:09:45
import logging
...
__all__ = [
 'PylonsHostFramework', 'validate', 'render', 'valid']
...
class PylonsHostFramework(HostFramework):
    ...
    def __init__(self, engines=None, default_view='mako', translator=ugettext, template_paths=[], engine_options=None):
        if engines is None:
        ...
        super(PylonsHostFramework, self).__init__(engines, default_view, translator)
        return


def validate(form=None, validators=None, error_handler=None, post_only=True, state_factory=None):
    ...
    def wrapper(func, self, *args, **kwargs):
        ...
        if not valid(self, form=form, validators=validators, post_only=post_only, state_factory=state_factory):
            if error_handler:
...
def valid(controller, form=None, validators=None, post_only=True, state_factory=None):
            ...
            for (field, validator) in validators.iteritems():
                try:
                    if pylons_version > pylons_097_version:
                        controller.form_result[field] = validator.to_python(field.decode() or None, state)
                    else:
                        controller.form_result[field] = validator.to_python(decoded[field] or None, state)
...
def _render_func_wrapper(func):
    ...
    def wrapper(*args, **kargs):
        import warnings
        warnings.warn('%s is deprecated since collecting resources from widgets at pylons.c.w is no longer needed to inject them in the page.' % func.func_name, DeprecationWarning, 2)
        from tw import framework
