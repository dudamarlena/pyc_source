...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\resources.py
# Compiled at: 2011-07-14 12:03:22
import threading, logging, os, errno, string, heapq, mimetypes
from operator import itemgetter
from itertools import chain, imap
from pkg_resources import resource_filename, working_set, Requirement, resource_stream
import tw
from tw.core.base import Widget
from tw.core.util import Enum, OrderedSet, RequestLocalDescriptor, asbool
from webob import Request, Response
__all__ = [
 'Resource', 'Link', 'JSLink', 'CSSLink', 'Source', 'JSSource', 'CSSSource', 'locations', 'merge_resources', 'retrieve_resources', 'JSFunctionCalls', 'IECSSLink', 'IECSSSource', 'IEJSLink', 'IEJSSource', 'JSMixin', 'CSSMixin', 'AggregatedJSLink', 'AggregatedCSSLink']
...
class ResourcesApp(object):
    ...
    def _is_registered(self, webdir, dirname, variant):
        for (old_wd, old_dn) in self.iter_variant(variant):
            if webdir == old_wd:
                if dirname != old_dn:
                    raise ValueError('%s is already registered for %s' % (webdir, old_dn))
                else:
                    return True
    ...
    def __iter__(self):
        return chain(self.iter_variant(self.ACTIVE_VARIANT), self.iter_variant(self.DEFAULT_VARIANT))
    ...
    def get_stream_type_encoding(self, req):
        ...
        try:
            ...
            if ct in ('application/javascript', 'application/x-javascript', 'text/javascript'):
...
class Link(Resource):
    ...
    **Attention**: Widgets automatically use the framework to register the
    ...
    """
    __module__ = __name__
    params = {'link': 'Use this to specify an external link. If provided this will be used as-is as the resources URL. modname and filename will be ignored.', 'filename': "The relative path (from the module's or distribution's path) of the file the Link should link to.This is either a string, or a dictionary. If it's the latter, it **must** contain a key **%s**. The keys are called a `variant`, and they are used to make the resource refer to various incarntations of itself. Most commonly, these are normal, min(ified), packed or debug. If you configure the :class:`tw.core.resources.ResourcesApp` to deliver a specific variant, it will be served instead of the normal one." % ResourcesApp.DEFAULT_VARIANT, 'modname': 'The module that contains the Widget declaration. If not given, it defaults to the name of the module where the Link is declared. Must be an existent module name. You can also pass a pkg_resources.Requirement instance to point to the root of an egg distribution.'}
    _link = None
    filename = None
    modname = None
...
class CSSLink(Link, CSSMixin):
    ...
    """
    __module__ = __name__
    template = '<link rel="stylesheet" type="text/css" href="$link" media="$media" />'
...
class JSLink(Link, JSMixin):
    ...
    params = dict(require_once='Use this turn require_once-protection on or off. Default is to use the configured application  wide setting')
    USE_CONFIG = object()
...
class Source(Resource):
    ...
    """
    __module__ = __name__
    params = {'src': "A string with the source to include between the resource's tags. Can also be a template for string.Template. Any attribute listed at ``source_vars`` will be fetched from the instance or from the kw args to ``display`` or ``render`` into a dictionary to provide values to fill in."}
    ...
    def update_params(self, d):
        ...
        src = d.get('src')
        if src:
            source_vars = dict((v for v in [ (k, getattr(self, k, None)) for k in self.source_vars ]))
            source_vars.update((v for v in d.iteritems() if v[0] in self.source_vars))
...
class JSDynamicFunctionCalls(JSFunctionCalls):
    ...
    def update_params(self, d):
        ...
        d.src = '\n%s\n' % ('\n').join(map(str, chain(self.call_getter(self.location), d.function_calls)))
...
class DynamicCalls(object):
    ...
    def __init__(self):
        self.call_widgets = {}
        for l in locations:
            w = JSDynamicFunctionCalls('dynamic_js_calls_for_' + l, location=l, call_getter=self._get_calls_for_request)
