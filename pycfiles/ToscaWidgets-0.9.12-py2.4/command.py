...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\command.py
# Compiled at: 2011-07-14 11:55:50
import errno, re, operator, shutil, sys, os, tempfile, subprocess
try:
...
class archive_tw_resources(Command):
    ...
    """
    __module__ = __name__
    description = 'Copies ToscaWidgets static resources into a directory where a fast web-server can serve them.'
    user_options = [
     ('output=', 'o', "Output directory. If it doesn't exist it will be created."), ('force', 'f', 'If output dir exists, it will be ovewritten'), ('onepass', None, 'If given, yuicompressor will only be called once for each kind of file with a all files together and then separated back into smaller files'), ('compresslevel=', 'c', 'Compression level: 0) for no compression (default). 1) for js-minification. 2) for js & css compression'), ('yuicompressor=', None, 'Name of the yuicompressor jar.'), ('distributions=', 'd', "List of widget dists. to include resources from (dependencies will be handled recursively). Note that these distributions need to define a 'toscawidgets.widgets' 'widgets' entrypoint pointing to a a module where resources are located."), ('requireonce', 'r', 'Surround the gathered Javascript with a require_once-guard.')]
    IGNORED_NAMES = [
     '.svn']
    NO_VARIANT = object()
    ...
    def run(self):
        ...
        if os.path.exists(self.output) and not self.force:
            print >> sys.stderr, 'Destination dir %s exists. ' % self.output + 'Use -f to ovewrite'
            return
        if self.compresslevel > 0 and not os.path.exists(self.yuicompressor):
            print >> sys.stderr, 'Could not find YUICompressor at ' + self.yuicompressor
            return
        ...
        if os.path.exists(self.output):
            self.execute(shutil.rmtree, (self.output,), 'Deleting old output dir %s' % self.output)
        self.execute(os.makedirs, (self.output,), 'Creating output dir')
        final_dest = os.path.join(self.output, registry.prefix.strip('/'))
        self.execute(shutil.move, (tempdir, final_dest), 'Moving build to %s' % final_dest)

    def _load_widgets(self, distribution):
        try:
            requires = [ r.project_name for r in pkg_resources.get_distribution(distribution).requires() ]
            map(self._load_widgets, requires)
            mod = pkg_resources.load_entry_point(distribution, 'toscawidgets.widgets', 'widgets')
    ...
    def _copy_resources(self):
        ...
        for (webdir, dirname) in registry:
            ...
            modname = parts[0]
            fname = ('/').join(parts[1:])
            self.execute(self._copy_resource_tree, (modname, fname), 'Copying %s recursively into %s' % (dirname, self.writer.base))
        ...
        try:
            for name in pkg_resources.resource_listdir(modname, fname):
                ...
                if pkg_resources.resource_isdir(modname, name):
                    self.execute(self._copy_resource_tree, (modname, name), 'Recursing into ' + rel_name)
                else:
                    ...
                    self.execute(self.writer.write_file, (stream, filename), 'Processing ' + filename)
...
class aggregate_tw_resources(Command):
    ...
    user_options = [
     ('output=', 'o', "Output directory. If it doesn't exist it will be created."), ('onepass', None, 'If given, yuicompressor will only be called once for each kind of file with a all files together and then separated back into smaller files'), ('compresslevel=', 'c', 'Compression level: 0) for no compression (default). 1) for js-minification. 2) for js & css compression'), ('yuicompressor=', None, 'Name of the yuicompressor jar.'), ('distributions=', 'd', "List of widget dists. to include resources from (dependencies will be handled recursively). Note that these distributions need to define a 'toscawidgets.widgets' 'widgets' entrypoint pointing to a a module where resources are located."), ('kind=', 'k', 'The kind, either js or css'), ('variant=', 'p', 'The registry variant to fetch the data from. If this option is used,only the selected kind of resources is gathered. The resulting fileset is processed and concatenated to a large file.'), ('package=', 'm', "If gathering a variant, this option can be used to filter the output for a given package. The name of the package can also contain path-elements.So you can group together e.g. 'foo.bar/public/javascript/subdir'."), ('rewrite', 'r', 'Use the CSS-rewriter to rewrite relative url/image-references before producing the output. To make this actually work, make user you have the AbletonCSSProcessor installed.')]
    IGNORED_NAMES = [
     '.svn']
    NO_VARIANT = object()
...
class CompressingWriter(FileWriter):
    ...
    def compress(self, stream, path):
        ...
        if typ not in ('css', 'js'):
            return stream
        args = [
         'java', '-jar', self.cmd.yuicompressor, '--type', typ]
