...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\tw\core\resource_injector.py
# Compiled at: 2011-07-14 12:01:24
"""
.. testoutput::
   :hide:
...
__all__ = [
 'inject_resources', 'injector_middleware']
...
class _ResourceInjector(MultipleReplacer):
    ...
    def __init__(self):
        return MultipleReplacer.__init__(self, {'<head.*?>': self._injector_for_location('head'), '</head.*?>': self._injector_for_location('headbottom', False), '<body.*?>': self._injector_for_location('bodytop'), '</body.*?>': self._injector_for_location('bodybottom', False)}, re.I | re.M)
...
_charset_re = re.compile('charset\\s*=\\s*(?P<charset>[\\w-]+)([^\\>])*', re.I | re.M)
