...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/grid.py
# Compiled at: 2015-12-25 03:29:51
"""
 Grid
...
class Grid:
    ...
    def __init__(self, SCMS, table, fields, q=False, menu=False, extra=False, well=False, path=False, edit=False, search=False, delete=False, new=False, view=False, smart=False):
        ...
        self.html = self.app.xhtml('table,div,h2,p,a,ul,li,thead,th,tr,td,span,form,input,button')
        self.y = 20
    ...
    def query(self):
        ...
        if self.app.r.vars.zoeken and self.cansearch:
            ...
            if [ k for k in smartsearch if k in zoek.lower() ] and self.smart:
                try:
                    if self.q:
                        res = self.app.db.smart_query(self.select_q(), zoek).select(limitby=(
                         a, b))
                    else:
                        res = self.app.db.smart_query([self.table], zoek).select(limitby=(
                         a, b))
                    self.tot = len(res)
                except:
                    res = False
                ...
                try:
                    if self.q:
                        res = self.app.db.smart_query(self.select_q(), term).select(limitby=(
                         a, b))
                    else:
                        res = self.app.db.smart_query([self.table], term).select(limitby=(
                         a, b))
                    self.tot = len(res)
                except:
                    res = False
            ...
            return res
        if self.q:
            return self.app.db(self.select_q()).select(limitby=(a, b), *self.query_fields())
        else:
            return self.app.db().select(limitby=(a, b), *self.query_fields())
    ...
    def message(self, text, classname='danger'):
        ...
        self.html.div.add(self.html.span.xml() + ' ' + self.app.T(text, l='nl'), _class=classname, _id='gridxid')
        return self.html.div.xml()

    def search_form(self):
        if not self.cansearch:
            return ''
        javascrip = "$('.submit_search').click(submit_search);function submit_search(){if($('#gridsearchval').val()){location.replace('" + self.app.url + "?zoeken=' + $('#gridsearchval').val());}}"
        self.app.javascript(code=javascrip)
        if self.smart:
            javascrip = "$('.smart_search').click(smart_search);function smart_search(){alert(this);}"
            self.app.javascript(code=javascrip)
        self.html.span.add('&times;', _class='closer close_gridsrchf')
        self.html.input.add('', _type='search', name='search', _value='', _id='gridsearchval', _placeholder=self.app.T('Zoeken...'))
        ...
        self.html.form.add(self.html.span.xml() + self.html.div.xml(), _method='get', _action='#', _class='well row hide', _id='gridsrchf')
    ...
    def menu(self, message=False, classname='error'):
        ...
        if self.app.r.vars.bekijken or self.app.r.vars.bewerk:
            self.html.a.add(' ' + self.app.T('Terug', l='nl'), _href=bl, _class=btnb)
        ...
        if self.app.r.vars.show and self.canedit:
            ...
            self.html.a.add(' ' + self.app.T('Bewerk'), _href=editl, _class=btnnew)
    ...
    def show(self):
        if self.app.r.vars.bewerk:
            ...
            if self.app.r.vars.weergeven:
                return self.show_row()
            if self.app.r.vars.xsearch:
                return self.show_results()
            if self.app.r.vars.verwijder:
    ...
    def show_results(self):
        res = self.app.db.smart_query([self.table], self.app.r.vars.zoeken).select()
        return str(res)
        ...
        if template == '404':
            for v in rij:
                if rij[str(v)] and str(v) not in ('update_record', 'delete_record'):
                    self.html.li.add('<em>' + str(v) + '</em><br />' + (str(rij[str(v)]) if rij[str(v)] else str(rij[str(v)])[(0,
                                                                                                                               255)]))
    ...
    def gridtable(self):
        javascrip = "$('.del_item').click(delete_item);function delete_item(e){var item=m(this.id).getAttribute('data-del');m('delete_ok').href=item;$('#deleter').fadeIn();}"
        self.app.javascript(code=javascrip)
    ...
    def pagination(self, delen):
        if delen > 1:
            ...
            self.html.div.add(self.html.ul.xml(), _class='pagination pull-right')
            p = self.html.div.xml()
