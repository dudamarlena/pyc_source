...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/simplecms_server.py
# Compiled at: 2016-07-15 07:35:13
"""
 Base
...
"""
import os, sys, gc, hashlib, datetime, string
from random import choice
from simplecms.template import TemplateParser
from simplecms.helpers import echo, serve_file, default_config, HTML, parse_qsl, timeparts, fetch_url, Tools, tag, extension, cookiedate, literal_evil
...
class Simplecms():
    ...
    def database(self, cdn=False):
        if not self.db and not cdn and not hasattr(self.db, '_tables'):
            ...
            if not self.r.env.get('APPENGINE_RUNTIME', False):
                ...
                self.db = DAL(cdn or self.m.cdn_string, folder=self.dbpath + '/database', migrate=self.m.migrate, fake_migrate=self.m.fake_migrate)
                self.gae = False
            else:
                self.db = DAL(self.m.gae_cdn_string)
                self.gae = True
        else:
            return DAL(cdn or self.m.cdn_string, folder=self.dbpath + '/databases', migrate=False)
    ...
    def build_request(self, environ):
        ...
        self.r.wsgi.multiprocess = environ.get('wsgi.multiprocess', False)
        ...
        if environ.get('REQUEST_METHOD', '') in ('POST', 'PUT') and int(environ.get('CONTENT_LENGTH', False)):
        ...
        oldkey = userkey.copy()
        tijdplus = self.r.now.year + self.r.now.day + self.r.now.hour + self.r.now.month
        tijdmin = self.r.now.year + self.r.now.day + (self.r.now.hour - 1) + self.r.now.month
        ...
        self.cookie_value = self.encrypt(self.cookie_salt + _(authkey[16:32]))
        oldcookie_value = self.encrypt(self.cookie_salt + _(oldauthkey[16:32]))
    ...
    def load_class(self, module, path='controllers', blanc_env=False, func=False, forcepath=False):
        ...
        try:
            ...
            exec serve_file(ophalen) in blanc_env
    ...
    def environment(self):
        ...
        """
        return dict(request=self.r, T=self.T, SCMS=self, db=self.db, time=self.time, date=self.date, model=self.model, javascript=self.javascript, prettydate=self.prettydate, css=self.css, xhtml=self.xhtml, segment=self.segment, segment_int=self.segment_int, segment_inarray=self.segment_inarray, segment_if=self.segment_if, timediff=self.timediff, grid=self.grid, user=self.user, vorm=self.vorm, post=self.r.post, validate=self.validate, now=self.r.now, get=self.r.vars, field=Field, view=self.view, alt_view=self.alt_view, encrypt=self.encrypt, memory=self.m)

    def delete_cookie(self, duur=-95000, cookie=False):
        self.status = '307 Temporary Redirect'
        if self.isadmin:
            waard = self.admin_cookie + ' = 1;' + self.cookie + ' = 1 ;Path = /; Expires =' + cookiedate(duur) + ';'
        else:
            waard = self.cookie + ' = 1; Path = /; Expires =' + cookiedate(duur) + ';'
        self.headers = [
    ...
    def set_cookie(self, userlevel=1, sduur=1800):
        if userlevel < 100:
            waard = self.cookie + '=' + self.cookie_value + '; Path = /; Expires =' + cookiedate(sduur) + '; HttpOnly; Session;'
        else:
            waard = self.admin_cookie + '=' + self.admin_cookie_value + '; Path = /; Expires =' + cookiedate(sduur) + '; HttpOnly; Session;'
        self.headers = [
         ('Content-type', 'text/html'),
         (
          'Set-Cookie', str(waard))]
    ...
    def create_page(self):
        ...
        if self.loggedin and self.r.arg(0) in ('logout', 'uitloggen'):
            ...
            return [
             self.status, self.headers, 'redirect']
        else:
            if self.r.arg(0) and self.r.arg(0) == self.m.secure:
                ...
                if aanvraag and aanvraag in items:
                    ...
                    e = self.load_class(self.m.secure_controller, path='controllers/cms', blanc_env=self.environment())
                    if hasattr(e, '_run'):
                        dmn = e._run()
                    else:
                        try:
            ...
            if self.r.arg(0) and self.r.arg(0) in self.m.settings.modules:
    ...
    def send_email(self, email=False, subject=False, message=False, bcc=False, tls=True):
        ...
        if email and subject and message:
            if not hasattr(self.mail, 'send'):
                self.mail = Mail(server=str(mailserver), sender=str(self.m.settings.mailsender), login=str(self.m.settings.maillogin), tls=tls)
    ...
    def translate(self, woord, waarden=False, l=False):
        ...
        if woord and self.m.language[lang]:
            try:
    ...
    def grid(self, table, fields, q=False, menu=False, extra=False, well=False, path=False, edit=False, search=False, delete=False, new=False, view=False, smart=False):
    ...
    def javascript(self, code=None, include=None):
        ...
        if code == 'show':
            ...
            for script in self.javascript_file:
                if script.startswith('js'):
                    script = '/' + str(self.m.settings.media_folder) + '/' + str(script)
                e += str(html.script.tag('', _type='text/javascript', _nonce=nonce, _src=script))
        ...
        elif include not in self.javascript_inc:
    ...
    def css(self, code=None, include=None):
        ...
        elif include not in self.css_inc:
...
def server(environ, start_response):
    ...
    if ip in memory.vuurmuur and memory.vuurmuur[ip] >= 8:
        ...
        pad = memory.folder + '/' + memory.appfolder + '/views/' + memory.base_template + '/http/blocked.html'
                ...
                try:
                    ...
                    if app:
                        if not hasattr(app, 'create_ticket'):
                            app = Simplecms(environ, memory)
                    app.create_ticket(ticket=app.encrypt(str(fout)), data=fout)
                except:
                    output = '404'
