...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/dal.py
# Compiled at: 2015-12-25 03:30:10
"""
...
class BaseAdapter(ConnectionPool):
    ...
    def create_table(self, table, migrate=True, fake_migrate=False, polymodel=None):
        ...
        for field in table:
            ...
            if isinstance(field_type, SQLCustomType):
                ...
                if '.' not in referenced and referenced != tablename and hasattr(table, '_primarykey'):
                    ftype = types['integer']
                elif hasattr(table, '_primarykey'):
                    ...
                    if rfieldname in rtable._primarykey or rfield.unique:
                        ...
                        if not rfield.unique and len(rtable._primarykey) > 1:
                            ...
                            ftype = ftype + types['reference FK'] % dict(constraint_name=constraint_name, foreign_key='%s (%s)' % (rtablename,
                             rfieldname), table_name=tablename, field_name=field_name, on_delete_action=field.ondelete)
                else:
                    if referenced in db:
                        ...
                        id_fieldname = 'id'
                    ftype = types[field_type[:9]] % dict(index_name=field_name + '__idx', field_name=field_name, constraint_name=constraint_name, foreign_key='%s (%s)' % (referenced,
                     id_fieldname), on_delete_action=field.ondelete)
        ...
        for rtablename in TFK:
            ...
            fields = fields + ',\n    ' + types['reference TFK'] % dict(table_name=tablename, field_name=(', ').join(fkeys), foreign_table=rtablename, foreign_key=(', ').join(pkeys), on_delete_action=field.ondelete)
        ...
        for key in keys:
            ...
            if key not in sql_fields_old:
                ...
                if self.dbengine in ('postgres', ) and ftype.startswith('geometry'):
                    ...
                    query = [
                     'ALTER TABLE %s DROP %s;' % (tablename, key)]
                else:
                    query = [
    ...
    def insert(self, table, fields):
        ...
        if hasattr(table, '_primarykey'):
            return dict([ (k[0].name, k[1]) for k in fields if k[0].name in table._primarykey
                        ])
    ...
    def _select(self, query, fields, attributes):
        ...
        for field in fields:
            if isinstance(field, basestring) and REGEX_TABLE_DOT_FIELD.match(field):
                tn, fn = field.split('.')
                field = self.db[tn][fn]
...
class MySQLAdapter(BaseAdapter):
    ...
    def _drop(self, table, mode):
        return [
         'SET FOREIGN_KEY_CHECKS=0;', 'DROP TABLE %s;' % table,
...
class IMAPAdapter(NoSQLAdapter):
    ...
    def __init__(self, db, uri, pool_size=0, folder=None, db_codec='UTF-8', credential_decoder=IDENTITY, driver_args={}, adapter_args={}, do_connect=True, after_connection=None):
        ...
        self.flags = {'deleted': '\\Deleted', 'draft': '\\Draft', 'flagged': '\\Flagged', 
           ...
           'answered': '\\Answered'}
        self.search_fields = {'id': 'MESSAGE', 
           'created': 'DATE', 'uid': 'UID', 
           'sender': 'FROM', 'to': 'TO', 
           'cc': 'CC', 'bcc': 'BCC', 
    ...
    def define_tables(self, mailbox_names=None):
        ...
        for name in names:
            self.db.define_table('%s' % name, Field('uid', 'string', writable=False), Field('answered', 'boolean'), Field('created', 'datetime', writable=False), Field('content', list, writable=False), Field('to', 'string', writable=False), Field('cc', 'string', writable=False), Field('bcc', 'string', writable=False), Field('size', 'integer', writable=False), Field('deleted', 'boolean'), Field('draft', 'boolean'), Field('flagged', 'boolean'), Field('sender', 'string', writable=False), Field('recent', 'boolean', writable=False), Field('seen', 'boolean'), Field('subject', 'string', writable=False), Field('mime', 'string', writable=False), Field('email', 'string', writable=False, readable=False), Field('attachments', list, writable=False, readable=False), Field('encoding', writable=False))
...
def sqlhtml_validators(field):
    ...
    if not have_validators:
        ...
        if field_type in ('string', 'text', 'password'):
            ...
            requires.append(validators.IS_INT_IN_RANGE(-1e+100, 1e+100))
        elif field_type.startswith('decimal'):
...
class DAL(object):
    ...
    def get_instances():
        ...
        for db_uid, db_group in dbs:
            for db in db_group:
                ...
                infos[k] = dict(dbstats=[ (row[0], row[1]) for row in db._timings ], dbtables={'defined': sorted(list(set(db.tables) - set(db._LAZY_TABLES.keys()))), 
        ...
        * '<adaptername>_nonreserved' Checks against the specific adapters
        ...
        if uri == '<zombie>' and db_uid is not None:
            ...
            if uri:
                ...
                for k in range(attempts):
                    for uri in uris:
                        try:
                            ...
                            kwargs = dict(db=self, uri=uri, pool_size=pool_size, folder=folder, db_codec=db_codec, credential_decoder=credential_decoder, driver_args=driver_args or {}, adapter_args=adapter_args or {}, do_connect=do_connect, after_connection=after_connection)
                ...
                self._adapter = BaseAdapter(db=self, pool_size=0, uri='None', folder=folder, db_codec=db_codec, after_connection=after_connection)
            ...
            if auto_import or items:
                self.import_table_definitions(adapter.folder, items=items)
            return
    ...
    def import_table_definitions(self, path, migrate=False, fake_migrate=False, items=None):
        pattern = pjoin(path, self._uri_hash + '_*.table')
        if items:
            for tablename, table in items.iteritems():
                fields = []
                [ table.pop(name) for name in ('name', 'fields') if name in table
                ]
                if 'items' in table:
                    for fieldname, field in table.pop('items').iteritems():
                        [ field.pop(key) for key in ('requires', 'name', 'compute',
                                                     'colname') if key in field ]
...
def index():
    ...
    def as_dict(self, flat=False, sanitize=True, field_options=True):
        ...
        db_as_dict = dict(items={}, tables=[], uri=uri, dbname=dbname, db_uid=db_uid, **dict([ (k, getattr(self, '_' + k)) for k in [
         'pool_size', 'folder', 'db_codec',
        ...
        for table in self:
            ...
            db_as_dict['items'][tablename] = table.as_dict(flat=flat, sanitize=sanitize, field_options=field_options)
    ...
    def as_xml(self, sanitize=True, field_options=True):
        ...
        d = self.as_dict(flat=True, sanitize=sanitize, field_options=field_options)
...
class Table(object):
    ...
    def __init__(self, db, tablename, *fields, **args):
        ...
        self._sequence_name = args.get('sequence_name') or db and db._adapter.sequence_name(tablename)
        ...
        if hasattr(self, '_primarykey'):
            ...
            if len(self._primarykey) == 1:
                self._id = [ f for f in fields if isinstance(f, Field) and f.name == self._primarykey[0] ][0]
        else:
            if not [ f for f in fields if isinstance(f, Field) and f.type == 'id' ]:
                field = Field('id', 'id')
            ...
            for field in fields:
                if isinstance(field, (FieldMethod, FieldVirtual)):
                    ...
                    include_new(field)
                elif isinstance(field, dict) and 'fieldname' and field['fieldname'] not in fieldnames:
    ...
    def _enable_record_versioning(self, archive_db=None, archive_name='%(tablename)s_archive', is_active='is_active', current_record='current_record', current_record_label=None):
        ...
        archive_db.define_table(archive_name, Field(current_record, field_type, label=current_record_label), *clones)
    ...
    def as_dict(self, flat=False, sanitize=True, field_options=True):
        tablename = str(self)
        table_as_dict = dict(name=tablename, items={}, fields=[], sequence_name=self._sequence_name, trigger_name=self._trigger_name, common_filter=self._common_filter, format=self._format, singular=self._singular, plural=self._plural)
        for field in self:
            if field.readable or field.writable or not sanitize:
                table_as_dict['fields'].append(field.name)
                table_as_dict['items'][field.name] = field.as_dict(flat=flat, sanitize=sanitize, options=field_options)
...
class Expression(object):
    ...
    def startswith(self, value):
        db = self.db
        if self.type not in ('string', 'text', 'json'):
    ...
    def contains(self, value, all=False, case_sensitive=False):
        ...
        if self.type not in ('string', 'text', 'json') and not self.type.startswith('list:'):
...
class Field(Expression):
    ...
    def __init__(self, fieldname, type='string', length=None, default=DEFAULT, required=False, requires=DEFAULT, ondelete='CASCADE', notnull=False, unique=False, uploadfield=True, widget=None, label=None, comment=None, writable=True, readable=True, update=None, authorize=None, autodelete=False, represent=None, uploadfolder=None, uploadseparate=False, uploadfs=None, compute=None, custom_store=None, custom_retrieve=None, custom_retrieve_file_properties=None, custom_delete=None, filter_in=None, filter_out=None, custom_qualifier=None, map_none=None, wysiwyg=False):
        ...
        self.represent = list_represent if represent == None and type in ('list:integer',
        ...
        if isinstance(self_uploadfield, Field):
            ...
            if path:
                ...
                if self.uploadseparate:
                    ...
                    path = pjoin(path, '%s.%s' % (self._tablename, self.name), uuid_key[:2])
                if not exists(path):
                    os.makedirs(path)
    ...
    def as_dict(self, flat=False, sanitize=True, options=True):
        attrs = ('type', 'length', 'default', 'required', 'ondelete', 'notnull', 'unique',
                 'uploadfield', 'widget', 'label', 'comment', 'writable', 'readable',
                 'update', 'authorize', 'autodelete', 'represent', 'uploadfolder',
                 'uploadseparate', 'uploadfs', 'compute', 'custom_store', 'custom_retrieve',
                 'custom_retrieve_file_properties', 'custom_delete', 'filter_in',
                 'filter_out', 'custom_qualifier', 'map_none', 'name')
        SERIALIZABLE_TYPES = (
         int, long, basestring, dict, list,
        ...
        def filter_requires(t, r, options=True):
            if sanitize and any([ keyword in str(t).upper() for keyword in ('CRYPT',
                                                                            'IS_STRONG')
                                ]):
                return
            else:
                ...
                if not options and newr.has_key('labels'):
                    [ newr.update({key: None}) for key in ('labels', 'theset') if key in newr ]
                for k, v in newr.items():
                    if k == 'other':
                        if isinstance(v, dict):
                            ...
                            other = v
                        newr[k] = {otype: filter_requires(otype, other, options=options)}
                    else:
                        newr[k] = flatten(v)
        ...
        if isinstance(self.requires, (tuple, list, set)):
            requires = dict([ (flatten(type(r)), filter_requires(type(r), r, options=options)) for r in self.requires
                            ])
        else:
            requires = {flatten(type(self.requires)): filter_requires(type(self.requires), self.requires, options=options)}
        d = dict(colname='%s.%s' % (self.tablename, self.name), requires=requires)
    ...
    def as_xml(self, sanitize=True, options=True):
        if have_serializers:
            ...
            raise ImportError('No xml serializers available')
        d = self.as_dict(flat=True, sanitize=sanitize, options=options)
        return xml(d)
...
class Query(object):
    ...
    def as_dict(self, flat=False, sanitize=True):
        ...
        """
        SERIALIZABLE_TYPES = (
         tuple, dict, list, int, long, float,
...
class Rows(object):
    ...
    def __or__(self, other):
        ...
        records += [ record for record in other.records if record not in records
                   ]
        return Rows(self.db, records, self.colnames)
