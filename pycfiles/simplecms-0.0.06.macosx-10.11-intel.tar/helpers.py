...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/helpers.py
# Compiled at: 2016-07-17 03:26:16
"""
 SimpleCMS helpers
...
_month = [None, 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep',
 'Oct', 'Nov', 'Dec']
regex_geocode = re.compile('<geometry>[\\W]*?<location>[\\W]*?<lat>(?P<la>[^<]*)</lat>[\\W]\n*?<lng>(?P<lo>[^<]*)</lng>[\\W]*?</location>')
sleutel = str(uuid.uuid5(uuid.NAMESPACE_DNS, str(uuid.getnode())))
EXT_TYPE = {'.load': 'text/html', 
   '.nessus': 'text/xml', 
...
config = {'version': '1.0', 
   ...
   'data_model': 'website', 
   'development': 'dev', 
   'worldfolders': {}, 'cdn_string': 'sqlite://database.db', 
   'migrate': True, 
   'fake_migrate': True, 
   'gae_cdn_string': 'gae', 
   'secure': 'management', 
   ...
   'blacklist': [
               ...
               'sql', 'cgi', 'csv', 'mysql', 'ftp', '_vt', '.hta', '.bak', 'pwdw00t',
               'pma', 'admin', 'git', 'svn'], 
   'modules': {}, 'backend_pages': [
                   ('Website', 'pagina', 'globe'), ('Bestanden', 'verkenner', 'picture-o')], 
   'backend_modules': [
                     ('Beheer', 'beheer', 'cogs')], 
   'language': {'en': 'English', 'nl': 'Nederlands'}, 'default_lang': 'en', 
   'sql_lang': True, 
...
def timeparts(datum):
    ...
    return [
     year, month, day, hour, seconds, millisec]
...
class TAG(object):
    ...
    def wrap(component, rules):
        if not rules:
            return component
        if not isinstance(component, TAG) or component.name not in rules:
    ...
    def find(self, expr):
        ...
        if ',' in expr:
            tags = reduce(union, [ self.find(x.strip()) for x in expr.split(',')
                                 ], set())
        elif ' ' in expr:
            tags = [
             self]
            ...
            tags = reduce(union, [ c.find(expr) for c in self if isinstance(c, TAG)
                                 ], set())
            tag = TAG.regex_tag.match(expr)
            id = TAG.regex_id.match(expr)
            _class = TAG.regex_class.match(expr)
            attr = TAG.regex_attr.match(expr)
            if (tag is None or self.name == tag.group(1)) and (id is None or self['_id'] == id.group(1)) and (_class is None or _class.group(1) in (self['_class'] or '').split()) and (attr is None or self[('_' + attr.group(1))] == attr.group(2)):
                tags.add(self)
        return tags
    ...
    def xml(self):
        ...
        ca = (' ').join('%s="%s"' % (k[1:], k[1:] if v == True else xmlescape(v)) for k, v in sorted(self.attributes.items()) if k.startswith('_') and v is not None)
        ca = ' ' + ca if ca else ''
...
class safe(TAG):
    default_allowed_tags = {'a': [
           'href', 'title', 'target'], 
       'b': [], 'blockquote': ['type'], 'br': [], 'i': [], 'li': [], 'ol': [], 'ul': [], 'p': [], 'cite': [], 'code': [], 'pre': [], 'img': ['src', 'alt'], 'strong': [], 'h1': [], 'h2': [], 'h3': [], 'h4': [], 'h5': [], 'h6': [], 'table': [], 'tr': [], 'td': ['colspan'], 'div': []}
