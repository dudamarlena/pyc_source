...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/vorm.py
# Compiled at: 2015-12-25 03:27:27
"""
 Vorm
...
def vorm_validate(soort, invoer, options=False):
    if not invoer:
        ...
        vorm_types = {'email': '^[a-zA-Z0-9._%-]+@[a-zA-Z0-9._%-]+.[a-zA-Z]{2,6}$', 
           'telephone': '^([\\+][0-9]{1,3}([ \\.\\-])?)?([\\(]{1}[0-9]{3}[\\)])?([0-9A-Z \\.\\-]        {1,32})((x|ext|extension)?[0-9]{1,4}?)$', 
           'username': '^([a-zA-Z])[a-zA-Z_-]*[\\w_-]*[\\S]$|^([a-zA-Z])[0-9_-]*[\\S]$|        ^[a-zA-Z]*[\\S]$', 
           'imageurl': '^(http\\:\\/\\/[a-zA-Z0-9\\-\\.]+\\.[a-zA-Z]{2,3}(?:\\/\\S*)?(?:[a-zA-Z0-9_])        +\\.(?:jpg|jpeg|gif|png))$', 
           'domainname': '(?:[^0-9][a-zA-Z0-9]+(?:(?:\\-|\\.)[a-zA-Z0-9]+)*)', 
           'number': '^(?:\\d+,\\s*)*\\d+\\s*$', 
           'password': '^[a-zA-Z0-9._%-+]+@[a-zA-Z0-9._%-]+.[a-zA-Z]{2,8}$', 
           'name': '^[a-zA-Z0-9._%-+]+@[a-zA-Z0-9._%-]+.[a-zA-Z]{2,8}$', 
           'url': '^[a-zA-Z0-9._%-+]+@[a-zA-Z0-9._%-]+.[a-zA-Z]{2,8}$', 
           'date': '([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})', 
           'datetime': '([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})', 
           'datetime-local': '([0-9]{1,2})-([0-9]{1,2})-([0-9]{4})', 
           'time': '([0-1][0-9]|2[0-3]):[0-5][0-9]', 
           'alphanumeric': '^[a-zA-Z0-9\\s]+$', 
           'null': '^.{0,0}', 
           'comma_integer': '^((\\d{1,3}(,\\d{3})*)|(\\d{1,3}))$', 
           'postcode': '^[0-9]{4}\\s{0,2}[a-zA-z]{2}$', 
           'age': '^(1[89]|[2-9]\\d)$', 
           'number': '^[-+]?([0-9]{1,3}[,]?)?([0-9]{3}[,]?)*[.]?[0-9]*$'}
        if soort == 'image':
            return True
        if soort == 'join':
            return True
        if soort == 'f':
            return False
        if re.match(vorm_types[soort], invoer) != None:
            return True
        return False
        return
...
class Vorm:
    ...
    def fields(self, item):
        if item.writable and not item.compute and item.label != 'Id':
            ...
            if item.comment:
                invoer = invoer.replace('{opt}', ' placeholder="' + str(item.comment) + '"{opt}')
            if item.required:
            ...
            if self.waarden:
                if self.waarden[tid] == None:
                    self.waarden[tid] = ''
                invoer = invoer.replace('{value}', ' value="' + str(schoon(self.waarden[tid]) if self.waarden[tid] else '') + '" ')
            if item.default:
                invoer = invoer.replace('{value}', ' value="' + str(item.default) + '" ')
    ...
    def form_id(self):
        ...
        if self.id > 0:
            if self.app.isadmin:
                return dcsrf + '<input type="hidden" class="xx" name="id" value="' + str(self.id) + '" />'
            else:
                return dcsrf
                    ...
                    if k in post:
                        if k in self.valideer:
                            if self.valideer[k] == 'alias':
                                ...
                                return False
                        self.opslaan[k] = post[k]
                elif self.app.addmin and k == 'id' and post['id']:
                    self.id = post[k]
