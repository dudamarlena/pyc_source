...
# [GCC 8.4.0]
# Embedded file name: /Library/Python/2.7/site-packages/simplecms/mail.py
# Compiled at: 2015-02-03 12:17:31
import base64, datetime, sys, glob, os, re, time, traceback, smtplib, urllib, urllib2, cStringIO
...
class Mail(object):
    ...
    class Attachment(MIMEBase.MIMEBase):
        ...
        def __init__(self, payload, filename=None, content_id=None, content_type=None, encoding='utf-8'):
    ...
    def send(self, to, subject='None', message='None', attachments=None, cc=None, bcc=None, reply_to=None, encoding='utf-8', raw=False, headers={}):
        """
        ...
        if message is None:
            ...
            if isinstance(message, (list, tuple)):
                ...
                if message.strip().startswith('<html') and message.strip().endswith('</html>'):
                ...
                if cipher_type == 'gpg':
                    ...
                    if sign:
                        ...
                        try:
                            ...
                            sig.seek(0, 0)
                            payload = MIMEMultipart.MIMEMultipart('signed', boundary=None, _subparts=None, **dict(micalg='pgp-sha1', protocol='application/pgp-signature'))
                    ...
                    if encrypt:
                        ...
                        try:
                            ...
                            cipher.seek(0, 0)
                            payload = MIMEMultipart.MIMEMultipart('encrypted', boundary=None, _subparts=None, **dict(protocol='application/pgp-encrypted'))
                ...
                payload['Date'] = time.strftime('%a, %d %b %Y %H:%M:%S +0000', time.gmtime())
            ...
            try:
                if self.server == 'logging':
                    ...
                    if attachments:
                        result = mail.send_mail(sender=self.sender, to=origTo, subject=subject, body=text, html=html, attachments=attachments, **xcc)
                    elif html and not raw:
                        result = mail.send_mail(sender=self.sender, to=origTo, subject=subject, body=text, html=html, **xcc)
                    else:
                        result = mail.send_mail(sender=self.sender, to=origTo, subject=subject, body=text, **xcc)
                else:
