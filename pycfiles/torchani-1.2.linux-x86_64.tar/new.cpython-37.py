...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.7.5/x64/lib/python3.7/site-packages/torchani/data/new.py
# Compiled at: 2019-11-23 13:53:55
# Size of source mod 2**32: 30746 bytes
import numpy as np, torch, functools
...
class CachedDataset(torch.utils.data.Dataset):
 ...
 -0.600953, -38.08316, -54.707756, -75.194466]):
        ...
        if subtract_self_energies:
            if 'energies' in self.properties_info['properties']:
    ...
    def __getitem__(self, index):
        ...
        for i, _ in enumerate(chunks):
            chunks[i] = (
             chunks[i][0], chunks[i][1].reshape(chunks[i][1].shape[0], -1, 3))
    ...
    def split(self, validation_split):
        ...
        if self.enable_pkbar:
            ...
            pbar = pkbar.Pbar(message.format(len(self), train_size, val_size, self.batch_size), len(self))
    ...
    def load(self):
        ...
        """
        if self.enable_pkbar:
            pbar = pkbar.Pbar('=> processing and caching dataset into cpu memory: \ntotal ' + 'batches: {}, batch_size: {}'.format(len(self), self.batch_size), len(self))
        for i, _ in enumerate(self):
            if self.enable_pkbar:
                pbar.update(i)

    def add_energies_to_properties(self):
        if 'properties' in self.properties_info:
            if 'energies' not in self.properties_info['properties']:
        ...
        if 'properties' not in self.properties_info:
            self.properties_info = {'properties':[
              'energies'], 
             'padding_values':[
              None], 
             'padded_shapes':[
    ...
    def check_properties(self):
        ...
        for i, prop in enumerate(self.properties_info['properties']):
            ...
            print(message.format(prop, self.properties_info['dtypes'][i], self.properties_info['padding_values'][i], self.properties_info['padded_shapes'][i]))
...
def ShuffledDataset(file_path, batch_size=1000, num_workers=0, shuffle=True, chunk_threshold=20, other_properties={}, include_energies=True, validation_split=0.0, species_order=[
 'H', 'C', 'N', 'O'], subtract_self_energies=False, self_energies=[
 -0.600953, -38.08316, -54.707756, -75.194466]):
    ...
    .. note::
        ...
        'energies': ``[2000, 1]``
    """
    dataset = TorchData(file_path, batch_size, other_properties, include_energies, species_order, subtract_self_energies, self_energies)
...
class TorchData(torch.utils.data.Dataset):
    ...
    def __init__(self, file_path, batch_size, other_properties, include_energies, species_order, subtract_self_energies, self_energies):
...
def collate_fn(data, chunk_threshold, properties_info):
    ...
    batch_species = torch.nn.utils.rnn.pad_sequence(batch_species, batch_first=True,
      padding_value=(-1))
    batch_coordinates = torch.nn.utils.rnn.pad_sequence(batch_coordinates, batch_first=True,
      padding_value=0)
    ...
    for i, key in enumerate(properties_info['properties']):
        ...
        if properties_info['padding_values'][i] is None:
            ...
            prop = torch.nn.utils.rnn.pad_sequence(batch_species, batch_first=True,
...
def split_to_two_chunks(counts, chunk_threshold):
    ...
    cost = torch.max(chunk_size_left * chunk_max_left * chunk_max_left, min_cost_threshold) + torch.max(chunk_size_right * chunk_max_right * chunk_max_right, min_cost_threshold)
    cost_min, cost_min_index = torch.min((cost.squeeze()), dim=(-1))
    final_chunk_size = [
     chunk_size_left[cost_min_index], chunk_size_right[cost_min_index]]
    final_chunk_max = [chunk_max_left[cost_min_index], chunk_max_right[cost_min_index]]
    if cost_min_index == counts.shape[0] - 1:
        return (
         False, counts, [final_chunk_size[0]], [final_chunk_max[0]], cost_min)
...
def split_to_chunks(counts, chunk_threshold=np.inf):
    ...
    if splitted:
        ...
        return (
         final_chunk_size, final_chunk_max)
