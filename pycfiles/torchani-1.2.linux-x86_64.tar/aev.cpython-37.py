...
# [GCC 8.4.0]
# Embedded file name: /opt/hostedtoolcache/Python/3.7.5/x64/lib/python3.7/site-packages/torchani/aev.py
# Compiled at: 2019-11-23 13:53:55
# Size of source mod 2**32: 19465 bytes
import torch
...
def angular_terms(Rca: float, ShfZ: Tensor, EtaA: Tensor, Zeta: Tensor, ShfA: Tensor, vectors1: Tensor, vectors2: Tensor) -> Tensor:
...
def neighbor_pairs(padding_mask: Tensor, coordinates: Tensor, cell: Tensor, shifts: Tensor, cutoff: float) -> Tuple[(Tensor, Tensor, Tensor)]:
...
def compute_aev(species: Tensor, coordinates: Tensor, cell: Tensor, shifts: Tensor, triu_index: Tensor, constants: Tuple[(float, Tensor, Tensor, float, Tensor, Tensor, Tensor, Tensor)], sizes: Tuple[(int, int, int, int, int)]) -> Tensor:
...
class AEVComputer(torch.nn.Module):
    ...
    def constants(self):
        return (
         self.Rcr, self.EtaR, self.ShfR, self.Rca, self.ShfZ, self.EtaA, self.Zeta, self.ShfA)
    ...
    def forward(self, input_: Tuple[(Tensor, Tensor)], cell: Optional[Tensor]=None, pbc: Optional[Tensor]=None) -> SpeciesAEV:
        ...
        if cell is None and pbc is None:
            ...
            if not (cell is not None and pbc is not None):
