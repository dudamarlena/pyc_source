...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/components/consumers/xlsxwriter/writer.py
# Compiled at: 2010-08-11 13:19:49
...
class XLSXWriter(MultipleFileWriter):
    name = 'xlsxwriter'
    configuration = [
     SimpleKey('target_filename', label='Target filename', default='filename.xlsx'),
     RepeatedKey('columns', 'column', content=CompoundKey('column', text_value='title', attributes={'attribute': 'attribute', 'renderer': 'renderer'}, fields=[
      InputField('title', label='Title', help_text='Column title in header'),
    ...
    def __retrieve_columns_definition(self):
        ...
        for column_def in self.columns:
            ...
            self.column_definitions.append(dict(attribute=attr, title=column_def.get('title'), renderer=column_def.get('renderer') and (lambda value, rd=column_def.get('renderer'): eval(rd)) or default_renderer))
