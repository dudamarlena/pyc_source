...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/NEMbox/api.py
# Compiled at: 2020-03-17 03:53:27
# Size of source mod 2**32: 21156 bytes
"""
网易云音乐 Api
"""
...
import platform, time, requests, requests_cache
...
PLAYLIST_CLASSES = OrderedDict([
 ...
 (
  '风格',
  ['流行', '摇滚', '民谣', '电子', '舞曲', '说唱', '轻音乐', '爵士', '乡村', 'R&B/Soul', '古典', '民族',
...
class NetEase(object):
    ...
    def _raw_request(self, method, endpoint, data=None):
        if method == 'GET':
            resp = self.session.get(endpoint,
              params=data, headers=(self.header), timeout=DEFAULT_TIMEOUT)
        else:
            if method == 'POST':
                resp = self.session.post(endpoint,
                  data=data, headers=(self.header), timeout=DEFAULT_TIMEOUT)
        return resp
    ...
    def songs_detail(self, ids):
        ...
        params = dict(c=(json.dumps([{'id': _id} for _id in ids])),
          ids=(json.dumps(ids)))
    ...
    def dig_info(self, data, dig_type):
        ...
        if dig_type == 'playlists' or dig_type == 'top_playlists':
