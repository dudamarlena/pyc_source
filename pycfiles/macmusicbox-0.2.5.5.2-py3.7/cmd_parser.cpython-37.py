...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/NEMbox/cmd_parser.py
# Compiled at: 2020-03-21 00:14:15
...
ERASE_SPEED = 5
__all__ = [
 'cmd_parser', 'parse_keylist', 'coroutine', 'erase_coroutine']
...
def _cmd_parser():
    ...
    while True:
        ...
        if key * pre_key < 0 and key > pre_key:
            ...
            if key * pre_key > 0 and key + pre_key > 0:
                temp_pre_key = key
                keylist.append(key)
            else:
                if key * pre_key < 0:
...
def parse_keylist(keylist):
    ...
    if tail_cmd in range(48, 58):
        if set(keylist) | set(range(48, 58)) == set(range(48, 58)):
            return int(''.join([chr(i) for i in keylist] + [chr(tail_cmd)]))
    if len(keylist) == 0:
        return (
         0, tail_cmd)
    if tail_cmd in (ord('['), ord(']'), ord('j'), ord('k'), 258, 259):
        if max(keylist) <= 57:
            if min(keylist) >= 48:
                return (
                 int(''.join([chr(i) for i in keylist])), tail_cmd)
