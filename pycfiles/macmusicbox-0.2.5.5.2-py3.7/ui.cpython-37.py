...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/NEMbox/ui.py
# Compiled at: 2020-03-19 13:14:36
# Size of source mod 2**32: 22939 bytes
"""
网易云音乐 Ui
"""
from __future__ import print_function, unicode_literals, division, absolute_import
import hashlib, re, curses, datetime
...
class Ui(object):
    ...
    def build_playinfo(self, song_name, artist, album_name, quality, start, pause=False):
        ...
        self.addstr(1, min(self.indented_startcol + 18, self.x - 1), song_name + self.space + artist + '  < ' + album_name + ' >', curses.color_pair(4))
        self.screen.refresh()

    def build_process_bar(self, song, now_playing, total_length, playing_flag, playing_mode):
        return song and playing_flag or None
                ...
                if playing_mode == 2:
                    ...
                    if playing_mode == 3:
                        ...
                        if playing_mode == 4:
                            ...
                            self.now_lyric = lyrics or '暂无歌词 😌 \n'
                            self.post_lyric = ''
                            if dbus_activity:
                                if self.config.get('osdlyrics'):
                                    self.now_playing = '{} - {}\n'.format(name, artist)
                                else:
                                    key = now
                                    index = 0
    ...
    def build_menu(self, datatype, title, datalist, offset, index, step, start):
        ...
        if datatype == 'main':
            ...
            if datatype == 'songs' or datatype == 'fmsongs':
                ...
                for i in range(offset, iter_range):
                    ...
                    if i == index:
                        ...
                        lead = '👉🏻' + str(i) + '. '
                        self.addstr(i - offset + 8, self.indented_startcol, lead, curses.color_pair(2))
                        ...
                        if truelen(name) < self.x - self.startcol - 1:
                            self.addstr(i - offset + 8, self.indented_startcol + len(lead), name, curses.color_pair(2))
                        else:
                            name = scrollstring(name + '  ', start)
                            self.addstr(i - offset + 8, self.indented_startcol + len(lead), str(name), curses.color_pair(2))
                    else:
                        self.addstr(i - offset + 8, 0, ' ' * self.startcol)
                        self.addstr(i - offset + 8, self.startcol, '{}. {}{}{}  < {} >'.format(i, datalist[i]['song_name'], self.space, datalist[i]['artist'], datalist[i]['album_name'])[:int(self.x * 2)])
                ...
                if datatype == 'comments':
                    for i in range(offset, min(len(datalist), offset + step)):
                        ...
                        if i == index:
                            self.addstr(20, self.indented_startcol, '👉🏻' + str(i) + '. ' + break_str(datalist[i], self.indented_startcol, maxlength), curses.color_pair(2))
                        else:
                            self.addstr(i - offset + 9, self.startcol, str(i) + '. ' + datalist[i][:maxlength])
                        ...
                        if datatype == 'artist_info':
                            ...
                            if datatype == 'albums':
                                ...
                                if datatype == 'recommend_lists':
                                    ...
                                    if datatype in ('top_playlists', 'playlists'):
                                        for i in range(offset, min(len(datalist), offset + step)):
                                            if i == index:
                                        ...
                                        if datatype in ('toplists', 'playlist_classes',
                                            ...
                                            if datatype == 'djchannels':
                                                ...
                                                if datatype == 'search':
                                                    ...
                                                    if datatype == 'help':
                                                        for i in range(offset, min(len(datalist), offset + step)):
                                                            if i == index:
                                                                self.addstr(i - offset + 9, self.indented_startcol, "👉  {}. '{}{}   {}".format(i, (datalist[i][0] + "'").ljust(11), datalist[i][1], datalist[i][2]), curses.color_pair(2))
                                                            else:
                                                                self.addstr(i - offset + 9, self.startcol, "{}. '{}{}   {}".format(i, (datalist[i][0] + "'").ljust(11), datalist[i][1], datalist[i][2]))
