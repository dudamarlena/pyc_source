...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/ui/thumbnail.py
...
class ThumbnailCaptureDialog(tpDcc.Dialog, object):
    ...
    def thumbnail_capture(path, start_frame=None, end_frame=None, step=1, clear_cache=False, captured=None, show=False, modifier=True):
        ...
        d.show()
        if not show:
            if not (modifier and qtutils.is_control_modifier()):
                d._on_capture()
                d.close()
        _instance = d
        return _instance
    ...
    def __init__(self, path='', start_frame=None, end_frame=None, step=1, parent=None):
        super(ThumbnailCaptureDialog, self).__init__(name='CaptureThumbnailDialog',
        ...
        if start_frame is None:
            start_frame = int(gui.get_current_frame())
        if end_frame is None:
            end_frame = int(gui.get_current_frame())
    ...
    def _on_capture(self):
        ...
        self._captured_path = playblast.playblast(filename=path,
