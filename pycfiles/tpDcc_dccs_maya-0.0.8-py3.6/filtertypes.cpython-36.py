...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/filtertypes.py
...
def filter_by_group(selection_only, search_hierarchy, dag):
    ...
    if selection_only:
        ...
        objs_list or maya.logger.warning('Nothing is selected, please select at least one object!')
        return list()
    else:
        obj_transforms = filter_list_by_types(filter_types='transform',
          objs_list=objs_list,
...
def filter_dag_transforms(filter_types, search_hierarchy=False, selection_only=True, dag=False, transforms_only=True):
    ...
    if selection_only:
        ...
        filtered_shapes = filter_list_by_types(filter_types=filter_types,
          objs_list=sel_objs,
          search_hierarchy=search_hierarchy,
          dag=dag)
    else:
...
def filter_by_type(filter_type, search_hierarchy=False, selection_only=True, dag=False, remove_maya_defaults=True, transforms_only=True):
    """
