...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/transform.py
# Compiled at: 2020-04-24 23:10:50
# Size of source mod 2**32: 61343 bytes
"""
...
TRANSFORM_SIDES = {'end':{'short':[
 ...
 'start':{'short':[
  ...
  'long':[
   ('left_', 'right_')]}}
TRANSFORM_BASE_ATTRS = ('.tx', '.ty', '.tz', '.rx', '.ry', '.rz', '.sx', '.sy', '.sz',
                        '.v')
...
class DuplicateHierarchy(object):
    ...
    def _duplicate(self, xform):
        new_name = xform
        if self._replace_old:
            if self._replace_new:
    ...
    def _duplicate_hierarchy(self, xform):
        ...
        if self._stop:
            ...
            if children:
                duplicates = list()
                for child in children:
                    if self._only_these_transform:
                        if child not in self._only_these_transform:
                            continue
                        else:
                        ...
                        if maya.cmds.nodeType(top_duplicate) == 'joint' and maya.cmds.nodeType(duplicate) == 'joint' and maya.cmds.isConnected('{}.scale'.format(xform), '{}.inverseScale'.format(duplicate)):
...
def get_position(point):
    ...
    if type(point) == list or type(point) == tuple:
        ...
        if type(point) == str or type(point) == unicode:
...
def get_matrix(transform, world_space=True, time=None, as_list=False):
    ...
    if as_list:
        ...
        xform_matrix = matrix.build_matrix(translate=(
...
def match_rotate_scale_pivot_to_translation(target, source):
    ...
    maya.cmds.move((position[0]), (position[1]), (position[2]), ('{}.scalePivot'.format(target)), ('{}.rotatePivot'.format(target)),
      a=True)
...
def create_group_in_plane(transform1, transform2, transform3):
    ...
    maya.cmds.aimConstraint(transform3, pole_group, offset=[0, 0, 0], weight=1, aimVector=[1, 0, 0], upVector=[0, 1, 0], worldUpType='object',
            ...
            if not prefix:
                ...
                for xform in scope:
                    ...
                    if attribute.is_translate_rotate_connected(other):
                        continue
                    if not maya.cmds.objExists(other):
                    ...
                    if maya.cmds.objExists(other):
                        if maya.cmds.objExists('{}.mirror'.format(other)):
                            ...
                            if maya.cmds.nodeType(other) == 'joint':
                                ...
                                maya.cmds.move((new_xform[0] * -1),
                                  (new_xform[1]), (new_xform[2]), ('{}.scalePivot'.format(other)),
                            ...
                            if maya.cmds.nodeType(other) == 'transform':
                                ...
                                maya.cmds.move((pivot[0] * -1), (pivot[1]), (pivot[2]), ('{}.scalePivot'.format(other)), ('{}.rotatePivot'.format(other)),
                                  a=True)
...
def get_ordered_distance_and_transform(source_transform, transform_list):
    ...
    distance_list.sort()
    return (
     distance_list, distance_dict, original_distance_order)
...
def freeze_transforms(node, translate=True, rotate=True, scale=True, normal=False, preserve_normals=True, clean_history=False):
    """
    ...
    if clean_history:
        delete_history(node)
    return maya.cmds.makeIdentity(node,
      apply=True, translate=translate,
      rotate=rotate,
      scale=scale,
      normal=normal,
      pn=preserve_normals)
