...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tpDcc/dccs/maya/core/constraint.py
# Compiled at: 2020-04-24 23:10:50
# Size of source mod 2**32: 27230 bytes
"""
...
class Constraint(object):
    ...
    def get_transform(self, constraint):
        ...
        """
        xform = attribute.get_attribute_input('{}.constraintParentInverseMatrix'.format(constraint))
        if not xform:
            return
        else:
            cns = xform.split('.')
...
class MatrixConstraintNodes(object):
    ...
    def __init__(self, source_transform, target_transform=None):
        ...
        self._source = python.force_list(source_transform)
...
class SpaceSwitch(MatrixConstraintNodes):
    ...
    def get_space_switches(self, target):
        ...
        for attr_name in attrs:
            ...
            input_value = attribute.get_attribute_input(node_and_attr, node_only=True)
            if input_value:
        ...
        selector_dict = dict()
        for other in found:
            input_value = attribute.get_attribute_input(('{}.inputMatrix'.format(other)), node_only=True)
    ...
    def get_source(self, switch_node):
        ...
        if maya.cmds.nodeType(switch_node) == 'choice':
            indices = attribute.get_indices('{}.input'.format(switch_node))
            for index in indices:
                input_attr = '{}.input[{}]'.format(switch_node, index)
                matrix_sum = attribute.get_attribute_input(input_attr, node_only=True)
                matrix_attr = '{}.targetMatrix'.format(matrix_sum)
                if maya.cmds.objExists(matrix_attr):
                    xform = attribute.get_attribute_input(matrix_attr, node_only=True)
                    found.append(xform)

        else:
            if maya.cmds.nodeType(switch_node) == 'wtAddMatrix':
    ...
    def _add_source(self, source, switch_node):
        ...
        if node_type == 'wtAddMatrix':
            slot_index = attribute.get_available_slot('{}.wtMatrix'.format(switch_node))
        ...
        if node_type == 'choice':
            slot_index = attribute.get_available_slot('{}.input'.format(switch_node))
            matrix.set_description('{}_{}'.format(slot_index + 1, self.description))
            matrix.create()
            matrix_node = matrix.node_multiply_matrix
