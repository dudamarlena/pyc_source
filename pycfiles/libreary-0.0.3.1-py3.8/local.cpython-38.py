...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/libreary/adapters/local.py
# Compiled at: 2020-03-19 14:48:15
# Size of source mod 2**32: 9978 bytes
import os
...
class LocalAdapter:
    ...
    def _store_canonical(self, current_path: str, r_id: str, checksum: str, filename: str) -> str:
        ...
        new_location = os.path.expanduser('{}/canonical_{}'.format(self.storage_dir, filename))
        new_dir = os.path.expanduser('/'.join(new_location.split('/')[:-1]))
        sha1Hash = hashlib.sha1(open(current_location, 'rb').read())
        sha1Hashed = sha1Hash.hexdigest()
        if not os.path.isdir(new_dir):
    ...
    def _delete_canonical(self, r_id: str) -> None:
        ...
        :param r_id - the resource to retrieve's UUID
        """
        logger.debug(f"Deleting canonical copy of object {r_id} from {self.adapter_id}")
        copy_info = self.metadata_man.get_canonical_copy_metadata(r_id)[0]
        copy_path = copy_info[3]
        os.remove(copy_path)
        self.metadata_man.delete_copy_metadata(copy_info[0])
