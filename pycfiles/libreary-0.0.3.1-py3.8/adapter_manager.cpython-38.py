...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/libreary/adapter_manager.py
# Compiled at: 2020-03-19 14:48:15
# Size of source mod 2**32: 27922 bytes
import json, os, string, random, hashlib, shutil
from typing import List
import logging, ast
import libreary.adapters.BaseAdapter as BaseAdapter
...
adapters_translate_table = {'LocalAdapter':LocalAdapter, 
 'S3Adapter':S3Adapter, 
 'GoogleDriveAdapter':GoogleDriveAdapter}
metadata_man_translate_table = {'SQLite3MetadataManager': SQLite3MetadataManager}
...
class AdapterManager:
    ...
    def get_all_levels(self) -> List[dict]:
        ...
        for level in level_data:
            ...
            logger.debug(f"Found level {level[1]} with adapters {json.loads(level[3])}")
        else:
            return levels
    ...
    def get_all_adapters(self) -> List[dict]:
        ...
        for level in self.levels.values():
            for adapter in level['adapters']:
                adapters[adapter['id']] = AdapterManager.create_adapter(adapter['type'], adapter['id'], self.config_dir, self.config['metadata'])
    ...
    def set_additional_adapter(self, adapter_id: str, adapter_type: str) -> BaseAdapter:
        ...
        """
        adapter = AdapterManager.create_adapter(adapter_type, adapter_id, self.config_dir, self.config['metadata'])
    ...
    def verify_adapter(self, adapter_id: str) -> bool:
        ...
        data_to_store = ''.join((random.choice(string.ascii_letters) for i in range(500)))
        ...
        locator = adapter._store_canonical(dropbox_path, r_id, real_checksum, 'libreary_test_resource.txt')
    ...
    @staticmethod
    def create_adapter(adapter_type: str, adapter_id: str, config_dir: str, metadata_man_config: dict, metadata_man_type='SQLite3MetadataManager') -> BaseAdapter:
        """
    ...
    def create_config_for_adapter(adapter_id: str, adapter_type: str, config_dir: str) -> dict:
        ...
        general_config = json.load(open('{}/config.json'.format(config_dir)))
        full_adapter_conf = {}
    ...
    def retrieve_by_preference--- This code section failed: ---

 L. 456         0  SETUP_FINALLY        26  'to 26'

 L. 457         2  LOAD_FAST                'self'
                4  LOAD_ATTR                adapters
                6  LOAD_FAST                'self'
                8  LOAD_ATTR                canonical_adapter
               10  BINARY_SUBSCR    
               12  LOAD_METHOD              retrieve
               14  LOAD_FAST                'r_id'
               16  CALL_METHOD_1         1  ''
               18  STORE_FAST               'new_loc'

 L. 458        20  LOAD_FAST                'new_loc'
               22  POP_BLOCK        
               24  RETURN_VALUE     
             26_0  COME_FROM_FINALLY     0  '0'

 L. 459        26  DUP_TOP          
               28  LOAD_GLOBAL              ChecksumMismatchException
 ...
 L. 460        40  LOAD_GLOBAL              logger
               42  LOAD_METHOD              error

 L. 461        44  LOAD_STR                 'Canonical Recovery Failed. Attempting to Restore Canonical Copy'

 L. 460        46  CALL_METHOD_1         1  ''
               48  POP_TOP          

 L. 462        50  LOAD_FAST                'self'
               52  LOAD_METHOD              restore_canonical_copy
               54  LOAD_FAST                'r_id'
               56  CALL_METHOD_1         1  ''
               58  POP_TOP          
               60  POP_EXCEPT       
 ...
 L. 464        66  LOAD_FAST                'self'
               68  LOAD_ATTR                adapters
               70  LOAD_METHOD              values
               72  CALL_METHOD_0         0  ''
               74  GET_ITER         
               76  FOR_ITER            148  'to 148'
               78  STORE_FAST               'adapter'

 L. 465        80  SETUP_FINALLY       102  'to 102'

 L. 466        82  LOAD_FAST                'adapter'
               84  LOAD_METHOD              retrieve
               86  LOAD_FAST                'r_id'
               88  CALL_METHOD_1         1  ''
               90  STORE_FAST               'new_loc'

 L. 467        92  LOAD_FAST                'new_loc'
               94  POP_BLOCK        
               96  ROT_TWO          
               98  POP_TOP          
              100  RETURN_VALUE     
 ...
 L. 468       102  DUP_TOP          
              104  LOAD_GLOBAL              ChecksumMismatchException
 ...
 L. 469       116  LOAD_GLOBAL              logger
              118  LOAD_METHOD              error

 L. 470       120  LOAD_STR                 'Canonical Recovery Failed. Attempting to Restore Canonical Copy'

 L. 469       122  CALL_METHOD_1         1  ''
              124  POP_TOP          

 L. 471       126  LOAD_FAST                'self'
              128  LOAD_METHOD              restore_from_canonical_copy
              130  LOAD_FAST                'adapter'
              132  LOAD_ATTR                adapter_id
              134  LOAD_FAST                'r_id'
              136  CALL_METHOD_2         2  ''
              138  POP_TOP          
              140  POP_EXCEPT       
...
Parse error at or near `POP_TOP' instruction at offset 36
    ...
    def check_single_resource_single_adapter(self, r_id: str, adapter_type: str, adapter_id: str) -> bool:
        """
            ...
            if adapter_id == copy[2]:
                ...
                if copy[2] != canonical_checksum:
                    try:
                        ...
                        logger.error('Restoration of {} in {} failed'.format(r_id, adapter_id))
                        found = False

        else:
            found or logger.debug(f"Could not find resource {r_id} in adapter {adapter_id}")
            try:
                a = AdapterManager.create_adapter(adapter_type, adapter_id, self.config_dir, self.config['metadata'])
                a.store(r_id)
                found = True
    ...
    def verify_adapter_metadata(self, adapter_id: str, r_id: str, delete_after_check: bool=True) -> bool:
        """
        Ensure that a copy of an object matches its canonical checksum.
    ...
    def restore_canonical_copy--- This code section failed: ---

 L. 582         0  SETUP_FINALLY        46  'to 46'

 L. 583         2  LOAD_FAST                'self'
                4  LOAD_METHOD              get_resource_metadata
                6  LOAD_FAST                'r_id'
                8  CALL_METHOD_1         1  ''
               10  STORE_FAST               'resource_info'

 L. 584        12  LOAD_FAST                'resource_info'
               14  LOAD_CONST               4
               16  BINARY_SUBSCR    
               18  STORE_FAST               'real_checksum'

 L. 585        20  LOAD_FAST                'resource_info'
               22  LOAD_CONST               2
               24  BINARY_SUBSCR    
               26  LOAD_METHOD              split
               28  LOAD_STR                 ','
               30  CALL_METHOD_1         1  ''
               32  STORE_FAST               'levels'

 L. 586        34  LOAD_FAST                'resource_info'
               36  LOAD_CONST               3
               38  BINARY_SUBSCR    
 ...
 L. 588        60  LOAD_GLOBAL              ResourceNotIngestedException
 ...
 L. 590        70  LOAD_FAST                'self'
               72  LOAD_ATTR                adapters
               74  LOAD_FAST                'self'
               76  LOAD_ATTR                canonical_adapter
               78  BINARY_SUBSCR    
               80  LOAD_METHOD              _delete_canonical
               82  LOAD_FAST                'r_id'
               84  CALL_METHOD_1         1  ''
               86  POP_TOP          

 L. 592        88  LOAD_CONST               0
               90  STORE_FAST               'current_location'

 L. 594        92  SETUP_FINALLY       260  'to 260'

 L. 595        94  LOAD_FAST                'levels'
               96  GET_ITER         
               98  FOR_ITER            256  'to 256'
              100  STORE_FAST               'level'

 L. 596       102  LOAD_FAST                'self'
              104  LOAD_METHOD              get_adapters_by_level
              106  LOAD_FAST                'level'
              108  CALL_METHOD_1         1  ''
              110  STORE_FAST               'adapters'

 L. 597       112  LOAD_FAST                'adapters'
              114  GET_ITER         
              116  FOR_ITER            232  'to 232'
              118  STORE_FAST               'adapter'

 L. 598       120  SETUP_FINALLY       162  'to 162'

 L. 599       122  LOAD_GLOBAL              logger
              124  LOAD_METHOD              debug

 L. 600       126  LOAD_STR                 'Trying to restore copy of '
              128  LOAD_FAST                'r_id'
              130  FORMAT_VALUE          0  ''
              132  LOAD_STR                 ' from adapter '
              134  LOAD_FAST                'adapter'
              136  FORMAT_VALUE          0  ''
              138  BUILD_STRING_4        4 

 L. 599       140  CALL_METHOD_1         1  ''
              142  POP_TOP          

 L. 601       144  LOAD_FAST                'adapter'
              146  LOAD_METHOD              retrieve
              148  LOAD_FAST                'r_id'
              150  CALL_METHOD_1         1  ''
              152  STORE_FAST               'current_location'

 L. 602       154  LOAD_GLOBAL              AdapterRestored
              156  RAISE_VARARGS_1       1  'exception instance'
              158  POP_BLOCK        
              160  JUMP_BACK           116  'to 116'
            162_0  COME_FROM_FINALLY   120  '120'

 L. 603       162  DUP_TOP          
              164  LOAD_GLOBAL              ResourceNotIngestedException
 ...
 L. 604       176  POP_EXCEPT       
              178  JUMP_BACK           116  'to 116'
              180  POP_EXCEPT       
              182  JUMP_BACK           116  'to 116'
            184_0  COME_FROM           168  '168'

 L. 605       184  DUP_TOP          
              186  LOAD_GLOBAL              ChecksumMismatchException
 ...
 L. 606       198  POP_EXCEPT       
              200  JUMP_BACK           116  'to 116'
              202  POP_EXCEPT       
              204  JUMP_BACK           116  'to 116'
            206_0  COME_FROM           190  '190'

 L. 607       206  DUP_TOP          
              208  LOAD_GLOBAL              NoCopyExistsException
 ...
 L. 608       220  POP_EXCEPT       
              222  JUMP_BACK           116  'to 116'
              224  POP_EXCEPT       
              226  JUMP_BACK           116  'to 116'
            228_0  COME_FROM           212  '212'
              228  END_FINALLY      
              230  JUMP_BACK           116  'to 116'

 L. 609       232  LOAD_GLOBAL              logger
              234  LOAD_METHOD              error
              236  LOAD_STR                 'Failed to restore copy of '
              238  LOAD_FAST                'r_id'
              240  FORMAT_VALUE          0  ''
              242  BUILD_STRING_2        2 
              244  CALL_METHOD_1         1  ''
              246  POP_TOP          
 ...
 L. 610       248  POP_TOP          
              ...
              254  RETURN_VALUE     
              256  POP_BLOCK        
              258  JUMP_FORWARD        306  'to 306'
            260_0  COME_FROM_FINALLY    92  '92'

 L. 611       260  DUP_TOP          
              262  LOAD_GLOBAL              AdapterRestored
              264  COMPARE_OP               exception-match
          266_268  POP_JUMP_IF_FALSE   304  'to 304'
              270  POP_TOP          
              272  POP_TOP          
              274  POP_TOP          

 L. 612       276  LOAD_FAST                'self'
              278  LOAD_ATTR                adapters
              280  LOAD_FAST                'self'
              282  LOAD_ATTR                canonical_adapter
              284  BINARY_SUBSCR    
              286  LOAD_METHOD              store_canonical

 L. 613       288  LOAD_FAST                'current_location'

 L. 613       290  LOAD_FAST                'r_id'

 L. 613       292  LOAD_FAST                'real_checksum'

 L. 613       294  LOAD_FAST                'filename'

 L. 612       296  CALL_METHOD_4         4  ''
              298  POP_TOP          
              300  POP_EXCEPT       
            ...
            306_1  COME_FROM           258  '258'

Parse error at or near `POP_EXCEPT' instruction at offset 180
    ...
    def verify_copy(self, r_id: str, adapter_id: str, deep: bool=False) -> bool:
        ...
        :param deep - specify whether to run a deep or shallow check
        """
        return self.compare_copies(r_id,
          adapter_id, (self.canonical_adapter), deep=deep)