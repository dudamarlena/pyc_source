...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/libreary/libreary.py
...
metadata_manager_translate_table = {'SQLite3MetadataManager': SQLite3MetadataManager}
...
class Libreary:
    ...
    def __init__(self, config_dir: str):
        ...
        try:
            ...
            self.adapter_man = AdapterManager((self.config),
              metadata_man=(self.metadata_man))
    ...
    def ingest(self, current_file_path: str, levels: List[str], description: str, delete_after_store: bool=False, metadata_schema: List[str]=[], metadata: List[dict]=[]) -> str:
        """
        ...
        """
        obj_id = self.ingester.ingest(current_file_path,
          levels,
          description,
          delete_after_store=False,
          metadata_schema=metadata_schema,
          metadata=metadata)
        self.adapter_man.send_resource_to_adapters(obj_id,
          delete_after_send=delete_after_store)
        logger.debug(f"Ingesting object {obj_id} to LIBREary. Description: {description}")
        return obj_id
