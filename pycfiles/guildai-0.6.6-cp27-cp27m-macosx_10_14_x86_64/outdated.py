...
# [GCC 8.4.0]
# Embedded file name: ./guild/external/pip/_internal/utils/outdated.py
    ...
    if not installed_version:
        ...
        try:
            ...
            if pypi_version is None:
                finder = PackageFinder(find_links=options.find_links, index_urls=[
                 options.index_url] + options.extra_index_urls, allow_all_prereleases=False, trusted_hosts=options.trusted_hosts, process_dependency_links=options.process_dependency_links, session=session)
            ...
            if pip_version < remote_version and pip_version.base_version != remote_version.base_version and was_installed_by_pip('pip'):
                if WINDOWS:
