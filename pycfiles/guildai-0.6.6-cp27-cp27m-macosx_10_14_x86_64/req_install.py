...
# [GCC 8.4.0]
# Embedded file name: ./guild/external/pip/_internal/req/req_install.py
# Compiled at: 2019-09-10 15:18:29
from __future__ import absolute_import
import io, logging, os, re, shutil, sys, sysconfig, traceback, zipfile
...
from pip._internal.utils.misc import _make_build_dir, ask_path_exists, backup_dir, call_subprocess, display_path, dist_in_site_packages, dist_in_usersite, ensure_dir, get_installed_version, is_installable_dir, read_text_file, rmtree
...
class InstallRequirement(object):
    ...
    def __init__(self, req, comes_from, source_dir=None, editable=False, link=None, update=True, markers=None, isolated=False, options=None, wheel_cache=None, constraint=False, extras=()):
    ...
    @classmethod
    def from_editable(cls, editable_req, comes_from=None, isolated=False, options=None, wheel_cache=None, constraint=False):
        from pip._internal.index import Link
        ...
        if name is not None:
            ...
            req = None
        return cls(req, comes_from, source_dir=source_dir, editable=True, link=Link(url), constraint=constraint, isolated=isolated, options=options if options else {}, wheel_cache=wheel_cache, extras=extras_override or ())
    ...
    def from_req(cls, req, comes_from=None, isolated=False, wheel_cache=None):
        ...
        if req.url:
            raise InstallationError('Direct url requirement (like %s) are not allowed for dependencies' % req)
    ...
    def from_line(cls, name, comes_from=None, isolated=False, options=None, wheel_cache=None, constraint=False):
        ...
        return cls(req, comes_from, link=link, markers=markers, isolated=isolated, options=options if options else {}, wheel_cache=wheel_cache, constraint=constraint, extras=extras)
    ...
    def move_wheel_files(self, wheeldir, root=None, home=None, prefix=None, warn_script_location=True, use_user_site=False, pycompile=True):
        move_wheel_files(self.name, self.req, wheeldir, user=use_user_site, home=home, root=root, prefix=prefix, pycompile=pycompile, isolated=self.isolated, warn_script_location=warn_script_location)
    ...
    def get_pep_518_info(self):
        ...
        if not os.path.isfile(self.pyproject_toml):
            ...
            if 'requires' not in build_system:
                raise InstallationError(error_template.format(package=self, reason="it has a 'build-system' table but not 'build-system.requires' which is mandatory in the table"))
            requires = build_system['requires']
            if not _is_list_of_str(requires):
                raise InstallationError(error_template.format(package=self, reason="'build-system.requires' is not a list of strings."))
            return requires
    ...
    def run_egg_info(self):
        if not self.source_dir:
            ...
            if self.name:
                logger.debug('Running setup.py (path:%s) egg_info for package %s', self.setup_py, self.name)
            else:
                logger.debug('Running setup.py (path:%s) egg_info for package from %s', self.setup_py, self.link)
            with indent_log():
                ...
                with self.build_env:
                    call_subprocess(egg_info_cmd + egg_base_option, cwd=self.setup_py_dir, show_stdout=False, command_desc='python setup.py egg_info')
            if self.req or isinstance(parse_version(self.pkg_info()['Version']), Version):
                op = '=='
            else:
                op = '==='
            ...
            if canonicalize_name(self.req.name) != metadata_name:
                logger.warning('Running setup.py (path:%s) egg_info for package %s produced metadata for project name %s. Fix your #egg=%s fragments.', self.setup_py, self.name, metadata_name, self.name)
    ...
    def egg_info_data(self, filename):
        if self.satisfied_by is not None:
            ...
            if not self.source_dir:
                ...
                return os.path.exists(filename) or None
    ...
    def egg_info_path(self, filename):
        if self._egg_info_path is None:
            ...
            if self.editable:
                ...
                for root, dirs, files in os.walk(base):
                    ...
                    for dir in list(dirs):
                        if os.path.lexists(os.path.join(root, dir, 'bin', 'python')) or os.path.exists(os.path.join(root, dir, 'Scripts', 'Python.exe')):
                            dirs.remove(dir)
            ...
            if not filenames:
                raise InstallationError('Files/directories (from %s) not found in %s' % (
                 filename, base))
            if len(filenames) > 1:
    ...
    def install_editable(self, install_options, global_options=(), prefix=None):
        ...
        with indent_log():
            with self.build_env:
                call_subprocess([
                 sys.executable, '-c', SETUPTOOLS_SHIM % self.setup_py] + list(global_options) + ['develop', '--no-deps'] + list(install_options), cwd=self.setup_py_dir, show_stdout=False)
    ...
    def update_editable(self, obtain=True):
        ...
        if self.link.scheme == 'file':
            return
        if not '+' in self.link.url:
            raise AssertionError('bad url: %r' % self.link.url)
            return self.update or None
    ...
    def install(self, install_options, global_options=None, root=None, home=None, prefix=None, warn_script_location=True, use_user_site=False, pycompile=True):
        ...
        if self.editable:
            ...
            if self.is_wheel:
                ...
                self.move_wheel_files(self.source_dir, root=root, prefix=prefix, home=home, warn_script_location=warn_script_location, use_user_site=use_user_site, pycompile=pycompile)
                self.install_succeeded = True
                return
            ...
            with TempDirectory(kind='record') as (temp_dir):
                ...
                with open_spinner(msg) as (spinner):
                    with indent_log():
                        with self.build_env:
                            call_subprocess(install_args + install_options, cwd=self.setup_py_dir, show_stdout=False, spinner=spinner)
...
def deduce_helpful_msg(req):
    ...
    if os.path.exists(req):
        ...
        try:
            ...
            logger.debug("Cannot parse '%s' as requirements             file" % req, exc_info=1)
