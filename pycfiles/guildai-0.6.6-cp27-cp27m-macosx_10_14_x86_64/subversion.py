...
# [GCC 8.4.0]
# Embedded file name: ./guild/external/pip/_internal/vcs/subversion.py
...
_svn_revision_re = re.compile('Revision: (.+)')
...
class Subversion(VersionControl):
    ...
    def get_info(self, location):
        ...
        if not not location.rstrip('/').endswith(self.dirname):
            raise AssertionError('Bad directory: %s' % location)
            output = self.run_command([
             'info', location], show_stdout=False, extra_environ={'LANG': 'C'})
            match = _svn_url_re.search(output)
            match or logger.warning('Cannot determine URL of svn checkout %s', display_path(location))
    ...
    def export(self, location):
        ...
        with indent_log():
            if os.path.exists(location):
                rmtree(location)
            cmd_args = [
             'export'] + rev_options.to_args() + [url, location]
    ...
    def fetch_new(self, dest, url, rev_options):
        ...
        cmd_args = [
         'checkout', '-q'] + rev_options.to_args() + [url, dest]
        self.run_command(cmd_args)

    def switch(self, dest, url, rev_options):
        cmd_args = [
         'switch'] + rev_options.to_args() + [url, dest]
        self.run_command(cmd_args)

    def update(self, dest, rev_options):
        cmd_args = [
         'update'] + rev_options.to_args() + [dest]
        self.run_command(cmd_args)
        ...
        if data.startswith('8') or data.startswith('9') or data.startswith('10'):
            ...
            try:
                xml = self.run_command([
                 'info', '--xml', location], show_stdout=False)
