...
# [GCC 8.4.0]
# Embedded file name: ./guild/external/pip/_internal/req/req_uninstall.py
...
from pip._internal.utils.misc import FakeFile, ask, dist_in_usersite, dist_is_local, egg_link_path, is_local, normalize_path, renames
...
def compact(paths):
    ...
    for path in sorted(paths, key=len):
        should_add = any(path.startswith(shortpath.rstrip('*')) and path[len(shortpath.rstrip('*').rstrip(sep))] == sep for shortpath in short_paths)
        if not should_add:
...
class UninstallPathSet(object):
    ...
    def _stash(self, path):
        return os.path.join(self.save_dir.path, os.path.splitdrive(path)[1].lstrip(os.path.sep))
    ...
    def from_dist(cls, dist):
        ...
        if not dist_is_local(dist):
            ...
            if dist_path in {p for p in {sysconfig.get_path('stdlib'),
             sysconfig.get_path('platstdlib')} if p if p}:
                logger.info('Not uninstalling %s at %s, as it is in the standard library.', dist.key, dist_path)
                return cls(dist)
            paths_to_remove = cls(dist)
            develop_egg_link = egg_link_path(dist)
            develop_egg_link_egg_info = ('{}.egg-info').format(pkg_resources.to_filename(dist.project_name))
            ...
            if egg_info_exists and dist.egg_info.endswith('.egg-info') and not dist.egg_info.endswith(develop_egg_link_egg_info):
                ...
                if distutils_egg_info:
                    raise UninstallationError(('Cannot uninstall {!r}. It is a distutils installed project and thus we cannot accurately determine which files belong to it which would lead to only a partial uninstall.').format(dist.project_name))
                else:
                    if dist.location.endswith('.egg'):
                        ...
                        if egg_info_exists and dist.egg_info.endswith('.dist-info'):
                            ...
                            assert link_pointer == dist.location, 'Egg-link %s does not match installed location of %s (at %s)' % (
