...
# [GCC 8.4.0]
# Embedded file name: /home/skylar/pinax/projects/NASA/apps/listings/models.py
# Compiled at: 2009-08-27 23:48:07
...
class Listing(models.Model):
    ...
    def get_absolute_url(self):
        return (
         'listings.views.listing', (), {'id': self.id, 'slug': self.slug})
    ...
    def save(self, force_insert=False, force_update=False):
        ...
        if new and notification:
            notification.send(User.objects.all().exclude(id=self.owner.id), 'listing_new', {'listing': self})
        elif notification and (self.state == 2 or self.state == 3):
            notification.send(User.objects.filter(watchrelation__listing=self).exclude(id=self.owner.id), 'listing_done', {'listing': self})
        elif notification:
            notification.send(User.objects.filter(watchrelation__listing=self).exclude(id=self.owner.id), 'listing_change', {'listing': self})

    def delete(self):
        if notification:
            notification.send(User.objects.filter(watchrelation__listing=self).exclude(id=self.owner.id), 'listing_delete', {'listing': self})
    ...
    if isinstance(instance.content_object, Listing):
        ...
        if notification:
            ...
            if recipient != commenter:
                notification.send([recipient], 'listing_comment', {'commenter': commenter, 'listing': listing, 'comment': instance})
