...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/blockcypher/utils.py
...
def format_crypto_units(input_quantity, input_type, output_type, coin_symbol=None, print_cs=False, safe_trimming=False, round_digits=0):
    ...
    Requires python >= 2.7
    """
    if not input_type in UNIT_CHOICES:
        raise AssertionError(input_type)
        assert output_type in UNIT_CHOICES, output_type
        assert print_cs and is_valid_coin_symbol(coin_symbol=coin_symbol), coin_symbol
    ...
    output_quantity = from_satoshis(input_satoshis=satoshis_float, output_type=output_type)
    ...
    if print_cs:
        curr_symbol = get_curr_symbol(coin_symbol=coin_symbol, output_type=output_type)
...
def get_txn_outputs(raw_tx_hex, output_addr_list, coin_symbol):
    ...
    for out in deserialized_tx.get('outs', []):
        ...
        pubkey_addr = script_to_address(out['script'], vbyte=COIN_SYMBOL_MAPPINGS[coin_symbol]['vbyte_pubkey'])
        script_addr = script_to_address(out['script'], vbyte=COIN_SYMBOL_MAPPINGS[coin_symbol]['vbyte_script'])
...
def get_txn_outputs_dict(raw_tx_hex, output_addr_list, coin_symbol):
    return compress_txn_outputs(txn_outputs=get_txn_outputs(raw_tx_hex=raw_tx_hex, output_addr_list=output_addr_list, coin_symbol=coin_symbol))
...
def flatten_txns_by_hash(tx_list, nesting=True):
    ...
    for tx in tx_list:
        ...
        if tx_hash in nested_cleaned_txs:
            ...
            nested_cleaned_txs[tx_hash] = {'txns_satoshis_list': [
                                    satoshis], 
...
def is_valid_bcy_block_representation(block_representation):
    ...
    if is_valid_block_num(block_representation):
        return True
    if is_valid_hash(block_representation):
...
def is_valid_block_representation(block_representation, coin_symbol):
    ...
    if coin_symbol in SHA_COINS:
        ...
        if coin_symbol in SCRYPT_COINS:
...
def _long_to_bytes(n, length, byteorder):
    ...
    return bytearray(n >> i * 8 & 255 for i in indexes)
...
def is_valid_address_for_coinsymbol(b58_address, coin_symbol):
    ...
    if not is_valid_coin_symbol(coin_symbol):
        ...
        if b58_address.startswith(COIN_SYMBOL_MAPPINGS[coin_symbol]['bech32_prefix']):
            return True
        if b58_address[0] in COIN_SYMBOL_MAPPINGS[coin_symbol]['address_first_char_list'] and is_valid_address(b58_address):
            return True
    return False