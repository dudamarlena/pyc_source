...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/blockcypher/api.py
# Compiled at: 2020-01-09 11:52:21
import os
...
def get_address_full(address, coin_symbol='btc', txn_limit=None, inout_limit=None, api_key=None, before_bh=None, after_bh=None, show_confidence=False, confirmations=0):
    assert is_valid_address_for_coinsymbol(b58_address=address, coin_symbol=coin_symbol), address
    assert isinstance(show_confidence, bool), show_confidence
...
def get_unconfirmed_balance(address, coin_symbol='btc', api_key=None):
    ...
    in satoshi.
    """
    return get_address_overview(address=address, coin_symbol=coin_symbol)['unconfirmed_balance']
...
def get_confirmed_balance(address, coin_symbol='btc', api_key=None):
    ...
    in satoshi.
    """
    return get_address_overview(address=address, coin_symbol=coin_symbol)['balance']
...
def get_num_confirmed_transactions(address, coin_symbol='btc', api_key=None):
    ...
    """
    return get_address_overview(address=address, coin_symbol=coin_symbol)['n_tx']
...
def get_num_unconfirmed_transactions(address, coin_symbol='btc', api_key=None):
    ...
    """
    return get_address_overview(address=address, coin_symbol=coin_symbol)['unconfirmed_n_tx']
...
def get_total_num_transactions(address, coin_symbol='btc', api_key=None):
    ...
    """
    return get_address_overview(address=address, coin_symbol=coin_symbol)['final_n_tx']
...
def generate_new_address(coin_symbol='btc', api_key=None):
    ...
    if coin_symbol not in ('btc-testnet', 'bcy'):
        WARNING_MSG = ['Generating private key details server-side.',
...
def get_transactions_details(tx_hash_list, coin_symbol='btc', limit=None, api_key=None):
    ...
    if len(tx_hash_list) == 0:
        ...
        if len(tx_hash_list) == 1:
            return [
             get_transaction_details(tx_hash=tx_hash_list[0], coin_symbol=coin_symbol, limit=limit, api_key=api_key)]
...
def get_num_confirmations(tx_hash, coin_symbol='btc', api_key=None):
    ...
    """
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, limit=1, api_key=api_key).get('confirmations')


def get_confidence(tx_hash, coin_symbol='btc', api_key=None):
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, show_confidence=True, api_key=api_key).get('confidence')


def get_miner_preference(tx_hash, coin_symbol='btc', api_key=None):
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, show_confidence=True, api_key=api_key).get('preference')


def get_receive_count(tx_hash, coin_symbol='btc', api_key=None):
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, show_confidence=True, api_key=api_key).get('receive_count')


def get_satoshis_transacted(tx_hash, coin_symbol='btc', api_key=None):
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, limit=1, api_key=api_key)['total']


def get_satoshis_in_fees(tx_hash, coin_symbol='btc', api_key=None):
    return get_transaction_details(tx_hash=tx_hash, coin_symbol=coin_symbol, limit=1, api_key=api_key)['fees']
...
def get_broadcast_transaction_hashes(coin_symbol='btc', api_key=None, limit=10):
    ...
    Warning, slow!
    """
    transactions = get_broadcast_transactions(coin_symbol=coin_symbol, api_key=api_key, limit=limit)
...
def get_merkle_root(block_representation, coin_symbol='btc', api_key=None):
    ...
    """
    return get_block_overview(block_representation=block_representation, coin_symbol=coin_symbol, txn_limit=1, api_key=api_key)['mrkl_root']
...
def get_block_height(block_hash, coin_symbol='btc', api_key=None):
    ...
    """
    return get_block_overview(block_representation=block_hash, coin_symbol=coin_symbol, txn_limit=1, api_key=api_key)['height']
...
def get_latest_block_height(coin_symbol='btc', api_key=None):
    ...
    """
    return get_blockchain_overview(coin_symbol=coin_symbol, api_key=api_key)['height']
...
def get_latest_block_hash(coin_symbol='btc', api_key=None):
    ...
    """
    return get_blockchain_overview(coin_symbol=coin_symbol, api_key=api_key)['hash']
...
def get_blockchain_fee_estimates(coin_symbol='btc', api_key=None):
    ...
    return {'high_fee_per_kb': overview['high_fee_per_kb'], 
...
def get_forwarding_address_details(destination_address, api_key, callback_url=None, coin_symbol='btc'):
    ...
    logger.info(url)
    params = {'token': api_key}
    data = {'destination': destination_address}
    if callback_url:
...
def get_forwarding_address(destination_address, api_key, callback_url=None, coin_symbol='btc'):
    ...
    """
    assert api_key, 'api_key required'
    resp_dict = get_forwarding_address_details(destination_address=destination_address, api_key=api_key, callback_url=callback_url, coin_symbol=coin_symbol)
...
def subscribe_to_address_webhook(callback_url, subscription_address, event='tx-confirmation', confirmations=0, confidence=0.0, coin_symbol='btc', api_key=None):
    """
...
def create_hd_wallet(wallet_name, xpubkey, api_key, subchain_indices=[], coin_symbol='btc'):
    ...
    assert inferred_coin_symbol and inferred_coin_symbol == coin_symbol
    assert api_key, 'api_key required'
    assert len(wallet_name) <= 25, wallet_name
    data = {'name': wallet_name, 
...
def get_wallet_addresses(wallet_name, api_key, is_hd_wallet=False, zero_balance=None, used=None, omit_addresses=False, coin_symbol='btc'):
    ...
    assert isinstance(omit_addresses, bool), omit_addresses
    params = {'token': api_key}
    kwargs = {'hd/' if is_hd_wallet else '': wallet_name}
...
def get_latest_paths_from_hd_wallet_addresses(wallet_addresses):
    ...
    for chain in wallet_addresses['chains']:
        ...
        latest_path_dict = {'latest_path': latest_path, 
...
def create_unsigned_tx(inputs, outputs, change_address=None, include_tosigntx=False, verify_tosigntx=False, min_confirmations=0, preference='high', coin_symbol='btc', api_key=None):
    ...
    if not isinstance(inputs, list):
        ...
        if not len(outputs) >= 1:
            ...
            for output in outputs:
                ...
                if output.get('script_type') == 'null-data':
                    ...
                    clean_output['addresses'] = [
                     output['address']]
                outputs_cleaned.append(clean_output)

            assert change_address and is_valid_address_for_coinsymbol(b58_address=change_address, coin_symbol=coin_symbol), change_address
        assert preference in ('high', 'medium', 'low', 'zero'), preference
        url = make_url(coin_symbol, **dict(txs='new'))
        data = {'inputs': inputs_cleaned, 
        ...
        if verify_tosigntx:
            ...
            print tx_is_correct or unsigned_tx
        ...
        for cnt, tosign_tx_toverify in enumerate(unsigned_tx['tosign_tx']):
            if double_sha256(tosign_tx_toverify) != unsigned_tx['tosign'][cnt]:
                ...
                return (
                 False, err_msg)
            try:
                txn_outputs_response_dict = get_txn_outputs_dict(raw_tx_hex=tosign_tx_toverify, output_addr_list=output_addr_list, coin_symbol=coin_symbol)
...
def simple_spend(from_privkey, to_address, to_satoshis, change_address=None, privkey_is_compressed=True, min_confirmations=0, api_key=None, coin_symbol='btc'):
    """
    ...
    """
    if not is_valid_coin_symbol(coin_symbol):
        raise AssertionError(coin_symbol)
        ...
        from_address = pubkey_to_address(pubkey=from_pubkey, magicbyte=COIN_SYMBOL_MAPPINGS[coin_symbol]['vbyte_pubkey'])
        inputs = [{'address': from_address}]
        logger.info('inputs: %s' % inputs)
        outputs = [{'address': to_address, 'value': to_satoshis}]
        logger.info('outputs: %s' % outputs)
        unsigned_tx = create_unsigned_tx(inputs=inputs, outputs=outputs, change_address=change_address, coin_symbol=coin_symbol, min_confirmations=min_confirmations, verify_tosigntx=False, include_tosigntx=True, api_key=api_key)
        ...
        tx_is_correct, err_msg = verify_unsigned_tx(unsigned_tx=unsigned_tx, inputs=inputs, outputs=outputs, sweep_funds=bool(to_satoshis == -1), change_address=change_address_to_use, coin_symbol=coin_symbol)
        print tx_is_correct or unsigned_tx
        raise Exception('TX Verification Error: %s' % err_msg)
    ...
    broadcasted_tx = broadcast_signed_transaction(unsigned_tx=unsigned_tx, signatures=tx_signatures, pubkeys=pubkey_list, coin_symbol=coin_symbol, api_key=api_key)
...
def simple_spend_p2sh(all_from_pubkeys, from_privkeys_to_use, to_address, to_satoshis, change_address=None, min_confirmations=0, api_key=None, coin_symbol='btc'):
    """
    ...
    if not is_valid_coin_symbol(coin_symbol):
        ...
        if 'errors' in unsigned_tx:
            ...
            raise Exception('Build Unsigned TX Error')
        tx_is_correct, err_msg = verify_unsigned_tx(unsigned_tx=unsigned_tx, inputs=None, outputs=outputs, sweep_funds=bool(to_satoshis == -1), change_address=change_address, coin_symbol=coin_symbol)
        print tx_is_correct or unsigned_tx
        raise Exception('TX Verification Error: %s' % err_msg)
    ...
    logger.info('pubkey_list: %s' % pubkey_list)
    tx_signatures = make_tx_signatures(txs_to_sign=txs_to_sign, privkey_list=privkey_list, pubkey_list=pubkey_list)
    logger.info('tx_signatures: %s' % tx_signatures)
    broadcasted_tx = broadcast_signed_transaction(unsigned_tx=unsigned_tx, signatures=tx_signatures, pubkeys=pubkey_list, coin_symbol=coin_symbol, api_key=api_key)
    logger.info('broadcasted_tx: %s' % broadcasted_tx)
    if 'errors' in broadcasted_tx:
...
def embed_data(to_embed, api_key, data_is_hex=True, coin_symbol='btc'):
    if not is_valid_coin_symbol(coin_symbol):
        ...
        params = {'token': api_key}
        data = {'data': to_embed}
        data['encoding'] = data_is_hex or 'string'
    r = requests.post(url, json=data, params=params, verify=True, timeout=TIMEOUT_IN_SECONDS)
    return get_valid_json(r)
...
def get_valid_metadata_identifier(coin_symbol, address, tx_hash, block_hash):
    ...
    if block_hash:
        assert is_valid_block_representation(block_representation=block_hash, coin_symbol=coin_symbol)
