...
# [GCC 8.4.0]
# Embedded file name: /Users/dan/dev/pyCast/shakecast/sc/app/server.py
# Compiled at: 2017-04-25 07:08:32
import socket, select as select_, time, os, sys
...
logging.basicConfig(filename=os.path.normpath(os.sep.join(log_path)), level=logging.DEBUG, format='[ShakeCast Server] %(levelname)-7.7s %(message)s')
...
class Server(object):
    ...
    def __init__(self):
        ...
        self.last_task = 0
        self.db_open = True
        self.log_file = os.path.join(sc_dir(), 'logs', 'server.log')
        self.sc_log_file = os.path.join(sc_dir(), 'logs', 'shakecast.log')
        self.socket_setup()
    ...
    def socket_check(self):
        ...
        if len(conns) > 0:
            ...
            conn.setblocking(0)
            client_thread = New_Thread(func=self.handle_client, args_in={'conn': conn, 'addr': addr})
            client_thread.start()
    ...
    def user_in(self, data=None, conn=None):
        ...
        if good_data is True:
            for key, value in new_task_dict.iteritems():
                ...
                self.queue += [new_task]
                self.log(message='Task added to queue: %s' % new_task.name, which='server')
    ...
    def queue_check(self):
        ...
        for i, task in enumerate(self.queue):
            if task.next_run < timestamp and task.status == 'stopped' and (task.db_use is False or self.db_open is True):
    ...
    def check_task(self, task=Task()):
        ...
        if task.loop is False:
            ...
            if task.output['status'] == 'finished':
                ...
                out_str = ('{} failed to run... \n{}: {}').format(task.name, type(task.error), task.error)
                server_log = ('Task: {} :: failed to run \n{}: {}').format(task.name, type(task.error), task.error)
            self.log(message=task.output.get('log', ''), which='shakecast')
            self.log(message=task.output.get('log', ''), which='server')
            ...
            task.status = 'stopped'
        logging.info(('{}: \n\tSTATUS: {} \n\tOUTPUT: {}').format(task.name, task.status, task.output))
        if task.db_use is True:
            self.db_open = True
    ...
    def info(self):
        ...
        """
        return {'status': 'finished', 'message': "{'queue': %s,                               'connections': %s}" % (self.queue,
                     self.connections)}
    ...
    def stop(self):
        ...
        """
        self.stop_loop = True
        return {'status': 'finished', 'message': 'Stopping loop...'}
    ...
    def shutdown(self):
        ...
        return {'status': 'finished', 'message': 'Stopping server...'}
