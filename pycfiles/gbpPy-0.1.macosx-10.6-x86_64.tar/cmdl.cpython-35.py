...
# [GCC 8.4.0]
# Embedded file name: /anaconda/lib/python3.5/site-packages/gbpPy/cmdl.py
# Compiled at: 2018-01-18 19:54:37
# Size of source mod 2**32: 7867 bytes
...
Developed with the following library versions:
    ...
    python:        3.5.2
"""
import sys, os, optparse, gbpPy.log as SID
...
class parser:
    ...
    def __init__(self, argv, positional_arguments=None, optional_arguments=None, n_positional_arguments=None):
        ...
        if positional_arguments != None:
            if n_positional_arguments != None:
                parser.error("Either 'positional_arguments' or 'n_positional_arguments' (but not both) must be specified when the CMD-line parser is called.")
        else:
            if n_positional_arguments == None:
                parser.error("Either 'positional_arguments' or 'n_positional_arguments' (but not both) must be specified when the CMD-line parser is called.")
            if positional_arguments == None:
        ...
        required_positional_arguments = [p_i for p_i in positional_arguments if p_i[1] == None]
        optional_positional_arguments = [p_i for p_i in positional_arguments if p_i[1] != None]
        ...
        for positional_arguments_type, arg_vers_txt_1, arg_vers_txt_2 in [[required_positional_arguments, '', ''], [optional_positional_arguments, '<', '>']]:
        ...
        parser = optparse.OptionParser(usage=usage_string, formatter=optparse.IndentedHelpFormatter(width=100), add_help_option=None)
        ...
        if not flag_help_added:
            parser.add_option('-?', '-h', '--help', action='help', help='display usage information')
        ...
        if len(args) < n_required_positional_arguments:
            ...
            if n_args > 1:
                plural_string2 = 'were'
            parser.error('program expects no more than %d command-line argument%s.  %d %s given instead: "%s".' % (
             n_required_positional_arguments, plural_string1, len(args), plural_string2, args))
