...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/ggplot/aes.py
# Compiled at: 2016-08-13 14:03:22
...
class aes(UserDict):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if b'colour' in self.data:
            self.data[b'color'] = self.data[b'colour']
            del self.data[b'colour']
        self.legend = []
    ...
    def _evaluate_expressions(self, data):
        ...
        for key, item in self.data.items():
            if item not in data:
                ...
                env = EvalEnvironment.capture(eval_env=self.__eval_env__ or 1).with_outer_namespace({b'factor': factor, b'pd': pd, b'np': np})
                try:
                    new_val = env.eval(item, inner_namespace=data)
                    data[item] = new_val
                except:
                    msg = b"Invalid column: '%s'" % str(item)
    ...
    def _get_discrete_aes(self, df):
        discrete_aes = []
        non_numeric_columns = df.select_dtypes(exclude=[b'number']).columns
