...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/ggplot/geoms/geom.py
# Compiled at: 2016-08-12 16:40:09
...
class geom(object):
    ...
    def _update_data(self, data, _aes):
        if b'mapping' in self.params:
            _aes = self.params[b'mapping']
            if b'data' not in self.params:
                data = _aes._evaluate_expressions(data)
                data = _aes.handle_identity_values(data)
        if b'data' in self.params:
            data = _aes._evaluate_expressions(self.params[b'data'])
            data = _aes.handle_identity_values(data)
        return (data, _aes)
    ...
    def _get_plot_args(self, data, _aes):
        ...
        for req in self.REQUIRED_AES:
            if req not in mpl_params:
                raise Exception(b'%s needed for %s' % (req, str(self)))
            else:
                del mpl_params[req]
