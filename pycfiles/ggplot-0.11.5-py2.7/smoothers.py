...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/ggplot/stats/smoothers.py
...
import scipy.stats as stats, datetime
date_types = (
 pd.tslib.Timestamp,
 pd.DatetimeIndex,
 pd.Period,
...
SPAN = 0.6666666666666666
ALPHA = 0.05

def _snakify(txt):
    txt = txt.strip().lower()
    return (b'_').join(txt.split())
...
def lm(x, y, alpha=ALPHA):
    ...
    fittedvalues = df[b'predicted_value'].values
    predict_mean_ci_low = df[b'mean_ci_95%_low'].values
    predict_mean_ci_upp = df[b'mean_ci_95%_upp'].values
    predict_ci_low = df[b'predict_ci_95%_low'].values
    predict_ci_upp = df[b'predict_ci_95%_upp'].values
    if x_is_date:
        x = [ Timestamp.fromordinal(int(i)) for i in x ]
    return (
     x, fittedvalues, predict_mean_ci_low, predict_mean_ci_upp)
