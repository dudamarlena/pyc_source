...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/ggplot/ggplot.py
# Compiled at: 2016-09-29 11:53:21
from __future__ import absolute_import, division, print_function, unicode_literals
import matplotlib.pyplot as plt, matplotlib as mpl
from matplotlib.colors import LinearSegmentedColormap
import six, numpy as np, pandas as pd, warnings
from .aes import aes
...
if os.environ.get(b'GGPLOT_DEV'):
...
class ggplot(object):
    ...
    """
    CONTINUOUS = [
     b'x', b'y', b'size', b'alpha']
    DISCRETE = [b'color', b'shape', b'marker', b'alpha', b'linestyle']
    ...
    def __repr__(self):
        self.make()
        if os.environ.get(b'GGPLOT_DEV'):
            self.fig.savefig(b'/tmp/ggplot-%d.png' % self.__hash__(), dpi=160)
            img = Image.open(b'/tmp/ggplot-%d.png' % self.__hash__())
            img.show()
            return b'<ggplot: (%d)>' % self.__hash__()
        plt.show()
        return b'<ggplot: (%d)>' % self.__hash__()
    ...
    def _handle_index(self):
        if b'__index__' in self._aes.values():
            self.data[b'__index__'] = self.data.index

    def add_labels(self):
        labels = [(self.fig.suptitle, self.title)]
        ...
        if self.facets.rowvar:
            for row, name in enumerate(sorted_unique(self.data[self.facets.rowvar])):
                ...
                ax.yaxis.set_label_position(b'right')
        ...
        if self.facets.colvar:
            for col, name in enumerate(sorted_unique(self.data[self.facets.colvar])):
                ...
                ax.set_title(name, fontdict={b'fontsize': 10})
    ...
    def apply_theme(self):
        if self.theme:
            ...
            for key, val in rcParams.items():
                try:
                    if key == b'text.dvipnghack' and val is None:
                        val = b'none'
                    mpl.rcParams[key] = val
                except Exception as e:
                    msg = b'Setting "mpl.rcParams[\'%s\']=%s" raised an Exception: %s' % (key, str(val), str(e))
                    warnings.warn(msg, RuntimeWarning)

        return

    def apply_coords(self):
        if self.coords == b'equal':
            for ax in self._iterate_subplots():
        ...
        elif self.coords == b'flip':
            if b'x' in self._aes.data and b'y' in self._aes.data:
                x = self._aes.data[b'x']
                y = self._aes.data[b'y']
                self._aes.data[b'x'] = y
                self._aes.data[b'y'] = x
        if self.margins:
    ...
    def apply_axis_labels(self):
        if self.xlab:
            ...
            if not isinstance(xlab, six.string_types):
                if xlab.args[2] == b'':
                    xlab.args[2] = self._aes.get(b'x')
        else:
            xlab = self._aes.get(b'x')
        if self.xbreaks:
            for ax in self._iterate_subplots():
        ...
        if self.ylab:
            ...
            if not isinstance(ylab, six.string_types):
                if ylab.args[2] == b'':
                    ylab.args[2] = self._aes.get(b'y')
        else:
            ylab = self._aes.get(b'y', b'')
        if isinstance(ylab, (str, six.text_type)):
            ylab = element_text(ylab)
        if ylab:
            ylab.override(0.05, 0.5, dict(rotation=b'vertical'))
    ...
    def apply_axis_scales(self):
        if self.scale_x_log:
            for ax in self._iterate_subplots():
                ax.set_xscale(b'log', basex=self.scale_x_log)

        if self.scale_y_log:
            for ax in self._iterate_subplots():
                ax.set_yscale(b'log', basey=self.scale_y_log)
    ...
    def _construct_plot_data(self):
        ...
        for aes_type, colname in discrete_aes:
            ...
            mappers[aes_type] = {b'name': colname, b'lookup': mapper}
            data[colname + b'_' + aes_type] = self.data[colname].apply(lambda x: mapper[x])
            self._aes.data[aes_type] = colname + b'_' + aes_type

        discrete_aes_types = [ aes_type for aes_type, _ in discrete_aes ]
        if b'color' in self._aes.data and b'color' not in discrete_aes_types:
            if self.colormap:
                self._aes.data[b'colormap'] = cmap = self.colormap
            else:
                self._aes.data[b'colormap'] = cmap = LinearSegmentedColormap.from_list(b'gradient2n', [b'#1f3347', b'#469cef'])
            colname = self._aes.data[b'color']
            quantiles_actual = quantiles = data[colname].quantile([0.0, 0.2, 0.4, 0.5, 0.6, 0.75, 1.0])
            quantiles = (quantiles - quantiles.min()) / quantiles.max()
            mappers[b'color'] = {b'name': colname, b'lookup': {}}
            colors = cmap(quantiles)
            for i, q in enumerate(quantiles_actual):
                mappers[b'color'][b'lookup'][q] = colors[i]

            data[colname] = (data[colname] - data[colname].min()) / data[colname].max()
        if b'alpha' in self._aes.data and b'alpha' not in discrete_aes_types:
            colname = self._aes.data[b'alpha']
            quantiles = data[colname].quantile([0.0, 0.2, 0.4, 0.5, 0.6, 0.75, 0.95])
            quantiles_scaled = (quantiles - quantiles.min()) / quantiles.max()
            mappers[b'alpha'] = {b'name': colname, 
               b'lookup': dict(zip(quantiles.values, quantiles_scaled.values))}
            data[colname] = (data[colname] - data[colname].min()) / data[colname].max()
            discrete_aes.append((b'alpha', colname))
        if b'size' in self._aes.data and b'size' not in discrete_aes_types:
            colname = self._aes.data[b'size']
            quantiles = data[colname].quantile([0.0, 0.2, 0.4, 0.5, 0.6, 0.75, 0.95])
            quantiles_scaled = (quantiles - quantiles.min()) / quantiles.max()
            mappers[b'size'] = {b'name': colname, 
               b'lookup': dict(zip(quantiles.values, 100 * quantiles_scaled.values))}
            data[colname] = 100 * (data[colname] - data[colname].min()) / data[colname].max()
            discrete_aes.append((b'size', colname))
    ...
    def make_facets(self):
        ...
        if self.facets:
            if self.facets.scales == b'free':
                sharex, sharey = False, False
            elif self.facets.scales == b'free_x':
                sharex, sharey = False, True
            elif self.facets.scales == b'free_y':
                sharex, sharey = True, False
        facet_params = dict(sharex=sharex, sharey=sharey)
        nrow, ncol = self.facets.nrow, self.facets.ncol
        facet_params[b'nrows'] = nrow
        facet_params[b'ncols'] = ncol
        if self.coords == b'polar':
            facet_params[b'subplot_kw'] = {b'polar': True}
    ...
    def get_subplot(self, row, col):
        ...
        if row is not None and col is not None:
            ...
            raise Exception(b'row and col were none!' + str(row) + b', ' + str(col))
            return
            ...
            if self.facets.is_wrap == True:
                ...
                for item in self.facets.generate_subplot_index(self.data, self.facets.rowvar, self.facets.colvar):
                    ...
                    if isinstance(item, tuple):
                        title = (b', ').join([ str(i) for i in item ])
                    else:
                        title = str(item)
                ...
                for _, (colname, subgroup) in enumerate(group.groupby(col_variable)):
                    ...
                    ax = self.subplots[col]
                    if self.facets.is_wrap == True:
                        ax.set_title(b'%s=%s' % (col_variable, colname))
                    else:
                        ax.set_title(colname, fontdict={b'fontsize': 10})
                    yield (
                     ax, subgroup)
                ...
                for row, (rowname, subgroup) in enumerate(group.groupby(row_variable)):
                    ...
                    if self.facets.is_wrap == True:
                        ax = self.subplots[row]
                        ax.set_title(b'%s=%s' % (row_variable, rowname))
                    else:
                        ax = self.subplots[row]
                        ax.yaxis.set_label_position(b'right')
                        ax.yaxis.labelpad = 10
    ...
    def save_as_base64(self, as_tag=False, width=None, height=None, dpi=180):
        ...
        imgdata.seek(0)
        uri = b'data:image/png;base64,' + urllib.quote(base64.b64encode(imgdata.buf))
        if as_tag == True:
            return b'<img src = "%s"/>' % uri
        else:
            return uri
    ...
    def _prep_layer_for_plotting(self, layer, facetgroup):
        ...
        """
        if layer.__class__.__name__ == b'geom_bar':
            mask = True
            ...
            if self.facets:
                ...
                df = df[mask]
            if b'fill' in self._aes:
                fillcol_raw = self._aes[b'fill'][:-5]
                fillcol = self._aes[b'fill']
                fill_levels = self.data[[fillcol_raw, fillcol]].sort(fillcol_raw)[fillcol].unique()
            else:
                fill_levels = None
            return dict(x_levels=self.data[self._aes[b'x']].unique(), fill_levels=fill_levels, lookups=df)
        else:
            if layer.__class__.__name__ in ('geom_boxplot', 'geom_violin', 'geom_errorbar'):
                x_levels = list(pd.Series(self.data[self._aes[b'x']].unique()).sort_values())
