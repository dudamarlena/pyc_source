...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-i386/egg/tw/extjs/base.py
# Compiled at: 2008-09-03 16:29:00
import re, os
from pkg_resources import resource_filename
from tw.api import JSLink, CSSLink, Link
__all__ = [
 'all', 'all_debug', 'core', 'core_debug', 'all_css', 'gray_theme', 'locale']
adapter = JSLink(modname='tw.extjs', filename='static/adapter/ext/ext-base.js')
all_css = CSSLink(modname='tw.extjs', filename='static/resources/css/ext-all.css')
gray_theme = CSSLink(modname='tw.extjs', filename='static/resources/css/ytheme-gray.css')
...
all = ExtJSLinkAutoInit(modname='tw.extjs', filename='static/ext-all.js', javascript=[
 adapter])
all_debug = all
core = ExtJSLinkAutoInit(modname='tw.extjs', filename='static/ext-core.js', javascript=[
 adapter])
core_debug = core
...
class LocaleBunch(object):
    ...
    def __init__(self, modname, locale_dir, locale_re):
        ...
        for fname in os.listdir(resource_filename(modname, locale_dir)):
            ...
            if m:
                link = JSLink(modname=modname, location='bodytop', filename=os.path.join(locale_dir, fname))
                setattr(self, m.group(1), link)


locale = LocaleBunch('tw.extjs', 'static/locale', 'extjs-lang-([^\\.]+).js')