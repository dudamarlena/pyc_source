...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/konduit/inference.py
# Compiled at: 2020-01-02 02:10:00
...
try:
    ...
    try:
        raise Exception("We couldn't initialize the Java classes used by Konduit. Make sure to have a konduit.jar available on your PATH, for instance by setting the environment variable KONDUIT_JAR_PATH to point to such a JAR.\n", e)
    finally:
        e = None
        del e
...
def set_input_func(self, schema=None, column_names=None, types=None, input_name='default'):
    ...
    if input_name not in names:
        ...
        return self
    raise Exception('Input name' + input_name + ' is already configured for this PipelineStep, ' + 'please choose another name for your next step.')
...
def set_output_func(self, schema=None, column_names=None, types=None, output_name='default'):
    ...
    if output_name not in names:
        ...
        return self
    raise Exception('Input name' + output_name + ' is already configured for this PipelineStep, ' + 'please choose another name for your next step.')
...
def python_step_func(self, python_config, input_name='default', input_schema=None, input_column_names=None, input_types=None, output_schema=None, output_column_names=None, output_types=None):
    if not input_name:
    ...
    self.set_input(schema=input_schema,
      column_names=input_column_names,
      types=input_types,
      input_name=input_name)
    self.set_output(schema=output_schema,
