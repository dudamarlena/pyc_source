...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/test_docs.py
# Compiled at: 2020-04-03 13:49:55
# Size of source mod 2**32: 6702 bytes
import glob, pytest, re
from konduit import *
...
def markdown_blocks(file_path, language='python'):
    ...
    code_blocks = []
    code_regex = '^```.*'
    code_block_open_re = '^```(`*)({0})$'.format(language)
    with open(file_path, 'r') as (f):
        ...
        for line in f.readlines():
            ...
            if in_code_block:
                ...
                in_code_block = True
                python = re.match(code_block_open_re, line) or False
...
def write_self_contained_example(block):
    ...
    """
    class_regex = 'public\\s+class\\s+(\\w+)'
