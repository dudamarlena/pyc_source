...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/konduit/server.py
# Compiled at: 2020-05-07 11:40:56
# Size of source mod 2**32: 7744 bytes
import json, logging, sys, os, re, requests, signal, subprocess, uuid
...
class Server(object):
    ...
    def __init__(self, inference_config=None, serving_config=None, steps=None, extra_start_args='-Xmx8g', extra_jar_args=None, config_path='config.json', jar_path=None, pid_file_path='konduit-serving.pid'):
        """Konduit Server
        ...
        self.port = -1
        if jar_path is None:
            jar_path = os.getenv('KONDUIT_JAR_PATH', os.path.join(KONDUIT_JAR_DIR, 'konduit.jar'))
        if inference_config:
            ...
            if serving_config:
                if steps:
                    if isinstance(steps, PipelineStep):
                        steps = [
                         steps]
                    self.config = InferenceConfiguration(steps=steps,
    ...
    def get_client(self, input_data_format=None, prediction_type=None, output_data_format=None):
        ...
        if not host:
            ...
            if not host.startswith('http://'):
                ...
                if not prediction_type:
                    prediction_type = 'RAW'
                output_data_format = output_data_format or serving_config._get_output_data_format()
    ...
    def start(self, server_id=None):
        ...
        if port != -1:
            try:
                r = requests.get(('http://localhost:{}/healthcheck'.format(port)),
