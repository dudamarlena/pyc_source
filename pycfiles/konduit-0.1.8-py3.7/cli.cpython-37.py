...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/cli.py
# Compiled at: 2020-05-07 12:40:27
# Size of source mod 2**32: 8294 bytes
import os, sys, requests, subprocess, click
...
INCOMPATIBLE_COMPILATION_TAGS = [
 'cli_base', 'cli_base_2', 'cli_base_3', 'cli_base_4']
DOWNLOAD_TAG = 'cli_base'
LAST_COMPATIBLE_KONDUIT_VERSION = '0.1.0-SNAPSHOT'
DEFAULT_KONDUIT_TAG = 'cli_base_4'
KONDUIT_JAR_URL_FORMAT = 'https://github.com/KonduitAI/konduit-serving/releases/download/{tag}/konduit-serving-uberjar-{version}-{spin}-{platform}-{chip}.jar'
os_choices = [
 'windows-x86_64',
 'linux-x86_64',
 'macosx-x86_64',
...
def download_if_required(url, save_path):
    ...
    with open(save_path, 'wb') as (f):
        if total is None:
            ...
            downloaded = 0
            total = int(total)
            for data in response.iter_content(chunk_size=(max(int(total / 1000), 1048576))):
                downloaded += len(data)
                f.write(data)
                done = int(50 * downloaded / total)
                sys.stdout.write('\r[{}{}]'.format('█' * done, '.' * (50 - done)) + ' ({}/{})'.format(size(downloaded), size(total)))
                sys.stdout.flush()
...
def git_clone_konduit(use_https=True, tag=DEFAULT_KONDUIT_TAG):
    ...
    try:
        if not os.listdir(KONDUIT_SOURCE_DIR):
            subprocess.call(['git', 'clone', repo, KONDUIT_SOURCE_DIR], shell=(sys.platform.startswith('win')))
        subprocess.call(['git', 'checkout', tag], cwd=KONDUIT_SOURCE_DIR, shell=(sys.platform.startswith('win')))
    except Exception as e:
        try:
            raise RuntimeError(">>> Could not clone konduit-serving repository and switch to commit hash {}Make sure to have git installed. Type 'konduit-init --help' for help .\n".format(tag), e)
        finally:
            e = None
            del e
...
def build_jar(operating_sys, spin, chip):
    ...
    if not os.path.exists(KONDUIT_JAR_DIR):
        ...
        try:
            ...
            try:
                raise RuntimeError('>>> Could not fetch and pull changes from konduit-serving repository. Make sure to have git installed. Type konduit-init --help for help resolving this.\n', e)
            finally:
                e = None
                del e
...
def get_jar_url(platform, version, spin, chip):
    ...
    return KONDUIT_JAR_URL_FORMAT.format(version=version, platform=platform,
      tag=DOWNLOAD_TAG,
      spin=spin,
      chip=chip)
...
if len(git_tags) == 0:
    git_tags = [
     DEFAULT_KONDUIT_TAG]
DEFAULT_KONDUIT_TAG = git_tags[0]

@click.command()
@click.option('-p',
  '--platform', help='Your operating system. Defaults to the cpu version of the current OS platform in use.',
...
@click.option('-t',
  '--tag', default=DEFAULT_KONDUIT_TAG,
  show_default=True,
...
@click.option('-s',
  '--spin', default='all',
  show_default=True,
  help="Whether to bundle Python ('python'), PMML ('pmml'), both ('all') or neither ('minimal'). Python bundling is not encouraged with ARM, and PMML bundling is not encouraged if agpl license is an issue.")
@click.option('-c',
  '--chip', default='cpu',
  show_default=True,
...
@click.option('-v',
  '--version', default=LAST_COMPATIBLE_KONDUIT_VERSION,
  show_default=True,
  help='Only works with the `--download` option to specify the version of konduit-serving to be downloaded.')
@click.option('-d',
  '--download', is_flag=True,
  help='If set, downloads the pre-built jar file instead of building it. Also works with `--platform`, `--chip`, `--version` and `--spin` options.')
def init(platform, https, tag, spin, chip, version, download):
    if download:
        if parse(LAST_COMPATIBLE_KONDUIT_VERSION.replace('-SNAPSHOT', '')) < parse(version.replace('-SNAPSHOT', '')):
            print('This version of Python CLI is only compatible with versions <= {}'.format(LAST_COMPATIBLE_KONDUIT_VERSION))
        else:
...
def cli():
    if not os.path.exists(KONDUIT_JAR_PATH):
        print("No konduit binaries found. See 'konduit-init --help' or just run 'konduit-init' to initialize a konduit jar binary.")
    else:
        arguments = [
         'java', '-jar', '-Dvertx.cli.usage.prefix=konduit', KONDUIT_JAR_PATH]
