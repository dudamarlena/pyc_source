...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tests/test_transform_process.py
# Compiled at: 2020-04-03 13:49:55
# Size of source mod 2**32: 1667 bytes
import json, pydatavec, pytest
from konduit import *
...
def test_build_tp(output_format):
    ...
    transform_process = TransformProcessStep().set_input(column_names=[
     'first'],
      types=['String']).set_output(column_names=[
     'first'],
      types=['String']).transform_process(as_python_json)
    serving_config = ServingConfig(output_data_format=output_format,
      log_timings=True)
