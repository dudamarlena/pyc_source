...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/konduit/client.py
# Compiled at: 2020-03-30 12:08:11
# Size of source mod 2**32: 9691 bytes
import io, json, logging, numpy as np, re, requests
...
class Client(object):
    ...
    def __init__(self, port, host='http://localhost', output_data_format=None, input_data_format=None, prediction_type=None, timeout=60, input_names=None, output_names=None):
        """Konduit Client
        ...
        if server_running:
            try:
                ...
                try:
                    logging.error('{}\nUnable to get configuration from the server. Please verify that the server is running without any issues...'.format(str(ex)))
            ...
            if not server_running:
                if insufficient_data:
                    raise RuntimeError('Unable to connect to the server at {}, not enough data provided to initialize the Client'.format(url))
                else:
                    logging.info('No server-side validation needed, enough information available to create Client.')
            else:
    ...
    def predict(self, data_input=None):
        if isinstance(data_input, np.ndarray):
            ...
            if data_input is None:
                data_input = {}
            elif self.input_format.upper() == 'JSON':
                resp = requests.post(('{}/{}/{}'.format(self.url, self.prediction_type.lower(), self.input_format.lower())),
                  json=data_input,
            ...
            if resp.status_code != 200:
                raise Exception('Inference failed with error code: {}. See the error log below for more info:\n---\n{}\n---\n'.format(resp.status_code, str(resp.content, 'utf-8')))
            else:
        ...
        try:
            if self.output_names is None or len(self.output_names) < 2:
                ...
                if self.convert_to_format.upper() == 'NUMPY':
                    ...
                    if self.convert_to_format.upper() == 'ARROW':
                        reader = RecordBatchFileReader(bytes_content)
                        return reader.read_pandas()
                    if self.convert_to_format.upper() == 'JSON':
                        return json.load(bytes_content)
                    if self.convert_to_format.upper() == 'IMAGE':
    ...
    def _convert_multi_part_output(self, content, content_type):
        ...
        for part in multipart_data.parts:
            ...
            name_str = re.sub('([";\\\\\\\']|name=|form-data|b\\\\)', '', name_with_form_data).replace('b ', '')
            if self.prediction_type.upper() == 'NUMPY':
                ...
                if self.prediction_type.upper() == 'ND4J':
