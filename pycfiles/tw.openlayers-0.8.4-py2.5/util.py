...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/tw/openlayers/util.py
# Compiled at: 2008-08-17 14:25:50
...
def inject_apis(layers):
    ...
    for layer in layers:
        if layer.__class__.__name__ == 'Google':
            JSLink(link='http://maps.google.com/maps?file=api&amp;v=2&amp;key=%s' % layer.apikey).inject()
        if layer.__class__.__name__ == 'Yahoo':
            JSLink(link='http://api.maps.yahoo.com/ajaxymap?v=3.0&amp;appid=%s' % layer.apikey).inject()
        if layer.__class__.__name__ == 'VirtualEarth':
            JSLink(link='http://dev.virtualearth.net/mapcontrol/v3/mapcontrol.js%s' % layer.apikey).inject()
        if layer.__class__.__name__ in ('OSMMapnik', 'OSMRenderer'):
            JSLink(modname=__name__, filename='static/javascript/OpenStreetMap.js').inject()
