...
# Embedded file name: \Ft\Lib\DistExt\BuildPy.py
# Compiled at: 2006-08-13 18:26:54
import os, sys, glob
...
class BuildPy(build_py.build_py):
    ...
    def copy_file(self, src, dest, *args, **kwds):
        ...
        """
        if src.endswith('.py') or sys.platform == 'win32' and src.endswith('.pyw'):
            try:
                f = open(src, 'rU')
            except:
                f = open(src, 'r')
            ...
            try:
                ...
                if not filename:
                    filename = src
                L = [
                 'Syntax error in file "%s", line %d: %s' % (filename, lineno, msg)]
                if line is not None:
                    i = 0
