...
# [GCC 8.4.0]
# Embedded file name: /Users/cdent/src/tiddlywebplugins.mysql/test/test_search.py
...
def test_limited_search():
    tiddlers = list(store.search('starts _limit:1'))
    assert len(tiddlers) == 1, tiddlers
    tiddlers = list(store.search('starts'))
    assert len(tiddlers) != 1, tiddlers
    tiddlers = list(store.search('starts _limit:so'))
    assert len(tiddlers) != 1, tiddlers
...
def test_text_as_field():
    tiddlers = list(store.search('text:hello'))
    assert len(tiddlers) == 4, tiddlers


def test_srevision_attr():
    tiddlers = list(store.search('fields:hello'))
