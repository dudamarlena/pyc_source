...
# [GCC 8.4.0]
# Embedded file name: /home/fsuba/Pictures/plbmng-0.3.7/plbmng/lib/planetlab_list_creator.py
# Compiled at: 2019-04-30 03:47:16
# Size of source mod 2**32: 12641 bytes
import xmlrpc.client, geocoder, codecs, argparse, sys, os, socket, traceback
...
locations = {}
continents = {'AD':'EU', 
 'AE':'AS',  'AF':'AS',  'AG':'NA',  'AI':'NA',  'AL':'EU',  'AM':'AS',  'AN':'NA',  'AO':'AF',  'AQ':'AQ', 
 'AR':'SA',  'AS':'OC',  'AT':'EU',  'AU':'OC',  'AW':'NA',  'AZ':'AS',  'BA':'EU',  'BB':'NA',  'BD':'AS', 
 'BE':'EU',  'BF':'AF',  'BG':'EU',  'BH':'AS',  'BI':'AF',  'BJ':'AF',  'BM':'NA',  'BN':'AS',  'BO':'SA', 
 'BR':'SA',  'BS':'NA',  'BT':'AS',  'BW':'AF',  'BY':'EU',  'BZ':'NA',  'CA':'NA',  'CC':'AS',  'CD':'AF', 
 'CF':'AF',  'CG':'AF',  'CH':'EU',  'CI':'AF',  'CK':'OC',  'CL':'SA',  'CM':'AF',  'CN':'AS',  'CO':'SA', 
 'CR':'NA',  'CU':'NA',  'CV':'AF',  'CX':'AS',  'CY':'AS',  'CZ':'EU',  'DE':'EU',  'DJ':'AF',  'DK':'EU', 
 'DM':'NA',  'DO':'NA',  'DZ':'AF',  'EC':'SA',  'EE':'EU',  'EG':'AF',  'EH':'AF',  'ER':'AF',  'ES':'EU', 
 'ET':'AF',  'FI':'EU',  'FJ':'OC',  'FK':'SA',  'FM':'OC',  'FO':'EU',  'FR':'EU',  'GA':'AF',  'GB':'EU', 
 'GD':'NA',  'GE':'AS',  'GF':'SA',  'GG':'EU',  'GH':'AF',  'GI':'EU',  'GL':'NA',  'GM':'AF',  'GN':'AF', 
 'GP':'NA',  'GQ':'AF',  'GR':'EU',  'GS':'AQ',  'GT':'NA',  'GU':'OC',  'GW':'AF',  'GY':'SA',  'HK':'AS', 
 'HN':'NA',  'HR':'EU',  'HT':'NA',  'HU':'EU',  'ID':'AS',  'IE':'EU',  'IL':'AS',  'IM':'EU',  'IN':'AS', 
 'IO':'AS',  'IQ':'AS',  'IR':'AS',  'IS':'EU',  'IT':'EU',  'JE':'EU',  'JM':'NA',  'JO':'AS',  'JP':'AS', 
 'KE':'AF',  'KG':'AS',  'KH':'AS',  'KI':'OC',  'KM':'AF',  'KN':'NA',  'KP':'AS',  'KR':'AS',  'KW':'AS', 
 'KY':'NA',  'KZ':'AS',  'LA':'AS',  'LB':'AS',  'LC':'NA',  'LI':'EU',  'LK':'AS',  'LR':'AF',  'LS':'AF', 
 'LT':'EU',  'LU':'EU',  'LV':'EU',  'LY':'AF',  'MA':'AF',  'MC':'EU',  'MD':'EU',  'ME':'EU',  'MG':'AF', 
 'MH':'OC',  'MK':'EU',  'ML':'AF',  'MM':'AS',  'MN':'AS',  'MO':'AS',  'MP':'OC',  'MQ':'NA',  'MR':'AF', 
 'MS':'NA',  'MT':'EU',  'MU':'AF',  'MV':'AS',  'MW':'AF',  'MX':'NA',  'MY':'AS',  'MZ':'AF',  'NA':'AF', 
 'NC':'OC',  'NE':'AF',  'NF':'OC',  'NG':'AF',  'NI':'NA',  'NL':'EU',  'NO':'EU',  'NP':'AS',  'NR':'OC', 
 'NU':'OC',  'NZ':'OC',  'OM':'AS',  'PA':'NA',  'PE':'SA',  'PF':'OC',  'PG':'OC',  'PH':'AS',  'PK':'AS', 
 'PL':'EU',  'PM':'NA',  'PN':'OC',  'PR':'NA',  'PS':'AS',  'PT':'EU',  'PW':'OC',  'PY':'SA',  'QA':'AS', 
 'RE':'AF',  'RO':'EU',  'RS':'EU',  'RU':'EU',  'RW':'AF',  'SA':'AS',  'SB':'OC',  'SC':'AF',  'SD':'AF', 
 'SE':'EU',  'SG':'AS',  'SH':'AF',  'SI':'EU',  'SJ':'EU',  'SK':'EU',  'SL':'AF',  'SM':'EU',  'SN':'AF', 
 'SO':'AF',  'SR':'SA',  'ST':'AF',  'SV':'NA',  'SY':'AS',  'SZ':'AF',  'TC':'NA',  'TD':'AF',  'TF':'AQ', 
 'TG':'AF',  'TH':'AS',  'TJ':'AS',  'TK':'OC',  'TM':'AS',  'TN':'AF',  'TO':'OC',  'TR':'AS',  'TT':'NA', 
 'TV':'OC',  'TW':'AS',  'TZ':'AF',  'UA':'EU',  'UG':'AF',  'US':'NA',  'UY':'SA',  'UZ':'AS',  'VC':'NA', 
 'VE':'SA',  'VG':'NA',  'VI':'NA',  'VN':'AS',  'VU':'OC',  'WF':'OC',  'WS':'OC',  'YE':'AS',  'YT':'AF', 
 'ZA':'AF',  'ZM':'AF',  'ZW':'AF'}
...
def arguments():
    ...
    """
    global auth
    parser = argparse.ArgumentParser(prog='planetlab_list_creator',
      formatter_class=(argparse.RawDescriptionHelpFormatter),
      description='Plb list creator is script, which will return you file with all available nodes from www.planet-lab.eu. \nRequired arguments: USERNAME, PASSWORD and OUTPUT.')
...
def append_to_file(node):
    ...
    try:
        with codecs.open((os.path.join(arg['path'] + '/database/default2.node')), mode='a',
          encoding='utf-8') as (f):
            f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(str(arg['id']), node['ip'], node['hostname'], node['continent'], node['country'], node['region'], node['city'], node['url'], node['name'], node['latitude'], node['longitude']))
    except Exception as err:
        try:
            traceback.print_exc()
            print(err)
    ...
    try:
        ...
        for node in all_nodes:
            ...
            if node['site_id'] in list(locations.keys()):
                ...
                node.update({'city':location[0], 
                 'region':location[1],  'country':location[2],  'continent':location[3]})
            else:
                sleep(1)
                if node['latitude'] is not None:
                    if node['longitude'] is not None:
                        ...
                        locations.update({node['site_id']: [node['city'], node['region'], node['country'], node['continent']]})
                    else:
                        node.update({'city':'unknown', 
                         'region':'unknown',  'country':'unknown',  'continent':'unknown'})
                else:
...
if __name__ == '__main__':
    ...
    run()
    print('Finish: %s' % asctime(localtime(time())))
# global plc_host ## Warning: Unused global