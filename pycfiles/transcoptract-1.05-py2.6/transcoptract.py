...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv5tel/egg/transcoptract/transcoptract.py
# Compiled at: 2011-09-29 20:06:06
import os, re, subprocess, logging, logging.handlers, shutil, sys, argparse, transmissionrpc, settings
logging.basicConfig()
...
class TransCopTract(object):
    ...
    def process(self, torrent_ids, dest_root, overwrite):
        ...
        logger.info(('Processing torrent(s): {0}').format(torrent_ids))
        for (current_torrent_id, torrent) in self.transmission.info(torrent_ids).items():
            logger.info(('Torrent Id: {0}').format(current_torrent_id))
            ...
            for (file_id, file) in torrent.files().items():
                ...
                try:
                    ...
                    elif not os.path.exists(os.path.join(dest, filename)) or overwrite:
    ...
    def unrar(self, filepath, dest, overwrite):
        ...
        if re.match(self.main_rar_file_match, os.path.basename(filepath), flags=re.IGNORECASE):
            ...
            if unrar.returncode != 0:
                logger.error('Error running unrar' + results[0])
        else:
