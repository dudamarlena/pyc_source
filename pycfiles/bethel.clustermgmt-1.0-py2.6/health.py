...
# [GCC 8.4.0]
# Embedded file name: /home/aaltepet/silva/dev-silva2.3/src/bethel.clustermgmt/src/bethel/clustermgmt/health.py
# Compiled at: 2012-04-30 10:18:55
import urllib
...
class HealthReporter(ZMIObject):
    ...
    manage_options = (
     {'label': 'Manage', 'action': 'managenodes'},) + ZMIObject.manage_options
...
manage_addHealthReporterForm = PageTemplateFile('healthReporterAdd', globals(), __name__='manage_addHealthReporterForm')
...
def nodes_source(context):
    ...
    for node in context.nodes_in_cluster:
        terms.append(SimpleTerm(value=node, title=node, token=node))
...
class IManageNodesSchema(Interface):
    ...
    """
    nodes_in_cluster = schema.Text(title='Nodes', description='Name of nodes, one per line', required=True)


class IOfflineNodesSchema(Interface):
    """ Schema for managing which nodes are offline 
    """
    offline_nodes = schema.Set(title='Offline Nodes', description='The following nodes are offline', required=False, value_type=schema.Choice(title='node', source=nodes_source))
