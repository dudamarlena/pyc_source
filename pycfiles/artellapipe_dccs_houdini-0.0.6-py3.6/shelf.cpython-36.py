...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/dccs/houdini/managers/shelf.py
...
class ArtellaHoudiniShelfManager(core_shelf.ArtellaShelfManager, object):
    ...
    def _add_action(self, item_info, shelf_name):
        ...
        if tool_type == 'command':
            ...
            tool_command = "import artellapipe; artellapipe.ToolsMgr().run_tool(artellapipe.{}, '{}', do_reload=False, debug=False)".format(self._project.get_clean_name(), tool_id)
            tool_icon = None
            if icon_name:
    ...
    def _add_tool(self, label, icon='customIcon.png', command=None, command_type='python'):
        return gui.create_shelf_tool(tool_name=('{}_{}'.format(self._shelf_name, label)),
          tool_label=label,
