...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/strainer/middleware.py
# Compiled at: 2011-03-21 01:30:30
"""Provides WSGI middleware for validating and tidying HTML output."""
import re, xhtmlify, logging
try:
...
class XHTMLifyMiddleware(BufferingMiddleware):
    ...
    def filter(self, status, headers, exc_info, response):
        ...
        if len(parts) == 2:
            ...
            rest = ''
        encoding = re.search('charset\\s*=\\s*("[A-Za-z0-9_-]*"|\'[A-Za-z0-9_-]*\'|[A-Za-z0-9_-]*)', rest)
        if encoding:
