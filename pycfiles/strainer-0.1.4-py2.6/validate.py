...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/strainer/validate.py
# Compiled at: 2011-03-21 01:30:30
"""Provides XHTML 1.0 validation using lxml."""
import lxml.etree, os, re, urlparse
try:
...
__all__ = [
 'validate_xhtml', 'validate_xhtml_fragment', 'XHTMLSyntaxError',
...
def validate_xhtml(xhtml, doctype=''):
    ...
    try:
        ...
        tline = doctype.count('\n')
        message = re.sub('line (\\d+)', lambda m: 'line %s' % (int(m.group(1)) - tline), e.message)
