...
# [GCC 8.4.0]
# Embedded file name: C:\Users\SIGPC4\Desktop\atamsPackage\build\lib\atams\atams.py
# Compiled at: 2018-09-27 02:43:30
"""
---------------------------------------------------
...
StructSatellite = STRUCTSATELLITE()
app = Flask(__name__)
OPERATOR_PW = 'O3b'
INSTALLER_PW = 'O3b4u!'
...
def uploadSDB():
    ...
    for file in request.files.getlist('spcfile'):
        ...
        destination = os.path.join(SDB_VALID_DIR, filename)
        file.save(destination)
        windll.kernel32.MoveFileExW(os.path.join(SDB_VALID_DIR, filename), os.path.join(SDB_VALID_DIR, 'spacecraft'), 1)
