...
# [GCC 8.4.0]
# Embedded file name: C:\Users\SIGPC4\Desktop\atamsPackage\build\lib\atams\schevents.py
# Compiled at: 2018-09-26 06:54:11
"""
---------------------------------------------------
...
"""
import time, xml.etree.ElementTree as ET, os, datetime, numpy as np
        ...
        try:
            for node in tree.iter('Track'):
                for element in node.iter('Handover'):
                    ...
                    if Handover_time > EpochTime - 86400:
...
def GenerateEvents(SCH, iSatelliteCount, iPeriod, iCurrentAntenna, EventCount, schtype, iHandoverCount):
    ...
    if EventCount == 4:
        ...
        for i in range(iSatelliteCount):
            SCHEVENTS[index] = (
             'Retrace/StartTime', SCH[i]['dwHandoverTime'], time.strftime('%m/%d/%Y %H:%M:%S', time.gmtime(SCH[i]['dwHandoverTime'])), SCH[i]['strSatelliteID'], iCurrentAntenna)
        ...
        for i in range(iSatelliteCount):
            ...
            SCHEVENTS[index] = (
             'Retrace/StartTime', SCH[i]['dwStartTime'], time.strftime('%m/%d/%Y %H:%M:%S', time.gmtime(SCH[i]['dwStartTime'])), SCH[i]['strSatelliteID'], iCurrentAntenna)
...
def SchEvents(ScheduleFilepath, Beam, EpochTime, iMode, newsch, iCurrentAntenna, iSchUploadTime, iHandoverCount):
    with lock:
        ...
        if newsch == 1:
            ...
            if Valid == True:
                if EpochTime > dwStartTime:
                    bInstantSwitch = False
                    if strAction == 'ADD':
                        SCHEVENTS_OLD = SCHEVENTS[0:2 * EventCount]
                        if SCH[0]['dwHandoverTime'] + iHandoverCount < EpochTime < SCH[1]['dwHandoverTime'] + iHandoverCount:
                        ...
                        SCHEVENTS_OLD = SCHEVENTS[0:2 * EventCount]
                        if SCH[0]['dwHandoverTime'] + iHandoverCount < EpochTime < SCH[1]['dwHandoverTime'] + iHandoverCount:
                            OldAntID_PRI = iCurrentAntenna
                            OldAntID_SEC = 3 - iCurrentAntenna
                        else:
                            OldAntID_PRI = 3 - iCurrentAntenna
                            OldAntID_SEC = iCurrentAntenna
                        if EpochTime < SCH[0]['dwHandoverTime'] + iHandoverCount:
                            OldSatID_PRI = SCH[(-1)]['strSatelliteID']
                            OldSatID_SEC = SCH[0]['strSatelliteID']
                        elif SCH[0]['dwHandoverTime'] + iHandoverCount < EpochTime < SCH[1]['dwStartTime']:
                    ...
                    if 0 < len(FutureEvents) < len(SCHEVENTS):
                        ...
                        if EventCount != 4:
                            if SCHEVENTS_OLD[(-1)][0].rstrip() not in ('MuteTime',
                                                                       'EndTime',
                                                                       'DC Retrace/StartTime'):
                                OldSatID = SatelliteEvents[(-2)]
                                iCurrentAntenna = OldAntID
                            else:
                                iCurrentAntenna = 3 - OldAntID
                        SCH, iPeriod, strAction, iTrackIndex, iSatelliteCount, ret, strError = ScheduleTracks(ScheduleFilepath, Beam, EpochTime, iMode, schtype)
                        SCH.sort(order='dwStartTime')
                        SCHEVENTS_NEW = GenerateEvents(SCH, iSatelliteCount, iPeriod, iCurrentAntenna, EventCount, schtype, iHandoverCount)
