...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/forms/widgets.py
# Compiled at: 2010-03-02 05:34:48
...
class ExtendedFileWidget(widgets.FileInput):
    """
    ...
    def __init__(self, attrs={}, *args, **kwargs):
        super(ExtendedFileWidget, self).__init__(attrs, *args, **kwargs)

    def render(self, name, value, attrs=None):
        output = []
        if value and hasattr(value, 'url'):
            output.append('<span class="filewidgetinfo">%s <a target="_blank" href="%s">%s</a></span> <br />' % (_('Currently:'), value.url, value))
        output.append(super(ExtendedFileWidget, self).render(name, value, attrs))
...
class WidgetHelper(object):
    ...
    def __init__(self, widget, *args, **kwargs):
        ...
        template_name = kwargs.pop('template_name', getattr(widget, 'template_name', getattr(widget, 'TEMPLATE_NAME', self.TEMPLATE_NAME)))
        template_loader = kwargs.pop('template_loader', getattr(widget, 'template_loader', getattr(widget, 'TEMPLATE_LOADER', self.TEMPLATE_LOADER)))
        self.widget = widget
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        d = dict(base_dict, basewidget=mark_safe(base_widget), field=mark_safe(name), id=widget_id)
...
class NullBooleanSelect(widgets.NullBooleanSelect):
    ...
    def render(self, name, value, attrs=None, choices=()):
        try:
            value = {True: '2', False: '3', '2': '2', '3': '3', 'True': '2', 'False': '3'}[value]
        except KeyError:
            value = '1'
    ...
    def value_from_datadict(self, data, files, name):
        ...
        return {'2': True, '3': False, True: True, False: False, 'True': True, 'False': False}.get(value, None)
...
class DateTimeWidget(forms.widgets.TextInput):
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        if value != '':
            try:
                final_attrs['value'] = force_unicode(value.strftime(self.get_date_format()))
            except:
        ...
        id = final_attrs['id']
        jsdformat = self.get_date_format()
        return dict(base_dict, flatattrs=mark_safe(forms.util.flatatt(final_attrs)), attrs=final_attrs, id=id, ifFormat=jsdformat)
...
import re
date_range_re = re.compile('([0-9]{2}-|/[0-9]{2}-|/[0-9]{4})?\\s+-\\s+([0-9]{2}-|/[0-9]{2}-|/[0-9]{4})?')
...
class DateRangeWidget(widgets.MultiWidget):
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe('<span class="daterange">%s</span>' % r)
...
class RelatedItemWidget(forms.widgets.TextInput):
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        return dict(base_dict, model=model, opts=opts, model_name=mark_safe(opts.object_name.lower()), id=id, value=value, obj=obj, obj_repr=mark_safe(obj_repr), tr_attrs=mark_safe(forms.util.flatatt(tr_attrs)), attrs=mark_safe(forms.util.flatatt(final_attrs)))
...
class SelectPopupWidget(forms.Select):
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        return dict(base_dict, basewidget=mark_safe(base_widget), field=mark_safe(name), model=model, opts=opts, multiple=False, instance=instance, instance_repr=mark_safe(instance_repr), obj_name=mark_safe(obj_name))
...
class SelectMultiplePopupWidget(forms.SelectMultiple):
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        opts = self.model._meta
        obj_name = opts.verbose_name
        return dict(base_dict, basewidget=mark_safe(base_widget), field=mark_safe(name), model=model, opts=opts, multiple=True, instance=None, instance_repr=None, obj_name=mark_safe(obj_name))
...
class SelectCascadeWidget(forms.Select):
    ...
    def __init__(self, model, slave_model, slave_pivot_field, attrs=None, choices=(), *args, **kwargs):
        template_name = kwargs.pop('template_name', self.TEMPLATE_NAME)
        template_loader = kwargs.pop('template_loader', self.TEMPLATE_LOADER)
    ...
    def get_context_dict(self, base_dict, name, value, attrs, base_widget, *args, **kwargs):
        ...
        obj_name = opts.verbose_name
        return dict(base_dict, basewidget=mark_safe(base_widget), field=mark_safe(name), model=model, opts=opts, master_model=opts.object_name, master_opts=opts, master_app=opts.app_label, slave_model=self.slave_model._meta.object_name, slave_opts=self.slave_model._meta, slave_app=self.slave_model._meta.app_label, slave_pivot_field=self.slave_pivot_field, cascade_url=self.cascade_url, master_selector=master_selector, slave_selector=slave_selector, multiple=False, instance=instance, instance_repr=mark_safe(instance_repr), obj_name=mark_safe(obj_name))
