...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/softwarefabrica/django/forms/extended.py
# Compiled at: 2010-03-02 05:32:37
...
class BaseForm(forms.BaseForm):
    ...
    def output_via_template(self, template_name=None, template_loader=None):
        ...
        c = Context(dict(form=self, bound_fields=bound_fields, global_errors=global_errors))
    ...
    @property
    def html_name(self):
        """Return form HTML name (eg for id HTML attribute)."""
        return self.add_prefix('form').replace('-', '_') or 'form'

    @property
    def html_row_name(self):
        """Return form field row HTML name prefix (eg for id HTML attribute)."""
        return self.add_prefix('fieldrow-form').replace('-', '_') or 'form'
    ...
    def html_id(self):
        ...
        if auto_id and '%s' in smart_unicode(auto_id):
            s = smart_unicode(auto_id) % self.html_name
            return s.replace('-', '_')
        elif auto_id:
            return self.html_name.replace('-', '_')
        return 'id_form'

    @property
    def html_row_id(self):
        """Return HTML id prefix for a field row."""
        from django.utils.encoding import smart_unicode
        auto_id = self.auto_id
        if auto_id and '%s' in smart_unicode(auto_id):
            s = smart_unicode(auto_id) % self.html_row_name
            return s.replace('-', '_')
        elif auto_id:
            return self.html_row_name.replace('-', '_')
        return 'id_fieldrow_form'
...
def extended_formfield_cb(field, *args, **kwargs):
    ...
    if isinstance(field, models.FileField):
        kwargs['widget'] = widgets.ExtendedFileWidget
        return field.formfield(**kwargs)
    if not old_datewidgets:
        if isinstance(field, models.DateTimeField):

            def fldconstructor(**defaults):
                return fields.DateTimeField(**defaults)

            return field.formfield(form_class=fldconstructor, **kwargs)
        if isinstance(field, models.DateField):

            def fldconstructor(**defaults):
                return fields.DateField(**defaults)
...
def modelform_factory(model, form=ModelForm, fields=None, exclude=None, formfield_callback=extended_formfield_cb):
    """
    ...
    return ModelFormMetaclass(class_name, (form,), {'Meta': Meta, 'formfield_callback': formfield_callback})
...
class BaseModelFormSet(models.BaseModelFormSet):
    ...
    def add_fields(self, form, index):
        ...
        self._pk_field = pk
        initial_form_count = None
        if hasattr(self, 'initial_form_count'):
...
def modelformset_factory(model, form=ModelForm, formfield_callback=extended_formfield_cb, formset=BaseModelFormSet, extra=1, can_delete=False, can_order=False, max_num=0, fields=None, exclude=None):
    """
    ...
    form = modelform_factory(model, form=form, fields=fields, exclude=exclude, formfield_callback=formfield_callback)
    FormSet = formset_factory(form, formset, extra=extra, max_num=max_num, can_order=can_order, can_delete=can_delete)
    FormSet.model = model
    return FormSet
...
class FormBuilder(object):
    ...
    class FormBuilderField(object):
        ...
        def __init__(self, fb, name, label=None, required=None, help_text=None, error_messages=None, initial=None, include_blank=None, null=None, field=None, widget=None, widget_args=None, widget_kwargs=None, validator=None, reference_model=None, name_in_model=None, **field_kwargs):
        ...
        def _setup(self):
            ...
            if self._db_field:
                ...
                if 'max_length' in self.field_kwargs:
                    if self.field_kwargs['max_length'] == -1:
                    ...
                    if not self.field or isinstance(self.field, type):
                        field_kwargs = dict(label=self.label, required=self.required, help_text=self.help_text, initial=self.initial)
                        ...
                        if self.widget:
                            if isinstance(self.widget, type):
                                widget = self.widget(*self.widget_args, **self.widget_kwargs)
                            else:
                                widget = self.widget
                            field_kwargs['widget'] = widget
                        self.field = self.field or self._db_field.formfield(**field_kwargs)
    ...
    def __init__(self, name, reference_model=None, validator=None, base_form_class=None, fields=None):
        """
    ...
    def add_field(self, name, label=None, required=None, help_text=None, error_messages=None, initial=None, field=None, widget=None, widget_args=None, widget_kwargs=None, validator=None, reference_model=None, name_in_model=None, **field_kwargs):
        fb_field = FormBuilder.FormBuilderField(self, name, label=label, required=required, help_text=help_text, error_messages=error_messages, initial=initial, field=field, widget=widget, widget_args=widget_args, widget_kwargs=widget_kwargs, validator=validator, reference_model=reference_model, name_in_model=name_in_model, **field_kwargs)
    ...
    def get_form_class(self):
        ...
        new_class = type(self.name, (base_form_class,), {'base_fields': base_fields})
