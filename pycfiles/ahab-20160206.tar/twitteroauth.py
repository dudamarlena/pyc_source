...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/twitteroauth/twitteroauth.py
# Compiled at: 2011-04-01 02:19:33
...
from plugin.twitteroauth.twitter_auth import TwitterOAuth, TWITTER_NAMESPACE, OAUTH_ACCESS_TOKEN_COOKIE, EXPIRE
...
class TwitteroauthController(MakoTemplateController):
    ...
    def _post_action(self, user):
        ...
        """
        if user:
            d = {'type': TwitterOAuth.TYPE, 'nickname': user.get('username', ''), 'email': '', 
               'userid': user.get('id', ''), 
            ...
            memcache.set(self.cookies.get(OAUTH_ACCESS_TOKEN_COOKIE), d, namespace=TWITTER_NAMESPACE, time=EXPIRE)
    ...
    def twitter_request(self, path, access_token, callback=None, post_args=None, **args):
        """
        A method to send request to twitter, by using informations in auth_obj.
        """
        api = self.auth_obj
        if not api:
            raise ValueError('auth_obj is None. You must do oauth authentication before sending request.')
        if not callback:
            callback = self._dummy_callback
        api.twitter_request(path, post_args=post_args, access_token=access_token, callback=callback)
