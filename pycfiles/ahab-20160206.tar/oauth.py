...
# Embedded file name: build/bdist.macosx-10.3-i386/egg/twitteroauth/oauth.py
# Compiled at: 2011-04-01 02:51:35
...
import binascii, cgi, functools, hashlib, hmac, logging, time, urllib, urlparse, uuid
...
class OAuthMixin(object):
    ...
    def authorize_redirect(self, callback_uri=None, oauth_authorize_url=None):
        ...
        return self._on_request_token(oauth_authorize_url, callback_uri, response)
    ...
    def get_authenticated_user(self, callback):
        ...
        try:
            ...
            if response.status_code < 200 or response.status_code >= 300:
                logging.warning('Invalid OAuth response: %s', response.content)
                response = None
        except urlfetch.DownloadError, e:
            logging.exception(e)
            response = None
    ...
    def _oauth_request_token_url(self):
        ...
        url = self._OAUTH_REQUEST_TOKEN_URL
        args = dict(oauth_consumer_key=consumer_token['key'], oauth_signature_method='HMAC-SHA1', oauth_timestamp=str(int(time.time())), oauth_nonce=binascii.b2a_hex(uuid.uuid4().bytes), oauth_version='1.0')
    ...
    def _on_request_token(self, authorize_url, callback_uri, response):
        ...
        if not response:
            ...
            logging.warning('Invalid OAuth response (%d): %s', response.status_code, response.content)
            return
        ...
        if callback_uri:
            args['oauth_callback'] = urlparse.urljoin(self.request.url, callback_uri)
    ...
    def _oauth_access_token_url(self, request_token):
        ...
        args = dict(oauth_consumer_key=consumer_token['key'], oauth_token=request_token['key'], oauth_signature_method='HMAC-SHA1', oauth_timestamp=str(int(time.time())), oauth_nonce=binascii.b2a_hex(uuid.uuid4().bytes), oauth_version='1.0')
    ...
    def _on_access_token(self, callback, response):
        ...
        if not response:
            ...
            logging.warning('Invalid OAuth access token response (%d): %s', response.status_code, response.content)
            return callback(None)
        access_token = _oauth_parse_response(response.content)
        return self._oauth_get_user(access_token, functools.partial(self._on_oauth_get_user, access_token, callback))
    ...
    def _oauth_request_parameters(self, url, access_token, parameters={}, method='GET'):
        ...
        """
        consumer_token = self._oauth_consumer_token()
        base_args = dict(oauth_consumer_key=consumer_token['key'], oauth_token=access_token['key'], oauth_signature_method='HMAC-SHA1', oauth_timestamp=str(int(time.time())), oauth_nonce=binascii.b2a_hex(uuid.uuid4().bytes), oauth_version='1.0')
        args = {}
