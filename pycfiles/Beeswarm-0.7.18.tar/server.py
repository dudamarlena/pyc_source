...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/server/server.py
# Compiled at: 2016-11-12 07:38:04
import json, logging, sys, os, gevent
    ...
    def prepare_environment(self, work_dir, customize, server_hostname=None):
        ...
        if not os.path.isfile(config_file):
            ...
            if customize:
                ...
                print 'IMPORTANT: Please make sure that "Common Name" is the IP address or fully qualified host name  that you want to use for the server API.'
                ...
                logging.warn('Beeswarm server will be configured using default ssl parameters and network configuration, this could be used to fingerprint the beeswarm server. If you want to customize these options please use the --customize options on first startup.')
                cert_cn = '*'
                cert_country = 'US'
                cert_state = 'None'
                cert_locality = 'None'
                ...
                cert_org_unit = ''
                web_port = 5000
            cert, priv_key = create_self_signed_cert(cert_country, cert_state, cert_org, cert_locality, cert_org_unit, cert_cn)
            ...
            if not server_hostname:
                ...
                print '* Please make sure that drones can always contact the Beeswarm server using the information that you are about to enter. *'
            ...
            message_dict = {'network': {'zmq_server_public_key': zmq_public, 'web_port': web_port, 
                           'zmq_port': zmq_port, 
                           'zmq_command_port': zmq_command_port, 
                           'server_host': server_hostname}, 
               'sql': {'connection_string': ('sqlite:///{0}').format(sqlite_db)}, 
               'ssl': {'certpath': 'server.crt', 
                       'keypath': 'server.key'}, 
               'general': {'mode': 'server'}, 
