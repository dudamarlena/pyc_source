...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/shared/helpers.py
# Compiled at: 2016-11-12 07:38:04
import urlparse, os, pwd, grp, platform, logging, json, shutil, sys
...
import netifaces, zmq.green, gevent, requests
...
def create_self_signed_cert(cert_country, cert_state, cert_organization, cert_locality, cert_organizational_unit, cert_common_name):
...
def send_zmq_request_socket(socket, request):
    ...
    if status != Messages.OK.value:
        logger.error(('Receiving actor on socket {0} failed to respond properly. The request was: {1}').format(socket, request))
        assert False
    else:
...
def stop_if_not_write_workdir(_dir):
    if not os.access(_dir, os.W_OK | os.X_OK):
        logger.error(('Beeswarm needs write permisison to the work directory, but did not have write permission to directory {0}.').format(_dir))
    ...
    for item in os.listdir(_dir):
        ...
        if os.path.isfile(item):
            ...
            if not os.access(item, os.W_OK | os.X_OK):
                error = True
        if error:
            logger.error(('Beeswarm needs write permisison to all files in the the work directory, but did not have write permission to {0}.').format(item))
            sys.exit(1)
