...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/server/db/database_actor.py
# Compiled at: 2016-11-12 07:38:04
import json, logging, random
from datetime import datetime, timedelta
from gevent import Greenlet
import zmq.green, gevent
from sqlalchemy.orm import joinedload
from sqlalchemy.sql.expression import func
from sqlalchemy import desc
import beeswarm, beeswarm.shared
from beeswarm.server.db import database_setup
from beeswarm.server.db.entities import BaitSession, Session, Client, Authentication, Classification, Transcript, Honeypot, Drone, DroneEdge, BaitUser
...
class DatabaseActor(gevent.Greenlet):
    ...
    def _run(self):
        ...
        while self.enabled:
            ...
            if self.do_classify:
                ...
                elif cmd == Messages.GET_SESSIONS_ALL.value or cmd == Messages.GET_SESSIONS_ATTACKS.value or cmd == Messages.GET_SESSIONS_BAIT.value:
                ...
                if topic == Messages.SESSION_HONEYPOT.value or topic == Messages.SESSION_CLIENT.value:
                    ...
                    self.drone_command_receiver.send(('{0} {1} {2}').format(drone_id, Messages.CONFIG.value, json.dumps(config_dict)))
    ...
    def persist_session(self, session_type, session_json):
        ...
        if self.max_session_count == 0:
            ...
            if db_session.query(Session).count() == self.max_session_count:
            ...
            if session_type == Messages.SESSION_HONEYPOT.value:
                ...
                ignore_failed_bait_sessions = self.send_config_request(('{0} {1}').format(Messages.GET_CONFIG_ITEM.value, 'ignore_failed_bait_session'))
    ...
    def extract_auth_entity(self, auth_data):
        ...
        authentication = Authentication(id=auth_data['id'], username=username, password=password, successful=auth_data['successful'], timestamp=isoformatToDatetime(auth_data['timestamp']))
    ...
    def get_matching_session(self, session, db_session, timediff=5):
        ...
        match = None
        classification = db_session.query(Classification).filter(Classification.type == 'pending').one()
        sessions = db_session.query(Session).options(joinedload(Session.authentication)).filter(Session.protocol == session.protocol).filter(Session.honeypot == session.honeypot).filter(Session.timestamp >= min_datetime).filter(Session.timestamp <= max_datetime).filter(Session.id != session.id).filter(Session.classification == classification)
        for potential_match in sessions:
            ...
            for honey_auth in session.authentication:
                for session_auth in potential_match.authentication:
                    if session_auth.username == honey_auth.username and session_auth.password == honey_auth.password and session_auth.successful == honey_auth.successful:
                        assert potential_match.id != session.id
    ...
    def merge_bait_and_session(self, honeypot_session, bait_session, db_session):
        logger.debug(('Classifying bait session with id {0} as legit bait and deleting matching honeypot_session with id {1}').format(bait_session.id, honeypot_session.id))
        ...
        db_session.commit()
        self.processedSessionsPublisher.send(('{0} {1} {2}').format(Messages.DELETED_DUE_TO_MERGE.value, honeypot_session.id, bait_session.id))
        self.processedSessionsPublisher.send(('{0} {1}').format(Messages.SESSION.value, json.dumps(bait_session.to_dict())))
    ...
    def _classify_malicious_sessions(self):
        ...
        db_session = database_setup.get_session()
        bait_sessions = db_session.query(BaitSession).options(joinedload(BaitSession.authentication)).filter(BaitSession.classification_id == 'pending').filter(BaitSession.did_complete == True).filter(BaitSession.received < min_datetime).all()
        ...
        sessions = db_session.query(Session, Drone.name).filter(Session.discriminator == None).filter(Session.timestamp <= min_datetime).filter(Session.classification_id == 'pending').all()
        for entry in sessions:
            session = entry[0]
            bait_match = None
            for a in session.authentication:
                bait_match = db_session.query(BaitSession).filter(BaitSession.authentication.any(username=a.username, password=a.password)).first()
                if bait_match:
                    break

            if bait_match:
                logger.debug(('Classifying session with id {0} as attack which involved the reuse of previously transmitted credentials.').format(session.id))
                ...
                session.classification = db_session.query(Classification).filter(Classification.type == 'bruteforce').one()
            db_session.commit()
            ...
            for capability in honeypot.capabilities:
                for client in clients:
                    ...
                    if capability.protocol in client_timings:
                        ...
                        logger.warning(('Bait timings for {0} not found on client drone {1}({2}), using defaults instead').format(capability.protocol, client.name, client.id))
                        activation_range = '00:00 - 23:59'
                        sleep_interval = '60'
                        activation_probability = 1
                    bait_credentials = random.choice(credentials)
                    client.add_bait(capability, activation_range, sleep_interval, activation_probability, bait_credentials.username, bait_credentials.password)
    ...
    def _handle_command_bait_user_delete(self, data):
        ...
        if bait_user:
            ...
            db_session.commit()
            drone_edge = db_session.query(DroneEdge).filter(DroneEdge.username == bait_user.username, DroneEdge.password == bait_user.password).first()
            if drone_edge:
                self._reconfigure_all_clients()
        else:
    ...
    def _handle_command_bait_user_add(self, data):
        ...
        db_session = database_setup.get_session()
        existing_bait_user = db_session.query(BaitUser).filter(BaitUser.username == username, BaitUser.password == password).first()
    ...
    def _get_drone_config(self, drone_id):
        ...
        zmq_command_port = self.send_config_request(('{0} {1}').format(Messages.GET_CONFIG_ITEM.value, 'network,zmq_command_port'))
        ...
        drone_config = {'general': {'mode': drone.discriminator, 
                       'id': int(drone.id), 
                       'fetch_ip': False, 
                       'name': drone.name}, 
           'beeswarm_server': {'zmq_url': server_zmq_url, 
        ...
        if drone.discriminator == 'honeypot':
            drone_config['certificate_info'] = {'common_name': drone.cert_common_name, 'country': drone.cert_country, 
            ...
            for capability in drone.capabilities:
                ...
                drone_config['capabilities'][capability.protocol] = {'port': capability.port, 'enabled': True, 'protocol_specific_data': json.loads(capability.protocol_specific_data), 
                   'users': users}

        elif drone.discriminator == 'client':
            drone_config['baits'] = {}
            for bait in drone.baits:
                _bait = {'server': bait.capability.honeypot.ip_address, 'port': bait.capability.port, 
    ...
    def _handle_command_get_db_stats(self):
        db_session = database_setup.get_session()
        database_stats = {'count_honeypots': db_session.query(Honeypot).count(), 
           ...
           'count_all_attacks': db_session.query(Session).filter(Session.classification_id != 'bait_session').filter(Session.classification_id != 'pending').filter(Session.classification_id is not None).count(), 
           'count_attack_type': {'http': self._get_num_attacks('http', db_session), 
           ...
           'baits': {'successful': db_session.query(BaitSession).filter(BaitSession.did_login).count(), 
    ...
    def _get_num_attacks(self, protocol, db_session):
        return db_session.query(Session).filter(Session.classification_id != 'bait_session').filter(Session.classification_id is not None).filter(Session.protocol == protocol).count()

    def _handle_command_get_sessions(self, _type):
        db_session = database_setup.get_session()
        query_iterators = {Messages.GET_SESSIONS_ALL.value: db_session.query(Session), 
           Messages.GET_SESSIONS_BAIT.value: db_session.query(BaitSession), 
           Messages.GET_SESSIONS_ATTACKS.value: db_session.query(Session).filter(Session.classification_id != 'bait_session')}
    ...
    def _handle_command_get_transcript(self, session_id):
        ...
        for transcript in transcripts:
            row = {'time': transcript.timestamp.strftime('%Y-%m-%d %H:%M:%S'), 'direction': transcript.direction, 'data': transcript.data}
            return_rows.append(row)
    ...
    def _handle_command_config_drone(self, data):
        ...
        drone = db_session.query(Drone).filter(Drone.id == drone_id).one()
        if not drone:
            self.databaseRequests.send(('{0} {1}').format(Messages.FAIL.value, ('Drone with id {0} could not found').format(drone_id)))
            ...
            self.databaseRequests.send(('{0} {1}').format(Messages.FAIL.value, ('Malformed data in drone config data.Drone id: {0}').format(drone_id)))
    ...
    def _db_maintenance(self):
        ...
        bait_session_retain_days = int(self.send_config_request(('{0} {1}').format(Messages.GET_CONFIG_ITEM.value, 'bait_session_retain')))
        bait_retain = datetime.utcnow() - timedelta(days=bait_session_retain_days)
        malicious_session_retain_days = int(self.send_config_request(('{0} {1}').format(Messages.GET_CONFIG_ITEM.value, 'malicious_session_retain')))
        malicious_retain = datetime.utcnow() - timedelta(days=malicious_session_retain_days)
        db_session = database_setup.get_session()
        malicious_deleted_count = db_session.query(Session).filter(Session.classification_id != 'bait_session').filter(Session.timestamp < malicious_retain).delete()
        bait_deleted_count = db_session.query(Session).filter(Session.classification_id == 'bait_session').filter(Session.timestamp < bait_retain).delete()
        db_session.commit()
        logger.info(('Database maintenance finished. Deleted {0} bait_sessions and {1} malicious sessions)').format(bait_deleted_count, malicious_deleted_count))