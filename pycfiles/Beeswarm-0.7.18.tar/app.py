...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/server/webapp/app.py
# Compiled at: 2016-11-12 07:38:04
import json, logging, random, string, ast, gevent, gevent.lock, os, uuid, zmq.green
...
def send_zmq_request_socket(socket, _request):
    ...
    if status != Messages.OK.value:
        logger.error(('Receiving actor on socket {0} failed to respond properly. The request was: {1}').format(socket, _request))
        if data:
            abort(500, data)
        else:
            abort(500)
...
def ensure_admin_password(reset_password, password=None):
    if not os.path.isfile(admin_passwd_file) or reset_password:
        ...
        with os.fdopen(os.open(admin_passwd_file, os.O_WRONLY | os.O_CREAT, 384), 'w') as (_file):
            _file.truncate()
            _file.write(password_hash)
        logger.info('Created default admin account for the beeswarm server, password has been printed to the console.')
        print '****************************************************************************'
...
def configure_honeypot(drone_id):
    ...
    if not form.validate_on_submit():
        ...
        honeypot_config = {'name': form.general__name.data, 'mode': 'honeypot', 
           'certificate': {'common_name': form.certificate_info__common_name.data, 
        ...
        if form.capabilities__ftp__enabled.data:
            honeypot_config['capabilities']['ftp'] = {'port': form.capabilities__ftp__port.data, 'protocol_specific_data': {'max_attempts': form.capabilities__ftp__protocol_specific_data__max_attempts.data, 
        ...
        if form.capabilities__telnet__enabled.data:
            honeypot_config['capabilities']['telnet'] = {'port': form.capabilities__telnet__port.data, 'protocol_specific_data': {'max_attempts': form.capabilities__telnet__protocol_specific_data__max_attempts.data}}
        if form.capabilities__pop3__enabled.data:
            honeypot_config['capabilities']['pop3'] = {'port': form.capabilities__pop3__port.data, 'protocol_specific_data': {'max_attempts': form.capabilities__pop3__protocol_specific_data__max_attempts.data}}
        if form.capabilities__pop3s__enabled.data:
            honeypot_config['capabilities']['pop3s'] = {'port': form.capabilities__pop3s__port.data, 'protocol_specific_data': {'max_attempts': form.capabilities__pop3s__protocol_specific_data__max_attempts.data}}
        if form.capabilities__ssh__enabled.data:
            honeypot_config['capabilities']['ssh'] = {'port': form.capabilities__ssh__port.data}
        if form.capabilities__http__enabled.data:
            honeypot_config['capabilities']['http'] = {'port': form.capabilities__http__port.data, 'protocol_specific_data': {'banner': form.capabilities__http__protocol_specific_data__banner.data}}
        if form.capabilities__https__enabled.data:
            honeypot_config['capabilities']['https'] = {'port': form.capabilities__https__port.data, 'protocol_specific_data': {'banner': form.capabilities__https__protocol_specific_data__banner.data}}
        if form.capabilities__smtp__enabled.data:
            honeypot_config['capabilities']['smtp'] = {'port': form.capabilities__smtp__port.data, 'protocol_specific_data': {'banner': form.capabilities__smtp__protocol_specific_data__banner.data}}
        if form.capabilities__vnc__enabled.data:
            honeypot_config['capabilities']['vnc'] = {'port': form.capabilities__vnc__port.data, 'protocol_specific_data': {}}
        send_database_request(('{0} {1} {2}').format(Messages.CONFIG_DRONE.value, drone_id, json.dumps(honeypot_config)))
...
def configure_client(drone_id):
    ...
    if not form.validate_on_submit():
        ...
        client_config['bait_timings'] = {'http': {'active_range': form.bait_timings__http__active_range.data, 
           ...
           'ftp': {'active_range': form.bait_timings__ftp__active_range.data, 
           ...
           'https': {'active_range': form.bait_timings__https__active_range.data, 
           ...
           'pop3': {'active_range': form.bait_timings__pop3__active_range.data, 
           ...
           'ssh': {'active_range': form.bait_timings__ssh__active_range.data, 
           ...
           'pop3s': {'active_range': form.bait_timings__pop3s__active_range.data, 
           ...
           'smtp': {'active_range': form.bait_timings__smtp__active_range.data, 
           ...
           'vnc': {'active_range': form.bait_timings__vnc__active_range.data, 
           ...
           'telnet': {'active_range': form.bait_timings__telnet__active_range.data, 
        ...
        send_database_request(('{0} {1} {2}').format(Messages.CONFIG_DRONE.value, drone_id, json.dumps(client_config)))
...
def settings():
    ...
    form = SettingsForm(bait_session_retain=bait_session_retain, malicious_session_retain=malicious_session_retain, ignore_failed_bait_session=ignore_failed_bait_session)
    if form.validate_on_submit():
        options = {'bait_session_retain': form.bait_session_retain.data, 'malicious_session_retain': form.malicious_session_retain.data, 
