...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/drones/honeypot/models/session.py
# Compiled at: 2016-11-12 07:38:04
import hmac, logging, json
    ...
    def try_auth(self, _type, **kwargs):
        ...
        if _type == 'plaintext':
            ...
            for valid_password in self.users.values():
                aligned_password = (valid_password + '\x00\x00\x00\x00\x00\x00\x00\x00')[:8]
        ...
        logger.debug(('{0} authentication attempt from {1}:{2}. Credentials: {3}').format(self.protocol, self.source_ip, self.source_port, json.dumps(kwargs)))
