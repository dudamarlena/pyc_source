...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/server/db/entities.py
...
honeypot_client_mtm = Table('association', Base.metadata, Column('client', String, ForeignKey('client.id')), Column('honeypot', String, ForeignKey('honeypot.id')))
...
class Client(Drone):
    ...
    def add_bait(self, capability, activation_range, sleep_interval, activation_probability, username, password):
        bait = DroneEdge(capability=capability, activation_range=activation_range, sleep_interval=sleep_interval, activation_probability=activation_probability, username=username, password=password)
...
class Session(Base):
    ...
    def to_dict(self):
        ...
        result = {'time': self.timestamp.strftime('%Y-%m-%d %H:%M:%S'), 'protocol': self.protocol, 
...
class Transcript(Base):
    ...
    def to_dict(self):
        return {'time': self.timestamp.strftime('%Y-%m-%d %H:%M:%S'), 'direction': self.direction, 
           'data': self.data}
