...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/drones/honeypot/capabilities/shared/shell.py
# Compiled at: 2016-11-12 07:38:04
import argparse, os, logging, sys, socket, traceback
...
class Commands(OwnGreenTelnetHandler):
    ...
    HOSTNAME = 'host'
    TERM = 'ansi'
    ENVIRONMENT_VARS = {'http_proxy': 'http://10.1.0.23/', 
    ...
    def command_ls(self, params):
        if '-l' in params:
            self.writeline('total ' + str(self.total_file_size))
    ...
    def command_echo(self, params):
        if not params:
            self.writeline('')
            return
        if params[0].startswith('$') and len(params) == 1:
            var_name = params[0][1:]
    ...
    def command_sudo(self, params):
        executable = params[0]
        self.writeline(("Sorry, user {} is not allowed to execute '{}' as root on {}.").format(self.username, executable, self.HOSTNAME))
