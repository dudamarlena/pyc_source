...
# [GCC 8.4.0]
# Embedded file name: /home/travis/build/honeynet/beeswarm/beeswarm/drones/honeypot/honeypot.py
# Compiled at: 2016-11-12 07:38:04
import logging, os, sys, shutil, _socket, ntplib, fs, gevent
...
from beeswarm.shared.helpers import create_self_signed_cert, send_zmq_push, extract_keys, get_most_likely_ip, stop_if_not_write_workdir
...
class Honeypot(object):
    ...
    def __init__(self, work_dir, config, key='server.key', cert='server.crt', **kwargs):
        ...
        if not os.path.isfile(os.path.join(work_dir, 'server.key')):
            ...
            cert, priv_key = create_self_signed_cert(cert_info['country'], cert_info['state'], cert_info['organization'], cert_info['locality'], cert_info['organization_unit'], cert_info['common_name'])
            ...
            send_zmq_push(SocketNames.SERVER_RELAY.value, ('{0} {1} {2}').format(Messages.KEY.value, self.honeypot_id, priv_key))
            send_zmq_push(SocketNames.SERVER_RELAY.value, ('{0} {1} {2}').format(Messages.CERT.value, self.honeypot_id, cert))
            ...
            if cap_name in self.config['capabilities']:
                ...
                try:
                    if cap_name.endswith('s'):
                        server = StreamServer(('0.0.0.0', port), cap.handle_session, keyfile=self.key, certfile=self.cert)
                    else:
    ...
    def prepare_environment(work_dir):
        ...
        shutil.copytree(os.path.join(package_directory, 'drones/honeypot/data'), os.path.join(work_dir, 'data/'), ignore=Honeypot._ignore_copy_files)
