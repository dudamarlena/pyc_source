...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/mass_post_office/tests/test_models.py
...
class TestModels(TestCase):
    ...
    def test_mailing_list(self):
        ml = MailingList(name='Some name', user_should_be_agree=True, all_users=True, or_list='')
        ml.save()
        self.assertEqual(str(ml), ml.name)
        ml = MailingList(name='Some name', user_should_be_agree=False, all_users=True, or_list='')
        ml.clean()
        ml.save()
        self.assertTrue(ml.user_should_be_agree)
        ml = MailingList(name='Some name', user_should_be_agree=True, all_users=True, or_list='{"some list": "123"}')
        ml.clean()
        ml.save()
        ...
        ml = MailingList(name='Some name', user_should_be_agree=True, all_users=True, or_list='{bad json}')
        is_exception = False
        try:
            ml.clean()
    ...
    def test_get_users_queryset(self):
        ...
        ml = MailingList(name='Some name', user_should_be_agree=False, all_users=True)
        ml.save()
        self.assertEqual(ml.get_users_queryset().count(), 3)
        ml = MailingList(name='Some name', user_should_be_agree=False, all_users=False, or_list='[{"id": %d}]' % user2.id)
        ml.save()
    ...
    def test_get_emails_generator(self):
        user1 = UserFactory(is_active=True)
        user2 = UserFactory(is_active=True)
        ml = MailingList(name='Some name', user_should_be_agree=True, all_users=True)
        ml.save()
    ...
    def test_mass_emails(self):
        ...
        ml = MailingList(name='Some name', user_should_be_agree=False, all_users=True)
        ml.save()
        email_template = EmailTemplate.objects.create(name='welcome', subject='Welcome!', content='Hi {{ user.first_name }} {{ user.last_name }}!')
        today = datetime.datetime.today()
        from django.db.models import Q
        mass_emails_qs = Q(scheduled_time=today, priority=priority, mailing_list=ml, template=email_template)
        mass_emails = MassEmail.objects.filter(mass_emails_qs)
        self.assertEqual(len(mass_emails), 0)
        me = MassEmail(scheduled_time=today, priority=priority, mailing_list=ml, template=email_template)
        me.save()
        mass_emails = MassEmail.objects.filter(mass_emails_qs)
