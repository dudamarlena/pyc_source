...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/mass_post_office/tests/test_subscription.py
...
class TagsTest(TestCase):
    ...
    def test_unsubscribe_tag(self):
        ...
        try:
            tpl = template.Template('{% load subscription %}{% unsubscribe_link %}')
        except TemplateSyntaxError as e:
            self.assertIn('tag requires a single argument', '%s' % e)

        user = UserFactory()
        tpl = template.Template('{% load subscription %}{% unsubscribe_link user %}')
    ...
    def test_email_templates(self):
        ...
        def __check_email_template(tpl_name, content):
            templates = EmailTemplate.objects.filter(name=tpl_name)
            self.assertEqual(templates.count(), 1)
            self.assertIn(content, templates[0].content)

        __check_email_template('post_office/canceled_subscription', 'You successfuly unsubscribed your subscription for')
        __check_email_template('post_office/unsubscribe_page', 'Visit {% unsubscribe_link user %} link to unsubscribe')

    def test_send_email_from_template(self):
        user = UserFactory(is_active=True)

        def __check_send_email(tpl_name, content=None):
            email = mail.from_template('bob@example.com', user.email, template=tpl_name, context=dict(user=user))
        ...
        __check_send_email('post_office/canceled_subscription', content={'message': 'You successfuly unsubscribed your subscription for %s' % user.email})
        data_tuple = (
         user.username, user.email)
        hashed, data = encode_data(data_tuple)
        url = reverse('mass_post_office:unsubscribe', kwargs={'hashed': hashed, 'data': data})
        __check_send_email('post_office/unsubscribe_page', content={'message': url})
        return