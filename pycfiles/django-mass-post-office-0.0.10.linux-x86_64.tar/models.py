...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/mass_post_office/models.py
...
class MailingList(models.Model):
    ...
    user_should_be_agree = models.BooleanField(verbose_name=_('User should be agreed to receive mails'), default=True)
    ...
    additional_users = models.ManyToManyField(USER_MODEL, verbose_name=_('Additional users'), null=True)
    ...
    def get_users_queryset(self):
        ...
        return user_qs.filter(is_active=True, email__isnull=False).exclude(email='').distinct()
...
class MassEmail(models.Model):
    ...
    template = models.ForeignKey(EmailTemplate, help_text='Template with access to `{{ user }}` variable', verbose_name='Template')
    emails = models.ManyToManyField(Email, verbose_name='Emails', null=True, blank=True)
    scheduled_time = models.DateTimeField(blank=True, null=True, db_index=True)
    priority = models.PositiveSmallIntegerField(choices=PRIORITY_CHOICES, blank=True, null=True, db_index=True)

    def __unicode__(self):
        return ('Mails for {self.mailing_list} with template {self.template}').format(self=self)

    def save(self, *args, **kwargs):
        super(MassEmail, self).save(*args, **kwargs)
        for user in self.mailing_list.get_users_queryset():
            email = from_template(settings.DEFAULT_FROM_EMAIL, recipient=user.email, template=self.template, context={'user': user}, scheduled_time=self.scheduled_time, priority=self.priority)
            email.save()
