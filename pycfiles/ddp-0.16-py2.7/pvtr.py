...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ddp\pvtr.py
# Compiled at: 2015-04-09 21:09:04
import linecache, os, re
from PIL import Image
import images2gif, matplotlib
matplotlib.use('Agg')
import numpy as np, pylab as pl, ddp.inc as inc

class PVTR:
    amp_factor = 1
    vtr_file = 'VTRoutput_1.vtr'
    gif_file = 'efield.gif'
    gif_duration = 0.2
    ...
    def clim_value(self):
        ...
        if PVTR.clim_slice_flag:
            ...
            return [
             np.min(slice_info['slice_intensity']),
    ...
    def get_size(file_path):
        ...
        m = re.search('\\d+\\s+(\\d+)\\s+\\d+\\s+(\\d+)\\s+\\d+\\s+(\\d+)', obj_str)
    ...
    def get_intensity(file_path, array_size):
        ...
        intensity_array = re.split('\\s+', intensity_line)
    ...
    def get_cord(file_path):
        ...
        x_arr = re.split('\\s+', x_line.lstrip().rstrip())
        y_arr = re.split('\\s+', y_line.lstrip().rstrip())
        z_arr = re.split('\\s+', z_line.lstrip().rstrip())
    ...
    def get_slice_info(self, slice_flag):
        ...
        if PVTR.slice_flag == 'x_slice':
            ...
            slice_info['fig_size'] = (self.y_size / self.z_size, self.y_size * 6 / self.z_size, 6) if self.y_size > self.z_size else (self.z_size / self.y_size, 6, self.z_size * 6 / self.y_size)
        ...
        if PVTR.slice_flag == 'y_slice':
            ...
            slice_info['fig_size'] = (self.x_size / self.z_size, self.x_size * 6 / self.z_size, 6) if self.x_size > self.z_size else (self.z_size / self.x_size, 6, self.z_size * 6 / self.x_size)
        ...
        if PVTR.slice_flag == 'z_slice':
            ...
            slice_info['fig_size'] = (self.x_size / self.y_size, self.x_size * 6 / self.y_size, 6) if self.x_size > self.y_size else (self.y_size / self.x_size, 6, self.y_size * 6 / self.x_size)
