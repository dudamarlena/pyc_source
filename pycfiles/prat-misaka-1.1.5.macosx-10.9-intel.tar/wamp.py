...
# Embedded file name: /usr/local/Cellar/python/2.7.10_2/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/geventwebsocket/protocols/wamp.py
# Compiled at: 2015-11-13 17:02:49
import inspect, random, six, string, types
try:
...
class Channels(object):
    ...
    def publish(self, uri, event, exclude=None, eligible=None):
        if uri not in self.channels:
            return
        msg = [
         WampProtocol.MSG_EVENT, uri, event]
...
class WampProtocol(BaseProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.session_id = ('').join([ random.choice(string.digits + string.letters) for i in xrange(16)
                                    ])
    ...
    def on_message(self, message):
        ...
        if data[0] == self.MSG_PREFIX and len(data) == 3:
            ...
            if data[0] in (self.MSG_SUBSCRIBE, self.MSG_UNSUBSCRIBE,
             self.MSG_PUBLISH):
