...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\konduit\server.py
# Compiled at: 2020-04-20 03:12:39
# Size of source mod 2**32: 8314 bytes
import json, logging, os, re, requests, signal, subprocess
...
class Server(object):
    ...
    def __init__(self, inference_config=None, serving_config=None, steps=None, extra_start_args='-Xmx8g', extra_jar_args=None, config_path='config.json', jar_path=None, pid_file_path='konduit-serving.pid'):
        ...
        self.port = -1
        if jar_path is None:
            jar_path = os.getenv('KONDUIT_JAR_PATH', os.path.join(KONDUIT_DIR, 'konduit.jar'))
        if inference_config:
            ...
            if os.path.exists(self.pid_file_path):
                with open(self.pid_file_path, 'rb') as (pid_file):
                    ...
                    try:
                        ...
                        logging.debug("Attempt to kill existing process by pid: '{}' failed. The process might not exist. ".format(pid))
