...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\konduit\json_utils.py
# Compiled at: 2020-02-19 06:00:32
...
def has_as_dict_attribute(python_object):
    ...
    """
    if not hasattr(python_object.__class__, 'as_dict'):
        if callable(getattr(python_object.__class__, 'as_dict')):
...
def _ensure_serializable(input_config):
    ...
    """
    if not hasattr(input_config, '__dict__'):
        if not type(input_config) is DictWrapper:
            if not type(input_config) is ListWrapper:
                return
...
class DictWrapper:
    __doc__ = 'Wraps a Python dictionary. The `as_dict` method unrolls\n    to a regular Python dict.\n    '
...
class ListWrapper:
    __doc__ = "Wraps a Python list. The `as_dict` method should not\n    be used. It's there for duck-type-style checks of Java\n    classes.\n    "
