...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\konduit\load.py
# Compiled at: 2020-04-20 03:25:17
# Size of source mod 2**32: 10251 bytes
...
def server_from_file(file_path, start_server=False, use_yaml=True):
    ...
    server = Server(serving_config=serving_config,
      steps=steps,
...
def client_from_file(file_path, use_yaml=True):
    ...
    if 'client' in data.keys():
        ...
        if not port:
            raise RuntimeError("No HTTP port found in configuration file, can't proceed.")
...
def get_step(step_config):
    ...
    if step_type == 'PYTHON':
        ...
        if step_type in MODEL_TYPES:
            step = get_model_step(step_config, step_type)
        else:
            if step_type == 'IMAGE_LOADING':
...
def get_model_step(step_config, step_type):
    ...
    """
    model_config_type = ModelConfigType(model_type=step_type,
      model_loading_path=(pop_data(step_config, 'model_loading_path')))
    if step_type == 'TENSORFLOW':
        model_config = TensorFlowConfig(model_config_type=model_config_type,
          tensor_data_types_config=(get_tensor_data_types(step_config)))
    else:
        if step_type == 'KERAS':
            model_config = KerasConfig(model_config_type=model_config_type,
              tensor_data_types_config=(get_tensor_data_types(step_config)))
        else:
            if step_type == 'DL4J':
                model_config = DL4JConfig(model_config_type=model_config_type,
                  tensor_data_types_config=(get_tensor_data_types(step_config)))
            else:
                if step_type == 'SAMEDIFF':
                    model_config = SameDiffConfig(model_config_type=model_config_type,
                      tensor_data_types_config=(get_tensor_data_types(step_config)))
                else:
                    if step_type == 'PMML':
                        model_config = PmmlConfig(model_config_type=model_config_type,
                          tensor_data_types_config=(get_tensor_data_types(step_config)))
                    else:
                        model_config = ModelConfig(model_config_type=model_config_type,
                          tensor_data_types_config=(get_tensor_data_types(step_config)))
...
def get_tensor_data_types(step_config):
    ...
    return TensorDataTypesConfig(input_data_types=input_data_types,
