...
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/navigator_tree_wrapper.py
...
class NavigatorTreeWrapper:
    ...
    def __start_in_root(self):
        try:
            ...
            self.__handle_exception(ex, 'Initializing navigator to start in the root failed')
    ...
    def move_to_parent(self):
        """
                ...
                """
        ret = None
        if self.__current_node.get_parent() is None or self.__current_node.get_parent() == self.__tree.get_sentinel():
            self.__current_node = self.__tree.get_sentinel()
            ret = True
        else:
            try:
    ...
    def __log_exception(self, error_message, ex, error_node):
        ...
        logging.warning(('thread=="{}", navigation exception in node "{}":{}". Detailed message: "{}".').format(threading.current_thread().name, path_str, error_message, ex))
