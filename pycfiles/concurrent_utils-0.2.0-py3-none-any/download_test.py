...
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/test/download_test.py
...
from concurrent_tree_crawler.common.delayed_http_files_server import DelayedHTTPFilesServer
from concurrent_tree_crawler.common.threads.token_bucket import TokenBucketFiller, StandardTokenBucket
...
from concurrent_tree_crawler.html_multipage_navigator.web_browser import MechanizeBrowserCreator
from concurrent_tree_crawler.html_multipage_navigator.throttled_web_browser import ThrottledWebBrowserCreator
...
class DownloadTestCase(unittest.TestCase):
    ...
    def test_single_threaded_download_without_manager(self):
        with TempDir() as (temp_dir):
            ...
            address = 'file:' + Resources.path(__file__, 'data/original_site/issues_1.html', convert_to_url=True)
            ...
            self.__check_if_each_node_is_processed_once(tree.get_root(), {'/root/2011-07-16/06': 0})
    ...
    def test_throttled_download(self):
        ...
        for threads_no in [1, 3]:
            seconds_taken = self.__check_download(threads_no, address, max_page_opens_per_second)
            self.assertGreaterEqual(seconds_taken, min_seconds_taken)

    def test_throttled_download_with_HTTP_server(self):
        with DelayedHTTPFilesServer(Resources.path(__file__, 'data/original_site'), 0) as (server):
            address, ip_number = server.start()
            root_address = ('http://{}:{}/issues_1.html').format(address, ip_number)
            web_pages_no = 34
            max_page_opens_per_second = 15
            min_seconds_taken = float(web_pages_no) / max_page_opens_per_second
            for threads_no in [1, 3]:
                seconds_taken = self.__check_download(threads_no, root_address, max_page_opens_per_second)
                self.assertGreaterEqual(seconds_taken, min_seconds_taken)

    def test_multithreaded_download_speedup_with_slow_HTTP_server(self):
        with DelayedHTTPFilesServer(Resources.path(__file__, 'data/original_site'), 0.1) as (server):
            address, ip_number = server.start()
            root_address = ('http://{}:{}/issues_1.html').format(address, ip_number)
            time_taken = []
            threads_no_list = [1, 4]
            ...
            assert_str = ('{} threads time taken: {}s while {} threads time taken: {}s').format(threads_no_list[0], time_taken[0], threads_no_list[1], time_taken[1])
            min_speedup = 1
            self.assert_(time_taken[0] > min_speedup * time_taken[1], assert_str)

    def __check_download(self, threads_no, address, max_page_opens_per_second=None):
        ...
        with TempDir() as (temp_dir):
            ...
            if max_page_opens_per_second is not None:
                ...
                token_filler = TokenBucketFiller(token_bucket, 1, max_page_opens_per_second)
                token_filler.start()
                browser_creator = ThrottledWebBrowserCreator(MechanizeBrowserCreator(), token_bucket)
            else:
                browser_creator = MechanizeBrowserCreator()
            navigators = []
            for _ in xrange(threads_no):
                navigators.append(HTMLMultipageNavigator(address, LevelsCreator(temp_dir.get_path()).create(), browser_creator))
            ...
            self.__check_if_each_node_is_processed_once(sentinel.get_child('root'), {'/root/2011-07-16/06': 0})
            if max_page_opens_per_second is not None:
                token_filler.stop()
            return end - start
        return
    ...
    def __check_if_each_node_is_processed_once(self, root, exception_nodes):
        self.__check_if_each_node_in_subtree_is_processed_once('/root', root, exception_nodes)

    def __check_if_each_node_in_subtree_is_processed_once(self, path, node, exception_nodes):
        expected_count = 1
        ...
        for child_name in children_names:
            ...
            self.__check_if_each_node_in_subtree_is_processed_once(child_path, child, exception_nodes)
...
class _MultithreadedCrawlerExtended(MultithreadedCrawler):
    ...
    def __init__(self, navigators, sentinel, activity_schedule=None, log_file_path=None, state_file_path=None, save_period=None, logging_level=logging.ERROR):
        MultithreadedCrawler.__init__(self, navigators, sentinel, activity_schedule, log_file_path, state_file_path, save_period, logging_level)
