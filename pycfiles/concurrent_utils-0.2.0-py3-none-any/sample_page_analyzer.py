...
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/html_multipage_navigator/sample_page_analyzer.py
...
from concurrent_tree_crawler.html_multipage_navigator.abstract_page_analyzer import PageLinks, AbstractPageAnalyzer, Level, AbstractLevelsCreator
...
class MagazinePageAnalyzer(AbstractPageAnalyzer):
    ...
    def get_links(self, page_file, child_links_retrieved_so_far):
        ...
        next_page_elem = doc.find("body/div/div[@id='navigation']/table/tr/td[3]/a")
        next_page_link = None
    ...
    def __convert_date(text):
        ...
        month_str, day_str, year_str = re.match('^(\\w+) (\\d+), (\\d+)$', text).group(1, 2, 3)
        month = _convert_to_2_digit_number(int(months[month_str]))
...
class IssuePageAnalyzer(AbstractPageAnalyzer):
    ...
    def process(self, tree_path, page_file):
        ...
        dir_path = os.path.join(self.__dst_dir_path, _convert_tree_path_to_dir_path(tree_path))
    ...
    def get_links(self, page_file, child_links_retrieved_so_far):
        ...
        for i, link in enumerate(links):
            link_text = _convert_to_2_digit_number(child_links_retrieved_so_far + i + 1)
            children.append((link_text, link.attrib['href']))
...
class ArticlePageAnalyzer(AbstractPageAnalyzer):
    ...
    def process(self, tree_path, page_file):
        assert len(tree_path) > 0
        dir_path = os.path.join(self.__dst_dir_path, _convert_tree_path_to_dir_path(tree_path[:-1]))
        lenient_makedir(dir_path)
...
class LevelsCreator(AbstractLevelsCreator):
    ...
    def create(self):
        return [
         Level('magazine', MagazinePageAnalyzer()),
