...
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/html_multipage_navigator/tree_navigator.py
# Compiled at: 2011-09-28 13:50:09
import logging
from collections import OrderedDict
from collections import deque
from concurrent_tree_crawler.abstract_tree_navigator import AbstractTreeNavigator, NavigationException
...
class HTMLMultipageNavigator(AbstractTreeNavigator):
    ...
    __generate_new_name_max_repetitions = int(100000.0)
    ...
    def __get_current_children(self):
        ...
        while True:
            page_links = current_level.page_analyzer.get_links(self.__br.response(), child_links_retrieved_so_far)
            for name, link in page_links.children:
                if name in children:
                    new_name = self.__generate_new_name(name, children)
                    if new_name is None:
                        logging.error(('Unable to generate a new name for a repeating child name "{}" (link="{}") in node "{}": all of the proposed new name variants are already in use').format(name, link, ('/').join(self.__path)))
                        continue
    ...
    def __generate_new_name(original_name, children_dict):
        ...
        for i in xrange(max_rep):
            name = original_name + HTMLMultipageNavigator.__repetition_suffix_template.format(i + 1)
