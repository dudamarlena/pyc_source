...
# [GCC 8.4.0]
# Embedded file name: spead.py
# Compiled at: 2012-02-13 07:51:55
"""
Data packet:
...
def mkfmt(*args):
    return pack(FORMAT_FMT, args)


def parsefmt(fmt):
    return unpack(FORMAT_FMT, fmt, cnt=-1)


DEFAULT_FMT = mkfmt(('u', ADDRSIZE))
HDR_FMT = mkfmt(('u', 8), ('u', 8), ('u', 8), ('u', 8), ('u', 16), ('u', 16))
RAW_ITEM_FMT = mkfmt(('u', 1), ('u', ITEMSIZE - ADDRSIZE - 1), ('c', 8), ('c', 8), ('c',
                                                                                    8), ('c',
                                                                                         8), ('c',
                                                                                              8))
...
STR_FMT = mkfmt(('c', 8))
ITEM = {'HEAP_CNT': {'ID': HEAP_CNT_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'HEAP_LEN': {'ID': HEAP_LEN_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'PAYLOAD_LEN': {'ID': PAYLOAD_LEN_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'PAYLOAD_OFF': {'ID': PAYLOAD_OFF_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'DESCRIPTOR': {'ID': DESCRIPTOR_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'STREAM_CTRL': {'ID': STREAM_CTRL_ID, 'FMT': DEFAULT_FMT, 'CNT': 1}, 'NAME': {'ID': NAME_ID, 'FMT': STR_FMT, 'CNT': -1}, 'DESCRIPTION': {'ID': DESCRIPTION_ID, 'FMT': STR_FMT, 'CNT': -1}, 'DTYPE': {'ID': DTYPE_ID, 'FMT': STR_FMT, 'CNT': -1}, 'SHAPE': {'ID': SHAPE_ID, 'FMT': SHAPE_FMT, 'CNT': -1}, 'FORMAT': {'ID': FORMAT_ID, 'FMT': FORMAT_FMT, 'CNT': -1}, 'ID': {'ID': ID_ID, 'FMT': ID_FMT, 'CNT': 1}}
NAME = {}
...
def readable_binpacket(pkt, prepend='', show_payload=False):
    ...
    magic, version, itemsize, addrsize, junk, n_options = unpack(HDR_FMT, pkt[o:o + ITEMLEN])[0]
    o += ITEMLEN
...
class Descriptor:
    ...
    def _calcsize(self):
        ...
        if self.nbits > 0 and self.nbits < ADDRSIZE:
            self._offset = ADDRSIZE - self.nbits
        else:
            self._offset = 0
    ...
    def to_descriptor_string(self):
        ...
        if self.size == -1:
            shape = pack(SHAPE_FMT, ((2, 0), ))
        else:
            shape = pack(SHAPE_FMT, [ (0, s) for s in self.shape ])
        heap = {ID_ID: (IMMEDIATEADDR, pack(ID_FMT, ((0, self.id),))), SHAPE_ID: (
                    DIRECTADDR, shape), 
           FORMAT_ID: (
                     DIRECTADDR, self.format), 
           NAME_ID: (
                   DIRECTADDR, self.name), 
           DESCRIPTION_ID: (
                          DIRECTADDR, self.description), 
           HEAP_CNT_ID: (
                       IMMEDIATEADDR, ADDRNULL)}
        if self.dtype_str is not None:
            heap[DTYPE_ID] = (
             DIRECTADDR, self.dtype_str)
...
class Item(Descriptor):
    ...
    def __init__(self, name='', id=None, description='', shape=[], fmt=DEFAULT_FMT, from_string=None, ndarray=None, init_val=None):
        if init_val is not None and type(init_val) == numpy.ndarray and shape == [] and fmt == DEFAULT_FMT:
            ndarray = init_val
        Descriptor.__init__(self, from_string=from_string, id=id, name=name, description=description, shape=shape, fmt=fmt, ndarray=ndarray)
        self._value = None
        self._changed = False
    ...
    def to_value_string(self):
        ...
        try:
            ...
            raise TypeError('item "%s" (ID=%d): had an invalid value for format=%s, shape=%s: %s' % (self.name, self.id, [self.format], self.shape, self._value))
...
class ItemGroup:
    ...
    def add_item(self, *args, **kwargs):
        ...
        if item.id is None:
            ...
            while self._items.has_key(item.id):
                item.id += 1
    ...
    def get_heap(self, heap=None):
        ...
        if heap is None:
            heap = {}
        heap[HEAP_CNT_ID] = (
         IMMEDIATEADDR, pack(DEFAULT_FMT, ((self.heap_cnt,),)))
        self.heap_cnt += 1
        heap[DESCRIPTOR_ID] = []
        ...
        for item in self._items.itervalues():
            ...
            if len(val) > ADDRLEN or item.size < 0:
                mode = DIRECTADDR
            else:
                mode = IMMEDIATEADDR
            if DEBUG:
...
def iter_genpackets(heap, max_pkt_size=MAX_PACKET_LEN):
    ...
    assert heap.has_key(HEAP_CNT_ID)
    pkt = SpeadPacket()
    ...
    while True:
        if payload_cnt == 0:
            ...
            h = [
             (
              IMMEDIATEADDR, HEAP_CNT_ID, unpack(DEFAULT_FMT, heap[HEAP_CNT_ID][1])[0][0])]
        hlen = ITEMLEN * (len(h) + 4)
...
class Transmitter:
    ...
    def send_halt(self):
        ...
        heap = {HEAP_CNT_ID: (IMMEDIATEADDR, b'\xff\xff\xff\xff\xff\xff'), STREAM_CTRL_ID: (
                          IMMEDIATEADDR, pack(DEFAULT_FMT, ((STREAM_CTRL_TERM_VAL,),)))}
    ...
    for pkt in tport.iterpackets():
        ...
        if not heaps.has_key(heap_cnt):
            while len(heaps) >= MAX_CONCURRENT_HEAPS:
                ...
                if partial_heap.is_valid:
                    ...
                    logger.warning('iterheaps: Invalid spead heap %d found (SpeadHeap.has_all_packets=%d)' % (pop_idx, partial_heap.has_all_packets))
        ...
        if heapdone:
            ...
            if heap.is_valid:
                ...
                logger.warning('iterheaps: Invalid spead heap %d found (SpeadHeap.has_all_packets=%d)' % (heap.heap_cnt, heap.has_all_packets))
