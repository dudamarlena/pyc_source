...
# Embedded file name: build/bdist.linux-x86_64/egg/local_worker/core/utils.py
...
def get_hashes(file):
    ...
    return {'sha256': sha256, 'sha1': sha1, 
       'md5': md5}
...
def APKFileType(fname):
    ...
    if not os.path.isfile(path):
        ...
        raise argparse.ArgumentTypeError('File %s is not  an apk' % fname.split('/')[(-1)])
    return path
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for av_name, avd_name in av_dict.items():
            try:
                ...
                parser.error('Antivirus %s does not exists in adapters folder' % av_name)
