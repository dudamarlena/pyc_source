...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python3.8/site-packages/notifications/models.py
# Compiled at: 2020-02-28 16:25:57
...
class Notification(models.Model):
    ...
    message = HTMLField(help_text='Full message as shown to user')
    ...
    snooze_lock = models.IntegerField(default=None, null=True, blank=True)
    snooze_time = models.IntegerField(default=None,
      null=True,
      blank=True,
      help_text='If user dismisses message (when relevant), message is shown again after this many days.')
    recipients = models.ManyToManyField(User, through='UserNotification', default=None, blank=True)
    send_email = models.BooleanField(default=False)
    groups = models.ManyToManyField(Group, default=None, blank=True)
    look = models.CharField(max_length=50,
      choices=NOTIFICATION_CHOICES,
      default=SIMPLE_OK,
      help_text='This controls the appearance of the notification.')
    image = models.ImageField(upload_to='notification_imgs',
      null=True,
      blank=True,
...
class UserNotification(models.Model):
    ...
    def __str__(self):
        return '{} - {} - seen:{} - answer:{} - answer_string:{}'.format(self.notification.name, self.user.username, self.seen, self.answer, self.answer_string)

    def update_unseen_cache(self, unseen=None):
        if unseen is None:
            unseen = Notification.unseen(self.user)
        if not self.seen or self.answer or self.notification.expires is not None and self.notification.expires < datetime.datetime.now():
    ...
    def update_for_user(user):
        uns = user.usernotification_set.exclude(answer=True).filter(Q(notification__expires=None) | Q(notification__expires__gt=(datetime.datetime.now())))
        for un in uns:
