...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/skypipe/cli.py
# Compiled at: 2012-11-06 21:12:34
...
"""
import sys, os, argparse, atexit, runpy
...
def get_parser():
    ...
    parser.add_argument('name', metavar='NAME', type=str, nargs='?', help='use a named skypipe', default='')
    parser.add_argument('--version', action='version', version=('%(prog)s {0}').format(skypipe.VERSION))
    parser.add_argument('--setup', action='store_const', const=True, default=False, help='setup account and satellite')
    parser.add_argument('--check', action='store_const', const=True, default=False, help='check if satellite is online')
    parser.add_argument('--reset', action='store_const', const=True, default=False, help='destroy any existing satellite')
    parser.add_argument('--satellite', action='store', default=None, metavar='PORT', help='manually run a satellite on PORT')
    return parser
...
def run():
    ...
    dotcloud_endpoint = os.environ.get('DOTCLOUD_API_ENDPOINT', 'https://rest.dotcloud.com/v1')
