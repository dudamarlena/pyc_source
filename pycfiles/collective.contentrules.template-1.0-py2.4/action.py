...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/contentrules/template/action.py
# Compiled at: 2010-10-20 09:03:11
...
class ICopyFromTemplateAction(Interface):
    ...
    """
    __module__ = __name__
    source_object = schema.Choice(title=_('Source object'), description=_('As a path relative to the portal root.'), required=True, source=SearchableTextSourceBinder({}, default_query='path:'))
    change_ownership = schema.Bool(title=_('Change ownership'), description=_('If selected, the newly created object will have the same owner as the object that triggered the content rule.'), required=True, default=True)
    copy_children = schema.Bool(title=_('Copy children of selected content'), description=_('If selected, children of selected content  will be copied. If not, the selected content itself will be copied '), default=False)
...
class CopyFromTemplateAction(SimpleItem):
    ...
    @property
    def summary(self):
        return _('Copy from template at ${template}.', mapping=dict(template=self.source_object))
...
class CopyFromTemplateActionExecutor(object):
    ...
    def copy_path(self, path, target):
        ...
        if obj is None:
            self.error(obj, _('Source object ${source} does not exist.', mapping={'source': path}))
            return False
    ...
    def error(self, obj, error):
        ...
        if request is not None:
            ...
            message = _("Unable to copy ${name} as part of content rule 'copy from template' action: ${error}", mapping={'name': title, 'error': error})
...
class CopyFromTemplateAddForm(AddForm):
    ...
    label = _('Add Copy from Template Action')
    description = _('An action to copy an template object into the current folder')
...
class CopyFromTemplateEditForm(EditForm):
    ...
    form_fields = form.FormFields(ICopyFromTemplateAction)
    form_fields['source_object'].custom_widget = UberSelectionWidget
    label = _('Edit Copy from Template Action')
    description = _('An action to copy an template object into the current folder')
    form_name = _('Configure element')