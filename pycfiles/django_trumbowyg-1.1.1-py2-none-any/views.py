...
# [GCC 8.4.0]
# Embedded file name: /Users/est/src/django-trumbowyg/trumbowyg/views.py
# Compiled at: 2017-03-11 06:56:05
import os, json
...
def save_image(image):
    ...
    if _settings.THUMBNAIL_SIZE:
        ...
        if im.size != initial_size:
            real_path = os.path.join(_settings.UPLOAD_PATH, 'thumb_%s' % image.name)
            try:
                im.save(os.path.join(settings.MEDIA_ROOT, real_path), 'JPEG')
                return real_path
            except IOError:
                print (
                 'cannot create thumbnail for', image)
                return
