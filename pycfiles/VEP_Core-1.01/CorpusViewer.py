...
# [GCC 8.4.0]
# Embedded file name: C:\LocalUsers\ealexand\VEP_Core\vep_core\Serendip\CorpusViewer.py
...
def view_by_name(corpus_name):
    return render_template('corpus/index.html', script='app', title=corpus_name + ' (Matrix View)', corpus_name=corpus_name, corpora=corpora, models=models)
...
def mesoview(corpus_name):
    ...
    return render_template('corpus/meso.html', title=corpus_name + ' (Meso View)', corpus_name=corpus_name, tags=[
     tagger.tags, []], included_tag_keys=included_tag_keys, included_text_names=included_text_names, colors=colors, corpora=corpora)
    ...
    with open(metadataCSV, 'rb') as (f):
        ...
        for row in reader:
            if rowNum == 0:
                ...
                filename_cache_path = os.path.join(metadata_root, corpus_name, 'TopicModel', 'Serendip', os.path.splitext(os.path.basename(temp['filename']))[0])
...
def get_theta(corpus_name):
    ...
    topicMetadataFields = [
     'min', 'max', 'median', 'mean', 'variance', 'range', 'numDocs']
    returnDict = {}
