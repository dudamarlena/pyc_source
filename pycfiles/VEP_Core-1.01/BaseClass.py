...
# [GCC 8.4.0]
# Embedded file name: C:\LocalUsers\ealexand\VEP_Core\vep_core\Ity\BaseClass.py
# Compiled at: 2014-05-14 12:25:04
...
class BaseClass(object):
    ...
    values::
        ...
        if tokenizer_one.full_label == tokenizer_two.full_label:
            raise StandardError("tokenizer_two's tokenizer() method could            produce different output than tokenizer_one.tokenize(), but we            can't tell because the implementer didn't update self._full_label.")
        else:
            print "Looks like tokenizer_one and tokenizer_two could have            different output, as expected."
    """
    ...
    def full_label(self):
        ...
        if self._label is not None:
            label_parts.append(str(self._label))
        if self._full_label is not None:
            label_parts.append(str(self._full_label))
