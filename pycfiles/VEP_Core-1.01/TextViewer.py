...
# [GCC 8.4.0]
# Embedded file name: C:/LocalUsers/ealexand/VEP_Core/vep_core\Serendip\TextViewer.py
# Compiled at: 2014-06-02 17:42:59
__author__ = 'kohlmannj'
import os, glob, json, codecs, HTMLParser, zipfile, re
...
formatters = dict(HTML=HTMLFormatter(), LineGraph=LineGraphFormatter())
text_data = {}
corpus_courier = None
...
def mallet_view_by_name(corpus_name, text_name):
    ...
    return render_template('text/malletIndex.html', title='%s/%s' % (corpus_name, text_name), corpus_name=corpus_name, text_name=text_name, pages=pages, rules=rules, tot_tags=tot_tags)
...
def get_mallet_line_graph(corpus_name, text_name, ranking_type):
    ...
    elif text_name in os.listdir(html_root):
        currHTMLdir = os.path.join(html_root, text_name)
