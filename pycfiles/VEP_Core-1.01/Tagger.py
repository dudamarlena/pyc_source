...
# [GCC 8.4.0]
# Embedded file name: C:\LocalUsers\ealexand\VEP_Core\Ity\Taggers\Tagger.py
...
class Tagger(BaseClass):
    ...
    * The tag is only returned by self.tag() if the appropriate value of
    ...
    empty_rule = {'name': None, 
       'full_name': None, 
       'num_tags': 0, 
       'num_included_tokens': 0}
    empty_tag = {'rules': [], 'index_start': -1, 
       'index_end': -1, 
       'len': 0, 
       'pos_start': -1, 
       'pos_end': -1, 
    ...
    def __init__(self, debug=False, label=None, excluded_token_types=(), case_sensitive=True, untagged_rule_name=None, no_rules_rule_name=None, excluded_rule_name=None, return_untagged_tags=False, return_no_rules_tags=False, return_excluded_tags=False, return_included_tags=False):
        """
        ...
        self._full_label += ('.').join([ str(setting) for setting in [
         ...
         self.case_sensitive]
                                       ] + [ 'EXCL_' + meta_rule_name for meta_rule_name in self.excluded_meta_rule_names
                                           ])
        return
    ...
    def _should_return_rule(self, rule):
        ...
        * self.return_included_tags is True OR self.return_included_tags is
        ...
        :rtype: bool
        """
        return rule['name'] not in self.excluded_meta_rule_names and (self.return_included_tags or not self.return_included_tags and rule['name'] in self.meta_rule_names)
    ...
    def _is_valid_rule(self, rule):
        ...
        * A "num_included_tokens" key whose value is a non-negative int.
        ...
        :rtype: bool
        """
        return rule is not None and 'name' in rule and type(rule['name']) is str and 'full_name' in rule and type(rule['full_name']) is str and rule['full_name'].startswith(self.full_label) and 'num_tags' in rule and type(rule['num_tags']) is int and rule['num_tags'] >= 0 and type(rule['num_included_tokens']) is int and rule['num_included_tokens'] >= 0
    ...
    def _is_valid_tag(self, tag):
        ...
        * A key "token_end_len" whose value is an int greater than zero.
        ...
        :rtype: bool
        """
        return tag is not None and tag['rules'] is not None and len(tag['rules']) > 0 and 0 <= tag['index_start'] < len(self.tokens) and tag['index_start'] <= tag['index_end'] < len(self.tokens) and tag['len'] > 0 and tag['pos_start'] >= 0 and tag['pos_end'] >= 0 and tag['pos_start'] <= tag['pos_end'] and tag['token_end_len'] > 0
