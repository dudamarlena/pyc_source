...
# [GCC 8.4.0]
# Embedded file name: /Users/issackelly/Projects/python/servee/servee/frontendadmin/sites.py
# Compiled at: 2012-01-31 09:47:57
...
class ServeeAdminSite(AdminSite):
    ...
    def get_urls(self):
        ...
        for path, view, name in self.custom_views:
            urls += patterns('', url('^%s$' % path, self.admin_view(view)))

        for insert_model_lookup, insert in self.insert_classes.iteritems():
            urls += patterns('', (
             '^insert/%s/%s/' % (insert.model._meta.app_label, insert.model._meta.module_name), include(insert.urls)))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.index_template = [
         'servee/index.html', 'admin/index.html']
