...
# [GCC 8.4.0]
# Embedded file name: /Users/issackelly/Projects/python/servee/servee/frontendadmin/insert.py
# Compiled at: 2012-09-09 15:18:07
...
class ModelInsert(BaseInsert):
    ...
    def render(self, item):
        ...
        return t.render({'MEDIA_URL': settings.MEDIA_URL, 
    ...
    def get_urls(self):
        ...
        return patterns('', url('^panel/$', wrap(self.panel_view), name='insert_%s_%s_panel' % info), url('^list/$', wrap(self.list_view), name='insert_%s_%s_list' % info), url('^add_minimal/$', self.add_view, name='insert_%s_%s_add' % info), url('^(.+)/detail/$', wrap(self.detail_view), name='insert_%s_%s_detail' % info), url('^(.+)/render/$', wrap(self.render_view), name='insert_%s_%s_render' % info), url('^(.+)/delete/$', wrap(self.delete_view), name='insert_%s_%s_delete' % info))
    ...
    def panel_view(self, request):
        return render_to_response(self.item_panel_template, {'insert': self}, context_instance=RequestContext(request))
    ...
    def list_view(self, request):
        ...
        return render_to_response(self.item_list_template, {'insert': self, 
           'form': form(prefix=self.base_url())}, context_instance=RequestContext(request))

    def detail_url(self, object_id):
        return reverse('%s:insert_%s_%s_detail' % (
         self.admin_site.name,
         self.model._meta.app_label,
         self.model._meta.module_name), args=(
         object_id,))

    def detail_view(self, request, object_id):
        obj = self.get_object(unquote(object_id))
        form = self.get_minimal_add_form()
        return render_to_response(self.item_detail_template, {'insert': self, 
           'object': obj, 
           'form': form(prefix=self.base_url())}, context_instance=RequestContext(request))
    ...
    def render_view(self, request, object_id):
        obj = self.get_object(unquote(object_id))
        return render_to_response(self.item_render_template, {'insert': self, 'object': obj}, context_instance=RequestContext(request))
    ...
    def get_minimal_add_form(self):
        ...
        instance_form = modelform_factory(self.model, form=self.add_form, exclude=exclude_fields)
