...
# [GCC 8.4.0]
# Embedded file name: /Users/ppd-03020186/PycharmProjects/cocoNLP/cocoNLP/config/basic/time_nlp/TimeUnit.py
# Compiled at: 2019-03-05 06:37:21
# Size of source mod 2**32: 41330 bytes
import regex as re, arrow, copy
...
class TimeUnit:
    ...
    def norm_sethour(self):
        ...
        if match is not None:
            if self.tp.tunit[3] == -1:
                ...
                if 12 <= self.tp.tunit[3] <= 23:
                    self.tp.tunit[3] -= 12
                else:
                    if self.tp.tunit[3] == 0:
                        self.tp.tunit[3] = 12
            self.preferFuture(3)
            self.isAllDayTime = False
    ...
    def norm_setHoliday(self):
        rule = '(情人节)|(母亲节)|(青年节)|(教师节)|(中元节)|(端午)|(劳动节)|(7夕)|(建党节)|(建军节)|(初13)|(初14)|(初15)|(初12)|(初11)|(初9)|(初8)|(初7)|(初6)|(初5)|(初4)|(初3)|(初2)|(初1)|(中和节)|(圣诞)|(中秋)|(春节)|(元宵)|(航海日)|(儿童节)|(国庆)|(植树节)|(元旦)|(重阳节)|(妇女节)|(记者节)|(立春)|(雨水)|(惊蛰)|(春分)|(清明)|(谷雨)|(立夏)|(小满 )|(芒种)|(夏至)|(小暑)|(大暑)|(立秋)|(处暑)|(白露)|(秋分)|(寒露)|(霜降)|(立冬)|(小雪)|(大雪)|(冬至)|(小寒)|(大寒)'
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
    ...
    def china_24_st(self, year: int, china_st: str):
        ...
        return (
         solar_terms[china_st][1], str(flag_day))
    ...
    def checkContextTime(self, checkTimeIndex):
        ...
        if self.isFirstTimeSolveContext is True:
            if checkTimeIndex == 3:
                if self.tp_origin.tunit[checkTimeIndex] >= 12:
                    if self.tp.tunit[checkTimeIndex] < 12:
