...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/mop/filesctl.py
# Compiled at: 2020-04-02 21:14:08
# Size of source mod 2**32: 5701 bytes
import logging
...
class AudioFileListStore:
    ...
    ALBUM = 4
    TEXT_WEIGHT = 5
    model_map = {FILENAME: ('Filename', str), 
     TRACK_NUM: ('Track', str), 
     TITLE: ('Title', str), 
     ARTIST: ('Artist', str), 
     ALBUM: ('Album', str), 
    ...
    def makeRow(audio_file):
        ...
        dirty = audio_file.tag and audio_file.tag.is_dirty or audio_file.second_v1_tag and audio_file.second_v1_tag.is_dirty
...
class FileListControl(GObject.GObject):
    __gsignals__ = {'current-edit-changed': (GObject.SIGNAL_RUN_LAST, None, [])}
    ...
    def dirty_files(self):
        curr_files = self.list_store._audio_files.values() if self.list_store._audio_files is not None else []
        return [af for af in curr_files if not af.tag.is_dirty if af.second_v1_tag and af.second_v1_tag.is_dirty]
