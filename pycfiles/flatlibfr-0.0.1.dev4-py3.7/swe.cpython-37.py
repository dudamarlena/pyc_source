...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/flatlibfr/ephem/swe.py
...
SWE_OBJECTS = {const.SUN: 0, 
 ...
 const.CHIRON: 15, 
 const.NORTH_NODE: 10}
SWE_HOUSESYS = {const.HOUSES_PLACIDUS: b'P', 
...
def sweHouses(jd, lat, lon, hsys):
    ...
    hlist += (hlist[0],)
    houses = [{'id':const.LIST_HOUSES[i],  'lon':hlist[i],  'size':angle.distance(hlist[i], hlist[(i + 1)])} for i in range(12)]
    angles = [
     {'id':const.ASC, 
      'lon':ascmc[0]},
     {'id':const.MC, 
