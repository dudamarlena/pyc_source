...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fdfs_client/client.py
# Compiled at: 2018-11-05 04:52:27
# Size of source mod 2**32: 26354 bytes
"""
...
class Fdfs_client(object):
    ...
    def upload_by_buffer(self, filebuffer, file_ext_name=None, meta_dict=None):
        ...
        tc = Tracker_client(self.tracker_pool)
        store_serv = tc.tracker_query_storage_stor_without_group()
        return self.get_storage(store_serv).storage_upload_by_buffer(tc, store_serv, filebuffer, file_ext_name, meta_dict)
    ...
    def upload_slave_by_filename(self, filename, remote_file_id, prefix_name, meta_dict=None):
        ...
        try:
            ret_dict = store.storage_upload_slave_by_filename(tc, store_serv, filename, prefix_name,
              remote_filename, meta_dict=None)
        except:
            raise
    ...
    def upload_slave_by_file(self, filename, remote_file_id, prefix_name, meta_dict=None):
        ...
        store = self.get_storage(store_serv)
        try:
            ret_dict = store.storage_upload_slave_by_file(tc, store_serv, filename, prefix_name,
              remote_filename, meta_dict=None)
        except:
            raise
    ...
    def upload_slave_by_buffer(self, filebuffer, remote_file_id, meta_dict=None, file_ext_name=None):
        ...
        store = self.get_storage(store_serv)
        return store.storage_upload_slave_by_buffer(tc, store_serv, filebuffer, remote_filename, meta_dict, file_ext_name)
    ...
    def download_to_buffer(self, remote_file_id, offset=0, down_bytes=0):
        ...
        store = Storage_client(store_serv.ip_addr, store_serv.port, self.timeout)
        file_buffer = None
        return store.storage_download_to_buffer(tc, store_serv, file_buffer, file_offset, download_bytes, remote_filename)
