...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/fdfs_client/tracker_client.py
# Compiled at: 2018-11-05 04:52:27
# Size of source mod 2**32: 22865 bytes
...
def parse_storage_status(status_code):
    try:
        ret = {FDFS_STORAGE_STATUS_INIT: lambda : 'INIT', 
         ...
         FDFS_STORAGE_STATUS_RECOVERY: lambda : 'RECOVERY'}[status_code]()
    except KeyError:
        ret = 'UNKNOW'
...
class Storage_info(object):
    ...
    def set_info(self, bytes_stream):
        self.status, id, ip_addr, domain_name, src_ip_addr, version, join_time, up_time, totalMB, freeMB, self.upload_prio, self.store_path_count, self.subdir_count_per_path, self.storage_port, self.storage_http_port, self.curr_write_path, self.total_upload_count, self.success_upload_count, self.total_append_count, self.success_append_count, self.total_modify_count, self.success_modify_count, self.total_truncate_count, self.success_truncate_count, self.total_setmeta_count, self.success_setmeta_count, self.total_del_count, self.success_del_count, self.total_download_count, self.success_download_count, self.total_getmeta_count, self.success_getmeta_count, self.total_create_link_count, self.success_create_link_count, self.total_del_link_count, self.success_del_link_count, self.total_upload_bytes, self.success_upload_bytes, self.total_append_bytes, self.total_append_bytes, self.total_modify_bytes, self.success_modify_bytes, self.total_download_bytes, self.success_download_bytes, self.total_sync_in_bytes, self.success_sync_in_bytes, self.total_sync_out_bytes, self.success_sync_out_bytes, self.total_file_open_count, self.success_file_open_count, self.total_file_read_count, self.success_file_read_count, self.total_file_write_count, self.success_file_write_count, last_source_sync, last_sync_update, last_synced_time, last_heartbeat_time, self.if_trunk_server = struct.unpack(self.fmt, bytes_stream)
        try:
...
class Group_info(object):
    ...
    def set_info(self, bytes_stream):
        group_name, totalMB, freeMB, trunk_freeMB, self.count, self.storage_port, self.store_http_port, self.active_count, self.curr_write_server, self.store_path_count, self.subdir_count_per_path, self.curr_trunk_file_id = struct.unpack(self.fmt, bytes_stream)
        try:
