...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/beetsplug/extrafiles.py
# Compiled at: 2020-03-07 18:36:19
# Size of source mod 2**32: 9139 bytes
"""beets-extrafiles plugin for beets."""
import glob, itertools, os, shutil, sys, traceback, beets.dbcore.db, beets.library, beets.mediafile, beets.plugins, beets.ui, beets.util.functemplate
...
class FormattedExtraFileMapping(beets.dbcore.db.FormattedMapping):
    __doc__ = 'Formatted Mapping that allows path separators for certain keys.'
...
class ExtraFileModel(beets.dbcore.db.Model):
    __doc__ = 'Model for a  FormattedExtraFileMapping instance.'
    _fields = {'artist': beets.dbcore.types.STRING, 
...
class ExtraFilesPlugin(beets.plugins.BeetsPlugin):
    ...
    def _copy_file(self, path, dest):
        ...
        if os.path.isdir(path):
            if os.path.exists(dest):
                raise beets.util.FilesystemError('file exists', 'copy', (
                 path, dest))
            ...
            try:
                ...
                raise beets.util.FilesystemError(exc, 'copy', (path, dest), traceback.format_exc())
    ...
    def process_items(self, files, action):
        ...
        for source, destination in files:
            if not os.path.exists(source):
                ...
                self._log.warning('Skipping already present destination file: {0}', destination)
                continue
                ...
                try:
                    ...
                    self._log.warning('Failed to process file: {} -> {}', source, destpath)
    ...
    def gather_files(self, itemops):
        ...
        for _, itemopgroup in itertools.groupby(sorted_itemops, key=group):
            ...
            self._log.debug('{0} -> {1} ({2.album} by {2.albumartist}, {3} tracks)', source, destination, item, len(items))
            meta = {'artist': item.artist or 'None', 
             'albumartist': item.albumartist or 'None', 
             'album': item.album or 'None', 
             'albumpath': beets.util.displayable_path(destination)}
            for path, category in self.match_patterns(source, skip=self._scanned_paths):
    ...
    def get_destination(self, path, category, meta):
        ...
        mapping = FormattedExtraFileMapping(ExtraFileModel(basename=old_basename, filename=old_filename, **meta), for_path=True)
        for query, path_format in self.path_formats:
            if query == category:
                break
        else:
            path_format = beets.util.functemplate.Template('$albumpath/$filename')
