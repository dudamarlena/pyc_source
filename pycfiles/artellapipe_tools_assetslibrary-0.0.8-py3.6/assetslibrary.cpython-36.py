...
# Compiled at: 2020-04-15 10:43:33
# Size of source mod 2**32: 21318 bytes
"""
            ...
            for j in range(self._assets_viewer.columnCount()):
                ...
                if not item:
                    ...
                    if asset_name not in self._cache:
                        pass
                self._cache[asset_name] = {'item':item, 
                 'has_label':False, 
                ...
                try:
                    try:
                        ...
                        for file_type, extensions in file_info.items():
                            if not extensions:
                                LOGGER.warning('No Extension defined for File Type "{}" in artellapipe.tools.assetslibrary configuration file!'.format(file_type))
                            else:
                                for extension, operation in extensions.items():
                                    ...
                                    if res and self._fit_camera_cbx.isChecked():
                                        try:
                                            ...
                                            LOGGER.warning('Impossible to fit camera view to referenced objects | {}!'.format(exc))
