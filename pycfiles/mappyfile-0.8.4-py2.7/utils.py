...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\mappyfile\utils.py
# Compiled at: 2020-01-11 08:01:06
from __future__ import unicode_literals
import codecs, warnings, functools
...
def deprecated(func):
    ...
    def new_func(*args, **kwargs):
        warnings.simplefilter(b'always', DeprecationWarning)
        warnings.warn((b'Call to deprecated function {}.').format(func.__name__), category=DeprecationWarning, stacklevel=2)
        warnings.simplefilter(b'default', DeprecationWarning)
...
def open(fn, expand_includes=True, include_comments=False, include_position=False, **kwargs):
    ...
    """
    p = Parser(expand_includes=expand_includes, include_comments=include_comments, **kwargs)
    ast = p.parse_file(fn)
    m = MapfileToDict(include_position=include_position, include_comments=include_comments, **kwargs)
    d = m.transform(ast)
    return d
...
def dump(d, fp, indent=4, spacer=b' ', quote=b'"', newlinechar=b'\n', end_comment=False):
    r"""
...
def save(d, output_file, indent=4, spacer=b' ', quote=b'"', newlinechar=b'\n', end_comment=False, **kwargs):
    r"""
...
def dumps(d, indent=4, spacer=b' ', quote=b'"', newlinechar=b'\n', end_comment=False, **kwargs):
    r"""
...
def update(d1, d2):
    ...
    """
    NoneType = type(None)
    if d2.get(b'__delete__', False):
        return {}
    else:
        for k, v in d2.items():
            if isinstance(v, dict):
                if v.get(b'__delete__', False):
                    del d1[k]
                else:
                ...
                for orig_item, new_item in pairs:
                    ...
                    if new_item is None:
                        new_item = {}
                    if new_item.get(b'__delete__', False):
                        d = None
                    else:
                ...
                d1[k] = new_list
            elif k in d1 and v == b'__delete__':
                del d1[k]
            else:
                d1[k] = v
...
def _save(output_file, map_string):
    with codecs.open(output_file, b'w', encoding=b'utf-8') as (f):
        f.write(map_string)


def _pprint(d, indent, spacer, quote, newlinechar, end_comment, **kwargs):
    pp = PrettyPrinter(indent=indent, spacer=spacer, quote=quote, newlinechar=newlinechar, end_comment=end_comment, **kwargs)
    return pp.pprint(d)