...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/codes/flexible_clustering_tree/flexible_clustering_tree/models.py
# Compiled at: 2019-10-29 19:19:45
# Size of source mod 2**32: 29918 bytes
...
class FeatureMatrixObject(object):
    ...
    def __init__(self, level: int, matrix_object: Union[(ndarray, csr_matrix)]=None, feature_strings: List[str]=None):
        self.level = level
        self.matrix_object = matrix_object
        self.feature_strings = feature_strings
        if matrix_object is None:
            if feature_strings is None:
        ...
        if matrix_object is None:
            ...
            elif matrix_object is not None:
    ...
    def get_index_size(self) -> int:
        ...
        if isinstance(self.feature_object, ndarray):
            return self.feature_object.shape[0]
        raise Exception()

    def __str__(self):
        _FeatureMatrixObject__feature_type = type(self.feature_object)
        __ = '@level={} feature-type={}'.format(self.level, _FeatureMatrixObject__feature_type)
...
class MultiFeatureMatrixObject(object):
    __doc__ = 'Class for 1-input of recursive-clustering'
    __slots__ = ('dict_level2feature_obj', 'dict_index2label', 'dict_index2attributes',
                 'matrix_first_layer', 'text_aggregation_field', 'n_docs')

    def __init__(self, matrix_objects: List[FeatureMatrixObject], dict_index2label: Dict[(int, str)], dict_index2attributes: Optional[Dict[(int, Dict)]]=None, text_aggregation_field: Optional[List[List[str]]]=None):
        """Init the class.
        ...
        """
        if not isinstance(matrix_objects, list):
            raise AssertionError
        else:
            if not isinstance(dict_index2label, dict):
                raise AssertionError
            else:
                self._MultiFeatureMatrixObject__check_level_zero(matrix_objects)
                matrix_index_sizes = [m_o.get_index_size() for m_o in matrix_objects]
                if len(set(matrix_index_sizes)) != 1:
                    raise Exception('The given matrix size is not same. Sizes of the given matrix sizes are {}'.format(set(matrix_index_sizes)))
                else:
                    self.n_docs = matrix_index_sizes[0]
            if text_aggregation_field is not None:
                assert len(text_aggregation_field) == self.n_docs, 'len(text_aggregation_field) should be same as index of the give matrix. len(text_aggregation_field)={} but len(index of the give matrix)={}'.format(len(text_aggregation_field), self.n_docs)
                self.text_aggregation_field = text_aggregation_field
            else:
                self.text_aggregation_field = text_aggregation_field
        self._MultiFeatureMatrixObject__check_match_index_size(matrix_index_sizes, dict_index2label)
...
class ClusteringOperator(object):
    ...
    def __init__(self, level: int, n_cluster: int, instance_clustering):
...
class MultiClusteringOperator(object):
    ...
    def __init__(self, clustering_operators: List[ClusteringOperator]):
        self._MultiClusteringOperator__check_level_zero(clustering_operators)
        self.clustering_operators = clustering_operators
        self.dict_level2operator = {c_operator.level:c_operator for c_operator in clustering_operators}
    ...
    def to_dict(self):
        return {'operators': [c.to_dict() for c in self.clustering_operators]}


class ClusterObject(object):
    __doc__ = 'Class for saving one-node of a tree'
    __types__ = ('cluster_id', 'parent_cluster_id', 'data_ids', 'average_vector', 'child_cluster_ids',
                 'feature_matrix', 'matrix_depth_level', 'feature_type', 'feature_object',
                 'dict_submatrix_index2original_matrix_index', 'clustering_label')

    def __init__(self, cluster_id: int, parent_cluster_id: int, average_vector: ndarray, data_ids: List[int], matrix_depth_level: int=None, child_cluster_ids: List[int]=None, feature_object: Union[(List[str], ndarray)]=None, clustering_label: str=None, dict_submatrix_index2original_matrix_index: Dict[(int, int)]=None, feature_matrix: ndarray=None):
        """
        ...
        elif isinstance(feature_object, list):
            if all([isinstance(_, str) for _ in feature_object]):
...
class ClusterTreeObject(object):
    __doc__ = 'Class for saving tree information as a result of clustering'

    def __init__(self, dict_child_id2parent_id: Dict[(int, int)], dict_depth2clustering_result: Dict[(int, Dict[(int, ClusterObject)])], multi_matrix_object: MultiFeatureMatrixObject, multi_clustering_object: MultiClusteringOperator, func_label_aggregation: Callable[([List[str]], Tuple[(str, int)])]=default_fun_string_aggregation):
        """
        ...
        self.dict_parent_cluster_id2children_cluster_id = self._ClusterTreeObject__generate_parent2children()
        self.dict_cluster_id2cluster_obj = self._ClusterTreeObject__generate_cluster_id2cluster_obj()
    ...
    def child_cluster_id2parent_cluster_id(self, child_cluster_id: int) -> Union[(int, bool)]:
        ...
        """
        if child_cluster_id in self.dict_child_id2parent_id:
    ...
    def __generate_parent2children(self) -> Dict[(int, List[int])]:
        ...
        for p_node_id, g_obj in groupby(sorted((self.dict_child_id2parent_id.items()), key=(lambda t_parent_and_child: t_parent_and_child[1])),
    ...
    def __generate_cluster_id2cluster_obj(self) -> Dict[(int, ClusterObject)]:
        ...
        """
        cluster_id2cluster_obj = {cluster_obj.cluster_id:cluster_obj for _s_cluster_obj in self.dict_depth2clustering_result.values() for cluster_obj in _s_cluster_obj.values()}
        return cluster_id2cluster_obj

    @staticmethod
    def __update_child_cluster_nodes(dict_cluster_id2cluster_obj: Dict[(int, ClusterObject)], dict_parent_cluster_id2children_cluster_id: Dict[(int, List[int])]) -> None:
        ...
        for cluster_node_id in dict_cluster_id2cluster_obj.keys():
            if cluster_node_id in dict_parent_cluster_id2children_cluster_id:
                _ClusterTreeObject__update(dict_cluster_id2cluster_obj[cluster_node_id], dict_parent_cluster_id2children_cluster_id[cluster_node_id])
                continue
    ...
    def aggregate_node_information(self, is_aggregation_text_field: bool=False) -> Dict[(int, Dict[(str, Any)])]:
        ...
        def make_aggregation(_cluster_id, _cluster_node_information):
            ...
            if _cluster_id in dict_parent_cluster_id2child_cluster_id:
                _n_data_point = sum([len(self.dict_cluster_id2cluster_obj[c_id].data_ids) for c_id in dict_parent_cluster_id2child_cluster_id[_cluster_id]])
            else:
                _n_data_point = len(self.dict_cluster_id2cluster_obj[_cluster_id].data_ids)
            if is_aggregation_text_field:
                _ClusterTreeObject__text_data_source = [self.multi_matrix_object.text_aggregation_field[d_id] for d_id in _cluster_node_information.data_ids]
                frequent_text = self.func_label_aggregation(list(chain.from_iterable(_ClusterTreeObject__text_data_source)))
            else:
                frequent_text = self.func_label_aggregation([self.multi_matrix_object.dict_index2label[d_id] for d_id in _cluster_node_information.data_ids])
            dict_nodeid2agg_info[_cluster_id] = {'node-type':'branch', 
             '#data':_n_data_point, 
             'frequent_labels':frequent_text}

        seq_cluster_node_information_obj = [t for t in sorted((self.dict_cluster_id2cluster_obj.items()), key=(lambda t: t[0]), reverse=True)]
        dict_nodeid2agg_info = {}
        t_parentid_g_obj = groupby(sorted((self.dict_child_id2parent_id.items()), key=(lambda t: t[1])), key=(lambda t: t[1]))
        dict_parent_cluster_id2child_cluster_id = {parent_id:[t[0] for t in g_obj] for parent_id, g_obj in t_parentid_g_obj}
    ...
    def reshape_tree(self) -> Dict[(int, int)]:
        ...
        def make_reshape_operation(_parent_cluster_id, _seq_child_cluster_id):
            if len(_seq_child_cluster_id) != 1:
                ...
                while True:
                    if current_parent_cluster_id in dict_parent_clusterid2child_cluster_id:
                        seq_child_cluster_id_next_level = dict_parent_clusterid2child_cluster_id[current_parent_cluster_id]
        ...
        t_parentid_g_obj = groupby(sorted((self.dict_child_id2parent_id.items()), key=(lambda t: t[1])), key=(lambda t: t[1]))
        dict_parent_clusterid2child_cluster_id = {parent_id:[t[0] for t in g_obj] for parent_id, g_obj in t_parentid_g_obj}
        tmp_dict_parent_clusterid2child_cluster_id = {}
        set_stack_processed_id = set()
        for parent_cluster_id, seq_child_cluster_id in sorted((dict_parent_clusterid2child_cluster_id.items()), key=(lambda t: t[0])):
        ...
        updated_dict_child2parent = {child_cluster_id:parent_cluster_id for parent_cluster_id, seq_child_cluster_id in tmp_dict_parent_clusterid2child_cluster_id.items() for child_cluster_id in seq_child_cluster_id}
        return updated_dict_child2parent

    def __generate_branch_node_template(self, cluster_node_data: ClusterObject, dict_cluster_id2cluster_agg_info: Optional[Dict[(int, Dict[(str, Counter)])]], sub_tree_information: Optional[List[Dict[(str, Any)]]]=None, is_child_with_underscore: bool=True) -> Dict[(str, Any)]:
        """
        """
        if sub_tree_information is None:
            children = [self._ClusterTreeObject__generate_leaf_node_template(data_id, cluster_node_data) for data_id in cluster_node_data.data_ids]
        else:
        ...
        cluster_size = len(set(cluster_node_data.child_cluster_ids)) if cluster_node_data.child_cluster_ids is not None else None
        return {'name': str(cluster_node_data.cluster_id), 
         child_field_name: children, 
    ...
    def __generate_leaf_node_template(self, data_id: int, cluster_node_data: ClusterObject) -> Dict[(str, Any)]:
        ...
        if isinstance(self.multi_matrix_object.dict_index2attributes, dict):
            if data_id in self.multi_matrix_object.dict_index2attributes:
                information = self.multi_matrix_object.dict_index2attributes[data_id]
        else:
            information = {}
        return {'name':str(data_id), 
    ...
    def get_labels(self) -> Dict[(int, int)]:
        ...
        return {data_id:c_id for c_id, c_obj in self.dict_cluster_id2cluster_obj.items() for data_id in c_obj.data_ids}
    ...
    def to_dict(self, is_child_with_underscore: bool=True) -> Dict[(str, Any)]:
        ...
        if len(self.updated_dict_child2parent) == 0:
            raise Exception('There is 0 cluster for given dataset. This might be too small amount of data to construct clusters.')
        root_node_id = min(self.updated_dict_child2parent.values())
        dict_subtree_pack = {parent_id:[t[0] for t in g_obj] for parent_id, g_obj in groupby(sorted((self.updated_dict_child2parent.items()), key=(lambda t: t[1])), key=(lambda t: t[1]))}
        dict_stack_constructed_subtree = {}
        for subtree_parent_id, seq_subtree_node in sorted((dict_subtree_pack.items()), key=(lambda t: t[0]),
          reverse=True):
            stack_subtree = [
             None] * len(seq_subtree_node)
            for i, sub_tree_cluster_id in enumerate(seq_subtree_node):
                ...
                if sub_tree_cluster_id in dict_stack_constructed_subtree:
                    generated_dict_object = self._ClusterTreeObject__generate_branch_node_template(cluster_node_data=cluster_information_data,
    ...
    def to_html(self, visual_html: str=None):
        ...
        visual_template_html = pkgutil.get_data('flexible_clustering_tree', 'resources/tree_template.html').decode('utf-8')
        ...
        dict_render_values = {'tree_object': json.dumps(dict_d3_object, ensure_ascii=False)}
    ...
    def to_objects(self) -> Dict[(str, Dict[(str, Any)])]:
        ...
        _cluster_relation_table_obj = {'cluster_id':None,  'parent_id':None, 
         'depth_level':None, 
         'clustering_method':None}
        _ClusterTreeObject__cluster_relation_table_rows = []
        for child_cid, parent_cid in self.dict_child_id2parent_id.items():
            c_obj = self.dict_cluster_id2cluster_obj[child_cid]
            _ClusterTreeObject__relation_table_obj = deepcopy(_cluster_relation_table_obj)
            _ClusterTreeObject__relation_table_obj['cluster_id'] = child_cid
            _ClusterTreeObject__relation_table_obj['parent_id'] = parent_cid
            _ClusterTreeObject__relation_table_obj['depth_level'] = c_obj.matrix_depth_level
            _ClusterTreeObject__relation_table_obj['clustering_method'] = c_obj.clustering_label
            _ClusterTreeObject__cluster_relation_table_rows.append(_ClusterTreeObject__relation_table_obj)

        _ClusterTreeObject__node_table_rows = []
        _node_table_obj = {'leaf_id':None, 
         'cluster_id':None,  'label':None,  'args':None}
        d_id2c_id = {d_id:c_obj.cluster_id for c_obj in self.dict_cluster_id2cluster_obj.values() for d_id in c_obj.data_ids}
        for data_id, label in self.multi_matrix_object.dict_index2label.items():
            _ClusterTreeObject__leaf_object = None if self.multi_matrix_object.dict_index2attributes is None else json.dumps((self.multi_matrix_object.dict_index2attributes[data_id]), ensure_ascii=False)
            _ClusterTreeObject__node_table_obj = deepcopy(_node_table_obj)
            _ClusterTreeObject__node_table_obj['leaf_id'] = data_id
            _ClusterTreeObject__node_table_obj['cluster_id'] = d_id2c_id[data_id]
            _ClusterTreeObject__node_table_obj['label'] = self.multi_matrix_object.dict_index2label[data_id]
            _ClusterTreeObject__node_table_obj['args'] = _ClusterTreeObject__leaf_object
            _ClusterTreeObject__node_table_rows.append(_ClusterTreeObject__node_table_obj)

        __['cluster_information'] = _ClusterTreeObject__cluster_relation_table_rows
        __['leaf_information'] = _ClusterTreeObject__node_table_rows
        return __