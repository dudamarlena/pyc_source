...
# [GCC 8.4.0]
# Embedded file name: /Users/kensuke-mi/codes/flexible_clustering_tree/flexible_clustering_tree/interface.py
# Compiled at: 2019-10-29 19:04:14
...
class FlexibleClustering(BaseEstimator):
    ...
    def __init__(self, max_depth: int, is_auto_switch: bool=True, threshold_ratio_auto_switch: float=10.0, threshold_minimum_unique_vector: int=10, multi_clustering_operator: MultiClusteringOperator=None):
        """Flexible clustering
    ...
    def fit(self, x: MultiFeatureMatrixObject, y=None, multi_clustering_operator: MultiClusteringOperator=None):
        """
        ...
        :return: itself
        """
        if self.multi_clustering_operator is None:
            if multi_clustering_operator is None:
        ...
        clustering_tree = clustering_runner.run_recursive_clustering(multi_clustering_operator=multi_clustering_operator,
        ...
        _FlexibleClustering__labels_ = [None] * len(x.dict_index2label.keys())
        for cluster_obj in self.clustering_tree.dict_cluster_id2cluster_obj.values():
            for d_id in cluster_obj.data_ids:
                _FlexibleClustering__labels_[d_id] = cluster_obj.cluster_id

        else:
            self.labels_ = _FlexibleClustering__labels_
    ...
    def fit_transform(self, x: MultiFeatureMatrixObject, multi_clustering_operator: MultiClusteringOperator=None) -> Dict[(int, int)]:
    ...
    def get_params(self, deep=True):
        return {'multi_clustering_operator':self.multi_clustering_operator.to_dict(), 
         'max_depth':self.max_depth, 
         'is_auto_switch':self.is_auto_switch, 
         'threshold_ratio_auto_switch':self.threshold_ratio_auto_switch, 
         'threshold_minimum_unique_vector':self.threshold_minimum_unique_vector}
