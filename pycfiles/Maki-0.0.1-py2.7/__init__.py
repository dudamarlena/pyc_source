...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/maki/__init__.py
# Compiled at: 2011-10-30 12:23:35
import unittest
...
class List(list):
    ...
    def map(self, f):
        return List((f(x) if hasattr(f, '__call__') else f) for x in self)
...
List(['append', 'extend', 'sort', 'reverse', 'insert']).map(List._proxy)
...
class ListTest(unittest.TestCase):
    ...
    def test_mix(self):
        r = self.seq.map(lambda x: x * 2).rem(lambda x: x < 5).keep(lambda x: x % 2 == 0).rem(6)
        self.assertEqual(r, [8, 10])

    def test_self(self):
        self.assertEqual(self.seq.map(lambda x: x * 2).rem(lambda x: x < 5).keep(lambda x: x % 2 == 0).rem(6).do(self.assertEqual, [8, 10]), [
         8, 10])
        self.seq.keep(lambda x: x > 4).do(self.assertEqual, [5]).last_value = [
         5]
    ...
    def test_uniq(self):
        self.assertEqual(List([2, 1, 2, 3, 2, 1, 2, 3]).uniq().sort(), [
         1, 2, 3])
        List([dict(a=1), dict(b=1), dict(c=2)]).uniq(lambda x: x.items()[0][1]).do(lambda seq: self.assertTrue(dict(a=1) in seq or dict(b=1) in seq)).do(lambda seq: self.assertTrue(dict(c=2) in seq)).do(lambda seq: self.assertEqual(seq.count(), 2))
        List('test').uniq().sort().do(self.assertEqual, list('est'))
...
class DictTest(unittest.TestCase):
    ...
    def test_map(self):
        ...
        self.assertEqual(self.seq.map(lambda x, y: x), dict(a='a', b='b', c='c'))
        self.assertEqual(self.seq.map(lambda x, y: y * 2), dict(a=2, b=4, c=6))
    ...
    def test_update(self):
        self.assertEqual(self.seq.update(dict(a=5, d=1)), dict(a=5, b=2, c=3, d=1))
    ...
    def test_fromkeys(self):
        ...
        self.assertEqual(Dict.fromkeys([1, 2, 3]), {1: None, 2: None, 3: None})
        return

    def test_compact(self):
        self.assertEqual(self.seq.update(a=None, b=[]).compact(), dict(c=3))
        self.assertEqual(self.seq.update(a=None, b=[]).compact(lambda x, y: y == None), dict(b=[], c=3))
        return
    ...
    def test_empty(self):
        ...
        self.assertTrue(Dict(zip([1, 2, 3], [None, 0, 0])).empty(lambda x, y: y in (None, 0)))
        self.assertFalse(Dict(zip([1, 2, 3], [None, [], 0])).empty(lambda x, y: y in (None, 0)))
        return

    def test_do(self):
        self.assertEqual(self.seq.map(lambda x, y: y * 2).rem(4).do(self.assertEqual, dict(a=2, c=6)), dict(a=2, c=6))
        self.assertEqual(self.seq.do(lambda self: 2).last_value, 2)

    def test_invoke(self):
        Dict(a='hello', b='hi').invoke('upper').do(self.assertEqual, dict(a='HELLO', b='HI'))
        Dict(a='hello', b='hi').invoke('find', 'h').do(self.assertEqual, dict(a=0, b=0))
