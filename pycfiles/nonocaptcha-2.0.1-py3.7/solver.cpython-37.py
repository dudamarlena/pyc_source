...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/nonocaptcha/solver.py
# Compiled at: 2019-02-17 16:32:55
...
class Solver(Base):
    ...
    def __init__(self, pageurl, sitekey, loop=None, proxy=None, proxy_auth=None, options={}, enable_injection=True, retain_source=True, **kwargs):
    ...
    async def inject_widget(self):
        ...
        widget_code = f"<div class=g-recaptcha data-sitekey={self.sitekey}></div>"
    ...
    async def deface(self):
        ...
        recaptcha_url = 'https://www.google.com/recaptcha/api.js?onload=recapReady&render=explicit'
    ...
    async def goto(self):
        ...
        try:
            await self.loop.create_task(self.page.goto((self.url),
    ...
    async def _solve(self):
        solve_image = False
        if solve_image:
            self.image = SolveImage(self.browser, self.image_frame, self.proxy, self.proxy_auth, self.proc_id)
            solve = self.image.solve_by_image
        else:
            self.audio = SolveAudio(self.page, self.loop, self.proxy, self.proxy_auth, self.proc_id)
    ...
    async def wait_for_checkbox(self):
        ...
        try:
            await self.checkbox_frame.waitForFunction("jQuery('#recaptcha-anchor').length",
    ...
    async def wait_for_audio_button(self):
        ...
        try:
            await self.image_frame.waitForFunction("jQuery('#recaptcha-audio-button').length",
              timeout=(self.animation_timeout))
        except ButtonError:
