...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/nonocaptcha/audio.py
# Compiled at: 2018-12-25 20:47:37
    ...
    async def get_audio_response(self):
        ...
        try:
            ...
            if not isinstance(audio_url, str):
                raise DownloadError('Audio url is not valid, expected `str`instead received {type(audio_url)}')
        except CancelledError:
            raise DownloadError('Audio url not found, aborting')

        self.log('Downloading audio file')
        try:
            audio_data = await self.loop.create_task(util.get_page(audio_url,
              proxy=(self.proxy),
            ...
            func = f'"{audio_url}" !== jQuery(".rc-audiochallenge-tdownload-link").attr("href")'
            try:
                await self.image_frame.waitForFunction(func,
