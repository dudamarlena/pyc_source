...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/payjp/test/helper.py
# Compiled at: 2018-06-22 02:45:13
# Size of source mod 2**32: 3776 bytes
import datetime, json, os, random, re, string, unittest
...
DUMMY_CARD = {'number':'4242424242424242', 
 'exp_month':NOW.month, 
 'exp_year':NOW.year + 4}
DUMMY_CHARGE = {'amount':100, 
 'currency':'jpy', 
 'card':DUMMY_CARD}
DUMMY_PLAN = {'amount':2000, 
 'interval':'month', 
 'name':'Amazing Gold Plan', 
 'currency':'jpy', 
 'id':'payjp-test-gold-' + ''.join(random.choice(string.ascii_lowercase) for x in range(10))}
DUMMY_TRANSFER = {'amount':400, 
 'currency':'jpy', 
 'recipient':'self'}
...
class PayjpTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        payjp.api_key = os.environ.get('PAYJP_API_KEY', 'sk_test_c62fade9d045b54cd76d7036')
    ...
    def assertRaisesRegexp(self, exception, regexp, callable, *args, **kwargs):
        try:
            ...
            if regexp is None:
                ...
                raise regexp.search(str(err)) or self.failureException('"%s" does not match "%s"' % (
...
class MyComposite(payjp.resource.ListableAPIResource, payjp.resource.CreateableAPIResource, payjp.resource.UpdateableAPIResource, payjp.resource.DeletableAPIResource):
    pass