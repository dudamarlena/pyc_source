...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/payjp/test/test_resources.py
# Compiled at: 2018-06-22 02:45:13
# Size of source mod 2**32: 30475 bytes
import json, pickle, sys, time, datetime, unittest, payjp, payjp.resource
from payjp.test.helper import PayjpUnitTestCase, PayjpApiTestCase, MyListable, MyCreatable, MyUpdateable, MyDeletable, MyResource, NOW, DUMMY_CARD, DUMMY_CHARGE, DUMMY_PLAN

class PayjpObjectTests(PayjpUnitTestCase):

    def test_initializes_with_parameters(self):
        obj = payjp.resource.PayjpObject('foo',
          'bar', myparam=5, yourparam='boo')
    ...
    def test_access(self):
        ...
        self.assertEqual(['id', 'myattr', 'mydef', 'myitem'], sorted(obj.keys()))
        self.assertEqual(['itval', 'myid', 'myval', 'sdef'], sorted(obj.values()))
        self.assertRaises(ValueError, setattr, obj, 'foo', '')
        self.assertRaises(TypeError, obj.__delitem__, 'myattr')

    def test_refresh_from(self):
        obj = payjp.resource.PayjpObject.construct_from({'foo':'bar', 
         'trans':'me'}, 'mykey')
        ...
        obj.refresh_from({'foo':'baz', 
         'johnny':5},
          'key2',
          payjp_account='acct_foo')
        ...
        obj.refresh_from({'trans':4, 
         'metadata':{'amount': 42}}, 'key2', True)
        self.assertEqual('baz', obj.foo)
        self.assertEqual(4, obj.trans)

    def test_passing_nested_refresh(self):
        obj = payjp.resource.PayjpObject.construct_from({'foos': {'type':'list', 
                  'data':[
                   {'id': 'nested'}]}},
          'key',
          payjp_account='acct_foo')
    ...
    def check_invoice_data(self, data):
        ...
        self.assertEqual('month', data['lines']['subscriptions'][0]['plan']['interval'])
...
class ListObjectTests(PayjpApiTestCase):
    ...
    def setUp(self):
        ...
        self.lo = payjp.resource.ListObject.construct_from({'id':'me', 
         'url':'/my/path'}, 'mykey')
        self.mock_response([
         {'object':'charge', 
          'foo':'bar'}])
    ...
    def test_all(self):
        ...
        self.requestor_mock.request.assert_called_with('get', '/my/path', {'myparam': 'you'}, None)
        self.assertResponse(res)

    def test_create(self):
        res = self.lo.create(myparam='eter')
        self.requestor_mock.request.assert_called_with('post', '/my/path', {'myparam': 'eter'}, None)
        self.assertResponse(res)

    def test_retrieve(self):
        res = self.lo.retrieve('myid', myparam='cow')
        self.requestor_mock.request.assert_called_with('get', '/my/path/myid', {'myparam': 'cow'}, None)
        self.assertResponse(res)
    ...
    def test_convert_to_payjp_object(self):
        sample = {'foo':'bar', 
         ...
         'alist':[
          {'object':'customer', 
           'name':'chilango'}]}
        converted = payjp.resource.convert_to_payjp_object(sample, 'akey', None)
...
class CreateableAPIResourceTests(PayjpApiTestCase):
    ...
    def test_create(self):
        self.mock_response({'object':'charge', 
         'foo':'bar'})
        res = MyCreatable.create()
        self.requestor_mock.request.assert_called_with('post', '/v1/mycreatables', {})
...
class UpdateableAPIResourceTests(PayjpApiTestCase):
    ...
    def setUp(self):
        ...
        self.obj = MyUpdateable.construct_from({'id':'myid', 
         'foo':'bar', 
         'baz':'boz', 
         'metadata':{'size':'l', 
          'score':4, 
    ...
    def test_idempotent_save(self):
        ...
        self.obj.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'metadata':{},  'baz':'updated'}, None)
    ...
    def test_save(self):
        ...
        self.checkSave()
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'baz':'updated', 
         'other':'newval', 
         'metadata':{'size':'m', 
          'info':'a2', 
          'height':''}}, None)
    ...
    def test_add_key_to_nested_object(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'size':'l', 
          'score':4, 
          'height':10}}, 'mykey')
        acct.legal_entity['first_name'] = 'bob'
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'first_name': 'bob'}}, None)
    ...
    def test_replace_nested_object(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'last_name': 'smith'}}, 'mykey')
        acct.legal_entity = {'first_name': 'bob'}
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'first_name':'bob', 
                          'last_name':''}}, None)
    ...
    def test_array_setting(self):
        ...
        acct.legal_entity.additional_owners = [
         {'first_name': 'Bob'}]
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'additional_owners': [
                                                {'first_name': 'Bob'}]}}, None)

    def test_array_none(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'additional_owners': None}}, 'mykey')
        acct.foo = 'bar'
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'foo':'bar', 
         'legal_entity':{}}, None)

    def test_array_insertion(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'additional_owners': []}}, 'mykey')
        acct.legal_entity.additional_owners.append({'first_name': 'Bob'})
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'additional_owners': {'0': {'first_name': 'Bob'}}}}, None)

    def test_array_update(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'additional_owners': [
                                {'first_name': 'Bob'},
                                {'first_name': 'Jane'}]}}, 'mykey')
        acct.legal_entity.additional_owners[1].first_name = 'Janet'
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'additional_owners': {'0':{},  '1':{'first_name': 'Janet'}}}}, None)
    ...
    def test_array_noop(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         ...
         'currencies_supported':[
          'jpy', 'cad']}, 'mykey')
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'additional_owners': {'0': {}}}}, None)

    def test_hash_noop(self):
        acct = MyUpdateable.construct_from({'id':'myid', 
         'legal_entity':{'address': {'line1': '1 Two Three'}}}, 'mykey')
        self.assertTrue(acct is acct.save())
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'legal_entity': {'address': {}}}, None)
    ...
    def test_save_overwrite_metadata(self):
        self.obj.metadata = {}
        self.checkSave()
        self.requestor_mock.request.assert_called_with('post', '/v1/myupdateables/myid', {'metadata': {'size':'', 
                      'score':'', 
                      'height':''}}, None)
...
class DeletableAPIResourceTests(PayjpApiTestCase):
    ...
    def test_delete(self):
        self.mock_response({'id':'mid', 
         'deleted':True})
        obj = MyDeletable.construct_from({'id': 'mid'}, 'mykey')
...
class ChargeTest(PayjpResourceTest):
    ...
    def test_charge_list_all(self):
        ...
        self.requestor_mock.request.assert_called_with('get', '/v1/charges', {'created': {'lt': NOW}})

    def test_charge_list_create(self):
        (payjp.Charge.create)(**DUMMY_CHARGE)
        self.requestor_mock.request.assert_called_with('post', '/v1/charges', DUMMY_CHARGE)

    def test_charge_list_retrieve(self):
        payjp.Charge.retrieve('ch_test_id')
        self.requestor_mock.request.assert_called_with('get', '/v1/charges/ch_test_id', {}, None)
    ...
    def test_create_with_source_param(self):
        payjp.Charge.create(amount=100, currency='jpy', source='btcrcv_test_receiver')
        self.requestor_mock.request.assert_called_with('post', '/v1/charges', {'amount':100, 
         'currency':'jpy', 
...
class AccountTest(PayjpResourceTest):
    ...
    def test_retrieve_account(self):
        ...
        self.requestor_mock.request.assert_called_with('get', '/v1/accounts/acct_foo', {}, None)
...
class CustomerTest(PayjpResourceTest):
    ...
    def test_create_customer(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/customers', {'description':'foo bar', 
         'card':DUMMY_CARD})
    ...
    def test_unset_description(self):
        ...
        customer.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/customers/cus_unset_desc', {'description': 'Hey'}, None)
    ...
    def test_customer_add_card(self):
        customer = payjp.Customer.construct_from({'id':'cus_add_card', 
         'cards':{'object':'list', 
          'url':'/v1/customers/cus_add_card/cards'}}, 'api_key')
        customer.cards.create(card=DUMMY_CARD)
        self.requestor_mock.request.assert_called_with('post', '/v1/customers/cus_add_card/cards', {'card': DUMMY_CARD}, None)
...
class CustomerSubscriptionTest(PayjpResourceTest):
    ...
    def test_create_customer(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/customers', {'card':DUMMY_CARD, 
         'plan':DUMMY_PLAN['id']})

    def test_list_customer_subscription(self):
        customer = payjp.Customer.construct_from({'id':'cus_foo', 
         'subscriptions':{'object':'list', 
          'url':'/v1/customers/cus_foo/subscriptions'}}, 'api_key')
        customer.subscriptions.all()
        self.requestor_mock.request.assert_called_with('get', '/v1/customers/cus_foo/subscriptions', {}, None)
    ...
    def test_retrieve_customer_subscription(self):
        ...
        self.requestor_mock.request.assert_called_with('get', '/v1/customers/cus_foo/subscriptions/sub_cus', {}, None)
...
class SubscriptionTest(PayjpResourceTest):
    ...
    def test_create_subscription(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/subscriptions', {'customer':customer.id, 
         'plan':DUMMY_PLAN['id']})

    def test_delete_subscriptions(self):
        sub = payjp.Subscription.construct_from({'id':'sub_delete', 
         'customer':'cus_foo'}, 'api_key')
        sub.delete()
        self.requestor_mock.request.assert_called_with('delete', '/v1/subscriptions/sub_delete', {}, None)

    def test_update_subscription(self):
        sub = payjp.Subscription.construct_from({'id':'sub_update', 
         'customer':'cus_foo', 
         'plan':'plan_foo'}, 'api_key')
        sub.plan = DUMMY_PLAN['id']
        sub.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/subscriptions/sub_update', {'plan': DUMMY_PLAN['id']}, None)
    ...
    def test_pause_subscriptions(self):
        ...
        sub.pause()
        self.requestor_mock.request.assert_called_with('post', '/v1/subscriptions/sub_delete/pause', {}, None)
    ...
    def test_cancel_subscriptions(self):
        sub = payjp.Subscription.construct_from({'id':'sub_delete', 
         ...
         'status':'active'}, 'api_key')
        sub.cancel()
        self.requestor_mock.request.assert_called_with('post', '/v1/subscriptions/sub_delete/cancel', {}, None)
...
class PlanTest(PayjpResourceTest):
    ...
    def test_create_plan(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/plans', DUMMY_PLAN)

    def test_delete_plan(self):
        p = payjp.Plan(id='pl_delete')
        p.delete()
        self.requestor_mock.request.assert_called_with('delete', '/v1/plans/pl_delete', {}, None)

    def test_update_plan(self):
        p = payjp.Plan(id='pl_update')
        p.name = 'Plan Name'
        p.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/plans/pl_update', {'name': 'Plan Name'}, None)


class RefundTest(PayjpResourceTest):

    def test_non_recursive_save(self):
        charge = payjp.Charge.construct_from({'id':'ch_nested_update', 
    ...
    def test_fetch_refund(self):
        charge = payjp.Charge.construct_from({'id':'ch_get_refund', 
         'refunds':{'object':'list', 
          'url':'/v1/charges/ch_get_refund/refunds'}}, 'api_key')
        charge.refunds.retrieve('ref_get')
        self.requestor_mock.request.assert_called_with('get', '/v1/charges/ch_get_refund/refunds/ref_get', {}, None)
    ...
    def test_list_refunds(self):
        charge = payjp.Charge.construct_from({'id':'ch_get_refund', 
         'refunds':{'object':'list', 
          'url':'/v1/charges/ch_get_refund/refunds'}}, 'api_key')
        charge.refunds.all()
        self.requestor_mock.request.assert_called_with('get', '/v1/charges/ch_get_refund/refunds', {}, None)
...
class MetadataTest(PayjpResourceTest):
    ...
    def test_noop_metadata(self):
        ...
        charge.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_foo', {'description': 'test'}, None)

    def test_unset_metadata(self):
        charge = payjp.Charge(id='ch_foo')
        charge.metadata = {}
        charge.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_foo', {'metadata': {}}, None)

    def test_whole_update(self):
        charge = payjp.Charge(id='ch_foo')
        charge.metadata = {'whole': 'update'}
        charge.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_foo', {'metadata': {'whole': 'update'}}, None)

    def test_individual_delete(self):
        charge = payjp.Charge(id='ch_foo')
        charge.metadata = {'whole': None}
        charge.save()
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_foo', {'metadata': {'whole': None}}, None)
