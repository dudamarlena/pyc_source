...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/payjp/resource.py
# Compiled at: 2018-06-26 22:27:41
# Size of source mod 2**32: 13721 bytes
...
class PayjpObject(dict):
    ...
    def __setitem__(self, k, v):
        if v == '':
            raise ValueError('You cannot set %s to an empty string. We interpret empty strings as None in requests.You may set %s.%s = None to delete the property' % (
             k, str(self), k))
    ...
    def __getitem__(self, k):
        try:
            ...
            if k in self._transient_values:
                raise KeyError("%r.  HINT: The %r attribute was set in the past.It was then wiped when refreshing the object with the result returned by PAY.JP's API, probably as a result of a save().  The attributes currently available on this object are: %s" % (
                 k, k, ', '.join(self.keys())))
            else:
                raise err

    def __delitem__(self, k):
        raise TypeError('You cannot delete attributes on a PayjpObject. To unset a property, set it to None.')

    @classmethod
    def construct_from(cls, values, key, payjp_account=None, api_base=None):
        instance = cls((values.get('id')), api_key=key, payjp_account=payjp_account)
        instance.refresh_from(values, api_key=key, payjp_account=payjp_account,
          api_base=api_base)
        return instance

    def refresh_from(self, values, api_key=None, partial=False, payjp_account=None, api_base=None):
        self.api_key = api_key or getattr(values, 'api_key', None)
        self.payjp_account = payjp_account or getattr(values, 'payjp_account', None)
        ...
        for k, v in values.items():
            super(PayjpObject, self).__setitem__(k, convert_to_payjp_object(v, api_key, payjp_account, api_base))
    ...
    def serialize(self, previous):
        ...
        for k, v in self.items():
            if k == 'id' or isinstance(k, str) and k.startswith('_'):
                ...
                if k == 'additional_owners' and v is not None:
        ...
        if hasattr(self, 'object'):
            if self.object == 'list':
                if hasattr(self, 'count'):
                    if self.count > 0:
                        if isinstance(self.data[0], Subscription):
                            raise NotImplementedError("Can't create a subscription via customer object. Use payjp.Subscription.create({'customer_id'}) instead.")
...
class APIResource(PayjpObject):
    ...
    def instance_url(self):
        id = self.get('id')
        if not id:
            raise error.InvalidRequestError("Could not create instance url without it's id", None)
        base = self.class_url()
        ext = quote_plus(id)
...
class UpdateableAPIResource(APIResource):
    ...
    def save(self):
        ...
        if updated_params:
            self.refresh_from(self.request('post', self.instance_url(), updated_params))
        else:
...
class Card(UpdateableAPIResource, DeletableAPIResource):
    ...
    def instance_url(self):
        ...
        if hasattr(self, 'customer'):
            ...
            raise error.InvalidRequestError('Could not determine whether card_id %s is attached to a customer or a recipient.' % self.id, 'id')
        return '%s/%s/cards/%s' % (base, owner_extn, extn)

    @classmethod
    def retrieve(cls, id, api_key=None, payjp_account=None, api_base=None, **params):
        raise NotImplementedError("Can't retrieve a card without a customer ID.Use customer.cards.retrieve('card_id') or recipient.cards.retrieve('card_id') instead.")


class Subscription(CreateableAPIResource, DeletableAPIResource, UpdateableAPIResource, ListableAPIResource):
