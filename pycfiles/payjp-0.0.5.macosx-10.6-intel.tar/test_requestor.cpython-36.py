...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/3.6/lib/python3.6/site-packages/payjp/test/test_requestor.py
# Compiled at: 2018-06-22 02:45:13
# Size of source mod 2**32: 12210 bytes
import base64, datetime, unittest
...
class APIHeaderMatcher(object):
    EXP_KEYS = [
     'X-Payjp-Client-User-Agent', 'User-Agent', 'Authorization']
    ...
    def _encode(self, api_key):
        if PY3:
            return str(base64.b64encode(bytes(''.join([api_key, ':']), 'utf-8')), 'utf-8')
        else:
    ...
    def _keys_match(self, other):
        ...
        if self.request_method is not None:
            if self.request_method in self.METHOD_EXTRA_KEYS:
...
class APIRequestorRequestTests(PayjpUnitTestCase):
    ENCODE_INPUTS = {'dict':{'astring':'bar', 
      'anint':5, 
      'anull':None, 
    ...
    def setUp(self):
        ...
        self.requestor = payjp.api_requestor.APIRequestor(client=(self.http_client))

    def mock_response(self, return_body, return_code, requestor=None):
        if not requestor:
            requestor = self.requestor
        self.http_client.request = Mock(return_value=(
         return_body, return_code))
    ...
    def check_call(self, meth, abs_url=None, headers=None, post_data=None, requestor=None):
        if not abs_url:
            ...
            if not requestor:
                requestor = self.requestor
            headers = headers or APIHeaderMatcher(request_method=meth)
    ...
    def _test_encode_naive_datetime(self):
        stk = []
        payjp.api_requestor.APIRequestor.encode_datetime(stk, 'test', datetime.datetime(2013, 1, 1))
        self.assertTrue(86400 > abs(stk[0][1] - 1356994800))
    ...
    def test_methods_with_params_and_response(self):
        for meth in VALID_API_METHODS:
            ...
            params = {'alist':[
             ...
             'adict':{'frobble': 'bits'}, 
             'adatetime':datetime.datetime(2013, 1, 1, tzinfo=GMT1())}
            encoded = 'adict%5Bfrobble%5D=bits&adatetime=1356994800&alist%5B%5D=1&alist%5B%5D=2&alist%5B%5D=3'
            body, key = self.requestor.request(meth, self.valid_path, params)
    ...
    def test_uses_instance_key(self):
        key = 'fookey'
        requestor = payjp.api_requestor.APIRequestor(key, client=(self.http_client))
    ...
    def test_passes_api_version(self):
        ...
        self.check_call('get', headers=APIHeaderMatcher(extra={'Payjp-Version': 'fooversion'},
          request_method='get'))

    def test_uses_instance_account(self):
        account = 'acct_foo'
        requestor = payjp.api_requestor.APIRequestor(account=account, client=(self.http_client))
        self.mock_response('{}', 200, requestor=requestor)
        requestor.request('get', self.valid_path, {})
        self.check_call('get',
          requestor=requestor,
          headers=APIHeaderMatcher(extra={'Payjp-Account': account},
          request_method='get'))

    def test_fails_without_api_key(self):
        payjp.api_key = None
        self.assertRaises(payjp.error.AuthenticationError, self.requestor.request, 'get', self.valid_path, {})

    def test_not_found(self):
        self.mock_response('{"error": {}}', 404)
        self.assertRaises(payjp.error.InvalidRequestError, self.requestor.request, 'get', self.valid_path, {})
    ...
    def test_card_error(self):
        ...
        self.assertRaises(payjp.error.CardError, self.requestor.request, 'get', self.valid_path, {})

    def test_server_error(self):
        self.mock_response('{"error": {}}', 500)
        self.assertRaises(payjp.error.APIError, self.requestor.request, 'get', self.valid_path, {})
    ...
    def test_invalid_method(self):
        self.assertRaises(payjp.error.APIConnectionError, self.requestor.request, 'foo', 'bar')
