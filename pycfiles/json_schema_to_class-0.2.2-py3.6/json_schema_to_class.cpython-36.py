...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/json_schema_to_class.py
# Compiled at: 2019-12-17 09:43:16
...
def indent_line(line: str, level: int):
    if line:
        return spaces(level=level) + line
    else:
        return ''
...
class Definition(Item):
    ...
    def __init__(self, name, class_type, path):
        super().__init__(name=name)
        self.path = path
...
class Array(Model):
    ...
    def __init__(self, name, items=None, default=None):
        super().__init__(name=name)
        self.items = items
    ...
    def to_init_code(self) -> str:
        if not self.is_inner_model():
            Array.use_list = True
            return '{spaces}self.{name}: {type_name} = values.get("{name}", {default})'.format(spaces=(spaces(2)),
              name=(self.name),
...
def cli():
    init_code_lines = ('from pathlib import Path', '', 'import json_schema_to_class',
                       '', 'current_dir: Path = Path(__file__).parent', 'json_schema_to_class.generate_dir(',
                       "    schema_dir=current_dir.parent / 'schema',", "    output_dir=current_dir / 'build'",
                       ')', '', "if __name__ != '__main__':", '    from .build import *  # noqa: F403',
                       '', '    del json_schema_to_class', '    del current_dir',
                       '    del Path')
    arg_parser = argparse.ArgumentParser(description='JSON Schema to Python Class')
    ...
    if command == 'print':
        ...
        if command == 'init':
            ...
            print('Please run [find . -path "*/configs/__init__.py" -exec python3 {} \\;]')
