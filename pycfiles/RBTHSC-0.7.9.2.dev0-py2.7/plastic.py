...
class PlasticClient(SCMClient):
    ...
    def parse_revision_spec(self, revisions=[]):
        ...
        if n_revisions == 0:
            raise InvalidRevisionSpecError('Either a changeset or a branch must be specified')
        else:
            if n_revisions == 1:
                return {'base': None, 
                   'tip': revisions[0]}
    ...
    def diff(self, revisions, include_files=[], exclude_patterns=[], extra_args=[]):
        ...
        diff_entries = execute([
         'cm', 'diff', tip, '--format={status} {path} rev:revid:{revid} rev:revid:{parentrevid} src:{srccmpath} dst:{dstcmpath}{newline}'], split_lines=True)
    ...
    def _process_diffs(self, my_diff_entries):
        ...
        for f in my_diff_entries:
            ...
            if changetype == 'M':
                ...
                dl = self._diff_files(tmp_diff_from_filename, empty_filename, oldfilename, 'rev:revid:-1', oldspec, changetype)
                diff_lines += dl
                self._write_file(newfilename, newspec, tmp_diff_to_filename)
                dl = self._diff_files(empty_filename, tmp_diff_to_filename, newfilename, newspec, 'rev:revid:-1', changetype)
                diff_lines += dl
            else:
