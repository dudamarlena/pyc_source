...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\utils\process_orig.py
...
def execute(command, env=None, split_lines=False, ignore_errors=False, extra_ignore_errors=(), translate_newlines=True, with_errors=True, none_on_ignored_error=False, return_error_code=False, log_output_on_error=True, results_unicode=True, return_errors=False):
...
``.
    ...
    def post_process_output(output):
        ...
        if split_lines and len(output) > 0:
            ...
            if not results_unicode and isinstance(output[0], six.text_type):
                return [ line.encode(b'utf-8') for line in output ]
        elif not split_lines:
            if results_unicode and isinstance(output, six.binary_type):
                return output.decode(encoding)
            if not results_unicode and isinstance(output, six.text_type):
                return output.encode(b'utf-8')
        return output
    ...
    if sys.platform.startswith(b'win'):
        new_env = dict((six.binary_type(key), six.binary_type(value)) for key, value in six.iteritems(new_env))
        p = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=errors_output, shell=False, universal_newlines=translate_newlines, env=new_env)
    else:
        p = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=errors_output, shell=False, close_fds=True, universal_newlines=translate_newlines, env=new_env)
    errors = None
    if split_lines:
    ...
    if rc and not ignore_errors and rc not in extra_ignore_errors:
        die(b'Failed to execute command: %s\n%s' % (command, data))
    elif rc:
