...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\commands\post.py
# Compiled at: 2017-04-19 05:14:04
from __future__ import print_function, unicode_literals
import logging, os, re, sys
from rbtools.api.errors import APIError
from rbtools.commands import Command, CommandError, Option, OptionGroup
from rbtools.utils.commands import AlreadyStampedError, get_review_request, stamp_commit_with_review_url
...
class Post(Command):
    ...
    option_list = [
     OptionGroup(name=b'Posting Options', description=b'Controls the behavior of a post, including what review request gets posted and how, and what happens after it is posted.', option_list=[
      ...
      Option(b'--diff-only', dest=b'diff_only', action=b'store_true', default=False, help=b'Uploads a new diff, but does not update fields from the change description (Perforce/Plastic only).')]),
     ...
     OptionGroup(name=b'Review Request Field Options', description=b'Options for setting the contents of fields in the review request.', option_list=[
      ...
      Option(b'--change-description', default=None, metavar=b'TEXT', help=b'A description of what changed in this update of the review request. This is ignored for new review requests.'),
      ...
      Option(b'--target-groups', dest=b'target_groups', config_key=b'TARGET_GROUPS', metavar=b'NAME[,...]', default=None, help=b'The names of the groups that should perform the review.'),
      Option(b'--target-people', dest=b'target_people', metavar=b'USERNAME[,...]', config_key=b'TARGET_PEOPLE', default=None, help=b'The usernames of the people who should perform the review.'),
      ...
      Option(b'--markdown', dest=b'markdown', action=b'store_true', config_key=b'MARKDOWN', default=False, help=b'Specifies if the summary and description should be interpreted as Markdown-formatted text.\nThis is only supported in Review Board 2.0+.', added_in=b'0.6')]),
    ...
    def post_request(self, repository_info, repository, server_url, api_root, review_request_id=None, changenum=None, diff_content=None, parent_diff_content=None, commit_id=None, base_commit_id=None, submit_as=None, retries=3, base_dir=None):
        ...
        if review_request_id:
            review_request = get_review_request(review_request_id, api_root, only_fields=b'absolute_url,bugs_closed,id,status', only_links=b'diffs,draft')
            ...
            if not repository_info.supports_changesets or not self.options.change_only:
                try:
                    ...
                    if e.error_code == 101 and e.http_status == 403:
                        ...
                        error_msg.append(b'The generated diff file was empty. This usually means no files were\nmodified in this change.\n')
                    else:
                        error_msg.append(str(e).decode(b'utf-8') + b'\n')
                    error_msg.append(b'Your review request still exists, but the diff is not attached.\n')
                    error_msg.append(b'%s\n' % review_request.absolute_url)
    ...
    def main(self, *args):
        ...
        if self.options.update and self.revisions:
            review_request = guess_existing_review_request(repository_info, self.options.repository_name, api_root, api_client, self.tool, self.revisions, guess_summary=False, guess_description=False, is_fuzzy_match_func=self._ask_review_request_match)
        ...
        if self.options.open_browser:
            try:
                import webbrowser
                if b'open_new_tab' in dir(webbrowser):
                    webbrowser.open_new_tab(review_url)
                elif b'open_new' in dir(webbrowser):
                    webbrowser.open_new(review_url)
                else:
                    os.system(b'start %s' % review_url)
            except:
                logging.error(b'Error opening review URL: %s' % review_url)
