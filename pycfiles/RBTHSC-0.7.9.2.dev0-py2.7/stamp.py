...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\commands\stamp.py
# Compiled at: 2017-04-19 05:14:04
import logging
from rbtools.commands import Command, CommandError, Option, OptionGroup
from rbtools.utils.commands import get_review_request, stamp_commit_with_review_url
...
class Stamp(Command):
    ...
    args = '[revisions]'
    option_list = [
     OptionGroup(name='Stamp Options', description='Controls the behavior of a stamp, including what review request URL gets stamped.', option_list=[
      Option('-r', '--review-request-id', dest='rid', metavar='ID', default=None, help='Specifies the existing review request ID to be stamped.')]),
    ...
    def determine_review_request(self, api_client, api_root, repository_info, repository_name, revisions):
        ...
        logging.debug('Attempting to guess review request based on summary and description')
    ...
    def main(self, *args):
        ...
        if not self.tool.can_amend_commit:
            raise NotImplementedError('rbt stamp is not supported with %s.' % self.tool.name)
        try:
        ...
        if not review_request_url:
            raise CommandError('Could not determine the existing review request URL to stamp with.')
