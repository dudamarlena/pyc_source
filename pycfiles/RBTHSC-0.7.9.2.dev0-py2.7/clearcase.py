...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\rbtools\clients\clearcase.py
# Compiled at: 2017-04-19 05:14:02
import datetime, itertools, logging, os, six, sys, threading
...
class ClearCaseClient(SCMClient):
    ...
    def get_repository_info(self):
        ...
        if not check_install(['cleartool', 'help']):
            logging.debug('Unable to execute "cleartool help": skipping ClearCase')
            return None
        else:
            ...
            property_lines = execute([
             'cleartool', 'lsview', '-full', '-properties', '-cview'], split_lines=True)
            for line in property_lines:
                properties = line.split(' ')
                if properties[0] == 'Properties:':
                    if 'webview' in properties:
                        die('Webviews are not supported. You can use rbt commands only in dynamic or snapshot views.')
            ...
            vobstag = execute(['cleartool', 'describe', '-short', 'vob:.'], ignore_errors=True).strip()
            if 'Error: ' in vobstag:
                die('To generate diff run rbt inside vob.')
            root_path = execute(['cleartool', 'pwv', '-root'], ignore_errors=True).strip()
    ...
    def _is_a_label(self, label, vobstag=None):
        ...
        if vobstag and label_vobstag and label_vobstag != vobstag:
            raise Exception('label vobstag %s does not match expected vobstag %s' % (
             label_vobstag, vobstag))
        output = execute(['cleartool', 'describe', '-short', label], extra_ignore_errors=(1, ), with_errors=False)
        return bool(output)
    ...
    def _set_label(self, label, path):
        ...
        if checkedout_elements:
            raise Exception('ClearCase backend cannot set label when some elements are checked out:\n%s' % ('').join(checkedout_elements))
        execute(['cleartool', 'mklbtype', '-c', 'label created for rbtools',
         label], with_errors=True)
        recursive_option = ''
        if cpath.isdir(path):
            recursive_option = '-recurse'
        execute(['cleartool', 'mklabel', '-nc', recursive_option, label, path], extra_ignore_errors=(1, ), with_errors=False)
    ...
    def _remove_label(self, label):
        ...
        if not label.startswith(self.REVISION_LABEL_PREFIX):
            label = '%s%s' % (self.REVISION_LABEL_PREFIX, label)
        execute(['cleartool', 'rmtype', '-rmall', '-force', label], with_errors=True)
    ...
    def _sanitize_activity_changeset(self, changeset):
        ...
        for change in changeset:
            ...
            if path.find('%s/' % (repository_info.vobstag,)) == -1:
                logging.debug('Vobstag does not match, so ignore changes on %s' % path)
                continue
            version_number = self._determine_version(current)
            if path not in changelist:
                changelist[path] = {'highest': version_number, 'lowest': version_number, 
                   'current': current}
            if version_number == 0:
        ...
        for path, version in six.iteritems(changelist):
            ...
            version['previous'] = self._construct_revision(branch_path, prev_version_number)
            changeranges.append((
    ...
    def _sanitize_branch_changeset(self, changeset):
        ...
        for path, previous, current in changeset:
            version_number = self._determine_version(current)
            if path not in changelist:
                changelist[path] = {'highest': version_number, 'current': current, 
                   'previous': previous}
            if version_number == 0:
    ...
    def _sanitize_version_0_file(self, file_revision):
        ...
        if file_revision.endswith('/0'):
            ...
            file_revision = execute(['cleartool',
             ...
             '-fmt', '%En@@%PSn',
             file_revision])
            logging.debug('Sanitized with predecessor, new file: %s', file_revision)
        return file_revision
    ...
    def _construct_changeset(self, output):
        return [ info.split('\t') for info in output.strip().split('\n')
               ]
    ...
    def _get_label_changeset(self, labels):
        ...
        try:
            try:
                ...
                if label_count != 2:
                    raise Exception('ClearCase label comparison does not support %d labels' % label_count)
                repository_info = self.get_repository_info()
                for label in labels:
                    if not self._is_a_label(label, repository_info.vobstag):
                        raise Exception('ClearCase label %s is not a valid label' % label)
                ...
                previous_elements = {}
                current_elements = {}
                previous_label_elements_thread = get_elements_from_label_thread(1, comparison_path, previous_label, previous_elements)
                previous_label_elements_thread.start()
                current_label_elements_thread = get_elements_from_label_thread(2, comparison_path, current_label, current_elements)
                ...
                for path in itertools.chain(previous_elements.keys(), current_elements.keys()):
                    if path in seen:
                        continue
                    seen.append(path)
                    changelist[path] = {'previous': '/main/0', 
                       'current': '/main/0'}
                    if path in current_elements:
                        changelist[path]['current'] = current_elements[path]['version']
                    if path in previous_elements:
                        changelist[path]['previous'] = previous_elements[path]['version']
                    ...
                    if changelist[path]['current'] == changelist[path]['previous']:
                        continue
                    changeset.append((
                     self._construct_extended_path(path, changelist[path]['previous']),
                     self._construct_extended_path(path, changelist[path]['current'])))
    ...
    def diff(self, revisions, include_files=[], exclude_patterns=[], extra_args=[]):
        if include_files:
            raise Exception('The ClearCase backend does not currently support the -I/--include parameter. To diff for specific files, pass in file@revision1:file@revision2 pairs as arguments')
        ...
        if revisions['base'] == self.REVISION_ACTIVITY_BASE:
            changeset = self._get_activity_changeset(revisions['tip'])
            return self._do_diff(changeset)
        if revisions['base'] == self.REVISION_BRANCH_BASE:
            changeset = self._get_branch_changeset(revisions['tip'])
            return self._do_diff(changeset)
        if revisions['base'] == self.REVISION_LABEL_BASE:
            changeset = self._get_label_changeset(revisions['tip'])
            return self._do_diff(changeset)
        if revisions['base'] == self.REVISION_FILES:
    ...
    def _diff_files(self, old_file, new_file):
        ...
        if len(dl) == 1 and dl[0].startswith('Files %s and %s differ' % (old_file, new_file)):
            dl = [
        ...
        if dl == [] or dl[0].startswith('Binary files '):
            if dl == []:
                dl = [
                 'File %s in your changeset is unmodified\n' % new_file]
            dl.insert(0, '==== %s %s ====\n' % (old_oid, new_oid))
            dl.append('\n')
        else:
            dl.insert(2, '==== %s %s ====\n' % (old_oid, new_oid))
    ...
    def _diff_directories(self, old_dir, new_dir):
        ...
        diff_cmd = [
         'diff', '-uN', old_tmp, new_tmp]
        dl = execute(diff_cmd, extra_ignore_errors=(1, 2), translate_newlines=False, split_lines=True)
        if dl:
...
class ClearCaseRepositoryInfo(RepositoryInfo):
    ...
    def __init__(self, path, base_path, vobstag, supports_parent_diffs=False):
        RepositoryInfo.__init__(self, path, base_path, supports_parent_diffs=supports_parent_diffs)
    ...
    def find_server_repository_info(self, server):
        ...
        for repository in repository_scan_order:
            ...
            try:
                ...
                if not (e.error_code == 101 and e.http_status == 403):
                    if repo_name == self.vobstag:
                        die('You do not have permission to access this repository.')
                    continue
                else:
                    raise e
            ...
            if not info or uuid != info['uuid']:
                continue
            path = info['repopath']
            logging.debug('Matching repository uuid:%s with path:%s', uuid, path)
    ...
    def _get_vobs_uuid(self, vobstag):
        ...
        property_lines = execute(['cleartool', 'lsvob', '-long', vobstag], split_lines=True)
        for line in property_lines:
