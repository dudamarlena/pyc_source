...
# [GCC 8.4.0]
# Embedded file name: /Users/amarin/dev/sphinx-mediatel/src/sphinx_paw/__init__.py
...
def setup(app):
    ...
    logger.debug('Load configuration from setup.cfg')
    ...
    metadata.configure('copyright', '{year}, {author}'.format(year=(datetime.now().year),
      author=(metadata.author)))
    lc_ctype = getenv('LC_CTYPE', 'en_US.UTF8')
    if '.' in lc_ctype:
        language, encoding = lc_ctype.split('.')
    else:
        language = 'en_US'
    logger.info(f"Init language f{language}")
    set_config_value(app, 'language', language)
    logger.debug('Attach localization strings')
