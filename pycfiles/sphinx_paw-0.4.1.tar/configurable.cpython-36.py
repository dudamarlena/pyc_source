...
# [GCC 8.4.0]
# Embedded file name: /Users/amarin/dev/sphinx-mediatel/src/sphinx_paw/configurable.py
...
class NoSuchSection(Exception):
    __doc__ = 'Wrap NoSectionError'


class ConfigFileSection(object):
    __doc__ = 'Wrap configuration file section'
...
class ConfigFile(object):
    __doc__ = 'Wrapper to get additional options from ini file'

    def __init__(self, app, file_path):
        if not path.exists(file_path):
            raise AssertionError('No such file %s' % file_path)
        elif not isinstance(app, Sphinx):
            raise AssertionError
        self.sphinx = app
    ...
    def get(self, section, key, default=NOTSET):
        ...
        try:
            ...
            if default != NOTSET:
                return default
            raise RequireConfigSection('Section %s is not defined and no default value for %s' % (
             section, key))
        except NoOptionError:
            if default != NOTSET:
                return default
            raise RequireConfigOption('Missed required option [%s]%s, default not set' % (
             section, key))

    def assert_has_section(self, section_name):
        """Ensure config has section"""
        if not self._config.has_section(section_name):
            raise RequireConfigSection('Required configuration section {section_name}'.format(section_name=section_name))
