...
# [GCC 8.4.0]
# Embedded file name: /home/argos/Workspace/mamba-framework/mamba-server/mamba_server/commands/serve.py
# Compiled at: 2020-05-13 09:17:32
            ...
            if opts.dump:
                ...
                if template_file:
                    with open(template_file, 'r') as (f):
                        print(f.read())
                return
            if opts.run and _find_launch_file(opts.run, mamba_dir) is not None:
                launch_file = join(mamba_dir, LAUNCH_FILES_DIR, '{}.launch.json'.format(opts.run))
                execute(launch_file, mamba_dir)


def _find_launch_file(launch_file, mamba_dir):
    launch_file = join(mamba_dir, LAUNCH_FILES_DIR, '{}.launch.json'.format(launch_file))
