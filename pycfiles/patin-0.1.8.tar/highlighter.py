...
# [GCC 8.4.0]
# Embedded file name: /home/hugoruscitti/proyectos/pilas/pilas/console/highlighter.py
# Compiled at: 2011-10-23 18:22:04
...
COLOR_SCHEME = {'keyword': 'darkMagenta', 
   'operator': 'darkRed', 
   'brace': '#858585', 
   'definition': 'black', 
   'string': 'green', 
...
def load_syntax():
    syntax_file = os.path.join(os.path.dirname(__file__), 'lang', 'python.json')
    structure = None
...
STYLES = {'keyword': format(COLOR_SCHEME['keyword'], 'bold'), 
...
class Highlighter(QSyntaxHighlighter):
    ...
    def apply_highlight(self, lang, scheme=None):
        ...
        rules = []
        rules += [ ('\\b%s\\b' % w, 0, STYLES['keyword']) for w in keywords
                 ]
        rules += [ ('%s' % o, 0, STYLES['operator']) for o in operators
                 ]
        rules += [ ('%s' % b, 0, STYLES['brace']) for b in Highlighter.braces
                 ]
        rules += [ ('\\b%s\\b' % e, 0, STYLES['extras']) for e in extras
                 ]
        proper = langSyntax.get('properObject', None)
        if proper is not None:
            proper = '\\b' + str(proper[0]) + '\\b'
            rules += [(proper, 0, STYLES['properObject'])]
        rules.append(('__\\w+__', 0, STYLES['properObject']))
        ...
        rules += [
         (
          '\\b[+-]?[0-9]+[lL]?\\b', 0, STYLES['numbers']),
         (
          '\\b[+-]?0[xX][0-9A-Fa-f]+[lL]?\\b', 0, STYLES['numbers']),
         (
        ...
        self.tri_single = (
         QRegExp("'''"), 1, STYLES['string2'])
        self.tri_double = (QRegExp('"""'), 2, STYLES['string2'])
        multi = langSyntax.get('multiline_comment', [])
        if multi:
            self.multi_start = (
             QRegExp(multi['open']), STYLES['comment'])
