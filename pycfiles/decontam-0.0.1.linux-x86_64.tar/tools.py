...
# [GCC 8.4.0]
# Embedded file name: /home/lkalan/my_python-2.7.5/lib/python2.7/site-packages/decontamlib/tools.py
# Compiled at: 2016-01-13 15:33:38
import collections, csv, inspect, os, random, re, shutil, subprocess, tempfile, utils
...
class Bwa(_FilteringTool):
    ...
    def _command(self, fwd_fp, rev_fp):
        return [
         self.bwa_fp, 'mem', '-M', '-t', str(self.num_threads), self.index, fwd_fp, rev_fp]
...
class Random_human(_FilteringTool):
    ...
    def annotate(self, R1, R2, pct, frac):
        ids = utils.parse_read_ids(R1)
        return [ (id, True if random.random() <= self.fraction_human else False) for id in ids
               ]
