...
# [GCC 8.4.0]
# Embedded file name: /Users/robin/DO/GitHub/pytunnel/pytunnel/tunnel.py
# Compiled at: 2019-10-02 12:25:36
        ...
        with self._recv_lock:
            while True:
                ...
                if data == None:
                    ...
                    if not r:
                        raise Exception('Socket Error:%s' % str(self.sock))
                    self._pi.feed(r)
                else:
                    break
...
class Tunnel(SockRunable):
    ...
    def _run_con(self, sock, ix):
        ...
        while self._running:
            ...
            if recv:
                ...
                send_package(self.sock, -1 * ix, 'close')
                sock_close(sock)
                self._del_con(ix)
                break
    ...
    def _add_con(self, sock, addr):
        with self._lock:
            ...
            self._client_map[self._client_ix] = {'th': th, 
               'sock': sock}
            return th
...
class Client(SockRunable):
    ...
    def _run_con(self, ix, sock):
        while self._running:
            ...
            if len(recv):
                ...
                send_package(self._sock, -1 * ix, 'close')
    ...
    def _run_ping(self):
        while self._running:
            send_package(self._sock, 0, 'ping')
            time.sleep(TIME_FOR_PING_S)
    ...
    def _run(self):
        try:
            ...
            if ret['status'] == 'ok':
                ...
                self._log('\x1b[31m%s: %s\x1b[0m' % (ret['status'], ret['message']))
            ...
            if recv:
                ...
                if ix > 0:
                    ...
                    if d:
                        ...
                        send_package(sock, -1 * ix, '')
                elif ix == 0:
                    if data == 'ping':
                        send_package(sock, 0, 'pong')
                    elif data == 'pong':
                        pass
                    else:
                        self._log('[Server]\x1b[31m%s\x1b[0m' % data.decode())
                        self.stop()
                else:
                    nix = abs(ix)
                    if nix in self._client_map:
                        if data == 'ping':
                            send_package(sock, -1 * ix, 'pong')
...
def parse_endpoint(s):
    ...
    raise Exception('%s not a endpoint type!' % s)
