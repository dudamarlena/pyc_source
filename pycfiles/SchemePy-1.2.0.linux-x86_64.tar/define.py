...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python2.7/site-packages/scheme/define.py
# Compiled at: 2015-09-06 15:52:06
        ...
        if not isinstance(params[0], list):
            ...
            if isinstance(name, list):
                ...
                if not name.isBound(env):
                    raise NameError('Name %s unbound in enclosing namespaces' % name)
                retval = [
                 Symbol('define'), o[(-1)], [Symbol('lambda'), args, [Symbol('begin')] + rest]]
                processer.process(retval, env)
            else:
                if not name.isBound(env):
                    raise NameError('Name %s unbound in enclosing namespaces' % name)
