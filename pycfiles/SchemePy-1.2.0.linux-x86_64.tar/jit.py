...
# [GCC 8.4.0]
# Embedded file name: /usr/lib64/python2.7/site-packages/scheme/jit.py
# Compiled at: 2015-09-07 21:32:16
enabled = False
...
import scheme.debug, operator as op, warnings
...
compare_ops = (
 op.lt, op.le, op.eq, op.ne, op.gt, op.ge, op.contains, object(), op.is_, object(), object(), object())
labelidx = count()
...
def get_locals(c, s=None, e=None):
    ...
    if c:
        ...
        if isinstance(f, scheme.define.define):
            ...
            return s
    for i in c:
        if isinstance(i, list) and get_locals(i, s, e) is None:
            return
            continue
        ...
        def write_nested_code(statement):
            if not isinstance(statement, list):
                ...
                if func in compare_ops:
                    ...
                    bytecode.append(label('false%i' % lidx))
                    return True
                if isinstance(func, (BuiltinFunctionType, function, type)):
                    isp = stackptr.ptr
                    istatement = iter(statement[1:])
                    ...
                    return True
                if isinstance(func, scheme.define.define):
                    var = statement[1]
                    ...
                    return True
                if isinstance(func, scheme.IF.IF):
                    lidx = labelidx.next()
                    ...
                    return True
                if isinstance(func, scheme.begin.begin):
                    c = iter(statement[1:])
                    isp = stackptr.ptr
                    for statement in c:
                        while stackptr > isp:
                ...
                if isinstance(func, Symbol):
                    ...
                    if isinstance(func, list):
                        ...
                        if isinstance(func, scheme.Lambda.Lambda):
