...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/kipp3/options/mlogger.py
...
class ConvertFailureLog:
    ...
    def format_failure_log(self, mls_sysid, mls_number, column_name, column_value, exc_info=None):
        ...
        Returns:
            str: message
        """
        s = 'mls_sysid: {mls_sysid}, mls_number: {mls_number}, column_name: {column_name}, column_value: {column_value}'.format(mls_sysid=mls_sysid,
    ...
    def record_failure_log(self, **kw):
        ...
        """
        if not self._failures_logger:
            raise AssertionError('You should invoke ``setup_failure_logger(stage, mls_id)`` first')
        elif kw.get('exc_info', None):
            self._failures_logger.error((self.format_failure_log)(**kw),
