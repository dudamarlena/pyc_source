...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/microformats2/schema.py
# Compiled at: 2018-03-21 14:55:07
import jsl, jsonschema
...
content_field = jsl.DictField(properties={'value': jsl.StringField(), 'html': jsl.StringField()})
content_array = jsl.ArrayField(jsl.OneOfField([jsl.StringField(), content_field]))
latitude = jsl.ArrayField(items=jsl.StringField(pattern='[-+]?\\d+(\\.\\d+)?'))
longitude = jsl.ArrayField(items=jsl.StringField(pattern='[-+]?\\d+(\\.\\d+)?'))
altitude = jsl.ArrayField(items=jsl.StringField(pattern='[-+]?\\d+(\\.\\d+)?'))
photo_field = jsl.ArrayField(jsl.OneOfField([
 jsl.StringField(format='uri'),
 jsl.DictField(properties={'value': jsl.StringField(format='uri'), 
    'alt': jsl.StringField()})]))
...
class hGeo(Microformat):
    type = type_of('h-geo')
    properties = jsl.DictField(required=True, properties={'latitude': latitude, 'longitude': longitude, 'altitude': altitude})


class hAdr(Microformat):
    type = type_of('h-adr')
    properties = jsl.DictField(required=True, properties={'street-address': string_array, 
...
class hCard(Microformat):
    type = type_of('h-card')
    properties = jsl.DictField(required=True, properties={'name': string_array, 
...
class hItem(Microformat):
    type = type_of('h-item')
    properties = jsl.DictField(required=True, properties={'name': string_array, 'url': uri_array, 'photo': photo_field})
...
class hProduct(Microformat):
    ...
    properties = jsl.DictField(required=True, properties={'name': string_array, 
       ...
       'url': uri_array, 
       'identifier': jsl.ArrayField(jsl.DictField(properties={'type': jsl.StringField(), 'value': jsl.StringField()})), 
...
class hEntry(Microformat):
    ...
    properties = jsl.DictField(required=True, properties={'name': string_array, 
       ...
       'rsvp': jsl.ArrayField(jsl.StringField(enum=[
              'yes', 'no', 'maybe', 'interested'], required=True), min_items=1, max_items=1), 
       'like-of': uri_array, 
       'repost-of': uri_array, 
...
class hRating(Microformat):
    ...
    properties = jsl.DictField(required=True, properties={'average': string_array, 
       'best': string_array, 
       'count': string_array, 
       'name': string_array})
