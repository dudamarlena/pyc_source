...
# Embedded file name: build\bdist.win-amd64\egg\accasim\base\resource_manager_class.py
# Compiled at: 2018-10-29 12:09:47
# Size of source mod 2**32: 16699 bytes
...
class Resources:
    ...
    def _define_group(self, _groups, name, group):
        ...
        elif not name not in _groups:
            raise AssertionError('Repreated name group: {}. Select another one.'.format(name))
        _groups[name] = group
    ...
    def allocate(self, node_name, **kwargs):
        ...
        if not self._resources:
            raise AssertionError('The resources must be setted before jobs allocation')
        elif not self._resources_status[node_name] == self.ON:
            raise AssertionError('The Node {} is {}, it is impossible to allocate any job')
        _capacity = self.SYSTEM_CAPACITY_NODES[node_name]
        _used_resources = self._resources[node_name]
        _done = []
        _full_usage = {}
        for res, v in kwargs.items():
            _rem_res = _capacity[res] - _used_resources[res]
            try:
    ...
    def release(self, node_name, **kwargs):
        ...
        """
        if not self._resources:
            raise AssertionError('The resources must be setted before release resources')
        elif not self._resources_status[node_name] == self.ON:
            raise AssertionError('The Node {} is {}.')
    ...
    def usage(self, type=None):
        ...
        if type == 'dict':
            return {res:self._total_resources[res] / value * 100 for res, value in self.SYSTEM_CAPACITY_TOTAL.items()}
