...
# [GCC 8.4.0]
# Embedded file name: \Anaconda3\Lib\site-packages\xing\xasession.py
# Compiled at: 2016-03-02 11:20:37
# Size of source mod 2**32: 8190 bytes
import win32com.client, time, pythoncom, os, configparser
...
class Session:
    __doc__ = '세션 관리를 XASession 확장 클래스\n\n        ::\n\n            session = Session()\n    '
    ...
    def account(self):
        ...
        for p in range(self.session.GetAccountListCount()):
            acc.append({'no': self.session.GetAccountList(p), 
    ...
    def heartbeat(self):
        ...
        ::
            ...
            session.heartbeat()
        """
        result = Query('t0167').request(input=None, output={'OutBlock': ('dt', 'time')})
        if result:
            return {'time': result['OutBlock']['time'][:6], 
