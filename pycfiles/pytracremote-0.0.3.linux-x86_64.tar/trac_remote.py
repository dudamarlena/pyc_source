...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/pytracremote/trac_remote.py
...
class TracRemote(object):
    ...
    def call_command(self, *args):
        ...
        proc = subprocess.Popen(full_args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    ...
    def add_trac_user(self, trac_name, username, password=None):
        ...
        if password:
            ...
            password = re.match('.*password=(?P<password>\\w+) .*', output)
        if not password:
