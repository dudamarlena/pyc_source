...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/launcher/core/launcher.py
...
class ArtellaLauncher(window.ArtellaWindow, object):
    ...
    def __init__(self, project, install_path, paths_to_register=None, tag=None, dev=False):
        ...
        config = tpDcc.ConfigsMgr().get_config(config_name='artellapipe-launcher',
        ...
        if not self._plugins:
            ...
            for p in self._plugins:
                ...
                try:
                    ...
                    try:
                        plugin_mod = importlib.import_module('{}.launcher.plugins.{}'.format(self._project.get_clean_name(), p))
                    except ImportError:
                        try:
                            plugin_mod = importlib.import_module('artella.launcher.plugins.{}'.format(self._project.get_clean_name(), p))
                        except ImportError:
...
def run(project, install_path, paths_to_register=None, tag=None, dev=False):
    win = ArtellaLauncher(project=project, install_path=install_path,
