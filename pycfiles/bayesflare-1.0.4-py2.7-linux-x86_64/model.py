...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bayesflare/models/model.py
...
class Model:
    ...
    def filter_model(self, m, filtermethod='savitzkygolay', nbins=101, order=3, filterknee=1.0 / 25920.0):
        """
        ...
        if filtermethod == 'runningmedian':
            return m - bf.running_median(m, nbins)
        if filtermethod == 'highpass':
            ml = bf.Lightcurve()
            ml.clc = np.copy(m)
    ...
    def __call__(self, q, ts=None, filt=False, filtermethod='savitzkygolay', nbins=101, order=3, filterknee=1.0 / 25920.0):
        if ts == None:
            ts = self.ts
...
class Flare(Model):
    ...
    def __init__(self, ts, paramranges=None, amp=1, t0=None, reverse=False):
        Model.__init__(self, ts, mtype='flare', amp=amp, t0=t0, reverse=reverse, paramnames=[
         't0', 'tauexp', 'taugauss', 'amp'], paramranges=paramranges)
...
class Transit(Model):
    ...
    def __init__(self, ts, amp=1, t0=None, paramranges=None):
        Model.__init__(self, ts, mtype='transit', amp=amp, t0=t0, paramnames=[
         't0', 'sigmag', 'tauf', 'amp'], paramranges=paramranges)
    ...
    def prior(self, pdict, sigmacutoff=3.0):
        """
        ...
        if pdict['tauf'] + 2.0 * sigmagcutoff * pdict['sigmag'] > self.maxdur:
            return -np.inf
        t0prior = 0.0
        if len(t0range) > 1:
            t0prior = -np.log(t0range[(-1)] - t0range[0])
        ...
        maxsg = sigmagrange[(-1)]
        minsg = sigmagrange[0]
        sigmag1 = (self.maxdur - mintf) / (2.0 * sigmagcutoff)
        sigmag2 = (self.maxdur - maxtf) / (2.0 * sigmagcutoff)
        tauf1 = self.maxdur - 2.0 * sigmagcutoff * minsg
        tauf2 = self.maxdur - 2.0 * sigmagcutoff * maxsg
        dsigmag = maxsg - minsg
        dtauf = maxtf - mintf
        lntaufprior = 0.0
        if sigmag1 < minsg and sigmag2 > minsg and sigmag2 <= maxsg:
            lnsigmagprior = -np.log((sigmag2 - minsg) * (tauf1 - mintf) / 2.0)
        elif sigmag1 > minsg and sigmag1 <= maxsg and sigmag2 > minsg and sigmag2 <= maxsg:
            lnsigmagprior = -np.log(dtauf / 2.0 * (sigmag2 + sigmag1 - 2.0 * minsg))
        elif tauf1 > mintf and tauf1 < maxtf and tauf2 > mintf and tauf2 < maxtf:
            lnsigmagprior = -np.log(dsigmag / 2.0 * (tauf2 + tauf1 - 2.0 * mintf))
        elif sigmag1 > minsg and sigmag1 <= maxsg and sigmag2 > maxsg:
            lnsigmagprior = -np.log(dtauf * (sigmag1 - minsg) + (maxsg - sigmag1) * (dtauf + (tauf2 - mintf)) / 2.0)
        else:
...
class Expdecay(Model):
    ...
    def __init__(self, ts, amp=1, t0=None, reverse=False, paramranges=None):
        Model.__init__(self, ts, mtype='expdecay', amp=amp, t0=t0, reverse=reverse, paramnames=[
         't0', 'amp', 'tauexp'], paramranges=paramranges)
...
class Impulse(Model):
    ...
    def __init__(self, ts, amp=1, t0=None, paramranges=None):
        Model.__init__(self, ts, mtype='impulse', amp=amp, t0=t0, paramnames=[
         't0', 'amp'], paramranges=paramranges)
...
class Gaussian(Model):
    ...
    def __init__(self, ts, amp=1, t0=None, paramranges=None):
        Model.__init__(self, ts, mtype='gaussian', amp=amp, t0=t0, paramnames=[
         't0', 'sigma', 'amp'], paramranges=paramranges)
...
class Step(Model):
    ...
    def __init__(self, ts, amp=1, t0=None, reverse=False, paramranges=None):
        Model.__init__(self, ts, mtype='step', amp=amp, t0=t0, paramnames=[
         't0', 'amp'], paramranges=paramranges)
        self.modelname = 'step'
