...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bayesflare/data/data.py
# Compiled at: 2017-04-26 11:11:14
import pyfits, glob, numpy as np, bayesflare as bf, scipy.signal as signal, matplotlib.mlab as ml, matplotlib.pyplot as pl, copy
__all__ = [
 'Loader', 'Lightcurve']
...
class Lightcurve:
    ...
    def __init__(self, curve=None, detrend=False, detrendmethod='savitzkygolay', nbins=101, order=3, knee=1.0 / 25920.0, maxgap=1):
        clc = None
        clc = np.array([])
        if curve != None:
    ...
    def plot(self, figsize=(10, 3)):
        ...
        self.trace = ax.plot(self.cts / 86400.0, self.clc)
        fig.autofmt_xdate()
        pl.xlabel('Time [days]')
