...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bayesflare/noise/noise.py
...
def estimate_noise_ps(lightcurve, estfrac=0.5):
    ...
    sk, f = lightcurve.psd()
    sk = np.mean(sk[np.floor((1.0 - estfrac) * len(sk)):])
    sk = sk * lightcurve.fs() / 2.0
    noise_v = np.ones(nextpow2(2 * len(lightcurve.clc) - 1)) * sk
    return (
     np.sqrt(sk), sk, noise_v)
...
def estimate_noise_tv(d, sigma=1.0):
    ...
    bincentres = (bins[:-1] + bins[1:]) / 2.0
    cs = np.cumsum(n * (bins[1] - bins[0]))
    csu, idx = np.unique(cs, return_index=True)
    binsu = bincentres[idx]
    cp = erf(sigma / np.sqrt(2.0))
    interpf = interp1d(csu, binsu)
    lowS = interpf(0.5 - cp / 2.0)
    highS = interpf(0.5 + cp / 2.0)
    m = interpf(0.5)
    std = (highS - lowS) / (2.0 * sigma)
    return (
     std, m)
...
def highpass_filter_lightcurve(lightcurve, knee=1.0 / 25920.0):
    """
