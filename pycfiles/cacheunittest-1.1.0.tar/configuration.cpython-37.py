...
class Configuration(object):
    ...
    def __init__(self, config, endpoint_index: int, client: CachetClient, token: str):
        ...
        self.endpoint_timeout = self.endpoint.get('timeout') or 
        self.endpoint_header = self.endpoint.get('header') or 
        self.allowed_fails = self.endpoint.get('allowed_fails') or 
        ...
        self.latency_unit = self.data['cachet'].get('latency_unit') or 
    ...
    def push_incident(self):
        ...
        elif not hasattr(self, 'incident_id'):
            if self.status != st.ComponentStatus.OPERATIONAL:
