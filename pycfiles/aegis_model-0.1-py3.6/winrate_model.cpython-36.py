...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/aegis_model/model/winrate_model.py
# Compiled at: 2020-01-27 21:51:56
# Size of source mod 2**32: 5745 bytes
import datetime, keras, numpy as np
...
class WinrateModel(object):
    ...
    def _construct_model(self):
        model = Sequential()
        model.add(LSTM(units=(self.lstm1_node), batch_input_shape=(self.batch_size, self.time_window, self.y_shape[2])))
    ...
    def training(self, contd=False):
        ...
        for ep in range(self.epoch):
            ...
            if ep == 0:
                if avg_acc < self.init_threshold:
                    ...
                    if avg_acc > self.acc_threshold:
                        print('Activity prediction finished! Final average loss - %.4f acc - %.4f%%' % (
                         avg_loss, avg_acc))
                        break
    ...
    def _train_each_epoch(self, ep):
        ...
        self.model.fit(x=(self.x_trn), y=(self.y_trn), batch_size=(self.batch_size), epochs=1,
          shuffle=False,
          verbose=1)
