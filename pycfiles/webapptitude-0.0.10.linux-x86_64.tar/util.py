...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/util.py
# Compiled at: 2016-08-31 16:32:16
from datetime import datetime, timedelta, tzinfo
import time, random, hashlib, os, re
RE_NUMERIC_INT = re.compile('^\\d+$')
RE_NUMERIC_FLOAT = re.compile('^\\d(\\.\\d+)?(E\\d+)?$')
...
def is_prod_server():
    ...
    if isinstance(service, basestring):
        return service.startswith('Google App Engine') or service.startswith('AppScaleServer')
    else:
        return False
...
RE_DOTSEP_SPLITTER = re.compile('\\.|\\[(\\d+)\\]')
...
def timecode_random_offset(ts):
    ...
    intpart = int(intpart)
    radpart = int(radpart * 10000000.0) + 1
