...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/billing.py
...
"""
import stripe, hashlib, os, re, urlparse, logging
from google.appengine.ext import ndb
from google.appengine.api import lib_config
from httpclient import MemcacheProxy
_config = lib_config.register('billing', dict(stripe_secret_key='MISSING_SECRET', stripe_api_version='2016-03-07'))
...
def setupStripe(api_secret=None, api_version=None):
...
class URLFetchCache(stripe.http_client.UrlFetchClient):
    ...
    cacherules = [
     (
      '^/v1/customers/[a-z0-9_]+', 300),
     (
      '^/v1/subscriptions/[a-z0-9_]+', 300),
     (
      '^/v1/subscriptions$', 300),
     (
      '^/v1/plans/[a-z0-9_]+', 300),
     (
      '^/v1/plans$', 300)]

    def __init__(self, *args, **kwargs):
        """Prepare the HTTP client with cache configuration."""
        self.cache = MemcacheProxy(namespace='stripe#', lifetime=self.default_lifetime)
    ...
    def request(self, method, url, headers, post_data=None):
        ...
        if is_cacheable and lifetime > 0:
            ...
            cacheval = self.cache.get(cachekey)
            if cacheval is None:
                if self.debug:
                    logging.info('Cache key not found: %r' % cachekey)
                cacheval = _super.request(method, url, headers, post_data=post_data)
                self.cache.set(cachekey, cacheval, time=lifetime)
            return cacheval
        if self.debug and method.upper() in ('GET', 'HEAD'):
            info = (
             lifetime, method, url, post_data)
...
def getSubscriptionsByPlanId(planId, created=None):
    for s in stripe.Subscription.all(plan=planId).auto_paging_iter():
        yield (
         stripe.Customer.retrieve(s.customer), s)
...
class Plan(stripe.Plan):
    fixed_attrs = ('id', 'name', 'amount', 'interval', 'interval_count', 'currency',
                   'trial_period_days', 'statement_descriptor')
    ...
    def save(self, **props):
        ...
        if needs_recreate:
            ...
            stripe.Plan(old_id).delete()
            ret = Plan.augment(Plan.create(**props))
...
class CustomerModel(ndb.Model):
    ...
    def create(cls, name, email, phone, plan=None, **props):
        ...
        stripe_cust = None
        try:
            stripe_cust = stripe.Customer.create(email=email, plan=plan, source=payment_source, metadata={'phone_number': phone}, description='%s <%s>' % (name, phone))
            instance = cls(name=name, contact_email=email, contact_phone=phone, stripe_id=stripe_cust.id)
            instance.put()
            return instance
    ...
    def subscribe(self, plan_id, **props):
        ...
        return stripe.Subscription.create(customer=self.stripe_id, plan=plan_id, **props)
