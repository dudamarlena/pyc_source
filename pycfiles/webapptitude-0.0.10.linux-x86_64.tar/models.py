...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/models.py
...
def uuid_namespace(kind):
    ...
    is_kind_type = type(kind) is type(ndb.Model)
...
class UUIDKeyModel(ndb.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if _key is None and _id is None:
            kind = self._get_kind()
            _key = ndb.Key(kind, generate_key(kind, **kwargs), parent=kwargs.pop('parent', None), app=kwargs.pop('app', None), namespace=kwargs.pop('namespace', None))
        if not _key and _id:
            kwargs['id'] = _id
        elif _key:
            kwargs['key'] = _key
...
class BooleanOptionsProperty(ndb.BooleanProperty):
    ...
    TRUEISH = [
     'true', 'True', 'ON', 'on', 'checked', 'enabled']
