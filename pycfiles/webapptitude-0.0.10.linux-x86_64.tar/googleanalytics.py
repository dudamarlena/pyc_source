...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/googleanalytics.py
...
class GoogleAnalyticsReportingService(googleapi.ServiceBase):
    ...
    def api_v4(self):
        ...
        return self.discover('analyticsreporting', 'v4', discoveryServiceUrl=url)
    ...
    def accounts(self):
        ...
        for record in result.get('items', []):
            yield (
             record.get('id'), record.get('name'), record)
    ...
    def list_profiles(self, account_id, property_id):
        ...
        result = self.execute(query, accountId=account_id, webPropertyId=property_id)
        for record in result.get('items', []):
            yield (
             record.get('id'),
             record.get('name'),
        ...
        for account_id, account_name, account in self.accounts:
            ...
            for prop_alias, prop_id, prop_name, prop in webproperties:
                ...
                for prof_id, prof_name, prof in profiles:
                    yield odict(ident=(
                     account_id, prop_id, prof_id), account=account, webproperty=prop, profile=prof, account_name=account_name, webproperty_name=prop_name, webproperty_alias=prop_alias, profile_name=prof_name)


RE_TIME_DURATION = re.compile('^([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})$')

def coerce_type(typename, value, name):
    assert isinstance(value, basestring), 'Expected string, got %r for %r' % (type(value), name)
...
class ReportProxy(object):
    ...
    def dimension_headers(self):
        return self.base.get('columnHeader', {}).get('dimensions')
...
class ReportBatch():
    ...
    def build_sequence_segment(cls, *steps, **kwargs):
        ...
        base = dict(sequenceSegment=dict(firstStepShouldMatchFirstHit=match_first, segmentSequenceSteps=steps))
        base['not'] = bool(kwargs.pop('invert', False))
        return base
    ...
    @classmethod
    def build_segment_filter_dimension_clause(cls, dimension, operator, value, invert=False, case_sensitive=False):
        """
        ...
        """
        assert operator in ('EXACT', 'REGEXP', 'BEGINS_WITH', 'ENDS_WITH', 'PARTIAL',
                            'NUMERIC_BETWEEN', 'NUMERIC_GREATER_THAN', 'NUMERIC_LESS_THAN',
                            'IN_LIST')
        expr = [value] if isinstance(value, basestring) else value
        filter_def = dict(dimensionName=dimension, operator=operator, caseSensitive=bool(case_sensitive), expressions=expr)
    ...
    @classmethod
    def build_segment_filter_metric_clause(cls, metric, scope, operator, value, invert=False):
        assert operator in ('EQUAL', 'LESS_THAN', 'GREATER_THAN', 'BETWEENIS_MISSING')
        assert scope in ('USER', 'SESSION', 'HIT', 'PRODUCT')
        if operator == 'BETWEEN':
            _min, _max = value
        else:
            _min, _max = value, None
        filter_def = dict(metricName=metric, operator=operator, scope=scope, comparisonValue=str(_min), maxComparisonValue=str(_max or 0))
        base = dict(metricFilter=filter_def)
        base['not'] = bool(invert)
        return base

    @classmethod
    def build_cohort(cls, name, date_start, date_end, cohort_type='FIRST_VISIT_DATE'):
        assert cohort_type in ('FIRST_VISIT_DATE', )
        return dict(name=name, dateRange=cls.build_date_range(date_start, date_end), type=cohort_type)
    ...
    def to_dict(self, view_id):
        ...
        assert 0 < len(self.reports) < 6, 'A report batch allows 1 to 5 reports simultaneous.'
        assert len(self.cohort_group) or 0 < len(self.date_ranges) < 3, 'A report batch requires one or two (maximum) date ranges.'
        assert 0 <= len(self.segments) < 5, 'A report batch allows a maximum of 4 segments.'
        if len(self.cohort_group):
            cohort_config = dict(cohorts=self.cohort_group, lifetimeValue=bool(self.use_lifetime_value))
    ...
    def fetch(self, analytics_service, *view_ids):
        ...
        """
        if not isinstance(analytics_service, GoogleAnalyticsReportingService):
            raise AssertionError
            service = analytics_service.api_v4
            view_ids = len(view_ids) or [
             self.view_ids]
        for v in view_ids:
...
class ReportRequestModel(object):
    ...
    def __init__(self):
        ...
        self.metrics = []
        self.order_by = []
        self.options = dict(pageSize=10000, includeEmptyRows=False, hideTotals=False, hideValueRanges=False, dimensionFilterClauses=[], metricFilterClauses=[], pivots=[], orderBys=[])
    ...
    def build_dimension_filter(cls, dimension, *expressions, **kwargs):
        ...
        assert operator in ('REGEXP', 'IN_LIST', 'EXACT', 'PARTIAL', 'BEGINS_WITH',
                            'ENDS_WITH', 'NUMERIC_EQUAL', 'NUMERIC_GREATER_THAN',
                            'NUMERIC_LESS_THAN')
        base = dict(dimensionName=dimension, operator=operator, expressions=list(expressions), caseSensitive=bool(kwargs.pop('case_sensitive', False)))
        base['not'] = bool(kwargs.pop('invert', False))
        return base

    @classmethod
    def build_metric_filter(cls, metric, compare_value, operator='EQUAL', invert=False):
        assert operator in ('EQUAL', 'LESS_THAN', 'GREATER_THAN', 'IS_MISSING')
        if operator == 'IS_MISSING':
            compare_value = None
        else:
            compare_value = str(compare_value)
        base = dict(metricName=metric, operator=operator, comparisonValue=compare_value)
        base['not'] = bool(invert)
        return base

    @classmethod
    def build_pivot_clause(cls, metrics, dimensions, dim_filters=None, start=1, max_groups=10):
        assert isinstance(metrics, list)
        assert isinstance(dimensions, list)
        assert dim_filters is None or isinstance(dim_filters, list)
        assert 0 < max_groups <= 1000
        return dict(metrics=metrics, dimensions=dimensions, startGroup=int(start), maxGroupCount=int(max_groups), dimensionFilterClauses=dim_filters or [])
    ...
    def build_dimension_filter_clause(cls, *filters, **kwargs):
        ...
        return dict(filters=[ f for f in filters if isinstance(f, dict) ], operator=operator or 'OR')
    ...
    def build_dimension_clause(cls, dimension_name, histogram=None):
        return dict(name=dimension_name, histogramBuckets=histogram)
    ...
    def add_dimension(self, *args, **kwargs):
        for a in args:
            ...
            if dim.get('histogramBuckets', None):
                self.apply_order(dim.get('name'), order_type='HISTOGRAM_BUCKET')
    ...
    def add_metric_filters(self, filter_group, operator='OR'):
        ...
        self.options['metricFilterClauses'].append(dict(filters=filter_group, operator=operator))

    @classmethod
    def build_metric_clause(cls, expression, alias=None, format='INTEGER'):
        assert format in ('INTEGER', 'FLOAT', 'CURRENCY', 'PERCENT', 'TIME')
        return dict(expression=expression, alias=alias, formattingType=format)

    def add_metric(self, *args, **kwargs):
        self.metrics.append(self.build_metric_clause(*args, **kwargs))

    def apply_order(self, name, order_type=None, sort_order=None):
        assert order_type in (None, 'VALUE', 'DELTA', 'SMART', 'HISTOGRAM_BUCKET',
                              'DIMENSION_AS_INTEGER')
        assert sort_order in (None, 'ASCENDING', 'DESCENDING')
        self.options['orderBys'].append(dict(fieldName=name, orderType=order_type or 'VALUE', sortOrder=sort_order or 'ASCENDING'))
        return
    ...
    def set_filter(self, *expressions):
        ...
        """
        self.properties['filtersExpression'] = (',').join(expressions)
    ...
    def to_dict(self, view_id):
        ...
        for i, m in enumerate(self.metrics):
            if isinstance(m, basestring):
                self.metrics[i] = dict(expressions=m, alias=m, formattingType='FLOAT')

        assert 0 < self.options['pageSize'] < 10001
        return dict(viewId=view_id, metrics=self.metrics, dimensions=self.dimensions, pageToken=None, **self.options)