...
def prepare_test_suite(top_level_dir, pattern, *dirs):
    ...
    for testpath in dirs:
        suite = loader.discover(testpath, pattern=pattern or 'test_*.py', top_level_dir=top_level_dir)
...
def perform_test_suite(suite, args):
    options = dict(verbosity=args.verbosity, failfast=args.failfast)
...
def parse_options(args):
    ...
    parser.add_argument('-v', '--verbosity', action='count', help='increase verbosity', default=1)
    parser.add_argument('-f', '--failfast', action='store_true', help='Quit on first failure', default=False)
    parser.add_argument('--coverage', action='store_true', help='Activate test coverage')
