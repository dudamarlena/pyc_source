...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/queue.py
...
import contextlib, random, time, logging, re, types, webapp2, pickle, traceback
DEFAULT_QUEUE = deferred.deferred._DEFAULT_QUEUE
MOCK_REQUEST = None
RE_INVALID_JOBNAME_CHARS = re.compile('[^a-z0-9\\-]+')
...
def generate_job_id(callback):
    ...
    return sanitize_job_id(('{module}-{method}-{rand}-{timestamp}').format(module=callback.__module__, method=callback.__name__, rand=random.randint(1000.0, 1000000.0), timestamp=time.time()))
...
class Flow(ndb.Model):
    ...
    def target_task_id(self):
        ...
        return get_header(TARGET_TASK_HEADER) or self.previous_task_id or self.current_task_id
...
class Task(namedtuple('Task', 'handler, args, kwargs, job_id')):
    ...
    def fixbinding(cls, callback):
        ...
        if isinstance(callback, types.BuiltinMethodType):
            ...
            if isinstance(callback, types.ObjectType) and hasattr(callback, '__call__'):
                return callback
            if isinstance(callback, (types.FunctionType, types.BuiltinFunctionType,
...
def is_callable_task(item):
    ...
    return isinstance(item, Task) and len(item) == 4 and callable(item[0]) and isinstance(item[3], basestring) and isinstance(item[1], (tuple, list)) and isinstance(item[2], dict)
...
def flowrunner(task_instance, key, done=None, fail=None, immediate=False):
    ...
    def notify_failure(err):
        ...
        if is_callable_task(fail):
            if immediate:
                ...
                deferred.defer(flowrunner, fail, key, _name=fail[3], _headers=next_task_headers())
    ...
    with Flow.locker(key, enforce_locking=not immediate) as (entry):
            ...
            try:
                try:
                    for value in iter(result):
                        if is_callable_task(value):
                            extended = True
                            if immediate:
                                flowrunner(value, key, done=done, fail=fail, immediate=True)
                            else:
                                deferred.defer(flowrunner, value, key, done=done, fail=fail, _headers=next_task_headers())
                        else:
        ...
        if is_callable_task(result):
            extended = True
            if immediate:
                flowrunner(result, key, done=done, fail=fail, immediate=True)
            else:
                deferred.defer(flowrunner, result, key, done=done, fail=fail, _headers=next_task_headers())
        ...
        if not extended and is_callable_task(done):
            if immediate:
                ...
                deferred.defer(flowrunner, done, key, fail=fail, _name=done.job_id, _headers=next_task_headers())
    return
