...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/site-packages/webapptitude/bigquery.py
...
RECORD_FIELD_TYPE_CONVERT = {'INTEGER': int, 
   'FLOAT': float, 
...
class BigQueryService(googleapi.ServiceBase):
    ...
    def projects(self):
        ...
        for project in result.get('projects', []):
            yield (
             project.get('id'), project.get('friendlyName'),
    ...
    def query(self, project_id, query_text, timeout=10000, retries=5):
        ...
        """
        props = dict(projectId=project_id, body=dict(query=query_text, timeoutMs=timeout))
    ...
    def query_async(self, project_id, query_text, batch=True, large=False, retries=5):
        """
        ...
        jobdata = dict(jobReference=dict(projectId=project_id, jobId=job_id, allowLargeResults=large), configuration=dict(query=dict(query=query_text, priority='BATCH' if batch else 'INTERACTIVE')))
        factory = bigquery.jobs().insert
        self.execute(factory, retries=retries, projectId=project_id, body=jobdata)
        return job_id
    ...
    def get_results(self, project_id, job_id):
        ...
        results = self.executor.map(results, 'rows', projectId=project_id, jobId=job_id)
...
class Project(object):
    ...
    def get_dataset(self, dataset_id):
        ...
        result = self.service.execute(query, projectId=self.project_id, datasetId=dataset_id)
    ...
    def create_dataset(self, dataset_id, name, **kwargs):
        request = dict(projectId=self.project_id, datasetReference=dict(projectId=self.project_id, datasetId=dataset_id), description=kwargs.pop('description', None), friendlyName=name, defaultTableExpiration=kwargs.pop('tables_expire', None))
        query = self.connection.datasets().insert
        result = self.service.execute(query, projectId=self.project_id, body=request)
        return Dataset(self.service, self.project_id, dataset_id, base=result)

    @property
    def datasets(self):
        query = self.connection.datasets().list
        datasets = self.executor.map(query, 'datasets', projectId=self.project_id)
...
class Dataset(Project):
    ...
    def delete(self, all_tables=True):
        ...
        request = self.service.execute(query, projectId=self.project_id, datasetId=self.dataset_id, deleteContents=all_tables)
        return request
    ...
    def __autopatch_table__(self, table_id, schema):
        if self.table_exists(table_id):
            ...
            schema.pop('tableId', None)
        result = self.service.execute(query, **schema)
        return Table(self.service, self.project_id, self.dataset_id, table_id, base=result)

    def prepare_table(self, table_id, table_name, schema):
        assert isinstance(schema, dict)
        schema = Table.create_schema(self.project_id, self.dataset_id, table_id, table_name, schema)
    ...
    def prepare_view(self, view_id, view_name, view_sql, **kwargs):
        ...
        base = Table.create_schema(self.project_id, self.dataset_id, view_id, view_name, view_sql, **kwargs)
    ...
    def get_table(self, table_id):
        ...
        result = self.service.execute(query, projectId=self.project_id, datasetId=self.dataset_id, tableId=table_id)
        return Table(self.service, self.project_id, self.dataset_id, table_id, base=result)
    ...
    def tables_raw(self):
        ...
        tables = self.executor.map(query, 'tables', projectId=self.project_id, datasetId=self.dataset_id)
    ...
    def tables(self):
        for record in self.tables_raw:
            yield (
             record.get('id'), record.get('friendlyName'),
             Table(self.service, self.project_id, self.dataset_id, record.get('id'), base=record))
...
class Table(Project):
    ...
    def table_schema_base(cls, project_id, dataset_id, table_id, name):
        return odict(projectId=project_id, datasetId=dataset_id, tableId=table_id, body=dict(tableReference=dict(projectId=project_id, datasetId=dataset_id, tableId=table_id), friendlyName=name))

    @classmethod
    def assemble_schema_body(cls, schema):
        fieldset = []
        for fieldname, attribs in schema.items():
            fieldset.append(dict(name=fieldname, type=attribs[0], mode=attribs[1] or 'NULLABLE', description=attribs[2] or fieldname))
    ...
    def assemble_view_body(cls, view_sql, kwargs):
        ...
        return dict(view=dict(query=view_sql, useLegacySql=bool(legacy), userDefinedFunctionResources=udf))

    @classmethod
    def create_schema(cls, project_id, dataset_id, table_id, name, *args, **kwargs):
        """
        ...
        if len(args):
            ...
            if isinstance(args[0], basestring):
                view_sql = args[0]
        base = cls.table_schema_base(project_id, dataset_id, table_id, name)
    ...
    def insert(self, *records):
        ...
        query = dict(projectId=self.project_id, datasetId=self.dataset_id, tableId=self.table_id, body=dict(rows=[ dict(json=r) for r in records ], insertId=str(uuid.uuid4())))
