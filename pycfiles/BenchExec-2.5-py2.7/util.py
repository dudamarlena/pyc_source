...
import bz2, collections, datetime, errno, fnmatch, glob, logging, os, shutil, signal, stat, subprocess, sys, time
...
def setup_logging(format=b'%(asctime)s - %(levelname)s - %(message)s', level=b'INFO'):
...
def get_capability(filename):
    ...
    if cap_object != None:
        ...
        res[b'set'] = [ char for char in cap_string.split(b'+')[1] ]
    return res
