...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchexec/baseexecutor.py
# Compiled at: 2019-11-28 13:06:28
from __future__ import absolute_import, division, print_function, unicode_literals
import errno, logging, os, signal, subprocess, sys, threading
...
def add_basic_executor_options(argument_parser):
    ...
    argument_parser.add_argument(b'args', nargs=b'+', metavar=b'ARG', help=b'command line to run (prefix with "--" to ensure all arguments are treated correctly)')
    argument_parser.add_argument(b'--version', action=b'version', version=b'%(prog)s ' + __version__)
    verbosity = argument_parser.add_mutually_exclusive_group()
    verbosity.add_argument(b'--debug', action=b'store_true', help=b'show debug output')
    verbosity.add_argument(b'--quiet', action=b'store_true', help=b'show only warnings')
...
class BaseExecutor(object):
    ...
    def _start_execution(self, args, stdin, stdout, stderr, env, cwd, temp_dir, cgroups, parent_setup_fn, child_setup_fn, parent_cleanup_fn):
        ...
        tmp_dir = os.path.join(temp_dir, b'tmp')
        home_dir = os.path.join(temp_dir, b'home')
        os.mkdir(tmp_dir)
        os.mkdir(home_dir)
        env[b'HOME'] = home_dir
        env[b'TMPDIR'] = tmp_dir
        env[b'TMP'] = tmp_dir
        env[b'TEMPDIR'] = tmp_dir
        env[b'TEMP'] = tmp_dir
        logging.debug(b'Executing run with $HOME and $TMPDIR below %s.', temp_dir)
        ...
        def wait_and_get_result():
            ...
            parent_cleanup = parent_cleanup_fn(parent_setup, util.ProcessExitCode.from_raw(exitcode), b'')
            return (
             exitcode, ru_child, parent_cleanup)

        return (
         p.pid, wait_and_get_result)
    ...
    def _wait_for_process(self, pid, name):
        ...
        """
        try:
            logging.debug(b'Waiting for process %s with pid %s', name, pid)
            unused_pid, exitcode, ru_child = os.wait4(pid, 0)
            return (exitcode, ru_child)
        except OSError as e:
            if self.PROCESS_KILLED and e.errno == errno.EINTR:
                logging.debug(b'OSError %s while waiting for termination of %s (%s): %s.', e.errno, name, pid, e.strerror)
                try:
                    unused_pid, exitcode, ru_child = os.wait4(pid, 0)
                    return (exitcode, ru_child)
                except OSError:
            ...
            logging.critical(b'OSError %s while waiting for termination of %s (%s): %s.', e.errno, name, pid, e.strerror)
            return (0, None)
    ...
    def stop(self):
        ...
        with self.SUB_PROCESS_PIDS_LOCK:
            for pid in self.SUB_PROCESS_PIDS:
                logging.warning(b'Killing process %s forcefully.', pid)
                try:
