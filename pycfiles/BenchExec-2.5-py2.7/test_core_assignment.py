...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchexec/test_core_assignment.py
# Compiled at: 2019-11-28 13:06:29
from __future__ import absolute_import, division, print_function, unicode_literals
import itertools, logging, sys, unittest, math
...
class TestCpuCoresPerRun(unittest.TestCase):
    ...
    def assertValid(self, coreLimit, num_of_threads, expectedResult=None):
        result = _get_cpu_cores_per_run0(coreLimit, num_of_threads, self.use_ht, *self.machine())
        if expectedResult:
            self.assertEqual(expectedResult, result, (b'Incorrect result for {} cores and {} threads.').format(coreLimit, num_of_threads))

    def assertInvalid(self, coreLimit, num_of_threads):
        self.assertRaises(SystemExit, _get_cpu_cores_per_run0, coreLimit, num_of_threads, self.use_ht, *self.machine())
    ...
    def machine(self):
        ...
        return (
         allCpus, cores_of_package, siblings_of_core)
    ...
    def test_oneCorePerRun(self):
        ...
        if not self.oneCore_assignment:
            self.skipTest(b'Need result specified')
    ...
    def test_eightCoresPerRun(self):
        ...
        if not self.use_ht and self.ht:
            ...
            if maxThreads == 0:
                ...
                maxThreads = self.cpus // cpus_per_run
        if not maxThreads:
            self.skipTest(b'Testing for runs that need to be split across CPUs is not implemented')
...
class TestCpuCoresPerRun_singleCPU_HT(TestCpuCoresPerRun_singleCPU):
    ...
    def test_halfPhysicalCore(self):
        self.assertRaises(SystemExit, _get_cpu_cores_per_run0, 1, 1, True, [
         0], {0: [0, 1]}, {0: [0, 1]})
...
class TestCpuCoresPerRun_threeCPU_HT(TestCpuCoresPerRun):
    ...
    def test_threeCPU_HT_noncontiguousId(self):
        ...
        result = _get_cpu_cores_per_run0(2, 3, True, [
         0, 1, 2, 3, 6, 7], {0: [0, 1], 2: [2, 3], 3: [6, 7]}, {0: [0, 1], 1: [0, 1], 2: [2, 3], 3: [2, 3], 6: [6, 7], 7: [6, 7]})
        self.assertEqual([
         [
          0, 1], [2, 3], [6, 7]], result, (b'Incorrect result for {} cores and {} threads.').format(2, 3))
...
class TestCpuCoresPerRun_quadCPU_HT(TestCpuCoresPerRun):
    ...
    def test_quadCPU_HT_noncontiguousId(self):
        ...
        result = _get_cpu_cores_per_run0(1, 8, True, [
         0, 1, 8, 9, 16, 17, 24, 25, 32, 33, 40, 41, 48, 49, 56, 57], {0: [
             0, 1, 8, 9], 
           1: [
             32, 33, 40, 41], 
           2: [
        ...
        self.assertEqual([
         [
          0], [32], [48], [16], [8], [40], [56], [24]], result, (b'Incorrect result for {} cores and {} threads.').format(1, 8))
...
class TestCpuCoresPerRun_dualCPU_no_ht(TestCpuCoresPerRun):
    ...
    def test_dualCPU_noncontiguousID(self):
        results = _get_cpu_cores_per_run0(2, 3, False, [
         0, 4, 9, 15, 21, 19, 31, 12, 10, 11, 8, 23, 27, 14, 1, 20], {0: [0, 4, 9, 12, 15, 19, 21, 31], 2: [10, 11, 8, 23, 27, 14, 1, 20]}, {0: [
             0, 4], 
           4: [
             0, 4], 
           9: [
        ...
        self.assertEqual(results, [
         [
          0, 9], [8, 10], [15, 21]], (b'Incorrect result for {} cores and {} threads.').format(2, 3))
