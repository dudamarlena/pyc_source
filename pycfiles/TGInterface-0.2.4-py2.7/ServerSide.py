...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/TGInterface/ServerSide.py
# Compiled at: 2012-01-13 09:52:26
...
class AutoAPI(object):
    ...
    def __init__(decorator_obj, table):
        ...
        if table not in ExposeTable.classes.values():
    ...
    def __call__(decorator_obj, cls):
        ...
        for exposed_attr in decorator_obj.api_class.__exposed_attrs__:
            ...
            if type(exposed_attr_type) == str:
                ...
                exposed_attr_type = [
                 ExposeTable.classes[exposed_attr_type[0]]]
...
def discriminate(typ):
    ...
    if typ in helper._primitives:
        ...
        if typ == None:
            return NONE
        if type(typ) == list and typ[0] in ExposeTable.classes.values():
            return LIST_OF_OBJECTS_FROM_MODEL
        if typ in ExposeTable.classes.values():
