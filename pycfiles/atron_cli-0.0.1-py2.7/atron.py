...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/atron_cli/atron.py
# Compiled at: 2019-03-27 13:02:42
import click, time, platform, os
...
@click.group()
@click.option('--port', '-p', envvar='ATRON_PORT', default='', type=click.STRING, help='Name of serial port for connected board.  Can optionally specify with ATRON_PORT environment variable.', metavar='PORT')
@click.option('--baud', '-b', envvar='ATRON_BAUD', default=115200, type=click.INT, help='Baud rate for the serial connection (default 115200).  Can optionally specify with ATRON_BAUD environment variable.', metavar='BAUD')
    ...
    if platform.system() == 'Windows':
        ...
        while True:
            try:
                ...
                click.secho(('reonnecting to board after {} seconds. press ctrl+c to cancel').format(seconds), fg='green')
                time.sleep(seconds)
                seconds *= 2


@cli.command()
@click.option('-h', '--hard', 'hard', is_flag=True, default=False, help='Perform a hard reboot, including running init.py')
def reset(hard):
    if not hard:
        _board.soft_reset()
        return
...
@cli.command()
@click.argument('local', default='main.py')
@click.argument('remote', default='main.py')
...
@click.option('--no-output', '-n', is_flag=True, help='Run the code without waiting for it to finish and print output.  Use this when running code with main loops that never return.')
...
@click.option('--long_format', '-l', is_flag=True, help='Print long format info including size of files.  Note the size of directories is not supported and will show 0 values.')
@click.option('--recursive', '-r', is_flag=True, help='recursively list all files and (empty) directories.')
def ls(directory, long_format, recursive):
    ...
    for f in files:
        if not long_format:
            click.secho(f, fg='green' if os.path.splitext(f)[1].lower() == '.py' else 'white')
        else:
            click.echo(f)
