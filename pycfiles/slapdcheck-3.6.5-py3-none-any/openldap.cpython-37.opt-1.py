...
# [GCC 8.4.0]
# Embedded file name: /slapdcheck/openldap.py
# Compiled at: 2020-02-08 11:55:34
# Size of source mod 2**32: 15185 bytes
"""
slapdcheck.openldap - OpenLDAP helper classes
"""
import socket, time, datetime, threading, ldap0
from ldap0.ldapobject import LDAPObject
from ldap0.ldapurl import LDAPUrl
from ldap0.openldap import SyncReplDesc
from .cnf import CATCH_ALL_EXC, CHECK_RESULT_ERROR, CHECK_RESULT_OK, LDAP0_TRACE_LEVEL, LDAP_TIMEOUT
...
class OpenLDAPMonitorCache:
    ...
    def operation_counters(self):
        ...
        return [(entry['cn'][0], int(entry['monitorOpInitiated'][0]), int(entry['monitorOpCompleted'][0])) for dn, entry in self._data.items() if dn.lower().endswith(op_counter_suffix_lower)]


class OpenLDAPObject:
    __doc__ = "\n    mix-in class for LDAPObject and friends which provides methods useful\n    for OpenLDAP's slapd\n    "
    syncrepl_filter = '(&(objectClass=olcDatabaseConfig)(olcDatabase=*)(olcSyncrepl=*)(olcSuffix=*))'
    slapd_sock_filter = '(&(|(objectClass=olcDbSocketConfig)(objectClass=olcOvSocketConfig))(olcDbSocketPath=*))'
    naming_context_attrs = [
     'configContext',
     'namingContexts',
     'monitorContext']
    all_real_db_filter = '(&(objectClass=olcMdbConfig)(olcDatabase=*)(olcDbDirectory=*)(olcSuffix=*))'
    all_monitor_entries_filter = '(|(objectClass=monitorServer)(objectClass=monitorOperation)(objectClass=monitoredObject)(objectClass=monitorCounterObject))'
    ...
    def get_monitor_entries(self):
        ...
        """
        return {res.dn_s:res.entry_s for res in self.search_s((self.monitorContext[0]),
          (ldap0.SCOPE_SUBTREE),
    ...
    def get_sock_listeners(self):
        ...
        """
        ldap_result = self.search_s((self.configContext[0]),
          (ldap0.SCOPE_SUBTREE),
          (self.slapd_sock_filter),
          attrlist=[
         'olcDbSocketPath', 'olcOvSocketOps'])
        result = {}
    ...
    def get_context_csn(self, naming_context):
        ...
        by `naming_context'
        """
        ldap_result = self.read_s(naming_context,
          '(contextCSN=*)',
          attrlist=[
        ...
        try:
            ...
            for csn_value in context_csn_vals:
                ...
                csn_dict[server_id] = time.mktime(time.strptime(timestamp, '%Y%m%d%H%M%S.%fZ'))
    ...
    def get_syncrepl_topology(self):
        ...
        ldap_result = self.search_s((self.configContext[0]),
          ...
          attrlist=[
         'olcDatabase', 'olcSuffix', 'olcSyncrepl'])
        syncrepl_list = []
        for ldap_res in ldap_result:
            db_num = int(ldap_res.entry_s['olcDatabase'][0].split('}')[0][1:])
            srd = [SyncReplDesc(attr_value) for attr_value in ldap_res.entry_s['olcSyncrepl']]
    ...
    def db_suffixes(self):
        ...
        ldap_result = self.search_s((self.configContext[0]),
          ...
          attrlist=[
         'olcDatabase', 'olcSuffix', 'olcDbDirectory'])
        result = []
...
class SlapdConnection(LDAPObject, OpenLDAPObject):
    ...
    def __init__(self, uri, trace_level=LDAP0_TRACE_LEVEL, tls_options=None, network_timeout=None, timeout=None, bind_method='sasl', sasl_mech='EXTERNAL', who=None, cred=None):
    ...
    def set_tls_options(self, cacert_filename=None, client_cert_filename=None, client_key_filename=None, req_cert=ldap0.OPT_X_TLS_DEMAND):
        ...
        LDAPObject.set_tls_options(self,
...
class SyncreplProviderTask(threading.Thread):
    ...
    def __init__(self, check_instance, syncrepl_topology, syncrepl_target_uri):
    ...
    def run(self):
        ...
        try:
            ldap_conn = SlapdConnection((self.syncrepl_target_uri),
              tls_options={'cacert_filename':syncrepl_obj.tls_cacert, 
        ...
        for db_num, db_suffix, _ in self.syncrepl_topology[syncrepl_target_uri]:
            ...
            try:
                ...
                try:
                    self.check_instance.result(CHECK_RESULT_ERROR, item_name, 'Exception while retrieving remote contextCSN for %r from %r: %s' % (
                     db_suffix,
                     ldap_conn.uri,
                     exc))
                    continue
            ...
            if not self.remote_csn_dict[db_suffix]:
                self.check_instance.result(CHECK_RESULT_ERROR,
                  item_name,
                ...
                self.check_instance.result(CHECK_RESULT_OK,
                  item_name,
