...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/service/file.py
# Compiled at: 2017-05-01 00:38:10
from ..debugging import bacpypes_debugging, ModuleLogger
from ..capability import Capability
from ..object import FileObject
from ..apdu import AtomicReadFileACK, AtomicReadFileACKAccessMethodChoice, AtomicReadFileACKAccessMethodRecordAccess, AtomicReadFileACKAccessMethodStreamAccess, AtomicWriteFileACK
...
class LocalRecordAccessFileObject(FileObject):
    ...
    def __init__(self, **kwargs):
        ...
        if _debug:
            LocalRecordAccessFileObject._debug('__init__ %r', kwargs)
...
class LocalStreamAccessFileObject(FileObject):
    ...
    def __init__(self, **kwargs):
        ...
        if _debug:
            LocalStreamAccessFileObject._debug('__init__ %r', kwargs)
        if 'fileAccessMethod' in kwargs:
...
class FileServices(Capability):
    ...
    def do_AtomicReadFileRequest(self, apdu):
        ...
        if apdu.accessMethod.recordAccess:
            ...
            if record_access.fileStartRecord < 0 or record_access.fileStartRecord >= len(obj):
                raise ExecutionError('services', 'invalidFileStartPosition')
            end_of_file, record_data = obj.read_record(record_access.fileStartRecord, record_access.requestedRecordCount)
            if _debug:
                FileServices._debug('    - record_data: %r', record_data)
            resp = AtomicReadFileACK(context=apdu, endOfFile=end_of_file, accessMethod=AtomicReadFileACKAccessMethodChoice(recordAccess=AtomicReadFileACKAccessMethodRecordAccess(fileStartRecord=record_access.fileStartRecord, returnedRecordCount=len(record_data), fileRecordData=record_data)))
            ...
            if stream_access.fileStartPosition < 0 or stream_access.fileStartPosition >= len(obj):
                raise ExecutionError('services', 'invalidFileStartPosition')
            end_of_file, record_data = obj.read_stream(stream_access.fileStartPosition, stream_access.requestedOctetCount)
            if _debug:
                FileServices._debug('    - record_data: %r', record_data)
            resp = AtomicReadFileACK(context=apdu, endOfFile=end_of_file, accessMethod=AtomicReadFileACKAccessMethodChoice(streamAccess=AtomicReadFileACKAccessMethodStreamAccess(fileStartPosition=stream_access.fileStartPosition, fileData=record_data)))
        if _debug:
    ...
    def do_AtomicWriteFileRequest(self, apdu):
        ...
        if apdu.accessMethod.recordAccess:
            ...
            start_record = obj.write_record(record_access.fileStartRecord, record_access.recordCount, record_access.fileRecordData)
            if _debug:
                FileServices._debug('    - start_record: %r', start_record)
            resp = AtomicWriteFileACK(context=apdu, fileStartRecord=start_record)
        elif apdu.accessMethod.streamAccess:
            if obj.fileAccessMethod != 'streamAccess':
                raise ExecutionError('services', 'invalidFileAccessMethod')
            stream_access = apdu.accessMethod.streamAccess
            ...
            if obj.readOnly:
                raise ExecutionError('services', 'fileAccessDenied')
            start_position = obj.write_stream(stream_access.fileStartPosition, stream_access.fileData)
            if _debug:
                FileServices._debug('    - start_position: %r', start_position)
            resp = AtomicWriteFileACK(context=apdu, fileStartPosition=start_position)
        if _debug:
            FileServices._debug('    - resp: %r', resp)
        self.response(resp)
        return
