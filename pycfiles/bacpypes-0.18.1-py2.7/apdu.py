...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/apdu.py
# Compiled at: 2020-04-22 18:29:10
"""
...
from .primitivedata import Boolean, CharacterString, Enumerated, Integer, ObjectIdentifier, ObjectType, OctetString, Real, TagList, Unsigned, expand_enumerations
from .constructeddata import Any, Choice, Element, Sequence, SequenceOf, SequenceOfAny
from .basetypes import ChannelValue, DateTime, DeviceAddress, ErrorType, EventState, EventTransitionBits, EventType, LifeSafetyOperation, NotificationParameters, NotifyType, ObjectPropertyReference, PropertyIdentifier, PropertyReference, PropertyValue, RecipientProcess, ResultFlags, Segmentation, TimeStamp, VTClass
_debug = 0
...
_max_apdu_length_encoding = [
 50, 128, 206, 480, 1024, 1476, None, None,
...
class APCI(PCI, DebugContents):
    _debug_contents = ('apduType', 'apduSeg', 'apduMor', 'apduSA', 'apduSrv', 'apduNak',
                       'apduSeq', 'apduWin', 'apduMaxSegs', 'apduMaxResp', 'apduService',
                       'apduInvokeID', 'apduAbortRejectReason')
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.apduSeq = None
        self.apduWin = None
        self.apduMaxSegs = None
        self.apduMaxResp = None
    ...
    def decode(self, pdu):
        ...
        PCI.update(self, pdu)
        buff = pdu.get()
        self.apduType = buff >> 4 & 15
        if self.apduType == ConfirmedRequestPDU.pduType:
            self.apduSeg = buff & 8 != 0
            self.apduMor = buff & 4 != 0
            self.apduSA = buff & 2 != 0
            buff = pdu.get()
            self.apduMaxSegs = buff >> 4 & 7
            self.apduMaxResp = buff & 15
            ...
            self.apduNak = buff & 2 != 0
            self.apduSrv = buff & 1 != 0
            self.apduInvokeID = pdu.get()
            self.apduSeq = pdu.get()
            self.apduWin = pdu.get()
        elif self.apduType == ErrorPDU.pduType:
...
class _APDU(APDU):
    ...
    def encode(self, pdu):
        if _debug:
            _APDU._debug('encode %r', pdu)
        APCI.update(pdu, self)
        pdu.put_data(self.pduData)

    def decode(self, pdu):
        if _debug:
            _APDU._debug('decode %r', pdu)
        APCI.update(self, pdu)
        self.pduData = pdu.get_data(len(pdu.pduData))

    def set_context(self, context):
        if _debug:
            _APDU._debug('set_context %r', context)
...
class ConfirmedRequestPDU(_APDU):
    ...
    def __init__(self, choice=None, *args, **kwargs):
        if _debug:
            ConfirmedRequestPDU._debug('__init__ %r %r %r', choice, args, kwargs)
...
class UnconfirmedRequestPDU(_APDU):
    ...
    def __init__(self, choice=None, *args, **kwargs):
        if _debug:
            UnconfirmedRequestPDU._debug('__init__ %r %r %r', choice, args, kwargs)
...
class SimpleAckPDU(_APDU):
    ...
    def __init__(self, choice=None, invokeID=None, context=None, *args, **kwargs):
        if _debug:
            SimpleAckPDU._debug('__init__ %r %r %r %r %r', choice, invokeID, context, args, kwargs)
...
class ComplexAckPDU(_APDU):
    ...
    def __init__(self, choice=None, invokeID=None, context=None, *args, **kwargs):
        if _debug:
            ComplexAckPDU._debug('__init__ %r %r %r %r %r', choice, invokeID, context, args, kwargs)
...
class SegmentAckPDU(_APDU):
    ...
    def __init__(self, nak=None, srv=None, invokeID=None, sequenceNumber=None, windowSize=None, *args, **kwargs):
        if _debug:
            SegmentAckPDU._debug('__init__ %r %r %r %r %r %r %r', nak, srv, invokeID, sequenceNumber, windowSize, args, kwargs)
...
class ErrorPDU(_APDU):
    ...
    def __init__(self, choice=None, invokeID=None, context=None, *args, **kwargs):
        if _debug:
            ErrorPDU._debug('__init__ %r %r %r %r %r', choice, invokeID, context, args, kwargs)
...
class RejectReason(Enumerated):
    vendor_range = (64, 255)
    enumerations = {'other': 0, 'bufferOverflow': 1, 
...
class RejectPDU(_APDU):
    ...
    def __init__(self, invokeID=None, reason=None, context=None, *args, **kwargs):
        if _debug:
            RejectPDU._debug('__init__ %r %r %r %r %r', invokeID, reason, context, args, kwargs)
...
class AbortPDU(_APDU):
    ...
    def __init__(self, srv=None, invokeID=None, reason=None, context=None, *args, **kwargs):
        if _debug:
            AbortPDU._debug('__init__ %r %r %r %r %r %r', srv, invokeID, reason, context, args, kwargs)
...
class APCISequence(APCI, Sequence):
    ...
    def __init__(self, *args, **kwargs):
        if _debug:
            APCISequence._debug('__init__ %r %r', args, kwargs)
    ...
    def decode(self, apdu):
        ...
        if _debug:
            APCISequence._debug('    - tag list: %r', self._tag_list)
        Sequence.decode(self, self._tag_list)
        if self._tag_list:
            if _debug:
                APCISequence._debug('    - trailing unmatched tags')
            raise TooManyArguments()

    def apdu_contents(self, use_dict=None, as_class=dict):
        """Return the contents of an object as a dict."""
        if _debug:
            APCISequence._debug('apdu_contents use_dict=%r as_class=%r', use_dict, as_class)
        if use_dict is None:
            use_dict = as_class()
...
class ConfirmedRequestSequence(APCISequence, ConfirmedRequestPDU):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ConfirmedRequestSequence, self).__init__(choice=self.serviceChoice, *args, **kwargs)
...
class ComplexAckSequence(APCISequence, ComplexAckPDU):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ComplexAckSequence, self).__init__(choice=self.serviceChoice, *args, **kwargs)
...
class UnconfirmedRequestSequence(APCISequence, UnconfirmedRequestPDU):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(UnconfirmedRequestSequence, self).__init__(choice=self.serviceChoice, *args, **kwargs)
...
class ErrorSequence(APCISequence, ErrorPDU):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ErrorSequence, self).__init__(choice=self.serviceChoice, *args, **kwargs)
...
class UnconfirmedTextMessageRequestMessagePriority(Enumerated):
    enumerations = {'normal': 0, 'urgent': 1}
...
class DeviceCommunicationControlRequestEnableDisable(Enumerated):
    enumerations = {'enable': 0, 'disable': 1, 
       'disableInitiation': 2}
...
class ReinitializeDeviceRequestReinitializedStateOfDevice(Enumerated):
    enumerations = {'coldstart': 0, 'warmstart': 1, 
       'startBackup': 2, 
       'endBackup': 3, 
       'startRestore': 4, 
       'endRestore': 5, 
...
class ConfirmedServiceChoice(Enumerated):
    enumerations = {'acknowledgeAlarm': 0, 
