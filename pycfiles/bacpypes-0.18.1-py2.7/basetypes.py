...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/bacpypes/basetypes.py
# Compiled at: 2020-04-20 14:21:25
"""
Base Types
"""
from .debugging import bacpypes_debugging, ModuleLogger
from .errors import MissingRequiredParameter
from .primitivedata import Atomic, BitString, Boolean, CharacterString, Date, Double, Enumerated, Integer, Null, ObjectIdentifier, OctetString, Real, Time, Unsigned, Unsigned16, Tag
from .constructeddata import Any, AnyAtomic, ArrayOf, Choice, Element, Sequence, SequenceOf
_debug = 0
_log = ModuleLogger(globals())

class DaysOfWeek(BitString):
    bitNames = {'monday': 0, 'tuesday': 1, 
       'wednesday': 2, 
       'thursday': 3, 
       'friday': 4, 
       'saturday': 5, 
...
class EventTransitionBits(BitString):
    bitNames = {'toOffnormal': 0, 'toFault': 1, 
       'toNormal': 2}
    bitLen = 3


class LimitEnable(BitString):
    bitNames = {'lowLimitEnable': 0, 'highLimitEnable': 1}
    bitLen = 2


class LogStatus(BitString):
    bitNames = {'logDisabled': 0, 'bufferPurged': 1, 
       'logInterrupted': 2}
    bitLen = 3


class ObjectTypesSupported(BitString):
    bitNames = {'analogInput': 0, 'analogOutput': 1, 
       'analogValue': 2, 
       'binaryInput': 3, 
       'binaryOutput': 4, 
       'binaryValue': 5, 
...
class ResultFlags(BitString):
    bitNames = {'firstItem': 0, 'lastItem': 1, 
       'moreItems': 2}
    bitLen = 3


class ServicesSupported(BitString):
    bitNames = {'acknowledgeAlarm': 0, 'confirmedCOVNotification': 1, 
...
class AccessAuthenticationFactorDisable(Enumerated):
    ...
    enumerations = {'none': 0, 'disabled': 1, 
       'disabledLost': 2, 
       'disabledStolen': 3, 
       'disabledDamaged': 4, 
       'disabledDestroyed': 5}
...
class AccessCredentialDisable(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'none': 0, 'disable': 1, 
       'disableManual': 2, 
       'disableLockout': 3}


class AccessCredentialDisableReason(Enumerated):
    enumerations = {'disabled': 0, 'disabledNeedsProvisioning': 1, 
...
class AccessPassbackMode(Enumerated):
    enumerations = {'passbackOff': 0, 'hardPassback': 1, 
       'softPassback': 2}


class AccessRuleTimeRangeSpecifier(Enumerated):
    enumerations = {'specified': 0, 'always': 1}


class AccessRuleLocationSpecifier(Enumerated):
    enumerations = {'specified': 0, 'all': 1}


class AccessUserType(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'asset': 0, 'group': 1, 
       'person': 2}


class AccessZoneOccupancyState(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'normal': 0, 'belowLowerLimit': 1, 
       'atLowerLimit': 2, 
       'atUpperLimit': 3, 
       'aboveUpperLimit': 4, 
       'disabled': 5, 
...
class AccumulatorRecordAccumulatorStatus(Enumerated):
    enumerations = {'normal': 0, 'starting': 1, 
       'recovered': 2, 
       'abnormal': 3, 
       'failed': 4}
...
class Action(Enumerated):
    enumerations = {'direct': 0, 'reverse': 1}


class AuthenticationFactorType(Enumerated):
    enumerations = {'undefined': 0, 'error': 1, 
       'custom': 2, 
       'simpleNumber16': 3, 
       'simpleNumber32': 4, 
       'simpleNumber56': 5, 
...
class AuthenticationStatus(Enumerated):
    enumerations = {'notReady': 0, 'ready': 1, 
       'disabled': 2, 
...
class AuthorizationException(Enumerated):
    vendor_range = (64, 255)
    enumerations = {'passback': 0, 'occupancyCheck': 1, 
       'accessRights': 2, 
       'lockout': 3, 
       'deny': 4, 
       'verification': 5, 
...
class AuthorizationMode(Enumerated):
    ...
    enumerations = {'authorize': 0, 'grantActive': 1, 
       'denyAll': 2, 
...
class BackupState(Enumerated):
    enumerations = {'idle': 0, 'preparingForBackup': 1, 
...
class DeviceStatus(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'operational': 0, 'operationalReadOnly': 1, 
       'downloadRequired': 2, 
...
class DoorSecuredStatus(Enumerated):
    enumerations = {'secured': 0, 'unsecured': 1, 
       'unknown': 2}


class DoorStatus(Enumerated):
    enumerations = {'closed': 0, 'opened': 1, 
       'unknown': 2}


class DoorValue(Enumerated):
    enumerations = {'lock': 0, 'unlock': 1, 
       'pulseUnlock': 2, 
...
class EngineeringUnits(Enumerated):
    vendor_range = (256, 65535)
    enumerations = {'metersPerSecondPerSecond': 166, 
       'squareMeters': 0, 
...
class ErrorClass(Enumerated):
    enumerations = {'device': 0, 'object': 1, 
       'property': 2, 
       'resources': 3, 
       'security': 4, 
       'services': 5, 
...
class ErrorCode(Enumerated):
    enumerations = {'abortApduTooLong': 123, 'abortApplicationExceededReplyTime': 124, 
...
class EventType(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'changeOfBitstring': 0, 'changeOfState': 1, 
       'changeOfValue': 2, 
       'commandFailure': 3, 
       'floatingLimit': 4, 
       'outOfRange': 5, 
...
class FaultType(Enumerated):
    enumerations = {'none': 0, 'fault-characterstring': 1, 
       'fault-extended': 2, 
       'fault-life-safety': 3, 
       'fault-state': 4, 
...
class FileAccessMethod(Enumerated):
    enumerations = {'recordAccess': 0, 'streamAccess': 1}


class LifeSafetyMode(Enumerated):
    enumerations = {'off': 0, 'on': 1, 
       'test': 2, 
       'manned': 3, 
       'unmanned': 4, 
       'armed': 5, 
...
class LifeSafetyState(Enumerated):
    enumerations = {'quiet': 0, 'preAlarm': 1, 
       'alarm': 2, 
       'fault': 3, 
       'faultPreAlarm': 4, 
       'faultAlarm': 5, 
...
class LightingInProgress(Enumerated):
    enumerations = {'idle': 0, 'fadeActive': 1, 
       'rampActive': 2, 
       'notControlled': 3, 
       'other': 4}
...
class LightingTransition(Enumerated):
    vendor_range = (64, 255)
    enumerations = {'none': 0, 'fade': 1, 
       'ramp': 2}


class LockStatus(Enumerated):
    enumerations = {'locked': 0, 'unlocked': 1, 
       'fault': 2, 
       'unknown': 3}
...
class NotifyType(Enumerated):
    enumerations = {'alarm': 0, 'event': 1, 
       'ackNotification': 2}


class Polarity(Enumerated):
    enumerations = {'normal': 0, 'reverse': 1}


class ProgramError(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'normal': 0, 'loadFailed': 1, 
       'internal': 2, 
       'program': 3, 
       'other': 4}
...
class PropertyIdentifier(Enumerated):
    ...
    enumerations = {'absenteeLimit': 244, 'tags': 486, 
       'profileLocation': 91, 
...
class Reliability(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'noFaultDetected': 0, 'noSensor': 1, 
       'overRange': 2, 
       'underRange': 3, 
       'openLoop': 4, 
       'shortedLoop': 5, 
...
class RestartReason(Enumerated):
    vendor_range = (64, 255)
    enumerations = {'unknown': 0, 'coldstart': 1, 
       'warmstart': 2, 
       'detectedPowerLost': 3, 
...
class SecurityLevel(Enumerated):
    enumerations = {'incapable': 0, 'plain': 1, 
       'signed': 2, 
       'encrypted': 3, 
       'signedEndToEnd': 4, 
       'encryptedEndToEnd': 4}
...
class SecurityPolicy(Enumerated):
    enumerations = {'plainNonTrusted': 0, 'plainTrusted': 1, 
       'signedTrusted': 2, 
       'encryptedTrusted': 3}


class ShedState(Enumerated):
    enumerations = {'shedInactive': 0, 'shedRequestPending': 1, 
       'shedCompliant': 2, 
       'shedNonCompliant': 3}


class Segmentation(Enumerated):
    enumerations = {'segmentedBoth': 0, 'segmentedTransmit': 1, 
       'segmentedReceive': 2, 
       'noSegmentation': 3}
...
class SilencedState(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'unsilenced': 0, 'audibleSilenced': 1, 
       'visibleSilenced': 2, 
       'allSilenced': 3}
...
class VTClass(Enumerated):
    vendor_range = (64, 65535)
    enumerations = {'defaultTerminal': 0, 'ansiX3-64': 1, 
       'decVt52': 2, 
       'decVt100': 3, 
       'decVt220': 4, 
       'hp-700-94': 5, 
...
class WriteStatus(Enumerated):
    enumerations = {'idle': 0, 'inProgress': 1, 
       'successful': 2, 
       'failed': 3}


class NetworkType(Enumerated):
    enumerations = {'ethernet': 0, 'arcnet': 1, 
       'mstp': 2, 
       'ptp': 3, 
       'lontalk': 4, 
       'ipv4': 5, 
...
class ProtocolLevel(Enumerated):
    enumerations = {'physical': 0, 'protocol': 1, 
       'bacnetApplication': 2, 
       'nonBacnetApplication': 3}


class NetworkNumberQuality(Enumerated):
    enumerations = {'unknown': 0, 'learned': 1, 
       'learnedConfigured': 2, 
       'configured': 3}


class NetworkPortCommand(Enumerated):
    enumerations = {'idle': 0, 'discardChanges': 1, 
...
class IPMode(Enumerated):
    enumerations = {'normal': 0, 'foreign': 1, 
       'bbmd': 2}


class RouterEntryStatus(Enumerated):
    enumerations = {'available': 0, 'busy': 1, 
       'disconnected': 2}
...
class BDTEntry(Sequence):
    sequenceElements = [
     ...
     Element('broadcastMask', OctetString)]


class FDTEntry(Sequence):
    sequenceElements = [
     Element('bacnetIPAddress', OctetString),
     Element('timeToLive', Unsigned16),
     Element('remainingTimeToLive', Unsigned16)]


class VMACEntry(Sequence):
    sequenceElements = [
     Element('virtualMACAddress', OctetString),
...
class RouterEntry(Sequence):
    sequenceElements = [
     ...
     Element('status', RouterEntryStatus)]
...
class NameValue(Sequence):
    ...
    def __init__(self, name=None, value=None):
        if _debug:
            NameValue._debug('__init__ name=%r value=%r', name, value)
        self.name = name
        self.value = None
        if value is None:
            pass
    ...
    def encode(self, taglist):
        if _debug:
            NameValue._debug('(%r)encode %r', self.__class__.__name__, taglist)
        tag = Tag()
    ...
    def decode(self, taglist):
        if _debug:
            NameValue._debug('(%r)decode %r', self.__class__.__name__, taglist)
        self.name = None
        self.value = None
        tag = taglist.Peek()
        if _debug:
        ...
        if tag and tag.tagClass == Tag.applicationTagClass:
            if tag.tagNumber == Tag.dateAppTag and len(taglist.tagList) >= 2:
                ...
                if next_tag.tagClass == Tag.applicationTagClass and next_tag.tagNumber == Tag.timeAppTag:
                    if _debug:
                        NameValue._debug('    - remaining tag list 0: %r', taglist.tagList)
                    self.value = DateTime()
                    self.value.decode(taglist)
                    if _debug:
                        NameValue._debug('    - date time value: %r', self.value)
...
class DeviceAddress(Sequence):
    sequenceElements = [Element('networkNumber', Unsigned),
     Element('macAddress', OctetString)]
