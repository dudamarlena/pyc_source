    ...
    def __new__(cls, class_name, bases, class_dict):
        if bases != (object,):
            if 'key' in class_dict:
                raise ValueError("You cannot override the 'key' when subclassing Field")
            key = class_dict.get('KEY', class_name)
            if key is not None and Field._key_index_sep in key:
                raise ValueError('The key %r is not valid: should not contain %r.' % (
                 key, Field._key_index_sep))
            reserved_keywords = ('verbose', 'debug')
            if key in reserved_keywords:
                raise ValueError('The key %r is not valid: it is one of the reserved keywords %r.' % (
...
class Field(object):
    ...
    def set_config(self, config, allowed_index=None):
        ...
        try:
            ...
            if allowed_index is None:
                raise FieldConfigError('Could not find %r in config %r' % (
                 key, config))
        else:
            return set([key])
        ...
        try:
            ...
            raise FieldConfigError('Could not find %r or %r in config %r' % (
             self.key, key, config))
        else:
            return set([key])
    ...
    def validate(self):
        for c in self.forbidden_chars():
            if c in self._value:
                raise FieldValueError('Value %r is not invalid for %r: forbidden character %s found.' % (
