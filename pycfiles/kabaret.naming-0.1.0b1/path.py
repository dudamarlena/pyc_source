...
# Compiled at: 2013-02-13 10:45:41
"""
...
class PathItem(object):
    ...
    class PathConfigError(PathValueError):
        ...
        def __init__(self, path_item, config, messages):
            super(PathItem.PathConfigError, self).__init__('Unable to get %r under %r (%s). Error(s): \n  ' % (
    ...
    class PathChildError(PathValueError):
        ...
        def __init__(self, path_item, value, errors):
            super(PathItem.PathChildError, self).__init__('Unable to get %r under %r (%s). Error(s): \n  ' % (
    ...
    def __call__(self, **config):
        ...
        for (k, v) in root_config.items():
            if config.get(k) not in (None, v):
                raise self.PathConfigError(next, config, [
        ...
        if remaining_keys:
            raise self.PathConfigError(item, config, [
    ...
    def validate(self, leaf_config):
        ...
        for (key, local_value) in config.items():
            ...
            if leaf_value != local_value:
                raise PathItem.PathValidationError('Invalid name %r for %r (%s): %s should be %r and not %r' % (
    ...
    def get_keys_for(cls, path_item_type_name, up_keys=[]):
        ...
        if cls.__name__ == path_item_type_name:
            seen = set()
            seen_add = seen.add
            return [ x for x in here_keys if x not in seen if not seen_add(x) ]
        else:
...
class FileOrFolder(PathItem):
    ...
    def _set_parent(self, parent):
        ...
        self._name_field.key_override = self._name_field.make_indexed_key(self._name_field.key, self._get_depth())


FileOrFolder.CHILD_CLASSES = (
 FileOrFolder,)
