...
class IndexingField(Field):
    ...
    def validate(self):
        ...
        if not self._value.startswith(self.prefix):
            raise FieldValueError('Value %r is invalid for %r: missing prefix %r.' % (
             self._value, self.key, self.prefix))
        rest = self._value[len(self.prefix):]
        if not rest.isdigit():
            raise FieldValueError('Value %r is not invalid for %r: index %r is not digit.' % (
        ...
        if len(rest) != paddlen and not len(striped) > paddlen:
            raise FieldValueError('Value %r is not invalid for %r: bad padding len (should be %r)' % (
