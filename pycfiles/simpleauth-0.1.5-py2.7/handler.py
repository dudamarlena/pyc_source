...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-intel/egg/simpleauth/handler.py
# Compiled at: 2014-10-02 09:45:01
import os, sys, logging, json
from urllib import urlencode
import urlparse, time, base64
try:
    import json
except ImportError:
    try:
...
class SimpleAuthHandler(object):
  ...
  """
    PROVIDERS = {'google': (
       ...
       'openid': ('openid', None)}
    TOKEN_RESPONSE_PARSERS = {'google': '_json_parser', 
    ...
    def _oauth2_callback(self, provider, access_token_url):
        ...
        if self.OAUTH2_CSRF_STATE:
            ...
            if not self._validate_csrf_token(_expected, _actual):
                raise InvalidCSRFTokenError('[%s] vs [%s]' % (_expected, _actual), provider)
        ...
        resp = urlfetch.fetch(url=access_token_url, payload=urlencode(payload), method=urlfetch.POST, headers={'Content-Type': 'application/x-www-form-urlencoded'})
    ...
    def _oauth1_init(self, provider, auth_urls, extra=None):
        ...
        if resp.status != 200:
            raise AuthProviderResponseError('%s (status: %d)' % (content, resp.status), provider)
        request_token = _parser(content)
        if not request_token.get('oauth_token', None):
            raise AuthProviderResponseError("Couldn't get a request token from %s" % str(request_token), provider)
        target_url = auth_urls['auth'].format(urlencode({'oauth_token': request_token.get('oauth_token', None), 
    ...
    def _oauth1_callback(self, provider, access_token_url):
        ...
        if not request_token:
            raise InvalidOAuthRequestToken('No request token in user session', provider)
        verifier = self.request.get('oauth_verifier')
        if not verifier:
            raise AuthProviderResponseError('No OAuth verifier was provided', provider)
    ...
    def _openid_init(self, provider='openid', identity=None, extra=None):
        ...
        callback_url = self._callback_uri_for(provider)
        target_url = users.create_login_url(dest_url=callback_url, federated_identity=identity_url)
        logging.debug('Redirecting user to %s', target_url)
        self.redirect(target_url)
    ...
    """
        ...
        uinfo = {'id': user.federated_identity(), 
           'nickname': user.nickname(), 
           'email': user.email()}
        return (
         uinfo, {'provider': user.federated_provider()})
    ...
    """
        logging.warn('Google userinfo endpoint is deprecated. Use Google+ API (googleplus provider): https://developers.google.com/+/api/auth-migration#timetable')
    ...
    """
        resp = self._oauth2_request('https://www.googleapis.com/plus/v1/people/me?{0}', auth_info['access_token'])
    ...
    """
        resp = self._oauth2_request('https://apis.live.net/v5.0/me?{0}', auth_info['access_token'])
    ...
    """
        resp = self._oauth2_request('https://graph.facebook.com/me?{0}', auth_info['access_token'])
        return json.loads(resp)
    ...
    """
        resp = self._oauth2_request('https://api.foursquare.com/v2/users/self?{0}&v=20130204', auth_info['access_token'], 'oauth_token')
        data = json.loads(resp)
    ...
    """
        logging.warn('LinkedIn OAuth 1.0a is deprecated. Use LinkedIn with OAuth 2.0: https://developer.linkedin.com/documents/authentication')
    ...
    """
        ...
        resp = self._oauth2_request(url, auth_info['access_token'], token_param='oauth2_access_token')
    ...
    def _oauth1_client(self, token=None, consumer_key=None, consumer_secret=None):
        ...
        args = [
         oauth1.Consumer(key=consumer_key, secret=consumer_secret)]
        if token:
            args.append(token)
