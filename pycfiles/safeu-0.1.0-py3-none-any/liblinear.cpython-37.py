...
# [GCC 8.4.0]
# Embedded file name: D:\文档\python\SafeU\safeu\libs\liblinear.py
...
try:
    ...
    if sys.platform == 'win32':
        liblinear = CDLL(path.join(dirname, 'liblinear_windows\\liblinear.dll'))
    else:
        liblinear = CDLL(path.join(dirname, 'liblinear_linux/liblinear.so.3'))
except:
    if find_library('linear'):
        liblinear = CDLL(find_library('linear'))
    else:
        if find_library('liblinear'):
...
class problem(Structure):
    _names = [
     'l', 'n', 'y', 'x', 'bias', 'W']
    _types = [c_int, c_int, POINTER(c_double),
     POINTER(POINTER(feature_node)), c_double, POINTER(c_double)]
    _fields_ = genFields(_names, _types)
...
class parameter(Structure):
    _names = [
     'solver_type', 'eps', 'C', 'nr_weight',
     'weight_label', 'weight', 'p', 'init_sol']
    _types = [c_int, c_double, c_double, c_int,
     POINTER(c_int), POINTER(c_double), c_double, POINTER(c_double)]
    _fields_ = genFields(_names, _types)
    ...
    def parse_options(self, options):
        ...
        if self.flag_find_C:
            if not self.flag_cross_validation:
                self.nr_fold = 5
            self.solver_type = self.flag_solver_specified or L2R_L2LOSS_SVC
            self.flag_solver_specified = True
        else:
            if self.solver_type not in [L2R_LR, L2R_L2LOSS_SVC]:
        ...
        if self.eps == float('inf'):
            if self.solver_type in [L2R_LR, L2R_L2LOSS_SVC]:
                ...
                if self.solver_type in [L2R_L2LOSS_SVR]:
                    self.eps = 0.001
                else:
                    if self.solver_type in [L2R_L2LOSS_SVC_DUAL, L2R_L1LOSS_SVC_DUAL, MCSVM_CS, L2R_LR_DUAL]:
                        self.eps = 0.1
                    else:
                        if self.solver_type in [L1R_L2LOSS_SVC, L1R_LR]:
                            self.eps = 0.01
                        else:
                            if self.solver_type in [L2R_L2LOSS_SVR_DUAL, L2R_L1LOSS_SVR_DUAL]:
                                self.eps = 0.1


class model(Structure):
    _names = [
     'param', 'nr_class', 'nr_feature', 'w', 'label', 'bias']
    ...
    def __del__(self):
        if hasattr(self, '__createfrom__'):
            if self.__createfrom__ == 'C':
...
fillprototype(liblinear.train, POINTER(model), [
 POINTER(problem), POINTER(parameter)])
fillprototype(liblinear.find_parameter_C, None, [POINTER(problem),
 POINTER(parameter), c_int, c_double, c_double, POINTER(c_double), POINTER(c_double)])
...
fillprototype(liblinear.predict, c_double, [
 POINTER(model), POINTER(feature_node)])
...
fillprototype(liblinear.get_decfun_coef, c_double, [
 POINTER(model), c_int, c_int])
