...
# [GCC 8.4.0]
# Embedded file name: D:\文档\python\SafeU\safeu\libs\svm.py
# Compiled at: 2019-05-29 14:10:34
# Size of source mod 2**32: 14653 bytes
from ctypes import *
...
try:
    ...
    if sys.platform == 'win32':
        libsvm = CDLL(path.join(dirname, 'libsvm_windows\\libsvm.dll'))
    else:
        libsvm = CDLL(path.join(dirname, 'libsvm_linux/libsvm.so.2'))
except:
    if find_library('svm'):
...
def gen_svm_nodearray(xi, feature_max=None, isKernel=False):
    if feature_max:
        if not isinstance(feature_max, int):
            ...
            if scipy and isinstance(xi, tuple) and len(xi) == 2 and isinstance(xi[0], scipy.ndarray) and isinstance(xi[1], scipy.ndarray):
                ...
                if scipy and isinstance(xi, scipy.ndarray):
                    if not isKernel:
                        xi_shift = 1
                        index_range = xi.nonzero()[0] + 1
                    else:
                        index_range = scipy.arange(0, len(xi))
                    if feature_max:
                        index_range = index_range[scipy.where(index_range <= feature_max)]
                else:
                    if isinstance(xi, (dict, list, tuple)):
                        if isinstance(xi, dict):
                            ...
                            if isinstance(xi, (list, tuple)):
                                if not isKernel:
                                    ...
                                    index_range = range(0, len(xi))
                            else:
                                if feature_max:
                                    index_range = filter(lambda j: j <= feature_max, index_range)
                                index_range = isKernel or filter(lambda j: xi[(j - xi_shift)] != 0, index_range)
...
class svm_problem(Structure):
    ...
    _types = [c_int, POINTER(c_double),
     POINTER(POINTER(svm_node)), POINTER(c_double)]
    _fields_ = genFields(_names, _types)

    def __init__(self, y, x, upbound, isKernel=False):
        if not isinstance(y, (list, tuple)):
            raise scipy and isinstance(y, scipy.ndarray) or TypeError('type of y: {0} is not supported!'.format(type(y)))
        ...
        self.upbound = (c_double * l)()
        if scipy != None and isinstance(upbound, scipy.ndarray):
            scipy.ctypeslib.as_array(self.upbound, (self.l,))[:] = upbound
        else:
            for i, ui in enumerate(y):
                self.upbound[i] = ui
...
class svm_model(Structure):
    _names = [
     'param', 'nr_class', 'l', 'SV', 'sv_coef', 'rho',
     'probA', 'probB', 'sv_indices', 'obj', 'label', 'nSV', 'free_sv']
    _types = [svm_parameter, c_int, c_int, POINTER(POINTER(svm_node)),
     POINTER(POINTER(c_double)), POINTER(c_double),
     POINTER(c_double), POINTER(c_double), POINTER(c_int),
     c_double, POINTER(c_int), POINTER(c_int), c_int]
    _fields_ = genFields(_names, _types)
    ...
    def get_sv_coef(self):
        return [tuple((self.sv_coef[j][i] for j in range(self.nr_class - 1))) for i in range(self.l)]
