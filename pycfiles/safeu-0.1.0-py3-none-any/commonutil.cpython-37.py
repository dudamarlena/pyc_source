...
# [GCC 8.4.0]
# Embedded file name: D:\文档\python\SafeU\safeu\libs\commonutil.py
...
__all__ = [
 'svm_read_problem', 'evaluations',
 'csr_find_scale_param', 'csr_scale']
...
def csr_find_scale_param(x, lower=-1, upper=1):
    ...
    if sum(offset != 0) * l > 3 * x.getnnz():
        print('WARNING: The #nonzeros of the scaled data is at least 2 times larger than the original one.\nIf feature values are non-negative and sparse, set lower=0 rather than the default lower=-1.',
          file=(sys.stderr))
...
def csr_scale(x, scale_param):
    if not isinstance(x, sparse.csr_matrix):
        ...
        l, n = x.shape
        n == len(coef) or print('WARNING: The dimension of scaling parameters and feature number do not match.', file=(sys.stderr))
        coef = resize(coef, n)
    ...
    if scaled_x.getnnz() > x.getnnz():
        print(('WARNING: original #nonzeros %d\n' % x.getnnz() + '       > new      #nonzeros %d\n' % scaled_x.getnnz() + 'If feature values are non-negative and sparse, get scale_param by setting lower=0 rather than the default lower=-1.'),
          file=(sys.stderr))