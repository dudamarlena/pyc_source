...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/app/admin.py
# Compiled at: 2008-06-27 12:03:50
import datetime, optparse, os, pkg_resources, sys, zipfile
...
class AdminRunner(object):
    ...
    def console_run(self, cmdargs=sys.argv[1:]):
        ...
        parser.add_option('-w', '--workingdir', action='store', dest='workingdir', help='working directory, must contain base of server')
        (options, args) = parser.parse_args(args=cmdargs)
        workingdir = getattr(options, 'workingdir', None) or getattr(self, 'workingdir', None)
        if not workingdir:
            self.logger.error('Working directory must be specified, please use the -w option')
            return 1
        ...
        if cmd == 'create':
            ...
            pmanager = self.project_manager_factory(cluemapperconfig, workingdir)
            try:
    ...
    def cmd_upgrade(self, workingdir):
        ...
        if not authz:
            pb = utils.PathBuilder(workingdir)
            authz = pb(os.path.join('etc', 'cluemapper', 'svnauthzfile.conf'))
        ...
        for sectionname in cluemapperconfig.keys():
            ...
            if 'svnrepo' in section:
                del section['svnrepo']
            for check in ('tracenv', 'tracenv_base', 'svnrepo_base', 'auth_file'):
                if check in section:
        ...
        for prj in pmanager.projects:
            ...
            tracconf['user_manager']['attribute_provider'] = 'ClueMapperAttributeProvider'
    ...
    def cmd_import(self, workingdir, args):
        ...
        if targetprjid in [ x.prjid for x in pmanager.projects ]:
            raise AdminError('Project with prjid "%s" already exists' % targetprjid)
        ...
        pb = utils.PathBuilder(workingdir)
        tracenv = os.path.join(pb(cluemapperconfig['base']['tracenv_base']), targetprjid)
        svnrepo = os.path.join(pb(cluemapperconfig['base']['svnrepo_base']), targetprjid)
        ...
        c['account-manager']['password_file'] = pb(cluemapperconfig['base']['auth_file'])
        c['cluemapper']['database'] = 'sqlite:' + os.path.join(workingdir, 'etc', 'cluemapper', 'cluemapper.db')
        c['cluemapper']['svnauthzfile'] = pb(cluemapperconfig['base']['svnauthzfile'])
        c.save()
    ...
    def cmd_export(self, workingdir, args):
        ...
        for project in projects:
            ...
            z.writestr(os.path.join(project.prjid, 'cluemapper-project.ini'), prjconfig.pretty_string())
