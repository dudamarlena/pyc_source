...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/app/tracplugins/sourcecontrol.py
...
class SourceControl(traccore.Component):
    svnauthzfile = tracconfig.Option('cluemapper', 'svnauthzfile', 'svnauthzfile.conf', 'Subversion authz file')
    ...
    def get_users_with_access(self):
        if not self.svnauthzfile:
            raise NoAccessFileError('No cluemapper.svnauthzfile property has been set')
        if not os.path.exists(self.svnauthzfile):
            raise NoAccessFileError('The svnauthzfile file "%s" does not exist' % self.svnauthzfile)
        authz = clueconfig.Config(self.svnauthzfile)
        acl = authz[(self.prjid + ':/')]
        return ((username,) + interpret_acl(access) for (username, access) in acl.items() if not username.startswith('@'))
    ...
    def update_multiple_access(self, users):
        ...
        for (username, read, write) in users:
            updated = self._update_access(authz, username, read, write) or updated
...
class SourceControlAdminPanel(traccore.Component):
    ...
    def apply_adduser(self, req):
        self.sourcecontrol.update_access(req.args['username'], bool(req.args.get('read', False)), bool(req.args.get('write', False)))
    ...
    def render_admin_panel(self, req, cat, page, path_info):
        ...
        data = {'existing_users': sorted(existing, name_cmp), 'noaccess_users': sorted(noaccess, name_cmp), 
           'message': message}
        return (
         'admin-sourcecontrol.html', data)
    ...
    def get_user_access(self):
        ...
        for (username, access) in users.items():
            ...
            if username == '*':
                ...
                if name:
                    fullname = '%s (%s)' % (_u(name), _u(username))
            (read, write) = access
            yield {'username': username, 'fullname': fullname, 
               'read': read or None, 
               'write': write or None}
