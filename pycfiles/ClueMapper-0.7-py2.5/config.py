...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/app/config.py
# Compiled at: 2008-06-27 12:03:50
import StringIO, ConfigParser, md5, os
...
class OrderedConfigParser(ConfigParser.RawConfigParser):
    ...
    def write(self, fp):
        ...
        if self.comparator is not None:
            sortedkeys = sorted(self._sections, self.comparator)
        else:
            sortedkeys = sorted(self._sections)
...
class Config(object):
    r"""A configuration helper for dealing with ini style files.  Makes
    ...
    def _file_changed(self):
        if self._digest is None:
            raise ConfigError('Configuration data has not yet been read for %r' % self.filename)
    ...
    def gen_config_parser(self):
        parser = OrderedConfigParser()
        for section in sorted(self._data):
            subdict = self._data[section]
    ...
    def save(self):
        if not self.filename:
            raise ConfigError('Cannot save file since filename was never specified')
        if os.path.exists(self.filename) and self._file_changed:
            raise ConfigError('Configuration data on the filesystem has changed for %r, save aborted' % self.filename)
        parser = self.gen_config_parser()
...
DEFAULT_CM_CONFIG = {'base': {'svnrepo_base': '', 
            'tracenv_base': '', 
            'auth_file': ''}, 
   'base:permissions': {}}
...
def load_config(workingdir):
    ...
    config = ensure_cm_config(pb(os.path.join('etc', 'cluemapper', 'cluemapper.ini')))
    changed = False
    if not config['base'].get('svnrepo_base', None):
        config['base']['svnrepo_base'] = pb(os.path.join('var', 'cluemapper', 'svnrepos'))
        changed = True
    if not config['base'].get('tracenv_base', None):
        config['base']['tracenv_base'] = pb(os.path.join('var', 'cluemapper', 'tracenvs'))
        changed = True
    if not config['base'].get('svnauthzfile', None):
        authz = pb(os.path.join('etc', 'cluemapper', 'svnauthzfile.conf'))
        config['base']['svnauthzfile'] = authz
        changed = True
        if not os.path.exists(authz):
            f = open(authz, 'w')
            f.close()
    if not config['base'].get('auth_file', None):
        rel = os.path.join('etc', 'cluemapper', 'cluemapper.passwd')
