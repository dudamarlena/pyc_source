...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/clue/app/patches.py
...
def patch_deliverance():
    ...
    if hasattr(htmlserialize.decodeAndParseHTML, '_cluemapper_patched'):
        return False
    htmlserialize.decodeAndParseHTML._cluemapper_patched = htmlserialize.decodeAndParseHTML.func_code
    htmlserialize.decodeAndParseHTML.func_code = deliver_decodeAndParseHTML.func_code
    return True
...
def unpatch_deliverance():
    ...
    if not hasattr(htmlserialize.decodeAndParseHTML, '_cluemapper_patched'):
        return False
    htmlserialize.decodeAndParseHTML.func_code = htmlserialize.decodeAndParseHTML._cluemapper_patched
...
def trac_render_preference_panel(self, req, panel):
    ...
    kwargs = {'settings': {'session': req.session, 'session_id': req.session.sid}, 
       'timezones': web_ui.all_timezones, 
       'timezone': web_ui.get_timezone}
    return (
     'prefs_%s.html' % (panel or 'datetime'), kwargs)
...
def patch_trac_prefs():
    ...
    """
    from trac.prefs import web_ui
    if hasattr(web_ui.PreferencesModule, '_cluemapper_get_pref_panels_patched'):
        return False
    web_ui.PreferencesModule._cluemapper_get_pref_panels_patched = web_ui.PreferencesModule.get_preference_panels
    web_ui.PreferencesModule.get_preference_panels = trac_get_preference_panels
    web_ui.PreferencesModule._cluemapper_render_pref_patched = web_ui.PreferencesModule.render_preference_panel
    web_ui.PreferencesModule.render_preference_panel = trac_render_preference_panel
    return True
...
def unpatch_trac_prefs():
    """Remove trac patch."""
    from trac.prefs import web_ui
    if not hasattr(web_ui.PreferencesModule, '_cluemapper_get_pref_panels_patched'):
        return False
    web_ui.PreferencesModule.get_preference_panels = web_ui.PreferencesModule._cluemapper_get_pref_panels_patched
    del web_ui.PreferencesModule._cluemapper_get_pref_panels_patched
    web_ui.PreferencesModule.render_preference_panel = web_ui.PreferencesModule._cluemapper_render_pref_patched
