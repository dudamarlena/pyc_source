...
# [GCC 8.4.0]
# Embedded file name: /Users/ctyfoxylos/personal/python/abnamrolib/abnamrolib/abnamroics.py
...
class CreditCard(Comparable):
    __doc__ = 'Models a credit card account.'
    ...
    def get_period(self, year, month):
        ...
        """
        return next((period for period in self.periods if period.period == f"{year}-{month.zfill(2)}"), None)
    ...
    def get_current_period_transactions(self):
        ...
        params = {'accountNumber':self.number,  'flushCache':True}
        response = self._contract.session.get(url, params=params)
        if not response.ok:
            self._logger.error('Error retrieving transactions for account "%s"response was : %s with status code : %s', self.number, response.text, response.status_code)
            return []
    ...
    def periods(self):
        ...
        if self._periods is None:
            ...
            response = self._contract.session.get(url, params=params)
            if not response.ok:
                self._logger.error('Error retrieving periods for account "%s"response was : %s with status code : %s', self.number, response.text, response.status_code)
                return []
            self._periods = [Period(self._contract, self, data) for data in response.json()]
        return self._periods


class Period:
    __doc__ = 'Models the payment period.'
    ...
    def transactions(self):
        ...
        if self._transactions is None:
            url = f"{self._contract.base_url}/sec/nl/sec/transactions"
            params = {'accountNumber':self._account.number,  'flushCache':True, 
             'fromPeriod':self.period, 
             'untilPeriod':self.period}
            response = self._contract.session.get(url, params=params)
            if not response.ok:
                self._logger.error('Error retrieving transactions for account "%s", response was : %s with status code : %s', self._account.number, response.text, response.status_code)
                return []
...
class CreditCardTransaction(Transaction):
    __doc__ = 'Models a credit card transaction.'
...
class CreditCardContract(Contract, CookieAuthenticator):
    ...
    def accounts(self):
        ...
        if self._accounts is None:
            ...
            if not response.ok:
                ...
                return []
            self._accounts = [CreditCard(self, self._get_account_data(data.get('accountNumber'))) for data in response.json()]
    ...
    def get_account(self, id_=None):
        ...
        """
        if id_:
            return self.get_account_by_number(id_)
    ...
    def get_account_by_number(self, account_number):
        ...
        """
        return next((account for account in self.accounts if str(account.number) == str(account_number)), None)
