...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/alembicmanager/widgets/maya/alembiceimporter.py
...
import os, json, logging, tpDcc as tp, artellapipe.register
...
class MayaAlembicImporter(alembicimporter.AlembicImporter, object):
    ...
    def import_alembic(cls, project, alembic_path, parent=None, fix_path=False):
        ...
        if not alembic_path or not os.path.isfile(alembic_path):
            ...
            valid_import = alembic.import_alembic(project,
              alembic_path, mode='import', nodes=None, parent=parent, fix_path=fix_path)
            if not valid_import:
                return
    ...
    def reference_alembic(project, alembic_path, namespace=None, fix_path=False):
        ...
        :param fix_path: bool
        """
        res = alembicimporter.AlembicImporter.reference_alembic(project=project,
    ...
    def _on_import_alembic(self, as_reference=False):
        ...
        if self._auto_smooth_display.isChecked():
            if reference_nodes:
                if type(reference_nodes) in [list, tuple]:
