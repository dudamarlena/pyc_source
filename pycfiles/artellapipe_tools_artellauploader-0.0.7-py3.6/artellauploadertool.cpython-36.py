...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/artellauploader/widgets/artellauploadertool.py
# Compiled at: 2020-04-15 10:43:54
# Size of source mod 2**32: 20776 bytes
"""
...
class ArtellaUploader(artellapipe.ToolWidget, object):
            ...
            try:
                ...
                if not_updated_items:
                    if not skip_update:
                        ...
                        result = qtutils.show_question(None, 'Some files are not synced', 'Following files are not synced:\n\n {}\n\nDo you want to sync them?'.format('\n'.join(item_paths)))
    ...
    def _on_browse(self):
        ...
        export_path = tp.Dcc.select_folder_dialog(title='Select Root Path',
            ...
            if not os.path.isfile(item.path):
                ...
                if not item.is_valid:
                    ...
                    if is_updated is not None:
                        self.show_warning_message('Item "{}" is not ready to be uploaded. Lock it or sync it first!'.format(item.text(1)))
                        continue
        ...
        if not items_to_upload:
            ...
            if res and comment:
                ...
                for i, item in enumerate(items_to_upload):
                    ...
                    self.repaint()
                    valid_upload = artellapipe.FilesMgr().upload_working_version((item.path),
                      skip_saving=True, notify=False, comment=comment)
                    if valid_upload:
                        self.show_ok_message('{} ({}) uploaded successfully to Artella server!'.format(item.text(1), item.text(3)))
                    else:
                        self.show_error_message('Error while uploaded {} ({}) to Artella server!'.format(item.text(1), item.text(3)))

                self._progress.setValue(0)
                self._progress_lbl.setText('')
                self._progress.setVisible(False)
                self._project.tray.show_message(title='New Working Versions',
                  msg='New versions uploaded to Artella server successfully!')
                self.repaint()
            if valid_upload:
                self._update_items()
            return valid_upload