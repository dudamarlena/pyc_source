...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pywoudc/__init__.py
# Compiled at: 2019-04-22 12:11:27
__version__ = '0.2.0'
...
class WoudcClient(object):
    ...
    def __init__(self, url='https://geo.woudc.org/ows', timeout=30):
        ...
        self.server = WebFeatureService(self.url, '1.1.0', timeout=self.timeout)
        try:
    ...
    def get_data(self, typename, **kwargs):
        ...
        if temporal is not None:
            ...
            constraints.append(fes.PropertyIsBetween('instance_datetime', temporal_start, temporal_end))
        if sort_order not in ('asc', 'desc'):
            ...
            if constraints:
                ...
                if len(constraints) == 1:
                    ...
                    filter_string = flt.setConstraint(constraints[0], tostring=True)
                if len(constraints) > 1:
                    LOGGER.debug('Multiple constraints')
                    filter_string = flt.setConstraintList([constraints], tostring=True)
            ...
            while True:
                LOGGER.debug('Fetching features %d - %d', startindex, startindex + self.maxfeatures)
                payload = self.server.getfeature(typename=typename, startindex=startindex, propertyname=variables, maxfeatures=self.maxfeatures, filter=filter_string, outputFormat=self.outputformat).read()
        ...
        if sort_property is not None:
            ...
            feature_collection['features'].sort(key=lambda e: e['properties'][sort_property], reverse=sort_descending)
    ...
    def _get_metadata(self, typename, raw=False):
        ...
        features = self.server.getfeature(typename=typename, outputFormat=self.outputformat)
        LOGGER.debug('Processing response')
        if raw:
