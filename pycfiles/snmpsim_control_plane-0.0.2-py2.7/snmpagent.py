...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/metrics/importers/snmpagent.py
        ...
        for transport_endpoint, peers in transport_endpoints.items():
            ...
            for peer_adddress, engines in peers.items():
                if not isinstance(engines, dict):
                    continue
                transport_model = models.Transport(transport_protocol=transport_protocol, endpoint=transport_endpoint, peer=peer_adddress)
                transport_model = db.session.merge(transport_model)
                autoincrement(transport_model, models.Transport)
                packet_model = models.Packet(transport_id=transport_model.id)
                                ...
                                for context_name, pdus in context_names.items():
                                    ...
                                    for pdu_type, recordings in pdus.items():
                                        ...
                                        for recording, counters in recordings.items():
                                            agent_model = models.Agent(transport_id=transport_model.id, engine=engine_id, security_model=security_model, security_level=security_level, context_engine=context_engine_id, context_name=context_name)
                                            agent_model = db.session.merge(agent_model)
                                            autoincrement(agent_model, models.Agent)
                                            recording_model = models.Recording(agent_id=agent_model.id, path=recording)
                                            recording_model = db.session.merge(recording_model)
                                            autoincrement(recording_model, models.Recording)
                                            pdu_model = models.Pdu(recording_id=recording_model.id, name=pdu_type)
                                            ...
                                            for name, counters in variations.items():
                                                variation_model = models.Variation(pdu_id=pdu_model.id, name=name)
