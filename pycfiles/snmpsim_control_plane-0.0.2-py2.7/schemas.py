...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/metrics/schemas.py
# Compiled at: 2020-01-30 12:14:23
import marshmallow, flask
...
class PacketsSchema(marshmallow.Schema, EnsureZeroMixIn):
    ...
    class Meta:
        fields = ('total', 'parse_failures', 'auth_failures', 'context_failures')
...
class ConsoleSchema(ma.ModelSchema):
    ...
    class ProcessSchema(ma.ModelSchema):
        ...
        _links = ma.Hyperlinks({'self': ma.URLFor('show_processes', id='<id>')})

    process = ma.Nested(ProcessSchema)
    _links = ma.Hyperlinks({'self': ma.URLFor('show_console', id='<process_id>', page_id='<id>'), 
...
class EndpointSchema(ma.ModelSchema):
    ...
    process = ma.Nested(ProcessSchema)
    _links = ma.Hyperlinks({'self': ma.URLFor('show_endpoints', id='<process_id>', endpoint_id='<id>'), 
...
class ProcessSchema(ma.ModelSchema):
    ...
    class Meta:
        model = models.Process
        fields = ('id', 'path', 'runtime', 'memory', 'cpu', 'files', 'exits', 'changes',
                  'last_update', 'update_interval', 'endpoints', 'supervisor', 'console_pages',
                  '_links')
    ...
    class EndpointsSchema(ma.ModelSchema):
        ...
        def get_links(self, endpoints):
            if endpoints:
                return {'self': flask.url_for('show_endpoints', id=endpoints[0].process_id)}
            else:
                return {}
    ...
    class ConsoleSchema(ma.ModelSchema):
        ...
        def get_links(self, console_pages):
            if console_pages:
                return {'self': flask.url_for('show_console', id=console_pages[0].process_id)}
            else:
                return {}
    ...
    class SupervisorSchema(ma.ModelSchema):
        ...
        _links = ma.Hyperlinks({'self': ma.URLFor('show_supervisors', id='<id>'), 
...
class SupervisorSchema(ma.ModelSchema):
    ...
    class Meta:
        model = models.Supervisor
        fields = ('id', 'hostname', 'watch_dir', 'started', 'processes', '_links')
