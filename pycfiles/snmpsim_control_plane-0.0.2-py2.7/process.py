...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/metrics/importers/process.py
...
def import_metrics(jsondoc):
    ...
    timestamp = datetime.datetime.utcfromtimestamp(jsondoc['started'])
    supervisor_model = models.Supervisor(hostname=jsondoc['host'], watch_dir=jsondoc['watch_dir'], started=timestamp)
    ...
    for executable in jsondoc['executables']:
        process_model = models.Process(path=executable['executable'], supervisor_id=supervisor_model.id)
        ...
        process_model.update_interval = jsondoc['last_update'] - jsondoc['first_update']
        timestamp = datetime.datetime.utcfromtimestamp(jsondoc['last_update'])
        process_model.last_update = timestamp
        query = models.Endpoint.query.filter_by(process_id=process_model.id)
        existing_endpoints = set((x.protocol, x.address) for x in query.all())
        ...
        for protocol, address in new_endpoints:
            endpoint_model = models.Endpoint(protocol=protocol, address=address, process_id=process_model.id)
        ...
        for protocol, address in removed_endpoints:
            query = db.session.query(models.Endpoint).filter_by(protocol=protocol).filter_by(address=address).filter_by(process_id=process_model.id)
            query.delete()

        query = db.session.query(models.ConsolePage).filter(models.ConsolePage.process_id == process_model.id).filter(models.ConsolePage.timestamp < old_times)
        query.delete()
        for console_page in executable['console']:
            timestamp = datetime.datetime.utcfromtimestamp(console_page['timestamp'])
            console_page_model = models.ConsolePage(timestamp=timestamp, text=console_page['text'], process_id=process_model.id)
