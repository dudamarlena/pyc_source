...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/supervisor/reporting/formats/jsondoc.py
# Compiled at: 2020-01-30 12:14:23
import json, os, socket, tempfile, uuid
...
class JsonDocReporter(base.BaseReporter):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not args:
            raise error.ControlPlaneError('Missing %s parameter(s). Expected: <method>:<reports-dir>[:dumping-period]' % self.__class__.__name__)
        self._reports_dir = os.path.join(args[0], self.REPORTING_FORMAT)
        if len(args) > 1:
            try:
                self.REPORTING_PERIOD = int(args[1])
            except Exception as exc:
                raise error.ControlPlaneError('Malformed reports dumping period: %s' % args[1])

        try:
            if not os.path.exists(self._reports_dir):
                os.makedirs(self._reports_dir)
        except OSError as exc:
            raise error.ControlPlaneError('Failed to create reports directory %s: %s' % (
             self._reports_dir, exc))

        log.debug('Initialized %s metrics reporter for instance %s, metrics directory %s' % (
         self.__class__.__name__, self.PRODUCER_UUID, self._reports_dir))

    @staticmethod
    def _json_serializer(obj):
        if isinstance(obj, lifecycle.ConsoleLog):
            return [ {'page': page, 'text': obj.text(page), 'timestamp': obj.timestamp(page)} for page in range(obj.first_page, obj.last_page + 1)
                   ]
        return obj
    ...
    def dump_metrics(self, metrics, watch_dir=None, started=None, begin=None, end=None):
        ...
        try:
            json_doc = json.dumps(json_metrics, indent=2, default=self._json_serializer)
            with tempfile.NamedTemporaryFile(delete=False) as (fl):
                fl.write(json_doc.encode('utf-8'))
            os.rename(fl.name, dump_path)
        except Exception as exc:
            log.error('Failure while dumping metrics into %s: %s' % (
             dump_path, exc))
