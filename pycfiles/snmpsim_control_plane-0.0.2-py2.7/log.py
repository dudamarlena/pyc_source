...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/log.py
# Compiled at: 2020-01-01 13:42:52
import logging, os, socket, stat, sys, time
from logging import handlers
from snmpsim_control_plane.error import ControlPlaneError
LOG_DEBUG = 0
LOG_INFO = 1
LOG_ERROR = 2
...
class SyslogLogger(AbstractLogger):
    ...
    def init(self, *priv):
        if len(priv) < 1:
            raise ControlPlaneError('Bad syslog params, need at least facility, also accept host, port, socktype (tcp|udp)')
        if len(priv) < 2:
            priv = [
             priv[0], 'debug']
        if len(priv) < 3:
        ...
        try:
            handler = handlers.SysLogHandler(address=priv[2].startswith('/') and priv[2] or (
             priv[2], int(priv[3])), facility=priv[0].lower(), socktype=len(priv) > 4 and priv[4] == 'tcp' and socket.SOCK_STREAM or socket.SOCK_DGRAM)
        except Exception as exc:
            raise ControlPlaneError('Bad syslog option(s): %s' % exc)
        ...
        def doRollover(self):
            try:
                ...
                if not self._failure:
                    self._failure = True
                    error('Failed to rotate log/timestamp file %s: %s' % (
                     self._filename, exc))

    def init(self, *priv):
        if not priv:
            raise ControlPlaneError('Bad log file params, need filename')
        ...
        if len(priv) > 1 and priv[1]:
            try:
                ...
                raise ControlPlaneError('Error in timed log rotation specification. Use <NNN>k,m,g for size or <NNN>S,M,H,D for time limits')
        ...
        try:
            ...
            raise ControlPlaneError('Failure configure logging: %s' % exc)

        handler.setFormatter(logging.Formatter('%(message)s'))
        self._logger.addHandler(handler)
        self('Log file %s, rotation rules: %s' % (
         priv[0],
         maxsize and '> %sKB' % (maxsize / 1024) or maxage and '%s%s' % (maxage[1], maxage[0]) or '<none>'))
        return

    def __call__(self, s):
        now = time.time()
        timestamp = time.strftime('%Y-%m-%dT%H:%M:%S', time.localtime(now))
        AbstractLogger.__call__(self, '%s.%02d %s: %s' % (
...
class StreamLogger(AbstractLogger):
    ...
    def init(self, *priv):
        try:
            ...
            raise ControlPlaneError('Stream logger failure: %s' % exc)
...
METHODS_MAP = {'syslog': SyslogLogger, 
   'file': FileLogger, 
   'stdout': StdoutLogger, 
   'stderr': StderrLogger, 
   'null': NullLogger}
LEVELS_MAP = {'debug': LOG_DEBUG, 
   'info': LOG_INFO, 
   'error': LOG_ERROR}
msg = lambda x: None
logLevel = LOG_INFO
...
def set_level(level):
    ...
    try:
        ...
        raise ControlPlaneError('Unknown log level "%s", known levels are: %s' % (
...
def set_logger(progId, *priv, **options):
    ...
    try:
        ...
        raise ControlPlaneError('Unknown logging method "%s", known methods are: %s' % (
         priv[0], (', ').join(METHODS_MAP)))