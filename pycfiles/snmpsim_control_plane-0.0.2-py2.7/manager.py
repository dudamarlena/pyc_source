...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/supervisor/reporting/manager.py
...
class ReportingManager(object):
    ...
    """
    REPORTING_PERIOD = 15
    REPORTERS = {'null': null.NullReporter, 
    ...
    def configure(cls, fmt, *args):
        try:
            ...
            raise error.ControlPlaneError('Unsupported reporting format: %s' % fmt)

        cls._reporter = reporter(*args)
        log.info('Using "%s" activity reporting method with params %s' % (
         cls._reporter, (', ').join(args)))
    ...
    def process_metrics(cls, watch_dir, *instances):
        ...
        cls._reporter.dump_metrics(all_metrics, watch_dir=watch_dir, started=cls.STARTED, begin=int(last_dump), end=int(now))