...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/snmpsim_control_plane/metrics/views.py
# Compiled at: 2020-02-08 05:05:54
import flask
...
PACKETS_QS_COLUMN_MAP = {'protocol': models.Transport.transport_protocol, 
   'local_address': models.Transport.endpoint, 
   'peer_address': models.Transport.peer}
MESSAGES_QS_COLUMN_MAP = {'engine_id': models.Agent.engine, 
...
def filter_by(query, *fields):
    ...
    if unknown_columns:
        raise exceptions.NotFound('Search term(s) %s not supported' % (', ').join(unknown_columns))
    for field in fields:
        args = search_columns.getlist(field)
        if args:
            query = query.filter(QS_COLUMN_MAP[field].in_(args))
...
@app.route(PREFIX + '/')
def show_root():
    return {'activity': flask.url_for('show_activity')}


@app.route(PREFIX + '/activity')
def show_activity():
    return {'packets': flask.url_for('show_packets'), 
       'messages': flask.url_for('show_messages')}


def _make_filter_hyperlinks(target):
    return {flt:{'self': {'_links': flask.url_for('show_filter', target=target, flt=flt)}} for flt in (PACKETS_QS_COLUMN_MAP if target == 'packets' else QS_COLUMN_MAP)}
...
def show_filter(target, flt):
    try:
        column = (PACKETS_QS_COLUMN_MAP if target == 'packets' else QS_COLUMN_MAP)[flt]
    except KeyError:
        raise exceptions.NotFound('No such filter')

    metrics = models.Transport.query.with_entities(column).group_by(column).all()
    return flask.jsonify([ mtr[0] for mtr in metrics ])


def _show_packets_or_messages(show_messages=False):
    transport_query = models.Transport.query.with_entities(func.sum(models.Packet.total).label('total'), func.sum(models.Packet.parse_failures).label('parse_failures'), func.sum(models.Packet.auth_failures).label('auth_failures'), func.sum(models.Packet.context_failures).label('context_failures'))
    metrics = {}
    if show_messages:
        agent_query = transport_query.join(models.Transport).join(models.Agent).join(models.Recording).join(models.Pdu)
        agent_query = filter_by(agent_query, *QS_COLUMN_MAP)
        var_binds_query = agent_query.join(models.VarBind).with_entities(func.sum(models.Pdu.total).label('pdus'), func.sum(models.VarBind.total).label('var_binds'), func.sum(models.VarBind.failures).label('failures'))
        variations_query = agent_query.join(models.Variation).with_entities(models.Variation.name, func.sum(models.Variation.total).label('total'), func.sum(models.Variation.failures).label('failures')).group_by(models.Variation.name)
        ...
        _self = flask.url_for('show_messages', **dict((field, flask.request.args.getlist(field)) for field in QS_COLUMN_MAP))
        links = {'self': _self}
        filters = {'_links': {'self': flask.url_for('show_messages_filters')}}
        metrics.update(variations=variations, _links=links, filters=filters, **messages)
    else:
        ...
        _self = flask.url_for('show_packets', **dict((field, flask.request.args.getlist(field)) for field in PACKETS_QS_COLUMN_MAP))
        links = {'self': _self}
        filters = {'_links': {'self': flask.url_for('show_packets_filters')}}
...
def show_processes(id=None, supervisor_id=None):
    process_query = models.Process.query.outerjoin(models.Endpoint).outerjoin(models.ConsolePage)
    if supervisor_id is not None:
        process_query = process_query.filter(models.Process.supervisor_id == supervisor_id)
    if id is None:
        processes = process_query.all()
    else:
        process_query = process_query.filter(models.Process.id == id)
...
def show_supervisors(id=None):
    ...
    if id is None:
        ...
        supervisor_query = supervisor_query.filter(models.Supervisor.id == id)
...
def show_console(id, page_id=None):
    console_query = models.ConsolePage.query.join(models.Process).filter(models.Process.id == id).order_by(models.ConsolePage.timestamp.asc())
    if page_id is None:
        pages = console_query.all()
    else:
        console_query = console_query.filter(models.ConsolePage.id == page_id)
