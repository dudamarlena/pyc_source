...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/DBApps/genWorks.py
...
def genWorks():
    ...
    try:
        ...
        import traceback
        print
        {'exception_value':value,  'value':tt, 
...
def parseArgs(argNamespace):
    ...
    _parser.add_argument('sourceFile', help="CSV file containing Work,     HOLLIS tuples with headings 'RID' and 'HOLLIS'")
...
def parseVolumeArgs(argNamespace):
    ...
    """
    _parser = argparse.ArgumentParser(description='Replicates work HOLLIS     pair file',
      usage="%(prog)s [-c CSV outputs csv format to file CSV.  -d DBAppSection:DbAppFile outputs to db whose parameters are given in dbConfig file 'DbAppFile' which contains section 'DbAppSection'")
