...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sqlturk/migration.py
# Compiled at: 2009-10-14 07:00:03
"""Public API.
...
__all__ = [
 'MigrationTool']
import re, glob, os.path, datetime
...
def execute_batch(engine, sql, regex="(?mx) ([^';]* (?:'[^']*'[^';]*)*)", comment_regex='(?mx) (?:^\\s*$)|(?:--.*$)'):
    """
...
class MigrationFile(object):
    ...
    """
    NAME_RE = re.compile('(?P<number>[0-9]+)(?P<sep>[-_])(?P<comment>[^.]+)\\.sql')
...
class MigrationTool(object):
    ...
    def __init__(self, dburi, migration_dir, schema_table='sql_migration'):
        ...
        self.table = Table(schema_table, metadata, Column('id', types.Integer, primary_key=True), Column('migration', types.String(80)), Column('applied', types.DateTime, default=datetime.datetime.now))
