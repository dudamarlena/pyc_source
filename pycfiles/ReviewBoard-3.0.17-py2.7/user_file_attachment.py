...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/user_file_attachment.py
...
from djblets.webapi.errors import DOES_NOT_EXIST, DUPLICATE_ITEM, INVALID_FORM_DATA, NOT_LOGGED_IN, PERMISSION_DENIED
...
class UserFileAttachmentResource(BaseFileAttachmentResource):
    ...
    """
    name = b'user_file_attachment'
    model_parent_key = b'user'
    added_in = b'3.0'
    mimetype_list_resource_name = b'user-file-attachments'
    mimetype_item_resource_name = b'user-file-attachment'
    ...
    def serialize_absolute_url_field(self, obj, request, **kwargs):
        ...
        return build_server_url(local_site_reverse(b'user-file-attachment', local_site_name=local_site_name, kwargs={b'file_attachment_uuid': obj.uuid, 
           b'username': obj.user.username}))
    ...
    def has_list_access(self, request, user):
        return request.user.is_authenticated() and (request.user.is_superuser or request.user == user)
    ...
    @webapi_response_errors(DOES_NOT_EXIST, PERMISSION_DENIED, NOT_LOGGED_IN, INVALID_FORM_DATA)
    @webapi_request_fields(optional={b'caption': {b'type': six.text_type, 
                    b'description': b'The optional caption describing the file.'}, 
       b'path': {b'type': file, 
                 b'description': b'The file to upload.'}})
    def create(self, request, local_site_name=None, *args, **kwargs):
        ...
        local_site = self._get_local_site(local_site_name)
        if local_site and not local_site.is_accessible_by(request.user) or not self.has_list_access(request, user):
            return self.get_no_access_error(request)
        ...
        return (
         201,
         {self.item_result_key: self.serialize_object(file_attachment, request=request, *args, **kwargs)})

    @webapi_check_local_site
    @webapi_login_required
    @webapi_response_errors(DOES_NOT_EXIST, DUPLICATE_ITEM, INVALID_FORM_DATA, NOT_LOGGED_IN, PERMISSION_DENIED)
    @webapi_request_fields(optional={b'caption': {b'type': six.text_type, 
                    b'description': b'The optional caption describing the file.'}, 
       b'path': {b'type': file, 
                 b'description': b'The file to upload.'}})
    def update(self, request, local_site_name=None, *args, **kwargs):
        ...
        """
        try:
            file_attachment = self.get_object(request, local_site_name=local_site_name, *args, **kwargs)
        except ObjectDoesNotExist:
            return DOES_NOT_EXIST

        if not self.has_modify_permissions(request, file_attachment, *args, **kwargs):
            return self.get_no_access_error(request)
        if b'path' in request.FILES and file_attachment.file:
