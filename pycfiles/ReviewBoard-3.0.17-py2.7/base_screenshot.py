...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/base_screenshot.py
...
class BaseScreenshotResource(WebAPIResource):
    ...
    fields = {b'id': {b'type': int, 
               b'description': b'The numeric ID of the screenshot.'}, 
       b'caption': {b'type': six.text_type, 
                    b'description': b"The screenshot's descriptive caption."}, 
       b'path': {b'type': six.text_type, 
                 b'description': b"The path of the screenshot's image file, relative to the media directory configured on the Review Board server."}, 
       b'filename': {b'type': six.text_type, 
                     b'description': b"The base file name of the screenshot's image.", 
                     b'added_in': b'1.7.10'}, 
       b'review_url': {b'type': six.text_type, 
                       b'description': b'The URL to the review UI for this screenshot.', 
                       b'added_in': b'1.7.10'}, 
       b'url': {b'type': six.text_type, 
                b'description': b"The URL of the screenshot file. If this is not an absolute URL (for example, if it is just a path), then it's relative to the Review Board server's URL. This is deprecated and will be removed in a future version.", 
                b'deprecated_in': b'2.0'}, 
       b'absolute_url': {b'type': six.text_type, 
                         b'description': b'The absolute URL of the screenshot file.', 
                         b'added_in': b'2.0'}, 
       b'thumbnail_url': {b'type': six.text_type, 
                          b'description': b"The URL of the screenshot's thumbnail file. If this is not an absolute URL (for example, if it is just a path), then it's relative to the Review Board server's URL."}}
    uri_object_key = b'screenshot_id'
    ...
    @webapi_response_errors(DOES_NOT_EXIST, NOT_LOGGED_IN, PERMISSION_DENIED, INVALID_FORM_DATA)
    @webapi_request_fields(required={b'path': {b'type': file, 
                 b'description': b'The screenshot to upload.'}}, optional={b'caption': {b'type': six.text_type, 
                    b'description': b'The optional caption describing the screenshot.'}})
    def create(self, request, *args, **kwargs):
        ...
        try:
            screenshot = form.create(request.FILES[b'path'], review_request)
    ...
    @webapi_request_fields(optional={b'caption': {b'type': six.text_type, 
                    b'description': b'The new caption for the screenshot.'}})
    ...
    def update(self, request, caption=None, *args, **kwargs):
        ...
        if not review_request.is_mutable_by(request.user):
            return self.get_no_access_error(request)
        try:
            screenshot = resources.screenshot.get_object(request, *args, **kwargs)
        except ObjectDoesNotExist:
            return DOES_NOT_EXIST
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not self.has_delete_permissions(request, screenshot, *args, **kwargs):
            return self.get_no_access_error(request)
        try:
