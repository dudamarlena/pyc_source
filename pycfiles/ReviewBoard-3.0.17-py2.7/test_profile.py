...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/tests/test_profile.py
...
class ProfileTests(TestCase):
    ...
    def test_is_star_unstar_updating_count_correctly(self):
        ...
        user1 = User.objects.get(username=b'admin')
        ...
        self.assertTrue(review_request in profile1.starred_review_requests.all())
        self.assertEqual(site_profile.starred_public_request_count, 1)
        profile1.unstar_review_request(review_request)
        site_profile = LocalSiteProfile.objects.get(pk=site_profile.pk)
        self.assertFalse(review_request in profile1.starred_review_requests.all())
    ...
    def test_get_display_name_unauthenticated_public(self):
        ...
        self.assertEqual(profile.get_display_name(AnonymousUser()), user.username)
    ...
    def test_get_display_name_public(self):
        ...
        profile = user.get_profile()
        profile.is_private = False
        self.assertEqual(profile.get_display_name(User.objects.get(username=b'grumpy')), user.get_full_name())
    ...
    def test_get_display_name_private(self):
        ...
        profile = user.get_profile()
        profile.is_private = True
        self.assertEqual(profile.get_display_name(User.objects.get(username=b'grumpy')), user.username)
    ...
    def test_get_display_name_admin_private(self):
        ...
        profile = user.get_profile()
        profile.is_private = True
        self.assertEqual(profile.get_display_name(User.objects.get(username=b'admin')), user.get_full_name())
    ...
    def test_get_display_name_localsite_member_private(self):
        ...
        profile = user.get_profile()
        profile.is_private = True
        viewer = User.objects.get(username=b'grumpy')
    ...
    def test_get_display_name_localsite_admin_private(self):
        ...
        a LocalSite member with a private profile
        """
        user = User.objects.get(username=b'admin')
        profile = user.get_profile()
        profile.is_private = True
        self.assertEqual(profile.get_display_name(User.objects.get(username=b'doc')), user.get_full_name())
    ...
    def test_get_display_name_localsite_admin_private_other_site(self):
        ...
        profile.is_private = True
        viewer = User.objects.get(username=b'grumpy')
        site = LocalSite.objects.create(name=b'site-3')
        site.users.add(viewer)
