...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/datagrids/sidebar.py
...
class BaseSidebarItem(object):
    ...
    def get_url(self):
        ...
        if not self.view_id and not self.view_args:
            ...
            if self.view_args:
                ...
                url_args = {}
            if self.view_id:
                url_args[b'view'] = self.view_id
            return b'%s?%s' % (self.datagrid.request.path, urlencode(url_args))
    ...
    def is_active(self):
        ...
        if self.view_id is None:
            ...
            view_id = request.GET.get(b'view', self.sidebar.default_view_id)
    ...
    def render(self):
        ...
        context = {b'datagrid': self.datagrid, 
           b'label': self.label, 
           b'icon_name': self.icon_name or b'', 
           b'view_id': self.view_id, 
           b'view_args': self.view_args, 
           b'count': count, 
           b'has_count': count is not None, 
           b'url': self.get_url(), 
           b'active': self.is_active(), 
           b'css_classes': self.css_classes or []}
        context.update(self.get_extra_context())
        return render_to_string(self.template_name, RequestContext(self.datagrid.request, context))
...
class BaseSidebarSection(BaseSidebarItem):
    ...
    """
    template_name = b'datagrids/sidebar_section.html'
...
class SidebarNavItem(BaseSidebarItem):
    ...
    """
    template_name = b'datagrids/sidebar_nav_item.html'

    def __init__(self, section, label, icon_name=None, view_id=None, view_args=None, count=None, url=None, url_name=None, css_classes=None):
        """Initialize the item.
    ...
    def get_url(self):
        ...
        if self.url:
            ...
            if self.url_name:
                return local_site_reverse(self.url_name, request=self.datagrid.request)
...
class Sidebar(object):
    ...
    def get_items(self, datagrid):
        ...
        """
        return [ item_cls(self, datagrid) for item_cls in self._item_classes
               ]
...
class DataGridSidebarMixin(object):
    ...
    def load_extra_state(self, *args, **kwargs):
        ...
        """
        result = super(DataGridSidebarMixin, self).load_extra_state(*args, **kwargs)
