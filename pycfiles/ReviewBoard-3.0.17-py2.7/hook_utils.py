...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/hostingsvcs/hook_utils.py
...
def get_repository_for_hook(repository_id, hosting_service_id, local_site_name, hooks_uuid=None):
...
def get_review_request_id(commit_message, server_url, commit_id=None, repository=None):
    ...
    """
    regex = settings.HOSTINGSVCS_HOOK_REGEX % {b'server_url': server_url}
    pattern = re.compile(regex, settings.HOSTINGSVCS_HOOK_REGEX_FLAGS)
    match = pattern.search(commit_message)
    if match:
        try:
            review_request_id = int(match.group(b'id'))
        except ValueError:
            logging.error(b'The review request ID must be an integer.')
            review_request_id = None

    elif commit_id:
        assert repository
        try:
            review_request = ReviewRequest.objects.get(commit_id=six.text_type(commit_id), repository=repository)
...
def close_review_request(review_request, review_request_id, description):
    ...
    if review_request.status == ReviewRequest.SUBMITTED:
        logging.warning(b'Review request #%s is already submitted.', review_request_id)
        return
    if not review_request.public:
        review_request.publish(review_request.submitter, trivial=True, validate_fields=False)
    review_request.close(ReviewRequest.SUBMITTED, description=description)
    logging.debug(b'Review request #%s is set to %s.', review_request_id, review_request.status)


def close_all_review_requests(review_request_id_to_commits, local_site_name, repository, hosting_service_id):
    ...
    if local_site_name:
        try:
            ...
            logging.error(b'close_all_review_requests: Local Site %s does not exist.', local_site_name)
            return

    else:
        local_site = None
    review_request_ids = [ review_request_id for review_request_id in review_request_id_to_commits if review_request_id is not None
                         ]
    if not review_request_ids:
        return
    else:
        q = Q(repository=repository) & Q(repository__hosting_account__service_name=hosting_service_id)
        if local_site:
        ...
        if len(review_request_ids) != len(review_requests):
            id_to_review_request = dict((review_request.display_id, review_request) for review_request in review_requests)
            for review_request_id in review_request_ids:
                if review_request_id not in id_to_review_request:
                    logging.error(b'close_all_review_requests: Review request #%s does not exist.', review_request_id)

        for review_request in review_requests:
            review_request_id = review_request.display_id
            close_review_request(review_request, review_request_id, b'Pushed to ' + (b', ').join(review_request_id_to_commits[review_request_id]))
