...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/admin/security_checks.py
...
class BaseExecutableFileCheck(BaseSecurityCheck):
    ...
    """
    FILENAME_PREFIX = b'exec_security_check'

    def __init__(self):
        """Initialize the security check."""
        self.storage = FileSystemStorage(location=os.path.join(settings.MEDIA_ROOT, b'uploaded', b'files'))
        self.directory = b'%suploaded/files/' % settings.MEDIA_URL
        self.file_checks = []
        ...
        if self._using_default_storage():
            for i, file_check in enumerate(self.file_checks):
                ...
                for ext in extensions_list:
                    try:
                        self.storage.save(b'%s%s' % (self.FILENAME_PREFIX, ext), ContentFile(content))
                    except OSError:
                        bad_extensions.add(ext)

                extensions_list = [ ext for ext in extensions_list if ext not in bad_extensions
                                  ]
                self.file_checks[i] = (
                 extensions_list, content)
    ...
    def tearDown(self):
        ...
        if self._using_default_storage():
            for extensions_list, content in self.file_checks:
                for ext in extensions_list:
                    self.storage.delete(b'%s%s' % (self.FILENAME_PREFIX, ext))
    ...
    def execute(self):
        ...
        if not self._using_default_storage():
            ...
            for extensions_list, content in self.file_checks:
                for ext in extensions_list:
                    try:
                        filename = b'%s%s' % (self.FILENAME_PREFIX, ext)
                        url = build_server_url(b'%s/%s' % (
                         self.directory.rstrip(b'/'),
                         filename))
                        if not self.check_file(filename, url):
                            failed_exts.append(ext)
                    except Exception as e:
                        return (
                         False, _(b'Uncaught exception during test: %s') % e)

            if failed_exts:
                return (
                 False,
                 ngettext(b'The web server incorrectly executed this file type: %s', b'The web server incorrectly executed these file types: %s', len(failed_exts)) % (b', ').join(failed_exts))
            return (
             True, None)
    ...
    def _using_default_storage(self):
        ...
        """
        return settings.DEFAULT_FILE_STORAGE == b'django.core.files.storage.FileSystemStorage'
...
class ServerExecutableFileCheck(BaseExecutableFileCheck):
    ...
    """
    name = _(b"Checking that uploaded files won't be executed by the server")
    desc = _(b'A misconfiguration in the web server can cause files attached to review requests to be executed as code. The file types checked in this test are: .html, .htm, .shtml, .php, .php3, .php4, .php5, .phps, .asp, .pl, .py, .fcgi, .cgi, .phtml, .phtm, .pht, .jsp, .sh, and .rb.')
    fix_info = _(b'For instructions on how to fix this problem, please visit <a href="http://support.beanbaginc.com/support/solutions/articles/110173-securing-file-attachments">http://support.beanbaginc.com/support/solutions/articles/110173-securing-file-attachments</a>')
    ...
    def setUp(self):
        ...
        self.file_checks = [
         (
          [
           b'.php', b'.php3', b'.php4', b'.php5', b'.phps', b'.phtml',
           b'.phtm'],
    ...
    def check_file(self, filename, url):
        ...
        with self.storage.open(filename, b'r') as (f):
            return data == f.read()
...
class BrowserExecutableFileCheck(BaseExecutableFileCheck):
    ...
    """
    name = _(b"Checking downloaded files aren't executable by the browser")
    desc = _(b'Certain file types (such as SVG or HTML files) can contain embedded scripts that would be executed by your browser when viewed. We recommend forcing all uploaded media files to download when directly accessed in a browser. This applies only to files being served by Review Board, and not from a CDN.')
    fix_info = _(b'For instructions on how to fix this problem, please visit <a href="http://support.beanbaginc.com/support/solutions/articles/110173-securing-file-attachments">http://support.beanbaginc.com/support/solutions/articles/110173-securing-file-attachments</a>')
    ...
    def setUp(self):
        ...
        self.file_checks = [
         (
          [
           b'.htm', b'.html', b'.shtml', b'.stm', b'.shtm'],
         ...
         (
          [
           b'.svgz'],
          zlib.compress(b'<svg xmlns="http://www.w3.org/2000/svg"> <script>alert("Hello!")</script></svg>'))]
    ...
    def check_file(self, filename, url):
        ...
        return headers.get(b'Content-Disposition', b'').startswith(b'attachment')
...
class AllowedHostsCheck(BaseSecurityCheck):
    ...
    """
    name = _(b'Checking ALLOWED_HOSTS setting')
    desc = _(b'ALLOWED_HOSTS is a list containing the host/domain names that Review Board will consider valid for this server to serve. This is a security measure to prevent an attacker from poisoning cache and password reset e-mails with links to malicious hosts by submitting requests with a fake HTTP Host header, which is possible even under many seemingly-safe web server configurations.')
    fix_info = _(b"To fix this, edit the settings_local.py in the site's conf directory and add a line like this with your site's URL: <pre>ALLOWED_HOSTS = ['example.com']</pre>")
    ...
    def execute(self):
        ...
        if len(settings.ALLOWED_HOSTS) < 1:
            result = False
            error_msg = _(b'ALLOWED_HOSTS is empty.')
        if b'*' in settings.ALLOWED_HOSTS:
            result = False
            error_msg = _(b"ALLOWED_HOSTS contains '*', which means that the server will respond to any host.")
        return (
         result, error_msg)
...
class SecurityCheckRunner(object):
    ...
    def run(self):
        ...
        for name, cls in six.iteritems(checks):
            ...
            check.tearDown()
            all_test_results.append({b'name': check.name, 
               b'desc': check.desc, 
               b'result': current_test_result, 
               b'error_msg': error_msg, 
               b'fix_info': check.fix_info})
...
def _populate_security_checks():
    ...
    if not _security_checks:
        _security_checks[b'hosts_check'] = AllowedHostsCheck
        _security_checks[b'server_executable_check'] = ServerExecutableFileCheck
        _security_checks[b'browser_executable_file_check'] = BrowserExecutableFileCheck
...
def register_security_check(name, cls):
    ...
    if name in _security_checks:
        raise KeyError(b'"%s" is already a registered security check' % name)
...
def unregister_security_check(name):
    ...
    try:
        ...
        logging.error(b'Failed to unregister unknown security check "%s"' % name)
        raise KeyError(b'"%s" is not a registered security check' % name)