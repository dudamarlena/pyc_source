...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/core.py
# Compiled at: 2020-02-11 04:03:56
"""Data structures and classes for defining and using SCMTools."""
from __future__ import unicode_literals
import base64, inspect, logging, os, subprocess, warnings
from pkg_resources import iter_entry_points
from django.utils import six
from django.utils.encoding import force_text, python_2_unicode_compatible
from django.utils.six.moves.urllib.error import HTTPError
from django.utils.six.moves.urllib.parse import urlparse
from django.utils.six.moves.urllib.request import Request as URLRequest, urlopen
from django.utils.translation import ugettext_lazy as _
import reviewboard.diffviewer.parser as diffparser
from reviewboard.deprecation import RemovedInReviewBoard40Warning
from reviewboard.scmtools.errors import AuthenticationError, FileNotFoundError, SCMError
...
class Branch(object):
    ...
    def __init__(self, id, name=None, commit=b'', default=False):
    ...
    def __eq__(self, other):
        ...
        """
        return self.id == other.id and self.name == other.name and self.commit == other.commit and self.default == other.default
    ...
    def __repr__(self):
        ...
        Returns:
            ...
            The string representation.
        """
        return b'<Branch %s (name=%s; commit=%s: default=%r)>' % (
...
class Commit(object):
    ...
    def __init__(self, author_name=b'', id=b'', date=b'', message=b'', parent=b'', diff=None, base_commit_id=None):
    ...
    def diff(self, new_diff):
        ...
        if isinstance(new_diff, six.text_type):
            new_diff = new_diff.encode(b'utf-8')
        self._diff = new_diff
    ...
    def __eq__(self, other):
        ...
        """
        return self.author_name == other.author_name and self.id == other.id and self.date == other.date and self.message == other.message and self.parent == other.parent
    ...
    def __repr__(self):
        ...
        Returns:
            ...
            The string representation.
        """
        return b'<Commit %r (author=%s; date=%s; parent=%r)>' % (
...
HEAD = Revision(b'HEAD')
UNKNOWN = Revision(b'UNKNOWN')
PRE_CREATION = Revision(b'PRE-CREATION')
...
class _SCMToolIDProperty(object):
    ...
    def __get__(self, owner_self, owner_cls):
        ...
        if not _SCMToolIDProperty._scmtool_ids_by_class_names:
            _SCMToolIDProperty._scmtool_ids_by_class_names = {b'%s.%s' % (ep.module_name, ep.attrs[0]):force_text(ep.name) for ep in iter_entry_points(b'reviewboard.scmtools')}
        if owner_cls is SCMTool:
            return
        else:
            key = b'%s.%s' % (owner_cls.__module__, owner_cls.__name__)
            try:
                return _SCMToolIDProperty._scmtool_ids_by_class_names[key]
            except KeyError:
                raise ValueError(_(b'Unable to determine an SCMTool ID for %r. You must set %s.scmtool_id to a unique value.') % (
...
class SCMTool(object):
    ...
    field_help_text = {b'path': _(b'The path to the repository. This will generally be the URL you would use to check out the repository.')}
    dependencies = {b'executables': [], b'modules': []}
    auth_form = None
    ...
    def diffs_use_absolute_paths(self):
        ...
        """
        if hasattr(self, b'get_diffs_use_absolute_paths'):
            warnings.warn(b'%(class_name)s.get_diffs_use_absolute_paths() is deprecated. Set the %(class_name)s.diffs_use_absolute_paths boolean instead.' % {b'class_name': self.__class__.__name__}, RemovedInReviewBoard40Warning)
    ...
    def file_exists(self, path, revision=HEAD, base_commit_id=None, **kwargs):
        ...
        try:
            if argspec.keywords is None:
                warnings.warn(b'SCMTool.get_file() must take keyword arguments, signature for %s is deprecated.' % self.name, RemovedInReviewBoard40Warning)
    ...
    def popen(cls, command, local_site_name=None, env={}):
        ...
        new_env.update(env)
        if local_site_name:
            new_env[b'RB_LOCAL_SITE'] = local_site_name.encode(b'utf-8')
        return subprocess.Popen(command, env=new_env, stderr=subprocess.PIPE, stdout=subprocess.PIPE, close_fds=os.name != b'nt')
    ...
    def check_repository(cls, path, username=None, password=None, local_site_name=None):
        ...
        if sshutils.is_ssh_uri(path):
            ...
            logging.debug(b'%s: Attempting ssh connection with host: %s, username: %s' % (
    ...
    def get_auth_from_uri(cls, path, username):
        ...
        url = urlparse(path)
        if b'@' in url[1]:
            netloc_username, hostname = url[1].split(b'@', 1)
        else:
            hostname = url[1]
...
class SCMClient(object):
    ...
    def get_file_http(self, url, path, revision, mime_type=None):
        ...
        """
        logging.info(b'Fetching file from %s' % url)
        try:
            request = URLRequest(url)
            if self.username:
                auth_string = base64.b64encode(b'%s:%s' % (self.username,
                 self.password))
                request.add_header(b'Authorization', b'Basic %s' % auth_string)
            ...
            if e.code == 404:
                ...
                msg = b'HTTP error code %d when fetching file from %s: %s' % (
                 e.code, url, e)
                logging.error(msg)
                raise SCMError(msg)
        except Exception as e:
            msg = b'Unexpected error fetching file from %s: %s' % (url, e)
            logging.error(msg)
            raise SCMError(msg)
