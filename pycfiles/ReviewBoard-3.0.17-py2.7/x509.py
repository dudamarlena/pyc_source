...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/backends/x509.py
...
class X509Backend(BaseAuthBackend):
    ...
    """
    backend_id = b'x509'
    name = _(b'X.509 Public Key')
    settings_form = X509SettingsForm
    supports_change_password = True

    def authenticate(self, x509_field=b'', **kwargs):
        ...
        if settings.X509_USERNAME_REGEX:
            try:
                ...
                if m:
                    ...
                    logging.warning(b"X509Backend: username '%s' didn't match regex.", username)
            except sre_constants.error as e:
                logging.error(b'X509Backend: Invalid regex specified: %s', e, exc_info=1)
    ...
    def get_or_create_user(self, username, request):
        ...
        try:
            ...
            if getattr(settings, b'X509_AUTOCREATE_USERS', False):
                user = User(username=username, password=b'')
                user.is_staff = False
