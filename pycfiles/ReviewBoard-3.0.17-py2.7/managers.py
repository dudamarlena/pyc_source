...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/managers.py
...
class ToolQuerySet(QuerySet):
    ...
    def get(self, *args, **kwargs):
        pk = kwargs.get(b'id__exact', None)
        if pk is None:
...
class RepositoryManager(Manager):
    ...
    def accessible(self, user, visible_only=True, local_site=None, show_all_local_sites=False):
        ...
        if user.is_superuser:
            ...
            if user.is_authenticated():
                q |= Q(users__pk=user.pk) | Q(review_groups__users=user.pk)
            qs = self.filter(q)
    ...
    def accessible_ids(self, *args, **kwargs):
        ...
        Returns:
            ...
            The list of IDs.
        """
        return self.accessible(*args, **kwargs).values_list(b'pk', flat=True)

    def can_create(self, user, local_site=None):
        return user.has_perm(b'scmtools.add_repository', local_site)

    def encrypt_plain_text_passwords(self):
        """Encrypts any stored plain-text passwords."""
        qs = self.exclude(Q(encrypted_password=None) | Q(encrypted_password=b'') | Q(encrypted_password__startswith=self.model.ENCRYPTED_PASSWORD_PREFIX))
        qs = qs.only(b'encrypted_password')
