...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_counters.py
...
class ReviewRequestCounterTests(SpyAgency, TestCase):
    ...
    def setUp(self):
        ...
        repository = Repository.objects.create(name=b'Test1', path=b'path1', tool=tool)
        self.user = User.objects.create_user(username=b'testuser', password=b'', email=b'user@example.com')
        self.profile = self.user.get_profile()
        self.test_site = LocalSite.objects.create(name=b'test')
        ...
        self.group = Group.objects.create(name=b'test-group')
    ...
    def test_closing_draft_requests_with_site(self):
        ...
        tool = Tool.objects.get(name=b'Subversion')
        repository = Repository.objects.create(name=b'Test1', path=b'path1', tool=tool, local_site=self.test_site)
    ...
    def test_remove_group_and_fail_publish(self):
        ...
        draft.target_groups.remove(self.group)
        self._check_counters(total_outgoing=1, pending_outgoing=1, total_incoming=1, group_incoming=1, starred_public=1)
        self.spy_on(ReviewRequestDraft.publish, call_fake=self._raise_publish_error)
    ...
    def test_populate_counters_after_change(self):
        ...
        Group.objects.update(incoming_request_count=None)
        profile_fields = [
         b'direct_incoming_request_count',
         b'total_incoming_request_count',
         b'pending_outgoing_request_count',
         b'total_outgoing_request_count',
         b'starred_public_request_count']
        locks = {self.site_profile: 1, 
    ...
    def test_counts_with_reassignment_in_initial_draft(self):
        ...
        """
        self._check_counters(total_outgoing=1, pending_outgoing=1)
        new_user = User.objects.create_user(username=b'test2', password=b'', email=b'user@example.com')
    ...
    def test_counts_with_reassignment_and_close(self):
        ...
        self.review_request.save(update_counts=True, old_submitter=self.user)
        self._check_counters(total_outgoing=0, pending_outgoing=0, starred_public=0)
        site_profile = LocalSiteProfile.objects.get(pk=site_profile.pk)
        self._check_counters_on_profile(site_profile, total_outgoing=1, pending_outgoing=0)
    ...
    def test_counts_with_reassignment_and_reopen(self):
        ...
        self._check_counters(total_outgoing=1, pending_outgoing=0, starred_public=0)
        new_user = User.objects.create_user(username=b'test2', password=b'', email=b'user@example.com')
        site_profile = new_user.get_site_profile(self.review_request.local_site)
        self.review_request.owner = new_user
        ...
        self._check_counters(total_outgoing=0, pending_outgoing=0, starred_public=0)
        site_profile = LocalSiteProfile.objects.get(pk=site_profile.pk)
        self._check_counters_on_profile(site_profile, total_outgoing=1, pending_outgoing=1)
    ...
    def test_counts_with_join_group(self):
        ...
        self.create_review_request(submitter=user2, target_groups=[
         group2], publish=True)
        self._check_counters(total_outgoing=1, pending_outgoing=1, total_incoming=0)
        group2.users.add(self.user)
        self._check_counters(total_outgoing=1, pending_outgoing=1, total_incoming=1)
    ...
    def _check_counters(self, total_outgoing=0, pending_outgoing=0, direct_incoming=0, total_incoming=0, starred_public=0, group_incoming=0, with_local_site=False):
        ...
        self.assertEqual(self.group.incoming_request_count, group_incoming, b'Expected Group.incoming_request_count to be %s. Got %s instead.' % (
    ...
    def _check_counters_on_profile(self, profile, total_outgoing=0, pending_outgoing=0, direct_incoming=0, total_incoming=0, starred_public=0):
        ...
        Args:
            ...
            starred_public (int):
                ...
                user.
        """
        msg = b'Expected %s to be %s. Got %s instead.'
        self.assertEqual(profile.total_outgoing_request_count, total_outgoing, msg % (b'total_outgoing_request_count', total_outgoing,
         profile.total_outgoing_request_count))
        self.assertEqual(profile.pending_outgoing_request_count, pending_outgoing, msg % (b'pending_outgoing_request_count', pending_outgoing,
         profile.pending_outgoing_request_count))
        self.assertEqual(profile.direct_incoming_request_count, direct_incoming, msg % (b'direct_incoming_request_count', direct_incoming,
         profile.direct_incoming_request_count))
        self.assertEqual(profile.total_incoming_request_count, total_incoming, msg % (b'total_incoming_request_count', total_incoming,
         profile.total_incoming_request_count))
        self.assertEqual(profile.starred_public_request_count, starred_public, msg % (b'starred_public_request_count', starred_public,
