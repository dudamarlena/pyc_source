...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/diffviewer/myersdiff.py
...
class MyersDiffer(Differ):
    ...
    def get_opcodes(self):
        ...
        if self.a_data.length == 0 and self.b_data.length == 0:
            ...
            while a_line < self.a_data.length or b_line < self.b_data.length:
                a_start = a_line
                b_start = b_line
                if a_line < self.a_data.length and not self.a_data.modified.get(a_line, False) and b_line < self.b_data.length and not self.b_data.modified.get(b_line, False):
                    a_changed = b_changed = 1
                    tag = b'equal'
                    a_line += 1
                    b_line += 1
                else:
                    while a_line < self.a_data.length and (b_line >= self.b_data.length or self.a_data.modified.get(a_line, False)):
                        a_line += 1

                    while b_line < self.b_data.length and (a_line >= self.a_data.length or self.b_data.modified.get(b_line, False)):
                        b_line += 1
    ...
    def _gen_diff_data(self):
        ...
        self.max_lines = self.a_data.undiscarded_lines + self.b_data.undiscarded_lines + 3
        ...
        self._lcs(0, self.a_data.undiscarded_lines, 0, self.b_data.undiscarded_lines, self.minimal_diff)
    ...
    def _gen_diff_codes(self, lines, is_modified_file):
        ...
        for line in lines:
            ...
            try:
                ...
                interesting_line_name = self.interesting_line_table.get(code, None)
            except KeyError:
                self.last_code += 1
                code = self.last_code
    ...
    def _find_sms(self, a_lower, a_upper, b_lower, b_upper, find_minimal):
        ...
        """
        down_vector = self.fdiag
        up_vector = self.bdiag
        down_k = a_lower - b_lower
        up_k = a_upper - b_upper
        ...
        while True:
            ...
            for k in range(down_max, down_min - 1, -2):
                ...
                y = x - k
                old_x = x
                while x < a_upper and y < b_upper and self.a_data.undiscarded[x] == self.b_data.undiscarded[y]:
                    x += 1
                    y += 1

                if odd_delta and up_min <= k <= up_max and up_vector[(self.upoff + k)] <= x:
            ...
            for k in range(up_max, up_min - 1, -2):
                ...
                y = x - k
                old_x = x
                while x > a_lower and y > b_lower and self.a_data.undiscarded[(x - 1)] == self.b_data.undiscarded[(y - 1)]:
                    x -= 1
                    y -= 1

                if not odd_delta and down_min <= k <= down_max and x <= down_vector[(self.downoff + k)]:
                    return (x, y, True, True)
            ...
            if cost > 200 and big_snake:
                ret_x, ret_y, best = self._find_diagonal(down_min, down_max, down_k, 0, self.downoff, down_vector, lambda x: x - a_lower, lambda x: a_lower + self.SNAKE_LIMIT <= x < a_upper, lambda y: b_lower + self.SNAKE_LIMIT <= y < b_upper, lambda i, k: i - k, 1, cost)
                if best > 0:
                    return (ret_x, ret_y, True, False)
                ret_x, ret_y, best = self._find_diagonal(up_min, up_max, up_k, best, self.upoff, up_vector, lambda x: a_upper - x, lambda x: a_lower < x <= a_upper - self.SNAKE_LIMIT, lambda y: b_lower < y <= b_upper - self.SNAKE_LIMIT, lambda i, k: i + k, 0, cost)
                if best > 0:
                    return (ret_x, ret_y, False, True)
            if cost >= max_cost:
                if self.compat_version >= DiffCompatVersion.MYERS_SMS_COST_BAIL:
                    fx_best = bx_best = 0
                    fxy_best = -1
                    ...
                    if a_upper + b_upper - bxy_best < fxy_best - (a_lower + b_lower):
                        return (fx_best, fxy_best - fx_best, True, False)
                    return (
                     bx_best, bxy_best - bx_best, False, True)

        raise Exception(b'The function should not have reached here.')

    def _find_diagonal(self, minimum, maximum, k, best, diagoff, vector, vdiff_func, check_x_range, check_y_range, discard_index, k_offset, cost):
        for d in range(maximum, minimum - 1, -2):
            ...
            if v > 12 * (cost + abs(dd)):
                if v > best and check_x_range(x) and check_y_range(y):
                    ...
                    while self.a_data.undiscarded[x_index] == self.b_data.undiscarded[y_index]:
    ...
    def _lcs(self, a_lower, a_upper, b_lower, b_upper, find_minimal):
        ...
        """
        while a_lower < a_upper and b_lower < b_upper and self.a_data.undiscarded[a_lower] == self.b_data.undiscarded[b_lower]:
            a_lower += 1
            b_lower += 1

        while a_upper > a_lower and b_upper > b_lower and self.a_data.undiscarded[(a_upper - 1)] == self.b_data.undiscarded[(b_upper - 1)]:
            a_upper -= 1
            b_upper -= 1
        ...
        if a_lower == a_upper:
            ...
            x, y, low_minimal, high_minimal = self._find_sms(a_lower, a_upper, b_lower, b_upper, find_minimal)
    ...
    def _discard_confusing_lines(self):
        ...
        self.a_data.undiscarded = [
         0] * self.a_data.length
