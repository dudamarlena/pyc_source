...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/detail.py
...
from djblets.registries.registry import ALREADY_REGISTERED, ATTRIBUTE_REGISTERED, NOT_REGISTERED
...
from reviewboard.reviews.models import BaseComment, Comment, FileAttachmentComment, GeneralComment, ReviewRequest, ScreenshotComment, StatusUpdate
...
class ReviewRequestPageData(object):
    ...
    def __init__(self, review_request, request, last_visited=None, entry_classes=None):
        ...
        self.issues = []
        self.issue_counts = {b'total': 0, 
           b'open': 0, 
           b'resolved': 0, 
           b'dropped': 0, 
           b'verifying': 0}
        self.status_updates_enabled = status_updates_feature.is_enabled(local_site=review_request.local_site)
        ...
        for entry_cls in self.entry_classes:
            ...
            self._needs_reviews = self._needs_reviews or entry_cls.needs_reviews
            self._needs_changedescs = self._needs_changedescs or entry_cls.needs_changedescs
            self._needs_status_updates = self._needs_status_updates or entry_cls.needs_status_updates
            self._needs_file_attachments = self._needs_file_attachments or entry_cls.needs_file_attachments
            self._needs_screenshots = self._needs_screenshots or entry_cls.needs_screenshots
    ...
    def query_data_pre_etag(self):
        ...
        if self._needs_reviews or self._needs_status_updates:
            self.reviews = list(self.review_request.reviews.filter(reviews_query).order_by(b'-timestamp').select_related(b'user', b'user__profile'))
        ...
        if self._needs_changedescs:
            self.changedescs = list(self.review_request.changedescs.filter(public=True))
        if self.changedescs:
        ...
        if self.status_updates_enabled and self._needs_status_updates:
            self.all_status_updates = list(self.review_request.status_updates.order_by(b'summary'))
    ...
    def query_data_post_etag(self):
        ...
        for status_update in self.all_status_updates:
            ...
            if status_update.change_description_id:
                self.change_status_updates.setdefault(status_update.change_description_id, []).append(status_update)
            else:
        ...
        for review in self.reviews:
            ...
            if parent_id is not None:
                ...
                if parent_id in self.latest_timestamps_by_review_id:
                    old_timestamp = self.latest_timestamps_by_review_id[parent_id]
                    if old_timestamp < new_timestamp:
                        self.latest_timestamps_by_review_id[parent_id] = new_timestamp
                else:
                    self.latest_timestamps_by_review_id[parent_id] = new_timestamp
            if not hasattr(review, b'_status_update_cache'):
        ...
        for reply_id, replies in six.iteritems(self.body_bottom_replies):
            self.reviews_by_id[reply_id]._body_bottom_replies = reversed(replies)

        self.review_request_details = self.draft or self.review_request
        if self._needs_file_attachments or self._needs_reviews:
            self.active_file_attachments = list(self.review_request_details.get_file_attachments())
            self.all_file_attachments = self.active_file_attachments + list(self.review_request_details.get_inactive_file_attachments())
            self.file_attachments_by_id = self._build_id_map(self.all_file_attachments)
            for attachment in self.all_file_attachments:
                attachment._comments = []

        if self._needs_screenshots or self._needs_reviews:
            self.active_screenshots = list(self.review_request_details.get_screenshots())
            self.all_screenshots = self.active_screenshots + list(self.review_request_details.get_inactive_screenshots())
        ...
        if self.reviews:
            ...
            for model, key, ordering in (
             (
              GeneralComment, b'general_comments', None),
             (
              ScreenshotComment, b'screenshot_comments', None),
             (
              FileAttachmentComment, b'file_attachment_comments', None),
             (
              Comment, b'diff_comments',
              ('comment__filediff', 'comment__first_line', 'comment__timestamp'))):
                ...
                for obj in objs:
                    ...
                    if not obj.review_id in self.reviews_by_id:
                        ...
                        if is_reply == comment.is_reply():
                            if is_reply:
                                ...
                                review.public or self.draft_reply_comments.setdefault(review.base_reply_to_id, []).append(comment)
                        else:
                            self.review_comments.setdefault(review.pk, []).append(comment)
                    if review.public and comment.issue_opened:
                        status_key = comment.issue_status_to_string(comment.issue_status)
                        if status_key in ('verifying-resolved', 'verifying-dropped'):
                            status_key = b'verifying'
                        self.issue_counts[status_key] += 1
                        self.issue_counts[b'total'] += 1
    ...
    def get_entries(self):
        ...
        return {b'initial': initial_entries, 
           b'main': main_entries}
...
class BaseReviewRequestPageEntry(object):
    ...
    template_name = None
    js_template_name = b'reviews/entries/entry.js'
    js_model_class = b'RB.ReviewRequestPage.Entry'
    js_view_class = b'RB.ReviewRequestPage.EntryView'
    has_content = True
    ...
    def __init__(self, data, entry_id, added_timestamp, updated_timestamp=None, avatar_user=None):
    ...
    def __repr__(self):
        ...
        """
        return b'%s(entry_type_id=%s, entry_id=%s, added_timestamp=%s, updated_timestamp=%s, collapsed=%s)' % (
    ...
    def is_entry_new(self, last_visited, user, **kwargs):
        ...
        """
        return self.added_timestamp is not None and last_visited < self.added_timestamp
    ...
    def calculate_collapsed(self):
        ...
        """
        data = self.data
        return bool(data.latest_changedesc_timestamp and self.updated_timestamp < data.latest_changedesc_timestamp and data.last_visited and self.updated_timestamp < data.last_visited)
    ...
    def get_dom_element_id(self):
        ...
        """
        return b'%s%s' % (self.entry_type_id, self.entry_id)
    ...
    def render_to_string(self, request, context):
        ...
        try:
            new_context = {b'entry': self, 
               b'entry_is_new': user.is_authenticated() and last_visited is not None and self.is_entry_new(last_visited=last_visited, user=user), 
               b'show_entry_statuses_area': self.entry_pos != BaseReviewRequestPageEntry.ENTRY_POS_INITIAL}
            new_context.update(self.get_extra_context(request, context))
        except Exception as e:
            logging.exception(b'Error generating template context for %s (ID=%s): %s', self.__class__.__name__, self.entry_id, e)
            return b''
        ...
        try:
            try:
                ...
                logging.exception(b'Error rendering template for %s (ID=%s): %s', self.__class__.__name__, self.entry_id, e)
                return b''
...
class ReviewEntryMixin(object):
    ...
    def is_review_collapsed(self, review):
        ...
        * The review has not been seen since the latest activity on it
        ...
        has_comments_with_issues = any(comment.issue_opened and comment.issue_status in (comment.OPEN, comment.VERIFYING_RESOLVED, comment.VERIFYING_DROPPED) for comment in data.review_comments.get(review.pk, []))
        return bool(not has_comments_with_issues and not data.draft_body_top_replies.get(review.pk) and not data.draft_body_bottom_replies.get(review.pk) and not data.draft_reply_comments.get(review.pk) and data.last_visited and review.timestamp < data.last_visited and (not latest_reply_timestamp or latest_reply_timestamp < data.last_visited))
    ...
    def serialize_review_js_model_data(self, review):
        ...
        """
        return {b'id': review.pk, 
           b'shipIt': review.ship_it, 
           b'public': True, 
           b'bodyTop': review.body_top, 
           b'bodyBottom': review.body_bottom}
...
class DiffCommentsSerializerMixin(object):
    ...
    def serialize_diff_comments_js_model_data(self, diff_comments):
        ...
        for comment in diff_comments:
            key = b'%s' % comment.filediff_id
            if comment.interfilediff_id:
                key = b'%s-%s' % (key, comment.interfilediff_id)
...
class StatusUpdatesEntryMixin(DiffCommentsSerializerMixin, ReviewEntryMixin):
    ...
    def build_etag_data(cls, data):
        ...
        if data.status_updates_enabled:
            timestamp = six.text_type(get_latest_timestamp(status_update.timestamp for status_update in data.all_status_updates))
        else:
    ...
    def are_status_updates_collapsed(self, status_updates):
        ...
        for status_update in status_updates:
            if data.last_visited and status_update.timestamp > data.last_visited:
                return False
    ...
    def add_update(self, update):
        ...
        update.comments = {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': []}
        state = update.effective_state
        if state in (StatusUpdate.DONE_FAILURE,
         StatusUpdate.ERROR,
         StatusUpdate.TIMEOUT):
            update.header_class = b'status-update-state-failure'
        elif state == StatusUpdate.PENDING:
            update.header_class = b'status-update-state-pending'
        elif state == StatusUpdate.DONE_SUCCESS:
            update.header_class = b'status-update-state-success'
        else:
            raise ValueError(b'Unexpected state "%s"' % state)
        if state == StatusUpdate.TIMEOUT:
            description = _(b'timed out.')
        else:
            description = update.description
        update.summary_html = render_to_string(b'reviews/status_update_summary.html', {b'description': description, 
           b'header_class': update.header_class, 
           b'summary': update.summary, 
           b'url': update.url, 
           b'url_text': update.url_text})
    ...
    def finalize(self):
        ...
        if self.state_counts[StatusUpdate.DONE_FAILURE] > 0:
            summary_parts.append(_(b'%s failed') % self.state_counts[StatusUpdate.DONE_FAILURE])
        if self.state_counts[StatusUpdate.DONE_SUCCESS] > 0:
            summary_parts.append(_(b'%s succeeded') % self.state_counts[StatusUpdate.DONE_SUCCESS])
        if self.state_counts[StatusUpdate.PENDING] > 0:
            summary_parts.append(_(b'%s pending') % self.state_counts[StatusUpdate.PENDING])
        if self.state_counts[StatusUpdate.ERROR] > 0:
            summary_parts.append(_(b'%s failed with error') % self.state_counts[StatusUpdate.ERROR])
        if self.state_counts[StatusUpdate.TIMEOUT] > 0:
            summary_parts.append(_(b'%s timed out') % self.state_counts[StatusUpdate.TIMEOUT])
        if self.state_counts[StatusUpdate.DONE_FAILURE] > 0 or self.state_counts[StatusUpdate.ERROR] > 0 or self.state_counts[StatusUpdate.TIMEOUT] > 0:
            self.state_summary_class = b'status-update-state-failure'
        elif self.state_counts[StatusUpdate.PENDING]:
            self.state_summary_class = b'status-update-state-pending'
        elif self.state_counts[StatusUpdate.DONE_SUCCESS]:
            self.state_summary_class = b'status-update-state-success'
        self.state_summary = (b', ').join(summary_parts)
    ...
    def get_js_model_data(self):
        ...
        """
        diff_comments_data = list(chain.from_iterable(self.serialize_diff_comments_js_model_data(update.comments[b'diff_comments']) for update in self.status_updates if update.comments[b'diff_comments']))
        reviews_data = [ self.serialize_review_js_model_data(update.review) for update in self.status_updates if update.review_id is not None
                       ]
        model_data = {b'pendingStatusUpdates': self.state_counts[StatusUpdate.PENDING] > 0}
        model_data.update({key:value for key, value in (
         (
          b'diffCommentsData', diff_comments_data),
         (
          b'reviewsData', reviews_data)) if value if value})
        return model_data
...
class ReviewRequestEntry(BaseReviewRequestPageEntry):
    ...
    """
    entry_type_id = b'review-request'
...
class InitialStatusUpdatesEntry(StatusUpdatesEntryMixin, BaseReviewRequestPageEntry):
    ...
    by this class.
    """
    entry_type_id = b'initial_status_updates'
    entry_pos = BaseReviewRequestPageEntry.ENTRY_POS_INITIAL
    template_name = b'reviews/entries/initial_status_updates.html'
    js_model_class = b'RB.ReviewRequestPage.StatusUpdatesEntry'
    js_view_class = b'RB.ReviewRequestPage.InitialStatusUpdatesEntryView'
    ...
    def __init__(self, data):
        ...
        """
        timestamps = [
         data.review_request.time_added] + [ status_update.timestamp for status_update in data.initial_status_updates
                                           ]
        StatusUpdatesEntryMixin.__init__(self)
        BaseReviewRequestPageEntry.__init__(self, data=data, entry_id=b'0', added_timestamp=data.review_request.time_added, updated_timestamp=get_latest_timestamp(timestamps))
    ...
    def calculate_collapsed(self):
        ...
        """
        data = self.data
        return (data.last_visited or len(data.changedescs) > 0) and self.are_status_updates_collapsed(data.initial_status_updates)


class ReviewEntry(ReviewEntryMixin, DiffCommentsSerializerMixin, BaseReviewRequestPageEntry):
    """A review box.
    ...
    """
    entry_type_id = b'review'
    needs_reviews = True
    template_name = b'reviews/entries/review.html'
    js_model_class = b'RB.ReviewRequestPage.ReviewEntry'
    js_view_class = b'RB.ReviewRequestPage.ReviewEntryView'
    ...
    def build_entries(cls, data):
        ...
        for review in data.reviews:
            if not review.public or review.is_reply() or data.status_updates_enabled and hasattr(review, b'status_update'):
                continue
            entry = cls(data=data, review=review)
    ...
    def __init__(self, data, review):
        ...
        self.comments = {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': []}
        updated_timestamp = data.latest_timestamps_by_review_id.get(review.pk, review.timestamp)
        super(ReviewEntry, self).__init__(data=data, entry_id=six.text_type(review.pk), added_timestamp=review.timestamp, updated_timestamp=updated_timestamp, avatar_user=review.user)
    ...
    def get_dom_element_id(self):
        ...
        Returns:
            ...
            The ID used for the element.
        """
        return b'%s%s' % (self.entry_type_id, self.review.pk)
    ...
    def is_entry_new(self, last_visited, user, **kwargs):
        ...
        Returns:
            ...
            will be shown as an existing entry.
        """
        return self.review.is_new_for_user(user=user, last_visited=last_visited)
    ...
    def get_js_model_data(self):
        ...
        Returns:
            ...
            A dictionary of attributes to pass to the Model instance.
        """
        model_data = {b'reviewData': self.serialize_review_js_model_data(self.review)}
        diff_comments_data = self.serialize_diff_comments_js_model_data(self.comments[b'diff_comments'])
        if diff_comments_data:
            model_data[b'diffCommentsData'] = diff_comments_data
        return model_data
...
class ChangeEntry(StatusUpdatesEntryMixin, BaseReviewRequestPageEntry):
    ...
    """
    entry_type_id = b'changedesc'
    needs_changedescs = True
    needs_file_attachments = True
    needs_screenshots = True
    template_name = b'reviews/entries/change.html'
    js_model_class = b'RB.ReviewRequestPage.ChangeEntry'
    js_view_class = b'RB.ReviewRequestPage.ChangeEntryView'
    ...
    def build_entries(cls, data):
        ...
        for changedesc in data.changedescs:
            entry = cls(data=data, changedesc=changedesc)
            entry.populate_status_updates(data.change_status_updates.get(changedesc.pk, []))
            yield entry
    ...
    def __init__(self, data, changedesc):
        ...
        timestamps = [
         changedesc.timestamp] + [ status_update.timestamp for status_update in status_updates
                                 ]
        BaseReviewRequestPageEntry.__init__(self, data=data, entry_id=six.text_type(changedesc.pk), added_timestamp=changedesc.timestamp, updated_timestamp=get_latest_timestamp(timestamps), avatar_user=changedesc.get_user(review_request))
        if data.status_updates_enabled:
            StatusUpdatesEntryMixin.__init__(self)
        cur_field_changed_group = None
        status_change = changedesc.fields_changed.get(b'status')
        if status_change:
            assert b'new' in status_change
            self.new_status = ReviewRequest.status_to_string(status_change[b'new'][0])
        else:
            self.new_status = None
        fieldsets = get_review_request_fieldsets(include_change_entries_only=True)
        for fieldset in fieldsets:
            for field_cls in fieldset.field_classes:
                ...
                if not cur_field_changed_group or cur_field_changed_group[b'inline'] != inline:
                    cur_field_changed_group = {b'inline': inline, 
                       b'fields': []}
                    self.fields_changed_groups.append(cur_field_changed_group)
                if issubclass(field_cls, ReviewRequestPageDataMixin):
                    field = field_cls(review_request, request=request, data=data)
                else:
                    field = field_cls(review_request, request=request)
                cur_field_changed_group[b'fields'] += field.get_change_entry_sections_html(changedesc.fields_changed[field_id])
    ...
    def get_dom_element_id(self):
        ...
        Returns:
            ...
            The ID used for the element.
        """
        return b'%s%s' % (self.entry_type_id, self.changedesc.pk)
    ...
    def is_entry_new(self, last_visited, user, **kwargs):
        ...
        Returns:
            ...
            will be shown as an existing entry.
        """
        return self.changedesc.is_new_for_user(user=user, last_visited=last_visited, model=self.data.review_request)
    ...
    def calculate_collapsed(self):
        ...
        status_updates = data.change_status_updates.get(changedesc.pk, [])
        return changedesc.timestamp < data.latest_changedesc_timestamp and (not status_updates or self.are_status_updates_collapsed(status_updates))
...
class ReviewRequestPageEntryRegistry(OrderedRegistry):
    ...
    lookup_attrs = [
     b'entry_type_id']
    errors = {ALREADY_REGISTERED: _(b'This review request page entry is already registered.'), 
       ATTRIBUTE_REGISTERED: _(b'A review request page entry with the entry_type_id "%(attr_value)s" is already registered by another entry (%(duplicate)s).'), 
       NOT_REGISTERED: _(b'"%(attr_value)s" is not a registered review request page entry ID.')}
    ...
    def get_entry(self, entry_type_id):
        ...
        """
        return self.get(b'entry_type_id', entry_type_id)
