...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_filediff.py
# Compiled at: 2020-02-11 04:03:57
from __future__ import unicode_literals
from django.utils import six
from reviewboard.webapi.resources import resources
from reviewboard.webapi.tests.base import BaseWebAPITestCase
from reviewboard.webapi.tests.mimetypes import filediff_item_mimetype, filediff_list_mimetype
...
from reviewboard.webapi.tests.urls import get_filediff_item_url, get_filediff_list_url
...
def _compare_item(self, item_rsp, filediff):
    ...
    self.assertEqual(item_rsp[b'dest_file'], filediff.dest_file)
    self.assertEqual(item_rsp[b'source_revision'], filediff.source_revision)
    self.assertEqual(item_rsp[b'dest_detail'], filediff.dest_detail)
    self.assertEqual(item_rsp[b'status'], filediff.status_string)
...
class ResourceListTests(ReviewRequestChildListMixin, BaseWebAPITestCase):
    ...
    sample_api_url = b'review-requests/<review-request-id>/diffs/<revision>/files/'
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
        ...
        review_request = self.create_review_request(with_local_site=with_local_site, repository=repository, submitter=user, publish=True)
        diffset = self.create_diffset(review_request)
        items = []
        if populate_items:
            items.append(self.create_filediff(diffset))
        return (
         get_filediff_list_url(diffset, review_request, local_site_name),
...
class ResourceItemTests(ExtraDataItemMixin, ReviewRequestChildItemMixin, BaseWebAPITestCase):
    ...
    resource = resources.filediff
    sample_api_url = b'review-requests/<review-request-id>/diffs/<revision>/files/<file-id>/'
    compare_item = _compare_item
    fixtures = [b'test_users', b'test_scmtools']
    test_http_methods = ('GET', )
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name):
        ...
        return (
         get_filediff_item_url(filediff, review_request, local_site_name),
