...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/notifications/tests/test_email_utils.py
...
from reviewboard.notifications.email.utils import build_recipients, get_email_addresses_for_group, recipients_to_addresses
...
class EmailUtilsTests(TestCase):
    ...
    def test_recipients_to_addresses_with_string_address(self):
        ...
        with self.assertRaises(AssertionError):
            recipients_to_addresses([b'foo@example.com'])
    ...
    def test_recipients_to_addresses_with_users(self):
        ...
        """
        users = list(User.objects.filter(username__in=[b'doc', b'grumpy']))
        addresses = recipients_to_addresses(users)
        self.assertEqual(len(addresses), 2)
        expected_addresses = set(build_email_address_for_user(u) for u in users)
    ...
    def test_recipients_to_addresses_with_groups_single_mailinglist(self):
        ...
        """
        groups = [
         Group(name=b'group1', display_name=b'Group One', mailing_list=b'group1@example.com'),
         Group(name=b'group2', display_name=b'Group Two', mailing_list=b'group2@example.com')]
        addresses = recipients_to_addresses(groups)
        self.assertEqual(len(addresses), 2)
        expected_addresses = set(sum((get_email_addresses_for_group(group) for group in groups), []))
        self.assertEqual(addresses, expected_addresses)
    ...
    def test_recipients_to_addresses_with_groups_many_mailinglist(self):
        ...
        """
        groups = [
         Group(name=b'group1', display_name=b'Group One', mailing_list=b'group1a@example.com,group1b@example.com'),
         Group(name=b'group2', display_name=b'Group Two', mailing_list=b'group2a@example.com,group2b@example.com')]
        addresses = recipients_to_addresses(groups)
    ...
    def test_recipients_to_addresses_with_groups_and_users(self):
        ...
        groups = [
         Group(name=b'group1', display_name=b'Group One', mailing_list=b'group1@example.com'),
         Group(name=b'group2', display_name=b'Group Two', mailing_list=b'group2@example.com')]
        users = list(User.objects.filter(username__in=[b'doc', b'grumpy']).all())
        addresses = recipients_to_addresses(groups + users)
        self.assertEqual(len(addresses), 4)
        user_addresses = [ build_email_address_for_user(u) for u in users
                         ]
        group_addresses = sum((get_email_addresses_for_group(group) for group in groups), [])
    ...
    def test_recipients_to_addresses_with_groups_with_members(self):
        ...
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create_user(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@example.com')
        group1.users = [
         user1]
        group2.users = [user2]
    ...
    def test_recipients_to_addresses_with_groups_local_site(self):
        ...
        local sites
        """
        local_site1 = LocalSite.objects.create(name=b'local-site1')
        local_site2 = LocalSite.objects.create(name=b'local-site2')
        group1 = Group.objects.create(name=b'group1', local_site=local_site1)
        group2 = Group.objects.create(name=b'group2', local_site=local_site2)
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create_user(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@example.com')
        local_site1.users = [
         user1]
    ...
    def test_recipients_to_addresses_with_groups_inactive_members(self):
        ...
        inactive members
        """
        group1 = self.create_review_group(b'group1')
        group2 = self.create_review_group(b'group2')
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create(username=b'user2', first_name=b'User', last_name=b'Two', is_active=False, email=b'user1@example.com')
        group1.users = [
         user1]
        group2.users = [user2]
        addresses = recipients_to_addresses([group1, group2])
    ...
    def test_recipients_to_addresses_groups_local_site_inactive_members(self):
        ...
        local_site1 = LocalSite.objects.create(name=b'local-site1')
        local_site2 = LocalSite.objects.create(name=b'local-site2')
        group1 = self.create_review_group(b'group1', local_site=local_site1)
        group2 = self.create_review_group(b'group2', local_site=local_site2)
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create(username=b'user2', first_name=b'User', last_name=b'Two', is_active=False, email=b'user2@example.com')
        local_site1.users = [
         user1]
    ...
    def test_build_recipients_target_people_inactive_and_groups(self):
        ...
        """
        group = self.create_review_group()
        user = User.objects.create(username=b'user', first_name=b'User', last_name=b'Foo', is_active=False, email=b'user@example.com')
        review_request = self.create_review_request()
        review_request.target_people = [user]
        review_request.target_groups = [group]
        submitter = review_request.submitter
    ...
    def test_build_recipients_target_people_inactive(self):
        ...
        submitter = review_request.submitter
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@example.com', is_active=False)
        review_request.target_people = [
         user1, user2]
        to, cc = build_recipients(submitter, review_request)
    ...
    def test_build_recipients_target_people_local_site_no_email(self):
        ...
        local_site = LocalSite.objects.create(name=self.local_site_name)
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create_user(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@exmaple.com')
        Profile.objects.create(user=user2, should_send_email=False)
        local_site.users = [
         user1, user2]
        review_request = self.create_review_request(with_local_site=True)
    ...
    def test_build_recipients_limit_to(self):
        ...
        review_request.target_people = [
         dopey]
        review_request.target_groups = [group]
        to, cc = build_recipients(submitter, review_request, limit_recipients_to=[
         grumpy])
    ...
    def test_build_recipients_limit_to_inactive(self):
        ...
        """
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@exmaple.com', is_active=False)
        review_request = self.create_review_request()
        submitter = review_request.submitter
        to, cc = build_recipients(submitter, review_request, limit_recipients_to=[
         user1, user2])
    ...
    def test_build_recipients_extra_recipients(self):
        ...
        submitter = review_request.submitter
        grumpy = User.objects.get(username=b'grumpy')
        to, cc = build_recipients(submitter, review_request, extra_recipients=[
         grumpy])
        self.assertEqual(to, set([submitter, grumpy]))
        self.assertEqual(len(cc), 0)
    ...
    def test_build_recipients_extra_recipients_and_limit_to(self):
        ...
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create_user(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user2@exmaple.com')
        user3 = User.objects.create_user(username=b'user3', first_name=b'User', last_name=b'Three', email=b'user3@exmaple.com')
        group = self.create_review_group()
        review_request = self.create_review_request()
        submitter = review_request.submitter
    ...
    def test_build_recipients_starred_inactive(self):
        ...
        submitter = review_request.submitter
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user@exmaple.com', is_active=False)
    ...
    def test_build_recipients_starred_local_site(self):
        ...
        submitter = review_request.submitter
        user1 = User.objects.create_user(username=b'user1', first_name=b'User', last_name=b'One', email=b'user1@example.com')
        user2 = User.objects.create_user(username=b'user2', first_name=b'User', last_name=b'Two', email=b'user@exmaple.com')
        local_site1.users = [
         user1]
        local_site2.users = [user2]
        profile1 = Profile.objects.create(user=user1)
