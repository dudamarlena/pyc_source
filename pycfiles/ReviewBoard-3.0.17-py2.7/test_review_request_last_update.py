...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_review_request_last_update.py
...
from reviewboard.reviews.models import Review, ReviewRequest, ReviewRequestDraft
...
class ResourceTests(BaseWebAPITestCase):
    ...
    sample_api_url = b'review-requests/<id>/last-update/'
    ...
    def compare_item(self, item_rsp, review_request, expected_user=None):
        ...
        item_type = item_rsp[b'type']
        last_updated = review_request.last_updated.replace(microsecond=0)
        last_activity = review_request.get_last_activity_info()
        updated_object = last_activity[b'updated_object']
        changedesc = last_activity[b'changedesc']
        rsp_timestamp = parse_date(item_rsp[b'timestamp']).replace(microsecond=0)
        if item_type == b'review-request':
            ...
            object_timestamp = updated_object.timestamp
            self.assertIsNone(last_activity[b'changedesc'])
        else:
            self.fail(b'Unknown item type "%s"' % item_type)
        ...
        if changedesc:
            self.assertEqual(expected_user, changedesc.get_user(review_request))
        self.assertEqual(expected_user.pk, item_rsp[b'user'][b'id'])
        return

    def setup_basic_get_test(self, user, with_local_site=False, local_site_name=None, update_review_request=False, update_diffset=False, update_review=False, update_reply=False, publish_user=None):
        ...
        review_request = self.create_review_request(create_repository=True, submitter=user, with_local_site=with_local_site, target_people=[
         user])
        review_request.publish(user=publish_user)
        if update_review_request:
            draft = ReviewRequestDraft.create(review_request)
            draft.summary = b'%s updated' % review_request.summary
            draft.save()
            review_request.publish(user=publish_user)
        if update_diffset:
            ...
            review_request.publish(user=publish_user)
        if update_review:
            review = self.create_review(review_request=review_request, user=user, publish=False)
            review.publish(user=publish_user)
            if update_reply:
                reply = self.create_reply(review, user=user)
                reply.publish(user=publish_user)
        return (
         get_review_request_last_update_url(local_site_name=local_site_name, review_request=review_request),
    ...
    def test_get_other_user(self):
        ...
        by another user
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review-request')
    ...
    def test_get_review_request_updated(self):
        ...
        """
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_review_request=True)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review-request')
    ...
    def test_get_review_request_updated_other_user(self):
        ...
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_review_request=True, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review-request')
    ...
    def test_get_diff_updated(self):
        ...
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_diffset=True)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'diff')
        self.compare_item(rsp[b'last_update'], review_request)
    ...
    def test_get_diff_updated_other_user(self):
        ...
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_diffset=True, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'diff')
    ...
    def test_get_review_published(self):
        ...
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_review=True)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review')
        self.compare_item(item_rsp, review_request)
    ...
    def test_get_review_published_other_user(self):
        ...
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_review=True, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review')
    ...
    def test_get_review_published_not_submitter(self):
        ...
        """
        submitter = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(submitter)
        self.create_review(review_request=review_request, user=self.user, publish=True)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review')
    ...
    def test_get_reply_published(self):
        ...
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_reply=True)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'reply')
        self.compare_item(item_rsp, review_request)
    ...
    def test_get_reply_published_other_user(self):
        ...
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_reply=True, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'reply')
    ...
    def test_get_reply_published_not_submitter(self):
        ...
        submitter = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(submitter)
        review = self.create_review(review_request=review_request, user=self.user, publish=True)
    ...
    def test_get_review_request_updated_review_published_other_user(self):
        ...
        """
        publish_user = User.objects.get(username=b'admin')
        url, expected_mimetype, review_request = self.setup_basic_get_test(self.user, update_review_request=True, update_review=True, publish_user=publish_user)
        rsp = self.api_get(url, expected_mimetype=expected_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        item_rsp = rsp[b'last_update']
        self.assertEqual(item_rsp[b'type'], b'review')
