...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/site/middleware.py
...
class LocalSiteMiddleware(object):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        """
        local_site_name = view_kwargs.get(b'local_site_name')
        request._local_site_name = local_site_name
        if request._local_site_name:
            request.local_site = SimpleLazyObject(lambda : get_object_or_none(LocalSite, name=local_site_name))
        else:
