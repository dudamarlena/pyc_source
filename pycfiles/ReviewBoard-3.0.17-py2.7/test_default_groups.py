...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_default_groups.py
...
class DefaultGroupTest(SpyAgency, TestCase):
    ...
    def test_user_registration(self):
        ...
        user = User.objects.create_user(username=b'reviewboard', email=b'user@example.com', password=b'password')
        user_registered.send(sender=None, user=user)
        self.assertTrue(_add_default_groups.called)
        self.assertTrue(_add_default_groups.last_called_with(user=user, local_site=None))
        return
    ...
    def test_local_site_add_user(self):
        ...
        self.assertTrue(_add_default_groups.called)
        self.assertTrue(_add_default_groups.last_called_with(user=user, local_site=local_site))
    ...
    def test_user_add_local_site(self):
        ...
        self.assertTrue(_add_default_groups.spy.last_called_with(user=user, local_site=local_site))
    ...
    def test_add_default_groups(self):
        ...
        self.assertEqual(group_count_before, User.objects.get(pk=user.id).review_groups.count())
        self.create_review_group(is_default_group=True)
        _add_default_groups(sender=None, user=user)
        self.assertEqual(group_count_before + 1, User.objects.get(pk=user.id).review_groups.count())
        return
