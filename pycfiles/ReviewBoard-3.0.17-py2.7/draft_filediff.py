...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/draft_filediff.py
...
class DraftFileDiffResource(FileDiffResource):
    ...
    added_in = b'2.0'
    name = b'draft_file'
    policy_id = b'draft_file_diff'
    uri_name = b'files'
    allowed_methods = ('GET', 'PUT')
    item_result_key = b'file'
    list_result_key = b'files'
    mimetype_list_resource_name = b'files'
    mimetype_item_resource_name = b'file'
    ...
    def get_queryset(self, request, diff_revision, *args, **kwargs):
        ...
        return self.model.objects.filter(diffset__review_request_draft=draft, diffset__revision=diff_revision)
    ...
    @webapi_request_fields(optional={b'dest_attachment_file': {b'type': file, 
                                 b'description': b'The file attachment to upload, representing the modified file. This can only be used for binary files.'}}, allow_unknown=True)
    def update(self, request, extra_fields={}, *args, **kwargs):
        ...
        if b'dest_attachment_file' in request.FILES:
            if not filediff.binary:
                return (INVALID_FORM_DATA,
                 {b'fields': {b'dest_attachment_file': [
                                                        b'Cannot upload a file attachment to a non-binary file in a diff.']}})
            try:
                FileAttachment.objects.get_for_filediff(filediff)
                return (
                 INVALID_FORM_DATA,
                 {b'fields': {b'dest_attachment_file': [
                                                        b'There is already a file attachment associated with this binary file.']}})
            except ObjectDoesNotExist:
                pass

            dest_attachment_file = request.FILES.get(b'dest_attachment_file')
            form = UploadFileForm(review_request_draft.review_request, {}, {b'path': dest_attachment_file})
