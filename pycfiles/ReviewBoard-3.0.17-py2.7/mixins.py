...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/mixins.py
...
class BasicTestsMetaclass(type):
    ...
    def __new__(meta, name, bases, d):
        test_local_sites = d.get(b'test_local_sites', True)
        test_api_token_access = d.get(b'test_api_token_access', True)
        test_oauth_token_access = d.get(b'test_oauth_token_access', True)
        resource = d[b'resource']
        is_singleton = False
        is_list = False
        if b'test_http_methods' in d:
            test_http_methods = d[b'test_http_methods']
        else:
            test_http_methods = ('DELETE', 'GET', 'POST', 'PUT')
            d[b'test_http_methods'] = test_http_methods
        if hasattr(resource, b'required_features') and resource.required_features:
            d[b'override_features'] = {feature.feature_id:True for feature in resource.required_features}
        else:
            d[b'override_features'] = {}
        if name == b'ResourceListTests':
            is_list = True
        elif name == b'ResourceTests':
            is_singleton = True
        if b'DELETE' in test_http_methods and not is_list:
            if b'DELETE' not in resource.allowed_methods:
                mixins = (
                 BasicDeleteNotAllowedTestsMixin,)
            elif test_local_sites:
                mixins = (
                 BasicDeleteTestsWithLocalSiteMixin,)
                ...
                mixins = (
                 BasicDeleteTestsMixin,)
            bases = mixins + bases
        if b'GET' in test_http_methods:
            if is_list:
                if test_local_sites:
                    mixins = (
                     BasicGetListTestsWithLocalSiteMixin,)
                    if test_api_token_access:
                        mixins += (
                    ...
                    mixins = (
                     BasicGetListTestsMixin,)
            elif test_local_sites:
                mixins = (
                 BasicGetItemTestsWithLocalSiteMixin,)
                if test_api_token_access:
                    mixins += (
                ...
                mixins = (
                 BasicGetItemTestsMixin,)
            bases = mixins + bases
        if b'POST' in test_http_methods and (is_list or is_singleton):
            if b'POST' not in resource.allowed_methods:
                mixins = (
                 BasicPostNotAllowedTestsMixin,)
            elif test_local_sites:
                mixins = (
                 BasicPostTestsWithLocalSiteMixin,)
                if test_api_token_access:
                ...
                mixins = (
                 BasicPostTestsMixin,)
            bases = mixins + bases
        if b'PUT' in test_http_methods and not is_list:
            if b'PUT' not in resource.allowed_methods:
                mixins = (
                 BasicPutNotAllowedTestsMixin,)
            elif test_local_sites:
                mixins = (
                 BasicPutTestsWithLocalSiteMixin,)
                if test_api_token_access:
...
class BasicTestsMixin(object):
    ...
    def compare_item(self, item_rsp, obj):
        raise NotImplementedError(b"%s doesn't implement compare_item" % self.__class__.__name__)
    ...
    def _authenticate_basic_tests(self, with_local_site=False, with_admin=False, with_webapi_token=False, webapi_token_local_site_id=None, with_oauth_token=False, oauth_application_enabled=True, user=None):
        if user is None:
            user = self._login_user(local_site=with_local_site, admin=with_admin)
        session = self.client.session
        if with_webapi_token:
            webapi_token = WebAPIToken.objects.get_or_create(user=user, token=b'abc123', local_site_id=webapi_token_local_site_id)[0]
            session[b'webapi_token_id'] = webapi_token.pk
        if with_oauth_token:
            application = Application.objects.get_or_create(local_site_id=webapi_token_local_site_id, user=user, defaults={b'authorization_grant_type': Application.GRANT_IMPLICIT, 
               b'client_id': generate_client_id(), 
               b'client_secret': generate_client_secret(), 
               b'client_type': Application.CLIENT_PUBLIC, 
               b'enabled': oauth_application_enabled, 
               b'name': b'Test Application', 
               b'redirect_uris': b'http://example.com', 
               b'user': user})[0]
            ...
            access_token = AccessToken.objects.create(application=application, user=user, token=b'abc123', scope=(b' ').join({b'%s:%s' % (self.resource.scope_name, self.resource.HTTP_SCOPE_METHOD_MAP[method]) for method in self.resource.allowed_methods}), expires=timezone.now() + timedelta(hours=1))
            session[b'oauth2_token_id'] = access_token.pk
...
class BasicDeleteTestsMixin(BasicTestsMixin):
    ...
    def setup_basic_delete_test(self, user, with_local_site, local_site_name):
        raise NotImplementedError(b"%s doesn't implement setup_basic_delete_test" % self.__class__.__name__)

    def check_delete_result(self, user, *args):
        raise NotImplementedError(b"%s doesn't implement check_delete_result" % self.__class__.__name__)
    ...
    def test_delete_not_owner(self):
        ...
        self.assertFalse(url.startswith(b'/s/' + self.local_site_name))
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=self.not_owner_status_code)
...
class BasicDeleteTestsWithLocalSiteMixin(BasicDeleteTestsMixin):
    ...
    def _setup_test_delete_with_site(self, **auth_kwargs):
        self.load_fixtures(self.basic_delete_fixtures)
        with_local_site = auth_kwargs.setdefault(b'with_local_site', True)
        user = self._authenticate_basic_tests(with_admin=self.basic_delete_use_admin, **auth_kwargs)
        if with_local_site:
            url, cb_args = self.setup_basic_delete_test(user, True, self.local_site_name)
        else:
            url, cb_args = self.setup_basic_delete_test(user, False, None)
        self.assertEqual(url.startswith(b'/s/' + self.local_site_name), with_local_site)
        return (
         user, url, cb_args)
...
class BasicDeleteTestsWithLocalSiteAndAPITokenMixin(object):
    ...
    def test_delete_with_restrict_site_and_allowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            self.api_delete(url)
        self.check_delete_result(user, *cb_args)
    ...
    def test_delete_with_restrict_site_and_not_allowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicDeleteTestsWithLocalSiteAndOAuthTokenMixin(object):
    ...
    def test_delete_site_with_oauth_token_enabled_allowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            self.api_delete(url)
        self.check_delete_result(user, *cb_args)
    ...
    def test_delete_site_with_oauth_token_enabled_disallowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_delete_site_with_oauth_token_disabled_disallowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id, oauth_application_enabled=False)
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_delete_no_site_with_site_oauth_token_disallowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_local_site=False, with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_delete_site_with_global_oauth_token_disallowed(self):
        ...
        """
        user, url, cb_args = self._setup_test_delete_with_site(with_oauth_token=True, webapi_token_local_site_id=None)
        with override_feature_checks(self.override_features):
            rsp = self.api_delete(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicDeleteNotAllowedTestsMixin(BasicTestsMixin):
    ...
    def setup_http_not_allowed_item_test(self, user):
        raise NotImplementedError(b"%s doesn't implement setup_http_not_allowed_item_test" % self.__class__.__name__)
...
class BasicGetItemTestsMixin(BasicTestsMixin):
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name):
        raise NotImplementedError(b"%s doesn't implement setup_basic_get_test" % self.__class__.__name__)
...
class BasicGetItemTestsWithLocalSiteMixin(BasicGetItemTestsMixin):
    ...
    def _setup_test_get_with_site(self, **auth_kwargs):
        self.load_fixtures(self.basic_get_fixtures)
        with_local_site = auth_kwargs.setdefault(b'with_local_site', True)
        user = self._authenticate_basic_tests(with_admin=self.basic_get_use_admin, **auth_kwargs)
        if with_local_site:
            url, mimetype, item = self.setup_basic_get_test(user, True, self.local_site_name)
        else:
            url, mimetype, item = self.setup_basic_get_test(user, False, None)
        self.assertEqual(url.startswith(b'/s/' + self.local_site_name), with_local_site)
        return (
         user, url, mimetype, item)
...
class BasicGetItemTestsWithLocalSiteAndAPITokenMixin(object):
    ...
    def test_get_with_restrict_site_and_allowed(self):
        ...
        and session restricted to the site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_mimetype=mimetype, expected_json=self.basic_get_returns_json)
        if self.basic_get_returns_json:
            self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def test_get_with_restrict_site_and_not_allowed(self):
        ...
        and session restricted to a different site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicGetItemTestsWithLocalsSiteAndOAuthTokenMixin(object):
    ...
    def test_get_site_with_oauth_token_enabled_allowed(self):
        ...
        OAuth token for an enabled application on the current site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_mimetype=mimetype, expected_json=self.basic_get_returns_json)
        if self.basic_get_returns_json:
            self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def test_get_site_with_oauth_token_enabled_disallowed(self):
        ...
        OAuth token for an enabled application on a different site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_site_with_oauth_token_disabled_disallowed(self):
        ...
        OAuth token for a disabled application on the current site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id, oauth_application_enabled=False)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_no_site_with_site_oauth_token_disallowed(self):
        ...
        OAuth token meant for a local site on the root
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_local_site=False, with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_site_with_global_oauth_token_disallowed(self):
        ...
        OAuth token for the root on a local site
        """
        user, url, mimetype, item = self._setup_test_get_with_site(with_oauth_token=True, webapi_token_local_site_id=None)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicGetListTestsMixin(BasicTestsMixin):
    ...
    def test_get(self):
        ...
        self.load_fixtures(self.basic_get_fixtures)
        self._login_user(admin=self.basic_get_use_admin)
        url, mimetype, items = self.setup_basic_get_test(self.user, False, None, True)
        self.assertFalse(url.startswith(b'/s/' + self.local_site_name))
        with override_feature_checks(self.override_features):
...
class BasicGetListTestsWithLocalSiteMixin(BasicGetListTestsMixin):
    ...
    def _setup_test_get_list_with_site(self, **auth_kwargs):
        self.load_fixtures(self.basic_get_fixtures)
        with_local_site = auth_kwargs.setdefault(b'with_local_site', True)
        user = self._login_user(local_site=with_local_site, admin=self.basic_get_use_admin)
        if with_local_site:
            url, mimetype, items = self.setup_basic_get_test(user, True, self.local_site_name, True)
        else:
            url, mimetype, items = self.setup_basic_get_test(user, False, None, True)
        user = self._authenticate_basic_tests(with_admin=self.basic_get_use_admin, user=user, **auth_kwargs)
        self.assertEqual(url.startswith(b'/s/' + self.local_site_name), with_local_site)
        return (
         user, url, mimetype, items)
...
class BasicGetListTestsWithLocalSiteAndAPITokenMixin(object):
    ...
    def test_get_with_restrict_site_and_allowed(self):
        ...
        and session restricted to the site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.list_result_key, rsp)
    ...
    def test_get_with_restrict_site_and_not_allowed(self):
        ...
        and session restricted to a different site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicGetListTestsWithLocalSiteAndOAuthTokenMixin(object):
    ...
    def test_get_list_site_with_oauth_token_enabled_allowed(self):
        ...
        OAuth token for an enabled application on the current site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.list_result_key, rsp)
    ...
    def test_get_list_site_with_oauth_token_enabled_disallowed(self):
        ...
        OAuth token for an enabled application on a different site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_list_site_with_oauth_token_disabled_disallowed(self):
        ...
        OAuth token for a disabled application on the current site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id, oauth_application_enabled=False)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_list_no_site_with_site_oauth_token_disallowed(self):
        ...
        OAuth token meant for a local site on the root
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_local_site=False, with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_get_list_site_with_global_oauth_token_disallowed(self):
        ...
        OAuth token for the root on a local site
        """
        user, url, mimetype, items = self._setup_test_get_list_with_site(with_oauth_token=True, webapi_token_local_site_id=None)
        with override_feature_checks(self.override_features):
            rsp = self.api_get(url, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicPostTestsMixin(BasicTestsMixin):
    ...
    def setup_basic_post_test(self, user, with_local_site, local_site_name, post_valid_data):
        raise NotImplementedError(b"%s doesn't implement setup_basic_post_test" % self.__class__.__name__)

    def check_post_result(self, user, rsp, *args):
        raise NotImplementedError(b"%s doesn't implement check_post_result" % self.__class__.__name__)
    ...
    def test_post(self):
        ...
        url, mimetype, post_data, cb_args = self.setup_basic_post_test(self.user, False, None, True)
        self.assertFalse(url.startswith(b'/s/' + self.local_site_name))
        with override_feature_checks(self.override_features):
...
class BasicPostTestsWithLocalSiteMixin(BasicPostTestsMixin):
    ...
    def test_post_with_site(self):
        ...
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site()
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_mimetype=mimetype)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def _setup_test_post_with_site(self, **auth_kwargs):
        self.load_fixtures(self.basic_post_fixtures)
        with_local_site = auth_kwargs.setdefault(b'with_local_site', True)
        user = self._authenticate_basic_tests(with_admin=self.basic_post_use_admin, **auth_kwargs)
        if with_local_site:
            url, mimetype, post_data, cb_args = self.setup_basic_post_test(user, True, self.local_site_name, True)
        else:
            url, mimetype, post_data, cb_args = self.setup_basic_post_test(user, False, None, True)
        self.assertEqual(url.startswith(b'/s/' + self.local_site_name), with_local_site)
        return (
...
class BasicPostTestsWithLocalSiteAndAPITokenMixin(object):
    ...
    def test_post_with_restrict_site_and_allowed(self):
        ...
        and session restricted to the site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_mimetype=mimetype)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def test_post_with_restrict_site_and_not_allowed(self):
        ...
        and session restricted to a different site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_status=403)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'fail')
...
class BasicPostTestsWithLocalSiteAndOAuthTokenMixin(object):
    ...
    def test_post_site_with_oauth_token_enabled_allowed(self):
        ...
        OAuth token for an enabled application on the current site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_mimetype=mimetype)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def test_post_site_with_oauth_token_enabled_disallowed(self):
        ...
        OAuth token for an enabled application on a different site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_status=403)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'fail')
    ...
    def test_post_site_with_oauth_token_disabled_disallowed(self):
        ...
        OAuth token for a disabled application on the current site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id, oauth_application_enabled=False)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_status=403)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'fail')
    ...
    def test_post_no_site_with_site_oauth_token_disallowed(self):
        ...
        OAuth token meant for a local site on the root
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_local_site=False, with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_status=403)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'fail')
    ...
    def test_post_site_with_global_oauth_token_disallowed(self):
        ...
        OAuth token for the root on a local site
        """
        user, url, mimetype, post_data, cb_args = self._setup_test_post_with_site(with_oauth_token=True, webapi_token_local_site_id=None)
        with override_feature_checks(self.override_features):
            rsp = self.api_post(url, post_data, expected_status=403)
        self._close_file_handles(post_data)
        self.assertEqual(rsp[b'stat'], b'fail')
...
class BasicPostNotAllowedTestsMixin(BasicTestsMixin):
    ...
    def setup_http_not_allowed_list_test(self, user):
        raise NotImplementedError(b"%s doesn't implement setup_http_not_allowed_list_test" % self.__class__.__name__)
...
class BasicPutTestsMixin(BasicTestsMixin):
    ...
    def setup_basic_put_test(self, user, with_local_site, local_site_name, put_valid_data):
        raise NotImplementedError(b"%s doesn't implement setup_basic_put_test" % self.__class__.__name__)

    def check_put_result(self, user, item_rsp, item, *args):
        raise NotImplementedError(b"%s doesn't implement check_put_result" % self.__class__.__name__)
    ...
    def test_put(self):
        ...
        url, mimetype, put_data, item, cb_args = self.setup_basic_put_test(self.user, False, None, True)
        self.assertFalse(url.startswith(b'/s/' + self.local_site_name))
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.item_result_key, rsp)
        self.check_put_result(self.user, rsp[self.resource.item_result_key], item, *cb_args)
        return
    ...
    def test_put_not_owner(self):
        ...
        self.assertFalse(url.startswith(b'/s/' + self.local_site_name))
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=self.not_owner_status_code)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], self.not_owner_error.code)
        return
...
class BasicPutTestsWithLocalSiteMixin(BasicPutTestsMixin):
    ...
    def test_put_with_site(self):
        ...
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site()
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.item_result_key, rsp)
        self.check_put_result(user, rsp[self.resource.item_result_key], item, *cb_args)
    ...
    def _setup_test_put_with_site(self, **auth_kwargs):
        self.load_fixtures(self.basic_put_fixtures)
        with_local_site = auth_kwargs.setdefault(b'with_local_site', True)
        user = self._authenticate_basic_tests(with_admin=self.basic_put_use_admin, **auth_kwargs)
        if with_local_site:
            url, mimetype, put_data, item, cb_args = self.setup_basic_put_test(user, True, self.local_site_name, True)
        else:
            url, mimetype, put_data, item, cb_args = self.setup_basic_put_test(user, False, None, True)
        self.assertEqual(url.startswith(b'/s/' + self.local_site_name), with_local_site)
        return (
...
class BasicPutTestsWithLocalSiteAndAPITokenMixin(object):
    ...
    def test_put_with_restrict_site_and_allowed(self):
        ...
        and session restricted to the site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.item_result_key, rsp)
    ...
    def test_put_with_restrict_site_and_not_allowed(self):
        ...
        and session restricted to a different site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_webapi_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BasicPutTestsWithLocalSiteAndOAuthTokenMixin(object):
    ...
    def test_put_site_with_oauth_token_enabled_allowed(self):
        ...
        OAuth token for an enabled application on the current site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_mimetype=mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertIn(self.resource.item_result_key, rsp)
    ...
    def test_put_site_with_oauth_token_enabled_disallowed(self):
        ...
        OAuth token for an enabled application on a different site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id + 1)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_put_site_with_oauth_token_disabled_disallowed(self):
        ...
        OAuth token for a disabled application on the current site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_oauth_token=True, webapi_token_local_site_id=self.local_site_id, oauth_application_enabled=False)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_put_no_site_with_site_oauth_token_disallowed(self):
        ...
        OAuth token meant for a local site on the root
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_local_site=False, with_oauth_token=True, webapi_token_local_site_id=self.local_site_id)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
    ...
    def test_put_site_with_global_oauth_token_disallowed(self):
        ...
        OAuth token for the root on a local site
        """
        user, url, mimetype, put_data, item, cb_args = self._setup_test_put_with_site(with_oauth_token=True, webapi_token_local_site_id=None)
        with override_feature_checks(self.override_features):
            rsp = self.api_put(url, put_data, expected_status=403)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], PERMISSION_DENIED.code)
...
class BaseReviewRequestChildMixin(object):
    ...
    def setup_review_request_child_test(self, review_request):
        raise NotImplementedError(b"%s doesn't implement setup_review_request_child_test" % self.__class__.__name__)
    ...
    def test_get_with_private_group(self):
        ...
        with override_feature_checks(self.override_features):
            self.api_get(url, expected_mimetype=mimetype, expected_json=self.basic_get_returns_json)
