...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/test.py
...
class RBTestRunner(TestRunner):
    ...
    def run_tests(self, *args, **kwargs):
        ...
        Returns:
            ...
            failures.
        """
        if b'--with-profiling' in sys.argv:
            sys.stderr.write(b'--with-profiling is no longer supported. Use --with-profile instead.\n')
            sys.exit(1)
        return super(RBTestRunner, self).run_tests(*args, **kwargs)

    def setup_dirs(self):
        settings.SITE_DATA_DIR = os.path.join(self.tempdir, b'data')
        images_dir = os.path.join(settings.MEDIA_ROOT, b'uploaded', b'images')
