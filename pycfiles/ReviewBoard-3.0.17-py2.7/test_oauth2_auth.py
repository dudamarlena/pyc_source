...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_oauth2_auth.py
...
class OAuth2TokenAuthTests(WebAPITestCaseMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.owner = User.objects.get(username=b'doc')
    ...
    def test_auth(self):
        ...
        token = self.create_oauth_token(application, self.user, b'session:read')
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_mimetype=session_mimetype)
    ...
    def test_auth_disabled_app(self):
        ...
        """
        application = self.create_oauth_application(user=self.owner, enabled=False)
        token = self.create_oauth_token(application, self.user, b'session:read')
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_status=401)
        self.assertIn(b'stat', rsp)
    ...
    def test_auth_expired(self):
        ...
        """
        application = self.create_oauth_application(user=self.owner)
        token = self.create_oauth_token(application, self.user, b'session:read', expires=timedelta(hours=-1))
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_status=401)
        self.assertIn(b'stat', rsp)
    ...
    def test_auth_invalid_scope(self):
        ...
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_status=403)
        self.assertIn(b'stat', rsp)
        self.assertEqual(rsp[b'stat'], b'fail')
    ...
    def test_auth_local_site(self):
        ...
        application = self.create_oauth_application(user=self.owner, local_site=local_site)
        token = self.create_oauth_token(application, self.user, b'session:read')
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_status=401)
    ...
    def test_auth_no_local_site(self):
        ...
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(local_site.name), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_status=401)
        self.assertIn(b'stat', rsp)
        self.assertEqual(rsp[b'stat'], b'fail')
    ...
    def test_auth_local_site_public(self):
        ...
        with override_feature_check(oauth2_service_feature.feature_id, True):
            load_site_config()
            rsp = self.api_get(get_session_url(local_site.name), HTTP_AUTHORIZATION=b'Bearer %s' % token.token, expected_mimetype=session_mimetype)
        self.assertIn(b'stat', rsp)
        self.assertEqual(rsp[b'stat'], b'ok')
