...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/review_screenshot_comment.py
...
class ReviewScreenshotCommentResource(BaseScreenshotCommentResource):
    ...
    policy_id = b'review_screenshot_comment'
    model_parent_key = b'review'

    def get_queryset(self, request, review_id, *args, **kwargs):
        q = super(ReviewScreenshotCommentResource, self).get_queryset(request, *args, **kwargs)
        return q.filter(review=review_id)

    @webapi_check_local_site
    @webapi_login_required
    @webapi_request_fields(required=dict({b'screenshot_id': {b'type': int, 
                          b'description': b'The ID of the screenshot being commented on.'}, 
       b'x': {b'type': int, 
              b'description': b'The X location for the comment.'}, 
       b'y': {b'type': int, 
              b'description': b'The Y location for the comment.'}, 
       b'w': {b'type': int, 
              b'description': b'The width of the comment region.'}, 
       b'h': {b'type': int, 
              b'description': b'The height of the comment region.'}}, **BaseScreenshotCommentResource.REQUIRED_CREATE_FIELDS), optional=BaseScreenshotCommentResource.OPTIONAL_CREATE_FIELDS, allow_unknown=True)
    def create(self, request, screenshot_id, *args, **kwargs):
        ...
        try:
            screenshot = Screenshot.objects.get(pk=screenshot_id, review_request=review_request)
        except ObjectDoesNotExist:
            return (
             INVALID_FORM_DATA,
             {b'fields': {b'screenshot_id': [
                                             b'This is not a valid screenshot ID']}})

        return self.create_comment(review=review, comments_m2m=review.screenshot_comments, screenshot=screenshot, fields=('screenshot',
                                                                                                                          'x',
                                                                                                                          'y',
                                                                                                                          'w',
                                                                                                                          'h'), **kwargs)
    ...
    @webapi_request_fields(optional=dict({b'x': {b'type': int, 
              b'description': b'The X location for the comment.'}, 
       b'y': {b'type': int, 
              b'description': b'The Y location for the comment.'}, 
       b'w': {b'type': int, 
              b'description': b'The width of the comment region.'}, 
       b'h': {b'type': int, 
              b'description': b'The height of the comment region.'}}, **BaseScreenshotCommentResource.OPTIONAL_UPDATE_FIELDS), allow_unknown=True)
    def update(self, request, *args, **kwargs):
        ...
        return self.update_comment(request=request, review=review, comment=screenshot_comment, update_fields=('x',
                                                                                                              'y',
                                                                                                              'w',
                                                                                                              'h'), **kwargs)
