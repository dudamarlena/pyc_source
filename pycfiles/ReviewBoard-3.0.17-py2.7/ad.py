...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/backends/ad.py
...
class ActiveDirectoryBackend(BaseAuthBackend):
    ...
    """
    backend_id = b'ad'
    name = _(b'Active Directory')
    settings_form = ActiveDirectorySettingsForm
    login_instructions = _(b'Use your standard Active Directory username and password.')
    ...
    def get_ldap_search_root(self, userdomain=None):
        ...
        """
        if getattr(settings, b'AD_SEARCH_ROOT', None):
            root = [
             settings.AD_SEARCH_ROOT]
        else:
            if userdomain is None:
                userdomain = self.get_domain_name()
            root = [ b'dc=%s' % x for x in userdomain.split(b'.') ]
            if settings.AD_OU_NAME:
                root = [
                 b'ou=%s' % settings.AD_OU_NAME] + root
        return (b',').join(root)
    ...
    def search_ad(self, con, filterstr, userdomain=None):
        ...
        logger.debug(b'Search root "%s" for filter "%s"', search_root, filterstr)
    ...
    def find_domain_controllers_from_dns(self, userdomain=None):
        ...
        req = DNS.Base.DnsRequest(b'_ldap._tcp.%s' % (userdomain or self.get_domain_name()), qtype=DNS.Type.SRV).req()
        return [ answer[b'data'][-2:] for answer in req.answers ]
    ...
    def can_recurse(self, depth):
        ...
        Returns:
            ...
            if it's too deep.
        """
        return settings.AD_RECURSION_DEPTH == -1 or depth <= settings.AD_RECURSION_DEPTH
            ...
            if self.can_recurse(depth):
                for group in new_groups:
                    if group in old_seen:
                        continue
                    group_data = self.search_ad(con, filter_format(b'(&(objectClass=group)(cn=%s))', (
                     group,)))
                    seen.update(self.get_member_of(con, group_data, seen=seen, depth=depth))

            else:
                logger.warning(b'Recursive group check reached maximum recursion depth (%s)', depth)
    ...
    def get_ldap_connections(self, userdomain=None):
        ...
        if settings.AD_FIND_DC_FROM_DNS:
            ...
            for dc in dcs:
                port, host = dc
                ldap_uri = b'ldap://%s:%s' % (host, port)
                connection = ldap.initialize(ldap_uri)
                if settings.AD_USE_TLS:
                    try:
                        connection.start_tls_s()
                    except ldap.UNAVAILABLE:
                        logger.warning(b'Domain controller "%s:%d" for domain "%s" unavailable', host, int(port), userdomain)
                        continue
                    except ldap.CONNECT_ERROR:
                        logger.warning(b'Could not connect to domain controller "%s:%d" for domain "%s". The certificate may not be verifiable.', host, int(port), userdomain)
                        continue
    ...
    def authenticate(self, username, password, **kwargs):
        ...
        if ldap is None:
            logger.error(b'Attempted to authenticate user "%s" in LDAP, but the python-ldap package is not installed!', username)
            return
        else:
            user_subdomain = b''
            if b'@' in username:
                username, user_subdomain = username.split(b'@', 1)
            else:
                if b'\\' in username:
                    user_subdomain, username = username.split(b'\\', 1)
                userdomain = self.get_domain_name()
                if user_subdomain:
                    userdomain = b'%s.%s' % (user_subdomain, userdomain)
                required_group = settings.AD_GROUP_NAME
                if isinstance(required_group, six.text_type):
                    required_group = required_group.encode(b'utf-8')
                if isinstance(username, six.text_type):
                    username_bytes = username.encode(b'utf-8')
                else:
                    username_bytes = username
                if isinstance(user_subdomain, six.text_type):
                    user_subdomain = user_subdomain.encode(b'utf-8')
                ...
                for uri, connection in self.get_ldap_connections(userdomain):
                    try:
                        ...
                        user_data = self.search_ad(connection, filter_format(b'(&(objectClass=user)(sAMAccountName=%s))', (
                        ...
                        if required_group:
                            try:
                                ...
                                logger.error(b'Unable to retrieve groups for user "%s" from controller "%s": %s', username, uri, e, exc_info=1)
                                return

                            if required_group not in group_names:
                                logger.warning(b'User %s is not in required group "%s" on controller "%s"', username, required_group, uri)
                                return
                        return self.get_or_create_user(username=username, request=None, ad_user_data=user_data)
                    except ldap.SERVER_DOWN:
                        logger.warning(b'domain controller "%s" is down', uri)
                        continue
                    except ldap.INVALID_CREDENTIALS:
                        logger.warning(b'Failed login for user "%s" on controller "%s"', username, uri)
                        return

            logger.error(b'Could not contact any domain controller servers')
            return
    ...
    def get_or_create_user(self, username, request=None, ad_user_data=None):
        ...
        try:
            ...
            try:
                ...
                first_name = user_info.get(b'givenName', [username])[0]
                last_name = user_info.get(b'sn', [b''])[0]
                email = user_info.get(b'mail', [
                 b'%s@%s' % (username, settings.AD_DOMAIN_NAME)])[0]
