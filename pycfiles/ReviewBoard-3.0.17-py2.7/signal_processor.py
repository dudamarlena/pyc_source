...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/search/signal_processor.py
...
class SignalProcessor(BaseSignalProcessor):
    ...
    save_signals = [
     (
      ReviewRequest, review_request_published, b'review_request'),
     (
      User, post_save, b'instance'),
     (
      Profile, post_save, b'instance')]
    delete_signals = [
     (
    ...
    def setup(self):
        ...
        if not self.is_setup:
            for cls, signal, instance_kwarg in self.save_signals:
                handler = partial(self.check_handle_save, instance_kwarg=instance_kwarg)
    ...
    def _handle_group_m2m_changed(self, instance, action, pk_set, reverse, **kwargs):
        ...
        """
        backend = search_backend_registry.current_backend
        if not (backend and search_backend_registry.on_the_fly_indexing_enabled):
            return
        if not hasattr(self._pending_user_changes, b'data'):
        ...
        if action in ('post_add', 'post_remove'):
            ...
            for user in users:
                self.handle_save(instance=user, instance_kwarg=b'instance', sender=User)

        elif action == b'pre_clear':
            if not reverse:
                self._pending_user_changes.data[instance.pk] = list(instance.users.values_list(b'pk', flat=True))
        elif action == b'post_clear':
            if reverse:
                self.handle_save(instance=instance, instance_kwarg=b'instance', sender=User)
            else:
