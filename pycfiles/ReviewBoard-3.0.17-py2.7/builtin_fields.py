...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/builtin_fields.py
...
from reviewboard.reviews.fields import BaseCommaEditableField, BaseEditableField, BaseReviewRequestField, BaseReviewRequestFieldSet, BaseTextAreaField
...
class BuiltinFieldMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self.review_request_details, self.field_id) and isinstance(self.review_request_details, ReviewRequestDraft):
            self.review_request_details = self.review_request_details.get_review_request()
...
class ReviewRequestPageDataMixin(object):
    ...
    def __init__(self, review_request_details, data=None, *args, **kwargs):
        ...
        """
        super(ReviewRequestPageDataMixin, self).__init__(review_request_details, *args, **kwargs)
        self.data = data
...
class BaseCaptionsField(ReviewRequestPageDataMixin, BaseReviewRequestField):
    ...
    def render_change_entry_html(self, info):
        ...
        s = [
         b'<table class="caption-changed">']
        for id_str, caption in six.iteritems(info):
            obj = obj_map[int(id_str)]
            s.append(format_html(b'<tr> <th><a href="{url}">{filename}</a>:</th> <td>{caption}</td></tr>', url=obj.get_absolute_url(), filename=obj.filename, caption=mark_safe(render_item(caption))))
    ...
    def serialize_change_entry(self, changedesc):
        ...
        return [ {b'old': data[six.text_type(obj.pk)][b'old'][0], b'new': data[six.text_type(obj.pk)][b'new'][0], self.caption_object_field: obj} for obj in self.model.objects.filter(pk__in=six.iterkeys(data))
               ]
...
class BaseModelListEditableField(BaseCommaEditableField):
    ...
    def record_change_entry(self, changedesc, old_value, new_value):
        ...
        Args:
            ...
            new_value (object):
                The new value of the field.
        """
        changedesc.record_field_change(self.field_id, old_value, new_value, self.model_name_attr)
    ...
    def render_change_entry_item_html(self, info, item):
        ...
        label, url, pk = item
        if url:
            return b'<a href="%s">%s</a>' % (escape(url), escape(label))
        else:
            return escape(label)
...
class SummaryField(BuiltinFieldMixin, BaseEditableField):
    ...
    is_required = True
    tag_name = b'h1'
    js_view_class = b'RB.ReviewRequestFields.SummaryFieldView'


class DescriptionField(BuiltinTextAreaFieldMixin, BaseTextAreaField):
    """The Description field on a review request."""
    field_id = b'description'
    label = _(b'Description')
    is_required = True
    js_view_class = b'RB.ReviewRequestFields.DescriptionFieldView'
...
class TestingDoneField(BuiltinTextAreaFieldMixin, BaseTextAreaField):
    ...
    field_id = b'testing_done'
    label = _(b'Testing Done')
    js_view_class = b'RB.ReviewRequestFields.TestingDoneFieldView'
...
class OwnerField(BuiltinFieldMixin, BaseEditableField):
    ...
    field_id = b'submitter'
    label = _(b'Owner')
    model = User
    model_name_attr = b'username'
    is_required = True
    js_view_class = b'RB.ReviewRequestFields.OwnerFieldView'
    ...
    def render_value(self, user):
        ...
        Returns:
            ...
            The rendered value.
        """
        return format_html(b'<a class="user" href="{0}">{1}</a>', local_site_reverse(b'user', local_site=self.review_request_details.local_site, args=[
         user]), user.get_profile().get_display_name(self.request.user))
...
class BranchField(BuiltinFieldMixin, BaseEditableField):
    ...
    field_id = b'branch'
    label = _(b'Branch')
    js_view_class = b'RB.ReviewRequestFields.BranchFieldView'


class BugsField(BuiltinFieldMixin, BaseCommaEditableField):
    """The Bugs field on a review request."""
    field_id = b'bugs_closed'
    label = _(b'Bugs')
    one_line_per_change_entry = False
    js_view_class = b'RB.ReviewRequestFields.BugsFieldView'
    ...
    def save_value(self, value):
        ...
        Args:
            value (object):
                The new value for the field.
        """
        setattr(self.review_request_details, self.field_id, (b', ').join(value))
    ...
    def render_item(self, bug_id):
        ...
        if bug_url:
            return format_html(b'<a class="bug" href="{url}">{id}</a>', url=bug_url, id=bug_id)
        else:
    ...
    def _get_bug_url(self, bug_id):
        ...
        try:
            if repository and repository.bug_tracker and b'%s' in repository.bug_tracker:
                bug_url = local_site_reverse(b'bug_url', local_site_name=local_site_name, args=[
                 review_request.display_id, bug_id])
...
class DependsOnField(BuiltinFieldMixin, BaseModelListEditableField):
    ...
    label = _(b'Depends On')
    model = ReviewRequest
    model_name_attr = b'summary'
    js_view_class = b'RB.ReviewRequestFields.DependsOnFieldView'
    ...
    def render_change_entry_item_html(self, info, item):
        ...
        rendered_item = format_html(b'<a href="{url}">{id} - {summary}</a>', url=item.get_absolute_url(), id=item.pk, summary=item.summary)
        if item.status in (ReviewRequest.SUBMITTED,
         ReviewRequest.DISCARDED):
            return b'<s>%s</s>' % rendered_item
        else:
    ...
    def render_item(self, item):
        ...
        Returns:
            ...
            The rendered item.
        """
        rendered_item = format_html(b'<a href="{url}" title="{summary}"   class="review-request-link">{id}</a>', url=item.get_absolute_url(), summary=item.summary, id=item.display_id)
        if item.status in (ReviewRequest.SUBMITTED,
         ReviewRequest.DISCARDED):
            return b'<s>%s</s>' % rendered_item
        else:
...
class BlocksField(BuiltinFieldMixin, BaseReviewRequestField):
    ...
    def render_value(self, blocks):
        ...
        Returns:
            ...
            The rendered value.
        """
        return format_html_join(b', ', b'<a href="{0}" class="review-request-link">{1}</a>', [ (item.get_absolute_url(), item.display_id) for item in blocks
                                                                                             ])
...
class ChangeField(BuiltinFieldMixin, BaseReviewRequestField):
    ...
    def render_value(self, changenum):
        ...
        if is_pending:
            return escape(_(b'%s (pending)') % changenum)
        else:
            return changenum
...
class CommitField(BuiltinFieldMixin, BaseReviewRequestField):
    ...
    def should_render(self):
        """Whether the field should be rendered."""
        return bool(self.value) and not self.review_request_details.get_review_request().changenum
    ...
    def render_value(self, commit_id):
        ...
        """
        if len(commit_id) == 40:
            abbrev_commit_id = commit_id[:7] + b'...'
            return b'<span title="%s">%s</span>' % (escape(commit_id),
             escape(abbrev_commit_id))
        else:
            return escape(commit_id)


class DiffField(ReviewRequestPageDataMixin, BuiltinFieldMixin, BaseReviewRequestField):
    ...
    def render_change_entry_html(self, info):
        ...
        Returns:
            ...
            The HTML representation of the change entry.
        """
        added_diff_info = info[b'added'][0]
        review_request = self.review_request_details.get_review_request()
        try:
        ...
        raw_insert_count = counts.get(b'raw_insert_count', 0)
        raw_delete_count = counts.get(b'raw_delete_count', 0)
        line_counts = []
        if raw_insert_count > 0:
            line_counts.append(b'<span class="insert-count">+%d</span>' % raw_insert_count)
        if raw_delete_count > 0:
            line_counts.append(b'<span class="delete-count">-%d</span>' % raw_delete_count)
        s.append(format_html(b'<p class="diff-changes"> <a href="{url}">{label}</a> <span class="line-counts">({line_counts})</span></p>', url=diff_url, label=_(b'Revision %s') % diff_revision, count=_(b'%d files') % diffset.file_count, line_counts=mark_safe((b' ').join(line_counts))))
        if past_revision > 0:
            interdiff_url = local_site_reverse(b'view-interdiff', local_site=review_request.local_site, args=[
             review_request.display_id,
             past_revision,
             diff_revision])
            s.append(format_html(b'<p><a href="{url}">{text}</a>', url=interdiff_url, text=_(b'Show changes')))
        if diffset.file_count > 0:
            s += [
             b'<div class="diff-index">',
             b' <table>']
            files = get_sorted_filediffs(enumerate(diffset.files.all()), key=lambda i: i[1])
            for i, filediff in files[:self.MAX_FILES_PREVIEW]:
                counts = filediff.get_line_counts()
                data_attrs = [ b'data-%s="%s"' % (attr.replace(b'_', b'-'), counts[attr]) for attr in ('insert_count',
                                                                                                       'delete_count',
                                                                                                       'replace_count',
                                                                                                       'total_line_count') if counts.get(attr) is not None
                             ]
                s.append(format_html(b'<tr {data_attrs}> <td class="diff-file-icon"></td> <td class="diff-file-info">  <a href="{url}">{filename}</a> </td></tr>', data_attrs=mark_safe((b' ').join(data_attrs)), url=diff_url + b'#%d' % i, filename=filediff.source_file))

            num_remaining = diffset.file_count - self.MAX_FILES_PREVIEW
            if num_remaining > 0:
                s.append(format_html(b'<tr> <td></td> <td class="diff-file-info">{text}</td></tr>', text=_(b'%s more') % num_remaining))
            s += [
             b' </table>',
             b'</div>']
        return (b'').join(s)
    ...
    def load_value(self, review_request_details):
        ...
        Returns:
            ...
            The loaded value.
        """
        return getattr(review_request_details, b'diffset', None)
    ...
    def record_change_entry(self, changedesc, unused, diffset):
        ...
        """
        review_request = self.review_request_details.get_review_request()
        url = local_site_reverse(b'view-diff-revision', local_site=review_request.local_site, args=[
         review_request.display_id, diffset.revision])
        changedesc.fields_changed[b'diff'] = {b'added': [
                    (
                     _(b'Diff r%s') % diffset.revision,
                     url,
                     diffset.pk)]}
    ...
    def serialize_change_entry(self, changedesc):
        ...
        Returns:
            ...
            An appropriate serialization for the field.
        """
        diffset_id = changedesc.fields_changed[b'diff'][b'added'][0][2]
        return {b'added': DiffSet.objects.get(pk=diffset_id)}
...
class FileAttachmentCaptionsField(BaseCaptionsField):
    ...
    ChangeDescription.
    """
    field_id = b'file_captions'
    label = _(b'File Captions')
    obj_map_attr = b'file_attachments_by_id'
    model = FileAttachment
    caption_object_field = b'file_attachment'


class FileAttachmentsField(ReviewRequestPageDataMixin, BuiltinFieldMixin, BaseCommaEditableField):
    ...
    label = _(b'Files')
    model = FileAttachment
    thumbnail_template = b'reviews/changedesc_file_attachment.html'
    ...
    def get_change_entry_sections_html(self, info):
        ...
        """
        sections = []
        if b'removed' in info:
            sections.append({b'title': _(b'Removed Files'), 
               b'rendered_html': mark_safe(self.render_change_entry_html(info[b'removed']))})
        if b'added' in info:
            sections.append({b'title': _(b'Added Files'), 
               b'rendered_html': mark_safe(self.render_change_entry_html(info[b'added']))})
        return sections
    ...
    def render_change_entry_html(self, values):
        ...
        for caption, filename, pk in values:
            ...
            items.append(template.render(Context({b'file': attachment, 
...
class ScreenshotCaptionsField(BaseCaptionsField):
    ...
    ChangeDescription.
    """
    field_id = b'screenshot_captions'
    label = _(b'Screenshot Captions')
    obj_map_attr = b'screenshots_by_id'
    model = Screenshot
    caption_object_field = b'screenshot'
...
class ScreenshotsField(BaseCommaEditableField):
    ...
    ChangeDescription.
    """
    field_id = b'screenshots'
    label = _(b'Screenshots')
    model = Screenshot


class TargetGroupsField(BuiltinFieldMixin, BaseModelListEditableField):
    """The Target Groups field on a review request."""
    field_id = b'target_groups'
    label = _(b'Groups')
    model = Group
    model_name_attr = b'name'
    js_view_class = b'RB.ReviewRequestFields.TargetGroupsFieldView'
    ...
    def render_item(self, group):
        ...
        Returns:
            ...
            The rendered item.
        """
        return b'<a href="%s">%s</a>' % (escape(group.get_absolute_url()),
         escape(group.name))


class TargetPeopleField(BuiltinFieldMixin, BaseModelListEditableField):
    """The Target People field on a review request."""
    field_id = b'target_people'
    label = _(b'People')
    model = User
    model_name_attr = b'username'
    js_view_class = b'RB.ReviewRequestFields.TargetPeopleFieldView'
    ...
    def render_item(self, user):
        ...
        if not user.is_active:
            extra_classes.append(b'inactive')
        return format_html(b'<a href="{0}" class="{1}">{2}</a>', local_site_reverse(b'user', local_site=self.review_request_details.local_site, args=[
         user]), (b' ').join(extra_classes), user.username)
...
class ChangeEntryOnlyFieldSet(BaseReviewRequestFieldSet):
    fieldset_id = b'_change_entries_only'
    field_classes = [
     DiffField,
