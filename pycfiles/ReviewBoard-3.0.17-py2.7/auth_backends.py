...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/auth_backends.py
# Compiled at: 2020-02-11 04:03:57
from __future__ import unicode_literals
from django.contrib.auth.models import User
from djblets.webapi.auth import WebAPIBasicAuthBackend as DjbletsWebAPIBasicAuthBackend
    ...
    * limited to the local site being requested.
    ...
    def verify_request(self, request, token, user):
        ...
        * If the Application is associated with a Local Site that site must be
        ...
        return application.enabled and application.local_site == request.local_site and (not application.local_site or application.local_site.is_accessible_by(user))
...
class WebAPIBasicAuthBackend(DjbletsWebAPIBasicAuthBackend):
    ...
    def get_credentials(self, request):
        ...
        """
        credentials = super(WebAPIBasicAuthBackend, self).get_credentials(request)
        if credentials and b'username' in credentials:
            user_exists = User.objects.filter(username=credentials[b'username']).exists()
            if not user_exists and b'@' in credentials[b'username']:
                users = User.objects.filter(email=credentials[b'username']).values_list(b'username', flat=True)
                if users:
                    credentials[b'username'] = users[0]
        return credentials