...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_file_diff_comment.py
...
class ResourceListTests(ReviewRequestChildListMixin, BaseWebAPITestCase):
    ...
    sample_api_url = b'review-requests/<id>/diffs/<revision>/files/<id>/diff-comments/'
    ...
    def setup_http_not_allowed_list_test(self, user):
        review_request = self.create_review_request(create_repository=True, submitter=user, publish=True)
        diffset = self.create_diffset(review_request)
        filediff = self.create_filediff(diffset)
    ...
    def compare_item(self, item_rsp, filediff):
        ...
        self.assertEqual(item_rsp[b'text'], filediff.text)
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
        ...
        if populate_items:
            ...
            items = []
        return (
         get_filediff_comment_list_url(filediff, local_site_name),
         filediff_comment_list_mimetype,
         items)
    ...
    def test_get_as_anonymous(self):
        ...
        """
        diff_comment_text = b'Sample comment.'
        review_request = self.create_review_request(create_repository=True, publish=True)
        diffset = self.create_diffset(review_request)
        filediff = self.create_filediff(diffset)
        review = self.create_review(review_request, publish=True)
        comment = self.create_diff_comment(review, filediff, text=diff_comment_text)
        self.client.logout()
        rsp = self.api_get(get_filediff_comment_list_url(filediff), expected_mimetype=filediff_comment_list_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'diff_comments']), 1)
        self.assertEqual(rsp[b'diff_comments'][0][b'text'], comment.text)
    ...
    def test_get_with_line(self):
        ...
        rsp = self.api_get(get_filediff_comment_list_url(filediff), {b'line': diff_comment_line}, expected_mimetype=filediff_comment_list_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'diff_comments']), 1)
        self.assertEqual(rsp[b'diff_comments'][0][b'text'], diff_comment_text)
        self.assertEqual(rsp[b'diff_comments'][0][b'first_line'], diff_comment_line)
