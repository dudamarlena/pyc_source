...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/notifications/email/hooks.py
...
from reviewboard.reviews.signals import review_request_published, review_published, reply_published, review_request_closed
_hooks = defaultdict(set)
_hook_compat = defaultdict(list)
_hook_compat.update({review_published: [
                    b'to_owner_only']})
...
def register_email_hook(signal, handler):
    ...
    assert signal in (review_request_published, review_request_closed,
     review_published, reply_published), b'Invalid signal %r' % signal
...
def filter_email_recipients_from_hooks(to_field, cc_field, signal, **kwargs):
    ...
    if signal in _hooks:
        for hook in _hooks[signal]:
            to_field = _call_hook_compat(hook, hook.get_to_field, optional_args=optional_args, value=to_field, **kwargs)
            cc_field = _call_hook_compat(hook, hook.get_cc_field, optional_args=optional_args, value=cc_field, **kwargs)
...
def _call_hook_compat(hook, method, optional_args, value, **kwargs):
    ...
    if removed:
        warnings.warn(b'%s.%s should accept **kwargs. Some arguments may not be passed to the hook.' % (
         hook.__name__, method.__name__), RemovedInReviewBoard40Warning)
