...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/base_watched_object.py
...
class BaseWatchedObjectResource(WebAPIResource):
    ...
    uri_object_key = b'watched_obj_id'
    profile_field = None
    star_function = None
    ...
    def get_queryset(self, request, username, local_site_name=None, *args, **kwargs):
        try:
            ...
            q = self.watched_resource.get_queryset(request, local_site_name=local_site_name, *args, **kwargs)
    ...
    def get(self, request, watched_obj_id, *args, **kwargs):
        ...
        return HttpResponseRedirect(self.watched_resource.get_href(obj, request, *args, **kwargs))

    @webapi_check_login_required
    @webapi_response_errors(DOES_NOT_EXIST)
    def get_list(self, request, *args, **kwargs):
        try:
            objects = [ self.serialize_object(obj) for obj in self.get_queryset(request, is_list=True, *args, **kwargs)
                      ]
            return (
             200,
    ...
    @webapi_request_fields(required={b'object_id': {b'type': six.text_type, 
                      b'description': b'The ID of the object to watch.'}})
