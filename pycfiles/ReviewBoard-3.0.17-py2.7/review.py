...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/review.py
...
class ReviewResource(BaseReviewResource):
    ...
    def get_base_reply_to_field(self, *args, **kwargs):
        return {b'base_reply_to__isnull': True}
    ...
    def update_review(self, request, review, ship_it=None, *args, **kwargs):
        ...
        if review.public:
            if ship_it is False:
                ...
                if not review.ship_it:
                    return (INVALID_FORM_DATA,
                     {b'fields': {b'ship_it': b'This review is not marked Ship It!'}})
                try:
                    review.revoke_ship_it(request.user)
                except RevokeShipItError as e:
                    return REVOKE_SHIP_IT_ERROR.with_message(six.text_type(e))
            ...
            if ship_it is True:
                return (INVALID_FORM_DATA,
                 {b'fields': {b'ship_it': b'Published reviews cannot be updated with ship_it=true'}})
        return super(ReviewResource, self).update_review(request, review, ship_it=ship_it, *args, **kwargs)
