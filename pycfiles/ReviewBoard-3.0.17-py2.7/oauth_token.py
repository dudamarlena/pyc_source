...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/oauth_token.py
...
class OAuthTokenResource(WebAPIResource):
    ...
    model = AccessToken
    name = b'oauth_token'
    verbose_name = _(b'OAuth2 Tokens')
    uri_object_key = b'oauth_token_id'
    item_result_key = b'oauth_token'
    required_features = [oauth2_service_feature]
    allowed_methods = ('GET', 'PUT', 'DELETE')
    api_token_access_allowed = False
    oauth2_token_access_allowed = False
    added_in = b'3.0'
    fields = {b'application': {b'type': six.text_type, 
                        b'description': b'The name of the application this token is for.'}, 
       b'expires': {b'type': six.text_type, 
                    b'description': b'When this token is set to expire.'}, 
       b'scope': {b'type': [
                          six.text_type], 
                  b'description': b'The scopes this token has access to.'}, 
       b'token': {b'type': six.text_type, 
                  b'description': b'The access token.'}}
    ...
    def get_queryset(self, request, *args, **kwargs):
        ...
        return AccessToken.objects.filter(q).select_related(b'application')
    ...
    def has_access_permissions(self, request, obj, *args, **kwargs):
        ...
        """
        return request.user.is_authenticated() and (obj.user_id == request.user.pk or request.user.is_superuser)
    ...
    @webapi_request_fields(optional={b'add_scopes': {b'type': six.text_type, 
                       b'description': b'A comma-separated list of scopes to add.'}, 
       b'remove_scopes': {b'type': six.text_type, 
                          b'description': b'A comma-separated list of scopes to remove.'}, 
       b'scopes': {b'type': six.text_type, 
                   b'description': b'A comma-separated list of scopes to override the current set with.\n\nThis field cannot be provided if either add_scopes or remove_scopes is provided.'}})
    def update(self, request, local_site=None, add_scopes=None, remove_scopes=None, scopes=None, *args, **kwargs):
        ...
        if not self.has_modify_permissions(request, access_token, *args, **kwargs):
            return self.get_no_access_error(request)
        else:
            if (add_scopes is not None or remove_scopes is not None) and scopes is not None:
                return (INVALID_FORM_DATA,
                 {b'fields': {b'scopes': [
                                          b'This field cannot be provided if either add_scopes or remove_scopes is provided.']}})
            field_errors = {}
            valid_scopes = get_scope_dictionary()
            if scopes is not None:
                scopes = self._validate_scopes(valid_scopes, scopes, b'scopes', field_errors)
            else:
                if add_scopes is not None or remove_scopes is not None:
                    add_scopes = self._validate_scopes(valid_scopes, add_scopes, b'add_scopes', field_errors)
                    remove_scopes = self._validate_scopes(valid_scopes, remove_scopes, b'remove_scopes', field_errors)
                if field_errors:
                    return (INVALID_FORM_DATA,
                     {b'fields': field_errors})
            if scopes is not None:
                access_token.scope = (b' ').join(scopes)
                access_token.save(update_fields=('scope', ))
            elif add_scopes is not None or remove_scopes is not None:
                current_scopes = set(access_token.scope.split(b' '))
                if add_scopes:
                    current_scopes.update(add_scopes)
                if remove_scopes:
                    current_scopes.difference_update(remove_scopes)
                access_token.scope = (b' ').join(current_scopes)
                access_token.save(update_fields=('scope', ))
            return (200,
    ...
    def _validate_scopes(self, valid_scopes, scopes, field, field_errors):
        ...
        if scopes is None:
            ...
            scopes = scopes.split(b',')
            invalid_scopes = {scope for scope in scopes if scope not in valid_scopes}
            if invalid_scopes:
                field_errors[field] = [ b'The scope "%s" is invalid.' % scope for scope in invalid_scopes
                                      ]
                return
            return scopes
