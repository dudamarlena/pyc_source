...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/hostingsvcs/service.py
# Compiled at: 2020-02-11 04:03:56
"""The base hosting service class and associated definitions."""
from __future__ import unicode_literals
import base64, hashlib, json, logging, mimetools, re, ssl
...
from django.utils.six.moves.urllib.request import Request as BaseURLRequest, HTTPBasicAuthHandler, urlopen
from django.utils.translation import ugettext_lazy as _
from djblets.registries.errors import ItemLookupError
from djblets.registries.registry import ALREADY_REGISTERED, LOAD_ENTRY_POINT, NOT_REGISTERED
...
class URLRequest(BaseURLRequest):
    ...
    def __init__(self, url, body=b'', headers=None, method=b'GET'):
    ...
    def add_basic_auth(self, username, password):
        ...
        if isinstance(username, six.text_type):
            username = username.encode(b'utf-8')
        if isinstance(password, six.text_type):
            password = password.encode(b'utf-8')
        auth = b'%s:%s' % (username, password)
        self.add_header(HTTPBasicAuthHandler.auth_header, b'Basic %s' % base64.b64encode(auth))
...
class HostingServiceClient(object):
    ...
    def http_delete(self, url, headers=None, *args, **kwargs):
        ...
        """
        return self.http_request(url, headers=headers, method=b'DELETE', **kwargs)
    ...
    def http_get(self, url, headers=None, *args, **kwargs):
        ...
        Raises:
            ...
            urllib2.URLError:
                When there is an error communicating with the URL.
        """
        return self.http_request(url, headers=headers, method=b'GET', **kwargs)

    def http_post(self, url, body=None, fields=None, files=None, content_type=None, headers=None, *args, **kwargs):
        ...
        return self.http_request(url, body=body, headers=headers, method=b'POST', **kwargs)
    ...
    def http_request(self, url, body=None, headers=None, method=b'GET', username=None, password=None):
        ...
        try:
            if self.hosting_service and b'ssl_cert' in self.hosting_service.account.data:
                context = ssl.create_default_context()
                context.load_verify_locations(cadata=self.hosting_service.account.data[b'ssl_cert'])
            ...
            if b'CERTIFICATE_VERIFY_FAILED' not in six.text_type(e) or not hasattr(ssl, b'create_default_context'):
                raise
            parts = urlparse(url)
            port = parts.port or 443
            cert_pem = ssl.get_server_certificate((parts.hostname, port))
            cert_der = ssl.PEM_cert_to_DER_cert(cert_pem)
            cert = x509.load_pem_x509_certificate(cert_pem.encode(b'ascii'), default_backend())
            ...
            raise UnverifiedCertificateError(Certificate(pem_data=cert_pem, valid_from=cert.not_valid_before.isoformat(), valid_until=cert.not_valid_after.isoformat(), issuer=issuer, hostname=subject, fingerprint=hashlib.sha256(cert_der).hexdigest()))
    ...
    def build_form_data(fields, files=None):
        ...
        content_parts = []
        for key, value in sorted(six.iteritems(fields), key=lambda pair: pair[0]):
            if isinstance(key, six.text_type):
                key = key.encode(b'utf-8')
            if isinstance(value, six.text_type):
                value = value.encode(b'utf-8')
            content_parts.append(b'--%(boundary)s\r\nContent-Disposition: form-data; name="%(key)s"\r\n\r\n%(value)s\r\n' % {b'boundary': boundary, 
               b'key': key, 
               b'value': value})

        if files:
            for key, data in sorted(six.iteritems(files), key=lambda pair: pair[0][b'filename']):
                filename = data[b'filename']
                ...
                if isinstance(content, six.text_type):
                    content = content.encode(b'utf-8')
                content_parts.append(b'--%(boundary)s\r\nContent-Disposition: form-data; name="%(key)s"; filename="%(filename)s"\r\n\r\n%(value)s\r\n' % {b'boundary': boundary, 
                   b'key': key, 
                   b'filename': filename, 
                   b'value': content})
...
class HostingService(object):
    ...
    def authorize(self, username, password, hosting_url, credentials, two_factor_auth_code=None, local_site_name=None, *args, **kwargs):
    ...
    def check_repository(self, path, username, password, scmtool_class, local_site_name, *args, **kwargs):
    ...
    def get_remote_repositories(self, owner=None, owner_type=None, filter_type=None, start=None, per_page=None, **kwargs):
    ...
    def normalize_patch(self, repository, patch, filename, revision):
        ...
        """
        return repository.get_scmtool().normalize_patch(patch=patch, filename=filename, revision=revision)

    @classmethod
    def get_repository_fields(cls, username, hosting_url, plan, tool_name, field_vars):
        ...
        if not cls.supports_repositories:
            raise NotImplementedError
        fields = cls.get_field(plan, b'repository_fields')
        new_vars = field_vars.copy()
        new_vars[b'hosting_account_username'] = username
        if cls.self_hosted:
            new_vars[b'hosting_url'] = hosting_url
            new_vars[b'hosting_domain'] = urlparse(hosting_url)[1]
        results = {}
        ...
        for field, value in six.iteritems(fields[tool_name]):
            try:
                ...
                logging.error(b'Failed to generate %s field for hosting service %s using %s and %r: Missing key %s' % (
                 field, six.text_type(cls.name), value,
                 new_vars, e), exc_info=1)
                raise KeyError(_(b'Internal error when generating %(field)s field (Missing key "%(key)s"). Please report this.') % {b'field': field, 
                   b'key': e})
    ...
    def get_bug_tracker_requires_username(cls, plan=None):
        ...
        if not cls.supports_bug_trackers:
            raise NotImplementedError
        return b'%(hosting_account_username)s' in cls.get_field(plan, b'bug_tracker_field', b'')
    ...
    def get_bug_tracker_field(cls, plan, field_vars):
        ...
        if not cls.supports_bug_trackers:
            raise NotImplementedError
        bug_tracker_field = cls.get_field(plan, b'bug_tracker_field')
        if not bug_tracker_field:
            return b''
        try:
            return bug_tracker_field % field_vars
        except KeyError as e:
            logging.error(b'Failed to generate %s field for hosting service %s using %r: Missing key %s' % (
             bug_tracker_field, six.text_type(cls.name),
             field_vars, e), exc_info=1)
            raise KeyError(_(b'Internal error when generating %(field)s field (Missing key "%(key)s"). Please report this.') % {b'field': bug_tracker_field, 
               b'key': e})
...
class HostingServiceRegistry(EntryPointRegistry):
    ...
    entry_point = b'reviewboard.hosting_services'
    lookup_attrs = [b'hosting_service_id']
    errors = {ALREADY_REGISTERED: _(b'"%(item)s" is already a registered hosting service.'), 
       LOAD_ENTRY_POINT: _(b'Unable to load repository hosting service %(entry_point)s: %(error)s.'), 
       NOT_REGISTERED: _(b'"%(attr_value)s" is not a registered hosting service.')}
    ...
    def register(self, service):
        ...
        if service.repository_url_patterns:
            cls_urlpatterns = [
             url(b'^(?P<hosting_service_id>%s)/' % re.escape(service.hosting_service_id), include(service.repository_url_patterns))]
...
def get_hosting_service(name):
    ...
    """
    try:
        return _hosting_service_registry.get(b'hosting_service_id', name)
...
def unregister_hosting_service(name):
    ...
    """
    try:
        _hosting_service_registry.unregister_by_attr(b'hosting_service_id', name)
    except ItemLookupError as e:
        logging.error(b'Failed to unregister unknown hosting service "%s"' % name)
        raise e


@receiver(initializing, dispatch_uid=b'populate_hosting_services')
