...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/testing/testcase.py
...
from djblets.testing.testcases import FixturesCompilerMixin, TestCase as DjbletsTestCase
...
from reviewboard.reviews.models import Comment, FileAttachmentComment, GeneralComment, Group, Review, ReviewRequest, ReviewRequestDraft, Screenshot, ScreenshotComment, StatusUpdate
...
class TestCase(FixturesCompilerMixin, DjbletsTestCase):
    ...
    and useless testing.
    """
    local_site_name = b'local-site-1'
    local_site_id = 1
    ws_re = re.compile(b'\\s+')
    DEFAULT_FILEDIFF_DATA = b'--- README\trevision 123\n+++ README\trevision 123\n@@ -1 +1 @@\n-Hello, world!\n+Hello, everybody!\n'
    DEFAULT_GIT_FILEDIFF_DATA = b'diff --git a/README b/README\nindex 94bdd3e..197009f 100644\n--- README\n+++ README\n@@ -2 +2 @@\n-blah blah\n+blah!\n'
    ...
    def create_user(self, username=b'test-user', password=b'', email=b'test@example.com', perms=None, **kwargs):
        ...
        user.save()
        if perms:
            user.user_permissions.add(*[ Permission.objects.get(codename=perm_name, content_type__app_label=perm_app_label) for perm_app_label, perm_name in perms
                                       ])
        return user

    def create_webapi_token(self, user, note=b'Sample note', policy={b'access': b'rw'}, with_local_site=False, **kwargs):
        """Creates a WebAPIToken for testing."""
        if with_local_site:
            local_site = self.get_local_site(name=self.local_site_name)
        else:
            local_site = None
        return WebAPIToken.objects.generate_token(user=user, note=note, policy=policy, local_site=local_site)
    ...
    def assert_warns(self, cls=DeprecationWarning, message=None):
        ...
        with warnings.catch_warnings(record=True) as (w):
            ...
            yield
            warning_found = any(issubclass(warning.category, cls) and message == six.text_type(warning.message) for warning in w)
            if not warning_found:
                self.fail(b'No warning was found matching type %r and message %r' % (
                 cls, message))

    def create_diff_file_attachment(self, filediff, from_modified=True, review_request=None, orig_filename=b'filename.png', caption=b'My Caption', mimetype=b'image/png', **kwargs):
        ...
        """
        file_attachment = FileAttachment.objects.create_from_filediff(filediff=filediff, from_modified=from_modified, caption=caption, orig_filename=orig_filename, mimetype=mimetype, **kwargs)
        filename = os.path.join(settings.STATIC_ROOT, b'rb', b'images', b'logo.png')
        with open(filename, b'rb') as (f):
            file_attachment.file.save(os.path.basename(filename), File(f), save=True)
        if review_request:
            review_request.file_attachments.add(file_attachment)
        return file_attachment

    def create_diffset(self, review_request=None, revision=1, repository=None, draft=False, name=b'diffset'):
        ...
        diffset = DiffSet.objects.create(name=name, revision=revision, repository=repository, diffcompat=DiffCompatVersion.DEFAULT)
        if review_request:
            if draft:
    ...
    def create_diff_comment(self, review, filediff, interfilediff=None, text=b'My comment', issue_opened=False, issue_status=None, first_line=1, num_lines=5, extra_fields=None, reply_to=None, **kwargs):
        ...
        comment = Comment(filediff=filediff, interfilediff=interfilediff, first_line=first_line, num_lines=num_lines, text=text, issue_opened=issue_opened, issue_status=issue_status, reply_to=reply_to, **kwargs)
        if extra_fields:
    ...
    def create_file_attachment(self, review_request, orig_filename=b'filename.png', caption=b'My Caption', draft=False, active=True, **kwargs):
        ...
        """
        file_attachment = self._create_base_file_attachment(caption=caption, orig_filename=orig_filename, **kwargs)
        if draft:
    ...
    def create_user_file_attachment(self, user, caption=b'My Caption', with_local_site=False, local_site_name=None, local_site=None, has_file=False, orig_filename=b'filename.png', **kwargs):
        ...
        """
        return self._create_base_file_attachment(caption=caption, user=user, has_file=has_file, orig_filename=orig_filename, with_local_site=with_local_site, local_site_name=local_site_name, local_site=local_site, **kwargs)

    def create_file_attachment_comment(self, review, file_attachment, diff_against_file_attachment=None, text=b'My comment', issue_opened=False, issue_status=None, extra_fields=None, reply_to=None, **kwargs):
        ...
        comment = FileAttachmentComment(file_attachment=file_attachment, diff_against_file_attachment=diff_against_file_attachment, text=text, issue_opened=issue_opened, issue_status=issue_status, reply_to=reply_to, **kwargs)
        if extra_fields:
            comment.extra_data = extra_fields
        comment.save()
    ...
    def create_filediff(self, diffset, source_file=b'/test-file', dest_file=b'/test-file', source_revision=b'123', dest_detail=b'124', status=FileDiff.MODIFIED, diff=DEFAULT_FILEDIFF_DATA, save=True):
        ...
        """
        filediff = FileDiff(diffset=diffset, source_file=source_file, dest_file=dest_file, source_revision=source_revision, dest_detail=dest_detail, status=status, diff=diff)
        if save:
            filediff.save()
        return filediff

    def create_repository(self, with_local_site=False, name=b'Test Repo', tool_name=b'Git', path=None, local_site=None, extra_data=None, **kwargs):
        ...
        if not local_site:
            if with_local_site:
                ...
                local_site = None
        testdata_dir = os.path.join(os.path.dirname(scmtools.__file__), b'testdata')
        if not path:
            if tool_name in ('Git', 'Test', 'TestToolSupportsPendingChangeSets'):
                path = os.path.join(testdata_dir, b'git_repo')
            elif tool_name == b'Subversion':
                path = b'file://' + os.path.join(testdata_dir, b'svn_repo')
            elif tool_name == b'Mercurial':
                path = os.path.join(testdata_dir, b'hg_repo.bundle')
            elif tool_name == b'CVS':
                path = os.path.join(testdata_dir, b'cvs_repo')
            elif tool_name == b'Perforce':
                path = b'localhost:1666'
            elif tool_name == b'Bazaar':
                path = b'file://%s' % os.path.join(testdata_dir, b'bzr_repo')
            else:
                raise NotImplementedError
        repository = Repository(name=name, local_site=local_site, tool=Tool.objects.get(name=tool_name), path=path, **kwargs)
        if extra_data is not None:
            repository.extra_data = extra_data
        repository.save()
        return repository

    def create_review_request(self, with_local_site=False, create_repository=False, publish=False, id=None, local_id=1001, local_site=None, repository=None, time_added=None, last_updated=None, status=ReviewRequest.PENDING_REVIEW, submitter=b'doc', summary=b'Test Summary', description=b'Test Description', testing_done=b'Testing', branch=b'my-branch', depends_on=None, target_people=None, target_groups=None, **kwargs):
        ...
        if create_repository:
            if not not repository:
                raise AssertionError
                repository = self.create_repository(with_local_site=with_local_site)
            submitter = isinstance(submitter, User) or User.objects.get(username=submitter)
        review_request = ReviewRequest(summary=summary, description=description, branch=branch, testing_done=testing_done, local_site=local_site, local_id=local_id, submitter=submitter, diffset_history=DiffSetHistory.objects.create(), repository=repository, status=status, **kwargs)
        ...
        if time_added and last_updated:
            ReviewRequest.objects.filter(pk=review_request.pk).update(time_added=time_added, last_updated=last_updated)
            review_request.time_added = time_added
            review_request.last_updated = last_updated
        elif time_added:
            ReviewRequest.objects.filter(pk=review_request.pk).update(time_added=time_added)
            review_request.time_added = time_added
        elif last_updated:
            ReviewRequest.objects.filter(pk=review_request.pk).update(last_updated=last_updated)
            review_request.last_updated = last_updated
        return review_request

    def create_visit(self, review_request, visibility, user=b'doc', username=None, timestamp=None):
        ...
        return ReviewRequestVisit.objects.create(review_request=review_request, visibility=visibility, user=user)

    def create_review(self, review_request, user=b'dopey', body_top=b'Test Body Top', body_bottom=b'Test Body Bottom', ship_it=False, publish=False, timestamp=None, **kwargs):
        ...
        if not isinstance(user, User):
            user = User.objects.get(username=user)
        review = Review.objects.create(review_request=review_request, user=user, body_top=body_top, body_bottom=body_bottom, ship_it=ship_it, **kwargs)
        if publish:
            review.publish()
        if timestamp:
    ...
    def create_review_group(self, name=b'test-group', with_local_site=False, local_site=None, visible=True, invite_only=False, is_default_group=False):
        ...
        if not local_site and with_local_site:
            local_site = self.get_local_site(name=self.local_site_name)
        return Group.objects.create(name=name, local_site=local_site, visible=visible, invite_only=invite_only, is_default_group=is_default_group)

    def create_reply(self, review, user=b'grumpy', body_top=b'Test Body Top', timestamp=None, publish=False, **kwargs):
        ...
        if not isinstance(user, User):
            user = User.objects.get(username=user)
        reply = Review.objects.create(review_request=review.review_request, user=user, body_top=body_top, base_reply_to=review, **kwargs)
        if publish:
            reply.publish()
        if timestamp:
    ...
    def create_screenshot(self, review_request, caption=b'My caption', draft=False, active=True, **kwargs):
        ...
        filename = os.path.join(settings.STATIC_ROOT, b'rb', b'images', b'logo.png')
        with open(filename, b'rb') as (f):
            screenshot.image.save(os.path.basename(filename), File(f), save=True)
        if draft:
            if isinstance(draft, ReviewRequestDraft):
                review_request_draft = draft
            else:
    ...
    def create_screenshot_comment(self, review, screenshot, text=b'My comment', x=1, y=1, w=5, h=5, issue_opened=False, issue_status=None, extra_fields=None, reply_to=None, **kwargs):
        ...
        comment = ScreenshotComment(screenshot=screenshot, text=text, x=x, y=y, w=w, h=h, issue_opened=issue_opened, issue_status=issue_status, reply_to=reply_to, **kwargs)
        if extra_fields:
            comment.extra_data = extra_fields
        comment.save()
    ...
    def _create_base_file_attachment(self, caption=b'My Caption', orig_filename=b'filename.png', has_file=True, user=None, with_local_site=False, local_site_name=None, local_site=None, **kwargs):
        ...
        file_attachment = FileAttachment(caption=caption, user=user, uuid=b'test-uuid', local_site=local_site, **kwargs)
        if has_file:
            filename = os.path.join(settings.STATIC_ROOT, b'rb', b'images', b'logo.png')
            file_attachment.orig_filename = orig_filename
            file_attachment.mimetype = b'image/png'
            with open(filename, b'rb') as (f):
                file_attachment.file.save(os.path.basename(filename), File(f), save=True)
        file_attachment.save()
        return file_attachment

    def create_general_comment(self, review, text=b'My comment', issue_opened=False, issue_status=None, extra_fields=None, reply_to=None, **kwargs):
        ...
        comment = GeneralComment(text=text, issue_opened=issue_opened, issue_status=issue_status, reply_to=reply_to, **kwargs)
        if extra_fields:
            comment.extra_data = extra_fields
        comment.save()
    ...
    def create_status_update(self, review_request, user=b'dopey', service_id=b'service', summary=b'Status Update', state=StatusUpdate.PENDING, review=None, change_description=None, timestamp=None):
        ...
        if not isinstance(user, User):
            user = User.objects.get(username=user)
        status_update = StatusUpdate.objects.create(review_request=review_request, change_description=change_description, service_id=service_id, summary=summary, state=state, review=review, user=user)
        if timestamp:
            StatusUpdate.objects.filter(pk=status_update.pk).update(timestamp=timestamp)
            status_update.timestamp = timestamp
        return status_update

    def create_webhook(self, enabled=False, events=WebHookTarget.ALL_EVENTS, url=b'http://example.com', encoding=WebHookTarget.ENCODING_JSON, use_custom_content=False, custom_content=b'', secret=b'', apply_to=WebHookTarget.APPLY_TO_ALL, repositories=None, with_local_site=False, local_site=None, extra_fields=None):
        ...
        if not local_site:
            if with_local_site:
                ...
                local_site = None
        webhook = WebHookTarget.objects.create(enabled=enabled, events=events.split(b','), url=url, encoding=encoding, use_custom_content=use_custom_content, custom_content=custom_content, secret=secret, apply_to=apply_to, local_site=local_site)
        if repositories:
            webhook.repositories = repositories
        if extra_fields:
            webhook.extra_data = extra_fields
            webhook.save(update_fields=[b'extra_data'])
        return webhook

    def create_oauth_application(self, user, local_site=None, with_local_site=False, redirect_uris=b'http://example.com', authorization_grant_type=Application.GRANT_CLIENT_CREDENTIALS, client_type=Application.CLIENT_PUBLIC, **kwargs):
        ...
        if not local_site:
            if with_local_site:
                ...
                local_site = None
        return Application.objects.create(user=user, local_site=local_site, authorization_grant_type=authorization_grant_type, redirect_uris=redirect_uris, client_type=client_type, extra_data=b'{}', **kwargs)

    def create_oauth_token(self, application, user, scope=b'', expires=None, **kwargs):
        ...
        return AccessToken.objects.create(application=application, token=generate_token(), expires=timezone.now() + expires, scope=scope, user=user)
