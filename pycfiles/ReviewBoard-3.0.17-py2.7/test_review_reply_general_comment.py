...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_review_reply_general_comment.py
...
from reviewboard.webapi.tests.mimetypes import review_reply_general_comment_item_mimetype, review_reply_general_comment_list_mimetype
...
from reviewboard.webapi.tests.urls import get_review_reply_general_comment_item_url, get_review_reply_general_comment_list_url
...
class ResourceListTests(CommentReplyListMixin, ReviewRequestChildListMixin, BaseWebAPITestCase):
    ...
    fixtures = [
     b'test_users']
    sample_api_url = b'review-requests/<id>/reviews/<id>/replies/<id>/general-comments/'
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name, populate_items):
        ...
        if populate_items:
            items = [self.create_general_comment(reply, reply_to=comment)]
        else:
            items = []
        return (get_review_reply_general_comment_list_url(reply, local_site_name),
         review_reply_general_comment_list_mimetype,
         items)
    ...
    def check_post_result(self, user, rsp, reply, comment):
        reply_comment = GeneralComment.objects.get(pk=rsp[b'general_comment'][b'id'])
        ...
        self.compare_item(rsp[b'general_comment'], reply_comment)
    ...
    def test_post_with_http_303(self):
        ...
        rsp = self.api_post(get_review_reply_general_comment_list_url(reply), {b'reply_to_id': comment.pk, 
           b'text': comment_text}, expected_status=303, expected_mimetype=review_reply_general_comment_item_mimetype)
...
class ResourceItemTests(CommentReplyItemMixin, ReviewRequestChildItemMixin, BaseWebAPITestCase):
    ...
    fixtures = [
     b'test_users']
    sample_api_url = b'review-requests/<id>/reviews/<id>/replies/<id>/general-comments/<id>/'
    resource = resources.review_reply_general_comment
    ...
    def setup_review_request_child_test(self, review_request):
        ...
        reply_comment = self.create_general_comment(reply, reply_to=comment)
        return (
         get_review_reply_general_comment_item_url(reply, reply_comment.pk),
         review_reply_general_comment_item_mimetype)
    ...
    def setup_basic_delete_test(self, user, with_local_site, local_site_name):
        ...
        reply_comment = self.create_general_comment(reply, reply_to=comment)
        return (
         get_review_reply_general_comment_item_url(reply, reply_comment.pk, local_site_name),
         [
          reply_comment, reply])
