...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_search.py
...
class ResourceTests(SpyAgency, BaseWebAPITestCase):
    ...
    def compare_item(self, item_rsp, local_site_name):
        if local_site_name:
            ...
            self.assertEqual(len(item_rsp[b'users']), local_site.users.count())
        else:
            self.assertEqual(len(item_rsp[b'users']), User.objects.count())

    def setup_basic_get_test(self, user, with_local_site, local_site_name):
        return (
         get_search_url(local_site_name),
         search_mimetype,
         local_site_name)
    ...
    def test_get_with_max_results(self):
        ...
        max_results = 10
        rsp = self.api_get(get_search_url(), query={b'max_results': max_results}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'review_requests']), max_results)
    ...
    def test_get_private_review_group(self):
        ...
        rsp = self.api_get(get_search_url(), query={b'q': group.name}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'groups'], [])
    ...
    def test_get_private_review_group_superuser(self):
        ...
        rsp = self.api_get(get_search_url(), query={b'q': group.name}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'groups']), 1)
    ...
    def test_get_private_review_group_superuser_local_site(self):
        ...
        """
        self.user = self._login_user(admin=True)
        group = self.create_review_group(invite_only=True, with_local_site=True)
        self.assertTrue(group.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': group.name}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'groups']), 1)
    ...
    def test_get_review_request_draft_only(self):
        ...
        rsp = self.api_get(get_search_url(), query={b'q': review_request.pk}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'groups'], [])
    ...
    def test_get_review_request_draft_local_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        review_request = self.create_review_request(with_local_site=True, submitter=b'admin')
        self.assertFalse(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'review_requests'], [])
    ...
    def test_get_review_request_different_local_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        review_request = self.create_review_request(with_local_site=True, submitter=b'admin', public=True)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'review_requests'], [])
    ...
    def test_get_review_request_local_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        review_request = self.create_review_request(submitter=b'admin', with_local_site=True, public=True)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'review_requests']), 1)
    ...
    def test_get_review_request_local_site_no_pk_query_q(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        review_request = self.create_review_request(submitter=b'admin', with_local_site=True, public=True, local_id=9999)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.pk}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'review_requests'], [])
    ...
    def test_get_review_request_local_site_no_pk_query_id(self):
        ...
        review_request = self.create_review_request(submitter=b'admin', with_local_site=True, public=True, local_id=9999)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'id': review_request.pk}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'review_requests'], [])
    ...
    def test_get_review_request_local_site_local_id_query_id(self):
        ...
        review_request = self.create_review_request(submitter=b'admin', with_local_site=True, public=True, local_id=9999)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'id': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'review_requests']), 1)
    ...
    def test_get_review_request_submitted(self):
        ...
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'review_requests']), 1)
        review_request_rsp = rsp[b'search'][b'review_requests'][0]
        self.assertEqual(review_request_rsp[b'id'], review_request.pk)
        self.assertEqual(review_request_rsp[b'status'], b'submitted')
    ...
    def test_get_review_request_discarded(self):
        ...
        review_request_rsp = rsp[b'search'][b'review_requests'][0]
        self.assertEqual(review_request_rsp[b'id'], review_request.pk)
        self.assertEqual(review_request_rsp[b'status'], b'discarded')
    ...
    def test_get_review_request_invite_only_group_local_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        review_group = self.create_review_group(invite_only=True, with_local_site=True)
        review_request = self.create_review_request(public=True, submitter=b'admin', with_local_site=True)
        review_request.target_groups.add(review_group)
        self.assertFalse(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
    ...
    def test_get_review_request_invite_only_group_local_site_superuser(self):
        ...
        self.user = self._login_user(admin=True)
        review_group = self.create_review_group(invite_only=True, with_local_site=True)
        review_request = self.create_review_request(public=True, submitter=b'doc', with_local_site=True)
        review_request.target_groups.add(review_group)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
    ...
    @add_fixtures([b'test_scmtools', b'test_site'])
    def test_get_review_request_private_repository_local_site(self):
        ...
        review_request = self.create_review_request(repository=repository, with_local_site=True, submitter=b'admin', public=True)
        self.assertFalse(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.local_id}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(rsp[b'search'][b'review_requests'], [])
    ...
    def test_get_review_request_private_repository_local_site_superuser(self):
        ...
        self.user = self._login_user(admin=True)
        repository = self.create_repository(public=False, with_local_site=True)
        review_request = self.create_review_request(repository=repository, with_local_site=True, submitter=b'doc', public=True)
        self.assertTrue(review_request.is_accessible_by(self.user))
        rsp = self.api_get(get_search_url(self.local_site_name), query={b'q': review_request.pk}, expected_mimetype=search_mimetype)
        self.assertEqual(rsp[b'stat'], b'ok')
        self.assertEqual(len(rsp[b'search'][b'review_requests']), 1)
    ...
    def test_get_review_request_private_multi_groups(self):
        ...
        """
        non_member_group = self.create_review_group(invite_only=True, name=b'non-member')
        member_group = self.create_review_group(invite_only=True, name=b'member')
    ...
    def test_get_review_request_private_multi_groups_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        non_member_group = self.create_review_group(invite_only=True, name=b'non-member', with_local_site=True)
        member_group = self.create_review_group(invite_only=True, name=b'member', with_local_site=True)
        member_group.users.add(self.user)
        review_request = self.create_review_request(public=True, with_local_site=True, submitter=b'admin')
        review_request.target_groups = [
         member_group,
         non_member_group]
        self.assertTrue(review_request.is_accessible_by(self.user))
    ...
    def test_get_review_request_private_public_groups(self):
        ...
        """
        private_group = self.create_review_group(invite_only=True, name=b'private')
        public_group = self.create_review_group(name=b'public')
        review_request = self.create_review_request(public=True)
        review_request.target_groups = [
         public_group,
         private_group]
    ...
    def test_get_review_request_private_public_groups_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        private_group = self.create_review_group(invite_only=True, name=b'private', with_local_site=True)
        public_group = self.create_review_group(name=b'public', with_local_site=True)
        review_request = self.create_review_request(public=True, with_local_site=True, submitter=b'admin')
        review_request.target_groups = [
         public_group,
         private_group]
    ...
    def test_get_review_request_private_public_groups_targeted_site(self):
        ...
        """
        self.user = self._login_user(local_site=True)
        private_group = self.create_review_group(invite_only=True, with_local_site=True)
        review_request = self.create_review_request(public=True, with_local_site=True, submitter=b'admin')
        review_request.target_groups.add(private_group)
        review_request.target_people.add(self.user)
        self.assertTrue(review_request.is_accessible_by(self.user))
    ...
    def test_get_serializing_unindexed(self):
        ...
        self.create_review_request(submitter=self.user, summary=b'Review request by %s' % self.user.username, public=True)
        self.spy_on(resources.review_request.serialize_object)
        self.spy_on(resources.user.serialize_object)
        rsp = self.api_get(get_search_url(), query={b'q': self.user.username}, expected_mimetype=search_mimetype)
        self.assertEqual(len(rsp[b'search'][b'groups']), 0)
        self.assertEqual(len(rsp[b'search'][b'review_requests']), 0)
        self.assertEqual(len(rsp[b'search'][b'users']), 1)
    ...
    def test_get_user_public_profile_unindexed(self):
        ...
        """
        rsp = self.api_get(get_search_url(), query={b'q': b'doc'}, expected_mimetype=search_mimetype)
        search_results = rsp[b'search']
        self.assertEqual(len(search_results[b'groups']), 0)
        self.assertEqual(len(search_results[b'review_requests']), 0)
        self.assertEqual(len(search_results[b'users']), 1)
        item_rsp = search_results[b'users'][0]
        self.assertIn(b'first_name', item_rsp)
        self.assertIn(b'last_name', item_rsp)
        self.assertIn(b'fullname', item_rsp)
    ...
    def test_get_private_user_unindexed(self):
        ...
        """
        profile = Profile.objects.get(user__username=b'doc')
        ...
        self.assertEqual(len(search_results[b'users']), 1)
        item_rsp = search_results[b'users'][0]
        self.assertNotIn(b'first_name', item_rsp)
        self.assertNotIn(b'last_name', item_rsp)
        self.assertNotIn(b'fullname', item_rsp)
