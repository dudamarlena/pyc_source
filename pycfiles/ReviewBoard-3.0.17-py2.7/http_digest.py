...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/backends/http_digest.py
...
class HTTPDigestBackend(BaseAuthBackend):
    ...
    """
    backend_id = b'digest'
    name = _(b'HTTP Digest Authentication')
    settings_form = HTTPBasicSettingsForm
    login_instructions = _(b'Use your standard username and password.')
    ...
    def authenticate(self, username, password, **kwargs):
        ...
        digest_text = b'%s:%s:%s' % (username, settings.DIGEST_REALM, password)
        digest_password = hashlib.md5(digest_text).hexdigest()
        try:
            with open(filename, b'r') as (fp):
                for line_no, line in enumerate(fp):
                    try:
                        user, realm, passwd = line.strip().split(b':')
                        if user == username and passwd == digest_password:
                            return self.get_or_create_user(username=username)
                    except ValueError as e:
                        logger.exception(b'Error parsing HTTP Digest password file "%s" at line %d: %s', filename, line_no, e)
                        break

        except IOError as e:
            logger.exception(b'Could not open the HTTP Digest password file "%s": %s', filename, e)
