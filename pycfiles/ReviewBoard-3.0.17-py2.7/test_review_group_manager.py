...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_review_group_manager.py
...
class ReviewGroupManagerTests(TestCase):
    ...
    def test_accessible_with_public(self):
        ...
        self.assertIn(group, Group.objects.accessible(user, visible_only=True))
        self.assertIn(group, Group.objects.accessible(user, visible_only=False))
        self.assertIn(group, Group.objects.accessible(anonymous, visible_only=True))
        self.assertIn(group, Group.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_public_and_hidden(self):
        ...
        self.assertNotIn(group, Group.objects.accessible(user, visible_only=True))
        self.assertIn(group, Group.objects.accessible(user, visible_only=False))
        self.assertNotIn(group, Group.objects.accessible(anonymous, visible_only=True))
        self.assertIn(group, Group.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_invite_only_and_not_member(self):
        ...
        self.assertNotIn(group, Group.objects.accessible(user, visible_only=True))
        self.assertNotIn(group, Group.objects.accessible(user, visible_only=False))
        self.assertNotIn(group, Group.objects.accessible(anonymous, visible_only=True))
        self.assertNotIn(group, Group.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_invite_only_hidden_not_member(self):
        ...
        anonymous = AnonymousUser()
        user = self.create_user()
        group = self.create_review_group(invite_only=True, visible=False)
        self.assertNotIn(group, Group.objects.accessible(user, visible_only=True))
        self.assertNotIn(group, Group.objects.accessible(user, visible_only=False))
        self.assertNotIn(group, Group.objects.accessible(anonymous, visible_only=True))
        self.assertNotIn(group, Group.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_local_site_accessible(self):
        ...
        self.assertIn(group, Group.objects.accessible(user, local_site=group.local_site))
        self.assertIn(group, Group.objects.accessible(user, show_all_local_sites=True))