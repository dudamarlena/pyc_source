...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/actions.py
...
class BaseReviewRequestAction(object):
    ...
    def copy_to_dict(self, context):
        ...
        """
        return {b'action_id': self.action_id, 
           b'label': self.get_label(context), 
           b'url': self.get_url(context), 
           b'hidden': self.get_hidden(context)}
    ...
    def render(self, context, action_key=b'action', template_name=b'reviews/action.html'):
    ...
    def register(self, parent=None):
        ...
        if self.action_id in _all_actions:
            raise KeyError(b'%s already corresponds to a registered review request action' % self.action_id)
    ...
    def unregister(self):
        ...
        try:
            ...
            raise KeyError(b'%s does not correspond to a registered review request action' % self.action_id)
...
class BaseReviewRequestMenuAction(BaseReviewRequestAction):
    ...
    def copy_to_dict(self, context):
        ...
        Returns:
            dict: The corresponding dictionary.
        """
        dict_copy = {b'child_actions': self.child_actions}
        dict_copy.update(super(BaseReviewRequestMenuAction, self).copy_to_dict(context))
        return dict_copy
    ...
    def max_depth(self):
        ...
        if self.child_actions and self._max_depth == 0:
            self._max_depth = 1 + max(child_action.max_depth for child_action in self.child_actions)
        return self._max_depth

    def render(self, context, action_key=b'menu_action', template_name=b'reviews/menu_action.html'):
        ...
        Returns:
            unicode: The action rendered in HTML.
        """
        return super(BaseReviewRequestMenuAction, self).render(context, action_key, template_name)
...
def register_actions(actions, parent_id=None):
    ...
    if parent_id is None:
        ...
        try:
            ...
            raise KeyError(b'%s does not correspond to a registered review request action' % parent_id)
...
def unregister_actions(action_ids):
    ...
    for action_id in action_ids:
        try:
            ...
            raise KeyError(b'%s does not correspond to a registered review request action' % action_id)
