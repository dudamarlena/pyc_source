...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/crypto_utils.py
...
def _create_cipher(iv, key):
    ...
    if not isinstance(key, bytes):
        raise ValueError(b'The encryption key must be of type "bytes", not "%s"' % type(key))
    return Cipher(algorithms.AES(key), modes.CFB8(iv), default_backend())
...
def get_default_aes_encryption_key():
    ...
    """
    return settings.SECRET_KEY[:16].encode(b'utf8')
...
def aes_decrypt(data, key=None):
    ...
    Raises:
        ValueError:
            The encryption key was not in the right format.
    """
    cipher = _create_cipher(data[:AES_BLOCK_SIZE], key or get_default_aes_encryption_key())
...
def decrypt(data):
    ...
    Returns:
        ...
        The decrypted value.
    """
    warnings.warn(b'decrypt() is deprecated. Use aes_decrypt() instead.', RemovedInReviewBoard40Warning)
...
def encrypt(data):
    ...
    Returns:
        ...
        The resulting encrypted value, with the random IV prepended.
    """
    warnings.warn(b'encrypt() is deprecated. Use aes_encrypt() instead.', RemovedInReviewBoard40Warning)
