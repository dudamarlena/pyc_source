...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_review_request_manager.py
# Compiled at: 2020-02-11 04:03:56
from __future__ import unicode_literals
from django.contrib.auth.models import User
from djblets.testing.decorators import add_fixtures
from reviewboard.diffviewer.models import DiffSetHistory
from reviewboard.reviews.models import DefaultReviewer, ReviewRequest, ReviewRequestDraft
...
class ReviewRequestManagerTests(TestCase):
    ...
    def test_create_with_site(self):
        ...
        review_request = ReviewRequest.objects.create(user, repository, local_site=local_site)
    ...
    def test_create_with_site_and_commit_id(self):
        ...
        local_site = LocalSite.objects.create(name=b'test')
        repository = self.create_repository()
        review_request = ReviewRequest.objects.create(user, repository, commit_id=b'123', local_site=local_site)
        self.assertEqual(review_request.repository, repository)
    ...
    def test_create_with_site_and_commit_id_conflicts_review_request(self):
        ...
        local_site = LocalSite.objects.create(name=b'test')
        repository = self.create_repository()
        ReviewRequest.objects.create(user, repository, commit_id=b'123', local_site=local_site)
    ...
    def test_create_with_site_and_commit_id_conflicts_draft(self):
        ...
        local_site = LocalSite.objects.create(name=b'test')
        repository = self.create_repository()
        existing_review_request = ReviewRequest.objects.create(user, repository, local_site=local_site)
        existing_draft = ReviewRequestDraft.create(existing_review_request)
        existing_draft.commit_id = b'123'
        existing_draft.save()
        self.assertEqual(local_site.review_requests.count(), 1)
        with self.assertRaises(ChangeNumberInUseError):
            ReviewRequest.objects.create(user, repository, commit_id=b'123', local_site=local_site)
    ...
    def test_create_with_site_and_commit_id_and_fetch_problem(self):
        ...
        with self.assertRaises(NotImplementedError):
            ReviewRequest.objects.create(user, repository, commit_id=b'123', local_site=local_site, create_from_commit_id=True)
        self.assertEqual(local_site.review_requests.count(), 0)
        self.assertEqual(DiffSetHistory.objects.count(), 0)
        self.assertEqual(ReviewRequestDraft.objects.count(), 0)
    ...
    def test_create_with_create_from_commit_id(self):
        ...
        review_request = ReviewRequest.objects.create(user, repository, commit_id=b'123', create_from_commit_id=True)
        self.assertEqual(review_request.repository, repository)
    ...
    def test_create_with_create_from_commit_id_and_default_reviewers(self):
        ...
        user = User.objects.get(username=b'doc')
        repository = self.create_repository(tool_name=b'Test')
        default_reviewer = DefaultReviewer.objects.create(name=b'Default Reviewer', file_regex=b'.')
    ...
    def test_public(self):
        ...
        user2 = User.objects.get(username=b'grumpy')
        self.create_review_request(summary=b'Test 1', publish=True, submitter=user1)
        self.create_review_request(summary=b'Test 2', submitter=user2)
        self.create_review_request(summary=b'Test 3', status=b'S', public=True, submitter=user1)
        self.create_review_request(summary=b'Test 4', status=b'S', public=True, submitter=user2)
        self.create_review_request(summary=b'Test 5', status=b'D', public=True, submitter=user1)
        self.create_review_request(summary=b'Test 6', status=b'D', submitter=user2)
        self.assertValidSummaries(ReviewRequest.objects.public(user=user1), [
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.public(status=None), [
         b'Test 5',
         b'Test 4',
         b'Test 3',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.public(user=user2, status=None), [
         b'Test 6',
         b'Test 5',
         b'Test 4',
         b'Test 3',
         b'Test 2',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.public(status=None, show_all_unpublished=True), [
         b'Test 6',
         b'Test 5',
         b'Test 4',
         b'Test 3',
    ...
    def test_public_with_repository_on_local_site(self):
        ...
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
    ...
    def test_public_without_private_repo_access_on_local_site(self):
        ...
        user = User.objects.get(username=b'grumpy')
        local_site.users.add(user)
        repository = self.create_repository(public=False, local_site=local_site)
        review_request = self.create_review_request(repository=repository, local_site=local_site, publish=True)
        self.assertFalse(review_request.is_accessible_by(user))
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
        self.assertEqual(review_requests.count(), 0)
    ...
    def test_public_with_private_repo_access_on_local_site(self):
        ...
        repository = self.create_repository(public=False, local_site=local_site)
        repository.users.add(user)
        review_request = self.create_review_request(repository=repository, publish=True, local_site=local_site)
        self.assertTrue(review_request.is_accessible_by(user))
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
        self.assertEqual(review_requests.count(), 1)
    ...
    def test_public_with_private_group_access_on_local_site(self):
        ...
        user = User.objects.get(username=b'grumpy')
        local_site.users.add(user)
        group = self.create_review_group(invite_only=True, local_site=local_site)
        group.users.add(user)
        review_request = self.create_review_request(publish=True, local_site=local_site)
        review_request.target_groups.add(group)
        self.assertTrue(review_request.is_accessible_by(user))
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
        self.assertEqual(review_requests.count(), 1)
    ...
    def test_public_with_private_repo_and_owner_on_local_site(self):
        ...
        local_site.users.add(user)
        repository = self.create_repository(public=False, local_site=local_site)
        review_request = self.create_review_request(repository=repository, submitter=user, local_site=local_site, publish=True)
        self.assertTrue(review_request.is_accessible_by(user))
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
        self.assertEqual(review_requests.count(), 1)
    ...
    def test_public_with_private_group_and_owner_on_local_site(self):
        ...
        local_site.users.add(user)
        group = self.create_review_group(invite_only=True, local_site=local_site)
        review_request = self.create_review_request(submitter=user, local_site=local_site, publish=True)
        review_request.target_groups.add(group)
        self.assertTrue(review_request.is_accessible_by(user))
        review_requests = ReviewRequest.objects.public(user=user, local_site=local_site)
        self.assertEqual(review_requests.count(), 1)
    ...
    def test_to_group(self):
        ...
        user1 = User.objects.get(username=b'doc')
        group1 = self.create_review_group(name=b'privgroup')
        group1.users.add(user1)
        review_request = self.create_review_request(summary=b'Test 1', public=True, submitter=user1)
        review_request.target_groups.add(group1)
        review_request = self.create_review_request(summary=b'Test 2', public=False, submitter=user1)
        review_request.target_groups.add(group1)
        review_request = self.create_review_request(summary=b'Test 3', public=True, status=b'S', submitter=user1)
        review_request.target_groups.add(group1)
        self.assertValidSummaries(ReviewRequest.objects.to_group(b'privgroup', None), [
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.to_group(b'privgroup', None, status=None), [
         b'Test 3',
         b'Test 1'])
        return
    ...
    def test_to_user_group(self):
        ...
        review_request = self.create_review_request(summary=b'Test 1', public=True, submitter=user1)
        review_request.target_groups.add(group1)
        review_request = self.create_review_request(summary=b'Test 2', submitter=user2, public=True, status=b'S')
        review_request.target_groups.add(group1)
        review_request = self.create_review_request(summary=b'Test 3', public=True, submitter=user2)
        review_request.target_groups.add(group1)
        review_request.target_groups.add(group2)
        self.assertValidSummaries(ReviewRequest.objects.to_user_groups(b'doc', local_site=None), [
         b'Test 3',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.to_user_groups(b'doc', status=None, local_site=None), [
         b'Test 3',
         b'Test 2',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.to_user_groups(b'grumpy', user=user2, local_site=None), [
         b'Test 3'])
        return
    ...
    def test_to_user_directly(self):
        ...
        review_request = self.create_review_request(summary=b'Test 2', submitter=user2, status=b'S')
        review_request.target_groups.add(group1)
        review_request.target_people.add(user2)
        ...
        review_request.target_groups.add(group2)
        review_request.target_people.add(user1)
        review_request = self.create_review_request(summary=b'Test 4', public=True, status=b'S', submitter=user2)
        review_request.target_people.add(user1)
        self.assertValidSummaries(ReviewRequest.objects.to_user_directly(b'doc', local_site=None), [
         b'Test 3'])
        self.assertValidSummaries(ReviewRequest.objects.to_user_directly(b'doc', status=None), [
         b'Test 4',
         b'Test 3'])
        self.assertValidSummaries(ReviewRequest.objects.to_user_directly(b'doc', user2, status=None, local_site=None), [
         b'Test 4',
         b'Test 3',
         b'Test 2'])
        return
    ...
    def test_from_user(self):
        ...
        user1 = User.objects.get(username=b'doc')
        self.create_review_request(summary=b'Test 1', public=True, submitter=user1)
        self.create_review_request(summary=b'Test 2', public=False, submitter=user1)
        self.create_review_request(summary=b'Test 3', public=True, status=b'S', submitter=user1)
        self.assertValidSummaries(ReviewRequest.objects.from_user(b'doc', local_site=None), [
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.from_user(b'doc', status=None, local_site=None), [
         b'Test 3',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.from_user(b'doc', user=user1, status=None, local_site=None), [
         b'Test 3',
         b'Test 2',
         b'Test 1'])
        return
    ...
    def test_to_user(self):
        ...
        group2 = self.create_review_group(name=b'group2')
        group2.users.add(user2)
        review_request = self.create_review_request(summary=b'Test 1', publish=True, submitter=user1)
        review_request.target_groups.add(group1)
        review_request = self.create_review_request(summary=b'Test 2', submitter=user2, status=b'S')
        review_request.target_groups.add(group1)
        review_request.target_people.add(user2)
        review_request.target_people.add(user1)
        review_request = self.create_review_request(summary=b'Test 3', publish=True, submitter=user2)
        review_request.target_groups.add(group1)
        review_request.target_groups.add(group2)
        review_request.target_people.add(user1)
        review_request = self.create_review_request(summary=b'Test 4', publish=True, status=b'S', submitter=user2)
        review_request.target_groups.add(group1)
        review_request.target_groups.add(group2)
        review_request.target_people.add(user1)
        self.assertValidSummaries(ReviewRequest.objects.to_user(b'doc', local_site=None), [
         b'Test 3',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.to_user(b'doc', status=None, local_site=None), [
         b'Test 4',
         b'Test 3',
         b'Test 1'])
        self.assertValidSummaries(ReviewRequest.objects.to_user(b'doc', user=user2, status=None, local_site=None), [
         b'Test 4',
         b'Test 3',
         b'Test 2',
         b'Test 1'])
    ...
    def assertValidSummaries(self, review_requests, summaries):
        ...
        for summary in r_summaries:
            self.assertIn(summary, summaries, b'summary "%s" not found in summary list' % summary)

        for summary in summaries:
            self.assertIn(summary, r_summaries, b'summary "%s" not found in review request list' % summary)