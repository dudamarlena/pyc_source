...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/tests/testcases.py
...
class SCMTestCase(SSHTestCase):
    ...
    def _check_can_test_ssh(self):
        ...
        if SCMTestCase._can_test_ssh is None:
            ...
            SCMTestCase._can_test_ssh = key is not None and self.ssh_client.is_key_authorized(key)
        if not SCMTestCase._can_test_ssh:
            raise nose.SkipTest(b"Cannot perform SSH access tests. The local user's SSH public key must be in the %s file and SSH must be enabled." % os.path.join(self.ssh_client.storage.get_ssh_dir(), b'authorized_keys'))
        return
    ...
    def _test_ssh(self, repo_path, filename=None):
        ...
        repo = Repository(name=b'SSH Test', path=repo_path, tool=self.repository.tool)
        ...
        try:
            ...
            if e.errno == ECONNREFUSED:
                ...
                raise nose.SkipTest(b'Cannot perform SSH access tests. No local SSH service is running.')
            else:
                raise
    ...
    def _test_ssh_with_site(self, repo_path, filename=None):
        ...
        self.tempdir = mkdtemp(prefix=b'rb-tests-home-')
        sshdir = os.path.join(self.tempdir, b'.ssh')
        self._set_home(self.tempdir)
        self.assertEqual(sshdir, self.ssh_client.storage.get_ssh_dir())
        self.assertFalse(os.path.exists(os.path.join(sshdir, b'id_rsa')))
        self.assertFalse(os.path.exists(os.path.join(sshdir, b'id_dsa')))
        self.assertEqual(self.ssh_client.get_user_key(), None)
        tool_class = self.repository.tool
        repo = Repository(name=b'SSH Test', path=repo_path, tool=tool_class)
        tool = repo.get_scmtool()
        self.assertRaises(AuthenticationError, lambda : tool.check_repository(repo_path))
        if filename:
            self.assertRaises(SCMError, lambda : tool.get_file(filename, HEAD))
        for local_site_name in ('site-1', ):
            local_site = LocalSite(name=local_site_name)
            local_site.save()
            repo = Repository(name=b'SSH Test', path=repo_path, tool=tool_class, local_site=local_site)
            tool = repo.get_scmtool()
            ssh_client = SSHClient(namespace=local_site_name)
            self.assertEqual(ssh_client.storage.get_ssh_dir(), os.path.join(sshdir, local_site_name))
