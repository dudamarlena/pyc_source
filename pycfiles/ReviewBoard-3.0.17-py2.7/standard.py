...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/backends/standard.py
...
class StandardAuthBackend(BaseAuthBackend, ModelBackend):
    ...
    """
    backend_id = b'builtin'
    name = _(b'Standard Registration')
    ...
    _VALID_LOCAL_SITE_PERMISSIONS = [
     b'hostingsvcs.change_hostingserviceaccount',
     b'hostingsvcs.create_hostingserviceaccount',
     b'reviews.add_group',
     b'reviews.can_change_status',
     b'reviews.can_edit_reviewrequest',
     b'reviews.can_submit_as_another_user',
     b'reviews.can_view_invite_only_groups',
     b'reviews.change_default_reviewer',
     b'reviews.change_group',
     b'reviews.delete_file',
     b'reviews.delete_screenshot',
     b'scmtools.add_repository',
     b'scmtools.change_repository']
    ...
    def get_all_permissions(self, user, obj=None):
        ...
        if obj is not None and not isinstance(obj, LocalSite):
            logging.error(b'Unexpected object %r passed to StandardAuthBackend.get_all_permissions. Returning an empty list.', obj)
            if settings.DEBUG:
                raise ValueError(b'Unexpected object %r' % obj)
            return set()
        if user.is_anonymous():
            return set()
        else:
            permissions = super(StandardAuthBackend, self).get_all_permissions(user)
            if obj is not None:
                if not hasattr(user, b'_local_site_perm_cache'):
                    user._local_site_perm_cache = {}
                if obj.pk not in user._local_site_perm_cache:
                    perm_cache = set()
                    try:
                        site_profile = user.get_site_profile(obj, create_if_missing=False)
                        site_perms = site_profile.permissions or {}
                        if site_perms:
                            perm_cache = set([ key for key, value in six.iteritems(site_perms) if value
                                             ])
    ...
    def has_perm(self, user, perm, obj=None):
        ...
        """
        if obj is not None and not isinstance(obj, LocalSite):
            logging.error(b'Unexpected object %r passed to has_perm. Returning False.', obj)
            if settings.DEBUG:
                raise ValueError(b'Unexpected object %r' % obj)
            return False
        if not user.is_active:
            ...
            if obj is not None:
                if not hasattr(user, b'_local_site_admin_for'):
