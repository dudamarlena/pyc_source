...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/datagrids/builtin_items.py
# Compiled at: 2020-02-11 04:03:56
from __future__ import unicode_literals
from django.utils.translation import ugettext_lazy as _
from reviewboard.datagrids.sidebar import BaseSidebarItem, BaseSidebarSection, SidebarNavItem
...
class OutgoingSection(BaseSidebarSection):
    ...
    def get_items(self):
        ...
        yield SidebarNavItem(self, label=_(b'All'), view_id=b'mine', count=site_profile.total_outgoing_request_count)
        yield SidebarNavItem(self, label=_(b'Open'), view_id=b'outgoing', count=site_profile.pending_outgoing_request_count)
...
class IncomingSection(BaseSidebarSection):
    ...
    def get_items(self):
        ...
        site_profile = self.datagrid.site_profile
        yield SidebarNavItem(self, label=_(b'Open'), view_id=b'incoming', count=site_profile.total_incoming_request_count)
        yield SidebarNavItem(self, label=_(b'To Me'), view_id=b'to-me', count=site_profile.direct_incoming_request_count)
        if site_profile.starred_public_request_count > 0:
            yield SidebarNavItem(self, label=_(b'Starred'), view_id=b'starred', icon_name=b'rb-icon-star-on', count=site_profile.starred_public_request_count)
        groups = self.datagrid.user.review_groups.filter(local_site=self.datagrid.local_site).order_by(b'name')
        seen_groups = set([ group.name for group in groups ])
        for item in self._add_groups(groups, view_id=b'to-group'):
            yield item

        starred_groups = profile.starred_groups.filter(local_site=self.datagrid.local_site).exclude(name__in=seen_groups).order_by(b'name')
        for item in self._add_groups(starred_groups, view_id=b'to-watched-group', icon_name=b'rb-icon-star-on'):
            yield item
    ...
    def _add_groups(self, groups, view_id, icon_name=None):
        for i, group in enumerate(groups):
            ...
            if i == 0:
                ...
                css_classes = []
            yield SidebarNavItem(self, label=name, view_id=view_id, view_args={b'group': name}, icon_name=icon_name, css_classes=css_classes, count=group.incoming_request_count)
...
class UserGroupsItem(BaseSidebarSection):
    ...
    def get_items(self):
        """Yield each of the items within this section."""
        request = self.datagrid.request
        groups = self.datagrid.user.review_groups.accessible(request.user).filter(local_site=self.datagrid.local_site).order_by(b'name')
        for group in groups:
            yield SidebarNavItem(self, label=group.name, url=group.get_absolute_url())