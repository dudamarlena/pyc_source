...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/tests/test_repository_manager.py
...
class RepositoryManagerTests(TestCase):
    ...
    def test_accessible_with_public(self):
        ...
        self.assertIn(repository, Repository.objects.accessible(user))
        self.assertIn(repository, Repository.objects.accessible(AnonymousUser()))
    ...
    def test_accessible_with_public_and_hidden(self):
        ...
        self.assertNotIn(repository, Repository.objects.accessible(user, visible_only=True))
        self.assertIn(repository, Repository.objects.accessible(user, visible_only=False))
        self.assertNotIn(repository, Repository.objects.accessible(anonymous, visible_only=True))
        self.assertIn(repository, Repository.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_private_and_not_member(self):
        ...
        self.assertNotIn(repository, Repository.objects.accessible(user, visible_only=True))
        self.assertNotIn(repository, Repository.objects.accessible(user, visible_only=False))
        self.assertNotIn(repository, Repository.objects.accessible(anonymous, visible_only=True))
        self.assertNotIn(repository, Repository.objects.accessible(anonymous, visible_only=False))
    ...
    def test_accessible_with_private_and_member(self):
        ...
        self.assertIn(repository, Repository.objects.accessible(user, visible_only=True))
        self.assertIn(repository, Repository.objects.accessible(user, visible_only=False))
    ...
    def test_accessible_with_local_site_accessible(self):
        ...
        self.assertIn(repository, Repository.objects.accessible(user, local_site=repository.local_site))
        self.assertIn(repository, Repository.objects.accessible(user, show_all_local_sites=True))