...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_actions.py
...
from reviewboard.reviews.actions import BaseReviewRequestAction, BaseReviewRequestMenuAction, MAX_DEPTH_LIMIT, clear_all_actions, get_top_level_actions, register_actions, unregister_actions
from reviewboard.reviews.default_actions import AddGeneralCommentAction, CloseMenuAction, DeleteAction, DownloadDiffAction, EditReviewAction, ShipItAction, SubmitAction, UpdateMenuAction, UploadDiffAction
from reviewboard.reviews.errors import DepthLimitExceededError
from reviewboard.reviews.models import ReviewRequest
from reviewboard.testing import TestCase

class FooAction(BaseReviewRequestAction):
    action_id = b'foo-action'
    label = b'Foo Action'
...
class BarAction(BaseReviewRequestMenuAction):
    ...
    def __init__(self, action_id, child_actions=None):
        ...
        self.action_id = b'bar-' + action_id


class TopLevelMenuAction(BaseReviewRequestMenuAction):
    action_id = b'top-level-menu-action'
    label = b'Top Level Menu Action'
...
class ActionRegistryTests(ActionsTestCase):
    ...
    def test_register_actions_with_invalid_parent_id(self):
        ...
        message = b'bad-id does not correspond to a registered review request action'
        foo_action.register()
        with self.assertRaisesMessage(KeyError, message):
            register_actions([foo_action], b'bad-id')

    def test_register_actions_with_already_registered_action(self):
        """Testing register_actions with an already registered action"""
        foo_action = FooAction()
        message = b'%s already corresponds to a registered review request action' % foo_action.action_id
        foo_action.register()
        with self.assertRaisesMessage(KeyError, message):
    ...
    def test_register_actions_with_max_depth(self):
        ...
        extra_action = BarAction(b'extra')
        foo_action = FooAction()
    ...
    def test_register_actions_with_too_deep(self):
        ...
        error_message = b'%s exceeds the maximum depth limit of %d' % (
    ...
    def test_unregister_actions(self):
        ...
        orig_action_ids = {action.action_id for action in get_top_level_actions()}
        self.assertIn(b'update-review-request-action', orig_action_ids)
        self.assertIn(b'review-action', orig_action_ids)
        unregister_actions([b'update-review-request-action', b'review-action'])
        new_action_ids = {action.action_id for action in get_top_level_actions()}
        self.assertEqual(len(orig_action_ids), len(new_action_ids) + 2)
        self.assertNotIn(b'update-review-request-action', new_action_ids)
        self.assertNotIn(b'review-action', new_action_ids)
    ...
    def test_unregister_actions_with_unregistered_action(self):
        ...
        foo_action = FooAction()
        error_message = b'%s does not correspond to a registered review request action' % foo_action.action_id
...
class BaseReviewRequestActionTests(ActionsTestCase):
    ...
    def test_register_then_unregister(self):
        ...
        foo_action = FooAction()
        foo_action.register()
        self.assertIn(foo_action.action_id, (action.action_id for action in get_top_level_actions()))
        foo_action.unregister()
        self.assertNotIn(foo_action.action_id, (action.action_id for action in get_top_level_actions()))
    ...
    def test_register_with_already_registered(self):
        ...
        """
        foo_action = FooAction()
        error_message = b'%s already corresponds to a registered review request action' % foo_action.action_id
        foo_action.register()
        with self.assertRaisesMessage(KeyError, error_message):
            foo_action.register()
    ...
    def test_unregister_with_unregistered_action(self):
        ...
        """
        foo_action = FooAction()
        message = b'%s does not correspond to a registered review request action' % foo_action.action_id
        with self.assertRaisesMessage(KeyError, message):
            foo_action.unregister()
    ...
    def test_render_pops_context_even_after_error(self):
        ...
        error
        """
        context = Context({b'comment': b'this is a comment'})
...
class AddGeneralCommentActionTests(ActionsTestCase):
    ...
    def test_should_render_with_authenticated(self):
        ...
        self.assertTrue(self.action.should_render({b'request': request}))
    ...
    def test_should_render_with_anonymous(self):
        ...
        self.assertFalse(self.action.should_render({b'request': request}))
...
class CloseMenuActionTests(ActionsTestCase):
    ...
    def test_should_render_for_owner(self):
        ...
        self.assertTrue(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
           b'perms': {b'reviews': {b'can_change_status': False}}}))
    ...
    def test_should_render_for_user(self):
        ...
        self.assertFalse(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
           b'perms': {b'reviews': {b'can_change_status': False}}}))
    ...
    def test_should_render_user_with_can_change_status(self):
        ...
        self.assertTrue(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
           b'perms': {b'reviews': {b'can_change_status': True}}}))
    ...
    def test_should_render_with_discarded(self):
        ...
        """
        review_request = self.create_review_request(status=ReviewRequest.DISCARDED)
        request = RequestFactory().request()
        request.user = review_request.submitter
        self.assertFalse(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
    ...
    def test_should_render_with_submitted(self):
        ...
        """
        review_request = self.create_review_request(status=ReviewRequest.SUBMITTED)
        request = RequestFactory().request()
        request.user = review_request.submitter
        self.assertFalse(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
...
class DeleteActionTests(ActionsTestCase):
    ...
    def test_should_render_with_published(self):
        ...
        self.assertFalse(self.action.should_render({b'perms': {b'reviews': {b'delete_reviewrequest': False}}}))

    def test_should_render_with_permission(self):
        """Testing SubmitAction.should_render with delete_reviewrequest
        permission
        """
        self.assertTrue(self.action.should_render({b'perms': {b'reviews': {b'delete_reviewrequest': True}}}))
...
class DownloadDiffActionTests(ActionsTestCase):
    ...
    def test_get_url_on_diff_viewer(self):
        ...
        request.resolver_match.url_name = b'view-diff'
        self.assertEqual(self.action.get_url({b'request': request}), b'raw/')

    def test_get_url_on_interdiff(self):
        """Testing DownloadDiffAction.get_url on diff viewer interdiff page"""
        request = RequestFactory().request()
        request.resolver_match = Mock()
        request.resolver_match.url_name = b'view-interdiff'
        self.assertEqual(self.action.get_url({b'request': request}), b'raw/')
    ...
    def test_get_url_on_diff_viewer_revision(self):
        ...
        request = RequestFactory().request()
        request.resolver_match = Mock()
        request.resolver_match.url_name = b'view-diff-revision'
        self.assertEqual(self.action.get_url({b'request': request}), b'raw/')
    ...
    def test_get_url_on_review_request(self):
        ...
        request = RequestFactory().request()
        request.resolver_match = Mock()
        request.resolver_match.url_name = b'review-request-detail'
        review_request = self.create_review_request()
        self.assertEqual(self.action.get_url({b'request': request, 
           b'review_request': review_request}), b'/r/%s/diff/raw/' % review_request.display_id)
    ...
    def test_get_url_on_review_request_with_local_site(self):
        ...
        review_request = self.create_review_request(id=123, with_local_site=True)
        self.assertEqual(self.action.get_url({b'request': request, 
           b'review_request': review_request}), b'/s/%s/r/%s/diff/raw/' % (self.local_site_name,
    ...
    def test_get_hidden_on_diff_viewer(self):
        ...
        request.resolver_match = Mock()
        request.resolver_match.url_name = b'view-diff'
        self.assertFalse(self.action.get_hidden({b'request': request}))
    ...
    def test_get_hidden_on_interdiff(self):
        ...
        request.resolver_match = Mock()
        request.resolver_match.url_name = b'view-interdiff'
        self.assertTrue(self.action.get_hidden({b'request': request}))
...
class SubmitActionTests(ActionsTestCase):
    ...
    def test_should_render_with_published(self):
        ...
        self.assertTrue(self.action.should_render({b'review_request': self.create_review_request(public=True)}))

    def test_should_render_with_unpublished(self):
        """Testing SubmitAction.should_render with unpublished review request
        """
        self.assertFalse(self.action.should_render({b'review_request': self.create_review_request(public=False)}))
...
class UpdateMenuActionTests(ActionsTestCase):
    ...
    def test_should_render_for_owner(self):
        ...
        self.assertTrue(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
           b'perms': {b'reviews': {b'can_edit_reviewrequest': False}}}))
    ...
    def test_should_render_user_with_can_edit_reviewrequest(self):
        ...
        self.assertTrue(self.action.should_render({b'review_request': review_request, 
           b'request': request, 
           b'perms': {b'reviews': {b'can_edit_reviewrequest': True}}}))
...
class UploadDiffActionTests(ActionsTestCase):
    ...
    def test_get_label_with_no_diffs(self):
        ...
        request = RequestFactory().request()
        request.user = review_request.submitter
        self.assertEqual(self.action.get_label({b'review_request': review_request, 
           b'request': request}), b'Upload Diff')
    ...
    def test_get_label_with_diffs(self):
        ...
        request.user = review_request.submitter
        self.assertEqual(self.action.get_label({b'review_request': review_request, 
           b'request': request}), b'Update Diff')
