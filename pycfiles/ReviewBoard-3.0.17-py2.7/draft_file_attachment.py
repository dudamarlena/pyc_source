...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/draft_file_attachment.py
...
class DraftFileAttachmentResource(BaseReviewRequestFileAttachmentResource):
    ...
    """
    added_in = b'1.6'
    name = b'draft_file_attachment'
    uri_name = b'file-attachments'
    model_parent_key = b'drafts'
    allowed_methods = ('GET', 'DELETE', 'POST', 'PUT')

    def get_queryset(self, request, review_request_id, *args, **kwargs):
        try:
            draft = resources.review_request_draft.get_object(request, review_request_id=review_request_id, *args, **kwargs)
            inactive_ids = draft.inactive_file_attachments.values_list(b'pk', flat=True)
    ...
    def _get_list_impl(self, request, *args, **kwargs):
        ...
        part of the draft.
        """
        return WebAPIResponsePaginated(request, queryset=self._get_queryset(request, is_list=True, *args, **kwargs), results_key=self.list_result_key, serialize_object_func=(lambda obj: self.serialize_object(obj, request=request, *args, **kwargs)), extra_data={b'links': self.get_links(self.list_child_resources, request=request, *args, **kwargs)}, **self.build_response_args(request))
