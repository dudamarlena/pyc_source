...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/review_diff_comment.py
...
from reviewboard.webapi.resources.base_diff_comment import BaseDiffCommentResource
...
class ReviewDiffCommentResource(BaseDiffCommentResource):
    ...
    policy_id = b'review_diff_comment'
    model_parent_key = b'review'
    mimetype_list_resource_name = b'review-diff-comments'
    mimetype_item_resource_name = b'review-diff-comment'

    def get_queryset(self, request, review_id, *args, **kwargs):
        q = super(ReviewDiffCommentResource, self).get_queryset(request, *args, **kwargs)
    ...
    @webapi_request_fields(required=dict({b'filediff_id': {b'type': int, 
                        b'description': b'The ID of the file diff the comment is on.'}, 
       b'first_line': {b'type': int, 
                       b'description': b'The line number the comment starts at.'}, 
       b'num_lines': {b'type': int, 
                      b'description': b'The number of lines the comment spans.'}}, **BaseDiffCommentResource.REQUIRED_CREATE_FIELDS), optional=dict({b'interfilediff_id': {b'type': int, 
                             b'description': b'The ID of the second file diff in the interdiff the comment is on.'}}, **BaseDiffCommentResource.OPTIONAL_CREATE_FIELDS), allow_unknown=True)
    def create(self, request, filediff_id, interfilediff_id=None, *args, **kwargs):
        ...
        if not resources.review.has_modify_permissions(request, review):
            ...
            invalid_fields = {}
            try:
                filediff = FileDiff.objects.get(pk=filediff_id, diffset__history__review_request=review_request)
            except ObjectDoesNotExist:
                invalid_fields[b'filediff_id'] = [
                 b'This is not a valid filediff ID']

            if filediff and interfilediff_id:
                if interfilediff_id == filediff.id:
                    invalid_fields[b'interfilediff_id'] = [b'This cannot be the same as filediff_id']
                else:
                    try:
                        interfilediff = FileDiff.objects.get(pk=interfilediff_id, diffset__history=filediff.diffset.history)
                    except ObjectDoesNotExist:
                        invalid_fields[b'interfilediff_id'] = [
                         b'This is not a valid interfilediff ID']

            if invalid_fields:
                return (INVALID_FORM_DATA,
                 {b'fields': invalid_fields})
            return self.create_comment(review=review, comments_m2m=review.comments, filediff=filediff, interfilediff=interfilediff, fields=('filediff',
                                                                                                                                            'interfilediff',
                                                                                                                                            'first_line',
                                                                                                                                            'num_lines'), **kwargs)
    ...
    @webapi_request_fields(optional=dict({b'first_line': {b'type': int, 
                       b'description': b'The line number the comment starts at.'}, 
       b'num_lines': {b'type': int, 
                      b'description': b'The number of lines the comment spans.'}}, **BaseDiffCommentResource.OPTIONAL_UPDATE_FIELDS), allow_unknown=True)
    def update(self, request, *args, **kwargs):
        ...
        return self.update_comment(request=request, review=review, comment=diff_comment, update_fields=('first_line',
                                                                                                        'num_lines'), **kwargs)
