...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/models/base_review_request_details.py
...
class BaseReviewRequestDetails(models.Model):
    ...
    description = models.TextField(_(b'description'), blank=True)
    description_rich_text = models.BooleanField(_(b'description in rich text'), default=False)
    testing_done = models.TextField(_(b'testing done'), blank=True)
    testing_done_rich_text = models.BooleanField(_(b'testing done in rich text'), default=False)
    bugs_closed = models.CharField(_(b'bugs'), max_length=300, blank=True)
    branch = models.CharField(_(b'branch'), max_length=300, blank=True)
    commit_id = models.CharField(_(b'commit ID'), max_length=64, blank=True, null=True, db_index=True)
    extra_data = JSONField(null=True)
    rich_text = models.BooleanField(_(b'rich text'), default=False)
    ...
    def get_bug_list(self):
        ...
        if self.bugs_closed == b'':
            return []
        bugs = list(set(re.split(b'[, ]+', self.bugs_closed)))
        try:
            bugs.sort(key=int)
        except ValueError:
            bugs.sort()
    ...
    def get_file_attachments(self):
        ...
        def get_attachments(review_request):
            for file_attachment in self.file_attachments.all():
                ...
                if not file_attachment.is_from_diff and file_attachment.attachment_history is None:
                    history = FileAttachmentHistory.objects.create(display_position=FileAttachmentHistory.compute_next_display_position(review_request))
                    review_request.file_attachment_histories.add(history)
                    file_attachment.attachment_history = history
                    file_attachment.save(update_fields=[b'attachment_history'])
        ...
        if self.file_attachments_count > 0:
            review_request = self.get_review_request()
            return sorted(get_attachments(review_request), key=get_display_position)
        else:
            return []
    ...
    def add_default_reviewers(self):
        ...
        files = diffset.files.values_list(b'source_file', b'dest_file')
        default_reviewers = DefaultReviewer.objects.for_repository(self.repository, self.local_site).only(b'pk', b'file_regex')
        ...
        if not match_default_reviewer_ids:
            return
        self.target_people.add(*(entry.user for entry in DefaultReviewer.people.through.objects.filter(defaultreviewer_id__in=match_default_reviewer_ids, user__is_active=True).select_related(b'user')))
        self.target_groups.add(*(entry.group for entry in DefaultReviewer.groups.through.objects.filter(defaultreviewer_id__in=match_default_reviewer_ids).select_related(b'group')))
    ...
    def __str__(self):
        if self.summary:
            ...
            return six.text_type(_(b'(no summary)'))
