...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/diffviewer/chunk_generator.py
# Compiled at: 2020-02-11 04:03:56
from __future__ import unicode_literals
import fnmatch, functools, hashlib, re
...
from reviewboard.diffviewer.diffutils import get_line_changed_regions, get_original_file, get_patched_file, convert_to_unicode, split_line_endings
from reviewboard.diffviewer.opcode_generator import DiffOpcodeGenerator, get_diff_opcode_generator
...
class RawDiffChunkGenerator(object):
    ...
    """
    NEWLINES_RE = re.compile(b'\\r?\\n')
    STYLED_MAX_LINE_LEN = 1000
    STYLED_MAX_LIMIT_BYTES = 200000
    STYLED_EXT_BLACKLIST = ('.txt', )
    TAB_SIZE = DiffOpcodeGenerator.TAB_SIZE

    def __init__(self, old, new, orig_filename, modified_filename, enable_syntax_highlighting=True, encoding_list=None, diff_compat=DiffCompatVersion.DEFAULT):
        self.old = old
        self.new = new
        self.orig_filename = orig_filename
        self.modified_filename = modified_filename
        self.enable_syntax_highlighting = enable_syntax_highlighting
        self.encoding_list = encoding_list or [b'iso-8859-15']
    ...
    def generate_chunks(self, old, new):
        ...
        if is_lists:
            ...
            if self._get_enable_syntax_highlighting(old, new, a, b):
                source_file = self.normalize_path_for_display(self.orig_filename)
                dest_file = self.normalize_path_for_display(self.modified_filename)
                try:
                    if not source_file.endswith(self.STYLED_EXT_BLACKLIST):
                        markup_a = self._apply_pygments(old or b'', source_file)
                    if not dest_file.endswith(self.STYLED_EXT_BLACKLIST):
                        markup_b = self._apply_pygments(new or b'', dest_file)
                except:
                    pass
            ...
            ignore_space = True
            for pattern in siteconfig.get(b'diffviewer_include_space_patterns'):
                if fnmatch.fnmatch(self.orig_filename, pattern):
                    ignore_space = False
                    break

            self.differ = get_differ(a, b, ignore_space=ignore_space, compat_version=self.diff_compat)
            self.differ.add_interesting_lines_for_headers(self.orig_filename)
            context_num_lines = siteconfig.get(b'diffviewer_context_num_lines')
            ...
            for tag, i1, i2, j1, j2, meta in opcodes_generator:
                ...
                lines = map(functools.partial(self._diff_line, tag, meta), range(line_num, line_num + num_lines), range(i1 + 1, i2 + 1), range(j1 + 1, j2 + 1), a[i1:i2], b[j1:j2], old_lines, new_lines)
                counts[tag] += num_lines
                if tag == b'equal' and num_lines > collapse_threshold:
                    ...
                    if line_num == 1:
                        ...
                        if i2 == a_num_lines and j2 == b_num_lines:
                            yield self._new_chunk(lines, context_num_lines, num_lines, True)
                        else:
                            yield self._new_chunk(lines, context_num_lines, last_range_start, True)
                            yield self._new_chunk(lines, last_range_start, num_lines)
                else:
    ...
    def normalize_source_string(self, s):
        ...
        if s and not s.endswith(b'\n'):
            s += b'\n'
        lines = self.NEWLINES_RE.split(s or b'')
        del lines[-1]
        return (
         s, lines)
    ...
    def normalize_source_list(self, l):
        ...
        """
        if self.encoding_list:
            l = [ convert_to_unicode(s, self.encoding_list)[1] for s in l
                ]
        return l
    ...
    def _get_enable_syntax_highlighting(self, old, new, a, b):
        ...
        if not self.enable_syntax_highlighting:
            return False
        siteconfig = SiteConfiguration.objects.get_current()
        threshold = siteconfig.get(b'diffviewer_syntax_highlighting_threshold')
        if threshold and (len(a) > threshold or len(b) > threshold):
            return False
        if len(old) > self.STYLED_MAX_LIMIT_BYTES or len(new) > self.STYLED_MAX_LIMIT_BYTES:
            return False
        for lines in (a, b):
            for line in lines:
    ...
    def _diff_line(self, tag, meta, v_line_num, old_line_num, new_line_num, old_line, new_line, old_markup, new_markup):
        ...
        and other metadata.
        """
        if tag == b'replace' and old_line and new_line and len(old_line) <= self.STYLED_MAX_LINE_LEN and len(new_line) <= self.STYLED_MAX_LINE_LEN and old_line != new_line:
            old_region, new_region = self.get_line_changed_regions(old_line_num, old_line, new_line_num, new_line)
        else:
            old_region = new_region = []
        old_markup = old_markup or b''
        new_markup = new_markup or b''
        line_pair = (
         old_line_num, new_line_num)
        indentation_changes = meta.get(b'indentation_changes', {})
        if line_pair[0] is not None and line_pair[1] is not None:
            indentation_change = indentation_changes.get(b'%d-%d' % line_pair)
            if indentation_change:
                old_markup, new_markup = self._highlight_indentation(old_markup, new_markup, *indentation_change)
        result = [v_line_num,
         old_line_num or b'', mark_safe(old_markup), old_region,
         new_line_num or b'', mark_safe(new_markup), new_region,
         line_pair in meta[b'whitespace_lines']]
        moved_info = {}
        for direction, moved_line_num in ((b'to', old_line_num),
         (
          b'from', new_line_num)):
            moved_meta = meta.get(b'moved-%s' % direction, {})
            direction_move_info = self._get_move_info(moved_line_num, moved_meta)
    ...
    def _get_move_info(self, line_num, moved_meta):
        ...
        return (
         other_line_num,
         line_num - 1 not in moved_meta or other_line_num != moved_meta[(line_num - 1)] + 1)

    def _highlight_indentation(self, old_markup, new_markup, is_indent, raw_indent_len, norm_indent_len_diff):
        ...
        """
        if is_indent:
            new_markup = self._wrap_indentation_chars(b'indent', new_markup, raw_indent_len, norm_indent_len_diff, self._serialize_indentation)
        else:
            old_markup = self._wrap_indentation_chars(b'unindent', old_markup, raw_indent_len, norm_indent_len_diff, self._serialize_unindentation)
        return (old_markup, new_markup)

    def _wrap_indentation_chars(self, class_name, markup, raw_indent_len, norm_indent_len_diff, serializer):
        ...
        """
        start_pos = 0
        while markup[start_pos] == b'<':
            end_tag_pos = markup.find(b'>', start_pos + 1)
        ...
        return b'%s<span class="%s">%s</span>%s' % (
         markup[:start_pos],
         class_name,
         serialized,
    ...
    def _new_chunk(self, all_lines, start, end, collapsable=False, tag=b'equal', meta=None):
        """Creates a chunk.
        ...
        if not meta:
            meta = {}
        left_headers = list(self._get_interesting_headers(all_lines, start, end - 1, False))
        right_headers = list(self._get_interesting_headers(all_lines, start, end - 1, True))
        meta[b'left_headers'] = left_headers
        meta[b'right_headers'] = right_headers
        lines = all_lines[start:end]
        num_lines = len(lines)
        compute_chunk_last_header(lines, num_lines, meta, self._last_header)
        if collapsable and end < len(all_lines) and (self._last_header[0] or self._last_header[1]):
            meta[b'headers'] = list(self._last_header)
        chunk = {b'index': self._chunk_index, 
           b'lines': lines, 
           b'numlines': num_lines, 
           b'change': tag, 
           b'collapsable': collapsable, 
           b'meta': meta}
        self._chunk_index += 1
        return chunk
    ...
    def _get_interesting_headers(self, lines, start, end, is_modified_file):
        ...
        """
        possible_functions = self.differ.get_interesting_lines(b'header', is_modified_file)
    ...
    def _apply_pygments(self, data, filename):
        ...
        """
        lexer = guess_lexer_for_filename(filename, data, stripnl=False, encoding=b'utf-8')
        lexer.add_filter(b'codetagify')
...
class DiffChunkGenerator(RawDiffChunkGenerator):
    ...
    def __init__(self, request, filediff, interfilediff=None, force_interdiff=False, enable_syntax_highlighting=True):
        assert filediff
        ...
        super(DiffChunkGenerator, self).__init__(old=None, new=None, orig_filename=filediff.source_file, modified_filename=filediff.dest_file, enable_syntax_highlighting=enable_syntax_highlighting, encoding_list=self.repository.get_encoding_list(), diff_compat=filediff.diffset.diffcompat)
        return

    def make_cache_key(self):
        """Create a cache key for any generated chunks."""
        key = b'diff-sidebyside-'
        if self.enable_syntax_highlighting:
            key += b'hl-'
        if not self.force_interdiff:
            key += six.text_type(self.filediff.pk)
        elif self.interfilediff:
            key += b'interdiff-%s-%s' % (self.filediff.pk,
             self.interfilediff.pk)
        else:
            key += b'interdiff-%s-none' % self.filediff.pk
        key += b'-%s' % get_language()
        return key
    ...
    def get_chunks(self):
        ...
        if self.filediff.binary or self.filediff.source_revision == b'' or (self.filediff.is_new or self.filediff.deleted or self.filediff.moved or self.filediff.copied) and counts[b'raw_insert_count'] == 0 and counts[b'raw_delete_count'] == 0:
            raise StopIteration
    ...
    def get_chunks_uncached(self):
        ...
        if self.filediff.orig_sha1 is None:
            self.filediff.extra_data.update({b'orig_sha1': self._get_checksum(old), 
               b'patched_sha1': self._get_checksum(new)})
            self.filediff.save(update_fields=[b'extra_data'])
        if self.interfilediff:
            old = new
            ...
            if self.interfilediff.orig_sha1 is None:
                self.interfilediff.extra_data.update({b'orig_sha1': self._get_checksum(interdiff_orig), 
                   b'patched_sha1': self._get_checksum(new)})
                self.interfilediff.save(update_fields=[b'extra_data'])
        else:
            if self.force_interdiff:
                old, new = new, old
            if self.interfilediff:
                log_timer = log_timed(b'Generating diff chunks for interdiff ids %s-%s (%s)' % (
                 self.filediff.id, self.interfilediff.id,
                 self.filediff.source_file), request=self.request)
            else:
                log_timer = log_timed(b'Generating diff chunks for self.filediff id %s (%s)' % (
                 self.filediff.id, self.filediff.source_file), request=self.request)
            for chunk in self.generate_chunks(old, new):
                yield chunk

        log_timer.done()
        if not self.interfilediff and not self.force_interdiff:
            insert_count = self.counts[b'insert']
            delete_count = self.counts[b'delete']
            replace_count = self.counts[b'replace']
            equal_count = self.counts[b'equal']
            self.filediff.set_line_counts(insert_count=insert_count, delete_count=delete_count, replace_count=replace_count, equal_count=equal_count, total_line_count=insert_count + delete_count + replace_count + equal_count)
        return
...
def compute_chunk_last_header(lines, numlines, meta, last_header=None):
    ...
    if last_header is None:
        last_header = [
         None, None]
    line = lines[0]
    for i, (linenum, header_key) in enumerate([(line[1], b'left_headers'),
     (
      line[4], b'right_headers')]):
        headers = meta[header_key]
        if headers:
            header = headers[(-1)]
            last_header[i] = {b'line': header[0], 
               b'text': header[1].strip()}
