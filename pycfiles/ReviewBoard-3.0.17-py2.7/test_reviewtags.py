...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_reviewtags.py
...
from reviewboard.reviews.fields import BaseReviewRequestField, BaseReviewRequestFieldSet, register_review_request_fieldset, unregister_review_request_fieldset
...
class DisplayReviewRequestTrophiesTests(TestCase):
    ...
    def test_old_style_trophy(self):
        ...
        t = Template(b'{% load reviewtags %}{% display_review_request_trophies review_request %}')
        request = self._request_factory.get(b'/')
        ...
        with self.assert_warns(RemovedInReviewBoard40Warning, message):
            text = t.render(RequestContext(request, {b'review_request': review_request}))
        self.assertIn(b'Trophy get!', text)
    ...
    def test_new_style_trophy(self):
        ...
        class SomeTrophy(TrophyType):
            ...
            image_width = 1
            image_height = 1
            display_format_str = b'Trophy get!'
...
class ForReviewRequestFieldTests(SpyAgency, TestCase):
    ...
    def test_render_instantiated_fields(self):
        ...
        try:
            t = Template(b'{% load reviewtags %}{% for_review_request_field review_request fieldset %}Never reached.{% end_for_review_request_field %}')
            result = t.render(Context({b'review_request': review_request, 
               b'fieldset': TestFieldSet(review_request)}))
        ...
        self.assertEqual(logging.exception.spy.calls[0].args[2].args, (
         exception_id,))
        return
...
class DiffCommentLineNumbersTests(TestCase):
    ...
    def test_delete_single_lines(self):
        ...
        commented line
        """
        t = Template(b'{% load reviewtags %}{% diff_comment_line_numbers chunks comment %}')
        result = t.render(Context({b'comment': Comment(first_line=20, num_lines=1), 
           b'chunks': [
                     {b'change': b'delete', 
                        b'lines': [
                                 (
        ...
        self.assertEqual(result, b'Line 30 (original)')
    ...
    def test_delete_mutiple_lines(self):
        ...
        """
        t = Template(b'{% load reviewtags %}{% diff_comment_line_numbers chunks comment %}')
        result = t.render(Context({b'comment': Comment(first_line=20, num_lines=2), 
           b'chunks': [
                     {b'change': b'delete', 
                        b'lines': [
                                 (
                                 ...
                                 (
                                  50, 60, b'deleted line', [], b'', b'', [], False)]}]}))
        self.assertEqual(result, b'Lines 30-31 (original)')
        ...
        result = t.render(Context({b'comment': Comment(first_line=20, num_lines=1), 
           b'chunks': [
                     {b'change': b'replace', 
                        b'lines': [
                                 (
                                  10, 20, b'foo', [], 20, b'replaced line', [], False),
                                 (
                                  50, 60, b'foo', [], 60, b'replaced line', [], False)]}]}))
        self.assertEqual(result, b'Line 30 (original), 30 (patched)')
           ...
           b'chunks': [
                     {b'change': b'replace', 
                        b'lines': [
                                 ...
                                 (
                                  50, 60, b'foo', [], 60, b'replaced line', [], False)]}]}))
        self.assertEqual(result, b'Lines 30-31 (original), 30-31 (patched)')
    ...
    def test_insert_single_line(self):
        ...
        self.assertEqual(result, b'Lines 30 (patched)')
           ...
           b'chunks': [
                     {b'change': b'insert', 
                        b'lines': [
                                 ...
                                 (
                                  50, b'', b'', [], 60, b'inserted line', [], False)]}]}))
        self.assertEqual(result, b'Lines 30-31 (patched)')
    ...
    def test_spanning_inserts_deletes(self):
        ...
        t = Template(b'{% load reviewtags %}{% diff_comment_line_numbers chunks comment %}')
        result = t.render(Context({b'comment': Comment(first_line=20, num_lines=50), 
           b'chunks': [
                     {b'change': b'delete', 
                        b'lines': [
                                 (
                     ...
                     {b'change': b'equal', 
                        b'lines': [
                                 ...
                                 (
                                  ...
                                  False)]}]}))
        self.assertEqual(result, b'Lines 30-60 (original), 61-79 (patched)')
                     ...
                     {b'change': b'equal', 
                        b'lines': [
                                 ...
                                 (
                                  ...
                                  False)]}]}))
        self.assertEqual(result, b'Lines 61-79 (original), 30-60 (patched)')
...
class CommentRepliesTests(TestCase):
    ...
    def _test_diff_comments(self, user_is_owner):
        review_request = self.create_review_request(publish=True, create_repository=True)
        diffset = self.create_diffset(review_request)
        filediff = self.create_filediff(diffset)
        review = self.create_review(review_request, publish=True)
        comment = self.create_diff_comment(review, filediff)
        self._check_replies(review, comment, self.create_diff_comment, {b'filediff': filediff}, user_is_owner)

    def _test_general_comments(self, user_is_owner):
        review_request = self.create_review_request(publish=True)
        review = self.create_review(review_request, publish=True)
        comment = self.create_general_comment(review)
        self._check_replies(review, comment, self.create_general_comment, {}, user_is_owner)
    ...
    def _test_file_attachment_comments(self, user_is_owner):
        ...
        self._check_replies(review, comment, self.create_file_attachment_comment, {b'file_attachment': file_attachment}, user_is_owner)
    ...
    def _test_screenshot_comments(self, user_is_owner):
        ...
        self._check_replies(review, comment, self.create_screenshot_comment, {b'screenshot': screenshot}, user_is_owner)

    def _check_replies(self, review, comment, create_comment_func, create_comment_kwargs, user_is_owner):
        reply_kwargs = {b'review': review, 
           b'user': review.user}
        create_comment_kwargs[b'reply_to'] = comment
        ...
        t = Template(b'{% load reviewtags %}{% comment_replies review comment "123" %}')
        request = RequestFactory().request()
        if user_is_owner:
            request.user = review.user
        else:
            request.user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        html = t.render(RequestContext(request, {b'comment': comment, 
           b'review': review}))
        self.assertIn(b'data-comment-id="%s"' % reply_comment1.pk, html)
        self.assertIn(b'data-comment-id="%s"' % reply_comment2.pk, html)
        self.assertIn(b'data-comment-id="%s"' % reply_comment3.pk, html)
        if user_is_owner:
            self.assertIn(b'<li class="draft" data-comment-id="%s"' % reply_comment4.pk, html)
        else:
            self.assertNotIn(b'data-comment-id="%s"' % reply_comment4.pk, html)
...
class ReviewBodyRepliesTests(TestCase):
    ...
    def _test_body_field(self, body_field, user_is_owner):
        ...
        reply_kwargs = {b'review': review, 
           b'user': review.user, 
           b'%s_reply_to' % body_field: review, 
           body_field: b'Some reply'}
        reply1 = self.create_reply(publish=True, **reply_kwargs)
        reply2 = self.create_reply(publish=True, **reply_kwargs)
        reply3 = self.create_reply(publish=False, **reply_kwargs)
        t = Template(b'{%% load reviewtags %%}{%% review_body_replies review "%s" "123" %%}' % body_field)
        request = RequestFactory().request()
        if user_is_owner:
            request.user = review.user
        else:
            request.user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        html = t.render(RequestContext(request, {b'review': review}))
        self.assertIn(b'id="comment_123-%s"' % reply1.pk, html)
        self.assertIn(b'id="comment_123-%s"' % reply2.pk, html)
        if user_is_owner:
            self.assertIn(b'id="draftcomment_123-%s"' % reply3.pk, html)
        else:
            self.assertNotIn(b'id="comment_123-%s"' % reply3.pk, html)
            self.assertNotIn(b'id="draftcomment_123-%s"' % reply3.pk, html)