...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/accounts/trophies.py
# Compiled at: 2020-02-11 04:03:56
from __future__ import unicode_literals
import re, warnings
from django.utils import six
from django.utils.translation import ugettext_lazy as _
from djblets.registries.registry import ALREADY_REGISTERED, AlreadyRegisteredError, ATTRIBUTE_REGISTERED, DEFAULT_ERRORS, ItemLookupError, NOT_REGISTERED, Registry, UNREGISTER
...
class TrophyType(object):
    ...
    def __init__(self, title=None, image_url=None, image_width=None, image_height=None):
        ...
        if not self.name and title:
            warnings.warn(b'%r should define "name" as a class attribute instead of passing "title" to the constructor.' % self.__class__, RemovedInReviewBoard40Warning)
            self.name = title
        if not self.image_urls and image_url:
            warnings.warn(b'%r should define "image_urls" as a class attribute instead of passing "image_url" to the constructor.' % self.__class__, RemovedInReviewBoard40Warning)
            self.image_urls = {b'1x': image_url}
        if not self.image_width:
            warnings.warn(b'%r should define "image_width" as a class attribute instead of passing "image_width" to the constructor.' % self.__class__, RemovedInReviewBoard40Warning)
            self.image_width = image_width or 32
        if not self.image_height:
            warnings.warn(b'%r should define "image_height" as a class attribute instead of passing "image_height" to the constructor.' % self.__class__, RemovedInReviewBoard40Warning)
    ...
    def format_display_text(self, request, trophy, **kwargs):
        ...
        Returns:
            ...
            The rendered text.
        """
        if getattr(self, b'display_format_str', None) is None:
            try:
                text = self.get_display_text(trophy)
            except NotImplementedError:
                raise NotImplementedError(b'%s does not define the format_display_text attribute.' % type(self).__name__)
            else:
                warnings.warn(b'TrophyType.get_display_text has been deprecated in favor of TrophyType.format_display_text.', RemovedInReviewBoard40Warning)
                return text

        return self.display_format_str % dict(kwargs, **{b'recipient': trophy.user.get_profile().get_display_name(getattr(request, b'user', None)), 
           b'review_request_id': trophy.review_request.display_id})
...
class MilestoneTrophy(TrophyType):
    ...
    title = _(b'Milestone Trophy')
    image_urls = {b'1x': static_lazy(b'rb/images/trophies/sparkly.png'), 
       b'2x': static_lazy(b'rb/images/trophies/sparkly@2x.png')}
    image_width = 33
    image_height = 35
    display_format_str = _(b'%(recipient)s got review request #%(review_request_id)d!')
    ...
    def qualifies(self, review_request):
        ...
        Returns:
            ...
            ``True`` if the trophy should be given, or ``False`` if not.
        """
        return review_request.display_id >= 1000 and re.match(b'^[1-9]0+$', six.text_type(review_request.display_id))
...
class FishTrophy(TrophyType):
    ...
    """
    category = b'fish'
    name = _(b'Fish Trophy')
    image_urls = {b'1x': static_lazy(b'rb/images/trophies/fish.png'), 
       b'2x': static_lazy(b'rb/images/trophies/fish@2x.png')}
    image_width = 33
    image_height = 37
    display_format_str = _(b'%(recipient)s got a fish trophy!')
    ...
    def qualifies(self, review_request):
        ...
        return review_request.display_id >= 1000 and id_str == (b'').join(reversed(id_str))
...
class UnknownTrophy(TrophyType):
    ...
    """
    name = b'Unknown Trophy'


class TrophyRegistry(Registry):
    lookup_attrs = ('category', )
    default_errors = dict(DEFAULT_ERRORS, **{ALREADY_REGISTERED: _(b'Could not register trophy type %(item)s. This trophy type is already registered or its category conficts with another trophy.'), 
       ATTRIBUTE_REGISTERED: _(b'Could not register trophy type %(item)s: Another trophy type (%(duplicate)s) is already registered with the same category.'), 
       NOT_REGISTERED: _(b'No trophy type was found matching "%(attr_value)s".'), 
       UNREGISTER: _(b'Could not unregister trophy type %(item)s: This trophy type was not yet registered.')})
    ...
    def get_for_category(self, category):
        ...
        """
        try:
            return self.get(b'category', category)
...
def register_trophy(trophy):
    ...
    """
    warnings.warn(b'register_trophy() is deprecated. Please use reviewboard.accounts.trophies:trophies_registry.register() instead.', RemovedInReviewBoard40Warning)
    if not issubclass(trophy, TrophyType):
        raise TypeError(b'Only TrophyType subclasses can be registered')
    try:
...
def unregister_trophy(trophy):
    ...
    """
    warnings.warn(b'unregister_trophy() is deprecated. Please use reviewboard.accounts.trophies:trophies_registry.unregister() instead.', RemovedInReviewBoard40Warning)
    if not issubclass(trophy, TrophyType):
        raise TypeError(b'Only TrophyType subclasses can be unregistered')
    try:
...
def get_registered_trophy_types():
    ...
    """
    warnings.warn(b'get_registered_trophy_types() is deprecated. Please iterate through reviewboard.accounts.trophies:trophies_registry instead.', RemovedInReviewBoard40Warning)
    return {trophy.category:trophy for trophy in trophies_registry}