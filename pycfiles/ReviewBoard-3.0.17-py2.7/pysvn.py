...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/scmtools/svn/pysvn.py
...
class Client(base.Client):
    ...
    def ssl_trust_prompt(self, trust_dict):
        if hasattr(self, b'callback_ssl_server_trust_prompt'):
        ...
        try:
            ...
            if self.client.is_url(normpath):
                ...
                if isinstance(path, six.text_type):
                    path = path.encode(b'utf-8', b'ignore')
            ...
            exc = bytes(e).decode(b'utf-8')
            if b'File not found' in exc or b'path not found' in exc:
                raise FileNotFoundError(path, revision, detail=exc)
            elif b'callback_ssl_server_trust_prompt required' in exc:
                raise SCMError(_(b'HTTPS certificate not accepted.  Please ensure that the proper certificate exists in %s for the user that reviewboard is running as.') % os.path.join(self.config_dir, b'auth'))
            else:
    ...
    def _get_file_data(self, normpath, normrev):
        ...
        if has_expanded_svn_keywords(data):
            keywords = self.client.propget(b'svn:keywords', normpath, normrev, recurse=True)
    ...
    def repository_info(self):
        ...
        * URL
        ...
        return {b'uuid': info[0][1].repos_UUID, 
           b'root_url': info[0][1].repos_root_URL, 
           b'url': info[0][1].URL}
    ...
    def accept_ssl_certificate(self, path, on_failure=None):
        ...
        def ssl_server_trust_prompt(trust_dict):
            ...
            if on_failure:
                ...
                del cert[b'failures']
                return (True, trust_dict[b'failures'], True)
    ...
    def get_log(self, path, start=None, end=None, limit=None, discover_changed_paths=False, limit_to_path=False):
        ...
        commits = self.client.log(self.normalize_path(path), limit=limit, revision_start=self._normalize_revision(start), revision_end=self._normalize_revision(end), discover_changed_paths=discover_changed_paths, strict_node_history=limit_to_path)
        for commit in commits:
            commit[b'revision'] = six.text_type(commit[b'revision'].number)
            if b'date' in commit:
                commit[b'date'] = datetime.utcfromtimestamp(commit[b'date'])
    ...
    def list_dir(self, path):
        ...
        * ``created_rev`` - The revision where the file or directory was
        ...
        for dirent, unused in dirents:
            name = dirent[b'path'].split(b'/')[(-1)]
            result[name] = {b'path': dirent[b'path'][repo_path_len:], 
               b'created_rev': six.text_type(dirent[b'created_rev'].number)}
    ...
    def diff(self, revision1, revision2, path=None):
        ...
        if path:
            ...
            path = self.repopath
        tmpdir = mkdtemp(prefix=b'reviewboard-svn.')
        try:
            try:
                diff = self.client.diff(tmpdir, path, revision1=self._normalize_revision(revision1), revision2=self._normalize_revision(revision2), header_encoding=b'UTF-8', diff_options=[
                 b'-u'])
            except Exception as e:
                logging.error(b'Failed to generate diff using pysvn for revisions %s:%s for path %s: %s', revision1, revision2, path, e, exc_info=1)
