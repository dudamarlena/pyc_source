...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/review_reply_screenshot_comment.py
...
from reviewboard.webapi.resources.review_screenshot_comment import ReviewScreenshotCommentResource
...
class ReviewReplyScreenshotCommentResource(BaseScreenshotCommentResource):
    ...
    policy_id = b'review_reply_screenshot_comment'
    model_parent_key = b'review'
    fields = dict({b'reply_to': {b'type': ReviewScreenshotCommentResource, 
                     b'description': b'The comment being replied to.'}}, **BaseScreenshotCommentResource.fields)
    mimetype_list_resource_name = b'review-reply-screenshot-comments'
    mimetype_item_resource_name = b'review-reply-screenshot-comment'

    def get_queryset(self, request, review_id, reply_id, *args, **kwargs):
        q = super(ReviewReplyScreenshotCommentResource, self).get_queryset(request, *args, **kwargs)
    ...
    @webapi_request_fields(required=BaseScreenshotCommentResource.REPLY_REQUIRED_CREATE_FIELDS, optional=BaseScreenshotCommentResource.REPLY_OPTIONAL_CREATE_FIELDS, allow_unknown=True)
    def create(self, request, reply_to_id, *args, **kwargs):
        ...
        try:
            comment = resources.review_screenshot_comment.get_object(request, comment_id=reply_to_id, *args, **kwargs)
        except ObjectDoesNotExist:
            return (
             INVALID_FORM_DATA,
             {b'fields': {b'reply_to_id': [
                                           b'This is not a valid screenshot comment ID']}})

        return self.create_or_update_comment_reply(request=request, comment=comment, reply=reply, comments_m2m=reply.screenshot_comments, default_attrs={b'screenshot': comment.screenshot, 
           b'x': comment.x, 
           b'y': comment.y, 
           b'w': comment.w, 
           b'h': comment.h}, *args, **kwargs)

    @webapi_check_local_site
    @webapi_login_required
    @webapi_response_errors(DOES_NOT_EXIST, NOT_LOGGED_IN, PERMISSION_DENIED)
    @webapi_request_fields(optional=BaseScreenshotCommentResource.REPLY_OPTIONAL_UPDATE_FIELDS, allow_unknown=True)
    def update(self, request, *args, **kwargs):
        ...
        if not resources.review_reply.has_modify_permissions(request, reply):
            return self.get_no_access_error(request)
        return self.update_comment(request=request, review=reply, comment=screenshot_comment, is_reply=True, **kwargs)
...
review_reply_screenshot_comment_resource = ReviewReplyScreenshotCommentResource()