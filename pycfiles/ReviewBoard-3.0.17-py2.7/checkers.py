...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/features/checkers.py
...
class RBFeatureChecker(SiteConfigFeatureChecker):
    ...
    def is_feature_enabled(self, feature_id, **kwargs):
        ...
        """
        local_site = kwargs.get(b'local_site')
        request = kwargs.get(b'request')
        force_check_user_local_sites = kwargs.get(b'force_check_user_local_sites', False)
        local_sites = []
        if local_site:
            ...
            if request is not None:
                try:
                    ...
                    if getattr(request, b'local_site', None):
                        local_sites.append(request.local_site)
                    if request.user.is_authenticated() and (force_check_user_local_sites or not getattr(settings, b'RUNNING_TEST', False)):
                        local_sites.extend(request.user.local_site.all())
                    request._user_local_sites_cache = local_sites

            for local_site in local_sites:
                if local_site.extra_data and local_site.extra_data.get(self.EXTRA_DATA_KEY, {}).get(feature_id):
                    return True

        return super(RBFeatureChecker, self).is_feature_enabled(feature_id, **kwargs)