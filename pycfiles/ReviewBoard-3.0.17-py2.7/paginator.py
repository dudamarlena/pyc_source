...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/hostingsvcs/utils/paginator.py
# Compiled at: 2020-02-11 04:03:56
from __future__ import unicode_literals
from django.utils import six
from django.utils.six.moves.urllib.parse import parse_qs, urlencode, urlsplit, urlunsplit
...
class APIPaginator(BasePaginator):
    ...
    def _fetch_page(self):
        ...
        self.prev_url = page_info.get(b'prev_url')
        self.next_url = page_info.get(b'next_url')
        self.per_page = page_info.get(b'per_page', self.per_page)
        self.page_data = page_info.get(b'data')
        self.page_headers = page_info.get(b'headers')
        self.total_count = page_info.get(b'total_count')
    ...
    def _add_query_params(self, url, new_query_params):
        ...
        new_query_string = urlencode([ (key, value) for key, value in sorted(six.iteritems(query_params), key=lambda i: i[0])
                                     ], doseq=True)
