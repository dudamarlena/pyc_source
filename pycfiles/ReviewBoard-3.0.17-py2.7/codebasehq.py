...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/hostingsvcs/codebasehq.py
...
from reviewboard.hostingsvcs.errors import AuthorizationError, HostingServiceAPIError, RepositoryError
...
class CodebaseHQAuthForm(HostingServiceAuthForm):
    api_key = forms.CharField(label=_(b'API key'), max_length=128, required=True, widget=forms.TextInput(attrs={b'size': b'60'}), help_text=_(b'The API key provided to your Codebase account. This is available in My Profile under API Credentials.'))
    domain = forms.CharField(label=_(b'Codebase domain'), max_length=128, required=True, widget=forms.TextInput(attrs={b'size': b'60'}), help_text=_(b'The subdomain used to access your Codebase account. This is the "<tt>subdomain</tt>" of <tt>subdomain</tt>.codebasehq.com.'))

    def get_credentials(self):
        credentials = super(CodebaseHQAuthForm, self).get_credentials()
        credentials.update({b'domain': self.cleaned_data[b'domain'], 
           b'api_key': self.cleaned_data[b'api_key']})
        return credentials

    class Meta(object):
        help_texts = {b'hosting_account_username': _(b'The username you use to log into Codebase. This should <em>not</em> include the domain name.'), 
           b'hosting_account_password': _(b'The password you use to log into Codebase. This is separate from the API key below.')}


class CodebaseHQForm(HostingServiceForm):
    codebasehq_project_name = forms.CharField(label=_(b'Project name'), max_length=64, required=True, widget=forms.TextInput(attrs={b'size': b'60'}))
    codebasehq_repo_name = forms.CharField(label=_(b'Repository short name'), max_length=128, required=True, widget=forms.TextInput(attrs={b'size': b'60'}), help_text=_(b'The short name of your repository. This can be found by clicking the Settings button on the right-hand side of the repository browser.'))
...
class CodebaseHQClient(HostingServiceClient):
    ...
    """
    API_MIMETYPE = b'application/xml'
    ...
    def api_get_file(self, repository, project_name, repo_name, path, revision):
        ...
        """
        url = b'%s/%s/blob/' % (project_name, repo_name)
        if repository.tool.name == b'Git':
            url += revision
        else:
            if path.startswith(b'/'):
                path = path[1:]
            url += b'%s/%s' % (revision, path)
    ...
    def api_get_public_keys(self, username):
        ...
        """
        return self.api_get(self.build_api_url(b'users/%s/public_keys' % username))
    ...
    def api_get_repository(self, project_name, repo_name):
        ...
        Returns:
            ...
            data returned.
        """
        return self.api_get(self.build_api_url(b'%s/%s' % (project_name, repo_name)))
    ...
    def build_api_url(self, url):
        ...
        """
        return b'https://api3.codebasehq.com/%s' % url
    ...
    def api_get(self, url, raw_content=False):
        ...
        try:
            ...
            api_username = b'%s/%s' % (account_data[b'domain'],
             hosting_service.account.username)
            api_key = decrypt_password(account_data[b'api_key'])
            data, headers = self.http_get(url, username=api_username, password=api_key, headers={b'Accept': self.API_MIMETYPE})
            if raw_content:
                return data
    ...
    def get_xml_text(self, nodes):
        ...
        """
        return (b'').join(node.data for node in nodes if node.nodeType == node.TEXT_NODE)
    ...
    def parse_xml(self, s):
        ...
        return {root.tagName: self._parse_xml_node(root)}
    ...
    def _parse_xml_node(self, node):
        ...
        Returns:
            ...
            processed.
        """
        node_type = node.getAttribute(b'type')
        is_nil = node.getAttribute(b'nil')
        if node_type == b'array':
            result = [ self._parse_xml_node(child) for child in node.childNodes if child.nodeType == child.ELEMENT_NODE
                     ]
        elif is_nil == b'true':
            result = None
        else:
            child_nodes = [ child for child in node.childNodes if child.nodeType == child.ELEMENT_NODE
                          ]
            if child_nodes:
                result = dict([ (child.tagName, self._parse_xml_node(child)) for child in child_nodes
                              ])
            else:
...
class CodebaseHQ(HostingService):
    ...
    supported_scmtools = [
     b'Git', b'Subversion', b'Mercurial']
    repository_fields = {b'Git': {b'path': b'git@codebasehq.com:%(domain)s/%(codebasehq_project_name)s/%(codebasehq_repo_name)s.git'}, 
       b'Subversion': {b'path': b'https://%(domain)s.codebasehq.com/%(codebasehq_project_name)s/%(codebasehq_repo_name)s.svn'}, 
       b'Mercurial': {b'path': b'https://%(domain)s.codebasehq.com/projects/%(codebasehq_project_name)s/repositories/%(codebasehq_repo_name)s/'}}
    bug_tracker_field = b'https://%(domain)s.codebasehq.com/projects/%(codebasehq_project_name)s/tickets/%%s'
    REPO_SCM_TOOL_MAP = {b'git': b'Git', 
       b'svn': b'Subversion', 
       b'hg': b'Mercurial'}
    ...
    def authorize(self, username, password, credentials, *args, **kwargs):
        ...
        """
        self.account.data.update({b'domain': credentials[b'domain'], 
           b'api_key': encrypt_password(credentials[b'api_key']), 
           b'password': encrypt_password(password)})
        try:
            self.client.api_get_public_keys(username)
        except AuthorizationError:
            raise AuthorizationError(ugettext(b'One or more of the credentials provided were not accepted by Codebase.'))
    ...
    def is_authorized(self):
        ...
        """
        return self.account.data.get(b'api_key') is not None and self.account.data.get(b'password') is not None and self.account.data.get(b'domain') is not None
    ...
    def check_repository(self, codebasehq_project_name=None, codebasehq_repo_name=None, tool_name=None, *args, **kwargs):
        ...
        assert tool_name
        try:
            info = self.client.api_get_repository(codebasehq_project_name, codebasehq_repo_name)
        except HostingServiceAPIError as e:
            logging.error(b'Error finding Codebase repository "%s" for project "%s": %s', codebasehq_repo_name, codebasehq_project_name, e)
            raise RepositoryError(ugettext(b'A repository with this name and project was not found.'))

        try:
            scm_type = info[b'repository'][b'scm']
        except KeyError:
            logging.error(b'Missing "scm" field for Codebase HQ repository payload: %r', info)
            raise RepositoryError(ugettext(b'Unable to determine the type of repository from the Codebase API. Please report this.'))

        try:
            expected_tool_name = self.REPO_SCM_TOOL_MAP[scm_type]
        except KeyError:
            logging.error(b'Unexpected "scm" value "%s" for Codebase HQ repository, using payload: %r', scm_type, info)
            raise RepositoryError(ugettext(b'Unable to determine the type of repository from the Codebase API. Please report this.'))

        if expected_tool_name != tool_name:
            raise RepositoryError(ugettext(b"The repository type doesn't match what you selected. Did you mean %s?") % expected_tool_name)
    ...
    def get_file(self, repository, path, revision, *args, **kwargs):
        ...
        """
        try:
            return self.client.api_get_file(repository, repository.extra_data[b'codebasehq_project_name'], repository.extra_data[b'codebasehq_repo_name'], path, revision)
        except HostingServiceAPIError as e:
            if e.http_code == 404:
                raise FileNotFoundError(path, revision)
            else:
                logging.warning(b'Failed to fetch file from Codebase HQ repository %s: %s', repository, e)
                raise
    ...
    def get_file_exists(self, repository, path, revision, *args, **kwargs):
        ...
        """
        try:
            self.client.api_get_file(repository, repository.extra_data[b'codebasehq_project_name'], repository.extra_data[b'codebasehq_repo_name'], path, revision)
            return True
