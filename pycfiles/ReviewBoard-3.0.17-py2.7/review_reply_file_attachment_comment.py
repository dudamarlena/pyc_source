...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/review_reply_file_attachment_comment.py
...
from reviewboard.webapi.resources.review_file_attachment_comment import ReviewFileAttachmentCommentResource
...
class ReviewReplyFileAttachmentCommentResource(BaseFileAttachmentCommentResource):
    ...
    policy_id = b'review_reply_file_attachment_comment'
    model_parent_key = b'review'
    fields = dict({b'reply_to': {b'type': ReviewFileAttachmentCommentResource, 
                     b'description': b'The comment being replied to.'}}, **BaseFileAttachmentCommentResource.fields)
    mimetype_list_resource_name = b'review-reply-file-attachment-comments'
    mimetype_item_resource_name = b'review-reply-file-attachment-comment'

    def get_queryset(self, request, review_id, reply_id, *args, **kwargs):
        q = super(ReviewReplyFileAttachmentCommentResource, self).get_queryset(request, *args, **kwargs)
    ...
    @webapi_request_fields(required=BaseFileAttachmentCommentResource.REPLY_REQUIRED_CREATE_FIELDS, optional=BaseFileAttachmentCommentResource.REPLY_OPTIONAL_CREATE_FIELDS, allow_unknown=True)
    def create(self, request, reply_to_id, *args, **kwargs):
        ...
        try:
            comment = resources.review_file_attachment_comment.get_object(request, comment_id=reply_to_id, *args, **kwargs)
        except ObjectDoesNotExist:
            return (
             INVALID_FORM_DATA,
             {b'fields': {b'reply_to_id': [
                                           b'This is not a valid file comment ID']}})

        return self.create_or_update_comment_reply(request=request, comment=comment, reply=reply, comments_m2m=reply.file_attachment_comments, default_attrs={b'file_attachment': comment.file_attachment}, *args, **kwargs)

    @webapi_check_local_site
    @webapi_login_required
    @webapi_response_errors(DOES_NOT_EXIST, NOT_LOGGED_IN, PERMISSION_DENIED)
    @webapi_request_fields(optional=BaseFileAttachmentCommentResource.REPLY_OPTIONAL_UPDATE_FIELDS, allow_unknown=True)
    def update(self, request, *args, **kwargs):
        ...
        return self.update_comment(request=request, review=reply, comment=file_comment, is_reply=True, **kwargs)
...
review_reply_file_attachment_comment_resource = ReviewReplyFileAttachmentCommentResource()