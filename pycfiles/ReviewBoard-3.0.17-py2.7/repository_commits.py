...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/resources/repository_commits.py
...
class RepositoryCommitsResource(WebAPIResource):
    ...
    added_in = b'2.0'
    name = b'commits'
    policy_id = b'repository_commits'
    singleton = True
    allowed_methods = ('GET', )
    mimetype_item_resource_name = b'repository-commits'
    fields = {b'id': {b'type': six.text_type, 
       ...
       b'author_name': {b'type': six.text_type, 
                        b'description': b'The real name or username of the author who made the commit.'}, 
       b'date': {b'type': six.text_type, 
                 b'description': b'The timestamp of the commit. This will be in ISO8601 format.'}, 
       b'message': {b'type': six.text_type, 
                    b'description': b'The commit message, if any.'}, 
       b'parent': {b'type': six.text_type, 
                   b'description': b'The revision of the parent commit. This may be an empty string if this is the first revision in the commit history for a repository or branch.'}}
    ...
    @webapi_request_fields(optional={b'branch': {b'type': six.text_type, 
                   b'description': b"The ID of the branch to limit the commits to, as provided by the 'id' field of the repository branches API.", 
                   b'added_in': b'2.5'}, 
       b'start': {b'type': six.text_type, 
                  b'description': b'A commit ID to start listing from.'}})
    def get(self, request, branch=None, start=None, *args, **kwargs):
        ...
        commit_ids = []
        for commit in items:
            commits.append({b'author_name': commit.author_name, 
               b'id': commit.id, 
               b'date': commit.date, 
               b'message': commit.message, 
               b'parent': commit.parent})
            commit_ids.append(commit.id)

        by_commit_id = {}
        for obj in ReviewRequest.objects.filter(repository=repository, commit_id__in=commit_ids):
            by_commit_id[obj.commit_id] = obj

        for commit in commits:
            try:
                review_request = by_commit_id[commit[b'id']]
                commit[b'review_request_url'] = review_request.get_absolute_url()
            except KeyError:
                commit[b'review_request_url'] = b''
