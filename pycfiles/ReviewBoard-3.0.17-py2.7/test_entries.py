...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/reviews/tests/test_entries.py
...
from reviewboard.reviews.detail import BaseReviewRequestPageEntry, ChangeEntry, InitialStatusUpdatesEntry, ReviewEntry, ReviewRequestPageData, StatusUpdatesEntryMixin
from reviewboard.reviews.models import BaseComment, GeneralComment, StatusUpdate
...
class BaseReviewRequestPageEntryTests(SpyAgency, TestCase):
    ...
    def setUp(self):
        ...
        self.data = ReviewRequestPageData(review_request=self.review_request, request=self.request)
    ...
    def test_init_with_no_updated_timestamp(self):
        ...
        """
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
        self.assertEqual(entry.updated_timestamp, datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))

    def test_render_to_string(self):
        """Testing BaseReviewRequestPageEntry.render_to_string"""
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=None)
        entry.template_name = b'reviews/entries/base.html'
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': timezone.now()}))
        self.assertNotEqual(html, b'')
        return
    ...
    def test_render_to_string_with_entry_pos_main(self):
        ...
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': timezone.now()}))
        self.assertIn(b'<div class="box-statuses">', html)
        return
    ...
    def test_render_to_string_with_entry_pos_initial(self):
        ...
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': timezone.now()}))
        self.assertNotIn(b'<div class="box-statuses">', html)
        return
    ...
    def test_render_to_string_with_new_entry(self):
        ...
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
        entry.template_name = b'reviews/entries/base.html'
        self.request.user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc)}))
        self.assertIn(b'class="review-request-page-entry new-review-request-page-entry', html)
    ...
    def test_render_to_string_without_new_entry(self):
        ...
        entry.template_name = b'reviews/entries/base.html'
        self.request.user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': datetime(2017, 9, 7, 18, 0, 0, tzinfo=utc)}))
        self.assertNotEqual(html, b'')
        self.assertNotIn(b'class="review-request-page-entry new-review-request-page-entry"', html)
    ...
    def test_render_to_string_with_exception(self):
        ...
        """
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=None)
        entry.template_name = b'reviews/entries/NOT_FOUND.html'
        self.spy_on(logging.exception)
        html = entry.render_to_string(self.request, RequestContext(self.request, {b'last_visited': timezone.now()}))
        self.assertEqual(html, b'')
        self.assertTrue(logging.exception.spy.called)
        self.assertEqual(logging.exception.spy.calls[0].args[0], b'Error rendering template for %s (ID=%s): %s')
        return

    def test_is_entry_new_with_timestamp(self):
        """Testing BaseReviewRequestPageEntry.is_entry_new with timestamp"""
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=datetime(2017, 9, 7, 15, 36, 0, tzinfo=utc))
        user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        self.assertTrue(entry.is_entry_new(last_visited=datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc), user=user))
        self.assertFalse(entry.is_entry_new(last_visited=datetime(2017, 9, 7, 16, 0, 0, tzinfo=utc), user=user))
        self.assertFalse(entry.is_entry_new(last_visited=datetime(2017, 9, 7, 15, 36, 0, tzinfo=utc), user=user))

    def test_is_entry_new_without_timestamp(self):
        """Testing BaseReviewRequestPageEntry.is_entry_new without timestamp
        """
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=None)
        self.assertFalse(entry.is_entry_new(last_visited=datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc), user=User.objects.create_user(username=b'test-user', email=b'user@example.com')))
        return
    ...
    def test_collapsed_with_older_than_last_visited(self):
        ...
        last visited
        """
        self.data.latest_changedesc_timestamp = self.review_request.time_added + timedelta(days=5)
        self.data.last_visited = datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc)
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=self.data.last_visited - timedelta(days=2), updated_timestamp=self.data.last_visited - timedelta(days=1))
    ...
    def test_collapsed_with_newer_than_last_visited(self):
        ...
        """
        self.data.last_visited = datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc)
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=self.data.last_visited, updated_timestamp=self.data.last_visited + timedelta(days=1))
    ...
    def test_collapsed_without_last_visited(self):
        ...
        timestamp
        """
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=datetime(2017, 9, 6, 10, 0, 0, tzinfo=utc), updated_timestamp=datetime(2017, 9, 7, 10, 0, 0, tzinfo=utc))
        self.assertFalse(entry.collapsed)
    ...
    def test_collapsed_with_older_than_changedesc(self):
        ...
        """
        self.data.latest_changedesc_timestamp = self.review_request.time_added + timedelta(days=5)
        self.data.last_visited = self.review_request.time_added + timedelta(days=10)
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=self.data.latest_changedesc_timestamp - timedelta(days=2), updated_timestamp=self.data.latest_changedesc_timestamp - timedelta(days=1))
        self.assertTrue(entry.collapsed)
    ...
    def test_collapsed_with_newer_than_changedesc(self):
        ...
        self.data.last_visited = self.review_request.time_added + timedelta(days=10)
        entry = BaseReviewRequestPageEntry(data=self.data, entry_id=b'test', added_timestamp=self.data.latest_changedesc_timestamp, updated_timestamp=self.data.latest_changedesc_timestamp + timedelta(days=1))
        self.assertFalse(entry.collapsed)
...
class StatusUpdatesEntryMixinTests(TestCase):
    ...
    def test_add_update_with_done_failure(self):
        ...
        self.assertEqual(status_update.header_class, b'status-update-state-failure')
    ...
    def test_add_update_with_pending(self):
        ...
        entry.add_update(status_update)
        self.assertEqual(entry.status_updates, [status_update])
        self.assertEqual(status_update.header_class, b'status-update-state-pending')
    ...
    def test_add_update_with_done_success(self):
        ...
        entry.add_update(status_update)
        self.assertEqual(entry.status_updates, [status_update])
        self.assertEqual(status_update.header_class, b'status-update-state-success')

    def test_add_update_html_rendering(self):
        """Testing StatusUpdatesEntryMixin.add_update HTML rendering"""
        status_update = StatusUpdate(state=StatusUpdate.DONE_SUCCESS, description=b'My description.', summary=b'My summary.')
        entry = StatusUpdatesEntryMixin()
        entry.add_update(status_update)
        self.assertHTMLEqual(status_update.summary_html, b'<div class="status-update-summary-entry status-update-state-success">\n <span class="summary">My summary.</span>\n My description.\n</div>')

    def test_add_update_html_rendering_with_url(self):
        """Testing StatusUpdatesEntryMixin.add_update HTML rendering with URL
        """
        status_update = StatusUpdate(state=StatusUpdate.DONE_SUCCESS, description=b'My description.', summary=b'My summary.', url=b'https://example.com/')
        entry = StatusUpdatesEntryMixin()
        entry.add_update(status_update)
    ...
    def test_add_update_html_rendering_with_url_and_text(self):
        ...
        and URL text
        """
        status_update = StatusUpdate(state=StatusUpdate.DONE_SUCCESS, description=b'My description.', summary=b'My summary.', url=b'https://example.com/', url_text=b'My URL')
        entry = StatusUpdatesEntryMixin()
        entry.add_update(status_update)
        self.assertHTMLEqual(status_update.summary_html, b'<div class="status-update-summary-entry status-update-state-success">\n <span class="summary">My summary.</span>\n My description.\n <a href="https://example.com/">My URL</a></div>')

    def test_add_update_html_rendering_with_timeout(self):
        """Testing StatusUpdatesEntryMixin.add_update HTML rendering with
        timeout
        """
        status_update = StatusUpdate(state=StatusUpdate.TIMEOUT, description=b'My description.', summary=b'My summary.')
        entry = StatusUpdatesEntryMixin()
        entry.add_update(status_update)
        self.assertHTMLEqual(status_update.summary_html, b'<div class="status-update-summary-entry status-update-state-failure">\n <span class="summary">My summary.</span>\n timed out.\n</div>')
    ...
    def test_add_comment(self):
        ...
        status_update = self.create_status_update(review_request=review_request, review=review)
        entry = StatusUpdatesEntryMixin()
        entry.add_update(status_update)
        entry.add_comment(b'general_comments', comment)
        self.assertEqual(status_update.comments[b'general_comments'], [comment])
    ...
    def test_finalize_with_all_states(self):
        ...
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 failed, 2 succeeded, 3 pending, 4 failed with error, 5 timed out')
    ...
    def test_finalize_with_done_failure(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.DONE_FAILURE))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 failed')
        self.assertEqual(entry.state_summary_class, b'status-update-state-failure')
    ...
    def test_finalize_with_error(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.ERROR))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 failed with error')
        self.assertEqual(entry.state_summary_class, b'status-update-state-failure')
    ...
    def test_finalize_with_timeout(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.TIMEOUT))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 timed out')
        self.assertEqual(entry.state_summary_class, b'status-update-state-failure')
    ...
    def test_finalize_with_pending(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.PENDING))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 pending')
        self.assertEqual(entry.state_summary_class, b'status-update-state-pending')
    ...
    def test_finalize_with_done_success(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.DONE_SUCCESS))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 succeeded')
        self.assertEqual(entry.state_summary_class, b'status-update-state-success')
    ...
    def test_finalize_with_failures_take_precedence(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.DONE_SUCCESS))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 failed, 1 succeeded, 1 pending')
        self.assertEqual(entry.state_summary_class, b'status-update-state-failure')
    ...
    def test_finalize_with_pending_take_precedence(self):
        ...
        entry.add_update(StatusUpdate(state=StatusUpdate.DONE_SUCCESS))
        entry.finalize()
        self.assertEqual(entry.state_summary, b'1 succeeded, 1 pending')
        self.assertEqual(entry.state_summary_class, b'status-update-state-pending')
    ...
    def test_populate_status_updates(self):
        ...
        comment = self.create_general_comment(review)
        comment._type = b'general_comments'
        comment.review_obj = review
        status_updates = [
         StatusUpdate(state=StatusUpdate.PENDING),
         StatusUpdate(state=StatusUpdate.DONE_FAILURE, review=review)]
        request = RequestFactory().get(b'/r/1/')
        request.user = AnonymousUser()
        data = ReviewRequestPageData(review_request=review_request, request=request)
        ...
        self.assertEqual(status_update.comments, {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': []})
...
class InitialStatusUpdatesEntryTests(TestCase):
    ...
    def setUp(self):
        ...
        self.request = RequestFactory().get(b'/r/1/')
        self.request.user = AnonymousUser()
        self.review_request = self.create_review_request(time_added=datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
        self.review = self.create_review(self.review_request, public=True, timestamp=datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc))
        self.general_comment = self.create_general_comment(self.review, issue_opened=False)
        self.status_update = self.create_status_update(self.review_request, review=self.review, timestamp=datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc), state=StatusUpdate.DONE_FAILURE)
        self.data = ReviewRequestPageData(review_request=self.review_request, request=self.request, last_visited=self.review_request.time_added + timedelta(days=10))
    ...
    def test_added_timestamp(self):
        ...
        self.assertEqual(entry.added_timestamp, datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
    ...
    def test_updated_timestamp(self):
        ...
        self.data.query_data_post_etag()
        entry = InitialStatusUpdatesEntry(data=self.data)
        self.assertEqual(entry.updated_timestamp, datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc))
    ...
    def test_build_entries(self):
        ...
        self.assertEqual(entry.status_updates_by_review, {self.review.pk: self.status_update})
        self.assertEqual(entry.status_updates[0].comments, {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': [
    ...
    def test_is_entry_new_with_timestamp(self):
        ...
        user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        entry = InitialStatusUpdatesEntry(data=self.data)
        self.assertFalse(entry.is_entry_new(last_visited=self.review_request.last_updated - timedelta(days=1), user=user))
    ...
    def test_collapsed_with_status_update_timestamp_gt_last_visited(self):
        ...
        """
        StatusUpdate.objects.filter(pk=self.status_update.pk).update(timestamp=self.data.last_visited + timedelta(days=1))
    ...
    def test_collapsed_with_status_updates_and_draft_body_top_replies(self):
        ...
        self.request.user = self.review_request.submitter
        self.assertEqual(self.status_update.state, StatusUpdate.DONE_FAILURE)
        self.create_reply(self.review, user=self.request.user, body_top_reply_to=self.review)
        self.review_request.changedescs.create(public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
    ...
    def test_collapsed_with_status_updates_and_draft_body_bottom_replies(self):
        ...
        self.request.user = self.review_request.submitter
        self.assertEqual(self.status_update.state, StatusUpdate.DONE_FAILURE)
        self.create_reply(self.review, user=self.request.user, body_bottom_reply_to=self.review)
        self.review_request.changedescs.create(public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
...
class ReviewEntryTests(TestCase):
    ...
    def setUp(self):
        ...
        self.request.user = AnonymousUser()
        self.review_request = self.create_review_request()
        self.review = self.create_review(self.review_request, id=123, public=True, timestamp=datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
        self.changedesc = self.review_request.changedescs.create(timestamp=self.review.timestamp + timedelta(days=10), public=True)
        self.data = ReviewRequestPageData(review_request=self.review_request, request=self.request, last_visited=self.changedesc.timestamp)

    def test_added_timestamp(self):
        """Testing ReviewEntry.added_timestamp"""
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertEqual(entry.added_timestamp, datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
    ...
    def test_updated_timestamp_with_replies(self):
        ...
        self.create_reply(self.review, timestamp=datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc), publish=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertEqual(entry.updated_timestamp, datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc))
    ...
    def test_get_dom_element_id(self):
        ...
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertEqual(entry.get_dom_element_id(), b'review123')

    def test_collapsed_with_open_issues(self):
        """Testing ReviewEntry.collapsed with open issues"""
        self.create_general_comment(self.review, issue_opened=True, issue_status=BaseComment.OPEN)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertFalse(entry.collapsed)
    ...
    def test_collapsed_with_open_issues_verifying_resolved(self):
        ...
        Resolved
        """
        self.create_general_comment(self.review, issue_opened=True, issue_status=BaseComment.VERIFYING_RESOLVED)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertFalse(entry.collapsed)
    ...
    def test_collapsed_with_open_issues_verifying_dropped(self):
        ...
        Dropped
        """
        self.create_general_comment(self.review, issue_opened=True, issue_status=BaseComment.VERIFYING_DROPPED)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertFalse(entry.collapsed)
    ...
    def test_collapsed_with_dropped_issues(self):
        ...
        self.create_general_comment(self.review, issue_opened=True, issue_status=BaseComment.DROPPED)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertTrue(entry.collapsed)
    ...
    def test_collapsed_with_resolved_issues(self):
        ...
        self.create_general_comment(self.review, issue_opened=True, issue_status=BaseComment.RESOLVED)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertTrue(entry.collapsed)
    ...
    def test_collapsed_with_reply_older_than_last_visited(self):
        ...
        reply = self.create_reply(self.review, publish=True, timestamp=self.review.timestamp + timedelta(days=2))
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
    ...
    def test_get_js_model_data(self):
        ...
        self.review.publish()
        entry = ReviewEntry(data=self.data, review=self.review)
        self.assertEqual(entry.get_js_model_data(), {b'reviewData': {b'id': self.review.pk, 
                           b'bodyTop': b'Test Body Top', 
                           b'bodyBottom': b'Test Body Bottom', 
                           b'public': True, 
                           b'shipIt': True}})
    ...
    def test_get_js_model_data_with_diff_comments(self):
        ...
        self.data.query_data_post_etag()
        entry = ReviewEntry(data=self.data, review=self.review)
        entry.add_comment(b'diff_comments', comment1)
        entry.add_comment(b'diff_comments', comment2)
        self.assertEqual(entry.get_js_model_data(), {b'reviewData': {b'id': self.review.pk, 
                           b'bodyTop': b'Test Body Top', 
                           b'bodyBottom': b'Test Body Bottom', 
                           b'public': True, 
    ...
    def test_add_comment_with_no_open_issues(self):
        ...
        entry.add_comment(b'general_comments', GeneralComment())
        self.assertFalse(entry.has_issues)
        self.assertEqual(entry.issue_open_count, 0)
    ...
    def test_add_comment_with_open_issues(self):
        ...
        self.assertFalse(entry.has_issues)
        self.assertEqual(entry.issue_open_count, 0)
        entry.add_comment(b'general_comments', GeneralComment(issue_opened=True, issue_status=GeneralComment.OPEN))
    ...
    def test_build_entries(self):
        ...
        review1 = self.create_review(self.review_request, timestamp=self.review.timestamp - timedelta(days=2), public=True)
        review2 = self.review
        comment = self.create_general_comment(review1)
        self.create_review(self.review_request, timestamp=self.review.timestamp - timedelta(days=1), public=False)
        self.create_reply(review1)
        status_update_review = self.create_review(self.review_request, public=True)
        self.create_general_comment(status_update_review)
        self.create_status_update(self.review_request, review=status_update_review, state=StatusUpdate.DONE_FAILURE)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        ...
        self.assertEqual(entry.comments, {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': []})
        entry = entries[1]
...
class ChangeEntryTests(TestCase):
    ...
    def setUp(self):
        ...
        self.request.user = AnonymousUser()
        self.review_request = self.create_review_request()
        self.changedesc = ChangeDescription.objects.create(id=123, public=True, timestamp=datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
    ...
    def test_added_timestamp(self):
        ...
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertEqual(entry.added_timestamp, datetime(2017, 9, 7, 17, 0, 0, tzinfo=utc))
    ...
    def test_updated_timestamp_with_status_update(self):
        ...
        self.create_status_update(self.review_request, change_description=self.changedesc, timestamp=datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc))
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertEqual(entry.updated_timestamp, datetime(2017, 9, 14, 15, 40, 0, tzinfo=utc))
    ...
    def test_get_dom_element_id(self):
        ...
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertEqual(entry.get_dom_element_id(), b'changedesc123')

    def test_collapsed_with_older_than_latest_changedesc(self):
        """Testing ChangeEntry.collapsed with older than latest Change
        Description
        """
        self.review_request.changedescs.create(timestamp=self.changedesc.timestamp + timedelta(days=1), public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertTrue(entry.collapsed)
    ...
    def test_collapsed_with_latest_changedesc(self):
        ...
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        self.assertEqual(self.changedesc.timestamp, self.data.latest_changedesc_timestamp)
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertFalse(entry.collapsed)

    def test_collapsed_with_status_updates_and_no_reviews(self):
        """Testing ChangeEntry.collapsed with status updates and no reviews"""
        self.create_status_update(self.review_request, change_description=self.changedesc, state=StatusUpdate.DONE_SUCCESS)
        self.review_request.changedescs.create(timestamp=self.changedesc.timestamp + timedelta(days=1), public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
    ...
    def test_collapsed_with_status_updates_and_draft_comment_replies(self):
        ...
        comment = self.create_general_comment(review)
        self.create_status_update(self.review_request, review=review, change_description=self.changedesc, state=StatusUpdate.DONE_FAILURE)
    ...
    def test_collapsed_with_pending_status_updates(self):
        ...
        self.request.user = self.review_request.submitter
        self.create_status_update(self.review_request, change_description=self.changedesc, state=StatusUpdate.PENDING)
        self.review_request.changedescs.create(timestamp=self.changedesc.timestamp + timedelta(days=1), public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
    ...
    def test_collapsed_with_status_update_timestamp_gt_last_visited(self):
        ...
        status_update = self.create_status_update(self.review_request, change_description=self.changedesc, state=StatusUpdate.DONE_SUCCESS, timestamp=self.data.last_visited + timedelta(days=1))
    ...
    def test_collapsed_with_status_updates_and_draft_body_top_replies(self):
        ...
        review = self.create_review(self.review_request, publish=True)
        self.create_status_update(self.review_request, review=review, change_description=self.changedesc, state=StatusUpdate.DONE_FAILURE)
        self.create_reply(review, user=self.request.user, body_top_reply_to=review)
        self.review_request.changedescs.create(timestamp=self.changedesc.timestamp + timedelta(days=1), public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
    ...
    def test_collapsed_with_status_updates_and_draft_body_bottom_replies(self):
        ...
        review = self.create_review(self.review_request, publish=True)
        self.create_status_update(self.review_request, review=review, change_description=self.changedesc, state=StatusUpdate.DONE_FAILURE)
        self.create_reply(review, user=self.request.user, body_bottom_reply_to=review)
        self.review_request.changedescs.create(timestamp=self.changedesc.timestamp + timedelta(days=1), public=True)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
    ...
    def test_get_js_model_data(self):
        ...
        """
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        self.assertEqual(entry.get_js_model_data(), {b'pendingStatusUpdates': False})
    ...
    def test_get_js_model_data_with_status_updates(self):
        ...
        diffset = self.create_diffset(self.review_request)
        filediff = self.create_filediff(diffset)
        review = self.create_review(self.review_request, body_top=b'Body top', body_bottom=b'Body bottom', ship_it=True)
        ...
        status_update = self.create_status_update(self.review_request, review=review, change_description=self.changedesc)
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        entry.add_update(status_update)
        entry.add_comment(b'diff_comments', comment1)
        entry.add_comment(b'diff_comments', comment2)
        self.assertEqual(entry.get_js_model_data(), {b'reviewsData': [
                          {b'id': review.pk, 
                             b'bodyTop': b'Body top', 
                             b'bodyBottom': b'Body bottom', 
                             b'public': True, 
                             b'shipIt': True}], 
           b'diffCommentsData': [
                               (
                               ...
                               (
                                six.text_type(comment2.pk), six.text_type(filediff.pk))], 
           b'pendingStatusUpdates': False})

    def test_build_entries(self):
        """Testing ChangeEntry.build_entries"""
        changedesc1 = self.changedesc
        changedesc2 = self.review_request.changedescs.create(timestamp=changedesc1.timestamp + timedelta(days=1), public=True)
        review = self.create_review(self.review_request, public=True)
        comment = self.create_general_comment(review)
        status_update = self.create_status_update(self.review_request, review=review, change_description=changedesc2)
        self.data.query_data_pre_etag()
        self.data.query_data_post_etag()
        ...
        self.assertFalse(entry.collapsed)
        self.assertEqual(entry.status_updates, [status_update])
        self.assertEqual(entry.status_updates_by_review, {review.pk: status_update})
        self.assertEqual(entry.status_updates[0].comments, {b'diff_comments': [], b'screenshot_comments': [], b'file_attachment_comments': [], b'general_comments': [
                               comment]})
        entry = entries[1]
    ...
    def test_is_entry_new_with_timestamp(self):
        ...
        entry = ChangeEntry(data=self.data, changedesc=self.changedesc)
        user = User.objects.create_user(username=b'test-user', email=b'user@example.com')
        self.assertTrue(entry.is_entry_new(last_visited=self.changedesc.timestamp - timedelta(days=1), user=user))
        self.assertFalse(entry.is_entry_new(last_visited=self.changedesc.timestamp, user=user))
        self.assertFalse(entry.is_entry_new(last_visited=self.changedesc.timestamp + timedelta(days=1), user=user))