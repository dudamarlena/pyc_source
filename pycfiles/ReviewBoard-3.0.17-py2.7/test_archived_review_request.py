...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/reviewboard/webapi/tests/test_archived_review_request.py
...
from reviewboard.webapi.tests.urls import get_archived_review_request_item_url, get_archived_review_request_list_url
...
class ResourceListTests(BaseWebAPITestCase):
    ...
    sample_api_url = b'users/<username>/archived-review-requests/'
    resource = resources.archived_review_request

    def compare_item(self, item_rsp, obj):
        self.assertEqual(item_rsp[b'id'], obj.display_id)
    ...
    def setup_basic_post_test(self, user, with_local_site, local_site_name, post_valid_data):
        ...
        if post_valid_data:
            ...
            post_data = {}
        return (get_archived_review_request_list_url(user.username, local_site_name),
    ...
    def check_post_result(self, user, rsp, review_request):
        ...
        self.compare_item(rsp[b'archived_review_request'], review_request)
    ...
    def test_post_with_does_not_exist_error(self):
        ...
        """
        rsp = self.api_post(get_archived_review_request_list_url(self.user.username), {b'object_id': 999}, expected_status=404)
    ...
    def test_post_with_site_does_not_exist_error(self):
        ...
        rsp = self.api_post(get_archived_review_request_list_url(user.username, self.local_site_name), {b'object_id': 10}, expected_status=404)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], DOES_NOT_EXIST.code)
...
class ResourceItemTests(BaseWebAPITestCase):
    ...
    sample_api_url = b'users/<username>/archived-review-requests/<id>/'
    resource = resources.archived_review_request
    ...
    def setup_basic_delete_test(self, user, with_local_site, local_site_name):
        review_request = self.create_review_request(with_local_site=with_local_site, publish=True)
        self.create_visit(review_request, visibility=ReviewRequestVisit.ARCHIVED, user=user)
        return (
         get_archived_review_request_item_url(user.username, review_request.display_id, local_site_name),
         [
          review_request])
    ...
    def test_delete_with_does_not_exist_error(self):
        ...
        """
        rsp = self.api_delete(get_archived_review_request_item_url(self.user.username, 999), expected_status=404)
        self.assertEqual(rsp[b'stat'], b'fail')
        self.assertEqual(rsp[b'err'][b'code'], DOES_NOT_EXIST.code)