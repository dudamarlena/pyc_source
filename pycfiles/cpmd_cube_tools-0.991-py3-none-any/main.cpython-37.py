...
# Compiled at: 2020-04-25 09:53:40
...
def main():
    ...
    parser.add_argument('-dvdd', '--dodvdd', help='yes/no (vdd charges from electrons only)')
        ...
        if args.dovdd != None:
            if args.geom != None:
                if args.dovdd.lower() == 'yes':
                    ...
                    if args.molecule == None:
                        args.molecule = 'mol'
                    elif args.molecule.split(',')[0] in ('mm', 'mol'):
        ...
        if args.dodvdd != None:
            if args.geom != None:
                if args.dodvdd.lower() == 'yes':
                    ...
                    if args.molecule == None:
                        args.molecule = 'mol'
                    if args.molecule.split(',')[0] in ('mm', 'mol'):
                        if args.inpfile == None:
                            print('Please supply also the inpfile of cpmd/mimic calculation! Otherwise, you are allowed to specify the atom list by QM orders only!\n e.g. -mol qm,id0,id1,...')
                            exit()
                        if args.Files:
