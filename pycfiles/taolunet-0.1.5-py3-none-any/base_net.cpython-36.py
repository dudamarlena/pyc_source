...
# [GCC 8.4.0]
# Embedded file name: /home/pegasus/PycharmProjects/taolunet/taolu/vision/base_nets/base_net.py
# Compiled at: 2018-08-14 21:56:33
...
import taolu.vision.utils.transformers as transformers, math, inspect, json, os
from taolu.vision.vision_units.linear import Linear
from taolu.vision.vision_units.l2_norm import L2Norm
activation_map = {'relu':torch.nn.ReLU(inplace=True), 
 'softmax':torch.nn.Softmax()}
loss_map = {'cross_entropy':CrossEntropyLoss, 
 'mse':MSELoss, 
 'yolo_loss':YoloLoss}
transformer_map = {'cross_entropy_transformer':transformers.cross_entropy_transformer.CrossEntropyTransformer, 
...
def parse_optim(optim_cfg, parameters):
    ...
    optim = list(filter(lambda x: type(x) is tuple and inspect.isclass(x[1]) and x[1].__name__.lower() == optim_type, inspect.getmembers(torch.optim)))
    if len(optim) > 0:
...
def parse_backbone(backbone_cfg, input_shape):
    ...
    remove_indexes[0] = len(backbone_origin_layers_cfg) + remove_indexes[0] if remove_indexes[0] < 0 else remove_indexes[0]
    remove_indexes[1] = len(backbone_origin_layers_cfg) + remove_indexes[1] if remove_indexes[1] < 0 else remove_indexes[1]
