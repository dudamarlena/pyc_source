...
class VersionImporter(object):
    __doc__ = 'Import modules that have the same name, but different versions.'
    ...
    def init(self, target_dir=None):
        ...
        if target_dir is None:
            ...
            if os.path.isfile(target_dir):
                ...
                os.path.exists(self._target_dir) or os.makedirs(self._target_dir)
            return self
...
Parse error at or near `POP_EXCEPT' instruction at offset 254
    ...
    def zip_import(self, name, version, path):
        ...
        for p in any((p == name for p in os.listdir(import_path))) or os.listdir(import_path):
