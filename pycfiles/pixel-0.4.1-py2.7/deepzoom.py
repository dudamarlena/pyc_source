...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pixel/deepzoom.py
# Compiled at: 2016-09-28 10:30:55
import math, optparse, os, PIL.Image, shutil, numpy as np
try:
    import cStringIO
    StringIO = cStringIO
except ImportError:
...
class DeepZoomImageDescriptor(object):
    ...
    def __init__(self, width=None, height=None, tile_size=254, tile_overlap=1, tile_format='jpg'):
        self.width = width
        self.height = height
...
class DeepZoomCollection(object):
    ...
    def __init__(self, filename, image_quality=0.8, max_level=7, tile_size=256, tile_format='jpg', items=[]):
    ...
    def from_file(self, filename):
        ...
        items = [ DeepZoomCollectionItem.from_xml(item) for item in doc.getElementsByTagName('I')
                ]
        collection = DeepZoomCollection(filename, image_quality=image_quality, max_level=max_level, tile_size=tile_size, tile_format=tile_format, items=items)
        return collection
    ...
    def append(self, source):
        ...
        item = DeepZoomCollectionItem(source, descriptor.width, descriptor.height, id=self.next_item_id)
...
class ImageCreator(object):
    ...
    def __init__(self, tile_size=254, tile_overlap=1, tile_format='jpg', image_quality=0.8, resize_filter=None, copy_metadata=False):
    ...
    def create(self, image, destination):
        """Creates Deep Zoom image from source file and saves it to destination."""
        if image.dtype == np.uint32:
            self.image = PIL.Image.fromarray(image, mode='RGBA')
        else:
            self.image = PIL.Image.fromarray(image)
        width, height = self.image.size
        self.descriptor = DeepZoomImageDescriptor(width=width, height=height, tile_size=self.tile_size, tile_overlap=self.tile_overlap, tile_format=self.tile_format)
...
class CollectionCreator(object):
    ...
    def __init__(self, image_quality=0.8, tile_size=256, max_level=7, tile_format='jpg', copy_metadata=False):
        self.image_quality = image_quality
        self.tile_size = tile_size
        self.max_level = max_level
        self.tile_format = tile_format
    ...
    def create(self, images, destination):
        ...
        collection = DeepZoomCollection(destination, image_quality=self.image_quality, max_level=self.max_level, tile_size=self.tile_size, tile_format=self.tile_format)
        for image in images:
...
def main():
    ...
    parser.add_option('-d', '--destination', dest='destination', help='Set the destination of the output.')
    parser.add_option('-s', '--tile_size', dest='tile_size', type='int', default=254, help='Size of the tiles. Default: 254')
    parser.add_option('-f', '--tile_format', dest='tile_format', default=DEFAULT_IMAGE_FORMAT, help='Image format of the tiles (jpg or png). Default: jpg')
    parser.add_option('-o', '--tile_overlap', dest='tile_overlap', type='int', default=1, help='Overlap of the tiles in pixels (0-10). Default: 1')
    parser.add_option('-q', '--image_quality', dest='image_quality', type='float', default=0.8, help='Quality of the image output (0-1). Default: 0.8')
    parser.add_option('-r', '--resize_filter', dest='resize_filter', default=DEFAULT_RESIZE_FILTER, help='Type of filter for resizing (bicubic, nearest, bilinear, antialias (best). Default: antialias')
    ...
    creator = ImageCreator(tile_size=options.tile_size, tile_format=options.tile_format, image_quality=options.image_quality, resize_filter=options.resize_filter)
