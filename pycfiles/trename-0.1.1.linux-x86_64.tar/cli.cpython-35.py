...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.5/dist-packages/trename/cli.py
# Compiled at: 2017-05-25 05:45:46
...
def main():
    ...
    try:
        ...
        for filename in args.files:
            ...
            if args.dry_run:
                print('{filename} will be renamed to {to_filename}'.format(filename=filename, to_filename=to_filename))
            else:
                rename(filename, to_filename)
                if args.verbose:
                    print('{filename} renamed to {to_filename}'.format(filename=filename, to_filename=to_filename))

    except trename.IdentifiersNotFound as ex:
        print('Invalid pattern: identifiers not found in from_tpl: {}'.format(', '.join(ex.identifiers)))
