...
# [GCC 8.4.0]
# Embedded file name: /Users/revisor/Documents/Snapper/snapper/worker.py
# Compiled at: 2019-09-09 10:23:25
# Size of source mod 2**32: 4988 bytes
import logging, os, requests, shutil, sys
...
def host_worker(host_queue, file_queue, timeout, user_agent, outpath, phantomjs_binary):
    ...
    while not host_queue.empty():
        ...
        if not host.startswith('http://'):
            if not host.startswith('https://'):
                ...
                filename_http = Path(outpath) / 'images' / (str(uuid4()) + '_http.png')
                filename_https = Path(outpath) / 'images' / (str(uuid4()) + '_https.png')
                logging.debug('Fetching %s', host_http)
                if host_reachable(host_http, timeout) and save_image(host_http, str(filename_http), driver):
                ...
                if host_reachable(host_https, timeout) and save_image(host_https, str(filename_https), driver):
...
def capture_snaps(urls, outpath, timeout, num_workers, user_agent, result, phantomjs_binary, task):
    ...
    shutil.copyfile(css_template_path / 'materialize.min.css', css_output_path / 'materialize.min.css')
    shutil.copyfile(js_template_path / 'jquery.min.js', js_output_path / 'jquery.min.js')
    shutil.copyfile(js_template_path / 'materialize.min.js', js_output_path / 'materialize.min.js')
    host_queue = Queue()
    file_queue = Queue()
    workers = []
    for host in urls:
