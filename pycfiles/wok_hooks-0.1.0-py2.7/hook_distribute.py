...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/wok_hooks/hook_distribute.py
# Compiled at: 2014-10-09 05:49:34
import logging, os
...
class Configuration(_Configuration):
    ...
    def __init__(self, path, **kwargs):
        ...
        if 'type' not in self or self['type'] not in ALLOWED_BACKEND_TYPES:
...
class FTP(FileBackend):
    ...
    DEFAULT_CONFIG = {'ftp_host': 'localhost', 
       'ftp_user': 'anonymous', 
       'ftp_password': '', 
       'ftp_output_path': ''}
    ...
    def _authenticate(self):
        self.session = FTPClient(self.config['ftp_host'], self.config['ftp_user'], self.config['ftp_password'])
...
class SFTP(FileBackend):
    ...
    DEFAULT_CONFIG = {'sftp_host': 'localhost', 
       'sftp_port': 22, 
       'sftp_user': 'anonymous', 
       'sftp_password': '', 
       'output_path': ''}
    ...
    def _authenticate(self):
        ...
        self._transport.connect(username=self.config['sftp_user'], password=self.config['sftp_password'])
    ...
    def put_file(self, path, file_handle):
        if self.state == self.STATE_CONNECTED:
            ...
            try:
                ...
                logging.info('Create remote file /' + path)
        ...
        try:
            ...
            for root, dirnames, filenames in os.walk(output_path, topdown=True):
                for filename in filenames:
                    ...
                    try:
                        remote_server.put_file(path.replace(output_path, remote_server.config['output_path']), file_handle)
                    except Exception as ex:
                        file_handle.close()
                        logging.error(ex)
                        raise ex
