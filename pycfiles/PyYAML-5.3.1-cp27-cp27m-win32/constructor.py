...
# [GCC 8.4.0]
# Embedded file name: build\lib.win32-2.7\yaml\constructor.py
...
from error import *
from nodes import *
import datetime, binascii, re, sys, types
...
class FullConstructor(SafeConstructor):
    ...
    def find_python_module(self, name, mark, unsafe=False):
        ...
        if unsafe:
            try:
                ...
                raise ConstructorError('while constructing a Python module', mark, 'cannot find module %r (%s)' % (name.encode('utf-8'), exc), mark)

        if name not in sys.modules:
            raise ConstructorError('while constructing a Python module', mark, 'module %r is not imported' % name.encode('utf-8'), mark)
    ...
    def find_python_name(self, name, mark, unsafe=False):
        ...
        if unsafe:
            try:
                ...
                raise ConstructorError('while constructing a Python object', mark, 'cannot find module %r (%s)' % (module_name.encode('utf-8'), exc), mark)

        if module_name not in sys.modules:
            raise ConstructorError('while constructing a Python object', mark, 'module %r is not imported' % module_name.encode('utf-8'), mark)
        module = sys.modules[module_name]
        if not hasattr(module, object_name):
            raise ConstructorError('while constructing a Python object', mark, 'cannot find %r in the module %r' % (object_name.encode('utf-8'),
    ...
    def construct_python_name(self, suffix, node):
        value = self.construct_scalar(node)
        if value:
            raise ConstructorError('while constructing a Python name', node.start_mark, 'expected the empty value, but found %r' % value.encode('utf-8'), node.start_mark)
        return self.find_python_name(suffix, node.start_mark)

    def construct_python_module(self, suffix, node):
        value = self.construct_scalar(node)
        if value:
            raise ConstructorError('while constructing a Python module', node.start_mark, 'expected the empty value, but found %r' % value.encode('utf-8'), node.start_mark)
