...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/odmltools/importers/import_datacite.py
# Compiled at: 2020-02-13 08:10:02
# Size of source mod 2**32: 23492 bytes
"""odmlImportDatacite
...
def camel_to_snake(in_string):
    tmp = re.sub('(.)([A-Z][a-z]+)', '\\1_\\2', in_string)
    return re.sub('([a-z0-9])([A-Z])', '\\1_\\2', tmp).lower()
...
def dict_from_xml(xml_file, extra_ns=None):
    ...
    try:
        with open(xml_file) as (file):
            doc = xmltodict.parse(file.read(), process_namespaces=True, namespaces=remove_nspaces)
        ...
        for sub in node:
            if sub in helper.attribute_map:
                ...
                if isinstance(sub, str) and sub.endswith('URI'):
                    dtype = DType.url
                Property(name=helper.attribute_map[sub], dtype=dtype, values=node[sub], parent=sec)
            else:
                print("[Warning] Ignoring node '%s/%s'" % (sec.name, sub))


def handle_name_identifiers(sub, node, sub_type_base, sec):
    name_identifier_map = {'#text': 'nameIdentifier', 
     '@schemeURI': 'schemeURI', 
     '@nameIdentifierScheme': 'nameIdentifierScheme'}
    name_identifier_helper = DataCiteItem(sec_name=sub, attribute_map=name_identifier_map, func=None, item_func=handle_props)
...
def handle_affiliations(sub, node, sub_type_base, sec):
    affiliation_map = {'#text': 'affiliation', 
     '@affiliationIdentifier': 'affiliationIdentifier', 
     '@affiliationIdentifierScheme': 'affiliationIdentifierScheme', 
     '@schemeURI': 'schemeURI'}
    affiliation_helper = DataCiteItem(sec_name=sub, attribute_map=affiliation_map, func=None, item_func=handle_props)
...
def handle_creators_item(_, node, sec):
    ...
    for sub in node:
        if sub == 'creatorName':
            creator_name_map = {'#text': 'creatorName', 
             '@nameType': 'nameType'}
            creator_name_helper = DataCiteItem(sec_name=sub, attribute_map=creator_name_map, func=None)
...
def handle_geo_entry(helper_list, node, sec, sub_sec_name, sub_sec_type):
    ...
    for entry in node:
        if entry in helper_list:
            try:
                Property(name=entry, dtype=DType.float, values=node[entry], parent=sub_sec)
            except ValueError:
    ...
    for sub in node:
        if sub in ('funderName', 'awardTitle'):
            ...
            if sub == 'awardNumber':
                ...
                award_number_helper = DataCiteItem(sec_name=sub, attribute_map=award_number_map, func=None)
                handle_props(award_number_helper, node[sub], sec)
            elif sub == 'funderIdentifier':
                funder_identifier_map = {'#text': 'funderIdentifier', 
                 '@funderIdentifierType': 'funderIdentifierType', 
                 '@schemeURI': 'schemeURI'}
                funder_identifier_helper = DataCiteItem(sec_name=sub, attribute_map=funder_identifier_map, func=None)
                handle_props(funder_identifier_helper, node[sub], sec)


def setup_supported_tags():
    identifier_map = {'#text': 'identifier', 
     '@identifierType': 'identifierType'}
    identifier_helper = DataCiteItem(sec_name='identifier', attribute_map=identifier_map, func=handle_sec)
    creators_helper = DataCiteItem(sec_name='creator', attribute_map=None, func=handle_container, container_name='creators', item_func=handle_creators_item)
    title_map = {'#text': 'title', 
     '@titleType': 'titleType'}
    title_helper = DataCiteItem(sec_name='title', attribute_map=title_map, func=handle_container, container_name='titles', item_func=handle_props)
    publisher_helper = DataCiteItem(sec_name='publisher', attribute_map={'#text': 'publisher'}, func=handle_props)
    publication_year_helper = DataCiteItem(sec_name='publicationYear', attribute_map={'#text': 'publicationYear'}, func=handle_props)
    subjects_map = {'#text': 'subject', 
     '@schemeURI': 'schemeURI', 
     '@subjectScheme': 'subjectScheme', 
     '@valueURI': 'valueURI'}
    subjects_helper = DataCiteItem(sec_name='subject', attribute_map=subjects_map, func=handle_container, container_name='subjects', item_func=handle_props)
    contributors_helper = DataCiteItem(sec_name='contributor', attribute_map=None, func=handle_container, container_name='contributors', item_func=handle_contributors_item)
    dates_map = {'#text': 'date', 
     '@dateType': 'dateType', 
     '@dateInformation': 'dateInformation'}
    dates_helper = DataCiteItem(sec_name='date', attribute_map=dates_map, func=handle_container, container_name='dates', item_func=handle_props)
    language_helper = DataCiteItem(sec_name='language', attribute_map={'#text': 'language'}, func=handle_props)
    res_type_map = {'#text': 'resourceType', 
     '@resourceTypeGeneral': 'resourceTypeGeneral'}
    res_type_helper = DataCiteItem(sec_name='resourceType', attribute_map=res_type_map, func=handle_sec)
    alternate_identifiers_map = {'#text': 'alternateIdentifier', 
     '@alternateIdentifierType': 'alternateIdentifierType'}
    alternate_identifiers_helper = DataCiteItem(sec_name='alternateIdentifier', attribute_map=alternate_identifiers_map, func=handle_container, container_name='alternateIdentifiers', item_func=handle_props)
    related_identifiers_map = {'#text': 'relatedIdentifier', 
     ...
     '@resourceTypeGeneral': 'resourceTypeGeneral'}
    related_identifiers_helper = DataCiteItem(sec_name='relatedIdentifier', attribute_map=related_identifiers_map, func=handle_container, container_name='relatedIdentifiers', item_func=handle_props)
    sizes_helper = DataCiteItem(sec_name='size', attribute_map={'#text': 'size'}, func=handle_container, container_name='sizes', item_func=handle_props)
    formats_helper = DataCiteItem(sec_name='format', attribute_map={'#text': 'format'}, func=handle_container, container_name='formats', item_func=handle_props)
    version_helper = DataCiteItem(sec_name='version', attribute_map={'#text': 'version'}, func=handle_props)
    rights_map = {'@schemeURI': 'schemeURI', 
     '@rightsIdentifierScheme': 'rightsIdentifierScheme', 
     '@rightsIdentifier': 'rightsIdentifier', 
     '@rightsURI': 'rightsURI'}
    rights_helper = DataCiteItem(sec_name='rights', attribute_map=rights_map, func=handle_container, container_name='rightsList', item_func=handle_props)
    descriptions_map = {'#text': 'description', 
     '@descriptionType': 'descriptionType'}
    descriptions_helper = DataCiteItem(sec_name='description', attribute_map=descriptions_map, func=handle_container, container_name='descriptions', item_func=handle_props)
    geo_locations_helper = DataCiteItem(sec_name='geoLocation', attribute_map=None, func=handle_container, container_name='geoLocations', item_func=handle_geo_locations)
    funding_references_helper = DataCiteItem(sec_name='fundingReference', attribute_map=None, func=handle_container, container_name='fundingReferences', item_func=handle_funding_references)
