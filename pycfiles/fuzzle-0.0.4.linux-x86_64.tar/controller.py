...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/fuzzle/controller.py
...
class FuzzyController:
    ...
    def __init__(self, input_vars, output_vars, rule_block):
        ...
        self.__output = dict([ (var.name, 0.0) for var in output_vars ])
    ...
    def __defuzzify(self, f_output):
        ...
        for var in self.__o_vars:
            if var in f_output:
                ...
                c_output[var] = self.__output[var] if c_value is None else c_value
            else:
    ...
    def as_dict(self):
        return {'in': {k:v.as_dict() for k, v in self.__i_vars.items()}, 'out': {k:v.as_dict() for k, v in self.__o_vars.items()}, 'rb': self.__rule_block.as_dict()}
