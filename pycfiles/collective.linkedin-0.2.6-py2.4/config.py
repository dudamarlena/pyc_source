...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/collective/linkedin/browser/config.py
# Compiled at: 2010-05-17 12:01:16
...
def add_company_info_js(portal, border=True, overwrite=False):
    ...
    if overwrite:
        ...
        if pjs.getResource(new_url) is None:
            pjs.manage_addScript(cacheable='False', compression='full', cookable='True', enabled='True', inline='True', id=new_url)
    elif pjs.getResource(old_url) is None and pjs.getResource(new_url) is None:
        pjs.manage_addScript(cacheable='False', compression='full', cookable='True', enabled='True', inline='True', id=new_url)
    return
...
class LinkedInConfigurationForm(ControlPanelForm):
    ...
    label = _('LinkedIn settings form')
    description = _('Setup options of LinkedIn inside your plone')
    form_name = _('LinkedIn Settings')
    ...
    @form.action(_('Generate Linkedin Credentials'), name='generate_credentials')
...
class LinkedInConfiguration(SimpleItem):
    ...
    def set_verification_number(self, value):
        ...
        acc_token = lin_api.accessToken(request_token=self.request_token, request_token_secret=self.request_token_secret, verifier=self.verification_number)
    ...
    def linkedin_get_user(self, member_linkedin_profile):
        if self.request_token and self.request_token_secret and self.verification_number and member_linkedin_profile:
            lin_api = linkedin.LinkedIn(self.api_key, self.secret_key, 'http://www.linkedin.com')
...
class LinkedinTool(BrowserView):
    ...
    def get_user_profile(self, user_profile_url):
        user_fields = [
         'current_status', 'educations', 'first_name', 'headline', 'honors', 'industry', 'interests', 'last_name', 'location', 'picture_url', 'positions', 'public_url', 'specialties', 'summary']
        dicter = lambda key, bogus: (
         key, getattr(user_profile, key))
