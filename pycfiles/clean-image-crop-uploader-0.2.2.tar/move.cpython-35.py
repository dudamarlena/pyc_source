...
# Embedded file name: /usr/local/lib/python3.5/dist-packages/clean/move.py
# Compiled at: 2018-03-25 11:52:18
# Size of source mod 2**32: 2185 bytes
__doc__ = 'Run file moving.'
...
def _replace_meta_tag(path: str, file: Path):
    file_stat = stat(str(file.resolve()))
    file_update_time = datetime.fromtimestamp(file_stat.st_mtime)
...
def _resolve_move_into(path: str, file: Path, use_meta_tag: bool=False):
    ...
    if move_into.exists():
        if not move_into.is_dir():
            click.echo('{} already exists. The file move setting will ignore.'.format(str(move_into)))
            return False
    else:
...
def move(is_fake: bool=True,
    ...
    for i in config.list_glob_path():
        ...
        for file in [Path(x) for x in file_lists]:
            ...
            if move_to.exists():
                click.echo('{} already exists. The file will not move.'.format(str(move_to)))
                continue
                if not is_silent:
