...
# Compiled at: 2018-10-16 03:08:24
"""
    ...
    def attribute_matching_regex(self, regex):
        ...
        for resource in self._resources:
            ...
            if self.validator.error_on_missing_attribute and not matched:
                errors.append(("[{0}.{1}] should have attribute matching regex: '{2}'").format(resource.type, resource.name, regex))
        ...
        for resource in self._resources:
            for attribute in attributes_list:
                if attribute not in resource.data.keys():
                    errors.append(("[{0}.{1}] should have attribute: '{2}'").format(resource.type, resource.name, attribute))
        ...
        for resource in self._resources:
            for attribute in attributes_list:
                if attribute in resource.data.keys():
                    errors.append(("[{0}.{1}] should not have attribute(s): '{2}'").format(resource.type, resource.name, attribute))
    ...
    def attribute(self, name):
        ...
        for attribute in self.attributes:
            if name in attribute.value.keys():
                attributes.append(Attribute(attribute._resource, ('{}.{}').format(attribute.name, name), attribute.value[name]))
            elif self.validator.error_on_missing_attribute:
                errors.append(("[{0}.{1}] should have attribute: '{2}'").format(attribute.resource_type, ('{0}.{1}').format(attribute.resource_name, attribute.name), attribute.name))
    ...
    def should_equal(self, value):
        ...
        for attribute in self.attributes:
            if not attribute.value == value:
                errors.append(("[{0}.{1}.{2}] should be '{3}'. Is: '{4}'").format(attribute.resource_type, attribute.resource_name, attribute.name, value, attribute.value))
    ...
    def should_not_equal(self, value):
        ...
        for attribute in self.attributes:
            if attribute.value == value:
                errors.append(("[{0}.{1}.{2}] should not be '{3}'. Is: '{4}'").format(attribute.resource_type, attribute.resource_name, attribute.name, value, attribute.value))
        ...
        for attribute in self.attributes:
            for provided_attribute in self.validator.to_list(attributes):
                if provided_attribute not in attribute.value.keys():
                    errors.append(("[{0}.{1}.{2}] should have attribute: '{3}'").format(attribute.resource_type, attribute.resource_name, attribute.name, provided_attribute))
        ...
        for attribute in self.attributes:
            for required_property_name in attributes:
                if required_property_name in attribute.keys():
                    errors.append(("[{0}.{1}.{2}] should not have attribute: '{3}'").format(attribute.resource_type, attribute.resource_name, attribute.name, required_property_name))
        ...
        for attribute in self.attributes:
            try:
                if not re.search(regex, attribute.value):
                    errors.append(("[{0}.{1}] should match regex '{2}'").format(attribute.resource_type, ('{0}.{1}').format(attribute.resource_name, attribute.name), regex))
    ...
    def should_be_valid_json(self):
        ...
        for attribute in self.attributes:
            try:
                ...
                errors.append(('[{0}.{1}.{2}] is not valid json').format(attribute.resource_type, attribute.resource_name, attribute.name))
...
class Variable(object):
    ...
    def value_exists(self):
        ...
        """
        assert self.value, ("Variable '{0}' should have a default value").format(self.name)
    ...
    def value_equals(self, value):
        ...
        """
        assert self.value == value, ("Variable '{0}' should have a default value of {1}. Is: {2}").format(self.name, value, self.value)
    ...
    def value_matches_regex(self, regex):
        ...
        """
        assert re.search(regex, self.value), ("Variable '{0}' value should match regex '{1}'. Is: {2}").format(self.name, regex, self.value)
