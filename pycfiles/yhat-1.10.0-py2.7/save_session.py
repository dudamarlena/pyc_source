...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yhat/deployment/save_session.py
...
import pickle, terragon, ast, inspect, tokenize, types, json, sys, re, io, os, pprint as pp
    ...
    if module in modules or module is None:
        ...
        if _is_on_syspath(module.__file__) == False:
            ...
            if module.__file__.endswith('.py'):
                ...
                modules[module] = {'parent_dir': parent_dir, 
                ...
                if os.path.isfile(finit):
                    with open(finit, 'rb') as (f):
                        init_source = f.read()
                        modules['init'] = {'parent_dir': parent_dir, 
                           'name': '__init__.py', 
                           'source': init_source}
                logging.info('\tparsing source for %s\n' % module_name)
                logging.debug('%s\n' % module_source)
...
def _is_tensor(obj):
    try:
        ...
        try:
            return None != re.search('(tensorflow)', str(obj.__class__))
        except:
            return False
...
def _is_spark(obj):
    try:
        ...
        try:
            return None != re.search('(pyspark)', str(obj.__class__))
        except:
            return False
...
def _is_pom(obj):
    try:
        ...
        try:
            return None != re.search('(pomegranate)', str(obj.__class__))
        except:
            return
...
def _spider_function(function, session, pickles={}):
    ...
    return (
     imports, source, pickles, modules)
