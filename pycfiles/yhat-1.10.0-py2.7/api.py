...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/yhat/api.py
...
import sys, warnings, base64, json, pickle, terragon, inspect, tempfile, zlib, logging, re, os, os.path, requests
...
getExplicitRequirements, getImplicitRequirements = (
 None, None)
try:
...
class API(object):
    ...
    def _post_file(self, endpoint, params, data, pb=True):
        ...
        encoder = MultipartEncoder(fields={'model_name': open(f.name, 'rb')})
        username, apikey = params['username'], params['apikey']
...
class Yhat(API):
    ...
    def deploy_tensorflow(self, name, model, session, sess, sure=False, packages=[], patch=None, dry_run=False, verbose=0, autodetect=True, custom_image=None):
        ...
        patch += "print('done!')\n\n"
        return self.deploy(name, model, session, sure=sure, packages=packages, patch=patch, dry_run=dry_run, verbose=verbose, autodetect=autodetect, is_tensorflow=True)
    ...
    def deploy_spark(self, name, model, session, sc, sure=False, packages=[], patch=None, dry_run=False, verbose=0, autodetect=True, custom_image=None):
        ...
        return self.deploy(name, model, session, sure=sure, packages=packages, patch=patch, dry_run=dry_run, verbose=verbose, autodetect=autodetect)
