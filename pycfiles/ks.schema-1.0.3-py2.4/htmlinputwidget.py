...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/ks/schema/html/browser/htmlinputwidget.py
...
class HTMLImageListJS(object):
    ...
    def __call__(self, *kv, **kw):
        ...
        images = getattr(queryMultiAdapter((context, self.request), interface=IHTMLImageListProvider, context=context), 'images', None)
        self.request.response.setHeader('Content-Type', 'text/javascript; charset=utf-8')
        if images:
            return 'var tinyMCEImageList = %s;\n' % demjson.encode(images)
        return ''