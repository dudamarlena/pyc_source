...
# [GCC 8.4.0]
# Embedded file name: /Users/thomas/Dev/Project/django-trusts/trusts/tests.py
# Compiled at: 2016-04-30 02:30:17
...
from trusts.models import Trust, TrustManager, Content, Junction, Role, RolePermission, TrustUserPermission
...
class TrustTest(TestCase):
    ...
    def test_read_permissions_added(self):
        ...
        self.assertIsNotNone(Permission.objects.get(content_type=ct, codename='%s_%s' % ('read', ct.model)))
...
class DecoratorsTest(TestCase):
    ...
    def test_permission_required(self):
        ...
        decorated_func = permission_required('auth.read_group', fieldlookups_kwargs={'pk': 'pk'}, raise_exception=False)(mock)
        ...
        self.user.has_perms = has_perms
        decorated_func = permission_required('auth.read_group', fieldlookups_kwargs={'pk': 'pk'})(mock)
        response = decorated_func(self.request, pk=self.group.pk)
    ...
    def test_permission_required_P(self):
        ...
        has_perms = Mock(return_value=False)
        self.user.has_perms = has_perms
        decorated_func = permission_required(P('auth.read_group', fieldlookups_kwargs={'pk': 'pk'}), raise_exception=False)(mock)
        response = decorated_func(self.request, pk=self.group.pk)
        self.assertFalse(mock.called)
        self.assertTrue(response.status_code, 403)
        self.assertTrue(has_perms.called)
        ...
        has_perms = Mock(return_value=True)
        self.user.has_perms = has_perms
        decorated_func = permission_required(P('auth.read_group', fieldlookups_kwargs={'pk': 'pk'}) & P('auth.add_group', fieldlookups_kwargs={'pk': 'pk'}), raise_exception=False)(mock)
        response = decorated_func(self.request, pk=self.group.pk)
        self.assertTrue(mock.called)
        mock.assert_called_with(self.request, pk=self.group.pk)
        self.assertEqual(response, 'Response')
...
class ContentModel(object):
    ...
    def set_perms(self):
        for codename in ['change', 'add', 'delete', 'read']:
            setattr(self, 'perm_%s' % codename, Permission.objects.get_by_natural_key('%s_%s' % (codename, self.model_name), self.app_label, self.model_name))
...
class ContentModelMixin(RuntimeModel, ContentModel):
    ...
    def setUp(self):
        ...
        self.model = ModelBase('Category', (mixin, models.Model), {'__module__': mixin.__module__})
...
class JunctionModelMixin(RuntimeModel, ContentModel):
    ...
    def setUp(self):
        ...
        self.model = ModelBase('TestGroupJunction', (mixin, models.Model), {'__module__': mixin.__module__})
...
class DecoratorExpressionTest(ContentModelMixin, TestCase):
    ...
    def test_P_and(self):
        p = P(self.get_perm_code(self.perm_change), fieldlookups_kwargs={'pk': 'pk'}) & P(self.get_perm_code(self.perm_delete), fieldlookups_kwargs={'pk': 'pk'})
        mock = Mock(return_value='Response')
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content1.pk)
        self.assertTrue(mock.called)
        mock = Mock(return_value='Response')
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content2.pk)
        self.assertFalse(mock.called)
        p = P(self.get_perm_code(self.perm_change), fieldlookups_kwargs={'pk': 'pk'}) & P('auth.falseperm_group', fieldlookups_kwargs={'pk': 'pk'})
        mock = Mock(return_value='Response')
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content1.pk)
        self.assertFalse(mock.called)
    ...
    def test_P_or(self):
        ...
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content2.pk)
        self.assertTrue(mock.called)
        p = P('auth.falseperm_group', fieldlookups_kwargs={'pk': 'pk'}) | P('auth.falsefalseperm_group', fieldlookups_kwargs={'pk': 'pk'})
        mock = Mock(return_value='Response')
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content1.pk)
        self.assertFalse(mock.called)

    def test_P_complex(self):
        p = P(self.get_perm_code(self.perm_change), fieldlookups_kwargs={'pk': 'pk'}) | P('auth.falseperm_group', fieldlookups_kwargs={'pk': 'pk'}) & P('admin.change_all')
        mock = Mock(return_value='Response')
        permission_required(p, raise_exception=False)(mock)(self.request, pk=self.content1.pk)
        self.assertTrue(mock.called)
        mock = Mock(return_value='Response')
