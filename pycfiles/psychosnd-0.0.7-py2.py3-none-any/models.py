...
# Embedded file name: /mindhive/dicarlolab/u/qbilius/Dropbox (MIT)/psychopy_ext/psychopy_ext/models.py
# Compiled at: 2016-04-22 08:50:11
...
import sys, os, glob, itertools, warnings, inspect, argparse, imp, tempfile, shutil, pickle
from collections import OrderedDict
import numpy as np, scipy.ndimage, pandas, seaborn as sns, matlab_wrapper, sklearn.manifold, sklearn.preprocessing, sklearn.metrics, sklearn.cluster, skimage.feature, skimage.data
...
class Model(object):
    ...
    def download_model(self, path=None):
        ...
        if self.model.model_url is None:
            ...
            print(('WARNING: Unfortunately, you need to download {} manually. Follow the instructions in the documentation.').format(self.nice_name))
        else:
            print(('Downloading and extracting {}...').format(self.nice_name))
            if path is None:
                path = os.getcwd()
                text = raw_input(('Where do you want the model to be extracted? (default: {})\n').format(path))
                if text != '':
                    path = text
            outpath, _ = utils.extract_archive(self.model.model_url, folder_name=self.safename, path=path)
            if self.name == 'phog':
            ...
            print(('If you want to use this model, either give this path when calling the model or add it to your path using {} as the environment variable.').format(self.safename.upper()))
        return
...
class _Model(object):
    ...
    def gen_report(self, test_ims, train_ims=None, html=None):
        ...
        html.writeimg('dis', caption='Dissimilarity across stimuli(blue: similar, red: dissimilar)')
        html.writeh('MDS', h=1)
        mds_res = mds(dis)
        ...
        if self.labels is not None:
            ...
            plot_data(lin, kind='linear_clf', chance=1.0 / len(np.unique(self.labels)))
    ...
    def _setup_layers(self, layers, model_keys):
        ...
        if layers in (None, 'top', 'output'):
            ...
            if isinstance(layers[0], int):
                ...
                raise ValueError('Layers can only be: None, "all", int or str, list of int or str, got', layers)
        else:
            raise ValueError('Layers can only be: None, "all", int or str, list of int or str, got', layers)
        return
        ...
        if isinstance(ims, (str, unicode)):
            ...
            if ndim == 1:
                ...
                if ndim == 2:
                    ...
                    raise ValueError('images must be 2D or 3D, got %d dimensions instead' % ndim)
            elif ndim == 3:
                out = [ims]
            elif ndim == 4:
                out = ims
            else:
                raise ValueError('images must be 3D or 4D, got %d dimensions instead' % ndim)
        return out
...
def mds(dis, ims=None, kind='metric', seed=None):
    ...
    for layer_name, this_dis in dis.items():
        if kind == 'classical':
            ...
            mds_model = sklearn.manifold.MDS(n_components=2, dissimilarity='precomputed', random_state=seed)
...
def cluster(resps, labels, metric=None, clust=None, bootstrap=True, stratified=False, niter=1000, ci=95, *func_args, **func_kwargs):
    if metric is None:
    ...
    for layer, data in resps.items():
        ...
        if bootstrap:
            pct = stats.bootstrap_resample(data1=data, data2=labels, niter=niter, func=mt, struct=struct, ci=None, *func_args, **func_kwargs)
...
def linear_clf(resps, y, clf=None):
    ...
    for layer, resp in resps.items():
        ...
        cv = sklearn.cross_validation.StratifiedKFold(y, n_folds=n_folds, shuffle=True)
        preds = sklearn.cross_validation.cross_val_predict(clf(), resp, y, cv=cv)
...
class Pixelwise(_Model):
    ...
    def test(self, test_ims, layers='output', return_dict=False):
        ...
        resps = self._fmt_output(OrderedDict([(self.safename, resps)]), layers, return_dict=return_dict)
        return resps
...
class Retinex(_Model):
    ...
    def gen(self, im, thres=0.078125, plot=True, save=False):
...
class Zoccolan(_Model):
    ...
    def get_gabors(self, rf):
        lams = float(rf[0]) / self.sfs
        sigma = rf[0] / 2.0 / np.pi
    ...
    def test(self, im):
        ...
        for r, rf in enumerate(self.rfs):
            ...
            def apply_filter(window, this_filter):
                ...
                return np.max((0, this_resp))

            def filter_bank(this_filter, rf):
                resp = scipy.ndimage.filters.generic_filter(im, apply_filter, size=rf, mode='nearest', extra_arguments=(
                 this_filter,))
                ii, jj = np.meshgrid(np.arange(0, field[0], size[0]), np.arange(0, field[1], size[1]))
                selresp = resp[(jj, ii)]
...
class GaborJet(_Model):
    ...
    def __init__(self, nscales=5, noris=8, imsize=256, grid_size=0):
        ...
        if grid_size == 0:
            s = imsize / 128.0
            rangeXY = np.arange(20 * s, 110 * s + 1, 10 * s) - 1
        elif grid_size == 1:
            s = imsize / 128.0
            rangeXY = np.arange(10 * s, 120 * s + 1, 10 * s) - 1
        else:
            rangeXY = np.arange(imsize)
        self.rangeXY = rangeXY.astype(int)
        xx, yy = np.meshgrid(rangeXY, rangeXY)
        self.grid = xx + complex(0.0, 1.0) * yy
        self.grid = self.grid.T.ravel()
        self.grid_pos = np.hstack([self.grid.imag, self.grid.real]).T

    def test(self, test_ims, cell_type='complex', sigma=2 * np.pi, layers='magnitudes', return_dict=False):
        """
    ...
    def _test(self, im, cell_type='complex', sigma=2 * np.pi):
        ...
        for scale in range(self.nscales):
            ...
            for ori in range(self.noris):
                ...
                k0x = k0 * np.cos(ka)
                k0y = k0 * np.sin(ka)
                freq_kernel = 2 * np.pi * (np.exp(-(sigma / k0) ** 2 / 2 * ((k0x - tx) ** 2 + (k0y - ty) ** 2)) - np.exp(-(sigma / k0) ** 2 / 2 * (k0 ** 2 + tx ** 2 + ty ** 2)))
...
class HMAX99(_Model):
    ...
    def __init__(self, matlab=False, filter_type='gaussian'):
        ...
        if filter_type == 'gaussian':
            if matlab:
                self.filts = self.get_gaussians_matlab(self.filter_sizes_all, self.n_ori)
            else:
                self.filts = self.get_gaussians(self.filter_sizes_all, self.n_ori)
    ...
    def train(self, train_ims):
        ...
        try:
            ...
            self.tuning = self.test(train_ims, op='training', layers='C2', return_dict=False)
    ...
    def get_gaussians(self, filter_sizes_all, n_ori=4, sigDivisor=4.0):
        """
        ...
        for fNo, filter_sizes in enumerate(filter_sizes_all):
            ...
            for filter_size in filter_sizes:
                ...
                for t in range(n_ori):
                    ...
                    x = ii * np.cos(theta) - jj * np.sin(theta)
                    y = ii * np.sin(theta) + jj * np.cos(theta)
                    gaussian = (y ** 2 / sigmaq - 1) / sigmaq * np.exp(-(x ** 2 + y ** 2) / (2 * sigmaq))
    ...
    def get_gaussians_matlab(self, filter_sizes_all, n_ori=4, sigDivisor=4.0):
        """
    ...
    def get_gabors(self, filter_sizes_all, n_ori=4, k=2.1, sx=2 * np.pi * 1 / 3.0, sy=2 * np.pi * 1 / 1.8, phase=0):
        """
        ...
        for fNo, filter_sizes in enumerate(filter_sizes_all):
            ...
            for filter_size in filter_sizes:
                ...
                for t in range(n_ori):
                    ...
                    x = ii * np.cos(theta) - jj * np.sin(theta)
                    y = ii * np.sin(theta) + jj * np.cos(theta)
                    gabor = np.cos(k * x - phase) * np.exp(-((x / sx) ** 2 + (y / sy) ** 2) / 2)
                    gabor *= circle
    ...
    def get_circle(self, filter_size, radius=1.0):
        inc = 2.0 / filter_size
    ...
    def addZeros(self, matrix, numZeros):
        ...
        matrix_new[numZeros:matrix.shape[0] + numZeros, numZeros:matrix.shape[1] + numZeros] = matrix
        return matrix_new
    ...
    def get_S1(self, im, whichBand):
        ...
        for j in range(num_filter):
            ...
            if self.mask_name == 'circle':
                ...
                mask = np.ones((fs, fs))
            norm = scipy.ndimage.convolve(im ** 2, mask, mode='constant') + sys.float_info.epsilon
            for i in range(self.n_ori):
                S1_buf = scipy.ndimage.convolve(im, S1_filter[:, :, i], mode='constant')
    ...
    def get_C1(self, S1, which_band):
        ...
        C1 = scipy.ndimage.filters.maximum_filter(S1, size=(
         C1_pooling, C1_pooling, 1, 1), mode='constant', origin=-(C1_pooling // 2))
        C1 = np.squeeze(np.max(C1, 2))
        return C1

    def get_S2(self, C1, which_band, target=1.0, sigma=1.0):
        """
        ...
        sharpness.
        """
        S2_shift = int(np.ceil(self.C1_pooling_all[which_band] / 2.0))
        C1_shift = S2_shift * 2
        S2_buf = [C1.shape[0] - C1_shift * (self.S2_config[0] - 1),
         C1.shape[1] - C1_shift * (self.S2_config[1] - 1)]
        seq = itertools.product(range(self.n_ori), repeat=self.S2_config[0] * self.S2_config[1])
    ...
    def get_VTU(self, C2resp, tuningWidth=0.1):
        """
        ...
        def func(row):
            diff = self.tuning - np.tile(row, (self.tuning.shape[0], 1))
            return np.exp(-0.5 * np.apply_along_axis(sq, 1, diff) / tuningWidth)

        if not self.istrained:
            raise Exception('You must first train VTUs by providing prototype images to them using the train() function')
        if C2resp.shape[1] != self.tuning.shape[1]:
            raise Exception('The size of exemplar matrix does not match that of the prototype matrix')
...
class HOG(_Model):
    ...
    def test(self, test_ims, layers='output', return_dict=False, **kwargs):
        ...
        output = self._fmt_output(OrderedDict([(self.safename, resps)]), layers, return_dict=return_dict)
        return output
...
class Caffe(_Model):
    ...
    def _set_paths(self):
        try:
            ...
            raise Exception("Caffe not found in your path; it must be set in the 'CAFFE' variable")
        else:
        ...
        if self.weight_file is not None:
            ...
            try:
                ...
                raise Exception('Model %s not recognized. Please provide weight_file when calling this model.' % self.name)
    ...
    def layers_from_prototxt(self, keep=[
 'Convolution', 'InnerProduct']):
        self._set_paths()
        net = caffe_pb2.NetParameter()
        model_file = glob.glob(os.path.join(self.model_file, '*deploy*.prototxt'))[0]
 ...
 'Convolution', 'InnerProduct']):
    ...
    def _classifier(self):
        ...
        self.transformer.set_mean('data', mn.mean(1).mean(1))
    ...
    def predict(self, ims, topn=5):
        ...
        for pred in preds:
            ...
            tmp = []
            for n in classno:
                d = {'classno': n, 'synset': labels[n][0], 'label': labels[n][1], 
                   'confidence': pred[n]}
                tmp.append(d)
    ...
    def _get_labels(self):
        synset_file = os.path.join(os.environ['CAFFE'], 'data/ilsvrc12/synset_words.txt')
        try:
...
class MATLABModel(_Model):
    ...
    def _set_model_path(self):
        if self.model_path is None:
            ...
            if safename not in os.environ:
                raise Exception('Please specify model_path to the location of %s or add it to your path using %s as the environment variable.' % (
                 self.name, safename))
            else:
    ...
    def test(self, test_ims, layers='output', return_dict=True):
        ...
        del matlab
        resps = self._fmt_output(resps, layers, return_dict=return_dict)
        return resps
...
class HMAX_HMIN(MATLABModel):
    ...
    def __init__(self, model_path=None, matlab_root=None):
        ...
        """
        super(HMAX_HMIN, self).__init__(model_path=model_path, matlab_root=matlab_root)
        self.name = 'HMAX-HMIN'
...
class HMAX_PNAS(MATLABModel):
    ...
    def __init__(self, model_path=None, matlab_root=None):
        ...
        """
        super(HMAX_PNAS, self).__init__(model_path=model_path, matlab_root=matlab_root)
        self.name = 'HMAX-PNAS'
    ...
    def _test(self, matlab, test_ims, layers=[
 'C3'], nfeatures=2000, **kwargs):
        """
        Test model
        ...
        if layers == 'all':
            ...
            matlab.put('featureFile', os.path.join(self.model_path, 'featureSets/featuresNatural_newGRBF.mat'))
...
class PHOG(MATLABModel):
    ...
    def __init__(self, model_path=None, matlab_root=None):
        ...
        """
        super(PHOG, self).__init__(model_path=model_path, matlab_root=matlab_root)
        self.name = 'PHOG'
        self.safename = 'phog'
...
class PHOW(MATLABModel):
    ...
    def __init__(self, model_path=None, matlab_root=None):
        ...
        """
        super(PHOW, self).__init__(model_path=model_path, matlab_root=matlab_root)
        self.name = 'PHOW'
        self.safename = 'phow'
        self.model_url = 'http://slazebni.cs.illinois.edu/research/SpatialPyramid.zip'

    def _test(self, matlab, test_ims, max_imsize=1000, grid_spacing=8, patch_size=16, dict_size=200, ntextons=50, npyrlevs=3, old_sift=False, can_skip=1, save_sift=1, **kwargs):
        params = [
         (
          'maxImageSize', max_imsize),
         (
         ...
         (
          'oldSift', old_sift)]
        params = np.array([tuple(p[1] for p in params)], dtype=[ (p[0], '<f8') for p in params ])
...
class RandomFilters(MATLABModel):
    ...
    def __init__(self, model_path=None, matlab_root=None):
        ...
        """
        super(RandomFilters, self).__init__(model_path=model_path, matlab_root=matlab_root)
    ...
    def _test(self, matlab, test_ims, **kwargs):
        ...
        for imno, im in enumerate(ims):
            ...
            im = self.load_image(im, flatten=True)
            mx = max(im.shape[:2])
            sh = (151.0 / mx * im.shape[0], 151.0 / mx * im.shape[1])
...
def run(model_name='HMAX', impaths=None):
    if model_name in KNOWN_MODELS:
        ...
        raise Exception(('ERROR: model {0} not recognized. Choose from:\n {1}').format(model_name, KNOWN_MODELS.keys()))
...
def compare(func, test_ims, layer='output', models=None, plot=True, save=False, html=None, *func_args, **func_kwargs):
    if models is None:
...
class Compare(object):
    ...
    def pairwise_stats(self, models1, models2=None, func=None, bootstrap=True, plot=False, niter=1000, ci=95, struct=None, *func_args, **func_kwargs):
        if models2 is None:
            models2 = models1
        if func is None:
            func = stats.corr
            ...
            for layer1, d1 in data1.items():
                for name2, data2 in models2.items():
                    for layer2, d2 in data2.items():
                        ...
                        if bootstrap:
                            pct = stats.bootstrap_resample(d1, data2=d2, func=func, niter=niter, ci=ci, struct=struct, *func_args, **func_kwargs)
                        else:
                            pct = (
                             np.nan, np.nan)
        ...
        cols = [
         'model1', 'layer1', 'model2', 'layer2', 'correlation',
         'ci_low', 'ci_high']
...
MATLAB_MODELS = {'hmax_hmin': HMAX_HMIN, 'hmax_pnas': HMAX_PNAS, 'phog': PHOG, 
   'phow': PHOW, 'randfilt': RandomFilters}
KNOWN_MODELS = {'px': Pixelwise, 'gaborjet': GaborJet, 'hmax99': HMAX99, 'hog': HOG}
KNOWN_MODELS.update(MATLAB_MODELS)
aliases_inv = {'px': ['px', 'pixelwise', 'pixel-wise'], 'gaborjet': [
              'gaborjet', 'gj'], 
   'vgg-19': [
            'vgg-19', 'VGG_ILSVRC_19_layers', 'VGG-19'], 
   'places': [
            'places', 'Places205-CNN', 'Places'], 
   'googlenet': [
               'googlenet', 'GoogleNet', 'GoogLeNet'], 
   'cifar10': [
             'cifar10', 'CIFAR10_full_deploy']}
ALIASES = {v:k for k, vals in aliases_inv.items() for v in vals}
ALIASES.update({k:k for k in KNOWN_MODELS})
NICE_NAMES = {'px': 'Pixelwise', 'gaborjet': 'GaborJet', 
   'retinex': 'Retinex', 
   'hog': 'HOG', 
   'hmax99': "HMAX'99", 
   'hmax_hmin': 'HMAX-HMIN', 
...
if __name__ == '__main__':
    ...
    if parsed.task == 'report':
        gen_report(models=parsed.models, test_ims=ims, path=parsed.output, labels=parsed.labels)
    else:
