...
# Embedded file name: /home/jpopelka/git/user-cont/colin/colin/core/checks/dockerfile.py
# Compiled at: 2018-09-04 03:34:18
# Size of source mod 2**32: 4930 bytes
import logging, re
...
class InstructionAbstractCheck(DockerfileAbstractCheck):
    ...
    def __init__(self, message, description, reference_url, tags, instruction, value_regex, required):
        super(InstructionAbstractCheck, self).__init__(message, description, reference_url, tags)
    ...
    def check(self, target):
        ...
        for inst in instructions:
            ...
            log = "Value for instruction {} {}mach regex: '{}'.".format(inst['content'], '' if match else 'does not ', self.value_regex)
            logs.append(log)
            logger.debug(log)
...
class InstructionCountAbstractCheck(DockerfileAbstractCheck):
    ...
    def __init__(self, message, description, reference_url, tags, instruction, min_count=None, max_count=None):
        super(InstructionCountAbstractCheck, self).__init__(message, description, reference_url, tags)
        self.instruction = instruction
    ...
    def check(self, target):
        ...
        log = 'Found {} occurrences of the {} instruction. Needed: min {} | max {}'.format(count, self.instruction, self.min_count, self.max_count)
        logger.debug(log)
        passed = True
...
class DockerfileLabelAbstractCheck(DockerfileAbstractCheck):
    ...
    def __init__(self, message, description, reference_url, tags, label, required, value_regex=None):
        super(DockerfileLabelAbstractCheck, self).__init__(message, description, reference_url, tags)
        self.label = label
        self.required = required
        self.value_regex = value_regex
    ...
    def check(self, target):
        ...
        passed = check_label(labels=(self.label), required=(self.required),
