...
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/catenae/connectors/mongodb.py
# Compiled at: 2019-08-07 08:55:15
# Size of source mod 2**32: 6585 bytes
...
class MongodbConnector:
    ...
    def _get_collection(self, database_name, collection_name):
        self.open_connection()
        database_name, collection_name = self._get_database_and_collection_names(database_name, collection_name)
    ...
    def _get_database_and_collection_names(self, database_name, collection_name):
        if not database_name:
            if hasattr(self, '_default_database'):
    ...
    def create_index(self, attribute=None, keys=None, database_name=None, collection_name=None, unique=False, type_='asc', background=True):
        if not keys:
            if not attribute:
                raise TypeError
        if not keys:
    ...
    def get(self, query=None, database_name=None, collection_name=None, sort=None, sort_attribute=None, sort_type=None, limit=None, index=None, index_attribute=None, index_type=None):
        collection = self._get_collection(database_name, collection_name)
        if sort_attribute:
            if sort_type:
                if sort_type == 'desc':
            ...
            if not index:
                if not index_attribute or index_type:
                    if index:
    ...
    def get_random(self, query=None, database_name=None, collection_name=None, sort=None, sort_attribute=None, sort_type=None, limit=1):
        collection = self._get_collection(database_name, collection_name)
        if sort_attribute:
            if sort_type:
                if sort_type == 'desc':
