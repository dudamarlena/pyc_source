...
# Embedded file name: build/bdist.linux-x86_64/egg/powerpack/electrical.py
# Compiled at: 2020-05-10 14:01:09
# Size of source mod 2**32: 234870 bytes
...
class ElecAnalysis(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     'type':'object', 
     'title':'powerpack.electrical.ElecAnalysis Base Schema', 
     'required':[],  'properties':{'electric_range':{'type':'number', 
      ...
      'number_cells':{'type':'number', 
       'title':'Cell Number', 
       'examples':[
        1], 
       'editable':False, 
       'description':'Cell Number'}, 
      'number_modules':{'type':'number', 
       'title':'Module Number', 
       'examples':[
        1], 
       'editable':False, 
    ...
    def __init__(self, electric_range=None, load_time=None, number_cells=None, number_modules=None, voltage_max=None, voltage_min=None, voltage_mean=None, current_max=None, current_min=None, current_mean=None, name=''):
    ...
    def __eq__(self, other):
        equa = self.electric_range == other.electric_range and self.load_time == other.load_time and self.number_cells == other.number_cells and self.number_modules == other.number_modules and self.voltage_max == other.voltage_max and self.voltage_min == other.voltage_min and self.voltage_mean == other.voltage_mean and self.current_max == other.current_max and self.current_min == other.current_min and self.current_mean == other.current_mean
        return equa
    ...
    def to_dict(self):
        ...
        dict_.update({'electric_range':self.electric_range,  'load_time':self.load_time, 
    ...
    def dict_to_object(cls, dict_):
        ...
        if dict_ != None:
            ...
            return cls(electric_range=electric_range, load_time=load_time, number_cells=number_cells,
...
class Evolution(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'evolution'], 
     'properties':{'evolution': {'type':'array', 
                    'title':'Evolution', 
                    'items':{'type':'number', 
                     'editable':True, 
                     'step':0.001}, 
    ...
    def __hash__(self):
        li_hash = int(hash(tuple(self.evolution)) % 100000000.0)
        return li_hash
    ...
    def _display_angular(self):
        if self.name == '':
            ...
            label = self.name
        displays = [
         {'angular_component':'graph', 
          'x_label':'increment', 
          'show_table':True, 
          'datasets':[
           {'label':label, 
            'color':None, 
            'values':[{'x':x,  'y':y} for x, y in enumerate(self.evolution)]}]}]
        return displays
...
class CombinationEvolution(DessiaObject):
    """CombinationEvolution"""
    _standalone_in_db = True
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     'definitions':{},  'type':'object', 
     'title':'powerpack.electrical.CombinationEvolution Base Schema', 
     'required':[
      'evolution1', 'evolution2'], 
     'properties':{'evolution1':{'type':'array', 
       'title':'Evolution x', 
       'order':3, 
       'editable':True, 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.Evolution']}}, 
      'evolution2':{'type':'array', 
       'title':'Evolution y', 
       'order':4, 
       'editable':True, 
        ...
        displays = [
         {'angular_component':'graph', 
          ...
          'datasets':[
           {'label':self.title2, 
            'color':None, 
            'values':[{'x':x,  'y':y} for x, y in zip(self.x, self.y)]}]}]
        return displays
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'evolution1':[e.to_dict() for e in self.evolution1],  'evolution2':[e.to_dict() for e in self.evolution2], 
         'title1':self.title1, 
         'title2':self.title2, 
         'x':self.x, 
         'y':self.y})
    ...
    def DictToObject(cls, dict_):
        ...
        """
        li_evolution1 = [Evolution.DictToObject(evolution1) for evolution1 in dict_['evolution1']]
        li_evolution2 = [Evolution.DictToObject(evolution2) for evolution2 in dict_['evolution2']]
        evol = cls(evolution1=li_evolution1, evolution2=li_evolution2,
          title1=(dict_['title1']),
          title2=(dict_['title2']),
          name=(dict_['name']))
        return evol
...
class PowerProfile(DessiaObject):
    ...
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'soc_init', 'combination_evolutions'], 
     'properties':{'combination_evolutions':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.CombinationEvolution']}}, 
      'soc_init':{'type':'number', 
       'title':'Initial SoC', 
       'minimum':0, 
       'step':0.001, 
       'examples':[
    ...
    def __init__(self, soc_init, combination_evolutions, power_accuracy=None, loop=False, max_loop=None, soc_end=None, charger=False, use_selection=True, name=''):
    ...
    def __eq__(self, other):
        equal = self.soc_init == other.soc_init and self.power_accuracy == other.power_accuracy and self.loop == other.loop and self.max_loop == other.max_loop and self.soc_end == other.soc_end and self.charger == other.charger and self.use_selection == other.use_selection
    ...
    def _display_angular(self):
        ...
        for combination_evolution in self.combination_evolutions:
            datasets.append({'label':combination_evolution.title2, 
             'color':None, 
             'values':[{'x':x,  'y':y} for x, y in zip(combination_evolution.x, combination_evolution.y)]})

        displays = [
         {'angular_component':'graph',  'x_label':'Power Profile ' + self.name, 
          'show_table':False, 
          'datasets':datasets}]
        return displays
    ...
    def Dict(self):
        ...
        dict_.update({'soc_init':self.soc_init,  'loop':self.loop, 
    ...
    def DictToObject(cls, dict_):
        ...
        """
        p_evol = [CombinationEvolution.dict_to_object(ce) for ce in dict_['combination_evolutions']]
        ...
        if 'soc_end' in dict_:
            ...
            soc_end = None
        profile = cls(combination_evolutions=p_evol, soc_init=(dict_['soc_init']), power_accuracy=power_accuracy,
          loop=(dict_['loop']),
          max_loop=max_loop,
          soc_end=soc_end,
          charger=(dict_['charger']),
          use_selection=(dict_['use_selection']),
          name=(dict_['name']))
        return profile


class CombinationPowerProfile(DessiaObject):
    """CombinationPowerProfile"""
    _standalone_in_db = True
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     'definitions':{},  'type':'object', 
     ...
     'required':[
      'power_profiles'], 
     'properties':{'power_profiles': {'type':'array', 
                         ...
                         'items':{'type':'object', 
                          'classes':[
                           'powerpack.electrical.PowerProfile']}}}}
    ...
    def DictToObject(cls, dict_):
        ...
        """
        p_evol = [PowerProfile.DictToObject(power_profile) for power_profile in dict_['power_profiles']]
...
class SpecsEvolution(DessiaObject):
    ...
    def __init__(self, temperature, charge, discharge=None, title='', name=''):
        ...
        if discharge is not None:
            ...
            self.discharge = CombinationEvolution(evolution1=[evolution1], evolution2=[
             evolution2])
        self.title = title
        DessiaObject.__init__(self, name=name)
        ...
        displays = [
         {'angular_component':'graph', 
          ...
          'datasets':[
           {'label':'charge', 
            'color':None, 
            'values':[{'x':x,  'y':y} for x, y in zip(self.charge.x, self.charge.y)]},
           {'label':'discharge', 
            'color':None, 
            'values':[{'x':x,  'y':y} for x, y in zip(self.discharge.x, self.discharge.y)]}]}]
        return displays
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'charge':self.charge.Dict(),  'discharge':self.discharge.Dict(), 
         'title':self.title, 
    ...
    def DictToObject(cls, dict_):
        ...
        if dict_['discharge'] is not None:
            evol = cls(temperature=(dict_['temperature']), charge=(CombinationEvolution.DictToObject(dict_['charge'])),
...
class CombinationSpecsEvolution(DessiaObject):
    ...
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'specs_evolutions'], 
     'properties':{'specs_evolutions':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.SpecsEvolution']}}, 
      'title':{'type':'string', 
       'title':'Title', 
       'examples':[
        'Title'], 
        ...
        for specs_evolution in self.specs_evolutions:
            datasets.extend([
             {'label':'charge' + ' Temp {}'.format(int(specs_evolution.temperature)), 
              'color':None, 
              'values':[{'x':x,  'y':y} for x, y in zip(specs_evolution.charge.x, specs_evolution.charge.y)]},
             {'label':'discharge' + ' Temp {}'.format(int(specs_evolution.temperature)), 
              'color':None, 
              'values':[{'x':x,  'y':y} for x, y in zip(specs_evolution.discharge.x, specs_evolution.discharge.y)]}])
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'specs_evolutions':[se.to_dict() for se in self.specs_evolutions],  'title':self.title})
        return dict_
    ...
    def DictToObject(cls, dict_):
        ...
        """
        if dict_ != None:
            li_specs_evolution = [SpecsEvolution.dict_to_object(se) for se in dict_['specs_evolutions']]
            return cls(specs_evolutions=li_specs_evolution, title=(dict_['title']),
              name=(dict_['name']))
...
class Cell(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'rated_capacity', 'ocv_specs', 'rint_specs', 'limits_voltage',
      'limits_current', 'limits_soc', 'charge_specs', 'size', 'mass',
      'thermal_transfert_specs'], 
     'properties':{'rated_capacity':{'type':'number', 
       ...
       'unit':'Ah', 
       'order':1}, 
      'ocv_specs':{'type':'object', 
       'title':'OCV Specifications', 
       'classes':[
        'powerpack.electrical.CombinationSpecsEvolution'], 
       'unit':'V', 
       'description':'OCV specification', 
       'editable':True, 
       'order':2}, 
      'rint_specs':{'type':'object', 
      ...
      'mass':{'type':'number', 
       ...
       'examples':[
        1.2]}, 
      'thermal_transfert_specs':{'type':'array', 
    ...
    def __init__(self, rated_capacity, ocv_specs, rint_specs, limits_voltage, limits_current, limits_soc, charge_specs, size, mass, thermal_transfert_specs, name=''):
    ...
    def __eq__(self, other_cell):
        equal = npy.allclose(self.size, other_cell.size) and self.mass == other_cell.mass and self.rated_capacity == other_cell.rated_capacity and self.ocv_specs == other_cell.ocv_specs and self.rint_specs == other_cell.rint_specs and self.charge_specs == other_cell.charge_specs
    ...
    def __hash__(self):
        size_hash = int(hash(sum(self.size)) % 100000.0)
    ...
    def _display_angular(self):
        ...
        displays += [{'angular_component':'plot_data',  'data':self.concept_plot_data()[0]},
         {'angular_component':'plot_data', 
          'data':self.iso_plot_data()}]
    ...
    def genere_interpolate(self):
        ...
        return (
         f_ocv, f_rint, f_charger, f_thermal_trans)
            ...
            if power < 0:
                if i1 < 0 and i1 >= i_min and i1 <= i_max:
                    ...
                    if i2 < i_min:
                        if p_max > 0:
                            i_optim = i_min
                    elif abs(power - p_max) < abs(power - p_min):
                        i_optim = i_max
                    else:
                        i_optim = i_min
                else:
    ...
    def Eval(self, soc, temperature, usecase, specs, f_specs):
        ...
        evaluate = float(interpolate.splev(soc / self.rated_capacity, f_specs[temperature_interp][usecase]))
    ...
    def plot_data(self, frame=vm.OXYZ, stroke_width=1):
        ...
        plot_frame = vm.Frame3D(frame.origin, size_xyz[0] * vm.X3D, size_xyz[1] * vm.Y3D, size_xyz[2] * vm.Z3D)
    ...
    def concept_plot_data_elem(self, detail, blocks, p_init_global, p_end_global, color_box):
        ...
        for block in blocks:
            ...
            plot_datas.append(vm.Circle2D(vm.Point2D((p_block_init.vector[0], p_block_init.vector[1])), diam_point).plot_data(color='black', fill='black'))
            plot_datas.append(vm.Circle2D(vm.Point2D((p_block_init_real.vector[0], p_block_init_real.vector[1])), diam_point).plot_data(color='black', fill='black'))
            for num_line, items_line in block.items():
                nb_elem = len(items_line)
                length_elem = length_block_real / nb_elem
                if nb_line > 1:
                    y_init = length_block_real / 2.0
            ...
            plot_datas.append(vm.Circle2D(vm.Point2D((p_block_end_real.vector[0], p_block_end_real.vector[1])), diam_point).plot_data(color='black', fill='black'))
            plot_datas.append(vm.Circle2D(vm.Point2D((p_block_end.vector[0], p_block_end.vector[1])), diam_point).plot_data(color='black', fill='black'))
    ...
    def CADVolume(self, frame):
        ...
        """
        volume = primitives3D.ExtrudedProfile((frame.origin - frame.u * self.size[0] / 2), (frame.v),
          (frame.w),
          (self.CADContour()),
          [], (frame.u * self.size[0]),
          name='Cell')
    ...
    def volume_model(self):
        frame = vm.Frame3D(vm.Point3D((0, 0, 0)), vm.Vector3D((1, 0, 0)), vm.Vector3D((0,
                                                                                       1,
                                                                                       0)), vm.Vector3D((0,
                                                                                                         0,
                                                                                                         1)))
    ...
    def Copy(self):
        ...
        """
        cell = Cell(self.rated_capacity, self.ocv_specs, self.rint_specs, self.limits_voltage, self.limits_current, self.limits_soc, self.charge_specs, self.size, self.mass, self.thermal_transfert_specs)
        return cell
    ...
    def to_dict(self):
        ...
        dict_.update({'rated_capacity':self.rated_capacity,  'limits_voltage':self.limits_voltage, 
    ...
    def dict_to_object(cls, dict_):
        ...
        if dict_['class_name'] == 'Cell':
            thermal_transfert_specs = [CombinationEvolution.dict_to_object(spec) for spec in dict_['thermal_transfert_specs']]
            cell = cls(rated_capacity=(dict_['rated_capacity']), ocv_specs=(CombinationSpecsEvolution.DictToObject(dict_['ocv_specs'])),
...
class CellRC(Cell):
    """CellRC"""
    _standalone_in_db = True
    _jsonschema = dict_merge(Cell._jsonschema, {'title':'powerpack.electrical.CellRC Base Schema', 
     'required':[
      'rint1_specs', 'c1_specs'], 
     'properties':{'rint1_specs':{'type':'object', 
    ...
    def __init__(self, rated_capacity, ocv_specs, rint_specs, rint1_specs, c1_specs, limits_voltage, limits_current, limits_soc, charge_specs, size, mass, thermal_transfert_specs, name=''):
        Cell.__init__(self, rated_capacity=rated_capacity, ocv_specs=ocv_specs, rint_specs=rint_specs,
          limits_voltage=limits_voltage,
          limits_current=limits_current,
          limits_soc=limits_soc,
    ...
    def __eq__(self, other_cell):
        equal = Cell.__eq__(self, other_cell) and self.rint1_specs == other_cell.rint1_specs and self.c1_specs == other_cell.c1_specs
        return equal

    def __hash__(self):
        cell_hash = Cell.__hash__(self)
        specs_hash = hash(self.rint1_specs) + hash(self.c1_specs)
        return int((cell_hash + specs_hash) % 100000000.0)
    ...
    def to_dict(self):
        ...
        dict_.update({'rint1_specs':self.rint1_specs.Dict(),  'c1_specs':self.c1_specs.Dict(), 
         'class_name':self.class_name})
        return dict_
...
class Cell2RC(CellRC):
    """Cell2RC"""
    _standalone_in_db = True
    _jsonschema = dict_merge(CellRC._jsonschema, {'title':'powerpack.electrical.Cell2RC Base Schema', 
     'required':[
      'rint2_specs', 'c2_specs'], 
     'properties':{'rint2_specs':{'type':'object', 
    ...
    def __init__(self, rated_capacity, ocv_specs, rint_specs, rint1_specs, c1_specs, rint2_specs, c2_specs, limits_voltage, limits_current, limits_soc, charge_specs, size, mass, thermal_transfert_specs, name=''):
        CellRC.__init__(self, rated_capacity=rated_capacity, ocv_specs=ocv_specs, rint_specs=rint_specs,
          rint1_specs=rint1_specs,
          c1_specs=c1_specs,
          limits_voltage=limits_voltage,
          limits_current=limits_current,
          limits_soc=limits_soc,
    ...
    def __eq__(self, other_cell):
        equal = CellRC.__eq__(self, other_cell) and self.rint2_specs == other_cell.rint2_specs and self.c2_specs == other_cell.c2_specs
        return equal
    ...
    def Voltage(self, current, usecase, delta_t, temperature, internal_parameter_cell):
        ...
        i_rint1 = min(i_rint1, self.limits_current[usecase]['maximum'])
        u_rint1 = rint1 * current_rint1_m
        i_rint2 = math.exp(-delta_t / (rint2 * c2)) * current_rint2_m + (1 - math.exp(-delta_t / (rint2 * c2))) * current
    ...
    def to_dict(self):
        ...
        dict_.update({'rint2_specs':self.rint2_specs.Dict(),  'c2_specs':self.c2_specs.Dict(), 
         'class_name':self.class_name})
        return dict_
...
class CellManagementSystem(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'cell'], 
     'properties':{'cell': {'type':'object', 
               'title':'Cell', 
               'classes':[
                'powerpack.electrical.Cell',
    ...
    def _display_angular(self):
        ...
        displays.remove({'angular_component': 'app-cad-viewer'})
        return displays
    ...
    def Ineq(self, soc_m, temperature, usecase, charger, delta_t, internal_parameter_cell):
        ...
        if charger:
            ...
            i_min = i_charger
            i_max = i_charger
        u_min = cell.Voltage(i_min, usecase, delta_t, temperature, internal_parameter_cell)
        u_max = cell.Voltage(i_max, usecase, delta_t, temperature, internal_parameter_cell)
        return (
         (
    ...
    def update(self, current, soc_m, temperature, usecase, charger, delta_t, results, internal_parameter_cell):
        ...
        """
        cell = self.cell
        voltage = cell.Voltage(current, usecase, delta_t, temperature, internal_parameter_cell)
        results['cms'].Add(voltage, current, temperature, delta_t, internal_parameter_cell)
        soc = internal_parameter_cell.soc
        thermal_loss = internal_parameter_cell.thermal_loss
        return (
         current, voltage, soc, thermal_loss)
...
class ElecModule(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'cms', 'number_cell_serie', 'number_cell_parallel'], 
     'properties':{'cms':{'type':'object', 
       'title':'Cell Management System', 
       'classes':[
        'powerpack.electrical.CellManagementSystem'], 
       'description':'Cell management system', 
       'editable':True, 
       'order':3}, 
      'number_cell_serie':{'type':'number', 
       ...
       'order':1}, 
      'number_cell_parallel':{'type':'number', 
    ...
    def __eq__(self, other_module):
        ...
        equal = self.number_cell_serie == other_module.number_cell_serie and self.number_cell_parallel == other_module.number_cell_parallel and cell == other_cell
        return equal

    def __hash__(self):
        module_hash = int(hash(self.number_cell_serie) + hash(self.number_cell_parallel) + hash(self.cms.cell) % 4500000.0)
        return module_hash

    def _display_angular(self):
        displays = [
         {'angular_component':'app-d3-plot-data', 
    ...
    def Copy(self):
        ...
        """
        module = ElecModule(self.cms, self.number_cell_serie, self.number_cell_parallel)
        return module
    ...
    def concept_plot_data(self, detail=True, p_init_global=vm.Point2D((0, 0)), p_end_global=vm.Point2D((1, 0)), color_box='red'):
        ...
        for block in blocks:
            ...
            for num_line, items_line in block.items():
                nb_elem = len(items_line)
                length_elem = length_block_real / nb_elem
                plot_data_elem, sup_y_elem, inf_y_elem = self.cms.cell.concept_plot_data(False, vm.Point2D((0,
                                                                                                            0)), vm.Point2D((length_elem, 0)))
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'cms':self.cms.Dict(),  'number_cell_serie':int(self.number_cell_serie), 
    ...
    def DictToObject(cls, dict_):
        ...
        elec_module = cls(cms=cms, number_cell_serie=(dict_['number_cell_serie']),
...
class ModuleManagementSystem(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'module', 'limits_voltage', 'limits_current'], 
     'properties':{'module':{'type':'object', 
       'title':'Electrical Module', 
       'classes':[
        'powerpack.electrical.ElecModule'], 
       'description':'Module', 
       'editable':True, 
       'order':3}, 
      'limits_voltage':{'type':'object', 
    ...
    def __eq__(self, other):
        valid = self.module == other.module and self.limits_voltage == other.limits_voltage and self.limits_current == other.limits_current and self.number_cell_serie == other.number_cell_serie and self.number_cell_parallel == other.number_cell_parallel
        return valid

    def __hash__(self):
        mms_hash = hash(self.module) + 7 * self.number_cell_serie + 13 * self.number_cell_parallel
        return mms_hash
    ...
    def Ineq(self, soc_m, temperature, usecase, charger, delta_t, internal_parameter_cell):
        ...
        """
        module = self.module
        (i_min_cell, i_max_cell), (u_min_cell, u_max_cell) = module.cms.Ineq(soc_m / self.number_cells, temperature, usecase, charger, delta_t, internal_parameter_cell)
    ...
    def update(self, current, soc_m, temperature, usecase, charger, delta_t, results, internal_parameter_cell):
        ...
        current_cell, voltage_cell, soc_cell, thermal_loss_cell = module.cms.update(current_cell, soc_m / self.number_cells, temperature, usecase, charger, delta_t, results, internal_parameter_cell)
        ...
        return (
         current, voltage, soc_p, thermal_loss_p)
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'module':self.module.Dict(),  'limits_voltage':self.limits_voltage, 
         'limits_current':self.limits_current})
        return dict_
    ...
    def DictToObject(cls, dict_):
        ...
        mms = cls(module=module, limits_voltage=(dict_['limits_voltage']),
          limits_current=(dict_['limits_current']),
          name=(dict_['name']))
        return mms
...
class ElecBattery(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'mms', 'number_module_serie', 'number_module_parallel'], 
     'properties':{'mms':{'type':'object', 
       'title':'Module Management System', 
       'classes':[
        'powerpack.electrical.ModuleManagementSystem'], 
       'description':'Module management system', 
       'editable':True, 
       'order':3}, 
      'number_module_serie':{'type':'number', 
       ...
       'order':2}, 
      'number_module_parallel':{'type':'number', 
    ...
    def __eq__(self, other_eb):
        ...
        equal = self.number_module_serie == other_eb.number_module_serie and self.number_module_parallel == other_eb.number_module_parallel and self.rated_capacity == other_eb.rated_capacity and module == other_module
        return equal

    def __hash__(self):
        eb_hash = hash(self.number_module_serie) + hash(self.number_module_parallel) + hash(self.rated_capacity) + hash(self.mms.module)
        return eb_hash
    ...
    def Copy(self):
        ...
        Returns a copy of a module object
        """
        battery = ElecBattery(self.mms, self.number_module_serie, self.number_module_parallel)
        return battery
    ...
    def concept_plot_data(self, p_init_global=vm.Point2D((0, 0)), p_end_global=vm.Point2D((1, 0)), color_box='green'):
        ...
        for block in blocks:
            ...
            for num_line, items_line in block.items():
                nb_elem = len(items_line)
                length_elem = length_block_real / nb_elem
                plot_data_elem, sup_y_elem, inf_y_elem = self.mms.module.concept_plot_data(False, vm.Point2D((0,
                                                                                                              0)), vm.Point2D((length_elem, 0)))
                high_line = max(high_line, sup_y_elem - inf_y_elem)
        ...
        plot_datas.append(vm.LineSegment2D(p3_box, p4_box).plot_data(color=color_box))
        return (
         plot_datas, sup_y + delta_box * length_global, inf_y - delta_box * length_global)
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'mms':self.mms.Dict(),  'number_module_serie':self.number_module_serie, 
    ...
    def DictToObject(cls, dict_):
        ...
        elec_battery = cls(mms=mms, number_module_serie=(dict_['number_module_serie']),
...
class BatteryManagementSystem(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'battery', 'limits_voltage', 'limits_current'], 
     'properties':{'battery':{'type':'object', 
       'title':'Electrical Battery', 
       'classes':[
        'powerpack.electrical.ElecBattery'], 
       'description':'Battery', 
       'editable':True, 
       'order':3}, 
      'limits_voltage':{'type':'object', 
    ...
    def Ineq(self, soc_m, temperature, usecase, charger, delta_t, internal_parameter_cell):
        ...
        (i_min_module, i_max_module), (u_min_module, u_max_module) = battery.mms.Ineq(soc_m / self.number_modules, temperature, usecase, charger, delta_t, internal_parameter_cell)
    ...
    def update(self, current, soc_m, temperature, usecase, charger, delta_t, results, internal_parameter_cell):
        ...
        current_module, voltage_module, soc_module, thermal_loss_module = battery.mms.update(current_module, soc_m / self.number_modules, temperature, usecase, charger, delta_t, results, internal_parameter_cell)
        ...
        results['bms'].Add(soc_p, voltage, current, temperature, thermal_loss_p, delta_t)
        return (
         current, voltage, soc_p, thermal_loss_p)
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'battery':self.battery.Dict(),  'limits_voltage':self.limits_voltage, 
         'limits_current':self.limits_current})
        return dict_
    ...
    def DictToObject(cls, dict_):
        ...
        bms = cls(battery=battery, limits_voltage=(dict_['limits_voltage']),
          limits_current=(dict_['limits_current']),
          name=(dict_['name']))
        return bms
...
class ElecBatteryResult(DessiaObject):
    ...
    _standalone_in_db = True
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     'definitions':{},  'title':'powerpack.electrical.ElecBatteryResult Base Schema', 
     'type':'object', 
     'properties':{'soc':{'type':'object', 
       'title':'SoC Evolution', 
       'classes':[
        'powerpack.electrical.Evolution'], 
    ...
    def __init__(self, soc=None, voltage=None, current=None, temperature=None, thermal_loss=None, time=None, name=''):
        if soc is None:
            self.soc = Evolution()
        else:
            self.soc = soc
    ...
    def __eq__(self, other):
        equal = self.soc == other.soc and self.voltage == other.voltage and self.current == other.current and self.temperature == other.temperature and self.thermal_loss == other.thermal_loss and self.time == other.time
        return equal

    def __hash__(self):
        li_hash = hash(self.soc) + hash(self.voltage) + hash(self.current) + hash(self.temperature) + hash(self.thermal_loss) + hash(self.time)
        return li_hash
    ...
    def DictToObject(cls, dict_):
        ...
        battery_result = cls(soc=soc, voltage=voltage, current=current, temperature=temperature,
...
class ElecModuleResult(DessiaObject):
    ...
    _standalone_in_db = True
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     'definitions':{},  'title':'powerpack.electrical.ElecModuleResult Base Schema', 
     'type':'object', 
     'properties':{'soc':{'type':'object', 
       'title':'SoC Evolution', 
       'classes':[
    ...
    def DictToObject(cls, dict_):
        ...
        if 'time' in dict_:
            if dict_['time'] is not None:
                time = Evolution.DictToObject(dict_['time'])
        module_result = cls(soc=soc, voltage=voltage, current=current, temperature=temperature,
          thermal_loss=thermal_loss,
          time=time,
          name=(dict_['name']))
...
class CellResult(DessiaObject):
    ...
    _standalone_in_db = True
    _jsonschema = {'$schema':'http://json-schema.org/draft-07/schema#', 
     'definitions':{},  'title':'powerpack.electrical.CellResult Base Schema', 
     'type':'object', 
     'properties':{'soc':{'type':'object', 
       'title':'SoC Evolution', 
       'classes':[
    ...
    def __init__(self, soc=None, voltage=None, current=None, ocv=None, rint=None, temperature=None, thermal_loss=None, time=None, rint1=None, c1=None, current_rint1=None, rint2=None, c2=None, current_rint2=None, name=''):
        if soc is None:
            self.soc = Evolution()
        else:
            self.soc = soc
    ...
    def __eq__(self, other):
        equal = self.soc == other.soc and self.voltage == other.voltage and self.current == other.current and self.temperature == other.temperature and self.thermal_loss == other.thermal_loss and self.time == other.time and self.ocv == other.ocv and self.rint == other.rint
    ...
    def __hash__(self):
        li_hash = hash(self.soc) + hash(self.voltage) + hash(self.current) + hash(self.temperature) + hash(self.thermal_loss) + hash(self.time) + hash(self.ocv) + hash(self.rint)
        if hasattr(self, 'rint1'):
    ...
    def Add(self, voltage, current, temperature, delta_t, internal_parameter_cell):
        """
    ...
    def DictToObject(cls, dict_):
        ...
        cell_result = cls(soc=soc, voltage=voltage, current=current, ocv=ocv,
          rint=rint,
          ...
          rint1=rint1,
          c1=c1,
          current_rint1=current_rint1,
          rint2=rint2,
          c2=c2,
          current_rint2=current_rint2,
          name=(dict_['name']))
        return cell_result
            ...
            for profile in combination_profil.power_profiles:
                ...
                for indice, (current, time) in enumerate(zip(profile.evolutions[1:], profile.times[1:])):
                    ...
                    if current > 0:
                        ...
                        usecase = 'discharge'
                    (i_min, i_max), (u_min, u_max) = self.cms.Ineq(soc_m, 298.15, usecase, False, delta_t, internal_parameter_cell)
                    if i_max < i_min:
                        valid = False
                        break
                    if u_max < u_min:
                    ...
                    p_tuple = self.cms.update(current_new, soc_m, 298.15, usecase,
                      charger=False, delta_t=delta_t,
                      results=results,
...
class PowerPackElectricSimulator(DessiaObject):
    ...
    _jsonschema = {'definitions':{},  '$schema':'http://json-schema.org/draft-07/schema#', 
     ...
     'required':[
      'bms', 'combination_profils'], 
     'properties':{'bms':{'type':'object', 
       'title':'Battery Management System', 
       'classes':[
        'powerpack.electrical.BatteryManagementSystem'], 
      ...
      'analysis':{'type':'object', 
       ...
       'classes':[
        'powerpack.electrical.ElecAnalysis'], 
       'description':'PowerPack Analysis', 
       'editable':False}, 
      'battery_results':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.ElecBatteryResult']}}, 
      'module_results':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.ElecModuleResult']}}, 
      'cell_results':{'type':'array', 
       'title':'Cells Results', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.CellResult']}}, 
      'combination_profils':{'type':'array', 
       ...
       'items':{'type':'object', 
        'classes':[
         'powerpack.electrical.CombinationPowerProfile']}}}}

    def __init__(self, bms, combination_profils, battery_results=None, module_results=None, cell_results=None, analysis=None, name=''):
        self.bms = bms
    ...
    def __hash__(self):
        ...
        return int(li_hash % 100000000.0)
    ...
    def Simulate(self, progress_callback=lambda x: 0):
        ...
        for num_result, combination_profil in enumerate(self.combination_profils):
            ...
            for profil_i in combination_profil.power_profiles:
                valid_profil_i = self.SimulateElem(profil=profil_i, battery_results=(self.battery_results[num_result]),
    ...
    def _analysis(self):
        ...
        electric_range = number_iter / 1478.0 * 14.6
        load_time = len(self.battery_results[1].time.evolution) / 10.0
        ...
        self.analysis = ElecAnalysis(electric_range, load_time, number_cells, number_modules, voltage_max, voltage_min, voltage_mean, current_max, current_min, current_mean)
        self.remove()
    ...
    def SimulateElem(self, profil, battery_results, module_results, cell_results):
        result = {'bms':battery_results, 
         'mms':module_results,  'cms':cell_results}
        valid = True
        valid_profil = True
        ...
        while valid:
            for indice, (power, time) in enumerate(zip(profil.evolutions[1:], profil.times[1:])):
                ...
                if power > 0:
                    ...
                    usecase = 'discharge'
                temperature = 298.15
                (i_min, i_max), (u_min, u_max) = self.bms.Ineq(soc_m, temperature, usecase, profil.charger, delta_t, internal_parameter_cell)
                if u_min > u_max:
                    valid_profil = False
                    valid = False
                    break
                ...
                if p_max < p:
                    p = min(p_max, p)
                try:
                    current = self.bms.CurrentObjective(p, (i_min, i_max), (u_min, u_max), internal_parameter_cell)
                except CurrentObjectiveError:
                    valid = False
                    valid_profil = False
                    break

                current, voltage, soc_p, thermal_loss_p = self.bms.update(current, soc_m, temperature, usecase, profil.charger, delta_t, result, internal_parameter_cell)
                ...
                if profil.power_accuracy is not None:
                    if delta_power > profil.power_accuracy * (abs(power) + 100.0):
                        valid = False
                        valid_profil = False
                        break
            ...
            number_loop += 1
            if profil.loop:
                if profil.max_loop is not None and profil.max_loop < number_loop:
                    valid = False
                    valid_profil = False
            else:
                valid = False
    ...
    def Dict(self):
        ...
        """
        dict_ = DessiaObject.base_dict(self)
        dict_.update({'bms':self.bms.Dict(),  'combination_profils':[p.to_dict() for p in self.combination_profils]})
        if self.battery_results is not None:
    ...
    def DictToObject(cls, dict_):
        ...
        if 'battery_results' in dict_:
            if dict_['battery_results'] is not None:
                battery_results = [ElecBatteryResult.DictToObject(case_result) for case_result in dict_['battery_results']]
        module_results = None
        if 'module_results' in dict_:
            if dict_['module_results'] is not None:
                module_results = [ElecModuleResult.DictToObject(case_result) for case_result in dict_['module_results']]
        cell_results = None
        if 'cell_results' in dict_:
            if dict_['cell_results'] is not None:
                cell_results = [CellResult.DictToObject(case_result) for case_result in dict_['cell_results']]
        profils = [CombinationPowerProfile.DictToObject(combination_profil) for combination_profil in dict_['combination_profils']]
        analysis = None
        if 'analysis' in dict_:
            analysis = ElecAnalysis.dict_to_object(dict_['analysis'])
        powerpack_simu = cls(bms=bms, combination_profils=profils,
...
class InternalParameterCell(DessiaObject):
    ...
    def __init__(self, soc_m=None, soc=None, thermal_loss=None, ocv=None, rint=None, rint1=None, c1=None, current_rint1=None, rint2=None, c2=None, current_rint2=None, current_rint1_m=0, current_rint2_m=0):
        self.soc_m = soc_m
        self.soc = soc
        self.thermal_loss = thermal_loss
        self.ocv = ocv
...
def GenerateElecBatteries(batteries_combinations, cell, electrical_limits, detail=False):
    """
        ...
        if detail:
            ...
            for _ in range(batteries_combination['battery'][0]):
                ...
                for _ in range(batteries_combination['battery'][1]):
                    ...
                    elec_modules.append(ElecModule(elec_cells, position_cells, electrical_limits['voltage_module'], electrical_limits['current_module'], detail))
                    count_cells += 1
            ...
            position_cms = [
             [
              0] * batteries_combination['module'][1]] * batteries_combination['module'][0]
            position_mms = [[0] * batteries_combination['battery'][1]] * batteries_combination['battery'][0]
            cms = CellManagementSystem([cell])
            elec_modules = [
             ElecModule([cms], position_cms, electrical_limits['voltage_module'], electrical_limits['current_module'], detail)]
            mms = ModuleManagementSystem(elec_modules)
        elec_battery = ElecBattery([mms], position_mms, electrical_limits['voltage_battery'], electrical_limits['current_battery'], detail)
