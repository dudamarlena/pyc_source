...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\Equation\core.py
# Compiled at: 2014-07-06 23:56:21
...
class ExpressionValue(ExpressionObject):
    ...
    def toStr(self, args, expression):
        if isinstance(self.value, complex):
            V = [
             self.value.real, self.value.imag]
            E = [0, 0]
            B = [0, 0]
            out = ['', '']
            for i in xrange(2):
...
class Expression(object):
    ...
    def __init__(self, expression, argorder=[], *args, **kwargs):
        ...
        if isinstance(expression, type(self)):
            ...
            self.__vars = dict(expression.__vars)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(self.__expr) == 0:
            ...
            if len(args) > len(self.__args):
                raise TypeError(('<{0:s}.{1:s}({2:s}) object at {3:0=#10x}>() takes at most {4:d} arguments ({5:d} given)').format(type(self).__module__, type(self).__name__, repr(self), id(self), len(self.__args), len(args)))
            for i in xrange(len(args)):
                if i < len(self.__args):
                    if self.__args[i] in kwargs:
                        raise TypeError(("<{0:s}.{1:s}({2:s}) object at {3:0=#10x}>() got multiple values for keyword argument '{4:s}'").format(type(self).__module__, type(self).__name__, repr(self), id(self), self.__args[i]))
            ...
            for arg in self.__argsused:
                if arg not in self.variables:
                    ...
                    raise TypeError(("<{0:s}.{1:s}({2:s}) object at {3:0=#10x}>() takes at least {4:d} arguments ({5:d} given) '{6:s}' not defined").format(type(self).__module__, type(self).__name__, repr(self), id(self), min_args, len(args) + len(kwargs), arg))
    ...
    def __next(self, _Expression__expect_op):
        if __expect_op:
            ...
            if m != None:
                ...
                if g['dec']:
                    if g['ivalue']:
                        ...
                        if g['rexpoent'] or g['rvalue'].find('.') >= 0:
...
functions = {}
smatch = re.compile('\\s*,')
vmatch = re.compile('\\s*(?:(?P<oct>(?P<octsign>[+-]?)\\s*0o(?P<octvalue>[0-7]+))|(?P<hex>(?P<hexsign>[+-]?)\\s*0x(?P<hexvalue>[0-9a-fA-F]+))|(?P<bin>(?P<binsign>[+-]?)\\s*0b(?P<binvalue>[01]+))|(?P<dec>(?P<rsign>[+-]?)\\s*(?P<rvalue>(?:\\d+\\.\\d+|\\d+\\.|\\.\\d+|\\d+))(?:[Ee](?P<rexpoent>[+-]?\\d+))?(?:\\s*(?P<sep>(?(rvalue)\\+|))?\\s*(?P<isign>(?(rvalue)(?(sep)[+-]?|[+-])|[+-]?)?)\\s*(?P<ivalue>(?:\\d+\\.\\d+|\\d+\\.|\\.\\d+|\\d+))(?:[Ee](?P<iexpoent>[+-]?\\d+))?[ij])?))')
