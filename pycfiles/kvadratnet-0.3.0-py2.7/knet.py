...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.11-intel/egg/knet.py
# Compiled at: 2017-03-27 15:11:29
"""
Command line utils for kvadratnet.
"""
import os, sys, shutil, glob, argparse, kvadratnet
...
def run_organize(args):
    ...
    units = args.unit
    for unit in units:
        if unit not in kvadratnet.UNITS:
...
def main():
    ...
    rename = subparsers.add_parser('rename', help='Rename files with kvadranet-names in them, e.g. add a prefix\n                before the cell identifier. If a pre- or postfix is not specified,\n                everything but the kvadratnet identifier is stripped from the filename')
    rename.add_argument('filespec', help='Files to rename. Should be a globbing expression')
    rename.add_argument('--prefix', default='', help='Text before kvadratnet cell identifier, e.g. prefix_1km_6666_444.tif')
    rename.add_argument('--postfix', default='', help='Text after kvadratnet cell identifier, e.g. 1km_6666_444_postfix.tif')
    rename.add_argument('--verbose', '-v', action='store_true', help='Be verbose')
    rename.set_defaults(func=run_rename)
    organize = subparsers.add_parser('organize', help='Organize files into subfolders according to supplied\n                  list of tile units.')
    organize.add_argument('filespec', help='Files to move into subfolders. Globbing expression')
    organize.add_argument('unit', nargs='+', help='Unit-folders to subdivide files into. More than one unit can be supplied.')
    organize.add_argument('--verbose', '-v', action='store_true', help='Be verbose')
