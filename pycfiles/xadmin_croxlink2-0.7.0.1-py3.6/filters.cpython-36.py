...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/filters.py
# Compiled at: 2020-02-27 23:34:22
# Size of source mod 2**32: 22718 bytes
...
class BaseFilter(object):
    ...
    def get_context(self):
        return {'title':self.title, 
         'spec':self,  'form_params':self.form_params()}
...
class FieldFilterManager(object):
    ...
    def register(self, list_filter_class, take_priority=False):
        if take_priority:
            self._field_list_filters.insert(self._take_priority_index, list_filter_class)
    ...
    def create(self, field, request, params, model, admin_view, field_path):
        for list_filter_class in self._field_list_filters:
            if not list_filter_class.test(field, request, params, model, admin_view, field_path):
                continue
            return list_filter_class(field, request, params, model,
              admin_view, field_path=field_path)
...
class BooleanFieldListFilter(ListFieldFilter):
    ...
    def choices(self):
        for lookup, title in (
         ...
         (
          '0', _('No'))):
            yield {'selected':self.lookup_exact_val == lookup and not self.lookup_isnull_val, 
             'query_string':self.query_string({self.lookup_exact_name: lookup}, [
              self.lookup_isnull_name]), 
             'display':title}

        if isinstance(self.field, models.NullBooleanField):
            yield {'selected':self.lookup_isnull_val == 'True',  'query_string':self.query_string({self.lookup_isnull_name: 'True'}, [
...
class ChoicesFieldListFilter(ListFieldFilter):
    ...
    def choices(self):
        yield {'selected':self.lookup_exact_val is '', 
         'query_string':self.query_string({}, [self.lookup_exact_name]), 
         'display':_('All')}
        for lookup, title in self.field.flatchoices:
            yield {'selected':smart_text(lookup) == self.lookup_exact_val, 
             'query_string':self.query_string({self.lookup_exact_name: lookup}), 
             'display':title}
...
class NumberFieldListFilter(FieldFilter):
    ...
    lookup_formats = {'equal':'%s__exact',  'lt':'%s__lt',  'gt':'%s__gt',  'ne':'%s__ne', 
     'lte':'%s__lte',  'gte':'%s__gte'}
...
class DateFieldListFilter(ListFieldFilter):
    ...
    lookup_formats = {'since':'%s__gte',  'until':'%s__lt',  'year':'%s__year', 
     'month':'%s__month',  'day':'%s__day',  'isnull':'%s__isnull'}
    ...
    def __init__(self, field, request, params, model, admin_view, field_path):
        ...
        self.date_params = dict([(FILTER_PREFIX + k, v) for k, v in params.items() if k.startswith(self.field_generic)])
        super(DateFieldListFilter, self).__init__(field, request, params, model, admin_view, field_path)
        now = timezone.now()
    ...
    def get_context(self):
        ...
        context['choice_selected'] = bool(self.lookup_year_val) or bool(self.lookup_month_val) or bool(self.lookup_day_val)
        return context
...
class RelatedFieldSearchFilter(FieldFilter):
    ...
    def test(cls, field, request, params, model, admin_view, field_path):
        if not is_related_field2(field):
            ...
            return related_modeladmin and getattr(related_modeladmin, 'relfield_style', None) in ('fk-ajax',
                                                                                                  'fk-select')
...
class RelatedFieldListFilter(ListFieldFilter):
    ...
    def has_output(self):
        if is_related_field(self.field) and self.field.field.null or hasattr(self.field, 'remote_field') and self.field.null:
            extra = 1
        else:
            extra = 0
    ...
    def choices(self):
        yield {'selected':self.lookup_exact_val == '' and self.lookup_isnull_val is None, 
         'query_string':self.query_string({}, [
          self.lookup_exact_name, self.lookup_isnull_name]), 
         'display':'(%s)' % _('All')}
        for pk_val, val in self.lookup_choices:
            yield {'selected':self.lookup_exact_val == smart_text(pk_val), 
             'query_string':self.query_string({self.lookup_exact_name: pk_val}, [
              self.lookup_isnull_name]), 
             'display':val}

        if is_related_field(self.field) and self.field.field.null or hasattr(self.field, 'remote_field') and self.field.null:
            yield {'selected':self.lookup_isnull_val is False and self.lookup_exact_val == '',  'query_string':self.query_string({self.lookup_isnull_name: 'False'}, [
              self.lookup_exact_name]), 
             'display':'(%s)' % _('Has value')}
            yield {'selected':self.lookup_isnull_val is True, 
...
class MultiSelectFieldListFilter(ListFieldFilter):
    ...
    def __init__(self, field, request, params, model, model_admin, field_path, field_order_by=None, field_limit=None, sort_key=None, cache_config=None):
        ...
        if field_limit is not None:
            if type(field_limit) == int:
                if queryset.count() > field_limit:
    ...
    def choices(self):
        ...
        yield {'selected':len(self.lookup_in_val) == 0, 
         'query_string':self.query_string({}, [self.lookup_in_name]), 
         'display':_('All')}
        for val in self.lookup_choices:
            yield {'selected':smart_text(val) in self.lookup_in_val, 
...
class AllValuesFieldListFilter(ListFieldFilter):
    ...
    def __init__(self, field, request, params, model, admin_view, field_path):
        ...
        self.lookup_choices = queryset.distinct().order_by(field.name).values_list((field.name),
          flat=True)
        super(AllValuesFieldListFilter, self).__init__(field, request, params, model, admin_view, field_path)

    def choices(self):
        yield {'selected':self.lookup_exact_val is '' and self.lookup_isnull_val is '', 
        ...
        for val in self.lookup_choices:
            if val is None:
                ...
                val = smart_text(val)
                yield {'selected':self.lookup_exact_val == val, 
                 'query_string':self.query_string({self.lookup_exact_name: val}, [
                  self.lookup_isnull_name]), 
                 'display':val}

        if include_none:
            yield {'selected':bool(self.lookup_isnull_val),  'query_string':self.query_string({self.lookup_isnull_name: 'True'}, [
              self.lookup_exact_name]), 
             'display':EMPTY_CHANGELIST_VALUE}