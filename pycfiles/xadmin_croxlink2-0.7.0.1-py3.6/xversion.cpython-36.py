...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/xversion.py
# Compiled at: 2020-02-27 23:24:20
# Size of source mod 2**32: 24128 bytes
...
class ReversionPlugin(BaseAdminPlugin):
    ...
    def do_post(self, __):
        ...
        def _method():
            ...
            if isinstance(admin_view, CreateAdminView):
                ...
                if isinstance(admin_view, UpdateAdminView):
                    comment = _('Change version.')
                else:
                    if isinstance(admin_view, RevisionView):
                        comment = _('Revert version.')
                    else:
                        if isinstance(admin_view, RecoverView):
                            comment = _('Rercover version.')
                        else:
                            if isinstance(admin_view, DeleteAdminView):
                                comment = _('Deleted %(verbose_name)s.') % {'verbose_name': self.opts.verbose_name}
    ...
    def block_nav_btns(self, context, nodes):
        ...
        if obj:
            revisionlist_url = self.admin_view.model_admin_url('revisionlist', quote(obj.pk))
            nodes.append(mark_safe('<a href="%s" class="btn btn-default"><i class="fa fa-calendar"></i> <span>%s</span></a>' % (revisionlist_url, _('History'))))
...
class RecoverListView(BaseReversionView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return TemplateResponse(request, self.recover_list_template or self.get_template_list('views/recover_list.html'), context)
...
class RevisionListView(BaseReversionView):
    ...
    def get_context(self):
        ...
        opts = self.opts
        action_list = [{'revision':version.revision,  'url':self.model_admin_url('revision', quote(version.object_id), version.id),  'version':version} for version in self._reversion_order_version_queryset(Version.objects.get_for_object_reference(self.model, self.obj.pk).select_related('revision__user'))]
        context.update({'title':_('Change history: %s') % force_text(self.obj), 
    ...
    def get_response(self):
        context = self.get_context()
        return TemplateResponse(self.request, self.object_history_template or self.get_template_list('views/model_history.html'), context)
    ...
    def post(self, request, object_id, *args, **kwargs):
        ...
        if 'version_a' not in params or 'version_b' not in params:
            ...
            for f in self.opts.fields + self.opts.many_to_many:
                ...
                if type(value_a) is QuerySet:
                    if type(value_b) is QuerySet:
                        is_diff = list(value_a) != list(value_b)
                diffs.append((label,
                 detail_a.get_field_result(f.name).val, detail_b.get_field_result(f.name).val, is_diff))

            context = super(RevisionListView, self).get_context()
            context.update({'object':self.obj, 
             'opts':self.opts, 
             'version_a':version_a, 
             'version_b':version_b, 
             ...
             'diffs':diffs})
            return TemplateResponse(self.request, self.revision_diff_template or self.get_template_list('views/revision_diff.html'), context)
...
class BaseRevisionView(ModelFormAdminView):
    ...
    def get_form_datas(self):
        ...
        if self.request_method == 'post':
            datas.update({'data':self.request.POST, 
    ...
    def get_context(self):
        ...
        context.update({'object': self.org_obj})
        return context
...
class RevisionView(BaseRevisionView):
    ...
    def init_request(self, object_id, version_id):
        ...
        self.version = get_object_or_404(Version,
          pk=version_id, object_id=(smart_text(self.org_obj.pk)))
        self.prepare_form()
    ...
    def get_response(self):
        ...
        return TemplateResponse(self.request, form_template or self.get_template_list('views/revision_form.html'), context)

    @filter_hook
    def post_response(self):
        self.message_user(_('The %(model)s "%(name)s" was reverted successfully. You may edit it again below.') % {'model':force_text(self.opts.verbose_name), 
         'name':smart_text(self.new_obj)}, 'success')
...
class RecoverView(BaseRevisionView):
    ...
    def get_response(self):
        ...
        return TemplateResponse(self.request, form_template or self.get_template_list('views/recover_form.html'), context)

    @filter_hook
    def post_response(self):
        self.message_user(_('The %(model)s "%(name)s" was recovered successfully. You may edit it again below.') % {'model':force_text(self.opts.verbose_name), 
         'name':smart_text(self.new_obj)}, 'success')
        return HttpResponseRedirect(self.model_admin_url('change', self.new_obj.pk))
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, **kwargs):
        ...
        if not instance.pk:
            ...
            for field in self.fields:
                ...
                f_html = render_field(field, form, form_style, context, template_pack=template_pack,
                  attrs=(self.attrs))
...
class InlineRevisionPlugin(BaseAdminPlugin):
    ...
    def get_related_versions(self, obj, version, formset):
        ...
        related_versions = dict([(related_version.object_id, related_version) for related_version in revision_versions if ContentType.objects.get_for_id(related_version.content_type_id).model_class() == formset.model if smart_text(related_version.field_dict[fk_name]) == smart_text(object_id)])
...
site.register_modelview('^recover/$',
  RecoverListView, name='%s_%s_recoverlist')
site.register_modelview('^recover/([^/]+)/$',
  RecoverView, name='%s_%s_recover')
site.register_modelview('^([^/]+)/revision/$',
  RevisionListView, name='%s_%s_revisionlist')
site.register_modelview('^([^/]+)/revision/([^/]+)/$',
  RevisionView, name='%s_%s_revision')
