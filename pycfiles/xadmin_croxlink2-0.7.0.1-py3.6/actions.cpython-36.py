...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/actions.py
# Compiled at: 2020-04-17 03:41:33
# Size of source mod 2**32: 11994 bytes
...
class DeleteSelectedAction(BaseActionView):
    ...
    description = ugettext_lazy('Delete selected %(verbose_name_plural)s')
    ...
    def delete_models(self, queryset):
        ...
        if n:
            ...
            self.message_user(_('Successfully deleted %(count)d %(items)s.') % {'count':n, 
             'items':model_ngettext(self.opts, n)}, 'success')
    ...
    def do_action(self, queryset):
        ...
        if self.request.POST.get('post'):
            ...
            return TemplateResponse(self.request, self.delete_selected_confirmation_template or self.get_template_list('views/model_delete_selected_confirm.html'), context)
...
class ActionPlugin(BaseAdminPlugin):
    ...
    def get_list_display_links(self, list_display_links):
        if self.actions:
            if list_display_links:
                if len(list_display_links) == 1:
                    if list_display_links[0] == 'action_checkbox':
    ...
    def get_context(self, context):
        if self.actions:
            if self.admin_view.result_count:
                ...
                new_context = {'selection_note':_('0 of %(cnt)s selected') % {'cnt': len(av.result_list)}, 
    ...
    def get_actions(self):
        if self.actions is None:
            ...
            actions = OrderedDict([(name, (ac, name, desc, icon)) for ac, name, desc, icon in actions])
            return actions
    ...
    def get_action_choices(self):
        ...
        for ac, name, description, icon in self.actions.values():
            choice = (
             name, description % model_format_dict(self.opts), icon)
    ...
    def get_action(self, action):
        if isinstance(action, type) and issubclass(action, BaseActionView):
            if not action.has_perm(self.admin_view):
                return
            return (
             action, getattr(action, 'action_name'), getattr(action, 'description'), getattr(action, 'icon'))
        else:
            if callable(action):
                func = action
                action = action.__name__
            else:
                if hasattr(self.admin_view.__class__, action):
            ...
            return (
             func, action, description, getattr(func, 'icon', 'tasks'))
    ...
    def block_results_bottom(self, context, nodes):
        if self.actions:
            if self.admin_view.result_count:
                nodes.append(loader.render_to_string('xadmin/blocks/model_list.results_bottom.actions.html', context=(get_context_dict(context))))
