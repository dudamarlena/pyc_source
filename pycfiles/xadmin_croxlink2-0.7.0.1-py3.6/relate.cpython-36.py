...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/relate.py
# Compiled at: 2020-02-22 02:50:11
# Size of source mod 2**32: 9142 bytes
...
class RelateMenuPlugin(BaseAdminPlugin):
    ...
    def _get_all_related_objects(self, local_only=False, include_hidden=False, include_proxy_eq=False):
        ...
        fields = self.opts._get_fields(forward=False,
          reverse=True,
          include_parents=include_parents,
          include_hidden=include_hidden)
        if include_proxy_eq:
            children = chain.from_iterable(c._relation_tree for c in self.opts.concrete_model._meta.proxied_children if c is not self.opts)
            relations = (f.remote_field for f in children if include_hidden or not f.remote_field.field.remote_field.is_hidden())
        ...
        if hasattr(self, '_related_acts'):
            ...
            for rel in self._get_all_related_objects():
                if self.related_list:
                    if rel.get_accessor_name() not in self.related_list:
                        continue
    ...
    def related_link(self, instance):
        ...
        for rel, view_perm, add_perm in self.get_related_list():
            ...
            link = ''.join(('<li class="with_menu_btn">',
             '<a href="%s?%s=%s" title="%s"><i class="icon fa fa-th-list"></i> %s</a>' % (reverse('%s:%s_%s_changelist' % (self.admin_site.app_name, label, model_name)), RELATE_PREFIX + lookup_name, str(instance.pk), verbose_name, verbose_name) if view_perm else '<a><span class="text-muted"><i class="icon fa fa-blank"></i> %s</span></a>' % verbose_name,
             '<a class="add_link dropdown-menu-btn" href="%s?%s=%s"><i class="icon fa fa-plus pull-right"></i></a>' % (reverse('%s:%s_%s_add' % (self.admin_site.app_name, label, model_name)), RELATE_PREFIX + lookup_name, str(instance.pk)) if add_perm else '',
             '</li>'))
            links.append(link)
    ...
    def get_list_display(self, list_display):
        if self.use_related_menu:
            if len(self.get_related_list()):
...
class BaseRelateDisplayPlugin(BaseAdminPlugin):
    ...
    def _get_relate_params(self):
        return (
         RELATE_PREFIX + self.relate_obj.lookup, self.relate_obj.value)
...
class EditRelateDisplayPlugin(BaseRelateDisplayPlugin):
    ...
    def get_form_datas(self, datas):
        if self.admin_view.org_obj is None:
            if self.admin_view.request_method == 'get':
    ...
    def post_response(self, response):
        ...
        if isinstance(response, cls_str):
            if response != self.get_admin_url('index'):
