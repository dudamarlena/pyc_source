...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/details.py
...
class DetailsPlugin(BaseAdminPlugin):
    ...
    def result_item(self, item, obj, field_name, row):
        if self.show_all_rel_details or field_name in self.show_detail_fields:
            rel_obj = None
            if hasattr(item.field, 'remote_field'):
                if isinstance(item.field.remote_field, models.ManyToOneRel):
                    rel_obj = getattr(obj, field_name)
            if field_name in self.show_detail_fields:
                rel_obj = obj
            if rel_obj:
            ...
            if rel_obj and has_view_perm:
                opts = rel_obj._meta
                try:
                    item_res_uri = reverse(('%s:%s_%s_detail' % (self.admin_site.app_name,
                     opts.app_label, opts.model_name)),
                      args=(
                     getattr(rel_obj, opts.pk.attname),))
                    if item_res_uri:
                        if has_change_perm:
                            edit_url = reverse(('%s:%s_%s_change' % (self.admin_site.app_name, opts.app_label, opts.model_name)),
                              args=(
                             getattr(rel_obj, opts.pk.attname),))
                        else:
                            edit_url = ''
                        item.btns.append('<a data-res-uri="%s" data-edit-uri="%s?_redirect=%s" class="details-handler" rel="tooltip" title="%s"><i class="fa fa-info-circle"></i></a>' % (
                         item_res_uri, edit_url, self.admin_view.request.get_full_path(), _('Details of %s') % str(rel_obj)))
