...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/sortablelist.py
...
class SortableListPlugin(BaseAdminPlugin):
    ...
    def result_item(self, item, obj, field_name, row):
        if self.is_list_sortable:
            if field_name == self.list_order_field:
    ...
    def block_top_toolbar(self, context, nodes):
        save_node = render_to_string('xadmin/blocks/model_list.top_toolbar.saveorder.html',
...
site.register_modelview('^save-order/$', SaveOrderView, name='%s_%s_save_order')