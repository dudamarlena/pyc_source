...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/passwords.py
# Compiled at: 2020-02-22 03:09:44
...
class MyPasswordResetForm(PasswordResetForm):
    ...
    def get_users(self, email):
        ...
        """
        active_users = get_user_model()._default_manager.filter((Q(username__iexact=email) | Q(email__iexact=email)),
          is_active=True)
...
class ResetPasswordSendView(BaseAdminView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            opts = {'use_https':request.is_secure(),  'token_generator':self.password_reset_token_generator, 
...
site.register_view('^password_reset/$', ResetPasswordSendView, name='xadmin_password_reset')
...
class ResetPasswordConfirmView(BaseAdminView):
    ...
    def do_view(self, request, uidb36, token, *args, **kwargs):
        ...
        return password_reset_confirm(request, uidb36, token, template_name=(self.password_reset_confirm_template),
    ...
    def get_media(self):
        return super(ResetPasswordConfirmView, self).get_media() + self.vendor('xadmin.page.form.js', 'xadmin.form.css')


site.register_view('^password_reset/(?P<uidb36>[0-9A-Za-z]{1,13})-(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', ResetPasswordConfirmView,
...
site.register_view('^password_reset/complete/$', ResetPasswordCompleteView, name='xadmin_password_reset_complete')