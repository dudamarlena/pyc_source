...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/relfield.py
# Compiled at: 2020-02-27 23:21:59
...
class RelateFieldPlugin(BaseAdminPlugin):
    ...
    def get_field_style(self, attrs, db_field, style, **kwargs):
        if style in ('fk-ajax', 'fk-select') and isinstance(db_field, models.ForeignKey) and db_field.remote_field.to in self.admin_view.admin_site._registry and self.has_model_perm(db_field.remote_field.to, 'view'):
            db = kwargs.get('using')
            return dict((attrs or {}), widget=style == 'fk-ajax' and ForeignKeySearchWidget or ForeignKeySelectWidget((db_field.remote_field), (self.admin_view), using=db))
        else:
            return attrs
