...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/views/delete.py
# Compiled at: 2020-04-17 04:02:33
...
class DeleteAdminView(ModelAdminView):
    ...
    def get(self, request, object_id):
        ...
        return TemplateResponse(request, self.delete_confirmation_template or self.get_template_list('views/model_delete_confirm.html'), context)
    ...
    def get_context(self):
        if self.perms_needed or self.protected:
            title = _('Cannot delete %(name)s') % {'name': force_text(self.opts.verbose_name)}
        else:
            title = _('Are you sure?')
        new_context = {'title':title, 
         'object':self.obj, 
    ...
    def get_breadcrumb(self):
        ...
        bcs.append({'title':force_text(self.obj), 
    ...
    def post_response(self):
        self.message_user(_('The %(name)s "%(obj)s" was deleted successfully.') % {'name':force_text(self.opts.verbose_name), 
         'obj':force_text(self.obj)}, 'success')
        if self.request.POST.get('_redirect'):
            return self.request.POST['_redirect']
        if self.request.GET.get('_redirect'):
