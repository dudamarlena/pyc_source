...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/auth.py
# Compiled at: 2019-04-03 05:29:53
# Size of source mod 2**32: 10052 bytes
from django import forms
from django.contrib.auth.forms import UserCreationForm, UserChangeForm, AdminPasswordChangeForm, PasswordChangeForm, ReadOnlyPasswordHashWidget
...
class UserAdmin(object):
    ...
    def get_form_layout(self):
        if self.org_obj:
            self.form_layout = (
             Main(Fieldset('', 'username',
               'password', css_class='unsort no_title'), Fieldset(_('Personal info'), Row('first_name', 'last_name'), 'email'), Fieldset(_('Permissions'), 'groups', 'user_permissions'), Fieldset(_('Important dates'), 'last_login', 'date_joined')),
...
class UserFieldPlugin(BaseAdminPlugin):
    ...
    def get_field_attrs(self, __, db_field, **kwargs):
        if self.user_fields:
            if db_field.name in self.user_fields:
    ...
    def get_form_datas(self, datas):
        if self.user_fields:
            if 'data' in datas:
                if hasattr(datas['data'], '_mutable'):
                    if not datas['data']._mutable:
...
class ChangePasswordView(ModelAdminView):
    ...
    def get_context(self):
        ...
        context.update({'title':_('Change password: %s') % escape(smart_text(self.obj)), 
         'form':self.form, 
    ...
    def get_response(self):
        return TemplateResponse(self.request, [
         self.change_user_password_template or 'xadmin/auth/user/change_password.html'], self.get_context())
...
class ChangeAccountPasswordView(ChangePasswordView):
    ...
    def get_context(self):
        ...
        context.update({'title':_('Change password'), 
         'account_view':True})
        return context
...
site.register_view(('^%s/(.+)/password/$' % user_model), ChangePasswordView,
  name='user_change_password')
site.register_view('^account/password/$', ChangeAccountPasswordView, name='account_password')