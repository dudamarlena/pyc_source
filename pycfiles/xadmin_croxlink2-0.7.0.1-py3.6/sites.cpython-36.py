...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/sites.py
# Compiled at: 2020-02-22 05:02:40
...
class AdminSite(object):
    ...
    def copy_registry(self):
        import copy
        return {'models':copy.copy(self._registry), 
    ...
    def check_dependencies(self):
        ...
        if not ('django.contrib.auth.context_processors.auth' in default_template_engine.context_processors or 'django.core.context_processors.auth' in default_template_engine.context_processors):
    ...
    def admin_view(self, view, cacheable=False):
        ...
        def inner(request, *args, **kwargs):
            if not self.has_permission(request):
                if getattr(view, 'need_site_permission', True):
    ...
    def _get_merge_attrs(self, option_class, plugin_class):
        return dict([(name, getattr(option_class, name)) for name in dir(option_class) if name[0] != '_' if not callable(getattr(option_class, name)) if hasattr(plugin_class, name)])
    ...
    def _get_settings_class(self, admin_view_class):
        ...
        if name.endswith('admin'):
            if name[0:-5] in self._registry_settings:
                return self._registry_settings[name[0:-5]]
        if name.endswith('adminview'):
            if name[0:-9] in self._registry_settings:
    ...
    def get_urls(self):
        ...
        urlpatterns = [
         re_path('^jsi18n/$', wrap((self.i18n_javascript), cacheable=True), name='jsi18n')]
        urlpatterns += [re_path(url_path, wrap(self.create_admin_view(clz_or_func)) if (inspect.isclass(clz_or_func) and issubclass(clz_or_func, BaseAdminView)) else (include(clz_or_func(self))), name=name) for url_path, clz_or_func, name in self._registry_views]
        for model, admin_class in iteritems(self._registry):
            view_urls = [re_path(url_path, (wrap(self.create_model_admin_view(clz, model, admin_class))), name=(name % (model._meta.app_label, model._meta.model_name))) for url_path, clz, name in self._registry_modelviews]
            urlpatterns += [
             re_path('^%s/%s/' % (model._meta.app_label, model._meta.model_name), include(view_urls))]
