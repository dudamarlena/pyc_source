...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/multiselect.py
# Compiled at: 2018-09-06 00:02:27
    ...
    def render(self, name, value, attrs=None, choices=()):
        ...
        for option_value, option_label in chain(self.choices, choices):
            if isinstance(option_label, (list, tuple)):
                available_output.append('<optgroup label="%s">' % escape(force_text(option_value)))
...
class M2MSelectPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        return hasattr(self.admin_view, 'style_fields') and ('m2m_transfer' in self.admin_view.style_fields.values() or 'm2m_dropdown' in self.admin_view.style_fields.values())

    def get_field_style(self, attrs, db_field, style, **kwargs):
        if style == 'm2m_transfer':
            if isinstance(db_field, ManyToManyField):
                return {'widget':SelectMultipleTransfer(db_field.verbose_name, False), 
                 'help_text':''}
        if style == 'm2m_dropdown':
            if isinstance(db_field, ManyToManyField):
                return {'widget':SelectMultipleDropdown, 
                 'help_text':''}
        return attrs
