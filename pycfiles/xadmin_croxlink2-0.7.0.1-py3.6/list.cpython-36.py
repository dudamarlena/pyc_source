...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/views/list.py
# Compiled at: 2020-02-24 04:39:13
# Size of source mod 2**32: 28023 bytes
...
class ListAdminView(ModelAdminView):
    ...
    def get_list_display(self):
        ...
        if COL_LIST_VAR in self.request.GET:
            if self.request.GET[COL_LIST_VAR]:
                ...
                columns_in_url = None
            if self.request.is_ajax() or columns_in_url:
                self.request.session['LIST_DISPLAY_%s' % self.__class__.__name__] = columns_in_url
        else:
            if columns_in_url is None:
                try:
                    del self.request.session['LIST_DISPLAY_%s' % self.__class__.__name__]
                except:
                    pass

        self.base_list_display = self.request.session.get('LIST_DISPLAY_%s' % self.__class__.__name__) or columns_in_url or self.list_display
            ...
            if self.list_select_related:
                ...
                for field_name in self.list_display:
                    try:
                        ...
                        if isinstance(field.remote_field, models.ManyToOneRel) and '%s__%s__isnull' % (field.name, field.related_model._meta.pk.name) not in self.request.META['QUERY_STRING']:
    ...
    def get_ordering(self):
        ...
        if ORDER_VAR in self.params:
            if self.params[ORDER_VAR]:
                ordering = [pfx + (self.get_ordering_field(field_name) or field_name) for n, pfx, field_name in map(lambda p: p.rpartition('-'), self.params[ORDER_VAR].split('.')) if self.get_ordering_field(field_name) or '__' in field_name]
    ...
    def result_item(self, obj, field_name, row):
        ...
        if f is None:
            ...
            if item.text == EMPTY_CHANGELIST_VALUE:
                item.text = mark_safe("<span class='text-muted'>%s</span>" % EMPTY_CHANGELIST_VALUE)
            elif isinstance(f, models.DateField) or isinstance(f, models.TimeField) or isinstance(f, models.ForeignKey):
                item.classes.append('nowrap')
            elif item.text:
                if field_name in getattr(self, 'align_right_fields', []):
                ...
                if self.list_display_links is not None:
                    if item.row['is_display_first'] and self.list_display_links == () or field_name in self.list_display_links:
    ...
    def block_pagination(self, context, nodes, page_type='normal'):
        ...
        if not pagination_required:
            ...
            ON_EACH_SIDE = {'normal':5, 
             'small':3}.get(page_type, 3)
            ON_ENDS = 2
