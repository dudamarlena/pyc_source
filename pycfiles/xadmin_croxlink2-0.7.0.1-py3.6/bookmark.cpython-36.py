...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/bookmark.py
# Compiled at: 2020-02-22 02:50:11
# Size of source mod 2**32: 8680 bytes
...
class BookmarkPlugin(BaseAdminPlugin):
    ...
    def get_context(self, context):
        if not self.show_bookmarks:
            ...
            bookmarks = []
            current_qs = '&'.join(['%s=%s' % (k, v) for k, v in sorted(filter(lambda i: bool(i[1] and (i[0] in (COL_LIST_VAR, ORDER_VAR, SEARCH_VAR) or i[0].startswith(FILTER_PREFIX) or i[0].startswith(RELATE_PREFIX))), self.request.GET.items()))])
            model_info = (
            ...
            bookmarks_queryset = Bookmark.objects.filter(content_type=content_type,
              url_name=('xadmin:%s_%s_changelist' % model_info)).filter(Q(user=(self.user)) | Q(is_share=True))
            for bk in bookmarks_queryset:
                ...
                bookmarks.append({'title':bk.title,  'selected':selected,  'url':bk.url,  'edit_url':reverse('xadmin:%s_%s_%s' % (bk_model_info[0], bk_model_info[1], change_or_detail), args=(
                  bk.id,))})
                if selected:
                    menu_title = bk.title
                    has_selected = True
            ...
            new_context = {'bk_menu_title':menu_title, 
...
class BookmarkWidget(PartialBaseWidget):
    ...
    bookmark = ModelChoiceField(label=(_('Bookmark')),
    ...
    def setup(self):
        ...
        self.list_view = self.get_view_class(ListAdminView,
          model, list_per_page=10, list_editable=[])(req)
...
site.register_modelview('^bookmark/$', BookmarkView, name='%s_%s_bookmark')