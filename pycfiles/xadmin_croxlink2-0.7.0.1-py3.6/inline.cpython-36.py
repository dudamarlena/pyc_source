...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/inline.py
# Compiled at: 2020-02-27 23:13:02
# Size of source mod 2**32: 18350 bytes
import copy, inspect
...
class TableInlineStyle(InlineStyle):
    ...
    def get_attrs(self):
        ...
        return {'fields':fields,  'readonly_fields':readonly_fields}
    ...
    if layout:
        ...
        for i, lo in enumerate(layout.fields):
            if isinstance(lo, Field) or issubclass(lo.__class__, Field):
                ...
                if isinstance(lo, cls_str):
                    layout.fields[i] = ShowField(av, lo)
                else:
                    if hasattr(lo, 'get_field_names'):
...
class InlineModelAdmin(ModelFormAdminView):
    ...
    def instance_form(self, **kwargs):
        ...
        attrs = {'instance':self.model_instance, 
         'queryset':self.queryset()}
        if self.request_method == 'post':
            attrs.update({'data':self.request.POST, 
             'files':self.request.FILES,  'save_as_new':'_saveasnew' in self.request.POST})
        ...
        if opts.auto_created:
            for field in opts.fields:
                if field.remote_field:
                    if field.remote_field.to != self.parent_model:
...
class GenericInlineModelAdmin(InlineModelAdmin):
    ...
    def get_formset(self, **kwargs):
        ...
        exclude = exclude or None
        can_delete = self.can_delete and self.has_delete_permission()
        defaults = {'ct_field':self.ct_field, 
...
class InlineFormset(Fieldset):
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK, **kwargs):
        ...
        context.update(dict(formset=self, 
         prefix=self.formset.prefix, 
         inline_style=self.inline_style, **self.extra_attrs))
    ...
    def inline_instances(self):
        if not hasattr(self, '_inline_instances'):
            ...
            for inline_class in self.inlines:
                inline = self.admin_view.get_view(getattr(inline_class, 'generic_inline', False) and GenericInlineModelAdmin or InlineModelAdmin, inline_class).init(self.admin_view)
                if not (inline.has_add_permission() or inline.has_change_permission() or inline.has_delete_permission() or inline.has_view_permission()):
                    pass
                else:
...
class DetailInlineFormsetPlugin(InlineFormsetPlugin):
    ...
    def get_model_form(self, form, **kwargs):
        self.formsets = [self._get_detail_formset_instance(inline) for inline in self.inline_instances]
        return form
