...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/batch.py
...
class ChangeFieldWidgetWrapper(forms.Widget):
    ...
    def render(self, name, value, attrs=None):
        ...
        output.append('<label class="btn btn-info btn-xs"><input type="checkbox" class="batch-field-checkbox" name="%s" value="%s"%s/> %s</label>' % (
...
class BatchChangeAction(BaseActionView):
    ...
    def change_models(self, queryset, cleaned_data):
        ...
        if n:
            ...
            self.message_user(_('Successfully change %(count)d %(items)s.') % {'count':n, 
    ...
    def do_action(self, queryset):
        if not self.has_change_permission():
            ...
            if change_fields and self.request.POST.get('post'):
                self.form_obj = self.get_change_form(True, change_fields)(data=(self.request.POST),
            ...
            helper.add_layout(Layout(Container(Col('full', Fieldset('', *(self.form_obj.fields.keys()), **{'css_class': 'unsort no_title'}),
        ...
        context.update({'title':_('Batch change %s') % objects_name, 
        ...
        return TemplateResponse(self.request, self.batch_change_form_template or self.get_template_list('views/batch_change_form.html'), context)
