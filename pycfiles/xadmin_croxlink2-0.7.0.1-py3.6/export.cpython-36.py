...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/export.py
# Compiled at: 2019-04-19 09:44:45
    ...
    def _format_value(self, o):
        if o.field is None and getattr(o.attr, 'boolean', False) or o.field and isinstance(o.field, (BooleanField, NullBooleanField)):
            ...
            if str(o.text).startswith("<span class='text-muted'>"):
                ...
                if str(o.text).startswith("<div class='pull-right'>"):
    ...
    def get_result_list(self, __):
        if self.request.GET.get('all', 'off') == 'on':
            if self.request.GET.get('export_type') == 'xls':
                self.admin_view.list_per_page = 65536
            else:
                if self.request.GET.get('export_type') == 'xlsx':
                    self.admin_view.list_per_page = 1048576
                else:
