...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/editable.py
# Compiled at: 2020-02-27 23:15:29
# Size of source mod 2**32: 7217 bytes
...
class EditablePlugin(BaseAdminPlugin):
    ...
    def result_item(self, item, obj, field_name, row):
        if self.list_editable:
            if item.field:
                if item.field.editable:
                    if field_name in self.list_editable:
                        pk = getattr(obj, obj._meta.pk.attname)
                        field_label = label_for_field(field_name, obj, model_admin=(self.admin_view),
                          return_attr=False)
                        item.wraps.insert(0, '<span class="editable-field">%s</span>')
                        item.btns.append('<a class="editable-handler" title="%s" data-editable-field="%s" data-editable-loadurl="%s"><i class="fa fa-edit"></i></a>' % (
                         _('Enter %s') % field_label, field_name, self.admin_view.model_admin_url('patch', pk) + '?fields=%s&reload=%s' % (field_name, self.reload_after_ajax_success)))
    ...
    def get_media(self, media):
        if self.editable_need_fields:
            media = media + self.model_form.media + self.vendor('xadmin.plugin.editable.js', 'xadmin.widget.editable.css')
        return media
...
class EditPatchView(ModelFormAdminView, ListAdminView):
    ...
    def get_new_field_html(self, f):
        result = self.result_item(self.org_obj, f, {'is_display_first':False, 
         'object':self.org_obj})
        if result.allow_tags:
            return mark_safe(result.text)
        else:
            return conditional_escape(result.text)
    ...
    def get(self, request, object_id):
        ...
        form.helper = helper
        s = '{% load i18n crispy_forms_tags %}<form method="post" action="{{action_url}}">{% csrf_token %}{% crispy form %}<button type="submit" class="btn btn-success btn-block btn-sm">{% trans "Apply" %}</button></form>'
...
site.register_modelview('^(.+)/patch/$', EditPatchView, name='%s_%s_patch')