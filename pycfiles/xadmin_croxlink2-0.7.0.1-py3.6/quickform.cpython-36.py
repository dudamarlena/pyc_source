...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/quickform.py
# Compiled at: 2020-04-04 04:16:12
...
class QuickFormPlugin(BaseAdminPlugin):
    ...
    def get_model_form(self, __, **kwargs):
        if '_field' in self.request.GET:
            defaults = {'form':self.admin_view.form,  'fields':self.request.GET['_field'].split(','), 
...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    def render(self, name, value, renderer=None, *args, **kwargs):
        ...
        output = []
        if self.add_url:
            if getattr(self.widget, 'can_add_related', True):
                output.append('<a href="%s" title="%s" class="btn btn-primary btn-sm btn-ajax pull-right" data-for-id="id_%s" data-refresh-url="%s"><i class="fa fa-plus"></i></a>' % (
                 self.add_url,
        ...
        if formfield:
            if self.model in self.admin_site._registry:
                if isinstance(db_field, (models.ForeignKey, models.ManyToManyField)):
                    if not getattr(self.admin_view, 'wizard_form_list', None):
                        ...
                        if rel_model in self.admin_site._registry:
                            if self.has_model_perm(rel_model, 'add'):
                                ...
                                formfield.widget = RelatedFieldWidgetWrapper(formfield.widget, db_field.remote_field, add_url, self.get_model_url(self.model, 'add'))
        return formfield
