...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/ajax.py
# Compiled at: 2018-03-12 00:53:29
...
class AjaxListPlugin(BaseAjaxPlugin):
    ...
    def get_list_display(self, list_display):
        list_fields = [field for field in self.request.GET.get('_fields', '').split(',') if field.strip() != '']
        if list_fields:
            return list_fields
        else:
            return list_display
    ...
    def get_result_list(self, response):
        if self.request.GET.get('_format') == 'html':
            ...
            headers = dict([(c.field_name, force_text(c.text)) for c in av.result_headers().cells if c.field_name in base_fields])
            objects = [dict([(o.field_name, escape(str(o.value))) for i, o in enumerate(filter(lambda c: c.field_name in base_fields, r.cells))]) for r in av.results()]
