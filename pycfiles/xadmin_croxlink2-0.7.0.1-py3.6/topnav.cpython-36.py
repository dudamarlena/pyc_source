...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/plugins/topnav.py
        ...
        for model in models:
            ...
            if self.has_model_perm(model, 'view'):
                ...
                if getattr(self.admin_site._registry[model], 'search_fields', None):
                    try:
                        search_models.append({'title':_('Search %s') % capfirst(model._meta.verbose_name_plural), 
        ...
        for model in models:
            ...
            if self.has_model_perm(model, 'add'):
                info = (
                 app_label, model._meta.model_name)
                try:
                    add_models.append({'title':_('Add %s') % capfirst(model._meta.verbose_name), 
                     'url':reverse('xadmin:%s_%s_add' % info, current_app=site_name), 
                     'model':model})
                except NoReverseMatch:
                    pass

        nodes.append(loader.render_to_string('xadmin/blocks/comm.top.topnav.html', {'add_models': add_models}))
