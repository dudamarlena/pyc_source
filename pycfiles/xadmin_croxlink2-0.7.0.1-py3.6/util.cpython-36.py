...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/xadmin/util.py
# Compiled at: 2020-04-15 09:42:00
...
def xstatic(*tags):
    ...
    return [f.startswith(('http://', 'https://')) and f or static(f) for f in fs]
...
def lookup_needs_distinct(opts, lookup_path):
    ...
    if hasattr(field, 'remote_field') and isinstance(field.remote_field, models.ManyToManyRel) or is_related_field(field) and not field.field.unique:
        return True
    else:
        return False
...
def lookup_field(name, obj, model_admin=None):
    ...
    try:
        ...
        if callable(name):
            attr = name
            value = attr(obj)
        elif model_admin is not None and hasattr(model_admin, name) and name not in ('__str__',
                                                                                     '__unicode__'):
...
def get_limit_choices_to_from_path(model, path):
    ...
    limit_choices_to = fields and hasattr(fields[(-1)], 'remote_field') and getattr(fields[(-1)].remote_field, 'limit_choices_to', None)
