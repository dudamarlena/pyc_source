...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmsdk/products.py
...
class ProductAPIService(BaseConnection):
    ...
    def deactivate(self, product_number, client_store, version=VERSION):
        """
        Description:
        ...
        """
        self.method = 'PUT'
        self.config.uri = ('/pcm/api/cancel/{product_number}/').format(product_number=product_number)
        self.query_params = {'client_store': client_store, 
           'version': version}
    ...
    def list(self, product_numbers=[], client_sk=None, page=1):
        ...
        if client_sk:
            self.query_params.update(**{'client_sk': client_sk})
        if prods:
            self.query_params.update(**{'products': prods})
        response = self.execute()
        return response
    ...
    def detail(self, product_number, client_sk=None):
        ...
        """
        self.method = 'GET'
        self.config.uri = ('/pcm/products/{product_number}/').format(product_number=product_number)
        if client_sk:
