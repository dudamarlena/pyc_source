...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/gmsdk/orders.py
# Compiled at: 2015-04-06 14:46:45
import json
...
class OrderAPIService(BaseConnection):
    ...
    def create(self, client_store, fc, data=None, version=VERSION):
        ...
        if version not in self.AVAILABLE_VERSIONS:
            raise InvalidVersionError("Version-%s is not supported by Delhivery-Godam API's" % version)
        if not data:
            raise MissingDataError('No input found provided for creating orders ! Request can not proceed !')
        data = json.dumps(data)
        self.query_params = {'client_store': client_store, 
           'fc': fc, 
           'version': version}
        if version == '2014.09':
            self.config['headers'].update(**{'content-type': 'application/json'})
            self.query_params.update(**{'fulfillment_center': self.query_params.pop('fc')})
        else:
            data = {'data': data}
        self.method = 'POST'
    ...
    def cancel(self, client_store, fc, order_id, sub_order_id=[], version=VERSION):
        """
        Description:
    ...
    def get_list(self, client_store, fc, status, order_id, order_line_id, after=None, before=None, version=VERSION):
        """
        Description:
        ...
        after: <yyyy\xadmm\xaddd>, will filter out orders
        updated after given date
        before: <yyyy\xadmm\xaddd>, will filter out orders
        ...
        if status not in allowed_status:
            raise InvalidStatusError("Status - '%s' is not a valid status, Can not process request" % status)
        self.method = 'GET'
    ...
    def get_detail(self, client_store, fc, order_id, sub_order_id, version=VERSION):
        """
        Description:
        ...
        client_store: Client store name for order need to be created
        fc: name of fulfillment center from where order will be fulfilled.
        order\xadid : Order Id, order details will be
            fetched against this id
        sub\xadorder\xadid : Sub Order Id, which belongs
        ...
        """
        self.method = 'GET'
        self.config.uri = ('/oms/api/detail/{order_id}/{sub_order_id}').format(order_id=order_id, sub_order_id=sub_order_id)
        self.query_params = {'client_store': client_store, 
           'fc': fc, 
           'version': version}
        response = self.execute()
