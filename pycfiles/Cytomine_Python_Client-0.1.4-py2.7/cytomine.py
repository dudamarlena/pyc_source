...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/cytomine/cytomine.py
# Compiled at: 2017-09-05 05:52:58
__author__ = 'Stévens Benjamin <b.stevens@ulg.ac.be>'
__contributors__ = ['Marée Raphaël <raphael.maree@ulg.ac.be>', 'Rollus Loïc <lrollus@ulg.ac.be']
__copyright__ = 'Copyright 2010-2015 University of Liège, Belgium, http://www.cytomine.be/'
import os, sys, sha, base64, hmac, httplib, urllib2, httplib2
from progressbar import ProgressBar
import json, warnings, locale
...
class Cytomine(object):
    ...
    def __init__(self, host, public_key, private_key, working_path='/tmp', protocol='http://', base_path='/api/', verbose=False, timeout=120):
    ...
    def __authorize(self, action, url='', content_type='', accept='application/json,*/*', sign_with_base_path=True):
        ...
        self.__headers = {'accept': accept, 'date': strftime('%a, %d %b %Y %H:%M:%S +0000', gmtime()), 'X-Requested-With': 'XMLHttpRequest'}
    ...
    def basic_auth(self, username, password):
        ...
        self.__headers = {'content-type': 'application/json', 'accept': 'application/json', 'Authorization': 'Basic ' + auth, 
    ...
    def fetch(self, model, url='', query=''):
        ...
        if self.__verbose:
            print 'GET %s...' % (self.__host + self.__base_path + url + model.to_url() + query)
        established = None
    ...
    def save_collection(self, collection, url='', query=''):
        if len(collection) < 1:
            ...
            if self.__verbose:
                print 'POST %s...' % (self.__base_path + url + model.to_url() + query)
            if self.__verbose:
                print 'DATA : %s' % data
            self.__conn.request('POST', self.__base_path + url + model.to_url() + query, body=data, headers=dict(self.__headers.items() + [('content-type', 'application/json')]))
            response = self.__conn.getresponse()
            response_text = response.read()
            if self.__verbose:
                print 'response_text : %s' % response_text
    ...
    def update(self, model, url='', query=''):
        ...
        if self.__verbose:
            print 'PUT %s...' % (self.__base_path + url + model.to_url() + query)
        if self.__verbose:
            print 'DATA : %s' % model.to_json()
        self.__conn.request('PUT', self.__base_path + url + model.to_url() + query, body=model.to_json(), headers=dict(self.__headers.items() + [('content-type', 'application/json')]))
        response = self.__conn.getresponse()
        response_text = response.read()
        if not self.__handle_response(response.status):
            print response_text
    ...
    def delete(self, model, url='', query=''):
        ...
        if self.__verbose:
            print 'DELETE %s...' % (self.__base_path + url + model.to_url() + query)
        established = None
        while not established:
            try:
    ...
    def get_annotations(self, id_project=None, id_user=None, id_image=None, id_term=None, showGIS=None, showWKT=None, showMeta=None, bbox=None, id_bbox=None, reviewed_only=False):
        ...
        if id_term:
            query += '&terms=' + str(id_term)
        if id_user:
            query = ('{}&{}={}').format(query, 'reviewUsers' if reviewed_only else 'users', str(id_user).strip('[]').replace(' ', ''))
        if id_image:
            query += '&images=%s' % str(id_image).strip('[]').replace(' ', '')
        if bbox:
            query += '&bbox=%s' % urllib.quote_plus(bbox)
        if id_bbox:
    ...
    def add_software_parameter(self, name, id_software, type, default_value, required, index, set_by_server, uri=None, uriPrintAttribut=None, uriSortAttribut=None):
        software_parameter = SoftwareParameter()
        software_parameter.name = name
        software_parameter.software = id_software
        software_parameter.type = type
    ...
    def add_job_parameters(self, job, software, values):
        ...
        for software_parameter in software.parameters:
            ...
            if values[software_parameter['name']]:
                job_parameters_values[software_parameter['id']] = (
                 software_parameter_name,
                 values[software_parameter_name])
                ...
                job_parameters_values[software_parameter['id']] = (
                 software_parameter_name,
                 software_parameter['defaultParamValue'])
    ...
    def update_job_status(self, job, status=None, status_comment=None, progress=None):
        ...
        if status:
            job.status = status
        if status_comment:
            job.statusComment = str(status_comment)
        if progress:
    ...
    def _dump_annotations(self, annotations, get_image_url_func=Annotation.get_annotation_crop_url, dest_path='/tmp', override=False, excluded_terms=[], desired_zoom=None, desired_max_size=None):
    ...
    def dump_project_images(self, id_project=None, dest_path='imageinstances/', override=False, image_instances=None, max_size=None):
        ...
        if not image_instances:
            ...
            image_instances = image_instances.data()
    ...
    def init_storage_for_user(self, id):
        url = 'storage/create/%d' % id
        self.__authorize('POST', url=url, content_type='application/json')
        if self.__verbose:
            print 'POST %s...' % (self.__base_path + url)
        self.__conn.request('POST', self.__base_path + url, body='', headers=dict(self.__headers.items() + [('content-type', 'application/json')]))
        response = self.__conn.getresponse()
        response_text = response.read()
        if self.__verbose:
            print 'response_text : %s' % response_text
