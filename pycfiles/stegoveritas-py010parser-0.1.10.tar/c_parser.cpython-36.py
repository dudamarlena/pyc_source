...
# Size of source mod 2**32: 71900 bytes
import re
...
class CParser(PLYParser):
    ...
    def __init__(self, lex_optimize=True, lextab='py010parser.lextab', yacc_optimize=True, yacctab='py010parser.yacctab', yacc_debug=False):
        ...
        """
        self.clex = CLexer(error_func=(self._lex_error_func),
    ...
    def parse(self, text, filename='', debuglevel=0, predefine_types=True, keep_scopes=False):
        ...
        self._last_yielded_token = None
        res = self.cparser.parse(input=text,
          lexer=(self.clex),
          debug=debuglevel)
    ...
    def _define_010_typedefs(self):
        ...
        self.clex.reset_lineno()
        return self.cparser.parse(input=typedefs,
          lexer=(self.clex),
          debug=0)
        ...
        if decls[0].get('bitsize') is not None:
            ...
            if decls[0]['decl'] is None:
             ...
             c_ast.Struct, c_ast.Union, c_ast.IdentifierType)):
                ...
                if is_typedef:
                    if isinstance(spec['type'][0], c_ast.Struct):
                        if spec['type'][0].args is not None:
    ...
    def p_decl_body_2(self, p):
        ...
        if len(p) > 3:
            ...
            info = {'decl':c_ast.TypeDecl(None, None, None), 
             'bitsize':p[2]}
        decls = self._build_declarations(spec=spec,
          decls=[
         info],
    ...
    def p_type_specifier_2(self, p):
        ...
        """
        if isinstance(p[1], c_ast.IdentifierType):
            if p[1].names[0] in self._structs_with_params:
                if self._get_yacc_lookahead_token().type == 'ID':
    ...
    def p_declarator_4(self, p):
        ...
        """
        decl = c_ast.TypeDecl(declname=(p[2]),
          type=None,
          quals=None,
          coord=(self._coord(p.lineno(2))))
    ...
    def p_direct_declarator_7(self, p):
        ...
        """
        p[0] = c_ast.StructCallTypeDecl(declname=(p[1].declname),
          quals=None,
          type=(p[1]),
          coord=(self._coord(p.lineno(1))),
          args=(p[4]))
    ...
    def p_pointer(self, p):
        ...
        """
        coord = self._coord(p.lineno(1))
        p[0] = c_ast.PtrDecl(quals=(p[2] or []),
    ...
    def p_byref(self, p):
        ...
        """
        coord = self._coord(p.lineno(1))
        p[0] = c_ast.ByRefDecl(quals=(p[2] or []),
          type=(p[3] if len(p) > 3 else None),
          coord=coord)
    ...
    def p_parameter_declaration_2(self, p):
        ...
        if not spec['type']:
            spec['type'] = [c_ast.IdentifierType(['int'], coord=(self._coord(p.lineno(1))))]
        elif len(spec['type']) > 1 and len(spec['type'][(-1)].names) == 1 and self._is_type_in_scope(spec['type'][(-1)].names[0]):
    ...
    def p_type_name(self, p):
        ...
        """
        typename = c_ast.Typename(quals=(p[1]['qual']),
          type=(p[2] or c_ast.TypeDecl(None, None, None)),
          coord=(self._coord(p.lineno(2))))
