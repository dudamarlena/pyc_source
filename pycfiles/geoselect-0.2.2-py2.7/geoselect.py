...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/geoselect.py
# Compiled at: 2015-10-21 06:35:38
from __future__ import print_function
from math import radians, cos, sin, asin, sqrt
from path import Path
import time, exifread, re, sys, argparse
VERSION = '0.2.2'
...
def add_based_on_time(files_with_tags, on_location):
    ...
    for veiling_f, veiling_tags in on_location.items():
        for compare_f, compare_tags in files_with_tags.items():
            ...
            if delta.total_seconds() < 600 and compare_f not in on_location.keys():
...
def main():
    desc = 'A script to select photos from a set, based on geographical location. Either via a decimal            latitude/longitude point. Or a photo taken in the desired location, it will look for            exif standerized gps lat- and longitude tags.'
    argparser = argparse.ArgumentParser(description=desc)
    argparser.add_argument('location', type=str, default='', help='location given in decimal degrees like: "40.783068, -73.965350",                                  or a path to a photo with exif gps info')
    argparser.add_argument('--path', type=str, help='path to look for image files, if not set files                           will be taken from stdin')
    argparser.add_argument('--extentions', type=str, default='jpg,png,jpeg,tiff', help='comma separated list of extension to look for in PATH')
    argparser.add_argument('--copy-to', dest='copyto', type=str, default='', help='path where found photos should be copied')
    argparser.add_argument('--time-based', dest='time_based', action='store_true', help='also add photos wich themselfs dont have gps information, but are taken                                 in a short time before or after one that has (in the right location)')
    argparser.add_argument('--radius', type=int, default=1, help='radius of area (in kilometers) to select                           photos from')
