...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\pyhapi\hnode.py
# Compiled at: 2020-03-21 00:37:01
...
class HNodeBase:
    __doc__ = "A base class for houdini engine's node, including shared operation        for setting input/output, setting/getting geometry, get node             information etc.\n\n    Attributes:\n        session (HSession): HEngine session containing this node\n        instantiated (bool): instantiated\n        node_id (int): Current node id\n        name (str): Current node name\n    "
    ...
    def connect_node_input(self, node_to_connect, input_index=0, output_index=0):
        ...
        """
        HAPI.connect_node_input(self.session.hapi_session, self.node_id, node_to_connect.node_id, input_index, output_index)
        return self
    ...
    def get_child_nodes(self):
        ...
        child_nodes = HAPI.get_composed_child_node_list(self.session.hapi_session, self.node_id, child_count)
    ...
    def __get_display_geo_by_node(self, node_id):
        ...
        for part_id in range(0, geo_info.partCount):
            ...
            if part_info.type == HDATA.PartType.MESH:
                ...
                if part_info.type == HDATA.PartType.CURVE:
                    extract_geo = HGeoCurve()
                else:
                    if part_info.type == HDATA.PartType.VOLUME:
                        volume_info = HAPI.get_volume_info(self.session.hapi_session, geo_info.nodeId, part_id)
    ...
    def _collect_params(self):
        ...
        self.param_info = HAPI.get_parameters(self.session.hapi_session, self.node_id, self.node_info)
    ...
    def set_param_value(self, param_name, value):
        ...
        Args:
            ...
            value (int/float/str): Value to set to that param,                 depends on param type
        ...
        try:
            if paraminfo.is_int():
                HAPI.set_parm_int_value(self.session.hapi_session, self.node_id, param_name, value)
            if paraminfo.is_float():
                HAPI.set_parm_float_value(self.session.hapi_session, self.node_id, param_name, value)
            if paraminfo.is_string():
                HAPI.set_parm_string_value(self.session.hapi_session, self.node_id, paramid, value)
            return True
    ...
    async def cook_async(self):
        ...
        if not self.is_inited():
            return
        await HAPI.cook_node_async(self.session.hapi_session, self.session.cook_option, self.node_id)
    ...
    def press_button(self, param_name, status_report_interval=5.0):
        ...
        Args:
            ...
            status_report_interval (float): Time interval                 in seconds to report status
        """
        if not self.is_inited():
            return
    ...
    def get_display_geos(self):
        ...
        if self.get_node_type() == HDATA.NodeType.OBJ:
            ...
            if child_sop_count == 0:
                return self._HNodeBase__get_display_geo_by_node(self.node_id)
            child_object_infos = HAPI.get_composed_object_list(self.session.hapi_session, self.node_id, child_sop_count)
            for objectinfo in child_object_infos:
                sop_geo = self._HNodeBase__get_display_geo_by_node(objectinfo.nodeId)
...
class HNode(HNodeBase):
    __doc__ = 'A node user created for operator name or hda assets, could get/set        params, press button and cook.\n    '
    ...
    def __init__(self, session, operator_name, node_name, parent_node=None):
        ...
        self.node_id = HAPI.create_node((self.session.hapi_session), operator_name, node_name, parent_node_id=(parent_node.node_id if parent_node is not None else -1),
...
class HInputNode(HNodeBase):
    __doc__ = 'A node dedicated to marshall geom datas\n    '
...
class HExistingNode(HNodeBase):
    __doc__ = 'A node created by hengine itself, such as child node of node instantiated        with HNode\n    '
...
class HHeightfieldInputNode(HNodeBase):
    __doc__ = 'A node dedicated to marshall heightfield datas\n    '

    def __init__(self, session, node_name, x_size, y_size, voxel_size):
        super(HHeightfieldInputNode, self).__init__(session)
        self.node_id, self.height_id, self.mask_id, self.merge_id = HAPI.create_heightfield_input_node(self.session.hapi_session, node_name, x_size, y_size, voxel_size)
...
class HHeightfieldInputVolumeNode(HNodeBase):
    __doc__ = 'A node dedicated to marshall heightfield volume datas\n\n    '

    def __init__(self, session, node_name, x_size, y_size, voxel_size):
        super(HHeightfieldInputVolumeNode, self).__init__(session)
        self.node_id = HAPI.create_heightfield_volume_input_node(self.session.hapi_session, node_name, x_size, y_size, voxel_size)
        self.instantiated = True
        self.name = node_name
        self.session.nodes[self.node_id] = self
        self.node_info = HAPI.get_node_info(self.session.hapi_session, self.node_id)
