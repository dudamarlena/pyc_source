...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/linkexchange/clients/linkfeed.py
# Compiled at: 2011-04-26 11:30:34
import xml.sax, xml.dom, xml.dom.pulldom, logging
...
class LinkFeedClient(SapeLikeClient):
    ...
    def get_links_new_page(self, data, request):
        if self.is_check_code_visible(data, request):
            if data.get('__linkfeed_start__', '') + data.get('__linkfeed_end__', ''):
                return ['']
        return []
                ...
                for event, node in events:
                    if event == xml.dom.pulldom.START_ELEMENT:
                        ...
                        if path == ['data', 'pages', 'page']:
                            ...
                            if name:
                                yield (
                                 '__linkfeed_%s__' % name,
                                 node_text(node))
                        elif path == ['data', 'bot_ips']:
                            events.expandNode(node)
                            path.pop()
                            ip_nodes = node.getElementsByTagName('ip')
                            yield ('__linkfeed_robots__', [ node_text(x) for x in ip_nodes ])
    ...
    def format_data(self, data):
        ...
        def xml_make_page(uri, links):
            return '<page url="%s">%s</page>' % (uri,
             ('').join([ '<link><![CDATA[%s]]></link>' % s for s in links
             ]))
        ...
        if self.server_format == 'xml':
            ...
            lines = [
             ...
             '<data>',
             '<config>',
             '<item name="start"><![CDATA[%s]]></item>' % data.get('__linkfeed_start__', ''),
             '<item name="end"><![CDATA[%s]]></item>' % data.get('__linkfeed_end__', ''),
             '<item name="delimiter"><![CDATA[%s]]></item>' % data.get('__linkfeed_delimiter__', ''),
             '<item name="before_text"><![CDATA[%s]]></item>' % data.get('__linkfeed_before_text__', ''),
             '<item name="after_text"><![CDATA[%s]]></item>' % data.get('__linkfeed_after_text__', ''),
             '</config>',
             '<pages>',
             pages,
             '</pages>',
