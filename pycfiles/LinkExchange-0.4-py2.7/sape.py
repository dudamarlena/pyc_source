...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/linkexchange/clients/sape.py
# Compiled at: 2011-04-26 11:27:01
import os, random, urllib, urllib2, urlparse, datetime, re, xml.sax, xml.sax.saxutils, xml.dom, xml.dom.pulldom, StringIO, HTMLParser, tempfile, logging
try:
    set
except NameError:
...
from linkexchange.clients.base import ClientError, ClientNetworkError, ClientDataError, ClientDataAccessError
...
class SapeLikeClient(BaseClient):
    ...
    def __init__(self, user, **kw):
        ...
        """
        self.user = user
        for param in ('db_lifetime', 'db_reloadtime', 'socket_timeout', 'force_show_code',
                      'no_query_string', 'server_charset', 'user_agent'):
            if param in kw:
    ...
    def parse_data(self, source, url, format):
        if format == 'php':
            ...
            try:
                ...
                raise ClientDataError('Could not deserialize response from server: %s' % str(e))
            ...
            for key, value in data.items():
                if key.startswith('/'):
                    if type(value) == dict:
                        value = value.values()
                    yield (
                     normalize_uri(key), map(self.parse_link, value))
                else:
                    yield (
    ...
    def refresh_data(self, db_driver, server_list, format, request):
        ...
        if not db_driver.save(host, data, blocking=False):
            ...
            raise ClientDataAccessError('Other process/tread is currently writes to the database')
        return
...
class SapeClient(SapeLikeClient):
    ...
    def parse_data(self, source, url, format):
        ...
        def node_text(node):
            return ('').join([ sn.nodeValue for sn in node.childNodes if sn.nodeType == xml.dom.Node.TEXT_NODE
                             ])
                ...
                for event, node in events:
                    if event == xml.dom.pulldom.START_ELEMENT:
                        ...
                        if path == ['sape', 'page']:
                            ...
                            if uri == '*':
                                yield (
                                 '__sape_new_url__', node_text(node))
                            else:
                                uri = normalize_uri(uri)
                                link_nodes = node.getElementsByTagName('link')
                                yield (uri,
                                 [ self.parse_link(node_text(x)) for x in link_nodes
                                 ])
                        elif path == ['sape', 'sape_ips']:
                            events.expandNode(node)
                            path.pop()
                            ip_nodes = node.getElementsByTagName('ip')
                            yield ('__sape_ips__', [ node_text(x) for x in ip_nodes ])
                        elif path == ['sape']:
                            delimiter = node.getAttribute('delimiter')
                            if delimiter:
                                yield (
                                 '__sape_delimiter__', delimiter)
...
class ContextLinksGenerator(HTMLParser.HTMLParser):
    ...
    def __init__(self, out, links, is_fragment, show_code, new_url_code, force_body_sape_index=False, exclude_tags=None, include_tags=None):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'body' and self.show_code:
            if not self.is_fragment and self.force_body_sape_index or self.is_fragment:
...
class SapeContextClient(SapeClient):
    ...
    def __init__(self, user, db_driver, **kw):
        ...
        self.start_doc_re = re.compile('^\\s*<(\\?xml|!DOCTYPE|html|HTML)\\b', re.S)
    ...
    def content_filter(self, request, content):
        ...
        generator = ContextLinksGenerator(out, links, is_fragment=is_fragment, show_code=show_code, new_url_code=data.get('__sape_new_url__', ''), force_body_sape_index=force_body_sape_index, include_tags=include_tags)
    ...
    def parse_link(self, link):
        ...
        sentence = re.escape(xml.sax.saxutils.unescape(self.tags_re.sub('', link)))
        sentence.replace(' ', '(\\s|(&nbsp;))+')
...
class SapeArticlesClient(SapeLikeClient):
    ...
    force_show_code = False
    index_server_list = [
     'http://dispenser.articles.sape.ru/?user=%(user)s&host=%(host)s&rtype=index']
    article_server_list = [
     'http://dispenser.articles.sape.ru/?user=%(user)s&host=%(host)s&rtype=article&artid=%(article_id)s']
    image_server_list = [
     'http://dispenser.articles.sape.ru/']
    output_charset = 'utf-8'
    _template_charset_re = re.compile('<meta\\s+http-equiv="Content-Type"\\s+content="text/html;\\s*charset=(?P<charset>[\\w-]+)"\\s*/?>', re.I + re.S)

    def __init__(self, user, index_db_driver, article_db_driver, image_db_driver, template_db_driver, **kw):
        """
        SapeClient constructor.
        ...
        if is_plugin_specifier(index_db_driver):
            index_db_driver = load_plugin('linkexchange.multihash_drivers', index_db_driver)
        self.index_db_driver = index_db_driver
        if is_plugin_specifier(article_db_driver):
            article_db_driver = load_plugin('linkexchange.multihash_drivers', article_db_driver)
        self.article_db_driver = article_db_driver
        if is_plugin_specifier(image_db_driver):
            image_db_driver = load_plugin('linkexchange.multihash_drivers', image_db_driver)
        self.image_db_driver = image_db_driver
        if is_plugin_specifier(template_db_driver):
            template_db_driver = load_plugin('linkexchange.multihash_drivers', template_db_driver)
        self.template_db_driver = template_db_driver
        for param in ('index_server_list', 'article_server_list', 'image_server_list',
                      'output_charset'):
            if param in kw:
                setattr(self, param, kw[param])
    ...
    def load_links_data(self, request):
        return self.load_data(self.index_db_driver, self.index_server_list, 'php', request)
    ...
    def fetch_template(self, host, template_url, template_id, index):
        template = {'id': template_id, 'date_updated': datetime.datetime.now()}
        if template_url.startswith('file://'):
            pathname = os.path.normcase(urllib.url2pathname(urlparse.urlsplit(template_url)[2]))
            log.debug('Template URL refer to local file: %s', pathname)
            allow = pathname.startswith(os.path.normcase(tempfile.gettempdir()))
            if not allow:
                log.error('URL is not allowed: %s', template_url)
                raise ClientNetworkError('URL is not allowed: %s' % template_url)
            url = template_url
        else:
    ...
    def load_template(self, request, template_meta, template_id, index):
        ...
        if self.db_lifetime is not None and now - template['date_updated'] > template_meta['lifetime']:
            template = None
        if template is None:
            template = self.fetch_template(host, template_meta['url'], template_id, index)
            self.template_db_driver.modify(host, {str(template_meta['url']): template})
        return template
    ...
    def fetch_article(self, host, article_id):
        ...
        while article is None:
            try:
                server = server_list.next()
                url = server % dict(user=self.user, host=host, article_id=article_id)
                article = do_fetch_article(url)
                if 'date_updated' in article:
                    article['date_updated'] = datetime.datetime.fromtimestamp(int(article['date_updated']))
    ...
    def load_article(self, request, article_meta):
        ...
        if self.db_lifetime is not None:
            if 'date_updated' not in article or article['date_updated'] != article_meta['date_updated']:
                article = None
        if article is None:
            article = self.fetch_article(host, article_meta['id'])
            self.article_db_driver.modify(host, {str(article_url): article})
        return article
    ...
    def fetch_image(self, host, dispenser_path):
        ...
        def do_fetch_image(url):
            ...
            if raw_data.startswith('FATAL ERROR:'):
                log.error('Server error: %s: %s', raw_data, url)
                raise ClientError(raw_data)
            return {'date_updated': datetime.datetime.now(), 'image': raw_data}
    ...
    def load_image(self, request, image_meta):
        ...
        if self.db_lifetime is not None:
            if 'date_updated' not in image or image['date_updated'] < image_meta['date_updated']:
                image = None
        if image is None:
            image = self.fetch_image(host, image_meta['dispenser_path'])
            self.image_db_driver.modify(host, {str(image_url): image})
        return image

    def refresh_db(self, request):
        self.refresh_data(self.index_db_driver, self.index_server_list, 'php', request)
        ...
        if article_meta:
            ...
            try:
                ...
                log.warning('Article with non existing template: %s: %s', str(request.uri), template_id)
        ...
        if template_meta:
            ...
            if not date_updated or now - date_updated > template_meta['lifetime']:
                try:
                    data_to_update[template_url] = self.fetch_template(host, template_url, template_id, index)
                except ClientError:
                    pass
        ...
        for template_url, template in data.items():
            ...
            if now - template['date_updated'] > template_meta['lifetime']:
                try:
                    data_to_update[template_url] = self.fetch_template(host, template_meta['url'], template['id'], index)
                except ClientError:
                    pass
        ...
        for article_url, article in data.items():
            ...
            if 'date_updated' not in article or article['date_updated'] != article_meta['date_updated']:
                try:
                    data_to_update[article_url] = self.fetch_article(host, article_meta['id'])
                except ClientError:
                    pass
        ...
        for image_url, image in data.items():
            ...
            if 'date_updated' not in image or image['date_updated'] < image_meta['date_updated']:
                try:
                    data_to_update[image_url] = self.fetch_image(host, image_meta['dispenser_path'])
                except ClientError:
                    pass
        ...
        if format == 'php':
            ...
            for uri, links in data.pop('announcements').items():
                if type(links) == dict:
                    links = links.values()
                yield (
                 'announcement_' + normalize_uri(uri),
                 map(self.parse_link, links))

            for uri, article_meta in data.pop('articles').items():
                article_meta['date_updated'] = datetime.datetime.fromtimestamp(int(article_meta['date_updated']))
                yield ('article_' + normalize_uri(uri), article_meta)

            for uri, image_meta in data.pop('images').items():
                image_meta['date_updated'] = datetime.datetime.fromtimestamp(int(image_meta['date_updated']))
                yield ('image_' + normalize_uri(uri), image_meta)

            for tpl_id, template_meta in data.pop('templates').items():
                template_meta['lifetime'] = datetime.timedelta(0, int(template_meta['lifetime']))
    ...
    def handle_request(self, request):
        ...
        def return_article(index, article_meta):
            ...
            try:
                ...
                log.error('Template not found in template index: %s', template_id)
                raise ClientDataError('Template not found in template index: %s' % template_id)

            template = self.load_template(request, template_meta, template_id, index)
            article = self.load_article(request, article_meta)
            article_body = template['body']
            article_body = article_body.replace('{meta_charset}', self.output_charset)
            for field in index['template_fields']:
                article_body = article_body.replace('{%s}' % field, article.get(field, ''))

            if self.is_bot(index, request):
                article_body += '<!--sape_noindex-->'
            content_type = 'text/html; charset=%s' % self.output_charset
            return PageResponse(status=200, body=article_body.encode(self.output_charset), headers={'Content-Type': content_type})

        def return_image(index, image_meta):
            image = self.load_image(request, image_meta)
            return PageResponse(status=200, body=image['image'], headers={'Content-Type': image['mime']})
        ...
        if self.is_bot(index, request):
            return PageResponse(status=200, body=index.get('checkCode', '') + '<!--sape_noindex-->', headers={'Content-Type': 'text/html'})
...
class SapeLikeTestServer(SimpleFileTestServer):
    ...
    def __init__(self, filename=None, data=None, extra_data=None, server_format=None):
        if data is not None:
            self.data = data
        ...
        super(SapeLikeTestServer, self).__init__(filename=filename, raw_data=raw_data)
        return
    ...
    def format_data(self, data):
        ...
        def xml_make_page(uri, links):
            return '<page uri="%s">%s</page>' % (uri.encode('utf-8'),
             ('').join([ '<link><![CDATA[%s]]></link>' % s.encode('utf-8') for s in links
             ]))

        if self.server_format == 'xml':
            pages = ('\n').join([ xml_make_page(uri, links) for uri, links in data.items() if uri.startswith('/')
                                ])
            lines = [
             '<?xml version="1.0" encoding="UTF-8"?>',
             '<sape delimiter="%s">' % data.get('__sape_delimiter__', '').encode('utf-8'),
             pages,
             '<page uri="*"><![CDATA[%s]]></page>' % data.get('__sape_new_url__', '').encode('utf-8'),
             '</sape>']
            return ('\n').join(lines)
...
class SapeArticlesIndexTestServer(SapeLikeTestServer):
    ...
    data = {'announcements': {'/': [
       ...
       'articles': {'/articles/1': {'id': '1', 
                                    'date_updated': '0', 
                                    'template_id': '1'}}, 
       'images': {}, 'templates': {'1': {'lifetime': '3600'}}, 
       'template_fields': [
                         'title', 'keywords', 'header', 'body'], 
       'template_required_fields': {'title': 'title', 
                                    'keywords': 'meta', 
                                    'header': 'h1', 
                                    'body': 'body'}, 
       'ext_links_allowed': [], 'checkCode': '<!-- announcements place -->', 
