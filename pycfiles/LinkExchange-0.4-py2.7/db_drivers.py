...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/linkexchange/db_drivers.py
# Compiled at: 2011-04-13 18:13:45
import shelve, shutil, anydbm, datetime, os, os.path, select, threading, glob
...
class MultiHashInFilesMixin:
    ...
    def save_with_locking(self, hashkey, newhash, blocking, do_save):
        ...
        try:
            ...
            to_move = zip(self.get_all_files(new_filename), self.get_all_files(real_filename))
...
class ShelveMultiHashDriver(MultiHashInFilesMixin, BaseMultiHashDriver):
    ...
    def __init__(self, filename, max_lock_time=None, no_excl=False, db_module=None, suffix_list=None):
        """
        @param filename: file name to use for hash files (string or callable)
        ...
        """
        MultiHashInFilesMixin.__init__(self, filename, max_lock_time, no_excl, suffix_list)
    ...
    def modify(self, hashkey, otherhash, blocking=True):
        ...
        def do_modify(real_filename, new_filename, newhash):
            if isinstance(newhash, dict):
                newhash = newhash.items()
            try:
                to_copy = zip(self.get_all_files(real_filename), self.get_all_files(new_filename))
