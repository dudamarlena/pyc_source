...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/linkexchange/tests.py
# Compiled at: 2011-04-26 15:16:12
import sys, os, tempfile, urllib, urlparse, datetime, re, signal, socket, time
try:
    import subprocess
except ImportError:
    subprocess = None
...
class MultiHashDriverTestMixin:
    ...
    def test_get_mtime(self):
        ...
        self.db.save('testkey', [ ('k%d' % i, 'v%d' % i) for i in range(100) ])
    ...
    def test_blocking(self):
        ...
        def test_generator():
            for i in range(100):
                if i == 5:
                    result = self.db.save('testkey', dict(bar=3), blocking=False)
...
class ClientLinksTestMixin(ClientBaseTestMixin):
    ...
    check_code = ''
    html_links_link_pattern = '<a[^>]+?href="(?P<href>[^"]+)"[^>]*>(?P<anchor>[^<>]+?)</a>'
    ...
    def test_links_get_html_links(self):
        ...
        for test_uri, test_links in self.page_link_map.items():
            ...
            for i in range(len(links)):
                ...
                if i > 0 and delim_re:
                    delim_found = delim_re.search(html, links[(i - 1)].end(), links[i].start()) is not None
    ...
    def test_links_check_code(self):
        ...
        self.assertEqual(self.check_code in client.get_raw_links(request)[0], True)
    ...
    def test_content_filter(self):
        ...
        for test_uri, test_content_list in self.page_content_map.items():
            ...
            for test_content in test_content_list:
                filtered = client.content_filter(request, test_content[0])
