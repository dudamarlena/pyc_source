...
# [GCC 8.4.0]
# Embedded file name: /home/br0th3r/salmonete/becas.com/codenerix/djng/angular_base.py
# Compiled at: 2020-01-08 12:47:11
# Size of source mod 2**32: 13571 bytes
...
class TupleErrorList(UserList, list):
    ...
    def get_json_data(self, escape_html=False):
        ...
        for error in self.as_data():
            ...
            errors.append({'message': escape(message) if escape_html else message, 
             'code': error.code or ''})
    ...
    def as_ul(self):
        ...
        if isinstance(first, tuple):
            ...
            return mark_safe(''.join([format_html(self.ul_format, first[0], first[1], prop, mark_safe(''.join(list_items))) for prop, list_items in error_lists.items()]))
        return format_html('<ul class="errorlist">{0}</ul>', format_html_join('', '<li>{0}</li>', ((force_text(e),) for e in self)))
...
class NgBoundField(forms.BoundField):
    ...
    def css_classes(self, extra_classes=None):
        ...
        if hasattr(field_css_classes, 'split'):
            ...
            if isinstance(field_css_classes, (list, tuple)):
    ...
    def label_tag(self, contents=None, attrs=None, label_suffix=None):
        ...
        if hasattr(label_css_classes, 'split'):
            ...
            if isinstance(label_css_classes, (list, tuple)):
...
class NgFormBaseMixin(object):
    ...
    def __init__(self, data=None, *args, **kwargs):
        try:
            ...
            form_name = b64encode(six.b(self.__class__.__name__)).rstrip(six.b('=')).decode()
    ...
    def get_field_errors(self, field):
        ...
        return self.error_class([SafeTuple((identifier, self.field_error_css_classes, '$pristine', '$pristine', 'invalid', e)) for e in errors])

    def non_field_errors(self):
        errors = super(NgFormBaseMixin, self).non_field_errors()
        return self.error_class([SafeTuple((self.form_name, self.form_error_css_classes, '$pristine', '$pristine', 'invalid', e)) for e in errors])
