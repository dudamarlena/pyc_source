...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/papyro/reports.py
...
class Font(object):
    ...
    def __init__(self):
        ...
        self.size = None
        self.color = None
        self.style = [False, False, False, False]
        return

    def getxml(self):
        valor = '<font>\n  <name>' + (self.name or '') + '</name>\n' + '  <size>' + unicode(self.size or 0) + '</size>\n' + '  <color>' + (self.color or '') + '</color>\n' + '  <style>\n' + ('    <bold/>' if self.style[0] else '') + ('    <italic/>' if self.style[1] else '') + ('    <underline/>' if self.style[2] else '') + ('    <strike/>' if self.style[3] else '') + '  </style>\n' + '</font>\n'
        return valor
...
class PageHeaderOptions(object):
    ...
    def getxml(self):
        valor = '<options>\n'
        if self.print_on_first_page:
            valor += '  <print_on_first_page/>\n'
        valor += '</options>\n'
        return valor
...
class PageHeader(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<page_header>\n  <id>' + (self.id or '') + '</id>\n' + ReportItem.getxml(self) + self.options.xml + self.body.xml + '</page_header>\n'
        return valor
...
class PageFooter(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<page_footer>\n  <id>' + (self.id or '') + '</id>\n' + ReportItem.getxml(self) + self.body.xml + '</page_footer>\n'
        return valor
...
class GroupHeaderOptions(object):
    ...
    def getxml(self):
        valor = '<options>\n' + ('  <print_on_new_page/>\n' if self.print_on_new_page else '') + '</options>\n'
        return valor
...
class GroupHeader(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<group_header>\n' + '  <id>' + (self.id or '') + '</id>\n' + '  <field>' + (self.field or '') + '</field>\n' + '  <footer_id>' + (self.footer_id or '') + '</footer_id>\n' + self.options.xml + (self.outline.xml if self.outline != None else '') + ReportItem.getxml(self) + self.header.xml + '</group_header>\n'
        return valor
...
class GroupFooter(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<group_footer>\n' + '  <id>' + (self.id or '') + '</id>\n' + ReportItem.getxml(self) + self.footer.xml + '</group_footer>\n'
        return valor
...
class Table(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<table>\n  <query>' + self.query + '</query>\n' + self.getxmlfields() + self.body.xml + '</table>\n'
        return valor
...
class HeaderDetail(NotPrintableItem):
    ...
    def getxml(self):
        value = '<header>\n' + ('  <print_if_detail_is_empty/>\n' if self.print_if_detail_is_empty else '') + self.body.xml + '</header>\n'
        return value
...
class Detail(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<detail>\n  <id>' + (self.id or '') + '</id>\n' + '  <master_field>' + (self.master_field or '') + '</master_field>\n' + self.table.xml + (self.header.xml if self.header != None else '') + (self.outline.xml if self.outline != None else '') + self.body.xml + '</detail>\n'
        return valor
...
class Master(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<master>\n' + self.options.xml() + self.table.xml + self.getxmlgroupheaders() + self.getxmlgroupfooters() + (self.outline.xml if self.outline != None else '') + self.body.xml + self.getxmldetails() + '</master>\n'
        return valor
...
class Image(PrintableItem):
    ...
    def getxml(self):
        valor = '<image>\n  <id>' + self.id + '</id>\n' + '  <filename>' + (self.filename or '') + '</filename>\n' + ('  <keep_aspect_ratio/>\n' if self.keep_aspect_ratio else '') + '  <print_if>' + self.print_if + '</print_if>\n' + ReportItem.getxml(self) + '</image>\n'
        return valor
...
class SubReport(NotPrintableItem):
    ...
    def getxml(self):
        value = '<subreport>\n  <id>' + (self.id or '') + '</id>\n' + '  <name>' + (self.name or '') + '</path>\n' + self.params.xml + '  <print_if>' + self.print_if + '</print_if>\n' + '</subreport>\n'
        return value
...
class Body(NotPrintableItem):
    ...
    def getxml(self):
        valor = '<body>\n' + ('  <split_on_new_page/>\n' if self.split_on_new_page else '') + '  <print_if>' + self.print_if + '</print_if>\n' + self.getxmlitems() + '</body>\n'
        return valor
...
class CTColumn(NotPrintableItem):
    ...
    def getxml(self):
        value = '<col>\n' + self.header.xml + self.body.xml + '  <width>' + unicode(self.width or 0) + '</width>\n' + '<\\col>\n'
        return value
...
class CTRowHeader(NotPrintableItem):
    ...
    def getxml(self):
        value = '<rowheader>\n' + self.header.xml + '  <height>' + unicode(self.height or 0) + '</height>\n' + '</rowheader>\n'
        return value
...
class CTRowHeaders(NotPrintableItem):
    ...
    def getxml(self):
        value = '<rowheaders>\n' + self.getrowheadersxml() + '</rowheaders>\n'
        return value
...
class CrossTab(NotPrintableItem):
    ...
    def getxml(self):
        value = '<crosstab>\n' + self.master.xml + self.detail.xml + self.columns.xml + self.rowheaders.xml + '<crosstab>\n'
        return value
...
class Header(Body):
    ...
    def getxml(self):
        valor = '<header>\n' + ('  <split_on_new_page/>\n' if self.split_on_new_page else '') + Body.getxmlitems(self) + '</header>\n'
        return valor
...
class ReportPage(object):
    ...
    def getxml(self):
        valor = '<page>\n  <id>' + self.id + '</id>\n' + '  <number>' + unicode(self.num) + '</number>\n' + (self.page_header.xml if self.page_header != None else '') + self.body.xml + (self.page_footer.xml if self.page_footer != None else '') + '</page>\n'
        return valor
...
class ReportTitle(NotPrintableItem):
    ...
    def getxml(self):
        valor = '  <report_title>\n' + '    <id>' + self.id + '</id>\n' + ReportItem.getxml(self) + '  </report_title>\n'
        return valor
...
class Report(object):
    ...
    def setxml(self, valor):
        ...
        if author != None:
            self.author = author.text or ''
        subject = report.find('subject')
        if subject != None:
            self.subject = subject.text or ''
        keywords = report.find('keywords')
        if keywords != None:
            self.keywords = keywords.text or ''
