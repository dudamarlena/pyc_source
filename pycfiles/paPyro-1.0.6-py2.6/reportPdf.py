...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/papyro/reportPdf.py
...
class ReportPdf(ReportBase):
    ...
    def apply_colors(self, color):
        return color.replace('BLACK', '000000').replace('WHITE', 'FFFFFF').replace('RED', 'FF0000').replace('GREEN', '00FF00').replace('BLUE', '0000FF').replace('LTGRAY', 'C0C0C0').replace('YELLOW', '00FFFF')
    ...
    def writeBody(self, body, y, mdata=None, ddata=None):
        ...
        for item in body.items:
            if not isinstance(item, reports.Text) and not isinstance(item, reports.Image) and not isinstance(item, reports.Line) and not isinstance(item, reports.Code):
                only_text = False
                break
        ...
        for (n, mdata) in enumerate(data):
            for (i, group_h) in enumerate(master.group_headers):
                if mdata[group_h.field] != values[i]:
                    if not first_gh:
                        if group_h.options.print_on_new_page:
                            self.newPage(save=True)
                            y = 0
                            min_y = y
                        first_gh = False
    ...
    def writePageFooter(self, page_footer):
        ...
        if self.debug:
            self.canvas.rect(0, 0, self.wd, -page_footer.height * mm)
    ...
    def writeImage(self, image, y):
        ...
        self.canvas.drawImage(os.path.join(self.report.path, image.filename), image.left * mm, y, image.width * mm, image.height * mm, preserveAspectRatio=image.keep_aspect_ratio)
        return (
         y, False)