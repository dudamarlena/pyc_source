...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py_db_wrapper/connection.py
...
class AzureConnection(Connection):
    ...
    def connect(self):
        param_string = AzureConnection.PARAM_STRING.format(HOST=(self.host),
          PORT=(self.port),
...
class HiveConnection(Connection):
    ...
    def connect(self):
        con_string = HiveConnection.CONNECTION_STRING.format(USER=(self.user),
          PASSWORD=(self.password),
          HOST=(self.host),
          PORT=(self.port),
          DATABASE=(self.database))
...
class MySqlConnection(Connection):
    ...
    def connect(self):
        con_string = MySqlConnection.CONNECTION_STRING.format(USER=(self.user),
          PASSWORD=(self.password),
          HOST=(self.host),
          PORT=(self.port),
          DATABASE=(self.database))
