...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/tests/kyoka/policy_test.py
...
class EpsilonGreedyPolicyTest(BaseUnitTest):
    ...
    def test_choose_action_boundary_test(self):
        ...
        self.eq(2, self.choose_action_with_rand_value(0.2 + 0.7 - eps))
        self.eq(3, self.choose_action_with_rand_value(0.2 + 0.7))
    ...
    def test_epsilon_annealing(self):
        ...
        for eps in expected_eps:
            policy.anneal_eps()
            self.almosteq(eps, policy.eps, 1e-06)
