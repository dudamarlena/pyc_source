...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/kyoka/algorithm/td_learning/sarsa_lambda.py
# Compiled at: 2016-10-26 09:22:48
from kyoka.algorithm.td_learning.base_td_method import BaseTDMethod
from kyoka.algorithm.td_learning.eligibility_trace.action_eligibility_trace import ActionEligibilityTrace as EligibilityTrace
import os, pickle
...
class SarsaLambda(BaseTDMethod):
    ...
    def update_action_value_function(self, domain, policy, value_function):
        ...
        while not domain.is_terminal_state(current_state):
            ...
            delta = self.__calculate_delta(value_function, current_state, current_action, next_state, next_action, reward)
            self.trace.update(current_state, current_action)
            for state, action, eligibility in self.trace.get_eligibilities():
                new_Q_value = self.__calculate_new_Q_value(value_function, state, action, eligibility, delta)
                value_function.update_function(state, action, new_Q_value)
                self.trace.decay(state, action)

            current_state, current_action = next_state, next_action

    def __calculate_delta(self, value_function, state, action, next_state, next_action, reward):
