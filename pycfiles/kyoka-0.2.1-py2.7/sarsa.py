...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/kyoka/algorithm/td_learning/sarsa.py
...
class Sarsa(BaseTDMethod):
    ...
    def update_action_value_function(self, domain, policy, value_function):
        ...
        while not domain.is_terminal_state(state):
            ...
            new_Q_value = self.__calculate_new_Q_value(value_function, state, action, next_state, next_action, reward)
            value_function.update_function(state, action, new_Q_value)
            state, action = next_state, next_action

    def __calculate_new_Q_value(self, value_function, state, action, next_state, next_action, reward):
