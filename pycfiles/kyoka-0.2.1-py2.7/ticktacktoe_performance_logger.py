...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/sample/ticktacktoe/ticktacktoe_performance_logger.py
# Compiled at: 2016-10-03 05:58:27
...
class TickTackToePerformanceLogger(BaseCallback):
    ...
    def after_update(self, iteration_count, domain, value_function, delta):
        ...
        if iteration_count % self.performance_test_interval == 0:
            ...
            game_results = [ TickTackToeHelper.measure_performance(domain, players) for _ in range(self.test_game_count)
                           ]
    ...
    def __setup_players(self, value_function):
        domains = [ TickTackToeDomain(is_first_player=is_first) for is_first in [
