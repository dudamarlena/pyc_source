...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/tests/kyoka/algorithm/montecarlo_test.py
# Compiled at: 2016-12-01 23:40:36
import os
from mock import Mock
from nose.tools import raises
from kyoka.algorithm.montecarlo import MonteCarlo, MonteCarloTabularActionValueFunction, MonteCarloApproxActionValueFunction, validate_value_function
...
class MonteCarloTest(BaseUnitTest):
    ...
    def test_integration(self):
        ...
        expected = [
         (0, 1, 35, 2), (1, 2, 34, 2), (3, 4, 28, 2), (0, 0, 0, 0)]
        update_counter = value_func.update_counter
        for state, action, value, update_count in expected:
            self.eq(value, value_func.fetch_value_from_table(value_func.table, state, action))
            self.eq(update_count, value_func.fetch_value_from_table(update_counter, state, action))
...
def cleanup_trash():
    filenames = [
     'montecarlo_update_counter.pickle', 'montecarlo_table_action_value_function_data.pickle']
