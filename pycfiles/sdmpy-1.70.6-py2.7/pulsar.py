...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sdmpy/pulsar.py
# Compiled at: 2020-01-07 17:38:36
from __future__ import print_function, division, absolute_import, unicode_literals
from builtins import bytes, dict, object, range, map, input, int
...
import tempo_utils, tempfile

def sdmpulsar_to_polyco(r, fmt=b'tempo_utils'):
    """Takes an SDM Pulsar row, returns a tempo_utils.polyco object."""
    p = tempo_utils.polyco(None)
    imjd = int(r.refTime / 86400000000000.0)
    fmjd = (int(r.refTime) - int(imjd * 86400000000000.0)) / 86400000000000.0
    ...
    p.rfreq = reffreq
    p.site = b'6'
    p.span = tspan / 60000000000.0
    p.ncoeff = len(coeffs)
    p.obsfreq = 1400.0
    p.coeffs = list(coeffs)
    if fmt == b'tempo_utils':
        return p
    else:
        if fmt == b'psrchive':
            if psrchive is None:
                raise RuntimeError(b'psrchive-format polycos requires psrchive')
            tmp = tempfile.NamedTemporaryFile(mode=b'w+')
...
_bin_file = b'/home/mchost/evla/scripts/opt/2016/09/16B-248/polyco/16B-248.period.log'
_bin_epochs = None
_bin_periods = None
_mjd1970 = 40587
...
class BinLog(object):
    ...
    def __init__(self, sdmname, logdir=b'/lustre/aoc/sciops/pdemores/binlogs'):
        self._logdir = logdir
    ...
    def _read(self):
        for l in open(os.path.join(self._logdir, self.sdmname + b'.binlog'), b'rb'):
...
def rotate_phase(data, turns, axis=1, method=b'lin'):
    if method == b'lin':
...
def rotate_phase_fft(data, turns, axis=1):
    ...
    phs = np.exp(complex(0.0, -2.0) * np.pi * turns * ff).reshape(shape)
    fdata = np.fft.fft(data, axis=axis)
    with warnings.catch_warnings():
        warnings.simplefilter(b'ignore', np.ComplexWarning)
        return np.fft.ifft(fdata * phs, axis=axis).astype(data.dtype)


def dedisperse_array(data, dm, freq, period, bin_axis=1, freq_axis=2, spw_axis=None, phase_shift=0.0):
    """
    ...
    for ispw in range(nspw):
        ...
        for ichan in range(nchan):
            ...
            data[tuple(fslice)] = rotate_phase(dtmp, dp[(ispw, ichan)], axis=bin_axis).squeeze()
