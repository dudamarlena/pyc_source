...
# [GCC 8.4.0]
# Embedded file name: /home/thomas/software/codegra.fs/env/lib/python3.7/site-packages/codegra_fs/cgapi.py
# Compiled at: 2019-02-14 08:40:52
# Size of source mod 2**32: 11530 bytes
    ...
    def meth(*args, **kwargs):
        try:
            ...
            try:
                ...
                try:
                    raise CGAPIException({'message':str(e), 
                     'description':str(e), 
                     'code':500})
                finally:
                    e = None
...
class APIRoutes:
    ...
    def get_submissions(self, assignment_id):
        return '{base}/assignments/{assignment_id}/submissions/'.format(base=(self.base),
          assignment_id=assignment_id)

    def get_files(self, submission_id: int) -> str:
        return '{base}/submissions/{submission_id}/files/?owner={owner}'.format(base=(self.base),
          submission_id=submission_id,
          owner=(self.owner))

    def get_file(self, submission_id: int, path: str) -> str:
        return '{base}/submissions/{submission_id}/files/?path={path}&owner={owner}'.format(base=(self.base),
          submission_id=submission_id,
          path=(quote(path)),
          owner=(self.owner))
    ...
    def select_rubricitems(self, submission_id):
        return '{base}/submissions/{submission_id}/rubricitems/'.format(base=(self.base),
          submission_id=submission_id)

    def get_submission_rubric(self, submission_id):
        return '{base}/submissions/{submission_id}/rubrics/'.format(base=(self.base),
          submission_id=submission_id)

    def get_submission(self, submission_id):
        return '{base}/submissions/{submission_id}'.format(base=(self.base),
          submission_id=submission_id)

    set_submission = get_submission

    def get_assignment_rubric(self, assignment_id):
        return '{base}/assignments/{assignment_id}/rubrics/'.format(base=(self.base),
          assignment_id=assignment_id)

    def get_file_rename(self, file_id, new_path):
        return '{base}/code/{file_id}?operation=rename&new_path={new_path}'.format(base=(self.base),
          file_id=file_id,
          new_path=(quote(new_path)))

    def get_feedbacks(self, assignment_id):
        return '{base}/assignments/{assignment_id}/feedbacks/'.format(base=(self.base),
          assignment_id=assignment_id)

    def get_feedback(self, file_id):
        return '{base}/code/{file_id}?type=feedback'.format(base=(self.base),
          file_id=file_id)

    def add_feedback(self, file_id, line):
        return '{base}/code/{file_id}/comments/{line}'.format(base=(self.base),
          file_id=file_id,
          line=line)
    ...
    def get_assignment(self, assignment_id):
        return '{base}/assignments/{assignment_id}'.format(base=(self.base),
          assignment_id=assignment_id)


class APICodes(IntEnum):
    __doc__ = 'Internal API codes that are used by :class:`APIException` objects.\n    '
...
class CGAPIException(Exception):
    ...
    def __init__(self, response):
        try:
            ...
            raise Exception('Could not get json from server, maybe wrong url? Url should end with "/api/v1/" and start with "https://"')
    ...
    def __str__(self):
        return 'codegra_fs.cgapi.CGAPIException: {} - {} [{}]'.format(self.message, self.description, self.code)


class CGAPI:

    def __init__(self, username: str, password: str, base: t.Optional[str]=None, fixed: bool=False) -> None:
        owner = 'student' if fixed else 'auto'
        self.routes = APIRoutes(base or DEFAULT_CGAPI_BASE_URL, owner)
        r = requests.post((self.routes.get_login()),
        ...
        self.s.get = make_request_method(self.s.get)
        self.s.patch = make_request_method(self.s.patch)
        self.s.post = make_request_method(self.s.post)
        self.s.delete = make_request_method(self.s.delete)
        self.s.put = make_request_method(self.s.put)
    ...
    def set_submission(self, submission_id: int, grade: t.Union[(None, float, str)]=None, feedback: t.Optional[bytes]=None):
