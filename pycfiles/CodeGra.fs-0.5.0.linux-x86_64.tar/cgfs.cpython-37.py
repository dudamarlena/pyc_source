...
# [GCC 8.4.0]
# Embedded file name: /home/thomas/software/codegra.fs/env/lib/python3.7/site-packages/codegra_fs/cgfs.py
# Compiled at: 2019-02-14 08:40:52
# Size of source mod 2**32: 66527 bytes
import os, abc, sys, enum, json, uuid, ctypes, socket, typing as t, hashlib, logging, datetime, tempfile, threading, traceback
from os import O_EXCL, O_CREAT, O_TRUNC, path, getenv
from enum import IntEnum
from stat import S_IFDIR, S_IFREG
from time import time
from errno import EPERM, EEXIST, EINVAL, EISDIR, ENOENT, ENOTDIR, ENOTSUP, ENOTEMPTY
...
cgapi = None
fuse_ptr = None
CGFS_TESTING = bool(os.getenv('CGFS_TESTING', False))
T = t.TypeVar('T')
...
def remove_permission(perm: int, read: bool=False, write: bool=False, execute: bool=False) -> int:
...
def create_permission(read: bool, write: bool, execute: bool) -> int:
    ...
    if sys.platform.startswith('win32'):
        return base + base * 8 + base * 64
    return 0 + base * 8 + base * 64
...
class BaseFile:
    ...
    def __init__(self, data: t.Dict[(str, t.Any)], name: t.Optional[str]=None) -> None:
        self.id = data.get('id', None)
        self.name = name if name is not None else data['name']
        self.stat = None

    def getattr(self, submission: t.Optional['Directory']=None, path: str=None) -> PartialStat:
        global cgapi
        if self.stat is None:
            self.stat = {'st_size':0,  'st_atime':time(), 
             'st_mtime':time(), 
             'st_ctime':time(), 
             'st_uid':getuid(), 
             'st_gid':getegid()}
...
class Directory(BaseFile):
    ...
    def __init__(self, data, name=None, type=DirTypes.REGDIR, writable=False):
        ...
        self.stat = None
        self.tld = NOT_PRESENT

    def getattr(self, submission=None, path=None):
        if self.stat is None:
            self.stat = t.cast(FullStat, {**{'st_mode': S_IFDIR | create_permission(read=True,
                          write=(self.writable),
                          execute=True)}, **(super(Directory, self).getattr(submission, path))})
...
class SingleFile(BaseFile):
    ...
    def base_getattr(self, submission=None, path=None):
        return super().getattr(submission, path)

    @abc.abstractclassmethod
    def getattr(self, submission: t.Optional['Directory']=None, path: str=None) -> FullStat:
...
class SpecialFile(SingleFile):
    ...
    def getattr(self, *_: object) -> FullStat:
        return {'st_size':len(self.get_data()), 
...
class HelpFile(SpecialFile):
    ...
    def get_data(self) -> bytes:
        return bytes('\n'.join([l[4:] if l[:4] == '    ' else l for l in (self.from_class.__doc__ or '').splitlines()]), 'utf8')
...
class CachedSpecialFile(SpecialFile, t.Generic[T]):
    ...
    def get_data(self) -> bytes:
        if self.has_data:
            if datetime.datetime.utcnow() - self.time < self.DELTA:
                return self.data
        if self.overwrite:
    ...
    def write(self, data: bytes, offset: int) -> int:
        ...
        elif len(self.data) - offset - len(data) > 0:
    ...
    def flush(self) -> None:
        if not self.overwrite:
            ...
            try:
                ...
                try:
                    logger.error('error from server: {} ({})'.format(e.message, e.description))
    ...
    def truncate(self, length: int) -> None:
        ...
        if length == 0:
            ...
            if length <= len(self.data):
                ...
                assert self.stat is not None
                self.data = self.data + bytes('\x00' * (length - self.stat['st_size']), 'utf8')
        self.overwrite = True
        ...
        for i, line in enumerate(data.split(b'\n')):
            if line.startswith(b'- [x]') or line.startswith(b'- [X]'):
                try:
                    ...
                    logger.warning('Item on line {} ({}) not found!'.format(i, line))
                    raise FuseOSError(EPERM)
...
class RubricEditorFile(CachedSpecialFile[t.List[RubricRow]]):
    ...
    def parse(self, data_b: bytes) -> t.List[RubricRow]:
        ...
        def parse_description(i, end=None, strip_leading=True, strip_trailing=False):
            if end is None:
                end = [
                 '-']
            lines = []
        ...
        def parse_list(i):
            ...
            while i < len(data) and data[i] != '#':
                ...
                while data[i] != '-':
                    ...
                    if data[i] == '\n':
                        raise ParseException('Item header cannot contain a newline, you probably missed a "-" in your header')
                    else:
                        i += 1
    ...
    def send_back(self, parsed: t.List[RubricRow]) -> None:
        ...
        for header, row_id, desc, items in parsed:
            ...
            for item_id_hash, points, i_head, i_desc in items:
                items_res.append({'description':i_desc, 
                 'header':i_head, 
                 'points':points})
                if item_id_hash is not None:
                    items_res[(-1)]['id'] = get_from_lookup(item_id_hash)

            res.append({'description':desc, 
             'header':header, 
             'items':items_res})
            if row_id is None:
                if any(('id' in i for i in items_res)):
        ...
        if self.append_only:
            if new_lookup:
                ...
                logging.critical('You cannot delete rubric items using the file system.')
                raise FuseOSError(EPERM)
...
class AssignmentSettingsFile(CachedSpecialFile[t.Dict[(str, str)]]):
    ...
    def __init__(self, api, assignment_id):
        super(AssignmentSettingsFile, self).__init__(name='.cg-assignment-settings.ini')
        self.assignment_id = assignment_id
        self.api = api
...
class TempFile(SingleFile):
    ...
    @property
    def stat(self) -> FullStat:
        st = os.lstat(self.full_path)
        stat = {key:getattr(st, key) for key in ('st_atime', 'st_ctime', 'st_gid',
                                                 'st_mtime', 'st_nlink', 'st_size',
                                                 'st_uid')}
        stat['st_mode'] = S_IFREG | create_permission(True, True, True)
        return t.cast(FullStat, stat)

    def getattr(self, submission: t.Optional[Directory]=None, path: t.Optional[str]=None) -> FullStat:
        return self.stat
...
class File(SingleFile):
    ...
    def getattr(self, submission: t.Optional[Directory]=None, path: t.Optional[str]=None) -> FullStat:
        if self.stat is None:
            ...
            self.stat['st_mode'] = S_IFREG | create_permission(True, True, True)
            self.stat['st_nlink'] = 1
    ...
    def truncate(self, length: int) -> None:
        ...
        if length == 0:
            ...
            if length <= len(old_data):
                self.data = self.data[:length]
            else:
                self.data = old_data + bytes('\x00' * (length - len(old_data)), 'utf8')
        assert self.stat is not None
        self.stat['st_atime'] = time()
    ...
    def write(self, data: bytes, offset: int) -> int:
        ...
        elif len(old_data) - offset - len(data) > 0:
            second_offset = offset + len(data)
...
class APIHandler:
    ...
    def __init__(self, cgfs: 'CGFS') -> None:
        self.ops = {'set_feedback':self.set_feedback, 
    ...
    def _get_file(self, f_name: str) -> t.Union[(APIHandlerResponse, SingleFile)]:
        f_name = self.cgfs.strippath(f_name)
        if sys.platform.startswith('win32'):
            ffi = cffi.FFI()
            in_str = ffi.new('wchar_t[]', f_name)
            out_str = ffi.new('char **')
            if winfspy.lib.FspPosixMapWindowsToPosixPathEx(in_str, out_str, True) != 0:
                return {'ok':False, 
                 'error':'Winfspy returned an error'}
    ...
    def delete_feedback(self, payload: t.Dict[(str, t.Any)]) -> APIHandlerResponse:
    ...
    def set_feedback(self, payload: t.Dict[(str, t.Any)]) -> APIHandlerResponse:
        ...
        with self.cgfs._lock:
            ...
            if not isinstance(f, SingleFile):
                return f
            if not isinstance(f, File):
                return {'ok':False,  'error':'File not found or not a server file'}
            assert cgapi is not None
            try:
...
class CGFS(LoggingMixIn, Operations):
    ...
    def init(self, path: str) -> None:
        global fuse_ptr
        fuse_ptr = ctypes.c_void_p(fuse._libfuse.fuse_get_context().contents.fuse)

    def __init__(self, latest_only: bool, socketfile: str, mountpoint: str, tmpdir: str, fixed: bool=False, rubric_append_only: bool=True, assigned_only: bool=False) -> None:
        ...
        self.special_socketfile = SocketFile(bytes(socketfile, 'utf8'), '.api.socket')
        self.rubric_append_only = rubric_append_only
        self.files = Directory({'id':None, 
         'name':'root'},
          type=(DirTypes.FSROOT))
        with self._lock:
            self.files.getattr()
            self.files.insert(self.special_socketfile)
            self.files.insert(SpecialFile('.cg-mode', b'FIXED\n' if self.fixed else b'NOT_FIXED\n'))
            self.load_courses()
        logger.info('Mounted')
    ...
    def load_courses(self) -> None:
        ...
        for dups in codegra_fs.utils.find_all_dups(courses, lambda x: x['name']):
            end = 4
            if len(dups) != len(set((d['created_at'][:4] for d in dups))):
                end = None
            for dup in dups:
                date = codegra_fs.utils.format_datestring(dup['created_at'])[:end]
                dup['name'] += ' - ' + date

        for course in courses:
            assignments = course['assignments']
            for dups in codegra_fs.utils.find_all_dups(assignments, lambda x: x['name']):
                for dup in dups:
                    dup['name'] += ' - ' + codegra_fs.utils.format_datestring(dup['created_at'])
            ...
            for assig in assignments:
                ...
                assig_dir.insert(SpecialFile('.cg-assignment-id',
    ...
    def load_submissions(self, assignment: Directory) -> None:
        ...
        seen = set()
        my_id = cgapi.user['id']
        user_assigned = self.assigned_only and any((get_assignee_id(s) == my_id for s in submissions))
        for sub in submissions:
            ...
            sub_dir = Directory(sub,
              name=(codegra_fs.utils.name_of_user(sub['user']) + ' - ' + codegra_fs.utils.format_datestring(sub['created_at'])),
            ...
            if sub['user'].get('group') is not None:
                members = [codegra_fs.utils.name_of_user(m).encode('utf-8') for m in sub['user']['group']['members']]
                sub_dir.insert(SpecialFile('.cg-group-members', (b'\n').join(members) + b'\n'))
    ...
    def load_submission_files(self, submission: Directory) -> None:
        ...
        submission.insert(SpecialFile('.cg-submission-id',
          data=(str(submission.id).encode() + b'\n')))
    ...
    def get_file(self, path: t.Union[(str, t.List[str])], start: t.Optional[Directory]=None, expect_type: t.Type[T]=None) -> T:
        ...
        for part in parts:
            if part == '':
                continue
            try:
                if not any((not isinstance(f, SpecialFile) for f in file.children.values())):
                    if file.type == DirTypes.ASSIGNMENT:
                        self.load_submissions(file)
                    else:
                        if file.type == DirTypes.SUBMISSION:
            ...
            if not part not in file.children:
                if file.children[part] is None:
                    raise FuseOSError(ENOENT)
    ...
    def get_dir(self, path: t.Union[(str, t.List[str])], start: t.Optional[Directory]=None) -> Directory:
    ...
    def _do_fsync_like(self, path: str, fh: OptFileHandle, todo: FsyncLike) -> None:
        with self._lock:
            file = self.get_file_with_fh(path, fh)
            if todo == FsyncLike.fsync:
                res = file.fsync()
            else:
                if todo == FsyncLike.flush:
                    res = file.flush()
                else:
                    assert False
            if res is not None:
        ...
        with self._lock:
            ...
            if not dir.children_loaded:
                if dir.type == DirTypes.ASSIGNMENT:
                    ...
                    if dir.type == DirTypes.SUBMISSION:
    ...
    def setxattr(self, path: str, name: str, value: object, options: object, position: int=0) -> None:
        raise FuseOSError(ENOTSUP)
    ...
    def truncate(self, path: str, length: int, fh: OptFileHandle=None) -> None:
        with self._lock:
            ...
            file = self.get_file_with_fh(path, fh)
            if self.fixed:
                if not isinstance(file, (TempFile, SpecialFile)):
                    raise FuseOSError(EPERM)
...
def create_and_mount_fs(username: str, password: str, url: t.Optional[str], fixed: bool, assigned_only: bool, latest_only: bool, mountpoint: str, rubric_append_only: bool) -> None:
    global cgapi
    logger.info('Mounting... ')
    try:
        cgapi = CGAPI(username,
          password,
          url,
          fixed=fixed)
    except:
    ...
    with tempfile.TemporaryDirectory(dir=(tempfile.gettempdir())) as (tmpdir):
        ...
        if sys.platform.startswith('win32'):
            ...
            if sys.platform.startswith('darwin'):
                kwargs = {'from_code':'UTF-8', 
                 'to_code':'UTF-8-MAC', 
                 'modules':'iconv'}
        fs = None
        try:
            try:
                fs = CGFS(latest_only=latest_only,
                  socketfile=sockfile,
                  fixed=fixed,
                ...
                FUSE(
 fs,
 mountpoint, nothreads=True, 
                 foreground=True, 
                 direct_io=True, **kwargs)
            except RuntimeError:
                logger.critical(traceback.format_exc())

        finally:
            if fs is not None:
                if hasattr(fs, 'api_handler'):
...
def check_version() -> None:
    if codegra_fs.utils.newer_version_available():
        msg = [
         'You are running an outdated version of CGFS, please consider upgrading.',
...
def main() -> None:
    ...
    if msg:
        ...
        sys.exit(2)
    check_version()
    argparser = ArgumentParser(description='CodeGra.fs: The CodeGrade file system',
    ...
    argparser.add_argument('--version',
      dest='version',
      action='version',
      version=('%(prog)s {}'.format('.'.join(map(str, codegra_fs.__version__)))),
    ...
    if password is None:
        sys.stdin.isatty() or print('Password:', end=' ')
    ...
    create_and_mount_fs(username=username,
      password=password,
