...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/PyCausality/Testing/Test_Utils/Time_Series_Generate.py
# Compiled at: 2018-07-22 09:16:11
...
def coupled_logistic_map(S1, S2, T, N, alpha, epsilon, r=4, g=None):
    ...
    [S2.append((1 - alpha) * _logistic_map_(S2[n], r) + alpha * g(S1[n], epsilon, r)) for n in range(N - 1)]
...
def coupled_random_walks(S1, S2, T, N, mu1, mu2, sigma1, sigma2, alpha, epsilon, lag, seed=None):
    ...
    S2[lag:] = (1 - alpha) * (epsilon * S2[:-lag] + (1 - epsilon) * S2[lag:]) + alpha * S1[:-lag]
