...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/type_constraint.py
# Compiled at: 2015-06-02 05:02:52
import collections, types, re, inspect, pprint
__authors__ = [
 'timchow']
__version__ = '0.0.10'
...
def parameter_binding(f, *a, **kw):
    ...
    default_parameters = dict(zip(argspec.args[::-1], argspec.defaults[::-1])) if argspec.defaults else {}
...
class Check:
    ...
    def _extract(self, dc):
        assert isinstance(dc, basestring), "param:dc should be `basestring', not`%s'" % type(dc)
        constract = []
    ...
    def check_params(self, constract, f, *a, **kw):
        ...
        for name, args in constract:
            if name != 'param' or len(args) < 2 or args[0] not in binding_result or args[1] not in self._name2type:
                continue
    ...
    def check_result(self, constract, result):
        for name, args in constract:
            if name != 'return' or len(args) < 1 or args[0] not in self._name2type:
                continue
