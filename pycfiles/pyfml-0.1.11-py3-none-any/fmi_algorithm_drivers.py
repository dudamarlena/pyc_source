...
# Embedded file name: build/bdist.linux-x86_64/egg/pyfmi/fmi_algorithm_drivers.py
# Compiled at: 2018-12-15 16:31:41
__doc__ = '\nModule for simulation algorithms to be used together with\npyfmi.fmi.FMUModel.simulate.\n'
import logging, time, numpy as N, pyfmi, pyfmi.fmi as fmi, pyfmi.fmi_coupled as fmi_coupled, pyfmi.fmi_extended as fmi_extended
...
class FMIResult(JMResultBase):
    ...
    def __init__(self, model=None, result_file_name=None, solver=None, result_data=None, options=None, status=0, detailed_timings=None):
        JMResultBase.__init__(self, model, result_file_name, solver, result_data, options)
        self.status = status
...
class AssimuloFMIAlgOptions(OptionBase):
    ...
    def __init__(self, *args, **kw):
        _defaults = {'solver': 'CVode', 
           ...
           'CVode_options': {'discr': 'BDF', 'iter': 'Newton', 'atol': 'Default', 
                             'rtol': 'Default', 'external_event_detection': False}, 
           'Radau5ODE_options': {'atol': 'Default', 'rtol': 'Default'}, 'RungeKutta34_options': {'atol': 'Default', 'rtol': 'Default'}, 'Dopri5_options': {'atol': 'Default', 'rtol': 'Default'}, 'RodasODE_options': {'atol': 'Default', 'rtol': 'Default'}, 'LSODAR_options': {'atol': 'Default', 'rtol': 'Default'}, 'ExplicitEuler_options': {}, 'ImplicitEuler_options': {}}
...
class AssimuloFMIAlg(AlgorithmBase):
    ...
    def __init__(self, start_time, final_time, input, model, options):
        ...
        if not assimulo_present:
            raise fmi.FMUException('Could not find Assimulo package. Check pyfmi.check_packages()')
        self.start_time = start_time
        self.final_time = final_time
        self.input = input
        if isinstance(options, dict) and not isinstance(options, AssimuloFMIAlgOptions):
        ...
        if self.input:
            if hasattr(self.input[1], '__call__'):
                ...
                input_traj = (
                 self.input[0],
                 TrajectoryLinearInterpolation(self.input[1][:, 0], self.input[1][:, 1:]))
        ...
        elif self.options['result_handling'] == 'none':
        ...
        if self.options['sensitivities']:
            if self.model.get_generation_tool() != 'JModelica.org' and self.model.get_generation_tool() != 'Optimica Compiler Toolkit':
        ...
        if not self.input and (isinstance(self.model, fmi.FMUModelME2) or isinstance(self.model, fmi_coupled.CoupledFMUModelME2)):
            ...
            if self.options['sensitivities']:
                self.probl = FMIODESENS2(self.model, input_traj, result_file_name=self.result_file_name, with_jacobian=self.with_jacobian, start_time=self.start_time, parameters=self.options['sensitivities'], logging=self.options['logging'], result_handler=self.result_handler)
            else:
                self.probl = FMIODE2(self.model, input_traj, result_file_name=self.result_file_name, with_jacobian=self.with_jacobian, start_time=self.start_time, logging=self.options['logging'], result_handler=self.result_handler, extra_equations=self.options['extra_equations'])
        elif not self.input:
            if self.options['sensitivities']:
        ...
        elif self.options['sensitivities']:
            self.probl = FMIODESENS(self.model, input_traj, result_file_name=self.result_file_name, with_jacobian=self.with_jacobian, start_time=self.start_time, parameters=self.options['sensitivities'], logging=self.options['logging'], result_handler=self.result_handler)
        else:
            self.probl = FMIODE(self.model, input_traj, result_file_name=self.result_file_name, with_jacobian=self.with_jacobian, start_time=self.start_time, logging=self.options['logging'], result_handler=self.result_handler)
    ...
    def _set_options(self):
        ...
        if not isinstance(self.model, fmi.FMUModelME2):
            self.with_jacobian = False
        elif self.with_jacobian == 'Default' and isinstance(self.model, fmi.FMUModelME2):
    ...
    def get_result(self):
        ...
        return FMIResult(self.model, self.result_file_name, self.simulator, res, self.options, detailed_timings=self.timings)
...
class FMICSAlg(AlgorithmBase):
    ...
    def __init__(self, start_time, final_time, input, model, options):
        ...
        self.input = input
        self.status = 0
        if isinstance(options, dict) and not isinstance(options, FMICSAlgOptions):
    ...
    def solve(self):
        ...
        for t in grid:
            ...
            if status != 0:
                ...
                elif status == fmi.FMI_DISCARD and (isinstance(self.model, fmi.FMUModelCS1) or isinstance(self.model, fmi.FMUModelCS2)):
                    try:
    ...
    def get_result(self):
        ...
        self.timings['other'] = end_time - self.time_start_total - sum(self.timings.values())
        self.timings['total'] = end_time - self.time_start_total
        return FMIResult(self.model, self.result_file_name, None, res, self.options, status=self.status, detailed_timings=self.timings)
...
class SciEstAlg(AlgorithmBase):
    ...
    def __init__(self, parameters, measurements, input, model, options):
        """
        Estimation algortihm for FMUs .
        ...
        self.input = input
        if isinstance(options, dict) and not isinstance(options, SciEstAlgOptions):
    ...
    def _set_options(self):
        ...
        self.options['simulate_options']['ncp'] = self.measurements[1].shape[0] - 1
        self.options['simulate_options']['filter'] = self.measurements[0]
        if 'solver' in self.options['simulate_options']:
            solver = self.options['simulate_options']['solver']
            self.options['simulate_options'][(solver + '_options')]['verbosity'] = 50
            self.options['simulate_options'][(solver + '_options')]['store_event_points'] = False
    ...
    def solve(self):
        ...
        """
        global niter
        import scipy as sci, scipy.optimize as sciopt
        ...
        print ' '
        res = sciopt.minimize(parameter_estimation_f, p0, args=(
         self.parameters, self.measurements, self.model, self.input, self.options), method=self.options['method'], bounds=None, constraints=(), tol=self.options['tolerance'], callback=parameter_estimation_callback)
    ...
    def get_result(self):
        ...
        return FMIResult(self.model, self.options['result_file_name'], None, self.result_handler.get_result(), self.options, status=self.status)
...
class SciEstAlgOptions(OptionBase):
    ...
    def __init__(self, *args, **kw):
        _defaults = {'tolerance': 1e-06, 'result_file_name': '', 
           'filter': None, 
           'method': 'Nelder-Mead', 
           'scaling': 'Default', 
