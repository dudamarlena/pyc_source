...
# Embedded file name: build/bdist.linux-x86_64/egg/pyfmi/common/io.py
# Compiled at: 2018-12-15 16:31:42
__doc__ = '\nModule for writing optimization and simulation results to file.\n'
from operator import itemgetter, attrgetter
import array, codecs, re, sys, numpy as N, numpy as np, scipy.io
from . import xmlparser
import pyfmi.fmi as fmi, pyfmi.fmi_util as fmi_util
...
class ResultWriterDymola(ResultWriter):
    ...
    def write_header(self, file_name='', parameters=None):
        ...
        types_noalias = sorted(zip(tuple(vrefs_noalias), tuple(types_noalias)), key=itemgetter(0))
        ...
        list_of_continuous_states = N.append(self.model._save_real_variables_val, self.model._save_int_variables_val)
        list_of_continuous_states = N.append(list_of_continuous_states, self.model._save_bool_variables_val).tolist()
        list_of_continuous_states = dict(zip(list_of_continuous_states, range(len(list_of_continuous_states))))
        ...
        for i, name in enumerate(names):
            if aliases[i][1] == 0:
                if variabilities[i][1] == fmi.FMI_PARAMETER or variabilities[i][1] == fmi.FMI_CONSTANT:
                    cnt_1 += 1
                    n_parameters += 1
        ...
        for i, name in enumerate(names_noalias):
            if variabilities_noalias[i][1] == fmi.FMI_CONSTANT or variabilities_noalias[i][1] == fmi.FMI_PARAMETER:
                if types_noalias[i][1] == fmi.FMI_REAL:
                    str_text = str_text + ' %.14E' % self.model.get_real([name[0]])
                elif types_noalias[i][1] == fmi.FMI_INTEGER:
                    str_text = str_text + ' %.14E' % self.model.get_integer([name[0]])
                elif types_noalias[i][1] == fmi.FMI_BOOLEAN:
                    str_text = str_text + ' %.14E' % float(self.model.get_boolean([name[0]])[0])
    ...
    def simulation_start(self):
            ...
            if update:
                if aliases[i][1] == 0:
                    ...
                    if data_type != types[i][1]:
                        raise Exception
                    if variability == fmi.FMI_PARAMETER or variability == fmi.FMI_CONSTANT:
                        cnt_1 += 1
                        n_parameters += 1
                        datatable1 = True
                        list_of_parameters.append((types[i][0], types[i][1]))
        ...
        for i, dtype in enumerate(list_of_parameters):
            ...
            if dtype[1] == fmi.FMI_REAL:
                str_text = str_text + ' %.14E' % self.model.get_real([vref])
            elif dtype[1] == fmi.FMI_INTEGER or dtype[1] == fmi.FMI_ENUMERATION:
                str_text = str_text + ' %.14E' % self.model.get_integer([vref])
            elif dtype[1] == fmi.FMI_BOOLEAN:
                str_text = str_text + ' %.14E' % float(self.model.get_boolean([vref])[0])
...
def robust_float(value):
    ...
    try:
        ...
        if value.startswith('-1.#INF'):
            return float(-N.inf)
        if value.startswith('1.#QNAN') or value.startswith('-1.#IND'):
            return float(N.nan)
        raise ValueError
...
class ResultHandlerBinaryFile(ResultHandler):
    ...
    def _write_header(self, name, nbr_rows, nbr_cols, data_type):
        ...
        self._file.write(np.compat.asbytes(name + '\x00'))
    ...
    def simulation_start(self):
        ...
        vars_real = self.model.get_model_variables(type=fmi.FMI_REAL, filter=self.options['filter'], _as_list=True)
        vars_int = self.model.get_model_variables(type=fmi.FMI_INTEGER, filter=self.options['filter'], _as_list=True)
        vars_bool = self.model.get_model_variables(type=fmi.FMI_BOOLEAN, filter=self.options['filter'], _as_list=True)
        vars_enum = self.model.get_model_variables(type=fmi.FMI_ENUMERATION, filter=self.options['filter'], _as_list=True)
        ...
        name_data = [
         'time'] + [ var.name for var in sorted_vars ]
