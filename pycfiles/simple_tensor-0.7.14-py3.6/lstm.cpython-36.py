...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simple_tensor/rnn_packs/lstm.py
# Compiled at: 2019-03-06 01:33:57
# Size of source mod 2**32: 21398 bytes
...
class LSTM(object):
    ...
    def __init__(self, input_feature_num, output_feature_num, memory_feature_num, dropout_val=0.85, data_type=tf.float64):
        """
                CLSTM Constructor
                Args:
    ...
    def inside_LSTM_nn(self, layer_out_num1, layer_out_num2, layer_out_num3, nn_code, cell_code):
        """
                ...
                """
        if cell_code != '0':
            fc1, w_fc1, b_fc1 = new_fc_layer((self.inside_LSTM_nn_input), (self.nn_inside_LSTM_inputfeature_num),
              layer_out_num1, name=('fc1_nn' + nn_code + '_' + cell_code),
              activation='LRELU',
              data_type=(self.tf_data_type))
        else:
            fc1, w_fc1, b_fc1 = new_fc_layer((self.inside_LSTM_nn_input), (self.nn_inside_LSTM_inputfeature_num - 1),
              layer_out_num1, name=('fc1_nn' + nn_code + '_' + cell_code),
              activation='LRELU',
              data_type=(self.tf_data_type))
        drop1 = tf.nn.dropout(fc1, self.dropout_val)
        fc2, w_fc2, b_fc2 = new_fc_layer(drop1, layer_out_num1,
          layer_out_num2,
          name=('fc2_nn' + nn_code + '_' + cell_code),
          activation='LRELU',
          data_type=(self.tf_data_type))
        drop2 = tf.nn.dropout(fc2, self.dropout_val)
        fc3, w_fc3, b_fc3 = new_fc_layer(drop2, layer_out_num2,
          layer_out_num3,
    ...
    def inside_LSTM_hybridnn(self, layer_out_num1, layer_out_num2, layer_out_num3, nn_code, cell_code):
        ...
        drop1 = tf.nn.dropout(fc1, self.dropout_val)
        fc2, w_fc2, b_fc2 = new_fc_layer(drop1, layer_out_num1,
          layer_out_num2, name=('fc2_nn' + nn_code + '_' + cell_code),
          activation='LRELU',
          data_type=(self.tf_data_type))
        drop2 = tf.nn.dropout(fc2, self.dropout_val)
        ...
        out, w_out, b_out = new_fc_layer(concat3, neuron_in,
          layer_out_num3, name=('fc1_nn' + nn_code + '_' + cell_code),
          activation='LRELU',
          data_type=(self.tf_data_type))
        vars = [
         w_fc1, b_fc1, w_fc2, b_fc2, conv1a_w, conv1a_b, conv1b_w, conv1b_b, conv1c_w, conv1c_b, conv2a_w, conv2a_b, conv2b_w, conv2b_b, conv2c_w, conv2c_b,
    ...
    def inside_LSTM_deepnn(self, layer_out_num1, layer_out_num2, layer_out_num3, nn_code, cell_code):
        ...
        fc1, w_fc1, b_fc1 = new_fc_layer(flatten1, (conv10.get_shape().as_list()[(-1)]), layer_out_num3, name=('fc1_nn' + nn_code + '_' + cell_code),
          activation='LRELU',
          data_type=(self.tf_data_type))
        vars = [
         conv1a_w, conv1a_b, conv1b_w, conv1b_b, conv1c_w, conv1c_b,
    ...
    def build_lstm_cell(self, last_output, last_memmory, input_tensor, num_hidden_neuron=5, cell_code='1', inside_nn_type='fc'):
        ...
        pre_out, w_out, b_out = new_fc_layer(drop_s10, (self.memory_feature_num),
          (self.output_feature_num),
          name=('out_nn_' + cell_code),
          activation='none',
          data_type=(self.tf_data_type))
        s10_vars = [
         w_out, b_out]
...
class SimpleSquenceLSTM(LSTM):
    ...
    def __init__(self, batch_size, num_lstm_cell, input_feature_num, output_feature_num, memory_feature_num, hidden_neuron_num=5, dropout_val=0.95, with_residual=False, residual_season=1, return_memmory=False, tf_data_type=tf.float32, np_data_type=np.float32):
        """
                A Constructor
                Args:
