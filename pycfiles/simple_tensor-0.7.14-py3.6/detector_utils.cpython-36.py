...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simple_tensor/object_detector/detector_utils.py
# Compiled at: 2020-01-22 09:13:00
# Size of source mod 2**32: 54654 bytes
"""
...
class ObjectDetector(object):
    ...
    def __init__(self, num_of_class, input_height=416, input_width=416, grid_height1=32, grid_width1=32, grid_height2=16, grid_width2=16, grid_height3=8, grid_width3=8, objectness_loss_alpha=2.0, noobjectness_loss_alpha=1.0, center_loss_alpha=1.0, size_loss_alpha=1.0, class_loss_alpha=1.0, add_modsig_toshape=False, anchor=[
 (10, 13), (16, 30), (33, 23), (30, 61), (62, 45), (59, 119), (116, 90), (156, 198), (373, 326)], dropout_rate=0.0, leaky_relu_alpha=0.1, convert_to_tflite=False):
        """[summary]
    ...
    def object_accuracy(self, objectness_pred, objectness_label, noobjectness_label):
        ...
        obj_acc = 1.0 - tf.reduce_sum(delta) / (tf.reduce_sum(objectness_label) + 0.0001)
        noobjecteness_mask = 1.0 - objectness_mask
        delta = (noobjectness_label - noobjecteness_mask) * noobjectness_label
        noobj_acc = 1.0 - tf.reduce_sum(delta) / (tf.reduce_sum(noobjectness_label) + 0.0001)
    ...
    def yolo_loss(self, outputs, labels):
        ...
        for i in range(3):
            ...
            for idx, val in enumerate(self.anchor[border_a:border_b]):
                ...
                total_loss = objectness_loss + noobjectness_loss + ctr_loss + sz_loss + class_loss
        ...
        self.iou_avg = iou_total / 9.0
        self.obj_acc_avg = obj_acc_total / 9.0
        self.noobj_acc_avg = noobj_acc_total / 9.0
        self.class_acc_avg = class_acc_total / 9.0
        ...
        for i in range(3):
            ...
            for idx_anchor, j in enumerate(self.anchor[border_a:border_b]):
                ...
                for k, l, m, n, o in zip(x, y, w, h, c):
                    ...
                    tmp[(cell_y, cell_x, base + 0)] = (k - cell_x * self.grid_relatif_width[i]) / self.grid_relatif_width[i]
                    tmp[(cell_y, cell_x, base + 1)] = (l - cell_y * self.grid_relatif_height[i]) / self.grid_relatif_height[i]
                    tmp[(cell_y, cell_x, base + 2)] = math.log(m * self.input_width / j[0])
                    tmp[(cell_y, cell_x, base + 3)] = math.log(n * self.input_height / j[1])
                    tmp[(cell_y, cell_x, base + 4)] = 1.0
    ...
    def build_yolov3_net(self, inputs, network_type, is_training):
        ...
        def darknet53_residual_block(inputs, filters, training, data_format, stride=1, name='res'):
            """[summary]
            Arguments:
            ...
            """
            shortcut = inputs
            inputs = new_conv2d_layer(input=inputs, filter_shape=[
             1, 1, inputs.get_shape().as_list()[(-1)], filters],
             ...
             1, stride, stride, 1],
              ...
              use_bias=False,
              use_batchnorm=True)
            inputs = new_conv2d_layer(input=(inputs if stride == 1 else fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], shortcut.get_shape().as_list()[(-1)]],
        ...
        def darknet53(inputs, training, data_format, network_type):
            inputs = new_conv2d_layer(input=inputs, filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], 32],
             ...
             1, 1, 1, 1],
              ...
              use_bias=False,
              use_batchnorm=True)
            inputs = new_conv2d_layer(input=(fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], 64],
            ...
            inputs = darknet53_residual_block(inputs, filters=32,
              ...
              name='res1')
            inputs = new_conv2d_layer(input=(fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], 128],
            ...
            inputs = new_conv2d_layer(input=(fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], 256],
            ...
            if network_type == 'special':
                ...
                if network_type == 'very_small':
                    ...
                    return_vars = tf.global_variables(scope='yolo_v3_model')
            for i in range(8):
                inputs = darknet53_residual_block(inputs, filters=filter_128,
                  training=training,
                  data_format=data_format,
                  name=('res' + str(i + 3)))
            ...
            route1 = inputs
            inputs = new_conv2d_layer(input=(fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], filter_512],
             ...
             1, 2, 2, 1],
              ...
              use_batchnorm=True)
            for i in range(8):
                inputs = darknet53_residual_block(inputs, filters=filter_256,
                  training=training,
                  data_format=data_format,
                  name=('res' + str(i + 11)))
            ...
            route2 = inputs
            inputs = new_conv2d_layer(input=(fixed_padding(inputs, 3, 'channels_last')), filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], filter_1024],
             ...
             1, 2, 2, 1],
              ...
              use_batchnorm=True)
            for i in range(4):
                inputs = darknet53_residual_block(inputs, filters=filter_512, training=training,
                  data_format=data_format,
                  name=('res' + str(i + 19)))

            return (
             route1, route2, inputs, return_vars)
        ...
        def yolo_convolution_block(inputs, filters, training, data_format):
            inputs = new_conv2d_layer(input=inputs, filter_shape=[
             ...
             1, 1, 1, 1],
              ...
              use_batchnorm=True)
            inputs = new_conv2d_layer(input=inputs, filter_shape=[
             3, 3, inputs.get_shape().as_list()[(-1)], 2 * filters],
        ...
        def yolo_layer(inputs, n_classes, anchors, img_size, data_format):
            ...
            inputs = tf.reshape(inputs, [-1, n_anchors * grid_shape[0] * grid_shape[1],
             5 + n_classes])
            strides = (
             img_size[0] // grid_shape[0], img_size[1] // grid_shape[1])
        ...
        if network_type == 'big':
            ...
            if network_type == 'medium':
                ...
                if network_type == 'special' or network_type == 'very_small':
                    route1, route2, inputs, self.yolo_special_vars = darknet53(inputs, training=is_training,
                      data_format=data_format,
                      network_type=network_type)
                    self.yolo_very_small_vars = self.yolo_special_vars
                else:
                    route1, route2, inputs, _ = darknet53(inputs, training=is_training,
                      data_format=data_format,
                      network_type=network_type)
            self.yolo_small_vars = tf.global_variables(scope='yolo_v3_model')
            route, inputs = yolo_convolution_block(inputs, filters=(filters['a']),
              training=is_training,
              data_format=data_format)
            inputs_detect1 = inputs
            inputs = new_conv2d_layer(input=route, filter_shape=[
             1, 1, route.get_shape().as_list()[(-1)], 256],
            ...
            route, inputs = yolo_convolution_block(inputs, filters=(filters['b']),
              training=is_training,
              data_format=data_format)
            inputs_detect2 = inputs
            inputs = new_conv2d_layer(input=route, filter_shape=[
             1, 1, route.get_shape().as_list()[(-1)], 128],
            ...
            route, inputs = yolo_convolution_block(inputs, filters=(filters['c']),
              training=is_training,
              data_format=data_format)
            ...
            self.detect1 = tf.layers.conv2d(inputs_detect1, filters=(len(self.anchor) / 3 * (5 + self.num_class)),
              kernel_size=1,
              strides=1,
              use_bias=True,
              data_format=data_format)
            self.detect2 = tf.layers.conv2d(inputs_detect2, filters=(len(self.anchor) / 3 * (5 + self.num_class)),
              kernel_size=1,
              strides=1,
              use_bias=True,
              data_format=data_format)
            self.detect3 = tf.layers.conv2d(inputs_detect3, filters=(len(self.anchor) / 3 * (5 + self.num_class)),
              kernel_size=1,
              strides=1,
              use_bias=True,
              data_format=data_format)
            self.output_list = [self.detect1, self.detect2, self.detect3]
            combine_box1 = yolo_layer((self.detect1), n_classes=(self.num_class),
              anchors=(self.anchor[6:9]),
              img_size=model_size,
              data_format=data_format)
            combine_box2 = yolo_layer((self.detect2), n_classes=(self.num_class),
              anchors=(self.anchor[3:6]),
              img_size=model_size,
              data_format=data_format)
            combine_box3 = yolo_layer((self.detect3), n_classes=(self.num_class),
