...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simple_tensor/transfer_learning/densenet.py
# Compiled at: 2019-03-28 02:20:52
# Size of source mod 2**32: 7827 bytes
...
@slim.add_arg_scope
def _conv(inputs, num_filters, kernel_size, stride=1, dropout_rate=None, scope=None, outputs_collections=None):
    with tf.variable_scope(scope, 'xx', [inputs]) as (sc):
...
@slim.add_arg_scope
def _dense_block(inputs, num_layers, num_filters, growth_rate, grow_num_filters=True, scope=None, outputs_collections=None):
...
@slim.add_arg_scope
def _transition_block(inputs, num_filters, compression=1.0, scope=None, outputs_collections=None):
...
def densenet(inputs, num_classes=1000, reduction=None, growth_rate=None, num_filters=None, num_layers=None, dropout_rate=None, data_format='NHWC', is_training=True, reuse=None, scope=None):
    if not reduction is not None:
        raise AssertionError
    else:
        if not growth_rate is not None:
            raise AssertionError
        else:
    ...
    with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes], reuse=reuse) as (sc):
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            with slim.arg_scope([slim.conv2d, _conv, _conv_block,
              ...
              outputs_collections=end_points_collection):
                with slim.arg_scope([_conv], dropout_rate=dropout_rate):
                    ...
                    for i in range(num_dense_blocks - 1):
                        net, num_filters = _dense_block(net, (num_layers[i]), num_filters, growth_rate,
                          scope=('dense_block' + str(i + 1)))
                        net, num_filters = _transition_block(net, num_filters, compression=compression,
                          scope=('transition_block' + str(i + 1)))

                    net, num_filters = _dense_block(net,
                      (num_layers[(-1)]), num_filters, growth_rate,
...
def densenet121(inputs, num_classes=1000, data_format='NHWC', is_training=True, reuse=None):
    return densenet(inputs, num_classes=num_classes,
      reduction=0.5,
      growth_rate=32,
      num_filters=64,
      num_layers=[
...
def densenet_arg_scope(weight_decay=0.0001, batch_norm_decay=0.99, batch_norm_epsilon=1.1e-05, data_format='NHWC'):
