...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simple_tensor/segmentation/unet.py
# Compiled at: 2019-09-07 00:42:07
# Size of source mod 2**32: 9449 bytes
...
class UNet:
    ...
    def __init__(self, input_height=200, input_width=200, input_channel=3, is_training=True):
        """[summary]
    ...
    def reducer_block(self, input_tensor):
        ...
        for i in range(4):
            ...
            out = new_conv2d_layer(input, filter_shape=[
             2, 2, input_depth, out_depth],
              name=(str(i) + '_'),
              dropout_val=0.75,
              activation='LRELU',
              strides=[
             1, 1 if i == 0 else 2, 1 if i == 0 else 2, 1],
              is_training=(self.is_training),
              use_bias=True,
              use_batchnorm=True)
            out = new_conv2d_layer(out, filter_shape=[
             2, 2, out_depth, out_depth],
              name=(str(i) + '_'),
              dropout_val=0.75,
              activation='LRELU',
              strides=[
    ...
    def upsampling_block(self, input_tensor):
        ...
        for i in range(4):
            ...
            out, _ = new_deconv_layer(input, filter_shape=[
             3, 3, input_depth, 64],
              output_shape=[
             batch_size, out_height, out_height, 64],
              name=(str(i) + '-'),
              strides=[
             1, std, std, 1],
              use_bias=True)
            ...
            out = new_conv2d_layer(out, filter_shape=[
             2, 2, out.get_shape().as_list()[(-1)], 64],
              name=(str(i) + '_'),
              dropout_val=0.75,
              activation='LRELU',
              strides=[
    ...
    def batch_generator(self, batch_size, input_folder_path, output_folder_path):
        """[summary]
        ...
        while True:
            ...
            for i in range(batch_size):
                ...
                tmp_x = cv2.resize(tmp_x, (self.input_width, self.input_height)).astype(np.float32) / 255.0
                x_batch.append(tmp_x)
                tmp_y = cv2.imread(output_folder_path + train_file_list[idx])
                tmp_y = cv2.cvtColor(tmp_y, cv2.COLOR_BGR2RGB)
                tmp_y = cv2.resize(tmp_y, (self.input_width, self.input_height)).astype(np.float32) / 255.0
