...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/simple_tensor/tensor_operations.py
# Compiled at: 2019-09-07 00:42:07
# Size of source mod 2**32: 15031 bytes
"""
    File name: test.py
...
def new_fc_layer(input, num_inputs, num_outputs, name=None, dropout_val=0.85, activation='LRELU', lrelu_alpha=0.2, data_type=tf.float32, is_training=True, use_bias=True):
    """[summary]
    ...
    if activation in ('RELU', 'relu', 'Relu'):
        ...
        if activation in ('LRELU', 'lrelu', 'Lrelu'):
            layer = tf.nn.leaky_relu(layer, alpha=lrelu_alpha)
        else:
            if activation in ('SELU', 'selu', 'Selu'):
                layer = tf.nn.selu(layer)
            else:
                if activation in ('ELU', 'elu', 'Elu'):
                    layer = tf.nn.elu(layer)
                else:
                    if activation in ('SIGMOID', 'sigmoid', 'Sigmoid'):
                        layer = tf.nn.sigmoid(layer)
                    else:
                        if activation in ('SOFTMAX', 'softmax', 'Softmax'):
                            layer == tf.nn.softmax(layer)
    layer = tf.nn.dropout(layer, dropout_val)
    return layer


def new_conv1d_layer(input, filter_shape, name=None, dropout_val=0.85, activation='LRELU', lrelu_alpha=0.2, padding='SAME', strides=1, data_type=tf.float32, is_training=True, use_bias=True, use_batchnorm=False):
    """[summary]
...
def new_conv2d_layer(input, filter_shape, name=None, dropout_val=0.85, activation='LRELU', lrelu_alpha=0.2, padding='SAME', strides=[
 1, 1, 1, 1], data_type=tf.float32, is_training=True, use_bias=True, use_batchnorm=False):
    """[summary]
...
def new_conv2d_depthwise_layer(input, filter_shape, name=None, dropout_val=0.85, activation='LRELU', lrelu_alpha=0.2, padding='SAME', strides=[
 1, 1, 1, 1], data_type=tf.float32, is_training=True, use_bias=True, use_batchnorm=False):
    """[summary]
    ...
    shape = filter_shape
    weights = new_weights(shape=shape, name=name, data_type=data_type)
    layer = tf.nn.depthwise_conv2d(input=input, filter=weights,
      strides=strides,
      padding=padding,
      name=('convolution_depthwise_' + str(name)))
    if use_bias:
        biases = new_biases(length=(filter_shape[3]), name=name, data_type=data_type)
        layer += biases
    if use_batchnorm:
...
def new_deconv_layer(input, filter_shape, output_shape, name=None, activation='LRELU', lrelu_alpha=0.2, padding='SAME', strides=[
 1, 1, 1, 1], data_type=tf.float32, use_bias=True):
    """[summary]
    ...
    Returns:
        [type] -- [description]
    """
    weights = tf.Variable(tf.truncated_normal(shape=[filter_shape[0], filter_shape[1], filter_shape[3], filter_shape[2]], stddev=0.05), name=('weight_' + name),
      dtype=data_type)
...
def batch_norm(inputs, training, momentum=0.9, epsilon=1e-05, scale=True, data_format='channel_last'):
