...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/agent/cephconfigparser.py
...
class CephConfigParser(manager.Manager):
    ...
    def add_global(self, dict_kvs={}):
        ...
        self._parser.set('global', 'mon osd down out interval', str(dict_kvs['down_out_interval']))
        for key, value in dict_kvs.items():
            if key not in ('max_file', 'is_cephx', 'down_out_interval', 'pool_default_size'):
                self._parser.set('global', key.replace('_', ' '), str(dict_kvs[key]))
    ...
    def add_mds_header(self, dict_kvs={}):
        ...
        for key, value in dict_kvs.items():
            if key not in ('keyring', ):
                self._parser.set('mds', key.replace('_', ' '), str(dict_kvs[key]))
    ...
    def add_mon_header(self, dict_kvs={}):
        if self._parser.has_section('mon'):
            ...
            self._parser.set('mon', 'mon clock drift allowed', '.' + str(dict_kvs['clock_drift']))
            if dict_kvs['cfth']:
            ...
            for key, value in dict_kvs.items():
                if key not in ('clock_drift', 'cnfth', 'cfth'):
                    self._parser.set('mon', key.replace('_', ' '), str(dict_kvs[key]))
    ...
    def add_osd(self, hostname, pub_addr, cluster_addr, osd_dev, journal_dev, osd_id):
        ...
        if self._parser.has_section(sec):
            ...
            self._parser.add_section(sec)
            if hostname is None or pub_addr is None or cluster_addr is None or journal_dev is None or osd_dev is None:
