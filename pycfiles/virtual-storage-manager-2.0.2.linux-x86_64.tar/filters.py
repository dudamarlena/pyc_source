...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/openstack/common/rootwrap/filters.py
...
class CommandFilter(object):
    ...
    def get_command(self, userargs, exec_dirs=[]):
        ...
        if self.run_as != 'root':
            return [
             'sudo', '-u', self.run_as, to_exec] + userargs[1:]
        return [
         to_exec] + userargs[1:]
...
class DnsmasqFilter(CommandFilter):
    ...
    def match(self, userargs):
        if userargs[0] == 'env' and userargs[1].startswith(self.CONFIG_FILE_ARG) and userargs[2].startswith('NETWORK_ID=') and userargs[3] == 'dnsmasq':
            return True
        return False
