...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/scheduler/rpcapi.py
...
class SchedulerAPI(vsm.openstack.common.rpc.proxy.RpcProxy):
    ...
    def __init__(self, topic=None):
        super(SchedulerAPI, self).__init__(topic=topic or CONF.scheduler_topic, default_version=self.BASE_RPC_API_VERSION)
    ...
    def create_cluster(self, context, server_list):
        ret = self.cast(context, self.make_msg('create_cluster', server_list=server_list))
        return ret

    def integrate_cluster(self, context, server_list):
        ret = self.cast(context, self.make_msg('integrate_cluster', server_list=server_list))
        return ret
    ...
    def osd_remove(self, ctxt, body):
        ret = self.call(ctxt, self.make_msg('osd_remove', osd_id=body), version='1.0', timeout=6000)
        return ret

    def osd_restart(self, ctxt, body):
        ret = self.call(ctxt, self.make_msg('osd_restart', osd_id=body), version='1.0', timeout=6000)
        return ret

    def osd_add(self, ctxt, body):
        ret = self.call(ctxt, self.make_msg('osd_add', osd_id=body), version='1.0', timeout=6000)
        return ret

    def osd_restore(self, ctxt, body):
        ret = self.call(ctxt, self.make_msg('osd_restore', osd_id=body), version='1.0', timeout=6000)
        return ret

    def osd_refresh(self, ctxt):
        ret = self.call(ctxt, self.make_msg('osd_refresh'), version='1.0', timeout=6000)
        return ret

    def cluster_refresh(self, ctxt):
        ret = self.call(ctxt, self.make_msg('cluster_refresh'), version='1.0', timeout=6000)
        return ret

    def health_status(self, context):
        ret = self.call(context, self.make_msg('health_status'), version='1.0', timeout=6000)
        return ret

    def import_ceph_conf(self, context, cluster_id, ceph_conf_path):
        ret = self.call(context, self.make_msg('import_ceph_conf', cluster_id=cluster_id, ceph_conf_path=ceph_conf_path))
        return ret

    def start_server(self, context, body=None):
        ret = self.call(context, self.make_msg('start_server', body=body))
        return ret

    def stop_server(self, context, body=None):
        ret = self.call(context, self.make_msg('stop_server', body=body))
        return ret

    def start_cluster(self, context, body=None):
        ret = self.call(context, self.make_msg('start_cluster', body=body))
        return ret

    def stop_cluster(self, context, body=None):
        ret = self.call(context, self.make_msg('stop_cluster', body=body))
        return ret

    def monitor_restart(self, ctxt, body):
        ret = self.call(ctxt, self.make_msg('monitor_restart', monitor_id=body), version='1.0', timeout=6000)
        return ret
    ...
    def rgw_create(self, ctxt, server_name, rgw_instance_name, is_ssl, uid, display_name, email, sub_user, access, key_type):
        return self.call(ctxt, self.make_msg('rgw_create', server_name=server_name, rgw_instance_name=rgw_instance_name, is_ssl=is_ssl, uid=uid, display_name=display_name, email=email, sub_user=sub_user, access=access, key_type=key_type))