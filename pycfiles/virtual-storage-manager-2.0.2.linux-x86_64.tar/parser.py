...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/manifest/parser.py
...
class ManifestParser(object):
    ...
    def __init__(self, fpath=FLAGS.server_manifest, is_server_manifest=True):
        ...
        self._single_type_list = [
         'vsm_controller_ip', 'role']
        self._cluster_names = [
         'storage_class',
    ...
    @classmethod
    def parse_cluster_manifest(cls, fpath=FLAGS.cluster_manifest, is_server_manifest=False):
    ...
    @classmethod
    def parse_server_manifest(cls, fpath=FLAGS.server_manifest, is_server_manifest=True):
    ...
    def _get_cluster_dict(self):
        ...
        ret = {'cluster_name': _get_cluster_value('cluster'), 'file_system': _get_cluster_value('file_system'), 
           'mkfs_option': mkfs_option, 
           'mount_option': mount_option, 
           'public_addr': _get_cluster_value('management_addr'), 
           'secondary_public_addr': _get_cluster_value('ceph_public_addr'), 
    ...
    def _get_segment(self, name):
        ...
        ret = {'name': name, 'single': [], 'first': [], 'second': [], 'third': [], 'fourth': [], 'fifth': []}
        ...
        for pos in range(right_pos + 1, len(self._lines)):
            ...
            if not single_line.startswith('[') and find_error == False:
                ...
                if len(columns) == 1:
                    ...
                    ret['single'] = ret['single'] + [ segs for segs in parts if len(segs) > 0 ]
            else:
                break
    ...
    def _dict_insert_auth_key(self):
        ...
        self._map['auth_key'] = ''
        if len(name_dict['single']) > 1 or len(name_dict['single']) == 0:
    ...
    def _dict_insert_storage_class(self, storage_type):
        ...
        for idx in range(0, len(name_dict['first'])):
            one_disk = {'osd': name_dict['first'][idx], 'journal': name_dict['second'][idx]}
    ...
    def _dict_insert_periodic_parameters_c(self):
        ...
        if len(name_dict['single']) == 0:
            LOG.error('Format error, at least, need set' + 'storage_group_near_full_threshold')
        self._map['storage_group_near_full_threshold'] = name_dict['single']
        name_dict = self._get_segment('storage_group_full_threshold')
        if len(name_dict['single']) == 0:
            LOG.error('Format error, at least, need set' + 'storage_group_full_threshold')
    ...
    def _dict_insert_storage_group_c(self):
        ...
        for idx in range(0, len(name_dict['first'])):
            one_storage_group = {'name': name_dict['first'][idx], 'friendly_name': name_dict['second'][idx], 'storage_class': name_dict['third'][idx]}
    ...
    def _dict_insert_ceph_conf(self):
        ...
        self._map['ceph_conf'] = []
        for idx in range(0, len(name_dict['first'])):
            ceph_conf = {'name': name_dict['first'][idx], 'default_value': name_dict['second'][idx]}
    ...
    def _dict_insert_settings_c(self):
        ...
        self._map['settings'] = []
        for idx in range(0, len(name_dict['first'])):
            setting = {'name': name_dict['first'][idx], 'default_value': name_dict['second'][idx]}
    ...
    def _dict_insert_ec_profiles_c(self):
        ...
        for idx in range(0, len(name_dict['first'])):
            one_ec_profile = {'name': name_dict['first'][idx], 'plugin_path': name_dict['second'][idx], 'plugin': name_dict['third'][idx], 
    ...
    def _trans_between_db(self):
        ...
        self._map['cluster']['ceph_public_network'] = info['secondary_public_addr']
