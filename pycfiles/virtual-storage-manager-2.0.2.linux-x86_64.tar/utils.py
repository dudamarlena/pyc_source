...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/utils.py
# Compiled at: 2016-06-13 14:11:03
"""Utilities and helper functions."""
import base64, contextlib, datetime, errno, functools, hashlib, inspect, os, paramiko, pyclbr, random, re, shlex, shutil, signal, socket, struct, sys, tempfile, time, types, warnings, uuid, json, anyjson, threading
...
def execute(*cmd, **kwargs):
    ...
    if len(kwargs):
        raise exception.Error(_('Got unknown keyword args to utils.execute: %r') % kwargs)
    if run_as_root:
        if FLAGS.rootwrap_config is None or FLAGS.root_helper != 'sudo':
            LOG.deprecated(_('The root_helper option (which lets you specify a root wrapper different from vsm-rootwrap, and defaults to using sudo) is now deprecated. You should use the rootwrap_config option instead.'))
    ...
    while attempts > 0:
        ...
        try:
            try:
                ...
                obj = subprocess.Popen(cmd, stdin=_PIPE, stdout=_PIPE, stderr=_PIPE, close_fds=True, preexec_fn=_subprocess_setup, shell=shell)
                result = None
                ...
                if _returncode:
                    ...
                    if not ignore_exit_code and _returncode not in check_exit_code:
                        ...
                        if cmd_str.lower().find('ceph') != -1 and cmd_str.lower().find('pgrep') == -1 and cmd_str.find('--connect-timeout') == -1:
                            LOG.info('---------CMD-------------')
...
def ssh_execute(ssh, cmd, process_input=None, addl_env=None, check_exit_code=True):
    ...
    if exit_status != -1:
        ...
        if check_exit_code and exit_status != 0:
            raise exception.ProcessExecutionError(exit_code=exit_status, stdout=stdout, stderr=stderr, cmd=cmd)
    channel.close()
...
class SSHPool(pools.Pool):
    ...
    def create(self):
        try:
            ...
            if self.password:
                ssh.connect(self.ip, port=self.port, username=self.login, password=self.password, timeout=self.conn_timeout)
            elif self.privatekey:
                LOG.info(' file path=%s' % self.privatekey)
                pkfile = os.path.expanduser(self.privatekey)
                privatekey = paramiko.RSAKey.from_private_key_file(pkfile)
                ssh.connect(self.ip, port=self.port, username=self.login, pkey=privatekey, timeout=self.conn_timeout)
            else:
...
class SSHClient:
    ...
    def _get_conn(self):
        ssh = SSHPool(ip=self.host, port=22, login=self.login, conn_timeout=self.timeout, privatekey=self.pfile)
        if ssh:
            try:
                return ssh.get()
...
EASIER_PASSWORD_SYMBOLS = ('23456789', 'ABCDEFGHJKLMNPQRSTUVWXYZ')
...
def get_my_linklocal(interface):
    try:
        ...
        if address[0] is not None:
            return address[0]
        raise exception.Error(_('Link Local address is not found.:%s') % if_str)
    except Exception as ex:
        raise exception.Error(_("Couldn't get Link Local IP of %(interface)s :%(ex)s") % locals())
...
def is_valid_boolstr(val):
    ...
    val = str(val).lower()
    return val == 'true' or val == 'false' or val == 'yes' or val == 'no' or val == 'y' or val == 'n' or val == '1' or val == '0'
...
def monkey_patch():
    ...
    Example: 'vsm.api.ec2.cloud:'      vsm.openstack.common.notifier.api.notify_decorator'
...
def timefunc(func):
    ...
    def inner(*args, **kwargs):
        ...
        try:
            ...
            LOG.debug(_("timefunc: '%(name)s' took %(total_time).2f secs") % dict(name=func.__name__, total_time=total_time))
...
def total_seconds(td):
    ...
    if hasattr(td, 'total_seconds'):
        ...
        return ((td.days * 86400 + td.seconds) * 1000000 + td.microseconds) / 1000000.0
...
def write_file_as_root(file_path, content, open_type='a+'):
    """Secure helper to read file as root."""
    try:
        out, _err = execute('vsm-assist', 'write_file', file_path, content, open_type, run_as_root=True)
        return out
    except exception.ProcessExecutionError:
        raise exception.FileNotFound(file_path=file_path)
...
def file_is_exist_as_root(file_path):
    """Secure helper to read file as root."""
    try:
        out, _err = execute('vsm-assist', 'checkfile', file_path, run_as_root=True)
        if out.find('1') != -1:
            return True
        return
    except exception.ProcessExecutionError:
...
def gen_ssh_key(username='vsm', hostname=None):
    """ Generate ssh key."""
    try:
        out, _err = execute('vsm-assist', 'checkfile', FLAGS.id_rsa_pub, run_as_root=True)
        ...
        if out[0] == '0':
            ...
            execute('ssh-keygen', '-f', FLAGS.key_name, '-N', '', '-C', str, run_as_root=True)
    except:
        raise
...
def check_string_length(value, name, min_length=0, max_length=None):
    ...
    if len(value) < min_length:
        msg = _('%(name)s has less than %(min_length)s characters.') % locals()
        raise exception.InvalidInput(message=msg)
    if max_length and len(value) > max_length:
        msg = _('%(name)s has more than %(max_length)s characters.') % locals()
        raise exception.InvalidInput(message=msg)
...
def clean_dirs(dir_path):
    try:
        ...
        for f in files:
            try:
                execute('rm', '-rf', dir_path + '/' + f, run_as_root=True)
            except:
    ...
    try:
        ...
        for file in files.split():
            if file.find(prog_name) != -1:
                execute('rm', '-rf', os.path.join(FLAGS.state_path, file), run_as_root=True)
