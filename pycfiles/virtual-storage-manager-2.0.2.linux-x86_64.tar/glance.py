...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/image/glance.py
# Compiled at: 2016-06-13 14:11:03
"""Implementation of an image service that uses Glance as the backend"""
from __future__ import absolute_import
import copy, itertools, random, sys, time, urlparse, glanceclient, glanceclient.exc
...
class GlanceClientWrapper(object):
    ...
    def __init__(self, context=None, host=None, port=None, use_ssl=False, version=None):
        if host is not None:
    ...
    def _create_static_client(self, context, host, port, use_ssl, version):
        ...
        self.version = version
        return _create_glance_client(context, self.host, self.port, self.use_ssl, self.version)
    ...
    def _create_onetime_client(self, context, version):
        ...
        return _create_glance_client(context, self.host, self.port, self.use_ssl, version)
    ...
    def call(self, context, method, *args, **kwargs):
        ...
        for attempt in xrange(1, num_attempts + 1):
            ...
            try:
                ...
                port = self.port
                extra = 'retrying'
                error_msg = _("Error contacting glance server '%(host)s:%(port)s' for '%(method)s', %(extra)s.")
                if attempt == num_attempts:
                    extra = 'done trying'
                    LOG.exception(error_msg, locals())
                    raise exception.GlanceConnectionFailed(host=host, port=port, reason=str(e))
                LOG.exception(error_msg, locals())
                time.sleep(1)
...
def _remove_read_only(image_meta):
    IMAGE_ATTRIBUTES = [
     'status', 'updated_at', 'created_at', 'deleted_at']
