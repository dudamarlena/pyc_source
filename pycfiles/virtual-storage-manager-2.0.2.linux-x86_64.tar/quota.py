...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/quota.py
...
quota_opts = [
 cfg.IntOpt('quota_storages', default=10, help='number of storages allowed per project'),
 cfg.IntOpt('quota_snapshots', default=10, help='number of storage snapshots allowed per project'),
 cfg.IntOpt('quota_gigabytes', default=1000, help='number of storage gigabytes (snapshots are also included) allowed per project'),
 cfg.IntOpt('reservation_expire', default=86400, help='number of seconds until a reservation expires'),
 cfg.IntOpt('until_refresh', default=0, help='count of reservations until usage is refreshed'),
 cfg.IntOpt('max_age', default=0, help='number of seconds between subsequent usage refreshes'),
 cfg.StrOpt('quota_driver', default='vsm.quota.DbQuotaDriver', help='default driver to use for quota checks')]
FLAGS = flags.FLAGS
...
class DbQuotaDriver(object):
    ...
    def get_project_quotas(self, context, resources, project_id, quota_class=None, defaults=True, usages=True):
        ...
        for resource in resources.values():
            if not defaults and resource.name not in project_quotas:
                continue
            quotas[resource.name] = dict(limit=project_quotas.get(resource.name, class_quotas.get(resource.name, resource.default)))
            if usages:
    ...
    def reserve(self, context, resources, deltas, expire=None, project_id=None):
        ...
        return db.quota_reserve(context, resources, quotas, deltas, expire, FLAGS.until_refresh, FLAGS.max_age, project_id=project_id)
...
def _sync_storages(context, project_id, session):
    storages, gigs = db.storage_data_get_for_project(context, project_id, session=session)
    return {'storages': storages}


def _sync_snapshots(context, project_id, session):
    snapshots, gigs = db.snapshot_data_get_for_project(context, project_id, session=session)
    return {'snapshots': snapshots}


def _sync_gigabytes(context, project_id, session):
    _junk, vol_gigs = db.storage_data_get_for_project(context, project_id, session=session)
    if FLAGS.no_snapshot_gb_quota:
        return {'gigabytes': vol_gigs}
    _junk, snap_gigs = db.snapshot_data_get_for_project(context, project_id, session=session)
