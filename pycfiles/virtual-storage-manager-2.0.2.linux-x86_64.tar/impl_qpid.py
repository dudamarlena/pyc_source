...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/openstack/common/rpc/impl_qpid.py
# Compiled at: 2016-06-13 14:11:03
import functools, itertools, time, uuid, eventlet, greenlet
...
qpid_opts = [
 ...
 cfg.StrOpt('qpid_port', default='5673', help='Qpid broker port'),
 cfg.ListOpt('qpid_hosts', default=[
  '$qpid_hostname:$qpid_port'], help='Qpid HA cluster host:port pairs'),
...
class ConsumerBase(object):
    ...
    def __init__(self, session, callback, node_name, node_opts, link_name, link_opts):
        ...
        self.receiver = None
        self.session = None
        addr_opts = {'create': 'always', 
           'node': {'type': 'topic', 
                    'x-declare': {'durable': True, 
                                  'auto-delete': True}}, 
           'link': {'name': link_name, 
                    'durable': True, 
                    'x-declare': {'durable': False, 
                                  'auto-delete': True, 
                                  'exclusive': False}}}
...
class Connection(object):
    ...
    def __init__(self, conf, server_params=None):
        ...
        params = {'qpid_hosts': self.conf.qpid_hosts, 
    ...
    def reconnect(self):
        ...
        while True:
            ...
            try:
                ...
                msg = _('Unable to connect to AMQP server: %(e)s. Sleeping %(delay)s seconds') % msg_dict
                LOG.error(msg)
                time.sleep(delay)
    ...
    def ensure(self, error_callback, method, *args, **kwargs):
        while True:
            ...
            except (qpid_exceptions.Empty, qpid_exceptions.ConnectionError) as e:
                if error_callback:
                    error_callback(e)
                self.reconnect()
    ...
    def join_consumer_pool(self, callback, pool_name, topic, exchange_name=None):
        ...
        consumer = TopicConsumer(conf=self.conf, session=self.session, topic=topic, callback=callback_wrapper, name=pool_name, exchange_name=exchange_name)
        self._register_consumer(consumer)
        return consumer
