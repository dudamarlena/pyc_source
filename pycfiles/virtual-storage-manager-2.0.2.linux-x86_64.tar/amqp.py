...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/openstack/common/rpc/amqp.py
...
"""
import collections, inspect, sys, uuid
...
amqp_opts = [
 cfg.BoolOpt('amqp_rpc_single_reply_queue', default=False, help='Enable a fast single reply queue if using AMQP based RPC like RabbitMQ or Qpid.')]
...
class ConnectionContext(rpc_common.Connection):
    ...
    def join_consumer_pool(self, callback, pool_name, topic, exchange_name):
        self.connection.join_consumer_pool(callback, pool_name, topic, exchange_name)
...
class ReplyProxy(ConnectionContext):
    ...
    def _process_data(self, message_data):
        ...
        if not waiter:
            LOG.warn(_('no calling threads waiting for msg_id : %s, message : %s') % (
             msg_id, message_data))
        else:
...
def msg_reply(conf, msg_id, reply_q, connection_pool, reply=None, failure=None, ending=False, log_failure=True):
    ...
    with ConnectionContext(conf, connection_pool) as (conn):
        ...
        try:
            ...
            msg = {'result': dict((k, repr(v)) for k, v in reply.__dict__.iteritems()), 
               'failure': failure}
...
class RpcContext(rpc_common.CommonRpcContext):
    ...
    def reply(self, reply=None, failure=None, ending=False, connection_pool=None, log_failure=True):
        if self.msg_id:
            msg_reply(self.conf, self.msg_id, self.reply_q, connection_pool, reply, failure, ending, log_failure)
            if ending:
                self.msg_id = None
        return
...
class ProxyCallback(_ThreadPoolWithWait):
    ...
    def __init__(self, conf, proxy, connection_pool):
        super(ProxyCallback, self).__init__(conf=conf, connection_pool=connection_pool)
        self.proxy = proxy
...
def cast_to_server(conf, context, server_params, topic, msg, connection_pool):
    ...
    _add_unique_id(msg)
    pack_context(msg, context)
    with ConnectionContext(conf, connection_pool, pooled=False, server_params=server_params) as (conn):
        conn.topic_send(topic, rpc_common.serialize_msg(msg))
