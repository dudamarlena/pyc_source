...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/ipcalc.py
...
class IP(object):
    ...
    _range = {4: {'01': 'CLASS A', 
           ...
           '1111': 'CLASS E', 
           '00001010': 'PRIVATE RFC1918', 
           '101011000001': 'PRIVATE RFC1918', 
           '1100000010101000': 'PRIVATE RFC1918'}, 
       6: {'00000000': 'RESERVED', 
           '00000001': 'UNASSIGNED', 
           '0000001': 'NSAP', 
           '0000010': 'IPX', 
           ...
           '0010000000000010': '6TO4', 
           '0011111111111110': '6BONE TEST', 
           '0011111111111111': 'RESERVED', 
           '010': 'GLOBAL-UNICAST', 
           '011': 'UNASSIGNED', 
           '100': 'GEO-UNICAST', 
           '101': 'UNASSIGNED', 
           ...
           '111110': 'UNASSIGNED', 
           '1111110': 'UNASSIGNED', 
           '111111100': 'UNASSIGNED', 
           '1111111010': 'LINKLOCAL', 
           '1111111011': 'SITELOCAL', 
           '11111111': 'MULTICAST', 
           '0' * 96: 'IPV4COMP', 
           '0' * 80 + '1111111111111111': 'IPV4MAP', 
           '0' * 128: 'UNSPECIFIED', 
           '0' * 127 + '1': 'LOOPBACK'}}
    ...
    def _dqtoi(self, dq):
        ...
        if ':' in dq:
            ...
            if len(hx) > 8:
                ...
                if len(hx) < 8:
                    if '' not in hx:
                        raise ValueError('%s: IPv6 address invalid: compressed format malformed' % dq)
                    else:
                        if not (dq.startswith('::') or dq.endswith('::')) and len([ x for x in hx if x == '' ]) > 1:
                            raise ValueError('%s: IPv6 address invalid: compressed format malformed' % dq)
                        ix = hx.index('')
                        px = len(hx[ix + 1:])
                    ...
                    raise ValueError('%s: IPv6 address invalid: compressed format detected in full notation' % dq())
                ip = ''
                hx = [ x == '' and '0' or x for x in hx ]
                for h in hx:
                    if len(h) < 4:
                        h = '%04x' % int(h, 16)
                    if not 0 <= int(h, 16) <= 65535:
                        raise ValueError('%r: IPv6 address invalid: hexlets should be between 0x0000 and 0xffff' % dq)
                    ip += h
        ...
        if '.' in dq:
            ...
            for x in q:
                if not 0 <= int(x) <= 255:
                    raise ValueError('%s: IPv4 address invalid: bytes should be between 0 and 255' % dq)
    ...
    def to_ipv4(self):
        ...
        if self.v == 4:
            ...
            if self.bin().startswith('0' * 80 + '1111111111111111'):
                return IP(long(self) & 4294967295, version=4)
            if long(self) & 42545680458834377588178886921629466624:
                return IP(long(self) - 42545680458834377588178886921629466624 >> 80, version=4)
            return ValueError('%s: IPv6 address is not IPv4 compatible or mapped, nor an 6-to-4 IP' % self.dq)
    ...
    def to_ipv6(self, type='6-to-4'):
        ...
        if self.v == 4:
            if type == '6-to-4':
                return IP(42545680458834377588178886921629466624 | long(self) << 80, version=6)
            if type == 'compat':
                return IP(long(self), version=6)
            if type == 'mapped':
                return IP(281470681743360 | long(self), version=6)
        else:
            return self
...
class Network(IP):
    ...
    def netmask_long(self):
        ...
        """
        if self.version() == 4:
            return 4294967295 >> 32 - self.mask << 32 - self.mask
        else:
            return 340282366920938463463374607431768211455 >> 128 - self.mask << 128 - self.mask
    ...
    def broadcast_long(self):
        ...
        """
        if self.version() == 4:
            return self.network_long() | 4294967295 - self.netmask_long()
        else:
            return self.network_long() | 340282366920938463463374607431768211455 - self.netmask_long()

    def host_first(self):
        """
        First available host in this subnet.
        """
        if self.version() == 4 and self.mask > 30 or self.version() == 6 and self.mask > 126:
            return self
    ...
    def in_network(self, other):
        ...
        """
        warnings.warn('%s.in_network is deprecated, use check_collision in stead' % (
         self.__class__.__name__,), DeprecationWarning, stacklevel=2)
        other = Network(other)
    ...
    def check_collision(self, other):
        ...
        """
        other = Network(other)
        return self.network_long() <= other.network_long() <= self.broadcast_long() or other.network_long() <= self.network_long() <= other.broadcast_long()
...
if __name__ == '__main__':
    tests = [('192.168.114.42', 23, ['192.168.0.1', '192.168.114.128', '10.0.0.1']),
     (
