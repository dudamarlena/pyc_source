...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/openstack/common/rpc/impl_zmq.py
# Compiled at: 2016-06-13 14:11:03
import os, pprint, re, socket, sys, types, uuid, eventlet, greenlet
...
zmq_opts = [
 cfg.StrOpt('rpc_zmq_bind_address', default='*', help='ZeroMQ bind address. Should be a wildcard (*), an ethernet interface, or IP. The "host" option should point or resolve to this address.'),
 cfg.StrOpt('rpc_zmq_matchmaker', default='vsm.openstack.common.rpc.matchmaker.MatchMakerLocalhost', help='MatchMaker driver'),
 cfg.IntOpt('rpc_zmq_port', default=9501, help='ZeroMQ receiver listening port'),
 cfg.IntOpt('rpc_zmq_contexts', default=1, help='Number of ZeroMQ contexts, defaults to 1'),
 cfg.IntOpt('rpc_zmq_topic_backlog', default=None, help='Maximum number of ingress messages to locally buffer per topic. Default is unlimited.'),
 cfg.StrOpt('rpc_zmq_ipc_dir', default='/var/run/openstack', help='Directory for holding IPC sockets'),
 cfg.StrOpt('rpc_zmq_host', default=socket.gethostname(), help='Name of this node. Must be a valid hostname, FQDN, or IP address. Must match "host" option, if running Nova.')]
CONF = cfg.CONF
...
class ZmqSocket(object):
    ...
    def __init__(self, addr, zmq_type, bind=True, subscribe=None):
        ...
        do_sub = {list: subscribe, 
           str: [
               subscribe], 
           type(None): []}[type(subscribe)]
        for f in do_sub:
            self.subscribe(f)

        str_data = {'addr': addr, 'type': self.socket_s(), 'subscribe': subscribe, 
           'bind': bind}
...
class InternalContext(object):
    ...
    def reply(self, ctx, proxy, msg_id=None, context=None, topic=None, msg=None):
        ...
        _multi_send(_cast, ctx, topic, {'method': '-process_reply', 
           'args': {'msg_id': msg_id, 
                    'response': response}}, _msg_id=msg_id)
    ...
    def process(self, proxy, ctx, data):
        ...
        if not method:
            ...
            if method == '-reply':
                ...
                return
            proxy.dispatch(ctx, data['version'], data['method'], **data['args'])
            return
...
class ZmqBaseReactor(ConsumerBase):
    ...
    def register(self, proxy, in_addr, zmq_type_in, out_addr=None, zmq_type_out=None, in_bind=True, out_bind=True, subscribe=None):
    ...
    def consume_in_thread(self):
        ...
        for k in self.proxies.keys():
            self.threads.append(self.pool.spawn(_consume, k))
...
class ZmqProxy(ZmqBaseReactor):
    ...
    def __init__(self, conf):
        ...
        self.badchars = re.compile('[%s]' % re.escape(('').join(pathsep)))
        self.topic_proxy = {}
    ...
    def consume(self, sock):
        ...
        if topic.startswith('fanout~'):
                ...
                def publisher(waiter):
                    ...
                    try:
                        ...
                        out_sock = ZmqSocket('ipc://%s/zmq_topic_%s' % (
                         ipc_dir, topic), sock_type, bind=True)
                    except RPCException:
            ...
            try:
                ...
                LOG.error(_('Local per-topic backlog buffer full for topic %(topic)s. Dropping message.') % {'topic': topic})
        ...
        try:
            try:
                msg_waiter = ZmqSocket('ipc://%s/zmq_topic_zmq_replies.%s' % (
                 ...
                 CONF.rpc_zmq_host), zmq.SUB, subscribe=msg_id, bind=False)
