...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/diamond/collectors/cephmetrics/cephmetrics.py
# Compiled at: 2016-06-13 14:11:03
import diamond.collector, commands
try:
    import json
except ImportError:
...
class CephMetricsCollector(diamond.collector.Collector):
    ...
    def get_default_config_help(self):
        ...
        config_help.update({'socket_path': 'The location of the ceph monitoring sockets. Defaults to "/var/run/ceph"', 
           'socket_prefix': 'The first part of all socket names. Defaults to "ceph-"', 
           'socket_ext': 'Extension for socket filenames. Defaults to "asok"', 
           'ceph_binary': 'Path to "ceph" executable. Defaults to /usr/bin/ceph.'})
        return config_help
    ...
    def get_default_config(self):
        ...
        config.update({'socket_path': '/var/run/ceph', 
           'socket_prefix': 'ceph-', 
           'socket_ext': 'asok', 
    ...
    def _get_socket_paths(self, type='all'):
        ...
        """
        if type == 'all':
            socket_pattern = os.path.join(self.config['socket_path'], self.config['socket_prefix'] + '*.' + self.config['socket_ext'])
        else:
            socket_pattern = os.path.join(self.config['socket_path'], self.config['socket_prefix'] + '%s.*.' % type + self.config['socket_ext'])
    ...
    def collect(self):
        ...
        for path in self._get_socket_paths(type='osd'):
            ...
            metrics = {'osd%s.ops_r' % instance_name: osd_perf_value_dict['op_r'], 
