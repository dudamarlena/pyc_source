...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/db/sqlalchemy/models.py
...
class HardwareMetadata(BASE, VsmBase):
    ...
    storage = relationship(Hardware, backref='storage_metadata', foreign_keys=storage_id, primaryjoin='and_(HardwareMetadata.storage_id == Hardware.id,HardwareMetadata.deleted == False)')
...
class HardwareTypes(BASE, VsmBase):
    ...
    storages = relationship(Hardware, backref=backref('storage_type', uselist=False), foreign_keys=id, primaryjoin='and_(Hardware.storage_type_id == HardwareTypes.id, HardwareTypes.deleted == False)')
...
class HardwareTypeExtraSpecs(BASE, VsmBase):
    ...
    key = Column(String(255))
    value = Column(String(255))
    storage_type_id = Column(String(36), ForeignKey('storage_types.id'), nullable=False)
    storage_type = relationship(HardwareTypes, backref='extra_specs', foreign_keys=storage_type_id, primaryjoin='and_(HardwareTypeExtraSpecs.storage_type_id == HardwareTypes.id,HardwareTypeExtraSpecs.deleted == False)')
...
class HardwareGlanceMetadata(BASE, VsmBase):
    ...
    key = Column(String(255))
    value = Column(Text)
    storage = relationship(Hardware, backref='storage_glance_metadata', foreign_keys=storage_id, primaryjoin='and_(HardwareGlanceMetadata.storage_id == Hardware.id,HardwareGlanceMetadata.deleted == False)')
...
class Migration(BASE, VsmBase):
    ...
    instance_uuid = Column(String(255), ForeignKey('instances.uuid'), nullable=True)
    status = Column(String(255))
...
class SMHardware(BASE, VsmBase):
    ...
    backend_id = Column(Integer, ForeignKey('sm_backend_config.id'), nullable=False)
...
class Device(BASE, VsmBase):
    ...
    service = relationship(Service, backref=backref('device'), foreign_keys=service_id, primaryjoin='and_(Device.service_id == Service.id,Device.deleted == False)')
...
class OsdState(BASE, VsmBase):
    ...
    id = Column(Integer, primary_key=True)
    service_id = Column(Integer, ForeignKey('services.id'), nullable=False)
    service = relationship(Service, backref=backref('osd_state'), lazy='subquery', foreign_keys=service_id, primaryjoin='and_(OsdState.service_id == Service.id,OsdState.deleted == False)')
    zone_id = Column(Integer, ForeignKey('zones.id'), nullable=False)
    zone = relationship(Zone, backref=backref('osd_state'), foreign_keys=zone_id, lazy='subquery', primaryjoin='and_(OsdState.zone_id== Zone.id,OsdState.deleted == False)')
    device_id = Column(Integer, ForeignKey('devices.id'), nullable=False)
    device = relationship(Device, backref=backref('osd_state'), lazy='subquery', foreign_keys=device_id, primaryjoin='and_(OsdState.device_id == Device.id,OsdState.deleted == False)')
    storage_group_id = Column(Integer, ForeignKey('storage_groups.id'), nullable=False)
    storage_group = relationship(StorageGroup, backref=backref('osd_state'), lazy='subquery', foreign_keys=storage_group_id, primaryjoin='and_(OsdState.storage_group_id == StorageGroup.id,OsdState.deleted == False)')
...
class StoragePool(BASE, VsmBase):
    ...
    recipes = relationship(Recipe, backref=backref('storage_pool'), foreign_keys=recipe_id, primaryjoin='and_(StoragePool.recipe_id == Recipe.id,StoragePool.deleted == False)')
    cluster_id = Column(Integer, ForeignKey('clusters.id'), nullable=False)
    cluster = relationship(Cluster, backref=backref('storage_pool'), foreign_keys=cluster_id, primaryjoin='and_(StoragePool.cluster_id == Cluster.id,StoragePool.deleted == False)')
    ...
    storage_group = relationship(StorageGroup, backref=backref('storage_pool'), lazy='subquery', foreign_keys=primary_storage_group_id, primaryjoin='and_(StoragePool.primary_storage_group_id == StorageGroup.id,StoragePool.deleted == False)')
    crash_replay_interval = Column(Integer)
...
class InitNode(BASE, VsmBase):
    ...
    service_id = Column(Integer, ForeignKey('services.id'), nullable=False)
    service = relationship(Service, backref=backref('init_node'), foreign_keys=service_id, primaryjoin='and_(InitNode.service_id == Service.id,InitNode.deleted == False)')
    ...
    zone = relationship(Zone, backref=backref('init_node'), foreign_keys=zone_id, primaryjoin='and_(InitNode.zone_id== Zone.id,InitNode.deleted == False)')
    ...
    cluster_id = Column(Integer, ForeignKey('clusters.id'), nullable=False)
    cluster = relationship(Cluster, backref=backref('init_node'), foreign_keys=cluster_id, primaryjoin='and_(InitNode.cluster_id == Cluster.id,InitNode.deleted == False)')
...
class Appnode(BASE, VsmBase):
    ...
    vsmapp = relationship(Vsmapp, backref=backref('appnode'), foreign_keys=vsmapp_id, primaryjoin='and_(Appnode.vsmapp_id == Vsmapp.id,Appnode.deleted == False)')
...
class StoragePoolUsage(BASE, VsmBase):
    ...
    id = Column(Integer, primary_key=True, nullable=False)
    pool_id = Column(Integer, ForeignKey(StoragePool.id), nullable=False)
    vsmapp_id = Column(Integer, ForeignKey(Vsmapp.id), nullable=False)
    ...
    pools = relationship(StoragePool, backref=backref('storage_pool_usage'), foreign_keys=pool_id, primaryjoin='and_(StoragePoolUsage.pool_id == StoragePool.id,StoragePoolUsage.deleted == False)')
    attach_at = Column(DateTime, default=timeutils.utcnow)
    terminate_at = Column(DateTime)
    vsmapps = relationship(Vsmapp, backref=backref('storage_pool_usage'), foreign_keys=vsmapp_id, primaryjoin='and_(StoragePoolUsage.vsmapp_id == Vsmapp.id,StoragePoolUsage.deleted == False)')
...
class Summary(BASE, VsmBase):
    ...
    cluster = relationship(Cluster, backref=backref('summary'), foreign_keys=cluster_id, primaryjoin='and_(Summary.cluster_id == Cluster.id,Summary.deleted == False)')
