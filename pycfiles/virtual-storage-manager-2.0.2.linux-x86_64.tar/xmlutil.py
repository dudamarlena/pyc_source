...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/api/xmlutil.py
...
XMLNS_VOLUME_V1 = 'http://docs.openstack.org/storage/api/v1'
XMLNS_VOLUME_V2 = 'http://docs.openstack.org/api/openstack-storage/2.0/content'

def validate_schema(xml, schema_name):
    if isinstance(xml, str):
        xml = etree.fromstring(xml)
    base_path = 'vsm/api/schemas/v1.1/'
    if schema_name in ('atom', 'atom-link'):
        base_path = 'vsm/api/schemas/'
    schema_path = os.path.join(utils.vsmdir(), '%s%s.rng' % (base_path, schema_name))
...
class TemplateElement(object):
    ...
    def tree(self):
        ...
        """
        contents = [
         self.tag, '!selector=%r' % self.selector]
        if self.text is not None:
...
class MasterTemplate(Template):
    ...
    def _siblings(self):
        ...
        """
        return [
         self.root] + [ slave.root for slave in self.slaves ]
    ...
    def attach(self, *slaves):
        ...
        for slave in slaves:
            ...
            if slave.root.tag != self.root.tag:
                ...
                msg = _('Template tree mismatch; adding slave %(slavetag)s to master %(mastertag)s') % locals()
