...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/vsm/api/urlmap.py
# Compiled at: 2016-06-13 14:11:03
import paste.urlmap, re, urllib2
from vsm.api.openstack import wsgi
from vsm.openstack.common import log as logging
_quoted_string_re = '"[^"\\\\]*(?:\\\\.[^"\\\\]*)*"'
_option_header_piece_re = re.compile(';\\s*([^\\s;=]+|%s)\\s*(?:=\\s*([^;]+|%s))?\\s*' % (
...
class Accept(object):
    ...
    def __init__(self, value):
        self._content_types = [ parse_options_header(v) for v in parse_list_header(value)
                              ]
...
class URLMap(paste.urlmap.URLMap):
    ...
    def __call__(self, environ, start_response):
        ...
        if ':' in host:
            ...
            if app:
                environ['vsm.best_content_type'] = mime_type
                return app(environ, start_response)
