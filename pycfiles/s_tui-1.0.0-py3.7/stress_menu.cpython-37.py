...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/s_tui/stress_menu.py
...
import re, logging, psutil, urwid
...
class StressMenu:
    ...
    def get_size(self):
        return (
         len(self.titles) + 5, self.MAX_TITLE_LEN)
    ...
    def get_pos_num(num, default):
        num_valid = re.match('\\A([0-9]+)\\Z', num, re.I)
        if (num_valid or num) == 'none':
            if default == 'none':
                return num
        return default
    ...
    def get_valid_byte(num, default):
        ...
        etc.. """
        num_valid = re.match('\\A([0-9]+)(M|G|m|g|)(B|b|\\b)\\Z', num, re.I)
        if num_valid:
            return num
        return default