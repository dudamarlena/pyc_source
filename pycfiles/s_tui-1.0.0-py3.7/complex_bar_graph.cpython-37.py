...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/s_tui/sturwid/complex_bar_graph.py
...
class ScalableBarGraph(urwid.BarGraph):
    __doc__ = 'Scale the graph acording to screen size'
    _size = (0, 0)
    ...
    def calculate_bar_widths(self, size, bardata):
        ...
        if self.bar_width is not None:
            return [
             self.bar_width] * min(len(bardata), int(maxcol / self.bar_width))
...
class LabeledBarGraphVector(urwid.WidgetPlaceholder):
    __doc__ = 'Add option to add labels for X and Y axes '
    ...
    def set_y_label(self, y_label):
        ...
        self.y_label = (
         'fixed', y_scale_len, y_list_walker)
    ...
    def set_visible_graphs(self, visible_graph_list=None):
        ...
        if visible_graph_list is None:
            ...
            self.visible_graph_list = graph_vector_column_list or visible_graph_list
    ...
    def check_label(label):
        if not len(label) >= 2 and None not in label or label or label is None:
            return True
        return False