...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/s_tui/s_tui.py
# Compiled at: 2019-12-27 09:31:39
# Size of source mod 2**32: 36026 bytes
"""CPU stress and monitoring utility"""
from __future__ import absolute_import
import argparse, signal, itertools, logging, os, subprocess, time, timeit
from collections import OrderedDict
from collections import defaultdict
import sys, psutil, urwid
try:
    import configparser
except ImportError:
...
VERSION_MESSAGE = 's-tui ' + __version__ + ' - (C) 2017-2019 Alex Manuskin, Gil Tsuker\n    Released under GNU GPLv2'
ERROR_MESSAGE = '\n        Oops! s-tui has encountered a fatal error\n        Please report this bug here: https://github.com/amanusk/s-tui'
graph_controller = None

class MainLoop(urwid.MainLoop):
    __doc__ = ' Inherit urwid Mainloop to catch special character inputs'
...
class StressController:
    ...
    def start_stress(self, stress_cmd):
        ...
        with open(os.devnull, 'w') as (dev_null):
            try:
                stress_proc = subprocess.Popen(stress_cmd, stdout=dev_null, stderr=dev_null)
...
class GraphView(urwid.WidgetPlaceholder):
    ...
    def __init__(self, controller):
        ...
        self.refresh_rate_ctrl = urwid.Edit('Refresh[s]:', self.controller.refresh_rate)
        ...
        self.graphs_menu = SensorsMenu(self.on_graphs_menu_close, self.controller.sources, self.controller.graphs_default_conf)
        self.summary_menu = SensorsMenu(self.on_summary_menu_close, self.controller.sources, self.controller.summary_default_conf)
    ...
    def update_displayed_information(self):
        ...
        for source in self.controller.sources:
            ...
            if any(self.graphs_menu.active_sensors[source_name]) or any(self.summary_menu.active_sensors[source_name]):
                source.update()
        ...
        if self.controller.stress_conroller.get_current_mode() != 'Monitor':
            self.clock_view.set_text(seconds_to_text(timeit.default_timer() - self.controller.stress_start_time))
    ...
    def on_graphs_menu_close(self, update):
        ...
        if update:
            for sensor, visible_sensors in self.graphs_menu.active_sensors.items():
                ...
                if sensor in self.visible_graphs:
                    del (any(visible_sensors) or self.visible_graphs)[sensor]
                else:
    ...
    def on_summary_menu_close(self, update):
        ...
        self.main_window_w.base_widget[0].body[self.summary_widget_index] = self._generate_summaries()
        self.original_widget = self.main_window_w

    def on_stress_menu_open(self, widget):
        """Open stress options"""
        self.original_widget = urwid.Overlay(self.stress_menu.main_window, self.original_widget, (
         'relative', self.left_margin), self.stress_menu.get_size()[1], (
         'relative', self.top_margin), self.stress_menu.get_size()[0])

    def on_help_menu_open(self, widget):
        """Open Help menu"""
        self.original_widget = urwid.Overlay(self.help_menu.main_window, self.original_widget, (
         'relative', self.left_margin), self.help_menu.get_size()[1], (
         'relative', self.top_margin), self.help_menu.get_size()[0])

    def on_about_menu_open(self, widget):
        """Open About menu"""
        self.original_widget = urwid.Overlay(self.about_menu.main_window, self.original_widget, (
         'relative', self.left_margin), self.about_menu.get_size()[1], (
         'relative', self.top_margin), self.about_menu.get_size()[0])

    def on_graphs_menu_open(self, widget):
        """Open Sensor menu on top of existing frame"""
        self.original_widget = urwid.Overlay(self.graphs_menu.main_window, self.original_widget, (
         'relative', self.left_margin), self.graphs_menu.get_size()[1], (
         'relative', self.top_margin), self.graphs_menu.get_size()[0])

    def on_summary_menu_open(self, widget):
        """Open Sensor menu on top of existing frame"""
        self.original_widget = urwid.Overlay(self.summary_menu.main_window, self.original_widget, (
         'relative', self.left_margin), self.summary_menu.get_size()[1], (
         'relative', self.top_margin), self.summary_menu.get_size()[0])
    ...
    def _generate_graph_controls(self):
        ...
        if self.controller.stress_exe:
            ...
            if urwid.get_encoding_mode() == 'utf8':
                unicode_checkbox = urwid.CheckBox('UTF-8',
                  state=default_smooth, on_state_change=(self.on_unicode_checkbox))
        ...
        if not self.controller.stress_exe:
            install_stress_message = urwid.Text(('button normal', '(N/A) install stress'))
        ...
        controls += [
         ...
         self.clock_view,
         urwid.Divider(),
         urwid.Text(('bold text', 'Control Options'), align='center'),
         animate_controls,
         urwid.Divider(),
    ...
    def _generate_cpu_stats():
        ...
        return [urwid.Text(('bold text', 'CPU Detected'), align='center'), cpu_name, urwid.Divider()]
    ...
    def show_graphs(self):
        ...
        elements = itertools.chain.from_iterable(([graph] for graph in self.visible_graphs.values()))
    ...
    def main_window(self):
        ...
        for source in self.controller.sources:
            ...
            self.graphs[source_name] = BarGraphVector(source,
              color_pallet, (len(source.get_sensor_list())),
              (self.graphs_menu.active_sensors[source_name]),
              alert_colors=alert_pallet)
            if self.controller.script_hooks_enabled:
                source.add_edge_hook(self.controller.script_loader.load_script(source.__class__.__name__, HOOK_INTERVAL))
            self.summaries[source_name] = SummaryTextList(self.graphs[source_name].source, self.summary_menu.active_sensors[source_name])

        self.visible_graphs = OrderedDict(((key, val) for key, val in self.graphs.items() if val.get_is_available()))
        for key in self.graphs.keys():
            if not any(self.graphs_menu.active_sensors[key]):
                del self.visible_graphs[key]

        self.visible_summaries = OrderedDict(((key, val) for key, val in self.summaries.items() if val.get_is_available()))
...
class GraphController:
    ...
    def _load_config(self, t_thresh):
        ...
        if user_config_dir is None:
            logging.warning('Failed to find or create scripts directory,                             proceeding without scripting support')
        ...
        try:
            if self.conf.getboolean('GraphControll', 'UTF8'):
                ...
                logging.debug('UTF8 selected as %s', self.conf.get('GraphControll', 'UTF8'))
        ...
        possible_sources = [TempSource(self.temp_thresh),
         ...
         RaplPowerSource(),
         FanSource()]
        sources = [x.get_source_name() for x in possible_sources if x.get_is_available()]
    ...
    def main(self):
        ...
        loop = MainLoop((self.view), DEFAULT_PALETTE, handle_mouse=(self.handle_mouse))
    ...
    def update_stress_mode(self):
        ...
        if self.stress_conroller.get_current_mode() == 'Stress':
            ...
            if self.stress_conroller.get_current_mode() == 'FIRESTARTER':
                stress_cmd = [
                 self.firestarter]
                self.stress_conroller.start_stress(stress_cmd)
    ...
    def animate_graph(self, loop, user_data=None):
        ...
        self.animate_alarm = loop.set_alarm_in(float(self.refresh_rate), self.animate_graph)
...
def get_args():
    ...
    parser.add_argument('-dr', '--debug_run', default=False,
      action='store_true',
    ...
    parser.add_argument('-nm', '--no-mouse', action='store_true', default=False,
      help='Disable Mouse for TTY systems')
    parser.add_argument('-v', '--version', default=False,
      action='store_true',
      help='Display version')
    parser.add_argument('-tt', '--t_thresh', default=None,
