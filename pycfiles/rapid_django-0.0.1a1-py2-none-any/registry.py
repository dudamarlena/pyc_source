...
# [GCC 8.4.0]
# Embedded file name: /home/marcos/rapid-django/src/rapid/registry.py
# Compiled at: 2015-09-03 17:00:52
...
class MenuEntry:
    ...
    def __unicode__(self):
        return 'Menu entry: ' + self.url_name + ' -> ' + unicode(self.action)
...
class Action:
    ...
    def __init__(self, name, url_parameters, query_parameters, view_factory, verbose_name=None, icon=None, visibility=None):
        self.name = name
...
class _Registry:
    ...
    def register_action(self, action, entry, **kwargs):
        ...
        if not registry._modules.has_key(module_name):
            ...
            return url('^%s/%s/%s$' % (entry.url_name, action.name, action.url_parameters), action.view_factory(model=entry.model, **kwargs), name=entry_url)
