...
# [GCC 8.4.0]
# Embedded file name: /home/marcos/rapid-django/src/rapid/wrappers.py
# Compiled at: 2015-09-08 14:44:52
    ...
    def actions(self):
        ...
        if self.request and acts:
            for a in acts.values():
                if self.has_permission(self.request, a.action.name) and a.action.visibility > Action.Visibility.hidden:
...
class ModelData:
    ...
    def all_fields(self):
        ...
        for f in itertools.chain(self.local_fields(), self.related_fields()):
            if f.is_relation():
                relations.append(f)
            elif f.name not in self.excludes:
                r.append(f)
    ...
    def actions(self):
        ...
        if self.request and acts:
            for a in acts.values():
                if self.has_permission(self.request, a.action.name) and not a.action.query_parameters and a.action.visibility > Action.Visibility.hidden:
