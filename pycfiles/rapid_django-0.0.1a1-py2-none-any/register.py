...
# [GCC 8.4.0]
# Embedded file name: /home/marcos/rapid-django/src/rapid/register.py
# Compiled at: 2015-09-03 16:32:09
__author__ = 'marcos'
from rapid.views import bare_or_main, ListView, ReadView, UpdateView, CreateView, SelectView, DeleteView, update_form_class
...
_default_actions = [
 ...
 (
  False,
  Action('select', '', [], _rvw(SelectView, 'select'), 'selecionar', 'fa-hand-o-up', visibility=Action.Visibility.hidden))]
...
def instance_form(model, action_name, entry_name, form, permission_set, url_name=None, icon=None, visibility=None):
    if not icon:
        icon = ''
    a = Action(action_name, '(?P<pk>[0-9]+)', ['pk'], _rvw(update_form_class(form), action_name), entry_name, icon, visibility)
...
def select(model, visible_fields, permission_set, url_name=None):
    ...
    a = Action('select', '', [], _rvw(Vw, 'select'), 'selecionar', 'fa-hand-o-up', visibility=Action.Visibility.hidden)
    u = registry.register_action(a, MenuEntry(model, permission_set, url_name=url_name))
    if u:
        return [u]
    return []