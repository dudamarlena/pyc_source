...
# Embedded file name: build/bdist.linux-x86_64/egg/App/Validation/Automation/unix.py
# Compiled at: 2012-05-03 09:59:34
import os, ssh, re, logging, types, pyDes
...
class Unix:
    ...
    def connect_to(self, remote_host, user, passwd=None):
        ...
        if passwd is None:
            ...
            key_files = [ key_dir + '/' + key_file for key_file in os.listdir(key_dir) ]
        client.set_missing_host_key_policy(ssh.AutoAddPolicy())
        logger = logging.getLogger(__name__)
        try:
            if passwd is None:
                client.connect(remote_host, port=22, username=user, key_filename=key_files)
            else:
                client.connect(remote_host, port=22, username=user, password=passwd)
    ...
    def validate_process(self, process_tmpl, remote_pcmd_tmpl):
        """Validates if process count of a given process is correct(as per
           ...
           >>> auto_obj = Unix()
           >>> remote_pcmd_tmpl =                  'ps -eaf | grep -i %s | grep -v grep | wc -l'
           >>> auto_obj.connect_to(socket.getfqdn(),"user",passwd="hur")
           True
        ...
        match = re.search('(?P<proc>.*)\\:(?P<count>.*)$', process_tmpl)
    ...
    def validate_mountpoint(self, mountpoint, remote_mcmd_tmpl):
        """Validate if the mountpoint if presented and accessible.Returns True
           ...
           >>> auto_obj.connect_to(socket.getfqdn(),"user",passwd="hur")
           True
           >>> auto_obj.validate_mountpoint("/export/home/",               remote_mcmd_tmpl)
           True
        """
        client = self.data['CONN_STATE']
