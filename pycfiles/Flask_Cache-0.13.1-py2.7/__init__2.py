...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/flask_cache/__init__2.py
# Compiled at: 2014-04-21 06:19:31
"""
    flask.ext.cache
    ~~~~~~~~~~~~~~
...
import base64, functools, hashlib, inspect, logging, string, uuid, warnings
...
class Cache(object):
    ...
    def _set_cache(self, app, config):
        ...
        app.extensions['cache'][self] = cache_obj(app, config, cache_args, cache_options)
    ...
    def _memoize_version(self, f, args, per_instance=False, reset=False, delete=False):
        """
    ...
    def _memoize_make_cache_key(self, make_name=None, per_instance=False):
        ...
        def make_cache_key(f, *args, **kwargs):
            fname, version_data = self._memoize_version(f, args, per_instance=per_instance)
    ...
    def memoize(self, timeout=None, make_name=None, unless=None, per_instance=False):
        """
        ...
        def memoize(f):
            ...
            decorated_function.make_cache_key = self._memoize_make_cache_key(make_name=make_name, per_instance=per_instance)
            return decorated_function
    ...
    def delete_memoized(self, f, *args, **kwargs):
        ...
        """
        if not callable(f):
            raise DeprecationWarning('Deleting messages by relative name is no longer reliable, please switch to a function reference')
        try:
            if f.per_instance and (getattr(f, '__self__', None) or len(args) == 1):
    ...
    def delete_memoized_verhash(self, f, *args):
        ...
        """
        if not callable(f):
            raise DeprecationWarning('Deleting messages by relative name is no longer reliable, please use a function reference')
        per_instance = f.per_instance and (getattr(f, '__self__', None) or len(args) == 1)
        try:
            self._memoize_version(f, args, delete=True, per_instance=per_instance)
        except Exception:
            if current_app.debug:
                raise
            logger.exception('Exception possibly due to cache backend.')
