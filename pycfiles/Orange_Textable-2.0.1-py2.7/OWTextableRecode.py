...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\_textable\widgets\OWTextableRecode.py
...
class OWTextableRecode(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None):
        ...
        substBox = OWGUI.widgetBox(widget=self.controlArea, box='Substitutions', orientation='vertical')
        substBoxLine1 = OWGUI.widgetBox(widget=substBox, box=False, orientation='horizontal', addSpace=True)
        self.substListbox = OWGUI.listBox(widget=substBoxLine1, master=self, value='selectedSubstLabels', labels='substLabels', callback=self.updateSubstBoxButtons, tooltip='The list of substitutions that will be applied to\neach segment of the input segmentation.\n\nSubstitutions will be applied in the same order\nas they appear in the list.\n\nColumn 1 shows the regex pattern.\nColumn 2 shows the associated replacement string\n(if not empty).\nColumn 3 shows the associated flags.')
        font = QFont()
        font.setFamily('Courier')
        font.setStyleHint(QFont.Courier)
        font.setPixelSize(12)
        self.substListbox.setFont(font)
        substBoxCol2 = OWGUI.widgetBox(widget=substBoxLine1, orientation='vertical')
        self.moveUpButton = OWGUI.button(widget=substBoxCol2, master=self, label='Move Up', callback=self.moveUp, tooltip='Move the selected substitution upward in the list.')
        self.moveDownButton = OWGUI.button(widget=substBoxCol2, master=self, label='Move Down', callback=self.moveDown, tooltip='Move the selected substitution downward in the list.')
        self.removeButton = OWGUI.button(widget=substBoxCol2, master=self, label='Remove', callback=self.remove, tooltip='Remove the selected substitution from the list.')
        self.clearAllButton = OWGUI.button(widget=substBoxCol2, master=self, label='Clear All', callback=self.clearAll, tooltip='Remove all substitutions from the list.')
        self.exportButton = OWGUI.button(widget=substBoxCol2, master=self, label='Export List', callback=self.exportList, tooltip='Open a dialog for selecting a file where the\nsubstitution list can be exported in JSON format.')
        self.importButton = OWGUI.button(widget=substBoxCol2, master=self, label='Import List', callback=self.importList, tooltip='Open a dialog for selecting a substitution list\nto import (in JSON format). Substitutions from\nthis list will be added to the existing ones.')
        substBoxLine2 = OWGUI.widgetBox(widget=substBox, box=False, orientation='vertical')
        addSubstBox = OWGUI.widgetBox(widget=substBoxLine2, box=True, orientation='vertical')
        OWGUI.lineEdit(widget=addSubstBox, master=self, value='newRegex', orientation='horizontal', label='Regex:', labelWidth=131, callback=self.updateGUI, tooltip="The regex pattern that will be added to the list\nwhen button 'Add' is clicked.")
        OWGUI.separator(widget=addSubstBox, height=3)
        OWGUI.lineEdit(widget=addSubstBox, master=self, value='newReplString', orientation='horizontal', label='Replacement string:', labelWidth=131, callback=self.updateGUI, tooltip="The (possibly empty) replacement string that will\nbe added to the list when button 'Add' is clicked.\n\nGroups of characters captured by parentheses in\nthe regex pattern may be inserted in the\nreplacement string by using the form '&'\n(ampersand) immediately followed by a digit\n indicating the captured group number (e.g. '&1',\n'&2', etc.).")
        OWGUI.separator(widget=addSubstBox, height=3)
        addSubstBoxLine3 = OWGUI.widgetBox(widget=addSubstBox, box=False, orientation='horizontal')
        OWGUI.checkBox(widget=addSubstBoxLine3, master=self, value='ignoreCase', label='Ignore case (i)', labelWidth=131, callback=self.updateGUI, tooltip='Regex pattern is case-insensitive.')
        OWGUI.checkBox(widget=addSubstBoxLine3, master=self, value='unicodeDependent', label='Unicode dependent (u)', callback=self.updateGUI, tooltip='Built-in character classes are Unicode-aware.')
        addSubstBoxLine4 = OWGUI.widgetBox(widget=addSubstBox, box=False, orientation='horizontal')
        OWGUI.checkBox(widget=addSubstBoxLine4, master=self, value='multiline', label='Multiline (m)', labelWidth=131, callback=self.updateGUI, tooltip="Anchors '^' and '$' match the beginning and\nend of each line (rather than just the beginning\nand end of each input segment).")
        OWGUI.checkBox(widget=addSubstBoxLine4, master=self, value='dotAll', label='Dot matches all (s)', callback=self.updateGUI, tooltip="Meta-character '.' matches any character (rather\nthan any character but newline).")
        OWGUI.separator(widget=addSubstBox, height=3)
        self.addButton = OWGUI.button(widget=addSubstBox, master=self, label='Add', callback=self.add, tooltip='Add the current substitution to the list.')
        ...
        self.advancedSettings.advancedWidgetsAppendSeparator()
        basicSubstBox = OWGUI.widgetBox(widget=self.controlArea, box='Substitution', orientation='vertical')
        OWGUI.lineEdit(widget=basicSubstBox, master=self, value='regex', orientation='horizontal', label='Regex:', labelWidth=180, callback=self.sendButton.settingsChanged, tooltip='The regex that will be applied to each segment in\nthe input segmentation.')
        OWGUI.separator(widget=basicSubstBox, height=3)
        OWGUI.lineEdit(widget=basicSubstBox, master=self, value='replString', orientation='horizontal', label='Replacement string:', labelWidth=180, callback=self.sendButton.settingsChanged, tooltip='The string that will be used for replacing each\nmatch of the above regex in the input segmentation.')
        OWGUI.separator(widget=basicSubstBox, height=3)
    ...
    def sendData(self):
        ...
        if not self.segmentation:
            ...
            if not self.segmentation.is_non_overlapping():
                self.infoBox.setText(message='Please make sure that input segments are not ' + 'overlapping.', state='error')
                self.send('Recoded data', None, self)
                return
            ...
            try:
                recoded_data = Segmenter.recode(segmentation=self.segmentation, substitutions=substitutions, label=self.captionTitle, copy_annotations=copyAnnotations, progress_callback=progressBar.advance)
                newNumInputs = len(Segmentation.data)
                self.createdInputIndices = range(previousNumInputs, newNumInputs)
                message = '%i segment@p sent to output.' % len(recoded_data)
    ...
    def exportList(self):
        ...
        for substitution in self.substitutions:
            toDump.append({'regex': substitution[0], 
        ...
        filePath = unicode(QFileDialog.getSaveFileName(self, 'Export Substitution List', self.lastLocation))
        if filePath:
            self.lastLocation = os.path.dirname(filePath)
            ...
            outputFile.close()
            QMessageBox.information(None, 'Textable', 'Substitution list correctly exported', QMessageBox.Ok)
        return
        ...
        if self.displayAdvancedSettings:
            ...
            if len(self.substitutions):
                ...
                for index in range(len(self.substitutions)):
                    ...
                    format = '%-' + unicode(maxReplStringLen + 2) + 's'
            ...
            if cachedLabel is not None:
                ...
                self.selectedSubstLabels.listBox.item(cachedLabel).setSelected(1)
