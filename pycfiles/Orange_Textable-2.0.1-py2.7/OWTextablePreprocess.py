...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win32\egg\_textable\widgets\OWTextablePreprocess.py
...
class OWTextablePreprocess(OWWidget):
    ...
    def __init__(self, parent=None, signalManager=None):
        ...
        optionsBox = OWGUI.widgetBox(widget=self.controlArea, box='Options', orientation='vertical', addSpace=True)
        self.preprocessingBoxLine1 = OWGUI.widgetBox(widget=optionsBox, orientation='horizontal')
        OWGUI.checkBox(widget=self.preprocessingBoxLine1, master=self, value='applyCaseTransform', label='Transform case:', labelWidth=180, callback=self.sendButton.settingsChanged, tooltip='Apply systematic case conversion.')
        self.caseTransformCombo = OWGUI.comboBox(widget=self.preprocessingBoxLine1, master=self, value='caseTransform', items=[
         'to lower', 'to upper'], sendSelectedValue=True, callback=self.sendButton.settingsChanged, tooltip='Case conversion mode: to lowercase or uppercase.')
        self.caseTransformCombo.setMinimumWidth(120)
        OWGUI.separator(widget=optionsBox, height=3)
        OWGUI.checkBox(widget=optionsBox, master=self, value='removeAccents', label='Remove accents', callback=self.sendButton.settingsChanged, tooltip='Replace accented characters with non-accented ones.')
        OWGUI.separator(widget=optionsBox, height=3)
    ...
    def sendData(self):
        ...
        if not self.segmentation:
            ...
            preprocessed_data = Segmenter.recode(self.segmentation, case=case, remove_accents=self.removeAccents, label=self.captionTitle, copy_annotations=self.copyAnnotations, progress_callback=progressBar.advance)
            progressBar.finish()
            newNumInputs = len(Segmentation.data)
            self.createdInputIndices = range(previousNumInputs, newNumInputs)
            message = '%i segment@p sent to output.' % len(preprocessed_data)
