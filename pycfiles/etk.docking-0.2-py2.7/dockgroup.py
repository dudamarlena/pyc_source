...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/etk/docking/dockgroup.py
# Compiled at: 2011-01-25 11:35:54
from __future__ import absolute_import
from logging import getLogger
from math import pi
from operator import attrgetter
from time import time
import cairo, gobject, gtk, gtk.gdk as gdk
from . import _
...
class DockGroup(gtk.Container):
    ...
    def do_realize(self):
        ...
        self.window = gdk.Window(self.get_parent_window(), x=self.allocation.x, y=self.allocation.y, width=self.allocation.width, height=self.allocation.height, window_type=gdk.WINDOW_CHILD, wclass=gdk.INPUT_OUTPUT, event_mask=gdk.EXPOSURE_MASK | gdk.POINTER_MOTION_MASK | gdk.BUTTON_PRESS_MASK | gdk.BUTTON_RELEASE_MASK)
    ...
    def do_size_request(self, requisition):
        ...
        for tab in self._tabs:
            ...
            tab.area.width = self._frame_width + self._spacing + iw + self._spacing + lw + self._spacing + bw + self._spacing + self._frame_width
            tab.area.height = self._frame_width + self._spacing + max(ih, lh, bh) + self._spacing + self._frame_width
        ...
        dh = max(dh, self._spacing + list_h + self._spacing, self._spacing + min_h + self._spacing, self._spacing + max_h + self._spacing)
    ...
    def do_size_allocate(self, allocation):
        ...
        for tab in self._visible_tabs:
            ...
            if len(self._visible_tabs) == 1:
                lw = tab.area.width - (self._frame_width + self._spacing + iw + self._spacing + self._spacing + bw + self._spacing + self._frame_width)
                lw = max(lw, 0)
            ...
            bx = cx + self._frame_width + self._spacing + iw + self._spacing + lw + self._spacing
    ...
    def do_expose_event(self, event):
        ...
        if self._visible_tabs:
            ...
            c.rectangle(self._frame_width + self.border_width / 2, self._decoration_area.height + self.border_width / 2, a.width - 2 * self._frame_width - self.border_width, a.height - self._decoration_area.height - self._frame_width - self.border_width)
    ...
    def do_button_release_event(self, event):
        ...
        if clicked_tab:
            if event.button == 1:
                ...
                self._tab_menu.popup(parent_menu_shell=None, parent_menu_item=None, func=_menu_position, button=event.button, activate_time=event.time)
        return True
    ...
    def do_motion_notify_event(self, event):
        ...
        if event.window is self.window:
            if event.state & gdk.BUTTON1_MASK and self.dragcontext.source_button == 1 and self.drag_check_threshold(int(self.dragcontext.source_x), int(self.dragcontext.source_y), int(event.x), int(event.y)):
                self.log.debug('drag_begin')
                self.dragcontext.dragging = True
                tab = self.get_tab_at_pos(self.dragcontext.source_x, self.dragcontext.source_y)
                if tab:
                ...
                if self.dragcontext.dragged_object:
                    self.drag_begin([DRAG_TARGET_ITEM_LIST], gdk.ACTION_MOVE, self.dragcontext.source_button, event)
            for tab in self._visible_tabs:
                if (
                 event.x, event.y) in tab:
    ...
    def do_drag_data_get(self, context, selection_data, info, timestamp):
        ...
        """
        selection_data.set(gdk.atom_intern(DRAG_TARGET_ITEM_LIST[0]), 8, '%d tabs' % len(self.dragcontext.dragged_object))
    ...
    def _update_visible_tabs(self):
        ...
        if len(self._visible_tabs) == 1:
            ...
            normal = self._frame_width + self._spacing + iw + self._spacing + lw + self._spacing + bw + self._spacing + self._frame_width
    ...
    def _on_list_button_clicked(self, button):
        ...
        self._list_menu.popup(parent_menu_shell=None, parent_menu_item=None, func=_menu_position, button=1, activate_time=0)
        return
