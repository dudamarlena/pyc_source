...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/etk/docking/compactbutton.py
# Compiled at: 2010-12-07 01:43:02
...
class CompactButton(gtk.Widget):
    ...
    def do_get_property(self, pspec):
        ...
        if pspec.name == 'icon-name-prelight':
            return self.get_icon_name_prelight()
        if pspec.name == 'icon-name-active':
            return self.get_icon_name_active()
        if pspec.name == 'size':
            return self.get_size()
        if pspec.name == 'has-frame':
    ...
    def do_expose_event(self, event):
        ...
        if self._has_frame and self.state != gtk.STATE_NORMAL:
            event.window.draw_rectangle(self.style.dark_gc[self.state], False, self.allocation.x, self.allocation.y, self.allocation.width - 1, self.allocation.height - 1)
        return False
