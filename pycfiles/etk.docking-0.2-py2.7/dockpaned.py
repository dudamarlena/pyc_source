...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-intel/egg/etk/docking/dockpaned.py
...
class DockPaned(gtk.Container):
    ...
    def _insert_item(self, child, position=None, weight=None):
        ...
        if not (weight and 0.0 <= weight <= 1.0):
    ...
    def _remove_item(self, child):
        ...
        assert len(self._items) == len(self._handles) + 1 or len(self._items) == len(self._handles) == 0
        self.queue_resize()
    ...
    def _redistribute_weight(self, size):
        ...
        * divide remaining space over other items.
        ...
        if min_size > size:
            ...
            sf = 1.0
        for i, w in zip(other_items, fair_scale(1.0 - sum(i.weight_request for i in requested_items), [ (i.weight, sf * i.min_size / size) for i in other_items ])):
            i.weight = w

        for i, w in zip(requested_items, fair_scale(1.0 - sum(i.weight for i in other_items), [ (i.weight_request, sf * i.min_size / size) for i in requested_items ])):
            i.weight = w
    ...
    def do_motion_notify_event(self, event):
        ...
        if self._dragcontext.dragging:
            if self._orientation == gtk.ORIENTATION_HORIZONTAL:
                cursor = self._hcursor
                delta_size = int(event.x - self._dragcontext.dragged_object.area.x - self._dragcontext.offset_x)
            else:
                cursor = self._vcursor
                delta_size = int(event.y - self._dragcontext.dragged_object.area.y - self._dragcontext.offset_y)
