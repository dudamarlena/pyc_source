...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/libaskocli/__init__.py
# Compiled at: 2019-03-27 06:07:43
# Size of source mod 2**32: 9956 bytes
...
class RequestApi(object):
    __doc__ = 'RequestApi contain method to communicate with\n    the AskOmics API'
    ...
    def set_cookie(self):
        ...
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when login: ' + str(response.status_code) + '\n' + response.text)
    ...
    def upload_file(self):
        ...
        files = {basename(self.path): open(self.path, 'rb')}
        response = requests.post(url, files=files, cookies=(self.cookies), headers=(self.headers))
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when uploading a file: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
    ...
    def guess_col_types(self):
        ...
        json_dict = {'filename': basename(self.path)}
        response = requests.post(url, cookies=(self.cookies), headers=(self.headers), json=json_dict)
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when guessing col types: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
    ...
    def integrate_data(self, uri):
        ...
        json_dict = {'file_name':basename(self.path), 
        ...
        response = requests.post(url, cookies=(self.cookies), headers=(self.headers), json=json_dict)
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when integrate csv: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
        return response.text
    ...
    def integrate_gff(self, taxon, entities, uri):
        ...
        response = requests.post(url, cookies=(self.cookies), headers=(self.headers), json=json_dict)
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when integrate gff: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
        return response.text
    ...
    def integrate_bed(self, entity_name, taxon, uri):
        ...
        response = requests.post(url, cookies=(self.cookies), headers=(self.headers), json=json_dict)
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when integrate bed: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
        return response.text
    ...
    def integrate_ttl(self):
        ...
        response = requests.post(url, cookies=(self.cookies), headers=(self.headers), json=json_dict)
        if response.status_code != 200:
            raise Exception('Unexpected response from AskOmics when integrate ttl: ' + str(response.status_code) + '\n' + response.text)
        if 'error' in json.loads(response.text):
            raise Exception('AskOmics error: ' + str(json.loads(response.text)['error']))
        return response.text