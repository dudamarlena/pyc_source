...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-universal/egg/ZombieAgent/Plugin/Manifest.py
# Compiled at: 2012-01-04 18:04:15
...
class PluginManifestException(ZombieAgentException):
    pass


class PluginManifestParseException(ZombieAgentException):
    pass


class FunctionNotFound(ZombieAgentException):
    pass


class ExecutionError(ZombieAgentException):
    pass


class ParameterRequired(ZombieAgentException):
    pass
...
class PluginManifest(object):
    ...
    def execute(self, function, params=''):
        ...
        for param in function_def['params']:
            ...
            if 'required' in pdef and pdef['required'] and param not in params_preparsed.keys() and 'default' not in pdef:
                raise ParameterRequired('Parameter "%s" is required' % param)
            elif 'default' in pdef and param not in params_preparsed.keys():
        ...
        proc = subprocess.Popen(command, cwd=self.basedir, universal_newlines=True, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        proc.wait()
