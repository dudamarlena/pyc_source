...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/nbi/views.py
...
def projects():
    ...
    return render_template('nbi/projects.html', title=config.get('PROJECTS', 'title'), grid_header='{}'.format(config.get('PROJECTS', 'title')), tags=list(tags.keys()), areas=area_choices, objects=entities, form=form)
...
def my_projects():
    area_choices = NBIProjectForm.area.kwargs.get('choices')
    form = TagsSearchForm()
    entities = [project for project in Project.get_all() if project._id in current_user.projects]
    return render_template('nbi/projects.html', title=config.get('PROJECTS', 'title'), grid_header='{}'.format(config.get('PROJECTS', 'title')), areas=area_choices, objects=entities, form=form)
