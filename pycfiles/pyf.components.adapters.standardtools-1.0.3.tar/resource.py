...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/componentized/components/resource.py
...
class ResourceManager(object):
    ...
    def get_plugins(self, plugin_type=None):
        if plugin_type:
            ...
            return dict([ (pplugin_type, self.get_plugins(pplugin_type)) for (splugin_type, pplugin_type) in self.plugin_types
                        ])

    def get_plugin_getter(self, plugin_type_namespace):

        def plugin_getter(name):
            return self.__get_plugin('%s.%s' % (self.main_namespace, plugin_type_namespace), name)
