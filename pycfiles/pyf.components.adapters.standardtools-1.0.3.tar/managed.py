...
# Embedded file name: build/bdist.linux-x86_64/egg/pyf/componentized/components/managed.py
...
import os, tempfile, datetime, shutil, logging
...
class ManagedComponent(object):
    ...
    def __init__(self, config_node):
        ...
        self.advanced_config = dict(separate_process=False)
        if advanced_node is not None:
            if advanced_node.find('separate_process') is not None:
                self.advanced_config['separate_process'] = advanced_node.find('separate_process').text.lower() in ('yes',
                                                                                                                   'true')
    ...
    def init_component(self, process_config_node, progression_callback=None, message_callback=None, params=None):
        self.process_config_node = process_config_node
        self.progression_callback = progression_callback
        self.message_callback = message_callback
        self.params = params
        self.outputs_files = getattr(self.component_class, 'outputs_files', False)
        ...
        if self.gets_values:
            if self.gets_filename:
                ...
                self.target_filename_path = os.path.join(self.target_path, self.target_filename)
                self.component_obj = self.component_class(self.node_tree, self.name, self.temp_filename_path)
                self.component_obj.set_config_key('target_filename', self.target_filename)
                self.component_obj.set_config_key('target_path', self.target_path)
                self.component_obj.set_config_key('target_filename_path', self.target_filename_path)
            else:
                self.component_obj = self.component_class(self.node_tree, self.name)
        elif self.source_mode == 'code':
            self.component_obj = self.component_class(self.node_tree, self.name, gets_data=False)
        else:
            self.component_obj = self.component_class(self.node_tree, self.name)
        if self.outputs_files and hasattr(self.component_obj, 'set_output_file_getter'):
    ...
    def init_process_proxy(self):
        try:
            ...
            mapped_functions = [
             '_get_output_file', '_get_message_callback']
            for function_name in mapped_functions:
                if hasattr(self.component_obj, function_name):
                    NodeManager.register(function_name, getattr(self.component_obj, function_name))
    ...
    def component(self):
        if not hasattr(self.component_obj.launch, 'zf_out_ports'):
            ...
            return generator_to_component(self.component_obj.launch(self.progression_callback, self.message_callback, params=self.params))
    ...
    def finalize(self):
        if self.gets_filename:
            self.target_filename_path = self.get_target_filename_path(self.node_tree)
            ...
            for (temp_filename, target_filename) in self.registered_output_files:
                target_filename_path = os.path.join(self.get_target_path(), target_filename)
    ...
    def get_target_filename(self, config_tree=None):
        """
        """
        pattern = self.get_config_key('target_filename', '', config_tree=config_tree)
        target_filename = datetime.datetime.strftime(datetime.datetime.now(), pattern)
