...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/xdis/codetype/code30.py
...
Code3FieldTypes.update({'co_kwonlyargcount': int})
...
class Code3(Code2):
    ...
    def __init__(self, co_argcount, co_kwonlyargcount, co_nlocals, co_stacksize, co_flags, co_code, co_consts, co_names, co_varnames, co_filename, co_name, co_firstlineno, co_lnotab, co_freevars, co_cellvars):
        super(Code3, self).__init__(co_argcount, co_nlocals, co_stacksize, co_flags, co_code, co_consts, co_names, co_varnames, co_filename, co_name, co_firstlineno, co_lnotab, co_freevars, co_cellvars)
    ...
    def to_native(self):
        if not 3.0 <= PYTHON_VERSION <= 3.7:
            raise TypeError('Python Interpreter needs to be in range 3.0..3.7; is %s' % PYTHON_VERSION)
        code = deepcopy(self)
        code.freeze()
        try:
            code.check()
        ...
        return types.CodeType(code.co_argcount, code.co_kwonlyargcount, code.co_nlocals, code.co_stacksize, code.co_flags, code.co_code, code.co_consts, code.co_names, code.co_varnames, code.co_filename, code.co_name, code.co_firstlineno, code.co_lnotab, code.co_freevars, code.co_cellvars)