...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/artellamanager/widgets/localmanager.py
# Compiled at: 2020-04-15 10:23:56
# Size of source mod 2**32: 33012 bytes
"""
...
import os, weakref, traceback, webbrowser, logging.config
            ...
            if file_versions:
                ...
                if is_updated:
                    if file_versions:
                        ...
                        is_locked or new_version_action.setText('Make New Version | Lock File first!')
                        new_version_action.setEnabled(False)
                    elif not locked_by_current_user:
...
class ArtellaSyncItem(QTreeWidgetItem, object):
    ...
    def get_sync_pixmap(self):
        ...
        if self._sync_status == ArtellaSyncItemStatus.WAIT:
            ...
            if self._sync_status == ArtellaSyncItemStatus.RUN:
                return self.RUN_PIXMAP
            if self._sync_status == ArtellaSyncItemStatus.OK:
