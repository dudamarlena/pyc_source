...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/artellapipe/tools/artellamanager/widgets/servermanager.py
# Compiled at: 2020-04-15 10:23:56
# Size of source mod 2**32: 49536 bytes
"""
...
import os, webbrowser, traceback, logging.config
...
class ArtellaServerManagerwidget(base.BaseWidget, object):
    ...
    def _on_artella_worker_failed(self, uid, msg, trace):
        ...
        parent_index = 1
        if self._last_add:
            if self._last_add in self._trees:
                parent_tree = self._trees[self._last_add]['parent']
                if parent_tree:
                    if parent_tree in self._trees:
...
class ArtellaSyncTree(treewidgets.TreeWidget, object):
    ...
    def update_data(self, data=None):
        ...
        if not self._path:
            ...
            if isinstance(status, artellaclasses.ArtellaDirectoryMetaData):
                ...
                if isinstance(status, artellaclasses.ArtellaAssetMetaData):
    ...
    def _create_tree_context_menu(self):
        ...
        add_all_items_action = QAction(queue_icon,
          'Add all Items to Sync Queue', context_menu, statusTip='Add all items into Sync queue')
        clear_queue_action = QAction(delete_icon,
          'Clear Sync Queue', context_menu, statusTip='Clear All items from Sync Queue')
    ...
    def _create_tree_item_context_menu(self, item):
        ...
        if not item:
            ...
            open_in_artella_action = QAction(artella_icon,
              'Open in Artella', context_menu, statusTip='Open Item in Artella')
            add_to_sync_queue_action = QAction(queue_icon,
              'Add to Sync Queue', context_menu, statusTip='Add item to Artella Syncer queue')
            ...
            if dcc_name != tp.Dccs.Unknown:
                if item.is_file():
                    ...
                    open_in_dcc_action = QAction(open_icon,
                      'Open', dcc_menu, statusTip=('Open Item in {}'.format(dcc_name.title())))
                    import_in_dcc_action = QAction(import_icon,
                      'Import', dcc_menu, statusTip=('Import Item in {}'.format(dcc_name.title())))
                    reference_in_dcc_action = QAction(reference_icon,
                      'Reference', dcc_menu, statusTip=('Reference Item in {}'.format(dcc_name.title())))
...
class ArtellaSyncItem(QTreeWidgetItem, object):
    ...
    def get_path(self):
        ...
        if not self._artella_data:
            ...
            if isinstance(self._artella_data, artellaclasses.ArtellaHeaderMetaData):
                return self._path
    ...
    def can_create_assets(self):
        ...
        if not isinstance(self._artella_data, artellaclasses.ArtellaDirectoryMetaData):
            ...
            rel_path = os.path.dirname(os.path.relpath(item_path, os.path.dirname(artellapipe.AssetsMgr().get_assets_path())))
    ...
    def get_icon(self):
        ...
        if not self._artella_data:
            ...
            if not self._is_file:
                ...
                if isinstance(self._artella_data, artellaclasses.ArtellaAssetMetaData):
...
class ArtellaSyncQueueItem(QTreeWidgetItem, object):
    ...
    def set_sync_status(self, sync_status):
        ...
        if self._sync_status == ArtellaSyncQueueItemStatus.WAIT:
            ...
            if self._sync_status == ArtellaSyncQueueItemStatus.RUN:
                self.setIcon(0, self.RUN_ICON)
            else:
                if self._sync_status == ArtellaSyncQueueItemStatus.OK:
...
class ArtellaSyncQueueTree(QTreeWidget, object):
    ...
    def _create_tree_item_context_menu(self, item):
        ...
        open_in_artella_action = QAction(artella_icon,
          'Open in Artella', context_menu, statusTip='Open Item in Artella')
        delete_action = QAction(delete_icon,
          'Remove from Sync Queue', context_menu, statusTip='Remove item from Sync Queue')
        open_in_artella_action.triggered.connect(partial(self._on_open_item_in_artella, item))
    ...
    def _create_tree_context_menu(self):
        ...
        context_menu = QMenu(self)
        delete_icon = tp.ResourcesMgr().icon('delete')
        remove_queue_action = QAction(delete_icon,
          'Clear Sync Queue', context_menu, statusTip='Remove All Items from Sync Queue')
