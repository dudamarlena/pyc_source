...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/iw/releaser/tests/test_project.py
...
from iw.releaser.project import archive_contents, _python, diff_releases, _set_dynlibs, _extract_url, _make_python, copy_archives, build_md5, project_eggs
...
class ProjectTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        previous_snap = ['eggs/file', 'eggs/stuff.egg', 'downloads/dist/file', 'downloads/dist/file2']
    ...
    def test_light_archiving(self):
        ...
        x = [ join(location, path) for path in ['parts', 'var', 'develop-eggs', 'bin', 'lib', 'Lib', 'Scripts', join('downloads', 'dist')] ]
        archive_contents('pack.tgz', location, source=True, exclude=x)
        import tarfile
        tar = tarfile.open(tarfile_, 'r:gz')
        try:
            elements = [ tarinfo.name for tarinfo in tar if not os.path.split(tarinfo.name)[(-1)].startswith('.') ]
        finally:
            tar.close()
        ...
        try:
            ...
            tar.close()
        wanted = ['downloads/dist/file', 'downloads/dist/file.py', 'downloads/dist/file2', 'downloads/file.pyc', 'downloads/file2', 'eggs/file', 'eggs/stuff.egg/file', 'file', 'file2.pyc', 'file3.pyo']
        wanted.sort()
        elements.sort()
    ...
    def test_python(self):
        test_bin = join(os.path.dirname(__file__), 'test_bin')
        os.mkdir(test_bin)
        os.chdir(test_bin)
        try:
            os.mkdir(join(test_bin, 'bin'))
            os.mkdir(join(test_bin, 'Scripts'))
            linux = (('bin', 'python'), ('bin', 'python2.4'), ('bin', 'python2.4.3'))
    ...
    def test_set_dynlibs(self):
        ...
        try:
            ...
            _set_dynlibs(root)
            self.assert_(os.path.exists(os.path.join(root, 'libs', 'libpython24.a')))
        finally:
    ...
    def test_copy_archives(self):
        ...
        try:
            ...
            res = [
             'downloads', 'downloads_old', 'eggs', 'eggs_old']
            res.sort()
            t = os.listdir(target)
            t.sort()
    ...
    def test_project_eggs(self):
        ...
        def _list_tarball(tarball):
            ...
            try:
                elements = [ t.name for t in tar if not s(t.name)[(-1)].startswith('.') ]
            finally:
                tar.close()
            elements.sort()
            return elements
        ...
        folder = tempfile.mkdtemp()
        try:
            eggs = project_eggs(join(project, 'buildout.cfg'), tarball, ('iw.f*', ))
            res = _list_tarball(tarball)
