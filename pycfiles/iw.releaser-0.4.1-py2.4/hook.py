...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/iw/releaser/hook.py
# Compiled at: 2008-04-29 08:14:25
import os, sys, smtplib, pkg_resources
...
def apply_hooks(**kwargs):
    ...
    <BLANKLINE>
    iw.releaser 0.1 has been released !
    <BLANKLINE>
    ...
    if not hooks:
        return
    name = kwargs.get('package_name', os.path.split(os.getcwd())[(-1)].strip())
    if os.path.isfile('setup.py'):
        sys.path.insert(0, os.getcwd())
        _run_setup('egg_info')
    else:
        raise pkg_resources.DistributionNotFound('Not able to find %s distribution in %s' % (name, os.getcwd()))
    try:
    ...
    for name in hooks:
        ...
        hook_meth = pkg_resources.load_entry_point(package, ENTRY_POINT, entry)
...
def mail_hook(dist, global_config, **local_config):
    ...
    subject = local_config.get('subject', '[Release] %(name)s %(version)s')
    subject = subject.strip() % dict(name=dist.project_name, version=local_config.get('version', ''))
    ...
    kicker = kicker % dict(name=dist.project_name, version=version)
    contents = [
     kicker, '', 'Work done in this version:']
