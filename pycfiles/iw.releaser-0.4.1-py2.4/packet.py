...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/iw/releaser/packet.py
# Compiled at: 2008-04-29 08:14:25
import os, shutil, re, pkg_resources, subprocess, tempfile, sys
from ConfigParser import ConfigParser
import datetime
from iw.releaser import base
re_version = re.compile('^version\\s*=\\s*(?:"|\')(.*?)(?:"|\')', re.MULTILINE | re.DOTALL)
re_name = re.compile('name\\s*=\\s*(?:"|\')(.*?)(?:"|\')', re.MULTILINE | re.DOTALL)
...
def raise_version(version):
    ...
    for (root, dirs, files) in os.walk(os.getcwd()):
        for dirname in dirs:
            if os.path.isdir(os.path.join(root, dirname, 'tests')):
...
def increment_changes():
    """increment changes"""
    version = get_version()
    locations = (os.path.join(os.getcwd(), 'CHANGES'), os.path.join(os.getcwd(), 'CHANGES.txt'))
    CHANGES = locations[0]
    if not os.path.exists(CHANGES) and os.path.exists(locations[1]):
        CHANGES = locations[1]
    year = datetime.datetime.now().strftime('%Y')
    now = datetime.datetime.now().strftime('%Y-%m-%d')
    trunk_re = re.compile('^trunk \\(.*\\)$', re.DOTALL)
    trunk_line = 'trunk (%s)' % now
    bootstrap = [trunk_line, len(trunk_line) * '=', '', '  - xxx [Ingeniweb]', '']
