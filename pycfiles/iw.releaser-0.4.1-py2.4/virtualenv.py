...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.3-fat/egg/iw/releaser/virtualenv.py
...
REQUIRED_MODULES = [
 'os', 'posix', 'posixpath', 'ntpath', 'fnmatch', 'locale', 'encodings', 'codecs', 'stat', 'UserDict', 'readline', 'copy_reg', 'types', 're', 'sre', 'sre_parse', 'sre_constants', 'sre_compile', 'lib-dynload', 'config']
...
def install_setuptools(py_executable):
    ...
    logger.indent += 2
    try:
        call_subprocess(cmd, show_stdout=False, filter_stdout=filter_ez_setup, extra_env=env)
    finally:
        logger.indent -= 2
        logger.end_progress()
...
def main():
    global logger
    parser = optparse.OptionParser(version='1.0', usage='%prog [OPTIONS] DEST_DIR')
