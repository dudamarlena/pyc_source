...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/kreveik/classes/network.py
# Compiled at: 2012-09-09 23:45:49
"""
Definition of network object.
"""
import numpy as num, matplotlib.pyplot as plt, copy, itertools, logging
...
class TopologicalNetwork(ProbeableObj):
    ...
    def __init__(self, adjacency_matrix):
        ...
        self.code = str(len(self.adjacency)) + '-' + str(reduce(lambda x, y: 2 * x + y, self.adjacency.flatten() * 1))
    ...
    def plot(self):
        ...
        for linksto, node in enumerate(self.adjacency):
            for linksfrom, link in enumerate(node):
                if linksto == linksfrom and link == True:
                    angle = math.atan2(list_of_coordinates[linksto][1] - 200, list_of_coordinates[linksto][0] - 200)
                    drawing.create_line(list_of_coordinates[linksto][0] + node_radius * math.cos(angle), list_of_coordinates[linksto][1] + node_radius * math.sin(angle), list_of_coordinates[linksto][0] + node_radius * 2 * math.cos(angle + 20), list_of_coordinates[linksto][1] + node_radius * 2 * math.sin(angle + 20), list_of_coordinates[linksto][0] + node_radius * 4 * math.cos(angle), list_of_coordinates[linksto][1] + node_radius * 4 * math.sin(angle), list_of_coordinates[linksto][0] + node_radius * 2 * math.cos(angle - 20), list_of_coordinates[linksto][1] + node_radius * 2 * math.sin(angle - 20), list_of_coordinates[linksto][0] + node_radius * math.cos(angle), list_of_coordinates[linksto][1] + node_radius * math.sin(angle), smooth=True, joinstyle='round', fill='black', width=2, arrow='last')
                elif link == True:
                    angle = math.atan2(list_of_coordinates[linksto][1] - list_of_coordinates[linksfrom][1], list_of_coordinates[linksto][0] - list_of_coordinates[linksfrom][0])
                    drawing.create_line(list_of_coordinates[linksfrom][0] + node_radius * math.cos(angle), list_of_coordinates[linksfrom][1] + node_radius * math.sin(angle), list_of_coordinates[linksto][0] - node_radius * math.cos(angle), list_of_coordinates[linksto][1] - node_radius * math.sin(angle), fill='black', width=2, arrow='last')
        ...
        for node_ctr, (x, y) in enumerate(list_of_coordinates):
            if type(self) != Network:
                ...
                node_color = 'white'
                text_color = 'black'
            elif self.state[(-1)][node_ctr] == True:
                node_color = 'black'
                text_color = 'white'
            else:
                node_color = 'white'
...
class Network(TopologicalNetwork, Element):
    ...
    def set_state(self, state):
        ...
        """
        if type(state) == int:
            state = [ int(strings) == True for strings in list(num.binary_repr(state, width=self.n_nodes)) ]
