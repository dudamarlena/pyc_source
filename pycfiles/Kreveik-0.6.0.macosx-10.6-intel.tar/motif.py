...
# [GCC 8.4.0]
# Embedded file name: /Library/Frameworks/Python.framework/Versions/2.7/lib/python2.7/site-packages/kreveik/network/motif.py
# Compiled at: 2012-09-09 23:45:49
import logging, numpy as num, kreveik
...
def all_conn_motifs(nodes):
    ...
    for number in xrange(2 ** degree):
        linear = num.array([ int(item) for item in [False] * (degree - len(list(bin(number)[2:]))) + list(bin(number)[2:])
                           ], dtype=bool)
        motifadj = num.reshape(linear, (nodes, nodes))
        motif = kreveik.classes.Motif(motifadj)
        if motif.is_connected() and not any([ motif == motiffromlist[0] for motiffromlist in motiflist
                                            ]):
...
__all__ = [
 motif_freqs, exclusive_conn_motifs, all_conn_motifs, node_duplication,
