...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/rafae/repo/epic/EPIC/cosmology/observations.py
# Compiled at: 2018-09-10 17:17:50
# Size of source mod 2**32: 15767 bytes
import configparser
...
class DataPoints(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        self.points, self.obs, self.errors = np.loadtxt(self.location, ndmin=2, unpack=True)
...
class Generic_Covariant_Gaussian_Dataset(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name, attributes=[
 'points', 'obs', 'sigmas']):
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        ...
        self.log_det_inverse_covariance_matrix = np.linalg.slogdet(self.inverse_covariance_matrix)[1]
...
class BAO_isotropic_DataPoints(DataPoints):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        self.points, self.obs, self.errors = np.loadtxt(os.path.join(self.location, 'data.txt'), ndmin=2, unpack=True)
        with open(os.path.join(self.location, 'fiducial.p'), 'rb') as (fhandle):
            self.fiducial = pickle.load(fhandle)
        self.predicting = predicting_function_name

    def function_of_predict(self, ratio):
        rs_zd_fiducial = self.fiducial.get('rs_zd_EH98', self.fiducial.get('rs_zd', self.fiducial.get('rs_zd_CAMB')))
...
class Generic_Covariant_Gaussian_one_point(Generic_Covariant_Gaussian_Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        super().__init__(observable, dataset_loc, predicting_function_name, attributes=[
         'obs', 'sigmas'])
...
class BAO_QuasarLyman_Dataset(BAO_aniso_Dataset):
    ...
    def function_of_predict(self, scales):
        DA_over_rd, rd_times_H = scales
        return (DA_over_rd, speedoflight / 1000.0 / rd_times_H)
...
class BAO_Lyalpha_Forests_Dataset(BAO_aniso_Dataset):
    ...
    def function_of_predict(self, scales):
        DA_over_rd, rd_times_H = scales
        return ((1 + self.points) * DA_over_rd, speedoflight / 1000.0 / rd_times_H)
...
class NonVirializedClusters(Dataset):
    ...
    def __init__(self, observable, dataset_file, predicting_function_name):
        self.location = os.path.join(user_folder, 'modifications', 'cosmology', 'observational_data', observable)
        self.clusters = []
        with open(os.path.join(self.location, dataset_file), 'r') as (datafile):
            ...
            while line:
                self.clusters.append(cosmo.Cluster(*line.strip().split()))
                line = datafile.readline()

        with open(os.path.join(self.location, dataset_file.replace('.txt', '.ovr')), 'w') as (ovr_file):
        ...
        self.nuisance_parameters = OrderedDict([
         (
          'gamma', cosmo.NuisanceParameter('gamma', tex='\\gamma')),
         (
          'logt0', cosmo.NuisanceParameter('logt0', tex='\\log_{10}t_0'))])
    ...
    def log_likelihood(self, predict, **kwargs):
        ...
        dlnrhoH = [model.clusters_dlnrho_over_H(cl, self.nuisance_parameters, **kwargs) for cl in self.clusters]
...
class JLA_SimplifiedDataset(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        ...
        self.points, self.obs = np.loadtxt(os.path.join(self.location, dataset_loc, 'data', 'jla_mub.txt'), ndmin=2, unpack=True)
        self.covariance_matrix = np.loadtxt(os.path.join(self.location, dataset_loc, 'data', 'jla_mub_covmatrix.dat'), skiprows=1).reshape([self.points.size, self.points.size])
    ...
    def log_likelihood(self, predict, **kwargs):
        return generic_likelihood_Covariant_Gaussian_with_nuisance(self, predict, **kwargs)
...
class full_JLA_likelihood(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        ...
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        z_cmb, z_hel, muB, x1, color, self.log10MB = np.loadtxt(os.path.join(self.location, 'data', 'jla_lcparams.txt'), unpack=True, usecols=(1,
                                                                                                                                               2,
                                                                                                                                               4,
                                                                                                                                               6,
                                                                                                                                               8,
        ...
        scoh, slens, sz = np.loadtxt(os.path.join(self.location, 'covmat', 'sigma_mu.txt'), unpack=True)
        ...
        self.C_eta = np.sum([fits.getdata(mat) for mat in glob(os.path.join(self.location, 'covmat', 'C*.fits'))], axis=0)
        ...
        sigz = 750000.0 / speedoflight / sz / np.log(10)
        self.sum_sig2 = np.diag(sigz ** 2 + slens ** 2 + scoh ** 2)
        self.nuisance_parameters = OrderedDict([
         (
          'alpha', cosmo.NuisanceParameter('alpha', tex='\\alpha')),
         (
          'beta', cosmo.NuisanceParameter('beta', tex='\\beta')),
         (
          'M_B', cosmo.NuisanceParameter('M_B')),
         (
          'DeltaM', cosmo.NuisanceParameter('DeltaM', tex='{\\Delta}M'))])
...
def generic_likelihood_Covariant_Gaussian_with_nuisance(dataset, predict, **kwargs):
    predicted = dataset.function_of_predict(predict(dataset.points, dataset.nuisance_parameters, **kwargs))
    return chisquare(dataset, predicted)
...
def chisquare(dataset, predicted):
    ...
    chi2 = float(chi2)
    return (chi2,
     -dataset.obs.size * log2pi_over_2 + dataset.log_det_inverse_covariance_matrix / 2 - chi2 / 2)


def generic_likelihood_Covariant_Gaussian_aniso(*args, **kwargs):
    kwargs.pop('isotropic', None)
    return generic_likelihood_Covariant_Gaussian(*args, **kwargs)


def check_conflicting_datasets(datasets):
    for path in [
     os.path.join(root, 'cosmology', 'observational_data', 'conflicting_dataset_pairs.txt'),
     os.path.join(user_folder, 'modifications', 'cosmology', 'observational_data', 'conflicting_dataset_pairs.txt')]:
...
def choose_from_datasets(dataset_dictionary):
    ...
    available.read([
     os.path.join(root, 'cosmology', 'observational_data', 'available_observables.ini'),
     os.path.join(user_folder, 'modifications', 'cosmology', 'observational_data', 'available_observables.ini')])
    datasets = {}
    for key in dataset_dictionary:
        ...
        for name in names:
            datasets[name] = eval(available['DatasetType'][name])(key, available[key][name] or name, available[key]['predicting_function_name'])

    if check_conflicting_datasets(datasets):
        from EPIC import DataError
        raise DataError('Two or more datasets are conflicting. Check conflicting_dataset_pairs.txt in EPIC/cosmology/observational_data/')
    return datasets