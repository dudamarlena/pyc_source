...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/rafae/repo/epic/EPIC/gui/custom_widgets.py
# Compiled at: 2018-07-19 16:37:41
# Size of source mod 2**32: 37991 bytes
import os, threading
...
import matplotlib.gridspec as gridspec, tkinter as tk
...
class ReadonlyCombobox(ttk.Combobox):
    ...
    def __init__(self, *args, state='readonly', **kwargs):
        ...
        self.bind('<<ComboboxSelected>>', func=lambda _: self.selection_clear())


class MenuButtonOptions(object):

    def __init__(self, parent, text=None, direction='above', padding=3, width=0, tearoff=1, menutitle='', options=[], current=0, command=None, menubackground='White', image=None, **menubtn_kw):
        self.var = tk.IntVar()
        self.var.set(current)
        if image:
            img_path = os.path.splitext(image)[0] + '.' + btn_img_ext
            image = tk.PhotoImage(file=img_path)
        self.btn = ttk.Menubutton(parent, direction=direction, width=width, padding=padding, text=text, image=image, **menubtn_kw)
        if image:
            self.btn.image = image
        self.menu = tk.Menu(self.btn, title=menutitle, tearoff=tearoff, background=menubackground)
        for item in options:
            if isinstance(item, str):
                ...
                label = str(item)
                value = item
            self.menu.add_radiobutton(label=label, value=value, variable=self.var, command=command)
...
class ScrollableFrame(ttk.Frame):
    ...
    def __init__(self, parent, background, **canvas_kw):
        ...
        self.canvas = tk.Canvas(parent, background=background, highlightthickness=0, **canvas_kw)
...
class ListboxEditable(tk.Listbox):
    ...
    def edit(self, editevent=None):
        ...
        extraheight = 4
        self.entry.place(x=x - 2, y=y - 2 - extraheight // 2, width=self.winfo_width() - self['borderwidth'] - self['highlightthickness'] - 1, height=h + extraheight)
...
class TwoPanes(ttk.PanedWindow):
    ...
    def __init__(self, *args, label=None, labelstyle='TLabel', pack=True, framestyle='TFrame', **kwargs):
        super().__init__(*args, **kwargs)
        ...
        if label:
            ttk.Label(self.second, style=labelstyle, text=label).pack(side=tk.TOP if orient == tk.VERTICAL else tk.LEFT)
        if pack:
...
class PriorLineSetup(object):
    ...
    def __init__(self, parent, text, label, row=0, background='white', par_label_width=10, cb_width=16, xpadding={}):
        self.label = label
        self.parent = parent
        self.frame = ttk.Frame(parent)
        self.frame.pack(anchor=tk.W)
        self.bg = background
        MyTextLabel(self.frame, text, background=self.bg, width=par_label_width, ending=':').grid(row=row, column=0, sticky=tk.E, **xpadding)
        ...
        self.parameters_entry = [ttk.Entry(self.frame, width=6, font=('Times', 16 - platform_reduce_font_parameter), textvariable=var) for var in self.parameters_var]
        for i, entry in enumerate(self.parameters_entry):
            entry.grid(row=row, column=2 + i, sticky=tk.W)

        self.add_separator(manager=[row, 4])
        MyTextLabel(self.frame, '\\sigma', background=self.bg, ending='=   ', width=6).grid(row=row, column=5, sticky=tk.E)
        self.sig_var = tk.DoubleVar()
        self.sig_var.set('')
        self.sig_entry = ttk.Entry(self.frame, width=6, font=('Times',
         16 - platform_reduce_font_parameter), textvariable=self.sig_var)
        self.sig_entry.grid(row=row, column=6, stick=tk.W)
        self.prior_dist.bind('<<ComboboxSelected>>', self.par_states_with_prior_type, add='+')
        self.row = row
    ...
    def add_unit(self, unit_label, orig_unit_lbl, frame=None, xpadding={}):
        ...
        t = MyTextLabel(unit_frame, unit_label, background=self.bg, justify=tk.LEFT, width=round(2 + 1.2 * len(orig_unit_lbl.replace(' ', '').replace('^', ''))))
        if frame is None:
    ...
    def add_separator(self, W=21, manager='pack'):
        h = 30
        C = tk.Canvas(self.frame, background=self.bg, highlightthickness=0, width=W, height=h)
...
class MyNavigationToolbar(NavigationToolbar2Tk):
    ...
    def __init__(self, canvas, parent, parent_figframe, plot_name, xscale_toggle=False, yscale_toggle=False, difference_toggle=False, customize_plot_buttons=True, **kwargs):
        ...
        buttons = [
         ...
         ('Pan', 'Pan axes with left click, zoom with right click', 'move', 'pan'),
         (None, None, None, None)]
        if customize_plot_buttons:
            if xscale_toggle:
                buttons.append(('Toggle scale', 'Toggle x-axis scale (linear/log)',
                                'xscale', 'toggle_scale_xaxis'))
            if yscale_toggle:
                buttons.append(('Toggle scale', 'Toggle y-axis scale (linear/log)',
                                'yscale', 'toggle_scale_yaxis'))
            if xscale_toggle or yscale_toggle:
                buttons.append((None, None, None, None))
            if difference_toggle:
                buttons.extend([
                 ('Residual', 'Show/hide fractional difference', 'residual', 'show_hide_difference_plot'),
                 (None, None, None, None)])
            buttons.extend([
             ('Change grid', 'Change grid style (thick/solid/dashed/dotted/off)', 'change_grid',
 'change_grid'),
             ('Change line width', 'Change line widths (0.5/1.0/1.5/2.0)', 'line_width', 'change_line_width'),
             ('Toggle color mode', 'Toggle line mode (colors/styles)', 'color_mode', 'toggle_color_mode'),
             ('Change legend', 'Change legend (off/on/on+title)', 'legend', 'change_legend'),
             (None, None, None, None)])
        buttons.extend([
         (
          'Save',
          'Save the figure and data files' if customize_plot_buttons else 'Save figure',
          'filesave',
    ...
    def _Spacer(self):
        s = Separator(master=self, height=21, relief=tk.RIDGE, pady=2, bg='#dddddd')
    ...
    def _Button(self, text, file, command, extension=None):
        ...
        b = ttk.Button(self, text=text, style='White.Flat.TButton', image=im, command=command)
        b._ntimage = im
        b.pack(side=tk.LEFT)
        return b
    ...
    def hide_difference_plot(self, fig, ax):
        ...
        fig.delaxes(ax[1])
        ax[0].tick_params(axis='x', which='both', direction='out', top=False, bottom=True)
        ax[0].tick_params(axis='y', which='both', direction='out', left=True, right=False)
    ...
    def show_difference_plot(self, fig, ax):
        ...
        grid = {'state': ax.xaxis._gridOnMajor or ax.xaxis._gridOnMinor or ax.yaxis._gridOnMajor or ax.yaxis._gridOnMinor, 
        ...
        if plt.rcParams['text.usetex']:
            res_ylabel = res_ylabel.replace('\\rm', '\\text')
        elif '$' in line0_label:
            if line0_label.startswith('$'):
                sep_rm, l0label = line0_label.lstrip('$').split('$', 1)
                res_ylabel = res_ylabel.replace('\\rm{' + line0_label, sep_rm + '\\rm{' + l0label)
            if line0_label.endswith('$'):
                l0label, sep_rm = split_last(line0_label.rstrip('$'), sep='$')
                res_ylabel = res_ylabel.replace(line0_label + '}', l0label + '}' + sep_rm)
            if not line0_label.startswith('$') and not line0_label.endswith('$'):
                a, l0label, b = line0_label.split('$', 2)
                res_ylabel = res_ylabel.replace(line0_label, a + '}' + l0label + '\\rm{' + b)
            res_ylabel = res_ylabel.replace(' ', '}\\,\\rm{')
        res.set_ylabel(res_ylabel)
        if grid['state']:
            for axes in [main, res]:
                axes.grid(which='minor', alpha=self.parent_figframe.ALPHA_MINOR, linestyle=grid['ls'], linewidth=grid['lw'])
                axes.grid(which='major', alpha=self.parent_figframe.ALPHA_MAJOR, linestyle=grid['ls'], linewidth=grid['lw'])
                axes.tick_params(axis='x', which='both', direction='in', top=True, bottom=True)
                axes.tick_params(axis='y', which='both', direction='in', left=True, right=True)
    ...
    def save_figure(self):
        ...
        for line in ax[0].get_lines():
            untex_label = line.get_label().replace('$\\Lambda$', 'Λ')
            line_label = self.map_species.get(untex_label, untex_label) if hasattr(self, 'map_species') else untex_label
            savetxt(os.path.join(local, '{0}_{1}.txt'.format(self.plot_name, split_last(line_label)[0]).replace(' ', '_')), line.get_xydata(), header='{0} plot, data for {1} curve: {2}, {3}'.format(self.plot_name, line_label, xdata_name, ydata_name))

        img_name = os.path.join(local, '{0}_plot'.format(self.plot_name).replace(' ', '_').replace('$', ''))
    ...
    def save_figure_only(self):
        ...
        if len(ax) == 0:
            return
        local = filedialog.asksaveasfilename(title='Save as...', defaultextension='.pdf', filetypes=[('PDF file', '*.pdf')], initialdir=EPIC.root, initialfile='{0}_plot.pdf'.format(self.plot_name).replace(' ', '_').replace('$', ''))
        if local:
            fig.savefig(local)
    ...
    def change_grid(self):
        ...
        for ax in axes:
            ax.grid(bool(style), which='minor', linestyle=style, linewidth=width, alpha=self.parent_figframe.ALPHA_MINOR)
            ax.grid(bool(style), which='major', linestyle=style, linewidth=width, alpha=self.parent_figframe.ALPHA_MAJOR)
    ...
    def toggle_color_mode(self):
        ...
        for ax in axes:
            ...
            if all([color == plt.rcParams['axes.labelcolor'] for color in previous_colors]):
...
class BasicFigureFrame(ttk.Frame):
    figproperties = {'tight_layout': True, 
     'constrained_layout': False}

    def __init__(self, *args, plot_name=None, xscale_toggle=False, yscale_toggle=False, difference_toggle=False, **kwargs):
        ...
        v_panes = TwoPanes(sup_pane, label='↕ Drag the borders to resize plot ↕', labelstyle='TLabel', framestyle='White.TFrame')
        h_panes = TwoPanes(v_panes.main, label='↔', orient=tk.HORIZONTAL, labelstyle='TLabel', framestyle='White.TFrame')
        self.canvas = FigureCanvasTkAgg(self.fig, h_panes.main)
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
        self.toolbar = MyNavigationToolbar(self.canvas, nav_pane, self, plot_name, xscale_toggle=xscale_toggle, yscale_toggle=yscale_toggle, difference_toggle=difference_toggle, customize_plot_buttons=customize_plot_buttons)
...
class FigureFrame(BasicFigureFrame):
    ...
    def __init__(self, *args, xlabel=None, ylabel=None, yscale='linear', **kwargs):
        super().__init__(*args, **kwargs)
        self.yscale = yscale
        self.xlabel = xlabel
        self.ylabel = ylabel

    def add_figure_to_widget(self, x, ydict, exclude=[], factor=1, ysecond=None, title=None, map_species={}, sec_weights=[1, 1]):
        self.toolbar.map_species = map_species

        def plot_from_dictionary(d, ax):
            for key, solution in d.items():
                if key not in exclude:
                    ax.plot(x, factor * solution, lw=2, label=map_species.get(key, key).replace('Λ', '$\\Lambda$'))

        reset_figure(self.fig, **self.figproperties)
        ax = self.fig.add_subplot(111)
        ax.format_coord = lambda X, Y: format_coord(ax, X, Y)
        plot_from_dictionary(ydict, ax)
        ax.set_xscale('log' if log10(x[(-1)]) > 1 or self.xlabel == '$a$' else 'linear')
    ...
    def add_distance_comparison_to_widget(self, plot_name, instructions, labels):
        reset_figure(self.fig, **self.figproperties)
        ax = self.fig.add_subplot(111)
        ax.format_coord = lambda X, Y: format_coord(ax, X, Y)
        for inst, label in zip(instructions, labels):
            ax.plot(inst['x'], inst['ydict'].get(plot_name, inst.get('ysecond', {}).get(plot_name, None)), label=label)
...
class MyTextLabel(tk.Text):
    ...
    def __init__(self, parent, label, justify=tk.RIGHT, background=None, reduce_=None, width=10, ending='= ', **kwargs):
        ...
        self.tag_config('times', offset=-reduce_, font=(
         'Times', self.regular_size))
        self.tag_config('align', offset=-reduce_, justify=justify)
        self.tag_config('italic', offset=-reduce_, font=(
         'Times', self.regular_size, 'italic'))
        self.tag_config('sub', offset=-3 - reduce_, font=(
         'Times', self.script_size))
        self.tag_config('subitalic', offset=-3 - reduce_, font=('Times',
         self.script_size, 'italic'))
        self.tag_config('super', offset=3 + self.regular_size - self.script_size - reduce_, font=(
         'Times', self.script_size))
        self.tag_config('superitalic', offset=3 + self.regular_size - self.script_size - reduce_, font=(
         'Times', self.script_size, 'italic'))
...
class TwoValues(object):
    ...
    def __init__(self, master, vartype, value, values, width=4):
        ...
        for v, box in list(zip(value, ['inf', 'sup']))[::-1]:
            ...
            ttk.Entry(master, width=width, textvariable=getattr(self, box)).pack(side=tk.RIGHT, anchor=tk.E)
...
def paint_backgrounds(widget, background=None):
    try:
        if not isinstance(widget, Separator):
            widget.config(bg=background or 'white')
    except tk.TclError:
        pass
...
def residual_label(label, ref):
    ...
    return '$\\frac{' + res + '}{' + res + '_{\\rm{' + ref + '}}}-1$'
...
def format_coord(ax, x, y):
    ...
    xs = '???' if x is None else '%.5e' % x
    ys = '???' if y is None else '%.5e' % y
    names = [format_label(getattr(axis.get_label(), 'get_text')()) for axis in ax._get_axis_list()]
    return ', '.join([format_with_unit(name, s) for name, s in zip(names, [xs, ys])])
...
def configure_ttk(theme='clam'):
    ...
    s.map('Readonly.TCombobox', background=[
     (
      'disabled', frame_bg),
     (
      'active', active_bg)], foreground=[
     (
      'disabled', disabled_text)], fieldforeground=[
     (
      'disabled', disabled_text)], fieldbackground=[
     (
      'disabled', frame_bg)])
    ...
    s.map('TLabelFrame', foreground=[
     (
      'disabled', disabled_text)])
    s.map('Flat.TButton', relief=[
    ...
    return (
     frame_bg, foreground_text)


def prior_combobox(frame, width=16):
    cb = ReadonlyCombobox(frame, width=width, values=[
     'Fixed at', 'Flat, between', 'Gaussian, (μ, σ) = '])
    cb.current(1)
    return cb