...
# [GCC 8.4.0]
# Embedded file name: c:\users\rafae\repo\epic\EPIC\gui\custom_widgets.py
# Compiled at: 2018-07-17 23:59:39
...
class ReadonlyCombobox(ttk.Combobox):
    ...
    def __init__(self, *args, state='readonly', **kwargs):
        (super().__init__)(args, state=state, style='Readonly.TCombobox', **kwargs)
...
class MenuButtonOptions(object):
    ...
    def __init__(self, parent, text=None, direction='above', padding=3, width=0, tearoff=1, menutitle='', options=[], current=0, command=None, menubackground='White', image=None, **menubtn_kw):
        ...
        self.btn = (ttk.Menubutton)(parent, direction=direction, width=width, padding=padding, 
         text=text, image=image, **menubtn_kw)
        if image:
        ...
        for item in options:
            if isinstance(item, str):
                ...
                label = str(item)
                value = item
            self.menu.add_radiobutton(label=label, value=value, variable=(self.var),
              command=command)
...
class ListboxEditable(tk.Listbox):
    ...
    def edit(self, editevent=None):
        ...
        extraheight = 4
        self.entry.place(x=(x - 2), y=(y - 2 - extraheight // 2), width=(self.winfo_width() - self['borderwidth'] - self['highlightthickness'] - 1),
...
class PriorLineSetup(object):
    ...
    def __init__(self, parent, text, label, row=0, background='white', par_label_width=10, cb_width=16, xpadding={}):
        ...
        self.bg = background
        (MyTextLabel((self.frame), text, background=(self.bg), width=par_label_width,
          ending=':').grid)(row=row, column=0, sticky=tk.E, **xpadding)
        ...
        self.prior_dist.bind('<<ComboboxSelected>>', (self.par_states_with_prior_type),
          add='+')
        self.row = row
    ...
    def add_unit(self, unit_label, orig_unit_lbl, frame=None, xpadding={}):
        ...
        t = MyTextLabel(unit_frame, unit_label, background=(self.bg),
          justify=(tk.LEFT),
          width=(round(2 + 1.2 * len(orig_unit_lbl.replace(' ', '').replace('^', '')))))
        if frame is None:
    ...
    def add_separator(self, W=21, manager='pack'):
        h = 30
        C = tk.Canvas((self.frame), background=(self.bg), highlightthickness=0, width=W,
          height=h)
 ...
 'change_grid'),
    ...
    def show_difference_plot(self, fig, ax):
        ...
        for line in ax.lines:
            ...
            for prop in ('agg_filter', 'animated', 'children', 'clip_box', 'clip_on',
                         'clip_path', 'contains', 'data', 'drawstyle', 'figure',
                         'gid', 'path', 'path_effects', 'picker', 'pickradius', 'rasterized',
                         'sketch_params', 'snap', 'solid_capstyle', 'solid_jointstyle',
                         'transform', 'transformed_clip_path_and_affine', 'url',
                         'visible', 'xdata', 'ydata', 'xydata'):
        ...
        if grid['state']:
            for axes in [main, res]:
                axes.grid(which='minor', alpha=(self.parent_figframe.ALPHA_MINOR), linestyle=(grid['ls']),
                  linewidth=(grid['lw']))
                axes.grid(which='major', alpha=(self.parent_figframe.ALPHA_MAJOR), linestyle=(grid['ls']),
                  linewidth=(grid['lw']))
    ...
    def save_figure(self):
        ...
        for line in ax[0].get_lines():
            ...
            savetxt((os.path.join(local, '{0}_{1}.txt'.format(self.plot_name, split_last(line_label)[0]).replace(' ', '_'))),
              (line.get_xydata()),
              header=('{0} plot, data for {1} curve: {2}, {3}'.format(self.plot_name, line_label, xdata_name, ydata_name)))
    ...
    def save_figure_only(self):
        ...
        if len(ax) == 0:
            return
        local = filedialog.asksaveasfilename(title='Save as...', defaultextension='.pdf',
          filetypes=[('PDF file', '*.pdf')],
          initialdir=(EPIC.root),
          initialfile=('{0}_plot.pdf'.format(self.plot_name).replace(' ', '_').replace('$', '')))
        if local:
            fig.savefig(local)
    ...
    def change_grid(self):
        ...
        for ax in axes:
            ax.grid((bool(style)), which='minor', linestyle=style, linewidth=width,
              alpha=(self.parent_figframe.ALPHA_MINOR))
            ax.grid((bool(style)), which='major', linestyle=style, linewidth=width,
              alpha=(self.parent_figframe.ALPHA_MAJOR))
...
class BasicFigureFrame(ttk.Frame):
    ...
    def __init__(self, *args, plot_name=None, xscale_toggle=False, yscale_toggle=False, difference_toggle=False, **kwargs):
        ...
        v_panes = TwoPanes(sup_pane, label='↕ Drag the borders to resize plot ↕', labelstyle='TLabel',
          framestyle='White.TFrame')
        h_panes = TwoPanes((v_panes.main), label='↔', orient=(tk.HORIZONTAL), labelstyle='TLabel',
          framestyle='White.TFrame')
        self.canvas = FigureCanvasTkAgg(self.fig, h_panes.main)
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(side=(tk.TOP), fill=(tk.BOTH), expand=1)
        self.toolbar = MyNavigationToolbar((self.canvas), nav_pane, self,
          plot_name, xscale_toggle=xscale_toggle, yscale_toggle=yscale_toggle,
...
class FigureFrame(BasicFigureFrame):
    ...
    def add_distance_comparison_to_widget(self, plot_name, instructions, labels):
        ...
        for inst, label in zip(instructions, labels):
            ax.plot((inst['x']), (inst['ydict'].get(plot_name, inst.get('ysecond', {}).get(plot_name, None))),
              label=label)
...
class MyTextLabel(tk.Text):
    ...
    def __init__(self, parent, label, justify=tk.RIGHT, background=None, reduce_=None, width=10, ending='= ', **kwargs):
        ...
        self.tag_config('super', offset=(3 + self.regular_size - self.script_size - reduce_),
          font=(
         'Times', self.script_size))
        self.tag_config('superitalic', offset=(3 + self.regular_size - self.script_size - reduce_),
          font=(
