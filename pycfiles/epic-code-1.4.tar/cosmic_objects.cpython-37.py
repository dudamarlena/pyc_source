...
# [GCC 8.4.0]
# Embedded file name: c:\users\rafae\repo\epic\EPIC\cosmology\cosmic_objects.py
# Compiled at: 2018-07-16 23:26:14
# Size of source mod 2**32: 40397 bytes
import numpy as np
...
class CosmologicalSetup(object):
    ...
    def __init__(self, model, optional_species=[], combined_species=[], interaction_setup={}, physical=True, derived=None, a0=1):
        ...
        species_labels = self.get_species(available_species,
        ...
        for fluid in species_labels:
            ...
            eos = EquationOfState(eos_type, value=eos_value, parameters=eos_parameters_obj,
              woa=eos_woa)
            physical_densities = 'physical ' if self.physical_density_parameters else ''
            if derived == fluid:
                density_parameter = DerivedParameter((available_species[('%sdensity parameter' % physical_densities)][fluid]),
                  (list(set(species_labels) - set([fluid]))),
                  tex=(available_species[('tex %sdensity parameter' % physical_densities)][fluid]),
                  physical=(bool(self.physical_density_parameters)))
            else:
                if fluid == 'radiation':
                    temperature_parameter = FreeParameter((available_species[('%sdensity parameter' % physical_densities)][fluid]),
                      (available_species[('tex %sdensity parameter' % physical_densities)][fluid]),
                      model_name=(self.model))
                    density_parameter = DensityFromTemperature(temperature_parameter,
                      physical=(bool(self.physical_density_parameters)),
                      model_name=(self.model))
                else:
                    density_parameter = DensityParameter((available_species[('%sdensity parameter' % physical_densities)][fluid]),
                      (available_species[('tex %sdensity parameter' % physical_densities)][fluid]),
                      physical=(bool(self.physical_density_parameters)),
                      model_name=(self.model))
            if fluid in interaction_setup.get('species', []):
                ...
                this_fluid = InteractingFluid(fluid,
                  eos, density_parameter, interaction_setup={'parameter':int_parameter, 
        ...
        if self.physical_density_parameters:
            if derived is None:
                Hubble_parameter = HubbleDerivedParameter('h', 'Hubble', [par for par in self.parameters if isinstance(par, DensityParameter)],
        ...
        for fluid in self.species.values():
            if not isinstance(fluid.density_parameter, DerivedParameter) or isinstance(fluid.density_parameter, (
             HubbleDerivedParameter, DerivedDensityDistribution,
             DerivedParameterDensityDistribution,
             DerivedDensityFromTemperature)):
                if self.physical_density_parameters or isinstance(fluid.density_parameter, DensityFromTemperature):
    ...
    def get_luminosity_distance(self, zhel, zcmb, **kwargs):
        luminosity_distance = (1 + zhel) / self.a0 * integrate.quad(lambda Z: 1 / (self.get_Hubble_Friedmann_of_z)(
         Z, **kwargs), 0, zcmb)[0]
    ...
    def get_distance_moduli(self, z, nuisance_magnitude, **kwargs):
        mu = 5 * np.log10(np.array([(self.get_luminosity_distance)(Z, Z, **kwargs) for Z in z],
          ndmin=2) / 10)
        M = (nuisance_magnitude['M'].get_value)(**kwargs)
        return M + mu

    def get_distance_moduli_full(self, z, **kwargs):
        return 5 * np.log10(np.array([(self.get_luminosity_distance)(zhel, zcmb, **kwargs) for zhel, zcmb in z],
          ndmin=2) / 10)
    ...
    def get_BAO_ratio(self, z, isotropic=True, **kwargs):
        if self.physical_density_parameters:
            ...
            Och2 = (self.species['cdm'].density_parameter.get_value)(**kwargs)
        else:
            h = (self.HubbleParameter.get_value)(**kwargs) / 100
            h2 = h * h
            Obh2 = (self.species['baryons'].density_parameter.get_value)(**kwargs) * h2
            Och2 = (self.species['cdm'].density_parameter.get_value)(**kwargs) * h2
        Omh2 = Obh2 + Och2
 ...
 [fluid.drho_da for fluid in self.species.values()], intermediate=[
 self.get_Hubble_Friedmann], 
                     a0=self.a0, **kwargs)
            ...
            if isinstance(self, InteractingFluid):
                ...
                if not hasattr(self, 'interacts_with'):
                    if self.EoS.type in ('constant', 'cosmological_constant', 'pressureless'):
                        ...
                        w0 = 0
                        exp_integral = True
                    return (self.rho_parametric_EoS)(a, w0=w0, exp_integral=exp_integral, 
                     xi=xi, a0=a0, **kwargs)
                if self.EoS.cosmological_constant:
                    ...
                    if self.interacts_with.EoS.type in ('cosmological_constant', 'constant',
                        ...
                        return (self.interacting_rho_parametric_EoS)(a, wb=wb, expintb=1, 
                         xi=xi, a0=a0, **kwargs)
                    return (self.interacting_rho_parametric_EoS)(a, wb=0, expintb=self.interacts_with, 
                     xi=xi, a0=a0, **kwargs)
                else:
...
class InteractingFluid(Fluid):
    ...
    def __init__(self, name, w, density_parameter, interaction_setup={}, model_name=None):
        ...
        if self.interaction_parameter is not None:
            ...
            self.interaction_parameter = FreeParameter((self.interaction_parameter), tex=tex_parameter,
              model_name=model_name)
        else:
            self.interaction_sign = interaction_setup.get('sign')
            self.interaction_propto_other = interaction_setup.get('propto_other')
            if self.interaction_propto_other is None and not self.interaction_sign is not None:
                raise AssertionError
            else:
         ...
         [
          'default_flatpriors', 'default_parameter_flatpriors.ini', '']):
            ...
            default_values.read([
             ...
             os.path.join(root, 'modifications', 'cosmology', source)])
            model_name_or_default = model_name if model_name in default_values else 'DEFAULT'
            setattr(self, attribute, eval(default_values[model_name_or_default].get(label, alt)))
...
class DensityFromTemperature(DensityParameter):
    ...
    def __init__(self, temp_parameter, physical=True, model_name='DEFAULT'):
        ...
        super().__init__((self.temperature.label), (self.temperature.tex), physical=physical,
          model_name=model_name)
