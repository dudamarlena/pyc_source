...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/rafae/repo/epic/EPIC/gui/__init__.py
# Compiled at: 2018-09-17 11:28:26
# Size of source mod 2**32: 96067 bytes
...
class Application(object):
    ...
    distances_to_compare = [
     ...
     'Lookback time $c\\,t(z)$',
     'Luminosity $d_L(z)$']
    styles_list = [style.replace('_', ' ') for style in EPIC.pyplot_available_styles]
    mcmc_styles_list = [style.replace('_', ' ') for style in EPIC.pyplot_grid_styles]
    ...
    def __init__(self, run_parser, analyze_parser, plot_parser, analyze_function, master=None, theme='clam'):
        self.rccustom()
        ...
        icon = tk.Image('photo', file=os.path.join(EPIC.root, 'gui_files', 'iconepic.gif'))
        self.master.tk.call('wm', 'iconphoto', self.master._w, icon)
        self.frame_bg, self.foreground_text = EPIC_widgets.configure_ttk(theme=theme)
    ...
    def set_tex_option(self, update_other=True):
        ...
        if option != self.current_tex_option:
            ...
            if self.texprefix in option:
                ...
                font_preamble = io_tools.tex_packages(io_tools.available_fonts[option[len(self.texprefix) + 1:]])
                plt.rcParams.update({'text.latex.preamble': font_preamble, 
    ...
    def set_font_size(self, menubutton, otherbutton, updating_from_other=True):
    ...
    def set_chosen_plot_style(self, update_other=True):
        ...
        if style != self.current_style:
            ...
            preserve_tex_options = dict((key, plt.rcParams[key]) for key in [
             'text.usetex',
             'text.latex.preamble',
             'text.latex.unicode',
             'text.latex.preview',
    ...
    def remake_plots(self):
        ...
        if viewing == 'plots':
            ...
            self.compare_distances(models=self.all_models, labels=self.all_labels)
    ...
    def set_mcmc_style(self, update_other=True):
        ...
        if style != self.current_mcmc_style:
            ...
            plt.style.use(style.replace(' ', '_'))
            plt.rcParams.update(preserve_tex_options)
            plt.setp(self.MCMCplot_tab.fig, 'facecolor', plt.rcParams['figure.facecolor'])
            self.check_and_remake_mcmc_plot()

    def remake_mcmc_plot(self, args, parser, sim, include_defaults=False):
        self.master.config(cursor='exchange')
        self.master.update()
        print(' '.join(io_tools.get_parsed_args(args, parser, include_defaults=include_defaults, command='plot')))
    ...
    def create_widgets(self):
        ...
        self.available_models.read([
         ...
         os.path.join(EPIC.user_folder, 'modifications', 'cosmology', 'model_recipes.ini')])
        ...
        self.available_species.read([
         ...
         os.path.join(EPIC.user_folder, 'modifications', 'cosmology', 'available_species.ini')])
        self.models = self.available_models.sections()
        self.models = OrderedDict((model, self.available_models[model]['name']) for model in self.models if 'name' in self.available_models[model])
        self.map_model = {self.please_select_model: self.please_select_model}
        for model in self.models:
            if 'name' in self.available_models[model]:
                self.map_model[model] = self.available_models[model]['name'].replace('$\\Lambda$', 'Λ').replace('$w$CDM', 'wCDM')

        remap(self.map_model)
        self.map_parameters = {'h': 'h', 'H0': 'H_0', 'xi': '\\xi'}
        ...
        self.status_bar = tk.Label(self.master, bg=self.foreground_text, fg='white', anchor=tk.W, relief=tk.FLAT, **self.default_padding)
        self.status_bar.config(text="Welcome to the EPIC's Cosmology Calculator. " + 'Choose a model to start, then specify the parameters ' + 'and make some plots.')
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)
        self.parameters_setup_notebook = ttk.Notebook(self.master, height=self.BOTTOM_RIGHT_FRAME_HEIGHT - self.PADY)
        self.parameters_setup_notebook.pack(side=tk.BOTTOM, fill=tk.X, **(self.default_iypadding), **(self.default_padding))
        parameter_space_frame = ttk.Frame(self.parameters_setup_notebook)
        self.parameters_setup_notebook.add(parameter_space_frame, text='Specify parameters')
        constrain_parameters_frame = ttk.Frame(self.parameters_setup_notebook)
        self.parameters_setup_notebook.add(constrain_parameters_frame, text='Constrain with MCMC')
        self.MCMC_terminal = EPIC_widgets.TwoPanes(self.parameters_setup_notebook, pack=False, orient=tk.HORIZONTAL)
        self.MCMC_output = EPIC_widgets.ConsoleText(self.MCMC_terminal.main, relief=tk.FLAT)
        ...
        for labelframe, w in [
         ...
         ('Marginalized fits', 34)]:
            lf = ttk.LabelFrame(self.MCMC_terminal.second, relief=tk.FLAT, text=labelframe + ':')
            lf.pack(side=tk.LEFT, fill=tk.BOTH, expand=1, **self.default_padding)
            scrolltext = EPIC_widgets.ScrollableText(lf, width=w, highlightthickness=0, state=tk.DISABLED)
        ...
        left_frame = ttk.Frame(parameter_space_frame, width=self.LEFT_FRAME_WIDTH)
        left_frame.pack(side=tk.LEFT, fill=tk.Y)
        left_frame.pack_propagate(False)
        left_frame_MCMC = ttk.Frame(constrain_parameters_frame, width=self.WIDTH // 2 + 20)
        ...
        self.parameter_priors = ttk.LabelFrame(left_frame_MCMC, text='Priors  |  Diagonal covariance matrix for proposal function (Multivariate Gaussian)  |  Unit:')
        self.parameter_priors.pack(fill=tk.BOTH, expand=1, **self.default_padding)
        ...
        datasets_label_frame = ttk.LabelFrame(constrain_parameters_frame, text='Datasets:', width=480)
        datasets_label_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=1, anchor=tk.W, **self.default_padding)
        datasets_label_frame.pack_propagate(False)
        datasets_frame = EPIC_widgets.ScrollableFrame(datasets_label_frame, self.frame_bg)
        self.data_cosmic_chrono = tk.IntVar()
        self.data_cosmic_chrono.set(1)
        ttk.Checkbutton(datasets_frame.frame, text='Cosmic chronometer H(z)', variable=self.data_cosmic_chrono).pack(anchor=tk.W, **self.default_padding)
        self.data_local_Hubble = tk.IntVar()
        self.data_local_Hubble.set(1)
        ttk.Checkbutton(datasets_frame.frame, text='Hubble local Measurement H(z=0)', variable=self.data_local_Hubble).pack(anchor=tk.W, **self.default_padding)
        SNe_frame = ttk.LabelFrame(datasets_frame.frame, text='SNe Ia Joint Light-curve Analysis:', relief=tk.FLAT)
        ...
        ttk.Checkbutton(SNe_frame, text='Simple', variable=self.data_JLA_simple, command=self.sne_simple_toggle).pack(anchor=tk.W)
        self.jla_simple_nuisance_par = EPIC_widgets.PriorLineSetup(SNe_frame, 'M', 'M', background=self.frame_bg, par_label_width=6, cb_width=14, xpadding=self.default_xpadding)
        config_all_widgets(self.jla_simple_nuisance_par.frame, state=tk.DISABLED)
        ttk.Checkbutton(SNe_frame, text='Full', variable=self.data_JLA_full, command=self.sne_full_toggle).pack(anchor=tk.W)
        self.jla_full_nuisance_pars = [EPIC_widgets.PriorLineSetup(SNe_frame, label[0], label[1], row=i, background=self.frame_bg, par_label_width=6, cb_width=14, xpadding=self.default_xpadding) for i, label in enumerate([
         ('\\alpha', 'alpha'),
         ('\\beta', 'beta'),
         ('M_B', 'M_B'),
         ('\\Delta{M}', 'DeltaM')])]
        ...
        for bao_set in bao_list:
            ...
            self.BAO_dict_check[bao_set] = ttk.Checkbutton(self.BAO_frame, text=bao_set, variable=self.BAO_dict[bao_set])
        ...
        self.CMB_frame = ttk.LabelFrame(datasets_frame.frame, text='CMB distance priors:', relief=tk.FLAT)
        ...
        self.data_CMB_LCDM_check = OrderedDict((
         (
          2015,
          ttk.Checkbutton(self.CMB_frame, text='Planck 2015 ΛCDM', variable=self.data_CMB_LCDM[2015], command=lambda : self.cmb_lcdm_toggle(2015))),
         (
          2018,
          ttk.Checkbutton(self.CMB_frame, text='Planck 2018 ΛCDM', variable=self.data_CMB_LCDM[2018], command=lambda : self.cmb_lcdm_toggle(2018)))))
        self.data_CMB_wCDM_check = OrderedDict((
         (
          2015,
          ttk.Checkbutton(self.CMB_frame, text='Planck 2015 wCDM', variable=self.data_CMB_wCDM[2015], command=lambda : self.cmb_wcdm_toggle(2015))),
         (
          2018,
          ttk.Checkbutton(self.CMB_frame, text='Planck 2018 wCDM', variable=self.data_CMB_wCDM[2018], command=lambda : self.cmb_wcdm_toggle(2018)))))
        ...
        simulation_grid.pack(anchor=tk.W, fill=tk.BOTH, expand=1, **self.default_padding)
        simulation_labelframe = ttk.LabelFrame(simulation_grid, text='Simulation:')
        sim_buttons = ttk.Frame(simulation_grid)
        sim_buttons.pack(side=tk.BOTTOM, fill=tk.X, anchor=tk.S, expand=1)
        simulation_labelframe.pack(side=tk.TOP, anchor=tk.N, fill=tk.BOTH, expand=1)
        self.load_button = ttk.Button(sim_buttons, text='Load', width=4, command=self.load_sim)
        self.load_button.pack(side=tk.LEFT, fill=tk.BOTH, expand=1)
        self.export_ini = ttk.Button(sim_buttons, text='Export', width=4, command=self.export_ini_only)
        self.export_ini.pack(side=tk.LEFT, fill=tk.BOTH, expand=1)
        self.MCMC_button = ttk.Button(sim_buttons, text='Start', width=4, command=self.export_ini_and_run_MCMC)
        self.MCMC_button.pack(side=tk.RIGHT, fill=tk.BOTH, expand=1)
        sim_options_frame = EPIC_widgets.ScrollableFrame(simulation_labelframe, background=self.frame_bg, width=147)
        nlines = 0
        for label, attribute, vartype, value, width, values in [
         ...
         (
          '# of bins:', 'bins_var', tk.IntVar, EPIC._defaults_['bins'], 4,
          (10, 60, 5)),
         (
         ...
         (
          ...
          'proposal_cov_file', 'choose_proposal_cov_file')]:
            ...
            if isinstance(value, list):
                widget = None
                setattr(self, attribute, EPIC_widgets.TwoValues(fr, vartype, value, values, width=width))
            else:
                setattr(self, attribute, vartype())
                getattr(self, attribute).set(value)
            if isinstance(values, tuple):
                from_, to_, delta = values
                widget = tk.Spinbox(fr, from_=from_, to_=to_, increment=delta, width=width - 1, textvariable=getattr(self, attribute), buttonbackground=self.frame_bg, buttondownrelief=tk.FLAT, buttonuprelief=tk.FLAT)
            else:
                if isinstance(values, str):
                    widget = ttk.Checkbutton(fr, text='', variable=getattr(self, attribute), command=getattr(self, values))
                    ...
                    if isinstance(values, list):
                        ...
                        if width is None:
                            widget = ttk.Checkbutton(fr, text='', variable=getattr(self, attribute))
                        else:
                            widget = ttk.Entry(fr, width=width, textvariable=getattr(self, attribute))
            if widget:
        ...
        for menubtn, text, menutitle, options, current, command, imgfile in [
         ...
         (
          ...
          lambda : self.set_font_size('mcmc_fontsize_choice', 'fontsize_choice', updating_from_other=False), 'fontsize.png')]:
            img_path = os.path.join(EPIC.root, 'gui_files', 'images', imgfile) if imgfile else None
            setattr(self, menubtn, EPIC_widgets.MenuButtonOptions(mcmcplotcfg, text=text, tearoff=0, menutitle=menutitle, options=options, current=current, command=command, menubackground=self.frame_bg, image=img_path, style='Flat.TMenubutton'))
            var_value = self.fs_list.index(current) if menubtn == 'mcmc_fontsize_choice' else current
        ...
        self.frame_scroll_priors = EPIC_widgets.ScrollableFrame(self.parameter_priors_baseframe, self.frame_bg)
        right_frame = ttk.Frame(parameter_space_frame, width=self.RIGHT_FRAME_WIDTH)
        ...
        model_setup_notebook.pack(side=tk.LEFT, fill=tk.Y, **self.default_padding)
        ...
        for plot, xlabel, ylabel, xscale, yscale, xtoggle, ytoggle in [
         ('Density parameters', '$a$', '$\\Omega$', 'log', 'linear', True, False),
         ('Energy densities', '$a$', '$\\rho \\, [\\rm{kg}/\\rm{m}^3]$', 'log', 'log', True,
 False),
         ('Distances', '$z$', '$d(z) \\, [\\rm{Mpc}]$', 'log', 'log', True, True),
         ('Lookback time', '$z$', '$t(z) \\, [\\rm{Gyr}]$', 'log', 'linear', True, True),
         ('Equation of state', '$a$', '$w(a)$', 'log', 'linear', False, False)]:
            self.model_solution_tabs_dict[plot] = EPIC_widgets.FigureFrame(self.plots_notebook, plot_name=plot, xlabel=xlabel, ylabel=ylabel, yscale=yscale, xscale_toggle=xtoggle, yscale_toggle=ytoggle, difference_toggle=False)
            self.plots_notebook.add(self.model_solution_tabs_dict[plot], text=plot, padding=EPIC_widgets.PlotNotebook_padding)

        self.distance_comparison_tabs_dict = OrderedDict((name, EPIC_widgets.FigureFrame(self.plots_notebook, plot_name=name, xlabel='$z$', ylabel=io_tools.split_last(name)[1].rstrip('$') + '\\, [\\rm{Mpc}]$', yscale='log', difference_toggle=True, xscale_toggle=True, yscale_toggle=True)) for name in self.distances_to_compare)
        for name in self.distances_to_compare:
            self.plots_notebook.add(self.distance_comparison_tabs_dict[name], text=io_tools.split_last(name)[0], padding=EPIC_widgets.PlotNotebook_padding)

        self.MCMCplot_tab = EPIC_widgets.BasicFigureFrame(self.plots_notebook, plot_name='MCMC', customize_plot_buttons=False)
        self.plots_notebook.add(self.MCMCplot_tab, text='MCMC', padding=EPIC_widgets.PlotNotebook_padding)
        ...
        include_plots_frame.pack(side=tk.LEFT, anchor=tk.NW, fill=tk.Y, expand=1)
        include_plots_LF = ttk.LabelFrame(include_plots_frame, relief=tk.FLAT, text='Include plots of:')
        include_plots_LF.pack(anchor=tk.NW, fill=tk.Y, expand=1, **self.default_padding)
        corner_frame = ttk.Frame(right_frame, width=(self.RIGHT_FRAME_WIDTH - 0 * self.PADX) // 4)
        corner_frame.pack(side=tk.LEFT, fill=tk.Y, expand=1, **self.default_padding)
        ...
        self.solve_button = ttk.Button(self.corner_frame_buttons, state=tk.DISABLED, text='Solve background\ncosmology', command=self.get_cosmo_solution)
        self.solve_button.grid(row=0, column=0, columnspan=3, sticky=tk.W + tk.E)
        calc_at_z_btn = ttk.Button(self.corner_frame_buttons, text='Calculate at:', state=tk.DISABLED, command=self.calculate_at_z)
        calc_at_z_btn.grid(row=1, column=0, sticky=tk.NW + tk.SW)
        EPIC_widgets.MyTextLabel(self.corner_frame_buttons, 'z', width=4, background=self.frame_bg).grid(row=1, column=1, sticky=tk.W + tk.E, **self.default_xpadding)
        self.z_var = tk.DoubleVar()
        self.z_var.set(1.0)
        at_z_entry = ttk.Entry(self.corner_frame_buttons, width=4, font=('Times',
         14 - EPIC_widgets.platform_reduce_font_parameter), state=tk.DISABLED, textvariable=self.z_var)
        at_z_entry.grid(row=1, column=2, sticky=tk.NW + tk.SE)
        self.age_label = ttk.Label(corner_frame, justify=tk.LEFT, anchor=tk.SW, text='')
        self.age_label.pack(anchor=tk.SW, expand=1)
        self.saved_models_frame = ttk.Frame(right_frame)
        self.saved_models_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=1, **self.default_padding)
        ...
        ttk.Button(select_buttons_frame, style='Flat.TButton', command=self.rename_active, text='Rename').grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Label(select_buttons_frame, text='Select:', anchor=tk.E).grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Button(select_buttons_frame, text='All', width=4, command=self.comparison_select_all, style='Flat.TButton').grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Button(select_buttons_frame, width=4, command=self.comparison_clear_selection, style='Flat.TButton', text='None').grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Button(select_buttons_frame, width=4, command=self.comparison_invert_selection, style='Flat.TButton', text='Invert').grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Button(select_buttons_frame, width=1, command=self.empty_list, style='Flat.TButton', text='❌').grid(row=0, column=next(columns_iter), sticky=tk.NW + tk.SE)
        ttk.Button(self.saved_models_frame, text='Compare distances', command=self.new_comparison).pack(side=tk.BOTTOM, fill=tk.BOTH, expand=1)
        ...
        self.saved_models_listbox = EPIC_widgets.ListboxEditable(listbox_frame, selectmode=tk.EXTENDED)
        ...
        for i, tab in enumerate(self.model_results_tabs_states.keys()):
            ...
            self.model_results_checkbuttons_dict[tab] = ttk.Checkbutton(include_plots_LF, text=tab, variable=self.model_results_checkbuttons_var[tab], command=getattr(self, 'show_hide_%s' % tab.replace(' ', '_')))
            self.model_results_checkbuttons_dict[tab].grid(row=i, column=0, columnspan=1 if tab == 'Distances' else 4, sticky=tk.NW + tk.SW)

        ttk.Label(include_plots_LF, text='(z max:').grid(row=2, column=1, sticky=tk.E)
        self.zrange_combobox = EPIC_widgets.ReadonlyCombobox(include_plots_LF, values=[
         '5', '100', '10³', '10⁴'], width=3, state=tk.DISABLED)
        self.zrange_combobox.current(3)
        self.zrange_combobox.grid(row=2, column=2, sticky=tk.W + tk.E)
        ttk.Label(include_plots_LF, text=')').grid(row=2, column=3, sticky=tk.W)
        ...
        for menubtn, text, menutitle, options, current, command, imgfile in [
         ...
         (
          ...
          lambda : self.set_font_size('fontsize_choice', 'mcmc_fontsize_choice', updating_from_other=False), 'fontsize.png')]:
            img_path = os.path.join(EPIC.root, 'gui_files', 'images', imgfile) if imgfile else None
            setattr(self, menubtn, EPIC_widgets.MenuButtonOptions(textcfg, text=text, menutitle=menutitle, options=options, current=current, command=command, menubackground=self.frame_bg, image=img_path))
        ...
        cb_model_values = [
         self.please_select_model] + [self.map_model[model] for model in self.models.keys()]
        self.model_options_combobox = EPIC_widgets.ReadonlyCombobox(model_setup_LF, width=round(210 / EPIC_widgets.pixels_to_char_size), values=cb_model_values)
        self.model_options_combobox.current(0)
        next_row = next(iter_rows)
        ttk.Label(model_setup_LF, text='Model:').grid(row=next_row, column=0, sticky=tk.E, **self.default_padding)
        self.model_options_combobox.grid(row=next_row, column=1, sticky=tk.W, **self.default_padding)
        self.model_options_combobox.bind('<<ComboboxSelected>>', func=self.show_optional_species, add='+')
        next_row = next(iter_rows)
        ttk.Label(model_setup_LF, text='Mandatory species:').grid(row=next_row, column=0, sticky=tk.E, **self.default_padding)
        self.mandatory_msg = ttk.Label(model_setup_LF, width=round(230 / EPIC_widgets.pixels_to_char_size), text=self.please_select_model + '\n', padding=self.PADX)
        self.mandatory_msg.grid(row=next_row, column=1, sticky=tk.W, **self.default_ypadding)
        next_row = next(iter_rows)
        self.optional_species_listbox = tk.Listbox(model_setup_LF, selectmode=tk.MULTIPLE, height=2, exportselection=0)
        ttk.Label(model_setup_LF, text='Add optional species:').grid(row=next_row, column=0, sticky=tk.E, **self.default_padding)
        self.optional_species_listbox.grid(row=next_row, column=1, sticky=tk.W, **self.default_padding)
        ...
        self.combined_option = ttk.Checkbutton(cb_frame, variable=self.combined_var, state=tk.DISABLED, text='Use combined matter\nfluid (cdm+baryons)')
        self.combined_option.pack(side=tk.LEFT, expand=1, anchor=tk.CENTER, **self.default_padding)
        self.physical_var = tk.IntVar()
        self.physical_var.set(1)
        ttk.Checkbutton(cb_frame, text='Use physical\ndensities', variable=self.physical_var).pack(side=tk.LEFT, expand=1, anchor=tk.CENTER, **self.default_padding)
        next_row = next(iter_rows)
        self.derived_lbl = ttk.Label(model_setup_LF, justify=tk.RIGHT, state=tk.DISABLED, text='Use density of this fluid\nas derived parameter:')
        self.derived_lbl.grid(row=next_row, column=0, sticky=tk.E, **self.default_padding)
        self.derived_combobox = EPIC_widgets.ReadonlyCombobox(model_setup_LF, state=tk.DISABLED, values=[self.please_select_species], width=8)
        self.derived_combobox.current(0)
        self.derived_combobox.grid(row=next_row, column=1, sticky=tk.W, **self.default_padding)
        next_row = next(iter_rows)
        self.int_frame = ttk.Frame(model_setup_LF)
        self.int_frame.grid(row=next_row, column=1, sticky=tk.W + tk.E)
        self.interaction_propto_options = EPIC_widgets.ReadonlyCombobox(self.int_frame, width=8, values=[self.please_select_species])
        self.int_propto_label = ttk.Label(model_setup_LF, justify=tk.RIGHT, text='Interaction setup -\nProportional to density of:', state=tk.DISABLED)
        self.int_propto_label.grid(row=next_row, column=0, sticky=tk.E, **self.default_padding)
        self.interaction_propto_options.current(0)
        self.interaction_propto_options.pack(side=tk.LEFT, anchor=tk.W, **self.default_padding)
        self.int_sign_combobox = EPIC_widgets.ReadonlyCombobox(self.int_frame, width=5, values=['(sign)', '+1', '-1'])
        self.int_sign_combobox.current(1)
        self.int_sign_combobox.pack(side=tk.LEFT, anchor=tk.CENTER, **self.default_padding)
        ttk.Button(self.int_frame, style='Flat.TButton', text='Set', width=5, command=self.set_and_update_gif).pack(side=tk.LEFT, anchor=tk.W, **self.default_padding)
        config_all_widgets(self.int_frame, state=tk.DISABLED)
        self.darksector_and_build_frame = ttk.Frame(model_setup_LF)
        self.darksector_and_build_frame.grid(row=next(iter_rows), column=0, columnspan=2)
        img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'blank.gif'))
        img_lbl_text = ttk.LabelFrame(self.darksector_and_build_frame, text='Dark sector:')
        ...
        self.img_lbl.pack()
        ttk.Button(self.darksector_and_build_frame, text='Build\nnew\nmodel', width=5, command=self.build_model).pack(side=tk.RIGHT, expand=1, anchor=tk.S, **(self.default_ipadding), **(self.default_padding))
    ...
    def read_bao_states(self):
        ...
        for bao_set, check in self.BAO_dict_check.items():
            self.BAO_states[bao_set] = tk.DISABLED not in check.state()

    def bao_sdss_boss_toggle(self):
        use = any([check.get() for k, check in self.BAO_dict.items() if k not in ('WiggleZ',
                                                                                  '6dF+SDSS_MGS')])
        self.BAO_dict_check['WiggleZ'].config(state=tk.DISABLED if use or not self.has_baryons_and_radiation() else tk.NORMAL)
        self.read_bao_states()

    def bao_wigglez_toggle(self):
        use = self.BAO_dict['WiggleZ'].get()
        for k, check in self.BAO_dict_check.items():
            if k not in ('WiggleZ', '6dF+SDSS_MGS'):
                check.config(state=tk.DISABLED if use or not self.has_baryons_and_radiation() else tk.NORMAL)
    ...
    def show_hide_tabs(self, which_tabs):
        ...
        for tab in which_tabs:
            if show:
                self.plots_notebook.add(set_of_tabs[tab], padding=EPIC_widgets.PlotNotebook_padding)
            else:
    ...
    def determine_solve_btn_state(self):
        if hasattr(self, 'cosmo_model') and any([check.get() for check in self.model_results_checkbuttons_var.values()]):
            state = tk.NORMAL
        else:
            state = tk.DISABLED
    ...
    def compare_distances(self, models=None, labels=None):
        if models is None:
            models = [self.list_of_solved_models[i] for i in self.saved_models_listbox.curselection()]
        if labels is None:
            labels = self.saved_models_listbox.get(0, tk.END)
            labels = [labels[i].replace('Λ', '$\\Lambda$').replace('wCDM', '$w$CDM') for i in self.saved_models_listbox.curselection()]
        ...
        for plot_name in self.distances_to_compare:
            instructions = [model.results['Distances'] for model in models]
            self.distance_comparison_tabs_dict[plot_name].add_distance_comparison_to_widget(plot_name, instructions, labels)

        if self.viewing != 'dist':
            for plot_name in self.distances_to_compare:
                self.plots_notebook.add(self.distance_comparison_tabs_dict[plot_name], padding=EPIC_widgets.PlotNotebook_padding)
    ...
    def visible_tab(self, tab):
        return tab in self.plots_notebook.tabs() and self.plots_notebook.tab(tab)['state'] == 'normal'

    def add_prior_config(self, baseframe, i, par):
        par.prior_setup = EPIC_widgets.PriorLineSetup(baseframe, self.map_parameters[par.label], par.label, row=i, background=self.frame_bg, xpadding=self.default_xpadding)
        if par.label == 'T_CMB':
            par.prior_setup.prior_dist.current(0)
        par.prior_setup.prior_dist.bind('<<ComboboxSelected>>', func=par.set_default_priors, add='+')
    ...
    def choose_proposal_cov_file(self):
        filename = filedialog.askopenfilename(initialdir=EPIC.user_folder, defaultextension='.txt', filetypes=[
         ('TXT file', '*.txt'),
         ('All files', '*.*')])
        self.proposal_cov_file.set(filename)
        self.cov_loaded.set(int(bool(filename)))
        self.status_bar.config(text='Proposal covariance file {0} loaded. Click the load button again and click Cancel to unload.'.format(filename) if filename else 'No file was loaded.')

    def export_ini_only(self, run=False):
        if not hasattr(self, 'cosmo_model'):
            messagebox.showerror('Error', 'Please, set up a model first.')
            return
        self.inifile = filedialog.asksaveasfilename(title='Export ini file...', defaultextension='.ini', filetypes=[
         ('INI file', '*.ini')], initialdir=EPIC.user_folder, initialfile='{0}.ini'.format(self.cosmo_model.model))
        if not self.inifile:
            return
        if EPIC._OS == 'Windows':
    ...
    def get_args_list(self):
        args_list = [
          ...
          self.inifile,
          str(self.chains_var.get()),
          str(self.steps_var.get()),
          '-c', self.check_interval_var.get(),
          '--tolerance', str(self.tol_var.get()),
    ...
    def start_simulation(self, load_previous=None):
        ...
        tabId = '.' + self.parameters_setup_notebook.winfo_name() + '.' + self.MCMC_terminal.winfo_name()
        ...
        if self.sim.start(args, external_fig=self.MCMCplot_tab, external_texts=self.MCMC_simulation_info, plot_args=plot_args):
            rargs_list = [
             self.sim.working_dir, '--convergence', '--dont-plot']
            if self.kde_var.get():
                rargs_list.append('--kde')
            rargs = self.analyze_parser.parse_args(args=rargs_list)
            self.analyze(self.sim, args=rargs, external_texts=self.MCMC_simulation_info)
        ...
        with open(self.tempfile_name, 'w') as (f):
            if getattr(self, 'args_list') is not None:
                ...
                if '--check-interval' in args:
                    ...
                    args[interval_index] = io_tools.revert_TimeString(int(args[interval_index]))
                f.write('\n'.join([
            ...
            f.write(' = '.join(['physical',
             ('yes' if self.cosmo_model.physical_density_parameters else 'no') + '\n']))
            for key in [
             'optional species',
             'combined species',
             'interaction setup']:
            ...
            if self.data_JLA_simple.get():
                ...
                for bao_set, check in self.BAO_dict.items():
                    if check.get() and tk.DISABLED not in self.BAO_dict_check[bao_set].state():
            ...
            for year in [2015, 2018]:
                if self.data_CMB_LCDM[year].get() and tk.DISABLED not in self.data_CMB_LCDM_check[year].state():
                    datasets_dict['CMB'] = 'Planck%i_distances_LCDM' % year
                    datasets_label.append('CMB')
                    break
                elif self.data_CMB_wCDM[year].get() and tk.DISABLED not in self.data_CMB_wCDM_check[year].state():
    ...
    def build_model(self):
        ...
        if 'cde' in modelname:
            if self.interaction_propto_options.get() == self.please_select_species or self.int_sign_combobox.get() == '(sign)':
            ...
            int_setup = {'species': self.mandatory_species[:2], 
        ...
        self.cosmo_model = cosmo.CosmologicalSetup(modelname, optional_species=optional, combined_species=combined, interaction_setup=int_setup, physical=bool(self.physical_var.get()), derived=derived_opt, a0=1)
        ...
        for widget in self.parameter_values.winfo_children() + self.frame_scroll_priors.frame.winfo_children():
            widget.destroy()

        default_units = configparser.ConfigParser()
        default_units.read([
         os.path.join(EPIC.root, 'cosmology', 'default_parameter_units.ini'),
         os.path.join(EPIC.user_folder, 'modifications', 'cosmology', 'default_parameter_units.ini')])
        self.free_parameters = list(filter(lambda p: isinstance(p, cosmo.FreeParameter), self.cosmo_model.parameters))
        sep = int(ceil(len(self.free_parameters) / 2))
        for i, par in enumerate(self.free_parameters):
            col = 0 if i < sep else 2
            EPIC_widgets.MyTextLabel(self.parameter_values, self.map_parameters[par.label], background=self.frame_bg).grid(row=i % sep, column=col, sticky=tk.E, **self.default_xpadding)
            ...
            ttk.Entry(par.entry_frame, width=6, font=('Times',
             16 - EPIC_widgets.platform_reduce_font_parameter), textvariable=par.entry_var).pack(side=tk.LEFT, anchor=tk.W)
            if par.label in default_units['DEFAULT']:
                ...
                for frame in [par.entry_frame, None]:
                    par.prior_setup.add_unit(unit_label, orig_unit_lbl, frame=frame, xpadding=self.default_xpadding)
        ...
        if len(self.variable_eos):
            self.model_results_checkbuttons_dict['Equation of state'].config(state=tk.NORMAL)
        else:
        ...
        if self.has_baryons_and_radiation():
            ...
            for bao_set, check in self.BAO_dict_check.items():
                check.config(state=tk.NORMAL if self.BAO_states[bao_set] else tk.DISABLED)
    ...
    def calculate_at_z(self):
        z = self.z_var.get()
        try:
            parameters = dict((par.label, par.entry_var.get()) for par in self.free_parameters)
        except tk.TclError:
            messagebox.showerror('Error', 'Invalid float value.')
            return
        ...
        Vave = (da ** 2 * z * D_H) ** 0.3333333333333333
        age_z = self.cosmo_model.get_age_of_the_universe(z, parameter_space=parameters)
        sep = '; '
        self.status_bar.config(text=self.map_model[self.cosmo_model.model] + ' - ' + 'At z = %s: ' % str(z) + sep.join(['Age of the universe: %.2f Gyr' % age_z, 'Comoving distance: %.2f Mpc' % da, 'Angular diameter distance: %.2f Mpc' % (da * self.cosmo_model.a0 / (1 + z)), 'Luminosity distance: %.2f Mpc' % (da * (1 + z) / self.cosmo_model.a0), 'Hubble distance: %.2f Mpc' % D_H]) + '.')

    def get_function_mp(self, model_d, key, shared_dict, z, **kwargs):
        factor = kwargs.pop('factor', 1)
        shared_dict[key] = factor * getattr(self.cosmo_model, model_d)(z, **kwargs)
    ...
    def get_cosmo_solution(self):
        ...
        age = self.cosmo_model.get_age_of_the_universe(0, parameter_space=parameters)
        if age:
            self.age_label.config(text='Age of the universe today:\n%.3f Gyr.' % age)
        hubble = self.cosmo_model.HubbleParameter.get_value(parameter_space=parameters)
        rho_cr0 = cosmo.rho_critical_SI(hubble * (100 if self.cosmo_model.physical_density_parameters else 1))
        self.cosmo_model.results = OrderedDict()
        self.cosmo_model.model_solution_plots_shown = []
        title = ' '.join([
         self.available_models[self.cosmo_model.model].get('tex', self.map_model[self.cosmo_model.model]).replace('text', 'rm'),
         'model'])
        if self.show_tab('Density parameters'):
            self.cosmo_model.results['Density parameters'] = {'x': self.cosmo_model.a_range, 
             'ydict': self.cosmo_model.background_solution_Omegas, 
             'title': title, 
             'map_species': self.map_species_abbrv}
            self.cosmo_model.model_solution_plots_shown.append(self.model_solution_tabs_dict['Density parameters'].winfo_name())
        if self.show_tab('Energy densities'):
            self.cosmo_model.results['Energy densities'] = {'x': self.cosmo_model.a_range, 
             'ydict': self.cosmo_model.background_solution_rhos, 
             'exclude': ['total'], 
             'factor': rho_cr0 * (hubble ** (-2) if self.cosmo_model.physical_density_parameters else 1), 
             
             'title': title, 
             'map_species': self.map_species_abbrv}
            self.cosmo_model.model_solution_plots_shown.append(self.model_solution_tabs_dict['Energy densities'].winfo_name())
        man = multiprocessing.Manager()
        distances_mdict = man.dict()
        jobs = []
        if self.show_tab('Distances'):
            self.cosmo_model.model_solution_plots_shown.extend([self.model_solution_tabs_dict[w].winfo_name() for w in [
             'Distances',
             'Lookback time']])
            ...
            if logzmax == 1:
                ...
                z_range = logspace(-logzmax, min(logzmax, log10(cosmo.z_of_a(self.cosmo_model.a_range[0]))), 500)
            for function, key in [
             ...
             ('get_age_of_various_z', 'Age of the universe')]:
                ...
                if function == 'get_age_of_various_z' and not self.cosmo_model.directly_solvable:
                    kw.update({'a_zinf': a_of_z(z_range[(-1)])})
                if EPIC._OS == 'Windows':
                    self.get_function_mp(function, key, distances_mdict, z_range, **kw)
                else:
                    jobs.append(multiprocessing.Process(target=self.get_function_mp, args=(
                     function, key, distances_mdict, z_range), kwargs=kw))

        if self.show_tab('Equation of state'):
            self.cosmo_model.model_solution_plots_shown.append(self.model_solution_tabs_dict['Equation of state'].winfo_name())
            eos_mdict = man.dict()
            for key, species in self.variable_eos.items():
                jobs.append(multiprocessing.Process(target=eos_get_function_mp, args=(
                 species.EoS.w_of_a, key, eos_mdict,
                 self.cosmo_model.a_range), kwargs={'a0': self.cosmo_model.a0, 
        ...
        if self.show_tab('Distances'):
            distances_mdict['Comoving luminosity distance'] = distances_mdict['Comoving angular distance'] * (1 + z_range) ** 2
            distances_mdict['Angular distance'] = distances_mdict['Comoving angular distance'] / (1 + z_range)
            distances_mdict['Luminosity distance'] = distances_mdict['Comoving luminosity distance'] / (1 + z_range)
             ...
             (
              ...
              {'x': z_range, 
               'ydict': OrderedDict([
                         ...
                         (
                          'Lookback time $c\\,t(z)$',
                          distances_mdict['Lookback time'] * speedoflight / 1000.0 / 976.480247152)]), 
    ...
    def set_and_update_gif(self):
        ...
        if species == 'idm':
            if model == 'cde':
                img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'cde_propto_c_%s.gif' % sign))
            else:
                assert model == 'cde lambda'
                img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'ilambda_propto_c_%s.gif' % sign))
        else:
            if species == 'ilambda':
                img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'ilambda_propto_de_%s.gif' % sign))
            else:
                assert species == 'ide'
                img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'cde_propto_de_%s.gif' % sign))
        self.img_lbl.config(image=img)
        self.img_lbl.image = img
    ...
    def show_optional_species(self, event):
        ...
        config_all_widgets(self.darksector_and_build_frame, state=tk.DISABLED if no_model else tk.NORMAL)
        if no_model:
            img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'blank.gif'))
            self.img_lbl.config(image=img)
            self.img_lbl.image = img
        ...
        self.mandatory_msg.config(text='\n'.join([self.map_species[species] for species in self.mandatory_species]))
        ...
        if 'cde' in model:
            self.combined_var.set(0)
            self.interaction_propto_options['values'] = [self.please_select_species] + [self.map_species_abbrv[species] for species in self.mandatory_species]
        else:
            self.interaction_propto_options['values'] = [
             self.please_select_species]
        self.interaction_propto_options.current(0)
        ...
        img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'blank.gif'))
        if model == 'lcdm':
            img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'lcdm.gif'))
        elif model in ('wcdm', 'cpl', 'jbp', 'ba', 'lh', 'fnt2', 'fnt3'):
            img = tk.PhotoImage(file=os.path.join(EPIC.root, 'gui_files', 'wde.gif'))
        self.img_lbl.config(image=img)
        self.img_lbl.image = img
    ...
    def show_densities(self):
        ...
        for plot_name, instructions in self.cosmo_model.results.items():
            self.model_solution_tabs_dict[plot_name].add_figure_to_widget(**instructions)
...
def remap(d):
    d.update({v:k for k, v in d.items() if k != v if k != v})


def launch_gui(args, run_parser, analyze_parser, plot_parser, analyze_function):
    plt.rcdefaults()
    master = tk.Tk()
    app = Application(run_parser, analyze_parser, plot_parser, analyze_function, master=master, theme=args.theme)
...
def config_all_widgets(w, **kwargs):
    for widget in w.winfo_children():
        try:
            ...
            elif isinstance(widget, EPIC_widgets.MyTextLabel) and 'state' in kwargs:
...
def break_list(_list, ident=1):
    return '\n'.join(['['] + ['    ' * ident + quote(e) + ',' for e in _list] + ['    ' * ident + ']'])
