...
# [GCC 8.4.0]
# Embedded file name: c:\users\rafae\repo\epic\EPIC\cosmology\observations.py
# Compiled at: 2018-07-11 20:44:35
...
class BAO_isotropic_DataPoints(DataPoints):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        self.points, self.obs, self.errors = np.loadtxt((os.path.join(self.location, 'data.txt')),
          ndmin=2, unpack=True)
        with open(os.path.join(self.location, 'fiducial.p'), 'rb') as (fhandle):
            self.fiducial = pickle.load(fhandle)
        self.predicting = predicting_function_name
...
class JLA_SimplifiedDataset(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable)
        self.points, self.obs = np.loadtxt((os.path.join(self.location, dataset_loc, 'data', 'jla_mub.txt')),
          ndmin=2, unpack=True)
        self.covariance_matrix = np.loadtxt((os.path.join(self.location, dataset_loc, 'data', 'jla_mub_covmatrix.dat')),
...
class full_JLA_likelihood(Dataset):
    ...
    def __init__(self, observable, dataset_loc, predicting_function_name):
        ...
        self.location = os.path.join(root, 'cosmology', 'observational_data', observable, dataset_loc)
        z_cmb, z_hel, muB, x1, color, self.log10MB = np.loadtxt((os.path.join(self.location, 'data', 'jla_lcparams.txt')),
          unpack=True,
          usecols=(1, 2, 4, 6, 8, 10))
        self.points = list(zip(z_hel, z_cmb))
        scoh, slens, sz = np.loadtxt((os.path.join(self.location, 'covmat', 'sigma_mu.txt')),
          unpack=True)
        ...
        self.C_eta = np.sum([fits.getdata(mat) for mat in glob(os.path.join(self.location, 'covmat', 'C*.fits'))],
          axis=0)
...
def check_conflicting_datasets(datasets):
    for path in [
     ...
     os.path.join(root, 'modifications', 'cosmology', 'observational_data', 'conflicting_dataset_pairs.txt')]:
        with open(path, 'r') as (fhandle):
            ...
            while line:
                if line.startswith('#'):
                    ...
                    a, b = line.split(',')
                    if a.strip() in datasets:
                        if b.strip() in datasets:
                            return True
                line = fhandle.readline()
...
def choose_from_datasets(dataset_dictionary):
    ...
    available.read([
     ...
     os.path.join(root, 'modifications', 'cosmology', 'observational_data', 'available_observables.ini')])
    datasets = {}
