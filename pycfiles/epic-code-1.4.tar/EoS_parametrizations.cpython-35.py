...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/rafae/repo/epic/EPIC/cosmology/EoS_parametrizations.py
# Compiled at: 2018-09-16 01:46:14
...
default_values.read([
 ...
 os.path.join(user_folder, 'modifications', 'cosmology', 'default_parameter_values.ini')])
...
def de_jbp(a, parameter_space={}, accepts_default=False, a0=1, expIwoa=False, **kwargs):
    model = 'jbp'
    w0 = get_value('w0', model, parameter_space=parameter_space, accepts_default=accepts_default)
    w1 = get_value('w1', model, parameter_space=parameter_space, accepts_default=accepts_default)
    if expIwoa:
        return (a / a0) ** w0 * np.exp(w1 * (a * (1 - a / 2) - a0 * (1 - a0 / 2)))
    return w0 + w1 * a * (1 - a)
