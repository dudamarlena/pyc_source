...
# [GCC 8.4.0]
# Embedded file name: c:\users\rafae\repo\epic\EPIC\gui\__init__.py
# Compiled at: 2018-07-18 12:18:20
# Size of source mod 2**32: 93653 bytes
...
import EPIC.gui as EPIC_widgets
import os
...
class Application(object):
    ...
    def set_tex_option(self, update_other=True):
        ...
        if option != self.current_tex_option:
            ...
            if update_other:
                if option in self.tex_options['MCMC']:
    ...
    def set_mcmc_tex_option(self, update_other=True):
        ...
        if option != self.current_mcmc_tex_option:
            ...
            if update_other:
                if option in self.tex_options['regular']:
    ...
    def set_chosen_plot_style(self, update_other=True):
        ...
        if style != self.current_style:
            ...
            if update_other:
                if style in self.mcmc_styles_list:
                    self.mcmc_style_choice.var.set(self.mcmc_styles_list.index(style))
                    self.set_mcmc_style(update_other=False)
            preserve_tex_options = dict(((key, plt.rcParams[key]) for key in ('text.usetex',
                                                                              'text.latex.preamble',
                                                                              'text.latex.unicode',
                                                                              'text.latex.preview',
                                                                              'font.family',
    ...
    def remake_mcmc_plot(self, args, parser, sim, include_defaults=False):
        self.master.config(cursor='exchange')
        self.master.update()
        print(' '.join(io_tools.get_parsed_args(args, parser, include_defaults=include_defaults,
          command='plot')))
    ...
    def create_widgets(self):
        ...
        self.status_bar = (tk.Label)(self.master, bg=self.foreground_text, fg='white', 
         anchor=tk.W, relief=tk.FLAT, **self.default_padding)
        ...
        self.MCMC_terminal = EPIC_widgets.TwoPanes((self.parameters_setup_notebook), pack=False,
        ...
        for labelframe, w in (('Acceptance rates', 16), ('# of accepted states', 16),
            ...
            scrolltext = EPIC_widgets.ScrollableText(lf, width=w, highlightthickness=0,
              state=(tk.DISABLED))
        ...
        datasets_label_frame = ttk.LabelFrame(constrain_parameters_frame, text='Datasets:',
          width=480)
        ...
        SNe_frame = ttk.LabelFrame((datasets_frame.frame), text='SNe Ia Joint Light-curve Analysis:',
          relief=(tk.FLAT))
        ...
        ttk.Checkbutton(SNe_frame, text='Simple', variable=(self.data_JLA_simple),
          command=(self.sne_simple_toggle)).pack(anchor=(tk.W))
        self.jla_simple_nuisance_par = EPIC_widgets.PriorLineSetup(SNe_frame, 'M',
          'M', background=(self.frame_bg), par_label_width=6, cb_width=14,
          xpadding=(self.default_xpadding))
        config_all_widgets((self.jla_simple_nuisance_par.frame), state=(tk.DISABLED))
        ttk.Checkbutton(SNe_frame, text='Full', variable=(self.data_JLA_full),
        ...
        for bao_set in bao_list:
            ...
            self.BAO_dict_check[bao_set] = ttk.Checkbutton((self.BAO_frame), text=bao_set,
              variable=(self.BAO_dict[bao_set]))
        ...
        self.CMB_frame = ttk.LabelFrame((datasets_frame.frame), text='CMB distance priors:',
          relief=(tk.FLAT))
        (self.CMB_frame.pack)(anchor=tk.W, **self.default_padding)
        self.data_CMB_LCDM = tk.IntVar()
        self.data_CMB_wCDM = tk.IntVar()
        self.data_CMB_LCDM_check = ttk.Checkbutton((self.CMB_frame), text='Planck 2015 ΛCDM',
          variable=(self.data_CMB_LCDM),
          command=(self.cmb_lcdm_toggle))
        self.data_CMB_LCDM_check.pack(anchor=(tk.W))
        self.data_CMB_wCDM_check = ttk.Checkbutton((self.CMB_frame), text='Planck 2015 wCDM',
          variable=(self.data_CMB_wCDM),
          command=(self.cmb_wcdm_toggle))
        self.data_CMB_wCDM_check.pack(anchor=(tk.W))
        ...
        self.export_ini = ttk.Button(sim_buttons, text='Export', width=6, command=(self.export_ini_only))
        self.export_ini.pack(side=(tk.LEFT), fill=(tk.BOTH), expand=1)
        self.MCMC_button = ttk.Button(sim_buttons, text='Start', width=5, command=(self.export_ini_and_run_MCMC))
        self.MCMC_button.pack(side=(tk.RIGHT), fill=(tk.BOTH), expand=1)
        sim_options_frame = EPIC_widgets.ScrollableFrame(simulation_labelframe, background=(self.frame_bg),
          width=147)
        nlines = 0
        for label, attribute, vartype, value, width, values in [
         ...
         (
          '# of chains:', 'chains_var', tk.IntVar, 4,
          multiprocessing.cpu_count(), (2, 128, 1)),
         (
          '# of steps:', 'steps_var', tk.IntVar, 5000, 4, None),
         (
          'Check every:', 'check_interval_var', tk.StringVar,
          io_tools.revert_TimeString(EPIC._defaults_['check_interval']), 4,
          None),
         (
          'Tolerance:', 'tol_var', tk.DoubleVar, EPIC._defaults_['tolerance'], 4,
          None),
         (
         ...
         (
          '# of bins:', 'bins_var', tk.IntVar, EPIC._defaults_['bins'], 4, (10, 60, 5)),
         (
          'Final plot with KDE:', 'kde_var', tk.IntVar, 0, None, None),
         (
         ...
         (
          ...
          'proposal_cov_file', 'choose_proposal_cov_file')]:
            ...
            if isinstance(value, list):
                widget = None
                setattr(self, attribute, EPIC_widgets.TwoValues(fr, vartype, value,
                  values, width=width))
            else:
            ...
            if isinstance(values, tuple):
                ...
                widget = tk.Spinbox(fr, from_=from_, to_=to_, increment=delta, width=(width - 1),
                  textvariable=(getattr(self, attribute)),
                  buttonbackground=(self.frame_bg),
                  buttondownrelief=(tk.FLAT),
                  buttonuprelief=(tk.FLAT))
            elif isinstance(values, str):
                widget = ttk.Checkbutton(fr,
                  text='', variable=(getattr(self, attribute)), command=(getattr(self, values)))
        ...
        for menubtn, text, menutitle, options, current, command, imgfile in [
         ...
         (
          ...
          lambda : self.set_font_size('mcmc_fontsize_choice', 'fontsize_choice',
            updating_from_other=False), 'fontsize.png')]:
            img_path = os.path.join(EPIC.root, 'gui_files', 'images', imgfile) if imgfile else None
            setattr(self, menubtn, EPIC_widgets.MenuButtonOptions(mcmcplotcfg,
              text=text,
              tearoff=0,
              menutitle=menutitle,
              options=options,
        ...
        for plot, xlabel, ylabel, xscale, yscale, xtoggle, ytoggle in (('Density parameters', '$a$', '$\\Omega$', 'log', 'linear', True, False),
                                                                       ('Energy densities', '$a$', '$\\rho \\, [\\rm{kg}/\\rm{m}^3]$', 'log', 'log', True, False),
            ...
            self.model_solution_tabs_dict[plot] = EPIC_widgets.FigureFrame((self.plots_notebook),
              plot_name=plot, xlabel=xlabel, ylabel=ylabel,
              yscale=yscale,
              xscale_toggle=xtoggle,
              yscale_toggle=ytoggle,
              difference_toggle=False)
            self.plots_notebook.add((self.model_solution_tabs_dict[plot]), text=plot,
        ...
        for name in self.distances_to_compare:
            self.plots_notebook.add((self.distance_comparison_tabs_dict[name]), text=(io_tools.split_last(name)[0]),
              padding=(EPIC_widgets.PlotNotebook_padding))

        self.MCMCplot_tab = EPIC_widgets.BasicFigureFrame((self.plots_notebook), plot_name='MCMC',
          customize_plot_buttons=False)
        ...
        calc_at_z_btn = ttk.Button((self.corner_frame_buttons), text='Calculate at:',
          state=(tk.DISABLED),
        ...
        at_z_entry = ttk.Entry((self.corner_frame_buttons), width=4, font=('Times',
         14 - EPIC_widgets.platform_reduce_font_parameter),
          state=(tk.DISABLED),
        ...
        ttk.Button(select_buttons_frame, style='Flat.TButton', command=(self.rename_active),
          text='Rename').grid(row=0, column=(next(columns_iter)),
          sticky=(tk.NW + tk.SE))
        ttk.Label(select_buttons_frame, text='Select:', anchor=(tk.E)).grid(row=0,
          column=(next(columns_iter)),
          sticky=(tk.NW + tk.SE))
        ttk.Button(select_buttons_frame, text='All', width=4, command=(self.comparison_select_all),
          style='Flat.TButton').grid(row=0, column=(next(columns_iter)), sticky=(tk.NW + tk.SE))
        ttk.Button(select_buttons_frame, width=4, command=(self.comparison_clear_selection),
          style='Flat.TButton',
          text='None').grid(row=0, column=(next(columns_iter)), sticky=(tk.NW + tk.SE))
        ttk.Button(select_buttons_frame, width=4, command=(self.comparison_invert_selection),
          style='Flat.TButton',
          text='Invert').grid(row=0, column=(next(columns_iter)), sticky=(tk.NW + tk.SE))
        ...
        self.saved_models_listbox = EPIC_widgets.ListboxEditable(listbox_frame,
        ...
        for i, tab in enumerate(self.model_results_tabs_states.keys()):
            ...
            self.model_results_checkbuttons_dict[tab].grid(row=i, column=0, columnspan=(1 if tab == 'Distances' else 4),
              sticky=(tk.NW + tk.SW))
        ...
        for menubtn, text, menutitle, options, current, command, imgfile in [
         ...
         (
          ...
          lambda : self.set_font_size('fontsize_choice', 'mcmc_fontsize_choice',
            updating_from_other=False), 'fontsize.png')]:
            img_path = os.path.join(EPIC.root, 'gui_files', 'images', imgfile) if imgfile else None
            setattr(self, menubtn, EPIC_widgets.MenuButtonOptions(textcfg,
              text=text,
              menutitle=menutitle,
              options=options,
              current=current,
        ...
        self.model_options_combobox = EPIC_widgets.ReadonlyCombobox(model_setup_LF, width=(round(210 / EPIC_widgets.pixels_to_char_size)),
        ...
        self.model_options_combobox.bind('<<ComboboxSelected>>', func=(self.show_optional_species),
          add='+')
        next_row = next(iter_rows)
        (ttk.Label(model_setup_LF, text='Mandatory species:').grid)(row=next_row, column=0, 
         sticky=tk.E, **self.default_padding)
        self.mandatory_msg = ttk.Label(model_setup_LF, width=(round(230 / EPIC_widgets.pixels_to_char_size)),
          text=(self.please_select_model + '\n'),
          padding=(self.PADX))
        (self.mandatory_msg.grid)(row=next_row, column=1, sticky=tk.W, **self.default_ypadding)
        next_row = next(iter_rows)
        self.optional_species_listbox = tk.Listbox(model_setup_LF, selectmode=(tk.MULTIPLE),
          height=2,
          exportselection=0)
        (ttk.Label(model_setup_LF, text='Add optional species:').grid)(row=next_row, 
        ...
        self.combined_option = ttk.Checkbutton(cb_frame, variable=(self.combined_var),
          state=(tk.DISABLED),
        ...
        next_row = next(iter_rows)
        self.derived_lbl = ttk.Label(model_setup_LF, justify=(tk.RIGHT), state=(tk.DISABLED),
        ...
        self.derived_combobox = EPIC_widgets.ReadonlyCombobox(model_setup_LF, state=(tk.DISABLED),
          values=[self.please_select_species],
          width=8)
        ...
        self.interaction_propto_options = EPIC_widgets.ReadonlyCombobox((self.int_frame),
          width=8, values=[self.please_select_species])
        ...
        self.int_sign_combobox = EPIC_widgets.ReadonlyCombobox((self.int_frame), width=5,
          values=['(sign)', '+1', '-1'])
        ...
        (ttk.Button((self.darksector_and_build_frame), text='Build\nnew\nmodel', width=5,
          command=(self.build_model)).pack)(side=tk.RIGHT, expand=1, anchor=tk.S, **self.default_ipadding, **self.default_padding)
    ...
    def cmb_lcdm_toggle(self):
        if self.data_CMB_LCDM.get():
            self.data_CMB_wCDM.set(0)

    def cmb_wcdm_toggle(self):
        if self.data_CMB_wCDM.get():
            self.data_CMB_LCDM.set(0)
    ...
    def bao_sdss_boss_toggle(self):
        use = any([check.get() for k, check in self.BAO_dict.items() if k not in ('WiggleZ',
                                                                                  '6dF+SDSS_MGS')])
        self.BAO_dict_check['WiggleZ'].config(state=(self.has_baryons_and_radiation() or tk.DISABLED if not use else tk.NORMAL))
        self.read_bao_states()
    ...
    def bao_wigglez_toggle(self):
        ...
        for k, check in self.BAO_dict_check.items():
            if k not in ('WiggleZ', '6dF+SDSS_MGS'):
                check.config(state=(self.has_baryons_and_radiation() or tk.DISABLED if not use else tk.NORMAL))
    ...
    def add_prior_config(self, baseframe, i, par):
        par.prior_setup = EPIC_widgets.PriorLineSetup(baseframe, (self.map_parameters[par.label]),
          (par.label), row=i, background=(self.frame_bg),
          xpadding=(self.default_xpadding))
        if par.label == 'T_CMB':
            par.prior_setup.prior_dist.current(0)
        par.prior_setup.prior_dist.bind('<<ComboboxSelected>>', func=(par.set_default_priors),
          add='+')
    ...
    def choose_proposal_cov_file(self):
        filename = filedialog.askopenfilename(initialdir=(EPIC.user_folder),
    ...
    def export_ini_only(self, run=False):
        if not hasattr(self, 'cosmo_model'):
            ...
            self.inifile = filedialog.asksaveasfilename(title='Export ini file...', defaultextension='.ini',
              filetypes=[
             ('INI file', '*.ini')],
            ...
            if self.prepare_sim_ini():
                run or self.status_bar.config(text=('File %s exported. You can start a MCMC simulation following instructions given in the header of this file.' % self.inifile))
    ...
    def export_ini_and_run_MCMC(self):
        if not self.inifile:
        ...
        elif not getattr(self, 'exported', None):
            return
            ...
            if self.sim.start(args, external_fig=(self.MCMCplot_tab), external_texts=(self.MCMC_simulation_info),
              plot_args=plot_args):
                rargs_list = [
                ...
                self.analyze((self.sim), rargs, external_texts=(self.MCMC_simulation_info))
    ...
    def attempt_prepare_sim_ini(self):
        with open(self.tempfile_name, 'w') as (f):
            ...
            for key in ('optional species', 'combined species', 'interaction setup'):
            ...
            if self.data_CMB_LCDM.get() and tk.DISABLED not in self.data_CMB_LCDM_check.state():
                datasets_dict['CMB'] = 'Planck2015_distances_LCDM'
                datasets_label.append('CMB')
            else:
                if self.data_CMB_wCDM.get():
                    if tk.DISABLED not in self.data_CMB_wCDM_check.state():
                        datasets_dict['CMB'] = 'Planck2015_distances_wCDM'
                        datasets_label.append('CMB')
                f.write('[analysis]\n')
    ...
    def build_model(self):
        ...
        if 'cde' in modelname:
            if not self.interaction_propto_options.get() == self.please_select_species:
                if self.int_sign_combobox.get() == '(sign)':
            ...
            self.cosmo_model = cosmo.CosmologicalSetup(modelname,
            ...
            for i, par in enumerate(self.free_parameters):
                ...
                (EPIC_widgets.MyTextLabel((self.parameter_values), (self.map_parameters[par.label]),
                  background=(self.frame_bg)).grid)(row=i % sep, 
                 column=col, sticky=tk.E, **self.default_xpadding)
                ...
                if par.label in default_units['DEFAULT']:
                    ...
                    for frame in [par.entry_frame, None]:
                        par.prior_setup.add_unit(unit_label, orig_unit_lbl, frame=frame,
                          xpadding=(self.default_xpadding))
    ...
    def get_cosmo_solution(self):
        try:
            ...
            if self.show_tab('Density parameters'):
                self.cosmo_model.results['Density parameters'] = {'x':self.cosmo_model.a_range,  'ydict':self.cosmo_model.background_solution_Omegas, 
                 'title':title, 
            ...
            if self.show_tab('Energy densities'):
                self.cosmo_model.results['Energy densities'] = {'x':self.cosmo_model.a_range,  'ydict':self.cosmo_model.background_solution_rhos, 
                 'exclude':[
                  'total'], 
            ...
            if self.show_tab('Distances'):
                self.cosmo_model.model_solution_plots_shown.extend([self.model_solution_tabs_dict[w].winfo_name() for w in ('Distances',
                                                                                                                            'Lookback time')])
                ...
                for function, key in (('d_a', 'Comoving angular distance'), ('D_H', 'Hubble distance'),
                                      ('get_lookback_time', 'Lookback time'), ('get_age_of_various_z', 'Age of the universe')):
                    kw = {'parameter_space': parameters}
                    if function == 'get_age_of_various_z':
                        if not self.cosmo_model.directly_solvable:
                            kw.update({'a_zinf': a_of_z(z_range[(-1)])})
                        if EPIC._OS == 'Windows':
                            (self.get_function_mp)(function, key, distances_mdict, 
                             z_range, **kw)
                    else:
                        jobs.append(multiprocessing.Process(target=(self.get_function_mp),
                          args=(
                         function, key, distances_mdict, z_range),
                          kwargs=kw))
            ...
            if self.show_tab('Equation of state'):
                ...
                for key, species in self.variable_eos.items():
                    jobs.append(multiprocessing.Process(target=eos_get_function_mp,
                      args=(
    ...
    def show_optional_species(self, event):
        ...
        if model == 'lcdm':
            ...
            if model in ('wcdm', 'cpl', 'ba', 'fv1', 'fv2', 'fv3'):
    ...
    def show_densities(self):
        if not hasattr(self, 'cosmo_model'):
            ...
            return hasattr(self.cosmo_model, 'results') or None
        self.status_bar.config(text='Plotting...')
        self.status_bar.update()
...
def launch_gui(args, run_parser, analyze_parser, plot_parser, analyze_function):
    plt.rcdefaults()
    master = tk.Tk()
    app = Application(run_parser, analyze_parser, plot_parser, analyze_function,
      master=master, theme=(args.theme))
