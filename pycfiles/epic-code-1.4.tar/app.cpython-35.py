...
# [GCC 8.4.0]
# Embedded file name: /mnt/c/Users/rafae/repo/epic/EPIC/gui/app.py
# Compiled at: 2018-07-03 15:42:15
# Size of source mod 2**32: 58762 bytes
...
from EPIC.utils.io_tools import parse_unit, split_last, tex_packages, available_fonts
...
class Application(object):
    ...
    distances_to_compare = [
     ...
     'Lookback time $c\\,t(z)$',
     'Luminosity $d_L(z)$']
    styles_list = [style.replace('_', ' ') for style in pyplot_available_styles]
    tex_options = [
     'Do not use TeX']
    texprefix = 'TeX with'
    ...
    def __init__(self, master=None, theme='clam'):
        ...
        icon = tk.Image('photo', file=os.path.join(root, 'gui_files', 'iconepic.gif'))
    ...
    def set_tex_option(self):
        ...
        if self.texprefix in option:
            ...
            font_preamble = tex_packages(available_fonts[option[len(self.texprefix) + 1:]])
    ...
    def create_widgets(self):
        ...
        self.available_models.read([
         ...
         os.path.join(root, 'modifications', 'cosmology', 'model_recipes.ini')])
        ...
        self.available_species.read([
         ...
         os.path.join(root, 'modifications', 'cosmology', 'available_species.ini')])
        ...
        parameters_setup_notebook = ttk.Notebook(self.master, height=self.BOTTOM_RIGHT_FRAME_HEIGHT - self.PADY)
        parameters_setup_notebook.pack(side=tk.BOTTOM, fill=tk.X, **(self.default_iypadding), **(self.default_padding))
        parameter_space_frame = ttk.Frame(parameters_setup_notebook)
        parameters_setup_notebook.add(parameter_space_frame, text='Specify parameters')
 ...
 False),
         ...
         ('Equation of state', '$a$', '$w(a)$', 'log', 'linear', False, False)]:
            self.model_solution_tabs_dict[plot] = EPIC_widgets.FigureFrame(self.plots_notebook, plot_name=plot, xlabel=xlabel, ylabel=ylabel, yscale=yscale, xscale_toggle=xtoggle, yscale_toggle=ytoggle, difference_toggle=False, bg=self.frame_bg)
            self.plots_notebook.add(self.model_solution_tabs_dict[plot], text=plot)

        self.distance_comparison_tabs_dict = OrderedDict((name, EPIC_widgets.FigureFrame(self.plots_notebook, plot_name=name, xlabel='$z$', ylabel=split_last(name)[1].rstrip('$') + '\\, [\\rm{Mpc}]$', yscale='log', difference_toggle=True, xscale_toggle=True, yscale_toggle=True, bg=self.frame_bg)) for name in self.distances_to_compare)
        for name in self.distances_to_compare:
            self.plots_notebook.add(self.distance_comparison_tabs_dict[name], text=split_last(name)[0])
        ...
        for i, tab in enumerate(self.model_results_tabs_states.keys()):
            ...
            self.model_results_checkbuttons_dict[tab].grid(row=i, column=0, columnspan=1 if i == 2 else 4, sticky=tk.NW + tk.SW)
        ...
        plot_style_btn = ttk.Menubutton(textcfg, direction='above', width=0, padding=3, text=self.please_select_style)
        ...
        for i, option in enumerate(self.styles_list):
            stylemenu.add_radiobutton(label=option, value=i, variable=self.style_choice, command=self.set_chosen_plot_style)

        plot_style_btn.config(menu=stylemenu)
        self.tex_choice = tk.IntVar()
        bw = 28
        ext = 'png' if tk.TkVersion >= 8.6 else 'gif'
        im = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'images', 'menu-tex.' + ext))
        tex_menubtn = ttk.Menubutton(textcfg, direction='above', width=0, padding=3, image=im)
        tex_menubtn.image = im
        texmenu = tk.Menu(tex_menubtn, title='TeX options')
        self.tex_choice.set(0)
        for i, option in enumerate(self.tex_options):
            texmenu.add_radiobutton(label=option, value=i, variable=self.tex_choice, command=self.set_tex_option)

        tex_menubtn.config(menu=texmenu)
        self.fontsize_choice = tk.IntVar()
        im = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'images', 'fontsize.' + ext))
        fontsize_btn = ttk.Menubutton(textcfg, direction='above', width=0, padding=3, image=im)
        fontsize_btn.image = im
        fontsize_menu = tk.Menu(fontsize_btn, title='Font size')
        self.fontsize_choice.set(0)
        for i in range(10, 20, 2):
            fontsize_menu.add_radiobutton(label=str(i), value=i, variable=self.fontsize_choice, command=self.set_font_size)
        ...
        self.BUTTONS = [
         plot_style_btn, tex_menubtn, fontsize_btn]
        for i in range(2):
        ...
        next_row = next(iter_rows)
        self.optional_species_listbox = tk.Listbox(model_setup_LF, selectmode=tk.MULTIPLE, height=2)
    ...
    def build_model(self):
        ...
        self.cosmo_model = cosmo.CosmologicalSetup(modelname, optional_species=optional, combined_species=combined, interaction_setup=int_setup, physical=bool(self.physical_var.get()), derived=self.map_species_abbrv[self.derived_combobox.get()], a0=1)
        ...
        default_units.read([
         ...
         os.path.join(root, 'modifications', 'cosmology', 'default_parameter_units.ini')])
        ...
        for i, par in enumerate(self.free_parameters):
            ...
            if par.label in default_units['DEFAULT']:
                ...
                EPIC_widgets.MyTextLabel(par.entry_frame, unit_label, background=self.frame_bg, justify=tk.LEFT, width=round(2 + 1.2 * len(org_unit_lbl.replace(' ', '').replace('^', '')))).pack(side=tk.LEFT, anchor=tk.W, **self.default_xpadding)
            ...
            for function, key in [
             ...
             (
              self.cosmo_model.get_age_of_various_z, 'Age of the universe')]:
                jobs.append(multiprocessing.Process(target=get_function_mp, args=(
                 function, key, distances_mdict, z_range), kwargs={'parameter_space': parameters}))
    ...
    def set_and_update_gif(self):
        ...
        if species == 'idm':
            if model == 'cde':
                img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'cde_propto_c_%s.gif' % sign))
            else:
                assert model == 'cde lambda'
                img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'ilambda_propto_c_%s.gif' % sign))
        else:
            if species == 'ilambda':
                img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'ilambda_propto_de_%s.gif' % sign))
            else:
                assert species == 'ide'
                img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'cde_propto_de_%s.gif' % sign))
        self.img_lbl.config(image=img)
        self.img_lbl.image = img
    ...
    def show_optional_species(self, event):
        ...
        img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'blank.gif'))
        if model == 'lcdm':
            img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'lcdm.gif'))
        elif model in ('wcdm', 'cpl', 'ba', 'fv1', 'fv2', 'fv3'):
            img = tk.PhotoImage(file=os.path.join(root, 'gui_files', 'wde.gif'))
        self.img_lbl.config(image=img)
        self.img_lbl.image = img
...
def config_all_widgets(w, **kwargs):
    for widget in w.winfo_children():
        try:
            ...
            if isinstance(widget, EPIC_widgets.ReadonlyCombobox) and kwargs.get('state') == tk.NORMAL:
