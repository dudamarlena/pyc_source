...
# [GCC 8.4.0]
# Embedded file name: pysqueezecenter/server.py
...
class Server(object):
    ...
    def __init__(self, hostname='localhost', port=9090, username='', password='', charset='utf8'):
        """
        Constructor
        """
        self.debug = False
    ...
    def request(self, command_string, preserve_encoding=False):
        ...
        Request
        """
        self.telnet.write(self._Server__encode(command_string + '\n'))
        response = self._Server__decode(self.telnet.read_until(self._Server__encode('\n'))[:-1])
        if not preserve_encoding:
            response = self._Server__unquote(response)
        else:
            command_string_quoted = command_string[0:command_string.find(':')] + command_string[command_string.find(':'):].replace(':', self._Server__quote(':'))
        start = command_string.split(' ')[0]
        if start in ('songinfo', 'trackstat', 'albums', 'songs', 'artists', 'rescan',
                     'rescanprogress'):
            if not preserve_encoding:
    ...
    def request_with_results(self, command_string, preserve_encoding=False):
        ...
        """
        quotedColon = self._Server__quote(':')
        try:
            ...
            for result in results:
                ...
                if len(result) > 0:
                    ...
                    for subResult in subResults:
                        ...
                        if not preserve_encoding:
                            item[urllib.unquote(key)] = self._Server__unquote(value)
                        else:
                            item[key] = value
