...
class Player(object):
    ...
    def update(self, index, update=True):
        ...
        self.index = index
        self.ref = str(self.__unquote(self.server.request('player id %i ?' % index)))
        self.name = str(self.__unquote(self.server.request('player name %i ?' % index)))
        if update:
            self.uuid = str(self.__unquote(self.server.request('player uuid %i ?' % index)))
            self.ip_address = str(self.__unquote(self.server.request('player ip %i ?' % index)))
            self.model = str(self.__unquote(self.server.request('player model %i ?' % index)))
            self.display_type = str(self.__unquote(self.server.request('player displaytype %i ?' % index)))
            self.can_power_off = bool(self.__unquote(self.server.request('player canpoweroff %i ?' % index)))
            self.is_player = bool(self.__unquote(self.server.request('player isplayer %i ?' % index)))
            self.is_connected = bool(self.__unquote(self.server.request('player connected %i ?' % index)))
