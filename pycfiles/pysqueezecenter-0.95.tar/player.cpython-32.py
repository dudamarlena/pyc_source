...
# [GCC 8.4.0]
# Embedded file name: pysqueezecenter/player.py
# Compiled at: 2011-08-14 07:55:55
"""
...
class Player(object):
    ...
    def update(self, index, update=True):
        ...
        self.index = index
        self.ref = str(self._Player__unquote(self.server.request('player id %i ?' % index)))
        self.name = str(self._Player__unquote(self.server.request('player name %i ?' % index)))
        if update:
            self.uuid = str(self._Player__unquote(self.server.request('player uuid %i ?' % index)))
            self.ip_address = str(self._Player__unquote(self.server.request('player ip %i ?' % index)))
            self.model = str(self._Player__unquote(self.server.request('player model %i ?' % index)))
            self.display_type = str(self._Player__unquote(self.server.request('player displaytype %i ?' % index)))
            self.can_power_off = bool(self._Player__unquote(self.server.request('player canpoweroff %i ?' % index)))
            self.is_player = bool(self._Player__unquote(self.server.request('player isplayer %i ?' % index)))
            self.is_connected = bool(self._Player__unquote(self.server.request('player connected %i ?' % index)))
    ...
    def has_permission(self, request_terms):
        ...
        request_terms = self._Player__quote(request_terms)
    ...
    def set_pref_value(self, name, value, namespace=None):
        ...
        if namespace:
            pref_string += namespace + ':'
        pref_string += name
        value = self._Player__quote(value)
    ...
    def playlist_get_info(self):
        ...
        for encoded in encoded_list:
            data = [self._Player__unquote(x) for x in ('position' + encoded).split(' ')]
            item = {}
            for info in data:
                info = info.split(':')
                key = info.pop(0)
    ...
    def show(self, line1='', line2='', duration=3, brightness=4, font='standard', centered=False):
        ...
        if font == 'huge':
            line1 = ''
        line1, line2 = self._Player__quote(line1), self._Player__quote(line2)
