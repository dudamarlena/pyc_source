...
# Embedded file name: /home/travis/build/PoorHttp/PoorWSGI/examples/simple.py
# Compiled at: 2020-02-05 15:58:26
# Size of source mod 2**32: 18159 bytes
...
python_path.insert(0, os.path.abspath(os.path.join(EXAMPLES_PATH, os.path.pardir)))
from poorwsgi import Application, state, request, redirect
from poorwsgi.session import PoorSession
from poorwsgi.response import Response, RedirectResponse, FileResponse, JSONResponse, EmptyResponse, HTTPException
    ...
    if req.method_number == state.METHOD_POST:
        ...
        try:
            req.form = request.FieldStorage(req,
              keep_blank_values=(app.keep_blank_values), strict_parsing=(app.strict_parsing),
...
def get_footer():
    return ('<hr>', '<small>Copyright (c) 2013 Ondřej Tůma. See ', '<a href="http://poorhttp.zeropage.cz">poorhttp.zeropage.cz</a></small>.',
            '</body>', '</html>')


def get_variables(req):
    usable = ('REQUEST_METHOD', 'QUERY_STRING', 'SERVER_NAME', 'SERVER_PORT', 'REMOTE_ADDR',
              'REMOTE_HOST', 'PATH_INFO')
    return sorted(tuple((key, html(val)) for key, val in req.environ.items() if key.startswith('wsgi.') or key.startswith('poor_') or key in usable))


app.set_filter('email', '[\\w\\.\\-]+@[\\w\\.\\-]+')
...
@app.route('/')
def root(req):
    buff = get_header('Index') + (get_crumbnav(req), '<ul>', '<li><a href="%s">/</a> - This Page</li>' % req.construct_url('/'), '<li><a href="/test/static">/test/static</a> - Testing Static Page</li>', '<li><a href="/test/42">/test/&lt;variable:int&gt;</a> - Testing regular:int Page</li>', '<li><a href="/test/3.14">/test/&lt;variable:float&gt;</a> - Testing regular:float Page</li>', '<li><a href="/test/word">/test/&lt;variable:word&gt;</a> - Testing regular:word Page</li>', '<li><a href="/test/user@example.net">/test/&lt;variable:user&gt;</a> - Testing regular:user Page</li>', '<li><a href="/test/[grr]">/test/&lt;variable:re:.*&gt;</a> - Testing regular:re Page</li>', '<li><a href="/test/one/too/three">/test/&lt;variable0&gt;/&lt;variable1&gt;/&lt;variable2&gt;</a> - Testing variable args</li>', '<li><a href="/test/headers">/test/headers</a> - Testing Headers</li>', '<li><a href="/login">/login</a> - Create login session</li>', '<li><a href="/logout">/logout</a> - Destroy login session</li>', '<li><a href="/test/form">/test/form</a> - Testing http form (only if you have login cookie / session)</li>', '<li><a href="/test/upload">/test/upload</a> - Testing file upload (only if you have login cookie / session)</li>', '<li><a href="/debug-info">/debug-info</a> - Debug Page (only if poor_Debug is set)</li>', '<li><a href="/no-page">/no-page</a> - No Exist Page</li>', '<li><a href="/internal-server-error">/internal-server-error</a> - Inernal Server Error</li>', '</ul>') + get_footer()
    response = Response()
    for line in buff:
        response.write(line + '\n')
...
def favicon(req):
    ...
    return (
     decodebytes(icon), 'image/vnd.microsoft.icon')
...
def test_dynamic(req, variable=None):
    var_info = {'type':type(variable),  'value':variable, 
     'uri_rule':req.uri_rule}
    title = 'Variable' if variable is not None else 'Static'
    buff = get_header(title + ' test') + (get_crumbnav(req), '<h2>Variable</h2>', '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, html(val)) for key, val in var_info.items()) + ('</table>',
                                                                                                                                                                                                '<h2>Browser Headers</h2>',
                                                                                                                                                                                                '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, val) for key, val in req.headers.items()) + ('</table>',
                                                                                                                                                                                                                                                                                                           '<h2>Request Variables </h2>',
                                                                                                                                                                                                                                                                                                           '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, val) for key, val in get_variables(req)) + ('</table>', ) + get_footer()
    response = Response()
    for line in buff:
        response.write(line + '\n')
...
def test_varargs(req, *args):
    ...
    buff = get_header('Variable args test') + (get_crumbnav(req), '<h2>Variables</h2>', '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, html(val)) for key, val in var_info.items()) + ('</table>',
                                                                                                                                                                                                      '<h2>Browser Headers</h2>',
                                                                                                                                                                                                      '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, val) for key, val in req.headers.items()) + ('</table>',
                                                                                                                                                                                                                                                                                                                 '<h2>Request Variables </h2>',
                                                                                                                                                                                                                                                                                                                 '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, val) for key, val in get_variables(req)) + ('</table>', ) + get_footer()
...
def test_form(req):
    var_info = OrderedDict((
     ...
     (
      'form_values',
      ', '.join(tuple(str(req.form.getvalue(key)) for key in req.form.keys()))),
     (
      'form_getfirst',
      '%s,%s' % (req.form.getfirst('pname'),
       req.form.getfirst('px'))),
     (
      'form_getlist',
      '%s,%s' % (list(req.form.getlist('pname')),
...
def test_upload(req):
    var_info = OrderedDict((
     ...
     (
      'form_value_fnames',
      ', '.join(tuple(str(req.form[key].filename) for key in req.form.keys()))),
     (
      'form_value_lenghts',
      ', '.join(tuple(str(req.form[key].length) for key in req.form.keys()))),
     (
      'form_value_files',
      ', '.join(tuple(str(req.form[key].file) for key in req.form.keys()))),
     (
      'form_value_lists',
      ', '.join(tuple(('Yes' if req.form[key].list else 'No') for key in req.form.keys())))))
    files = []
    ...
    buff = get_header('HTTP file upload test') + (get_crumbnav(req), '<h2>Upload Form</h2>', '<form method="post" enctype="multipart/form-data">', '<input type="file" name="file_0"><br/>', '<input type="file" name="file_1"><br/>', '<input type="file" name="file_2"><br/>', '<input type="submit" name="btn" value="Upload"></form>', '<h2>Uploaded File</h2>', '<table>') + tuple('<tr><td>%s:</td><td>%s</td></tr>' % (key, html(val)) for key, val in var_info.items()) + ('</table>', ) + tuple(files) + get_footer()
    response = Response()
    for line in buff:
        response.write(line + '\n')
...
def not_found(req):
    buff = (
     ...
     get_crumbnav(req),
     '<p>Your reqeuest <code>%s</code> was not found.</p>' % req.uri) + get_footer()
...
def test_headers(req):
    return (
     dumps({'Content-Type':(
      ...
      'Accept-MimeType':{'html':req.accept_html, 
...
@app.route('/test/json')
def test_json(req):
    return JSONResponse(status_code=418, message="I'm teapot :-)", numbers=(list(range(5))))
