...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/FeatureServer/DataSource/VersionedPostGIS.py
...
class VersionedPostGIS(PostGIS):
    ...
    def create(self, action):
        ...
        values = {'geom': self.to_wkt(feature.geometry), 'uuid': uuid.uuid1().hex, 'attrs': self._serializeattrs(feature.properties)}
        sql = 'INSERT INTO %s (%s, uuid, attrs)\n                    VALUES (SetSRID(%%(geom)s::geometry, %s),\n                                %%(uuid)s, %%(attrs)s)' % (self.table, self.geom_col, self.srid)
        cursor = self.db.cursor()
    ...
    def update(self, action):
        feature = action.feature
        sql = 'UPDATE %s SET %s = SetSRID(%%(geom)s::geometry, %s),\n                               attrs = %%(attrs)s WHERE %s = %(id)d' % (self.table, self.geom_col, self.srid, self.fid_col)
        values = {'geom': self.to_wkt(feature.geometry), 'id': action.id, 'attrs': self._serializeattrs(feature.properties)}
        cursor = self.db.cursor()
        cursor.execute(str(sql), values)
    ...
    def select(self, action):
        ...
        if action.id is not None:
            sql = 'SELECT AsText(%s) as fs_binary_geom_col, * FROM %s WHERE %s = %%(%s)d' % (self.geom_col, self.table, self.fid_col, self.fid_col)
        ...
        for row in result:
            ...
            if props.has_key(self.geom_col):
