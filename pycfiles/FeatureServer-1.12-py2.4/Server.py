...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/FeatureServer/Server.py
# Compiled at: 2008-03-18 11:03:27
__author__ = 'MetaCarta'
__copyright__ = 'Copyright (c) 2006-2008 MetaCarta'
__license__ = 'Clear BSD'
__version__ = '$Id: Server.py 443 2008-03-18 15:03:27Z crschmidt $'
...
if sys.platform == 'win32':
    ...
    cfgfiles = (
     'featureserver.cfg', os.path.join('..', 'featureserver.cfg'), '/etc/featureserver.cfg')
...
class Server(object):
    ...
    def dispatchRequest(self, params, path_info, host, post_data=None, request_method='GET', requested_content_types=''):
        ...
        request = None
        content_types = {'application/vnd.google-earth.kml+xml': 'KML', 'application/json': 'JSON', 'text/javascript': 'JSON', 'application/rss+xml': 'GeoRSS', 'text/html': 'HTML', 'osm': 'OSM', 'gml': 'WFS', 'wfs': 'WFS', 'kml': 'KML', 'json': 'JSON', 'georss': 'GeoRSS', 'atom': 'GeoRSS', 'html': 'HTML', 'geojson': 'GeoJSON'}
...
def modPythonHandler(apacheReq, service):
    ...
    try:
        ...
        (format, content) = service.dispatchRequest(params, apacheReq.path_info, host, post_data, request_method)
        ...
        apacheReq.write(error)
        if service and service.metadata.has_key('error_log'):
            f = open(service.metadata['error_log'], 'a')
            f.write(error)
            f.close()
        else:
    ...
    try:
        ...
        try:
            for key in input.keys():
                params[key.lower()] = input[key].value
...
def handler(apacheReq):
    ...
    if options.has_key('FeatureServerConfig'):
        cfgs = (
         options['FeatureServerConfig'],) + cfgs
    if not theServer:
