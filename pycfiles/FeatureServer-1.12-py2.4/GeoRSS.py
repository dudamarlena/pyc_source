...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/FeatureServer/Service/GeoRSS.py
# Compiled at: 2008-01-01 03:15:59
__author__ = 'MetaCarta'
__copyright__ = 'Copyright (c) 2006-2008 MetaCarta'
__license__ = 'Clear BSD'
__version__ = '$Id: GeoRSS.py 412 2008-01-01 08:15:59Z crschmidt $'
...
class GeoRSS(Request):
    ...
    def encode_metadata(self, action):
        ...
        return (
         'application/rss+xml', action.metadata)
    ...
    def coordinates_to_geom(self, coordinates, type):
        ...
        if type == 'Line':
            ...
            coords = [
             [
              [
               float(coords[1]), float(coords[0])], [float(coords[3]), float(coords[0])], [float(coords[3]), float(coords[2])], [float(coords[1]), float(coords[2])], [float(coords[1]), float(coords[0])]]]
            return {'type': 'Polygon', 'coordinates': coords}
