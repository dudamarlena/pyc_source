...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/FeatureServer/DataSource/OGR.py
# Compiled at: 2008-01-15 18:46:53
...
class OGR(DataSource):
    ...
    __module__ = __name__
    freeze_type = {ogr.wkbPoint: 'Point', ogr.wkbLineString: 'Line', ogr.wkbPolygon: 'Polygon', ogr.wkbMultiPoint: 'Point', ogr.wkbMultiLineString: 'Line', ogr.wkbMultiPolygon: 'Polygon'}
    thaw_type = {'Point': ogr.wkbPoint, 'Line': ogr.wkbLineString, 'Polygon': ogr.wkbPolygon}
    error_msgs = [
     'OK', 'Not enough data', 'Not enough memory', 'Unsupported geometry type', 'Unsupported operation', 'Corrupt data', 'Unknown failure', 'Unsupported SRS']

    def __init__(self, name, writable=0, lockfile=0, dsn=None, layer=None, **args):
    ...
    def update(self, action):
        ...
        if err:
            raise ogr.OGRError('Update error on FID %d: %s' % (action.id, self.error_msgs[err]))
        feature.Destroy()
        return self.select(action)
    ...
    def delete(self, action):
        ...
        if err:
            raise ogr.OGRError('Delete error on FID %d: %s' % (action.id, self.error_msgs[err]))
        return []
    ...
    def thaw_feature(self, feature):
        ...
        if geom['type'] not in self.thaw_type:
            raise ogr.OGRError('Geometry type %d not supported by FeatureServer' % geom['type'])
    ...
    def _freeze_geometry(self, geom):
        ...
        if geomtype in (ogr.wkbMultiPoint, ogr.wkbMultiLineString, ogr.wkbMultiPolygon):
            geom = geom.GetGeometryRef(0)
        if geomtype not in self.freeze_type:
            raise ogr.OGRError('Geometry type %d not supported by FeatureServer' % geomtype)
