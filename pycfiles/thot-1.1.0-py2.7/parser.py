...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thot/parser.py
# Compiled at: 2013-03-05 21:43:25
import logging, yaml, pytz, types
...
class Parser(object):
    ...
    def parse_headers(self):
        ...
        if not self.headers:
            ...
            if 'mtime' in self.headers:
                logging.warn('File "%s" overwrites property "mtime" - which leads to caching errors with proxies and browsers.', self.filename)
            for key in self.headers:
                value = self.headers[key]
                if value:
                    header_method = getattr(self, '_parse_%s_header' % key, None)
                    if header_method:
    ...
    def _parse_date_header(self, value):
        ...
        """
        assert type(value) not in types.StringTypes, 'Date header has been set in "%s" but cannot be parsed. Please use ISO-8601. Time with seconds.' % self.filename
        ...
        if 'timezone' in self.headers:
            if self.headers['timezone'] in pytz.all_timezones_set:
                ...
                logging.warn('Timezone "%s" specified in "%s" is unknown, "%s" will be used instead', self.headers['timezone'], self.filename, user_tz)
    ...
    if len(parser_map) <= 0:
        for entrypoint in pkg_resources.iter_entry_points('thot.renderer'):
            try:
                ...
                logging.debug('Parser "%s" has not been loaded due to: %s', entrypoint, e)
