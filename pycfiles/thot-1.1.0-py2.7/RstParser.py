...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thot/plugins/RstParser.py
...
class ThotHTMLTranslator(html4css1.HTMLTranslator):
    ...
    def _wrap_displaymath(self, math, label=None):
        ...
        for i, part in enumerate(parts):
            if label is not None and i == 0:
                ret.append('\\begin{split}%s\\end{split}' % part + (label and '\\label{' + label + '}' or ''))
            else:
    ...
    def _render_math(self, latex, alt_text=None):
        ...
        if png_path:
            ...
            os.remove(png_path)
            alt_text = self.encode(alt_text).strip() if alt_text else self.encode(latex).strip()
            return '<img class="math" src="%s" alt="%s" />' % (
             embedded_image, alt_text)
        else:
            logging.error('something went horribly wrong at rendering the formula: %s', latex)
            return
            return
...
if has_pygments:
    directives.register_directive('sourcecode', PygmentsDirective)
...
class RstParser(Parser):
    ...
    def _parse_text(self):
        self.text = publish_parts(source=self.text, settings_overrides={'doctitle_xform': False, 
           'initial_header_level': 2}, writer=ThotHTMLWriter())['fragment']