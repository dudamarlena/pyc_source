...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thot/app.py
...
import sys, time, pytz, pkg_resources
...
GZIP_ENDINGS = ['.css', '.js', '.xml', '.txt', '.sh', '.svg',
 '.xls', '.doc', '.xjs', '.psd', '.ppt',
...
def quickstart(settings):
    ...
    while timezone not in pytz.all_timezones_set:
        if timezone != 'tbd':
            print "Sorry, '%s' is unknown. Try again." % timezone
        timezone = raw_input("Your timezone, e.g. 'Europe/Berlin', 'US/Eastern', 'US/Pacific', \n" + "'UTC' or something other: ")

    language_default = 'de'
    language = raw_input('Default language tag (de, en_US, en_GB...) [%s]: ' % language_default) or language_default
    config = {'thot': {'website_url': website_url, 
                'timezone': timezone, 
...
def main():
    ...
    parser.add_option('--quickstart', help='quickstart a thot site', action='store_true', dest='quickstart')
    parser.add_option('--logging', help="sets the logging level. 'info' (default) or 'debug'")
    parser.add_option('--hardlinks', action='store_true', help='instead of copying static files, creates hardlinks' + ' - which is faster and saves space')
    parser.add_option('-z', '--gzip', action='store_true', help='make a gzip-compressed copy of rendered files')
    parser.add_option('-t', '--templating', default='mako', dest='templating_engine', help='templating engine (e.g. jinja2, mako) for output')
    parser.add_option('-s', '--source', default='filesystem', help="data source, e.g. 'filesystem' for files")
    ...
    settings = {'project_dir': project_dir, 'output_dir': join(project_dir, '_output'), 
    ...
    if 'timezone' not in settings:
        ...
        logging.warn('No timezone has been set. Assuming all dates are in "%s".', settings['timezone'])
    else:
        if settings['timezone'] not in pytz.all_timezones_set:
            logging.error('Timezone "%s" is unknown (not in pytz.all_timezones_set).' + ' Try "Europe/Berlin", "UTC" or "US/Pacific" for example.', settings['timezone'])
            sys.exit(1)
    ...
    source = source_cls(settings['project_dir'], settings['build_time'], settings['timezone'], settings['default_template'] if 'default_template' in settings else get_templating_cls(settings['templating_engine']).default_template, settings['page_defaults'] if 'page_defaults' in settings else dict())
