...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/thot/plugins/TracParser.py
...
class TracParser(Parser):
    ...
    def create_trac_ctx(self, website_url, author_name, timezone_str, uri):
        req = Mock(href=Href(uri), abs_href=Href(website_url), authname=author_name, perm=MockPerm(), tz=timezone_str, args={})
        ...
        env.config.set('intertrac', 'trac.url', website_url)
        ...
        env.config.set('wiki', 'safe_schemes', 'file,ftp,http,https,svn,svn+ssh,git,rfc-2396.compatible,rfc-2396+under_score')
        env.href = req.href
        env.abs_href = req.abs_href
        return (env, context)

    def _parse_text(self):
        env, context = self.create_trac_ctx(self.settings['website_url'], self.headers['author']['name'] if 'author' in self.headers and 'name' in self.headers['author'] else self.settings['author']['name'], str(self.headers['timezone']) if 'timezone' in self.headers else str(self.settings['timezone']), self.filename if self.filename.startswith('/') else '/' + self.filename)
