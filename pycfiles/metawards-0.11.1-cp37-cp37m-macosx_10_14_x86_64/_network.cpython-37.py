...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.169.1/work/MetaWards/MetaWards/tests/../build/lib.macosx-10.14-x86_64-3.7/metawards/_network.py
# Compiled at: 2020-05-10 15:00:16
# Size of source mod 2**32: 18549 bytes
...
class Network:
    ...
    @staticmethod
    def build(params: Parameters, calculate_distances: bool=True, build_function=None, distance_function=None, max_nodes: int=16384, max_links: int=4194304, nthreads: int=1, profiler=None):
        ...
        if profiler is None:
            ...
            network = build_function(params=params, profiler=p,
              max_nodes=max_nodes,
              max_links=max_links,
              nthreads=nthreads)
            p = p.stop()
            ...
            if calculate_distances:
                ...
                network.add_distances(distance_function=distance_function, nthreads=nthreads)
                p = p.stop()
    ...
    def get_min_max_distances(self, nthreads: int=1, profiler=None):
        ...
        self._min_max_distances = get_min_max_distances(network=self, nthreads=nthreads)
        return self._min_max_distances
    ...
    def update(self, params: Parameters, demographics=None, nthreads: int=1, profiler=None):
        ...
        if profiler is None:
            ...
            p = p.stop()
            if demographics:
                network = demographics.specialise(network=self, profiler=profiler,
                  nthreads=nthreads)
            else:
                network = self
        p = p.start('rescale_play_matrix')
    ...
    def move_from_play_to_work(self, nthreads: int=1, profiler=None):
        ...
        move_population_from_play_to_work(network=self, nthreads=nthreads, profiler=profiler)
    ...
    def specialise(self, demographic, profiler=None, nthreads: int=1):
        ...
        """
        return demographic.specialise(network=self, profiler=profiler,
          nthreads=nthreads)

    def scale_susceptibles(self, ratio: any=None, work_ratio: any=None, play_ratio: any=None):
        ...
        self.nodes.scale_susceptibles(work_ratio=work_ratio, play_ratio=play_ratio)

    def run(self, population: Population, output_dir: OutputFiles, seed: int=None, nsteps: int=None, nthreads: int=None, iterator=None, extractor=None, mixer=None, mover=None, profiler=None) -> Population:
