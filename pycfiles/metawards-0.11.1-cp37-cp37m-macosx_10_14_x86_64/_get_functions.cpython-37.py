...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.169.1/work/MetaWards/MetaWards/tests/../build/lib.macosx-10.14-x86_64-3.7/metawards/utils/_get_functions.py
# Compiled at: 2020-05-10 15:00:16
# Size of source mod 2**32: 12525 bytes
...
def get_functions(stage: str, network: _Union[(Network, Networks)], population: Population, infections: Infections, output_dir: OutputFiles, workspace: Workspace, iterator: MetaFunction, extractor: MetaFunction, mixer: MetaFunction, mover: MetaFunction, rngs, nthreads, profiler: Profiler, trajectory: Populations=None, results=None) -> _List[MetaFunction]:
    ...
    """
    stages = [
     'initialise', 'setup', 'foi', 'infect',
     'analyse', 'finalise', 'summary']
    if stage not in stages:
        raise ValueError(f"Cannot recognise the stage {stage}. Available stages are {stages}")
    else:
        kwargs = {'stage':stage, 
         'network':network, 
         'population':population, 
        ...
        if stage == 'summary':
            ...
            funcs = mover(**kwargs) + iterator(**kwargs) + mixer(**kwargs) + extractor(**kwargs)
    return funcs
...
def get_finalise_functions(trajectory: Populations, **kwargs) -> _List[MetaFunction]:
    """Convenience function that returns all of the functions
...
def get_summary_functions(network: _Union[(Network, Networks)], results, output_dir: OutputFiles, extractor: MetaFunction, nthreads: int=1, **kwargs):
    """Convenience function that returns all of the functions
    ...
    return get_functions(stage='summary', network=network, output_dir=output_dir, 
     results=results, extractor=extractor, 
     nthreads=nthreads, **kwargs)


def call_function_on_network(network: _Union[(Network, Networks)], infections: Infections, workspace: Workspace, population: Population, func: MetaFunction=None, parallel: MetaFunction=None, nthreads: int=1, switch_to_parallel: int=2, call_on_overall: bool=False, **kwargs):
    ...
    if isinstance(network, Networks):
        for i, subnet in enumerate(network.subnets):
            ...
            func(network=subnet, infections=subinf, population=subpop, 
             workspace=subwork, **kwargs)
            if call_on_overall:
                func(network=network.overall, infections=infections, population=population, 
                 workspace=workspace, **kwargs)

    else:
        func(network=network, infections=infections, population=population, 
         workspace=workspace, **kwargs)