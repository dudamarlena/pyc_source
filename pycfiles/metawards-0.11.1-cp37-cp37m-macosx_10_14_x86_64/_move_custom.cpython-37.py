...
# [GCC 8.4.0]
# Embedded file name: /Users/runner/runners/2.169.1/work/MetaWards/MetaWards/tests/../build/lib.macosx-10.14-x86_64-3.7/metawards/movers/_move_custom.py
# Compiled at: 2020-05-10 15:00:16
# Size of source mod 2**32: 6847 bytes
from typing import Union as _Union
from typing import List as _List
from utils._get_functions import MetaFunction, accepts_stage
__all__ = [
 'move_custom', 'build_custom_mover']

def build_custom_mover(custom_function: _Union[(str, MetaFunction)], parent_name='__main__') -> MetaFunction:
    ...
    if isinstance(custom_function, str):
        ...
        if module is None:
            print(f"Cannot find the mover '{custom_function}'.Please make sure this is spelled correctly and any python modules/files needed are in the PYTHONPATH or current directory")
            raise ImportError(f"Could not import the mover '{custom_function}'")
        if func_name is None:
            import inspect
            for name, value in inspect.getmembers(module):
                if name.startswith('move') and hasattr(value, '__call__'):
                    return build_custom_mover(getattr(module, name))

            print(f"Could not find any function in the module {custom_function} that has a name that starts with 'move'. Please manually specify the name using the '{custom_function}::your_function syntax")
            raise ImportError(f"Could not import the mover {custom_function}")
        else:
    ...
    if not hasattr(custom_function, '__call__'):
        print(f"Cannot build a mover for {custom_function} as it is missing a __call__ function, i.e. it is not a function.")
        raise ValueError(f"You can only build custom movers for actual functions... {custom_function}")
    print(f"Building a custom mover for {custom_function}")
    return lambda **kwargs: move_custom(custom_function=custom_function, **kwargs)


def move_custom(custom_function: MetaFunction, stage: str, **kwargs) -> _List[MetaFunction]:
