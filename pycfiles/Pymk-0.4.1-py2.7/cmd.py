...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymk/tests/extra/cmd.py
# Compiled at: 2013-12-18 01:24:56
from mock import patch, call, MagicMock
from contextlib import nested
from subprocess import PIPE
import signal, pymk.error as Perror
...
class RunCmdTest(PymkTestCase):
    ...
    def test_fail_with_show_enabled(self):
        self.assertRaises(Perror.CommandError, extra.run, ['ls *.py'], True)
...
class ProccessTest(PymkTestCase):
    ...
    def test_init(self):
        with nested(patch.object(extra.cmd.Process, 'run'), patch.object(extra.cmd.Process, 'wait_for_termination'), patch.object(extra.cmd.Process, 'append_proccess'), patch.object(extra.cmd.Process, 'end_proccess')) as (run, wait_for_termination, append_proccess, end_proccess):
    ...
    def test_run(self, Popen):
        ...
        self.proccess.run()
        Popen.assert_called_once_with('args', stdout=PIPE, stderr=PIPE, shell=True, env=None)
        return
    ...
    def test_raise_error_without_output(self, CommandError):
        ...
        CommandError.return_value = Exception()
        self.assertRaises(Exception, self.proccess.raise_error, 5)
        CommandError.assert_called_once_with(5, self.proccess.spp.stderr.read.return_value)
    ...
    def test_end_proccess(self, SignalHandling):
        ...
        SignalHandling.return_value.send_signal.assert_called_once_with(self.proccess.spp, signal.SIGTERM)
