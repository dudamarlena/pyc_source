...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pymk/tests/recipe.py
...
class RecipeTest(PymkTestCase):
    ...
    def test_set_path(self):
        ...
        self.assertEqual(os.path.join('/tmp', 'elf', 'samantha'), self.recipe.paths['path'])
    ...
    def test_assign_main_path(self):
        ...
        self.assertEqual(os.path.dirname(__file__), self.recipe.paths['main'])
    ...
    def test_validate_settings_equal(self, compare_version):
        ...
        with patch('pymk.recipe.VERSION') as (VERSION):
            ...
            compare_version.assert_called_once_with(VERSION, self.recipe.settings['minimal pymk version'])
        return
    ...
    def test_import_wrapper(self):
        with nested(patch('__builtin__.globals'), patch('__builtin__.locals'), patch('__builtin__.__import__')) as (_globals, _locals, _import):
            self.recipe.import_wrapper('name')
            _import.assert_called_once_with('name', _globals.return_value, _locals.return_value, [''])

    def test_add_recipe(self):
        task = MagicMock()
        with nested(patch.dict(RecipeType.recipes, {'pymkmodules.name': task}), patch.object(self.recipe, 'import_wrapper')) as (recipes, import_wrapper):
    ...
    def test_add_recipe_no_recipe(self):
        task = MagicMock()
        with nested(patch.dict(RecipeType.recipes, {}), patch.object(self.recipe, 'import_wrapper')) as (recipes, import_wrapper):
            self.recipe.add_recipe('name')
            import_wrapper.assert_called_once_with('pymkmodules.name')
...
class RecipeTypeTest(PymkTestCase):
    ...
    def test_init(self):
        with nested(patch.object(RecipeType, 'check_if_recipe_exists'), patch.object(RecipeType, 'assign_recipe_to_tasks')) as (check_if_recipe_exists, assign_recipe_to_tasks):
            ...
            self.assertTrue(RecipeType.getName.return_value in RecipeType.recipes)
    ...
    def test_assign_recipe_to_task_if_able_do_assign(self):
        task = TaskExample
        recipe = MagicMock()
        with nested(patch.object(RecipeType, 'is_not_a_base_class'), patch.object(task, 'assign_recipe')) as (is_not_a_base_class, assign_recipe):
    ...
    def test_run_exists(self, existsmock):
        ...
        with patch.object(self.downloader, 'create_pymkmodules_path') as (create_pymkmodules_path):
            with patch.object(self.downloader, 'download') as (download):
                ...
                existsmock.assert_called_once_with('%s/%s.egg' % (self.downloader.modules_path, self.downloader.name))
    ...
    def test_download(self, log):
        with patch('pymk.recipe.download') as (download):
            with patch('pymk.recipe.extract_egg') as (extract_egg):
                ...
                destination_egg_path = self.downloader.destination_path + '.zip'
                self.assertEqual(2, log.info.call_count)
                download.assert_called_once_with(self.downloader.url, destination_egg_path)
                extract_egg.assert_called_once_with(destination_egg_path, self.downloader.destination_path)