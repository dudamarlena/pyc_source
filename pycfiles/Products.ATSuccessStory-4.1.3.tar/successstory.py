...
# [GCC 8.4.0]
# Embedded file name: /opt/Plone-4.3.7-was-4.2.1/zeocluster/src/Products.ATSuccessStory/Products/ATSuccessStory/browser/portlets/successstory.py
...
class ISuccessStoryPortlet(IPortletDataProvider):
    ...
    same.
    """
    header = schema.TextLine(title=_('Header'), description=_('Portlet header'), required=True)
    searchpath = schema.Choice(title=_('Stories Path'), description=_('Search for success stories inside this path'), required=True, vocabulary='atss.existing_folders')
    number_of_stories = schema.Int(title=_('Number of stories'), description=_('Specify how many Success Stories you want displayed at the same time in the portlet. Most commonly you will need 1.'), required=True)
    keywords = schema.Tuple(title=_('Categories'), description=_('Items containing what categories to show. If none is selected, this field will be ignored.'), required=False, value_type=schema.Choice(vocabulary='plone.app.vocabularies.Keywords'))
...
class Assignment(base.Assignment):
    ...
    def __init__(self, header='Success Stories', searchpath='/', number_of_stories=1, keywords=None):
        self.header = header
...
class Renderer(base.Renderer):
    ...
    def search_stories(self):
        ...
        if folder_path:
            query = {'path': folder_path, 'portal_type': 'ATSuccessStory'}
            if self.keywords:
