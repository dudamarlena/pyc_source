...
# [GCC 8.4.0]
# Embedded file name: /Users/rmartins/Desenvolvimento/Django/Apps/wagtaildemo/smart_selects/views.py
...
from smart_selects.utils import get_keywords, sort_results, serialize_results, get_queryset, get_limit_choices_to
...
def is_m2m(model_class, field):
    ...
    if is_pre_19_syntax:
        if isinstance(getattr(model_class, field), ReverseManyRelatedObjectsDescriptor):
            return True
    elif isinstance(getattr(model_class, field), ManyToManyDescriptor) and not getattr(model_class, field).reverse:
        return True
    return False


def filterchain(request, app, model, field, foreign_key_app_name, foreign_key_model_name, foreign_key_field_name, value, manager=None):
...
def filterchain_all(request, app, model, field, foreign_key_app_name, foreign_key_model_name, foreign_key_field_name, value):
    ...
    serialized_results = serialize_results(filtered) + [empty_choice] + serialize_results(excluded)
    results_json = json.dumps(serialized_results)
    return HttpResponse(results_json, content_type='application/json')