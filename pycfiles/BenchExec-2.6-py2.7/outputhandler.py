...
RESULT_XML_PUBLIC_ID = b'+//IDN sosy-lab.org//DTD BenchExec result 2.3//EN'
RESULT_XML_SYSTEM_ID = b'https://www.sosy-lab.org/benchexec/result-2.3.dtd'
...
class OutputHandler(object):
    ...
    def store_system_info(self, opSystem, cpu_model, cpu_number_of_cores, cpu_max_frequency, memory, hostname, runSet=None, environment={}, cpu_turboboost=None):
        ...
        osElem = ElementTree.Element(b'os', {b'name': opSystem})
        cpuElem = ElementTree.Element(b'cpu', {b'model': cpu_model, 
        ...
        ramElem = ElementTree.Element(b'ram', {b'size': str(memory) + b'B'})
        systemInfo = ElementTree.Element(b'systeminfo', {b'hostname': hostname})
        systemInfo.append(osElem)
        systemInfo.append(cpuElem)
        systemInfo.append(ramElem)
        env = ElementTree.SubElement(systemInfo, b'environment')
        for var, value in sorted(environment.items()):
            ElementTree.SubElement(env, b'var', name=var).text = value
    ...
    def store_header_in_xml(self, version, memlimit, timelimit, corelimit):
        self.xml_header = ElementTree.Element(b'result', {b'benchmarkname': self.benchmark.name, 
        ...
        if self.benchmark.description:
            description_tag = ElementTree.Element(b'description')
            description_tag.text = self.benchmark.description
            self.xml_header.append(description_tag)
        columntitlesElem = ElementTree.Element(b'columns')
        columntitlesElem.append(ElementTree.Element(b'column', {b'title': b'status'}))
        columntitlesElem.append(ElementTree.Element(b'column', {b'title': b'cputime'}))
        columntitlesElem.append(ElementTree.Element(b'column', {b'title': b'walltime'}))
        for column in self.benchmark.columns:
            columnElem = ElementTree.Element(b'column', {b'title': column.title})
    ...
    def output_before_run_set(self, runSet, start_time=None):
        ...
        for run in runSet.runs:
            ...
            run.xml = ElementTree.Element(b'run', run_attributes)
    ...
    def add_column_to_xml(self, xml, title, value, prefix=b'', value_suffix=b''):
        if value is None:
            ...
            xml.append(ElementTree.Element(b'column', attributes))
            return
    ...
    def _write_rough_result_xml_to_file(self, xml, filename):
        ...
        with open(temp_filename, b'wb') as (file):
            ElementTree.ElementTree(xml).write(file, encoding=b'utf-8', xml_declaration=True)
    ...
    def _write_pretty_result_xml_to_file(self, xml, filename):
        ...
        with io.TextIOWrapper(open_func(actual_filename, b'wb'), encoding=b'utf-8') as (file):
            rough_string = ElementTree.tostring(xml, encoding=b'unicode')
