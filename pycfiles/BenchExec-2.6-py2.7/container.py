...
def determine_directory_mode(dir_modes, path, fstype=None):
    ...
    if fstype == b'proc':
        ...
        if result_mode == DIR_OVERLAY and not os.path.isdir(path):
            logging.debug(b'Cannot use overlay mode for %s because it is not a directory. Using read-only mode instead. ', path.decode())
            return DIR_READ_ONLY
...
def make_overlay_mount(mount, lower, upper, work):
    ...
    libc.mount(b'none', mount, b'overlay', 0, b'lowerdir=' + escape(lower) + b',upperdir=' + escape(upper) + b',workdir=' + escape(work))
