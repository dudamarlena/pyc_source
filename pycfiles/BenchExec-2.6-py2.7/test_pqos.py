...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchexec/test_pqos.py
# Compiled at: 2020-05-07 05:52:35
"""
    Unit tests for pqos module
"""
import json, copy, logging, unittest
from subprocess import CalledProcessError
from unittest.mock import patch, MagicMock
from benchexec.pqos import Pqos
mock_pqos_wrapper_output = {'load_pqos': {'function_output': {}, 'returncode': 0, 
                 'function': 'pqos_init', 
                 'error': False, 
                 'message': 'MSR interface intialised'}, 
   'check_capability': {'function_output': {'mem_size': 32, 'cdp_on': 0, 'num_classes': 4}, 'returncode': 0, 
                        'function': 'get_capability_info', 
                        'error': False, 
                        'message': 'Retrieved l3ca capability'}, 
   'allocate_resource': {'function_output': {'cache_per_run': 4, 'cores': {0: 0, 1: 0}}, 'returncode': 0, 
                         'function': 'allocate_resource', 
                         'error': False, 
                         'message': 'Allocated l3ca'}, 
   'monitor_events': {'function_output': {'monitoring_data': [
                                                            {'cores': [
                                                                       0, 1, 2], 
                                                               'ipc': 0.987, 
                                                               'llc_misses': 10240, 
                                                               'llc': {'avg': 25028, 'max': 30000}, 'mbm_local': {'avg': 25028, 'max': 30000}}]}, 
                      'returncode': 0, 
                      'function': 'monitor_events', 
                      'error': False, 
                      'message': 'Event monitoring successfull'}, 
   'reset_monitoring': {'returncode': 0, 
                        'function': 'reset_monitoring', 
                        'error': False, 
                        'message': 'Reset monitoring successfull'}, 
   'reset_resources': {'returncode': 0, 
                       'function': 'reset_resources', 
                       'error': False, 
                       'message': 'Resource reset successfull'}}
mock_pqos_wrapper_error = {'function': 'mock_function', 
...
class MockPopen:
    ...
    def communicate(self):
        ...
        if self.returncode == 0:
            ...
            return (
             None, json.dumps(mock_pqos_wrapper_error).encode())
...
class TestPqos(unittest.TestCase):
    ...
    def test_pqos_init(self, mock_find_executable):
        ...
        self.assertEqual(pqos.cli_exists, True)
    ...
    def test_pqos_init_error(self, mock_find_executable):
        ...
        pqos = Pqos()
        self.assertIsInstance(pqos, Pqos)
        self.assertEqual(pqos.cli_exists, False)

    @patch('benchexec.pqos.find_executable', return_value='/path/to/pqos_wrapper/lib')
    @patch('benchexec.pqos.check_output', side_effect=mock_check_output)
    @patch('benchexec.pqos.Popen', side_effect=mock_popen)
    def test_pqos_execute_command(self, mock_find_executable, mock_check_output, mock_popen):
        """
        ...
        self.assertEqual(ret, True)
        ret = pqos.execute_command('l3ca', 'allocate_resource', False, '-a', 'l3ca', '[[0,1],[2,3]]')
        self.assertEqual(ret, True)
        ret = pqos.execute_command('l3ca', 'reset_resources', True, '-r')
        self.assertEqual(ret, True)
        ret = pqos.execute_command('mon', 'monitor_events', False, '-m', '[[0,1],[2,3]]')
        self.assertEqual(ret, True)
    ...
    def test_pqos_execute_command_cli_error(self, mock_find_executable, mock_check_output):
        """
    ...
    @patch('benchexec.pqos.find_executable', return_value='/path/to/pqos_wrapper/lib')
    @patch('benchexec.pqos.check_output', side_effect=mock_check_output_capability_error)
    ...
    @patch('benchexec.pqos.check_output', side_effect=mock_check_output)
    @patch('benchexec.pqos.Popen', side_effect=mock_popen)
    def test_pqos_stop_monitoring(self, mock_find_executable, mock_check_output, mock_popen):
        """
            Test for pqos.stop_monitoring
        """
        flatten_mon_data = {'ipc': 0.987, 
           'llc_misses': 10240, 
           'llc_avg': 25028, 
           'llc_max': 30000, 
           'mbm_local_avg': 25028, 
    ...
    @patch('benchexec.pqos.check_output', side_effect=mock_check_output)
    @patch('benchexec.pqos.Popen', side_effect=mock_popen)
    def test_pqos_stop_monitoring_error(self, mock_find_executable, mock_check_output, mock_popen):
        """
    ...
    def test_pqos_flatten_mon_data(self):
        ...
        flatten_mon_data = {'ipc': 0.987, 
           ...
           'mbm_local_avg': 25028, 
           'mbm_local_max': 30000}
        mon_data = copy.deepcopy(mock_pqos_wrapper_output['monitor_events']['function_output']['monitoring_data'])
    ...
    def test_pqos_flatten_mon_data_multiple(self):
        ...
        """
        flatten_mon_data_multiple = {'ipc_cpus0,1,2': 0.987, 
