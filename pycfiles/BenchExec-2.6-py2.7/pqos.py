...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchexec/pqos.py
...
class Pqos(object):
    ...
    def __init__(self, show_warnings=False):
        ...
        self.executable_path = find_executable('pqos_wrapper', exitOnError=False, use_current_dir=False)
        if self.executable_path is not None:
            self.cli_exists = True
        elif self.show_warnings:
            logging.info('Unable to find pqos_wrapper, please install it for cache allocation and monitoring if your CPU supports Intel RDT (cf. https://gitlab.com/sosy-lab/software/pqos-wrapper).')
        return

    def execute_command(self, _Pqos__type, function, suppress_warning, *args):
        """
    ...
    def print_error_message(self, err, _Pqos__type, args_list):
        """
        ...
        """
        msg_prefix = {'mon': 'Could not monitor events', 
        ...
        try:
            ...
            logging.warning(('{0}...Unable to execute command {1}').format(msg_prefix[__type], (' ').join(args_list)))
    ...
    def check_capacity(self, technology):
        ...
        """
        return self.execute_command(technology, 'check_capability', False, '-c', technology)
    ...
    def allocate_l3ca(self, core_assignment):
        ...
        if self.check_capacity('l3ca'):
            ...
            if self.execute_command('l3ca', 'allocate_resource', False, '-a', 'l3ca', core_string):
    ...
    def stop_monitoring(self):
        ...
        if self.mon_process:
            ...
            if self.mon_process.returncode == 0:
                ...
                ret = self.flatten_mon_data(mon_data['monitor_events']['function_output']['monitoring_data'])
            elif self.show_warnings:
                self.print_error_message(mon_output[1].decode(), 'mon', self.mon_process.args)
        ...
        if cap['error'] is False:
            if self.CAP_SYS_RAWIO in cap['capabilities']:
                if not all(x in cap['set'] for x in ['e', 'p']):
                    logging.warning('Insufficient capabilities for pqos_wrapper, Please add e,p in cap_sys_rawio capability set of pqos_wrapper')
            else:
                logging.warning('Insufficient capabilities for pqos_wrapper, Please set capabilitiy cap_sys_rawio with e,p for pqos_wrapper')
        msr = check_msr()
        if msr['loaded']:
            current_user = grp.getgrgid(os.getegid()).gr_name
            if msr['read']:
                if not msr['write']:
                    logging.warning(('Add write permissions for msr module for {}').format(current_user))
            else:
                logging.warning(('Add read and write permissions for msr module for {}').format(current_user))
        else:
