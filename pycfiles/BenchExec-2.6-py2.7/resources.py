...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/benchexec/resources.py
...
import collections, itertools, logging, math, os, sys
from benchexec import cgroups
from benchexec import util
__all__ = [
 b'check_memory_size',
 b'get_cpu_cores_per_run',
 b'get_memory_banks_per_run',
 b'get_cpu_package_for_core']
...
def get_cpu_cores_per_run(coreLimit, num_of_threads, use_hyperthreading, my_cgroups, coreSet=None):
    ...
    """
    try:
        allCpus = util.parse_int_list(my_cgroups.get_value(cgroups.CPUSET, b'cpus'))
        if coreSet:
            invalid_cores = sorted(set(coreSet).difference(set(allCpus)))
            if len(invalid_cores) > 0:
                raise ValueError((b'The following provided CPU cores are not available: {}').format((b', ').join(map(str, invalid_cores))))
            allCpus = [ core for core in allCpus if core in coreSet ]
        logging.debug(b'List of available CPU cores is %s.', allCpus)
        cores_of_memory_region = collections.defaultdict(list)
        for core in allCpus:
            coreDir = (b'/sys/devices/system/cpu/cpu{0}/').format(core)
            memory_regions = _get_memory_banks_listed_in_dir(coreDir)
            if memory_regions:
                cores_of_memory_region[memory_regions[0]].append(core)
            else:
                logging.warning(b'Kernel does not have NUMA support. Use benchexec at your own risk.')
                cores_of_memory_region = {}
                break

        logging.debug(b'Memory regions of cores are %s.', cores_of_memory_region)
        cores_of_package = collections.defaultdict(list)
        for core in allCpus:
            package = get_cpu_package_for_core(core)
            cores_of_package[package].append(core)

        logging.debug(b'Physical packages of cores are %s.', cores_of_package)
        if len(cores_of_memory_region) >= len(cores_of_package):
            cores_of_unit = cores_of_memory_region
            logging.debug(b'Using memory regions as the basis for cpu core division')
        else:
            cores_of_unit = cores_of_package
            logging.debug(b'Using physical packages as the basis for cpu core division')
        siblings_of_core = {}
        for core in allCpus:
            siblings = util.parse_int_list(util.read_file((b'/sys/devices/system/cpu/cpu{0}/topology/thread_siblings_list').format(core)))
            siblings_of_core[core] = siblings

        logging.debug(b'Siblings of cores are %s.', siblings_of_core)
    except ValueError as e:
        sys.exit((b'Could not read CPU information from kernel: {0}').format(e))

    return _get_cpu_cores_per_run0(coreLimit, num_of_threads, use_hyperthreading, allCpus, cores_of_unit, siblings_of_core)


def _get_cpu_cores_per_run0(coreLimit, num_of_threads, use_hyperthreading, allCpus, cores_of_unit, siblings_of_core):
    ...
    if coreLimit > len(allCpus):
        sys.exit((b'Cannot run benchmarks with {0} CPU cores, only {1} CPU cores available.').format(coreLimit, len(allCpus)))
    if coreLimit * num_of_threads > len(allCpus):
        sys.exit((b'Cannot run {0} benchmarks in parallel with {1} CPU cores each, only {2} CPU cores available. Please reduce the number of threads to {3}.').format(num_of_threads, coreLimit, len(allCpus), len(allCpus) // coreLimit))
    if not use_hyperthreading:
        ...
        for core, siblings in siblings_of_core.items():
            if core in allCpus:
                ...
                cores_of_unit[unit_of_core[core]] = [ c for c in cores_of_unit[unit_of_core[core]] if c not in siblings ]
                siblings_of_core[core] = [
                 core]
        ...
        logging.debug((b'Running in no-hyperthreading mode, avoiding the use of CPU cores {}').format(unused_cores))
    unit_size = None
    for unit, cores in cores_of_unit.items():
        if unit_size is None:
            unit_size = len(cores)
        elif unit_size != len(cores):
            sys.exit((b'Asymmetric machine architecture not supported: CPU/memory region {0} has {1} cores, but other CPU/memory region has {2} cores.').format(unit, len(cores), unit_size))
    ...
    for core, siblings in siblings_of_core.items():
        if core_size is None:
            ...
            sys.exit((b'Asymmetric machine architecture not supported: CPU core {0} has {1} siblings, but other core has {2} siblings.').format(core, len(siblings), core_size))

    all_cpus_set = set(allCpus)
    for core, siblings in siblings_of_core.items():
        siblings_set = set(siblings)
        if not siblings_set.issubset(all_cpus_set):
            sys.exit((b'Core assignment is unsupported because siblings {0} of core {1} are not usable. Please always make all virtual cores of a physical core available.').format(siblings_set.difference(all_cpus_set), core))
    ...
    if units_per_run > 1 and units_per_run * num_of_threads > unit_count:
        sys.exit((b'Cannot split runs over multiple CPUs/memory regions and at the same time assign multiple runs to the same CPU/memory region. Please reduce the number of threads to {0}.').format(unit_count // units_per_run))
    ...
    if units_per_run == 1 and runs_per_unit * coreLimit > unit_size:
        sys.exit((b'Cannot run {} benchmarks with {} cores on {} CPUs/memory regions with {} cores, because runs would need to be split across multiple CPUs/memory regions. Please reduce the number of threads.').format(num_of_threads, coreLimit, unit_count, unit_size))
    need_HT = False
    if units_per_run == 1:
        assert coreLimit * runs_per_unit <= unit_size
        if coreLimit_rounded_up * runs_per_unit > unit_size:
            need_HT = True
            logging.warning(b'The number of threads is too high and hyper-threading sibling cores need to be split among different runs, which makes benchmarking unreliable. Please reduce the number of threads to %s.', unit_size // coreLimit_rounded_up * unit_count)
    else:
        if coreLimit_rounded_up * num_of_threads > len(allCpus):
            assert coreLimit_rounded_up * runs_per_unit > unit_size
            need_HT = True
            logging.warning(b'The number of threads is too high and hyper-threading sibling cores need to be split among different runs, which makes benchmarking unreliable. Please reduce the number of threads to %s.', len(allCpus) // coreLimit_rounded_up)
        logging.debug(b'Going to assign at most %s runs per CPU/memory region, each one using %s cores and blocking %s cores on %s CPUs/memory regions.', runs_per_unit, coreLimit, coreLimit_rounded_up, units_per_run)
        result = []
        used_cores = set()
        for run in range(num_of_threads):
            ...
            for unit_nr in range(start_unit, start_unit + units_per_run):
                ...
                for core in cores_of_unit[unit]:
                    if core not in cores:
                        cores.extend(c for c in siblings_of_core[core] if c not in used_cores)
            ...
            assert len(cores) == coreLimit, (b'Wrong number of cores for run {} of {} - previous results: {}, remaining cores per CPU/memory region: {}, current cores: {}').format(run + 1, num_of_threads, result, cores_of_unit, cores)
    ...
    assert len(set(itertools.chain(*result))) == num_of_threads * coreLimit, b'Cores are not uniquely assigned to runs: ' + result
    logging.debug(b'Final core assignment: %s.', result)
    return result
...
def get_memory_banks_per_run(coreAssignment, cgroups):
    ...
    try:
        ...
        for cores in coreAssignment:
            ...
            logging.debug(b'Memory banks for cores %s are %s, of which we can use %s.', cores, list(mems), allowedMems)
            result.append(allowedMems)

        assert len(result) == len(coreAssignment)
        if any(result) and os.path.isdir(b'/sys/devices/system/node/'):
            return result
        return
    except ValueError as e:
        sys.exit((b'Could not read memory information from kernel: {0}').format(e))
...
def _get_memory_banks_listed_in_dir(path):
    ...
    return [ int(entry[4:]) for entry in os.listdir(path) if entry.startswith(b'node') ]
...
def check_memory_size(memLimit, num_of_threads, memoryAssignment, my_cgroups):
    ...
    try:
        ...
        def check_limit(actualLimit):
            if actualLimit < memLimit:
                sys.exit((b'Cgroups allow only {} bytes of memory to be used, cannot execute runs with {} bytes of memory.').format(actualLimit, memLimit))
            elif actualLimit < memLimit * num_of_threads:
                sys.exit((b'Cgroups allow only {} bytes of memory to be used, not enough for {} benchmarks with {} bytes each. Please reduce the number of threads').format(actualLimit, num_of_threads, memLimit))

        if not os.path.isdir(b'/sys/devices/system/node/'):
            logging.debug(b'System without NUMA support in Linux kernel, ignoring memory assignment.')
            return
        if cgroups.MEMORY in my_cgroups:
            for key, value in my_cgroups.get_key_value_pairs(cgroups.MEMORY, b'stat'):
                if key == b'hierarchical_memory_limit' or key == b'hierarchical_memsw_limit':
        ...
        if not memoryAssignment:
            if cgroups.CPUSET in my_cgroups:
                ...
                allMems = _get_memory_banks_listed_in_dir(b'/sys/devices/system/node/')
            memoryAssignment = [allMems] * num_of_threads
        else:
    ...
    for mems_of_run in memoryAssignment:
        ...
        if totalSize < memLimit:
            sys.exit((b'Memory banks {} do not have enough memory for one run, only {} bytes available.').format(mems_of_run, totalSize))
        usedMem[tuple(mems_of_run)] += memLimit
        if usedMem[tuple(mems_of_run)] > totalSize:
            sys.exit((b'Memory banks {} do not have enough memory for all runs, only {} bytes available. Please reduce the number of threads.').format(mems_of_run, totalSize))


def _get_memory_bank_size(memBank):
    """Get the size of a memory bank in bytes."""
    fileName = (b'/sys/devices/system/node/node{0}/meminfo').format(memBank)
    size = None
    with open(fileName) as (f):
        for line in f:
            if b'MemTotal' in line:
                size = line.split(b':')[1].strip()
                if size[-3:] != b' kB':
                    raise ValueError((b'"{}" in file {} is not a memory size.').format(size, fileName))
                size = int(size[:-3]) * 1024
                logging.debug(b'Memory bank %s has size %s bytes.', memBank, size)
                return size

    raise ValueError((b'Failed to read total memory from {}.').format(fileName))
    return


def get_cpu_package_for_core(core):
    """Get the number of the physical package (socket) a core belongs to."""
    return int(util.read_file((b'/sys/devices/system/cpu/cpu{0}/topology/physical_package_id').format(core)))


def get_cores_of_same_package_as(core):
    return util.parse_int_list(util.read_file((b'/sys/devices/system/cpu/cpu{0}/topology/core_siblings_list').format(core)))