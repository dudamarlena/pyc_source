...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\core\server.py
...
class Server(object):
    ...
    def __init__(self, host, port=None, use_ssl=False, allowed_referral_hosts=None, get_info=SCHEMA, tls=None, formatter=None, connect_timeout=None, mode=IP_V6_PREFERRED, validator=None):
        self.ipc = False
        url_given = False
        host = host.strip()
        ...
        if isinstance(allowed_referral_hosts, SEQUENCE_TYPES):
            ...
            if isinstance(allowed_referral_hosts, tuple):
            ...
            if tls:
                ...
                if self.ipc or self._is_ipv6(self.host):
            ...
            self._address_info = []
            self._address_info_resolved_time = datetime(MINYEAR, 1, 1)
        ...
        if not self._address_info or (datetime.now() - self._address_info_resolved_time).seconds > conf_refresh_interval:
            ...
            try:
                if self.ipc:
                    addresses = [
                     (
                      socket.AF_UNIX, socket.SOCK_STREAM, 0, None, self.host, None)]
            ...
            try:
                ...
                for bind_port in candidate_bind_ports:
                    try:
                        ...
                        break
                    except Exception, bind_ex:
                        last_bind_exc = bind_ex
                        if log_enabled(NETWORK):
                            log(NETWORK, 'Unable to bind to local address <%s> with source port <%s> due to <%s>', bind_address, bind_port, bind_ex)

                if not socket_bind_succeeded:
                    if log_enabled(ERROR):
                        log(ERROR, 'Unable to locally bind to local address <%s> with any of the source ports <%s> due to <%s>', bind_address, candidate_bind_ports, last_bind_exc)
                    raise LDAPSocketOpenError(('Unable to bind socket locally to address {} with any of the source ports {} due to {}').format(bind_address, candidate_bind_ports, last_bind_exc))
                if self.connect_timeout:
                    temp_socket.settimeout(self.connect_timeout)
                else:
                    temp_socket.settimeout(conf_availability_timeout)
                try:
                    try:
    ...
    def _get_dsa_info(self, connection):
        ...
        """
        if connection.strategy.no_real_dsa:
            return
            result = connection.strategy.pooled or connection.search(search_base='', search_filter='(objectClass=*)', search_scope=BASE, attributes=[
             'altServer',
             'namingContexts',
             'supportedControl',
             'supportedExtension',
    ...
    def _get_schema_info(self, connection, entry=''):
        ...
        if connection.strategy.no_real_dsa:
            ...
            if schema_entry and not connection.strategy.pooled:
                if isinstance(schema_entry, bytes) and str is not bytes:
                    schema_entry = to_unicode(schema_entry, from_server=True)
                result = connection.search(schema_entry, search_filter='(objectClass=subschema)', search_scope=BASE, attributes=[
                 'objectClasses',
                 'attributeTypes',
                 'ldapSyntaxes',
                 'matchingRules',
                ...
                with self.dit_lock:
                    ...
                    if result:
                        ...
                        if self._schema_info and not self._schema_info.is_valid():
                            self._schema_info = None
    ...
    def from_definition(host, dsa_info, dsa_schema, port=None, use_ssl=False, formatter=None, validator=None):
        ...
        if isinstance(host, SEQUENCE_TYPES):
            dummy = Server(host=host[0], port=port, use_ssl=use_ssl, formatter=formatter, validator=validator, get_info=ALL)
        else:
        ...
        if isinstance(dsa_info, DsaInfo):
            ...
            try:
                dummy._dsa_info = DsaInfo.from_json(dsa_info)
            except Exception:
                dummy._dsa_info = DsaInfo.from_file(dsa_info)
    ...
    def candidate_addresses(self):
        ...
        if self.ipc:
            ...
            addresses = self.address_info[:]
            candidates = []
            if addresses:
                if self.mode == IP_SYSTEM_DEFAULT:
                    candidates.append(addresses[0])
                elif self.mode == IP_V4_ONLY:
                    candidates = [ address for address in addresses if address[0] == socket.AF_INET if address[5] or address[5] is None ]
                elif self.mode == IP_V6_ONLY:
                    candidates = [ address for address in addresses if address[0] == socket.AF_INET6 if address[5] or address[5] is None ]
    ...
    def _check_info_property(self, kind, name):
        ...
        for prop in properties:
            if name == prop[0] or prop[2] and name.lower() == prop[2].lower():
                return True
