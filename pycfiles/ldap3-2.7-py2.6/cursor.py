...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\abstract\cursor.py
...
class Cursor(object):
    ...
    def _get_attributes(self, response, attr_defs, entry):
        ...
        for attr in attr_defs:
            ...
            if attribute_name or attr_def.default is not NotImplemented:
                ...
                elif attr_def.default is NotImplemented or attribute_name and response['raw_attributes'][attribute_name] != list():
                ...
                if not isinstance(attribute.values, list):
                    attribute.values = [
                     attribute.values]
                if attr_def.dereference_dn:
                    if attribute.values:
    ...
    def _execute_query(self, query_scope, attributes):
        ...
        with self.connection:
            result = self.connection.search(search_base=self.base, search_filter=self.query_filter, search_scope=query_scope, dereference_aliases=self.dereference_aliases, attributes=attributes if attributes else list(self.attributes), get_operational_attributes=self.get_operational_attributes, controls=self.controls)
            ...
            if attr in self.definition:
                ...
                for val in vals:
                    ...
                    if self.definition[attr].validate:
                        validated = self.definition[attr].validate(value)
    ...
    def _create_query_filter(self):
        ...
        if self.definition._object_class:
            ...
            if isinstance(self.definition._object_class, SEQUENCE_TYPES) and len(self.definition._object_class) == 1:
                ...
                if log_enabled(ERROR):
                    log(ERROR, '%s for <%s>', error_message, self)
                raise LDAPCursorError(error_message)
        if self._query and self._query.startswith('(') and self._query.endswith(')'):
        ...
        if self._query:
            ...
            if not self.components_in_and:
                ...
                self.query_filter += ')'
            if not self.definition._object_class and attr_counter == 1:
                self.query_filter = self.query_filter[2:-1]
            if self.query_filter == '(|)' or self.query_filter == '(&)':
                self.query_filter = ''
        else:
...
class Writer(Cursor):
    ...
    def _refresh_object(self, entry_dn, attributes=None, tries=4, seconds=2, controls=None):
        ...
        with self.connection:
            counter = 0
            while counter < tries:
                result = self.connection.search(search_base=entry_dn, search_filter='(objectclass=*)', search_scope=BASE, dereference_aliases=DEREF_NEVER, attributes=attributes if attributes else self.attributes, get_operational_attributes=self.get_operational_attributes, controls=controls)
                if not self.connection.strategy.sync:
                    (response, result, request) = self.connection.get_response(result, get_request=True)
                else:
                    response = self.connection.response
    ...
    def new(self, dn):
        ...
        entry = self.entry_class(dn, self)
        for attr in entry.entry_mandatory_attributes:
        ...
        for rdn in rdns:
            if rdn[0] in entry._state.definition._attributes:
                rdn_name = entry._state.definition._attributes[rdn[0]].name
        ...
        entry._state.set_status(STATUS_PENDING_CHANGES)
        self.entries.append(entry)
        return entry
    ...
    def refresh_entry(self, entry, tries=4, seconds=2):
        ...
        for attr in entry._state.attributes:
        ...
        temp_entry = self._refresh_object(entry.entry_dn, attr_list, tries, seconds=seconds)
        self._do_not_reset = False
        if temp_entry:
