...
class LDAPOperationResult(LDAPException):
    ...
    def __new__(cls, result=None, description=None, dn=None, message=None, response_type=None, response=None):
        if cls is LDAPOperationResult and result and result in exception_table:
            exc = super(LDAPOperationResult, exception_table[result]).__new__(exception_table[result])
            exc.result = result
...
exception_table = {RESULT_OPERATIONS_ERROR: LDAPOperationsErrorResult, RESULT_PROTOCOL_ERROR: LDAPProtocolErrorResult, 
        ...
        if self.args:
            if isinstance(self.args, tuple):
                ...
                if len(self.args) > 1:
                    ...
                    prev_exc = ''
                    for (i, exc) in enumerate(self.args[1]):
                        if str(exc) != prev_exc:
