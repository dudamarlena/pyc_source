...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\abstract\objectDef.py
...
class ObjectDef(object):
    ...
    def __init__(self, object_class=None, schema=None, custom_validator=None, auxiliary_class=None):
        ...
        if isinstance(schema, Connection) and (schema._deferred_bind or schema._deferred_open):
        ...
        if self._schema:
            object_class = [ schema.object_classes[name].name[0] for name in object_class ]
        ...
        if isinstance(definition, STRING_TYPES):
            ...
            if definition.key.lower() not in conf_attributes_excluded_from_object_def:
                if definition.key not in self._attributes:
                    ...
                    if definition.name:
                        if definition.name != definition.key:
                    ...
                    validator = definition.validate or find_attribute_validator(self._schema, definition.key, self._custom_validator)
