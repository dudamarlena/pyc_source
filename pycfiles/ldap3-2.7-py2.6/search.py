...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\operation\search.py
# Compiled at: 2020-02-23 05:35:52
"""
"""
...
def evaluate_match(match, schema, auto_escape, auto_encode, validator, check_names):
    ...
    if left_part.endswith('~'):
        ...
        elif len(extended_filter_list) <= 3:
...
def parse_filter(search_filter, schema, auto_escape, auto_encode, validator, check_names):
    if str is not bytes and isinstance(search_filter, bytes):
...
def compile_filter(filter_node):
    ...
    if filter_node.tag == AND:
        ...
        compiled_filter.setComponentByName('notFilter', boolean_filter, verifyConstraints=False)
...
def build_attribute_selection(attribute_list, schema):
    ...
    for (index, attribute) in enumerate(attribute_list):
        if schema and schema.attribute_types:
            if ';' in attribute:
                if attribute[0:attribute.index(';')] not in schema.attribute_types and attribute.lower() not in conf_attributes_excluded_from_check:
                    raise LDAPAttributeError('invalid attribute type in attribute list: ' + attribute)
            elif attribute not in schema.attribute_types and attribute.lower() not in conf_attributes_excluded_from_check:
                raise LDAPAttributeError('invalid attribute type in attribute list: ' + attribute)
...
def search_operation(search_base, search_filter, search_scope, dereference_aliases, attributes, size_limit, time_limit, types_only, auto_escape, auto_encode, schema=None, validator=None, check_names=False):
    ...
    request['filter'] = compile_filter(parse_filter(search_filter, schema, auto_escape, auto_encode, validator, check_names).elements[0])
...
def decode_vals_fast(vals):
    try:
        if vals:
            return [ to_unicode(val[3], from_server=True) for val in vals if val ]
        else:
            return
    except UnicodeDecodeError:
        if vals:
            return [ val[3] for val in vals if val ]
        return
...
def decode_raw_vals_fast(vals):
    if vals:
        return [ bytes(val[3]) for val in vals ]
    else:
        return
...
def search_request_to_dict(request):
    return {'base': str(request['baseObject']), 'scope': int(request['scope']), 
...
def search_result_entry_response_to_dict(response, schema, custom_formatter, check_names):
    entry = dict()
    if response['object']:
        if isinstance(response['object'], STRING_TYPES):
...
def search_result_done_response_to_dict(response):
    result = {'result': int(response['resultCode']), 'description': ResultCode().getNamedValues().getName(response['resultCode']), 
...
def search_result_entry_response_to_dict_fast(response, schema, custom_formatter, check_names):
    ...
    entry_dict['raw_attributes'] = raw_attributes_to_dict_fast(response[1][3])
    if check_names:
        entry_dict['attributes'] = checked_attributes_to_dict_fast(response[1][3], schema, custom_formatter)
    else:
        entry_dict['attributes'] = attributes_to_dict_fast(response[1][3])
    return entry_dict
