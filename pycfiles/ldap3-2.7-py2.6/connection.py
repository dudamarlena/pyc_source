...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\core\connection.py
...
def _format_socket_endpoint(endpoint):
    ...
    if endpoint and len(endpoint) == 4:
        return '[' + str(endpoint[0]) + ']:' + str(endpoint[1])
    try:
        return str(endpoint)
    except Exception:
...
class Connection(object):
    ...
    def __init__(self, server, user=None, password=None, auto_bind=AUTO_BIND_DEFAULT, version=3, authentication=None, client_strategy=SYNC, auto_referrals=True, auto_range=True, sasl_mechanism=None, sasl_credentials=None, check_names=True, collect_usage=False, read_only=False, lazy=False, raise_exceptions=False, pool_name=None, pool_size=None, pool_lifetime=None, cred_store=None, fast_decoder=True, receive_timeout=None, return_empty_attributes=True, use_referral_cache=False, auto_escape=True, auto_encode=True, pool_keepalive=None, source_address=None, source_port=None, source_port_list=None):
    ...
    def do_auto_bind(self):
        if self.auto_bind:
            if self.auto_bind not in [AUTO_BIND_NONE, AUTO_BIND_DEFAULT]:
                if log_enabled(BASIC):
                ...
                self.last_error = self.bound or 'automatic bind not successful' + (' - ' + self.last_error if self.last_error else '')
                if log_enabled(ERROR):
                    log(ERROR, '%s for <%s>', self.last_error, self)
                self.unbind()
    ...
    def __str__(self):
        s = [str(self.server) if self.server else 'None',
         'user: ' + str(self.user),
    ...
    def stream(self):
        ...
        if self.strategy.can_stream:
            return self.strategy.get_stream()
        else:
            return
    ...
    def __enter__(self):
        with self.connection_lock:
            self._context_state.append((self.bound, self.closed))
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        with self.connection_lock:
            ...
            if not context_bound and self.bound or self.stream:
                try:
                    self.unbind()
            ...
            if self.lazy and not self._executing_deferred:
                ...
                if self.authentication == ANONYMOUS:
                    ...
                    if not self.strategy.pooled:
                        ...
                        response = self.strategy.validate_bind(controls)
                    ...
                    if self.user:
                        if self.password and len(self.user.split('\\')) == 2:
                            if log_enabled(PROTOCOL):
                                log(PROTOCOL, 'performing NTLM BIND for <%s>', self)
                            response = self.strategy.pooled or self.do_ntlm_bind(controls)
                        else:
                            response = self.strategy.validate_bind(controls)
                    else:
                    ...
                    if log_enabled(ERROR):
                        log(ERROR, '%s for <%s>', self.last_error, self)
                    raise LDAPUnknownAuthenticationMethodError(self.last_error)
                if not self.strategy.sync and not self.strategy.pooled and self.authentication not in (SASL, NTLM):
                ...
                elif self.strategy.pooled or self.authentication in (SASL, NTLM):
                    result = response
                else:
                    self.last_error = 'unknown authentication method'
                    if log_enabled(ERROR):
    ...
    def rebind(self, user=None, password=None, authentication=None, sasl_mechanism=None, sasl_credentials=None, read_server_info=True, controls=None):
        if log_enabled(BASIC):
    ...
    def unbind(self, controls=None):
        ...
        self.last_error = None
        with self.connection_lock:
            if self.lazy and not self._executing_deferred and (self._deferred_bind or self._deferred_open):
                self.strategy.close()
                self._deferred_open = False
                self._deferred_bind = False
                self._deferred_start_tls = False
    ...
    def search(self, search_base, search_filter, search_scope=SUBTREE, dereference_aliases=DEREF_ALWAYS, attributes=None, size_limit=0, time_limit=0, types_only=False, get_operational_attributes=False, controls=None, paged_size=None, paged_criticality=False, paged_cookie=None, auto_escape=None):
        """
        ...
        with self.connection_lock:
            ...
            request = search_operation(search_base, search_filter, search_scope, dereference_aliases, attributes, size_limit, time_limit, types_only, self.auto_escape if auto_escape is None else auto_escape, self.auto_encode, self.server.schema if self.server else None, validator=self.server.custom_validator, check_names=self.check_names)
            if log_enabled(PROTOCOL):
    ...
    def add(self, dn, object_class=None, attributes=None, controls=None):
        ...
        with self.connection_lock:
            ...
            attr_object_class = [ to_unicode(object_class) for object_class in attr_object_class ]
            _attributes[object_class_attr_name] = reduce(lambda x, y: x + [y] if y not in x else x, parm_object_class + attr_object_class, [])
    ...
    def abandon(self, message_id, controls=None):
        ...
        with self.connection_lock:
            ...
            if self.strategy._outstanding or message_id == 0:
                if self.strategy._outstanding and message_id in self.strategy._outstanding and self.strategy._outstanding[message_id]['type'] not in ('abandonRequest',
                                                                                                                                                      'bindRequest',
                                                                                                                                                      'unbindRequest') or message_id == 0:
    ...
    def start_tls(self, read_server_info=True):
        ...
        with self.connection_lock:
            ...
            if self.lazy and not self._executing_deferred:
                ...
                if self.closed:
                    self.open()
                if self.server.tls.start_tls(self) and self.strategy.sync:
                    if read_server_info:
                        self.refresh_server_info()
                    return_value = True
    ...
    def response_to_json(self, raw=False, search_result=None, indent=4, sort=True, stream=None, checked_attributes=True, include_empty=True):
        with self.connection_lock:
            if search_result is None:
                search_result = self.response
            if isinstance(search_result, SEQUENCE_TYPES):
    ...
    def response_to_file(self, target, raw=False, indent=4, sort=True):
        with self.connection_lock:
            if self.response:
