...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\core\tls.py
...
class Tls(object):
    ...
    def __init__(self, local_private_key_file=None, local_certificate_file=None, validate=ssl.CERT_NONE, version=None, ssl_options=None, ca_certs_file=None, valid_names=None, ca_certs_path=None, ca_certs_data=None, local_private_key_password=None, ciphers=None, sni=None):
    ...
    def wrap_socket(self, connection, do_handshake=False):
        ...
        if use_ssl_context:
            ...
            if self.ciphers:
                try:
                    wrapped_socket = ssl.wrap_socket(connection.socket, keyfile=self.private_key_file, certfile=self.certificate_file, server_side=False, cert_reqs=self.validate, ssl_version=self.version, ca_certs=self.ca_certs_file, do_handshake_on_connect=do_handshake, ciphers=self.ciphers)
                except ssl.SSLError:
                    raise
                except TypeError:
                    self.ciphers = None
        ...
        if connection.server.ssl:
            ...
            if connection.tls_started and not connection._executing_deferred or connection.strategy._outstanding or connection.sasl_in_progress:
                ...
                if not connection.strategy.sync:
                    connection._awaiting_for_async_start_tls = True
                result = connection.extended('1.3.6.1.4.1.1466.20037')
                response = connection.strategy.sync or connection.get_response(result)
...
def check_hostname(sock, server_name, additional_names):
    ...
    if additional_names:
        host_names = [
         server_name] + (additional_names if isinstance(additional_names, SEQUENCE_TYPES) else [additional_names])
    else:
        host_names = [
         server_name]
    for host_name in host_names:
        if not host_name:
            ...
            return
        try:
            match_hostname(server_certificate, host_name)
            if log_enabled(NETWORK):
