...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\protocol\formatters\formatters.py
...
def format_ad_timestamp(raw_value):
    ...
    if raw_value == '9223372036854775807':
        return datetime.max.replace(tzinfo=utc_timezone)
    try:
        ...
        try:
            return datetime.fromtimestamp(timestamp / 10000000.0 - 11644473600, tz=utc_timezone)
...
try:
    ...
    def format_time(raw_value):
        try:
            ...
            offset = timedelta(hours=int(matches['OffHour'] or 0), minutes=int(matches['OffMinute'] or 0))
            if matches['Offset'] == '-':
                offset *= -1
            ...
            return datetime(int(matches['Year']), int(matches['Month']), int(matches['Day']), int(matches['Hour']), minute, second, microseconds, timezone(offset))
        except Exception:
            pass
    ...
    def format_time(raw_value):
        ...
        """
        if len(raw_value) < 10 or not all(c in '0123456789+-,.Z' for c in raw_value) or 'Z' in raw_value and not raw_value.endswith('Z'):
            return raw_value
        ...
        microsecond = 0
        remain = raw_value[10:]
        if remain and remain.endswith('Z'):
            sep = 'Z'
        elif '+' in remain:
            sep = '+'
        elif '-' in remain:
        ...
        if time and ('.' in time or ',' in time):
            if time[0] in ',.':
                minute = 6 * int(time[1] if str is bytes else chr(time[1]))
            elif time[2] in ',.':
                minute = int(raw_value[10:12])
                second = 6 * int(time[3] if str is bytes else chr(time[3]))
            elif time[4] in ',.':
                minute = int(raw_value[10:12])
                second = int(raw_value[12:14])
                microsecond = 100000 * int(time[5] if str is bytes else chr(time[5]))
        elif len(time) == 2:
            minute = int(raw_value[10:12])
        elif len(time) == 0:
            pass
        ...
        if sep == 'Z':
            ...
            try:
                ...
                if timezone_hour > 23 or timezone_minute > 59:
                    return raw_value
                if str is not bytes:
                    timezone = OffsetTzInfo((timezone_hour * 60 + timezone_minute) * (1 if sep == '+' else -1), 'UTC' + str(sep + offset, encoding='utf-8'))
                else:
                    timezone = OffsetTzInfo((timezone_hour * 60 + timezone_minute) * (1 if sep == '+' else -1), unicode('UTC' + sep + offset, encoding='utf-8'))

            try:
                return datetime(year=year, month=month, day=day, hour=hour, minute=minute, second=second, microsecond=microsecond, tzinfo=timezone)
            except (TypeError, ValueError):
                pass
...
def format_ad_timedelta(raw_value):
    ...
    """
    if raw_value == '-9223372036854775808':
...
def format_sid(raw_value):
    ...
    try:
        if str is not bytes:
            ...
            if identifier_authority >= 4294967296:
                identifier_authority = hex(identifier_authority)
            sub_authority = ''
            i = 0
            while i < sub_authority_count:
                sub_authority += '-' + str(int.from_bytes(raw_value[8 + i * 4:12 + i * 4], byteorder='little'))
                i += 1
            ...
            i = 0
            while i < sub_authority_count:
                sub_authority += '-' + str(int(hexlify(raw_value[11 + i * 4:7 + i * 4:-1]), 16))
                i += 1
