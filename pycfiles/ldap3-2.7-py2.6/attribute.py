...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\abstract\attribute.py
...
class Attribute(object):
    ...
    def __init__(self, attr_def, entry, cursor):
        ...
        self.other_names = set(other_names) if other_names else None
        return
...
class WritableAttribute(Attribute):
    ...
    def _update_changes(self, changes, remove_old=False):
        if self.definition and self.definition.name:
            ...
            key = self.key
        if key not in self.entry._changes or remove_old:
    ...
    def add(self, values):
        ...
        if values is not None:
            validated = self.definition.validate(values)
