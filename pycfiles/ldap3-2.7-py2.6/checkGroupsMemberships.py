...
def _check_members_have_memberships(connection, members_dn, groups_dn):
    """
    ...
    for member in members_dn:
        ...
        for group in groups_dn:
            if group.lower() not in existing_group_membership:
                return (
                 False, False)
            elif group.lower() not in existing_security_equals:
                partial = True
...
def _check_groups_contain_members(connection, groups_dn, members_dn):
    """
    :param connection: a bound Connection object
    :param members_dn: the list of members to add to groups
    :param groups_dn: the list of groups where members are to be added
    ...
    for group in groups_dn:
        ...
        for member in members_dn:
            if member.lower() not in existing_members:
                return (
                 False, False)
            elif member.lower() not in existing_equivalent_to_me:
                partial = True
...
def edir_check_groups_memberships(connection, members_dn, groups_dn, fix, transaction):
    ...
    try:
        ...
        if not members_have_memberships and not groups_contain_members:
            return False
        if fix:
            if members_have_memberships and not groups_contain_members or groups_contain_members and not members_have_memberships or partial_group_security or partial_member_security:
                for member in members_dn:
                    for group in groups_dn:
