...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\core\pooling.py
...
class ServerPoolState(object):
    ...
    def refresh(self):
        ...
        for server in self.server_pool.servers:
            self.server_states.append(ServerState(server, datetime(MINYEAR, 1, 1), True))
    ...
    def find_active_random_server(self):
        counter = self.server_pool.active
        while counter:
            if log_enabled(NETWORK):
                log(NETWORK, 'entering loop for finding active server in pool <%s>', self)
            temp_list = self.server_states[:]
            while temp_list:
                server_state = temp_list.pop(randint(0, len(temp_list) - 1))
                if not server_state.available:
                    if isinstance(self.server_pool.exhaust, bool) and self.server_pool.exhaust or (datetime.now() - server_state.last_checked_time).seconds < self.server_pool.exhaust:
                        if log_enabled(NETWORK):
...
class ServerPool(object):
    ...
    def __init__(self, servers=None, pool_strategy=ROUND_ROBIN, active=True, exhaust=False, single_state=True):
