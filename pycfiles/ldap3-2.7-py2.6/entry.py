...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\abstract\entry.py
# Compiled at: 2020-02-23 12:53:27
"""
"""
import json
try:
    ...
    from ..utils.ordDict import OrderedDict
...
from . import STATUS_VIRTUAL, STATUS_WRITABLE, STATUS_PENDING_CHANGES, STATUS_COMMITTED, STATUS_DELETED, STATUS_INIT, STATUS_READY_FOR_DELETION, STATUS_READY_FOR_MOVING, STATUS_READY_FOR_RENAMING, STATUS_MANDATORY_MISSING, STATUSES, INITIAL_STATUSES
...
class EntryState(object):
    ...
    def set_status(self, status):
        ...
        if self.status == STATUS_VIRTUAL or self.status == STATUS_PENDING_CHANGES and self._initial_status == STATUS_VIRTUAL:
            for attr in self.definition._attributes:
                if self.definition._attributes[attr].mandatory:
                    if attr.lower() not in conf_ignored_mandatory_attributes_in_object_def and (attr not in self.attributes or self.attributes[attr].virtual) and attr not in self.changes:
...
class EntryBase(object):
    ...
    def entry_raw_attribute(self, name):
        ...
        """
        if name in self._state.raw_attributes:
            return self._state.raw_attributes[name]
        else:
            return
    ...
    def entry_to_json(self, raw=False, indent=4, sort=True, stream=None, checked_attributes=True, include_empty=True):
        ...
        json_output = json.dumps(json_entry, ensure_ascii=True, sort_keys=sort, indent=indent, check_circular=True, default=format_json, separators=(',',
                                                                                                                                                     ': '))
        if stream:
...
class Entry(EntryBase):
    ...
    def entry_writable(self, object_def=None, writer_cursor=None, attributes=None, custom_validator=None, auxiliary_class=None):
        ...
        writable_entry._state.origin = self
        attr_to_add = []
        attr_to_remove = []
        object_def_to_add = []
        ...
        for attr in writable_entry._state.origin.entry_definition._attributes:
            ...
            if attr != original_attr.name and (attr not in writable_entry._state.attributes or conf_operational_attribute_prefix + original_attr.name not in writable_entry._state.attributes):
                ...
                new_attr_def = AttrDef(original_attr.name, key=attr, validate=original_attr.validate, pre_query=original_attr.pre_query, post_query=original_attr.post_query, default=original_attr.default, dereference_dn=original_attr.dereference_dn, description=original_attr.description, mandatory=old_attr_def.mandatory, single_value=old_attr_def.single_value, alias=original_attr.other_names)
    ...
    def __setattr__(self, item, value):
        ...
        if value is not Ellipsis:
            if item in self.entry_cursor.definition._attributes or item.lower() in conf_attributes_excluded_from_object_def:
                if item not in self._state.attributes:
                    new_attribute = WritableAttribute(self.entry_cursor.definition._attributes[item], self, cursor=self.entry_cursor)
                    self._state.attributes[str(item)] = new_attribute
                self._state.attributes[item].set(value)
            else:
    ...
    def entry_commit_changes(self, refresh=True, controls=None, clear_history=True):
        ...
        if self.entry_status == STATUS_READY_FOR_DELETION:
            ...
            if result['result'] == RESULT_SUCCESS:
                dn = self.entry_dn
                if self._state.origin and self.entry_cursor.connection.server == self._state.origin.entry_cursor.connection.server:
            ...
            if self.entry_status == STATUS_READY_FOR_MOVING:
                ...
                if result['result'] == RESULT_SUCCESS:
                    ...
                    if refresh:
                        if self.entry_refresh():
                            if self._state.origin:
                                if self.entry_cursor.connection.server == self._state.origin.entry_cursor.connection.server:
                                    self._state.origin._state.dn = self.entry_dn
                        self._state.set_status(STATUS_COMMITTED)
                        self._state._to = None
                        return True
                    return False
                if self.entry_status == STATUS_READY_FOR_RENAMING:
                    rdn = ('+').join(safe_rdn(self._state._to))
                    result = self.entry_cursor.connection.modify_dn(self.entry_dn, rdn)
                    (response, result, request) = self.entry_cursor.connection.strategy.sync or self.entry_cursor.connection.get_response(result, get_request=True)
                else:
                    response = self.entry_cursor.connection.response
                    result = self.entry_cursor.connection.result
                    request = self.entry_cursor.connection.request
                ...
                if result['result'] == RESULT_SUCCESS:
                    ...
                    return True
                return False
            if self.entry_status in [STATUS_VIRTUAL, STATUS_MANDATORY_MISSING]:
                missing_attributes = []
                ...
                if self._changes:
                    if self.entry_definition._auxiliary_class:
                                ...
                                if self._state.origin and self.entry_cursor.connection.server == self._state.origin.entry_cursor.connection.server:
                                    for attr in self:
                                        if attr.key in self.entry_definition._attributes and attr.key not in self._state.origin.entry_definition._attributes:
                                            self._state.origin.entry_cursor.definition.add_attribute(self.entry_cursor.definition._attributes[attr.key])
