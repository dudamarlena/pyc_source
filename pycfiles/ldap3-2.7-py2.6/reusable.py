...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\strategy\reusable.py
...
class ReusableStrategy(BaseStrategy):
    ...
    class ConnectionPool(object):
        ...
        def __new__(cls, connection):
            if connection.pool_name in ReusableStrategy.pools:
                ...
                if connection.pool_keepalive and pool.keepalive != connection.pool_keepalive:
                    pool.keepalive = connection.pool_keepalive
                if connection.pool_lifetime and pool.lifetime != connection.pool_lifetime:
                    pool.lifetime = connection.pool_lifetime
                if connection.pool_size and pool.pool_size != connection.pool_size:
                    pool.terminate_pool()
        ...
        def rebind_pool(self):
            for worker in self.workers:
                with worker.worker_lock:
                    worker.connection.rebind(self.master_connection.user, self.master_connection.password, self.master_connection.authentication, self.master_connection.sasl_mechanism, self.master_connection.sasl_credentials)
        ...
        def terminate_pool(self):
            if not self.terminated:
                ...
                for _ in range(len([ worker for worker in self.workers if worker.thread.is_alive() ])):
            ...
            while not terminate:
                ...
                with self.worker.worker_lock:
                    ...
                    if counter == TERMINATE_REUSABLE:
                        ...
                        if (datetime.now() - self.worker.creation_time).seconds >= self.master_connection.strategy.pool.lifetime:
                            try:
                                self.worker.connection.unbind()
                            except LDAPExceptionError:
                                pass
                        ...
                        if message_type not in ('bindRequest', 'unbindRequest'):
                            try:
                                ...
                                elif pool.open_pool and not self.worker.connection.closed:
                                    if pool.tls_pool and not self.worker.connection.tls_started:
                                        self.worker.connection.start_tls(read_server_info=False)
                                ...
                                with pool.pool_lock:
                                    pool._incoming[counter] = (
                                     response, result, BaseStrategy.decode_request(message_type, request, controls))
                            except LDAPOperationResult, e:
                                with pool.pool_lock:
                                    pool._incoming[counter] = (
                                     e, None, None)
    ...
    def validate_bind(self, controls):
        if self.connection.user != self.pool.master_connection.user or self.connection.password != self.pool.master_connection.password or self.connection.authentication != self.pool.master_connection.authentication or self.connection.sasl_mechanism != self.pool.master_connection.sasl_mechanism or self.connection.sasl_credentials != self.pool.master_connection.sasl_credentials:
