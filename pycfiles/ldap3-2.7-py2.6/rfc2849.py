...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\protocol\rfc2849.py
...
def sort_ldif_lines(lines, sort_order):
    if sort_order:
        return sorted(lines, key=lambda x: ldif_sort(x, sort_order))
    return lines
...
def decode_persistent_search_control(change):
    if 'controls' in change and '2.16.840.1.113730.3.4.7' in change['controls']:
        ...
        if decoded_control['changeType'] == 1:
            decoded['changeType'] = 'add'
        elif decoded_control['changeType'] == 2:
            decoded['changeType'] = 'delete'
        elif decoded_control['changeType'] == 4:
            decoded['changeType'] = 'modify'
        elif decoded_control['changeType'] == 8:
