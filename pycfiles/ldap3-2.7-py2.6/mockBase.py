...
# [GCC 8.4.0]
# Embedded file name: build\bdist.win-amd64\egg\ldap3\strategy\mockBase.py
# Compiled at: 2020-02-29 11:24:43
"""
"""
import json, re
...
from ..operation.search import search_request_to_dict, parse_filter, ROOT, AND, OR, NOT, MATCH_APPROX, MATCH_GREATER_OR_EQUAL, MATCH_LESS_OR_EQUAL, MATCH_EXTENSIBLE, MATCH_PRESENT, MATCH_SUBSTRING, MATCH_EQUAL
...
from ..core.results import RESULT_SUCCESS, RESULT_OPERATIONS_ERROR, RESULT_UNAVAILABLE_CRITICAL_EXTENSION, RESULT_INVALID_CREDENTIALS, RESULT_NO_SUCH_OBJECT, RESULT_ENTRY_ALREADY_EXISTS, RESULT_COMPARE_TRUE, RESULT_COMPARE_FALSE, RESULT_NO_SUCH_ATTRIBUTE, RESULT_UNWILLING_TO_PERFORM
...
class PagedSearchSet(object):
    ...
    def next(self, size=None):
        ...
        result = {'resultCode': RESULT_SUCCESS, 'matchedDN': '', 
           'diagnosticMessage': to_unicode(message, SERVER_ENCODING), 
           'referral': None, 
           'controls': [
                      BaseStrategy.decode_control(response_control)]}
        return (
         response, result)
...
class MockBaseStrategy(object):
        ...
        with self.connection.server.dit_lock:
            ...
            if escaped_dn not in self.connection.server.dit:
                ...
                for attribute in attributes:
                    if attribute in self.operational_attributes:
                        continue
                    if not isinstance(attributes[attribute], SEQUENCE_TYPES):
                        attributes[attribute] = [
                         attributes[attribute]]
                    if self.connection.server.schema and self.connection.server.schema.attribute_types[attribute].single_value and len(attributes[attribute]) > 1:
                        return False
                    if attribute.lower() == 'objectclass' and self.connection.server.schema:
                        class_set = set()
                ...
                for rdn in safe_rdn(escaped_dn, decompose=True):
                    if rdn[0] not in new_entry:
                        new_entry[rdn[0]] = [
                         to_raw(rdn[1])]
                    else:
                        raw_rdn = to_raw(rdn[1])
                        if raw_rdn not in new_entry[rdn[0]]:
    ...
    def mock_bind(self, request_message, controls):
        ...
        return {'resultCode': result_code, 'matchedDN': '', 
           'diagnosticMessage': to_unicode(message, SERVER_ENCODING), 
           'referral': None, 
    ...
    def mock_modify_dn(self, request_message, controls):
        ...
        if dn in self.connection.server.dit:
            ...
            elif new_rdn and not new_superior:
    ...
    def mock_modify(self, request_message, controls):
        ...
        if dn in self.connection.server.dit:
            ...
            for modification in changes:
                ...
                if operation == 0:
                    if attribute not in entry and elements:
                        if self.connection.server.schema and self.connection.server.schema.attribute_types and self.connection.server.schema.attribute_types[attribute].single_value and len(elements) > 1:
                            result_code = 19
                            message = 'attribute is single-valued'
                        else:
                            entry[attribute] = [ to_raw(element) for element in elements ]
                    elif self.connection.server.schema and self.connection.server.schema.attribute_types and self.connection.server.schema.attribute_types[attribute].single_value:
                        result_code = 19
                        message = 'attribute is single-valued'
                    else:
                    ...
                    if attribute not in entry:
                        ...
                        for element in elements:
                            ...
                            if self.equal(dn, attribute, raw_element):
                    ...
                    if attribute not in entry and elements:
                        if self.connection.server.schema and self.connection.server.schema.attribute_types and self.connection.server.schema.attribute_types[attribute].single_value and len(elements) > 1:
                            ...
                            entry[attribute] = [ to_raw(element) for element in elements ]
                    elif not elements and attribute in rdns:
                        result_code = 67
        ...
        if controls:
            ...
            for decoded_control in decoded_controls:
                if decoded_control[1]['criticality'] and decoded_control[0] not in SEARCH_CONTROLS:
                    ...
                    result = {'resultCode': RESULT_UNAVAILABLE_CRITICAL_EXTENSION, 'matchedDN': '', 
                       'diagnosticMessage': to_unicode(message, SERVER_ENCODING), 
                       'referral': None}
                    return ([], result)
                elif decoded_control[0] == '1.2.840.113556.1.4.319':
                    if not decoded_control[1]['value']['cookie']:
                        ...
                        for paged_set in self._paged_sets:
                            if paged_set.cookie == decoded_control[1]['value']['cookie']:
                                (response, result) = paged_set.next()
                                if paged_set.done:
                        ...
                        result = {'resultCode': RESULT_OPERATIONS_ERROR, 'matchedDN': '', 
                           'diagnosticMessage': to_unicode(message, SERVER_ENCODING), 
                           'referral': None}
                        return ([], result)
    ...
    def _execute_search(self, request):
        ...
        if scope == 0:
            ...
            for entry in self.connection.server.dit:
                if entry.lower().endswith(base.lower()) and ',' not in entry[:-len(base) - 1]:
        ...
        if not candidates:
            ...
            if self.connection.raise_exceptions and 0 < request['sizeLimit'] < len(matched):
                ...
                for match in matched:
                    responses.append({'object': match, 
                       'attributes': [ {'type': attribute, 'vals': [] if request['typesOnly'] else self.connection.server.dit[match][attribute]} for attribute in self.connection.server.dit[match] if attribute.lower() in attributes or ALL_ATTRIBUTES in attributes
                                   ]})
                ...
                result_code = 0
                message = ''
        result = {'resultCode': result_code, 'matchedDN': '', 'diagnosticMessage': to_unicode(message, SERVER_ENCODING), 
           'referral': None}
        return (
         responses[:request['sizeLimit']] if request['sizeLimit'] > 0 else responses, result)
    ...
    def mock_extended(self, request_message, controls):
        ...
        if self.connection.server.info:
            for extension in self.connection.server.info.supported_extensions:
                if request['name'] == extension[0]:
                    if extension[0] == '2.16.840.1.113719.1.27.100.31':
                        result_code = 0
                        message = ''
                        response_name = OctetString('2.16.840.1.113719.1.27.100.32')
                        response_value = OctetString(self.bound)
                    elif extension[0] == '1.3.6.1.4.1.4203.1.11.3':
                        result_code = 0
                        message = ''
                        response_name = OctetString('1.3.6.1.4.1.4203.1.11.3')
                        response_value = OctetString(self.bound)
                    break
    ...
    def evaluate_filter_node(self, node, candidates):
        ...
        if node.tag == AND:
            ...
            if node.tag == NOT:
                ...
                for candidate in candidates:
                    if attr_name in self.connection.server.dit[candidate]:
                        for value in self.connection.server.dit[candidate][attr_name]:
                            ...
                            elif to_unicode(value, SERVER_ENCODING).lower() >= to_unicode(attr_value, SERVER_ENCODING).lower():
                                node.matched.add(candidate)
                            else:
                                node.unmatched.add(candidate)
                ...
                if substring_filter and not node.assertion.get('any', None) and not node.assertion.get('final', None):
                    substring_filter += '.*'
                regex_filter = re.compile(substring_filter, flags=re.UNICODE | re.IGNORECASE)
                for candidate in candidates:
                    if attr_name in self.connection.server.dit[candidate]:
                        for value in self.connection.server.dit[candidate][attr_name]:
    ...
    def _check_equality(value1, value2):
        ...
        if str(value1).isdigit() and str(value2).isdigit():
            if int(value1) == int(value2):
                return True
        try:
            if to_unicode(value1, SERVER_ENCODING).lower() == to_unicode(value2, SERVER_ENCODING).lower():
                return True
        except UnicodeError:
            pass
