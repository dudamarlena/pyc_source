...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanhelmig/Code/django-drip/drip/admin.py
# Compiled at: 2014-12-10 01:47:36
# Size of source mod 2**32: 3858 bytes
import base64, json, six
...
class DripForm(forms.ModelForm):
    message_class = forms.ChoiceField(choices=((k, '%s (%s)' % (k, v)) for k, v in configured_message_classes().items()))
...
class DripAdmin(admin.ModelAdmin):
    ...
    def timeline(self, request, drip_id, into_past, into_future):
        ...
        for shifted_drip in drip.drip.walk(into_past=int(into_past), into_future=int(into_future) + 1):
            shifted_drip.prune()
            shifted_drips.append({'drip': shifted_drip, 
    ...
    def add_view(self, request, extra_context=None):
        return super(DripAdmin, self).add_view(request, extra_context=self.build_extra_context(extra_context))

    def change_view(self, request, object_id, extra_context=None):
        return super(DripAdmin, self).change_view(request, object_id, extra_context=self.build_extra_context(extra_context))

    def get_urls(self):
        from django.conf.urls import patterns, url
        urls = super(DripAdmin, self).get_urls()
        my_urls = patterns('', url('^(?P<drip_id>[\\d]+)/timeline/(?P<into_past>[\\d]+)/(?P<into_future>[\\d]+)/$', self.av(self.timeline), name='drip_timeline'), url('^(?P<drip_id>[\\d]+)/timeline/(?P<into_past>[\\d]+)/(?P<into_future>[\\d]+)/(?P<user_id>[\\d]+)/$', self.av(self.view_drip_email), name='view_drip_email'))
        return my_urls + urls
