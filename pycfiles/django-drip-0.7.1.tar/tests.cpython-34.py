...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanhelmig/Code/django-drip/drip/tests.py
# Compiled at: 2014-12-10 02:09:47
# Size of source mod 2**32: 17810 bytes
import six
...
class RulesTestCase(TestCase):
    ...
    def setUp(self):
        self.drip = Drip.objects.create(name='A Drip just for Rules', subject_template='Hello', body_html_template='KETTEHS ROCK!')
...
class DripsTestCase(TestCase):
    ...
    def build_joined_date_drip(self, shift_one=7, shift_two=8):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='lt', field_value='now-{0} days'.format(shift_one))
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='gte', field_value='now-{0} days'.format(shift_two))
        return model_drip
    ...
    def test_custom_drip(self):
        ...
        drip.send()
        self.assertEqual(2, SentDrip.objects.count())
        ...
        self.assertEqual(2, drip.get_queryset().count())
        drip.prune()
        self.assertEqual(0, drip.get_queryset().count())
    ...
    def test_custom_drip_with_count(self):
        model_drip = self.build_joined_date_drip()
        QuerySetRule.objects.create(drip=model_drip, field_name='profile__credits', lookup_type='gte', field_value='5')
        drip = model_drip.drip
    ...
    def test_exclude_and_include(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        QuerySetRule.objects.create(drip=model_drip, field_name='profile__credits', lookup_type='gte', field_value='1')
        QuerySetRule.objects.create(drip=model_drip, field_name='profile__credits', method_type='exclude', lookup_type='exact', field_value=100)
        QuerySetRule.objects.create(drip=model_drip, field_name='profile__credits', method_type='exclude', lookup_type='exact', field_value=125)
        self.assertEqual(7, model_drip.drip.get_queryset().count())

    def test_custom_drip_static_datetime(self):
        model_drip = self.build_joined_date_drip()
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='lte', field_value=(datetime.now() - timedelta(days=8)).strftime('%Y-%m-%d %H:%M:%S'))
        drip = model_drip.drip
    ...
    def test_custom_drip_static_now_datetime(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='gte', field_value=(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d 00:00:00'))
        drip = model_drip.drip
    ...
    def test_admin_timeline_prunes_user_output(self):
        ...
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='gte', field_value=(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d 00:00:00'))
        rf = RequestFactory()
        timeline_url = reverse('admin:drip_timeline', kwargs={'drip_id': model_drip.id, 
         'into_past': 3, 
         'into_future': 3})
    ...
    def test_annotated_field_name_property_no_count(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        qsr = QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='exact', field_value=2)
        self.assertEqual(qsr.annotated_field_name, 'date_joined')

    def test_annotated_field_name_property_with_count(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        qsr = QuerySetRule.objects.create(drip=model_drip, field_name='userprofile__user__groups__count', lookup_type='exact', field_value=2)
    ...
    def test_apply_annotations_with_count(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        qsr = QuerySetRule.objects.create(drip=model_drip, field_name='profile__user__groups__count', lookup_type='exact', field_value=2)
    ...
    def test_apply_multiple_rules_with_aggregation(self):
        model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='KETTEHS ROCK!')
        qsr = QuerySetRule.objects.create(drip=model_drip, field_name='profile__user__groups__count', lookup_type='exact', field_value='0')
        QuerySetRule.objects.create(drip=model_drip, field_name='date_joined', lookup_type='gte', field_value=(datetime.now() - timedelta(days=1)).strftime('%Y-%m-%d 00:00:00'))
        qsr.clean()
...
class CustomMessagesTest(TestCase):
    ...
    def setUp(self):
        ...
        self.model_drip = Drip.objects.create(name='A Custom Week Ago', subject_template='HELLO {{ user.username }}', body_html_template='<h2>This</h2> is an <b>example</b> html <strong>body</strong>.')
        QuerySetRule.objects.create(drip=self.model_drip, field_name='id', lookup_type='exact', field_value=self.user.id)
