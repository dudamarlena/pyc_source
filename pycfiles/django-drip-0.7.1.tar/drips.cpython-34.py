...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanhelmig/Code/django-drip/drip/drips.py
# Compiled at: 2014-12-10 01:58:41
# Size of source mod 2**32: 8092 bytes
import operator, six, functools
...
class DripBase(object):
    ...
    def prune(self):
        ...
        exclude_user_ids = SentDrip.objects.filter(date__lt=conditional_now(), drip=self.drip_model, user__id__in=target_user_ids).values_list('user_id', flat=True)
    ...
    def send(self):
        ...
        for user in self.get_queryset():
            ...
            try:
                ...
                if result:
                    SentDrip.objects.create(drip=self.drip_model, user=user, from_email=self.from_email, from_email_name=self.from_email_name, subject=message_instance.subject, body=message_instance.body)
                    count += 1
