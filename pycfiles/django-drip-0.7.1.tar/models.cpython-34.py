...
# [GCC 8.4.0]
# Embedded file name: /Users/bryanhelmig/Code/django-drip/drip/models.py
# Compiled at: 2014-12-10 01:47:36
...
class Drip(models.Model):
    ...
    name = models.CharField(max_length=255, unique=True, verbose_name='Drip Name', help_text='A unique name for this drip.')
    ...
    body_html_template = models.TextField(null=True, blank=True, help_text='You will have settings and user in the context.')
    message_class = models.CharField(max_length=120, blank=True, default='default')

    @property
    def drip(self):
        from drip.drips import DripBase
        drip = DripBase(drip_model=self, name=self.name, from_email=self.from_email if self.from_email else None, from_email_name=self.from_email_name if self.from_email_name else None, subject_template=self.subject_template if self.subject_template else None, body_template=self.body_html_template if self.body_html_template else None)
        return drip
...
METHOD_TYPES = (('filter', 'Filter'), ('exclude', 'Exclude'))
LOOKUP_TYPES = (('exact', 'exactly'), ('iexact', 'exactly (case insensitive)'), ('contains', 'contains'),
                ('icontains', 'contains (case insensitive)'), ('regex', 'regex'),
                ('iregex', 'contains (case insensitive)'), ('gt', 'greater than'),
                ('gte', 'greater than or equal to'), ('lt', 'less than'), ('lte', 'less than or equal to'),
                ('startswith', 'starts with'), ('endswith', 'starts with'), ('istartswith', 'ends with (case insensitive)'),
...
class QuerySetRule(models.Model):
    ...
    def clean(self):
        ...
        try:
            ...
            raise ValidationError('%s raised trying to apply rule: %s' % (type(e).__name__, e))
    ...
    def filter_kwargs(self, qs, now=datetime.now):
        ...
        if self.field_value.startswith('now-'):
            ...
            if self.field_value.startswith('now+'):
                ...
                if self.field_value.startswith('today-'):
