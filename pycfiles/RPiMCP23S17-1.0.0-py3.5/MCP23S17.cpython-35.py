...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/RPiMCP23S17/MCP23S17.py
# Compiled at: 2019-03-20 14:58:49
# Size of source mod 2**32: 10470 bytes
import time, RPi.GPIO as GPIO, spidev

class MCP23S17(object):
    __doc__ = 'This class provides an abstraction of the GPIO expander MCP23S17\n    for the Raspberry Pi.\n    It is depndent on the Python packages spidev and RPi.GPIO, which can\n    be get from https://pypi.python.org/pypi/RPi.GPIO/0.5.11 and\n    https://pypi.python.org/pypi/spidev.\n    '
    PULLUP_ENABLED = 0
    PULLUP_DISABLED = 1
    DIR_INPUT = 0
    DIR_OUTPUT = 1
    ...
    IOCON_BANK_MODE = 128
    IOCON_INIT = 40
    MCP23S17_CMD_WRITE = 64
    MCP23S17_CMD_READ = 65

    def __init__(self, bus=0, pin_cs=0, pin_reset=-1, device_id=0):
        """
        Constructor
    ...
    def setPullupMode(self, pin, mode):
        ...
        """
        assert pin < 16
        if not mode == MCP23S17.PULLUP_ENABLED:
            assert mode == MCP23S17.PULLUP_DISABLED
    ...
    def setDirection(self, pin, direction):
        ...
        """
        assert pin < 16
        if not direction == MCP23S17.DIR_INPUT:
            assert direction == MCP23S17.DIR_OUTPUT
            assert self.isInitialized
            if pin < 8:
    ...
    def digitalWrite(self, pin, level):
        ...
        assert self.isInitialized
        assert pin < 16
        if not level == MCP23S17.LEVEL_HIGH:
            assert level == MCP23S17.LEVEL_LOW
            if pin < 8:
    ...
    def _writeRegisterWord(self, register, data):
        assert self.isInitialized
        self._writeRegister(register, data & 255)
