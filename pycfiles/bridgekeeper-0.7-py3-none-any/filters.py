...
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/filters.py
# Compiled at: 2015-11-05 10:40:17
__doc__ = 'Functions for filtering :class:`Bridges <bridgedb.bridges.Bridge>`.'
import logging
...
def bySubring(hmac, assigned, total):
    ...
    """
    logging.debug('Creating a filter for assigning bridges to subhashring %s-of-%s...' % (
     assigned, total))
...
def byFilters(filtres):
    ...
    try:
        ...
        setattr(_byFilters, 'description', (' ').join([ getattr(f, 'description', '') for f in filtres ]))
        ...
        def _byNotBlockedIn(bridge):
            ...
            if methodname == 'vanilla':
                if bridge.address.version == ipVersion:
                    if not bridge.addressIsBlockedIn(countryCode, bridge.address, bridge.orPort):
                        return True
            else:
