...
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/https/server.py
...
import base64, gettext, logging, random, re, time, os
...
lookup = TemplateLookup(directories=[TEMPLATE_DIR], output_encoding='utf-8', filesystem_checks=False, collection_size=500)
...
class CSPResource(resource.Resource):
    ...
    def __init__(self, includeSelf=False, enabled=True, reportViolations=False, useForwardedHeader=False):
        u"""Create a new :api:`twisted.web.resource.Resource` which adds a
        ...
        self.csp = "default-src 'none'; base-uri {0}; script-src {0}; style-src {0}; img-src {0} data:; font-src {0}; "
        if includeSelf:
    ...
    def setCSPHeader(self, request):
        ...
        **request** (or a ``'Content-Security-Policy-Report-Only:'`` header,
        ...
        """
        self.fqdn = self.fqdn or getFQDN()
        if self.enabled and self.fqdn:
            if not self.reportViolations:
                request.setHeader('Content-Security-Policy', self.csp.format(self.fqdn))
            else:
                logging.debug('Sending report-only CSP header...')
                request.setHeader('Content-Security-Policy-Report-Only', self.csp.format(self.fqdn) + 'report-uri /%s' % self.reportURI)
...
class CaptchaProtectedResource(CustomErrorHandlingResource, CSPResource):
    ...
    def __init__(self, publicKey=None, secretKey=None, useForwardedHeader=False, protectedResource=None):
        CSPResource.__init__(self)
    ...
    def render_GET(self, request):
        ...
        try:
            ...
            rendered = template.render(strings, rtl=rtl, lang=langs[0], imgstr=imgstr, challenge_field=challenge)
        except Exception as err:
    ...
    def render_POST(self, request):
        ...
        if self.checkSolution(request) is True:
            ...
            return rendered
        logging.debug('Client failed a CAPTCHA; returning redirect to %s' % request.uri)
        return redirectTo(request.uri, request)
...
class GimpCaptchaProtectedResource(CaptchaProtectedResource):
    ...
    def checkSolution(self, request):
        ...
        try:
            valid = captcha.GimpCaptcha.check(challenge, solution, self.secretKey, clientHMACKey)
    ...
    def getCaptchaImage(self, request):
        ...
        clientIP = self.getClientIP(request)
        clientHMACKey = crypto.getHMAC(self.hmacKey, clientIP)
        capt = captcha.GimpCaptcha(self.publicKey, self.secretKey, clientHMACKey, self.captchaDir)
        try:
            capt.get()
...
class ReCaptchaProtectedResource(CaptchaProtectedResource):
    ...
    def _renderDeferred(self, checkedRequest):
        ...
        if valid is True:
            ...
            logging.info('Client failed a CAPTCHA; redirecting to %s' % request.uri)
    ...
    def getRemoteIP(self):
        ...
        if self.remoteIP:
            ...
            remoteIP = IPv4Address(random.randint(0, 4294967295)).compressed
        return remoteIP
    ...
    def checkSolution(self, request):
        ...
        d = txrecaptcha.submit(challenge, response, self.secretKey, remoteIP).addCallback(checkResponse, request)
        return d
...
class BridgesResource(CustomErrorHandlingResource, CSPResource):
    ...
    def __init__(self, distributor, schedule, N=1, useForwardedHeader=False, includeFingerprints=True):
    ...
    def getBridgeRequestAnswer(self, request):
        ...
        if ip:
            ...
            bridgeLines = [ replaceControlChars(bridge.getBridgeLine(bridgeRequest, self.includeFingerprints)) for bridge in bridges
                          ]
    ...
    def renderAnswer(self, request, bridgeLines=None):
        ...
        if format == 'plain':
            ...
            try:
                ...
                rendered = template.render(strings, rtl=rtl, lang=langs[0], answer=bridgeLines, qrcode=qrcode)
            except Exception as err:
                rendered = replaceErrorPage(request, err)
...
def addWebServer(config, distributor):
    ...
    csp = CSPResource(enabled=config.CSP_ENABLED, includeSelf=config.CSP_INCLUDE_SELF, reportViolations=config.CSP_REPORT_ONLY, useForwardedHeader=fwdHeaders)
    ...
    if config.RECAPTCHA_ENABLED:
        ...
        captcha = partial(ReCaptchaProtectedResource, remoteIP=config.RECAPTCHA_REMOTEIP)
        ...
        captcha = partial(GimpCaptchaProtectedResource, hmacKey=hmacKey, captchaDir=config.GIMP_CAPTCHA_DIR)
    if config.HTTPS_ROTATION_PERIOD:
        count, period = config.HTTPS_ROTATION_PERIOD.split()
        sched = ScheduledInterval(count, period)
    else:
        sched = Unscheduled()
    bridges = BridgesResource(distributor, sched, numBridges, fwdHeaders, includeFingerprints=fprInclude)
    if captcha:
        protected = captcha(publicKey=publicKey, secretKey=secretKey, useForwardedHeader=fwdHeaders, protectedResource=bridges)
    ...
    if config.HTTP_UNENCRYPTED_PORT:
    ...
    if config.HTTPS_PORT:
        ...
        try:
            ...
            factory = DefaultOpenSSLContextFactory(config.HTTPS_KEY_FILE, config.HTTPS_CERT_FILE)
