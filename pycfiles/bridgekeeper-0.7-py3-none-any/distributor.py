...
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/https/distributor.py
...
import ipaddr, logging, bridgedb.Storage
...
class HTTPSDistributor(Distributor):
    ...
    def prepopulateRings(self):
        u"""Prepopulate this distributor's hashrings and subhashrings with
        bridges.
        ...
        for filterFn in [byIPv4, byIPv6]:
            for subring in range(1, self.totalSubrings + 1):
                ...
                self.hashring.addRing(ring, filters, byFilters(filters), populate_from=self.hashring.bridges)
