...
class ICaptcha(Interface):
    ...
    image = Attribute('A string containing the contents of a CAPTCHA image file.')
    challenge = Attribute('A unique string associated with the dispursal of this CAPTCHA.')
    publicKey = Attribute('A public key used for encrypting CAPTCHA challenge strings.')
    secretKey = Attribute('A private key used for decrypting challenge strings during CAPTCHAsolution verification.')
...
class ReCaptcha(Captcha):
    ...
    def __init__(self, publicKey=None, secretKey=None):
        ...
        """
        super(ReCaptcha, self).__init__(publicKey=publicKey, secretKey=secretKey)
    ...
    def get(self):
        ...
        cField = soup.find('input', {'name': 'recaptcha_challenge_field'})
        self.challenge = str(cField['value'])
...
class GimpCaptcha(Captcha):
    ...
    def __init__(self, publicKey=None, secretKey=None, hmacKey=None, cacheDir=None):
        ...
        if not cacheDir or not os.path.isdir(cacheDir):
            raise GimpCaptchaError("Gimp captcha cache isn't a directory: %r" % cacheDir)
        if not (publicKey and secretKey and hmacKey):
            raise CaptchaKeyError('Invalid key supplied to GimpCaptcha: SK=%r PK=%r HMAC=%r' % (
             secretKey, publicKey, hmacKey))
        super(GimpCaptcha, self).__init__(publicKey=publicKey, secretKey=secretKey)
        ...
        try:
            ...
            if hmacIsValid:
                ...
                if now >= then:
                    ...
                    raise CaptchaExpired('Solution %r was for a CAPTCHA which already expired at %s.' % (
                     solution, exp))
    ...
    def get(self):
        ...
        try:
            ...
            raise GimpCaptchaError('CAPTCHA cache dir appears empty: %r' % self.cacheDir)
        except (OSError, IOError):
            raise GimpCaptchaError('Could not read Gimp captcha image file: %r' % imageFilename)

        self.answer = imageFilename.rsplit(os.path.extsep, 1)[0]
        self.challenge = self.createChallenge(self.answer)
        return (
         self.image, self.challenge)