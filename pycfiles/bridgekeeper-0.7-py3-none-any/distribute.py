...
class IDistribute(IName):
    ...
    _bridgesPerResponseMin = Attribute('The minimum number of bridges to distribute (if possible), per client request.')
    _bridgesPerResponseMax = Attribute('The maximum number of bridges to distribute (if possible), per client request.')
    _hashringLevelMin = Attribute('The bare minimum number of bridges which should be in a hashring. If there less bridges than this, then the implementer of IDistribute should only distribute _bridgesPerResponseMin number of bridges, per client request.')
    _hashringLevelMax = Attribute('The number of bridges which should be in a hashring for the implementer of IDistribute to distribute _bridgesPerResponseMax number of bridges, per client request.')
    hashring = Attribute('An implementer of ``bridgedb.hashring.IHashring`` which stores the entirety of bridges allocated to this ``Distributor`` by the BridgeDB.  This ``Distributor`` is only capable of distributing these bridges to its clients, and these bridges are only distributable by this ``Distributor``.')
    key = Attribute("A master key which is used to HMAC bridge and client data into this Distributor's **hashring** and its subhashrings.")
...
class Distributor(Named):
    ...
    def name(self, name):
        ...
        try:
            ...
            logging.debug("Couldn't set distributor attribute for %s Distributor's hashring." % name)
    ...
    def bridgesPerResponse(self, hashring=None):
        ...
        if len(hashring) < self._hashringLevelMin:
            ...
            n = int(math.ceil((self._bridgesPerResponseMin + self._bridgesPerResponseMax) / 2.0))
