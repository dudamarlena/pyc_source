...
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/proxy.py
# Compiled at: 2015-11-05 10:40:17
__doc__ = 'Classes for finding and managing lists of open proxies.'
from __future__ import print_function
from collections import MutableSet
from functools import update_wrapper
from functools import wraps
import ipaddr, logging, os, time
...
class ProxySet(MutableSet):
    ...
    def getAllWithTag(self, tag):
        ...
        :returns: A set of all proxies which are contained within this
            ...
            **tag**.
        """
        return set([ key for key, value in filter(lambda x: x[1] == tag, self._proxydict.items())
                   ])
    ...
    def __len__(self):
        return len(self._proxydict.items())

    def __repr__(self):
        return type('')(self.proxies)

    def __str__(self):
        return os.linesep.join(self.proxies)
    ...
    def add(self, ip, tag=None):
        return self.__add__(ip, tag)

    def copy(self):
        return self.__class__(self._proxydict.copy())

    def contains(self, ip):
        return self.__contains__(ip)

    def discard(self, ip):
        return self.__sub__(ip)
    ...
    def difference(self, other):
        return self._proxies.difference(other)

    def issubset(self, other):
        return self._proxies.issubset(other)

    def issuperset(self, other):
        return self._proxies.issuperset(other)

    def intersection(self, other):
        return self._proxies.intersection(other)

    def symmetric_difference(self, other):
        return self._proxies.symmetric_difference(other)

    def union(self, other):
        return self._proxies.union(other)
...
class ExitListProtocol(protocol.ProcessProtocol):
    ...
    def parseData(self):
        ...
        if unparseable:
            logging.warn('There were unparseable lines in the downloaded list of Tor exit relays: %r' % unparseable)
    ...
    def processEnded(self, reason):
        ...
        if reason.value.exitCode != 0:
            ...
            logging.error('There was an error downloading Tor exit list: %s' % reason.value)
        else:
