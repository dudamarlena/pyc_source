...
# Embedded file name: build/bdist.linux-x86_64/egg/bridgedb/configure.py
# Compiled at: 2015-11-05 10:40:17
__doc__ = 'Utilities for dealing with configuration files for BridgeDB.'
import logging, os
from bridgedb import strings

def loadConfig(configFile=None, configCls=None):
    u"""Load configuration settings on top of the current settings.
    ...
    **Note:**
    ...
    if itsSafeToUseLogging:
        ...
        logging.debug(('\n').join([ ('{0} = {1}').format(key, value) for key, value in configuration.items() if not key.startswith('_')
                                  ]))
    ...
    for attr in ['PROXY_LIST_FILES', 'BRIDGE_FILES', 'EXTRA_INFO_FILES']:
        ...
        if setting is None:
            ...
            setattr(config, attr, [ os.path.abspath(os.path.expanduser(f)) for f in setting ])
    ...
    logging.info('Currently supported transports: %s' % (' ').join(strings._getSupportedTransports()))
