...
# Embedded file name: /home/travis/build/pkgcore/pychroot/build/lib/pychroot/utils.py
# Compiled at: 2019-12-01 01:06:22
# Size of source mod 2**32: 3825 bytes
__doc__ = 'Various chroot-related utilities mostly dealing with bind mounting.'
import errno
...
def getlogger(log, name):
    ...
    if isinstance(log, logging.Logger):
        if not log.name.startswith(name.partition('.')[0]):
            return log
...
def bind(src, dest, chroot, create=False, log=None, readonly=False, recursive=False, **_kwargs):
    ...
    if create:
        ...
        if not os.path.isdir(src) and src not in fstypes:
            try:
                ...
                raise ChrootMountError(f"cannot bind mount to {dest!r}", getattr(e, 'errno', None))
    ...
    try:
        mount(source=src, target=dest, fstype=fstype, flags=(reduce(operator.or_, mount_flags, 0)),
        ...
        raise ChrootMountError(f"failed mounting: mount -t {fstype} {src} {dest}", e.errno)