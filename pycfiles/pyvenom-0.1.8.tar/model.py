...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/model/model.py
...
__all__ = [
 'Model', 'MetaModel', 'PropertySchema', 'ModelSchema']
...
class PropertySchema(object):
    ...
    def __eq__(self, value):
        return self.property.__equals__(value.property) and self.search == value.search and self.datastore == value.datastore and self.indexed_datastore == value.indexed_datastore

    def __repr__(self):
        return ('PropertySchema({}, datastore={}, search={}, indexed_datastore={})').format(self.property, self.datastore, self.search, self.indexed_datastore)
...
class ModelSchema(dict):
    ...
    def _build_schema(self, properties, queries):
        schema = {name:PropertySchema(prop, datastore=True) for name, prop in properties.items()}
    ...
    def to_table(self):
        ...
        for name, property_schema in self.items():
            doc += template.format(name, property_schema.datastore, property_schema.indexed_datastore, property_schema.search)
...
class Model(object):
    ...
    def _to_route_parameters(cls):
        return {key:prop.to_route_parameter() for key, prop in cls._properties.items()}
    ...
    def __json__(self):
        json = {key:prop._get_value(self) for key, prop in self._properties.items() if not prop.hidden}
        json['key'] = self.key
        return json
