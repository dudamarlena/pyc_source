...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/internal/hybrid_model.py
...
__all__ = [
 'DynamicModel', 'HybridModel', 'MetaHybridModel', 'HybridSearchDocument', 'HybridDatastoreEntity', 'HybridPutManager']
...
class HybridSearchDocument(object):
    ...
    def has_diff(self, fields):
        ...
        if provided_fields.keys() != document_fields.keys():
            return True
        for key, provided_field in provided_fields.items():
            document_field = document_fields[key]
            if not document_field.value == provided_field.value:
                return True
    ...
    def _put_search_documents(self):
        ...
        for hybrid in self.hybrids:
            if hybrid.document_has_diff():
                ...
                if hybrid.kind not in search_indexes:
                    search_indexes[hybrid.kind] = {'index': hybrid.index, 'documents': [], 'hybrids': []}
...
class HybridModel(object):
    ...
    def entity_key(self):
        if self.search_document.document_id:
            ...
            if self.datastore_entity.entity_key:
    ...
    def _get_datastore_properties(self):
        return [ DatastorePropertyContainer(property, name, value) for name, (value, property) in self._datastore_properties.items()
               ]
    ...
    def get_multi(cls, entity_keys_or_document_ids):
        to_grab = [ entity_key_or_document_id if isinstance(entity_key_or_document_id, ndb.Key) else cls._document_id_to_key(entity_key_or_document_id) for entity_key_or_document_id in entity_keys_or_document_ids
                  ]
        grabbed = ndb.get_multi(to_grab)
        return [ cls(entity=entity) if entity else None for entity in grabbed
               ]
