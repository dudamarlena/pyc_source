...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/model/query.py
...
class QueryArgumentList(list):
    ...
    def apply(self, *args, **kwargs):
        ...
        for key in kwargs:
            if key not in query_kwarg_keys:
...
class PropertyComparison(QueryComponent):
    ...
    def to_query_arguments(self):
        ...
        if inspect.isclass(self.value) and issubclass(self.value, QueryParameter):
            return QueryArgumentList([QueryArgument(self.property._name, optional_key=True)])
    ...
    def to_datastore_query(self, args):
        ...
        if isinstance(self.value, QueryParameter):
            ...
            if self.operator == self.LT:
                return prop < value
            if self.operator == self.LE:
                return prop <= value
            if self.operator == self.GT:
                return prop > value
            if self.operator == self.GE:
                return prop >= value
...
class QueryLogicalOperator(QueryComponent):
    ...
    def to_datastore_query(self, args):
        ...
        if not self.components:
            ...
            return self.datastore_conjuntion(*map(lambda component: component.to_datastore_query(args), self.components))
