...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/routing/Protocols.py
# Compiled at: 2016-04-26 14:29:46
import traceback, json
__all__ = [
 'Protocol', 'TextProtocol', 'JSONProtocol']
    ...
    def _exit_error(self, exception_type, exception_value, exception_traceback):
        ...
        try:
            if exception_type in self.errors:
                self._write({'message': str(exception_value), 
                   'success': False, 
                   'code': self.errors[exception_type]})
            else:
                traceback.print_exc()
                self._write({'message': 'An unknown error occured. Check the server logs for more information', 
                   'success': False, 
                   'code': -1})
        except Exception:
            traceback.print_exc()
