...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/model/Properties.py
...
class Property(ModelAttribute):
    ...
    def __equals__(self, value):
        ...
        return self._name == value._name and self._model is value._model
    ...
    def _validate_required(self, value):
        if self.required and value == None:
            raise PropertyValidationFailed(("'{}' property was set to None, but is required").format(self._code_name))
        return
    ...
    def _validate_types(self, value):
        if value != None and len(self.allowed_types) > 0:
            for allowed_type in self.allowed_types:
                ...
                raise PropertyValidationFailed(("{} property was set to {} of type '{}' when the only allowed types are {}").format(self._code_name, value, type(value).__name__, map(lambda typ: typ.__name__, self.allowed_types)))
    ...
    def _validate_unique(self, entity, value):
        ...
        for result in results:
            if result.key != entity.key:
                raise PropertyValidationFailed(('{0} most contain a unique value but another entity already contains {0} == {1!r}').format(self._code_name, value))
    ...
    def _validate_choices(self, value):
        if self.choices == None:
            ...
            if value not in self.choices:
                raise PropertyValidationFailed(("'{}' field must be one of {} but instead it was '{}'").format(self._code_name, self.choices, value))
            return
...
class Integer(ChoicesProperty):
    ...
    def _validate_min(self, value):
        if self.min == None:
            ...
            if value < self.min:
                raise PropertyValidationFailed(('{} property must be at least {} but was {}').format(self._code_name, self.min, value))
            return
    ...
    def _validate_max(self, value):
        if self.max == None:
            ...
            if value > self.max:
                raise PropertyValidationFailed(('{} property must be at most {} but was {}').format(self._code_name, self.max, value))
            return
    ...
    def to_route_parameter(self):
        return Parameters.Integer(required=self.required, choices=self.choices, min=self.min, max=self.max)
...
class Float(Integer):
    ...
    def to_route_parameter(self):
        return Parameters.Float(required=self.required, choices=self.choices, min=self.min, max=self.max)
...
class String(ChoicesProperty):
    ...
    def _validate_min(self, value):
        if self.min == None:
            ...
            if len(value) < self.min:
                raise PropertyValidationFailed(("{} property requires at least {} characters but was provided '{}' of length {}").format(self._code_name, self.min, value, len(value)))
            return
    ...
    def _validate_max(self, value):
        if self.max == None:
            ...
            if len(value) > self.max:
                raise PropertyValidationFailed(("{} property requires at most {} characters but was provided '{}' of length {}").format(self._code_name, self.max, value, len(value)))
            return
    ...
    def _validate_characters(self, value):
        if self.characters == None:
            ...
            if len(difference) > 0:
                raise PropertyValidationFailed(("{} property can only contain characters from '{}' but found characters from '{}'").format(self._code_name, ('').join(self.characters), ('').join(difference)))
            return
    ...
    def to_route_parameter(self):
        return Parameters.String(required=self.required, choices=self.choices, min=self.min, max=self.max, characters=self.characters)
