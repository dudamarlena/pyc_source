...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/internal/index_yaml.py
# Compiled at: 2016-04-26 14:29:46
import os, yaml as pyyaml
from builtin_file import bfile
__all__ = [
 'IndexYaml', 'IndexYamlFromFile', 'IndexGenerator']
...
class IndexYaml(object):
    ...
    def __init__(self, yaml):
        ...
        if not yaml:
            yaml = {'indexes': []}
        elif 'indexes' not in yaml or not yaml['indexes']:
            yaml['indexes'] = []
    ...
    def _clean_yaml(self, yaml):
        return [ model for model in yaml['indexes'] if model['properties']
               ]
    ...
    def _validate_property(self, prop):
        ...
        if 'direction' in prop:
            ...
            if prop['direction'] not in {'asc', 'desc'}:
...
class IndexYamlFromFile(IndexYaml):
    ...
    def __str__(self):
        string = ('{}\n\n{}{}\n{}').format(self.manual, self.venom_marker, self.venom_info, super(IndexYamlFromFile, self).__str__())
...
class IndexGenerator(object):
    ...
    def __init__(self, yaml=None, schemas=None):
        ...
        self.index = {model['kind']:model for model in self.yaml['indexes']}
    ...
    def _get_properties_from_schema(self, schema):
        return [ {'name': name} for name, prop_schema in schema.items() if prop_schema.datastore and prop_schema.indexed_datastore
               ]
