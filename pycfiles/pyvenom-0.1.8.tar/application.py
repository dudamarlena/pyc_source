...
# [GCC 8.4.0]
# Embedded file name: /Users/Larco/Documents/Github/pyvenom/framework/venom/routing/application.py
# Compiled at: 2016-04-26 14:47:51
from collections import defaultdict
import inspect, routes
        ...
        def serve(self):
            ...
            for route in app.routes:
                ...
                if route.matches_path(path):
                    ...
                    meta['routes'].append({'headers': self._removenull(dict(route._headers)['template']), 
        ...
        def serve(self):
            ...
            for route in app.routes:
                if not route.path.startswith('/api/'):
                    continue
                returned_routes.append({'path': route.path, 
                   'methods': list(route.allowed_methods), 
                   'ui.guid': ui.get_guid(route)})

            return {'routes': returned_routes, 
               'version': app.version}
...
class _RoutesShortHand(WSGIEntryPoint):
    ...
    def CRUD(self, base_path, model, protocol=None):
        ...
        """
                ...
                if not query_name or query_name not in model._queries:
                    query_name = 'all'
                query = model._queries[query_name]
                query_kwargs = {key:value for key, value in self.query.items() if key != 'query' if key != 'query'}
                return {'entities': query(**query_kwargs), 
                   'query': query_name, 
                   'type': model.kind}
        ...
        self._add_route(base_path, BaseHandler, protocol, routes.GET).query({'query': Parameters.String(required=False)})
        ...
        """
                entity = self.url.get('entity')
                entity.populate(**{key:value for key, value in self.body.items() if value != None if value != None})
                return entity.save()
...
class Application(_RoutesShortHand):
    ...
    default_errors = {Parameters.ParameterCastingFailed: 1000, 
