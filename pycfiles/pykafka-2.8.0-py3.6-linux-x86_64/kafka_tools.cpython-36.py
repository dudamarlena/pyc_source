...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/cli/kafka_tools.py
# Compiled at: 2018-08-21 19:08:14
# Size of source mod 2**32: 17005 bytes
from __future__ import print_function
import argparse, calendar, datetime as dt, sys, time
from pkg_resources import parse_version
import tabulate, pykafka
...
def consume_topic(client, args):
    ...
    consumer = topic.get_simple_consumer('pykafka_cli', consumer_timeout_ms=100,
...
def desc_topic(client, args):
    ...
    print(tabulate.tabulate([(p.id, p.leader.id,
     [r.id for r in p.replicas], [r.id for r in p.isr]) for p in topic.partitions.values()],
      headers=[
...
def print_managed_consumer_groups(client, args):
    ...
    print(tabulate.tabulate([(group_id, x.state, x.protocol, x.protocol_type) for group_id, x in iteritems(consumer_groups)],
      headers=[
     'GroupId', 'State', 'Protocol', 'ProtocolType']))
...
def print_consumer_lag(client, args):
    ...
    lag_info = [(k, '{:,}'.format(v[0] - v[1]), v[0], v[1]) for k, v in iteritems(lag_info)]
    print(tabulate.tabulate(lag_info,
      headers=[
...
def print_offsets(client, args):
    ...
    print(tabulate.tabulate([(k, v.offset[0]) for k, v in iteritems(offsets)], headers=[
     'Partition', 'Offset'],
      numalign='center'))
...
def print_topics(client, args):
    ...
    :type client:  :class:`pykafka.KafkaClient`
    """
    print(tabulate.tabulate([(t.name, len(t.partitions), len(list(t.partitions.values())[0].replicas) - 1) for t in client.topics.values()],
      headers=[
...
def reset_offsets(client, args):
    ...
    reqs = [PartitionOffsetCommitRequest(topic.name, partition_id, res.offset[0], tmsp, b'kafka-tools') for partition_id, res in iteritems(offsets)]
    broker = client.cluster.get_group_coordinator(args.consumer_group)
    broker.commit_consumer_group_offsets(args.consumer_group, 1, b'kafka-tools', reqs)


def create_topic(client, args):
    if parse_version(args.broker_version) < parse_version('0.10.1'):
        raise ValueError('The topic creation API is not usable on brokers older than 0.10.1. Use --broker_version to specify the version')
    topic_req = CreateTopicRequest(args.topic, args.num_partitions, args.replication_factor, args.replica_assignment or [], args.config_entries or [])
    client.cluster.controller_broker.create_topics([topic_req], args.timeout)


def delete_topic(client, args):
    if parse_version(args.broker_version) < parse_version('0.10.1'):
        raise ValueError('The topic deletion API is not usable on brokers older than 0.10.1. Use --broker_version to specify the version')
...
def _add_consumer_group(parser):
    ...
    parser.add_argument('consumer_group', metavar='CONSUMER_GROUP',
...
def _add_limit(parser):
    ...
    parser.add_argument('-l', '--limit', help='Number of messages to consume (default: %(default)s)',
      type=int,
      default=10)


def _add_timeout(parser):
    parser.add_argument('-t', '--timeout', help='Time in ms to wait for the operation to complete(default: %(default)s)',
      type=int,
      default=5000)
...
def _add_offset(parser):
    ...
    parser.add_argument('offset', metavar='OFFSET',
      type=str,
      help='Offset to fetch. Can be EARLIEST, LATEST, or a datetime in the format YYYY-MM-DDTHH:MM:SS.')
...
def _get_arg_parser():
    ...
    output.add_argument('-b', '--broker', required=False,
      default='localhost:9092',
      dest='host',
      help='host:port of any Kafka broker. [default: localhost:9092]')
    output.add_argument('-o', '--broker_version', required=False,
      default='0.9.0',
      dest='broker_version',
    ...
    parser = subparsers.add_parser('consume_topic',
      help='Dump messages for a topic to a file or stdout.')
    parser.set_defaults(func=consume_topic)
    _add_topic(parser)
    _add_limit(parser)
    _add_outfile(parser)
    parser = subparsers.add_parser('desc_topic',
      help='Print detailed info for a topic.')
    parser.set_defaults(func=desc_topic)
    _add_topic(parser)
    parser = subparsers.add_parser('print_managed_consumer_groups',
      help='Get consumer groups for a topic')
    parser.set_defaults(func=print_managed_consumer_groups)
    _add_topic(parser)
    parser = subparsers.add_parser('print_consumer_lag',
      help='Get consumer lag for a topic.')
    parser.set_defaults(func=print_consumer_lag)
    _add_topic(parser)
    _add_consumer_group(parser)
    parser = subparsers.add_parser('print_offsets',
      help='Fetch broker offsets for a topic')
    parser.set_defaults(func=print_offsets)
    _add_topic(parser)
    _add_offset(parser)
    parser = subparsers.add_parser('print_topics',
      help='Print information about all topics in the cluster.')
    parser.set_defaults(func=print_topics)
    parser = subparsers.add_parser('reset_offsets',
      help='Reset offsets for a topic/consumer group')
    parser.set_defaults(func=reset_offsets)
    _add_topic(parser)
    _add_consumer_group(parser)
    _add_offset(parser)
    parser = subparsers.add_parser('create_topic',
      help='Create a topic')
    parser.set_defaults(func=create_topic)
    _add_topic(parser)
    _add_timeout(parser)
    parser.add_argument('-p', '--num_partitions', help='Number of partitions to be created. -1 indicates unset. (default: %(default)s)',
      type=int,
      default=1)
    parser.add_argument('-r', '--replication_factor', help='Replication factor for the topic. -1 indicates unset. (default: %(default)s)',
      type=int,
      default=1)
    parser.add_argument('-a', '--replica_assignment', help='Replica assignment among kafka brokers for this topic partitions. If this is set num_partitions and replication_factor must be unset. Represent as a JSON object with partition IDs as keys as lists of node IDs as values',
      type=_encode_utf8)
    parser.add_argument('-c', '--config_entries', help='Topic level configuration for topic to be set. Represent as a JSON object.',
      type=_encode_utf8)
    parser = subparsers.add_parser('delete_topic',
      help='Delete a topic')
