...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/simpleconsumer.py
# Compiled at: 2018-09-17 17:43:07
# Size of source mod 2**32: 46143 bytes
...
import datetime as dt, itertools, logging, json, socket, sys, threading, time
from collections import defaultdict
import weakref
from six import reraise
from .common import OffsetType, EPOCH
from .utils.compat import Queue, Empty, iteritems, itervalues, range, iterkeys, get_bytes, get_string
from .exceptions import UnknownError, OffsetOutOfRangeError, UnknownTopicOrPartition, OffsetMetadataTooLarge, GroupLoadInProgress, NotCoordinatorForGroup, SocketDisconnectedError, ConsumerStoppedException, KafkaException, NotLeaderForPartition, OffsetRequestFailedError, RequestTimedOut, UnknownMemberId, RebalanceInProgress, IllegalGeneration, ERROR_CODES, UnicodeException
from .protocol import PartitionFetchRequest, PartitionOffsetCommitRequest, PartitionOffsetFetchRequest, PartitionOffsetRequest
from .utils.error_handlers import handle_partition_responses, raise_error, build_parts_by_error, valid_int
...
class SimpleConsumer(object):
    ...
    def __init__(self, topic, cluster, consumer_group=None, partitions=None, fetch_message_max_bytes=1048576, num_consumer_fetchers=1, auto_commit_enable=False, auto_commit_interval_ms=60000, queued_max_messages=2000, fetch_min_bytes=1, fetch_error_backoff_ms=500, fetch_wait_max_ms=100, offsets_channel_backoff_ms=1000, offsets_commit_max_retries=5, auto_offset_reset=OffsetType.EARLIEST, consumer_timeout_ms=-1, auto_start=True, reset_offset_on_start=False, compacted_topic=False, generation_id=-1, consumer_id=b'', deserializer=None, reset_offset_on_fetch=True):
        ...
        if consumer_group:
            try:
                ...
                raise UnicodeException("Consumer group name '{}' contains non-ascii characters".format(consumer_group))
            ...
            self._consumer_timeout_ms = valid_int(consumer_timeout_ms, allow_zero=True,
              allow_negative=True)
            self._offsets_channel_backoff_ms = valid_int(offsets_channel_backoff_ms)
            self._auto_offset_reset = auto_offset_reset
            offsets_commit_max_retries = valid_int(offsets_commit_max_retries, allow_zero=True)
            ...
            if partitions is not None:
                self._partitions = {p:OwnedPartition(p, self._cluster.handler, self._messages_arrived, self._is_compacted_topic) for p in partitions}
            else:
                self._partitions = {topic.partitions[k]:OwnedPartition(p, self._cluster.handler, self._messages_arrived, self._is_compacted_topic) for k, p in iteritems(topic.partitions)}
        self._partitions_by_id = {p.partition.id:p for p in itervalues(self._partitions)}
    ...
    def generation_id(self, value):
        self._generation_id = valid_int(value, allow_zero=True, allow_negative=True)

    def __repr__(self):
        return '<{module}.{name} at {id_} (consumer_group={group})>'.format(module=(self.__class__.__module__),
    ...
    def _build_default_error_handlers(self):
        ...
        def _handle_OffsetOutOfRangeError(parts):
            ...
            self.reset_offsets(partition_offsets=[(owned_partition.partition, self._auto_offset_reset) for owned_partition, pres in parts])
        ...
        return {UnknownTopicOrPartition.ERROR_CODE: lambda p: raise_error(UnknownTopicOrPartition), 
    ...
    def partitions(self):
        ...
        return {id_:partition.partition for id_, partition in iteritems(self._partitions_by_id)}

    @property
    def held_offsets(self):
        """Return a map from partition id to held offset for each partition"""
        return {p.partition.id:OffsetType.EARLIEST if p.last_offset_consumed == -1 else p.last_offset_consumed for p in itervalues(self._partitions_by_id)}
    ...
    def _setup_fetch_workers(self):
        ...
        return [self._cluster.handler.spawn(fetcher, name='pykafka.SimpleConsumer.fetcher') for i in range(self._num_consumer_fetchers)]
    ...
    def consume(self, block=True, unblock_event=None):
        ...
        if any(op.message_count <= self._queued_max_messages for op in itervalues(self._partitions)):
            if not self._slot_available.is_set():
                self._slot_available.set()
        if self._deserializer is not None:
            ret.value, ret.partition_key = self._deserializer(ret.value, ret.partition_key)
        return ret
    ...
    def _auto_commit(self):
        ...
        if (time.time() - self._last_auto_commit) * 1000.0 >= self._auto_commit_interval_ms:
            log.debug('Autocommitting consumer offset for consumer group %s and topic %s', self._consumer_group, self._topic.name)
            if self._consumer_group is not None:
                self.commit_offsets()
            self._last_auto_commit = time.time()
    ...
    def commit_offsets(self, partition_offsets=None):
        ...
        if not self._consumer_group:
            ...
            try:
                owned_partition_offsets = {self._partitions[p]:offset for p, offset in partition_offsets}
            except KeyError as e:
                raise KafkaException('Unknown partition supplied to commit_offsets\n%s', e)

        reqs = [p.build_offset_commit_request(offset=o) for p, o in iteritems(owned_partition_offsets)]
        log.debug('Committing offsets for %d partitions to broker id %s', len(reqs), self._group_coordinator.id)
        for i in range(self._offsets_commit_max_retries):
            if i > 0:
                ...
                try:
                    response = self._group_coordinator.commit_consumer_group_offsets(self._consumer_group, self._generation_id, self._consumer_id, reqs)
                except (SocketDisconnectedError, IOError):
                    log.error("Error committing offsets for topic '%s' from consumer id '%s'(SocketDisconnectedError)", self._topic.name, self._consumer_id)
                    if i >= self._offsets_commit_max_retries - 1:
                        raise
                    self._update()
                    continue

                parts_by_error = handle_partition_responses((self._default_error_handlers),
                  response=response,
                  partitions_by_id=(self._partitions_by_id))
                if len(parts_by_error) == 1 and 0 in parts_by_error or len(parts_by_error) == 0:
                    break
                log.error("Error committing offsets for topic '%s' from consumer id '%s'(errors: %s)", self._topic.name, self._consumer_id, {ERROR_CODES[err]:[op.partition.id for op, _ in parts] for err, parts in iteritems(parts_by_error)})
                if 0 in parts_by_error:
                    parts_by_error.pop(0)
            errored_partitions = [op for code, err_group in iteritems(parts_by_error) for op, res in err_group]
            reqs = [op.build_offset_commit_request(offset=(owned_partition_offsets[op])) for op in errored_partitions]
    ...
    def fetch_offsets(self):
        ...
        def _handle_success(parts):
            ...
            for owned_partition, pres in parts:
                if pres.offset == -1:
                    log.debug('Partition %s has no committed offsets in consumer group %s.  Resetting to %s', owned_partition.partition.id, self._consumer_group, self._auto_offset_reset)
                    partition_offsets_to_reset.append((
                     owned_partition.partition,
                     self._auto_offset_reset))
                else:
                    log.debug('Set offset for partition %s to %s', owned_partition.partition.id, pres.offset - 1)
        ...
        success_responses = []
        log.debug('Fetching offsets for %d partitions from broker id %s', len(reqs), self._group_coordinator.id)
        for i in range(self._offsets_fetch_max_retries):
            ...
            parts_by_error = handle_partition_responses((self._default_error_handlers),
              ...
              partitions_by_id=(self._partitions_by_id))
            success_responses.extend([(op.partition.id, r) for op, r in parts_by_error.get(0, [])])
            if len(reqs) == 0 or len(parts_by_error) == 1 and 0 in parts_by_error:
                return success_responses
            log.error("Error fetching offsets for topic '%s' (errors: %s)", self._topic.name, {ERROR_CODES[err]:[op.partition.id for op, _ in parts] for err, parts in iteritems(parts_by_error)})
            self._cluster.handler.sleep(i * (self._offsets_channel_backoff_ms / 1000))
    ...
    def reset_offsets(self, partition_offsets=None):
        ...
        def _handle_success(parts):
            for owned_partition, pres in parts:
                if len(pres.offset) > 0:
                    ...
                    log.warning('Partition {id_}: no offsets available before {offset}.Defaulting to OffsetType.EARLIEST.'.format(id_=(owned_partition.partition.id),
                      offset=(owned_partition_timestamps[owned_partition])))
                    owned_partition_offsets[owned_partition] = OffsetType.EARLIEST

        if partition_offsets is None:
            partition_offsets = [(a, self._auto_offset_reset) for a in self._partitions.keys()]
        try:
            owned_partition_offsets = {self._partitions[p]:offset for p, offset in partition_offsets}
        except KeyError as e:
        ...
        owned_partition_timestamps = {op:timestamp for op, timestamp in iteritems(owned_partition_offsets) if isinstance(timestamp, dt.datetime) or timestamp in MAGIC_OFFSETS if isinstance(timestamp, dt.datetime) or timestamp in MAGIC_OFFSETS}
        if owned_partition_timestamps:
            for i in range(self._offsets_reset_max_retries):
                ...
                for broker, timestamps in iteritems(by_leader):
                    reqs = [owned_partition.build_offset_request(timestamp) for owned_partition, timestamp in timestamps]
                    ...
                    if not parts_by_error:
                        ...
                        log.error("Error in ListOffsetRequest for topic '%s' (errors: %s)", self._topic.name, {ERROR_CODES[err]:[op.partition.id for op, _ in parts] for err, parts in iteritems(parts_by_error)})
                        self._cluster.handler.sleep(i * (self._offsets_channel_backoff_ms / 1000))
            ...
            if owned_partition_timestamps:
                raise OffsetRequestFailedError('Offset request failed after %d retries', self._offsets_reset_max_retries)
        ...
        for owned_partition, offset in sorted_offsets:
            if not isinstance(offset, int):
                raise ValueError("Invalid offset value encountered in reset_offsets:\n\tPartition {pid} got offset '{offset}'.".format(pid=(owned_partition.partition.id),
                  offset=offset))
        ...
        def _handle_success(parts):
            for owned_partition, pres in parts:
                if len(pres.messages) > 0:
                    log.debug('Fetched %s messages for partition %s', len(pres.messages), owned_partition.partition.id)
                    owned_partition.enqueue_messages(pres.messages)
                    log.debug('Partition %s queue holds %s messages', owned_partition.partition.id, owned_partition.message_count)
            ...
            for owned_partition in sorted_offsets:
                if owned_partition.fetch_lock.acquire(False):
                    ...
                    if owned_partition.message_count < self._queued_max_messages:
                        fetch_req = owned_partition.build_fetch_request(self._fetch_message_max_bytes)
                        partition_reqs[owned_partition] = fetch_req

            if partition_reqs:
                try:
                    response = broker.fetch_messages([a for a in itervalues(partition_reqs) if a],
                    ...
                    if self._running:
                        log.info('Updating cluster in response to error in fetch() for broker id %s', broker.id)
                        self._update()
                    return
                except socket.error:
                    raise ValueError("Failed to decode IO buffer. Ensure that the KafkaClient's broker_version kwarg matches the version of the Kafka cluster.")

                parts_by_error = build_parts_by_error(response, self._partitions_by_id)
                handle_partition_responses((self._default_error_handlers),
    ...
    def _wait_for_slot_available(self):
        ...
        if all(op.message_count >= self._queued_max_messages for op in itervalues(self._partitions)):
            for op in itervalues(self._partitions):
...
class OwnedPartition(object):
    __doc__ = 'A partition that is owned by a SimpleConsumer.\n\n    Used to keep track of offsets and the internal message queue.\n    '

    def __init__(self, partition, handler=None, semaphore=None, compacted_topic=False, consumer_id=b''):
        """
    ...
    def set_consumer_id(self, value):
        self._consumer_id = value
        self._offset_metadata = {'consumer_id':get_string(self._consumer_id), 
    ...
    def build_offset_request(self, offsets_before):
        ...
        if isinstance(offsets_before, dt.datetime):
            ...
            if offsets_before not in MAGIC_OFFSETS:
                raise ValueError('offsets_before is an invalid timestamp: {}'.format(offsets_before))
        return PartitionOffsetRequest(self.partition.topic.name, self.partition.id, offsets_before, 1)
    ...
    def build_fetch_request(self, max_bytes):
        ...
        :type max_bytes: int
        """
        return PartitionFetchRequest(self.partition.topic.name, self.partition.id, self.next_offset, max_bytes)
    ...
    def build_offset_commit_request(self, offset=None):
        ...
        :type offset: int
        """
        return PartitionOffsetCommitRequest(self.partition.topic.name, self.partition.id, offset if offset is not None else self.last_offset_consumed + 1, int(time.time() * 1000), get_bytes('{}'.format(self._offset_metadata_json)))

    def build_offset_fetch_request(self):
        """Create a PartitionOffsetFetchRequest for this partition
        """
        return PartitionOffsetFetchRequest(self.partition.topic.name, self.partition.id)
    ...
    def enqueue_messages(self, messages):
        ...
        for message in messages:
            if self._is_compacted_topic and message.offset < self.next_offset or not self._is_compacted_topic and message.offset != self.next_offset:
                log.debug('Skipping enqueue for offset (%s) not equal to next_offset (%s)', message.offset, self.next_offset)
            else:
                message.partition = self.partition
                if message.partition_id != self.partition.id:
                    log.error('Partition %s enqueued a message meant for partition %s', self.partition.id, message.partition_id)
