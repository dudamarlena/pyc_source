...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/topic.py
...
class Topic(object):
    ...
    def __repr__(self):
        return '<{module}.{classname} at {id_} (name={name})>'.format(module=(self.__class__.__module__),
    ...
    def fetch_offset_limits(self, offsets_before, max_offsets=1):
        ...
        for part in itervalues(self.partitions):
            requests[part.leader].append(PartitionOffsetRequest(self.name, part.id, offsets_before, max_offsets))
    ...
    def update(self, metadata):
        ...
        for id_, meta in iteritems(p_metas):
            ...
            if meta.id not in self._partitions:
                ...
                self._partitions[meta.id] = Partition(self, meta.id, brokers[meta.leader], [brokers[b] for b in meta.replicas if b in brokers], [brokers[b] for b in meta.isr])
            else:
                self._partitions[id_].update(brokers, meta)

    def get_simple_consumer(self, consumer_group=None, use_rdkafka=False, **kwargs):
        ...
        if GEventHandler:
            if isinstance(self._cluster.handler, GEventHandler):
                if use_rdkafka:
                    raise ImportError('use_rdkafka cannot be used with gevent')
        Cls = rdkafka.RdKafkaSimpleConsumer if (rdkafka and use_rdkafka) else SimpleConsumer
        return Cls(self,
 self._cluster, consumer_group=consumer_group, **kwargs)
    ...
    def get_balanced_consumer(self, consumer_group, managed=False, **kwargs):
        ...
        """
        if not managed:
            if 'zookeeper_connect' not in kwargs:
                if self._cluster._zookeeper_connect is not None:
