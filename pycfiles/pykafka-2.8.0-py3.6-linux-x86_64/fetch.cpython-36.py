...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/protocol/fetch.py
# Compiled at: 2018-07-25 17:55:59
# Size of source mod 2**32: 8485 bytes
import struct
...
class PartitionFetchRequest(_PartitionFetchRequest):
    __doc__ = 'Fetch request for a specific topic/partition\n\n    :ivar topic_name: Name of the topic to fetch from\n    :ivar partition_id: Id of the partition to fetch from\n    :ivar offset: Offset at which to start reading\n    :ivar max_bytes: Max bytes to read from this partition (default: 300kb)\n    '

    def __new__(cls, topic, partition, offset, max_bytes=1048576):
        return super(PartitionFetchRequest, cls).__new__(cls, topic, partition, offset, max_bytes)


class FetchRequest(Request):
    __doc__ = 'A Fetch request sent to Kafka\n\n    Specification::\n\n        FetchRequest => ReplicaId MaxWaitTime MinBytes [TopicName [Partition FetchOffset MaxBytes]]\n        \xa0\xa0ReplicaId => int32\n        \xa0\xa0MaxWaitTime => int32\n        \xa0\xa0MinBytes => int32\n        \xa0\xa0TopicName => string\n        \xa0\xa0Partition => int32\n        \xa0\xa0FetchOffset => int64\n        \xa0\xa0MaxBytes => int32\n    '
    API_KEY = 1
    ...
    def __init__(self, partition_requests=[], timeout=1000, min_bytes=1024, api_version=0):
    ...
    def get_bytes(self):
        ...
        struct.pack_into('!iiii', output, offset, -1, self.timeout, self.min_bytes, len(self._reqs))
        offset += 16
        for topic_name, partitions in iteritems(self._reqs):
            ...
            for partition_id, (fetch_offset, max_bytes) in iteritems(partitions):
                struct.pack_into('!iqi', output, offset, partition_id, fetch_offset, max_bytes)
                offset += 16

        return output


FetchPartitionResponse = namedtuple('FetchPartitionResponse', [
 'max_offset', 'messages', 'err'])

class FetchResponse(Response):
    __doc__ = 'Unpack a fetch response from the server\n\n    Specification::\n\n        FetchResponse => [TopicName [Partition ErrorCode HighwaterMarkOffset MessageSetSize MessageSet]]\n        \xa0\xa0TopicName => string\n        \xa0\xa0Partition => int32\n        \xa0\xa0ErrorCode => int16\n        \xa0\xa0HighwaterMarkOffset => int64\n        \xa0\xa0MessageSetSize => int32\n    '
    API_VERSION = 0
    API_KEY = 1
    ...
    def __init__(self, buff, offset=0, broker_version='0.9.0'):
        ...
        for topic, partitions in response:
            for partition in partitions:
                self.topics[topic][partition[0]] = FetchPartitionResponse(partition[2], self._unpack_message_set((partition[3]), partition_id=(partition[0]),
                  broker_version=broker_version), partition[1])
    ...
    def _unpack_message_set(self, buff, partition_id=-1, broker_version='0.9.0'):
        ...
        for message in message_set.messages:
            if message.compression_type == CompressionType.NONE:
                ...
                if message.compression_type == CompressionType.GZIP:
                    decompressed = compression.decode_gzip(message.value)
                    messages = self._unpack_message_set(decompressed, partition_id=partition_id)
                else:
                    if message.compression_type == CompressionType.SNAPPY:
                        decompressed = compression.decode_snappy(message.value)
                        messages = self._unpack_message_set(decompressed, partition_id=partition_id)
                    else:
                        if message.compression_type == CompressionType.LZ4:
...
class FetchResponseV1(FetchResponse):
    ...
    def __init__(self, buff, offset=0, broker_version='0.9.0'):
        ...
        super(FetchResponseV1, self).__init__(buff, (offset + 4), broker_version=broker_version)
