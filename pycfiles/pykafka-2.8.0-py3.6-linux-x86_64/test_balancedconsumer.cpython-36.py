...
class TestBalancedConsumer(unittest2.TestCase):
    ...
    @classmethod
    def buildMockConsumer(self, consumer_group=b'testgroup', num_partitions=10, num_participants=1, timeout=2000):
        topic = mock.Mock()
...
class BalancedConsumerIntegrationTests(unittest2.TestCase):
    ...
    def get_balanced_consumer(self, consumer_group, **kwargs):
        ...
        return (self.client.topics[self.topic_name].get_balanced_consumer)(consumer_group, managed=self.MANAGED_CONSUMER, **kwargs)
    ...
    def test_consume_earliest(self):
        try:
            consumer_a = self.get_balanced_consumer(b'test_consume_earliest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              use_rdkafka=(self.USE_RDKAFKA))
            consumer_b = self.get_balanced_consumer(b'test_consume_earliest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.EARLIEST),
              use_rdkafka=(self.USE_RDKAFKA))
    ...
    def test_consume_latest(self):
        try:
            consumer_a = self.get_balanced_consumer(b'test_consume_latest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.LATEST),
              use_rdkafka=(self.USE_RDKAFKA))
            consumer_b = self.get_balanced_consumer(b'test_consume_latest',
              zookeeper_connect=(self.kafka.zookeeper),
              auto_offset_reset=(OffsetType.LATEST),
              use_rdkafka=(self.USE_RDKAFKA))
            self.wait_for_rebalancing(consumer_a, consumer_b)
    ...
    def test_external_kazoo_client(self):
        ...
        zk.start()
        consumer = self.get_balanced_consumer(b'test_external_kazoo_client',
          zookeeper=zk,
    ...
    def test_no_partitions(self):
        ...
        consumer = self.get_balanced_consumer(b'test_no_partitions',
          zookeeper_connect=(self.kafka.zookeeper),
          auto_start=False,
