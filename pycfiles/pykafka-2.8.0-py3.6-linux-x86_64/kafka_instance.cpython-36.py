...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/test/kafka_instance.py
# Compiled at: 2018-07-24 15:14:34
# Size of source mod 2**32: 22886 bytes
"""
...
import argparse, errno, logging, os, signal, shutil, socket, subprocess, sys, tempfile, time
...
class KafkaInstance(ManagedInstance):
    __doc__ = 'A managed kafka instance for testing'

    def __init__(self, num_instances=1, kafka_version='0.8.2.1', scala_version='2.11', bin_dir='/tmp/kafka-bin', name='kafka', use_gevent=False):
        ...
        self._processes = []
        self._broker_procs = []
        self._brokers_started = 0
        self.zookeeper = None
        self.brokers = None
        self.brokers_ssl = None
        self.certs = self._gen_ssl_certs()
        ManagedInstance.__init__(self, name, use_gevent=use_gevent)
        self.connection = KafkaConnection(bin_dir, self.brokers, self.zookeeper, self.brokers_ssl)
    ...
    def _download_kafka(self):
        ...
        url = url_fmt.format(scala_version=(self._scala_version),
    ...
    def _start_log_watcher(self):
        ...
        for logfile in os.listdir(self._log_dir):
            ...
            if self.use_gevent:
                ...
                import threading
                watch_thread = threading.Thread(target=(self._watcher_threading),
                  args=[logfile])
    ...
    def _start_broker_proc(self, port, ssl_port=None):
        ...
        if self.certs is not None:
            port_config = _kafka_ssl_properties.format(port=port,
              ssl_port=ssl_port,
        ...
        conf = os.path.join(self._conf_dir, 'kafka_{instance}.properties'.format(instance=i))
        with open(conf, 'w') as (f):
            f.write(_kafka_properties.format(broker_id=i,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='kafka_instance',
      usage='foo bars',
      formatter_class=(argparse.ArgumentDefaultsHelpFormatter))
    parser.add_argument('num_brokers', type=int, default=3, help='Numbers of brokers in the cluster.')
    parser.add_argument('--download-dir', type=str, default='/tmp/kafka-bin', help='Download destination for Kafka')
    parser.add_argument('--kafka-version', type=str, default='0.8.2.1', help='Kafka version to download')
    parser.add_argument('--scala-version', type=str, default='2.11', help='Scala version for kafka build')
    parser.add_argument('--export-hosts', type=str, help='Write host strings to given file path')
    ...
    cluster = KafkaInstance(num_instances=(args.num_brokers), kafka_version=(args.kafka_version),
