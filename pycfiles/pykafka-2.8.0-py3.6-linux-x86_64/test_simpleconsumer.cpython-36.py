...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/pykafka/test_simpleconsumer.py
# Compiled at: 2018-09-17 15:34:06
# Size of source mod 2**32: 15711 bytes
from contextlib import contextmanager
import datetime as dt, json, mock, os, platform, pytest, time, threading, unittest2
...
class TestSimpleConsumer(unittest2.TestCase):
    ...
    def _get_simple_consumer(self, **kwargs):
        ...
        consumer = (topic.get_simple_consumer)(use_rdkafka=self.USE_RDKAFKA, **kwargs)
        try:
            yield consumer
        finally:
            consumer.stop()
    ...
    def _convert_offsets(offset_responses):
        ...
        return {partition_id:f1(f2(offset_response.offset)) for partition_id, offset_response in offset_responses}

    def test_offset_commit(self):
        """Check fetched offsets match pre-commit internal state"""
        with self._get_simple_consumer(consumer_group=b'test_offset_commit') as (consumer):
    ...
    def test_offset_resume(self):
        ...
        with self._get_simple_consumer(consumer_group=b'test_offset_resume') as (consumer):
            [consumer.consume() for _ in range(100)]
            offsets_committed = consumer.held_offsets
            consumer.commit_offsets()
        with self._get_simple_consumer(consumer_group=b'test_offset_resume') as (consumer):
    ...
    def test_reset_offset_on_start(self):
        ...
        with self._get_simple_consumer(auto_offset_reset=(OffsetType.EARLIEST),
          reset_offset_on_start=True) as (consumer):
            earliest_offs = self._convert_offsets(consumer.topic.earliest_available_offsets())
            self.assertEquals(earliest_offs, consumer.held_offsets)
            self.assertIsNotNone(consumer.consume())
        with self._get_simple_consumer(auto_offset_reset=(OffsetType.LATEST),
          reset_offset_on_start=True,
          consumer_timeout_ms=500) as (consumer):
            latest_offs = self._convert_offsets(consumer.topic.latest_available_offsets())
            self.assertEquals(latest_offs, consumer.held_offsets)
            self.assertIsNone(consumer.consume(block=False))
        difference = sum((latest_offs[i] - earliest_offs[i] if earliest_offs[i] >= 0 else latest_offs[i] + 1 if latest_offs[i] >= 0 else 0) for i in latest_offs)
    ...
    def test_reset_offsets_timestamp(self):
        ...
        with self._get_simple_consumer(auto_offset_reset=(OffsetType.EARLIEST)) as (consumer):
            part_id, latest_offset = next((p, res.offset[0]) for p, res in consumer.topic.latest_available_offsets().items() if res.offset[0] > 0)
            target_part = consumer.partitions[part_id]
            partition_offsets = {p:OffsetType.LATEST for p in consumer.partitions.values()}
    ...
    def test_reset_offsets(self):
        ...
        with self._get_simple_consumer(auto_offset_reset=(OffsetType.EARLIEST)) as (consumer):
            ...
            partition_offsets[target_part] = latest_offset + 5
            consumer.reset_offsets(partition_offsets.items())
            msg = consumer.consume()
    ...
    def test_update_cluster(self):
        ...
        with self._get_simple_consumer() as (consumer):
            ...
            t_start = time.time()
            timeout = 40.0 if self.USE_GEVENT else 20.0
            try:
                for broker in self.client.brokers.values():
                    while not broker._connection.connected:
                        time.sleep(0.1)
                        self.assertTrue((time.time() - t_start < timeout), msg='Broker reconnect failed.')
    ...
    def test_consumer_lag(self):
        ...
        with self._get_simple_consumer(consumer_group=b'test_lag_group', consumer_timeout_ms=1000) as (consumer):
            while 1:
            ...
            consumer.commit_offsets()
            latest_offsets = {p_id:res.offset[0] for p_id, res in iteritems(consumer.topic.latest_available_offsets())}
