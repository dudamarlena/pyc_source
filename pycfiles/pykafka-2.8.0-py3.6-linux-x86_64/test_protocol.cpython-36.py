...
class TestMetadataAPI(unittest2.TestCase):
    ...
    def test_request(self):
        ...
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00\x15\x00\x03\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\x00\x00\x00'))
    ...
    def test_response(self):
        ...
        self.assertEqual(cluster.topics[b'test'].partitions[0].leader, cluster.brokers[0].id)
...
class TestProduceAPI(unittest2.TestCase):
    ...
    test_messages = [
     ...
     protocol.Message(b'this is a test message', partition_key=b'asdf', timestamp=1497302164,
       protocol_version=1),
    ...
    def test_partition_error(self):
        response = protocol.ProduceResponse(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\x02'))
        self.assertEqual(response.topics[b'test'][0].err, 3)

    def test_response(self):
        response = protocol.ProduceResponse(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02'))
        self.assertEqual(response.topics, {b'test': {0: protocol.ProducePartitionResponse(0, 2)}})
...
class TestFetchAPI(unittest2.TestCase, _FetchAPITestBase):
    ...
    def test_gzip_decompression(self):
        ...
        for i in range(len(expected_data)):
            self.assertDictEqual(self.msg_to_dict(response.topics[b'test_gzip'][0].messages[i]), expected_data[i])
    ...
    def test_snappy_decompression(self):
        ...
        expected_data = self._get_expected()
        for i in range(len(expected_data)):
            self.assertDictEqual(self.msg_to_dict(response.topics[b'test_snappy'][0].messages[i]), expected_data[i])
...
class TestFetchAPIV1(unittest2.TestCase, _FetchAPITestBase):
    ...
    def test_gzip_decompression(self):
        ...
        expected_data = self._get_expected()
        for i in range(len(expected_data)):
            self.assertDictEqual(self.msg_to_dict(response.topics[b'test_gzip_5'][0].messages[i]), expected_data[i])
...
class TestListOffsetAPI(unittest2.TestCase):
    ...
    def test_partition_error(self):
        response = protocol.ListOffsetResponse(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x03\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x02'))
        self.assertEqual(response.topics[b'test'][0].err, 3)

    def test_response(self):
        resp = protocol.ListOffsetResponse(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x02'))
...
class TestListOffsetAPIV1(unittest2.TestCase):
    ...
    def test_partition_error(self):
        response = protocol.ListOffsetResponseV1(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x02'))
        self.assertEqual(response.topics[b'test'][0].err, 3)

    def test_response(self):
        resp = protocol.ListOffsetResponseV1(buffer(b'\x00\x00\x00\x01\x00\x04test\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x02'))
        self.assertEqual(resp.topics[b'test'][0].offset, [2])
...
class TestOffsetCommitFetchAPI(unittest2.TestCase):
    ...
    def test_consumer_metadata_request(self):
        ...
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00\x17\x00\n\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\x04test'))
    ...
    def test_offset_commit_request(self):
        preq = protocol.PartitionOffsetCommitRequest(b'test', 0, 68, 1426632066, b'testmetadata')
        req = protocol.OffsetCommitRequest(b'test',
          1, b'pykafka', partition_requests=[preq])
        msg = req.get_bytes()
    ...
    def test_offset_commit_response(self):
        response = protocol.OffsetCommitResponse(buffer(b'\x00\x00\x00\x01\x00\x0cemmett.dummy\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00'))
    ...
    def test_offset_fetch_request(self):
        ...
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00.\x00\t\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\x04test\x00\x00\x00\x01\x00\ttesttopic\x00\x00\x00\x01\x00\x00\x00\x00'))

    def test_offset_fetch_response(self):
        response = protocol.OffsetFetchResponse(buffer(b'\x00\x00\x00\x01\x00\x0cemmett.dummy\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00'))
...
class TestOffsetCommitFetchAPIV2(unittest2.TestCase):
    ...
    def test_offset_fetch_response(self):
        response = protocol.OffsetFetchResponseV2(buffer(b'\x00\x00\x00\x01\x00\x0cemmett.dummy\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00'))
        self.assertEqual(response.topics[b'emmett.dummy'][0].metadata, b'')
        self.assertEqual(response.topics[b'emmett.dummy'][0].offset, 1)
...
class TestGroupMembershipAPI(unittest2.TestCase):
    ...
    def test_consumer_group_protocol_metadata(self):
        ...
        msg = meta.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00\x00\x00\x01\x00\ndummytopic\x00\x00\x00\x0ctestuserdata'))
    ...
    def test_join_group_request(self):
        ...
        req = protocol.JoinGroupRequest(b'dummygroup', b'testmember', topic_name, membership_protocol)
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00h\x00\x0b\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\ndummygroup\x00\x00u0\x00\ntestmember\x00\x08consumer\x00\x00\x00\x01\x00\x05range\x00\x00\x00"\x00\x00\x00\x00\x00\x01\x00\nabcdefghij\x00\x00\x00\x0ctestuserdata'))
    ...
    def test_member_assignment_construction(self):
        ...
        self.assertEqual(msg, bytearray(b'\x00\x01\x00\x00\x00\x02\x00\x08mytopic1\x00\x00\x00\x04\x00\x00\x00\x03\x00\x00\x00\x05\x00\x00\x00\x07\x00\x00\x00\t\x00\x08mytopic2\x00\x00\x00\x04\x00\x00\x00\x02\x00\x00\x00\x04\x00\x00\x00\x06\x00\x00\x00\x08'))
    ...
    def test_heartbeat_request(self):
        ...
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00-\x00\x0c\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\ndummygroup\x00\x00\x00\x01\x00\ntestmember'))

    def test_heartbeat_response(self):
        response = protocol.HeartbeatResponse(bytearray(b'\x00\x00'))
        self.assertEqual(response.error_code, 0)

    def test_leave_group_request(self):
        req = protocol.LeaveGroupRequest(b'dummygroup', b'testmember')
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00)\x00\r\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\ndummygroup\x00\ntestmember'))

    def test_leave_group_response(self):
        response = protocol.LeaveGroupResponse(bytearray(b'\x00\x00'))
        self.assertEqual(response.error_code, 0)
...
class TestAdministrativeAPI(unittest2.TestCase):
    ...
    def test_list_groups_request(self):
        ...
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00\x11\x00\x10\x00\x00\x00\x00\x00\x00\x00\x07pykafka'))

    def test_list_groups_response(self):
        response = protocol.ListGroupsResponse(bytearray(b'\x00\x00\x00\x00\x00\x01\x00\ttestgroup\x00\x08consumer'))
    ...
    def test_create_topics_request(self):
        req = protocol.CreateTopicsRequest([
         protocol.CreateTopicRequest(b'mycooltopic', 4, 2, [], [])])
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x004\x00\x13\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\x00\x00\x01\x00\x0bmycooltopic\x00\x00\x00\x04\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00'))

    def test_create_topics_response(self):
        response = protocol.CreateTopicsResponse(bytearray(b'\x00\x00\x00\x01\x00\ttesttopic\x00\x00'))

    def test_delete_topics_request(self):
        req = protocol.DeleteTopicsRequest([b'mycooltopic'])
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00&\x00\x14\x00\x00\x00\x00\x00\x00\x00\x07pykafka\x00\x00\x00\x01\x00\x0bmycooltopic\x00\x00\x00\x00'))

    def test_delete_topics_response(self):
        response = protocol.DeleteTopicsResponse(bytearray(b'\x00\x00\x00\x01\x00\ttesttopic\x00\x00'))

    def test_api_versions_request(self):
        req = protocol.ApiVersionsRequest()
        msg = req.get_bytes()
        self.assertEqual(msg, bytearray(b'\x00\x00\x00\x11\x00\x12\x00\x00\x00\x00\x00\x00\x00\x07pykafka'))

    def test_api_versions_response(self):
        response = protocol.ApiVersionsResponse(bytearray(b'\x00\x00\x00\x00\x00\x15\x00\x00\x00\x00\x00\x02\x00\x01\x00\x00\x00\x03\x00\x02\x00\x00\x00\x01\x00\x03\x00\x00\x00\x02\x00\x04\x00\x00\x00\x00\x00\x05\x00\x00\x00\x00\x00\x06\x00\x00\x00\x02\x00\x07\x00\x01\x00\x01\x00\x08\x00\x00\x00\x02\x00\t\x00\x00\x00\x01\x00\n\x00\x00\x00\x00\x00\x0b\x00\x00\x00\x01\x00\x0c\x00\x00\x00\x00\x00\r\x00\x00\x00\x00\x00\x0e\x00\x00\x00\x00\x00\x0f\x00\x00\x00\x00\x00\x10\x00\x00\x00\x00\x00\x11\x00\x00\x00\x00\x00\x12\x00\x00\x00\x00\x00\x13\x00\x00\x00\x00\x00\x14\x00\x00\x00\x00\x00\x00\x00\x00'))
