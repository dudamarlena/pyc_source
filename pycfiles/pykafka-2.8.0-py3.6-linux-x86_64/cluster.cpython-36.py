...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/cluster.py
# Compiled at: 2018-09-20 15:28:30
# Size of source mod 2**32: 24552 bytes
from __future__ import division
__license__ = '\nCopyright 2015 Parse.ly, Inc.\n\nLicensed under the Apache License, Version 2.0 (the "License");\nyou may not use this file except in compliance with the License.\nYou may obtain a copy of the License at\n\n    http://www.apache.org/licenses/LICENSE-2.0\n\nUnless required by applicable law or agreed to in writing, software\ndistributed under the License is distributed on an "AS IS" BASIS,\nWITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.\nSee the License for the specific language governing permissions and\nlimitations under the License.\n'
__all__ = ['Cluster']
import json, logging, random, time, weakref
from pkg_resources import parse_version
from kazoo.client import KazooClient
from .broker import Broker
from .exceptions import ERROR_CODES, GroupCoordinatorNotAvailable, NoBrokersAvailableError, SocketDisconnectedError, LeaderNotFoundError, LeaderNotAvailable, UnicodeException
from .protocol import GroupCoordinatorRequest, GroupCoordinatorResponse, API_VERSIONS_090, API_VERSIONS_080
from .topic import Topic
from .utils.compat import iteritems, itervalues, range, get_string
log = logging.getLogger(__name__)

class TopicDict(dict):
    __doc__ = 'Lazy dict, which will also attempt to auto-create unknown topics'
    ...
    def __getitem__(self, key):
        try:
            ...
            raise UnicodeException("Topic name '{}' contains non-ascii characters".format(key))

        if self._should_exclude_topic(key):
            raise KeyError('You have configured KafkaClient/Cluster to hide double-underscored, internal topics')
        ...
        while True:
            ...
            if err == LeaderNotAvailable.ERROR_CODE:
                ...
                if err == 0:
                    ...
                    raise ERROR_CODES[err]("Failed to auto-create topic '{}'".format(topic_name))
    ...
    def _update_topics(self, metadata):
        ...
        for name, meta in iteritems(metadata):
            if self._should_exclude_topic(name) or name not in self.keys():
                self[name] = None
...
class Cluster(object):
    ...
    def __init__(self, hosts, handler, socket_timeout_ms=30000, offsets_channel_socket_timeout_ms=10000, exclude_internal_topics=True, source_address='', zookeeper_hosts=None, ssl_config=None, broker_version='0.9.0'):
    ...
    def _request_metadata(self, broker_connects, topics):
        return self._request_random_broker(broker_connects, lambda b: b.request_metadata(topics))
    ...
    def _request_random_broker(self, broker_connects, req_fn):
        ...
        for i in range(self._max_connection_retries):
            for host, port in broker_connects:
                try:
                    broker = Broker((-1), host, (int(port)), (self._handler), (self._socket_timeout_ms),
                    ...
                    log.error('Socket disconnected during request for broker %s:%s. Continuing.', host, port)
    ...
    def _get_metadata(self, topics=None):
        ...
        if metadata is not None:
            return metadata
        raise NoBrokersAvailableError('Unable to connect to a broker to fetch metadata. See logs.')
    ...
    def _get_broker_connection_info(self):
        ...
        if self._zookeeper_connect is not None:
            ...
            broker_connects = [[broker_str.split(':')[0], broker_str.split(':')[1].split('/')[0]] for broker_str in self._seed_hosts.split(',')]
    ...
    def _get_brokers_from_zookeeper(self, zk_connect):
        ...
        try:
            timeout = len(zk_connect.split(',')) * self._max_connection_retries * self._socket_timeout_ms / 1000
    ...
    def _update_brokers(self, broker_metadata, controller_id):
        ...
        for id_, meta in iteritems(broker_metadata):
            if id_ not in self._brokers:
                ...
                self._brokers[id_] = Broker.from_metadata(meta,
                  (self._handler), (self._socket_timeout_ms), (self._offsets_channel_socket_timeout_ms),
                  buffer_size=1048576,
                  source_host=(self._source_host),
                  source_port=(self._source_port),
                  ssl_config=(self._ssl_config),
                ...
                if meta.host == broker.host:
                    if meta.port == broker.port:
                        log.info('Broker %s:%s metadata unchanged. Continuing.', broker.host, broker.port)
                        continue
                raise Exception('Broker host/port change detected! %s', broker)

        if controller_id is not None:
            if controller_id not in self._brokers:
                raise KeyError('Controller ID {} not present in cluster'.format(controller_id))
    ...
    def get_managed_group_descriptions(self):
        ...
        for broker in itervalues(self.brokers):
            res = broker.describe_groups([group.group_id for group in itervalues(broker.list_groups().groups)])
            descriptions.update(res.groups)

        return descriptions

    def get_offset_manager(self, consumer_group):
        log.warning('WARNING: Cluster.get_offset_manager is deprecated since pykafka 2.3.0. Instead, use Cluster.get_group_coordinator.')
    ...
    def get_group_coordinator(self, consumer_group):
        ...
        """
        log.info("Attempting to discover offset manager for consumer group '%s'", consumer_group)
        ...
        for i in range(max_connection_retries):
            ...
            for broker in itervalues(self.brokers):
                ...
                try:
                    ...
                    log.error('Broker {} not connected during offset manager discovery'.format(broker.id))
    ...
    def fetch_api_versions(self):
        ...
        if version < parse_version('0.10.0'):
            log.info('Broker version is too old to use automatic API version discovery. Falling back to hardcoded versions list.')
        ...
        broker_connects = self._get_broker_connection_info()
        for i in range(self._max_connection_retries):
            response = self._request_random_broker(broker_connects, lambda b: b.fetch_api_versions())
            if not response:
        ...
        for i in range(self._max_connection_retries):
            ...
            if len(metadata.brokers) == 0:
                if len(metadata.topics) == 0:
                    log.warning('No broker metadata found. If this is a fresh cluster, this may be due to a bug in Kafka. You can force broker metadata to be returned by manually creating a topic in the cluster. See https://issues.apache.org/jira/browse/KAFKA-2154 for information.')
            self._update_brokers(metadata.brokers, metadata.controller_id)
            try:
                self._topics._update_topics(metadata.topics)
            except LeaderNotFoundError:
                log.warning('LeaderNotFoundError encountered. This may be because one or more partitions have no available replicas.')
