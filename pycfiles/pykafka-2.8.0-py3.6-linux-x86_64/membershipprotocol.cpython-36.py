...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/membershipprotocol.py
...
GroupMembershipProtocol = namedtuple('GroupMembershipProtocol', ['protocol_type',
 'protocol_name',
 'metadata',
 'decide_partitions'])
...
def decide_partitions_range(participants, partitions, consumer_id):
    ...
    log.info('%s: Balancing %i participants for %i partitions. Owning %i partitions.', consumer_id, len(participants), len(all_parts), len(new_partitions))
    log.debug('My partitions: %s', [p_to_str(p) for p in new_partitions])
    return new_partitions


RangeProtocol = GroupMembershipProtocol(b'consumer', b'range', ConsumerGroupProtocolMetadata(), decide_partitions_range)
...
def decide_partitions_roundrobin(participants, partitions, consumer_id):
    ...
    log.info('%s: Balancing %i participants for %i partitions. Owning %i partitions.', consumer_id, len(participants), len(partitions), len(new_partitions))
    log.debug('My partitions: %s', [p_to_str(p) for p in new_partitions])
    return new_partitions


RoundRobinProtocol = GroupMembershipProtocol(b'consumer', b'roundrobin', ConsumerGroupProtocolMetadata(), decide_partitions_roundrobin)