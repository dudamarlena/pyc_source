...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/managedbalancedconsumer.py
# Compiled at: 2018-05-29 17:13:24
# Size of source mod 2**32: 20632 bytes
...
import logging, sys, uuid, weakref
from .balancedconsumer import BalancedConsumer
from .common import OffsetType
from .exceptions import IllegalGeneration, RebalanceInProgress, NotCoordinatorForGroup, GroupCoordinatorNotAvailable, ERROR_CODES, GroupLoadInProgress, UnicodeException
...
class ManagedBalancedConsumer(BalancedConsumer):
    ...
    def __init__(self, topic, cluster, consumer_group, fetch_message_max_bytes=1048576, num_consumer_fetchers=1, auto_commit_enable=False, auto_commit_interval_ms=60000, queued_max_messages=2000, fetch_min_bytes=1, fetch_error_backoff_ms=500, fetch_wait_max_ms=100, offsets_channel_backoff_ms=1000, offsets_commit_max_retries=5, auto_offset_reset=OffsetType.EARLIEST, consumer_timeout_ms=-1, rebalance_max_retries=5, rebalance_backoff_ms=2000, auto_start=True, reset_offset_on_start=False, post_rebalance_callback=None, use_rdkafka=False, compacted_topic=True, heartbeat_interval_ms=3000, membership_protocol=RangeProtocol, deserializer=None, reset_offset_on_fetch=True):
        ...
        if use_rdkafka is True:
            raise ImportError('use_rdkafka is not available for {}'.format(self.__class__.__name__))
    ...
    def _setup_heartbeat_worker(self):
        ...
        def fetcher():
            while True:
                try:
                    ...
                    res = self._group_coordinator.heartbeat(self._connection_id, self._consumer_group, self._generation_id, self._consumer_id)
                    if res.error_code != 0:
        ...
        return self._cluster.handler.spawn(fetcher,
          name='pykafka.ManagedBalancedConsumer.heartbeats')
    ...
    def stop(self):
        ...
        if self._group_coordinator is not None:
            self._group_coordinator.leave_group(self._connection_id, self._consumer_group, self._consumer_id)
    ...
    def _update_member_assignment(self):
        ...
        for i in range(self._rebalance_max_retries):
            try:
                ...
                group_assignments = [(member_id,
                 MemberAssignment([
                  (self._topic.name,
                   [p.id for p in self._membership_protocol.decide_partitions(iterkeys(members), self._topic.partitions, member_id)])])) for member_id in members]
                assignment = self._sync_group(group_assignments)
                self._setup_internal_consumer(partitions=[self._topic.partitions[pid] for pid in assignment[0][1]])
    ...
    def _build_default_error_handlers(self):
        ...
        return {GroupCoordinatorNotAvailable.ERROR_CODE: _handle_GroupCoordinatorNotAvailable, 
    ...
    def _join_group(self):
        ...
        for i in range(self._cluster._max_connection_retries):
            join_result = self._group_coordinator.join_group(self._connection_id, self._consumer_group, self._consumer_id, self._topic.name, self._membership_protocol)
            if join_result.error_code == 0:
                break
            log.info("Error code %d encountered during JoinGroupRequest for generation '%s'", join_result.error_code, self._generation_id)
    ...
    def _sync_group(self, group_assignments):
        ...
        for i in range(self._cluster._max_connection_retries):
            sync_result = self._group_coordinator.sync_group(self._connection_id, self._consumer_group, self._generation_id, self._consumer_id, group_assignments)
            if sync_result.error_code == 0:
                break
            log.info('Error code %d encountered during SyncGroupRequest', sync_result.error_code)
            if i == self._cluster._max_connection_retries - 1:
