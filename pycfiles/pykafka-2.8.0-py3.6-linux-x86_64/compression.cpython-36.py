...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pykafka/utils/compression.py
...
def encode_snappy(buff, xerial_compatible=False, xerial_blocksize=32768):
    ...
    if xerial_compatible:
        ...
        header = (b'').join([struct.pack('!' + fmt, dat) for fmt, dat in full_data])
        out.write(header)
...
if lz4:
    encode_lz4 = lz4.compress
else:
    if lz4f:
        encode_lz4 = lz4f.compressFrame
    else:
        encode_lz4 = None
...
def decode_lz4_old_kafka(buff):
    ...
    if content_size_bit:
        header_size += 8
    hc = xxhash.xxh32(buff[4:header_size - 1]).digest()[-2:-1]
    munged_buff = (b'').join([
