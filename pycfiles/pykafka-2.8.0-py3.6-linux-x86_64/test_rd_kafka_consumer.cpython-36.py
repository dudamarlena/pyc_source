...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tests/pykafka/rdkafka/test_rd_kafka_consumer.py
# Compiled at: 2018-05-14 12:02:28
# Size of source mod 2**32: 3863 bytes
from contextlib import contextmanager
import unittest2, pytest
...
class TestRdKafkaConsumer(unittest2.TestCase):
    ...
    def test_start_fail(self):
        ...
        with self.assert_thread_cnt_non_increasing():
            ...
            with self.assertRaises(RdKafkaException):
                consumer.start(brokers=b'', topic_name=(self.topic_name),
    ...
    def test_stop(self):
        ...
        with self.assert_thread_cnt_non_increasing():
            ...
            consumer.start(brokers=(get_bytes(self.kafka.brokers)), topic_name=(self.topic_name),
              partition_ids=(self.partition_ids),
              start_offsets=(self.start_offsets))
    ...
    def test_stopped_exception(self):
        ...
        consumer = _rd_kafka.Consumer(brokers=(get_bytes(self.kafka.brokers)), topic_name=(self.topic_name),
          partition_ids=(self.partition_ids),
          start_offsets=(self.start_offsets))
        consumer.stop()
