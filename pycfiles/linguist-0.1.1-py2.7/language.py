...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/linguist/libs/language.py
...
class Language(object):
    ...
    def __init__(self, attributes={}):
        ...
        if self.primary_extension not in self.extensions:
            self.extensions = [
             self.primary_extension] + self.extensions
...
for name, options in sorted(LANGUAGES.iteritems(), key=lambda k: k[0]):
    ...
    Language.create(dict(name=name, color=options.get('color'), type=options.get('type'), aliases=options.get('aliases', []), lexer=options.get('lexer'), ace_mode=options.get('ace_mode'), wrap=options.get('wrap'), group_name=options.get('group'), searchable=options.get('searchable', True), search_term=options.get('search_term'), extensions=sorted(options['extensions']), primary_extension=options.get('primary_extension'), filenames=options['filenames'], popular=name in popular))