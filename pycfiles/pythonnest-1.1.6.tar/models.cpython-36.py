...
# [GCC 8.4.0]
# Embedded file name: /Users/flanker/Developer/Github/PythonNest/pythonnest/models.py
# Compiled at: 2018-04-21 03:57:59
# Size of source mod 2**32: 13768 bytes
...
class CachedName(models.Model):
    ...
    creation = models.DateTimeField((_('creation')), db_index=True, auto_now_add=True)
    modification = models.DateTimeField((_('modification')), db_index=True, auto_now=True)
    _CachedName__cache = None

    @classmethod
    def get(cls, name):
        if cls._CachedName__cache is None:
            cls._CachedName__cache = ObjectCache(lambda key: cls.objects.get_or_create(name=key)[0])
        return cls._CachedName__cache.get(name)
...
class Classifier(CachedName):
    _Classifier__cache = None


class Dependence(CachedName):
    _Dependence__cache = None


class PackageType(CachedName):
    _PackageType__cache = None
...
class Package(models.Model):
    ...
    creation = models.DateTimeField((_('creation')), db_index=True, auto_now_add=True)
    modification = models.DateTimeField((_('modification')), db_index=True, auto_now=True)
    group = models.ForeignKey(Group, verbose_name=(_('restrict to this group')), db_index=True, null=True, blank=True, on_delete=(models.SET_NULL))
    ...
    def data(self):
        result = {}
        for attr_name in ('name', 'author', 'author_email', 'maintainer', 'maintainer_email',
                          'home_page', 'license', 'summary', 'download_url', 'project_url'):
...
class Release(models.Model):
    ...
    stable_version = models.CharField((_('stable version name')), db_index=True, null=True, blank=True, default='', max_length=455)
    ...
    def data(self):
        result = {}
        for attr_name in ('name', 'version', 'stable_version', 'author', 'author_email',
                          'maintainer', 'maintainer_email', 'home_page', 'license',
                          'summary', 'description', 'platform', 'download_url', 'project_url',
                          'keywords', 'docs_url'):
            result[attr_name] = getattr(self, attr_name)

        for attr_name in ('classifiers', 'requires', 'requires_dist', 'provides', 'provides_dist',
                          'requires_external', 'requires_python', 'obsoletes', 'obsoletes_dist'):
...
class ReleaseDownload(models.Model):
    ...
    package_type = models.ForeignKey(PackageType, db_index=True, null=True, blank=True, default=None, on_delete=(models.CASCADE))
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        ...
        super(ReleaseDownload, self).save(force_insert=force_insert, force_update=force_update, using=using, update_fields=update_fields)
    ...
    def data(self, request=None):
        result = dict([(x, getattr(self, x)) for x in ('has_sig', 'upload_time', 'comment_text',
                                                       'python_version', 'url', 'md5_digest',
                                                       'downloads', 'filename', 'size')])
...
class Log(models.Model):
    ...
    def save(self, force_insert=False, force_update=False, using=None, update_fields=None):
        ...
        super(Log, self).save(force_insert=force_insert, force_update=force_update, using=using, update_fields=update_fields)