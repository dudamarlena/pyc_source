...
# Embedded file name: D:\ProgramData\envs\Test\lib\arelle\FileSource.py
# Compiled at: 2018-03-15 11:12:05
# Size of source mod 2**32: 36561 bytes
...
archivePathSeparators = (
 '.zip' + os.sep, '.tar.gz' + os.sep, '.eis' + os.sep, '.xml' + os.sep, '.xfd' + os.sep, '.frm' + os.sep, '.taxonomyPackage.xml' + os.sep) + (('.zip/',
                                                                                                                                               '.tar.gz/',
                                                                                                                                               '.eis/',
                                                                                                                                               '.xml/',
                                                                                                                                               '.xfd/',
                                                                                                                                               '.frm/',
                                                                                                                                               '.taxonomyPackage.xml/') if os.sep != '/' else ())
    ...
    for archiveSep in archivePathSeparators:
        if filename:
            if archiveSep in filename:
                if not archiveSep.startswith('.xml') or checkIfXmlIsEis:
...
class FileSource:
    ...
    def open(self, reloadCache=False):
        if self.isOpen or self.isZip or self.isTarGz or self.isEis or self.isXfd or self.isRss or self.isInstalledTaxonomyPackage:
            if self.cntlr:
    ...
    def fileSourceContainingFilepath(self, filepath):
        if self.isOpen:
            if filepath.startswith(self.basefile) or filepath.startswith(self.baseurl):
                return self
        ...
        if referencedFileParts is not None:
            ...
            elif not self.isOpen or referencedArchiveFile != self.basefile and referencedArchiveFile != self.baseurl:
...
def openXmlFileStream(cntlr, filepath, stripDeclaration=False):
    ...
    if encoding.lower() in ('utf-8', 'utf8', 'utf-8-sig'):
        if cntlr is None or not cntlr.isGAE:
            if not stripDeclaration:
                text = None
                openedFileStream.close()
    else:
...
gaeMemcache = None
GAE_MEMCACHE_MAX_ITEM_SIZE = 921600
GAE_EXPIRE_WEEK = 604800
