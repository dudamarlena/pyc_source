...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/RenderingResolver.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 44344 bytes
...
def resolveTableAxesStructure(view, table, tblAxisRelSet):
    ...
    for disposition in ('z', 'x', 'y'):
        for i, tblAxisRel in enumerate(tblAxisRels):
            definitionNode = tblAxisRel.toModelObject
            if tblAxisRel.axisDisposition == disposition and isinstance(definitionNode, (ModelEuAxisCoord, ModelBreakdown, ModelDefinitionNode)):
                if disposition == 'x' and xTopStructuralNode is None:
                    ...
                    if disposition == 'y' and yTopStructuralNode is None:
    ...
    return (
     tblAxisRelSet, xTopStructuralNode, yTopStructuralNode, zTopStructuralNode)
...
def childContainsOpenNodes(childStructuralNode):
    if isinstance(childStructuralNode.definitionNode, ModelFilterDefinitionNode) and (childStructuralNode.isLabeled or any([node.isEntryPrototype(default=False) for node in childStructuralNode.childStructuralNodes])):
        return True
    else:
...
def expandDefinition(view, structuralNode, breakdownNode, definitionNode, depth, axisDisposition, facts, i=None, tblAxisRels=None, processOpenDefinitionNode=True):
    ...
    def checkLabelWidth(structuralNode, checkBoundFact=False):
        if axisDisposition == 'y':
            label = structuralNode.header(lang=view.lang, returnGenLabel=not checkBoundFact, returnMsgFormatString=not checkBoundFact)
            if label:
        ...
        try:
            try:
                ...
                if isinstance(definitionNode, ModelConceptRelationshipDefinitionNode):
                    view.modelXbrl.error('xbrlte:expressionNotCastableToRequiredType', _('Relationship node %(xlinkLabel)s expression not castable to required type (%(xpathError)s)'), modelObject=(
                     view.modelTable, definitionNode), xlinkLabel=definitionNode.xlinkLabel, axis=definitionNode.localName, xpathError=str(ex))
                    return

            if not definitionNode.isAbstract and isinstance(definitionNode, ModelClosedDefinitionNode) and ordCardinality == 0:
                view.modelXbrl.error('xbrlte:closedDefinitionNodeZeroCardinality', _('Closed definition node %(xlinkLabel)s does not contribute at least one structural node'), modelObject=(
                 view.modelTable, definitionNode), xlinkLabel=definitionNode.xlinkLabel, axis=definitionNode.localName)
            ...
            if axisDisposition == 'z':
                ...
                if axisDisposition == 'x':
                    if ordDepth and nestedDepth - 1 > view.colHdrRows:
                ...
                if not isinstance(definitionNode, ModelFilterDefinitionNode):
                    ...
                    for axisSubtreeRel in subtreeRelationships:
                        ...
                        if childDefinitionNode.isRollUp:
                            ...
                            if isinstance(definitionNode, (ModelBreakdown, ModelCompositionDefinitionNode)) and isinstance(childDefinitionNode, ModelRelationshipDefinitionNode):
            ...
            if isinstance(definitionNode, ModelRelationshipDefinitionNode):
                ...
                if axisDisposition == 'z':
                    ...
                    if isinstance(definitionNode, ModelConceptRelationshipDefinitionNode):
                        if definitionNode._sourceQname != XbrlConst.qnXfiRoot and definitionNode._sourceQname not in view.modelXbrl.qnameConcepts:
                            view.modelXbrl.error('xbrlte:invalidConceptRelationshipSource', _('Concept relationship rule node %(xlinkLabel)s source %(source)s does not refer to an existing concept.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, source=definitionNode._sourceQname)
                if isinstance(definitionNode, ModelDimensionRelationshipDefinitionNode):
                    ...
                    if dim is None or not dim.isExplicitDimension:
                        view.modelXbrl.error('xbrlte:invalidExplicitDimensionQName', _('Dimension relationship rule node %(xlinkLabel)s dimension %(dimension)s does not refer to an existing explicit dimension.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, dimension=definitionNode._dimensionQname)
                    domMbr = view.modelXbrl.qnameConcepts.get(definitionNode._sourceQname)
                    if domMbr is None or not domMbr.isDomainMember:
                        view.modelXbrl.error('xbrlte:invalidDimensionRelationshipSource', _('Dimension relationship rule node %(xlinkLabel)s source %(source)s does not refer to an existing domain member.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, source=definitionNode._sourceQname)
                    if definitionNode._axis in ('child', 'child-or-self', 'parent',
                                                'parent-or-self', 'sibling', 'sibling-or-self') and (not isinstance(definitionNode._generations, _NUM_TYPES) or definitionNode._generations > 1):
                        view.modelXbrl.error('xbrlte:relationshipNodeTooManyGenerations ', _('Relationship rule node %(xlinkLabel)s formulaAxis %(axis)s implies a single generation tree walk but generations %(generations)s is greater than one.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, axis=definitionNode._axis, generations=definitionNode._generations)
                else:
                    if isinstance(definitionNode, ModelSelectionDefinitionNode):
                        ...
                        filteredFactsPartitions = structuralNode.evaluate(definitionNode, definitionNode.filteredFactsPartitions, evalArgs=(
                         facts,))
                        if structuralNode._rendrCntx.formulaOptions.traceVariableFilterWinnowing:
                            view.modelXbrl.info('table:trace', _('Filter node %(xlinkLabel)s facts partitions: %(factsPartitions)s'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, factsPartitions=str(filteredFactsPartitions))
                        if axisDisposition != 'z':
                        ...
                        childList.sort(key=lambda childStructuralNode: childStructuralNode.header(lang=view.lang, returnGenLabel=False, returnMsgFormatString=False) or '')
            if isinstance(definitionNode, ModelTupleDefinitionNode):
                structuralNode.abstract = True
                matchingTupleFacts = structuralNode.evaluate(definitionNode, definitionNode.filteredFacts, evalArgs=(
                 facts,))
                ...
                if structuralNode.childStructuralNodes and any(sOC.header(lang=view.lang) for sOC in structuralNode.childStructuralNodes):
                            ...
                            if not constraintSet.aspectValueDependsOnVars(aspect):
                                if aspect == Aspect.CONCEPT:
                                    ...
                                    if concept is None or not concept.isItem or concept.isDimensionItem or concept.isHypercubeItem:
                                        view.modelXbrl.error('xbrlte:invalidQNameAspectValue', _('Rule node %(xlinkLabel)s specifies concept %(concept)s does not refer to an existing primary item concept.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, concept=conceptQname)
                            elif isinstance(aspect, QName):
                                memQname = definitionNode.aspectValue(view.rendrCntx, aspect)
                                mem = view.modelXbrl.qnameConcepts.get(memQname)
                                if isinstance(memQname, QName) and (mem is None or not mem.isDomainMember) and memQname != XbrlConst.qnFormulaDimensionSAV:
                                    view.modelXbrl.error('xbrlte:invalidQNameAspectValue', _('Rule node %(xlinkLabel)s specifies domain member %(concept)s does not refer to an existing domain member concept.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel, concept=memQname)

                    if not definitionNode.constraintSets:
                        view.modelXbrl.error('xbrlte:incompleteAspectRule', _('Rule node %(xlinkLabel)s does not specify an aspect value.'), modelObject=definitionNode, xlinkLabel=definitionNode.xlinkLabel)
                    if axisDisposition == 'z':
            ...
            e = ResolutionException('arelle:resolutionException', _('Exception in resolution of definition node %(node)s: %(error)s'), modelObject=definitionNode, node=definitionNode.qname, error=str(ex))
            if sys.version[0] >= '3':
        ...
        for j, tblRel in enumerate(tblAxisRels[i + 1:]):
            ...
            if isinstance(tblObj, (ModelEuAxisCoord, ModelDefinitionNode)) and axisDisposition == tblRel.axisDisposition:
                ...
                if isinstance(childStructuralNode.definitionNode, ModelClosedDefinitionNode):
                    matchingFacts = childStructuralNode.evaluate(childStructuralNode.definitionNode, childStructuralNode.definitionNode.filteredFacts, evalArgs=(
                     facts,))
                else:
                    matchingFacts = facts
