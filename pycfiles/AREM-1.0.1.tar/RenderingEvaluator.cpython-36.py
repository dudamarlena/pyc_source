...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\RenderingEvaluator.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 17073 bytes
    ...
    if hasXbrlTables:
        ...
        for modelTable in modelXbrl.modelRenderingTables:
            ...
            if modelTable.aspectModel not in ('non-dimensional', 'dimensional') and hasNsWithAspectModel:
                modelXbrl.error('xbrlte:unknownAspectModel', (_('Table %(xlinkLabel)s, aspect model %(aspectModel)s not recognized')),
                ...
                if Aspect.CONCEPT not in aspectsCovered:
                    if not hasNsWithAspectModel:
                        modelXbrl.error('xbrlte:tableMissingConceptAspect', (_('Table %(xlinkLabel)s does not include the concept aspect as one of its participating aspects')),
                          modelObject=modelTable,
                          xlinkLabel=(modelTable.xlinkLabel))
                ...
                for tblParamRel in modelXbrl.relationshipSet((XbrlConst.tableParameter, XbrlConst.tableParameterMMDD)).fromModelObject(modelTable):
                    ...
                    if parameterName in parameterNames:
                        modelXbrl.error('xbrlte:tableParameterNameClash ', (_('Table %(xlinkLabel)s has parameter name clash for variable %(name)s')),
                          modelObject=(
                         modelTable, tblParamRel, parameterNames[parameterName]),
                          xlinkLabel=(modelTable.xlinkLabel),
                          name=parameterName)
                    else:
    ...
    if isinstance(definitionNode, (ModelDefinitionNode, ModelEuAxisCoord)):
        for aspect in definitionNode.aspectsCovered():
            ...
            if aspect in uncoverableAspects or isinstance(aspect, QName) and modelTable.aspectModel == 'non-dimensional':
                modelXbrl.error('xbrlte:axisAspectModelMismatch', (_('%(definitionNode)s %(xlinkLabel)s, aspect model %(aspectModel)s, aspect %(aspect)s not allowed')),
                  modelObject=modelTable,
            ...
            if aspect in modelTable.priorAspectAxisDisposition:
                ...
                if tblAxisDisposition != otherAxisDisposition:
                    if aspect != Aspect.DIMENSIONS:
                        modelXbrl.error('xbrlte:aspectClashBetweenBreakdowns', (_('%(definitionNode)s %(xlinkLabel)s, aspect %(aspect)s defined on axes of disposition %(axisDisposition)s and %(axisDisposition2)s')),
                          modelObject=(
                         modelTable, definitionNode, otherDefinitionNode),
                          definitionNode=(definitionNode.localName),
                          xlinkLabel=(definitionNode.xlinkLabel),
        ...
        if definitionNode.isMerged:
            if ruleSetChildren:
                modelXbrl.error('xbrlte:mergedRuleNodeWithTaggedRuleSet', (_('Merged %(definitionNode)s %(xlinkLabel)s has tagged rule set(s)')),
                  modelObject=([
                 modelTable, definitionNode] + ruleSetChildren),
                  definitionNode=(definitionNode.localName),
                  xlinkLabel=(definitionNode.xlinkLabel))
            labelRels = modelXbrl.relationshipSet(XbrlConst.elementLabel).fromModelObject(definitionNode)
            if labelRels:
                modelXbrl.error('xbrlte:invalidUseOfLabel', (_('Merged %(definitionNode)s %(xlinkLabel)s has label(s)')),
                  modelObject=([
                 modelTable, definitionNode] + [r.toModelObject for r in labelRels]),
                  definitionNode=(definitionNode.localName),
                  xlinkLabel=(definitionNode.xlinkLabel))
            if not definitionNode.isAbstract:
                modelXbrl.error('xbrlte:nonAbstractMergedRuleNode', (_('Merged %(definitionNode)s %(xlinkLabel)s is not abstract')),
                  modelObject=(
                 modelTable, definitionNode),
                  definitionNode=(definitionNode.localName),
                  xlinkLabel=(definitionNode.xlinkLabel))
    if isinstance(definitionNode, ModelRuleDefinitionNode):
        ...
        for ruleSet in XmlUtil.children(definitionNode, definitionNode.namespaceURI, 'ruleSet'):
            ...
            if tag in tagConstraintSets:
                modelXbrl.error('xbrlte:duplicateTag', (_('%(definitionNode)s %(xlinkLabel)s duplicate rule set tags %(tag)s')),
                  modelObject=(
                 modelTable, definitionNode, tagConstraintSets[tag], ruleSet),
                  definitionNode=(definitionNode.localName),
                  xlinkLabel=(definitionNode.xlinkLabel),
                  tag=tag)
            else:
        ...
        for tag, constraintSet in definitionNode.constraintSets.items():
            if otherConstraintSet is None:
                ...
                modelXbrl.error('xbrlte:constraintSetAspectMismatch', (_('%(definitionNode)s %(xlinkLabel)s constraint set mismatches between %(tag1)s and %(tag2)s in constraints %(aspects)s')),
                  modelObject=(
                 modelTable, definitionNode, otherConstraintSet, constraintSet),
                  definitionNode=(definitionNode.localName),
                  xlinkLabel=(definitionNode.xlinkLabel),
                  tag1=(getattr(otherConstraintSet, 'tagName', '(no tag)')),
                  tag2=(getattr(constraintSet, 'tagName', '(no tag)')),
                  aspects=(', '.join(aspectStr(aspect) for aspect in otherConstraintSet.aspectsCovered() ^ constraintSet.aspectsCovered() if aspect != Aspect.DIMENSIONS)))

    elif isinstance(definitionNode, ModelDimensionRelationshipDefinitionNode):
        hasCoveredAspect = True
        if modelTable.aspectModel == 'non-dimensional':
            modelXbrl.error('xbrlte:axisAspectModelMismatch', (_("DimensionRelationship axis %(xlinkLabel)s can't be used in non-dimensional aspect model")),
              modelObject=(
             modelTable, definitionNode),
              xlinkLabel=(definitionNode.xlinkLabel))
    else:
            ...
            for aspect in definitionNode.aspectsCovered():
                if isinstance(aspect, QName):
                    ...
                    if concept is None or not concept.isDimensionItem:
                        modelXbrl.error('xbrlte:invalidDimensionQNameOnAspectNode', (_('Aspect node %(xlinkLabel)s dimensional aspect %(dimension)s is not a dimension')),
                          modelObject=(
                         modelTable, definitionNode),
                          xlinkLabel=(definitionNode.xlinkLabel),
                          dimension=aspect)
        ...
        if not definitionNodeHasChild:
            if definitionNode.namespaceURI in ('http://www.eurofiling.info/2010/rendering',
                ...
                if not hasCoveredAspect:
                    modelXbrl.error('xbrlte:aspectValueNotDefinedByOrdinate', (_('%(definitionNode)s %(xlinkLabel)s does not define an aspect')),
                      modelObject=(
                     modelTable, definitionNode),
                      xlinkLabel=(definitionNode.xlinkLabel),
                      definitionNode=(definitionNode.localName))
            if isinstance(definitionNode, ModelClosedDefinitionNode):
                if definitionNode.isAbstract:
                    modelXbrl.error('xbrlte:abstractRuleNodeNoChildren', (_('Abstract %(definitionNode)s %(xlinkLabel)s has no children')),
                      modelObject=(
                     modelTable, definitionNode),
                      xlinkLabel=(definitionNode.xlinkLabel),
                      definitionNode=(definitionNode.localName))
...
def checkBreakdownLeafNodeAspects(modelXbrl, modelTable, tblAxisRel, parentAspectsCovered, breakdownAspects):
    ...
    if isinstance(definitionNode, (ModelDefinitionNode, ModelEuAxisCoord)):
        ...
        if not definitionNode.isAbstract:
            if not isinstance(definitionNode, ModelBreakdown):
                missingAspects = set(aspect for aspect in breakdownAspects if aspect not in aspectsCovered if aspect != Aspect.DIMENSIONS if not isinstance(aspect, QName))
                if missingAspects:
                    modelXbrl.error('xbrlte:missingAspectValue', (_('%(definitionNode)s %(xlinkLabel)s does not define an aspect for %(aspect)s')),
                      modelObject=(
                     modelTable, definitionNode),
                      xlinkLabel=(definitionNode.xlinkLabel),
                      definitionNode=(definitionNode.localName),
