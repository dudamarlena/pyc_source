...
# Embedded file name: /home/nagat/.local/lib/python3.5/site-packages/arelle/ModelDocument.py
# Compiled at: 2018-08-09 04:15:45
# Size of source mod 2**32: 99992 bytes
...
from arelle import PackageManager, XbrlConst, XmlUtil, UrlUtil, ValidateFilingText, XhtmlValidate, XmlValidateSchema, ViewFileFactList
...
def load(modelXbrl, uri, base=None, referringElement=None, isEntry=False, isDiscovered=False, isIncluded=None, namespace=None, reloadCache=False, **kwargs):
    ...
    if modelXbrl.modelManager.validateDisclosureSystem and not normalizedUri.startswith(modelXbrl.uriDir) and not modelXbrl.modelManager.disclosureSystem.hrefValid(normalizedUri):
        ...
        if normalizedUri not in modelXbrl.urlUnloadableDocs:
            modelXbrl.error(('EFM.6.22.02', 'GFM.1.1.3', 'SBR.NL.2.1.0.06' if normalizedUri.startswith('http') else 'SBR.NL.2.2.0.17'), _('Prohibited file for filings %(blockedIndicator)s: %(url)s'), modelObject=referringElement, url=normalizedUri, blockedIndicator=_(' blocked') if blocked else '', messageCodes=('EFM.6.22.02',
                                                                                                                                                                                                                                                                                                                          'GFM.1.1.3',
                                                                                                                                                                                                                                                                                                                          'SBR.NL.2.1.0.06',
                                                                                                                                                                                                                                                                                                                          'SBR.NL.2.2.0.17'))
    ...
    if filepath is None:
        if modelXbrl.modelManager.abortOnMajorError and (isEntry or isDiscovered):
            modelXbrl.error('FileNotLoadable', _('File can not be loaded: %(fileName)s \nLoading terminated.'), modelObject=referringElement, fileName=mappedUri)
            raise LoadingException()
        if normalizedUri not in modelXbrl.urlUnloadableDocs:
            if 'referringElementUrl' in kwargs:
                modelXbrl.error('FileNotLoadable', _('File can not be loaded: %(fileName)s, referenced from %(referencingFileName)s'), modelObject=referringElement, fileName=normalizedUri, referencingFileName=kwargs['referringElementUrl'])
            else:
                modelXbrl.error('FileNotLoadable', _('File can not be loaded: %(fileName)s'), modelObject=referringElement, fileName=normalizedUri)
    ...
    if not isPullLoadable and os.path.splitext(filepath)[1] in ('.xlsx', '.xls', '.csv',
                                                                '.json'):
        modelXbrl.error('FileNotLoadable', _('File can not be loaded, requires loadFromExcel or loadFromOIM plug-in: %(fileName)s'), modelObject=referringElement, fileName=normalizedUri)
        return
    ...
    try:
        ...
        if modelXbrl.modelManager.validateDisclosureSystem and modelXbrl.modelManager.disclosureSystem.validateFileText and normalizedUri not in modelXbrl.modelManager.disclosureSystem.standardTaxonomiesDict:
        ...
        for error in _parser.error_log:
            modelXbrl.error('xmlSchema:syntax', _('%(error)s, %(fileName)s, line %(line)s, column %(column)s, %(sourceAction)s source element'), modelObject=referringElement, fileName=os.path.basename(uri), error=error.message, line=error.line, column=error.column, sourceAction='including' if isIncluded else 'importing')
        ...
        if not isIncluded and namespace and namespace in XbrlConst.standardNamespaceSchemaLocations and uri != XbrlConst.standardNamespaceSchemaLocations[namespace]:
            ...
            if modelXbrl.modelManager.abortOnMajorError and (isEntry or isDiscovered):
                modelXbrl.error('IOerror', _('%(fileName)s: file error: %(error)s \nLoading terminated.'), modelObject=referringElement, fileName=os.path.basename(uri), error=str(err))
                raise LoadingException()
            modelXbrl.error('IOerror', _('%(fileName)s: file error: %(error)s'), modelObject=referringElement, fileName=os.path.basename(uri), error=str(err))
            modelXbrl.urlUnloadableDocs[normalizedUri] = True
            return
    except (etree.LxmlError, etree.XMLSyntaxError, SAXParseException,
     ValueError) as err:
        if file:
            file.close()
        if not isEntry and str(err) == "Start tag expected, '<' not found, line 1, column 1":
            ...
            modelXbrl.error('xmlSchema:syntax', _('Unrecoverable error: %(error)s, %(fileName)s, %(sourceAction)s source element'), modelObject=referringElement, fileName=os.path.basename(uri), error=str(err), sourceAction='including' if isIncluded else 'importing', exc_info=True)
            modelXbrl.urlUnloadableDocs[normalizedUri] = True
            return
    except Exception as err:
        modelXbrl.error(type(err).__name__, _('Unrecoverable error: %(error)s, %(fileName)s, %(sourceAction)s source element'), modelObject=referringElement, fileName=os.path.basename(uri), error=str(err), sourceAction='including' if isIncluded else 'importing', exc_info=True)
        modelXbrl.urlUnloadableDocs[normalizedUri] = True
        return
    ...
    if rootNode is not None:
        ...
        if otherModelDoc.basename == os.path.basename(uri):
            if os.path.normpath(otherModelDoc.uri) != os.path.normpath(uri):
                ...
                modelXbrl.warning('info:duplicatedSchema', _('Schema file with same targetNamespace %(targetNamespace)s loaded from %(fileName)s and %(otherFileName)s'), modelObject=referringElement, targetNamespace=targetNamespace, fileName=uri, otherFileName=otherModelDoc.uri)
            return otherModelDoc
    else:
        if (isEntry or isDiscovered or kwargs.get('isSupplemental', False)) and ns == XbrlConst.link:
            if ln == 'linkbase':
                _type = Type.LINKBASE
            else:
                if ln == 'xbrl':
            ...
            if isEntry and ns == XbrlConst.xbrli:
                if ln == 'xbrl':
                    _type = Type.INSTANCE
                else:
                    _type = Type.UnknownXML
                ...
                if ns == XbrlConst.xhtml and (ln == 'html' or ln == 'xhtml'):
                    ...
                    if ln == 'report' and ns == XbrlConst.ver:
                        ...
                        if ln in ('testcases', 'documentation', 'testSuite'):
                            ...
                            if ln in ('testcase', 'testSet'):
                                ...
                                if ln == 'registry' and ns == XbrlConst.registry:
                                    _type = Type.REGISTRY
                                else:
                                    if ln == 'test-suite' and ns == 'http://www.w3.org/2005/02/query-test-XQTSCatalog':
    ...
    if _type is None:
        ...
        if nestedInline is not None and XbrlConst.ixbrlAll.intersection(nestedInline.nsmap.values()):
            _type = Type.INLINEXBRL
...
def loadSchemalocatedSchema(modelXbrl, element, relativeUrl, namespace, baseUrl):
    ...
    if doc:
        if doc.targetNamespace != namespace:
            modelXbrl.error('xmlSchema1.4.2.3:refSchemaNamespace', _('SchemaLocation of %(fileName)s expected namespace %(namespace)s found targetNamespace %(targetNamespace)s'), modelObject=element, fileName=baseUrl, namespace=namespace, targetNamespace=doc.targetNamespace)
    else:
        doc.inDTS = False
    return doc
...
class ModelDocument:
    ...
    def creationSoftware(self):
        ...
        if creationSoftwareNames is None:
            ...
            try:
                with io.open(os.path.join(self.modelXbrl.modelManager.cntlr.configDir, 'creationSoftwareNames.json'), 'rt', encoding='utf-8') as (f):
                    for key, pattern in json.load(f):
                        if key != '_description_':
                            creationSoftwareNames.append((key, re.compile(pattern, re.IGNORECASE)))

            except Exception as ex:
                self.modelXbrl.error('arelle:creationSoftwareNamesTable', _('Error loading creation software names table %(error)s'), modelObject=self, error=ex)
    ...
    def schemaDiscover(self, rootElement, isIncluded, namespace):
        ...
        if targetNamespace:
            ...
            if namespace and targetNamespace != namespace:
                self.modelXbrl.error('xmlSchema1.4.2.3:refSchemaNamespace', _('Discovery of %(fileName)s expected namespace %(namespace)s found targetNamespace %(targetNamespace)s'), modelObject=rootElement, fileName=self.basename, namespace=namespace, targetNamespace=targetNamespace)
            if self.modelXbrl.modelManager.validateDisclosureSystem and self.modelXbrl.modelManager.disclosureSystem.disallowedHrefOfNamespace(self.uri, targetNamespace):
                self.modelXbrl.error(('EFM.6.22.02', 'GFM.1.1.3', 'SBR.NL.2.1.0.06' if self.uri.startswith('http') else 'SBR.NL.2.2.0.17'), _('Namespace: %(namespace)s disallowed schemaLocation %(schemaLocation)s'), modelObject=rootElement, namespace=targetNamespace, schemaLocation=self.uri, url=self.uri, messageCodes=('EFM.6.22.02',
                                                                                                                                                                                                                                                                                                                                 'GFM.1.1.3',
                                                                                                                                                                                                                                                                                                                                 'SBR.NL.2.1.0.06',
                                                                                                                                                                                                                                                                                                                                 'SBR.NL.2.2.0.17'))
    ...
    def schemaDiscoverChildElements(self, parentModelObject):
        for modelObject in parentModelObject.iterchildren():
            if isinstance(modelObject, ModelObject):
                ...
                if modelObject.namespaceURI == XbrlConst.xsd and ln in {'include', 'redefine', 'import'}:
        ...
        while baseElt is not None:
            ...
            if baseAttr:
                if self.modelXbrl.modelManager.validateDisclosureSystem:
                    self.modelXbrl.error(('EFM.6.03.11', 'GFM.1.1.7', 'EBA.2.1', 'EIOPA.2.1'), _('Prohibited base attribute: %(attribute)s'), modelObject=element, attribute=baseAttr, element=element.qname)
                else:
    ...
    def importDiscover(self, element):
        ...
        if importNamespace and schemaLocation:
            ...
            if self.modelXbrl.modelManager.validateDisclosureSystem and self.modelXbrl.modelManager.disclosureSystem.blockDisallowedReferences and self.modelXbrl.modelManager.disclosureSystem.disallowedHrefOfNamespace(importSchemaLocation, importNamespace):
                self.modelXbrl.error(('EFM.6.22.02', 'GFM.1.1.3', 'SBR.NL.2.1.0.06' if importSchemaLocation.startswith('http') else 'SBR.NL.2.2.0.17'), _('Namespace: %(namespace)s disallowed schemaLocation blocked %(schemaLocation)s'), modelObject=element, namespace=importNamespace, schemaLocation=importSchemaLocation, url=importSchemaLocation, messageCodes=('EFM.6.22.02',
                                                                                                                                                                                                                                                                                                                                                                         'GFM.1.1.3',
                                                                                                                                                                                                                                                                                                                                                                         'SBR.NL.2.1.0.06',
                                                                                                                                                                                                                                                                                                                                                                         'SBR.NL.2.2.0.17'))
                return
            ...
            if doc is not None:
                ...
                doc = load(self.modelXbrl, schemaLocation, base=importElementBase, isDiscovered=self.inDTS, isIncluded=isIncluded, namespace=importNamespace, referringElement=element)
    ...
    def linkbaseDiscover(self, linkbaseElement, inInstance=False):
        ...
        for lbElement in linkbaseElement:
            if isinstance(lbElement, ModelObject):
                ...
                if lbNs == XbrlConst.link and (lbLn == 'roleRef' or lbLn == 'arcroleRef'):
                    href = self.discoverHref(lbElement)
                    if href is None:
                        self.modelXbrl.error('xmlSchema:requiredAttribute', _('Linkbase reference for %(linkbaseRefElement)s href attribute missing or malformed'), modelObject=lbElement, linkbaseRefElement=lbLn)
                    continue
                    if lbElement.get('{http://www.w3.org/1999/xlink}type') == 'extended':
                        if isinstance(lbElement, ModelLink):
                                    ...
                                    if xlinkType == 'locator':
                                        ...
                                        if href is None:
                                            if isStandardExtLink:
                                                self.modelXbrl.error('xmlSchema:requiredAttribute', _('Locator href attribute "%(href)s" missing or malformed in standard extended link'), modelObject=linkElement, href=linkElement.get('{http://www.w3.org/1999/xlink}href'))
                                            else:
                                                self.modelXbrl.warning('arelle:hrefWarning', _('Locator href attribute "%(href)s" missing or malformed in non-standard extended link'), modelObject=linkElement, href=linkElement.get('{http://www.w3.org/1999/xlink}href'))
                                        else:
                            ...
                            self.modelXbrl.error('xbrl:schemaDefinitionMissing', _('Linkbase extended link %(element)s missing schema definition'), modelObject=lbElement, element=lbElement.prefixedName)
    ...
    def discoverHref(self, element, nonDTS=False, urlRewritePluginClass=None):
        ...
        if href:
            ...
            if not nonDTS and doc is not None and doc not in self.referencesDocument:
                ...
                if not doc.inDTS and doc.type > Type.UnknownTypes:
                    doc.inDTS = True
                    if doc.type == Type.SCHEMA and not self.skipDTS:
                        doc.schemaDiscoverChildElements(doc.xmlRootElement)
                    href = (
    ...
    def instanceContentsDiscover(self, xbrlElement):
                ...
                if ns == XbrlConst.xbrli:
                    if ln == 'context':
                        ...
                        if ln == 'unit':
                            ...
                            continue
                        if ns in XbrlConst.ixbrlAll and ln == 'relationship':
                            pass
                        else:
        ...
        if len(self.modelXbrl.undefinedFacts) > nextUndefinedFact:
            ...
            self.modelXbrl.error('xbrl:schemaImportMissing', _('Instance facts missing schema concept definition: %(elements)s'), modelObject=undefFacts, elements=', '.join(sorted(set(str(f.prefixedName) for f in undefFacts))))
    ...
    def inlineXbrlDiscover(self, htmlElement):
        ...
        if conflictingNSelts:
            self.modelXbrl.error('ix:multipleIxNamespaces', _('Multiple ix namespaces were found'), modelObject=conflictingNSelts)
        self.ixNS = ixNS
        ...
        for inlineElement in htmlElement.iterdescendants(tag=ixNStag + 'resources'):
            ...
            for refElement in inlineElement.iterchildren('{http://www.xbrl.org/2003/linkbase}roleRef'):
                ...
                if self.discoverHref(refElement) is None:
                    self.modelXbrl.error('xmlSchema:requiredAttribute', _('Reference for roleURI href attribute missing or malformed'), modelObject=refElement)
            ...
            for refElement in inlineElement.iterchildren('{http://www.xbrl.org/2003/linkbase}arcroleRef'):
                ...
                if self.discoverHref(refElement) is None:
                    self.modelXbrl.error('xmlSchema:requiredAttribute', _('Reference for arcroleURI href attribute missing or malformed'), modelObject=refElement)
    ...
    def testcaseDiscover(self, testcaseElement):
        ...
        if len(self.testcaseVariations) == 0 and XbrlConst.ixbrlAll.intersection(testcaseElement.values()):
...
def inlineIxdsDiscover(modelXbrl, modelIxdsDocument):
                ...
                if modelInlineTuple.qname is not None:
                    if modelInlineTuple.tupleID:
                        if modelInlineTuple.tupleID not in tuplesByTupleID:
                            ...
                            modelXbrl.error(ixMsgCode('tupleIdDuplication', modelInlineTuple, sect='validation'), _('Inline XBRL tuples have same tupleID %(tupleID)s: %(qname1)s and %(qname2)s'), modelObject=(
                             modelInlineTuple, tuplesByTupleID[modelInlineTuple.tupleID]), tupleID=modelInlineTuple.tupleID, qname1=modelInlineTuple.qname, qname2=tuplesByTupleID[modelInlineTuple.tupleID].qname)
                ...
                for attrName, attrValue in elt.items():
                    if attrName.startswith('{') and not attrName.startswith(mdlDoc.ixNStag) and attrName != '{http://www.w3.org/XML/1998/namespace}base':
                        if attrName in targetReferenceAttrsDict:
                            modelXbrl.error(ixMsgCode('referencesAttributeDuplication', ns=mdlDoc.ixNS, name='references', sect='validation'), _('Inline XBRL ix:references attribute %(name)s duplicated in target %(target)s'), modelObject=(
                             elt, targetReferenceAttrsDict[attrName]), name=attrName, target=target)
                        else:
                            targetReferenceAttrsDict[attrName] = elt

                if elt.id:
                    if ixdsEltById[elt.id] != [elt]:
                        modelXbrl.error(ixMsgCode('referencesIdDuplication', ns=mdlDoc.ixNS, name='references', sect='validation'), _('Inline XBRL ix:references id %(id)s duplicated in inline document set'), modelObject=ixdsEltById[elt.id], id=elt.id)
                    if target in targetReferencesIDs:
                        modelXbrl.error(ixMsgCode('referencesTargetId', ns=mdlDoc.ixNS, name='references', sect='validation'), _('Inline XBRL has multiple ix:references with id in target %(target)s'), modelObject=(
                         elt, targetReferencesIDs[target]), target=target)
                    else:
                    ...
                    for _prefix, _ns in elt.nsmap.items():
                        if _prefix in targetReferencePrefixNsDict and _ns != targetReferencePrefixNsDict[_prefix][0]:
                            modelXbrl.error(ixMsgCode('referencesNamespacePrefixConflict', ns=mdlDoc.ixNS, name='references', sect='validation'), _('Inline XBRL ix:references prefix %(prefix)s has multiple namespaces %(ns1)s and %(ns2)s in target %(target)s'), modelObject=(
                             elt, targetReferencePrefixNsDict[_prefix][1]), prefix=_prefix, ns1=_ns, ns2=targetReferencePrefixNsDict[_prefix], target=target)
                        else:
            ...
            for subEltTag in ('{http://www.xbrl.org/2003/instance}context', '{http://www.xbrl.org/2003/instance}unit'):
                for resElt in elt.iterdescendants(tag=subEltTag):
                    if resElt.id and ixdsEltById[resElt.id] != [resElt]:
                        modelXbrl.error(ixMsgCode('resourceIdDuplication', ns=mdlDoc.ixNS, name='resources', sect='validation'), _('Inline XBRL ix:resources descendant id %(id)s duplicated in inline document set'), modelObject=ixdsEltById[resElt.id], id=resElt.id)

    if not hasResources:
        modelXbrl.error(ixMsgCode('missingResources', ns=mdlDoc.ixNS, name='resources', sect='validation'), _('Inline XBRL ix:resources element not found'), modelObject=modelXbrl)
    del ixdsEltById
    ...
    def locateFactInTuple(modelFact, tuplesByTupleID, ixNStag):
        ...
        if tupleRef:
            if tupleRef not in tuplesByTupleID:
                modelXbrl.error(ixMsgCode('tupleRefMissing', modelFact, sect='validation'), _('Inline XBRL tupleRef %(tupleRef)s not found'), modelObject=modelFact, tupleRef=tupleRef)
            else:
        ...
        if tuple is not None:
            if modelFact.order is not None:
                ...
                modelXbrl.error(ixMsgCode('tupleMemberOrderMissing', modelFact, sect='validation'), _('Inline XBRL tuple member %(qname)s must have a numeric order attribute'), modelObject=modelFact, qname=modelFact.qname)
        ...
        if contAt:
            ...
            if contAt not in continuationElements:
                if contAt in element.modelDocument.idObjects:
                    ...
                    modelXbrl.error(ixMsgCode('continuationTarget', element, sect='validation'), _('continuedAt %(continuationAt)s target is an %(targetElement)s element instead of ix:continuation element.'), modelObject=(
                     element, _contAtTarget), continuationAt=contAt, targetElement=_contAtTarget.elementQname)
                else:
                    modelXbrl.error(ixMsgCode('continuationMissing', element, sect='validation'), _('Inline XBRL continuation %(continuationAt)s not found'), modelObject=element, continuationAt=contAt)
            else:
                if chain is None:
                    chain = [element]
                ...
                if contElt in chain:
                    ...
                    modelXbrl.error(ixMsgCode('continuationCycle', element, sect='validation'), _('Inline XBRL continuation cycle: %(continuationCycle)s'), modelObject=chain, continuationCycle=cycle)
                else:
                    chain.append(contElt)
            ...
            for chainElt in chain:
                for chainEltAncestor in chainElt.iterancestors(tag=chainElt.modelDocument.ixNStag + '*'):
                    if chainEltAncestor in chainSet:
                        ...
                        modelXbrl.error(ixMsgCode('continuationChainNested', chainElt, sect='validation'), _('Inline XBRL continuation chain element %(ancestorElement)s has descendant element %(descendantElement)s'), modelObject=(
                         chainElt, chainEltAncestor), ancestorElement=chainEltAncestor.id or chainEltAncestor.get('name', chainEltAncestor.get('continuedAt')), descendantElement=chainElt.id or chainElt.get('name', chainElt.get('continuedAt')))
        ...
        for childElt in parentElt.iterchildren():
            if isinstance(childElt, ModelObject) and childElt.namespaceURI in XbrlConst.ixbrlAll:
                if childElt.localName in ('numerator', 'denominator'):
                    modelXbrl.error(ixMsgCode('tupleContentError', tupleFact, sect='validation'), _('Inline XBRL tuple content illegal %(qname)s'), modelObject=(
                     tupleFact, childElt), qname=childElt.qname)
                else:
    ...
    for htmlElement in modelXbrl.ixdsHtmlElements:
        ...
        for modelInlineFact in htmlElement.iterdescendants(tag=ixNStag + '*'):
            if isinstance(modelInlineFact, ModelInlineFact) and modelInlineFact.localName in ('nonNumeric',
                                                                                              'nonFraction',
                                                                                              'fraction'):
                factTargetIDs.add(modelInlineFact.get('target'))
                if modelInlineFact.qname is not None:
                    if modelInlineFact.concept is None:
                        modelXbrl.error(ixMsgCode('missingReferences', modelInlineFact, name='references', sect='validation'), _('Instance fact missing schema definition: %(qname)s of Inline Element %(localName)s'), modelObject=modelInlineFact, qname=modelInlineFact.qname, localName=modelInlineFact.elementQname)
                    else:
                        if modelInlineFact.isFraction == (modelInlineFact.localName == 'fraction'):
                            ...
                            modelXbrl.error(ixMsgCode('fractionDeclaration', modelInlineFact, name='fraction', sect='validation'), _('Inline XBRL element %(qname)s base type %(type)s mapped by %(localName)s'), modelObject=modelInlineFact, qname=modelInlineFact.qname, localName=modelInlineFact.elementQname, type=modelInlineFact.concept.baseXsdType)

        for tupleFact in tupleElements:
            for order, facts in tupleFact.unorderedTupleFacts.items():
                if len(facts) > 1:
                    if not all(normalizeSpace(facts[0].value) == normalizeSpace(f.value) and all(normalizeSpace(facts[0].get(attr)) == normalizeSpace(f.get(attr)) for attr in facts[0].keys() if attr != 'order') for f in facts[1:]):
                        modelXbrl.error(ixMsgCode('tupleSameOrderMembersUnequal', facts[0], sect='validation'), _('Inline XBRL tuple members %(qnames)s values %(values)s and attributes not whitespace-normalized equal'), modelObject=facts, qnames=', '.join(str(f.qname) for f in facts), values=', '.join(f.value for f in facts))
        ...
        def checkForTupleCycle(parentTuple, tupleNesting):
            for fact in parentTuple.modelTupleFacts:
                if fact in tupleNesting:
                    ...
                    modelXbrl.error(ixMsgCode('tupleNestingCycle', fact, sect='validation'), _('Tuple nesting cycle: %(tupleCycle)s'), modelObject=tupleNesting, tupleCycle='->'.join(str(t.qname) for t in tupleNesting))
                    tupleNesting.pop()
                else:
                    tupleNesting.append(fact)
    ...
    if len(targetReferenceAttrs) == 0:
        modelXbrl.error(ixMsgCode('missingReferences', None, name='references', sect='validation'), _('There must be at least one reference'), modelObject=modelXbrl)
    if factTargetIDs - set(targetReferenceAttrs.keys()):
        modelXbrl.error(ixMsgCode('missingReferenceTargets', None, name='references', sect='validation'), _('Instance fact targets without reference: %(missingReferenceTargets)s'), modelObject=modelXbrl, missingReferenceTargets=','.join(sorted(('(default)' if t is None else t) for t in factTargetIDs - set(targetReferenceAttrs.keys()))))
    ...
    for htmlElement in modelXbrl.ixdsHtmlElements:
        ...
        for modelInlineFootnote in htmlElement.iterdescendants(tag=XbrlConst.qnIXbrlFootnote.clarkNotation):
            if isinstance(modelInlineFootnote, ModelObject):
                ...
                linkPrototype.childElements.append(ArcPrototype(mdlDoc, linkPrototype, XbrlConst.qnLinkFootnoteArc, footnoteLocLabel, footnoteID, linkrole, arcrole, sourceElement=modelInlineFootnote))
        ...
        for modelInlineRel in htmlElement.iterdescendants(tag=XbrlConst.qnIXbrl11Relationship.clarkNotation):
            if isinstance(modelInlineRel, ModelObject):
                ...
                if toIdsNotFound:
                    modelXbrl.error(ixMsgCode('relationshipToRef', ns=XbrlConst.ixbrl11, name='relationship', sect='validation'), _('Inline relationship toRef(s) %(toIds)s not found.'), modelObject=modelInlineRel, toIds=', '.join(sorted(toIdsNotFound)))
                if fromToMatchedIds:
                    modelXbrl.error(ixMsgCode('relationshipFromToMatch', ns=XbrlConst.ixbrl11, name='relationship', sect='validation'), _('Inline relationship has matching values in fromRefs and toRefs: %(fromToMatchedIds)s'), modelObject=modelInlineRel, fromToMatchedIds=', '.join(sorted(fromToMatchedIds)))
                for fromLabel in fromLabels:
                    for toLabel in toLabels:
                        linkPrototype.childElements.append(ArcPrototype(mdlDoc, linkPrototype, XbrlConst.qnLinkFootnoteArc, fromLabel, toLabel, linkrole, arcrole, modelInlineRel.get('order', '1'), sourceElement=modelInlineRel))

                if toFootnoteIds and toFactQnames:
                    modelXbrl.error(ixMsgCode('relationshipReferencesMixed', ns=XbrlConst.ixbrl11, name='relationship', sect='validation'), _('Inline relationship references footnote(s) %(toFootnoteIds)s and thereby is not allowed to reference %(toFactQnames)s.'), modelObject=modelInlineRel, toFootnoteIds=', '.join(sorted(toFootnoteIds)), toFactQnames=', '.join(sorted(toFactQnames)))
    ...
    for _contAt, _contReferences in continuationReferences.items():
        if len(_contReferences) > 1:
            ...
            modelXbrl.error(ixMsgCode('continuationReferences', ns=XbrlConst.ixbrl11, name='continuation', sect='validation'), _('continuedAt %(continuedAt)s has %(referencesCount)s references on %(sourceElements)s elements, only one reference allowed.'), modelObject=_contReferences, continuedAt=_contAt, referencesCount=len(_contReferences), sourceElements=', '.join(str(qn) for qn in sorted(_refEltQnames)))

    for _contAt, _contElt in continuationElements.items():
        if _contAt not in continuationReferences:
            modelXbrl.error(ixMsgCode('continuationNotReferenced', ns=XbrlConst.ixbrl11, name='continuation', sect='validation'), _('ix:continuation %(continuedAt)s is not referenced by a, ix:footnote, ix:nonNumeric or other ix:continuation element.'), modelObject=_contElt, continuedAt=_contAt)
