...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/FunctionIxt.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 37402 bytes
...
gregorianHindiMonthNumber = {'जनवरी': '01', 
 ...
 'नवम्बर': '11', 
 'दिसम्बर': '12'}
sakaMonthNumber = {'Chaitra': 1, 'चैत्र': 1, 
...
def sakaToGregorian(sYr, sMo, sDay):
    ...
    if gMo == 2 and gYr % 4 == 0 and (not gYr % 100 == 0 or gYr % 400 == 0):
        gMoLength += 1
...
def datemonthyeardk(arg):
    ...
    if m and m.lastindex == 4:
        ...
        monEnd = m.group(2)
        monPer = m.group(3)
        if mon3 in monthnumber and (not monEnd and not monPer or not monEnd and monPer or monEnd and not monPer):
            pass
...
def datedaymonthyeardk(arg):
    ...
    if m and m.lastindex == 5:
        ...
        _monEnd = m.group(3)
        _monPer = m.group(4)
        if _mon3 in monthnumber and (not _monEnd and not _monPer or not _monEnd and _monPer or _monEnd and not _monPer):
