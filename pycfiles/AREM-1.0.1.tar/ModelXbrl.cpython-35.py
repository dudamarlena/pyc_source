...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/ModelXbrl.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 62965 bytes
...
class ModelXbrl:
    ...
    def matchContext(self, entityIdentScheme, entityIdentValue, periodType, periodStart, periodEndInstant, dims, segOCCs, scenOCCs):
        ...
        for c in self.contexts.values():
            if c.entityIdentifier == (entityIdentScheme, entityIdentValue) and (c.isInstantPeriod and periodType == 'instant' and dateUnionEqual(c.instantDatetime, periodEndInstant, instantEndDate=True) or c.isStartEndPeriod and periodType == 'duration' and dateUnionEqual(c.startDatetime, periodStart) and dateUnionEqual(c.endDatetime, periodEndInstant, instantEndDate=True) or c.isForeverPeriod and periodType == 'forever') and (dims is None or c.qnameDims.keys() == dims.keys() and all([cDim.isEqualTo(dims[cDimQn]) for cDimQn, cDim in c.qnameDims.items()])) and all((all([sEqual(self, cOCCs[i], mOCCs[i]) for i in range(len(mOCCs))]) if len(cOCCs) == len(mOCCs) else False) for cOCCs, mOCCs in (
             (
    ...
    def createContext(self, entityIdentScheme, entityIdentValue, periodType, periodStart, periodEndInstant, priItem, dims, segOCCs, scenOCCs, afterSibling=None, beforeSibling=None, id=None):
        ...
        newCntxElt = XmlUtil.addChild(xbrlElt, XbrlConst.xbrli, 'context', attributes=('id', cntxId), afterSibling=afterSibling, beforeSibling=beforeSibling)
        ...
        XmlUtil.addChild(entityElt, XbrlConst.xbrli, 'identifier', attributes=(
         'scheme', entityIdentScheme), text=entityIdentValue)
        ...
        if dims:
            if priItem is not None:
                ...
                if not isFactDimensionallyValid(self, fp, setPrototypeContextElements=True):
                    self.info('arelle:info', _('Create context for %(priItem)s, cannot determine valid context elements, no suitable hypercubes'), modelObject=self, priItem=priItem)
            ...
            for dimQname in sorted(fpDims.keys()):
                ...
                if contextEltName == 'segment':
                    ...
                    if contextEltName == 'scenario':
                        ...
                        (self.info('arelleLinfo', _('Create context, %(dimension)s, cannot determine context element, either no all relationship or validation issue'), modelObject=self, dimension=dimQname),)
                        continue
                dimAttr = (
                ...
                if dimValue.isTyped:
                    ...
                    dimElt = XmlUtil.addChild(contextElt, XbrlConst.xbrldi, 'xbrldi:explicitMember', attributes=dimAttr, text=XmlUtil.addQnameValue(xbrlElt, dimMemberQname))
    ...
    def createUnit(self, multiplyBy, divideBy, afterSibling=None, beforeSibling=None, id=None):
        ...
        newUnitElt = XmlUtil.addChild(xbrlElt, XbrlConst.xbrli, 'unit', attributes=('id', unitId), afterSibling=afterSibling, beforeSibling=beforeSibling)
        if len(divideBy) == 0:
            ...
            if not matchId or otherFact.id == fact.id:
                ...
                if fact.isMultiLanguage and matchLang:
                    if fact.xmlLang == otherFact.xmlLang:
                        return fact
                elif fact.decimals == otherFact.decimals and fact.precision == otherFact.precision:
                    return fact
    ...
    def createFact(self, conceptQname, attributes=None, text=None, parent=None, afterSibling=None, beforeSibling=None, validate=True):
        ...
        newFact = XmlUtil.addChild(parent, conceptQname, attributes=attributes, text=text, afterSibling=afterSibling, beforeSibling=beforeSibling)
        ...
        try:
            if activityCompleted:
                ...
                if timeTaken > minTimeToShow:
                    self.info('info:profileActivity', _('%(activity)s %(time)s secs\n'), modelObject=self.modelXbrl.modelDocument, activity=activityCompleted, time=format_string(self.modelManager.locale, '%.3f', timeTaken, grouping=True))
        except AttributeError:
            pass
    ...
    def saveDTSpackage(self):
        ...
        self.info('info', _('DTS of %(entryFile)s has %(numberOfFiles)s files packaged into %(packageOutputFile)s'), modelObject=self, entryFile=os.path.basename(entryFilename), packageOutputFile=pkgFilename, numberOfFiles=numFiles)