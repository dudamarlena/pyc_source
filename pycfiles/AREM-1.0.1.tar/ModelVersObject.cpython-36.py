...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelVersObject.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 27240 bytes
...
class ModelConceptChange(ModelVersObject):
    ...
    @property
    def physical(self):
        return self.get('physical') or 'true'
    ...
    def fromConceptQname(self):
        fromConcept = XmlUtil.child(self, None, 'fromConcept')
        if fromConcept is not None:
            if fromConcept.get('name'):
    ...
    def setConceptEquivalence(self):
        if self.fromConcept is not None:
            if self.toConcept is not None:
    ...
    def viewText(self, labelrole=XbrlConst.conceptNameLabelRole, lang=None):
        ...
        toConcept = self.toConcept
        if labelrole != XbrlConst.conceptNameLabelRole:
            if fromConceptQname is None or fromConceptQname is not None and fromConcept is not None:
                if toConceptQname is None or toConceptQname is not None and toConcept is not None:
...
class ModelConceptDetailsChange(ModelConceptChange):
    ...
    def customAttributeQname(self, eventName):
        custAttrElt = XmlUtil.child(self, None, eventName)
        if custAttrElt is not None:
            if custAttrElt.get('name'):
...
class ModelRelationshipSet(ModelVersObject):
    ...
    def dts(self):
        if self.isFromDTS:
            return self.modelDocument.fromDTS
        else:
            return self.modelDocument.toDTS
...
class ModelRelationships(ModelVersObject):
    ...
    def toName(self):
        if self.get('toName'):
            return self.prefixedNameQname(self.get('toName'))

    @property
    def fromConcept(self):
        if self.fromName:
            return self.modelRelationshipSet.dts.qnameConcepts.get(self.fromName)

    @property
    def toConcept(self):
        if self.toName:
            return self.modelRelationshipSet.dts.qnameConcepts.get(self.toName)
...
class ModelInstanceAspect(ModelVersObject):
    ...
    @property
    def propertyView(self):
        return self.modelAspects.propertyView + (('aspect', self.localName),) + self.elementAttributesTuple
...
class ModelConceptsDimsAspect(ModelInstanceAspect):
    ...
    def conceptName(self):
        if self.get('name'):
            return self.prefixedNameQname(self.get('name'))

    @property
    def concept(self):
        if self.conceptName:
            return self.modelAspects.dts.qnameConcepts.get(self.conceptName)
...
class ModelRelatedConcept(ModelVersObject):
    ...
    def concept(self):
        if self.conceptName:
            return self.modelAspect.modelAspects.dts.qnameConcepts.get(self.conceptName)
    ...
    def arc(self):
        ...
        if arc:
            return self.prefixedNameQname(arc)

    @property
    def link(self):
        link = XmlUtil.childAttr(self, XbrlConst.verdim, ('network', 'drsNetwork'), 'link')
        if link:
            return self.prefixedNameQname(link)

    @property
    def propertyView(self):
        return self.modelAspect.propertyView + ((self.localName, ''),) + self.elementAttributesTuple
