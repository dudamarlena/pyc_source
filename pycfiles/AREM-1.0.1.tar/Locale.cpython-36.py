...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\Locale.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 26398 bytes
...
def format_picture(conv, value, picture):
    ...
    if value.is_nan():
        ...
        if not numPlaces:
            if prefix != minus_sign:
                raise ValueError(_('Sub-picture must contain at least one digit position or sign character {0}').format(pic))
        if intPlaces == 0:
            if fractPlaces == 0:
                intPlaces = 1
        return format_decimal(None, value, intPlaces=intPlaces, fractPlaces=fractPlaces, sep=thousands_sep,
          dp=decimal_point,
          grouping=grouping,
          pos=prefix,
