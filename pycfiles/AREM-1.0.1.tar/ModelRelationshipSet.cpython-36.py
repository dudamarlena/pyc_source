...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelRelationshipSet.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 20957 bytes
    ...
    def __init__(self, modelXbrl, arcrole, linkrole=None, linkqname=None, arcqname=None, includeProhibits=False):
        ...
        for modelLink in modelLinks:
            ...
            for linkChild in modelLink:
                ...
                if linkChild.get('{http://www.w3.org/1999/xlink}type') == 'arc':
                    if linkChildArcrole:
                        if isFootnoteRel:
                            ...
                            if isDimensionRel:
                                ...
                                if isFormulaRel:
                                    ...
                                    if isTableRenderingRel:
                                        ...
                                        if arcqname is None or arcqname == linkChild.qname:
                                            if linkqname is None or linkqname == linkEltQname:
                                                arcs.append(linkChild)
    ...
    def isRelated(self, modelFrom, axis, modelTo=None, visited=None, isDRS=False):
        if isinstance(modelFrom, ModelValue.QName):
            ...
            if isinstance(modelTo, ModelValue.QName):
                ...
                if modelTo is None:
                    return False
            if axis.endswith('self'):
                if modelTo is None or modelFrom == modelTo:
                    return True
