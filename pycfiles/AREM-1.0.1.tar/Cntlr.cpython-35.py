...
# Embedded file name: /home/nagat/.local/lib/python3.5/site-packages/arelle/Cntlr.py
# Compiled at: 2018-08-09 04:25:27
# Size of source mod 2**32: 37212 bytes
    ...
    def __init__(self, hasGui=False, logFileName=None, logFileMode=None, logFileEncoding=None, logFormat=None):
        ...
        if self.hasFileSystem:
            if not os.path.exists(self.userAppDir):
                ...
                self.userAppDir = '/home/nagat/.local/lib/python3.5/site-packages/arelle'
                if not os.path.exists(self.userAppDir):
    ...
    def startLogging(self, logFileName=None, logFileMode=None, logFileEncoding=None, logFormat=None, logLevel=None, logHandler=None, logToBuffer=False):
                ...
                if logFileName in ('logToPrint', 'logToStdErr') and not logToBuffer:
                    ...
                    if logFileName == 'logToBuffer':
                        ...
                        if logFileName.endswith('.xml') or logFileName.endswith('.json') or logToBuffer:
                            ...
                            self.logHandler = logging.FileHandler(filename=logFileName, mode=logFileMode or 'a', encoding=logFileEncoding or 'utf-8')
        ...
        if self.logger:
            try:
                ...
                self.addToLog(_('Unknown log level name: {0}, please choose from {1}').format(logLevel, ', '.join(logging.getLevelName(l).lower() for l in sorted([i for i in logging.loggingLevelNums.keys() if isinstance(i, _INT_TYPES) and i > 0]))), level=logging.ERROR, messageCode='arelle:logLevel')
...
class LogFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        try:
            ...
            if isinstance(record.args, dict) and 'error' in record.args:
...
class LogToPrintHandler(logging.Handler):
    ...
    def emit(self, logRecord):
        ...
        try:
            ...
            print(logEntry.encode(sys.stdout.encoding, 'backslashreplace').decode(sys.stdout.encoding, 'strict'), file=file)
                ...
                if self.filename in ('logToPrint', 'logToStdErr'):
                    ...
                    for logRec in self.logRecordBuffer:
                        ...
                        try:
                            ...
                            print(logEntry.encode(sys.stdout.encoding, 'backslashreplace').decode(sys.stdout.encoding, 'strict'), file=_file)
