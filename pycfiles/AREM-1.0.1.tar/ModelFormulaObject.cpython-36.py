...
# Embedded file name: D:\ProgramData\lib\site-packages\arelle\ModelFormulaObject.py
# Compiled at: 2018-02-26 09:10:06
# Size of source mod 2**32: 129385 bytes
...
aspectModelAspect = {Aspect.VALUE: Aspect.ENTITY_IDENTIFIER, Aspect.SCHEME: Aspect.ENTITY_IDENTIFIER, 
...
aspectRuleAspects = {Aspect.ENTITY_IDENTIFIER: (Aspect.VALUE, Aspect.SCHEME), 
...
aspectFromToken = {'location':Aspect.LOCATION, 
 'concept':Aspect.CONCEPT,  'entityIdentifier':Aspect.ENTITY_IDENTIFIER, 
 'entity-identifier':Aspect.ENTITY_IDENTIFIER,  'period':Aspect.PERIOD, 
 'unit':Aspect.UNIT,  'nonXDTSegment':Aspect.NON_XDT_SEGMENT, 
 'non-XDT-segment':Aspect.NON_XDT_SEGMENT,  'nonXDTScenario':Aspect.NON_XDT_SCENARIO, 
 'non-XDT-scenario':Aspect.NON_XDT_SCENARIO,  'dimension':Aspect.DIMENSIONS, 
 'dimensions':Aspect.DIMENSIONS,  'segment':Aspect.COMPLETE_SEGMENT, 
 'complete-segment':Aspect.COMPLETE_SEGMENT,  'scenario':Aspect.COMPLETE_SCENARIO, 
 'complete-scenario':Aspect.COMPLETE_SCENARIO}
aspectToToken = {Aspect.LOCATION: 'location', Aspect.CONCEPT: 'concept', 
...
aspectElementNameAttrValue = {Aspect.LOCATION_RULE: ('location', XbrlConst.tuple, None, None), 
...
class FormulaOptions:
    ...
    def typedParameters(self, prefixedNamespaces=None):
        return dict((qname(paramName, prefixedNamespaces), paramValue) for paramName, paramValue in self.parameterValues.items())
            ...
            for modelRel in self.modelXbrl.relationshipSet(arcrole).fromModelObject(self):
                ...
                if isinstance(toModelObject, ModelFormulaResource):
                    ...
                    if toModelObject is not None:
                        self.modelXbrl.error('formula:internalError', (_('Invalid formula object %(element)s')),
                          modelObject=self,
...
class ModelVariableSet(ModelFormulaResource):
    ...
    def descendantArcroles(self):
        return (
         XbrlConst.variableSet, XbrlConst.variableSetFilter, XbrlConst.variableSetPrecondition)
...
class ModelFormulaRules:
                ...
                if isinstance(ruleElt, ModelObject):
                    ...
                    if name == 'qname':
                        ...
                        if ruleElt.getparent().localName == 'concept':
                            ...
                            if ruleElt.getparent().getparent().get('dimension') is not None:
                                qnDim = qname(ruleElt.getparent().getparent(), ruleElt.getparent().getparent().get('dimension'))
                                if qnDim in self.aspectValues:
                                    self.modelXbrl.error('xbrlfe:conflictingAspectRules', (_('Dimension %(dimension)s aspect has multiple rules in formula.')),
                                      modelObject=self,
                                      dimension=qnDim)
                                self.aspectValues[qnDim] = value
                    else:
                        if name == 'qnameExpression':
                            if ruleElt.getparent().localName == 'concept':
                                exprs = [
                                 (
                                  Aspect.CONCEPT, XmlUtil.text(ruleElt))]
                            else:
                                if ruleElt.getparent().getparent().get('dimension') is not None:
                                    exprs = [
                                     (
                                      qname(ruleElt.getparent().getparent(), ruleElt.getparent().getparent().get('dimension')), XmlUtil.text(ruleElt))]
                        else:
                            if name == 'omit' and ruleElt.getparent().get('dimension') is not None:
                                ...
                                if name == 'entityIdentifier':
                                    ...
                                    if name == 'instant':
                                        ...
                                        if ruleElt.get('value') is not None:
                                            exprs = [
                                             (
                                              Aspect.INSTANT, ruleElt.get('value'))]
                                    else:
                                        if name == 'duration':
                                            self.aspectValues[Aspect.PERIOD_TYPE] = name
                                            ...
                                            if name == 'forever':
                                                ...
                                                if name == 'multiplyBy':
                                                    if ruleElt.get('measure') is not None:
                                                        exprs = [
                                                         (
                                                          Aspect.MULTIPLY_BY, ruleElt.get('measure'))]
                                                    ...
                                                    if name == 'divideBy':
                                                        ...
                                                        if name in ('occEmpty', 'occFragments',
                                                            ...
                                                            if ruleElt.get('occ') == 'segment':
                                                                ...
                                                                if name == 'occFragments':
                                                                    ...
                                                                    if name == 'occXpath':
                                                                        exprs = [
                                                                         (
                                                                          aspect, ruleElt.get('select'))]
                                                                    else:
                                                                        if name == 'occEmpty':
                                                                            self.aspectValues[aspect].insert(0, XbrlConst.qnFormulaOccEmpty)
                                                        elif name in ('explicitDimension',
                                                                      'typedDimension') and ruleElt.get('dimension') is not None:
                                                            qnDim = qname(ruleElt, ruleElt.get('dimension'))
                                                            self.aspectValues[Aspect.DIMENSIONS].append(qnDim)
                                                            if not XmlUtil.hasChild(ruleElt, XbrlConst.formula, ('omit',
                                                                                                                 'member',
                                                                                                                 'value',
                                                                                                                 'xpath')):
                                                                if qnDim in self.aspectValues:
                                                            ...
                                                            if name == 'precision':
                                                                exprs = [
                                                                 (
                                                                  Aspect.PRECISION, XmlUtil.text(ruleElt))]
                                                                self.hasPrecision = True
                                                            else:
                                                                if name == 'decimals':
                                                                    exprs = [
                                                                     (
                                                                      Aspect.DECIMALS, XmlUtil.text(ruleElt))]
    ...
    def evaluateRule(self, xpCtx, aspect):
        ...
        elif aspect in self.aspectValues:
            ...
            if aspect in (Aspect.CONCEPT, Aspect.MULTIPLY_BY, Aspect.DIVIDE_BY) or isinstance(aspect, QName):
                ...
                if aspect == Aspect.ENTITY_IDENTIFIER:
                    ...
                    if aspect == Aspect.START:
                        ...
                        if aspect in (Aspect.INSTANT, Aspect.END, Aspect.INSTANT_END):
                            type = 'xs:DATETIME_INSTANT_END'
                        else:
                            if aspect in (Aspect.DECIMALS, Aspect.PRECISION):
                                type = 'xs:float'
                            else:
                                type = 'xs:string'
            ...
            if xpCtx:
                if aspect in self.aspectProgs:
                    for prog in self.aspectProgs[aspect]:
                        if aspect in self.typedDimProgAspects:
    ...
    def source(self, aspect=None, ruleElement=None, acceptFormulaSource=True):
        if aspect is None:
            if ruleElement is None:
                if self.get('source'):
                    return qname(self, (self.get('source')), noPrefixIsNoNamespace=True)
                else:
                    return
            if ruleElement is None:
                ...
                if len(ruleElements) > 0:
                    ruleElement = ruleElements[0]
        else:
            if ruleElement is None:
                if aspect not in (Aspect.MULTIPLY_BY, Aspect.DIVIDE_BY, Aspect.LOCATION_RULE):
                    ruleElement = self
    ...
    def aspectRuleElements(self, aspect):
        if aspect in aspectElementNameAttrValue:
            ...
            if isinstance(aspect, QName):
                return [d for d in XmlUtil.descendants(self, XbrlConst.formula, ('explicitDimension',
                                                                                 'typedDimension')) if aspect == qname(d, d.get('dimension'))]
            return []
...
class ModelConsistencyAssertion(ModelFormulaResource):
    ...
    def radiusExpression(self):
        if self.get('proportionalAcceptanceRadius') is not None:
            ...
            if self.get('absoluteAcceptanceRadius') is not None:
...
class ModelParameter(ModelFormulaResource):
    ...
    def init(self, modelDocument):
        ...
        if self.parameterQname in self.modelXbrl.qnameParameters:
            self.modelXbrl.error('xbrlve:parameterNameClash', (_('Parameter name used on multiple parameters %(name)s')),
              modelObject=self,
              name=(self.parameterQname))
        else:
...
class ModelFactVariable(ModelVariable):
    ...
    @property
    def viewExpression(self):
        if self.fallbackValue:
            return 'fallbackValue =' + self.fallbackValue
        else:
            return ''
...
class ModelConceptName(ModelFilter):
    ...
    def qnameExpressions(self):
        return [XmlUtil.text(qnameExpression) for qnameExpression in XmlUtil.descendants(self, XbrlConst.cf, 'qnameExpression')]
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        if not self.qnameExpressionProgs:
            qnamedFacts = (set.union)(*[inst.factsByQname[qn] for inst in varBinding.instances for qn in self])
            if cmplmt:
                return facts - qnamedFacts
            return facts & qnamedFacts
        else:
            return set(fact for fact in facts if cmplmt ^ (fact.qname in self.conceptQnames | self.evalQnames(xpCtx, fact)))
...
class ModelConceptPeriodType(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        factsOfPeriodType = (set.union)(*[inst.factsByPeriodType(self.periodType) for inst in varBinding.instances])
        if cmplmt:
            return facts - factsOfPeriodType
        else:
            return facts & factsOfPeriodType
...
class ModelConceptBalance(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.concept.balance == self.balance if fact.concept.balance else self.balance == 'none'))
...
class ModelConceptFilterWithQnameExpression(ModelFilter):
    ...
    def qnameExpression(self):
        ...
        if qnExprElt is not None:
            return XmlUtil.text(qnExprElt)
...
class ModelConceptCustomAttribute(ModelConceptFilterWithQnameExpression):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts for qn in (
         self.evalQname(xpCtx, fact),))
    ...
    def viewExpression(self):
        if self.value:
            return XmlUtil.innerTextList(self) + (' = ' + self.value)
        else:
            return ''
...
class ModelConceptDataType(ModelConceptFilterWithQnameExpression):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        if self.filterQname:
            factsOfType = (set.union)(*[inst.factsByDatatype(notStrict, self.filterQname) for inst in varBinding.instances])
            if cmplmt:
                return facts - factsOfType
            return facts & factsOfType
        else:
            return set(fact for fact in facts for qn in (
             self.evalQname(xpCtx, fact),))
...
class ModelConceptSubstitutionGroup(ModelConceptFilterWithQnameExpression):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        if self.strict == 'true':
            return set(fact for fact in facts if cmplmt ^ (fact.concept.substitutionGroupQname == self.evalQname(xpCtx, fact)))
        else:
            return set(fact for fact in facts if cmplmt ^ fact.concept.substitutesForQname(self.evalQname(xpCtx, fact)))
...
class ModelConceptRelation(ModelFilter):
    ...
    def variableRefs(self, progs=[], varRefSet=None):
        if self.variable:
            if self.variable != XbrlConst.qnXfiRoot:
                if varRefSet is None:
                    varRefSet = set()
                varRefSet.add(self.variable)
        return super(ModelConceptRelation, self).variableRefs([p for p in (self.sourceQnameExpressionProg,
         self.linkroleExpressionProg, self.linknameExpressionProg,
         self.arcroleExpressionProg, self.arcnameExpressionProg) if p], varRefSet)
        ...
        if not sourceQname:
            ...
            for fact in facts:
                ...
                for modelRel in relationships:
                    if isFromAxis and isinstance(modelRel.fromModelObject, ModelObject) and factQname == modelRel.fromModelObject.qname or not isFromAxis and isinstance(modelRel.toModelObject, ModelObject) and factQname == modelRel.toModelObject.qname:
                        if hasNoTest or self.evalTest(xpCtx, modelRel):
                            factOk = True
                            break
                ...
                if not factOk:
                    if axis.startswith('sibling'):
                        if factQname != sourceQname:
                            if not concept_relationships(xpCtx, None, (sourceQname, linkrole, arcrole, 'parent', 1, linkQname, arcQname)):
                                if not concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'parent', 1, linkQname, arcQname)):
                                    if concept_relationships(xpCtx, None, (factQname, linkrole, arcrole, 'child', 1, linkQname, arcQname)):
                                        factOk = True
                if not factOk:
...
class ModelEntityIdentifier(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and self.evalTest(xpCtx, fact.context.entityIdentifierElement)))
...
class ModelEntitySpecificIdentifier(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.entityIdentifier[0] == xpCtx.evaluateAtomicValue(self.schemeProg, 'xs:string', fact) and fact.context.entityIdentifier[1] == xpCtx.evaluateAtomicValue(self.valueProg, 'xs:string', fact)))
...
class ModelEntityScheme(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.entityIdentifier[0] == xpCtx.evaluateAtomicValue(self.schemeProg, 'xs:string', fact)))
...
class ModelEntityRegexpIdentifier(ModelPatternFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and self.rePattern.search(fact.context.entityIdentifierElement.xValue) is not None))
...
class ModelEntityRegexpScheme(ModelPatternFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and self.rePattern.search(fact.context.entityIdentifier[0]) is not None))
...
class ModelGeneral(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        if self.isFilterShared and self.hasNoFilterVariableDependencies(xpCtx):
            if self in xpCtx.cachedFilterResults:
                qualifyingFacts = xpCtx.cachedFilterResults[self]
            else:
                xpCtx.cachedFilterResults[self] = qualifyingFacts = set(fact for inst in varBinding.instances for fact in inst.factsInInstance)
            if cmplmt:
                return facts - qualifyingFacts
            return facts & qualifyingFacts
        else:
            return set(fact for fact in facts if cmplmt ^ self.evalTest(xpCtx, fact))
...
class ModelMatchFilter(ModelFilter):
    ...
    def dimension(self):
        if self.get('dimension'):
            return qname(self, self.get('dimension'))
    ...
    def variable(self):
        if self.get('variable'):
            return qname(self, (self.get('variable')), noPrefixIsNoNamespace=True)
        ...
        if isinstance(otherFact, (tuple, list)):
            ...
            for fact in otherFact:
                if not isinstance(fact, ModelFact):
                    ...
                    if matchAll:
                        if not aspectMatches(xpCtx, fact, firstFact, aspect):
                            raise XPathContext.XPathException(xpCtx, 'xbrlmfe:inconsistentMatchedVariableSequence', _('Matched variable sequence includes fact {0} inconsistent in aspect {1}').format(str(fact), aspectStr(aspect)))
                            return cmplmt
            ...
            if matchAll:
                otherFact = firstFact
            return isinstance(otherFact, (ModelFact, tuple, list)) or set()
        else:
            if matchAll:
                return set(fact for fact in facts if cmplmt ^ aspectMatches(xpCtx, fact, otherFact, aspect))
            return set(fact for fact in facts if cmplmt ^ any(aspectMatches(xpCtx, fact, anotherFact, aspect) for anotherFact in otherFact))
    ...
    def propertyView(self):
        return (
         ...
         (
          'matchAny', self.matchAny.lower())('variable', self.variable))
...
class ModelPeriod(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and self.evalTest(xpCtx, fact.context.period)))
...
class ModelPeriodStart(ModelDateTimeFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.startDatetime == self.evalDatetime(xpCtx, fact, addOneDay=False)))
...
class ModelPeriodEnd(ModelDateTimeFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.isStartEndPeriod and fact.context.endDatetime == self.evalDatetime(xpCtx, fact, addOneDay=True)))
...
class ModelPeriodInstant(ModelDateTimeFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.instantDatetime == self.evalDatetime(xpCtx, fact, addOneDay=True)))
...
class ModelForever(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.isForeverPeriod))
...
class ModelInstantDuration(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        otherFact = xpCtx.inScopeVars.get(self.variable)
        if otherFact is not None and isinstance(otherFact, ModelFact) and otherFact.isItem and otherFact.context is not None and otherFact.context.isStartEndPeriod:
            if self.boundary == 'start':
                otherDatetime = otherFact.context.startDatetime
            else:
                otherDatetime = otherFact.context.endDatetime
            return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context is not None and fact.context.isInstantPeriod and fact.context.instantDatetime == otherDatetime))
        else:
            return facts
...
class ModelExplicitDimension(ModelFilter):
    ...
    def compile(self):
        if not hasattr(self, 'dimQnameExpressionProg'):
            ...
            for memberElt in XmlUtil.children(self, XbrlConst.df, 'member'):
                ...
                memberModel = MemberModel(qname(qnameElt, XmlUtil.text(qnameElt)) if qnameElt is not None else None, XPathParser.parse(self, XmlUtil.text(qnameExpr), memberElt, 'memQnameExpressionProg', Trace.VARIABLE) if qnameExpr is not None else None, qname(variableElt, (XmlUtil.text(variableElt)), noPrefixIsNoNamespace=True) if variableElt is not None else None, XmlUtil.text(linkrole) if linkrole is not None else None, XmlUtil.text(arcrole) if arcrole is not None else None, XmlUtil.text(axis) if axis is not None else None)
                self.memberProgs.append(memberModel)

            super(ModelExplicitDimension, self).compile()
            self.isFilterStatic = self.dimQname and not self.dimQnameExpressionProg and all(mp.isMemberStatic for mp in self.memberProgs)
            if self.isFilterStatic:
                ...
                if dimConcept is None or not dimConcept.isExplicitDimension:
                    self.modelXbrl.error('xfie:invalidExplicitDimensionQName', (_('%(dimension)s is not an explicit dimension concept QName.')),
                      modelObject=self,
                      dimension=(self.dimQname))
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        if self.isFilterStatic:
            ...
            if memQnames:
                dimedFacts = (set.union)(*[inst.factsByDimMemQname(dimQname, memQname) for inst in varBinding.instances for memQname in memQnames])
            else:
                dimedFacts = (set.union)(*[inst.factsByDimMemQname(dimQname) for inst in varBinding.instances])
            if cmplmt:
                return facts - dimedFacts
            return facts & dimedFacts
        else:
            outFacts = set()
            for fact in facts:
                        ...
                        if len(self.memberProgs) > 0:
                            ...
                            for memberModel in self.memberProgs:
                                ...
                                if not memberModel.axis or memberModel.axis.endswith('-self'):
                                    if matchMemQname == memQname:
                                        factOk = True
                                        break
                                    else:
                                        if memberModel.axis:
                                            if memberModel.linkrole:
                                                if memberModel.arcrole:
                                                    ...
                                                    if relSet:
                                                        if relSet.isRelated(matchMemQname, memberModel.axis, memQname):
                                                            factOk = True
                                                            break
                                                        elif not domainMembersExist:
                                                            pass
                                                        if relSet.isRelated(matchMemQname, memberModel.axis):
                                                            domainMembersExist = True
    ...
    def propertyView(self):
        members = []
        for memberElt in XmlUtil.children(self, XbrlConst.df, 'member'):
            member = XmlUtil.childText(memberElt, XbrlConst.df, ('qname', 'qnameExpression',
                                                                 'variable'))
...
class ModelTypedDimension(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        for fact in facts:
            ...
            if cmplmt ^ (dim is not None and (not self.test or self.evalTest(xpCtx, dim))):
                outFacts.add(fact)
...
class ModelRelativeFilter(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        return set(fact for fact in facts if cmplmt ^ (hasOtherFactVar and aspectsMatch(xpCtx, otherFact, fact, aspectsUncovered) and (fact.isTuple or all(aspectMatches(xpCtx, otherFact, fact, dimAspect) for dimAspect in fact.context.dimAspects(xpCtx.defaultDimensionAspects) if not varBinding.hasAspectValueCovered(dimAspect) if not otherVarBinding.hasAspectValueCovered(dimAspect)))))
...
class ModelSegmentFilter(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.hasSegment and self.evalTest(xpCtx, fact.context.segment)))
...
class ModelScenarioFilter(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.context.hasScenario and self.evalTest(xpCtx, fact.context.scenario)))
...
class ModelAncestorFilter(ModelFilter):
    ...
    @property
    def viewExpression(self):
        if self.ancestorQname:
            return self.ancestorQname
        else:
            return self.qnameExpression
...
class ModelLocationFilter(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        if isinstance(varFacts, ModelFact):
            ...
            if isinstance(varFacts, (list, tuple)):
                candidateElts = set(f for f in varFacts if isinstance(f, ModelFact))
        return set(fact for fact in facts if cmplmt ^ (len(candidateElts & self.evalLocation(xpCtx, fact)) > 0))
...
class ModelSiblingFilter(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        while isinstance(otherFact, (list, tuple)) and len(otherFact) > 0:
            otherFact = otherFact[0]

        if isinstance(otherFact, ModelFact):
            otherFactParent = otherFact.parentElement
        else:
            otherFactParent = None
        return set(fact for fact in facts if cmplmt ^ (fact.parentElement == otherFactParent))
...
class ModelGeneralMeasures(ModelTestFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isItem and fact.isNumeric and self.evalTest(xpCtx, fact.unit)))
...
class ModelSingleMeasure(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ (fact.isNumeric and fact.unit.isSingleMeasure and fact.unit.measures[0][0] == self.evalQname(xpCtx, fact)))
...
class ModelNilFilter(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        return set(fact for fact in facts if cmplmt ^ fact.isNil)
...
class ModelPrecisionFilter(ModelFilter):
    ...
    def filter(self, xpCtx, varBinding, facts, cmplmt):
        ...
        return set(fact for fact in facts if cmplmt ^ (self.minimum != 'INF' and not fact.isNil and fact.isNumeric and not fact.isFraction and inferredPrecision(fact) >= numMinimum))
...
class ModelMessage(ModelFormulaResource):
    ...
    def compile(self):
        if not hasattr(self, 'expressionProgs'):
...
class ModelCustomFunctionSignature(ModelFormulaResource):
    ...
    def init(self, modelDocument):
        ...
        self.modelXbrl.modelCustomFunctionSignatures[self.functionQname] = None
    ...
    def inputTypes(self):
        try:
            ...
            self._inputTypes = [elt.prefixedNameQname(elt.get('type')) for elt in XmlUtil.children(self, XbrlConst.variable, 'input')]
            return self._inputTypes
    ...
    def propertyView(self):
        return (
         ...
         (
          'name', self.name),
         (
          'output type', self.outputType)) + tuple((_('input {0}').format(i + 1), type) for i, type in enumerate(self.inputTypes))
...
class ModelCustomFunctionImplementation(ModelFormulaResource):
    ...
    def inputNames(self):
        try:
            ...
            self._inputNames = [qname(elt, elt.get('name')) for elt in XmlUtil.children(self, XbrlConst.cfi, 'input')]
            return self._inputNames
    ...
    def stepExpressions(self):
        try:
            ...
            self._stepExpressions = [(qname(elt, elt.get('name')), elt.text) for elt in XmlUtil.children(self, XbrlConst.cfi, 'step')]
            return self._stepExpressions
    ...
    def propertyView(self):
        return (
         (
          'label', self.xlinkLabel),) + tuple((_('input {0}').format(i + 1), str(name)) for i, name in enumerate(self.inputNames)) + tuple((_('step {0}').format(str(qname)), expr) for qname, expr in enumerate(self.stepExpressions)) + (
         (
