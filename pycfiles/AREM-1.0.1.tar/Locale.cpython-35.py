...
# Embedded file name: /tmp/pip-install-ncu5lfw4/arelle/arelle/Locale.py
# Compiled at: 2018-08-09 04:11:41
# Size of source mod 2**32: 25720 bytes
...
def getUserLocale(localeCode=''):
    ...
    if sys.platform == 'darwin' and not localeCode:
        localeQueryResult = subprocess.getstatusoutput('defaults read -g AppleLocale')
        if localeQueryResult[0] == 0 and '_' in localeQueryResult[1]:
            localeCode = localeQueryResult[1]
    try:
        locale.setlocale(locale.LC_ALL, _STR_8BIT(localeCode))
    ...
    locale.setlocale(locale.LC_ALL, _STR_8BIT('C'))
    if conv is None:
        conv = locale.localeconv()
    if C_LOCALE is None:
...
def getLanguageCode():
    if sys.platform == 'darwin':
        localeQueryResult = subprocess.getstatusoutput('defaults read -g AppleLocale')
        if localeQueryResult[0] == 0 and localeQueryResult[1]:
            pass
...
def getLanguageCodes(lang=None):
    ...
    return [
     lang, lang.replace('-', '_'), lang.partition('-')[0]]
...
def availableLocales():
    ...
    if _availableLocales is not None:
        ...
        localeQueryResult = subprocess.getstatusoutput('locale -a')
        if localeQueryResult[0] == 0:
            _availableLocales = set(locale.partition('.')[0].replace('_', '-') for locale in localeQueryResult[1].split('\n'))
        else:
...
def languageCodes():
    ...
    if _languageCodes is not None:
        ...
        _languageCodes = {_('Afrikaans (South Africa)'): 'af-ZA afrikaans', 
...
def rtlString(source, lang):
    if lang and source and lang[0:2] in {'he', 'ar'}:
        line = []
        lineInsertion = 0
        words = []
        rtl = True
    ...
    if isinstance(val, collections.Mapping):
        ...
        for perc in percents:
            if perc.group()[(-1)] == '%':
                ...
                new_val.append(_format(conv, perc.group(), val[i], grouping, False, *val[i + 1:i + 1 + starcount]))
                i += 1 + starcount
...
def format_picture(conv, value, picture):
    ...
    if isinstance(value, float):
        ...
        if isinstance(value, _STR_NUM_TYPES):
            value = Decimal(value)
        else:
    ...
    for c in reversed(intPart):
        if (c.isdecimal() or c == '#' or c == thousands_sep) and prefix:
    ...
    if intPlaces == 0 and fractPlaces == 0:
        intPlaces = 1
    return format_decimal(None, value, intPlaces=intPlaces, fractPlaces=fractPlaces, sep=thousands_sep, dp=decimal_point, grouping=grouping, pos=prefix, neg=prefix if negPic else prefix + minus_sign, trailpos=suffix, trailneg=suffix)
    ...
    if conv is not None:
        ...
        if pos is None and trailpos is None:
            ...
            if pospos in ('0', 0):
                ...
                if pospos in ('1', 1, '3', 3):
                    pos = possign
                    trailpos = ''
                else:
                    if pospos in ('2', 2, '4', 4):
                        pos = ''
                        trailpos = possign
                    else:
                        pos = ''
            ...
            if neg is None and trailneg is None:
                ...
                if negpos in ('0', 0):
                    ...
                    if negpos in ('1', 1, '3', 3):
                        neg = negsign
                        trailneg = ''
                    else:
                        if negpos in ('2', 2, '4', 4):
                            neg = ''
                            trailneg = negsign
                        else:
                            if negpos == 127:
