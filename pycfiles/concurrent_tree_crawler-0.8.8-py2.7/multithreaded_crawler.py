...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/multithreaded_crawler.py
...
class MultithreadedCrawler:
    ...
    def __init__(self, navigators, sentinel, activity_schedule=None, log_file_path=None, state_file_path=None, save_period=None, logging_level=logging.ERROR):
        ...
        if state_file_path is not None:
            if os.path.exists(state_file_path):
                print 'State file already exists. Loading the tree from this file and changing nodes with state PROCESSING to OPEN ... ',
    ...
    def run(self):
        """
                ...
                """
        self.__manager = self._create_crawlers_manager(self.__tree, self.__navigators)
        if self.__log_file_path is not None:
            Logger.start(file_path=self.__log_file_path, logging_level=self.__logging_level)
        while True:
            ...
            threads_finished = self.__manager.wait_until_finish(timeout=activity_time)
            if self.__state_file_path is not None:
    ...
    def __start_tree_saver_thread(self):
        t = TreeSaverThread(self.__state_file_path, self.__tree, self.__save_period)
        t.daemon = True
        t.start()
        return t
    ...
    def __sleep_until_activity_period(self):
        ...
        while True:
            ...
            if not info.is_in_activity_period:
                logging.info(('Going to sleep for {:.1f} seconds (according to schedule)').format(mode_change_time))
                time.sleep(mode_change_time)
                logging.info('Awaken')
            else:
                logging.info(('Starting activity for {:.1f} seconds (according to schedule)').format(mode_change_time))
    ...
    def __load_state_file(file_path, sentinel):
        ...
        MultithreadedCrawler.__change_state_from_PROCESSING_to_OPEN(sentinel.get_child('root'))
