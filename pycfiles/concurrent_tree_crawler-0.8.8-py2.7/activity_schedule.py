...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/concurrent_tree_crawler/common/activity_schedule.py
...
class DaySchedule(AbstractActivitySchedule):
    ...
    def __get_closest_event_interval(self, now):
        ...
        if self.__start < self.__end:
            if today.time() < self.__start or today.time() > self.__start and today.time() < self.__end:
                return (self.__get_datetime(today, self.__start),
                 self.__get_datetime(today, self.__end))
            else:
                return (
                 self.__get_datetime(tomorrow, self.__start),
                 self.__get_datetime(tomorrow, self.__end))

        else:
            if today.time() > self.__end and today.time() < self.__start or today.time() > self.__start:
                return (self.__get_datetime(today, self.__start),
                 self.__get_datetime(tomorrow, self.__end))
            else:
                return (
                 self.__get_datetime(yesterday, self.__start),
                 self.__get_datetime(today, self.__end))
    ...
    def __get_datetime(date, time_):
        return datetime.datetime(date.year, date.month, date.day, time_.hour, time_.minute, time_.second, time_.microsecond)