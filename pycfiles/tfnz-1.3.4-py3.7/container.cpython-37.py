...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/tfnz/container.py
# Compiled at: 2018-05-18 20:34:30
# Size of source mod 2**32: 17010 bytes
import logging, shortuuid, weakref
...
class Container(Waitable, Killable, Connectable):
    __doc__ = 'An object representing a single container. Do not instantiate directly, use node.spawn.'
    ...
    def stdin(self, data: bytes):
        ...
        self.ensure_alive()
        self.wait_until_ready()
        self.conn().send_cmd(b'stdin_container', {'node':self.parent().pk,  'container':self.uuid},
          bulk=data)
    ...
    def wait_http_200(self, *, dest_port: Optional[int]=80, fqdn: Optional[str]='localhost', path: Optional[str]='') -> Tunnel:
    ...
    def spawn_process(self, remote_command: str, data_callback: Optional[Callable]=None, termination_callback: Optional[Callable]=None, stderr_callback: Optional[Callable]=None) -> Process:
        ...
        self.processes[spawn_command_uuid] = Process(self, spawn_command_uuid, data_callback, termination_callback, stderr_callback)
        self.conn().send_cmd(b'spawn_process', {'node':self.parent().pk,  'container':self.uuid, 
    ...
    def run_process(self, remote_command: str, *, data_callback: Optional[Callable]=None, stderr_callback: Optional[Callable]=None, nolog: Optional[bool]=False) -> (bytes, bytes, str):
        ...
        if isinstance(remote_command, list):
            ...
            msg = self.conn().send_blocking_cmd(b'run_process', {'node':self.parent().pk,  'container':self.uuid, 
             'command':remote_command})
            if msg is None:
                raise ValueError('Blocking process failed: ' + remote_command)
            if data_callback is not None:
                if len(msg.params['stdout']) > 0:
        ...
        return (
         msg.params['stdout'], msg.params['stderr'], msg.params['exit_code'])

    def spawn_shell(self, *, data_callback: Optional[Callable]=None, termination_callback: Optional[Callable]=None, echo: bool=False) -> Process:
        ...
        self.conn().send_cmd(b'spawn_shell', {'node':self.parent().pk,  'container':self.uuid, 
         'echo':echo},
          uuid=spawn_command_uuid,
          reply_callback=(self._process_callback))
    ...
    def fetch(self, filename: str) -> bytes:
        ...
        self.ensure_alive()
        self.wait_until_ready()
        return self.conn().send_blocking_cmd(b'fetch_file', {'node':self.parent().pk,  'container':self.uuid, 
    ...
    def put(self, filename: str, data: bytes):
        ...
        self.ensure_alive()
        self.wait_until_ready()
        self.conn().send_blocking_cmd(b'put_file', {'node':self.parent().pk,  'container':self.uuid, 
         'filename':filename},
          bulk=data)
    ...
    def reboot(self, *, reset_filesystem: Optional[bool]=False):
        ...
        self.mark_not_ready()
        self.conn().send_cmd(b'reboot_container', {'node':self.parent().pk,  'container':self.uuid, 
    ...
    def internal_destroy(self, send_cmd=True):
        if self.bail_if_dead():
            ...
            if send_cmd:
                ...
                self.conn().send_cmd(b'destroy_container', {'node':self.parent().pk,  'container':self.uuid})
            else:
...
class ExternalContainer(Connectable, Taggable):
    __doc__ = 'An object representing a container managed by another session (and the same user) but advertised using a tag.\n    Do not instantiate directly, use Location.external_container'
