...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python2.7/dist-packages/Digenpy_/Spanish.py
# Compiled at: 2012-07-25 14:08:56
"""
...
class Dlink:
    ...
    def __init__(self, *args):
        ...
        self.S1 = ''
        if self.sm[(-1)][1] is 0:
            self.S1 = self.hexa_minus_one(self.sm[(-1)][0])
        if self.S1 is -1:
            self.S2, self.S1 = 0, 0
    ...
    def hexa_minus_one(self, hexa):
        if hex(int(hexa, 16) - 1).split('x')[0] == '-':
            return -1
...
class Telefonica:
    ...
    def __init__(self, *args):
        if not len(args[0]) > 1:
            ...
            return
        self.dictionary = JazztelAndTelefonica(args[0][1], args[0][2], {'00:60:B3': [
                      'Z-com', 'Z001349'], 
           '00:01:38': [
                      'Xavi 7768r', 'X000138'], 
           '00:03:C9': [
                      'Comtrend 535', 'C0030DA'], 
           '00:A0:C5': [
                      'Zyxel 650HW/660HW', 'Z001349'], 
           '00:16:38': [
                      'Comtrend_536+', 'C0030DA'], 
           '00:1A:2B': [
                      'Comtrend 5361', 'C001D20'], 
           '00:02:CF': [
                      'P-660HW-D1', 'Z0002CF'], 
           '00:23:F8': [
                      'P-660HW-D1', 'Z0023F8'], 
           ...
           '50:67:F0': [
                      'Zyxel', 'Z5067F0'], 
           'E0:91:53': [
                      'Xavi 7968-Solos 4610RD/461x', 'E0:91:53'], 
           '00:1F:9F': [
                      'Thomson 2009', 'T5YF69A'], 
           '00:18:03': [
                      'Huawei echolife hg520v (ftth)', 'H4A60BA', 'H538FBF'], 
           '00:13:49': [
                      'P-660HW-D1', 'Z001349', 'Z0002CF']}).dictionary
    ...
    def dictionary(self):
        ...
        try:
            for i in range(1, len(self.valid_routers[(':').join(self.sm[:3])])):
                [ [ [ [ self.dicts.append('%s%s%s%s%s%s%s' % (self.valid_routers[(':').join(self.sm[:3])][i], m, j, k, l, self.essid[(-2)], self.essid[(-1)])) for m in a ] for j in a ] for k in a ] for l in a
                ]
