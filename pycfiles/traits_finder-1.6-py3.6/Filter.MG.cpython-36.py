...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/traits_finder/scripts/Filter.MG.py
# Compiled at: 2020-01-20 07:55:59
# Size of source mod 2**32: 5640 bytes
import argparse, os
...
parser.add_argument('-s', help="set the method to search the your database                     (1: blast; 2: hmm),                     (default '1' for blast search)",
 ...
 1, 2],
  ...
  default=1,
  type=int)
parser.add_argument('--g', help="input genomes (T) or metagenomes (F)                     (default 'F' for metagenomes)",
  metavar='1 or 2',
  choices=[
 'T', 'F'],
  action='store',
...
def blast_list(file, Cutoff_identity, Cutoff_hitlength):
    ...
    for line in open(file, 'r'):
        if float(str(line).split('\t')[2]) >= Cutoff_identity:
            try:
                if args.g == 'T':
                    length_cut = float(str(line).split('\t')[3]) >= Cutoff_hitlength * float(DB_length.get(str(line).split('\t')[1], 0.0)) / 100
                else:
                ...
                if length_cut:
                    if str(line).split('\t')[0] not in All_hit:
                        ...
                        All_hit[str(line).split('\t')[0]] = compare_blast(All_hit[str(line).split('\t')[0]][0], line)
            except TypeError:
