...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/traits_finder/scripts/Filter.WG.py
# Compiled at: 2019-12-04 01:43:59
...
def blast_list(file, Cutoff_identity, Cutoff_hitlength):
    ...
    for line in open(file, 'r'):
        if float(str(line).split('\t')[2]) >= Cutoff_identity:
            try:
                if float(str(line).split('\t')[3]) >= Cutoff_hitlength * float(DB_length.get(str(line).split('\t')[1], 0.0)) / 100:
