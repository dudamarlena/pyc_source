...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/traits_finder/Traits_WG.py
# Compiled at: 2020-01-23 22:55:50
...
parser.add_argument('--e', default=0.01,
  action='store',
  type=float,
  metavar='1e-5',
...
parser.add_argument('--hmm', help='Optional: complete path to hmmscan if not in PATH,',
  ...
  default='hmmscan',
  type=str)
parser.add_argument('--bp', help="Optional: complete path to blast if not in PATH, 'None' for no blast search",
...
cmds = 'python ' + workingdir + '/scripts/Search.WG.py -i ' + args.i + ' -db ' + args.db + ' -dbf ' + str(args.dbf) + ' -s ' + str(args.s) + ' --r ' + str(args.r) + ' --t ' + str(args.t) + ' --dm ' + str(args.dm) + ' --u ' + str(args.u) + ' --hs ' + str(args.hs) + ' --hmm ' + str(args.hmm) + ' --bp ' + str(args.bp) + ' --ht ' + str(args.ht) + ' --id ' + str(args.id) + ' --fa ' + str(args.fa) + ' --e ' + str(args.e) + ' --orf ' + str(args.orf) + ' --r16 ' + str(args.r16) + ' --bwa ' + str(args.bwa) + ' --mf ' + str(args.mf) + ' --pro ' + str(args.pro) + ' \n'
os.system(cmds)
...
for file_name in list_of_files:
    if all(keys not in file_name for keys in ['SearchWG.sh', 'all.sh']):
        f1.write('sbatch -p defq,sched_mem1TB -c 40 -t 3-00:00:00 --mem=100000 -J ' + os.path.split(str(file_name))[(-1)] + '_' + str(os.path.split(args.db)[1]) + ' -o ' + str(file_name) + '.out -e ' + str(file_name) + '.err ' + str(file_name) + ' \n')
