...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/traits_finder/__main__.py
# Compiled at: 2020-01-23 23:50:42
# Size of source mod 2**32: 17880 bytes
import os
from Bio import SeqIO
import argparse, glob, statistics, traits_finder, sys
...
def main():
    ...
    version_string = 'traits_finder {v}, on Python {pyv[0]}.{pyv[1]}.{pyv[2]}'.format(v=(traits_finder.__version__),
      pyv=(sys.version_info))
    parser = argparse.ArgumentParser(formatter_class=(argparse.RawDescriptionHelpFormatter))
    required = parser.add_argument_group('required arguments')
    optional = parser.add_argument_group('optional arguments')
    required.add_argument('command', help='traits_finder genome for analyzing genomes,                         traits_finder mge for analyzing mobile genetic elements (MGEs),                         traits_finder meta for analyzing metagenomes,                         traits_finder sum_genome for summarizing genome results,                        traits_finder sum_genome for summarizing MGE results,                        traits_finder sum_meta for summarizing metagenome results,                             traits_finder HGT for finding candidate HGT of traits across species',
      type=str,
      default='genome',
      choices=[
     'genome', 'mge', 'meta',
    ...
    required.add_argument('-db', help='file name of your input database',
      type=str,
      default='Butyrate.pro.aa',
      metavar='database.aa')
    optional.add_argument('-dbf', help="sequence format of your input database                        (1: nucleotide; 2: protein),                         (default '2' for protein)",
      metavar='1 or 2',
      choices=[
     1, 2],
      action='store',
      default=2,
      type=int)
    required.add_argument('-m', help='mapping file of traits to function',
      type=str,
      default='Butyrate.pro.mapping.txt',
      metavar='database.mapping.txt')
    required.add_argument('-i', help='input folder of metagenomes or genomes',
      type=str,
      default='.',
      metavar='current dir (.)')
    required.add_argument('-fa', help='input format of genome/metagenome sequence',
      type=str,
      default='.fa',
      metavar='.fasta, .fna, .fastq or .fa')
    optional.add_argument('-s', help="set the method to search the your database                         (1: blast; 2: hmm; 3: alignment),                         (default '1' for blast search)",
      metavar='1 or 2',
      choices=[
     1, 2, 3],
      action='store',
      default=1,
      type=int)
    optional.add_argument('--meta', help='metadata  of metagenomes',
      type=str,
      default='None',
      metavar='metadata.metagenomes.txt')
    optional.add_argument('--orf', help='input format of genomes orfs, set prodigal to predict ORF if needed (--pro prodigal)',
      type=str,
      default='.faa',
      metavar='.faa')
    optional.add_argument('--l', help='input list of a subset of metagenomes',
      type=str,
      default='None',
      metavar='list.txt')
    optional.add_argument('--g', help='Optional: gene-level HGT finding; --g T (default: function-level; --g F)',
      metavar=[
     'T', 'F'],
      action='store',
      default='F',
      type=str)
    optional.add_argument('--r', help='output directory or folder of your results',
      type=str,
      default='Result',
      metavar='Result',
      nargs='*')
    optional.add_argument('--r16', help='output directory or folder of your 16S sequences',
      type=str,
      default='Result',
      metavar='Result')
    optional.add_argument('--t', help='Optional: set the thread number assigned for running XXX (default 1)',
      metavar='1 or more',
      action='store',
      default=1,
      type=int)
    optional.add_argument('--id', '--identity', default=75.0,
      action='store',
      type=float,
      metavar='60.0',
      help='Optional: set the amno acid based identity cutoff for blast (default is 80.0)\nLeave it alone if hmm is used')
    optional.add_argument('--ht', '--hitlength', default=75.0,
      action='store',
      type=float,
      metavar='60.0',
      help='Optional: set the amno acid based hit-length cutoff for blast (default is 80.0)\nLeave it alone if hmm is used')
    optional.add_argument('--e', '--evalue', default=0.01,
      action='store',
      type=float,
      metavar='1e-5',
      help='Optional: set the evalue cutoff for blast or hmm (default is 1e-5)')
    optional.add_argument('--u', '--usearch', help="Optional: use two-step method for blast search, 'None' for using one step, 'usearch' for using two-step                              (complete path to usearch if not in PATH), (default: 'None')",
      metavar='None or usearch',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--dm', '--diamond', help="Optional: use two-step method for blast search, 'None' for using one step, 'diamond' for using two-step                                (complete path to diamond if not in PATH), (default: 'None')",
      metavar='None or diamond',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--hs', help="Optional: use two-step method for blast search, 'None' for using one step, 'hs-blastn' for using two-step                                (complete path to hs-blastn if not in PATH), (default: 'None')",
      metavar='None or hs-blastn',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--hmm', help='Optional: complete path to hmmscan if not in PATH,',
      metavar='/usr/local/bin/hmmscan',
      action='store',
      default='hmmscan',
      type=str)
    optional.add_argument('--bp', '--blast', help="Optional: complete path to blast if not in PATH, 'None' for no blast search",
      metavar='/usr/local/bin/blast',
      action='store',
      default='blast',
      type=str)
    optional.add_argument('--bwa', help='Optional: complete path to bwa if not in PATH,',
      metavar='/usr/local/bin/bwa',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--mf', '--mafft', help='Optional: complete path to mafft if not in PATH,',
      metavar='/usr/local/bin/mafft',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--ft', '--fasttree', help='Optional: complete path to fasttree if not in PATH,',
      metavar='/usr/local/bin/fasttree',
      action='store',
      default='None',
      type=str)
    optional.add_argument('--pro', '--prodigal', help='Optional: complete path to prodigal if not in PATH, None for no prodigal (default)',
            ...
            if db_type == 1:
                if 'blast' in search_method:
                    ...
                    if 'hs-blastn' in search_method:
                        ...
                        if 'hmm' in search_method and '.hmm' not in db_file:
                            try:
