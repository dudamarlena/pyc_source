...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/traits_finder/scripts/Traits_summary_MG.py
# Compiled at: 2020-01-22 13:30:36
# Size of source mod 2**32: 10453 bytes
import os
from Bio import SeqIO
import argparse, glob, statistics
...
parser.add_argument('-s', help="set the method to search the your database                     (1: blast; 2: hmm),                     (default '1' for blast search)",
  metavar='1 or 2',
  choices=[
 1, 2],
  action='store',
...
parser.add_argument('--meta', help='metadata  of metagenomes',
  type=str,
  default='None',
...
if args.s == 1:
    ...
    for lines in open(args.db + '.length', 'r'):
        Mapping.setdefault(lines.split('\t')[0], float(lines.split('\t')[1].split('\r')[0].split('\n')[0]))
...
if args.meta != 'None':
    for lines in open(args.meta, 'r'):
        Meta.setdefault(lines.split('\t')[0].split(args.fa)[0].split('.blast.txt.filter')[0].split('_1')[0].split('_2')[0], '\t'.join(lines.split('\r')[0].split('\n')[0].split('\t')[1:]))
...
for searchfile in searchfiles:
    ...
    if filename_short not in Trait:
        if '_1_2' not in filename:
