...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/beerme/beerme.py
# Compiled at: 2018-12-13 13:20:17
import sys, argparse, json, requests
from bs4 import BeautifulSoup
from . import get_version
location_map = {'bh-bridge': 'https://business.untappd.com/boards/24264', 
    ...
    for bev in data:
        ...
        if show_containers:
            for container in bev['containers']:
                print ('\t{}\t${}\t{}').format(container.get('name'), container.get('price'), container.get('container_calories'))
...
def main():
    a = argparse.ArgumentParser('beerme', description=('BeerMe BigHops script {}').format(get_version()))
    ...
    a.add_argument('-f', '--fills-only', action='store_true', help='Only include beer that can be purchased in growlers')
    a.add_argument('-s', '--search', action='append', help='A keyword to search for. For example: "IPA". This can be used multiple times to create a compound search')
    a.add_argument('-e', '--exclude', action='append', help='A keyword used to exclude results. For example: "Sour". This can be used multiple times')
