...
# Embedded file name: build/bdist.macosx-10.8-intel/egg/ccp_sde_parser/bigqueryize.py
# Compiled at: 2012-11-04 23:10:40
__doc__ = 'Upload a directory of JSON files into a BigQuery.'
import json, os, sys
...
def load_tables(http, service, schema, json_dir):
    ...
    pbar = ProgressBar(widgets=['Uploading...', Bar(), ' ', Percentage(), ' ', ETA()], maxval=len(schema) - len(FLAGS.skip_tables)).start()
    jobs = dict()
...
def main():
    ...
    FLOW = OAuth2WebServerFlow(client_id=FLAGS.client_id, client_secret=FLAGS.client_secret, scope='https://www.googleapis.com/auth/bigquery', user_agent='bigqueryize/1.0')
