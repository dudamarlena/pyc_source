...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/persist/simple_repository.py
...
class SimpleMongoDbRepository(repositories.CrudRepository):
    ...
    def __init__(self, app=None):
        ...
        self._SimpleMongoDbRepository__client = MongoClient(host=app.conf.MONGODB_HOST, port=app.conf.MONGODB_PORT, tz_aware=app.conf.MONGODB_TZ_AWARE)
        self._app = app
    ...
    def update(self, obj, key=None, upsert=False):
        ...
        if r.upserted_id is None:
            raise exceptions.NotFoundException('document not found, oid is : ' + d.get(constants.FIELD_ID, 'None').__str__())
        else:
    ...
    def update_partial(self, obj, path=None):
        ...
        if r.matched_count == 1:
            return root_id
        raise exceptions.OperationFailedException('failed to apply partial update on : ' + str(root_id))

    def load_partial(self, key, path):
        f = SimpleMongoDbRepository.compose_path(path)
        coll = self.get_collection()
        d = coll.find_one({constants.FIELD_ID: SimpleMongoDbRepository.get_oid(key)}, {constants.FIELD_CHILDREN: {'$slice': [path[0], 1]}})
    ...
    def get_collection(self):
        db = self._SimpleMongoDbRepository__client.get_database(self._app.conf.MONGODB_DB)
