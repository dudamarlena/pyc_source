...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/service/task_service.py
# Compiled at: 2017-09-08 11:48:18
...
class TaskService:
    ...
    def execute(self, op, ctx, last_op):
        ...
        if target is None and executor.is_target_sensitive:
            return
        if op.is_leaf:
    ...
    def get_target(self, op, ctx, last_op):
        if isinstance(op.target, str):
            ...
            target = op.target
            if target is None and isinstance(ctx, task.Context):
                target = ctx.target
            if target is None:
                root = op.lt_root()
                target = root.target
