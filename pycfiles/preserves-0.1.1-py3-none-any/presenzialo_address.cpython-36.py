...
# Embedded file name: build/bdist.linux-x86_64/egg/presenzialo/presenzialo_address.py
...
Worker = namedtuple('Worker', 'name id status phone phone2 office group boss today tomorrow')
...
class PRaddress:
    ...
    def parse(self, json):
        ...
        for i in json:
            ...
            w = Worker(i['nominativo'].strip(), i['id'], i['descrstato'].strip(), i['telefono'].strip(), i['libero'].strip(), i['altro'], i['centrocontr'], i['responsabile'], day.get('oggi', None), day.get('domani', None))
            d[w.id] = w
...
def add_parser_address(parser):
    ...
    parser_group.add_argument('--in',
      dest='workers', metavar='worker', nargs='+', help="Worker's presence")
    parser_group.add_argument('-p',
      '--phone',
      dest='phones',
      metavar='phone',
      nargs='+',
      help="Worker's phone number")
    parser_group.add_argument('--cache-address',
      action='store_true', help='save address phone for future use')


def main():
    parser = argparse.ArgumentParser(prog='PRaddress',
