...
# Embedded file name: build/bdist.linux-x86_64/egg/presenzialo/presenzialo_id.py
...
class PRworker:
    ...
    def parse(self, json):
        ...
        for i in json['results']:
            if self.raw:
                print(i)
            d[i['iddip']] = WorkerID(i['cognome'], i['nome'], '{} {}'.format(i['nome'], i['cognome']), i['iddip'])

        return (
         d, json['total'])

    def __str__(self):
        return '\n'.join('{:5d} {} {}'.format(v.id, v.surname, v.name) for k, v in self.workers.items())
...
def main():
    parser = argparse.ArgumentParser(prog='PRids',
