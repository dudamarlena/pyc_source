...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-intel/egg/eidanagios/check_EIDA_auth.py
# Compiled at: 2020-02-10 06:49:32
# Size of source mod 2**32: 8630 bytes
...
"""
import requests, os, sys, time, argparse, xml.etree.ElementTree as ET
...
def check_EIDA_auth(url, timeout=9, token=None, verbose=0):
    ...
    headers = {'User-Agent': 'check_EIDA_auth python-requests/' + requests.__version__}
    start_time = time.time()
    wadl = url.replace('https:', 'http:').replace('/1/auth', '/1/application.wadl')
    try:
        r = requests.get(wadl, headers=headers)
    except Exception:
        nagios_output('AUTH', 'FAILED', ('Error while reading from %s' % wadl), ('time=%fs' % (time.time() - start_time)),
          verbose=verbose)
        return 2
    else:
        try:
            ...
            nagios_output('AUTH', 'FAILED', ('Error parsing %s' % wadl), ('time=%fs' % (time.time() - start_time)),
              verbose=verbose)
            return 2
        else:
            ...
            for resources in root.findall(namesp + 'resources'):
                ...
                for resource in resources.findall(namesp + 'resource'):
                    if 'path' in resource.attrib and resource.attrib['path'] == 'auth':
                        authenabled = True
                        break

            if not authenabled:
                nagios_output('AUTH', 'DISABLED', url, ('time=%fs' % (time.time() - start_time)),
                  verbose=verbose)
                return 0
            else:
                ...
                try:
                    ...
                    if r.status_code != 200:
                        nagios_output('AUTH', 'FAILED', url, ('time=%fs' % (time.time() - start_time)), ('HTTP error code: %s\nText: %s' % (r.status_code, r.text)),
                          verbose=verbose)
                        return 2
                    ...
                    if r.status_code == 200:
                        ...
                        nagios_output('AUTH', 'FAILED_AND_INVALID_CERTIFICATE', url, 'time=%fs' % (time.time() - start_time))
                        return 2

                resp = r.text.split(':')
                if len(resp) != 2:
                    nagios_output('AUTH', 'WRONG_FORMAT', url, ('time=%fs' % (time.time() - start_time)), ('Response= "%s"' % resp),
                      verbose=verbose)
                    return 2
...
def main():
    ...
    desc = 'Nagios plugin to check an EIDA auth endpoint. If no arguments are passed all EIDA nodes are tested.'
    parser = argparse.ArgumentParser(description=desc)
    group = parser.add_mutually_exclusive_group()
    group.add_argument('-H', '--hostname', default=None, help=('Hostname providing the "auth" method at the default location. Valid values are domain names (e.g. geofon.gfz-potsdam.de) or the data centre ID (%s)' % ', '.join(urls.keys())))
    group.add_argument('-u', '--url', default=None, help='URL pointing to the "auth" method to check. Use with non-standard locations')
    ...
    parser.add_argument('-v', '--verbose', action='count', default=0, help='If this option is enabled more lines with details will follow the expected one-line message')
    args = parser.parse_args()
    requests.packages.urllib3.disable_warnings(requests.packages.urllib3.exceptions.InsecureRequestWarning)
    if args.hostname is not None:
        if args.hostname in urls:
            sys.exit(check_EIDA_auth((urls[args.hostname]), timeout=(args.timeout), token=(args.authentication),
              verbose=(args.verbose)))
        sys.exit(check_EIDA_auth(('https://%s/fdsnws/dataselect/1/auth' % args.hostname), timeout=(args.timeout),
          token=(args.authentication),
          verbose=(args.verbose)))
    if args.url is not None:
        sys.exit(check_EIDA_auth((args.url), timeout=(args.timeout), token=(args.authentication),
          verbose=(args.verbose)))
    retcode = 0
    for dc, url in urls.items():
        retcode = max(retcode, check_EIDA_auth(url, timeout=(args.timeout), token=(args.authentication),
          verbose=(args.verbose)))
