...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tlp/core.py
# Compiled at: 2020-04-23 19:40:12
# Size of source mod 2**32: 23951 bytes
import collections, json, os.path, shutil, sys, tarfile, tempfile
from typing import BinaryIO, Dict, Iterator, List, Optional, Set, TextIO, Tuple, Union
...
class Program:
    ...
    """
        ...
        self._tasks = collections.OrderedDict((name, Task(name=name, **task)) for name, task in sorted((obj['tasks'].items()), key=(lambda x: x[0])))
    ...
    def get_rtl(self, name: str, prefix: bool=True) -> str:
        return os.path.join(self.rtl_dir, (util.get_module_name(name) if prefix else name) + rtl.RTL_SUFFIX)
    ...
    def run_hls(self, clock_period: Union[(int, float, str)], part_num: str) -> 'Program':
        ...
        def worker(task):
            with open(self.get_tar(task.name), 'wb') as (tarfileobj):
                with hls.RunHls(tarfileobj,
                  kernel_files=[
                 (
                  self.get_cpp(task.name), self.cflags)],
                  top_name=(task.name),
                  clock_period=clock_period,
                  part_num=part_num,
                  auto_prefix=True) as (proc):
                    yield
    ...
    def instrument_rtl(self, directive: Optional[Tuple[(Dict[(str, List[str])], TextIO)]]=None) -> 'Program':
        ...
        for task in self._tasks.values():
            ...
            for file_name in ('async_mmap.v', 'detect_burst.v', 'generate_last.v',
                              'relay_station.v'):
                shutil.copy(os.path.join(os.path.dirname(util.__file__), 'assets', 'verilog', file_name), self.rtl_dir)

        return self

    def pack_rtl(self, output_file: BinaryIO) -> 'Program':
        rtl.pack(top_name=(self.top), ports=(self.ports),
          rtl_dir=(self.rtl_dir),
          output_file=output_file)
        return self

    def _process_partition_directive(self, directive: Dict[(str, List[Union[(str, Dict[(str, List[str])])]])], constraints: TextIO) -> Tuple[(int, Dict[(str, int)])]:
        ...
        if missing_instances:
            raise InputError('missing region assignment: {%s}' % ', '.join(missing_instances))
        unnecessary_instances = directive_instance_set - program_instance_set - rtl.BUILTIN_INSTANCES
        if unnecessary_instances:
            raise InputError('unnecessary region assignment: {%s}' % ', '.join(unnecessary_instances))
        fifo_partition_count = {}
        for name, meta in self.top_task.fifos.items():
            name = rtl.sanitize_array_name(name)
            producer_region = instance_dict[util.get_instance_name(meta['produced_by'])]
            consumer_region = instance_dict[util.get_instance_name(meta['consumed_by'])]
            if producer_region == consumer_region:
                ...
                if consumer_region not in topology[producer_region]:
                    raise InputError(f"{consumer_region} is not reachable from {producer_region}")
                idx = 0
        ...
        self.top_task.module.register_level = max(map(len, topology[instance_dict[self.ctrl_instance_name]].values())) + 1
    ...
    def _generate_child_instances(self, task: Task) -> Iterator[Instance]:
        for task_name, instance_objs in task.tasks.items():
            for instance_idx, instance_obj in enumerate(instance_objs):
                yield Instance(self.get_task(task_name), verilog=rtl, 
                 instance_id=instance_idx, **instance_obj)
    ...
    def _instantiate_fifos(self, task: Task) -> None:
        for fifo_name, fifo in task.fifos.items():
            ...
            task.module.add_signals(ast.Wire(name=(rtl.wire_name(fifo_name, suffix)), width=(child_ports[rtl.fifo_port_name(fifo_port, suffix)].width)) for suffix in rtl.ISTREAM_SUFFIXES)
            task_name, task_idx = fifo['produced_by']
            fifo_port = task.tasks[task_name][task_idx]['args'][fifo_name]['port']
            child_ports = self.get_task(task_name).module.ports
            task.module.add_signals(ast.Wire(name=(rtl.wire_name(fifo_name, suffix)), width=(child_ports[rtl.fifo_port_name(fifo_port, suffix)].width)) for suffix in rtl.OSTREAM_SUFFIXES)
            width = child_ports[rtl.fifo_port_name(fifo_port, rtl.OSTREAM_SUFFIXES[0])].width
            fifo_width = int(width.msb.value) - int(width.lsb.value) + 1
            task.module.add_fifo_instance(name=fifo_name,
              width=fifo_width,
              depth=(fifo['depth']))
            debugging_blocks = []
            col_width = max(max(len(name), len(util.get_instance_name(fifo['consumed_by'])), len(util.get_instance_name(fifo['produced_by']))) for name, fifo in task.fifos.items())
            fmtargs = {'fifo_prefix':'\\033[97m', 
             'fifo_suffix':'\\033[0m', 
             'task_prefix':'\\033[90m', 
             'task_suffix':'\\033[0m'}
            for suffixes, fmt, fifo_tag in zip((
             rtl.ISTREAM_SUFFIXES, rtl.OSTREAM_SUFFIXES), ('DEBUG: R: {fifo_prefix}{fifo:>{width}}{fifo_suffix} -> {task_prefix}{task:<{width}}{task_suffix} %h',
                                                           'DEBUG: W: {task_prefix}{task:>{width}}{task_suffix} -> {fifo_prefix}{fifo:<{width}}{fifo_suffix} %h'), ('consumed_by',
                                                                                                                                                                    'produced_by')):
                display = ast.SingleStatement(statement=ast.SystemCall(syscall='display',
                  args=(
                 ast.StringConst(value=(fmt.format)(width=col_width, fifo=fifo_name, 
                  task=util.get_instance_name(fifo[fifo_tag]), **fmtargs)),
                 ast.Identifier(name=(rtl.wire_name(fifo_name, suffixes[0]))))))
                debugging_blocks.append(ast.Always(sens_list=(rtl.CLK_SENS_LIST),
                  statement=(ast.make_block(ast.IfStatement(cond=ast.Eq(left=ast.Identifier(name=(rtl.wire_name(fifo_name, suffixes[(-1)]))),
                  right=(rtl.TRUE)),
    ...
    def _instantiate_children_tasks(self, task: Task, width_table: Dict[(str, int)]) -> List[ast.Identifier]:
        is_done_signals = []
        arg_table = {}
        ...
        for instance in self._generate_child_instances(task):
                 ...
                 Instance.Arg.Cat.OSTREAM}:
                    ...
                    if arg.cat == Instance.Arg.Cat.ASYNC_MMAP:
                        for tag in ('read_addr', 'read_data', 'write_addr', 'write_data'):
                            if set(x.portname for x in rtl.generate_async_mmap_ports(tag=tag,
                              port=(arg.port),
                              arg=arg_name,
                              instance=instance)) & child_port_set:
                                async_mmap_args.setdefault(arg_name, []).append(tag)

                    if arg.cat == Instance.Arg.Cat.MMAP or arg.cat == Instance.Arg.Cat.ASYNC_MMAP and task.name == self.top:
                        task.module.add_m_axi(name=arg_name, data_width=(width_table[arg_name]))
                for tag in async_mmap_args.get(arg_name, []):
                    if task.name == self.top:
                        task.module.add_signals(rtl.generate_async_mmap_signals(tag=tag,
                          arg=arg_name,
                          data_width=(width_table[arg_name])))
                    else:
                        task.module.add_ports(rtl.generate_async_mmap_ioports(tag=tag,
                          arg=arg_name,
                          data_width=(width_table[arg_name])))
            ...
            if instance.is_autorun:
                ...
                else_branch = (
                 ast.make_if_with_block(cond=(instance.is_state(STATE00)),
                   true=ast.make_if_with_block(cond=(start_q[(-1)]),
                   true=(instance.set_state(STATE01)))),
                 ast.make_if_with_block(cond=(instance.is_state(STATE01)),
                   true=ast.make_if_with_block(cond=(instance.ready),
                   true=ast.make_if_with_block(cond=(instance.done),
                   true=(instance.set_state(STATE10)),
                   false=(instance.set_state(STATE11))))),
                 ast.make_if_with_block(cond=(instance.is_state(STATE11)),
                   true=ast.make_if_with_block(cond=(instance.done),
                   true=(instance.set_state(STATE10)))),
                 ast.make_if_with_block(cond=(instance.is_state(STATE10)),
                   true=ast.make_if_with_block(cond=(done_q[(-1)]),
                   true=(instance.set_state(STATE00)))))
                task.module.add_logics([
                 ast.Always(sens_list=(rtl.CLK_SENS_LIST),
                   statement=(ast.make_block(ast.make_if_with_block(cond=(ast.Unot(rst_q[(-1)])),
                   true=if_branch,
                   false=else_branch)))),
                 ast.Assign(left=(instance.start),
            ...
            for arg_name, arg in sorted((item for item in instance.args.items()), key=(lambda x: x[0])):
                if arg.cat == Instance.Arg.Cat.SCALAR:
                    portargs.append(ast.PortArg(portname=(arg.port),
                      argname=(arg_table[instance.get_instance_arg(arg_name)][(-1)])))
                else:
                    if arg.cat == Instance.Arg.Cat.ISTREAM:
                        portargs.extend(rtl.generate_istream_ports(port=(arg.port), arg=arg_name))
                        portargs.extend(portarg for portarg in rtl.generate_peek_ports(rtl,
                          port=(arg.port), arg=arg_name) if portarg.portname in child_port_set)
                    else:
                        if arg.cat == Instance.Arg.Cat.OSTREAM:
                            portargs.extend(rtl.generate_ostream_ports(port=(arg.port), arg=arg_name))
                        else:
                            if arg.cat == Instance.Arg.Cat.MMAP:
                                portargs.extend(rtl.generate_m_axi_ports(port=(arg.port),
                                  arg=arg_name,
                                  arg_reg=(arg_table[arg_name][(-1)].name)))
                            elif arg.cat == Instance.Arg.Cat.ASYNC_MMAP:
                                for tag in async_mmap_args[arg_name]:
                                    portargs.extend(rtl.generate_async_mmap_ports(tag=tag,
                                      port=(arg.port),
                                      arg=arg_name,
                                      instance=instance))

            task.module.add_instance(module_name=(util.get_module_name(instance.task.name)),
              instance_name=(instance.name),
              ports=portargs)

        if task.name == self.top:
            for arg_name in async_mmap_args:
                task.module.add_async_mmap_instance(name=arg_name,
    ...
    def _instantiate_global_fsm(self, task: Task, is_done_signals: List[rtl.Pipeline]) -> None:
        ...
        global_fsm = [
         ast.make_if_with_block(cond=(is_state(STATE00)),
           true=ast.make_if_with_block(cond=(self.start_q[(-1)]),
           true=(set_state(STATE01)))),
         ast.make_if_with_block(cond=(is_state(STATE01)),
           true=ast.make_if_with_block(cond=ast.make_operation(operator=(ast.Land),
           nodes=(x[(-1)] for x in reversed(is_done_signals))),
           true=(set_state(STATE10)))),
         ast.make_if_with_block(cond=(is_state(STATE10)),
           true=(ast.make_block([
          set_state(STATE11 if self.register_level else STATE00),
          ast.NonblockingSubstitution(left=countdown,
            right=(ast.make_int(max(0, self.register_level - 1))))]))),
         ast.make_if_with_block(cond=(is_state(STATE11)),
           true=ast.make_if_with_block(cond=ast.Eq(left=countdown,
           right=ast.make_int(0, width=countdown_width)),
           true=(set_state(STATE00)),
           false=ast.NonblockingSubstitution(left=countdown,
           right=ast.Minus(left=countdown,
           right=ast.make_int(1, width=countdown_width)))))]
        task.module.add_logics([
         ast.Always(sens_list=(rtl.CLK_SENS_LIST),
           statement=(ast.make_block(ast.make_if_with_block(cond=(rtl.RST),
           true=(set_state(STATE00)),
