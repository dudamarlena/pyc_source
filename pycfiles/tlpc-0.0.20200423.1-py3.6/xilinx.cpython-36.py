...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/tlp/verilog/xilinx.py
# Compiled at: 2020-04-23 19:40:12
# Size of source mod 2**32: 31327 bytes
"""Helpers for Xilinx verilog.

This module defines constants and helper functions for verilog files generated
for Xilinx devices.
"""
import collections, copy, itertools, os, shutil, sys, tempfile
from typing import IO, BinaryIO, Callable, Dict, Iterable, Iterator, List, Optional, TextIO, Tuple, Union
...
from .util import Pipeline, async_mmap_instance_name, match_array_name, sanitize_array_name, wire_name
RTL_SUFFIX = '.v'
M_AXI_PORT_WIDTHS = dict(ADDR=0,
  BURST=2,
  CACHE=4,
  DATA=0,
  ID=1,
  ...
  USER=1,
  VALID=1)
M_AXI_ADDR_PORTS = (('ADDR', 'output'), ('BURST', 'output'), ('CACHE', 'output'), ('ID', 'output'),
                    ('LEN', 'output'), ('LOCK', 'output'), ('PROT', 'output'), ('QOS', 'output'),
                    ('READY', 'input'), ('REGION', 'output'), ('SIZE', 'output'),
                    ('USER', 'output'), ('VALID', 'output'))
M_AXI_PORTS = collections.OrderedDict(AR=M_AXI_ADDR_PORTS,
  AW=M_AXI_ADDR_PORTS,
  B=(('ID', 'input'), ('READY', 'output'), ('RESP', 'input'), ('USER', 'input'), ('VALID', 'input')),
  R=(('DATA', 'input'), ('ID', 'input'), ('LAST', 'input'), ('READY', 'output'), ('RESP', 'input'),
     ('USER', 'input'), ('VALID', 'input')),
  W=(('DATA', 'output'), ('ID', 'output'), ('LAST', 'output'), ('READY', 'input'),
     ('STRB', 'output'), ('USER', 'output'), ('VALID', 'output')))
M_AXI_PORTS: Dict[(str, Tuple[(Tuple[(str, str)], ...)])]
M_AXI_PREFIX = backend.M_AXI_PREFIX
M_AXI_SUFFIXES = ('_ARADDR', '_ARBURST', '_ARCACHE', '_ARID', '_ARLEN', '_ARLOCK',
                  '_ARPROT', '_ARQOS', '_ARREADY', '_ARREGION', '_ARSIZE', '_ARUSER',
                  '_ARVALID', '_AWADDR', '_AWBURST', '_AWCACHE', '_AWID', '_AWLEN',
                  '_AWLOCK', '_AWPROT', '_AWQOS', '_AWREADY', '_AWREGION', '_AWSIZE',
                  '_AWUSER', '_AWVALID', '_BID', '_BREADY', '_BRESP', '_BUSER', '_BVALID',
                  '_RDATA', '_RID', '_RLAST', '_RREADY', '_RRESP', '_RUSER', '_RVALID',
                  '_WDATA', '_WID', '_WLAST', '_WREADY', '_WSTRB', '_WUSER', '_WVALID')
M_AXI_PARAM_PREFIX = 'C_M_AXI_'
M_AXI_PARAM_SUFFIXES = ('_ID_WIDTH', '_ADDR_WIDTH', '_DATA_WIDTH', '_AWUSER_WIDTH',
                        '_ARUSER_WIDTH', '_WUSER_WIDTH', '_RUSER_WIDTH', '_BUSER_WIDTH',
                        '_USER_VALUE', '_PROT_VALUE', '_CACHE_VALUE', '_WSTRB_WIDTH')
M_AXI_PARAMS = ('C_M_AXI_DATA_WIDTH', 'C_M_AXI_WSTRB_WIDTH')
ISTREAM_SUFFIXES = ('_dout', '_empty_n', '_read')
OSTREAM_SUFFIXES = ('_din', '_full_n', '_write')
FIFO_READ_PORTS = ('if_dout', 'if_empty_n', 'if_read', 'if_read_ce')
FIFO_WRITE_PORTS = ('if_din', 'if_full_n', 'if_write', 'if_write_ce')
...
class Module:
    ...
    def _calculate_indices(self) -> None:
        for idx, item in enumerate(self._module_def.items):
            if isinstance(item, ast.Decl):
                if any(isinstance(x, (ast.Input, ast.Output, ast.Input)) for x in item.list):
                    self._last_io_port_idx = idx
                else:
                    if any(isinstance(x, (ast.Wire, ast.Reg)) for x in item.list):
                        self._last_signal_idx = idx
                    elif any(isinstance(x, ast.Parameter) for x in item.list):
                        self._last_param_idx = idx
            else:
                if isinstance(item, (ast.Assign, ast.Always)):
                    self._last_logic_idx = idx
                    if isinstance(item, ast.Assign) and item.left.var.name in HANDSHAKE_OUTPUT_PORTS:
    ...
    def _module_def(self) -> ast.ModuleDef:
        _module_defs = [x for x in self.ast.description.definitions if isinstance(x, ast.ModuleDef)]
    ...
    def ports(self) -> Dict[(str, IOPort)]:
        port_lists = (x.list for x in self._module_def.items if isinstance(x, ast.Decl))
        return collections.OrderedDict((x.name, x) for x in itertools.chain.from_iterable(port_lists) if isinstance(x, (ast.Input, ast.Output, ast.Inout)))

    @property
    def params(self) -> Dict[(str, ast.Parameter)]:
        param_lists = (x.list for x in self._module_def.items if isinstance(x, ast.Decl))
        return collections.OrderedDict((x.name, x) for x in itertools.chain.from_iterable(param_lists) if isinstance(x, ast.Parameter))

    @property
    def code(self) -> str:
        return '\n'.join(directive for _, directive in self.directives) + codegen.ASTCodeGenerator().visit(self.ast)
    ...
    def add_ports(self, ports: Iterable[IOPort]) -> 'Module':
        ...
        self._module_def.portlist.ports += tuple(ast.Port(name=(port.name), width=None, dimensions=None, type=None) for port in port_tuple)
        self._module_def.items = self._module_def.items[:self._last_io_port_idx + 1] + port_tuple + self._module_def.items[self._last_io_port_idx + 1:]
        self._increment_idx(len(port_tuple), 'io_port')
        return self

    def add_signals(self, signals: Iterable[Signal]) -> 'Module':
        signal_tuple = tuple(signals)
        self._module_def.items = self._module_def.items[:self._last_signal_idx + 1] + signal_tuple + self._module_def.items[self._last_signal_idx + 1:]
    ...
    """
        ...
        self.add_logics((
         ast.Always(sens_list=CLK_SENS_LIST,
           statement=(ast.make_block(ast.NonblockingSubstitution(left=(q[(i + 1)]), right=(q[i])) for i in range(q.level)))),
    ...
    def add_params(self, params: Iterable[ast.Parameter]) -> 'Module':
        ...
        self._module_def.items = self._module_def.items[:self._last_param_idx + 1] + param_tuple + self._module_def.items[self._last_param_idx + 1:]
    ...
    def add_instance(self, module_name: str, instance_name: str, ports: Iterable[ast.ParamArg], params: Iterable[ast.ParamArg]=()) -> 'Module':
        self._module_def.items = self._module_def.items[:self._last_instance_idx + 1] + (ast.InstanceList(module=module_name, parameterlist=params, instances=(ast.Instance(module=None, name=instance_name, parameterlist=None, portlist=ports),)),) + self._module_def.items[self._last_instance_idx + 1:]
        self._increment_idx(1, 'instance')
        return self

    def add_logics(self, logics: Iterable[Logic]) -> 'Module':
        logic_tuple = tuple(logics)
        self._module_def.items = self._module_def.items[:self._last_logic_idx + 1] + logic_tuple + self._module_def.items[self._last_logic_idx + 1:]
    ...
    def add_fifo_instance(self, name: str, width: int, depth: int) -> 'Module':
        ...
        def ports():
            ...
            yield from (ast.make_port_arg(port=port_name, arg=(wire_name(name, arg_suffix))) for port_name, arg_suffix in zip(FIFO_READ_PORTS, ISTREAM_SUFFIXES))
            yield ast.make_port_arg(port=(FIFO_READ_PORTS[(-1)]), arg=TRUE)
            yield from (ast.make_port_arg(port=port_name, arg=(wire_name(name, arg_suffix))) for port_name, arg_suffix in zip(FIFO_WRITE_PORTS, OSTREAM_SUFFIXES))
        ...
        if partition_count > 1:
            ...
            level.append(ast.ParamArg(paramname='LEVEL',
              argname=(ast.Constant(partition_count))))
        return self.add_instance(module_name=module_name,
          instance_name=name,
          ports=(ports()),
          params=(
         
          ast.ParamArg(paramname='DATA_WIDTH', argname=(ast.Constant(width))),
          ast.ParamArg(paramname='ADDR_WIDTH', argname=(ast.Constant((depth - 1).bit_length()))),
          ast.ParamArg(paramname='DEPTH', argname=(ast.Constant(depth))),
         *level))

    def add_async_mmap_instance(self, name: str, tags: Iterable[str], data_width: int, addr_width: int=64, buffer_size: Optional[int]=None, max_wait_time: int=3, max_burst_len: Optional[int]=None, reg_name: str='') -> 'Module':
        rst_q = Pipeline(f"{name}__rst", level=(self.register_level))
        self.add_pipeline(rst_q, init=(ast.Unot(RST_N)))
        paramargs = [
         ast.ParamArg(paramname='DataWidth', argname=(ast.Constant(data_width))),
         ast.ParamArg(paramname='DataWidthBytesLog', argname=(ast.Constant((data_width // 8 - 1).bit_length())))]
        portargs = [
         ast.make_port_arg(port='clk', arg=CLK),
         ast.make_port_arg(port='rst', arg=(rst_q[(-1)]))]
        paramargs.append(ast.ParamArg(paramname='AddrWidth', argname=(ast.Constant(addr_width))))
        if buffer_size:
            paramargs.extend((
             ast.ParamArg(paramname='BufferSize', argname=(ast.Constant(buffer_size))),
             ast.ParamArg(paramname='BufferSizeLog', argname=(ast.Constant((buffer_size - 1).bit_length())))))
        max_wait_time = max(1, max_wait_time)
        paramargs.append(ast.ParamArg(paramname='WaitTimeWidth', argname=(ast.Constant(max_wait_time.bit_length()))))
        portargs.append(ast.make_port_arg(port='max_wait_time', arg=("{}'d{}".format(max_wait_time.bit_length(), max_wait_time))))
        if max_burst_len is None:
            max_burst_len = max(0, 4096 // data_width - 1)
        paramargs.append(ast.ParamArg(paramname='BurstLenWidth', argname=(ast.Constant(max(1, max_burst_len.bit_length())))))
        portargs.append(ast.make_port_arg(port='max_burst_len', arg=("{}'d{}".format(max(1, max_burst_len.bit_length()), max_burst_len))))
        for channel, ports in M_AXI_PORTS.items():
            for port, direction in ports:
                ...
                if width == 0:
                    ...
                    if width == 1:
                        if port not in frozenset({'ID', 'LOCK'}):
                            width = None
                if width is not None:
                    width = ast.Width(msb=(ast.Constant(width - 1)), lsb=(ast.Constant(0)))
                portargs.append(ast.make_port_arg(port=f"{M_AXI_PREFIX}{channel}{port}", arg=f"{M_AXI_PREFIX}{name}_{channel}{port}"))
            ...
            for suffix in async_mmap_suffixes(tag=tag):
                if tag in tags:
                    ...
                    if tag.endswith('_addr'):
                        if suffix.endswith('_din'):
                            elem_size_bytes_m1 = data_width // 8 - 1
                            arg = "{name} + {{{arg}[{}:0], {}'d0}}".format((addr_width - elem_size_bytes_m1.bit_length() - 1),
        ...
        return self.add_instance(module_name='async_mmap', instance_name=(async_mmap_instance_name(name)),
          ports=portargs,
          params=paramargs)
    ...
    def add_m_axi(self, name: str, data_width: int, addr_width: int=64) -> 'Module':
        for channel, ports in M_AXI_PORTS.items():
            io_ports = []
            for port, direction in ports:
                width = M_AXI_PORT_WIDTHS[port]
                ...
                if width is not None:
                    width = ast.Width(msb=(ast.Constant(width - 1)), lsb=(ast.Constant(0)))
                io_ports.append(ast.Input if direction == 'input' else ast.Output(name=f"{M_AXI_PREFIX}{name}_{channel}{port}",
                  width=width))
...
def is_data_port(port: str) -> bool:
    return port.endswith(ISTREAM_SUFFIXES[0]) or port.endswith(OSTREAM_SUFFIXES[0])
...
def is_m_axi_param(param: Union[(str, ast.Parameter)]) -> bool:
    ...
    return len(param_split) > 5 and param.startswith(M_AXI_PARAM_PREFIX) and ''.join(map('_{}'.format, param_split[-2:])) in M_AXI_PARAM_SUFFIXES
...
def rename_m_axi_name(mapping: Dict[(str, str)], name: str, idx1: int, idx2: int) -> str:
    try:
        ...
        return '_'.join(name_snippets[:idx1] + [mapping['_'.join(name_snippets[idx1:idx2])]] + name_snippets[idx2:])
    except KeyError:
        pass

    raise ValueError("'%s' is a result of renaming done by Vivado HLS; " % '_'.join(name_snippets[idx1:idx2]) + 'please use a different variable name')
...
def rename_m_axi_port(mapping: Dict[(str, str)], port: IOPort) -> IOPort:
    ...
    if port.width is not None:
        if isinstance(port.width.msb, ast.Minus):
            new_port.width = copy.copy(new_port.width)
            new_port.width.msb = copy.copy(new_port.width.msb)
            new_port.width.msb.left = ast.Identifier(rename_m_axi_name(mapping, port.width.msb.left.name, 3, -2))
    return new_port


def rename_m_axi_param(mapping: Dict[(str, str)], param: ast.Parameter) -> ast.Parameter:
    new_param = copy.copy(param)
    new_param.name = rename_m_axi_name(mapping, param.name, 3, -2)
    return new_param


def generate_handshake_ports(instance: tlp.instance.Instance, rst_q: Pipeline) -> Iterator[ast.PortArg]:
    ...
    for port in HANDSHAKE_OUTPUT_PORTS:
        yield ast.make_port_arg(port=port,
          arg=('' if instance.is_autorun else wire_name(instance.name, port)))


def generate_m_axi_ports(port: str, arg: str, arg_reg: str='') -> Iterator[ast.PortArg]:
    for suffix in M_AXI_SUFFIXES:
        yield ast.make_port_arg(port=(M_AXI_PREFIX + port + suffix), arg=(M_AXI_PREFIX + arg + suffix))
...
def generate_istream_ports(port: str, arg: str) -> Iterator[ast.PortArg]:
    for suffix in ISTREAM_SUFFIXES:
        yield ast.make_port_arg(port=(fifo_port_name(port, suffix)), arg=(wire_name(arg, suffix)))
...
def async_mmap_suffixes(tag: str) -> Tuple[(str, str, str)]:
    if tag in frozenset({'write_data', 'write_addr', 'read_addr'}):
        return OSTREAM_SUFFIXES
    if tag == 'read_data':
        return ISTREAM_SUFFIXES
    raise ValueError('invalid tag `%s`; tag must be one of `read_addr`, `read_data`, `write_addr`, or `write_data`' % tag)
...
def async_mmap_width(tag: str, suffix: str, data_width: int) -> Optional[ast.Width]:
...
def generate_async_mmap_ports(tag: str, port: str, arg: str, instance: tlp.instance.Instance) -> Iterator[ast.PortArg]:
    ...
    for suffix in async_mmap_suffixes(tag):
        ...
        if port_name is not None:
            yield ast.make_port_arg(port=port_name, arg=async_mmap_arg_name(arg=arg, tag=tag,
              suffix=suffix))


def generate_async_mmap_signals(tag: str, arg: str, data_width: int) -> Iterator[ast.Wire]:
    for suffix in async_mmap_suffixes(tag):
        yield ast.Wire(name=async_mmap_arg_name(arg=arg, tag=tag, suffix=suffix), width=async_mmap_width(tag=tag, suffix=suffix,
          data_width=data_width))


def generate_async_mmap_ioports--- This code section failed: ---

 L. 859         0  SETUP_LOOP           94  'to 94'
                2  LOAD_GLOBAL              async_mmap_suffixes
                4  LOAD_FAST                'tag'
                6  CALL_FUNCTION_1       1  '1 positional argument'
                8  GET_ITER         
               10  FOR_ITER             92  'to 92'
               12  STORE_FAST               'suffix'

 L. 860        14  LOAD_FAST                'suffix'
               16  LOAD_GLOBAL              ISTREAM_SUFFIXES
               18  LOAD_CONST               -1
               20  BINARY_SUBSCR    
               22  BUILD_SET_1           1 
               24  LOAD_GLOBAL              OSTREAM_SUFFIXES
               26  LOAD_CONST               None
               28  LOAD_CONST               None
               30  LOAD_CONST               2
               32  BUILD_SLICE_3         3 
               34  BINARY_SUBSCR    
               36  BUILD_SET_UNPACK_2     2 
               38  COMPARE_OP               in
               40  POP_JUMP_IF_FALSE    50  'to 50'

 L. 861        42  LOAD_GLOBAL              ast
               44  LOAD_ATTR                Output
               46  STORE_FAST               'ioport_type'
               48  JUMP_FORWARD         56  'to 56'
               50  ELSE                     '56'

 L. 863        50  LOAD_GLOBAL              ast
               52  LOAD_ATTR                Input
               54  STORE_FAST               'ioport_type'
             56_0  COME_FROM            48  '48'

 L. 864        56  LOAD_FAST                'ioport_type'
               58  LOAD_GLOBAL              async_mmap_arg_name
               60  LOAD_FAST                'arg'
               62  LOAD_FAST                'tag'
               64  LOAD_FAST                'suffix'
               66  LOAD_CONST               ('arg', 'tag', 'suffix')
               68  CALL_FUNCTION_KW_3     3  '3 total positional and keyword args'

 L. 865        70  LOAD_GLOBAL              async_mmap_width
               72  LOAD_FAST                'tag'

 L. 866        74  LOAD_FAST                'suffix'

 L. 867        76  LOAD_FAST                'data_width'
               78  LOAD_CONST               ('tag', 'suffix', 'data_width')
               80  CALL_FUNCTION_KW_3     3  '3 total positional and keyword args'
               82  LOAD_CONST               ('name', 'width')
               84  CALL_FUNCTION_KW_2     2  '2 total positional and keyword args'
               86  YIELD_VALUE      
               88  POP_TOP          
               90  JUMP_BACK            10  'to 10'
               92  POP_BLOCK        
             94_0  COME_FROM_LOOP        0  '0'

Parse error at or near `BUILD_SET_UNPACK_2' instruction at offset 36


def pack(top_name: str, rtl_dir: str, ports: Iterable[tlp.instance.Port], output_file: Union[(str, BinaryIO)]) -> None:
    port_tuple = tuple(ports)
    if isinstance(output_file, str):
        xo_file = output_file
    else:
        xo_file = tempfile.mktemp(prefix=('tlp_' + top_name + '_'), suffix='.xo')
    with tempfile.NamedTemporaryFile(mode='w+', prefix=('tlp_' + top_name + '_'),
      suffix='_kernel.xml') as (kernel_xml_obj):
        ...
        with backend.PackageXo(xo_file=xo_file,
          top_name=top_name,
         ...
         tlp.instance.Instance.Arg.Cat.ASYNC_MMAP})) as (proc):
...
def print_constraints(instance_dict: Dict[(str, str)], output: TextIO) -> None:
    ...
    for pblock, modules in directive_dict.items():
        output.write(f"add_cells_to_pblock [get_pblocks {pblock}] [get_cells -hierarchical -regex {{\n")
...
def print_kernel_xml(name: str, ports: Iterable[tlp.instance.Port], kernel_xml: IO[str]) -> None:
        ...
        if port.cat == tlp.instance.Instance.Arg.Cat.SCALAR:
            ...
            if port.cat in {
             ...
             tlp.instance.Instance.Arg.Cat.ASYNC_MMAP}:
                ...
                if port.cat == tlp.instance.Instance.Arg.Cat.ISTREAM:
                    cat = backend.Cat.ISTREAM
                else:
                    if port.cat == tlp.instance.Instance.Arg.Cat.OSTREAM:
                        cat = backend.Cat.OSTREAM
                    else:
                        raise ValueError(f"unexpected port.cat: {port.cat}")
        args.append(backend.Arg(cat=cat, name=(port.name),
          port=port_name,
          ctype=(port.ctype),
          width=(port.width)))
...
OTHER_MODULES = {'fifo_bram':backend.BRAM_FIFO_TEMPLATE.format(name='fifo_bram', width=32,
   depth=32,
   addr_width=(31).bit_length()), 
 'fifo_srl':backend.SRL_FIFO_TEMPLATE.format(name='fifo_srl', width=32,
   depth=32,
   addr_width=(31).bit_length()), 
 'fifo':backend.AUTO_FIFO_TEMPLATE.format(name='fifo', width=32,
   depth=32,
   addr_width=(31).bit_length())}