...
# [GCC 8.4.0]
# Embedded file name: /usr/local/lib/python3.6/dist-packages/configfiles/local/db.py
# Compiled at: 2018-11-06 09:11:46
...
import tempfile, subprocess, click
...
class DotConfigFiles:
    ...
    def sync(self, fastforward=True, remote=None, maxiter=-1):
        ...
        """
        if remote != None:
            if get_remote_hash(remote) != self.current_remote:
                if self.index['at'] != '':
        ...
        if fastforward:
            ...
            if all([x in self.index['files'] and target in self.index['files'][x]['chain'] for x in current_script['files']]):
