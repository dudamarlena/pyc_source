...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/friends/views.py
...
class MemberDetail(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({'object': self.member, 'notifications': Notification.objects.filter(member=self.request.user).count() if member_is_self else False, 
...
class Inbox(ListView):
    ...
    def get_queryset(self):
        return DirectMessage.objects.filter(Q(to_member=self.request.user) | Q(from_member=self.request.user), reply_to=None).exclude(state='archived').order_by('-created', '-state')
...
class ViewMessage(DetailView):
    ...
    def get_queryset(self):
        return DirectMessage.objects.filter(Q(to_member=self.request.user) | Q(from_member=self.request.user))
...
class ReplyToDirectMessage(CreateView):
    ...
    def get_object(self):
        try:
            ...
            raise Http404(_('No DirectMessage found matching the query'))

    def get_form_kwargs(self):
        kwargs = super(ReplyToDirectMessage, self).get_form_kwargs()
        kwargs.update({'from_member': self.message.to_member if self.message.to_member.id == self.request.user.id else self.message.from_member, 'to_member': self.message.from_member if self.message.to_member.id == self.request.user.id else self.message.to_member, 
           'reply_to': self.message})
        return kwargs

    def get_context_data(self, **kwargs):
        context = super(ReplyToDirectMessage, self).get_context_data(**kwargs)
        context.update({'unread_messages': DirectMessage.objects.filter(to_member__id=self.request.user.id, state='sent').count(), 'original_message': self.message})
        return context
...
def friend_request(request, member_id):
    ...
    if request.method == 'POST':
        form = FriendRequestForm(request.POST, initial=dict(member=member, friend=friend), request=request)
        if form.is_valid():
            instance = form.save()
        ...
        form = FriendRequestForm(initial=dict(member=request.user, friend=friend), request=request)
...
class MyFriendRequests(ListView):

    def get_queryset(self, *args, **kwargs):
        return MemberFriend.objects.filter(friend=self.request.user, state='invited')
...
def accept_friend_request(request, memberfriend_id):
    obj = get_object_or_404(MemberFriend, id=memberfriend_id, friend=request.user, state='invited')
    obj.accept()
