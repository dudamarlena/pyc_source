...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/friends/models.py
...
def can_friend(self, friend):
    if self == friend:
        return False
    return not MemberFriend.objects.filter(Q(member=self, friend=friend) | Q(member=friend, friend=self)).exists()
...
def get_friends_with_ids(self, exclude_ids=[], limit=0, pick_random=False):
    ...
    if not hasattr(Member, '_excluded_member_ids') and hasattr(settings, 'EXCLUDED_MEMBERS'):
        Member._excluded_member_ids = Member.objects.filter(username__in=settings.EXCLUDED_MEMBERS).values_list('id', flat=True)
    exclude_ids += Member._excluded_member_ids
    qs = MemberFriend.objects.filter(Q(member=self) | Q(friend=self), state='accepted').exclude(Q(member__in=exclude_ids) | Q(friend__in=exclude_ids))
    if pick_random:
        qs = qs.order_by('?')
    if limit > 0:
        qs = qs[0:limit]
    ...
    return (
     Member.objects.filter(id__in=ids), ids)
...
class MemberFriend(models.Model):
    ...
    state = models.CharField(max_length=32, default='invited', db_index=True, choices=(
     ('invited', 'Invited'),
     ('accepted', 'Accepted'),
     ('declined', 'Declined')))
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            link, dc = Link.objects.get_or_create(title=ugettext('You have pending friend requests'), view_name='my-friend-requests')
...
class DirectMessage(models.Model):
    ...
    state = models.CharField(max_length=32, default='sent', db_index=True, choices=(
     ('sent', 'Sent'),
     ('read', 'Read'),
     ('archived', 'Archived')))
