...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-armv7l/egg/NaughtyClustered/naughty_and_nice_cluster.py
# Compiled at: 2019-06-25 23:14:44
# Size of source mod 2**32: 9444 bytes
import dispy, tweepy, json, re, string
...
def sort_tweets(tweets, pos_emojis, neg_emojis):
    ...
    pos_tweets = [re.sub('[^\\x00-\\x7F]+', '', tweet) for tweet in pos_tweets]
    neg_tweets = [re.sub('[^\\x00-\\x7F]+', '', tweet) for tweet in neg_tweets]
...
def getUsers(tweets, all_emojis):
    ...
    users = [re.sub(':\\S+', '', tweet) for tweet in users]
