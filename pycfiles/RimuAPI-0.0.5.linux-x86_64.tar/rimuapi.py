...
class Api:
    ...
    def __send_request(self, url, data=None, method='GET'):
        if self._key:
            headers = {'Authorization': 'rimuhosting apikey=%s' % self._key, 'Content-Type': 'application/json', 
    ...
    def reinstall(self, domain, order_oid, **kwargs):
        ...
        _req['vps_parameters'] = _params
        payload = {'new_order_request': _req}
        r = self.__send_request('/r/orders/order-%s-%s/vps/reinstall' % (order_oid, domain), data=payload, method='PUT')
        if r.ok:
            return r.json()
        raise Exception(r.status_code, r.reason, r.text)
    ...
    def delete(self, domain, order_oid):
        if not valid_domain_name(domain):
            ...
            r = self.__send_request('/r/orders/order-%s-%s/vps' % (order_oid, domain), method='DELETE')
            if r.ok:
                return r.json()
            raise Exception(r.status_code, r.reason, r.text)
    ...
    def change_state(self, domain, order_oid, new_state):
        if not valid_domain_name(domain):
            raise Exception(418, 'Domain not valid')
        r = self.__send_request('/r/orders/order-%s-%s/vps/running-state' % (order_oid, domain), data={'running_state_change_request': {'running_state': new_state}}, method='PUT')
        if r.ok:
            return r.json()
        raise Exception(r.status_code, r.reason)
    ...
    def move(self, domain, order_oid, update_dns=False, move_reason='', pricing_change_option='CHOOSE_BEST_OPTION', selected_host_server_oid=None):
        if not valid_domain_name(domain):
            raise Exception(418, 'Domain not valid')
        r = self.__send_request('/r/orders/order-%s-%s/vps/host-server' % (order_oid, domain), data={'vps_move_request': {'is_update_dns': is_update_dns, 
                                'move_reason': move_reason, 
                                'pricing_change_option': pricing_change_option, 
                                'selected_host_server_oid': selected_host_server_oid}}, method='PUT')
        if r.ok:
            return r.json()
        raise Exception(r.status_code, r.reason)