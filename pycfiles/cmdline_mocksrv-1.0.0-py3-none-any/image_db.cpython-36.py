...
# Embedded file name: c:\users\user\documents\pycharmprojects\imageanalyser\ipl\database\image_db.py
# Compiled at: 2020-04-04 03:19:09
# Size of source mod 2**32: 9638 bytes
import datetime, functools, io, os, sqlite3
...
class ImageDatabase:
    ...
    def execute_statement(self, statement: str, *args):
        self.cursor.execute(statement, args)

    def fetch_n(self, fetch_limit: Optional[int]):
    ...
    def insert_image(self, field_id: int, revision: int, image_bitmap: np.ndarray, capture_date: datetime.date, capture_satellite: str, mysterious_date: datetime.date):
        if not self.check_if_image_details_exist(field_id, revision, capture_date, mysterious_date, capture_satellite):
            if not self.check_if_field_exists(field_id):
                self.insert_field(field_id)
            statement = _get_sql_statement('insert_image')
            self.execute_statement(statement, field_id, revision, image_bitmap, capture_date, capture_satellite, mysterious_date)
            return self.cursor.lastrowid
        else:
            return

    @_interacts_with_database
    def insert_field(self, field_id: int):
    ...
    @_interacts_with_database
    def insert_image_statistics(self, image_id: int, cloudiness: float, index_average: float, standard_deviation: float, confidence_interval_lower: float, confidence_interval_upper: float):
        statement = _get_sql_statement('insert_image_statistics')
        self.execute_statement(statement, image_id, cloudiness, index_average, standard_deviation, confidence_interval_lower, confidence_interval_upper)
        return self.cursor.lastrowid

    @_interacts_with_database
    def check_if_image_details_exist(self, field_id: int, revision: int, capture_date: datetime.date, mysterious_date: datetime.date, capture_satellite: str):
        statement = _get_sql_statement('check_if_image_details_exist')
        self.execute_statement(statement, field_id, revision, capture_date, mysterious_date, capture_satellite)
        return self.cursor.fetchone()[0]

    @_interacts_with_database
    def check_if_field_exists(self, field_id: int):
        statement = _get_sql_statement('check_if_field_exists')
        self.execute_statement(statement, field_id)
        return self.cursor.fetchone()[0]

    @_interacts_with_database
    def check_if_image_exist(self, image_id: int):
        statement = _get_sql_statement('check_if_image_exist')
        self.execute_statement(statement, image_id)
        return self.cursor.fetchone()[0]

    @_interacts_with_database
    def check_if_has_cached_statistics(self, image_id: int):
        statement = _get_sql_statement('check_if_has_cached_statistics')
        self.execute_statement(statement, image_id)
        return self.cursor.fetchone()[0]

    @_interacts_with_database
    def select_field_images(self, field_id: int, filtered_columns: Optional[List[str]]=None, date_start: datetime.date=datetime.date.min, date_end: datetime.date=datetime.date.max, limit: Optional[int]=None):
    ...
    @_interacts_with_database
    def select_image(self, image_id: int, filtered_columns: Optional[List[str]]=None):
        statement = 'SELECT * FROM image WHERE image_id = ?'
    ...
    @_interacts_with_database
    def select_field_statistics(self, field_id: str, filtered_columns: Optional[List[str]]=None, date_start: datetime.date=datetime.date.min, date_end: datetime.date=datetime.date.max, max_cloudiness: float=1.0):
