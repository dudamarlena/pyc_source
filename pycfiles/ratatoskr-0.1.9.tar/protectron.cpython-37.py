...
# [GCC 8.4.0]
# Embedded file name: /Users/ngergo/Workspaces/Nordcloud/ratatoskr/ratatoskr/protectron.py
...
def protectron(input_schema, output_schema=schema.EmptySchema()):
    ...
    def protectron_decorator(func):
        ...
        def func_wrapper(*args, **kwargs):
            ...
            LOG.debug('input schema [%s] is applied to arguments [%s]', input_schema, arguments)
            output = output_schema(func(**arguments))
            LOG.debug('output schema [%s] is applied to return value[%s]', output_schema, output)
            return output
