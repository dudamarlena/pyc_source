...
# [GCC 8.4.0]
# Embedded file name: /Users/ngergo/Workspaces/Nordcloud/ratatoskr/ratatoskr/operation_registry.py
...
from .exceptions import InvalidOperationWrapperError, OperationAlreadyRegisteredError, UnregisteredOperationError
...
class OperationRegistry:
    ...
    def register_operation(cls, operation_wrapper):
        ...
        if not issubclass(operation_wrapper.__class__, OperationWrapper):
            LOG.error('operation_wrapper [%s] is not a sublass of OperationWrapper', operation_wrapper.__class__)
        ...
        if operation_name in OperationRegistry.registry:
            LOG.error('operation [%s] is already registered in OperationRegistry', operation_name)
            raise OperationAlreadyRegisteredError('%s' % operation_name)
        OperationRegistry.registry[operation_name] = operation_wrapper
        LOG.info('operation [%s] registered, operation_wrapper is [%s]', operation_name, operation_wrapper.__class__)
    ...
    def call(cls, event):
        ...
        LOG.debug('event [%s] is dispatched to operation [%s]', event, operation_name)
        LOG.info('operation [%s] is called', operation_name)
