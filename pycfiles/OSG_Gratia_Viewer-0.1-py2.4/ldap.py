...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gratia/gip/ldap.py
...
def config_file(*additional_files):
    ...
    p.add_option('-c', '--config', dest='config', help='Configuration file.', default='')
...
def query_bdii(cp, query='(objectClass=GlueCE)', binding='o=grid'):
    ...
    fp = os.popen("ldapsearch -h %(hostname)s -p %(port)s -xLLL -b %(binding)s '%(query)s'" % info)
    return fp
