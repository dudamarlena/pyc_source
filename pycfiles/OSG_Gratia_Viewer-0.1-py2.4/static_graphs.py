...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gratia/tools/static_graphs.py
...
def generateImages(cp, timestamp, src, dest, replace=False, variables={}, entry=None):
    for section in cp.sections():
        if entry != None:
            ...
            filename = has_generated or dest % section
    ...
    while curDate <= today:
        ...
        if curDate == today:
            ...
            try:
                ...
                dest = os.path.join(dest, '%s' + suffix)
                generateImages(cp, timestamp, src, dest, replace=True, variables=variables, entry=entry)
...
def main():
    ...
    config_files = [ os.path.expandvars(i) for i in config_files if len(i.strip()) > 0 ]
