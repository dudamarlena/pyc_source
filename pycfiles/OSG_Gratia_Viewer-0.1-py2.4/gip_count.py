...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gratia/tools/gip_count.py
...
def main():
    if len(sys.argv) > 1:
        ...
        ce_glob = '*'
    cp = config_file()
    entries = read_bdii(cp, query='(&(objectClass=GlueCE)(GlueCEInfoHostName=%s))' % ce_glob)
    ...
    for (cluster, cpu) in cluster_info.items():
        ...
        for sc in sc_info[cluster]:
            ...
            print '   - %(SubClusterUniqueID)s, CPU Model: %(HostProcessorModel)s, Cores: %(SubClusterLogicalCPUs)s, KSI2K: %(KSI2K)s' % sc.glue
            ksi2k_ctr += ksi2k
        ...
        elif my_sc_cores < cpu and add_missing and cluster not in do_not_add_missing:
    ...
    print '----------------------------'
    (core_count, msi2k_count, vo_info) = correct_count(cluster_info, ksi2k_info, ownership, correction, duplicate)
    print '----------------------------'
