...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/gratia/gip/analysis.py
...
def correct_sc_info(cluster, cpu, sc_info, specint):
    ...
    for sc in sc_info[cluster]:
        ...
        ksi2k = int(sc.glue['HostBenchmarkSI00']) * int(sc.glue['SubClusterLogicalCPUs']) / 1000
        sc.glue['KSI2K'] = ksi2k
...
def create_count_dict(entries):
    ...
    for entry in entries:
        ...
        if cluster in cluster_info and cluster_info[cluster] <= cpus or cluster not in cluster_info:
...
def correct_count(core_info, ksi2k_info, ownership, correction, duplicate):
    ...
    for cluster in core_info:
        if cluster in duplicate:
            ...
            for entry in duplicate[cluster]:
                if entry in core_info:
                    print 'Not counting %s because it is a duplicate.' % cluster
                    is_duplicate = True
                    break
        ...
        if cluster in correction:
            ...
            print 'Correction factor for %s; before %i cores, %i KSI2K; after %i cores, %i KSI2K' % (cluster, old_cores, old_ksi2k, cores, ksi2k)
        core_count += cores
    ...
    return (
     core_count, msi2k_count, vo_info)