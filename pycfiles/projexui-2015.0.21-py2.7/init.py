...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/init.py
        ...
        if type(package_or_toc) in (str, unicode):
            ...
            if not toc:
                ...
                try:
                    ...
                    try:
                        paths = [
                         os.path.dirname(package_or_toc.__file__)]
                    except AttributeError:
                        paths = []
            ...
            for modname in sorted(toc):
                if callable(ignore) and ignore(modname):
                    ...
                    if type(modname) in (list, tuple) and modname not in ignore:
                        continue
                    if modname.endswith('__init__'):
                        continue
                    else:
                        if modname.endswith('__plugins__'):
                            continue
                        try:
