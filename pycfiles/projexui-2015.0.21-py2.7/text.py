...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/text.py
# Compiled at: 2016-07-03 23:28:12
__doc__ = ' Defines a variety of useful string operators and methods. '
import ast, datetime, logging, re, sys
...
def findkeys(text):
    ...
    :return     [<str>, ..]
    """
    return map(lambda x: x[1], re.findall('(^|[^%])%\\((\\w+)\\)\\w', nativestring(text)))
...
def pluralize(word, count=None, format='{word}'):
    """
            ...
            for pref in prefs:
                ...
                if 'lower' == pref:
                    ...
                    if len(opts) == 2:
                        ...
                        logger.warning('Invalid options for replace: %s', (', ').join(opts))
                elif 'slice' == pref:
                    if len(opts) == 2:
                        value = value[int(opts[0]):int(opts[1])]
                    else:
                        logger.warning('Invalid options for slice: %s', (', ').join(opts))
                elif 'lstrip' == pref:
                    if not opts:
                        value = value.lstrip()
                    else:
