...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/scaffold.py
# Compiled at: 2016-07-03 23:28:12
__doc__ = ' Defines a file/folder template architecture. '
import logging, os, re, tempfile, zipfile
...
class Scaffold(object):
    ...
    def structure(self):
        ...
        if zipfile.is_zipfile(self.source()):
            ...
            try:
                filename = os.path.join(os.path.dirname(self.source()), 'structure.xml')
    ...
    def uifile(self):
        ...
        if zipfile.is_zipfile(self.source()):
            ...
            if 'properties.ui' in zfile.namelist():
                ...
                output = os.path.join(tempdir, ('{0}_properties.ui').format(self.name()))
                f = open(output, 'w')
