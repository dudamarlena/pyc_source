...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/envmanager.py
...
class EnvManager(object):
    ...
    def expandvars(self, text, environ=None, cache=None):
        """
            Recursively expands the text variables, vs. the os.path             method which only works at one level.  The cache value should be             left blank as it is used to protect against recursion.
    ...
    def refactor(module, name, repl):
        """
        Replaces the name in the module dictionary with the inputted replace         value.
    ...
    def packageSplit(filepath):
        ...
        return (
         os.path.sep.join(pathsplit), ('.').join(packagesplit))