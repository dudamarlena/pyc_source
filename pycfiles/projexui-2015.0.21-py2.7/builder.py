...
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/projex/xbuild/builder.py
# Compiled at: 2016-07-03 23:28:12
__doc__ = ' \nDefines the builder class for building versions.\n'
import logging, os, projex, projex.resources, projex.pyi, re, shutil, subprocess, sys, time, zipfile
...
class Builder(object):
    Options = enum('GenerateRevision', 'GenerateDocs', 'GenerateExecutable', 'GenerateInstaller', 'GenerateSetupFile', 'GenerateEgg', 'GenerateZipFile', 'Signed')
    _plugins = {}
    ...
    def generatePlugins(self, basepath):
        for root, folders, files in os.walk(basepath):
            ...
            packages = filter(lambda x: ('{0}/{1}/__init__.py').format(root, x), folders)
    ...
    def generateExecutable(self, outpath='.', signed=False):
        ...
        if not specfile:
            ...
            for typ, data in self.executableData():
                if typ == 'tree':
                    args = {'path': data[0], 'prefix': data[1], 
    ...
    def loadXml(self, xdata, filepath=''):
        ...
        if xexe is not None:
            exe_tags = {'runtime': '_runtime', 'exe': '_executableName', 'cli': '_executableCliName', 
    ...
    def sign(self, filename):
        ...
        if not sign:
            ...
            return False
        if not certificate and '{cert}' in sign:
...
class ApplicationBuilder(PackageBuilder):
    ...
    def __init__(self, pkg):
        ...
        self.setOptions(self.Options.GenerateDocs | self.Options.GenerateExecutable | self.Options.GenerateInstaller)
...
class SignedApplicationBuilder(ApplicationBuilder):
    ...
    def __init__(self, pkg):
        super(ApplicationBuilder, self).__init__(pkg)
        self.setOptions(self.Options.GenerateDocs | self.Options.GenerateExecutable | self.Options.GenerateInstaller | self.Options.Signed)
...
def build_cmd():
    ...
    if '--no-remote' not in sys.argv and 'PYTHON' in env and env['PYTHON'] != sys.executable:
