...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/alchemy/engine.py
# Compiled at: 2016-03-07 07:11:03
import logging
...
from ztfy.alchemy.datamanager import ZopeTransactionExtension, join_transaction, _SESSION_STATE, STATUS_ACTIVE, STATUS_READONLY
...
class AlchemyEngineUtility(Persistent):
    ...
    def __init__(self, name='', dsn='', echo=False, pool_size=25, pool_recycle=-1, register_geotypes=False, register_opengis=False, encoding='utf-8', convert_unicode=False, **kw):
        self.name = name
        self.dsn = dsn
    ...
    def getEngine(self):
        ...
        if engine is not None:
            ...
            kw = {}
            kw.update(self.kw)
            self._v_engine = sqlalchemy.create_engine(str(self.dsn), echo=self.echo, pool_size=self.pool_size, pool_recycle=self.pool_recycle, encoding=self.encoding, convert_unicode=self.convert_unicode, strategy='threadlocal', **kw)
            if self.register_geotypes:
                try:
                    import psycopg2, psycopg2.extensions as psycoext
                    from GeoTypes import initialisePsycopgTypes
                    url = self._v_engine.url
                    initialisePsycopgTypes(psycopg_module=psycopg2, psycopg_extensions_module=psycoext, connect_string='host=%(host)s port=%(port)s dbname=%(dbname)s user=%(user)s password=%(password)s' % {'host': url.host, 'port': url.port, 
                       'dbname': url.database, 
                       'user': url.username, 
                       'password': url.password}, register_opengis_types=self.register_opengis)
                except:
                    pass
...
def getSession(engine, join=True, status=STATUS_ACTIVE, request=None, alias=None, twophase=True, use_zope_extension=True):
    ...
    if session is None:
        ...
        if use_zope_extension:
            factory = scoped_session(sessionmaker(bind=_engine, twophase=twophase, extension=ZopeTransactionExtension()))
        else:
...
def getUserSession(engine, join=True, status=STATUS_ACTIVE, request=None, alias=None, twophase=True, use_zope_extension=True):
    """Shortcut function to get user session"""
    if isinstance(engine, basestring):
        session = getSession(engine, join=join, status=status, request=request, alias=alias, twophase=twophase, use_zope_extension=use_zope_extension)
    else:
        session = engine
    return session
