...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokergui/server/message_manager.py
# Compiled at: 2017-04-01 11:23:14
import time, logging, tornado.escape
...
def _gen_alert_server_restart_message(handler):
    ...
    return {'message_type': 'alert_restart_server', 
       'message': message}
...
def _gen_config_update_message(handler, game_manager, uuid):
    ...
    html_str = handler.render_string('game_config.html', config=game_manager, registered=registered)
    html = tornado.escape.to_basestring(html_str)
    return {'message_type': 'config_update', 
       'html': html, 
...
def _gen_start_game_message(handler, game_manager, uuid):
    registered = game_manager.get_human_player_info(uuid)
    html_str = handler.render_string('poker_game.html', config=game_manager, registered=registered)
    html = tornado.escape.to_basestring(html_str)
    return {'message_type': 'start_game', 
       'html': html}
...
def _gen_game_update_message(handler, message):
    ...
    if 'round_start_message' == message_type:
        ...
        event_html_str = handler.render_string('event_round_start.html', round_count=round_count, hole_card=hole_card)
        ...
        table_html_str = handler.render_string('round_state.html', round_state=round_state)
        event_html_str = handler.render_string('event_update_game.html', action=action, round_state=round_state)
        content = {'update_type': message_type, 
           'table_html': tornado.escape.to_basestring(table_html_str), 
           'event_html': tornado.escape.to_basestring(event_html_str)}
        ...
        round_count = message['message']['round_count']
        table_html_str = handler.render_string('round_state.html', round_state=round_state)
        event_html_str = handler.render_string('event_round_result.html', round_state=round_state, hand_info=hand_info, winners=winners, round_count=round_count)
        content = {'update_type': message_type, 
           'table_html': tornado.escape.to_basestring(table_html_str), 
           'event_html': tornado.escape.to_basestring(event_html_str)}
        ...
        action_histories = message['message']['action_histories']
        table_html_str = handler.render_string('round_state.html', round_state=round_state)
        event_html_str = handler.render_string('event_ask_action.html', hole_card=hole_card, valid_actions=valid_actions, action_histories=action_histories)
        content = {'update_type': message_type, 
           'table_html': tornado.escape.to_basestring(table_html_str), 
           'event_html': tornado.escape.to_basestring(event_html_str)}
    else:
        raise Exception('Unexpected message received : %r' % message)
    return {'message_type': 'update_game', 
       'content': content}
