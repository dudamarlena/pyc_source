...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokergui/server/poker.py
...
import yaml, uuid, tornado.ioloop, tornado.options, tornado.web, tornado.websocket
from tornado.options import define, options
import pypokerengine.utils.action_utils as AU, pypokergui.server.game_manager as GM, pypokergui.server.message_manager as MM
    ...
    def __init__(self):
        handlers = [
         (
          '/', PokerRequestHandler),
         (
          '/pokersocket', PokerWebSocketHandler)]
...
def setup_config(config):
    global_game_manager.define_rule(config['max_round'], config['initial_stack'], config['small_blind'], config['ante'], config['blind_structure'])
