...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/pypokergui/server/game_manager.py
# Compiled at: 2017-03-29 22:05:08
import pypokergui.engine_wrapper as Engine, pypokergui.ai_generator as AG
...
class GameManager(object):
    ...
    def update_game(self, action, amount):
        assert len(self.latest_messages) != 0
    ...
    def ask_action_to_ai_player(self, uuid):
        ...
        return ai_player.declare_action(ask_message['message']['valid_actions'], ask_message['message']['hole_card'], ask_message['message']['round_state'])


def fetch_next_player_uuid(new_messages):
    ask_uuid, ask_message = has_game_finished(new_messages) or new_messages[(-1)]
    if not ask_message['type'] == 'ask':
