...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-intel/egg/tests/pypokergui/server/message_manager_test.py
...
class MessageManagerTest(BaseUnitTest):
    ...
    def test_broadcast_config_update(self):
        ...
        with patch('pypokergui.server.message_manager._gen_config_update_message', side_effect=lambda x, y, uuid: 'config_update:%s' % uuid):
    ...
    def test_broadcast_start_game(self):
        ...
        with patch('pypokergui.server.message_manager._gen_start_game_message', side_effect=lambda x, y, uuid: 'start_game:%s' % uuid):
    ...
    def test_broadcast_update_game(self):
        ...
        with patch('pypokergui.server.message_manager._gen_game_update_message', return_value='update_game'):
            with patch('pypokergui.server.message_manager._broadcast_message_to_ai', side_effect=self._append_log_on_player):
