...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/MonitorPlugins.py
# Compiled at: 2015-05-06 05:03:08
import sys, re, pickle, pkg_resources
...
class MonitorPlugins:
    ...
    def __init__(self, conf=None):
        ...
        if conf.has_option('plugins', 'monitors_enabled'):
            self.enabled = re.split('\\s+', conf.get('plugins', 'monitors_enabled'))
        if conf.has_option('plugins', 'monitors_disabled'):
            self.disabled = re.split('\\s+', conf.get('plugins', 'monitors_disabled'))
        return
...
class MonitorPlugin(object):
    ...
    def _getKernelRev(self):
        ...
        kernel_rev = float(re.search('version (\\d+\\.\\d+)\\.\\d+', version).group(1))
        return kernel_rev
