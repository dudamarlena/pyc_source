...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/Distributed.py
# Compiled at: 2015-05-06 05:03:08
import os, platform, re, socket, threading, time
...
import json, sys, paramiko
from utils import mmn_encode, trace, package_tests, get_virtualenv_script, get_version
try:
    from funkload.rtfeedback import FeedbackPublisher, DEFAULT_ENDPOINT, DEFAULT_PUBSUB
    LIVE_FEEDBACK = True
except ImportError:
    LIVE_FEEDBACK = False
...
class SSHDistributor(DistributorBase):
    ...
    def __init__(self, name, host, username=None, password=None, key_filename=None, channel_timeout=None):
        """
    ...
    @requiresconnection
    def threaded_execute(self, cmd_string, shell_interpreter='bash -c', cwdir=None):
        """
        ...
        class ThreadedExec(threading.Thread):
            ...
            def run(self_):
                ...
                if self_.cwdir:
                    exec_str += '; popd;'
                try:
                    self_.input, self_.output, self_.err = self_.exec_command(self.connection, exec_str, bufsize=1, timeout=self.channel_timeout)
...
class DistributionMgr(threading.Thread):
    ...
    def __init__(self, module_name, class_name, method_name, options, cmd_args):
        """
        ...
        if test.conf_get('distribute', 'channel_timeout', '', quiet=True):
            self.channel_timeout = test.conf_getFloat('distribute', 'channel_timeout')
        else:
        ...
        if options.distributed_log_path:
            ...
            if options.workerlist:
                for h in options.workerlist.split(','):
                    ...
                    if len(cred_host) == 1:
                        ...
                        if len(uname_pwd) == 1:
                            ...
                            uname, pwd = uname_pwd
                    worker = {'name': host.replace(':', '_'), 'host': host, 'password': pwd, 
                       'username': uname, 
                ...
                for host in hosts:
                    ...
                    workers.append({'name': host.replace(':', '_'), 
        ...
        if LIVE_FEEDBACK and options.feedback:
            ...
            self.feedback = FeedbackPublisher(endpoint=options.feedback_endpoint or DEFAULT_ENDPOINT, pubsub_endpoint=options.feedback_pubsub_endpoint or DEFAULT_PUBSUB, handler=_print_rt)
            self.feedback.start()
        else:
            self.feedback = None
        return
    ...
    def __repr__(self):
        ...
        text.append('* Tarred tests: %0.2fMB' % (float(size) / 1000000.0))
        ...
        text.append('* Workers :%s\n\n' % (',').join(w.name for w in self._workers))
        return ('\n').join(text)
    ...
    def prepare_workers(self, allow_errors=False):
        ...
        def local_prep_worker(worker):
            ...
            worker.put(get_virtualenv_script(), os.path.join(remote_res_dir, 'tmpvenv.py'))
            trace('.')
            worker.execute('%s tmpvenv.py %s' % (
             self.python_bin, os.path.join(remote_res_dir, self.tarred_testsdir)), cwdir=remote_res_dir)
        ...
        for worker in list(self._workers):
            ...
            if out.strip() == 'true':
                threads.append(threading.Thread(target=local_prep_worker, args=(
                 worker,)))
            elif allow_errors:
                trace('Cannot find Python binary at path `%s` on %s, ' + 'removing from pool' % (self.python_bin, worker.name))
                self._workers.remove(worker)
            else:
                raise RuntimeError('%s is not contactable with error %s' % (
                 worker.name, worker.error))
    ...
    def run(self):
        ...
        for worker in self._workers:
            ...
            obj = worker.threaded_execute('bin/fl-run-bench ' + (' ').join(self.cmd_args), cwdir=venv)
            trace('.')
            threads.append(obj)
    ...
    def final_collect(self):
        ...
        for worker, results in self._worker_results.items():
            ...
            if res:
                ...
                local_file = os.path.join(self.distribution_output, '%s-%s' % (
    ...
    def write_statistics(self, successful_results):
        ...
        config = {'id': self.test_id, 'description': self.test_description, 'class_title': self.class_title, 
