...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/ReportRenderHtmlGnuPlot.py
...
def gnuplot(script_path):
    ...
    if sys.platform.lower().startswith('win'):
        ret = os.system('cd "' + path + '" && wgnuplot "' + os.path.abspath(script_path) + '"')
        if ret != 0:
            raise RuntimeError('Failed to run wgnuplot cmd on ' + os.path.abspath(script_path))
    else:
...
class RenderHtmlGnuPlot(RenderHtmlBase):
    ...
    def fixXLabels(self, lines):
        ...
        out = out.replace(' 1:', ' :')
        out = self.ticpattern.sub('\\1:xticlabels(1) ', out)
        return out
    ...
    def createPageChart(self):
        ...
        for cycle in self.cycles:
            ...
            score_classes = Apdex.score_classes[:]
    ...
    def createRPSTimeChart(self):
        ...
        min_rps = 0
        for cycle in self.cycles:
            dpath = gnuplot_scriptpath(self.report_dir, ('time_rps-{0}.data').format(cycle))
            f = open(dpath, 'w')
        ...
        for i, cycle in enumerate(self.cycles):
            ...
            plot_line += ' w linespoints smooth sbezier lw 1 lt 2 lc ' + 'rgbcolor "#696969" notitle'
            plot_line += ', \\\n'
            plot_line += '"' + dpath + '" u ($1):($2)'
            plot_line += ' w linespoints lw 1 lt 2 lc ' + ('rgbcolor "#{0}" t "{1} CUs"').format(colors[(i % len(colors))], stats[cycle]['response'].cvus)
    ...
    def createResponseChart(self, step):
        ...
        image_path = gnuplot_scriptpath(self.report_dir, 'request_%s.png' % step)
        gplot_path = str(os.path.join(self.report_dir, 'request_%s.gplot' % step))
        data_path = gnuplot_scriptpath(self.report_dir, 'request_%s.data' % step)
        stats = self.stats
        lines = [
