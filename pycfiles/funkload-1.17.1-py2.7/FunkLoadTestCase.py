...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/FunkLoadTestCase.py
...
"""
import os, sys, time, string, re, logging, gzip, threading
    ...
    def __getattr__(self, section):
        ...
        class ConfKeyFinder(object):
            ...
            def __getattr__(sself, attr):
                assert sself.validInput(section, attr), 'To use the convenient .section.attr access to your config variables, they can not contain any special characters (alphanumeric and _ only)'
...
class FunkLoadTestCase(unittest.TestCase):
    ...
    def _funkload_init(self):
        ...
        self.setOkCodes(self.conf_getList(section, 'ok_codes', [
         200, 301, 302, 303, 307], quiet=True))
    ...
    def _connect(self, url, params, ok_codes, rtype, description, redirect=False, consumer=None):
        ...
        t_start = time.time()
        try:
            response = self._browser.fetch(url, params, ok_codes=ok_codes, key_file=self._keyfile_path, cert_file=self._certfile_path, method=rtype, consumer=consumer)
        except:
    ...
    def _browse(self, url_in, params_in=None, description=None, ok_codes=None, method='post', follow_redirect=True, load_auto_links=True, sleep=True):
        ...
        if self._loop_mode and self.steps == self._loop_steps[(-1)]:
            ...
            text = 'End of loop: %d pages rendered in %.3fs, avg of %.3fs per page, %.3f SPPS without concurrency.' % (
             count, t_delta,
             t_delta / count,
             count / t_delta)
            self.logi(text)
    ...
    def post(self, url, params=None, description=None, ok_codes=None, load_auto_links=True, follow_redirect=True):
        ...
        self.steps += 1
        self.page_responses = 0
        response = self._browse(url, params, description, ok_codes, method='post', load_auto_links=load_auto_links, follow_redirect=follow_redirect)
        return response
    ...
    def get(self, url, params=None, description=None, ok_codes=None, load_auto_links=True, follow_redirect=True):
        ...
        self.steps += 1
        self.page_responses = 0
        response = self._browse(url, params, description, ok_codes, method='get', load_auto_links=load_auto_links, follow_redirect=follow_redirect)
        return response
    ...
    def exists(self, url, params=None, description='Checking existence'):
        ...
        resp = self.get(url, params, description=description, ok_codes=[
         200, 301, 302, 303, 307, 404, 503], load_auto_links=False)
    ...
    def waitUntilAvailable(self, url, time_out=20, sleep_time=2):
        ...
        while True:
            try:
                self._browser.fetch(url, None, ok_codes=[
                 200, 301, 302, 303, 307], key_file=self._keyfile_path, cert_file=self._certfile_path, method='get')
            except SocketError:
    ...
    def __call__(self, result=None):
        ...
        try:
            ...
            try:
                if not self.in_bench_mode:
                    self.logd('Starting -----------------------------------\n\t%s' % self.conf_get(self.meta_method_name, 'description', ''))
                self.setUp()
