...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/demo/cps/CPS338TestCase.py
...
class CPSTestCase(ZopeTestCase):
    ...
    def cpsLogin(self, login, password, comment=None):
        ...
        params = [['__ac_name', login],
         ...
         [
          'submit', 'Login']]
        self.post('%s/logged_in' % self.server_url, params, description='Log in user [%s] %s' % (login, comment or ''))
        self.assert_('%s/logout' % self.server_url in self.listHref(), 'invalid credential: [%s:%s].' % (login, password))
    ...
    def cpsLogout(self):
        ...
        if self._cps_login is not None:
            self.get('%s/logout' % self.server_url, description='Log out [%s]' % self._cps_login)
        return

    def cpsCreateSite(self, admin_id, admin_pwd, manager_id, manager_password, manager_mail, langs=None, title=None, description=None, interface='portlets', zope_url=None, site_id=None):
        """Create a CPS Site.
        ...
        params = {'id': site_id, 'title': title or 'CPS Portal', 
           ...
           'interface': interface, 
           'submit': 'Create'}
        self.post('%s/manage_addProduct/CPSDefault/manage_addCPSDefaultSite' % zope_url, params, description='Create a CPS Site')
        self.clearBasicAuth()
        return
    ...
    def cpsCreateGroup(self, group_name):
        ...
        self.assert_(self.getLastUrl().find('psm_entry_created') != -1, 'Failed to create group %s' % group_name)
    ...
    def cpsVerifyGroup(self, group_name):
        ...
        params = [['dirname', 'groups'],
         [
          'id', group_name]]
        if self.exists('%s/cpsdirectory_entry_view' % server_url, params, description='Check that group [%s] exists.' % group_name):
            self.logd('Group %s exists.')
        else:
            self.cpsCreateGroup(group_name)

    def cpsCreateUser(self, user_id=None, user_pwd=None, user_givenName=None, user_sn=None, user_email=None, groups=None):
        ...
        self.post('%s/' % self.server_url, params, description='Create user [%s]' % user_id)
        self.assert_(self.getLastUrl().find('psm_entry_created') != -1, 'Failed to create user %s' % user_id)
        return (user_id, user_pwd)

    def cpsVerifyUser(self, user_id=None, user_pwd=None, user_givenName=None, user_sn=None, user_email=None, groups=None):
        ...
        if user_id:
            params = [
             ...
             [
              'id', user_id]]
            if self.exists('%s/cpsdirectory_entry_view' % self.server_url, params):
                self.logd('User %s exists.')
                return (
                 user_id, None)
        return self.cpsCreateUser(user_id, user_pwd, user_givenName, user_sn, user_email, groups)

    def cpsSetLocalRole(self, url, name, role):
        """Setup local role role to url."""
        params = [
         [
          'member_ids:list', name],
         [
          'member_role', role]]
        self.post('%s/folder_localrole_add' % url, params, description='Grant local role %s to %s' % (role, name))

    def cpsCreateSection(self, parent_url, title, description='ftest section for funkload testing.', lang=None):
        """Create a section."""
        return self.cpsCreateFolder('Section', parent_url, title, description, lang or self.cpsGetRandomLanguage())

    def cpsCreateWorkspace(self, parent_url, title, description='ftest workspace for funkload testing.', lang=None):
        """Create a workspace."""
        return self.cpsCreateFolder('Workspace', parent_url, title, description, lang or self.cpsGetRandomLanguage())

    def cpsCreateFolder(self, type, parent_url, title, description, lang):
        ...
        self.post('%s/cpsdocument_create_form' % parent_url, params, 'Create a %s' % type)
    ...
    def cpsCreateDocument(self, parent_url):
        ...
        params = [['type_name', 'Document'],
         ...
         [
          'cpsdocument_create_button', 'Create']]
        self.post('%s/cpsdocument_create_form' % parent_url, params, description='Creating a document')
        self.assert_(self.getLastUrl().find('psm_content_created') != -1, 'Failed to create [%s] in %s/.' % (title, parent_url))
    ...
    def cpsCreateNewsItem(self, parent_url):
        ...
        params = [['type_name', 'News Item'],
         ...
         [
          'cpsdocument_create_button', 'Create']]
        self.post('%s/cpsdocument_create_form' % parent_url, params, description='Creating a news item')
        last_url = self.getLastUrl()
        self.assert_('psm_content_created' in last_url, 'Failed to create [%s] in %s/.' % (title, parent_url))
        doc_url = self.cpsCleanUrl(self.getLastBaseUrl())
        doc_id = doc_url.split('/')[(-1)]
        return (doc_url, doc_id)
    ...
    def cpsChangeUiLanguage(self, lang):
        ...
        self.get('%s/cpsportlet_change_language' % self.server_url, params=[
         [
          'lang', lang]], description='Change UI language to %s' % lang)
    ...
    def cpsSearchDocId(self, doc_id):
        ...
        params = [
         [
          'SearchableText', doc_id]]
        self.post('%s/search_form' % self.server_url, params, description='Searching doc_id %s' % doc_id)
    ...
    def cpsCleanUrl(self, url_in):
        ...
        url = url_in
        server_url = self.server_url
        for ending in ('/', '/view', '/folder_contents', '/folder_view', '/cpsdocument_metadata',
