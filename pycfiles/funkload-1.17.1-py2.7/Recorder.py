...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/Recorder.py
...
class Request():
    ...
    def extractParam(self):
        ...
        try:
            ...
            trace('# Warning: missing header content-type or content-length in file: %s not an http request ?\n' % self.file_path)
            return params
...
class RecorderProgram():
    ...
    tcpwatch_cmd = [
     'tcpwatch-httpproxy', 'tcpwatch.py', 'tcpwatch']
    ...
    def getTcpWatchCmd(self):
        ...
        raise RuntimeError('Tcpwatch is not installed no %s found. Visit http://funkload.nuxeo.org/INSTALL.html' % str(self.tcpwatch_cmd))
    ...
    def parseArgs(self, argv):
        ...
        parser.add_option('-L', type='string', dest='forward', default=None, help='Forwarded connection <listen_port>:<dest_host>:<dest_port>.')
        ...
        parser.add_option('-l', '--loop', type='int', dest='loop', default=1, help='Loop mode.')
    ...
    def extractRequests(self, files):
        ...
        for request_path, response_path in files:
            ...
            url = request.url
            if request.method != 'POST' and (last_code in ('301', '302') or [ x for x in filter_ctypes if x in ctype ] or [ x for x in filter_url if url.endswith(x) ]):
    ...
    def writeScript(self, script):
        ...
        content = tpl % {'script': script, 'test_name': self.test_name, 
    ...
    def writeConfiguration(self):
        ...
        content = tpl % {'server_url': self.server_url, 'test_name': self.test_name, 
           'class_name': self.class_name}
