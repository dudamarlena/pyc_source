...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/ReportRenderRst.py
...
class AllResponseRst(BaseRst):
    ...
    headers = [
     'CUs', 'Apdex*', 'Rating*', 'RPS', 'maxRPS', 'TOTAL', 'SUCCESS', 'ERROR',
     'MIN', 'AVG', 'MAX']
...
class PageRst(AllResponseRst):
    """Page rendering."""
    headers = [
     'CUs', 'Apdex*', 'Rating', 'SPPS', 'maxSPPS', 'TOTAL', 'SUCCESS',
     'ERROR', 'MIN', 'AVG', 'MAX']
...
class ResponseRst(BaseRst):
    """Response rendering."""
    headers = [
     'CUs', 'Apdex*', 'Rating', 'TOTAL', 'SUCCESS', 'ERROR', 'MIN', 'AVG', 'MAX']
    indent = 4
    ...
    def __init__(self, stats):
        ...
        self.image_names = [ name + str(stats.step) + '.' + str(stats.number) for name in self.image_names
                           ]
...
class RenderRst:
    ...
    def renderConfig(self):
        ...
        self.append(LI + ' Sleeptime between test cases: %ss' % config['sleep_time'])
        self.append(LI + ' Startup delay between threads: %ss' % config['startup_delay'])
    ...
    def renderTestContent(self, test):
        ...
        self.append(LI + ' %s tests' % total_tests + (total_tests_error and ',' + pluralized_t_errs or ''))
        self.append(LI + ' %s pages' % total_pages + (total_pages_error and ',' + pluralized_p_errs or ''))
        self.append(LI + ' %s requests' % total_responses + (total_responses_error and ',' + pluralized_r_errs or ''))
        self.append('')
    ...
    def renderCyclesStat(self, key, title, description=''):
        ...
        if renderer is not None:
            ...
            self.append('Sorry no %s have finished during a cycle, the cycle duration is too short.\n' % key)
        return
    ...
    def renderSlowestRequests(self, number):
        ...
        if not (cycle and stats[cycle].has_key('response_step')):
            ...
            items.sort()
            items.reverse()
            self.append('The %d slowest average response time during the best cycle with **%s** CUs:\n' % (
             number, cycle_name))
        ...
        for status in ('Failure', 'Error'):
            ...
            for err_type in err_types:
                ...
                if err_type[1]:
                    ...
                    traceback = stat.traceback and stat.traceback.replace('File ', '\n    File ') or 'No traceback.'
    ...
    def renderDefinitions(self):
        ...
        self.append(LI + ' CUs: Concurrent users or number of concurrent threads executing tests.')
        self.append(LI + ' Request: a single GET/POST/redirect/XML-RPC request.')
        self.append(LI + ' Page: a request with redirects and resource links (image, css, js) for an HTML page.')
        ...
        self.append(LI + ' P10: 10th percentile, response time where 10 percent of pages or requests are delivered.')
        self.append(LI + ' MED: Median or 50th percentile, response time where half of pages or requests are delivered.')
        self.append(LI + ' P90: 90th percentile, response time where 90 percent of pages or requests are delivered.')
        self.append(LI + ' P95: 95th percentile, response time where 95 percent of pages or requests are delivered.')
        self.append(LI + Apdex.description_para)
        self.append(LI + Apdex.rating_para)
        self.append('')
        self.append('Report generated with FunkLoad_ ' + get_version() + ', more information available on the `FunkLoad site <http://funkload.nuxeo.org/#benching>`_.')
    ...
    def __repr__(self):
        ...
        self.renderCyclesStat('test', 'Test stats', 'The number of Successful **Tests** Per Second (STPS) over Concurrent Users (CUs).')
        self.renderCyclesStat('page', 'Page stats', 'The number of Successful **Pages** Per Second (SPPS) over Concurrent Users (CUs).\nNote: an XML-RPC call counts as a page.')
        self.renderCyclesStat('response', 'Request stats', 'The number of **Requests** Per Second (RPS) (successful or not) over Concurrent Users (CUs).')
    ...
    def getMonitorConfig(self, host):
        ...
        if host in self.monitorconfig:
            ...
            return {'MonitorCPU': MonitorCPU().getConfig(), 'MonitorMemFree': MonitorMemFree().getConfig(), 
