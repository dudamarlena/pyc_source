...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/BenchRunner.py
# Compiled at: 2015-05-06 05:03:08
"""FunkLoad Bench runner.

$Id: BenchRunner.py 24746 2005-08-31 09:59:27Z bdelbosc $
"""
import os, platform, sys, threading, time, traceback, unittest
...
from utils import mmn_encode, set_recording_flag, recording, thread_sleep, trace, red_str, green_str, get_version
try:
    from funkload.rtfeedback import FeedbackSender, DEFAULT_ENDPOINT, DEFAULT_PUBSUB
    LIVE_FEEDBACK = True
except ImportError:
    LIVE_FEEDBACK = False
...
class LoopTestRunner(threading.Thread):
    ...
    def __init__(self, test_module, test_class, test_name, options, cycle, cvus, thread_id, thread_signaller, sleep_time, debug=False, feedback=None):
        meta_method_name = mmn_encode(test_name, cycle, cvus, thread_id)
        threading.Thread.__init__(self, target=self.run, name=meta_method_name, args=())
        self.test = load_unittest(test_module, test_class, meta_method_name, options)
...
class BenchRunner():
    ...
    def __init__(self, module_name, class_name, method_name, options):
        ...
        self.test_description = test.conf_get(self.method_name, 'description', 'No test description')
        ...
        if LIVE_FEEDBACK and options.is_distributed and options.feedback:
            ...
            self.feedback = FeedbackSender(endpoint=options.feedback_endpoint or DEFAULT_ENDPOINT)
        else:
            self.feedback = None
        return
    ...
    def run(self):
        ...
        for cvus in self.cycles:
            ...
            trace('* Cycle result: **%s**, %i success, %i failure, %i errors.\n\n' % (
    ...
    def createThreads(self, cycle, number_of_threads):
        ...
        for i in range(number_of_threads):
            ...
            thread = LoopTestRunner(self.module_name, self.class_name, self.method_name, self.options, cycle, number_of_threads, thread_id, thread_signaller, self.sleep_time, feedback=self.feedback)
            trace('.')
            try:
                thread.start()
            except ThreadError:
    ...
    def logr_open(self):
        """Start logging tag."""
        config = {'id': self.test_id, 'description': self.test_description, 
...
class BenchLoader(unittest.TestLoader):
    ...
    def loadTestsFromTestCase(self, testCaseClass):
        ...
        if not testCaseNames and hasattr(testCaseClass, 'runTest'):
            testCaseNames = [
             'runTest']
        return [ dict(module_name=testCaseClass.__module__, class_name=testCaseClass.__name__, method_name=x) for x in testCaseNames
               ]
...
def discover(sys_args):
    ...
    for test in flattened:
        ...
        if options.distribute:
            ...
            ret = run_distributed(options, module_name, class_name, method_name, dist_args)
        else:
...
def parse_sys_args(sys_args):
    parser = get_shared_OptionParser()
    parser.add_option('', '--config', type='string', dest='config', metavar='CONFIG', help='Path to alternative config file')
    parser.add_option('-l', '--label', type='string', help='Add a label to this bench run for easier identification (it will be appended to the directory name for reports generated from it).')
    options, args = parser.parse_args(sys_args)
    if len(args) != 2:
...
def get_shared_OptionParser():
    ...
    discover mode.
    """
    parser = OptionParser(USAGE, formatter=TitledHelpFormatter(), version='FunkLoad %s' % get_version())
    parser.add_option('-r', '--runner-class', type='string', dest='bench_runner_class', default='funkload.BenchRunner.BenchRunner', help='Python dotted import path to BenchRunner class to use.')
    parser.add_option('', '--no-color', action='store_true', help='Monochrome output.')
    parser.add_option('', '--accept-invalid-links', action='store_true', help='Do not fail if css/image links are not reachable.')
    parser.add_option('', '--simple-fetch', action='store_true', dest='bench_simple_fetch', help="Don't load additional links like css or images when fetching an html page.")
    parser.add_option('--enable-debug-server', action='store_true', dest='debugserver', help='Instantiates a debug HTTP server which exposes an interface using which parameters can be modified at run-time. Currently supported parameters: /cvu?inc=<integer> to increase the number of CVUs, /cvu?dec=<integer> to decrease the number of CVUs, /getcvu returns number of CVUs ')
    parser.add_option('--debug-server-port', type='string', dest='debugport', help='Port at which debug server should run during the test')
    parser.add_option('--distribute', action='store_true', dest='distribute', help='Distributes the CVUs over a group of worker machines that are defined in the workers section')
    parser.add_option('--distribute-workers', type='string', dest='workerlist', help='This parameter will  override the list of workers defined in the config file. expected notation is uname@host,uname:pwd@host or just host...')
    parser.add_option('--distribute-python', type='string', dest='python_bin', help='When running in distributed mode, this Python binary will be used across all hosts.')
    parser.add_option('--is-distributed', action='store_true', dest='is_distributed', help="This parameter is for internal use only. It signals to a worker node that it is in distributed mode and shouldn't perform certain actions.")
    parser.add_option('--distributed-packages', type='string', dest='distributed_packages', help='Additional packages to be passed to easy_install on remote machines when being run in distributed mode.')
    parser.add_option('--distributed-log-path', type='string', dest='distributed_log_path', help='Path where all the logs will be stored when running a distributed test')
    parser.add_option('--distributed-key-filename', type='string', dest='distributed_key_filename', help='Path of the SSH key to use when running a distributed test')
    parser.add_option('--feedback-endpoint', type='string', dest='feedback_endpoint', help='ZMQ push/pull socket used between the master and the node to send feedback.')
    parser.add_option('--feedback-pubsub-endpoint', type='string', dest='feedback_pubsub_endpoint', help='ZMQ pub/sub socket use to publish feedback.')
    parser.add_option('--feedback', action='store_true', dest='feedback', help='Activates the realtime feedback')
    parser.add_option('--failfast', action='store_true', dest='failfast', help='Stop on first fail or error. (For discover mode)')
    parser.add_option('-u', '--url', type='string', dest='main_url', help='Base URL to bench.')
    parser.add_option('-c', '--cycles', type='string', dest='bench_cycles', help='Cycles to bench, colon-separated list of virtual concurrent users. To run a bench with 3 cycles of 5, 10 and 20 users, use: -c 5:10:20')
    parser.add_option('-D', '--duration', type='string', dest='bench_duration', help='Duration of a cycle in seconds.')
    parser.add_option('-m', '--sleep-time-min', type='string', dest='bench_sleep_time_min', help='Minimum sleep time between requests.')
    parser.add_option('-M', '--sleep-time-max', type='string', dest='bench_sleep_time_max', help='Maximum sleep time between requests.')
    parser.add_option('-t', '--test-sleep-time', type='string', dest='bench_sleep_time', help='Sleep time between tests.')
    parser.add_option('-s', '--startup-delay', type='string', dest='bench_startup_delay', help='Startup delay between thread.')
    parser.add_option('-f', '--as-fast-as-possible', action='store_true', help='Remove sleep times between requests and between tests, shortcut for -m0 -M0 -t0')
    return parser
...
def run_distributed(options, module_name, class_name, method_name, sys_args):
    ...
    try:
        distmgr = DistributionMgr(module_name, class_name, method_name, options, sys_args)
        _manager = distmgr
