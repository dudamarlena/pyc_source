...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/TestRunner.py
...
import os, sys, types, time, unittest, re
...
try:
    ...
    def DTC_runTest(self):
        ...
        runner = DocTestRunner(optionflags=optionflags, checker=self._dt_checker, verbose=g_doctest_verbose)
        try:
            runner.DIVIDER = '-' * 70
...
class TestLoader(unittest.TestLoader):
    ...
    def loadTestsFromTestCase(self, testCaseClass):
        ...
        if not issubclass(testCaseClass, FunkLoadTestCase):
            ...
            return self.suiteClass([ testCaseClass(name, options) for name in self.getTestCaseNames(testCaseClass)
                                   ])
    ...
    def loadTestsFromName(self, name, module=None):
        ...
        if type(obj) == types.ModuleType:
            ...
            if type(obj) == types.UnboundMethodType:
                ...
                if callable(obj):
                    test = obj()
                    if not isinstance(test, unittest.TestCase) and not isinstance(test, unittest.TestSuite):
...
class TestProgram(unittest.TestProgram):
    ...
    def __init__(self, module=None, defaultTest=None, argv=None, testRunner=None, testLoader=unittest.defaultTestLoader):
        if argv is None:
            argv = sys.argv
        self.module = module
        self.testNames = None
    ...
    def loadTests(self):
        ...
        if self.checkAsDocFile:
            self.test = DocFileSuite(os.path.abspath(self.module), module_relative=False)
    ...
    def parseArgs(self, argv):
        ...
        parser.add_option('-q', '--quiet', action='store_true', help='Minimal output.')
        parser.add_option('-v', '--verbose', action='store_true', help='Verbose output.')
        parser.add_option('-d', '--debug', action='store_true', help='FunkLoad and doctest debug output.')
        parser.add_option('--debug-level', type='int', help='Debug level 3 is more verbose.')
        parser.add_option('-u', '--url', type='string', dest='main_url', help="Base URL to bench without ending '/'.")
        parser.add_option('-m', '--sleep-time-min', type='string', dest='ftest_sleep_time_min', help='Minumum sleep time between request.')
        parser.add_option('-M', '--sleep-time-max', type='string', dest='ftest_sleep_time_max', help='Maximum sleep time between request.')
        parser.add_option('--dump-directory', type='string', dest='dump_dir', help='Directory to dump html pages.')
        parser.add_option('-V', '--firefox-view', action='store_true', help='Real time view using firefox, you must have a running instance of firefox in the same host.')
        parser.add_option('--no-color', action='store_true', help='Monochrome output.')
        parser.add_option('-l', '--loop-on-pages', type='string', dest='loop_steps', help='Loop as fast as possible without concurrency on pages, expect a page number or a slice like 3:5. Output some statistics.')
        parser.add_option('-n', '--loop-number', type='int', dest='loop_number', default=10, help='Number of loop.')
        parser.add_option('--accept-invalid-links', action='store_true', help='Do not fail if css/image links are not reachable.')
        parser.add_option('--simple-fetch', action='store_true', dest='ftest_simple_fetch', help="Don't load additional links like css or images when fetching an html page.")
        parser.add_option('--stop-on-fail', action='store_true', help='Stop tests on first failure or error.')
        parser.add_option('-e', '--regex', type='string', default=None, help='The test names must match the regex.')
        parser.add_option('--list', action='store_true', help='Just list the test names.')
        parser.add_option('--doctest', action='store_true', default=False, help='Check for a doc test.')
        parser.add_option('--pause', action='store_true', help='Pause between request, press ENTER to continue.')
        parser.add_option('--profile', action='store_true', help='Run test under the Python profiler.')
    ...
    def runTests(self):
        ...
        if self.testRunner is None:
            if self.color:
                self.testRunner = unittest.TextTestRunner(stream=ColoredStream(sys.stderr), resultclass=_ColoredTextTestResult, verbosity=self.verbosity)
            else:
