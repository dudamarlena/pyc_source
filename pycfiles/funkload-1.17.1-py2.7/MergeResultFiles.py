...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/MergeResultFiles.py
...
class MergeResultFiles:
    ...
    def __init__(self, input_files, output_file):
        ...
        i = 0
        for input_file in xml_parser.files:
            dic = {xml_parser.cycles: str(cycles), 'host="localhost"': 'host="%s"' % node_names[i], 'thread="': 'thread="' + str(i)}
            if i == 0:
                dic['key="node" value="%s"' % node_names[0]] = 'key="node" value="%s"' % (', ').join(node_names)
            c = 0
