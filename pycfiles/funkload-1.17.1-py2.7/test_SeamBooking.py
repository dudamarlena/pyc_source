...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/demo/seam-booking-1.1.5/test_SeamBooking.py
...
class SeamBooking(FunkLoadTestCase):
    ...
    def test_seam_booking(self):
        ...
        self.get(server_url + '/seam-booking/home.seam', description='Booking home page')
        ...
        self.assert_('register_SUBMIT' in self.getBody(), 'Failing to view Registration page.')
        ...
        self.post(server_url + '/seam-booking/register.seam', self.jsfParams(params=[
         ...
         [
          'jsf_viewid', '/register.xhtml']]), description='Submit register form')
        self.assert_('Successfully registered as' in self.getBody(), 'Failing register new user.')
        ...
        self.post(server_url + '/seam-booking/home.seam', params, description='Submit account login')
        self.assert_(username in self.getBody(), 'Failing login new user %s:%s' % (username, password) + str(params))
        self.assert_('No Bookings Found' in self.getBody(), 'Weird there should be no booking for new user %s:%s' % (
         username, password))
        ...
        for i in range(1, nb_letters + 1):
            self.post(server_url + '/seam-booking/main.seam', self.jsfParams(params=[
             ...
             [
              'main:findHotels', 'main:findHotels']]), description='Ajax search %i letter' % i)
            self.assert_('View Hotel' in self.getBody(), 'No match for search hotel.')

        hotel_links = self.listHref(content_pattern='View Hotel')
        self.get(server_url + random.choice(hotel_links), description='View a random hotel in the result list')
        ...
        self.post(server_url + '/seam-booking/hotel.seam', self.jsfParams(params=[
         ...
         [
          'hotel:_link_hidden_', ''],
         [
          'jsf_viewid', '/hotel.xhtml']]), description='Book hotel')
        ...
        self.post(server_url + '/seam-booking/book.seam', self.jsfParams(params=[
         ...
         [
          'jsf_viewid', '/book.xhtml']]), description='Proceed booking')
        ...
        self.post(server_url + '/seam-booking/confirm.seam', self.jsfParams(params=[
         ...
         [
          'jsf_viewid', '/confirm.xhtml']]), description='Confirm booking')
        self.assert_('No Bookings Found' not in self.getBody(), 'Booking is not taken in account.')
