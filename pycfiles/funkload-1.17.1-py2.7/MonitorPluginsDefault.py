...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/MonitorPluginsDefault.py
...
class MonitorCUs(MonitorPlugin):
    ...
    plots = [
     Plot(plot1, title='Concurent users', ylabel='CUs')]
...
class MonitorMemFree(MonitorPlugin):
    plot1 = {'MEM': ['lines lw 2', 'Memory'], 'SWAP': [
              'lines lw 2', 'Swap']}
    ...
    def parseStats(self, stats):
        if not (hasattr(stats[0], 'memTotal') and hasattr(stats[0], 'memFree') and hasattr(stats[0], 'swapTotal') and hasattr(stats[0], 'swapFree')):
            return None
        else:
...
class MonitorCPU(MonitorPlugin):
    plot1 = {'CPU': ['impulse lw 2', 'CPU 1=100%%'], 'LOAD1': [
               'lines lw 2', 'Load 1min'], 
       'LOAD5': [
               'lines lw 2', 'Load 5min'], 
       'LOAD15': [
                'lines lw 2', 'Load 15min']}
    ...
    def _getCPU(self):
        ...
        for line in lines:
            ...
            if l[0].startswith('cpu'):
                ...
                idl = long(l[4])
                return {'CPUTotalJiffies': cpu, 'IDLTotalJiffies': idl}
    ...
    def parseStats(self, stats):
        if not (hasattr(stats[0], 'loadAvg1min') and hasattr(stats[0], 'loadAvg5min') and hasattr(stats[0], 'loadAvg15min')):
            return
        else:
            cpu_usage = [
             0]
            for i in range(1, len(stats)):
                if not (hasattr(stats[i], 'CPUTotalJiffies') and hasattr(stats[(i - 1)], 'CPUTotalJiffies')):
                    cpu_usage.append(None)
                else:
                    dt = long(stats[i].IDLTotalJiffies) + long(stats[i].CPUTotalJiffies) - (long(stats[(i - 1)].IDLTotalJiffies) + long(stats[(i - 1)].CPUTotalJiffies))
                    if dt:
                        ttl = float(long(stats[i].CPUTotalJiffies) - long(stats[(i - 1)].CPUTotalJiffies)) / dt
                    else:
                        ttl = None
            ...
            return {'LOAD1': load_avg_1, 'LOAD5': load_avg_5, 'LOAD15': load_avg_15, 
               'CPU': cpu_usage}


class MonitorNetwork(MonitorPlugin):
    interface = 'eth0'
    plot1 = {'NETIN': ['lines lw 2', 'In'], 'NETOUT': [
                'lines lw 2', 'Out']}
    ...
    def parseStats(self, stats):
        if not (hasattr(stats[0], 'transmitBytes') or hasattr(stats[0], 'receiveBytes')):
            return
        else:
            net_in = [
             None]
            net_out = [None]
            for i in range(1, len(stats)):
                if not (hasattr(stats[i], 'receiveBytes') and hasattr(stats[(i - 1)], 'receiveBytes')):
                    net_in.append(None)
                else:
                    net_in.append((int(stats[i].receiveBytes) - int(stats[(i - 1)].receiveBytes)) / (1024 * (float(stats[i].time) - float(stats[(i - 1)].time))))
                if not (hasattr(stats[i], 'transmitBytes') and hasattr(stats[(i - 1)], 'transmitBytes')):
                    net_out.append(None)
                else:
                    net_out.append((int(stats[i].transmitBytes) - int(stats[(i - 1)].transmitBytes)) / (1024 * (float(stats[i].time) - float(stats[(i - 1)].time))))
