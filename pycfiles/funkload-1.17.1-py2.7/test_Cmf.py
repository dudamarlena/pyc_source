...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/demo/cmf/test_Cmf.py
...
class CmfTestCase(FunkLoadTestCase):
    ...
    def cmfLogin(self, login, pwd):
        params = [
         ...
         [
          'submit', ' Login ']]
        self.post('%s/logged_in' % self.server_url, params, description='Log xin user %s' % login)
        self.assert_('Login success' in self.getBody(), 'Invalid credential %s:%s' % (login, pwd))
        self._cmf_login = login

    def cmfLogout(self):
        self.get('%s/logout' % self.server_url, description='logout %s' % self._cmf_login)
    ...
    def cmfCreateNews(self, parent_url):
        ...
        params = [['type_name', 'News Item'],
         ...
         [
          'add', 'Add']]
        self.post('%s/folder_factories' % parent_url, params, description='Create an empty news')
        ...
        self.post('%s/metadata_edit_form' % doc_url, params, description='Set metadata')
        ...
        params = [
         ...
         [
          'change_and_view', 'Change and View']]
        self.post('%s/newsitem_edit_form' % doc_url, params, description='Set news content')
...
class Cmf(CmfTestCase):
    ...
    def setUp(self):
        ...
        self.cred_admin = xmlrpc_get_credential(credential_host, credential_port, 'AdminZope')
        self.cred_member = xmlrpc_get_credential(credential_host, credential_port, 'FL_Member')
    ...
    def test_00_verifyCmfSite(self):
        ...
        params = [['id', site_id],
         ...
         [
          'description',
          'See http://svn.nuxeo.org/pub/funkload/trunk/README.txt for more info about FunkLoad'],
         [
          'submit', ' Add ']]
        self.post('%s/manage_addProduct/CMFDefault/manage_addCMFSite' % zope_url, params, description='Create a CMF Site')
    ...
    def test_05_verifyUsers(self):
        ...
        lipsum = Lipsum()
        self.setBasicAuth(*self.cred_admin)
        for user_id, user_pwd in xmlrpc_list_credentials(self.credential_host, self.credential_port, 'FL_Member'):
            params = [
             [
              'member_id', user_id],
             [
            ...
            html = self.getBody()
            self.assert_('Member registered' in html or 'The login name you selected is already in use' in html, 'Member %s not created' % user_id)

        self.clearBasicAuth()

    def test_anonymous_reader(self):
        server_url = self.server_url
        self.get('%s/Members' % server_url, description='Try to see Members area')
        self.get('%s/recent_news' % server_url, description='Recent news')
        self.get('%s/search_form' % server_url, description='View search form')
        self.get('%s/login_form' % server_url, description='View login form')
        self.get('%s/join_form' % server_url, description='View join form')
    ...
    def test_member_reader(self):
        ...
        self.get('%s/personalize_form' % server_url, description='Preference page')
        self.cmfLogout()
