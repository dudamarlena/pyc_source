...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/ReportStats.py
...
class ApdexStat:
    ...
    def getScore(self):
        return Apdex.score(self.apdex_satisfied, self.apdex_tolerating, self.apdex_frustrated)
...
class AllResponseStat:
    ...
    def add(self, date, result, duration):
        ...
        self.per_second[date_s] = self.per_second.setdefault(int(date_s), 0) + 1
        self.count += 1
...
class PageStat(AllResponseStat):
    ...
    def add(self, thread, step, date, result, duration, rtype):
        """Add a new response to stat."""
        thread = self.threads.setdefault(thread, {'count': 0, 'pages': {}})
        ...
        if not thread['count']:
            return
        stat = thread['pages'].setdefault(thread['count'], SinglePageStat(step))
...
class TestStat:
    ...
    def add(self, result, pages, xmlrpc, redirects, images, links, duration, traceback=None):
        """Add a new response to stat."""
        self.finalized = False
        self.count += 1
