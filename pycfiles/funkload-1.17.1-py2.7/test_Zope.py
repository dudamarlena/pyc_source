...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/funkload/demo/zope/test_Zope.py
...
class Zope(ZopeTestCase):
    ...
    def test_restart(self):
        self.zopeRestart(self.zope_url, self.admin_id, self.admin_pwd, time_out=10)
    ...
    def test_00_verifyExample(self):
        if not self.exists(self.zope_url + '/Examples'):
            ...
            self.get(self.zope_url + '/manage_importObject?file=Examples.zexp&set_owner:int=1')
    ...
    def test_exampleFileLibrary(self):
        ...
        for sort in ('type', 'size', 'date'):
            params = [
             ...
             [
              'reverse:int', '0']]
            self.post('%s/Examples/FileLibrary/index_html' % server_url, params, description='File Library sort by %s' % sort)
