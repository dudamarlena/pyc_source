...
# [GCC 8.4.0]
# Embedded file name: /usr/lib/python2.5/site-packages/graphite/property.py
# Compiled at: 2008-02-01 08:24:57
"""property.py
...
class IntProperty(Property):
    ...
    def acceptable(self, val):
        ...
        try:
            ...
            return (
             False, 'unable to convert to int: %s' % str(val))

        if out < self.minval or out > self.maxval:
            return (
             False,
             'value %d out of range (%d to %d)' % (
...
class FloatProperty(Property):
    ...
    def acceptable(self, val):
        ...
        try:
            ...
            return (
             False, 'unable to convert to float: %s' % str(val))

        if out < self.minval or out > self.maxval:
            return (
             False,
             'value %g out of range (%g to %g)' % (
              out, self.minval, self.maxval))
        return (True, out)
    ...
    def acceptable(self, val):
        ...
        if val in self.vallist:
            return (
             True, val)
        return (
         False, '%s not in acceptable values %s' % (val, self.vallist))
...
class ClassProperty(Property):
    ...
    def acceptable(self, val):
        ...
        try:
            ...
            return (
             False, 'unable to convert to %s: %s' % (self.okclass.__name__, val))
...
class ListProperty(Property):
    ...
    def acceptable(self, val):
        ...
        if len(val) < self.minqty:
            return (
             0, 'list too short -- must contain at least %d elements' % self.minqty)
        if len(val) > self.maxqty:
            return (
             0, 'list too long -- must be at most %d elements' % self.maxqty)
    ...
    def exportStringFunc(self, selfname, proplist=[], compProplist=[]):
        ...
        for prop in proplist:
            ...
            if prop in compProplist:
                ...
                for index in range(0, len(proplist)):
                    retval = retval + proplist[index].exportString('%s.%s[%s]' % (selfname, prop, index))
...
class TestClass(PropHolder):
    ...
    _properties = {'ref': Property('', 'put anything you want here'), 
       ...
       'freckles': ListProperty(IntProperty(0, '', minval=0), (0, 0, 0, 0), 'freckles per limb', minqty=4, maxqty=4)}
