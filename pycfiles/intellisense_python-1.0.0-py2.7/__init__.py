...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/intellisense/__init__.py
# Compiled at: 2014-07-11 15:16:32
import version, logging
...
def init(apiKey, **kwargs):
    """Create a default instance of a intellisense-python client

    :param str apiKey: The Intellify API Secret

    Kwargs:

    : param string sensorId: The sensorId provided to you by Intellify Learning
    : param string host: The IntelliStore host value provided to you by Intellify Learning
    : param string port: The IntelliStore port value provided to you by Intellify Learning
    ...
    if hasattr(this_module, 'default_client'):
        return
    default_client = Client(secret=apiKey, stats=stats, **kwargs)
...
def _get_default_client():
    ...
    if hasattr(this_module, 'default_client'):
        ...
        sys.stderr.write('Please call intellisense.init(apiKey) ' + 'before calling intellisense methods.\n')
...
def describe(entity_type=None, entity_id=None, properties={}, timestamp=None):
    ...
    """
    log('debug', 'intellisense describe ' + ' entity_type = ' + entity_type + ' entity_id = ' + entity_id)
...
def measure(action=None, learning_context={}, activity_context={}, timestamp=None):
    """Measure an interaction between entities in the learning graph
    ...
    If you do choose to provide a timestamp, make sure it has a timezone.
    """
    log('debug', 'intellisense measure ')
    default_client = _get_default_client()
    if default_client:
...
def identify(user_id=None, traits={}, context={}, timestamp=None):
    ...
    default_client = _get_default_client()
    if default_client:
        default_client.identify(user_id=user_id, traits=traits, context=context, timestamp=timestamp)


def track(user_id=None, event=None, properties={}, context={}, timestamp=None):
    ...
    :param timestamp: Time in millis since epoch. If this event happened in the past,
    ...
    default_client = _get_default_client()
    if default_client:
        default_client.track(user_id=user_id, event=event, properties=properties, context=context, timestamp=timestamp)
...
def alias(from_id, to_id, context={}, timestamp=None):
    ...
    default_client = _get_default_client()
    if default_client:
        default_client.alias(from_id=from_id, to_id=to_id, context=context, timestamp=timestamp)
