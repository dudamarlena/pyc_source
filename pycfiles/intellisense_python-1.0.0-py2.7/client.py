...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/intellisense/client.py
# Compiled at: 2014-07-11 15:16:32
import collections, time
from datetime import datetime, timedelta
import json, logging, numbers, threading
    ...
    try:
        ...
        if len(describes) > 0:
            for describe in describes:
                response_d = requests.put(describe_url, data=json.dumps(describe, cls=DatetimeSerializer), headers={'content-type': 'application/json'}, timeout=client.timeout)
        ...
        if len(measures) > 0:
            for measure in measures:
                response_m = requests.put(measure_url, data=json.dumps(measure, cls=DatetimeSerializer), headers={'content-type': 'application/json'}, timeout=client.timeout)
        ...
        if len(defaults) > 0:
            response = requests.put(url, data=json.dumps(defaults, cls=DatetimeSerializer), headers={'content-type': 'application/json'}, timeout=client.timeout)
...
class Client(object):
    ...
    def __init__(self, secret=None, sensorId='_ERROR_NO_SENSORID_', host='', port='', log_level=logging.INFO, log=True, flush_at=20, flush_after=timedelta(0, 10), async=True, max_queue_size=10000, stats=Statistics(), timeout=10, send=True):
    ...
    def _check_for_secret(self):
        if not self.secret:
            raise Exception('Please set intellisense.secret before calling ' + 'identify or track.')
    ...
    def _clean_dict(self, d):
        ...
        for k, v in d.iteritems():
            try:
                ...
                log('warn', 'Dictionary values must be serializeable to ' + 'JSON "%s" value %s of type %s is unsupported.' % (
                 k, v, type(v)))
    ...
    def describe(self, entity_type=None, entity_id=None, properties={}, timestamp=None):
        ...
        action = {'entityId': entity_id, 'type': entity_type, 
    ...
    def measure(self, action=None, learning_context={}, activity_context={}, timestamp=None):
        ...
        if timestamp is None:
            timestamp = self.current_time()
        action = {'action': action, 'learningContext': learning_context, 
    ...
    def identify(self, user_id=None, traits={}, context={}, timestamp=None):
        ...
        action = {'userId': user_id, 'traits': cleaned_traits, 
           'context': context, 
           'timestamp': timestamp, 
           '__action': 'identify'}
    ...
    def track(self, user_id=None, event=None, properties={}, context={}, timestamp=None):
        ...
        action = {'userId': user_id, 'event': event, 
           'context': context, 
