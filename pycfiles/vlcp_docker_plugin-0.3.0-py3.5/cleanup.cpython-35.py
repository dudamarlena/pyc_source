...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/vlcp_docker/cleanup.py
# Compiled at: 2018-08-10 07:27:57
# Size of source mod 2**32: 16924 bytes
"""
...
import vlcp.service.sdn.viperflow as viperflow, vlcp.service.kvdb.objectdb as objectdb
...
import json, vlcp.utils.encoders as encoders
...
class Cleanup(ScriptModule):
    ...
    options = (('skipovs', None, False), ('skipiplink', None, False), ('skiplogicalport', None, False),
               ('host', 'H', True))
    ...
    async def run(self, host=None, skipovs=None, skipiplink=None, skiplogicalport=None):
        ...
        ca_path, cert_path, key_path = [os.path.join(cert_root_path, f) for f in ('ca.pem',
                                                                                  'cert.pem',
                                                                                  'key.pem')]
        if '/' not in host:
            if enable_ssl:
            ...
            if self._docker_conn is None or not self._docker_conn.connected:
                ...
                if method is None:
                    if data is None:
                        ...
                        method = b'POST'
                    if data is None:
                        final_resp, _ = await http_protocol.request_with_response(self.apiroutine, self._docker_conn, b'docker', _bytes(path), method, [
                         (b'Accept-Encoding', b'gzip, deflate')])
                    else:
                        final_resp, _ = await http_protocol.request_with_response(self.apiroutine, self._docker_conn, b'docker', _bytes(path), method, [
                         (b'Content-Type', b'application/json;charset=utf-8'),
                         (b'Accept-Encoding', b'gzip, deflate')], MemoryStream(_bytes(json.dumps(data))))
        ...
        async def detect_unused_logports():
            ...
            result = await call_docker_api(b'/v1.24/networks?filters={"driver":["vlcp"]}')
            network_ports = dict((n['Id'],
             dict((p['EndpointID'], p['IPv4Address']) for p in n['Containers'].values())) for n in result if n['Driver'] == 'vlcp')
            ...
            async def recheck_ports():
                await self.apiroutine.wait_with_timeout(5)
                result = await call_docker_api(b'/v1.24/networks?filters={"driver":["vlcp"]}')
                second_network_ports = dict((n['Id'],
                 dict((p['EndpointID'], p['IPv4Address']) for p in n['Containers'].values())) for n in result if n['Id'] in network_ports and n['Driver'] == 'vlcp')
            ...
            async def check_viperflow():
                first_vp_ports = {}
                for nid in network_ports:
                    result = await call_api(self.apiroutine, 'viperflow', 'listlogicalports', {'logicalnetwork': 'docker-' + nid + '-lognet'})
                    first_vp_ports[nid] = dict((p['id'], p.get('ip_address')) for p in result if p['id'].startswith('docker-'))
                ...
                for nid in network_ports:
                    result = await call_api(self.apiroutine, 'viperflow', 'listlogicalports', {'logicalnetwork': 'docker-' + nid + '-lognet'})
                    second_vp_ports[nid] = dict((p['id'], p.get('ip_address')) for p in result if p['id'] in first_vp_ports[nid])
                ...
                second_vp_ports = dict((nid, dict((pid[len('docker-'):], addr) for pid, addr in v.items())) for nid, v in second_vp_ports.items())
                return second_vp_ports

            second_vp_ports = await check_viperflow()
            second_ports = await recheck_ports()
            unused_logports = dict((nid,
             dict((pid, addr) for pid, addr in v.items() if pid not in network_ports[nid] and pid not in second_ports[nid])) for nid, v in second_vp_ports.items())
        ...
        if any(ports for ports in unused_logports.values()):
            print('Find %d unused logical ports, first 20 ips:\n%r' % (
             ...
             [v for _, v in itertools.takewhile(lambda x: x[0] <= 20, enumerate(addr for ports in unused_logports.values() for addr in ports.values()))]))
            print('Will remove them in 5 seconds, press Ctrl+C to cancel...')
            await self.apiroutine.wait_with_timeout(5)
            for ports in unused_logports.values():
                for p, addr in ports.items():
                    try:
                        await call_api(self.apiroutine, 'viperflow', 'deletelogicalport', {'id': 'docker-' + p})
                    except Exception as exc:
