...
# [GCC 8.4.0]
# Embedded file name: D:\workspace\mine\python\haf-webmanager\hafweb\model\model_api.py
# Compiled at: 2019-04-09 02:36:50
...
class ApiCase(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'ids_id', 'run', 'dependent', 'bench_name', 'request_id', 'response_id', 'expect_id', 'sqlinfo_id', 'type', 'suite_id', 'detail_id']
        rev = {}
...
class ApiCaseExpect(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'response_id', 'sql_check_func', 'sql_response_result']
        rev = {}
        for attr in attr_list:
            rev[attr] = getattr(self, attr)
...
class ApiCaseIds(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'case_id', 'case_sub_id', 'case_name', 'case_api_name']
        rev = {}
        for attr in attr_list:
            rev[attr] = getattr(self, attr)
...
class ApiCaseRequest(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'header', 'data', 'url', 'method', 'protocol', 'host_port']
        rev = {}
        for attr in attr_list:
            rev[attr] = getattr(self, attr)
...
class ApiCaseSqlinfo(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'scripts_id', 'config', 'config_id', 'check_list_id']
        rev = {}
        for attr in attr_list:
            rev[attr] = getattr(self, attr)
...
class ApiDetail(Base):
    ...
    def __repr__(self):
        attr_list = [
         'id', 'case_name', 'result_check_response', 'case_name', 'result_check_sql_response', 'run_error', 'result', 'begin_time', 'end_time', 'log_dir', 'runner']
        rev = {}
        for attr in attr_list:
            rev[attr] = getattr(self, attr)
