...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/rankboost.py
# Compiled at: 2018-05-07 13:54:25
from protect.common import docker_call, get_files_from_filestore, export_results
import os

def wrap_rankboost(job, rsem_files, merged_mhc_calls, transgene_out, univ_options, rankboost_options):
    """
...
def boost_ranks(job, isoform_expression, merged_mhc_calls, transgene_out, univ_options, rankboost_options):
    """
    ...
    """
    work_dir = os.getcwd()
    input_files = {'rsem_quant.tsv': isoform_expression, 
    ...
    for mhc in ('mhci', 'mhcii'):
        ...
        parameters = ['--' + mhc,
         ...
         '--ratios', ratios]
        docker_call(tool='rankboost', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=rankboost_options['version'])
        ...
        if os.path.exists(mhc_concise):
            ...
            export_results(job, output_files[os.path.basename(mhc_concise)], mhc_concise, univ_options, subfolder='rankboost')
        else:
            output_files[os.path.basename(mhc_concise)] = None
        if os.path.exists(mhc_detailed):
            output_files[os.path.basename(mhc_detailed)] = job.fileStore.writeGlobalFile(mhc_detailed)
            export_results(job, output_files[os.path.basename(mhc_detailed)], mhc_detailed, univ_options, subfolder='rankboost')
        else:
