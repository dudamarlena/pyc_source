...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/mutation_translation.py
# Compiled at: 2018-05-07 13:54:25
from __future__ import absolute_import, print_function
from collections import defaultdict
from math import ceil
from protect.common import docker_call, get_files_from_filestore, export_results, untargz, docker_path
import os

def transgene_disk(rna_bamfiles, tdna_bam=None):
    return int((ceil(rna_bamfiles['rna_genome']['rna_genome_sorted.bam'].size) if rna_bamfiles else 0) + (ceil(tdna_bam['tumor_dna_fix_pg_sorted.bam'].size) if tdna_bam is not None else 0) + 104857600)
...
def run_transgene(job, snpeffed_file, rna_bam, univ_options, transgene_options, tumor_dna_bam=None, fusion_calls=None):
    ...
    work_dir = os.getcwd()
    input_files = {'pepts.fa.tar.gz': transgene_options['gencode_peptide_fasta'], 
    ...
    if rna_bam:
        input_files.update({'rna.bam': rna_bam['rna_genome']['rna_genome_sorted.bam'], 
           'rna.bam.bai': rna_bam['rna_genome']['rna_genome_sorted.bam.bai']})
    if tumor_dna_bam is not None:
        input_files.update({'tumor_dna.bam': tumor_dna_bam['tumor_dna_fix_pg_sorted.bam'], 
    ...
    if fusion_calls:
        fusion_files = {'fusion_calls': fusion_calls, 'transcripts.fa.tar.gz': transgene_options['gencode_transcript_fasta']}
    ...
    docker_call(tool='transgene', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=transgene_options['version'])
        ...
        for tissue_type in ['tumor', 'normal']:
            ...
            if os.path.exists(os.path.join(work_dir, old_pepfile)):
                ...
                if tissue_type == 'tumor':
                    os.rename(os.path.join(work_dir, old_pepfile + '.map'), os.path.join(work_dir, pepfile + '.map'))
    ...
    if snpeffed_file:
        ...
        export_results(job, job.fileStore.writeGlobalFile('mutations.vcf'), 'mutations.vcf', univ_options, subfolder='mutations/transgened')
