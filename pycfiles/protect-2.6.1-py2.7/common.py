...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/mutation_calling/common.py
...
def run_mutation_aggregator(job, mutation_results, univ_options):
    ...
    for chrom in mutation_results['mutect'].keys():
        out[chrom] = job.addChildJobFn(merge_perchrom_mutations, chrom, mutation_results, univ_options).rv()
...
def merge_perchrom_mutations(job, chrom, mutations, univ_options):
    ...
    vcf_processor = {'snvs': {'mutect': process_mutect_vcf, 'muse': process_muse_vcf, 
    ...
    for mut_type in vcf_processor.keys():
        perchrom_mutations = {caller:vcf_processor[mut_type][caller](job, mutations[caller][chrom], work_dir, univ_options) for caller in vcf_processor[mut_type]}
    ...
    with open(('').join([work_dir, '/', chrom, '.vcf']), 'w') as (outfile):
        ...
        print('##INFO=<ID=callers,Number=.,Type=String,Description=List of supporting callers.', file=outfile)
        ...
        for chrom in chrom_sorted(accepted_hits.keys()):
            for position in sorted(accepted_hits[chrom]):
                print(chrom, position, '.', accepted_hits[chrom][position][0], accepted_hits[chrom][position][1], '.', 'PASS', 'callers=' + accepted_hits[chrom][position][2], sep='\t', file=outfile)
...
def unmerge(job, input_vcf, tool_name, chromosomes, tool_options, univ_options):
    ...
    """
    work_dir = os.getcwd()
    input_files = {'input.vcf': input_vcf, 
    ...
    for chrom, chromvcf in read_chromosomes.items():
        ...
        export_results(job, outdict[chrom], chromvcf.name, univ_options, subfolder='mutations/' + tool_name)
