...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/mutation_calling/muse.py
...
def run_muse_with_merge(job, tumor_bam, normal_bam, univ_options, muse_options):
    ...
    """
    spawn = job.wrapJobFn(run_muse, tumor_bam, normal_bam, univ_options, muse_options, disk='100M').encapsulate()
...
def run_muse(job, tumor_bam, normal_bam, univ_options, muse_options):
    ...
    for chrom in chromosomes:
        call = job.addChildJobFn(run_muse_perchrom, tumor_bam, normal_bam, univ_options, muse_options, chrom, disk=PromisedRequirement(muse_disk, tumor_bam['tumor_dna_fix_pg_sorted.bam'], normal_bam['normal_dna_fix_pg_sorted.bam'], muse_options['genome_fasta']), memory='6G')
        sump = call.addChildJobFn(run_muse_sump_perchrom, call.rv(), univ_options, muse_options, chrom, disk=PromisedRequirement(muse_sump_disk, muse_options['dbsnp_vcf']), memory='6G')
...
def run_muse_perchrom(job, tumor_bam, normal_bam, univ_options, muse_options, chrom):
    ...
    docker_call(tool='muse', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=muse_options['version'])
...
def run_muse_sump_perchrom(job, muse_output, univ_options, muse_options, chrom):
    ...
    """
    work_dir = os.getcwd()
    input_files = {'MuSE.txt': muse_output, 
...
def process_muse_vcf(job, muse_vcf, work_dir, univ_options):
    ...
    with open(muse_vcf, 'r') as (infile):
        with open(muse_vcf + 'muse_parsed.tmp', 'w') as (outfile):
            for line in infile:
                line = line.strip()
                if line.startswith('#'):
