...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/mutation_calling/fusion.py
# Compiled at: 2018-05-07 13:54:25
from __future__ import print_function
from math import ceil
from bd2k.util.expando import Expando
from toil.job import PromisedRequirement
import collections, csv, logging, os, re, string
...
def fusion_disk(rna_fastqs, star_tar):
    return int(4 * ceil(sum([ f.size for f in rna_fastqs ]) + 524288) + 2 * ceil(star_tar.size + 524288) + 5242880)


def wrap_fusion(job, fastqs, star_output, univ_options, star_fusion_options, fusion_inspector_options):
    """
    ...
    if not star_fusion_options['run']:
        ...
        return
    fusion = job.wrapJobFn(run_fusion, fastqs, star_output['rnaChimeric.out.junction'], univ_options, star_fusion_options, fusion_inspector_options, cores=star_fusion_options['n'], memory=PromisedRequirement(lambda x: int(1.85 * x.size), star_fusion_options['index']), disk=PromisedRequirement(fusion_disk, fastqs, star_fusion_options['index'])).encapsulate()
    job.addChild(fusion)
    return fusion.rv()


def run_fusion(job, fastqs, junction_file, univ_options, star_fusion_options, fusion_inspector_options):
    """
    ...
    input_files = {'rna_1.fq.gz': fastqs[0], 'rna_2.fq.gz': fastqs[1], 
    ...
    parameters.extend(['--output_dir', '/data/fusion-output',
     ...
     '--CPU', str(cores)])
    docker_call(tool='star-fusion', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=star_fusion_options['version'])
    star_output = 'fusion-output/star-fusion.fusion_candidates.final.abridged'
    fusion_path = os.path.join(work_dir, star_output)
    export_results(job, job.fileStore.writeGlobalFile(fusion_path), 'star-fusion-predictions.tsv', univ_options, subfolder='mutations/fusions')
    ...
    docker_call(tool='fusion-inspector', tool_parameters=parameters, work_dir=work_dir, dockerhub=univ_options['dockerhub'], tool_version=fusion_inspector_options['version'])
    found_fusion = False
    ...
    export_results(job, job.fileStore.writeGlobalFile(fusion_path), 'fusion-inspector-predictions.tsv', univ_options, subfolder='mutations/fusions')
    ...
    if found_fusion:
        ...
        return job.addChildJobFn(reformat_star_fusion_output, fusion_annotation, filtered_fusions, transcripts, transcript_annotation, univ_options).rv()
    else:
...
def get_transcripts(transcript_file):
    ...
    with open(transcript_file, 'r') as (fa):
        transcripts = {}
        regex_s = '(?P<ID>TRINITY.*)\\s(?P<fusion>.*--.*):(?P<left_start>\\d+)-(?P<right_start>\\d+)'
...
def split_fusion_transcript(annotation_path, transcripts):
    ...
    regex = re.compile('ID=(?P<ID>.*);Name=(?P<Name>.*);Target=(?P<Target>.*)\\s(?P<start>\\d+)\\s(?P<stop>\\d+)')
        ...
        for line in gff:
            ...
            if line.startswith('#'):
                ...
                if m:
                    ...
                    if strand == '-' and rb:
                        transcript_split = int(m.group('stop')) + 1
    ...
    return (
     five_pr_splits, three_pr_splits)
...
def get_gene_ids(fusion_bed):
    ...
    with open(fusion_bed, 'r') as (f):
        gene_to_id = {}
        regex = re.compile('(?P<gene>ENSG\\d*)')
        for line in f:
            line = line.split('\t')
...
def reformat_star_fusion_output(job, fusion_annot, fusion_file, transcript_file, transcript_gff_file, univ_options):
    ...
    :rtype: toil.fileStore.FileID
    """
    input_files = {'results.tsv': fusion_file, 'fusion.bed': fusion_annot}
    ...
    with open(input_files['results.tsv'], 'r') as (in_f):
