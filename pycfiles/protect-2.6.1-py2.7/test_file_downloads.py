...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/protect/test/unit/test_file_downloads.py
...
class TestFileDownloads(ProtectTest):
    ...
    def _download_files(job):
        ...
        get_file_from_s3(job, http_base + encrypted_with_key, encryption_key=keyfile, per_file_encryption=False, write_to_jobstore=False)
        get_file_from_s3(job, http_base + encrypted_with_hash, encryption_key=keyfile, write_to_jobstore=False)
        get_file_from_s3(job, s3_base + unencrypted, write_to_jobstore=False)
        try:
            get_file_from_s3(job, 's' + s3_base + encrypted_with_hash, encryption_key=keyfile, write_to_jobstore=False)
        ...
        try:
            get_file_from_s3(job, s3_base + encrypted_with_hash, encryption_key=keyfile, per_file_encryption=False, write_to_jobstore=False)
        except RuntimeError as err:
            if '403' not in err.message:
                raise

        try:
            get_file_from_s3(job, s3_base + unencrypted, encryption_key=keyfile, per_file_encryption=False, write_to_jobstore=False)
        except RuntimeError as err:
            if '400' not in err.message:
                raise
    ...
    def _download_https_S3_files(job):
        ...
        Attempts to download an unencrypted file, a file encrypted with a key.
        """
        http_base = 'https://s3-us-west-2.amazonaws.com/cgl-pipeline-inputs/protect/ci_references/rsem_index_chr6.tar.gz'
        get_file_from_url(job, http_base, write_to_jobstore=False)
