...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/toil_http_parser/http.py
...
class NoMoreData(Exception):
    __doc__ = " exception raised when trying to parse headers but\n    we didn't get all data needed.\n    "


class ParserError(Exception):
    __doc__ = ' error while parsing http request '


class BadStatusLine(Exception):
    __doc__ = ' error when status line is invalid '


class HttpStream(object):
    __doc__ = ' An HTTP parser providing higher-level access to a readable,\n    sequential io.RawIOBase object. You can use implementions of\n    toil_http_parser.reader (IterReader, StringReader, SocketReader) or\n    create your own.\n    '

    def __init__(self, stream, kind=HTTP_BOTH, decompress=False, parser_class=HttpParser):
    ...
    def body_file(self, buffering=None, binary=True, encoding=None, errors=None, newline=None):
    ...
    def body_string(self, binary=True, encoding=None, errors=None, newline=None):
        """ return body as string """
        return self.body_file(binary=binary, encoding=encoding, newline=newline).read()
