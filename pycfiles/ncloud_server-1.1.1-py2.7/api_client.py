...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/ncloud_server/api_client.py
...
import os, re, tempfile, hashlib, hmac, base64, time, sys, six
from six.moves.urllib.parse import quote
from ncloud_server.configuration import Configuration
import ncloud_server.model
from ncloud_server import rest
...
class ApiClient(object):
    ...
    def __call_api(self, resource_path, method, path_params=None, query_params=None, header_params=None, body=None, post_params=None, files=None, response_type=None, auth_settings=None, _return_http_data_only=None, collection_formats=None, _preload_content=True, _request_timeout=None):
        ...
        self.update_params_for_auth(method, resource_path, header_params, query_params, auth_settings)
        if body:
            ...
            for key in body:
                ...
                if type(value) is list:
                    ...
                    serialized_body = serialized_body + quote(key) + '=' + quote(str(value)) + '&'
    ...
    def __deserialize(self, data, klass):
        ...
        if type(klass) == str:
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(ncloud_server.model, klass)
    ...
    def call_api(self, resource_path, method, path_params=None, query_params=None, header_params=None, body=None, post_params=None, files=None, response_type=None, auth_settings=None, _async=None, _return_http_data_only=None, collection_formats=None, _preload_content=True, _request_timeout=None):
    ...
    def update_params_for_auth(self, method, resource_path, headers, querys, auth_settings):
        ...
        for auth in auth_settings:
            ...
            if auth_setting:
                if not auth_setting['value']:
                    ...
                    headers['x-ncp-iam-access-key'] = self.configuration.access_key
                    headers['x-ncp-apigw-signature-v1'] = self.make_signature(method, resource_path, querys, timestamp)
                elif auth_setting['in'] == 'query':
                    querys.append((auth_setting['key'], auth_setting['value']))
                else:
                    raise ValueError('Authentication token must be in `query` or `header`')

    def make_signature(self, method, uri, querys, timestamp):
        uri = '/' + self.configuration.host.split('/', 3)[3] + uri
        if querys:
            uri = uri + '?'
