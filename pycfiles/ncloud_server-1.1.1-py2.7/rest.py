...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/ncloud_server/rest.py
...
class RESTClientObject(object):
    ...
    def __init__(self, configuration, pools_size=4, maxsize=None):
        ...
        if configuration.proxy:
            self.pool_manager = urllib3.ProxyManager(num_pools=pools_size, maxsize=maxsize, cert_reqs=cert_reqs, ca_certs=ca_certs, cert_file=configuration.cert_file, key_file=configuration.key_file, proxy_url=configuration.proxy, **addition_pool_args)
        else:
            self.pool_manager = urllib3.PoolManager(num_pools=pools_size, maxsize=maxsize, cert_reqs=cert_reqs, ca_certs=ca_certs, cert_file=configuration.cert_file, key_file=configuration.key_file, **addition_pool_args)
        return
    ...
    def request(self, method, url, query_params=None, headers=None, body=None, post_params=None, _preload_content=True, _request_timeout=None):
        ...
        if not method in ('GET', 'HEAD', 'DELETE', 'POST', 'PUT', 'PATCH', 'OPTIONS'):
            ...
            try:
                if method in ('POST', 'PUT', 'PATCH', 'OPTIONS', 'DELETE'):
                    ...
                    if re.search('json', headers['Content-Type'], re.IGNORECASE):
                        ...
                        r = self.pool_manager.request(method, url, body=body, preload_content=_preload_content, timeout=timeout, headers=headers)
            ...
            raise (200 <= r.status <= 299 or ApiException)(http_resp=r)
        return r
