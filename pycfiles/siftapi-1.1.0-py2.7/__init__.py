...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.13-x86_64/egg/siftapi/__init__.py
# Compiled at: 2019-03-28 17:03:29
...
class Sift(object):
    ...
    def _request(self, method, url, params=None, data=None):
        ...
        args.update(params)
        args.update(data)
        params['signature'] = self._generate_signature(args, method, '/%s%s' % (VERSION, url)).hexdigest()
    ...
    def _generate_signature(self, params, method, path):
        ...
        signature = hmac.new(self.api_secret.encode('utf-8'), base_string.encode('utf-8'), hashlib.sha1)
        return signature
    ...
    def get_email_connections(self, user, limit=100, offset=0, include_invalid=False):
        ...
        return self._request('GET', '/users/%s/email_connections' % user, params=params)

    def add_email_connection(self, user, data):
        u"""Link a new email connection to the account
        ...
        return self._request('POST', '/users/%s/email_connections' % user, data=data)
    ...
    def delete_email_connection(self, user, connection_id):
        ...
        """
        return self._request('DELETE', '/users/%s/email_connections/%s' % (user, connection_id))
    ...
    def get_sift(self, user, sift_id, include_eml=0):
        ...
        return self._request('GET', '/users/%s/sifts/%s' % (user, sift_id), params=params)
    ...
    def get_emails(self, username, limit=None, offset=None, last_update_time=None):
