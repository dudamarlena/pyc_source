...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/pyqart/qr/painter/canvas.py
# Compiled at: 2016-08-02 13:52:38
...
__all__ = [
 'QrCanvas']
_TYPE_CHAR_MAP = {QrPointType.UNKNOWN: '? ', 
...
class QrCanvas(object):
    ...
    def __init__(self, args):
        ...
        self._points = [[QrPoint(False) for _ in range(self.size)] for _ in range(self.size)]
    ...
    def _add_empty_data_ec(self):
        ...
        data_points = [QrPoint(False, QrPointType.DATA, offset) for offset in range(dbc)]
        ec_points = [QrPoint(False, QrPointType.CORRECTION, dbc + offset) for offset in range(ecbc)]
        data_blocks = []
        ec_blocks = []
        ...
        if di != dbc or eci != ecbc:
            raise QrCanvasException('Error when split data and ec points to blocks.')
        data_ec_points = []
        for cwi in range(self.args.ndcwcpb + 1):
            for blki in range(self.args.bc):
                if cwi * _BIT_PER_CW < len(data_blocks[blki]):
                    bi = cwi * _BIT_PER_CW
                    data_ec_points.extend(data_blocks[blki][bi:bi + _BIT_PER_CW])
