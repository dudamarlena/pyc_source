...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/imgserve/worker.py
# Compiled at: 2009-08-21 05:08:36
"""
...
import os, shutil, socket, urllib, urllib2, Image, ImageFile, ImageFont, ImageDraw, cairo, rsvg, pycurl, tempfile
from pkg_resources import resource_filename
from imgserve.utils import *
GETSIZES_TIMEOUT = 30
socket.setdefaulttimeout(GETSIZES_TIMEOUT)
UPLOAD_CONNECT_TIMEOUT = 10
UPLOAD_TIMEOUT = 30
...
def retrieve(url):
    ...
    if not success or url.startswith('http') and not retrieved_headers.is_empty() and not http_200_ok(retrieved_headers.contents):
    ...
    for (req_id, req) in iter(i_queue.get, None):
        ...
        if req['operationType'] == 'rasterize':
            ...
            try:
                ...
                ctx.scale(float(width) / svg.props.width, float(height) / svg.props.height)
