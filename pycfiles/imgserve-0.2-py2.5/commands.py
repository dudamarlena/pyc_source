...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-i686/egg/imgserve/commands.py
# Compiled at: 2009-08-08 05:29:17
"""
...
"""
import os, signal, sys, time, imgserve
...
def start_command(pid=PID_FILEPATH, logdir=LOG_DIR, FORCE=False, chroot=False, chdir='.', boot='config.boot', uid=False, gid=False, umask=False, host='0.0.0.0', port=8602):
    """
    Runs an imgserve server out of the current directory:

        imgserve start -host 127.0.0.1 -port 8602
    """
    utils.start_server(pid, logdir, FORCE, chroot, chdir, uid, gid, umask, ImgManager, host, port)
