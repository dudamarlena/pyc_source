...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.12-x86_64/egg/django_payload_validator/base_payload_validator.py
# Compiled at: 2018-02-21 05:56:56
...
class BasePayloadValidator(object):
    ...
    def validate_request_data(self):
        ...
        """
        validators = [
         self.validate_request_body, self.validate_mandatory_params, self.validate_allowed_params]
    ...
    def validate_mandatory_params(self):
        ...
        if mandatory_params:
            if len([x for x in mandatory_params if x not in list(self.data)]):
                return (False,
                 {'error': 'mandatory params missing. mandatory parameters are {mandatory_params}'.format(mandatory_params=mandatory_params)})
        return (True, )
    ...
    def validate_request_body(self):
        ...
        try:
            ...
            return (
             False, {'error': 'provide a valid json.'})
        else:
            return (True, )
    ...
    def validate_allowed_params(self):
        ...
        if invalid_params:
            return (False,
             {'error': 'invalid params found in request body. invalid parameters are {invalid_params}'.format(invalid_params=invalid_params)})
        else:
            return (True, )
    ...
    def get_mandatory_param(self):
        ...
        """
        mandatory_params = [k for k, v in self.validation_rule.get('fields').items() if self.validation_rule.get(k).get('required') is not None]
        if mandatory_params:
