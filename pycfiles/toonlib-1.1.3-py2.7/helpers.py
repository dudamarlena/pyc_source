...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/toonlib/helpers.py
...
ThermostatInfo = namedtuple('ThermostatInfo', ('active_state', 'boiler_connected',
                                               ...
                                               'current_temperature', 'error_found',
                                               'have_ot_boiler', 'next_program',
                                               'next_set_point', 'next_state', 'next_time',
                                               'ot_communication_error', 'program_state',
                                               'random_configuration_id', 'real_set_point'))
...
Agreement = namedtuple('Agreement', ('id', 'checksum', 'city', 'display_common_name',
                                     'display_hardware_version', 'display_software_version',
                                     'heating_type', 'house_number', 'boiler_management',
                                     'solar', 'toonly', 'post_code', 'street_name'))
PersonalDetails = namedtuple('PersonalDetails', ('number', 'email', 'first_name', 'last_name',
                                                 'middle_name', 'mobile_number',
                                                 'phone_number'))
Client = namedtuple('Client', ('id', 'checksum', 'hash', 'sample', 'personal_details'))
...
class Data(object):
    ...
    class Flow(object):
        ...
        def __init__(self, toon_instance):
            ...
            self._endpoint = {'power': '/client/auth/getElecFlowData', 'gas': '/client/auth/getGasFlowData', 
    ...
    class Graph(object):
        ...
        def __init__(self, toon_instance):
            self.toon = toon_instance
            self._endpoint = {'power': '/client/auth/getElecGraphData', 'gas': '/client/auth/getGasGraphData', 
               'solar': '/client/auth/getSolarGraphData', 
               'district_heat': '/client/auth/getDistrictHeatGraphData'}
...
class Switch(object):
    ...
    def _get_value(self, name, config=False):
        ...
        return next((item.get(name) for item in self.toon._state.get(key).get('device') if item.get('name') == self.name), None)
    ...
    def _change_state(self, state):
        if not self.can_toggle:
            ...
            data = copy.copy(self.toon._parameters)
            data.update({'state': int(state), 'devUuid': self.device_uuid})
            response = self.toon._get_data('/client/auth/smartplug/setTarget', data)
