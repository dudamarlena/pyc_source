...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/toonlib/toonlib.py
...
from .configuration import STATES, STATE_CACHING_SECONDS, DEFAULT_STATE, AUTHENTICATION_ERROR_STRINGS, REQUEST_TIMEOUT
...
class Toon(object):
    ...
    def _populate_info(self, data):
        ...
        self.agreements = [ Agreement(agreement.get('agreementId'), agreement.get('agreementIdChecksum'), agreement.get('city'), agreement.get('displayCommonName'), agreement.get('displayHardwareVersion'), agreement.get('displaySoftwareVersion'), agreement.get('heatingType'), agreement.get('houseNumber'), agreement.get('isBoilerManagement'), agreement.get('isToonSolar'), agreement.get('isToonly'), agreement.get('postalCode'), agreement.get('street')) for agreement in agreements
                          ]
        self.agreement = self.agreements[0]
        details = PersonalDetails(data.get('enecoClientNumber'), data.get('enecoClientEmailAddress'), data.get('enecoClientFirstName'), data.get('enecoClientLastName'), data.get('enecoClientMiddleName'), data.get('enecoClientMobileNumber'), data.get('enecoClientPhoneNumber'))
        self.client = Client(data.get('clientId'), data.get('clientIdChecksum'), data.get('passwordHash'), data.get('sample'), details)

    @property
    def _parameters(self):
        return {'clientId': self.client.id, 'clientIdChecksum': self.client.checksum, 
    ...
    def _get_session(self):
        ...
        data.update({'agreementId': self.agreement.id, 'agreementIdChecksum': self.agreement.checksum})
    ...
    def _state(self):
        ...
        """
        state = {}
        required_keys = ('deviceStatusInfo', 'gasUsage', 'powerUsage', 'thermostatInfo',
                         'thermostatStates')
        try:
        ...
        message = ('Updating internal state with retrieved state:{state}').format(state=state)
    ...
    def _get_data(self, endpoint, params=None):
        ...
        try:
            response = self._session.get(url, params=params or self._parameters, timeout=REQUEST_TIMEOUT)
        except Timeout:
            self._logger.warning('Detected a timeout. Re-authenticating and retrying request.')
        ...
        if response.status_code == 500:
            ...
            if any([ message in error_message for message in AUTHENTICATION_ERROR_STRINGS
                   ]):
                self._logger.warning('Detected an issue with authentication. Trying to reauthenticate.')
                self._login()
                return self._get_data(endpoint, params)
    ...
    def smokedetectors(self):
        ...
        return [ SmokeDetector(smokedetector.get('devUuid'), smokedetector.get('name'), smokedetector.get('lastConnectedChange'), smokedetector.get('connected'), smokedetector.get('batteryLevel'), smokedetector.get('type')) for smokedetector in self._state.get('smokeDetectors', {}).get('device', [])
               ]
    ...
    def lights(self):
        ...
        return [ Light(self, light.get('name')) for light in self._state.get('deviceStatusInfo', {}).get('device', []) if light.get('rgbColor')
               ]
    ...
    def smartplugs(self):
        ...
        return [ SmartPlug(self, plug.get('name')) for plug in self._state.get('deviceStatusInfo', {}).get('device', []) if plug.get('networkHealthState')
               ]
    ...
    def thermostat_states(self):
        ...
        return [ ThermostatState(STATES[state.get('id')], state.get('id'), state.get('tempValue'), state.get('dhw')) for state in self._state['thermostatStates']['state']
               ]
    ...
    def _validate_thermostat_state_name(name):
        if name.lower() not in [ value.lower() for value in STATES.values() if not value.lower() == 'unknown'
                               ]:
