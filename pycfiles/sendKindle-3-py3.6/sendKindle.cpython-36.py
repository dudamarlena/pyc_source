...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/sendKindle.py
# Compiled at: 2018-07-17 04:45:13
# Size of source mod 2**32: 7606 bytes
"""
...
"""
import configparser, getpass, optparse, os, smtplib, sys, traceback
...
class SendKindle:
    ...
    def create_config(self):
        ...
        try:
            ...
            traceback.print_exc()
            message = 'Error creating a new config file, maybe wrong permissions?'
            print(message, file=(sys.stderr))
            sys.exit(4)
        else:
            message = 'A new config file created in: %s\nPlease edit it, provide correct values and run the program again.' % self.conffile
            print(message, file=(sys.stderr))
            sys.exit(5)
        ...
        try:
            if not config.read([self.conffile]):
                ...
                if config.has_option('Default', 'smtp_password'):
                    self.smtp_password = self.smtp_password or config.get('Default', 'smtp_password')
            ...
            traceback.print_exc()
            message = 'Your config file could not be read or contains invalid values: %s' % self.conffile
            print(message, file=(sys.stderr))
            sys.exit(3)

    def parse_args(self):
        usage = 'Usage: %prog [options] FILE...\n  FILE is a file to be sent as an email attachment.'
        description = 'SendKindle will send any number of files via email to your Kindle device.'
        parser = optparse.OptionParser(usage=usage, version=_version, description=description)
        self.parser = parser
        parser.add_option('--password', help="Use provided password instead of smtp_password value from your config file. If you provide neither of these, you'll be asked interactively.")
        parser.add_option('-c', '--convert', action='store_true', default=False,
          help='Ask Kindle service to convert the documents to Kindle format (mainly for PDFs) [default: %default]')
    ...
    def check_args(self):
        if not self.smtp_password:
            self.smtp_password = getpass.getpass('Password for %s: ' % self.user_email)
    ...
    def send_mail(self):
        ...
        try:
            ...
            traceback.print_exc()
            message = 'Communication with your SMTP server failed. Maybe wrong connection details? Check exception details and your config file: %s' % self.conffile
            print(message, file=(sys.stderr))
            sys.exit(7)
    ...
    def get_attachment(self, file_path):
        ...
        try:
            ...
            traceback.print_exc()
            message = 'The requested file could not be read. Maybe wrong permissions?'
            print(message, file=(sys.stderr))
            sys.exit(6)
