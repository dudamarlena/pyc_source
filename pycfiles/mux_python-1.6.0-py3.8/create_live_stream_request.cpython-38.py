...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.14-x86_64/egg/mux_python/models/create_live_stream_request.py
...
class CreateLiveStreamRequest(object):
    ...
    openapi_types = {'playback_policy':'list[PlaybackPolicy]', 
     'new_asset_settings':'CreateAssetRequest', 
     'reconnect_window':'float', 
     'passthrough':'str', 
     'reduced_latency':'bool'}
    attribute_map = {'playback_policy':'playback_policy', 
     'new_asset_settings':'new_asset_settings', 
     'reconnect_window':'reconnect_window', 
     'passthrough':'passthrough', 
     'reduced_latency':'reduced_latency'}

    def __init__(self, playback_policy=None, new_asset_settings=None, reconnect_window=None, passthrough=None, reduced_latency=None):
    ...
    def reconnect_window(self, reconnect_window):
        ...
        """
        if reconnect_window is not None:
            if reconnect_window > 300:
                raise ValueError('Invalid value for `reconnect_window`, must be a value less than or equal to `300`')
        if reconnect_window is not None:
            if reconnect_window < 0.1:
                raise ValueError('Invalid value for `reconnect_window`, must be a value greater than or equal to `0.1`')
