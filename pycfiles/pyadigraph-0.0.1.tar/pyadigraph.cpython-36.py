...
# [GCC 8.4.0]
# Embedded file name: /Users/lucacappelletti/github/pyadigraph/pyadigraph/pyadigraph.py
# Compiled at: 2018-12-09 10:23:34
...
class Adigraph:
    ...
    def __init__(self, row_size: int=2, vertices_color_fallback: str='', edges_color_fallback: str='', layout: Dict[(str, Tuple[(float, float)])]=None, weights: Dict[(str, float)]=None, style: str='', directed: bool=True, vertices_color: Dict[(str, str)]=None, edges_color: Dict[(str, str)]=None, vertices_width: Dict[(str, float)]=None, edges_width: Dict[(str, float)]=None, vertices_label: Dict[(str, str)]=None, edges_label: Dict[(str, str)]=None, sub_caption: str='', sub_label: str='', caption: str='', label: str=''):
    ...
    def _load_placeholders(self):
        ...
        self._document_placeholder = self._load_placeholder('{dir}/placeholders/document.txt'.format(dir=script_dir))
        self._figure_placeholder = self._load_placeholder('{dir}/placeholders/figure.txt'.format(dir=script_dir))
        self._subfigure_placeholder = self._load_placeholder('{dir}/placeholders/subfigure.txt'.format(dir=script_dir))
    ...
    def _subfigure(self, i: int, adigraph: str, caption: str, label: str) -> str:
        """Return subfigure."""
        return self._subfigure_placeholder.format(content=adigraph,
    ...
    def _figure(self, adigraphs: List[str], captions: List[str], labels: List[str]) -> str:
        ...
        return self._figure_placeholder.format(content=('\n'.join([self._subfigure(i, a, c, l) for i, (a, c, l) in enumerate(zip(adigraphs, self._padding(adigraphs, captions), self._padding(adigraphs, labels)), 1)])),
    ...
    def _vertex(self, vertex: str, x: float, y: float, color: str, width: float, label: str) -> str:
        ...
        return '\n\t\t\t{vertex},{color},{width}:{x}\\textwidth,{y}\\textwidth:{label};'.format(vertex=vertex,
          color=color,
          x=(x / 2),
          y=(y / 2),
          width=width,
    ...
    def _vertices(self, vertices: List[str], layout: Dict[(str, Tuple[(float, float)])], colors: Dict[(str, str)], default_color: str, widths: Dict[(str, float)], labels: Dict[(str, str)]) -> str:
        ...
        return ''.join([(self._vertex)(v, *layout[v], *(self._get(colors, v, default_color), self._get(widths, v), self._get(labels, v))) for v in vertices])

    def _edge(self, start: str, end: str, weight: float, color: str, width: float, label: str) -> str:
        """Return formatted edge."""
        return '\n\t\t\t{start},{end},{color},{width}:{weight}:{label};'.format(start=start,
          end=end,
          color=color,
          weight=weight,
          width=width,
    ...
    def _edges(self, edges: List[Tuple[(str, str)]], weights: Dict[(str, float)], colors: Dict[(str, str)], default_color: str, widths: Dict[(str, float)], labels: Dict[(str, str)], directed: bool) -> str:
        ...
        return ''.join([(self._edge)(*e, *(self._simmetric_get(weights, e, directed=directed), self._simmetric_get(colors, e, directed, default_color), self._simmetric_get(widths, e, directed=directed), self._simmetric_get(labels, e, directed=directed))) for e in edges])

    def _adigraph(self, graph: nx.Graph, layout: Dict[(str, Tuple[(float, float)])], weights: Dict[(str, float)], style: str, directed: bool, vertices_color: Dict[(str, str)], vertices_color_fallback: str, edges_color: Dict[(str, str)], edges_color_fallback: str, vertices_width: Dict[(str, float)], edges_width: Dict[(str, float)], vertices_label: Dict[(str, str)], edges_label: Dict[(str, str)]):
        """Return given graph in adigraph syntax."""
        return '\t\\NewAdigraph{{myAdigraph}}{{{vertices}\n\t\t}}{{{edges}\n\t\t}}[{style}]\n\t\t\\myAdigraph{{}}'.format(vertices=(self._vertices(graph.nodes, layout, vertices_color, vertices_color_fallback, vertices_width, vertices_label)),
          edges=(self._edges(graph.edges, weights, edges_color, edges_color_fallback, edges_width, edges_label, directed)),
          style=style)
    ...
    def add_graph(self, graph: nx.Graph, vertices_color_fallback: str='', edges_color_fallback: str='', layout: Dict[(str, Tuple[(float, float)])]=None, weights: Dict[(str, float)]=None, style: str='', directed: bool=None, vertices_color: Dict[(str, str)]=None, edges_color: Dict[(str, str)]=None, vertices_width: Dict[(str, float)]=None, edges_width: Dict[(str, float)]=None, vertices_label: Dict[(str, str)]=None, edges_label: Dict[(str, str)]=None, caption: str='', label: str='') -> str:
    ...
    def __str__(self):
        ...
        return self._figure([(self._adigraph)(*G) for G in zip(self._graphs, self._layouts, self._weights, self._styles, self._directed, self._vertices_color, self._vertices_color_fallbacks, self._edges_color, self._edges_color_fallbacks, self._vertices_width, self._edges_width, self._vertices_label, self._edges_label)], self._captions, self._labels)
