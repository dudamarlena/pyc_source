...
# [GCC 8.4.0]
# Embedded file name: /home/ubuntu/workspace/kotti_google_analytics/kotti_google_analytics/views/view.py
# Compiled at: 2017-06-09 02:40:53
"""
...
@view_config(name='analytics-code', renderer='kotti_google_analytics:templates/tracking_code.pt')
class CartView(BaseView):

    def __call__(self):
        return {'tracking_id': get_setting('property_id') or AnalyticsDefault.property_id, 
...
class Analytics(BaseView):
    ...
    def flow(self):
        ...
        if not hasattr(self, '__flow'):
            self.__flow = ga.auth.Flow(client_id, client_secret, redirect_uri=self.request.resource_url(self.context, 'google-analytics-callback'))
        return self.__flow

    @view_config(name='analytics-report', root_only=True, permission='admin', renderer='kotti_google_analytics:templates/analytics.pt')
    def view(self):
        ga_css_js.need()
        ...
        if not property_id or not access_token:
            return httpexc.HTTPFound(location=('{}?setting_id=kotti_google_analytics').format(self.request.resource_url(self.context, 'controlpanel')))
        else:
    ...
    @view_config(name='google-analytics-callback', root_only=True, permission='admin', renderer='kotti_google_analytics:templates/redirect_result.pt')
    def callback(self):
        try:
            if not self.flow:
                return httpexc.HTTPFound(location=('{}?setting_id=kotti_google_analytics').format(self.request.resource_url(self.context, 'controlpanel')))
        ...
        return {'cp_links': CONTROL_PANEL_LINKS, 
           'credentials': jcred}