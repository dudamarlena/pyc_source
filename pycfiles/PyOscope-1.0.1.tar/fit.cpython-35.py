...
# Embedded file name: build/bdist.linux-x86_64/egg/pyosci/fit.py
# Compiled at: 2017-02-13 02:35:40
# Size of source mod 2**32: 25928 bytes
__doc__ = '\nProvide routines for fitting charge histograms\n'
import sys, pylab as p, numpy as np
from scipy.misc import factorial
from functools import reduce
from copy import deepcopy as copy
from collections import namedtuple
import scipy.optimize as optimize, seaborn.apionly as sb, dashi as d
from . import tools
...
def get_n_hit(charges, nbins):
    ...
    ped_mod = Model(one_gauss, (1000, -0.1, 1))
...
def calculate_peak_to_valley_ratio(bestfitmodel, mu_ped, mu_spe, control_plot=False):
    ...
    valley = min(tmpdata[np.logical_and(bestfitmodel.xs > mu_ped, bestfitmodel.xs < mu_spe)])
...
class Model(object):
    ...
    def __init__(self, func, startparams=None, norm=1):
        ...
        self.norm = None
        self.prediction = lambda xs: reduce(lambda x, y: x + y, [f(xs) for f in self.components])
    ...
    def couple_models(self, coupling_variable):
        ...
        Returns:
            None
        """
        assert len(np.unique(self.n_params)) == 1, 'Models have different numbers of parameters,difficult to couple!'
    ...
    def fit_to_data(self, data, nbins, silent=False, subtract=None, normalize=True, **kwargs):
        """
        ...
        parameters, covariance_matrix = optimize.curve_fit(model, self.xs, data, p0=startparams, **fitkwargs)
        if not silent:
            print('Fit yielded parameters', parameters)
        if not silent:
            print('{:4.2f} NANs in covariance matrix'.format(len(covariance_matrix[np.isnan(covariance_matrix)])))
        if not silent:
            print('##########################################')
        norm = 1
        if normalize:
    ...
    def plot_result(self, ymin=1000, xmax=8, ylabel='normed bincount', xlabel='Q [C]', fig=None, log=True, model_alpha=0.3, add_parameter_text=(('$\\mu_{{SPE}}$& {:4.2e}\\\\', 0), )):
        """
        Show the fit result
        ...
        infotext = '\\begin{tabular}{ll}'
        infotext += '$\\chi^2/ndf$ & {:4.2f}\\\\'.format(self.chi2_ndf)
        infotext += 'entries& {}\\\\'.format(self.data.stats.nentries)
        ...
        ax.text(0.9, 0.9, infotext, horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
        sb.despine()
        return fig
...
def pedestal_fit(filename, nbins, fig=None):
    ...
    * calculating mu
    ...
    charges = -1000000000000.0 * tools.integrate_wf(head, wf)
    ...
    ped_mod = fit.Model(one_gauss, (1000, -0.1, 1))
    ped_mod.fit_to_data(charges, nbins, normalize=False)
    fig = ped_mod.plot_result(add_parameter_text=(('$\\mu_{{ped}}$& {:4.2e}\\\\', 1),
                                                  ('$\\sigma_{{ped}}$& {:4.2e}\\\\', 2)), xlabel='$Q$ [pC]', ymin=1, xmax=8, model_alpha=0.2, fig=fig, ylabel='events')
    ax = fig.gca()
    n_hit = abs(ped_mod.data.bincontent - ped_mod.prediction(ped_mod.xs)).sum()
    ax.grid(1)
    ...
    ax.fill_between(ped_mod.xs, 0.0001, ped_mod.prediction(ped_mod.xs), facecolor=PALETTE[2], alpha=0.2)
    p.savefig(filename.replace('.npy', '.pdf'))
    return ped_mod


def fit_model(charges, model, startparams=None, rej_outliers=False, nbins=200, silent=False, parameter_text=(
 (
  '$\\mu_{{SPE}}$& {:4.2e}\\\\', 5),), use_minuit=False, normalize=True, **kwargs):
    """
...
if __name__ == '__main__':
    ...
    conversion = charge_factor / 10000000000.0
    conversion = 1.0
    charge_factor = 10000000000.0
    print(charge_factor)
    print(conversion)
    nbins = 70
    charges = abs(np.load(sys.argv[1])) * charge_factor
    print(charges)
    ...
    start_params = np.array([0, 10, 1, 0.5, 0.0] + [1] * 10)
    start_params_noise = np.array([10, 0, 1 * conversion, 0.1, 2 * conversion, 0.5 * conversion, 1.0, 1 * conversion])
    ...
    print(params_duo[4])
    mu_peak = 1e-10 * params_duo[4] * 50 * 1e-12
