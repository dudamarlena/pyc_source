...
# Embedded file name: build/bdist.linux-x86_64/egg/pyosci/osci.py
# Compiled at: 2017-02-16 01:04:29
# Size of source mod 2**32: 21667 bytes
__doc__ = '\nCommunicate with oscilloscope via vxi11 protocol over LAN network\n'
import abc, time, numpy as np, pylab as p, vxi11
...
def setget(command):
    ...
    Returns:
        property object
    """
    return property(lambda self: self._send(q(command)), lambda self, value: self._set(aarg(command, value)))
...
class TektronixDPO4104B(AbstractBaseOscilloscope):
    ...
    def samplingrate(self):
        ...
        """
        head = self.wf_header()
        self.logger.debug('Got samplingrate of {}'.format(1.0 / head['xincr']))
        return 1.0 / head['xincr']
    ...
    def histogram(self):
        ...
        assert None not in [start, end, bincontent], 'Try again! might just be a hickup {} {} {}'.format(start, end, bincontent)
        ...
        bincenters = r_binedges + (r_binedges - l_binedges) / 2.0
        return (
         bincenters, bincontent)
    ...
    def acquire_waveform(self, buff_header=False):
        ...
        waveform = (waveform - np.ones(len(waveform)) * header['yoff']) * (np.ones(len(waveform)) * header['ymult']) + np.ones(len(waveform)) * header['yzero']
        return waveform
    ...
    def set_feature_acquisition_window(self, leading, trailing, n_waveforms=20):
        ...
        leading_bins = 1e-09 * float(leading) / bin_width
        trailing_bins = 1e-09 * float(trailing) / bin_width
    ...
    def make_n_acquisitions(self, n, trials=20, return_only_charge=False, single_acquisition=True):
        """
        Acquire n waveforms
    ...
    def pull(self, buff_header=True, use_buffered_acq_window=True, use_channel_info=True):
        """
