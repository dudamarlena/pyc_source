...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus/provision/common/certs.py
# Compiled at: 2011-08-25 12:53:02
"""
...
class CertificateGenerator(object):
    ...
    def gen_selfsigned_ca_cert(self, cn):
        return self.__gen_certificate(cn=cn, issuer_cert=None, issuer_key=None)

    def gen_user_cert(self, cn):
        return self.__gen_certificate(cn=cn, issuer_cert=self.ca_cert, issuer_key=self.ca_key)

    def gen_host_cert(self, hostname):
        return self.__gen_certificate(cn='host/' + hostname, issuer_cert=self.ca_cert, issuer_key=self.ca_key)
