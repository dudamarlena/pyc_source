...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus/provision/common/persistence.py
...
PropertyTypes = enum('STRING', 'INTEGER', 'NUMBER', 'BOOLEAN', 'OBJECT', 'ARRAY', 'NULL', 'ANY')

def pt_to_str(pt, items_type=None):
    if pt == PropertyTypes.STRING:
        return 'string'
    else:
        if pt == PropertyTypes.INTEGER:
            return 'integer'
        if pt == PropertyTypes.NUMBER:
            return 'number'
        if pt == PropertyTypes.BOOLEAN:
            return 'boolean'
        if pt == PropertyTypes.OBJECT:
            return 'object'
        if pt == PropertyTypes.ARRAY:
            return 'list of %s' % pt_to_str(items_type)
        if pt == PropertyTypes.NULL:
            return 'null'
        if pt == PropertyTypes.ANY:
            return 'any'
        if inspect.isclass(pt) and issubclass(pt, PersistentObject):
            return pt.__name__
        return 'unknown'
...
class PrimitivePropertyChange(PropertyChange):
    ...
    def to_dict(self):
        ...
        if self.change_type == PropertyChange.REMOVE:
            return {'__action__': 'remove'}
        if self.change_type == PropertyChange.EDIT:
            d = {}
...
class PersistentObject(object):
    ...
    def add_to_array(self, p_name, item_value):
        ...
        if p.items_unique:
            if inspect.isclass(p.items) and issubclass(p.items, PersistentObject):
                p_value = self.has_property(p_name) or {}
    ...
    def validate_update(self, pobj):
        ...
        for (name, property) in self.properties.items():
            ...
            if property.type in (PropertyTypes.STRING, PropertyTypes.INTEGER, PropertyTypes.NUMBER, PropertyTypes.BOOLEAN, PropertyTypes.NULL):
                if not self_hasattr:
                    if pobj_hasattr:
                        raise property.editable or ObjectValidationException("Tried to add a property, but it is non-editable (setting '%s' to %s)" % (name, pobj_value))
                    else:
                        changes[name] = PrimitivePropertyChange(PropertyChange.ADD, None, pobj_value)
                elif self_hasattr:
                    if not pobj_hasattr:
                        raise property.editable or ObjectValidationException("Tried to remove a property, but it is non-editable (removing '%s' = %s)" % (name, self_value))
                    else:
                        changes[name] = PrimitivePropertyChange(PropertyChange.REMOVE, self_value, None)
                elif self_hasattr:
                    if pobj_hasattr:
                        if self_value != pobj_value:
                            raise property.editable or ObjectValidationException("Tried to change the value of non-editable property '%s' (from %s to %s)" % (name, self_value, pobj_value))
                        else:
                ...
                if not self_hasattr:
                    ...
                    if pobj_hasattr:
                        ...
                        if len(property_changes.changes) > 0:
                            raise property.editable or ObjectValidationException("Tried to to change the value of non-editable property '%s' (Changes: '%s')" % (name, property_changes.to_dict()))
                        else:
        ...
        for (name, property) in self.properties.items():
            if hasattr(self, name):
                if property.type in (PropertyTypes.STRING, PropertyTypes.INTEGER, PropertyTypes.NUMBER, PropertyTypes.BOOLEAN, PropertyTypes.NULL):
                    ...
                    if issubclass(property.type, PersistentObject):
    ...
    def __primitive_to_ruby(self, value, p_type):
        if p_type == PropertyTypes.STRING:
            return '"%s"' % value
        if p_type == PropertyTypes.INTEGER:
            return '%i' % value
        if p_type == PropertyTypes.NUMBER:
            return '%f' % value
        if p_type == PropertyTypes.BOOLEAN:
            if value == True:
                return 'true'
            else:
                return 'false'
    ...
    def from_json_dict(cls, obj_dict):
        ...
        for (p_name, p_value) in obj_dict.items():
            ...
            elif property.type in (PropertyTypes.STRING, PropertyTypes.INTEGER, PropertyTypes.NUMBER, PropertyTypes.BOOLEAN, PropertyTypes.NULL):
