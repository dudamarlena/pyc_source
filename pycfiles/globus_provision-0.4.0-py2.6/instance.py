...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus/provision/core/instance.py
...
class InstanceStore(object):
    ...
    def create_new_instance(self, topology_json, config_txt):
        created = False
        while not created:
            inst_id = 'gpi-' + hex(random.randint(1, 2147483647))[2:].rjust(8, '0')
...
class Instance(object):
    ...
    def gen_certificates(self, force_hosts=False, force_users=False, force_ca=False):
        ...
        certg.save_certificate(cert=ca_cert, key=ca_key, cert_file=ca_cert_file, key_file=ca_key_file, force=force_ca)
        ...
        for user in users:
            ...
            certg.save_certificate(cert=cert, key=key, cert_file=cert_file, key_file=key_file, force=force_users)
        ...
        for n in nodes:
            ...
            cert_files.append(cert_file)
            cert_files.append(key_file)
            certg.save_certificate(cert=cert, key=key, cert_file=cert_file, key_file=key_file, force=force_hosts)
