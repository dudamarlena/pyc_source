...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus/provision/tests/test_samples.py
# Compiled at: 2012-03-02 22:17:19
from globus.provision.core.api import API
from globus.provision.tests.common import load_config_file, create_instances_dir, remove_instances_dir, create_temp_config_file
from globus.provision.core.topology import Topology
import json
from globus.provision.cli.api import gp_instance_create, gp_instance_describe, gp_instance_start, gp_instance_stop, gp_instance_list, gp_instance_update, gp_instance_terminate
import os
SAMPLE_DIR = './samples/'
...
def test_samples_api():
    for s in SAMPLES:
        yield (
         check_sample_api, SAMPLE_DIR + s)


def test_samples_cli():
    for s in SAMPLES:
        yield (
         check_sample_cli, SAMPLE_DIR + s)
