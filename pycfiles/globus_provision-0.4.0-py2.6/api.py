...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/globus/provision/core/api.py
...
import globus.provision.deploy.ec2 as ec2_deploy, globus.provision.deploy.dummy as dummy_deploy
...
from globus.provision.common.persistence import ObjectValidationException, PropertyChange
from globus.provision.common.go_transfer import GlobusOnlineCLIHelper, GlobusOnlineHelper, GlobusOnlineException
...
class API(object):
    ...
    def instance_update(self, inst_id, topology_json, extra_files, run_cmds):
        try:
            ...
            if inst.topology.state not in (Topology.STATE_RUNNING, Topology.STATE_FAILED):
    ...
    def instance_list(self, inst_ids):
        ...
        return (
         API.STATUS_SUCCESS, 'Success', instances_json)
    ...
    def __configure_vms(self, deployer, node_vm, basic=True, chef=True):
        ...
        threads = {}
        for node in order:
            threads[node] = deployer.NodeConfigureThread(mt_configure, 'configure-%s' % node.id, node, node_vm[node], deployer, depends=[ threads[t] for t in topology.get_depends(node) ], basic=basic, chef=chef)
    ...
    def __stop_vms(self, deployer, nodes):
        ...
        threads = {}
        for node in order:
            threads[node] = deployer.NodeConfigureThread(mt_configure, 'stop-configure-%s' % node.id, node, node_vm[node], deployer, depends=[ threads[t] for t in topology.get_depends(node) ])
