...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/html/html_tools.py
# Compiled at: 2019-10-17 02:10:40
# Size of source mod 2**32: 7874 bytes
import jinja2
...
class HTMLToolkit:
    ...
    def head_plist2html(cls, headpair_list):
        ...
        l = [
         HTMLToolkit.str2html_comment('CSS import'),
         html_css,
         '',
    ...
    def kv_iter2html_attrs(cls, iterable):
        return cls.join_html(' ', [ternary((v is None), f_true=(lambda b: k),
          f_false=(lambda b: cls.format_html('{0}="{1}"', k, escape_doublequotes(str(v))))) for k, v in iterable])
    ...
    def wrap_html_tag(cls, s, tag, attrs=None, delim=''):
        html_attrs = cls.kv_iter2html_attrs(attrs.items()) if attrs else ''
        l = [
         cls.format_html('<{0} {1}>', tag, html_attrs),
         s,
    ...
    def html_div_height(cls, height, attrs=None):
        a = merge_dicts([attrs, {'style': 'height:{0}px;'.format(height)}], vwrite=(cls.vwrite_attrs))
