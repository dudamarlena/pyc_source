...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/messenger/slack/slack_tool.py
...
class SlackTool:
    ...
    def str2j_block_list(cls, str_in):
        ...
        j_list = [{'type':'section',  'text':{'type':'mrkdwn',  'text':'\n'.join(l[i * p:(i + 1) * p])}} for i in range(k)]
        return j_list
    ...
    @classmethod
    def h_payload2text(cls, h):
        return h['data'].get('text')

    @classmethod
    def h_payload2channel(cls, h):
        return h['data'].get('channel')

    @classmethod
    def h_payload2web_client(cls, h):
        return h['web_client']
