...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/entity/calendar/dayofweek/locale/ko/dayofweek_entity_ko.py
...
class DayofweekEntityKo:
    ...
    def _h_str2value(cls):
        return merge_dicts([{s: v} for v, s in cls.h_value2str().items()],
    ...
    def str2entity_list(cls, str_in):
        ...
        logger.debug({'entity_list_single':entity_list_single,  'entity_list_concat':entity_list_concat, 
         'll':ll, 
         'l':l})
        return l
...
class DayofweekEntityKoSingle:
    ...
    @classmethod
    def rstr(cls):
        return format_str('{}(?:요일)?', rstr2wrapped(DayofweekEntityKo.rstr_short()))
...
class DayofweekEntityKoConcat:
    ...
    def rstr(cls):
        ...
        rstr = format_str('{}(?:(?:\\s*,\\s*)?{})+', rstr2wrapped(rstr_short), rstr2wrapped(rstr_short))
        return rstr
    ...
    def match2entity_list(cls, m):
        ...
        text = m.group()
        n = len(text)
        l = [{Entity.F.SPAN: (s + i, s + i + 1), Entity.F.VALUE: DayofweekEntityKo.str2value(text[i]), Entity.F.TEXT: text[i]} for i in range(n) if text[i] != ',' if not text[i].isspace()]
