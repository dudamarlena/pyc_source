...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/collections/chunk_tools.py
...
class ChunkToolkit:
    ...
    def index_list2chunks(cls, l, index_list):
        ...
        n = len(index_list)
        return [l[index_list[(i - 1)] if i else 0:index_list[i]] for i in range(n)]
    ...
    def iter_batches2yoo(cls, iter, batch_chunksize_list):
        ...
        iter_out = reduce(lambda x_iter, fp: cls.iter_batch2yoo(x_iter, fp[0], fp[1]), batch_chunksize_list, iter)
        yield from iter_out
        if False:
            yield None
    ...
    def iter_batch_cond2processed(cls, x_iter, f_batch, f_cond, size_minimax):
        ...
        def inputs_indexes2y_list(x_list, i_list):
            ...
            y_list = [x if i not in h_i2k else y_list_target[h_i2k[i]] for i, x in enumerate(x_list)]
            return y_list
        ...
        for x in x_iter:
            ...
            if not is_buffer_empty:
                ...
                logger.debug({'# i_list_target':len(i_list_target),  '# y_list_buffer':len(y_list_buffer)})
...
class BatchPoolTool:
    ...
    def iter2backoff_batches(cls, iter, batch_chunksize_list, buffer_size):
        ...
        def j2batch(j):
            ...
            if j == m - 1:
                ix_list = [(i, buffer_in[i]) for i, (b, y) in zip_strict(i_list, by_list) if not b]
