...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/collections/collections_tool.py
# Compiled at: 2020-01-28 21:02:33
...
from itertools import chain, product, combinations, islice, count, groupby, repeat, starmap, tee, zip_longest, cycle, filterfalse
...
class IterTool:
    ...
    def list_func_count2index_list_continuous_valid(cls, l, f_valid, count_match):
        n = len(l)
        i_list_valid = lfilter(lambda i: all((f_valid(l[(i + j)]) for j in range(count_match))), range(n - (count_match - 1)))
        return i_list_valid
    ...
    def classify_by(cls, iterable, func_list):
        ...
        for obj in l_all:
            index = next((i for i, func in enumerate(func_list) if func(obj)), len(func_list))
...
class ListPairAlign:
    ...
    def list_pivotlist2indexes(cls, target_list, pivot_list):
        n = len(pivot_list)
        h = merge_dicts([{pivot_list[i]: i} for i in range(n)], vwrite=vwrite_no_duplicate_key)
    ...
    def list_pair2l2_aligned(cls, l1, l2, mode, default=None):
        ...
        l2_aligned = [l2[i2] if i2 is not None else default for i2 in i2_list]
        return l2_aligned
...
class ListTool:
    ...
    def iv_lists2merged(cls, iv_lists):
        h_list = [{i: v} for iv_list in iv_lists for i, v in iv_list]
...
class DictTool:
    ...
    bn_list_h_list2v_or_f_else = VersionTool.deprecated(func=branchname_list2lookup_h_list_or_f_default, version_current=__version__,
      version_tos='0.3')
    ...
    bn_list_h_list2v_or_else = VersionTool.deprecated(func=branchname_list2lookup_h_list_or_default, version_current=__version__,
      version_tos='0.3')
    ...
    def tree_func_list2reduced(cls, h, f_list):
        ...
        if len(f_list) <= 1:
            ...
            h_CHILD = {k:cls.tree_func_list2reduced(v, f_list[1:]) for k, v in h.items()}
        if f is None:
            h_OUT = h_CHILD
        else:
            h_OUT = f(h_CHILD)
    ...
    tree_func_list2RnC = VersionTool.deprecated(func=tree_func_list2reduced_and_cleaned, version_current=__version__,
      version_tos='0.3')
...
class SingletonToolkit:
    ...
    class TooManyObjectsError(Exception):
        ...
        @classmethod
        def chk_n_raise(cls, obj_list, count):
            if not obj_list or len(obj_list) <= count:
                return obj_list
            raise cls()
...
class LLToolkit:
    ...
    def f_batch2f_n_ll(cls, f_batch):
        ...
        def f_wrapped(count_unwrap, ll, *args, **kwargs):
            if count_unwrap == 0:
                return f_batch(ll, *args, **kwargs)
            flat_list, dim = cls._ll2flat_dim(ll, count_unwrap)
            n_input = len(flat_list)
            v_list = f_batch(flat_list, *args, **kwargs)
            n_output = len(v_list)
            assert_equal(n_input, n_output, 'f_list() should neither remove elements nor modify order of elements!')
