...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/span/span_tool.py
# Compiled at: 2020-01-28 17:23:36
# Size of source mod 2**32: 10483 bytes
import logging
...
class SpanTool:
    ...
    def span_iter2merged(cls, span_iter):
        span_list_in = lfilter(bool, span_iter)
        if not span_list_in:
            return []
    ...
    def index_total_beam2span(cls, index, total, beam):
        ...
        if index <= buffer_pre:
            return (
             0, min(count_return, total))
        if index + buffer_post >= total - 1:
            return (
             max(0, total - buffer_post), total)
        ...
        if len(spans_list) == 1:
            ...
            if len(spans_list) == 2:
                ...
                for j1, j2 in product(range(p1), range(p2)):
                    ...
                    logger.debug({'span_gap':span_gap,  'is_valid':is_valid})
                    if is_valid:
                        yield (
                         j1, j2)
                ...
                logger.debug({'j_pairs_head':j_pairs_head,  'j1_list_valid':j1_list_valid, 
                ...
                h_j1_to_j_tuples_tail_filtered = {j1_list_valid[k1]:l for k1, l in gb_tree_global(j_tuples_tail_filtered, [ig(0)])}
