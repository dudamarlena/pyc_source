...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/file/file_tools.py
...
class FileToolkit:
    ...
    @classmethod
    def filepath2bytes(cls, filepath, f_open=None):
        if f_open is None:
    ...
    def filepath2utf8(cls, filepath, encoding=None, f_open=None):
        if f_open is None:
            ...
            return os.path.exists(filepath) or None
        with f_open(filepath) as (f):
            s_dec = f.read()
        return s_dec
    ...
    @classmethod
    def bytes2file(cls, bytes, filepath, f_open=None):
        if f_open is None:
    ...
    @classmethod
    def utf82file(cls, utf8, filepath, encoding='utf-8', f_open=None):
...
class FileTimeToolkit:
    ...
    @classmethod
    def filepath2dt_mtime(cls, filepath_in, empty_file_allowed=False):
        if empty_file_allowed is None:
            ...
            if not empty_file_allowed:
                if st_size == 0:
                    return cls.dt_always_outdated()
        mtime = os.path.getmtime(filepath_in)
        dt_utc = pytz_localize(datetime.utcfromtimestamp(mtime), pytz.utc)
        return dt_utc

    @classmethod
    def filepath2is_uptodate(cls, filepath, pivot_datetime, empty_file_allowed=False):
