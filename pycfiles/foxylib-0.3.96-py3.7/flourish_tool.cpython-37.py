...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/data_visualization/flourish/flourish_tool.py
# Compiled at: 2020-01-28 16:34:48
# Size of source mod 2**32: 21857 bytes
import logging
...
from foxylib.tools.collections.collections_tool import iter2singleton, AbsoluteOrder, ListTool, lchain, IterTool, f_iter2f_list
...
class FlourishTable:
    ...
    def table2value_max(cls, table):
        return max((int(v) for l in table[1:] for v in l[FlourishTable.COUNT_COLHEAD:]))
    ...
    def table2percentage(cls, table, v):
        ...
        h_j2col_sum = {j:sum(map(int, filter(bool, map(ig(j), table[1:])))) for j in range(cls.COUNT_COLHEAD, count_col)}
        for i, l in enumerate(table):
            if i == 0:
                yield l
                continue
            l_head = l[:cls.COUNT_COLHEAD]
            l_right = ['{:.02f}'.format(int(l[j]) * 100 / h_j2col_sum[j]) if l[j] else l[j] for j in range(cls.COUNT_COLHEAD, count_col)]
        ...
        for i in range(n_in):
            if not i == n_in - 1:
                if not cls._index2is_data(i):
                    ...
                    continue
                for j in range(multiple):
                    if not (str_list[i] and str_list[(i + 1)]):
                        yield
                        continue
                    v0 = float(str_list[i])
    ...
    def str_ll_col_index_extended2interpolated_list(cls, str_ll, j_col, offset, multiple):
        ...
        return [(float(str_ll[i][j_col]) * (multiple - offset) + float(str_ll[i][(j_col + 1)]) * offset) / multiple for i in range(n_row)]
    ...
    def str_ll2interpolated_list(cls, str_ll_in, i_pivot, multiple):
        ...
        n_col_extended = cls.col_count_raw2extended(n_col_raw, multiple)
        logger.debug({'n_col_raw':n_col_raw,  'n_col_extended':n_col_extended, 
         'multiple':multiple})
        ll_tr = [cls._col_index_extended2str_list(str_ll_in, k, i_pivot, multiple) for k in range(n_col_extended)]
        ll_out = [[ll_tr[k][i] for k in range(n_col_extended)] for i in range(n_row)]
        return ll_out
    ...
    def _index_strs_beam2index_iter(cls, str_list_in, i_pivot_in, beam):
        ...
        v_list = lmap(lambda i: (-float(str_list[i]) if str_list[i] else AbsoluteOrder.MAX,
         -1 if i == i_pivot_in else 0), range(count_value))
    ...
    def _i2j_list2l_out(cls, row, j_list):
        ...
        l_data = lmap(lambda j:         if j in j_list:
row[j] # Avoid dead code: '', range(cls.COUNT_COLHEAD, col_count))
    ...
    def table2beamed(cls, table, i_pivot, beam):
        ...
        table_filtered = lchain([table[0]], [cls._i2j_list2l_out(table[i], j_list) for i, j_list in i2j_list])
    ...
    def _table_colspan_i2is_valid(cls, table, colspan, rowindex, fresh_start_req=True):
        ...
        if i == 0:
            ...
            if s == cls.COUNT_COLHEAD:
                return True
            return fresh_start_req or True
        assert_greater_equal(s - cls.COUNT_COLHEAD - 1, 0, {'start':s, 
    ...
    def _table_colspan2rowindexes_valid(cls, table, colspan, fresh_start_req=True):
        n_row = len(table)
        return lfilter(lambda i: cls._table_colspan_i2is_valid(table, colspan, i, fresh_start_req=fresh_start_req), range(n_row))
    ...
    def table_colspan2fresh_rows_filtered(cls, table, colspan):
        ...
        logger.debug({'colspan':colspan,  'len(table)':len(table), 
    ...
    def table_rowindexes_colspan2trimmed(cls, table, rowindex_list, colspan):
        ...
        logger.debug({'colspan':colspan,  '# table':len(table), 
    ...
    def table_spans2summed_row_iter(cls, table, colspan_list):
        ...
        def row_span2str(row, span):
            s, e = span
            v = sum(map(lambda x:             if x:
int(x) # Avoid dead code: 0, row[s:e]))
