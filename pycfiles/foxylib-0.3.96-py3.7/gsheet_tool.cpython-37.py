...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/googleapi/gsheet_tool.py
# Compiled at: 2020-01-28 16:34:48
# Size of source mod 2**32: 12059 bytes
import re
...
from foxylib.tools.collections.collections_tool import lfilter_duplicate, ListTool, vwrite_no_duplicate_key, merge_dicts, luniq, filter2single_or_none, list2tuple
...
class GSSTool:
    ...
    def str_list2j_row(cls, j_colhead_list, str_list_ROW):
        ...
        j_cells = luniq([GSSTool.Cell.parse_str2j(j_colhead_list[k], str_list_ROW[k]) for k in k_list_valid],
    ...
    class DataUniqueValidatorException(Exception):
        pass
    ...
    def data2check_unique(cls, j_colhead_list, str_COL_list_ROW_list):
        ...
        if not j_list_uniq:
            ...
            iList_duplicate = sorted(lfilter_duplicate((range(count_row)), key=(lambda i: tuple_ROW_list[i])), key=(lambda i: (
             tuple_ROW_list[i], i)))
            return iList_duplicate or None
    ...
    def table_ll2j_pair(cls, ll_IN):
        ...
        j_colhead_list = [cls.ColHead.parse_str2j_colhead(str_list_HEAD[k]) for k in range(col_count)]
        cls.data2check_unique(j_colhead_list, str_COL_list_ROW_list)
        j_row_list_raw = [cls.str_list2j_row(j_colhead_list, str_COL_list_ROW) for str_COL_list_ROW in str_COL_list_ROW_list]
    ...
    def sheet2unmerged(cls, creds, gsheet_id_IN, sheet_name_IN, gsheet_id_OUT, sheet_name_OUT):
        ...
        request = {'function':'run', 
         'parameters':[
          gsheet_id_IN, sheet_name_IN,
          gsheet_id_OUT, sheet_name_OUT]}
        try:
            response = service.scripts().run(body=request, scriptId=(AppsscriptToolkit.SCRIPT_ID_SPREADSHEET2UNMERGE)).execute()
    ...
    class Cell:
        ...
        def j2key(cls, j):
            return (
             cls.j2cn(j), list2tuple(cls.j2v(j)))

        @classmethod
        def _j_list2kj_dict(cls, j_row):
            return merge_dicts([{cls.j2cn(j_cell): j_cell} for j_cell in j_row], vwrite=vwrite_no_duplicate_key)

        @classmethod
        def j_list2kv_dict(cls, l):
            h_k2j = cls._j_list2kj_dict(l)
            return merge_dicts([{k: cls.j2v(j)} for k, j in h_k2j.items()], vwrite=vwrite_no_duplicate_key)
