...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/log/logger_tool.py
# Compiled at: 2020-01-01 15:08:00
# Size of source mod 2**32: 7685 bytes
...
class FoxylibLogFormatter:
    ...
    @classmethod
    def config(cls):
        h = {'format':cls.format(),  'datefmt':cls.datefmt()}
        return h
...
class LoggerTool:
    ...
    def filepath2handler_default(cls, filepath):
        handler = RotatingFileHandler(filepath, mode='a', maxBytes=5242880, backupCount=2,
          encoding='utf-8',
          delay=False)
        return handler
    ...
    class SEWrapper:
        ...
        @classmethod
        def info(cls, *a, **k):
            return (LoggerTool.f_log2f_level(cls.log, logging.INFO))(*a, **k)
    ...
    def attach_filepath2rootname_list(cls, rootname_list, filepath, level):
        ...
        handler = LoggerTool.handler_formatter2formatted(LoggerTool.filepath2handler_default(filepath), FoxylibLogFormatter.formatter())
