...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/database/elasticsearch/elasticsearch_tool.py
...
class ElasticsearchTool:
    ...
    @classmethod
    def j_result2scroll_id(cls, j_result):
        return j_result['_scroll_id']
    ...
    def index2ids(cls, es_client, index):
        if not ESTool.index2exists(es_client, index):
            raise StopIteration()
        j_iter = scan(es_client, query={'query':{'match_all': {}}, 
         'stored_fields':[]},
          index=index)
        for j in j_iter:
            yield j['_id']
    ...
    def fieldname_list2j_mapping_fielddata(cls, fieldname_list):
        j_property_list = [{fieldname: {'type':'text',  'fielddata':True}} for fieldname in fieldname_list]
...
class BulkTool:
    ...
    @classmethod
    def j_action2id(cls, j):
        return j.get(cls.F.ID)

    @classmethod
    def j_action2index(cls, j):
        return j.get(cls.F.INDEX)

    @classmethod
    def j_action2body(cls, j):
        return j.get(cls.F.SOURCE)

    @classmethod
    def j_action2doc_type(cls, j):
        return j.get(cls.F.TYPE)

    @classmethod
    def j_action2op_type(cls, j):
        return j.get(cls.F.OP_TYPE, cls.op_type_default())

    @classmethod
    def op_type_default(cls):
        return 'index'
...
class ElasticsearchQuery:
    ...
    @classmethod
    def j_function_score2jq(cls, j):
        return {'query': j}

    @classmethod
    def jqi2prefix(cls, jqi):
        return {'prefix': jqi}
    ...
    @classmethod
    def kl2jqi_terms(cls, k, l):
        return {'terms': {k: l}}

    @classmethod
    def kl2jqi_match(cls, k, l):
        return {'match': {k: l}}
    ...
    def query_fields2jqi_multimatch(cls, str_query, field_list):
        return {'multi_match': {'query':str_query, 
                         'fields':field_list}}
    ...
    def index_jq_list2j_msearch(cls, index_jq_list):
        j_msearch = lchain(*[[{'index': index}, jq] for index, jq in index_jq_list])
        return j_msearch
...
class ElasticsearchFunction:
    ...
    def jf_decay(cls, decay_function, fieldname, origin, scale, offset=None, decay=None):
        ...
        if decay is not None:
            l.append({'decay': decay})
        j = {decay_function: {fieldname: merge_dicts(l)}}
        return j
