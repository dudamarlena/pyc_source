...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/database/mongodb/mongodb_tool.py
# Compiled at: 2020-02-07 17:13:26
# Size of source mod 2**32: 7193 bytes
import logging
...
from foxylib.tools.collections.collections_tool import vwrite_no_duplicate_key, merge_dicts, f_iter2f_list, DictTool, sfilter
...
class MongoDBTool:
    ...
    def result2j_doc_iter(cls, find_result):
        for h in find_result:
            j = {k:v if k != '_id' else str(v) for k, v in h.items()}
            yield j

    @classmethod
    def update_result2json(cls, update_result):
        return {'acknowledged':update_result.acknowledged,  'matched_count':update_result.matched_count, 
         'modified_count':update_result.modified_count, 
         'upserted_id':str(update_result.upserted_id)}

    @classmethod
    def delete_result2json(cls, delete_result):
        return {'acknowledged':delete_result.acknowledged,  'deleted_count':delete_result.deleted_count}
    ...
    @classmethod
    def j_doc2id(cls, j_doc):
        return j_doc['_id']

    @classmethod
    def doc_id2datetime(cls, doc_id):
    ...
    def j_doc_iter2h_doc_id2j_doc(cls, j_doc_iter):
        h = merge_dicts([{MongoDBTool.j_doc2id(j_doc): j_doc} for j_doc in j_doc_iter], vwrite=vwrite_no_duplicate_key)
        return h
...
class DocumentTool:
    ...
    def docs_pair2j_diff(cls, docs_pair, keys=None):
        ...
        h1 = merge_dicts([{h2key(h): h} for h in doc_list_1], vwrite=vwrite_no_duplicate_key)
