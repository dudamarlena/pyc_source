...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/database/mongodb/mongodb_tools.py
...
class MongoDBToolkit:
    ...
    def j_pair_list2upsert(cls, collection, j_pair_list):
        ...
        requests = [UpdateOne(j_filter, {'$set': j_update}, upsert=True) for j_filter, j_update in j_pair_list]
    ...
    def j_doc_iter2h_doc_id2j_doc(cls, j_doc_iter):
        h = merge_dicts([{MongoDBToolkit.j_doc2id(j_doc): j_doc} for j_doc in j_doc_iter], vwrite=vwrite_no_duplicate_key)
        return h