...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.15-x86_64/egg/foxylib/tools/date/date_tools.py
# Compiled at: 2020-01-28 16:34:48
# Size of source mod 2**32: 7410 bytes
import calendar, os, re, yaml
...
class DateToolkit:
    ...
    def date_pair2matches_week_boundary(cls, date_pair):
        if not DayOfWeek.date2is_monday(date_pair[0]):
            ...
            return DayOfWeek.date2is_sunday(date_pair[1]) or False
        return True
    ...
    def date_pair2matches_year_boundary(cls, date_pair):
        if not DateToolkit.date2is_jan_1st(date_pair[0]):
            ...
            return DateToolkit.date2is_dec_31st(date_pair[1]) or False
        return True
...
class RelativeDeltaToolkit:
    ...
    def pattern_timedelta(cls):
        ...
        rstr_reldelta_list = [format_str('(?:(?P<{0}>\\d+)\\s*(?:{1}))?', k, '|'.join(lmap(re.escape, j_name2strs(j_reldelta[k])))) for k in reldalta_name_list]
        rstr_reldeltas = '\\s*'.join(['(?:{0})'.format(rstr) for rstr in rstr_reldelta_list])
        rstr = '\\s*'.join(['(?P<sign>[+-])', rstr_reldeltas])
    ...
    def parse_str2reldelta(cls, s):
        ...
        kv_list = [(k, int_sign * int(m.group(k))) for k in cls.reldelta_name_list() if m.group(k)]
