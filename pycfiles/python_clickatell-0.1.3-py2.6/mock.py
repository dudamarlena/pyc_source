...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/clickatell/tests/mock.py
# Compiled at: 2010-05-24 04:29:06
import logging
...
class TestHttpClient(HttpClient):
    ...
    def get_mocked(self, method, url, data={}, headers={}):
        ...
        for (index, (s_url, s_data, s_headers, response)) in enumeration:
            if url == s_url and data == s_data and headers == s_headers:
    ...
    def log_mocks(self):
        ...
        for (method, mocks) in self.queue.items():
            for (url, data, headers, response) in mocks:
                logging.debug('\t%(method)s %(url)s with %(data)s and headers (%(headers)s):' % locals())
