...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.6-x86_64/egg/clickatell/api.py
# Compiled at: 2010-05-26 04:43:38
...
class Batch(object):
    ...
    def sendmsg(self, context={}, **options):
        ...
        options.update({'session_id': self.clickatell.session_id, 
           'batch_id': batch_id})
    ...
    def end(self, batch_id):
        ...
        """
        (resp,) = self.clickatell.client.batch('endbatch', {'session_id': self.clickatell.session_id, 
           'batch_id': batch_id})
...
class Clickatell(object):
    ...
    def __init__(self, username, password, api_id, client_class=Client, sendmsg_defaults={}):
    ...
    def get_new_session_id(self):
        ...
        """
        (resp,) = self.client.http('auth', {'user': self.username, 
    ...
    def session_expired(self):
        ...
        if not self._session_start_time:
            return True
        return datetime.now() - self._session_start_time >= self.SESSION_TIME_OUT
    ...
    def ping(self):
        ...
        """
        (resp,) = self.client.http('ping', {'session_id': self.session_id})
        if isinstance(resp, OKResponse):
            self.reset_session_timeout()
            return True
        raise ClickatellError, resp
    ...
    def sendmsg(self, **options):
        ...
        options = validator.validate(options)
        options.update({'to': validator.dispatch('to', options.pop('recipients')), 
    ...
    def getbalance(self):
        ...
        """
        (resp,) = self.client.http('getbalance', {'session_id': self.session_id})
    ...
    def check_coverage(self, msisdn):
        ...
        """
        (resp,) = self.client.utils('routeCoverage.php', {'msisdn': msisdn, 
           'session_id': self.session_id})
        return resp
    ...
    def getmsgcharge(self, apimsgid):
        ...
        """
        (resp,) = self.client.http('getmsgcharge', {'session_id': self.session_id, 
           'apimsgid': apimsgid})
        return resp
