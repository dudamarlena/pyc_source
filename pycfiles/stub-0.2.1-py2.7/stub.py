...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/stub.py
# Compiled at: 2013-09-22 01:27:46
            ...
            if len(args) == len(varnames):
                ...
                for n in varnames:
                    if not defaults and len(args) <= varnames.index(n):
                        pass
                    if n not in kwargs and n in defdict:
                        kwargs[n] = defdict[n]
...
def stub_attributes(target, **attributes):
    """Stub out old attribute values on `target` with new ones.
        ...
        """
    if type(target.__class__) != TypeType:
        raise TypeError('Stubbing attributes is not supported on old-style classes')
    if [ a for a in attributes if a not in dir(target) ]:
    ...
    def unstubbed_builder(name):
        ...
        def unstubbed(self):
            if name in target._old_class.__dict__:
                ...
                if hasattr(target._old_class, name):
