...
# Embedded file name: build/bdist.linux-x86_64/egg/plottwist/libs/pyblish/validators/modeling/general/hierarchy.py
# Compiled at: 2020-04-17 19:10:58
# Size of source mod 2**32: 8658 bytes
    ...
    def process(self, context, plugin):
        if not tpDcc.is_maya():
            ...
            if check_proxy_geo:
                ...
                assert proxy_geo and tpDcc.Dcc.object_exists(proxy_geo), 'Proxy geometry "{}" not found in scene!'.format(proxy_geo)
                assert tpDcc.Dcc.object_exists(proxy_geo_parent), 'Proxy geo parent "{}" does not exists in scene!'.format(proxy_geo_parent)
            ...
            if len(top_transforms) > 1:
                ...
                assert tpDcc.Dcc.object_exists(high_geo_parent), 'High geo parent "{}" does not exists in scene!'.format(high_geo_parent)
...
class ValidatePlotTwistModelingHierarchy(pyblish.api.ContextPlugin):
    """ValidatePlotTwistModelingHierarchy"""
    ...
    def process(self, context):
        if not tpDcc.is_maya():
            raise AssertionError('Validate Modeling Hierarchy is only available in Maya!')
        else:
            from tpDcc.dccs.maya.core import scene
            ...
            top_transforms = scene.get_top_dag_nodes(exclude_cameras=True)
            if len(top_transforms) > 1:
                self.log.warning('Multiple root nodes ({}) found in the scene: {}! Only should be 1 root node (root)'.format(len(top_transforms), top_transforms))
                multiple_roots = True
            root_group_name = artellapipe.NamesMgr().solve_name('root_group')
            ...
            if not create_root:
                if not create_high:
                    ...
                    if parent != root_group_name:
                        self.log.warning('High Group "{}" is not parented to root group "{}"!'.format(high_group_name, root_group_name))
                        parent_high = True
            if not create_high:
            ...
            if not create_root:
                if not create_proxy:
                    parent = tpDcc.Dcc.node_parent(high_group_name)
                    if parent != root_group_name:
                        self.log.warning('Proxy Group "{}" is not parented to root group "{}"!'.format(high_group_name, root_group_name))
                        parent_proxy = True
            if not create_proxy:
            ...
            assert not create_root and not create_high and not create_proxy and not parent_high and not parent_proxy and not create_high_geo and not create_proxy and not multiple_roots and not check_proxy_geo, 'Hierarchy is not valid!'