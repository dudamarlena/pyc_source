...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/cgcloud/toil/test/test_toil.py
# Compiled at: 2016-11-22 15:21:45
import logging, os, tempfile, time, unittest
...
class ToilClusterTests(MesosTestCase):
    ...
    def _create_cluster(self, growth, *args):
        self._cgcloud('create-cluster', 'toil', ('-s=%d' % (num_workers - growth)), '--ssh-opts', self.ssh_opts_str(), *args)
        if growth:
    ...
    def _hello_world(self):
        ...
        def hello_world():
            ...
            if __name__ == '__main__':
                ...
                job = Job.wrapFn(hello, 'world', cores=1, memory=1000000.0, disk=1000000.0)
        ...
        try:
            self._ssh(leader, 'python2.7', script, '--batchSystem=mesos', '--mesosMaster=mesos-master:5050', job_store)
        except:
            with panic(log):
                self._ssh(leader, 'toil', 'clean', job_store)
    ...
    def test_persistence(self):
        ...
        volume_size_gb = 1
        self._create_cluster(0, '--ebs-volume-size', str(volume_size_gb), '-O', 'persist_var_lib_toil=True')
        try:
            try:
                self._wait_for_workers()
            ...
            self._create_cluster(0, '--ebs-volume-size', str(volume_size_gb), '-O', 'persist_var_lib_toil=False')
            try:
                self._wait_for_workers()
                for ordinal in range(num_workers):
