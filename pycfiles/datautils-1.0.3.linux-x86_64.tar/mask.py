...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/np/mask.py
...
def listify(i, n):
    if not islist(i):
        return [i] * n
    assert len(i) == n, 'Attempt to listify list of length %s to length %s' % (len(i), n)
    return i
...
def test_mask_array():
    ...
    assert s(ma(a, (4, 6, 3), ('greater', 'less', 'equal'), combiner=('and', 'and',
                                                                      'or'))) == 2