...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/grouping/coding.py
...
class ContinuousCodec:
    ...
    def __init__(self, code_range=(0, 1.0), value_domain=None, values=None):
    ...
    def __repr__(self):
        return 'ContinuousCodec[values(%g,%g)->codes(%g,%g)]' % (self.value_domain + self.code_range)
...
def discrete_to_codes_from_auto_codec(values, code_min, code_max):
    return discrete_to_codes_from_auto_sorted_codec(values, code_min, code_max, lambda v: v)


def discrete_to_codes_from_auto_sorted_codec(values, code_min, code_max, sort_codec):
    codec = {}
    for v in values:
        codec[v] = 1
...
def discrete_to_codes(values, code_min=0.0, code_max=1.0, codec=None, sort_codec=None):
    if codec is not None:
        ...
        return discrete_to_codes_from_auto_sorted_codec(values, code_min, code_max, sort_codec)
...
def to_codes(values, type='auto', min=0.0, max=1.0, codec=None, sort_codec=True, return_codec=False):
    if not len(values):
        return ([], None)
    else:
        if type == 'auto':
