...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/qfilter/query.py
...
vtests = {'$lt': lambda t: lambda v: v < t, 
   ...
   '$in': lambda t: lambda v: any(vi in t for vi in v) if isinstance(v, (tuple, list)) else v in t, 
   '$nin': lambda t: lambda v: all(vi not in t for vi in v) if isinstance(v, (tuple, list)) else v not in t, 
   '$nor': nor_vtest, 
   '$or': or_vtest, 
   '$and': and_vtest, 
...
def make_value_test(value):
    ...
    if not isinstance(value, dict):
        return lambda v: value in v if isinstance(v, (tuple, list)) else eq(v, value)
...
def test_qfilter():
    items = [{'a': {'b': {'c': 1}}}, {'a': {'b': {'c': 2}}}, {'all': [1, 2, 3]}]
    ...
    assert len(qfilter([{'a': {'b': 'foo'}}], {'a.b': {'$in': ['foo', 'bar']}})) == 1
