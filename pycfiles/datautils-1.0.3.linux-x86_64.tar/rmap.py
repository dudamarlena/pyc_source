...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/rmap.py
# Compiled at: 2015-07-21 09:32:00
"""
...
def parse_mapping(mapping):
    ...
    for k, v in mapping.iteritems():
        ...
        if 'k' not in v:
            raise MappingError('Mapping value missing key[k] [key=%s, value=%s]' % (k, v))
        if 'f' in v:
            if 'q' in v:
    ...
    if asdocs:
        for fk, fv in frs.iteritems():
            if hasattr(fv, '__len__') and not isinstance(fv, (str, unicode)):
                if len(rs) != len(fv):
                    raise MappingError('Function [%s] result length [%s] != doc length [%s]' % (
                     fk, len(fv), len(rs)))
...
def test_remap():
    l = [{'a': 1, 'b': 1}, {'a': 2, 'b': 2}]
    m = {'c': 'a', 
       'd': {'k': 'a', 'f': lambda x: [ i * 2 for i in x ]}, 'e': {'k': 'b', 'q': {'$gt': 1}}}
    r = remap(l, m, asdocs=True)
    assert len(r) == 1
    assert r[0]['c'] == l[1]['a']
    assert r[0]['d'] == l[1]['a'] * 2
    assert r[0]['e'] == l[1]['b']
