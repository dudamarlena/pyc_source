...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/grouping/ops.py
...
def breadth(d, level=0, f=max):
    ...
    if level == 0:
        if not isinstance(d, dict):
            raise GroupingOpsError('breadth can only be measured for dicts not %s' % type(d))
        return len(d.keys())
    else:
        if depth(d) <= level:
            return 0
        return f([ breadth(d[k], level - 1) for k in d.keys() if isinstance(d[k], dict)
                 ])
...
def all_keys(d, level=0):
    if level == 0:
        ...
        if depth(d) <= level:
            return []
        return list(set(reduce(lambda a, b: a + b, [ all_keys(d[k], level - 1) for k in d.keys() if isinstance(d[k], dict)
                                                   ])))
...
def combine(d0, d1, r=None):
    ...
    if d0 is None:
        ...
        for k in set(d0.keys() + d1.keys()):
            r[k] = combine(ddict.ops.tdget(d0, k, None), ddict.ops.tdget(d1, k, None))
...
def prune(gts, *keys):
    ...
    for k in gts.keys():
        if isinstance(gts[k], dict):
            ...
            r[k] = [ dict([ (sk, i[sk]) for sk in i.keys() if sk not in keys ]) for i in gts[k] ]
...
def test_drop_levels():
    ...
    if not c1 == d1:
        ...
        raise (good or Exception)('drop_levels failed to raise an exception')
    c = drop_levels(d, (0, 1))
...
def test_collapse():
    ...
    if not collapse(d, (0, 1)) == d01:
        ...
        raise (good or Exception)('collapse failed to raise an exception')