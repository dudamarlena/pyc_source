...
# [GCC 8.4.0]
# Embedded file name: /home/graham/.virtualenvs/temcagt/lib/python2.7/site-packages/datautils/plot/mapped.py
# Compiled at: 2013-12-13 14:50:04
import numpy, pylab, mpl_toolkits.mplot3d as mplot3d
...
def pfunc(required=None, optional=None, auto=True, shadowkwargs=None):
    ...
    shadowkwargs = ['decorate', 'xt', 'xtl', 'yt', 'ytl', 'zt', 'ztl'] if shadowkwargs is None else shadowkwargs
    ...
    def wrapper(function):
        if auto and not hasattr(pylab, function.__name__):
            raise AttributeError('Autocall failed, pylab has no: %s' % function.__name__)
        ...
        def wrapped(d, mapping, **kwargs):
            if not isinstance(mapping, dict):
                ...
                mapping = isinstance(d, dict) or remap(d, mapping, asdocs=False)
            else:
                for k in mapping:
            ...
            skwargs = dict([ (k, kwargs.pop(k)) for k in shadowkwargs if k in kwargs
                           ])
            if auto:
...
def bin(d, m, *args, **kwargs):
    ax = pylab.gca()
    g = grouping.group(args[0], gtype='d')
...
def get_3d_axes():
    ...
    if not isinstance(ax, mplot3d.Axes3D):
        ...
        f = pylab.gcf()
        f.delaxes(ax)
        ax = f.add_subplot(projection='3d', *axg)
    return ax
