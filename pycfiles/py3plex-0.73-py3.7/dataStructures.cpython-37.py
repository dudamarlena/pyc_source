...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/core/HINMINE/dataStructures.py
...
class HeterogeneousInformationNetwork:
    ...
    def add_label(self, node, label_id, label_name=None):
        ...
        elif label_id in self.labels_by_id:
                ...
                if list_so_far['node_list'][(-1)] != self.basic_type or len(list_so_far['node_list']) == 1:
                    ...
                    for neighbor in schema[current]:
                        ...
                        for key in schema[current][neighbor]:
                            append_to.append({'node_list':list_so_far['node_list'] + [neighbor], 
    ...
    def decompose_from_iterator(self, name, weighing, summing, generator=None, degrees=None, parallel=True, pool=None):
        classes = [lab for lab in self.label_list if lab if len(lab.not_test_members) > 0]
        ...
        for i, label in enumerate(classes):
            label_matrix[(
             [universal_inv[item] for item in label.not_test_members], i)] = 1
