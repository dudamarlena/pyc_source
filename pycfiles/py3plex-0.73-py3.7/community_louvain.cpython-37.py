...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/algorithms/community_detection/community_louvain.py
# Compiled at: 2019-02-24 13:10:35
# Size of source mod 2**32: 19002 bytes
"""
This module implements community detection.
"""
from __future__ import print_function
import array, random, networkx as nx
...
def best_partition(graph, partition=None, weight='weight', resolution=1.0, randomize=False):
...
def generate_dendrogram(graph, part_init=None, weight='weight', resolution=1.0, randomize=False):
...
def __one_level(graph, status, weight_key, resolution, randomize):
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        for node in __randomly(graph.nodes(), randomize):
            ...
            remove_cost = -resolution * neigh_communities.get(com_node, 0) + (status.degrees.get(com_node, 0.0) - status.gdegrees.get(node, 0.0)) * degc_totw
            ...
            for com, dnc in __randomly(neigh_communities.items(), randomize):
                incr = remove_cost + resolution * dnc - status.degrees.get(com, 0.0) * degc_totw
