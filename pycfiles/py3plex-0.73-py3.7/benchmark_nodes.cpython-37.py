...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/py3plex/wrappers/benchmark_nodes.py
# Compiled at: 2019-02-24 13:10:35
# Size of source mod 2**32: 7845 bytes
import numpy, sys
...
import sklearn.utils as skshuffle
...
def benchmark_node_classification(path, core_network, labels_matrix, percent='all'):
    ...
    for train_percent in sorted(all_results.keys()):
        ...
        rdict[train_percent] = (
         numpy.mean(micros), numpy.mean(macros), numpy.std(micros), numpy.std(macros))

    return rdict


def main():
    parser = ArgumentParser('scoring', formatter_class=ArgumentDefaultsHelpFormatter,
    ...
    parser.add_argument('--all', default=False, action='store_true', help='The embeddings are evaluated on all training percents from 10 to 90 when this flag is set to true. By default, only training percents of 10, 50 and 90 are used.')
    ...
    all_results = defaultdict(list)
    if args.all:
        training_percents = numpy.asarray(range(1, 10)) * 0.1
    else:
        training_percents = [
         0.5]
    for train_percent in training_percents:
