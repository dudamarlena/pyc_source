...
# Embedded file name: /usr/lib/python3.5/site-packages/airsign/wallet.py
...
class Wallet(object):
    ...
    def __init__(self, rpc, password=None, *args, **kwargs):
        ...
        if not password:
            ...
            print('No wallet has been created yet. ' + 'Please provide a passphrase for it!')
            while True:
                pw = getpass.getpass('Passphrase: ')
                if not pw:
                    print('You have chosen an empty password! ' + 'We assume you understand the risks!')
                    self._openWallet(pw)
                    break
                else:
