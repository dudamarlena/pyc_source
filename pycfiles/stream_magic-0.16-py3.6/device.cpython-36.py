...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.9-x86_64/egg/stream_magic/device.py
# Compiled at: 2018-06-07 12:16:51
# Size of source mod 2**32: 28415 bytes
"""
...
class StreamMagicDevice:
    __doc__ = ' Representation of a DLNA Media Player (UPnP-AV renderer) device.\n        Provides all the methods to control the device and retrieve\n        information from it.\n    '
    host = None
    port = None
    description = None
    location = None
    ...
    def __init__(self, host, port, description, location, name='Unknown'):
        ...
        for node in root_xml.getElementsByTagName('service'):
            service_type = self._xml_get_node_text(node.getElementsByTagName('serviceType')[0])
            control_url = '%s%s' % (urlbase,
             self._xml_get_node_text(node.getElementsByTagName('controlURL')[0]))
            scpd_url = '%s%s' % (urlbase,
             self._xml_get_node_text(node.getElementsByTagName('SCPDURL')[0]))
            self.services.update({service_type: {'scpdUrl':scpd_url,  'ctrlUrl':control_url}})
    ...
    def _get_scpd(self, scpdurl=None):
        ...
        try:
            ...
            print('Something went wrong fetching the SCPD XML file from %s' % self.host, ex)
    ...
    def _send_cmd(self, action, instanceId=0, service_type=StreamMagic.URN_AVTransport, omitInstanceId=False, **kwargs):
        ...
        soapBody = '<?xml version="1.0" encoding="utf-8"?>\n<s:Envelope xmlns:s="{3}" s:encodingStyle="{0}">\n<s:Body>\n<u:{2} xmlns:u="{1}">\n{4}</u:{2}>\n</s:Body>\n</s:Envelope>\n'.format(StreamMagic.SOAP_ENCODING, service_type, action, StreamMagic.SOAP_ENVELOPE, params)
        headers = {'SOAPACTION':'"%s"' % service_type + '#' + action, 
         'Host':'%s:%s' % (self.host, self.port), 
         'Content-Type':'text/xml; charset="utf-8"', 
         'Accept':'*/*', 
         'Content-Length':len(soapBody)}
        ctrlUrl = self._get_service_data(service_type)['ctrlUrl']
        request = urllib.request.Request(ctrlUrl, str.encode(soapBody), headers)
        try:
    ...
    def _update_actions(self):
        ...
        for service in self.services:
            ...
            for node in xml.getElementsByTagName('action'):
                action = self._xml_get_node_text(node.getElementsByTagName('name')[0])
                for arg in node.getElementsByTagName('argument'):
                    argument = self._xml_get_node_text(arg.getElementsByTagName('name')[0])
                    direction = self._xml_get_node_text(arg.getElementsByTagName('direction')[0])
                    rsv = self._xml_get_node_text(arg.getElementsByTagName('relatedStateVariable')[0])
                    ...
                    self.actions[service][action][argument] = {'direction':direction, 
    ...
    def _get_protocol_info(self):
        ...
        response = self._send_cmd('GetProtocolInfo', omitInstanceId=True,
    ...
    def get_services(self, init=False):
        ...
        if self.actions.keys():
            ...
            return self.get_services(init=(not initialized))
    ...
    def get_mute_state(self):
        ...
        response = self._send_cmd('GetMute', service_type=svc_type, Channel='Master')
    ...
    def volume_mute(self, state=True):
        ...
        svc_type = 'urn:schemas-upnp-org:service:RenderingControl:1'
        response = self._send_cmd('SetMute', service_type=svc_type, Channel='Master',
          DesiredMute=(int(state)))
        return response
    ...
    def get_volume_control(self):
        ...
        response = self._send_cmd('GetVolumeControl', service_type=svc_type, omitInstanceId=True)
        return bool(int(self._get_response_tag_value(response, 'Enabled')))

    def get_volume(self):
        """ Return the current volume setting. """
        svc_type = 'urn:schemas-upnp-org:service:RenderingControl:1'
        response = self._send_cmd('GetVolume', service_type=svc_type, Channel='Master')
    ...
    def set_volume(self, volume):
        ...
        """
        svc_type = 'urn:schemas-upnp-org:service:RenderingControl:1'
        self._send_cmd('SetVolume', service_type=svc_type, Channel='Master', DesiredVolume=volume)
    ...
    def trnsprt_play_pause(self):
        ...
        response = self._send_cmd('KeyPressed', Key='PLAY_PAUSE', Duration='SHORT',
          service_type=svc_type,
          omitInstanceId=True)
        return response
    ...
    def trnsprt_next(self):
        """ Skip to next track. """
        svc_type = 'urn:UuVol-com:service:UuVolSimpleRemote:1'
        response = self._send_cmd('KeyPressed', Key='SKIP_NEXT', Duration='SHORT',
          service_type=svc_type,
          omitInstanceId=True)
        return response
    ...
    def trnsprt_prev(self, press_twice=False):
        ...
        """
        svc_type = 'urn:UuVol-com:service:UuVolSimpleRemote:1'
        response = self._send_cmd('KeyPressed', Key='SKIP_PREVIOUS', Duration='SHORT',
          service_type=svc_type,
          omitInstanceId=True)
        if press_twice:
            self.trnsprt_prev()
    ...
    def get_shuffle(self):
        ...
        response = self._send_cmd('Shuffle', omitInstanceId=True, service_type=svc_type)
    ...
    def set_shuffle(self, state):
        ...
        state = int(state)
        svc_type = 'urn:UuVol-com:service:PlaylistExtension:1'
        self._send_cmd('SetShuffle', aShuffle=state, omitInstanceId=True, service_type=svc_type)

    def get_repeat(self):
        """ Return the state of the repeat function as a boolean. """
        svc_type = 'urn:UuVol-com:service:PlaylistExtension:1'
        response = self._send_cmd('Repeat', omitInstanceId=True, service_type=svc_type)
    ...
    def set_repeat(self, state):
        ...
        state = int(state)
        svc_type = 'urn:UuVol-com:service:PlaylistExtension:1'
        self._send_cmd('SetRepeat', aRepeat=state, omitInstanceId=True, service_type=svc_type)
    ...
    def get_current_track_info(self):
        ...
        if self.get_audio_source() == 'media player':
            f = self._get_response_tag_value
            ...
            data['trackLength'] = minidom.parseString(track_data).getElementsByTagName('res')[0].attributes['duration'].firstChild.data
            data['trackLength'] = data['trackLength'][:-4]
        else:
            for key in ('artist', 'trackTitle', 'albumArtURI', 'genre', 'origTrackNo',
                        'album', 'currentPos', 'trackLength'):
    ...
    def _navigator_release(self, navigator_id):
        ...
        svc_type = 'urn:UuVol-com:service:UuVolControl:5'
        self._send_cmd('ReleaseNavigator', NavigatorId=navigator_id,
          omitInstanceId=True,
          service_type=svc_type)
    ...
    def _navigator_is_registered(self, navigator_id):
        ...
        """
        svc_type = 'urn:UuVol-com:service:UuVolControl:5'
        response = self._send_cmd('IsRegisteredNavigatorId', NavigatorId=navigator_id,
          omitInstanceId=True,
          service_type=svc_type)
    ...
    def _set_av_transport_uri(self, uri):
        ...
        response = self._send_cmd('SetAVTransportURI', CurrentURI=uri,
          CurrentURIMetaData='')
    ...
    def get_preset_list(self):
        ...
        """
        svc_type = 'urn:UuVol-com:service:UuVolControl:5'
        response = self._send_cmd('GetPresetList', Start='1',
          End=(self._get_number_of_presets()),
          omitInstanceId=True,
          service_type=svc_type)
        presetListXML = self._get_response_tag_value(response, 'RetPresetListXML')
        presets = minidom.parseString(presetListXML).getElementsByTagName('preset')
        presetList = []
        for p in presets:
    ...
    def play_preset(self, num):
        ...
        svc_type = 'urn:UuVol-com:service:UuVolControl:5'
        self._send_cmd('PlayPreset', NewPresetNumberValue=num, omitInstanceId=True,
          service_type=svc_type)
    ...
    def get_playback_details(self):
        ...
        if self._pwrstate != 'on':
            ...
            if self.get_transport_state() == 'TRANSITIONING':
                return {'state':'', 
                 'format':'',  'artist':'',  'stream':''}
            else:
                nid = self._navigator_register()
                svc_type = 'urn:UuVol-com:service:UuVolControl:5'
                response = self._send_cmd('GetPlaybackDetails', NavigatorId=nid, omitInstanceId=True,
                  service_type=svc_type)
                ...
                try:
                    pbd = minidom.parseString(pb_details).getElementsByTagName('playback-details')[0]
                    state = pbd.getElementsByTagName('state')[0].firstChild.nodeValue
                    fmt = dict(pbd.getElementsByTagName('format')[0].attributes.items())
                    artist = pbd.getElementsByTagName('artist')[0].firstChild.nodeValue
                    stream = pbd.getElementsByTagName('stream')[0].getElementsByTagName('title')[0].firstChild.nodeValue
                except IndexError:
                    pass

            data = {'state':state, 
             'format':fmt,  'artist':artist, 
             'stream':stream}
            return data
    ...
    def power_on(self):
        ...
        """
        svc_type = 'urn:UuVol-com:service:UuVolControl:5'
        response = self._send_cmd('SetPowerState', NewPowerStateValue='ON', service_type=svc_type,
          omitInstanceId=True)
        self._pwrstate = 'on'
        return response
    ...
    def power_off(self, power_state='OFF'):
        ...
        if power_state not in ('OFF', 'IDLE'):
            ...
            svc_type = 'urn:UuVol-com:service:UuVolControl:5'
            response = self._send_cmd('SetPowerState', NewPowerStateValue=power_state,
              service_type=svc_type,
              omitInstanceId=True)
