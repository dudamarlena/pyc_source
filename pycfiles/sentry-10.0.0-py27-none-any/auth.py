...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/auth.py
...
def get_auth_providers():
    return [ key for key, cfg_names in six.iteritems(settings.AUTH_PROVIDERS) if all(getattr(settings, c, None) for c in cfg_names)
           ]
...
def login(request, user, passed_2fa=None, after_2fa=None, organization_id=None, source=None):
    ...
    if has_2fa and not passed_2fa:
        request.session['_pending_2fa'] = [
         user.id, time(), organization_id]
        if after_2fa is not None:
...
def log_auth_success(request, username, organization_id=None, source=None):
    logger.info('user.auth.success', extra={'ip_address': request.META['REMOTE_ADDR'], 
       'username': username, 
...
def log_auth_failure(request, username=None):
    logger.info('user.auth.fail', extra={'ip_address': request.META['REMOTE_ADDR'], 'username': username})
