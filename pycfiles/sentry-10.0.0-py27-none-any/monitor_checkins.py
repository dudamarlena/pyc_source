...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/monitor_checkins.py
...
class MonitorCheckInsEndpoint(MonitorEndpoint):
    ...
    def get(self, request, project, monitor):
        ...
        return self.paginate(request=request, queryset=queryset, order_by='-date_added', on_results=lambda x: serialize(x, request.user), paginator_cls=OffsetPaginator)
    ...
    def post(self, request, project, monitor):
        ...
        serializer = CheckInSerializer(data=request.data, context={'project': project, 'request': request})
        if not serializer.is_valid():
            return self.respond(serializer.errors, status=400)
        result = serializer.validated_data
        with transaction.atomic():
            checkin = MonitorCheckIn.objects.create(project_id=project.id, monitor_id=monitor.id, duration=result.get('duration'), status=getattr(CheckInStatus, result['status'].upper()))
            if checkin.status == CheckInStatus.ERROR and monitor.status != MonitorStatus.DISABLED:
                ...
                monitor_params = {'last_checkin': checkin.date_added, 
                ...
                Monitor.objects.filter(id=monitor.id).exclude(last_checkin__gt=checkin.date_added).update(**monitor_params)
