...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/authentication.py
...
class RelayAuthentication(BasicAuthentication):
    ...
    def authenticate_credentials(self, relay_id, relay_sig, request):
        ...
        try:
            data = relay.public_key_object.unpack(request.body, relay_sig, max_age=300)
            request.relay = relay
...
class ClientIdSecretAuthentication(QuietBasicAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            return (
             application.sentry_app.proxy_user, None)
        except Exception:
            raise invalid_pair_error
...
class TokenAuthentication(StandardAuthentication):
    ...
    def authenticate_credentials(self, request, token_str):
        ...
        try:
            token = token or ApiToken.objects.filter(token=token_str).select_related('user', 'application').get()
