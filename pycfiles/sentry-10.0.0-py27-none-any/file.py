...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/models/file.py
# Compiled at: 2019-09-06 12:12:44
from __future__ import absolute_import
import os, six, mmap, tempfile
...
ONE_DAY = 86400
UPLOAD_RETRY_TIME = getattr(settings, 'SENTRY_UPLOAD_RETRY_TIME', 60)
DEFAULT_BLOB_SIZE = 1048576
...
class FileBlob(Model):
    ...
    def deletefile(self, commit=False):
        assert self.path
        delete_file_task.apply_async(kwargs={'path': self.path, 'checksum': self.checksum}, countdown=60)
        self.path = None
        if commit:
            self.save()
        return
...
class File(Model):
    ...
    def _get_chunked_blob(self, mode=None, prefetch=False, prefetch_to=None, delete=True):
        return ChunkedFileBlobIndexWrapper(FileBlobIndex.objects.filter(file=self).select_related('blob').order_by('offset'), mode=mode, prefetch=prefetch, prefetch_to=prefetch_to, delete=delete)
