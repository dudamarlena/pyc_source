...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/integrations/slack/notify_action.py
...
class SlackNotifyServiceForm(forms.Form):
    ...
    def clean(self):
        ...
        if channel_id is None and workspace is not None:
            params = {'channel': channel, 'workspace': dict(self.fields['workspace'].choices).get(int(workspace))}
            raise forms.ValidationError(_('The slack resource "%(channel)s" does not exist or has not been granted access in the %(workspace)s Slack workspace.'), code='invalid', params=params)
...
class SlackNotifyServiceAction(EventAction):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form_fields = {'workspace': {'type': 'choice', 
                         'choices': [ (i.id, i.name) for i in self.get_integrations() ]}, 
           'channel': {'type': 'string', 'placeholder': 'i.e #critical'}, 'tags': {'type': 'string', 'placeholder': 'i.e environment,user,my_tag'}}
    ...
    def after(self, event, state):
        ...
        try:
            integration = Integration.objects.get(provider='slack', organizations=self.project.organization, id=integration_id)
        ...
        def send_notification(event, futures):
            ...
            payload = {'token': integration.metadata['access_token'], 
               'channel': channel, 
        ...
        key = ('slack:{}:{}').format(integration_id, channel)
        metrics.incr('notifications.sent', instance='slack.notification', skip_internal=False)
        yield self.future(send_notification, key=key)

    def render_label(self):
        try:
            integration_name = Integration.objects.get(provider='slack', organizations=self.project.organization, id=self.get_option('workspace')).name
        except Integration.DoesNotExist:
            integration_name = '[removed]'

        tags = self.get_tags_list()
        return self.label.format(workspace=integration_name, channel=self.get_option('channel'), tags=('[{}]').format((', ').join(tags)))
    ...
    def get_form_instance(self):
        return self.form_cls(self.data, integrations=self.get_integrations(), channel_transformer=self.get_channel_id)
    ...
    def get_channel_id(self, integration_id, name):
        ...
        channels_payload = dict(token_payload, **{'exclude_archived': False, 'exclude_members': True})
