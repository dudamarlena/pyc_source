...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/team_projects.py
...
def list_team_projects_scenario(runner):
    runner.request(method='GET', path='/teams/%s/%s/projects/' % (runner.org.slug, runner.default_team.slug))


@scenario('CreateNewProject')
def create_project_scenario(runner):
    runner.request(method='POST', path='/teams/%s/%s/projects/' % (runner.org.slug, runner.default_team.slug), data={'name': 'The Spoiled Yoghurt'})


class ProjectSerializer(serializers.Serializer):
    name = serializers.CharField(max_length=64, required=True)
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50, required=False, allow_null=True)
...
class TeamProjectsEndpoint(TeamEndpoint, EnvironmentMixin):
    ...
    def post(self, request, team):
        ...
        if serializer.is_valid():
            ...
            self.create_audit_entry(request=request, organization=team.organization, target_object=project.id, event=AuditLogEntryEvent.PROJECT_ADD, data=project.get_audit_log_data())
