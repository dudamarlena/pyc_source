...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/discover/endpoints/discover_query.py
...
class DiscoverQueryEndpoint(OrganizationEndpoint):
    ...
    def handle_results(self, snuba_results, requested_query, projects):
        if 'project.name' in requested_query['selected_columns']:
            ...
            snuba_results['meta'].insert(project_name_index, {'name': 'project.name', 'type': 'String'})
            if 'project.id' not in requested_query['selected_columns']:
                snuba_results['meta'] = [ field for field in snuba_results['meta'] if field['name'] != 'project.id' ]
    ...
    def do_query(self, projects, request, **kwargs):
        ...
        if not kwargs['aggregations']:
            ...
            return self.paginate(request=request, on_results=lambda results: self.handle_results(results, requested_query, projects), paginator=GenericOffsetPaginator(data_fn=data_fn), max_per_page=1000)
        else:
            snuba_results = snuba.transform_aliases_and_query(referrer='discover', **kwargs)
            return Response(self.handle_results(snuba_results, requested_query, projects), status=200)

    def post(self, request, organization):
        if not features.has('organizations:discover', organization, actor=request.user):
            return Response(status=404)
        requested_projects = request.data['projects']
        projects = list(Project.objects.filter(id__in=requested_projects, organization=organization, status=ProjectStatus.VISIBLE))
        ...
        return self.do_query(projects=projects_map, start=serialized.get('start'), end=serialized.get('end'), groupby=groupby, selected_columns=selected_columns, conditions=serialized.get('conditions'), orderby=serialized.get('orderby'), limit=serialized.get('limit'), aggregations=serialized.get('aggregations'), rollup=serialized.get('rollup'), filter_keys={'project.id': serialized.get('projects')}, arrayjoin=serialized.get('arrayjoin'), request=request, turbo=serialized.get('turbo'))