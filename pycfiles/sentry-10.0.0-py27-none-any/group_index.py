...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/helpers/group_index.py
...
from sentry.models import Activity, Commit, Group, GroupAssignee, GroupHash, GroupLink, GroupStatus, GroupTombstone, GroupResolution, GroupBookmark, GroupSeen, GroupShare, GroupSnooze, GroupSubscription, GroupSubscriptionReason, Release, Repository, TOMBSTONE_FIELDS_FROM_GROUP, Team, User, UserOption
from sentry.models.group import looks_like_short_id
from sentry.api.issue_search import convert_query_values, InvalidSearchQuery, parse_search_query
from sentry.signals import issue_deleted, issue_ignored, issue_resolved, advanced_search_feature_gated
    ...
    for search_filter in search_filters:
        for feature_condition, feature_name in advanced_search_features:
            if feature_condition(search_filter):
                advanced_search_feature_gated.send_robust(user=user, organization=organization, sender=validate_search_filter_permissions)
                raise ValidationError(('You need access to the advanced search feature to use {}').format(feature_name))
...
STATUS_CHOICES = {'resolved': GroupStatus.RESOLVED, 
...
class StatusDetailsValidator(serializers.Serializer):
    ...
    ignoreWindow = serializers.IntegerField(max_value=10080)
    ignoreUserCount = serializers.IntegerField()
    ignoreUserWindow = serializers.IntegerField(max_value=10080)
...
class GroupValidator(serializers.Serializer):
    ...
    def validate_assignedTo(self, value):
        if value and value.type is User and not self.context['project'].member_set.filter(user_id=value.id).exists():
            raise serializers.ValidationError('Cannot assign to non-team member')
        if value and value.type is Team and not self.context['project'].teams.filter(id=value.id).exists():
            raise serializers.ValidationError('Cannot assign to a team without access to the project')
        return value
...
def handle_discard(request, group_list, projects, user):
    ...
    for group in group_list:
        with transaction.atomic():
            try:
                tombstone = GroupTombstone.objects.create(previous_group_id=group.id, actor_id=(user.id if user else None), **{name:getattr(group, name) for name in TOMBSTONE_FIELDS_FROM_GROUP})
...
def _delete_groups(request, project, group_list, delete_type):
    ...
    Group.objects.filter(id__in=group_ids).exclude(status__in=[
     GroupStatus.PENDING_DELETION, GroupStatus.DELETION_IN_PROGRESS]).update(status=GroupStatus.PENDING_DELETION)
    ...
    delete_groups_task.apply_async(kwargs={'object_ids': group_ids, 
       'transaction_id': transaction_id, 
       'eventstream_state': eventstream_state}, countdown=3600)
    for group in group_list:
        create_audit_entry(request=request, transaction_id=transaction_id, logger=audit_logger, organization_id=project.organization_id, target_object=group.id)
        delete_logger.info('object.delete.queued', extra={'object_id': group.id, 
           'transaction_id': transaction_id, 
...
def delete_groups(request, projects, organization_id, search_fn):
    ...
    if group_ids:
        group_list = list(Group.objects.filter(project__in=projects, project__organization_id=organization_id, id__in=set(group_ids)).exclude(status__in=[
...
def self_subscribe_and_assign_issue(acting_user, group):
    if acting_user:
        GroupSubscription.objects.subscribe(user=acting_user, group=group, reason=GroupSubscriptionReason.status_change)
        self_assign_issue = UserOption.objects.get_value(user=acting_user, key='self_assign_issue', default='0')
...
def update_groups(request, projects, organization_id, search_fn):
    group_ids = request.GET.getlist('id')
    if group_ids:
        group_list = Group.objects.filter(project__organization_id=organization_id, project__in=projects, id__in=group_ids)
        group_ids = [ g.id for g in group_list ]
        if not group_ids:
            return Response(status=204)
    else:
    ...
    if discard:
        ...
        if status in ('resolved', 'resolvedInNextRelease'):
            if status == 'resolvedInNextRelease' or statusDetails.get('inNextRelease'):
                if len(projects) > 1:
                    return Response({'detail': 'Cannot set resolved in next release for multiple projects.'}, status=400)
                release = statusDetails.get('inNextRelease') or Release.objects.filter(projects=projects[0], organization_id=projects[0].organization_id).extra(select={'sort': 'COALESCE(date_released, date_added)'}).order_by('-sort')[0]
                ...
                if statusDetails.get('inRelease'):
                    if len(projects) > 1:
                        return Response({'detail': 'Cannot set resolved in release for multiple projects.'}, status=400)
                    release = statusDetails['inRelease']
                    activity_type = Activity.SET_RESOLVED_IN_RELEASE
                    activity_data = {'version': release.version}
                    status_details = {'inRelease': release.version, 
                       'actor': serialize(extract_lazy_object(request.user), request.user)}
                    ...
                    if len(projects) > 1:
                        return Response({'detail': 'Cannot set resolved in commit for multiple projects.'}, status=400)
                    commit = statusDetails['inCommit']
                    activity_type = Activity.SET_RESOLVED_IN_COMMIT
                    activity_data = {'commit': commit.id}
                    status_details = {'inCommit': serialize(commit, request.user), 
                       'actor': serialize(extract_lazy_object(request.user), request.user)}
                ...
                if commit and not release:
                    try:
                        release = Release.objects.filter(projects__in=projects, releasecommit__commit=commit).extra(select={'sort': 'COALESCE(date_released, date_added)'}).order_by('-sort')[0]
                        res_type = GroupResolution.Type.in_release
                        res_status = GroupResolution.Status.resolved
                    except IndexError:
                        release = None
                ...
                for group in group_list:
                    with transaction.atomic():
                        resolution = None
                        if release:
                            resolution_params = {'release': release, 'type': res_type, 
                               'status': res_status, 
                               'actor_id': request.user.id if request.user.is_authenticated() else None}
                            resolution, created = GroupResolution.objects.get_or_create(group=group, defaults=resolution_params)
                            if not created:
                                resolution.update(datetime=timezone.now(), **resolution_params)
                        if commit:
                            GroupLink.objects.create(group_id=group.id, project_id=group.project_id, linked_type=GroupLink.LinkedType.commit, relationship=GroupLink.Relationship.resolves, linked_id=commit.id)
                        affected = Group.objects.filter(id=group.id).update(status=GroupStatus.RESOLVED, resolved_at=now)
                        if not resolution:
                            created = affected
                        ...
                        if created:
                            activity = Activity.objects.create(project=project_lookup[group.project_id], group=group, type=activity_type, user=acting_user, ident=resolution.id if resolution else None, data=activity_data)
                            if not is_bulk:
                                activity.send_notification()
                    issue_resolved.send_robust(organization_id=organization_id, user=acting_user or request.user, group=group, project=project_lookup[group.project_id], resolution_type=res_type_str, sender=update_groups)
            ...
            with transaction.atomic():
                ...
                if new_status == GroupStatus.IGNORED:
                    ...
                    ignore_duration = statusDetails.pop('ignoreDuration', None) or statusDetails.pop('snoozeDuration', None) or None
                    ...
                    if ignore_duration or ignore_count or ignore_user_count:
                        ...
                        for group in group_list:
                            ...
                            GroupSnooze.objects.create_or_update(group=group, values={'until': ignore_until, 
                               'count': ignore_count, 
                               ...
                               'state': state, 
                               'actor_id': request.user.id if request.user.is_authenticated() else None})
                            result['statusDetails'] = {'ignoreCount': ignore_count, 
            ...
            if group_list and happened:
                if new_status == GroupStatus.UNRESOLVED:
                    ...
                    if new_status == GroupStatus.IGNORED:
                        ...
                        activity_data = {'ignoreCount': ignore_count, 
                    ...
                    for project in projects:
                        ...
                        if project_groups:
                            issue_ignored.send_robust(project=project, user=acting_user, group_list=project_groups, activity_data=activity_data, sender=update_groups)

                    for group in group_list:
                        group.status = new_status
                        activity = Activity.objects.create(project=project_lookup[group.project_id], group=group, type=activity_type, user=acting_user, data=activity_data)
                        if not is_bulk:
                            if acting_user:
                                GroupSubscription.objects.subscribe(user=acting_user, group=group, reason=GroupSubscriptionReason.status_change)
                            activity.send_notification()
        ...
        if 'assignedTo' in result:
            ...
            if assigned_actor:
                ...
                result['assignedTo'] = serialize(assigned_actor.resolve(), acting_user, ActorSerializer())
            else:
                for group in group_list:
                    GroupAssignee.objects.deassign(group, acting_user)

        is_member_map = {project.id:project.member_set.filter(user=acting_user).exists() for project in projects}
        if result.get('hasSeen'):
            for group in group_list:
                if is_member_map.get(group.project_id):
                    instance, created = create_or_update(GroupSeen, group=group, user=acting_user, project=project_lookup[group.project_id], values={'last_seen': timezone.now()})

        elif result.get('hasSeen') is False:
            GroupSeen.objects.filter(group__in=group_ids, user=acting_user).delete()
        if result.get('isBookmarked'):
            for group in group_list:
                GroupBookmark.objects.get_or_create(project=project_lookup[group.project_id], group=group, user=acting_user)
                GroupSubscription.objects.subscribe(user=acting_user, group=group, reason=GroupSubscriptionReason.bookmark)
        ...
        if result.get('isSubscribed') in (True, False):
            ...
            for group in group_list:
                GroupSubscription.objects.create_or_update(user=acting_user, group=group, project=project_lookup[group.project_id], values={'is_active': is_subscribed, 'reason': GroupSubscriptionReason.unknown})

            result['subscriptionDetails'] = {'reason': SUBSCRIPTION_REASON_MAP.get(GroupSubscriptionReason.unknown, 'unknown')}
        if 'isPublic' in result:
            for group in group_list:
                if GroupShare.objects.filter(group=group).delete():
                    result['shareId'] = None
                    Activity.objects.create(project=project_lookup[group.project_id], group=group, type=Activity.SET_PRIVATE, user=acting_user)

        if result.get('isPublic'):
            for group in group_list:
                share, created = GroupShare.objects.get_or_create(project=project_lookup[group.project_id], group=group, user=acting_user)
                if created:
                    result['shareId'] = share.uuid
                    Activity.objects.create(project=project_lookup[group.project_id], group=group, type=Activity.SET_PUBLIC, user=acting_user)
        ...
        if result.get('merge') and len(group_list) > 1:
            ...
            eventstream_state = eventstream.start_merge(primary_group.project_id, group_ids_to_merge, primary_group.id)
            Group.objects.filter(id__in=group_ids_to_merge).update(status=GroupStatus.PENDING_MERGE)
            transaction_id = uuid4().hex
            merge_groups.delay(from_object_ids=group_ids_to_merge, to_object_id=primary_group.id, transaction_id=transaction_id, eventstream_state=eventstream_state)
            Activity.objects.create(project=project_lookup[primary_group.project_id], group=primary_group, type=Activity.MERGE, user=acting_user, data={'issues': [ {'id': c.id} for c in groups_to_merge ]})
            result['merge'] = {'parent': six.text_type(primary_group.id), 
