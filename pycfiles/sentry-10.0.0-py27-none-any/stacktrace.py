...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/interfaces/stacktrace.py
...
def is_newest_frame_first(event):
    ...
    if env.request and env.request.user.is_authenticated():
        display = UserOption.objects.get_value(user=env.request.user, key='stacktrace_order', default=None)
        if display == '1':
            newest_first = False
        elif display == '2':
            newest_first = True
...
class Frame(Interface):
    ...
    def to_python(cls, data, raw=False):
        for key in ('abs_path', 'colno', 'context_line', 'data', 'errors', 'filename',
                    'function', 'raw_function', 'image_addr', 'in_app', 'instruction_addr',
                    'lineno', 'module', 'package', 'platform', 'post_context', 'pre_context',
                    'symbol', 'symbol_addr', 'trust', 'vars'):
            data.setdefault(key, None)

        return cls(**data)

    def to_json(self):
        return prune_empty_keys({'abs_path': self.abs_path or None, 
    ...
    def get_api_context(self, is_public=False, pad_addr=None, platform=None):
        ...
        data = {'filename': self.filename, 
           ...
           'symbol': self.symbol, 
           'context': get_context(lineno=self.lineno, context_line=self.context_line, pre_context=self.pre_context, post_context=self.post_context), 
           'lineNo': self.lineno, 
           'colNo': self.colno, 
           'inApp': self.in_app, 
           'trust': self.trust, 
        ...
        if self.data and 'sourcemap' in data:
            data.update({'map': self.data['sourcemap'].rsplit('/', 1)[(-1)], 
    ...
    def get_meta_context(self, meta, is_public=False, platform=None):
        ...
        return {'filename': meta.get('filename'), 
           ...
           'context': get_context(lineno=meta.get('lineno'), context_line=meta.get('context_line'), pre_context=meta.get('pre_context'), post_context=meta.get('post_context')), 
    ...
    def is_unhashable_module(self, platform):
        if platform == 'javascript' and '/' in self.module and self.abs_path and self.abs_path.endswith(self.module):
            return True
        if platform == 'java' and '$$Lambda$' in self.module:
            return True
        return False
    ...
    def to_string(self, event):
        ...
        return render_to_string(templates, {'abs_path': self.abs_path, 
           'filename': self.filename, 
           'function': self.function, 
           'module': self.module, 
           'lineno': self.lineno, 
           'colno': self.colno, 
           'context_line': self.context_line}).strip('\n')
...
class Stacktrace(Interface):
    ...
    def to_json(self):
        return prune_empty_keys({'frames': [ f and f.to_json() for f in self.frames ] or None, 'frames_omitted': self.frames_omitted, 
           'registers': self.registers})
