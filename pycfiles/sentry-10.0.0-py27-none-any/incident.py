...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/serializers/models/incident.py
...
from sentry.incidents.models import Incident, IncidentGroup, IncidentProject, IncidentSeen, IncidentSubscription
...
class IncidentSerializer(Serializer):
    ...
    def get_attrs(self, item_list, user, **kwargs):
        ...
        for incident, stats in zip(item_list, bulk_get_incident_stats(item_list)):
            results[incident] = {'projects': incident_projects.get(incident.id, []), 'event_stats': stats['event_stats'], 
...
class DetailedIncidentSerializer(IncidentSerializer):
    ...
    def get_attrs(self, item_list, user, **kwargs):
        ...
        if user.is_authenticated():
            subscribed_incidents = set(IncidentSubscription.objects.filter(incident__in=item_list, user=user).values_list('incident_id', flat=True))
        incident_groups = defaultdict(list)
        for incident_id, group_id in IncidentGroup.objects.filter(incident__in=item_list).values_list('incident_id', 'group_id'):
    ...
    def _get_incident_seen_list(self, incident, user):
        incident_seen = list(IncidentSeen.objects.filter(incident=incident).select_related('user').order_by('-last_seen'))
        seen_by_list = []
        has_seen = False
