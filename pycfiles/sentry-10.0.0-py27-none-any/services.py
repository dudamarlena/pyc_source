...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/services.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import
import functools, inspect, itertools, logging, threading, six
...
class LazyServiceWrapper(LazyObject):
    ...
    def __init__(self, backend_base, backend_path, options, dangerous=(), metrics_path=None):
        ...
        self.__dict__.update({'_backend': backend_path, 
           '_options': options, 
           '_base': backend_base, 
    ...
    def __getattr__(self, name):
        ...
        if self._metrics_path and callable(attr) and name in self._base.__all__:
            return metrics.wraps(self._metrics_path, instance=name, tags={'backend': self._backend})(attr)
        return attr
    ...
    def _setup(self):
        ...
        if backend in self._dangerous:
            warnings.warn(warnings.UnsupportedBackend(('The {!r} backend for {} is not recommended for production use.').format(self._backend, self._base)))
        ...
        def execute(*args, **kwargs):
            ...
            if self.__backend_base in context.backends:
                ...
                if selected_backend_names[0] not in self.__backends:
                    raise self.InvalidBackend(('{!r} is not a registered backend.').format(selected_backend_names[0]))
                        ...
                        try:
                            ...
                            if not is_primary:
                                ...
                                if not expected_raises or not isinstance(e, tuple(expected_raises)):
                                    logger.warning('%s caught in executor while calling %r on %s.', type(e).__name__, attribute_name, type(backend).__name__, exc_info=True)
                            raise
                ...
                results = [
                 None] * len(selected_backend_names)
                for i, backend_name in enumerate(selected_backend_names[1:], 1):
                    try:
                        backend, executor = self.__backends[backend_name]
                    except KeyError:
                        logger.warning('%r is not a registered backend and will be ignored.', backend_name, exc_info=True)
                    else:
                        results[i] = executor.submit(functools.partial(call_backend_method, context.copy(), backend, is_primary=False), priority=1, block=False)

                backend, executor = self.__backends[selected_backend_names[0]]
                results[0] = executor.submit(functools.partial(call_backend_method, context.copy(), backend, is_primary=True), priority=0, block=True)
                if self.__callback_func is not None:
                    FutureSet(filter(None, results)).add_done_callback(lambda *a, **k: self.__callback_func(context, attribute_name, callargs, selected_backend_names, results))
