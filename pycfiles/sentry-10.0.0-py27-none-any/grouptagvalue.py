...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tagstore/v2/models/grouptagvalue.py
...
from sentry.db.models import Model, BoundedPositiveIntegerField, BoundedBigIntegerField, FlexibleForeignKey, sane_repr
...
class GroupTagValue(Model):
    ...
    def value(self):
        ...
        try:
            tv = TagValue.objects.filter(project_id=self.project_id, id=self._value_id).values_list('value', flat=True).get()
    ...
    def merge_counts(self, new_group):
        try:
            with transaction.atomic(using=router.db_for_write(GroupTagValue)):
                new_obj, _ = GroupTagValue.objects.get_or_create(group_id=new_group.id, project_id=new_group.project_id, _key_id=self._key_id, _value_id=self._value_id)
                GroupTagValue.objects.filter(id=new_obj.id, project_id=new_group.project_id).update(first_seen=min(new_obj.first_seen, self.first_seen), last_seen=max(new_obj.last_seen, self.last_seen), times_seen=new_obj.times_seen + self.times_seen)
        except DataError:
            pass
