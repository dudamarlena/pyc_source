...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/organization_auth_settings.py
...
class AuthProviderSettingsForm(forms.Form):
    require_link = forms.BooleanField(label=_('Require SSO'), help_text=_('Require members use a valid linked SSO account to access this organization'), required=False)
    default_role = forms.ChoiceField(label=_('Default Role'), choices=roles.get_choices(), help_text=_('The default role new members will receive when logging in for the first time.'))
...
class OrganizationAuthSettingsView(OrganizationView):
    ...
    def _disable_provider(self, request, organization, auth_provider):
        self.create_audit_entry(request, organization=organization, target_object=auth_provider.id, event=AuditLogEntryEvent.SSO_DISABLE, data=auth_provider.get_audit_log_data())
        OrganizationMember.objects.filter(organization=organization).update(flags=F('flags').bitand(~OrganizationMember.flags['sso:linked']).bitand(~OrganizationMember.flags['sso:invalid']))
    ...
    def handle_existing_provider(self, request, organization, auth_provider):
        ...
        if request.method == 'POST':
            ...
            if op == 'reinvite':
                ...
                return self.redirect(next_uri)
        form = AuthProviderSettingsForm(data=request.POST if request.POST.get('op') == 'settings' else None, initial={'require_link': not auth_provider.flags.allow_unlinked, 
        ...
        if form.is_valid():
            ...
            if form.initial != form.cleaned_data:
                ...
                self.create_audit_entry(request, organization=organization, target_object=auth_provider.id, event=AuditLogEntryEvent.SSO_EDIT, data=changed_data)
        ...
        if isinstance(response, HttpResponse):
            ...
            if isinstance(response, Response):
                response = response.render(request, {'auth_provider': auth_provider, 
    ...
    def handle(self, request, organization):
        try:
            ...
            if requires_feature and not features.has(requires_feature, organization, actor=request.user) and not is_active_superuser(request):
                home_url = organization.get_url()
                messages.add_message(request, messages.ERROR, ERR_NO_SSO)
                return HttpResponseRedirect(home_url)
            return self.handle_existing_provider(request=request, organization=organization, auth_provider=auth_provider)
