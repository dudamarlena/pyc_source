...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_activity.py
...
class OrganizationActivityEndpoint(OrganizationMemberEndpoint, EnvironmentMixin):
    ...
    def get(self, request, organization, member):
        queryset = Activity.objects.filter(project__in=Project.objects.filter(organization=organization, teams__in=OrganizationMemberTeam.objects.filter(organizationmember=member).values('team'))).exclude(type=Activity.UNMERGE_SOURCE).select_related('project', 'group', 'user')
