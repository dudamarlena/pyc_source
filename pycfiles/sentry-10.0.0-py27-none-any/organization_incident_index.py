...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_incident_index.py
...
class IncidentSerializer(serializers.Serializer):
    ...
    def validate_groups(self, group_ids):
        groups = Group.objects.filter(project__organization=self.context['organization'], id__in=group_ids).select_related('project')
...
class OrganizationIncidentIndexEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        return self.paginate(request, queryset=incidents, order_by='-date_started', paginator_cls=OffsetPaginator, on_results=lambda x: serialize(x, request.user), default_per_page=25)
    ...
    def post(self, request, organization):
        ...
        if serializer.is_valid():
            ...
            incident = create_incident(organization=organization, type=IncidentType.CREATED, title=result['title'], query=result.get('query', ''), date_started=result.get('dateStarted'), date_detected=result.get('dateDetected'), projects=result['projects'], groups=groups, user=request.user)
