...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/relay_register.py
...
from semaphore import create_register_challenge, validate_register_response, get_register_response_relay_id, PublicKey

class RelayIdSerializer(serializers.Serializer):
    relay_id = serializers.RegexField('^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$', required=True)
...
def is_internal_relay(request, public_key):
    ...
    """
    if settings.DEBUG or request.META.get('REMOTE_ADDR', None) in settings.INTERNAL_IPS or public_key in settings.SENTRY_RELAY_WHITELIST_PK:
        return True
    return False
...
class RelayRegisterChallengeEndpoint(Endpoint):
    ...
    def post(self, request):
        ...
        if not settings.SENTRY_RELAY_OPEN_REGISTRATION and not is_internal_relay(request, json_data.get('public_key')):
            return Response({'detail': 'Relay is not allowed to register'}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        default_cache.set('relay-auth:%s' % relay_id, {'token': challenge['token'], 'public_key': six.text_type(challenge['public_key'])}, 60)
        return Response(serialize({'relay_id': six.text_type(challenge['relay_id']), 'token': challenge['token']}))
    ...
    def post(self, request):
        ...
        if not serializer.is_valid():
            ...
            try:
                ...
                relay = Relay.objects.create(relay_id=relay_id, public_key=params['public_key'], is_internal=is_internal)
            else:
