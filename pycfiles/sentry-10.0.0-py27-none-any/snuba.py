...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/snuba.py
...
import os, pytz, re, six, time, urllib3
from concurrent.futures import ThreadPoolExecutor
from django.conf import settings
from sentry import quotas
from sentry.models import Environment, Group, GroupRelease, Organization, Project, Release, ReleaseProject
from sentry.net.http import connection_from_url
from sentry.utils import metrics, json
from sentry.utils.dates import to_timestamp
MAX_ISSUES = 500
MAX_HASHES = 5000
SAFE_FUNCTION_RE = re.compile('-?[a-zA-Z_][a-zA-Z0-9_]*$')
QUOTED_LITERAL_RE = re.compile("^'.*'$")
OVERRIDE_OPTIONS = {'consistent': os.environ.get('SENTRY_SNUBA_CONSISTENT', 'false').lower() in ('true', '1')}
SENTRY_SNUBA_MAP = {'id': 'event_id', 
...
clickhouse_error_codes_map = {43: QueryIllegalTypeOfArgument, 
...
_snuba_pool = connection_from_url(settings.SENTRY_SNUBA, retries=urllib3.Retry(total=5, method_whitelist={
 'GET', 'POST'}), timeout=30, maxsize=10)
...
def get_function_index(column_expr, depth=0):
    ...
    if isinstance(column_expr, (tuple, list)):
        ...
        while i < len(column_expr) - 1:
            if isinstance(column_expr[i], six.string_types) and isinstance(column_expr[(i + 1)], (tuple, list)):
...
def parse_columns_in_functions(col, context=None, index=None):
    ...
    elif context is not None and index is not None:
        context[index] = get_snuba_column_name(col)
    return


def get_arrayjoin(column):
    match = re.match('^(exception_stacks|exception_frames|contexts)\\..+$', column)
    if match:
...
def transform_aliases_and_query(skip_conditions=False, **kwargs):
    ...
    if orderby:
        ...
        for field_with_order in orderby:
            ...
            translated_orderby.append(('{}{}').format('-' if field_with_order.startswith('-') else '', field if field in derived_columns else get_snuba_column_name(field)))
    ...
    if len(translated_columns):
        ...
        if rollup and rollup > 0:
            result['data'] = zerofill(result['data'], kwargs['start'], kwargs['end'], kwargs['rollup'], kwargs['orderby'])
    return result
...
def _prepare_query_params(query_params):
    ...
    with timer('get_snuba_map'):
        forward, reverse = get_snuba_translators(query_params.filter_keys, is_grouprelease=query_params.is_grouprelease)
    if 'project_id' in query_params.filter_keys:
        project_ids = list(set(query_params.filter_keys['project_id']))
    else:
        if query_params.filter_keys:
            with timer('get_related_project_ids'):
                ids = [ get_related_project_ids(k, query_params.filter_keys[k]) for k in query_params.filter_keys
                      ]
    ...
    query_params.kwargs.update({'from_date': start.isoformat(), 
       'to_date': end.isoformat(), 
       'groupby': query_params.groupby, 
       'conditions': query_params.conditions, 
       'aggregations': query_params.aggregations, 
       'project': project_ids, 
       'granularity': query_params.rollup})
    kwargs = {k:v for k, v in six.iteritems(query_params.kwargs) if v is not None if v is not None}
...
class SnubaQueryParams(object):
    ...
    def __init__(self, start=None, end=None, groupby=None, conditions=None, filter_keys=None, aggregations=None, rollup=None, referrer=None, is_grouprelease=False, **kwargs):
        self.start = start or datetime.utcfromtimestamp(0)
...
def raw_query(start=None, end=None, groupby=None, conditions=None, filter_keys=None, aggregations=None, rollup=None, referrer=None, is_grouprelease=False, **kwargs):
    """
    Sends a query to snuba.  See `SnubaQueryParams` docstring for param
    descriptions.
    """
    snuba_params = SnubaQueryParams(start=start, end=end, groupby=groupby, conditions=conditions, filter_keys=filter_keys, aggregations=aggregations, rollup=rollup, is_grouprelease=is_grouprelease, **kwargs)
    ...
    def snuba_query(params):
        ...
        try:
            with timer('snuba_query'):
                return (
                 _snuba_pool.urlopen('POST', '/query', body=json.dumps(query_params), headers=headers),
                 forward,
                 reverse)
        ...
        if response.status != 200:
            if body.get('error'):
                ...
                if response.status == 429:
                    ...
                    raise clickhouse_error_codes_map.get(error['code'], QueryExecutionError)(error['message'])
                else:
...
def query(start=None, end=None, groupby=None, conditions=None, filter_keys=None, aggregations=None, selected_columns=None, totals=None, **kwargs):
    aggregations = aggregations or [['count()', '', 'aggregate']]
    filter_keys = filter_keys or {}
    selected_columns = selected_columns or []
    groupby = groupby or []
    try:
        body = raw_query(start=start, end=end, groupby=groupby, conditions=conditions, filter_keys=filter_keys, aggregations=aggregations, selected_columns=selected_columns, totals=totals, **kwargs)
...
def nest_groups(data, groups, aggregate_cols):
    ...
    if not groups:
        if len(aggregate_cols) == 1:
            if data:
                return data[0][aggregate_cols[0]]
            return
        if data:
            return {c:data[0][c] for c in aggregate_cols}
        return
    else:
        ...
        return OrderedDict((k, nest_groups(v, rest, aggregate_cols)) for k, v in six.iteritems(inter))
    return


JSON_TYPE_MAP = {'UInt8': 'boolean', 
   'UInt16': 'integer', 
   'UInt32': 'integer', 
   'UInt64': 'integer', 
   'Float32': 'number', 
...
def get_json_type(snuba_type):
    ...
    Default is string
    """
    nullable_match = re.search('^Nullable\\((.+)\\)$', snuba_type)
    if nullable_match:
        snuba_type = nullable_match.group(1)
    array_match = re.search('^Array\\(.+\\)$', snuba_type)
    if array_match:
        return 'array'
...
def get_snuba_translators(filter_keys, is_grouprelease=False):
    ...
    forward = identity
    reverse = identity
    map_columns = {'environment': (
                     Environment, 'name', lambda name: None if name == '' else name), 
       'tags[sentry:release]': (
    ...
    for col, (model, field, fmt) in six.iteritems(map_columns):
        ...
        if is_grouprelease and col == 'tags[sentry:release]':
            gr_map = GroupRelease.objects.filter(id__in=ids).values_list('id', 'group_id', 'release_id')
            ver = dict(Release.objects.filter(id__in=[ x[2] for x in gr_map ]).values_list('id', 'version'))
            fwd_map = {gr:(group, ver[release]) for gr, group, release in gr_map}
            rev_map = dict(reversed(t) for t in six.iteritems(fwd_map))
            fwd = (lambda col, trans: lambda filters: replace(filters, col, [ trans[k][1] for k in filters[col] ]))(col, fwd_map)
            rev = (lambda col, trans: lambda row: replace(row, col, trans.get((row['issue'], row[col]))))(col, rev_map)
        else:
            fwd_map = {k:fmt(v) for k, v in model.objects.filter(id__in=ids).values_list('id', field)}
            rev_map = dict(reversed(t) for t in six.iteritems(fwd_map))
            fwd = (lambda col, trans: lambda filters: replace(filters, col, [ trans[k] for k in filters[col] if k ]))(col, fwd_map)
            rev = (lambda col, trans: lambda row: replace(row, col, trans[row[col]]) if col in row else row)(col, rev_map)
        if fwd:
            forward = compose(forward, fwd)
        if rev:
            reverse = compose(reverse, rev)

    reverse = compose(reverse, lambda row: replace(row, 'time', int(to_timestamp(parse_datetime(row['time'])))) if 'time' in row else row)
    return (
     forward, reverse)
...
def get_related_project_ids(column, ids):
    ...
    mappings = {'issue': (
               Group, 'id', 'project_id'), 
       'tags[sentry:release]': (
                              ReleaseProject, 'release_id', 'project_id')}
    if ids:
        if column == 'project_id':
            return ids
        if column in mappings:
            model, id_field, project_field = mappings[column]
            return model.objects.filter(**{id_field + '__in': ids, project_field + '__isnull': False}).values_list(project_field, flat=True)
    return []
