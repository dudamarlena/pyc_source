...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_api_key_details.py
...
class OrganizationApiKeyDetailsEndpoint(OrganizationEndpoint):
    ...
    def put(self, request, organization, api_key_id):
        ...
        if serializer.is_valid():
            ...
            self.create_audit_entry(request=request, organization=organization, target_object=api_key_id, event=AuditLogEntryEvent.APIKEY_EDIT, data=api_key.get_audit_log_data())
            return Response(serialize(api_key, request.user))
    ...
    def delete(self, request, organization, api_key_id):
        ...
        api_key.delete()
        self.create_audit_entry(request, organization=organization, target_object=api_key.id, event=AuditLogEntryEvent.APIKEY_REMOVE, data=audit_data)
