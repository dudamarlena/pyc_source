...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_rule_details.py
...
class ProjectRuleDetailsEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project, rule_id):
        ...
        """
        rule = Rule.objects.get(project=project, id=rule_id, status__in=[RuleStatus.ACTIVE, RuleStatus.INACTIVE])
    ...
    def put(self, request, project, rule_id):
        ...
        if serializer.is_valid():
            ...
            self.create_audit_entry(request=request, organization=project.organization, target_object=rule.id, event=AuditLogEntryEvent.RULE_EDIT, data=rule.get_audit_log_data())
            return Response(serialize(rule, request.user))
    ...
    def delete(self, request, project, rule_id):
        ...
        self.create_audit_entry(request=request, organization=project.organization, target_object=rule.id, event=AuditLogEntryEvent.RULE_REMOVE, data=rule.get_audit_log_data())
