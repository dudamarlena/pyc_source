...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/group_tag_export.py
...
class GroupTagExportView(ProjectView, CsvMixin, EnvironmentMixin):
    ...
    def get_user_header(self):
        return ('value', 'id', 'email', 'username', 'ip_address', 'times_seen', 'last_seen',
                'first_seen')
    ...
    def get(self, request, organization, project, group_id, key):
        ...
        if key == 'user':
            callbacks = [
             attach_eventuser(project.id)]
        else:
            callbacks = []
        gtv_iter = tagstore.get_group_tag_value_iter(group.project_id, group.id, environment_id, lookup_key, callbacks=callbacks)
        filename = ('{}-{}').format(group.qualified_short_id or group.id, key)
