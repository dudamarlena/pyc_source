...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/integrations/github_enterprise/webhook.py
# Compiled at: 2019-09-04 11:05:35
from __future__ import absolute_import
import hashlib, hmac, logging, six
...
def get_installation_metadata(event, host):
    if not host:
        return
    try:
        integration = Integration.objects.get(external_id=('{}:{}').format(host, event['installation']['id']), provider='github_enterprise')
...
class GitHubEnterpriseWebhookBase(View):
    ...
    def handle(self, request):
        ...
        try:
            ...
            logger.warning('github_enterprise.webhook.invalid-json', extra=self.get_logging_data(), exc_info=True)
            return HttpResponse(status=400)
        ...
        try:
            ...
            if not self.is_valid_signature(method, body, secret, signature):
                logger.warning('github_enterprise.webhook.invalid-signature', extra=self.get_logging_data())
                return HttpResponse(status=401)
        except (KeyError, IndexError) as e:
            logger.info('github_enterprise.webhook.missing-signature', extra={'host': host, 'error': six.text_type(e)})
...
class GitHubEnterpriseWebhookEndpoint(GitHubEnterpriseWebhookBase):
    _handlers = {'push': GitHubEnterprisePushEventWebhook, 
