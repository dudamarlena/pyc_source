...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/helpers.py
...
def get_default_context(request, existing_context=None, team=None):
    ...
    context = {'URL_PREFIX': options.get('system.url-prefix'), 
    ...
    if request:
        ...
        if (not existing_context or 'TEAM_LIST' not in existing_context) and team:
            context['TEAM_LIST'] = Team.objects.get_for_user(organization=team.organization, user=request.user, with_projects=True)
        user = request.user
    else:
        user = AnonymousUser()
    if organization:
    ...
    if not existing_context or 'ACCESS' not in existing_context:
        if request:
            context['ACCESS'] = access.from_request(request=request, organization=organization).to_django_context()
        else:
            context['ACCESS'] = access.from_user(user=user, organization=organization).to_django_context()
    return context
