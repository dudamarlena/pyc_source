...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/sdk_updates.py
...
SDK_INDEX_CACHE_KEY = 'sentry:sdk-versions'
...
class UpdateSDKSuggestion(Suggestion):
    ...
    def get_new_state(self, old_state):
        if self.new_sdk_version is None:
            ...
            try:
                has_newer_version = LooseVersion(old_state.sdk_version) < LooseVersion(self.new_sdk_version)
            except Exception:
...
SDK_SUPPORTED_MODULES = [
 ...
 {'sdk_name': 'sentry.python', 
    ...
    'suggestion': EnableIntegrationSuggestion('sqlalchemy', 'https://docs.sentry.io/platforms/python/sqlalchemy/')},
 {'sdk_name': 'sentry.python', 
    'sdk_version_added': '0.11.0', 
    'module_name': 'apache_beam', 
    'module_version_min': '2.12.0', 
    'suggestion': EnableIntegrationSuggestion('beam', 'https://docs.sentry.io/platforms/python/beam/')},
 ...
 {'sdk_name': 'sentry.dotnet', 
    ...
    'module_version_min': '2.1.0', 
    'suggestion': ChangeSDKSuggestion('sentry.dotnet.extensions.logging', [
...
def _get_suggested_updates_step(setup_state, index_state):
    ...
    for support_info in SDK_SUPPORTED_MODULES:
        ...
        if support_info['module_name'] not in setup_state.modules:
            continue
        try:
            if LooseVersion(support_info['sdk_version_added']) > LooseVersion(setup_state.sdk_version):
                continue
        except Exception:
            continue

        try:
            if LooseVersion(support_info['module_version_min']) > LooseVersion(setup_state.modules[support_info['module_name']]):
                continue
        except Exception:
            continue
