...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/admin.py
...
from sentry.models import ApiKey, AuthIdentity, AuthProvider, AuditLogEntry, Option, Organization, OrganizationMember, Project, Team, User
...
class UserChangeForm(UserChangeForm):
    username = forms.RegexField(label=_('Username'), max_length=128, regex='^[\\w.@+-]+$', help_text=_('Required. 128 characters or fewer. Letters, digits and @/./+/-/_ only.'), error_messages={'invalid': _('This value may contain only letters, numbers and @/./+/-/_ characters.')})
...
class UserAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        return patterns('', ('^(\\d+)/password/$', self.admin_site.admin_view(self.user_change_password))) + super(UserAdmin, self).get_urls()
    ...
    def user_change_password(self, request, id, form_url=''):
        ...
        context = {'title': _('Change password: %s') % escape(user.get_username()), 
           'adminForm': adminForm, 
           'form_url': form_url, 
           'form': form, 
...
class AuditLogEntryAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('organization', 'actor', 'actor_key', 'target_object', 'target_user',
                       'event', 'ip_address', 'data', 'datetime')
