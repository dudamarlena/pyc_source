...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/monitor_checkin_details.py
...
class MonitorCheckInDetailsEndpoint(Endpoint):
    ...
    def put(self, request, project, monitor, checkin):
        ...
        if checkin.status in CheckInStatus.FINISHED_VALUES:
            return self.respond(status=400)
        serializer = CheckInSerializer(data=request.data, partial=True, context={'project': project, 'request': request})
        ...
        with transaction.atomic():
            ...
            if checkin.status == CheckInStatus.ERROR:
                ...
                monitor_params = {'last_checkin': current_datetime, 
                ...
                Monitor.objects.filter(id=monitor.id).exclude(last_checkin__gt=current_datetime).update(**monitor_params)
        return self.respond(serialize(checkin, request.user))