...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_member_index.py
...
class OrganizationMemberIndexEndpoint(OrganizationEndpoint):
    ...
    def save_team_assignments(self, organization_member, teams):
        ...
        OrganizationMemberTeam.objects.bulk_create([ OrganizationMemberTeam(team=team, organizationmember=organization_member) for team in teams
                                                   ])

    def get(self, request, organization):
        queryset = OrganizationMember.objects.filter(Q(user__is_active=True) | Q(user__isnull=True), organization=organization).select_related('user').order_by('email', 'user__email')
        query = request.GET.get('query')
        if query:
            tokens = tokenize_query(query)
            for key, value in six.iteritems(tokens):
                if key == 'email':
                    queryset = queryset.filter(Q(email__in=value) | Q(user__email__in=value) | Q(user__emails__email__in=value))
                elif key == 'scope':
                    ...
                    value = (' ').join(value)
                    queryset = queryset.filter(Q(email__icontains=value) | Q(user__email__icontains=value) | Q(user__name__icontains=value))
                else:
    ...
    def post(self, request, organization):
        ...
        if not features.has('organizations:invite-members', organization, actor=request.user):
            return Response({'organization': 'Your organization is not allowed to invite members'}, status=403)
        ...
        if result.get('teams'):
            ...
            teams = []
        if result['role'] not in {r.id for r in allowed_roles}:
            return Response({'role': 'You do not have permission to invite that role.'}, 403)
        existing = OrganizationMember.objects.filter(organization=organization, user__email__iexact=result['email'], user__is_active=True).exists()
        if existing:
            return Response({'email': 'The user %s is already a member' % result['email']}, 409)
        om = OrganizationMember(organization=organization, email=result['email'], role=result['role'])
        ...
        if settings.SENTRY_ENABLE_INVITES and result.get('sendInvite'):
            ...
            member_invited.send_robust(member=om, user=request.user, sender=self, referrer=request.data.get('referrer'))
        self.create_audit_entry(request=request, organization_id=organization.id, target_object=om.id, data=om.get_audit_log_data(), event=AuditLogEntryEvent.MEMBER_INVITE if settings.SENTRY_ENABLE_INVITES else AuditLogEntryEvent.MEMBER_ADD)
