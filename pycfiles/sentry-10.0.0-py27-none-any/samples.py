...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/samples.py
...
def random_geo():
    return random.choice([{'country_code': 'US', 'region': 'CA', 'city': 'San Francisco'}, {'country_code': 'AU', 'region': 'VIC', 'city': 'Melbourne'}, {'country_code': 'GB', 'region': 'H9', 'city': 'London'}])
...
def name_for_username(username):
    return {'ben': 'Ben Vinegar', 
       ...
       'eric': 'Eric Feng', 
       'armin': 'Armin Ronacher'}.get(username, username.replace('_', ' ').title())
...
def load_data(platform, default=None, sample_name=None):
    ...
    if data is None:
        ...
        data.setdefault('user', generate_user(ip_address='127.0.0.1', username='sentry', id=1, email='sentry@example.com'))
        data.setdefault('extra', {'session': {'foo': 'bar'}, 'results': [
                     1, 2, 3, 4, 5], 
           'emptyList': [], 'emptyMap': {}, 'length': 10837790, 
           'unauthorized': False, 
           'url': 'http://example.org/foo/bar/'})
        data.setdefault('modules', {'my.package': '1.0.0'})
        data.setdefault('request', {'cookies': 'foo=bar;biz=baz', 
           'url': 'http://example.com/foo', 
           'headers': {'Referer': 'http://example.com', 
                       'Content-Type': 'application/json', 
                       'User-Agent': 'Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.72 Safari/537.36'}, 
           'env': {'ENV': 'prod'}, 'query_string': 'foo=bar', 
           'data': '{"hello": "world"}', 
           'method': 'GET'})
        return data
