...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/team_groups_trending.py
...
class TeamGroupsTrendingEndpoint(TeamEndpoint, EnvironmentMixin):
    ...
    def get(self, request, team):
        ...
        sort_value = 'score'
        group_list = list(Group.objects.filter(project__in=project_dict.keys(), status=GroupStatus.UNRESOLVED, last_seen__gte=cutoff_dt).extra(select={'sort_value': sort_value}).order_by(('-{}').format(sort_value))[:limit])
