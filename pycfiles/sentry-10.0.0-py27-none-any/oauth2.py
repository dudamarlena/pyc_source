...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/identity/oauth2.py
...
class OAuth2Provider(Provider):
    ...
    def get_pipeline_views(self):
        return [
         OAuth2LoginView(authorize_url=self.get_oauth_authorize_url(), client_id=self.get_oauth_client_id(), scope=(' ').join(self.get_oauth_scopes())),
         OAuth2CallbackView(access_token_url=self.get_oauth_access_token_url(), client_id=self.get_oauth_client_id(), client_secret=self.get_oauth_client_secret())]
    ...
    def handle_refresh_error(self, req, payload):
        ...
        if req.status_code == 401:
            self.logger.info('identity.oauth.refresh.identity-not-valid-error', extra={'error_name': error_name, 
        ...
        if req.status_code != 200:
            self.logger.info('identity.oauth.refresh.api-error', extra={'error_name': error_name, 
               'error_status_code': req.status_code, 
               'error_description': error_description, 
               'provider_key': self.key})
    ...
    def refresh_identity(self, identity, *args, **kwargs):
        ...
        req = safe_urlopen(url=self.get_refresh_token_url(), headers=self.get_refresh_token_headers(), data=data)
        try:
...
class OAuth2LoginView(PipelineView):
    ...
    def dispatch(self, request, pipeline):
        ...
        state = uuid4().hex
        params = self.get_authorize_params(state=state, redirect_uri=absolute_uri(pipeline.redirect_url()))
...
class OAuth2CallbackView(PipelineView):
    ...
    def exchange_token(self, request, pipeline, code):
        ...
        try:
            ...
            logger.info('identity.oauth2.ssl-error', extra={'url': self.access_token_url, 'verify_ssl': verify_ssl})
            url = self.access_token_url
            return {'error': 'Could not verify SSL certificate', 
            ...
            return {'error': 'Could not decode a JSON Response', 
