...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/group_details.py
...
from sentry.models import Activity, Group, GroupHash, GroupRelease, GroupSeen, GroupStatus, Release, ReleaseEnvironment, ReleaseProject, User, UserReport
...
class GroupDetailsEndpoint(GroupEndpoint, EnvironmentMixin):
    ...
    def _get_activity(self, request, group, num):
        ...
        for item in activity_qs[:num * 2]:
            sig = (
             item.type, item.ident, item.user_id)
    ...
    def _get_seen_by(self, request, group):
        seen_by = list(GroupSeen.objects.filter(group=group).select_related('user').order_by('-last_seen'))
    ...
    def _get_context_plugins(self, request, group):
        project = group.project
        return serialize([ plugin for plugin in plugins.for_project(project, version=None) if plugin.has_project_conf() and hasattr(plugin, 'get_custom_contexts') and plugin.get_custom_contexts()
                         ], request.user, PluginSerializer(project))

    def _get_release_info(self, request, group, version):
        try:
            release = Release.objects.get(projects=group.project, organization_id=group.project.organization_id, version=version)
    ...
    def get(self, request, group):
        ...
        daily_stats = tsdb.rollup(get_range(model=tsdb.models.group, keys=[group.id], end=now, start=now - timedelta(days=30)), 86400)[group.id]
        participants = list(User.objects.filter(groupsubscription__is_active=True, groupsubscription__group=group))
        data.update({'firstRelease': first_release, 
        ...
        if environments:
            ...
            data.update({'currentRelease': serialize(current_release, request.user, GroupReleaseWithStatsSerializer())})
    ...
    def put(self, request, group):
        ...
        try:
            response = client.put(path=('/projects/{}/{}/issues/').format(group.project.organization.slug, group.project.slug), params={'id': group.id}, data=request.data, request=request)
    ...
    def delete(self, request, group):
        ...
        if updated:
            ...
            delete_groups.apply_async(kwargs={'object_ids': [
                            group.id], 
               'transaction_id': transaction_id, 
               'eventstream_state': eventstream_state}, countdown=3600)
            self.create_audit_entry(request=request, organization_id=project.organization_id if project else None, target_object=group.id, transaction_id=transaction_id)
            delete_logger.info('object.delete.queued', extra={'object_id': group.id, 
               'transaction_id': transaction_id, 
               'model': type(group).__name__})
            issue_deleted.send_robust(group=group, user=request.user, delete_type='delete', sender=self.__class__)
