...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_integration_details.py
...
class OrganizationIntegrationDetailsEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization, integration_id):
        try:
            integration = OrganizationIntegration.objects.get(integration_id=integration_id, organization=organization)
    ...
    def delete(self, request, organization, integration_id):
        try:
            org_integration = OrganizationIntegration.objects.get(integration_id=integration_id, organization=organization)
        except OrganizationIntegration.DoesNotExist:
            raise Http404

        updated = OrganizationIntegration.objects.filter(id=org_integration.id, status=ObjectStatus.VISIBLE).update(status=ObjectStatus.PENDING_DELETION)
        if updated:
            delete_organization_integration.apply_async(kwargs={'object_id': org_integration.id, 
               'transaction_id': uuid4().hex, 
               'actor_id': request.user.id}, countdown=0)
