...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/grouping/fingerprinting.py
    ...
    def get_exceptions(self):
        if self._exceptions is None:
            ...
            for exc in get_path(self.event, 'exception', 'values', filter=True) or ():
                self._exceptions.append({'type': exc.get('type'), 
                   'value': exc.get('value'), 
                   'family': get_behavior_family_for_platform(self.event.get('platform'))})
    ...
    def get_frames(self, with_functions=False):
        ...
        if self._frames is None:
            ...
            def _push_frame(frame):
                ...
                self._frames.append({'function': func or '<unknown>', 
...
class FingerprintingRules(object):
    ...
    def from_config_string(self, s):
        try:
            ...
            if len(context) == 33:
                context = context[:-1] + '...'
            raise InvalidFingerprintingConfig('Invalid syntax near "%s" (line %s, column %s)' % (context, e.line(), e.column()))
        ...
        if value is None:
            ...
            if self.key in ('path', 'package'):
                ...
                if ref_val is not None and ref_val == value:
                    return True
            elif glob_match(value, self.pattern, ignorecase=self.key in ('message',
                                                                         'value')):
                return True
            return False
...
class Rule(object):
    ...
    def _to_config_structure(self):
        return {'matchers': [ x._to_config_structure() for x in self.matchers ], 'fingerprint': self.fingerprint}
