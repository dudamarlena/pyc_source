...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/api_application_details.py
...
class ApiApplicationSerializer(serializers.Serializer):
    ...
    homepageUrl = serializers.URLField(max_length=255, required=False, allow_null=True, allow_blank=True)
    termsUrl = serializers.URLField(max_length=255, required=False, allow_null=True, allow_blank=True)
    privacyUrl = serializers.URLField(max_length=255, required=False, allow_null=True, allow_blank=True)
...
class ApiApplicationDetailsEndpoint(Endpoint):
    ...
    def get(self, request, app_id):
        try:
            instance = ApiApplication.objects.get(owner=request.user, client_id=app_id, status=ApiApplicationStatus.active)
    ...
    def delete(self, request, app_id):
        ...
        if updated:
            ...
            delete_api_application.apply_async(kwargs={'object_id': instance.id, 'transaction_id': transaction_id}, countdown=3600)
            delete_logger.info('object.delete.queued', extra={'object_id': instance.id, 
