...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_teams.py
...
def create_new_team_scenario(runner):
    runner.request(method='POST', path='/organizations/%s/teams/' % runner.org.slug, data={'name': 'Ancient Gabelers'})
...
class OrganizationTeamsPermission(OrganizationPermission):
    scope_map = {'GET': [
       ...
       'POST': [
              'org:write', 'org:admin', 'team:write'], 
       'PUT': [
             'org:write', 'org:admin', 'team:write'], 
       'DELETE': [
                'org:admin', 'team:write']}


class TeamSerializer(serializers.Serializer):
    name = serializers.CharField(max_length=64, required=False, allow_null=True, allow_blank=True)
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50, required=False, allow_null=True, error_messages={'invalid': _('Enter a valid slug consisting of lowercase letters, numbers, underscores or hyphens.')})
...
class OrganizationTeamsEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        return self.paginate(request=request, queryset=queryset, order_by='slug', on_results=lambda x: serialize(x, request.user, serializer()), paginator_cls=OffsetPaginator)
        ...
        if serializer.is_valid():
            ...
            try:
                with transaction.atomic():
                    team = Team.objects.create(name=result.get('name') or result['slug'], slug=result.get('slug'), organization=organization)
            except IntegrityError:
                return Response({'non_field_errors': [
                                      CONFLICTING_SLUG_ERROR], 
                   'detail': CONFLICTING_SLUG_ERROR}, status=409)
            ...
            self.create_audit_entry(request=request, organization=organization, target_object=team.id, event=AuditLogEntryEvent.TEAM_ADD, data=team.get_audit_log_data())
