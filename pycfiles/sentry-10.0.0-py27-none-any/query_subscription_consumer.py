...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/snuba/query_subscription_consumer.py
...
class QuerySubscriptionConsumer(object):
    ...
    def __init__(self, group_id, topic=None, commit_batch_size=100, initial_offset_reset='earliest'):
    ...
    def commit_offsets(self):
        if self.offsets and self.consumer:
            to_commit = [ TopicPartition(self.topic, partition, offset) for partition, offset in self.offsets.items()
                        ]
    ...
    def handle_message(self, message):
        ...
        try:
            ...
            logger.exception('Subscription update could not be parsed', extra={'offset': message.offset(), 
               'partition': message.partition(), 
               'value': message.value()})
            return

        try:
            subscription = QuerySubscription.objects.get(subscription_id=contents['subscription_id'])
        except QuerySubscription.DoesNotExist:
            metrics.incr('snuba_query_subscriber.subscription_doesnt_exist')
            logger.error('Received subscription update, but subscription does not exist', extra={'offset': message.offset(), 
               'partition': message.partition(), 
               'value': message.value()})
            return
        ...
        if subscription.type not in subscriber_registry:
            ...
            logger.error('Received subscription update, but no subscription handler registered', extra={'offset': message.offset(), 
               'partition': message.partition(), 
               'value': message.value()})
            return
