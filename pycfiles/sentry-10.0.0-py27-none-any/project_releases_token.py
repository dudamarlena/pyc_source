...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_releases_token.py
...
def _get_webhook_url(project, plugin_id, token):
    return absolute_uri(reverse('sentry-release-hook', kwargs={'plugin_id': plugin_id, 
       'project_id': project.id, 
       'signature': _get_signature(project.id, plugin_id, token)}))


def _get_signature(project_id, plugin_id, token):
    return hmac.new(key=token.encode('utf-8'), msg=('{}-{}').format(plugin_id, project_id).encode('utf-8'), digestmod=sha256).hexdigest()
