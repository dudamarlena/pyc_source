...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_releases.py
        ...
        if serializer.is_valid():
            ...
            try:
                with transaction.atomic():
                    release, created = Release.objects.create(organization_id=project.organization_id, version=result['version'], ref=result.get('ref'), url=result.get('url'), owner=result.get('owner'), date_released=result.get('dateReleased')), True
                was_released = False
