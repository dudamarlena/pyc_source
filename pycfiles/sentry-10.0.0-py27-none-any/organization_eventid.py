...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_eventid.py
...
def resolve_event_id_scenario(runner):
    runner.request(method='GET', path='/organizations/%s/eventids/%s/' % (runner.org.slug, runner.default_event.event_id))
...
class EventIdLookupEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization, event_id):
        ...
        try:
            event = eventstore.get_events(filter_keys={'project_id': project_slugs_by_id.keys(), 'event_id': event_id}, limit=1)[0]
        except IndexError:
            raise ResourceDoesNotExist()
        else:
            return Response({'organizationSlug': organization.slug, 
