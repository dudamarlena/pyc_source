...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_search_details.py
...
class ProjectSearchDetailsEndpoint(ProjectEndpoint):
    ...
    def put(self, request, project, search_id):
        ...
        has_team_scope = any(request.access.has_team_scope(team, 'project:write') for team in project.teams.all())
        if has_team_scope:
        ...
        if result.get('isDefault'):
            SavedSearch.objects.filter(project=project).exclude(id=search_id).update(is_default=False)
        if result.get('isUserDefault'):
            SavedSearchUserDefault.objects.create_or_update(user=request.user, project=project, values={'savedsearch': search})
        return Response(serialize(search, request.user))
