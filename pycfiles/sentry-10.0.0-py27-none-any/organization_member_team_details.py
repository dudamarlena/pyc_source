...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_member_team_details.py
...
from sentry.models import AuditLogEntryEvent, OrganizationAccessRequest, OrganizationMember, OrganizationMemberTeam, Team
...
class OrganizationMemberTeamDetailsEndpoint(OrganizationEndpoint):
    ...
    def _can_access(self, request, member, organization):
        ...
        * If the user is an "admin" and they are modifying a team they are a member of
        ...
        if request.user.id == member.user_id:
            return True
        acting_member = OrganizationMember.objects.get(organization=organization, user__id=request.user.id, user__is_active=True)
        if roles.get(acting_member.role).is_global and roles.can_manage(acting_member.role, member.role):
            return True
        return False

    def _can_admin_team(self, request, organization, team_slug):
        return OrganizationMember.objects.filter(organization=organization, user__id=request.user.id, user__is_active=True, role='admin', organizationmemberteam__team__slug=team_slug).exists()
    ...
    def post(self, request, organization, member_id, team_slug):
        ...
        try:
            ...
            if not (request.access.has_scope('org:write') or organization.flags.allow_joinleave or can_admin_team):
        ...
        self.create_audit_entry(request=request, organization=organization, target_object=omt.id, target_user=om.user, event=AuditLogEntryEvent.MEMBER_JOIN_TEAM, data=omt.get_audit_log_data())
    ...
    def delete(self, request, organization, member_id, team_slug):
        ...
        try:
            ...
            self.create_audit_entry(request=request, organization=organization, target_object=omt.id, target_user=om.user, event=AuditLogEntryEvent.MEMBER_LEAVE_TEAM, data=omt.get_audit_log_data())
            omt.delete()
