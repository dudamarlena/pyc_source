...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/accounts.py
...
def recover(request):
    ...
    if request.method == 'POST' and ratelimiter.is_limited(('accounts:recover:{}').format(extra['ip_address']), limit=5, window=60):
        logger.warning('recover.rate-limited', extra=extra)
        return HttpResponse('You have made too many password recovery attempts. Please try again later.', content_type='text/plain', status=429)
    else:
...
def start_confirm_email(request):
    from sentry.app import ratelimiter
    if ratelimiter.is_limited(('auth:confirm-email:{}').format(request.user.id), limit=10, window=60):
        return HttpResponse('You have made too many email confirmation requests. Please try again later.', content_type='text/plain', status=429)
    ...
    if request.user.has_unverified_emails():
        ...
        for email in unverified_emails:
            logger.info('user.email.start_confirm', extra={'user_id': request.user.id, 
...
def confirm_email(request, user_id, hash):
    ...
    try:
        ...
        if request.user.is_anonymous() or request.user.has_unverified_emails():
            msg = _('There was an error confirming your email. Please try again or visit your Account Settings to resend the verification email.')
            level = messages.ERROR
    else:
        ...
        logger.info('user.email.confirm', extra={'user_id': user_id, 
           'ip_address': request.META['REMOTE_ADDR'], 
           'email': email.email})
...
def disconnect_identity(request, identity_id):
    ...
    messages.add_message(request, messages.SUCCESS, ('Your {} identity has been disconnected.').format(settings.AUTH_PROVIDER_LABELS.get(backend_name, backend_name)))
    logger.info('user.identity.disconnect', extra={'user_id': request.user.id, 
       'ip_address': request.META['REMOTE_ADDR'], 
