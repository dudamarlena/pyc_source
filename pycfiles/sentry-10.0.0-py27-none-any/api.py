...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/api.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import, print_function
import base64, math, io, jsonschema, logging, random, six, traceback, uuid
...
from sentry.coreapi import Auth, APIError, APIForbidden, APIRateLimited, ClientApiHelper, ClientAuthHelper, SecurityAuthHelper, MinidumpAuthHelper, safely_load_json_string, logger as api_logger
...
from sentry.lang.native.minidump import merge_attached_event, merge_attached_breadcrumbs, write_minidump_placeholder, MINIDUMP_ATTACHMENT_TYPE
...
kafka_publisher = QueuedPublisherService(KafkaPublisher(getattr(settings, 'KAFKA_RAW_EVENTS_PUBLISHER_CONNECTION', None), asynchronous=False)) if getattr(settings, 'KAFKA_RAW_EVENTS_PUBLISHER_ENABLED', False) else None
...
class APIView(BaseView):
    ...
    def _parse_header(self, request, project_config):
        ...
        if auth.version not in PROTOCOL_VERSIONS:
            track_outcome(project_config.organization_id, project_config.project_id, None, Outcome.INVALID, 'auth_version')
    ...
    def dispatch(self, request, project_id=None, *args, **kwargs):
        helper = None
        try:
            helper = ClientApiHelper(agent=request.META.get('HTTP_USER_AGENT'), project_id=project_id, ip_address=request.META['REMOTE_ADDR'])
        ...
        metrics.incr('client-api.all-versions.responses.%s' % (response.status_code,), skip_internal=False)
        metrics.incr('client-api.all-versions.responses.%sxx' % (six.text_type(response.status_code)[0],), skip_internal=False)
        if helper is not None and helper.context is not None and helper.context.version:
            ...
            metrics.incr('client-api.v%s.responses.%s' % (helper.context.version, response.status_code), skip_internal=False)
            metrics.incr('client-api.v%s.responses.%sxx' % (
             helper.context.version, six.text_type(response.status_code)[0]), skip_internal=False)
        return response
    ...
    def _dispatch(self, request, helper, project_config, origin=None, *args, **kwargs):
        ...
        if origin is not None:
            if not is_valid_origin(origin, allowed=allowed):
                track_outcome(project_config.organization_id, project_config.project_id, None, Outcome.INVALID, FilterStatKeys.CORS)
        ...
        response = super(APIView, self).dispatch(request=request, project=project, auth=auth, helper=helper, key=key, project_config=project_config, **kwargs)
        return response
    ...
    def options(self, request, *args, **kwargs):
        ...
        """
        raise NotImplementedError('Options request should have been handled by @allow_cors_options.\nIf dispatch was overridden either decorate it with @allow_cors_options or provide a valid implementation for options.')
...
class StoreView(APIView):
    ...
    def process(self, request, project, key, auth, helper, data, project_config, attachments=None, **kwargs):
        ...
        metrics.timing('events.size.data.post_storeendpoint', data_size, tags={'project_id': project_id})
...
class EventAttachmentStoreView(StoreView):
    ...
    def post(self, request, project, event_id, project_config, **kwargs):
        ...
        for name, uploaded_file in six.iteritems(request.FILES):
            file = File.objects.create(name=uploaded_file.name, type='event.attachment', headers={'Content-Type': uploaded_file.content_type})
            file.putfile(uploaded_file)
            EventAttachment.objects.create(project_id=project_id, event_id=event_id, name=uploaded_file.name, file=file)
...
class MinidumpView(StoreView):
    ...
    def _dispatch(self, request, helper, project_config, origin=None, config_flags=None, *args, **kwargs):
        if request.method != 'POST':
            ...
            if key.project_id != project_id:
                track_outcome(project_config.organization_id, project_id, None, Outcome.INVALID, 'multi_project_id')
                raise APIError('Two different projects were specified')
            helper.context.bind_auth(auth)
            return super(APIView, self).dispatch(request=request, project=project, auth=auth, helper=helper, key=key, project_config=project_config, **kwargs)
    ...
    def post(self, request, project, project_config, **kwargs):
        ...
        if content_type in self.dump_types:
            ...
            data['extra'] = extra
        if not minidump:
            track_outcome(project_config.organization_id, project_config.project_id, None, Outcome.INVALID, 'missing_minidump_upload')
        ...
        if minidump.read(2) == '--':
            ...
            handlers = [ uploadhandler.load_handler(handler, request) for handler in settings.FILE_UPLOAD_HANDLERS
                       ]
        ...
        if minidump.read(4) != 'MDMP':
            track_outcome(project_config.organization_id, project_config.project_id, None, Outcome.INVALID, 'invalid_minidump')
            raise APIError('Uploaded file was not a minidump')
        attachments = []
        minidump.seek(0)
        attachments.append(CachedAttachment(name=minidump_name, content_type='application/octet-stream', data=minidump.read(), type=MINIDUMP_ATTACHMENT_TYPE))
        ...
        event_id = self.process(request, attachments=attachments, data=data, project=project, project_config=project_config, **kwargs)
...
class UnrealView(StoreView):
    ...
    def _dispatch(self, request, helper, project_config, sentry_key, origin=None, config_flags=None, *args, **kwargs):
        if request.method != 'POST':
            track_outcome(0, 0, None, Outcome.INVALID, 'disallowed_method')
            return HttpResponseNotAllowed(['POST'])
        else:
    ...
    def post(self, request, project, project_config, **kwargs):
        attachments_enabled = features.has('organizations:event-attachments', project.organization, actor=request.user)
        is_apple_crash_report = False
        attachments = []
        event = {'event_id': uuid.uuid4().hex}
        try:
            unreal = process_unreal_crash(request.body, request.GET.get('UserID'), request.GET.get('AppEnvironment'), event)
            ...
            track_outcome(project_config.organization_id, project_config.project_id, None, Outcome.INVALID, 'process_minidump_unreal')
        ...
        for file in unreal.files():
            ...
            if file.type == 'minidump' or attachments_enabled:
                attachments.append(CachedAttachment(name=file.name, data=file.open_stream().read(), type=unreal_attachment_type(file)))

        if is_minidump:
            write_minidump_placeholder(event)
        event_id = self.process(request, attachments=attachments, data=event, project=project, project_config=project_config, **kwargs)
        return HttpResponse(six.text_type(uuid.UUID(event_id)), content_type='text/plain')
...
class SecurityReportView(StoreView):
    ...
    content_types = ('application/csp-report', 'application/json', 'application/expect-ct-report',
                     'application/expect-ct-report+json', 'application/expect-staple-report')
    ...
    def _dispatch(self, request, helper, project_config, origin=None, config_flags=None, *args, **kwargs):
        if request.method != 'POST':
            ...
            key = helper.project_key_from_auth(auth)
            if key.project_id != project_id:
                track_outcome(project.organization_id, project.id, None, Outcome.INVALID, 'multi_project_id')
                raise APIError('Two different projects were specified')
            helper.context.bind_auth(auth)
            return super(APIView, self).dispatch(request=request, project=project, auth=auth, helper=helper, key=key, project_config=project_config, **kwargs)
    ...
    def post(self, request, project, helper, key, project_config, **kwargs):
        ...
        if report_type is None:
            track_outcome(project_config.organization_id, project_config.project_id, key.id, Outcome.INVALID, 'security_report_type')
        ...
        try:
            ...
            track_outcome(project_config.organization_id, project_config.project_id, key.id, Outcome.INVALID, 'security_report')
            raise APIError('Invalid security report: %s' % str(e).splitlines()[0])

        origin = instance.get_origin()
        if not is_valid_origin(origin, project):
            track_outcome(project_config.organization_id, project_config.project_id, key.id, Outcome.INVALID, FilterStatKeys.CORS)
            raise APIForbidden('Invalid origin')
        data = {'interface': interface.path, 
           'report': instance, 
           'release': request.GET.get('sentry_release'), 
           'environment': request.GET.get('sentry_environment')}
        self.process(request, project=project, helper=helper, data=data, key=key, project_config=project_config, **kwargs)
        return HttpResponse(content_type='application/javascript', status=201)

    def security_report_type(self, body):
        report_type_for_key = {'csp-report': 'csp', 
