...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/group.py
...
from sentry.db.models import BaseManager, BoundedBigIntegerField, BoundedIntegerField, BoundedPositiveIntegerField, FlexibleForeignKey, GzippedDictField, Model, sane_repr
from sentry.utils.http import absolute_uri
from sentry.utils.numbers import base32_decode, base32_encode
from sentry.utils.strings import strip, truncatechars
logger = logging.getLogger(__name__)
_short_id_re = re.compile('^(.*?)(?:[\\s_-])([A-Za-z0-9]+)$')
...
def get_group_with_redirect(id_or_qualified_short_id, queryset=None, organization=None):
    ...
    try:
        ...
        if short_id:
            params = {'id': GroupRedirect.objects.filter(organization_id=organization.id, previous_short_id=short_id.short_id, previous_project_slug=short_id.project_slug).values_list('group_id', flat=True)}
        else:
            params['id'] = GroupRedirect.objects.filter(previous_group_id=params['id']).values_list('group_id', flat=True)
        try:
            return (
             queryset.get(**params), True)
...
def get_oldest_or_latest_event_for_environments(ordering, environments=(), issue_id=None, project_id=None):
    from sentry.utils import snuba
    from sentry.models import SnubaEvent
    conditions = []
    if len(environments) > 0:
        conditions.append(['environment', 'IN', environments])
    result = snuba.raw_query(selected_columns=SnubaEvent.selected_columns, conditions=conditions, filter_keys={'issue': [issue_id], 'project_id': [project_id]}, orderby=ordering.value, limit=1, referrer='Group.get_latest')
...
class GroupManager(BaseManager):
    ...
    def by_qualified_short_id(self, organization_id, short_id):
        ...
        return Group.objects.exclude(status__in=[
         ...
         GroupStatus.PENDING_MERGE]).get(project__organization=organization_id, project__slug=short_id.project_slug, short_id=short_id.short_id)
    ...
    def filter_by_event_id(self, project_ids, event_id):
        data = eventstore.get_events(conditions=[
         [
          'group_id', 'IS NOT NULL', None]], filter_keys={'event_id': [event_id], 'project_id': project_ids}, limit=len(project_ids), referrer='Group.filter_by_event_id')
    ...
    def add_tags(self, group, environment, tags):
        ...
        for tag_item in tags:
            ...
            tagstore.incr_tag_value_times_seen(project_id, environment.id, key, value, extra={'last_seen': date, 'data': data})
            tagstore.incr_group_tag_value_times_seen(project_id, group.id, environment.id, key, value, extra={'project_id': project_id, 'last_seen': date})

        return

    def get_groups_by_external_issue(self, integration, external_issue_key):
        from sentry.models import ExternalIssue, GroupLink
        return Group.objects.filter(id__in=GroupLink.objects.filter(linked_id__in=ExternalIssue.objects.filter(key=external_issue_key, integration_id=integration.id, organization_id__in=integration.organizations.values_list('id', flat=True)).values_list('id', flat=True)).values_list('group_id', flat=True), project__organization_id__in=integration.organizations.values_list('id', flat=True))
...
class Group(Model):
    ...
    culprit = models.CharField(max_length=MAX_CULPRIT_LENGTH, blank=True, null=True, db_column='view')
    ...
    status = BoundedPositiveIntegerField(default=0, choices=(
     ...
     (
      GroupStatus.IGNORED, _('Ignored'))), db_index=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.times_seen is None:
            self.times_seen = 1
        self.score = type(self).calculate_score(times_seen=self.times_seen, last_seen=self.last_seen)
    ...
    def get_latest_event_for_environments(self, environments=()):
        return get_oldest_or_latest_event_for_environments(EventOrdering.LATEST, environments=environments, issue_id=self.id, project_id=self.project_id)

    def get_oldest_event_for_environments(self, environments=()):
        return get_oldest_or_latest_event_for_environments(EventOrdering.OLDEST, environments=environments, issue_id=self.id, project_id=self.project_id)
    ...
    def count_users_seen(self):
        return tagstore.get_groups_user_counts([self.project_id], [self.id], environment_ids=None)[self.id]
