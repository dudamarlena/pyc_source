...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/rules/conditions/event_frequency.py
...
intervals = {'1m': (
        'one minute', timedelta(minutes=1)), 
   '1h': (
        'one hour', timedelta(hours=1)), 
   '1d': (
        'one day', timedelta(hours=24)), 
   '1w': (
        'one week', timedelta(days=7)), 
   '30d': (
         '30 days', timedelta(days=30))}

class EventFrequencyForm(forms.Form):
    interval = forms.ChoiceField(choices=[ (key, label) for key, (label, duration) in sorted(intervals.items(), key=lambda key____label__duration: key____label__duration[1][1])
                                         ])
...
class BaseEventFrequencyCondition(EventCondition):
    ...
    form_fields = {'value': {'type': 'number', 'placeholder': 100}, 'interval': {'type': 'choice', 
                    'choices': [ (key, label) for key, (label, duration) in sorted(intervals.items(), key=lambda key____label__duration: key____label__duration[1][1])
                             ]}}
...
class EventFrequencyCondition(BaseEventFrequencyCondition):
    ...
    def query(self, event, start, end, environment_id):
        return self.tsdb.get_sums(model=self.tsdb.models.group, keys=[
         event.group_id], start=start, end=end, environment_id=environment_id)[event.group_id]
...
class EventUniqueUserFrequencyCondition(BaseEventFrequencyCondition):
    ...
    def query(self, event, start, end, environment_id):
        return self.tsdb.get_distinct_counts_totals(model=self.tsdb.models.users_affected_by_group, keys=[
         event.group_id], start=start, end=end, environment_id=environment_id)[event.group_id]