...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_events.py
...
class OrganizationEventsEndpoint(OrganizationEventsEndpointBase):
    ...
    def get(self, request, organization):
        ...
        try:
            direct_hit_resp = get_direct_hit_response(request, query, self.get_filter_params(request, organization), 'api.organization-events')
        ...
        try:
            ...
            data_fn = partial(eventstore.get_events, additional_columns=cols, referrer='api.organization-events', **snuba_args)
    ...
    def handle_results(self, request, organization, project_ids, results):
        projects = {p['id']:p['slug'] for p in Project.objects.filter(organization=organization, id__in=project_ids).values('id', 'slug')}
...
class OrganizationEventsV2Endpoint(OrganizationEventsEndpointBase):
    ...
    def get(self, request, organization):
        ...
        if not has_global_views and len(filters.get('project_id', [])) > 1:
            return Response({'detail': 'You cannot view events from multiple projects.'}, status=400)
        data_fn = partial((lambda **kwargs: snuba.transform_aliases_and_query(skip_conditions=True, **kwargs)), referrer='api.organization-events-v2', **snuba_args)
        try:
            return self.paginate(request=request, paginator=GenericOffsetPaginator(data_fn=data_fn), on_results=lambda results: self.handle_results_with_meta(request, organization, params['project_id'], results))
        except snuba.SnubaError as error:
            logger.info('organization.events.snuba-error', extra={'organization_id': organization.id, 
