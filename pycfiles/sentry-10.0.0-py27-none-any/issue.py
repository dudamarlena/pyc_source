...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/bases/issue.py
...
class IssueTrackingPlugin(Plugin):
    ...
    def get_auth_for_user(self, user, **kwargs):
        ...
        if not self.auth_provider:
            raise AssertionError('There is no auth provider configured for this plugin.')
            return user.is_authenticated() or None
        else:
            try:
    ...
    def get_link_existing_issue_form(self, request, group, event, **kwargs):
        if not self.link_issue_form:
            ...
            return self.link_issue_form(request.POST or None, initial=self.get_initial_link_form_data(request, group, event))
    ...
    def get_initial_form_data(self, request, group, event, **kwargs):
        return {'description': self._get_group_description(request, group, event), 
    ...
    def view(self, request, group, **kwargs):
        ...
        if not (has_auth_configured and self.is_configured(project=group.project, request=request)):
            ...
            if self.needs_auth(project=group.project, request=request):
                return self.render(self.needs_auth_template, {'title': self.get_title(), 'project': group.project})
            ...
            if op == 'create':
                if create_form.is_valid():
                    try:
                        issue_id = self.create_issue(group=group, form_data=create_form.cleaned_data, request=request)
                    except forms.ValidationError as e:
                        create_form.errors['__all__'] = [
                         'Error creating issue: %s' % e]

                if create_form.is_valid():
                    GroupMeta.objects.set_value(group, '%s:tid' % prefix, issue_id)
                    issue_information = {'title': create_form.cleaned_data['title'], 
                    ...
                    issue_tracker_used.send_robust(plugin=self, project=group.project, user=request.user, sender=IssueTrackingPlugin)
                    return self.redirect(group.get_absolute_url())
            elif op == 'link':
                ...
                if link_form.is_valid():
                    ...
                    GroupMeta.objects.set_value(group, '%s:tid' % prefix, issue_id)
                    issue_information = {'title': self.get_issue_title_by_id(request, group, issue_id), 
                       'provider': self.get_title(), 
                       'location': self.get_issue_url(group, issue_id), 
                       'label': self.get_issue_label(group=group, issue_id=issue_id)}
                    Activity.objects.create(project=group.project, group=group, type=Activity.CREATE_ISSUE, user=request.user, data=issue_information)
                    return self.redirect(group.get_absolute_url())
            context = {'create_form': create_form, 'form': create_form, 
    ...
    def tags(self, request, group, tag_list, **kwargs):
        ...
        if not issue_id:
            return tag_list
        tag_list.append(format_html('<a href="{}" rel="noreferrer">{}</a>', self.get_issue_url(group=group, issue_id=issue_id), self.get_issue_label(group=group, issue_id=issue_id)))
        return tag_list
