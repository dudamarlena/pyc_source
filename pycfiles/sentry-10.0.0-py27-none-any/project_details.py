...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_details.py
...
from sentry.models import AuditLogEntryEvent, Group, GroupStatus, Project, ProjectBookmark, ProjectRedirect, ProjectStatus, ProjectTeam, UserOption
...
def get_project_scenario(runner):
    runner.request(method='GET', path='/projects/%s/%s/' % (runner.org.slug, runner.default_project.slug))
...
def update_project_scenario(runner):
    with runner.isolated_project('Plain Proxy') as (project):
        runner.request(method='PUT', path='/projects/%s/%s/' % (runner.org.slug, project.slug), data={'name': 'Plane Proxy', 
           'slug': 'plane-proxy', 
           'platform': 'javascript', 
...
class ProjectAdminSerializer(ProjectMemberSerializer):
    ...
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50)
    team = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50)
    ...
    securityToken = serializers.RegexField('^[-a-zA-Z0-9+/=\\s]+$', max_length=255, allow_blank=True)
    securityTokenHeader = serializers.RegexField('^[a-zA-Z0-9_\\-]+$', max_length=20, allow_blank=True)
    ...
    groupingEnhancementsBase = serializers.CharField(required=False, allow_blank=True, allow_null=True)
    ...
    def validate_slug(self, slug):
        if slug in RESERVED_PROJECT_SLUGS:
            raise serializers.ValidationError('The slug "%s" is reserved and not allowed.' % (slug,))
...
class ProjectDetailsEndpoint(ProjectEndpoint):
    ...
    def put(self, request, project):
        ...
        :auth: required
        """
        has_project_write = request.auth and request.auth.has_scope('project:write') or request.access and request.access.has_scope('project:write')
        ...
        if not serializer.is_valid():
            ...
            new_team = None
            if result.get('team'):
                return Response({'detail': ['Editing a team via this endpoint has been deprecated.']}, status=400)
            ...
            if changed:
                ...
                if old_team_id is not None:
                    ProjectTeam.objects.filter(project=project, team_id=old_team_id).update(team=new_team)
                if old_slug:
            ...
            if result.get('groupingEnhancementsBase') is not None:
                if project.update_option('sentry:grouping_enhancements_base', result['groupingEnhancementsBase']):
            ...
            if result.get('isSubscribed'):
                UserOption.objects.set_value(user=request.user, key='mail:alert', value=1, project=project)
            elif result.get('isSubscribed') is False:
                UserOption.objects.set_value(user=request.user, key='mail:alert', value=0, project=project)
            if has_project_write:
                ...
                if 'sentry:store_crash_reports' in options:
                    project.update_option('sentry:store_crash_reports', bool(options['sentry:store_crash_reports']))
                ...
                if ('filters:{}').format(FilterTypes.RELEASES) in options:
                    if features.has('projects:custom-inbound-filters', project, actor=request.user):
                        project.update_option(('sentry:{}').format(FilterTypes.RELEASES), clean_newline_inputs(options[('filters:{}').format(FilterTypes.RELEASES)]))
                    else:
                        return Response({'detail': ['You do not have that feature enabled']}, status=400)
                if ('filters:{}').format(FilterTypes.ERROR_MESSAGES) in options:
                    if features.has('projects:custom-inbound-filters', project, actor=request.user):
                        project.update_option(('sentry:{}').format(FilterTypes.ERROR_MESSAGES), clean_newline_inputs(options[('filters:{}').format(FilterTypes.ERROR_MESSAGES)], case_insensitive=False))
                    else:
                        return Response({'detail': ['You do not have that feature enabled']}, status=400)
                ...
                self.create_audit_entry(request=request, organization=project.organization, target_object=project.id, event=AuditLogEntryEvent.PROJECT_EDIT, data=changed_proj_settings)
            data = serialize(project, request.user, DetailedProjectSerializer())
            return Response(data)
    ...
    def delete(self, request, project):
        ...
        updated = Project.objects.filter(id=project.id, status=ProjectStatus.VISIBLE).update(status=ProjectStatus.PENDING_DELETION)
        if updated:
            transaction_id = uuid4().hex
            delete_project.apply_async(kwargs={'object_id': project.id, 'transaction_id': transaction_id}, countdown=3600)
            self.create_audit_entry(request=request, organization=project.organization, target_object=project.id, event=AuditLogEntryEvent.PROJECT_REMOVE, data=project.get_audit_log_data(), transaction_id=transaction_id)
            delete_logger.info('object.delete.queued', extra={'object_id': project.id, 
