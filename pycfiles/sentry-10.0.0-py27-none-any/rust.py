...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/rust.py
...
FRAME_RE = re.compile('(?xm)\n    ^\n        [\\ ]*                               # leading whitespace\n        (?:\\d+:[\\ ]*)?                      # leading frame number\n        (?P<addr>0x[a-f0-9]+)?              # instruction address\n        [\\ ]-[\\ ]\n        (?P<symbol>[^\\r\\n]+)\n        (?:\n            \\r?\\n\n            [\\ \\t]+at[\\ ]\n            (?P<path>[^\\r\\n]+?)\n            (?::(?P<lineno>\\d+))?\n        )?\n    $\n')
HASH_FUNC_RE = re.compile('(?x)\n    ^(.*)::h[a-f0-9]{16}$\n')
PATTERN_MATCH_RE = re.compile('^(_?\\<|\\w+ as )+')
RUST_CRATE_RE = re.compile('([a-zA-Z0-9_]+?)(?:\\.\\.|::)')
RUST_ESCAPES_RE = re.compile('(?x)\n    \\$\n        (SP|BP|RF|LT|GT|LP|RP|C|\n            u7e|u20|u27|u5b|u5d|u7b|u7d|u3b|u2b|u22)\n    \\$\n')
RUST_ESCAPES = {'SP': '@', 
   'BP': '*', 
   'RF': '&', 
   'LT': '<', 
...
def frames_from_rust_info(rust_info):
    ...
    end = next((i for i, f in enumerate(frames) if matches_frame(f['function'], OUTER_BORDER_FRAMES)), len(frames))
    start = -next((i for i, f in enumerate(reversed(frames)) if matches_frame(f['function'], INNER_BORDER_FRAMES)), 0)
