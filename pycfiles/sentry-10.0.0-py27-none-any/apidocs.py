...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/apidocs.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import
import os, re, json, inspect, requests, mimetypes
...
optional_group_matcher = re.compile('\\(\\?\\:([^\\)]+)\\)')
named_group_matcher = re.compile('\\(\\?P<(\\w+)>[^\\)]+\\)')
non_named_group_matcher = re.compile('\\([^\\)]+\\)')
either_option_matcher = re.compile('\\[([^\\]]+)\\|([^\\]]+)\\]')
camel_re = re.compile('([A-Z]+)([a-z])')
rst_indent_re = re.compile('^\\s{2,}')
rst_block_re = re.compile('^\\.\\.\\s[a-z]+::$')
API_PREFIX = '/api/0/'
scenarios = {}

def simplify_regex(pattern):
    r"""Clean up urlpattern regexes into something somewhat readable by
...
def extract_endpoint_info(pattern, internal_endpoint):
    ...
    for method_name in HTTP_METHODS:
        ...
        yield dict(path=API_PREFIX + path.lstrip('/'), method=method_name, title=title, text=text, warning=warning, params=params, scenarios=getattr(method, 'api_scenarios', None) or [], section=section.name.lower(), internal_path='%s:%s' % (get_endpoint_path(internal_endpoint), method.__name__), endpoint_name=endpoint_name)
...
def create_sample_time_series(event):
    ...
    for _ in range(60):
        ...
        tsdb.incr_multi((
         ...
         (
          tsdb.models.project_total_received, group.project.id)), now, int(count * 1.1))
        tsdb.incr_multi((
         (
          tsdb.models.organization_total_rejected, group.project.organization_id),
         (
          tsdb.models.project_total_rejected, group.project.id)), now, int(count * 0.1))
...
class MockUtils(object):
    ...
    def create_org(self, name, owner):
        ...
        dummy_member, _ = OrganizationMember.objects.get_or_create(user=owner, organization=org, defaults={'role': 'owner'})
        return org

    def create_api_token(self, user):
        from django.conf import settings
        from sentry.models import ApiToken
        return ApiToken.objects.create(user=user, scope_list=settings.SENTRY_SCOPES, refresh_token=None, expires_at=None)
    ...
    def join_team(self, team, user):
        ...
        member = OrganizationMember.objects.get(organization_id=team.organization_id, user_id=user.id)
    ...
    def create_release(self, project, user, version=None):
        ...
        with transaction.atomic():
            release = Release.objects.filter(version=version, organization_id=project.organization_id, projects=project).first()
            if not release:
                release = Release.objects.filter(version=version, organization_id=project.organization_id).first()
                if not release:
                    release = Release.objects.create(version=version, organization_id=project.organization_id)
                release.add_project(project)
        Activity.objects.create(type=Activity.RELEASE, project=project, ident=Activity.get_version_ident(version), user=user, data={'version': version})
        return release
    ...
    def create_release_file(self, project, release, path, content_type=None, contents=None):
        ...
        f = File.objects.create(name=path.rsplit('/', 1)[(-1)], type='release.file', headers={'Content-Type': content_type})
        f.putfile(StringIO(contents or ''))
        return ReleaseFile.objects.create(organization_id=project.organization_id, release=release, file=f, name=path)

    def create_event(self, project, release, platform='python', raw=True):
        from sentry.utils.samples import create_sample_event
        event = create_sample_event(project=project, platform=platform, release=release.version, raw=raw)
