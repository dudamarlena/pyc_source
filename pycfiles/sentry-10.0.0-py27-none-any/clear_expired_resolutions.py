...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/clear_expired_resolutions.py
...
def clear_expired_resolutions(release_id):
    ...
    resolution_list = list(GroupResolution.objects.filter(Q(type=GroupResolution.Type.in_next_release) | Q(type__isnull=True), release__projects=release.projects.all(), release__date_added__lt=release.date_added, status=GroupResolution.Status.pending).exclude(release=release))
