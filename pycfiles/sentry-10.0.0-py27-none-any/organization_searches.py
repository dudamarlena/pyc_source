...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_searches.py
...
class OrganizationSearchesEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        if request.GET.get('use_org_level') == '1':
            ...
            saved_searches = list(SavedSearch.objects.filter(org_searches_q | global_searches_q, type=search_type).extra(select={'has_owner': 'owner_id is not null', 'name__upper': 'UPPER(name)'}, order_by=[
             '-has_owner', 'name__upper']))
            results = []
            if saved_searches:
                pinned_search = None
            ...
            org_searches = Q(Q(owner=request.user) | Q(owner__isnull=True), ~Q(query__in=DEFAULT_SAVED_SEARCH_QUERIES), project__in=self.get_projects(request, organization))
            global_searches = Q(is_global=True)
            results = list(SavedSearch.objects.filter(org_searches | global_searches).order_by('name', 'project'))
    ...
    def post(self, request, organization):
        ...
        if serializer.is_valid():
            ...
            if SavedSearch.objects.filter(Q(is_global=True) | Q(organization=organization, owner__isnull=True), query=result['query']).exists():
                return Response({'detail': ('Query {} already exists').format(result['query'])}, status=400)
            saved_search = SavedSearch.objects.create(organization=organization, type=result['type'], name=result['name'], query=result['query'])
