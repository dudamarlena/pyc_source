...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/oauth_authorize.py
...
class OAuthAuthorizeView(AuthLoginView):
    ...
    def redirect_response(self, response_type, redirect_uri, params):
        if response_type == 'token':
            return self.redirect(('{}#{}').format(redirect_uri, urlencode([ (k, v) for k, v in six.iteritems(params) if v is not None ])))
        else:
    ...
    def error(self, request, response_type, redirect_uri, name, state=None, client_id=None):
        logging.error('oauth.authorize-error', extra={'error_name': name, 
    ...
    def get(self, request, **kwargs):
        ...
        if not client_id:
            logging.error('oauth.authorize-error', extra={'error_name': 'unauthorized_client', 
               'response_type': response_type, 
               'client_id': client_id, 
               'redirect_uri': redirect_uri})
        ...
        if not redirect_uri:
            ...
            logging.error('oauth.authorize-error', extra={'error_name': 'invalid_request', 
               'response_type': response_type, 
               'client_id': client_id, 
               'redirect_uri': redirect_uri})
            return self.respond('sentry/oauth-error.html', {'error': mark_safe('Missing or invalid <em>redirect_uri</em> parameter.')})
        if not application.is_allowed_response_type(response_type):
            return self.error(request=request, client_id=client_id, response_type=response_type, redirect_uri=redirect_uri, name='unsupported_response_type', state=state)
        if scopes:
            scopes = scopes.split(' ')
            for scope in scopes:
        ...
        if not force_prompt:
            try:
                existing_auth = ApiAuthorization.objects.get(user=request.user, application=application)
        ...
        payload = {'rt': response_type, 'cid': client_id, 
           'ru': redirect_uri, 
           'sc': scopes, 
           'st': state, 
           'uid': request.user.id}
    ...
    def post(self, request, **kwargs):
        ...
        if not request.user.is_authenticated():
            response = super(OAuthAuthorizeView, self).post(request, application=application, **kwargs)
    ...
    def approve(self, request, application, **params):
        try:
            with transaction.atomic():
                ApiAuthorization.objects.create(application=application, user=request.user, scope_list=params['scopes'])
        ...
        if params['response_type'] == 'code':
            grant = ApiGrant.objects.create(user=request.user, application=application, redirect_uri=params['redirect_uri'], scope_list=params['scopes'])
            return self.redirect_response(params['response_type'], params['redirect_uri'], {'code': grant.code, 'state': params['state']})
        else:
            if params['response_type'] == 'token':
                token = ApiToken.objects.create(application=application, user=request.user, refresh_token=None, scope_list=params['scopes'])
                return self.redirect_response(params['response_type'], params['redirect_uri'], {'access_token': token.token, 
