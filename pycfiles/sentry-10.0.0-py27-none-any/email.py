...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/email.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import
import logging, os, six, subprocess, tempfile, time
from email.utils import parseaddr
from functools import partial
from operator import attrgetter
from random import randrange
import lxml, toronado
...
def get_from_email_domain():
    ...
    if not _from_email_domain_cache[0] == from_:
        _from_email_domain_cache = (
         from_, domain_from_email(from_))
...
default_list_type_handlers = {Activity: attrgetter('project.slug', 'project.organization.slug'), 
...
make_listid_from_instance = ListResolver(options.get('mail.list-namespace'), default_list_type_handlers)

class MessageBuilder(object):

    def __init__(self, subject, context=None, template=None, html_template=None, body=None, html_body=None, headers=None, reference=None, reply_reference=None, from_email=None, type=None):
        ...
        self.reply_reference = reply_reference
    ...
    def build(self, to, reply_to=None, cc=None, bcc=None):
        ...
        if isinstance(reference, Group):
            thread, created = GroupEmailThread.objects.get_or_create(email=to, group=reference, defaults={'project': reference.project, 'msgid': message_id})
            if not created:
                headers.setdefault('In-Reply-To', thread.msgid)
                headers.setdefault('References', thread.msgid)
        msg = EmailMultiAlternatives(subject=subject.splitlines()[0], body=self.__render_text_body(), from_email=self.from_email, to=(
         to,), cc=cc or (), bcc=bcc or (), headers=headers)
    ...
    def get_built_messages(self, to=None, cc=None, bcc=None):
        ...
        results = [ self.build(to=email, reply_to=send_to, cc=cc, bcc=bcc) for email in send_to if email ]
        if not results:
    ...
    def send(self, to=None, cc=None, bcc=None, fail_silently=False):
        return send_messages(self.get_built_messages(to, cc=cc, bcc=bcc), fail_silently=fail_silently)
...
def send_messages(messages, fail_silently=False):
    ...
    for message in messages:
        extra = {'message_id': message.extra_headers['Message-Id'], 'size': len(message.message().as_bytes())}
...
def get_connection(fail_silently=False):
    ...
    """
    return _get_connection(backend=get_mail_backend(), host=options.get('mail.host'), port=options.get('mail.port'), username=options.get('mail.username'), password=options.get('mail.password'), use_tls=options.get('mail.use-tls'), timeout=options.get('mail.timeout'), fail_silently=fail_silently)
...
class PreviewBackend(BaseEmailBackend):
    ...
    def send_messages(self, email_messages):
        for message in email_messages:
            ...
            preview = tempfile.NamedTemporaryFile(delete=False, prefix='sentry-email-preview-', suffix='.eml')
            try:
