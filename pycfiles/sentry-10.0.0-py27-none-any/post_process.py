...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/post_process.py
...
def _should_send_error_created_hooks(project):
    ...
    if result is None:
        ...
        if not features.has('organizations:integrations-event-hooks', organization=org):
            ...
            return False
        result = ServiceHook.objects.filter(organization_id=org.id).extra(where=[
         "events @> '{error.created}'"]).exists()
...
def check_event_already_post_processed(event):
    ...
    if cluster_key is None:
        ...
        result = client.set(('pp:{}/{}').format(event.project_id, event.event_id), ('{:.0f}').format(time.time()), ex=3600, nx=True)
        return not result
...
def post_process_group(event, is_new, is_regression, is_sample, is_new_group_environment, **kwargs):
    ...
    with snuba.options_override({'consistent': True}):
        if check_event_already_post_processed(event):
            logger.info('post_process.skipped', extra={'project_id': event.project_id, 
        ...
        if event.group_id:
            ...
            if event.get_event_type() == 'error' and _should_send_error_created_hooks(event.project):
                process_resource_change_bound.delay(action='created', sender='Error', instance_id=event.event_id, instance=event)
            if is_new:
                process_resource_change_bound.delay(action='created', sender='Group', instance_id=event.group_id)
        ...
        event_processed.send_robust(sender=post_process_group, project=event.project, event=event, primary_hash=kwargs.get('primary_hash'))
...
def plugin_post_process_group(plugin_slug, event, **kwargs):
    ...
    safe_execute(plugin.post_process, event=event, group=event.group, expected_errors=(
     PluginError,), **kwargs)


@instrumented_task(name='sentry.tasks.index_event_tags', queue='events.index_event_tags', default_retry_delay=300, max_retries=None)
def index_event_tags(organization_id, project_id, event_id, tags, group_id, environment_id, date_added=None, **kwargs):
    ...
    tagstore.create_event_tags(project_id=project_id, group_id=group_id, environment_id=environment_id, event_id=event_id, tags=tags, **create_event_tags_kwargs)
    return