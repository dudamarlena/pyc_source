...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/mediators/external_requests/issue_link_requester.py
...
class IssueLinkRequester(Mediator):
    ...
    def _make_request(self):
        req = safe_urlopen(url=self._build_url(), headers=self._build_headers(), method='POST', data=self.body)
        try:
            body = safe_urlread(req)
            response = json.loads(body)
        except Exception:
            logger.info('issue-link-requester.error', extra={'sentry_app': self.sentry_app.slug, 
