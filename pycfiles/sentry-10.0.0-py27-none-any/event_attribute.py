...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/rules/conditions/event_attribute.py
...
class EventAttributeForm(forms.Form):
    attribute = forms.CharField(widget=FixedTypeaheadInput(choices=[ {'id': a, 'text': a} for a in ATTR_CHOICES ]))
...
class EventAttributeCondition(EventCondition):
    ...
    form_fields = {'attribute': {'type': 'choice', 
                     'placeholder': 'i.e. exception.type', 
                     'choices': [ [a, a] for a in ATTR_CHOICES ]}, 
       'match': {'type': 'choice', 'choices': MATCH_CHOICES.items()}, 'value': {'type': 'string', 'placeholder': 'value'}}
    ...
    def _get_attribute_values(self, event, attr):
        ...
        if path[0] == 'http':
            if path[1] not in ('url', 'method'):
                return []
            return [
             getattr(event.interfaces['request'], path[1])]
    ...
    def render_label(self):
        data = {'attribute': self.data['attribute'], 
