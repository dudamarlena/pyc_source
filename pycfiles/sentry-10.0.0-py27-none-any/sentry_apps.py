...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/sentry_apps.py
...
from sentry.models import SentryAppInstallation, EventCommon, Group, Project, Organization, User, ServiceHook, ServiceHookProject, SentryApp, SnubaEvent
from sentry.models.sentryapp import VALID_EVENTS
logger = logging.Logger('sentry.tasks.sentry_apps')
TASK_OPTIONS = {'queue': 'app_platform', 
...
def _webhook_event_data(event, group_id, project_id):
    ...
    event_context['url'] = absolute_uri(reverse('sentry-api-0-project-event-details', args=[
     project.organization.slug, project.slug, event.event_id]))
    event_context['web_url'] = absolute_uri(reverse('sentry-organization-event-detail', args=[organization.slug, group_id, event.event_id]))
...
def send_alert_event(event, rule, sentry_app_id):
    ...
    organization = Organization.objects.get_from_cache(id=project.organization_id)
    extra = {'sentry_app_id': sentry_app_id, 
    ...
    try:
        install = SentryAppInstallation.objects.get(organization=organization.id, sentry_app=sentry_app)
    ...
    request_data = AppPlatformEvent(resource='event_alert', action='triggered', install=install, data=data)
    safe_urlopen(url=sentry_app.webhook_url, data=request_data.body, headers=request_data.headers, timeout=5)
...
def _process_resource_change(action, sender, instance_id, retryer=None, *args, **kwargs):
    ...
    if event not in VALID_EVENTS:
        ...
        if isinstance(instance, Group) or issubclass(model, EventCommon):
            org = Organization.objects.get_from_cache(id=Project.objects.get_from_cache(id=instance.project_id).organization_id)
        installations = filter(lambda i: event in i.sentry_app.events, org.sentry_app_installations.select_related('sentry_app'))
...
def notify_sentry_app(event, futures):
    for f in futures:
        if not f.kwargs.get('sentry_app'):
            continue
        send_alert_event.delay(event=event, rule=f.rule.label, sentry_app_id=f.kwargs['sentry_app'].id)


def send_webhooks(installation, event, **kwargs):
    try:
        servicehook = ServiceHook.objects.get(organization_id=installation.organization_id, actor_id=installation.id)
    ...
    if not project_limited:
        ...
        safe_urlopen(url=servicehook.sentry_app.webhook_url, data=request_data.body, headers=request_data.headers, timeout=5)