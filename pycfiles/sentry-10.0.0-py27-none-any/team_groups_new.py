...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/team_groups_new.py
...
class TeamGroupsNewEndpoint(TeamEndpoint, EnvironmentMixin):
    ...
    def get(self, request, team):
        ...
        sort_value = 'score'
        group_list = list(Group.objects.filter(project__in=project_dict.keys(), status=GroupStatus.UNRESOLVED, active_at__gte=cutoff_dt).extra(select={'sort_value': sort_value}).order_by(('-{}').format(sort_value), '-first_seen')[:limit])
        for group in group_list:
            group._project_cache = project_dict.get(group.project_id)

        return Response(serialize(group_list, request.user, GroupSerializer(environment_func=self._get_environment_func(request, team.organization_id))))