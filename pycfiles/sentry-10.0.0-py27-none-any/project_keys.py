...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_keys.py
...
def list_keys_scenario(runner):
    runner.request(method='GET', path='/projects/%s/%s/keys/' % (runner.org.slug, runner.default_project.slug))


@scenario('CreateClientKey')
def create_key_scenario(runner):
    runner.request(method='POST', path='/projects/%s/%s/keys/' % (runner.org.slug, runner.default_project.slug), data={'name': 'Fabulous Key'})


class KeySerializer(serializers.Serializer):
    name = serializers.CharField(max_length=200, required=False, allow_blank=True, allow_null=True)
    public = serializers.RegexField('^[a-f0-9]{32}$', required=False, allow_null=True)
    secret = serializers.RegexField('^[a-f0-9]{32}$', required=False, allow_null=True)
...
class ProjectKeysEndpoint(ProjectEndpoint):
    ...
    def post(self, request, project):
        ...
        if serializer.is_valid():
            ...
            key = ProjectKey.objects.create(project=project, label=result.get('name'), public_key=result.get('public'), secret_key=result.get('secret'))
            self.create_audit_entry(request=request, organization=project.organization, target_object=key.id, event=AuditLogEntryEvent.PROJECTKEY_ADD, data=key.get_audit_log_data())
