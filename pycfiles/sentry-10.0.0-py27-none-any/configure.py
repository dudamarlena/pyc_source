...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/integrations/jira/configure.py
...
class JiraConfigForm(forms.Form):
    organizations = forms.TypedMultipleChoiceField(label='Enabled Sentry Organizations', help_text='Select which Sentry organizations the Jira Integration is enabled for. Note, removing the integration from an organization will clear its settings.', coerce=int, choices=tuple(), required=False, widget=forms.CheckboxSelectMultiple)
...
class JiraConfigureView(View):
    ...
    def get_response(self, context):
        context['ac_js_src'] = '%(base_url)s%(context_path)s/atlassian-connect/all.js' % {'base_url': self.request.GET['xdm_e'], 
    ...
    def handle(self, request):
        ...
        if not request.user.is_authenticated():
            return self.get_response({'login_required': True, 'login_url': absolute_uri(reverse('sentry-login'))})
        organizations = list(request.user.get_orgs().filter(id__in=OrganizationMember.objects.filter(role__in=[ r.id for r in roles.get_all() if r.is_global ], user=request.user).values('organization')))
        form = JiraConfigForm(organizations, request.POST)
        if request.method == 'GET' or not form.is_valid():
            active_orgs = OrganizationIntegration.objects.filter(integration__provider='jira', integration=integration, organization__in=organizations).values_list('organization_id', flat=True)
        ...
        OrganizationIntegration.objects.filter(integration__provider='jira', integration=integration, organization__in=disabled_orgs).delete()
