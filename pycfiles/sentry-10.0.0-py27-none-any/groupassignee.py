...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/groupassignee.py
...
def get_user_project_ids(users):
    ...
    user_teams = list(OrganizationMemberTeam.objects.filter(organizationmember__user__in=users, is_active=True).values('organizationmember__user', 'team'))
...
def sync_group_assignee_inbound(integration, email, external_issue_key, assign=True):
    ...
    affected_groups = list(Group.objects.get_groups_by_external_issue(integration, external_issue_key).filter(project__organization_id__in=orgs_with_sync_enabled))
    ...
    users = {u.id:u for u in User.objects.filter(id__in=UserEmail.objects.filter(is_verified=True, email=email).values_list('user_id', flat=True))}
    projects_by_user = get_user_project_ids(users.values())
    groups_assigned = []
    for group in affected_groups:
        try:
            user_id = [ user_id for user_id, projects in projects_by_user.items() if group.project_id in projects
                      ][0]
        except IndexError:
            logger.info('assignee-not-found-inbound', extra={'integration_id': integration.id, 
               'email': email, 
...
def sync_group_assignee_outbound(group, user_id, assign=True):
    ...
    for external_issue_id in external_issue_ids:
        sync_assignee_outbound.apply_async(kwargs={'external_issue_id': external_issue_id, 'user_id': user_id, 'assign': assign})
...
class GroupAssigneeManager(BaseManager):
    ...
    def assign(self, group, assigned_to, acting_user=None):
        ...
        now = timezone.now()
        assignee, created = GroupAssignee.objects.get_or_create(group=group, defaults={'project': group.project, assignee_type: assigned_to, 'date_added': now})
        if not created:
            affected = GroupAssignee.objects.filter(group=group).exclude(**{assignee_type: assigned_to}).update(**{assignee_type: assigned_to, other_type: None, 'date_added': now})
        else:
            affected = True
            issue_assigned.send_robust(project=group.project, group=group, user=acting_user, sender=self.__class__)
        if affected:
            activity = Activity.objects.create(project=group.project, group=group, type=Activity.ASSIGNED, user=acting_user, data={'assignee': six.text_type(assigned_to.id), 
...
class GroupAssignee(Model):
    ...
    user = FlexibleForeignKey(settings.AUTH_USER_MODEL, related_name='sentry_assignee_set', null=True)
