...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/message_filters.py
...
def set_filter_state(filter_id, project, state):
    ...
    if flt == _legacy_browsers_filter:
        ...
        ProjectOption.objects.set_value(project=project, key=('filters:{}').format(filter_id), value=option_val)
        return option_val
    else:
        if state is None:
            state = {'active': True}
        ProjectOption.objects.set_value(project=project, key=('filters:{}').format(filter_id), value='1' if state.get('active', False) else '0')
        if state:
...
def get_filter_state(filter_id, project):
    ...
    if flt is None:
        raise FilterNotRegistered(filter_id)
    filter_state = ProjectOption.objects.get_value(project=project, key=('filters:{}').format(flt.spec.id))
    if filter_state is None:
        raise ValueError(('Could not find filter state for filter {0}. You need to register default filter state in projectoptions.defaults.').format(filter_id))
    if flt == _legacy_browsers_filter:
        if filter_state == '1':
            return True
        if filter_state == '0':
...
_localhost_filter.spec = _FilterSpec(id=FilterStatKeys.LOCALHOST, name='Filter out events coming from localhost', description='This applies to both IPv4 (``127.0.0.1``) and IPv6 (``::1``) addresses.')
_EXTENSION_EXC_VALUES = re.compile(('|').join(re.escape(x) for x in ('top.GLOBALS',
                                                                     ...
                                                                     'should_do_lastpass_here')), re.I)
_EXTENSION_EXC_SOURCES = re.compile(('|').join(('graph\\.facebook\\.com', 'connect\\.facebook\\.net',
                                                'eatdifferent\\.com\\.woopra-ns\\.com',
                                                ...
                                                'kaspersky-labs\\.com', '2mdn\\.net')), re.I)
...
_browser_extensions_filter.spec = _FilterSpec(id=FilterStatKeys.BROWSER_EXTENSION, name='Filter out errors known to be caused by browser extensions', description='Certain browser extensions will inject inline scripts and are known to cause errors.')
MIN_VERSIONS = {'Chrome': 0, 
   'IE': 10, 
   'Firefox': 0, 
   'Safari': 6, 
   'Edge': 0, 
...
_legacy_browsers_filter.spec = _FilterSpec(id=FilterStatKeys.LEGACY_BROWSER, name='Filter out known errors from legacy browsers', description='Older browsers often give less accurate information, and while they may report valid issues, the context to understand them is incorrect or missing.', serializer_cls=_LegacyBrowserFilterSerializer)
...
_legacy_browsers_sub_filters = {'default': _filter_default, 
...
_CRAWLERS = re.compile(('|').join(('Mediapartners\\-Google', 'AdsBot\\-Google', 'Googlebot',
                                   'FeedFetcher\\-Google', 'BingBot', 'BingPreview',
                                   'Baiduspider', 'Slurp', 'Sogou', 'facebook', 'ia_archiver',
                                   'bots?[\\/\\s\\)\\;]', 'spider[\\/\\s\\)\\;]',
                                   'Slack', 'Calypso AppCrawler', 'pingdom', 'lyticsbot')), re.I)
...
_web_crawlers_filter.spec = _FilterSpec(id=FilterStatKeys.WEB_CRAWLER, name='Filter out known web crawlers', description='Some crawlers may execute pages in incompatible ways which then cause errors that are unlikely to be seen by a normal user.')