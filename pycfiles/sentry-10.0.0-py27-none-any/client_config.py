...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/client_config.py
...
def _get_project_key(project_id):
    try:
        return ProjectKey.objects.filter(project=project_id, roles=F('roles').bitor(ProjectKey.roles.store))[0]
    except IndexError:
        return
...
def get_client_config(request=None):
    ...
    context = {'singleOrganization': settings.SENTRY_SINGLE_ORGANIZATION, 
       ...
       'messages': [ {'message': msg.message, 'level': msg.tags} for msg in messages ], 'isOnPremise': settings.SENTRY_ONPREMISE, 
       ...
       'sentryConfig': {'dsn': _get_public_dsn(), 
                        'release': version_info['build'], 
                        'whitelistUrls': list(settings.ALLOWED_HOSTS)}}
    if user and user.is_authenticated():
        context.update({'isAuthenticated': True, 'user': serialize(user, user, DetailedUserSerializer())})
