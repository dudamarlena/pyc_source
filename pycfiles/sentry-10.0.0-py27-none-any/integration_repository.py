...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/providers/integration_repository.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import dateutil.parser, six
    ...
    def dispatch(self, request, organization, **kwargs):
        ...
        try:
            with transaction.atomic():
                repo = Repository.objects.create(organization_id=organization.id, name=result['name'], external_id=result.get('external_id'), url=result.get('url'), config=result.get('config') or {}, provider=self.id, integration_id=result.get('integration_id'))
        except IntegrityError:
            try:
                repo = Repository(organization_id=organization.id, name=result['name'], external_id=result.get('external_id'), url=result.get('url'), config=result.get('config') or {}, provider=self.id, integration_id=result.get('integration_id'))
                self.on_delete_repository(repo)
            except IntegrationError:
                pass

            return Response({'errors': {'__all__': 'A repository with that name already exists'}}, status=400)
    ...
    def handle_api_error(self, error):
        ...
        if isinstance(error, IntegrationError):
            if '503' in error.message:
                context.update({'error_type': 'service unavailable', 'errors': {'__all__': error.message}})
                status = 503
            else:
