...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/lostpasswordhash.py
...
class LostPasswordHash(Model):
    ...
    def send_email(self, request, mode='recover'):
        ...
        msg = MessageBuilder(subject=('{}Password Recovery').format(options.get('mail.subject-prefix')), template=('sentry/emails/{name}.txt').format(name=template), html_template=('sentry/emails/{name}.html').format(name=template), type='user.password_recovery', context=context)
