...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/serializers/models/organization_dashboard.py
...
class WidgetSerializer(Serializer):
    ...
    def get_attrs(self, item_list, user):
        result = {}
        data_sources = serialize(list(WidgetDataSource.objects.filter(widget_id__in=[ i.id for i in item_list ])))
        for widget in item_list:
            widget_data_sources = [ d for d in data_sources if d['widgetId'] == six.text_type(widget.id) ]
...
class DashboardWithWidgetsSerializer(Serializer):
    ...
    def get_attrs(self, item_list, user):
        ...
        for dashboard in item_list:
            dashboard_widgets = [ w for w in widgets if w['dashboardId'] == six.text_type(dashboard.id) ]
