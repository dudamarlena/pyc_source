...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/sentry_app_details.py
...
class SentryAppDetailsEndpoint(SentryAppBaseEndpoint):
    ...
    def put(self, request, sentry_app):
        if self._has_hook_events(request) and not features.has('organizations:integrations-event-hooks', sentry_app.owner, actor=request.user):
            return Response({'non_field_errors': [
                                  "Your organization does not have access to the 'error' resource subscription."]}, status=403)
        serializer = SentryAppSerializer(sentry_app, data=request.data, partial=True)
        if serializer.is_valid():
            result = serializer.validated_data
            updated_app = Updater.run(user=request.user, sentry_app=sentry_app, name=result.get('name'), author=result.get('author'), status=result.get('status'), webhook_url=result.get('webhookUrl'), redirect_url=result.get('redirectUrl'), is_alertable=result.get('isAlertable'), verify_install=result.get('verifyInstall'), scopes=result.get('scopes'), events=result.get('events'), schema=result.get('schema'), overview=result.get('overview'))
        ...
        if 'schema' in serializer.errors:
            for error_message in serializer.errors['schema']:
                ...
                log_info = {'schema': json.dumps(request.data['schema']), 
