...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/event_attachment_details.py
...
class EventAttachmentDetailsPermission(ProjectPermission):
    ...
    def has_object_permission(self, request, view, project):
        result = super(EventAttachmentDetailsPermission, self).has_object_permission(request, view, project)
        if not result:
            return result
        ...
        try:
            current_role = OrganizationMember.objects.filter(organization=organization, user=request.user).values_list('role', flat=True).get()
...
class EventAttachmentDetailsEndpoint(ProjectEndpoint):
    ...
    def download(self, attachment):
        ...
        response['Content-Disposition'] = 'attachment; filename="%s"' % posixpath.basename((' ').join(attachment.name.split()))
        return response
    ...
    def get(self, request, project, event_id, attachment_id):
        ...
        if not features.has('organizations:event-attachments', project.organization, actor=request.user):
            ...
            try:
                attachment = EventAttachment.objects.filter(project_id=project.id, event_id=event.event_id, id=attachment_id).select_related('file').get()
            ...
            return self.respond({'id': six.text_type(attachment.id), 
