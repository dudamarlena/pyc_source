...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/group_hashes.py
...
class GroupHashesEndpoint(GroupEndpoint):
    ...
    def get(self, request, group):
        ...
        :auth: required
        """
        data_fn = partial(lambda *args, **kwargs: raw_query(*args, **kwargs)['data'], aggregations=[
         ('argMax(event_id, timestamp)', None, 'event_id'),
         ('max', 'timestamp', 'latest_event_timestamp')], filter_keys={'project_id': [group.project_id], 'group_id': [group.id]}, groupby=[
         'primary_hash'], referrer='api.group-hashes', orderby=[
    ...
    def delete(self, request, group):
        ...
        if id_list is None:
            ...
            hash_list = GroupHash.objects.filter(project_id=group.project_id, group=group.id, hash__in=id_list).exclude(state=GroupHash.State.LOCKED_IN_MIGRATION).values_list('hash', flat=True)
            if not hash_list:
                return Response()
    ...
    def __handle_result(self, user, project_id, group_id, result):
        event = {'timestamp': result['latest_event_timestamp'], 
           'event_id': result['event_id'], 
           'group_id': group_id, 
           'project_id': project_id}
        return {'id': result['primary_hash'], 
