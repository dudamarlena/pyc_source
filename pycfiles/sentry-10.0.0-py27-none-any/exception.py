...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/interfaces/exception.py
...
def upgrade_legacy_mechanism(data):
    ...
    if posix_signal and posix_signal.get('signal'):
        result.setdefault('meta', {})['signal'] = prune_empty_keys({'number': posix_signal.get('signal'), 
    ...
    if mach_exception:
        result.setdefault('meta', {})['mach_exception'] = prune_empty_keys({'exception': mach_exception.get('exception'), 
...
class Mechanism(Interface):
    ...
    def to_python(cls, data):
        for key in ('type', 'synthetic', 'description', 'help_link', 'handled', 'data',
                    'meta'):
    ...
    def to_json(self):
        return prune_empty_keys({'type': self.type, 
    ...
    def iter_tags(self):
        ...
        if self.handled is not None:
            yield (
             'handled', self.handled and 'yes' or 'no')
        return
...
def uncontribute_non_stacktrace_variants(variants):
    ...
    for key, component in six.iteritems(variants):
        if any(s.contributes for s in component.iter_subcomponents(id='stacktrace', recursive=True)):
    ...
    if any_stacktrace_contributes:
        ...
        for component in non_contributing_components:
            component.update(contributes=False, hint='ignored because this variant does not contain a stacktrace, but %s' % hint_suffix)
...
class SingleException(Interface):
    ...
    def to_python(cls, data, slim_frames=True):
        ...
        if get_path(data, 'raw_stacktrace', 'frames', filter=True):
            raw_stacktrace = Stacktrace.to_python(data['raw_stacktrace'], slim_frames=slim_frames, raw=True)
        else:
            raw_stacktrace = None
        type = data.get('type')
    ...
    def to_json(self):
        mechanism = isinstance(self.mechanism, Mechanism) and self.mechanism.to_json() or self.mechanism or None
        if self.stacktrace:
    ...
    def get_api_context(self, is_public=False, platform=None):
        mechanism = isinstance(self.mechanism, Mechanism) and self.mechanism.get_api_context(is_public=is_public, platform=platform) or self.mechanism or None
        if self.stacktrace:
            stacktrace = self.stacktrace.get_api_context(is_public=is_public, platform=platform)
        else:
            stacktrace = None
        if self.raw_stacktrace:
            raw_stacktrace = self.raw_stacktrace.get_api_context(is_public=is_public, platform=platform)
        else:
            raw_stacktrace = None
    ...
    def get_api_meta(self, meta, is_public=False, platform=None):
        mechanism_meta = self.mechanism.get_api_meta(meta['mechanism'], is_public=is_public, platform=platform) if isinstance(self.mechanism, Mechanism) and meta.get('mechanism') else None
        stacktrace_meta = self.stacktrace.get_api_meta(meta, is_public=is_public, platform=platform) if self.stacktrace and meta.get('stacktrace') else None
        return {'': meta.get(''), 
           'type': meta.get('type'), 
...
class Exception(Interface):
    ...
    @classmethod
    def to_python(cls, data):
        return cls(values=[ v and SingleException.to_python(v, slim_frames=False) for v in get_path(data, 'values', default=[])
                          ], exc_omitted=data.get('exc_omitted'))

    def to_json(self):
        return prune_empty_keys({'values': [ v and v.to_json() for v in self.values ] or None, 'exc_omitted': self.exc_omitted})

    def get_api_context(self, is_public=False, platform=None):
        return {'values': [ v.get_api_context(is_public=is_public, platform=platform) for v in self.values if v ], 'hasSystemFrames': any(v.stacktrace.get_has_system_frames() for v in self.values if v and v.stacktrace), 
    ...
    def to_string(self, event, is_public=False, **kwargs):
        ...
        for exc in self.values:
            ...
            if exc.stacktrace:
                output.append(exc.stacktrace.get_stacktrace(event, system_frames=False, max_frames=5, header=False) + '\n\n')
