...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/base.py
...
from sentry.models import Authenticator, Organization, OrganizationMember, OrganizationStatus, Project, ProjectStatus, Team, TeamStatus
...
class OrganizationMixin(object):
    ...
    def get_active_organization(self, request, organization_slug=None):
        ...
        cached_active_org = active_organization and active_organization[0].slug == organization_slug and active_organization[1] == request.user
        if cached_active_org:
            ...
            if organization_slug is not None:
                if is_active_superuser(request):
                    try:
                        active_organization = Organization.objects.get_from_cache(slug=organization_slug)
            ...
            if active_organization is None and organization_slug:
                try:
                    active_organization = six.next(o for o in organizations if o.slug == organization_slug)
            ...
            self._active_org = (
             active_organization, request.user)
    ...
    def is_not_2fa_compliant(self, request, organization):
        return organization.flags.require_2fa and not Authenticator.objects.user_has_2fa(request.user) and not is_active_superuser(request)
...
class OrganizationView(BaseView):
    ...
    def has_permission(self, request, organization, *args, **kwargs):
        if organization is None:
            ...
            if self.required_scope and not request.access.has_scope(self.required_scope):
                logger.info('User %s does not have %s permission to access organization %s', request.user, self.required_scope, organization)
                return False
            return True
    ...
    def is_auth_required(self, request, organization_slug=None, *args, **kwargs):
        ...
        if not organization_slug:
            return False
        active_organization = self.get_active_organization(request=request, organization_slug=organization_slug)
    ...
    def handle_permission_required(self, request, organization, *args, **kwargs):
        if self.needs_sso(request, organization):
            logger.info('access.must-sso', extra={'organization_id': organization.id, 'user_id': request.user.id})
            auth.initiate_login(request, next_url=request.get_full_path())
...
class TeamView(OrganizationView):
        ...
        if team is None:
            ...
            if self.required_scope:
                if not request.access.has_team_scope(team, self.required_scope):
                    logger.info('User %s does not have %s permission to access team %s', request.user, self.required_scope, team)
                    return False
    ...
    def convert_args(self, request, organization_slug, team_slug, *args, **kwargs):
        ...
        if active_organization:
            active_team = self.get_active_team(request=request, team_slug=team_slug, organization=active_organization)
        else:
            active_team = None
        kwargs['organization'] = active_organization
...
class ProjectView(OrganizationView):
        ...
        if project is None:
            ...
            if self.required_scope:
                if not any(request.access.has_team_scope(team, self.required_scope) for team in teams):
                    logger.info('User %s does not have %s permission to access project %s', request.user, self.required_scope, project)
                    return False
    ...
    def convert_args(self, request, organization_slug, project_slug, *args, **kwargs):
        active_organization = self.get_active_organization(request=request, organization_slug=organization_slug)
        if active_organization:
            active_project = self.get_active_project(request=request, organization=active_organization, project_slug=project_slug)
        else:
            active_project = None
