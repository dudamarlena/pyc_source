...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/grouping/strategies/newstyle.py
        ...
        if platform == 'javascript' and '/' in module and abs_path and abs_path.endswith(module):
            ...
            if module[:35] == 'sun.reflect.GeneratedMethodAccessor':
                module_component.update(values=[
                 'sun.reflect.GeneratedMethodAccessor'], hint='removed reflection marker')
            elif module[:44] == 'jdk.internal.reflect.GeneratedMethodAccessor':
                module_component.update(values=[
                 'jdk.internal.reflect.GeneratedMethodAccessor'], hint='removed reflection marker')
            else:
                old_module = module
...
def get_function_component(function, platform, legacy_function_logic, sourcemap_used=False, context_line_available=False, raw_function=None, javascript_fuzzing=False):
    """
        ...
        if func.startswith('block '):
            ...
            if new_function != func:
                function_component.update(values=[
                 new_function], hint='removed generated erb template suffix')
    elif platform == 'php':
...
def frame_v3(frame, event, **meta):
    ...
    return get_frame_component(frame, event, meta, legacy_function_logic=False, use_contextline=good_source, javascript_fuzzing=True)
...
def get_contextline_component(frame, platform):
    ...
    if line:
        ...
        elif get_behavior_family_for_platform(platform) == 'javascript' and has_url_origin(frame.abs_path, allow_file_origin=True):
...
def get_frame_component(frame, event, meta, legacy_function_logic=False, use_contextline=False, javascript_fuzzing=False):
    platform = frame.platform or event.platform
    filename_component = get_filename_component(frame.abs_path, frame.filename, platform, allow_file_origin=javascript_fuzzing)
    ...
    function_component = get_function_component(function=frame.function, raw_function=frame.raw_function, platform=platform, sourcemap_used=frame.data and frame.data.get('sourcemap') is not None, context_line_available=context_line_component and context_line_component.contributes, legacy_function_logic=legacy_function_logic, javascript_fuzzing=javascript_fuzzing)
    values = [
    ...
    if javascript_fuzzing and get_behavior_family_for_platform(platform) == 'javascript':
        ...
        if func in (None, '?', '<anonymous function>', '<anonymous>', 'Anonymous function') or func.endswith('/<'):
            function_component.update(contributes=False, hint='ignored unknown function name')
        if func == 'eval' or frame.abs_path in ('[native code]', 'native code', 'eval code',
                                                '<anonymous>'):
...
def get_stacktrace_component(stacktrace, config, variant, meta):
    ...
    if len(frames) == 1 and values[0].contributes and get_behavior_family_for_platform(frames[0].platform or meta['event'].platform) == 'javascript' and not frames[0].function and frames[0].is_url():
        values[0].update(contributes=False, hint='ignored single non-URL JavaScript frame')
    return config.enhancements.assemble_stacktrace_component(values, frames_for_filtering, meta['event'].platform)
...
@strategy(id='chained-exception:v1', interfaces=['exception'], variants=['!system', 'app'], score=2000)
