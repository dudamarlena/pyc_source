...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/mailgun_inbound_webhook.py
...
class MailgunInboundWebhookView(View):
    ...
    def verify(self, api_key, token, timestamp, signature):
        return constant_time_compare(signature, hmac.new(key=api_key.encode('utf-8'), msg=('{}{}').format(timestamp, token).encode('utf-8'), digestmod=sha256).hexdigest())
