...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/team.py
    ...
    def get_for_user(self, organization, user, scope=None, with_projects=False):
        ...
        if not user.is_authenticated():
            ...
            if with_projects:
                ...
                for project_id, team_id in ProjectTeam.objects.filter(project__in=project_list, team__in=team_list).values_list('project_id', 'team_id'):
...
class Team(Model):
    ...
    status = BoundedPositiveIntegerField(choices=(
     ...
     (
      TeamStatus.DELETION_IN_PROGRESS, _('Deletion in Progress'))), default=TeamStatus.VISIBLE)
    ...
    @property
    def member_set(self):
        return self.organization.member_set.filter(organizationmemberteam__team=self, organizationmemberteam__is_active=True, user__is_active=True).distinct()
    ...
    def transfer_to(self, organization):
        ...
        """
        from sentry.models import OrganizationAccessRequest, OrganizationMember, OrganizationMemberTeam, Project, ProjectTeam, ReleaseProject, ReleaseProjectEnvironment
        try:
        ...
        project_ids = list(Project.objects.filter(teams=self).exclude(organization=organization).values_list('id', flat=True))
        ...
        old_memberships = OrganizationMember.objects.filter(teams=self).exclude(organization=organization)
        for member in old_memberships:
            try:
                new_member = OrganizationMember.objects.get(user=member.user, organization=organization)
            except OrganizationMember.DoesNotExist:
                continue

            try:
                with transaction.atomic():
                    OrganizationMemberTeam.objects.create(team=new_team, organizationmember=new_member)
            except IntegrityError:
                pass

        OrganizationMemberTeam.objects.filter(team=self).exclude(organizationmember__organization=organization).delete()
        if new_team != self:
