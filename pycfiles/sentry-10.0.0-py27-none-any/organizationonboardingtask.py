...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/organizationonboardingtask.py
...
from sentry.db.models import BaseManager, BoundedBigIntegerField, BoundedPositiveIntegerField, FlexibleForeignKey, JSONField, Model, sane_repr
...
class OrganizationOnboardingTask(Model):
    ...
    TASK_CHOICES = (
     ...
     (
      OnboardingTask.INVITE_MEMBER, 'Invite member'),
     (
      OnboardingTask.ISSUE_TRACKER, 'Issue tracker'),
     (
      OnboardingTask.NOTIFICATION_SERVICE,
      'Notification services'),
     (
      OnboardingTask.SECOND_PLATFORM, 'Second platform'),
     (
      OnboardingTask.USER_CONTEXT, 'User context'),
     (
      OnboardingTask.SOURCEMAPS, 'Upload sourcemaps'),
     (
      OnboardingTask.RELEASE_TRACKING, 'Release tracking'),
     (
      OnboardingTask.USER_REPORTS, 'User reports'))
    STATUS_CHOICES = (
     (
