...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/invite_helper.py
...
class ApiInviteHelper(object):
    ...
    def handle_success(self):
        member_joined.send_robust(member=self.om, organization=self.om.organization, sender=self.instance)

    def handle_member_already_exists(self):
        if self.logger:
            self.logger.info('Pending org invite not accepted - User already org member', extra={'organization_id': self.om.organization.id, 'user_id': self.request.user.id})
    ...
    def member_already_exists(self):
        if not self.user_authenticated:
            return False
        return OrganizationMember.objects.filter(organization=self.om.organization, user=self.request.user).exists()

    @property
    def valid_request(self):
        return self.member_pending and self.valid_token and self.user_authenticated and not self.needs_2fa
    ...
    def accept_invite(self):
        ...
        if self.member_already_exists:
            ...
            om.save()
            self.instance.create_audit_entry(self.request, organization=om.organization, target_object=om.id, target_user=self.request.user, event=AuditLogEntryEvent.MEMBER_ACCEPT, data=om.get_audit_log_data())
            self.handle_success()
