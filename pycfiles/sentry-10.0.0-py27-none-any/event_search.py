...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/api/event_search.py
# Compiled at: 2019-09-06 12:12:44
...
from sentry.search.utils import parse_datetime_range, parse_datetime_string, parse_datetime_value, InvalidQuery
...
SEARCH_MAP = dict({'start': 'start', 
   'end': 'end', 
   'project_id': 'project_id', 
   'first_seen': 'first_seen', 
   'last_seen': 'last_seen', 
   'times_seen': 'times_seen'}, **SENTRY_SNUBA_MAP)
...
class SearchFilter(namedtuple('SearchFilter', 'key operator value')):
    ...
    def is_negation(self):
        return self.operator == '!=' and self.value.raw_value != '' or self.operator == '=' and self.value.raw_value == ''
...
class SearchVisitor(NodeVisitor):
    ...
    def visit_boolean_term(self, node, children):
        ...
        return [
         build_boolean_tree(children, 0, len(children))]
    ...
    def visit_numeric_filter(self, node, children):
        ...
        operator = (isinstance(operator, Node) or operator)[0] if 1 else '='
        if search_key.name in self.numeric_keys:
            ...
            search_value = SearchValue(operator + search_value.text if operator != '=' else search_value.text)
    ...
    def visit_has_filter(self, node, children):
        ...
        if isinstance(search_key, SearchValue):
            raise InvalidSearchQuery('Invalid format for "has" search: %s' % (search_key.raw_value,))
        operator = '=' if self.is_negated(negation) else '!='
...
def convert_search_boolean_to_snuba_query(search_boolean):
    ...
    def convert_term(term):
        ...
        raise InvalidSearchQuery('Attempted to convert term of unrecognized type %s into a snuba expression' % term.__class__.__name__)
...
def convert_search_filter_to_snuba_query(search_filter):
    ...
    if snuba_name in no_conversion:
        ...
        if snuba_name == 'tags[environment]':
            env_conditions = []
        ...
        if snuba_name == 'message':
            if search_filter.value.is_wildcard():
                value = search_filter.value.value[1:-1]
                return [
                 [
                  'match', ['message', "'(?i)%s'" % (value,)]], search_filter.operator, 1]
            else:
            ...
            value = int(to_timestamp(value)) * 1000 if isinstance(value, datetime) and snuba_name != 'timestamp' else value
            ...
            if search_filter.operator in ('=', '!=') and search_filter.value.value == '':
                if search_filter.key.is_tag:
                    ...
                    return [
                     [
                      'isNull', [snuba_name]], search_filter.operator, 1]
            ...
            if search_filter.value.is_wildcard():
                ...
                condition = [
                 snuba_name, search_filter.operator, value]
...
def get_snuba_query_args(query=None, params=None):
    ...
    if query is not None:
        try:
            ...
            raise InvalidSearchQuery('Parse error: %r (column %d)' % (e.expr.name, e.column()))

    if params is not None:
        parsed_terms.extend(convert_endpoint_params(params))
    kwargs = {'conditions': [], 'filter_keys': defaultdict(list)}
    projects = {}
    has_project_term = any(isinstance(term, SearchFilter) and term.key.name == PROJECT_KEY for term in parsed_terms)
    if has_project_term:
        projects = {p['slug']:p['id'] for p in Project.objects.filter(id__in=params['project_id']).values('id', 'slug')}
    for term in parsed_terms:
        if isinstance(term, SearchFilter):
            snuba_name = term.key.snuba_name
            if term.key.name == PROJECT_KEY:
                condition = [
                 'project_id', '=', projects.get(term.value.value)]
...
FIELD_ALIASES = {'last_seen': {'aggregations': [['max', 'timestamp', 'last_seen']]}, 'latest_event': {'aggregations': [
                                   [
                                    'argMax(event_id, timestamp)', '', 'latest_event']]}, 
   'project': {'fields': ['project.id']}, 'user': {'fields': ['user.id', 'user.name', 'user.username', 'user.email', 'user.ip']}}
VALID_AGGREGATES = {'count_unique': {'snuba_name': 'uniq', 'fields': '*'}, 'count': {'snuba_name': 'count', 'fields': '*'}, 'min': {'snuba_name': 'min', 'fields': ['timestamp', 'duration']}, 'max': {'snuba_name': 'max', 'fields': ['timestamp', 'duration']}, 'sum': {'snuba_name': 'sum', 'fields': ['duration']}, 'avg': {'snuba_name': 'avg', 'fields': ['duration']}, 'p75': {'snuba_name': 'quantileTiming(0.75)', 'fields': ['duration']}}
AGGREGATE_PATTERN = re.compile('^(?P<function>[^\\(]+)\\((?P<column>[a-z\\._]*)\\)$')
...
def validate_aggregate(field, match):
    ...
    if column not in function_data['fields'] and function_data['fields'] != '*':
        raise InvalidSearchQuery("Invalid column '%s' in aggregate function '%s'" % (column, function_name))
...
def find_reference_event(snuba_args, reference_event_slug):
    ...
    try:
        project = Project.objects.get(slug=project_slug, id__in=snuba_args['filter_keys']['project_id'])
