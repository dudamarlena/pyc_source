...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/integrations/vsts/issues.py
...
class VstsIssueSync(IssueSyncMixin):
    ...
    def get_project_choices(self, group, **kwargs):
        ...
        return (
         default_project, project_choices)
    ...
    def get_create_issue_config(self, group, **kwargs):
        ...
        return [
         {'name': 'project', 
            ...
            'label': _('Project'), 
            'placeholder': default_project or _('MyProject')}] + fields
    ...
    def create_issue(self, data, **kwargs):
        ...
        try:
            created_item = client.create_work_item(instance=self.instance, project=project_id, title=title, description=markdown(description), comment=markdown(description))
        except Exception as e:
            self.raise_error(e)

        project_name = created_item['fields']['System.AreaPath']
        return {'key': six.text_type(created_item['id']), 
           'title': title, 
    ...
    def get_issue(self, issue_id, **kwargs):
        ...
        return {'key': six.text_type(work_item['id']), 
           'title': work_item['fields']['System.Title'], 
           'description': work_item['fields'].get('System.Description'), 
           'metadata': {'display_name': '%s#%s' % (work_item['fields']['System.AreaPath'], work_item['id'])}}
    ...
    def sync_assignee_outbound(self, external_issue, user, assign=True, **kwargs):
        ...
        if assign is True:
            ...
            if assignee is None:
                self.logger.info('vsts.assignee-not-found', extra={'integration_id': external_issue.integration_id, 
                   'user_id': user.id, 
        ...
        try:
            ...
            self.logger.info('vsts.failed-to-assign', extra={'integration_id': external_issue.integration_id, 
               'user_id': user.id, 
               'issue_key': external_issue.key})
    ...
    def sync_status_outbound(self, external_issue, is_resolved, project_id, **kwargs):
        ...
        try:
            external_project = IntegrationExternalProject.objects.get(external_id=vsts_project_id, organization_integration_id__in=OrganizationIntegration.objects.filter(organization_id=external_issue.organization_id, integration_id=external_issue.integration_id))
        except IntegrationExternalProject.DoesNotExist:
            self.logger.info('vsts.external-project-not-found', extra={'integration_id': external_issue.integration_id, 
               'is_resolved': is_resolved, 
               'issue_key': external_issue.key})
            return

        status = external_project.resolved_status if is_resolved else external_project.unresolved_status
        try:
            client.update_work_item(self.instance, external_issue.key, state=status)
        except (ApiUnauthorized, ApiError) as error:
            self.logger.info('vsts.failed-to-change-status', extra={'integration_id': external_issue.integration_id, 
               'is_resolved': is_resolved, 
               'issue_key': external_issue.key, 
               'exception': error})
    ...
    def should_unresolve(self, data):
        ...
        return data['old_state'] in done_states or data['old_state'] is None and data['new_state'] not in done_states

    def should_resolve(self, data):
        done_states = self.get_done_states(data['project'])
        return data['old_state'] not in done_states and data['new_state'] in done_states
    ...
    def get_done_states(self, project):
        ...
        try:
            ...
            self.logger.info('vsts.get-done-states.failed', extra={'integration_id': self.model.id, 'exception': err})
            return []

        done_states = [ state['name'] for state in all_states if state['category'] in self.done_categories ]
        return done_states
