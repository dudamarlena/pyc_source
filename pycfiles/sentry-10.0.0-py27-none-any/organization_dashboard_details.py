...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_dashboard_details.py
...
class DashboardWithWidgetsSerializer(serializers.Serializer):
    ...
    def validate_widgets(self, widgets):
        ...
        widgets_count = Widget.objects.filter(id__in=[ w['id'] for w in widgets ], dashboard_id=self.context['dashboard_id'], status=ObjectStatus.VISIBLE).count()
        if len(widgets) != widgets_count:
            raise ValidationError('All widgets must exist within this dashboard prior to reordering.')
        return widgets
...
class OrganizationDashboardDetailsEndpoint(OrganizationDashboardEndpoint):
    ...
    def put(self, request, organization, dashboard):
        ...
        :auth: required
        """
        serializer = DashboardWithWidgetsSerializer(data=request.data, context={'dashboard_id': dashboard.id})
