...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/auth/access.py
...
from sentry.models import AuthIdentity, AuthProvider, OrganizationMember, Project, ProjectStatus, SentryApp, UserPermission
    ...
    if auth_provider.flags.allow_unlinked:
        ...
        try:
            ...
            if member.role == roles.get_top_dog().id:
                requires_sso = AuthIdentity.objects.filter(auth_provider=auth_provider, user__in=OrganizationMember.objects.filter(organization=member.organization_id, role=roles.get_top_dog().id, user__is_active=True).exclude(id=member.id).values_list('user_id')).exists()
        else:
            sso_is_valid = auth_identity.is_valid(member)

    return (
     requires_sso, sso_is_valid)
...
class Access(BaseAccess):
    ...
    def __init__(self, scopes, is_active, organization_id, teams, projects, has_global_access, sso_is_valid, requires_sso, permissions=None):
...
def from_request(request, organization=None, scopes=None):
    if not organization:
        ...
        if is_active_superuser(request):
            ...
            team_list = ()
            project_list = ()
            return Access(scopes=scopes if scopes is not None else settings.SENTRY_SCOPES, is_active=True, organization_id=organization.id if organization else None, teams=team_list, projects=project_list, sso_is_valid=sso_is_valid, requires_sso=requires_sso, has_global_access=True, permissions=UserPermission.for_user(request.user.id))
...
def from_sentry_app(user, organization=None):
    if not organization:
        ...
        project_list = list(Project.objects.filter(status=ProjectStatus.VISIBLE, teams__in=team_list).distinct())
        return Access(scopes=sentry_app.scope_list, is_active=True, organization_id=organization.id if organization else None, teams=team_list, projects=project_list, permissions=(), has_global_access=False, sso_is_valid=True, requires_sso=False)
