...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/useroption.py
...
def project_metakey(user, project):
    return (
     user.pk, project.pk, 'project')


def organization_metakey(user, organization):
    return (
     user.pk, organization.pk, 'organization')
...
class UserOptionManager(BaseManager):
    ...
    def set_value(self, user, key, value, **kwargs):
        ...
        if organization and project:
            raise NotImplementedError(option_scope_error)
        inst, created = self.get_or_create(user=user, project=project, organization=organization, key=key, defaults={'value': value})
    ...
    def get_all_values(self, user, project=None, organization=None):
        ...
        if project:
            ...
            metakey = user_metakey(user)
        if metakey not in self.__metadata:
            result = dict((i.key, i.value) for i in self.filter(user=user, project=project, organization=organization))
