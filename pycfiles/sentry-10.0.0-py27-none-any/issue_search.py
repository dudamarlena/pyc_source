...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/issue_search.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
from django.utils.functional import cached_property
from parsimonious.exceptions import IncompleteParseError
from sentry.api.event_search import event_search_grammar, InvalidSearchQuery, SearchFilter, SearchKey, SearchValue, SearchVisitor
...
class IssueSearchVisitor(SearchVisitor):
    ...
    def is_filter_translators(self):
        is_filter_translators = {'assigned': (
                      SearchKey('unassigned'), SearchValue(False)), 
           'unassigned': (
                        SearchKey('unassigned'), SearchValue(True))}
    ...
    def visit_is_filter(self, node, children):
        ...
        if search_value.raw_value not in self.is_filter_translators:
            raise InvalidSearchQuery(('Invalid value for "is" search, valid values are {}').format(sorted(self.is_filter_translators.keys())))
...
value_converters = {'assigned_to': convert_actor_value, 
