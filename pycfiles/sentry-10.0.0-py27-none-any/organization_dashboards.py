...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_dashboards.py
...
class OrganizationDashboardsEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        return self.paginate(request=request, queryset=dashboards, order_by='title', paginator_cls=OffsetPaginator, on_results=lambda x: serialize(x, request.user))
    ...
    def post(self, request, organization):
        ...
        try:
            with transaction.atomic():
                dashboard = Dashboard.objects.create(organization_id=organization.id, title=result['title'], created_by=request.user)
