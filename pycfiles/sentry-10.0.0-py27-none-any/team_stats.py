...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/team_stats.py
...
def retrieve_event_counts_team(runner):
    runner.request(method='GET', path='/teams/%s/%s/stats/' % (runner.org.slug, runner.default_team.slug))
...
class TeamStatsEndpoint(TeamEndpoint, EnvironmentMixin, StatsMixin):
    ...
    def get(self, request, team):
        ...
        if not projects:
            return Response([])
        data = list(tsdb.get_range(model=tsdb.models.project, keys=[ p.id for p in projects ], **self._parse_args(request, environment_id)).values())
        summarized = []
