...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/digests/utilities.py
...
def build_custom_digest(original_digest, events):
    ...
    for rule, rule_groups in six.iteritems(original_digest):
        ...
        for group, group_records in six.iteritems(rule_groups):
            user_group_records = [ record for record in group_records if record.value.event in events ]
...
def team_actors_to_user_ids(team_actors, user_ids):
    ...
    members = OrganizationMemberTeam.objects.filter(team_id__in=team_ids, is_active=True, organizationmember__user_id__in=user_ids).select_related('organizationmember')
