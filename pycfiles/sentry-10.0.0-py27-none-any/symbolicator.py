...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/lang/native/symbolicator.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import sys, jsonschema, logging, six, time
...
VALID_FILE_TYPES = ('pe', 'pdb', 'mach_debug', 'mach_code', 'elf_debug', 'elf_code',
                    'breakpad')
VALID_CASINGS = ('lowercase', 'uppercase', 'default')
LAYOUT_SCHEMA = {'type': 'object', 
   'properties': {'type': {'type': 'string', 'enum': list(VALID_LAYOUTS)}, 'casing': {'type': 'string', 'enum': list(VALID_CASINGS)}}, 'required': [
              'type'], 
   'additionalProperties': False}
COMMON_SOURCE_PROPERTIES = {'id': {'type': 'string', 'minLength': 1}, 'name': {'type': 'string'}, 'layout': LAYOUT_SCHEMA, 
   'filetypes': {'type': 'array', 'items': {'type': 'string', 'enum': list(VALID_FILE_TYPES)}}}
HTTP_SOURCE_SCHEMA = {'type': 'object', 
   'properties': dict(type={'type': 'string', 'enum': ['http']}, url={'type': 'string'}, **COMMON_SOURCE_PROPERTIES), 
   'required': [
              'type', 'id', 'url', 'layout'], 
   'additionalProperties': False}
S3_SOURCE_SCHEMA = {'type': 'object', 
   'properties': dict(type={'type': 'string', 'enum': ['s3']}, bucket={'type': 'string'}, region={'type': 'string'}, access_key={'type': 'string'}, secret_key={'type': 'string'}, prefix={'type': 'string'}, **COMMON_SOURCE_PROPERTIES), 
   'required': [
              'type', 'id', 'bucket', 'region', 'access_key', 'secret_key', 'layout'], 
   'additionalProperties': False}
GCS_SOURCE_SCHEMA = {'type': 'object', 
   'properties': dict(type={'type': 'string', 'enum': ['gcs']}, bucket={'type': 'string'}, client_email={'type': 'string'}, private_key={'type': 'string'}, prefix={'type': 'string'}, **COMMON_SOURCE_PROPERTIES), 
   'required': [
              'type', 'id', 'bucket', 'client_email', 'private_key', 'layout'], 
   'additionalProperties': False}
SOURCES_SCHEMA = {'type': 'array', 
...
class Symbolicator(object):
    ...
    def __init__(self, project, event_id):
        ...
        assert base_url
        self.sess = SymbolicatorSession(url=base_url, project_id=six.text_type(project.id), event_id=six.text_type(event_id), timeout=SYMBOLICATOR_TIMEOUT, sources=get_sources_for_project(project))
    ...
    def _process(self, create_task):
        ...
        with self.sess:
            ...
            metrics.incr('events.symbolicator.response', tags={'response': json.get('status') or 'null', 'project_id': self.sess.project_id})
    ...
    def process_payload(self, stacktraces, modules, signal=None):
        return self._process(lambda : self.sess.symbolicate_stacktraces(stacktraces=stacktraces, modules=modules, signal=signal))
...
def get_internal_source(project):
    ...
    if not internal_url_prefix:
        ...
        if sys.platform == 'darwin':
            internal_url_prefix = internal_url_prefix.replace('localhost', 'host.docker.internal').replace('127.0.0.1', 'host.docker.internal')
    assert internal_url_prefix
    sentry_source_url = '%s%s' % (
     internal_url_prefix.rstrip('/'),
     reverse('sentry-api-0-dsym-files', kwargs={'organization_slug': project.organization.slug, 'project_slug': project.slug}))
    return {'type': 'sentry', 
       'id': 'sentry:project', 
    ...
    def _request(self, method, path, **kwargs):
        ...
        while True:
            try:
                ...
                metrics.incr('events.symbolicator.status_code', tags={'status_code': response.status_code, 'project_id': self.project_id})
