...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/user_details.py
...
class UserOptionsSerializer(serializers.Serializer):
    ...
    stacktraceOrder = serializers.ChoiceField(choices=(
     ...
     (
      '2', _('Most recent call first'))), required=False)
...
class UserDetailsEndpoint(UserEndpoint):
    ...
    def delete(self, request, user):
        ...
        org_list = Organization.objects.filter(member_set__role__in=[ x.id for x in roles.with_scope('org:admin') ], member_set__user=user, status=OrganizationStatus.VISIBLE)
        org_results = []
        for org in org_list:
        ...
        remaining_org_ids = [ o.id for o in org_list if o.slug in avail_org_slugs.difference(orgs_to_remove) ]
        if remaining_org_ids:
            OrganizationMember.objects.filter(organization__in=remaining_org_ids, user=user).delete()
        ...
        if hard_delete and not is_active_superuser(request):
            return Response({'detail': 'Only superusers may hard delete a user account'}, status=status.HTTP_403_FORBIDDEN)
