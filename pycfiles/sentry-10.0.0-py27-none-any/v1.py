...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/base/v1.py
...
class IPlugin(local, PluggableViewMixin, PluginConfigMixin, PluginStatusMixin):
    ...
    def get_url(self, group):
        ...
        return reverse('sentry-group-plugin-action', args=(
         group.organization.slug, group.project.slug, group.pk, self.slug))
    ...
    def get_conf_version(self, project):
        ...
        return md5_text(('&').join(sorted('%s=%s' % o for o in six.iteritems(options)))).hexdigest()[:3]
    ...
    def get_view_response(self, request, group):
        ...
        return response.respond(request, {'plugin': self, 
    ...
    def view_configure(self, request, project, **kwargs):
        if request.method == 'GET':
            return Response(self.get_configure_plugin_fields(request=request, project=project, **kwargs))
