...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_event_details.py
...
class OrganizationEventDetailsEndpoint(OrganizationEventsEndpointBase):
    ...
    def get(self, request, organization, project_slug, event_id):
        if not features.has('organizations:events-v2', organization, actor=request.user):
            ...
            snuba_args['conditions'].extend(get_reference_event_conditions(snuba_args, event.snuba_data))
            next_event = eventstore.get_next_event_id(event, conditions=snuba_args['conditions'], filter_keys=snuba_args['filter_keys'])
            prev_event = eventstore.get_prev_event_id(event, conditions=snuba_args['conditions'], filter_keys=snuba_args['filter_keys'])
    ...
    def get(self, latest_or_oldest, request, organization):
        if not features.has('organizations:events-v2', organization, actor=request.user):
            ...
            if latest_or_oldest == EventOrdering.LATEST:
                ...
                orderby = [
                 'timestamp', 'event_id']
            result = raw_query(start=snuba_args['start'], end=snuba_args['end'], selected_columns=SnubaEvent.selected_columns, conditions=snuba_args['conditions'], filter_keys=snuba_args['filter_keys'], orderby=orderby, limit=2, referrer='api.organization-event-details-latest-or-oldest')
...
class OrganizationEventDetailsLatestEndpoint(OrganizationEventsLatestOrOldest):
    ...
    def get(self, request, organization):
        return super(OrganizationEventDetailsLatestEndpoint, self).get(EventOrdering.LATEST, request, organization)


class OrganizationEventDetailsOldestEndpoint(OrganizationEventsLatestOrOldest):

    def get(self, request, organization):
        return super(OrganizationEventDetailsOldestEndpoint, self).get(EventOrdering.OLDEST, request, organization)