...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_event_details.py
...
def retrieve_event_for_project_scenario(runner):
    runner.request(method='GET', path='/projects/%s/%s/events/%s/' % (
    ...
    def get(self, request, project, event_id):
        ...
        if event is None:
            ...
            if event.group_id:
                ...
                next_event = eventstore.get_next_event_id(event, conditions=conditions, filter_keys=filter_keys)
                prev_event = eventstore.get_prev_event_id(event, conditions=conditions, filter_keys=filter_keys)
