...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/auth/providers/saml2.py
...
class SAML2AcceptACSView(BaseView):
    ...
    def dispatch(self, request, organization_slug):
        ...
        helper = AuthHelper(request=request, organization=organization, auth_provider=auth_provider, flow=AuthHelper.FLOW_LOGIN)
...
class SAML2Provider(Provider):
    ...
    def build_identity(self, state):
        ...
        return {'id': attributes[Attributes.IDENTIFIER], 
...
def build_saml_config(provider_config, org):
    ...
    security_config = {'authnRequestsSigned': avd.get('authn_request_signed', False), 
    ...
    if idp is not None:
        saml_config['idp'] = {'entityId': idp['entity_id'], 'x509cert': idp['x509cert'], 
           'singleSignOnService': {'url': idp['sso_url']}, 'singleLogoutService': {'url': idp['slo_url']}}
...
def build_auth(request, saml_config):
    ...
    saml_request = {'https': 'on' if url.scheme == 'https' else 'off', 
