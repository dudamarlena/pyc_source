...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_access_request_details.py
...
class OrganizationAccessRequestDetailsEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        """
        if request.access.has_scope('org:write'):
            access_requests = list(OrganizationAccessRequest.objects.filter(team__organization=organization, member__user__is_active=True).select_related('team', 'member__user'))
        elif request.access.has_scope('team:write') and request.access.teams:
            access_requests = list(OrganizationAccessRequest.objects.filter(member__user__is_active=True, team__in=request.access.teams).select_related('team', 'member__user'))
        else:
            return Response([])
        ...
        if not self._can_access(request, access_request):
            ...
            if is_approved:
                try:
                    ...
                    self.create_audit_entry(request=request, organization=organization, target_object=omt.id, target_user=access_request.member.user, event=AuditLogEntryEvent.MEMBER_JOIN_TEAM, data=omt.get_audit_log_data())
