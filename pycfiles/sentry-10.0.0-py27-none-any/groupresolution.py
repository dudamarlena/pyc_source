...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/groupresolution.py
...
class GroupResolution(Model):
    ...
    type = BoundedPositiveIntegerField(choices=(
     (
      Type.in_next_release, 'in_next_release'), (Type.in_release, 'in_release')), null=True)
    actor_id = BoundedPositiveIntegerField(null=True)
    datetime = models.DateTimeField(default=timezone.now, db_index=True)
    status = BoundedPositiveIntegerField(default=Status.pending, choices=(
     (
      Status.pending, _('Pending')), (Status.resolved, _('Resolved'))))
    ...
    def has_resolution(cls, group, release):
        ...
        """
        try:
            res_type, res_release, res_release_datetime = cls.objects.filter(group=group).select_related('release').values_list('type', 'release__id', 'release__date_added')[0]
        except IndexError:
            return False
        ...
        if not release:
            ...
            if res_type in (None, cls.Type.in_next_release):
                ...
                if res_release_datetime > release.date_added:
                    return True
                return False
            if res_type == cls.Type.in_release:
                if res_release == release.id:
                    return False
                if res_release_datetime < release.date_added:
                    return False
