...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/bases/notify.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import, print_function
import logging, six, urllib2
...
class NotificationPlugin(Plugin):
    slug = ''
    description = 'Notify project members when a new event is seen for the first time, or when an already resolved event has changed back to unresolved.'
    ...
    def notify(self, notification):
        ...
        try:
            return self.notify_users(event.group, event, triggering_rules=[ r.label for r in notification.rules ])
        except (SSLError, HTTPError, ApiError, PluginError, urllib2.HTTPError) as err:
            self.logger.info('notification-plugin.notify-failed', extra={'error': six.text_type(err), 
               'plugin': self.slug, 
    ...
    def rule_notify(self, event, futures):
        ...
        if hasattr(self, 'notify_digest') and digests.enabled(project):
            ...
            def get_digest_option(key):
                return ProjectOption.objects.get_value(project, get_digest_option_key(self.get_conf_key(), key))

            digest_key = unsplit_key(self, event.group.project)
            extra['digest_key'] = digest_key
            immediate_delivery = digests.add(digest_key, event_to_record(event, rules), increment_delay=get_digest_option('increment_delay'), maximum_delay=get_digest_option('maximum_delay'))
