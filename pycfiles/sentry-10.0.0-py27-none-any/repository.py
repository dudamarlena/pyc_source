...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/providers/repository.py
...
class RepositoryProvider(ProviderMixin):
    ...
    def dispatch(self, request, organization, **kwargs):
        if self.needs_auth(request.user):
            return Response({'error_type': 'auth', 
               'auth_url': reverse('socialauth_associate', args=[self.auth_provider])}, status=400)
        try:
        ...
        try:
            with transaction.atomic():
                repo = Repository.objects.create(organization_id=organization.id, name=result['name'], external_id=result.get('external_id'), url=result.get('url'), config=result.get('config') or {}, provider=self.id)
        except IntegrityError:
            try:
                repo = Repository(organization_id=organization.id, name=result['name'], external_id=result.get('external_id'), url=result.get('url'), config=result.get('config') or {}, provider=self.id)
