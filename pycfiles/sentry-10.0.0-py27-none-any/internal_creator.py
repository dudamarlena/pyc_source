...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/mediators/sentry_apps/internal_creator.py
...
class InternalCreator(Mediator):
    ...
    def _install(self):
        self.install = InstallationCreator.run(organization=self.organization, slug=self.sentry_app.slug, user=self.user, request=self.request, notify=False)

    def audit(self):
        from sentry.utils.audit import create_audit_entry
        if self.request:
            create_audit_entry(request=self.request, organization=self.organization, target_object=self.organization.id, event=AuditLogEntryEvent.INTERNAL_INTEGRATION_ADD)

    def record_analytics(self):
        from sentry import analytics
        analytics.record('internal_integration.created', user_id=self.user.id, organization_id=self.organization.id, sentry_app=self.sentry_app.slug)