...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_group_index.py
...
from sentry.api.helpers.group_index import build_query_params_from_request, delete_groups, get_by_short_id, update_groups, ValidationError
...
def bulk_update_issues_scenario(runner):
    ...
    runner.request(method='PUT', path='/projects/%s/%s/issues/?id=%s&id=%s' % (
     runner.org.slug, project.slug, group1.id, group2.id), data={'status': 'unresolved', 'isPublic': False})
...
def bulk_remove_issues_scenario(runner):
    with runner.isolated_project('Amazing Plumbing') as (project):
        group1, group2 = Group.objects.filter(project=project)[:2]
        runner.request(method='DELETE', path='/projects/%s/%s/issues/?id=%s&id=%s' % (
...
def list_project_issues_scenario(runner):
    project = runner.default_project
    runner.request(method='GET', path='/projects/%s/%s/issues/?statsPeriod=24h' % (runner.org.slug, project.slug))


class ProjectGroupIndexEndpoint(ProjectEndpoint, EnvironmentMixin):
    doc_section = DocSection.EVENTS
    permission_classes = (
     ProjectEventPermission,)
    ...
    def _search(self, request, project, extra_query_kwargs=None):
        try:
            ...
            query_kwargs = build_query_params_from_request(request, project.organization, [project], environments)
                    ...
                    if event_id:
                        ...
                        if matching_group is not None:
                            ...
                            response = Response(serialize([
                             matching_group], request.user, serializer(matching_event_id=getattr(matching_event, 'id', None), matching_event_environment=matching_event_environment)))
