...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/debug_files.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import six, jsonschema, logging, posixpath
...
from sentry.tasks.assemble import get_assemble_status, set_assemble_status, AssembleTask, ChunkFileState
from sentry.utils import json
try:
    from django.http import CompatibleStreamingHttpResponse as StreamingHttpResponse, HttpResponse, Http404
except ImportError:
...
class DebugFilesEndpoint(ProjectEndpoint):
    ...
    def download(self, debug_file_id, project):
        rate_limited = ratelimits.is_limited(project=project, key='rl:DSymFilesEndpoint:download:%s:%s' % (debug_file_id, project.id), limit=10)
        if rate_limited:
            logger.info('notification.rate_limited', extra={'project_id': project.id, 'project_debug_file_id': debug_file_id})
            ...
            try:
                ...
                response = StreamingHttpResponse(iter(lambda : fp.read(4096), ''), content_type='application/octet-stream')
    ...
    def get(self, request, project):
        ...
        if download_requested and request.access.has_scope('project:write'):
            ...
            if debug_id:
                ...
                q = Q(object_name__icontains=query) | Q(debug_id__icontains=query) | Q(code_id__icontains=query) | Q(cpu_name__icontains=query) | Q(file__headers__icontains=query)
            ...
            return self.paginate(request=request, queryset=queryset, order_by='-id', paginator_cls=OffsetPaginator, default_per_page=20, on_results=lambda x: serialize(x, request.user))
    ...
    def delete(self, request, project):
        ...
        if request.GET.get('id') and request.access.has_scope('project:write'):
            with transaction.atomic():
                debug_file = ProjectDebugFile.objects.filter(id=request.GET.get('id'), project=project).select_related('file').first()
...
def find_missing_chunks(organization, chunks):
    ...
    owned = set(FileBlobOwner.objects.filter(blob__checksum__in=chunks, organization=organization).values_list('blob__checksum', flat=True))
...
class DifAssembleEndpoint(ProjectEndpoint):
    ...
    def post(self, request, project):
        ...
        """
        schema = {'type': 'object', 
           'patternProperties': {'^[0-9a-f]{40}$': {'type': 'object', 
                                                    'required': [
                                                               'name', 'chunks'], 
                                                    'properties': {'name': {'type': 'string'}, 'debug_id': {'type': 'string'}, 'chunks': {'type': 'array', 
        ...
        for checksum, file_to_assemble in six.iteritems(files):
            ...
            if state == ChunkFileState.OK:
                file_response[checksum] = {'state': state, 'detail': None, 
                   'missingChunks': [], 'dif': detail}
                continue
            elif state is not None:
                file_response[checksum] = {'state': state, 'detail': detail, 'missingChunks': []}
                continue
            dif = ProjectDebugFile.objects.filter(project=project, file__checksum=checksum).select_related('file').order_by('-id').first()
            if dif is not None:
                file_response[checksum] = {'state': ChunkFileState.OK, 'detail': None, 
                   'missingChunks': [], 'dif': serialize(dif)}
                continue
            if not chunks:
            ...
            if missing_chunks:
                file_response[checksum] = {'state': ChunkFileState.NOT_FOUND, 'missingChunks': missing_chunks}
                continue
            set_assemble_status(AssembleTask.DIF, project.id, checksum, ChunkFileState.CREATED)
            from sentry.tasks.assemble import assemble_dif
            assemble_dif.apply_async(kwargs={'project_id': project.id, 
               'name': name, 
               'debug_id': debug_id, 
               'checksum': checksum, 
               'chunks': chunks})
