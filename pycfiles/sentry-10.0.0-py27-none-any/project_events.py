...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_events.py
...
def list_project_available_samples_scenario(runner):
    runner.request(method='GET', path='/projects/%s/%s/events/' % (runner.org.slug, runner.default_project.slug))
...
class ProjectEventsEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project):
        ...
        data_fn = partial(eventstore.get_events, conditions=conditions, filter_keys={'project_id': [project.id]}, additional_columns=cols, referrer='api.project-events')
