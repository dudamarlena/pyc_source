...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_group_index.py
    ...
    def get(self, request, organization):
        ...
        if stats_period not in (None, '', '24h', '14d'):
            ...
            if stats_period is None:
                ...
                serializer = functools.partial(StreamGroupSerializerSnuba, environment_ids=[ env.id for env in environments ], stats_period=stats_period)
                projects = self.get_projects(request, organization)
                project_ids = [ p.id for p in projects ]
                if not projects:
                    return Response([])
                if len(projects) > 1 and not features.has('organizations:global-views', organization, actor=request.user):
                    return Response({'detail': 'You do not have the multi project stream feature enabled'}, status=400)
                query = request.GET.get('query', '').strip()
                if query:
                    event_id = normalize_event_id(query)
                    if event_id:
                        direct_hit_projects = set(project_ids) | set([ project.id for project in request.access.projects ])
                        groups = list(Group.objects.filter_by_event_id(direct_hit_projects, event_id))
                        if len(groups) == 1:
                            response = Response(serialize(groups, request.user, serializer(matching_event_id=event_id)))
