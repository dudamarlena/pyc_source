...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_member_details.py
...
from sentry.api.serializers import DetailedUserSerializer, serialize, RoleSerializer, OrganizationMemberWithTeamsSerializer
from sentry.api.serializers.rest_framework import ListField
from sentry.auth.superuser import is_active_superuser
from sentry.models import AuditLogEntryEvent, AuthIdentity, AuthProvider, OrganizationMember, OrganizationMemberTeam, Team, TeamStatus
...
def get_allowed_roles(request, organization, member=None):
    ...
    if can_admin and not is_active_superuser(request):
        acting_member = member or OrganizationMember.objects.get(user=request.user, organization=organization)
...
class OrganizationMemberDetailsEndpoint(OrganizationEndpoint):
    ...
    def _get_member(self, request, organization, member_id):
        if member_id == 'me':
            queryset = OrganizationMember.objects.filter(organization=organization, user__id=request.user.id, user__is_active=True)
        else:
            try:
                queryset = OrganizationMember.objects.filter(Q(user__is_active=True) | Q(user__isnull=True), organization=organization, id=member_id)
            except ValueError:
    ...
    def _is_only_owner(self, member):
        if member.role != roles.get_top_dog().id:
            return False
        queryset = OrganizationMember.objects.filter(organization=member.organization_id, role=roles.get_top_dog().id, user__isnull=False, user__is_active=True).exclude(id=member.id)
        if queryset.exists():
            return False
        return True
    ...
    def _serialize_member(self, member, request, allowed_roles=None):
        ...
        context['roles'] = serialize(roles.get_all(), serializer=RoleSerializer(), allowed_roles=allowed_roles)
        return context
    ...
    def put(self, request, organization, member_id):
        ...
        if not serializer.is_valid():
            ...
            if 'teams' in result:
                teams = list(Team.objects.filter(organization=organization, status=TeamStatus.VISIBLE, slug__in=result['teams']))
                ...
                with transaction.atomic():
                    ...
                    OrganizationMemberTeam.objects.bulk_create([ OrganizationMemberTeam(team=team, organizationmember=om) for team in teams ])
            if result.get('role'):
                ...
                if result['role'] not in allowed_role_ids:
                    return Response({'role': 'You do not have permission to assign the given role.'}, status=403)
                if om.role not in allowed_role_ids:
                    return Response({'role': 'You do not have permission to assign a role to the given user.'}, status=403)
            ...
            self.create_audit_entry(request=request, organization=organization, target_object=om.id, target_user=om.user, event=AuditLogEntryEvent.MEMBER_EDIT, data=om.get_audit_log_data())
    ...
    def delete(self, request, organization, member_id):
        ...
        if request.user.is_authenticated() and not is_active_superuser(request):
            try:
                acting_member = OrganizationMember.objects.get(organization=organization, user=request.user)
            except OrganizationMember.DoesNotExist:
            ...
            if acting_member != om:
                if not request.access.has_scope('member:admin'):
                    return Response({'detail': ERR_INSUFFICIENT_SCOPE}, status=400)
                if not roles.can_manage(acting_member.role, om.role):
                    return Response({'detail': ERR_INSUFFICIENT_ROLE}, status=400)
        ...
        with transaction.atomic():
            AuthIdentity.objects.filter(user=om.user, auth_provider__organization=organization).delete()
            om.delete()
        self.create_audit_entry(request=request, organization=organization, target_object=om.id, target_user=om.user, event=AuditLogEntryEvent.MEMBER_REMOVE, data=audit_data)
