...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tagstore/v2/models/grouptagkey.py
    ...
    def merge_counts(self, new_group):
        ...
        try:
            with transaction.atomic(using=router.db_for_write(GroupTagKey)):
                GroupTagKey.objects.filter(group_id=new_group.id, project_id=new_group.project_id, _key_id=self._key_id).update(values_seen=GroupTagValue.objects.filter(group_id=new_group.id, project_id=new_group.project_id, _key_id=self._key_id).count())
        except DataError:
            pass