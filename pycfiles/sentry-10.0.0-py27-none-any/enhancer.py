...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/grouping/enhancer.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import os, six, base64, msgpack, inspect
    ...
    def matches_frame(self, frame_data, platform):
        if self.key in ('path', 'package'):
            ...
            if glob_match(value, self.pattern, ignorecase=True, doublestar=True, path_normalize=True):
                return True
            if not value.startswith('/') and glob_match('/' + value, self.pattern, ignorecase=True, doublestar=True, path_normalize=True):
                return True
            return False
        if self.key == 'family':
...
class Action(object):
    ...
    def _from_config_structure(cls, val):
        ...
        return FlagAction(ACTIONS[(val & 15)], flag, range)
...
class FlagAction(Action):
    ...
    def __str__(self):
        return '%s%s%s' % ({'up': '^', 'down': 'v'}.get(self.range, ''),
         self.flag and '+' or '-',
         self.key)
    ...
    def update_frame_components_contributions(self, components, frames, idx, rule=None):
        ...
        for component, frame in izip(sliced_components, sliced_frames):
            if self.key == 'group' and self.flag != component.contributes:
                component.update(contributes=self.flag, hint='%s by %s' % (self.flag and 'un-ignored' or 'ignored', rule_hint))
            elif self.key == 'app' and self._in_app_changed(frame, component):
                component.update(hint='marked %s by %s' % (self.flag and 'in-app' or 'out of app', rule_hint))
...
class Enhancements(object):
    ...
    def assemble_stacktrace_component(self, components, frames, platform):
        ...
        if min_frames > 0:
            ...
            if 0 < total_contributes < min_frames:
                hint = 'discarded because stacktrace only contains %d frame%s which is under the configured threshold' % (
                 total_contributes, 's' if total_contributes != 1 else '')
                hint = stacktrace_state.add_to_hint(hint, var='min-frames')
                contributes = False
        return GroupingComponent(id='stacktrace', values=components, hint=hint, contributes=contributes)

    def as_dict(self, with_rules=False):
        rv = {'id': self.id, 
           'changelog': self.changelog, 
           'bases': self.bases, 
           'latest': projectoptions.lookup_well_known_key('sentry:grouping_enhancements_base').get_default(epoch=projectoptions.LATEST_EPOCH) == self.id}
        if with_rules:
    ...
    def _to_config_structure(self):
        return [
         self.version, self.bases, [ x._to_config_structure() for x in self.rules ]]

    def dumps(self):
        return base64.urlsafe_b64encode(msgpack.dumps(self._to_config_structure()).encode('zlib')).strip('=')
    ...
    def _from_config_structure(cls, data):
        ...
        return cls(rules=[ Rule._from_config_structure(x) for x in rules ], version=version, bases=bases)
    ...
    def loads(cls, data):
        ...
        try:
            return cls._from_config_structure(msgpack.loads(base64.urlsafe_b64decode(padded).decode('zlib')))
    ...
    def from_config_string(self, s, bases=None, id=None):
        try:
            ...
            if len(context) == 33:
                context = context[:-1] + '...'
            raise InvalidEnhancerConfig('Invalid syntax near "%s" (line %s, column %s)' % (context, e.line(), e.column()))
...
class Rule(object):
    ...
    def _to_config_structure(self):
        return [[ x._to_config_structure() for x in self.matchers ], [ x._to_config_structure() for x in self.actions ]]

    @classmethod
    def _from_config_structure(cls, tuple):
        return Rule([ Match._from_config_structure(x) for x in tuple[0] ], [ Action._from_config_structure(x) for x in tuple[1] ])
...
class EnhancmentsVisitor(NodeVisitor):
    ...
    def visit_enhancements(self, node, children):
        ...
        return Enhancements(rules, inspect.cleandoc(('\n').join(changelog)).rstrip() or None, bases=self.bases, id=self.id)
