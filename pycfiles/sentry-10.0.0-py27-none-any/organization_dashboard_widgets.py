...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_dashboard_widgets.py
    ...
    def post(self, request, organization, dashboard):
        ...
        try:
            with transaction.atomic():
                widget = Widget.objects.create(display_type=result['displayType'], display_options=result.get('displayOptions', {}), title=result['title'], order=get_next_dashboard_order(dashboard.id), dashboard_id=dashboard.id)
                for widget_data in result.get('dataSources', []):
                    WidgetDataSource.objects.create(name=widget_data['name'], data=widget_data['data'], type=widget_data['type'], order=widget_data['order'], widget_id=widget.id)
