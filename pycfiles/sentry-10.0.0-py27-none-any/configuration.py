...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/base/configuration.py
...
def react_plugin_config(plugin, project, request):
    response = client.get(('/projects/{}/{}/plugins/{}/').format(project.organization.slug, project.slug, plugin.slug), request=request)
...
def default_plugin_config(plugin, project, request):
    ...
    if form_class is None:
        ...
        test_results = None
        form = form_class(request.POST if request.POST.get('plugin') == plugin.slug else None, initial=plugin.get_conf_options(project), prefix=plugin_key)
        if form.is_valid():
        ...
        is_configured = True
        return mark_safe(render_to_string(template, {'form': form, 
           'request': request, 
           'plugin': plugin, 
           'plugin_description': plugin.get_description() or '', 
           'plugin_test_results': test_results, 
           'plugin_is_configured': is_configured}, context_instance=RequestContext(request)))
