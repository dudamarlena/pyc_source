...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/auditlogentry.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import, print_function
import six
from django.db import models
from django.utils import timezone
from sentry.db.models import Model, BoundedPositiveIntegerField, FlexibleForeignKey, GzippedDictField, sane_repr
...
class AuditLogEntry(Model):
    ...
    def save(self, *args, **kwargs):
        if not (self.actor_label or self.actor or self.actor_key):
    ...
    def get_note(self):
        ...
        if self.event == AuditLogEntryEvent.MEMBER_JOIN_TEAM:
            if self.target_user == self.actor:
                return 'joined team %s' % (self.data['team_slug'],)
            return 'added %s to team %s' % (
             self.data.get('email') or self.target_user.get_display_name(),
             self.data['team_slug'])
        if self.event == AuditLogEntryEvent.MEMBER_LEAVE_TEAM:
            if self.target_user == self.actor:
                return 'left team %s' % (self.data['team_slug'],)
            return 'removed %s from team %s' % (
             self.data.get('email') or self.target_user.get_display_name(),
             self.data['team_slug'])
        if self.event == AuditLogEntryEvent.MEMBER_PENDING:
            return 'required member %s to setup 2FA' % (
             self.data.get('email') or self.target_user.get_display_name(),)
        if self.event == AuditLogEntryEvent.ORG_ADD:
            return 'created the organization'
        if self.event == AuditLogEntryEvent.ORG_EDIT:
            return 'edited the organization setting: ' + (', ').join(('{} {}').format(k, v) for k, v in self.data.items())
        if self.event == AuditLogEntryEvent.ORG_REMOVE:
            return 'removed the organization'
        if self.event == AuditLogEntryEvent.ORG_RESTORE:
            return 'restored the organization'
        if self.event == AuditLogEntryEvent.TEAM_ADD:
            return 'created team %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.TEAM_EDIT:
            return 'edited team %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.TEAM_REMOVE:
            return 'removed team %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.PROJECT_ADD:
            return 'created project %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.PROJECT_EDIT:
            return 'edited project settings ' + (' ').join([ ' in %s to %s' % (key, value) for key, value in six.iteritems(self.data) ])
        if self.event == AuditLogEntryEvent.PROJECT_REMOVE:
            return 'removed project %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.PROJECT_REQUEST_TRANSFER:
            return 'requested to transfer project %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.PROJECT_ACCEPT_TRANSFER:
            return 'accepted transfer of project %s' % (self.data['slug'],)
        if self.event == AuditLogEntryEvent.PROJECT_ENABLE:
        ...
        if self.event == AuditLogEntryEvent.PROJECT_DISABLE:
            if isinstance(self.data['state'], set):
                return 'disabled project filter %s' % (self.data['state'],)
            return 'disabled project filter %s' % ((', ').join(self.data['state']),)
        if self.event == AuditLogEntryEvent.TAGKEY_REMOVE:
            return 'removed tags matching %s = *' % (self.data['key'],)
        if self.event == AuditLogEntryEvent.PROJECTKEY_ADD:
            return 'added project key %s' % (self.data['public_key'],)
        if self.event == AuditLogEntryEvent.PROJECTKEY_EDIT:
            return 'edited project key %s' % (self.data['public_key'],)
        if self.event == AuditLogEntryEvent.PROJECTKEY_REMOVE:
            return 'removed project key %s' % (self.data['public_key'],)
        if self.event == AuditLogEntryEvent.PROJECTKEY_ENABLE:
            return 'enabled project key %s' % (self.data['public_key'],)
        if self.event == AuditLogEntryEvent.PROJECTKEY_DISABLE:
            return 'disabled project key %s' % (self.data['public_key'],)
        if self.event == AuditLogEntryEvent.SSO_ENABLE:
            return 'enabled sso (%s)' % (self.data['provider'],)
        if self.event == AuditLogEntryEvent.SSO_DISABLE:
            return 'disabled sso (%s)' % (self.data['provider'],)
        if self.event == AuditLogEntryEvent.SSO_EDIT:
            return 'edited sso settings: ' + (', ').join(('{} {}').format(k, v) for k, v in self.data.items())
        if self.event == AuditLogEntryEvent.SSO_IDENTITY_LINK:
            return 'linked their account to a new identity'
        if self.event == AuditLogEntryEvent.APIKEY_ADD:
            return 'added api key %s' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.APIKEY_EDIT:
            return 'edited api key %s' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.APIKEY_REMOVE:
            return 'removed api key %s' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.RULE_ADD:
            return 'added rule "%s"' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.RULE_EDIT:
            return 'edited rule "%s"' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.RULE_REMOVE:
            return 'removed rule "%s"' % (self.data['label'],)
        if self.event == AuditLogEntryEvent.SET_ONDEMAND:
        ...
        if self.event == AuditLogEntryEvent.TRIAL_STARTED:
            return 'started trial'
        if self.event == AuditLogEntryEvent.PLAN_CHANGED:
            return 'changed plan to %s' % (self.data['plan_name'],)
        if self.event == AuditLogEntryEvent.PLAN_CANCELLED:
            return 'cancelled plan'
        if self.event == AuditLogEntryEvent.SERVICEHOOK_ADD:
            return 'added a service hook for "%s"' % (truncatechars(self.data['url'], 64),)
        if self.event == AuditLogEntryEvent.SERVICEHOOK_EDIT:
            return 'edited the service hook for "%s"' % (truncatechars(self.data['url'], 64),)
        if self.event == AuditLogEntryEvent.SERVICEHOOK_REMOVE:
            return 'removed the service hook for "%s"' % (truncatechars(self.data['url'], 64),)
        if self.event == AuditLogEntryEvent.SERVICEHOOK_ENABLE:
            return 'enabled theservice hook for "%s"' % (truncatechars(self.data['url'], 64),)
        if self.event == AuditLogEntryEvent.SERVICEHOOK_DISABLE:
            return 'disabled the service hook for "%s"' % (truncatechars(self.data['url'], 64),)
        if self.event == AuditLogEntryEvent.INTEGRATION_ADD:
            return 'enabled integration %s for project %s' % (
             self.data['integration'],
             self.data['project'])
        if self.event == AuditLogEntryEvent.INTEGRATION_EDIT:
            return 'edited integration %s for project %s' % (
             self.data['integration'],
             self.data['project'])
        if self.event == AuditLogEntryEvent.INTEGRATION_REMOVE:
            return 'disabled integration %s from project %s' % (
             self.data['integration'],
             self.data['project'])
        if self.event == AuditLogEntryEvent.SENTRY_APP_ADD:
            return 'created sentry app %s' % self.data['sentry_app']
        if self.event == AuditLogEntryEvent.SENTRY_APP_REMOVE:
            return 'removed sentry app %s' % self.data['sentry_app']
        if self.event == AuditLogEntryEvent.SENTRY_APP_INSTALL:
            return 'installed sentry app %s' % self.data['sentry_app']
        if self.event == AuditLogEntryEvent.SENTRY_APP_UNINSTALL:
            return 'uninstalled sentry app %s' % self.data['sentry_app']
        if self.event == AuditLogEntryEvent.INTERNAL_INTEGRATION_ADD_TOKEN:
            return 'created a token for internal integration %s' % self.data['sentry_app']
        if self.event == AuditLogEntryEvent.INTERNAL_INTEGRATION_REMOVE_TOKEN:
