...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/team_details.py
...
def update_team_scenario(runner):
    ...
    runner.request(method='PUT', path='/teams/%s/%s/' % (runner.org.slug, team.slug), data={'name': 'The Inflated Philosophers'})
...
class TeamDetailsEndpoint(TeamEndpoint):
    ...
    def put(self, request, team):
        ...
        if serializer.is_valid():
            ...
            self.create_audit_entry(request=request, organization=team.organization, target_object=team.id, event=AuditLogEntryEvent.TEAM_EDIT, data=team.get_audit_log_data())
    ...
    def delete(self, request, team):
        ...
        **Note:** Deletion happens asynchronously and therefor is not
        ...
        """
        updated = Team.objects.filter(id=team.id, status=TeamStatus.VISIBLE).update(status=TeamStatus.PENDING_DELETION)
        if updated:
            transaction_id = uuid4().hex
            self.create_audit_entry(request=request, organization=team.organization, target_object=team.id, event=AuditLogEntryEvent.TEAM_REMOVE, data=team.get_audit_log_data(), transaction_id=transaction_id)
            delete_team.apply_async(kwargs={'object_id': team.id, 'transaction_id': transaction_id})
            delete_logger.info('object.delete.queued', extra={'object_id': team.id, 
