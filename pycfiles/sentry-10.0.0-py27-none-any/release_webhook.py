...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/release_webhook.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import, print_function
from hashlib import sha256
import hmac, logging, six
...
class ReleaseWebhookView(View):
    ...
    def verify(self, plugin_id, project_id, token, signature):
        return constant_time_compare(signature, hmac.new(key=token.encode('utf-8'), msg=('{}-{}').format(plugin_id, project_id).encode('utf-8'), digestmod=sha256).hexdigest())
    ...
    def _handle_builtin(self, request, project):
        ...
        try:
            ...
            return HttpResponse(status=400, content=json.dumps({'error': six.text_type(exc)}), content_type='application/json')
    ...
    def post(self, request, plugin_id, project_id, signature):
        try:
            ...
            logger.warn('release-webhook.invalid-project', extra={'project_id': project_id, 'plugin_id': plugin_id})
        ...
        if token is None:
            logger.warn('release-webhook.missing-token', extra={'project_id': project_id, 'plugin_id': plugin_id})
            return HttpResponse(status=403)
        else:
            if not self.verify(plugin_id, project_id, token, signature):
                logger.warn('release-webhook.invalid-signature', extra={'project_id': project_id, 'plugin_id': plugin_id})
                return HttpResponse(status=403)
            if plugin_id == 'builtin':
                return self._handle_builtin(request, project)
            plugin = plugins.get(plugin_id)
            if not plugin.is_enabled(project):
                logger.warn('release-webhook.plugin-disabled', extra={'project_id': project_id, 'plugin_id': plugin_id})
                return HttpResponse(status=403)
