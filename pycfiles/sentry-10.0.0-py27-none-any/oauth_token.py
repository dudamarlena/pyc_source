...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/oauth_token.py
...
class OAuthTokenView(View):
    ...
    def error(self, request, name, reason=None, status=400):
        ...
        logging.error('oauth.token-error', extra={'error_name': name, 
           'status': status, 
           'client_id': client_id, 
    ...
    def post(self, request):
        ...
        return HttpResponse(json.dumps({'access_token': token.token, 
           ...
           'user': {'id': six.text_type(token.user.id), 
                    'name': token.user.name, 
                    'email': token.user.email}}), content_type='application/json')