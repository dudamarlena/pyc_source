...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/projectkey.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import, print_function
import petname, six, re
...
from sentry.db.models import Model, BaseManager, BoundedPositiveIntegerField, FlexibleForeignKey, JSONField, sane_repr
_uuid4_re = re.compile('^[a-f0-9]{32}$')
...
class ProjectKey(Model):
    ...
    roles = BitField(flags=(
     ...
     ('api', 'Web API access')), default=[
     'store'])
    status = BoundedPositiveIntegerField(default=0, choices=(
     (
      ProjectKeyStatus.ACTIVE, _('Active')),
     (
      ProjectKeyStatus.INACTIVE, _('Inactive'))), db_index=True)
    ...
    data = JSONField()
    scopes = ('project:read', 'project:write', 'project:admin', 'project:releases',
    ...
    def unreal_endpoint(self):
        return self.get_endpoint() + reverse('sentry-api-unreal', args=[self.project_id, self.public_key])
