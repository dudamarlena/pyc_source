...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_user_reports.py
...
class OrganizationUserReportsEndpoint(OrganizationEndpoint):
    ...
    def get(self, request, organization):
        ...
        queryset = UserReport.objects.filter(project_id__in=filter_params['project_id'], group__isnull=False).select_related('group')
        ...
        if filter_params['start'] and filter_params['end']:
            queryset = queryset.filter(date_added__range=(
        ...
        return self.paginate(request=request, queryset=queryset, order_by='-date_added', on_results=lambda x: serialize(x, request.user, UserReportWithGroupSerializer()), paginator_cls=DateTimePaginator)