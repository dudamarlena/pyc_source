...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/data_scrubber.py
...
class SensitiveDataFilter(object):
    ...
    """
    VALUES_RE = re.compile(('|').join([
     '\\b(?:3[47]\\d|(?:4\\d|5[1-5]|65)\\d{2}|6011)\\d{12}\\b',
     '-----BEGIN[A-Z ]+(PRIVATE|PUBLIC) KEY-----.+-----END[A-Z ]+(PRIVATE|PUBLIC) KEY-----',
     '^\\b(?!(000|666|9))\\d{3}-(?!00)\\d{2}-(?!0000)\\d{4}\\b']), re.DOTALL)
    URL_PASSWORD_RE = re.compile('\\b((?:[a-z0-9]+:)?//[a-zA-Z0-9%_.-]+:)([a-zA-Z0-9%_.-]+)@')
        ...
        if key and key in self.exclude_fields:
            ...
            if isinstance(value, six.string_types):
                ...
                if '//' in value and '@' in value:
                    value = self.URL_PASSWORD_RE.sub('\\1' + FILTER_MASK + '@', value)
            if isinstance(value, six.string_types):
