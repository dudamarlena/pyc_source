...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/db/mixin.py
...
class PendingDeletionMixin(object):
    ...
    def rename_on_pending_deletion(self, fields=None, extra_fields_to_save=None):
        ...
        with transaction.atomic():
            ...
            OrganizationOption.objects.create(organization_id=self.organization_id, key=self.build_pending_deletion_key(), value=original_data)
        logger.info('rename-on-pending-deletion', extra={'organization_id': self.organization_id, 
           'model': original_data['model'], 
           'id': original_data['id']})

    def get_pending_deletion_option(self):
        return OrganizationOption.objects.get(organization_id=self.organization_id, key=self.build_pending_deletion_key())
    ...
    def reset_pending_deletion_field_names(self, extra_fields_to_save=None):
        ...
        try:
            ...
            logger.info('reset-on-pending-deletion.does-not-exist', extra={'organization_id': self.organization_id, 
        ...
        logger.info('reset-on-pending-deletion.success', extra={'organization_id': self.organization_id, 
           'model': self.__class__.__name__, 
           'id': self.id})
        return True
    ...
    def delete_pending_deletion_option(self):
        ...
        option.delete()
        logger.info('delete-pending-deletion-option.success', extra={'organization_id': self.organization_id, 
           'model': self.__class__.__name__, 
           'id': self.id})