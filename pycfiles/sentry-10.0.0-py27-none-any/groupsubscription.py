...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/groupsubscription.py
...
class GroupSubscriptionReason(object):
    ...
    mentioned = 6
    team_mentioned = 7
    descriptions = {implicit: 'have opted to receive updates for all issues within projects that you are a member of', 
...
def get_user_options(key, user_ids, project, default):
    ...
    options = {(option.user_id, option.project_id):option.value for option in UserOption.objects.filter(Q(project__isnull=True) | Q(project=project), user_id__in=user_ids, key='workflow:notifications')}
    results = {}
...
class GroupSubscriptionManager(BaseManager):
    ...
    def bulk_subscribe(self, group, user_ids, reason=GroupSubscriptionReason.unknown):
        ...
        for i in range(4, -1, -1):
            ...
            subscriptions = [ GroupSubscription(user_id=user_id, group=group, project=group.project, is_active=True, reason=reason) for user_id in user_ids if user_id not in existing_subscriptions
                            ]
            try:
                with transaction.atomic():
    ...
    def get_participants(self, group):
        ...
        users = {user.id:user for user in User.objects.filter(sentry_orgmember_set__teams=group.project.teams.all(), is_active=True)}
        excluded_ids = set()
        subscriptions = {subscription.user_id:subscription for subscription in GroupSubscription.objects.filter(group=group, user_id__in=users.keys())}
        for user_id, subscription in subscriptions.items():
            if not subscription.is_active:
                excluded_ids.add(user_id)

        options = get_user_options('workflow:notifications', users.keys(), group.project, UserOptionValue.participating_only)
