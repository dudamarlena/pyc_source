...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tsdb/redissnuba.py
...
method_specifications = {'get_range': (
   ...
   'get_frequency_totals': (
                          READ, single_model_argument), 
   'incr': (
          WRITE, single_model_argument), 
   'incr_multi': (
                WRITE, lambda callargs: {model for model, key in callargs['items']}), 
   'merge': (
           WRITE, single_model_argument), 
   'delete': (
            WRITE, multiple_model_argument), 
   'record': (
            WRITE, single_model_argument), 
   'record_multi': (
                  WRITE, lambda callargs: {model for model, key, values in callargs['items']}), 
   'merge_distinct_counts': (
                           WRITE, single_model_argument), 
   'delete_distinct_counts': (
                            WRITE, multiple_model_argument), 
   'record_frequency_multi': (
                            WRITE, lambda callargs: {model for model, data in callargs['requests']}), 
   'merge_frequencies': (
                       WRITE, single_model_argument), 
   'delete_frequencies': (
                        WRITE, multiple_model_argument), 
...
model_backends = {model:('redis', 'redis') if model not in SnubaTSDB.model_columns else ('snuba',
                                                                                         'dummy') for model in BaseTSDB.models}
