...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_plugin_details.py
...
from sentry.api.serializers.models.plugin import PluginSerializer, PluginWithConfigSerializer, serialize_field
...
class ProjectPluginDetailsEndpoint(ProjectEndpoint):
    ...
    def post(self, request, project, plugin_id):
        ...
        if request.data.get('reset'):
            ...
            context = serialize(plugin, request.user, PluginWithConfigSerializer(project))
            self.create_audit_entry(request=request, organization=project.organization, target_object=project.id, event=AuditLogEntryEvent.INTEGRATION_EDIT, data={'integration': plugin_id, 'project': project.slug})
            return Response(context, status=200)
        if not plugin.can_disable:
            return Response({'detail': ERR_ALWAYS_ENABLED}, status=400)
        plugin.enable(project)
        self.create_audit_entry(request=request, organization=project.organization, target_object=project.id, event=AuditLogEntryEvent.INTEGRATION_ADD, data={'integration': plugin_id, 'project': project.slug})
    ...
    def delete(self, request, project, plugin_id):
        ...
        self.create_audit_entry(request=request, organization=project.organization, target_object=project.id, event=AuditLogEntryEvent.INTEGRATION_REMOVE, data={'integration': plugin_id, 'project': project.slug})
