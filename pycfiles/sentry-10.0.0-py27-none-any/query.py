...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/query.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import
import progressbar, re, six
from django.db import connections, IntegrityError, router, transaction
from django.db.models import ForeignKey
from django.db.models.deletion import Collector
from django.db.models.signals import pre_delete, pre_save, post_save, post_delete
from sentry.utils import db
_leaf_re = re.compile('^(UserReport|Event|Group)(.+)')
...
class RangeQuerySetWrapper(object):
    ...
    def __init__(self, queryset, step=1000, limit=None, min_id=None, order_by='pk', callbacks=()):
        if queryset.query.low_mark == 0 and not (queryset.query.order_by or queryset.query.extra_order_by):
            if limit is None:
...
def merge_into(self, other, callback=lambda x: x, using='default'):
    ...
    for model, objects in six.iteritems(collector.data):
        fields = set(f.name for f in model._meta.fields if isinstance(f, ForeignKey) and f.rel.to == s_model and f.rel.to)
        if not fields:
            if model == s_model:
                continue
...
def bulk_delete_objects(model, limit=10000, transaction_id=None, logger=None, partition_key=None, **filters):
    ...
    if db.is_postgres():
        query = '\n            delete from %(table)s\n            where %(partition_query)s id = any(array(\n                select id\n                from %(table)s\n                where (%(query)s)\n                limit %(limit)d\n            ))\n        ' % dict(partition_query=(' AND ').join(partition_query) + (' AND ' if partition_query else ''), query=(' AND ').join(query), table=model._meta.db_table, limit=limit)
    else:
