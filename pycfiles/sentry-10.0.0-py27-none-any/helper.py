...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/auth/helper.py
...
from sentry.models import AuditLogEntry, AuditLogEntryEvent, AuthIdentity, AuthProvider, Organization, OrganizationMember, OrganizationMemberTeam, User, UserEmail
...
def handle_existing_identity(auth_provider, provider, organization, request, state, auth_identity, identity):
    now = timezone.now()
    auth_identity.update(data=provider.update_identity(new_data=identity.get('data', {}), current_data=auth_identity.data), last_verified=now, last_synced=now)
    try:
        ...
        if not auth.login(request, user, after_2fa=request.build_absolute_uri(), organization_id=organization.id):
...
def handle_new_membership(auth_provider, organization, request, auth_identity):
    user = auth_identity.user
    om = OrganizationMember.objects.create(organization=organization, role=organization.default_role, user=user, flags=OrganizationMember.flags['sso:linked'])
    default_teams = auth_provider.default_teams.all()
    for team in default_teams:
        OrganizationMemberTeam.objects.create(team=team, organizationmember=om)

    AuditLogEntry.objects.create(organization=organization, actor=user, ip_address=request.META['REMOTE_ADDR'], target_object=om.id, target_user=om.user, event=AuditLogEntryEvent.MEMBER_ADD, data=om.get_audit_log_data())
    return om
...
def handle_attach_identity(auth_provider, request, organization, provider, identity, member=None):
    ...
    try:
        ...
        if auth_identity.user != user:
            AuthIdentity.objects.exclude(id=auth_identity.id).filter(auth_provider=auth_provider, user=user).delete()
            try:
                other_member = OrganizationMember.objects.get(user=auth_identity.user_id, organization=organization)
            except OrganizationMember.DoesNotExist:
                pass
            else:
        ...
        auth_identity.update(user=user, ident=identity['id'], data=provider.update_identity(new_data=identity.get('data', {}), current_data=auth_identity.data), last_verified=now, last_synced=now)
        auth_is_new = False
    ...
    if member is None:
        try:
            ...
            member = OrganizationMember.objects.create(organization=organization, role=organization.default_role, user=user, flags=OrganizationMember.flags['sso:linked'])
            default_teams = auth_provider.default_teams.all()
            for team in default_teams:
...
def handle_unknown_identity(request, organization, auth_provider, provider, state, identity):
    ...
    if not request.user.is_authenticated():
        try:
            acting_user = User.objects.filter(id__in=UserEmail.objects.filter(email__iexact=identity['email']).values('user'), is_active=True).first()
        except IndexError:
            acting_user = None

        login_form = AuthenticationForm(request, request.POST if request.POST.get('op') == 'login' else None, initial={'username': acting_user.username if acting_user else None})
    else:
        acting_user = request.user
    if acting_user and identity.get('email_verified'):
        has_membership = OrganizationMember.objects.filter(user=acting_user, organization=organization).exists()
        if has_membership:
            if not auth.login(request, acting_user, after_2fa=request.build_absolute_uri(), organization_id=organization.id):
    ...
    if op == 'confirm' and request.user.is_authenticated():
        auth_identity = handle_attach_identity(auth_provider, request, organization, provider, identity)
    elif op == 'newuser':
        ...
        if login_form.is_valid():
            if not auth.login(request, login_form.get_user(), after_2fa=request.build_absolute_uri(), organization_id=organization.id):
                return HttpResponseRedirect(auth.get_login_redirect(request))
        else:
    ...
    if not op:
        if request.user.is_authenticated():
            return respond('sentry/auth-confirm-link.html', organization, request, {'identity': identity, 
               'existing_user': request.user, 
               'identity_display_name': get_display_name(identity), 
               'identity_identifier': get_identifier(identity)})
        return respond('sentry/auth-confirm-identity.html', organization, request, {'existing_user': acting_user, 
           'identity': identity, 
...
def handle_new_user(auth_provider, organization, request, identity):
    user = User.objects.create(username=uuid4().hex, email=identity['email'], name=identity.get('name', '')[:200])
...
class AuthHelper(object):
    ...
    def get_for_request(cls, request):
        ...
        if not state.is_valid():
            ...
            return cls(request, organization, flow, auth_provider=auth_provider, provider_key=provider_key)
    ...
    def init_pipeline(self):
        self.state.regenerate({'uid': self.request.user.id if self.request.user.is_authenticated() else None, 
    ...
    def _finish_login_pipeline(self, identity):
        ...
        user_id = identity['id']
        lock = locks.get(('sso:auth:{}:{}').format(auth_provider.id, md5_text(user_id).hexdigest()), duration=5)
        with TimedRetryPolicy(5)(lock.acquire):
            try:
                auth_identity = AuthIdentity.objects.select_related('user').get(auth_provider=auth_provider, ident=user_id)
            except AuthIdentity.DoesNotExist:
                auth_identity = None

            if not auth_identity and isinstance(user_id, MigratingIdentityId):
                try:
                    auth_identity = AuthIdentity.objects.select_related('user').get(auth_provider=auth_provider, ident=user_id.legacy_id)
                    auth_identity.update(ident=user_id.id)
                except AuthIdentity.DoesNotExist:
                    auth_identity = None

            if not auth_identity:
                return handle_unknown_identity(self.request, self.organization, self.auth_provider, self.provider, self.state, identity)
            if not auth_identity.user.is_active:
                if not self.request.user.is_authenticated():
                    return handle_unknown_identity(self.request, self.organization, self.auth_provider, self.provider, self.state, identity)
                auth_identity = handle_attach_identity(self.auth_provider, self.request, self.organization, self.provider, identity)
            return handle_existing_identity(self.auth_provider, self.provider, self.organization, self.request, self.state, auth_identity, identity)
        return
    ...
    def _finish_setup_pipeline(self, identity):
        ...
        self.auth_provider = AuthProvider.objects.create(organization=self.organization, provider=self.provider.key, config=config)
        handle_attach_identity(self.auth_provider, self.request, self.organization, self.provider, identity, om)
        auth.mark_sso_complete(request, self.organization.id)
        sso_enabled.send_robust(organization=self.organization, user=request.user, provider=self.provider.key, sender=self.__class__)
        AuditLogEntry.objects.create(organization=self.organization, actor=request.user, ip_address=request.META['REMOTE_ADDR'], target_object=self.auth_provider.id, event=AuditLogEntryEvent.SSO_ENABLE, data=self.auth_provider.get_audit_log_data())
        ...
        self.clear_session()
        next_uri = reverse('sentry-organization-auth-provider-settings', args=[self.organization.slug])
    ...
    def error(self, message):
        ...
        metrics.incr('sso.error', tags={'provider': self.provider.key, 'flow': self.state.flow}, skip_internal=False)
        messages.add_message(self.request, messages.ERROR, ('Authentication error: {}').format(message))
    ...
    def disable_2fa_required(self):
        ...
        logger.info('Require 2fa disabled during sso setup', extra={'organization_id': self.organization.id})
        create_audit_entry(request=self.request, organization=self.organization, target_object=self.organization.id, event=AuditLogEntryEvent.ORG_EDIT, data={'require_2fa': 'to False when enabling SSO'})