...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/commits.py
...
def generate_invalid_identity_email(identity, commit_failure=False):
    new_context = {'identity': identity, 
       'auth_url': absolute_uri(reverse('socialauth_associate', args=[identity.provider])), 
       'commit_failure': commit_failure}
    return MessageBuilder(subject='Unable to Fetch Commits' if commit_failure else 'Action Required', context=new_context, template='sentry/emails/identity-invalid.txt', html_template='sentry/emails/identity-invalid.html')


def generate_fetch_commits_error_email(release, error_message):
    new_context = {'release': release, 'error_message': error_message}
    return MessageBuilder(subject='Unable to Fetch Commits', context=new_context, template='sentry/emails/unable-to-fetch-commits.txt', html_template='sentry/emails/unable-to-fetch-commits.html')
...
@instrumented_task(name='sentry.tasks.commits.fetch_commits', queue='commits', default_retry_delay=300, max_retries=5)
...
def fetch_commits(release_id, user_id, refs, prev_release_id=None, **kwargs):
    ...
    for ref in refs:
        try:
            ...
            logger.info('repository.missing', extra={'organization_id': release.organization_id, 
               'user_id': user_id, 
        ...
        if ref.get('previousCommit'):
            ...
            try:
                ...
                logger.info('fetch_commits.error', extra={'organization_id': repo.organization_id, 
                   'user_id': user_id, 
            ...
            logger.info('fetch_commits.complete', extra={'organization_id': repo.organization_id, 
               'user_id': user_id, 
               'repository': repo.name, 
               'end_sha': end_sha, 
               'start_sha': start_sha, 
    ...
    if commit_list:
        ...
        for deploy_id, environment_id, date_finished in deploys:
            last_deploy_per_environment[environment_id] = (
             deploy_id, date_finished)
        ...
        for repository_id, commit_id in repo_queryset:
            for environment_id, (deploy_id, date_finished) in six.iteritems(last_deploy_per_environment):
                if not Deploy.objects.filter(id__in=LatestRelease.objects.filter(repository_id=repository_id, environment_id=environment_id).values('deploy_id'), date_finished__gt=date_finished).exists():
                    LatestRelease.objects.create_or_update(repository_id=repository_id, environment_id=environment_id, values={'release_id': release.id, 
                       'deploy_id': deploy_id, 
                       'commit_id': commit_id})
