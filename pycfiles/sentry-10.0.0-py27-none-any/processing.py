...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/stacktraces/processing.py
...
def find_stacktraces_in_data(data, include_raw=False):
    ...
    def _report_stack(stacktrace, container):
        if not stacktrace or not get_path(stacktrace, 'frames', filter=True):
            return
        platforms = set(frame.get('platform') or data.get('platform') for frame in get_path(stacktrace, 'frames', filter=True, default=()))
...
def should_process_for_stacktraces(data):
    ...
    for plugin in plugins.all(version=2):
        processors = safe_execute(plugin.get_stacktrace_processors, data=data, stacktrace_infos=infos, platforms=platforms, _with_transaction=False)
        if processors:
            return True
...
def get_processors_for_stacktraces(data, infos):
    ...
    processors = []
    for plugin in plugins.all(version=2):
        processors.extend(safe_execute(plugin.get_stacktrace_processors, data=data, stacktrace_infos=infos, platforms=platforms, _with_transaction=False) or ())
...
def get_processable_frames(stacktrace_info, processors):
    ...
    for idx, frame in enumerate(frames):
        ...
        if processor is not None:
            rv.append(ProcessableFrame(frame, frame_count - idx - 1, processor, stacktrace_info, rv))
...
def get_crash_frame_from_event_data(data, frame_filter=None):
    frames = get_path(data, 'exception', 'values', -1, 'stacktrace', 'frames') or get_path(data, 'stacktrace', 'frames')
    if not frames:
...
def get_stacktrace_processing_task(infos, processors):
    ...
    for info in infos:
        ...
        for processable_frame in processable_frames:
            ...
            by_stacktrace_info.setdefault(processable_frame.stacktrace_info, []).append(processable_frame)
    ...
    return StacktraceProcessingTask(processable_stacktraces=by_stacktrace_info, processors=by_processor)
    ...
    if not processors:
        ...
        try:
            ...
            for stacktrace_info, processable_frames in processing_task.iter_processable_stacktraces():
                new_frames, new_raw_frames, errors = process_single_stacktrace(processing_task, stacktrace_info, processable_frames)
                if new_frames is not None:
                    stacktrace_info.stacktrace['frames'] = new_frames
                    changed = True
                if set_raw_stacktrace and new_raw_frames is not None and stacktrace_info.container is not None:
                    stacktrace_info.container['raw_stacktrace'] = dict(stacktrace_info.stacktrace, frames=new_raw_frames)
                    changed = True
                if errors:
