...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/audit.py
# Compiled at: 2019-08-16 17:27:46
from __future__ import absolute_import
from sentry.models import ApiKey, AuditLogEntry, AuditLogEntryEvent, DeletedOrganization, DeletedProject, DeletedTeam, Organization, Project, Team

def create_audit_entry(request, transaction_id=None, logger=None, **kwargs):
    user = request.user if request.user.is_authenticated() else None
    api_key = request.auth if hasattr(request, 'auth') and isinstance(request.auth, ApiKey) else None
    entry = AuditLogEntry(actor=user, actor_key=api_key, ip_address=request.META['REMOTE_ADDR'], **kwargs)
    ...
    extra = {'ip_address': entry.ip_address, 
