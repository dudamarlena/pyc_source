...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/organizationmember.py
...
from sentry.db.models import BaseModel, BoundedAutoField, BoundedPositiveIntegerField, FlexibleForeignKey, Model, sane_repr
...
class OrganizationMemberTeam(BaseModel):
    ...
    def get_audit_log_data(self):
        return {'team_slug': self.team.slug, 
...
class OrganizationMember(Model):
    ...
    user = FlexibleForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, related_name='sentry_orgmember_set')
    email = models.EmailField(null=True, blank=True, max_length=75)
    role = models.CharField(choices=roles.get_choices(), max_length=32, default=roles.get_default().id)
    ...
    teams = models.ManyToManyField('sentry.Team', blank=True, through='sentry.OrganizationMemberTeam')
    ...
    def get_invite_link(self):
        if not self.is_pending:
            ...
            return absolute_uri(reverse('sentry-accept-invite', kwargs={'member_id': self.id, 'token': self.token or self.legacy_token}))
    ...
    def send_sso_link_email(self, actor, provider):
        ...
        msg = MessageBuilder(subject='Action Required for %s' % (self.organization.name,), template='sentry/emails/auth-link-identity.txt', html_template='sentry/emails/auth-link-identity.html', type='organization.auth_link', context=context)
        msg.send_async([self.get_email()])
    ...
    def get_audit_log_data(self):
        ...
        return {'email': self.get_email(), 
           'user': self.user_id, 
           'teams': [ t['id'] for t in teams ], 'teams_slugs': [ t['slug'] for t in teams ], 'has_global_access': self.has_global_access, 
           'role': self.role}

    def get_teams(self):
        from sentry.models import Team
        return Team.objects.filter(status=TeamStatus.VISIBLE, id__in=OrganizationMemberTeam.objects.filter(organizationmember=self, is_active=True).values('team'))
    ...
    def delete_expired(cls, threshold):
        ...
        ``threshold`` days ago.
        """
        cls.objects.filter(token_expires_at__lt=threshold, user_id__exact=None).exclude(email__exact=None).delete()
        return