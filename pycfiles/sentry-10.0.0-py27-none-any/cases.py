...
# [GCC 8.4.0]
# Embedded file name: /c/Users/byk/Documents/Projects/sentry/sentry/src/sentry/testutils/cases.py
...
import base64, calendar, contextlib, os, os.path, pytest, requests, six, types, mock
...
from sentry.auth.superuser import Superuser, COOKIE_SALT as SU_COOKIE_SALT, COOKIE_NAME as SU_COOKIE_NAME, ORG_ID as SU_ORG_ID, COOKIE_SECURE as SU_COOKIE_SECURE, COOKIE_DOMAIN as SU_COOKIE_DOMAIN, COOKIE_PATH as SU_COOKIE_PATH
from sentry.constants import MODULE_ROOT
from sentry.eventstream.snuba import SnubaEventStream
from sentry.models import GroupEnvironment, GroupHash, GroupMeta, ProjectOption, Repository, DeletedOrganization, Environment, Organization, TotpInterface, Dashboard, ObjectStatus, WidgetDataSource, WidgetDataSourceTypes
...
from .helpers import AuthProvider, Feature, get_auth_header, TaskRunner, override_options, parse_queries
...
class BaseTestCase(Fixtures, Exam):
    ...
    def _postWithHeader(self, data, key=None, secret=None, protocol=None):
        ...
        with self.tasks():
            resp = self.client.post(reverse('sentry-api-store'), message, content_type='application/octet-stream', HTTP_X_SENTRY_AUTH=get_auth_header('_postWithHeader/0.0.0', key, secret, protocol))
        return resp
    ...
    def _postCspWithHeader(self, data, key=None, **extra):
        ...
        with self.tasks():
            return self.client.post(path, data=body, content_type='application/csp-report', HTTP_USER_AGENT=DEFAULT_USER_AGENT, **extra)

    def _postMinidumpWithHeader(self, upload_file_minidump, data=None, key=None, raw=False, **extra):
        if raw:
    ...
    def _postUnrealWithHeader(self, upload_unreal_crash, data=None, key=None, **extra):
        path = reverse('sentry-api-unreal', kwargs={'project_id': self.project.id, 'sentry_key': self.projectkey.public_key})
        with self.tasks():
            return self.client.post(path, data=upload_unreal_crash, content_type='application/octet-stream', HTTP_USER_AGENT=DEFAULT_USER_AGENT, **extra)

    def _postEventAttachmentWithHeader(self, attachment, **extra):
        path = reverse('sentry-api-event-attachment', kwargs={'project_id': self.project.id, 'event_id': self.event.id})
        key = self.projectkey.public_key
        secret = self.projectkey.secret_key
        with self.tasks():
            return self.client.post(path, attachment, HTTP_X_SENTRY_AUTH=get_auth_header('_postWithHeader/0.0.0', key, secret, 7), **extra)
    ...
    def _getWithReferer(self, data, key=None, referer='sentry.io', protocol='4'):
        ...
        qs = {'sentry_version': protocol, 
           'sentry_client': 'raven-js/lol', 
           'sentry_key': key, 
           'sentry_data': message}
        with self.tasks():
            resp = self.client.get(('%s?%s' % (reverse('sentry-api-store', args=(self.project.pk,)), urlencode(qs))), **headers)
        return resp
    ...
    def _postWithReferer(self, data, key=None, referer='sentry.io', protocol='4'):
        ...
        with self.tasks():
            resp = self.client.post(('%s?%s' % (reverse('sentry-api-store', args=(self.project.pk,)), urlencode(qs))), data=message, content_type='application/json', **headers)
        return resp
    ...
    def assert_valid_deleted_log(self, deleted_log, original_object):
        if not deleted_log is not None:
            ...
            assert isinstance(deleted_log, DeletedOrganization) or deleted_log.organization_id == original_object.organization.id
        ...
        if exc_type is not None:
            ...
            for table, num in parsed_queries.items():
                ...
                if expected == 0:
                    ...
                    self.test_case.assertTrue(num == expected, '%d write queries expected on `%s`, got %d, add debug=True to see all the queries' % (
                     expected, table, num))
...
class TwoFactorAPITestCase(APITestCase):
    ...
    def __helper_enable_organization_2fa(self, organization, user, status_code, err_msg=None):
        ...
        if not response.status_code == status_code:
            raise AssertionError
            assert err_msg and err_msg in response.content
    ...
    def assertAppInstalled(self, name, path):
        for ep in iter_entry_points('sentry.apps'):
            if ep.name == name:
                ...
                if ep_path == path:
                    return
                self.fail('Found app in entry_points, but wrong class. Got %r, expected %r' % (
                 ep_path, path))
    ...
    def assertPluginInstalled(self, name, plugin):
        ...
        for ep in iter_entry_points('sentry.plugins'):
            if ep.name == name:
                ...
                if ep_path == path:
                    return
                self.fail('Found plugin in entry_points, but wrong class. Got %r, expected %r' % (
                 ep_path, path))
...
class AcceptanceTestCase(TransactionTestCase):
    ...
    def setUp(self):
        patcher = patch('django.utils.timezone.now', return_value=datetime(2013, 5, 18, 15, 13, 58, 132928, tzinfo=timezone.utc))
        patcher.start()
...
class IntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.pipeline = IntegrationPipeline(request=self.request, organization=self.organization, provider_key=self.provider.key)
...
class SnubaTestCase(BaseTestCase):
    ...
    def store_event(self, *args, **kwargs):
        with contextlib.nested(mock.patch('sentry.eventstream.insert', self.snuba_eventstream.insert), mock.patch('sentry.tagstore.delay_index_event_tags', self.snuba_tagstore.delay_index_event_tags), mock.patch('sentry.tagstore.incr_tag_value_times_seen', self.snuba_tagstore.incr_tag_value_times_seen), mock.patch('sentry.tagstore.incr_group_tag_value_times_seen', self.snuba_tagstore.incr_group_tag_value_times_seen)):
    ...
    def create_event(self, *args, **kwargs):
        ...
        if 'environment' in tags:
            ...
            GroupEnvironment.objects.get_or_create(environment_id=environment.id, group_id=event.group_id)
        primary_hash = event.get_primary_hash()
        grouphash, _ = GroupHash.objects.get_or_create(project=event.project, group=event.group, hash=primary_hash)
    ...
    def snuba_insert(self, events):
        ...
        if not isinstance(events, list):
            events = [
             events]
        assert requests.post(settings.SENTRY_SNUBA + '/tests/events/insert', data=json.dumps(events)).status_code == 200
...
class IntegrationRepositoryTestCase(APITestCase):
    ...
    def create_repository(self, repository_config, integration_id, organization_slug=None, add_responses=True):
        ...
        response = self.client.post(path=reverse('sentry-api-0-organization-repositories', args=[
         organization_slug or self.organization.slug]), data=data)
        return response
...
class SetRefsTestCase(APITestCase):
    ...
    def assert_fetch_commits(self, mock_fetch_commit, prev_release_id, release_id, refs):
        ...
        assert kwargs == {'prev_release_id': prev_release_id, 
           'refs': refs, 
...
class OrganizationDashboardWidgetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.login_as(self.user)
        self.dashboard = Dashboard.objects.create(title='Dashboard 1', created_by=self.user, organization=self.organization)
        self.anon_users_query = {'name': 'anonymousUsersAffectedQuery', 
           'fields': [], 'conditions': [
                        [
                         'user.email', 'IS NULL', None]], 
           'aggregations': [
                          [
                           'count()', None, 'Anonymous Users']], 
           'limit': 1000, 
           'orderby': '-time', 
           'groupby': [
                     'time'], 
           'rollup': 86400}
        self.known_users_query = {'name': 'knownUsersAffectedQuery', 
           'fields': [], 'conditions': [
                        [
                         'user.email', 'IS NOT NULL', None]], 
           'aggregations': [
                          [
                           'uniq', 'user.email', 'Known Users']], 
           'limit': 1000, 
           'orderby': '-time', 
           'groupby': [
                     'time'], 
           'rollup': 86400}
        self.geo_erorrs_query = {'name': 'errorsByGeo', 
           'fields': [
                    'geo.country_code'], 
           'conditions': [
                        [
                         'geo.country_code', 'IS NOT NULL', None]], 
           'aggregations': [
                          [
                           'count()', None, 'count']], 
           'limit': 10, 
    ...
    def assert_widget_data_sources(self, widget_id, data):
        result_data_sources = sorted(WidgetDataSource.objects.filter(widget_id=widget_id, status=ObjectStatus.VISIBLE), key=lambda x: x.order)
    ...
    def assert_widget(self, widget, order, title, display_type, display_options=None, data_sources=None):
        if not widget.order == order:
            raise AssertionError
            if not widget.display_type == display_type:
                raise AssertionError
                assert display_options and widget.display_options == display_options
            assert widget.title == title
            return data_sources or None
        self.assert_widget_data_sources(widget.id, data_sources)

    def assert_widget_data(self, data, order, title, display_type, display_options=None, data_sources=None):
        if not data['order'] == order:
            raise AssertionError
            if not data['displayType'] == display_type:
                raise AssertionError
                assert display_options and data['displayOptions'] == display_options
