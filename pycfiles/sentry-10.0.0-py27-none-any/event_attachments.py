...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/event_attachments.py
...
class EventAttachmentsEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project, event_id):
        ...
        if not features.has('organizations:event-attachments', project.organization, actor=request.user):
            ...
            queryset = EventAttachment.objects.filter(project_id=project.id, event_id=event.event_id).select_related('file')
