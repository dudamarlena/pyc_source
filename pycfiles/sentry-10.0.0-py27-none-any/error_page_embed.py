...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/error_page_embed.py
# Compiled at: 2019-08-23 05:13:12
...
DEFAULT_OPTIONS = {'title': DEFAULT_TITLE, 
...
class UserReportForm(forms.ModelForm):
    name = forms.CharField(max_length=128, widget=forms.TextInput(attrs={'placeholder': _('Jane Doe')}))
    email = forms.EmailField(max_length=75, widget=forms.TextInput(attrs={'placeholder': _('jane@example.com'), 'type': 'email'}))
    comments = forms.CharField(widget=forms.Textarea(attrs={'placeholder': _("I clicked on 'X' and then hit 'Confirm'")}))
        ...
        if not key:
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    UserReport.objects.filter(project=report.project, event_id=report.event_id).update(name=report.name, email=report.email, comments=report.comments, date_added=timezone.now())
            ...
            template = render_to_string('sentry/error-page-embed.html', {'form': form, 
            ...
            context = {'endpoint': mark_safe('*/' + json.dumps(request.build_absolute_uri()) + ';/*'), 
               'template': mark_safe('*/' + json.dumps(template) + ';/*'), 
               'strings': json.dumps_htmlsafe({'generic_error': six.text_type(options['errorGeneric']), 
                           'form_error': six.text_type(options['errorFormEntry']), 
                           'sent_message': six.text_type(options['successMessage'])})}
            return render_to_response('sentry/error-page-embed.js', context, request, content_type='text/javascript')