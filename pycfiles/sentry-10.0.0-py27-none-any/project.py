...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/project.py
...
from sentry.db.models import BaseManager, BoundedPositiveIntegerField, FlexibleForeignKey, Model, sane_repr
...
class ProjectManager(BaseManager):
    ...
    def get_for_user(self, team, user, scope=None, _skip_team_check=False):
        ...
        if not _skip_team_check:
            team_list = Team.objects.get_for_user(organization=team.organization, user=user, scope=scope)
            try:
...
class Project(Model, PendingDeletionMixin):
    ...
    status = BoundedPositiveIntegerField(default=0, choices=(
     ...
     (
      ObjectStatus.DELETION_IN_PROGRESS, _('Deletion in Progress'))), db_index=True)
    first_event = models.DateTimeField(null=True)
    flags = BitField(flags=(('has_releases', 'This Project has sent release data'), ), default=0, null=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            with TimedRetryPolicy(10)(lock.acquire):
                slugify_instance(self, self.name, organization=self.organization, reserved=RESERVED_PROJECT_SLUGS)
    ...
    def member_set(self):
        ...
        return self.organization.member_set.filter(id__in=OrganizationMember.objects.filter(organizationmemberteam__is_active=True, organizationmemberteam__team__in=self.teams.all()).values('id'), user__is_active=True).distinct()
    ...
    def has_access(self, user, access=None):
        ...
        try:
            auth_identity = AuthIdentity.objects.get(auth_provider__organization=self.organization_id, user=member.user_id)
    ...
    def get_member_alert_settings(self, user_option):
        ...
        return {o.user_id:int(o.value) for o in UserOption.objects.filter(project=self, key=user_option)}
    ...
    def get_notification_recipients(self, user_option):
        ...
        if members_to_check:
            disabled = set(uo.user_id for uo in UserOption.objects.filter(key='subscribe_by_default', user__in=members_to_check) if uo.value == '0')
    ...
    def transfer_to(self, team=None, organization=None):
        from sentry.models import Environment, EnvironmentProject, ProjectTeam, ReleaseProject, ReleaseProjectEnvironment, Rule
        ...
        for rule_id, environment_id in Rule.objects.filter(project_id=self.id, environment_id__isnull=False).values_list('id', 'environment_id'):
            rules_by_environment_id[environment_id].add(rule_id)

        environment_names = dict(Environment.objects.filter(id__in=rules_by_environment_id).values_list('id', 'name'))
        for environment_id, rule_ids in rules_by_environment_id.items():
            Rule.objects.filter(id__in=rule_ids).update(environment_id=Environment.get_or_create(self, environment_names[environment_id]).id)
    ...
    def copy_settings_from(self, project_id):
        ...
        model_list = [
         EnvironmentProject, ProjectOwnership, ProjectTeam, Rule]
        ...
        try:
            ...
            logging.exception('Error occurred during copy project settings.', extra={'error': e.message, 'project_to': self.id, 'project_from': project_id})
            return False
