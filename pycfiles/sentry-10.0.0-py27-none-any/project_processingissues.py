...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_processingissues.py
    ...
    def get(self, request, project):
        ...
        if request.user_from_signed_request and request.user.is_authenticated():
            ...
            if not tokens:
                ...
                token = tokens[0]
        resp = render_to_response('sentry/reprocessing-script.sh', {'issues': [ {'uuid': issue.data.get('image_uuid'), 'arch': issue.data.get('image_arch'), 'name': (issue.data.get('image_path') or '').split('/')[(-1)]} for issue in ProcessingIssue.objects.filter(project=project)
                   ], 
           'project': project, 
           'token': token, 
