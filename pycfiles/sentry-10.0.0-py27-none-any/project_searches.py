...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_searches.py
    ...
    def post(self, request, project):
        ...
        if serializer.is_valid():
            ...
            with transaction.atomic():
                try:
                    search = SavedSearch.objects.create(project=project, name=result['name'], query=result['query'], is_default=result.get('isDefault', False), owner=None if request.access.has_scope('project:write') else request.user)
                ...
                if search.is_default:
                    if request.access.has_scope('project:write'):
                        SavedSearch.objects.filter(project=project).exclude(id=search.id).update(is_default=False)
                    else:
                        return Response({'detail': "User doesn't have permission to set default view"}, status=400)
                if result.get('isUserDefault'):
                    SavedSearchUserDefault.objects.create_or_update(savedsearch=search, user=request.user, project=project)
