...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/helpers/processing_issues.py
...
def get_processing_issues(user, projects, include_detailed_issues=False):
    ...
    for project in projects:
        ...
        signed_link = None
        if num_issues > 0:
            signed_link = generate_signed_link(user, 'sentry-api-0-project-fix-processing-issues', kwargs={'project_slug': project.slug, 
               'organization_slug': project.organization.slug})
        last_seen = agg_results.get('last_seen')
        data = {'hasIssues': num_issues > 0, 
