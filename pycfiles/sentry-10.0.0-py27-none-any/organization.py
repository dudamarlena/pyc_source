...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/organization.py
...
OrganizationStatus._labels = {OrganizationStatus.ACTIVE: 'active', 
...
class Organization(Model):
    ...
    status = BoundedPositiveIntegerField(choices=OrganizationStatus.as_choices(), default=OrganizationStatus.ACTIVE.value)
    ...
    default_role = models.CharField(choices=roles.get_choices(), max_length=32, default=roles.get_default().id)
    flags = BitField(flags=(
     ...
     ('disable_new_visibility_features', 'Temporarily opt out of new visibility features and ui')), default=1)
    ...
    def get_owners(self):
        ...
        return User.objects.filter(sentry_orgmember_set__role=roles.get_top_dog().id, sentry_orgmember_set__organization=self, is_active=True)
    ...
    def has_single_owner(self):
        from sentry.models import OrganizationMember
        count = OrganizationMember.objects.filter(organization=self, role=roles.get_top_dog().id, user__isnull=False, user__is_active=True)[:2].count()
        return count == 1

    def merge_to(from_org, to_org):
        from sentry.models import ApiKey, AuditLogEntry, AuthProvider, Commit, OrganizationAvatar, OrganizationIntegration, OrganizationMember, OrganizationMemberTeam, Project, Release, ReleaseCommit, ReleaseEnvironment, ReleaseFile, ReleaseHeadCommit, Repository, Team, Environment
        for from_member in OrganizationMember.objects.filter(organization=from_org, user__isnull=False):
            ...
            try:
                ...
                for omt in qs:
                    OrganizationMemberTeam.objects.create_or_update(organizationmember=to_member, team=omt.team, defaults={'is_active': True})

                logger.info('user.migrate', extra={'instance_id': from_member.id, 
        ...
        for from_team in Team.objects.filter(organization=from_org):
            ...
            logger.info('team.migrate', extra={'instance_id': from_team.id, 
        ...
        for from_project in Project.objects.filter(organization=from_org):
            try:
                ...
                slugify_instance(from_project, from_project.name, organization=to_org, reserved=RESERVED_PROJECT_SLUGS)
                from_project.update(organization=to_org, slug=from_project.slug)

            logger.info('project.migrate', extra={'instance_id': from_project.id, 
        ...
        for from_release in Release.objects.filter(organization=from_org):
            ...
            logger.info('release.migrate', extra={'instance_id': from_release.id, 
               'from_organization_id': from_org.id, 
               'to_organization_id': to_org.id})
            ...
            try:
                ...
                for instance in queryset:
                    try:
                        ...
                        logger.info(('{}.migrate-skipped').format(model_name), extra={'from_organization_id': from_org.id, 
                           'to_organization_id': to_org.id})
                    else:
                        logger.info(('{}.migrate').format(model_name), extra={'instance_id': instance.id, 
                           'from_organization_id': from_org.id, 
                           'to_organization_id': to_org.id})

            else:
                logger.info(('{}.migrate').format(model_name), extra={'from_organization_id': from_org.id, 'to_organization_id': to_org.id})
    ...
    def send_delete_confirmation(self, audit_log_entry, countdown):
        ...
        MessageBuilder(subject='%sOrganization Queued for Deletion' % (options.get('mail.subject-prefix'),), template='sentry/emails/org_delete_confirm.txt', html_template='sentry/emails/org_delete_confirm.html', type='org.confirm_delete', context=context).send_async([ o.email for o in owners ])
    ...
    def handle_2fa_required(self, request):
        ...
        api_key_id = request.auth.id if hasattr(request, 'auth') and isinstance(request.auth, ApiKey) else None
        ip_address = request.META['REMOTE_ADDR']
        remove_2fa_non_compliant_members.delay(self.id, actor_id=actor_id, actor_key_id=api_key_id, ip_address=ip_address)
        return
