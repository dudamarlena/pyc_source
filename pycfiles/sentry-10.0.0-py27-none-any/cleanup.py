...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/runner/commands/cleanup.py
    ...
    while True:
        ...
        if not configured:
            ...
            skip_models = [
             ...
             similarity.features] + [ b[0] for b in EXTRA_BULK_QUERY_DELETES ]
            configured = True
        model, chunk = j
        model = import_string(model)
        try:
            try:
                task = deletions.get(model=model, query={'id__in': chunk}, skip_models=skip_models, transaction_id=uuid4().hex)
                while True:
                    if not task.chunk():
                        break
...
def cleanup(days, project, concurrency, silent, model, router, timed):
    ...
    BULK_QUERY_DELETES = [
     ...
     (
      models.GroupRuleStatus, 'date_added', None)] + EXTRA_BULK_QUERY_DELETES
    DELETES = (
     (
      models.Event, 'datetime', 'datetime'), (models.Group, 'last_seen', 'last_seen'))
    if not silent:
