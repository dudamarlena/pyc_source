...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_repository_details.py
...
class OrganizationRepositoryDetailsEndpoint(OrganizationEndpoint):
    ...
    def put(self, request, organization, repo_id):
        ...
        if result.get('integrationId'):
            try:
                integration = Integration.objects.get(id=result['integrationId'], organizations=organization)
    ...
    def delete(self, request, organization, repo_id):
        ...
        updated = Repository.objects.filter(id=repo.id, status__in=[ObjectStatus.VISIBLE, ObjectStatus.DISABLED]).update(status=ObjectStatus.PENDING_DELETION)
        if updated:
            ...
            delete_repository.apply_async(kwargs={'object_id': repo.id, 
               'transaction_id': transaction_id, 
               'actor_id': request.user.id}, countdown=countdown)
            delete_logger.info('object.delete.queued', extra={'object_id': repo.id, 
               'transaction_id': transaction_id, 
