...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_onboarding_tasks.py
    ...
    def post(self, request, organization):
        ...
        if request.data['status'] == 'skipped' and task_id in (
         ...
         OnboardingTask.NOTIFICATION_SERVICE):
            rows_affected, created = OrganizationOnboardingTask.objects.create_or_update(organization=organization, user=request.user, task=request.data['task'], values={'status': OnboardingTaskStatus.SKIPPED, 'date_completed': timezone.now()})
