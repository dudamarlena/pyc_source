...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/utils/monitors.py
...
def connect(app):
    ...
    schedule = app.conf.beat_schedule if hasattr(app.conf, 'beat_schedule') else app.conf['CELERYBEAT_SCHEDULE']
    for schedule_name, monitor_id in six.iteritems(settings.SENTRY_CELERYBEAT_MONITORS):
        schedule[schedule_name].setdefault('options', {}).setdefault('headers', {}).setdefault('X-Sentry-Monitor', monitor_id)
...
def report_monitor_begin(task, **kwargs):
    ...
    session = SafeSession()
    req = session.post(('{}/api/0/monitors/{}/checkins/').format(API_ROOT, monitor_id), headers={'Authorization': ('DSN {}').format(SENTRY_DSN)}, json={'status': 'in_progress'})
    req.raise_for_status()
    task.request.headers['X-Sentry-Monitor-CheckIn'] = (
     req.json()['id'], time())
...
def report_monitor_complete(task, retval, **kwargs):
    ...
    session = SafeSession()
    session.put(('{}/api/0/monitors/{}/checkins/{}/').format(API_ROOT, monitor_id, checkin_id), headers={'Authorization': ('DSN {}').format(SENTRY_DSN)}, json={'status': 'error' if isinstance(retval, Exception) else 'ok', 'duration': duration}).raise_for_status()