...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/plugins/bases/issue2.py
...
class IssueTrackingPlugin2(Plugin):
    ...
    def get_group_urls(self):
        ...
        for action in self.allowed_actions:
            ...
            _urls.append(url('^%s/' % action, PluginGroupEndpoint.as_view(view=getattr(self, view_method_name))))
    ...
    def view_create(self, request, group, **kwargs):
        ...
        if auth_errors:
            ...
            issue_information = {'title': issue.get('title') or request.data.get('title') or self._get_issue_label_compat(group, issue), 
    ...
    def view_link(self, request, group, **kwargs):
        ...
        if auth_errors:
            ...
            issue_information = {'title': issue.get('title') or self._get_issue_label_compat(group, issue), 'provider': self.get_title(), 
               'location': self._get_issue_url_compat(group, issue), 
               'label': self._get_issue_label_compat(group, issue)}
            Activity.objects.create(project=group.project, group=group, type=Activity.CREATE_ISSUE, user=request.user, data=issue_information)
    ...
    def plugin_issues(self, request, group, plugin_issues, **kwargs):
        if not self.is_configured(request=request, project=group.project):
            ...
            issue = self.build_issue(group)
            if issue:
                item['issue'] = {'issue_id': issue.get('id'), 'url': self._get_issue_url_compat(group, issue), 
                   'label': self._get_issue_label_compat(group, issue)}
