...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_release_assemble.py
...
class OrganizationReleaseAssembleEndpoint(OrganizationReleasesBaseEndpoint):
    ...
    def post(self, request, organization, version):
        ...
        if not self.has_release_permission(request, organization, release):
            raise ResourceDoesNotExist
        schema = {'type': 'object', 
           'properties': {'checksum': {'type': 'string', 'pattern': '^[0-9a-f]{40}$'}, 'chunks': {'type': 'array', 
        ...
        if state == ChunkFileState.OK:
            ...
            set_assemble_status(AssembleTask.ARTIFACTS, organization.id, checksum, ChunkFileState.CREATED)
            from sentry.tasks.assemble import assemble_artifacts
            assemble_artifacts.apply_async(kwargs={'org_id': organization.id, 
               'version': version, 
               'checksum': checksum, 
               'chunks': chunks})
