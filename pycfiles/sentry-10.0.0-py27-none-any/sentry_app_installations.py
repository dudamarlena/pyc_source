...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/sentry_app_installations.py
...
class SentryAppInstallationsSerializer(serializers.Serializer):
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=SENTRY_APP_SLUG_MAX_LENGTH, error_messages={'invalid': _('Enter a valid slug consisting of lowercase letters, numbers, underscores or hyphens.')})
...
class SentryAppInstallationsEndpoint(SentryAppInstallationsBaseEndpoint):
    ...
    def post(self, request, organization):
        ...
        install = Creator.run(organization=organization, slug=serializer.validated_data.get('slug'), user=request.user, request=request)
