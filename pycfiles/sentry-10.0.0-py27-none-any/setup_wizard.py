...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/web/frontend/setup_wizard.py
...
from sentry.models import Organization, OrganizationStatus, Project, ProjectKey, ProjectKeyStatus, ProjectStatus
...
class SetupWizardView(BaseView):
    ...
    def get(self, request, wizard_hash):
        ...
        if wizard_data is None:
            ...
            orgs = Organization.objects.filter(member_set__role__in=[ x.id for x in roles.with_scope('org:read') ], member_set__user=request.user, status=OrganizationStatus.VISIBLE).order_by('-date_added')[:50]
            filled_projects = []
            for org in orgs:
                projects = list(Project.objects.filter(organization=org, status=ProjectStatus.VISIBLE).order_by('-date_added')[:50])
                for project in projects:
                    enriched_project = serialize(project)
                    enriched_project['organization'] = serialize(org)
                    keys = list(ProjectKey.objects.filter(project=project, roles=F('roles').bitor(ProjectKey.roles.store), status=ProjectKeyStatus.ACTIVE))
            ...
            if not tokens:
                token = ApiToken.objects.create(user=request.user, scope_list=[
                 'project:releases'], refresh_token=None, expires_at=None)
            else:
                token = tokens[0]
