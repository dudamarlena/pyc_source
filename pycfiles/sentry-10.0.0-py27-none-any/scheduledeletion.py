...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/scheduledeletion.py
...
class ScheduledDeletion(Model):
    ...
    def schedule(cls, instance, days=30, data=None, actor=None):
        return cls.objects.create(app_label=instance._meta.app_label, model_name=type(instance).__name__, object_id=instance.pk, date_scheduled=timezone.now() + timedelta(days=days), data=data or {}, actor_id=actor.id if actor else None)
