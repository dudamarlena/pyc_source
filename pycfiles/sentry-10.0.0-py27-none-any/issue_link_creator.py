...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/mediators/external_issues/issue_link_creator.py
...
class IssueLinkCreator(Mediator):
    ...
    def _make_external_request(self):
        self.response = external_requests.IssueLinkRequester.run(install=self.install, uri=self.uri, group=self.group, fields=self.fields, user=self.user)

    def _format_response_data(self):
        web_url = self.response['webUrl']
        display_name = ('{}#{}').format(escape(self.response['project']), escape(self.response['identifier']))
        return [
         web_url, display_name]

    def _create_external_issue(self):
        web_url, display_name = self._format_response_data()
        self.external_issue = PlatformExternalIssue.objects.create(group_id=self.group.id, service_type=self.sentry_app.slug, display_name=display_name, web_url=web_url)
