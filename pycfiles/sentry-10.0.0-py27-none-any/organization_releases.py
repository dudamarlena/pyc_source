...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_releases.py
...
from sentry.api.serializers.rest_framework import ReleaseHeadCommitSerializer, ReleaseHeadCommitSerializerDeprecated, ReleaseWithVersionSerializer, ListField
...
def create_new_org_release_ref_scenario(runner):
    runner.request(method='POST', path='/organizations/%s/releases/' % (runner.org.slug,), data={'version': '2.0rc2', 
...
def create_new_org_release_commit_scenario(runner):
    runner.request(method='POST', path='/organizations/%s/releases/' % (runner.org.slug,), data={'version': '2.0rc3', 
       'projects': [
                  runner.default_project.slug], 
       'commits': [
                 {'patch_set': [{'path': 'path/to/added-file.html', 'type': 'A'}, {'path': 'path/to/modified-file.html', 'type': 'M'}, {'path': 'path/to/deleted-file.html', 'type': 'D'}], 'repository': 'owner-name/repo-name', 
...
class OrganizationReleasesEndpoint(OrganizationReleasesBaseEndpoint, EnvironmentMixin):
    ...
    def get(self, request, organization):
        ...
        queryset = Release.objects.filter(organization=organization, projects__id__in=filter_params['project_id']).select_related('owner').distinct()
        if 'environment' in filter_params:
            queryset = queryset.filter(releaseprojectenvironment__environment__name__in=filter_params['environment'], releaseprojectenvironment__project_id__in=filter_params['project_id'])
        if query:
        ...
        if filter_params['start'] and filter_params['end']:
            queryset = queryset.extra(where=[
             '%s BETWEEN %%s and %%s' % sort_query], params=[
             filter_params['start'], filter_params['end']])
        return self.paginate(request=request, queryset=queryset, order_by='-sort', paginator_cls=OffsetPaginator, on_results=lambda x: serialize(x, request.user))
        ...
        if serializer.is_valid():
            ...
            try:
                with transaction.atomic():
                    release, created = Release.objects.create(organization_id=organization.id, version=result['version'], ref=result.get('ref'), url=result.get('url'), owner=result.get('owner'), date_released=result.get('dateReleased')), True
            except IntegrityError:
                release, created = Release.objects.get(organization_id=organization.id, version=result['version']), False
            else:
            ...
            if release.date_released:
                for project in new_projects:
                    Activity.objects.create(type=Activity.RELEASE, project=project, ident=Activity.get_version_ident(result['version']), data={'version': result['version']}, datetime=release.date_released)
