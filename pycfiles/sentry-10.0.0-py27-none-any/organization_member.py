...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/serializers/models/organization_member.py
...
class OrganizationMemberSerializer(Serializer):
    ...
    def get_attrs(self, item_list, user):
        ...
        return {item:{'user': users[six.text_type(item.user_id)] if item.user_id else None} for item in item_list}
    ...
    def serialize(self, obj, attrs, user):
        d = {'id': six.text_type(obj.id), 
           ...
           'flags': {'sso:linked': bool(getattr(obj.flags, 'sso:linked')), 
...
class OrganizationMemberWithTeamsSerializer(OrganizationMemberSerializer):
    ...
    def get_attrs(self, item_list, user):
        ...
        member_team_map = list(OrganizationMemberTeam.objects.filter(team__status=TeamStatus.VISIBLE, organizationmember__in=item_list).values_list('organizationmember_id', 'team_id'))
        teams = {team.id:team for team in Team.objects.filter(id__in=[ team_id for _, team_id in member_team_map ])}
        ...
        for org_member in item_list:
            ...
            for team in org_member.teams.all():
                ...
                for project_team in team.projectteam_set.all():
                    if project_team.project_id in self.project_ids and project_team.project.status == TeamStatus.VISIBLE:
