...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_index.py
...
from sentry.models import AuditLogEntryEvent, Organization, OrganizationMember, OrganizationMemberTeam, OrganizationStatus, ProjectPlatform
...
class OrganizationSerializer(serializers.Serializer):
    ...
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50, required=False)
...
class OrganizationIndexEndpoint(Endpoint):
    ...
    def get(self, request):
        ...
        if request.auth and not request.user.is_authenticated():
            ...
            queryset = queryset.filter(member_set__role=roles.get_top_dog().id, member_set__user=request.user, status=OrganizationStatus.VISIBLE)
            org_results = []
        ...
        if not (is_active_superuser(request) and request.GET.get('show') == 'all'):
            queryset = queryset.filter(id__in=OrganizationMember.objects.filter(user=request.user).values('organization'))
        ...
        if query:
            ...
            for key, value in six.iteritems(tokens):
                if key == 'query':
                    value = (' ').join(value)
                    queryset = queryset.filter(Q(name__icontains=value) | Q(slug__icontains=value) | Q(members__email__iexact=value))
                elif key == 'slug':
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated():
            ...
            if limit and ratelimiter.is_limited(('org-create:{}').format(request.user.id), limit=limit, window=3600):
            ...
            if serializer.is_valid():
                ...
                try:
                    with transaction.atomic():
                        ...
                        self.create_audit_entry(request=request, organization=org, target_object=org.id, event=AuditLogEntryEvent.ORG_ADD, data=org.get_audit_log_data())
                ...
                if result.get('agreeTerms'):
                    terms_accepted.send_robust(user=request.user, organization=org, ip_address=request.META['REMOTE_ADDR'], sender=type(self))
