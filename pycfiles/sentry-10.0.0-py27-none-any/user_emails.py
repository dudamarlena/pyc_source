...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/user_emails.py
...
class UserEmailsEndpoint(UserEndpoint):
    ...
    def post(self, request, user):
        ...
        if not validator.is_valid():
            ...
            logger.info('user.email.add', extra={'user_id': user.id, 
    ...
    def put(self, request, user):
        ...
        logger.info('user.email.edit', extra={'user_id': user.id, 
           'ip_address': request.META['REMOTE_ADDR'], 
           'email': new_email})
