...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_servicehook_details.py
...
class ProjectServiceHookDetailsEndpoint(ProjectEndpoint):
    ...
    def put(self, request, project, hook_id):
        ...
        if not request.user.is_authenticated():
            ...
            with transaction.atomic():
                hook.update(**updates)
                self.create_audit_entry(request=request, organization=project.organization, target_object=hook.id, event=AuditLogEntryEvent.SERVICEHOOK_EDIT, data=hook.get_audit_log_data())
            return self.respond(serialize(hook, request.user))
    ...
    def delete(self, request, project, hook_id):
        ...
        with transaction.atomic():
            hook.delete()
            self.create_audit_entry(request=request, organization=project.organization, target_object=hook.id, event=AuditLogEntryEvent.SERVICEHOOK_REMOVE, data=hook.get_audit_log_data())
