...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_details.py
...
from sentry.models import AuditLogEntryEvent, Authenticator, AuthProvider, Organization, OrganizationAvatar, OrganizationOption, OrganizationStatus
...
def update_organization_scenario(runner):
    with runner.isolated_org('Badly Misnamed') as (org):
        runner.request(method='PUT', path='/organizations/%s/' % org.slug, data={'name': 'Impeccably Designated', 'slug': 'impeccably-designated'})


class OrganizationSerializer(serializers.Serializer):
    name = serializers.CharField(max_length=64)
    slug = serializers.RegexField('^[a-z0-9_\\-]+$', max_length=50)
    accountRateLimit = EmptyIntegerField(min_value=0, max_value=1000000, required=False, allow_null=True)
    projectRateLimit = EmptyIntegerField(min_value=50, max_value=100, required=False, allow_null=True)
    avatar = AvatarField(required=False, allow_null=True)
    avatarType = serializers.ChoiceField(choices=(
     ('upload', 'upload'), ('letter_avatar', 'letter_avatar')), required=False, allow_null=True)
    ...
    def validate_slug(self, value):
        if len(value) < 3:
            raise serializers.ValidationError('This slug "%s" is too short. Minimum of 3 characters.' % (value,))
        if value in RESERVED_ORGANIZATION_SLUGS:
            raise serializers.ValidationError('This slug "%s" is reserved and not allowed.' % (value,))
    ...
    def validate(self, attrs):
        ...
        if attrs.get('avatarType') == 'upload':
            has_existing_file = OrganizationAvatar.objects.filter(organization=self.context['organization'], file__isnull=False).exists()
    ...
    def save(self):
        ...
        for key, option, type_, default_value in ORG_OPTIONS:
            ...
            try:
                ...
                OrganizationOption.objects.set_value(organization=org, key=option, value=type_(self.initial_data[key]))
        ...
        org_tracked_field = {'name': org.name, 
           ...
           'flag_field': {'allow_joinleave': org.flags.allow_joinleave.is_set, 
        ...
        if 'avatar' in self.initial_data or 'avatarType' in self.initial_data:
            OrganizationAvatar.save_avatar(relation={'organization': org}, type=self.initial_data.get('avatarType', 'upload'), avatar=self.initial_data.get('avatar'), filename=('{}.png').format(org.slug))
...
class OrganizationDetailsEndpoint(OrganizationEndpoint):
    ...
    def put(self, request, organization):
        ...
        if serializer.is_valid():
            ...
            if was_pending_deletion:
                ...
                delete_logger.info('object.delete.canceled', extra={'object_id': organization.id, 'model': Organization.__name__})
            elif changed_data:
                self.create_audit_entry(request=request, organization=organization, target_object=organization.id, event=AuditLogEntryEvent.ORG_EDIT, data=changed_data)
            return self.respond(serialize(organization, request.user, org_serializers.DetailedOrganizationSerializerWithProjectsAndTeams(), access=request.access))
    ...
    def delete(self, request, organization):
        ...
        updated = Organization.objects.filter(id=organization.id, status=OrganizationStatus.VISIBLE).update(status=OrganizationStatus.PENDING_DELETION)
        if updated:
            transaction_id = uuid4().hex
            countdown = 86400
            entry = self.create_audit_entry(request=request, organization=organization, target_object=organization.id, event=AuditLogEntryEvent.ORG_REMOVE, data=organization.get_audit_log_data(), transaction_id=transaction_id)
            organization.send_delete_confirmation(entry, countdown)
            delete_organization.apply_async(kwargs={'object_id': organization.id, 
               'transaction_id': transaction_id, 
               'actor_id': request.user.id}, countdown=countdown)
            delete_logger.info('object.delete.queued', extra={'object_id': organization.id, 
               'transaction_id': transaction_id, 
