...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/assemble.py
...
ChunkFileState = enum(OK='ok', NOT_FOUND='not_found', CREATED='created', ASSEMBLING='assembling', ERROR='error')
AssembleTask = enum(DIF='project.dsym', ARTIFACTS='organization.artifacts')
...
def _get_cache_key(task, scope, checksum):
    ...
    assembled.
    """
    return 'assemble-status:%s' % hashlib.sha1('%s|%s|%s' % (str(scope).encode('ascii'), checksum.encode('ascii'), task)).hexdigest()
    ...
    if rv is None:
        ...
        try:
            try:
                with temp_file:
                    try:
                        result = debugfile.detect_dif_from_path(temp_file.name, name=name, debug_id=debug_id)
                    except BadDif as e:
...
def assemble_artifacts(org_id, version, checksum, chunks, **kwargs):
    ...
    rv = assemble_file(AssembleTask.ARTIFACTS, organization, 'release-artifacts.zip', checksum, chunks, file_type='release.bundle')
    if rv is None:
        return
    else:
        bundle, temp_file = rv
        ...
        try:
            try:
                ...
                for rel_path, artifact in six.iteritems(artifacts):
                    ...
                    file = File.objects.create(name=artifact_basename, type='release.file', headers=artifact.get('headers', {}))
                ...
                set_assemble_status(AssembleTask.ARTIFACTS, org_id, checksum, ChunkFileState.ERROR, detail=e.message)
            except BaseException:
                logger.error('failed to assemble release bundle', exc_info=True)
                set_assemble_status(AssembleTask.ARTIFACTS, org_id, checksum, ChunkFileState.ERROR, detail='internal server error')
            else:
...
def assemble_file(task, org_or_project, name, checksum, chunks, file_type):
    ...
    if file_size > get_max_file_size(organization):
        set_assemble_status(task, org_or_project.id, checksum, ChunkFileState.ERROR, detail='File exceeds maximum size')
        return
    if set(x[1] for x in file_blobs) != set(chunks):
        set_assemble_status(task, org_or_project.id, checksum, ChunkFileState.ERROR, detail='Not all chunks available for assembling')
        return
    ...
    try:
        ...
        file.delete()
        set_assemble_status(task, org_or_project.id, checksum, ChunkFileState.ERROR, detail='Reported checksum mismatch')
    else:
        file.save()
