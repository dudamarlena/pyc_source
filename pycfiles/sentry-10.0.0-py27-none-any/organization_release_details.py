...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_release_details.py
...
def retrieve_organization_release_scenario(runner):
    runner.request(method='GET', path='/organizations/%s/releases/%s/' % (runner.org.slug, runner.default_release.version))
...
def update_organization_release_scenario(runner):
    ...
    runner.request(method='PUT', path='/organization/%s/releases/%s/' % (runner.org.slug, release.version), data={'url': 'https://vcshub.invalid/user/project/refs/deadbeef1337', 
       'ref': 'deadbeef1337'})
