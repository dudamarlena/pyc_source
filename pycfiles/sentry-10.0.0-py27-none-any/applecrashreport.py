...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/lang/native/applecrashreport.py
...
class AppleCrashReport(object):
    ...
    def _get_exception_info(self):
        ...
        if not exception:
            ...
            if name or signal:
                rv.append('Exception Type: %s%s' % (name or 'Unknown', signal and ' (%s)' % signal or ''))
    ...
    def get_binary_images_apple_string(self):
        if self.symbolicated or self.debug_images is None:
            return ''
        binary_images = map(lambda i: self._convert_debug_meta_to_binary_image_row(debug_image=i), sorted(self.debug_images, key=lambda i: parse_addr(i['image_addr'])))
