...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_ownership.py
    ...
    def validate(self, attrs):
        if not attrs.get('raw', '').strip():
            ...
            try:
                ...
                raise serializers.ValidationError({'raw': 'Parse error: %r (line %d, column %d)' % (
            ...
            if bad_actors:
                raise serializers.ValidationError({'raw': ('Invalid rule owners: {}').format((', ').join(bad_actors))})
            attrs['schema'] = schema
            return attrs
...
class ProjectOwnershipEndpoint(ProjectEndpoint):
    ...
    def put(self, request, project):
        ...
        :auth: required
        """
        serializer = ProjectOwnershipSerializer(data=request.data, partial=True, context={'ownership': self.get_ownership(project)})
