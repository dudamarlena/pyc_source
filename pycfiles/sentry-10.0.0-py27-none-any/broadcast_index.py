...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/broadcast_index.py
...
class BroadcastIndexEndpoint(OrganizationEndpoint):
    ...
    def convert_args(self, request, organization_slug=None, *args, **kwargs):
        if organization_slug:
            args, kwargs = super(BroadcastIndexEndpoint, self).convert_args(request, organization_slug)
        return (
         args, kwargs)
    ...
    def put(self, request):
        ...
        if result.get('hasSeen'):
            ...
            if ids:
                ...
                unseen_queryset = queryset.exclude(id__in=queryset.filter(broadcastseen__user=request.user).values('id'))
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            logger.info('broadcasts.create', extra={'ip_address': request.META['REMOTE_ADDR'], 
