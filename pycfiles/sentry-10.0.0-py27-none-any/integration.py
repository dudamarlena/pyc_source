...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/integration.py
...
class Integration(Model):
    __core__ = False
    organizations = models.ManyToManyField('sentry.Organization', related_name='integrations', through=OrganizationIntegration)
    projects = models.ManyToManyField('sentry.Project', related_name='integrations', through=ProjectIntegration)
    provider = models.CharField(max_length=64)
    external_id = models.CharField(max_length=64)
    name = models.CharField(max_length=200)
    metadata = EncryptedJsonField(default=dict)
    status = BoundedPositiveIntegerField(default=ObjectStatus.VISIBLE, choices=ObjectStatus.as_choices(), null=True)
    date_added = models.DateTimeField(default=timezone.now, null=True)
    ...
    def add_organization(self, organization, user=None, default_auth_id=None):
        ...
        """
        try:
            org_integration, created = OrganizationIntegration.objects.get_or_create(organization_id=organization.id, integration_id=self.id, defaults={'default_auth_id': default_auth_id, 'config': {}})
            if not created and default_auth_id:
                org_integration.update(default_auth_id=default_auth_id)
        except IntegrityError:
            return False

        integration_added.send_robust(integration=self, organization=organization, user=user, sender=self.__class__)
