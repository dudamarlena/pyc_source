...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/organization_projects.py
...
class OrganizationProjectsEndpoint(OrganizationEndpoint, EnvironmentMixin):
    ...
    def get(self, request, organization):
        ...
        if stats_period not in (None, '', '24h', '14d', '30d'):
            ...
            return self.paginate(request=request, queryset=queryset, order_by=order_by, on_results=lambda x: serialize(x, request.user, ProjectSummarySerializer(environment_id=self._get_environment_id_from_request(request, organization.id), stats_period=stats_period)), paginator_cls=OffsetPaginator)