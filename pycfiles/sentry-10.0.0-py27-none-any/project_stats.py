...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_stats.py
...
def retrieve_event_counts_project(runner):
    runner.request(method='GET', path='/projects/%s/%s/stats/' % (runner.org.slug, runner.default_project.slug))
...
class ProjectStatsEndpoint(ProjectEndpoint, EnvironmentMixin, StatsMixin):
    ...
    def get(self, request, project):
        ...
        data = tsdb.get_range(model=stat_model, keys=[project.id], **self._parse_args(request, **query_kwargs))[project.id]
