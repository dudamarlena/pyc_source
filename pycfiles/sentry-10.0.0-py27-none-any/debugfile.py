...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/debugfile.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import re, os, six, uuid, time, errno, shutil, hashlib, logging, tempfile
...
CONVERSION_ERROR_TTL = 600
DIF_MIMETYPES = dict((v, k) for k, v in KNOWN_DIF_FORMATS.items())
_proguard_file_re = re.compile('/proguard/(?:mapping-)?(.*?)\\.txt$')
...
class ProjectDebugFileManager(BaseManager):
    ...
    def find_missing(self, checksums, project):
        ...
        found = ProjectDebugFile.objects.filter(file__checksum__in=checksums, project=project).values('file__checksum')
        for values in found:
    ...
    def find_by_debug_ids(self, project, debug_ids, features=None):
        ...
        difs = ProjectDebugFile.objects.filter(project=project, debug_id__in=debug_ids).select_related('file').order_by('-id')
        difs_by_id = {}
        for dif in difs:
...
class ProjectDebugFile(Model):
    ...
    class Meta:
        index_together = (
         ('project', 'debug_id'), ('project', 'code_id'))
...
def clean_redundant_difs(project, debug_id):
    ...
    """
    difs = ProjectDebugFile.objects.filter(project=project, debug_id=debug_id).select_related('file').order_by('-id')
    all_features = set()
...
def create_dif_from_id(project, meta, fileobj=None, file=None):
    ...
    dif = ProjectDebugFile.objects.select_related('file').filter(project=project, debug_id=meta.debug_id, file__checksum=checksum, data__isnull=False).order_by('-id').first()
    if dif is not None:
        return (dif, False)
    else:
        if file is None:
            ...
            file.save()
        dif = ProjectDebugFile.objects.create(file=file, debug_id=meta.debug_id, code_id=meta.code_id, cpu_name=meta.arch, object_name=object_name, project=project, data=meta.data)
...
def detect_dif_from_path(path, name=None, debug_id=None):
    ...
    if proguard_id is not None:
        ...
        return [
         DifMeta(file_format='proguard', arch='any', debug_id=proguard_id, code_id=None, path=path, name=name, data=data)]
    else:
        try:
