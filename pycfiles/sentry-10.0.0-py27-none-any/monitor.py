...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/monitor.py
...
from sentry.db.models import Model, BoundedPositiveIntegerField, EncryptedJsonField, UUIDField, sane_repr
SCHEDULE_INTERVAL_MAP = {'year': rrule.YEARLY, 
   'month': rrule.MONTHLY, 
   'week': rrule.WEEKLY, 
   'day': rrule.DAILY, 
   'hour': rrule.HOURLY, 
...
def get_next_schedule(base_datetime, schedule_type, schedule):
    if schedule_type == ScheduleType.CRONTAB:
        ...
        rule = rrule.rrule(freq=SCHEDULE_INTERVAL_MAP[unit_name], interval=count, dtstart=base_datetime, count=2)
...
class Monitor(Model):
    ...
    status = BoundedPositiveIntegerField(default=MonitorStatus.ACTIVE, choices=MonitorStatus.as_choices())
    type = BoundedPositiveIntegerField(default=MonitorType.UNKNOWN, choices=MonitorType.as_choices())
    ...
    def mark_failed(self, last_checkin=None):
        ...
        affected = type(self).objects.filter(Q(last_checkin__lte=last_checkin) | Q(last_checkin__isnull=True), id=self.id).update(next_checkin=self.get_next_scheduled_checkin(next_checkin_base), status=MonitorStatus.ERROR, last_checkin=last_checkin)
        if not affected:
            return False
        else:
            event_manager = EventManager({'logentry': {'message': 'Monitor failure: %s' % (self.name,)}, 'contexts': {'monitor': {'id': six.text_type(self.guid)}}}, project=Project(id=self.project_id))
