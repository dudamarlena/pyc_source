...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/tasks/unmerge.py
...
from sentry.models import Activity, Environment, Event, EventUser, Group, GroupEnvironment, GroupHash, GroupRelease, Project, Release, UserReport
...
def get_caches():
    return {'Environment': cache(lambda organization_id, name: Environment.objects.get(organization_id=organization_id, name=name)), 
       'GroupRelease': cache(lambda group_id, environment, release_id: GroupRelease.objects.get(group_id=group_id, environment=environment, release_id=release_id)), 
       'Project': cache(lambda id: Project.objects.get(id=id)), 
       'Release': cache(lambda organization_id, version: Release.objects.get(organization_id=organization_id, version=version))}
...
initial_fields = {'culprit': lambda event: _generate_culprit(event), 
   'data': lambda event: {'last_received': event.data.get('received') or float(event.datetime.strftime('%s')), 
...
backfill_fields = {'platform': lambda caches, data, event: event.platform, 
   ...
   'first_release': lambda caches, data, event: caches['Release'](caches['Project'](event.project_id).organization_id, event.get_tag('sentry:release')) if event.get_tag('sentry:release') else data.get('first_release', None), 
   'times_seen': lambda caches, data, event: data['times_seen'] + 1, 
   'score': lambda caches, data, event: Group.calculate_score(data['times_seen'] + 1, data['last_seen'])}

def get_group_creation_attributes(caches, events):
    latest_event = events[0]
    return reduce(lambda data, event: merge_mappings([
     data, {name:f(caches, data, event) for name, f in backfill_fields.items()}]), events, {name:f(latest_event) for name, f in initial_fields.items()})


def get_group_backfill_attributes(caches, group, events):
    return {k:v for k, v in reduce(lambda data, event: merge_mappings([
     data, {name:f(caches, data, event) for name, f in backfill_fields.items()}]), events, {name:getattr(group, name) for name in set(initial_fields.keys()) | set(backfill_fields.keys())}).items() if k in backfill_fields if k in backfill_fields}
...
def migrate_events(caches, project, source_id, destination_id, fingerprints, events, actor_id, eventstream_state):
    if not events:
        ...
        if destination_id is None:
            ...
            eventstream_state = eventstream.start_unmerge(project.id, fingerprints, source_id, destination_id)
            GroupHash.objects.filter(project_id=project.id, hash__in=fingerprints).update(group=destination_id)
            Activity.objects.create(project_id=project.id, group_id=destination_id, type=Activity.UNMERGE_DESTINATION, user_id=actor_id, data={'fingerprints': fingerprints, 'source_id': source_id})
            Activity.objects.create(project_id=project.id, group_id=source_id, type=Activity.UNMERGE_SOURCE, user_id=actor_id, data={'fingerprints': fingerprints, 'destination_id': destination_id})
        else:
        ...
        tagstore.update_group_for_events(project_id=project.id, event_ids=event_id_set, destination_id=destination_id)
        event_event_id_set = set(event.event_id for event in events)
        UserReport.objects.filter(project_id=project.id, event_id__in=event_event_id_set).update(group=destination_id)
        return (
         destination.id, eventstream_state)
    ...
    for event in events:
        ...
        for key, value in event.tags:
            ...
            if value in values:
                ...
                values[value] = (
                 1, event.datetime, event.datetime)
...
def repair_tag_data(caches, project, events):
    for (group_id, env_name), keys in collect_tag_data(events).items():
        ...
        for key, values in keys.items():
            tagstore.get_or_create_group_tag_key(project_id=project.id, group_id=group_id, environment_id=environment.id, key=key)
            for value, (times_seen, first_seen, last_seen) in values.items():
                _, created = tagstore.get_or_create_group_tag_value(project_id=project.id, group_id=group_id, environment_id=environment.id, key=key, value=value, defaults={'first_seen': first_seen, 
                   'last_seen': last_seen, 
                   'times_seen': times_seen})
                if not created:
                    tagstore.incr_group_tag_value_times_seen(project_id=project.id, group_id=group_id, environment_id=environment.id, key=key, value=value, count=times_seen, extra={'first_seen': first_seen})
    ...
    for event in events:
        ...
        if user:
            sets[event.datetime][tsdb.models.users_affected_by_group][(
             event.group_id, environment.id)].add(get_event_user_from_interface(user).tag_value)
...
def unlock_hashes(project_id, fingerprints):
    GroupHash.objects.filter(project_id=project_id, hash__in=fingerprints, state=GroupHash.State.LOCKED_IN_MIGRATION).update(state=GroupHash.State.UNLOCKED)


@instrumented_task(name='sentry.tasks.unmerge', queue='unmerge')
def unmerge(project_id, source_id, destination_id, fingerprints, actor_id, cursor=None, batch_size=500, source_fields_reset=False, eventstream_state=None):
    ...
    unmerge.delay(project_id, source_id, destination_id, fingerprints, actor_id, cursor=events[(-1)].id, batch_size=batch_size, source_fields_reset=source_fields_reset, eventstream_state=eventstream_state)
    return