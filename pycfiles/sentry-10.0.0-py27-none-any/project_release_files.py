...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_release_files.py
...
def upload_file_scenario(runner):
    runner.request(method='POST', path='/projects/%s/%s/releases/%s/files/' % (
     runner.org.slug, runner.default_project.slug, runner.default_release.version), data={'header': 'Content-Type:text/plain; encoding=utf-8', 
       'name': '/demo/hello.py', 
       'file': (
              'hello.py', BytesIO('print "Hello World!"'))}, format='multipart')


@scenario('ListReleaseFiles')
def list_files_scenario(runner):
    runner.utils.create_release_file(project=runner.default_project, release=runner.default_release, path='/demo/message-for-you.txt', contents='Hello World!')
    runner.request(method='GET', path='/projects/%s/%s/releases/%s/files/' % (
...
class ProjectReleaseFilesEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project, version):
        ...
        return self.paginate(request=request, queryset=file_list, order_by='name', paginator_cls=OffsetPaginator, max_offset=MAX_RELEASE_FILES_OFFSET, on_results=lambda r: serialize(load_dist(r), request.user))
        ...
        if 'file' not in request.data:
            ...
            try:
                with transaction.atomic():
                    releasefile = ReleaseFile.objects.create(organization_id=release.organization_id, release=release, file=file, name=full_name, dist=dist)
