...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/releaseenvironment.py
...
class ReleaseEnvironment(Model):
    ...
    def get_cache_key(cls, organization_id, release_id, environment_id):
        return ('releaseenv:2:{}:{}:{}').format(organization_id, release_id, environment_id)
    ...
    def get_or_create(cls, project, release, environment, datetime, **kwargs):
        ...
        if instance is None:
            instance, created = cls.objects.get_or_create(release_id=release.id, organization_id=project.organization_id, environment_id=environment.id, defaults={'first_seen': datetime, 'last_seen': datetime})
