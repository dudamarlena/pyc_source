...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/project_servicehooks.py
...
def list_hooks_scenario(runner):
    runner.request(method='GET', path='/projects/%s/%s/hooks/' % (runner.org.slug, runner.default_project.slug))


@scenario('CreateServiceHook')
def create_hook_scenario(runner):
    runner.request(method='POST', path='/projects/%s/%s/hooks/' % (runner.org.slug, runner.default_project.slug), data={'url': 'https://example.com/sentry-hook', 'events': ['event.alert', 'event.created']})
...
class ProjectServiceHooksEndpoint(ProjectEndpoint):
    ...
    def get(self, request, project):
        ...
        if not self.has_feature(request, project):
            return self.respond({'error_type': 'unavailable_feature', 
               'detail': [
                        'You do not have that feature enabled']}, status=403)
    ...
    def post(self, request, project):
        ...
        if not request.user.is_authenticated():
            ...
            with transaction.atomic():
                hook = service_hooks.Creator.run(projects=[
                 project], organization=project.organization, url=result['url'], actor=request.user, events=result.get('events'), application=getattr(request.auth, 'application', None) if request.auth else None)
