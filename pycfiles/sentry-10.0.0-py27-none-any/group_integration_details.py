...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/api/endpoints/group_integration_details.py
...
class GroupIntegrationDetailsEndpoint(GroupEndpoint):
    ...
    def create_issue_activity(self, request, group, installation, external_issue):
        issue_information = {'title': external_issue.title, 
    ...
    def get(self, request, group, integration_id):
        ...
        if not (integration.has_feature(IntegrationFeatures.ISSUE_BASIC) or integration.has_feature(IntegrationFeatures.ISSUE_SYNC)):
            return Response({'detail': 'This feature is not supported for this integration.'}, status=400)
        try:
            return Response(serialize(integration, request.user, IntegrationIssueConfigSerializer(group, action, params=request.GET), organization_id=organization_id))
    ...
    def post(self, request, group, integration_id):
        ...
        external_issue_key = installation.make_external_key(data)
        external_issue, created = ExternalIssue.objects.get_or_create(organization_id=organization_id, integration_id=integration.id, key=external_issue_key, defaults={'title': data.get('title'), 
           'description': data.get('description'), 
           'metadata': data.get('metadata')})
        try:
            with transaction.atomic():
    ...
    def delete(self, request, group, integration_id):
        ...
        with transaction.atomic():
            GroupLink.objects.filter(group_id=group.id, project_id=group.project_id, linked_type=GroupLink.LinkedType.issue, linked_id=external_issue_id, relationship=GroupLink.Relationship.references).delete()
