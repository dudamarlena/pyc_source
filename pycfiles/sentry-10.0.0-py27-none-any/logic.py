...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/incidents/logic.py
...
from sentry.incidents.models import AlertRule, AlertRuleAggregations, AlertRuleStatus, Incident, IncidentActivity, IncidentActivityType, IncidentGroup, IncidentProject, IncidentSnapshot, IncidentSeen, IncidentStatus, IncidentSubscription, IncidentType, SnubaDatasets, TimeSeriesSnapshot
from sentry.models import Commit, Release
from sentry.incidents import tasks
from sentry.utils.committers import get_event_file_committers
from sentry.utils.snuba import _snuba_pool, bulk_raw_query, raw_query, SnubaError, SnubaQueryParams, SnubaTSResult, zerofill
MAX_INITIAL_INCIDENT_PERIOD = timedelta(days=7)
alert_aggregation_to_snuba = {AlertRuleAggregations.TOTAL: ('count()', '', 'count'), 
...
def create_incident(organization, type, title, query, date_started=None, date_detected=None, detection_uuid=None, projects=None, groups=None, user=None):
    if groups:
    ...
    with transaction.atomic():
        incident = Incident.objects.create(organization=organization, detection_uuid=detection_uuid, status=IncidentStatus.OPEN.value, type=type.value, title=title, query=query, date_started=date_started, date_detected=date_detected)
        if projects:
            IncidentProject.objects.bulk_create([ IncidentProject(incident=incident, project=project) for project in projects ])
        if groups:
            IncidentGroup.objects.bulk_create([ IncidentGroup(incident=incident, group=group) for group in groups ])
        ...
        create_incident_activity(incident, activity_status, event_stats_snapshot=event_stats_snapshot, user=user)
        analytics.record('incident.created', incident_id=incident.id, organization_id=incident.organization_id, incident_type=type.value)
...
def calculate_incident_start(query, projects, groups):
    ...
    result = raw_query(aggregations=[
     ('count()', '', 'count'), ('min', 'timestamp', 'first_seen')], orderby='time', groupby=[
     'time'], rollup=rollup, referrer='incidents.calculate_incident_start', limit=10000, **query_args)['data']
...
def update_incident_status(incident, status, user=None, comment=None):
    ...
    with transaction.atomic():
        create_incident_activity(incident, IncidentActivityType.STATUS_CHANGE, user=user, value=status.value, previous_value=incident.status, comment=comment)
        if user:
        ...
        analytics.record('incident.status_change', incident_id=incident.id, organization_id=incident.organization_id, incident_type=incident.type, prev_status=prev_status, status=incident.status)
        return incident
    return
...
def set_incident_seen(incident, user=None):
    ...
    """
    incident_seen, created = IncidentSeen.objects.create_or_update(incident=incident, user=user, values={'last_seen': timezone.now()})
...
@transaction.atomic
def create_incident_activity(incident, activity_type, user=None, value=None, previous_value=None, comment=None, event_stats_snapshot=None, mentioned_user_ids=None):
    ...
    activity = IncidentActivity.objects.create(incident=incident, type=activity_type.value, user=user, value=value, previous_value=previous_value, comment=comment, event_stats_snapshot=event_stats_snapshot)
    if mentioned_user_ids:
        user_ids_to_subscribe = set(mentioned_user_ids) - set(IncidentSubscription.objects.filter(incident=incident, user_id__in=mentioned_user_ids).values_list('user_id', flat=True))
        if user_ids_to_subscribe:
            IncidentSubscription.objects.bulk_create([ IncidentSubscription(incident=incident, user_id=mentioned_user_id) for mentioned_user_id in user_ids_to_subscribe
                                                     ])
    tasks.send_subscriber_notifications.apply_async(kwargs={'activity_id': activity.id}, countdown=10)
    if activity_type == IncidentActivityType.COMMENT:
        analytics.record('incident.comment', incident_id=incident.id, organization_id=incident.organization_id, incident_type=incident.type, user_id=user.id if user else None, activity_id=activity.id)
    return activity
...
def create_incident_snapshot(incident):
    ...
    event_stats_snapshot = create_event_stat_snapshot(incident, incident.date_started, incident.date_closed)
    aggregates = get_incident_aggregates(incident)
    return IncidentSnapshot.objects.create(incident=incident, event_stats_snapshot=event_stats_snapshot, unique_users=aggregates['unique_users'], total_events=aggregates['count'])
...
def create_event_stat_snapshot(incident, start, end):
    ...
    return TimeSeriesSnapshot.objects.create(start=start, end=end, values=[ [row['time'], row['count']] for row in event_stats.data['data'] ], period=event_stats.rollup)
...
def bulk_build_incident_query_params(incidents, start=None, end=None):
    ...
    for incident_id, group_id in IncidentGroup.objects.filter(incident__in=incidents).values_list('incident_id', 'group_id'):
        incident_groups[incident_id].append(group_id)

    incident_projects = defaultdict(list)
    for incident_id, project_id in IncidentProject.objects.filter(incident__in=incidents).values_list('incident_id', 'project_id'):
        incident_projects[incident_id].append(project_id)

    query_args_list = []
    for incident in incidents:
        params = {'start': incident.date_started if start is None else start, 'end': incident.current_end_date if end is None else end}
...
def bulk_get_incident_event_stats(incidents, query_params_list, data_points=50):
    snuba_params_list = [ SnubaQueryParams(aggregations=[('count()', '', 'count')], orderby='time', groupby=['time'], rollup=max(int(incident.duration.total_seconds() / data_points), 1), limit=10000, **query_param) for incident, query_param in zip(incidents, query_params_list)
                        ]
    results = bulk_raw_query(snuba_params_list, referrer='incidents.get_incident_event_stats')
    return [ SnubaTSResult(result, snuba_params.start, snuba_params.end, snuba_params.rollup) for snuba_params, result in zip(snuba_params_list, results)
           ]
...
def bulk_get_incident_aggregates(query_params_list):
    snuba_params_list = [ SnubaQueryParams(aggregations=[('count()', '', 'count'), ('uniq', 'tags[sentry:user]', 'unique_users')], limit=10000, **query_param) for query_param in query_params_list
                        ]
...
def bulk_get_incident_stats(incidents):
    ...
    for snapshot in snapshots:
        ...
        incident_stats[snapshot.incident_id] = {'event_stats': SnubaTSResult(event_stats.snuba_values, event_stats.start, event_stats.end, event_stats.period), 
    ...
    if to_fetch:
        ...
        for incident, event_stats, aggregates in zip(to_fetch, all_event_stats, all_aggregates):
            incident_stats[incident.id] = {'event_stats': event_stats, 'total_events': aggregates['count'], 
...
def get_incident_activity(incident):
    return IncidentActivity.objects.filter(incident=incident).select_related('user', 'event_stats_snapshot', 'incident')


def get_incident_suspects(incident, projects):
    return Commit.objects.filter(incidentsuspectcommit__incident=incident, releasecommit__release__projects__in=projects).distinct()
...
def create_alert_rule(project, name, threshold_type, query, aggregations, time_window, alert_threshold, resolve_threshold, threshold_period):
    """
    ...
    try:
        subscription_id = create_snuba_subscription(project, dataset, query, aggregations, time_window, resolution)
        alert_rule = AlertRule.objects.create(project=project, name=name, subscription_id=subscription_id, threshold_type=threshold_type.value, dataset=SnubaDatasets.EVENTS.value, query=query, aggregations=[ agg.value for agg in aggregations ], time_window=time_window, resolution=resolution, alert_threshold=alert_threshold, resolve_threshold=resolve_threshold, threshold_period=threshold_period)
    except Exception:
        if subscription_id:
...
def update_alert_rule(alert_rule, name=None, threshold_type=None, query=None, aggregations=None, time_window=None, alert_threshold=None, resolve_threshold=None, threshold_period=None):
    """
    ...
    """
    if name and alert_rule.name != name and AlertRule.objects.filter(project=alert_rule.project, name=name).exists():
        raise AlertRuleNameAlreadyUsedError()
...
def delete_alert_rule(alert_rule):
    ...
    alert_rule.update(name=uuid4().get_hex(), status=AlertRuleStatus.PENDING_DELETION.value)
...
def create_snuba_subscription(project, dataset, query, aggregations, time_window, resolution):
    ...
    """
    response = _snuba_pool.urlopen('POST', '/subscriptions', body=json.dumps({'project_id': project.id, 
       'dataset': dataset.value, 
       'conditions': get_snuba_query_args(query)['conditions'], 
       'aggregates': [ alert_aggregation_to_snuba[agg] for agg in aggregations ], 'time_window': time_window, 
       'resolution': resolution}), retries=False)
