...
# [GCC 8.4.0]
# Embedded file name: /usr/src/sentry/src/sentry/models/authenticator.py
# Compiled at: 2019-08-16 17:27:45
from __future__ import absolute_import
import os, hmac, time, base64, hashlib, six
...
from sentry.db.models import BaseManager, BaseModel, BoundedAutoField, BoundedPositiveIntegerField, EncryptedPickledObjectField, FlexibleForeignKey
...
class AuthenticatorManager(BaseManager):
    ...
    def all_interfaces_for_user(self, user, return_missing=False, ignore_backup=False):
        ...
        ifaces = [ x.interface for x in Authenticator.objects.filter(user=user, type__in=[ a.type for a in available_authenticators(ignore_backup=ignore_backup) ])
                 ]
        if return_missing:
    ...
    def auto_add_recovery_codes(self, user, force=False):
        ...
        if not force:
            for authenticator in Authenticator.objects.filter(user=user, type__in=[ a.type for a in available_authenticators() ]):
    ...
    def user_has_2fa(self, user):
        ...
        """
        return Authenticator.objects.filter(user=user, type__in=[ a.type for a in available_authenticators(ignore_backup=True) ]).exists()

    def bulk_users_have_2fa(self, user_ids):
        """Checks if a list of user ids have 2FA configured.
        Returns a dict of {<id>: <has_2fa>}
        """
        authenticators = set(Authenticator.objects.filter(user__in=user_ids, type__in=[ a.type for a in available_authenticators(ignore_backup=True) ]).distinct().values_list('user_id', flat=True))
...
class AuthenticatorInterface(object):
    ...
    def enroll(self, user):
        ...
        if self.authenticator is None:
            self.authenticator = Authenticator.objects.create(user=user, type=self.type, config=self.config)
        else:
...
class RecoveryCodeInterface(AuthenticatorInterface):
    ...
    description = _('Recovery codes are the only way to access your account if you lose your device and cannot receive two factor authentication codes.')
...
class OtpMixin(object):
    ...
    def validate_otp(self, otp):
        if not otp:
            ...
            used_counter = self.make_otp().verify(otp, return_counter=True, check_counter_func=self.check_otp_counter)
...
class TotpInterface(OtpMixin, AuthenticatorInterface):
    ...
    description = _('An authenticator application that supports TOTP (like Google Authenticator or 1Password) can be used to conveniently secure your account.  A new token is generated every 30 seconds.')
...
class SmsInterface(OtpMixin, AuthenticatorInterface):
    ...
    interface_id = 'sms'
    name = _('Text Message')
    description = _("This authenticator sends you text messages for verification.  It's useful as a backup method or when you do not have a phone that supports an authenticator application.")
    code_ttl = 45
    ...
    del _get_phone_number
    del _set_phone_number
    ...
    def activate(self, request):
        ...
        if self.send_text(request=request):
            return ActivationMessageResult(_('A confirmation code was sent to %(phone_mask)s. It is valid for %(ttl)d seconds.') % {'phone_mask': '<strong>%s</strong>' % mask, 'ttl': self.code_ttl})
        return ActivationMessageResult(_('Error: we failed to send a text message to you. You can try again later or sign in with a different method.'), type='error')

    def send_text(self, for_enrollment=False, request=None):
        ctx = {'code': self.make_otp().generate_otp()}
        if for_enrollment:
            text = _('%(code)s is your Sentry two-factor enrollment code. You are about to set up text message based two-factor authentication.')
        else:
...
class U2fInterface(AuthenticatorInterface):
    ...
    description = _("Authenticate with a U2F hardware device. This is a device like a Yubikey or something similar which supports FIDO's U2F specification. This also requires a browser which supports this system (like Google Chrome).")
    ...
    def get_registered_devices(self):
        rv = []
        for device in self.config.get('devices') or ():
            rv.append({'timestamp': to_datetime(device['ts']), 
               'name': device['name'], 
    ...
    def try_enroll(self, enrollment_data, response_data, device_name=None):
        ...
        devices.append({'name': device_name or 'Security Key', 
           'ts': int(time.time()), 
    ...
    def activate(self, request):
        ...
        for registered_key in challenge['registeredKeys']:
            authenticate_requests.append({'challenge': challenge['challenge'], 
