...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hybrid/util/test_wsgi.py
# Compiled at: 2016-01-19 04:33:27
import urllib, tempfile, urlparse
...
class TestWSGIApplication:
    ...
    def __getattr__(self, attr):
        if attr.upper() not in self.HTTP_METHODS:
    ...
    def finalize(self):
        ...
        return '%s ' % self._env['SERVER_PROTOCOL'] + self._status + '\r\n' + ('\r\n').join([ '%s: %s' % (k, v) for (k, v) in self.headers.items() ]) + '\r\n\r\n' + self.body