...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hybrid/commands.py
# Compiled at: 2016-08-05 07:51:44
import os.path, os, shutil, sys
from pkg_resources import resource_filename, resource_isdir, resource_listdir, resource_string
...
def show_subcommands(exitcode=0, color=COLOR['green']):
    subcommands = ('\n').join([ '    ' + attr[12:] for attr in dir(_DispatchCommand) if attr.startswith('_subcommand_') if callable(getattr(_DispatchCommand, attr))
                              ])
    show_and_exit('Available subcommands:\n[hybrid]\n%s' % subcommands, exitcode, color)
...
class _DispatchCommand:
    ...
    def execute(self):
        ...
        if not hasattr(self, subcommand_name) or not callable(getattr(self, subcommand_name)):
            show_and_exit("Unknown command: '%s'\nType '%s help' for usage." % (self._subcommand, os.path.basename(__file__)))
    ...
    def _subcommand_startproject(self):
        ...
        if not is_valid_name(self._args[0]):
            show_and_exit("'%s' is not a valid project name. Please make sure the name begins with a letter or underscore." % self._args[0])
    ...
    def _subcommand_linecount(self):
        if len(self._args) < 1 or not os.path.isdir(self._args[0]) or not os.access(self._args[0], os.R_OK):
