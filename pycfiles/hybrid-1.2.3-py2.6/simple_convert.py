...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hybrid/util/simple_convert.py
# Compiled at: 2016-08-28 09:44:22
            ...
            if isinstance(value, _simple_types):
                ...
                for one_value in value._values if hasattr(value, '_values') else value:
                    if isinstance(one_value, _simple_types):
                        result.setdefault(field_descriptor.name, []).append(one_value)
                    else:
                        result.setdefault(field_descriptor.name, []).append(get_json(one_value, {}))
...
class json2pb:
    ...
    def __init__(self, pb_module, root_name, strip_keys=None):
        self._module = pb_module if type(pb_module) == types.ModuleType else __import__(pb_module)
