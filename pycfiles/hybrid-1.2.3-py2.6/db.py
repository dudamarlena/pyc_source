...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/hybrid/util/db.py
# Compiled at: 2016-02-03 01:16:25
import types, re
...
class db_pool(object):
    ...
    def __init__(self, db_api, **kwargs):
        if type(db_api) == types.ModuleType:
            ...
            raise InterfaceError('db_api should be a module or basestring object, not %s.' % type(db_api).__name__)
        self._kwargs = kwargs
    ...
    def create_pool(self):
        if self._pool is None:
            self._pool = PooledDB.PooledDB(self._db_api, self.mincached, self.maxcached, self.maxshared, self.maxconnections, self.blocking, self.ping, **self._kwargs)
        return
...
class db(object):
    ...
    def escape_string(self, s):
        ...
        ESCAPE_MAP = {'\x00': '\\0', '\n': '\\n', '\r': '\\r', '\x1a': '\\Z', "'": "\\'", 
           '"': '\\"', '\\': '\\\\'}
        return '%s' % ESCAPE_REGEX.sub(lambda match: ESCAPE_MAP.get(match.group(0)), s)
    ...
    def insert(self, table, fields, escape=True, update=''):
        items = map(lambda t: (t[0],
         self.escape_string(t[1]) if isinstance(t[1], basestring) and escape else t[1]), fields.items())
