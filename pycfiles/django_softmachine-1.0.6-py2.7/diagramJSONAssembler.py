...
# [GCC 8.4.0]
# Embedded file name: build/bdist.macosx-10.7-x86_64/egg/dbDesigner/service/diagramJSONAssembler.py
...
class JSONAssembler:
    ...
    def getJSONElements(self, entities, selectedTables, connectors):
        ...
        for entity in entities:
            table = {'type': 'dbModel.shape.DBTable', 'id': str(uuid.UUID(entity.smUUID)), 'x': x, 
               'y': y, 
               'width': 98, 
               'height': 81.265625, 
               'userData': '', 
    ...
    def addOutputPorts(self, entity, table, connectors):
        ...
        for port in relationships:
            if port.refEntity != port.entity:
                ...
                table['tablePorts'].append({'type': 'draw2d_OutputPort', 
                   'name': outputPortName, 
                   'position': 'right'})
    ...
    def addConnectors(self, relationship, table, connectors):
        ...
        if relationship.refEntity == relationship.entity:
            ...
            table['tablePorts'].append({'type': 'draw2d_HybridPort', 
               'name': outputPortName, 
               'position': 'bottom'})
        table['tablePorts'].append({'type': 'draw2d_InputPort', 
           'name': inputPortName, 
           'position': 'left'})
    ...
    def appendConnector(self, relationship, outputPortName, inputPortName, connectors):
        connector = {'type': 'dbModel.shape.TableConnection', 
           'name': relationship.code, 
           'id': str(uuid.UUID(relationship.smUUID)), 
           'userData': {'isPrimary': relationship.isPrimary, 'useDecorators': False}, 'cssClass': 'draw2d_Connection', 
           'stroke': 2, 
           'color': '#5BCAFF', 
           'policy': 'draw2d.policy.line.OrthogonalSelectionFeedbackPolicy', 
           'router': 'draw2d.layout.connection.InteractiveManhattanConnectionRouter', 
           'source': {'node': str(uuid.UUID(relationship.refEntity.smUUID)), 
                      'port': outputPortName}, 
           'target': {'node': str(uuid.UUID(relationship.entity.smUUID)), 
                      'port': inputPortName}}
