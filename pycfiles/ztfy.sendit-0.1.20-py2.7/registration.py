...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/sendit/skin/app/registration.py
# Compiled at: 2015-02-06 09:38:46
...
from ztfy.sendit.profile.interfaces import IProfile, IUserBaseRegistrationInfo, IUserRegistrationInfo, IUserRegistrationConfirmInfo
...
class UserRegistrationForm(BaseAddForm):
        ...
        if not errors:
            ...
            if error:
                view = getMultiAdapter((error, self.request, self.widgets['email'],
                 self.widgets['email'].field, self, self.context), IErrorViewSnippet)
                view.update()
                errors += (view,)
    ...
    def create(self, data):
        ...
        principal = InternalPrincipal(login=email, password=data.get('password'), title='%s %s' % (data.get('lastname'), data.get('firstname')), description=data.get('company_name') or '')
        ...
        if mailer is not None:
            ...
            message_body = self.registration_message_template(request=self.request, context=self.context, hash=profile.activation_hash)
            message = HTMLMessage(subject=translate(_('[%s] Please confirm registration'), context=self.request) % II18n(self.context).queryAttribute('mail_subject_header', request=self.request), fromaddr='%s <%s>' % (self.context.mail_sender_name,
...
class ExternalUserRegistrationForm(BaseDialogAddForm):
    ...
    def extractData(self, setErrors=True):
        ...
        if not errors:
            ...
            if error:
                view = getMultiAdapter((error, self.request, self.widgets['email'], self.widgets['email'].field, self, self.context), IErrorViewSnippet)
                view.update()
                errors += (view,)
                self.widgets.errors = errors
        if errors:
    ...
    def create(self, data):
        ...
        principal = InternalPrincipal(login=email, password=password, title='%s %s' % (data.get('lastname'), data.get('firstname')), description=data.get('company_name') or '')
        notify(ObjectCreatedEvent(principal))
        plugin = queryUtility(IAuthenticatorPlugin, self.context.external_auth_plugin)
        profile = getUserProfile(plugin.prefix + principal.login)
        ...
        if mailer is not None:
            ...
            if not source_mail:
                source_mail = (
                 self.context.mail_sender_name, self.context.mail_sender_address)
            message_body = self.registration_message_template(request=self.request, context=self.context, hash=profile.activation_hash)
            message = HTMLMessage(subject=translate(_('[%s] Please confirm registration'), context=self.request) % II18n(self.context).queryAttribute('mail_subject_header', request=self.request), fromaddr='%s via %s <%s>' % (source_mail[0],
