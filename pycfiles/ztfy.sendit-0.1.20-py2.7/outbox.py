...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/sendit/skin/app/outbox.py
# Compiled at: 2016-09-02 09:17:56
...
class SendedColumn(GetAttrColumn):
    ...
    def getValue(self, obj):
        ...
        packet_oid = intids.register(obj)
...
class ISenditOutboxDeleteInfo(Interface):
    ...
    packet_oid = Int(title=_('Packet OID'), required=True)
    notify_recipients = Bool(title=_('Notify recipients?'), description=_("Do you want to notify recipients of this packet that it won't be available anymore?"), required=True, default=True)
    ...
    def ajaxCreate(self):
        ...
        if errors:
            ...
            if packet is not None and ISecurityManager(packet).canUsePermission('ztfy.ManageSenditPacket'):
                ...
                return writer.write({'output': 'OK', 'packet_oid': packet_oid})
