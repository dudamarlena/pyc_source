...
# [GCC 8.4.0]
# Embedded file name: build/bdist.linux-x86_64/egg/ztfy/sendit/skin/app/login.py
# Compiled at: 2016-09-02 08:26:04
...
class ILoginFormFields(Interface):
    ...
    username = TextLine(title=_('login-field', 'Login'), description=_('Principal ID or email address'), required=True)
...
class LoginView(BaseAddForm):
    ...
    def handleLogin(self, action):
        ...
        if errors:
            ...
            if self.principal is not None:
                ...
                if isUnauthorized(self):
                    context, _action, _permission = self.context.args
                    self.request.response.redirect(absoluteURL(context, self.request), trusted=app.trusted_redirects)
                else:
                    came_from = data.get('came_from')
                    if came_from:
                        self.request.response.redirect(came_from, trusted=app.trusted_redirects)
                    else:
                        ...
                        self.request.response.redirect('%s/%s' % (absoluteURL(self.context, self.request),
                         target.viewname if target is not None else '@@index.html'), trusted=app.trusted_redirects)
                return ''
            self.request.response.redirect('%s/@@login.html?came_from=%s' % (absoluteURL(self.context, self.request),
             data.get('came_from')), trusted=app.trusted_redirects)
            return
    ...
    def handleRegister(self, action):
        app = getParent(self.context, ISenditApplication)
        self.request.response.redirect('%s/@@register.html' % absoluteURL(self.context, self.request), trusted=app.trusted_redirects)
    ...
    def handleRedirect(self, action):
        app = getParent(self.context, ISenditApplication)
        self.request.response.redirect('%s/@@index.html' % absoluteURL(app, self.request), trusted=app.trusted_redirects)
    ...
    def handleForgottenPassword(self, action):
        app = getParent(self.context, ISenditApplication)
        self.request.response.redirect('%s/@@forgotten_password.html' % absoluteURL(app, self.request), trusted=app.trusted_redirects)
        ...
        if errors:
            ...
            if profile is not None:
                ...
                if mailer is not None:
                    ...
                    message_body = self.forgotten_password_template(request=self.request, context=self.context, hash=profile.activation_hash)
                    message = HTMLMessage(subject=translate(_('[%s] Forgotten password'), context=self.request) % II18n(self.context).queryAttribute('mail_subject_header', request=self.request), fromaddr='%s <%s>' % (self.context.mail_sender_name,
                     self.context.mail_sender_address), toaddr='%s <%s>' % (principal.title, principal.login), html=message_body)
                    mailer.send(self.context.mail_sender_address, (principal.login,), message.as_string())
            self.request.response.redirect('%s/@@forgotten_password_ack.html' % absoluteURL(self.context, self.request), trusted=self.context.trusted_redirects)
            return
...
class ForgottenPasswordAckView(BaseAddForm):
    ...
    help = _('A new activation message has been sent to given email address. Please check your messages and follow the instructions.')
...
class LogoutView(BaseAddForm):
    ...
    def __call__(self):
        ...
        app = getParent(self.context, ISenditApplication)
        self.request.response.redirect('%s/%s' % (absoluteURL(self.context, self.request),
         target.viewname if target is not None else '@@SelectedManagementView.html'), trusted=app.trusted_redirects)
        return ''
