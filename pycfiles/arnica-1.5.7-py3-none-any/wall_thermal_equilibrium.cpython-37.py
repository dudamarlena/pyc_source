...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/phys/wall_thermal_equilibrium.py
...
def compute_equilibrium(hot_t_ad, cold_t_ad, hot_h, cold_h, metal, ceram, ep_metal, ep_ceram):
    """ 
    ...
    def estimate_phi_and_temps(t_ceram, t_metal):
        ...
        phi = (hot_t_ad - cold_t_ad) / (1.0 / hot_h + r_th_ceram + r_th_metal + 1.0 / cold_h)
        t_ceram_hot = hot_t_ad - phi / hot_h
        t_ceram_metal = t_ceram_hot - phi * r_th_ceram
        t_metal_cold = t_ceram_metal - phi * r_th_metal
        return (
         t_ceram_hot, t_ceram_metal, t_metal_cold)

    t_ceram_hot, t_eq, t_metal_cold = estimate_phi_and_temps(t_ceram=t_eq, t_metal=t_eq)
    ncount = 0
    cvgce = 1000.0
    while cvgce > 1.0:
        ncount += 1
        t_eq_m1 = t_eq
        t_ceram_hot, t_eq, t_metal_cold = estimate_phi_and_temps(t_ceram=(0.5 * (t_ceram_hot + t_eq)),
