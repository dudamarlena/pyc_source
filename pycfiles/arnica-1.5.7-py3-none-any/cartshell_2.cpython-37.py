...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/cartshell_2.py
...
class CartShell(Shell):
    ...
    def __init__(self, n_trans, n_longi, z_range, ctrl_pts_x, ctrl_pts_y, z_min=None):
        """
    ...
    def _build_shell(self):
        ...
        *Build shell from geometric features*
        ...
        tmp_z = np.transpose(np.tile(np.linspace(min_z, max_z,
          num=(self.shape[0])), (
         self.shape[1], 1)))
        self._xyz = np.stack((tmp_x, tmp_y, tmp_z), axis=(-1))
        self._rad = np.sqrt(np.take(self._xyz, 1, -1) ** 2 + np.take(self._xyz, 2, -1) ** 2)
        self._theta = np.arctan2(np.take(self._xyz, 2, -1), np.take(self._xyz, 1, -1))
        ...
        self._du = np.pad(np.sqrt(np.diff((self._xyz[:, :, 0]), axis=1) ** 2 + np.diff((self._xyz[:, :, 1]), axis=1) ** 2), ((0, 0),
                                                                                                                             (1, 0)), 'edge')
        self._dv = np.pad(np.diff((self._xyz[:, :, 2]), axis=0), ((1, 0), (0, 0)), 'edge')
