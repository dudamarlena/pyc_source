...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/lay_and_temp_manager.py
...
import pandas as pd
from arnica.utils.string_manager import string_minus_string, replace_stars_in_string
__all__ = [
 'fetch_avbp_layouts', 'fetch_avbp_templates', 'decompact_template']
...
def fetch_avbp_layouts():
    ...
    for layout_file_name in layouts_files_list:
        layout_string = resource_string('pyavbp.visu', 'avbp_layouts/' + layout_file_name)
...
def fetch_avbp_templates():
    ...
    for template_file_name in templates_files_list:
        template_string = resource_string('pyavbp.visu', 'avbp_templates/' + template_file_name)
...
def decompact_template(template, data):
    ...
    for graph_template in template['graphs']:
        layout['graphs'] += decompact_graph_template(graph_template, dataframe)
...
def decompact_graph_template(graph_template, dataframe):
    ...
    if not curve_dyn_var:
        ...
        for data_name in dataframe.keys():
            ...
            data_name_spec = string_minus_string(curve_dyn_var_without_star, data_name)
            graph_layout = replace_stars_in_graph_template(graph_template, data_name_spec, dataframe)
            if graph_layout:
                layouts_of_graphs.append(graph_layout)
...
def replace_stars_in_graph_template(graph_template, data_name_spec, dataframe):
    ...
    for curve_template in graph_template_cp['curves']:
        success = replace_stars_in_curve_template(curve_template, data_name_spec, dataframe)
        if not success:
            break
        if curve_template is graph_template_cp['curves'][(-1)]:
            if graph_template != graph_template_cp:
                if 'title' in graph_template_cp:
                    if '*' in graph_template_cp['title']:
                        if graph_template_cp['title'].count('*') == len(data_name_spec):
                            graph_template_cp['title'] = replace_stars_in_string(graph_template_cp['title'], data_name_spec)
...
def replace_stars_in_curve_template(curve_template, data_name_spec, dataframe):
    ...
    if '*' in curve_template['var']:
        if curve_template['var'].count('*') == len(data_name_spec):
            curve_template['var'] = replace_stars_in_string(curve_template['var'], data_name_spec)
            if curve_template['var'] in dataframe.keys():
                if 'legend' in curve_template and '*' in curve_template['legend'] and curve_template['legend'].count('*') == len(data_name_spec):
                    curve_template['legend'] = replace_stars_in_string(curve_template['legend'], data_name_spec)
            else:
                success = False
        else:
            success = False
