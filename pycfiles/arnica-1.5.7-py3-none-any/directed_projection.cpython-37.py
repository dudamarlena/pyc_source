...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/directed_projection.py
...
def projection_kdtree(points, directions, point_surface, normal_surface, **kwargs):
    """
    ...
    Algorithm :
        ...
        - Gets the [1] spherical closest node for each [p] points
        - Project [p] points on plan defined [proj_axis='nml'|'dir'] either by :
            - closest node and normal closest node
            - closest node and point direction
    ...
    """
    opts = {'neigbors':1000, 
     'tol':1000.0,  'proj_axis':'nml'}
    for keyword in opts:
        if keyword in kwargs:
            opts[keyword] = kwargs[keyword]

    print('KDTree with  \n-   Surface points | ' + str(point_surface.shape[0]) + '\n- Projected points | ' + str(points.shape[0]) + '\n-        Neighbors | ' + str(opts['neigbors']))
    kdtree = spatial.cKDTree(point_surface)
    _, index = kdtree.query(points, k=1)
    if opts['proj_axis'] == 'nml':
        proj_axis = normal_surface[index]
    else:
        if opts['proj_axis'] == 'dir':
            proj_axis = directions
    projected_pts = project_points(points, proj_axis, point_surface[index][:, np.newaxis, :]).proj_pts
    projected_pts = np.squeeze(projected_pts, axis=1)
    _, indexes = kdtree.query(projected_pts, k=(opts['neigbors']))
    cyl_dists = compute_cyl_dist(projected_pts, directions, point_surface[indexes], normal_surface[indexes], opts['tol'])
...
def compute_cyl_dist(points, directions, points_surf, normals_surf, tol):
    ...
    *Compute cylindrical distances*
    ...
    cyl_dists = np.where(align < 0.0, cyl_dists, BIG * np.ones(cyl_dists.shape))
    cyl_dists = np.where(np.absolute(axi_dists) < tol, cyl_dists, BIG * np.ones(cyl_dists.shape))
    return cyl_dists
...
def project_points(points_source, normals, points_target):
    ...
    *Compute the projected points, radial dists and axial dists*
    ...
    axi_dists = np.sum((np.multiply(points_target - points_source, normals)),
      axis=(-1))
