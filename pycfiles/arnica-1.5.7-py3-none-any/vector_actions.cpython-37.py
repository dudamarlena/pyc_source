...
# [GCC 8.4.0]
# Embedded file name: /Users/duranton/Documents/CERFACS/CODES/arnica/src/arnica/utils/vector_actions.py
# Compiled at: 2020-04-24 09:54:23
# Size of source mod 2**32: 10252 bytes
...
-0.5pi=-90deg     o------>Z   0.5pi=90deg
    ...
    """
    return np.arctan2(np.take(np_ar_vect, 2, axis=(-1)), np.take(np_ar_vect, 1, axis=(-1)))
...
def rotate_vect_around_x(np_ar_vect, angle_deg):
    ...
    arr_r = np.hypot(np.take(np_ar_vect, 1, axis=(-1)), np.take(np_ar_vect, 2, axis=(-1)))
    arr_theta = yz_to_theta(np_ar_vect) + np.deg2rad(angle_deg)
...
def rotate_vect_around_axis(xyz, *tuples_rot):
    ...
    *Rotate vector around vector or series of vector*
    ...
    :param tuples_rot: List of tuple with rotation data :                        Axis array of shape (3,) axis,                        Float angle in degree

    :returns: Array of rotated xyz-coordinates of shape (n,3)
    """
    quat = Rot.from_rotvec([0, 0, 0])
    for axis, angle in tuples_rot:
        rot_vect = np.multiply(renormalize(axis), angle * np.pi / 180.0)
...
def dilate_vect_around_x(azimuth, np_ar_vect, angle_deg_init=None, angle_deg_targ=360):
    ...
    result = rotate_vect_around_x(np_ar_vect, azimuth * (dilate_factor - 1))
    return result
...
def cart_to_cyl(vects_xyz):
    ...
    *Transform vects from xyz-system to xrtheta-system*
    ...
    np_r = np.sqrt(np.take(vects_xyz, 1, -1) ** 2 + np.take(vects_xyz, 2, -1) ** 2)
...
def clip_by_bounds(points_coord, bounds_dict, keep='in', return_mask=False):
    ...
    *Clip a cloud by keeping only or removing a bounded region*
    ...
    If returns=False, returns the mask of boolean than can be applied    on other arrays
...
def vect_to_quat(vect_targ, vect_source):
    ...
    *Generate a quaternion from two vectors*
    ...
    A quaternion is a rotation object. From two vectors,    the rotation angle and the rotation axis are computed.
    ...
    prod_scal = np.clip(np.sum((vect_targ * vect_source), axis=(-1)), -1, 1)
...
def make_radial_vect(coord, vects):
    ...
    *Recalibrate vectors to make them radial.*
    ...
    """
    tol = 10
    diff_angle = (yz_to_theta(coord) - yz_to_theta(vects)) * 180.0 / np.pi
    diff_angle = np.where(diff_angle > 90.0, diff_angle - 180.0, diff_angle)
    diff_angle = np.where(diff_angle < -90.0, diff_angle + 180.0, diff_angle)
...
def mask_cloud(np_ar_xyz, axis, support):
    ...
    if axis not in possible_axes:
        raise IOError('axis ' + axis + 'not among possible axes ' + ';'.join(possible_axes))
    if axis == 'x':
        mask = true_between(np.take(np_ar_xyz, 0, axis=(-1)), support[0], support[1])
    if axis == 'y':
        mask = true_between(np.take(np_ar_xyz, 1, axis=(-1)), support[0], support[1])
    if axis == 'z':
        mask = true_between(np.take(np_ar_xyz, 2, axis=(-1)), support[0], support[1])
    if axis == 'r':
        mask = true_between(np.hypot(np.take(np_ar_xyz, 1, axis=(-1)), np.take(np_ar_xyz, 2, axis=(-1))), support[0], support[1])
    if axis == 'theta':
        mask = true_between(np.rad2deg(yz_to_theta(np_ar_xyz)), support[0], support[1])
    return mask